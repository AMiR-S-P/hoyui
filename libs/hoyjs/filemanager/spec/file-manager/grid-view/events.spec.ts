...
describe('FileManager control Grid view', () => {
  describe('events testing', () => {
    ...
    it('for fileOpen', (done) => {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          expect(i).toBe(1);
          feObj.element
            .getElementsByClassName('e-address-list-item')[0]
            .click();
          this.request = jasmine.Ajax.requests.mostRecent();
          this.request.respondWith({
            status: 200,
    ...
    it('for fileSelection with cancel', (done: Function) => {
      feObj = new FileManager({
        ...
        showThumbnail: false,
        fileSelection: (args: FileSelectionEventArgs) => {
          i++;
          args.cancel = true;
        },
      });
      ...
      setTimeout(function () {
        ...
        expect(i).toEqual(1);
        expect(
          rows[0].querySelector('.e-frame').classList.contains('e-check')
        ).toBe(false);
    ...
    it('for toolbarCreate', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        feObj.toolbarCreate = (args: ToolbarCreateEventArgs) => {
          clickFn();
          let ind: number = (<FileManager>feObj).getToolbarItemIndex(
            'Custom tool'
          );
          if (ind !== -1) {
        ...
        expect(
          (<FileManager>(
            feObj
          )).toolbarModule.toolbarObj.element.querySelectorAll('.e-custom')
            .length
        ).toBe(1);
        done();
      }, 500);
    ...
    it('for uploadListCreate', () => {
      feObj = new FileManager({
        ...
        uploadListCreate: (args: UploadListCreateArgs) => {
          let ele: HTMLElement = createElement('span', {
            className: 'e-fm-upload-icon',
          });
      ...
      expect(document.querySelector('.e-file-status').textContent).toBe(
        'File type is not allowed'
      );
      expect(i).toEqual(1);
      expect(
        feObj.uploadDialogObj.element.querySelectorAll('.e-fm-upload-icon')
          .length
      ).toBe(1);
    });
    ...
    it('for beforePopupOpen with preventing file extension', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        (<HTMLElement>(
          document.getElementById('file_dialog').querySelectorAll('.e-btn')[0]
        )).click();
        feObj.detailsviewModule.gridObj.selectRows([2]);
        items[0].click();
        expect(
        ...
        expect(
          document.getElementsByClassName('e-fe-error')[0].textContent
        ).not.toEqual('');
        (<HTMLInputElement>document.getElementById('rename')).value = 'New.';
        (<HTMLElement>(
          document.getElementById('file_dialog').querySelectorAll('.e-btn')[1]
        ...
        setTimeout(function () {
          ...
          expect(gridLi[1].querySelector('.e-fe-grid-name').innerText).toBe(
            '2'
          );
          done();
        }, 500);
  ...
  describe('events testing popup event', () => {
    ...
    it('for BeforeClose cancel testing', () => {
      name = 'Create Folder';
      feObj.beforePopupClose = (args: BeforePopupOpenCloseEventArgs) => {
        args.cancel = true;
        k++;
      };
      let item: any = document.getElementById('file_tb_newfolder');
    ...
    it('for upload and for Upload retry', (done) => {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          expect((<any>feObj.extDialogObj).btnObj[1].element.textContent).toBe(
            'Replace'
          );
          ...
          setTimeout(function () {
            name = 'Upload';
            (<HTMLElement>(
              document.querySelector('.e-dlg-closeicon-btn')
            )).click();
            expect(
              (<any>feObj.detailsviewModule.gridObj.contentModule).rows.length
            ).toBe(6);
    ...
    it('for details with OK button', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        const okButton: any = document.querySelector(
          '#file_dialog .e-footer-content button'
        );
        okButton.click();
        expect(i).toBe(2);
        expect(j).toBe(1);
