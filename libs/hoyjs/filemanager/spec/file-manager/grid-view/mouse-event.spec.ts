...
import { extend } from '@hoyui/grids';
...
describe('FileManager control Grid view', () => {
  describe('mouse event testing', () => {
    ...
    it('mouse click on empty folder', (done) => {
      ...
      let args = {
        rowData: {
          name: 'Food',
          size: 0,
          dateModified: '10/15/2018 5:39:03 PM',
          dateCreated: '10/15/2018 5:39:03 PM',
          hasChild: true,
          isFile: false,
          type: '',
          filterPath: '\\',
      ...
      setTimeout(function () {
        ...
        (<HTMLElement>(
          (<any>feObj.detailsviewModule.gridObj.contentModule).contentPanel
            .firstElementChild
        )).click();
        done();
      }, 500);
    ...
    it('navigation pane folder open cancel testing', () => {
      ...
      var treeObj: any = (document.getElementById('file_tree') as any)
        .ej2_instances[0];
      let li: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
        document.getElementById('file_tree').querySelectorAll('li')
      );
    ...
    it('mouse click on grid element with row selection', () => {
      var gridObj: any = (document.getElementById('file') as any)
        .ej2_instances[0];
      feObj.detailsviewModule.gridObj.selectRows([2]);
      mouseEventArgs.target = <HTMLElement>(
        (<any>(
          feObj.detailsviewModule.gridObj.element.querySelectorAll(
            '.e-content'
          )[0]
        ))
      );
      ...
      expect(
        feObj.detailsviewModule.gridObj.getSelectedRowCellIndexes().length
      ).toBe(0);
      expect(
        feObj.detailsviewModule.gridObj.element
    ...
    it('mouse click on TreeView text', (done: Function) => {
      ...
      treeObj.touchClickObj.tap(tapEvent);
      let li1: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
        document.getElementById('file_tree').querySelectorAll('li')
      );
      expect((li[0] as Element).classList.contains('e-active')).toBe(true);
      ...
      setTimeout(function () {
        expect(
          document
            ...
            .classList.contains('e-spin-show')
        ).toEqual(false);
        expect(
          document
            .getElementById('file_grid')
        ...
        let li2: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
          document.getElementById('file_tree').querySelectorAll('li')
        );
        ...
        setTimeout(() => {
          let li3: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
            document.getElementById('file_tree').querySelectorAll('li')
          );
          ...
          setTimeout(function () {
            expect(
              (
                document
                  .getElementById('file_dialog')
                  .querySelector('#newname') as any
              ).value
            ).toEqual('');
            let ele: HTMLInputElement = document
              .getElementById('file_dialog')
              .querySelector('#newname') as HTMLInputElement;
            ele.value = 'New folder';
            (
              document
                .getElementById('file_dialog')
                .querySelectorAll('.e-btn')[1] as HTMLElement
            ).click();
            this.request = jasmine.Ajax.requests.mostRecent();
            this.request.respondWith({
              status: 200,
            ...
            setTimeout(function () {
              ...
              expect(
                document.getElementById('file_grid').querySelectorAll('.e-row')
                  .length
              ).toEqual(1);
              mouseEventArgs.target = li[0].querySelector('.e-fullrow');
              treeObj.touchClickObj.tap(tapEvent);
            ...
            setTimeout(function () {
              ...
              setTimeout(function () {
                expect(
                  document
                    .getElementById('file_dialog')
                    .querySelector('.e-dlg-content').textContent
                ).toBe('Cannot create a file when that file already exists');
                ...
                setTimeout(function () {
                  ...
                  setTimeout(function () {
                    expect(
                      document
                        .getElementById('file_extn_dialog')
                        .querySelector('.e-dlg-content').textContent
                    ).toBeTruthy();
                    (
                      document
                        .getElementById('file_extn_dialog')
                        .querySelectorAll('.e-btn')[2] as HTMLElement
                    ).click();
                    setTimeout(function () {
                      items[0].click();
                      setTimeout(function () {
    ...
    it('File name validation in rename', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          expect(
            inputValue.parentElement.nextElementSibling.textContent
          ).toEqual(
            'The file or folder name "My Folder*" contains invalid characters. Please use a different name. Valid file or folder names cannot end with a dot or space, and cannot contain any of the following characters: \\/:*?"<>|'
          );
          inputValue.onkeyup({
            bubbles: true,
            cancelable: true,
    ...
    it('for upload element', () => {
      ...
      expect(feObj.element.querySelectorAll('.e-toolbar-item').length).toEqual(
        toolbarItems3.length
      );
      expect(feObj.element.querySelectorAll('.e-toolbar-item').length).toEqual(
        16
      );
      ...
      let uploadObj: any = document.querySelector(
        '#' + feObj.element.id + '_upload'
      );
    ...
    it('double click on folder', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        (obj as any).dblClickHandler({
          target: obj.element.querySelectorAll('.e-row')[3].firstElementChild,
        });
        this.request = jasmine.Ajax.requests.mostRecent();
        this.request.respondWith({
          status: 200,
    ...
    it('mouse click on file', () => {
      ...
      expect(
        li[4].querySelector('.e-frame').classList.contains('e-check')
      ).toBe(false);
      feObj.detailsviewModule.gridObj.selectRows([4]);
      let nli: any = document
        .getElementById('file_grid')
        .querySelectorAll('.e-row');
      expect(nli[4].getAttribute('aria-selected')).toEqual('true');
      expect(
        nli[4].querySelector('.e-frame').classList.contains('e-check')
      ).toBe(true);
    });
    it('mouse double click on file', () => {
      ...
      let args = {
        rowData: {
          name: '1.png',
          size: 49792,
          dateModified: '1/3/2018 4:07:28 PM',
          dateCreated: '10/17/2018 12:57:59 PM',
          hasChild: false,
          isFile: true,
          type: '.png',
        },
    ...
    it('mouse click on header cell of name', () => {
      ...
      expect(li.length).toEqual(5);
      expect(
        li[4].querySelectorAll('.e-rowcell')[2].querySelector('.e-fe-text')
          .textContent
      ).toBe('1.png');
      (
        feObj.element.querySelectorAll('.e-headercell')[2] as HTMLElement
      ).click();
      let nli: any = document
        .getElementById('file_grid')
        .querySelectorAll('.e-row');
      ...
      expect(
        nli[4].querySelectorAll('.e-rowcell')[2].querySelector('.e-fe-text')
          .textContent
      ).toBe('1.png');
    });
    it('check box selection and allow multi selection', (done: Function) => {
      ...
      setTimeout(function () {
        let obj = feObj.detailsviewModule.gridObj as any;
        expect(
          obj.getHeaderContent().querySelectorAll('.e-checkselectall').length
        ).toBe(1);
        expect(obj.getContent().querySelectorAll('.e-checkselect').length).toBe(
          5
        );
        //check box selection
        var rows = obj.getRows();
        var chkAll = obj.element.querySelector('.e-checkselectall')
    ...
    it('check box selection and allow multi selection false', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        setTimeout(() => {
          expect(
            JSON.parse(
              JSON.stringify(feObj.detailsviewModule.gridObj.columns[0])
            ).type
          ).not.toBe('checkbox');
          (
            feObj.detailsviewModule.gridObj
              .getRows()[0]
    ...
    it('for sortby element click testing', (done) => {
      expect(feObj.toolbarSettings.visible).toEqual(true);
      expect(feObj.toolbarSettings.items).toEqual(toolbarItems3);
      let sortbyObj: any = feObj.toolbarModule.toolbarObj.element.querySelector(
        '#' + feObj.element.id + '_tb_sortby'
      );
      sortbyObj.click();
      setTimeout(function () {
        done();
    ...
    it('for sortby name element Descending testing', (done) => {
      ...
      sortbyObj.click();
      setTimeout(function () {
        sortbyObj.ej2_instances[0].dropDown.element
          .querySelectorAll('.e-item')[5]
          .click();
        expect(
          feObj.detailsviewModule.gridObj.sortSettings.columns[0].field
        ).toBe('name');
        expect(
          feObj.detailsviewModule.gridObj.sortSettings.columns[0].direction
        ).toBe('Descending');
        done();
      }, 200);
    });
    ...
    it('mouse click on the grid header for sorting testing', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        expect(
          rows[0].querySelectorAll('td')[2].querySelector('.e-fe-text')
            .textContent
        ).toBe('Nature');
        (document.querySelectorAll('.e-headercell')[2] as any).click();
        setTimeout(function () {
          ...
          expect(
            rows1[0].querySelectorAll('td')[2].querySelector('.e-fe-text')
              .textContent
          ).toBe('Documents');
          var treeObj: any = (document.getElementById('file_tree') as any)
            .ej2_instances[0];
          ...
          setTimeout(function () {
            ...
            expect(
              rows2[0].querySelectorAll('td')[2].querySelector('.e-fe-text')
                .textContent
            ).toBe('Documents');
            expect(
              rows2[1].querySelectorAll('td')[2].querySelector('.e-fe-text')
                .textContent
            ).toBe('Employees');
            expect(
              rows2[5].querySelectorAll('td')[2].querySelector('.e-fe-text')
                .textContent
            ).toBe('emp.png');
            done();
          }, 500);
    ...
    it('mouse click on the layout switch icon', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          setTimeout(function () {
            let li1: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
              largeIcons.querySelectorAll('li')
            );
  ...
  describe('grid settings testing', () => {
    ...
    it('columnResizing testing', (done: Function) => {
      ...
      expect(
        (feObj.detailsViewSettings.columns[0] as any).headerTextAlign
      ).toEqual('Left');
      expect((feObj.detailsViewSettings.columns[0] as any).minWidth).toEqual(
        120
      );
      expect((feObj.detailsViewSettings.columns[0] as any).maxWidth).toEqual(
        300
      );
      expect((feObj.detailsViewSettings.columns[2] as any).field).toEqual(
        'size'
      );
      expect((feObj.detailsViewSettings.columns[1] as any).textAlign).toEqual(
        'Left'
      );
      expect(
        document.getElementById('file_grid').classList.contains('e-grid')
      ).toEqual(true);
      let headerText: any = document
        .getElementById('file_grid')
        .querySelectorAll('.e-headertext');
      expect(headerText[3].innerText).toBe('Has Children');
      expect(
        document
          .getElementById('file_grid')
          .classList.contains('e-resize-lines')
      ).toEqual(true);
      expect(
        document.getElementById('file_grid').querySelectorAll('.e-rhandler')
          .length
      ).toBe(4);
      done();
    });
  ...
  describe('Grid viewtesting', () => {
    ...
    it('script error for grid header sort with no sortby in toolbar', () => {
      ...
      (<HTMLElement>(
        document.getElementsByClassName('e-sortfilterdiv')[2]
      )).click();
  ...
  describe('Grid viewtesting', () => {
    ...
    it('title attribute testing', () => {
      ...
      let Li: any =
        feObj.navigationpaneModule.treeObj.element.querySelectorAll('li');
      for (let i: number = 0; i < Li.length; i++) {
        expect(Li[i].getAttribute('title')).toBe(
          Li[i].querySelector('.e-list-text').textContent
        );
      }
    });
    it('select item maintain by sortby testing', (done: Function) => {
      feObj.detailsviewModule.gridObj.selectRows([0]);
      let li: any = feObj.detailsviewModule.gridObj
        .getRowByIndex(0)
      ...
      sortbyObj.ej2_instances[0].dropDown.element
        .querySelectorAll('.e-item')[2]
        .click();
      let li1: any = feObj.detailsviewModule.gridObj
        .getRowByIndex(0)
        .getElementsByTagName('td')[2];
...
describe('Default functionality testing', () => {
  ...
  it('Delete operation', (done: Function) => {
    ...
    setTimeout(function () {
      let okBtn: any = document.getElementsByClassName(
      ...
      setTimeout(function () {
        let activeLi: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
          document.getElementById('file_tree').querySelectorAll('li')
        );
        expect(
          activeLi[0].querySelectorAll(
            '[data-uid="filecontent/employees/documents/"]'
          ).length
        ).toBe(0);
        done();
      }, 500);
