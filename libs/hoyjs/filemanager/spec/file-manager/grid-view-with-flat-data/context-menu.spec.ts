...
import { createElement, isNullOrUndefined, select } from '@hoyui/base';
import { MenuOpenEventArgs, MenuClickEventArgs } from '../../../src';
import { ContextMenu } from '@hoyui/navigations';
...
describe('FileManager control Grid view', () => {
  let flatData2: any = [
    ...
    },
    {
      ...
      type: '.png',
      imgUrl:
        'https://ej2.syncfusion.com/demos/src/treeview/images/employees/7.png',
    },
  ];
  describe('context menu testing', () => {
    ...
    it('setmodel context menu testing', () => {
      ...
      expect(isNullOrUndefined((feObj as FileManager).contextmenuModule)).toBe(
        true
      );
    ...
    it('Opening context menu in empty space area related test case', () => {
      let el: any = document.getElementById(feObj.element.id + '_contextmenu');
      let gridEle: any = feObj.detailsviewModule.element.querySelector(
    ...
    it('folder context menu in tree view open item testing', () => {
      let el: any = document.getElementById(feObj.element.id + '_contextmenu');
      let Li: Element =
        feObj.navigationpaneModule.treeObj.element.querySelectorAll('li')[2];
      let sourceElement: any = el.ej2_instances[0];
      let evt = document.createEvent('MouseEvents');
      evt.initEvent('contextmenu', true, true);
      Li.dispatchEvent(evt);
    ...
    it('mouse click on refresh item', (done) => {
      ...
      document
        ...
        .remove();
      document
        .getElementsByClassName('e-addressbar-ul')[0]
        .querySelector('li')
        .remove();
      ...
      expect(
        tr[0].querySelector('.e-frame').classList.contains('e-check')
      ).toBe(true);
      expect(tr[1].getAttribute('aria-selected')).toEqual('true');
      expect(
        tr[1].querySelector('.e-frame').classList.contains('e-check')
      ).toBe(true);
      let grid: Element = (<any>feObj.detailsviewModule.gridObj.contentModule)
        .contentPanel.children[0];
      let evt = document.createEvent('MouseEvents');
      evt.initEvent('contextmenu', true, true);
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          expect(
            ntr[1].querySelector('.e-frame').classList.contains('e-check')
          ).toBe(true);
          expect(ntr[2].getAttribute('aria-selected')).toEqual('true');
          expect(
            ntr[2].querySelector('.e-frame').classList.contains('e-check')
          ).toBe(true);
          done();
        }, 500);
      }, 100);
  ...
  describe('for Grid View', () => {
    ...
    it('Grid view context menu open process testing', (done: Function) => {
      let el: any = document.getElementById(feObj.element.id + '_contextmenu');
      feObj.detailsviewModule.gridObj.selectRows([0]);
      let li = feObj.detailsviewModule.gridObj
        .getRowByIndex(0)
        .getElementsByTagName('td')[2];
      ...
      sourceElement.element.querySelectorAll('li')[0].click();
      setTimeout(function () {
        let li1: any = document
          .getElementById('file_grid_content_table')
          .querySelectorAll('tr');
    ...
    it('Treeview context menu details option testing', () => {
      let ntr: any = document
        .getElementById('file_grid')
        .querySelectorAll('.e-rowcell.e-fe-grid-name');
      let treeObj: any = (document.getElementById('file_tree') as any)
        .ej2_instances[0];
      let el: any = document.getElementById(feObj.element.id + '_contextmenu');
      let li: any =
      ...
      expect(
        (<any>document.querySelectorAll('.e-fe-value')[2]).textContent
      ).toBe('Files/Downloads');
    });
    it('Treeview context menu upload process when select grid view item', () => {
      ...
      expect(li.querySelector('.e-fe-text').textContent).toBe('Base');
      let li1: any =
        feObj.navigationpaneModule.treeObj.element.querySelectorAll('li')[3];
    ...
    it('Toolbar context menu open process testing', () => {
      let toolbarObj: any = (document.getElementById('file_toolbar') as any)
        .ej2_instances[0];
      let el: any = document.getElementById(feObj.element.id + '_contextmenu');
      let sourceElement: any = el.ej2_instances[0];
      let evt = document.createEvent('MouseEvents');
    ...
    it('folder context menu with open', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        expect(
          document
            .getElementsByClassName('e-addressbar-ul')[0]
            .querySelectorAll('li')[1].textContent
        ).toBe('Documents');
    ...
    it('layout context menu with view', (done: Function) => {
      ...
      expect(document.getElementById('file_grid').offsetWidth != 0).toEqual(
        true
      );
      expect(document.getElementById('file_grid').offsetWidth != 0).toEqual(
        true
      ...
      expect(
        document.getElementById('file_largeicons').offsetHeight == 0
      ).toEqual(true);
      expect(
        document.getElementById('file_largeicons').offsetHeight == 0
      ).toEqual(true);
    ...
    it('folder context menu based on target - menuType', () => {
      ...
      expect(
        document
          .getElementById(feObj.element.id + '_contextmenu')
    ...
    it('layout Contextmenu - custom menu items - testing submenu', () => {
      ...
      expect(menuItems).toBe(13);
      let menuItem: HTMLElement = <HTMLElement>(
        document.querySelectorAll('#file_contextmenu .e-menu-item')[10]
      );
      mouseEventArgs.target = menuItem;
      mouseEventArgs.type = 'mouseover';
      sourceElement.moverHandler(mouseEventArgs);
      expect(
        document.getElementById('item2').classList.contains('e-disabled')
      ).toBe(true);
    ...
    it('file Contextmenu - custom menu items - testing submenu', () => {
      ...
      let menuItem: HTMLElement = <HTMLElement>(
        document.querySelectorAll('#file_contextmenu .e-menu-item')[0]
      );
      mouseEventArgs.target = menuItem;
      mouseEventArgs.type = 'mouseover';
      sourceElement.moverHandler(mouseEventArgs);
        ...
        setTimeout(function () {
          setTimeout(function () {
            ...
            feObj.menuClick = (args: MenuClickEventArgs) => {
              i++;
              expect(
                (<any>args.fileDetails[0]).name === 'textDocument.doc'
              ).toBe(true);
            };
            let el: any = document.getElementById(
              feObj.element.id + '_contextmenu'
            ...
            Li.dispatchEvent(evt);
            expect(
              sourceElement.element.querySelectorAll('li')[0].innerText
            ).toBe('Open');
            expect(
              sourceElement.element
                .querySelectorAll('li')[0]
