...
import { addClass, L10n } from '@hoyui/base';
...
export class ColorPicker {
  ...
  private render(): void {
    ...
    let input: HTMLInputElement = this.parent.createElement('input', {
      attrs: { type: 'color' },
    }) as HTMLInputElement;
    const tileRenderHandler: (args: PaletteTileEventArgs) => void = (
      args: PaletteTileEventArgs
    ): void => {
    ...
    this.fontColorPicker = new ColorPickerComponent({
      ...
      beforeModeSwitch: (args: ModeSwitchEventArgs): void =>
        this.beforeModeSwitch(this.fontColorPicker, args),
      change: (args: ColorPickerEventArgs): void => {
        const color: string = this.fontColorPicker.getValue(
        ...
        if (eventArgs.cancel) {
          this.fontColorPicker.setProperties(
            {
              value: this.fontColorPicker.getValue(
                args.previousValue.rgba,
                'HEXA'
              ),
            },
            true
          ...
          this.updateSelectedColor(
            eventArgs.style.color,
            this.fontColorPicker.element,
            'TextColor'
          );
        }
      },
    ...
    addClass(
      [input.nextElementSibling.getElementsByClassName('e-selected-color')[0]],
    ...
    this.filColorPicker = new ColorPickerComponent({
      ...
      beforeModeSwitch: (args: ModeSwitchEventArgs): void =>
        this.beforeModeSwitch(this.filColorPicker, args),
      change: (args: ColorPickerEventArgs): void => {
        const color: string = this.filColorPicker.getValue(
          args.currentValue.rgba
        );
        const eventArgs: SetCellFormatArgs = {
        ...
        this.parent.notify(setCellFormat, eventArgs);
        if (eventArgs.cancel) {
          this.filColorPicker.setProperties(
            {
              value: this.filColorPicker.getValue(
                args.previousValue.rgba,
                'HEXA'
              ),
            },
          ...
          this.updateSelectedColor(
            eventArgs.style.backgroundColor,
            this.filColorPicker.element,
            'FillColor'
          );
        }
      },
  ...
  private updateSelectedColor(
    ...
    const localeText: string = this.parent.serviceLocator
      .getService<L10n>(locale)
      .getConstant(name);
    if (isCreated) {
      ele.parentElement
        .querySelector('.e-dropdown-btn')
        .setAttribute('aria-label', localeText);
    }
    const primaryBtn: HTMLElement = ele.parentElement.querySelector(
      '.e-split-colorpicker'
    );
    primaryBtn.setAttribute('aria-label', `${localeText} ${color}`);
    (primaryBtn.firstElementChild as HTMLElement).style.borderBottomColor =
      color;
  }
  ...
  private beforeModeSwitch(
    inst: ColorPickerComponent,
    args: ModeSwitchEventArgs
  ): void {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    if (args.mode === 'Picker') {
      inst.showButtons = true;
      inst.dataBind();
      (args.element.querySelector('.e-apply') as HTMLElement).title =
        l10n.getConstant('Apply');
      (args.element.querySelector('.e-cancel') as HTMLElement).title =
        l10n.getConstant('Cancel');
      (args.element.querySelector('.e-mode-switch-btn') as HTMLElement).title =
        l10n.getConstant('StandardColors');
    } else {
      inst.showButtons = false;
      inst.dataBind();
      (args.element.querySelector('.e-mode-switch-btn') as HTMLElement).title =
        l10n.getConstant('MoreColors');
    }
  }
