...
import { ItemModel } from '@hoyui/splitbuttons';
import { isCollide, OffsetPosition, calculatePosition } from '@hoyui/popups';
import {
  rippleEffect,
  L10n,
  closest,
...
export class SheetTabs {
  ...
  private createSheetTabs(): void {
    ...
    const addBtn: HTMLElement = this.parent.createElement('button', {
      className:
        'e-add-sheet-tab e-btn e-css e-flat e-icon-btn' +
        (this.parent.allowInsert ? '' : ' e-disabled'),
    ...
    const ddb: HTMLElement = this.parent.createElement('button', {
      attrs: { title: l10n.getConstant('ListAllSheets'), type: 'button' },
    });
    panel.appendChild(ddb);
    this.parent.element.appendChild(panel);
    const items: { tabItems: TabItemModel[]; ddbItems: ItemModel[] } =
      this.getSheetTabItems();
    this.dropDownInstance = new DropDownButton({
      ...
      beforeItemRender: (args: MenuEventArgs): void => {
        const sheet: SheetModel =
          this.parent.sheets[this.dropDownInstance.items.indexOf(args.item)];
      ...
      select: (args: MenuEventArgs): void =>
        this.updateSheetTab({
          idx: this.dropDownInstance.items.indexOf(args.item),
        }),
      beforeOpen: (args: BeforeOpenCloseMenuEventArgs): void =>
        this.beforeOpenHandler(
          this.dropDownInstance,
          args.element,
          l10n.getConstant('ListAllSheets')
        ),
      open: (args: OpenCloseMenuEventArgs): void =>
        this.openHandler(this.dropDownInstance, args.element, 'left'),
    ...
    const sheetTab: HTMLElement = this.parent.createElement('div', {
    ...
    this.tabInstance = new Tab({
      ...
      created: (): void => {
        const tBarItems: HTMLElement =
          this.tabInstance.element.querySelector('.e-toolbar-items');
  ...
  private goToSheet(
    ...
    if (this.parent.isEdit) {
      ...
      const editArgs: { action: string; editedValue: string } = {
        action: 'getCurrentEditValue',
        editedValue: '',
      };
      this.parent.notify(editOperation, editArgs);
      let formula: boolean = editArgs.editedValue
        ? checkIsFormula(editArgs.editedValue, true)
        : false;
      if (
        !formula &&
        selection &&
        ...
        (selection.focusNode as Element).classList.contains(
          'e-formula-bar-panel'
        )
      ) {
        formula = checkIsFormula(
          (
  ...
  private updateDropDownItems(curIdx: number, prevIdx?: number): void {
    if (prevIdx > -1) {
      this.dropDownInstance.items[prevIdx as number].iconCss = '';
    }
    this.dropDownInstance.items[curIdx as number].iconCss =
      'e-selected-icon e-icons';
  ...
  private beforeOpenHandler(
    ...
    const actualHeight: number =
      parseInt(getComputedStyle(element.firstElementChild).height, 10) *
  ...
  private openHandler(
    ...
    const wrapper: HTMLElement = element.parentElement;
    let height: number;
    const collide: string[] = isCollide(wrapper);
    if (collide.indexOf('bottom') === -1) {
      height =
        element.style.overflowY === 'auto'
          ? this.parent.viewport.height
          : wrapper.getBoundingClientRect().height;
      const offset: OffsetPosition = calculatePosition(
        instance.element,
        positionX,
        'top'
      );
  ...
  private getSheetTabItems(): {
    ...
    this.parent.sheets.forEach((sheet: SheetModel, index: number) => {
      sheetName = getSheetName(this.parent as Workbook, index)
        ...
        .replace(/>/g, '&gt;');
      tabItems.push({
        header: { text: sheetName },
        visible: sheet.state === 'Visible',
      });
      ddbItems.push({
        text: sheetName,
        iconCss:
          index === this.parent.activeSheetIndex
            ? 'e-selected-icon e-icons'
            : '',
      });
    });
  ...
  private insertSheetTab(args: {
    ...
    if (
      ...
      this.dropDownInstance.items[args.startIdx].iconCss =
        'e-selected-icon e-icons';
    }
    this.dropDownInstance.setProperties(
      { items: this.dropDownInstance.items },
  ...
  private updateSheetTab(args: {
    idx: number;
    name?: string;
    preventDataBind?: boolean;
  }): void {
  ...
  private showSheet(args: {
    sheetIndex: number;
    triggerEvent?: boolean;
  }): void {
    this.parent.setSheetPropertyOnMute(
      this.parent.sheets[args.sheetIndex],
      'state',
      'Visible'
    );
    ...
    if (args.triggerEvent) {
      this.parent.notify(completeAction, {
        action: 'showSheet',
        eventArgs: { sheetIndex: args.sheetIndex },
      });
    }
  }
  ...
  private switchSheetTab(args: BeforeOpenCloseMenuEventArgs): void {
    ...
    if (
      args.element.classList.contains('e-contextmenu') &&
      args.items[0] &&
      ...
      if (
        !this.parent.allowDelete &&
        disableItems.indexOf(`${id}_delete_sheet`) > -1
      ) {
  ...
  private renameSheetTab(): void {
    let target: Element = this.tabInstance.element.querySelector(
    ...
    if (target) {
      ...
      const args: {
        eventArgs: { name: string; index: number };
        action: string;
        cancel: boolean;
      } = {
        eventArgs: {
      ...
      EventHandler.remove(
        closest(target, '.e-toolbar-items'),
        'dblclick',
        this.renameSheetTab
      );
    }
  ...
  private renameInputFocusOut(e: KeyboardEvent | MouseEvent): void {
    ...
    target = document.getElementById(
      this.parent.element.id + '_rename_input'
    ) as HTMLInputElement;
    ...
    if (value) {
      ...
      if (!value.match(new RegExp('.*[\\[\\]\\*\\\\/\\?].*'))) {
        if (this.parent.sheets[idx as number].name !== value) {
          for (
            ...
            if (
              ...
              this.parent.sheets[i as number].name.toLowerCase() ===
                value.toLowerCase()
            ) {
              this.showRenameDialog(
                target,
                l10n.getConstant('SheetRenameAlreadyExistsAlert')
              );
              return;
            }
          }
        ...
        if (this.parent.sheets[idx as number].name !== value) {
          this.parent.setSheetPropertyOnMute(
            this.parent.sheets[idx as number],
            'name',
            value
          );
        ...
        this.showRenameDialog(
          target,
          l10n.getConstant('SheetRenameInvalidAlert')
        );
      }
    } else {
    ...
    this.parent.notify(completeAction, {
      eventArgs: { index: this.parent.getActiveSheet().id, value: value },
      action: 'renameSheet',
    });
  }
  ...
  private updateSheetName(args: {
    value: string;
    idx: number;
    items?: Element;
  }): void {
    const pName: string = (
      this.tabInstance.items[args.idx].header.text as string
    )
      .replace(/&lt;/g, '<')
      .replace(/&gt;/g, '>');
    ...
    const sheetTabText: HTMLElement = args.items
      .querySelectorAll('.e-toolbar-item')
      [args.idx].querySelector('.e-tab-text');
  ...
  private hideSheet(args: {
    sheetIndex: number;
    triggerEvent?: boolean;
  }): void {
    const actionArgs: {
      action: string;
      eventArgs: { cancel?: boolean; sheetIndex?: number };
    } = {
      action: 'hideSheet',
      eventArgs: { sheetIndex: args.sheetIndex, cancel: false },
    };
    if (args.triggerEvent) {
  ...
  private removeRenameInput(target: HTMLInputElement): Element {
    const textEle: HTMLElement =
      target.parentElement.querySelector('.e-tab-text');
  ...
  private showRenameDialog(target: HTMLInputElement, content: string): void {
    ...
    dialogInst.show({
      ...
      beforeOpen: (args: BeforeOpenEventArgs): void => {
        ...
        args.element
          .querySelector('.e-footer-content .e-primary')
          .setAttribute(
            'aria-label',
            `${content} ${this.parent.serviceLocator
    ...
    if (this.skipHiddenSheets() > 1) {
      const sheet: SheetModel = args.sheetName
        ? getSheet(
            ...
            getSheetIndex(this.parent as Workbook, args.sheetName)
          )
        : this.parent.getActiveSheet();
      const sheetIndex: number = isNullOrUndefined(args.index)
        ? getSheetIndex(this.parent as Workbook, sheet.name)
        : args.index;
      ...
      const isDataAvail: boolean =
        sheet.rows && sheet.rows.length
          ? sheet.rows.length === 1
            ? sheet.rows[0].cells && sheet.rows[0].cells.length
              ? true
              : false
            : true
          : false;
          ...
          dialogInst.show({
            ...
            beforeOpen: (args: BeforeOpenEventArgs): void => {
              ...
              args.element
                ...
                .setAttribute(
                  'aria-label',
                  `${l10n.getConstant('DeleteSheetAlert')} ${l10n.getConstant(
                    'Ok'
                  )}`
                );
              focus(this.parent.element);
  ...
  private destroySheet(sheetIndex?: number): void {
    const activeSheetIdx: number = isNullOrUndefined(sheetIndex)
    ...
    const activeIndex: number = this.parent.skipHiddenSheets(
      this.tabInstance.selectedItem
    );
  ...
  private showAggregate(): void {
    if (
      isSingleCell(getRangeIndexes(this.parent.getActiveSheet().selectedRange))
    ) {
      return;
    }
    const eventArgs: AggregateArgs = {
      Count: 0,
      Sum: '0',
      Avg: '0',
      Min: '0',
    ...
    if (eventArgs.Count > 1) {
      this.aggregateContent = eventArgs.countOnly
        ? 'Count'
        : this.selaggregateCnt;
      ...
      const btnClass: string = eventArgs.countOnly
        ? 'e-aggregate-list e-flat e-aggregate-list-countonly e-caret-hide'
      ...
      if (!this.aggregateDropDown) {
        ...
        document
          .getElementById(`${this.parent.element.id}_sheet_tab_panel`)
          .appendChild(aggregateEle);
        this.aggregateDropDown = new DropDownButton({
          ...
          open: (args: OpenCloseMenuEventArgs): void =>
            this.openHandler(this.aggregateDropDown, args.element, 'right'),
  ...
  private getAggregateItems(args: AggregateArgs): ItemModel[] {
    ...
    Object.keys(args).forEach((key: string): void => {
      ...
      if (argsKey !== aggregateComputation) {
        text = `${key}: ${argsKey}`;
        iconCss =
          key === this.aggregateContent ? 'e-selected-icon e-icons' : '';
  ...
  public destroy(): void {
    ...
    const ele: HTMLElement = document.getElementById(
      this.parent.element.id + '_sheet_tab_panel'
    );
    if (ele) {
      remove(ele);
    }
