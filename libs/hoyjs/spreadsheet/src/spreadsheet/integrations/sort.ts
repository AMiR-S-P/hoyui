...
import { ListView } from '@hoyui/lists';
...
export class Sort {
  ...
  private isValidSortRange(): boolean {
    ...
    const range: number[] = getSwapRange(
      getIndexesFromAddress(sheet.selectedRange)
    );
    if (
      range[0] > sheet.usedRange.rowIndex ||
      range[1] > sheet.usedRange.colIndex
    ) {
      return false;
    }
    return true;
  ...
  private sortImport(args?: { sheetIdx: number }): void {
    const sort: SortCollectionModel[] = this.parent.sortCollection;
    let cell: HTMLElement;
    const rowId: number = this.parent.getActiveSheet().usedRange.rowIndex - 1;
    const sheetIdx: number = args
      ? args.sheetIdx
      ...
      if (sort[j as number].sheetIndex === sheetIdx) {
        for (let i: number = 0; i < rowId; i++) {
          ...
          if (cell && cell.querySelector('.e-filter-icon')) {
            if (
              sort[j as number].order === 'Ascending' ||
              sort[j as number].order === 'OnTop'
            ) {
              if (
                !cell
                  .querySelector('.e-filter-icon')
                  .classList.contains('e-sortasc-filter')
              ) {
                cell
                  .querySelector('.e-filter-icon')
              ...
              if (
                !cell
                  .querySelector('.e-filter-icon')
                  .classList.contains('e-sortdesc-filter')
              ) {
                cell
                  .querySelector('.e-filter-icon')
    ...
    dialogInst.show({
      ...
      beforeOpen: (openArgs: BeforeOpenEventArgs): void => {
        const dlgArgs: DialogBeforeOpenEventArgs = {
          dialogName:
            args.error === l10n.getConstant('MultiRangeSortError')
              ? 'MultiRangeSortDialog'
              : 'SortRangeDialog',
          content: args.error,
      ...
      buttons: [
        {
          ...
          click: (): void => {
            const element: HTMLElement = dialogInst.dialogInstance
              .content as HTMLElement;
            const listview: ListView = getComponent(
              element.getElementsByClassName(
                'e-list-sort e-listview e-lib'
              )[0] as HTMLElement,
              'listview'
            ) as ListView;
            const data: { [key: string]: string }[] = listview.dataSource as {
              [key: string]: string;
            }[];
            this.clearError();
            const errorElem: HTMLElement = element.getElementsByClassName(
              'e-sort-error'
            )[0] as HTMLElement;
      ...
      close: (): void => {
        if (sortOptions) {
          getUpdateUsingRaf((): void =>
            this.applySortHandler({ sortOptions: sortOptions })
          );
        }
      },
    });
  ...
  private validateError(
    ...
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const hasEmpty: boolean = json.some(
      (element: { [key: string]: string }) => element.field.toString() === ''
    );
    if (hasEmpty) {
      Array.prototype.some.call(
        dialogElem.getElementsByClassName('e-sort-field'),
        (dropDown: HTMLElement) => {
          const hasError: boolean = !(<DropDownList>(
            getComponent(dropDown, 'dropdownlist')
          )).value;
          if (hasError) {
    ...
    const hasDuplicate: boolean = json.some(
    ...
    if (hasDuplicate) {
      ...
      Array.prototype.some.call(
        dialogElem.getElementsByClassName('e-sort-field'),
        (dropDown: HTMLElement) => {
          const dropDownList: DropDownList = <DropDownList>(
            getComponent(dropDown, 'dropdownlist')
          );
      ...
      errorElem.appendChild(
        document.createTextNode(l10n.getConstant('SortDuplicateFieldError'))
      );
      return true;
    }
    return false;
  ...
  private customSortContent(): HTMLElement {
    ...
    const listview: HTMLElement = this.parent.createElement('div', {
    ...
    const errorElem: HTMLElement = this.parent.createElement('div', {
  ...
  private getFields(): { [key: string]: string }[] {
    ...
    if (range[0] === range[2] && range[2] - range[0] === 0) {
      ...
      const args: { [key: string]: number[] | boolean } = {
        filterRange: [],
        hasFilter: false,
      };
  ...
  private setHeaderTab(
    ...
    const footer: Element = this.parent.element.querySelector(
      '.e-customsort-dlg .e-footer-content'
    );
    ...
    const checkHeaderObj: CheckBox = new CheckBox({
      ...
      change: (args: CheckBoxChangeEventArgs) => {
        const fieldsMap: FieldSettingsModel = args.checked
          ? { text: 'text', value: 'value' }
          : { text: 'value' };
        Array.prototype.forEach.call(
          dialogElem.getElementsByClassName(
            'e-sort-field e-dropdownlist e-lib'
          ),
          (dropDown: HTMLElement) => {
            const dropDownListObj: DropDownList = getComponent(
              dropDown,
              'dropdownlist'
            ) as DropDownList;
  ...
  private renderListItem(
    ...
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const element: Element = lvObj.element.querySelector(
      'li[data-uid=' + id + ']'
    );
    const fieldsMap: FieldSettingsModel = containsHeader
      ? { text: 'text', value: 'value' }
      : { text: 'value' };
    ...
    const orderRadio: Element =
      element.getElementsByClassName('e-sort-order')[0];
    const ordertxtElem: HTMLElement = orderRadio.getElementsByClassName(
      'e-sort-ordertxt'
    )[0] as HTMLElement;
    const isAscending: boolean =
      ordertxtElem.innerText.toLocaleLowerCase() === 'ascending';
    const radiobutton: RadioButton = new RadioButton({
      ...
      change: (args: ChangeArgs) => {
        this.setRadioBtnValue(lvObj, id, args.value);
      },
    });
  ...
  private clearError(): void {
    const dialogElem: HTMLElement = document.getElementsByClassName(
      'e-sort-dialog'
    )[0] as HTMLElement;
    const errorElem: HTMLElement = dialogElem.getElementsByClassName(
      'e-sort-error'
    )[0] as HTMLElement;
    if (errorElem.style.display !== 'none' && errorElem.innerHTML !== '') {
      errorElem.style.display = 'none';
      Array.prototype.forEach.call(
        dialogElem.getElementsByClassName('e-error'),
        (element: HTMLElement) => {
  ...
  private applySortHandler(args: {
    ...
    const sortOptions: SortOptions = (args && args.sortOptions) || {
      sortDescriptors: {},
    };
    let isSingle: boolean = range[0] === range[2];
    const eventArgs: FilterInfoArgs = {
      filterRange: [],
      hasFilter: false,
    ...
    if (
      eventArgs.hasFilter &&
      (isSingle
        ? inRange(<number[]>eventArgs.filterRange, range[0], range[1])
        : eventArgs.filterRange[0] === range[0] &&
          (this.parent.element.querySelector('.e-selectall.e-highlight') ||
            (range[1] >= eventArgs.filterRange[1] &&
              range[1] <= eventArgs.filterRange[3]) ||
            (range[3] >= eventArgs.filterRange[1] &&
              range[3] <= eventArgs.filterRange[3])))
    ) {
    ...
    const beforeArgs: BeforeSortEventArgs = {
      range: address,
      sortOptions: sortOptions,
      cancel: false,
    };
    ...
    const promise: Promise<SortEventArgs> = new Promise((resolve: Function) => {
      resolve(
        (() => {
          /** */
        })()
    ...
    const sortArgs: {
      promise: Promise<SortEventArgs>;
      args: {
        range: string;
        checkForHeader?: boolean;
        sortOptions: SortOptions;
      };
      previousSort: SortCollectionModel;
    } = {
  ...
  private sortCompleteHandler(args: SortEventArgs): void {
    ...
    this.parent.serviceLocator
      ...
      .refreshRange(range, true, true, false, false, isImported(this.parent));
    if (
      sheetIdx === this.parent.activeSheetIndex &&
