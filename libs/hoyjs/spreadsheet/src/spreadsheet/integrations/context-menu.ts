...
import { closest, extend, detach, L10n, isNullOrUndefined } from '@hoyui/base';
import {
  MenuSelectEventArgs,
  removeSheetTab,
  cMenuBeforeOpen,
...
export class ContextMenu {
    ...
    this.contextMenuInstance = new ContextMenuComponent(
      {
        ...
        filter:
          'e-numericcontainer e-active-cell e-selection e-row e-header-row e-select-all-cell e-sheet-tabs-items',
  ...
  private beforeCloseHandler(args: BeforeOpenCloseMenuEventArgs): void {
    ...
    if (
      this.parent.enableKeyboardShortcut &&
      args.event &&
      (args.event as KeyboardEvent).keyCode === 27
    ) {
      // Esc key
  ...
  private selectHandler(args: MenuEventArgs): void {
    const selectArgs: MenuSelectEventArgs = extend(
      { cancel: false },
      args
    ) as MenuSelectEventArgs;
    ...
    if (
      ...
      for (let i: number = this.parent.sortCollection.length - 1; i >= 0; i--) {
        if (
          ...
          this.parent.sortCollection[i as number].sheetIndex ===
    ...
    if (
      args.item.id === id + '_delete_row' ||
      args.item.id === id + '_delete_column' ||
      args.item.id === id + '_insert_column_before' ||
      args.item.id === id + '_insert_column_after' ||
      args.item.id === id + '_insert_row_above' ||
      args.item.id === id + '_insert_row_below'
    ) {
      const row: RowModel = getRow(this.parent.getActiveSheet(), range[0]);
      const column: ColumnModel = getColumn(
    ...
    if (
      args.item.id === id + '_ascending' ||
      args.item.id === id + '_descending' ||
      args.item.id === id + '_customsort'
    ) {
      const sortRange: number[] = getDataRange(
        range[0],
        range[1],
        this.parent.getActiveSheet()
      );
      ...
      switch (args.item.id) {
        ...
        case id + '_ascending':
          this.parent.notify(updateSortCollection, {
            sortOptions: { sortDescriptors: { order: 'Ascending' } },
          });
        ...
        case id + '_descending':
          this.parent.notify(updateSortCollection, {
            sortOptions: { sortDescriptors: { order: 'Descending' } },
          });
          this.parent.notify(applySort, {
            sortOptions: { sortDescriptors: { order: 'Descending' } },
            previousSort: prevSort,
        ...
        case id + '_clearfilter':
          field = getColumnHeaderText(
            getCellIndexes(this.parent.getActiveSheet().activeCell)[1] + 1
          );
  ...
  private beforeOpenHandler(args: BeforeOpenCloseMenuEventArgs): void {
    ...
    let target: string = this.getTarget(trgt);
    ...
    if (args.element.classList.contains('e-contextmenu')) {
      ...
      if (
        ...
        if (
          trgt.classList.contains('e-rowresize')
            ? this.parent.hiddenCount(range[0], range[2]) !==
              Math.abs(range[2] - range[0]) + 1
            : this.parent.hiddenCount(range[1], range[3], 'columns') !==
              Math.abs(range[3] - range[1]) + 1
        ) {
        ...
        if (target === 'Content') {
          const range: number[] = getRangeIndexes(sheet.selectedRange);
          const rowSelect: boolean =
            range[1] === 0 && range[3] === sheet.colCount - 1;
          const colSelect: boolean =
            range[0] === 0 && range[2] === sheet.rowCount - 1;
          target =
            rowSelect && colSelect
              ? 'SelectAll'
    ...
    if (target === 'ColumnHeader' || target === 'RowHeader') {
      if (args.element && args.element.childElementCount > 0) {
        const insertEle: HTMLElement =
          target === 'ColumnHeader'
            ? args.element.querySelector(
                '#' + this.parent.element.id + '_cmenu_insert_column'
              )
            : args.element.querySelector(
                '#' + this.parent.element.id + '_cmenu_insert_row'
              );
        const deleteEle: HTMLElement =
          target === 'ColumnHeader'
            ? args.element.querySelector(
                '#' + this.parent.element.id + '_cmenu_delete_column'
              )
            : args.element.querySelector(
                '#' + this.parent.element.id + '_cmenu_delete_row'
              );
        if (
          this.parent.allowInsert &&
        ...
        if (
          ...
          deleteEle.classList.contains('e-disabled')
        ) {
        ...
        if (this.parent.selectionSettings.mode === 'None') {
          if (target === 'ColumnHeader') {
            this.parent.enableContextMenuItems(
              ['Insert Column', 'Delete Column', 'Hide Column'],
              false,
              false
            );
          }
          if (target === 'RowHeader') {
            this.parent.enableContextMenuItems(
              ['Insert Row', 'Delete Row', 'Hide Row'],
              false,
              false
            );
          }
      ...
      if (sheetIdx === 0) {
        args.element
          .querySelector('#' + this.parent.element.id + '_cmenu_move_left')
          .classList.add('e-disabled');
      }
      if (sheetIdx === this.parent.sheets.length - 1) {
        args.element
          .querySelector('#' + this.parent.element.id + '_cmenu_move_right')
          .classList.add('e-disabled');
      }
      if (this.parent.selectionSettings.mode === 'None') {
    ...
    this.parent.notify(
      cMenuBeforeOpen,
      extend(args, { target: target, items: items })
    );
  }
  ...
  private getTarget(target: Element): string {
    if (closest(target, '.e-sheet-content')) {
      ...
      return target.classList.contains('e-header-cell')
        ? 'ColumnHeader'
        : 'Content';
  ...
  private getDataSource(target: string, targetEle?: Element): MenuItemModel[] {
    ...
    if (target === 'Content') {
      ...
      if (
        !targetEle ||
        targetEle.classList.contains('e-rowresize') ||
      ...
      if (!targetEle || !targetEle.classList.value.includes('e-hide')) {
        this.setHideShowItems(items, l10n, 'Column', id, [
          indexes[1],
          indexes[3],
        ]);
      }
  ...
  private setFilterItems(items: MenuItemModel[], id: string): void {
    if (this.parent.allowFiltering) {
      ...
      items.push({
        ...
        items: [
          {
            text: args.clearFilterText as string,
  ...
  private setHyperLink(items: MenuItemModel[], id: string): void {
    if (this.parent.allowHyperlink) {
      const l10n: L10n = this.parent.serviceLocator.getService(locale);
      if (
        !document.activeElement.getElementsByClassName('e-hyperlink')[0] &&
    ...
    if (this.parent.enableNotes) {
      ...
      if (
        ...
        targetElement.children[
          (targetElement.children.length - 1) as number
        ].className.indexOf('addNoteIndicator') > -1
      ) {
        items.push(
          {
  ...
  private addItemsHandler(args: InsertArgs): void {
    if (args.insertAfter) {
      this.contextMenuInstance.insertAfter(
        args.items,
        args.text,
        args.isUniqueId
      );
    } else {
      this.contextMenuInstance.insertBefore(
        args.items,
        args.text,
        args.isUniqueId
      );
  ...
  private enableItemsHandler(args: EnableDisableArgs): void {
    this.contextMenuInstance.enableItems(
      args.items,
      args.enable,
      args.isUniqueId
    );
