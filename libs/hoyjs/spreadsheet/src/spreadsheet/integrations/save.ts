...
export class Save {
        ...
        buttons: [
          {
            ...
            click: (): void => {
              ...
              if (this.checkValidName(name)) {
                ...
                const saveButtonObj: { disabled: boolean } = getComponent(
                  saveButton,
                  'btn'
                ) as { disabled: boolean };
                saveButtonObj.disabled = true;
                const l10n: L10n =
                  this.parent.serviceLocator.getService(locale);
                const error: string =
                  name.length === 0
                    ? l10n.getConstant('EmptyFileName')
                    : name.length > 218
                    ? l10n.getConstant('LargeName')
                    : l10n.getConstant('FileNameError');
                const fileSpan: HTMLElement = this.parent.createElement(
                  'span',
                ...
                if (this.parent.element.querySelector('.e-file-alert-span')) {
                  this.parent.element
                    .querySelector('.e-file-alert-span')
                    .remove();
                }
                this.parent.element
                  .querySelector('.e-open-dlg')
                  .querySelector('.e-dlg-content')
                  .appendChild(fileSpan);
              }
            },
          },
        ],
  ...
  private OpenContent(type: string): HTMLElement {
    ...
    const openTextHeader: HTMLElement = this.parent.createElement('div', {
      className: 'e-open-head',
    });
    const openTextE: HTMLElement = this.parent.createElement('div', {
    ...
    const openTextH: HTMLElement = this.parent.createElement('p', {
      className: 'e-header',
      id: id,
      attrs: { 'aria-label': `${l10n.getConstant('SaveAs')} ${header}` },
    });
    openTextH.innerText = header;
    const openTextIp: HTMLElement = this.parent.createElement('input', {
      className: 'e-input e-text-open',
      attrs: { type: 'Text', 'aria-labelledby': id },
    });
    const openTextSpan: HTMLElement = this.parent.createElement('span', {
      className: 'e-input-group-icon',
    });
    openTextIp.onkeyup = (e: KeyboardEventArgs): void => {
      if (
        this.parent.element.querySelector('.e-file-alert-span') &&
        e.keyCode !== 13
      ) {
        const saveButton: HTMLElement = this.parent.element.querySelector(
          '.e-btn-open-ok'
        ) as HTMLElement;
        const buttonObj: { disabled: boolean } = getComponent(
          saveButton,
          'btn'
        ) as { disabled: boolean };
