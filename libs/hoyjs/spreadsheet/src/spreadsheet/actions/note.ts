...
import { isNullOrUndefined, EventHandler, closest, detach } from '@hoyui/base';
...
export class SpreadsheetNote {
  ...
  protected destroy(): void {
    ...
    if (
      ...
      const noteIndicators: HTMLCollectionOf<Element> =
        this.parent.element.getElementsByClassName('e-addNoteIndicator');
  ...
  private deleteNote(args?: {
    rowIndex: number;
    columnIndex: number;
    isDeleteFromMenu?: boolean;
  }): void {
    const cellIndexes: number[] = getCellIndexes(
      this.parent.getActiveSheet().activeCell
    );
    const rowIndex: number =
      !isNullOrUndefined(args) && !isNullOrUndefined(args.rowIndex)
        ? args.rowIndex
        : cellIndexes[0];
    const columnIndex: number =
      !isNullOrUndefined(args) && !isNullOrUndefined(args.columnIndex)
        ? args.columnIndex
        : cellIndexes[1];
    const targetElement: HTMLElement = this.parent.getCell(
      rowIndex,
    ...
    if (
      ...
      targetElement.removeChild(
        targetElement.children[targetElement.children.length - 1]
      );
  ...
  private editNote(): void {
    ...
    const noteContainerElement: HTMLTextAreaElement =
  ...
  private createNoteIndicator(args: {
    targetElement: HTMLElement;
    rowIndex: number;
    columnIndex: number;
    skipEvent?: boolean;
  }): void {
    const noteIndicator: HTMLElement = this.parent.createElement('div', {
      className: 'e-addNoteIndicator',
      styles:
        'position: absolute;top: 0;right: 0;width: 0;height: 0;border-left: 8px solid transparent;border-top: 8px solid red;cursor: pointer;',
    });
    if (args.targetElement.children.length > 0) {
      ...
      for (let i: number = 0; i < args.targetElement.childElementCount; i++) {
        if (
          args.targetElement.children[i as number].className.indexOf(
            'e-filter-btn'
          ) > -1
        ) {
          noteIndicator.style.right =
            (rowHeight < defaultFilterButtonHeight + 10
              ? args.targetElement.children[i as number].getBoundingClientRect()
                  .width <= 0
                ? defaultFilterButtonHeight
                : args.targetElement.children[
                    i as number
    ...
    if (
      (!isNullOrUndefined(args.targetElement) &&
        args.targetElement.children.length === 0) ||
      (args.targetElement.children.length > 0 &&
        args.targetElement.children[
          args.targetElement.childElementCount - 1
        ].className.indexOf('e-addNoteIndicator') === -1)
    ) {
      if (!args.skipEvent) {
        EventHandler.add(args.targetElement, 'mouseover', this.mouseOver, [
          this,
          args.rowIndex,
          args.columnIndex,
        ]);
  ...
  private mouseOver(): void {
    ...
    if (
      (noteModule.isNoteVisibleOnTouch &&
        !isNullOrUndefined(
          document.getElementsByClassName('e-addNoteContainer')[0]
        )) ||
      isNullOrUndefined(
        document.getElementsByClassName('e-addNoteContainer')[0]
      )
  ...
  private mouseOut(e: MouseEvent): void {
    if (
      ...
      if (
        ...
        !isNullOrUndefined(e.relatedTarget as HTMLElement) &&
        (e.relatedTarget as HTMLElement).className.indexOf(
          'e-connectorLine'
        ) === -1 &&
        (e.relatedTarget as HTMLElement).className.indexOf(
          'e-addNoteContainer'
  ...
  private createNoteContainer(
    ...
    const noteContainer: HTMLTextAreaElement = this.parent.createElement(
      'textarea',
    ...
    EventHandler.add(
      document.getElementsByClassName('e-connectorLine')[0] as HTMLElement,
      'mouseout',
      this.mouseOut,
      this
    );
  ...
  private createContainer(
    ...
    if (
      cellRect.top >=
        document
          .getElementsByClassName('e-select-all-cell')[0]
      ...
      cellRect.bottom <=
        document.getElementsByClassName('e-scroller')[0].getBoundingClientRect()
          .top &&
      cellRect.right <=
        document.getElementsByClassName('e-scroller')[0].getBoundingClientRect()
          .width
    ...
    const elementPosition: string =
      this.parent.element.style.getPropertyValue('position');
    noteContainer.style.position = 'absolute';
    noteContainer.style.top =
      cellRect.top -
      (elementClientRect.top -
        (elementPosition === 'absolute' ? 0 : this.parent.element.offsetTop)) -
      containerTop +
      'px';
    noteContainer.style.left =
      ...
      (elementClientRect.left -
        (elementPosition === 'absolute' ? 0 : this.parent.element.offsetLeft)) +
      10 +
      'px';
  ...
  private createConnectorLine(
    ...
    const connectorLine: HTMLCanvasElement = this.parent.createElement(
      'canvas',
      {
    ...
    connectorLine.style.top =
      noteContainer.getBoundingClientRect().top -
      (elementClientRect.top -
        (elementPosition === 'absolute' ? 0 : this.parent.element.offsetTop)) -
      5 +
      'px';
  ...
  private showNote(args: {
    rowIndex: number;
    columnIndex: number;
    isNoteEditable?: boolean;
    isScrollWithNote?: boolean;
    cellElement?: HTMLElement;
  }): void {
    const targetElement: HTMLElement = !isNullOrUndefined(
      this.parent.getCell(args.rowIndex, args.columnIndex)
    )
      ? this.parent.getCell(args.rowIndex, args.columnIndex)
      : args.cellElement;
    const contextMenuElement: HTMLElement = document.getElementById(
      this.parent.element.id + '_contextmenu'
    );
    const contextMenuDisplayStyle: string = !isNullOrUndefined(
      contextMenuElement
    )
      ? contextMenuElement.style.getPropertyValue('display')
      : 'none';
    const showNoteOverContextMenu: boolean = args.isNoteEditable
      ? true
    ...
    if (
      ...
      showNoteOverContextMenu &&
      (args.isScrollWithNote ||
  ...
  private updateNoteContainer(): void {
    ...
    const noteContainer: HTMLTextAreaElement = document.getElementsByClassName(
      'e-addNoteContainer'
    )[0] as HTMLTextAreaElement;
    if (
      (isNullOrUndefined(cell) ||
    ...
    this.isShowNote = isNullOrUndefined(this.isShowNote)
      ? this.parent.selectionModule.isNoteActiveElement
      : this.isShowNote;
    if (this.isShowNote) {
      const isScrollWithNote: boolean =
        !isNullOrUndefined(cell) && !isNullOrUndefined(cell.isNoteEditable)
          ? cell.isNoteEditable
          : false;
      ...
      this.parent.notify(showNote, {
        ...
        isNoteEditable: true,
        isScrollWithNote: isScrollWithNote,
      });
    } else {
      this.parent.notify(removeNoteContainer, '');
      this.isShowNote = null;
