...
import { ListView, SelectedCollection, SelectEventArgs } from '@hoyui/lists';
import {
  L10n,
  EventHandler,
  closest,
...
import { Dialog as DialogComponent } from '@hoyui/popups';
/**
...
export class ProtectSheet {
  ...
  private protect(args: { isActive: boolean; sheetIndex: number }): void {
    if (args.isActive) {
      const sheet: ExtendedSheet =
        this.parent.getActiveSheet() as ExtendedSheet;
  ...
  private createDialogue(): void {
    ...
    const dialogElem: HTMLElement = this.parent.createElement('div', {
      className: 'e-sheet-password-dialog',
    });
    const pwdCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-sheet-password-content',
    });
    const textH: HTMLElement = this.parent.createElement('div', {
      className: 'e-header',
    });
    textH.innerText = l10n.getConstant('SheetPassword');
    const pwdInput: HTMLElement = this.parent.createElement('input', {
    ...
    const headerContent: HTMLElement = this.parent.createElement('div', {
      className: 'e-header-content',
    });
    headerContent.innerText = l10n.getConstant('ProtectSheet');
    const checkBoxElement: HTMLElement = this.parent.createElement('input', {
    ...
    this.dialog.show(
      {
        ...
        content:
          dialogElem.outerHTML +
          checkBoxElement.outerHTML +
          protectHeaderCntent.outerHTML +
          listViewElement.outerHTML,
        showCloseIcon: true,
        isModal: true,
  ...
  private checkBoxClickHandler(): void {
    const ch: HTMLInputElement = document.getElementById(
      this.parent.element.id + '_protect_check'
    ) as HTMLInputElement;
    if (ch.checked === false) {
      this.dialog.dialogInstance.element
  ...
  private dialogOpen(args: SelectEventArgs): void {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    if (
      args.text === l10n.getConstant('SelectCells') &&
      args.isChecked &&
      args.isInteracted
    ) {
    ...
    if (
      args.text === l10n.getConstant('SelectUnlockedCells') &&
      !args.isChecked &&
      args.isInteracted
    ) {
  ...
  private selectOption(): void {
    ...
    const pwd: HTMLElement = this.parent.element
      .querySelector('.e-sheet-password-dialog')
      .getElementsByClassName('e-sheet-password-content')[0]
      .querySelector('.e-input');
    if ((pwd as CellModel).value.length === 0) {
      this.parent.setSheetPropertyOnMute(actSheet, 'isProtected', true);
      this.parent.setSheetPropertyOnMute(
        actSheet,
        'password',
        (pwd as CellModel).value
      );
      this.updateProtectSheet((pwd as CellModel).value);
      this.dialog.hide();
      if (
        !actSheet.protectSettings.selectCells &&
        !actSheet.protectSettings.selectUnLockedCells
      ) {
  ...
  private selectSheetPassword(): void {
    ...
    const pwd: HTMLElement = this.parent.element
      ...
      .getElementsByClassName('e-sheet-password-content')[0]
      .querySelector('.e-input');
    const cnfrmPwd: HTMLElement = this.parent.element
      .querySelector('.e-reenterpwd-dialog')
      .getElementsByClassName('e-reenterpwd-content')[0]
      .querySelector('.e-input');
    ...
    if ((pwd as CellModel).value === (cnfrmPwd as CellModel).value) {
      ...
      const sheetDlgPopup: HTMLElement = document.querySelector(
      ...
      this.parent.element
        .querySelector('.e-reenterpwd-dlg')
        .querySelector('.e-reenterpwd-dialog')
        .appendChild(pwdSpan);
    }
  }
  ...
  private updateProtectSheet(password: string): void {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const selectedItems: SelectedCollection =
      this.optionList.getSelectedItems() as SelectedCollection;
    const protectSettings: { [key: string]: boolean | string } = {
      selectCells:
        selectedItems.text.indexOf(l10n.getConstant('SelectCells')) > -1,
      formatCells:
        selectedItems.text.indexOf(l10n.getConstant('FormatCells')) > -1,
      formatRows:
        selectedItems.text.indexOf(l10n.getConstant('FormatRows')) > -1,
      formatColumns:
        selectedItems.text.indexOf(l10n.getConstant('FormatColumns')) > -1,
      insertLink:
        selectedItems.text.indexOf(l10n.getConstant('InsertLinks')) > -1,
      selectUnLockedCells:
        selectedItems.text.indexOf(l10n.getConstant('SelectUnlockedCells')) >
        -1,
    };
  ...
  private protectSheetHandler(args: {
    ...
    const disableHomeBtnId: string[] = [
      ...
      id + '_sorting',
      id + '_clear',
      id + '_conditionalformatting',
    ];
    const enableHomeBtnId: string[] = [
      id + '_cut',
      id + '_copy',
      id + '_number_format',
      id + '_font_name',
    ...
    const enableInsertBtnId: string[] = [
      id + '_hyperlink',
      id + '_',
      id + '_chart',
    ];
    ...
    const sheetElement: HTMLElement = document.getElementById(
      this.parent.element.id + '_sheet_panel'
    );
    if (sheetElement) {
      if (sheet.isProtected) {
        if (sheet.protectSettings.selectCells) {
          ...
          !isLockedCells(this.parent, getRangeIndexes(sheet.selectedRange))
        ) {
          sheetElement.classList.remove('e-protected');
        } else {
    ...
    this.parent.notify(protectCellFormat, {
      ...
      enableHomeBtnId: enableHomeBtnId,
      enableFrmlaBtnId: enableFrmlaBtnId,
      enableInsertBtnId: enableInsertBtnId,
      findBtnId: findBtnId,
      dataValidationBtnId: dataValidationBtnId,
      imageBtnId: imageBtnId,
      chartBtnId: chartBtnId,
    });
    ...
    if (args && args.triggerEvent) {
      this.parent.notify(completeAction, {
        ...
        eventArgs: {
          ...
          protectSettings:
            (sheet.protectSettings as { properties: {} }).properties ||
            sheet.protectSettings,
        },
      });
    }
  }
  ...
  private passwordProtectContent(): HTMLElement {
    ...
    const cnfrmPwdCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-password-content',
    });
    const textH: HTMLElement = this.parent.createElement('div', {
      className: 'e-header',
    ...
    const urlH: HTMLElement = this.parent.createElement('div', {
      className: 'e-header',
    });
    ...
    const cnfrmPwdInput: HTMLElement = this.parent.createElement('input', {
      className: 'e-input e-text',
      attrs: { type: 'password' },
    });
    pwdInput.setAttribute('placeholder', l10n.getConstant('EnterThePassword'));
    cnfrmPwdInput.setAttribute(
      'placeholder',
      l10n.getConstant('EnterTheConfirmPassword')
    );
    pwdCont.appendChild(pwdInput);
    pwdCont.insertBefore(textH, pwdInput);
  ...
  private KeyUpHandler(e: MouseEvent): void {
    ...
    if (
      trgt.classList.contains('e-text') &&
      closest(trgt, '.e-password-content')
    ) {
      if (
        closest(trgt, '.e-password-dialog') &&
        closest(trgt, '.e-password-dialog').getElementsByClassName(
          'e-password-content'
        )[1] === trgt.parentElement
      ) {
        const dlgEle: Element = closest(trgt, '.e-protectworkbook-dlg');
        const ftrEle: HTMLElement = dlgEle.getElementsByClassName(
          'e-footer-content'
        )[0] as HTMLElement;
    ...
    if (
      trgt.classList.contains('e-text') &&
      closest(trgt, '.e-unprotectpwd-content')
    ) {
      if (
        closest(trgt, '.e-unprotectpwd-dialog') &&
        closest(trgt, '.e-unprotectpwd-dialog').getElementsByClassName(
          'e-unprotectpwd-content'
        )[0] === trgt.parentElement
      ) {
        const dlgElement: Element = closest(trgt, '.e-unprotectworkbook-dlg');
        const ftrElement: HTMLElement = dlgElement.getElementsByClassName(
          'e-footer-content'
        )[0] as HTMLElement;
    ...
    if (
      trgt.classList.contains('e-text') &&
      closest(trgt, '.e-reenterpwd-content')
    ) {
      if (
        closest(trgt, '.e-reenterpwd-dialog') &&
        closest(trgt, '.e-reenterpwd-dialog').getElementsByClassName(
          'e-reenterpwd-content'
        )[0] === trgt.parentElement
      ) {
        const dlgCnt: Element = closest(trgt, '.e-reenterpwd-dlg');
        const ftrCnt: HTMLElement = dlgCnt.getElementsByClassName(
          'e-footer-content'
        )[0] as HTMLElement;
    ...
    if (
      trgt.classList.contains('e-text') &&
      closest(trgt, '.e-unprotectsheetpwd-content')
    ) {
      if (
        closest(trgt, '.e-unprotectsheetpwd-dialog') &&
        closest(trgt, '.e-unprotectsheetpwd-dialog').getElementsByClassName(
          'e-unprotectsheetpwd-content'
        )[0] === trgt.parentElement
      ) {
        const dlg: Element = closest(trgt, '.e-unprotectworksheet-dlg');
        const ftr: HTMLElement = dlg.getElementsByClassName(
          'e-footer-content'
        )[0] as HTMLElement;
    ...
    if (
      trgt.classList.contains('e-text') &&
      closest(trgt, '.e-importprotectpwd-content')
    ) {
      if (
        closest(trgt, '.e-importprotectpwd-dialog') &&
        closest(trgt, '.e-importprotectpwd-dialog').getElementsByClassName(
          'e-importprotectpwd-content'
        )[0] === trgt.parentElement
      ) {
        const dlgElem: Element = closest(trgt, '.e-importprotectworkbook-dlg');
        const ftrElem: HTMLElement = dlgElem.getElementsByClassName(
          'e-footer-content'
        )[0] as HTMLElement;
  ...
  private alertMessage(): void {
    const spanElem: Element =
      this.parent.element.querySelector('.e-pwd-alert-span');
    const unpotectSpanElem: Element = this.parent.element.querySelector(
      '.e-unprotectpwd-alert-span'
    );
    const importpotectSpanElem: Element = this.parent.element.querySelector(
      '.e-importprotectpwd-alert-span'
    );
    const protectSheetSpanElem: Element = this.parent.element.querySelector(
      '.e-reenterpwd-alert-span'
    );
    const unProtectSheetSpanElem: Element = this.parent.element.querySelector(
  ...
  private dlgClickHandler(dialogInst: Dialog): void {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const pwdVal: HTMLElement = this.parent.element
      .querySelector('.e-password-dialog')
      .getElementsByClassName('e-password-content')[0]
      .querySelector('.e-input');
    const cnfrmPwd: HTMLElement = this.parent.element
      .querySelector('.e-password-dialog')
      .getElementsByClassName('e-password-content')[1]
      .querySelector('.e-input');
    const pwdSpan: HTMLElement = this.parent.createElement('span', {
    ...
    if ((pwdVal as CellModel).value === (cnfrmPwd as CellModel).value) {
      ...
      this.parent.notify(completeAction, {
        ...
        eventArgs: { isProtected: true, password: (pwdVal as CellModel).value },
      });
    ...
    if (dialogInst.dialogInstance) {
      this.parent.element
        .querySelector('.e-protectworkbook-dlg')
    ...
    dialogInst.show({
      ...
      beforeOpen: (args: BeforeOpenEventArgs): void => {
        ...
        dialogInst.dialogInstance.content =
          this.unProtectSheetPasswordContent();
        dialogInst.dialogInstance.dataBind();
        this.parent.element.focus();
      },
      buttons: [
        {
          buttonModel: {
            ...
            isPrimary: true,
            disabled:
              this.parent.allowOpen &&
              this.parent.openModule.isImportedFile &&
              this.parent.openModule.unProtectSheetIdx.indexOf(
                this.parent.activeSheetIndex
              ) === -1
                ? false
                : true,
  ...
  private reEnterSheetPassword(): void {
    ...
    dialogInst.show({
      ...
      beforeOpen: (args: BeforeOpenEventArgs): void => {
        ...
        dialogInst.dialogInstance.content = this.reEnterSheetPasswordContent();
        dialogInst.dialogInstance.dataBind();
        const focusEle: HTMLElement =
          this.parent.element.querySelector(
            '.e-protect-dlg.e-dialog .e-footer-content .e-btn'
          ) || this.parent.element;
        focus(focusEle);
      },
      close: (): void => {
  ...
  private unProtectPasswordContent(): HTMLElement {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const dlgElem: HTMLElement = this.parent.createElement('div', {
      className: 'e-unprotectpwd-dialog',
    });
    const pwdCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-unprotectpwd-content',
    });
    const textHeader: HTMLElement = this.parent.createElement('div', {
      className: 'e-header',
    });
    textHeader.innerText = l10n.getConstant('EnterThePassword');
    const pwdInputElem: HTMLElement = this.parent.createElement('input', {
      className: 'e-input e-text',
      attrs: { type: 'password' },
    });
  ...
  private unProtectSheetPasswordContent(): HTMLElement {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const dialogCnt: HTMLElement = this.parent.createElement('div', {
      className: 'e-unprotectsheetpwd-dialog',
    });
    const pwdCnt: HTMLElement = this.parent.createElement('div', {
  ...
  private unprotectdlgOkClick(dialogInst: Dialog): void {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const pwd: HTMLElement = this.parent.element
      .querySelector('.e-unprotectpwd-dialog')
      .getElementsByClassName('e-unprotectpwd-content')[0]
      .querySelector('.e-input');
    if (this.parent.password === (pwd as CellModel).value) {
      ...
      this.parent.element
        .querySelector('.e-unprotectworkbook-dlg')
        .querySelector('.e-dlg-content')
        .appendChild(pwdSpan);
    }
  }
  ...
  private removeWorkbookProtection(): void {
    ...
    const elem: Element = document.getElementById(
      this.parent.element.id + '_protectworkbook'
    );
    if (elem) {
  ...
  private unprotectSheetdlgOkClick(
    dialogInst: Dialog,
    isImportedSheet?: boolean
  ): void {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const sheet: SheetModel = this.parent.getActiveSheet();
    const pwd: HTMLElement = this.parent.element
      .querySelector('.e-unprotectsheetpwd-dialog')
      .getElementsByClassName('e-unprotectsheetpwd-content')[0]
      .querySelector('.e-input');
    if (isImportedSheet && sheet.password.length === 0) {
      ...
      if (sheet.password === (pwd as CellModel).value) {
        ...
        pwdSpan.innerText = l10n.getConstant('UnprotectPasswordAlert');
        this.parent.element
          .querySelector('.e-unprotectworksheet-dlg')
          .querySelector('.e-dlg-content')
          .appendChild(pwdSpan);
      }
    }
  ...
  private importProtectWorkbook(fileArgs: OpenOptions): void {
    ...
    dialogInst.show({
      ...
      beforeOpen: (args: BeforeOpenEventArgs): void => {
        ...
        dialogInst.dialogInstance.content =
          this.importProtectPasswordContent(fileArgs);
        dialogInst.dialogInstance.dataBind();
        this.parent.element.focus();
      },
      buttons: [
  ...
  private importOkClick(args: OpenOptions): void {
    const pwd: HTMLElement = this.parent.element
      .querySelector('.e-importprotectpwd-dialog')
      .getElementsByClassName('e-importprotectpwd-content')[0]
      .querySelector('.e-input');
