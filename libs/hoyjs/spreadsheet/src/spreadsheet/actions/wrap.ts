import { closest, Browser, isNullOrUndefined } from '@hoyui/base';
...
export class WrapText {
    ...
    if (args.initial || inView(this.parent, args.range, true)) {
      ...
      for (let i: number = args.range[0]; i <= args.range[2]; i++) {
        ...
        for (let j: number = args.range[1]; j <= args.range[3]; j++) {
          ...
          ele = (
            args.initial ? args.td : this.parent.getCell(i, j)
          ) as HTMLElement;
          if (ele) {
            if (args.wrap) {
              lineHgt = getLineHeight(
            ...
            if (
              ...
              isMerge ||
              row.height < 20 ||
              (!isNullOrUndefined(args.sheet.standardHeight) &&
                args.sheet.standardHeight < 20)
            ) {
          ...
          if (!(isCustomHgt || isMerge)) {
            colwidth = getExcludedColumnWidth(
              args.sheet,
              i,
              j,
              cell.colSpan > 1 ? j + cell.colSpan - 1 : j
            );
            let displayText: string = this.parent
              .getDisplayText(cell)
              .toString();
            if (ele && displayText.indexOf('\n') < 0) {
              const editElem: HTMLElement = this.parent.element.querySelector(
                '.e-spreadsheet-edit'
              );
              if (editElem) {
            ...
            if (displayText) {
              if (args.wrap) {
                ...
                if (cell.rowSpan > 1) {
                  const prevHeight: number = getRowsHeight(
                    args.sheet,
                    i,
                    i + (cell.rowSpan - 1)
                  );
                  ...
                  hgt = maxHgt =
                    getRowHeight(args.sheet, i) + (maxHgt - prevHeight);
                }
            ...
            if (
              ...
              ((args.wrap &&
                maxHgt > 20 &&
                getMaxHgt(args.sheet, i) <= maxHgt) ||
                ((!args.wrap || !displayText) &&
                  getMaxHgt(args.sheet, i) < getRowHeight(args.sheet, i) &&
                  getRowHeight(args.sheet, i) > 20))
            ) {
              if (ele) {
                setRowEleHeight(
                  this.parent,
                ...
                if (
                  args.sheet.conditionalFormats &&
                  args.sheet.conditionalFormats.length
                ) {
  ...
  private ribbonClickHandler(args: ClickEventArgs): void {
    ...
    if (target && target.id === this.parent.element.id + '_wrap') {
      ...
      const address: string = getAddressFromSelectedRange(
        this.parent.getActiveSheet()
      );
      const eventArgs: BeforeWrapEventArgs = {
        address: address,
        wrap: wrap,
        cancel: false,
      };
      ...
      if (!eventArgs.cancel) {
        ...
        this.parent.notify(completeAction, {
          action: 'wrap',
          eventArgs: { address: address, wrap: wrap },
        });
      }
    }
  }

  private rowHeightChangedHandler(args: {
    rowIdx: number;
    isCustomHgt: boolean;
  }): void {
    if (args.isCustomHgt) {
      ...
      for (
        let i: number = this.parent.viewport.leftIndex,
          len: number = this.parent.viewport.rightIndex;
        i <= len;
        i++
      ) {
  ...
  private colWidthChanged(args: {
    colIdx: number;
    checkWrapCell: boolean;
  }): void {
    if (args.checkWrapCell) {
      const sheet: SheetModel = this.parent.getActiveSheet();
      for (
        let i: number = this.parent.viewport.topIndex,
          len: number = this.parent.viewport.bottomIndex;
        i <= len;
        i++
      ) {
        if (getCell(i, args.colIdx, sheet, false, true).wrap) {
          this.updateWrapCell(
            ...
            args.colIdx,
            sheet,
            this.parent.getCell(i, args.colIdx)
          );
        }
      }
    }
  ...
  private updateWrapCell(
    ...
    if (ele && !ele.querySelector('.e-wrap-content')) {
      ...
      while (
        ...
        !isNullOrUndefined(ele.firstElementChild) &&
        ele.firstElementChild.className.indexOf('e-addNoteIndicator') === -1
      ) {
