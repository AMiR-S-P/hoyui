...
import { closest, isNullOrUndefined } from '@hoyui/base';
import {
...
export class VirtualScroll {
  ...
  private createVirtualElement(args: { top: number; left: number }): void {
    ...
    const vTrack: HTMLElement = container.appendChild(
      this.parent.createElement('div', { className: 'e-virtualtrack' })
    );
    let height: number = 0;
    let width: number;
    ...
    let startIndex: number = this.parent.frozenRowCount(sheet);
    ...
    height += getRowsHeight(
      sheet,
      startIndex,
      this.scroll[this.parent.activeSheetIndex].rowCount - 1,
      true
    );
    ...
    if (sheet.colCount > endIndex + 1 || sheet.usedRange.colIndex > endIndex) {
      ...
      width =
        ...
        getColumnsWidth(
          sheet,
          endIndex + 1,
          this.scroll[this.parent.activeSheetIndex].colCount - 1,
          true
        );
    } else {
      if (!this.parent.scrollSettings.isFinite) {
    ...
    const rowVTrack: HTMLElement = container.appendChild(
      vTrack.cloneNode() as HTMLElement
    );
    ...
    const colVTrack: HTMLElement = container.appendChild(
      vTrack.cloneNode() as HTMLElement
    );
    ...
    if (this.parent.allowScrolling) {
      const scrollVTrack: HTMLElement = colVTrack.cloneNode(
        true
      ) as HTMLElement;
      scrollVTrack.style.width = `${
        width +
        (this.parent.scrollSettings.isFinite
  ...
  private setScrollCount(count: number, layout: string): void {
    ...
    if (
      !this.scroll[activeSheetIdx as number][layout + 'Count'] ||
  ...
  private getRowAddress(indexes: number[]): string {
    ...
    return getRangeAddress([
      indexes[0],
      sheet.frozenColumns
        ? getCellIndexes(sheet.topLeftCell)[1]
  ...
  private updateScrollCount(
    idx: number,
    layout: string,
    threshold: number = idx
  ): void {
    const sheet: SheetModel = this.parent.getActiveSheet();
    let rowCount: number =
      idx + this.parent.viewport[layout + 'Count'] + 1 + threshold;
    const usedRangeCount: number =
      this.scroll[this.parent.activeSheetIndex][layout + 'Count'];
  ...
  private onVerticalScroll(args: IScrollArgs): void {
    let idx: number = args.cur.idx;
    const height: number = args.cur.size;
    ...
    if (idxDiff > Math.round(threshold / 2)) {
        ...
        if (!args.increase) {
          if (this.translateY && prevIdx > threshold) {
            ...
            if (!args.preventScroll) {
              const colIndex: number = frozenCol
                ? getCellIndexes(sheet.topLeftCell)[1]
                : this.parent.viewport.leftIndex;
              const fIndexes: number[] = frozenCol
                ? [frozenRow, this.parent.viewport.leftIndex + frozenCol]
                : [];
              if (idxDiff < this.parent.viewport.rowCount + threshold) {
                startIdx = skipHiddenIdx(sheet, frozenRow, true);
                lastIdx = skipHiddenIdx(
                  sheet,
                  this.parent.viewport.topIndex + frozenRow - 1,
                  false
                );
                ...
                const btmIdx: number = this.skipHiddenLastIdx(
                  this.parent.viewport.bottomIndex -
                    (lastIdx -
                      startIdx +
                      1 -
                      this.hiddenCount(startIdx, lastIdx)),
                  this.parent.viewport.bottomIndex
                );
                this.parent.viewport.bottomIndex = skipHiddenIdx(
                  sheet,
                  btmIdx,
                  false
                );
                this.parent.renderModule.refreshUI(
                  ...
                  this.getRowAddress([
                    startIdx,
                    skipHiddenIdx(sheet, lastIdx, false),
                  ])
                );
              } else {
      ...
      if (
        ...
        const frozenRow: number = this.parent.frozenRowCount(sheet);
        const thresholdIdx: number =
          this.parent.viewport.topIndex +
          frozenRow +
          getRangeIndexes(sheet.paneTopLeftCell)[0] -
          1;
          ...
          if (idxDiff < this.parent.viewport.rowCount + threshold) {
            if (args.increase) {
              ...
              lastIdx =
                this.parent.viewport.bottomIndex +
                (this.parent.viewport.topIndex - prevTopIdx);
              lastIdx -= this.hiddenCount(
                prevTopIdx + frozenRow,
                this.parent.viewport.topIndex - 1 + frozenRow
              );
              if (
                lastIdx <= this.parent.viewport.bottomIndex ||
                (this.parent.scrollSettings.isFinite &&
                  startIdx >
                    skipHiddenIdx(sheet, sheet.rowCount - 1, false, 'rows'))
              ) {
                this.parent.viewport.topIndex = prevTopIdx;
                return;
              }
              ...
              const finiteProps: { index: number; diff: number } =
                this.checkLastIdx(indexes[1], 'row');
              startIdx = indexes[0];
              lastIdx = finiteProps.index;
              let topIdx: number = this.parent.viewport.topIndex + frozenRow;
              if (finiteProps.diff) {
                const diffCount: number =
                  lastIdx - startIdx + 1 - this.hiddenCount(startIdx, lastIdx);
                topIdx =
                  skipHiddenIdx(sheet, prevTopIdx, true, 'rows', diffCount) +
                  frozenRow;
              }
              this.parent.viewport.topIndex =
                skipHiddenIdx(sheet, topIdx, !finiteProps.diff) - frozenRow;
              this.setThresholdHeight(
                height,
                idx - this.parent.viewport.topIndex,
                frozenRow,
                !!finiteProps.diff,
            ...
            if (this.parent.viewport.topIndex < 0) {
              this.parent.viewport.topIndex =
                skipHiddenIdx(sheet, frozenRow, true) - frozenRow;
            }
            this.parent.renderModule.refreshUI({
  ...
  private checkLastIdx(
    ...
    if (this.parent.scrollSettings.isFinite) {
      const sheet: SheetModel = this.parent.getActiveSheet();
      const count: number = skipHiddenIdx(
        sheet,
        sheet[layout + 'Count'] - 1,
        false
      );
      if (idx > count) {
        diff = idx - count;
  ...
  private onHorizontalScroll(args: IScrollArgs): void {
    let idx: number = args.cur.idx;
    const width: number = args.cur.size;
    const prevIdx: number = args.prev.idx;
    let idxDiff: number = Math.abs(idx - prevIdx);
    ...
    if (idxDiff > Math.round(threshold / 2)) {
      ...
      if (idx <= threshold) {
        if (!args.increase) {
          if (this.translateX && prevIdx > threshold) {
            ...
            const frozenRow: number = this.parent.frozenRowCount(sheet);
            if (!args.preventScroll) {
              const rowIndex: number = frozenRow
                ? getCellIndexes(sheet.topLeftCell)[0]
                : this.parent.viewport.topIndex;
              const fIndexes: number[] = frozenRow
                ? [this.parent.viewport.topIndex + frozenRow, frozenCol]
                : [];
              if (idxDiff < this.parent.viewport.colCount + threshold) {
                startIdx = skipHiddenIdx(sheet, frozenCol, true, 'columns');
                endIdx = skipHiddenIdx(
                  sheet,
                  this.parent.viewport.leftIndex + frozenCol - 1,
                  false,
                  'columns'
                );
                ...
                const rightIdx: number = this.skipHiddenLastIdx(
                  this.parent.viewport.rightIndex -
                    (endIdx -
                      startIdx +
                      1 -
                      this.hiddenCount(startIdx, endIdx, 'columns')),
                  this.parent.viewport.rightIndex,
                  'columns'
                );
                this.parent.viewport.rightIndex = skipHiddenIdx(
                  sheet,
                  rightIdx,
                  false,
                  'columns'
                ...
                this.parent.renderModule.refreshUI(
                  ...
                  this.getColAddress([
                    startIdx,
                    skipHiddenIdx(sheet, endIdx, false, 'columns'),
                  ])
                );
              } else {
      ...
      if (
        ...
        let thresholdIdx: number =
          this.parent.viewport.leftIndex +
          frozenCol +
          getRangeIndexes(sheet.paneTopLeftCell)[1] -
          1;
        thresholdIdx += this.hiddenCount(
          this.parent.viewport.leftIndex + frozenCol,
          thresholdIdx
        );
      ...
      if (idx > threshold) {
        ...
        if (!args.preventScroll) {
          ...
          if (frozenRow) {
            frozenIndexes = [
              frozenRow + this.parent.viewport.topIndex,
              frozenCol,
            ];
          }
          if (idxDiff < this.parent.viewport.colCount + threshold) {
            if (args.increase) {
              ...
              endIdx =
                this.parent.viewport.rightIndex +
                (this.parent.viewport.leftIndex - prevLeftIdx);
              endIdx -= this.hiddenCount(
                prevLeftIdx + frozenCol,
                this.parent.viewport.leftIndex - 1 + frozenCol,
                'columns'
              );
              if (
                endIdx <= this.parent.viewport.rightIndex ||
                (this.parent.scrollSettings.isFinite &&
                  startIdx >
                    skipHiddenIdx(sheet, sheet.colCount - 1, false, 'columns'))
              ) {
              ...
              const finiteOffset: { index: number; diff: number } =
                this.checkLastIdx(indexes[1], 'col');
              startIdx = indexes[0];
              ...
              this.setThresholdWidth(
                width,
                idx - this.parent.viewport.leftIndex,
                frozenCol,
                !!finiteOffset.diff,
            ...
            if (this.parent.viewport.leftIndex < 0) {
              this.parent.viewport.leftIndex =
                skipHiddenIdx(sheet, frozenCol, true, 'columns') - frozenCol;
            }
            this.parent.renderModule.refreshUI({
              rowIndex: rowIndex,
              colIndex: this.parent.viewport.leftIndex,
  ...
  private focusSheet(): void {
    if (
      !document.activeElement.classList.contains('e-text-findNext-short') ||
      !closest(document.activeElement, '#' + this.parent.element.id)
    ) {
      focus(this.parent.element);
    }
  }
  ...
  private setThresholdHeight(
    ...
    if (endReached || isInit || end < start) {
      this.translateY =
        start <= frozenRow
          ? 0
          : getRowsHeight(sheet, frozenRow, start - 1, true);
    } else {
  ...
  private setThresholdWidth(
    ...
    if (endReached || isInit || end < start) {
      this.translateX =
        start <= frozenCol
          ? 0
          : getColumnsWidth(sheet, frozenCol, start - 1, true);
    } else {
      this.translateX = scrollWidth - getColumnsWidth(sheet, start, end, true);
    }
  }

  private translate(args: {
    refresh: RefreshType;
  ...
  private updateColumnWidth(args: {
    refresh: RefreshType;
    isUpdate?: boolean;
  }): void {
    if (args.refresh === 'Column') {
      ...
      if (!this.parent.scrollSettings.isFinite || args.isUpdate) {
        const scroll: HTMLElement = this.parent.element.querySelector(
          '.e-scroller .e-virtualtrack'
        ) as HTMLElement;
        if (!scroll) {
          return;
        }
        ...
        const newWidth: number =
          width + this.translateX + this.parent.viewport.beforeFreezeWidth;
        if (newWidth > scrollWidth) {
          const diff: number = newWidth - scrollWidth;
          scroll.style.width = scrollWidth + diff + 'px';
        } else {
          const diff: number = scrollWidth - newWidth;
          const vTrack: HTMLElement = this.parent
            .getMainContent()
            .getElementsByClassName('e-virtualtrack')[0] as HTMLElement;
      ...
      const vTrack: HTMLElement = this.parent
        .getMainContent()
        .getElementsByClassName('e-virtualtrack')[0] as HTMLElement;
      const vTrackHeight: number = parseInt(vTrack.style.height, 10);
      const height: number = this.content.getBoundingClientRect().height;
      const newHeight: number =
        height + this.translateY + this.parent.viewport.beforeFreezeHeight;
      if (newHeight > vTrackHeight) {
        const diff: number = newHeight - vTrackHeight;
        vTrack.style.height = vTrackHeight + diff + 'px';
      } else {
        const diff: number = vTrackHeight - newHeight;
        const hVTrack: HTMLElement = this.parent
          .getRowHeaderContent()
          .getElementsByClassName('e-virtualtrack')[0] as HTMLElement;
  ...
  private updateRowColCount(args: {
    index: number;
    update: string;
    start?: number;
    end?: number;
    ...
    if (args.update === 'row') {
      if (
        ...
        const height: number = this.getVTrackHeight('height');
        let newHeight: number = height;
        if (args.index >= this.scroll[this.parent.activeSheetIndex].rowCount) {
          if (args.start === undefined) {
            newHeight += getRowsHeight(
              sheet,
              this.scroll[this.parent.activeSheetIndex].rowCount,
              args.index,
              true
            );
          } else {
      ...
      if (
        ...
        const width: number = this.getVTrackHeight('width');
        let newWidth: number = width;
        if (args.index >= this.scroll[this.parent.activeSheetIndex].colCount) {
          if (args.start === undefined) {
            newWidth += getColumnsWidth(
              sheet,
              this.scroll[this.parent.activeSheetIndex].colCount,
              args.index,
              true
            );
          } else {
  ...
  private getVTrackHeight(str: string): number {
    let height: string = (this.content.nextElementSibling as HTMLElement).style[
      `${str}`
    ];
  ...
  private updateVTrackHeight(args: {
    rowIdx: number;
    threshold: number;
  }): void {
    ...
    if (args.rowIdx < this.scroll[this.parent.activeSheetIndex].rowCount) {
      this.updateVTrack(
        this.rowHeader,
        this.getVTrackHeight('height') + args.threshold,
        'height'
      );
    }
    if (
      args.rowIdx >= frozenRow &&
      args.rowIdx < this.parent.viewport.topIndex + frozenRow
    ) {
      const mainPanel: Element =
        this.parent.element.getElementsByClassName('e-main-panel')[0];
      this.parent.scrollModule.prevScroll.scrollTop =
  ...
  private updateVTrackWidth(args: { colIdx: number; threshold: number }): void {
    ...
    if (
      args.colIdx >= this.parent.viewport.leftIndex + frozenCol &&
      args.colIdx <= this.parent.viewport.rightIndex
    ) {
      const hdrVTrack: HTMLElement = this.parent
        .getColumnHeaderContent()
        .getElementsByClassName('e-virtualtrack')[0] as HTMLElement;
      hdrVTrack.style.width =
        parseFloat(hdrVTrack.style.width) + args.threshold + 'px';
      const cntVTrack: HTMLElement = this.parent
        .getMainContent()
        .getElementsByClassName('e-virtualtrack')[0] as HTMLElement;
      cntVTrack.style.width =
        parseFloat(cntVTrack.style.width) + args.threshold + 'px';
      const scrollVTrack: HTMLElement = this.parent
        .getScrollElement()
        .getElementsByClassName('e-virtualtrack')[0] as HTMLElement;
      scrollVTrack.style.width =
        parseFloat(scrollVTrack.style.width) + args.threshold + 'px';
      const hdrColumn: HTMLElement = this.parent
        .getColumnHeaderContent()
        .getElementsByClassName('e-virtualable')[0] as HTMLElement;
      hdrColumn.style.width =
        parseFloat(hdrColumn.style.width) + args.threshold + 'px';
      const cntColumn: HTMLElement = this.parent
        .getMainContent()
        .getElementsByClassName('e-virtualable')[0] as HTMLElement;
      cntColumn.style.width =
        parseFloat(cntColumn.style.width) + args.threshold + 'px';
    } else if (
  ...
  private updateVTrack(
    ...
    (header.nextElementSibling as HTMLElement).style[
      `${sizeStr}`
    ] = `${size}px`;
    (this.content.nextElementSibling as HTMLElement).style[
      `${sizeStr}`
    ] = `${size}px`;
    if (sizeStr === 'width' && this.parent.allowScrolling) {
      (
        this.parent.getScrollElement().firstElementChild as HTMLElement
      ).style.width = `${size}px`;
    }
  }
  ...
  private updateScrollProps(
    args: { sheetIndex: number; sheets: SheetModel[] } = {
      sheetIndex: 0,
    ...
    if (args.height) {
      if (args.isRender) {
        ...
        if (args.isHide) {
          ...
          this.setThresholdHeight(
            this.translateY,
            args.prevSize - 1 - this.parent.viewport.topIndex + 1,
  ...
  private destroy(): void {
    ...
    const noteIndicatorElement: NodeListOf<Element> = document.querySelectorAll(
      '.e-addNoteIndicator'
    );
