...
export class Scroll {
  ...
  private onContentScroll(e: ScrollEventArgs): void {
    ...
    const scrollLeft: number = e.scrollLeft;
    const top: number = e.scrollTop || target.scrollTop;
    const left: number =
      scrollLeft && this.parent.enableRtl
        ? this.initScrollValue - scrollLeft
        : scrollLeft;
    let scrollArgs: IScrollArgs;
    let prevSize: number;
    if (this.parent.allowAutoFill) {
      const elem: Element = document.querySelector(
        '#' + this.parent.element.id + '_autofilloptionbtn-popup'
      );
      const DDBElem: HTMLElement = document.querySelector(
        '#' + this.parent.element.id + '_autofilloptionbtn'
      );
      if (elem) {
        const DDBObj: { toggle: Function } = getComponent(
          DDBElem,
          'dropdown-btn'
        );
        DDBObj.toggle();
    ...
    if (!isNullOrUndefined(scrollLeft) && this.prevScroll.scrollLeft !== left) {
      ...
      scrollArgs = {
        ...
        prev: { idx: this.leftIndex, size: prevSize },
    ...
    if (Math.round(this.prevScroll.scrollTop) !== Math.round(top)) {
      ...
      scrollArgs = {
        cur: this.offset.top,
        prev: { idx: this.topIndex, size: prevSize },
        increase: scrollDown,
        preventScroll: e.preventScroll,
      };
      ...
      if (
        e.preventScroll &&
        this.offset.top.idx <= this.parent.getThreshold('row')
      ) {
        this.offset.top = { idx: 0, size: 0 };
    ...
    if (args.isEdit) {
      const textArea: HTMLTextAreaElement =
        this.parent.element.querySelector('.e-formula-bar');
      textArea.focus();
    }
  ...
  private updateScrollValue(args: {
    scrollLeft?: number;
    scrollTop?: number;
  }): void {
    if (args.scrollLeft !== undefined) {
      this.prevScroll.scrollLeft =
        args.scrollLeft + (this.prevScroll.scrollLeft - this.offset.left.size);
      this.offset.left.size = args.scrollLeft;
    }
    if (args.scrollTop !== undefined) {
      this.prevScroll.scrollTop =
        args.scrollTop + (this.prevScroll.scrollTop - this.offset.top.size);
  ...
  private updateNonVirtualRows(): void {
    ...
    if (
      this.offset.top.idx >
      sheet.rowCount - (this.parent.viewport.rowCount + threshold)
    ) {
      this.parent.renderModule.refreshUI(
        ...
        `${getCellAddress(sheet.rowCount, 0)}:${getCellAddress(
          sheet.rowCount + threshold - 1,
          sheet.colCount - 1
        )}`
      );
  ...
  private updateNonVirtualCols(): void {
    ...
    if (
      this.offset.left.idx >
      sheet.colCount - (this.parent.viewport.colCount + threshold)
    ) {
      this.parent.renderModule.refreshUI(
        {
          rowIndex: 0,
        ...
        `${getCellAddress(0, sheet.colCount)}:${getCellAddress(
          sheet.rowCount - 1,
          sheet.colCount + threshold - 1
        )}`
      );
      this.parent.setSheetPropertyOnMute(
        sheet,
  ...
  private updateTopLeftCell(increase: boolean, isLeft?: boolean): void {
    const sheet: SheetModel = this.parent.getActiveSheet();
    let top: number = this.offset.top.idx;
    let left: number = this.offset.left.idx;
    if (!increase) {
      ...
      left =
        skipHiddenIdx(sheet, left + frozenCol, true, 'columns') - frozenCol;
    }
    if (isLeft) {
  ...
  private getRowOffset(scrollTop: number, scrollDown: boolean): IOffset {
    ...
    const sheet: SheetModel = this.parent.getActiveSheet();
    let i: number = scrollDown
      ? this.offset.top.idx + 1
      : this.offset.top.idx
      ? this.offset.top.idx - 1
      : 0;
    const frozenRow: number = this.parent.frozenRowCount(sheet);
    const count: number = this.parent.scrollSettings.isFinite
      ? sheet.rowCount
      : Infinity;
    ...
    while (i < count) {
      if (scrollDown) {
        ...
        if (Math.abs(Math.round(temp) - scrollTop) <= 1) {
          // <=1 -> For other resolution scrollTop value slightly various with row height
          return {
        ...
        if (Math.round(temp) > scrollTop) {
          return {
            idx: i - 1,
            size: temp - getRowHeight(sheet, i - 1 + frozenRow, true),
          };
        }
        i++;
      } else {
        ...
        if (Math.round(temp) < scrollTop) {
          ...
          if (Math.round(temp) > scrollTop) {
            return {
              idx: i,
              size:
                temp - getRowHeight(sheet, i + frozenRow, true) < 0
                  ? 0
                  : temp - getRowHeight(sheet, i + frozenRow, true),
            };
          } else {
  ...
  private getColOffset(
    ...
    const sheet: SheetModel = this.parent.getActiveSheet();
    let i: number = increase
      ? this.offset.left.idx + 1
      : this.offset.left.idx
      ? this.offset.left.idx - 1
      : 0;
    const frozenCol: number = this.parent.frozenColCount(sheet);
    ...
    while (i < count) {
      if (increase) {
        ...
        if (Math.abs(Math.round(temp) - scrollLeft) <= 1) {
          return {
            idx:
              skipHiddenIdx(sheet, i + frozenCol, true, 'columns') - frozenCol,
            size: temp,
          };
        }
        ...
        if (Math.round(temp) < scrollLeft) {
          ...
          if (Math.round(temp) > scrollLeft) {
            ...
            return {
              idx:
                skipHiddenIdx(sheet, i + 1 + frozenCol, true, 'columns') -
                frozenCol,
              size: temp,
            };
          }
  ...
  private contentLoaded(args: { left?: number }): void {
    if (!this.parent.scrollSettings.enableVirtualization) {
      const scrollTrack: HTMLElement = this.parent.createElement('div', {
  ...
  private updateNonVirualScrollWidth(args: {
    scrollTrack?: HTMLElement;
  }): void {
    if (!args.scrollTrack) {
      args.scrollTrack = this.parent
        .getScrollElement()
        .getElementsByClassName('e-virtualtrack')[0] as HTMLElement;
    }
    args.scrollTrack.style.width = `${Math.abs(
      this.parent.getContentTable().getBoundingClientRect().width +
        (this.parent.scrollSettings.isFinite
  ...
  private scrollHandler(e: MouseEvent): void {
    this.onContentScroll(<ScrollEventArgs>{
      scrollLeft: (e.target as Element).scrollLeft,
    });
  }
  ...
  private setScrollEvent(): void {
    EventHandler.add(
      this.parent.sheetModule.contentPanel,
      'scroll',
  ...
  public setPadding(isRtlChange?: boolean): void {
    ...
    if (scrollWidth > 0) {
      ...
      const cssProps: { margin: string; border: string } = this.parent.enableRtl
        ? { margin: 'marginLeft', border: 'borderLeftWidth' }
  ...
  private setClientX(e: PointerEvent | TouchEvent): void {
    if (e.type === 'mousedown' || (e as PointerEvent).pointerType === 'mouse') {
      return;
    }
    const args: { touchSelectionStarted: boolean; isOverlayClicked?: boolean } =
    ...
    EventHandler.add(
      sheetContent,
      Browser.isPointer ? 'pointermove' : 'touchmove',
      this.onTouchScroll,
      this
    );
    EventHandler.add(
      sheetContent,
      Browser.isPointer ? 'pointerup' : 'touchend',
  ...
  private destroy(): void {
    ...
    const ddbEle: HTMLElement = document.querySelector(
      '#' + this.parent.element.id + '_autofilloptionbtn'
    );
    if (ddbEle) {
      const ddbObj: { destroy: Function } = getComponent(
        ddbEle,
        'dropdown-btn'
      );
      if (ddbObj) {
