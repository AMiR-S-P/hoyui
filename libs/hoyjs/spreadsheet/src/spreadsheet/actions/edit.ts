...
import {
  checkFormulaRef,
  getData,
  VisibleMergeIndexArgs,
} from '../../workbook/index';
import { L10n } from '@hoyui/base';
import { Dialog } from '../services/dialog';
import { BeforeOpenEventArgs, Dialog as DialogComponent } from '@hoyui/popups';
...
export class Edit {
    ...
    switch (action) {
      case 'renderEditor':
        ...
        if (
          ...
          const focusEditEle: HTMLElement = this.parent.createElement('div', {
          ...
          const sheetPanel: HTMLElement =
            this.parent.element.querySelector('.e-sheet-panel');
          if (sheetPanel) {
      ...
      case 'refreshEditor':
        this.refreshEditor(
          <string>args.value,
          <boolean>args.refreshFormulaBar,
          <boolean>args.refreshEditorElem,
          <boolean>args.isAppend,
          <boolean>args.trigEvent
        );
  ...
  private keyUpHandler(e: KeyboardEventArgs): void {
    if (this.isEdit) {
      const editElement: HTMLElement = this.getEditElement(
      ...
      if (e.altKey && e.keyCode === 13) {
        ...
        this.isCellEdit &&
        this.editCellData.value !== editElement.textContent &&
        e.keyCode !== 16 &&
      ...
      if (
        ...
        if (this.endFormulaRef) {
          const curOffset: { start?: number; end?: number } =
            this.getCurPosition();
          ...
          if (
            curOffset.end &&
            validCharacters.indexOf(
              this.editCellData.value[curOffset.end - 1]
            ) > -1
          ) {
  ...
  private keyDownHandler(e: KeyboardEventArgs): void | boolean {
    ...
    if (!closest(trgtElem, '.e-spreadsheet .e-dialog')) {
      if (
        !sheet.isProtected ||
        trgtElem.classList.contains('e-sheet-rename') ||
        !isLocked(cell, getColumn(sheet, actCell[1])) ||
        (trgtElem.classList.contains('e-formula-bar') &&
          !(trgtElem as HTMLTextAreaElement).disabled)
      ) {
        if (this.isEdit) {
          ...
          if (
            this.isCellEdit ||
            (isFormulaEdit &&
              this.editCellData.value !== editorElem.textContent &&
              e.keyCode !== 16 &&
              e.keyCode !== 17)
          ) {
            if (
              actCell[1] < this.parent.frozenColCount(sheet) &&
              (!sheet.frozenRows ||
                actCell[0] >= this.parent.frozenRowCount(sheet)) &&
              editorElem &&
              ...
              if (
                getTextWidth(
                  ...
                  cell.style,
                  this.parent.cellStyle
                ) > parseInt(editorElem.style.maxWidth, 10)
              ) {
                editorElem.style.height = 'auto';
              }
            }
            if (
              ...
              parseInt(editorElem.style.maxWidth, 10) - 5
            ) {
              // 5 decreased for padding.
              editorElem.style.height = 'auto';
            }
            if (
              actCell[0] < this.parent.frozenRowCount(sheet) &&
              editorElem &&
              !editorElem.style.overflow &&
              getTextWidth(
                editorElem.textContent,
                cell.style,
                this.parent.cellStyle
            ...
            switch (keyCode) {
              case this.keyCodes.ENTER:
                ...
                if (!isFormulaEdit) {
                  ...
                  const formulaRefIndicator: HTMLElement =
                    this.parent.element.querySelector(
                      '.e-formularef-indicator'
                    );
                  if (formulaRefIndicator) {
                    formulaRefIndicator.parentElement.removeChild(
                      formulaRefIndicator
                    );
                  }
                  if (
                    getSheet(this.parent, this.editCellData.sheetIndex).id ===
                    sheet.id
                  ) {
                    this.endEdit(false, e);
                  } else {
          ...
          trgtElem.classList.contains('e-spreadsheet') ||
          closest(trgtElem, '.e-sheet-panel')
        ) {
          ...
          const isAlphabet: boolean =
            (keyCode >= this.keyCodes.FIRSTALPHABET &&
              keyCode <= this.keyCodes.LASTALPHABET) ||
            (key.toLowerCase() !== key.toUpperCase() &&
              !(keyCode >= 112 && keyCode <= 123));
          const isNumeric: boolean =
            keyCode >= this.keyCodes.FIRSTNUMBER &&
            keyCode <= this.keyCodes.LASTNUMBER;
          const isNumpadKeys: boolean =
            keyCode >= this.keyCodes.FIRSTNUMPAD &&
            keyCode <= this.keyCodes.LASTNUMPAD;
          let isSymbolkeys: boolean =
            keyCode >= this.keyCodes.SYMBOLSETONESTART &&
            keyCode <= this.keyCodes.SYMBOLSETONEEND;
          if (!isSymbolkeys) {
            isSymbolkeys =
              keyCode >= this.keyCodes.SYMBOLSETTWOSTART &&
              keyCode <= this.keyCodes.SYMBOLSETTWOEND;
          }
          const isFirefoxExceptionkeys: boolean =
            keyCode === this.keyCodes.FIREFOXEQUALPLUS ||
            keyCode === this.keyCodes.FIREFOXMINUS;
          const isF2Edit: boolean =
            !e.shiftKey &&
            !e.ctrlKey &&
            !e.metaKey &&
            keyCode === this.keyCodes.F2;
          const isBackSpace: boolean = keyCode === this.keyCodes.BACKSPACE;
          const isMacDelete: boolean =
            /(Macintosh|MacIntel|MacPPC|Mac68K|Mac|Mac OS|iPod|iPad)/i.test(
              navigator.userAgent
            ) && isBackSpace;
          const readonlyDialog: Element = this.parent.element.querySelector(
            '.e-readonly-alert-dlg'
          );
          const overlayElements: HTMLCollection =
            this.parent.element.getElementsByClassName('e-ss-overlay-active');
          if (
            (!e.ctrlKey &&
              !e.metaKey &&
              !e.altKey &&
              ((!e.shiftKey && keyCode === this.keyCodes.SPACE) ||
                isAlphabet ||
                isNumeric ||
                isNumpadKeys ||
                isSymbolkeys ||
                (Browser.info.name === 'mozilla' && isFirefoxExceptionkeys))) ||
            isF2Edit ||
            isBackSpace
          ) {
            if (isF2Edit) {
          ...
          if (keyCode === this.keyCodes.DELETE || isMacDelete) {
            const islockcell: boolean =
              sheet.isProtected && isLockedCells(this.parent);
            if (!readonlyDialog) {
              if (islockcell) {
        ...
        ((keyCode >= this.keyCodes.FIRSTALPHABET &&
          ...
          (keyCode >= this.keyCodes.FIRSTNUMBER &&
            keyCode <= this.keyCodes.LASTNUMBER) ||
          keyCode === this.keyCodes.DELETE ||
          keyCode === this.keyCodes.BACKSPACE ||
          keyCode === this.keyCodes.SPACE ||
          (keyCode >= this.keyCodes.FIRSTNUMPAD &&
            keyCode <= this.keyCodes.LASTNUMPAD) ||
          (keyCode >= this.keyCodes.SYMBOLSETONESTART &&
            keyCode <= this.keyCodes.SYMBOLSETONEEND) ||
        ...
        if (
          !e.ctrlKey &&
          e.keyCode !== 70 &&
          !this.parent.element.querySelector('.e-editAlert-dlg') &&
          !trgtElem.parentElement.classList.contains(
  ...
  private renderEditor(): void {
    if (
      !this.editorElem ||
      !select('#' + this.parent.element.id + '_edit', this.parent.element)
    ) {
      const editor: HTMLElement = this.parent.createElement('div', {
      ...
      if (this.parent.element.getElementsByClassName('e-spreadsheet-edit')[0]) {
        this.parent.element
          .getElementsByClassName('e-spreadsheet-edit')[0]
          .remove();
      }
      const sheetContentElem: Element =
        this.parent.element.querySelector('.e-sheet-content');
      if (!sheetContentElem) {
        return;
      }
  ...
  private refreshEditor(
    value: string,
    refreshFormulaBar?: boolean,
    refreshEditorElem?: boolean,
    isAppend?: boolean,
    trigEvent: boolean = true,
    prevCellValue?: string
  ): void {
    if (isAppend) {
    ...
    const editorElem: HTMLElement = this.getEditElement(
      this.parent.getActiveSheet()
    );
  ...
  private startEdit(
    address?: string,
    value?: string,
    refreshCurPos: boolean = true,
    preventFormulaReference?: boolean
  ): void {
    if (this.parent.showSheetTabs) {
      this.parent.element
        .querySelector('.e-add-sheet-tab')
    ...
    if (
      ...
      const cellEle: HTMLTableCellElement = this.parent.getCell(
        range[0],
        range[1]
      ) as HTMLTableCellElement;
      let isDelTemplate: boolean = false;
      const value: string = cellEle.innerHTML;
      if (cellEle) {
        if (
          value.indexOf('<') > -1 &&
          value.indexOf('>') > -1 &&
    ...
    const isMergedHiddenCell: boolean = this.updateEditCellDetail(
      address,
      value
    );
  ...
  private hasFormulaSuggSelected(): boolean {
    const suggDdlElem: HTMLElement = document.getElementById(
      this.parent.element.id + '_ac_popup'
    );
    return (
      suggDdlElem &&
      suggDdlElem.style.visibility === 'visible' &&
      suggDdlElem.querySelectorAll('.e-item-focus').length > 0
    );
  }

  private editingHandler(action: string): void {
    const pictureElements: HTMLCollection = document.getElementsByClassName(
      'e-ss-overlay-active'
    );
    ...
    switch (action) {
      case 'delete':
        if (pictureLen > 0) {
          if (
            pictureElements[0].classList.contains('e-datavisualization-chart')
          ) {
          ...
          const cellDeleteArgs: { address: string; cancel: boolean } = {
          ...
          const args: { cellIdx: number[]; isUnique: boolean } = {
            cellIdx: range,
            isUnique: false,
          };
          ...
          if (
            args.isUnique &&
            this.uniqueColl.split(':')[0] === address.split(':')[0]
          ) {
            ...
            if (
              getCell(uniqueRange[0], uniqueRange[1], sheet).value === '#SPILL!'
            ) {
          ...
          if (args.isUnique) {
            ...
            this.parent.notify(completeAction, {
              action: 'cellDelete',
              eventArgs: { address: sheet.name + '!' + address },
            });
          }
        }
        break;
  ...
  private getCurPosition(): { start?: number; end?: number } {
    ...
    if (
      selection &&
      selection.focusNode &&
      (selection.focusNode as Element).classList &&
      ...
      ).getElementsByClassName('e-formula-bar e-css')[0] as HTMLTextAreaElement;
      ...
      this.getEditElement(this.parent.getActiveSheet()).textContent ===
      this.editCellData.value
    ) {
  ...
  private mouseDownHandler(e: MouseEvent & TouchEvent): void {
    if (
      ...
      !closest(e.target as Element, '.e-validation-error-dlg')
    ) {
      if (this.isEdit) {
        const curOffset: { start?: number; end?: number } =
          this.getCurPosition();
        ...
        const sheet: SheetModel = this.parent.getActiveSheet();
        const formulaRefIndicator: HTMLElement =
          this.parent.element.querySelector('.e-formularef-indicator');
        ...
        if (
          trgtElem.classList.contains('e-cell') ||
          trgtElem.classList.contains('e-header-cell') ||
          trgtElem.classList.contains('e-selectall') ||
          closest(trgtElem, '.e-toolbar-item.e-active') ||
          closest(trgtElem, '.e-table')
        ) {
          if (this.isAltEnter) {
          ...
          if (!isFormula || this.endFormulaRef) {
            ...
            const editorValue: string =
              document.activeElement.classList.contains('e-formula-bar')
                ? (document.activeElement as HTMLTextAreaElement).value
                : editorElem.textContent;
            if (this.editCellData.value === editorValue) {
              if (selectionStart === selectionEnd) {
                if (
                  this.editCellData.sheetIndex !==
                  getSheetIndex(this.parent, sheet.name)
                ) {
                  if (
                    validCharacters.indexOf(
                      editorValue.substring(selectionStart - 1, selectionStart)
                    ) === -1
                  ) {
                    if (formulaRefIndicator) {
                      formulaRefIndicator.parentElement.removeChild(
                  ...
                  validCharacters.indexOf(
                    editorElem.textContent.substring(
                      selectionStart - 1,
                      selectionStart
                    )
                  ) === -1
            ...
            if (isFormula && this.parent.isEdit) {
              ...
              if (this.editCellData.value.length === curPos) {
                if (
                  this.editCellData.value.substring(
                    this.editCellData.value.length - 1
                  ) === ')' ||
                  isNumber(
                    this.editCellData.value.substring(
                      this.editCellData.value.length - 1
                ...
                this.editCellData.value === editorValue &&
                validCharacters.indexOf(
                  editorValue.substring(curPos - 1, curPos)
                ) === -1
              ) {
                if (formulaRefIndicator) {
                  formulaRefIndicator.parentElement.removeChild(
          ...
          if (
            isFormula &&
            this.editCellData.value === editorElem.textContent &&
            editorElem.textContent.indexOf('(') !==
              editorElem.textContent.length - 1 &&
            !this.isCellEdit &&
            !trgtElem.classList.contains('e-formula-bar') &&
            validCharacters.indexOf(
              this.editCellData.value.substring(
            ...
            if (
              ...
              if (
                validCharacters.indexOf(
                  editorElem.textContent.substring(curPos - 1, curPos)
                ) === -1
              ) {
                if (formulaRefIndicator) {
                  formulaRefIndicator.parentElement.removeChild(
  ...
  private tapHandler(e: TouchEvent): void {
    if (!this.tapedTwice) {
      ...
      setTimeout((): void => {
        ...
        if (
          !this.parent.isEdit &&
          (e.target as Element).classList.contains('e-cell')
        ) {
          const focusEditEle: HTMLElement = this.parent.element.querySelector(
            '.e-ss-focus-edit'
          ) as HTMLElement;
          if (focusEditEle) {
            focus(focusEditEle);
  ...
  private dblClickHandler(e: MouseEvent | TouchEvent): void {
    ...
    if (
      !closest(trgt, '.e-datavisualization-chart') &&
      !trgt.classList.contains('e-ss-overlay') &&
      (trgt.classList.contains('e-active-cell') ||
        trgt.classList.contains('e-cell') ||
        trgt.classList.contains('e-wrap-content') ||
        closest(trgt, '.e-sheet-content') ||
        trgt.classList.contains('e-table'))
    ) {
      const sheet: SheetModel = this.parent.getActiveSheet();
      const actCell: number[] = getCellIndexes(sheet.activeCell);
      const cell: CellModel = getCell(
      ...
      if (
        isReadOnly(
          cell,
          getColumn(sheet, actCell[1]),
          getRow(sheet, actCell[0])
        )
      ) {
        this.parent.notify(readonlyAlert, null);
      } else if (
        ...
        if (this.isEdit) {
          if (!trgt.classList.contains('e-spreadsheet-edit')) {
            if (checkIsFormula(this.editCellData.value)) {
              const sheetName: string = this.editCellData.fullAddr.substring(
                0,
                this.editCellData.fullAddr.indexOf('!')
              );
  ...
  private updateEditCellDetail(addr?: string, value?: string): boolean {
    ...
    if (addr) {
      ...
      const cellPosition: { top: number; left: number } = getCellPosition(
        ...
        this.parent.frozenColCount(sheet),
        this.parent.viewport.beforeFreezeHeight,
        this.parent.viewport.beforeFreezeWidth,
  ...
  private initiateEditor(
    refreshCurPos: boolean,
    isMergedHiddenCell: boolean
  ): void {
    getData(
      this.parent,
      this.editCellData.fullAddr,
      false,
      isMergedHiddenCell
    ).then((values: Map<string, CellModel>): void => {
      if (!this.parent) {
        return;
      }
      (values as Map<string, CellModel>).forEach((cell: CellModel): void => {
        ...
        const updateEditValue: Function = (): void => {
          const args: { [key: string]: CellModel | string | boolean } = {
            cell: cell,
        ...
        updateEditValue();
        const evtArgs: CellEditEventArgs = this.triggerEvent(
          'cellEdit',
          null,
          value
        );
  ...
  private positionEditor(isWrap?: boolean): void {
    ...
    if (arg.isEdit && isNullOrUndefined(tdElem)) {
      cellEle = this.parent.getCell(
        this.editCellData.rowIndex,
    ...
    if (tdElem) {
      ...
      const minHeight: number =
        getRowsHeight(sheet, <number>args.range[0], <number>args.range[2]) - 3;
      const minWidth: number =
        getColumnsWidth(sheet, <number>args.range[1], <number>args.range[3]) -
        3;
      ...
      if (this.editCellData.colIndex < frozenCol) {
        ...
        if (this.editCellData.rowIndex < frozenRow) {
          ...
          if (
            getTextWidth(cell.value, cell.style, this.parent.cellStyle) >
            editWidth
          ) {
            addWrap = true;
          }
        ...
        editWidth =
          mainContElement.offsetWidth -
          (left - cont.scrollLeft) -
          28 -
          this.parent.sheetModule.getRowHeaderWidth(sheet);
        const tdEleInf: ClientRect = tdElem.getBoundingClientRect();
        const mainContEleInf: ClientRect =
          mainContElement.getBoundingClientRect();
        const getCellRight: number = this.parent.enableRtl
          ? tdEleInf.left
          : tdEleInf.right;
        const getMainConEleRight: number = this.parent.enableRtl
          ? mainContEleInf.left
          : mainContEleInf.right;
        const horizontalScrollBar: Element = this.parent.getScrollElement();
        const verticalScrollBarWidth: number =
          this.parent.sheetModule.getScrollSize();
      ...
      const height: string =
        !preventWrap && ((cell && cell.wrap) || (tdElem && isWrap) || addWrap)
          ? 'auto;'
          : minHeight + 'px;';
      ...
      let inlineStyles: string =
        ...
        minWidth +
        'px;max-width:' +
        (cell && cell.wrap ? minWidth : editWidth) +
        'px;' +
        'height:' +
        height +
        (cell && cell.wrap ? 'width:' + minWidth + 'px;' : '') +
        'min-height:' +
        minHeight +
        'px;' +
        (zIndex ? 'z-index: ' + zIndex + ';' : '') +
        (preventWrap &&
        ((cell && !cell.wrap) || (tdElem && isWrap)) &&
        (getTextWidth(cell.value, cell.style, this.parent.cellStyle) >
          editWidth ||
          (tdElem && isWrap))
          ? 'overflow: auto;'
      ...
      const actCell: HTMLElement = this.parent.element.querySelector(
        '.e-active-cell'
      ) as HTMLElement;
      if (actCell) {
  ...
  private updateEditedValue(
    tdRefresh: boolean,
    value: string,
    e: (MouseEvent & TouchEvent) | KeyboardEventArgs,
    isPublic: boolean
  ): void {
    ...
    this.parent.notify(setActionData, {
      args: {
        ...
        eventArgs: { address: this.editCellData.addr },
      },
    });
    ...
    if (this.parent.allowDataValidation) {
      ...
      if (
        cell.validation ||
        checkColumnValidation(column, cellIndex[0], cellIndex[1])
      ) {
        const editedValue: string =
          this.editCellData.value || this.getEditElement(sheet).innerText;
        const sheetIdx: number = this.parent.activeSheetIndex;
        const range: number[] =
          typeof this.editCellData.addr === 'string'
            ? getRangeIndexes(this.editCellData.addr)
        ...
        if (isValidCellValue) {
          if (checkIsFormula(editedValue) || !cell.format) {
            ...
            editedValue !== validEventArgs.value ||
    ...
    if (!isPublic && checkIsFormula(this.editCellData.value)) {
      const eventArgs: { formula: string; isInvalid?: boolean } = {
      ...
      if (eventArgs.isInvalid) {
        ...
        const erroDialogInst: Dialog = this.parent.serviceLocator.getService(
          dialog
        ) as Dialog;
        erroDialogInst.show(
          {
            ...
            content: `${l10n.getConstant('CellReferenceTypoError')}<br>${
              eventArgs.formula
            }`,
  ...
  private updateCell(
    ...
    const oldValue: string = oldCellValue
      ? oldCellValue.toString().toUpperCase()
      : '';
    if (
      oldCellValue !== this.editCellData.value ||
      oldValue.indexOf('=NOW()') > -1 ||
      oldValue.indexOf('NOW()') > -1 ||
      oldValue.indexOf('=RAND()') > -1 ||
      oldValue.indexOf('RAND()') > -1 ||
      ...
      if (
        oldCellValue &&
        oldCellValue.toString().indexOf('=UNIQUE(') > -1 &&
        this.editCellData.value === ''
      ) {
        this.parent.notify(removeUniquecol, null);
      }
      ...
      if (
        ...
        if (
          getCell(rangeIdx[0], rangeIdx[1], sheet)
            .value.toString()
            .indexOf('#SPILL!') === -1
        ) {
          return;
      ...
      if (
        ...
        oldCellValue.toString().indexOf('UNIQUE') > -1 &&
        this.editCellData.value &&
        this.editCellData.value.toString().indexOf('UNIQUE') > -1 &&
        isUniqueRange
      ) {
      ...
      const updatedCell: CellModel = getCell(
        cellIndex[0],
        cellIndex[1],
        sheet,
        true
      ...
      const isInvalidFormula: boolean =
        this.formulaErrorStrings.indexOf(cellValue) > -1 ||
        (cellValue && cellValue.includes('circular reference:'));
      if (isInvalidFormula) {
        delete updatedCell.value;
        delete updatedCell.formula;
        if (e) {
          const target: Element = e.target as Element;
          const ribbonCls: string[] = [
            'e-toolbar-item',
            'e-tab-wrap',
            'e-text-wrap',
            'e-tab-text',
          ...
          const skipAlertCls: string[] = [
            'e-scroller',
            'e-main-panel',
            'e-autofill',
          ];
          if (
            (!ribbonCls.some((cls: string) => target.classList.contains(cls)) ||
              !closest(target, '.e-ribbon')) &&
      ...
      if (sheet.conditionalFormats && sheet.conditionalFormats.length) {
        this.parent.notify(applyCF, <ApplyCFArgs>{
      ...
      if (isUniqueRange) {
        ...
        if (
          (oldCellValue !== '' && this.editCellData.value === '') ||
          (this.editCellData.formula &&
            this.editCellData.formula.length > 1 &&
  ...
  private checkUniqueRange(uniquArgs: {
    cellIdx: number[];
    isUnique: boolean;
    uniqueRange?: string;
    sheetName?: string;
          ...
          for (let k: number = rangeIdx[1]; k <= rangeIdx[3]; k++) {
            if (uniquArgs.cellIdx[0] === j && uniquArgs.cellIdx[1] === k) {
              ...
              const uniqueIndex: number[] =
                this.uniqueColl !== ''
                  ? getRangeIndexes(this.uniqueColl)
                  : [0, 0, 0, 0];
              const collectionIndex: number[] = getRangeIndexes(
                collection[i as number]
              );
              if (
                uniqueIndex[0] === collectionIndex[0] &&
                uniqueIndex[1] === collectionIndex[1]
              ) {
                const index: number[] = [
                  uniqueIndex[0],
                  collectionIndex[1],
                ...
                index[2] =
                  uniqueIndex[2] > collectionIndex[2]
                    ? uniqueIndex[2]
                    : collectionIndex[2];
                index[3] =
                  uniqueIndex[3] > collectionIndex[3]
                    ? uniqueIndex[3]
                    : collectionIndex[3];
  ...
  private updateUniqueRange(value: string): void {
    ...
    let skip: boolean = false;
    if (
      getCell(rangeIdx[0], rangeIdx[1], this.parent.getActiveSheet()).value !==
      '#SPILL!'
    ) {
      skip = true;
    }
      ...
      for (let k: number = rangeIdx[1]; k <= rangeIdx[3]; k++) {
        if (skip) {
          if (j === rangeIdx[0] && k === rangeIdx[1]) {
            ...
            if (
              getRangeAddress([j, k]).split(':')[0] === this.editCellData.addr
            ) {
  ...
  private reApplyFormula(): void {
    ...
    this.parent.updateCell(
      { value: '' },
      getRangeAddress([cellIdx[0], cellIdx[1]])
    );
  ...
  private refreshDependentCellValue(
    ...
    if (rowIdx && colIdx) {
      ...
      colIdx--;
      if (
        ((this.editCellData.rowIndex !== rowIdx ||
          this.editCellData.colIndex !== colIdx) &&
          this.parent.activeSheetIndex === sheetIdx) ||
        (this.uniqueCell && this.parent.activeSheetIndex === sheetIdx)
      ) {
        ...
        if (td) {
          if (td.parentElement) {
            const curRowIdx: string =
              td.parentElement.getAttribute('aria-rowindex');
  ...
  public endEdit(
    refreshFormulaBar: boolean = false,
    ...
    const triggerEventArgs: CellEditEventArgs =
      this.triggerEvent('beforeCellSave');
    ...
    if (
      triggerEventArgs.value &&
      triggerEventArgs.value.toString().indexOf('\n') > -1
    ) {
      const cell: CellModel = getCell(
        this.editCellData.rowIndex,
        this.editCellData.colIndex,
      ...
      wrapText(
        this.parent.getActiveSheet().selectedRange,
        cell ? (cell.wrap === false ? false : true) : true,
        this.parent as Workbook
      );
      this.refreshEditor(
        triggerEventArgs.value,
  ...
  public cancelEdit(
    refreshFormulaBar: boolean = true,
    trigEvent: boolean = true,
    event?: (MouseEvent & TouchEvent) | KeyboardEventArgs,
    isInternal?: boolean
  ): void {
    this.refreshEditor(
      this.editCellData.oldValue,
      refreshFormulaBar,
      false,
      false,
      false
  ...
  private focusElement(e?: KeyboardEventArgs): void {
    if (
      ...
      const focusEle: HTMLElement = this.parent.element.querySelector(
        `.e-formula-bar-panel ${
          e.shiftKey ? '.e-insert-function' : '.e-combobox'
        }`
      ) as HTMLElement;
      if (focusEle) {
        focus(focusEle);
  ...
  private altEnter(): void {
    ...
    offset =
      node.nodeType === 3 ? selection.anchorOffset : node.textContent.length;
  ...
  private resetEditState(elemRefresh: boolean = true): void {
    if (elemRefresh) {
      ...
      if (
        checkIsFormula(editorElem.textContent) ||
        editorElem.textContent === ''
      ) {
    ...
    if (this.parent.showSheetTabs && !this.parent.isProtected) {
      const addSheetBtn: Element =
        this.parent.element.querySelector('.e-add-sheet-tab');
      if (addSheetBtn) {
  ...
  private initiateRefSelection(): void {
    ...
    if (
      this.parent.getActiveSheet().name === sheetName &&
      checkIsFormula(this.editCellData.value, true)
    ) {
  ...
  private addressHandler(args: {
    ...
    const dlgInst: { element: Element } =
      this.parent.serviceLocator.getService<Dialog>(dialog).dialogInstance;
    if (
      dlgInst &&
      dlgInst.element &&
      dlgInst.element.classList.contains('e-validation-error-dlg')
    ) {
    ...
    const sheetIdx: number = this.editCellData.sheetIndex;
    const editorEle: HTMLElement = this.getEditElement(
      this.parent.getActiveSheet()
    );
    ...
    if (args.isSelect) {
      ...
      let startVal: string;
      let endVal: string;
      const editorContent: string = document.activeElement.classList.contains(
        'e-formula-bar'
      )
        ? (document.activeElement as HTMLTextAreaElement).value
      ...
      if (args.isMouseDown && editorContent !== editedValue) {
        startVal =
          editorContent.substring(0, this.curEndPos) +
          this.parent.listSeparator;
  ...
  private updateFormulaBarValue(args: {
    isMouseDown?: boolean;
    formulaBarVal?: string;
  }): void {
    const value: string = this.editCellData.value;
    const lastRange: string[] = this.parent
      .getActiveSheet()
      .selectedRange.split(' ');
    let address: string = lastRange[lastRange.length - 1];
    address = isSingleCell(getIndexesFromAddress(address))
      ? address.split(':')[0]
      : address;
    const formulaBar: HTMLTextAreaElement = this.parent.element.querySelector(
      '.e-formula-bar'
    ...
    if (value && checkIsFormula(value, true)) {
      ...
      if (
        ...
        lastRange.length > 1 &&
        args.formulaBarVal &&
        args.formulaBarVal !== value
      ) {
        startVal =
          args.formulaBarVal.substring(0, this.curEndPos) +
          this.parent.listSeparator;
  ...
  private initiateCurPosition(
    args: { isCellEdit: boolean } = { isCellEdit: false }
  ): void {
    const el: HTMLElement = this.getEditElement(
      this.parent.getActiveSheet(),
      true
    );
    if (args.isCellEdit) {
    ...
    if (sheetIdx !== this.parent.activeSheetIndex) {
      const elem: HTMLTextAreaElement = this.parent.element.querySelector(
        '.e-formula-bar'
      ) as HTMLTextAreaElement;
      if (elem.value) {
        if (elem.value.indexOf(')') === this.curEndPos - 1) {
          this.setFormulaBarCurPosition(
            elem,
            this.curEndPos - 1,
            this.curEndPos - 1
          );
  ...
  private getEditElement(sheet: SheetModel, isEdit?: boolean): HTMLElement {
    if ((this.isEdit || isEdit) && (sheet.frozenRows || sheet.frozenColumns)) {
      ...
      let editEle: HTMLElement = content.getElementsByClassName(
        'e-spreadsheet-edit'
      )[0] as HTMLElement;
      if (!editEle && isEdit) {
        editEle = content.appendChild(
          this.editorElem.cloneNode()
        ) as HTMLElement;
      }
      return editEle;
    }
  ...
  private showFormulaAlertDlg(errorString: string): void {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const alertDialog: Dialog = this.parent.serviceLocator.getService(
      'dialog'
    ) as Dialog;
    ...
    alertDialog.show(
      {
        ...
        close: (): void => {
          const elem: HTMLElement = this.getEditElement(
            this.parent.getActiveSheet()
          );
          const selection: Selection = document.getSelection();
          const range: Range = document.createRange();
  ...
  private readOnlyAlertHandler(): void {
    ...
    const findDialog: HTMLElement = this.parent.element.querySelector(
      '.e-find-dlg'
    ) as HTMLElement;
