...
import { detach, isUndefined } from '@hoyui/base';
...
export class ShowHide {
  ...
  private hideShow(args: HideShowEventArgs): void {
    const sheetIndex: number = isUndefined(args.sheetIndex)
    ...
    if (
      isReadOnlyCells(
        this.parent,
        getRangeIndexes(this.parent.getActiveSheet().selectedRange)
      )
    ) {
    ...
    const performHideShow: Function = (updateViewport?: boolean): void => {
      if (args.isCol) {
        this.hideCol(args);
        if (updateViewport) {
          this.parent.sheetModule.colWidthChanged({
        ...
        this.hideRow(args);
        if (updateViewport) {
          this.parent.sheetModule.rowHeightChanged({
    ...
    const freezePane: number = args.isCol
  ...
  private hideRow(eventArgs: HideShowEventArgs): void {
    ...
    const isFinite: boolean =
      this.parent.scrollSettings.isFinite && !args.freezePane;
    let height: number;
    if (isFinite) {
    ...
    if (args.hide) {
      let content: HTMLTableElement;
      let rowHdr: HTMLTableElement;
      ...
      const updateBtmIdx: boolean =
        isFinite &&
        args.endIndex === skipHiddenIdx(sheet, sheet.rowCount - 1, false);
      for (let i: number = args.startIndex; i <= args.endIndex; i++) {
        ...
        if (row) {
          if (!merge) {
            for (let j: number = 0; j <= sheet.usedRange.colIndex; j++) {
              ...
              if (
                ...
                (args.startIndex >= this.parent.viewport.topIndex ||
          ...
          if (row && i === args.endIndex) {
            ...
            const first: string =
              nextIdx !== skipHiddenIdx(sheet, 0, true) &&
              nextIdx ===
                (this.parent.viewport.topIndex >= args.startIndex
                  ? args.endIndex + 1
                  : this.parent.viewport.topIndex)
                ? 'Row'
                : '';
            for (
              let j: number = this.parent.viewport.leftIndex;
              j <= this.parent.viewport.rightIndex;
              j++
            ) {
              ...
              if (borderTop !== '') {
                ...
                this.parent.notify(applyCellFormat, <CellFormatArgs>{
                  ...
                  row: row,
                  pRow: <HTMLElement>row.previousElementSibling,
                  first: first,
                  td: cell,
                });
              }
      ...
      if (!args.freezePane && this.parent.scrollSettings.enableVirtualization) {
        ...
        if (
          args.startIndex < getCellIndexes(sheet.paneTopLeftCell)[0] ||
          count > this.parent.viewport.rowCount
        ) {
          if (args.isFiltering) {
            eventArgs.refreshUI = true;
            return;
          ...
          this.parent.updateTopLeftCell(
            skipHiddenIdx(
              sheet,
              args.startIndex - 1 < frozenRow ? frozenRow : args.startIndex - 1,
              true
            ) - frozenRow,
            null,
            'col'
          ...
          if (isFinite) {
            if (startIndex >= sheet.rowCount) {
              if (
                ...
                skipHiddenIdx(sheet, frozenRow, true)
              ) {
                updateHideClass();
          ...
          this.parent.renderModule.refreshUI(
            ...
            `${getCellAddress(startIndex, colIndex)}:${getCellAddress(
              endIndex,
      ...
      let hFrag: DocumentFragment;
      let frag: DocumentFragment;
      let hRow: Element;
      let row: Element;
      let newStartRow: number;
      let rowRenderer: IRowRenderer;
      let content: HTMLTableElement;
      let rowHdr: HTMLTableElement;
      let startRow: number;
      ...
      if (isFinite) {
        ...
        if (this.parent.viewport.bottomIndex === lastIdx) {
          if (
            ...
            const topLeftCell: number = getRangeIndexes(
            ...
            if (
              count <
              this.parent.viewport.rowCount +
                Math.round(this.parent.getThreshold('row') / 2)
            ) {
      ...
      for (
        let i: number = args.startIndex, len: number = args.endIndex;
        i <= len;
        i++
      ) {
        if (!isHiddenRow(sheet, i)) {
        ...
        if (idx === undefined) {
          ...
          rowRenderer =
            this.parent.serviceLocator.getService<IRowRenderer>('row');
          if (args.freezePane) {
            rowHdr = this.parent.sheetModule.getSelectAllTable();
            content = this.parent.getColHeaderTable();
          } else {
        ...
        if (content && content.rows.length && !skipDetach) {
          ...
          if (direction === 'firstElementChild') {
            ...
            this.parent.viewport.topIndex = skipHiddenIdx(
              sheet,
              this.parent.viewport.topIndex + 1,
              true
            );
          }
        }
        for (
          ...
          if (cell.rowSpan !== undefined || cell.colSpan !== undefined) {
            ...
            if (
              !mergeCollection.length ||
              mergeArgs.range[1] !==
                mergeCollection[mergeCollection.length - 1].range[1] ||
              mergeArgs.range[0] !==
                mergeCollection[mergeCollection.length - 1].range[0]
            ) {
      ...
      if (sheet.showHeaders) {
        ...
        if (isHiddenRow(sheet, endRow + 1)) {
          ...
          if (rowHdr.rows[idx as number]) {
            rowHdr.rows[idx as number].classList.remove('e-hide-end');
          }
        }
      }
      if (row && content && content.rows[idx as number]) {
        ...
        for (
          let i: number = this.parent.viewport.leftIndex;
          ...
          if (borderTop !== '') {
            this.parent.notify(applyCellFormat, <CellFormatArgs>{
              ...
              colIdx: i,
              style: { borderTop: borderTop },
              pRow: <HTMLElement>row,
              td: content.rows[idx as number].cells[i as number],
              first: '',
            });
            ...
            if (prevIdx > -1) {
              if (
                content.rows[idx - 1] &&
                !this.parent.getCellStyleValue(['borderBottom'], [prevIdx, i])
                  .borderBottom &&
                !this.parent.getCellStyleValue(['borderTop'], [startRow, i])
                  .borderTop
              ) {
                (
                  content.rows[idx - 1].cells[i as number] as HTMLElement
                ).style.borderBottom = '';
              }
            } else {
              (
                content.rows[idx as number].cells[i as number] as HTMLElement
              ).style.borderTop = '';
            }
          }
        }
  ...
  private hideCol(args: HideShowEventArgs): void {
    ...
    const skipColCount: number = skipHiddenIdx(
      sheet,
      sheet.colCount - 1,
      false,
      'columns'
        ...
        (i < viewportLeftIdx ||
          (i > this.parent.viewport.rightIndex &&
            (!this.parent.scrollSettings.isFinite ||
              !(
                skipColCount === this.parent.viewport.rightIndex &&
                i >= skipColCount &&
                i < sheet.colCount
              ))))
      ) {
    ...
    let hTable: HTMLTableElement;
    const getRowIndexes: Function = (): number[] => {
      let idx: number[];
      if (this.parent.scrollSettings.enableVirtualization) {
        idx = [
          frozenRow ? topLeftIdx[0] : this.parent.viewport.topIndex,
          this.parent.viewport.bottomIndex,
        ];
      } else {
    ...
    if (args.hide) {
      ...
      if (
        hiddenIndex.length <= this.parent.getThreshold('col') ||
        !this.parent.scrollSettings.enableVirtualization ||
        args.freezePane
      ) {
      ...
      if (!args.freezePane && this.parent.scrollSettings.enableVirtualization) {
        if (scrollable) {
          this.parent.updateTopLeftCell(
            ...
            skipHiddenIdx(
              sheet,
              hiddenIndex[0] - 1 < frozenCol ? frozenCol : hiddenIndex[0] - 1,
              true,
              'columns'
            ) - frozenCol,
            'row'
        ...
        if (this.parent.scrollSettings.isFinite) {
          const colCount: number = skipHiddenIdx(
            sheet,
            sheet.colCount - 1,
            false,
            'columns'
          ...
          if (endIndex + hiddenIndex.length >= colCount) {
            const index: number = skipHiddenIdx(
              sheet,
              startIdx - (endIndex + hiddenIndex.length - colCount),
              false,
              'columns'
            );
            if (index >= frozenCol) {
              this.parent.viewport.leftIndex = index;
              this.parent.viewport.leftIndex -= this.parent.hiddenCount(
                endIndex,
                colCount
              );
            }
        ...
        if (hiddenIndex.length <= this.parent.getThreshold('col')) {
          const indexes: number[] = this.parent.skipHidden(
            this.parent.viewport.rightIndex + 1,
            this.parent.viewport.rightIndex + hiddenIndex.length,
            'columns'
          );
      ...
      if (
        beforeViewportIdx.length &&
        this.parent.scrollSettings.enableVirtualization
      ) {
        beforeViewportIdx.sort((i: number, j: number): number => {
          return i - j;
        });
        const leftIdx: number =
          beforeViewportIdx[0] - 1 < frozenCol
            ? frozenCol
            : beforeViewportIdx[0] - 1;
        this.parent.updateTopLeftCell(
          null,
          skipHiddenIdx(sheet, leftIdx, true, 'columns') - frozenCol,
          'row'
        );
        this.parent.renderModule.refreshSheet(false, false, true);
        if (args.autoFit) {
      ...
      if (
        (!this.parent.scrollSettings.isFinite ||
          this.parent.viewport.rightIndex < skipColCount) &&
        (hiddenIndex.length <= this.parent.getThreshold('col') ||
          !this.parent.scrollSettings.enableVirtualization ||
          args.freezePane)
      ) {
  ...
  private removeCell(
    ...
    while (startIdx <= endIdx) {
      ...
      row =
        frozenRow && startIdx < frozenRow
          ? hTable.rows[rowIdx + 1]
          : table.rows[rowIdx as number];
        ...
        if (index === 0) {
          ...
          if (
            (cell.colSpan !== undefined &&
              (cell.rowSpan === undefined || cell.colSpan > 1)) ||
            (cell.rowSpan < 0 &&
              startIdx - 1 > -1 &&
              isHiddenRow(sheet, startIdx - 1) &&
              Math.abs(cell.rowSpan) ===
                this.parent.hiddenCount(
                  startIdx + cell.rowSpan,
                  startIdx,
                  'rows',
                  sheet
                ))
    ...
    if (sheet.showHeaders && hRow.cells[cellIdx as number]) {
      hRow.cells[cellIdx as number].classList.add('e-hide-end');
    }
  }
  private appendCell(
    sheet: SheetModel,
    ...
    const cellRenderer: ICellRenderer =
      this.parent.serviceLocator.getService<ICellRenderer>('cell');
    indexes.sort((i: number, j: number): number => {
      return i - j;
    });
    const mergeCollection: MergeArgs[] = [];
    const cellIdx: number[] = [];
    let cell: Element;
    let refCell: HTMLElement;
    ...
    while (startIdx <= endIdx) {
      ...
      indexes.forEach((idx: number, index: number): void => {
        if (rowIdx === 0) {
          ...
          if (sheet.showHeaders) {
            ...
            if (refCell) {
              if (
                index === 0 &&
                indexes[index as number] &&
                !isHiddenCol(sheet, indexes[index as number] - 1) &&
          ...
          if (startIdx >= frozenRow) {
            if (colgrp.children[cellIdx[index as number]]) {
              colgrp.insertBefore(
                this.parent.sheetModule.updateCol(sheet, idx),
        ...
        cellArgs = {
          ...
          first:
            idx !== skipHiddenIdx(sheet, 0, true, 'columns') &&
        ...
        if (index === 0 && cell.previousSibling) {
          ...
          if (borderLeft !== '') {
            ...
            if (
              prevIdx > -1 &&
              !this.parent.getCellStyleValue(['borderRight'], [rowIdx, prevIdx])
                .borderRight &&
              !this.parent.getCellStyleValue(
                ['borderLeft'],
                [rowIdx, indexes[0]]
              ).borderLeft
            ) {
        ...
        if (
          (cellModel.colSpan !== undefined &&
            (cellModel.rowSpan === undefined || cellModel.colSpan > 1)) ||
          (cellModel.rowSpan < 0 &&
            startIdx - 1 > -1 &&
            isHiddenRow(sheet, startIdx - 1) &&
            Math.abs(cellModel.rowSpan) ===
              this.parent.hiddenCount(
                startIdx + cellModel.rowSpan,
                startIdx,
                'rows',
                sheet
              ))
    ...
    this.parent.viewport.rightIndex = skipHiddenIdx(
      sheet,
      this.parent.viewport.rightIndex - indexes.length,
      false,
      'columns'
    );
  }
