...
import { extend, isNullOrUndefined, L10n, removeClass } from '@hoyui/base';
...
export class ConditionalFormatting {
  ...
  private clearCF(args: { indexes: number[] }): void {
    ...
    const clearFn: Function = (
      ...
      removeClass(
        [td],
        ['e-redft', 'e-yellowft', 'e-greenft', 'e-redf', 'e-redt', 'e-iconset']
      );
      let styleVal: string;
      ...
      ['backgroundColor', 'color'].forEach((styleProp: string): void => {
        if (td.style[`${styleProp}`]) {
          td.style[`${styleProp}`] = '';
          styleVal =
            (cell && cell.style && cell.style[`${styleProp}`]) ||
            this.parent.commonCellStyle[`${styleProp}`];
          if (styleVal) {
            style = {};
      ...
      ['.e-cf-databar', '.e-iconsetspan'].forEach(
        (clsSelector: string): void => {
    ...
    this.updateRange(
      ...
      getCellIndexes(sheet.topLeftCell),
      clearFn
    );
  }
  ...
  private dialogBeforeClose(): void {
    ...
    if (this.value1Inp) {
      this.value1Inp.removeEventListener(
        'input',
        this.validateCFInput.bind(this)
      );
    ...
    if (this.value2Inp) {
      this.value2Inp.removeEventListener(
        'input',
        this.validateCFInput.bind(this)
      );
  ...
  private dlgClickHandler(action: string): void {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const cont: HTMLElement = this.parent.element.querySelector(
      '.e-conditionalformatting-dlg .e-dlg-content .e-cf-dlg'
    ) as HTMLElement;
    const cf: ConditionalFormatModel = {
      cFColor: this.getCFColor(
        (cont.querySelector('.e-cfsub .e-input') as HTMLInputElement).value
      ),
      range: this.parent.getActiveSheet().selectedRange,
    };
    const cfInputs: NodeListOf<HTMLInputElement> = cont.querySelectorAll(
      '.e-cfmain .e-input'
    ) as NodeListOf<HTMLInputElement>;
    if (action === l10n.getConstant('DuplicateValues') + '...') {
      cf.type =
        cfInputs[0].value === l10n.getConstant('Duplicate')
          ? 'Duplicate'
          : 'Unique';
    } else {
  ...
  private cfDlgContent(action: string): HTMLElement {
    ...
    const subDiv: HTMLElement = this.parent.createElement('div', {
      className: 'e-cfsub',
    });
    ...
    this.value1Inp = this.parent.createElement('input', {
      className: 'e-input',
      id: 'valueInput',
      attrs: { type: 'text', 'aria-label': dlgText },
    }) as HTMLInputElement;
    const duplicateSelectEle: HTMLElement = this.parent.createElement('input', {
      className: 'e-select',
    });
    const subDivText: HTMLElement = this.parent.createElement('span', {
      className: 'e-header',
    });
    subDivText.innerText = l10n.getConstant('With');
    const colorSelectEle: HTMLElement = this.parent.createElement('input', {
      className: 'e-select',
    });
    ...
    let setValidation: boolean;
    if (action !== l10n.getConstant('DuplicateValues') + '...') {
      if (
        action !== l10n.getConstant('AboveAverage') + '...' &&
        action !== l10n.getConstant('BelowAverage') + '...'
      ) {
        mainDiv.appendChild(this.value1Inp);
        setValidation = true;
        const percent: boolean =
          action === l10n.getConstant('Top10') + ' %...' ||
          action === l10n.getConstant('Bottom10') + ' %...';
        if (
          action === l10n.getConstant('Top10Items') + '...' ||
          action === l10n.getConstant('Bottom10Items') + '...' ||
          percent
        ) {
    ...
    if (action === l10n.getConstant('Between') + '...') {
      const value2Text: HTMLElement = this.parent.createElement('span', {
  ...
  private validateCFInput(e: { target: HTMLElement }): void {
    ...
    const btn: HTMLButtonElement =
      dialogInst.dialogInstance.element.getElementsByClassName(
        'e-primary'
      )[0] as HTMLButtonElement;
    if (
      text &&
      (e.target.classList.contains('e-between') ||
        e.target.parentElement.querySelector('.e-between'))
    ) {
      text = (
        e.target.parentElement.querySelector(
          e.target.classList.contains('e-between') ? '.e-input' : '.e-between'
        ) as HTMLInputElement
      ).value;
    }
  ...
  private updateResult(
    ...
    const dupValueObj: { [key: string]: boolean } = {};
  ...
  private applyCF(args: ApplyCFArgs): void {
    ...
    const cfRule: ConditionalFormatModel[] =
      args.cfModel || sheet.conditionalFormats;
    ...
    for (let i: number = cfRule.length - 1; i >= 0; i--) {
      ...
      this.updateCF(
        args,
        sheet,
        <ConditionalFormat>cfRule[i as number],
        updatedCFCellRef
      );
    }
  }
  ...
  private updateCF(
    ...
    sheet: SheetModel,
    cf: ConditionalFormat,
    updatedCFCellRef: { [key: string]: boolean }
  ): void {
    let value1: string;
    let value2: string;
    if (cf.value) {
      const valueArr: string[] = cf.value
        .split(',')
      ...
      if (valueArr.length > 1) {
        if (valueArr[0].split('(').length > 1) {
          ...
          for (let idx: number = 0; idx < valueArr.length; idx++) {
            ...
            if (
              valueStr.split('(').length === valueStr.split(')').length &&
              value1 === undefined
            ) {
          ...
          for (let idx: number = 1; idx < valueArr.length; idx++) {
            value2 +=
              idx + 1 === valueArr.length
                ? valueArr[idx as number]
                : valueArr[idx as number] + ',';
          }
        }
      } else {
    ...
    const isIconSets: boolean =
      cf.type.includes('Three') ||
      cf.type.includes('Four') ||
      cf.type.includes('Five');
    const isDataBar: boolean = cf.type.includes('DataBar');
    const isColorScale: boolean = cf.type.includes('ColorScale');
    if (
      (!args.isRender || !cf.result) &&
      (cf.type === 'Duplicate' ||
        cf.type === 'Unique' ||
        isDataBar ||
        isColorScale ||
    ...
    const updateCF: Function = (
      ...
      const cellVal: string =
        cell && !isNullOrUndefined(cell.value) ? cell.value.toString() : '';
      let isApply: boolean;
      ...
      switch (cf.type) {
        ...
        case 'Between':
          isApply =
            isNumber(cellVal) && this.isBetWeen(cf, cellVal, value1, value2);
          break;
        case 'EqualTo':
        ...
        case 'AboveAverage':
          isApply =
            cf.result.length && parseFloat(cellVal) > <number>cf.result[0];
          break;
        case 'BelowAverage':
          isApply =
            cf.result.length && parseFloat(cellVal) < <number>cf.result[0];
        ...
        default:
          ...
          if (isDataBar) {
            ...
            if (!updatedCFCellRef[`${rIdx}_${cIdx}`]) {
              ...
              if (isNaN(value)) {
                if (
                  ...
                  !td.classList.contains('e-yellowft') &&
                  !td.classList.contains('e-greenft') &&
                  !td.classList.value.includes('e-redf')
                ) {
                  td.style.backgroundColor = '';
                  const style: CellStyleModel = extend(
                    {},
                    this.parent.commonCellStyle,
                    cell && cell.style
                  );
            ...
            if (!updatedCFCellRef[`${rIdx}_${cIdx}_icons`]) {
              ...
              const cfIcon: HTMLElement = this.parent.createElement('span', {
                className: 'e-icon e-iconsetspan',
              });
              const iconSetUpdated: boolean = this.applyIconSet(
                cellVal,
                cf,
                td,
                cfIcon
      ...
      if (args.isAction && isValueCFRule) {
        this.parent.trigger('beforeConditionalFormat', <
          ConditionalFormatEventArgs
        >{
      ...
      if (isApply && !updatedCFCellRef[`${rIdx}_${cIdx}`]) {
        ...
        removeClass(
          [td],
          ['e-redft', 'e-yellowft', 'e-greenft', 'e-redf', 'e-redt']
        );
        if (cf.cFColor) {
    ...
    if (args.ele) {
      ...
      for (let i: number = 0; i < rangeArr.length; i++) {
        this.updateRange(
          sheet,
          getRangeIndexes(rangeArr[i as number]),
          frozenRow,
          frozenCol,
          topLeftIdx,
          updateCF
  ...
  private updateRange(
    ...
    rangeIdx[0] =
      rangeIdx[0] < frozenRow
        ? rangeIdx[0] < topLeftIdx[0]
          ? topLeftIdx[0]
          : rangeIdx[0]
        : rangeIdx[0] < this.parent.viewport.topIndex + frozenRow
        ? this.parent.viewport.topIndex + frozenRow
        : rangeIdx[0];
    rangeIdx[1] =
      rangeIdx[1] < frozenCol
        ? rangeIdx[1] < topLeftIdx[1]
          ? topLeftIdx[1]
          : rangeIdx[1]
        : rangeIdx[1] < this.parent.viewport.leftIndex + frozenCol
        ? this.parent.viewport.leftIndex + frozenCol
        : rangeIdx[1];
    rangeIdx[2] =
      rangeIdx[2] < frozenRow
        ? rangeIdx[2] < topLeftIdx[0]
          ? topLeftIdx[0] - 1
          : rangeIdx[2]
        : rangeIdx[2] > this.parent.viewport.bottomIndex
        ? this.parent.viewport.bottomIndex
        : rangeIdx[2];
    rangeIdx[3] =
      rangeIdx[3] < frozenCol
        ? rangeIdx[3] < topLeftIdx[1]
          ? topLeftIdx[1] - 1
          : rangeIdx[3]
        : rangeIdx[3] > this.parent.viewport.rightIndex
  ...
  private applyIconSet(
    ...
    if (iconList.length === 3) {
      ...
      const percent2: number = min + 0.5 * (max - min);
      ...
      const percent1: number = min + 0.2 * (max - min);
      const percent2: number = min + 0.4 * (max - min);
      const percent3: number = min + 0.6 * (max - min);
      const percent4: number = min + 0.8 * (max - min);
      currentSymbol =
        'e-' +
        (value >= percent4
          ? iconList[0].trim()
  ...
  private applyDataBars(
    ...
    const sheet: SheetModel = this.parent.getActiveSheet();
    const result: number[] = cf.result as number[];
    ...
    const databar: HTMLElement = this.parent.createElement('div', {
      id: 'spreadsheet-databar',
      className: 'e-cf-databar',
    });
    const leftSpan: HTMLElement = this.parent.createElement('span', {
      id: 'spreadsheet-leftspan',
      className: 'e-databar',
    });
    const rightSpan: HTMLElement = this.parent.createElement('span', {
      id: 'spreadsheet-rightspan',
      className: 'e-databar',
    });
    const dataSpan: HTMLElement = this.parent.createElement('span', {
    ...
    if (result[1] === undefined) {
      rightSpan.style.width =
        '' + Math.ceil(Math.abs((value / topVal) * 100)) + '%';
    ...
    const curEle: HTMLElement = td.querySelector(
      `#${this.parent.element.id}_currency`
    );
    if (curEle) {
  ...
  private getGradient(
    ...
    if (isNullOrUndefined(end)) {
      ...
      return t >= center
        ? this.getLinear(middle, end, Math.abs((t - center) / center))
        : this.getLinear(start, middle, t / center);
    }
  }
  ...
  private isBetWeen(
    ...
    if (isNumber(input1)) {
      let firstVal: number = parseFloat(input1);
      let secondVal: number = parseFloat(input2);
      if (firstVal > secondVal) {
        [firstVal, secondVal] = [secondVal, firstVal];
      }
      return parseFloat(value) >= firstVal && parseFloat(value) <= secondVal;
    } else if (input1 && input2) {
      const dateEventArgs1: DateFormatCheckArgs = {
        value: input1,
        cell: {},
        updatedVal: '',
      };
      const dateEventArgs2: DateFormatCheckArgs = {
        value: input2,
        cell: {},
        updatedVal: '',
      };
      ...
      if (
        (dateEventArgs1.isDate || dateEventArgs1.isTime) &&
        (dateEventArgs2.isDate || dateEventArgs2.isTime)
      ) {
        cf.value = dateEventArgs1.updatedVal + ',' + dateEventArgs2.updatedVal;
        if (dateEventArgs1.updatedVal > dateEventArgs2.updatedVal) {
          [dateEventArgs1.updatedVal, dateEventArgs2.updatedVal] = [
            dateEventArgs2.updatedVal,
            dateEventArgs1.updatedVal,
          ];
        }
        return (
          value >= dateEventArgs1.updatedVal &&
          value <= dateEventArgs2.updatedVal
        );
      } else {
        return (
          value.toLowerCase() >= input1.toLowerCase() &&
          value.toLowerCase() <= input2.toLowerCase()
        );
      }
    }
    return false;
  ...
  private isEqualTo(
    ...
    if (isNumber(input)) {
      ...
      const dateTimeArgs: DateFormatCheckArgs = {
        value: input,
        cell: {},
        updatedVal: '',
      };
