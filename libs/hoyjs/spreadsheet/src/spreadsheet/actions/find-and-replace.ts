...
import { Toolbar, ItemModel, ClickEventArgs } from '@hoyui/navigations';
import {
  DialogModel,
  Dialog as FindDialog,
...
export class FindAndReplace {
  ...
  private findToolDlg(args: {
    event?: MouseEvent;
    findValue?: string;
    isPublic?: boolean;
    headerHgt?: number;
    refreshPosition?: boolean;
    dialogEle?: HTMLElement;
  }): void {
    const updateDisableState: (disable: boolean) => void = (
      disable: boolean
    ): void => {
      const ribbon: HTMLElement =
        this.parent.showRibbon &&
        (this.parent.element.querySelector('.e-ribbon') as HTMLElement);
      if (ribbon) {
        const findBtn: HTMLButtonElement = (
          args.event && args.event.target
            ? closest(
                args.event.target as Element,
                `#${this.parent.element.id}_findbtn`
              )
    ...
    let findTextInput: HTMLInputElement;
    let findSpan: HTMLElement;
    const findHandlerFn: Function = (e?: KeyboardEvent): void => {
      ...
      if (e && e.keyCode === 13) {
        if (findTextInput.value && findSpan.textContent !== '0 of 0') {
          this.parent.notify(findHandler, {
        ...
        if (inputValue === '') {
          ...
          const countArgs: { [key: string]: string | number | boolean } = {
            value: inputValue,
            mode: 'Sheet',
            isCSen: false,
    ...
    if (dialogDiv) {
      if (args.isPublic || args.refreshPosition) {
        if (args.isPublic) {
          findTextInput = dialogDiv.querySelector(
            '.e-text-findNext-short'
          ) as HTMLInputElement;
          findTextInput.value = this.findValue;
          findSpan = dialogDiv.querySelector(
            '.e-input-group-icon'
          ) as HTMLElement;
          toolbarObj = getComponent(
            dialogDiv.querySelector('.e-find-toolbar') as HTMLElement,
            'toolbar'
          ) as Toolbar;
          findHandlerFn();
        }
        if (args.refreshPosition) {
          ...
          if (this.parent.getActiveSheet().showHeaders) {
            const hdrPanel: HTMLElement =
              this.parent.getColumnHeaderContent().parentElement;
      ...
      const findTextElement: HTMLElement = this.parent.createElement('div', {
      ...
      const largeData: boolean =
        sheet.usedRange.rowIndex * sheet.usedRange.colIndex > 100;
      ...
      toolbarObj = new Toolbar({
        clicked: (args: ClickEventArgs): void => {
          if (args.item.cssClass === 'e-findRib-next') {
            ...
            this.findDialog.setProperties(
              { animationSettings: this.findDialog.animationSettings },
              true
            );
            this.renderFindDlg();
            this.findDialog.hide();
        ...
        created: (): void => {
          const tbarBtns: NodeList = toolbarObj.element.querySelectorAll(
            '.e-toolbar-item .e-tbar-btn'
          );
          tbarBtns.forEach((tbarBtn: HTMLElement): void =>
            tbarBtn.removeAttribute('tabindex')
          );
        },
      });
      const tbarEle: HTMLElement = this.parent.createElement('div', {
      ...
      dialogDiv = this.parent.createElement('div', {
        className: 'e-dlg-div',
        attrs: { 'aria-label': l10n.getConstant('FindValue') },
      });
      ...
      const findDlgModel: DialogModel = {
        ...
        open: (): void => {
          ...
          const inputContainer: HTMLElement = toolbarObj.element.querySelector(
            '.e-input-group'
          ) as HTMLElement;
          if (inputContainer) {
            inputContainer.addEventListener('focus', (): void => {
              const textInput: HTMLInputElement =
                toolbarObj.element.querySelector('.e-text-findNext-short');
              focus(textInput);
        ...
        created: (): void => {
          ...
          dialogDiv.style.width =
            this.parent.getMainContent().offsetWidth + 'px';
          dialogDiv.style.visibility = 'hidden';
          dialogDiv.style.display = 'block';
          this.findDialog.width =
            parseInt(getComputedStyle(dialogDiv).borderLeftWidth, 10) * 2 +
            dialogDiv.querySelector('.e-toolbar-items').getBoundingClientRect()
              .width +
            'px';
          ...
          dialogDiv.style.top = `${
            (args && args.headerHgt) ||
            this.parent.getColumnHeaderContent().parentElement.offsetHeight + 1
          }px`;
          dialogDiv.style.left = '';
          dialogDiv.style[
            this.parent.enableRtl ? 'left' : 'right'
          ] = `${this.parent.sheetModule.getScrollSize()}px`;
  ...
  private refreshFindDlg(): void {
    ...
    if (findDialog) {
      const findToolInput: HTMLInputElement = findDialog.querySelector(
        '.e-text-findNext-short'
      ) as HTMLInputElement;
      this.findToolDlg({
  ...
  private closeDialog(e: MouseEvent & TouchEvent): void {
    if (
      (closest(e.target as Element, '.e-findRib-close') ||
        !closest(e.target as Element, '.e-spreadsheet')) &&
      this.findDialog
    ) {
      this.findToolDlg({});
    }
  ...
  private renderFindDlg(): void {
    ...
    if (!this.parent.element.querySelector('.e-find-dlg')) {
      ...
      const dlg: DialogModel = {
        ...
        open: (): void => {
          const findInput: HTMLInputElement = this.parent.element.querySelector(
            '.e-text-findNext'
          ) as HTMLInputElement;
          if (findInput.value) {
            const prevButton: HTMLElement = this.parent.element.querySelector(
              '.e-btn-findPrevious'
            ) as HTMLElement;
            const prevButtonObj: Button = getComponent(
              prevButton,
              'btn'
            ) as Button;
  ...
  private textFocus(): void {
    const element: HTMLElement =
      this.parent.element.querySelector('.e-text-goto');
  ...
  private findHandler(findOpt?: ToolbarFind): void {
    let findInput: HTMLInputElement = this.parent.element.querySelector(
      '.e-text-findNext'
    ) as HTMLInputElement;
    if (!findInput) {
      findInput = this.parent.element.querySelector(
        '.e-text-findNext-short'
      ) as HTMLInputElement;
      if (!findInput) {
        this.gotoAlert();
    ...
    if (findInput.value !== '') {
      ...
      const checkCase: HTMLElement = this.parent.element.querySelector(
        '.e-findnreplace-checkcase'
      ) as HTMLElement;
      let isCSen: boolean;
      if (!checkCase) {
        isCSen = false;
      ...
      const checkmatch: HTMLElement = this.parent.element.querySelector(
      ...
      if (!checkmatch) {
        ...
        const entireMatchCheckbox: CheckBox = getComponent(
          checkmatch,
          'checkbox'
        ) as CheckBox;
        isEMatch = entireMatchCheckbox.checked;
      }
      const searchitem: HTMLElement = this.parent.element.querySelector(
        '.e-findnreplace-searchby'
      ) as HTMLElement;
      let searchBy: string;
      if (!searchitem) {
        searchBy = 'By Row';
      } else {
        const searchDDL: DropDownList = getComponent(
          searchitem,
          'dropdownlist'
        ) as DropDownList;
        searchBy = searchDDL.value.toString();
      }
      const modeitem: HTMLElement = this.parent.element.querySelector(
        '.e-findnreplace-searchwithin'
      ) as HTMLElement;
      let mode: string;
      if (!modeitem) {
        mode = 'Sheet';
      } else {
        const modeDDL: DropDownList = getComponent(
          modeitem,
          'dropdownlist'
        ) as DropDownList;
  ...
  private replaceHandler(action: string): void {
    ...
    const findValue: string = (
      dlg.querySelector('.e-text-findNext') as HTMLInputElement
    ).value;
    ...
    const searchInValue: string = (
      this.parent.element.querySelector(
        '.e-search-within .e-ddl-hidden'
      ) as HTMLInputElement
    ).value;
    const searchByValue: string = (
      this.parent.element.querySelector(
        '.e-searchby .e-ddl-hidden'
      ) as HTMLInputElement
    ).value;
    this.parent.notify(action, <FindOptions>{
      value: findValue,
      mode: searchInValue,
      isCSen: checkCase,
      isEMatch: checkmatch,
  ...
  private gotoHandler(address?: { [key: string]: string }): boolean {
    ...
    if (address) {
      ...
      let gotoAddress: string = (
        this.parent.element.querySelector('.e-text-goto') as HTMLInputElement
      ).value;
      for (
        let nameIdx: number = 0;
        nameIdx < this.parent.definedNames.length;
        nameIdx++
      ) {
        if (this.parent.definedNames[nameIdx as number].name === gotoAddress) {
          gotoAddress =
            this.parent.definedNames[nameIdx as number].refersTo.slice(1);
          break;
        }
      }
  ...
  private gotoAlert(): void {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const gotoSpan: HTMLElement = this.parent.createElement('span', {
    ...
    this.parent.element
      .querySelector('.e-goto-dlg')
      .querySelector('.e-dlg-content')
      .appendChild(gotoSpan);
  }
  ...
  private showFindAlert(): void {
    ...
    const replaceDlgCont: HTMLElement = this.parent.element.querySelector(
      '.e-find-dlg .e-dlg-content'
    );
    if (replaceDlgCont) {
      const findSpan: HTMLElement = this.parent.createElement('span', {
        className: 'e-find-alert-span',
      });
      findSpan.innerText = this.parent.serviceLocator
        .getService<L10n>(locale)
        .getConstant('NoElements');
  ...
  private replaceAllDialog(options: { [key: string]: number | string }): void {
    ...
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const replaceSpan: HTMLElement = this.parent.createElement('span', {
      className: 'e-replace-alert-span',
    });
    replaceSpan.innerText =
      options.count + l10n.getConstant('ReplaceAllEnd') + options.replaceValue;
    if (this.parent.element.querySelector('.e-find-dlg')) {
      this.parent.element
        .querySelector('.e-find-dlg')
        .querySelector('.e-dlg-content')
        .appendChild(replaceSpan);
    }
  }
  ...
  private findKeyUp(e: KeyboardEvent): void {
    if ((e.target as HTMLElement).classList.contains('e-text-findNext')) {
      ...
      if (!isNullOrUndefined(findValue) && findValue !== '') {
        ...
        (
          getComponent(
            this.parent.element.querySelector('.e-btn-findNext') as HTMLElement,
            'btn'
          ) as Button
        ).disabled = false;
      } else {
    ...
    const findValue: string = (
      this.parent.element.querySelector('.e-text-findNext') as HTMLInputElement
    ).value;
    const replaceValue: string = (
      this.parent.element.querySelector(
        '.e-text-replaceInp'
    ...
    if (
      !isNullOrUndefined(findValue) &&
      !isNullOrUndefined(replaceValue) &&
      findValue !== '' &&
      replaceValue !== ''
    ) {
      if (!this.parent.getActiveSheet().isProtected) {
        (
          getComponent(
            this.parent.element.querySelector('.e-btn-replace') as HTMLElement,
            'btn'
          ) as Button
        ).disabled = false;
        (
      ...
      (
        getComponent(
          this.parent.element.querySelector('.e-btn-replaceAll') as HTMLElement,
          'btn'
        ) as Button
      ).disabled = true;
    }
  ...
  private findandreplaceContent(): HTMLElement {
    ...
    const findElem: HTMLElement = this.parent.createElement('div', {
      className: 'e-find',
    });
    const findCheck: HTMLElement = this.parent.createElement('div', {
    ...
    const findTextE: HTMLElement = this.parent.createElement('div', {
      className: 'e-cont',
    });
    const findTextH: HTMLElement = this.parent.createElement('p', {
      className: 'e-header',
    });
    ...
    const replaceTextE: HTMLElement = this.parent.createElement('div', {
      className: 'e-cont',
    });
    const replaceTextH: HTMLElement = this.parent.createElement('p', {
      className: 'e-header',
    });
    ...
    const replaceTextIp: HTMLElement = this.parent.createElement('input', {
      ...
      attrs: { type: 'Text', placeholder: l10n.getConstant('ReplaceValue') },
    });
    ...
    const withIn: HTMLElement = this.parent.createElement('input', {
      ...
      attrs: { type: 'select', label: label },
    });
    const withinTextH: HTMLElement = this.parent.createElement('p', {
      className: 'e-header',
    });
    ...
    const searchTextH: HTMLElement = this.parent.createElement('p', {
      className: 'e-header',
    });
    ...
    const entirematchCheckbox: HTMLElement = this.parent.createElement(
      'input',
      {
  ...
  private GotoContent(): HTMLElement {
    ...
    const gotoElem: HTMLElement = this.parent.createElement('div', {
      className: 'e-goto',
    });
    dialogElem.appendChild(gotoElem);
    const gotoTextE: HTMLElement = this.parent.createElement('div', {
      className: 'e-cont',
    });
    const gotoTextH: HTMLElement = this.parent.createElement('p', {
      className: 'e-header',
    });
    ...
    const gotoTextIp: HTMLElement = this.parent.createElement('input', {
