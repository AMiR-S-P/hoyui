...
export class Selection {
  ...
  private addEventListener(): void {
    ...
    this.parent.on(
      virtualContentLoaded,
      this.virtualContentLoadedHandler,
      this
    );
  ...
  private isTouchSelectionStarted(args: {
    touchSelectionStarted: boolean;
  }): void {
    args.touchSelectionStarted = this.touchSelectionStarted;
  }

  private selectionByKeydown(args: {
    range: number[];
    e: KeyboardEvent;
  }): void {
  ...
  private rowHeightChanged(args: { threshold: number; rowIdx: number }): void {
    ...
    getUpdateUsingRaf((): void => {
      ...
      if (
        ...
        (sheet.frozenRows ||
          sheet.frozenColumns ||
          sheet.selectedRange.includes(' '))
      ) {
        this.selectRange({ address: sheet.selectedRange });
        return;
      }
      const sRange: number[] = getSwapRange(
        getRangeIndexes(sheet.selectedRange)
      );
      ...
      if (ele) {
        ...
        if (mergeArgs.isActiveCell) {
          ...
          isActiveCell =
            cell.rowSpan > 1 &&
            sRange[0] <= args.rowIdx &&
            sRange[2] >= args.rowIdx;
        }
        if (rowIdx === args.rowIdx || isActiveCell) {
          ele.style.height = `${
            parseFloat(ele.style.height) + args.threshold
          }px`;
      ...
      if (ele) {
        if (
          isActiveCell ||
          (sRange[0] === sRange[2] && sRange[1] === sRange[3])
        ) {
          return;
        }
    ...
    getUpdateUsingRaf((): void => {
      ...
      if (ele) {
        ...
        if (colIdx === args.colIdx || isActiveCell) {
          ...
          if (isRtl) {
            ele.style.right = `${
              parseFloat(ele.style.right) + args.threshold
            }px`;
          } else {
      ...
      const colStart: number = sRange[1];
      const colEnd: number = sRange[3];
  ...
  private selectRange(args: { address: string; skipChecking?: boolean }): void {
    args.address =
      this.parent.selectionSettings.mode === 'Single'
        ? getRangeAddress(getCellIndexes(args.address))
        : args.address;
  ...
  private selectMultiRange(
    ...
    if (address.indexOf('!') > -1) {
      sheetIdx = getSheetIndex(
        this.parent as Workbook,
        getSheetNameFromAddress(address)
      );
  ...
  private createSelectionElement(): void {
    ...
    let ele: Element = this.parent.createElement('div', {
  ...
  private mouseDownHandler(e: MouseEvent & TouchEvent): void {
    this.isNoteActiveElement =
      !isNullOrUndefined(document) &&
      !isNullOrUndefined(document.activeElement) &&
      !isNullOrUndefined(document.activeElement.className) &&
      document.activeElement.className.indexOf('e-addNoteContainer') > -1;
    if (
      closest(e.target as Element, '.e-scrollbar') ||
      (e.target as Element).classList.contains('e-main-panel') ||
      (e.target as Element).classList.contains('e-sheet')
    ) {
      return;
    }
    const eventArgs: { action: string; editedValue: string } = {
    ...
    const isNoteCellIndex: boolean =
      this.parent.enableNotes &&
      !isNullOrUndefined(this.parent.spreadsheetNoteModule.noteCellIndexes);
    const cellIndexes: number[] = isNoteCellIndex
      ? this.parent.spreadsheetNoteModule.noteCellIndexes
      : getCellIndexes(this.parent.getActiveSheet().activeCell);
    const targetElement: HTMLElement = this.parent.getCell(
      cellIndexes[0],
      cellIndexes[1]
    );
    if (
      !isNullOrUndefined(targetElement) &&
      targetElement.children !== null &&
      targetElement.children.length > 0 &&
      this.isNoteActiveElement &&
      targetElement.children[
        targetElement.children.length - 1
      ].classList.contains('e-addNoteIndicator')
    ) {
      const cell: CellModel = getCell(cellIndexes[0], cellIndexes[1], sheet);
      const eventAction: string =
        !isNullOrUndefined(cell) && cell.notes ? 'editNote' : 'addNote';
      const noteContainer: HTMLTextAreaElement =
        document.getElementsByClassName(
          'e-addNoteContainer'
        )[0] as HTMLTextAreaElement;
      const address: string =
        getSheetName(this.parent as Workbook, this.parent.activeSheetIndex) +
        '!' +
        getRangeAddress(cellIndexes);
      if (
        !isNullOrUndefined(noteContainer) &&
        !isNullOrUndefined(noteContainer.value) &&
        (e.target as HTMLElement).className !== 'e-addNoteContainer' &&
        (isNullOrUndefined(cell) ||
          isNullOrUndefined(cell.notes) ||
          cell.notes !== noteContainer.value)
      ) {
        this.parent.notify(setActionData, {
          args: { action: 'beforeCellSave', eventArgs: { address: address } },
        });
        ...
        const eventArgs: NoteSaveEventArgs = {
    ...
    if (
      !this.isNoteTouch &&
      (e.target as HTMLElement).className !== 'e-addNoteContainer' &&
      document.getElementsByClassName('e-addNoteContainer') &&
      document.getElementsByClassName('e-addNoteContainer').length > 0
    ) {
    ...
    if (!this.parent.isEdit || isFormulaEdit) {
      const overlayElem: HTMLElement = document.getElementById(
        this.parent.element.id + '_overlay'
      );
      ...
      if (
        sheet.isProtected &&
        !sheet.protectSettings.selectCells &&
      ...
      if (
        !(
          closest(e.target as Element, '.e-findtool-dlg') ||
          closest(e.target as Element, '.e-dragfill-ddb')
        )
      ) {
        if (
          this.getSheetElement().contains(e.target as Node) &&
          !(e.target as HTMLElement).classList.contains('e-colresize') &&
          !(e.target as HTMLElement).classList.contains('e-rowresize')
        ) {
          const sheet: SheetModel = this.parent.getActiveSheet();
          const mode: string = this.parent.selectionSettings.mode;
          const rowIdx: number = this.getRowIdxFromClientY({
            clientY: getClientY(e),
            target: e.target as Element,
          });
          const colIdx: number = this.getColIdxFromClientX({
          ...
          if (sheet.showHeaders) {
            ...
            if (sheet.frozenColumns || sheet.frozenRows) {
              let headerEle: HTMLElement = this.parent
                .getSelectAllContent()
                .querySelector('thead');
              if (headerEle) {
                isColSelected =
                  (this.parent.getColumnHeaderContent().contains(trgt) ||
                    headerEle.contains(trgt)) &&
              ...
              headerEle = this.parent
                .getSelectAllContent()
                .querySelector('tbody');
              if (headerEle) {
                isRowSelected =
                  (this.parent.getRowHeaderContent().contains(trgt) ||
                    headerEle.contains(trgt)) &&
                  trgt.classList.contains('e-header-cell');
              } else {
                isRowSelected =
              ...
              isRowSelected = this.parent
                .getRowHeaderContent()
                .contains(e.target as Node);
              isColSelected = this.parent
          ...
          if ((e.target as HTMLElement).classList.contains('e-autofill')) {
            ...
            const autoFillDdb: Element = (
              e.target as HTMLElement
            ).parentElement.querySelector('.e-dragfill-ddb');
          ...
          if (isRowSelected) {
            ...
            range = [
              this.startCell[0],
              sheet.frozenColumns ? topLeftIdx[1] : 0,
              rowIdx,
              sheet.colCount - 1,
            ];
            ...
            range = [
              sheet.frozenRows ? topLeftIdx[0] : 0,
              this.startCell[1],
              sheet.rowCount - 1,
              colIdx,
            ];
            ...
            range = [].concat(this.startCell, [
              sheet.rowCount - 1,
              sheet.colCount - 1,
            ]);
          } else if (
            !(e.target as Element).classList.contains('e-sheet-content')
          ) {
            if (!e.shiftKey || mode === 'Single') {
              this.startCell = [rowIdx, colIdx];
            }
            if (
              !this.isautoFillClicked &&
              !closest(e.target as Element, '.e-filloption')
            ) {
              range = [].concat(
                this.startCell
                  ? this.startCell
          ...
          if (isTouchStart(e) && !(isRowSelected || isColSelected) && range) {
            const colRowSelectArgs: {
              isRowSelected: boolean;
              isColSelected: boolean;
            } = this.isRowColSelected(range);
            this.isRowSelected = colRowSelectArgs.isRowSelected;
            this.isColSelected = colRowSelectArgs.isColSelected;
          }
          const preventEvt: boolean =
            e.ctrlKey &&
            range &&
            sheet.selectedRange.includes(getRangeAddress(range));
          if (
            !preventEvt &&
            mode === 'Multiple' &&
            ((!isTouchEnd(e) &&
            ...
            if (!Browser.isPointer) {
              if (
                ...
                isTouchStart(e) &&
                e.target &&
                (e.target as HTMLElement).classList.contains('e-cell')
              ) {
                e.preventDefault();
              }
              document.addEventListener(
                getMoveEvent().split(' ')[1],
                this.mouseMoveEvt,
                { passive: false }
              );
            }
          ...
          if (!isTouchEnd(e)) {
            if (preventEvt) {
              if (this.parent.isEdit) {
                const updateFormulaCurPos: Function = (
          ...
          const isNoteAvailable: boolean =
            (e.target as HTMLElement).className === 'e-addNoteIndicator' ||
            ((e.target as HTMLElement).children.length > 0 &&
              (e.target as HTMLElement).children[
                (e.target as HTMLElement).childElementCount - 1
              ].className.indexOf('e-addNoteIndicator') > -1);
          if (isTouchStart(e) && isNoteAvailable) {
            const cellIndexes: number[] = getCellIndexes(
              getRangeAddress(range).split(':')[0]
            );
    ...
    if (
      isFormulaEdit &&
      ((e.target as HTMLElement).classList.contains('e-cell') ||
        (e.target as HTMLElement).classList.contains('e-wrap-content') ||
        (e.target as HTMLElement).classList.contains('e-header-cell')) &&
      this.parent.isEdit
    ) {
      let range: string = this.parent.getActiveSheet().selectedRange;
      const lastRange: string[] = range.split(' ');
      range = isSingleCell(
        getIndexesFromAddress(lastRange[lastRange.length - 1])
      )
        ? lastRange[lastRange.length - 1].split(':')[0]
  ...
  private mouseMoveHandler(e: MouseEvent & TouchEvent): void {
    ...
    const horizontalContent: Element =
      this.parent.element.getElementsByClassName('e-scroller')[0];
    const clientRect: ClientRect = verticalContent.getBoundingClientRect();
    const frozenCol: number = this.parent.frozenColCount(sheet);
    let left: number =
      clientRect.left + this.parent.sheetModule.getRowHeaderWidth(sheet);
    let right: number = clientRect.right - getScrollBarWidth();
    const top: number = clientRect.top;
    const bottom: number = clientRect.bottom;
    const clientX: number = getClientX(e);
    const clientY: number = getClientY(e);
    // remove math.min or handle top and left auto scroll
    let colIdx: number = this.isRowSelected
      ? sheet.colCount - 1
      : this.getColIdxFromClientX({
          clientX: clientX,
          target: e.target as Element,
        });
    let rowIdx: number = this.isColSelected
      ? sheet.rowCount - 1
    ...
    if (
      mergeArgs.range[2] === prevIndex[2] &&
      mergeArgs.range[3] === prevIndex[3] &&
      clientY <= bottom &&
      clientY >= top &&
      clientX <= right &&
      clientX >= left
    ...
    this.clearInterval();
    let scrollUpRowIdx: number;
    let scrollUpColIdx: number;
    ...
    if (isScrollDown || isScrollUp || isScrollRight || isScrollLeft) {
      ...
      const scrollSelection: Function = () => {
        if ((isScrollDown || isScrollUp) && !this.isColSelected) {
          rowIdx = this.getRowIdxFromClientY({
            clientY: isScrollDown ? bottom : top,
          });
          if (rowIdx >= sheet.rowCount) {
            // clear interval when scroll up
            this.clearInterval();
            return;
          }
          verticalContent.scrollTop +=
            (isScrollDown ? 1 : -1) * getRowHeight(sheet, rowIdx);
        }
        if ((isScrollRight || isScrollLeft) && !this.isRowSelected) {
          colIdx = this.getColIdxFromClientX({
            clientX: isScrollRight ? right : left,
          });
          if (colIdx >= sheet.colCount) {
            // clear interval when scroll left
            this.clearInterval();
            return;
          }
          horizontalContent.scrollLeft +=
            (isScrollRight ? 1 : -1) * getColumnWidth(sheet, colIdx);
        }
        if (
          (isScrollUp && sheet.frozenRows && !verticalContent.scrollTop) ||
          (isScrollLeft && sheet.frozenColumns && !horizontalContent.scrollLeft)
        ) {
          this.selectRangeByIdx(
            [].concat(prevIndex[0], prevIndex[1], [
              scrollUpRowIdx,
              scrollUpColIdx,
            ]),
            e
        ...
        this.selectRangeByIdx(
          [].concat(prevIndex[0], prevIndex[1], [rowIdx, colIdx]),
          e
        );
      };
      scrollSelection();
      ...
      let indexes: number[] = [].concat(prevIndex[0], prevIndex[1], [
        rowIdx,
        colIdx,
      ]);
      if (
      ...
      if (this.isautoFillClicked) {
        ...
        const args: { e: MouseEvent & TouchEvent; indexes?: number[] } = {
          e: e,
          indexes: null,
        };
    ...
    if (
      isFormulaEdit &&
      this.parent.isEdit &&
      !closest(e.target as Element, '#' + this.parent.element.id + '_edit')
    ) {
      let range: string;
      if (this.isautoFillClicked) {
  ...
  private mouseUpHandler(e: MouseEvent & TouchEvent): void {
    ...
    if (
      isTouchEnd(e) &&
      !(this.isColSelected || this.isRowSelected) &&
      this.getRowIdxFromClientY({
    ...
    if (this.isautoFillClicked) {
      ...
      if (
        !(this.isColSelected && indexes[1] === colIdx) &&
        !(this.isRowSelected && indexes[0] === rowIdx)
      ) {
      ...
      !e.ctrlKey &&
      !isDiscontinuousRange(getSelectedRange(this.parent.getActiveSheet()))
    ) {
  ...
  private updateFormulaCursorPosition(e: MouseEvent & TouchEvent): void {
    if (this.parent.isEdit) {
      ...
      this.parent.notify(editOperation, eventArgs);
      const isFormulaEdit: boolean = checkIsFormula(
        eventArgs.editedValue,
        true
      );
      if (isFormulaEdit) {
        this.parent.notify(initiateCur, {
          isCellEdit: (e.target as HTMLElement).classList.contains(
            'e-spreadsheet-edit'
          ),
        });
      }
  ...
  private isSelected(rowIdx: number, colIdx: number): boolean {
    ...
    let indexes: number[];
    const ranges: string[] = this.parent
      .getActiveSheet()
      .selectedRange.split(' ');
  ...
  private virtualContentLoadedHandler(args: {
    prevRowColCnt: SheetModel;
  }): void {
    // do only for scroll down
    const sheet: SheetModel = this.parent.getActiveSheet();
    let indexes: number[];
    let isColSelected: boolean;
    let isRowSelected: boolean;
    sheet.selectedRange.split(' ').forEach((rng: string, idx: number) => {
      ...
      isRowSelected =
        indexes[1] === 0 && indexes[3] === args.prevRowColCnt.colCount - 1;
      isColSelected =
        indexes[0] === 0 && indexes[2] === args.prevRowColCnt.rowCount - 1;
      if (isRowSelected || isColSelected) {
        ...
        if (sheet.frozenRows || sheet.frozenColumns) {
          this.selectRangeByIdx(
            indexes,
            <MouseEvent>{ type: 'mousedown', ctrlKey: idx !== 0 },
            false,
            false,
            false,
            false,
        ...
        indexes = getRangeIndexes(rng);
        const topIdx: number =
          this.parent.viewport.topIndex + this.parent.frozenRowCount(sheet);
        const leftIdx: number =
          this.parent.viewport.leftIndex + this.parent.frozenColCount(sheet);
        this.highlightHdr(
          indexes,
          idx === 0 ? false : true,
          indexes[0] >= topIdx || indexes[2] >= topIdx,
          indexes[1] >= leftIdx || indexes[3] >= leftIdx
        );
      }
    });
  }
  ...
  private getScrollLeft(): number {
    return this.parent.scrollModule
      ? this.parent.scrollModule.prevScroll.scrollLeft
      : 0;
  }

  private cellNavigateHandler(args: {
    range: number[];
    ...
    this.selectRangeByIdx(
      ...
      false,
      undefined,
      args.preventAnimation
    );
  }

  private getColIdxFromClientX(e: {
    clientX: number;
    isImage?: boolean;
    target?: Element;
    size?: number;
    ...
    if (e.isImage) {
      ...
      const cliRect: ClientRect = document
        .getElementById(this.parent.element.id + '_sheet')
        .getBoundingClientRect();
      if (this.parent.enableRtl) {
        left =
          cliRect.right -
          this.parent.sheetModule.getRowHeaderWidth(sheet, true) -
          1 -
          e.clientX;
      } else {
        left =
      ...
      if (
        !e.target ||
        (!closest(e.target, '.e-row-header') &&
          !closest(e.target, '.e-selectall-container')) ||
        this.isScrollableArea(e.clientX, e.target, true)
      ) {
        if (this.parent.frozenColCount(sheet)) {
          const frozenColumn: HTMLElement =
            this.parent.element.querySelector('.e-frozen-column');
          left =
            parseInt(frozenColumn.style.left, 10) > left
              ? left
    ...
    for (let i: number = 0; ; i++) {
      ...
      if (
        left < (e.isImage ? Number(addDPRValue(size).toFixed(2)) : size) ||
  ...
  private isScrollableArea(
    ...
    if (isclientX) {
      ...
      return (
        offset >
        this.parent.getMainContent().parentElement.getBoundingClientRect().top
      );
    }
  }

  private getRowIdxFromClientY(args: {
    clientY: number;
    isImage?: boolean;
    target?: Element;
    size?: number;
    ...
    if (args.isImage) {
      ...
      const sheetEle: HTMLElement = document.getElementById(
        this.parent.element.id + '_sheet'
      );
      top =
        args.clientY +
        this.parent.viewport.beforeFreezeHeight -
        (sheetEle.getBoundingClientRect().top + (sheet.showHeaders ? 31 : 0));
      if (
        !args.target ||
        !closest(args.target, '.e-header-panel') ||
        this.isScrollableArea(args.clientY, args.target)
      ) {
    ...
    for (let i: number = 0; ; i++) {
      ...
      if (
        top < (args.isImage ? Number(addDPRValue(size).toFixed(2)) : size) ||
  ...
  private initFormulaReferenceIndicator(range: number[]): void {
    if (this.parent.isEdit) {
      const forRefIndicator: HTMLElement = this.parent.createElement('div', {
        className: 'e-formularef-indicator',
      });
      forRefIndicator.appendChild(
        this.parent.createElement('div', { className: 'e-top' })
      );
      forRefIndicator.appendChild(
        this.parent.createElement('div', { className: 'e-bottom' })
      );
      forRefIndicator.appendChild(
        this.parent.createElement('div', { className: 'e-left' })
      );
      forRefIndicator.appendChild(
        this.parent.createElement('div', { className: 'e-right' })
      );
  ...
  private selectRangeByIdx(
    ...
    isScrollRefresh?: boolean,
    isActCellChanged?: boolean,
    isInit?: boolean,
    skipChecking?: boolean,
    selectedRowColIdx?: number,
    preventAnimation?: boolean
  ): void {
    ...
    this.parent.notify(editOperation, eventArgs);
    const isFormulaEdit: boolean =
      (this.parent.isEdit
        ? checkIsFormula(eventArgs.editedValue, true)
        : false) && !eventArgs.endFormulaRef;
    const isMultiRange: boolean =
      e && e.ctrlKey && isMouseDown(e as MouseEvent);
    let ele: HTMLElement;
    if (!isMultiRange) {
    ...
    const sheet: SheetModel = this.parent.getActiveSheet();
    const topLeftIdx: number[] = getRangeIndexes(sheet.topLeftCell);
    const formulaRefIndicator: HTMLElement = this.parent.element.querySelector(
      '.e-formularef-indicator'
    );
    const mergeArgs: MergeArgs = {
    ...
    let rowColSelectArgs: { isRowSelected: boolean; isColSelected: boolean } =
      this.isRowColSelected(range);
    ...
    let promise: Promise<null> = new Promise((resolve: Function) => {
      resolve(
        (() => {
          /** */
        })()
    ...
    const args: BeforeSelectEventArgs = {
    ...
    if (sheet.isProtected) {
      ...
      if (
        ...
        if (!isNullOrUndefined(protectCell)) {
          if (
            protectCell.isLocked === true ||
            isNullOrUndefined(protectCell.isLocked)
          ) {
            return;
          } else {
            const sheetEle: Element =
              this.parent.element.getElementsByClassName('e-sheet-panel')[0];
    ...
    if ((isSingleCell(range) || mergeArgs.isActiveCell) && !isMultiRange) {
      if (ele) {
        ...
        if (sheet.frozenRows || sheet.frozenColumns) {
          const clsName: string = isMouseMove(e as MouseEvent)
            ? 'e-cur-selection'
            : 'e-selection';
          removeRangeEle(
            this.parent.getSelectAllContent(),
            null,
            clsName,
            true
          );
          removeRangeEle(
            this.parent.getColumnHeaderContent(),
            null,
            clsName,
            true
          );
          removeRangeEle(
            this.parent.getRowHeaderContent(),
            null,
            clsName,
            true
          );
      ...
      if (isFormulaEdit && this.parent.isEdit) {
        ...
        if (ele) {
          ...
          if (sheet.frozenRows || sheet.frozenColumns) {
            if ((e && e.target) || isMultiRange) {
              ...
              if (
                isMouseMove(e as MouseEvent) &&
                ele.classList.contains('e-cur-selection')
              ) {
                ele.classList.add('e-hide');
              } else {
        ...
        const offset: { left: IOffset; top: IOffset } =
          this.isColSelected && this.isRowSelected
            ? undefined
            : this.getOffset(range[2], range[3]);
        if (isMergeRange && offset) {
          // Need to handle half hidden merge cell in better way
        ...
        promise =
          (setPosition(
            ...
            preventAnimation,
            isMultiRange,
            isMultiRange && !e.target
          ) as Promise<null>) || promise;
      }
    }
    const eArgs: { action: string; sheetIndex: number } = {
    ...
    if (
      sheet.frozenColumns &&
      range[1] > 0 &&
      range[1] === topLeftIdx[1] &&
      range[3] === sheet.colCount - 1
    ) {
      range[1] = 0;
    }
    if (
      sheet.frozenRows &&
      range[0] > 0 &&
      range[0] === topLeftIdx[0] &&
      range[2] === sheet.rowCount - 1
    ) {
      range[0] = 0;
    }
    let selRange: string = getRangeAddress(range);
    if (
      e &&
      e.ctrlKey &&
      (isMouseMove(e as MouseEvent) || isMouseUp(e as MouseEvent)) &&
      !isFormulaEdit
    ) {
      selRange =
        sheet.selectedRange.slice(0, sheet.selectedRange.lastIndexOf(' ')) +
        ' ' +
        selRange;
    ...
    if (
      !isScrollRefresh &&
      !(e && (e.type === 'mousemove' || isTouchMove(e)))
    ) {
      if (!isFormulaEdit) {
        this.updateActiveCell(
          isActCellChanged ? getRangeIndexes(sheet.activeCell) : range,
          isInit,
          preventAnimation
        );
      } else if (
        eArgs.sheetIndex === this.parent.getActiveSheet().id - 1 &&
        isInit
      ) {
  ...
  private isRowColSelected(indexes: number[]): {
    isRowSelected: boolean;
    isColSelected: boolean;
  } {
    const sheet: SheetModel = this.parent.getActiveSheet();
  ...
  private updateActiveCell(
    ...
    if (sheet.activeCell !== getCellAddress(range[0], range[1]) || isInit) {
      this.previousActiveCell =
        sheet.activeCell.indexOf(':') > -1
          ? this.previousActiveCell
          : sheet.activeCell;
      this.parent.setSheetPropertyOnMute(
        sheet,
        'activeCell',
        getCellAddress(range[0], range[1])
      );
      if (sheet.isProtected) {
        const element: HTMLTextAreaElement = this.parent.element.querySelector(
          '.e-formula-bar'
        ) as HTMLTextAreaElement;
        const cell: CellModel = getCell(range[0], range[1], sheet);
        const isCellLocked: boolean = isLocked(
          cell,
          getColumn(sheet, range[1])
        );
      ...
      if (this.getActiveCell()) {
        const offset: { left: IOffset; top: IOffset } = this.getOffset(
          range[2],
          range[3]
        );
        if (isMergeRange) {
  ...
  private getSelectionElement(
    ...
    if (e && e.ctrlKey && !this.parent.isEdit) {
      if (isMouseUp(e) || isMouseMove(e)) {
        if (sheet.frozenColumns || sheet.frozenRows) {
          let ele: HTMLElement = this.parent
            .getMainContent()
            .querySelector('.e-cur-selection');
          if (ele) {
            return ele;
          } else {
          ...
          return this.parent
            .getMainContent()
            .querySelector('.e-selection:last-child');
        }
      } else {
        const selElem: HTMLElement = this.parent
          .getMainContent()
          .getElementsByClassName('e-selection')[0] as HTMLElement;
        ...
        if (sheet.frozenColumns || sheet.frozenRows) {
          if (!sheet.selectedRange.includes(' ')) {
            ...
            setPosition(
              this.parent,
              selElem,
              getSwapRange(getRangeIndexes(sheet.selectedRange)),
              undefined,
              false,
              true
            );
          ...
          if (
            !this.parent.getMainContent().querySelector('.e-multi-range') &&
            selElem.classList.contains('e-hide')
          ) {
            return selElem;
          }
          return ele;
      ...
      return (
        sheet.frozenRows || sheet.frozenColumns
          ? this.parent.element
              .querySelector('.e-sheet')
              .getElementsByClassName('e-selection')[
              selectedRowColIdx as number
            ]
          : this.parent.getMainContent().getElementsByClassName('e-selection')[
              selectedRowColIdx as number
            ]
      ) as HTMLElement;
    } else {
      const elems: NodeListOf<Element> = [].slice.call(
        this.parent.element.getElementsByClassName('e-multi-range')
      );
  ...
  private getActiveCell(): HTMLElement {
    return this.parent
      .getMainContent()
      .getElementsByClassName('e-active-cell')[0] as HTMLElement;
  }
  ...
  private highlightHdr(
    ...
    isRowRefresh: boolean = true,
    isColRefresh: boolean = true
  ): void {
    const sheet: SheetModel = this.parent.getActiveSheet();
    if (sheet.showHeaders) {
      if (!isMultiRange) {
        removeClass(
          this.getSheetElement().querySelectorAll('.e-highlight'),
          'e-highlight'
        );
        removeClass(
          this.getSheetElement().querySelectorAll('.e-prev-highlight'),
          'e-prev-highlight'
        );
      }
      const selectAllEle: Element =
        this.parent.element.getElementsByClassName('e-select-all-cell')[0];
      if (selectAllEle) {
        removeClass(
          [selectAllEle],
          ['e-prev-highlight-right', 'e-prev-highlight-bottom']
        );
      }
      ...
      const updateIndex: Function = (
        ...
        if (freezePane && swapRange[i as number] < freezePane) {
          topLeftIndex[i as number] = skipHiddenIdx(
            sheet,
            topLeftIndex[i as number],
            true,
            layout
          );
          const startIdx: number = skipHiddenIdx(
            sheet,
            swapRange[i as number],
            true,
            layout
          );
          ...
          hiddenCount = this.parent.hiddenCount(
            topLeftIndex[i as number],
            swapRange[i as number] - 1,
            layout,
            sheet
          );
          frozenIdx[i as number] =
            swapRange[i as number] - hiddenCount - topLeftIndex[i as number];
          idx =
            swapRange[j as number] < freezePane
              ? swapRange[j as number]
              : freezePane - 1;
          frozenIdx[j as number] =
            idx -
          ...
          if (swapRange[j as number] >= idx) {
            ...
            indexes[i as number] -= this.parent.hiddenCount(
              idx,
              idx,
              layout,
              sheet
          ...
          idx = skipHiddenIdx(
            sheet,
            this.parent.viewport[`${offset}`] + freezePane,
            true,
            layout
          );
          const startIdx: number = skipHiddenIdx(
      ...
      const updateCell: Function = (
        ...
        const header: Element[] = [].slice.call(
          parent.getElementsByClassName('e-header-cell')
        );
      ...
      if (isRowRefresh) {
        ...
        if (sheet.frozenRows) {
          const selectAllBody: Element = this.parent
            .getSelectAllContent()
            .querySelector('tbody');
          if (selectAllBody) {
      ...
      if (isColRefresh) {
        ...
        if (sheet.frozenColumns) {
          const selectAllHdr: Element = this.parent
            .getSelectAllContent()
            .querySelector('thead');
          if (selectAllHdr) {
      ...
      if (rowHdr.length && rowHdr[0].parentElement.previousElementSibling) {
        rowHdr[0].parentElement.previousElementSibling.classList.add(
          'e-prev-highlight'
        );
      }
      ...
      if (this.isRowSelected && this.isColSelected) {
        if (sheet.isProtected && !sheet.protectSettings.selectCells) {
          document
            .getElementById(`${this.parent.element.id}_select_all`)
            .classList.remove('e-highlight');
        } else {
          document
            .getElementById(`${this.parent.element.id}_select_all`)
            .classList.add('e-highlight');
        }
      }
      if (selectAllEle) {
        if (
          skipHiddenIdx(sheet, swapRange[0], true) ===
          skipHiddenIdx(sheet, 0, true)
        ) {
          selectAllEle.classList.add('e-prev-highlight-bottom');
        }
        if (
          skipHiddenIdx(sheet, swapRange[1], true, 'columns') ===
  ...
  private protectHandler(): void {
    const range: number[] = getRangeIndexes(
      this.parent.getActiveSheet().selectedRange
    );
    const swapRange: number[] = getSwapRange(range);
    const actRange: number[] = getCellIndexes(
      this.parent.getActiveSheet().activeCell
    );
    const inRange: boolean =
      swapRange[0] <= actRange[0] &&
      swapRange[2] >= actRange[0] &&
      swapRange[1] <= actRange[1] &&
      swapRange[3] >= actRange[1];
    this.selectRangeByIdx(range, null, null, inRange);
  }

  private initiateFormulaSelection(args: {
    range: string;
    formulaSheetIdx: number;
  }): void {
  ...
  private processFormulaEditRange(
    ...
    const formulaBorder: string[][] = [
      ['e-vborderright', 'e-vborderbottom'],
      ['e-pborderright', 'e-pborderbottom'],
      ['e-cborderright', 'e-cborderbottom'],
      ['e-gborderright', 'e-gborderbottom'],
      ['e-oborderright', 'e-oborderbottom'],
      ...
      if (isCellReference(str.toUpperCase())) {
        ...
        if (i > 0) {
          if (parsedVal[i - 1].indexOf('!') === parsedVal[i - 1].length - 1) {
            ...
            formulaSheetIdx = getSheetIndex(
              this.parent as Workbook,
              splitStr[0].substring(1, splitStr[0].length - 1)
            );
          }
        }
        if (parsedVal[i + 1] === ':') {
          ...
          if (
            parsedVal[i + 1] &&
            isCellReference(parsedVal[i + 1].toUpperCase())
          ) {
  ...
  private updateFormulaEditRange(
    ...
    this.focusBorder(
      this.dStartCell,
      this.dEndCell,
      formulaBorder[i % 6] as string[]
    );
  }

  private chartBorderHandler(args: {
    startcell: { rowIndex: number; colIndex: number };
    endcell: { rowIndex: number; colIndex: number };
    classes: string[];
  }): void {
  ...
  private focusBorder(
    ...
    const topLeftIdx: number[] = getRangeIndexes(sheet.topLeftCell);
    const hiddenCol: number = this.parent.hiddenCount(
      topLeftIdx[1],
      range[3] - 1,
      'columns',
      sheet
    ...
    if (sheet.frozenRows || sheet.frozenColumns) {
      const rangeReference: HTMLElement = this.parent.createElement('div', {
        className: isChart
          ? 'e-range-indicator e-chart-range'
          : 'e-range-indicator e-formuala-range',
      });
      rangeReference.appendChild(
        this.parent.createElement('div', { className: 'e-top' })
      );
      rangeReference.appendChild(
        this.parent.createElement('div', { className: 'e-bottom' })
    ...
    if (minr) {
      this.getEleFromRange([minr - 1, minc, minr - 1, maxc]).forEach(
        (td: HTMLElement): void => {
          if (td) {
    ...
    this.getEleFromRange([minr, maxc, maxr, maxc]).forEach(
      (td: HTMLElement): void => {
        if (td) {
    ...
    this.getEleFromRange([maxr, minc, maxr, maxc]).forEach(
      (td: HTMLElement): void => {
        if (td) {
          td.classList.add(classes[1]);
          if (!isChart) {
    ...
    if (minc) {
      this.getEleFromRange([minr, minc - 1, maxr, minc - 1]).forEach(
        (td: HTMLElement): void => {
          if (td) {
            td.classList.add(classes[0]);
            if (!isChart) {
  ...
  private getEleFromRange(range: number[]): HTMLElement[] {
    let startRIndex: number = range[0];
    let startCIndex: number = range[1];
    let endRIndex: number = range[2];
    let endCIndex: number = range[3];
    let i: number;
    let rowIdx: number;
    let temp: number;
    ...
    if (this.parent.scrollSettings.enableVirtualization) {
      for (i = startRIndex; i <= endRIndex; i++) {
        ...
        if (rowIdx > -1) {
          ...
          if (row) {
            rowCells = row.getElementsByClassName(
              'e-cell'
            ) as HTMLCollectionOf<Element>;
            tempCells =
              endCIndex === startCIndex
                ? [rowCells[endCIndex as number]]
                : this.getRowCells(rowCells, startCIndex, endCIndex + 1);
  ...
  private getRowCells(
    rowCells: HTMLCollectionOf<Element>,
    startCIndex: number,
    endCIndex: number
  ): HTMLElement[] {
  ...
  private clearBorder(): void {
    const sheet: SheetModel = this.parent.getActiveSheet();
    if (sheet.frozenColumns || sheet.frozenRows) {
      const formualIndicator: Element[] = [].slice.call(
        this.parent.element.getElementsByClassName('e-formuala-range')
      );
      formualIndicator.forEach((indicator: Element): void => {
        detach(indicator);
      });
      return;
    }
