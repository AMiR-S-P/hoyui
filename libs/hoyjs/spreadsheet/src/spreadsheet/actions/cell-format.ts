...
export class CellFormat {
  ...
  private applyCellFormat(args: CellFormatArgs): void {
    ...
    if (cell) {
      ...
      if (
        args.style.border !== undefined ||
        args.style.borderTop !== undefined ||
        args.style.borderLeft !== undefined
      ) {
        const curStyle: CellStyleModel = {};
        Object.keys(args.style).forEach((key: string): void => {
          curStyle[`${key}`] = args.style[`${key}`];
        });
        if (curStyle.border !== undefined) {
          Object.assign(cell.style, <CellStyleModel>{
        ...
        if (
          CellElem &&
          (CellElem.rowSpan || CellElem.colSpan) &&
          cell.offsetHeight > 0
        ) {
          const height: number = getTextHeight(
            this.parent as Workbook,
            CellElem.style || this.parent.cellStyle
          );
      ...
      const cellModel: CellModel = getCell(
        args.rowIdx,
        args.colIdx,
        sheet,
        null,
      ...
      if (!sheet.rows[args.rowIdx] || !sheet.rows[args.rowIdx].customHeight) {
        if (args.isHeightCheckNeeded) {
          if (!args.manualUpdate) {
            ...
            if (!this.checkHeight) {
              this.checkHeight = isHeightCheckNeeded(
                args.style,
                args.onActionUpdate
              );
            }
            if (
              cell &&
              cell.children[0] &&
              cell.children[0].className === 'e-cf-databar' &&
              args.style.fontSize
            ) {
              (
                cell.children[0].querySelector(
                  '.e-databar-value'
                ) as HTMLElement
              ).style.fontSize = args.style.fontSize;
            }
            ...
            if (
              cellModel.wrap &&
              (args.style.fontSize || args.style.fontFamily)
            ) {
              cell.style.lineHeight =
                parseFloat(
                  (cellModel.style && cellModel.style.fontSize) ||
                    this.parent.cellStyle.fontSize
                ) *
                  getLineHeight(
                    cellModel.style && cellModel.style.fontFamily
                      ? cellModel.style
        ...
        const hgt: number =
          getRowHeight(sheet, args.rowIdx, true) -
          getBorderHeight(args.rowIdx, args.colIdx, sheet);
  ...
  private updateRowHeight(
    ...
    if (this.checkHeight) {
      ...
      if (!cell.rowSpan) {
        ...
        const defaultHeight: number = sheet.standardHeight
          ? sheet.standardHeight
          : 20;
        ...
        if (!outsideViewport) {
          ...
          if (
            td &&
            td.children[0] &&
            td.children[0].className === 'e-cf-databar'
          ) {
            (td.children[0] as HTMLElement).style.height = '100%';
            (
              td.children[0].firstElementChild.nextElementSibling as HTMLElement
            ).style.height = '100%';
          }
        }
        if (isLastCell) {
          ...
          if (onActionUpdate ? maxHgt !== prevHgt : maxHgt > prevHgt) {
            if (outsideViewport) {
              ...
              this.parent.notify(rowHeightChanged, {
                rowIdx: rowIdx,
                threshold: getDPRValue(maxHgt) - prevHgt,
              });
            } else {
              setRowEleHeight(
                this.parent,
  ...
  private updateMergeBorder(args: CellFormatArgs, sheet: SheetModel): void {
    ...
    if (cellModel.colSpan > 1 && !args.style.borderRight) {
      const rightCell: CellModel = getCell(
        mergeArgs.range[0],
        mergeArgs.range[3],
        sheet,
        null,
  ...
  private setLeftBorder(
    ...
    if (prevCell) {
      ...
      if (
        ...
        if (
          model.style &&
          model.style.borderRight &&
          model.style.borderRight !== 'none'
        ) {
          return;
        } else {
          model = getCell(
  ...
  private setTopBorder(
    ...
    const prevCell: HTMLElement = this.parent.getCell(
      rowIdx - 1,
      col,
      <HTMLTableRowElement>pRow
    );
    if (prevCell) {
      ...
      if (
        ...
        if (
          model.style &&
          model.style.borderBottom &&
          model.style.borderBottom !== 'none'
        ) {
          return;
        } else {
          model = getCell(
        ...
        if (isHiddenRow(sheet, rowIdx - 1)) {
          const index: number[] = [
            Number(prevCell.parentElement.getAttribute('aria-rowindex')) - 1,
            colIdx,
          ];
          if (
            (
        ...
        if (
          actionUpdate &&
          border !== '' &&
          sheet.topLeftCell.includes(`${rowIdx + 1}`)
        ) {
          this.parent.getMainContent().parentElement.scrollTop -=
        ...
        prevCell.style.borderBottom =
          border === 'none' ? prevCell.style.borderBottom : border;
      }
    } else {
      cell.style.borderTop = border;
    }
  ...
  private setThickBorderHeight(
    ...
    const size: number = border ? this.getBorderSize(border) : 1;
    const sheet: SheetModel = this.parent.getActiveSheet();
    if (
      size > 2 &&
  ...
  private clearObj(args: {
    ...
    const range: string = options.range
      ? options.range.indexOf('!') > 0
        ? options.range.split('!')[1]
        : options.range.split('!')[0]
      : this.parent.getActiveSheet().selectedRange;
    const sheetIndex: number =
      options.range && options.range.indexOf('!') > 0
        ? getSheetIndex(this.parent as Workbook, options.range.split('!')[0])
    ...
    if (isOverlay) {
      ...
      if (options.type === 'Clear Formats' || isClearAll) {
        ...
        (args as { cfClearActionArgs?: object }).cfClearActionArgs =
          clearCFArgs.cfClearActionArgs;
        if (isClearAll) {
          ...
          if (
            sRIdx === 0 &&
            rangeIdx[1] === 0 &&
            eRIdx >= sheet.usedRange.rowIndex &&
              ...
              if (cell.hyperlink) {
                ...
                if (isClearAll) {
                  this.parent.removeHyperlink(
                    sheet.name +
                      '!' +
                      getRangeAddress([sRIdx, sCIdx, sRIdx, sCIdx])
                  );
                }
              }
            }
