...
import { L10n, isNullOrUndefined, closest } from '@hoyui/base';
...
import { Tab, TreeView } from '@hoyui/navigations';
...
export class SpreadsheetHyperlink {
  ...
  private keyUpHandler(e: MouseEvent): void {
    ...
    if (closest(trgt, '.e-document')) {
      const hyperlinkText: HTMLInputElement = document.querySelector(
        '.e-hyp-text'
      ) as HTMLInputElement;
      const hyperlinkSpan: HTMLElement = this.parent.element.querySelector(
        '.e-hyperlink-alert-span'
      );
      const dlgElement: Element =
        closest(trgt, '.e-hyperlink-dlg') ||
        closest(trgt, '.e-edithyperlink-dlg');
      const footerEle: HTMLElement = dlgElement.getElementsByClassName(
        'e-footer-content'
      )[0] as HTMLElement;
    ...
    if (trgt.classList.contains('e-text') && closest(trgt, '.e-cont')) {
      if (
        closest(trgt, '.e-webpage') &&
        closest(trgt, '.e-webpage').getElementsByClassName('e-cont')[1] ===
          trgt.parentElement
      ) {
        const dlgEle: Element =
          closest(trgt, '.e-hyperlink-dlg') ||
          closest(trgt, '.e-edithyperlink-dlg');
        ...
        if ((trgt as CellModel).value !== '') {
          ...
          const isUrl: boolean = (
            webPage
              .querySelectorAll('.e-cont')[1]
              .querySelector('.e-text') as CellModel
          ).value
            ? true
            : false;
          if (!isUrl) {
  ...
  private initiateHyperlinkHandler(): void {
    ...
    if (
      sheet.isProtected &&
      (!sheet.protectSettings.insertLink || isLockedCells(this.parent))
    ) {
    ...
    if (!this.parent.element.querySelector('.e-hyperlink-dlg')) {
      ...
      dialogInst.show({
        ...
        beforeOpen: (args: BeforeOpenEventArgs): void => {
          ...
          displayText = (
            dialogInst.dialogInstance.content.querySelector(
              '.e-text'
            ) as HTMLInputElement
          ).value;
        ...
        open: (): void => {
          setTimeout(() => {
            focus(
              dialogInst.dialogInstance.element.querySelectorAll(
                '.e-webpage input'
              )[1] as HTMLElement
            );
          });
  ...
  private dlgClickHandler(displayText: string): void {
    ...
    const item: HTMLElement = this.parent.element
      .querySelector('.e-link-dialog')
      .getElementsByClassName('e-content')[0]
      .querySelector('.e-item.e-active') as HTMLElement;
    if (item) {
      value = (
        item
          .getElementsByClassName('e-cont')[0]
          .querySelector('.e-text') as CellModel
      ).value;
      ...
      if (item.querySelector('.e-webpage')) {
        address = (
          item
            .getElementsByClassName('e-cont')[1]
            .querySelector('.e-text') as CellModel
        ).value;
        ...
        const dlgContent: HTMLElement = item.getElementsByClassName(
          'e-cont'
        )[2] as HTMLElement;
        if (
          dlgContent
            .getElementsByClassName('e-list-item')[0]
            .querySelector('.e-active')
        ) {
          const sheetName: string = item
            .getElementsByClassName('e-cont')[2]
            .querySelector('.e-active').textContent;
          ...
          const definedName: string = item
            .getElementsByClassName('e-cont')[2]
            .querySelector('.e-active').textContent;
          for (
            let idx: number = 0;
            idx < this.parent.definedNames.length;
            idx++
          ) {
  ...
  private showDialog(): void {
    ...
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    const hyperlinkSpan: HTMLElement = this.parent.createElement('span', {
      className: 'e-hyperlink-alert-span',
    });
    hyperlinkSpan.innerText = l10n.getConstant('HyperlinkAlert');
    const dlgEle: HTMLElement =
      this.parent.element.querySelector('.e-hyperlink-dlg') ||
      this.parent.element.querySelector('.e-edithyperlink-dlg');
  ...
  private hlOpenHandler(
    ...
    if (trgt.classList.contains('e-hyperlink')) {
      ...
      const colIdx: number =
        parseInt(cellEle.getAttribute('aria-colindex'), 10) - 1;
      const rowIdx: number =
        parseInt(cellEle.parentElement.getAttribute('aria-rowindex'), 10) - 1;
        ...
        if (!isNullOrUndefined(address)) {
          ...
          if (isCellReference(range[1]) || isDefinedNamed) {
            ...
            if (this.parent.scrollSettings.enableVirtualization) {
              rangeIndexes[0] =
                rangeIndexes[0] >= this.parent.viewport.topIndex
                  ? rangeIndexes[0] - this.parent.viewport.topIndex
                  : rangeIndexes[0];
              rangeIndexes[1] =
                rangeIndexes[1] >= this.parent.viewport.leftIndex
                  ? rangeIndexes[1] - this.parent.viewport.leftIndex
                  : rangeIndexes[1];
            }
            if (!isNullOrUndefined(sheet)) {
              ...
              if (sheet === this.parent.getActiveSheet()) {
                ...
                getUpdateUsingRaf((): void => {
                  this.parent.goTo(
                    this.parent.sheets[sheetIdx as number].name +
                      '!' +
                      rangeAddr
                  );
                });
  ...
  private hyperlinkClickHandler(e: MouseEvent): void {
    ...
    if (closest(trgt, '.e-link-dialog') && closest(trgt, '.e-toolbar-item')) {
      ...
      const webEleText: string = webEle
        ? (
            webEle
              .querySelectorAll('.e-cont')[0]
              .querySelector('.e-text') as CellModel
          ).value
        : (
            docEle
      ...
      const docEleText: string = docEle
        ? (
            docEle
              .querySelectorAll('.e-cont')[0]
              .querySelector('.e-text') as CellModel
      ...
      if (
        toolbarItems
          .getElementsByClassName('e-toolbar-item')[1]
          .classList.contains('e-active')
      ) {
        const actEle: Element = docEle
          .querySelectorAll('.e-cont')[2]
          .querySelector('.e-active');
        (
          docEle
            .querySelectorAll('.e-cont')[0]
        ...
        if (
          closest(actEle, '.e-list-item').classList.contains('e-level-2') &&
          insertBut.hasAttribute('disabled')
        ) {
          insertBut.removeAttribute('disabled');
        } else if (
          closest(actEle, '.e-list-item').classList.contains('e-level-1') &&
          !insertBut.hasAttribute('disabled')
        ) {
          insertBut.setAttribute('disabled', 'true');
        }
    ...
    if (closest(trgt, '.e-list-item') && trgt.classList.contains('e-fullrow')) {
      let item: HTMLElement = this.parent.element.getElementsByClassName(
        'e-link-dialog'
      )[0] as HTMLElement;
      if (item) {
        item = item
          .getElementsByClassName('e-content')[0]
          .getElementsByClassName('e-active')[0] as HTMLElement;
      } else {
        return;
      }
      const cellRef: HTMLElement = item
        .getElementsByClassName('e-cont')[1]
        .getElementsByClassName('e-text')[0] as HTMLElement;
      const dlgEle: Element =
        closest(trgt, '.e-hyperlink-dlg') ||
        closest(trgt, '.e-edithyperlink-dlg');
      const ftrEle: HTMLElement = dlgEle.getElementsByClassName(
      ...
      if (closest(trgt, '.e-list-item').classList.contains('e-level-2')) {
        if (
          closest(trgt, '.e-list-item').getAttribute('data-uid') === 'defName'
        ) {
          if (
            !cellRef.classList.contains('e-disabled') &&
            !cellRef.hasAttribute('readonly')
          ) {
          ...
          closest(trgt, '.e-list-item').getAttribute('data-uid') === 'sheet'
        ) {
          if (
            cellRef.classList.contains('e-disabled') &&
            cellRef.hasAttribute('readonly')
          ...
          if (
            isCellReference((cellRef as HTMLInputElement).value.toUpperCase())
          ) {
            if (insertBut.hasAttribute('disabled')) {
              insertBut.removeAttribute('disabled');
            }
        ...
        closest(trgt, '.e-list-item').classList.contains('e-level-1')
      ) {
        insertBut.setAttribute('disabled', 'true');
      }
    } else {
  ...
  private createHyperlinkEle(args: {
    cell: CellModel;
    td: HTMLElement;
    rowIdx: number;
    colIdx: number;
    ...
    if (!isNullOrUndefined(cell.hyperlink)) {
      ...
      const hyperEle: HTMLElement = this.parent.createElement('a', {
  ...
  private hyperEditContent(): HTMLElement {
    ...
    const indexes: number[] = getRangeIndexes(
      this.parent.getActiveSheet().activeCell
    );
    const cell: CellModel =
      this.parent.sheets[this.parent.getActiveSheet().id - 1].rows[indexes[0]]
        .cells[indexes[1]];
    if (this.parent.scrollSettings.enableVirtualization) {
    ...
    if (isWeb) {
      const webContElem: HTMLElement = item.querySelector(
        '.e-webpage'
      ) as HTMLElement;
      webContElem
        .getElementsByClassName('e-cont')[0]
        .getElementsByClassName('e-text')[0]
        .setAttribute('value', value);
      if (typeof hyperlink === 'string') {
        webContElem
          .getElementsByClassName('e-cont')[1]
          .querySelector('.e-text')
          .setAttribute('value', hyperlink);
      } else {
        const address: HTMLElement = webContElem
          .getElementsByClassName('e-cont')[1]
          .querySelector('.e-text') as HTMLElement;
        address.setAttribute('value', hyperlink.address);
      }
    } else {
      let isDefinedNamed: boolean;
      const docContElem: HTMLElement = item.querySelector(
        '.e-document'
      ) as HTMLElement;
      docContElem
        .getElementsByClassName('e-cont')[0]
      ...
      if (isDefinedNamed) {
        const cellRef: HTMLElement = docContElem
          .getElementsByClassName('e-cont')[1]
          .getElementsByClassName('e-text')[0] as HTMLElement;
        cellRef.setAttribute('readonly', 'true');
        cellRef.classList.add('e-disabled');
        cellRef.setAttribute('disabled', 'true');
        const treeCont: HTMLElement = docContElem.getElementsByClassName(
          'e-cont'
        )[2] as HTMLElement;
        const listEle: HTMLElement = treeCont.querySelectorAll(
          '.e-list-item.e-level-1'
        )[1] as HTMLElement;
        for (
          let idx: number = 0;
          idx < listEle.getElementsByTagName('li').length;
          idx++
        ) {
          if (
            (listEle.getElementsByTagName('li')[idx as number] as HTMLElement)
              .innerText === address
          ) {
            listEle
              .getElementsByTagName('li')
              [idx as number].classList.add('e-active');
          }
        }
      } else {
        ...
        for (
          ...
          if (
            ...
            if (
              listEle
                .getElementsByTagName('li')
                [idx as number].classList.contains('e-active')
            ) {
              break;
            } else {
              listEle
            ...
            if (
              ...
              listEle
                .getElementsByTagName('li')
                [idx as number].classList.remove('e-active');
            }
          }
        }
      }
  ...
  private hyperlinkContent(): HTMLElement {
    ...
    if (cell) {
      ...
      if (address) {
        ...
        const sheets: SheetModel[] = this.parent.sheets;
        for (
          let idx: number = 0, len: number = sheets.length;
          idx < len;
          idx++
        ) {
          if (address.includes(sheets[idx as number].name)) {
    ...
    const webContElem: HTMLElement = this.parent.createElement('div', {
      className: 'e-webpage',
    });
    const docContElem: HTMLElement = this.parent.createElement('div', {
    ...
    if (isWeb) {
      dialogElem
        ...
        .querySelector('.e-indicator')
        .setAttribute('style', 'left: 0; right: 136px');
    } else {
      dialogElem
        .querySelector('.e-toolbar-items')
        .querySelector('.e-indicator')
        .setAttribute('style', 'left: 136px; right: 0');
    }
    const textCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-cont',
    });
    const urlCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-cont',
    });
    ...
    const textInput: HTMLElement = this.parent.createElement('input', {
    ...
    const urlInput: HTMLElement = this.parent.createElement('input', {
      className: 'e-input e-text',
      attrs: { type: 'Text' },
    });
    ...
    textInput.setAttribute(
      'placeholder',
      l10n.getConstant('EnterTheTextToDisplay')
    );
    ...
    const cellrefCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-cont',
    });
    const cellrefH: HTMLElement = this.parent.createElement('div', {
      className: 'e-header',
    });
    ...
    const textCont1: HTMLElement = this.parent.createElement('div', {
      className: 'e-cont',
    });
    const textH1: HTMLElement = this.parent.createElement('div', {
      className: 'e-header',
    });
    textH1.innerText = l10n.getConstant('DisplayText');
    const textInput1: HTMLElement = this.parent.createElement('input', {
      className: 'e-input e-text',
      attrs: { type: 'Text' },
    });
    ...
    textInput1.setAttribute(
      'placeholder',
      l10n.getConstant('EnterTheTextToDisplay')
    );
    textCont1.appendChild(textInput1);
    textCont1.insertBefore(textH1, textInput1);
    const sheetCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-cont',
    });
    const sheetH: HTMLElement = this.parent.createElement('div', {
      className: 'e-header',
    });
    sheetH.innerText = l10n.getConstant('Sheet');
    const refCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-refcont',
    });
  ...
  private removeHyperlinkHandler(args: {
    range: string;
    ...
    if (!args.preventEventTrigger) {
      const eventArgs: { address: string; cancel: boolean } = {
    ...
    const rangeIndexes: number[] = range
      ? getRangeIndexes(range)
      : getRangeIndexes(sheet.activeCell);
    let cellEle: HTMLElement;
    let classList: string[];
    for (
      let rowIdx: number = rangeIndexes[0];
      rowIdx <= rangeIndexes[2];
      rowIdx++
    ) {
      for (
        let colIdx: number = rangeIndexes[1];
        colIdx <= rangeIndexes[3];
        colIdx++
      ) {
        if (
          sheet &&
          sheet.rows[rowIdx as number] &&
