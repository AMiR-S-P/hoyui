...
import { Browser, closest, getComponent, isNullOrUndefined } from '@hoyui/base';
...
export class KeyboardNavigation {
  ...
  private keyDownHandler(e: KeyboardEvent): void {
    ...
    if (
      e.altKey &&
      e.keyCode === 38 &&
      this.parent.element.lastElementChild.classList.contains('e-filter-popup')
    ) {
    ...
    if (
      !isNullOrUndefined(textarea) &&
      textarea.classList.contains('e-addNoteContainer')
    ) {
      if (e.key === 'Escape' || e.keyCode === 27) {
        const isNoteCellIndex: boolean = !isNullOrUndefined(
        ...
        if (
          !isNullOrUndefined(textarea) &&
          !isNullOrUndefined(textarea.value) &&
          (isNullOrUndefined(cell) ||
            isNullOrUndefined(cell.notes) ||
            cell.notes !== textarea.value) &&
    ...
    const isNameBox: boolean =
      target.id === `${this.parent.element.id}_name_box`;
    if (
      this.parent.selectionSettings.mode === 'None' ||
      dlgInst ||
      this.parent.isEdit ||
      (target.classList.contains('e-ss-ddb') &&
        e.keyCode !== 117 &&
        e.keyCode !== 9) ||
      (isNameBox && e.keyCode !== 117) ||
      target.classList.contains('e-sheet-rename') ||
      target.id === `${this.parent.element.id}_SearchBox` ||
      target.classList.contains('e-chk-hidden') ||
      (target.classList.contains('e-ddl') &&
        target.classList.contains('e-input-focus'))
    ) {
      if (dlgInst) {
        if (e.keyCode === 13) {
          if (
            dlgInst.element.classList.contains('e-spreadsheet-function-dlg') &&
            (target.classList.contains('e-formula-list') ||
              target.classList.contains('e-list-item'))
          ) {
            focus(
              dlgInst.element.querySelector('.e-footer-content .e-primary')
            );
          }
        } else if (e.keyCode === 9) {
          // To maintain the focus inside the dialogs on the tab or shift + tab key
          if (dlgInst.element.classList.contains('e-find-dlg')) {
            const footerBtns: NodeList = dlgInst.element.querySelectorAll(
              '.e-footer-content .e-btn:not(:disabled)'
            );
            const cls: string = footerBtns.length
              ? (footerBtns[footerBtns.length - 1] as Element).className
            ...
            if (e.shiftKey) {
              if (
                ...
                if (footerBtns.length) {
                  ...
                  const cBoxWrapper: HTMLElement =
                    dlgInst.element.querySelector(
                      '.e-findnreplace-exactmatchcheckbox'
                    );
                  if (cBoxWrapper) {
                    focus(
                      cBoxWrapper.querySelector(
                        '.e-findnreplace-checkmatch'
                      ) as HTMLElement
                    );
            ...
            if (
              e.shiftKey
                ? document.activeElement.classList.contains('e-primary')
                : document.activeElement.id ===
                  `${this.parent.element.id}_protect_check`
            ) {
              const listWrapper: HTMLElement = dlgInst.element.querySelector(
                '.e-protect-option-list'
              );
              if (
                listWrapper &&
                !listWrapper.querySelector('.e-list-item.e-focused')
              ) {
                const listEle: HTMLElement =
                  listWrapper.querySelector('.e-list-item');
                if (listEle) {
            ...
            dlgInst.element.classList.contains('e-custom-format-dlg')
          ) {
            if (!e.shiftKey) {
              if (
                document.activeElement.classList.contains('e-btn') &&
                document.activeElement.parentElement.classList.contains(
                  'e-custom-dialog'
                )
              ) {
                const listWrapper: HTMLElement =
                  dlgInst.element.querySelector('.e-custom-listview');
                const listObj: { selectItem: Function } = getComponent(
                  listWrapper,
                  'listview'
                );
                if (listWrapper) {
                  let listEle: HTMLElement = listWrapper.querySelector(
                ...
                document.activeElement.classList.contains('e-list-item')
              ) {
                focus(dlgInst.element as HTMLElement);
              }
            } else if (
              document.activeElement.className.includes('e-list-item e-active')
            ) {
              const listWrapper: HTMLElement = closest(
            ...
            dlgInst.element.classList.contains('e-goto-dlg') &&
            !dlgInst.element.classList.contains('e-dlg-modal')
          ) {
            if (e.shiftKey) {
              if (
                document.activeElement.className.includes('e-dlg-closeicon-btn')
              ) {
                const footerOkBtn: HTMLElement = dlgInst.element.querySelector(
              ...
              document.activeElement.className.includes('e-btn-goto-ok')
            ) {
              focus(dlgInst.element as HTMLElement);
            }
          }
    ...
    if (e.altKey) {
      if (e.keyCode === 40) {
        if (
          target.classList.contains('e-dropdown-btn') ||
          target.classList.contains('e-split-btn')
        ) {
          return;
        }
        const filterArgs: { [key: string]: KeyboardEvent | boolean } = {
          e: e,
          isFilterCell: false,
        };
        this.parent.notify(filterCellKeyDown, filterArgs);
        if (filterArgs.isFilterCell) {
          /*alt + down to open filter popup*/
          return;
        }
      }
      if (
        e.keyCode === 40 &&
        !document.getElementById(this.parent.element.id + 'listValid_popup')
      ) {
        const cell: HTMLElement = this.parent.getCell(actIdxes[0], actIdxes[1]);
        if (cell) {
          const listValidation: HTMLElement = cell.querySelector(
            '.e-validation-list .e-ddl'
          );
          if (listValidation) {
            focus(listValidation);
            const ddlEle: HTMLElement =
              listValidation.querySelector('.e-dropdownlist') || listValidation;
            const listObj: { showPopup: Function } = getComponent(
              ddlEle,
              'dropdownlist'
            );
            if (listObj) {
    ...
    const isSheetTabFocus: boolean =
      target.classList.contains('e-add-sheet-tab') ||
      target.classList.contains('e-sheets-list') ||
      (target.classList.contains('e-tab-wrap') &&
        !!closest(target, '.e-sheet-tabs-items')) ||
      target.classList.contains('e-aggregate-list') ||
      target.classList.contains('e-scroll-nav');
    if (
      [9, 37, 38, 39, 40, 33, 34, 35, 36].indexOf(e.keyCode) > -1 &&
      !isSheetTabFocus
    ) {
      e.preventDefault();
    }
    ...
    if (e.keyCode === 36) {
      ...
      if (e.ctrlKey || e.metaKey) {
        const frozenRow: number = skipHiddenIdx(
          sheet,
        ...
        if (e.shiftKey) {
          ...
          selectIdxes = [
            ...
            actIdxes[1],
            frozenRow,
            skipHiddenIdx(sheet, frozenCol, true, 'columns'),
          ];
        } else {
          /* ctrl+home */
          selectIdxes = [
        ...
        const mainPanel: Element =
          this.parent.element.querySelector('.e-main-panel');
        ...
        /* shift+home */
        const startCol: number = skipHiddenIdx(
          sheet,
          frozenCol,
          true,
          'columns'
        ...
        if (
          sheet.frozenColumns &&
          skipHiddenIdx(sheet, actIdxes[1], true, 'columns') === startCol
        ) {
          selectIdxes = [
            selectIdx[0],
            actIdxes[1],
        ...
        if (
          sheet.frozenColumns &&
          (startCol === actIdxes[1] || frozenCol === actIdxes[1])
        ) {
      ...
      if (
        ...
        if (e.shiftKey) {
          if (e.keyCode === 40) {
            /* ctrl+shift+down */
            selectIdx[2] = this.getNextNonEmptyCell(
              selectIdx[2],
              actIdxes[1],
              'down'
            );
            ...
            /* ctrl+shift+right */
            selectIdx[3] = this.getNextNonEmptyCell(
              actIdxes[0],
              selectIdx[3],
              'right'
            );
          ...
          if (e.keyCode === 37) {
            /*ctrl + left*/
            actIdxes[1] = this.getNextNonEmptyCell(
              actIdxes[0],
              actIdxes[1],
              'left'
            );
            ...
            /*ctrl + up*/
            actIdxes[0] = this.getNextNonEmptyCell(
              actIdxes[0],
              actIdxes[1],
              'top'
            );
        ...
        if (
          activeEle.classList.contains('e-spreadsheet') ||
          closest(activeEle, '.e-sheet')
        ) {
          ...
          isNameBox ||
          activeEle.classList.contains('e-insert-function')
        ) {
      ...
      if (e.shiftKey) {
        ...
        this.shiftSelection(e);
        if (
          (e.keyCode === 34 || e.keyCode === 33) &&
          this.parent.scrollModule &&
          ...
          const mainPanel: Element =
            this.parent.element.querySelector('.e-main-panel');
          const topRow: number = skipHiddenIdx(
            sheet,
            getCellIndexes(sheet.paneTopLeftCell)[0],
            true
          );
          const viewportHgt: number = getBottomOffset(
            this.parent,
            topRow
          ).height;
          if (e.keyCode === 34) {
            ...
            if (!this.parent.scrollSettings.isFinite) {
              const vTrack: HTMLElement = this.parent
                .getMainContent()
                .querySelector('.e-virtualtrack') as HTMLElement;
              if (
                vTrack &&
                parseFloat(vTrack.style.height) < scrollTop + viewportHgt
              ) {
          ...
          const aRowIdx: number = skipHiddenIdx(
            sheet,
            getRangeIndexes(sheet.selectedRange)[2],
            true
          );
          ...
          getUpdateUsingRaf((): void => {
            if (e.keyCode === 34) {
              selectIdx[2] = skipHiddenIdx(
                sheet,
                getCellIndexes(sheet.paneTopLeftCell)[0] + selectDiff,
                true
              );
              if (
        ...
        if (
          e.keyCode === 9 ||
          (this.parent.enableRtl ? e.keyCode === 37 : e.keyCode === 39)
        ) {
          /*Right or Tab key*/
          ...
          if (
            actIdxes[1] < sheet.colCount - 1 &&
            (!sheet.isProtected || sheet.protectSettings.selectCells)
          ) {
            actIdxes[1] += 1;
            isNavigate = true;
          ...
          const aRowIdx: number = skipHiddenIdx(
            sheet,
            getCellIndexes(sheet.activeCell)[0],
            true
          );
          const viewportHgt: number = getBottomOffset(
            this.parent,
          ...
          if (e.keyCode === 34) {
            ...
            if (
              sheet.frozenRows &&
              actIdxes[0] < this.parent.frozenRowCount(sheet)
            ) {
              this.parent.selectRange(
                getRangeAddress([topRow, selectIdx[1], topRow, selectIdx[1]])
              );
              return;
            }
            if (Math.round(scrollTop) < 0) {
      ...
      if (
        ...
        if (!this.parent.element.querySelector('.e-find-toolbar')) {
          if (
            ...
            isNavigate =
              actIdxes[0] !== cellIdx[0] || actIdxes[1] !== cellIdx[1];
            actIdxes[1] = cellIdx[1];
            actIdxes[0] = cellIdx[0];
          }
          if (actIdxes[0] <= 0) {
            const contentEle: Element =
              this.parent.getMainContent().parentElement;
    ...
    if (
      isNavigate &&
      (!this.parent.scrollModule || this.parent.scrollModule.isKeyScroll) &&
      !isSheetTabFocus &&
      !closest(document.activeElement, '.e-ribbon') &&
      !target.classList.contains('e-insert-function') &&
      ...
      const ribbon: Element =
        this.parent.showRibbon && closest(document.activeElement, '.e-ribbon');
      if (ribbon) {
        if (closest(document.activeElement, '.e-tab-header')) {
          if (e.shiftKey || ribbon.classList.contains('e-collapsed')) {
            ...
            this.focusEle(
              e,
              '.e-ribbon .e-content .e-toolbar-item:not(.e-separator):not(.e-overlay) .e-btn'
            );
          }
          ...
          if (e.shiftKey) {
            this.focusEle(
              e,
              '.e-ribbon .e-toolbar-items .e-toolbar-item.e-active .e-tab-wrap',
              true
            );
          } else {
            this.focusEle(e, '.e-ribbon .e-drop-icon');
        ...
        if (e.shiftKey) {
          const isNavOrAggregate: boolean =
            target.classList.contains('e-aggregate-list') ||
            target.classList.contains('e-scroll-left-nav');
          if (
            target.classList.contains('e-add-sheet-tab') ||
            isNavOrAggregate ||
            (target.classList.contains('e-sheets-list') &&
              target.previousElementSibling &&
              (target.previousElementSibling as HTMLButtonElement).disabled)
          ) {
            const focusEle: HTMLElement =
              !isNavOrAggregate &&
              this.parent.element.querySelector('.e-aggregate-list');
            if (focusEle) {
              this.focusEle(e, null, false, false, <HTMLInputElement>focusEle);
            } else if (
              !target.classList.contains('e-scroll-left-nav') &&
              this.parent.element.querySelector(
                '.e-sheet-tab-panel .e-scroll-nav'
              )
            ) {
              this.focusEle(e, '.e-sheet-tab-panel .e-scroll-right-nav', true);
            } else {
              const items: HTMLInputElement[] = [].slice.call(
              ...
              if (items[items.length - 1]) {
                this.focusEle(
                  ...
                  true,
                  false,
                  items[items.length - 1].querySelector('.e-tab-wrap')
                );
              }
            }
            ...
            if (idx === 0) {
              ...
              this.focusEle(
                ...
                true,
                false,
                items[idx - 1].querySelector('.e-tab-wrap')
              );
            }
          }
        } else {
          const isAggAvail: boolean =
            !!this.parent.element.querySelector('.e-aggregate-list');
          if (
            target.classList.contains('e-aggregate-list') ||
            (!isAggAvail && target.classList.contains('e-scroll-right-nav'))
          ) {
            this.focusEle(e, '.e-sheet-tab-panel .e-icon-btn:not(:disabled)');
          } else if (target.classList.contains('e-sheets-list')) {
            this.focusEle(
              e,
              '.e-sheet-tab-panel .e-toolbar-item .e-tab-wrap',
              true
            );
            ...
            if (index === items.length - 1) {
              const isNav: boolean = !!this.parent.element.querySelector(
                '.e-sheet-tab-panel .e-scroll-nav'
              );
              if (isNav || !isAggAvail) {
                this.focusEle(
                  e,
                  `.e-sheet-tab-panel .${
                    isNav ? 'e-scroll-nav' : 'e-icon-btn:not(:disabled)'
                  }`,
                  isNav
                );
              }
              ...
              this.focusEle(
                ...
                true,
                false,
                <HTMLInputElement>items[index + 1].querySelector('.e-tab-wrap')
              );
            }
          }
        }
  ...
  private focusEle(
    ...
    if (setTabIndex && !focusEle) {
      const tabEle: HTMLElement = <HTMLElement>(
        this.parent.element.querySelector('.e-ribbon .e-tab')
      );
      const selectedTab: number =
        (tabEle &&
          (getComponent(tabEle, 'tab') as { selectedItem: number })
            .selectedItem) ||
        0;
      focusEle = this.parent.element
        .querySelector('.e-tab-header')
        .getElementsByClassName('e-toolbar-item')[
        selectedTab as number
      ] as HTMLInputElement;
      focusEle =
        focusEle && (focusEle.querySelector('.e-tab-wrap') as HTMLInputElement);
    }
    if (focusEle) {
      e.preventDefault();
      if (startEdit) {
        focusEle.click();
        focus(focusEle);
        focusEle.setSelectionRange(
          focusEle.value.length,
          focusEle.value.length
        );
      } else if (setTabIndex) {
        focusEle.setAttribute('tabindex', '0');
        if (
          focusEle.parentElement.classList.contains('e-active') &&
          focusEle.parentElement.classList.contains('e-toolbar-item')
        ) {
          const tabEle: HTMLElement = this.parent.element.querySelector(
            '.e-sheet-tab-panel .e-sheet-tab'
          );
  ...
  private getNextNonEmptyCell(
    ...
    const sheet: SheetModel = this.parent.getActiveSheet();
    const isNonEmptyCell: (rowIdx: number, colIdx: number) => boolean = (
      rowIdx: number,
      colIdx: number
    ): boolean => {
    ...
    if (position === 'down') {
      ...
      const lastCol: number = skipHiddenIdx(
        sheet,
        sheet.colCount - 1,
        false,
        'columns'
      ...
      for (let colIdx: number = startCol; colIdx < sheet.colCount; colIdx++) {
        ...
        if (checkForEmptyCell) {
          if (
            !isNonEmptyCell(
              rowIdx,
              skipHiddenIdx(sheet, colIdx, true, 'columns')
            )
          ) {
      ...
      for (let colIdx: number = startCol; colIdx >= 0; colIdx--) {
        ...
        if (checkForEmptyCell) {
          if (
            !isNonEmptyCell(
              rowIdx,
              skipHiddenIdx(sheet, colIdx, false, 'columns')
            )
          ) {
  ...
  private getNextUnlockedCell(
    ...
    if (position === 'right') {
      ...
      let colIdx: number;
      let secIteration: boolean;
      let rowLen: number = sheet.usedRange.rowIndex;
      let colLen: number = sheet.usedRange.colIndex;
      while (rowIdx <= rowLen) {
      ...
      while (rowIdx >= rowLen) {
        colIdx =
          colIdx === undefined ? actCellIdx[1] - 1 : sheet.usedRange.colIndex;
        if (secIteration && rowIdx === actCellIdx[0]) {
          colLen = actCellIdx[1] + 1;
        }
        for (colIdx; colIdx >= colLen; colIdx--) {
      ...
      while (colIdx >= colLen) {
        rowIdx =
          rowIdx === undefined ? actCellIdx[0] - 1 : sheet.usedRange.rowIndex;
        if (secIteration && colIdx === actCellIdx[1]) {
          rowLen = actCellIdx[0] + 1;
        }
        for (rowIdx; rowIdx >= rowLen; rowIdx--) {
  ...
  private shiftSelection(e: KeyboardEvent): void {
    ...
    if (e.keyCode === 38) {
      ...
      for (let i: number = swapRange[1]; i <= swapRange[3]; i++) {
        ...
        if (!isNullOrUndefined(cell) && cell.rowSpan && cell.rowSpan < 0) {
          selectedRange[2] = skipHiddenIdx(
            sheet,
            selectedRange[2] - (Math.abs(cell.rowSpan) + 1),
            false
          );
          noHidden = false;
          break;
    ...
    if (e.keyCode === 40) {
      ...
      for (let i: number = swapRange[1]; i <= swapRange[3]; i++) {
        ...
        if (!isNullOrUndefined(cell) && cell.rowSpan && cell.rowSpan > 0) {
          selectedRange[2] = skipHiddenIdx(
            sheet,
            selectedRange[2] + Math.abs(cell.rowSpan),
            true
          );
          noHidden = false;
          break;
    ...
    if (e.keyCode === 39) {
      ...
      for (let i: number = swapRange[0]; i <= swapRange[2]; i++) {
        ...
        if (!isNullOrUndefined(cell) && cell.colSpan && cell.colSpan > 0) {
          selectedRange[3] = skipHiddenIdx(
            sheet,
            selectedRange[3] + Math.abs(cell.colSpan),
            true,
            'columns'
          );
          noHidden = false;
    ...
    if (e.keyCode === 37) {
      ...
      for (let i: number = swapRange[0]; i <= swapRange[2]; i++) {
        ...
        if (!isNullOrUndefined(cell) && cell.colSpan && cell.colSpan < 0) {
          selectedRange[3] = skipHiddenIdx(
            sheet,
            selectedRange[3] - (Math.abs(cell.colSpan) + 1),
            false,
            'columns'
          );
          noHidden = false;
    ...
    if (
      ...
      this.scrollNavigation([
        isColumnSelected(sheet, selectedRange)
          ? activeIdxes[0]
          : selectedRange[2],
  ...
  private scrollNavigation(actIdxes: number[], scrollToCell?: boolean): void {
    ...
    const hCont: Element = this.parent.getScrollElement();
    const sheet: SheetModel = this.parent.getActiveSheet();
    const selectedRange: number[] = getSwapRange(
    ...
    const topIdx: number = skipHiddenIdx(
      sheet,
      actIdxes[0] < frozenRow ? topLeftIdxes[0] : paneTopLeftIdxes[0],
      true
    );
    ...
    if (viewportBtmIdx <= actIdxes[0]) {
      if (actIdxes[0] >= frozenRow) {
        if (scrollToCell) {
          ...
          const rowsHeight: number = getRowsHeight(
            sheet,
            paneTopLeftIdxes[0],
            actIdxes[0],
            true
          ...
          if (rowsHeight > viewPortHeight * 2) {
            ...
            cont.scrollTop =
              offsetTopSize +
              rowsHeight -
              getRowHeight(sheet, actIdxes[0], true);
          }
          focus(this.parent.element);
        } else {
          cont.scrollTop =
            offsetTopSize +
            getRowsHeight(sheet, viewportBtmIdx, actIdxes[0], true);
        }
      }
      ...
      cont.scrollTop =
        offsetTopSize -
        Math.ceil(getRowsHeight(sheet, actIdxes[0], topIdx - 1, true));
      if (scrollToCell) {
        focus(this.parent.element);
      }
    }
    ...
    if (scrollLeftIdx <= actIdxes[1] && hCont) {
      if (actIdxes[1] >= frozenCol) {
        if (scrollToCell) {
          ...
          const scrollWidth: number = getColumnsWidth(
            sheet,
            paneTopLeftIdxes[1],
            actIdxes[1],
            true
          ...
          if (scrollWidth > contWidth * 2) {
            hCont.scrollLeft =
              (this.parent.scrollModule.offset.left.size +
                scrollWidth -
                contWidth) *
              x;
          } else {
            hCont.scrollLeft =
              (this.parent.scrollModule.offset.left.size +
                (scrollWidth -
                  getColumnWidth(sheet, actIdxes[0], null, true))) *
              x;
          }
          focus(this.parent.element);
        } else {
          hCont.scrollLeft =
            (this.parent.scrollModule.offset.left.size +
              getColumnsWidth(sheet, scrollLeftIdx, actIdxes[1], true)) *
            x;
        }
      }
      ...
      hCont.scrollLeft =
        (this.parent.scrollModule.offset.left.size -
          getColumnsWidth(sheet, actIdxes[1], paneTopLeftIdxes[1] - 1, true)) *
        x;
      if (scrollToCell) {
        focus(this.parent.element);
      }
