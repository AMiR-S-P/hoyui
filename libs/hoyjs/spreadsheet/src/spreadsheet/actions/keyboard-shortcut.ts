...
export class KeyboardShortcut {
  ...
  private isTrgtNotInput(e: KeyboardEvent): boolean {
    ...
    return (
      !closest(trgt, '.e-filter-popup') &&
      !closest(trgt, '.e-find-dlg') &&
      !closest(trgt, '.e-hyperlink-dlg') &&
      !closest(trgt, '.e-sheet-tab') &&
      !closest(trgt, '.e-name-box') &&
      !closest(trgt, '.e-link-dialog')
    );
  }

  private ribbonShortCuts(e: KeyboardEvent): void {
    //switch between ribbon tabs
    if (this.parent.showRibbon && e.altKey && !e.ctrlKey && !e.shiftKey) {
      const tabObj: {
        items: { disabled: boolean; cssClass: string }[];
        select: Function;
      } = getComponent(
      ...
      if (e.keyCode === 72) {
        ...
        e.preventDefault();
        select(
          '#' + this.parent.element.id + '_File',
          this.parent.element
        ).click();
      } else if (e.keyCode === 18) {
        /** alt = active tab focus */
        e.preventDefault();
        const activeCell: number[] = getCellIndexes(
        ...
        if (
          !(
            args.hasFilter &&
            args.filterRange &&
            args.filterRange[0] === activeCell[0] &&
            args.filterRange[1] <= activeCell[1] &&
            args.filterRange[3] >= activeCell[1]
          )
        ) {
          const actEle: HTMLElement = document.activeElement as HTMLElement;
          if (
            !actEle.classList.contains('e-spreadsheet-edit') &&
            !actEle.classList.contains('e-sheet-rename') &&
            actEle.id !== `${this.parent.element.id}_name_box` &&
            !closest(actEle, '.e-dropdown-btn') &&
            !closest(actEle, '.e-split-btn') &&
            !closest(actEle, '.e-popup-open')
          ) {
            const cell: HTMLElement = this.parent.getCell(
              activeCell[0],
              activeCell[1]
            );
      ...
      if (tabIdx) {
        if (
          !tabObj.items[tabIdx as number].disabled &&
          !tabObj.items[tabIdx as number].cssClass.includes('e-hide')
        ) {
          e.preventDefault();
  ...
  private keyUpHandler(e: KeyboardEvent): void {
    if (e.keyCode === 37 || e.keyCode === 39) {
      ...
      if (
        activeEle.classList.contains('e-colorpicker-wrapper') ||
        activeEle.classList.contains('e-split-btn-wrapper')
      ) {
        const colorPickerBtn: HTMLElement =
          activeEle.querySelector('.e-split-btn');
        if (colorPickerBtn) {
  ...
  private keyDownHandler(e: KeyboardEvent): void {
    ...
    if ((e.ctrlKey || e.metaKey) && this.isTrgtNotInput(e)) {
      ...
      if (e.keyCode === 79) {
        ...
        const startCell: { colIndex?: number; rowIndex?: number } = {
          colIndex: selectRange[1],
          rowIndex: selectRange[0],
        };
        ...
        this.parent.notify(performAutoFill, {
          ...
          rangeInfo: <FillRangeInfo>{
            direction: direction,
            startCell: startCell,
            endCell: { rowIndex: selectRange[2], colIndex: selectRange[3] },
      ...
      const isLocked: boolean =
        sheet.isProtected &&
        isCellLocked(
          getCell(indexes[0], indexes[1], sheet),
          getColumn(sheet, indexes[1])
        );
      if (
        e.keyCode === 70 &&
        !isSelectionNone &&
        this.parent.allowFindAndReplace
      ) {
        /* Ctrl + F */
        e.preventDefault();
      ...
      if (
        (!isLocked || !sheet.isProtected || e.keyCode === 86) &&
        e.keyCode !== 70 &&
        !isSelectionNone
      ) {
        if (e.keyCode === 71) {
      ...
      if (
        !isSelectionNone &&
        (!sheet.isProtected || sheet.protectSettings.formatCells)
      ) {
        if (e.keyCode === 66) {
          /* Ctrl + B */
          e.preventDefault();
    ...
    if (
      (((e.ctrlKey || e.metaKey) && e.keyCode === 119) ||
        (e.keyCode === 13 && target.classList.contains('e-drop-icon'))) &&
      this.parent.showRibbon
    ) {
      /*ctrl + f8 or Enter*/
      e.preventDefault();
      const expandCollapseIcon: HTMLElement =
        this.parent.element.querySelector('.e-drop-icon');
      ...
      if (e.keyCode === 113) {
        ...
        if (
          ...
          (e.target as HTMLElement).children[
            (e.target as HTMLElement).children.length - 1
        ...
        /* Context menu open Shift+F10 */
        const className: string = document.activeElement.className;
        if (
          ['e-spreadsheet', 'e-cell', 'e-header-cell', 'e-clipboard'].some(
            (cls: string) => className.includes(cls)
          )
        ) {
    ...
    if ((e.ctrlKey || e.metaKey) && e.shiftKey && !isSelectionNone) {
      ...
      if (e.keyCode === 85) {
        /*ctrl + shift + U*/
        e.preventDefault();
        const formulaExpand: HTMLElement = this.parent.element.querySelector(
    ...
    if (
      ...
      e.altKey &&
      e.keyCode === 75 &&
      this.parent.showSheetTabs
    ) {
      /* Shift + Alt + K*/
      (
        this.parent.element.querySelector('.e-sheets-list') as HTMLElement
      ).click();
    }
  }
  private focusTarget(sheet: SheetModel): void {
    ...
    if (isRowSelected && isColSelected) {
      ...
      if (indexes[0] < frozenRow) {
        const freezeHdrRow: HTMLTableRowElement = this.parent.getRow(
          indexes[0],
      ...
      if (indexes[1] < frozenCol) {
        const freezeHdrRow: HTMLTableRowElement =
          this.parent.element.querySelector(
            '.e-selectall-container .e-header-row'
          );
        target =
          freezeHdrRow && this.parent.getCell(0, indexes[1], freezeHdrRow);
      } else {
        indexes[1] = skipHiddenIdx(sheet, indexes[1], true, 'columns');
        const leftIdx: number = skipHiddenIdx(
          sheet,
        ...
        const hdrRow: HTMLTableRowElement = this.parent.element.querySelector(
