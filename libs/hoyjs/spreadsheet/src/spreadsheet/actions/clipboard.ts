...
import { Deferred } from '@hoyui/data';
import { BeforeOpenEventArgs } from '@hoyui/popups';
import {
  refreshRibbonIcons,
  refreshClipboard,
  getColumn,
  isLocked as isCellLocked,
...
export class Clipboard {
  ...
  private cMenuBeforeOpenHandler(e: { target: string }): void {
    ...
    let delRowItems: string[] = [];
    let hideRowItems: string[] = [];
    let delColItems: string[] = [];
    let hideColItems: string[] = [];
    ...
    const isLocked: boolean =
      sheet.isProtected &&
      isCellLocked(cellObj, getColumn(sheet, actCellIndex[1]));
    const isReadonlyCell: boolean = isReadOnly(
      cellObj,
      getColumn(sheet, actCellIndex[1]),
      getRow(sheet, actCellIndex[0])
    );
    if (
      e.target === 'Content' ||
      e.target === 'RowHeader' ||
      ...
      this.parent.enableContextMenuItems(
        [l10n.getConstant('Paste'), l10n.getConstant('PasteSpecial')],
        this.copiedInfo || (this.copiedShapeInfo && !isLocked) ? true : false
      );
    ...
    if (e.target === 'Content') {
      if (isLocked) {
        this.parent.enableContextMenuItems(
          [
            l10n.getConstant('Cut'),
            l10n.getConstant('Filter'),
            l10n.getConstant('Sort'),
            l10n.getConstant('Hyperlink'),
            l10n.getConstant('AddNote'),
            l10n.getConstant('EditNote'),
            l10n.getConstant('DeleteNote'),
          ],
          false
        );
        ...
        this.parent.enableContextMenuItems(
          [
            l10n.getConstant('Cut'),
            l10n.getConstant('Paste'),
            l10n.getConstant('PasteSpecial'),
            l10n.getConstant('Filter'),
            l10n.getConstant('Sort'),
            l10n.getConstant('Hyperlink'),
            l10n.getConstant('EditHyperlink'),
            l10n.getConstant('OpenHyperlink'),
            l10n.getConstant('RemoveHyperlink'),
          ],
          false
        );
    ...
    if (sheet.isProtected) {
      if (e.target === 'ColumnHeader') {
        delColItems = [
          l10n.getConstant('DeleteColumn'),
          l10n.getConstant('DeleteColumns'),
          l10n.getConstant('InsertColumn'),
          l10n.getConstant('InsertColumns'),
        ];
        hideColItems = [
          l10n.getConstant('HideColumn'),
          l10n.getConstant('HideColumns'),
          l10n.getConstant('UnhideColumns'),
        ];
        this.parent.enableContextMenuItems(delColItems, false);
        this.parent.enableContextMenuItems(
          hideColItems,
          sheet.protectSettings.formatColumns
        );
      }
      if (e.target === 'RowHeader') {
        delRowItems = [
          l10n.getConstant('DeleteRow'),
          l10n.getConstant('DeleteRows'),
          l10n.getConstant('InsertRow'),
          l10n.getConstant('InsertRows'),
        ];
        hideRowItems = [
          l10n.getConstant('HideRow'),
          l10n.getConstant('HideRows'),
          l10n.getConstant('UnhideRows'),
        ];
        this.parent.enableContextMenuItems(delRowItems, false);
        this.parent.enableContextMenuItems(
          hideRowItems,
          sheet.protectSettings.formatRows
        );
      }
    }
  }
    ...
    if (this.copiedInfo) {
      ...
      if (ele) {
        if (this.copiedInfo.range[0] > args.rowIdx) {
          ...
          inRange(this.copiedInfo.range, args.rowIdx, this.copiedInfo.range[1])
        ) {
          ele.style.height = `${
    ...
    if (this.copiedInfo) {
      ...
      if (ele) {
        if (this.copiedInfo.range[1] > args.colIdx) {
          ...
          inRange(this.copiedInfo.range, this.copiedInfo.range[0], args.colIdx)
        ) {
  ...
  private paste(
    ...
    if (
      this.parent.isEdit ||
      this.parent.element.getElementsByClassName('e-dlg-overlay').length > 0
    ) {
      const editEle: HTMLElement = this.parent.element.getElementsByClassName(
        'e-spreadsheet-edit'
      )[0] as HTMLElement;
    ...
    let isExternal: DataTransfer | boolean =
      (args && args.clipboardData) || window['clipboardData'];
    if (
      isExternal &&
      args.clipboardData &&
      args.clipboardData.getData('isInternalCut').length &&
      !this.copiedInfo
    ) {
    ...
    if (
      ...
      (args.isInternal && this.copiedInfo)
    ) {
      ...
      const cSIdx: number =
        args && args.sIdx > -1 ? args.sIdx : this.parent.activeSheetIndex;
      const curSheet: SheetModel = getSheet(this.parent, cSIdx);
      let selIdx: number[] = getSwapRange(
        (args && args.range) || getRangeIndexes(curSheet.selectedRange)
      );
      let pasteModelArgs:
        | PasteModelArgs
        | { internal: boolean }
      ...
      if (isExternal) {
        ...
        if (
          !args.isInternal &&
          (pasteModelArgs as { internal: boolean }).internal
        ) {
          isExternal = false;
          if (!this.copiedInfo) {
            return;
        ...
        if (!rows || !rows.length) {
          ...
          if ((pasteModelArgs as { file: File }).file) {
            this.parent.notify(insertImage, <OpenOptions>{
              file: (pasteModelArgs as { file: File }).file,
            });
            return;
      ...
      const copyInfo: { range: number[]; sId: number; isCut: boolean } =
        Object.assign({ isExternal: isExternal }, this.copiedInfo);
      let cIdx: number[];
      let pSheetIdx: number;
      let column: ColumnModel;
      let notRemoveMerge: boolean;
      let isRepeative: boolean;
      let cSheetSel: string;
      let prevSheet: SheetModel;
      ...
      if (isExternal) {
        ...
        isRepeative =
          cSheetSel !== 'Sheet' &&
          (selIdx[2] - selIdx[0] + 1) % pasteModelArgs.rowCount === 0 &&
          (selIdx[3] - selIdx[1] + 1) % pasteModelArgs.colCount === 0;
        cIdx = [0, 0, pasteModelArgs.usedRowIndex, pasteModelArgs.usedColIndex];
      } else {
        cIdx = getSwapRange(
          this.copiedShapeInfo
            ? getRangeIndexes(curSheet.selectedRange)
            : this.copiedInfo.range
        );
        pSheetIdx = copiedIdx;
        column = getColumn(curSheet, cIdx[1]);
        notRemoveMerge =
          isSingleCell(cIdx) && this.isRangeMerged(selIdx, curSheet);
        prevSheet = getSheet(this.parent, pSheetIdx);
        isRepeative =
          !notRemoveMerge &&
          (selIdx[2] - selIdx[0] + 1) % (cIdx[2] - cIdx[0] + 1) === 0 &&
      ...
      rfshRange = isRepeative
        ? selIdx
        : [selIdx[0], selIdx[1]].concat([
            selIdx[0] + cIdx[2] - cIdx[0],
            selIdx[1] + cIdx[3] - cIdx[1] || selIdx[1],
          ]);
      if (cSheetSel) {
        if (
          (cSheetSel === 'Sheet' || cSheetSel === 'Column') &&
          rfshRange[2] < curSheet.usedRange.rowIndex
        ) {
          rfshRange[2] = curSheet.usedRange.rowIndex;
        }
        if (
          (cSheetSel === 'Sheet' || cSheetSel === 'Row') &&
      ...
      if (args.isAction && !this.copiedShapeInfo) {
        ...
        if (isExternal) {
          isRepeative =
            pasteModelArgs.selection !== 'Sheet' &&
            (selIdx[2] - selIdx[0] + 1) % pasteModelArgs.rowCount === 0 &&
            (selIdx[3] - selIdx[1] + 1) % pasteModelArgs.colCount === 0;
        } else {
          isRepeative =
      ...
      let prevCell: CellModel;
      let rowIdx: number = selIdx[0];
      ...
      if (this.copiedShapeInfo && !this.copiedInfo) {
        const pictureElem: HTMLElement = this.copiedShapeInfo
          .pictureElem as HTMLElement;
        if (pictureElem.classList.contains('e-datavisualization-chart')) {
          ...
          this.parent.notify(createImageElement, {
            options: {
              src: pictureElem.style.backgroundImage.replace(
                /url\((['"])?(.*?)\1\)/gi,
                '$2'
              ),
        ...
        if (
          pastedCell &&
          !isNullOrUndefined(pastedCell.image) &&
          pastedCell.image.length > 0
        ) {
          const eventArgs: Object = {
            ...
            pastedRange:
              getSheetName(this.parent as Workbook) +
              '!' +
              getRangeAddress([rowIdx, selIdx[1], rowIdx, selIdx[1]]),
            pastedPictureElement: document.getElementById(
              pastedCell.image[pastedCell.image.length - 1].id
            ),
          };
        ...
        const copyCellArgs: {
          sheet: SheetModel;
          isExternal?: boolean;
          isRandFormula?: boolean;
        } = {
          sheet: curSheet,
          isExternal: !!isExternal,
        };
        const pasteSetCell: Function = this.setCell(copyCellArgs);
        const cutSetCell: Function =
          !isExternal &&
          this.copiedInfo.isCut &&
          this.setCell({ sheet: prevSheet });
        for (let i: number = cIdx[0], l: number = 0; i <= cIdx[2]; i++, l++) {
          ...
          for (let j: number = cIdx[1], k: number = 0; j <= cIdx[3]; j++, k++) {
            if (isInRange) {
              cRows[selIdx[0] + l].cells[selIdx[1] + k] = getCell(
                selIdx[0] + l,
                selIdx[1] + k,
                prevSheet,
                false,
            ...
            cell = isExternal
              ? (rows[i as number] && rows[i as number].cells[j as number]) ||
                {}
              : extend(
                  {},
                  isInRange &&
                    cRows[i as number] &&
                    cRows[i as number].cells[j as number]
                    ? cRows[i as number].cells[j as number]
                    : getCell(i, j, prevSheet),
                  null,
                  true
                );
            ...
            if (isRowSelected || isColSelected) {
              if (cell && cell.rowSpan) {
                if (cell.rowSpan > 0) {
                  ...
                  const spanCell: CellModel = getCell(
                    rowIdx + rowSpan,
                    selIdx[1] + k + colSpan,
                    curSheet
                  );
              ...
              switch (pasteType) {
                ...
                case 'Values':
                  ...
                  if (cell.value && cell.value.toString().indexOf('\n') > -1) {
                    const ele: Element = this.parent.getCell(
                      selIdx[0],
                      selIdx[1]
                    );
            ...
            if (
              (!this.parent.scrollSettings.isFinite &&
                (cIdx[2] - cIdx[0] > 1048575 - selIdx[0] ||
                  cIdx[3] - cIdx[1] > 16383 - selIdx[1])) ||
              (this.parent.scrollSettings.isFinite &&
                (cIdx[2] - cIdx[0] > curSheet.rowCount - 1 - selIdx[0] ||
            ...
            if (isRepeative) {
              for (
                ...
                if (
                  !copyInfo.isCut &&
                  !hiddenCount &&
                  isFilterHidden(curSheet, x + l)
                ) {
                  continue;
                }
                for (
                  let y: number = selIdx[1];
                  y <= selIdx[3];
                  y += cIdx[3] - cIdx[1] + 1
                ) {
                  if (
                    i === cIdx[0] &&
                    j === cIdx[1] &&
                    (cfRule === undefined || cfRule.length) &&
                    pasteType !== 'Values'
                  ) {
                    cfRule = this.setCF(
                      cIdx,
                      [x, y, x + (cIdx[2] - cIdx[0]), y + (cIdx[3] - cIdx[1])],
                      prevSheet,
                      curSheet,
                      cf,
                      cfRule
                  ...
                  if (
                    !isExternal &&
                    (!isNullOrUndefined(prevCell.colSpan) ||
                      !isNullOrUndefined(prevCell.rowSpan))
                  ) {
                    if (isRowSelected || isColSelected) {
                      continue;
                    }
                    const merge: MergeArgs = {
                  ...
                  if (
                    ...
                    const newFormula: string = getUpdatedFormula(
                      [x + l, colInd],
                      [i, j],
                      prevSheet,
                      this.parent,
                  ...
                  if (
                    prevCell &&
                    prevCell.formula &&
                    prevCell.formula.indexOf('=UNIQUE(') > -1
                  ) {
              ...
              if (
                isExternal ||
                !hasTemplate(this.parent as Workbook, i, j, copiedIdx)
              ) {
                if (notRemoveMerge) {
                  pasteSetCell(
                    rowIdx,
        ...
        if (uniqueCellColl.length) {
          for (let i: number = 0; i < uniqueCellColl.length; i++) {
            this.parent.serviceLocator
              ...
              .refresh(
                uniqueCellColl[i as number][0],
                uniqueCellColl[i as number][1]
              );
          }
        }
        ...
        if (
          cSIdx === this.parent.activeSheetIndex &&
          !args.isFromUpdateAction
        ) {
          ...
          setMaxHgt(
            ...
            (
              this.parent.getRow(
                cIdx[0],
                null,
                this.parent.frozenColCount(prevSheet)
              ) || { offsetHeight: 20 }
            ).offsetHeight
          );
        ...
        const copySheet: SheetModel = getSheet(
        ...
        if (args.isAction) {
          const eventArgs: Object = {
            ...
            copiedRange:
              ...
              '!' +
              (copyInfo.range
                ? getRangeAddress(copyInfo.range)
                : prevSheet.selectedRange),
  ...
  private setCF(
    ...
    cSheet: SheetModel,
    pSheet: SheetModel,
    cf: ConditionalFormatModel[],
    conditionalFormats?: ConditionalFormatModel[]
  ): ConditionalFormatModel[] {
    let cfRange: number[];
    let indexes: number[];
    const assignCF: Function = (
      conditionalFormat: ConditionalFormatModel
    ): void => {
      cfRange = [
        pRange[0] + (indexes[0] <= cRange[0] ? 0 : indexes[0] - cRange[0]),
    ...
    if (conditionalFormats) {
      for (
        let i: number = 0, len: number = conditionalFormats.length;
        i < len;
        i++
      ) {
      ...
      if (cSheet.conditionalFormats) {
        for (
          let i: number = 0, len: number = cSheet.conditionalFormats.length;
          i < len;
          i++
        ) {
          indexes = getRangeIndexes(
            cSheet.conditionalFormats[i as number].range
          );
          if (
            checkRange([cRange], cSheet.conditionalFormats[i as number].range)
          ) {
  ...
  private updateFilter(
    copyInfo: { range: number[]; sId: number; isCut: boolean },
    pastedRange: number[]
  ): void {
    ...
    for (let i: number = 0; i < this.parent.sheets.length; i++) {
      if (
        ...
        this.parent.filterCollection[i as number] &&
        this.parent.filterCollection[i as number].sheetIndex ===
          getSheetIndexFromId(this.parent as Workbook, copyInfo.sId)
      ) {
        let range: number[] = copyInfo.range;
        const fRange: number[] = getRangeIndexes(
          this.parent.filterCollection[i as number].filterRange
        );
        ...
        if (
          ...
          isFilterCut = true;
          diff = [
            Math.abs(range[0] - fRange[0]),
            Math.abs(range[1] - fRange[1]),
            Math.abs(range[2] - fRange[2]),
            Math.abs(range[3] - fRange[3]),
          ];
        }
      }
    }
    let cell: HTMLElement = this.parent.getCell(
      copyInfo.range[0],
      copyInfo.range[1]
    );
    cell = cell
      ? cell.querySelector('.e-filter-icon')
        ? cell
        : this.parent.getCell(copyInfo.range[2], copyInfo.range[3])
      : cell;
    const asc: HTMLElement = cell
      ? cell.querySelector('.e-sortasc-filter')
      : cell;
    const desc: HTMLElement = cell
      ? cell.querySelector('.e-sortdesc-filter')
      : cell;
    if (isFilterCut) {
      for (let n: number = 0; n < this.parent.filterCollection.length; n++) {
        const filterCol: FilterCollectionModel =
          this.parent.filterCollection[n as number];
        const sheetIndex: number =
          copyInfo && copyInfo.sId
            ? getSheetIndexFromId(this.parent as Workbook, copyInfo.sId)
        ...
        if (
          filterCol.sheetIndex === sheetIndex &&
          sheetIndex === this.parent.activeSheetIndex
        ) {
          diff = [
            pastedRange[0] + diff[0],
            pastedRange[1] + diff[1],
            Math.abs(pastedRange[2] - diff[2]),
            Math.abs(pastedRange[3] - diff[3]),
          ];
          this.parent.notify(initiateFilterUI, {
            predicates: null,
          ...
          if (copyInfo.range[3] === copyInfo.range[1]) {
            // To update sorted icon after pasting.
            const filteredCell: HTMLElement = this.parent.getCell(
              pastedRange[0],
              pastedRange[1]
            );
            if (asc && filteredCell) {
              filteredCell
                .querySelector('.e-filter-icon')
                .classList.add('e-sortasc-filter');
            }
            if (desc && filteredCell) {
              filteredCell
                .querySelector('.e-filter-icon')
                .classList.add('e-sortdesc-filter');
            }
          }
        }
      }
  ...
  private isInRange(cRng: number[], pRng: number[], sIdx: number): boolean {
    ...
    return (
      (inRange(cRng, pRng[0], pRng[1]) && sIdx === activeSheetIndex) ||
      (inRange(cRng, pRng[2], pRng[3]) && sIdx === activeSheetIndex)
    );
  }

  private setCell(args: {
    sheet: SheetModel;
    isExternal?: boolean;
    isRandFormula?: boolean;
  }): Function {
    ...
    return (
      ...
      if (
        cell &&
        cell.formula &&
        (cell.formula.indexOf('RANDBETWEEN(') > -1 ||
          cell.formula.indexOf('RAND(') > -1 ||
      ...
      if (!cancel && cell) {
        if (cell.validation) {
          ...
          this.parent.dataValidationRange +=
        ...
        if (cell.style && args.isExternal) {
          let hgt: number = getTextHeightWithBorder(
            ...
            cell.wrap
              ? getLines(
                  this.parent.getDisplayText(cell),
                  getExcludedColumnWidth(sheet, rIdx, cIdx),
                  cell.style,
  ...
  private setCopiedInfo(
    args?: ClipboardInfo & ClipboardEvent & { isFromUpdateAction?: boolean },
    isCut?: boolean
  ): void {
    ...
    if (args && args.range) {
      const isRowSelected: boolean =
        args.range[1] === 0 && args.range[3] === sheet.colCount - 1;
      const isColSelected: boolean =
        args.range[0] === 0 && args.range[2] === sheet.rowCount - 1;
    ...
    this.checkForUncalculatedFormula(
      range,
      args && args.sId ? args.sId : sheet.id
    );
    option.promise.then(() => {
      if (pictureLen > 0) {
        ...
        this.copiedShapeInfo = {
          ...
          isCut: isCut,
          pictureElem: pictureElements[0] as HTMLElement,
          copiedRange: getRangeAddress([
            imgRowIdx.clientY,
            imgColIdx.clientX,
            imgRowIdx.clientY,
            imgColIdx.clientX,
          ]),
          height: (pictureElements[0] as HTMLElement).offsetHeight,
        ...
        if (
          ...
          const imgURL: string = window
            .getComputedStyle(pictureElements[0])
            .backgroundImage.slice(5, -2);
          this.addImgToClipboard(
            imgURL,
            this.copiedShapeInfo.height,
            this.copiedShapeInfo.width
          );
        }
        this.hidePaste(true);
        if (isCut) {
          if (
            ...
            this.parent.notify(deleteImage, {
              id: this.copiedShapeInfo.pictureElem.id,
              sheetIdx: this.copiedShapeInfo.sId,
    ...
    if (args && args.clipboardData) {
      ...
      this.getClipboardEle().setAttribute(
        'aria-label',
        `${sheet.selectedRange} ${this.parent.serviceLocator
          .getService<L10n>(locale)
          .getConstant(isCut ? 'Cut' : 'Copy')}`
      );
    }
  }
  ...
  private async addImgToClipboard(
    src: string,
    height: number,
    width: number
  ): Promise<void> {
  ...
  private removeIndicator(ele: Element): void {
    if (ele) {
      const indicator: Element =
        ele.getElementsByClassName('e-copy-indicator')[0];
      if (indicator) {
        detach(indicator);
      }
    }
  ...
  private initCopyIndicator(): void {
    if (
      this.copiedInfo &&
      this.parent.getActiveSheet().id === this.copiedInfo.sId
    ) {
      const copyIndicator: HTMLElement = this.parent.createElement('div', {
        className: 'e-copy-indicator',
      });
      copyIndicator.appendChild(
  ...
  private showDialog(): void {
    (this.parent.serviceLocator.getService(dialog) as Dialog).show({
      ...
      showCloseIcon: true,
      content: (
        this.parent.serviceLocator.getService(locale) as L10n
      ).getConstant('PasteAlert'),
  ...
  private hidePaste(isShow?: boolean): void {
    ...
    this.parent.notify(enableToolbarItems, [
      { items: [this.parent.element.id + '_paste'], enable: isShow || false },
    ]);
  }
  ...
  private setExternalCells(args: ClipboardEvent, isCut?: boolean): void {
    ...
    const isRowSelected: boolean =
      range[1] === 0 && range[3] === sheet.colCount - 1;
    const isColSelected: boolean =
      range[0] === 0 && range[2] === sheet.rowCount - 1;
    let data: string =
      '<html><body><table class="e-spreadsheet" xmlns="http://www.w3.org/1999/xhtml"';
    if (isRowSelected || isColSelected) {
          ...
          Object.keys(cell.style).forEach((style: string) => {
            ...
            cellStyle +=
              (style === 'backgroundColor'
                ? 'background'
                : regex
                ? style.replace(regex[0], '-' + regex[0].toLowerCase())
                : style) +
              ':' +
              (style === 'backgroundColor' || style === 'color'
                ? cell.style[`${style}`].slice(0, 7)
                : cell.style[`${style}`]) +
              ';';
          });
          data += cellStyle.includes('"')
            ? ` style='${cellStyle}'`
            : ` style="${cellStyle}"`;
        } else {
          data +=
            ' style="white-space:' +
            (cell.wrap ? 'normal' : 'nowrap') +
            ';vertical-align:bottom;"';
        }
        if (!isNullOrUndefined(cell.value)) {
          val = cell.value;
          if (cell.format && cell.format !== 'General') {
            data += cell.value.toString().includes('"')
              ? " cell-value='" + val + "'"
              : ' cell-value="' + cell.value + '"';
            data += cell.format.includes('"')
              ? " num-format='" + cell.format + "'"
              : ' num-format="' + cell.format + '"';
  ...
  private getExternalCells(
    args: ClipboardEvent
  ): PasteModelArgs | { internal: boolean } | { file: File } {
    let html: string;
    let text: string;
    const rows: RowModel[] = [];
    ...
    const clearClipboard: Function = () =>
      setTimeout(() => {
        this.getClipboardEle().innerHTML = '';
      }, 0);
    if (Browser.isIE) {
      ...
      if (this.copiedInfo && html.includes('<table class="e-spreadsheet"')) {
        ...
        if (!this.copiedInfo.isCut) {
          const filterArgs: FilterInfoArgs = {
            sheetIdx: getSheetIndexFromId(
              this.parent as Workbook,
              this.copiedInfo.sId
            ),
          };
          ...
          if (filterArgs.isFiltered) {
            ...
            isFilteredRange =
              indexes[0] === copyIndexes[0] &&
              indexes[1] === copyIndexes[1] &&
              indexes[2] === copyIndexes[2] &&
    ...
    if (ele.querySelector('table')) {
      ...
      this.parent.notify(createImageElement, {
        options: { src: img.src, height: img.height, width: img.width },
        isPublic: true,
      });
    } else if (text) {
      let cells: CellModel[] = [];
      let cellStyle: CellStyleModel;
      let childArr: Element[];
      let filteredChild: Element;
      if (html) {
      ...
      text.split('\n').forEach((row: string) => {
        ...
        row.split('\t').forEach((col: string, j: number) => {
          if (col || cellStyle) {
            ...
            if (cellStyle) {
              if (
                (cellStyle as { whiteSpace: string }).whiteSpace &&
        ...
        pasteModelArgs.colCount = Math.max(
          pasteModelArgs.colCount,
          cells.length
        );
        cells = [];
      });
      ...
      args.clipboardData.files &&
      args.clipboardData.files[0] &&
  ...
  private generateCells(ele: Element, pasteModelArgs: PasteModelArgs): void {
    ...
    const tableStyleObj: CellStyleModel = {};
    const rowStyleObj: CellStyleModel = {};
    ...
    if (isSpreadsheet) {
      ...
      if (pasteModelArgs.selection) {
        if (pasteModelArgs.selection === 'Sheet') {
          pasteModelArgs.usedRowIndex = Number(
            table.getAttribute('aria-rowcount')
          );
          pasteModelArgs.usedColIndex = Number(
    ...
    let curColIdx: number;
    pasteModelArgs.colCount = 1;
    const rowStyles: string[] = [];
    for (
      let rowIdx: number = 0, rowLen: number = pasteModelArgs.usedRowIndex;
      rowIdx <= rowLen;
      rowIdx++
    ) {
      ...
      colLen =
        pasteModelArgs.usedColIndex < tr.cells.length
          ? pasteModelArgs.usedColIndex
          : tr.cells.length - 1;
      for (let colIdx: number = 0; colIdx <= colLen; colIdx++) {
        ...
        if (Object.keys(cellStyle).length) {
          if (cellStyle.border) {
            ['borderBottom', 'borderTop', 'borderLeft', 'borderRight'].forEach(
              (prop: string): void => {
        ...
        if (td.textContent) {
          cells[colIdx as number].value = <string>(
            parseIntValue(td.textContent.trim())
          );
        }
        ...
        if (td.getAttribute(formatStr)) {
          ...
          if (cells[colIdx as number].value && td.getAttribute('cell-value')) {
            cells[colIdx as number].value = <string>(
              parseIntValue(td.getAttribute('cell-value').trim())
            );
          }
        }
        if (
          ...
          cells[colIdx as number].colSpan = parseInt(
            td.getAttribute('colspan'),
            10
          );
        }
        if (
          ...
          cells[colIdx as number].rowSpan = parseInt(
            td.getAttribute('rowspan'),
            10
          );
        }
        if (
          cells[colIdx as number].colSpan > 1 &&
          cells[colIdx as number].rowSpan > 1
        ) {
          let cell: CellModel;
          for (
            let k: number = rowIdx,
              len: number = rowIdx + cells[colIdx as number].rowSpan;
            k < len;
            k++
          ) {
            for (
              let l: number = colIdx,
                len: number = colIdx + cells[colIdx as number].colSpan;
              l < len;
              l++
            ) {
              ...
              cell = cells[colIdx as number].style
                ? { style: extend({}, cells[colIdx as number].style) }
                : {};
              if (k !== rowIdx) {
          ...
          for (
            ...
            (rows as RowModel[])[k as number].cells[colIdx as number] = {
              rowSpan: rowIdx - k,
              style: extend({}, cellStyle),
            };
          }
  ...
  private cellStyle(ele: Element, isSpreadsheet?: boolean): Function {
    ...
    if (!isSpreadsheet) {
      eleStyle =
        ele.querySelector('style') && ele.querySelector('style').innerHTML;
      const keys: string[] = Object.keys(this.parent.commonCellStyle);
      if (keys && keys.length && eleStyle) {
        let tdStyle: string = eleStyle.includes('td')
          ? eleStyle.split('td')[1]
          : eleStyle;
        tdStyle = tdStyle.includes('{')
          ? tdStyle.split('{')[1].split('}')[0]
          : tdStyle.split('}')[0];
        commonStyle = {};
        for (let i: number = 0; i < keys.length; i++) {
          ...
          if (tdStyle.indexOf(key) > -1) {
            commonStyle[keys[i as number]] = tdStyle
              .split(key + ':')[1]
              .split(';')[0]
              .trim();
          }
    ...
    return (
      td: Element,
      rowStyleObj: CellStyleModel,
      tableStyleObj: CellStyleModel
    ): CellStyleModel => {
      ...
      if (isSpreadsheet) {
        ...
        if (eleStyle && td.className && eleStyle.includes(td.classList[0])) {
          ...
          styles.push(
            styleTagCSS[styleTagCSS.length - 1].split('{')[1].split('}')[0]
          );
        }
      ...
      if (
        ...
        ['underline', 'line-through', 'underline line-through', 'none'].indexOf(
          cellStyle.textDecoration
        ) === -1
      ) {
      ...
      if (
        cellStyle.textAlign &&
        ['left', 'center', 'right'].indexOf(cellStyle.textAlign) === -1
      ) {
        cellStyle.textAlign = 'left';
      }
      if (
        cellStyle.verticalAlign &&
        ['bottom', 'middle', 'top'].indexOf(cellStyle.verticalAlign) === -1
      ) {
      ...
      if (cellStyle.fontSize) {
        cellStyle.fontSize =
          Math.round(
            parseFloat(
              cellStyle.fontSize.indexOf('px') > -1
                ? (parseFloat(cellStyle.fontSize) * 0.75).toString()
                : cellStyle.fontSize.indexOf('em') > -1
                ? ((parseFloat(cellStyle.fontSize) * 16) / 1.3333).toString()
                : cellStyle.fontSize
            )
          ) + 'pt';
      }
      if (
        ...
        ['bold', 'normal'].indexOf(cellStyle.fontWeight) === -1
      ) {
  ...
  private refreshOnInsertDelete(args: {
    model: SheetModel;
    start: number;
    end: number;
    modelType: ModelType;
  ...
  private getClipboardEle(): HTMLInputElement {
    return this.parent.element.getElementsByClassName(
      'e-clipboard'
