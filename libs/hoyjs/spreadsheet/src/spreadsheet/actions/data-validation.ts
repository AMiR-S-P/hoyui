...
import { DropDownList, PopupEventArgs } from '@hoyui/dropdowns';
import { DialogModel, BeforeOpenEventArgs } from '@hoyui/popups';
import {
  ValidationModel,
  ValidationType,
  CellStyleModel,
...
export class DataValidation {
  ...
  protected destroy(): void {
    ...
    if (
      ...
      const activeCellIndex: number[] = getCellIndexes(
        this.parent.getActiveSheet().activeCell
      );
      const activeCell: HTMLElement = this.parent.getCell(
        activeCellIndex[0],
        activeCellIndex[1]
      );
      if (activeCell) {
    ...
    this.removeListDropdownHandler(
      this.parent.element.querySelector('.e-validation-list'),
      validationArgs
    );
    const dataValPopup: HTMLElement = select(
      '#' + this.parent.element.id + '_datavalidation-popup'
    );
    if (dataValPopup) {
      dataValPopup.remove();
    }
  ...
  private addEventListener(): void {
    ...
    this.parent.on(
      initiateDataValidation,
      this.initiateDataValidationHandler,
      this
    );
  ...
  private removeValidationHandler(eventArgs: {
    isAction: boolean;
    range?: string;
    isCol?: boolean;
  }): void {
    ...
    let range: string;
    const args: { range?: string; cancel: boolean; isColSelected?: boolean } = {
      cancel: false,
    ...
    if (eventArgs.range && eventArgs.range.includes('!')) {
      ...
      sheet = getSheet(
        this.parent,
        getSheetIndex(this.parent, range.split('!')[0])
      );
      if (!sheet) {
        return;
      }
      ...
      if (eventArgs.range) {
        ...
        const rangeArgs: { range: string; isColSelected: boolean } =
          this.getRange(sheet.selectedRange);
  ...
  private keyUpHandler(e: KeyboardEvent): void {
    ...
    const dlgEle: HTMLElement = this.parent.element.querySelector(
  ...
  private listOpen(e: MouseEvent): void {
    ...
    if (
      ...
      target.querySelector('.e-validation-list') &&
      this.parent.isEdit
    ) {
  ...
  private invalidDataHandler(args: {
    isRemoveHighlight: boolean;
    range?: string;
    isPublic?: boolean;
  }): void {
    const eventArgs: { range: string; cancel: boolean } = {
      range: args.range || this.parent.dataValidationRange,
      cancel: false,
    };
    if (!eventArgs.range) {
      return;
    ...
    if (!args.isPublic) {
      actionArgs = {
        eventArgs: eventArgs,
        action: args.isRemoveHighlight ? 'removeHighlight' : 'addHighlight',
      };
  ...
  private listHandler(): void {
    ...
    const tdEle: HTMLTableCellElement = <HTMLTableCellElement>(
      this.parent.getCell(indexes[0], indexes[1])
    );
    if (!tdEle) {
      return;
    }
    this.removeListDropdownHandler(
      <HTMLElement>document.getElementsByClassName('e-validation-list')[0]
    );
    const validation: ValidationModel =
      (cell && cell.validation) ||
      (sheet.columns &&
        sheet.columns[indexes[1]] &&
        sheet.columns[indexes[1]].validation);
    if (validation && validation.type === 'List') {
      if (
        validation.address &&
        !isInMultipleRange(validation.address, indexes[0], indexes[1])
      ) {
        return;
      }
      this.addListValidationDropdownHandler({
  ...
  private removeListDropdownHandler(
    ...
    if (listEle) {
      ...
      if (!validationArgs) {
        if (
          !isNullOrUndefined(this.parent.selectionModule.previousActiveCell)
        ) {
          const pervActiveCellIdx: number[] = getCellIndexes(
            this.parent.selectionModule.previousActiveCell
          );
          const pervActiveCellEle: HTMLElement = this.parent.getCell(
            pervActiveCellIdx[0],
            pervActiveCellIdx[1]
          );
        ...
        this.updateNoteIndicator(
          validationArgs.td,
          validationArgs.rowIdx,
          validationArgs.colIdx
        );
      }
    }
  }

  private addListValidationDropdownHandler(args: {
    ...
    if (args.isRefresh) {
      if (!args.td) {
        args.td = <HTMLTableCellElement>(
          this.parent.getCell(args.rowIdx, args.colIdx)
        );
        if (!args.td) {
          return;
        }
      ...
      this.removeListDropdownHandler(
        args.td.querySelector('.e-validation-list'),
        args
      );
    ...
    if (inCellDropDown) {
      ...
      const ddlEle: HTMLElement = this.parent.createElement('input', {
      ...
      const parent: Element =
        tdEle.getElementsByClassName('e-wrap-content')[0] || tdEle;
      this.listValidationHeightHandler({ ddlCont: ddlCont });
      parent.insertBefore(ddlCont, parent.firstChild);
      const dataSource: { [key: string]: string }[] = this.getListDataSource(
        args.validation
      );
      this.listObj = new DropDownList({
        ...
        beforeOpen: () => {
          isDevice = (window as { browserDetails?: { isDevice?: boolean } })
            .browserDetails.isDevice;
          if (isDevice) {
            (
              window as { browserDetails?: { isDevice?: boolean } }
        ...
        open: (args: PopupEventArgs) => {
          ...
          args.popup.offsetY = -(
            (tdEle.querySelector('.e-control-wrapper.e-ddl') as HTMLElement)
              .offsetHeight - 18
          );
        ...
        close: (args: PopupEventArgs): void => {
          if (
            args.event &&
            ((args.event as KeyboardEvent).keyCode === 13 ||
              ((args.event as KeyboardEvent).altKey &&
                (args.event as KeyboardEvent).keyCode === 38))
          ) {
  ...
  private listValidationHeightHandler(args: {
    threshold?: number;
    ddlCont?: HTMLElement;
  }): void {
    const sheet: SheetModel = this.parent.getActiveSheet();
    ...
    if (validation && validation.type === 'List') {
      if (validation.inCellDropDown) {
        const tdRowHeight: number =
          getRowHeight(sheet, indexes[0], true) -
          getBorderHeight(indexes[0], indexes[1], sheet);
        if (tdRowHeight <= 18) {
          const ddlCont: HTMLElement =
            args.ddlCont ||
            this.parent.element.querySelector('.e-validation-list');
          if (ddlCont) {
  ...
  private setDropDownListIndex(
    ...
    if (cell && !isNullOrUndefined(cell.value)) {
      ...
      const numObj: LocaleNumericSettings =
        isNumVal && <LocaleNumericSettings>getNumericObject(this.parent.locale);
      for (
        let idx: number = 0, len: number = dataSource.length;
        idx < len;
        idx++
      ) {
        if (
          dataSource[idx as number].text === cellVal ||
          (isNumVal &&
            this.parseValidationValue(
              dataSource[idx as number].text,
              numObj
            ).toString() === cellVal)
        ) {
          return idx;
  ...
  private getListDataSource(
    ...
    if (definedNames.length > 0 && isRange) {
      ...
      for (
        let idx: number = 0, len: number = definedNames.length;
        idx < len;
        idx++
      ) {
    ...
    if (isRange) {
      ...
      if (sheet.name !== activeSheet.name) {
        ...
        const selectedRange: number[] = getRangeIndexes(
        ...
        if (isNotLoaded) {
          ...
          getData(this.parent, `${sheet.name}!${address}`).then((): void => {
            ...
            if (activeSheet.name === this.parent.getActiveSheet().name) {
              const curRange: number[] = getRangeIndexes(
                this.parent.getActiveSheet().selectedRange
              );
              if (
                curRange[0] === selectedRange[0] &&
                curRange[1] === selectedRange[1]
              ) {
                const dataSource: { [key: string]: string }[] =
                  this.getListDataSource(validation);
                this.listObj.dataSource = dataSource;
                this.listObj.index = this.setDropDownListIndex(
                  dataSource,
                  getCell(curRange[0], curRange[1], activeSheet)
                );
      ...
      if (
        (range[0].match(/[a-z]+$/gi) && range[1].match(/[a-z]+$/gi)) ||
        ...
        const addressInfo: {
          ...
          endIdx: number;
          isCol: boolean;
        } = this.parent.getIndexes(address);
        if (addressInfo.isCol) {
          indexes = [
            0,
            addressInfo.startIdx,
      ...
      for (let rowIdx: number = indexes[0]; rowIdx <= indexes[2]; rowIdx++) {
        ...
        for (let colIdx: number = indexes[1]; colIdx <= indexes[3]; colIdx++) {
          if (!sheet.rows[rowIdx as number].cells) {
  ...
  private listValueChange(value: string): void {
    ...
    const cellObj: CellModel = Object.assign(
      {},
      getCell(cellIdx[0], cellIdx[1], sheet)
    );
    if (sheet.isProtected && isLocked(cellObj, getColumn(sheet, cellIdx[1]))) {
      ...
      isReadOnly(
        cellObj,
        getColumn(sheet, cellIdx[1]),
        getRow(sheet, cellIdx[0])
      )
    ) {
      ...
      const cancelled: boolean = updateCell(this.parent, sheet, {
  ...
  private initiateDataValidationHandler(
    ...
    const validationArgs: {
      moreValidation: boolean;
      extendValidation: boolean;
    } = this.validateRange(indexes, sheet);
    if (
      (!validationArgs.extendValidation && !validationArgs.moreValidation) ||
      okClick
    ) {
      for (let rowIdx: number = indexes[0]; rowIdx <= indexes[2]; rowIdx++) {
        if (sheet.rows[rowIdx as number]) {
            ...
            if (
              ...
              if (cell.validation) {
                ...
                ignoreBlank = !isNullOrUndefined(cell.validation.ignoreBlank)
                  ? cell.validation.ignoreBlank
                  : ignoreBlank;
                inCellDropDown = !isNullOrUndefined(
                  cell.validation.inCellDropDown
                )
                  ? cell.validation.inCellDropDown
                  : inCellDropDown;
              }
        ...
        for (let i: number = indexes[1]; i <= indexes[3]; i++) {
          ...
          if (column && column.validation) {
            ...
            ignoreBlank = !isNullOrUndefined(column.validation.ignoreBlank)
              ? column.validation.ignoreBlank
              : ignoreBlank;
            inCellDropDown = !isNullOrUndefined(
              column.validation.inCellDropDown
            )
              ? column.validation.inCellDropDown
              : inCellDropDown;
          }
      ...
      if (!this.parent.element.querySelector('.e-datavalidation-dlg')) {
        ...
        dialogInst.show({
          ...
          beforeOpen: (args: BeforeOpenEventArgs): void => {
            ...
            dialogInst.dialogInstance.content = this.dataValidationContent(
              isNew,
              type,
              operator,
              value1,
  ...
  private dataValidationContent(
    ...
    if (isNew) {
      ...
      if (type === 'Date' || type === 'Time') {
        const getFormattedDate: (val: string) => string = (
          val: string
        ): string => {
          if (isNumber(val)) {
            const args: { cell: CellModel; type: string; value: string } = {
              cell: { value: val },
    ...
    const cellRangeCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-cellrange',
    });
    const allowDataCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-allowdata',
    });
    const valuesCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-values',
    });
    const ignoreBlankCont: HTMLElement = this.parent.createElement('div', {
    ...
    const cellRangeText: HTMLElement = this.parent.createElement('span', {
      className: 'e-header',
    });
    ...
    const allowCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-allow',
    });
    const dataCont: HTMLElement = this.parent.createElement('div', {
      className: 'e-data',
    });
    allowDataCont.appendChild(allowCont);
    allowDataCont.appendChild(dataCont);
    const allowText: HTMLElement = this.parent.createElement('span', {
      className: 'e-header',
    });
    ...
    const allowSelectEle: HTMLElement = this.parent.createElement('input', {
      className: 'e-select',
    });
    ...
    if (!isNew) {
      for (let idx: number = 0; idx < this.typeData.length; idx++) {
        if (
          type ===
          this.formattedType(this.typeData[idx as number].text as string)
        ) {
          allowIdx = idx;
          break;
        }
    ...
    if (isNew || type !== 'List') {
      ...
      const dataText: HTMLElement = this.parent.createElement('span', {
        className: 'e-header',
      });
      dataText.innerText = l10n.getConstant('Data');
      const dataSelectEle: HTMLElement = this.parent.createElement('input', {
        className: 'e-select',
      });
      if (!isNew) {
        for (let idx: number = 0; idx < this.operatorData.length; idx++) {
          if (
            operator ===
            this.formattedValue(this.operatorData[idx as number].text as string)
          ) {
            dataIdx = idx;
            break;
          }
      ...
      const ignoreBlankEle: HTMLElement = this.parent.createElement('input', {
    ...
    if (
      isNew ||
      (this.typeList.value !== l10n.getConstant('List') &&
        (this.dataList.value === l10n.getConstant('Between') ||
          this.dataList.value === l10n.getConstant('NotBetween')))
    ) {
      const minimumCont: HTMLElement = this.parent.createElement('div', {
        className: 'e-minimum',
      });
      const maximumCont: HTMLElement = this.parent.createElement('div', {
        className: 'e-maximum',
      });
      valuesCont.appendChild(minimumCont);
      valuesCont.appendChild(maximumCont);
      const minimumText: HTMLElement = this.parent.createElement('span', {
        className: 'e-header',
      });
      minimumText.innerText = l10n.getConstant('Minimum');
      const maximumText: HTMLElement = this.parent.createElement('span', {
        className: 'e-header',
      });
      ...
      const minimumInp: HTMLElement = this.parent.createElement('input', {
        id: 'minvalue',
        className: 'e-input',
        attrs: { value: value1, 'aria-label': l10n.getConstant('Minimum') },
      });
      const maximumInp: HTMLElement = this.parent.createElement('input', {
        id: 'maxvalue',
        className: 'e-input',
        attrs: { value: value2, 'aria-label': l10n.getConstant('Maximum') },
      });
      ...
      const valueEle: HTMLElement = this.parent.createElement('input', {
        className: 'e-input',
        attrs: { value: value1 },
      });
  ...
  private validateRange(
    indexes: number[],
    sheet: SheetModel
  ): { moreValidation: boolean; extendValidation: boolean } {
    let moreValidation: boolean = false;
    let extendValidation: boolean = false;
    const type: string[] = [];
      ...
      if (sheet.rows[rowIndex as number]) {
        for (let colIdx: number = indexes[1]; colIdx <= indexes[3]; colIdx++) {
          if (
            ...
            sheet.rows[rowIndex as number].cells[colIdx as number]
          ) {
      ...
      for (
        ...
        if (sheet.rows[startRow as number]) {
          for (
            ...
            if (
              sheet.rows[startRow as number].cells &&
              sheet.rows[startRow as number].cells[colIdx as number]
            ) {
              const cell: CellModel =
                sheet.rows[startRow as number].cells[colIdx as number];
              cellCount++;
    ...
    const dlg: DialogModel = {
      ...
      beforeOpen: (args: BeforeOpenEventArgs): void => {
        ...
        dialogInst.dialogInstance.content =
          l10n.getConstant('ExtendValidation');
        dialogInst.dialogInstance.dataBind();
        focus(this.parent.element);
      },
      buttons: [
  ...
  private userInput(): void {
    const listObj: DropDownList = this.typeList;
    const listObj1: DropDownList = this.dataList;
    const dlgEle: HTMLElement = this.parent.element.querySelector(
      '.e-datavalidation-dlg'
    );
    ...
    if (
      listObj.value !== l10n.getConstant('List') &&
      (listObj1.value === l10n.getConstant('Between') ||
        listObj1.value === l10n.getConstant('NotBetween'))
    ) {
      const minimumCont: HTMLElement = this.parent.createElement('div', {
        className: 'e-minimum',
      });
      ...
      valueText.innerText =
        listObj.value === l10n.getConstant('List')
          ? l10n.getConstant('Sources')
          : l10n.getConstant('Value');
      const valueEle: HTMLElement =
        listObj.value === l10n.getConstant('List')
  ...
  private dlgClickHandler(dialogInst: Dialog): void {
    ...
    const dlgContEle: HTMLElement = dlgEle
      .getElementsByClassName('e-dlg-content')[0]
      .getElementsByClassName('e-validation-dlg')[0] as HTMLElement;
    const allowData: HTMLElement = dlgContEle.getElementsByClassName(
      'e-allowdata'
    )[0] as HTMLElement;
    const allowEle: HTMLInputElement = allowData
      .getElementsByClassName('e-allow')[0]
      .getElementsByTagName('input')[0];
    const dataEle: HTMLElement = allowData
      .getElementsByClassName('e-data')[0]
      .getElementsByTagName('input')[0];
    const values: HTMLElement = dlgContEle.getElementsByClassName(
      'e-values'
    )[0] as HTMLElement;
    const valueArr: string[] = [];
    valueArr[0] = values.getElementsByTagName('input')[0].value;
    valueArr[1] = values.getElementsByTagName('input')[1]
      ? values.getElementsByTagName('input')[1].value
      : '';
    const type: ValidationType = this.formattedType(allowEle.value);
    let isValid: boolean = true;
    const numObj: LocaleNumericSettings = <LocaleNumericSettings>(
      getNumericObject(this.parent.locale)
    );
    if (type === 'Decimal' && numObj.decimal !== '.') {
      const isNotCulturedNumber: (val: string) => boolean = (
        val: string
      ): boolean =>
        isNumber(val) &&
        val.includes('.') &&
      ...
      if (
        isNotCulturedNumber(valueArr[0]) ||
        isNotCulturedNumber(valueArr[1])
      ) {
        isValid = false;
    ...
    const ignoreBlank: boolean = (
      dlgContEle.querySelector('.e-ignoreblank .e-checkbox') as HTMLInputElement
    ).checked;
    const inCellDropDown: boolean = allowData
      .querySelector('.e-data')
    ...
    if (type === 'List') {
      if (valueArr[0].startsWith('=')) {
        ...
        if (address.includes(':')) {
          ...
          const isSingleCol: boolean =
            address.match(/[a-z]/gi) &&
            cellRef[0].replace(/[0-9]/g, '') ===
              cellRef[1].replace(/[0-9]/g, '');
          const isSingleRow: boolean =
            address.match(/\d/g) &&
            cellRef[0].replace(/\D/g, '') === cellRef[1].replace(/\D/g, '');
    ...
    if (isValid) {
      const sheet: SheetModel = this.parent.getActiveSheet();
      const validDlg: { isValidate: boolean; errorMsg: string } =
        this.isDialogValidator(valArr, type, operator);
      if (
        operator === 'Between' &&
        validDlg.isValidate &&
        !isNaN(parseFloat(valArr[0])) &&
        !isNaN(parseFloat(valArr[1])) &&
      ...
      if (isValid) {
        const args: CellValidationEventArgs = {
  ...
  private isDialogValidator(
    ...
    type: ValidationType,
    operator: string
  ): { isValidate: boolean; errorMsg: string } {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    let count: number = 0;
    ...
    if (!isEmpty) {
      ...
      for (let idx: number = 0; idx < values.length; idx++) {
        if (checkIsFormula(values[idx as number])) {
          value = this.parent
            .computeExpression(values[idx as number])
            .toString();
        } else if (type === 'Date' || type === 'Time') {
          value = values[idx as number] = this.getDateAsNumber(
            { range: [], cell: { value: values[idx as number] } },
            values[idx as number]
          );
        } else {
  ...
  private getListOfValues(listValue: string): string[] {
    ...
    if (
      ...
      listValue.includes(this.parent.listSeparator)
    ) {
  ...
  private isValidationHandler(args: CheckCellValidArgs): {
    isValidate: boolean;
    errorMsg: string;
  } {
    const l10n: L10n = this.parent.serviceLocator.getService(locale);
    ...
    if (validation) {
      ...
      const ignoreBlank: boolean = isNullOrUndefined(validation.ignoreBlank)
        ? true
      ...
      if (ignoreBlank && enterValue === '') {
        ...
        if (args.value) {
          if (isDateTimeType || validation.type === 'TextLength') {
            ...
            const numObj: LocaleNumericSettings =
              args.isEdit &&
              <LocaleNumericSettings>getNumericObject(this.parent.locale);
            const numVal: string | number = this.parseValidationValue(
              args.value,
              numObj
            );
        ...
        const formValidation: { isValidate: boolean; errorMsg: string } =
          this.formatValidation(args.value, type);
            ...
            if (value1.indexOf('=') !== -1) {
              ...
              for (let idx: number = 0; idx < data.length; idx++) {
                ...
                if (
                  ...
                  (isNumVal &&
                    val ===
                      this.parseValidationValue(listVal, numObj).toString())
                ) {
                  isValidate = true;
                  break;
                }
              ...
              for (let idx: number = 0; idx < listValues.length; idx++) {
                if (
                  enterValue === listValues[idx as number] ||
                  val === listValues[idx as number] ||
                  (isNumVal &&
                    val ===
                      this.parseValidationValue(
                        listValues[idx as number],
                        numObj
                      ).toString())
                ) {
                  isValidate = true;
    ...
    if (
      isValidate &&
      ((cell && cell.validation && cell.validation.isHighlighted) ||
        (column && column.validation && column.validation.isHighlighted))
    ) {
      const style: CellStyleModel = this.parent.getCellStyleValue(
  ...
  private checkDataValidation(args: CheckCellValidArgs): void {
    ...
    if (cell && cell.validation) {
      if (
        checkIsFormula(cell.validation.value1) &&
        !isCellReference(
          cell.validation.value1.substring(1, cell.validation.value1.length)
        ) &&
      ...
      if (
        !formulaArgs.skip &&
        checkIsFormula(cell.validation.value2) &&
        !isCellReference(
          cell.validation.value2.substring(1, cell.validation.value2.length)
        ) &&
        cell.validation.value1.indexOf('(') > -1
      ) {
    ...
    if (!formulaArgs.skip) {
      const isValid: { isValidate: boolean; errorMsg: string } =
        this.isValidationHandler(args);
  ...
  private formatValidation(
    value: string,
    type: ValidationType,
    isDialogValidator?: boolean
  ): { isValidate: boolean; errorMsg: string } {
    const sheetPanel: HTMLElement = this.parent.element.getElementsByClassName(
      'e-sheet-panel'
    )[0] as HTMLElement;
    let errorMsg: string;
    ...
    sheetPanel.removeChild(
      sheetPanel.getElementsByClassName('form-horizontal')[0]
    );
    return { isValidate: isValidate, errorMsg: errorMsg };
  }

  private invalidElementHandler(args: {
    isRemoveHighlightedData: boolean;
    rowIdx: number;
    colIdx: number;
    td?: HTMLElement;
  }): void {
