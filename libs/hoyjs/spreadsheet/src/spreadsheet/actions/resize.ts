...
import { closest, detach, EventHandler } from '@hoyui/base';
...
export class Resize {
  ...
  private autoFit(args: {
    isRow: boolean;
    startIndex: number;
    endIndex: number;
  }): void {
    const element: Element = args.isRow
      ? this.parent.getRowHeaderTable()
      : this.parent.getColHeaderTable().rows[0];
    for (let i: number = args.startIndex; i <= args.endIndex; i++) {
      this.trgtEle = args.isRow
        ? this.parent.getRow(i, <HTMLTableElement>element)
  ...
  private wireEvents(): void {
    ...
    const colHeader: Element =
      this.parent.element.getElementsByClassName('e-header-panel')[0];
    if (!colHeader) {
      return;
    }
  ...
  private mouseMoveHandler(e: MouseEvent): void {
    const colResizeHandler: HTMLElement =
      this.parent.element.getElementsByClassName(
        'e-colresize-handler'
      )[0] as HTMLElement;
    const rowResizeHandler: HTMLElement =
      this.parent.element.getElementsByClassName(
        'e-rowresize-handler'
      )[0] as HTMLElement;
      ...
      if (colResizeHandler) {
        if (
          isRtl
            ? e.x <
              (
                this.trgtEle.parentElement.firstChild as HTMLElement
        ...
        if (
          e.y >=
          (
            this.trgtEle.parentElement.parentElement.firstChild as HTMLElement
          ).getBoundingClientRect().top
        ) {
          rowResizeHandler.style.top =
            e.clientY -
            document
              .getElementById(this.parent.element.id + '_sheet')
              .getBoundingClientRect().top +
  ...
  private mouseDownHandler(e: MouseEvent & TouchEvent): void {
    if (
      !closest(e.target as Element, '.e-header-cell') ||
      (e.target as Element).className.includes('e-filter-icon')
    ) {
      return;
    }
    this.event = e;
    ...
    if (
      this.trgtEle.parentElement.classList.contains('e-hide-end') ||
      this.trgtEle.classList.contains('e-hide-end')
    ) {
      const offsetSize: number = this.trgtEle.offsetHeight;
      const offset: number = e.offsetY;
      if (
        (offsetSize >= 10 && offset < 5) ||
        (offsetSize - 2 < 8 && offset < Math.ceil((offset - 2) / 2))
      ) {
    ...
    const className: string = trgt.classList.contains('e-colresize')
      ? 'e-colresize-handler'
      : trgt.classList.contains('e-rowresize')
      ? 'e-rowresize-handler'
      : '';
  ...
  private mouseUpHandler(e: MouseEvent & TouchEvent): void {
    const resizeHandler: HTMLElement =
      this.parent.element.getElementsByClassName(
        'e-resize-handle'
      )[0] as HTMLElement;
    this.resizeOn(e);
    this.isMouseMoved = null;
    const HeaderTooltip: HTMLElement =
      document.querySelector('.e-header-tooltip');
    if (resizeHandler) {
  ...
  private dblClickHandler(e?: MouseEvent & TouchEvent): void {
    ...
    if (this.trgtEle.classList.contains('e-colresize')) {
      const colIndx: number =
        parseInt(this.trgtEle.getAttribute('aria-colindex'), 10) - 1;
      const prevWidth: string = `${getColumnWidth(
      ...
      const rowIndx: number =
        parseInt(this.trgtEle.parentElement.getAttribute('aria-rowindex'), 10) -
        1;
      const prevHeight: string = `${getRowHeight(
        this.parent.getActiveSheet(),
        rowIndx
      )}px`;
  ...
  private setTarget(e: MouseEvent): void {
    ...
    if (tOffsetV - 2 < 8 && eOffsetV !== Math.ceil((tOffsetV - 2) / 2)) {
      ...
      const resEle: HTMLCollectionOf<Element> =
        this.parent.element.getElementsByClassName(
          tClass
  ...
  private getColPrevSibling(trgt: HTMLElement): Element {
    ...
    return (
      ...
      (frozenCol && closest(trgt, '.e-column-header')
        ? this.parent.getSelectAllContent().querySelector('.e-header-row')
            .lastElementChild
        : null)
    );
  }
  ...
  private getRowPrevSibling(trgt: HTMLElement): Element {
    ...
    return (
      trgt.parentElement.previousElementSibling ||
      (frozenRow && closest(trgt, '.e-row-header')
        ? this.parent.getSelectAllContent().querySelector('tbody')
            .lastElementChild
        : null)
    );
  }
  ...
  private updateTarget(e: MouseEvent, trgt: HTMLElement): void {
    if (closest(trgt, '.e-header-row')) {
      const offsetX: number = this.parent.enableRtl
        ? trgt.offsetWidth - e.offsetX
        : e.offsetX;
      if (
        (trgt.offsetWidth < 10 &&
          offsetX < Math.ceil((trgt.offsetWidth - 2) / 2)) ||
        (offsetX < 5 &&
          trgt.offsetWidth >= 10 &&
          trgt.classList.contains('e-colresize'))
      ) {
        const sheet: SheetModel = this.parent.getActiveSheet();
        const prevIdx: number =
          Number(this.trgtEle.getAttribute('aria-colindex')) - 2;
        const prevSibling: Element = this.getColPrevSibling(trgt);
      ...
      if (
        (trgt.offsetHeight < 10 &&
          e.offsetY < Math.ceil((trgt.offsetHeight - 2) / 2)) ||
        (e.offsetY < 5 &&
          trgt.offsetHeight >= 10 &&
          trgt.classList.contains('e-rowresize'))
      ) {
        const sheet: SheetModel = this.parent.getActiveSheet();
        const prevIdx: number =
          Number(trgt.parentElement.getAttribute('aria-rowindex')) - 2;
        const prevSibling: Element = this.getRowPrevSibling(trgt);
        if (prevSibling || isHiddenRow(sheet, prevIdx)) {
          if (e.type === 'dblclick' && isHiddenRow(sheet, prevIdx)) {
            const selectRange: number[] = getSwapRange(
              getRangeIndexes(sheet.selectedRange)
            );
            let eventArgs: HideShowEventArgs;
            if (
              prevIdx <= selectRange[2] &&
              prevIdx > selectRange[0] &&
              isRowSelected(sheet, selectRange)
            ) {
              eventArgs = {
                startIndex: selectRange[0],
                endIndex: selectRange[2],
                hide: false,
                autoFit: true,
              };
            } else {
            ...
            if (!isHiddenRow(sheet, prevIdx)) {
              this.trgtEle = prevSibling.getElementsByClassName(
                'e-header-cell'
              )[0] as HTMLElement;
            }
          }
  ...
  private getWrapText(
    ...
    const setDisplayText: Function = (): void => {
      ...
      if (
        curWidth > textWidth ||
        (curWidth === textWidth &&
          getTextWidth(val.trim(), style, this.parent.cellStyle) >
    ...
    textArr.forEach((txt: string, index: number): void => {
      ...
      if ((prevWidth + width) / colwidth > 1) {
        ...
        width +=
          (prevWidth + width + spaceWidth) / colwidth >= 1 ? 0 : spaceWidth;
        prevWidth += width;
      }
      val += txt + ' ';
  ...
  private setAutofit(
    ...
    let colGrp: HTMLElement;
    let wrapCell: boolean;
    ...
    const rowEle: HTMLElement = this.parent.createElement('tr', {
      className: 'e-row',
    });
    const tdEle: HTMLElement = this.parent.createElement('td', {
      className: 'e-cell',
    });
    if (isCol) {
      let row: HTMLElement;
      ...
      const appendRow: Function = (content: string): void => {
        ...
        cellEle.style.fontFamily =
          (cell.style && cell.style.fontFamily) ||
          this.parent.cellStyle.fontFamily;
        cellEle.style.fontSize =
          (cell.style && cell.style.fontSize) || this.parent.cellStyle.fontSize;
        cellEle.style.fontWeight =
          (cell.style && cell.style.fontWeight) ||
          this.parent.cellStyle.fontWeight;
      ...
      for (
        let rowIdx: number = 0, len: number = sheet.rows.length;
        rowIdx < len;
        rowIdx++
      ) {
            ...
            appendRow(
              this.getWrapText(
                ...
                getExcludedColumnWidth(
                  ...
                  idx,
                  idx,
                  cell.colSpan > 1 ? idx + cell.colSpan - 1 : idx
                ),
                cell.style
              )
            );
      ...
      const colLength: number =
        sheet.rows[idx as number] && sheet.rows[idx as number].cells
          ? sheet.rows[idx as number].cells.length
          : 0;
      ...
      for (let colIdx: number = 0; colIdx < colLength; colIdx++) {
        ...
        if (cell) {
          ...
          cellEle.style.fontFamily =
            ...
            this.parent.cellStyle.fontFamily;
          cellEle.style.fontSize =
            (cell.style && cell.style.fontSize) ||
            this.parent.cellStyle.fontSize;
          rowEle.appendChild(cellEle);
          colGrp.appendChild(
            this.parent.createElement('col', {
              styles: `width:${getColumnWidth(sheet, colIdx, false, true)}px`,
            })
          );
        }
      }
    ...
    const wrapper: HTMLElement = this.parent.createElement('div', {
    ...
    let autofitValue: number =
      (isCol
        ? this.getFloatingElementWidth(fitSize + (wrapCell ? 1 : 0), idx)
        : fitSize) || oldValue;
    ...
    if (isCol) {
      ...
      if (
        (frozenCol &&
          idx >= getRangeIndexes(sheet.topLeftCell)[1] &&
          idx < frozenCol) ||
        (idx >= this.parent.viewport.leftIndex + frozenCol &&
          idx <= this.parent.viewport.rightIndex)
      ) {
        getColumn(sheet, idx).width = autofitValue > 0 ? autofitValue : 0;
        this.resizeStart(
          idx,
          this.parent.getViewportIndex(idx, true),
          autofitValue + 'px',
          isCol,
          true,
          prevData
      ...
      if (
        (frozenRow &&
          idx >= getRangeIndexes(sheet.topLeftCell)[0] &&
          idx < frozenRow) ||
        (idx >= this.parent.viewport.topIndex + frozenRow &&
          idx <= this.parent.viewport.bottomIndex)
      ) {
        setRowHeight(sheet, idx, autofitValue);
        setRow(sheet, idx, { customHeight: false });
        this.resizeStart(
          idx,
          this.parent.getViewportIndex(idx),
          autofitValue + 'px',
          isCol,
          true,
          prevData
  ...
  private createResizeHandler(trgt: HTMLElement, className: string): void {
    ...
    const sheet: HTMLElement = document.getElementById(
      this.parent.element.id + '_sheet'
    );
    if (trgt.classList.contains('e-colresize')) {
      editor.style.height =
        this.parent.getMainContent().parentElement.clientHeight +
        this.parent.getColumnHeaderContent().offsetHeight +
        'px';
      editor.style.left =
        this.event.clientX - sheet.getBoundingClientRect().left + 'px';
      editor.style.top = '0px';
    } else if (trgt.classList.contains('e-rowresize')) {
      editor.style.width =
        this.parent.getMainContent().parentElement.clientWidth + 'px';
      editor.style.left = '0px';
      editor.style.top =
        this.event.clientY - sheet.getBoundingClientRect().top + 'px';
    }
  ...
  private resizeTooltip(
    ...
    if (isResize) {
      const isRtl: boolean = this.parent.enableRtl;
      const HeaderTolltip: HTMLElement =
        document.querySelector('.e-header-tooltip');
      const colResizeHandler: HTMLElement =
        this.parent.element.getElementsByClassName(
          'e-colresize-handler'
      ...
      if (colResizeHandler) {
        const trgtWidth: number = isRtl
          ? Math.round(this.trgtEle.getBoundingClientRect().right) - e.clientX
          : e.clientX - Math.round(this.trgtEle.getBoundingClientRect().left);
        if (HeaderTolltip) {
          HeaderTolltip.firstChild.textContent =
            trgtWidth > 0
              ? 'Width:(' + trgtWidth.toString() + ' pixels)'
              : 'Width: 0.00';
        }
      } else if (rowResizeHandler) {
        const trgtHeight: number =
          e.clientY - Math.round(this.trgtEle.getBoundingClientRect().top);
        if (HeaderTolltip) {
          HeaderTolltip.firstChild.textContent =
            trgtHeight > 0
              ? 'Height:(' + trgtHeight.toString() + ' pixels)'
              : 'Height: 0.00';
        }
      }
    } else {
      ...
      if (isColResize || isRowResize) {
        const className: string = isColResize
          ? 'e-colresize-handler'
          : 'e-rowresize-handler';
        ...
        if (isColResize) {
          tooltip.content =
            'Width:(' +
            Math.round(trgt.getBoundingClientRect().width).toString() +
            ' pixels)';
        } else if (isRowResize) {
          tooltip.content =
            'Height:(' +
            Math.round(trgt.getBoundingClientRect().height).toString() +
            ' pixels)';
          tooltip.offsetX = -(
            this.parent.getMainContent().parentElement.clientWidth / 2 -
  ...
  private setColWidth(
    ...
    if (width > 0) {
      if (
        this.isMouseMoved &&
        this.trgtEle.classList.contains('e-unhide-column')
      ) {
  ...
  private showHiddenColumns(index: number, width?: number): void {
    ...
    if (width !== undefined) {
      for (let i: number = startIdx; i <= endIdx; i++) {
        ...
        if (
          i >= this.parent.viewport.leftIndex &&
          i <= this.parent.viewport.rightIndex &&
          !isHiddenCol(sheet, i)
        ) {
          (
            colgroup.children[
              this.parent.getViewportIndex(i, true)
            ] as HTMLElement
    ...
    const hideEvtArgs: HideShowEventArgs = {
      startIndex: startIdx,
      endIndex: endIdx,
      hide: false,
      isCol: true,
  ...
  private setRowHeight(
    ...
    const sheet: SheetModel = this.parent.getActiveSheet();
    const frozenCol: number = this.parent.frozenColCount(sheet);
    const eleHeight: number = parseInt(
      this.parent.getRow(rowIdx, null, frozenCol).style.height,
      10
    );
  ...
  private resizeOn(e: MouseEvent): void {
    ...
    if (this.trgtEle.classList.contains('e-rowresize')) {
      const prevIdx: number =
        Number(this.trgtEle.parentElement.getAttribute('aria-rowindex')) - 2;
      if (
        this.isMouseMoved &&
        isHiddenRow(sheet, prevIdx) &&
        this.trgtEle.classList.contains('e-skip-resize') &&
        e.clientY > this.trgtEle.getBoundingClientRect().top
      ) {
        this.trgtEle.classList.remove('e-skip-resize');
        const eventArgs: HideShowEventArgs = {
          startIndex: prevIdx,
          endIndex: prevIdx,
          hide: false,
          skipAppend: true,
        ...
        rTbody.insertBefore(
          eventArgs.hdrRow,
          rTbody.children[eventArgs.insertIdx]
        );
        ...
        if (this.trgtEle.classList.contains('e-skip-resize')) {
          this.trgtEle.classList.remove('e-skip-resize');
          if (
            (!this.isMouseMoved && isHiddenRow(sheet, prevIdx)) ||
            !this.trgtEle.parentElement.previousElementSibling
          ) {
            return;
          }
          this.trgtEle =
            this.trgtEle.parentElement.previousElementSibling.getElementsByClassName(
              'e-header-cell'
            )[0] as HTMLElement;
        }
      }
      ...
      const frozenCol: number = this.parent.frozenColCount(sheet);
      let prevData: string = this.parent.getRow(actualIdx, null, frozenCol)
        .style.height;
      let rowHeight: number =
        e.clientY - this.event.clientY + parseInt(prevData, 10);
      if (rowHeight <= 0) {
      ...
      if (this.trgtEle.parentElement.style.display === 'none') {
        ...
        if (actualIdx <= selectedRange[2] && actualIdx > selectedRange[0]) {
          ...
          if (idx !== 0 && !isHiddenRow(sheet, actualIdx - 1)) {
            this.trgtEle.parentElement.previousElementSibling.classList.remove(
              'e-hide-start'
            );
          } else {
            if (idx !== 0) {
        ...
        this.parent.getContentTable().rows[idx as number].style.display = '';
      }
      ...
      if (
        ...
        if (this.trgtEle.previousElementSibling) {
          this.trgtEle = <HTMLElement>this.trgtEle.previousElementSibling;
        }
      }
      ...
      this.setColWidth(
        ...
        (this.parent.enableRtl
          ...
          : e.clientX - this.event.clientX) + curWidth,
        curWidth
      );
    }
    if (cell && cell.format && cell.format.includes('*')) {
      this.parent.notify(getFormattedCellObject, <NumberFormatArgs>{
        value: cell.value,
        format: cell.format,
        cell: cell,
  ...
  private getFloatingElementWidth(oldWidth: number, colIdx: number): number {
    ...
    if (eventArgs.hasFilter && eventArgs.filterRange) {
      if (
        eventArgs.filterRange[1] <= colIdx &&
        eventArgs.filterRange[3] >= colIdx
      ) {
