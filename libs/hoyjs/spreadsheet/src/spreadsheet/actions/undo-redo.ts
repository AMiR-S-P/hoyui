...
export class UndoRedo {
    ...
    switch (args.action) {
      case 'format':
        address = getRangeIndexes(
          (args.eventArgs as BeforeCellFormatArgs).range
        );
        break;
      case 'clipboard':
        ...
        if (copiedInfo && copiedInfo.isCut) {
          cutCellDetails = this.getCellDetails(
            copiedInfo.range as number[],
            getSheet(
              this.parent as Workbook,
              getSheetIndexFromId(
                this.parent as Workbook,
                <number>copiedInfo.sId
              )
            ),
            'clipboard'
          );
      ...
      case 'beforeSort':
        address = getRangeIndexes(
          (args.eventArgs as BeforeSortEventArgs).range
        );
      ...
      case 'removeValidation':
        if (eventArgs.isColSelected) {
          ...
          for (
            let start: number = getColIndex(rangeArr[0]),
              end: number = getColIndex(rangeArr[1]);
            start <= end;
            start++
          ) {
            if (
              sheet.columns[start as number] &&
              sheet.columns[start as number].validation
            ) {
              this.beforeActionData.cellDetails.push({
                colIndex: start,
                validation: sheet.columns[start as number].validation,
              });
            }
          }
        } else {
  ...
  private performUndoRedo(args: {
    ...
    if (undoRedoArgs) {
      ...
      if (!args.isPublic) {
        const actionData: BeforeActionData =
          undoRedoArgs.eventArgs.beforeActionData;
        ...
        (undoRedoArgs as unknown as { preventAction: boolean }).preventAction =
      ...
      switch (undoRedoArgs.action) {
        ...
        case 'deleteNote':
        case 'removeHyperlink':
          undoRedoArgs = this.performOperation(
            undoRedoArgs,
            args.preventEvt,
            args.preventReSelect,
            args.isFromAutoFillOption
          );
          break;
        case 'sorting':
          ...
          break;
        case 'clipboard':
          undoRedoArgs = this.undoForClipboard(
            undoRedoArgs,
            args.isUndo,
            actionArgs
          );
        ...
        case 'replaceAll':
          (
            undoRedoArgs.eventArgs as unknown as { isAction: boolean }
          ).isAction = false;
          if (args.isUndo) {
            replaceArgs.value = (
              undoRedoArgs.eventArgs as unknown as { replaceValue: string }
            ).replaceValue;
            replaceArgs.replaceValue = undoRedoArgs.eventArgs.value;
            replaceArgs.skipFormatCheck = isImported(this.parent);
          }
          this.parent.notify(replaceAll, {
            ...undoRedoArgs.eventArgs,
            ...replaceArgs,
          });
          break;
        case 'insert':
        ...
        case 'removeHighlight':
          ...
          break;
        case 'merge':
          undoRedoArgs.eventArgs.merge = (
            undoRedoArgs as unknown as { isFromUpdateAction: boolean }
          ).isFromUpdateAction
            ? undoRedoArgs.eventArgs.merge
            : !undoRedoArgs.eventArgs.merge;
            ...
            updateAction(
              {
                ...
                eventArgs: <UndoRedoEventArgs>(
                  undoRedoArgs.eventArgs.cfClearActionArgs
                ),
              },
              this.parent,
              !args.isUndo
        ...
        case 'chartRefresh':
          ...
          break;
        case 'chartDesign':
          (undoRedoArgs.eventArgs as unknown as { isUndo: boolean }).isUndo =
            args.isUndo;
      ...
      if (!args.isFromUpdateAction) {
        ...
        if (!args.isPublic && !preventEvt) {
          this.parent.notify(
            completeAction,
            extend({ isUndoRedo: true, isUndo: args.isUndo }, undoRedoArgs)
          );
        }
      }
  ...
  private undoForSorting(args: CollaborativeEditArgs, isUndo: boolean): void {
    const sheetIndex: number = getSheetIndexFromAddress(
      this.parent,
      args.eventArgs.range
    );
    ...
    const updateSortIcon: Function = (idx: number, add: boolean): void => {
      if (sheetIndex === this.parent.activeSheetIndex) {
        let td: Element = this.parent.getCell(
          range[0] - 1,
          this.parent.sortCollection[idx as number].columnIndex
        );
        if (td) {
          td = select('.e-filter-icon', td);
          if (td) {
            // Determine the class to add or remove
            const sortClass: string = `e-sort${
              this.parent.sortCollection[idx as number].order === 'Ascending'
                ? 'asc'
                : 'desc'
            }-filter`;
            if (add) {
    ...
    if (isUndo) {
      this.parent.notify(updateSortedDataOnCell, {
        ...
        jsonData: (args.eventArgs.beforeActionData as BeforeActionDataInternal)
          .sortedCellDetails,
        isUndo: true,
      });
      ...
      if (this.parent.sortCollection && args.eventArgs.previousSort) {
        for (
          let i: number = this.parent.sortCollection.length - 1;
          i >= 0;
          i--
        ) {
          if (
            this.parent.sortCollection[i as number].sheetIndex === sheetIndex
          ) {
            updateSortIcon(i, false);
            this.parent.sortCollection.splice(i, 1);
            const prevSort: SortCollectionModel | SortCollectionModel[] =
              args.eventArgs.previousSort;
      ...
      if (args.eventArgs.previousSort) {
        ...
        this.parent.sortCollection.splice(idx, 0, {
          sortRange: args.eventArgs.range.split('!')[1],
          sheetIndex: sheetIndex,
          columnIndex: getColIndex(
            (args.eventArgs.sortOptions.sortDescriptors as SortDescriptor).field
          ),
          order: (args.eventArgs.sortOptions.sortDescriptors as SortDescriptor)
            .order,
        });
  ...
  private updateUndoRedoCollection(options: {
    args: CollaborativeEditArgs;
    isPublic?: boolean;
  }): void {
    ...
    if (
      (options.args.action === 'insert' || options.args.action === 'delete') &&
      options.args.eventArgs.modelType !== 'Sheet'
    ) {
    ...
    if (
      ...
      const beforeActionDetails: { beforeDetails: BeforeActionData } = {
        beforeDetails: { cellDetails: [] },
      };
    ...
    if (
      action === 'clipboard' &&
      eventArgs.copiedInfo &&
      eventArgs.copiedInfo.isExternal
    ) {
      const addressInfo: { sheetIndex: number; indices: number[] } =
        this.parent.getAddressInfo(eventArgs.pastedRange);
  ...
  private undoForClipboard(
    ...
    const sheetIndex: number = getSheetIndex(
      this.parent as Workbook,
      address[0]
    );
    ...
    const isFromUpdateAction: boolean = (
      args as unknown as { isFromUpdateAction: boolean }
    ).isFromUpdateAction;
    ...
    if (actionArgs) {
      ...
      (
        actionArgs.eventArgs as { beforeActionData: BeforeActionData }
      ).beforeActionData = actionData;
    }
    if (args.eventArgs.requestType === 'imagePaste') {
      const copiedShapeInfo: { [key: string]: Object } =
        eventArgs.copiedShapeInfo;
      if (isUndo) {
        ...
        if (copiedShapeInfo.isCut) {
          ...
          this.parent.notify(createImageElement, {
            options: {
              ...
              height: pictureElem.offsetHeight,
              width: pictureElem.offsetWidth,
              imageId: pictureElem.id,
            },
            range: copiedShapeInfo.pastedRange,
            isPublic: false,
      ...
      if (isUndo) {
        if (copiedInfo.isCut) {
          ...
          this.updateCellDetails(
            ...
            getSwapRange(copiedInfo.range as number[]),
            isRefresh,
            args
          );
          if (eventArgs.cfClearActionArgs) {
            updateAction(
              {
                action: 'clearCF',
                eventArgs: <UndoRedoEventArgs>eventArgs.cfClearActionArgs,
              },
              this.parent,
              false
            );
        ...
        if (actionData) {
          this.updateCellDetails(
            ...
            null,
            actionArgs
              ? (actionArgs as unknown as { isUndoRedo: boolean }).isUndoRedo
              : null
          );
        }
        if (eventArgs.cfActionArgs) {
          eventArgs.cfActionArgs.cfModel.forEach(
            (cf: ConditionalFormatModel): void => {
              updateAction(
                {
                  eventArgs: <UndoRedoEventArgs>{
                    range: cf.range,
        ...
        if (copiedInfo.isExternal) {
          const addressInfo: { sheetIndex: number; indices: number[] } =
            this.parent.getAddressInfo(eventArgs.pastedRange);
          this.updateCellDetails(
            copiedInfo.cellDetails as PreviousCellDetails[],
            getSheet(this.parent, addressInfo.sheetIndex),
            addressInfo.indices,
            true,
            args,
          ...
          const clipboardPromise: Promise<Object> = new Promise(
            (resolve: Function) => {
              resolve(
                (() => {
                  /** */
          ...
          const addressInfo: { sheetIndex: number; indices: number[] } =
            this.parent.getAddressInfo(eventArgs.copiedRange);
          ...
          clipboardPromise.then(() => {
            this.parent.notify(paste, {
              range: address
                ? getIndexesFromAddress(eventArgs.pastedRange)
                : address,
              sIdx: address
                ? getSheetIndex(
                    this.parent,
  ...
  private undoForResize(args: CollaborativeEditArgs): CollaborativeEditArgs {
    ...
    if (CellElem && CellElem.rowSpan) {
      ...
      (
        this.parent.element.querySelector('.e-active-cell') as HTMLElement
  ...
  private performOperation(
    ...
    if (args.action === 'autofill') {
      ...
      address =
        ...
        args.action === 'wrap' ||
        args.action === 'replace' ||
        args.action === 'cellDelete' ||
        args.action === 'hyperlink' ||
        args.action === 'addNote' ||
        args.action === 'editNote' ||
        args.action === 'deleteNote' ||
        args.action === 'removeHyperlink'
    ...
    if (eventArgs.isColSelected) {
      ...
      range = [
        0,
        getColIndex(rangeArr[0]),
        sheet.rowCount - 1,
        getColIndex(rangeArr[1]),
      ];
    } else {
    ...
    const isRefresh: boolean = this.checkRefreshNeeded(
      sheetIndex,
      isFromUpdateAction
    );
    const uniqueArgs: {
      ...
      isUnique: boolean;
      uniqueRange: string;
    } = { cellIdx: [range[0], range[1]], isUnique: false, uniqueRange: '' };
    if (!eventArgs.isColSelected) {
    ...
    if (this.isUndo) {
      if (
        uniqueArgs.isUnique &&
        eventArgs.formula &&
        eventArgs.formula.indexOf('UNIQUE') > -1
      ) {
        const rangeIdx: number[] = getRangeIndexes(uniqueArgs.uniqueRange);
        if (
          getCell(rangeIdx[0], rangeIdx[1], this.parent.getActiveSheet())
            .value !== '#SPILL!'
        ) {
      ...
      let cellValue: TextDecoration = this.parent.getCellStyleValue(
        ['textDecoration'],
        activeCellIndexes
      ).textDecoration;
      if (
        argsEventArgs &&
        argsEventArgs.style &&
        (argsEventArgs.style as CellStyleModel).textDecoration
      ) {
        const value: TextDecoration = (argsEventArgs.style as CellStyleModel)
          .textDecoration;
        ...
        if (cellValue === 'underline') {
          changedValue =
            value === 'underline' ? 'none' : 'underline line-through';
        } else if (cellValue === 'line-through') {
          changedValue =
            value === 'line-through' ? 'none' : 'underline line-through';
        ...
        for (let i: number = indexes[0]; i <= indexes[2]; i++) {
          for (let j: number = indexes[1]; j <= indexes[3]; j++) {
            changedStyle = {};
            cellValue = this.parent.getCellStyleValue(
              ['textDecoration'],
              [i, j]
            ).textDecoration;
            if (cellValue === 'none') {
              ...
              if (removeProp) {
                ...
                changedStyle.textDecoration =
                  value !== cellValue ? 'underline line-through' : value;
              }
            } else if (cellValue === 'underline line-through') {
              if (removeProp) {
                changedStyle.textDecoration =
                  value === 'underline' ? 'line-through' : 'underline';
              } else {
                continue;
              }
            }
            this.parent.notify(setCellFormat, {
              style: { textDecoration: changedStyle.textDecoration },
              range: [i, j, i, j],
              refreshRibbon: true,
              onActionUpdate: true,
            });
        ...
        if (
          !isNullOrUndefined(eventArgs.oldValue) &&
          eventArgs.oldValue !== eventArgs.value &&
          uniqueArgs.isUnique
        ) {
          const indexes: number[] = getRangeIndexes(uniqueArgs.uniqueRange);
          if (
            getCell(indexes[0], indexes[1], this.parent.getActiveSheet())
              .value !== '#SPILL!'
          ) {
            for (let j: number = indexes[0]; j <= indexes[2]; j++) {
              for (let k: number = indexes[1]; k <= indexes[3]; k++) {
                if (j === indexes[0] && k === indexes[1]) {
                  this.parent.updateCell(
                    { value: '#SPILL!' },
                    getRangeAddress([indexes[0], indexes[1]])
                  );
                  k = k + 1;
                }
                this.parent.updateCell({ value: '' }, getRangeAddress([j, k]));
        ...
        if (
          ...
          for (let i: number = indexes[0]; i <= indexes[1]; i++) {
            for (let j: number = indexes[1]; j <= indexes[3]; j++) {
              ...
              if (
                getCell(i, j, sheet) &&
                !isNullOrUndefined(getCell(i, j, sheet).value) &&
          ...
          if (!Skip) {
            ...
            this.parent.updateCell(
              cell,
              getCellAddress(indexes[0], indexes[1])
            );
            this.parent.notify(reApplyFormula, null);
          }
        }
    ...
    if (args.action === 'autofill') {
      address[1] = this.isUndo
        ? args.eventArgs.dataRange
        : args.eventArgs.selectedRange;
    }
  ...
  private getCellDetails(
    ...
    let cell: CellModel;
    const filterCheck: boolean = action === 'cellDelete';
  ...
  private updateCellDetails(
    ...
    const triggerEvt: boolean =
      args &&
      !preventEvt &&
      (args.action === 'cellSave' ||
        args.action === 'cellDelete' ||
        args.action === 'autofill' ||
    ...
    const cf: ConditionalFormat[] =
      args &&
      !args.eventArgs.cfClearActionArgs &&
    ...
    for (let i: number = 0; i < len; i++) {
      if (isColSelected) {
        setColumn(sheet, cells[i as number].colIndex, {
          validation: cells[i as number].validation,
        });
        continue;
      }
      prevCell = getCell(
        cells[i as number].rowIndex,
        cells[i as number].colIndex,
        sheet,
        false,
        true
      );
      if (
        prevCell.style &&
        args &&
        (args.action === 'format' || args.action === 'clipboard')
      ) {
        if (
          prevCell.style.borderTop &&
          (!cells[i as number].style ||
            !(cells[i as number].style as CellStyleModel).borderTop)
        ) {
        ...
        if (
          prevCell.style.borderLeft &&
          (!cells[i as number].style ||
            !(cells[i as number].style as CellStyleModel).borderLeft)
        ) {
          this.parent.setBorder(
            { borderLeft: '' },
            sheet.name +
        ...
        if (
          prevCell.style.fontSize &&
          (!cells[i as number].style ||
            !(cells[i as number].style as CellStyleModel).fontSize)
        ) {
          prevCell.style.fontSize = '11pt';
          select = true;
          this.parent.notify(applyCellFormat, <CellFormatArgs>{
            style: { fontSize: '11pt' },
            rowIdx: cells[i as number].rowIndex,
        ...
        if (
          prevCell.style.fontFamily &&
          (!cells[i as number].style ||
            !(cells[i as number].style as CellStyleModel).fontFamily)
        ) {
          select = true;
      ...
      const currentCell: PreviousCellDetails = {
        value:
          cells[i as number].formula &&
          cells[i as number].formula.toUpperCase().includes('UNIQUE')
            ? null
            : cells[i as number].value,
        formula: cells[i as number].formula,
      };
      if (cells[i as number].format) {
        currentCell.format = cells[i as number].format;
      }
      if (cells[i as number].isLocked) {
        currentCell.isLocked = cells[i as number].isLocked;
      }
      if (cells[i as number].style) {
        currentCell.style = Object.assign({}, cells[i as number].style);
      }
      if (cells[i as number].wrap) {
        currentCell.wrap = cells[i as number].wrap;
      }
      if (cells[i as number].rowSpan) {
        currentCell.rowSpan = cells[i as number].rowSpan;
      }
      if (cells[i as number].colSpan) {
        currentCell.colSpan = cells[i as number].colSpan;
      }
      if (cells[i as number].hyperlink) {
        currentCell.hyperlink = cells[i as number].hyperlink;
      }
      if (cells[i as number].image) {
        currentCell.image = cells[i as number].image;
      }
      if (cells[i as number].notes) {
        currentCell.notes = cells[i as number].notes;
      }
      if (cells[i as number].isReadOnly) {
        currentCell.isReadOnly = cells[i as number].isReadOnly;
      }
      if (cells[i as number].formattedText) {
        currentCell.formattedText = cells[i as number].formattedText;
      }
      setCell(
        cells[i as number].rowIndex,
        cells[i as number].colIndex,
      ...
      evtArgs = {
        ...
        notes: cells[i as number].notes,
        value: cells[i as number].formula
          ? cells[i as number].formula
          : cells[i as number].value,
      ...
      if (
        (args && args.action === 'wrap' && args.eventArgs.wrap) ||
        (prevCell.wrap && !cells[i as number].wrap)
      ) {
      ...
      if (args && cells[i as number].hyperlink && args.action === 'clear') {
        ...
        cellElem = this.parent.getCell(
          cells[i as number].rowIndex,
          cells[i as number].colIndex
        );
        if (
          args.eventArgs.type === 'Clear All' ||
          args.eventArgs.type === 'Clear Hyperlinks'
        ) {
          this.parent.addHyperlink(
            cells[i as number].hyperlink,
            args.eventArgs.range,
            cells[i as number].value
          );
        } else if (args.eventArgs.type === 'Clear Formats' && cellElem) {
          addClass(
            cellElem.querySelectorAll('.e-hyperlink'),
            'e-hyperlink-style'
          );
        }
      }
      if (cells[i as number].validation) {
        this.parent.notify(cellValidation, {
          rules: cells[i as number].validation,
          range: `${sheet.name}!${getCellAddress(
            cells[i as number].rowIndex,
            cells[i as number].colIndex
          )}`,
        });
      ...
      if (triggerEvt && cells[i as number].value !== prevCell.value) {
        this.parent.trigger('cellSave', {
          element: null,
          value: cells[i as number].value,
          oldValue: prevCell.value,
          formula: cells[i as number].formula,
          cancel: false,
          address: `${sheet.name}!${getCellAddress(
            cells[i as number].rowIndex,
            cells[i as number].colIndex
          )}`,
