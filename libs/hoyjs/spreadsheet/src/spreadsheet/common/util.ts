import {
  Browser,
  setStyleAttribute as setBaseStyleAttribute,
  getComponent,
  detach,
  isNullOrUndefined,
  removeClass,
 ...
 */
export function getSiblingsHeight(
  element: HTMLElement,
  classList: string[] = classes
): number {
  const previous: number = getHeightFromDirection(
    element,
    'previous',
    classList
  );
 ...
 */
function getHeightFromDirection(
  element: HTMLElement,
  direction: string,
  classList: string[]
): number {
...
export function inView(
  ...
  if (context.scrollSettings.enableVirtualization) {
    ...
    if (sheet.frozenRows || sheet.frozenColumns) {
      if (
        context.insideViewport(range[0], range[1]) ||
        context.insideViewport(range[2], range[3])
      ) {
        return true;
      }
    } else if (
    ...
    if (isModify) {
      if (
        (range[0] < topIdx && range[2] < topIdx) ||
        (range[0] > bottomIdx && range[2] > bottomIdx)
      ) {
        return false;
      } else {
      ...
      if (
        (range[1] < leftIdx && range[3] < leftIdx) ||
        (range[1] > rightIdx && range[3] > rightIdx)
      ) {
        return false;
      } else {
...
export function getCellPosition(
  ...
  frozenRow?: number,
  frozenColumn?: number,
  freezeScrollHeight?: number,
  freezeScrollWidth?: number,
  rowHdrWidth?: number,
  isOverlay?: boolean
  ...
  if (sheet.frozenRows || sheet.frozenColumns) {
    ...
    if (cls === 'e-active-cell') {
      if (range[0] < frozenRow || range[1] < frozenCol) {
        ...
        content =
          range[0] < frozenRow && range[1] < frozenCol
            ? parent.getSelectAllContent()
            : range[0] < frozenRow
            ? parent.getColumnHeaderContent()
            : parent.getRowHeaderContent();
        ...
        locateElem(
          ...
          preventAnimation,
          true,
          parent.viewport.beforeFreezeHeight,
          parent.viewport.beforeFreezeWidth,
          parent.sheetModule.colGroupWidth
        );
      } else {
        locateElem(
          parent,
      ...
      const freezeRow: number = parent.frozenRowCount(sheet);
      const freezeCol: number = parent.frozenColCount(sheet);
      if (range[0] < freezeRow || range[1] < freezeCol) {
        ele.style.display = 'none';
        contentElem =
          range[0] < freezeRow && range[1] < freezeCol
            ? parent.getSelectAllContent()
            : range[0] < freezeRow
            ? parent.getColumnHeaderContent()
            : parent.getRowHeaderContent();
      ...
      if (range[0] < freezeRow || range[1] < freezeCol) {
        if (range[3] + 1 === freezeCol && range[2] + 1 > freezeRow) {
          locateElem(
            parent,
            parent.getMainContent().querySelector('.e-filloption'),
            range,
            sheet,
            parent.enableRtl,
            freezeRow,
      ...
      if (swapRange[0] < frozenRow || swapRange[1] < frozenCol) {
        ...
        ranges.forEach((rng: number[]): void => {
          content =
            rng[2] < frozenRow && rng[3] < frozenCol
              ? parent.getSelectAllContent()
              : rng[2] < frozenRow
              ? parent.getColumnHeaderContent()
              : rng[3] < frozenCol
              ? parent.getRowHeaderContent()
              : (parent.getMainContent() as HTMLElement);
          ...
          if (cls === 'e-copy-indicator' || cls === 'e-range-indicator') {
            ...
            if (frozenRow) {
              ...
              if (
                rng[0] === frozenRow &&
                content.parentElement.classList.contains('e-main-panel')
              ) {
                ranges.forEach((subRng: number[]): void => {
                  if (subRng !== rng) {
            ...
            if (frozenCol) {
              ...
              if (
                rng[1] === frozenCol &&
                (content.classList.contains('e-sheet-content') ||
                  content.classList.contains('e-column-header'))
              ) {
                ranges.forEach((subRng: number[]): void => {
                  if (subRng !== rng) {
            ...
            if (!rangeEle) {
              ...
              if (
                isMultiRange &&
                !rangeEle.classList.contains('e-multi-range')
              ) {
        ...
        if (
          cls === 'e-range-indicator' ||
          !parent.getMainContent().querySelector('.' + cls)
        ) {
    ...
    const promise: Promise<null> = locateElem(
      parent,
      ele,
      range,
      sheet,
 ...
 */
export function removeRangeEle(
  content: Element,
  checkEle: HTMLElement,
  cls: string,
  isSelection?: boolean,
  removeCls?: boolean
): void {
  if (isSelection || content !== checkEle) {
    if (removeCls) {
      const collection: NodeListOf<Element> = content.querySelectorAll(
        '.' + cls
      );
      let i: number = 0;
...
export function locateElem(
  ...
  rowHdrWidth?: number,
  cls?: string,
  isFillOptShow?: boolean,
  freezeFillOpt?: { top?: number; left?: number }
): Promise<null> | void {
  const swapRange: number[] = getSwapRange(range);
  ...
  if (parent.scrollSettings.isFinite) {
    height =
      swapRange[0] >= sheet.rowCount
        ? 0
        : getRowsHeight(
            sheet,
            swapRange[0],
            swapRange[2] < sheet.rowCount ? swapRange[2] : sheet.rowCount - 1,
            true
          );
    width =
      swapRange[1] >= sheet.colCount
        ? 0
        : getColumnsWidth(
            sheet,
            swapRange[1],
            swapRange[3] < sheet.colCount ? swapRange[3] : sheet.colCount - 1,
            true
          );
  } else {
  ...
  const isRowSelected: boolean =
    swapRange[1] === 0 && swapRange[3] === sheet.colCount - 1;
  const isColSelected: boolean =
    swapRange[0] === 0 && swapRange[2] === sheet.rowCount - 1;
  let top: number = 0;
  let tdiff: number = -5;
  let ldiff: number = -5;
  let left: number = 0;
  ...
  if (isNullOrUndefined(cls)) {
    const attrs: { [key: string]: string } = {
      top:
        (swapRange[0] === startIndex[0]
          ? cellPosition.top
          : cellPosition.top - getDPRValue(1)) + 'px',
      height:
        height &&
        height + (swapRange[0] === startIndex[0] ? 0 : getDPRValue(1)) + 'px',
      width:
        width &&
        width +
          (swapRange[1] === startIndex[1] ? 0 : getDPRValue(1)) +
          (isActiveCell &&
          frozenColumn &&
          swapRange[1] < frozenColumn &&
          swapRange[3] >= frozenColumn
            ? 1
            : 0) +
          'px',
    };
    attrs[isRtl ? 'right' : 'left'] =
      (swapRange[1] === startIndex[1]
        ? cellPosition.left
        : cellPosition.left - 1) + 'px';
    if (ele) {
      const promise: Promise<null> = setStyleAttribute(
        [{ element: ele, attrs: attrs }],
        preventAnimation
      );
      return promise;
    }
...
export function isTouchStart(e: Event): boolean {
  return (
    e.type === 'touchstart' ||
    (e.type === 'pointerdown' && (e as PointerEvent).pointerType === 'touch')
  );
}
...
export function isTouchMove(e: Event): boolean {
  return (
    e.type === 'touchmove' ||
    (e.type === 'pointermove' && (e as PointerEvent).pointerType === 'touch')
  );
}
...
export function getDPRValue(value: number, preventDecrease?: boolean): number {
  if (window.devicePixelRatio % 1 > 0) {
    ...
    return (
      value +
      (pointValue
        ? (pointValue > 0.5 || preventDecrease
            ? 1 - pointValue
            : -1 * pointValue) / window.devicePixelRatio
        : 0)
    );
  } else {
    return value;
 ...
 */
export function setAriaOptions(
  target: HTMLElement,
  options: IAriaOptions<boolean>
): void {
  const props: string[] = Object.keys(options);
  props.forEach((name: string) => {
    if (target) {
      target.setAttribute(config[`${name}`], <string>options[`${name}`]);
    }
  });
}
...
export function setResize(
  ...
  if (isCol) {
    const header: Element =
      idx < frozenCol
        ? parent.getSelectAllContent()
        : parent.getColumnHeaderContent();
    curEle = header.getElementsByTagName('th')[index as number];
    curEleH = header.getElementsByTagName('col')[index as number];
    curEleC = (
      idx < frozenCol ? parent.getRowHeaderContent() : parent.getMainContent()
    ).getElementsByTagName('col')[index as number];
  } else {
    curEle = curEleH =
      frozenRow || frozenCol
        ? parent.getRow(idx, null, frozenCol - 1)
        : parent.getRow(idx, parent.getRowHeaderTable());
    curEleH.style.height =
      parseInt(value, 10) > 0 ? getDPRValue(parseInt(value, 10)) + 'px' : '2px';
    curEleC = parent.getRow(idx, null, frozenCol);
    curEleC.style.height =
      parseInt(value, 10) > 0 ? getDPRValue(parseInt(value, 10)) + 'px' : '0px';
    let hdrFntSize: number;
    if (sheet.showHeaders) {
      const hdrRow: HTMLCollectionOf<HTMLTableRowElement> = parent
        .getRowHeaderContent()
        .getElementsByClassName(
          'e-row'
        ) as HTMLCollectionOf<HTMLTableRowElement>;
      const hdrClone: HTMLElement[] = [];
      hdrClone[0] = hdrRow[index as number]
        .getElementsByTagName('td')[0]
        .cloneNode(true) as HTMLElement;
      hdrFntSize =
        findMaxValue(parent.getRowHeaderTable(), hdrClone, false, parent) + 1;
    }
    const contentRow: HTMLCollectionOf<HTMLTableRowElement> = parent
      .getMainContent()
      .getElementsByClassName('e-row') as HTMLCollectionOf<HTMLTableRowElement>;
    const contentClone: HTMLElement[] = [];
    for (
      let idx: number = 0;
      idx < contentRow[index as number].getElementsByTagName('td').length;
      idx++
    ) {
      contentClone[idx as number] = contentRow[index as number]
        .getElementsByTagName('td')
        [idx as number].cloneNode(true) as HTMLElement;
    }
    const cntFntSize: number =
      findMaxValue(parent.getContentTable(), contentClone, false, parent) + 1;
    const fntSize: number = hdrFntSize >= cntFntSize ? hdrFntSize : cntFntSize;
    if (
      parseInt(curEleC.style.height, 10) < fntSize ||
      (curEle &&
        curEle.classList.contains('e-reach-fntsize') &&
        parseInt(curEleC.style.height, 10) === fntSize)
    ) {
      if (sheet.showHeaders) {
        curEle.classList.add('e-reach-fntsize');
        curEleH.style.lineHeight =
          parseInt(value, 10) >= 4
            ? parseInt(value, 10) - 4 + 'px'
            : parseInt(value, 10) > 0
            ? parseInt(value, 10) - 1 + 'px'
            : '0px';
      }
      curEleC.style.lineHeight =
  ...
  if (
    parseInt(value, 10) <= 0 &&
    !(
      curEleC.classList.contains('e-zero') ||
      curEleC.classList.contains('e-zero-start')
    )
  ) {
    if (preEleC && nxtEleC) {
      if (isCol) {
      ...
      if (
        nxtEle &&
        !nxtEle.classList.contains('e-zero') &&
        !nxtEle.classList.contains('e-zero-last')
      ) {
        if (sheet.showHeaders) {
      ...
      if (sheet.showHeaders && nxtEle.classList.contains('e-zero')) {
        if (curEle.classList.contains('e-zero-start')) {
          while (nxtEle) {
            if (
              (nxtEle.classList.contains('e-zero') &&
                parseInt(nxtEleH.style.height, 10) !== 0 &&
                !isCol) ||
              (parseInt(nxtEleH.style.width, 10) !== 0 && isCol)
            ) {
              if (isCol) {
                curEleH.style.width =
                  parseInt(curEleH.style.width, 10) - 1 + 'px';
                nxtEleH.style.width =
                  parseInt(nxtEleH.style.width, 10) - 1 + 'px';
              } else {
                curEleH.style.height =
                  parseInt(curEleH.style.height, 10) - 1 + 'px';
                nxtEleH.style.height =
                  parseInt(nxtEleH.style.height, 10) - 1 + 'px';
              }
              ...
              if (isCol) {
                nxtIndex =
                  parseInt(nxtEle.getAttribute('aria-colindex'), 10) - 1;
                nxtEle = parent.getColHeaderTable().getElementsByTagName('th')[
                  nxtIndex + 1
                ];
                nxtEleH = parent
                  .getColHeaderTable()
                  .getElementsByTagName('col')[nxtIndex + 1];
              } else {
                nxtIndex =
                  parseInt(nxtEle.getAttribute('aria-rowindex'), 10) - 1;
                nxtEle = parent.getRowHeaderTable().getElementsByTagName('tr')[
                  nxtIndex + 1
                ];
                nxtEleH = parent.getRowHeaderTable().getElementsByTagName('tr')[
                  nxtIndex + 1
                ];
              }
            }
      ...
      if (sheet.showHeaders && nxtEle.classList.contains('e-zero')) {
        while (nxtEle) {
          if (
            (nxtEle.classList.contains('e-zero') &&
              parseInt(nxtEleH.style.width, 10) !== 0 &&
              isCol) ||
            (parseInt(nxtEleH.style.height, 10) !== 0 && !isCol)
          ) {
            if (isCol) {
              nxtEleH.style.width =
                parseInt(nxtEleH.style.width, 10) - 1 + 'px';
            ...
            if (isCol) {
              ...
              nxtEleH = parent.getColHeaderTable().getElementsByTagName('col')[
                nxtIndex + 1
              ];
              nxtEle = parent.getColHeaderTable().getElementsByTagName('th')[
                nxtIndex + 1
    ...
    if (sheet.showHeaders && preEle && nxtEle) {
      if (preEle.classList.contains('e-zero')) {
        if (curEle.classList.contains('e-zero')) {
          if (isCol) {
            ...
            preEleH.style.height =
              parseInt(preEleH.style.height, 10) + 2 + 'px';
            curEleH.style.height =
              parseInt(curEleH.style.height, 10) - 1 + 'px';
          }
        } else {
        ...
        if (curEle.classList.contains('e-zero')) {
          ...
          if (curEle.classList.contains('e-zero-start')) {
            if (isCol) {
              preEleH.style.width =
                parseInt(preEleH.style.width, 10) + 1 + 'px';
              curEleH.style.width =
                parseInt(curEleH.style.width, 10) - 1 + 'px';
            } else {
              preEleH.style.height =
                parseInt(preEleH.style.height, 10) + 1 + 'px';
              curEleH.style.height =
                parseInt(curEleH.style.height, 10) - 1 + 'px';
            }
          }
      ...
      if (nxtEle.classList.contains('e-zero')) {
        ...
        if (
          curEle.classList.contains('e-zero') ||
          curEle.classList.contains('e-zero-start')
        ) {
          setWidthAndHeight(nxtEleH, 1, isCol);
        }
      }
      ...
      if (
        preEle.classList.contains('e-zero') ||
        preEle.classList.contains('e-zero-start')
      ) {
...
export function findMaxValue(
  ...
  const myTableDiv: HTMLElement = parent.createElement('div', {
...
export function updateAction(
  ...
  isRedo?: boolean,
  undoCollections?: CollaborativeEditArgs[],
  ...
  let isFromUpdateAction: boolean =
    (options as unknown as { isFromUpdateAction: boolean })
      .isFromUpdateAction || isUndefined(isRedo);
  ...
  if (isFromUpdateAction && !isRecursive) {
    const address: string =
      eventArgs.address ||
      eventArgs.range ||
      eventArgs.pastedRange ||
      (eventArgs.addressCollection && eventArgs.addressCollection[0]) ||
      eventArgs.dataRange;
    const sheetIndex: number = isUndefined(eventArgs.sheetIndex)
      ? isUndefined(eventArgs.sheetIdx)
        ? isUndefined(eventArgs.activeSheetIndex)
          ? address
            ? getSheetIndexFromAddress(spreadsheet, address)
            : spreadsheet.activeSheetIndex
          : eventArgs.activeSheetIndex
        : eventArgs.sheetIdx
      : eventArgs.sheetIndex;
    if (sheetIndex !== spreadsheet.activeSheetIndex) {
      const args: {
        sheet: SheetModel;
        indexes: number[];
        promise?: Promise<Cell>;
        resolveAfterFullDataLoaded?: boolean;
      } = {
        sheet: getSheet(spreadsheet, sheetIndex),
  ...
  let cellSaveArgs: CellSaveEventArgs;
  let addrInfo: { sheetIndex: number; indices: number[] };
  let clearArgs: {
    options: object;
    isFromUpdateAction: boolean;
    cfClearActionArgs?: {
      previousConditionalFormats: object;
  ...
  switch (options.action) {
    case 'sorting':
      ...
      (sortArgs.promise as Promise<SortEventArgs>).then(
        (args: SortEventArgs) => {
          spreadsheet.serviceLocator
            .getService<ICellRenderer>('cell')
            .refreshRange(getIndexesFromAddress(args.range));
        }
      );
      break;
    case 'cellSave':
    ...
    case 'deleteNote':
      ...
      if (isRedo) {
        updateCell(spreadsheet as Workbook, spreadsheet.getActiveSheet(), {
        ...
        spreadsheet.serviceLocator
          .getService<ICellRenderer>('cell')
          .refreshRange(
            getIndexesFromAddress(eventArgs.address),
            false,
            false,
            true,
            false,
            isImported(spreadsheet)
          );
      }
      break;
    case 'cellDelete':
    ...
    case 'format':
      if (eventArgs.requestType === 'CellFormat') {
        if (
          eventArgs.style &&
          eventArgs.style.border &&
          !isNullOrUndefined(eventArgs.borderType)
        ) {
        ...
        getUpdateUsingRaf((): void =>
          spreadsheet.selectRange(spreadsheet.getActiveSheet().selectedRange)
        );
      } else {
      ...
      break;
    case 'clipboard':
      clipboardPromise = new Promise((resolve: Function) => {
        resolve(
          (() => {
            /** */
          })()
      ...
      clipboardPromise.then(() =>
        spreadsheet.notify(paste, {
          range: getIndexesFromAddress(eventArgs.pastedRange),
          sIdx: getSheetIndex(
            spreadsheet,
            getSheetNameFromAddress(eventArgs.pastedRange)
          ),
          type: eventArgs.type,
          isAction: false,
          isInternal: true,
    ...
    case 'gridLines':
      spreadsheet.setSheetPropertyOnMute(
        spreadsheet.sheets[eventArgs.sheetIdx],
        'showGridLines',
        eventArgs.isShow
      );
      (
        spreadsheet.serviceLocator.getService('sheet') as IRenderer
      ).toggleGridlines();
    ...
    case 'resizeToFit':
      if (isFromUpdateAction) {
        ...
        if (
          (eventArgs.isCol && column && column.hidden) ||
          (row && row.hidden)
        ) {
    ...
    case 'renameSheet':
      spreadsheet.setSheetPropertyOnMute(
        spreadsheet.sheets[sheetIndex as number],
        'name',
        eventArgs.value
      );
    ...
    case 'hideShow':
      if (eventArgs.isCol) {
        spreadsheet.notify(hideShow, <HideShowEventArgs>{
      ...
      if (actionEventArgs && !isFromUpdateAction) {
        promise.then((): void => {
          spreadsheet.notify(
            completeAction,
            extend(
              { isUndo: !isRedo, isUndoRedo: !isFromUpdateAction },
              actionEventArgs
            )
          );
        });
    ...
    case 'insert':
      ...
      if (isRedo === false) {
        spreadsheet.notify(deleteModel, <InsertDeleteModelArgs>{
          model: sheet,
        ...
        spreadsheet.notify(insertModel, <InsertDeleteModelArgs>{
          ...
          modelType: options.eventArgs.modelType,
          checkCount:
            isRedo === undefined ? options.eventArgs.sheetCount : null,
    ...
    case 'delete':
      ...
      if (isRedo === false) {
        spreadsheet.notify(insertModel, <InsertDeleteModelArgs>{
          ...
          isUndoRedo: true,
          insertType:
            options.eventArgs.modelType === 'Row' ? 'above' : 'before',
    ...
    case 'addHighlight':
      spreadsheet.notify(invalidData, {
        isRemoveHighlight:
          options.action === 'removeHighlight'
            ? isRedo !== false
            : isRedo === false,
        range: eventArgs.range,
        isPublic: true,
    ...
    case 'merge':
      ...
      model = [];
      for (
        let rIdx: number = 0, rCnt: number = eventArgs.model.length;
        rIdx < rCnt;
        rIdx++
      ) {
        model.push({ cells: [] });
        for (
          let cIdx: number = 0,
            cCnt: number = eventArgs.model[rIdx as number].cells.length;
          cIdx < cCnt;
          cIdx++
        ) {
          ...
          Object.assign(
            model[rIdx as number].cells[cIdx as number],
            eventArgs.model[rIdx as number].cells[cIdx as number]
          );
        }
      }
    ...
    case 'clear':
      ...
      if (!isFromUpdateAction && clearArgs.cfClearActionArgs) {
        eventArgs.cfClearActionArgs.previousConditionalFormats =
          clearArgs.cfClearActionArgs.previousConditionalFormats;
        eventArgs.cfClearActionArgs.conditionalFormats =
          clearArgs.cfClearActionArgs.conditionalFormats;
      }
      break;
    case 'conditionalFormat':
      if (isRedo === false) {
        spreadsheet.notify(clearCFRule, <CFArgs>{
          range: eventArgs.range,
          cfModel: {
            type: eventArgs.type,
        ...
        spreadsheet.notify(setCFRule, <CFArgs>{
          cfModel: {
            type: eventArgs.type,
            cFColor: eventArgs.cFColor,
            value: eventArgs.value,
    ...
    case 'imageRefresh':
      ...
      if (element) {
        element.style.height =
          isRedo === false
            ? options.eventArgs.prevHeight + 'px'
            : options.eventArgs.currentHeight + 'px';
        element.style.width =
          isRedo === false
            ? options.eventArgs.prevWidth + 'px'
            : options.eventArgs.currentWidth + 'px';
        element.style.top =
          isRedo === false
            ? options.eventArgs.prevTop + 'px'
            : options.eventArgs.currentTop + 'px';
        element.style.left =
          isRedo === false
            ? options.eventArgs.prevLeft + 'px'
            : options.eventArgs.currentLeft + 'px';
      }
      break;
    case 'insertChart':
      if (isRedo === false) {
    ...
    case 'autofill':
      if (isFromUpdateAction && eventArgs.undoArgs) {
        eventArgs.undoArgs.isFromUpdateAction =
          eventArgs.undoArgs.isUndo =
          eventArgs.undoArgs.preventEvt =
            true;
    ...
    case 'protectSheet':
      if (eventArgs.isProtected) {
        ...
        spreadsheet.setSheetPropertyOnMute(
          getSheet(spreadsheet, eventArgs.sheetIndex),
          'password',
          ''
        );
...
export function hasTemplate(
  ...
  for (let i: number = 0, len: number = ranges.length; i < len; i++) {
    if (ranges[i as number].template) {
      range = getRangeIndexes(
        ranges[i as number].address.length
          ? ranges[i as number].address
          : ranges[i as number].startCell
      );
      if (
        range[0] <= rowIdx &&
        range[1] <= colIdx &&
...
export function setRowEleHeight(
  ...
  row?: HTMLElement,
  hRow?: HTMLElement,
  notifyRowHgtChange: boolean = true
): void {
  ...
  row =
    row ||
    (sheet.frozenRows
      ? parent.getRow(rowIdx, null, frozenCol)
      : parent.getRow(rowIdx));
  if (row) {
...
export function getTextHeight(
  ...
  const fontSize: string =
    (style && style.fontSize) || context.cellStyle.fontSize;
  const fontSizePx: number =
    fontSize.indexOf('pt') > -1
      ? parseInt(fontSize, 10) / 0.75
      : parseInt(fontSize, 10);
  const hgt: number =
    fontSizePx *
    (lineHeight ||
      getLineHeight(style && style.fontFamily ? style : context.cellStyle)) *
    lines;
...
export function getTextWidth(
  ...
  return preventDpr
    ...
    : getDPRValue(context.measureText(text).width, true);
}
      ...
      if (splitTextArr.length > 1) {
        splitTextArr.forEach((splitText: string) => {
          ...
          if (lines >= 1) {
            calculateCount(
              splitText,
              splitTextArr[splitTextArr.length - 1] !== splitText
            );
          } else {
            if (
              splitTextArr[splitTextArr.length - 1] === splitText &&
              textArr[textArr.length - 1] !== txt
            ) {
...
function getBorderWidth(
  ...
  const rightSideCell: CellModel = getCell(
    rowIdx,
    colIdx + 1,
    sheet,
    null,
  ...
  if (cell.style) {
    if (cell.style.border) {
      width =
        (colIdx === 0 ? 2 : 1) * parseFloat(cell.style.border.split('px')[0]);
    } else {
  ...
  if (
    !(cell.style && (cell.style.border || cell.style.borderRight)) &&
    rightSideCell.style &&
    rightSideCell.style.borderLeft
  ) {
...
export function getBorderHeight(
  ...
  if (cell.style) {
    if (cell.style.border) {
      height =
        (rowIdx === 0 ? 2 : 1) * parseFloat(cell.style.border.split('px')[0]);
    } else {
  ...
  const bottomSideCell: CellModel = getCell(
    rowIdx + 1,
    colIdx,
    sheet,
    null,
  ...
  if (
    !(cell.style && (cell.style.border || cell.style.borderBottom)) &&
    bottomSideCell.style &&
    bottomSideCell.style.borderTop
  ) {
 ...
 * @hidden
 */
export function getExcludedColumnWidth(
  sheet: SheetModel,
  rowIdx: number,
  startColIdx: number,
  endColIdx: number = startColIdx
): number {
  return (
    getColumnsWidth(sheet, startColIdx, endColIdx, true) -
    getDPRValue(4 + (getBorderWidth(rowIdx, startColIdx, sheet) || 1))
  ); // 4 -> For cell padding
}
...
export function getTextHeightWithBorder(
  ...
  return (
    getTextHeight(context, style, lines, lineHeight) +
    getBorderHeight(rowIdx, colIdx, sheet)
  );
}
 ...
 * @hidden
 */
export function isLockedCells(
  parent: Spreadsheet,
  rangeIndexes?: number[]
): boolean {
  const sheet: SheetModel = parent.getActiveSheet();
  let hasLockCell: boolean;
  const address: number[] = !isNullOrUndefined(rangeIndexes)
    ? rangeIndexes
...
export function clearRange(
  ...
  let isCellUpdated: boolean = false;
  for (
    let sRIdx: number = range[0], eRIdx: number = range[2];
    sRIdx <= eRIdx;
    sRIdx++
  ) {
    ...
    for (
      let sCIdx: number = range[1], eCIdx: number = range[3];
      sCIdx <= eCIdx;
      sCIdx++
    ) {
      const args: {
        ...
        isUnique: boolean;
        uniqueRange: string;
      } = { cellIdx: [sRIdx, sCIdx], isUnique: false, uniqueRange: '' };
      ...
      if (!args.isUnique || skip) {
        ...
        if (
          ...
          if (
            !Object.keys(newCell).length ||
            updateCell(context, sheet, prop)
          ) {
            continue;
          }
          if (cf && !cfRefreshAll) {
  ...
  if ((cfRule.length || cfRefreshAll) && uiRefresh) {
    context.notify(applyCF, <ApplyCFArgs>{
  ...
  const viewPortHeight: number =
    (sheet.frozenRows
      ? parent.viewport.height -
        parent.sheetModule.getColHeaderHeight(sheet, true)
...
export function getRightIdx(parent: Spreadsheet, left: number): number {
  let width: number = 0;
  const sheet: SheetModel = parent.getActiveSheet();
  const contWidth: number =
    (parent.getMainContent() as HTMLElement).parentElement.offsetWidth -
    parent.sheetModule.getRowHeaderWidth(sheet) -
    parent.sheetModule.getScrollSize();
 ...
 * @hidden
 */
export function setColMinWidth(
  spreadsheet: Spreadsheet,
  minWidth: number
): void {
...
export function addDPRValue(size: number): number {
  if (window.devicePixelRatio % 1 > 0) {
    ...
    return (
      size +
      (pointValue
        ? (pointValue > 0.5 ? 1 - pointValue : -1 * pointValue) /
 ...
 * @hidden
 */
export function getSheetProperties(
  context: Spreadsheet,
  keys?: string[]
): string {
  ...
  if (keys) {
    ...
    let propList: {
      ...
      propNames: string[];
    } = Object.getPrototypeOf(new Cell(<any>context, 'cells', {}, true))
      .constructor.prototype.propList;
    const cellProps: string[] = propList.colPropNames
      .concat(propList.complexPropNames)
      .concat(propList.propNames);
    cellProps.push('formattedText');
    propList = Object.getPrototypeOf(new Row(<any>context, 'rows', {}, true))
      .constructor.prototype.propList;
    const rowProps: string[] = propList.colPropNames
      .concat(propList.complexPropNames)
      .concat(propList.propNames);
    propList = Object.getPrototypeOf(
      new Column(<any>context, 'columns', {}, true)
    ).constructor.prototype.propList;
    const colProps: string[] = propList.colPropNames
      .concat(propList.complexPropNames)
      .concat(propList.propNames);
    propList = Object.getPrototypeOf(
      new Sheet(<any>context, 'sheets', {}, true)
    ).constructor.prototype.propList;
    /* eslint-enable */
    const sheetProps: string[] = propList.colPropNames
      .concat(propList.complexPropNames)
      .concat(propList.propNames);
  ...
  const eventArgs: { skipProps: string[]; model?: string } = {
    skipProps: skipProps,
  };
 ...
 * @hidden
 */
export function getChartsIndexes(
  context?: Spreadsheet
): { chart: ChartModel; chartRowIdx: number; chartColIdx: number }[] {
  ...
  for (
    let i: number = 0, len: number = context.chartColl.length;
    i < len;
    i++
  ) {
    chart = context.chartColl[i as number];
    if (sheetName === getSheetNameFromAddress(chart.range)) {
      const prevTop: { clientY: number; isImage?: boolean } = {
        clientY: chart.top,
        isImage: true,
      };
      const prevLeft: { clientX: number; isImage?: boolean } = {
        clientX: chart.left,
        isImage: true,
      };
 ...
 * @hidden
 */
export function isReadOnlyCells(
  parent: Spreadsheet,
  rangeIndexes?: number[]
): boolean {
  const sheet: SheetModel = parent.getActiveSheet();
  let hasReadOnlyCell: boolean;
  const address: number[] = !isNullOrUndefined(rangeIndexes)
    ? rangeIndexes
    : getSwapRange(getRangeIndexes(sheet.selectedRange));
  for (let row: number = address[0]; row <= address[2]; row++) {
 ...
 * @hidden
 */
export function setStandardHeight(
  context: Spreadsheet,
  sheetIndex: number,
