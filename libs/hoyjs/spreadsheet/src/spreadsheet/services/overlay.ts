...
export class Overlay {
  ...
  public insertOverlayElement(
    ...
    if (this.parent.allowEditing) {
      const actOverlayElem: HTMLElement =
    ...
    let parent: Element;
    if (indexes[0] < frozenRow || indexes[1] < frozenCol) {
      parent = this.parent.element.querySelector(
        '#' + this.parent.element.id + '_sheet'
      );
      if (frozenRow) {
        if (indexes[0] >= frozenRow) {
          pos.top +=
            (sheet.showHeaders ? 31 : 0) +
            getRowsHeight(
              sheet,
    ...
    this.originalWidth = parseFloat(
      getComputedStyle(div, null).getPropertyValue('width').replace('px', '')
    );
    this.originalHeight = parseFloat(
      getComputedStyle(div, null).getPropertyValue('height').replace('px', '')
    );
  ...
  public adjustFreezePaneSize(
    ...
    if (isNullOrUndefined(model.top)) {
      ...
      model.top =
        this.parent.viewport.beforeFreezeHeight +
        (frozenRow && startTop === indexes[0]
          ? 0
          : getRowsHeight(sheet, frozenRow ? startTop : 0, indexes[0] - 1));
    }
    if (isNullOrUndefined(model.left)) {
      const startLeft: number = getCellIndexes(sheet.topLeftCell)[1];
      model.left =
        this.parent.viewport.beforeFreezeWidth +
        (frozenCol && startLeft === indexes[1]
          ? 0
          : getColumnsWidth(sheet, frozenCol ? startLeft : 0, indexes[1] - 1));
    }
  }
  ...
  private overlayMouseMoveHandler(e: MouseEvent & TouchEvent): void {
    ...
    const overlayElem: HTMLElement = document.getElementsByClassName(
      'e-ss-overlay-active'
    )[0] as HTMLElement;
    const sheet: SheetModel = this.parent.getActiveSheet();
    const checkOffset: number[] =
      sheet.frozenRows || sheet.frozenColumns
        ? [29, this.parent.sheetModule.getRowHeaderWidth(sheet, true)]
        : [-1, -1];
    let height1: number;
    let top: number;
    let width1: number;
    ...
    if (this.isOverlayClicked && this.isResizerClicked) {
      ...
      switch (this.resizer) {
        case 'e-ss-overlay-t':
          height1 = Math.max(
            this.originalMouseY - getClientY(e) + this.originalHeight,
            20
          );
        ...
        case 'e-ss-overlay-l':
          width2 = Math.max(
            this.originalMouseX - getClientX(e) + this.originalWidth,
            20
          );
          left =
            getClientX(e) - (this.originalMouseX - this.originalResizeLeft);
      ...
      if (
        (closest(target, '.e-sheet') &&
          !target.classList.contains('e-sheet-content')) ||
        target.classList.contains('e-cell')
      ) {
        if (!overlayElem) {
          return;
        }
        const aX: number = getClientX(e) - this.diffX;
        const aY: number = getClientY(e) - this.diffY;
        if (aX > checkOffset[1]) {
  ...
  private overlayMouseUpHandler(
    ...
    const eventArgs: BeforeImageRefreshData = {
      ...
      prevLeft:
        sheet.frozenRows || sheet.frozenColumns
          ? this.prevX
          : this.originalReorderLeft,
      currentTop:
        this.resizedReorderTop >= 0
          ? parseInt(this.resizedReorderTop.toString(), 10)
          : this.originalReorderTop,
      currentLeft:
        this.resizedReorderLeft >= 0
          ? parseInt(this.resizedReorderLeft.toString(), 10)
          : this.originalReorderLeft,
      id: elem.id,
    ...
    if (this.isOverlayClicked || isMouseUp) {
      ...
      if (
        ...
        !closest(overlayEle, '.e-sheet-content')
      ) {
        const frozenCol: number = this.parent.frozenColCount(sheet);
        const frozenRow: number = this.parent.frozenRowCount(sheet);
        if (
          eventArgs.currentRowIdx >= frozenRow &&
          eventArgs.currentColIdx >= frozenCol
        ) {
          let top: number = parseInt(overlayEle.style.top, 10);
          let left: number = parseInt(overlayEle.style.left, 10);
          const mainPanel: HTMLElement = this.parent.sheetModule.contentPanel;
      ...
      if (
        this.originalReorderTop !== this.resizedReorderTop ||
        this.originalReorderLeft !== this.resizedReorderLeft
      ) {
        ...
        this.currenHeight !== this.originalHeight ||
        this.originalWidth !== this.currentWidth
      ) {
        eventArgs.id =
          elem.id.indexOf('overlay') > 0
            ? elem.id
            : elem.parentElement &&
              elem.parentElement.classList.contains('e-ss-overlay')
  ...
  private refreshOverlayElem(args: { selector?: string }): void {
    if (args) {
      const selector: string =
        '.e-ss-overlay-active' + ((args && args.selector) || '');
      const overlayElem: HTMLElement =
        this.parent.element.querySelector(selector);
      if (overlayElem) {
      ...
      const overlayElems: HTMLElement[] = Array.from(
        this.parent.element.getElementsByClassName(
          selector
  ...
  private overlayClickHandler(e: TouchEvent & MouseEvent): void {
    ...
    if (
      (sheet.frozenColumns || sheet.frozenRows) &&
      closest(overlayElem, '.e-sheet-content')
    ) {
      let top: number = parseInt(overlayElem.style.top, 10);
      let left: number = parseInt(overlayElem.style.left, 10);
      const mainPanel: HTMLElement = this.parent.sheetModule.contentPanel;
      top -= mainPanel.scrollTop;
      top += this.parent
        .getColumnHeaderContent()
      ...
      overlayElem.style.left = left + 'px';
      this.parent.element
        .querySelector('#' + this.parent.element.id + '_sheet')
        .appendChild(overlayElem);
    }
    this.originalWidth = this.currentWidth = parseFloat(
      overlayElem.style.width
    );
    this.originalHeight = this.currenHeight = parseFloat(
      overlayElem.style.height
    );
    ...
    const actOverlayElem: HTMLElement = document.getElementsByClassName(
      'e-ss-overlay-active'
    )[0] as HTMLElement;
    if (actOverlayElem) {
      removeClass([actOverlayElem], 'e-ss-overlay-active');
    ...
    this.parent.element
      .querySelector('#' + overlayElem.id)
      .classList.add('e-ss-overlay-active');
  ...
  private renderResizeHandler(overlay: HTMLElement): void {
    ...
    while (idx < handles.length) {
      handleElem = this.parent.createElement('div', {
        className:
          handles[idx as number] +
          ' ' +
          'e-ss-resizer' +
          (Browser.isDevice ? ' e-ss-resizer-touch' : ''),
  ...
  private removeEventListener(): void {
    const overlays: NodeList =
      this.parent.element.querySelectorAll('.e-ss-overlay');
    if (overlays.length) {
      // Added this condition temporarly to handle chart destroy from spec file.
      for (let idx: number = 0; idx < overlays.length; idx++) {
        EventHandler.remove(
          <Element>overlays[idx as number],
          getStartEvent(),
          this.overlayClickHandler
        );
        EventHandler.remove(
    ...
    const ele: Element = this.parent.element.querySelector(
      '#' + this.parent.element.id + '_sheet'
    );
    if (ele) {
