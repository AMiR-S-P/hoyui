...
import { Dialog as DialogComponent, DialogModel } from '@hoyui/popups';
import { extend, remove, L10n, isNullOrUndefined } from '@hoyui/base';
...
export class Dialog {
  ...
  public show(dialogModel: DialogModel, cancelBtn?: boolean): void {
    ...
    if (cancelBtn) {
      ...
      ).getConstant(model.buttons.length ? 'Cancel' : 'Ok');
      model.buttons.push({
        buttonModel: {
          content: btnContent,
