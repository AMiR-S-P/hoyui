...
import { extend, remove } from '@hoyui/base';
import {
  CellModel,
  SheetModel,
  getSheetName,
...
export class Render {
  ...
  public render(): void {
    this.parent.setProperties(
      {
        activeSheetIndex: this.parent.skipHiddenSheets(
    ...
    if (
      ...
      (this.parent.password.length > 0 || this.parent.isProtected)
    ) {
  ...
  private checkTopLeftCell(
    ...
    let isFreezeScrolled: boolean;
    if (
      sheet.topLeftCell !== sheet.paneTopLeftCell &&
      ...
      isTopLeftCell =
        sheet.frozenRows && sheet.frozenColumns
          ? indexes[0] + sheet.frozenRows === paneIndexes[0] &&
            indexes[1] + sheet.frozenColumns === paneIndexes[1]
          : sheet.frozenRows
          ? indexes[0] + sheet.frozenRows === paneIndexes[0] && indexes[1] === 0
          : indexes[1] + sheet.frozenColumns === paneIndexes[1] &&
            indexes[0] === 0;
      if (indexes[0] && paneIndexes[0] > indexes[0]) {
        this.parent.viewport.beforeFreezeHeight = getRowsHeight(
          sheet,
          0,
          indexes[0] - 1,
          true
      ...
      if (indexes[1] && paneIndexes[1] > indexes[1]) {
        this.parent.viewport.beforeFreezeWidth = getColumnsWidth(
          sheet,
          0,
          indexes[1] - 1,
          true
    ...
    if (!this.parent.scrollSettings.enableVirtualization || isTopLeftCell) {
      this.refreshUI(
        { rowIndex: indexes[0], colIndex: indexes[1], refresh: 'All' },
        null,
        initLoad,
        isRefreshing,
        preventModelCheck,
      ...
      const pIndexes: number[] =
        sheet.paneTopLeftCell === sheet.topLeftCell
          ? indexes
          : getCellIndexes(sheet.paneTopLeftCell);
      const eventArgs: ScrollEventArgs = { preventScroll: true };
      eventArgs.scrollTop =
        scrollTop ||
        (pIndexes[0] > frozenRow
          ? getRowsHeight(sheet, frozenRow, pIndexes[0] - 1, true)
          : 0);
      eventArgs.scrollLeft =
        scrollLeft ||
        (pIndexes[1] > frozenCol
          ? getColumnsWidth(sheet, frozenCol, pIndexes[1] - 1, true)
          : 0);
      this.parent.notify(onContentScroll, eventArgs);
      let threshold: number = this.parent.getThreshold('row');
      const rowIndex: number = sheet.frozenRows
        ? indexes[0]
        : indexes[0] > threshold
        ? skipHiddenIdx(sheet, indexes[0] - threshold, true)
        : 0;
      const frozenIndexes: number[] = [];
      if (sheet.frozenRows) {
        frozenIndexes.push(
          pIndexes[0] - threshold > frozenRow
            ? pIndexes[0] - threshold
            : frozenRow
        );
      }
      threshold = this.parent.getThreshold('col');
      const colIndex: number = sheet.frozenColumns
        ? indexes[1]
        : indexes[1] > threshold
        ? skipHiddenIdx(sheet, indexes[1] - threshold, true, 'columns')
        : 0;
      if (sheet.frozenColumns) {
        ...
        frozenIndexes.push(
          pIndexes[1] - threshold > frozenCol
            ? pIndexes[1] - threshold
            : frozenCol
        );
      ...
      if (isFreezeScrolled) {
        if (frozenRow && frozenIndexes[0] >= frozenRow) {
          this.parent.viewport.topIndex =
            skipHiddenIdx(sheet, frozenIndexes[0], true) - frozenRow;
        }
        if (frozenCol && frozenIndexes[1] >= frozenCol) {
          this.parent.viewport.leftIndex =
            skipHiddenIdx(sheet, frozenIndexes[1], true, 'columns') - frozenCol;
        }
      }
    }
  }
  ...
  private renderSheet(
    panel: HTMLElement = document.getElementById(
  ...
  public refreshUI(
    ...
    const sheetModule: IRenderer = <IRenderer>(
      this.parent.serviceLocator.getService('sheet')
    );
    const sheet: SheetModel = this.parent.getActiveSheet();
    const sheetName: string = getSheetName(this.parent as Workbook);
    const prevRowColCnt: SheetModel = {
    ...
    if (!address) {
      if (this.parent.scrollSettings.enableVirtualization) {
        ...
        let lastCol: number =
          args.colIndex +
          this.parent.viewport.colCount +
          this.parent.getThreshold('col') * 2;
        const frozenRow: number = this.parent.frozenRowCount(sheet);
        const frozenCol: number = this.parent.frozenColCount(sheet);
        ...
        const rowIdx: number =
          args.frozenIndexes[0] > frozenRow
            ? args.frozenIndexes[0]
            : args.rowIndex +
              (args.refresh === 'Row' ? frozenRow : sheet.frozenRows);
        ...
        if (args.refresh === 'Row') {
          args.rowIndex =
            skipHiddenIdx(sheet, startRow + frozenRow, true) - frozenRow;
        } else {
          startRow = args.rowIndex = frozenRow
            ? skipHiddenIdx(sheet, startRow, true)
            : indexes[0];
        }
        const colIdx: number =
          args.frozenIndexes[1] > frozenCol
            ? args.frozenIndexes[1]
            : args.colIndex +
              (args.refresh === 'Column' ? frozenCol : sheet.frozenColumns);
        ...
        if (args.refresh === 'Column') {
          args.colIndex =
            skipHiddenIdx(sheet, startCol + frozenCol, true, 'columns') -
            frozenCol;
        } else {
          startCol = args.colIndex = frozenCol
            ? skipHiddenIdx(sheet, startCol, true, 'columns')
            : indexes[0];
        }
        if (args.refresh === 'Row') {
          startRow += frozenRow;
        ...
        address = `${getCellAddress(startRow, startCol)}:${getCellAddress(
          lastRow,
          lastCol
        )}`;
      } else {
    ...
    const isOpen: boolean =
      this.parent.isOpen || (this.parent as any).refreshing;
    ...
    getData(
      ...
      if (
        !preventModelCheck &&
        (sheetIdx === -1 || sheetIdx !== this.parent.activeSheetIndex)
      ) {
        if (sheetIdx > -1) {
      ...
      const indexes: number[] = [
        args.rowIndex,
        args.colIndex,
        ...getCellIndexes(address.split(':')[1]),
      ];
      let isEdit: boolean;
  ...
  private updateTopLeftScrollPosition(args: {
    top?: number;
    left?: number;
    rowIndex?: number;
    colIndex?: number;
  ...
  public setSheetPanelSize(colMinWidth?: number): void {
    ...
    this.parent.viewport.colCount = this.roundValue(
      offset.width,
      this.colMinWidth || 64
    );
  }
  ...
  private roundValue(size: number, threshold: number): number {
    const value: number = size / threshold;
    const roundedValue: number = Math.round(value);
    return Math.abs(value - roundedValue) < 0.5
      ? roundedValue
      : roundedValue - 1;
  }

  private moveOrDuplicateSheetHandler(args: {
    refresh: boolean;
    isDuplicate: boolean;
  }): void {
  ...
  private addEventListener(): void {
    ...
    this.parent.on(
      getUpdatedScrollPosition,
      this.updateTopLeftScrollPosition,
      this
    );
  }
