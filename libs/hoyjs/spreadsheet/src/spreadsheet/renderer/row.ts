  ...
  public render(
    ...
    if (actualRowHgt < 20) {
      row.style.lineHeight =
        rowHeight > this.bottomBorderWidth
          ? rowHeight - this.bottomBorderWidth + 'px'
          : '0px';
    }
        ...
        while (i <= len) {
          if (!isHiddenCol(sheet, i)) {
            this.cellRenderer.render(<CellRenderArgs>{
              colIdx: i,
              rowIdx: index,
              cell: getCell(index, i, sheet),
              address: getCellAddress(index, i),
              lastCell: i === len,
              row: row,
              hRow: hRow,
              ...
              first:
                ...
                skipHiddenIdx(sheet, index, true) !==
