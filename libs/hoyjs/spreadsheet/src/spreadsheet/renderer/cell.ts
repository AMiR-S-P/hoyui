...
export class CellRenderer implements ICellRenderer {
  ...
  public renderColHeader(
    ...
    this.parent.trigger('beforeCellRender', <CellRenderEventArgs>{
      cell: null,
      element: headerCell,
      address: headerText,
      colIndex: index,
  ...
  public render(args: CellRenderArgs): Element {
    ...
    const evtArgs: CellRenderEventArgs = {
      cell: args.cell,
      element: args.td,
    ...
    if (!sheet.rows[args.rowIdx] || !sheet.rows[args.rowIdx].customHeight) {
      if (
        evtArgs.needHeightCheck ||
        (evtArgs.element && evtArgs.element.children.length && !args.cell.wrap)
      ) {
        const clonedCell: HTMLElement = evtArgs.element.cloneNode(
          true
        ) as HTMLElement;
        clonedCell.style.width =
          getColumnWidth(sheet, args.colIdx, true) + 'px';
        this.tableRow.appendChild(clonedCell);
      }
      if (args.lastCell && this.tableRow.childElementCount) {
        const tableRow: HTMLElement =
          args.row || this.parent.getRow(args.rowIdx);
        ...
        if (rowHeight > previouseHeight) {
          ...
          (
            args.hRow ||
            this.parent.getRow(args.rowIdx, this.parent.getRowHeaderTable())
          ).style.height = `${dprHgt}px`;
  ...
  private setWrapByValue(sheet: SheetModel, args: CellRenderArgs): void {
    if (
      args.cell &&
      isNullOrUndefined(args.cell.wrap) &&
      args.cell.value &&
      args.cell.value.toString().includes('\n')
    ) {
  ...
  private update(args: CellRenderArgs): void {
    ...
    if (
      !args.isRefresh ||
      (args.cell && (args.cell as ExtendedCellModel).template)
    ) {
      const compiledTemplate: string | Element[] = this.processTemplates(
        args.cell,
        args.rowIdx,
        args.colIdx
      );
    ...
    if (args.isRefresh) {
      ...
      if (
        ...
        const noteIndicator: Element = args.td.querySelector(
          '.e-addNoteIndicator'
        );
        if (noteIndicator) {
        ...
        const rowSpan: number =
          ...
          args.cell.rowSpan -
            this.parent.hiddenCount(
              args.rowIdx,
              args.rowIdx + (args.cell.rowSpan - 1)
            );
        if (rowSpan > 1) {
      ...
      if (args.cell.colSpan > 1) {
        const colSpan: number =
          ...
          args.cell.colSpan -
            this.parent.hiddenCount(
              args.colIdx,
              args.colIdx + (args.cell.colSpan - 1),
              'columns'
            );
        if (colSpan > 1) {
    ...
    if (args.checkNextBorder === 'Row') {
      const borderTop: string = this.parent.getCellStyleValue(
        ...
        [
          Number(
            this.parent.getContentTable().rows[0].getAttribute('aria-rowindex')
          ) - 1,
          args.colIdx,
        ]
      ).borderTop;
      if (
        ...
        (!args.cell ||
          !args.cell.style ||
          !(args.cell.style as CellStyleExtendedModel).bottomPriority)
      ) {
    ...
    if (args.checkNextBorder === 'Column') {
      const borderLeft: string = this.parent.getCellStyleValue(
        ['borderLeft'],
        [args.rowIdx, args.colIdx + 1]
      ).borderLeft;
      if (
        borderLeft !== '' &&
        (!args.cell ||
          !args.cell.style ||
          (!args.cell.style.borderRight && !args.cell.style.border))
      ) {
    ...
    if (args.cell && !isNullOrUndefined(args.cell.wrap)) {
      this.parent.notify(wrapEvent, {
        ...
        row: args.row,
        hRow: args.hRow,
        isCustomHgt: !args.isRefresh && getRowHeight(sheet, args.rowIdx) > 20,
      });
    }
    const validation: ValidationModel =
      (args.cell && args.cell.validation) ||
      (sheet.columns &&
        sheet.columns[args.colIdx] &&
        sheet.columns[args.colIdx].validation);
    if (
      validation &&
      (!validation.address ||
        isInMultipleRange(validation.address, args.rowIdx, args.colIdx))
    ) {
      if (validation.isHighlighted) {
        this.parent.notify(addHighlight, {
          range: getRangeAddress([args.rowIdx, args.colIdx]),
          td: args.td,
        });
      }
      if (
  ...
  private applyStyle(args: CellRenderArgs, style: CellStyleModel): void {
    if (
      ...
      Object.keys(this.parent.commonCellStyle).length ||
      args.lastCell
    ) {
      ...
      for (let i: number = 0; i < args.cell.image.length; i++) {
        if (args.cell.image[i as number]) {
          this.parent.notify(createImageElement, {
            options: {
              src: args.cell.image[i as number].src,
              imageId: args.cell.image[i as number].id,
              height: args.cell.image[i as number].height,
              width: args.cell.image[i as number].width,
              top: args.cell.image[i as number].top,
              left: args.cell.image[i as number].left,
            },
  ...
  private calculateFormula(args: CellRenderArgs): void {
    if (args.cell.value !== undefined && args.cell.value !== null) {
      const eventArgs: { [key: string]: string | number | boolean } = {
        ...
        added: true,
        address: args.address,
        sheetId: (args.sheetIndex === undefined
    ...
    args.cell.value = getCell(
      args.rowIdx,
      args.colIdx,
      isNullOrUndefined(args.sheetIndex)
        ? this.parent.getActiveSheet()
        : getSheet(this.parent, args.sheetIndex)
    ).value;
    if (
      isFormula &&
      (args.cell.formula.indexOf('^+') > -1 ||
        args.cell.formula.indexOf('&+') > -1)
    ) {
  ...
  private checkMerged(args: CellRenderArgs): boolean {
    if (args.cell && (args.cell.colSpan < 0 || args.cell.rowSpan < 0)) {
      ...
      if (sheet.frozenRows || sheet.frozenColumns) {
        ...
        if (sheet.frozenRows && sheet.frozenColumns) {
          if (
            mergeArgs.range[0] < frozenRow &&
            mergeArgs.range[1] < frozenCol
          ) {
          ...
          setDisplay = frozenRow
            ? mergeArgs.range[0] >= frozenRow || args.rowIdx < frozenRow
            : mergeArgs.range[1] >= frozenCol || args.colIdx < frozenCol;
        }
        if (setDisplay) {
      ...
      args.isMerged = true;
      const rowSpan: number = args.cell.rowSpan;
  ...
  private mergeFreezeRow(
    ...
    if (
      frozenRow &&
      rowIdx < frozenRow &&
      rowIdx + (rowSpan - 1) >= frozenRow
    ) {
      let rowEle: HTMLElement;
      let spanRowTop: number = 0;
      let height: number;
      const frozenCol: number = this.parent.frozenColCount(sheet);
      const row: HTMLTableRowElement =
        (tr as HTMLTableRowElement) || this.parent.getRow(rowIdx, null, colIdx);
      const emptyRows: Element[] = [].slice.call(
        row.parentElement.querySelectorAll('.e-empty')
      );
      if (unMerge) {
        ...
        if (curEmptyLength < emptyRows.length) {
          ...
          if (curEmptyLength === emptyRows.length) {
            ...
            for (i; i < len; i++) {
              ...
              if (
                curCell.rowSpan &&
                rowIdx + curCell.rowSpan - frozenRow > curSpan
              ) {
          ...
          for (
            let i: number = curSpan, len: number = emptyRows.length;
            i < len;
            i++
          ) {
      ...
      for (
        let i: number = frozenRow, len: number = rowIdx + (rowSpan - 1);
        i <= len;
        i++
      ) {
  ...
  private updateSpanTop(
    ...
    const mainPanel: HTMLElement =
      this.parent.serviceLocator.getService<IRenderer>('sheet').contentPanel;
    if (update) {
    ...
    if (
      ...
      (update ||
        ...
        top < parseInt(mainPanel.style.top, 10))
    ) {
  ...
  private mergeFreezeCol(
    ...
    if (
      frozenCol &&
      colIdx < frozenCol &&
      colIdx + (colSpan - 1) >= frozenCol
    ) {
      let col: HTMLElement;
      let width: number;
      const frozenRow: number = this.parent.frozenRowCount(sheet);
      const colGrp: Element = (
        rowIdx < frozenRow
          ? this.parent.getSelectAllContent()
          : this.parent.getRowHeaderContent()
      ).querySelector('colgroup');
      const emptyCols: Element[] = [].slice.call(
        colGrp.querySelectorAll('.e-empty')
      );
      if (unMerge) {
        ...
        if (curEmptyLength < emptyCols.length) {
          ...
          if (curEmptyLength === emptyCols.length) {
            ...
            for (i; i < len; i++) {
              ...
              if (
                curCell.colSpan &&
                colIdx + curCell.colSpan - frozenCol > curSpan
              ) {
          ...
          for (
            let i: number = curSpan, len: number = emptyCols.length;
            i < len;
            i++
          ) {
          ...
          this.parent.serviceLocator
            .getService<IRenderer>('sheet')
            .setPanelWidth(sheet, this.parent.getRowHeaderContent());
          if (!emptyCols.length) {
      ...
      for (
        let i: number = frozenCol, len: number = colIdx + (colSpan - 1);
        i <= len;
        i++
      ) {
        ...
        width = getColumnWidth(
          sheet,
          skipHiddenIdx(sheet, i, true, 'columns'),
          null,
          true
        );
  ...
  private updateSelectAllZIndex(remove: boolean): void {
    const frozenRowEle: HTMLElement =
      this.parent.element.querySelector('.e-frozen-row');
    const frozenColEle: HTMLElement =
    ...
    if (remove) {
      ...
      if (
        this.parent.getRowHeaderContent().style.zIndex ||
        this.parent.getColumnHeaderContent().style.zIndex
      ) {
    ...
    for (let j: number = 0, len: number = ranges.length; j < len; j++) {
      if (ranges[j as number].template) {
        range = getRangeIndexes(
          ranges[j as number].address.length
            ? ranges[j as number].address
            : ranges[j as number].startCell
        );
        if (
          range[0] <= rowIdx &&
          range[1] <= colIdx &&
          ...
          if (cell) {
            return this.compileCellTemplate(
              ...
              Object.assign({ rowIndex: rowIdx, colIndex: colIdx }, cell)
            );
          } else {
  ...
  private compileCellTemplate(
    ...
    if (typeof template === 'string') {
      ...
      if (!this.parent.isVue || this.isSelector(template)) {
        return (
          compiledStr(cell, this.parent, 'ranges', '', true)[0] as HTMLElement
        ).outerHTML;
      } else {
      ...
      const compiledTemplate: Element | Element[] = compiledStr(
        cell,
        this.parent,
        'ranges',
        ''
      );
      return (<Element[]>compiledTemplate)[0]
  ...
  private getRowHeightOnInit(): number {
    const tTable: HTMLElement = this.parent.createElement('table', {
      className: 'e-table e-test-table',
    });
    const tBody: HTMLElement = tTable.appendChild(
      this.parent.createElement('tbody')
    );
  ...
  private removeStyle(
    ...
    if (element.style.length) {
      cellStyle = this.parent.getCellStyleValue(
        ['borderLeft', 'border'],
        [rowIdx, colIdx + 1]
      );
      ...
      if (rightBorder || bottomBorder) {
        [].slice.call(element.style).forEach((style: string) => {
          if (rightBorder && bottomBorder) {
            ...
            (rightBorder &&
              !(style.indexOf('border-right') > -1) &&
              (!bottomBorder || bottomBorder === 'none')) ||
            (bottomBorder &&
              !(style.indexOf('border-bottom') > -1) &&
              (!rightBorder || rightBorder === 'none'))
          ) {
            element.style.removeProperty(style);
          }
        });
    ...
    if (prevRowCell && prevRowCell.style.borderBottom) {
      const prevRowIdx: number =
        Number(prevRowCell.parentElement.getAttribute('aria-rowindex')) - 1;
      cellStyle = this.parent.getCellStyleValue(
        ['borderBottom', 'border'],
        [prevRowIdx, colIdx]
      );
    ...
    const prevColCell: HTMLElement = <HTMLElement>(
        ...
        for (let j: number = cRange[1]; j <= cRange[3]; j++) {
          ...
          if (cell) {
            args = {
              ...
              isFromAutoFillOption: isFromAutoFillOption,
            };
  ...
  public refresh(
    ...
    if (
      !element &&
      (isHiddenRow(sheet, rowIdx) || isHiddenCol(sheet, colIdx))
    ) {
      return;
    }
    if (
      ...
      this.parent.insideViewport(rowIdx, colIdx)
    ) {
      const cell: HTMLTableCellElement = <HTMLTableCellElement>(
        (element || this.parent.getCell(rowIdx, colIdx))
      );
      if (!cell) {
        return;
      }
  ...
  private updateView(args: {
    ...
    if (
      isNullOrUndefined(args.sheetIndex) ||
      args.sheetIndex === this.parent.activeSheetIndex
    ) {
      if (!args.indexes) {
        const sheet: SheetModel = this.parent.getActiveSheet();
        const frozenRow: number = this.parent.frozenRowCount(sheet);
        ...
        if (frozenRow && frozenCol) {
          this.refreshRange(
            [topLeftCell[0], topLeftCell[1], frozenRow - 1, frozenCol - 1],
            args.refreshing,
            args.checkWrap,
            false,
            args.checkCF
