...
export class Print {
  ...
  public print(spreadsheet: Spreadsheet, printOptions: PrintOptions): void {
    ...
    if (printOptions.type === 'ActiveSheet') {
      const sheet: SheetModel =
        spreadsheet.sheets[spreadsheet.activeSheetIndex as number];
      ...
      for (let i: number = 0; i < spreadsheet.sheets.length; i++) {
        if (spreadsheet.sheets[i as number].state === 'Visible') {
          ...
          if (
            this.totalSheetCount.length > 1 &&
            this.parent.sheets[i as number].ranges.length > 0
          ) {
            const isDatasourceAvailable: boolean = this.parent.sheets[
              i as number
            ].ranges.some((range: RangeModel): boolean => {
            ...
            if (isDatasourceAvailable) {
              ...
              const address: string =
                getCellAddress(0, 0) +
                ':' +
                getCellAddress(sheet.rowCount - 1, sheet.colCount - 1);
      ...
      if (this.allowTimer) {
        ...
        setTimeout(() => {
          this.activeSheetPrint(
            spreadsheet,
            spreadsheet.sheets[this.totalSheetCount[0]],
            printOptions,
  ...
  private activeSheetPrint(
    ...
    this.pageCounts = this.calculatePageCount(
      sheet,
      1000,
      printOptions.allowRowColumnHeader
    );
    ...
    for (let i: number = page; i < this.pageCounts.length; i++) {
      ...
      for (let j: number = rowsCount; j < this.endRow; j++) {
        ...
        const rowHeight: number =
          (sheet.rows[j as number] && sheet.rows[j as number].height) ||
        ...
        height += isNullOrUndefined(sheet.rows[j as number])
          ? this.defaultCellHeight
          : rowHeight +
            (j === 0 && allowColumnAndRow ? this.defaultCellHeight : 0);
        let cellHeight: number = isNullOrUndefined(sheet.rows[j as number])
          ? this.defaultCellHeight
          : (sheet.rows[j as number] && sheet.rows[j as number].height) ||
            this.defaultCellHeight;
        if (height > 1080) {
          this.startNewPageCount = j;
          lineHeight = allowColumnAndRow
            ? rowHeight +
              (j === 0 && allowColumnAndRow ? this.defaultCellHeight : 0)
            : 0;
          ...
          height =
            allowColumnAndRow || printOptions.allowGridLines
              ? rowHeight + (allowColumnAndRow ? this.defaultCellHeight : 0)
              : rowHeight;
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const style: any = {
          ...
          for (let k: number = start; k <= end; k++) {
            ...
            for (let m: number = 0; m < columnIndex; m++) {
              let titleWidth: number =
                m === 0 && columnIndex === 2
                  ? headerWidth
                  : sheet.columns[k as number] &&
                    sheet.columns[k as number].hidden
                  ? 0
                  : (sheet.columns[k as number] &&
                      sheet.columns[k as number].width) ||
                    this.defaultCellWidth;
              titleWidth =
                currentX + titleWidth > 1000
                  ? titleWidth - 1 - (currentX + titleWidth - 1000)
                  : titleWidth;
              if (titleWidth !== 0) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const textMetrics: any = context.measureText(columnText);
                const textWidth: number = textMetrics.width;
                const locationX: number = printInstance.calculateTextPosition(
                  textWidth,
                  titleWidth,
                  currentX,
                  'Center'
                ...
                context.fillText(
                  k === start && m === 0 ? '' : columnText,
                  locationX,
                  locationY
                );
                printInstance.drawBorder(
                  context,
                  style,
                  currentX,
                  0,
        ...
        if (
          !isNullOrUndefined(sheet.rows[j as number]) &&
          (isNullOrUndefined(sheet.rows[j as number].hidden) ||
            !sheet.rows[j as number].hidden)
        ) {
          if (isExtraLine) {
            const extaSpace: number = sheet.rows
              .slice(j, j + sheet.rows.length - j)
              .map((row: RowModel) => {
            ...
            if (canvas.height > height + (this.chartHeight - extaSpace)) {
              ...
              this.endRow += Math.ceil(
                (canvas.height - (height + extaSpace)) / 19
              );
            }
            isExtraLine = false;
          }
          borderOfHeaderText =
            this.endRow === sheet.rows.length
              ? height +
                  (sheet.rows[j + 1]
                    ? sheet.rows[j + 1].height || this.defaultCellHeight
                    : this.defaultCellHeight) +
                  (j === 0 && allowColumnAndRow ? this.defaultCellHeight : 0) >
                  1080 || j === sheet.rows.length - 1
              : false;
          // Loop through cells in a row
          for (let k: number = start; k <= end; k++) {
            const cell: CellModel =
              sheet.rows[j as number] &&
              !isNullOrUndefined(sheet.rows[j as number].cells) &&
              sheet.rows[j as number].cells[k as number];
            if (
              isNullOrUndefined(sheet.columns[k as number]) ||
              isNullOrUndefined(sheet.columns[k as number].hidden) ||
              !sheet.columns[k as number].hidden
            ) {
              const isColumnSpan: boolean =
                !isNullOrUndefined(sheet.rows[j as number].cells) &&
                !isNullOrUndefined(
                  sheet.rows[j as number].cells[k as number]
                ) &&
              ...
              const isRowSpan: boolean =
                ...
                !(
                  j + sheet.rows[j as number].cells[k as number].rowSpan >
                  this.startNewPageCount
                ) &&
              ...
              const cellRowSpan: number = !isNullOrUndefined(cell)
                ? isNaN(cell.rowSpan) || isColumnSpan || isRowSpan
                  ? 1
                  : Math.max(cell.rowSpan, 0)
                : 1;
              currentY[k as number] = isNaN(currentY[k as number])
                ? defaultCellSpace
                : currentY[k as number];
                ...
                if (cellRowSpan > 0) {
                  for (
                    ...
                    rowHeaderHeight +=
                      o === 0
                        ? 0
                        : (sheet.rows[j + o] && sheet.rows[j + o - 1].height) ||
                          this.defaultCellHeight;
                    printInstance.rowHeaderText(
                      (j + 1 + o).toString(),
                      context,
                      printInstance,
                      headerWidth,
              ...
              if (!isNullOrUndefined(cell) && cell) {
                const fontSize: string = cell.style
                  ? cell.style.fontSize || '11pt'
                  : '11pt';
                let color: string = cell.style
                ...
                if (
                  ...
                  if (isNaN(sheet.rows[j + 1].cells[k as number].rowSpan)) {
                    this.parent.merge(
                      `${getColumnHeaderText(k + 1)}${
                        j + 1
                      }:${getColumnHeaderText(k + 1)}${
                        j +
                        sheet.rows[j as number].cells[k as number].rowSpan -
                        1
                      }`
                    );
                  }
                ...
                if (
                  ...
                  if (
                    !isNullOrUndefined(sheet.rows[j + 1].cells) &&
                    sheet.rows[j + 1].cells.length > 0 &&
                    ...
                    this.parent.merge(
                      '' +
                        ...
                        getColumnHeaderText(
                          k +
                            1 +
                            sheet.rows[j as number].cells[k as number].colSpan -
                            1
                        ) +
                        (j + 1)
                    );
                    ...
                    isNullOrUndefined(
                      sheet.rows[j as number].cells[k + 1].colSpan
                    )
                  ) {
                    for (
                      let m: number = 1;
                      m < sheet.rows[j as number].cells[k as number].colSpan;
                      m++
                    ) {
                ...
                if (
                  (k === start ||
                    (k > 0 &&
                      sheet.columns &&
                      sheet.columns[k - 1] &&
                      sheet.columns[k - 1].hidden &&
                      sheet.rows[j as number].cells[k - 1] &&
                      sheet.rows[j as number].cells[k - 1].colSpan > 1)) &&
                  cellWidthSpan <= 0
                ) {
                  cellWidthSpan = 1;
                }
                const backgroundColor: string = cell.style
                  ? cell.style.backgroundColor || '#ffffff'
                  : '#ffffff';
                const textAlign: string = cell.style
                ...
                if (cellWidthSpan > 1) {
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  const widthColumn: any[] = sheet.columns.slice(
                    k,
                    k + cellWidthSpan
                  );
                  for (let o: number = 0; o < widthColumn.length; o++) {
                    cellWidthSpanArray.push(
                      widthColumn['hidden']
                        ? 0
                        : (widthColumn[o as number] &&
                            widthColumn[o as number]['width']) ||
                            this.defaultCellWidth
                    );
                  }
                  if (cellWidthSpanArray.length === 0) {
                    cellWidthSpanArray.push(
                      this.defaultCellWidth * cellWidthSpan
                    );
                  } else if (cellWidthSpanArray.length !== cellWidthSpan) {
                    cellWidthSpanArray = cellWidthSpanArray.map(
                      (value: number) =>
                        value +
                        this.defaultCellWidth *
                          (cellWidthSpan - 1 - cellWidthSpanArray.length)
                    );
                  }
                }
                cellWidth =
                  cellWidthSpan > 1
                    ? cellWidthSpanArray.reduce(
                    ...
                    : 0;
                cellWidth =
                  currentX + cellWidth > 1000
                    ? cellWidth - 1 - (currentX + cellWidth - 1000)
                    : cellWidth;
                cellHeight =
                  cellRowSpan > 1
                    ? sheet.rows
                        .slice(j, j + cellRowSpan)
                        .map(
                          (row: RowModel) =>
                            row.height || this.defaultCellHeight
                        )
                        .reduce(
                          (accumulator: number, currentValue: number) =>
                            accumulator +
                            (currentValue || this.defaultCellHeight),
                          0
                        )
                    : cellRowSpan === 1
                    ? rowHeight
                ...
                if (
                  ...
                  !isNullOrUndefined(cell.formula) ||
                  ...
                  if (cell.formula && cell.format) {
                    ...
                    const numberFormatArgs: NumberFormatArgs = {
                      ...
                      rowIndex: j,
                      colIndex: k,
                      sheetIndex: this.parent.activeSheetIndex as number,
                      cell: cell,
                      refresh: true,
                    };
                    cellText =
                      this.parent.workbookNumberFormatModule.getFormattedCell(
                        numberFormatArgs
                      );
                    position = `${
                      textAlign
                        ? textAlign
                        : numberFormatArgs.isRightAlign
                        ? 'Right'
                        : 'Left'
                    }`;
                  } else if (cell.format) {
                      ...
                      if (isNullOrUndefined(cell.value)) {
                        cell.value =
                          ...
                          typeof cell.hyperlink === 'object'
                            ...
                            : cell.hyperlink || cell.hyperlink['address'];
                      } else if (
                        isNullOrUndefined(cell.value) ||
                        cell.value === ''
                      ) {
                    ...
                    const isNumber: boolean =
                      !isNullOrUndefined(cell.value) && cell.value !== ''
                        ? new RegExp('^[^\\p{L}]*$', 'u').test(cell.value)
                        : false;
                    position =
                      '' +
                      (textAlign ? textAlign : isNumber ? 'Right' : 'Left');
                  }
                  const textSize: string = cell.style
                    ? cell.style.fontSize || '11pt'
                    : '11pt';
                  const fontFamily: string = cell.style
                    ? cell.style.fontFamily || 'Calibri'
                    : 'Calibri';
                  const fontstyle: string = cell.style
                    ? cell.style.fontStyle || ''
                    : '';
                  const fontWeight: string = cell.style
                  ...
                  if (cell.wrap) {
                    const cellLineHeight: number =
                      this.defaultCellHeight < cellHeight
                        ? (parseInt(textSize.replace('pt', ''), 10) / 72) * 96
                        : cellHeight;
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const textLines: any = printInstance.wrapText(
                      context,
                      cellText,
                      cellWidth
                    );
                    const space: number =
                      textLines.length === 1
                        ...
                        ? cellLineHeight
                        : textLines.length * cellLineHeight < cellHeight
                        ? cellHeight - (textLines.length - 1) * cellLineHeight
                        : cellLineHeight;
                    const startY: number =
                      currentY[k as number] +
                      (verticalAlign === 'top'
                        ...
                        ? space > cellLineHeight + space / 4
                          ? cellLineHeight + space / 4
                          : space
                        : space);
                    context.save();
                    context.beginPath();
                    ...
                    context.fillRect(
                      ...
                      cellWidth,
                      cellHeight +
                        (verticalAlign === 'top' ? cellLineHeight : 0)
                    );
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    textLines.forEach((line: any, index: number) => {
                      ...
                      const locationX: number =
                        printInstance.calculateTextPosition(
                          textWidth,
                          cellWidth,
                          currentX,
                          position
                      ...
                      context.fillText(
                        line,
                        locationX,
                        startY + index * cellLineHeight
                      );
                      printInstance.textDecoration(
                        cell,
                        context,
                        locationX,
                        startY + index * cellLineHeight,
                    ...
                    context.fillStyle = color;
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const locationX: any = printInstance.calculateTextPosition(
                      textWidth,
                      cellWidth,
                      currentX,
                      position
                    ...
                    const locationY: number =
                      ...
                      (verticalAlign === 'top'
                        ? textHeight + 2
                        : verticalAlign === 'middle'
                        ? cellHeight > cellHeight / 2 + textHeight / 2
                          ? cellHeight / 2 + textHeight / 2
                          : cellHeight
                        : cellHeight);
              ...
              if (cellRowSpan > 1) {
                ...
                for (let m: number = k; m < k + cellWidthSpan; m++) {
                  ...
                  currentY[m as number] =
                    (isNaN(currentY[m as number])
                      ? defaultCellSpace
              ...
              if (k === end && allowColumnAndRow) {
                printInstance.drawBorder(
                  ...
                  rightStyle,
                  currentX - 2,
                  currentY[k as number] - currentYValue,
                  2,
                  cellHeight
                );
              }
  ...
  private handleCharts(
    ...
    lineHeight: number,
    rowHeight: number,
    allowColumnAndRow: boolean,
    printOptions: PrintOptions,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    sheet: SheetModel,
    printInstance: any,
    multipleCanvasDataURL: string[],
    currentX: number,
    currentY: number[],
    pageHeight: number,
    sheetIndex: number
    ...
    if (!isNullOrUndefined(cell.chart)) {
      for (const chart of cell.chart) {
        ...
        if (!isNullOrUndefined(chartElement)) {
          ...
          const isExtraLine: boolean =
            this.initialRowCount + Math.ceil(chart.height / 19) > this.endRow &&
          ...
          if (isExtraLine) {
            if (sheet.rows.length - 1 === j) {
              ...
              if (canvas.height > height + (this.chartHeight - extraSpace)) {
                ...
                this.endRow += Math.ceil(
                  (canvas.height - (height + extraSpace)) / 19
                );
              }
            }
          }
          const svgElement: HTMLElement = chartElement.cloneNode(
            true
          ) as HTMLElement;
            ...
            new Blob(
              [
                new XMLSerializer().serializeToString(
                  svgElement.childNodes[1] as Node
                ),
              ],
              { type: 'image/svg+xml' }
            )
          ...
          image.onload = () => {
            ...
            const top: number =
              chart.top +
              (allowColumnAndRow ? 19 : 0) +
              (chartTopIndex > 0 ? lineHeight : 0);
            context.drawImage(
              image,
              chartLeftIndex <= 0 ? left : left - chartLeftIndex * 1000,
              chartTopIndex <= 0 ? top : top - chartTopIndex * 1160,
              chart.width,
              chart.height
            );
            ...
            if (this.chartLoadedCount === this.totalCharts) {
              ...
              if (!this.isImageLoaded) {
                ...
                const startValue: number =
                  this.initialRowCount === sheet.rows.length - 1 &&
                  height < 1080
                    ? sheet.rows.length
                    : this.initialRowCount;
                ...
                this.processCell(
                  i,
                  startValue + 1 === endValue ? endValue : startValue,
                  endValue,
                  currentX,
                  currentY,
                  context,
  ...
  private processImages(
    ...
    if (!isNullOrUndefined(cell.image)) {
      for (const image of cell.image) {
        ...
        const isExtraLine: boolean =
          this.initialRowCount + Math.ceil(image.height / 19) > this.endRow &&
          (printOptions.allowRowColumnHeader || printOptions.allowGridLines);
        if (isExtraLine) {
        ...
        img.onload = () => {
          ...
          if (this.imageLoadedCount === this.totalImages) {
            ...
            if (!this.isChartLoaded) {
              this.multipleCanvasDataURL.push(canvas.toDataURL());
              const startValue: number =
                this.initialRowCount === sheet.rows.length - 1 && height < 1080
                  ? sheet.rows.length
                  : this.initialRowCount;
              const endValue: number = this.endRow;
              this.processCell(
  ...
  private textDecoration(
    ...
    if (
      cellText !== '' &&
      ((!isNullOrUndefined(cell.style) &&
        !isNullOrUndefined(cell.style.textDecoration) &&
        cell.style.textDecoration.toLowerCase().indexOf('underline') > -1) ||
        !isNullOrUndefined(cell.hyperlink))
    ) {
      context.beginPath();
      ...
      context.lineTo(
        locationX +
          (textMetrics.width > cellWidth ? cellWidth : textMetrics.width),
        locationY - 3
      );
  ...
  private canvasPrint(
    ...
    if (
      printOptions.type === 'Workbook' &&
      this.totalSheetCount.length - 1 >= this.workbookActiveSheetCount
    ) {
      const currentSheetIndex: number =
        this.totalSheetCount[this.workbookActiveSheetCount];
      ...
      for (let i: number = 0; i < this.chartElements.length; i++) {
        const chartElement: HTMLElement = document.getElementById(
          this.chartElements[i as number]
        );
        if (!isNullOrUndefined(chartElement)) {
          chartElement.remove();
        }
      ...
      if (this.multipleCanvasDataURL.length > 0) {
        ...
        if (
          browserUserAgent.indexOf('Chrome') !== -1 ||
          browserUserAgent.indexOf('Safari') !== -1 ||
          ...
          printWindow.document.write(
            '<html><head><style>html, body {  }' +
              ...
              ' div{ page-break-inside: avoid; }} @page{ size:' +
        ...
        this.multipleCanvasDataURL.forEach((dataURL: string, index: number) => {
          const canvas: HTMLCanvasElement =
            printWindow.document.createElement('canvas');
  ...
  private drawBorder(
    ...
    if (
      !isNullOrUndefined(borderBottom) &&
      borderBottom.indexOf('#FFFFFF') === -1
    ) {
      this.drawPath(
        locationX,
    ...
    if (
      !isNullOrUndefined(borderLeft) &&
      borderLeft.indexOf('#FFFFFF') === -1
    ) {
      this.drawPath(
        locationX,
        locationY,
    ...
    if (
      !isNullOrUndefined(borderRight) &&
      borderRight.indexOf('#FFFFFF') === -1
    ) {
      this.drawPath(
        locationX + cellWidth,
        locationY,
    ...
    if (availableSpace >= textWidth) {
      ...
      x =
        position.toLowerCase() === 'left'
          ...
          ? currentX + (totalWidth - textWidth) / 2
  ...
  private calculatePageCount(
    ...
    if (sheet.columns.length === 0) {
      const columnCount: number =
        Math.floor(columnHeaderWidth / this.defaultCellWidth) -
        (allowHeader ? 1 : 0);
      return Array(Math.max(1, Math.ceil(usedRangeColumn / columnCount))).fill(
        columnCount
      );
    }
    ...
    const lastRowIndex: number = Math.max(
    ...
    for (let i: number = 0; i <= colIndex; i++) {
      ...
      const columnWidth: number =
        ...
        (column && column.hidden
          ? 0
          : column && !isNullOrUndefined(column.width)
          ? column.width
          : this.defaultCellWidth);
