...
export class SheetRender implements IRenderer {
  ...
  private refreshSelectALLContent(): void {
    ...
    cell.appendChild(
      this.parent.createElement('button', {
        ...
        attrs: {
          'aria-label': this.parent.serviceLocator
  ...
  private updateLeftColGroup(width?: number, rowHdr?: HTMLElement): void {
    ...
    if (width) {
      (
        table.querySelector('colgroup').firstElementChild as HTMLElement
      ).style.width = `${this.colGroupWidth}px`;
    } else {
  ...
  public setPanelWidth(
    ...
    if (isRtlChange) {
      ...
      this.getContentPanel().style[`${rtlOffset}`] =
        this.getColHeaderPanel().style[`${rtlOffset}`] = '';
    }
    if (sheet.frozenColumns) {
      const frozenCol: HTMLElement = document
        .getElementById(this.parent.element.id + '_sheet')
        .getElementsByClassName('e-frozen-column')[0] as HTMLElement;
    ...
    let scroll: HTMLElement = (
      this.contentPanel.nextElementSibling
        ? this.contentPanel.nextElementSibling
        : null
    ) as HTMLElement;
    if (scroll) {
      ...
      if (
        Browser.userAgent.indexOf('Mac OS') > -1 &&
  ...
  public getScrollSize(addOffset?: boolean): number {
    const scrollSize: number = parseInt(
      this.headerPanel.style[
        this.parent.enableRtl ? 'margin-left' : 'margin-right'
      ],
      10
    );
  ...
  private setHeaderPanelWidth(content: HTMLElement, width: number): void {
    const emptyCol: HTMLElement[] = [].slice.call(
      content.querySelectorAll('col.e-empty')
    );
  ...
  private setPanelHeight(sheet: SheetModel): void {
    ...
    if (sheet.frozenRows) {
      ...
      (this.contentPanel as HTMLElement).style.height = `calc(100% - ${
        frozenHeight + scrollSize
      }px)`;
      const frozenRow: HTMLElement = document
        .getElementById(this.parent.element.id + '_sheet')
        .getElementsByClassName('e-frozen-row')[0] as HTMLElement;
      frozenRow.style.width = Browser.isDevice
        ? '100%'
        : `calc(100% - ${scrollSize}px)`;
      frozenRow.style.top =
        frozenHeight - 1 - (sheet.showHeaders ? 1 : 0) + 'px';
      frozenRow.style.display = '';
    } else {
      (this.contentPanel as HTMLElement).style.height = `calc(100% - ${
        (sheet.showHeaders ? getDPRValue(31) : 0) + scrollSize
      }px)`;
    }
  }
  ...
  private createHeaderTable(): void {
    const rowHdrEle: HTMLElement =
      this.contentPanel.querySelector('.e-row-header');
    const sheet: SheetModel = this.parent.getActiveSheet();
    if (sheet.frozenRows || sheet.frozenColumns) {
      this.updateTable(
        sheet.frozenRows ? ['thead', 'tbody'] : ['thead'],
        'selectall',
        this.headerPanel.querySelector('.e-selectall-container')
      );
    }
    this.updateTable(
      sheet.frozenRows ? ['thead', 'tbody'] : ['thead'],
      'colhdr',
      this.headerPanel.querySelector('.e-column-header')
    );
  ...
  private updateTable(
    ...
    tagName.forEach((tag: string): void => {
      table.appendChild(this.parent.createElement(tag));
    });
  }
  ...
  public renderTable(args: SheetRenderArgs): void {
    ...
    if (this.parent.allowScrolling) {
      const scrollPanel: HTMLElement = this.parent.createElement('div', {
        className: 'e-scrollbar',
      });
      scrollPanel.appendChild(
        this.parent.createElement('div', { className: 'e-scroller' })
      );
      frag.appendChild(scrollPanel);
    }
    this.createHeaderTable();
    this.updateTable(['tbody'], 'content', this.contentPanel.lastElementChild);
    const colGrp: Element = this.parent.createElement('colgroup');
    let col: Element;
    const cTBody: Element = this.contentPanel.querySelector(
      '.e-sheet-content tbody'
    );
    this.refreshSelectALLContent();
    const selectAllColGrp: Element =
      this.getSelectAllContent().querySelector('colgroup');
    const rowHdrColGrp: Element =
      this.getRowHeaderPanel().querySelector('colgroup');
    const selectAllHdrRow: Element = this.getSelectAllContent().querySelector(
      'thead .e-header-row'
    );
    const rHdrTBody: Element = this.contentPanel.querySelector(
      '.e-row-header tbody'
    );
    const selectAllTBody: Element =
      this.getSelectAllContent().querySelector('tbody');
    const cHdrTHead: Element = this.headerPanel.querySelector(
      '.e-column-header thead'
    );
    const cHdrTBody: Element = this.headerPanel.querySelector(
      '.e-column-header tbody'
    );
    const cHdrRow: Element = this.rowRenderer.render();
    ...
    const lastFrozenCol: number = skipHiddenIdx(
      sheet,
      frozenCol - 1,
      false,
      'columns'
    ...
    const colCount: string = sheet.colCount.toString();
    const rowCount: string = sheet.colCount.toString();
    const layout: string =
      args.top && args.left
        ? 'RowColumn'
        : args.top
    ...
    let emptyRow: Element;
    attributes(this.parent.getContentTable(), {
      'aria-rowcount': rowCount,
      'aria-colcount': colCount,
    });
    (args.cells as Map<string, CellModel>).forEach(
      (value: CellModel, key: string): void => {
        ...
        this.cellRenderer.render(<CellRenderArgs>{
          colIdx: indexes[1],
          rowIdx: indexes[0],
          cell: value,
          address: key,
          ...
          first: layout
            ? layout.includes('Row')
              ? indexes[0] === args.indexes[0]
                ? 'Row'
                : layout.includes('Column')
                ? indexes[1] === args.indexes[1]
                  ? 'Column'
                  : ''
                : ''
              : indexes[1] === args.indexes[1]
    ...
    getUpdateUsingRaf((): void => {
      ...
      const sheetContent: HTMLElement = document.getElementById(
        this.parent.element.id + '_sheet'
      );
      if (
        sheetContent.childElementCount &&
        sheetContent.querySelector('.e-header-panel') !== this.headerPanel
      ) {
        const sheetChild: HTMLCollection = sheetContent.children;
        for (let i: number = 0; i < sheetChild.length; i++) {
          if (
            !sheetChild[i as number].classList.contains('e-frozen-row') &&
      ...
      this.parent.notify(editOperation, {
        action: 'renderEditor',
        initLoad: args.initLoad && !this.parent.isOpen,
      });
      ...
      if (
        args.openOptions &&
        args.openOptions.eventArgs &&
        args.openOptions.eventArgs.triggerEvent
      ) {
      ...
      if (args.initLoad) {
        ...
        if (this.parent.scrollSettings.enableVirtualization) {
          for (let i: number = 0; i < sheet.ranges.length; i++) {
            if (
              (<ExtendedRange>sheet.ranges[i as number]).info.count - 1 >
    ...
    if (this.parent.createdHandler) {
      if ((this.parent.createdHandler as { observers: object }).observers) {
        (this.parent['created'] as { observers?: object }).observers = (
          this.parent.createdHandler as { observers: object }
        ).observers;
        if (
          this.parent.isAngular &&
          (this.parent.createdHandler as { currentObservers: object })
            .currentObservers
        ) {
          (
            this.parent['created'] as { currentObservers?: object }
          ).currentObservers = (
            this.parent.createdHandler as { currentObservers: object }
          ).currentObservers;
        }
  ...
  private checkRowHeightChanged(
    args: { top?: number; left?: number },
    sheet: SheetModel
  ): void {
    const eventArgs: { top: number; left: number; sheet: SheetModel } = {
      top: args.top,
      left: args.left,
      sheet: sheet,
    };
    ...
    if (args.top !== eventArgs.top) {
      if (
        ...
        this.parent.scrollModule.offset.top.idx &&
        eventArgs.top - args.top <
    ...
    if (args.left !== eventArgs.left) {
      if (
        this.parent.scrollModule &&
        this.parent.scrollModule.offset.left.idx &&
        eventArgs.left - args.left <
  ...
  private checkTableWidth(sheet: SheetModel): void {
    if (
      ...
      sheet.colCount - 1 === this.parent.viewport.rightIndex
    ) {
      ...
      if (
        cellsWidth <
        this.contentPanel.getBoundingClientRect().width -
          rowHdrWidth -
          scrollSize
      ) {
  ...
  private clearCFResult(sheet: SheetModel): void {
    if (sheet.conditionalFormats && sheet.conditionalFormats.length) {
      const cfRule: ConditionalFormat[] =
        sheet.conditionalFormats as ConditionalFormat[];
  ...
  public refreshColumnContent(args: SheetRenderArgs): void {
    ...
    let tBody: Element = this.parent.element.querySelector(
      '.e-sheet-content tbody'
    );
    let hTBody: Element = this.parent.element.querySelector(
      '.e-column-header tbody'
    );
    let colGrp: Element = this.parent.element.querySelector(
      '.e-sheet-content colgroup'
    );
    colGrp = colGrp.cloneNode() as Element;
    frag.appendChild(colGrp);
    tBody = frag.appendChild(tBody.cloneNode(true) as Element);
    const hColGrp: Element = colGrp.cloneNode() as Element;
    ...
    const hRow: HTMLElement = tHead.querySelector('tr');
    hRow.innerText = '';
    const frozenRow: number = this.parent.frozenRowCount(sheet);
    const frozenCol: number = this.parent.frozenColCount(sheet);
    if (frozenRow) {
      hTBody = hFrag.appendChild(hTBody.cloneNode(true) as Element);
    }
    const lastFrozenRow: number = skipHiddenIdx(sheet, frozenRow - 1, false);
    const notFirstRow: boolean =
      this.parent.scrollSettings.enableVirtualization &&
    ...
    (args.cells as Map<string, CellModel>).forEach(
      (value: CellModel, key: string): void => {
        indexes = getRangeIndexes(key);
        if (indexes[0] === args.indexes[0]) {
          col = this.updateCol(sheet, indexes[1], hColGrp);
        ...
        cell = this.cellRenderer.render(<CellRenderArgs>{
          ...
          pRow: row.previousSibling,
          first:
            !args.skipUpdateOnFirst && indexes[1] === args.indexes[1]
              ? 'Column'
              : notFirstRow && indexes[0] === args.indexes[0]
              ? 'Row'
              : '',
          isRefreshing: true,
        });
    ...
    const appendColumns: Function = (): void => {
      ...
      setAriaOptions(this.parent.getMainContent() as HTMLElement, {
        busy: false,
      });
    };
  ...
  public refreshRowContent(args: SheetRenderArgs): void {
    let indexes: number[];
    let row: HTMLElement;
    let hdrRow: HTMLElement;
    let colGroupWidth: number = this.colGroupWidth;
    const sheet: SheetModel = this.parent.getActiveSheet();
    let cell: Element;
    const frag: DocumentFragment = document.createDocumentFragment();
    ...
    const notFirstCol: boolean =
      this.parent.scrollSettings.enableVirtualization &&
      this.parent.viewport.leftIndex !==
    ...
    (args.cells as Map<string, CellModel>).forEach(
      (value: CellModel, key: string): void => {
        ...
        if (frozenCol) {
          hdrRow = (hTBody.lastElementChild as HTMLElement) || hdrRow;
        }
        cell = this.cellRenderer.render(<CellRenderArgs>{
          rowIdx: indexes[0],
          colIdx: indexes[1],
          ...
          isHeightCheckNeeded: true,
          first:
            !args.skipUpdateOnFirst && indexes[0] === args.indexes[0]
              ? 'Row'
              : notFirstCol && indexes[1] === args.indexes[1]
              ? 'Column'
              : '',
          isRefreshing: true,
        });
  ...
  public updateCol(
    ...
    if (appendTo) {
      ...
      return empty
        ? appendTo.insertBefore(col, empty)
  ...
  public updateColContent(args: SheetRenderArgs): void {
    getUpdateUsingRaf((): void => {
      ...
      let col: HTMLElement;
      const sheet: SheetModel = this.parent.getActiveSheet();
      const hRow: Element = this.parent.element.querySelector(
        '.e-column-header .e-header-row'
      );
      const hRefChild: Element = hRow.firstElementChild;
      const colGrp: Element = this.parent.element.querySelector(
        '.e-sheet-content colgroup'
      );
      const hColGrp: Element = this.parent.element.querySelector(
        '.e-column-header colgroup'
      );
      ...
      const tBody: HTMLTableSectionElement = this.parent.element.querySelector(
        '.e-sheet-content tbody'
      );
      const hTBody: HTMLTableSectionElement = this.parent.element.querySelector(
        '.e-column-header tbody'
      );
      const frozenRow: number = this.parent.frozenRowCount(sheet);
      const frozenCol: number = this.parent.frozenColCount(sheet);
      ...
      (args.cells as Map<string, CellModel>).forEach(
        (value: CellModel, key: string): void => {
          ...
          if (args.direction === 'first' && indexes[1] === args.indexes[1]) {
            this.checkColMerge(
              [indexes[0], this.parent.viewport.leftIndex + frozenCol],
              args.indexes,
              (
                (indexes[0] < frozenRow ? hTBody : tBody).rows[
                  rowCount as number
                ] || { cells: [] }
              ).cells[args.indexes[3] - args.indexes[1] + 1],
              getCell(
                indexes[0],
                this.parent.viewport.leftIndex + frozenCol,
                sheet
              ) || {}
            );
          }
          if (indexes[0] === firstRow) {
            if (args.direction === 'last') {
              ...
              col.style.width = formatUnit(
                getColumnWidth(sheet, indexes[1], null, true)
              );
              ...
              col = this.updateCol(sheet, indexes[1], colGrp) as HTMLElement;
          ...
          cellArgs = {
            ...
            first:
              args.direction === 'last' &&
              !args.skipUpdateOnFirst &&
              indexes[1] === args.indexes[1]
                ? 'Column'
                : '',
            checkNextBorder:
              args.direction === 'last' && indexes[3] === args.indexes[3]
                ? 'Column'
                : '',
  ...
  public updateRowContent(args: SheetRenderArgs): void {
    ...
    let colGroupWidth: number = this.colGroupWidth;
    let row: HTMLElement;
    let hRow: HTMLElement;
    let cell: Element;
    const sheet: SheetModel = this.parent.getActiveSheet();
    let count: number = 0;
    const tBody: HTMLTableSectionElement = mainContent.querySelector('tbody');
    const rTBody: HTMLTableSectionElement = this.parent
      .getRowHeaderContent()
      .querySelector('tbody');
    ...
    const firstCol: number = skipHiddenIdx(
      sheet,
      args.indexes[1],
      true,
      'columns'
    ...
    this.clearCFResult(sheet);
    (args.cells as Map<string, CellModel>).forEach(
      (value: CellModel, cKey: string): void => {
        indexes = getRangeIndexes(cKey);
        if (args.direction === 'first' && indexes[0] === args.indexes[0]) {
          if (firstRow === undefined) {
            firstRow =
              (indexes[1] < frozenCol ? rTBody : tBody).rows[
                args.indexes[2] - args.indexes[0] + 1
              ] || null;
          }
          this.checkRowMerge(
            [this.parent.viewport.topIndex + frozenRow, indexes[1]],
            args.indexes,
            (firstRow || { cells: [] }).cells[
              indexes[1] < frozenCol ? count + 1 : count
            ],
            getCell(
              this.parent.viewport.topIndex + frozenRow,
              indexes[1],
              sheet
            ) || {}
          );
        ...
        if (frozenCol) {
          hRow = (rFrag.lastElementChild as HTMLElement) || hRow;
        }
        cell = this.cellRenderer.render(<CellRenderArgs>{
          colIdx: indexes[1],
          rowIdx: indexes[2],
          ...
          pHRow: hRow.previousSibling,
          checkNextBorder:
            args.direction === 'last' && indexes[2] === args.indexes[2]
              ? 'Row'
              : '',
          pRow: row.previousSibling,
          isHeightCheckNeeded:
            args.direction === 'first' || args.direction === '',
          hRow: hRow,
          first:
            args.direction === 'last' &&
            !args.skipUpdateOnFirst &&
        ...
        if (args.direction === 'last' && tBody.rows.length) {
          this.checkRowMerge(
            ...
            cell,
            value,
            (indexes[1] < frozenCol ? rTBody : tBody).rows[0].cells[
              indexes[1] < frozenCol ? count + 1 : count
            ]
          );
        }
  ...
  private checkRowMerge(
    ...
    if (
      ...
      if (indexes[0] === this.parent.viewport.topIndex + frozenRow) {
        ...
        if (firstcell) {
          this.refreshFirstCell(
            indexes[0] + (range[2] - range[0]) + 1,
            indexes[1],
            firstcell
          );
        }
        ...
        if (
          indexes[0] + model.rowSpan - 1 >= prevTopIdx &&
          indexes[0] < prevTopIdx
        ) {
          this.refreshPrevMerge(
            prevTopIdx,
            indexes[1],
            this.parent.viewport.topIndex + frozenRow
          );
  ...
  private refreshPrevMerge(
    ...
    const td: HTMLTableCellElement = this.parent.getCell(
      prevTopIdx,
      colIndex,
      this.parent.getRow(currTopIdx ? currTopIdx : 0, null, colIndex)
    ...
    if (
      ...
      if (indexes[1] === this.parent.viewport.leftIndex + frozenCol) {
        if (model.colSpan < 0) {
          ...
          if (this.parent.viewport.leftIndex + frozenCol >= range[3]) {
            const td: HTMLTableCellElement = this.parent.getCell(
              indexes[0],
              indexes[3] + 1,
              this.parent.getRow(indexes[0], null, indexes[3] + 1)
            ) as HTMLTableCellElement;
            if (td) {
        ...
        if (firstcell) {
          this.refreshFirstCell(
            indexes[0],
            indexes[1] + (range[3] - range[1]) + 1,
            firstcell
          );
        }
      } else if (model.colSpan > 1) {
        if (
          indexes[1] + model.colSpan - 1 >= range[3] + 1 &&
          indexes[1] < range[3] + 1
        ) {
          const td: HTMLTableCellElement = this.parent.getCell(
            indexes[0],
  ...
  public toggleGridlines(): void {
    const sheetElem: Element = document.getElementById(
      this.parent.element.id + '_sheet'
    );
  ...
  public showHideHeaders(): void {
    ...
    getUpdateUsingRaf((): void => {
      if (sheet.showHeaders) {
        ...
        document
          .getElementById(this.parent.element.id + '_sheet')
          .classList.remove('e-hide-headers');
        this.getColHeaderPanel().scrollLeft = content.scrollLeft;
        this.parent.selectRange(sheet.selectedRange);
      } else {
        this.updateHideHeaders(
          sheet,
          document.getElementById(this.parent.element.id + '_sheet')
        );
        this.setPanelHeight(sheet);
        if (
          ...
          this.getContentPanel().style[
            this.parent.enableRtl ? 'right' : 'left'
          ] = '';
        }
        this.getScrollElement().style.left =
          this.getRowHeaderWidth(sheet) + 'px';
      }
    });
  }
  ...
  public getColHeaderHeight(sheet: SheetModel, skipHeader?: boolean): number {
    ...
    return (
      (sheet.showHeaders && !skipHeader ? getDPRValue(31) : 0) +
  ...
  public getSelectAllContent(): HTMLElement {
    return this.headerPanel.getElementsByClassName(
  ...
  public getScrollElement(): HTMLElement {
    ...
    if (this.contentPanel) {
      elem =
        this.contentPanel.parentElement || this.contentPanel.nextElementSibling;
    }
  ...
  public getRowHeaderTable(): HTMLTableElement {
    return this.contentPanel.getElementsByClassName(
      'e-rowhdr-table'
    )[0] as HTMLTableElement;
  }
