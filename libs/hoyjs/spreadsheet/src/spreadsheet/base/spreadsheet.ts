...
import { PredicateModel, fltrPrevent } from '@hoyui/grids';
...
export class Spreadsheet extends Workbook implements INotifyPropertyChanged {
  ...
  public actionComplete: EmitType<
    SortEventArgs | CellSaveEventArgs | SaveCompleteEventArgs | Object
  >;
  /* eslint-enable */
  /**
  ...
  public getRow(
    ...
    if (!table) {
      ...
      if (
        isNullOrUndefined(colIdx) ||
        (index > frozenRow - 1 && colIdx > frozenCol - 1)
      ) {
        table = this.getContentTable();
      } else {
        table =
          index < frozenRow && colIdx < frozenCol
            ? this.sheetModule.getSelectAllTable()
            : index < frozenRow
            ? this.getColHeaderTable()
            : this.getRowHeaderTable();
      }
    }
  ...
  public hiddenCount(
    ...
    endIndex: number,
    layout: string = 'rows',
    sheet: SheetModel = this.getActiveSheet()
  ): number {
    let count: number = 0;
    let rowColModel: { hidden?: boolean };
    ...
    if (isCol) {
      if (frozenCol) {
        ...
        if (index < frozenCol) {
          ...
          index -= this.hiddenCount(
            this.viewport.leftIndex + frozenCol,
            index,
            'columns'
          );
  ...
  private initServices(): void {
    this.serviceLocator.register(
      locale,
      new L10n(this.getModuleName(), defaultLocale, this.locale)
    );
    ...
    if (this.created && !this.refreshing) {
      if ((this.created as { observers?: object[] }).observers) {
        if ((this.created as { observers?: object[] }).observers.length > 0) {
          let observerObject: Object = {
            observers: (this.created as { observers?: object }).observers,
          };
          if (this.isAngular) {
            observerObject = {
              observers: (this.created as { observers?: object }).observers,
              currentObservers: (this.created as { observers?: object })
                .observers,
            };
  ...
  private renderSpreadsheet(): void {
    ...
    if (
      this.cssClass &&
      this.cssClass.indexOf('e-mobile-view') === -1 &&
      this.isMobileView()
    ) {
  ...
  public find(args: FindOptions): void | string {
    ...
    if (!args.isAction) {
      if (activeCell !== this.getActiveSheet().activeCell) {
        return (
          this.getActiveSheet().name + '!' + this.getActiveSheet().activeCell
        );
      } else {
        return null;
      }
  ...
  public replace(args: FindOptions): void {
    args = {
      ...
      sheetIndex: isUndefined(args.sheetIndex)
  ...
  public findAll(
    ...
    sheetIndex =
      sheetIndex < this.sheets.length ? sheetIndex : this.activeSheetIndex;
  ...
  public goTo(address: string): void {
    if (address.includes('!')) {
      ...
      if (idx !== this.activeSheetIndex) {
        ...
        if (sheet.frozenColumns || sheet.frozenRows) {
          ...
          if (
            !(
              (sheet.frozenRows && cellIndex[0] < topLeftCell[0]) ||
              (sheet.frozenColumns && cellIndex[1] < topLeftCell[1])
            )
          ) {
            const frozenRow: number = this.frozenRowCount(sheet);
            const frozenCol: number = this.frozenColCount(sheet);
            const curCell: number[] = [];
            const paneCell: number[] = [];
            const paneTopLeftCell: number[] = getCellIndexes(
            ...
            this.setSheetPropertyOnMute(
              sheet,
              'topLeftCell',
              getCellAddress(curCell[0], curCell[1])
            );
            this.setSheetPropertyOnMute(
              sheet,
              'paneTopLeftCell',
              getCellAddress(paneCell[0], paneCell[1])
            );
          }
        } else {
    ...
    const indexes: number[] = getRangeIndexes(address);
    const sheet: SheetModel = this.getActiveSheet();
    const frozenRow: number = this.frozenRowCount(sheet);
    const frozenCol: number = this.frozenColCount(sheet);
    ...
    if (insideDomCount) {
      ...
      let viewportSize: number =
        ...
        this.getScrollElement().parentElement.getBoundingClientRect().height;
      let threshold: number = 0;
      let lastRowIdx: number = 0;
      if (frozenRow) {
      ...
      if (lastRowIdx === 0) {
        for (
          let i: number = viewportIndexes[0];
          i <= this.viewport.bottomIndex;
          i++
        ) {
          threshold += getRowHeight(sheet, i);
          if (threshold > viewportSize) {
      ...
      if (lastColIdx === 0) {
        for (
          let i: number = viewportIndexes[1];
          i <= this.viewport.rightIndex;
          i++
        ) {
          threshold += getColumnWidth(sheet, i);
          if (threshold > viewportSize) {
      ...
      if (
        indexes[0] >= viewportIndexes[0] &&
        indexes[0] < viewportIndexes[2] &&
        indexes[1] >= viewportIndexes[1] &&
        indexes[1] < viewportIndexes[3]
      ) {
        return;
      }
      if (frozenRow || frozenCol) {
        viewportIndexes = [].concat(getCellIndexes(sheet.topLeftCell), [
          frozenRow,
          viewportIndexes[3],
        ]);
        if (
    ...
    let vTrack: HTMLElement;
    let cVTrack: HTMLElement;
    let offset: number;
    let vWidth: number;
    let vHeight: number;
    ...
    if (indexes[0] === frozenRow) {
      ...
      if (this.scrollSettings.enableVirtualization) {
        scrollableSize =
          offset + this.getContentTable().getBoundingClientRect().height;
        vHeight = parseFloat(
          (content.querySelector('.e-virtualtrack') as HTMLElement).style.height
        );
    ...
    if (indexes[1] === frozenCol) {
      ...
      if (this.scrollSettings.enableVirtualization) {
        scrollableSize =
          offset + this.getContentTable().getBoundingClientRect().width;
        vWidth = parseFloat(
          (content.querySelector('.e-virtualtrack') as HTMLElement).style.width
        );
        if (scrollableSize > vWidth) {
          scrollableSize += 10;
          vTrack = content.querySelector('.e-virtualtrack') as HTMLElement;
          vTrack.style.width = `${scrollableSize}px`;
          cVTrack = this.getColumnHeaderContent().querySelector(
            '.e-virtualtrack'
          ) as HTMLElement;
          cVTrack.style.width = `${scrollableSize}px`;
          vTrack = this.getMainContent().querySelector(
            '.e-virtualtrack'
          ) as HTMLElement;
          vTrack.style.width = `${scrollableSize}px`;
          getUpdateUsingRaf((): void => {
  ...
  public insideViewport(rowIndex: number, colIndex: number): boolean {
    ...
    if (sheet.frozenRows || sheet.frozenColumns) {
      ...
      return (
        ((rowIndex >= indexes[0] && rowIndex < frozenRow) ||
          (rowIndex >= this.viewport.topIndex + frozenRow &&
            rowIndex <= this.viewport.bottomIndex)) &&
        ((colIndex >= indexes[1] && colIndex < frozenCol) ||
          (colIndex >= this.viewport.leftIndex + frozenCol &&
            colIndex <= this.viewport.rightIndex))
      );
    } else {
      return (
        rowIndex >= this.viewport.topIndex &&
        rowIndex <= this.viewport.bottomIndex &&
        colIndex >= this.viewport.leftIndex &&
        colIndex <= this.viewport.rightIndex
      );
    }
  }
  ...
  public cut(address?: string): Promise<Object> {
    const promise: Promise<Object> = new Promise((resolve: Function) => {
      resolve(
        (() => {
          /** */
        })()
    ...
    this.notify(
      ...
      address
        ? {
            ...
            sId: this.sheets[
              getSheetIndex(this as Workbook, getSheetNameFromAddress(address))
            ]
              ? this.sheets[
                  getSheetIndex(
                    this as Workbook,
   ...
   */
  public setColWidth(
    width: number | string = 64,
    colIndex: number = 0,
    sheetIndex?: number
  ): void {
    const sheet: SheetModel = isNullOrUndefined(sheetIndex)
      ? this.getActiveSheet()
      : this.sheets[sheetIndex as number];
    if (sheet) {
      const mIndex: number = colIndex;
      const colWidth: string = typeof width === 'number' ? width + 'px' : width;
      colIndex = isNullOrUndefined(colIndex)
        ? getCellIndexes(sheet.activeCell)[1]
        : colIndex;
      const setColModel: Function = (): void => {
        getColumn(sheet, mIndex).width =
          parseInt(colWidth, 10) > 0 ? parseInt(colWidth, 10) : 0;
      ...
      if (sheet.id === this.getActiveSheet().id) {
        if (
          ...
          let threshold: number =
            getDPRValue(parseInt(colWidth, 10)) - eleWidth;
  ...
  public setRowHeight(
    ...
    if (sheet) {
      ...
      rowIndex = isNullOrUndefined(rowIndex)
        ? getCellIndexes(sheet.activeCell)[0]
        : rowIndex;
      if (
        skipCustomRow &&
        sheet.rows[rowIndex as number] &&
        sheet.rows[rowIndex as number].customHeight
      ) {
        return;
      }
      const rowHeight: string =
        typeof height === 'number' ? height + 'px' : height;
      const setRowModel: Function = (): void => {
        setRowHeight(
          sheet,
          mIndex,
          parseInt(rowHeight, 10) > 0 ? parseInt(rowHeight, 10) : 0
        );
      ...
      if (sheet.id === this.getActiveSheet().id) {
        ...
        if (
          ...
          let threshold: number =
            getDPRValue(parseInt(rowHeight, 10)) - eleHeight;
  ...
  public setRowsHeight(
    ...
    this.setSize(
      height,
      ranges,
      (idx: string) => Number(idx) - 1,
      this.setRowHeight.bind(this),
      skipCustomRows
    );
  }
  ...
  public setColumnsWidth(width: number = 64, ranges?: string[]): void {
    if (!ranges) {
      ranges = [
        `A:${getColumnHeaderText(
          this.getActiveSheet().usedRange.colIndex + 1
        )}`,
      ];
    }
    this.setSize(
      width,
      ranges,
      (headerText: string) => getColIndex(headerText),
  ...
  public autoFit(range: string): void {
    const values: { startIdx: number; endIdx: number; isCol: boolean } =
      this.getIndexes(range);
   ...
   *
   */
  public getIndexes(range: string): {
    startIdx: number;
    endIdx: number;
    isCol: boolean;
  } {
    ...
    if (!isNullOrUndefined(start)) {
      const startValues: { address: string; isCol: boolean } =
        this.getAddress(start);
      start = startValues.address;
      startIsCol = startValues.isCol;
    }
    if (!isNullOrUndefined(end)) {
      const endValues: { address: string; isCol: boolean } =
        this.getAddress(end);
      end = endValues.address;
      endIsCol = endValues.isCol;
    }
    const isCol: boolean =
      startIsCol === true && endIsCol === true ? true : false;
    const startIdx: number = isCol
      ? getColIndex(start.toUpperCase())
      : parseInt(start, 10);
    const endIdx: number = isCol
      ? getColIndex(end.toUpperCase())
      : parseInt(end, 10);
  ...
  private getAddress(address: string): { address: string; isCol: boolean } {
    ...
    if (address.substring(0, 1).match(/\D/g)) {
      ...
      address.substring(0, 1).match(/[0-9]/g) &&
      address.match(/\D/g)
    ) {
  ...
  public insertHyperlink(
    ...
    if (this.allowHyperlink) {
      ...
      const befArgs: BeforeHyperlinkArgs = {
        hyperlink: hyperlink,
        address: address,
        displayText: displayText,
        cancel: false,
      };
      const aftArgs: AfterHyperlinkArgs = {
        hyperlink: hyperlink,
        address: address,
        displayText: displayText,
      };
      ...
      if (!befArgs.cancel) {
        ...
        if (sheet === this.getActiveSheet()) {
          this.serviceLocator
            ...
            .refreshRange(cellIdx, false, false, false, true, isImported(this));
          for (let i: number = 0; i < classList.length; i++) {
            if (
              !this.getCell(cellIdx[0], cellIdx[1]).classList.contains(
                classList[i as number]
              )
            ) {
              this.getCell(cellIdx[0], cellIdx[1]).classList.add(
                classList[i as number]
              );
            }
          }
  ...
  public setPanelSize(): void {
    if (this.height !== 'auto') {
      const panel: HTMLElement = document.getElementById(
        this.element.id + '_sheet_panel'
      );
      panel.style.height = `${
        this.element.getBoundingClientRect().height - getSiblingsHeight(panel)
      }px`;
    }
  }
  ...
  public getThreshold(layout: 'row' | 'col'): number {
    const threshold: number = Math.round(
      (this.viewport[layout + 'Count'] + 1) / 2
    );
  ...
  public isMobileView(): boolean {
    return (
      !isNullOrUndefined(this.cssClass) &&
      (this.cssClass.indexOf('e-mobile-view') > -1 || Browser.isDevice) &&
      this.cssClass.indexOf('e-desktop-view') === -1 &&
      false
    );
  }
  ...
  public getValueRowCol(
    ...
    rowIndex: number,
    colIndex: number,
    formulaCellReference?: string,
    refresh?: boolean,
    isUnique?: boolean,
    isSubtotal?: boolean
  ): string | number {
    return super.getValueRowCol(
      sheetId,
      rowIndex,
      colIndex,
  ...
  public sort(
    ...
    if (this.sortCollection) {
      for (let i: number = this.sortCollection.length - 1; i >= 0; i--) {
        if (
          this.sortCollection[i as number] &&
          this.sortCollection[i as number].sheetIndex === this.activeSheetIndex
        ) {
    ...
    return super
      .sort(sortOptions, range, prevSort)
      .then((args: SortEventArgs) => {
  ...
  public refreshNode(td: Element, args?: NumberFormatArgs): void {
    ...
    if (td) {
      if (args) {
        args.result = isNullOrUndefined(args.formattedText)
          ? isNullOrUndefined(args.result)
            ? ''
            : args.result
        ...
        const spanElem: Element = select(
          '#' + this.element.id + '_currency',
          td
        );
        if (spanElem) {
          detach(spanElem);
        ...
        if (
          args.type === 'Accounting' &&
          isNumber(args.value) &&
          args.result.includes(args.curSymbol)
        ) {
          ...
          if (args.result.trim().endsWith(args.curSymbol)) {
            ...
            curSymbol = args.result.includes(' ' + args.curSymbol)
              ? ' ' + args.curSymbol
              : args.curSymbol;
          ...
          if (
            ...
            td.children[td.childElementCount - 1].className.indexOf(
              'e-addNoteIndicator'
            ) > -1
          ) {
            nodeElement = document.getElementsByClassName(
              'e-addNoteIndicator'
            )[0] as HTMLElement;
          }
          if (dataBarVal) {
          ...
          if (curSymbol) {
            const curr: HTMLElement = this.createElement('span', {
          ...
          if (
            args.result &&
            (args.result.toLowerCase() === 'true' ||
              args.result.toLowerCase() === 'false')
          ) {
      ...
      if (
        ...
        !isNullOrUndefined(args.colIndex)
      ) {
        attributes(td, {
          'aria-label':
            (value ? value + ' ' : '') +
            getCellAddress(args.rowIndex, args.colIndex),
        });
      }
      ...
      if (
        (this.isAngular || this.isVue) &&
        td.classList.contains('e-cell-template') &&
        node &&
        ...
        if (!args.isRowFill) {
          if (
            !isNullOrUndefined((node as HTMLElement).className) &&
  ...
  public skipHidden(
    ...
    endIdx: number,
    layout: string = 'rows',
    finite: boolean = this.scrollSettings.isFinite
  ): number[] {
    const sheet: SheetModel = this.getActiveSheet();
    ...
    for (let i: number = startIdx; i <= endIdx; i++) {
      if (
        sheet[`${layout}`][i as number] &&
        sheet[`${layout}`][i as number].hidden
      ) {
        if (startIdx === i) {
          startIdx++;
        }
   ...
   */
  public updateActiveBorder(
    nextTab: HTMLElement,
    selector: string = '.e-ribbon'
  ): void {
    const indicator: HTMLElement = select(
      `${selector} .e-tab-header .e-indicator`,
      this.element
    ) as HTMLElement;
    ...
    setStyleAttribute(indicator, {
      ...
      right:
        ...
        (nextTab.offsetLeft + nextTab.offsetWidth) +
        'px',
    });
  ...
  private keyDownHandler(e: KeyboardEvent): void {
    const findToolDlg: HTMLElement = closest(
      e.target as Element,
      '.e-findtool-dlg'
    ) as HTMLElement;
    if (findToolDlg) {
      if (e.keyCode === 9) {
        ...
        if (e.shiftKey) {
          if (target.classList.contains('e-text-findNext-short')) {
            const focusEle: HTMLElement = findToolDlg.querySelector(
              '.e-findRib-close .e-tbar-btn'
            );
            if (focusEle) {
              e.preventDefault();
              focusEle.focus();
          ...
          target.classList.contains('e-tbar-btn') &&
          target.parentElement.classList.contains('e-findRib-close')
        ) {
          focus(findToolDlg);
        }
      }
      ...
      if (
        ...
        document.activeElement.classList.contains('e-cell')
      ) {
  ...
  public enableToolbarItems(
    ...
    this.notify(enableToolbarItems, [
      { tab: tab, items: items, enable: enable === undefined ? true : enable },
    ]);
  }
  ...
  private selectOverlay(id?: string, isChart?: boolean): void {
    ...
    if (id) {
      ...
      overlayEle = this.element.querySelector(
        `.e-ss-overlay${
          isChart
            ? '.e-datavisualization-chart'
            : ':not(.e-datavisualization-chart)'
        }`
      );
    }
    if (overlayEle) {
      let isChartActive: boolean;
      const activeOverlay: Element = this.element.getElementsByClassName(
        'e-ss-overlay-active'
      )[0];
      if (activeOverlay) {
        activeOverlay.classList.remove('e-ss-overlay-active');
        isChartActive = activeOverlay.classList.contains(
          'e-datavisualization-chart'
        );
        if (isChartActive) {
  ...
  public onPropertyChanged(
    newProp: SpreadsheetModel,
    oldProp: SpreadsheetModel
  ): void {
      ...
      switch (prop) {
        case 'enableRtl':
          ...
          if (newProp.enableRtl) {
            ...
            document
              .getElementById(this.element.id + '_sheet_panel')
              .classList.add('e-rtl');
          } else {
            header.style.marginLeft = '';
            document
              .getElementById(this.element.id + '_sheet_panel')
              .classList.remove('e-rtl');
          }
          if (this.allowScrolling) {
            this.scrollModule.setPadding(true);
          }
          if (this.allowAutoFill) {
            const autofillEle: HTMLElement = this.element.querySelector(
              '.e-dragfill-ddb'
            ) as HTMLElement;
            if (autofillEle) {
              const autofillDdb: { enableRtl: boolean; dataBind: Function } =
                getComponent(autofillEle, 'dropdown-btn');
              if (autofillDdb) {
          ...
          this.sheetModule.setPanelWidth(
            sheet,
            this.getRowHeaderContent(),
            true
          );
          if (this.allowImage || this.allowChart) {
            const overlays: HTMLCollectionOf<Element> =
              this.element.getElementsByClassName('e-ss-overlay');
            let chart: { destroy: Function };
            let overlay: HTMLElement;
            let chartEle: HTMLElement;
            for (
              let idx: number = 0, overlayLen: number = overlays.length - 1;
              idx <= overlayLen;
              idx++
            ) {
        ...
        case 'allowEditing':
          ...
          break;
        case 'allowInsert':
          addBtn = this.element.getElementsByClassName(
            'e-add-sheet-tab'
        ...
        case 'sheets':
          ...
          Object.keys(newProp.sheets).forEach(
            (sheetIndex: string, index: number) => {
              ...
              if (sheet.ranges && Object.keys(sheet.ranges).length) {
                ...
                const pIdx: number[] = getCellIndexes(
                  oldProp.sheets[`${sheetIdx}`].paneTopLeftCell
                );
                if (cIdx[0] !== pIdx[0]) {
                  const frozenRow: number = this.frozenRowCount(
                    this.getActiveSheet()
                  );
                  const top: number =
                    cIdx[0] > frozenRow
                ...
                if (cIdx[1] !== pIdx[1]) {
                  const frozenCol: number = this.frozenColCount(
                    this.getActiveSheet()
                  );
                  const left: number =
                    cIdx[1] > frozenCol
  ...
  public applyFilter(
    ...
    const sheetIdx: number = range
      ? getSheetIndexFromAddress(this, range)
      : this.activeSheetIndex;
    const filterArgs: {
  ...
  public setRangeReadOnly(
    ...
    this.notify('actionBegin', {
      action: 'readonly',
      eventArgs: { readOnly, range, sheetIdx },
    });
    if (
      this.isColumnRange(range) ||
      (indexes[0] === 0 && indexes[2] === sheet.rowCount - 1)
    ) {
      ...
      this.isRowRange(range) ||
      (indexes[1] === 0 && indexes[3] === sheet.colCount - 1)
    ) {
    ...
    for (
      ...
      for (
        ...
        const cell: HTMLElement = this.getCell(
          rowIndex as number,
          colIndex as number
        );
