...
export function setValue(nameSpace: string, value: any, obj: any): any {
  const keyValues: string[] = nameSpace
    .replace(/\[/g, '.')
    .replace(/\]/g, '')
    .split('.');
  const start: any = obj || {};
...
export class ModuleLoader {
  ...
  public inject(
    ...
    for (let i: number = 0; i < reqLengthVal; i++) {
      ...
      for (const module of moduleList) {
        ...
        if (
          module.prototype.getModuleName() === modl.member &&
          !this.isModuleLoaded(modName)
        ) {
  ...
  private clearUnusedModule(moduleListName: ModuleDeclaration[]): void {
    const usedModule: string[] = moduleListName.map(
      (arg: ModuleDeclaration) => {
        return this.getMemberName(arg.member);
      }
    );
    const removeModule: ModuleDeclaration[] = this.loadedModules.filter(
