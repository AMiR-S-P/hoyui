...
import { isNullOrUndefined, getValue, Internationalization } from '@hoyui/base';
import { DataUtil } from '@hoyui/data';
import {
  DateFormatCheckArgs,
  checkDateFormat,
  dateToInt,
...
export class BasicFormulas {
  ...
  public formulas: IBasicFormula[] = [
    ...
    },
    {
      ...
      category: 'Math & Trig',
      description:
        'Sums the cells specified by a given set of conditionsor criteria.',
    },
    {
    ...
    },
    {
      ...
      description:
        'The sum of the numbers divided by how many numbers are being averaged.',
    },
    {
      formulaName: 'AVERAGEIF',
      category: 'Statistical',
      description:
        'Computes the average of the numbers in a range that meet the supplied criteria.',
    },
    {
      formulaName: 'COUNT',
      category: 'Statistical',
      description:
        'Counts the numbers in the list of arguments, exclude text entries.',
    },
    {
      formulaName: 'COUNTA',
      category: 'Statistical',
    ...
    },
    {
      ...
      category: 'Statistical',
      description:
        'Calculates the average of values in the list of arguments.Arguments can be numbers, names, arrays or references.',
    },
    {
    ...
    },
    {
      ...
      category: 'Logical',
      description:
        'Checks multiple conditions and returns a value corresponding to the first TRUE result.',
    },
    {
      formulaName: 'IFERROR',
      category: 'Logical',
    ...
    },
    {
      ...
      category: 'Logical',
      description:
        'Returns TRUE if any arguments considered TRUE, and all the arguments are FALSE it will return FALSE.',
    },
    {
      formulaName: 'NOT',
      category: 'Logical',
    ...
    },
    {
      ...
      category: 'Lookup & Reference',
      description:
        'Returns the relative position of an checked item in range that matches a specified value in a specified order',
    },
    {
    ...
    },
    {
      ...
      category: 'Statistical',
      description:
        'Returns the square of the Pearson product moment correlation coefficient based on data points in known_ys and known_xs',
    },
    {
      formulaName: 'UNIQUE',
      category: 'Lookup & Reference',
    ...
    },
    {
      ...
      description:
        'Returns TRUE, if the argument is number and FALSE otherwise.',
    },
    {
      formulaName: 'ROUND',
      category: 'Math & Trig',
    ...
    },
    {
      ...
      category: 'Lookup & Reference',
      description:
        'Looks for a value in a one-row or one-column range, then returns a value from the same position in a second one-row or one-column range.',
    },
    {
      formulaName: 'VLOOKUP',
      category: 'Lookup & Reference',
      description:
        'Looks for a specific value in the first column of a lookup range and returns a corresponding value from a different column within the same row.',
    },
    {
      formulaName: 'HLOOKUP',
      category: 'Lookup & Reference',
      description:
        'Looks for a value in the top row of the array of values and then returns a value in the same column from a row in the array that you specify.',
    },
    {
      formulaName: 'T',
      category: 'Text',
      description:
        'Checks whether a value is text or not and returns the text.',
    },
    {
      formulaName: 'EXACT',
      category: 'Text',
    ...
    },
    {
      ...
      category: 'Date & Time',
      description:
        'Converts hours, minutes, seconds to the time formatted text.',
    },
    {
      formulaName: 'CHAR',
      category: 'Text',
    ...
    },
    {
      ...
      category: 'Text',
      description:
        'Returns the numeric code for the first character in a given string.',
    },
    {
      formulaName: 'DOLLAR',
      category: 'Text',
    ...
    },
    {
      ...
      category: 'Date & Time',
      description:
        'Returns the last day of the month that is a specified number of months before or after an initially supplied start date.',
    },
    {
  ...
  private init(): void {
    ...
    for (let i: number = 0; i < this.formulas.length; i++) {
      fn = getValue(
        'Compute' + this.formulas[i as number].formulaName,
        this
      ).bind(this);
      this.addFormulaCollection(
        this.formulas[i as number].formulaName.toUpperCase(),
        fn,
        this.formulas[i as number].category,
  ...
  private addFormulaCollection(
    ...
    this.parent.libraryFormulas = {
      fName: formulaName,
      handler: functionName as Function,
  ...
  public ComputeSUM(...args: string[]): string | number {
    ...
    if (isNullOrUndefined(args) || (args.length === 1 && args[0] === '')) {
      return this.parent.formulaErrorStrings[
        FormulasErrorsStrings.InvalidArguments
      ];
    }
    let sum: number = 0;
    let val: string;
    ...
    if (!isNullOrUndefined(args)) {
      ...
      for (let i: number = 0; i < argArr.length; i++) {
        ...
        if (
          argValue.indexOf(':') > -1 &&
          this.parent.isCellReference(argValue)
        ) {
          const cellCollection: string[] | string =
            this.parent.getCellCollection(
              argValue.split(this.parent.tic).join('')
            );
          for (let j: number = 0; j < cellCollection.length; j++) {
            val = !isSubtotalFormula
              ? this.parent.getValueFromArg(cellCollection[j as number])
              : this.parent.getValueFromArg(
                  cellCollection[j as number],
                  null,
                  null,
                  true
                );
            ...
            if (
              isNullOrUndefined(val[0]) ||
              isNaN(this.parent.parseFloat(val))
            ) {
              continue;
            }
          ...
          if (argArr[i as number].indexOf(this.parent.tic) > -1) {
            if (
              isNaN(
                this.parent.parseFloat(
                  argArr[i as number].split(this.parent.tic).join('')
                )
              ) ||
          ...
          if (
            argArr[i as number].split(this.parent.tic).join('') ===
            this.parent.trueValue
          ) {
            argArr[i as number] = '1';
          }
          if (
            argArr[i as number].split(this.parent.tic).join('') ===
            this.parent.falseValue
          ) {
            argArr[i as number] = '0';
          }
          orgValue = !isSubtotalFormula
            ? this.parent.getValueFromArg(
                argArr[i as number].split(this.parent.tic).join('')
              )
            : this.parent.getValueFromArg(
          ...
          if (
            isNullOrUndefined(orgValue) ||
            isNaN(this.parent.parseFloat(orgValue))
          ) {
            continue;
          }
    ...
    return sum.toString().indexOf('.') > -1
      ? sum.toFixed(maxDecimalLength)
      : sum;
  }
  ...
  public ComputeINT(...args: string[]): string | number {
    ...
    if (args[0] === '' && args.length === 1) {
      ...
      return this.parent.formulaErrorStrings[
        FormulasErrorsStrings.WrongNumberArguments
      ];
    }
  ...
  public ComputeTODAY(...args: string[]): Date | string {
    ...
    if (args.length !== 1 || args[0] !== '') {
      ...
      if (
        (
          this.parent.parentObject as { getModuleName: Function }
        ).getModuleName() === 'spreadsheet'
      ) {
        str = (this.parent.parentObject as { dateToInt: Function }).dateToInt(
          dt
        );
        if (
          (
            this.parent.parser as unknown as { storedStringText: string }
          ).storedStringText
            .toUpperCase()
            .indexOf('TODAY') === 0 &&
          this.parent.cell !== ''
          ...
          (
            this.parent.parentObject as { setDateFormat: Function }
          ).setDateFormat(
            (this.parent as unknown as { getSheetId: Function }).getSheetId(
              this.parent.grid
            ),
            this.parent.rowIndex(this.parent.cell) - 1,
            this.parent.colIndex(this.parent.cell) - 1
          );
        }
      } else {
        str =
          dt.getFullYear() +
          '/' +
          this.parent.calculateDate((dt.getMonth() + 1).toString()) +
          '/' +
  ...
  public ComputeWEEKDAY(...args: string[]): number | string {
    ...
    if (
      (args[0].indexOf(this.parent.tic) > -1 &&
        args[0].split(this.parent.tic).join('').trim() === '') ||
      args[1].split(this.parent.tic).join('').trim() === '' ||
      (args[1].indexOf(this.parent.tic) > -1 &&
        isNaN(this.parent.parseFloat(args[1].split(this.parent.tic).join(''))))
    ) {
      return this.parent.getErrorStrings()[CommonErrors.Value];
    }
    let date: string;
    ...
    if (this.parent.isCellReference(args[0])) {
      date =
        this.parent.getValueFromArg(args[0].split(this.parent.tic).join('')) ||
        '0';
      ...
      if (
        args[0].indexOf(this.parent.tic) > -1 &&
        isNaN(this.parent.parseFloat(args[0].split(this.parent.tic).join('')))
      ) {
        date = this.parent.getValueFromArg(args[0]);
      } else {
        date =
          this.parent.getValueFromArg(
            args[0].split(this.parent.tic).join('')
          ) || '0';
      }
    }
    if (this.parent.isCellReference(args[1])) {
      value =
        this.parent.getValueFromArg(args[1].split(this.parent.tic).join('')) ||
        '0';
    } else {
      value = this.parent.getValueFromArg(
        args[1].split(this.parent.tic).join('')
      );
    }
    ...
    date =
      date === this.parent.trueValue
        ? '1'
        : date === this.parent.falseValue
        ? '0'
        : date;
    value =
      value === this.parent.trueValue
        ? '1'
        : value === this.parent.falseValue
        ? '0'
        : value;
  ...
  public ComputePROPER(...args: string[]): string {
    ...
    if (args[0].indexOf(this.parent.tic) > -1) {
      ...
      !args[0].startsWith('n') &&
      str.split('%').length === 2 &&
      this.parent.isNumber(str.split('%')[0])
    ) {
      str = (Number(str.split('%')[0]) / 100).toString();
    }
    str = str
      .toLowerCase()
      .replace(/\b\w/g, function (char: string): string {
        return char.toUpperCase();
      })
      .replace(
        /(\d)([a-z])/g,
        function (match: string, number: string, char: string): string {
  ...
  public ComputeSUMPRODUCT(...args: string[]): string | number {
    ...
    for (let k: number = 0; k < ranges.length; ++k) {
      ...
      if (
        !range.startsWith(this.parent.tic) &&
        this.parent.isCellReference(range)
      ) {
        ...
        for (let row: number = startRow; row <= endRow; ++row) {
          for (let col: number = col1; col <= col2; ++col) {
            const cellRef: string =
              this.getSheetReference(range) +
              this.parent.convertAlpha(col) +
              row;
            ...
            if (!isNaN(this.parent.parseFloat(result))) {
              ...
              mulValues[i as number] =
                mulValues[i as number] * this.parent.parseFloat(result);
            } else {
  ...
  public ComputeROUNDUP(...args: string[]): string | number {
    ...
    if (len === 1 && args[0] !== '') {
      ...
      arg1 =
        arg1.toUpperCase() === 'TRUE' ? '1' : arg1 === 'FALSE' ? '0' : arg1;
      arg1 = this.parent.getValueFromArg(arg1);
      num = this.parent.parseFloat(arg1);
      if (num > 0) {
        num += 0.4999999999; // To round the number, we using this value.
      } else if (num < 0) {
        num -= 0.4999999999;
      }
      ...
      if (this.parent.isCellReference(args[0])) {
        ...
        if (
          args[0].indexOf(this.parent.tic) > -1 &&
          (args[0].split(this.parent.tic).join('') === this.parent.trueValue ||
            args[0].split(this.parent.tic).join('') === this.parent.falseValue)
        ) {
          return this.parent.getErrorStrings()[CommonErrors.Value];
        }
        arg1 = this.parent
          .getValueFromArg(args[0])
          .split(this.parent.tic)
          .join('');
      }
      ...
      if (this.parent.isCellReference(args[1])) {
        ...
        if (
          args[1].indexOf(this.parent.tic) > -1 &&
          (args[1].split(this.parent.tic).join('') === this.parent.trueValue ||
            args[1].split(this.parent.tic).join('') === this.parent.falseValue)
        ) {
          return this.parent.getErrorStrings()[CommonErrors.Value];
        }
        arg2 = this.parent
          .getValueFromArg(args[1])
          .split(this.parent.tic)
          .join('');
      }
      ...
      arg1 =
        arg1.toUpperCase() === 'TRUE' ? '1' : arg1 === 'FALSE' ? '0' : arg1;
      arg2 =
        arg2.toUpperCase() === 'TRUE' ? '1' : arg2 === 'FALSE' ? '0' : arg2;
      const isInvalidNumStr: boolean =
        isNaN(Number(arg1)) || arg1.trim() === '';
      const isInvalidDigStr: boolean =
        isNaN(Number(arg2)) || arg2.trim() === '';
      if (
        ((args[0].indexOf('"') > -1 || this.parent.isCellReference(args[0])) &&
          isInvalidNumStr) ||
        ((args[1].indexOf('"') > -1 || this.parent.isCellReference(args[1])) &&
          isInvalidDigStr)
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      }
      ...
      if (digits > 0) {
        ...
        const decimalCount: number =
          decimalArr.length === 2
            ? decimalArr[1].length >= digits
              ? digits
              : decimalArr[1].length
            : 0;
        num = this.parent.parseFloat(
          this.preciseRound(num, decimalCount, 'ROUNDUP')
        );
        str = num.toString();
        if (isNaN(num)) {
      ...
      str =
        index > -1
          ? this.parent.getErrorStrings()[CommonErrors.Value]
          : this.parent.formulaErrorStrings[
              FormulasErrorsStrings.InvalidArguments
            ];
    }
    return str;
  ...
  public ComputeROUNDDOWN(...args: string[]): string | number {
    ...
    if (len === 1 && args[0] !== '') {
      ...
      num = this.parent.parseFloat(arg1);
      if (num > 0) {
        num -= 0.4999999999; // To round the number, we are using this value.
      } else if (num < 0) {
        num += 0.4999999999;
      }
      num = this.parent.parseFloat(num.toFixed(0));
      ...
      if (digits > 0) {
        ...
        num = this.parent.parseFloat(
          this.preciseRound(num, decimalCount, 'ROUNDDOWN')
        );
        result = num.toString();
        if (isNaN(num)) {
          if (digits.toString().indexOf('"') > -1) {
  ...
  public ComputeCOUNT(...args: string[]): number | string {
    ...
    for (let i: number = 0; i < argArr.length; i++) {
      ...
      if (this.parent.isCellReference(argVal)) {
        if (argVal.indexOf(':') > -1) {
          cellColl = this.parent.getCellCollection(
            argVal.split(this.parent.tic).join('')
          );
          for (let j: number = 0; j < cellColl.length; j++) {
            cellValue = !isSubtotalFormula
              ? this.parent.getValueFromArg(cellColl[j as number])
              : this.parent.getValueFromArg(
                  cellColl[j as number],
                  null,
                  null,
                  true
                );
        ...
        value = this.parent
          .getValueFromArg(argVal)
          .split(this.parent.tic)
          .join('');
        if (argVal.length === 0 && value.trim() === '') {
          result++;
        } else if (
          !isNaN(this.parent.parseFloat(value)) ||
          argVal === this.parent.trueValue ||
          argVal === this.parent.falseValue
        ) {
          if (
            argVal.length > 0 &&
            argVal.trim() !== '' &&
  ...
  public ComputeDATE(...args: string[]): Date | string {
    ...
    argArr[0] =
      argArr[0] === this.parent.trueValue
        ? '1'
        : argArr[0] === this.parent.falseValue
        ? '0'
        : argArr[0];
    argArr[1] =
      argArr[1] === this.parent.trueValue
        ? '1'
        : argArr[1] === this.parent.falseValue
        ? '0'
        : argArr[1];
    argArr[2] =
      argArr[2] === this.parent.trueValue
        ? '1'
        : argArr[2] === this.parent.falseValue
        ? '0'
        : argArr[2];
    for (let idx: number = 0; idx < argArr.length; idx++) {
      ...
      if (this.parent.getErrorStrings().indexOf(argsValue) > -1) {
        ...
        argsValue === '""' ||
        (argsValue === '"0"' && args[idx as number] !== '"0"') ||
        argsValue === '"TRUE"' ||
        argsValue === '"FALSE"'
      ) {
    ...
    let year: number = Math.floor(
      this.parent.parseFloat(argArr[0].split(this.parent.tic).join(''))
    );
    let month: number = Math.floor(
      this.parent.parseFloat(argArr[1].split(this.parent.tic).join(''))
    );
    const day: number = Math.floor(
      this.parent.parseFloat(argArr[2].split(this.parent.tic).join(''))
    );
    let days: number = 0;
    ...
    if (date.toString() !== 'Invalid Date') {
      ...
      if (!nestedFormula) {
        return new Internationalization(
          (this.parent.parentObject as { locale?: string }).locale || 'en-US'
        ).formatDate(date, { type: 'date', skeleton: 'yMd' });
      }
    }
  ...
  public ComputeFLOOR(...args: string[]): number | string {
    ...
    for (let i: number = 0; i < argArr.length; i++) {
      const argVal: string = argArr[i as number]
        .split(this.parent.tic)
        .join('')
        .trim();
      if (
        argVal === '' ||
        (argArr[i as number].indexOf(this.parent.tic) > -1 &&
          isNaN(this.parent.parseFloat(argVal)))
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      }
  ...
  public ComputeDAY(...args: string[]): number | string {
    ...
    if (args[0].startsWith(this.parent.tic)) {
      ...
      if (
        dateVal === '' ||
        dateVal === this.parent.trueValue ||
        dateVal === this.parent.falseValue
      ) {
        return errCollection[CommonErrors.Value];
      }
    } else {
      dateVal = this.parent.getValueFromArg(
        args[0].split(this.parent.tic).join('')
      );
      if (
        this.parent.isCellReference(args[0]) &&
        dateVal.indexOf(this.parent.tic) > -1
      ) {
        return errCollection[CommonErrors.Value];
      }
    }
    ...
    result = this.parent.isNaN(Number(dateVal))
      ? this.parent.parseDate(dateVal)
      : this.parent.intToDate(dateVal);
    if (Object.prototype.toString.call(result) === '[object Date]') {
      /* eslint-disable-next-line */
      result =
        new Date(result).getFullYear() < 1900 ||
        new Date(result).getFullYear() > 9999
          ? this.parent.isNumber(dateVal)
            ? 'Num'
            : 'NaN'
          : (result as any).getDate();
    }
  ...
  public ComputeIF(...args: string[]): string | number {
    ...
    if (argArr.length > 3 || argArr.length === 1) {
      ...
      if (
        condUpper === this.parent.trueValue ||
        parsedVal < 0 ||
        parsedVal > 0
      ) {
        ...
        result =
          argArr[1] === '' ? '0' : this.parent.getValueFromArg(argArr[1]);
        ...
        result =
          argArr[2] === '' ? '0' : this.parent.getValueFromArg(argArr[2]);
      } else {
        return this.parent.formulaErrorStrings[
          FormulasErrorsStrings.Requires3Args
        ];
      }
    }
  ...
  public ComputeIFERROR(...args: string[]): number | string {
    ...
    if (this.parent.isCellReference(argArr[0])) {
      ...
      if (
        this.parent.getErrorStrings().indexOf(condition) === -1 &&
        condition !== 'NaN'
      ) {
        return condition;
      }
    ...
    if (!isNullOrUndefined(range)) {
      ...
      for (let i: number = 0; i < argArr.length; i++) {
        ...
        if (
          ...
          this.parent.isCellReference(rangevalue)
        ) {
          const cellCollection: string[] | string =
            this.parent.getCellCollection(rangevalue);
          for (let j: number = 0; j < cellCollection.length; j++) {
            val = !isSubtotalFormula
              ? this.parent.getValueFromArg(cellCollection[j as number])
              : this.parent.getValueFromArg(
          ...
          orgValue = !isSubtotalFormula
            ? this.parent.getValueFromArg(argArr[i as number])
            : this.parent.getValueFromArg(
                argArr[i as number],
                null,
                null,
          ...
          const isEmptyStr: boolean =
            orgValue.indexOf(this.parent.tic) > -1 &&
            orgValue.split(this.parent.tic).join('').trim() === '';
          if (
            isEmptyStr ||
            (argArr[i as number].indexOf(this.parent.tic) > -1 &&
              isNaN(
                this.parent.parseFloat(orgValue.split(this.parent.tic).join(''))
              ))
          ) {
            return this.parent.getErrorStrings()[CommonErrors.Value];
          }
          orgValue = this.parent.getValueFromArg(
            argArr[i as number].split(this.parent.tic).join('')
          );
          orgValue =
            orgValue === this.parent.trueValue
              ? '1'
              : orgValue === this.parent.falseValue
              ? '0'
              : orgValue.split(this.parent.tic).join('');
          if (this.parent.getErrorStrings().indexOf(orgValue) > -1) {
            return orgValue;
          }
        }
  ...
  public ComputeDAYS(...args: string[]): number | string {
    ...
    const processArgs: Function = (actuaValue: string): string => {
      ...
      if (value.indexOf(this.parent.tic) > -1) {
        ...
        if (
          value === '' ||
          this.parent.isCellReference(actuaValue) ||
          value.toUpperCase() === this.parent.trueValue ||
      ...
      if (dateCheck.isDate || dateCheck.isTime) {
        value = (
          this.parent.parseDate(value).getTime() /
          (1000 * 3600 * 24)
        ).toString();
      }
      return value;
    ...
    if (errCollection.indexOf(startDate as string) > -1) {
      return startDate;
    }
    const result: number | string =
      Math.floor(Number(endDate)) - Math.floor(Number(startDate));
    if (isNaN(result)) {
      return errCollection[CommonErrors.Value];
    }
    return result;
   ...
   */
  public ComputeUNIQUE(
    ...argArr: string[]
  ): number | string | number[] | string[] {
    let result: string;
    let isComputeExp: boolean;
    const errCollection: string[] = this.parent.getErrorStrings();
    ...
    if (
      isNullOrUndefined(argArr) ||
      (argArr[0] === '' && argArr.length === 1)
    ) {
      return this.parent.formulaErrorStrings[
        FormulasErrorsStrings.InvalidArguments
      ];
    ...
    const processArgs: Function = (actualValue: string): string => {
      ...
      if (
        ...
        value.indexOf(this.parent.tic) > -1 ||
        actualValue.indexOf(this.parent.tic) > -1
      ) {
        return errCollection[CommonErrors.Value];
      } else {
    ...
    if (argArr[0].indexOf(':') > -1) {
      if (isNullOrUndefined(argArr[0].match(/[0-9]/))) {
        ...
        argArr[0] =
          ...
          ':' +
          splitArray[1] +
          (this.parent.spreadSheetUsedRange[0] + 1);
        ...
        argArr[0] =
          ...
          splitArray[0] +
          ':' +
          getAlphalabel(this.parent.spreadSheetUsedRange[1] + 1) +
          splitArray[1];
      }
      const rangeSplit: string[] = argArr[0].split(':');
      if (
        this.parent.isCellReference(rangeSplit[0]) &&
        this.parent.isCellReference(rangeSplit[1])
      ) {
        const collection: string[] = this.parent.dependencyCollection;
        for (let i: number = 0; i < collection.length && !isComputeExp; i++) {
          if (
            collection[i as number].split(':')[0] === argArr[0].split(':')[0]
          ) {
        ...
        const j: number = argArr[0].indexOf(':');
        let swap: number;
        let rowIdx: number = this.parent.rowIndex(
          this.parent.substring(argArr[0], 0, j)
        );
        let colIdx: number = this.parent.colIndex(
          this.parent.substring(argArr[0], 0, j)
        );
        let endRowIdx: number = this.parent.rowIndex(
          this.parent.substring(argArr[0], j + 1, j + argArr[0].length - j - 1)
        );
        let endColIdx: number = this.parent.colIndex(
          this.parent.substring(argArr[0], j + 1, j + argArr[0].length - j - 1)
        );
        if (rowIdx > endRowIdx) {
          swap = endRowIdx;
        ...
        if (argArr[0].indexOf('!') === 0) {
          sheetIndex = argArr[0].substring(
            0,
            argArr[0].replace('!', '').indexOf('!') + 2
          );
        }
        argArr[0] =
          sheetIndex +
          getAlphalabel(colIdx) +
          rowIdx +
          ':' +
          getAlphalabel(endColIdx) +
          endRowIdx;
        const colDiff: number = endColIdx - colIdx;
        const cellValues: string[] = this.parent.getCellCollection(
          argArr[0]
        ) as string[];
        let actCell: string;
        let uniqueActCell: string;
        actCell = uniqueActCell = this.parent.actCell;
        if (byColumn === this.parent.falseValue) {
          if (colDiff === 0) {
            for (let i: number = 0; i < cellValues.length; i++) {
              let val: string = this.parent.getValueFromArg(
                cellValues[i as number]
              );
              val = val === '' ? '0' : val;
          ...
          let temp: string = '';
          const diff: number = colDiff + 1;
          const rowDiff: number = endRowIdx - rowIdx;
        ...
        const duplicateCollection: string[] = [];
        let tmp: string[] = [];
        const tmp2: string[] = [];
        for (let i: number = 0; i < valueCollection.length; i++) {
          if (
            uniqueCollection.indexOf(
              valueCollection[i as number].toLowerCase()
            ) === -1
          ) {
            ...
            if (
              duplicateCollection.indexOf(
                valueCollection[i as number].toLowerCase()
              ) === -1
            ) {
              duplicateCollection.push(
                valueCollection[i as number].toLowerCase()
              );
            }
        ...
        if (exactlyOne === this.parent.trueValue) {
          ...
          for (let i: number = 0; i < tmp.length; i++) {
            if (
              duplicateCollection.indexOf(tmp[i as number].toLowerCase()) === -1
            ) {
        ...
        if (isComputeExp) {
          ...
          if (colDiff !== 0) {
            ...
            tmp.forEach(function (item: string): void {
              computeExpResult = [
                ...(computeExpResult as string[]),
                ...item.split('++'),
              ];
            });
          } else {
            computeExpResult =
              byColumn === this.parent.falseValue ? tmp : tmp[0].split('++');
          }
        ...
        if (this.parent.dependencyLevel === 0) {
          ...
          if (byColumn === this.parent.falseValue) {
            for (
              let i: number = actRowIdx, diff: number = tmp.length + actRowIdx;
              i < diff;
              i++
            ) {
              const splitValue: string[] = tmp[0].split('++');
              for (
                let j: number = actColIdx,
                  diff2: number = splitValue.length + actColIdx;
                j < diff2;
                j++
              ) {
                if (
                  i === diff - 1 &&
                  j === diff2 - 1 &&
                  this.parent.uniqueRange.indexOf(
                    uniqueActCell + ':' + getAlphalabel(j) + i
                  ) === -1
                ) {
                  this.parent.uniqueRange.push(
                    uniqueActCell + ':' + getAlphalabel(j) + i
                  );
                }
            ...
            for (
              let i: number = actColIdx, diff: number = tmp.length + actColIdx;
              i < diff;
              i++
            ) {
              const splitValue: string[] = tmp[0].split('++');
              for (
                let j: number = actRowIdx,
                  diff2: number = splitValue.length + actRowIdx;
                j < diff2;
                j++
              ) {
                if (
                  ...
                  j === diff2 - 1 &&
                  this.parent.uniqueRange.indexOf(
                    this.parent.actCell + ':' + getAlphalabel(i) + j
                  ) === -1
                ) {
                  this.parent.uniqueRange.push(
                    this.parent.actCell + ':' + getAlphalabel(i) + j
          ...
          this.parent.dependencyLevel > 0 &&
          this.parent
            .getValueFromArg(getAlphalabel(actColIdx) + actRowIdx, true)
            .indexOf('#SPILL!') > -1
        ) {
          return this.parent.formulaErrorStrings[FormulasErrorsStrings.Spill];
        }
        if (byColumn === this.parent.falseValue) {
          const calcFamily: CalcSheetFamilyItem =
            this.parent.getSheetFamilyItem(this.parent.grid);
          let token: string = '';
          let cellTxt: string;
          if (
            calcFamily.sheetNameToParentObject !== null &&
            calcFamily.sheetNameToParentObject.size > 0
          ) {
          ...
          for (let i: number = 0; i < tmp.length; i++) {
            ...
            for (let j: number = 0; j < splitValue.length; j++) {
              this.setValueRefresh(
                splitValue[j as number],
                actRowIdx,
                actColIdx
              );
              if (i > 0 || j > 0) {
                this.parent.refresh(
                  token + getAlphalabel(actColIdx) + actRowIdx.toString(),
                  cellTxt
                );
              }
              if (splitValue[j + 1]) {
  ...
  private setValueRefresh(
    ...
    (this.parent.parentObject as any).setValueRowCol(
  ...
  private checkSpill(i: number, j: number): boolean {
    ...
    const value: string = this.parent.getValueFromArg(
      getAlphalabel(i) + j,
      true
    );
    const formulaAddress: string =
      '!' +
      this.parent.getSheetID(this.parent.grid) +
      '!' +
      getAlphalabel(i) +
    ...
    if (this.parent.getFormulaInfoTable().get(formulaAddress)) {
      formulaString = this.parent
        .getFormulaInfoTable()
        .get(formulaAddress).formulaText;
    }
    if (
      value &&
      (value.toUpperCase().indexOf('UNIQUE') < 0 ||
  ...
  public clearDependency(value: string): void {
    ...
    const rowIndex: number = this.parent.rowIndex(
      this.parent.substring(value, 0, j)
    );
    const colIndex: number = this.parent.colIndex(
      this.parent.substring(value, 0, j)
    );
    const eRowIdx: number = this.parent.rowIndex(
      this.parent.substring(value, j + 1, j + value.length - j - 1)
    );
    const eColIdx: number = this.parent.colIndex(
      this.parent.substring(value, j + 1, j + value.length - j - 1)
    );
    const rowDiff: number = eRowIdx - rowIndex + actRowIdx;
    const colDiff: number = eColIdx - colIndex + actColIdx;
    const formulaText: string = this.parent
      .getFormulaInfoTable()
      .get('!' + this.parent.getSheetID(this.parent.grid) + '!' + actCell)
      ? this.parent
          .getFormulaInfoTable()
          .get('!' + this.parent.getSheetID(this.parent.grid) + '!' + actCell)
          .getFormulaText()
    ...
    for (let i: number = actRowIdx; i <= rowDiff; i++) {
      for (let j: number = actColIdx; j <= colDiff; j++) {
        if (
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (this.parent.parentObject as any).setValueRowCol(
            this.parent.getSheetID(this.parent.grid) + 1,
            '',
            i,
            j
          );
  ...
  public ComputeT(...args: string[]): string {
    ...
    if (args[0].indexOf(this.parent.tic) > -1) {
      ...
      if (
        ...
        this.parent.isNumber(value.split('%')[0])
      ) {
  ...
  public ComputeHOUR(...args: string[]): number | string {
    ...
    cellVal =
      cellVal === this.parent.trueValue
        ? '1'
        : cellVal === this.parent.falseValue
        ? '0'
        : cellVal;
    let date: Date;
    if (this.parent.isNumber(cellVal)) {
      if (
        this.parent.parseFloat(cellVal) < 0 ||
        this.parent.parseFloat(cellVal) > 2958465
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Num];
      }
    ...
    if (date.toString() === 'Invalid Date') {
      const argVal: string =
        new Date(Date.now()).toLocaleDateString() + ' ' + cellVal;
  ...
  public ComputeMINUTE(...argArr: string[]): number | string {
    ...
    if (this.parent.isCellReference(argArr[0])) {
      cellVal =
        this.parent.getValueFromArg(
          argArr[0].split(this.parent.tic).join('')
        ) || '0';
      if (cellVal.indexOf(this.parent.tic) > -1) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      }
      ...
      if (
        argArr[0].indexOf(this.parent.tic) > -1 &&
        (argArr[0].split(this.parent.tic).join('') === this.parent.trueValue ||
          argArr[0].split(this.parent.tic).join('') === this.parent.falseValue)
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      }
      cellVal = this.parent
        .getValueFromArg(argArr[0])
        .split(this.parent.tic)
        .join('');
    }
    if (this.parent.getErrorStrings().indexOf(cellVal) > -1) {
  ...
  public ComputeNOW(...args: string[]): string {
    ...
    const dFormatter: Function = intl.getDateFormat({
      format: 'M/d/yyyy h:mm:ss a',
    });
    const dt: number = (
      this.parent.parentObject as { dateToInt: Function }
    ).dateToInt(dFormatter(date), true);
    return dt.toString();
  }
  ...
  public ComputeEXACT(...args: string[]): string | boolean {
    ...
    const nestedFormula: boolean =
      args.length && args[args.length - 1] === 'nestedFormulaTrue';
    const errCollection: string[] = this.parent.getErrorStrings();
    if (nestedFormula) {
      args.pop();
    }
  ...
  public ComputeMOD(...args: string[]): string | number {
    ...
    for (let i: number = 0; i < args.length; i++) {
      const argVal: string = args[i as number]
        .split(this.parent.tic)
        .join('')
        .trim();
      if (
        argVal === '' ||
        (args[i as number].indexOf(this.parent.tic) > -1 &&
          isNaN(this.parent.parseFloat(argVal)))
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      }
      if (isCellReference(args[i as number])) {
        ...
        value = this.parent.getValueFromArg(
          args[i as number].split(this.parent.tic).join('')
        );
        value =
          value === this.parent.trueValue
            ? '1'
  ...
  public ComputeODD(...args: string[]): string | number {
    ...
    if (isCellReference(args[0])) {
      ...
      if (val === this.parent.trueValue) {
        ...
        val.toUpperCase().match(/[A-Z]/) ||
        val.includes('"') ||
        !this.parent.isNumber(val)
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      }
    } else {
      val = this.parent.getValueFromArg(
        args[0].split(this.parent.tic).join('')
      );
      if (val === '#NAME?') {
        return this.parent.getErrorStrings()[CommonErrors.Name];
  ...
  public ComputeEVEN(...args: string[]): string | number {
    ...
    if (isCellReference(args[0])) {
      ...
      if (value1 === this.parent.trueValue) {
        ...
        value1.toUpperCase().match(/[A-Z]/) ||
        value1.includes('"') ||
        !this.parent.isNumber(value1)
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      }
    } else {
      value1 = this.parent.getValueFromArg(
        args[0].split(this.parent.tic).join('')
      );
      if (value1 === '#NAME?') {
        return this.parent.getErrorStrings()[CommonErrors.Name];
    ...
    if (Math.ceil(parseInt(value1, 10)) % 2 === 0) {
      if (
        parseInt(value1, 10) > 0 &&
        parseFloat(value1) > parseInt(value1, 10)
      ) {
        result = result + 2;
      } else if (
        parseInt(value1, 10) < 0 &&
  ...
  public ComputeMEDIAN(...args: string[]): string | number {
    ...
    for (let i: number = 0; i < args.length; i++) {
      if (this.parent.isCellReference(args[i as number])) {
        if (args[i as number].indexOf(':') > -1) {
          const cellCollection: string | string[] =
            this.parent.getCellCollection(args[i as number]);
          for (let a: number = 0; a < cellCollection.length; a++) {
            const cellValue: string = this.parent.getValueFromArg(
              cellCollection[a as number]
            );
          ...
          const cellVal: string = this.parent.getValueFromArg(
            args[i as number]
          );
          if (this.parent.getErrorStrings().indexOf(cellVal) > -1) {
            return cellVal;
        ...
        args[i as number] === '' ||
        (args[i as number] === this.parent.falseValue &&
          !this.parent.isCellReference(args[i as number]))
      ) {
        num.push(0);
      } else if (
        args[i as number] === this.parent.trueValue &&
        !this.parent.isCellReference(args[i as number])
      ) {
        num.push(1);
      } else if (
        args[i as number].indexOf(this.parent.tic) > -1 &&
        isNaN(parseFloat(args[i as number].split(this.parent.tic).join('')))
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      } else {
        const cValue: string = this.parent
          .getValueFromArg(args[i as number])
          .split(this.parent.tic)
          .join('');
    ...
    if (
      num.length % 2 !== 0 &&
      !isNaN(num[parseInt((num.length / 2).toString(), 10)])
    ) {
  ...
  public ComputeEDATE(...args: string[]): Date | string {
    ...
    if (this.parent.isCellReference(args[0])) {
      ...
      dValue = this.parent.getValueFromArg(
        args[0].split(this.parent.tic).join('')
      );
    }
    if (this.parent.isCellReference(args[1])) {
      ...
      mValue = this.parent.getValueFromArg(
        args[1].split(this.parent.tic).join('')
      );
    }
    ...
    if (
      isNaN(mValue) ||
      isNullOrUndefined(this.parent.isDate(date)) ||
  ...
  public ComputeEOMONTH(...args: string[]): Date | string {
    ...
    if (this.parent.isCellReference(args[1])) {
      ...
      monthValue = this.parent.getValueFromArg(
        args[1].split(this.parent.tic).join('')
      );
    }
  ...
  public ComputeDATEVALUE(...args: string[]): Date | string {
    ...
    if (
      !(
        !this.parent.isNumber(dValue) &&
        !isNullOrUndefined(this.parent.isDate(dValue))
      )
    ) {
      return errCollection[CommonErrors.Value];
    }
  ...
  public ComputeCOUNTBLANK(...args: string[]): string | number {
    ...
    if (
      ...
      const cellRange: string | string[] = this.parent.getCellCollection(
        args[0]
      );
  ...
  public ComputeFACT(...args: string[]): string | number {
    ...
    if (
      this.parent.isCellReference(args[0]) ||
      isNaN(Number(getValue.split(this.parent.tic).join('')))
    ) {
      ...
      if (
        getValue.startsWith(this.parent.tic) ||
    ...
    if (
      getValue.toUpperCase() === this.parent.trueValue ||
      getValue.toUpperCase() === this.parent.falseValue
    ) {
      return 1;
    }
  ...
  public ComputeDECIMAL(...args: string[]): string | number {
    ...
    if (isNullOrUndefined(args) || args.length !== 2) {
      ...
      if (this.parent.getErrorStrings().indexOf(val) > -1) {
        ...
        val === '' ||
        num < 0 ||
        (!isNaN(num) && !Number.isInteger(parseFloat(val)))
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Num];
        ...
        isNaN(this.parent.parseFloat(val)) &&
        this.parent.parseFloat(val1) <= 10
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Num];
      }
    ...
    return isNaN(value)
      ? this.parent.getErrorStrings()[CommonErrors.Num]
      : value;
  }
  ...
  public ComputeDEGREES(...args: string[]): string | number {
    ...
    if (this.parent.isCellReference(args[0])) {
      if (val === '' || val.indexOf(':') > -1) {
        ...
        val.indexOf(this.parent.tic) > -1 ||
        (isNaN(Number(val)) && !isNaN(parseInt(val, 10)))
      ) {
        return errCollection[CommonErrors.Value];
      }
  ...
  public ComputeADDRESS(...argArr: string[]): string | number {
    ...
    if (
      ...
      argArr[0].split(this.parent.tic).join('').trim() === '' ||
      argArr[1].split(this.parent.tic).join('').trim() === ''
    ) {
      return errCollection[CommonErrors.Value];
    }
    const processArgs: Function = (actualValue: string): string => {
      ...
      if (
        this.parent.isCellReference(actualValue) &&
        value.indexOf(this.parent.tic) > -1
      ) {
        return errCollection[CommonErrors.Value];
      }
    ...
    if (errCollection.indexOf(rowIndex as string) > 0) {
      return rowIndex;
    }
    ...
    if (errCollection.indexOf(colIndex as string) > 0) {
      return colIndex;
    }
    ...
    if (isNullOrUndefined(argArr[3]) || argArr[3].trim() === '') {
      ...
      if (
        refStyle.toUpperCase() === this.parent.trueValue ||
        Number(refStyle) > 1
      ) {
        refStyle = '1';
      } else if (
        refStyle === '' ||
        refStyle.toUpperCase() === this.parent.falseValue
      ) {
        refStyle = '0';
      }
    }
    if (refStyle === '1') {
      if (absIndex === '1') {
        value =
          '$' +
          getAlphalabel(parseInt(colIndex, 10)) +
          '$' +
          parseInt(rowIndex, 10);
      } else if (absIndex === '2') {
        value =
          getAlphalabel(parseInt(colIndex, 10)) + '$' + parseInt(rowIndex, 10);
      } else if (absIndex === '3') {
        value =
          '$' + getAlphalabel(parseInt(colIndex, 10)) + parseInt(rowIndex, 10);
      } else if (absIndex === '4') {
      ...
      if (absIndex === '1') {
        ...
        value =
          'R' + parseInt(rowIndex, 10) + 'C[' + parseInt(colIndex, 10) + ']';
      } else if (absIndex === '3') {
        value = 'R[' + parseInt(rowIndex, 10) + ']C' + parseInt(colIndex, 10);
      } else if (absIndex === '4') {
        value =
  ...
  public ComputeTIME(...args: string[]): string | number {
    ...
    if (
      ...
      args[2].indexOf(this.parent.tic) > -1 &&
      args[2].split(this.parent.tic).join('').trim() === ''
    ) {
      return this.parent.getErrorStrings()[CommonErrors.Value];
    }
    ...
    if (
      isNaN(this.parent.parseFloat(hours)) &&
      !(hours === this.parent.trueValue || hours === this.parent.falseValue)
    ) {
      return this.parent.getErrorStrings()[CommonErrors.Value];
    }
    if (this.parent.isCellReference(args[1])) {
    ...
    if (
      isNaN(this.parent.parseFloat(minutes)) &&
      !(minutes === this.parent.trueValue || minutes === this.parent.falseValue)
    ) {
      return this.parent.getErrorStrings()[CommonErrors.Value];
    }
    if (this.parent.isCellReference(args[2])) {
      seconds =
        this.parent.getValueFromArg(args[2].split(this.parent.tic).join('')) ||
        '0';
      ...
      if (
        args[2].indexOf(this.parent.tic) > -1 &&
        (args[2].split(this.parent.tic).join('') === this.parent.trueValue ||
          args[2].split(this.parent.tic).join('') === this.parent.falseValue)
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      }
      seconds =
    ...
    if (
      isNaN(this.parent.parseFloat(seconds)) &&
      !(seconds === this.parent.trueValue || seconds === this.parent.falseValue)
    ) {
      return this.parent.getErrorStrings()[CommonErrors.Value];
    }
    hours =
      hours === this.parent.trueValue
        ? '1'
        : hours === this.parent.falseValue
        ? '0'
        : hours;
    minutes =
      minutes === this.parent.trueValue
        ? '1'
        : minutes === this.parent.falseValue
        ? '0'
        : minutes;
    seconds =
      seconds === this.parent.trueValue
        ? '1'
        : seconds === this.parent.falseValue
        ? '0'
        : seconds;
  ...
  public ComputeCHAR(...args: string[]): string | number {
    ...
    if (errCollection.indexOf(val) > -1) {
      ...
      val.indexOf('"TRUE"') > -1 ||
      (this.parent.isCellReference(args[0]) && val.startsWith(this.parent.tic))
    ) {
      return errCollection[CommonErrors.Value];
    }
    val = val.split(this.parent.tic).join('');
  ...
  public ComputeDOLLAR(...args: string[]): string | number {
    ...
    let val: string = this.parent
      .getValueFromArg(args[0])
      .split(this.parent.tic)
      .join('');
    let val2: string = this.parent
      .getValueFromArg(args[1])
      .split(this.parent.tic)
      .join('');
    val =
      val === '' || val === this.parent.falseValue
        ? '0'
        : val === this.parent.trueValue
        ? '1'
        : val;
    val2 =
      val2 === '' || val2 === this.parent.falseValue
        ? '0'
        : val2 === this.parent.trueValue
        ? '1'
        : val2;
    ...
    if (
      val.toUpperCase().match(/^[-]?[0-9.]+$/) &&
      val2.toUpperCase().match(/^[-]?[0-9.]+$/)
    ) {
      const intl: Internationalization = new Internationalization();
      ...
      for (
        let decimalIdx: number = 1;
        decimalIdx <= decimalCount;
        decimalIdx++
      ) {
        decimalValue += '0';
      }
      const roundedNumber: number =
        Math.round(this.parent.parseFloat(val) / divisor) * divisor;
  ...
  public ComputeSMALL(...args: string[]): string | number {
    ...
    if (!isNullOrUndefined(args[0])) {
      ...
      if (this.parent.isCellReference(args[0])) {
        ...
        for (let i: number = 0; i < cellCollection.length; i++) {
          originalValue = this.parent.getValueFromArg(
            cellCollection[i as number]
          );
      ...
      for (let i: number = 0; i < valueCollection.length; i++) {
        if (
          valueCollection[i as number] !== '' &&
          !isNaN(this.parent.parseFloat(valueCollection[i as number]))
        ) {
      ...
      if (numArr.length === 0) {
        if (
          isNullOrUndefined(valueCollection[0]) ||
          args[0].indexOf(this.parent.tic) > -1
        ) {
          return errCollection[CommonErrors.Value];
        }
    ...
    if (!isNullOrUndefined(args[1])) {
      ...
      if (errCollection.indexOf(smallIndex) > -1) {
        ...
        return args[1].length > 0
          ? errCollection[CommonErrors.Value]
          : errCollection[CommonErrors.Num];
    ...
    const finalIndex: string | number =
      Number(smallIndex) < 1 ? 0 : Math.floor(Number(smallIndex));
    if (
      isNullOrUndefined(numArr[(finalIndex as number) - 1]) &&
      !isNaN(Number(finalIndex))
    ) {
      return errCollection[CommonErrors.Num];
    } else {
  ...
  public ComputeLARGE(...args: string[]): string | number {
    ...
    const finalIndex: string | number =
      Number(largeIndex) < 1 ? 0 : Math.ceil(Number(largeIndex));
    if (
      isNullOrUndefined(numArr[(finalIndex as number) - 1]) &&
      !isNaN(Number(finalIndex))
    ) {
  ...
  public ComputeCHOOSE(...args: string[]): string | number {
    ...
    if (this.parent.isCellReference(args[0])) {
      ...
      if (
        getIndexValue.split('%').length === 2 &&
        this.parent.isNumber(getIndexValue.split('%')[0])
      ) {
    ...
    if (
      getIndexValue < 1 ||
      isNaN(getIndexValue) ||
      isNullOrUndefined(args[getIndexValue as number])
    ) {
      return errCollection[CommonErrors.Value];
    }
    ...
    if (this.parent.isCellReference(getIndexValue)) {
      ...
      if (
        getIndexValue.indexOf(this.parent.tic) > -1 &&
        (errCollection.indexOf(getIndexValue.split(this.parent.tic).join('')) >
          -1 ||
          this.parent
            .removeTics(getIndexValue)
            .match(/^(\d*\.\d+|\d+)\s*[-*/]\s*(\d*\.\d+|\d+)$/))
      ) {
        getIndexValue = this.parent.removeTics(getIndexValue);
      }
      getIndexValue = this.parent.removeTics(
        this.parent.getValueFromArg(getIndexValue)
      );
  ...
  public ComputeSUMIF(...range: string[]): string | number {
    const argArr: string[] = range;
    if (
      (argArr[0].indexOf(':') < 0 && !this.parent.isCellReference(argArr[0])) ||
      (argArr[2] &&
        argArr[2].indexOf(':') < 0 &&
        !this.parent.isCellReference(argArr[2]))
    ) {
      return this.parent.formulaErrorStrings[
        FormulasErrorsStrings.ImproperFormula
      ];
    }
    ...
    const result: number[] | string = this.parent.computeSumIfAndAvgIf(
      range,
      false
    );
    if (
      typeof result === 'string' &&
      (this.parent.formulaErrorStrings.indexOf(result) ||
  ...
  public ComputeAVERAGEIF(...range: string[]): string | number {
    ...
    if (
      argList[0].indexOf(':') < 0 &&
      !this.parent.isCellReference(argList[0])
    ) {
      return this.parent.formulaErrorStrings[
        FormulasErrorsStrings.ImproperFormula
      ];
    }
    const resultVal: number[] | string = this.parent.computeSumIfAndAvgIf(
      range,
      true
    );
    ...
    if (
      ...
      (this.parent.formulaErrorStrings.indexOf(resultVal) ||
        this.parent.getErrorStrings().indexOf(resultVal))
    ) {
      return resultVal;
    }
    return (
      this.parent.parseFloat(resultVal[0]) /
      this.parent.parseFloat(resultVal[1])
    );
  }
    ...
    for (let i: number = 0; i < argsList.length; i++) {
      ...
      if (this.parent.isCellReference(val)) {
        if (val.indexOf(':') > -1) {
          if (this.isConcat) {
            ...
            for (let i: number = 0; i < cells.length; i++) {
              const tempString: string = this.parent.getValueFromArg(
                cells[i as number]
              );
          ...
          if (
            argsList.length === 1 &&
            argsList[0].indexOf(this.parent.tic) < 0 &&
        ...
        if (
          ...
          if (
            !(
              !this.parent.isNumber(tempStr) &&
              !isNullOrUndefined(this.parent.isDate(tempStr))
            ) ||
            val.startsWith(' n')
          ) {
            tempStr = this.parent
    ...
    const processArgs: Function = (actuaValue: string): string => {
      ...
      if (!this.parent.isCellReference(actuaValue)) {
        if (
          ...
          if (
            this.parent
              .removeTics(actuaValue)
              .match(/^(\d*\.\d+|\d+)\s*[-*/]\s*(\d*\.\d+|\d+)$/)
          ) {
            value = this.parent.getValueFromArg(
              this.parent.removeTics(actuaValue)
            );
  ...
  public ComputeINDEX(...argArr: string[]): string | number {
    ...
    const processArgs: Function = (actualValue: string): string | number => {
      ...
      let value: string | number = this.parent.getValueFromArg(
      ...
      if (
        (value as string).split('%').length === 2 &&
        this.parent.isNumber((value as string).split('%')[0])
      ) {
  ...
  private getSheetReference(range: string): string {
    return range.indexOf(this.parent.sheetToken) === 0 &&
      range.lastIndexOf(this.parent.sheetToken) >
        range.indexOf(this.parent.sheetToken)
      ? range.substring(0, range.lastIndexOf(this.parent.sheetToken) + 1)
      : '';
  }
  ...
  public ComputeIFS(...range: string[]): string | number {
    ...
    for (let i: number = 0; i < argArr.length; i++) {
      ...
      if (
        condition !== this.parent.trueValue &&
        condition !== this.parent.falseValue
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      }
      if (condition === this.parent.trueValue) {
        if (
          this.parent.isCellReference(
            argArr[i + 1].split(this.parent.tic).join('')
          ) ||
          argArr[i + 1].includes(this.parent.arithMarker)
        ) {
          ...
          result =
            argArr[i + 1] === ''
              ? '0'
              : this.parent.getValueFromArg(argArr[i + 1]);
  ...
  public ComputeCOUNTA(...args: string[]): number | string {
    ...
    for (let i: number = 0; i < argArr.length; i++) {
      if (this.parent.isCellReference(argArr[i as number])) {
        ...
        value = this.parent
          .getValueFromArg(argArr[i as number])
          .split(this.parent.tic)
          .join('');
        if (value.length > 0) {
          result++;
    ...
    for (let k: number = 0; k < argArrs.length; k++) {
      if (this.parent.isCellReference(argArrs[k as number])) {
        if (argArrs[k as number].indexOf(':') > -1) {
          cellCol = this.parent.getCellCollection(
            argArrs[k as number].split(this.parent.tic).join('')
          );
        ...
        argArrs[k as number] = this.processLogicalCellValue(
          argArrs[k as number]
        );
        value = this.parent.getValueFromArg(
          argArrs[k as number].split(this.parent.tic).join('')
  ...
  public ComputeSORT(...args: string[]): number | string {
    ...
    if (argArr[0].indexOf(':') > -1) {
      ...
      if (
        ...
        let eRowIdx: number = this.parent.rowIndex(
          this.parent.substring(argArr[0], j + 1, j + argArr[0].length - j - 1)
        );
        let eColIdx: number = this.parent.colIndex(
          this.parent.substring(argArr[0], j + 1, j + argArr[0].length - j - 1)
        );
        if (rowIdx > eRowIdx) {
          swap = eRowIdx;
        ...
        if (this.parent.isCellReference(argArr[3])) {
          ...
          if (
            argArr[3] !== this.parent.trueValue &&
          ...
          isStringVal =
            argArr[3].startsWith(this.parent.tic) &&
            argArr[3].endsWith(this.parent.tic);
          argArr[3] = this.parent.getValueFromArg(argArr[3]);
          argArr[3] = isNumber(argArr[3])
            ? Number(argArr[3]) === 0
              ? this.parent.falseValue
              : this.parent.trueValue
          ...
          if (
            ...
            return this.parent.getErrorStrings()[
              isStringVal ? CommonErrors.Value : CommonErrors.Name
            ];
          }
        }
        if (this.parent.isCellReference(argArr[2])) {
          ...
          argArr[2] =
            argArr[2] === this.parent.trueValue
              ? '1'
              : argArr[2].split(this.parent.tic).join('');
        }
        argArr[2] = isNumber(argArr[2])
          ? Math.floor(Number(argArr[2])).toString()
          : argArr[2];
        if (argArr[2] !== '1' && argArr[2] !== '-1') {
          return this.parent.getErrorStrings().indexOf(argArr[2]) > -1
        ...
        if (this.parent.isCellReference(argArr[1])) {
          ...
          argArr[1] = isNumber(argArr[1])
            ? Math.floor(Number(argArr[1])).toString()
            : argArr[1] === this.parent.trueValue
            ? '1'
            : argArr[1] === this.parent.falseValue
            ? '0'
            : argArr[1];
          if (
            !isNaN(this.parseDouble(argArr[1]))
              ? this.parseDouble(argArr[1]) < 1 ||
                (argArr[3] === this.parent.trueValue
                  ? eRowIdx - rowIdx + 1 < this.parseDouble(argArr[1])
                  : eColIdx - colIdx + 1 < this.parseDouble(argArr[1]))
              : true
          ) {
            return this.parent.getErrorStrings()[CommonErrors.Value];
          }
          ...
          isStringVal =
            argArr[1].startsWith(this.parent.tic) &&
            argArr[1].endsWith(this.parent.tic);
          argArr[1] = this.parent.getValueFromArg(argArr[1]);
          argArr[1] = isNumber(argArr[1])
            ? Math.floor(Number(argArr[1])).toString()
            : argArr[1] === this.parent.trueValue
            ...
            : argArr[1] === this.parent.falseValue
            ? '0'
            : argArr[1].split(this.parent.tic).join('');
          if (
            !isNaN(this.parseDouble(argArr[1]))
              ? this.parseDouble(argArr[1]) < 1 ||
                (argArr[3] === this.parent.trueValue
        ...
        argArr[0] =
          ...
          rowIdx +
          ':' +
          getAlphalabel(eColIdx) +
          eRowIdx;
        cellCollection = this.parent.getCellCollection(argArr[0]);
        for (let i: number = 0; i < cellCollection.length; i++) {
          valueCollection.push(
            this.parent.getValueFromArg(cellCollection[i as number])
          );
        }
        ...
        if (argArr[3] === 'TRUE') {
          for (let i: number = 0; i < totalColumn; i++) {
            sortRangeValuesHandler(
              valueCollection[
                i + (this.parseDouble(argArr[1]) - 1) * totalColumn
              ]
            );
          }
        }
        ...
        const sortedNumColl: object[] =
          numColl.length > 0
            ? DataUtil.sort(numColl, null, DataUtil.fnSort(order))
            : [];
        const sortedStrColl: object[] =
          strColl.length > 0
            ? DataUtil.sort(strColl, null, DataUtil.fnSort(order))
            : [];
        const sortedBooleanColl: object[] =
          booleanColl.length > 0
            ? DataUtil.sort(booleanColl, null, DataUtil.fnSort(order))
            : [];
        const sortedVal: object[] =
          order === 'Ascending'
            ? sortedNumColl.concat(
                sortedStrColl,
                sortedBooleanColl,
                emptyCellColl
              )
            : sortedBooleanColl.concat(
                sortedStrColl,
                sortedNumColl,
                emptyCellColl
              );
        ...
        for (let a: number = 0; a < sortedVal.length; a++) {
          for (let b: number = 0; b < colSort.length; b++) {
            if (
              JSON.stringify(sortedVal[a as number]) ===
              JSON.stringify(colSort[b as number])
            ) {
              if (id.indexOf(b) === -1) {
                id.push(b);
              }
        ...
        if (argArr[3] === 'TRUE') {
          for (
            let startRow: number = rowIdx, rowInc: number = 0;
            startRow <= eRowIdx;
            startRow++, rowInc++
          ) {
            for (
              let a: number = 0, colInc: number = 0;
              a < id.length;
              a++, colInc++
            ) {
          ...
          result = this.parent.getValueFromArg(
            sheetIdx + getAlphalabel(id[0] + colIdx) + rowIdx
          );
        }
        if (argArr[3] === 'FALSE') {
          for (
            let a: number = 0, rowInc: number = 0;
            a < id.length;
            a++, rowInc++
          ) {
            for (
              let startCol: number = colIdx, colInc: number = 0;
              startCol <= eColIdx;
              startCol++, colInc++
            ) {
          ...
          result = this.parent.getValueFromArg(
            sheetIdx + getAlphalabel(colIdx) + (id[0] + rowIdx)
          );
        }
      }
    }
  ...
  public ComputeCOUNTIF(...args: string[]): number | string {
    ...
    if (isAsterisk && !isAsteriskOnly) {
      ...
      if (asteriskIndex > 0 && asteriskIndex < condition.length - 1) {
        criteriaValue =
          condition.substring(0, asteriskIndex) +
          '*' +
          condition.substring(asteriskIndex + 1);
      }
    }
    ...
    const isWildCardCondition: boolean =
      condition.indexOf('*') > -1 || condition.indexOf('?') > -1;
    if (
      (!isStringVal &&
        this.parent.isCellReference(condition) &&
        !isCellReferenceValue) ||
      condition.includes(this.parent.arithMarker) ||
      (condition.includes(this.parent.getParseDecimalSeparator()) &&
        !isWildCardCondition)
    ) {
    ...
    if (argArr[0].indexOf(':') > -1 && this.parent.isCellReference(argArr[0])) {
      ...
      for (let j: number = 0; j < cellColl.length; j++) {
        ...
        if (isWildCardCondition) {
          cellValue = this.parent.findWildCardValue(
  ...
  private calculateIFS(ranges: string[], isAvgIfs?: string): string | number {
    ...
    if (ranges.length === 3) {
      // SUMIFS and AVERAGEIFS OR operation will contains only 3 arguments.
      if (ranges[2].includes(this.parent.tic + this.parent.tic)) {
        let result: string = '';
        let sumVal: string;
        const separator: string = this.parent.getParseArgumentSeparator();
        const criterias: string[] = ranges[2].split(
          this.parent.tic + this.parent.tic
        );
    ...
    return this.parent.computeIfsFormulas(
      ranges,
      this.parent.falseValue,
      isAvgIfs
    );
  ...
  public ComputeTEXT(...args: string[]): string | number {
    ...
    if (
      s1 === '' &&
      ((s2.length > 0 &&
        (s2.toUpperCase().indexOf('M') > -1 ||
          s2.toUpperCase().indexOf('D') > -1 ||
          s2.toUpperCase().indexOf('Y') > -1 ||
          s2.toUpperCase().indexOf('S') > -1 ||
          s2.toUpperCase().indexOf('T') > -1)) ||
    ...
    if (!isNaN(d) || !isNaN(dTime)) {
      if (
        ...
        let isMFound: boolean = false;
        let i: number = 0;
        // let mcount: number = 0;
        let lastCharIndex: number = 0;
        let totalCharforM: number = 0;
        ...
        if (getSkeleton === '') {
          ...
          const dateString: string = (
            this.parent.parentObject as any
          ).getDisplayText({ format: s2, value: date });
          return dateString;
        }
        const dFormatter: Function = intl.getDateFormat({
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        s1 = (this.parent.parentObject as any).getDisplayText({
          format: s2,
          value: d,
        });
      }
  ...
  public ComputeCOUNTIFS(...args: string[]): number | string {
    const sum: string | number = this.parent.computeIfsFormulas(
      args,
      this.parent.trueValue
    );
    return sum;
  ...
  public ComputeMATCH(...args: string[]): string | number {
    ...
    if (isNullOrUndefined(args[2])) {
      ...
      argArr[2] = this.parent.getValueFromArg(argArr[2]);
      if (
        argArr[2].indexOf(this.parent.tic) > -1 &&
        isNaN(Number(argArr[2].split(this.parent.tic).join('')))
      ) {
        return errCollection[CommonErrors.Value];
      }
    ...
    const valueCollection: string[] = [];
    let cellCollection: string[] | string;
    let isStringCollection: boolean = false;
    if (argArr[1].indexOf(':') > -1 || this.parent.isCellReference(argArr[1])) {
      cellCollection = this.parent.getCellCollection(argArr[1]);
      for (let j: number = 0; j < cellCollection.length; j++) {
        const cellValue: number | string = this.parent.getValueFromArg(
          cellCollection[j as number]
        );
        if (
          cellValue.indexOf(this.parent.tic) > -1 ||
      ...
      if (
        (isStringValue && !isStringCollection) ||
        (!isStringValue && isStringCollection)
      ) {
        return errCollection[CommonErrors.NA];
      }
    ...
    let matchValue: string | number = !isNaN(Number(argArr[0]))
      ? Number(argArr[0])
      : argArr[0];
    for (let i: number = 0; i < valueCollection.length; i++) {
      ...
      const matchCollectionValue: string | number = !isNaN(
        Number(valueCollection[i as number])
      )
        ? Number(valueCollection[i as number])
        : valueCollection[i as number];
      if (matchType === 1) {
        if (matchValue === matchCollectionValue) {
          ...
          matchValue > matchCollectionValue &&
          !isStringValue &&
          !isIndexFound
        ) {
          if (
            !indexVal ||
            matchCollectionValue >
              (!isNaN(Number(indexVal)) ? Number(indexVal) : indexVal)
          ) {
            index = i + 1;
        ...
        if (argArr[0].indexOf('*') > -1 || argArr[0].indexOf('?') > -1) {
          valueCollection[i as number] = this.parent.findWildCardValue(
            argArr[0],
            valueCollection[i as number]
          );
        }
        ...
        if (
          Number(valueCollection[i as number]) >
            Number(valueCollection[(i as number) + 1]) ||
  ...
  public ComputeSUBTOTAL(...argArr: string[]): string | number {
    ...
    if (
      isNaN(value) ||
      ((1 > value || value > 11) && (101 > value || value > 111))
    ) {
      return errCollection[CommonErrors.Value];
    }
  ...
  public ComputeRADIANS(...argValue: string[]): string | number {
    ...
    const errCollection: string[] = this.parent.getErrorStrings();
    if (
      isNullOrUndefined(argValue) ||
      (argValue[0] === '' && argValue.length === 1) ||
      (argValue[0].split('!').length === 2 &&
        argValue[0].indexOf(this.parent.tic) === -1)
    ) {
      return this.parent.formulaErrorStrings[
        FormulasErrorsStrings.InvalidArguments
      ];
      ...
      argValue[0].indexOf(':') > -1 ||
      argValue[0].split(this.parent.tic).join('').trim() === '' ||
      argValue[0].split(this.parent.tic).join('').trim() === '!' ||
      argValue[0].split('!').length === 2
    ) {
      return errCollection[CommonErrors.Value];
    ...
    if (!isNaN(value)) {
      ...
      if (
        this.parent.isCellReference(argValue[0]) ||
        argValue[0].indexOf(this.parent.tic) > -1
      ) {
        return errCollection[CommonErrors.Value];
      } else {
    ...
    const processArgs: Function = (orgValue: string): number | string => {
      ...
      if (this.parent.isCellReference(orgValue)) {
        if (actualValue === '') {
          ...
          orgValue.indexOf(':') > -1 ||
          actualValue.match(/^(\d*\.\d+|\d+)\s*[+\-*/]\s*(\d*\.\d+|\d+)$/)
        ) {
          return errCollection[CommonErrors.Value];
        }
      } else {
        if (actualValue === '') {
          ...
          if (actualValue.indexOf(':') > -1) {
            ...
            if (values.length <= 3) {
              if (
                !this.parent.isNumber(values[0]) ||
                !this.parent.isNumber(values[1])
              ) {
                return errCollection[CommonErrors.Value];
              }
              let hours: number =
                Number(values[0]) + Number(Number(values[1]) / 60);
              if (values.length === 3) {
        ...
        if (
          actualValue.split('%').length === 2 &&
          this.parent.isNumber(actualValue.split('%')[0])
        ) {
  ...
  public ComputeSLOPE(...argValue: string[]): string | number {
    ...
    if (
      isNullOrUndefined(argValue) ||
      (argValue.length === 1 && argValue[0].trim() === '')
    ) {
      return this.parent.formulaErrorStrings[
        FormulasErrorsStrings.InvalidArguments
      ];
    ...
    const cellCollection: Function = (
      actuaValue: string | string[]
    ): string | string[] => {
      if (actuaValue.indexOf(this.parent.tic) === -1) {
        actuaValue = this.parent.getCellCollection(
          (actuaValue as string).split(this.parent.tic).join('')
        );
      } else {
    ...
    if (
      isNullOrUndefined(yPoints) ||
      isNullOrUndefined(xPoints) ||
      (yPoints.length < 2 && xPoints.length < 2)
    ) {
    ...
    const dataCollection: Function = (
      ...
      for (let b: number = 0; b < actuaValue.length; b++) {
        if (errCollection.indexOf(actuaValue[b as number]) > -1) {
          return actuaValue[b as number].toString();
        }
      }
      return actuaValue;
    };
    ...
    for (let i: number = 0, len: number = xPoints.length; i < len; ++i) {
      if (
        xPoints[i as number] !== '' &&
        Number(xPoints[i as number]).toString() !== 'NaN' &&
        yPoints[i as number] !== '' &&
        Number(yPoints[i as number]).toString() !== 'NaN'
      ) {
    ...
    const value: string = (
      (sumXY - (sumX * sumY) / length) /
      (sumX2 - (sumX * sumX) / length)
    ).toString();
    if (value === 'NaN') {
      return errCollection[CommonErrors.DivZero];
    }
  ...
  public ComputeINTERCEPT(...argValue: string[]): string | number {
    ...
    if (
      isNullOrUndefined(yValues) ||
      isNullOrUndefined(xValues) ||
      (yValues.length < 2 && xValues.length < 2)
    ) {
      return errCollection[CommonErrors.DivZero];
    ...
    const calculation: Function = (isSum: boolean): void => {
      for (let i: number = 0, len: number = xValues.length; i < len; ++i) {
        if (
          yValues[i as number] !== '' &&
          Number(yValues[i as number]).toString() !== 'NaN' &&
          xValues[i as number] !== '' &&
          Number(xValues[i as number]).toString() !== 'NaN'
        ) {
          if (isSum) {
  ...
  public ComputeLN(...logValue: string[]): string | number {
    ...
    const errCollection: string[] = this.parent.getErrorStrings();
    if (
      isNullOrUndefined(logValue) ||
      (logValue[0] === '' && logValue.length === 1)
    ) {
      return this.parent.formulaErrorStrings[
        FormulasErrorsStrings.InvalidArguments
      ];
    ...
    if (
      ...
      this.parent.isNumber(cellvalue.split('%')[0])
    ) {
  ...
  public ComputeISNUMBER(...logValue: string[]): boolean | string {
    ...
    const orgValue: number | string = this.parent.isCellReference(argArr[0])
      ? this.parent.getValueFromArg(argArr[0])
      : this.parent.getValueFromArg(argArr[0].split(this.parent.tic).join(''));
    if (
      orgValue.toString() === '' ||
      logValue.toString().startsWith(this.parent.tic)
    ) {
      return false;
    }
  ...
  public ComputeROUND(...logValue: string[]): number | string {
    ...
    if (logValue.length === 1) {
      const orgValue: number | string =
        argArr[0].split(this.parent.tic).join('') === 'TRUE'
          ? '1'
    ...
    const isInvalidNumStr: boolean =
      isNaN(Number(numStr)) || numStr.trim() === '';
    const isInvalidDigStr: boolean =
      isNaN(Number(digStr)) || digStr.trim() === '';
    if (
      ((argArr[0].indexOf('"') > -1 ||
        this.parent.isCellReference(argArr[0])) &&
        isInvalidNumStr) ||
      ((argArr[1].indexOf('"') > -1 ||
        this.parent.isCellReference(argArr[1])) &&
        isInvalidDigStr)
    ) {
      return this.parent.getErrorStrings()[CommonErrors.Value];
    }
  ...
  public ComputePOWER(...argArr: string[]): boolean | string {
    ...
    const processArgs: Function = (actualValue: string): string | number => {
      ...
      if (
        ...
        value.indexOf('/') > -1 &&
        this.parent.isNumber(value.split('/').join(''))
      ) {
        return errCollection[CommonErrors.Num];
      }
      value = this.parent.parseFloat(value);
    ...
    if (errCollection.indexOf(numValue as string) > -1) {
      return numValue as string;
    }
    const powValue: string | number = processArgs(argArr[1]);
    if (errCollection.indexOf(powValue as string) > -1) {
      return powValue as string;
    }
  ...
  public ComputeSQRT(...args: string[]): number | string {
    ...
    if (this.parent.isCellReference(arrValue)) {
      ...
      if (
        arrValue.indexOf(this.parent.tic) > -1 &&
        (arrValue.split(this.parent.tic).join('') === this.parent.trueValue ||
          arrValue.split(this.parent.tic).join('') === this.parent.falseValue)
      ) {
        return this.parent.getErrorStrings()[CommonErrors.Value];
      }
      sqrtValue = this.parent
        .getValueFromArg(arrValue)
        .split(this.parent.tic)
        .join('');
    }
    ...
    sqrtValue =
      sqrtValue === this.parent.trueValue
        ? '1'
        : sqrtValue === this.parent.falseValue
        ? '0'
        : sqrtValue;
  ...
  public ComputeLOG(...logValue: string[]): number | string {
    ...
    const processArgs: Function = (orgValue: string): string => {
      ...
      if (!this.parent.isCellReference(orgValue) && actualValue !== '') {
        if (
          actualValue.indexOf(this.parent.tic) > -1 &&
          errCollection.indexOf(actualValue.split(this.parent.tic).join('')) ===
            -1
        ) {
          actualValue = this.parent.removeTics(actualValue);
          if (actualValue.trim() === '') {
    ...
    return (
      Math.log(orgNumValue) /
      Math.LN10 /
      (Math.log(orgBaseValue) / Math.LN10)
    ).toString();
  }
  /**
   * @hidden
  ...
  public ComputeTRUNC(...logValue: string[]): boolean | string {
    ...
    const processArgs: Function = (orgValue: string): string => {
      ...
      if (actualValue === this.parent.trueValue) {
        ...
        if (
          ...
          actualValue = this.parent.getValueFromArg(
            this.parent.removeTics(orgValue)
          );
        } else if (actualValue.indexOf(this.parent.tic) > -1) {
          actualValue = this.parent.removeTics(actualValue);
          if (actualValue.trim() === '') {
    ...
    return (
      ((orgNumValue < 0 ? -1 : 1) *
        Math.floor(orgDigitValue * Math.abs(orgNumValue))) /
      orgDigitValue
    ).toString();
  }
  /**
  ...
  public ComputeEXP(...logValue: string[]): boolean | string {
    ...
    if (
      orgNumValue.indexOf(this.parent.tic) > -1 &&
      (this.parent.isCellReference(logValue[0]) ||
  ...
  public ComputeGEOMEAN(...logValue: string[]): boolean | string {
    ...
    for (r = 0; r < argArr.length; r++) {
      if (argArr[r as number].indexOf(':') > -1) {
        ...
        cell = this.parent.getCellCollection(
          argArr[r as number].split(this.parent.tic).join('')
        );
        for (s = 0; s < cell.length; s++) {
          cellVal = this.parent.getValueFromArg(cell[s as number]);
          cellStr = cellVal.split(this.parent.tic).join('');
          isBoolean =
            cellStr === this.parent.trueValue ||
            cellStr === this.parent.falseValue;
          ...
          if (
            isBoolean ||
            this.parent.getErrorStrings().indexOf(cellVal) > -1
          ) {
            continue;
          } else if (!isNaN(dev)) {
            count++;
        ...
        if (
          cellVal.indexOf('"') > -1 &&
          isNaN(this.parent.parseFloat(cellStr))
        ) {
          return this.parent.getErrorStrings()[CommonErrors.Value];
        }
        argArr[r as number] = argArr[r as number].startsWith('n')
          ? argArr[r as number].slice(1)
          : argArr[r as number];
        if (cellVal === '' && argArr[r as number] === '') {
          return this.parent.getErrorStrings()[CommonErrors.Num];
        }
        if (
          (cellStr === 'TRUE' || cellStr === 'FALSE') &&
          this.parent.isCellReference(argArr[r as number])
        ) {
          continue;
        }
        if (cellVal.length > 0) {
          ...
          cellVal =
            cellVal.split(this.parent.tic).join('') === 'TRUE'
              ? '1'
              : cellVal.split(this.parent.tic).join('') === 'FALSE'
              ? '0'
              : cellVal;
  ...
  public ComputeRSQ(...range: string[]): string | number {
    ...
    if (
      argArr[0].indexOf(':') === -1 &&
      isCellReference(argArr[0]) &&
      argArr[1].indexOf(':') === -1 &&
      isCellReference(argArr[1])
    ) {
      return this.parent.getErrorStrings()[CommonErrors.DivZero];
    }
    const yValuesRange: string | string[] = this.parent.getCellCollection(
      argArr[0]
    );
    const xValuesRange: string | string[] = this.parent.getCellCollection(
      argArr[1]
    );
    ...
    for (let a: number = 0; a < xValues.length; a++) {
      if (this.parent.getErrorStrings().indexOf(xValues[a as number]) > -1) {
        return xValues[a as number];
      }
    }
    const yValues: string[] = this.getDataCollection(yValuesRange);
    for (let b: number = 0; b < yValues.length; b++) {
      if (this.parent.getErrorStrings().indexOf(yValues[b as number]) > -1) {
        return yValues[b as number];
      }
    }
    let xValue: number;
    let yValue: number;
    ...
    const correlation: number = this.getCorrelation(
      xValues,
      yValues,
      meanX,
      meanY
  ...
  private spreadsheetDisplayText(value: string): string {
    ...
    if (
      ...
      (this.parent.parentObject as any).element &&
      (this.parent.parentObject as any).element.classList.contains(
        'e-spreadsheet'
      ) &&
      this.parent.isCellReference(value)
    ) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const indexes: { startIdx: number; endIdx: number; isCol: boolean } = (
        this.parent.parentObject as any
      ).getIndexes(value);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      value = (this.parent.parentObject as any).getDisplayText(
        (this.parent.parentObject as any).getActiveSheet().rows[
          indexes.startIdx
        ].cells[indexes.endIdx]
      );
    }
    return value;
  }
  ...
  private spreadsheetFormat(value: string): string {
    ...
    if (
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const index: { startIdx: number; endIdx: number; isCol: boolean } = (
        this.parent.parentObject as any
      ).getIndexes(value);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      value = (this.parent.parentObject as any).getActiveSheet().rows[
        index.startIdx
      ].cells[index.endIdx].format;
    }
    return value;
  }
