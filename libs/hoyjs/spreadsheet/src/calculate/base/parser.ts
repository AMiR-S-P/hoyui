...
export class Parser {
  ...
  public parse(text: string, fkey?: string): string {
    ...
    if (
      this.parent.getFormulaCharacter() !== String.fromCharCode(0) &&
      this.parent.getFormulaCharacter() === text[0]
    ) {
          ...
          default:
            throw new FormulaError(
              this.parent.formulaErrorStrings[
                FormulasErrorsStrings.InvalidExpression
              ],
              true
            );
        }
    ...
    text = text.split('=<').join('<=');
    if (
      text[text.length - 1] !== this.parent.arithMarker ||
      this.indexOfAny(text, this.tokens) !== text.length - 2
    ) {
      text = text.toUpperCase();
    }
    if (text.indexOf(this.sheetToken) > -1) {
      const family: CalcSheetFamilyItem = this.parent.getSheetFamilyItem(
        this.parent.grid
      );
      if (
        ...
        const sheetToken: string = this.parent.getSheetToken(
          text.split(this.parent.tic).join(this.emptyStr)
        );
        const scopedRange: string = this.checkScopedRange(
          text.split('"').join(this.emptyStr).split(this.sheetToken).join('')
        );
        if (
          isNullOrUndefined(sheetToken) &&
          sheetToken !== '' &&
          this.parent.namedRanges.size > 0 &&
          scopedRange !== ''
        ) {
          text = scopedRange;
        }
    ...
    try {
      ...
      if (!args.isForceCalculable) {
        throw this.parent.formulaErrorStrings[
          FormulasErrorsStrings.InvalidExpression
        ];
      }
    ...
    if (!this.ignoreBracet) {
      ...
      while (i > -1) {
        ...
        if (k === -1) {
          throw new FormulaError(
            this.parent.formulaErrorStrings[
              FormulasErrorsStrings.MismatchedParentheses
            ]
          );
        }
        if (k === i - 1) {
          throw new FormulaError(
            this.parent.formulaErrorStrings[
              FormulasErrorsStrings.EmptyExpression
            ]
          );
        }
        ...
        try {
          text =
            text.substring(0, k) + this.parseSimple(s) + text.substring(i + 1);
        } catch (ex) {
          ...
          return (
            (this.parent.getErrorLine(ex)
              ? ''
              : '#' + this.parent.getErrorLine(ex) + ': ') + errorMessage
          );
        }
        i = text.indexOf(')');
      }
    ...
    if (this.parent.formulaErrorStrings.indexOf(formula) > -1) {
      ...
      while (i < formula.length) {
        formula = formula
          ...
          .join('+');
        if (
          this.parent.isDigit(formula[i as number]) &&
          formula.length > i + 1 &&
          this.indexOfAny(formula[i + 1], arithemeticArr) > -1 &&
          formula.length > i + 2 &&
          !isNullOrUndefined(formula[i + 2]) &&
          this.indexOfAny(formula[i + 2], arithemeticArr) > -1 &&
          (formula[i + 2] !== '-' ||
            (formula[i + 1] !== '*' &&
              formula[i + 1] !== '/' &&
              formula[i + 1] !== '^' &&
          ...
          if (args && args.computeForceCalculate) {
            if (this.parent.isDigit(formula[i as number])) {
              if (countDigit < 1) {
                ...
                countDigit = 0;
                if (secondprevOp === '-') {
                  secondnextOp = isNullOrUndefined(secondnextOp)
                    ? this.emptyStr
                    : secondnextOp;
          ...
          (this.parent.isDigit(formula[i as number]) ||
            formula[i as number] === this.parent.rightBracket ||
            this.parent.storedData.has(formula[i as number].toUpperCase())) &&
          (isNullOrUndefined(formula[i + 1]) ||
            this.indexOfAny(formula[i + 1], arithemeticArr) > -1)
        ) {
          op = isNullOrUndefined(formula[i + 1])
            ? this.emptyStr
            : formula[i + 1];
          op = op === '&' && formula[i + 2] !== '-' ? '' : op; // for the cases 5&3=>53 and 5&-3=>5-3.
          form =
            formula[i - 1] === '-'
              ? form + formula[i - 1] + formula[i as number] + op
              : form + formula[i as number] + op;
          i = i + 2;
        } else if (
          this.indexOfAny(formula[i as number], logicalSym) > -1 &&
          !isNullOrUndefined(formula[i - 1]) &&
          !isNullOrUndefined(formula[i + 1])
        ) {
          ...
          formula[i as number] === this.parent.leftBracket ||
          formula[i as number] === this.parent.rightBracket ||
          formula[i as number] === '{' ||
          formula[i as number] === '}' ||
          formula[i as number] === '(' ||
          ...
          this.parent.isUpperChar(formula[i as number]) ||
          formula[i as number].indexOf(':') > -1 ||
          formula[i as number] === this.parent.getParseArgumentSeparator() ||
          (formula[i as number] === '%' && this.parent.isDigit(formula[i - 1]))
        ) {
          form = form + formula[i as number];
          i = i + 1;
        } else if (
          formula[i as number] === this.parent.tic ||
          formula[i as number] === ' ' ||
          formula[i as number] === this.parent.getParseDecimalSeparator() ||
          formula[i as number] === this.sheetToken ||
          formula[i as number] === '$' ||
          ...
          if (this.parent.isDigit(formula[i as number])) {
            form =
              formula[i - 1] === '-'
                ? form + formula[i - 1] + formula[i as number]
                : form + formula[i as number];
          }
  ...
  private checkScopedRange(text: string): string {
    ...
    const sheet: CalcSheetFamilyItem = this.parent.getSheetFamilyItem(
      this.parent.grid
    );
    if (text[0] === this.sheetToken.toString()) {
      ...
      if (sheet.sheetNameToToken.get(key).toString() === token + '!') {
        ...
        this.parent.namedRanges.forEach((value: string, key: string) => {
          ...
          if (!isNullOrUndefined(this.parent.parentObject as any)) {
            /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
            s =
              (this.parent.parentObject as any).getActiveSheet().name +
              this.sheetToken +
              text.toUpperCase();
          } else {
  ...
  private storeStrings(tempString: string): Map<string, string> {
    ...
    if (ticLoc > -1) {
      ...
      while (i > -1 && tempString.length > 0) {
        ...
        j =
          i + 1 < tempString.length
            ? tempString.indexOf(this.parent.tic, i + 1)
            : -1;
        if (j === -1) {
          throw new FormulaError(
            this.parent.formulaErrorStrings[
              FormulasErrorsStrings.MismatchedTics
            ]
          );
        }
        condition = this.parent.substring(tempString, i, j - i + 1);
        key =
          this.parent.tic + this.spaceString + id.toString() + this.parent.tic;
        storedString = storedString.set(key, condition);
        tempString =
          tempString.substring(0, i) + key + tempString.substring(j + 1);
        i = i + key.length;
  ...
  public parseSimple(formulaText: string): string {
    ...
    const compareTokenArray: string[] = [
      this.tokenLess,
      this.tokenGreater,
      this.tokenEqual,
      this.tokenLessEq,
  ...
  public parseSimpleOperators(
    ...
    text = text
      ...
      .split('--')
      .join('+')
      .split(this.parent.getParseArgumentSeparator() + '-')
      .join(this.parent.getParseArgumentSeparator() + 'u')
      .split(this.parent.leftBracket + '-')
      .join(this.parent.leftBracket + 'u')
      .split('=-')
      .join('=u');
    text = text
      .split(',+')
    ...
    if (text.length > 0 && text[0] === '-') {
      ...
      const textValue: string[] = text.split(
        this.parent.getParseArgumentSeparator()
      );
      for (let i: number = 0; i < textValue.length; i++) {
        if (textValue[i as number].startsWith('u')) {
          ...
          textExp = this.parseSimpleOperators(
            textExp,
            [this.tokenSubtract],
            [this.charSubtract]
          );
      ...
      if (this.indexOfAny(text, operators) > -1) {
        if (text.includes(' ')) {
          ...
          for (let index: number = 0; index < text.length; index++) {
            ...
            if (operators.indexOf(currChar) >= 0) {
              ...
              currChar === ' ' &&
              operators.indexOf(newText[newText.length - 1]) >= 0
            ) {
              continue;
            } else {
              newText += currChar;
        ...
        while (i > -1) {
          ...
          if (!isNotOperator) {
            ...
            if (text[j as number] === this.parent.arithMarker) {
              ...
              if (k < 0) {
                throw new FormulaError(
                  this.parent.formulaErrorStrings[
                    FormulasErrorsStrings.CannotParse
                  ]
                );
              }
              ...
              while (
                k > 0 &&
                (text[k as number] !== 'q' || bracketCount !== 0)
              ) {
              ...
              while (
                j > -1 &&
                (this.parent.isDigit(text[j as number]) ||
                  (!period &&
                    (text[j as number] === decimalSep ||
                      text[j as number] === '%')))
              ) {
                if (
                  !this.parent.isDigit(text[j as number]) &&
              ...
              if (j > -1 && period && text[j as number] === decimalSep) {
                throw new FormulaError(
                  this.parent.formulaErrorStrings[
                    FormulasErrorsStrings.NumberContains2DecimalPoints
                  ]
                );
              }
              j = j + 1;
              if (j === 0 || (j > 0 && !this.parent.isUpperChar(text[j - 1]))) {
                ...
                j > 0 &&
                text[j - 1] === 'E' &&
                ((text.substring(j - 4, j) === 'TRUE' &&
                  (!isNullOrUndefined(text[j - 5])
                    ? arithOp.indexOf(text[j - 5]) > -1
                    : true)) ||
                  (text.substring(j - 5, j) === 'FALSE' &&
                    (!isNullOrUndefined(text[j - 6])
                      ? arithOp.indexOf(text[j - 6]) > -1
                      : true))) &&
                (text.substring(j + 1, j + 5) === 'TRUE' ||
                  text.substring(j + 1, j + 6) === 'FALSE')
              ) {
                j = text.substring(j - 4, j) === 'TRUE' ? j - 4 : j - 5;
                left =
                  text.substring(j, i) === 'TRUE'
                    ? 'n1'
                    : text.substring(j, i) === 'FALSE'
                    ? 'n0'
                    : left;
                leftIndex = j;
                isLeftBool = true;
                ...
                while (
                  j > -1 &&
                  (this.parent.isUpperChar(text[j as number]) ||
                    this.parent.isDigit(text[j as number]))
                ) {
                  j = j - 1;
                }
                ...
                if (j > -1 && text[j as number] === ':') {
                  ...
                  if (j > 0 && !this.parent.isUpperChar(text[j as number])) {
                    uFound =
                      text[j as number] === 'u' &&
                      text[j - 1] === this.parent.getParseArgumentSeparator();
                  }
                  if (!uFound) {
                    j = j + 1;
                  }
              ...
              if (
                (this.parent.namedRanges.size > 0 &&
                  this.parent.namedRanges.has(left.toUpperCase())) ||
          ...
          if (i === text.length - 1) {
            throw new FormulaError(
              this.parent.formulaErrorStrings[
                FormulasErrorsStrings.ExpressionCannotEndWithAnOperator
              ]
            );
          } else {
            j = i + 1;
            ...
            if (text[j as number] === this.parent.tic[0]) {
              ...
              while (
                k < text.length &&
                (text[k as number] !== this.parent.rightBracket ||
                  bracketCount !== 0)
              ) {
              ...
              this.parent.isUpperChar(text[j as number]) ||
              text[j as number] === this.sheetToken ||
              text[j as number] === 'u'
            ) {
              ...
              while (
                ...
                (this.parent.isUpperChar(text[j as number]) ||
                  ...
                  text[j as number] === ' ' ||
                  text[j as number] === '%' ||
                  (text[j as number] === decimalSep && inbracket))
              ) {
                if (
                  j !== text.length - 1 &&
                  text[j as number] === '[' &&
              ...
              let noCellReference: boolean =
                j === text.length || !this.parent.isDigit(text[j as number]);
              if (jTemp > 1) {
                while (
                  j < text.length &&
                  (this.parent.isUpperChar(text[j as number]) ||
              ...
              if (
                (this.parent.namedRanges.size > 0 &&
                  this.parent.namedRanges.has(right.toUpperCase())) ||
                this.parent.storedData.has(right.toUpperCase())
              ) {
                right = 'n' + this.checkForNamedRangeAndKeyValue(right);
              }
              const isPrevArithOp: boolean =
                ['*', '+', '-', '/', 'w'].indexOf(text[j - right.length]) > -1;
              right =
                right === 'TRUE' && (isLeftBool || isPrevArithOp)
                  ? 'n1'
                  : right === 'FALSE' && (isLeftBool || isPrevArithOp)
                  ? 'n0'
                  : right;
              rightIndex = j + 1;
            }
          ...
          s = s
            .split(this.parent.arithMarker2)
            .join(this.parent.arithMarker.toString());
          text = s;
          i = this.indexOfAny(text, operators);
        }
      } else {
        if (
          text.length > 0 &&
          (this.parent.isUpperChar(text[0]) || text[0] === this.sheetToken)
        ) {
          ...
          for (let k: number = 0; k < textLen; ++k) {
            if (text[k as number] === this.sheetToken) {
              ...
              if (
                this.parent.isChar(text[k as number]) ||
                this.parent.isDigit(text[k as number]) ||
                text[k as number] === this.sheetToken
              ) {
    ...
    if (text.indexOf(this.parent.arithMarker) > -1) {
      ...
      for (let i: number = text.length - 1; i >= 0; --i) {
        if (text[i as number] === this.parent.rightBracket) {
          ...
          text[i as number] === this.parent.arithMarker &&
          bracketLevel === 0
        ) {
          ret = i;
          break;
    ...
    if (text.indexOf(this.parent.arithMarker) > -1) {
      ...
      for (let j: number = 0; j < text.length; ++j) {
        if (text[j as number] === this.parent.rightBracket) {
          ...
          text[j as number] === this.parent.arithMarker &&
          bracketLevel === 0
        ) {
          ret = j;
          break;
  ...
  public parseFormula(formula: string, fKey?: string): string {
    if (
      formula.length > 0 &&
      formula[0] === this.parent.getFormulaCharacter()
    ) {
    ...
    try {
      ...
      const errorMessage: string =
        typeof args.exception === 'string' ? args.exception : args.message;
      formula =
        (isNullOrUndefined(this.parent.getErrorLine(ex))
          ? ''
          : '#' + this.parent.getErrorLine(ex) + ': ') + errorMessage;
      this.isError = true;
    }
  ...
  public markLibraryFormulas(formula: string): string {
    ...
    if (rightParens === -1) {
      ...
      while (rightParens > -1) {
        ...
        while (
          leftParens > -1 &&
          (formula[leftParens as number] !== '(' || parenCount !== 0)
        ) {
        ...
        if (
          len > 0 &&
          !isNullOrUndefined(this.parent.getFunction(libFormula))
        ) {
          let substr: string = this.parent.substring(
            formula,
            leftParens,
            rightParens - leftParens + 1
          );
          ...
          if (libFormula === 'AREAS') {
            ...
            if (
              libFormula.includes('IFS') &&
              libFormula !== 'COUNTIFS' &&
              substr.includes('{')
            ) {
              const leftBraceIdx: number = substr.indexOf('{');
              const criteriaStr: string = this.parent.substring(
                substr,
                leftBraceIdx,
                substr.indexOf('}') - leftBraceIdx + 1
              );
              substr = substr
                .split(criteriaStr)
                .join(
                  criteriaStr
                    .split(argsSep)
                    .join(this.parent.tic + this.parent.tic)
                );
            }
          }
          try {
            ...
            args.computeForceCalculate =
              bracCount > 0 ? true : args.computeForceCalculate;
            if (args.isForceCalculable) {
          ...
          formula =
            formula.substring(0, i + 1) +
            'q' +
            this.parent.substring(formula, i + 1, len) +
            substr
              .split('(')
              .join(this.parent.leftBracket)
              .split(')')
              .join(this.parent.rightBracket) +
  ...
  public swapInnerParens(fSubstr: string): string {
    if (fSubstr.length > 2) {
      fSubstr =
        fSubstr[0] +
        fSubstr
          .substr(1, fSubstr.length - 2)
          .split('(')
          .join('{')
          .split(')')
          .join('}') +
        fSubstr[fSubstr.length - 1];
    }
    return fSubstr;
  }
  ...
  public addParensToArgs(fSubstr: string): string {
    ...
    let id: number = fSubstr.lastIndexOf(
      this.parent.getParseArgumentSeparator()
    );
    let k: number = 0;
    if (id === -1) {
      if (
        ...
        if (fSubstr[1] !== '{' && fSubstr[1] !== '(') {
          fSubstr =
            fSubstr.substring(0, fSubstr.length - 1) +
            '}' +
            fSubstr.substring(fSubstr.length - 1);
          ...
          if (k > 0) {
            if (fSubstr[id + 1] !== '{' && fSubstr[id - 1] === '}') {
              ...
              fSubstr =
                fSubstr.substr(0, id + 1) + '{' + fSubstr.substr(id + 1);
            }
          }
        }
      }
      ...
      while (id > -1) {
        let j: number = this.indexOfAny(
          fSubstr.substring(id + 1, fSubstr.length),
          rightSides
        );
        if (j >= 0) {
          j = id + j + 1;
        ...
        if (j > 0) {
          if (
            ...
            fSubstr =
              fSubstr.substr(0, id + 1) + '{' + fSubstr.substr(id + 1).trim();
          }
        }
        id = fSubstr
          .substr(0, id)
          .lastIndexOf(this.parent.getParseArgumentSeparator());
  ...
  public markNamedRanges(formula: string): string {
    ...
    let i: number =
      formula.length > 0 && (formula[0] === '(' || formula[0] === '{') ? 1 : 0;
    ...
    while (end > -1 && end + i < formula.length) {
      ...
      if (this.parent.substring(formula, i, end).indexOf('[') > -1) {
        ...
        this.parent.storedData.has(this.parent.substring(formula, i, end))
      ) {
        s = this.checkForNamedRangeAndKeyValue(
          this.parent.substring(formula, i, end)
        );
      } else if (
        this.parent.namedRanges.has(this.parent.substring(formula, i, end))
      ) {
        s = this.checkForNamedRangeAndKeyValue(
          this.parent.substring(formula, i, end)
        );
      ...
      if (isNullOrUndefined(s)) {
        scopedRange = this.checkScopedRange(
          this.parent.substring(formula, i, end)
        );
        if (scopedRange !== 'NaN') {
          ...
          this.parent
            .substring(formula, i, end)
            .startsWith(this.sheetToken.toString())
        ) {
      ...
      if (
        (end === -1 || formula.substring(i).indexOf('[') > -1) &&
        i < formula.length
      ) {
        if (formula.substring(i).indexOf('[') > -1) {
          ...
          if (this.parent.storedData.has(formula.substring(i))) {
            s =
              this.parent.storedData.size > 0
                ? this.checkForNamedRangeAndKeyValue(formula.substring(i))
                : s;
          } else {
            s =
              this.parent.namedRanges.size > 0
        ...
        if (!isNullOrUndefined(s) && s !== this.emptyStr) {
          ...
          i < formula.length
            ? this.indexOfAny(formula.substring(i), markers)
            : -1;
      }
    }
    if (formula.indexOf('#N~A') > -1) {
    ...
    if (scopedRange !== 'NaN') {
      ...
      if (text.indexOf(this.sheetToken) > -1) {
        ...
        if (sheet.sheetNameToToken.has(value.toUpperCase())) {
          /* eslint-disable */
          let sheetIndex: number = parseInt(
            sheet.sheetNameToToken
      ...
      if (
        this.parent.namedRanges.size > 0 &&
        this.parent.namedRanges.has(text.toUpperCase())
      ) {
        if (!isNullOrUndefined(this.parent.parentObject as any)) {
  ...
  private getTableRange(text: string): string {
    ...
    let name: string = text
      .replace(']', this.emptyStr)
      .replace('#DATA', this.emptyStr);
    let tableName: string = name;
    if (name.indexOf(this.parent.getParseArgumentSeparator()) > -1) {
      tableName = name
        .substring(0, name.indexOf(this.parent.getParseArgumentSeparator()))
        .replace('[', this.emptyStr);
      name = name
        .replace('[', this.emptyStr)
        .replace(this.parent.getParseArgumentSeparator(), '_');
    }
