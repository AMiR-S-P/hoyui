...
import { Collection, Complex } from '@hoyui/base';
import {
  Tab,
  Toolbar,
  ItemModel,
...
import { SelectEventArgs as TabSelectEventArgs } from '@hoyui/navigations';
import {
  RibbonModel,
  RibbonItemModel,
  RibbonHeaderModel,
} from './ribbon-model';
import { SelectEventArgs } from '@hoyui/dropdowns';
  ...
  implements INotifyPropertyChanged
{
   ...
   */
  public destroy(): void {
    const expandCollapseElem: HTMLElement =
      this.element.querySelector('.e-drop-icon');
    if (expandCollapseElem) {
      expandCollapseElem.removeEventListener(
        'click',
        this.ribbonExpandCollapse.bind(this)
      );
    }
    if (this.menuItems.length) {
      const fileMenu: HTMLElement = document.getElementById(
        `${this.element.id}_menu`
      );
      if (fileMenu) {
        (getComponent(fileMenu, 'menu') as Menu).destroy();
      }
    }
    if (this.toolbarObj) {
  ...
  private initMenu(menuItems: MenuItemModel[]): HTMLElement {
    const menu: HTMLElement = this.createElement('ul', {
    ...
    const menuObj: Menu = new Menu({
      ...
      beforeClose: (args: BeforeOpenCloseMenuEventArgs): void => {
        if (
          ...
          !closest(args.event.target as Element, '.e-menu-popup')
        ) {
          args.cancel = true;
          return;
        }
  ...
  private renderRibbon(): void {
    ...
    this.tabObj = new Tab({
      ...
      selecting: (args: SelectingEventArgs): void => {
        ...
        isShortcut =
          args.event &&
          (args.event as unknown as { isShortcut: boolean }).isShortcut;
        if (this.menuItems.length && args.selectingIndex === 0) {
          args.cancel = true;
          if (
            (!args.event || isShortcut || args.event.type === 'keydown') &&
            args.selectingItem
          ) {
            const fileMenu: HTMLElement = args.selectingItem.querySelector(
              '.e-file-menu .e-menu-item'
            );
            if (fileMenu) {
              fileMenu.click();
            }
      ...
      selected: (args: TabSelectEventArgs): void => {
        ...
        this.setProperties(
          { selectedTab: this.getIndex(args.selectedIndex, true) },
          true
        );
        if (this.element.classList.contains('e-collapsed')) {
          this.element.classList.remove('e-collapsed');
          this.trigger('expandCollapse', {
    ...
    const collapseBtn: Element = this.createElement('span', {
  ...
  private ribbonExpandCollapse(e: MouseEvent): void {
    const eventArgs: ExpandCollapseEventArgs = {
      element: this.toolbarObj.element,
      expanded: true,
    };
    let activeTab: Element;
    if (this.element.classList.contains('e-collapsed')) {
      activeTab = this.tabObj.element
        .querySelector('.e-tab-header')
        .getElementsByClassName('e-toolbar-item')[this.tabObj.selectedItem];
      this.element.classList.remove('e-collapsed');
      ...
      activeTab = this.tabObj.element.querySelector(
  ...
  private updateToolbar(index: number): void {
    this.toolbarObj.items = this.items[index as number].content;
    this.toolbarObj.dataBind();
  }
  ...
  public enableMenuItems(
    ...
    (
      getComponent(
        document.getElementById(`${this.element.id}_menu`),
        'menu'
      ) as Menu
  ...
  public hideMenuItems(
    ...
    const menuInstance: Menu = getComponent(
      document.getElementById(`${this.element.id}_menu`),
      'menu'
    ) as Menu;
    if (hide) {
  ...
  public hideTabs(tabs: string[], hide: boolean = true): void {
    let idx: number;
    let activeTab: boolean;
    let stateChanged: boolean;
    let isAllHidden: boolean;
    if (!hide) {
      ...
      if (idx > -1) {
        if (hide) {
          if (!this.items[idx as number].cssClass.includes(' e-hide')) {
            this.items[idx as number].cssClass = `${
              this.items[idx as number].cssClass
            } e-hide`;
            this.tabObj.items[this.getIndex(idx)].cssClass =
              this.items[idx as number].cssClass;
          ...
          if (this.items[idx as number].cssClass.includes(' e-hide')) {
            this.items[idx as number].cssClass = this.items[
              idx as number
            ].cssClass.replace(' e-hide', '');
            this.tabObj.items[this.getIndex(idx as number)].cssClass =
              this.items[idx as number].cssClass;
            if (activeTab === undefined && idx === this.selectedTab) {
              activeTab = true;
            }
  ...
  public enableTabs(tabs: string[], enable: boolean = true): void {
    tabs.forEach((tab: string): void => {
      ...
      if (idx > -1) {
        this.items[idx as number].disabled = !enable;
  ...
  public addTabs(items: RibbonItemModel[], insertBefore?: string): void {
    ...
    items.forEach((item: RibbonItemModel): void => {
      ...
      this.tabObj.addTab(
        [{ header: item.header, content: this.toolbarObj.element }],
        this.getIndex(idx)
      );
      idx++;
    });
  ...
  private getTabIndex(
    headerText: string,
    idx: number = this.items.length
  ): number {
    if (headerText) {
      for (let i: number = 0; i < this.items.length; i++) {
        if (this.items[i as number].header.text === headerText) {
          idx = i;
          break;
        }
      }
  ...
  public addToolbarItems(
    ...
    if (isNullOrUndefined(index)) {
      index = this.items[tabIdx as number].content.length;
    }
    items.forEach((item: ItemModel): void => {
      item = new Item(
        <Item>this.items[tabIdx as number].content[0],
        'content',
        item,
        true
      );
      this.items[tabIdx as number].content.splice(index, 0, item);
      index++;
    });
    this.setProperties({ items: this.items }, true);
    ...
    if (items) {
      ...
      for (let i: number = 0; i < items.length; i++) {
        if (typeof items[i as number] === 'string') {
          for (
            let j: number = 0;
            j < this.items[tabIdx as number].content.length;
            j++
          ) {
            if (
              this.items[tabIdx as number].content[j as number].id ===
              items[i as number]
            ) {
              items[i as number] = j;
              break;
        ...
        this.items[tabIdx as number].content[items[i as number]].disabled =
          !enable;
        this.setProperties({ items: this.items }, true);
        ...
        indexes.forEach((idx: number): void => {
          ...
          if (tabContent) {
            if (hide) {
              ...
              if (tabContent.cssClass.includes(' e-hide')) {
                tabContent.cssClass = tabContent.cssClass.replace(
                  ' e-hide',
                  ''
                );
              }
