...
export class Range extends ChildProperty<Sheet> {
  ...
  protected setProperties(prop: object, muteOnChange: boolean): void {
    if (
      this['parentObj'].isComplexArraySetter &&
      this['controlParent'] &&
      this['controlParent'].isAngular
    ) {
      if (Object.keys(prop).length) {
        ...
        const range: ExtendedRange =
          this['parentObj'].ranges[this['parentObj']['currRangeIdx']];
        if (range && range.info) {
        ...
        this['controlParent'].tagObjects[0].instance.hasChanges &&
        !this['controlParent'].tagObjects[0].instance.isInitChanges
      ) {
        const sheetIdx: number = this['controlParent'].sheets.indexOf(
          this['parentObj']
        );
        if (this['parentObj'].changedRangeIdx === undefined) {
          let rangeIdx: number;
          const tagObjects: Object[] =
            this['controlParent'].tagObjects[0].instance.list[
              sheetIdx as number
            ].tagObjects;
          for (let i: number = 0; i < tagObjects.length; i++) {
            if (tagObjects[i as number]['name'] === 'ranges') {
              tagObjects[i as number]['instance'].list.forEach(
                (range: { hasChanges: boolean }, idx: number) => {
                  if (range.hasChanges) {
                    rangeIdx = idx;
                  }
                }
...
export function getSheetIndexByName(
  context: Workbook,
  name: string,
  info: { visibleName: string; sheet: string; index: number }[]
): number {
 ...
 */
export function updateSelectedRange(
  context: Workbook,
  range: string,
  sheet: SheetModel = {},
  ...
  context.setSheetPropertyOnMute(
    sheet,
    'selectedRange',
    isMultiRange ? sheet.selectedRange + ' ' + range : range
  );
}
...
export function initSheet(
  ...
  sheets.forEach((sheet: ExtendedSheet) => {
    ...
    context.setSheetPropertyOnMute(
      sheet,
      'selectedRange',
      sheet.selectedRange || sheet.activeCell + ':' + sheet.activeCell
    );
    context.setSheetPropertyOnMute(
      sheet,
      'usedRange',
      sheet.usedRange || { rowIndex: 0, colIndex: 0 }
    );
    context.setSheetPropertyOnMute(
      sheet,
      'ranges',
    ...
    context.setSheetPropertyOnMute(
      sheet,
      'rows',
      (sheet.rows && extend([], sheet.rows, null, true)) || []
    );
    context.setSheetPropertyOnMute(sheet, 'columns', sheet.columns || []);
    context.setSheetPropertyOnMute(
      sheet,
      'showHeaders',
      isUndefined(sheet.showHeaders) ? true : sheet.showHeaders
    );
    context.setSheetPropertyOnMute(
      sheet,
      'showGridLines',
      isUndefined(sheet.showGridLines) ? true : sheet.showGridLines
    );
    ...
    sheet.protectSettings = sheet.protectSettings || {
      selectCells: false,
      formatCells: false,
      formatRows: false,
      formatColumns: false,
...
export function getSheetName(
  context: Workbook,
  idx: number = context.activeSheetIndex
): string {
...
export function moveSheet(
  ...
  position = getNextPrevVisibleSheetIndex(
    context.sheets,
    position,
    context.activeSheetIndex > position
  );
  const args: {
    action: string;
    eventArgs: { position: number; sheetIndexes: number[]; cancel: boolean };
  } = {
    action: 'moveSheet',
    eventArgs: {
      position: position,
  ...
  if (!args.eventArgs.cancel) {
    ...
    sheetIndexes.forEach((sIdx: number, idx: number) => {
      context.sheets.splice(
        position + idx,
        0,
        context.sheets.splice(sIdx + (position > sIdx ? -1 * idx : 0), 1)[0]
      );
    });
    context.setProperties(
      {
        activeSheetIndex: isFromUpdateAction
          ? getSheetIndex(context, sheetName)
          : position > sheetIndexes[0]
          ? position - (sheetIndexes.length - 1)
          : position,
      },
      true
    );
...
export function duplicateSheet(
  ...
  const args: {
    action: string;
    eventArgs: { sheetIndex: number; cancel: boolean };
  } = {
    action: 'duplicateSheet',
    eventArgs: { sheetIndex: sheetIndex, cancel: false },
  };
  if (action) {
    context.trigger('actionBegin', args);
  }
  if (!args.eventArgs.cancel) {
    ...
    const sheet: SheetModel = extend(
      {},
      (originalSheet as { properties: Object }).properties
        ? (originalSheet as { properties: Object }).properties
        : originalSheet,
      {},
      true
 ...
 */
function getNextPrevVisibleSheetIndex(
  sheets: SheetModel[],
  startIndex: number,
  isPrevious: boolean
): number {
  for (
    ...
    isPrevious ? i >= 0 : i < sheets.length;
    isPrevious ? i-- : i++
  ) {
    if (
      !(
        sheets[i as number].state === 'Hidden' ||
        sheets[i as number].state === 'VeryHidden'
      )
    ) {
      startIndex = i;
      break;
