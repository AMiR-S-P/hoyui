...
export function getColumnWidth(
  ...
  if (sheet && sheet.columns && sheet.columns[index as number]) {
    if (!skipHidden && sheet.columns[index as number].hidden) {
      return 0;
    }
    width =
      sheet.columns[index as number].width ||
      sheet.columns[index as number].customWidth
        ? sheet.columns[index as number].width
        : 64;
  } else {
    width = 64;
...
export function isHiddenCol(sheet: SheetModel, index: number): boolean {
  return (
    sheet.columns[index as number] && sheet.columns[index as number].hidden
  );
}
 ...
 */
export function checkColumnValidation(
  column: ColumnModel,
  rowIndex: number,
  colIndex: number
): boolean {
  if (column && column.validation) {
    if (
      ...
      (column.validation.address &&
        isInMultipleRange(column.validation.address, rowIndex, colIndex))
    ) {
      return true;
    }
  }
