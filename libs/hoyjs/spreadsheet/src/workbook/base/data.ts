...
export function getData(
  context: Workbook,
  address: string,
  columnWiseData?: boolean,
  valueOnly?: boolean,
  frozenIndexes?: number[],
  filterDialog?: boolean,
  formulaCellRef?: string,
  idx?: number,
  skipHiddenRows: boolean = true,
  commonAddr?: string,
  dateValueForSpecificColIdx?: number,
  dateColData?: { [key: string]: Object }[]
      ...
      (() => {
        ...
        return args.promise.then(() => {
          ...
          if (columnWiseData) {
            ...
            let localeObj: { decimal: string };
            let intl: Internationalization;
            let autoDetectFormatFn: (cell: CellModel) => void;
            if (valueOnly) {
              localeObj = getNumericObject(context.locale) as {
                decimal: string;
              };
            ...
            address
              .split(',')
              .forEach((addr: string, addrIdx: number): void => {
                indexes = getRangeIndexes(addr);
                index = 0;
                sRow = indexes[0];
                while (sRow <= indexes[2]) {
                  cells =
                    (<{ [key: string]: CellModel }[]>data)[index as number] ||
                    {};
                  ...
                  while (i <= indexes[3]) {
                    ...
                    const cell: CellModel = row
                      ? getCell(sRow, i, sheet)
                      : null;
                    if (valueOnly) {
                      if (cell && (cell.value || <unknown>cell.value === 0)) {
                        if (
                          ...
                          if (isCustomDateTime(cell.format, false)) {
                            ...
                            cells[key as string] = cell.formattedText
                              .toString()
                              .trim();
                          }
                        } else {
                          cells[key as string] = getValueFromFormat(
                            context,
                            cell,
                            sRow,
                            i,
                        ...
                        if (typeof cellProp === 'string') {
                          if (
                            localeObj.decimal !== '.' &&
                            cellProp.includes(localeObj.decimal)
                          ) {
                            parsedNumVal = cellProp.replace(
                              localeObj.decimal,
                              '.'
                            );
                      ...
                      if (
                        (cell &&
                          (cell.formula || !isNullOrUndefined(cell.value))) ||
                        Object.keys(cells).length
                      ) {
                        if (i === dateValueForSpecificColIdx) {
                          cellProp = {
                            value: getValueFromFormat(
                              context,
                              cell,
                              sRow,
                              i,
            ...
            while (sRow <= indexes[2]) {
              ...
              i = indexes[1];
              while (i <= indexes[3]) {
                const eventArgs: CellInfoEventArgs = {
                ...
                if (
                  cellObj.formula &&
                  cellObj.formula.toUpperCase().includes('UNIQUE')
                ) {
                ...
                if (cellObj.colSpan > 1 && cellObj.rowSpan > 1) {
                  let cell: CellModel;
                  for (
                    let j: number = sRow, len: number = sRow + cellObj.rowSpan;
                    j < len;
                    j++
                  ) {
                    for (
                      let k: number = i, len: number = i + cellObj.colSpan;
                      k < len;
                      k++
                    ) {
                      if (j === sRow && k === i) {
                  ...
                  for (
                    let j: number = i + 1, len: number = i + cellObj.colSpan;
                    j < len;
                    j++
                  ) {
                    setCell(sRow, j, sheet, { colSpan: i - j }, true);
                    if (
                      sheet.rows[sRow as number] &&
                      sheet.rows[sRow as number].cells &&
                      sheet.rows[sRow as number].cells[j as number]
                    ) {
                      delete sheet.rows[sRow as number].cells[j as number]
                        .value;
                      delete sheet.rows[sRow as number].cells[j as number]
                        .formula;
                    }
                  ...
                  for (
                    let j: number = sRow + 1,
                      len: number = sRow + cellObj.rowSpan;
                    j < len;
                    j++
                  ) {
                ...
                (data as Map<string, CellModel>).set(
                  eventArgs.address,
                  cellObj
                );
                i++;
...
export function getValueFromFormat(
  ...
  if (cell) {
    ...
    if (cell.format) {
      ...
      return args.dateObj && args.dateObj.toString() !== 'Invalid Date'
        ? args.dateObj
        : getIntValueFromDate
        ? <string>args.value
        : typeof args.formattedText === 'string'
      ...
      if (isNumber(cellVal)) {
        ...
        if (cellVal.includes('.') || cellVal.length > 11) {
          const options: AutoDetectGeneralFormatArgs = {
            args: { value: cellVal, updateValue: true },
            fResult: cellVal,
            cellVal: cellVal,
            intl: intl,
          };
...
export function getModel(
  model: (SheetModel | RowModel | CellModel)[],
  idx: number
): SheetModel | RowModel | CellModel {
  let diff: number;
  let j: number;
  let prevIdx: number;
  if (
    isUndefined(model[idx as number]) ||
    !(model[idx as number] && model[idx as number].index === idx)
  ) {
    for (let i: number = 0; i <= idx; i++) {
      if (model && model[i as number]) {
        ...
        if (diff > 0) {
          model.forEach(
            (value: SheetModel | RowModel | CellModel, index: number) => {
...
export function processIdx(
  ...
  for (let i: number = 0; i < len; i++) {
    if (
      !isNullOrUndefined(model[i as number]) &&
      !isUndefined(model[i as number].index)
    ) {
    ...
    if (isSheet) {
      if ((model[i as number] as SheetModel).id < 1) {
        ...
        if ((model[i as number] as { properties: {} }).properties) {
          (model[i as number] as { properties: { id: number } }).properties.id =
            (model[i as number] as SheetModel).id;
        }
      }
      if (!(model[i as number] as SheetModel).name) {
        context.setSheetPropertyOnMute(
          model[i as number],
          'name',
          'Sheet' + getSheetNameCount(context)
        );
      }
      ...
      (model[i as number] as SheetModel).rows.forEach((row: RowModel) => {
        cellCnt = Math.max(
          cellCnt,
          (row && row.cells && row.cells.length - 1) || 0
        );
      });
      context.setSheetPropertyOnMute(model[i as number], 'usedRange', {
        rowIndex: (model[i as number] as SheetModel).rows.length
          ? (model[i as number] as SheetModel).rows.length - 1
          : 0,
        colIndex: cellCnt,
      });
    }
