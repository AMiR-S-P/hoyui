...
export class WorkbookChart {
  ...
  private setChartHandler(args: {
    ...
    let i: number = 0;
    args.isInitCell = isNullOrUndefined(args.isInitCell)
      ? false
      : args.isInitCell;
    args.isUndoRedo = isNullOrUndefined(args.isUndoRedo)
      ? true
      : args.isUndoRedo;
      ...
      while (i < chart.length) {
        if (args.isCut === false) {
          if (document.getElementById(args.chart[i as number].id)) {
            chart[i as number] = {
              range: chart[i as number].range,
              id: getUniqueID('e_spreadsheet_chart'),
              theme: chart[i as number].theme,
              isSeriesInRows: chart[i as number].isSeriesInRows,
              type: chart[i as number].type,
        ...
        chartModel.range =
          chartModel.range || this.parent.getActiveSheet().selectedRange;
        const rangeIdx: number[] = getSwapRange(
          getRangeIndexes(chartModel.range)
        );
        const rangeAddress: string = getRangeAddress(rangeIdx);
        if (chartModel.range.indexOf('!') > 0) {
          chartModel.range =
            chartModel.range.split('!')[0] + '!' + rangeAddress;
        } else {
          chartModel.range =
            this.parent.getActiveSheet().name + '!' + rangeAddress;
        }
        ...
        if (!args.isInitCell || args.isPaste) {
          ...
          if (
            indexes[0] !== chartRowIdx.clientY ||
            indexes[1] !== chartColIdx.clientX
          ) {
    ...
    while (j < sheetCnt) {
      ...
      if (chartCnt) {
        while (chartCnt--) {
          ...
          if (
            !isNullOrUndefined(args.overlayEle.querySelector('#' + chart.id))
          ) {
            const chartObj: HTMLElement = this.parent.element.querySelector(
              '.' + chart.id
            );
            const excelFilter: { height: string; width: string } =
              getComponent(chartObj, 'chart') ||
              getComponent(chartObj, 'accumulationchart');
            if (excelFilter) {
  ...
  private focusChartBorder(args: { id: string }): void {
    for (let idx: number = 0; idx < this.parent.chartColl.length; idx++) {
      ...
      const chartEle: HTMLElement = document.getElementById(
        this.parent.chartColl[idx as number].id
      );
      if (
        overlayEle &&
        chartEle &&
        closest(chartEle, '.' + overlayEle.classList[1]) === overlayEle
      ) {
        this.parent.notify(initiateChart, {
          option: this.parent.chartColl[idx as number],
          isRefresh: true,
        });
      }
    }
