...
import { isNullOrUndefined, getNumericObject } from '@hoyui/base';
...
import {
  ...
  DefineName,
  selectionComplete,
  DefinedNameEventArgs,
  getRangeIndexes,
  InvalidFormula,
  getSwapRange,
} from '../common/index';
import {
  FormulaCalculateArgs,
...
export class WorkbookFormula {
  ...
  private init(): void {
    ...
    this.initCalculate();
    this.registerSheet();
    this.parent.customFormulaCollection.forEach(
      (value: IFormulaColl, key: string) => {
  ...
  public destroy(): void {
    ...
    if ((this.parent as any).refreshing) {
      ...
      const formulaCollect: Map<string, IFormulaColl> =
        this.calculateInstance.getLibraryFormulas();
      formulaCollect.forEach((value: IFormulaColl, key: string) => {
        if (value.isCustom) {
          this.parent.customFormulaCollection.set(key, {
  ...
  private addEventListener(): void {
    ...
    this.parent.on(
      clearFormulaDependentCells,
      this.clearFormulaDependentCells,
      this
    );
    ...
    this.parent.on(
      getUpdatedFormulaOnInsertDelete,
      this.getUpdatedFormulaOnInsertDelete,
      this
    );
  ...
  private initCalculate(): void {
    ...
    const decimalSeparator: string = (
      getNumericObject(this.parent.locale) as LocaleNumericSettings
    ).decimal;
    if (
      decimalSeparator !== '.' &&
      this.parent.listSeparator !== decimalSeparator
    ) {
      this.calculateInstance.setParseDecimalSeparator(decimalSeparator);
    }
  }

  private clearFormulaDependentCells(args: {
    cellRef?: string;
    isOpen?: boolean;
  }): void {
    ...
    const family: CalcSheetFamilyItem =
      this.calculateInstance.getSheetFamilyItem(sheetId);
    if (
      family.isSheetMember &&
      !isNullOrUndefined(family.parentObjectToToken)
    ) {
  ...
  private formulaInValidation(args: InvalidFormula): void {
    const col: IFormulaColl = this.calculateInstance
      .getLibraryFormulas()
      .get(args.value);
  ...
  private performFormulaOperation(args: { [key: string]: Object }): void {
    ...
    const family: CalcSheetFamilyItem =
      this.calculateInstance.getSheetFamilyItem(args.sheetId);
    switch (action) {
      ...
      case 'dropDownSelectFormulas':
        for (let i: number = 0; i < Array.from(formulas.values()).length; i++) {
          if (args.selectCategory === formulaInfo[i as number].category) {
            args.formulaCollection[i as number] = Array.from(formulas.keys())[
              i as number
            ];
          }
        }
      ...
      case 'registerSheet':
        ...
        if (args.isImport) {
          this.calculateInstance.setParseArgumentSeparator(
      ...
      case 'unRegisterSheet':
        this.unRegisterSheet(
          ...
          <number>args.sheetCount,
          <boolean>args.propertyChange
        );
        break;
      case 'initSheetInfo':
        this.updateSheetInfo();
      ...
      case 'setArgumentSeparator':
        ...
        break;
      case 'addDefinedName':
        args.isAdded = this.addDefinedName(
          <DefineNameModel>args.definedName,
          false,
          <number>args.index,
          <boolean>args.isEventTrigger
        );
        break;
      case 'removeDefinedName':
        args.isRemoved = this.removeDefinedName(
          <string>args.definedName,
          <string>args.scope,
          <boolean>args.isEventTrigger
        );
        break;
      ...
      case 'addSheet':
        this.sheetInfo.push({
          ...
          sheet: <string>args.sheetName,
          index: <number>args.sheetId,
        });
        break;
      case 'getSheetInfo':
      ...
      case 'deleteSheetTab':
        for (let i: number = 0; i < this.sheetInfo.length; i++) {
          if (this.sheetInfo[i as number].index === <number>args.sheetId) {
            ...
            const sheetName: string = this.sheetInfo[i as number].sheet;
      ...
      case 'addCustomFunction':
        this.addCustomFunction(
          <string | Function>args.functionHandler,
          <string>args.functionName,
          <string>args.formulaDescription
        );
        break;
      case 'computeExpression':
        args.calcValue = this.calculateInstance.computeExpression(
          <string>args.formula,
          <boolean>args.isFromComputeExpression
        );
        break;
      ...
      case 'checkFormulaAdded':
        ...
        if (action === 'checkFormulaAdded') {
          args.added = this.calculateInstance
            .getFormulaInfoTable()
            .has(<string>args.address);
        } else {
          args.isAvailable = this.calculateInstance
            .getDependentCells()
            .has(<string>args.address);
        }
        break;
      case 'calculateNow':
        this.calculateNow(
          <{ sheets: SheetModel[]; promise: Promise<object> }>args
        );
        break;
    }
  }
  private definedNamesDeletion(sheetName: string): void {
    ...
    if (definedNames && definedNames.length > 0) {
      for (let i: number = definedNames.length - 1; i >= 0; i--) {
        if (
          definedNames[i as number].refersTo
            .split('=')[1]
            .split('!')[0]
            .split("'")
            .join('') === sheetName
          ...
          this.removeDefinedName(
            definedNames[i as number].name,
            definedNames[i as number].scope
          );
        }
      }
    }
  ...
  private addCustomFunction(
    ...
    this.calculateInstance.defineFunction(
      functionName,
      functionHandler,
      formulaDescription
    );
  ...
  private sheetDeletion(delSheetName: string, sheetId: string): void {
    const dependentCell: Map<string, string[]> =
      this.calculateInstance.getDependentCells();
    ...
    const family: CalcSheetFamilyItem =
      this.calculateInstance.getSheetFamilyItem(sheetId);
    const definedNames: Map<string, string> =
      this.calculateInstance.namedRanges;
    let keyArray: string[];
    ...
    dependentCell.forEach((dependentCellRefs: string[], cellRef: string) => {
      dependentCellRefs.forEach((dependentCellRef: string): void => {
        ...
        if (!isNullOrUndefined(fInfo)) {
          ...
          if (sheetNameIdx > -1) {
            if (
              formulaVal[sheetNameIdx - 1] === "'" &&
              formulaVal[sheetNameIdx + prevSheetName.length] === "'"
            ) {
            ...
            formulaVal = formulaVal
              .split(prevSheetName)
              .join(this.referenceError());
            rowId = this.calculateInstance.rowIndex(dependentCellRef);
            colId = this.calculateInstance.colIndex(dependentCellRef);
            token = dependentCellRef.slice(
              0,
              dependentCellRef.lastIndexOf(this.calculateInstance.sheetToken) +
                1
            );
            this.updateDataContainer([rowId - 1, colId - 1], {
              value: formulaVal,
              visible: false,
              isNamedRange: namedRange,
              sheetId: family.tokenToParentObject.has(token)
                ? Number(family.tokenToParentObject.get(token))
        ...
        token = cellRef.slice(
          0,
          cellRef.lastIndexOf(this.calculateInstance.sheetToken) + 1
        );
        if (
          sheetId ===
          (family.tokenToParentObject.has(token)
            ? family.tokenToParentObject.get(token)
            : cellRef.split('!')[1])
        ) {
  ...
  private renameUpdation(name: string, pName: string): void {
    ...
    let cell: CellModel;
    const uPName: string = pName.toUpperCase();
    const escapeRegx: RegExp = new RegExp(
      '[!@#$%^&()+=\';,.{}|\\":<>~_-]',
      'g'
    );
    const exp: string = '(?=[\'!])(?=[^"]*(?:"[^"]*"[^"]*)*$)';
    ...
    const regx: RegExp = new regExp(
      pName.replace(escapeRegx, '\\$&') + exp,
      'gi'
    );
    const renameValidationSheetRef: Function = (
      validation: ValidationModel
    ): void => {
      if (
        checkIsFormula(validation.value1) &&
        validation.value1.toUpperCase().includes(uPName) &&
        validation.value1.match(regx)
      ) {
        validation.value1 = validation.value1.replace(regx, name);
      }
      if (
        checkIsFormula(validation.value2) &&
        validation.value2.toUpperCase().includes(uPName) &&
        validation.value2.match(regx)
      ) {
        validation.value2 = validation.value2.replace(regx, name);
      }
    };
    this.sheetInfo.forEach(
      (info: { visibleName: string }, index: number): void => {
        sheet = getSheet(this.parent, index);
        if (sheet && sheet.rows && sheet.rows.length) {
          for (
            let i: number = 0, rowLen: number = sheet.rows.length;
            i < rowLen;
            i++
          ) {
            if (sheet.rows[i as number] && sheet.rows[i as number].cells) {
              for (
                let j: number = 0,
                  cellsLen: number = sheet.rows[i as number].cells.length;
                j < cellsLen;
                j++
              ) {
                ...
                if (
                  ...
                  cell.formula.toUpperCase().includes(uPName) &&
        ...
        if (sheet && sheet.columns && sheet.columns.length) {
          ...
          for (
            let i: number = 0, colsLen: number = sheet.columns.length;
            i < colsLen;
            i++
          ) {
        ...
        for (let i: number = 0; i < definedNames.length; i++) {
          if (
            checkIsFormula(definedNames[i as number].refersTo) &&
            definedNames[i as number].refersTo.includes(pName) &&
            definedNames[i as number].refersTo.match(regx)
          ) {
            definedNames[i as number].refersTo = definedNames[
              i as number
            ].refersTo.replace(regx, name);
  ...
  private updateDataContainer(
    ...
    for (
      let i: number = 0, len: number = this.parent.sheets.length;
      i < len;
      i++
    ) {
      ...
      if (sheet.id === data.sheetId) {
        if (indexes[0] in sheet.rows) {
          ...
          if (indexes[1] in rowData.cells) {
            ...
            if (data.visible) {
              if (
                i === this.parent.activeSheetIndex &&
                sheet.activeCell === getCellAddress(indexes[0], indexes[1])
              ) {
  ...
  private parseSheetRef(value: string, addSheetQuotes?: boolean): string {
    ...
    let i: number = 0;
    const sheetInfo: { visibleName: string; sheet: string }[] =
      this.getSheetInfo();
    ...
    let idx: number;
    let valSearchIdx: number;
    let regxVisible: RegExp;
    const exp: string = '(?=[\'!])(?=[^"]*(?:"[^"]*"[^"]*)*$)';
    const regExp: RegExpConstructor = RegExp;
    for (i = 0; i < sheetCount; i++) {
      if (sheetInfo[i as number].sheet !== sheetInfo[i as number].visibleName) {
        regx = new regExp(
          sheetInfo[i as number].visibleName.replace(escapeRegx, '\\$&') + exp,
          'gi'
        );
        idx = i;
        if (value.match(regx)) {
          for (let j: number = i + 1; j < sheetCount; j++) {
            if (
              sheetInfo[j as number].visibleName.includes(
                sheetInfo[i as number].visibleName
              )
            ) {
              regxTemp = new regExp(
                sheetInfo[j as number].visibleName.replace(escapeRegx, '\\$&') +
                  exp,
                'gi'
              );
              ...
              if (
                searchIdx > -1 &&
                (searchIdx < valSearchIdx ||
                  (searchIdx === valSearchIdx &&
                    sheetInfo[j as number].visibleName.length >
                      sheetInfo[i as number].visibleName.length))
              ) {
                regxVisible = new RegExp('Sheet', 'gi');
                if (
                  sheetInfo[j as number].visibleName.search(regxVisible) !== 0
                ) {
                  regx = regxTemp;
                  idx = j;
                }
    ...
    while (i < temp.length) {
      ...
      sheetRef = addSheetQuotes
        ? '`' + sheetInfo[temp[i as number]].sheet + '`'
        : sheetInfo[temp[i as number]].sheet;
  ...
  private registerSheet(
    sheetIndex: number = 0,
    sheetCount: number = this.parent.sheets.length
  ): void {
    let id: string;
  ...
  private removeUniquecol(args?: { clearAll: boolean }): void {
    ...
    for (
      let i: number = 0;
      i < this.calculateInstance.uniqueRange.length;
      i++
    ) {
      const uniqRngAddress: string[] = this.calculateInstance.uniqueRange[
        i as number
      ]
        .split(':')[0]
        .split('!');
      if (
        uniqRngAddress[0] === sheet.name &&
        uniqRngAddress[1] === sheet.activeCell
      ) {
        const range: number[] = getRangeIndexes(
          this.calculateInstance.uniqueRange[i as number]
        );
  ...
  public refreshCalculate(args: FormulaCalculateArgs): void {
    args.sheet = isNullOrUndefined(args.sheetIndex)
      ? this.parent.getActiveSheet()
      : getSheet(this.parent, args.sheetIndex);
    const sheetId: string = args.sheet.id + '';
    const family: CalcSheetFamilyItem =
      this.calculateInstance.getSheetFamilyItem(sheetId);
    let cellRef: string =
      getColumnHeaderText(args.colIndex + 1) + (args.rowIndex + 1);
    if (
      family.isSheetMember &&
      !isNullOrUndefined(family.parentObjectToToken)
    ) {
    ...
    if (args.isFormula) {
      ...
      this.calculateInstance.refresh(
        cellRef,
        null,
        null,
        null,
    ...
    this.calculateInstance.cell = '';
    args.isFormulaDependent = this.calculateInstance
      .getDependentCells()
      .has(cellRef);
    if (args.value) {
  ...
  private calculateFormula(args: FormulaCalculateArgs, cellRef: string): void {
    ...
    const cellArgs: ValueChangedArgs = new ValueChangedArgs(
      args.rowIndex + 1,
      args.colIndex + 1,
      value
    );
    const usedRange: number[] = [
      ...
      if (formulaStr) {
        ...
        if (formula.includes(')')) {
          ...
          while (formula.includes('(') && formula.includes(')')) {
            ...
            formula =
              formula.slice(0, idx) +
              (fStr.includes(')') ? fStr.slice(fStr.indexOf(')') + 1) : fStr);
          }
        }
        const cellRefArr: string[] = formula.split(
          this.calculateInstance.getParseArgumentSeparator()
        );
        let cellRef: string;
        let fCell: CellModel;
        ...
        const depCells: Map<string, string> = this.calculateInstance
          .getDependentFormulaCells()
          .get(cellRef);
        if (
          depCells &&
          depCells.size &&
          this.calculateInstance.getFormulaInfoTable().has(cellRef) &&
          this.calculateInstance
            .getFormulaInfoTable()
            .get(cellRef)
            .getParsedFormula()
            .lastIndexOf('q') === -1 &&
          ...
          let idx: number[];
          const family: CalcSheetFamilyItem =
            this.calculateInstance.getSheetFamilyItem(null, this.calcID);
              ...
              if (isCellReference(cellRef)) {
                ...
                if (family.tokenToParentObject.has(sheetRef)) {
                  sheetIdx = getSheetIndexFromId(
                    this.parent,
                    Number(family.tokenToParentObject.get(sheetRef))
                  );
                  model =
                    sheetIdx !== undefined
                      ? getSheet(this.parent, sheetIdx)
  ...
  private refreshRandomFormula(): void {
    ...
    if (
      this.calculateInstance.randomValues.size > 1 &&
      this.calculateInstance.randomValues.size === referenceCollection.length
    ) {
      for (
        let i: number = 0;
        i < this.calculateInstance.randomValues.size;
        i++
      ) {
        rowId = this.calculateInstance.rowIndex(
          referenceCollection[i as number]
        );
        colId = this.calculateInstance.colIndex(
          referenceCollection[i as number]
        );
        refValue = this.calculateInstance.randomValues.get(
          referenceCollection[i as number]
        );
        const sheetId: string = (
          parseFloat(
            this.calculateInstance
              .getSheetToken(referenceCollection[i as number])
              .split(this.calculateInstance.sheetToken)
              .join('')
          ) + 1
        ).toString();
        const tempArgs: ValueChangedArgs = new ValueChangedArgs(
          rowId,
          colId,
          refValue
        );
  ...
  private autoCorrectFormula(
    args: { value?: string; rowIndex?: number; colIndex?: number },
    sheet: SheetModel
  ): void {
  ...
  private correctCellReference(cellRef: string): {
    isInvalid: boolean;
    ref: string;
  } {
    const cellRefArr: string[] = cellRef.split(':');
    let refArr: string[];
    let sheetRefArr: string[];
    let oprMatchArr: RegExpMatchArray;
    let isInvalid: boolean;
    let updatedRef: string;
    cellRefArr.forEach((cellAddr: string, idx: number): void => {
      ...
      if (cellRef.includes('&')) {
        refArr = cellRef.split('&');
        if (
          this.calculateInstance.isCellReference(refArr[1].split('$').join(''))
        ) {
          refArr[1] = this.getUpdatedCellRef(refArr[1]);
          updatedRef = refArr.join('&');
        }
      } else if (
        this.calculateInstance.isCellReference(cellRef.split('$').join(''))
      ) {
        ...
        if (oprMatchArr) {
          ...
          for (let refIdx: number = 0; refIdx < refArr.length; refIdx++) {
            ...
            if (
              this.calculateInstance.isCellReference(
                cellRef.split('$').join('')
              )
            ) {
              refArr[refIdx as number] = this.getUpdatedCellRef(cellRef);
              if (sheetRefArr.length > 1) {
                refArr[refIdx as number] =
                  sheetRefArr[0] + '!' + refArr[refIdx as number];
              }
            }
          }
  ...
  private autoCorrectCellRef(args: {
    formula: string;
    isInvalid?: boolean;
  }): string {
    ...
    if (rightParens > -1 && args.formula.split(')').length === 2) {
      ...
      if (leftParens > -1) {
        const formulaArgs: string = args.formula.substring(
          leftParens + 1,
          rightParens
        );
        ...
        for (
          let argsIdx: number = 0;
          argsIdx < formulaArgsArr.length;
          argsIdx++
        ) {
          refCorrectObj = this.correctCellReference(
            formulaArgsArr[argsIdx as number]
          );
        ...
        if (isInValidRef) {
          args.formula = args.formula
            .split(formulaArgs)
            .join(formulaArgsArr.join(listSeparator));
          args.isInvalid = true;
        }
      }
    } else if (args.formula.startsWith('=') && !args.formula.includes(')')) {
      refCorrectObj = this.correctCellReference(
        args.formula.substring(1, args.formula.length)
      );
      if (refCorrectObj.isInvalid) {
  ...
  private getUpdatedCellRef(cellRef: string): string {
    ...
    if (
      (isAbsolute ? digitStartIdx > 1 : digitStartIdx > 0) &&
      isNumber(cellRef.substring(digitStartIdx, cellRef.length))
    ) {
      return orgCellRef;
    } else {
      return (
        cellRef.substring(alphabetStartIdx, cellRef.length) +
        cellRef.substring(0, alphabetStartIdx)
      );
    }
  }
  ...
  private initiateDefinedNames(): void {
    ...
    while (i < definedNames.length) {
      ...
      const isLink: boolean =
        refersTo.indexOf('http:') > -1
          ? true
          : refersTo.indexOf('https:') > -1
          ? true
          : false;
      ...
      if (range.indexOf(':') > -1) {
        ...
        if (
          (isCellReference(rangeSplit[0]) && isCellReference(rangeSplit[1])) ||
          (rangeSplit[0].match(/[0-9]/) && rangeSplit[1].match(/[0-9]/)) ||
          (rangeSplit[0].toUpperCase().match(/[A-Z]/) &&
            rangeSplit[1].toUpperCase().match(/[A-Z]/))
        ) {
          cellRef = true;
        }
  ...
  private addDefinedName(
    ...
    if (definedName.refersTo.indexOf('!') < 0) {
      ...
      sheetName =
        sheetName.indexOf(' ') !== -1 ? "'" + sheetName + "'" : sheetName;
      definedName.refersTo =
        sheetName +
        '!' +
        (definedName.refersTo.indexOf('=') < 0
          ? definedName.refersTo
          : definedName.refersTo.split('=')[1]);
    }
    ...
    if (
      !isValidate &&
      this.checkIsNameExist(definedName.name, definedName.scope)
    ) {
      isAdded = false;
    } else {
      this.calculateInstance.addNamedRange(
        name,
        refersTo[0] === '=' ? refersTo.substr(1) : refersTo
      );
      if (refersTo[0] !== '=') {
    ...
    const eventArgs: DefinedNameEventArgs = {
  ...
  private checkIsNameExist(name: string, sheetName?: string): boolean {
    const isExist: boolean = this.parent.definedNames.some(
      (key: DefineNameModel) => {
        return (
          key.name === name &&
          (sheetName ? key.scope === sheetName : key.scope === '')
        );
      }
    );
    return isExist;
  ...
  private calculateNow(args: {
    ...
    const initCalculate: () => void = (): void => {
      ...
      let sheetId: string;
      let cellRef: string;
      const options: FormulaCalculateArgs = { isRefreshing: true };
      const formulaInfo: Map<string, FormulaInfo> =
        this.calculateInstance.getFormulaInfoTable();
      args.sheets.forEach((sheet: SheetModel): void => {
        ...
        token = family.isSheetMember
          ? family.parentObjectToToken.get(sheetId)
          : '';
        options.sheet = sheet;
          ...
          if (row && row.cells) {
            row.cells.forEach((cell: CellModel, colIdx: number): void => {
              if (cell && checkIsFormula(cell.formula)) {
                cellRef =
                  token + getColumnHeaderText(colIdx + 1) + (rowIdx + 1);
                if (
                  cell.value === undefined ||
                  cell.value === null ||
                  !formulaInfo.has(cellRef) ||
                  (dependentCells && dependentCells[cellRef as string])
                ) {
    ...
    const getDependentCellsCheckFn: (sheet: SheetModel) => Function = (
      sheet: SheetModel
    ): ((rowIdx: number, colIdx: number) => void) => {
      const sheetId: string = sheet.id.toString();
      const family: CalcSheetFamilyItem =
        this.calculateInstance.getSheetFamilyItem(sheetId);
      const token: string = family.isSheetMember
        ? family.parentObjectToToken.get(sheetId)
        : '';
      dependentCells = {};
      const dependentCellsInfo: Map<string, string[]> =
        this.calculateInstance.getDependentCells();
  ...
  private aggregateComputation(args: AggregateArgs): void {
    ...
    if (
      indexes[0] + 1 === sheet.rowCount &&
      indexes[1] + 1 === sheet.colCount
    ) {
      range = `A1:${getCellAddress(
        sheet.usedRange.rowIndex,
        sheet.usedRange.colIndex
      )}`;
    }
    ...
    let i: number;
    const cellCol: string | string[] =
      this.calculateInstance.getCellCollection(range);
    ...
    for (i = 0; i < 4; i++) {
      calcValue = this.toFixed(
        this.calculateInstance.getFunction(formulaVal[i as number])(range)
      );
      if (cell.format) {
  ...
  private refreshInsertDelete(args: InsertDeleteEventArgs): void {
    const formulaDependentCells: Map<
      string,
      Map<string, string>
    > = this.calculateInstance.getDependentFormulaCells();
    let cell: CellModel;
    ...
    this.parent.sheets.forEach((sheet: SheetModel, index: number): void => {
      for (
        let i: number = 0, rowLen: number = sheet.usedRange.rowIndex;
        i <= rowLen;
        i++
      ) {
        for (
          let j: number = 0, colLen: number = sheet.usedRange.colIndex;
          j <= colLen;
          j++
        ) {
          cell = getCell(i, j, sheet, false, true);
          if (cell.formula && checkIsFormula(cell.formula)) {
            if (index === sheetIndex) {
              if (
                args.isInsert ||
                !(args.modelType === 'Row'
                  ? i >= args.startIndex && i <= args.endIndex
  ...
  private getUpdatedFormulaOnInsertDelete(args: {
    insertDeleteArgs: InsertDeleteEventArgs;
    cell: CellModel;
    row: number;
    col: number;
    sheetIdx: number;
  ...
  private updateFormula(
    ...
    let updated: boolean;
    let range: number[];
    let isRangeFormula: boolean;
    const containAlphabetAndDigit: RegExp = new RegExp(
      /^(?=.*[a-zA-Z])(?=.*\d)/g
    );
    let isValidCellReference: boolean;
    let isFullColumn: boolean;
    ...
    const getAddress: () => string = (): string => {
      ...
      if (index[0] !== index[2] || index[1] !== index[3]) {
        range +=
          ':' +
          (isAbsoluteRef ? '$' : '') +
          getColumnHeaderText(index[3] + 1) +
          (isAbsoluteRef ? '$' : '') +
          (index[2] + 1);
      }
      return range;
    };
    const formulaArr: string[] = this.parseFormulaArgument({
    ...
    const sheetInfo: { visibleName: string; sheet: string }[] =
      this.getSheetInfo();
    let sheetName: string;
    let refChanged: boolean;
    let isAbsoluteRef: boolean;
      ...
      if (this.calculateInstance.isCellReference(ref)) {
        ...
        if (pVal && pVal[pVal.length - 1] === '!') {
          ...
          sheetName =
            ...
            sheetInfo[sheetIdx as number].visibleName
              ? args.sheet.name
              : '`' + sheetInfo[sheetIdx as number].sheet + '`';
          if (pVal !== sheetName) {
            continue;
        ...
        updated = this.parent.updateRangeOnInsertDelete(
          args,
          index,
          isRangeFormula
        );
        ...
        if (updated) {
          formulaArr[i as number] =
            range[2] < range[0] || range[3] < range[1]
              ? this.calculateInstance.getErrorStrings()[CommonErrors.Ref]
              : !isValidCellReference
              ? isFullColumn
                ? getRangeAddress(index).replace(/\d/g, '')
                : getRangeAddress(index).replace(/[a-zA-Z]/g, '')
              : getAddress();
          refChanged = true;
        }
      }
    ...
    if (refChanged) {
      ...
      const regExp: RegExpConstructor = RegExp;
      sheetInfo.forEach(
        (info: { visibleName: string; sheet: string }): void => {
  ...
  private parseFormulaArgument(args: {
    formula: string;
    rangeRef?: boolean;
    formulaArr?: string[];
  }): string[] {
    ...
    let i: number = 0;
    const arr: string[] = [];
    let formulaVal: string[] | string = this.markSpecialChar(
      args.formula.replace('=', ''),
      args.rangeRef
    );
    const regExp: RegExpConstructor = RegExp;
    const validCharRegx: RegExp = new regExp(
      args.rangeRef
        ? /\(|\)|=|\^|>|<|\+|-|\*|\/|%|&/g
        : /\(|\)|=|\^|>|<|:|\+|-|\*|\/|%|&/g
    );
    const sepRegx: RegExp = new regExp(this.parent.listSeparator, 'g');
    formulaVal = formulaVal.split(
      new regExp(validCharRegx.source + '|' + sepRegx.source, 'g')
    );
  ...
  private markSpecialChar(formula: string, rangeRef?: boolean): string {
    formula = formula
      .replace(/\(/g, '(' + this.uniqueOBracket)
      .replace(/\)/g, ')' + this.uniqueCBracket);
    const regEx: RegExpConstructor = RegExp;
    if (rangeRef) {
      formula = formula.replace(
        new regEx(this.parent.listSeparator, 'g'),
        this.parent.listSeparator + this.uniqueCSeparator
      );
    } else {
      formula = formula
        .replace(
        ...
        .replace(/:/g, `:${this.uniqueCOperator}`);
    }
    formula = formula
      .replace(/\+/g, '+' + this.uniquePOperator)
      .replace(/-/g, '-' + this.uniqueSOperator);
    formula = formula
      .replace(/\*/g, '*' + this.uniqueMOperator)
      .replace(/\//g, '/' + this.uniqueDOperator);
    ...
    formula = formula
      .replace(/>/g, '>' + this.uniqueGTOperator)
      .replace(/</g, '<' + this.uniqueLTOperator);
  ...
  private refreshNamedRange(args: InsertDeleteEventArgs): void {
    ...
    const definedNames: DefineNameModel[] = Object.assign(
      {},
      this.parent.definedNames
    );
    let range: number[];
    let sheetName: string;
    let splitedRef: string[];
    ...
    for (let i: number = 0; i < len; i++) {
      ...
      if (
        !containAlphabetAndDigit.test(splitedRef[1]) &&
        splitedRef[1].indexOf(':') > -1
      ) {
        isValidCellReference = false;
        isFullColumn = isNullOrUndefined(splitedRef[1].match(/[0-9]/))
          ? true
          : false;
      }
      ...
      if (!isValidCellReference) {
        rangeAddress = isFullColumn
          ? getRangeAddress(range).replace(/\d/g, '')
          : getRangeAddress(range).replace(/[a-zA-Z]/g, '');
      } else {
  ...
  private updateDefinedNames(
    ...
    if (idx) {
      let oldDefinedName: DefineNameModel = {
        name: definedName.name,
        comment: definedName.comment,
        refersTo: definedName.refersTo,
        scope: definedName.scope,
    ...
    const refreshArgs: DefinedNameEventArgs = {
      name: definedName.name,
      scope: definedName.scope,
      comment: definedName.comment,
      refersTo: definedName.refersTo,
