...
export class WorkbookAutoFill {
  ...
  private getFillInfo(options: {
    ...
    while (i--) {
      val =
        data[i as number] && !isNullOrUndefined(data[i as number].value)
          ? data[i as number].value
          : '';
      if (
        isNumber(val) ||
        checkIsFormula(val) ||
        isNumber(val[val.length - 1])
      ) {
        isStringType = false;
    ...
    if (!len || isStringType) {
      ...
      fillType =
        options.fillType === 'FillSeries' ? fillType : options.fillType;
    }
    if (
      !isVFill ||
      (isVFill && options.dataRange[1] !== options.dataRange[3])
    ) {
  ...
  private autoFill(options: {
    ...
    if (
      ...
      !options.fillRange ||
      !options.direction ||
      !this.parent.allowEditing ||
      (this.parent.getActiveSheet().isProtected && options.isLockedCell)
    ) {
      return;
    }
    const dataRangeIndices: number[] = getSwapRange(
      getRangeIndexes(options.dataRange)
    );
    const fillRangeIndices: number[] = getSwapRange(
      getRangeIndexes(options.fillRange)
    );
    const autoFillOptions: {
      ...
      sheetIndex: getSheetIndexFromAddress(this.parent, options.dataRange),
    };
    ...
    this.fillInfo.fillType = options.isFillOptClick
      ? options.fillType
      : this.fillInfo.fillType;
  ...
  private fillSeries(options: {
    ...
    let tot: number;
    let hasRef: boolean;
    let cells: { rowIndex: number; colIndex: number }[];
    let clen: number;
    let cellIdx: { rowIndex: number; colIndex: number };
    let cellProps: CellModel = {};
    let i: number = 0;
    let prevCellData: CellModel;
    let dateVal: Date;
    let dateObj: Date;
    const sheetIndex: number = isUndefined(options.sheetIndex)
    ...
    const isVFill: boolean = ['Down', 'Up'].indexOf(options.direction) > -1;
    const isReverseFill: boolean =
      ['Up', 'Left'].indexOf(options.direction) > -1;
    const len: number = isVFill ? dmaxc - dminc : dmaxr - dminr;
    const withFrmt: boolean = options.fillType === 'FillSeries';
    ...
    while (i <= len) {
      ...
      cells = this.getSelectedRange(
        sheet,
        { rowIndex: fillRange[0], colIndex: fillRange[1] },
        ...
        switch (patrn['type']) {
          ...
          case 'date':
            ...
            if (patrn.isStartWithMonth && dlen === 1) {
              ...
              if (
                dateObj.getDate() > 28 &&
                dateObj.getDate() !== dateVal.getDate()
              ) {
                dateObj.setDate(1);
              ...
              val = this.round(
                patrn['regVal'].a + patrn['regVal'].b * patrn['i'],
                5
              ).toString();
              if (patrn.val) {
                match =
                  typeof patrn.val[0] === 'string' && patrn.val[0].match(/^0+/);
                if (match) {
                  nextStringValue = this.getNextFormattedValue(
                    patrn.val[0],
                    Number(val)
                  );
                }
            ...
            if (patrn.dataVal) {
              if (patrn.copy === undefined && !match) {
                ...
                if (patrn.copy) {
                  for (
                    let m: number = 2;
                    m < (patrn.val as number[]).length;
                    m++
                  ) {
                    patrn.copy =
                      Math.abs(
                        this.round(patrn['regVal'].a + patrn['regVal'].b * m, 5)
              ...
              val = patrn.copy
                ? data[l as number] &&
                  !isNullOrUndefined(data[l as number].value)
                  ? data[l as number].value
                  : ''
                : patrn.start
                ? Math.abs(Number(val)) + patrn.dataVal
                : match
                ? patrn.dataVal + nextStringValue
            ...
            while (k < tlen) {
              ...
              if (typeof temp === 'object') {
                ...
                if (tot < 1) {
                  ...
                  val += isVFill
                    ? temp['c'] + (temp['b'] ? tot : '$' + tot)
                    : (temp['b']
                        ? getColumnHeaderText(tot)
                        : (temp['c'] as string).substring(
                            0,
                            (temp['c'] as string).search(/\d/)
                          ) +
                          '$' +
                          getColumnHeaderText(tot)) +
                      (temp['c'] as string).trim();
                }
              } else {
                val += temp;
              }
  ...
  private updateFillValues(
    ...
    fmaxc: number,
    i: number
  ): { patternRange: number[]; fillRange: number[] } {
  ...
  private getDataPattern(
    ...
    let isStartNum: boolean;
    let isDateStartsWithMonth: boolean;
    if (dlen) {
      let count: number;
      let dataVal: string;
      let format: string;
      let isNumVal: boolean;
      const minusOperator: Function = (data: string): string => {
        return !isStartNum && data && data[data.length - 1] === '-'
      ...
      while (i < dlen) {
        ...
        if (data[i as number]) {
          if (
            data[i as number].formula &&
            checkIsFormula(data[i as number].formula)
          ) {
            val = data[i as number].formula;
            type = 'formula';
          } else {
            val = isNullOrUndefined(data[i as number].value)
              ? ''
              : data[i as number].value;
            ...
            if (format && isCustomDateTime(format, true, option)) {
              ...
              isDateStartsWithMonth =
                type === 'date' && format.toLowerCase().startsWith('mmm');
            } else {
        ...
        if (type === 'string') {
          ...
          if (isNumber(val[0])) {
            ...
            if (val[count as number] === ' ') {
              isStartNum = true;
              type = 'number';
              dataVal = (val as string).slice(count, (val as string).length);
              val = Number((val as string).slice(0, count));
            }
          }
          val = val as string;
          if (!isStartNum && isNumber(val[val.length - 1])) {
            ...
            if (
              obj.dataVal &&
              obj.dataVal !== dataVal &&
              obj.dataVal === minusOperator(dataVal)
            ) {
              dataVal = obj.dataVal;
            }
          }
        ...
        if (i === 0) {
          ...
          type === obj.type &&
          (!obj.dataVal ||
            minusOperator(obj.dataVal) === minusOperator(dataVal))
        ) {
  ...
  private getPredictionValue(
    ...
    if (!isTime) {
      a = this.round(
        (sumy * sumxx - sumx * sumxy) / (n * sumxx - sumx * sumx),
        5
      );
        ...
        switch (patrn.type) {
          ...
          case 'date':
            ...
            if (len === 1) {
              ...
              if (
                typeof patrn.val[0] === 'string' &&
                patrn.val[0].match(/^0+/)
              ) {
                (patrn.val as string[]).push(
                  this.getNextFormattedValue(patrn.val[0] as string, newVal)
                );
              } else {
                (patrn.val as number[]).push(newVal);
              }
            }
            regVal = this.getPredictionValue(
              patrn.dataVal
                ? (patrn.val as number[]).slice(0, 2)
                : (patrn.val as number[])
            );
            temp = {
              regVal: regVal,
          ...
          case 'formula':
            ...
            if (patrn.isInPattern) {
              ...
              while (j < len) {
                (pattern as PatternInfo[]).push({
                  val: patrn.val[j as number],
                  type: 'formula',
                  i: diff,
                });
                j++;
          ...
          case 'time':
            ...
            if (len === 1) {
              ...
              const newTimeVal: number = dateToInt(
                new Date(oldTimeVal.getTime() + 60 * 60000),
                true,
                isTimeOnly
              );
  ...
  private getNextFormattedValue(value: string, numValue: number): string {
    const val: string = new Internationalization().formatNumber(
      Math.abs(numValue),
      { minimumIntegerDigits: value.length, useGrouping: false }
    );
    const numeArgs: { isNumber: boolean; value: string } =
      checkIsNumberAndGetNumber({ value: val }, this.parent.locale);
    return numeArgs.isNumber ? numeArgs.value : val;
  }

  private isCellReference(text: string): string | boolean {
    return /^[a-z]{1,3}\d{1,7}$/gi.test(text)
      ? 'relative'
      : /^\$[a-z]{1,3}\$\d{1,7}$/gi.test(text)
      ? 'absolute'
      : /^((\$[a-z]{1,3})\d{1,7}|[a-z]{1,3}(\$\d{1,7}))$/gi.test(text)
      ? 'mixed'
      : false;
  }
  ...
  private round(value: string | number, round: string | number): number {
    return (
      Number(Math.round(parseFloat(value + 'e' + round)) + 'e-' + round) ||
      Number(value)
    );
  }
    ...
    if (isInPatrn) {
      ...
      while (i--) {
        ...
        if (cRfrType && cRfrType !== 'absolute') {
          patrn[i as number] = this.getCellRefPrediction(
            temp,
            options,
            null,
            cRfrType
      ...
      while (i < len) {
        ...
        while (j--) {
          ...
          if (cRfrType && cRfrType !== 'absolute') {
            patrns[i as number][j as number] = this.getCellRefPrediction(
              temp,
              options,
              len,
              cRfrType
  ...
  private generateColCount(text: string): number {
    ...
    for (let i: number = newText.length - 1; i >= 0; i--) {
      colCount +=
        (text[i as number].charCodeAt(parseInt(newText[i as number], 10)) -
          64) *
        Math.pow(26, i);
    }
    return colCount;
  ...
  private getCellRefPrediction(
    ...
    const nRegx: RegExp = new RegExp('[0-9$]', 'g');
    let str: string = options.isVFill
      ? text.replace(nRegx, eStr)
      : text.replace(aRegx, eStr);
    let temp: number[] | number | object = options.isVFill
      ? Number(text.replace(aRegx, eStr))
    ...
    if (rfrType && rfrType === 'mixed') {
      if (isColAbslt === options.isVFill) {
        if (dollarPosition) {
          str =
            str.substring(0, dollarPosition) +
            '$' +
            str.substring(dollarPosition);
        } else {
          str = '$' + str;
        }
      } else {
  ...
  private isInPattern(patrn: string[], isVFill: boolean): boolean {
    ...
    let i: number = 0;
    let j: number = 1;
    const plen: number = patrn.length;
    const nregx: RegExp = new RegExp('[0-9$]', 'g');
    const aregx: RegExp = new RegExp('[a-z$]', 'gi');
    if (plen === 1) {
      return false;
  ...
  private getSelectedRange(
    ...
    let k: number;
    let l: number;
    const arr: { rowIndex: number; colIndex: number }[] = [];
