...
export class WorkbookEdit {
    ...
    switch (action) {
      case 'updateCellValue':
        args.isFormulaDependent = this.updateCellValue(
          ...
          <boolean>args.isValueOnly,
          <boolean>args.skipFormatCheck,
          <boolean>args.isRandomFormula,
          <boolean>args.skipCellFormat,
          <boolean>args.isDelete,
          <number[]>args.deletedRange
        );
        break;
    }
  }
  ...
  private updateCellValue(
    ...
    let range: number[];
    let isFormulaDependent: boolean;
    ...
    if (!isValueOnly) {
      ...
      const isNotTextFormat: boolean =
        getTypeFromFormat(cell.format) !== 'Text' &&
        (!isFormula || !value.toLowerCase().startsWith('=text('));
      ...
      if (isNotTextFormat && !skipFormatCheck) {
        ...
        if (
          !isNullOrUndefined(dateEventArgs.updatedVal) &&
          (dateEventArgs.updatedVal as string).length > 0
        ) {
      ...
      if (value === '#SPILL!') {
        ...
        if (isFormula) {
          ...
          (cell.value.indexOf('www.') === 0 ||
            cell.value.indexOf('https://') === 0 ||
            cell.value.indexOf('http://') === 0 ||
            cell.value.indexOf('ftp://') === 0)
        ) {
