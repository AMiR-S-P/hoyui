...
import { extend, isNullOrUndefined, isUndefined } from '@hoyui/base';
...
export class WorkbookMerge {
  ...
  private merge(args: MergeArgs): void {
    ...
    if (!args.merge || args.type === 'All') {
      ...
      for (
        let rowIdx: number = args.range[0], endIdx: number = args.range[2];
        rowIdx <= endIdx;
        rowIdx++
      ) {
      ...
      for (
        let colIdx: number = args.range[1], endIdx: number = args.range[3];
        colIdx <= endIdx;
        colIdx++
      ) {
  ...
  private mergeAll(
    ...
    const sheet: SheetModel = isUndefined(args.sheetIndex)
      ? this.parent.getActiveSheet()
      : getSheet(this.parent, args.sheetIndex);
    const updateObj: CellUpdateArgs = {
      cell: new Object(),
      rowIdx: args.range[0],
      colIdx: args.range[1],
    ...
    for (
      let rowIdx: number = args.range[0], rIdx: number = startRow;
      rowIdx <= args.range[2];
      rowIdx++, rIdx++
    ) {
      colSpan = 0;
      ...
      for (
        let colIdx: number = args.range[1], cIdx: number = startCol;
        colIdx <= args.range[3];
        colIdx++, cIdx++
      ) {
        ...
        if (
          cell &&
          (cell.value || <unknown>cell.value === 0 || cell.formula) &&
          !cellValue
        ) {
  ...
  private activeCellRange(args: MergeArgs): void {
    ...
    if (cell) {
      ...
      if (cell) {
        if (
          cell.rowSpan > 1 &&
          (args.range[0] + (cell.rowSpan - 1) >= args.range[2] ||
            args.insertCount)
        ) {
        ...
        if (
          cell.colSpan > 1 &&
          (args.range[1] + (cell.colSpan - 1) >= args.range[3] ||
            args.insertCount)
        ) {
  ...
  private mergedRange(args: MergeArgs): void {
    ...
    if (args.range[0] <= args.range[2] && args.range[1] <= args.range[3]) {
      ...
      args.range[0] >= args.range[2] &&
      args.range[1] >= args.range[3]
    ) {
      this.reverse(args);
  ...
  private forward(args: MergeArgs): void {
    ...
    let rowIdx: number = (endRowIdx = args.range[0]);
    let colIdx: number = (endColIdx = args.range[1]);
    if (cell) {
      if (!isNullOrUndefined(cell.rowSpan) && cell.rowSpan < 0) {
    ...
    if (
      args.range[0] === rowIdx &&
      args.range[1] === colIdx &&
      args.range[2] === endRowIdx &&
      args.range[3] === endColIdx
    ) {
  ...
  private forwardReverse(args: MergeArgs): void {
    ...
    if (cell) {
      ...
      if (cell) {
        if (
          cell.rowSpan > 1 &&
          rowIndex + (cell.rowSpan - 1) >= args.range[2]
        ) {
          args.range[2] = rowIndex + (cell.rowSpan - 1);
        }
        if (
          cell.colSpan > 1 &&
          colIndex + (cell.colSpan - 1) >= args.range[1]
        ) {
    ...
    for (let i: number = args.range[3]; i <= args.range[1]; i++) {
      ...
      if (cell) {
        ...
        if (cell) {
          ...
          if (
            cell.colSpan > 1 &&
            cellIdx + (cell.colSpan - 1) > args.range[1]
          ) {
    ...
    for (let i: number = args.range[3]; i <= args.range[1]; i++) {
      ...
      if (cell) {
        ...
        if (cell) {
          if (
            cell.rowSpan > 1 &&
            startRowIndex + (cell.rowSpan - 1) > args.range[2]
          ) {
    ...
    for (let i: number = args.range[2]; i <= args.range[0]; i++) {
      ...
      if (cell) {
        ...
        if (cell) {
          ...
          if (
            cell.colSpan > 1 &&
            cellIndex + (cell.colSpan - 1) > args.range[1]
          ) {
  ...
  private reverseForward(args: MergeArgs): void {
    ...
    const sheet: SheetModel = isUndefined(args.sheetIndex)
      ? this.parent.getActiveSheet()
      : getSheet(this.parent, args.sheetIndex);
    let rIdx: number = args.range[0];
    let cIdx: number = args.range[1];
    let cell: CellModel = getCell(args.range[0], args.range[1], sheet);
    if (cell) {
      if (!isNullOrUndefined(cell.rowSpan) && cell.rowSpan < 0) {
        rIdx += cell.rowSpan;
    ...
    let cIndex: number = args.range[3];
    let rIndex: number;
    for (let i: number = args.range[1]; i <= args.range[3]; i++) {
      cell = getCell(args.range[2], i, sheet);
      if (cell) {
    ...
    for (let i: number = args.range[1]; i <= args.range[3]; i++) {
      ...
      if (cell) {
        ...
        if (cell) {
          if (
            cell.rowSpan > 1 &&
            sRowIdx + (cell.rowSpan - 1) > args.range[0]
          ) {
    ...
    for (let i: number = args.range[2]; i <= args.range[0]; i++) {
      ...
      if (cell) {
        ...
        if (cell) {
          ...
          if (
            cell.colSpan > 1 &&
            cell.colSpan - 1 + cellIndex > args.range[3]
          ) {
            args.range[3] = cellIndex;
