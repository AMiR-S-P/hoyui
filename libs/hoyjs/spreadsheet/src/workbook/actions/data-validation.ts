...
export class WorkbookDataValidation {
  ...
  private validationHandler(args: {
    range: string;
    rules?: ValidationModel;
    isRemoveValidation?: boolean;
    cancel?: boolean;
    viewport: object;
  }): void {
    ...
    this.parent.dataValidationRange =
      (this.parent.dataValidationRange.indexOf('!') > -1
        ? ''
        : sheet.name + '!') +
      this.parent.dataValidationRange +
      onlyRange +
      ',';
    const rangeInfo: { range: string; isFullCol: boolean } =
      this.getRangeWhenColumnSelected(onlyRange, sheet);
    ...
    if (
      !args.isRemoveValidation &&
      args.rules.type === 'List' &&
      args.rules.value1
    ) {
      args.rules.value1 = args.rules.value1.trim();
      if (
        args.rules.value1[args.rules.value1.length - 1] ===
        this.parent.listSeparator
      ) {
        args.rules.value1 = args.rules.value1.substring(
          0,
          args.rules.value1.length - 1
        );
      }
    }
    const activeCellIndex: number[] = getSwapRange(
          ...
          if (column && column.validation) {
            ...
            if (viewportIndexes) {
              viewportIndexes.forEach((viewportIndex: number[]) => {
                for (
                  let rowIdx: number = viewportIndex[0];
                  rowIdx <= viewportIndex[2];
                  rowIdx++
                ) {
                  ...
                  this.parent.notify(applyCellFormat, <CellFormatArgs>{
                    rowIdx: rowIdx,
                    colIdx: colIdx,
                    style: this.parent.getCellStyleValue(
          ...
          if (rowIdx === indexes[2]) {
            ...
            if (column && column.validation) {
              column.validation.address = getSplittedAddressForColumn(
                column.validation.address,
                [indexes[0], colIdx, indexes[2], colIdx],
                colIdx
              );
            }
          }
  ...
  private addHighlightHandler(args: {
    range: string;
    td?: HTMLElement;
    isclearFormat?: boolean;
  }): void {
  ...
  private invalidDataHandler(
    ...
    const sheetIdx: number = range
      ? getSheetIndexFromAddress(this.parent, range)
      : this.parent.activeSheetIndex;
    const sheet: SheetModel = getSheet(this.parent, sheetIdx);
    range = range || sheet.selectedRange;
    const sheetName: string = range.includes('!')
      ? getSheetNameFromAddress(range)
      : sheet.name;
    const rangeInfo: { range: string; isFullCol: boolean } =
      this.getRangeWhenColumnSelected(getRangeFromAddress(range), sheet);
              ...
              if (!isValid) {
                if (
                  ...
                  this.parent.notify(validationHighlight, {
                    isRemoveHighlightedData: isRemoveHighlightedData,
                    rowIdx: rowIdx,
                    colIdx: colIdx,
                    td: td,
                  });
  ...
  private beforeInsertDeleteHandler(args: InsertDeleteEventArgs): void {
    ...
    let sheet: SheetModel;
    for (
      let i: number = 0, sheetLen: number = this.parent.sheets.length;
      i < sheetLen;
      i++
    ) {
      sheet = this.parent.sheets[i as number];
      for (
        let j: number = 0, rowLen: number = sheet.rows.length;
        j < rowLen;
        j++
      ) {
        if (sheet.rows[j as number] && sheet.rows[j as number].cells) {
          for (
            let k: number = 0,
              cellLen: number = sheet.rows[j as number].cells.length;
            k < cellLen;
            k++
          ) {
            ...
            if (cell && cell.validation) {
              const isInsert: boolean =
                (args as { name: string }).name === 'beforeInsert';
              const endIndex: number = args.index + (args.model.length - 1);
              const isNewlyInsertedModel: boolean =
                args.modelType === 'Row'
                  ? j >= args.index && j <= endIndex
                  : k >= args.index && k <= endIndex;
              let eventArgs: {
              ...
              if (isInsert) {
                eventArgs = {
                  insertDeleteArgs: {
                    ...
                    endIndex: args.index + args.model.length - 1,
                    modelType: args.modelType,
                    isInsert: true,
                    sheet: getSheet(this.parent, args.activeSheetIndex),
                  },
                  row: j,
                  col: k,
                  sheetIdx: i,
                ...
                eventArgs = {
                  insertDeleteArgs: {
                    startIndex: (args as InsertDeleteModelArgs).start as number,
                    modelType: args.modelType,
                    endIndex: (args as InsertDeleteModelArgs).end,
                    sheet: args.model as SheetModel,
                  },
                  row: j,
                  col: k,
                  sheetIdx: i,
              ...
              if (args.activeSheetIndex === i && isInsert) {
                this.updateValidationForInsertedModel(
                  args,
                  sheet,
                  j,
                  k,
  ...
  private getRangeWhenColumnSelected(
    range: string,
    sheet: SheetModel
  ): { range: string; isFullCol: boolean } {
    ...
    if (range.match(/\D/g) && !range.match(/[0-9]/g)) {
      ...
      colNames[1] =
        getCellAddress(0, sheet.colCount - 1).replace(/[0-9]/g, '') +
        colNames[1];
      range = colNames[0] + ':' + colNames[1];
    }
  ...
  private updateValidationForInsertedModel(
    ...
    if (args.modelType === 'Column') {
      if (
        (args.insertType === 'before' && endIndex === colIndex - 1) ||
        (args.insertType === 'after' && args.index - 1 === colIndex)
      ) {
      ...
      if (
        (args.insertType === 'above' && endIndex === rowIndex - 1) ||
        (args.insertType === 'below' && args.index - 1 === rowIndex)
      ) {
        for (let l: number = args.index; l <= endIndex; l++) {
          setCell(
            l,
