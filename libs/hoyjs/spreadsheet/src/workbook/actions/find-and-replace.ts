...
import { isNullOrUndefined, isUndefined, getNumericObject } from '@hoyui/base';
import {
  findAllValues,
  FindAllArgs,
  workBookeditAlert,
  BeforeReplaceEventArgs,
  updateCell,
  beginAction,
} from '../common/index';
import {
...
export class WorkbookFindAndReplace {
  ...
  private find(args: FindOptionsArgs): void {
    args.sheetIndex = isUndefined(args.sheetIndex)
    ...
    const hdrPanel: HTMLElement =
      args.showDialog &&
      this.parent.element &&
      this.parent.element.querySelector('.e-header-panel');
    if (hdrPanel) {
      headerHgt = (hdrPanel.offsetHeight || (sheet.showHeaders ? 30 : 0)) + 1;
    }
    args.localeObj = <LocaleNumericSettings>(
  ...
  private findNext(args: FindOptions, findArgs: FindArgs): void {
        ...
        if (sheetIdx === findArgs.sheetIdx) {
          if (initIteration) {
            ...
            cellAddr = this.findNextOnSheet(
              ...
              findArgs.findVal,
              sheet,
              args.searchBy === 'By Row' ? findArgs.startRow : findArgs.startCol
            );
          }
        } else {
    ...
    let cellAddr: string = findOnSheet(
      findArgs.sheetIdx,
      findArgs.sheets.length - 1,
      true
    );
    if (!cellAddr) {
    ...
    if (args.searchBy === 'By Row') {
      ...
      for (rowIdx = startRow; rowIdx <= endIdx; rowIdx++) {
        ...
        colLen =
          ...
          sheet.rows[rowIdx as number].cells.length;
        for (colIdx; colIdx < colLen; colIdx++) {
          if (!isHiddenCol(sheet, colIdx)) {
            cellAddr = this.checkMatch(
              args,
              findVal,
              rowIdx,
              colIdx,
  ...
  private findPrevOnSheet(
    ...
    if (args.searchBy === 'By Row') {
      for (rowIdx = startRow; rowIdx >= endRow; rowIdx--) {
        ...
        colIdx =
          activeCell && rowIdx === startRow
            ? startCol
            : sheet.rows[rowIdx as number] &&
              sheet.rows[rowIdx as number].cells &&
              sheet.rows[rowIdx as number].cells.length - 1;
  ...
  private checkMatch(
    ...
    const checkValues: (cellVal: string) => string = (
      cellVal: string
    ): string => {
      if (cellVal) {
        if (!args.isCSen) {
    ...
    if (!cellAddr) {
      ...
      if (
        cell.format &&
        !isCustomDateTime(cell.format, true) &&
  ...
  public replace(args: FindOptions): void {
    ...
    let activeCellModel: CellModel = getCell(
      activeCell[0],
      activeCell[1],
      sheet,
      false,
    ...
    let compareVal: string = this.parent
      .getDisplayText(activeCellModel)
      .toString();
    ...
    const getReplaceValue: (isRecursive?: boolean) => string = (
      isRecursive?: boolean
    ): string => {
      ...
      if (args.isCSen) {
        if (args.isEMatch) {
          ...
          replaceVal =
            compareVal.indexOf(args.value) > -1 &&
            compareVal.replace(args.value, args.replaceValue);
        }
      } else {
        if (args.isEMatch) {
          replaceVal =
            compareVal.toLowerCase() === checkValue && args.replaceValue;
        } else {
          const regExp: RegExpConstructor = RegExp;
          replaceVal =
            compareVal.toLowerCase().includes(checkValue) &&
            compareVal.replace(
              new regExp(
      ...
      if (
        !isRecursive &&
        !replacedValue &&
        activeCellModel.format &&
        !isCustomDateTime(activeCellModel.format, true) &&
        !compareVal.includes('%')
      ) {
    ...
    if (
      isReadOnly(
        getCell(activeCell[0], activeCell[1], sheet),
        getColumn(sheet, activeCell[1]),
        getRow(sheet, activeCell[0])
      )
    ) {
      this.parent.notify(workbookReadonlyAlert, null);
      return;
    }
    const eventArgs: BeforeReplaceEventArgs & { sheetIndex: number } = {
  ...
  public replaceAll(args: FindOptions): void {
    ...
    const eventArgs: ReplaceAllEventArgs & FindOptions = {
      addressCollection: addressCollection,
      cancel: false,
      ...args,
    };
    const updateAsync: (cellValue: string, index: number) => void = (
      cellValue: string,
      index: number
    ): void => {
      if (requestAnimationFrame) {
        requestAnimationFrame(() => {
          if (
            !eventArgs.cancel &&
            eventArgs.addressCollection[index as number]
          ) {
            const indexes: number[] = getCellIndexes(
              eventArgs.addressCollection[index as number].split('!')[1]
            );
            const sheetIndex: number = getSheetIndexFromAddress(
              this.parent,
              eventArgs.addressCollection[index as number]
            );
            updateCell(this.parent, this.parent.sheets[sheetIndex as number], {
              cell: { value: cellValue },
              rowIdx: indexes[0],
              uiRefresh: true,
              colIdx: indexes[1],
              valChange: true,
              skipFormatCheck: (<{ skipFormatCheck?: boolean }>args)
                .skipFormatCheck,
            });
            if (
              index === eventArgs.addressCollection.length - 1 &&
              triggerEvent
            ) {
              this.parent.notify('actionComplete', {
                action: 'replaceAll',
    ...
    const checkMatch: (cellval: string) => boolean = (
      cellval: string
    ): boolean => {
      ...
      if (cellval) {
        if (args.isCSen) {
          if (args.isEMatch) {
            if (cellval === args.value) {
              ...
              addressCollection.push(
                sheet.name + '!' + getCellAddress(startRow, startColumn)
              );
              matchFound = true;
            }
          } else {
            if (cellval.indexOf(args.value) > -1) {
              updateAsync(
                cellval.replace(args.value, args.replaceValue),
                addressCollection.length
              );
              addressCollection.push(
                sheet.name + '!' + getCellAddress(startRow, startColumn)
              );
          ...
          if (args.isEMatch) {
            ...
            if (
              cellval === args.value ||
              val.indexOf(args.value.toString().toLowerCase()) > -1 ||
              val === args.value ||
              cellval === args.value ||
              val.indexOf(args.value) > -1
            ) {
              const regExp: RegExpConstructor = RegExp;
              regX = new regExp(
                args.value.toString().replace(/[.*+?^${}()|[\]\\]/g, '\\$&'),
                'ig'
              );
              updateAsync(
                cellval.replace(regX, args.replaceValue),
                addressCollection.length
              );
              addressCollection.push(
                sheet.name + '!' + getCellAddress(startRow, startColumn)
        ...
        for (startColumn; startColumn <= endColumn; startColumn++) {
          if (row) {
            ...
            if (cell) {
              ...
              if (
                !checkMatch(displayText) &&
                cell.format &&
                !isCustomDateTime(cell.format, true) &&
                !displayText.includes('%')
              ) {
                checkMatch(
                  this.getCellVal(row.cells[startColumn as number], localeObj)
                );
              }
            }
          }
    ...
    if (addressCollection.length && triggerEvent) {
      ...
      if (!eventArgs.cancel) {
        this.parent.notify(replaceAllDialog, {
          count: eventArgs.addressCollection.length,
  ...
  private getDisplayText(
    ...
    if (!cell.value && <unknown>cell.value !== 0) {
      if (cell.hyperlink) {
        return typeof cell.hyperlink === 'string'
          ? cell.hyperlink
  ...
  private totalCount(args: FindOptions): void {
    ...
    let displayText: string;
    sheet.rows.filter(
      (row: ExtendedRowModel, rowIdx: number) =>
        row &&
        row.cells &&
        !row.isFiltered &&
        !row.hidden &&
        row.cells.filter((cell: CellModel, colIdx: number) => {
          if (
            cell &&
            (cell.value || <unknown>cell.value === 0) &&
            ...
            if (
              displayText.includes(findValue) ||
              (cell.format &&
                !isCustomDateTime(cell.format, true) &&
                !displayText.includes('%') &&
                this.getCellVal(cell, localeObj).includes(findValue))
            ) {
              count++;
              if (
    ...
    if (
      count !== 0 &&
      !this.parent
        .getDisplayText(getCell(activeCell[0], activeCell[1], sheet))
        .toLowerCase()
        .includes(findValue)
    ) {
      count -= 1;
  ...
  private findAllValues(findAllArguments: FindAllArgs): void {
    let startSheet: number = findAllArguments.sheetIndex;
    let sheet: SheetModel = this.parent.sheets[startSheet as number];
    let endRow: number = sheet.usedRange.rowIndex;
    ...
    const initialSheet: number = findAllArguments.sheetIndex;
              ...
              if (row.cells && row.cells[columnIndex as number]) {
                ...
                if (
                  ...
                  if (findAllArguments.isCSen && findAllArguments.isEMatch) {
                    ...
                    findAllArguments.isCSen &&
                    !findAllArguments.isEMatch
                  ) {
                    const index: boolean =
                      cellvalue.indexOf(findAllArguments.value) > -1;
                    ...
                    const val: string = cellvalue.toString().toLowerCase();
                    const index: boolean =
                      val.indexOf(findAllArguments.value.toLowerCase()) > -1;
                    if (
                      val === findAllArguments.value ||
