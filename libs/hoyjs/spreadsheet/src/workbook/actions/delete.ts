...
export class WorkbookDelete {
  ...
  private deleteModel(args: InsertDeleteModelArgs): void {
    ...
    const eventArgs: InsertDeleteEventArgs = {
    ...
    const insertArgs: InsertDeleteEventArgs = {
      startIndex: args.start,
      endIndex: args.end,
      modelType: args.modelType,
      sheet: args.model,
    ...
    const isFinite: boolean = (
      this.parent as { scrollSettings: { isFinite: boolean } } & Workbook
    ).scrollSettings.isFinite;
    if (args.modelType === 'Row') {
      if (
        args.checkCount !== undefined &&
        args.checkCount === args.model.usedRange.rowIndex
      ) {
        return;
      }
      ...
      const curIdx: number = args.end + 1;
      let cell: CellModel;
      ...
      if (args.model.rows[args.start] && args.model.rows[args.start].cells) {
        for (let i: number = 0; i <= args.model.usedRange.colIndex; i++) {
          if (
            args.model.rows[args.start as number].cells[i as number] &&
            args.model.rows[args.start as number].cells[i as number].rowSpan !==
              undefined &&
            args.model.rows[args.start as number].cells[i as number].rowSpan <
              0 &&
            args.model.rows[args.start as number].cells[i as number].colSpan ===
              undefined
          ) {
            ...
            if (mergeArgs.range[2] <= args.end) {
              ...
              if (prevCell && prevCell.rowSpan > 1) {
                if (
                  prevCell.rowSpan - (mergeArgs.range[2] - args.start + 1) >
                  1
                ) {
                  setCell(
                    mergeArgs.range[0],
                    ...
                    args.model,
                    {
                      colSpan:
                        prevCell.rowSpan -
                        (mergeArgs.range[2] - args.start + 1),
                    },
                    true
                  );
                } else {
                  delete args.model.rows[mergeArgs.range[0]].cells[i as number]
                    .rowSpan;
                }
              }
              mergeArgs = null;
          ...
          if (
            args.model.rows[curIdx as number] &&
            args.model.rows[curIdx as number].cells &&
            args.model.rows[curIdx as number].cells[i as number] &&
            args.model.rows[curIdx as number].cells[i as number].rowSpan !==
              undefined &&
            args.model.rows[curIdx as number].cells[i as number].rowSpan < 0 &&
            args.model.rows[curIdx as number].cells[i as number].colSpan ===
              undefined
          ) {
            if (!mergeArgs) {
            ...
            Object.assign(
              cell,
              getCell(mergeArgs.range[0], mergeArgs.range[1], args.model)
            );
            if (
              cell &&
              cell.rowSpan &&
              ...
              if (mergeArgs.range[0] < args.start) {
                ...
                if (cell.rowSpan - (args.end - mergeArgs.range[0] + 1) > 1) {
                  indexes[2] +=
                    cell.rowSpan - (args.end - mergeArgs.range[0] + 1) - 1;
                }
              }
      ...
      if (
        args.checkCount !== undefined &&
        args.checkCount === args.model.usedRange.colIndex
      ) {
        return;
      }
      this.parent.notify(refreshInsertDelete, insertArgs);
      ...
      if (args.start <= args.model.usedRange.colIndex) {
        this.parent.setSheetPropertyOnMute(args.model, 'usedRange', {
          ...
          colIndex: args.model.usedRange.colIndex - count,
        });
      ...
      for (let i: number = 0; i <= args.model.usedRange.rowIndex; i++) {
        deletedCells.push({});
        if (
          args.model.rows[i as number] &&
          args.model.rows[i as number].cells
        ) {
          if (
            args.model.rows[i as number].cells[args.start] &&
            args.model.rows[i as number].cells[args.start].colSpan !==
              undefined &&
            args.model.rows[i as number].cells[args.start].colSpan < 0 &&
            ...
            if (mergeArgs.range[3] <= args.end) {
              const prevCell: CellModel = getCell(
                i,
                mergeArgs.range[1],
                args.model
              );
              if (prevCell && prevCell.colSpan > 1) {
                if (
                  prevCell.colSpan - (mergeArgs.range[3] - args.start + 1) >
                  1
                ) {
                  setCell(
                    i,
                    ...
                    args.model,
                    {
                      colSpan:
                        prevCell.colSpan -
                        (mergeArgs.range[3] - args.start + 1),
                    },
                    true
                  );
                } else {
                  delete args.model.rows[i as number].cells[mergeArgs.range[1]]
                    .colSpan;
                }
              }
              mergeArgs = null;
          ...
          if (
            args.model.rows[i as number].cells[curIdx as number] &&
            args.model.rows[i as number].cells[curIdx as number].colSpan !==
              undefined &&
            args.model.rows[i as number].cells[curIdx as number].colSpan < 0 &&
            args.model.rows[i as number].cells[curIdx as number].rowSpan ===
              undefined
          ) {
            if (!mergeArgs) {
            ...
            if (
              ...
              if (mergeArgs.range[1] < args.start) {
                ...
                if (cell.colSpan - (args.end - mergeArgs.range[1] + 1) > 1) {
                  indexes[3] +=
                    cell.colSpan - (args.end - mergeArgs.range[1] + 1) - 1;
                }
              }
              mergeArgsCollection.push(<MergeArgs>{
                range: indexes,
          ...
          deletedCells[i as number].cells = args.model.rows[
            i as number
          ].cells.splice(args.start, count);
          mergeArgs = null;
        }
      }
      ...
      if (
        args.end - args.start === this.parent.sheets.length - 1 ||
        (args.checkCount !== undefined &&
          args.checkCount === this.parent.sheets.length)
      ) {
        return;
      }
    ...
    const deleteMaxHgt: boolean =
      args.modelType === 'Row' && args.start < args.model.maxHgts.length;
    for (let i: number = args.start; i <= args.end; i++) {
      ...
      if (args.modelType === 'Sheet' && sheetsModel[i as number]) {
        this.parent.notify(workbookFormulaOperation, {
          ...
          sheetId: sheetsModel[i as number].id,
        });
      }
    ...
    mergeArgsCollection.forEach((merge: MergeArgs): void =>
  ...
  private deleteConditionalFormats(
    args: InsertDeleteModelArgs,
    eventArgs: InsertDeleteEventArgs
  ): void {
    const cfCollection: ConditionalFormatModel[] =
      args.model.conditionalFormats;
    if (cfCollection) {
      for (let i: number = 0; i < cfCollection.length; i++) {
        eventArgs['conditionalFormats'].push(
          extend({}, cfCollection[i as number], null, true)
        );
        const cfRange: number[] = getRangeIndexes(
          cfCollection[i as number].range
        );
        const sltRangeIndex: number[] = getRangeIndexes(
          args.model.selectedRange
        );
        if (
          (args.modelType === 'Column' &&
            sltRangeIndex[1] <= cfRange[1] &&
            sltRangeIndex[3] >= cfRange[3]) ||
          (args.modelType === 'Row' &&
            sltRangeIndex[0] <= cfRange[0] &&
            sltRangeIndex[2] >= cfRange[2])
        ) {
          cfCollection.splice(
            cfCollection.indexOf(cfCollection[i as number]),
            1
          );
          i--;
        } else {
          cfCollection[i as number].range = getRangeAddress(
            deleteFormatRange(args, cfRange)
          );
        }
      }
    }
