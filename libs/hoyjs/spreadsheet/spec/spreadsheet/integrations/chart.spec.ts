...
import { getComponent, EventHandler } from '@hoyui/base';
import { Chart, Export } from '@hoyui/charts';
Chart.Inject(Export);
...
describe('Chart ->', () => {
  ...
  describe('public method ->', () => {
    ...
    it('Insert', (done: Function) => {
        ...
        [
          {
            type: 'Column',
            range: 'D1:E5',
            left: getColumnsWidth(spreadsheet.sheets[0], 0, 2, true) + 1,
          },
        ],
      ]);
      ...
      expect(
        helper.getElementFromSpreadsheet('#' + cell.chart[0].id).classList
      ).toContain('e-chart');
      done();
    });
    it('Allow Editing property change', (done: Function) => {
      const overlay: HTMLElement =
        helper.getElementFromSpreadsheet('.e-ss-overlay');
      expect(overlay.classList.contains('e-ss-overlay-active')).toBeTruthy();
      const ribbonTabObj: any = getComponent(
        helper.getElementFromSpreadsheet('.e-tab'),
        'tab'
      );
    ...
    it('select and deselect the chart', (done: Function) => {
      const chartOverlay: HTMLElement = helper.getElementFromSpreadsheet(
        `#${cell.chart[0].id}`
      ).parentElement;
      const ribbonObj: any = spreadsheet.ribbonModule.ribbon;
      expect(
        chartOverlay.classList.contains('e-ss-overlay-active')
      ).toBeTruthy();
      ...
      helper.invoke('insertImage', [
        [
          {
            src: 'https://cdn.syncfusion.com/content/images/Logo/Logo_150dpi.png',
            width: 110,
            height: 70,
          },
        ],
      ...
      const imageOverlay: HTMLElement = helper.getElementFromSpreadsheet(
        `#${imageCell.image[0].id}`
      );
      expect(
        imageOverlay.classList.contains('e-ss-overlay-active')
      ).toBeTruthy();
      ...
      const chartOverlay1: HTMLElement = helper.getElementFromSpreadsheet(
        `#${chartCell.chart[0].id}`
      ).parentElement;
      expect(
        chartOverlay1.classList.contains('e-ss-overlay-active')
      ).toBeTruthy();
      expect(ribbonObj.selectedTab).toBe(5);
      helper.invoke('selectChart');
      expect(
    ...
    it('select and deselect the image', (done: Function) => {
      ...
      const imageOverlay1: HTMLElement = helper.getElementFromSpreadsheet(
        `#${imageCell1.image[0].id}`
      );
      expect(
        imageOverlay1.classList.contains('e-ss-overlay-active')
      ).toBeTruthy();
      expect(
        imageOverlay.classList.contains('e-ss-overlay-active')
      ).toBeFalsy();
    ...
    it('Apply freezepane with chart', (done: Function) => {
      ...
      setTimeout((): void => {
        expect(
          helper.getElementFromSpreadsheet('#' + cell.chart[0].id).parentElement
    ...
    it('Delete', (done: Function) => {
      ...
      (
        helper.getInstance().serviceLocator.getService('shape') as Overlay
      ).destroy(); // Need to remove once destory of overlay service handled in chart.
  ...
  describe('UI - Interaction for Chart Type->', () => {
    ...
    it('100% stacked Column Chart->', (done: Function) => {
      ...
      const target: HTMLElement = helper.getElement(
        '#' + helper.id + '_chart-type-btn-popup .e-menu-item'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
      helper.triggerMouseAction(
        'mouseover',
        {
          x: target.getBoundingClientRect().left + 5,
          y: target.getBoundingClientRect().top + 5,
        },
        document,
        target
      );
      ...
      setTimeout(() => {
        const chart: HTMLElement = helper
          .getElement()
          .querySelector('.e-datavisualization-chart');
    ...
    it('clustered Bar Chart->', (done: Function) => {
      helper.getElement('#' + helper.id + '_chart-type-btn').click();
      const target: HTMLElement = helper.getElement(
        '#' + helper.id + '_chart-type-btn-popup .e-menu-item[aria-label="Bar"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
    ...
    it('Area Chart->', (done: Function) => {
      ...
      const target: HTMLElement = helper.getElement(
        '#' +
          helper.id +
          '_chart-type-btn-popup .e-menu-item[aria-label="Area"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
    ...
    it('Pie Chart->', (done: Function) => {
      ...
      const target: HTMLElement = helper.getElement(
        '#' +
          helper.id +
          '_chart-type-btn-popup .e-menu-item[aria-label="Pie/Doughnut"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
      ...
      setTimeout(() => {
        const chart: HTMLElement = helper
          .getElement()
          .querySelector('.e-accumulationchart');
        expect(chart).not.toBeNull();
        done();
      });
    });
    ...
    it('Line Chart->', (done: Function) => {
      ...
      const target: HTMLElement = helper.getElement(
        '#' +
          helper.id +
          '_chart-type-btn-popup .e-menu-item[aria-label="Line"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
    ...
    it('Scatter Chart->', (done: Function) => {
      ...
      const target: HTMLElement = helper.getElement(
        '#' +
          helper.id +
          '_chart-type-btn-popup .e-menu-item[aria-label="Scatter"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
        ...
        .insertChart([
          {
            ...
            dataLabelSettings: { visible: true, position: 'Outer' },
          },
        ]);
      setTimeout(() => {
        const chart: HTMLElement = helper
        ...
        .insertChart([
          {
            ...
            range: 'Sheet1!A1:E5',
            id: 'Chart1',
            dataLabelSettings: { visible: true, position: 'Middle' },
          },
        ]);
      setTimeout(() => {
        const chart: HTMLElement = helper
    ...
    it('Chart with uncalculated formulas and non auto-detected formatted values in the data range', (done: Function) => {
        ...
        [
          {
            type: 'Column',
            range: 'J1:K4',
            dataLabelSettings: { position: 'Middle', visible: true },
          },
        ],
      ]);
      ...
      expect(
        chart
          .querySelector(`#${chartId}_Series_0_Point_0`)
          .getAttribute('aria-label')
      ).toBe('1:10, Sale 1');
      expect(
        chart.querySelector(`#${chartId}_Series_0_Point_0_Text_0`).textContent
      ).toBe('$10');
      expect(
        chart
          .querySelector(`#${chartId}_Series_1_Point_0`)
          .getAttribute('aria-label')
      ).toBe('1:20, Sale 2');
      expect(
        chart.querySelector(`#${chartId}_Series_1_Point_0_Text_0`).textContent
      ).toBe('$20');
      expect(
        chart
          .querySelector(`#${chartId}_Series_0_Point_1`)
          .getAttribute('aria-label')
      ).toBe('2:30, Sale 1');
      expect(
        chart.querySelector(`#${chartId}_Series_0_Point_1_Text_0`).textContent
      ).toBe('$30');
      expect(
        chart
          .querySelector(`#${chartId}_Series_1_Point_1`)
          .getAttribute('aria-label')
      ).toBe('2:50, Sale 2');
      expect(
        chart.querySelector(`#${chartId}_Series_1_Point_1_Text_0`).textContent
      ).toBe('$50');
      ...
      expect(
        chart
          .querySelector(`#${chartId}_Series_0_Point_3`)
          .getAttribute('aria-label')
      ).toBe('4:50, Sale 1');
      expect(
        chart.querySelector(`#${chartId}_Series_0_Point_3_Text_0`).textContent
      ).toBe('$50');
      expect(
        chart
          .querySelector(`#${chartId}_Series_1_Point_3`)
          .getAttribute('aria-label')
      ).toBe('4:70, Sale 2');
      expect(
        chart.querySelector(`#${chartId}_Series_1_Point_3_Text_0`).textContent
      ).toBe('$70');
      done();
    });
    it('Changing number format in the chart range', (done: Function) => {
      ...
      const chart: HTMLElement = helper.getElementFromSpreadsheet(
        `#${chartId}`
      );
      expect(
        chart.querySelector(`#${chartId}_Series_0_Point_0_Text_0`).textContent
      ...
      expect(
        chart.querySelector(`#${chartId}_Series_0_Point_2_Text_0`).textContent
      ).toBe('3.00E+01');
      expect(
        chart.querySelector(`#${chartId}_Series_0_Point_3_Text_0`).textContent
      ).toBe('5.00E+01');
  ...
  describe('UI - interaction for copy and paste chart->', () => {
    ...
    it('Copy and paste chart->', (done: Function) => {
      ...
      const target: HTMLElement = helper.getElement(
        '#' + helper.id + '_chart-btn-popup .e-menu-item[aria-label="Line"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          helper.invoke('paste');
          expect(
            helper.getElementFromSpreadsheet(
              '#' + helper.getInstance().sheets[0].rows[0].cells[3].chart[0].id
            ).classList
          ).toContain('e-chart');
          done();
        });
  ...
  describe('UI - Interaction ->', () => {
    ...
    it('Copy and paste the Line Chart With Marker->', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          expect(
            helper.getElementFromSpreadsheet(
              '#' + helper.getInstance().sheets[0].rows[3].cells[8].chart[0].id
            ).classList
          ).toContain('e-chart');
          expect(
            helper.getInstance().sheets[0].rows[3].cells[8].chart[0]
    ...
    it('Chart for the date values', (done: Function) => {
      helper.invoke('insertChart', [
        [{ type: 'Column', range: 'B4:B7', id: 'e_spreadsheet_chart_1' }],
      ]);
      helper.invoke('insertChart', [
        [
          {
      ...
      setTimeout(() => {
        expect(
          document
            .getElementById('e_spreadsheet_chart_1_Series_0_Point_0')
            .getAttribute('aria-label')
        ).toBe('1:41847, series0');
        expect(
          document
            .getElementById('e_spreadsheet_chart_1_Series_0_Point_1')
            .getAttribute('aria-label')
        ).toBe('2:41964, series0');
        expect(
          document
            .getElementById('e_spreadsheet_chart_1_Series_0_Point_2')
            .getAttribute('aria-label')
        ).toBe('3:41813, series0');
        done();
      });
    });
    it('Chart for the date and time values', (done: Function) => {
      helper.invoke('insertChart', [
        [{ type: 'Column', range: 'B4:C11', id: 'e_spreadsheet_chart_2' }],
      ]);
      setTimeout(() => {
        expect(
          document
            .getElementById('e_spreadsheet_chart_2_Series_0_Point_0')
            .getAttribute('aria-label')
        ).toBe('1:41847, series0');
        expect(
          document
            .getElementById('e_spreadsheet_chart_2_Series_0_Point_1')
            .getAttribute('aria-label')
        ).toBe('2:41964, series0');
        expect(
          document
            .getElementById('e_spreadsheet_chart_2_Series_0_Point_2')
            .getAttribute('aria-label')
        ).toBe('3:41813, series0');
        expect(
          document
            .getElementById('e_spreadsheet_chart_2_Series_0_Point_3')
            .getAttribute('aria-label')
        ).toBe('4:41674, series0');
        expect(
          document
            .getElementById('e_spreadsheet_chart_2_Series_0_Point_4')
            .getAttribute('aria-label')
        ).toBe('5:41973, series0');
        expect(
          document
            .getElementById('e_spreadsheet_chart_2_Series_0_Point_5')
            .getAttribute('aria-label')
        ).toBe('6:41829, series0');
        expect(
          document
            .getElementById('e_spreadsheet_chart_2_Series_0_Point_6')
            .getAttribute('aria-label')
        ).toBe('7:41943, series0');
        done();
      });
    });
  });
  ...
  describe('UI - Interaction for chart resize->', () => {
    ...
    it('Apply chart->', (done: Function) => {
      ...
      helper
        .getInstance()
        .insertChart([
          { type: 'Line', theme: 'Material', range: 'Sheet1!G1:H7' },
        ]);
      setTimeout(() => {
        const chart: HTMLElement = helper
          .getElement()
    ...
    it('Chart size refreshing ->', (done: Function) => {
      ...
      helper
        .getInstance()
        .workbookChartModule.refreshChartSize({
          height: '300',
          width: '300',
          overlayEle: chart,
        });
  ...
  describe('UI - Interaction for insert chart with protect sheet and allowchart as false->', () => {
    ...
    it('Insert chart with protect sheet and allowchart as false->', (done: Function) => {
      ...
      helper
        .getInstance()
        .insertChart([
          { type: 'Column', theme: 'Material', range: 'Sheet1!A1:E5' },
        ]);
      setTimeout(() => {
        const chart: HTMLElement = helper
          .getElement()
  ...
  describe('UI - Interaction for Chart Design Tab->', () => {
    ...
    it('Add Chart Title->', (done: Function) => {
      ...
      helper.switchRibbonTab(2);
      helper.getElement('#' + helper.id + '_chart-btn').click();
      const target1: HTMLElement = helper.getElement(
        '#' + helper.id + '_chart-btn-popup .e-menu-item[aria-label="Column"]'
      );
      (
        getComponent(target1.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
      helper.triggerMouseAction(
        'mouseover',
        {
          x: target1.getBoundingClientRect().left + 5,
          y: target1.getBoundingClientRect().top + 5,
        },
        document,
        target1
      );
      ...
      const target: HTMLElement = helper.getElement(
        '#' +
          helper.id +
          '_addchart-popup .e-menu-item[aria-label="Chart Title"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
      ...
      setTimeout(() => {
        ...
        const input: HTMLInputElement = helper.getElement(
          '#' + helper.id + ' .e-title-dlg .e-dlg-content .e-input'
        ) as HTMLInputElement;
        input.value = 'Chart';
        helper.click('.e-title-dlg .e-primary');
        setTimeout(() => {
          expect(
            document.getElementsByClassName('e-chart-focused')[1].textContent
          ).toBe('Chart');
          done();
        });
      });
    ...
    it('Add Chart Horizontal Title->', (done: Function) => {
      ...
      const target: HTMLElement = helper.getElement(
        '#' +
          helper.id +
          '_addchart-popup .e-menu-item[aria-label="Axis Title"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
    ...
    it('Undo after Adding Chart Title->', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          document.getElementsByClassName('e-chart-focused')[1]
        ).toBeUndefined();
        done();
      });
    });
    ...
    it('Choosing Material Dark in Chart Theme Dropdown->', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          helper.getElement('#' + helper.id + '_chart_theme').textContent
        ).toContain('Material Dark');
        done();
      }, 20);
    });
    ...
    it('Choosing HighContrast in Chart Theme Dropdown->', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          helper.getElement('#' + helper.id + '_chart_theme').textContent
        ).toContain('HighContrast');
        done();
      }, 20);
    });
    it('Undo after Adding Chart Theme->', (done: Function) => {
      // need to check
      helper.switchRibbonTab(1);
      helper.click('#spreadsheet_undo');
      setTimeout(() => {
  ...
  describe('UI - Interaction for Data Label in Chart Design Tab->', () => {
    ...
    it('Apply Center Data Labels->', (done: Function) => {
      ...
      const target: HTMLElement = helper.getElement(
        '#' +
          helper.id +
          '_addchart-popup .e-menu-item[aria-label="Data Labels"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
  ...
  describe('UI - Interaction for Gridlines and Legends in Chart Design Tab->', () => {
    ...
    it('Apply Major Horizonatal Gridlines->', (done: Function) => {
      ...
      helper.getElement('#' + helper.id + '_addchart').click();
      const target: HTMLElement = helper.getElement(
        '#' + helper.id + '_addchart-popup .e-menu-item[aria-label="Gridlines"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
    ...
    it('Apply Right Legend->', (done: Function) => {
      helper.getElement('#' + helper.id + '_addchart').click();
      const target: HTMLElement = helper.getElement(
        '#' + helper.id + '_addchart-popup .e-menu-item[aria-label="Legends"]'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
          ...
          sheets: [
            {
              ...
              rows: [
                ...
                {
                  cells: [{ index: 6, chart: [{ range: 'A3:E10' }] }],
                },
              ],
            },
          ],
  ...
  describe('Bar UI - Interaction for Gridlines and Legends in Chart Design Tab->', () => {
    ...
    it('Apply Major Horizonatal Gridlines->', (done: Function) => {
      ...
      helper.getElement('#' + helper.id + '_chart-btn').click();
      const target1: HTMLElement = helper.getElement(
        '#' + helper.id + '_chart-btn-popup .e-menu-item[aria-label="Bar"]'
      );
      (
        getComponent(target1.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
  ...
  describe('Chart data refreshing after merge action->', () => {
    beforeAll((done: Function) => {
      let model: SheetModel[] = [
        {
          ranges: [{ dataSource: defaultData, startCell: 'C3' }],
          rows: [
            {
              index: 0,
              cells: [
    ...
    it('Merge with chart applied range - 3', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          document.getElementById('e_spreadsheet_chart_1_Series_0_Point_0')
        ).not.toBeNull();
        done();
      });
    });
    ...
    it('Merge with chart applied range - 4', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          document
            .getElementById('e_spreadsheet_chart_1_Series_1_Point_0')
            .getAttribute('aria-label')
        ).toBe('1:0, Price');
        done();
      });
    ...
    it('Merge with chart applied range - 5', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          document
            .getElementById('e_spreadsheet_chart_1_Series_1_Point_9')
            .getAttribute('aria-label')
        ).toBe('10:10, Price');
        done();
      });
    ...
    it('Merge with chart applied range - 9', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          document
            .getElementById('e_spreadsheet_chart_1_Series_1_Point_1')
            .getAttribute('aria-label')
        ).toBe('2:0, Price');
        done();
      });
    ...
    it('Merge with chart applied range - 13', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          document
            .getElementById('e_spreadsheet_chart_1_Series_0_Point_8')
            .getAttribute('aria-label')
        ).toBe('41:30, Price');
        done();
      });
  ...
  describe('Chart data refreshing after clear content action->', () => {
    ...
    it('Clear contents with chart applied range in single cell', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          document.getElementById('e_spreadsheet_chart_10_AxisLabel_0')
            .textContent
        ).toBe('');
        done();
      });
    ...
    it('Clear contents with chart applied range in multiple cells', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          document.getElementById('e_spreadsheet_chart_1_Series_0_Point_2')
        ).not.toBeNull();
        expect(
          document.getElementById('e_spreadsheet_chart_10_AxisLabel_2')
            .textContent
        ).toBe('');
        expect(
          document.getElementById('e_spreadsheet_chart_1_Series_0_Point_3')
        ).not.toBeNull();
        expect(
          document.getElementById('e_spreadsheet_chart_10_AxisLabel_3')
            .textContent
        ).toBe('');
        expect(
          document.getElementById('e_spreadsheet_chart_1_Series_0_Point_4')
        ).not.toBeNull();
        expect(
          document.getElementById('e_spreadsheet_chart_10_AxisLabel_4')
            .textContent
        ).toBe('');
        done();
      });
  ...
  describe('Chart data refreshing after merge action->', () => {
    ...
    it('Merge action after the chart applied', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          document.getElementById('e_spreadsheet_chart_1_Series_0_Point_1')
        ).not.toBeNull();
        expect(
          document.getElementById('e_spreadsheet_chart_10_AxisLabel_1')
            .textContent
        ).toBe('');
        expect(
          document.getElementById('e_spreadsheet_chart_1_Series_0_Point_2')
  ...
  describe('CR-Issues ->', () => {
    ...
    describe('Chart actions->', () => {
        ...
        let model: SheetModel[] = [
          {
            ...
            rows: [
              {
                index: 1,
                cells: [{ index: 6, chart: [{ type: 'Pie', range: 'A1:E8' }] }],
              },
            ],
            columns: [
              { width: 80 },
      ...
      it('EJ2-50430 -> Aggregate value wrongly displayed in chart Sample', (done: Function) => {
        ...
        expect(
          helper.getElement('#' + helper.id + '_aggregate-popup ul li')
            .textContent
        ).toBe('Count: 28');
        ...
        expect(
          helper.getElement(
            '#' + helper.id + '_aggregate-popup ul li:nth-child(2)'
          ).textContent
        ).toBe('Sum: 197.61');
        done();
      });
      it('Chart data not refreshed after merge action', (done: Function) => {
        const chartObj: any = getComponent(
          helper.getElementFromSpreadsheet('.e-accumulationchart'),
          'accumulationchart'
        );
      ...
      it('ej2-react-ui-components/issues/79 -> Charts are not loaded if data range is not from active sheet', (done: Function) => {
        ...
        setTimeout((): void => {
          ...
          expect(spreadsheet.sheets[1].rows[0].cells[0].chart[0].range).toEqual(
            'Sheet1!A1:C3'
          );
          done();
        });
    ...
    describe('Dependent use cases->', () => {
                ...
                rows: [
                  {
                    index: 1,
                    cells: [
                      { index: 6, chart: [{ type: 'Pie', range: 'A1:E8' }] },
                    ],
                  },
                ],
                columns: [
      ...
      it('Sheet rename update on chart range', (done: Function) => {
        const spreadsheet: Spreadsheet = helper.getInstance();
        expect(spreadsheet.sheets[0].rows[1].cells[6].chart[0].range).toEqual(
          'Sheet1!A1:E8'
        );
        helper.triggerMouseAction(
          'dblclick',
          null,
          helper.getElementFromSpreadsheet('.e-sheet-tab .e-toolbar-items'),
          helper.getElementFromSpreadsheet(
            '.e-sheet-tab .e-active .e-text-wrap'
          )
        );
        const editorElem: HTMLInputElement = <HTMLInputElement>(
          helper.getElementFromSpreadsheet('.e-sheet-tab .e-sheet-rename')
        );
        editorElem.click();
      ...
      it('Apply Undo after Chart Refresh', (done: Function) => {
        ...
        helper.triggerMouseAction(
          'mousedown',
          {
            x: chart.getBoundingClientRect().left + 1,
            y: chart.getBoundingClientRect().top + 1,
          },
          chart,
          chart
        );
        helper.triggerMouseAction(
          'mousemove',
          {
            x: chart.getBoundingClientRect().left + 200,
            y: chart.getBoundingClientRect().top + 100,
          },
          chart,
          chart
        );
        ...
        setTimeout(() => {
          ...
          EventHandler.remove(
            document,
            'mouseup',
            helper.getInstance().serviceLocator.services.shape
    ...
    describe('EJ2-70711->', () => {
      ...
      it('Inserting chart remains blank when selecting the range from downwards->', (done: Function) => {
        helper
          .getInstance()
          .insertChart([{ type: 'Column', theme: 'Material', range: 'H7:H2' }]);
        setTimeout(() => {
          const chart: HTMLElement = helper
            .getElement()
            .querySelector('.e-datavisualization-chart');
      ...
      it('Inserting chart remains blank when selecting the multiple range from downwards->', (done: Function) => {
        helper
          .getInstance()
          .insertChart([{ type: 'Column', theme: 'Material', range: 'H7:G2' }]);
        setTimeout(() => {
          const chart: HTMLElement = helper
            .getElement()
            .querySelector('.e-datavisualization-chart');
      ...
      it('Insert chart with time formatted values->', (done: Function) => {
        ...
        spreadsheet.insertChart([
          { type: 'Scatter', theme: 'Material', range: 'C1:D5' },
        ]);
        setTimeout(() => {
          ...
          expect(
            chartObj.series[0].dataModule.dataManager.dataSource.json.length
          ).toBe(4);
          expect(
            chartObj.series[0].dataModule.dataManager.dataSource.json[0].x
          ).toBe('11:34:32 AM');
          expect(
            chartObj.series[0].dataModule.dataManager.dataSource.json[0].y
          ).toBe(10);
          expect(
            chart.querySelector(`${chartId}0_AxisLabel_0`).textContent
          ).toBe('11:34:32 AM');
          expect(
            chart.querySelector(`${chartId}0_AxisLabel_3`).textContent
          ).toBe('6:23:54 AM');
          done();
        });
      });
      it('Insert chart with date formatted values->', (done: Function) => {
        ...
        const spreadsheet: Spreadsheet = helper.getInstance();
        spreadsheet.insertChart([
          { type: 'Column', theme: 'Material', range: 'E1:F4' },
        ]);
        setTimeout(() => {
    ...
    describe('EJ2-71624', () => {
      beforeAll((done: Function) => {
        helper.initializeSpreadsheet(
          {
            sheets: [
              { ranges: [{ dataSource: defaultData }], selectedRange: 'K2:K2' },
            ],
          },
          done
        );
                    ...
                    cells: [
                      {
                        index: 6,
                        chart: [
                          { type: 'Pie', range: 'A1:E8', top: 50, left: 20 },
                          {
                            type: 'Column',
                            range: 'H7:J10',
                            top: 60,
  ...
  describe('Provide support for inserting a line chart with/without marker options in the spreadsheet', () => {
    ...
    it('insert Line chart without marker', (done: Function) => {
      ...
      const chart: HTMLElement = helper
        .getElement()
        .querySelector('.e-datavisualization-chart');
      const marker: HTMLCollectionOf<SVGEllipseElement> =
        chart.getElementsByTagName('ellipse');
        ...
        [
          {
            type: 'StackingLine',
            range: 'D1:E5',
            markerSettings: { visible: true, size: 10, shape: 'Diamond' },
          },
        ],
      ]);
      const chart: HTMLElement = helper
        .getElement()
        .querySelector('.e-datavisualization-chart');
      const marker: HTMLCollectionOf<SVGPathElement> =
        chart.getElementsByTagName('path');
      expect(marker.length).not.toBe(0);
      helper.invoke('deleteChart');
      done();
    });
  ...
  describe('EJ2-852097', () => {
    ...
    it('Scatter chart X - axis plottings are not ordered with numerical values in a series order', (done: Function) => {
      ...
      let chartEle: HTMLElement = spreadsheet.element.querySelector(
        chartId
      ) as HTMLElement;
      ...
      expect(
        chartEle.querySelector(chartId + '0_AxisLabel_0').textContent
      ).toBe('1');
      expect(
        chartEle.querySelector(chartId + '_chart_legend_translate_g')
          .childElementCount
      ).toEqual(1);
      expect(
        chartEle.querySelector(chartId + '_chart_legend_g_0').textContent
      ).toBe('Heading2');
      helper.getElement('#' + helper.id + '_chart-type-btn').click();
      let target: HTMLElement = helper.getElement(
        '#' + helper.id + '_chart-type-btn-popup .e-menu-item'
      );
      (
        getComponent(target.parentElement, 'menu') as any
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          helper
            .getInstance()
            .insertChart([{ type: 'Scatter', range: 'B1:D4' }]);
              ...
              rows: [
                {
                  index: 1,
                  cells: [
                    { index: 6, chart: [{ type: 'Column', range: 'A1:E8' }] },
                  ],
                },
              ],
              columns: [
    ...
    it('Insert column chart and resize the chart ->', (done: Function) => {
      ...
      setTimeout(() => {
        const chart: HTMLElement = helper
          ...
          .querySelector('.e-datavisualization-chart');
        expect(chart).not.toBeNull();
        const overlayObj: any = helper
          .getInstance()
          .serviceLocator.getService('shape') as Overlay;
        expect(overlayObj.originalWidth).toBe(480);
        expect(overlayObj.originalHeight).toBe(290);
        const overlay: HTMLElement = helper.getElementFromSpreadsheet(
          '.e-ss-overlay-active'
        );
        const overlayHgtHanlde: HTMLElement =
          overlay.querySelector('.e-ss-overlay-b');
        let offset: DOMRect =
          overlayHgtHanlde.getBoundingClientRect() as DOMRect;
        helper.triggerMouseAction(
          'mousedown',
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              expect(spreadsheet.sheets[0].rows[4].cells[8].chart.length).toBe(
                1
              );
              expect(spreadsheet.sheets[0].rows[4].cells[7].chart.length).toBe(
                0
              );
              helper.invoke('hideColumn', [2, 2, false]);
              setTimeout(() => {
                expect(
                  spreadsheet.sheets[0].rows[4].cells[7].chart.length
                ).toBe(1);
                expect(
                  spreadsheet.sheets[0].rows[4].cells[8].chart.length
                ).toBe(0);
                done();
              });
            });
  ...
  describe('EJ2-879107 ->', () => {
                  ...
                  cells: [
                    {
                      index: 6,
                      chart: [
                        { type: 'Pie', range: 'A1:E8', top: 80, left: 20 },
                      ],
                    },
                  ],
                },
    ...
    it('Handle Chart model when position top and left is given ->', (done: Function) => {
      ...
      setTimeout(() => {
        helper.invoke('insertChart', [
          [{ type: 'Column', range: 'H5:H10', top: 100, left: 50 }],
        ]);
          ...
          sheets: [
            {
              ranges: [{ dataSource: defaultData }],
              rows: [
                { cells: [{ index: 10, value: '12' }] },
                { cells: [{ index: 10, value: '10' }] },
              ],
            },
          ],
        },
  ...
  describe('Testing chart by providing chart titles ->', () => {
    ...
    it('Add Chart Title for accumulation charts ->', (done: Function) => {
      ...
      const target1: HTMLElement = helper.getElement(
        '#' +
          helper.id +
          '_chart-btn-popup .e-menu-item[aria-label="Pie/Doughnut"]'
      );
      (
        getComponent(target1.parentElement, 'menu') as any
      ).animationSettings.effect = 'None';
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          expect(
            helper.getInstance().sheets[0].rows[0].cells[3].chart[0].title
          ).toBe('Chart');
          done();
        });
      });
