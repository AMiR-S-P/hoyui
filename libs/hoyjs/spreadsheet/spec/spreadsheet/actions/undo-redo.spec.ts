...
describe('Undo redo ->', () => {
  ...
  describe('UI Interaction ->', () => {
    ...
    it('Undo issue after unhide column', (done: Function) => {
      ...
      setTimeout((): void => {
        ...
        expect(
          helper.getInstance().sheets[0].columns[1].hidden
        ).toBeUndefined();
        expect(helper.getInstance().sheets[0].columns[2].hidden).toBe(true);
        expect(
          helper.getInstance().sheets[0].columns[3].hidden
        ).toBeUndefined();
        expect(prevElement.classList).toContain('e-hide-start');
        expect(endElement.classList).toContain('e-hide-end');
        expect(helper.invoke('getCell', [0, 2]).textContent).toBe('Date');
    ...
    it('Undo issue after unhide row', (done: Function) => {
      ...
      let prevElement: HTMLElement =
        helper.getElement('.e-rowhdr-table').children[1].children[1];
      expect(prevElement.classList).toContain('e-hide-start');
      let endElement: HTMLElement =
        helper.getElement('.e-rowhdr-table').children[1].children[2];
      expect(endElement.classList).toContain('e-hide-end');
  ...
  describe('UI- Interaction ->', () => {
    beforeAll((done: Function) => {
      helper.initializeSpreadsheet(
        {
          sheets: [
            { ranges: [{ dataSource: defaultData }], selectedRange: 'A1:A2' },
          ],
        },
        done
      );
    ...
    it('Redo after Strikethrough and Underline ->', (done: Function) => {
      ...
      expect(
        helper.getInstance().sheets[0].rows[3].cells[2].style.textDecoration
    ...
    it('Redo after Strikethrough and Underline for Many Cells->', (done: Function) => {
      ...
      helper.getElement('#' + helper.id + '_underline').click();
      expect(
        helper.getInstance().sheets[0].rows[3].cells[3].style.textDecoration
      ).toBe('underline line-through');
      expect(
        helper.getInstance().sheets[0].rows[4].cells[3].style.textDecoration
      ).toBe('underline line-through');
      helper.click('#spreadsheet_undo');
      expect(
        helper.getInstance().sheets[0].rows[3].cells[3].style.textDecoration
    ...
    it('Redo after Strikethrough and Underline and cell having Strikethrough Underline  for Single cell->', (done: Function) => {
      ...
      helper.getElement('#' + helper.id + '_underline').click();
      expect(
        helper.getInstance().sheets[0].rows[4].cells[2].style.textDecoration
      ).toBe('line-through');
      helper.click('#spreadsheet_undo');
      expect(
        helper.getInstance().sheets[0].rows[4].cells[2].style.textDecoration
    ...
    it('Redo after Strikethrough and Underline and cell having Strikethrough Underline Many cells->', (done: Function) => {
      ...
      helper.getElement('#' + helper.id + '_underline').click();
      expect(
        helper.getInstance().sheets[0].rows[5].cells[2].style.textDecoration
      ).toBe('line-through');
      expect(
        helper.getInstance().sheets[0].rows[6].cells[3].style.textDecoration
      ).toBe('line-through');
      helper.click('#spreadsheet_undo');
      expect(
        helper.getInstance().sheets[0].rows[5].cells[2].style.textDecoration
    ...
    it('Undo after Unique Formula Applied->', (done: Function) => {
      ...
      expect(spreadsheet.sheets[0].rows[0].cells[8].formula).toBe(
        '=UNIQUE(E2:E11)'
      );
    ...
    it('redo after Wrongly applied Unique Formula->', (done: Function) => {
      ...
      helper.edit('J1', '=UNIQUE(E2:E11)');
      expect(spreadsheet.sheets[0].rows[0].cells[9].formula).toBe(
        '=UNIQUE(E2:E11)'
      );
      expect(spreadsheet.sheets[0].rows[0].cells[9].value).toBe('20');
      expect(spreadsheet.sheets[0].rows[1].cells[9].value).toBe('30');
    ...
    it('redo after applied Unique Formula->', (done: Function) => {
      ...
      expect(spreadsheet.sheets[0].rows[0].cells[10].formula).toBe(
        '=UNIQUE(E2:E11)'
      );
    ...
    it('undo for Column Resize ->', (done: Function) => {
      expect(getComputedStyle(helper.invoke('getCell', [0, 7])).width).toBe(
        '64px'
      );
      const spreadsheet: Spreadsheet = helper.getInstance();
      const colHdr: HTMLElement = helper.invoke('getCell', [
        null,
        8,
        helper.invoke('getColHeaderTable').rows[0],
      ]);
      const hdrPanel: HTMLElement = spreadsheet.element.querySelector(
        '.e-header-panel'
      ) as HTMLElement;
      const offset: DOMRect = colHdr.getBoundingClientRect() as DOMRect;
      helper.triggerMouseAction(
        'mousemove',
        { x: offset.left + 0.5, y: offset.top + 1, offsetX: 3 },
        hdrPanel,
        colHdr
      );
      helper.triggerMouseAction(
        'dblclick',
        { x: offset.left + 1, y: offset.top + 1, offsetX: 3 },
        hdrPanel,
        colHdr
      );
      expect(getComputedStyle(helper.invoke('getCell', [0, 7])).width).toBe(
    ...
    it('undo for Row Resize ->', (done: Function) => {
      ...
      const rowHdr: HTMLElement =
        helper.invoke('getRowHeaderTable').rows[2].cells[0];
      ...
      helper.triggerMouseAction(
        'mousemove',
        { x: offset.top + 0.5, y: offset.left + 1, offsetY: 3 },
        rowHdrPanel,
        rowHdr
      );
      helper.triggerMouseAction(
        'dblclick',
        { x: offset.top + 1, y: offset.left + 1, offsetY: 3 },
        rowHdrPanel,
        rowHdr
      );
  ...
  describe('UI_Interaction ->', () => {
    ...
    it('Undo after sorting using Filter icon ->', (done: Function) => {
      ...
      expect(
        helper.invoke('getCell', [0, 0]).querySelector('.e-filter-iconbtn')
      ).not.toBeNull();
      ...
      helper
        .getInstance()
        .keyboardNavigationModule.keyDownHandler({
      ...
      setTimeout(() => {
        setTimeout(() => {
          const sortAsc: HTMLElement = helper.getElement(
            '.e-excelfilter .e-filter-sortasc'
          );
          helper.triggerMouseAction(
            'mousedown',
            {
              x: sortAsc.getBoundingClientRect().left + 1,
              y: sortAsc.getBoundingClientRect().top + 1,
            },
            null,
            sortAsc
          );
          ...
          setTimeout(() => {
            ...
            expect(
              helper
                .invoke('getCell', [0, 0])
                .querySelector('.e-filter-iconbtn')
            ).not.toBeNull();
            expect(td.children[0].children[0].classList).not.toContain(
              'e-sortasc-filter'
            );
            expect(helper.invoke('getCell', [1, 0]).textContent).toBe(
              'Casual Shoes'
    ...
    it('Undo after apply both sorting using Filter icon ->', (done: Function) => {
      ...
      setTimeout(() => {
        setTimeout(() => {
          const sortAsc: HTMLElement = helper.getElement(
            '.e-excelfilter .e-filter-sortdesc'
          );
          helper.triggerMouseAction(
            'mousedown',
            {
    ...
    it('Undo after Removing Column Validation->', (done: Function) => {
      ...
      helper
        .getElementFromSpreadsheet('#' + helper.id + '_datavalidation')
        .click();
      setTimeout(() => {
        helper.click('.e-datavalidation-ddb li:nth-child(1)');
        setTimeout(() => {
          let ddlElem: any = helper.getElements(
            '.e-datavalidation-dlg .e-allow .e-dropdownlist'
          )[0];
          ddlElem.ej2_instances[0].value = 'List';
          ddlElem.ej2_instances[0].dataBind();
          helper.getElements(
            '.e-datavalidation-dlg .e-values .e-input'
          )[0].value = '1,2,3';
          helper.setAnimationToNone('.e-datavalidation-dlg.e-dialog');
          helper.click(
            '.e-datavalidation-dlg .e-footer-content button:nth-child(2)'
          );
          setTimeout(() => {
            const cell: CellModel =
              helper.getInstance().sheets[0].rows[1].cells[4];
            expect(JSON.stringify(cell.validation)).toBe(
              '{"type":"List","operator":"Between","value1":"1,2,3","value2":"","ignoreBlank":true,"inCellDropDown":true}'
            );
            helper.switchRibbonTab(4);
            helper
              .getElementFromSpreadsheet('#' + helper.id + '_datavalidation')
    ...
    it('Undo after Removing Hyperlink->', (done: Function) => {
      ...
      expect(helper.getInstance().sheets[0].rows[2].cells[4].hyperlink).toBe(
        'http://www.google.com'
      );
      ...
      expect(
        helper.getInstance().sheets[0].rows[2].cells[4].style.textDecoration
      ).toBe('underline');
    ...
    it('Cell format action - undo/redo collection->', (done: Function) => {
      ...
      expect(
        helper
          .getElement('#' + helper.id + '_redo')
          .parentElement.classList.contains('e-overlay')
      ).toBeTruthy();
      let actionArgs: any =
        undoRedo.undoCollection[undoRedo.undoCollection.length - 1];
      ...
      actionArgs = undoRedo.undoCollection[undoRedo.undoCollection.length - 1];
      expect(actionArgs.eventArgs.style.textDecoration).toBe(
  ...
  describe('UI - Interaction II', () => {
    ...
    it('Apply undo after clear Conditional Formats', (done: Function) => {
      ...
      const target: HTMLElement = helper.getElement(
        '#' + helper.id + '_conditionalformatting-popup .e-clearrules'
      );
      (
        getComponent(target.parentElement.parentElement, 'menu') as any
      ...
      setTimeout(() => {
        ...
        expect(
          helper
            .invoke('getCell', [1, 7])
            .getElementsByClassName('e-databar')[1].style.width
        ).toBe('7%');
        expect(
          helper
            .invoke('getCell', [1, 7])
            .getElementsByClassName('e-databar')[1].style.height
        ).toBe('17px');
        expect(
          helper
            .invoke('getCell', [9, 7])
    ...
    it('Apply undo after after Cut/Paste Conditional Formats', (done: Function) => {
      ...
      helper.invoke('cut').then(() => {
        ...
        helper.click('#spreadsheet_undo');
        expect(
          helper.invoke('getCell', [1, 9]).querySelector('.e-databar')
        ).toBeNull();
        expect(
          helper.invoke('getCell', [1, 9]).querySelector('.e-databar')
        ).toBeNull();
        expect(
          helper.invoke('getCell', [9, 9]).querySelector('.e-databar')
        ).toBeNull();
        expect(
          helper
            .invoke('getCell', [1, 7])
    ...
    it('Redo after Delete Spill causing Value in Unique Formula Applied in First Column->', (done: Function) => {
      ...
      expect(spreadsheet.sheets[0].rows[11].cells[0].formula).toBe(
        '=UNIQUE(E2:E11)'
      );
    ...
    it('Redo after apply Unique Formula in used Range->', (done: Function) => {
      ...
      helper.edit('G1', '=UNIQUE(E2:E11);');
      expect(spreadsheet.sheets[0].rows[0].cells[6].formula).toBe(
        '=UNIQUE(E2:E11);'
      );
    ...
    it('Redo Image, after cut/Paste', (done: Function) => {
      ...
      setTimeout(() => {
        const image: ImageModel =
          helper.getInstance().sheets[0].rows[0].cells[12].image[0];
        expect(image.height).toBe(300);
        expect(image.width).toBe(400);
        expect(image.top).toBe(0);
        expect(image.left).toBe(768);
    ...
    it('Undo Image, after copy/Paste', (done: Function) => {
      helper.invoke('copy').then(() => {
        ...
        setTimeout(() => {
          expect(
            JSON.stringify(
              helper.getInstance().sheets[0].rows[2].cells[3].image
            )
          ).toBe('[]');
          EventHandler.remove(
            document,
    ...
    it('Apply Undo after Image Refresh', (done: Function) => {
      const image: HTMLElement = helper
        .getElement()
        .querySelector('.e-ss-overlay-active');
      const inst: Spreadsheet = helper.getInstance();
      inst.element.focus();
      helper.triggerMouseAction(
        'mousedown',
        {
          x: image.getBoundingClientRect().left + 1,
      ...
      helper.triggerMouseAction(
        'mousemove',
        {
          x: image.getBoundingClientRect().left + 200,
          y: image.getBoundingClientRect().top + 100,
        },
        image,
        image
      );
  ...
  describe('CR Issues ->', () => {
    describe('SF-359671,SF-356044,SF-361047 -> actionBegin and actionComplete event', () => {
      ...
      it('cell delete', (done: Function) => {
        ...
        expect(isTriggered).toBeFalsy();
        expect(
          spreadsheet.sheets[0].rows[1].cells[3].value as any
        ).toBeUndefined();
        cancel = true;
        action = '';
        helper.click('#spreadsheet_undo');
  ...
  describe('SF-362962', () => {
    ...
    beforeAll((done: Function) => {
      helper.initializeSpreadsheet(
        {
          sheets: [
            { ranges: [{ dataSource: defaultData }], selectedRange: 'A3' },
          ],
        },
        done
      );
    ...
    it('Invalid expression throws While delete row and apply undo for formula applied rows', (done: Function) => {
      ...
      setTimeout((): void => {
        ...
        setTimeout((): void => {
          expect(sheet.rows[2].cells[9].formula).toEqual(
            '=IF(I3="","empty","not empty")'
          );
  ...
  describe('EJ2-60728', () => {
    ...
    it('Hyperlink text changed to URL after clearing all the hyperlink cell', (done: Function) => {
      (
        helper
          .getRibbonElement()
          .querySelector('.e-tab-header .e-toolbar-items')
          .children[3] as HTMLElement
      ).click();
      (
        helper
          .getRibbonElement()
          .querySelector('.e-control #spreadsheet_hyperlink') as HTMLElement
      ).click();
      setTimeout(() => {
        helper.getElements('.e-dlg-container .e-webpage input')[1].value =
          'www.syncfusion.com';
        helper.triggerKeyEvent(
          ...
          null,
          null,
          helper.getElements('.e-hyperlink-dlg .e-webpage input')[1]
        );
