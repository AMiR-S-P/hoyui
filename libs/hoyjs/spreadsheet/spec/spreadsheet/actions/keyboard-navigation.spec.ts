...
describe('Spreadsheet cell navigation module ->', () => {
  ...
  describe('Keyboard Navigations-I ->', () => {
    ...
    it('Ctrl + Right arrow key for Navigation', (done: Function) => {
      ...
      helper.triggerKeyNativeEvent(39, true);
      setTimeout(() => {
        expect(helper.getInstance().sheets[0].selectedRange).toBe('CV4:CV4');
        done();
      }, 20);
    });
    ...
    it('Insert function using shift+F3', (done: Function) => {
      ...
      setTimeout(() => {
        expect(spreadsheet.isEdit).toBeFalsy();
        const dialog: HTMLElement = helper.getElement(
          '.e-spreadsheet-function-dlg.e-dialog'
        );
        expect(dialog).not.toBeNull();
        helper.click(
          '.e-spreadsheet-function-dlg.e-dialog .e-list-item:nth-child(4)'
        );
        helper.setAnimationToNone('.e-spreadsheet-function-dlg.e-dialog');
        helper.click(
          '.e-spreadsheet-function-dlg.e-dialog .e-footer-content .e-primary'
        );
    ...
    it('Ctrl + F6 to navigate between each part in spreadsheet', (done: Function) => {
      ...
      expect(
        document.activeElement.classList.contains('e-add-sheet-tab')
      ).toBeTruthy();
      helper.triggerKeyNativeEvent(
        117,
        true,
      ...
      expect(
        document.activeElement.classList.contains('e-tab-wrap')
      ).toBeTruthy();
      ...
      expect(
        document.activeElement.classList.contains('e-combobox')
      ).toBeTruthy();
      ...
      expect(
        document.activeElement.classList.contains('e-selectall')
      ).toBeTruthy();
      done();
    });
    ...
    it('Tab and Shift + Tab navigation inside sheet tabs', (done: Function) => {
      ...
      const sheetTabs: HTMLElement[] = [].slice.call(
        helper.getElements('.e-sheet-tab-panel .e-toolbar-item')
      );
      ...
      sheetTabs.forEach((sheetTab: HTMLElement, index: number): void => {
        ...
        expect(sheetTabs.indexOf(document.activeElement.parentElement)).toBe(
          index
        );
      });
      helper.triggerKeyNativeEvent(
      ...
      sheetTabs
        .reverse()
        .forEach((sheetTab: HTMLElement, index: number): void => {
          helper.triggerKeyNativeEvent(
            9,
            false,
            true,
    ...
    it('Tab and Shift + Tab navigation inside the scrollable sheet tabs', (done: Function) => {
      ...
      expect(
        document.activeElement.classList.contains('e-scroll-left-nav')
      ).toBeTruthy();
      helper.triggerKeyNativeEvent(
        9,
        false,
      ...
      expect(
        document.activeElement.classList.contains('e-scroll-right-nav')
      ).toBeTruthy();
      helper.triggerKeyNativeEvent(
        9,
        false,
      ...
      expect(
        document.activeElement.classList.contains('e-aggregate-list')
      ).toBeTruthy();
      helper.triggerKeyNativeEvent(
        9,
        false,
    ...
    it('Enter key action in scoll nav button in the sheet tabs', (done: Function) => {
      const scrollNav: HTMLElement = helper.getElementFromSpreadsheet(
        '.e-sheet-tab-panel .e-scroll-left-nav'
      );
    ...
    it('Tab and Shift + Tab navigation inside the ribbon', (done: Function) => {
      ...
      expect(
        document.activeElement.querySelector('.e-tab-text').textContent
      ).toBe('Home');
      helper.triggerKeyNativeEvent(
        9,
        false,
      ...
      expect(
        document.activeElement.classList.contains('e-drop-icon')
      ).toBeTruthy();
      helper.triggerKeyNativeEvent(
        9,
        false,
    ...
    it('Left and right navigation inside the ribbon toolbar', (done: Function) => {
      ...
      expect(document.activeElement.id).toBe(`${helper.id}_undo`);
      const ele: HTMLElement = helper.getElementFromSpreadsheet(
        '.e-content .e-toolbar'
      );
      helper.triggerKeyNativeEvent(
        39,
      ...
      expect(document.activeElement.parentElement.id).toBe(
        `${helper.id}_number_format-popup`
      );
      helper.triggerKeyNativeEvent(
        38,
        false,
      ...
      expect(
        document.activeElement.classList.contains('e-split-colorpicker')
      ).toBeTruthy();
      helper.triggerKeyNativeEvent(
        39,
        false,
      ...
      helper.initializeSpreadsheet(
        {
          ...
          created: (): void => {
            const spreadsheet: Spreadsheet = helper.getInstance();
            spreadsheet.protectSheet('Sheet1', {
              selectCells: false,
  ...
  describe('Dependent features using keyboard actions ->', () => {
    ...
    beforeAll((done: Function) => {
      helper.initializeSpreadsheet(
        {
          sheets: [
            { ranges: [{ dataSource: defaultData }], selectedRange: 'A2:A2' },
          ],
        },
        done
      );
    ...
    const navigateMenuItemAndSelect: (
      row: number,
      col: number,
      countArr: number[],
      isRowHdr?: boolean,
      ...
      const cMenuWrapper: HTMLElement = document.activeElement
      ...
      countArr.forEach((count: number, index: number) => {
        ...
        if (countArr.length - 1 === index) {
          ...
          const focuedItem: HTMLElement = cMenuWrapper.querySelector(
    ...
    it('Cut/paste cell using context menu', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          helper.getElementFromSpreadsheet('.e-copy-indicator')
        ).not.toBeNull();
        ...
        expect(spreadsheet.sheets[0].rows[6].cells[0].value).toBe(
          'Casual Shoes'
        );
    ...
    it('Copy/paste cell using context menu', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(spreadsheet.sheets[0].rows[1].cells[0].value).toBe(
          'Casual Shoes'
        );
    ...
    it('Apply and clear filter using context menu', (done: Function) => {
      ...
      setTimeout(() => {
        const checkFilterCases: Function = () => {
          expect(spreadsheet.filterModule.filterCollection.get(0).length).toBe(
            1
          );
          expect(
            spreadsheet.filterModule.filterCollection.get(0)[0].value
          ).toBe('Casual Shoes');
    ...
    it('Sort Ascending', (done: Function) => {
      ...
      setTimeout(() => {
        setTimeout(() => {
          expect(spreadsheet.sheets[0].rows[2].cells[0].value).toBe(
            'Casual Shoes'
          );
          expect(spreadsheet.sheets[0].rows[3].cells[0].value).toBe(
            'Cricket Shoes'
          );
          expect(spreadsheet.sheets[0].rows[4].cells[0].value).toBe(
            'Flip- Flops & Slippers'
          );
          expect(spreadsheet.sheets[0].rows[10].cells[0].value).toBe(
            'T-Shirts'
          );
    ...
    it('Apply hyperlink', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        const urlInput: HTMLInputElement = helper
          .getElementFromSpreadsheet('.e-hyperlink-dlg .e-webpage')
          .lastElementChild.querySelector('.e-text');
        focus(urlInput);
        ...
        helper
          .getElementFromSpreadsheet(
            '.e-hyperlink-dlg .e-footer-content .e-primary'
          )
          .click();
        expect(spreadsheet.sheets[0].rows[1].cells[0].hyperlink.address).toBe(
          'https://www.syncfusion.com/'
        );
        expect(
          helper.invoke('getCell', [1, 0]).querySelector('.e-hyperlink')
        ).not.toBeNull();
        done();
      });
    });
    it('Edit and remove hyperlink', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        const urlInput: HTMLInputElement = helper
          .getElementFromSpreadsheet('.e-edithyperlink-dlg .e-webpage')
          .lastElementChild.querySelector('.e-text');
        urlInput.value = 'https://www.google.com/';
        helper
          .getElementFromSpreadsheet(
            '.e-edithyperlink-dlg .e-footer-content .e-primary'
          )
          .click();
        expect(spreadsheet.sheets[0].rows[1].cells[0].hyperlink.address).toBe(
        ...
        expect(
          spreadsheet.sheets[0].rows[1].cells[0].hyperlink
        ).toBeUndefined();
  ...
  describe('CR-Issues ->', () => {
    ...
    describe('I348582, EJ2-60424 ->', () => {
      beforeAll((done: Function) => {
        helper.initializeSpreadsheet(
          {
            sheets: [
              { isProtected: true, protectSettings: { selectCells: true } },
            ],
          },
          done
        );
      ...
      it('Shift selection on whole row makes scroll right even it is in viewport', (done: Function) => {
        helper.invoke('selectRange', [
          getRangeAddress([
            4,
            0,
            4,
            helper.getInstance().sheets[0].colCount - 1,
          ]),
        ]);
        helper.getElement().focus();
    ...
    describe('SF-360092 ->', () => {
      ...
      it('Cell navigation occurs while selecting cell template dropdown items', (done: Function) => {
        ...
        expect(
          cell.firstElementChild.classList.contains('e-validation-list')
        ).toBeTruthy();
        const ddl: HTMLElement = cell.getElementsByClassName(
          'e-ddl'
        )[0] as HTMLElement;
        ddl.focus();
        ...
        expect(helper.getInstance().sheets[0].selectedRange).toBe('D1:D1');
        helper
          .getElement('#' + helper.id + 'listValid_popup .e-list-item')
          .click();
        expect(
          document.activeElement.classList.contains('e-spreadsheet')
        ).toBeTruthy();
        done();
      });
    });
    ...
    describe('EJ2-59225 ->', () => {
      beforeAll((done: Function) => {
        helper.initializeSpreadsheet(
          {
            sheets: [{ rowCount: 3, colCount: 3, selectedRange: 'B2:C3' }],
      ...
      it('The spreadsheet selection moves away from the spreadsheet table when we move it with the keyboard', (done: Function) => {
        const selectionEle: HTMLElement = helper.getElement(
          '#' + helper.id + ' .e-selection'
        );
        const autofillEle: HTMLElement = helper.getElement(
          '#' + helper.id + ' .e-autofill'
        );
        const table: HTMLElement = helper.invoke('getContentTable');
        const height: number = table.offsetHeight;
        const width: number = table.offsetWidth;
  ...
  describe('EJ2-60990 ->', () => {
    beforeAll((done: Function) => {
      helper.initializeSpreadsheet(
        {
          sheets: [{ showHeaders: false, rowCount: 5, selectedRange: 'C4:C4' }],
          scrollSettings: { isFinite: true },
        },
        done
      );
  ...
  describe('Testing keyboard navigation with protect sheet->', () => {
    ...
    it('Name box Shitf + Tab Navigation', (done: Function) => {
      ...
      setTimeout(() => {
        let nameBox: HTMLInputElement = <HTMLInputElement>(
          helper.getElementFromSpreadsheet('#' + helper.id + '_name_box')
        );
        nameBox.click();
