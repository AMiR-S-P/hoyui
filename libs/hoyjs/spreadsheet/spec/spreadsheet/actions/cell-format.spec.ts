...
describe('Cell Format ->', () => {
  ...
  describe('public method ->', () => {
    ...
    it('Border', (done: Function) => {
      ...
      expect(helper.invoke('getCell', [0, 1]).style.borderRight).toBe(
        '1px solid red'
      );
      ...
      expect(helper.invoke('getCell', [3, 1]).style.borderBottom).toBe(
        '2px solid rgb(235, 64, 52)'
      );
  ...
  describe('Set Border Method->', () => {
    ...
    it('setBorder->', (done: Function) => {
      ...
      expect(
        helper.getInstance().sheets[0].rows[1].cells[0].style.borderTop
      ).toBe('1px solid red');
      expect(helper.getInstance().sheets[0].rows[1].cells[0].style.borderBottom)
        .toBeUndefined;
      expect(helper.getInstance().sheets[0].rows[1].cells[0].style.borderRight)
        .toBeUndefined;
      expect(helper.getInstance().sheets[0].rows[1].cells[0].style.borderLeft)
        .toBeUndefined;

      spreadsheet.setBorder({ borderLeft: '1px solid red' }, 'C3');
      expect(helper.getInstance().sheets[0].rows[2].cells[2].style.borderTop)
        .toBeUndefined;
      expect(helper.getInstance().sheets[0].rows[2].cells[2].style.borderBottom)
        .toBeUndefined;
      expect(helper.getInstance().sheets[0].rows[2].cells[2].style.borderRight)
        .toBeUndefined;
      expect(
        helper.getInstance().sheets[0].rows[2].cells[2].style.borderLeft
      ).toBe('1px solid red');
      ...
      expect(td.style.borderLeft).toBe('');
      expect(
        helper.getInstance().sheets[0].rows[2].cells[3].style.borderBottom
      ).toBe('3px solid red');
      expect(
        helper.getInstance().sheets[0].rows[2].cells[3].style.borderRight
      ).toBe('3px solid red');
      expect(
        helper.getInstance().sheets[0].rows[2].cells[3].style.borderTop
      ).toBe('3px solid red');
      expect(
        helper.getInstance().sheets[0].rows[2].cells[3].style.borderLeft
      ).toBe('3px solid red');
      done();
    });
    ...
    it('Apply Top Border after Hidden Rows->', (done: Function) => {
      ...
      expect(
        helper.getInstance().sheets[0].rows[9].cells[0].style.borderTop
    ...
    it('Apply Redo after applying Horizontal Border->', (done: Function) => {
      ...
      helper
        .getElement('.e-menu-item[aria-label="Horizontal Borders"]')
        .click();
      expect(
        helper.getInstance().sheets[0].rows[6].cells[3].style.borderTop
      ).toBe('1px solid #000000');
      expect(
        helper.getInstance().sheets[0].rows[6].cells[3].style.borderBottom
      ).toBe('1px solid #000000');
      helper.click('#spreadsheet_undo');
  ...
  describe('Clear Method->', () => {
    ...
    it('Clear Wrap->', (done: Function) => {
      ...
      expect(
        helper.getInstance().sheets[0].rows[0].cells[0].classList
      ).not.toContain('e-wraptext');
      done();
    });
    ...
    it('Clear Conditional Formatting->', (done: Function) => {
      helper
        .getInstance()
        .workbookConditionalFormattingModule.setCFRule({
          cfModel: { type: 'BlueDataBar', range: 'H2:H11' },
          isAction: true,
        });
      expect(
        helper.invoke('getCell', [1, 7]).getElementsByClassName('e-databar')[1]
          .style.width
      ).toBe('7%');
      expect(
        helper.invoke('getCell', [9, 7]).getElementsByClassName('e-databar')[1]
          .style.width
      ).toBe('100%');
      let spreadsheet: Spreadsheet = helper.getInstance();
      spreadsheet.clear({ type: 'Clear Formats', range: 'H2:H11' });
      expect(
        helper.invoke('getCell', [1, 7]).querySelector('.e-databar')
      ).toBeNull();
      expect(
        helper.invoke('getCell', [9, 7]).querySelector('.e-databar')
      ).toBeNull();
      done();
    });
    ...
    it('Clear Formula->', (done: Function) => {
      ...
      expect(helper.getInstance().sheets[0].rows[11].cells[7].formula).toEqual(
        '=SUM(H2:H11)'
      );
      expect(helper.getInstance().sheets[0].rows[11].cells[7].value).toEqual(
        554
      );
      let spreadsheet: Spreadsheet = helper.getInstance();
      spreadsheet.clear({ type: 'Clear All', range: 'H12' });
      expect(
        helper.getInstance().sheets[0].rows[11].cells[7].formula
      ).toBeUndefined();
      expect(
        helper.getInstance().sheets[0].rows[11].cells[7].textContent
      ).toBeUndefined();
      done();
    });
    ...
    it('Merged cell with small Row Height ->', (done: Function) => {
      ...
      expect(
        helper.getInstance().sheets[0].rows[0].cells[0].rowSpan
      ).toBeUndefined();
    ...
    it('Conditional Formatting Size updated depends on Font Size ->', (done: Function) => {
      helper.invoke('conditionalFormat', [
        { type: 'BlueDataBar', range: 'H3' },
      ]);
      expect(
        helper.invoke('getCell', [2, 7]).getElementsByClassName('e-databar')[1]
          .style.height
      ).toBe('17px');
      expect(
        helper.invoke('getCell', [2, 7]).getElementsByClassName('e-databar')[1]
  ...
  describe('Color Picker ->', () => {
    ...
    it('Cancelling cell fill color ->', (done: Function) => {
      helper.click(
        '.e-colorpicker-popup.e-popup-open span[aria-label="#ffff00ff"]'
      );
    ...
    it('Cancelling text fill color ->', (done: Function) => {
      ...
      helper.click(
        '.e-colorpicker-popup.e-popup-open span[aria-label="#ff0000ff"]'
      );
  ...
  describe('Apply border, image and chart with cell formatting->', () => {
    ...
    it('Apply cell format with merged cell and small sized row->', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          helper.getInstance().sheets[0].rows[0].cells[0].style.fontStyle
        ).toBe('italic');
        expect(
          helper.getInstance().sheets[0].rows[0].cells[0].style.fontWeight
        ).toBe('bold');
    ...
    it('Apply all border with alt + enter edited cell->', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          helper.getInstance().sheets[0].rows[10].cells[0].style.borderRight
        ).toBe('1px solid #000000');
        expect(
          helper.getInstance().sheets[0].rows[10].cells[0].style.borderBottom
        ).toBe('1px solid #000000');
        done();
      });
    });
    ...
    it('Delete image with clear all option->', (done: Function) => {
      helper
        .getInstance()
        .spreadsheetImageModule.createImageElement({
          options: {
      ...
      const imageId: string =
        helper.getInstance().sheets[0].rows[2].cells[2].image[0].id;
  ...
  describe('CR-Issues ->', () => {
        ...
        helper.initializeSpreadsheet(
          {
            ...
            sheets: [
              {
                rows: [{ index: 3, cells: [{ index: 3, value: '10-0347d' }] }],
                selectedRange: 'D4',
              },
            ],
          },
    ...
    describe('fb21556, fb21625 ->', () => {
      beforeEach((done: Function) => {
        helper.initializeSpreadsheet(
          { sheets: [{ rows: [{ cells: [{ value: 'Item Name' }] }] }] },
          done
        );
      });
      afterEach(() => {
    ...
    describe('SF-356947 ->', () => {
      ...
      it('Selection issue while apply font size as 72pt for whole column with custom height enabled', (done: Function) => {
        ...
        helper
          .getElement('#' + helper.id + '_font_size-popup')
          .firstElementChild.lastElementChild.click();
        ...
        helper
          .getElement('#' + helper.id + '_font_size-popup')
          .firstElementChild.children[5].click();
        expect(cell.style.fontSize).toBe('14pt');
        expect(cell.style.lineHeight).toBe('');
        helper.getElement('#' + helper.id + '_font_size').click();
        helper
          .getElement('#' + helper.id + '_font_size-popup')
          .firstElementChild.children[13].click();
    ...
    describe('EJ2-49588, EJ2-46530, EJ2-51216, EJ2-52952, EJ2-53948, EJ2-53048, EJ2-54306, EJ2-56161, EJ2-51575, EJ2-50222 ->', () => {
        ...
        helper.initializeSpreadsheet(
          {
            sheets: [
              {
                rows: [{ index: 3, cells: [{ index: 3, value: 'Style' }] }],
                selectedRange: 'D4',
              },
              {
                rows: [
              ...
              {
                columns: [{ width: 200 }, { width: 150 }, { width: 350 }],
                rows: [
                  {
                    cells: [
                      { value: 'CASOS DE PRUEBAS GENERALES PARA M�DULO' },
                    ],
                  },
                  { cells: [{}] },
                  { cells: [{ value: 'GENERAL' }] },
                  {
                    cells: [
                      { value: 'CASOS DE PRUEBAS' },
                      { value: 'PRUEBAS A REALIZAR' },
                      { value: 'RESULTADO ESPERADO' },
                    ],
                  },
                  {
                    cells: [
                      { value: 'Interface: Men� principal' },
                      { value: '' },
                      { wrap: true, value: '' },
                    ],
                  },
                  ...
                  {
                    cells: [
                      { value: 'Interface: �rea de Trabajo (Visor)' },
                      { wrap: true, value: '' },
                      { value: '' },
                    ],
                  },
      ...
      it('EJ2-46530 - fix the alignment issue with wrap and merge with resized row', function (done) {
        ...
        helper.invoke('updateCell', [
          { value: 'text test word one two three' },
          'A1',
        ]);
        helper.getElement('#' + helper.id + '_wrap').click();
        helper.getElement('#' + helper.id + '_vertical_align').click();
        helper
          .getElement(
            '#' + helper.id + '_vertical_align-popup .e-item:nth-child(2)'
          )
          .click();
        expect(helper.invoke('getCell', [0, 0]).textContent).toEqual(
          'text test word one two three'
        );
        done();
      });
      it('EJ2-51216 - Underline and strikethrough not working after performing row resize action->', (done: Function) => {
        ...
        helper.invoke('updateCell', [
          {
            value:
              'one two three four five six seven eight nine ten eleven twelve thirtneen fourteen fifteen',
          },
          'B10',
        ]);
        helper.getElement('#' + helper.id + '_wrap').click();
        ...
        expect(
          helper.getInstance().sheets[1].rows[9].cells[1].style.textDecoration
      ...
      it('EJ2-52952 - Borders are not getting applied properly for merged cells after applying border style and color->', (done: Function) => {
        ...
        helper.invoke('setBorder', [
          { border: '3px solid #1e88e5' },
          'C1:D2',
          'Outer',
        ]);
        expect(
          helper.getInstance().sheets[1].rows[0].cells[2].style.borderTop
        ).toBe('3px solid #1e88e5');
        expect(
          helper.getInstance().sheets[1].rows[0].cells[2].style.borderLeft
        ).toBe('3px solid #1e88e5');
        helper.invoke('selectRange', ['C1']);
        helper.getElement('#' + helper.id + '_borders').click();
        helper.getElement('.e-menu-item[aria-label="All Borders"]').click();
        expect(
          helper.getInstance().sheets[1].rows[0].cells[2].style.borderRight
        ).toBe('1px solid #000000');
        expect(
          helper.getInstance().sheets[1].rows[0].cells[2].style.borderBottom
        ).toBe('1px solid #000000');
        done();
      });
      it('EJ2-53948 - Unable to apply bottom border to a merged cell->', (done: Function) => {
        ...
        expect(
          helper.getInstance().sheets[1].rows[0].cells[5].style.borderBottom
        ).toBe('1px solid #000000');
        done();
      });
      it('EJ2-53048 - undo redo for merged cell with border does not work->', (done: Function) => {
        ...
        helper.invoke('copy', ['H1:I2']).then(() => {
          ...
          helper.click('#spreadsheet_undo');
          expect(helper.invoke('getCell', [0, 10]).style.backgroundColor)
            .toBeNull;
          expect(helper.invoke('getCell', [1, 10]).style.backgroundColor)
            .toBeNull;
          done();
        });
      });
      it('EJ2-54306 - Toggle Button state not refreshed in ribbon while clearing format for a cell using clear format option->', (done: Function) => {
        ...
        helper.getElement('#' + helper.id + '_underline').click();
        expect(
          helper.getInstance().sheets[1].rows[0].cells[10].style.fontWeight
        ).toBe('bold');
        expect(
          helper.getInstance().sheets[1].rows[0].cells[10].style.fontStyle
        ).toBe('italic');
        expect(
          helper.getInstance().sheets[1].rows[0].cells[10].style.textDecoration
        ).toBe('underline line-through');
        helper.getElement('#' + helper.id + '_clear').click();
        helper.click('#' + helper.id + '_clear-popup ul li:nth-child(2)');
        expect(
          helper.getElement('#' + helper.id + '_bold').classList
        ).not.toContain('e-active');
        expect(
          helper.getElement('#' + helper.id + '_italic').classList
        ).not.toContain('e-active');
        expect(
          helper.getElement('#' + helper.id + '_line-through').classList
        ).not.toContain('e-active');
        expect(
          helper.getElement('#' + helper.id + '_underline').classList
        ).not.toContain('e-active');
        done();
      });
      it('EJ2-56161 - While copy paste the merge cell with bottom border, the border is missing in pasted cell->', (done: Function) => {
        ...
        helper.getElement('.e-menu-item[aria-label="Bottom Borders"]').click();
        expect(
          helper.getInstance().sheets[1].rows[0].cells[11].style.borderBottom
        ).toBe('1px solid #000000');
        expect(
          helper.getInstance().sheets[1].rows[1].cells[12].style.borderBottom
        ).toBe('1px solid #000000');
        helper.invoke('copy', ['L1:M2']).then(() => {
          helper.invoke('paste', ['L4']);
          expect(
            helper.getInstance().sheets[1].rows[3].cells[11].style.borderBottom
          ).toBe('1px solid #000000');
          expect(
            helper.getInstance().sheets[1].rows[4].cells[12].style.borderBottom
          ).toBe('1px solid #000000');
          done();
        });
      });
      it('EJ2-51575 - Undo button doesnot work as expected in spreadsheet control', (done: Function) => {
        ...
        helper.click('_fill_color_picker .e-dropdown-btn');
        helper.click(
          '.e-colorpicker-popup.e-popup-open span[aria-label="#33ffffff"]'
        );
        helper.getElement('#' + helper.id + '_underline').click();
        helper.getElement('#' + helper.id + '_line-through').click();
        expect(
          helper.getInstance().sheets[1].rows[0].cells[14].style.textDecoration
        ).toBe('underline line-through');
        expect(
          helper.getInstance().sheets[1].rows[1].cells[14].style.textDecoration
        ).toBe('underline line-through');
        expect(helper.invoke('getCell', [0, 14]).style.backgroundColor).toBe(
          'rgb(51, 255, 255)'
        );
        expect(helper.invoke('getCell', [1, 14]).style.backgroundColor).toBe(
          'rgb(51, 255, 255)'
        );
        ...
        expect(helper.invoke('getCell', [0, 14]).style.backgroundColor)
          .toBeNull;
        expect(helper.invoke('getCell', [1, 14]).style.backgroundColor)
          .toBeNull;
        done();
      });
      it('EJ2-50222 - cell border issue while applying Merge and wrap text', (done: Function) => {
        ...
        setTimeout(() => {
          ...
          expect(helper.invoke('getCell', [4, 0]).style.borderLeft).toBe(
            '2px solid rgb(0, 0, 0)'
          );
          done();
        });
  ...
  describe('EJ2-58338, EJ2-840548 ->', () => {
    ...
    it('Clear Option does not work, when values are selected from bottom to top / left to right', (done: Function) => {
      ...
      helper.getElement('#' + helper.id + '_clear').click();
      helper
        .getElement('#' + helper.id + '_clear-popup li:nth-child(3)')
        .click();
    ...
    it('Cell borders removed on undo action after copy/paste from external excel file', (done: Function) => {
      ...
      spreadsheet.setBorder(
        { borderRight: '1px dashed red', borderBottom: '1px dashed red' },
        'G2:H2'
      );
      spreadsheet.setBorder(
        { borderRight: '1px dashed red', borderBottom: '1px dashed red' },
      ...
      helper.invoke('copy', ['J1:J5']).then(() => {
        ...
        helper.getElement('#' + helper.id + '_undo').click();
        expect(spreadsheet.sheets[0].rows[0].cells[7].style.borderBottom).toBe(
          '1px dashed red'
        );
        expect(helper.invoke('getCell', [0, 7]).style.borderBottom).toBe(
          '1px dashed red'
        );
        expect(spreadsheet.sheets[0].rows[1].cells[6].style.borderBottom).toBe(
          '1px dashed red'
        );
        expect(helper.invoke('getCell', [1, 6]).style.borderBottom).toBe(
          '1px dashed red'
        );
        expect(spreadsheet.sheets[0].rows[3].cells[7].style.borderBottom).toBe(
          '1px dashed red'
        );
        expect(helper.invoke('getCell', [3, 7]).style.borderBottom).toBe(
          '1px dashed red'
        );
        expect(spreadsheet.sheets[0].rows[4].cells[6].style.borderBottom).toBe(
          '1px dashed red'
        );
        expect(helper.invoke('getCell', [4, 6]).style.borderBottom).toBe(
          '1px dashed red'
        );
        done();
      });
  ...
  describe('Applying formats to the hidden rows ->', () => {
    ...
    it('Apply cell format border to the hidden row ', (done: Function) => {
      ...
      spreadsheet.applyFilter(
        [{ field: 'E', predicate: 'or', operator: 'notequal', value: '15' }],
        'A1:H1'
      );
      setTimeout(() => {
        spreadsheet.cellFormat(
          { fontWeight: 'bold', color: '#ffffff', border: '12pt' },
          'A4:E4'
        );
        spreadsheet.cellFormat(
          { fontWeight: 'bold', color: '#ffffff', border: '12pt' },
        ...
        helper.getElement('#' + helper.id + '_line-through').click();
        expect(
          spreadsheet.sheets[0].rows[0].cells[0].style.textDecoration
        ).toBe('underline');
