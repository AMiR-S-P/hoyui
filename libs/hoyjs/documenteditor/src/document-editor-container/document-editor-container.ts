...
import {
  ...
  DocumentSettings,
} from '../document-editor/document-editor';
...
import { createSpinner } from '@hoyui/popups';
import {
...
} from '../document-editor/document-editor-model';
import {
  CharacterFormatProperties,
  ParagraphFormatProperties,
  SectionFormatProperties,
} from '../document-editor/implementation';
...
} from '../document-editor/base/events-helper';
import { ClickEventArgs } from '@hoyui/navigations';
import {
  beforeAutoResize,
  internalAutoResize,
  internalZoomFactorChange,
  beforeCommentActionEvent,
  commentDeleteEvent,
  contentChangeEvent,
  trackChangeEvent,
  beforePaneSwitchEvent,
  serviceFailureEvent,
  documentChangeEvent,
  ...
  protectionTypeChangeEvent,
  internalDocumentEditorSettingsChange,
  internalStyleCollectionChange,
  revisionActionEvent,
  trackChanges,
 ...
 */
@NotifyPropertyChanges
export class DocumentEditorContainer
  ...
  implements INotifyPropertyChanged
{
   ...
   */
  public constructor(
    options?: DocumentEditorContainerModel,
  ...
  public defaultLocale: Object = {
    ...
    'Toggle between the internal clipboard and system clipboard':
      'Toggle between the internal clipboard and system clipboard.</br>' +
    ...
    'Link to PreviousTooltip':
      'Link this section with previous section header or footer',
  ...
  public onPropertyChanged(
    newModel: DocumentEditorContainerModel,
    oldModel: DocumentEditorContainerModel
  ): void {
    for (let prop of Object.keys(newModel)) {
      switch (prop) {
        ...
        case 'enableTrackChanges':
          if (
            this.documentEditor.documentHelper.isTrackedOnlyMode &&
            !newModel.enableTrackChanges &&
            newModel.enableTrackChanges !== this.enableTrackChanges
          ) {
            this.enableTrackChanges = true;
          }
          if (this.documentEditor) {
            this.documentEditor.enableTrackChanges =
              newModel.enableTrackChanges;
            if (this.toolbarModule) {
              this.toolbarModule.toggleTrackChanges(
                newModel.enableTrackChanges
              );
            }
        ...
        case 'enableComment':
          ...
          if (this.toolbarModule) {
            this.toolbarModule.enableDisableInsertComment(
              newModel.enableComment
            );
          }
          break;
        ...
        case 'documentSettings':
          if (this.documentEditor) {
            this.documentEditor.documentSettings.compatibilityMode =
              this.documentSettings.compatibilityMode;
          }
          break;
        case 'documentEditorSettings':
          ...
          if (
            !isNullOrUndefined(newModel.documentEditorSettings.fontFamilies)
          ) {
            const fontFamilyValue: string[] =
              newModel.documentEditorSettings.fontFamilies;
        ...
        case 'autoResizeOnVisibilityChange':
          if (this.documentEditor) {
            this.documentEditor.autoResizeOnVisibilityChange =
              newModel.autoResizeOnVisibilityChange;
          }
          break;
      }
    }
  ...
  protected render(): void {
    ...
    this.headerFooterProperties = new HeaderFooterProperties(
      this,
      this.enableRtl
    );
  ...
  private restrictEditingToggleHelper(restrictEditing: boolean): void {
    ...
    this.documentEditor.trackChangesPane.enableDisableButton(
      !restrictEditing &&
  ...
  private setserverActionSettings(): void {
    if (this.serviceUrl) {
      this.documentEditor.serviceUrl = HelperMethods.sanitizeString(
        this.serviceUrl
      );
    }
    if (this.serverActionSettings.spellCheck) {
      this.documentEditor.serverActionSettings.spellCheck =
        HelperMethods.sanitizeString(this.serverActionSettings.spellCheck);
    }
    if (this.serverActionSettings.spellCheckByPage) {
      this.documentEditor.serverActionSettings.spellCheckByPage =
        HelperMethods.sanitizeString(
          this.serverActionSettings.spellCheckByPage
        );
    }
    if (this.serverActionSettings.restrictEditing) {
      this.documentEditor.serverActionSettings.restrictEditing =
        HelperMethods.sanitizeString(this.serverActionSettings.restrictEditing);
    }
    if (this.serverActionSettings.systemClipboard) {
      this.documentEditor.serverActionSettings.systemClipboard =
        HelperMethods.sanitizeString(this.serverActionSettings.systemClipboard);
    }
    if (this.serverActionSettings.import) {
      this.documentEditor.serverActionSettingsImport =
        HelperMethods.sanitizeString(this.serverActionSettings.import);
    }
    if (this.headers) {
      this.documentEditor.headers = JSON.parse(
        HelperMethods.sanitizeString(JSON.stringify(this.headers))
      );
    }
  }
  private customizeDocumentEditorSettings(): void {
    if (this.documentEditorSettings.formFieldSettings) {
      let settings: FormFieldSettingsModel =
        this.documentEditorSettings.formFieldSettings;
      let documentEditor: DocumentEditor = this.documentEditor;
      if (!isNullOrUndefined(settings.applyShading)) {
        documentEditor.documentEditorSettings.formFieldSettings.applyShading =
          settings.applyShading;
      }
      if (!isNullOrUndefined(settings.formFillingMode)) {
        documentEditor.documentEditorSettings.formFieldSettings.formFillingMode =
          settings.formFillingMode;
      }
      if (!isNullOrUndefined(settings.formattingExceptions)) {
        documentEditor.documentEditorSettings.formFieldSettings.formattingExceptions =
          settings.formattingExceptions;
      }
      if (!isNullOrUndefined(settings.selectionColor)) {
        documentEditor.documentEditorSettings.formFieldSettings.selectionColor =
          settings.selectionColor;
      }
      if (!isNullOrUndefined(settings.shadingColor)) {
        documentEditor.documentEditorSettings.formFieldSettings.shadingColor =
          settings.shadingColor;
      }
    }
    if (this.documentEditorSettings.searchHighlightColor) {
      this.documentEditor.documentEditorSettings.searchHighlightColor =
        HelperMethods.sanitizeString(
          this.documentEditorSettings.searchHighlightColor
        );
    }
    if (this.documentEditorSettings.fontFamilies) {
      this.documentEditor.documentEditorSettings.fontFamilies = JSON.parse(
        HelperMethods.sanitizeString(
          JSON.stringify(this.documentEditorSettings.fontFamilies)
        )
      );
    }
    if (this.documentEditorSettings.collaborativeEditingSettings) {
      this.documentEditor.documentEditorSettings.collaborativeEditingSettings =
        this.documentEditorSettings.collaborativeEditingSettings;
    }
    if (this.documentEditorSettings.printDevicePixelRatio) {
      this.documentEditor.documentEditorSettings.printDevicePixelRatio =
        this.documentEditorSettings.printDevicePixelRatio;
    }
    if (
      !isNullOrUndefined(
      ...
      this.documentEditor.documentEditorSettings.enableOptimizedTextMeasuring =
        this.documentEditorSettings.enableOptimizedTextMeasuring;
    }
    if (!isNullOrUndefined(this.documentEditorSettings.maximumRows)) {
      this.documentEditor.documentEditorSettings.maximumRows =
        this.documentEditorSettings.maximumRows;
    }
    if (!isNullOrUndefined(this.documentEditorSettings.maximumColumns)) {
      this.documentEditor.documentEditorSettings.maximumColumns =
        this.documentEditorSettings.maximumColumns;
    }
    ...
    if (!isNullOrUndefined(this.documentEditorSettings.showBookmarks)) {
      this.documentEditor.documentEditorSettings.showBookmarks =
        this.documentEditorSettings.showBookmarks;
    }
    if (
      !isNullOrUndefined(this.documentEditorSettings.highlightEditableRanges)
    ) {
      this.documentEditor.documentEditorSettings.highlightEditableRanges =
        this.documentEditorSettings.highlightEditableRanges;
    }

    if (!isNullOrUndefined(this.documentEditorSettings.allowDragAndDrop)) {
      this.documentEditor.documentEditorSettings.allowDragAndDrop =
        this.documentEditorSettings.allowDragAndDrop;
    }
    if (!isNullOrUndefined(this.documentEditorSettings.optimizeSfdt)) {
      this.documentEditor.documentEditorSettings.optimizeSfdt =
        this.documentEditorSettings.optimizeSfdt;
    }
    if (!isNullOrUndefined(this.documentEditorSettings.autoResizeSettings)) {
      this.documentEditor.documentEditorSettings.autoResizeSettings =
        this.documentEditorSettings.autoResizeSettings;
    }
    ...
    if (!isNullOrUndefined(this.documentEditorSettings.colorPickerSettings)) {
      this.documentEditor.documentEditorSettings.colorPickerSettings =
        this.documentEditorSettings.colorPickerSettings;
    }
    if (!isNullOrUndefined(this.documentEditorSettings.popupTarget)) {
      this.documentEditor.documentEditorSettings.popupTarget =
        this.documentEditorSettings.popupTarget;
    }
    if (!isNullOrUndefined(this.documentEditorSettings.showNavigationPane)) {
      this.documentEditor.documentEditorSettings.showNavigationPane =
        this.documentEditorSettings.showNavigationPane;
    }
    if (!isNullOrUndefined(this.documentEditorSettings.mentionSettings)) {
      this.documentEditor.documentEditorSettings.mentionSettings =
        this.documentEditorSettings.mentionSettings;
    }
  }
  /**
   * @private
  ...
  private initContainerElement(): void {
    ...
    this.propertiesPaneContainer = this.createElement('div', {
      className: propertiesPaneContainerBorder,
      styles: 'display:none',
    });
    this.editorContainer.appendChild(this.propertiesPaneContainer);
    this.containerTarget.appendChild(this.editorContainer);
    this.statusBarElement = this.createElement('div', {
      className: 'e-de-status-bar',
    });
    if (isRtl) {
  ...
  private createToolbarContainer(isRtl: boolean, isCustom?: boolean): void {
    if (isNullOrUndefined(this.editorContainer)) {
      this.editorContainer = this.createElement('div', {
        className:
          'e-de-tool-ctnr-properties-pane' + (isRtl ? ' e-de-ctnr-rtl' : ''),
      });
    }
    if (this.enableToolbar) {
  ...
  private initializeDocumentEditor(): void {
    ...
    let documentEditorTarget: HTMLElement = this.createElement('div', {
      id: id,
    ...
    this.documentEditorInternal = new DocumentEditor({
      ...
      customContextMenuBeforeOpen:
        this.onCustomContextMenuBeforeOpen.bind(this),
  ...
  private wireEvents(): void {
    ...
    this.documentEditor.on(
      internalZoomFactorChange,
      this.onZoomFactorChange,
      this
    );
  ...
  public showHidePropertiesPane(show: boolean): void {
    ...
    if (this.toolbarModule) {
      this.toolbarModule.propertiesPaneButton.element.style.opacity = show
        ? '1'
        : '0.5';
    }
    this.documentEditor.resize();
  ...
  private updateStyleCollection(): void {
    if (
      ...
      !isNullOrUndefined(this.tableProperties.tableTextProperties) &&
      !isNullOrUndefined(this.tableProperties.tableTextProperties.paragraph)
    ) {
  ...
  public refreshFontFamilies(fontFamilies: string[]): void {
    if (
      !isNullOrUndefined(this.tableProperties) &&
      !isNullOrUndefined(this.tableProperties.tableTextProperties) &&
      !isNullOrUndefined(this.tableProperties.tableTextProperties.text)
    ) {
      ...
      for (let i: number = 0; i < fontFamilies.length; i++) {
        ...
        const fontStyleValue: { [key: string]: any } = {
          FontName: fontValue,
          FontValue: fontValue,
        };
  ...
  public onContentChange(args: ContentChangeEventArgs): void {
    ...
    let eventArgs: ContainerContentChangeEventArgs = {
      source: this,
  ...
  public onDocumentChange(): void {
    ...
    if (
      !isNullOrUndefined(this.documentSettings) &&
      !isNullOrUndefined(this.documentEditor) &&
    ...
    if (
      !isNullOrUndefined(this.documentEditorSettings) &&
      !isNullOrUndefined(this.documentEditorSettings.fontFamilies)
    ) {
      const fontFamilyValue: string[] =
        this.documentEditorSettings.fontFamilies;
      this.refreshFontFamilies(fontFamilyValue);
    }
    if (this.toolbarModule) {
    ...
    if (
      this.documentEditor.documentHelper.contentControlCollection.length > 0
    ) {
  ...
  public onSelectionChange(): void {
    setTimeout(() => {
      if (!isNullOrUndefined(this.documentEditor)) {
        this.showPropertiesPaneOnSelection();
        let eventArgs: ContainerSelectionChangeEventArgs = {
          source: this,
          isCompleted: this.documentEditor.documentHelper.isSelectionCompleted,
        };
    ...
    if (args.linkType !== 'Bookmark') {
      ...
      if (
        ...
        DialogUtility.alert({
          ...
          content: this.localObj.getConstant(
            'The address of this site is not valid. Check the address and try again.'
          ),
  ...
  private onCustomContextMenuBeforeOpen(
    args: BeforeOpenCloseCustomContentMenuEventArgs
  ): void {
  ...
  public showPropertiesPaneOnSelection(): void {
    if (
      (this.restrictEditing && !this.showPropertiesPane) ||
      isNullOrUndefined(this.tableProperties)
    ) {
      return;
    }
    let isProtectedDocument: boolean =
      this.documentEditor.documentHelper.protectionType !== 'NoProtection';
    let allowFormatting: boolean =
      isProtectedDocument &&
      this.documentEditor.documentHelper.restrictFormatting;
    let isSelectionInProtectecRegion: boolean =
      this.documentEditor.editorModule.restrictEditing;

    if (isProtectedDocument) {
      if (this.toolbarModule) {
        this.toolbarModule.enableDisableToolBarItem(
          !isSelectionInProtectecRegion,
          true
        );
      }
      this.tableProperties.enableDisableElements(
        !allowFormatting && !isSelectionInProtectecRegion
      );
      this.tocProperties.enableDisableElements(!isSelectionInProtectecRegion);
      this.headerFooterProperties.enableDisableElements(
        !isSelectionInProtectecRegion
      );
    ...
    let currentContext: string =
      this.documentEditor.selectionModule.contextType;
    let isInHeaderFooter: boolean =
      currentContext.indexOf('Header') >= 0 ||
    ...
    if (!this.showPropertiesPane) {
      ...
      if (isInHeaderFooter && this.showHeaderProperties) {
        ...
        currentContext.indexOf('List') >= 0 ||
        (currentContext.indexOf('Text') >= 0 &&
    ...
    if (this.toolbarModule && this.toolbarModule.toolbar) {
      this.toolbarModule.enableDisableInsertComment(
        !this.documentEditor.enableHeaderAndFooter &&
          this.enableComment &&
          !this.documentEditor.isReadOnlyMode &&
          !this.documentEditor.selectionModule.isinFootnote &&
  ...
  public showProperties(property: string): void {
    ...
    this.tableProperties.showTableProperties(
      property === 'table' || property === 'text',
      property
    );
    this.imageProperties.showImageProperties(property === 'image');
    this.headerFooterProperties.showHeaderFooterPane(
      property === 'headerfooter'
    );
  ...
  public destroy(): void {
    ...
    if (
      this.propertiesPaneContainer &&
      this.propertiesPaneContainer.parentElement
    ) {
