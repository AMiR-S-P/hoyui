...
import { Toolbar as EJ2Toolbar, ClickEventArgs } from '@hoyui/navigations';
...
import { showSpinner, hideSpinner, DialogUtility } from '@hoyui/popups';
...
import {
  ...
  toolbarClickEvent,
  beforeFileOpenEvent,
} from '../../document-editor/base/index';
...
import { ListView, SelectEventArgs as ListSelectEventArgs } from '@hoyui/lists';
...
export class Toolbar {
  ...
  private renderToolBar(): void {
    ...
    const toolbarTarget: HTMLElement = createElement('div', {
    ...
    const propertiesPaneDiv: HTMLElement = createElement('div', {
    ...
    let cssClassName: string =
      'e-tbar-btn e-tbtn-txt e-control e-btn e-de-showhide-btn';
    ...
    if (this.container.showPropertiesPane) {
      ...
      classList(
        propertiesPaneDiv,
        this.container.restrictEditing ? ['e-de-overlay'] : [],
        this.container.restrictEditing ? [] : ['e-de-overlay']
      );
        ...
        const breakDataSource: any = [
          {
            ...
            iconCss:
              'e-de-listview e-de-listview-icon e-icons e-de-ctnr-page-break',
            id: PAGE_BREAK,
          ...
          },
          {
            ...
            iconCss:
              'e-de-listview e-de-listview-icon e-icons e-de-ctnr-page-break-column',
            id: COLUMN_BREAK,
            category: locale.getConstant('Page Breaks'),
          },
          {
            ...
            iconCss:
              'e-de-listview e-de-listview-icon e-icons e-de-ctnr-section-break',
            id: SECTION_BREAK,
          ...
          },
          {
            ...
            iconCss:
              'e-de-listview e-de-listview-icon e-icons e-de-ctnr-section-break-continuous',
  ...
  private onListViewSelection(args: ListSelectEventArgs): void {
    ...
    if (id === parentId + BREAK_ID + LISTVIEW_ID + '_' + PAGE_BREAK) {
      ...
      id ===
      parentId + BREAK_ID + LISTVIEW_ID + '_' + SECTION_BREAK_CONTINUOUS
    ) {
      this.container.documentEditor.editorModule.insertSectionBreak(
  ...
  private onBeforeRenderRestrictDropdown(
    ...
    const selectedIcon: HTMLElement = args.element.getElementsByClassName(
      'e-menu-icon'
    )[0] as HTMLElement;
    if (!isNullOrUndefined(selectedIcon)) {
      ...
      if (args.item.id === id + PROTECTIONS) {
        const restrictPane: HTMLElement = document.getElementsByClassName(
          'e-de-restrict-pane'
        )[0] as HTMLElement;
        if (!isNullOrUndefined(restrictPane)) {
          const toggleProtection: boolean = !(
            restrictPane.style.display === 'none'
          );
  ...
  private showHidePropertiesPane(): void {
    const paneDiv: HTMLElement = document.getElementsByClassName(
    ...
    if (this.container.propertiesPaneContainer.style.display === 'none') {
      ...
      this.buttonElement.setAttribute(
        'aria-label',
        locale.getConstant('Hide properties pane')
      );
      ...
      this.container.previousContext.indexOf('Header') >= 0 ||
      this.container.previousContext.indexOf('Footer') >= 0
    ) {
      this.container.showHeaderProperties =
        !this.container.showHeaderProperties;
    } else {
      ...
      this.buttonElement.setAttribute(
        'aria-label',
        locale.getConstant('Show properties pane')
      );
      ...
      switch (tItem[parseInt(i.toString(), 10)]) {
        ...
        case 'Image':
          toolbarItems.push({
            template:
              '<button title="' +
              locale.getConstant('Insert inline picture from a file') +
              '" class="e-tbar-btn e-tbtn-txt e-control e-btn e-lib e-dropdown-btn e-de-toolbar-btn-first e-caret-hide" type="button" id="' +
              id +
              INSERT_IMAGE_ID +
              '"><span class="e-btn-icon e-icons e-de-ctnr-image e-icon-left"></span><span class="e-tbar-btn-text">' +
              locale.getConstant('Image') +
              '</span><span class="e-btn-icon e-icons e-icon-right e-caret"></span></button>',
            id: id + INSERT_IMAGE_ID,
            htmlAttributes: {
              'aria-label': locale.getConstant(
        ...
        case 'Bookmark':
          toolbarItems.push({
            ...
            tooltipText: locale.getConstant(
        ...
        case 'PageSetup':
          toolbarItems.push({
            template:
              '<button title="' +
              locale.getConstant('Page Setup') +
              '" class="e-tbar-btn e-tbtn-txt e-control e-btn e-lib e-dropdown-btn e-caret-hide" type="button" id="' +
              id +
              PAGE_SET_UP_ID +
              '"><span class="e-btn-icon e-icons e-de-ctnr-pagesetup e-icon-left"></span><span class="e-tbar-btn-text">' +
              this.onWrapText(locale.getConstant('Page Setup')) +
              '</span><span class="e-btn-icon e-icons e-icon-right e-caret"></span></button>',
            id: id + PAGE_SET_UP_ID,
            htmlAttributes: { 'aria-label': locale.getConstant('Page Setup') },
          });
          break;
        case 'PageNumber':
          toolbarItems.push({
            ...
            cssClass: className,
            htmlAttributes: { 'aria-label': locale.getConstant('Page Number') },
          });
          break;
        case 'Break':
          toolbarItems.push({
            template:
              '<button title="' +
              locale.getConstant('Break') +
              '" class="e-tbar-btn e-tbtn-txt e-control e-btn e-lib e-dropdown-btn e-caret-hide" type="button" id="' +
              id +
              BREAK_ID +
              '"><span class="e-btn-icon e-icons e-de-ctnr-break e-icon-left"></span><span class="e-tbar-btn-text">' +
              locale.getConstant('Break') +
              '</span><span class="e-btn-icon e-icons e-icon-right e-caret"></span></button><div id="' +
              id +
              BREAK_ID +
              LISTVIEW_ID +
              '"></div>',
            id: id + BREAK_ID,
            htmlAttributes: { 'aria-label': locale.getConstant('Break') },
          });
          break;
        case 'Find':
          toolbarItems.push({
            ...
            cssClass: className,
            htmlAttributes: { 'aria-label': locale.getConstant('Find Text') },
          });
          break;
        case 'LocalClipboard':
          toolbarItems.push({
        ...
        case 'RestrictEditing':
          toolbarItems.push({
            template:
              '<button title="' +
              locale.getConstant('Restrict editing') +
              '" class="e-tbar-btn e-tbtn-txt e-control e-btn e-lib e-dropdown-btn e-de-toolbar-btn-first e-caret-hide" type="button" id="' +
              id +
              RESTRICT_EDITING_ID +
              '"><span class="e-btn-icon e-de-ctnr-lock e-icons e-icon-left"></span><span class="e-tbar-btn-text">' +
              this.onWrapText(locale.getConstant('Restrict Editing')) +
              '</span><span class="e-btn-icon e-icons e-icon-right e-caret"></span></button>',
            htmlAttributes: {
        ...
        case 'FormFields':
          toolbarItems.push({
            template:
              '<button title="' +
              locale.getConstant('Form Fields') +
              '" class="e-tbar-btn e-tbtn-txt e-control e-btn e-lib e-dropdown-btn e-de-toolbar-btn-first e-caret-hide" type="button" id="' +
              id +
              FORM_FIELDS_ID +
              '"><span class="e-btn-icon e-de-formfield e-icons e-icon-left"></span><span class="e-tbar-btn-text">' +
              this.onWrapText(locale.getConstant('Form Fields')) +
              '</span><span class="e-btn-icon e-icons e-icon-right e-caret"></span></button>',
        ...
        case 'ContentControl':
          toolbarItems.push({
            template:
              '<button title="' +
              locale.getConstant('Content Control') +
              '" class="e-tbar-btn e-tbtn-txt e-control e-btn e-lib e-dropdown-btn e-caret-hide" type="button" id="' +
              id +
              CONTENT_CONTROL_ID +
              '"><span class="e-btn-icon e-icons e-de-ctnr-content-control e-icon-left"></span><span class="e-tbar-btn-text">' +
              this.onWrapText(locale.getConstant('Content Control')) +
              '</span><span class="e-btn-icon e-icons e-icon-right e-caret"></span></button>',
            id: id + CONTENT_CONTROL_ID,
            htmlAttributes: {
              'aria-label': locale.getConstant('Content Control'),
            },
          });
          break;
        case 'XML Mapping':
          toolbarItems.push({
            prefixIcon: 'e-de-ctnr-xml-mapping',
            tooltipText: locale.getConstant('XML Mapping Pane'),
            id: id + XMLMAPPING_ID,
            text: this.onWrapText(locale.getConstant('XML Mapping Pane')),
            cssClass: className,
            htmlAttributes: {
              'aria-label': locale.getConstant('XML Mapping Pane'),
            },
          });
          break;
        default:
  ...
  private clickHandler(args: ClickEventArgs): void {
    ...
    if (
      args.item.id === id + NEW_ID ||
      args.item.id === id + OPEN_ID ||
      args.item.id === id + UNDO_ID ||
      args.item.id === REDO_ID ||
      args.item.id === id + XMLMAPPING_ID ||
      args.item.id === id + COMMENT_ID ||
      args.item.id === id + TRACK_ID ||
      args.item.id === id + HEADER_ID ||
      args.item.id === id + TABLE_OF_CONTENT_ID ||
      args.item.id === id + FOOTER_ID ||
      args.item.id === id + PAGE_NUMBER_ID ||
      args.item.id === id + CLIPBOARD_ID ||
      args.item.id === id + UPDATE_FIELDS_ID ||
      args.item.id === id + FOOTNOTE_ID ||
      args.item.id === id + ENDNOTE_ID ||
      args.item.id === id + PAGE_SET_UP_ID ||
      args.item.id === id + BREAK_ID ||
      args.item.id === id + RESTRICT_EDITING_ID ||
      args.item.id === id + FORM_FIELDS_ID
    ) {
      this.documentEditor.focusIn();
    }
  }
  ...
  public toggleRestrictEditing(enable: boolean): void {
    const restrictEditingId: string =
      this.container.element.id + TOOLBAR_ID + RESTRICT_EDITING_ID;
  ...
  private onDropDownButtonSelect(args: MenuEventArgs): void {
    ...
    if (id === parentId + INSERT_IMAGE_LOCAL_ID) {
      ...
      this.documentEditor.documentHelper.restrictEditingPane.showHideRestrictPane(
        true
      );
  ...
  private onFileChange(): void {
    ...
    let check: boolean;
    const eventArgs: BeforeFileOpenArgs = {
      fileSize: filesize,
      isCanceled: check,
    };
    ...
    if (file) {
      ...
      if (formatType === '.sfdt' || formatType === '.txt') {
        ...
        fileReader.onload = (): void => {
          if (formatType === '.txt') {
            this.container.documentEditor.documentHelper.openTextFile(
              fileReader.result as string
            );
          } else {
            /* eslint-disable */
            this.container.documentEditor.openAsync(
              fileReader.result as string
            );
            /* eslint-enable */
          }
        ...
        if (this.isSupportedFormatType(formatType.toLowerCase())) {
          ...
          const localizeValue: L10n = new L10n(
            'documenteditor',
            this.documentEditor.defaultLocale
          );
          DialogUtility.alert({
      ...
      this.container.documentEditor.documentName = file.name.substr(
        0,
  ...
  private failureHandler(args: any): void {
    if (args.name === 'onError') {
      DialogUtility.alert({
        content: this.container.localObj.getConstant(
  ...
  private insertImage(data: string): void {
    ...
    image.addEventListener('load', function (): void {
      container.documentEditor.editorModule.insertImageInternal(
        data,
        true,
        this.width,
        this.height,
  ...
  public enableDisableInsertComment(enable: boolean): void {
    ...
    if (!this.container.enableComment && element) {
      ...
      if (
        ...
        (this.documentEditor.isReadOnly ||
          this.documentEditor.documentHelper.isDocumentProtected)
      ) {
        enable =
          this.documentEditor.documentHelper.isCommentOnlyMode ||
   ...
   * @returns {void}
   */
  public enableDisableToolBarItem(
    enable: boolean,
    ...
    if (!isNullOrUndefined(this.container.element)) {
      ...
      for (const item of this.toolbar.items) {
        ...
        if (
          ...
          itemId !== id + OPEN_ID &&
          itemId !== id + FIND_ID &&
          itemId !== id + CLIPBOARD_ID &&
          itemId !== id + RESTRICT_EDITING_ID &&
          item.type !== 'Separator'
        ) {
          ...
          if (
            itemId !== id + UNDO_ID &&
            itemId !== id + REDO_ID &&
            itemId !== id + INSERT_TABLE_ID &&
            itemId !== id + INSERT_LINK_ID &&
            itemId !== id + BOOKMARK_ID &&
            itemId !== id + COMMENT_ID &&
            itemId !== id + HEADER_ID &&
            itemId !== id + XMLMAPPING_ID &&
            itemId !== id + TABLE_OF_CONTENT_ID &&
            itemId !== id + FOOTER_ID &&
            itemId !== id + PAGE_SET_UP_ID &&
            itemId !== id + CONTENT_CONTROL_ID &&
            itemId !== id + PAGE_NUMBER_ID &&
            itemId !== id + INSERT_IMAGE_ID &&
            itemId !== id + FORM_FIELDS_ID &&
            itemId !== id + BREAK_ID &&
            itemId !== id + TRACK_ID &&
            itemId !== id + FOOTNOTE_ID &&
          ...
          if (
            isProtectedContent &&
            this.documentEditor.documentHelper.isFormFillProtectedMode &&
            itemId === id + UPDATE_FIELDS_ID
          ) {
            continue;
          }
      ...
      if (!isNullOrUndefined(this.documentEditor)) {
        this.enableDisableFormField(
          !this.documentEditor.enableHeaderAndFooter &&
            enable &&
            !this.documentEditor.isReadOnlyMode
        );
      }
      const isPlainContetnControl: boolean =
      ...
      if (
        ...
        this.documentEditor.enableHeaderAndFooter ||
        isPlainContetnControl
      ) {
        if (this.containsItem(id + ENDNOTE_ID)) {
          this.toolbar.enableItems(
            document.getElementById(id + ENDNOTE_ID).parentElement,
            false
          );
        }
        if (this.containsItem(id + FOOTNOTE_ID)) {
          this.toolbar.enableItems(
            document.getElementById(id + FOOTNOTE_ID).parentElement,
            false
          );
        }
        if (this.containsItem(id + BREAK_ID)) {
          this.toolbar.enableItems(
            document.getElementById(id + BREAK_ID).parentElement,
            false
          );
        }
        if (isPlainContetnControl) {
          if (this.containsItem(id + INSERT_TABLE_ID)) {
            this.toolbar.enableItems(
              document.getElementById(id + INSERT_TABLE_ID).parentElement,
              false
            );
          }
          if (this.containsItem(id + INSERT_IMAGE_ID)) {
            this.toolbar.enableItems(
              document.getElementById(id + INSERT_IMAGE_ID).parentElement,
              false
            );
          }
          if (this.containsItem(id + COMMENT_ID)) {
            this.toolbar.enableItems(
              document.getElementById(id + COMMENT_ID).parentElement,
              false
            );
          }
          if (this.containsItem(id + BOOKMARK_ID)) {
            this.toolbar.enableItems(
              document.getElementById(id + BOOKMARK_ID).parentElement,
              false
            );
          }
          if (this.containsItem(id + INSERT_LINK_ID)) {
            this.toolbar.enableItems(
              document.getElementById(id + INSERT_LINK_ID).parentElement,
              false
            );
          }
          if (this.containsItem(id + FORM_FIELDS_ID)) {
            this.toolbar.enableItems(
              document.getElementById(id + FORM_FIELDS_ID).parentElement,
              false
            );
          }
          if (this.containsItem(id + CONTENT_CONTROL_ID)) {
            this.toolbar.enableItems(
              document.getElementById(id + CONTENT_CONTROL_ID).parentElement,
              false
            );
          }
        }
      ...
      if (!isProtectedContent || this.container.showPropertiesPane) {
        ...
        classList(
          this.propertiesPaneButton.element.parentElement,
          !enable ? ['e-de-overlay'] : [],
          !enable ? [] : ['e-de-overlay']
        );
      }
      const protectionType: ProtectionType =
        this.documentEditor.documentHelper.protectionType;
      if (
        enable ||
        (this.documentEditor.documentHelper.isDocumentProtected &&
      ...
      if (
        ...
        this.containsItem(id + TRACK_ID)
      ) {
        this.toolbar.enableItems(
          document.getElementById(id + TRACK_ID).parentElement,
          false
        );
      }
    }
  ...
  public enableDisableUndoRedo(): void {
    ...
    if (this.toolbarItems.indexOf('Undo') >= 0) {
      ...
      if (!isNullOrUndefined(undoElement)) {
        this.toolbar.enableItems(
          undoElement.parentElement,
          this.container.documentEditor.editorHistoryModule.canUndo()
        );
      }
    }
    if (this.toolbarItems.indexOf('Redo') >= 0) {
      ...
      if (!isNullOrUndefined(redoElement)) {
        this.toolbar.enableItems(
          redoElement.parentElement,
          this.container.documentEditor.editorHistoryModule.canRedo()
        );
      }
    }
  }
  private onToc(): void {
    if (
      ...
      this.container.propertiesPaneContainer.style.display === 'none'
    ) {
      this.container.showPropertiesPane = false;
      this.documentEditor.focusIn();
      return;
  ...
  public enableDisablePropertyPaneButton(isShow: boolean): void {
    if (isShow) {
      classList(
        this.propertiesPaneButton.element.firstChild as HTMLElement,
        ['e-pane-enabled'],
        ['e-pane-disabled']
      );
    } else {
  ...
  public destroy(): void {
    ...
    if (this.container.toolbarContainer) {
      this.container.containerTarget.removeChild(
