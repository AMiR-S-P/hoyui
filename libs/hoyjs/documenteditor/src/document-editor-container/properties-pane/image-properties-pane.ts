...
export class ImageProperties {
  ...
  private onAspectRatioBtnClickHook: EventListenerOrEventListenerObject =
    this.onAspectRatioBtnClick.bind(this);
  private widthBlurHook: EventListenerOrEventListenerObject =
    this.widthBlur.bind(this);
  private heightBlurHook: EventListenerOrEventListenerObject =
    this.heightBlur.bind(this);
  private onImageWidthHook: EventListenerOrEventListenerObject =
    this.onImageWidth.bind(this);
  private onImageHeightHook: EventListenerOrEventListenerObject =
    this.onImageHeight.bind(this);
  private widthNumericBlurHook: EventListenerOrEventListenerObject =
    this.widthNumericBlur.bind(this);
  private heightNumericBlurHook: EventListenerOrEventListenerObject =
    this.heightNumericBlur.bind(this);
  private altTextAreaBlurHook: EventListenerOrEventListenerObject =
    this.altTextAreaBlur.bind(this);
  ...
  private initializeImageProperties(): void {
    this.element = createElement('div', {
      id: this.elementId + '_imageProperties',
  ...
  private initImageProp(): void {
    const localObj: L10n = new L10n(
      'documenteditorcontainer',
      this.container.defaultLocale,
      this.container.locale
    );
    this.imageDiv = createElement('div', {
      id: this.elementId + '_imageDiv',
      className: 'e-de-cntr-pane-padding e-de-prop-separator-line',
    });
    ...
    this.widthElement = this.createImagePropertiesDiv(
      '_widthDiv',
      this.outerDiv,
      '_widthInput',
      localObj.getConstant('W'),
      localObj.getConstant('Width')
    );
    this.widthNumericBox = new NumericTextBox({
      min: 0,
      max: 23500,
      cssClass: 'e-de-image-property',
      showSpinButton: false,
      format: 'n0',
      decimals: 2,
    });
    this.widthNumericBox.appendTo(this.widthElement);
    this.heightElement = this.createImagePropertiesDiv(
      '_heightDiv',
      this.outerDiv,
      '_heightInput',
      localObj.getConstant('H'),
      localObj.getConstant('Height')
    );
    ...
    this.aspectRatioDiv = createElement('div', {
      id: this.elementId + '_aspectRatioDiv',
    });
    this.aspectRatioDiv.setAttribute(
      'title',
      localObj.getConstant('Aspect ratio')
    );
    this.outerDiv.appendChild(this.aspectRatioDiv);
    this.aspectRatio = createElement('input', {
      id: this.elementId + '_aspectRatio',
      className: 'e-de-ctnr-prop-label',
    }) as HTMLInputElement;
    this.aspectRatioDiv.appendChild(this.aspectRatio);
    this.aspectRatioBtn = new CheckBox(
      { label: localObj.getConstant('Aspect ratio'), enableRtl: this.isRtl },
      this.aspectRatio
    );
  }
  private initImageAltProp(): void {
    ...
    this.altDiv = createElement('div', {
      id: this.elementId + '_altDiv',
      className: 'e-de-cntr-pane-padding e-de-prop-separator-line',
    });
  ...
  public wireEvents(): void {
    this.aspectRatioBtn.element.addEventListener(
      'change',
      this.onAspectRatioBtnClickHook
    );
    this.widthNumericBox.element.addEventListener('click', this.widthBlurHook);
    this.heightNumericBox.element.addEventListener(
      'click',
      this.heightBlurHook
    );
    this.widthNumericBox.element.addEventListener(
      'keydown',
      this.onImageWidthHook
    );
    this.heightNumericBox.element.addEventListener(
      'keydown',
      this.onImageHeightHook
    );
    this.widthNumericBox.element.addEventListener(
      'blur',
      this.widthNumericBlurHook
    );
    this.heightNumericBox.element.addEventListener(
      'blur',
      this.heightNumericBlurHook
    );
    this.textArea.addEventListener('blur', this.altTextAreaBlurHook);
  }
  private altTextAreaBlur(): void {
    if (
      this.documentEditor.selectionModule.imageFormat.alternateText !==
  ...
  private applyImageAlternativeText(): void {
    const altText: string = SanitizeHtmlHelper.sanitize(
      (this.textArea as HTMLInputElement).value
    );
    if (!isNullOrUndefined(altText)) {
      this.documentEditor.selectionModule.imageFormat.applyImageAlternativeText(
        altText
      );
    }
  }
  ...
  private applyImageWidth(): void {
    if (!this.isMaintainAspectRatio) {
      ...
      const ratio: number =
        width / this.documentEditor.selectionModule.imageFormat.width;
  ...
  private applyImageHeight(): void {
    if (!this.isMaintainAspectRatio) {
      ...
      const height: number = this.heightNumericBox.value;
      const ratio: number =
        height / this.documentEditor.selectionModule.imageFormat.height;
  ...
  public updateImageProperties(): void {
    this.widthNumericBox.value =
      this.documentEditor.selectionModule.imageFormat.width;
    this.heightNumericBox.value =
      this.documentEditor.selectionModule.imageFormat.height;
    if (
      isNullOrUndefined(
        this.documentEditor.selectionModule.imageFormat.alternateText
      )
  ...
  private unWireEvents(): void {
    this.aspectRatioBtn.element.removeEventListener(
      'change',
      this.onAspectRatioBtnClickHook
    );
    this.widthNumericBox.element.removeEventListener(
      'click',
      this.widthBlurHook
    );
