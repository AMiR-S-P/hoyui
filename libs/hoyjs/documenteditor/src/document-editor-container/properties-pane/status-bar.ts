...
export class StatusBar {
  ...
  //Event Handler
  private onPageLayoutClickHandler: EventListenerOrEventListenerObject =
    this.onPageLayoutClick.bind(this);
  private onWebLayoutClickHandler: EventListenerOrEventListenerObject =
    this.onWebLayoutClick.bind(this);
  private onPageNumberKeyDownHandler: EventListenerOrEventListenerObject =
    this.onPageNumberKeyDown.bind(this);
  private onPageNumberKeyUpHandler: EventListenerOrEventListenerObject =
    this.onPageNumberKeyUp.bind(this);
  private onPageNumberBlurHandler: EventListenerOrEventListenerObject =
    this.onPageNumberBlur.bind(this);
  private onPageNumberFocusHandler: EventListenerOrEventListenerObject =
    this.onPageNumberFocus.bind(this);
    ...
    if (e.which === 13) {
      ...
      if (pageNumber > this.editorPageCount) {
        ...
        if (this.documentEditor.selectionModule) {
          this.documentEditor.selectionModule.goToPage(
            parseInt(this.pageNumberInput.value, 10)
          );
          this.documentEditor.focusIn();
        } else {
  ...
  private onPageNumberBlur(): void {
    if (
      ...
      parseInt(this.pageNumberInput.value, 10) > this.editorPageCount
    ) {
      this.updatePageNumber();
    }
    this.pageNumberInput.contentEditable = 'false';
  ...
  public constructor(
    ...
    docEditor: DocumentEditorContainer
  ) {
  ...
  private initializeStatusBar(): void {
    ...
    this.documentEditor.enableSpellCheck = this.container.enableSpellCheck
      ? true
      : false;
    ...
    this.pageNumDiv = createElement('div', {
      className: this.container.enableSpellCheck
        ? 'e-de-ctnr-pg-no'
    ...
    this.pageNumberInput = createElement('input', {
      styles:
        'text-transform:capitalize;white-space:pre;overflow:hidden;user-select:none;cursor:text',
      attrs: {
        type: 'text',
        'aria-label': this.localObj.getConstant('Current Page Number'),
      },
      className: 'e-de-pagenumber-input',
    }) as HTMLInputElement;
    this.editablePageNumber = createElement('div', {
      styles: 'display: inline-flex',
      className: 'e-input e-de-pagenumber-text',
    });
    ...
    this.editablePageNumber.setAttribute(
      'title',
      this.localObj.getConstant('Current Page Number')
    );
    ...
    if (this.documentEditor.enableSpellCheck) {
      this.verticalLine = createElement('div', {
        className: 'e-de-statusbar-separator',
      });
    ...
    this.pageButton = this.createButtonTemplate(
      this.container.enableSpellCheck
        ? 'e-de-statusbar-pageweb e-btn-pageweb-spellcheck'
        : 'e-de-statusbar-pageweb',
      'e-de-printlayout e-icons',
      'Print layout',
      this.statusBarDiv,
      this.pageButton,
      this.documentEditor.layoutType === 'Pages' ? true : false
    );
    this.webButton = this.createButtonTemplate(
      'e-de-statusbar-pageweb',
      'e-de-weblayout e-icons',
      'Web layout',
      this.statusBarDiv,
      this.webButton,
      this.documentEditor.layoutType === 'Continuous' ? true : false
    );
    ...
    this.zoomBtn.setAttribute(
      'title',
      this.localObj.getConstant('ZoomLevelTooltip')
    );
  ...
  private addSpellCheckElement(): HTMLButtonElement {
    ...
    this.spellCheckButton = new DropDownButton({
      content: this.localObj.getConstant('Spelling'),
      items: spellCheckItems,
      enableRtl: this.container.enableRtl,
      select: this.onSpellCheck.bind(this),
      beforeItemRender: (args: MenuEventArgs) => {
        args.element.innerHTML = '<span></span>' + args.item.text;
        if (isNullOrUndefined(this.currentLanguage)) {
          this.currentLanguage =
            this.documentEditor.spellCheckerModule.languageID;
        }
        if (isNullOrUndefined(this.allowSuggestion)) {
          this.allowSuggestion =
            this.documentEditor.spellCheckerModule.allowSpellCheckAndSuggestion;
        }
        const span: HTMLElement = args.element.children[0] as HTMLElement;
        if (
          args.item.text === this.localObj.getConstant('Spell Check') &&
          this.documentEditor.enableSpellCheck &&
          this.documentEditor.spellCheckerModule.enableSpellCheck
        ) {
          span.style.marginRight = '10px';
          span.setAttribute('class', 'e-de-selected-spellcheck-item');
        } else if (
          args.item.text === this.localObj.getConstant('Underline errors') &&
          this.documentEditor.enableSpellCheck &&
          this.documentEditor.spellCheckerModule.enableSpellCheck &&
          ...
          (args.element.children[0] as HTMLElement).classList.remove(
            'e-de-selected-spellcheck-item'
          );
          (args.element.children[0] as HTMLElement).classList.remove(
            'e-de-selected-underline-item'
          );
        }
      },
    });
  ...
  private setSpellCheckValue(text: string): void {
    ...
    if (text.match(this.localObj.getConstant('Spell Check'))) {
      this.documentEditor.spellCheckerModule.enableSpellCheck = this
        .documentEditor.spellCheckerModule.enableSpellCheck
        ? false
        : true;
      ...
      if (
        ...
        this.documentEditor.spellCheckerModule.removeUnderline = this
          .documentEditor.spellCheckerModule.removeUnderline
          ? false
          : true;
      }
    }
  ...
  private wireEvents(): void {
    this.pageNumberInput.addEventListener(
      'keydown',
      this.onPageNumberKeyDownHandler
    );
    this.pageNumberInput.addEventListener(
      'keyup',
      this.onPageNumberKeyUpHandler
    );
    this.pageNumberInput.addEventListener('blur', this.onPageNumberBlurHandler);
    this.pageNumberInput.addEventListener(
      'focus',
      this.onPageNumberFocusHandler
    );
  }
  private unWireEvents(): void {
    ...
    this.pageNumberInput.removeEventListener(
      'keydown',
      this.onPageNumberKeyDownHandler
    );
    this.pageNumberInput.removeEventListener(
  ...
  private updatePageNumberWidth(): void {
    if (this.pageNumberInput) {
      this.pageNumberInput.style.width =
        this.pageNumberInput.value.length >= 3 ? '30px' : '22px';
    }
  }
  /**
   * @private
  ...
  private createButtonTemplate(
    ...
    toolTipText: string,
    div: HTMLElement,
    appendDiv: HTMLButtonElement,
    toggle: boolean
  ): HTMLButtonElement {
    appendDiv = createElement('Button', {
      className: className,
      attrs: { type: 'button' },
    }) as HTMLButtonElement;
