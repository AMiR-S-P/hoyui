...
import { createElement, L10n, classList } from '@hoyui/base';
import { Button, CheckBox } from '@hoyui/buttons';
...
export class TocProperties {
  ...
  private onCloseClickHook: EventListenerOrEventListenerObject =
    this.onCloseClick.bind(this);
  private onInsertToClickHook: EventListenerOrEventListenerObject =
    this.onInsertToc.bind(this);
  ...
  private initializeTocPane(): void {
    ...
    this.element = createElement('div', {
      id: this.elementId + '_tocProperties',
  ...
  private onClose(): void {
    if (
      this.container.showPropertiesPane &&
  ...
  private tocHeaderDiv(container: HTMLElement): void {
    ...
    this.closeButton = createElement('span', {
      ...
      styles:
        'cursor: pointer;display:inline-block;color: #4A4A4A;' +
        closeButtonFloat +
        closeButtonMargin,
    });
  ...
  private template1(container: HTMLElement): void {
    ...
    this.templateContent1.textContent =
      this.localObj.getConstant('HEADING - - - - 1');
    ...
    this.templateContent2.textContent =
      this.localObj.getConstant('HEADING - - - - 2');
    ...
    this.templateContent3.textContent =
      this.localObj.getConstant('HEADING - - - - 3');
  ...
  public createDropDownButton(
    ...
    const buttonElement: HTMLButtonElement = createElement('input', {
      id: id,
    }) as HTMLButtonElement;
    parentDiv.appendChild(buttonElement);
    const dropDownBtn: DropDownList = new DropDownList(
      {
        index: selectedIndex,
        dataSource: content,
        popupHeight: '150px',
        cssClass: 'e-de-prop-font-button',
        placeholder: this.localObj.getConstant('Levels'),
      },
      buttonElement
    );
    return dropDownBtn;
  ...
  private contentStylesDropdown(container: HTMLElement): void {
    ...
    this.contentStyleElement.setAttribute(
      'title',
    ...
    this.dropDownLabel = createElement('label', {
  ...
  private checkboxContent(container: HTMLElement): void {
    ...
    this.checkboxElement = createElement('div', {
      id: 'toc_checkboxDiv',
      styles: 'margin-bottom:36px;',
    });
    container.appendChild(this.checkboxElement);
    this.showPageNumberDiv = createElement('div', {
      className: 'e-de-toc-checkbox1',
    });
    this.showPageNumberDiv.setAttribute(
      'title',
      this.localObj.getConstant('Show page numbers in table of contents')
    );
    this.checkboxElement.appendChild(this.showPageNumberDiv);
    this.showpagenumberCheckboxElement = createElement('input', {
      id: 'showpagenumber',
      styles: 'width:12px;height:12px;margin-bottom:8px',
      className: 'e-de-prop-sub-label',
    });
    ...
    this.rightAlignDiv = createElement('div', {
      className: 'e-de-toc-checkbox2',
    });
    this.rightAlignDiv.setAttribute(
      'title',
      this.localObj.getConstant('Right align page numbers in table of contents')
    );
    this.checkboxElement.appendChild(this.rightAlignDiv);
    this.rightalignpagenumberCheckboxElement = createElement('input', {
      id: 'rightalignpagenumber',
    ...
    this.rightalignPageNumber.appendTo(
      this.rightalignpagenumberCheckboxElement
    );
    this.hyperlinkDiv = createElement('div', {
      className: 'e-de-toc-checkbox3',
    });
    this.hyperlinkDiv.setAttribute(
      'title',
      this.localObj.getConstant('Use hyperlinks instead of page numbers')
    );
    this.checkboxElement.appendChild(this.hyperlinkDiv);
    this.hyperlinkCheckboxElement = createElement('input', {
      id: 'hyperlinkdiv',
      styles: 'width:12px;height:12px',
      className: 'e-de-prop-sub-label',
    });
  ...
  private buttonDiv(container: HTMLElement): void {
    ...
    this.updateBtn = new Button({
      content: this.localObj.getConstant('Update'),
      cssClass: 'btn-update',
      isPrimary: true,
    });
    ...
    this.cancelBtn = new Button({
      ...
      cssClass: this.isRtl ? 'e-de-btn-cancel-rtl' : 'e-de-btn-cancel',
    });
    this.cancelbuttoncontentStyleElement.setAttribute(
      'aria-label',
      this.cancelBtn.content
    );
  ...
  public showTocPane(isShow: boolean, previousContextType?: ContextType): void {
    ...
    this.updateBtn.content =
      this.documentEditor.selectionModule.contextType === 'TableOfContents'
        ? this.localObj.getConstant('Update')
        : this.localObj.getConstant('Insert');
    ...
    this.enableDisableInsertButton(
  ...
  private unWireEvents(): void {
    ...
    this.updateBtn.element.removeEventListener(
      'click',
      this.onInsertToClickHook
    );
