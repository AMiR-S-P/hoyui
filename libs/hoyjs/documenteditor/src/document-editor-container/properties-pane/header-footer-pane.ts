...
import { createElement, KeyboardEventArgs, L10n, classList } from '@hoyui/base';
...
export class HeaderFooterProperties {
  ...
  private HeaderTopApplyClickHook: EventListenerOrEventListenerObject =
    this.headerTopApply.bind(this);
  private FooterTopApplyClickHook: EventListenerOrEventListenerObject =
    this.footerTopapply.bind(this);
  private OnHeaderValueKeyDownHook: EventListenerOrEventListenerObject =
    this.onHeaderValue.bind(this);
  private OnFooterValueKeyDownHook: EventListenerOrEventListenerObject =
    this.onFooterValue.bind(this);
  private ChangeHeaderBlurHook: EventListenerOrEventListenerObject =
    this.changeHeaderBlur.bind(this);
  private ChangeFooterBlurHook: EventListenerOrEventListenerObject =
    this.changeFooterBlur.bind(this);
  ...
  public showHeaderFooterPane(isShow: boolean): void {
    ...
    if (
      (!isShow && this.element.style.display === 'none') ||
      (isShow && this.element.style.display === 'block')
    ) {
      return;
    }
  ...
  private initializeHeaderFooter(): void {
    ...
    this.element = createElement('div', {
      id: this.documentEditor.element.id + this.elementId,
      className: 'e-de-prop-pane',
    });
    this.headerDiv = this.createDivTemplate(
      '_header_footer',
      this.element,
      'padding-bottom:0'
    );
    classList(this.headerDiv, ['e-de-cntr-pane-padding'], []);
    this.headerLabel = createElement('label', {
    ...
    this.optionsLabelDiv = this.createDivTemplate(
      this.elementId + '_options',
      this.element
    );
    classList(
    ...
    this.optionsLabel = createElement('label', {
    ...
    this.optionsDiv = this.createDivTemplate(
      this.elementId + '_optionsDiv',
      this.optionsLabelDiv
    );
    this.firstPageDiv = this.createDivTemplate(
      this.elementId + '_firstPageDiv',
      this.optionsDiv
    );
    classList(this.firstPageDiv, ['e-de-hdr-ftr-frst-div'], []);
    const firstPage: HTMLInputElement = createElement('input', {
      id: 'firstPage',
      className: 'e-de-prop-sub-label',
    }) as HTMLInputElement;
    this.firstPageDiv.appendChild(firstPage);
    this.firstPage = new CheckBox({
      label: this.localObj.getConstant('Different First Page'),
      change: this.changeFirstPageOptions.bind(this),
      cssClass: 'e-de-prop-sub-label',
      enableRtl: this.isRtl,
    });
    this.firstPage.appendTo(firstPage);
    this.firstPageDiv.children[0].setAttribute(
      'title',
      this.localObj.getConstant('Different header and footer for first page')
    );
    this.oddOrEvenDiv = this.createDivTemplate(
      this.elementId + '_oddOrEvenDiv',
      this.optionsDiv
    );
    classList(this.oddOrEvenDiv, ['e-de-hdr-ftr-frst-div'], []);
    const oddOrEven: HTMLInputElement = createElement('input', {
      id: 'oddOrEven',
      className: 'e-de-sub-prop-label',
    }) as HTMLInputElement;
    this.oddOrEvenDiv.appendChild(oddOrEven);
    this.oddOrEven = new CheckBox({
      label: this.localObj.getConstant('Different Odd And Even Pages'),
      change: this.changeoddOrEvenOptions.bind(this),
      cssClass: 'e-de-prop-sub-label',
      enableRtl: this.isRtl,
    });
    this.oddOrEven.appendTo(oddOrEven);
    this.oddOrEvenDiv.children[0].setAttribute(
      'title',
      this.localObj.getConstant(
        'Different header and footer for odd and even pages'
      )
    );
    this.linkToPreviousDiv = this.createDivTemplate(
      this.elementId + '_linkToPreviousDiv',
      this.optionsDiv
    );
    const linkToPrevious: HTMLInputElement = createElement('input', {
      id: 'linkToPrevious',
      className: 'e-de-sub-prop-label',
    }) as HTMLInputElement;
    this.linkToPreviousDiv.appendChild(linkToPrevious);
    this.linkToPrevious = new CheckBox({
      label: this.localObj.getConstant('Link to Previous'),
      change: this.changeLinkToPreviousOptions.bind(this),
      cssClass: 'e-de-prop-sub-label',
      enableRtl: this.isRtl,
      checked: true,
    });
    this.linkToPrevious.appendTo(linkToPrevious);
    this.linkToPreviousDiv.children[0].setAttribute(
      'title',
      this.localObj.getConstant('Link to the previous Title')
    );
    ...
    this.positionLabelDiv = this.createDivTemplate(
      this.elementId + '_positionLabelDiv',
      this.element
    );
    classList(
      this.positionLabelDiv,
      ['e-de-cntr-pane-padding', 'e-de-prop-separator-line'],
      []
    );
    this.positionLabel = createElement('label', {
      className: 'e-de-ctnr-prop-label',
      styles: 'height:20px;',
    });
    this.positionLabel.innerHTML = this.localObj.getConstant('Position');
    this.positionLabelDiv.appendChild(this.positionLabel);
    this.positionDiv = this.createDivTemplate(
      this.elementId + '_positionDiv',
      this.positionLabelDiv
    );
    //let width: string;
    ...
    //}
    this.headerTopDiv = this.createDivTemplate(
      this.elementId + '_headerTopDiv',
      this.positionDiv
    );
    classList(this.headerTopDiv, ['e-de-hdr-ftr-top-div'], []);
    this.headerTopLabel = createElement('label', {
      className: 'e-de-prop-sub-label',
      styles: 'display:block',
    });
    this.headerTopLabel.innerHTML =
      this.localObj.getConstant('Header from Top');
    this.headerTopDiv.appendChild(this.headerTopLabel);
    const headerFromTop: HTMLInputElement = createElement('input', {
      id: this.documentEditor.element.id + '_headerFromTop',
      className: 'e-de-prop-sub-label',
    }) as HTMLInputElement;
    headerFromTop.setAttribute(
      'aria-label',
      this.localObj.getConstant('Header from Top')
    );
    ...
    this.headerFromTop = new NumericTextBox({
      value: 36,
      cssClass: 'e-de-prop-header-numeric',
    ...
    this.headerFromTop.element.parentElement.setAttribute(
      'title',
      this.localObj.getConstant(
        'Distance from top of the page to top of the header'
      )
    );
    this.footerBottomDiv = this.createDivTemplate(
      this.elementId + '_footerBottomDiv',
      this.positionDiv
    );
    this.footerBottomLabel = createElement('label', {
      className: 'e-de-prop-sub-label',
      styles: 'display:block',
    });
    this.footerBottomLabel.innerHTML =
      this.localObj.getConstant('Footer from Bottom');
    this.footerBottomDiv.appendChild(this.footerBottomLabel);
    const footerFromTop: HTMLInputElement = createElement('input', {
      id: this.documentEditor.element.id + '_footerFromTop',
      className: 'e-de-prop-sub-label',
    }) as HTMLInputElement;
    footerFromTop.setAttribute(
      'aria-label',
      this.localObj.getConstant('Footer from Bottom')
    );
    ...
    this.footerFromTop.element.parentElement.setAttribute(
      'title',
      this.localObj.getConstant(
  ...
  private wireEvents(): void {
    this.headerFromTop.element.addEventListener(
      'click',
      this.HeaderTopApplyClickHook
    );
    this.footerFromTop.element.addEventListener(
      'click',
      this.FooterTopApplyClickHook
    );
    this.headerFromTop.element.addEventListener(
      'keydown',
      this.OnHeaderValueKeyDownHook
    );
    this.footerFromTop.element.addEventListener(
      'keydown',
      this.OnFooterValueKeyDownHook
    );
    this.headerFromTop.element.addEventListener(
      'blur',
      this.ChangeHeaderBlurHook
    );
    this.footerFromTop.element.addEventListener(
      'blur',
      this.ChangeFooterBlurHook
    );
  }
  ...
  private changeFirstPageOptions(): void {
    if (!this.documentEditor.isReadOnly) {
      this.documentEditor.selectionModule.sectionFormat.differentFirstPage =
        this.firstPage.checked;
      setTimeout((): void => {
  ...
  private changeoddOrEvenOptions(): void {
    if (!this.documentEditor.isReadOnly) {
      this.documentEditor.selectionModule.sectionFormat.differentOddAndEvenPages =
        this.oddOrEven.checked;
      setTimeout((): void => {
        this.documentEditor.focusIn();
      }, 10);
    }
    ...
    if (!this.documentEditor.isReadOnly) {
      const headerFooterType: HeaderFooterType = (
        this.documentEditor.selectionModule.start.paragraph
          .containerWidget as HeaderFooterWidget
      ).headerFooterType;
      const value: boolean = this.linkToPrevious.checked;
      switch (headerFooterType) {
        case 'OddHeader':
          this.documentEditor.selectionModule.sectionFormat.oddPageHeader.linkToPrevious =
            value;
          break;
        case 'OddFooter':
          this.documentEditor.selectionModule.sectionFormat.oddPageFooter.linkToPrevious =
            value;
          break;
        case 'EvenHeader':
          this.documentEditor.selectionModule.sectionFormat.evenPageHeader.linkToPrevious =
            value;
          break;
        case 'EvenFooter':
          this.documentEditor.selectionModule.sectionFormat.evenPageFooter.linkToPrevious =
            value;
          break;
        case 'FirstPageHeader':
          this.documentEditor.selectionModule.sectionFormat.firstPageHeader.linkToPrevious =
            value;
          break;
        case 'FirstPageFooter':
          this.documentEditor.selectionModule.sectionFormat.firstPageFooter.linkToPrevious =
            value;
          break;
      }
      setTimeout((): void => {
  ...
  private changeHeaderValue(): void {
    ...
    if (!this.documentEditor.isReadOnly) {
      ...
      this.documentEditor.selectionModule.sectionFormat.headerDistance =
        headerTop;
    }
  }
  ...
  private changeFooterValue(): void {
    ...
    if (!this.documentEditor.isReadOnly) {
      ...
      this.documentEditor.selectionModule.sectionFormat.footerDistance =
        footerTop;
    }
  }
  public onSelectionChange(): void {
    this.headerFromTop.value =
      this.documentEditor.selectionModule.sectionFormat.headerDistance;
    this.footerFromTop.value =
      this.documentEditor.selectionModule.sectionFormat.footerDistance;
    ...
    if (
      this.documentEditor.selectionModule.start.paragraph.bodyWidget
        .sectionIndex === 0
    ) {
  ...
  private unWireEvents(): void {
    this.headerFromTop.element.removeEventListener(
      'click',
      this.HeaderTopApplyClickHook
    );
