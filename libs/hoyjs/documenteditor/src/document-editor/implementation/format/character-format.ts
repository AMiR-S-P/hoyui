...
export class WCharacterFormat {
            ...
            for (let i: number = 0; i < length; i++) {
              ...
              if (element instanceof TextElementBox) {
                ...
                if (text.startsWith('HYPERLINK')) {
                  ...
                  if (index !== -1) {
                    this.baseCharStyle = (
                      this.ownerBase as TextElementBox
                    ).paragraph.paragraphFormat.baseStyle;
                  }
                }
              }
            }
  ...
  private documentCharacterFormat(): WCharacterFormat {
    ...
    if (!isNullOrUndefined(this.ownerBase)) {
      if (
        ...
        !isNullOrUndefined(
          (this.ownerBase as TextElementBox).paragraph.bodyWidget.page
        )
      ) {
  ...
  private checkCharacterStyle(property: string): Object {
    ...
    if (!isNullOrUndefined(baseStyle)) {
      while (
        ...
        if (hasKey) {
          const returnPropertyType: number = WUniqueFormat.getPropertyType(
            WCharacterFormat.uniqueFormatType,
            property
          );
          return baseStyle.characterFormat.uniqueCharacterFormat.propertiesHash.get(
  ...
  private setPropertyValue(property: string, value: Object): void {
    ...
    if (
      ...
      this.uniqueCharacterFormat =
        WCharacterFormat.uniqueCharacterFormats.updateUniqueFormat(
          this.uniqueCharacterFormat,
          property,
          value
        );
  ...
  private initializeUniqueCharacterFormat(
    ...
    const uniqueCharFormatTemp: Dictionary<number, object> = new Dictionary<
      number,
      object
    >();
    ...
    this.uniqueCharacterFormat =
      WCharacterFormat.uniqueCharacterFormats.addUniqueFormat(
        uniqueCharFormatTemp,
        WCharacterFormat.uniqueFormatType
      );
  }
  ...
  public mergeFormat(format: WCharacterFormat): void {
    ...
    if (isNullOrUndefined(this.getValue('CapitalizeEachWord'))) {
      this.CapitalizeEachWord = format.getValue(
        'CapitalizeEachWord'
      ) as boolean;
    }
    ...
    if (isNullOrUndefined(this.getValue('fontFamilyNonFarEast'))) {
      this.fontFamilyNonFarEast = format.getValue(
        'fontFamilyNonFarEast'
      ) as string;
    }
  }
  ...
  public hasValueWithParent(property: string): boolean {
    ...
    if (
      !hasValue &&
      !isNullOrUndefined(this.baseCharStyle) &&
      this.baseCharStyle instanceof WCharacterStyle
    ) {
      hasValue = (
        this.baseCharStyle as WCharacterStyle
      ).characterFormat.hasValue(property);
    }
