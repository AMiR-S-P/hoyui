...
export class HtmlExport {
  ...
  public writeHtml(document: any, isOptimizeSfdt: boolean): string {
    ...
    for (
      let i: number = 0;
      i < document[sectionsProperty[this.keywordIndex]].length;
      i++
    ) {
      html += this.serializeSection(
        document[sectionsProperty[this.keywordIndex]][i]
      );
    }
    return html;
  }
  ...
  private serializeSection(section: any): string {
    ...
    for (
      let i: number = 0;
      i < section[blocksProperty[this.keywordIndex]].length;
      i++
    ) {
    ...
    if (
      listCloseCount.length > 0 &&
      (isNullOrUndefined(listLevel) ||
        isNullOrUndefined(this.prevListLevel) ||
        (this.prevListLevel[restartLevelProperty[this.keywordIndex]] ===
          listLevel[restartLevelProperty[this.keywordIndex]] &&
          this.prevListLevel[paragraphFormatProperty[this.keywordIndex]][
  ...
  private serializeParagraph(paragraph: any, listCloseCount?: any[]): string {
    ...
    if (
      !isNullOrUndefined(
        paragraph[paragraphFormatProperty[this.keywordIndex]][
          listFormatProperty[this.keywordIndex]
        ]
      )
    ) {
      ...
      if (
        !isNullOrUndefined(listCloseCount) &&
        !isNullOrUndefined(listLevel) &&
        !isNullOrUndefined(this.prevListLevel) &&
        ((this.prevListLevel[restartLevelProperty[this.keywordIndex]] <
          listLevel[restartLevelProperty[this.keywordIndex]] &&
    ...
    if (isList && !isPreviousList) {
      blockStyle += this.getHtmlList(
        listLevel,
        paragraph[paragraphFormatProperty[this.keywordIndex]][
          listFormatProperty[this.keywordIndex]
        ][listLevelNumberProperty[this.keywordIndex]]
      );
    }
    tagAttributes.push(
      'style="' +
        this.serializeParagraphStyle(paragraph, '', isList) +
        ';' +
        'white-space:pre' +
        '"'
    );
    if (isList) {
      ...
      this.prevListLevel = undefined;
      this.isOrdered = undefined;
      blockStyle += this.createAttributesTag(
        this.getStyleName(
          paragraph[paragraphFormatProperty[this.keywordIndex]][
            styleNameProperty[this.keywordIndex]
          ]
        ),
        tagAttributes
      );
  ...
  private getListLevel(paragraph: any): any {
    ...
    for (
      let i: number = 0;
      i < this.document[listsProperty[this.keywordIndex]].length;
      i++
    ) {
      if (
        this.document[listsProperty[this.keywordIndex]][i][
          listIdProperty[this.keywordIndex]
        ] ===
        paragraph[paragraphFormatProperty[this.keywordIndex]][
          listFormatProperty[this.keywordIndex]
        ][listIdProperty[this.keywordIndex]]
    ...
    if (list) {
      for (
        let j: number = 0;
        j < this.document[abstractListsProperty[this.keywordIndex]].length;
        j++
      ) {
        if (
          this.document[abstractListsProperty[this.keywordIndex]][j][
            abstractListIdProperty[this.keywordIndex]
          ] === list[abstractListIdProperty[this.keywordIndex]]
        ) {
          ...
          listLevel =
            this.document[abstractListsProperty[this.keywordIndex]][j][
              levelsProperty[this.keywordIndex]
            ][levelNumber];
          break;
        }
      }
  ...
  private getHtmlList(listLevel: any, levelNumer: number): string {
    ...
    if (
      ...
      switch (levelNumer) {
        case 0:
          ...
          listLevel[characterFormatProperty[this.keywordIndex]][
            fontFamilyProperty[this.keywordIndex]
          ] = 'Symbol';
          break;
        case 1:
          html += 'circle';
      ...
      html +=
        '" start="' +
        listLevel[startAtProperty[this.keywordIndex]].toString() +
        '">';
    }
    return html;
  }
  ...
  private serializeInlines(paragraph: any, blockStyle: string): string {
    ...
    while (paragraph[inlinesProperty[this.keywordIndex]].length > i) {
      ...
      if (
        inline.hasOwnProperty(textProperty[this.keywordIndex]) &&
        ...
        blockStyle +=
          this.createAttributesTag('span', tagAttributes) + this.endTag('span');
        tabCount = 0;
      }
      if (inline.hasOwnProperty(imageStringProperty[this.keywordIndex])) {
        ...
        if (inline[fieldTypeProperty[this.keywordIndex]] === 0) {
          let fieldCode: any =
            paragraph[inlinesProperty[this.keywordIndex]][i + 1];
          if (isNullOrUndefined(fieldCode[textProperty[this.keywordIndex]])) {
            fieldCode = paragraph[inlinesProperty[this.keywordIndex]][i + 2];
          }
          if (
            !isNullOrUndefined(fieldCode) &&
            !isNullOrUndefined(fieldCode[textProperty[this.keywordIndex]]) &&
            (fieldCode[textProperty[this.keywordIndex]].indexOf('TOC') >= 0 ||
              fieldCode[textProperty[this.keywordIndex]].indexOf('HYPERLINK') >=
                0)
          ) {
            this.fieldCheck = 1;
            let tagAttributes: string[] = [];
        ...
        const text: string = isNullOrUndefined(
          inline[textProperty[this.keywordIndex]]
        )
          ? ''
          : inline[textProperty[this.keywordIndex]];
        if (inline.hasOwnProperty(bookmarkTypeProperty[this.keywordIndex])) {
          switch (inline[bookmarkTypeProperty[this.keywordIndex]]) {
            case 0:
              blockStyle +=
                '<a name=' + inline[nameProperty[this.keywordIndex]] + '>';
              break;
            case 1:
              blockStyle += '</a>';
              break;
  ...
  private serializeSpan(
    ...
    tagAttributes.push(
      'style="' + this.serializeInlineStyle(characterFormat) + '"'
    );
  ...
  private serializeImageContainer(image: any): string {
    ...
    const tagAttributes: string[] = [];
    this.serializeInlineStyle(
      image[characterFormatProperty[this.keywordIndex]]
    );
    let imageSource: string = '';
    if (!isNullOrUndefined(image[imageStringProperty[this.keywordIndex]])) {
      let base64ImageString: string[] = this.images.get(
        parseInt(image[imageStringProperty[this.keywordIndex]])
      );
      imageSource =
        base64ImageString[
          HelperMethods.parseBoolValue(
            image[isMetaFileProperty[this.keywordIndex]]
          )
            ? 1
            : 0
        ];
    }
    const width: number = HelperMethods.convertPointToPixel(
      image[widthProperty[this.keywordIndex]]
    );
    const height: number = HelperMethods.convertPointToPixel(
      image[heightProperty[this.keywordIndex]]
    );
  ...
  public serializeCell(cell: any, row: any): string {
    ...
    if (!isNullOrUndefined(cell[cellFormatProperty[this.keywordIndex]])) {
      ...
      if (
        !isNullOrUndefined(
          cell[cellFormatProperty[this.keywordIndex]][
            shadingProperty[this.keywordIndex]
          ][backgroundColorProperty[this.keywordIndex]]
        ) &&
        cell[cellFormatProperty[this.keywordIndex]][
          shadingProperty[this.keywordIndex]
      ...
      if (
        !isNullOrUndefined(
          cell[cellFormatProperty[this.keywordIndex]][
            columnSpanProperty[this.keywordIndex]
          ]
        ) &&
        cell[cellFormatProperty[this.keywordIndex]][
          columnSpanProperty[this.keywordIndex]
      ...
      if (
        !isNullOrUndefined(
          cell[cellFormatProperty[this.keywordIndex]][
            rowSpanProperty[this.keywordIndex]
          ]
        ) &&
        cell[cellFormatProperty[this.keywordIndex]][
          rowSpanProperty[this.keywordIndex]
      ...
      if (
        !isNullOrUndefined(
          cell[cellFormatProperty[this.keywordIndex]][
            cellWidthProperty[this.keywordIndex]
          ]
        ) &&
        cell[cellFormatProperty[this.keywordIndex]][
          cellWidthProperty[this.keywordIndex]
        ...
        const cellWidth: number = HelperMethods.convertPointToPixel(
          cell[cellFormatProperty[this.keywordIndex]][
            cellWidthProperty[this.keywordIndex]
          ]
        );
      ...
      const cellAlignment: string = isNullOrUndefined(
        cell[cellFormatProperty[this.keywordIndex]][
      ...
      if (
        !isNullOrUndefined(
          cell[cellFormatProperty[this.keywordIndex]][
            bordersProperty[this.keywordIndex]
          ]
        )
      ) {
    ...
    for (
      let k: number = 0;
      k < cell[blocksProperty[this.keywordIndex]].length;
      k++
    ) {
  ...
  private convertVerticalAlignment(
    cellVerticalAlignment: number | CellVerticalAlignment
  ): string {
  ...
  private serializeTable(table: any): string {
    ...
    for (
      let j: number = 0;
      j < table[rowsProperty[this.keywordIndex]].length;
      j++
    ) {
  ...
  private serializeRow(row: any): string {
    ...
    for (
      let k: number = 0;
      k < row[cellsProperty[this.keywordIndex]].length;
      k++
    ) {
  ...
  private serializeParagraphStyle(
    ...
    if (paragraph[inlinesProperty[this.keywordIndex]].length > 0) {
      paragraphClass += this.serializeCharacterFormat(
    ...
    paragraphClass += this.serializeParagraphFormat(
      paragraph[paragraphFormatProperty[this.keywordIndex]],
      isList
    );
  ...
  private serializeTableBorderStyle(borders: any): string {
    ...
    if (
      !isNullOrUndefined(border) &&
      border[lineStyleProperty[this.keywordIndex]] !==
        (this.keywordIndex == 1 ? 1 : 'None') &&
      border[lineStyleProperty[this.keywordIndex]] !==
        (this.keywordIndex == 1 ? 26 : 'Cleared')
    ) {
      border[colorProperty[this.keywordIndex]] = isNullOrUndefined(
        border[colorProperty[this.keywordIndex]]
      )
        ? '#000000'
        : border[colorProperty[this.keywordIndex]];
      border[lineWidthProperty[this.keywordIndex]] = isNullOrUndefined(
        border[lineWidthProperty[this.keywordIndex]]
      )
        ? 0.5
        : border[lineWidthProperty[this.keywordIndex]];
      ...
      !isNullOrUndefined(border) &&
      HelperMethods.parseBoolValue(
        border[hasNoneStyleProperty[this.keywordIndex]]
      )
    ) {
  ...
  private serializeCellBordersStyle(borders: any, row: any): string {
    ...
    if (
      ...
      !isNullOrUndefined(
        row[rowFormatProperty[this.keywordIndex]][
          bordersProperty[this.keywordIndex]
        ][leftProperty[this.keywordIndex]]
      )
    ) {
      border =
        row[rowFormatProperty[this.keywordIndex]][
    ...
    if (
      ...
      !isNullOrUndefined(
        row[rowFormatProperty[this.keywordIndex]][
          bordersProperty[this.keywordIndex]
        ][rightProperty[this.keywordIndex]]
      )
    ) {
      border =
        row[rowFormatProperty[this.keywordIndex]][
    ...
    if (
      ...
      !isNullOrUndefined(
        row[rowFormatProperty[this.keywordIndex]][
          bordersProperty[this.keywordIndex]
        ][bottomProperty[this.keywordIndex]]
      )
    ) {
      border =
        row[rowFormatProperty[this.keywordIndex]][
  ...
  private serializeBorderStyle(border: any, borderPosition: string): string {
    ...
    borderStyle +=
      ...
      borderPosition +
      '-style:' +
      this.convertBorderLineStyle(border[lineStyleProperty[this.keywordIndex]]);
    borderStyle += ';';
    if (border[lineWidthProperty[this.keywordIndex]] > 0) {
      borderStyle +=
        ...
        borderPosition +
        '-width:' +
        border[lineWidthProperty[this.keywordIndex]].toString() +
        'pt;';
    }
    ...
    if (!isNullOrUndefined(border[colorProperty[this.keywordIndex]])) {
      borderStyle +=
        ...
        borderPosition +
        '-color:' +
        HelperMethods.getColor(border[colorProperty[this.keywordIndex]]) +
        ';';
    }
    return borderStyle;
  }
  ...
  private serializeCharacterFormat(
    ...
    if (
      !isNullOrUndefined(
        characterFormat[inlineFormatProperty[this.keywordIndex]]
      )
    ) {
      return this.serializeCharacterFormat(
        characterFormat[inlineFormatProperty[this.keywordIndex]],
        isEmptyLine
      );
    }
    ...
    charStyle += ':';
    charStyle += HelperMethods.parseBoolValue(
      characterFormat[boldProperty[this.keywordIndex]]
    )
      ? 'bold'
      : 'normal';
    charStyle += ';';
    ...
    if (
      HelperMethods.parseBoolValue(
        characterFormat[italicProperty[this.keywordIndex]]
      )
    ) {
      charStyle += 'italic';
    } else {
    ...
    if (
      characterFormat[baselineAlignmentProperty[this.keywordIndex]] ===
        (this.keywordIndex == 1 ? 1 : 'Superscript') ||
      characterFormat[baselineAlignmentProperty[this.keywordIndex]] ===
        (this.keywordIndex == 1 ? 2 : 'Subscript')
    ) {
    ...
    if (
      !isNullOrUndefined(
        characterFormat[highlightColorProperty[this.keywordIndex]]
      ) &&
      characterFormat[highlightColorProperty[this.keywordIndex]] !==
        (this.keywordIndex == 1 ? 0 : 'NoColor') &&
      !isEmptyLine
    ) {
      ...
      charStyle +=
        this.keywordIndex == 1
          ? this.getHighlightColorCode(
              characterFormat[highlightColorProperty[this.keywordIndex]]
            )
          : HelperMethods.getHighlightColorCode(
              characterFormat.highlightColor.toString()
            );
      charStyle += ';';
    }
    //Font Color
    ...
    if (
      ...
      HelperMethods.parseBoolValue(
        characterFormat[allCapsProperty[this.keywordIndex]]
      )
    ) {
    ...
    propertyValue =
      characterFormat[characterSpacingProperty[this.keywordIndex]];
    if (!isNullOrUndefined(propertyValue)) {
  ...
  private serializeTextDecoration(characterFormat: any): string {
    ...
    if (
      characterFormat[strikethroughProperty[this.keywordIndex]] ===
        (this.keywordIndex == 1 ? 1 : 'SingleStrike') ||
      characterFormat[strikethroughProperty[this.keywordIndex]] ===
        (this.keywordIndex == 1 ? 2 : 'DoubleStrike')
    ) {
      value += 'line-through ';
    }
    if (
      !isNullOrUndefined(
        characterFormat[underlineProperty[this.keywordIndex]]
      ) &&
      characterFormat[underlineProperty[this.keywordIndex]] !==
        (this.keywordIndex == 1 ? 0 : 'None')
    ) {
      value += 'underline';
    }
    if (value.length > 1) {
   ...
   * @private
   */
  public serializeParagraphFormat(
    paragraphFormat: any,
    isList: boolean,
    keywordIndex?: number
  ): string {
    ...
    if (
      !isNullOrUndefined(
        paragraphFormat[inlineFormatProperty[this.keywordIndex]]
      )
    ) {
      return this.serializeParagraphFormat(
        paragraphFormat[inlineFormatProperty[this.keywordIndex]],
        isList
      );
    }
    ...
    propertyValue = this.getTextAlignment(
      paragraphFormat[textAlignmentProperty[this.keywordIndex]]
    );
    if (!isNullOrUndefined(propertyValue)) {
    ...
    if (
      !isNullOrUndefined(paragraphFormat[tabsProperty[this.keywordIndex]]) &&
      paragraphFormat[tabsProperty[this.keywordIndex]].length > 0
    ) {
      paraStyle += this.serializeTabs(
        paragraphFormat[tabsProperty[this.keywordIndex]]
      );
    }
    propertyValue = paragraphFormat[lineSpacingProperty[this.keywordIndex]];
    if (!isNullOrUndefined(propertyValue)) {
      const lineSpacingType =
        paragraphFormat[lineSpacingTypeProperty[this.keywordIndex]];
      const isMultiple =
        lineSpacingType === (this.keywordIndex == 1 ? 0 : 'Multiple');
      const isAtLeast =
        lineSpacingType === (this.keywordIndex == 1 ? 0 : 'AtLeast');
      ...
      if (propertyValue == 1 && isMultiple) {
        ...
        paraStyle +=
          'line-height:' + (Math.abs(propertyValue) * 100).toString() + '%;';
  ...
  private serializeTabs(tabs: any): string {
    ...
    for (let i: number = 0; i < tabs.length; i++) {
      ...
      if (tab.hasOwnProperty(tabJustificationProperty[this.keywordIndex])) {
        let tabJustification: string = this.getTabJustification(
      ...
      if (tab.hasOwnProperty(tabLeaderProperty[this.keywordIndex])) {
        let tabLeader: string = this.getTabLeader(
  ...
  public createTableStartTag(table: any): string {
    ...
    if (!isNullOrUndefined(table[tableFormatProperty[this.keywordIndex]])) {
      ...
      if (
        !isNullOrUndefined(
          table[tableFormatProperty[this.keywordIndex]][
            shadingProperty[this.keywordIndex]
          ]
        ) &&
        !isNullOrUndefined(
      ...
      if (
        !isNullOrUndefined(
          table[tableFormatProperty[this.keywordIndex]][
            cellSpacingProperty[this.keywordIndex]
          ]
        ) &&
        table[tableFormatProperty[this.keywordIndex]][
          cellSpacingProperty[this.keywordIndex]
      ...
      if (!isNullOrUndefined(table[titleProperty[this.keywordIndex]])) {
        tagAttributes.push(
          'title="' + table[titleProperty[this.keywordIndex]] + '"'
        );
      }
      //Table description property
      if (!isNullOrUndefined(table[descriptionProperty[this.keywordIndex]])) {
        tagAttributes.push(
          'summary="' + table[descriptionProperty[this.keywordIndex]] + '"'
        );
      }
      if (
        !isNullOrUndefined(
        ...
        table[tableFormatProperty[this.keywordIndex]][
          tableAlignmentProperty[this.keywordIndex]
        ] === (this.keywordIndex == 1 ? 0 : 'Left')
      ) {
        tableStyle +=
          'margin-left:' +
          (table[tableFormatProperty[this.keywordIndex]][
      ...
      if (!isNullOrUndefined(table[tableFormatProperty[this.keywordIndex]])) {
        tableStyle += this.serializeTableWidth(
          table[tableFormatProperty[this.keywordIndex]]
        );
      }
      if (
        ...
        tableStyle += this.serializeTableBorderStyle(
          table[tableFormatProperty[this.keywordIndex]][
            bordersProperty[this.keywordIndex]
          ]
        );
  ...
  private serializeTableWidth(tableFormat: any): string {
    ...
    switch (tableFormat[preferredWidthTypeProperty[this.keywordIndex]]) {
      ...
      case 1:
        width +=
          'width: ' +
          tableFormat[preferredWidthProperty[this.keywordIndex]].toString() +
          '%;';
        break;
      case 'Point':
      case 2:
  ...
  private createRowStartTag(row: any): string {
    ...
    if (
      HelperMethods.parseBoolValue(
        row[rowFormatProperty[this.keywordIndex]][
          isHeaderProperty[this.keywordIndex]
        ]
      )
    ) {
    ...
    if (
      !isNullOrUndefined(
        row[rowFormatProperty[this.keywordIndex]][
          heightProperty[this.keywordIndex]
        ]
      ) &&
      row[rowFormatProperty[this.keywordIndex]][
        heightProperty[this.keywordIndex]
