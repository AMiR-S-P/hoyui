...
import { isNullOrUndefined, L10n, classList, Browser } from '@hoyui/base';
...
const CONTEXTMENU_COMPLETE_DELETE_TABLE: string =
  '_contextmenu_complete_table_delete';
...
const CONTEXTMENU_AUTO_FIT_TO_CONTENTS: string =
  '_contextmenu_auto_fit_contents';
const CONTEXTMENU_AUTO_FIT_TO_WINDOW: string = '_contextmenu_auto_fit_window';
const CONTEXTMENU_FIXED_COLUMN_WIDTH: string =
  '_contextmenu_fixed_column_width';
const CONTEXTMENU_CONTINUE_NUMBERING: string =
  '_contextmenu_continue_numbering';
...
const CONTEXTMENU_SPELLCHECK_OTHERSUGGESTIONS: string =
  '_contextmenu_otherSuggestions_spellcheck_';
...
const CONTEXTMENU_CONTENT_CONTROL_PROPERTIES: string =
  '_content_control_properties';
/**
 * Context Menu class
 */
export class ContextMenu {
  ...
  public initContextMenu(localValue: L10n, isRtl?: boolean): void {
    ...
    this.contextMenu.id =
      this.documentHelper.owner.containerId + 'e-de-contextmenu';
    ...
    this.contextMenuInstance = new Menu(
      menuOptions,
      '#' + this.documentHelper.owner.containerId + 'e-de-contextmenu-list'
    );
    this.contextMenuInstance.beforeOpen = () => {
      for (
        let index: number = 0;
        index < this.customMenuItems.length;
        index++
      ) {
      ...
      if (this.enableCustomContextMenu) {
        for (let index: number = 0; index < this.menuItems.length; index++) {
          if (typeof this.menuItems[index].id !== 'undefined') {
            document.getElementById(this.menuItems[index].id).style.display =
              'none';
          } else {
            (
              document.getElementById(this.menuItems[index - 1].id)
      ...
      if (this.viewer && this.documentHelper.selection) {
        classList(
          this.documentHelper.selection.caret,
          [],
    ...
    if (
      !isNullOrUndefined(
        this.documentHelper.owner.documentEditorSettings.popupTarget
      )
    ) {
      this.documentHelper.owner.documentEditorSettings.popupTarget.append(
        document.getElementsByClassName('e-de-contextmenu-wrapper')[0]
      );
    }
    this.contextMenuInstance.onClose = () => {
      if (this.viewer && this.documentHelper.selection) {
        ...
        if (
          id + CONTEXTMENU_FONT_DIALOG !== this.itemsmenu &&
          id + CONTEXTMENU_HYPERLINK !== this.itemsmenu &&
          id + CONTEXTMENU_TABLE !== this.itemsmenu &&
          id + CONTEXTMENU_PARAGRAPH !== this.itemsmenu
        ) {
  ...
  private openPasteDialog(): void {
    ...
    const contentValue: HTMLParagraphElement = createElement(
      'p'
    ...
    const contentWithBookMark: HTMLDivElement = createElement(
      'div'
    ) as HTMLDivElement;
    this.pasteCheckBoxDialog = createElement('input', {
      attrs: { type: 'checkbox' },
    }) as HTMLInputElement;
    contentWithBookMark.appendChild(this.pasteCheckBoxDialog);
    let bookmarkCheckbox = new CheckBox({
      label: this.locale.getConstant('Paste Content CheckBox'),
    });
    bookmarkCheckbox.appendTo(this.pasteCheckBoxDialog);
    container.appendChild(contentWithBookMark);
    let getDialogBox: boolean = JSON.parse(
      localStorage.getItem('ej_de_hidePasteAlert')
    );
    if (!getDialogBox) {
    ...
    switch (item) {
      case id + CONTEXTMENU_LOCK:
        if (this.documentHelper.owner.collaborativeEditingModule) {
          this.documentHelper.owner.collaborativeEditingModule.lockContent(
      ...
      case id + CONTEXTMENU_UNLOCK:
        if (this.documentHelper.owner.collaborativeEditingModule) {
          this.documentHelper.owner.collaborativeEditingModule.unlockContent(
            this.documentHelper.owner.currentUser
          );
        }
        break;
      ...
      case id + CONTEXTMENU_ADD_COMMENT:
        if (
          ...
          this.documentHelper.isCommentOnlyMode
        ) {
      ...
      case id + CONTEXTMENU_UPDATE_FIELD:
        ...
        if (
          this.documentHelper.selection.isReferenceField() &&
          (!isReadOnly ||
            (isReadOnly &&
      ...
      case id + CONTEXTMENU_CONTENT_CONTROL_PROPERTIES:
        if (
          !isNullOrUndefined(this.documentHelper.selection) &&
          this.documentHelper.selection.checkContentControlLocked &&
          this.documentHelper.owner.contentControlPropertiesDialogModule
        ) {
      ...
      case id + CONTEXTMENU_RESTART_AT:
        this.documentHelper.owner.editorModule.applyRestartNumbering(
      ...
      case id + CONTEXTMENU_SPELLING_DIALOG:
        ...
        this.documentHelper.owner.spellCheckDialogModule.show(
          contextInfo.text,
          contextInfo.element
        );
        break;
      ...
      case id + CONTEXTMENU_PROPERTIES:
        let inline: ElementBox =
          this.documentHelper.selection.getCurrentFormField();
      ...
      default:
        let expectedData: string =
          this.documentHelper.owner.element.id +
          CONTEXTMENU_SPELLCHECK_OTHERSUGGESTIONS;
  ...
  public addCustomMenu(
    ...
    let menuItems: MenuItemModel[] = JSON.parse(
      HelperMethods.sanitizeString(JSON.stringify(items))
    );
    this.destroy();
    ...
    for (let index: number = 0; index < menuItems.length; index++) {
      ...
      this.customMenuItems[index].id =
        this.documentHelper.owner.element.id + this.customMenuItems[index].id;
    }
  ...
  public onContextMenuInternal = (event: MouseEvent | TouchEvent): void => {
    ...
    if (
      ...
      let exactData: string = this.spellChecker.manageSpecialCharacters(
        this.currentContextInfo.text,
        undefined,
        true
      );
      if (
        !isNullOrUndefined(exactData) &&
        this.spellChecker.errorWordCollection.containsKey(exactData)
      ) {
        this.spellChecker.currentContextInfo = this.currentContextInfo;
        if (this.spellChecker.errorSuggestions.containsKey(exactData)) {
          allSuggestions = this.spellChecker.errorSuggestions
            .get(exactData)
            .slice();
          splittedSuggestion =
            this.spellChecker.handleSuggestions(allSuggestions);
          this.processSuggestions(
            allSuggestions,
            splittedSuggestion,
            isTouch ? (event as TouchEvent) : (event as MouseEvent)
          );
        } else {
              ...
              .then((data: any) => {
                ...
                if (!isNullOrUndefined(this.documentHelper)) {
                  if (!isNullOrUndefined(allSuggestions)) {
                    this.spellChecker.errorSuggestions.add(
                      exactData,
  ...
  private showContextMenuOnSel(event: MouseEvent | TouchEvent): void {
    ...
    let yPos: number = 0;
    if (isTouch) {
      let point: Point = this.documentHelper.getTouchOffsetValue(
        event as TouchEvent
      );
      xPos = point.x;
      yPos = point.y;
    ...
    if (this.spellContextItems.length > 0) {
      for (let i: number = 0; i < this.spellContextItems.length; i++) {
        if (this.customItems.indexOf(this.spellContextItems[i]) === -1) {
          let item: HTMLElement = document.getElementById(
            this.documentHelper.owner.element.id + this.spellContextItems[i].id
          );
   ...
   */
  /* eslint-disable @typescript-eslint/no-explicit-any */
  public processSuggestions(
    allSuggestions: any,
    splittedSuggestion: string[],
    event: MouseEvent | TouchEvent
  ): void {
    this.spellContextItems = this.constructContextmenu(
      allSuggestions,
      splittedSuggestion
    );
    this.addCustomMenu(this.spellContextItems);
    this.noSuggestion = document.getElementById(
      this.documentHelper.owner.element.id + CONTEXTMENU_NO_SUGGESTION
    );
   ...
   */
  /* eslint-disable @typescript-eslint/no-explicit-any */
  public constructContextmenu(
    allSuggestion: any[],
    splittedSuggestion: any
  ): any[] {
    let contextMenuItems: any[] =
      this.customItems.length > 0 ? this.customItems.slice() : [];
    // classList(this.noSuggestion,['e-disabled'],[]);
    if (isNullOrUndefined(allSuggestion) || allSuggestion.length === 0) {
      contextMenuItems.push({
        text: this.locale.getConstant('no suggestions'),
        id: CONTEXTMENU_NO_SUGGESTION,
        classList: ['e-focused'],
        iconCss: '',
      });
    } else {
      for (let i: number = 0; i < allSuggestion.length; i++) {
        contextMenuItems.push({
          text: allSuggestion[i],
          id: CONTEXTMENU_SPELLCHECK_OTHERSUGGESTIONS + allSuggestion[i],
          iconCss: '',
        });
      }
    }
    contextMenuItems.push({
      separator: true,
      id: '_contextmenu_suggestion_separator',
    });
    if (
      !isNullOrUndefined(splittedSuggestion) &&
      splittedSuggestion.length > 1
    ) {
      contextMenuItems.push({
        text: this.locale.getConstant('More Suggestion'),
        items: splittedSuggestion,
      });
      contextMenuItems.push({
        separator: true,
        id: '_contextmenu_moreSuggestion_separator',
      });
    } else {
      contextMenuItems.push({
        text: this.locale.getConstant('Add to Dictionary'),
        id: '_contextmenu_otherSuggestions_spellcheck_Add to Dictionary',
        iconCss: '',
      });
    }
    contextMenuItems.push({
      text: this.locale.getConstant('Ignore Once'),
      id: '_contextmenu_otherSuggestions_spellcheck_Ignore Once',
      iconCss: '',
    });
    contextMenuItems.push({
      text: this.locale.getConstant('Ignore All'),
      id: '_contextmenu_otherSuggestions_spellcheck_Ignore All',
      iconCss: '',
    });
    contextMenuItems.push({
      separator: true,
      id: '_contextmenu_change_separator',
    });
    contextMenuItems.push({
      text: this.locale.getConstant('Spelling'),
      id: CONTEXTMENU_SPELLING_DIALOG,
      iconCss: 'e-icons e-de-spellcheck',
      items: [],
    });
    contextMenuItems.push({
      separator: true,
      id: '_contextmenu_spelling_separator',
    });
  ...
  private showHideElements(selection: Selection): boolean {
    ...
    let updateField: HTMLElement = document.getElementById(
      id + CONTEXTMENU_UPDATE_FIELD
    );
    let editField: HTMLElement = document.getElementById(
      id + CONTEXTMENU_EDIT_FIELD
    );
    let font: HTMLElement = document.getElementById(
      id + CONTEXTMENU_FONT_DIALOG
    );
    let paragraph: HTMLElement = document.getElementById(
      id + CONTEXTMENU_PARAGRAPH
    );
    let tableProperties: HTMLElement = document.getElementById(
      id + CONTEXTMENU_TABLE
    );
    let insertTable: HTMLElement = document.getElementById(
      id + CONTEXTMENU_INSERT_TABLE
    );
    let deleteTable: HTMLElement = document.getElementById(
      id + CONTEXTMENU_DELETE_TABLE
    );
    let mergeCells: HTMLElement = document.getElementById(
      id + CONTEXTMENU_MERGE_CELL
    );
    let hyperlink: HTMLElement = document.getElementById(
      id + CONTEXTMENU_HYPERLINK
    );
    let openHyperlink: HTMLElement = document.getElementById(
      id + CONTEXTMENU_OPEN_HYPERLINK
    );
    let editHyperlink: HTMLElement = document.getElementById(
      id + CONTEXTMENU_EDIT_HYPERLINK
    );
    let copyHyperlink: HTMLElement = document.getElementById(
      id + CONTEXTMENU_COPY_HYPERLINK
    );
    let removeHyperlink: HTMLElement = document.getElementById(
      id + CONTEXTMENU_REMOVE_HYPERLINK
    );
    let properties: HTMLElement = document.getElementById(
      id + CONTEXTMENU_PROPERTIES
    );
    let continueNumbering: HTMLElement = document.getElementById(
      id + CONTEXTMENU_CONTINUE_NUMBERING
    );
    let restartAt: HTMLElement = document.getElementById(
      id + CONTEXTMENU_RESTART_AT
    );
    let autoFitTable: HTMLElement = document.getElementById(
      id + CONTEXTMENU_AUTO_FIT
    );
    let addComment: HTMLElement = document.getElementById(
      id + CONTEXTMENU_ADD_COMMENT
    );
    let editNoteoptions: HTMLElement = document.getElementById(
      id + CONTEXTMENU_NOTE_OPTIONS
    );
    let removeContentControl: HTMLElement = document.getElementById(
      id + CONTEXTMENU_REMOVE_CONTENT_CONTROL
    );
    let contentControlProperties: HTMLElement = document.getElementById(
      id + CONTEXTMENU_CONTENT_CONTROL_PROPERTIES
    );
    if (!this.documentHelper.owner.enableLockAndEdit) {
      ...
      if (
        this.documentHelper.editRanges.containsKey(
          this.documentHelper.owner.currentUser
        )
      ) {
        lock.style.display = 'none';
        ...
        if (
          this.documentHelper.owner.collaborativeEditingModule &&
    ...
    let acceptChange: HTMLElement = document.getElementById(
      id + CONTEXTMENU_ACCEPT_CHANGES
    );
    let rejectChange: HTMLElement = document.getElementById(
      id + CONTEXTMENU_REJECT_CHANGES
    );
    ...
    let contentControl: ContentControl =
      this.documentHelper.owner.editor.getContentControl();
    let contentControlImage: ElementBox =
      this.documentHelper.owner.getImageContentControl();
    if (
      (!isNullOrUndefined(contentControl) &&
        contentControl instanceof ContentControl &&
        !contentControl.contentControlProperties.lockContentControl &&
        !this.documentHelper.owner.isReadOnlyMode) ||
    ...
    if (
      ...
      (contentControlProperties.nextSibling as HTMLElement).style.display =
        'block';
    } else {
      contentControlProperties.style.display = 'none';
      (contentControlProperties.nextSibling as HTMLElement).style.display =
    ...
    if (
      field instanceof FieldElementBox &&
      isCrossRefField &&
      (this.documentHelper.protectionType === 'FormFieldsOnly' ||
        !this.documentHelper.owner.isReadOnlyMode)
    ) {
    ...
    let isHideComment: boolean =
      this.documentHelper.owner.isReadOnlyMode ||
      this.documentHelper.owner.enableHeaderAndFooter ||
      !this.documentHelper.owner.enableComment;
    addComment.style.display = isHideComment ? 'none' : 'block';
    if (this.documentHelper.isCommentOnlyMode) {
      addComment.style.display = 'block';
    }
    (addComment.nextSibling as HTMLElement).style.display = isHideComment
      ? 'none'
      : 'block';
    ...
    if (
      this.viewer &&
      this.documentHelper.owner &&
      this.documentHelper.owner.commentReviewPane &&
    ...
    if (!Browser.isIE) {
      ...
      if (getDialogBox) {
        let enablePaste: boolean =
          owner.enableLocalPaste &&
          !isNullOrUndefined(owner.editorModule.copiedData);
        classList(
          paste,
          enablePaste ? [] : ['e-disabled'],
          enablePaste ? ['e-disabled'] : []
        );
      }
    }
    if (selection.contextType === 'TableOfContents') {
      ...
      if (
        selection.contextType === 'List' &&
        owner.selectionModule.getListLevel(start.paragraph).listLevelPattern !==
          'Bullet'
      ) {
      ...
      let isCellOrRowSelected: boolean =
        start.paragraph.isInsideTable &&
        (!end.paragraph.isInsideTable ||
      ...
      if (
        selection.isFormField() &&
        this.documentHelper.owner.enableFormField
      ) {
        hyperlink.style.display = 'none';
    ...
    if (
      this.documentHelper.owner.selectionModule.start.paragraph.isInsideTable &&
      ...
      autoFitTable.style.display =
        this.documentHelper.selection.isTableSelected() ? 'block' : 'none';
    }
    if (this.documentHelper.owner.fontDialogModule) {
    ...
    if (this.documentHelper.owner.paragraphDialogModule) {
      ...
      !isDialogHidden &&
      !this.documentHelper.owner.fontDialogModule &&
      !isHideComment
    ) {
    ...
    if (this.documentHelper.selection.hasRevisions()) {
      (acceptChange.previousSibling as HTMLElement).style.display = this
        .documentHelper.owner.enableHeaderAndFooter
        ? 'none'
        : 'block';
    ...
    if (
      this.documentHelper.selection.isinFootnote ||
      this.documentHelper.selection.isinEndnote
    ) {
