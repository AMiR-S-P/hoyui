import { isNullOrUndefined, L10n } from '@hoyui/base';
...
import {
  ...
  DocumentCanvasRenderingContext2D,
...
export class Renderer {
   ...
   * @private
   */
  public commentMarkDictionary: Dictionary<
    HTMLElement,
    CommentCharacterElementBox[]
  ...
  public get pageCanvas(): HTMLCanvasElement | DocumentCanvasElement {
    ...
    if (this.isExporting) {
      ...
      return isNullOrUndefined(this.viewer)
        ? undefined
        : this.documentHelper.containerCanvas;
    }
  }
  ...
  private get selectionCanvas(): HTMLCanvasElement {
    return isNullOrUndefined(this.viewer)
      ? undefined
      : this.documentHelper.selectionCanvas;
  }
  ...
  public renderWidgets(
    ...
    this.pageContext.fillStyle = HelperMethods.getColor(
      this.documentHelper.backgroundColor
    );
    this.pageContext.beginPath();
    if (this.viewer instanceof WebLayoutViewer) {
      height =
        height > this.documentHelper.visibleBounds.height
          ? height
          : this.documentHelper.visibleBounds.height;
      let marginTop: number = top;
      if (page.index === 0) {
        marginTop = top - this.viewer.padding.top;
      }
      /* eslint-disable-next-line max-len */
      this.pageContext.fillRect(
        left - this.viewer.padding.left,
        marginTop,
        width + this.viewer.padding.left,
        height + this.viewer.padding.top
      );
    } else {
    ...
    for (let i: number = 0; i < page.bodyWidgets.length; i++) {
      ...
      if (
        page.footnoteWidget &&
        this.documentHelper.owner.layoutType === 'Pages'
      ) {
  ...
  private setPageSize(page: Page): void {
    ...
    const dpr: number = Math.max(
      ...
      this.documentHelper.owner.documentEditorSettings.printDevicePixelRatio
    );
    if (
  ...
  private renderHFWidgets(
    ...
    if (!this.isPrinting) {
      this.pageContext.globalAlpha = this.documentHelper.owner
    ...
    const headerFooterHeight: number =
      (page.boundingRectangle.height / 100) * 40;
    if (isHeader) {
      const topMargin: number = HelperMethods.convertPointToPixel(
        page.bodyWidgets[0].sectionFormat.topMargin
      );
      const widgetHeight: number = Math.max(
      ...
      if (widgetHeight > headerFooterHeight) {
        ...
        this.pageContext.beginPath();
        this.pageContext.save();
        this.pageContext.rect(
          this.pageLeft,
          this.pageTop,
          width,
          this.getScaledValue(headerFooterHeight)
        );
        this.pageContext.clip();
      }
    } else {
      const footerDistance: number = HelperMethods.convertPointToPixel(
        page.bodyWidgets[0].sectionFormat.footerDistance
      );
      let footerHeight: number;
      if (isNullOrUndefined(page.footerWidgetIn.sectionFormat)) {
        footerHeight =
          page.boundingRectangle.height -
          /* eslint-disable-next-line max-len */
          Math.max(
            page.footerWidgetIn.height + footerDistance,
      ...
      height = Math.max(
        page.boundingRectangle.height - headerFooterHeight,
        footerHeight
      );
    ...
    for (let i: number = 0; i < widget.childWidgets.length; i++) {
      const block: BlockWidget = widget.childWidgets[
  ...
  private renderHeaderSeparator(
    ...
    let y: number = this.getScaledValue(
      Math.max(widget.y + widget.height, Math.abs(topMargin))
    );
    const pageWidth: number = this.getScaledValue(page.boundingRectangle.width);
    const ctx: CanvasRenderingContext2D | DocumentCanvasRenderingContext2D =
      this.pageContext;
    ctx.save();
    ctx.globalAlpha = 0.85;
    const headerFooterHeight: number =
      (this.getScaledValue(page.boundingRectangle.height) / 100) * 40;
    ...
    const index: number = (this.viewer as PageLayoutViewer).getHeaderFooter(
      widget.headerFooterType
    );
    const sectionIndex: number = page.sectionIndex;
    const l10n: L10n = new L10n(
      'documenteditor',
      this.documentHelper.owner.defaultLocale
    );
    ...
    if (
      this.documentHelper.headersFooters[parseInt(sectionIndex.toString(), 10)]
    ) {
      headerFooterWidget = this.documentHelper.headersFooters[
        parseInt(sectionIndex.toString(), 10)
      ][parseInt(index.toString(), 10)] as HeaderFooterWidget;
    }
    ...
    if (page.footerWidget) {
      ...
      let footerHeight: number =
        this.getScaledValue(page.boundingRectangle.height) -
        /* eslint-disable-next-line max-len */
        this.getScaledValue(
          Math.max(
            page.footerWidgetIn.height + footerDistance,
      ...
      footerHeight = Math.max(
        this.getScaledValue(page.boundingRectangle.height) - headerFooterHeight,
        footerHeight
      );
      this.renderDashLine(
        ctx,
      ...
      this.renderHeaderFooterMarkText(
        ctx,
        type,
        left + 10,
        top + footerHeight - 5
      ...
      const index: number = (this.viewer as PageLayoutViewer).getHeaderFooter(
        page.footerWidget.headerFooterType
      );
      let headerFooterWidget: HeaderFooterWidget;
      if (
        this.documentHelper.headersFooters[
          parseInt(sectionIndex.toString(), 10)
        ]
      ) {
        headerFooterWidget = this.documentHelper.headersFooters[
  ...
  private getFooterHeight(page: Page): number {
    const footerWidgetHeight: number =
      (page.boundingRectangle.height / 100) * 40;
    const footerDistance: number = HelperMethods.convertPointToPixel(
      page.bodyWidgets[0].sectionFormat.footerDistance
    );
    ...
    return Math.max(
      page.boundingRectangle.height - footerWidgetHeight,
      actualHeight
    );
  }
  ...
  private getHeaderFooterType(page: Page, isHeader: boolean): string {
    ...
    if (
      page.bodyWidgets[0].sectionFormat.differentFirstPage &&
      (isNullOrUndefined(page.previousPage) ||
        page.sectionIndex !== page.previousPage.sectionIndex)
    ) {
      type = isHeader
        ? l10n.getConstant('First Page Header')
        : l10n.getConstant('First Page Footer');
    } else if (page.bodyWidgets[0].sectionFormat.differentOddAndEvenPages) {
      if ((this.documentHelper.pages.indexOf(page) + 1) % 2 === 0) {
        type = isHeader
          ? l10n.getConstant('Even Page Header')
          : l10n.getConstant('Even Page Footer');
      } else {
        type = isHeader
          ? l10n.getConstant('Odd Page Header')
          : l10n.getConstant('Odd Page Footer');
      }
    }
    return type;
  }
  ...
  /* eslint-disable-next-line max-len */
  public renderDashLine(
    context: CanvasRenderingContext2D | DocumentCanvasRenderingContext2D,
    x: number,
    y: number,
    width: number,
    fillStyle: string,
  ...
  private renderHeaderFooterMark(
    ctx: CanvasRenderingContext2D | DocumentCanvasRenderingContext2D,
    x: number,
    y: number,
    w: number,
    h: number
  ...
  private render(page: Page, bodyWidget: BodyWidget): void {
    ...
    for (let i: number = 0; i < page.bodyWidgets.length; i++) {
      if (!isNullOrUndefined(page.bodyWidgets[i].floatingElements)) {
        this.renderFloatingItems(
          page,
          page.bodyWidgets[i].floatingElements,
          'Behind'
        );
      }
    }
    ...
    if (
      !isNullOrUndefined(bodyWidget.nextRenderedWidget) &&
      bodyWidget.columnIndex + 1 ===
        (bodyWidget.nextRenderedWidget as BodyWidget).columnIndex
    ) {
      nextColumnBody = bodyWidget.nextRenderedWidget as BodyWidget;
    }
    if (
      !(this.viewer instanceof WebLayoutViewer) &&
      bodyWidget.sectionFormat.columns.length > 1 &&
      ...
      if (bodyWidget.columnIndex === 0) {
        /* eslint-disable */
        xPos =
          page.bodyWidgets[colIndex].x -
          HelperMethods.convertPointToPixel(
            page.bodyWidgets[colIndex].sectionFormat.leftMargin
          );
        width =
          HelperMethods.convertPointToPixel(
            page.bodyWidgets[colIndex].sectionFormat.leftMargin
          ...
          (
            bodyWidget.sectionFormat.columns[
        ...
        width =
          HelperMethods.convertPointToPixel(
            page.bodyWidgets[colIndex].sectionFormat.rightMargin
          ) +
          (
            bodyWidget.sectionFormat.columns[
              bodyWidget.columnIndex
      ...
      this.clipRect(
        xPos,
        page.bodyWidgets[colIndex].y,
        this.getScaledValue(width),
        this.getScaledValue(page.boundingRectangle.height)
      );
      /* eslint-enable */
      isClipped = true;
    ...
    for (let i: number = 0; i < bodyWidget.childWidgets.length; i++) {
      const widget: Widget = bodyWidget.childWidgets[
        parseInt(i.toString(), 10)
      ] as ParagraphWidget;
      if (
        i === 0 &&
        bodyWidget.childWidgets[0] instanceof TableWidget &&
        ...
        this.renderHeader(
          ...
          this.documentHelper.layout.getHeader(
            bodyWidget.childWidgets[0] as TableWidget
          )
        );
      }
    ...
    for (let i: number = 0; i < page.bodyWidgets.length; i++) {
      if (
        page.bodyWidgets[parseInt(i.toString(), 10)].sectionFormat
          .lineBetweenColumns === true
      ) {
        if (
          page.bodyWidgets[parseInt(i.toString(), 10)].columnIndex !== 0 &&
          page.bodyWidgets.length > 1
        ) {
          const topMargin: number = HelperMethods.convertPointToPixel(
            page.bodyWidgets[0].sectionFormat.topMargin
          );
          const linestartY: number = this.getScaledValue(
            Math.max(
              page.headerWidgetIn.y + page.headerWidgetIn.height,
              topMargin
            )
          );
          const headerFooterHeight: number =
          ...
          const inBetweenSpace: number =
            (page.bodyWidgets[parseInt(i.toString(), 10)].x -
              (page.bodyWidgets[parseInt(i.toString(), 10)]
                .previousRenderedWidget.x +
                page.bodyWidgets[parseInt(i.toString(), 10)]
                  .previousRenderedWidget.width)) /
            2;
          const startX: number =
            inBetweenSpace +
            (page.bodyWidgets[parseInt(i.toString(), 10)].previousRenderedWidget
              .x +
              page.bodyWidgets[parseInt(i.toString(), 10)]
                .previousRenderedWidget.width);
          ...
          if (page.footnoteWidget) {
            endY =
              (footerHeight -
                page.footerWidgetIn.height / 2 -
                page.footnoteWidget.height * this.documentHelper.zoomFactor) /
              this.documentHelper.zoomFactor;
          } else {
            endY =
              (footerHeight - page.footerWidgetIn.height / 2) /
              this.documentHelper.zoomFactor;
          }
          const firstBody: BodyWidget =
            this.documentHelper.layout.getBodyWidget(
              page.bodyWidgets[parseInt(i.toString(), 10)],
              true
            );
          const height: number =
            this.documentHelper.layout.getNextWidgetHeight(firstBody);
          startY = page.bodyWidgets[parseInt(i.toString(), 10)].y;
          endY = height;
          const color: string = '#000000';
          this.renderSingleBorder(
            color,
            startX,
            startY,
            endX,
  ...
  private renderFloatingItems(
    ...
    wrappingType: TextWrappingStyle
  ): void {
    if (!isNullOrUndefined(floatingElements) && floatingElements.length > 0) {
      const overLappedShapeWidgets: Dictionary<number, ShapeBase> =
        new Dictionary<number, ShapeBase>();
      /* eslint-disable */
      ...
      for (let i: number = 0; i < floatingElements.length; i++) {
        ...
        if (
          !this.isOverLappedShapeWidget(shape) ||
          overLappedShapeWidgets.containsKey(shape.zOrderPosition)
        ) {
      ...
      if (overLappedShapeWidgets.length > 0) {
        let sortedOverLappedShapeWidgets: number[] =
          overLappedShapeWidgets.keys.sort();
        for (let j: number = 0; j < sortedOverLappedShapeWidgets.length; j++) {
          let shape: ShapeBase = overLappedShapeWidgets.get(
            sortedOverLappedShapeWidgets[j]
          ) as ShapeBase;
          if (shape instanceof ImageElementBox) {
            this.renderImageElementBox(shape, shape.x, shape.y, 0);
          } else if (shape instanceof ShapeElementBox) {
  ...
  private isOverLappedShapeWidget(floatingElement: ShapeBase): boolean {
    return (
      (floatingElement instanceof ImageElementBox &&
  ...
  private renderShapeElementBox(
    ...
    if (
      shape.fillFormat &&
      shape.fillFormat.color &&
      shape.fillFormat.fill &&
      shapeType !== 'StraightConnector'
    ) {
      ...
      if (shapeType === 'Rectangle') {
        this.pageContext.fillRect(
          shapeLeft,
          shapeTop,
          this.getScaledValue(shape.width),
          this.getScaledValue(shape.height)
        );
      } else {
    ...
    if (!isNullOrUndefined(shapeType)) {
      if (shape.lineFormat.line && shape.lineFormat.lineFormatType !== 'None') {
        ...
        this.pageContext.strokeStyle = HelperMethods.getColor(
          shape.lineFormat.color
        );
        if (shapeType === 'Rectangle') {
          this.pageContext.strokeRect(
            shapeLeft,
            shapeTop,
            this.getScaledValue(shape.width),
            this.getScaledValue(shape.height)
          ...
          this.pageContext.lineTo(
            shapeLeft + this.getScaledValue(shape.width),
            shapeTop + this.getScaledValue(shape.height)
          );
  ...
  private renderPath(canvas: any, collection: Object[]): void {
    ...
    let length: number;
    let i: number;
    const segs: Object[] = collection;
    for (x = 0, y = 0, i = 0, length = segs.length; i < length; ++i) {
      const obj: Object = segs[parseInt(i.toString(), 10)];
  ...
  private r(u: number[], v: number[]): number {
    return (u[0] * v[0] + u[1] * v[1]) / (this.m(u) * this.m(v));
  }
  // angle between two vectors
  private a(u: number[], v: number[]): number {
  ...
  private updatePath(
    ...
    if (
      this.getScaledValue(shape.width) !== bounds.width ||
      this.getScaledValue(shape.height) !== bounds.height
    ) {
      scaleX =
        this.getScaledValue(shape.width) /
        Number(bounds.width ? bounds.width : 1);
      scaleY =
        this.getScaledValue(shape.height) /
        Number(bounds.height ? bounds.height : 1);
      isScale = true;
    }
    arrayCollection = processPathData(pathData);
    arrayCollection = splitArrayCollection(arrayCollection);
    if (isScale) {
      newPathString = transformPath(
        arrayCollection,
        scaleX,
        scaleY,
        isScale,
  ...
  private renderLockRegionBorder(page: Page, widget: Widget) {
    if (
      !(widget as BlockWidget).isInsideTable &&
      widget instanceof BlockWidget &&
      widget.locked
    ) {
      let settinsModel: CollaborativeEditingSettingsModel =
        this.documentHelper.owner.documentEditorSettings
          .collaborativeEditingSettings;
      let sectionFormat: WSectionFormat = page.bodyWidgets[0].sectionFormat;
      let leftPosition: number =
        HelperMethods.convertPointToPixel(sectionFormat.leftMargin) - 5;
      let pageWidth: number =
        sectionFormat.pageWidth -
        sectionFormat.leftMargin -
        sectionFormat.rightMargin;
      pageWidth = HelperMethods.convertPointToPixel(pageWidth) + 10;
      if (this.viewer instanceof WebLayoutViewer) {
        leftPosition = widget.x - 5;
        pageWidth =
          (this.documentHelper.visibleBounds.width -
            this.viewer.padding.right * 5) /
          this.documentHelper.zoomFactor;
      }
      let previousWidget: BlockWidget =
        widget.previousRenderedWidget as BlockWidget;
      let nextWidget: BlockWidget = widget.nextRenderedWidget as BlockWidget;

      let color: string =
        widget.lockedBy === this.documentHelper.owner.currentUser
          ? settinsModel.editableRegionColor
          : settinsModel.lockedRegionColor;
      ...
      //Top border
      if (
        isNullOrUndefined(previousWidget) ||
        !previousWidget.locked ||
        widget.lockedBy !== previousWidget.lockedBy
      ) {
        this.renderSingleBorder(
          color,
          leftPosition,
      ...
      if (
        isNullOrUndefined(nextWidget) ||
        !nextWidget.locked ||
        widget.lockedBy !== nextWidget.lockedBy
      ) {
        // Bottom border
        this.renderSingleBorder(
          color,
  ...
  private renderHeader(
    ...
    let parentTable: TableWidget =
      header.ownerTable.getSplitWidgets()[0] as TableWidget;
    ...
    table.childWidgets = [];
    page.viewer.updateClientAreaLocation(
      table,
      new Rect(page.viewer.clientArea.x, top, table.width, table.height)
    );
    this.updateTableHeaderRows(header, table, page, top);
    this.isRenderHeader = true;
    for (let j: number = 0; j < table.childWidgets.length; j++) {
      let headerWidget: TableRowWidget = table.childWidgets[
        j
      ] as TableRowWidget;
  ...
  private updateTableHeaderRows(
    headerRow: TableRowWidget,
    clonedTable: TableWidget,
    page: Page,
    top: number
  ): void {
      ...
      if (row.rowFormat.isHeader) {
        ...
        if (row == headerRow) {
          for (let j: number = 0; j < headerRow.childWidgets.length; j++) {
            let cell: TableCellWidget = headerRow.childWidgets[
              j
            ] as TableCellWidget;
          ...
          if (rowSpan > 1 && i + rowSpan < table.childWidgets.length) {
            for (let k: number = 1; k < rowSpan; k++) {
              let nextRow: TableRowWidget = table.childWidgets[
                i + k
              ] as TableRowWidget;
              if (!isNullOrUndefined(nextRow)) {
                ...
                page.documentHelper.layout.updateChildLocationForRow(
                  top,
                  clonedRow
                );
                top += clonedRow.height;
  ...
  private renderParagraphWidget(page: Page, paraWidget: ParagraphWidget): void {
    ...
    if (paddingLeft > 0) {
      this.clipRect(
        paraWidget.x + paddingLeft,
        this.getScaledValue(page.boundingRectangle.y),
        this.getScaledValue(page.boundingRectangle.width),
        this.getScaledValue(page.boundingRectangle.height)
      );
      isClipped = true;
    }
    if (
      !(
        paraWidget.containerWidget instanceof HeaderFooterWidget &&
        paraWidget.containerWidget.isEmpty &&
        !isNullOrUndefined(this.documentHelper.owner.selectionModule) &&
        !isNullOrUndefined(this.documentHelper.selection.start.paragraph) &&
  ...
  private renderParagraphBorder(page: Page, paragraphWidet: ParagraphWidget) {
    ...
    let canRenderParagraphBorders: BorderRenderInfo =
      this.documentHelper.canRenderBorder(paragraphWidet);
    if (!isNullOrUndefined(leftBorder) && leftBorder.lineStyle !== 'None') {
      startX =
        this.documentHelper.getParagraphLeftPosition(paragraphWidet) -
        HelperMethods.convertPointToPixel(leftBorder.space);
      endX = startX;
      endY = startY + paragraphWidet.height;
      if (
        topBorder.lineStyle !== 'None' &&
        firstLine.isFirstLine() &&
        !canRenderParagraphBorders.skipTopBorder
      ) {
        startY =
          paragraphWidet.y +
          this.getTopMargin(paragraphWidet) -
          HelperMethods.convertPointToPixel(
            topBorder.lineWidth + topBorder.space
          );
        endY =
          startY +
          paragraphWidet.height -
          (this.getTopMargin(paragraphWidet) -
            HelperMethods.convertPointToPixel(
              topBorder.lineWidth + topBorder.space
            ));
      ...
      if (
        bottomBorder.lineStyle !== 'None' &&
        lastLine.isLastLine() &&
        !canRenderParagraphBorders.skipBottomBorder
      ) {
        endY =
          endY +
          HelperMethods.convertPointToPixel(
            bottomBorder.lineWidth + bottomBorder.space
          ) -
          this.getBottomMargin(paragraphWidet);
      }
    ...
    if (!isNullOrUndefined(rightBorder) && rightBorder.lineStyle !== 'None') {
      startX =
        this.documentHelper.getParagraphLeftPosition(paragraphWidet) +
        this.getContainerWidth(paragraphWidet, page) +
        HelperMethods.convertPointToPixel(rightBorder.space);
      startY = endY;
      endX = startX;
      endY = startY + paragraphWidet.height;
      if (
    ...
    if (
      !isNullOrUndefined(bottomBorder) &&
      bottomBorder.lineStyle !== 'None' &&
      lastLine.isLastLine() &&
      !canRenderParagraphBorders.skipBottomBorder
    ) {
  ...
  private renderImgBorder(
    ...
    let previous =
      this.documentHelper.selection.getPreviousTextElement(elementBox);

    // Calculate top margin based on baseline alignment
    let topMargin: number =
      baselineAlignment === 'Subscript'
        ? elementBox.margin.top + (elementBox.height - elementBox.height / 1.5)
        : elementBox.margin.top;
    ...
    let renderedHeight: number =
      elementBox.height / (baselineAlignment === 'Normal' ? 1 : 1.5) + 15;
    let startX = 0;
    let startY = 0;
    let endX = 0;
    let endY = 0;
    ...
    if (previous instanceof TextElementBox) {
      if (
        previous.line !== elementBox.line ||
        previous.contentControlProperties == undefined
      ) {
        borderStyle = 'Solid';
      } else {
        borderStyle = 'None';
    ...
    if (next instanceof TextElementBox) {
      if (
        next.line !== elementBox.line ||
        next.contentControlProperties == undefined
      ) {
        borderStyle = 'Solid';
      } else {
        borderStyle = 'None';
  ...
  private getContainerWidth(paraWidget: ParagraphWidget, page: Page): any {
    ...
    if (paraWidget.isInsideTable) {
      ...
      return (
        cell.width + cell.margin.left + cell.margin.right - cell.leftBorderWidth
      );
    } else {
      if (this.viewer instanceof WebLayoutViewer) {
        let indent: number = HelperMethods.convertPointToPixel(
          paraWidget.leftIndent + paraWidget.rightIndent
        );
        return (
          (this.documentHelper.visibleBounds.width -
            indent -
            this.viewer.padding.right * 4 -
            this.viewer.padding.left * 2) /
          this.documentHelper.zoomFactor
        );
      } else {
        let section: BodyWidget = paraWidget.bodyWidget as BodyWidget;
        if (
          section instanceof BodyWidget &&
          section.sectionFormat.columns.length > 1
        ) {
          let colIndex: number = section.columnIndex;
          return (
            (section.sectionFormat.columns[colIndex] as WColumnFormat).width +
            HelperMethods.convertPointToPixel(
              hangingIndent - (paraWidget.rightIndent + paraWidget.leftIndent)
            )
          );
        } else {
          let width: number =
          ...
          return HelperMethods.convertPointToPixel(
            width +
              hangingIndent -
              (paraWidget.rightIndent + paraWidget.leftIndent)
          );
        }
      }
    }
  ...
  private getBottomMargin(paragarph: ParagraphWidget) {
    if (paragarph.isEmpty()) {
      return (
        paragarph.childWidgets[paragarph.childWidgets.length - 1] as LineWidget
      ).margin.bottom;
    } else {
      let widget: LineWidget = paragarph.childWidgets[
        paragarph.childWidgets.length - 1
      ] as LineWidget;
  ...
  private renderfootNoteWidget(
    ...
    let isEmptyPage: boolean =
      footnote.page.bodyWidgets.length === 1 &&
      ((footnote.page.bodyWidgets[0].childWidgets.length === 1 &&
        (footnote.page.bodyWidgets[0].childWidgets[0] as ParagraphWidget)
          .isEmpty != undefined &&
        (
          footnote.page.bodyWidgets[0].childWidgets[0] as ParagraphWidget
        ).isEmpty()) ||
        footnote.page.bodyWidgets[0].childWidgets.length === 0);
    ...
    this.pageContext.rect(
      this.pageLeft,
      this.getScaledValue(footnote.y, 2),
      this.getScaledValue(width),
      this.getScaledValue(height)
    );
    this.pageContext.clip();
    for (let i: number = 0; i < footnote.bodyWidgets.length; i++) {
      ...
      for (let j: number = 0; j < bodyWidget.childWidgets.length; j++) {
        ...
        if (i === 0 && j === 0) {
          let ctx: CanvasRenderingContext2D | DocumentCanvasRenderingContext2D =
            this.pageContext;
          let xPos: number = page.bodyWidgets[0].x;
          if (
            page.bodyWidgets.length > 1 &&
            !isNullOrUndefined(bodyWidget.nextWidget) &&
            !(
              (bodyWidget.nextWidget as BodyWidget).sectionFormat.breakCode ===
              'NoBreak'
            )
          ) {
            ...
            this.renderSolidLine(
              ctx,
              this.getScaledValue(xPos, 1),
              this.getScaledValue(footnote.y + footnote.margin.top / 2 + 1, 2),
              footWidth * this.documentHelper.zoomFactor,
              '#000000'
            );
          } else {
            this.renderSolidLine(
              ...
              this.getScaledValue(xPos, 1),
              this.getScaledValue(footnote.y + footnote.margin.top / 2 + 1, 2),
              210 * this.documentHelper.zoomFactor,
              '#000000'
            );
          }
        }
        if (
          !isNullOrUndefined(footNoteReference) &&
          !this.documentHelper.owner.editorModule.isFootNoteInsert
        ) {
          //if (j < 1 || (j > 0 && widget.footNoteReference !== (bodyWidget.childWidgets[j - 1] as BlockWidget).footNoteReference)) {
          const paragraph: ParagraphWidget =
            this.documentHelper.getFirstParagraphBlock(widget);
          if (
            (paragraph.firstChild as LineWidget).children[0] instanceof
            TextElementBox
          ) {
            let footNoteElement: TextElementBox = (
              paragraph.firstChild as LineWidget
            ).children[0] as TextElementBox;
            if (footNoteElement.text === '\u0002') {
              footNoteElement.text = footNoteElement.text.replace(
                footNoteElement.text,
  ...
  private renderTableWidget(page: Page, tableWidget: TableWidget): void {
    ...
    for (let i: number = 0; i < tableWidget.childWidgets.length; i++) {
      ...
      if (
        (widget as TableRowWidget).isRenderBookmarkEnd &&
        this.documentHelper.owner.documentEditorSettings.showBookmarks
      ) {
        let cellWidget: TableCellWidget = widget.lastChild as TableCellWidget;
        let border = cellWidget.ownerTable.isBidiTable
          ? TableCellWidget.getCellLeftBorder(cellWidget)
          : TableCellWidget.getCellRightBorder(cellWidget);
        let lineWidth = HelperMethods.convertPointToPixel(
          border.getLineWidth()
        );
        let lastPara: ParagraphWidget =
          this.documentHelper.selection.getLastParagraph(
            widget.lastChild as TableCellWidget
          );
        let height: number =
          (lastPara.lastChild as LineWidget).height -
          (lastPara.lastChild as LineWidget).margin.bottom;
        this.renderBookmark(
          this.getScaledValue(
            cellWidget.x +
              cellWidget.width +
              ...
              lineWidth / 2 +
              this.documentHelper.textHelper.getParagraphMarkWidth(
                lastPara.characterFormat
              ),
            1
          ),
          this.getScaledValue(cellWidget.y - cellWidget.margin.top, 2),
          this.getScaledValue(height),
          1
        );
      }
  ...
  private renderTableRowWidget(page: Page, rowWidget: Widget): void {
    for (let i: number = 0; i < rowWidget.childWidgets.length; i++) {
      let widget: TableCellWidget = rowWidget.childWidgets[
        i
      ] as TableCellWidget;
      this.renderTableCellWidget(page, widget);
      if (
        widget.isRenderBookmarkEnd &&
        this.documentHelper.owner.documentEditorSettings.showBookmarks
      ) {
        let lastPara: ParagraphWidget =
          this.documentHelper.selection.getLastParagraph(
        ...
        let position: Point =
          this.documentHelper.selection.getEndPosition(lastPara);
        this.renderBookmark(
          this.getScaledValue(position.x, 1),
          this.getScaledValue(position.y, 2),
          this.getScaledValue(lastLine.height - lastLine.margin.bottom),
          1
        );
      }
    }
  ...
  private renderTableCellWidget(page: Page, cellWidget: TableCellWidget): void {
    if (!this.isPrinting && !this.isExporting) {
      ...
      cellTopMargin =
        cellWidget.margin.top -
        (cellWidget.containerWidget as TableRowWidget).topBorderWidth;
      cellBottomMargin =
        cellWidget.margin.bottom -
        (cellWidget.containerWidget as TableRowWidget).bottomBorderWidth;
      if (
        this.getScaledValue(cellWidget.y, 2) +
          cellWidget.height +
          cellBottomMargin * this.documentHelper.zoomFactor <
          0 ||
        this.getScaledValue(cellWidget.y, 2) - cellTopMargin >
          this.documentHelper.visibleBounds.height
      ) {
        return;
      }
    ...
    for (let i: number = 0; i < cellWidget.childWidgets.length; i++) {
      ...
      let width: number =
        cellWidget.width +
        cellWidget.margin.left +
        cellWidget.margin.right -
        cellWidget.leftBorderWidth;
      if (!this.isPrinting) {
        this.clipRect(
          cellWidget.x - cellWidget.margin.left,
          cellWidget.y,
          this.getScaledValue(width),
          this.getScaledValue(this.height)
        );
      }
      this.renderWidget(page, widget);
      this.pageContext.restore();
    ...
    if (cellWidget.isRenderEditRangeStart) {
      let firstPara: ParagraphWidget =
        this.documentHelper.selection.getFirstParagraph(
          cellWidget
        ) as ParagraphWidget;
      ...
      if (
        this.documentHelper.owner.documentEditorSettings.highlightEditableRanges
      ) {
        let highlighters = this.documentHelper.selection.editRegionHighlighters;
        let widgetInfo: SelectionWidgetInfo[] = !isNullOrUndefined(highlighters)
          ? highlighters.get(firstLine)
          : [];
        let color: string =
          !isNullOrUndefined(widgetInfo) && !isNullOrUndefined(widgetInfo[0])
            ? widgetInfo[0].color
            : 'ffff00';
        this.renderBookmark(
          this.getScaledValue(xLeft, 1),
          this.getScaledValue(ytop, 2),
          this.getScaledValue(firstLine.height - firstLine.margin.bottom),
          0,
          color
        );
      }
    ...
    if (cellWidget.isRenderEditRangeEnd) {
      ...
      let position: Point =
        this.documentHelper.selection.getEndPosition(lastPara);
      let xLeft =
        this.documentHelper.textHelper.getWidth(
          String.fromCharCode(164),
          lastLine.paragraph.characterFormat
        ) + position.x;
      if (
        this.documentHelper.owner.documentEditorSettings.highlightEditableRanges
      ) {
  ...
  private checkHeaderFooterLineWidget(
    ...
    if (!isNullOrUndefined(headerFooter.parentHeaderFooter)) {
      ...
      for (let j: number = 0; j < keys.length; j++) {
        ...
        if (
          selectedWidget instanceof LineWidget &&
          widget instanceof LineWidget
        ) {
          let parentIndex: string = selectedWidget.getHierarchicalIndex('');
          let currentLineIndex: string = widget.getHierarchicalIndex('');
          if (
            selectedWidget.paragraph.isInHeaderFooter &&
            headerFooter === selectedWidget.paragraph.bodyWidget &&
            parentIndex.substring(parentIndex.indexOf(';')) ===
              currentLineIndex.substring(currentLineIndex.indexOf(';'))
          ) {
            return selectedWidget;
          }
        } else if (
          selectedWidget instanceof TableCellWidget &&
          widget instanceof TableCellWidget
        ) {
          let parentIndex: string = selectedWidget.getHierarchicalIndex('');
          let currentLineIndex: string = widget.getHierarchicalIndex('');
          if (
            selectedWidget.ownerTable.isInHeaderFooter &&
            headerFooter === selectedWidget.ownerTable.bodyWidget &&
            parentIndex.substring(parentIndex.indexOf(';')) ===
              currentLineIndex.substring(currentLineIndex.indexOf(';'))
          ) {
            return selectedWidget;
  ...
  private renderEditregionContentHighlight(
    ...
    top: number,
    contenControl: ContentControl
  ): void {
    if (
      !isNullOrUndefined(this.documentHelper.owner.editorModule) &&
      page.documentHelper.selection &&
      !isNullOrUndefined(
        page.documentHelper.selection.contentControleditRegionHighlighters
      )
    ) {
      if (!isNullOrUndefined(contenControl)) {
        let widgetInfo: Dictionary<LineWidget, SelectionWidgetInfo[]> =
          this.documentHelper.selection.contentControleditRegionHighlighters.get(
            contenControl
          );
        let totalHeight: number = top;
        if (
          !isNullOrUndefined(widgetInfo) &&
          lineWidget == contenControl.line
        ) {
          for (var i = 0; i < widgetInfo.length; i++) {
            ...
            let widget: SelectionWidgetInfo[] = widgetInfo.get(
              widgetInfo.keys[i]
            );
            ...
            for (var j = 0; j < widget.length; j++) {
              ...
              if (widgetInfo.length - 1 > i) {
                endX =
                  this.documentHelper.getParagraphLeftPosition(
                    lineWidget.paragraph
                  ) +
                  this.getContainerWidth(lineWidget.paragraph, page) +
                  HelperMethods.convertPointToPixel(
                    lineWidget.paragraph.paragraphFormat.borders.right.space
                  );
              }
              ...
              if (i > 0 && i < widgetInfo.length - 1) {
                ...
                //top
                let widgets: SelectionWidgetInfo[] = widgetInfo.get(
                  widgetInfo.keys[i - 1]
                );
                this.renderSingleBorder(
                  color,
              ...
              if (i == widgetInfo.length - 1 && widgetInfo.length > 1) {
                ...
                if (
                  !isNullOrUndefined(lastLine) &&
                  widgetInfo.keys[i] === lastLine &&
                  lastLine.children[lastLine.children.length - 1] instanceof
                    ContentControl &&
                  lastLine.children[lastLine.children.length - 1] ===
                    contenControl.reference &&
                  contenControl.contentControlWidgetType === 'Block'
                ) {
                  //bottom
                  endX =
                    this.documentHelper.getParagraphLeftPosition(
                      widgetInfo.keys[i].paragraph
                    ) +
                    this.getContainerWidth(widgetInfo.keys[i].paragraph, page) +
                    HelperMethods.convertPointToPixel(
                      widgetInfo.keys[i].paragraph.paragraphFormat.borders.right
                        .space
                    );
                  this.renderSingleBorder(
                    color,
              ...
              if (
                i == widgetInfo.length - 1 &&
                !isNullOrUndefined(contenControl) &&
                (contenControl.contentControlProperties.type === 'ComboBox' ||
                  contenControl.contentControlProperties.type ===
                    'DropDownList' ||
                  contenControl.contentControlProperties.type === 'Date')
              ) {
                ...
                if (element) {
                  ...
                  element.addEventListener(
                    'click',
                    this.documentHelper.owner.editor.contentControlDropDownChange.bind(
                      this
                    )
                  );
                } else {
  ...
  private renderEditRegionHighlight(
    ...
    if (
      page.documentHelper.selection &&
      !isNullOrUndefined(page.documentHelper.selection.editRegionHighlighters)
    ) {
      let renderHighlight: boolean =
        this.documentHelper.selection.editRegionHighlighters.containsKey(
          lineWidget
        );
      if (!renderHighlight && lineWidget.paragraph.isInHeaderFooter) {
        let keys: LineWidget[] =
          this.documentHelper.selection.editRegionHighlighters.keys;
        lineWidget = this.checkHeaderFooterLineWidget(
          lineWidget,
          keys
        ) as LineWidget;
      ...
      if (renderHighlight) {
        let widgetInfo: SelectionWidgetInfo[] =
          this.documentHelper.selection.editRegionHighlighters.get(lineWidget);
        for (let i: number = 0; i < widgetInfo.length; i++) {
          this.pageContext.fillStyle =
            widgetInfo[i].color !== '' ? widgetInfo[i].color : '#add8e6';
          this.pageContext.fillRect(
            this.getScaledValue(widgetInfo[i].left, 1),
            this.getScaledValue(top, 2),
            this.getScaledValue(widgetInfo[i].width),
            this.getScaledValue(lineWidget.height)
          );
        }
      }
    }
  ...
  private renderSearchHighlight(
    ...
    if (
      this.documentHelper.owner.searchModule &&
      !isNullOrUndefined(
        page.documentHelper.owner.searchModule.searchHighlighters
      )
    ) {
      let renderHighlight: boolean =
        page.documentHelper.owner.searchModule.searchHighlighters.containsKey(
          lineWidget
        );
      if (!renderHighlight && lineWidget.paragraph.isInHeaderFooter) {
        let keys: LineWidget[] =
          page.documentHelper.owner.searchModule.searchHighlighters.keys;
        lineWidget = this.checkHeaderFooterLineWidget(
          lineWidget,
          keys
        ) as LineWidget;
      ...
      if (renderHighlight) {
        let widgetInfo: SearchWidgetInfo[] =
          page.documentHelper.owner.searchModule.searchHighlighters.get(
            lineWidget
          );
        for (let i: number = 0; i < widgetInfo.length; i++) {
          this.pageContext.fillStyle =
            this.viewer.owner.documentEditorSettings.searchHighlightColor;
  ...
  private renderSelectionHighlight(
    ...
    if (
      !this.isPrinting &&
      page.documentHelper.owner.selectionModule &&
      !this.documentHelper.isScrollToSpellCheck &&
      page.documentHelper.owner.selectionModule.selectedWidgets.length > 0
    ) {
      let renderHighlight: boolean =
        page.documentHelper.owner.selectionModule.selectedWidgets.containsKey(
          lineWidget
        );
      if (!renderHighlight && lineWidget.paragraph.isInHeaderFooter) {
        let keys: IWidget[] =
          page.documentHelper.owner.selectionModule.selectedWidgets.keys;
        lineWidget = this.checkHeaderFooterLineWidget(
          lineWidget,
          keys
        ) as LineWidget;
      ...
      if (renderHighlight) {
        page.documentHelper.owner.selectionModule.addSelectionHighlight(
  ...
  private renderSelectionHighlightOnTable(
    ...
    if (
      ...
      if (!renderHighlight && cellWidget.ownerTable.isInHeaderFooter) {
        let keys: IWidget[] =
          page.documentHelper.owner.selectionModule.selectedWidgets.keys;
        cellWidget = this.checkHeaderFooterLineWidget(
          cellWidget,
          keys
        ) as TableCellWidget;
      ...
      if (renderHighlight) {
        page.documentHelper.owner.selectionModule.addSelectionHighlightTable(
          this.selectionContext,
          cellWidget,
          page
        );
  ...
  private renderLine(
    ...
    if (
      this.documentHelper &&
      this.documentHelper.selection &&
      !isNullOrUndefined(this.documentHelper.selection.formFieldHighlighters) &&
      this.documentHelper.selection.formFieldHighlighters.containsKey(
        lineWidget
      )
    ) {
      if (
        this.documentHelper.owner.documentEditorSettings &&
        this.documentHelper.owner.documentEditorSettings.formFieldSettings
          .applyShading
      ) {
        let widgetInfo: SelectionWidgetInfo[] =
          page.documentHelper.selection.formFieldHighlighters.get(lineWidget);
        for (let i: number = 0; i < widgetInfo.length; i++) {
          this.pageContext.fillStyle =
            this.documentHelper.owner.documentEditorSettings.formFieldSettings.shadingColor;
          ...
          if (isLastLine) {
            height =
              height -
              HelperMethods.convertPointToPixel(
                this.documentHelper.layout.getAfterSpacing(lineWidget.paragraph)
              );
            if (lineWidget.paragraph.paragraphFormat.lineSpacing > 1) {
              let formField: FieldElementBox =
                this.getFormfieldInLine(lineWidget);
              if (!isNullOrUndefined(formField)) {
                const sizeInfo: TextSizeInfo =
                  this.documentHelper.textHelper.getHeight(
                    formField.characterFormat
                  );
                let maxHeight: number = sizeInfo.Height;
                if (
                  lineWidget.paragraph.paragraphFormat.lineSpacingType ===
                  'Multiple'
                ) {
                  height =
                    height -
                    HelperMethods.convertPointToPixel(
                      this.documentHelper.layout.getLineSpacing(
                        lineWidget.paragraph,
                        maxHeight,
                        true
                      )
                  ...
                  top =
                    top +
                    HelperMethods.convertPointToPixel(
                      lineWidget.paragraph.paragraphFormat.beforeSpacing
                    );
    ...
    let bookmarks: any = [];
    if (
      !isNullOrUndefined(lineWidget.paragraph.associatedCell) &&
      lineWidget.paragraph.associatedCell.isRenderBookmarkStart
    ) {
      let firstLineInFirstPara: LineWidget = (
        lineWidget.paragraph.associatedCell.firstChild as ParagraphWidget
      ).firstChild as LineWidget;
    ...
    for (let i: number = 0; i < children.length; i++) {
      ...
      if (
        elementBox instanceof ContentControl &&
        elementBox.type === 0 &&
        isNullOrUndefined(contentControlstart) &&
        !isNullOrUndefined(this.documentHelper.owner.editorModule) &&
        !isNullOrUndefined(this.documentHelper.selection) &&
        !isNullOrUndefined(
          this.documentHelper.selection.contentControleditRegionHighlighters
        )
      ) {
        contentControlstart =
          this.documentHelper.owner.editorModule.getContentControl(elementBox);
      }
      if (
        elementBox instanceof ShapeBase &&
        elementBox.textWrappingStyle !== 'Inline'
      ) {
        continue;
      }
      if (
        elementBox instanceof CommentCharacterElementBox ||
        elementBox instanceof EditRangeStartElementBox
      ) {
        ...
        let topPosition: string =
          this.getScaledValue(
            top + (page.boundingRectangle.y - pageGap * (page.index + 1))
          ) +
          pageGap * (page.index + 1) +
          'px;';
        if (
          elementBox instanceof EditRangeStartElementBox &&
          (this.documentHelper.owner.currentUser === elementBox.user ||
            (elementBox.group === 'Everyone' && elementBox.user === ''))
        ) {
          if (
            this.documentHelper.owner.documentEditorSettings
              .highlightEditableRanges &&
            elementBox.columnFirst == -1
          ) {
            var height = elementBox.line.height - elementBox.line.margin.bottom;
            let xLeft = left;
            let yTop = top;
            let highlighters =
              this.documentHelper.selection.editRegionHighlighters;
            let widgetInfo: SelectionWidgetInfo[] = !isNullOrUndefined(
              highlighters
            )
              ? highlighters.get(lineWidget)
              : [];
            let color: string =
              !isNullOrUndefined(widgetInfo) &&
              !isNullOrUndefined(widgetInfo[0])
                ? widgetInfo[0].color
                : 'ffff00';
            this.renderBookmark(
              this.getScaledValue(xLeft, 1),
              this.getScaledValue(yTop, 2),
              this.getScaledValue(lineWidget.height - lineWidget.margin.bottom),
              0,
              color
            );
          }
          if (this.documentHelper.owner.enableLockAndEdit) {
            ...
            l10n.setLocale(this.documentHelper.owner.locale);
            elementBox.renderLockMark(
              this.documentHelper.owner.currentUser,
              l10n
            );

            let settings: CollaborativeEditingSettingsModel =
              this.documentHelper.owner.documentEditorSettings
                .collaborativeEditingSettings;
            let color: string =
              elementBox.user === this.documentHelper.owner.currentUser
                ? settings.editableRegionColor
                : settings.lockedRegionColor;
            style += `color:${color};`;
            let leftMargin: number = HelperMethods.convertPointToPixel(
              page.bodyWidgets[0].sectionFormat.leftMargin
            );
            let leftPosition: string =
              page.boundingRectangle.x +
              this.getScaledValue(leftMargin - 20) +
              'px;';
            if (this.viewer instanceof WebLayoutViewer) {
          ...
          elementBox instanceof CommentCharacterElementBox &&
          elementBox.commentType === 0 &&
          this.documentHelper.owner.selectionModule
        ) {
          if (this.documentHelper.owner.enableComment) {
            let rightMargin: number = HelperMethods.convertPointToPixel(
              page.bodyWidgets[0].sectionFormat.rightMargin
            );
            let pageWidth: number = HelperMethods.convertPointToPixel(
              page.bodyWidgets[0].sectionFormat.pageWidth
            );

            let leftPosition: string =
              page.boundingRectangle.x +
              this.getScaledValue(pageWidth - rightMargin + rightMargin / 4) +
              'px;';
            if (this.viewer instanceof WebLayoutViewer) {
              leftPosition =
                page.boundingRectangle.width -
                this.viewer.padding.right * 2 -
                this.viewer.padding.left * 2 +
                'px;';
            }
            style = style + 'left:' + leftPosition + 'top:' + topPosition;
            if (
              commentIDList.indexOf(elementBox.commentId) === -1 &&
              !isNullOrUndefined(elementBox.comment) &&
              !elementBox.comment.isReply
            ) {
      ...
      if (
        elementBox instanceof EditRangeEndElementBox &&
        (this.documentHelper.owner.currentUser ===
          elementBox.editRangeStart.user ||
          (elementBox.editRangeStart.group === 'Everyone' &&
            elementBox.editRangeStart.user === ''))
      ) {
        if (
          elementBox.editRangeStart.columnFirst == -1 &&
          this.documentHelper.owner.documentEditorSettings
            .highlightEditableRanges
        ) {
          var height = elementBox.line.height - elementBox.line.margin.bottom;
      ...
      if (
        elementBox instanceof BookmarkElementBox &&
        this.documentHelper.owner.documentEditorSettings.showBookmarks &&
        this.documentHelper.getBookmarks().indexOf(elementBox.name) !== -1
      ) {
        var height = elementBox.line.height - elementBox.line.margin.bottom;
        let xLeft = left;
        let yTop = top;
        if (elementBox.bookmarkType == 1) {
          if (
            this.isBookmarkEndAtStart(elementBox) &&
            isNullOrUndefined(elementBox.properties)
          ) {
            ...
            while (
              !isRenderablePresent &&
              !isNullOrUndefined(previousParaElement)
            ) {
              let lineIndex: number =
                previousParaElement.childWidgets.indexOf(elementBox.line) >= 0
                  ? previousParaElement.childWidgets.indexOf(elementBox.line)
                  : previousParaElement.childWidgets.length - 1;
              for (let i = lineIndex; i >= 0; i--) {
                let line: LineWidget = previousParaElement.childWidgets[
                  i
                ] as LineWidget;
                let elementIndex: number =
                  line.children.indexOf(elementBox) >= 0
                    ? line.children.indexOf(elementBox)
              ...
              if (!isRenderablePresent) {
                previousParaElement =
                  this.documentHelper.selection.getPreviousParagraphBlock(
                    previousParaElement
                  );
              }
            }
            if (!isNullOrUndefined(prevRenderableElement)) {
              xLeft +=
                this.documentHelper.selection.getWidth(
                  prevRenderableElement.line,
                  false
                ) +
                this.documentHelper.textHelper.getParagraphMarkWidth(
                  elementBox.line.paragraph.characterFormat
                );
              yTop = this.documentHelper.selection.getTop(
                prevRenderableElement.line
              );
            }
          }
          if (!isNullOrUndefined(elementBox.properties)) {
            if (elementBox.properties['isAfterParagraphMark']) {
              xLeft += this.documentHelper.textHelper.getParagraphMarkWidth(
                elementBox.line.paragraph.characterFormat
              );
            }
            if (elementBox.properties['isAfterCellMark']) {
              xLeft += this.documentHelper.textHelper.getWidth(
                String.fromCharCode(164),
        ...
        if (elementBox.bookmarkType == 1) {
          if (!isNullOrUndefined(elementBox.reference)) {
            if (isNullOrUndefined(elementBox.reference.properties)) {
              if (
                !this.documentHelper.selection.isRenderBookmarkAtEnd(elementBox)
              ) {
                bookmarks.push({
                  x: xLeft,
                  y: yTop,
              ...
              let bookmarkEndCell: TableCellWidget =
                elementBox.paragraph.associatedCell;
      ...
      if (
        elementBox instanceof FieldElementBox ||
        this.isFieldCode ||
      ...
      if (!this.isPrinting) {
        if (
          this.getScaledValue(top + elementBox.margin.top, 2) +
            elementBox.height * this.documentHelper.zoomFactor <
            0 ||
          this.getScaledValue(top + elementBox.margin.top, 2) >
            this.documentHelper.visibleBounds.height
        ) {
          ...
          if (elementBox instanceof TextElementBox) {
            ...
            if (
              !elementBox.isSpellChecked ||
              elementBox.line.paragraph.isChangeDetected
            ) {
      ...
      if (elementBox instanceof ListTextElementBox) {
        ...
        left += elementBox.padding.left;
        this.renderTextElementBox(
          elementBox as TextElementBox,
          left,
          top,
          underlineY
        );
    ...
    this.renderEditregionContentHighlight(
      page,
      lineWidget,
      top,
      contentControlstart
    ...
    if (
      ...
      for (let i = 0; i < bookmarks.length; i++) {
        ...
        this.renderBookmark(
          this.getScaledValue(bookmark.x, 1),
          this.getScaledValue(bookmark.y, 2),
          this.getScaledValue(bookmark.height),
          bookmark.type
        );
      }
    }
    if (
      this.documentHelper.owner.documentEditorSettings.showHiddenMarks &&
      !this.isPrinting
    ) {
      let text: string = '';
      let currentCharFormat: WCharacterFormat =
        lineWidget.paragraph.characterFormat;
      let y: number = 0;
      let characterFont: string =
        this.retriveCharacterformat(currentCharFormat);
      let l10n: L10n = new L10n(
        'documenteditor',
        this.documentHelper.owner.defaultLocale
      );
      l10n.setLocale(this.documentHelper.owner.locale);
      this.pageContext.fillStyle = HelperMethods.getColor(
        currentCharFormat.fontColor
      );
      let pageIndex: number = 0;
      if (
        lineWidget.paragraph.bodyWidget &&
        !(lineWidget.paragraph.bodyWidget instanceof HeaderFooterWidget)
      ) {
        pageIndex = this.documentHelper.pages.indexOf(
          lineWidget.paragraph.bodyWidget.page
        );
      }
      if (
        (children.length == 0 &&
          !lineWidget.isEndsWithLineBreak &&
          !isNullOrUndefined(lineWidget.paragraph)) ||
        lineWidget.paragraph.childWidgets.length === 1
      ) {
        y =
          lineWidget.paragraph.y +
          this.documentHelper.textHelper.getHeight(currentCharFormat)
            .BaselineOffset +
          this.documentHelper.layout.getBeforeSpacing(
            lineWidget.paragraph,
            pageIndex
          );
      ...
      if (currentCharFormat.revisions.length > 0) {
        ...
        let color: string = this.documentHelper.authors.get(
          currentCharFormat.revisions[0].author
        );
      ...
      if (lineWidget.isEndsWithColumnBreak) {
        ...
        let textinfo: number = this.documentHelper.textHelper.getWidth(
          text,
          currentCharFormat,
        ...
        if (this.viewer instanceof PageLayoutViewer) {
          ...
          if (lineWidget.paragraph.bidi) {
            ...
            if (
              x + textinfo >
              this.viewer.clientActiveArea.width +
                this.viewer.clientActiveArea.x
            ) {
              text = '.....';
            }
          }
        ...
        text = lineWidget.paragraph.bidi
          ? String.fromCharCode(8627)
          : String.fromCharCode(8629);
      } else if (
        lineWidget.paragraph.isInsideTable &&
        isNullOrUndefined(lineWidget.nextLine) &&
        isNullOrUndefined(lineWidget.paragraph.nextWidget)
      ) {
      ...
      if (
        lineWidget.paragraph.containerWidget instanceof BodyWidget &&
        !isNullOrUndefined(lineWidget.paragraph.nextRenderedWidget)
      ) {
        if (
          lineWidget.paragraph.containerWidget.sectionIndex !==
            (
              lineWidget.paragraph.nextRenderedWidget
                .containerWidget as BodyWidget
            ).sectionIndex &&
          lineWidget.isLastLine()
        ) {
          if (
            (
              lineWidget.paragraph.nextRenderedWidget
                .containerWidget as BodyWidget
            ).sectionFormat.breakCode === 'NoBreak' &&
            lineWidget.paragraph.containerWidget.index !==
              (
                lineWidget.paragraph.nextRenderedWidget
                  .containerWidget as BodyWidget
              ).index
            ...
            text =
              ':::::' + l10n.getConstant('Section Break Continuous') + ':::::';
          } else {
            text =
              ':::::' + l10n.getConstant('Section Break Next Page') + ':::::';
          }
          characterFont = this.retriveCharacterformat(currentCharFormat, 0.7);
          ...
          if (this.viewer instanceof PageLayoutViewer) {
            if (lineWidget.paragraph.bidi) {
              ...
              if (
                x + textinfo - this.viewer.clientActiveArea.x >
                this.viewer.clientActiveArea.width
              ) {
                text = ':::::';
                if (lineWidget.isEndsWithPageBreak) {
            ...
            if (lineWidget.paragraph.bidi) {
              ...
              if (
                x +
                  textinfo -
                  this.documentHelper.pages[0].boundingRectangle.x >
                this.viewer.clientActiveArea.width
              ) {
                text = ':::::';
                if (lineWidget.isEndsWithPageBreak) {
      ...
      if (text.length > 0) {
        if (lineWidget.paragraph.bidi && !lineWidget.paragraph.isEmpty()) {
          x -= this.documentHelper.textHelper.getWidth(
            text,
            currentCharFormat,
            FontScriptType.English
          );
        ...
        let scaleFactor: number =
          currentCharFormat.scaling < 100 ? 1 : currentCharFormat.scaling / 100;
        (this.pageContext as any).letterSpacing =
          currentCharFormat.characterSpacing * this.documentHelper.zoomFactor +
          'pt';
        this.pageContext.save();
        this.pageContext.scale(scaleFactor, 1);
        this.pageContext.fillText(
          text,
          this.getScaledValue(x, 1) / scaleFactor,
          this.getScaledValue(y, 2)
        );
        this.pageContext.restore();
      }
    }
  ...
  private isBookmarkEndAtStart(bookmark: BookmarkElementBox): boolean {
    ...
    let bookmarkIndex: number =
      firstLine.children.indexOf(bookmark) >= 0
        ? firstLine.children.indexOf(bookmark)
        : -1;
  ...
  private isRenderable(element: any): boolean {
    if (
      !(
        element instanceof BookmarkElementBox ||
  ...
  private retriveCharacterformat(
    ...
    fontSize =
      character.fontSize === 0
        ? 0.5
        : character.fontSize /
          (character.baselineAlignment === 'Normal' ? 1 : 1.5);
  ...
  private toSkipFieldCode(element: ElementBox): void {
    if (element instanceof FieldElementBox) {
      if (element.fieldType === 0) {
        if (
          !this.isFieldCode &&
          (!isNullOrUndefined(element.fieldEnd) || element.hasFieldEnd)
        ) {
        ...
        if (
          this.fieldStacks.length > 0 &&
          element.fieldBegin === this.fieldStacks[this.fieldStacks.length - 1]
        ) {
  ...
  public getUnderlineYPosition(lineWidget: LineWidget): number {
    ...
    for (let i: number = 0; i < lineWidget.children.length; i++) {
      if (
        lineWidget.children[i] instanceof FieldElementBox ||
        (lineWidget.children[i].width === 0 &&
          lineWidget.children[i].height === 0)
      ) {
        continue;
      }
      if (lineWidget.children[i] instanceof ShapeElementBox) {
        ...
        if (
          height <
          lineWidget.children[i].height + lineWidget.children[i].margin.top
        ) {
          height =
            lineWidget.children[i].margin.top + lineWidget.children[i].height;
          lineHeight =
            lineWidget.children[i] instanceof ImageElementBox
              ? 0.9
              : lineWidget.children[i].height / 20;
        }
      }
    }
  ...
  private renderListTextElementBox(
    ...
    let breakCharacterFormat: WCharacterFormat =
      elementBox.line.paragraph.characterFormat;
    let color: string = format.hasValue('fontColor')
      ? format.fontColor
      : breakCharacterFormat.fontColor;
    this.pageContext.textBaseline = 'alphabetic';
    let bold: string = '';
    let italic: string = '';
    let fontFamily: string = format.hasValue('fontFamily')
      ? format.fontFamily
      : breakCharacterFormat.fontFamily;
    if (
      this.documentHelper.isIosDevice &&
      (elementBox.text === String.fromCharCode(9679) ||
        elementBox.text === String.fromCharCode(9675))
    ) {
      fontFamily = '';
    }
    let fontSize: number = format.hasValue('fontSize')
      ? format.fontSize
      : breakCharacterFormat.fontSize;

    let baselineAlignment: BaselineAlignment = format.hasValue(
      'baselineAlignment'
    )
      ? format.baselineAlignment
      : breakCharacterFormat.baselineAlignment;
    bold = format.hasValue('bold')
      ? format.bold
        ? 'bold'
        : ''
      : breakCharacterFormat.bold
      ? 'bold'
      : '';
    italic = format.hasValue('italic')
      ? format.italic
        ? 'italic'
        : ''
      : breakCharacterFormat.italic
      ? 'italic'
      : '';
    fontSize =
      fontSize === 0
        ? 0.5
        : fontSize / (baselineAlignment === 'Normal' ? 1 : 1.5);
    fontSize = this.isPrinting
      ? fontSize
      : fontSize * this.documentHelper.zoomFactor;
    let strikethrough: Strikethrough = format.hasValue('strikethrough')
      ? format.strikethrough
      : breakCharacterFormat.strikethrough;
    let highlightColor: HighlightColor = format.hasValue('highlightColor')
      ? format.highlightColor
      : breakCharacterFormat.highlightColor;
    if (highlightColor !== 'NoColor') {
      if (highlightColor.substring(0, 1) !== '#') {
        this.pageContext.fillStyle =
          HelperMethods.getHighlightColorCode(highlightColor);
      } else {
        this.pageContext.fillStyle = HelperMethods.getColor(highlightColor);
      }

      this.pageContext.fillRect(
        Math.floor(this.getScaledValue(left + leftMargin, 1)),
        Math.floor(this.getScaledValue(top + topMargin, 2) - 1),
        Math.ceil(this.getScaledValue(elementBox.width) + 1),
        Math.ceil(this.getScaledValue(elementBox.height) + 1)
      );
    }
    this.pageContext.font =
      bold +
      ' ' +
      italic +
      ' ' +
    ...
    topMargin =
      format.baselineAlignment === 'Normal'
        ? topMargin + baselineOffset
        : topMargin + baselineOffset / 1.5;
    ...
    if (
      ...
      if (
        elementBox instanceof ListTextElementBox &&
        elementBox.text === '\t'
      ) {
        this.tabMark(elementBox, format, left, top, leftMargin, topMargin);
      }
      (this.pageContext as any).letterSpacing =
        format.characterSpacing * this.documentHelper.zoomFactor + 'pt';
      this.pageContext.save();
      this.pageContext.scale(scaleFactor, 1);
      this.pageContext.fillText(
        this.replaceSpace(text),
        this.getScaledValue(left + leftMargin, 1) / scaleFactor,
        this.getScaledValue(top + topMargin, 2),
        this.getScaledValue(elementBox.width)
      );
      this.pageContext.restore();
    } else {
      (this.pageContext as any).letterSpacing =
  ...
  private renderTextElementBox(
    ...
    if (containerWidget instanceof TableCellWidget) {
      isHeightType =
        (containerWidget as TableCellWidget).ownerRow.rowFormat.heightType ===
        'Exactly';
    }
    let topMargin: number = elementBox.margin.top;
    let leftMargin: number = elementBox.margin.left;
    if (isHeightType && containerWidget instanceof TableCellWidget) {
      let width: number =
        ...
        containerWidget.margin.left +
        containerWidget.margin.right -
        containerWidget.leftBorderWidth;
      this.clipRect(
        containerWidget.x - containerWidget.margin.left,
        containerWidget.y,
        this.getScaledValue(width),
        this.getScaledValue(containerWidget.height)
      );
    }
    ...
    if (format.highlightColor != 'NoColor' && elementBox.text.trim() == '') {
      ...
      if (
        !isNullOrUndefined(firstLine) &&
        firstLine instanceof LineWidget &&
        firstLine.children.length > 0 &&
        elementBox === firstLine.children[0]
      ) {
        doHighLight = false;
      }
      if (
        !isNullOrUndefined(lastLine) &&
        lastLine instanceof LineWidget &&
        lastLine.children.length > 0 &&
        elementBox === lastLine.children[lastLine.children.length - 1]
      ) {
        doHighLight = false;
      }
    }
    if (format.highlightColor !== 'NoColor' && doHighLight) {
      if (format.highlightColor.substring(0, 1) !== '#') {
        this.pageContext.fillStyle = HelperMethods.getHighlightColorCode(
          format.highlightColor
        );
      } else {
        this.pageContext.fillStyle = HelperMethods.getColor(
    ...
    let color: string =
      revisionInfo.length > 0
        ? this.getRevisionColor(revisionInfo)
        : format.fontColor;
    this.pageContext.textBaseline = 'alphabetic';
    let bold: string = '';
    let italic: string = '';
    ...
    fontSize =
      format.fontSize === 0
        ? 0.5
        : format.fontSize / (format.baselineAlignment === 'Normal' ? 1 : 1.5);
    fontSize = this.isPrinting
      ? fontSize
      : fontSize * this.documentHelper.zoomFactor;
    let renderFontFamily: string =
      this.documentHelper.textHelper.getFontNameToRender(
        elementBox.scriptType,
        format
      );
    this.pageContext.font =
      bold +
    ...
    if (elementBox instanceof TabElementBox) {
      ...
      if (
        tabElement.tabText === '' &&
        !isNullOrUndefined(tabElement.tabLeader) &&
        tabElement.tabLeader !== 'None'
      ) {
    ...
    let isRTL: boolean =
      format.bidi || this.documentHelper.textHelper.isRTLText(elementBox.text);
    text = this.documentHelper.textHelper.setText(
      text,
      isRTL,
      format.bdo,
      true
    ...
    if (
      (characterRange == CharacterRangeType.WordSplit ||
        ((characterRange & CharacterRangeType.WordSplit) ==
          CharacterRangeType.WordSplit &&
          (characterRange & CharacterRangeType.RightToLeft) ==
            CharacterRangeType.RightToLeft)) &&
      format.bidi
    ) {
    ...
    if (
      ...
      if (
        (elementBox instanceof TabElementBox ||
          elementBox instanceof TextElementBox) &&
        elementBox.text === '\t'
      ) {
        this.tabMark(elementBox, format, left, top, leftMargin, topMargin);
      }
      ...
      if (
        ((elementBox.paragraph.containerWidget as TextFrame)
          .containerShape as ShapeElementBox) &&
        (elementBox.paragraph.containerWidget as TextFrame)
        ...
        let y: number =
          this.getScaledValue(top + topMargin, 2) +
          this.getScaledValue(shapeHeight) / 4.5;
        let height: number = 0;
        if (
          elementBox.paragraph.containerWidget.childWidgets &&
          elementBox.paragraph.containerWidget.childWidgets.length > 0
        ) {
          for (
            let i = 0;
            i < elementBox.paragraph.containerWidget.childWidgets.length;
            i++
          ) {
            height =
              height +
        ...
        if (
          height + this.getScaledValue(shapeHeight) / 2 <
          this.getScaledValue(shapeHeight)
        ) {
          this.pageContext.fillText(
            text,
            this.getScaledValue(left + leftMargin, 1) / scaleFactor,
        ...
        let y: number =
          this.getScaledValue(top + topMargin, 2) +
          this.getScaledValue(shapeHeight) / 2;
        let height: number = 0;
        if (
          elementBox.paragraph.containerWidget.childWidgets &&
          elementBox.paragraph.containerWidget.childWidgets.length > 0
    ...
    if (this.documentHelper.owner.isSpellCheck) {
      if (
        ...
        !this.spellChecker.removeUnderline &&
        (this.documentHelper.triggerSpellCheck || elementBox.canTrigger) &&
        elementBox.text !== ' ' &&
        !this.documentHelper.isScrollHandler &&
        (isNullOrUndefined(elementBox.previousNode) ||
          !(elementBox.previousNode instanceof FieldElementBox))
      ) {
        elementBox.canTrigger = true;
        this.leftPosition = this.pageLeft;
        this.topPosition = this.pageTop;
        let errorDetails: ErrorInfo =
          this.spellChecker.checktextElementHasErrors(
            elementBox.text,
            elementBox,
            left
          );
        if (errorDetails.errorFound && !this.isPrinting) {
          ...
          let backgroundColor: string =
            containerWidget instanceof TableCellWidget
              ? (containerWidget as TableCellWidget).cellFormat.shading
                  .backgroundColor
              : this.documentHelper.backgroundColor;
          ...
          for (let i: number = 0; i < errorDetails.elements.length; i++) {
            ...
            const exactText = this.spellChecker.manageSpecialCharacters(
              currentElement.text,
              undefined,
              true
            );
            if (elementBox.ignoreOnceItems.indexOf(exactText) === -1) {
              if (isRTL) {
                this.renderWavyLine(
                  currentElement,
                  isNullOrUndefined(currentElement.end)
                    ? left
                    : currentElement.end.location.x,
                  top,
                  underlineY,
                  color,
                  'Single',
                ...
                this.renderWavyLine(
                  currentElement,
                  isNullOrUndefined(currentElement.start)
                    ? left
                    : currentElement.start.location.x,
                  top,
                  underlineY,
                  color,
                  'Single',
          ...
          elementBox.ischangeDetected ||
          this.documentHelper.triggerElementsOnLoading
        ) {
          elementBox.ischangeDetected = false;
          this.handleChangeDetectedElements(
            elementBox,
            underlineY,
            left,
            top,
    ...
    if (
      (format.underline !== 'None' && !isNullOrUndefined(format.underline)) ||
      (!isNullOrUndefined(currentInfo) &&
        (currentInfo.type === 'Insertion' || currentInfo.type === 'MoveTo'))
    ) {
      this.renderUnderline(
        elementBox,
        left,
    ...
    if (
      (format.strikethrough !== 'None' &&
        !isNullOrUndefined(format.strikethrough)) ||
      (!isNullOrUndefined(currentInfo) &&
        (currentInfo.type === 'Deletion' || currentInfo.type === 'MoveFrom'))
    ) {
      this.renderStrikeThrough(
        elementBox,
        left,
  ...
  private tabMark(
    ...
    let tabMarkString: string = elementBox.paragraph.bidi
      ? String.fromCharCode(8592)
      : String.fromCharCode(8594);
    let tabMarkWidth: number = this.documentHelper.textHelper.getWidth(
      tabMarkString,
      format,
      elementBox.scriptType
    );
    ...
    this.pageContext.font = this.retriveCharacterformat(
      elementBox.characterFormat
    );
    (this.pageContext as any).letterSpacing =
      format.characterSpacing * this.documentHelper.zoomFactor + 'pt';
    this.pageContext.save();
    ...
    if (availableSpaceWidth > tabMarkWidth) {
      this.pageContext.fillText(
        tabMarkString,
        this.getScaledValue(
          left + leftMargin + availableSpaceWidth - tabWidth,
          1
        ) / scaleFactor,
        this.getScaledValue(top + topMargin, 2)
      );
  ...
  private replaceSpace(text: string) {
    text = text.replace(
      new RegExp(String.fromCharCode(32), 'g'),
  ...
  private getBackgroundColorHeirachy(element: ElementBox): string {
    ...
    if (element.paragraph.isInsideTable) {
      let cell: TableCellWidget = element.paragraph
        .containerWidget as TableCellWidget;
      ...
      element.paragraph.containerWidget instanceof TextFrame &&
      !isNullOrUndefined(
        element.paragraph.containerWidget.containerShape as ShapeElementBox
      ) &&
      !isNullOrUndefined(
        (element.paragraph.containerWidget.containerShape as ShapeElementBox)
          .fillFormat
  ...
  private handleChangeDetectedElements(
    ...
    if (
      !this.spellChecker.checkElementCanBeCombined(
        elementBox,
        underlineY,
        beforeIndex,
        true
      ...
      if (splittedText.length > 1) {
        for (let i: number = 0; i < splittedText.length; i++) {
          ...
          let retrievedText: string = this.spellChecker.manageSpecialCharacters(
            currentText,
            undefined,
            true
          );
          ...
          if (
            this.spellChecker.ignoreAllItems.indexOf(retrievedText) === -1 &&
            elementBox.ignoreOnceItems.indexOf(retrievedText) === -1
          ) {
            this.handleUnorderedElements(
              retrievedText,
              elementBox,
              underlineY,
              i,
        ...
        if (checkText.length > 0) {
          if (
            ...
            let indexinParagraph: number =
              elementBox.line.paragraph.indexInOwner;
            let spellInfo: WordSpellInfo =
              this.spellChecker.checkSpellingInPageInfo(retrievedText);
            if (
              spellInfo.isElementPresent &&
              this.spellChecker.enableOptimizedSpellCheck
            ) {
              let jsonObject: any = JSON.parse(
                '{"HasSpellingError":' + spellInfo.hasSpellError + '}'
              );

              this.spellChecker.handleWordByWordSpellCheck(
                jsonObject,
                elementBox,
                left,
                top,
              ...
              if (this.spellChecker.isInUniqueWords(retrievedText)) {
                let hasSpellingError: boolean = this.spellChecker.isErrorWord(
                  retrievedText
                )
                  ? true
                  : false;
                ...
                !this.documentHelper.owner.editorModule.triggerPageSpellCheck ||
                this.documentHelper.triggerElementsOnLoading
              ) {
                /* eslint-disable @typescript-eslint/no-explicit-any */
                this.spellChecker
                  .callSpellChecker(
                    this.spellChecker.languageID,
                    checkText,
                    true,
                    this.spellChecker.allowSpellCheckAndSuggestion
                  )
                  .then((data: any) => {
                    /* eslint-disable @typescript-eslint/no-explicit-any */
                    ...
                    if (!isNullOrUndefined(this.spellChecker)) {
                      let canUpdate: boolean =
                        (beforeIndex === this.pageIndex ||
                          elementBox.isVisible) &&
                        indexInLine === elementBox.indexInOwner &&
                        indexinParagraph ===
                          elementBox.line.paragraph.indexInOwner;
                      this.spellChecker.handleWordByWordSpellCheck(
                        jsonObject,
  ...
  public handleUnorderedElements(
    currentText: string,
    elementBox: TextElementBox,
    underlineY: number,
    iteration: number,
    ...
    if (currentText.length > 0) {
      let spellInfo: WordSpellInfo =
        this.spellChecker.checkSpellingInPageInfo(currentText);
      if (
        spellInfo.isElementPresent &&
        this.spellChecker.enableOptimizedSpellCheck
      ) {
        ...
        this.spellChecker.handleSplitWordSpellCheck(
          jsonObject,
          currentText,
          elementBox,
          true,
        ...
        let canUpdate: boolean =
          elementBox.isVisible &&
          indexInLine === elementBox.indexInOwner &&
          indexinParagraph === elementBox.line.paragraph.indexInOwner;
  ...
  public renderWavyLine(
    ...
    if (elementBox.text.length > 1) {
      ...
      let specialCharacter: SpecialCharacterInfo =
        this.spellChecker.getSpecialCharactersInfo(elementBox);

      let whiteSpaceData: SpaceCharacterInfo =
        this.spellChecker.getWhiteSpaceCharacterInfo(elementBox);

      let x: number =
        left +
        specialCharacter.beginningWidth +
        (whiteSpaceData.isBeginning ? whiteSpaceData.width : 0) +
        elementBox.margin.left;
      ...
      let x2: number =
        ...
        this.getScaledValue(
          elementBox.width -
            (specialCharacter.beginningWidth + specialCharacter.endWidth) -
  ...
  public drawWavy(
    ...
    this.pageContext.save();
    this.pageContext.fillStyle = !isNullOrUndefined(backColor)
      ? backColor
      : this.documentHelper.backgroundColor;
    this.pageContext.fillRect(
      from.x,
  ...
  public getTabLeader(elementBox: TabElementBox): string {
    ...
    textWidth = this.documentHelper.textHelper.getWidth(
      tabText,
      elementBox.characterFormat,
      elementBox.scriptType
    );
  ...
  private getTrimmedWidth(elementBox: ElementBox): number {
    ...
    if (
      elementBox instanceof TextElementBox &&
      !(elementBox instanceof TabElementBox) &&
      isNullOrUndefined(elementBox.nextNode)
    ) {
      width = this.documentHelper.textHelper.getWidth(
        HelperMethods.trimEnd(elementBox.text),
        elementBox.characterFormat,
        elementBox.scriptType
      );
    }
  ...
  private renderUnderline(
    ...
    while (lineCount < (underline === 'Double' ? 2 : 1)) {
      ...
      this.pageContext.fillRect(
        this.getScaledValue(left + elementBox.margin.left, 1),
        this.getScaledValue(y, 2),
        this.getScaledValue(width),
        this.getScaledValue(underlineHeight)
      );
      y += 2 * lineHeight;
    }
  }
  ...
  private renderStrikeThrough(
    ...
    if (!isNullOrUndefined(revisionInfo)) {
      strikethrough =
        revisionInfo.type === 'Deletion' ? 'SingleStrike' : 'DoubleStrike';
    }
    if (elementBox instanceof ImageElementBox) {
      lineHeight = 0.9;
    }
    ...
    while (lineCount < (strikethrough === 'DoubleStrike' ? 2 : 1)) {
      ...
      this.pageContext.fillRect(
        this.getScaledValue(left + elementBox.margin.left, 1),
        this.getScaledValue(y + top, 2),
        this.getScaledValue(width),
        this.getScaledValue(lineHeight)
      );
      y += 2 * lineHeight;
    }
  }
  ...
  private renderImageElementBox(
    ...
    let revisionInfo: RevisionInfo[] = this.checkRevisionType(elementBox);
    let color: string =
      revisionInfo.length > 0 ? this.getRevisionColor(revisionInfo) : 'black';
    ...
    if (containerWid instanceof TableCellWidget) {
      isHeightType =
        (containerWid as TableCellWidget).ownerRow.rowFormat.heightType ===
        'Exactly';
    }
      ...
      if (topMargin < 0 || elementBox.line.paragraph.width < elementBox.width) {
        ...
        if (containerWid instanceof TableCellWidget) {
          ...
          if (containerWid.childWidgets[0] instanceof ParagraphWidget) {
            let paraAdv: ParagraphWidget = containerWid
              .childWidgets[0] as ParagraphWidget;
            leftIndent = paraAdv.paragraphFormat.leftIndent;
          }
          widgetWidth =
            containerWid.width +
            containerWid.margin.left -
            containerWid.leftBorderWidth -
            leftIndent;
          isClipped = true;
          ...
          this.clipRect(
            left + leftMargin,
            top + topMargin,
            this.getScaledValue(widgetWidth),
            this.getScaledValue(containerWid.height)
          );
        }
        ...
        let width: number =
          containerWid.width +
          containerWid.margin.left -
          (containerWid as TableCellWidget).leftBorderWidth;
        isClipped = true;
    ...
    if (
      elementBox.isMetaFile &&
      !isNullOrUndefined(elementBox.metaFileImageString)
    ) {
      this.pageContext.drawImage(
        elementBox.element,
        this.getScaledValue(left + leftMargin, 1),
        this.getScaledValue(top + topMargin, 2),
        this.getScaledValue(elementBox.width),
      ...
      try {
        ...
        elementBox.element.onload = (): void => {
          const lastLoaded: string | null =
            elementBox.element.getAttribute('lastLoaded');

          if (
            !isNullOrUndefined(render.documentHelper) &&
            lastLoaded !== elementBox.element.src
          ) {
            if (!elementBox.isCrop) {
              render.pageContext.drawImage(
                elementBox.element,
                imgX,
                imgY,
                width,
        ...
        elementBox.element.onerror = (): void => {
          ...
          let imageString: string =
            this.documentHelper.getImageString(elementBox);
          if (
            imageString &&
            (HelperMethods.startsWith(imageString, 'http://') ||
              HelperMethods.startsWith(imageString, 'https://'))
          ) {
            let imageStrings: string[] = this.viewer.documentHelper.images.get(
              parseInt(elementBox.imageString)
            );
    ...
    this.pageContext.fillStyle = HelperMethods.getColor(color);
    let currentRevision: RevisionInfo = this.getRevisionType(
      revisionInfo,
      false
    );

    if (
      !isNullOrUndefined(currentRevision) &&
      (currentRevision.type === 'Deletion' ||
        currentRevision.type === 'MoveFrom')
    ) {
      this.renderStrikeThrough(
        elementBox,
        left,
    ...
    if (
      !isNullOrUndefined(currentRevision) &&
      (currentRevision.type === 'Insertion' ||
        currentRevision.type === 'MoveTo')
    ) {
      let y: number = this.getUnderlineYPosition(elementBox.line);
      this.renderUnderline(elementBox, left, top, y, color, 'Single', 'Normal');
    }
    this.documentHelper.owner.picturePositionY = this.getScaledValue(
      top + topMargin,
      2
    );
    if (
      !isNullOrUndefined(this.documentHelper.selection) &&
      this.documentHelper.selection.checkContentControlLocked() &&
      this.documentHelper.selection.start.currentWidget === elementBox.line
    ) {
      let contentControlImage: ElementBox =
        this.documentHelper.owner.getImageContentControl();
      if (
        !isNullOrUndefined(contentControlImage) &&
        contentControlImage.contentControlProperties.type == 'Picture'
      ) {
        let format: WCharacterFormat = elementBox.characterFormat;
  ...
  private renderTableOutline(tableWidget: TableWidget): void {
    ...
    let border: WBorder = !table.isBidiTable
    ...
    this.renderSingleBorder(
      border.color,
      tableWidget.x - tableWidget.margin.left - lineWidth / 2,
      tableWidget.y,
      tableWidget.x - tableWidget.margin.left - lineWidth / 2,
      tableWidget.y + tableWidget.height,
      lineWidth,
      border.lineStyle
    );
    // }
    ...
    this.renderSingleBorder(
      border.color,
      tableWidget.x - tableWidget.margin.left - lineWidth,
      tableWidget.y - lineWidth / 2,
      tableWidget.x + tableWidget.width + lineWidth + tableWidget.margin.right,
      tableWidget.y - lineWidth / 2,
      lineWidth,
      border.lineStyle
    );
    // }
    border = !table.isBidiTable
      ? layout.getTableRightBorder(table.tableFormat.borders)
      : layout.getTableLeftBorder(table.tableFormat.borders);
    lineWidth = 0;
    // if (!isNullOrUndefined(border )) {
    ...
    this.renderSingleBorder(
      border.color,
      tableWidget.x - tableWidget.margin.left - lineWidth,
      tableWidget.y + tableWidget.height - lineWidth / 2,
      tableWidget.x + tableWidget.width + lineWidth + tableWidget.margin.right,
      tableWidget.y + tableWidget.height - lineWidth / 2,
      lineWidth,
      border.lineStyle
  ...
  private renderTableCellOutline(
    ...
    if (
      !isNullOrUndefined(tableCell.ownerRow) &&
      tableCell.ownerRow.rowFormat.heightType === 'Exactly'
    ) {
      height =
        HelperMethods.convertPointToPixel(tableCell.ownerRow.rowFormat.height) +
        cellTopMargin +
        cellBottomMargin;
    } else {
      if (
        !isNullOrUndefined(tableCell.ownerRow) &&
        [tableCell.ownerRow].length <= 1
      ) {
        height =
          Math.max(
            HelperMethods.convertPointToPixel(
              tableCell.ownerRow.rowFormat.height
            ),
            cellWidget.height
          ) +
          cellTopMargin +
    ...
    let border: WBorder = !isBidiTable
      ? TableCellWidget.getCellLeftBorder(tableCell)
      : TableCellWidget.getCellRightBorder(tableCell);
    let lineWidth: number = 0;
    // if (!isNullOrUndefined(border )) {
    ...
    if (
      ...
      (tableCell.cellFormat.rowSpan > 1 && tableCell.columnIndex === 0)
    ) {
      this.renderSingleBorder(
        border.color,
        cellWidget.x - cellLeftMargin - lineWidth,
        cellWidget.y - cellWidget.margin.top,
        cellWidget.x - cellLeftMargin - lineWidth,
        cellWidget.y + cellWidget.height + cellBottomMargin,
        lineWidth,
        border.lineStyle
      );
    } else {
      for (
        let i: number = 0;
        i < tableCell.ownerTable.childWidgets.length;
        i++
      ) {
        let row: TableRowWidget = tableCell.ownerTable.childWidgets[
          i
        ] as TableRowWidget;
        let cell: TableCellWidget;
        for (let j: number = 0; j < row.childWidgets.length; j++) {
          let currentCell: TableCellWidget = row.childWidgets[
            j
          ] as TableCellWidget;
          if (
            currentCell.columnIndex + currentCell.cellFormat.columnSpan - 1 ===
            tableCell.columnIndex - 1
          ) {
            cell = currentCell as TableCellWidget;
            break;
          } else if (
            (row.childWidgets[j] as TableCellWidget).columnIndex >=
              tableCell.columnIndex &&
            (row.childWidgets[j] as TableCellWidget).previousWidget
          ) {
            cell = (row.childWidgets[j] as TableCellWidget)
              .previousWidget as TableCellWidget;
            break;
          }
        }
        if (
          cell &&
          cell.columnIndex + cell.cellFormat.columnSpan - 1 ===
            tableCell.columnIndex - 1
        ) {
          let border: WBorder = !isBidiTable
            ? TableCellWidget.getCellRightBorder(cell)
            : TableCellWidget.getCellLeftBorder(cell);
          let lineWidthInt: number = HelperMethods.convertPointToPixel(
            border.getLineWidth()
          );
          ...
          if (cell.y + cell.height < tableCell.y) {
            ...
            cell.y < tableCell.y &&
            cell.y + cell.height > tableCell.y
          ) {
            this.renderSingleBorder(
              border.color,
              tableCell.x - cellLeftMargin - lineWidthInt,
              tableCell.y - cellTopMargin,
              tableCell.x - cellLeftMargin - lineWidthInt,
              cell.y + cell.height + cell.margin.bottom,
              lineWidthInt,
              border.lineStyle
            );
          } else if (
            cell.y === tableCell.y ||
            (cell.y > tableCell.y &&
              cell.y + cell.height < tableCell.y + tableCell.height)
          ) {
            this.renderSingleBorder(
              border.color,
              tableCell.x - cellLeftMargin - lineWidthInt,
            ...
            cell.y < tableCell.y + tableCell.height &&
            cell.y + cell.height >= tableCell.y + tableCell.height
          ) {
            this.renderSingleBorder(
              border.color,
              ...
              cell.y - cell.margin.top,
              tableCell.x - cellLeftMargin - lineWidthInt,
              cell.y + cell.height + cellBottomMargin,
              lineWidthInt,
              border.lineStyle
            );
    ...
    if (tableCell.updatedTopBorders && tableCell.updatedTopBorders.length > 1) {
      ...
      for (let a: number = 0; a < tableCell.updatedTopBorders.length; a++) {
        ...
        if (
          ...
          border.lineWidth <
            TableCellWidget.getCellTopBorder(tableCell).lineWidth
        ) {
          border.lineWidth =
            TableCellWidget.getCellTopBorder(tableCell).lineWidth;
        }
      ...
      this.renderSingleBorder(
        border.color,
        cellWidget.x - cellWidget.margin.left - leftBorderWidth / 2,
        cellWidget.y - cellWidget.margin.top + lineWidth / 2,
        cellWidget.x + cellWidget.width + cellWidget.margin.right + width,
        cellWidget.y - cellWidget.margin.top + lineWidth / 2,
        lineWidth,
        border.lineStyle
      );
    ...
    if (!isBidiTable) {
      isLastCell =
        tableCell.cellIndex === tableCell.ownerRow.childWidgets.length - 1;
    } else {
      isLastCell = tableCell.columnIndex === 0;
    }
    let prevRowSpannedCells: TableCellWidget[] = (
      tableCell.containerWidget as TableRowWidget
    ...
    for (let k: number = 0; k < prevRowSpannedCells.length; k++) {
      ...
      if (
        tableCell.rowIndex <
          spannedCell.rowIndex + spannedCell.cellFormat.rowSpan &&
        tableCell.columnIndex < spannedCell.columnIndex
      ) {
    ...
    if (
      (tableCell.ownerTable.tableFormat.cellSpacing > 0 || isLastCell) &&
      (isBidiTable ||
        tableCell.columnIndex + tableCell.cellFormat.columnSpan ===
          tableCell.ownerTable.tableHolder.columns.length ||
        !isAffectedByRowSpan)
    ) {
      border = isBidiTable
        ? TableCellWidget.getCellLeftBorder(tableCell)
    ...
    let nextRow: TableRowWidget = tableCell.ownerRow
      .nextWidget as TableRowWidget;
    ...
    if (!isNullOrUndefined(nextRow)) {
      if (nextRow.lastChild) {
        let lastCellWidget: TableCellWidget =
          nextRow.lastChild as TableCellWidget;
        previousCellIndex =
          lastCellWidget.columnIndex + lastCellWidget.cellFormat.columnSpan;
      }
      ...
      if (nextRowWidget instanceof TableRowWidget) {
        if (
          cellWidget.containerWidget instanceof TableRowWidget &&
          cellWidget.containerWidget.containerWidget instanceof TableWidget
        ) {
          nextRowIsInCurrentTableWidget =
            (
              cellWidget.containerWidget.containerWidget as TableWidget
            ).childWidgets.indexOf(nextRowWidget) !== -1;
        }
      }
    }
    if (
      tableCell.ownerTable.tableFormat.cellSpacing > 0 ||
      tableCell.ownerRow.rowIndex ===
        tableCell.ownerTable.childWidgets.length - 1 ||
      (tableCell.cellFormat.rowSpan > 1 &&
        tableCell.ownerRow.rowIndex + tableCell.cellFormat.rowSpan >=
          tableCell.ownerTable.childWidgets.length) ||
      !nextRowIsInCurrentTableWidget ||
      this.isRenderHeader ||
      (previousCellIndex &&
        nextRow.childWidgets.length < tableCell.ownerRow.childWidgets.length &&
        previousCellIndex <
          tableCell.columnIndex + tableCell.cellFormat.columnSpan) ||
      (!isNullOrUndefined(tableCell.cellFormat.borders.bottom) &&
        tableCell.cellFormat.borders.bottom.lineStyle !== 'Cleared' &&
        tableCell.cellFormat.rowSpan === 1 &&
        !isBidiTable &&
        ((!isNullOrUndefined(nextRow) &&
          cellWidget.x <
            (nextRow.firstChild as TableCellWidget).x -
              (nextRow.firstChild as TableCellWidget).margin.left &&
          nextRow.rowFormat.gridBefore > 0 &&
          (nextRow.rowFormat.beforeWidth !== 0 ||
            nextRow.rowFormat.gridBeforeWidth !== 0)) ||
          (!isNullOrUndefined(nextRow) &&
            cellWidget.x + cellWidget.width >
              (nextRow.lastChild as TableCellWidget).x +
                (nextRow.lastChild as TableCellWidget).width +
                (nextRow.lastChild as TableCellWidget).margin.right &&
            nextRow.rowFormat.gridAfter > 0 &&
            (nextRow.rowFormat.afterWidth !== 0 ||
              nextRow.rowFormat.gridAfterWidth !== 0))))
    ) {
      let bottomBorder: WBorder = tableCell.cellFormat.borders.bottom;
      if (
        !isNullOrUndefined(bottomBorder) &&
        bottomBorder.lineStyle === 'Cleared'
      ) {
        ...
        border =
          ...
          tableCell.ownerRow.rowIndex + tableCell.cellFormat.rowSpan ===
            ...
            ? //true part for vertically merged cells specifically.
              tableCell.getBorderBasedOnPriority(
                tableCell.getBorderBasedOnPriority(
                  tableCell.cellFormat.borders.bottom,
                  tableCell.ownerRow.rowFormat.borders.bottom
                ),
                tableCell.ownerTable.tableFormat.borders.bottom
              )
      ...
      let startX: number =
        cellWidget.x - cellWidget.margin.left - leftBorderWidth / 2;
      let endX: number =
        cellWidget.x + cellWidget.width + cellWidget.margin.right;
      if (
        !(
          previousCellIndex &&
          nextRow.childWidgets.length <
            tableCell.ownerRow.childWidgets.length &&
          previousCellIndex <
            tableCell.columnIndex + tableCell.cellFormat.columnSpan
        ) &&
        !isNullOrUndefined(nextRow) &&
        ...
        (nextRow.rowFormat.afterWidth !== 0 ||
          nextRow.rowFormat.gridAfterWidth !== 0) &&
        !isNullOrUndefined(nextRow.lastChild) &&
        !isNullOrUndefined(
          TableCellWidget.getCellTopBorder(nextRow.lastChild as TableCellWidget)
        ) &&
        TableCellWidget.getCellTopBorder(nextRow.lastChild as TableCellWidget)
          .lineStyle !== 'None' &&
        cellWidget.x <
      ...
      if (
        ...
        (nextRow.rowFormat.beforeWidth !== 0 ||
          nextRow.rowFormat.gridBeforeWidth !== 0) &&
        !isNullOrUndefined(nextRow.firstChild) &&
        !isNullOrUndefined(
          TableCellWidget.getCellTopBorder(
            nextRow.firstChild as TableCellWidget
          )
        ) &&
        TableCellWidget.getCellTopBorder(nextRow.firstChild as TableCellWidget)
          .lineStyle !== 'None' &&
        cellWidget.x + cellWidget.width >
          (nextRow.firstChild as TableCellWidget).x -
            (nextRow.firstChild as TableCellWidget).margin.left
      ...
      this.renderSingleBorder(
        border.color,
        startX,
        cellWidget.y + cellWidget.height + cellBottomMargin + lineWidth / 2,
        endX,
        cellWidget.y + cellWidget.height + cellBottomMargin + lineWidth / 2,
        lineWidth,
        border.lineStyle
    ...
    if (lineWidth > 0) {
      this.renderSingleBorder(
        border.color,
        cellWidget.x - cellLeftMargin,
        cellWidget.y + cellWidget.height + cellBottomMargin,
        cellWidget.x + cellWidget.width + cellRightMargin,
        cellWidget.y - cellTopMargin,
        lineWidth,
        border.lineStyle
      );
      // }
  ...
  private renderCellBackground(
    ...
    let bgColor: string =
      cellFormat.shading.backgroundColor === '#ffffff'
        ? cellWidget.ownerTable.tableFormat.shading.backgroundColor
    ...
    if (cellWidget.ownerRow.rowFormat.revisions.length > 0) {
      let revision: Revision =
        cellWidget.ownerRow.rowFormat.revisions[
          cellWidget.ownerRow.rowFormat.revisions.length - 1
        ];
    ...
    if (
      cellFormat.shading.hasValue('foregroundColor') &&
      cellFormat.shading.textureStyle !== 'TextureNone'
    ) {
      this.pageContext.beginPath();
      this.pageContext.fillStyle = this.drawTextureStyle(
        cellFormat.shading.textureStyle,
        HelperMethods.getColor(cellFormat.shading.foregroundColor),
        HelperMethods.getColor(cellFormat.shading.backgroundColor),
        cellFormat.shading.foregroundColor === 'empty',
  ...
  private drawTextureStyle(
    ...
    foreColor: string,
    backColor: string,
    isForeColorEmpty: boolean,
    isBackColorEmpty: boolean
  ): string {
    if (isBackColorEmpty) {
      backColor = '#ffffff';
    }
  ...
  private getForeColor(
    ...
    return (
      '#' +
      HelperMethods.convertRgbToHex(r) +
      HelperMethods.convertRgbToHex(g) +
      HelperMethods.convertRgbToHex(b)
    );
  }
  private getColorValue(
    foreColorValue: number,
    ...
    if (percent == 100) {
      ...
      if (isForeColorEmpty) {
        ...
        if (isBackColorEmpty) {
          ...
          colorValue =
            backColorValue +
            Math.round(foreColorValue * (percent / 100)) -
            Math.round(backColorValue * (percent / 100));
        }
      }
    }
    return colorValue;
  ...
  private renderSingleBorder(
    ...
    this.pageContext.beginPath();
    this.pageContext.moveTo(
      this.getScaledValue(startX, 1),
      this.getScaledValue(startY, 2)
    );
    this.pageContext.lineTo(
      this.getScaledValue(endX, 1),
      this.getScaledValue(endY, 2)
    );
  ...
  private getRevisionColor(revisionInfo: RevisionInfo[]): string {
    ...
    for (let i = 0; i < revisionInfo.length; i++) {
      if (
        revisionInfo[i].type === 'Deletion' ||
        revisionInfo[i].type === 'MoveFrom'
      ) {
  ...
  private getRevisionType(
    ...
    for (let i: number = 0; i < revisionInfo.length; i++) {
      ...
      if (
        checkInsert &&
        (revisionInfo[i].type === 'Insertion' ||
          revisionInfo[i].type === 'MoveTo')
      ) {
        type = revisionInfo[i];
  ...
  private getFormfieldInLine(line: LineWidget): FieldElementBox {
    for (let i: number = 0; i < line.children.length; i++) {
      if (
        line.children[i] instanceof FieldElementBox &&
        !isNullOrUndefined((line.children[i] as FieldElementBox).formFieldData)
      ) {
  ...
  private shapes: {} = {
    ...
    RightTriangle:
      'M836.293,292.9238 L776.293,292.9238 L776.293,254.8408 L836.293,292.9238 z',
    ...
    Diamond:
      'M397.784,287.875 L369.5,316.159 L341.216,287.875 L369.5,259.591 L397.784,287.875 z',
    ...
    Heptagon:
      'M223.7783,195.7134 L207.1303,174.8364 L213.0713,148.8034 L237.1303,137.2174 L261.1883,148.8034 L267.1303,174.8364' +
      'L250.4813,195.7134 L223.7783,195.7134 z',

    Octagon:
      'M98.7319,196.4653 L81.1579,178.8923 L81.1579,154.0393 L98.7319,136.4653 L123.5849,136.4653' +
      'L141.1579,154.0393 L141.1579,178.8923 L123.5849,196.4653 L98.7319,196.4653 z',

    Decagon:
      'M657.3379,302.4141 L642.3369,291.5161 L636.6089,273.8821 L642.3369,256.2481 L657.3379,245.3511 L675.8789,245.3511' +
    ...
    Frame:
      'M 1894 154 v 1740 h -1740 v -1740 m 1433 307 h -1126 v 1126 h 1126 z',
    ...
    Cross:
      'M 666 1894 v -512 h -512 v -716 h 512 v -512 h 716 v 512 h 512 v 716 h -512 v 512 z',

    Plaque:
      'M 563 1894 v -51 q 0 -66 -31 -131 q -31 -64 -81 -115 q -51 -50 -115 -81 q -65 -31 -131 -31 h -51 v -922 h 51 q 66 0 131 -31 q 64 -31 115 -82 q 50 -50 81 -115 q 31 -64 31 -130 v -51 h 922 v 51 q 0 66 31 130 q 31 65 82 115 q 50 51 115 82 q 64 31 130 31 h 51 v 922 h -51 q -66 0 -130 31 q -65 31 -115 81 q -51 51 -82 115 q -31 65 -31 131 v 51 z',
    ...
    Bevel:
      'M 1946 102 v 1844 h -1844 v -1844 m 175 103 l 205 205 h 1084 l 205 -205 m -235 1331 v -1024 h -1024 v 1024 m -307 235 l 205 -205 v -1084 l -205 -205 m 1566 1566 l -205 -205 h -1084 l -205 205 m 1566 -1566 l -205 205 v 1084 l 205 205 z',
    ...
    Heart:
      'M 997 1758 q -401 -286 -612 -551 q -212 -265 -231 -504 v -101 q 9 -106 45 -187 q 35 -80 87 -135 q 52 -55 114 -85 q 62 -30 122 -36 q 21 -2 41 -3 q 19 -1 38 -1 q 67 0 119 12 q 51 13 96 38 q 44 26 85 65 q 40 39 84 91 l 39 46 l 39 -46 q 44 -52 85 -91 q 40 -39 85 -65 q 44 -25 96 -38 q 51 -12 118 -12 q 19 0 39 1 q 19 1 40 3 q 60 6 122 36 q 62 31 114 86 q 51 56 87 137 q 36 82 45 189 v 100 q -19 240 -229 503 q -211 264 -609 548 l -30 21 z',

    LightningBolt:
      'M-8,-55L7,-24L1,-20L19,6L13,11L36,55L-3,21L5,16L-19,-6L-11,-12L-36,-35Z',
    ...
    LeftBracket:
      'M 1331 1843 v 103 h -307 q -63 0 -119 -25 q -56 -24 -98 -66 q -42 -42 -66 -98 q -24 -56 -24 -119 v -1228 q 0 -63 24 -119 q 24 -56 66 -98 q 42 -42 98 -67 q 56 -24 119 -24 h 307 v 103 h -307 q -42 0 -79 16 q -38 16 -66 44 q -28 28 -44 65 q -16 38 -16 80 v 1228 q 0 42 16 79 q 16 38 44 66 q 28 28 66 44 q 37 16 79 16 z',

    RightBracket:
      'M 1024 102 q 63 0 119 24 q 56 25 98 67 q 42 42 66 98 q 24 56 24 119 v 1228 q 0 63 -24 119 q -24 56 -66 98 q -42 42 -98 66 q -56 25 -119 25 h -307 v -103 h 307 q 42 0 80 -16 q 37 -16 65 -44 q 28 -28 44 -66 q 16 -37 16 -79 v -1228 q 0 -42 -16 -80 q -16 -37 -44 -65 q -28 -28 -65 -44 q -38 -16 -80 -16 h -307 v -103 z',
    ...
    Plus:
      'M696.6084,158.2656 L674.8074,158.2656 L674.8074,136.4656 L658.4084,136.4656 L658.4084,158.2656 L636.6084,158.2656' +
    ...
    Star:
      'M540.3643,137.9336 L546.7973,159.7016 L570.3633,159.7296 L550.7723,171.9366 L558.9053,194.9966 L540.3643,179.4996' +
      'L521.8223,194.9966 L529.9553,171.9366 L510.3633,159.7296 L533.9313,159.7016 L540.3643,137.9336 z',

    Can:
      'M 542.802,362.009C 542.802,368.452 525.341,373.676 503.802,373.676C 482.263,373.676 464.802,368.452 464.802,362.009' +
    ...
    FlowChartAlternateProcess:
      'M 456 1690 q -62 0 -117 -24 q -55 -24 -96 -65 q -41 -41 -65 -96 q -24 -55 -24 -118 v -726 q 0 -63 24 -118 q 24 -55 65 -96 q 41 -41 96 -65 q 55 -24 117 -24 h 1136 q 62 0 117 24 q 55 24 96 65 q 41 41 65 96 q 24 55 24 118 v 726 q 0 63 -24 118 q -24 55 -65 96 q -41 41 -96 65 q -55 24 -117 24 z',

    FlowChartPunchedTape:
      'M 563 1894 q -94 0 -169 -15 q -76 -15 -129 -38 q -54 -23 -82 -50 q -29 -27 -29 -50 v -1276 q 69 45 175 71 q 105 27 234 27 q 109 0 203 -20 q 94 -19 163 -54 q 68 -34 107 -81 q 39 -46 39 -101 q 0 -23 29 -50 q 29 -27 83 -50 q 53 -23 129 -38 q 75 -15 169 -15 q 94 0 170 15 q 75 15 128 38 q 53 23 82 50 q 29 27 29 50 v 1276 q -69 -45 -174 -72 q -106 -26 -235 -26 q -110 0 -203 19 q -94 20 -162 54 q -69 35 -108 81 q -39 47 -39 102 q 0 23 -29 50 q -29 27 -82 50 q -54 23 -129 38 q -76 15 -170 15 Z',
    ...
    FlowChartPredefinedProcess:
      'M 0,0 L 50,0 L 50,50 L 0,50 Z  M 8.334,0 L 8.334,50 M 41.667,0 L 41.667,50',

    FlowChartTerminator:
      'M 269.711,29.33C 269.71,44.061 257.77,56 243.04,56L 158.058,56C 143.33,56 131.39,44.061 131.39,29.33L 131.39,29.33' +
      'C 131.391,14.6057 143.33,2.669 158.058,2.669L 243.044,2.669C 257.772,2.669 269.711,14.6057 269.711,29.333 Z',

    PaperTap:
      'M0.0009,17.2042 L0.0009,47.165 C0.001,47.165 14.403,53.5455 25.00,47.165 C35.599,40.7852 44.403,43.5087 50.00,47.165' +
    ...
    DirectData:
      'M 132.62 0 L 17.38 0 C 7.78 0 0 13.43 0 30 C 0 46.57 7.78 60 17.38 60 L 132.62 60 M 132.62 0' +
    ...
    SequentialData:
      'M0.0029,24.999 C0.0029,11.1922 10.433,0.0021 23.295,0.0021 C36.159,0.00216 46.585,11.1922 46.585,24.999' +
    ...
    FlowChartSort:
      'M50.001,24.9971 L25.001,49.9971 L0.00097,24.9971 L25.001,-0.00286865 L50.001,24.9971 z' +
      ' M0.000976562,24.9971 L50.001,24.9971',

    FlowChartMultiDocument:
      'M43.6826,40 C44.8746,40.6183 45.8586,41.3502 46.8366,42.1122 L46.8366,4.74487 L3.09857,4.74487 L3.09857,10.9544' +
    ...
    FlowChartCollate:
      'M50.001,0.0028 L25.001,25.0029 L0.00097,0.0028 L50.001,0.002 z M0.0009,50.002 L25.001,25.002' +
      'L50.001,50.002 L0.0009,50.0029 z',

    FlowChartSummingJunction:
      'M7.3252,42.6768 L42.6772,7.3247 M42.6768,42.6768 L7.3248,7.3247 M0.0009,25.001 ' +
    ...
    FlowChartOr:
      'M 0 50 L 100 50 M 50 100 L 50 0.0 M 0 50 C 0 22.384 22.392 0 50 0 C 77.616 0 100 22.384 100 50' +
      ' C 100 77.616 77.616 100 50 100 C 22.392 100 0 77.616 0 50 Z',

    FlowChartInternalStorage:
      'M 0 3.81946A 2.5,3.81946 0 0,1 2.5,0L 47.5 0A 2.5,3.81946 0 0,1 50,3.81946' +
    ...
    FlowChartConnector:
      'M 973 1792 q -113 0 -217 -30 q -105 -29 -196 -82 q -91 -53 -166 -128 q -75 -75 -128 -166 q -53 -91 -82 -196 q -30 -104 -30 -217 q 0 -113 30 -218 q 29 -104 82 -195 q 53 -91 128 -166 q 75 -75 166 -128 q 91 -53 196 -83 q 104 -29 217 -29 q 113 0 218 29 q 104 30 195 83 q 91 53 166 128 q 75 75 128 166 q 53 91 83 195 q 29 105 29 218 q 0 113 -29 217 q -30 105 -83 196 q -53 91 -128 166 q -75 75 -166 128 q -91 53 -195 82 q -105 30 -218 30 Z',

    FlowChartOffPageConnector: 'M 358 1407 v -1253 h 1332 v 1253 l -666 476 Z',

    FlowChartDirectAccessStorage:
      'M 358 1024 q 0 -128 13 -243 q 12 -114 34 -211 q 22 -96 52 -174 q 30 -77 64 -131 q 34 -53 71 -82 q 37 -29 74 -29 h 716 q 36 0 74 29 q 37 29 71 82 q 34 54 64 131 q 30 78 52 174 q 22 97 35 211 q 12 115 12 243 q 0 128 -12 242 q -13 115 -35 211 q -22 97 -52 174 q -30 78 -64 131 q -34 54 -71 83 q -38 29 -74 29 h -716 q -37 0 -74 -29 q -37 -29 -71 -83 q -34 -53 -64 -131 q -30 -77 -52 -174 q -22 -96 -34 -211 q -13 -114 -13 -242 Z',
    ...
    FlowChartSequentialAccessStorage:
      'M 60 30 C 60 13.4 46.6 0 30 0 S 0 13.4 0 30 s 13.4 30 30 30 h 28.6 v -6.5 h -9.9' +
      'C 55.5 48 60 39.5 60 30 Z',

    Annotation:
      'M49.9984,50.0029 L-0.00271199,50.0029 L-0.00271199,0.00286865 L49.9984,0.00286865',

    Annotation2:
      'M49.9977,50.0029 L25.416,50.0029 L25.416,0.00286865 L49.9977,0.00286865 M25.4166,25.0029 L-0.00227869,25.0029',
    ...
    FlowChartPreparation:
      'M 1048.17 572 C 1051.06 568.86 1055.17 567.05 1059.5 567 L 1094.51 567' +
    ...
    FlowChartDisplay:
      'M47.8809,19.2914 L32.7968,-0.00594145 L11.3902,-0.00594145 C7.93166,-0.00594145 0.00124586,11.187 0.00124586,24.9968' +
    ...
    FlowChartManualInput:
      'M 912 732 L 1006.85 707 C 1008.2 707 1009.5 707.53 1010.46 708.46 C 1011.41 709.4 1011.95 710.67 1011.95 712' +
    ...
    LoopLimit:
      'M 8 9 L 27 9 L 33 15 L 33 26 C 33 27 33 27 32 27 L 4 27 C 3 27 2 27 2 26 L 2 15 L 8 9 Z',

    FlowChartStoredData:
      'M 5.55 0L 50 0A 1.5,30 0 0,1 50,0A 5.555,25 0 0,0 50,50A 1.5,30 0 0,1 50,50L 5.555 50A 5.55,25 0 0,1 5.555,0Z',

    //<---------------- Block Arrow Shapes ---------------->

    RightArrow:
      'M 1178 1382 h -1024 v -716 h 1024 v -338 l 695 696 l -695 696 z',
    ...
    LeftRightArrow:
      'M 1382 1280 h -716 v 235 l -491 -491 l 491 -491 v 235 h 716 v -235 l 491 491 l -491 491 z',

    UpDownArrow:
      'M 768 1382 v -716 h -235 l 491 -491 l 491 491 h -235 v 716 h 235 l -491 491 l -491 -491 z',

    QuadArrow:
      'M 738 1587 h 132 v -409 h -409 v 132 l -286 -286 l 286 -286 v 132 h 409 v -409 h -132 l 286 -286 l 286 286 h -132 v 409 h 409 v -132 l 286 286 l -286 286 v -132 h -409 v 409 h 132 l -286 286 z',

    LeftRightUpArrow:
      'M 461 1485 v 132 l -286 -286 l 286 -286 v 133 h 409 v -615 h -132 l 286 -286 l 286 286 h -132 v 615 h 409 v -133 l 286 286 l -286 286 v -132 z',

    BentArrow:
      'M 154 1894 v -1126 q 0 -63 24 -119 q 24 -56 66 -98 q 41 -42 97 -66 q 56 -24 120 -24 h 1024 v -235 l 388 388 l -388 389 v -235 h -768 q -53 0 -99 20 q -47 20 -82 55 q -35 35 -55 81 q -20 47 -20 100 v 870 z',

    UTurnArrow:
      'M 154 1894 v -1075 q 0 -92 24 -177 q 24 -85 67 -159 q 43 -73 104 -134 q 61 -61 135 -104 q 73 -43 158 -67 q 85 -24 177 -24 h 103 q 92 0 177 24 q 85 24 159 67 q 73 43 134 104 q 61 61 104 134 q 43 74 67 159 q 24 85 24 177 v 359 h 235 l -388 388 l -389 -388 h 235 v -359 q 0 -74 -28 -139 q -28 -65 -77 -114 q -49 -49 -114 -77 q -65 -28 -139 -28 h -103 q -74 0 -139 28 q -65 28 -114 77 q -49 49 -77 114 q -28 65 -28 139 v 1075 z',

    LeftUpArrow:
      'M 175 1434 l 388 -389 v 235 h 717 v -717 h -235 l 389 -388 l 388 388 h -235 v 1024 h -1024 v 235 z',

    BentUpArrow:
      'M 154 1894 v -307 h 1126 v -1024 h -235 l 389 -388 l 388 388 h -235 v 1331 z',

    CurvedRightArrow:
      'M 1280 1587 h -205 q -99 0 -190 -26 q -92 -25 -171 -72 q -80 -47 -145 -112 q -66 -65 -113 -145 q -47 -80 -72 -172 q -26 -91 -26 -190 q 0 -99 26 -191 q 25 -91 72 -171 q 47 -79 113 -145 q 65 -65 145 -112 q 79 -46 171 -72 q 91 -25 190 -25 h 205 v 307 h -205 q -108 0 -208 31 q -101 31 -187 88 q -87 57 -155 137 q -68 81 -110 180 l -10 25 l 14 23 q 50 79 116 141 q 66 62 144 105 q 78 43 166 66 q 87 23 179 23 h 256 v -235 l 388 389 l -388 388 z',

    CurvedLeftArrow:
      'M 768 1822 l -388 -388 l 388 -389 v 235 h 256 q 92 0 180 -23 q 87 -23 165 -66 q 78 -43 144 -105 q 66 -62 116 -141 l 14 -23 l -10 -25 q -42 -99 -110 -180 q -68 -80 -154 -137 q -87 -57 -187 -88 q -101 -31 -209 -31 h -205 v -307 h 205 q 99 0 191 25 q 91 26 171 72 q 79 47 145 112 q 65 66 112 145 q 47 80 73 171 q 25 92 25 191 q 0 99 -25 190 q -26 92 -73 172 q -47 80 -112 145 q -66 65 -145 112 q -80 47 -171 72 q -92 26 -191 26 h -205 z',
    ...
    StripedRightArrow:
      'M 102 614 h 103 v 820 h -103 m 205 -820 h 103 v 820 h -103 m 819 409 v -409 h -614 v -820 h 614 v -409 l 820 819 m -717 307 v 265 l 572 -572 l -572 -572 v 265 h -615 v 614 z',

    NotchedRightArrow:
      'M 1126 1843 v -409 h -1024 l 512 -410 l -512 -410 h 1024 v -409 l 820 819 m -717 307 v 265 l 572 -572 l -572 -572 v 265 h -835 l 384 307 l -384 307 z',
    ...
    RightArrowCallout:
      'M 154 154 h 1024 v 716 h 307 v -234 l 388 388 l -388 388 v -234 h -307 v 716 h -1024 z',

    DownArrowCallout:
      'M 154 154 h 1740 v 1024 h -716 v 307 h 234 l -388 388 l -388 -388 h 234 v -307 h -716 z',

    LeftArrowCallout:
      'M 1894 154 v 1740 h -1024 v -716 h -307 v 234 l -388 -388 l 388 -388 v 234 h 307 v -716 z',

    UpArrowCallout:
      'M 1894 1894 h -1740 v -1024 h 716 v -307 h -234 l 388 -388 l 388 388 h -234 v 307 h 716 z',

    LeftRightArrowCallout:
      'M 666 1894 v -716 h -205 v 132 l -286 -286 l 286 -286 v 132 h 205 v -716 h 716 v 716 h 205 v -132 l 286 286 l -286 286 v -132 h -205 v 716 z',

    QuadArrowCallout:
      'M 738 1587 h 132 v -205 h -204 v -204 h -205 v 132 l -286 -286 l 286 -286 v 132 h 205 v -204 h 204 v -205 h -132 l 286 -286 l 286 286 h -132 v 205 h 204 v 204 h 205 v -132 l 286 286 l -286 286 v -132 h -205 v 204 h -204 v 205 h 132 l -286 286 z',
    ...
    Explosion1:
      'M 833 1509 l -270 225 v -352 h -282 l 309 -193 l -374 -421 h 450 v -337 l 260 260 l 354 -443 v 418 h 361 l -226 317 l 381 297 h -394 l 228 457 l -479 -218 l -134 312 z',

    Explosion2:
      'M 591 1480 l -324 -1 l 208 -249 l -270 -360 h 492 l -135 -270 l 247 82 l 39 -239 l 187 281 l 168 -418 l 35 281 l 417 -186 l -135 408 l 253 42 l -215 172 l 246 206 h -300 l 174 346 l -347 -173 v 353 l -305 -175 l -416 270 z',

    Star4Point:
      'M 848 1200 l -572 -176 l 572 -176 l 176 -572 l 176 572 l 572 176 l -572 176 l -176 572 z',

    Star5Point:
      'M 1219 768 h 603 l -446 446 l 175 612 l -527 -352 l -527 352 l 175 -612 l -446 -446 h 598 l 202 -521 z',

    Star6Point:
      'M 786 1485 h -580 l 350 -461 l -350 -461 h 580 l 238 -367 l 238 367 h 580 l -350 461 l 350 461 h -580 l -238 367 z',

    Star7Point:
      'M 1024 1541 l -395 307 l -42 -422 l -396 -176 l 314 -314 l -207 -373 h 461 l 265 -372 l 265 372 h 461 l -207 373 l 314 314 l -396 176 l -42 422 z',

    Star8Point:
      'M 738 1587 h -277 v -277 l -286 -286 l 286 -286 v -277 h 282 l 281 -286 l 286 286 h 277 v 277 l 286 286 l -286 286 v 277 h -277 l -286 286 z',

    Star10Point:
      'M 828 1635 l -367 48 v -286 l -277 -93 l 198 -280 l -198 -280 l 277 -93 v -286 l 367 48 l 196 -231 l 196 231 l 370 -48 v 286 l 274 92 l -198 281 l 198 280 l -277 93 v 286 l -367 -48 l -196 231 z',

    Star12Point:
      'M 868 1611 l -305 231 v -357 h -357 l 231 -305 l -241 -156 l 241 -156 l -231 -305 h 357 v -357 l 305 231 l 156 -241 l 156 241 l 305 -231 v 357 h 357 l -231 305 l 241 156 l -241 156 l 231 305 h -357 v 357 l -305 -231 l -156 241 z',
    ...
    MathNotEqual:
      'M 1372 870 l -136 205 h 658 v 512 h -1000 l -205 307 h -389 l 205 -307 h -351 v -512 h 693 l 136 -205 h -829 v -512 h 1170 l 137 -204 h 389 l -137 204 h 181 v 512 z',

    //<---------------- Rectangle Shapes ---------------->

    RoundedRectangle:
      'M 20,0 h 160 a 20,20 0 0 1 20,20 v 60 a 20,20 0 0 1 -20,20 h -160 a 20,20 0 0 1 -20,-20 v -60 a 20,20 0 0 1 20,-20 z',

    SnipSingleCornerRectangle: 'M 154 1690 v -1332 h 1258 l 482 482 v 850 z',

    SnipSameSideCornerRectangle:
      'M 154 1690 v -850 l 482 -482 h 776 l 482 482 v 850 z',

    SnipDiagonalCornerRectangle:
      'M 636 1690 l -482 -482 v -850 h 1258 l 482 482 v 850 z',

    SnipAndRoundSingleCornerRectangle:
      'M 154 1690 v -1029 q 0 -63 24 -118 q 24 -55 65 -96 q 41 -41 96 -65 q 55 -24 117 -24 h 956 l 482 482 v 850 z',

    RoundSingleCornerRectangle:
      'M 154 1690 v -1332 h 1438 q 62 0 117 24 q 55 24 96 65 q 41 41 65 96 q 24 55 24 118 v 1029 z',

    RoundSameSideCornerRectangle:
      'M 154 1690 v -1029 q 0 -63 24 -118 q 24 -55 65 -96 q 41 -41 96 -65 q 55 -24 117 -24 h 1136 q 62 0 117 24 q 55 24 96 65 q 41 41 65 96 q 24 55 24 118 v 1029 z',

    RoundDiagonalCornerRectangle:
      'M 456 1690 q -62 0 -117 -24 q -55 -24 -96 -65 q -41 -41 -65 -96 q -24 -55 -24 -118 v -1029 h 1438 q 62 0 117 24 q 55 24 96 65 q 41 41 65 96 q 24 55 24 118 v 1029 z',
