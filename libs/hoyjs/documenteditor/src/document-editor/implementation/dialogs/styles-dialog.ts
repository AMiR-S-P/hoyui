import { ListView, SelectEventArgs } from '@hoyui/lists';
...
export class StylesDialog {
  ...
  private selecHandlerClickHandler: EventListener =
    this.onSelecHandlerClick.bind(this);
  private addNewStyleClickHandler: EventListenerOrEventListenerObject =
    this.onAddNewStyleClick.bind(this);
  private modifyStyleClickHandler: EventListenerOrEventListenerObject =
    this.onModifyStyleClick.bind(this);
  /**
  ...
  public initStylesDialog(
    ...
    this.dlgFields = createElement('div', {
      innerHTML: headerValue,
    ...
    this.listviewDiv = createElement('div', {
      className: 'e-styles-listViewDiv',
      id: 'styles_listview',
    });
    ...
    this.newButtonElement.addEventListener(
      'click',
      this.addNewStyleClickHandler
    );

    this.modifybuttonDiv = createElement('div', {
      className: 'e-styles-addbutton',
    });
    this.buttonDiv.appendChild(this.modifybuttonDiv);
    this.modifyButtonElement = createElement('button', {
      innerHTML: localValue.getConstant('Modify') + '...',
      id: 'modify',
      attrs: { type: 'button' },
    });
    ...
    this.modifyButtonElement.addEventListener(
      'click',
      this.modifyStyleClickHandler
    );
  }
  /**
   * @private
   ...
   */
  public show(): void {
    const localValue: L10n = new L10n(
      'documenteditor',
      this.documentHelper.owner.defaultLocale
    );
    localValue.setLocale(this.documentHelper.owner.locale);
    const paraStyles: { [key: string]: string }[] = this.updateStyleNames(
      'Paragraph'
    ).filter((obj: any) => (obj as any).Type === 'Paragraph');
    const linkedStyles: { [key: string]: string }[] = this.updateStyleNames(
      'Paragraph'
    ).filter((obj: any) => (obj as any).Type === 'Linked');
    const charStyles: { [key: string]: string }[] = this.updateStyleNames(
      'Character'
    ).filter((obj: any) => (obj as any).Type === 'Character');
    ...
    const styles: { [key: string]: string }[] = paraStyles.concat(
      linkedStyles,
      charStyles
    );
    this.localValue = localValue;
    this.initStylesDialog(
      localValue,
      styles,
      this.documentHelper.owner.enableRtl
    );
    ...
    this.documentHelper.dialog.buttons = [
      {
        ...
        buttonModel: {
          content: localValue.getConstant('Cancel'),
          cssClass: 'e-flat e-hyper-insert',
          isPrimary: true,
        },
      },
    ];
  ...
  private updateStyleNames(type: StyleType): { [key: string]: string }[] {
    ...
    const collection: object[] =
      this.documentHelper.owner.documentHelper.styles.getStyles(type);
    const paraIcon: string =
      'e-de-listview-icon e-de-e-paragraph-style-mark e-icons';
    const charIcon: string =
      'e-de-listview-icon e-de-e-character-style-mark e-icons';
    const linkedIcon: string =
      'e-de-listview-icon e-de-e-linked-style-mark e-icons';
    const finalList: { [key: string]: string }[] = [];
    for (let i: number = 0; i < collection.length; i++) {
      let styleName: string = localValue.getConstant(
        (collection[parseInt(i.toString(), 10)] as any).name
      );
      if (styleName === '') {
        styleName = (collection[parseInt(i.toString(), 10)] as any).name;
      }
      if (
        (collection[parseInt(i.toString(), 10)] as any).type === 'Paragraph'
      ) {
        finalList.push({
          StyleName: styleName,
          IconClass: paraIcon,
          Type: (collection[parseInt(i.toString(), 10)] as any).type,
        });
      } else if (
        (collection[parseInt(i.toString(), 10)] as any).type === 'Character'
      ) {
        finalList.push({
          StyleName: styleName,
          IconClass: charIcon,
  ...
  private modifyStyles = (): void => {
    ...
    this.documentHelper.owner.styleDialogModule.show(
      this.styleName,
      this.localValue.getConstant('Modify Style')
    );
  };
  ...
  private removeEvents(): void {
    if (this.newButtonElement) {
      this.newButtonElement.removeEventListener(
        'click',
        this.addNewStyleClickHandler
      );
    }
    if (this.modifyButtonElement) {
      this.modifyButtonElement.removeEventListener(
        'click',
        this.modifyStyleClickHandler
      );
    }
    if (this.listviewInstance) {
      this.listviewInstance.removeEventListener(
        'select',
        this.selecHandlerClickHandler
      );
    }
