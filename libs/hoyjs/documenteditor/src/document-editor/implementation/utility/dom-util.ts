...
export class RulerHelper {
  ...
  private onMarkIndicatorClick() {
    const divElements: HTMLElement = document.querySelector(
      '.e-de-ruler-markIndicator'
    ) as HTMLElement;
    for (let i: number = 0; i < divElements.childNodes.length; i++) {
      const currentDiv: HTMLElement = divElements.childNodes[
      ...
      if (currentDiv.style.display === 'block') {
        ...
        (
          divElements.childNodes[
            parseInt(nextIndex.toString(), 10)
          ] as HTMLElement
        ).style.display = 'block';
        break;
      }
  ...
  public setAttributeSvg(svg: SVGElement, attributes: Object): void {
    ...
    for (let i: number = 0; i < keys.length; i++) {
      ...
      if (svg && keys[parseInt(i.toString(), 10)] !== 'style') {
        svg.setAttribute(
          keys[parseInt(i.toString(), 10)],
          attributes[keys[parseInt(i.toString(), 10)]]
        );
      } else {
        this.applyStyleAgainstCsp(
          svg,
          attributes[keys[parseInt(i.toString(), 10)]]
        );
      }
  ...
  public renderOverlapElement(documentEditor: DocumentEditor): HTMLElement {
    ...
    const attributes: Object = {
      ...
      style:
        ...
        'px;width:' +
        rulerSize.width +
        'px;position:absolute;margin-left:0;margin-top:0;diplay:none',
      class: 'e-ruler-overlap',
    };
    this.overlap = this.createHtmlElement('div', attributes);
    const element: HTMLElement = document.getElementById(
      documentEditor.element.id + '_viewerContainer'
    );
    element.insertBefore(this.overlap, element.firstChild);
    return this.overlap;
  }
  public renderRulerMarkerIndicatorElement(
    ...
    const attributes: Object = {
      ...
      style:
        ...
        'px;width:' +
        rulerSize.width +
        'px;position:absolute;margin-left:0;margin-top:0;z-index:5;border:1px solid #ccc;display:' +
        (documentEditor.layoutType === 'Pages' ? 'block;' : 'none;'),
    ...
    this.firstLineIndentRuler = document
      .getElementById(ownerId + '_firstLineIndent')
      .cloneNode(true) as HTMLElement;
    this.hangingIndentRuler = document
      .getElementById(ownerId + '_hangingIndent')
      .cloneNode(true) as HTMLElement;
    ...
    this.firstLineIndentRuler.setAttribute(
      'id',
      ownerId + '_firstLineIndent_-1'
    );
    ...
    const locale: L10n = new L10n(
      'documenteditor',
      documentEditor.defaultLocale
    );
    ...
    for (let i: number = 0; i < 5; i++) {
      this.renderTab(
        ...
        rulerSize,
        undefined,
        justification[parseInt(i.toString(), 10)] as TabJustification,
        -1,
        locale
      );
      const element: HTMLElement = document.getElementById(
        documentEditor.element.id +
          '_' +
          justification[parseInt(i.toString(), 10)] +
          'Tab_-1'
      );
    ...
    this.markIndicator.addEventListener(
      'click',
      this.onmarkIndicatorClickHandler
    );
  }
  ...
  public renderRuler(
    ...
    this.rulerDiv = document.getElementById(
      documentEditor.element.id + (isHorizontal ? '_hRuler' : '_vRuler')
    );
    const rulerSize: Size = this.getRulerSize(documentEditor);
    const rulerGeometry: Size = this.getRulerGeometry(documentEditor);
    const height: number = isHorizontal
      ? documentEditor.selectionModule.end.paragraph.bodyWidget.page
          .boundingRectangle.x
      : documentEditor.selectionModule.getPageTop(
          documentEditor.selectionModule.end.paragraph.bodyWidget.page
        );
    const margin: string = isHorizontal
    ...
    if (!this.rulerDiv) {
      const style: string =
        'height:' +
        (isHorizontal ? rulerSize.height : rulerGeometry.height) +
        'px;overflow:hidden;width:' +
        (isHorizontal ? rulerGeometry.width : rulerSize.width) +
        'px;position:absolute;font-size:9px;text-align: left;z-index: 4;user-select:none;' +
        margin;
      const attributes: Object = {
    ...
    const pageElement: HTMLElement = document.getElementById(
      documentEditor.element.id + '_pageContainer'
    );
    const style: string =
      'height:' +
      (isHorizontal
      ...
      (isHorizontal
        ...
        : rulerSize.width) +
      'px;position:absolute;z-index: 3;';
    const attributes: Object = {
      id:
        documentEditor.element.id +
        (isHorizontal ? '_hRulerBottom' : '_vRulerBottom'),
      style: style,
      class: isHorizontal ? 'e-de-hRuler' : 'e-de-vRuler',
    };
    ...
    const element: HTMLElement = isHorizontal
      ? document.getElementById(documentEditor.element.id + '_hRulerBottom')
      : document.getElementById(documentEditor.element.id + '_vRulerBottom');
    ...
    isHorizontal
      ? (documentEditor.hRuler = ruler)
      : (documentEditor.vRuler = ruler);
    this.updateRulerPosition(documentEditor, isHorizontal);
    const rulerObj: HTMLElement = document.getElementById(
      documentEditor.element.id + (isHorizontal ? '_hRuler' : '_vRuler')
    );
    // eslint-disable-next-line
    isHorizontal
      ? (documentEditor.hRuler.element = rulerObj)
      : (documentEditor.vRuler.element = rulerObj);
    if (rulerObj) {
      ...
      if (isHorizontal) {
        rulerObj.scrollLeft =
          ruler.zeroPosition -
          HelperMethods.convertPointToPixel(
            documentEditor.selectionModule.end.paragraph.bodyWidget
    ...
    this.hRuler = document.getElementById(
      documentEditor.element.id + '_hRuler'
    );
    ...
    if (isHorizontal) {
      document.addEventListener('mousemove', (e: MouseEvent) => {
        ...
        const leftMargin: number =
          documentEditor.selectionModule.end.paragraph.bodyWidget.sectionFormat
            .leftMargin * documentEditor.zoomFactor;
        const rightMargin: number =
          HelperMethods.convertPixelToPoint(divRect.width) -
          documentEditor.selectionModule.end.paragraph.bodyWidget.sectionFormat
            .rightMargin *
            documentEditor.zoomFactor;
        let pixelValue: number = Math.round(e.clientX - divRect.left);
        let mouseXRelativeToDiv: number =
          HelperMethods.convertPixelToPoint(pixelValue);
        if (!isDragging) {
          if (documentEditor.isOnIndent) {
            ...
            leftMargin - 3 <= mouseXRelativeToDiv &&
            leftMargin + 3 >= mouseXRelativeToDiv
          ) {
            if (documentEditor.layoutType === 'Pages') {
              this.hRuler.style.cursor = 'e-resize';
              this.hRuler.setAttribute(
                'title',
                this.locale.getConstant('Left Margin')
              );
              resizerEnabled = true;
              isLeftRulerMargin = true;
            }
          } else if (
            rightMargin - 3 <= mouseXRelativeToDiv &&
            rightMargin + 3 >= mouseXRelativeToDiv
          ) {
            if (documentEditor.layoutType === 'Pages') {
              this.hRuler.style.cursor = 'e-resize';
              this.hRuler.setAttribute(
                'title',
                this.locale.getConstant('Right Margin')
              );
              resizerEnabled = true;
            ...
            documentEditor.selectionModule.end.paragraph.bodyWidget
              .sectionFormat.columns.length > 0
          ) {
            const columns: WColumnFormat[] =
              documentEditor.selectionModule.end.paragraph.bodyWidget
                .sectionFormat.columns;
            if (documentEditor.layoutType === 'Pages') {
              for (let i: number = 1; i <= columns.length; i++) {
                const rulerMarginDiv: HTMLElement = document.getElementById(
                  documentEditor.element.id + '_hRuler_Margin' + i
                );
                const maginLeft: number =
                  rulerMarginDiv.getBoundingClientRect().left;
                const width: number =
                  rulerMarginDiv.getBoundingClientRect().width;
                if (maginLeft - 3 <= e.clientX && maginLeft + 3 >= e.clientX) {
                  this.hRuler.style.cursor = 'e-resize';
        ...
        if (isDragging) {
          ...
          const rightMarginValue: number =
            documentEditor.selectionModule.sectionFormat.rightMargin;
          let rightIndentValue: number =
            documentEditor.selectionModule.paragraphFormat.rightIndent;
          rightIndentValue = rightIndentValue > 0 ? rightIndentValue : 0;
          const minimumValue: number = 42;
          let firstLineIndent: number =
            documentEditor.selectionModule.paragraphFormat.firstLineIndent;
          const leftMarginValue: number =
            documentEditor.selectionModule.sectionFormat.leftMargin;
          firstLineIndent = firstLineIndent >= 0 ? firstLineIndent : 0;
          const leftIndent: number =
            documentEditor.selectionModule.paragraphFormat.leftIndent;
          if (isLeftRulerMargin) {
            const leftMaxLimit: number =
              rulerZeroPoint +
              HelperMethods.convertPointToPixel(
                pageWidth -
                  rightMarginValue -
            ...
            const rightMinLimit: number =
              ...
              HelperMethods.convertPointToPixel(
                ...
                documentEditor.zoomFactor;
            const rightMaxLimit: number =
              rulerZeroPoint +
              HelperMethods.convertPointToPixel(pageWidth) *
                documentEditor.zoomFactor;
            if (pixelValue + rulerZeroPoint > rightMaxLimit) {
              pixelValue = rightMaxLimit - rulerZeroPoint;
              mouseXRelativeToDiv =
          ...
          const currentRightMargin: number =
            HelperMethods.convertPixelToPoint(divRect.width) -
            documentEditor.selectionModule.end.paragraph.bodyWidget
          ...
          if (
            documentEditor.selectionModule.end.paragraph.bodyWidget
              .sectionFormat.numberOfColumns <= 1
          ) {
            /* eslint-disable-next-line max-len */
            this.resizeRulerMargins(
              isLeftRulerMargin,
              initialValue,
              currentScrollLeft,
              currentRightMargin,
          ...
          const rightIndent: HTMLElement = document.getElementById(
            documentEditor.element.id + '_rightIndent'
          );
          if (isLeftRulerMargin) {
            const difference: number = mouseXRelativeToDiv - initialValue;
            rightIndent.style.left =
              initialRightMargin -
              HelperMethods.convertPointToPixel(difference) +
              'px';
          } else {
            const difference: number = mouseXRelativeToDiv - initialValue;
            rightIndent.style.left =
          ...
          const indicatorLineValue: number = startValue + pixelValue;
          const lineSvg: HTMLElement = document.getElementById(
            documentEditor.element.id + '_hRuler_indicator_svg'
          );
          lineSvg.style.left = indicatorLineValue + 'px';
        }
      });
      ...
      this.hRuler.addEventListener('mousedown', (e: MouseEvent) => {
        if (resizerEnabled && !documentEditor.isTableMarkerDragging) {
          ...
          if (
            ...
            if (isLeftMultiColumn) {
              columnInitialValue =
                multiColumnElement.getBoundingClientRect().left;
          ...
          const divRect: DOMRect =
            this.hRuler.getBoundingClientRect() as DOMRect;
          initialValue = HelperMethods.convertPixelToPoint(
            Math.round(e.clientX - divRect.left)
          );
          currentScrollLeft = this.hRuler.scrollLeft;
          const rightIndent: HTMLElement = document.getElementById(
            documentEditor.element.id + '_rightIndent'
          );
          initialRightMargin = HelperMethods.getNumberFromString(
            rightIndent.style.left
          );
        ...
        if (divRect.y + divRect.height / 2 <= e.clientY) {
          mouseDownTabValue =
            e.clientX - this.hRuler.getBoundingClientRect().left;
          if (documentEditor.layoutType === 'Pages') {
            ...
            if (this.position.paragraph.paragraphFormat.bidi) {
              let paraWidth: number = !isNullOrUndefined(
                this.position.paragraph['absoluteXPosition']
              )
                ? parseFloat(
                    this.position.paragraph['absoluteXPosition'][
                      'width'
                ...
                : this.position.paragraph.width;
              paraWidth = HelperMethods.convertPixelToPoint(
                paraWidth * documentEditor.zoomFactor
              );
      ...
      this.hRuler.addEventListener('mouseup', (e: MouseEvent) => {
        const container: HTMLElement = document.getElementById(
          documentEditor.element.id + '_markIndicator'
        );
        const divRect: DOMRect = this.hRuler.getBoundingClientRect() as DOMRect;
        ...
        if (divRect.y + divRect.height / 2 <= e.clientY) {
          mouseUpTabValue =
            e.clientX - this.hRuler.getBoundingClientRect().left;
          if (documentEditor.layoutType === 'Pages') {
            mouseUpTabValue = HelperMethods.convertPixelToPoint(
              mouseUpTabValue -
                HelperMethods.convertPointToPixel(
                  documentEditor.selectionModule.end.paragraph.bodyWidget
                    .sectionFormat.leftMargin
          ...
          const rightIndentValue: number = HelperMethods.getNumberFromString(
            rightIndent.style.left
          );
          const maxValue: number = rightIndentValue;
          if (
            mouseUpTabValue > 0 &&
            mouseUpTabValue < maxValue &&
            mouseDownTabValue === mouseUpTabValue
          ) {
            if (!isNullOrUndefined(container)) {
              const visibleElement: Element = container.querySelector(
                '.e-de-ruler-marker[style*="display: block;"]'
              );
              if (!isNullOrUndefined(visibleElement)) {
                mouseUpTabValue /= documentEditor.zoomFactor;
                const dataNameValue: string =
                  visibleElement.getAttribute('data-name');
                if (
                  dataNameValue === 'LeftTab' ||
                  dataNameValue === 'CenterTab' ||
                  dataNameValue === 'RightTab' ||
                  dataNameValue === 'DecimalTab' ||
                  dataNameValue === 'BarTab'
                ) {
                  const tabStop: WTabStop = new WTabStop();
                  tabStop.position = mouseUpTabValue;
                  tabStop.tabJustification =
                    this.getTabJustification(dataNameValue);
                  tabStop.deletePosition = 0;
                  tabStop.tabLeader = 'None';
                  documentEditor.editorModule.onApplyParagraphFormat(
                    'tabStop',
                    [tabStop],
                    false,
                    false
                  ...
                  dataNameValue === 'FirstLineIndent' ||
                  dataNameValue === 'HangingIndent'
                ) {
                  const property: string = 'firstLineIndent';
                  if (dataNameValue === 'HangingIndent') {
                    ...
                    const differenceValue: number =
                      mouseUpTabValue + initialValue;
                    let currentValue: number =
                      documentEditor.selectionModule.start.paragraph
                    ...
                    const leftIndentCurrentValue: number =
                      documentEditor.selectionModule.start.paragraph
                        .paragraphFormat.leftIndent + currentValue;
                    currentValue = currentValue - differenceValue;
                    documentEditor.editorModule.onApplyParagraphFormat(
                      'leftIndent',
                      leftIndentCurrentValue - currentValue,
                      false,
                      false,
                      true
                    );
        ...
        if (isDragging && !documentEditor.isTableMarkerDragging) {
          ...
          if (isLeftMultiColumn || isRightMultiColumn) {
            ...
            const pageWidth: number =
              ...
              documentEditor.selectionModule.end.paragraph.bodyWidget
                .sectionFormat.rightMargin;
            const columnSpace: number = isLeftMultiColumn
              ? secFormat.columns[0].space +
                HelperMethods.convertPixelToPoint(finalvalue)
              : secFormat.columns[0].space -
                HelperMethods.convertPixelToPoint(finalvalue);
            for (let i: number = 0; i < secFormat.columns.length; i++) {
              const col: WColumnFormat =
                secFormat.columns[parseInt(i.toString(), 10)];
              if (columnSpace >= 0 && col.width >= 36) {
                const widthCal: number = HelperMethods.convertPointToPixel(
                  (pageWidth -
                    HelperMethods.convertPixelToPoint(columnSpace) *
                      (secFormat.numberOfColumns - 1)) /
                    secFormat.numberOfColumns
                );
                col.width = widthCal;
            ...
            documentEditor.editorModule.onApplySectionFormat(
              undefined,
              secFormat
            );
            isLeftMultiColumn = false;
            ...
            documentEditor.hRuler.startMargin =
              mouseXRelativeToDiv / documentEditor.zoomFactor;
            documentEditor.selectionModule.sectionFormat.leftMargin =
              mouseXRelativeToDiv / documentEditor.zoomFactor;
          } else {
            const rightMargin: number =
              HelperMethods.convertPixelToPoint(rulerGeometry.width) -
              mouseXRelativeToDiv / documentEditor.zoomFactor;
    ...
    this.vRuler = document.getElementById(
      documentEditor.element.id + '_vRuler'
    );
    ...
    if (!isHorizontal) {
      document.addEventListener('mousemove', (e: MouseEvent) => {
        ...
        const topMargin: number =
          documentEditor.selectionModule.end.paragraph.bodyWidget.sectionFormat
            .topMargin * documentEditor.zoomFactor;
        const bottomMargin: number =
          HelperMethods.convertPixelToPoint(divRect.height) -
          documentEditor.selectionModule.end.paragraph.bodyWidget.sectionFormat
            .bottomMargin *
            documentEditor.zoomFactor;
        const mouseXRelativeToDiv: number = HelperMethods.convertPixelToPoint(
          Math.round(e.clientY - divRect.top)
        );
        let pixelValue: number = Math.round(e.clientY - divRect.top);
        if (!isDragging) {
          if (
            topMargin - 3 <= mouseXRelativeToDiv &&
            topMargin + 3 >= mouseXRelativeToDiv
          ) {
            this.vRuler.style.cursor = 'n-resize';
            this.vRuler.setAttribute(
              'title',
              this.locale.getConstant('Top Margin')
            );
            resizerEnabled = true;
            isTopRulerMargin = true;
          } else if (
            bottomMargin - 3 <= mouseXRelativeToDiv &&
            bottomMargin + 3 >= mouseXRelativeToDiv
          ) {
            this.vRuler.style.cursor = 'n-resize';
            this.vRuler.setAttribute(
              'title',
              this.locale.getConstant('Bottom Margin')
            );
            resizerEnabled = true;
        ...
        if (isDragging) {
          let mouseXRelativeToDiv: number = HelperMethods.convertPixelToPoint(
            Math.round(e.clientY - divRect.top)
          );
          const rulerZeroPoint: number =
            HelperMethods.convertPointToPixel(
          ...
          const pageHeight: number =
            documentEditor.selectionModule.sectionFormat.pageHeight;
          const minimumValue: number = 12;
          const bottomMarginValue: number =
            documentEditor.selectionModule.sectionFormat.bottomMargin;
          const topMarginValue: number =
            documentEditor.selectionModule.sectionFormat.topMargin;
          if (isTopRulerMargin) {
            const topMinLimit: number = rulerZeroPoint;
            const topMaxLimit: number =
              rulerZeroPoint +
              HelperMethods.convertPointToPixel(
                pageHeight - bottomMarginValue - minimumValue
              ) *
                documentEditor.zoomFactor;
            ...
            const bottomMinLimit: number =
              rulerZeroPoint +
              HelperMethods.convertPointToPixel(topMarginValue + minimumValue) *
                documentEditor.zoomFactor;
            const bottomMaxLimit: number =
              rulerZeroPoint +
              HelperMethods.convertPointToPixel(pageHeight) *
                documentEditor.zoomFactor;
          ...
          const currentBottomMargin: number =
            HelperMethods.convertPixelToPoint(divRect.height) -
            documentEditor.selectionModule.end.paragraph.bodyWidget
              .sectionFormat.bottomMargin *
              documentEditor.zoomFactor;
          this.resizeVRulerMargins(
            isTopRulerMargin,
            initialYValue,
            currentScrollTop,
            currentBottomMargin,
          ...
          const startValue: number =
            documentEditor.documentHelper.currentPage.boundingRectangle.y *
            documentEditor.zoomFactor;
          const indicatorLineValue: number = startValue + pixelValue; // + 15;
          const lineSvg: HTMLElement = document.getElementById(
            documentEditor.element.id + '_vRuler_indicator_svg'
          );
      ...
      this.vRuler.addEventListener('mousedown', (e: MouseEvent) => {
        if (resizerEnabled) {
          isDragging = true;
          const divRect: DOMRect =
            this.vRuler.getBoundingClientRect() as DOMRect;
          initialYValue = HelperMethods.convertPixelToPoint(
            Math.round(e.clientY - divRect.top)
          );
      ...
      document.addEventListener('mouseup', (e: MouseEvent) => {
        if (isDragging) {
          ...
          // updateRuler(documentEditor, documentEditor.hRuler, true);
          if (isTopRulerMargin) {
            documentEditor.vRuler.startMargin =
              mouseXRelativeToDiv / documentEditor.zoomFactor;
            documentEditor.selectionModule.sectionFormat.topMargin =
              mouseXRelativeToDiv / documentEditor.zoomFactor;
          } else {
            const bottomtMargin: number =
              HelperMethods.convertPixelToPoint(rulerGeometry.height) -
              mouseXRelativeToDiv / documentEditor.zoomFactor;
  ...
  public updateIndicatorLines(documentEditor: DocumentEditor): void {
    const hRulerSvg: HTMLElement = document.getElementById(
      documentEditor.element.id + '_hRuler_indicator_svg'
    );
    const hRulerLine: HTMLElement = document.getElementById(
      documentEditor.element.id + '_hRuler_indicator'
    );
    const vRulerSvg: HTMLElement = document.getElementById(
      documentEditor.element.id + '_vRuler_indicator_svg'
    );
    const vRulerLine: HTMLElement = document.getElementById(
      documentEditor.element.id + '_vRuler_indicator'
    );

    const pageContainer: HTMLElement = document.getElementById(
      documentEditor.element.id + '_pageContainer'
    );
  ...
  public createIndicatorLines(documentEditor: DocumentEditor): void {
    ...
    const viewerContainer: HTMLElement = document.getElementById(
      documentEditor.element.id + '_viewerContainer'
    );
    const pageContainer: HTMLElement = document.getElementById(
      documentEditor.element.id + '_pageContainer'
    ...
    const verticalLineAttr: Object = {
      x1: 0,
      y1: this.hRuler.getBoundingClientRect().height + 5,
      x2: 0,
      y2: pageHeight,
      'stroke-width': 0.5,
      stroke: 'black',
    ...
    const vRuler: HTMLElement = document.getElementById(
      documentEditor.element.id + '_vRuler'
    );
    ...
    const horizontalLineAttr: Object = {
  ...
  public updateIndentMarkers(documentEditor: DocumentEditor): void {
    if (
      isNullOrUndefined(documentEditor) ||
      isNullOrUndefined(documentEditor.element) ||
      isNullOrUndefined(documentEditor.element.id) ||
      isNullOrUndefined(documentEditor.hRuler) ||
    ...
    const rulerGeometry: Size = this.getRulerGeometry(documentEditor);
    if (
      this.position.paragraph.paragraphFormat.bidi ||
      (this.position.paragraph.isInsideTable &&
        this.position.paragraph.associatedCell.ownerTable.tableFormat.bidi)
    ) {
      const rulerMarginDivWidth: number =
        rulerGeometry.width / documentEditor.zoomFactor -
        HelperMethods.convertPointToPixel(
          this.position.paragraph.bodyWidget.sectionFormat.rightMargin +
            this.position.paragraph.bodyWidget.sectionFormat.leftMargin
        );
      rulerZeroPoint -= rulerMarginDivWidth;
    }

    const paraStart: number = !isNullOrUndefined(
      this.position.paragraph['absoluteXPosition']
    )
      ? parseFloat(this.position.paragraph['absoluteXPosition']['x'].toString())
      : this.position.paragraph.x;
    const paraWidth: number = !isNullOrUndefined(
      this.position.paragraph['absoluteXPosition']
    )
      ? parseFloat(
          this.position.paragraph['absoluteXPosition']['width'].toString()
        )
      : this.position.paragraph.width;
    ...
    const leftIndent: HTMLElement = document.getElementById(
      documentEditor.element.id + '_leftIndent'
    );
    const rightIndent: HTMLElement = document.getElementById(
      documentEditor.element.id + '_rightIndent'
    );
    if (this.position.paragraph.paragraphFormat.bidi) {
      ...
      rightIndent.setAttribute(
        'title',
        this.locale.getConstant('Right Indent')
      );
      finalValueTemp = rulerZeroPoint + paraStart;
    }
    const firstLineIndent: number =
      this.position.paragraph.paragraphFormat.firstLineIndent;
    indent = document.getElementById(ownerId + '_leftIndent');
    if (!isNullOrUndefined(indent)) {
      if (documentEditor.layoutType === 'Pages') {
        indent.style.left =
          finalValueTemp * documentEditor.zoomFactor - 6 + 'px';
      } else if (documentEditor.layoutType === 'Continuous') {
        if (this.position.paragraph.paragraphFormat.bidi) {
          rulerZeroPoint =
            documentEditor.hRuler.zeroPosition * documentEditor.zoomFactor -
            20 -
            documentEditor.viewer.clientArea.width;
          finalValueTemp = rulerZeroPoint + paraStart + paraWidth;
          indent.style.left = finalValueTemp + 'px';
        } else {
          rulerZeroPoint =
            documentEditor.hRuler.zeroPosition * documentEditor.zoomFactor - 20;
          finalValueTemp =
            rulerZeroPoint + paraStart * documentEditor.zoomFactor;
          indent.style.left = finalValueTemp + 'px';
        }
      }
    }
    ...
    if (documentEditor.layoutType === 'Pages') {
      if (!isNullOrUndefined(indent)) {
        if (this.position.paragraph.paragraphFormat.bidi) {
          indent.style.left =
            (finalValueTemp -
              HelperMethods.convertPointToPixel(firstLineIndent)) *
              documentEditor.zoomFactor -
            6 +
            'px';
        } else {
      ...
      if (this.position.paragraph.paragraphFormat.bidi) {
        ...
        finalValueTemp =
          rulerZeroPoint +
          (paraStart + HelperMethods.convertPointToPixel(firstLineIndent)) *
            documentEditor.zoomFactor;
      }
      indent.style.left = finalValueTemp + 'px';
    }
    ...
    if (documentEditor.layoutType === 'Pages') {
      ...
      if (this.position.paragraph.paragraphFormat.bidi) {
        rulerZeroPoint =
          documentEditor.hRuler.zeroPosition * documentEditor.zoomFactor -
          20 -
          documentEditor.viewer.clientArea.width * documentEditor.zoomFactor;
        finalValueTemp = rulerZeroPoint + paraStart;
        indent.style.left = finalValueTemp + 'px';
      } else {
        rulerZeroPoint =
          documentEditor.hRuler.zeroPosition * documentEditor.zoomFactor - 20;
        finalValueTemp =
          rulerZeroPoint + (paraStart + paraWidth) * documentEditor.zoomFactor;
        indent.style.left = finalValueTemp + 'px';
      }
    }
  }
  ...
  public updateTabStopMarkers(documentEditor: DocumentEditor): void {
    ...
    this.markIndicator.style.display =
      documentEditor.layoutType === 'Pages' ? 'block' : 'none';
    ...
    const rulerSize: Size = this.getRulerSize(documentEditor);
    const RenderedTabElement: HTMLElement[] =
      HelperMethods.convertNodeListToArray(
        document.querySelectorAll('.e-de-ruler-tab')
      );
    for (let i: number = 0; i < tabs.length; i++) {
      ...
      const id: string =
        documentEditor.element.id + '_' + justification + 'Tab_' + i.toString();
      const tabMarker: HTMLElement = document.getElementById(id);
      if (!isNullOrUndefined(tabMarker)) {
        if (
          !isNullOrUndefined(RenderedTabElement) &&
          RenderedTabElement.length > 0
        ) {
    ...
    if (!isNullOrUndefined(RenderedTabElement)) {
      for (let i: number = 0; i < RenderedTabElement.length; i++) {
        const elementToRemove: HTMLElement =
          RenderedTabElement[parseInt(i.toString(), 10)];
  ...
  private renderRulerMargins(
    ...
    const height: number = isHorizontal
      ? documentEditor.selectionModule.end.paragraph.bodyWidget.page
          .boundingRectangle.x
      : documentEditor.selectionModule.end.paragraph.bodyWidget.page
          .boundingRectangle.y + rulerSize.height;
    ...
    const rulerMargin: string = isHorizontal
      ? 'margin-left:' + leftMarginValue + 'px;'
      : 'margin-top:' + leftMarginValue + 'px;';
    const rulerHeight: number = isHorizontal
      ? rulerSize.height
      : rulerGeometry.height -
        HelperMethods.convertPointToPixel(
          documentEditor.selectionModule.end.paragraph.bodyWidget.sectionFormat
            .topMargin +
            documentEditor.selectionModule.end.paragraph.bodyWidget
    ...
    if (isHorizontal) {
      for (let i: number = 1; i <= 13; i++) {
        this.rulerMarginDiv = document.getElementById(
          documentEditor.element.id + '_hRuler_Margin' + i
        );
        if (!this.rulerMarginDiv) {
          const rulerstyle: string =
            'height:' +
            rulerHeight +
            'px;overflow:hidden;width:' +
            (rulerGeometry.width -
            ...
            'font-size:9px;text-align: left;z-index: -1;display: block' +
            rulerMargin;
      ...
      if (!this.verticalRulerMarginDiv) {
        this.verticalRulerMarginDiv = document.getElementById(
          documentEditor.element.id + '_vRuler_Margin'
        );
        const rulerstyle: string =
          'height:' +
          rulerHeight +
          ...
          rulerSize.width +
          'px;position:absolute;' +
          'font-size:9px;text-align: left;z-index: -1;' +
          rulerMargin;
        const rulerattributes: Object = {
        ...
        this.verticalRulerMarginDiv = this.createHtmlElement(
          'div',
          rulerattributes
        );
      }
  ...
  private updateRulerMargins(documentEditor: DocumentEditor): void {
    ...
    const verticalRulerMarginDiv: HTMLElement = document.getElementById(
      documentEditor.element.id + '_vRuler_Margin'
    );
    const rulerMarginDivHeight: number =
      rulerGeometry.height -
      HelperMethods.convertPointToPixel(
        documentEditor.selectionModule.end.paragraph.bodyWidget.sectionFormat
          .bottomMargin *
  ...
  private updateHorizontalRulerMargin(documentEditor: DocumentEditor): void {
    ...
    const rulerMarginDivWidth: number =
      this.getRulerGeometry(documentEditor).width -
      HelperMethods.convertPointToPixel(
        documentEditor.selectionModule.end.paragraph.bodyWidget.sectionFormat
          .rightMargin *
          documentEditor.zoomFactor +
    ...
    for (let i: number = 0; i < 13; i++) {
      const horizontalRulerMarginDiv: HTMLElement = document.getElementById(
        documentEditor.element.id + '_hRuler_Margin' + (i + 1)
      );
      if (horizontalRulerMarginDiv) {
        if (
          (columns.length === 0 && !skipLoop) ||
          (documentEditor.layoutType === 'Continuous' && !skipLoop)
        ) {
          if (paraBidi || tableBidi) {
            const startValue: number =
              documentEditor.hRuler.zeroPosition * documentEditor.zoomFactor -
          ...
          if (documentEditor.layoutType === 'Continuous') {
            const paraWidth: number = !isNullOrUndefined(
              ...
              : this.position.paragraph.width;
            horizontalRulerMarginDiv.style.width =
              paraWidth * documentEditor.zoomFactor + 'px';
          } else {
          ...
          columns.length >= i + 1 &&
          documentEditor.layoutType === 'Pages'
        ) {
          if (paraBidi || tableBidi) {
            horizontalRulerMarginDiv.style.marginLeft = leftMarginValue + 'px';
            leftMarginValue -=
              ((currnLefttMargin + currentRightMargin) / columns.length) *
              documentEditor.zoomFactor;
            leftMarginValue =
              leftMarginValue +
              (columns[parseInt(i.toString(), 10)].width +
                columns[parseInt(i.toString(), 10)].space) *
                documentEditor.zoomFactor;
          } else {
            horizontalRulerMarginDiv.style.marginLeft = leftMarginValue + 'px';
            leftMarginValue -=
  ...
  public resizeVRulerMargins(
    isRulerTopMargin: boolean,
    ...
    if (isRulerTopMargin) {
      ...
      if (currentTopMargin < mousePosition) {
        ruler.scrollTop =
          currentScrollTop -
          HelperMethods.convertPointToPixel(mousePosition - currentTopMargin);
      } else {
        ruler.scrollTop =
          currentScrollTop +
          HelperMethods.convertPointToPixel(currentTopMargin - mousePosition);
      }
    } else {
      const bottomMargin: number =
        HelperMethods.convertPixelToPoint(rulerGeometry.height) - mousePosition;
      rulerMarginDiv.style.height =
        (
          rulerGeometry.height -
          HelperMethods.convertPointToPixel(
  ...
  private resizeRulerMargins(
    ...
    const rulerMarginDiv: HTMLElement = document.getElementById(
      documentEditor.element.id +
        (isHorizontal ? '_hRuler_Margin1' : '_vRuler_Margin')
    );
    const rulerGeometry: Size = this.getRulerGeometry(documentEditor);
    if (!isNullOrUndefined(isRulerLeftMargin) && isRulerLeftMargin) {
      rulerMarginDiv.style.width =
        (
          rulerGeometry.width -
          HelperMethods.convertPointToPixel(
            documentEditor.selectionModule.end.paragraph.bodyWidget
      ...
      if (currentLeftMargin < mousePosition) {
        ruler.scrollLeft =
          currentScrollLeft -
          HelperMethods.convertPointToPixel(mousePosition - currentLeftMargin);
      } else {
        ruler.scrollLeft =
          currentScrollLeft +
          HelperMethods.convertPointToPixel(currentLeftMargin - mousePosition);
      }
    } else {
      const rightMargin: number =
        HelperMethods.convertPixelToPoint(rulerGeometry.width) - mousePosition;
      rulerMarginDiv.style.width =
        (
          rulerGeometry.width -
          HelperMethods.convertPointToPixel(
  ...
  public renderIndents(
    ...
    this.firstLineIndent = document.getElementById(
      documentEditor.element.id + '_firstLineIndent'
    );
    if (!this.firstLineIndent) {
      ...
      const style: string =
        'height:' +
        (rulerSize.height - 3) / 2 +
        'px;overflow:hidden;width:12px;position:absolute;font-size:11px;text-align: left;z-index: 5;' +
        margin;
      const attributes: Object = {
      ...
      this.firstLineIndent.setAttribute(
        'title',
        locale.getConstant('First Line Indent')
      );
      // let svg: SVGElement;
      ...
      this.firstLineIndentPath.setAttribute(
        'd',
        'M 0.5 0.5 H 11.5 V 2.7128 L 6 5.4211 L 0.5 2.7128 V 0.5 Z'
      );
      ...
      this.firstLineIndent.addEventListener(
        'dblclick',
      ...
      let initialValue: number = HelperMethods.getNumberFromString(
        this.firstLineIndent.style.left
      ); // for mouse up event
      const initialValue2: number = HelperMethods.getNumberFromString(
        this.firstLineIndent.style.left
      ...
      this.firstLineIndent.addEventListener('mousedown', (e: MouseEvent) => {
        isDragging = true;
        firstLineOffset =
          e.clientX - this.firstLineIndent.getBoundingClientRect().left;
        initialValue = HelperMethods.getNumberFromString(
          this.firstLineIndent.style.left
        );
        ...
        const value: number =
          ...
          e.clientX -
          firstLineOffset -
          this.hRuler.getBoundingClientRect().left;
        let startValue: number =
          documentEditor.documentHelper.currentPage.boundingRectangle.x;
        startValue =
          documentEditor.layoutType === 'Continuous' ? 0 : startValue;
        const indicatorLineValue: number =
          startValue + (value - rulerZeroPoint) + 6;
        const lineSvg: HTMLElement = document.getElementById(
          documentEditor.element.id + '_hRuler_indicator_svg'
        );
        lineSvg.style.left = indicatorLineValue + 'px';
        ...
        if (isDragging) {
          ...
          if (documentEditor.layoutType === 'Pages') {
            if (documentEditor.selectionModule.paragraphFormat.bidi) {
              ...
              minValue =
                rightIndentValue +
                HelperMethods.convertPointToPixel(42) *
                  documentEditor.zoomFactor;
            } else {
              rulerZeroPoint =
                HelperMethods.convertPointToPixel(
      ...
      document.addEventListener('mouseup', () => {
        if (isDragging) {
          isDragging = false;
          let finalValue: number = HelperMethods.getNumberFromString(
            this.firstLineIndent.style.left
          );
          if (
            parseInt(this.firstLineIndent.style.left.replace('px', ''), 10) < 0
          ) {
            finalValue *= -1;
          }
          if (documentEditor.selectionModule.paragraphFormat.bidi) {
            documentEditor.editorModule.applyRulerMarkerValues(
              'firstLineIndent',
              finalValue,
              initialValue
            );
    ...
    this.hangingIndent = document.getElementById(
      documentEditor.element.id + '_hangingIndent'
    );
    if (!this.hangingIndent) {
      ...
      this.hangingIndent.setAttribute(
        'title',
        locale.getConstant('Hanging Indent')
      );
      ...
      this.hangingIndentPath.setAttribute(
        'd',
        'M 0.5 5.3211 H 11.5 V 3.1083 L 6 0.4 L 0.5 3.1083 V 5.3211 Z'
      );
      ...
      this.hangingIndent.addEventListener(
        'dblclick',
        function (event: MouseEvent): void {
          documentEditor.showDialog('Paragraph');
          event.stopPropagation();
      ...
      this.hangingIndent.addEventListener('mousedown', (e: MouseEvent) => {
        isDragging = true;
        hangingLineOffset =
          e.clientX - this.hangingIndent.getBoundingClientRect().left;
        initialValue = HelperMethods.getNumberFromString(
          this.hangingIndent.style.left
        );
        const rightIndent: HTMLElement = document.getElementById(
        ...
        const rightPosition: number = HelperMethods.getNumberFromString(
          rightIndent.style.left
        );
        let rulerZeroPoint: number;
        if (documentEditor.layoutType === 'Pages') {
          if (documentEditor.selectionModule.paragraphFormat.bidi) {
            ...
            minLimit =
              rightPosition +
              HelperMethods.convertPointToPixel(42) * documentEditor.zoomFactor;
            maxLimit =
              rulerZeroPoint -
              6 +
              HelperMethods.convertPointToPixel(
        ...
        leftIndent = document.getElementById(
          documentEditor.element.id + '_leftIndent'
        );
        const value: number =
          rulerZeroPoint +
      ...
      document.addEventListener('mouseup', () => {
        if (isDragging) {
          ...
          if (
            parseInt(this.hangingIndent.style.left.replace('px', ''), 10) < 0
          ) {
            finalValue *= -1;
          }
          if (documentEditor.selectionModule.paragraphFormat.bidi) {
    ...
    this.leftIndent = document.getElementById(
      documentEditor.element.id + '_leftIndent'
    );
    if (!this.leftIndent) {
      const margin: string =
        ...
        'px;';
      const style: string =
        'height:4px;top:11px;overflow:hidden;width:12px;position:absolute;font-size:11px;text-align: left;z-index: 5;' +
        margin;
      const attributes: Object = {
      ...
      this.leftIndentPath.setAttribute(
        'd',
        'M 0.5 3.5 H 11.5 V 0.5 H 0.5 V 3.5 Z'
      );
      ...
      this.leftIndent.addEventListener(
        'dblclick',
        function (event: MouseEvent): void {
          documentEditor.showDialog('Paragraph');
          event.stopPropagation();
      ...
      this.leftIndent.addEventListener('mousedown', (e: MouseEvent) => {
        ...
        isDragging = true;
        leftLineOffset =
          e.clientX - this.leftIndent.getBoundingClientRect().left;
        initialValue = HelperMethods.getNumberFromString(
          this.leftIndent.style.left
        );
        firstIndentInitialValue = HelperMethods.getNumberFromString(
          this.firstLineIndent.style.left
        );
          ...
          if (documentEditor.selectionModule.paragraphFormat.bidi) {
            ...
            maxLimit =
              ...
              HelperMethods.convertPointToPixel(
                ...
                documentEditor.zoomFactor;
            isHangingIndent =
              HelperMethods.getNumberFromString(this.hangingIndent.style.left) -
                rightPosition <=
              HelperMethods.getNumberFromString(
                this.firstLineIndent.style.left
              ) -
                rightPosition;
          } else {
      ...
      document.addEventListener('mousemove', (e: MouseEvent) => {
        if (isDragging) {
          ...
          this.firstLineIndent.style.left =
            firstIndentInitialValue + (value - initialValue) + 'px';
          let startValue: number =
            documentEditor.documentHelper.currentPage.boundingRectangle.x;
          startValue =
            documentEditor.layoutType === 'Continuous' ? 0 : startValue;
    ...
    this.rightIndent = document.getElementById(
      documentEditor.element.id + '_rightIndent'
    );
    if (!this.rightIndent) {
      const margin: string =
        ...
        'px;';
      const style: string =
        'height:7px;top:8px;overflow:hidden;width:12px;position:absolute;font-size:11px;text-align: left;z-index: 4;' +
        margin;
      const attributes: Object = {
      ...
      this.rightIndent.setAttribute(
        'title',
        locale.getConstant('Right Indent')
      );
      ...
      this.rightIndentPath.setAttribute(
        'd',
        'M 0.5 6.5 H 11.5 V 4.2872 L 6 1.5789 L 0.5 4.2872 V 6.5 Z'
      );
      ...
      this.rightIndent.addEventListener(
        'dblclick',
        function (event: MouseEvent): void {
          documentEditor.showDialog('Paragraph');
          event.stopPropagation();
      ...
      this.rightIndent.addEventListener('mousedown', (e: MouseEvent) => {
        isDragging = true;
        rightLineOffset =
          e.clientX - this.rightIndent.getBoundingClientRect().left;
        initialValue = HelperMethods.getNumberFromString(
          this.rightIndent.style.left
        );
        let rulerZeroPoint: number =
      ...
      document.addEventListener('mousemove', (e: MouseEvent) => {
        if (isDragging) {
          ...
          const firstLineIndent: HTMLElement = document.getElementById(
            documentEditor.element.id + '_firstLineIndent'
          );
          let maxValue: number;
          let minValue: number;
          if (documentEditor.selectionModule.paragraphFormat.bidi) {
            ...
            value =
              ...
              rightLineOffset -
              this.hRuler.getBoundingClientRect().left;
            const nearestElement: HTMLElement =
              HelperMethods.getNumberFromString(leftIndent.style.left) -
                value <=
              HelperMethods.getNumberFromString(firstLineIndent.style.left) -
                value
                ? leftIndent
                : firstLineIndent;
            const indentValue: number = HelperMethods.getNumberFromString(
              nearestElement.style.left
            );
            maxValue =
              indentValue -
              HelperMethods.convertPointToPixel(42) * documentEditor.zoomFactor;
            ...
            maxValue =
              rulerZeroPoint +
              documentEditor.documentHelper.currentPage.boundingRectangle
                .width *
                documentEditor.zoomFactor -
              6;
            minValue =
      ...
      document.addEventListener('mouseup', () => {
        if (isDragging) {
          isDragging = false;
          const finalValue: number = HelperMethods.getNumberFromString(
            this.rightIndent.style.left
          );
          if (documentEditor.selectionModule.paragraphFormat.bidi) {
            documentEditor.editorModule.applyRulerMarkerValues(
  ...
  public updateRuler(
    documentEditor: DocumentEditor,
    rerenderRuler: boolean
  ): void {
    /* eslint-disable-next-line max-len */
    if (
      documentEditor.isDestroyed ||
      (documentEditor.rulerHelper &&
        documentEditor.documentEditorSettings &&
        !documentEditor.documentEditorSettings.showRuler) ||
    ...
    if (
      isNullOrUndefined(documentEditor.hRuler) &&
      isNullOrUndefined(documentEditor.vRuler)
    ) {
      return;
    }
    if (documentEditor.selectionModule.isForward) {
    ...
    this.updateRulerDimension(
      documentEditor,
      documentEditor.hRuler,
      hOffset,
      rerenderRuler
    );
    this.updateRulerDimension(
      documentEditor,
      documentEditor.vRuler,
      hOffset,
      rerenderRuler
    );
  ...
  private removeTableMarkers(
    ...
    const renderedTableMarkers: HTMLElement[] =
      HelperMethods.convertNodeListToArray(
        document.querySelectorAll('.e-de-ruler-table-marker')
      );
    if (!isNullOrUndefined(renderedTableMarkers)) {
      for (let i: number = 0; i < renderedTableMarkers.length; i++) {
        const elementToRemove: HTMLElement =
          renderedTableMarkers[parseInt(i.toString(), 10)];
        if (!isNullOrUndefined(elementToRemove)) {
          elementToRemove.parentNode.removeChild(elementToRemove);
        }
      }
  ...
  private renderTableMarkers(
    ...
    const tablewidget: TableWidget = (
      this.position.paragraph.containerWidget as TableCellWidget
    ).ownerTable as TableWidget;
    const tableRowWidget: TableRowWidget = (
      (this.position.paragraph.associatedCell as TableCellWidget)
        .ownerRow as TableRowWidget
    ).clone();
    const cellWidgets: IWidget[] = tableRowWidget.childWidgets;
    let value: number = this.position.paragraph.associatedCell.ownerTable
      .tableFormat.bidi
      ? cellWidgets.length
      : 0;
    ...
    for (let i: number = 0; i <= cellWidgets.length; i++) {
      let tableMarker: HTMLElement = document.getElementById(
        documentEditor.element.id + '_tableMarker_' + value
      );
      //if (!tableMarker) {
      let margin: string;
      if (i === 0) {
        tableXPos =
          ((cellWidgets[parseInt(i.toString(), 10)] as TableCellWidget).x -
            (cellWidgets[parseInt(i.toString(), 10)] as TableCellWidget).margin
              .left) *
            documentEditor.zoomFactor +
          this.hRuler.scrollLeft;
        margin = 'left:' + (tableXPos - 4) + 'px;';
      } else {
        tableXPos =
          tableXPos +
          ((cellWidgets[i - 1] as TableCellWidget).width +
            (cellWidgets[i - 1] as TableCellWidget).margin.left +
            (cellWidgets[i - 1] as TableCellWidget).margin.right) *
            documentEditor.zoomFactor;
        margin = 'left:' + (tableXPos - 4) + 'px;';
      }
      const style: string =
        'height:' +
        ruler.thickness +
        'px;overflow:hidden;width:10px;position:absolute;font-size:11px;text-align: left;z-index: 4;' +
        margin;
      const attributes: Object = {
      ...
      tableMarker.setAttribute(
        'title',
        locale.getConstant('Move Table Column')
      );
      // Create an SVG element
      const attr: Object = {
      ...
      pathElement.setAttribute(
        'd',
        'M1 1V0H2V1H3V0H4V1H5V0H6V1H7V2H6V3H7V4H6V5H7V6H6V7H7V8H0V7H1V6H0V5H1V4H0V3H1V2H0V1H1ZM2 2V3H3V2H2ZM4 2V3H5V2H4ZM5 4H4V5H5V4ZM5 6H4V7H5V6ZM3 7V6H2V7H3ZM2 5H3V4H2V5Z'
      );
      ...
      tableMarker.addEventListener('mousedown', (e: MouseEvent) => {
        tableMarkerOffset =
          e.clientX - tableMarker.getBoundingClientRect().left;
        documentEditor.startXPosition = HelperMethods.convertPixelToPoint(
          e.clientX
        );
        documentEditor.isTableMarkerDragging = true;
        const cursorPoint: Point = new Point(e.clientX, e.clientY);
        const touchPoint: Point = documentEditor.viewer.findFocusedPage(
          cursorPoint,
          true,
          true
        );
        ...
        const tableWidget: TableWidget = (
          documentEditor.selectionModule.end.paragraph
            .containerWidget as TableCellWidget
        ).ownerTable as TableWidget;
        documentEditor.editorModule.tableResize.currentResizingTable =
          tableWidget;
        documentEditor.editorModule.tableResize.resizeNode = 0;
        documentEditor.editorModule.tableResize.resizerPosition =
          currentMarkerPostion;
        ...
        documentEditor.editorHistoryModule.initResizingHistory(
          touchPoint,
          documentEditor.editorModule.tableResize
        );
      ...
      document.addEventListener('mousemove', (e: MouseEvent) => {
        ...
        if (documentEditor.isTableMarkerDragging) {
          ...
          if (
            documentEditor.selectionModule.end.paragraph.associatedCell
              .ownerTable.tableFormat.bidi
          ) {
            const rulerGeometry: Size = this.getRulerGeometry(documentEditor);
            const rulerMarginDivWidth: number =
              rulerGeometry.width -
      ...
      document.addEventListener('mouseup', (e: MouseEvent) => {
        ...
        if (documentEditor.isTableMarkerDragging) {
          const cursorPoint: Point = new Point(e.clientX, e.clientY);
          const dragValue: number = this.position.paragraph.associatedCell
            .ownerTable.tableFormat.bidi
            ? documentEditor.startXPosition -
              HelperMethods.convertPixelToPoint(e.clientX)
            : HelperMethods.convertPixelToPoint(e.clientX) -
              documentEditor.startXPosition;
          documentEditor.editorModule.tableResize.handleResizing(
            cursorPoint,
            true,
            dragValue / documentEditor.zoomFactor
          );
  ...
  private updateRulerDimension(
    ...
    const isHorizontal: boolean =
      ruler.orientation === 'Horizontal' ? true : false;
    const rulerSize: Size = this.getRulerSize(documentEditor);
    const rulerGeometry: Size = this.getRulerGeometry(documentEditor);
    //const documentEditorRuler: DocumentEditorRulerModel = isHorizontal ? documentEditor.documentEditorSettings.rulerSettings.horizontalRuler : documentEditor.documentEditorSettings.rulerSettings.verticalRuler;
    ...
    if (isHorizontal) {
      rulerObj.style.marginLeft =
        (documentEditor.layoutType === 'Pages'
          ? documentEditor.selectionModule.end.paragraph.bodyWidget.page
              .boundingRectangle.x
          : 0) + 'px';
    } else {
      rulerObj.parentElement.style.display =
        documentEditor.layoutType === 'Pages' ? 'block' : 'none';
      rulerObj.style.marginTop =
        documentEditor.selectionModule.getPageTop(
          documentEditor.selectionModule.end.paragraph.bodyWidget.page
        ) + 'px';
    }
      ...
      if (documentEditor.layoutType === 'Pages') {
        ...
        if (paraBidi || tableBidi) {
          const rulerMarginDivWidth: number =
            ...
            HelperMethods.convertPointToPixel(
              documentEditor.selectionModule.end.paragraph.bodyWidget
                .sectionFormat.rightMargin * documentEditor.zoomFactor
            );
          rulerObj.scrollLeft =
            documentEditor.hRuler.zeroPosition * documentEditor.zoomFactor -
            rulerMarginDivWidth;
        ...
        if (paraBidi || tableBidi) {
          ...
          rulerObj.scrollLeft =
            (ruler.zeroPosition -
              documentEditor.viewer.clientActiveArea.width) *
              documentEditor.zoomFactor -
            20;
        } else {
          // 20 is set approximately to the web layout.
          rulerObj.scrollLeft =
            ruler.zeroPosition * documentEditor.zoomFactor - 20;
        }
      }
    } else {
      rulerObj.scrollTop =
  ...
  private updateRulerSpace(
    ...
    this.rulerSpaceDiv = document.getElementById(
      documentEditor.element.id +
        (isHorizontal ? '_hRuler_ruler_space' : '_vRuler_ruler_space')
    );
    if (this.rulerSpaceDiv && documentEditor && rulerGeometry) {
      this.rulerSpaceDiv.style.width =
        (isHorizontal
          ? rulerGeometry.width + ruler.segmentWidth * 2
          : ruler.thickness) + 'px';
      this.rulerSpaceDiv.style.height =
        (isHorizontal
          ? ruler.thickness
          : rulerGeometry.height + ruler.segmentWidth * 2) + 'px';
    }
  }
  ...
  public updateRulerDiv(
    ...
    // parent div
    this.parentRulerDiv = document.getElementById(
      documentEditor.element.id + (isHorizontal ? '_hRuler' : '_vRuler')
    );
    if (this.parentRulerDiv && documentEditor && rulerGeometry) {
      this.parentRulerDiv.style.width =
        (isHorizontal
          ? documentEditor.layoutType === 'Continuous'
            ? rulerGeometry.width / documentEditor.zoomFactor
            : rulerGeometry.width
          : ruler.thickness) + 'px';
      this.parentRulerDiv.style.height =
        (isHorizontal ? ruler.thickness : rulerGeometry.height) + 'px';
      this.parentRulerDiv = document.getElementById(
        documentEditor.element.id + '_overlapRuler'
      );
      if (this.parentRulerDiv) {
        // eslint-disable-next-line
        isHorizontal
          ? this.parentRulerDiv.style.height === ruler.thickness + 'px'
          : this.parentRulerDiv.style.width === ruler.thickness + 'px';
      }
    }
    if (isHorizontal) {
      if (this.hRulerBottom) {
        ...
        this.hRulerBottom.style.width =
          pageElement.getBoundingClientRect().width + 'px';
      }
    }
  ...
  private getRulerGeometry(documentEditor: DocumentEditor): Size {
    ...
    const height: number =
      documentEditor.selectionModule.end.paragraph.bodyWidget.page
        .boundingRectangle.height * documentEditor.zoomFactor;
    const width: number =
      documentEditor.selectionModule.end.paragraph.bodyWidget.page
        .boundingRectangle.width * documentEditor.zoomFactor;
  ...
  private getVerticalHeight(documentEditor: DocumentEditor): number {
    ...
    const containerHeight: number =
      documentEditor.element.getBoundingClientRect().height;
    if (pageheight < containerHeight) {
      return pageheight;
    } else {
      return (
        containerHeight -
        documentEditor.documentHelper.pages[0].boundingRectangle.y
      );
    }
  }
  ...
  public renderTab(
    ...
    switch (tabJustification) {
      case 'Left': {
        this.leftTab = document.getElementById(
          documentEditor.element.id + '_LeftTab' + '_' + i
        );
        if (!this.leftTab) {
          let style: string = '';
          if (!isNullOrUndefined(tabStop)) {
            ...
            style =
              'height:9px;overflow:hidden;top:7px;width:12px;position:absolute;font-size:11px;text-align: left;z-index: 4;' +
              margin;
          }
          const attributes: Object = {
          ...
          // Create a path element inside the SVG
          const pathElement: SVGElement = this.createSvgElement(
            'path',
            pathattr
          );
      ...
      case 'Center': {
        this.centerTab = document.getElementById(
          documentEditor.element.id + '_CenterTab' + '_' + i
        );
        if (!this.centerTab) {
          let style: string = '';
          if (!isNullOrUndefined(tabStop)) {
          ...
          this.centerTab.setAttribute(
            'title',
            locale.getConstant('Center Tab')
          );
          // let svg: SVGElement;
          // Create an SVG element
          const attr: Object = {
      ...
      case 'Right': {
        this.rightTab = document.getElementById(
          documentEditor.element.id + '_RightTab' + '_' + i
        );
        if (!this.rightTab) {
          let style: string = '';
          if (!isNullOrUndefined(tabStop)) {
      ...
      case 'Decimal': {
        this.decimalTab = document.getElementById(
          documentEditor.element.id + '_DecimalTab' + '_' + i
        );
        if (!this.decimalTab) {
          let style: string = '';
          if (!isNullOrUndefined(tabStop)) {
          ...
          this.decimalTab.setAttribute(
            'title',
            locale.getConstant('Decimal Tab')
          );
      ...
      case 'Bar': {
        this.barTab = document.getElementById(
          documentEditor.element.id + '_BarTab' + '_' + i
        );
        if (!this.barTab) {
          let style: string = '';
          if (!isNullOrUndefined(tabStop)) {
    ...
    if (!isNullOrUndefined(tabStop)) {
      const tabStopElement: HTMLElement = document.getElementById(
        documentEditor.element.id + '_' + tabJustification + 'Tab' + '_' + i
      );
      if (!isNullOrUndefined(tabStop)) {
        tabStopElement.addEventListener(
          'dblclick',
          function (event: MouseEvent): void {
      ...
      tabStopElement.addEventListener('mousedown', (e: MouseEvent) => {
        ...
        tabIndex = parseInt(
          tabStopElement.id.split('_')[tabStopElement.id.split('_').length - 1],
          10
        );
        currentTabStop =
          currrentParagraph.paragraphFormat.tabs[
            parseInt(tabIndex.toString(), 10)
          ];
        ...
        startValue =
          documentEditor.layoutType === 'Continuous' ? 0 : startValue;
        const indicatorLineValue: number =
          startValue + (value - rulerZeroPoint);
        const lineSvg: HTMLElement = document.getElementById(
          documentEditor.element.id + '_hRuler_indicator_svg'
        );
        lineSvg.style.left = indicatorLineValue + 'px';
      ...
      document.addEventListener('mousemove', (e: MouseEvent) => {
        if (isDragging) {
          ...
          const leftIndentValue: number = HelperMethods.getNumberFromString(
            leftIndent.style.left
          );
      ...
      document.addEventListener('mouseup', () => {
        if (isDragging && !isNullOrUndefined(currentTabStop)) {
          if (
            !isNullOrUndefined(this.currentTabStopElement) &&
            this.currentTabStopElement.style.display === 'none'
          ) {
            documentEditor.editorModule.removeTabStops(
              [currrentParagraph],
              [currentTabStop]
            );
            this.currentTabStopElement.parentNode.removeChild(
              this.currentTabStopElement
            );
          } else {
            let finalValue: number = HelperMethods.getNumberFromString(
              tabStopElement.style.left
            ...
            finalValue =
              HelperMethods.convertPixelToPoint(
                finalValue / documentEditor.zoomFactor
              ) - 1584;
            finalValue = currrentParagraph.paragraphFormat.bidi
              ? finalValue * -1
              : finalValue;
            currentTabStop.position = finalValue;
            documentEditor.editorModule.updateTabStopCollection(
              currrentParagraph,
              [currentTabStop]
            );
          }
  ...
  public updateMargin(
    ...
    if (isHorizontal) {
      ...
      ruler.startMargin =
        documentEditor.selectionModule.end.paragraph.bodyWidget.sectionFormat.topMargin;
      ruler.endMargin =
  ...
  private unWireEvents(): void {
    if (this.markIndicator) {
      this.markIndicator.removeEventListener(
        'click',
        this.onmarkIndicatorClickHandler
      );
    }
    ...
    // }
    if (this.rulerDiv) {
      this.rulerDiv.removeEventListener(
        'dblclick',
        this.onRulerDblClickHandler
      );
    }
