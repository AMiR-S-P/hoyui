...
import { isNullOrUndefined, L10n, EmitType, Browser } from '@hoyui/base';
import { Save } from '@hoyui/file-utils';
import {
...
import {
  ...
  LineWidget,
  TextElementBox,
} from './implementation/viewer/page';
import {
  internalZoomFactorChange,
  contentChangeEvent,
  documentChangeEvent,
  selectionChangeEvent,
  zoomFactorChangeEvent,
  beforeFieldFillEvent,
  afterFieldFillEvent,
  serviceFailureEvent,
  viewChangeEvent,
  customContextMenuSelectEvent,
  customContextMenuBeforeOpenEvent,
  internalviewChangeEvent,
  internalDocumentEditorSettingsChange,
  trackChanges,
  internalOptionPaneChange,
} from './base/constants';
...
import { ZipArchiveItem, ZipArchive } from '@hoyui/compression';
...
import { ColorPickerModel } from '@hoyui/inputs';
import { MentionModel } from '@hoyui/dropdowns';
...
import { DialogUtility, hideSpinner, showSpinner } from '@hoyui/popups';
import { Comment } from './base/events-helper';
import { FieldSettingsModel } from '@hoyui/dropdowns';
/**
...
export class DocumentEditorSettings extends ChildProperty<DocumentEditorSettings> {
  ...
  @Property({
    roomName: '',
    editableRegionColor: '#22b24b',
    lockedRegionColor: '#f44336',
  })
  ...
  implements INotifyPropertyChanged
{
      ...
      for (const comment of this.documentHelper.comments) {
        ...
        for (const replyComment of comment.replyComments) {
          ...
          const commentText: string =
            HelperMethods.parseCommentAsText(replyComment);
          const newComment: Comment = new Comment(
            comment.commentId,
            replyCommentProperties,
            commentText
          );
  ...
  public get isDocumentEmpty(): boolean {
    if (this.documentHelper.pages.length === 1) {
      ...
      if (
        !isNullOrUndefined(headerWidget) &&
        !isNullOrUndefined(headerWidget.firstChild)
      ) {
        if (
          !(headerWidget.firstChild instanceof ParagraphWidget) ||
          !headerWidget.firstChild.isEmpty()
        ) {
          isHeaderEmpty = false;
        }
      }
      if (
        !isNullOrUndefined(footerWidget) &&
        !isNullOrUndefined(footerWidget.firstChild)
      ) {
        if (
          !(footerWidget.firstChild instanceof ParagraphWidget) ||
          !footerWidget.firstChild.isEmpty()
        ) {
          isFooterEmpty = false;
        }
      }
      if (isHeaderEmpty && isFooterEmpty) {
        ...
        if (isNullOrUndefined(firstBodywidget.nextWidget)) {
          ...
          if (
            firstChild instanceof ParagraphWidget &&
            firstChild.isEmpty() &&
  ...
  public get enableHistoryMode(): boolean {
    return (
      this.enableEditorHistory && !isNullOrUndefined(this.editorHistoryModule)
    );
  }
  /**
  ...
  public get isReadOnlyMode(): boolean {
    return (
      this.isReadOnly ||
      isNullOrUndefined(this.editorModule) ||
      isNullOrUndefined(this.selectionModule) ||
      (!isNullOrUndefined(this.editorModule) &&
        this.editorModule.restrictEditing)
    );
  }
  /**
   * @private
  ...
  public get enableImageResizerMode(): boolean {
    return (
      this.enableImageResizer && !isNullOrUndefined(this.imageResizerModule)
    );
  }
  /**
  ...
  protected preRender(): void {
    if (
      ...
      this.documentEditorSettings.enableOptimizedTextMeasuring
    ) {
  ...
  public showHideRulers(): void {
    if (
      this.rulerHelper &&
      this.documentEditorSettings &&
      !isNullOrUndefined(!this.documentEditorSettings.showRuler)
    ) {
      const showRuler: boolean =
        this.documentEditorSettings.showRuler && !this.isReadOnlyMode;
  ...
  private renderNavigationPane(): void {
    if (!this.isReadOnly && this.optionsPaneModule) {
      this.optionsPaneModule.showHideOptionsPane(
        this.documentEditorSettings.showNavigationPane
      );
    }
  }
  /**
    ...
    for (const prop of Object.keys(model)) {
      switch (prop) {
        case 'enableTrackChanges':
          ...
          if (
            this.documentHelper.isTrackedOnlyMode &&
            !model.enableTrackChanges
          ) {
        ...
        case 'zoomFactor':
          if (this.viewer && oldProp.zoomFactor !== model.zoomFactor) {
            ...
            if (
              this.rulerHelper &&
              this.documentEditorSettings &&
              this.documentEditorSettings.showRuler
            ) {
              this.rulerHelper.updateRuler(this, true);
            }
            if (
              this.enableCollaborativeEditing &&
              this.collaborativeEditingHandlerModule
            ) {
        ...
        case 'layoutType':
          ...
          const paragraph: ParagraphWidget =
            this.selectionModule.start.paragraph;
        ...
        case 'isReadOnly':
          if (
            !isNullOrUndefined(this.optionsPaneModule) &&
            this.optionsPaneModule.isOptionsPaneShow
          ) {
          ...
          this.trackChangesPane.enableDisableButton(
            !this.isReadOnly && !this.documentHelper.isDocumentProtected
          );
          this.showHideRulers();
          break;
        case 'currentUser':
        ...
        case 'documentSettings':
          if (!isNullOrUndefined(model.documentSettings.compatibilityMode)) {
            let oldValue: CompatibilityMode =
              oldProp.documentSettings.compatibilityMode;
            let newValue: CompatibilityMode =
              model.documentSettings.compatibilityMode;
            if (
              (oldValue == 'Word2013' && newValue != 'Word2013') ||
              (oldValue != 'Word2013' && newValue == 'Word2013')
            ) {
        ...
        case 'documentEditorSettings':
          if (
            !isNullOrUndefined(
              model.documentEditorSettings.enableOptimizedTextMeasuring
            )
          ) {
          ...
          if (
            (!isNullOrUndefined(model.documentEditorSettings.showHiddenMarks) &&
              model.documentEditorSettings.showHiddenMarks !==
                oldProp.documentEditorSettings.showHiddenMarks) ||
            (!isNullOrUndefined(model.documentEditorSettings.showBookmarks) &&
              model.documentEditorSettings.showBookmarks !==
                oldProp.documentEditorSettings.showBookmarks)
          ) {
          ...
          if (
            !isNullOrUndefined(
              model.documentEditorSettings.highlightEditableRanges
            )
          ) {
            if (
              this.documentHelper &&
              ...
              this.documentHelper.restrictEditingPane.highlightCheckBox.checked =
                model.documentEditorSettings.highlightEditableRanges;
            }
          }
          if (
            !isNullOrUndefined(model.documentEditorSettings.colorPickerSettings)
          ) {
          ...
          if (
            !isNullOrUndefined(model.documentEditorSettings.showNavigationPane)
          ) {
            if (!this.isReadOnly && this.optionsPaneModule) {
              this.optionsPaneModule.showHideOptionsPane(
                this.documentEditorSettings.showNavigationPane
  ...
  private applyColorPickerProperties(model: DocumentEditorModel): void {
    if (model.documentEditorSettings.colorPickerSettings) {
      ...
      if (
        ...
        this.bordersAndShadingDialogModule &&
        this.documentHelper.borderColorPicker &&
        this.documentHelper.shadingColorPicker
      ) {
        // setting border and shading color picker properties
        if (!isNullOrUndefined(settings.mode)) {
          this.documentHelper.borderColorPicker.mode =
            model.documentEditorSettings.colorPickerSettings.mode;
          this.documentHelper.shadingColorPicker.mode =
            model.documentEditorSettings.colorPickerSettings.mode;
        }
        if (!isNullOrUndefined(settings.showButtons)) {
          this.documentHelper.borderColorPicker.showButtons =
            model.documentEditorSettings.colorPickerSettings.showButtons;
          this.documentHelper.shadingColorPicker.showButtons =
            model.documentEditorSettings.colorPickerSettings.showButtons;
        }
        if (!isNullOrUndefined(settings.modeSwitcher)) {
          this.documentHelper.borderColorPicker.modeSwitcher =
            model.documentEditorSettings.colorPickerSettings.modeSwitcher;
          this.documentHelper.shadingColorPicker.modeSwitcher =
            model.documentEditorSettings.colorPickerSettings.modeSwitcher;
        }
      }
      ...
      if (
        !this.isReadOnly &&
        !isNullOrUndefined(settings) &&
        this.fontDialogModule &&
        this.documentHelper.fontColor
      ) {
        if (!isNullOrUndefined(settings.mode)) {
          this.documentHelper.fontColor.mode =
            model.documentEditorSettings.colorPickerSettings.mode;
        }
        if (!isNullOrUndefined(settings.showButtons)) {
          this.documentHelper.fontColor.showButtons =
            model.documentEditorSettings.colorPickerSettings.showButtons;
        }
        if (!isNullOrUndefined(settings.modeSwitcher)) {
          this.documentHelper.fontColor.modeSwitcher =
            model.documentEditorSettings.colorPickerSettings.modeSwitcher;
        }
      }
      ...
      if (
        !this.isReadOnly &&
        !isNullOrUndefined(settings) &&
        this.styleDialogModule &&
        this.documentHelper.colorPicker
      ) {
        if (!isNullOrUndefined(settings.mode)) {
          this.documentHelper.colorPicker.mode =
            model.documentEditorSettings.colorPickerSettings.mode;
        }
        if (!isNullOrUndefined(settings.showButtons)) {
          this.documentHelper.colorPicker.showButtons =
            model.documentEditorSettings.colorPickerSettings.showButtons;
        }
        if (!isNullOrUndefined(settings.modeSwitcher)) {
          this.documentHelper.colorPicker.modeSwitcher =
            model.documentEditorSettings.colorPickerSettings.modeSwitcher;
        }
      }
      ...
      if (!isNullOrUndefined(settings)) {
        if (!isNullOrUndefined(settings.mode)) {
          this.documentHelper.fontColorInputElement.mode =
            model.documentEditorSettings.colorPickerSettings.mode;
          this.documentHelper.shadingBtn.mode =
            model.documentEditorSettings.colorPickerSettings.mode;
          this.documentHelper.borderBtn.mode =
            model.documentEditorSettings.colorPickerSettings.mode;
        }
        if (!isNullOrUndefined(settings.showButtons)) {
          this.documentHelper.fontColorInputElement.showButtons =
            model.documentEditorSettings.colorPickerSettings.showButtons;
          this.documentHelper.shadingBtn.showButtons =
            model.documentEditorSettings.colorPickerSettings.showButtons;
          this.documentHelper.borderBtn.showButtons =
            model.documentEditorSettings.colorPickerSettings.showButtons;
        }
        if (!isNullOrUndefined(settings.modeSwitcher)) {
          this.documentHelper.fontColorInputElement.modeSwitcher =
            model.documentEditorSettings.colorPickerSettings.modeSwitcher;
          this.documentHelper.shadingBtn.modeSwitcher =
            model.documentEditorSettings.colorPickerSettings.modeSwitcher;
          this.documentHelper.borderBtn.modeSwitcher =
            model.documentEditorSettings.colorPickerSettings.modeSwitcher;
        }
      }
    }
  ...
  private localizeDialogs(enableRtl?: boolean): void {
    if (this.locale !== '') {
      ...
      if (this.tablePropertiesDialogModule) {
        this.tablePropertiesDialogModule.initTablePropertyDialog(
          l10n,
          enableRtl
        );
      }
      if (this.bordersAndShadingDialogModule) {
        this.bordersAndShadingDialogModule.initBordersAndShadingsDialog(
          l10n,
          enableRtl
        );
      }
      ...
      if (this.tableOfContentsDialogModule) {
        this.tableOfContentsDialogModule.initTableOfContentDialog(
          l10n,
          enableRtl
        );
      }
   ...
   * @returns {void}
   */
  public setDefaultCharacterFormat(
    characterFormat: CharacterFormatProperties
  ): void {
    this.characterFormat = JSON.parse(
      HelperMethods.sanitizeString(JSON.stringify(characterFormat))
    );
   ...
   * @returns {void}
   */
  public setDefaultParagraphFormat(
    paragraphFormat: ParagraphFormatProperties
  ): void {
    this.paragraphFormat = JSON.parse(
      HelperMethods.sanitizeString(JSON.stringify(paragraphFormat))
    );
    this.documentHelper.setDefaultDocumentFormat();
    if (!isNullOrUndefined(this.selectionModule)) {
      this.selectionModule.retrieveCurrentFormatProperties();
  ...
  public setDefaultSectionFormat(sectionFormat: SectionFormatProperties): void {
    this.sectionFormat = JSON.parse(
      HelperMethods.sanitizeString(JSON.stringify(sectionFormat))
    );
    this.documentHelper.setDefaultDocumentFormat();
    if (!isNullOrUndefined(this.selectionModule)) {
      this.selectionModule.retrieveCurrentFormatProperties();
    ...
    if (this.enableCollaborativeEditing) {
      ...
      if (this.isSettingOp) {
        ...
        if (!isNullOrUndefined(this.editorHistoryModule)) {
          if (
            !isNullOrUndefined(this.editorHistoryModule.currentHistoryInfo) &&
            this.editorHistoryModule.currentHistoryInfo.action === 'IMEInput'
          ) {
            eventArgs.operations =
              this.editorHistoryModule.currentHistoryInfo.getActionInfo();
          } else if (
            !isNullOrUndefined(this.editorHistoryModule.lastOperation)
          ) {
            let history: BaseHistoryInfo =
              this.editorHistoryModule.lastOperation;
  ...
  public fireSelectionChange(): void {
    ...
    if (
      !this.documentHelper.isCompositionStart &&
      Browser.isDevice &&
      this.editorModule
    ) {
      this.editorModule.predictText();
    }
    const eventArgs: SelectionChangeEventArgs = {
      source: this,
      isCompleted: this.documentHelper.isCompleted,
    };
  ...
  public showTableOptionsDialog(): void {
    if (this.tableOptionsDialogModule && !this.isReadOnlyMode && this.viewer) {
      ...
      this.checkModuleInjection(
        'TableOptionsDialog',
        this.enableTableOptionsDialog
      );
    }
  }
  /**
  ...
  public showPicContentControlDialog(): void {
    ...
    if (this.selection.isEmpty) {
      this.renderPictureContentControlElement(
        this,
        showPicCCButton,
        pictureElement,
        picturePositionY
    ...
    if (this.selection.isImageSelected) {
      ...
      if (
        !(
          contentControlImage instanceof ContentControl &&
  ...
  public showpicContentControlDialogModule(): void {
    if (
      this.picContentControlDialogModule &&
      !this.isReadOnlyMode &&
      this.viewer
    ) {
   ...
   * @returns {void}
   */
  public renderPictureContentControlElement(
    documentEditor: DocumentEditor,
    showPicContentControl: boolean,
    ...
    let left: number =
      this.documentHelper.owner.selection.sectionFormat.leftMargin;
    let top: number;
    ...
    const attributes: Object = {
      id: this.element.id + 'PICTURE_CONTENT_CONTROL',
      innerHTML: 'Picture',
      ...
      style:
        ...
        'px;top:' +
        top +
        'px;z-index:5;position:absolute;background-color:#ccc;border:1px solid #ccc;display:',
    };
    let pictureCC: HTMLElement;
    let contentControlImage: ElementBox;
    let element: HTMLElement = document.getElementById(
      this.element.id + '_viewerContainer'
    );
    let picture_cc: HTMLElement = document.getElementById(
      this.element.id + 'PICTURE_CONTENT_CONTROL'
    );
    if (
      (isNullOrUndefined(picture_cc) && showPicContentControl) ||
      pictureElement
    ) {
      ...
      !isNullOrUndefined(picture_cc) &&
      showPicContentControl &&
      !pictureElement
    ) {
      ...
      picture_cc.addEventListener('click', (event) => {
        ...
        if (
          contentControlImage instanceof ContentControl &&
          !contentControlImage.contentControlProperties.lockContentControl &&
          contentControlImage.contentControlProperties.type == 'Picture' &&
          !this.documentHelper.owner.isReadOnlyMode
        ) {
          this.picContentControlDialogModule.show();
        }
      });
  ...
  public getImageContentControl(): ElementBox {
    let contentControlImage: ElementBox;
    let contentControlPara =
      this.documentHelper.owner.selectionModule.start.currentWidget.paragraph;
    for (let i: number = 0; i < contentControlPara.childWidgets.length; i++) {
      for (
        let j: number = 0;
        j <
        ((contentControlPara as ParagraphWidget).childWidgets[i] as LineWidget)
          .children.length;
        j++
      ) {
        let contentControlImageCheck = (
          (contentControlPara as ParagraphWidget).childWidgets[i] as LineWidget
        ).children[j];
        if (
          contentControlImageCheck instanceof ContentControl &&
          contentControlImageCheck.type == 0 &&
          contentControlImageCheck.contentControlProperties.type == 'Picture'
        ) {
          contentControlImage = (
            (contentControlPara as ParagraphWidget).childWidgets[
              i
            ] as LineWidget
          ).children[j];
          break;
  ...
  public showContentPropertiesDialog(): void {
    if (
      this.contentControlPropertiesDialogModule &&
      !this.isReadOnlyMode &&
      this.viewer
    ) {
  ...
  public showTableOfContentsDialog(): void {
    if (
      this.tableOfContentsDialogModule &&
      !this.isReadOnlyMode &&
      this.viewer
    ) {
      ...
      this.checkModuleInjection(
        'TableOfContentsDialog',
        this.enableTableOfContentsDialog
      );
    }
  }
  /**
  ...
  public showTablePropertiesDialog(): void {
    if (
      this.tablePropertiesDialogModule &&
      !this.isReadOnlyMode &&
      this.viewer
    ) {
      ...
      this.checkModuleInjection(
        'TablePropertiesDialog',
        this.enableTablePropertiesDialog
      );
    }
  }
  /**
  ...
  protected requiredModules(): ModuleDeclaration[] {
    ...
    if (this.enableCollaborativeEditing) {
      modules.push({
        member: 'CollaborativeEditingHandler',
        args: [this],
      });
    }
    ...
    if (
      this.enableSfdtExport ||
      this.enableWordExport ||
      this.enableTextExport ||
      this.enableSelection ||
      this.enableEditor
    ) {
      modules.push({
        member: 'SfdtExport',
        args: [this.documentHelper],
      });
    }
    ...
    if (this.enableEditor) {
      ...
      if (this.enableImageResizer) {
        modules.push({
          member: 'ImageResizer',
          args: [this, this.documentHelper],
        });
      }
      if (this.enableEditorHistory) {
        modules.push({
          member: 'EditorHistory',
          args: [this],
        });
      }
      if (this.enableHyperlinkDialog) {
        modules.push({
          member: 'HyperlinkDialog',
          args: [this.documentHelper],
        });
      }
      ...
      if (this.enableStyleDialog) {
        ...
        modules.push({
          member: 'BulletsAndNumberingDialog',
          args: [this.documentHelper],
        });
      }
      ...
      if (this.enableTablePropertiesDialog) {
        modules.push({
          member: 'TablePropertiesDialog',
          args: [this.documentHelper],
        });
        modules.push({
          member: 'CellOptionsDialog',
          args: [this.documentHelper],
        });
      }
      if (this.enableBordersAndShadingDialog) {
        modules.push({
          member: 'BordersAndShadingDialog',
          args: [this.documentHelper],
        });
      }
      if (this.enableTableOptionsDialog) {
        modules.push({
          member: 'TableOptionsDialog',
          args: [this.documentHelper],
        });
      }
      if (this.enableSpellCheck) {
        ...
        modules.push({
          member: 'SpellCheckDialog',
          args: [this.documentHelper],
        });
      }
      if (this.enableFormField) {
        modules.push({
          member: 'TextFormFieldDialog',
          args: [this],
        });
        modules.push({
          member: 'DropDownFormFieldDialog',
          args: [this],
        });
        modules.push({
          member: 'CheckBoxFormFieldDialog',
          args: [this],
        });
      }
    }
  ...
  public checkModuleInjection(moduleName: string, value: boolean) {
    if (!value && !this.isReadOnly) {
      console.warn(
        `[WARNING] :: Module "${moduleName}" is not available in Document Editor component! You either misspelled the module name or forgot to load it.`
      );
  ...
  public defaultLocale: Object = {
    ...
    'Number format tooltip information':
      'Single-level number format: </br>[PREFIX]%[LEVELNUMBER][SUFFIX]</br>' +
    ...
    'Discard Content Control':
      'You can not insert a content control when the selection includes another content control.',
    ...
    'Unsupported format':
      "The file format you have selected isn't supported. Please choose valid format.",
    One: 'One',
    Two: 'Two',
    Three: 'Three',
    Presets: 'Presets',
    ...
    'Paste Content Dialog':
      'Due to browser’s security policy, paste from system clipboard is restricted. Alternatively use the keyboard shortcut',
    ...
    'Left and right margins.':
      'Settings you chose for the left and right margins, column spacing, or pargraph indents are too large for the page width in same secitions.',
  ...
  private processSfdt(sfdtText: string): void {
    if (!isNullOrUndefined(this.viewer) && !isNullOrUndefined(sfdtText)) {
      ...
      if (this.isSpellCheck) {
        if (
          this.isSpellCheck &&
          !this.spellCheckerModule.enableOptimizedSpellCheck
        ) {
      ...
      if (!isNullOrUndefined(sfdtText) && this.viewer) {
        ...
        this.documentHelper.onDocumentChanged(
          this.parser.convertJsonToDocument(sfdtText as string, incrementalOps),
          incrementalOps
        );
      }
      if (this.isSpellCheck) {
  ...
  private async getSfdtFromBase64string(value: string): Promise<string> {
    ...
    for (let i: number = 0; i < binaryString.length; i++) {
      byteArray[parseInt(i.toString(), 10)] = binaryString.charCodeAt(
  ...
  private send(formData: FormData): Promise<string> {
    const serviceUrl: string =
      this.serviceUrl + this.serverActionSettingsImport;
    return new Promise(
      (
        resolve: (value: string | PromiseLike<string>) => void,
        reject: (reason: any) => void
  ...
  public scrollToPage(pageNumber: number): boolean {
    if (
      isNullOrUndefined(this.viewer) ||
      pageNumber < 1 ||
      pageNumber > this.documentHelper.pages.length
    ) {
      return false;
    }
  ...
  public enableAllModules(): void {
    this.enablePrint =
      ...
      this.enableSearch =
      this.enableOptionsPane =
      this.enableEditor =
      this.enableImageResizer =
      this.enableEditorHistory =
      this.enableHyperlinkDialog =
      this.enableTableDialog =
      this.enableBookmarkDialog =
      this.enableTableOfContentsDialog =
      this.enableFootnoteAndEndnoteDialog =
      this.enablePageSetupDialog =
      this.enableStyleDialog =
      this.enableDateContentDialog =
      this.enablePicContentControlDialog =
      this.enableContentControlPropertiesDialog =
      this.enableListDialog =
      this.enableParagraphDialog =
      this.enableFontDialog =
      this.enableTablePropertiesDialog =
      this.enableBordersAndShadingDialog =
      this.enableTableOptionsDialog =
  ...
  public getFormFieldNames(): string[] {
    ...
    for (let i: number = 0; i < formFields.length; i++) {
      if (formFields[parseInt(i.toString(), 10)].formFieldData.name !== '') {
        formFieldNames.push(
          formFields[parseInt(i.toString(), 10)].formFieldData.name
        );
      }
    }
  ...
  public getFormFieldInfo(
    ...
    for (let i: number = 0; i < formFields.length; i++) {
      if (
        formFields[parseInt(i.toString(), 10)].formFieldData.name === name &&
        formFields[parseInt(i.toString(), 10)].formFieldData.name !== ''
      ) {
        return formFields[
          parseInt(i.toString(), 10)
        ].formFieldData.getFormFieldInfo();
      }
    }
    return undefined;
  }
  ...
  public setFormFieldInfo(
    ...
    for (let i: number = 0; i < formFields.length; i++) {
      if (
        ...
        const currentField: FieldElementBox =
          formFields[parseInt(i.toString(), 10)];
        if (this.selectionModule) {
  ...
  public resetFormFields(name?: string): void {
    ...
    for (let i: number = 0; i < formFields.length; i++) {
      if (
        ...
        name === formFields[parseInt(i.toString(), 10)].formFieldData.name
      ) {
        if (
          formFields[parseInt(i.toString(), 10)].formFieldData instanceof
          TextFormField
        ) {
          this.editorModule.updateFormField(
            formFields[parseInt(i.toString(), 10)],
            (
              formFields[parseInt(i.toString(), 10)]
                .formFieldData as TextFormField
            ).defaultValue,
            true
          );
        } else if (
          ...
          /* eslint-disable-next-line max-len */
          this.editorModule.toggleCheckBoxFormField(
            formFields[parseInt(i.toString(), 10)],
            true,
            (
              formFields[parseInt(i.toString(), 10)]
                .formFieldData as CheckBoxFormField
            ).defaultValue
          );
        } else if (
          formFields[parseInt(i.toString(), 10)].formFieldData instanceof
  ...
  public importFormData(formData: FormFieldData[]): void {
    ...
    for (let i: number = 0; i < formData.length; i++) {
      ...
      for (let j: number = 0; j < formField.length; j++) {
        if (
          formField[parseInt(j.toString(), 10)].formFieldData.name === fieldName
        ) {
          if (
            formField[parseInt(j.toString(), 10)].formFieldData instanceof
            CheckBoxFormField
          ) {
            this.editorModule.toggleCheckBoxFormField(
              formField[parseInt(j.toString(), 10)],
              true,
              formFieldData.value as boolean
            );
          } else if (
            formField[parseInt(j.toString(), 10)].formFieldData instanceof
            TextFormField
  ...
  public exportFormData(): FormFieldData[] {
    ...
    for (let i: number = 0; i < formField.length; i++) {
      if (formField[parseInt(i.toString(), 10)].formFieldData.name !== '') {
        ...
        formData.fieldName = (
          formField[parseInt(i.toString(), 10)].formFieldData as TextFormField
        ).name;
        if (
          formField[parseInt(i.toString(), 10)].formFieldData instanceof
          CheckBoxFormField
        ) {
          formData.value = (
            formField[parseInt(i.toString(), 10)]
              .formFieldData as CheckBoxFormField
          ).checked;
        } else if (
          formField[parseInt(i.toString(), 10)].formFieldData instanceof
          ...
          if (this.documentHelper.isInlineFormFillProtectedMode) {
            resultText = this.editorModule.getFieldResultText(
              formField[parseInt(i.toString(), 10)]
            );
          } else {
            resultText = formField[parseInt(i.toString(), 10)].resultText;
          }
          const rex: RegExp = RegExp(
            this.documentHelper.textHelper.getEnSpaceCharacter(),
            'gi'
          );
          ...
          formData.value = (
            formField[parseInt(i.toString(), 10)]
              .formFieldData as DropDownFormField
          ).selectedIndex;
        }
        data.push(formData);
      }
   ...
   */
  public importContentControlData(
    contentControlInfo: ContentControlInfo[]
  ): ContentControlInfo[] {
    for (let i: number = 0; i < contentControlInfo.length; i++) {
      const contentInfo: ContentControlInfo =
        contentControlInfo[parseInt(i.toString(), 10)];
      for (
        let j: number = 0;
        j < this.documentHelper.contentControlCollection.length;
        j++
      ) {
        ...
        if (
          ...
          this.editorModule.updateContentControl(
            contentControl,
            contentInfo.value
          );
        }
  ...
  public exportContentControlData(): ContentControlInfo[] {
    ...
    for (const contentControl of properties) {
      if (contentControl instanceof ContentControl) {
        ...
        contentControlData.title =
          contentControl.contentControlProperties.title;
        ...
        if (contentControl.contentControlProperties.type === 'Picture') {
          if (element instanceof ImageElementBox) {
            contentControlData.value =
              this.documentHelper.getImageString(element);
          }
        } else if (
          contentControl.contentControlProperties.type === 'CheckBox'
        ) {
          contentControlData.value = String(
          ...
          contentControl.contentControlProperties.type === 'ComboBox' ||
          contentControl.contentControlProperties.type === 'DropDownList'
        ) {
          contentControlData.value = this.getContentControlValue(
            element as TextElementBox
          );
        } else {
          contentControlData.value =
            this.getContentControlValueForText(contentControl);
        }
  ...
  public getContentControlValueForText(element: ContentControl): string {
    ...
    if (
      this.selection.contentControleditRegionHighlighters.containsKey(element)
    ) {
      ...
      for (let i: number = 0; i < contentInfo.keys.length; i++) {
        ...
        for (let j: number = startIndex; j < line.children.length; j++) {
          startIndex = 0;
          const textElement: ElementBox = line.children[
            parseInt(j.toString(), 10)
          ] as ElementBox;
  ...
  public updateFields(): void {
    ...
    for (let i: number = 0; i < this.documentHelper.fields.length; i++) {
      const field: FieldElementBox =
        this.documentHelper.fields[parseInt(i.toString(), 10)];
  ...
  public exportAsImage(
    ...
    if (this.printModule) {
      ...
      return this.printModule.exportAsImage(
        this.documentHelper,
        pageNumber,
        mimeType
      );
  ...
  public exportAsPath(pageNumber: number): string {
    if (
      ...
      const imageData: string =
        this.documentHelper.render.pageCanvas.toDataURL();
      (
        this.documentHelper.render.pageCanvas as DocumentCanvasElement
      ).getContext('2d').renderedPath = '';
  ...
  public saveAsBlob(formatType?: FormatType): Promise<Blob> {
    ...
    return new Promise((resolve: (value: Blob | PromiseLike<Blob>) => void) => {
      if (formatType === 'Docx' || formatType === 'Dotx') {
        if (this.wordExportModule) {
          resolve(
            this.wordExportModule.saveAsBlob(this.documentHelper, formatType)
          );
        } else {
          this.checkModuleInjection('WordExport', this.enableWordExport);
        }
        ...
        if (this.sfdtExportModule) {
          if (this.documentEditorSettings.optimizeSfdt) {
            this.sfdtExportModule
              .saveAsBlob(this.documentHelper)
              .then((blob: Blob) => {
            ...
            resolve(
              this.sfdtExportModule.saveAsBlobNonOptimized(this.documentHelper)
            );
          }
        } else {
          this.checkModuleInjection('SfdtExport', this.enableSfdtExport);
  ...
  public openBlank(): void {
    ...
    /* eslint-disable-next-line max-len */
    const hfs: HeaderFooters = this.parser.parseHeaderFooter(
      {
        header: {},
        footer: {},
        evenHeader: {},
    ...
    if (this.viewer) {
      ...
      if (this.editorModule) {
        ...
        const style: WStyle = this.documentHelper.styles.findByName(
          'Normal'
        ) as WStyle;
        for (let i: number = 0; i < sections.length; i++) {
          const paragraph: ParagraphWidget = sections[
            parseInt(i.toString(), 10)
  ...
  public toggleShowHiddenMarksInternal(): void {
    this.documentEditorSettings.showHiddenMarks =
      !this.documentEditorSettings.showHiddenMarks;
    this.notify(
      internalDocumentEditorSettingsChange,
      this.documentEditorSettings
  ...
  public showXmlPane(): void {
    if (
      !isNullOrUndefined(this.xmlPaneModule) &&
      isNullOrUndefined(this.xmlPaneModule.element)
    ) {
  ...
  public updateStyle(styleInCollection: WStyle, style: WStyle): void {
    if (!isNullOrUndefined(this.styleDialogModule)) {
      ...
      if (type === 'Paragraph' || type === 'Linked Style') {
        ...
        (styleInCollection as WParagraphStyle).characterFormat.copyFormat(
          (style as WParagraphStyle).characterFormat
        );
        const oldListId: number = (styleInCollection as WParagraphStyle)
          .paragraphFormat.listFormat.listId;
        (styleInCollection as WParagraphStyle).paragraphFormat.destroy();
        (styleInCollection as WParagraphStyle).paragraphFormat.copyFormat(
        ...
        if (!isNullOrUndefined(oldListId) && oldListId > -1) {
          ...
          if (!isNullOrUndefined(list)) {
            this.documentHelper.lists.splice(
              this.documentHelper.lists.indexOf(list),
              1
            );
          }
        }
        ...
        (styleInCollection as WCharacterStyle).characterFormat.copyFormat(
          (style as WCharacterStyle).characterFormat
        );
      }
  ...
  public setStyleData(name: string, listId?: number): void {
    ...
    if (
      !isNullOrUndefined(name) &&
      !isNullOrUndefined(this.documentHelper.owner.sfdtExportModule)
    ) {
  ...
  public getStyleObject(style: Object, listId: number): any {
    if (!isNullOrUndefined(style)) {
      const keyIndex: number =
        this.documentHelper.owner.sfdtExportModule.keywordIndex;
      this.documentHelper.owner.sfdtExportModule.keywordIndex = 1;
      const styleData: any =
        this.documentHelper.owner.sfdtExportModule.writeStyle(style as WStyle);
      const styleObject: any = {
        optimizeSfdt: true,
        sty: [styleData],
      };
      if (this.editorModule.isLinkedStyle((style as WStyle).name)) {
        const linkedStyle: WStyle = this.documentHelper.styles.findByName(
          (style as WStyle).name + ' Char'
        ) as WStyle;
        const linkedStyleData: any =
          this.documentHelper.owner.sfdtExportModule.writeStyle(linkedStyle);
      ...
      if (!isNullOrUndefined(listId) && listId > -1) {
        ...
        styleObject[listsProperty[1]].push(
          this.sfdtExportModule.writeList(list)
        );
        styleObject[abstractListsProperty[1]] = [];
        styleObject[abstractListsProperty[1]].push(
          this.sfdtExportModule.writeAbstractList(list.abstractList)
        );
      }
