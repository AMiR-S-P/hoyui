...
export class WUniqueFormats {
   ...
   * @private
   */
  public addUniqueFormat(
    format: Dictionary<number, object>,
    type: number
  ): WUniqueFormat {
   ...
   * @private
   */
  public updateUniqueFormat(
    uniqueFormat: WUniqueFormat,
    property: string,
    value: object
  ): WUniqueFormat {
    let matchedFormat: WUniqueFormat = undefined;
    ...
    if (isNullOrUndefined(matchedFormat)) {
      ...
      matchedFormat.cloneItems(
        uniqueFormat,
        property,
        value,
        uniqueFormat.uniqueFormatType
      );
      matchedFormat.referenceCount = 1;
      this.items.push(matchedFormat);
    } else {
