...
export class XmlHttpRequestHandler {
  ...
  public send(
    jsonObject: object,
    httpRequestEventArgs?: XmlHttpRequestEventArgs,
    isAsync?: boolean
  ): void {
    ...
    if (!isNullOrUndefined(httpRequestEventArgs)) {
      this.xmlHttpRequest.withCredentials =
        httpRequestEventArgs.withCredentials;
      timeout =
        httpRequestEventArgs.timeout >= 0 ? httpRequestEventArgs.timeout : 0;
  ...
  private sendRequest(
    ...
    const data: FormData | string =
      jsonObj instanceof FormData ? jsonObj : JSON.stringify(jsonObj);
    this.xmlHttpRequest.send(data); // jshint ignore:line
  }

  private stateChange(proxyReq: XmlHttpRequestHandler): void {
    if (
      proxyReq.xmlHttpRequest.readyState === 4 &&
      proxyReq.xmlHttpRequest.status === 200
    ) {
      let data: any;
      if (this.responseType) {
