...
describe('Editor functions', () => {
  ...
  it('Insert Footnote enter text after backspace validation', () => {
    ...
    let body: BlockContainer = editor.documentHelper.pages[0].footnoteWidget
  ...
  it('Insert Footnote validation', () => {
    ...
    let body: BlockContainer = editor.documentHelper.pages[0].footnoteWidget
      .bodyWidgets[0] as BodyWidget;
    expect(
      (body.childWidgets[0] as BlockWidget).y <
        (body.childWidgets[1] as BlockWidget).y
    ).toBe(true);
  });
  ...
  it('EditRangeElement paragraph and table validation with removeUserRestrictions', () => {
    console.log(
      'EditRangeElement paragraph and table validation with removeUserRestrictions'
    );
    // editor.openBlank();
  ...
  it('Enforce Protection Async ', () => {
    ...
    expect(() => {
      editor.editorModule.stopProtectionAsync('123');
    }).not.toThrowError();
  });
});
