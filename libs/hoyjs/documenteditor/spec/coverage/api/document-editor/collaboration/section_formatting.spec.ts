...
describe('Validate the operation for section format', () => {
  ...
  beforeAll(() => {
    ...
    (editor2.documentHelper as any).containerCanvasIn =
      TestHelper.containerCanvas;
    (editor2.documentHelper as any).selectionCanvasIn =
      TestHelper.selectionCanvas;
    (editor2.documentHelper.render as any).pageCanvasIn = TestHelper.pageCanvas;
    (editor2.documentHelper.render as any).selectionCanvasIn =
      TestHelper.pageSelectionCanvas;
  ...
  it('Apply section format', () => {
    ...
    let connections: CollaborativeEditingHandler =
      editor2.collaborativeEditingHandlerModule;
    ...
    expect(argsEle.operations[0].type).toBe('SectionFormat');
    //Remote action
    expect(editor2.selection.sectionFormat.endnoteNumberFormat).toBe(
      'UpperCaseLetter'
    );
    ...
    expect(argsEle.operations[0].type).toBe('SectionFormat');
    //Remote action
    expect(editor2.selection.sectionFormat.footNoteNumberFormat).toBe(
      'UpperCaseLetter'
    );

    editor1.selection.sectionFormat.restartIndexForEndnotes =
    ...
    expect(argsEle.operations[0].type).toBe('SectionFormat');
    //Remote action
    expect(editor2.selection.sectionFormat.restartIndexForEndnotes).toBe(
      'RestartForEachSection'
    );

    editor1.selection.sectionFormat.restartIndexForFootnotes =
      'RestartForEachSection';
    expect(argsEle.operations[0].format).toBeDefined();
    expect(argsEle.operations[0].length).toBe(1);
    expect(argsEle.operations[0].offset).toBe(1);
    expect(argsEle.operations[0].type).toBe('SectionFormat');
    //Remote action
    expect(editor2.selection.sectionFormat.restartIndexForFootnotes).toBe(
      'RestartForEachSection'
    );
