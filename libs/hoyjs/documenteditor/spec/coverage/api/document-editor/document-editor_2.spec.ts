...
import { Search } from '@hoyui/dropdowns';
...
describe('checkModuleInjection', function () {
  ...
  it('should inject the Selection module if it is not already present', function () {
    ...
    var result = editor.checkModuleInjection(
      'Selection',
...
describe('ContentControl API', () => {
  ...
  it('ContentControl import', () => {
    ...
    documenteditor.editor.insertContentControl('ComboBox', 'One', [
      'One',
      'Two',
      'Three',
    ]);
    ...
    let contentControlData: ContentControlInfo = {
      title: placeHolderPrefix + 'Name',
      tag: '',
      value: 'John',
      canDelete: false,
      canEdit: false,
    ...
    expect(documenteditor.documentHelper.contentControlCollection.length).toBe(
      4
    );
  });
  it('ContentControl export', () => {
    ...
    documenteditor.editor.insertContentControl('Text', 'Hello World');
    let contentControlInfos: ContentControlInfo[] =
      documenteditor.exportContentControlData();
