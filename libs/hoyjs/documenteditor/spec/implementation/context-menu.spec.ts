...
describe('Context Menu Testing - 1', () => {
  ...
  it('Context Menu Opening Texting without selection', () => {
    ...
    for (
      let i: number = 0;
      i < (menu.contextMenuInstance.items as MenuItemModel[]).length;
      i++
    ) {
      if (
        (menu.contextMenuInstance.items as MenuItemModel[])[i].text === 'Cut' ||
        (menu.contextMenuInstance.items as MenuItemModel[])[i].text === 'Copy'
      ) {
        let element = document.getElementById(
          (menu.contextMenuInstance.items as MenuItemModel[])[i].id
        );
...
describe('Context Menu Testing - 2', () => {
  ...
  it('open Contextmenu in CommentOnly mode', () => {
    ...
    for (
      ...
      if (
        (menu.contextMenuInstance.items as MenuItemModel[])[i].text ===
        'New Comment'
      ) {
        let element = document.getElementById(
          (menu.contextMenuInstance.items as MenuItemModel[])[i].id
  ...
  it('open content Menu inside table', () => {
    ...
    for (
      ...
      let text: string = (menu.contextMenuInstance.items as MenuItemModel[])[i]
        .text;
      if (
  ...
  it('Open context menu in hyperlink', () => {
    ...
    editor.editorModule.insertHyperlinkInternal(
...
describe('handle Context menu item validation-1 for editing', () => {
  ...
  it('paragraph dialog and hyperlink dilaog open using context menu', () => {
    console.log(
      'paragraph dialog and hyperlink dilaog open using context menu'
    );
    editor.openBlank();
    editor.editorModule.insertText('Syncfusion software');
    let event: MouseEvent = document.createEvent('MouseEvent');
  ...
  it('paragraph and hyperlink dialog open using contextmenu validation', () => {
    console.log(
      'paragraph and hyperlink dialog open using contextmenu validation'
    );
    editor.openBlank();
...
describe('Apply table auto fit types', () => {
  ...
  it('Apply auto fit type fit to contents', () => {
    ...
    editor.contextMenu.handleContextMenuItem(
      'container_contextmenu_auto_fit_contents'
    );
    expect(
      editor.selection.start.paragraph.associatedCell.ownerTable.tableFormat
        .allowAutoFit
    ).toBe(true);
  });
  it('Apply auto fit type fit to window', () => {
    ...
    editor.contextMenu.handleContextMenuItem(
      'container_contextmenu_auto_fit_window'
    );
    expect(
      editor.selection.start.paragraph.associatedCell.ownerTable.tableFormat
        .preferredWidthType
  ...
  it('Apply auto fit type fit to contents', () => {
    console.log('Apply auto fit type fit to contents');
    editor.contextMenu.handleContextMenuItem(
      'container_contextmenu_fixed_column_width'
    );
    expect(
      editor.selection.start.paragraph.associatedCell.ownerTable.tableFormat
        .allowAutoFit
    ...
    expect(
      editor.selection.start.paragraph.associatedCell.cellFormat
        .preferredWidthType
    ).toBe('Point');
  });
});
...
describe('Context Menu element show validation', () => {
  ...
  it('Show context menu with hyperlink dialog as false', () => {
    ...
    expect(
      document.getElementById(editor.element.id + '_contextmenu_hyperlink')
        .style.display
    ).toBe('none');
    expect(
      (
        document.getElementById(editor.element.id + '_contextmenu_font_dialog')
