...
describe('Select Table API validation', () => {
  ...
  beforeAll(() => {
    ...
    (editor.documentHelper as any).containerCanvasIn =
      TestHelper.containerCanvas;
    (editor.documentHelper as any).selectionCanvasIn =
      TestHelper.selectionCanvas;
    (editor.documentHelper.render as any).pageCanvasIn = TestHelper.pageCanvas;
    (editor.documentHelper.render as any).selectionCanvasIn =
      TestHelper.pageSelectionCanvas;
  ...
  it('Insert table cursor validation', () => {
    ...
    let table: TableWidget = documentHelper.pages[0].bodyWidgets[0]
      .childWidgets[0] as TableWidget;
    expect(editor.selection.start.hierarchicalPosition).toBe('0;0;0;0;0;0;0;0');
    expect(editor.selection.start.paragraph.associatedCell).toBe(
      (table.childWidgets[0] as TableRowWidget)
        .childWidgets[0] as TableCellWidget
    );
  });
  it('Nested insert Table cursor validation', () => {
    ...
    editor.editorModule.insertTable(2, 2);
    editor.editorModule.insertTable(2, 2);
    expect(editor.selection.start.hierarchicalPosition).toBe(
  ...
  it('Select Table in Forward selection', () => {
    ...
    let table: TableWidget = documentHelper.pages[0].bodyWidgets[0]
      .childWidgets[0] as TableWidget;
    let firstCell: TableCellWidget = (table.firstChild as TableRowWidget)
      .firstChild as TableCellWidget;
    let lastCell: TableCellWidget = (table.lastChild as TableRowWidget)
      .lastChild as TableCellWidget;
  ...
  it('Select Table in nested Table case', () => {
    ...
    let table: TableWidget = (
      (
        (documentHelper.pages[0].bodyWidgets[0].childWidgets[0] as TableWidget)
          .firstChild as TableRowWidget
      ).firstChild as TableCellWidget
    ).childWidgets[0] as TableWidget;
    let firstCell: TableCellWidget = (table.firstChild as TableRowWidget)
      .firstChild as TableCellWidget;
  ...
  it('Select Table if selection is not empty', () => {
    ...
    expect(editor.selection.start.paragraph.associatedCell).toEqual(
      editor.selection.end.paragraph.associatedCell
    );
    editor.selection.selectTable();
    expect(editor.selection.start.paragraph.associatedCell).toBe(firstCell);
    expect(editor.selection.end.paragraph.associatedCell).toBe(lastCell);
...
describe('Select Row API validation', () => {
  ...
  it('Select Row in Forward selection', () => {
    ...
    let firstCell: TableCellWidget = (table.firstChild as TableRowWidget)
      .firstChild as TableCellWidget;
    let lastCell: TableCellWidget = (table.firstChild as TableRowWidget)
      .lastChild as TableCellWidget;
    editor.selection.selectRow();
    expect(editor.selection.start.paragraph.associatedCell).toBe(firstCell);
    expect(editor.selection.end.paragraph.associatedCell).toBe(lastCell);
    expect(editor.selection.start.paragraph.associatedCell.ownerRow).toEqual(
      editor.selection.end.paragraph.associatedCell.ownerRow
    );
  });
  ...
  it('Select Row if selection is not empty', () => {
    ...
    expect(editor.selection.start.paragraph.associatedCell).not.toEqual(
      editor.selection.end.paragraph.associatedCell
    );
    editor.selection.selectRow();
    expect(
      editor.selection.start.paragraph.associatedCell.ownerRow
    ).not.toEqual(editor.selection.end.paragraph.associatedCell.ownerRow);
  });
});
...
describe('Select Cell API validation', () => {
  ...
  it('Select Cell if selection is not empty', () => {
    ...
    expect(editor.selection.end.paragraph.associatedCell.ownerRow).toEqual(
      firstRow
    );
    editor.selection.selectCell();
    expect(editor.selection.start.paragraph.associatedCell).toEqual(firstCell);
...
describe('BookMark validation in double tap', () => {
  ...
  it('start of line selection containing Bookmark element-select current word validation', () => {
    console.log(
      'start of line selection containing Bookmark element-select current word validation'
    );
...
describe('selection table row navigation', () => {
  ...
  it('selection table row forward navigation', () => {
    ...
    expect(
      container.documentEditor.selection.start.paragraph.isInsideTable
    ).toBe(true);
    expect(
      container.documentEditor.selection.start.paragraph.associatedCell
        .columnIndex
    ).toBe(0);
    expect(
      container.documentEditor.selection.start.paragraph.associatedCell.rowIndex
    ).toBe(0);
