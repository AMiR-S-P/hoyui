...
describe('New Document - Create and Apply Style', () => {
  ...
  it('Paragraph Style', () => {
    ...
    let styleJson: string =
      '{"type":"Paragraph","name":"Style3","basedOn":"Normal","next":"Normal","characterFormat":{"bold":true,"italic":true,"underline":"Single","fontSize":24.0,"fontFamily":"Monotype Corsiva"}}';
    let style: WStyle = editor.editorModule.createStyleIn(
      styleJson,
      undefined
    ) as WStyle;
    editor.editorModule.insertText('Heading');
    editor.editorModule.applyStyle(style.name);
    expect(editor.selection.characterFormat.fontFamily).toBe(
      'Monotype Corsiva'
    );
