...
describe('Get Minimum and maximum width form cell', () => {
  ...
  it('Get minimum and maximum width from cell', () => {
    ...
    editor.editor.insertHyperlinkInternal(
      'https://syncfusion.com',
      'Syncfusion',
      true,
      false
    );
    let widthInfo: WidthInfo =
      editor.selection.start.paragraph.associatedCell.getMinimumAndMaximumWordWidth(
        0,
        0
      );
  ...
  it('Get min and max width from table', () => {
    ...
    editor.selection.start.paragraph.associatedCell.ownerTable.isGridUpdated =
      false;
    let widthInfo: WidthInfo =
      editor.selection.start.paragraph.associatedCell.ownerTable.getMinimumAndMaximumWordWidth(
        0,
        0
      );
    expect(widthInfo.maximumWordWidth).toBe(
      editor.selection.start.paragraph.associatedCell.ownerTable.tableHolder.getTotalWidth(
        0
      )
    );
    expect(widthInfo.minimumWordWidth).toBe(
      editor.selection.start.paragraph.associatedCell.ownerTable.tableHolder.getTotalWidth(
        0
      )
    );
  ...
  it('Get min and max width from Row', () => {
    ...
    let widthInfo: WidthInfo =
      editor.selection.start.paragraph.associatedCell.ownerRow.getMinimumAndMaximumWordWidth(
        0,
        0
      );
...
describe('Table width greater than container width validation', () => {
  ...
  it('Table width greater than container width validation', () => {
    ...
    let table: TableWidget = editor.documentHelper.pages[0].bodyWidgets[0]
      .childWidgets[0] as TableWidget;
    table.tableFormat.preferredWidth = 800;
    editor.selection.selectAll();
    editor.editor.reLayout(editor.selection);
    expect(Math.round(editor.viewer.clientActiveArea.width)).toBeLessThan(
  ...
  it('Page number validation', () => {
    ...
    expect(
      (
        (
          (
            editor.documentHelper.pages[1].footerWidget
              .childWidgets[0] as ParagraphWidget
          ).childWidgets[0] as LineWidget
        ).children[3] as FieldTextElementBox
      ).text
    ).toBe('2');
  });
});
