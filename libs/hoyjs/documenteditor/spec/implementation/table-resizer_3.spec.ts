...
describe('Table Cell Resizing With Final Cell Selection testing', () => {
  ...
  it('Resize Table Cell in Final Row Drag Value Testing ', () => {
    ...
    let startPosition: TextPosition = getTextPosition(
      editor,
      '0;0;0;1;1;0;0;0'
    );
  ...
  it('Resize Table Cell With Selection table cell Spacing Resizing Testing ', () => {
    console.log(
      'Resize Table Cell With Selection table cell Spacing Resizing Testing '
    );
    editor.editor.insertTable(3, 3);
    let table: TableWidget =
      editor.selection.start.paragraph.associatedCell.ownerTable;
...
describe('Cell Width Restricting on cell Resizing testing', () => {
  ...
  it('Resize Table Without Selection on Zero index Testing', () => {
    ...
    editor.documentHelper.onMouseDownInternal(event);
    let compareTable: any =
      editor.documentHelper.pages[0].bodyWidgets[0].childWidgets[0];
    expect(
      compareTable.childWidgets[0].childWidgets[0].cellFormat.cellWidth
    ).toBeGreaterThanOrEqual(116);
    expect(
      compareTable.childWidgets[1].childWidgets[0].cellFormat.cellWidth
    ).toBeGreaterThanOrEqual(116);
    editor.editorHistory.undo();
    editor.editorHistory.redo();
  });
...
describe('Check the backspace is working properly in table case with bookmark', () => {
  ...
  beforeAll(() => {
    ...
    document.body.appendChild(ele);
    DocumentEditor.Inject(Editor, Selection, EditorHistory);
    container = new DocumentEditor({
      enableEditor: true,
      isReadOnly: false,
      enableEditorHistory: true,
      enableSfdtExport: true,
    });
    (container.documentHelper as any).containerCanvasIn =
      TestHelper.containerCanvas;
    (container.documentHelper as any).selectionCanvasIn =
      TestHelper.selectionCanvas;
    (container.documentHelper.render as any).pageCanvasIn =
      TestHelper.pageCanvas;
    (container.documentHelper.render as any).selectionCanvasIn =
      TestHelper.pageSelectionCanvas;
  ...
  it('Check the backspace is working properly in table case with bookmark', () => {
    console.log(
      'Check the backspace is working properly in table case with bookmark'
    );
    container.openBlank();
    ...
    container.editor.onBackSpace();
    let bodyWidget: BodyWidget = container.documentHelper.pages[0]
      .bodyWidgets[0] as BodyWidget;
    let tableWidget: TableWidget = bodyWidget.childWidgets[0] as TableWidget;
    let tableRowWidget: TableRowWidget = tableWidget
      .childWidgets[0] as TableRowWidget;
    let tableCellWidget: TableCellWidget = tableRowWidget
      .childWidgets[0] as TableCellWidget;
    let paragraphWidget: ParagraphWidget = tableCellWidget
      .childWidgets[2] as ParagraphWidget;
    let lineWidget: LineWidget = paragraphWidget.childWidgets[0] as LineWidget;
    let textElementBox: TextElementBox = lineWidget
      .children[1] as TextElementBox;
