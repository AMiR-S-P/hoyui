...
import { Tooltip as tool, TooltipTheme } from '@hoyui/svg-base';
...
export class Tooltip {
    ...
    if (!isNullOrUndefined(this.heatMap)) {
      ...
      if (!isShow) {
        if (!isNullOrUndefined(ele) && ele.style.visibility !== 'hidden') {
          if (
            !isNullOrUndefined(this.tooltipObject) &&
            isFadeout &&
            this.heatMap.isRectBoundary
          ) {
            this.tooltipObject.fadeOut();
          } else {
            if (
              !isNullOrUndefined(this.tooltipObject) &&
              !isNullOrUndefined(this.tooltipObject.element)
            ) {
              const tooltipElement: HTMLElement = this.tooltipObject.element
  ...
  private createTooltip(
    ...
    if (
      this.heatMap.cellSettings.showLabel &&
      this.heatMap.heatMapSeries.checkLabelXDisplay &&
    ...
    this.tooltipObject = new tool(
      {
        opacity:
          this.heatMap.theme === 'Tailwind' ||
          this.heatMap.theme === 'TailwindDark' ||
          this.heatMap.theme === 'Bootstrap5' ||
          this.heatMap.theme === 'Bootstrap5Dark' ||
          this.heatMap.theme === 'Fluent' ||
          this.heatMap.theme === 'FluentDark' ||
          this.heatMap.theme === 'Fluent2' ||
          this.heatMap.theme === 'Fluent2Dark' ||
          this.heatMap.theme === 'Fluent2HighContrast'
            ? 1
            : 0.75,
        ...
        data: {
          ...
          xLabel: this.heatMap.heatMapSeries.hoverXAxisLabel
            ? this.heatMap.heatMapSeries.hoverXAxisLabel.toString()
            : null,
          yLabel: this.heatMap.heatMapSeries.hoverYAxisLabel
            ? this.heatMap.heatMapSeries.hoverYAxisLabel.toString()
            : null,
        },
        ...
        enableShadow: true,
        template:
          this.heatMap.tooltipSettings.template === ''
            ? null
            : this.heatMap.tooltipSettings.template,
        border: {
        ...
        textStyle: {
          ...
          fontWeight:
            this.heatMap.tooltipSettings.textStyle.fontWeight.toLowerCase(),
          color: this.heatMap.tooltipSettings.textStyle.color,
          fontStyle:
            this.heatMap.tooltipSettings.textStyle.fontStyle.toLowerCase(),
          fontFamily: this.heatMap.tooltipSettings.textStyle.fontFamily,
        },
        areaBounds: {
          height:
            this.heatMap.initialClipRect.height +
            this.heatMap.initialClipRect.y,
  ...
  public createTooltipDiv(heatMap: HeatMap): void {
    ...
    const top: number =
      heatMap.enableCanvasRendering && heatMap.allowSelection
        ? heatMap.availableSize.height
        : 0;
    const element2: Element = <HTMLElement>createElement('div', {
      id: this.heatMap.element.id + 'Celltooltipcontainer',
    });
    (element2 as HTMLElement).style.cssText =
      'position:' + position + '; z-index: 3;top:-' + top + 'px';
    const tooltipElement: Element = <HTMLElement>createElement('div', {
  ...
  private getTooltipContent(
    ...
    if (heatMap.bubbleSizeWithColor) {
      const xAxis: string =
        heatMap.xAxis.title && heatMap.xAxis.title.text !== ''
          ? heatMap.xAxis.title.text
          : 'X-Axis';
      const yAxis: string =
        heatMap.yAxis.title && heatMap.yAxis.title.text !== ''
          ? heatMap.yAxis.title.text
          : 'Y-Axis';
      const value1: string =
        adaptData.isJsonData && adaptData.adaptorType === 'Cell'
          ? adaptData.bubbleDataMapping.size
          : 'Value 1';
      const value2: string =
        adaptData.isJsonData && adaptData.adaptorType === 'Cell'
          ? adaptData.bubbleDataMapping.color
          : 'Value 2';
      value = hetmapSeries.getFormatedText(
        (currentRect.value as BubbleTooltipData[])[0].bubbleData,
        this.heatMap.cellSettings.format
      );
      content = [
        xAxis +
          ' : ' +
          hetmapSeries.hoverXAxisLabel +
          '<br/>' +
          yAxis +
          ' : ' +
          ...
          hetmapSeries.getFormatedText(
            (currentRect.value as BubbleTooltipData[])[1].bubbleData,
            this.heatMap.cellSettings.format
          ),
      ];
    } else {
  ...
  public renderTooltip(currentRect: CurrentRect): void {
    ...
    const showTooltip: boolean = this.heatMap.bubbleSizeWithColor
      ? !isNullOrUndefined(currentRect.value) &&
        !isNullOrUndefined(
          (currentRect.value as BubbleTooltipData[])[0].bubbleData
        ) &&
        (currentRect.value as BubbleTooltipData[])[0].bubbleData.toString() !==
          ''
        ? true
        : false
      : isNullOrUndefined(currentRect.value) ||
        (!isNullOrUndefined(currentRect.value) &&
          currentRect.value.toString() === '')
      ? false
      : true;
    if (!showTooltip) {
      ...
      if (!isNullOrUndefined(this.heatMap.tooltipRender)) {
        ...
        this.heatMap.trigger(
          'tooltipRender',
          argData,
          (observedArgs: ITooltipEventArgs) => {
  ...
  private tooltipCallback(
    ...
    this.tooltipObject.enableAnimation =
      this.isFirst || this.isFadeout ? false : true;
