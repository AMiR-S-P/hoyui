...
import {
  ILoadedEventArgs,
  ITooltipEventArgs,
} from '../../src/heatmap/model/interface';
...
describe('Heatmap Control', () => {
  ...
  describe('Heatmap tooltip properties and its behavior', () => {
    ...
    it('Check tooltip visibility for a null cell', () => {
      ...
      tempElement = document.getElementById(
        'containerCelltooltipcontainer_svg'
      );
      ...
      heatmap.tooltipRender = function (args: ITooltipEventArgs) {
        args.content = [
          args.xLabel +
            ...
            args.yLabel +
            '=' +
            parseInt(args.value.toString()) * 10,
        ];
      };
      heatmap.refresh();
    ...
    it('Check tooltip visibility beyond heatmap container', () => {
      ...
      heatmap.heatMapMouseLeave(
        <PointerEvent>(
          trigger.onTouchStart(tempElement, null, null, null, null, 0, 0)
        )
      );
      tempElement = document.getElementById('containerCelltooltipcontainer');
      expect(tempElement.style.visibility).toBe('hidden');
    ...
    it('Check tooltip visibility on touch', (done: Function) => {
      tempElement = document.getElementById('container_HeatMapRect_1');
      heatmap.heatMapMouseMove(
        <PointerEvent>(
          trigger.onTouchStart(tempElement, null, null, null, null, 70, 80)
        )
      );
      tempElement = document.getElementById('containerCelltooltipcontainer');
    ...
    it('Check tooltip visibility on touch', (done: Function) => {
      ...
      heatmap.heatMapMouseMove(
        <PointerEvent>(
          trigger.onTouchStart(tempElement, null, null, null, null, 170, 80)
        )
      );
      tempElement = document.getElementById('containerCelltooltipcontainer');
      expect(tempElement.style.visibility).toBe('visible');
    ...
    it('Check tooltip Color', () => {
      ...
      expect(
        document
          .getElementById('containerCelltooltipcontainer_path')
          .getAttribute('fill')
      ).toBe('RED');
    });
    ...
    it('Check tooltip font family', function () {
      (heatmap.theme = 'TailwindDark'), heatmap.dataBind();
      tempElement = document.getElementById('container_HeatMapRect_0');
      trigger.mousemoveEvent(tempElement, 0, 0, 60, 20, false);
      expect(
        document
          .getElementById('containerCelltooltipcontainer_text')
    ...
    it('Check tooltip template support', function () {
      heatmap.tooltipSettings.template =
        '<div>${xValue}</div><div>${yValue}</div>${value}';
      heatmap.refresh();
      tempElement = document.getElementById('container_HeatMapRect_0');
      trigger.mousemoveEvent(tempElement, 0, 0, 60, 20, false);
      tempElement = document.getElementById(
        'containerCelltooltipcontainerparent_template'
      );
  ...
  describe('Heatmap tooltip with Bootstrap5 theme', () => {
    ...
    it('Check tooltip visibility of Bootstrap5 Theme', () => {
      ...
      expect(
        document
          .getElementById('containerCelltooltipcontainer_text')
          .getAttribute('font-family') == 'Segoe UI'
      ).toBe(true);
    });
  });
