...
export class DatePicker extends Calendar implements IInput {
  ...
  private initialize(): void {
    ...
    this.previousDate = !isNullOrUndefined(this.value)
      ? new Date(+this.value)
      : null;
  ...
  protected updateInput(
    ...
    if (!isNullOrUndefined(this.value)) {
      ...
      const tempFormat: string = !isNullOrUndefined(this.formatString)
        ? this.formatString
        : this.dateTimeFormat;
  ...
  private checkStringValue(val: string): Date {
    ...
    if (this.getModuleName() === 'datetimepicker') {
      const culture: Internationalization = new Internationalization(
        this.locale
      );
      if (this.calendarMode === 'Gregorian') {
        formatOptions = {
        ...
        formatDateTime = {
          format: culture.getDatePattern({ skeleton: 'yMd' }),
          type: 'dateTime',
        };
      } else {
        formatOptions = {
    ...
    returnDate = this.checkDateValue(
      this.globalize.parseDate(this.getAmPmValue(val), formatOptions)
    );
    if (
      isNullOrUndefined(returnDate) &&
      this.getModuleName() === 'datetimepicker'
    ) {
      returnDate = this.checkDateValue(
        this.globalize.parseDate(this.getAmPmValue(val), formatDateTime)
      );
    }
    return returnDate;
  }
  ...
  protected bindEvents(): void {
    ...
    this.keyboardModules = new KeyboardEvents(<HTMLElement>this.inputElement, {
      eventName: 'keydown',
  ...
  protected resetFormHandler(): void {
    ...
    if (!this.inputElement.disabled) {
      ...
      if (
        this.element.tagName === 'EJS-DATEPICKER' ||
        this.element.tagName === 'EJS-DATETIMEPICKER'
      ) {
        value = '';
      ...
      for (const key of Object.keys(this.htmlAttributes)) {
        if (!isNullOrUndefined(this.htmlAttributes[`${key}`])) {
          if (containerAttr.indexOf(key) > -1) {
            if (key === 'class') {
              const updatedClassValues: string = this.htmlAttributes[`${key}`]
                .replace(/\s+/g, ' ')
                .trim();
    ...
    switch (e.action) {
      case 'escape':
        if (this.isCalendar()) {
          ...
          (
            this.inputWrapper.container.children[this.index] as HTMLElement
          ).blur();
        }
        break;
      case 'enter':
  ...
  private documentHandler(e: MouseEvent): void {
    if (
      ...
      ((this.inputWrapper.container.contains(<HTMLElement>e.target) &&
        e.type !== 'mousedown') ||
        (this.popupObj.element &&
          this.popupObj.element.contains(<HTMLElement>e.target))) &&
      e.type !== 'touchstart'
    ) {
      e.preventDefault();
    }
    ...
    if (
      !closest(target, '.e-datepicker.e-popup-wrapper') &&
      !isNullOrUndefined(this.inputWrapper) &&
      !(
      ...
      if (
        ...
        closest(target, '.e-content').classList.contains(
          'e-' + this.depth.toLowerCase()
        )
      ) {
        this.hide(e);
      } else if (
        closest(target, '.e-footer-container') &&
    ...
    switch (e.action) {
      ...
      case 'shiftTab': {
        ...
        if (this.enableMask && !this.popupObj && !this.readonly) {
          ...
          if (
            (start === 0 && end === length) ||
            (end !== length && e.action === 'tab') ||
            (start !== 0 && e.action === 'shiftTab')
          ) {
            e.preventDefault();
          }
          this.notify('keyDownHandler', {
  ...
  protected defaultAction(e: KeyboardEventArgs): void {
    this.previousDate =
      (!isNullOrUndefined(this.value) && new Date(+this.value)) || null;
    if (this.isCalendar()) {
  ...
  protected popupUpdate(): void {
    if (
      ...
      (this.getModuleName() === 'datetimepicker' &&
        +this.value !== +this.previousDateTime)
    ) {
      if (this.popupObj) {
  ...
  protected strictModeUpdate(): void {
    ...
    if (this.getModuleName() === 'datetimepicker') {
      ...
      !pattern.test(this.formatString) ||
      charPattern.test(this.formatString)
    ) {
      format = isNullOrUndefined(this.formatString)
        ? this.formatString
        : this.formatString.replace('dd', 'd');
    }
    ...
    if (this.getModuleName() === 'datetimepicker') {
      if (
        this.checkDateValue(
          this.globalize.parseDate(
            this.getAmPmValue(this.inputElement.value),
            dateOptions
          )
        )
      ) {
        date = this.globalize.parseDate(
          this.getAmPmValue(this.inputElement.value),
          dateOptions
        );
      } else {
      ...
      if (
        !isNullOrUndefined(this.formatString) &&
    ...
    if (
      this.strictMode &&
      !date &&
      this.inputElement.value === (this.enableMask ? this.maskedDateValue : '')
    ) {
      this.setProperties({ value: null }, true);
    }
    if (isNaN(+this.value)) {
  ...
  private modelHeader(): void {
    ...
    const yearHeading: HTMLElement = this.createElement('h1', {
      className: 'e-model-year',
    });
    const h2: HTMLElement = this.createElement('div');
    const daySpan: HTMLElement = this.createElement('span', {
      className: 'e-model-day',
    });
    const monthSpan: HTMLElement = this.createElement('span', {
    ...
    yearHeading.textContent =
      '' + this.globalize.formatDate(this.value || new Date(), dateOptions);
    if (this.calendarMode === 'Gregorian') {
    ...
    if (this.fullScreenMode) {
      ...
      const modelTodayButton: Element =
        this.calendarElement.querySelector('button.e-today');
    ...
    this.calendarElement.insertBefore(
      modelHeader,
      this.calendarElement.firstElementChild
    );
  }
  ...
  protected changeTrigger(event?: MouseEvent | KeyboardEvent): void {
    if (this.inputElement.value !== this.previousElementValue) {
      if (
        ...
        this.previousElementValue = this.inputElement.value;
        this.previousDate = !isNaN(+new Date(this.checkValue(this.value)))
          ? new Date(this.checkValue(this.value))
          : null;
  ...
  protected keyupHandler(e: KeyboardEventArgs): void {
    this.isKeyAction =
      this.inputElement.value !== this.previousElementValue ? true : false;
  }
  ...
  protected selectCalendar(e?: MouseEvent | KeyboardEvent | Event): void {
    ...
    let formatOptions: DateFormatOptions;
    if (this.getModuleName() === 'datetimepicker') {
      tempFormat = !isNullOrUndefined(this.formatString)
        ? this.formatString
        : this.dateTimeFormat;
    } else {
  ...
  protected isCalendar(): boolean {
    if (
      this.popupWrapper &&
      this.popupWrapper.classList.contains('' + POPUPWRAPPER)
    ) {
      return true;
    }
    return false;
  ...
  public show(
    ...
    if ((this.enabled && this.readonly) || !this.enabled || this.popupObj) {
      ...
      if (
        !isNullOrUndefined(this.value) &&
        !(
          +this.value >= +new Date(this.checkValue(this.min)) &&
      ...
      this.trigger('open', eventArgs, (eventArgs: PopupObjectArgs) => {
        ...
        if (
          ...
          if (
            !isNullOrUndefined(this.tableBodyElement) &&
            this.tableBodyElement.querySelectorAll('td.e-selected').length > 0
          ) {
            addClass(
              [this.tableBodyElement.querySelector('td.e-selected')],
              FOCUSEDDATE
            );
            removeClass(
              this.tableBodyElement.querySelectorAll('td.e-selected'),
              SELECTED
            );
          }
        }
      ...
      if (Browser.isDevice) {
        const dlgOverlay: any = this.createElement('div', {
  ...
  private closeEventCallback(
    ...
    if (Browser.isDevice) {
      if (
        !isNullOrUndefined(this.mobilePopupWrapper) &&
        prevent &&
        (isNullOrUndefined(this.preventArgs) || !this.preventArgs.cancel)
      ) {
  ...
  public destroy(): void {
    ...
    if (this.ngTag === null) {
      if (this.inputElement) {
        if (!isNullOrUndefined(this.inputWrapper)) {
          this.inputWrapper.container.insertAdjacentElement(
            'afterend',
            this.inputElement
          );
        }
  ...
  protected ensureInputAttribute(): void {
    ...
    for (let i: number = 0; i < prop.length; i++) {
      if (
        isNullOrUndefined(this.inputElementCopy.getAttribute(prop[i as number]))
      ) {
        ...
        if (prop[i as number].toLowerCase() === 'value') {
          this.inputElement.value = this.inputElementCopy.getAttribute(
            prop[i as number]
          );
        }
        this.inputElement.setAttribute(
          prop[i as number],
          this.inputElementCopy.getAttribute(prop[i as number])
        );
      }
    }
  }
  ...
  protected validationAttribute(
    ...
    const nameAttribute: string = target.getAttribute('name')
    ...
    for (const prop of attributes) {
      if (!isNullOrUndefined(this.inputElement.getAttribute(prop))) {
        switch (prop) {
          case 'disabled':
            if (
              isNullOrUndefined(this.datepickerOptions) ||
              this.datepickerOptions['enabled'] === undefined ||
              dynamic
            ) {
          ...
          case 'readonly':
            if (
              isNullOrUndefined(this.datepickerOptions) ||
              this.datepickerOptions['readonly'] === undefined ||
              dynamic
            ) {
          ...
          case 'placeholder':
            if (
              isNullOrUndefined(this.datepickerOptions) ||
              this.datepickerOptions['placeholder'] === undefined ||
              dynamic
            ) {
              this.setProperties(
          ...
          case 'value':
            if (
              isNullOrUndefined(this.datepickerOptions) ||
              this.datepickerOptions['value'] === undefined ||
              dynamic
            ) {
              ...
              this.setProperties(
                setValue(
                  prop,
                  this.globalize.parseDate(this.getAmPmValue(value), options),
                  {}
                ),
                !dynamic
              );
          ...
          case 'min':
            if (+this.min === +new Date(1900, 0, 1) || dynamic) {
              ...
              this.setProperties(
                setValue(
                  prop,
                  this.globalize.parseDate(this.getAmPmValue(min), options),
                  {}
                ),
                !dynamic
              );
          ...
          case 'max':
            if (+this.max === +new Date(2099, 11, 31) || dynamic) {
              ...
              this.setProperties(
                setValue(
                  prop,
                  this.globalize.parseDate(this.getAmPmValue(max), options),
                  {}
                ),
                !dynamic
              );
  ...
  private disabledDates(
    ...
    const valueCopy: Date = this.checkDateValue(this.value)
      ? new Date(+this.value)
      : new Date(this.checkValue(this.value));
    ...
    if (!this.strictMode) {
      if (
        ...
        (typeof this.value === 'object' && +this.value !== +valueCopy)
      ) {
        this.setProperties({ value: valueCopy }, true);
      }
    }
    if (
      !isNullOrUndefined(this.calendarElement) &&
      !isNullOrUndefined(this.calendarElement.querySelectorAll(dateIdString)[0])
    ) {
      if (
        this.calendarElement
          .querySelectorAll(dateIdString)[0]
  ...
  protected errorClass(): void {
    const dateIdString: string = '*[id^="/id"]'.replace(
      '/id',
      '' + +this.value
    );
    ...
    if (
      (!isNullOrUndefined(this.value) &&
        ...
        !(
          new Date(this.value as Date).setMilliseconds(0) >=
            new Date(this.min as Date).setMilliseconds(0) &&
          new Date(this.value as any).setMilliseconds(0) <=
            new Date(this.max as Date).setMilliseconds(0)
        )) ||
      (!this.strictMode &&
        this.inputElement.value !== '' &&
        this.inputElement.value !== this.maskedDateValue &&
  ...
  public onPropertyChanged(
    newProp: DatePickerModel,
    oldProp: DatePickerModel
  ): void {
      ...
      switch (prop) {
        ...
        case 'locale':
          ...
          if (
            ...
            this.datepickerOptions.placeholder == null
          ) {
            this.setProperties(
