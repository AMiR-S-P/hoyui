...
import { Popup } from '@hoyui/popups';
import { Button } from '@hoyui/buttons';
import {
  BlurEventArgs,
  FocusEventArgs,
  ClearedEventArgs,
} from '../calendar/calendar';
import { Input, InputObject, FloatLabelType } from '@hoyui/inputs';
import { ListBase } from '@hoyui/lists';
...
export class DateRangePicker extends CalendarBase {
  ...
  protected preRender(): void {
    ...
    if (!this.invalidValueString && typeof this.value === 'string') {
      const rangeArray: string[] = (<string>this.value).split(
        ' ' + this.separator + ' '
      );
  ...
  private updateValue(): void {
    if (this.value && (<Date[]>this.value).length > 0) {
      if (
        (<Date[]>this.value)[0] instanceof Date &&
        !isNaN(+(<Date[]>this.value)[0])
      ) {
        ...
        if (
          +(<Date[]>this.value)[0] === 0 ||
          isNaN(+new Date(this.checkValue((<Date[]>this.value)[0])))
        ) {
          this.startValue = null;
          this.setValue();
        } else {
          this.setProperties(
            { startDate: new Date(this.checkValue((<Date[]>this.value)[0])) },
            true
          );
      ...
      if (
        (<Date[]>this.value)[1] instanceof Date &&
        !isNaN(+(<Date[]>this.value)[1])
      ) {
        ...
        if (
          ...
          this.setProperties(
            { endDate: new Date(this.checkValue((<Date[]>this.value)[1])) },
            true
          );
      ...
      if (
        (<DateRange>this.value).start instanceof Date &&
        !isNaN(+(<DateRange>this.value).start)
      ) {
        ...
        this.setProperties(
          {
            startDate: new Date(this.checkValue((<DateRange>this.value).start)),
          },
          true
        );
        this.startValue = new Date(
          this.checkValue((<DateRange>this.value).start)
        );
      } else {
        this.startValue = null;
        this.setValue();
      ...
      if (
        (<DateRange>this.value).end instanceof Date &&
        !isNaN(+(<DateRange>this.value).end)
      ) {
        ...
        this.setProperties(
          { endDate: new Date(this.checkValue((<DateRange>this.value).end)) },
          true
        );
      ...
      this.endValue = this.checkDateValue(
        new Date(this.checkValue(this.endDate))
      );
      this.startValue = this.checkDateValue(
        new Date(this.checkValue(this.startDate))
      );
      this.setValue();
    }
  }
  private initProperty(): void {
    ...
    this.checkView();
    if (
      isNullOrUndefined(this.firstDayOfWeek) ||
  ...
  protected checkFormat(): void {
    if (this.format) {
      if (typeof this.format === 'string') {
        ...
        this.format.skeleton !== '' &&
        !isNullOrUndefined(this.format.skeleton)
      ) {
  ...
  private initialize(): void {
    ...
    const start: Date = this.checkDateValue(
      new Date(this.checkValue(this.startValue))
    );
    ...
    if (!isNullOrUndefined(this.htmlAttributes)) {
      for (const key of Object.keys(this.htmlAttributes)) {
        if (wrapperAttr.indexOf(key) > -1) {
          if (key === 'class') {
            const updatedClassValue: string = this.htmlAttributes[`${key}`]
              .replace(/\s+/g, ' ')
              .trim();
            ...
            let dateRangeStyle: string =
              this.inputWrapper.container.getAttribute(key);
            dateRangeStyle = !isNullOrUndefined(dateRangeStyle)
              ? dateRangeStyle + this.htmlAttributes[`${key}`]
  ...
  private processPresets(): void {
    ...
    let i: number = 0;
    if (
      !isNullOrUndefined(this.presets[0]) &&
      !isUndefined(
        this.presets[0].start && this.presets[0].end && this.presets[0].label
      )
    ) {
      ...
      const startDate: Date = isNullOrUndefined(this.startValue)
        ? null
        : new Date(+this.startValue);
      const endDate: Date = isNullOrUndefined(this.endValue)
        ? null
      ...
      if (
        !isNullOrUndefined(this.startValue) &&
  ...
  protected bindEvents(): void {
    ...
    if (!this.isMobile) {
      this.keyInputConfigs = extend(this.keyInputConfigs, this.keyConfigs) as {
        [key: string]: string;
      };
  ...
  private updateHiddenInput(): void {
    if (this.firstHiddenChild && this.secondHiddenChild) {
      ...
      if (typeof this.startDate === 'string') {
        this.startDate = this.globalize.parseDate(
          this.getAmPmValue(this.startDate),
          format
        );
      }
      if (typeof this.endDate === 'string') {
        this.endDate = this.globalize.parseDate(
          this.getAmPmValue(this.endDate),
          format
        );
      }
      this.firstHiddenChild.value =
        (this.startDate && this.globalize.formatDate(this.startDate, format)) ||
        this.inputElement.value;
      this.secondHiddenChild.value =
        (this.endDate && this.globalize.formatDate(this.endDate, format)) ||
        this.inputElement.value;
    ...
    if (
      ...
      if (!isNullOrUndefined(this.startCopy)) {
        if (
          ...
          !isNullOrUndefined((<DateRange>this.value).start)
        ) {
          this.setProperties(
            { value: { start: this.startCopy, end: this.endCopy } },
            true
          );
          this.startValue = (<DateRange>this.value).start;
          this.endValue = (<DateRange>this.value).end;
        ...
        this.setProperties(
          { startDate: this.startValue, endDate: this.endValue },
          true
        );
      } else {
        this.setProperties(
  ...
  private checkHtmlAttributes(isDynamic: boolean): void {
    this.globalize = new Internationalization(this.locale);
    const attributes: string[] = isDynamic
      ? isNullOrUndefined(this.htmlAttributes)
        ? []
        : Object.keys(this.htmlAttributes)
      : [
          'startDate',
          'endDate',
          'minDays',
    ...
    for (const prop of attributes) {
      if (!isNullOrUndefined(this.inputElement.getAttribute(prop))) {
        switch (prop) {
          case 'disabled':
            if (
              isNullOrUndefined(this.dateRangeOptions) ||
              this.dateRangeOptions['enabled'] === undefined ||
              isDynamic
            ) {
              const disabled: boolean =
                this.inputElement.getAttribute(prop) === 'disabled' ||
          ...
          case 'readonly':
            if (
              isNullOrUndefined(this.dateRangeOptions) ||
              this.dateRangeOptions['readonly'] === undefined ||
              isDynamic
            ) {
              const readonly: boolean =
                this.inputElement.getAttribute(prop) === 'readonly' ||
                this.inputElement.getAttribute(prop) === 'true' ||
                this.inputElement.getAttribute(prop) === ''
                  ? true
                  : false;
          ...
          case 'placeholder':
            if (
              isNullOrUndefined(this.dateRangeOptions) ||
              this.dateRangeOptions['placeholder'] === undefined ||
              isDynamic
            ) {
              this.setProperties(
          ...
          case 'value':
            if (
              isNullOrUndefined(this.dateRangeOptions) ||
              this.dateRangeOptions['value'] === undefined ||
              isDynamic
            ) {
          ...
          case 'min':
            if (
              ...
              const dateValue: Date = this.globalize.parseDate(
                this.getAmPmValue(this.inputElement.getAttribute(prop)),
                format
              );
          ...
          case 'minDays':
            if (isNullOrUndefined(this.minDays)) {
              this.setProperties(
                setValue(
                  prop,
                  parseInt(this.inputElement.getAttribute(prop), 10),
                  {}
                ),
                true
              );
  ...
  private createPopup(): void {
    for (let i: number = 0; i < this.presetsItem.length; i++) {
      if (
        i !== this.presetsItem.length - 1 &&
        this.presetsItem[i as number].id === 'custom_range'
      ) {
    ...
    for (let i: number = 0; i <= this.presetsItem.length - 2; i++) {
      ...
      if (this.startValue && this.endValue) {
        if (
          startDate.getDate() === this.startValue.getDate() &&
          startDate.getMonth() === this.startValue.getMonth() &&
          startDate.getFullYear() === this.startValue.getFullYear() &&
          endDate.getDate() === this.endValue.getDate() &&
          endDate.getMonth() === this.endValue.getMonth() &&
          endDate.getFullYear() === this.endValue.getFullYear()
        ) {
          this.activeIndex = i;
  ...
  private renderControl(): void {
    ...
    if (
      !isNullOrUndefined(this.endValue) &&
      !isNullOrUndefined(this.startValue) &&
      !isNullOrUndefined(this.renderDayCellArgs) &&
      this.renderDayCellArgs.isDisabled
    ) {
  ...
  private clearCalendarEvents(): void {
    if (
      ...
      this.leftCalNextIcon &&
      this.rightCalPrevIcon &&
      this.rightCalNextIcon
    ) {
  ...
  private calendarIconEvent(): void {
    ...
    if (
      this.leftCalPrevIcon &&
      !this.leftCalPrevIcon.classList.contains(DISABLED)
    ) {
      EventHandler.add(this.leftCalPrevIcon, 'mousedown', this.navPrevFunction);
    }
    if (
      this.leftCalNextIcon &&
      !this.leftCalNextIcon.classList.contains(DISABLED)
    ) {
      EventHandler.add(this.leftCalNextIcon, 'mousedown', this.navNextFunction);
    }
    if (
      this.rightCalPrevIcon &&
      !this.rightCalPrevIcon.classList.contains(DISABLED)
    ) {
      EventHandler.add(
        this.rightCalPrevIcon,
        'mousedown',
        this.navPrevFunction
      );
    }
    if (
      this.rightCalNextIcon &&
      !this.rightCalNextIcon.classList.contains(DISABLED)
    ) {
      EventHandler.add(
        this.rightCalNextIcon,
        'mousedown',
        this.navNextFunction
      );
    }
  }
  private bindCalendarEvents(): void {
    if (!this.isMobile) {
      ...
      this.headerTitleElement = <HTMLElement>(
        this.popupObj.element.querySelector(
          '.' + RIGHTCALENDER + ' .' + HEADER + ' .' + TITLE
        )
      );
      this.headerTitleElement = <HTMLElement>(
        this.popupObj.element.querySelector(
          '.' + LEFTCALENDER + ' .' + HEADER + ' .' + TITLE
        )
      );
      this.defaultKeyConfigs = extend(
      ...
      EventHandler.add(
        ...
        'click',
        this.deviceHeaderClick,
        this
      );
      EventHandler.add(
        this.endButton.element,
        'click',
        this.deviceHeaderClick,
        this
      );
  ...
  private deviceNavNext(e: MouseEvent): void {
    const calendar: HTMLElement = <HTMLElement>(
      closest(<HTMLElement>e.target, '.' + CALENDAR)
    );
  ...
  private updateDeviceCalendar(calendar: HTMLElement): void {
    if (calendar) {
      ...
      this.table = <HTMLElement>(
        calendar.querySelector('.' + CONTENT).getElementsByTagName('table')[0]
      );
      this.headerTitleElement = <HTMLElement>(
        calendar.querySelector('.' + HEADER + ' .' + TITLE)
      );
  ...
  private deviceHeaderClick(event: MouseEvent): void {
    ...
    if (
      element.classList.contains(STARTBUTTON) &&
      !isNullOrUndefined(this.startValue)
    ) {
      this.endButton.element.classList.remove(ACTIVE);
      this.startButton.element.classList.add(ACTIVE);
      const calendar: HTMLElement = <HTMLElement>(
        this.popupObj.element.querySelector('.' + CALENDAR)
      );
      this.updateDeviceCalendar(calendar);
      if (
        isNullOrUndefined(
          this.calendarElement.querySelector(
            '.' + STARTDATE + ':not(.e-other-month)'
          )
        )
      ) {
      ...
      this.updateMinMaxDays(
        <HTMLElement>this.popupObj.element.querySelector('.' + CALENDAR)
      );
    ...
    if (!this.preventBlur) {
      ...
      if (!isNullOrUndefined(value) && value.trim() !== '') {
        ...
        if (range.length > 1) {
          ...
          const start: Date = this.globalize.parseDate(
            this.getAmPmValue(range[0]),
            dateOptions
          );
          const end: Date = this.globalize.parseDate(
            this.getAmPmValue(range[1]),
            dateOptions
          );
          const startDate: Date = this.getStartEndDate(
            start,
            false,
            range,
            dateOptions
          );
          const endDate: Date = this.getStartEndDate(
            end,
            true,
            range,
            dateOptions
  ...
  private getStartEndDate(
    ...
    if (this.depth === 'Month') {
      return this.globalize.parseDate(
        this.getAmPmValue(range[isEnd ? 1 : 0]).trim(),
        dateOptions
      );
    } else if (this.depth === 'Year' && !isNullOrUndefined(date)) {
      return new Date(
        date.getFullYear(),
        date.getMonth() + (isEnd ? 1 : 0),
        isEnd ? 0 : 1
      );
  ...
  private errorClass(): void {
    const inputStr: string = !isNullOrUndefined(this.inputElement.value)
    ...
    if (
      ((isNullOrUndefined(this.endValue) &&
        ...
        (this.endValue && this.isDateDisabled(this.endValue))) &&
      inputStr !== ''
    ) {
  ...
  private navInCalendar(
    ...
    let max: Date;
    if (
      !isNullOrUndefined(this.maxDays) &&
      this.isMaxDaysClicked &&
      !isNullOrUndefined(this.startValue)
    ) {
      max = new Date(
        new Date(+this.startValue).setDate(
          this.startValue.getDate() + (this.maxDays - 1)
        )
      );
    } else {
      max = this.max;
  ...
  private firstCellToFocus(calendar: HTMLElement): Date {
    const focusAbleEle: HTMLElement =
      this.getViewNumber(this.currentView()) === 2
        ? calendar.children[1].firstElementChild.querySelector(
            'td.e-cell:not(.e-week-number):not(.e-disabled):not(.e-other-year)'
          )
        : calendar.children[1].firstElementChild.querySelector(
            'td.e-cell:not(.e-week-number):not(.e-disabled):not(.e-other-month)'
          );
    const focusEleID: string =
      focusAbleEle && focusAbleEle.id ? focusAbleEle.id.split('_')[0] : null;
    const currentFirstDay: Date = new Date(
      this.currentDate.getFullYear(),
      this.currentDate.getMonth(),
      1
    );
    const focusDate: Date = focusEleID
      ? new Date(+focusEleID)
      : currentFirstDay;
    return focusDate;
  }
  private keyInputHandler(e: KeyboardEventArgs, value?: Date): void {
    let date: Date;
    let view: number = this.getViewNumber(this.currentView());
    const rightDateLimit: Date = new Date(
      this.rightCalCurrentDate.getFullYear(),
      this.rightCalCurrentDate.getMonth(),
      1
    );
    const leftDateLimit: Date = new Date(
      this.leftCalCurrentDate.getFullYear(),
      this.leftCalCurrentDate.getMonth() + 1,
      0
    );
    let ele: HTMLElement = <HTMLElement>(
      closest(<HTMLElement>e.target, '.' + RIGHTCALENDER)
    );
    ...
    const selectedDate: Element =
      this.tableBodyElement.querySelector('tr td.e-selected');
    ...
    const levelRestrict: boolean =
      view === depthValue && this.getViewNumber(this.start) >= depthValue;
    const leftCalendar: HTMLElement = <HTMLElement>(
      closest(<HTMLElement>e.target, '.' + LEFTCALENDER)
    );
    const rightCalendar: HTMLElement = <HTMLElement>(
      closest(<HTMLElement>e.target, '.' + RIGHTCALENDER)
    );
    const presetElement: HTMLElement = <HTMLElement>(
      closest(<HTMLElement>e.target, '.' + PRESETS)
    );
    ...
    switch (e.action) {
      ...
      case 'select':
        if (levelRestrict) {
          const element: Element = !isNullOrUndefined(focusedDate)
            ? focusedDate
            : startDate;
          if (
          ...
          if (
            (!isNullOrUndefined(selectedDate) && !levelRestrict) ||
            !isNullOrUndefined(focusedDate)
          ) {
      ...
      case 'altRightArrow':
        if (!isNullOrUndefined(leftCalendar)) {
          (<HTMLElement>(
            this.rightCalendar.children[1].firstElementChild
          )).focus();
      ...
      case 'controlEnd':
        {
          ...
          super.navigateTo.call(
            this,
            'Month',
            new Date(this.currentDate.getFullYear(), 11, 31)
          );
          this.keyNavigation(ele, e);
        }
        break;
  ...
  private tabKeyValidation(ele: HTMLElement, calendarPos: string): boolean {
    ...
    const rightHeader: HTMLElement =
      this.rightCalendar.querySelector('.e-header');
    const leftHeader: HTMLElement =
      this.leftCalendar.querySelector('.e-header');
    const isRightMonth: boolean = rightHeader
      ? rightHeader.classList.contains('e-month')
      : false;
    const isLeftMonth: boolean = leftHeader
      ? leftHeader.classList.contains('e-month')
      : false;
    const isRightYear: boolean = rightHeader
      ? rightHeader.classList.contains('e-year')
      : false;
    const isLeftYear: boolean = leftHeader
      ? leftHeader.classList.contains('e-year')
      : false;
    const isRightDecade: boolean = rightHeader
      ? rightHeader.classList.contains('e-decade')
      : false;
    const isLeftDecade: boolean = leftHeader
      ? leftHeader.classList.contains('e-decade')
      : false;
    return (
      isLeftCalendar &&
      (isLeftMonth || isLeftYear || isLeftDecade) &&
      (isRightMonth || isRightYear || isRightDecade) &&
      !this.isMobile
    );
  }
  ...
  private bindCalendarCellEvents(calendar?: HTMLElement): void {
    ...
    if (calendar) {
      tdCells = <HTMLElement[] & NodeListOf<Element>>(
        calendar.querySelectorAll('.' + CALENDAR + ' td')
      );
    } else {
      tdCells = <HTMLElement[] & NodeListOf<Element>>(
    ...
    for (const cell of tdCells) {
      ...
      const disabledCell: boolean =
        cell.classList.contains(DISABLED) ||
        cell.classList.contains(DATEDISABLED);
    ...
    for (const ele of focusedDate) {
      ...
      if (
        (this.depth === 'Month' &&
          ...
          (!ele.classList.contains(TODAY) ||
            (ele.classList.contains(TODAY) && isDate))) ||
        (this.depth === 'Year' &&
          this.currentView() === 'Year' &&
          ((!this.isSameMonth(today, eleDate) &&
            !this.isSameYear(today, eleDate)) ||
            isDate)) ||
        (this.depth === 'Decade' &&
        ...
        if (
          !ele.classList.contains(STARTDATE) &&
          !ele.classList.contains(ENDDATE)
        ) {
  ...
  private hoverSelection(
    event: MouseEvent | KeyboardEventArgs | TouchEvent,
    element: Element
  ): void {
    const currentElement: HTMLElement =
      <HTMLElement>element || <HTMLElement>event.currentTarget;
    ...
    if (
      ...
      if (
        (!this.isDateDisabled(this.endValue) &&
        ...
        for (const ele of tdCells) {
          const isDisabledCell: boolean =
            !ele.classList.contains(DISABLED) ||
            ele.classList.contains(DATEDISABLED);
  ...
  private isSameStartEnd(startVal: Date, endVal: Date): boolean {
    ...
    if (this.depth === 'Month') {
      ...
      if (
        startVal.getFullYear() === endVal.getFullYear() &&
  ...
  private updateRange(elementCollection: HTMLElement[]): void {
    if (!isNullOrUndefined(this.startValue)) {
      for (const calendar of elementCollection) {
        ...
        for (const ele of tdCells) {
          if (
            !ele.classList.contains(WEEKNUMBER) &&
            !ele.classList.contains(DISABLED)
          ) {
            const eleDate: Date = this.getIdValue(null, ele);
            ...
            if (!isNullOrUndefined(this.endValue)) {
              if (
                this.currentView() === this.depth &&
                +eleDateValue.setHours(0, 0, 0, 0) >=
                  +new Date(+this.startValue).setHours(0, 0, 0, 0) &&
                +eleDateValue.setHours(0, 0, 0, 0) <=
                  +new Date(+this.endValue).setHours(0, 0, 0, 0) &&
                !this.isSameStartEnd(
                  new Date(+this.startValue),
                  new Date(+this.endValue)
                ) &&
            ...
            if (
              ele.classList.contains(RANGEHOVER) &&
              +eleDateValue > +this.endValue
            ) {
              removeClass([ele], [RANGEHOVER]);
            }
            if (!ele.classList.contains(OTHERMONTH)) {
              ...
              if (this.currentView() === 'Year') {
                eleDateValue = new Date(
                  eleDateValue.getFullYear(),
                  eleDateValue.getMonth() + 1,
                  0
                );
              ...
              if (
                +eleDate === +this.startValue &&
                !isNullOrUndefined(this.endValue) &&
                +eleDate === +this.endValue
              ) {
  ...
  private checkMinMaxDays(): void {
    if (
      (!isNullOrUndefined(this.minDays) && this.minDays > 0) ||
      (!isNullOrUndefined(this.maxDays) && this.maxDays > 0)
    ) {
      if (!this.isMobile) {
        this.updateMinMaxDays(
          <HTMLElement>this.popupObj.element.querySelector('.' + LEFTCALENDER)
        );
        this.updateMinMaxDays(
          <HTMLElement>this.popupObj.element.querySelector('.' + RIGHTCALENDER)
        );
      } else {
        this.updateMinMaxDays(
          <HTMLElement>this.popupObj.element.querySelector('.' + CALENDAR)
  ...
  private rangeArgs(
    e: MouseEvent | KeyboardEvent | TouchEvent | KeyboardEventArgs
  ): RangeEventArgs {
    ...
    if (
      ...
      range =
        Math.round(
          Math.abs(
            (this.removeTimeValueFromDate(this.startValue).getTime() -
              this.removeTimeValueFromDate(this.endValue).getTime()) /
              (1000 * 60 * 60 * 24)
          )
        ) + 1;
    } else {
  ...
  private otherMonthSelect(
    ...
    const dateIdString: string = '*[id^="/id"]:not(.e-other-month)'.replace(
      '/id',
      '' + value
    );
    const tdCell: Element =
      this.popupObj && this.popupObj.element.querySelector(dateIdString);
  ...
  private selectRange(
    ...
    const date: Date = isNullOrUndefined(event)
      ? this.getIdValue(null, element)
    ...
    if (
      ...
      this.isMobile &&
      this.startButton.element.classList.contains(ACTIVE)
    ) {
      this.removeSelection();
    }
    ...
    if (isNullOrUndefined(this.startValue)) {
      ...
      if (+date === +this.startValue || +date > +this.startValue) {
        if (
          ...
          !isNullOrUndefined(this.minDays) &&
          this.minDays > 1
        ) {
          return;
        }
        ...
        this.endValue =
          this.depth === 'Month'
            ...
            : lastMonth;
        this.setValue();
        let endEle: Element[] = <Element[] & NodeListOf<Element>>(
          this.popupObj.element.querySelectorAll('.' + ENDDATE)
        );
        if (this.isMobile) {
          this.startButton.element.classList.remove(ACTIVE);
          this.endButton.element.classList.add(ACTIVE);
        ...
        endEle = <Element[] & NodeListOf<Element>>(
          this.popupObj.element.querySelectorAll('.' + ENDDATE)
        );
        for (const ele of endEle) {
          if (ele.classList.contains(STARTDATE)) {
        ...
        removeClass(this.popupObj.element.querySelectorAll('.' + STARTDATE), [
          STARTDATE,
          SELECTED,
        ]);
        addClass([ele], STARTDATE);
    ...
    if (event) {
      leftCalendar = <HTMLElement>(
        closest(<HTMLElement>event.target, '.' + LEFTCALENDER)
      );
    }
    if (!isNullOrUndefined(leftCalendar)) {
      ...
      if (event) {
        rightCalendar =
          event &&
          <HTMLElement>closest(<HTMLElement>event.target, '.' + RIGHTCALENDER);
      }
      if (!isNullOrUndefined(rightCalendar)) {
  ...
  private selectableDates(): void {
    if (!isNullOrUndefined(this.startValue)) {
      const tdCells: HTMLElement[] = <HTMLElement[] & NodeListOf<Element>>(
        this.calendarElement.querySelectorAll('.' + CALENDAR + ' td')
      );
      ...
      if (this.currentView() === this.depth) {
        for (const ele of tdCells) {
          ...
          if (
            ele.classList.contains(STARTDATE) &&
            !ele.classList.contains(OTHERMONTH)
          ) {
            isStartDate = true;
            break;
          }
  ...
  private updateMinMaxDays(calendar: HTMLElement): void {
    if (
      ...
      (this.isMobile &&
        this.endButton &&
        this.endButton.element.classList.contains(ACTIVE))
    ) {
      if (
        (!isNullOrUndefined(this.minDays) && this.minDays > 0) ||
        (!isNullOrUndefined(this.maxDays) && this.maxDays > 0)
      ) {
        const startValueSelected: Date = this.removeTimeValueFromDate(
          this.startValue
        );
        let minDate: Date = new Date(
          new Date(+startValueSelected).setDate(
            startValueSelected.getDate() + (this.minDays - 1)
          )
        );
        let maxDate: Date = new Date(
          new Date(+startValueSelected).setDate(
            startValueSelected.getDate() + (this.maxDays - 1)
          )
        );
        minDate =
          !isNullOrUndefined(this.minDays) && this.minDays > 0 ? minDate : null;
        maxDate =
          !isNullOrUndefined(this.maxDays) && this.maxDays > 0 ? maxDate : null;
        if (this.currentView() === 'Year') {
          minDate = isNullOrUndefined(minDate)
            ? null
            : new Date(minDate.getFullYear(), minDate.getMonth(), 0);
          maxDate = isNullOrUndefined(maxDate)
            ? null
            : new Date(maxDate.getFullYear(), maxDate.getMonth(), 1);
        } else if (this.currentView() === 'Decade') {
          minDate = isNullOrUndefined(minDate)
            ? null
            : new Date(minDate.getFullYear() - 1, 11, 1);
          maxDate = isNullOrUndefined(maxDate)
            ? null
        ...
        if (!isNullOrUndefined(maxEle)) {
          if (this.isMobile) {
            ...
            let calendar: HTMLElement = <HTMLElement>(
              closest(maxEle, '.' + RIGHTCALENDER)
            );
            calendar = isNullOrUndefined(calendar)
              ? this.leftCalendar
              : calendar;
            const isLeftCalendar: boolean =
              calendar.classList.contains(LEFTCALENDER);
            if (!isLeftCalendar) {
              if (!this.rightCalNextIcon.classList.contains(DISABLED)) {
                addClass(
                  [this.rightCalNextIcon],
                  [ICONDISABLED, DISABLED, OVERLAY]
                );
              }
            } else {
              if (!this.rightCalNextIcon.classList.contains(DISABLED)) {
  ...
  private removeClassDisabled(): void {
    const tdCells: HTMLElement[] = <HTMLElement[] & NodeListOf<Element>>(
      this.popupObj.element.querySelectorAll(
        '.' + CALENDAR + ' td' + '.' + DATEDISABLED
      )
    );
    for (const ele of tdCells) {
  ...
  private updateHeader(): void {
    ...
    if (
      ...
      this.popupObj.element.querySelector('.' + DAYSPAN).textContent =
        range.toString() + ' ' + this.l10n.getConstant('days');
    } else {
      this.popupObj.element.querySelector('.' + DAYSPAN).textContent =
        this.l10n.getConstant('selectedDays');
    }
    if (!this.isMobile) {
      if (
        !isNullOrUndefined(this.endValue) &&
        ...
        this.popupObj.element.querySelector('.' + ENDLABEL).textContent =
          this.globalize.formatDate(this.endValue, format);
      } else {
        this.popupObj.element.querySelector('.' + ENDLABEL).textContent =
          this.l10n.getConstant('endLabel');
      }
      if (!isNullOrUndefined(this.startValue)) {
        this.popupObj.element.querySelector('.' + STARTLABEL).textContent =
          this.globalize.formatDate(this.startValue, format);
      } else {
        this.popupObj.element.querySelector('.' + STARTLABEL).textContent =
          this.l10n.getConstant('startLabel');
      }
    } else {
      if (!isNullOrUndefined(this.startValue)) {
        this.startButton.element.textContent = this.globalize.formatDate(
          this.startValue,
          format
        );
      } else {
        this.startButton.element.textContent =
          this.l10n.getConstant('startLabel');
      }
      if (
        !isNullOrUndefined(this.endValue) &&
        ...
        this.endButton.element.textContent = this.globalize.formatDate(
          this.endValue,
          format
        );
      } else {
    ...
    if (this.popupObj.element.querySelector('#custom_range')) {
      this.popupObj.element.querySelector('#custom_range').textContent =
        this.l10n.getConstant('customRange') !== ''
          ? this.l10n.getConstant('customRange')
          : 'Custom Range';
    }
  }
  private removeSelection(): void {
    ...
    if (this.popupObj) {
      if (this.popupObj.element.querySelectorAll('.' + SELECTED).length > 0) {
        removeClass(this.popupObj.element.querySelectorAll('.' + SELECTED), [
          STARTDATE,
          ENDDATE,
          SELECTED,
        ]);
      ...
      if (this.popupObj.element.querySelectorAll('.' + FOCUSDATE).length > 0) {
        removeClass(
          this.popupObj.element.querySelectorAll('.' + FOCUSDATE),
          FOCUSDATE
        );
      }
      if (this.popupObj.element.querySelectorAll('.' + RANGEHOVER).length > 0) {
        removeClass(this.popupObj.element.querySelectorAll('.' + RANGEHOVER), [
          RANGEHOVER,
        ]);
      }
    }
  ...
  private addSelectedAttributes(
    ...
    if (ele) {
      const title: string = this.globalize.formatDate(date, {
        type: 'date',
        skeleton: 'full',
      });
      if (!isNullOrUndefined(sameDate) && sameDate) {
        ele.setAttribute(
          'aria-label',
          'The current start and end date is ' + '' + title
        );
      } else {
        ele.setAttribute(
          'aria-label',
  ...
  private removeSelectedAttributes(): void {
    if (this.popupObj) {
      const start: Element[] = <Element[] & NodeListOf<Element>>(
        this.popupObj.element.querySelectorAll('.' + STARTDATE)
      );
      for (const ele of start) {
        ele.setAttribute('aria-selected', 'false');
        ele.removeAttribute('aria-label');
      }
      const end: Element[] = <Element[] & NodeListOf<Element>>(
        this.popupObj.element.querySelectorAll('.' + ENDDATE)
      );
  ...
  private navPrevMonth(e: MouseEvent): void {
    ...
    ele = isNullOrUndefined(ele)
      ? <HTMLElement>closest(<HTMLElement>e.target, '.' + RIGHTCALENDER)
      : ele;
    this.updateCalendarElement(ele);
    this.navigatePrevious(e);
    if (
  ...
  private updateControl(calendar: HTMLElement, customDate: Date = null): void {
    if (calendar.classList.contains(RIGHTCALENDER)) {
      this.rightCalCurrentDate = new Date(
  ...
  private isPopupOpen(): boolean {
    if (
      ...
      this.popupObj.element.classList.contains(POPUP)
    ) {
      return true;
    }
    return false;
  ...
  protected createRangeHeader(): HTMLElement {
    const labelContainer: HTMLElement = this.createElement('div', {
      className: STARTENDCONTAINER,
    });
    if (!this.isMobile) {
      const startLabel: HTMLElement = this.createElement('a', {
        className: STARTLABEL,
      });
      const endLabel: HTMLElement = this.createElement('a', {
        className: ENDLABEL,
      });
      const changeIcon: HTMLElement = this.createElement('span', {
      ...
      const endBtn: HTMLElement = this.createElement('button', {
        className: ENDBUTTON,
      });
      const startBtn: HTMLElement = this.createElement('button', {
        className: STARTBUTTON,
      });
      this.startButton = new Button(
        { content: this.l10n.getConstant('startLabel') },
        <HTMLButtonElement>startBtn
      );
      this.endButton = new Button(
        { content: this.l10n.getConstant('endLabel') },
        <HTMLButtonElement>endBtn
      );
  ...
  private disableInput(): void {
    if (this.strictMode) {
      if (
        !isNullOrUndefined(this.previousStartValue) &&
        !isNullOrUndefined(this.previousEndValue)
      ) {
  ...
  private validateMinMax(): void {
    this.min =
      isNullOrUndefined(this.min) || !+this.min
        ? (this.min = new Date(1900, 0, 1))
        : this.min;
    this.max =
      isNullOrUndefined(this.max) || !+this.max
        ? (this.max = new Date(2099, 11, 31))
        : this.max;
  ...
  private startMonthCurrentDate(): void {
    if (
      this.isSameMonth(this.min, this.max) ||
      +this.currentDate > +this.max ||
      this.isSameMonth(this.currentDate, this.max)
    ) {
  ...
  private selectNextMonth(): void {
    if (
      !isNullOrUndefined(this.endValue) &&
      !isNullOrUndefined(this.startValue) &&
      !this.isSameMonth(this.endValue, this.currentDate) &&
      !this.isDateDisabled(this.endValue) &&
      !this.isDateDisabled(this.startValue)
    ) {
      this.currentDate = new Date(+this.endValue);
  ...
  private selectNextYear(): void {
    if (
      !isNullOrUndefined(this.endValue) &&
      !isNullOrUndefined(this.startValue) &&
      !this.isSameYear(this.endValue, this.currentDate) &&
      !this.isDateDisabled(this.endValue) &&
      !this.isDateDisabled(this.startValue)
    ) {
      this.currentDate = new Date(+this.endValue);
  ...
  private selectNextDecade(): void {
    if (
      !isNullOrUndefined(this.endValue) &&
      !isNullOrUndefined(this.startValue) &&
      !this.isSameDecade(this.endValue, this.currentDate) &&
      !this.isDateDisabled(this.endValue) &&
      !this.isDateDisabled(this.startValue)
    ) {
      this.currentDate = new Date(+this.endValue);
  ...
  private selectStartMonth(): void {
    if (!isNullOrUndefined(this.startValue)) {
      if (
        !isNullOrUndefined(this.max) &&
        this.isSameMonth(this.startValue, this.max)
      ) {
        this.currentDate = new Date(+this.max);
        this.currentDate.setDate(1);
        this.currentDate.setMonth(this.currentDate.getMonth() - 1);
      } else if (
        !(this.startValue >= this.min && this.startValue <= this.max) ||
        this.isDateDisabled(this.startValue)
      ) {
        this.currentDate = new Date(new Date().setHours(0, 0, 0, 0));
      } else {
  ...
  private createCalendar(): HTMLElement {
    const calendarContainer: HTMLElement = this.createElement('div', {
    ...
    if (!this.isMobile) {
      ...
      this.leftCalPrevIcon = <HTMLElement>(
        this.calendarElement.querySelector('.' + LEFTCALENDER + ' .' + PREVICON)
      );
      this.leftCalNextIcon = <HTMLElement>(
        this.calendarElement.querySelector('.' + LEFTCALENDER + ' .' + NEXTICON)
      );
      this.leftTitle = <HTMLElement>(
        this.calendarElement.querySelector('.' + LEFTCALENDER + ' .' + TITLE)
      );
      remove(
        this.calendarElement.querySelector(
          '.' + LEFTCALENDER + ' .' + ICONCONTAINER
        )
      );
      this.calendarElement
        .querySelector('.' + LEFTCALENDER + ' .' + HEADER)
        .appendChild(this.leftCalNextIcon);
      this.calendarElement
        .querySelector('.' + LEFTCALENDER + ' .' + HEADER)
        .appendChild(this.leftCalPrevIcon);
      prepend(
        [this.leftCalPrevIcon],
        this.calendarElement.querySelector('.' + LEFTCALENDER + ' .' + HEADER)
      );
      this.leftCalendar = this.calendarElement;
      const leftContainer: HTMLElement = this.createElement('div', {
        className: LEFTCONTAINER,
      });
      const rightContainer: HTMLElement = this.createElement('div', {
      ...
      removeClass(
        [
          this.leftCalendar &&
            this.leftCalendar.querySelector('.e-content tbody'),
        ],
        'e-zoomin'
      );
      removeClass(
        [
          this.rightCalendar &&
            this.rightCalendar.querySelector('.e-content tbody'),
        ],
        'e-zoomin'
      );
      this.rightCalPrevIcon = <HTMLElement>(
        this.calendarElement.querySelector(
          '.' + RIGHTCALENDER + ' .' + PREVICON
        )
      );
      this.rightCalNextIcon = <HTMLElement>(
        this.calendarElement.querySelector(
          '.' + RIGHTCALENDER + ' .' + NEXTICON
        )
      );
      this.rightTitle = <HTMLElement>(
        this.calendarElement.querySelector('.' + RIGHTCALENDER + ' .' + TITLE)
      );
      remove(
        this.calendarElement.querySelector(
          '.' + RIGHTCALENDER + ' .' + ICONCONTAINER
        )
      );
      this.calendarElement.querySelector('table').setAttribute('tabindex', '0');
      this.calendarElement
        .querySelector('.' + RIGHTCALENDER + ' .' + HEADER)
        .appendChild(this.rightCalNextIcon);
      this.calendarElement
        .querySelector('.' + RIGHTCALENDER + ' .' + HEADER)
        .appendChild(this.rightCalPrevIcon);
      prepend(
        [this.rightCalPrevIcon],
        this.calendarElement.querySelector('.' + RIGHTCALENDER + ' .' + HEADER)
      );
      ...
      super.minMaxUpdate();
      super.render();
      const prevIcon: HTMLElement = <HTMLElement>(
        this.calendarElement.querySelector('.' + CALENDAR + ' .' + PREVICON)
      );
      const nextIcon: HTMLElement = <HTMLElement>(
        this.calendarElement.querySelector('.' + CALENDAR + ' .' + NEXTICON)
      );
      remove(
        this.calendarElement.querySelector(
          '.' + CALENDAR + ' .' + ICONCONTAINER
        )
      );
      this.calendarElement
      ...
      this.calendarElement
        .querySelector('.' + CALENDAR + ' .' + HEADER)
        .appendChild(prevIcon);
      prepend(
        [prevIcon],
        this.calendarElement.querySelector('.' + CALENDAR + ' .' + HEADER)
      );
    ...
    if (!isNullOrUndefined(this.popupObj)) {
      ...
      if (this.isMobile) {
        ...
        if (!this.isMobile && view === this.depth) {
          element = this.calendarElement.classList.contains('e-left-calendar')
            ? this.leftCalendar
            : this.rightCalendar;
          if (
            element === this.leftCalendar &&
            ((e &&
              !(<HTMLElement>e.currentTarget).children[0].classList.contains(
                'e-icons'
              )) ||
            ...
            const customDate: Date = this.rightCalCurrentDate
              ? this.rightCalCurrentDate
              : this.currentDate;
  ...
  private createControl(): void {
    const controlContainer: HTMLElement = this.createElement('div', {
      className: RANGECONTAINER,
    });
    const headerContainer: HTMLElement = this.createElement('div', {
      className: RANGEHEADER,
    });
    if (this.isMobile && this.fullScreenMode) {
      const modelHeaderIconWrapper: any = this.createElement('div', {
        className: 'e-model-header-wrapper',
      });
      const modelCloseIcon: any = this.createElement('span', {
        className: 'e-popup-close',
      });
      EventHandler.add(
        modelCloseIcon,
        'mousedown touchstart',
        this.modelRangeCloseHandler,
        this
      );
      const modelApplyButton: any = this.createElement('span', {
        className: 'e-apply',
      });
      EventHandler.add(
        modelApplyButton,
    ...
    const daySpan: HTMLElement = this.createElement('div', {
      className: DAYSPAN,
    });
    ...
    const footerSection: HTMLElement = this.createElement('div', {
      className: FOOTER,
    });
    const cancelBtn: HTMLButtonElement = <HTMLButtonElement>(
      this.createElement('button', {
        className: CANCEL + ' ' + FLAT + ' ' + CSS,
      })
    );
    const applyBtn: HTMLButtonElement = <HTMLButtonElement>(
      this.createElement('button')
    );
    ...
    this.applyButton = new Button(
      { content: this.l10n.getConstant('applyText'), disabled: !enable },
      applyBtn
    );
    ...
    if (!this.isMobile) {
      if (
        ...
        const presets: HTMLElement = <HTMLElement>(
          this.popupWrapper.querySelector('.' + PRESETS)
        );
        presets.style.height =
          this.popupWrapper
            .querySelector('.' + RANGECONTAINER)
  ...
  private applyFunction(eve?: MouseEvent | KeyboardEventArgs): void {
    ...
    if (this.closeEventArgs && this.closeEventArgs.cancel) {
      this.startValue =
        this.popupWrapper.querySelector('.e-start-date') &&
        this.getIdValue(null, this.popupWrapper.querySelector('.e-start-date'));
      this.endValue =
        this.popupWrapper.querySelector('.e-end-date') &&
    ...
    if (
      ...
      if (
        !(
          ...
          this.startValue.getFullYear() ===
            this.previousStartValue.getFullYear() &&
      ...
      this.previousEleValue = this.inputElement.value;
      if (
        +this.initStartDate !== +this.startValue ||
        +this.initEndDate !== +this.endValue
      ) {
        isValueChanged = true;
      }
    ...
    if (
      !closest(eve.target as HTMLElement, '.' + INPUTCONTAINER) &&
      !isValueChanged
    ) {
      this.focusOut();
    }
  ...
  private onMouseClick(
    ...
    if (!this.isMobile) {
      ...
      if (
        li &&
        li.classList.contains(LISTCLASS) &&
        li.getAttribute('id') === 'custom_range'
      ) {
        (<HTMLElement>this.leftCalendar.children[1].firstElementChild).focus();
      } else {
  ...
  private onMouseOver(event: MouseEvent): void {
    const li: HTMLElement = <HTMLElement>(
      closest(<Element>event.target, '.' + LISTCLASS)
    );
    if (
      li &&
      li.classList.contains(LISTCLASS) &&
      !li.classList.contains(HOVER)
    ) {
      addClass([li], HOVER);
    }
  }
  private onMouseLeave(event: MouseEvent): void {
    const item: HTMLElement = <HTMLElement>(
      closest(<Element>event.target, '.' + HOVER)
    );
  ...
  private setListSelection(
    ...
    if (
      ...
      (!li.classList.contains(ACTIVE) ||
        (this.isMobile && li.classList.contains(ACTIVE)))
    ) {
      if (this.isMobile && li.classList.contains(ACTIVE)) {
        this.activeIndex = Array.prototype.slice
          .call(this.liCollections)
          .indexOf(li);
        const values: { [key: string]: Object } =
          this.presetsItem[this.activeIndex];
  ...
  private removeListSelection(): void {
    const item: HTMLElement = <HTMLElement>(
      this.presetElement.querySelector('.' + ACTIVE)
    );
    if (!isNullOrUndefined(item)) {
  ...
  private showPopup(
    element?: HTMLElement,
    event?: MouseEvent | KeyboardEventArgs | Event
  ): void {
    this.presetHeight();
    if (this.zIndex === 1000) {
  ...
  private createPresets(): void {
    if (
      ...
      const listTag: HTMLElement = ListBase.createList(
        this.createElement,
        this.presetsItem,
        null,
        true
      ...
      const customElement: HTMLElement =
        this.presetElement.querySelector('#custom_range');
      ...
      this.liCollections = <HTMLElement[] & NodeListOf<Element>>(
        this.presetElement.querySelectorAll('.' + LISTCLASS)
      );
      this.wireListEvents();
      if (this.isMobile) {
        if (this.fullScreenMode) {
          const modelWrapper: HTMLElement = createElement('div', {
            className: 'e-range-mob-popup-wrap',
          });
          const modelHeader: HTMLElement = this.createElement('div', {
            className: 'e-model-header',
          });
          const modelTitleSpan: any = this.createElement('span', {
          ...
          this.popupWrapper.insertBefore(
            modelWrapper,
            this.popupWrapper.firstElementChild
          );
          this.presetElement.style.width = '100%';
        } else {
          this.presetElement.style.width =
            this.inputWrapper.container.getBoundingClientRect().width + 'px';
        }
      }
  ...
  private renderPopup(): void {
    ...
    this.popupObj = new Popup(this.popupWrapper as HTMLElement, {
      relateTo:
        this.isMobile && this.isCustomWindow
          ? document.body
      ...
      position: this.isMobile
        ? !isUndefined(
            this.presets[0].start &&
              this.presets[0].end &&
              this.presets[0].label
          ) && !this.isCustomWindow
          ? { X: 'left', Y: 'bottom' }
          : { X: 'center', Y: 'center' }
        : this.enableRtl
      ...
      collision: this.isMobile
        ...
        : { X: 'fit', Y: 'flip' },
      targetType:
        this.isMobile && this.isCustomWindow ? 'container' : 'relative',
      ...
      open: () => {
        if (this.isMobile && this.fullScreenMode) {
          this.iconRangeRight =
            ...
            window.getComputedStyle(
              this.calendarElement.querySelector('.e-header.e-month .e-prev')
            ).cssFloat;
          if (this.iconRangeRight) {
            this.touchRangeModule = new Touch(
              <HTMLElement>(
                this.calendarElement.querySelector('.e-content.e-month')
              ),
              {
        ...
        attributes(this.inputElement, {
          'aria-expanded': 'true',
          'aria-owns': this.element.id,
          'aria-controls': this.inputElement.id,
        });
        if (this.value) {
          attributes(this.inputElement, {
            'aria-activedescendant': this.inputElement.id,
          });
        } else {
      ...
      targetExitViewport: () => {
        ...
        if (
          !Browser.isDevice &&
          popupEle &&
          popupEle.getBoundingClientRect().height < window.innerHeight
        ) {
          this.hide();
        }
      },
  ...
  private calendarFocus(): void {
    const startDate: Element =
      this.popupObj && this.popupObj.element.querySelector('.' + STARTDATE);
    if (startDate) {
      let ele: HTMLElement = <HTMLElement>(
        closest(startDate, '.' + RIGHTCALENDER)
      );
      ele = isNullOrUndefined(ele) ? this.leftCalendar : ele;
      if (this.isRangeIconClicked) {
  ...
  private presetHeight(): void {
    const presets: HTMLElement =
      this.popupObj &&
      <HTMLElement>this.popupObj.element.querySelector('.' + PRESETS);
    const rangeContainer: HTMLElement =
      this.popupObj &&
      <HTMLElement>this.popupObj.element.querySelector('.' + RANGECONTAINER);
    if (!isNullOrUndefined(presets) && !isNullOrUndefined(rangeContainer)) {
      presets.style.height =
        rangeContainer.getBoundingClientRect().height + 'px';
    }
  }
      ...
      case 'enter':
        {
          ...
          if (
            !isNullOrUndefined(this.leftCalendar) &&
            !isNullOrUndefined(actItem)
          ) {
            if (
              isNullOrUndefined(hvrItem) ||
              (!isNullOrUndefined(actItem) && actItem === hvrItem)
            ) {
              this.activeIndex = Array.prototype.slice
                .call(this.liCollections)
                .indexOf(actItem);
  ...
  private getHoverLI(): HTMLElement {
    const item: HTMLElement = <HTMLElement>(
      this.presetElement.querySelector('.' + HOVER)
    );
    return item;
  }
  ...
  private setScrollPosition(): void {
    const listHeight: number =
      this.presetElement.getBoundingClientRect().height;
    const hover: HTMLElement = <HTMLElement>(
      this.presetElement.querySelector('.' + HOVER)
    );
    const active: HTMLElement = <HTMLElement>(
      this.presetElement.querySelector('.' + ACTIVE)
    );
    const element: HTMLElement = !isNullOrUndefined(hover) ? hover : active;
    if (!isNullOrUndefined(element)) {
      const nextEle: Element = element.nextElementSibling;
      const height: number = nextEle
        ? (<HTMLElement>nextEle).offsetTop
        : element.offsetTop;
      const liHeight: number = element.getBoundingClientRect().height;
      if (height + element.offsetTop > listHeight) {
        this.presetElement.scrollTop = nextEle
          ? height - (listHeight / 2 + liHeight / 2)
          : height;
      } else {
    ...
    switch (e.action) {
      ...
      case 'altRightArrow':
        if (!isNullOrUndefined(presetElement)) {
          ...
          if (
            document.activeElement === this.cancelButton.element &&
            this.applyButton.element.disabled !== true
          ) {
            this.applyButton.element.focus();
          } else {
      ...
      case 'altLeftArrow':
        if (!isNullOrUndefined(presetElement)) {
          ...
          if (
            document.activeElement === this.applyButton.element &&
            this.applyButton.element.disabled !== true
          ) {
            this.cancelButton.element.focus();
          } else {
            if (
              !isNullOrUndefined(this.presetElement) &&
              document.activeElement === this.cancelButton.element
            ) {
              this.presetElement.focus();
            } else {
  ...
  private documentHandler(e: MouseEvent): void {
    ...
    if (
      !this.inputWrapper.container.contains(target as Node) ||
      (!isNullOrUndefined(this.popupObj) &&
    ...
    if (
      ...
      !(
        closest(target, '.e-daterangepicker.e-popup') &&
  ...
  private refreshControl(): void {
    ...
    this.errorClass();
    this.previousStartValue =
      isNullOrUndefined(this.startValue) || isNaN(+this.startValue)
        ? null
        : new Date(+this.startValue);
    this.previousEndValue =
      isNullOrUndefined(this.endValue) || isNaN(+this.endValue)
        ? null
        : new Date(+this.endValue);
  }
  private updateInput(): void {
    if (
      ...
      const startDate: string = this.globalize.formatDate(
        this.startValue,
        formatOption
      );
      const endDate: string = this.globalize.formatDate(
        this.endValue,
        formatOption
      );
      Input.setValue(
        startDate + ' ' + this.separator + ' ' + endDate,
        this.inputElement,
        this.floatLabelType,
        this.showClearButton
      );
  ...
  protected checkInvalidRange(value: string | DateRange | Date[]): void {
    if (!isNullOrUndefined(value)) {
      ...
      if (typeof value === 'string') {
        const range: string[] = (<string>value).split(
          ' ' + this.separator + ' '
        );
        if (range.length === 2) {
          startString = range[0];
        ...
        if (
          !(startinvalue instanceof Date) &&
          typeof startinvalue !== 'object'
        ) {
      ...
      if (
        (isNullOrUndefined(startString) &&
          !startObject &&
          !isNullOrUndefined(endString)) ||
        (!isNullOrUndefined(startString) &&
          !endObject &&
          isNullOrUndefined(endString))
      ) {
    ...
    if (typeof valueString !== 'string') {
      ...
      if (!this.checkDateValue(globalize.parseDate(valueString, formatOpt))) {
        ...
        basISOString =
          /^\s*((?:[+-]\d{6}|\d{4})(?:\d\d\d\d|W\d\d\d|W\d\d|\d\d\d|\d\d))(?:(T| )(\d\d(?:\d\d(?:\d\d(?:[.,]\d+)?)?)?)([\+\-]\d\d(?::?\d\d)?|\s*Z)?)?/;
        if (
          (!extISOStr.test(valueString) && !basISOString.test(valueString)) ||
          /^[a-zA-Z0-9- ]*$/.test(valueString) ||
          isNaN(+new Date(this.checkValue(valueString)))
        ) {
          invalid = true;
        }
      }
  ...
  private setModelValue(): void {
    if (!this.value && this.startDate === null && this.endDate === null) {
      ...
      if (
        (this.value && (<Date[]>this.value).length > 0) ||
        (this.valueType && (<Date[]>this.valueType).length > 0)
      ) {
        if (
          +this.startDate !== +(<Date[]>this.value)[0] ||
          +this.endDate !== +(<Date[]>this.value)[1]
        ) {
          this.setProperties({ value: [this.startDate, this.endDate] }, true);
        }
        if (
          this.value &&
          (<Date[]>this.value)[0] == null &&
          (<Date[]>this.value)[1] == null
        ) {
          this.setProperties({ value: null }, true);
        }
      } else {
  ...
  private changeTrigger(
    e?: MouseEvent | KeyboardEvent | KeyboardEventArgs
  ): void {
    if (
      +this.initStartDate !== +this.startValue ||
      +this.initEndDate !== +this.endValue
  ...
  public destroy(): void {
    ...
    if (this.inputElement) {
      ...
      if (!isNullOrUndefined(this.inputWrapper)) {
        ...
        if (this.angularTag === null) {
          this.inputWrapper.container.parentElement.appendChild(
            this.inputElement
          );
        }
    ...
    if (
      !isNullOrUndefined(this.firstHiddenChild) &&
  ...
  protected ensureInputAttribute(): void {
    ...
    for (let i: number = 0; i < attr.length; i++) {
      if (
        isNullOrUndefined(this.cloneElement.getAttribute(attr[i as number]))
      ) {
        ...
        if (attr[i as number].toLowerCase() === 'value') {
          this.inputElement.value = this.cloneElement.getAttribute(
            attr[i as number]
          );
        }
        this.inputElement.setAttribute(
          attr[i as number],
          this.cloneElement.getAttribute(attr[i as number])
        );
      }
    }
  }
  ...
  public show(
    ...
    if ((this.enabled && this.readonly) || !this.enabled || this.popupObj) {
      ...
      if (!this.isPopupOpen()) {
        ...
        this.trigger('open', eventArgs, (eventArgs: RangePopupEventArgs) => {
          ...
          if (!this.openEventArgs.cancel) {
            ...
            const isPreset: boolean =
              !this.isCustomRange || (this.isMobile && this.isCustomRange);
            if (
              !isUndefined(
                this.presets[0].start &&
                  this.presets[0].end &&
        ...
        if (Browser.isDevice) {
          const dlgOverlay: HTMLElement = this.createElement('div', {
      ...
      if (this.isPopupOpen()) {
        ...
        this.trigger('close', eventArgs, (eventArgs: RangePopupEventArgs) => {
          ...
          if (!this.closeEventArgs.cancel) {
            if (this.isMobile) {
              if (
                !isNullOrUndefined(this.startButton) &&
                !isNullOrUndefined(this.endButton)
              ) {
                EventHandler.remove(
                  this.startButton.element,
                  'click touchstart',
            ...
            if (!this.isMobile) {
              if (
                !isNullOrUndefined(this.leftKeyboardModule) &&
                !isNullOrUndefined(this.rightKeyboardModule)
              ) {
  ...
  private clearModelvalue(
    newProp: DateRangePickerModel,
    oldProp: DateRangePickerModel
  ): void {
    this.setProperties({ startDate: null }, true);
    this.setProperties({ endDate: null }, true);
  ...
  private getAmPmValue(date: string | null | undefined): string {
    try {
      if (typeof date === 'string' && date.trim() !== '') {
        ...
        return date.replace(/(am|pm|Am|aM|pM|Pm)/g, (match: string) =>
      ...
      switch (prop) {
        ...
        case 'locale':
          ...
          if (
            ...
            this.dateRangeOptions.placeholder == null
          ) {
            this.setProperties(
              { placeholder: this.l10n.getConstant('placeholder') },
              true
        ...
        case 'startDate':
          if (typeof newProp.startDate === 'string') {
            newProp.startDate = this.globalize.parseDate(
              this.getAmPmValue(<string>newProp.startDate),
              format
            );
          }
        ...
        case 'endDate':
          if (typeof newProp.endDate === 'string') {
            newProp.endDate = this.globalize.parseDate(
              this.getAmPmValue(<string>newProp.endDate),
              format
            );
          }
        ...
        case 'value':
          ...
          if (typeof newProp.value === 'string') {
            if (!this.invalidValueString) {
              const rangeArray: string[] = (<string>newProp.value).split(
                ' ' + this.separator + ' '
              );
              this.value = [new Date(rangeArray[0]), new Date(rangeArray[1])];
              this.updateValue();
            ...
            if (
              (!isNullOrUndefined(newProp.value) &&
                (<Date[]>newProp.value).length > 0) ||
              (!isNullOrUndefined(newProp.value) &&
                (<DateRange>newProp.value).start)
            ) {
              this.valueType = newProp.value;
              if (
                (<Date[]>newProp.value)[0] === null ||
                (<DateRange>newProp.value).start === null
              ) {
                if (
                  (<Date[]>newProp.value).length === 1 ||
                  (<DateRange>newProp.value).start
                ) {
                  this.clearModelvalue(newProp, oldProp);
                } else if (
                  (<Date[]>newProp.value)[1] === null ||
                  (<DateRange>newProp.value).start === null
                ) {
                  this.clearModelvalue(newProp, oldProp);
                }
              } else if (
                +this.initStartDate !== +(<Date[]>newProp.value)[0] ||
          ...
          if (this.isPopupOpen()) {
            ...
            if (this.isMobile) {
              this.navigate(
                this.deviceCalendar,
                this.startValue,
                this.currentView() as CalendarView
              );
              this.deviceHeaderUpdate();
            } else {
              this.navigate(
            ...
            this.updateHeader();
            this.applyButton.disabled = this.applyButton.element.disabled =
              false;
          }
          this.preventChange =
