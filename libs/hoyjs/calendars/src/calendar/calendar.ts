...
import { cldrData, getDefaultDateObject, rippleEffect } from '@hoyui/base';
import {
  removeClass,
  detach,
  closest,
...
import { getValue, getUniqueID, extend, Browser } from '@hoyui/base';
import {
  Property,
  Event,
  EmitType,
  ...
  implements INotifyPropertyChanged
{
  ...
  protected setOverlayIndex(
    ...
    if (
      isDevice &&
      !isNullOrUndefined(popupElement) &&
      !isNullOrUndefined(modal) &&
      !isNullOrUndefined(popupWrapper)
    ) {
      const index: number = parseInt(popupElement.style.zIndex, 10)
        ? parseInt(popupElement.style.zIndex, 10)
        : 1000;
  ...
  protected createHeader(): void {
    ...
    const iconContainer: HTMLElement = this.createElement('div', {
    ...
    attributes(
      ...
      <{ [key: string]: string }>ariaTitleAttrs
    );
    attributes(
      this.headerTitleElement,
      <{ [key: string]: string }>tabIndexAttr
    );
    this.headerElement.appendChild(this.headerTitleElement);
    this.previousIcon.appendChild(
      this.createElement('span', { className: '' + PREVSPAN + ' ' + ICON })
    );
    this.nextIcon.appendChild(
      this.createElement('span', { className: '' + NEXTSPAN + ' ' + ICON })
    );
  ...
  private addContentFocus(args: any): void {
    const focusedDate: Element = this.tableBodyElement.querySelector(
      'tr td.e-focused-date'
    );
  ...
  private removeContentFocus(args: any): void {
    const focusedDate: Element = !isNullOrUndefined(this.tableBodyElement)
      ? this.tableBodyElement.querySelector('tr td.e-focused-date')
      : null;
    const selectedDate: Element = !isNullOrUndefined(this.tableBodyElement)
      ? this.tableBodyElement.querySelector('tr td.e-selected')
      : null;
    if (!isNullOrUndefined(selectedDate)) {
  ...
  protected getCultureValues(): string[] {
    ...
    let cldrObj: string[];
    const dayFormat: string = !isNullOrUndefined(this.dayHeaderFormat)
      ? 'days.stand-alone.' + this.dayHeaderFormat.toLowerCase()
      : null;
    if (
      (this.locale === 'en' || this.locale === 'en-US') &&
      !isNullOrUndefined(dayFormat)
    ) {
  ...
  protected toCapitalize(text: string): string {
    return !isNullOrUndefined(text) && text.length
  ...
  protected createContentHeader(): void {
    if (this.getModuleName() === 'calendar') {
      if (
        !isNullOrUndefined(
          this.element.querySelectorAll('.e-content .e-week-header')[0]
        )
      ) {
      ...
      if (
        !isNullOrUndefined(
          this.calendarElement.querySelectorAll('.e-content .e-week-header')[0]
        )
      ) {
        detach(
          this.calendarElement.querySelectorAll('.e-content .e-week-header')[0]
    ...
    const shortNames: string[] =
      this.getCultureValues().length > 0 && this.getCultureValues()
        ? this.shiftArray(
            this.getCultureValues().length > 0 && this.getCultureValues(),
            this.firstDayOfWeek
          )
    ...
    if (!isNullOrUndefined(shortNames)) {
      for (let days: number = 0; days <= daysCount; days++) {
        html +=
          '<th  class="">' +
          this.toCapitalize(shortNames[days as number]) +
          '</th>';
      }
    }
    html = '<tr>' + html + '</tr>';
  ...
  protected createContentBody(): void {
    if (this.getModuleName() === 'calendar') {
      if (
        !isNullOrUndefined(this.element.querySelectorAll('.e-content tbody')[0])
      ) {
      ...
      if (
        !isNullOrUndefined(
          this.calendarElement.querySelectorAll('.e-content tbody')[0]
        )
      ) {
  ...
  protected createContentFooter(): void {
    if (this.showTodayButton) {
      ...
      if (
        !(
          +new Date(minimum.setHours(0, 0, 0, 0)) <= +this.todayDate &&
          +this.todayDate <= +new Date(maximum.setHours(0, 0, 0, 0))
        ) ||
        this.todayDisabled
      ) {
  ...
  protected wireEvents(
    ...
    if (this.getModuleName() === 'calendar') {
      this.keyboardModule = new KeyboardEvents(<HTMLElement>this.element, {
        eventName: 'keydown',
  ...
  protected dateWireEvents(
    ...
    this.defaultKeyConfigs = extend(this.defaultKeyConfigs, keyConfig) as {
      [key: string]: string;
    };
    this.serverModuleName = moduleName;
  }
  protected todayButtonClick(
    ...
    if (this.showTodayButton) {
      ...
      if (this.getViewNumber(this.start) >= this.getViewNumber(this.depth)) {
        this.navigateTo(
          this.depth,
          new Date(this.checkValue(value)),
          isCustomDate
        );
      } else {
        this.navigateTo(
  ...
  protected resetCalendar(): void {
    ...
    this.calendarElement =
      ...
      this.footer =
      this.tableHeadElement =
      this.nextIcon =
      this.previousIcon =
      this.table =
        null;
  }
  protected keyActionHandle(
    e: KeyboardEventArgs,
    value?: Date,
    ...
    if (multiSelection) {
      if (
        ...
        +value === parseInt(focusedDate.getAttribute('id').split('_')[0], 10)
      ) {
    ...
    switch (e.action) {
      case 'moveLeft':
        if (
          this.getModuleName() !== 'daterangepicker' &&
          !isNullOrUndefined(e.target as any)
        ) {
      ...
      case 'select':
        if (e.target === this.headerTitleElement) {
          ...
          e.target === this.previousIcon &&
          !(e.target as HTMLElement).className.includes(DISABLED)
        ) {
          this.navigatePrevious(e);
        } else if (
          e.target === this.nextIcon &&
          !(e.target as HTMLElement).className.includes(DISABLED)
        ) {
          this.navigateNext(e);
        } else if (
      ...
      case 'controlDown':
        if (
          (!isNullOrUndefined(focusedDate) && !levelRestrict) ||
  ...
  protected renderMonths(
    ...
    if (this.calendarMode === 'Gregorian') {
      ...
      tdEles = !isNullOrUndefined(this.islamicModule)
        ? this.islamicModule.islamicRenderDays(this.currentDate, value)
        : null;
    }
    this.createContentHeader();
    if (this.calendarMode === 'Gregorian') {
      ...
      this.islamicModule.islamicRenderTemplate(
        tdEles,
        numCells,
        MONTH,
        e,
  ...
  protected renderDays(
    ...
    const todayDate: Date = isTodayDate
      ? new Date(+currentDate)
      : this.getDate(new Date(), this.timezone);
    ...
    for (let day: number = 0; day < cellsCount; ++day) {
      const weekEle: HTMLElement = this.createElement('td', {
        className: CELL,
      });
      ...
      if (day % 7 === 0 && this.weekNumber) {
        ...
        const numberOfDays: number =
          this.weekRule === 'FirstDay'
            ? 6
            : this.weekRule === 'FirstFourDayWeek'
            ? 3
            : 0;
      ...
      const date: Date = this.globalize.parseDate(
        this.globalize.formatDate(localDate, dateFormatOptions),
        dateFormatOptions
      );
      const tdEle: HTMLElement = this.dayCell(localDate);
      const title: string = this.globalize.formatDate(localDate, {
        type: 'date',
        skeleton: 'full',
      });
        ...
        if (multiSelection) {
          if (!isNullOrUndefined(values) && values.length > 0) {
            for (let index: number = 0; index < values.length; index++) {
              const localDateString: number = +new Date(
                this.globalize.formatDate(argument.date, {
                  type: 'date',
                  skeleton: 'yMd',
                })
              );
              const tempDateString: number = +new Date(
                this.globalize.formatDate(values[index as number], {
                  type: 'date',
                  skeleton: 'yMd',
                })
              );
      ...
      if (
        ...
        !tdEle.classList.contains(SELECTED)
      ) {
        addClass([tdEle], DISABLED);
        addClass([tdEle], OVERLAY);
        dayLink.setAttribute('aria-disabled', 'true');
      ...
      if (multiSelection && !isNullOrUndefined(values) && !disabledCls) {
        for (
          ...
          const type: string =
            this.calendarMode === 'Gregorian' ? 'gregorian' : 'islamic';
          ...
          const localDateString: string = this.globalize.formatDate(
            localDate,
            formatOptions
          );
          const tempDateString: string = this.globalize.formatDate(
            values[tempValue as number],
            formatOptions
          );
          if (
            (localDateString === tempDateString &&
              this.getDateVal(localDate, values[tempValue as number])) ||
          ...
          if (
            ...
            currentTarget.innerText === tdEle.innerText &&
            this.previousDates &&
            tdEle.classList.contains(SELECTED) &&
            ...
            for (let i: number = 0; i < copyValues.length; i++) {
              ...
              const tempDateString: string = this.globalize.formatDate(
                copyValues[i as number],
                formatOptions
              );
              if (localDateString === tempDateString) {
                const index: number = copyValues.indexOf(
                  copyValues[i as number]
                );
  ...
  protected updateFocus(
    ...
    if (
      ...
      if (
        ...
        parseInt(tableElement.id, 0) === +this.max &&
        !otherMonth &&
        !disabled
      ) {
        addClass([tableElement], FOCUSEDDATE);
      ...
      if (
        ...
        parseInt(tableElement.id, 0) === +this.min &&
        !otherMonth &&
        !disabled
      ) {
        addClass([tableElement], FOCUSEDDATE);
  ...
  protected renderYears(e?: Event, value?: Date): void {
    ...
    for (let month: number = 0; month < 12; ++month) {
      const tdEle: HTMLElement = this.dayCell(localDate);
      const dayLink: HTMLElement = this.createElement('span');
      const localMonth: boolean =
        value && value.getMonth() === localDate.getMonth();
      const select: boolean = value && value.getFullYear() === yr && localMonth;
      const title: string = this.globalize.formatDate(localDate, {
        type: 'date',
        format: 'MMM y',
      });
      dayLink.textContent = this.toCapitalize(
      ...
      if (
        (this.min &&
          (curYrs < minYr || (month < minMonth && curYrs === minYr))) ||
        ...
        if (
          localDate.getMonth() === mon &&
          this.currentDate.getMonth() === mon
        ) {
  ...
  protected renderDecades(e?: Event, value?: Date): void {
    ...
    const startYr: Date = new Date(
      localDate.setFullYear(localYr - (localYr % 10))
    );
    const endYr: Date = new Date(
      localDate.setFullYear(localYr - (localYr % 10) + (10 - 1))
    );
    ...
    const endHdrYr: string = this.globalize.formatDate(endYr, {
      format: null,
      type: 'dateTime',
      skeleton: 'y',
    });
    ...
    for (let rowIterator: number = 0; rowIterator < yearCell; ++rowIterator) {
      ...
      if (year < startFullYr || year > endFullYr) {
        ...
        if (
          !isNullOrUndefined(value) &&
          localDate.getFullYear() === value.getFullYear()
        ) {
          addClass([tdEle], SELECTED);
        }
        if (
          year < new Date(this.checkValue(this.min)).getFullYear() ||
        ...
        if (
          localDate.getFullYear() === this.currentDate.getFullYear() &&
    ...
    const collection: Element[] =
      this.currentView() !== 'Decade'
        ? <NodeListOf<HTMLTableDataCellElement> & Element[]>(
            this.tableBodyElement.querySelectorAll(
              'td:not(.' + OTHERMONTH + '):not(.' + WEEKNUMBER + ')'
            )
          )
        : <NodeListOf<HTMLTableDataCellElement> & Element[]>(
            this.tableBodyElement.querySelectorAll(
  ...
  protected lastDay(date: Date, view: number): Date {
    ...
    if (view !== 2) {
      const timeOffset: number = Math.abs(
        lastDate.getTimezoneOffset() - this.firstDay(date).getTimezoneOffset()
      );
      if (timeOffset) {
  ...
  protected checkDateValue(value: Date): Date {
    return !isNullOrUndefined(value) && value instanceof Date && !isNaN(+value)
      ? value
      : null;
  }
  ...
  protected removeTableHeadElement(): void {
    if (this.getModuleName() === 'calendar') {
      if (
        !isNullOrUndefined(
          this.element.querySelectorAll('.e-content table thead')[0]
        )
      ) {
      ...
      if (
        !isNullOrUndefined(
          this.calendarElement.querySelectorAll('.e-content table thead')[0]
        )
      ) {
        detach(this.tableHeadElement);
      }
  ...
  protected renderTemplate(
    ...
    removeClass(
      [this.contentElement, this.headerElement],
      [MONTH, DECADE, YEAR]
    );
    ...
    for (
      let dayCell: number = 0;
      dayCell < elements.length / count;
      ++dayCell
    ) {
      trEle = this.createElement('tr');
      for (rowIterator = 0 + rowIterator; rowIterator < row; rowIterator++) {
        if (
          !elements[rowIterator as number].classList.contains(
            'e-week-number'
          ) &&
          !isNullOrUndefined(elements[rowIterator as number].children[0])
        ) {
          addClass([elements[rowIterator as number].children[0]], [LINK]);
          rippleEffect(
            <HTMLElement>elements[rowIterator as number].children[0],
            {
              duration: 600,
              isCenterRipple: true,
            }
        ...
        if (
          this.weekNumber &&
          rowIterator === otherMonthCell + 1 &&
          elements[otherMonthCell + 1].classList.contains(OTHERMONTH)
        ) {
          addClass([trEle], OTHERMONTHROW);
        }
        if (
          !this.weekNumber &&
          rowIterator === otherMonthCell &&
          elements[otherMonthCell as number].classList.contains(OTHERMONTH)
        ) {
          addClass([trEle], OTHERMONTHROW);
        }
        if (this.weekNumber) {
          if (
            rowIterator === weekNumCell &&
            elements[weekNumCell as number].classList.contains(OTHERMONTH)
          ) {
            addClass([trEle], OTHERMONTHROW);
          }
        } else {
          if (
            rowIterator === numberCell &&
            elements[numberCell as number].classList.contains(OTHERMONTH)
          ) {
            addClass([trEle], OTHERMONTHROW);
          }
        }
    ...
    if (
      ...
      (view === 0 && view !== this.getViewNumber(this.currentView()))
    ) {
  ...
  protected clickHandler(e: MouseEvent, value: Date): void {
    ...
    if (eve.classList.contains(OTHERMONTH)) {
      ...
      view === this.getViewNumber(this.depth) &&
      this.getViewNumber(this.start) >= this.getViewNumber(this.depth)
    ) {
  ...
  protected contentClick(
    ...
    switch (view) {
      case 0:
        if (
          currentView === this.getViewNumber(this.depth) &&
          this.getViewNumber(this.start) >= this.getViewNumber(this.depth)
        ) {
          detach(this.tableBodyElement);
          this.currentDate = d;
      ...
      case 1:
        if (
          ...
          if (this.calendarMode === 'Gregorian') {
            ...
            const islamicDate: IslamicDateArgs =
              this.islamicModule.getIslamicDate(d);
            this.currentDate = this.islamicModule.toGregorian(
              islamicDate.year,
              islamicDate.month,
              1
            );
  ...
  public onPropertyChanged(
    newProp: CalendarBaseModel,
    oldProp: CalendarBaseModel,
    multiSelection?: boolean,
    values?: Date[]
  ): void {
    this.effect = '';
      ...
      switch (prop) {
        ...
        case 'max':
          ...
          if (prop === 'min') {
            this.setProperties(
              {
                min: this.checkDateValue(
                  new Date(this.checkValue(newProp.min))
                ),
              },
              true
            );
            ...
            this.setProperties(
              {
                max: this.checkDateValue(
                  new Date(this.checkValue(newProp.max))
                ),
              },
              true
            );
          ...
          if (
            (this.todayDate < this.min || this.max < this.todayDate) &&
            this.footer &&
            this.todayElement
          ) {
            ...
            if (
              this.footer &&
              this.todayElement &&
              this.todayElement.classList.contains('e-disabled')
            ) {
        ...
        case 'showTodayButton':
          if (newProp.showTodayButton) {
            ...
            if (
              !isNullOrUndefined(this.todayElement) &&
              !isNullOrUndefined(this.footer)
            ) {
              detach(this.todayElement);
              detach(this.footer);
  ...
  protected validateValues(multiSelection?: boolean, values?: Date[]): void {
    if (multiSelection && !isNullOrUndefined(values) && values.length > 0) {
      ...
      for (
        ...
        const minFormatOption: object = {
          type: 'date',
          skeleton: 'yMd',
          calendar: type,
        };
        const minStringValue: string = this.globalize.formatDate(
          this.min,
          minFormatOption
        );
        const minString: string = minStringValue;
        const maxFormatOption: object = {
          type: 'date',
          skeleton: 'yMd',
          calendar: type,
        };
        const maxStringValue: string = this.globalize.formatDate(
          this.max,
          maxFormatOption
        );
        const maxString: string = maxStringValue;
        if (
          +new Date(tempValueString) < +new Date(minString) ||
  ...
  protected titleUpdate(date: Date, view: string): void {
    ...
    switch (view) {
      case 'days':
        this.headerTitleElement.textContent =
          this.toCapitalize(dayFormatOptions);
        break;
      case 'months':
  ...
  protected setActiveDescendant(): string {
    let id: string;
    const focusedEle: Element = this.tableBodyElement.querySelector(
      'tr td.e-focused-date'
    );
    const selectedEle: Element =
      this.tableBodyElement.querySelector('tr td.e-selected');
    const type: string =
      this.calendarMode === 'Gregorian' ? 'gregorian' : 'islamic';
    let title: string;
  ...
  protected iconHandler(): void {
    ...
    switch (this.currentView()) {
      case 'Month':
        this.previousIconHandler(
          this.compareMonth(
            new Date(this.checkValue(this.currentDate)),
            this.min
          ) < 1
        );
        this.nextIconHandler(
          this.compareMonth(
            new Date(this.checkValue(this.currentDate)),
            this.max
          ) > -1
  ...
  protected navigatePrevious(
    e: MouseEvent | KeyboardEvent | SwipeEventArgs
  ): void {
  ...
  protected navigateNext(
    eve: MouseEvent | KeyboardEvent | SwipeEventArgs
  ): void {
  ...
  public navigateTo(
    ...
    if (this.getViewNumber(this.depth) >= this.getViewNumber(view)) {
      if (
        this.getViewNumber(this.depth) <= this.getViewNumber(this.start) ||
  ...
  public currentView(): string {
    if (
      !isNullOrUndefined(this.contentElement) &&
      this.contentElement.classList.contains(YEAR)
    ) {
      return 'Year';
    } else if (
      !isNullOrUndefined(this.contentElement) &&
      this.contentElement.classList.contains(DECADE)
    ) {
      return 'Decade';
    } else {
      return 'Month';
  ...
  protected getCultureObjects(ld: Object, c: string): Object {
    const gregorianFormat: string = !isNullOrUndefined(this.dayHeaderFormat)
      ? '.dates.calendars.gregorian.days.format.' +
        this.dayHeaderFormat.toLowerCase()
      : null;
    const islamicFormat: string = !isNullOrUndefined(this.dayHeaderFormat)
      ? '.dates.calendars.islamic.days.format.' +
        this.dayHeaderFormat.toLowerCase()
      : null;
  ...
  protected getWeek(d: Date): number {
    ...
    return Math.ceil(
      (currentDate - date + dayMilliSeconds) / dayMilliSeconds / 7
    );
  }
  ...
  protected addMonths(date: Date, i: number): void {
    if (this.calendarMode === 'Gregorian') {
      ...
      const islamicDate: IslamicDateArgs =
        this.islamicModule.getIslamicDate(date);
      this.currentDate = this.islamicModule.toGregorian(
        islamicDate.year,
        islamicDate.month + i,
        1
  ...
  protected getIdValue(
    ...
    // eslint-disable-next-line radix
    const dateString: string = this.globalize.formatDate(
      new Date(parseInt('' + eve.getAttribute('id'), 0)),
      dateFormatOptions
    );
  ...
  protected selectDate(
    ...
    if (
      !multiSelection &&
      !isNullOrUndefined(tableBodyElement.querySelector('.' + SELECTED))
    ) {
      removeClass([tableBodyElement.querySelector('.' + SELECTED)], SELECTED);
    }
    if (
      !multiSelection &&
      !isNullOrUndefined(tableBodyElement.querySelector('.' + FOCUSEDDATE))
    ) {
      removeClass(
        [tableBodyElement.querySelector('.' + FOCUSEDDATE)],
        FOCUSEDDATE
      );
    }
    if (
      !multiSelection &&
      !isNullOrUndefined(tableBodyElement.querySelector('.' + FOCUSEDCELL))
    ) {
      removeClass(
        [tableBodyElement.querySelector('.' + FOCUSEDCELL)],
        FOCUSEDCELL
      );
    }
    if (multiSelection) {
      const copyValues: Date[] = this.copyValues(values);
      const collection: Element[] = Array.prototype.slice.call(
        this.tableBodyElement.querySelectorAll('td')
      );
      for (let index: number = 0; index < collection.length; index++) {
        const tempElement: Element = tableBodyElement.querySelectorAll(
          'td' + '.' + FOCUSEDDATE
        )[0];
        const selectedElement: Element = tableBodyElement.querySelectorAll(
          'td' + '.' + SELECTED
        )[0];
        ...
        if (
          collection[index as number] === selectedElement &&
          !this.checkPresentDate(
            new Date(
              parseInt(selectedElement.getAttribute('id').split('_')[0], 10)
            ),
            values
          )
        ) {
  ...
  protected checkPresentDate(dates: Date, values: Date[]): boolean {
    ...
    if (!isNullOrUndefined(values)) {
      for (
        let checkPrevious: number = 0;
        checkPrevious < values.length;
        checkPrevious++
      ) {
        const type: string =
          this.calendarMode === 'Gregorian' ? 'gregorian' : 'islamic';
        ...
        const tempDateString: string = this.globalize.formatDate(
          values[checkPrevious as number],
          {
            format: null,
            type: 'date',
            skeleton: 'short',
  ...
  protected navigatedEvent(
    eve: MouseEvent | KeyboardEvent | SwipeEventArgs
  ): void {
    extend(this.navigatedArgs, { name: 'navigated', event: eve });
    this.trigger('navigated', this.navigatedArgs);
  }
  protected triggerNavigate(
    event: MouseEvent | KeyboardEvent | SwipeEventArgs
  ): void {
  ...
  protected isMonthYearRange(date: Date): boolean {
    if (this.calendarMode === 'Gregorian') {
      return (
        date.getMonth() >= this.min.getMonth() &&
        date.getFullYear() >= this.min.getFullYear() &&
        date.getMonth() <= this.max.getMonth() &&
        date.getFullYear() <= this.max.getFullYear()
      );
    } else {
      const islamicDate: IslamicObject =
        this.islamicModule.getIslamicDate(date);
      return (
        islamicDate.month >=
          (<IslamicObject>(
            this.islamicModule.getIslamicDate(new Date(1944, 1, 18))
          )).month &&
        islamicDate.year >=
          (<IslamicObject>(
            this.islamicModule.getIslamicDate(new Date(1944, 1, 18))
        ...
        islamicDate.month <=
          (<IslamicObject>(
            this.islamicModule.getIslamicDate(new Date(2069, 1, 16))
          )).month &&
        islamicDate.year <=
          (<IslamicObject>(
            this.islamicModule.getIslamicDate(new Date(2069, 1, 16))
  ...
  protected addDay(
    ...
    if (!isNullOrUndefined(this.tableBodyElement) && !isNullOrUndefined(e)) {
      ...
      const rangeValue: Date = new Date(
        value.setDate(value.getDate() + column)
      );
      column =
  ...
  protected findNextTD(
    ...
    if (
      (!isNullOrUndefined(value) && value.getMonth()) ===
      (!isNullOrUndefined(this.currentDate) && this.currentDate.getMonth())
    ) {
      let tdEles: Element[];
      if (this.calendarMode === 'Gregorian') {
      ...
      collection = <NodeListOf<HTMLTableDataCellElement> & Element[]>(
        this.tableBodyElement.querySelectorAll('td.' + DISABLED)
      );
    }
    if (+value <= +max && +value >= +min) {
      if (collection.length) {
        for (let i: number = 0; i < collection.length; i++) {
          // eslint-disable-next-line radix
          isDisabled =
            +value === +new Date(parseInt(collection[i as number].id, 0))
              ? true
              : false;
          if (isDisabled) {
            break;
...
export class Calendar extends CalendarBase {
  ...
  protected render(): void {
    if (this.calendarMode === 'Islamic' && this.islamicModule === undefined) {
      throwError(
        'Requires the injectable Islamic modules to render Calendar in Islamic mode'
      );
    }
    if (
      this.isMultiSelection &&
      typeof this.values === 'object' &&
      !isNullOrUndefined(this.values) &&
      ...
      for (let index: number = 0; index < this.values.length; index++) {
        if (!this.checkDateValue(this.values[index as number])) {
          if (
            typeof this.values[index as number] === 'string' &&
            this.checkDateValue(
              new Date(this.checkValue(this.values[index as number]))
            )
          ) {
            const copyDate: Date = new Date(
              this.checkValue(this.values[index as number])
            );
  ...
  protected isDayLightSaving(): boolean {
    const secondOffset: number = new Date(
      this.value.getFullYear(),
      6,
      1
    ).getTimezoneOffset();
    const firstOffset: number = new Date(
      this.value.getFullYear(),
      0,
      1
    ).getTimezoneOffset();
  ...
  protected setTimeZone(offsetValue: number): void {
    if (!isNullOrUndefined(this.serverTimezoneOffset) && this.value) {
      ...
      this.value = new Date(
        this.value.getTime() + timeZoneDiff * 60 * 60 * 1000
      );
    }
  }
  ...
  protected renderDays(
    ...
    const tempDays: HTMLElement[] = super.renderDays(
      currentDate,
      this.value,
      this.isMultiSelection,
      this.values,
  ...
  protected renderTemplate(
    ...
    e &&
    e.type === 'click' &&
    (e.currentTarget as HTMLElement).classList.contains(OTHERMONTH)
  ...
  protected clickHandler(e: MouseEvent): void {
    ...
    if (eve.classList.contains(OTHERMONTH)) {
      if (this.isMultiSelection) {
        const copyValues: Date[] = this.copyValues(this.values);
        if (
          copyValues.toString().indexOf(this.getIdValue(e, null).toString()) ===
          -1
        ) {
    ...
    if (
      ...
      !isNullOrUndefined(
        this.tableBodyElement.querySelectorAll('.' + FOCUSEDDATE)[0]
      ) &&
      storeView === 'Year'
    ) {
      this.tableBodyElement
        .querySelectorAll('.' + FOCUSEDDATE)[0]
        .classList.remove(FOCUSEDDATE);
    }
  }
   ...
   */
  public onPropertyChanged(
    newProp: CalendarModel,
    oldProp: CalendarModel
  ): void {
    this.effect = '';
    this.rangeValidation(this.min, this.max);
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        case 'values':
          if (this.isDateSelected) {
            if (
              typeof newProp.values === 'string' ||
              typeof newProp.values === 'number'
            ) {
              ...
              for (let index: number = 0; index < copyValues.length; index++) {
                ...
                if (
                  this.checkDateValue(tempDate) &&
                  !super.checkPresentDate(tempDate, copyValues)
                ) {
    ...
    if (typeof dates !== 'string' && typeof dates !== 'number') {
      ...
      if (typeof dates === 'object' && (<Date[]>dates).length > 0) {
        ...
        for (let i: number = 0; i < tempDates.length; i++) {
          if (
            this.checkDateValue(tempDates[i as number]) &&
            !super.checkPresentDate(tempDates[i as number], copyValues)
          ) {
        ...
        if (
          this.checkDateValue(<Date>dates) &&
          !super.checkPresentDate(<Date>dates, copyValues)
        ) {
          if (!isNullOrUndefined(copyValues) && copyValues.length > 0) {
  ...
  protected changeEvent(e: Event): void {
    if (
      (this.value && this.value.valueOf()) !==
        (this.previousDate && +this.previousDate.valueOf()) ||
      this.isMultiSelection
    ) {
  ...
  protected triggerChange(e: MouseEvent | KeyboardEvent): void {
    ...
    if (
      ...
      ((!isNullOrUndefined(this.value) &&
        !isNullOrUndefined(this.previousDate)) ||
        (this.previousDate === null && !isNaN(+this.value)))
    ) {
      this.changeEvent(e);
    } else if (
      !isNullOrUndefined(this.values) &&
