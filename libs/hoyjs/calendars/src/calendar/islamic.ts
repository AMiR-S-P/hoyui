...
import { rippleEffect } from '@hoyui/base';
import { removeClass, addClass, attributes, HijriParser } from '@hoyui/base';
import { getUniqueID } from '@hoyui/base';
...
export class Islamic {
  ...
  public islamicTitleUpdate(date: Date, view: string): void {
    const globalize: Internationalization = new Internationalization(
      this.calendarInstance.locale
    );
    switch (view) {
      case 'days':
        this.calendarInstance.headerTitleElement.textContent =
  ...
  public islamicRenderDays(
    ...
    const gregorianObject: Date = this.toGregorian(
      islamicDate.year,
      islamicDate.month,
      1
    );
    ...
    for (let day: number = 0; day < cellsCount; ++day) {
      const weekEle: HTMLElement = this.calendarInstance.createElement('td', {
        className: CELL,
      });
      const weekAnchor: HTMLElement =
        this.calendarInstance.createElement('span');
      ...
      const date: Date = this.calendarInstance.globalize.parseDate(
        this.calendarInstance.globalize.formatDate(
          localDate,
          dateFormatOptions
        ),
        dateFormatOptions
      ...
      const title: string = this.calendarInstance.globalize.formatDate(
        localDate,
        { type: 'date', skeleton: 'full', calendar: 'islamic' }
      );
      ...
      dayLink.textContent = this.calendarInstance.globalize.formatDate(
        localDate,
        { type: 'date', skeleton: 'd', calendar: 'islamic' }
      );
      const disabled: boolean =
        this.calendarInstance.min > localDate ||
        this.calendarInstance.max < localDate;
      if (disabled) {
      ...
      const argument: RenderDayCellEventArgs =
        this.calendarInstance.renderDayCellArgs;
      this.calendarInstance.renderDayCellEvent(argument);
      if (argument.isDisabled) {
        if (this.calendarInstance.isMultiSelection) {
          if (
            !isNullOrUndefined(this.calendarInstance.values) &&
            this.calendarInstance.values.length > 0
          ) {
            for (let index: number = 0; index < values.length; index++) {
              const localDateString: number = +new Date(
                this.calendarInstance.globalize.formatDate(argument.date, {
                  type: 'date',
                  skeleton: 'yMd',
                  calendar: 'islamic',
                })
              ...
              const tempDateString: number = +new Date(
                this.calendarInstance.globalize.formatDate(
                  this.calendarInstance.values[index as number],
                  { type: 'date', skeleton: 'yMd', calendar: 'islamic' }
                )
              );
      ...
      if (
        this.calendarInstance.renderDayCellArgs.isDisabled &&
        !tdEle.classList.contains(SELECTED)
      ) {
        addClass([tdEle], DISABLED);
        addClass([tdEle], OVERLAY);
        if (
          +this.calendarInstance.renderDayCellArgs.date ===
          +this.calendarInstance.todayDate
        ) {
      ...
      if (!disabledCls) {
        EventHandler.add(
          tdEle,
          'click',
          this.calendarInstance.clickHandler,
          this.calendarInstance
        );
      }
      if (
        this.calendarInstance.isMultiSelection &&
        !isNullOrUndefined(this.calendarInstance.values) &&
        !otherMnthBool &&
        !disabledCls
      ) {
        for (
          ...
          tempValue < this.calendarInstance.values.length;
          tempValue++
        ) {
          const localDateString: string =
            this.calendarInstance.globalize.formatDate(localDate, {
              type: 'date',
              skeleton: 'short',
              calendar: 'islamic',
            });
          const tempDateString: string =
            this.calendarInstance.globalize.formatDate(
              this.calendarInstance.values[tempValue as number],
              { type: 'date', skeleton: 'short', calendar: 'islamic' }
            );
          if (
            localDateString === tempDateString &&
            this.calendarInstance.getDateVal(
              localDate,
              this.calendarInstance.values[tempValue as number]
            )
          ) {
            ...
            this.calendarInstance.updateFocus(
              otherMnthBool,
              disabledCls,
              localDate,
              tdEle,
        ...
        !otherMnthBool &&
        !disabledCls &&
        this.calendarInstance.getDateVal(localDate, value)
      ) {
        addClass([tdEle], SELECTED);
      } else {
        this.calendarInstance.updateFocus(
      ...
      this.calendarInstance.addDay(
        localDate,
        1,
        null,
        this.calendarInstance.max,
        this.calendarInstance.min
      );
    }
    return tdEles;
  }
  ...
  public islamicIconHandler(): void {
    ...
    const date: Date = new Date(
      this.islamicInValue(this.calendarInstance.currentDate)
    );
    switch (this.calendarInstance.currentView()) {
      ...
      case 'Decade': {
        ...
        const gregorianValue: Date = HijriParser.toGregorian(
          startIslamicYear,
          1,
          1
        );
        ...
        prevDecadeCompare =
          HijriParser.toGregorian(
            ...
            1,
            1
          ).getFullYear() === gregorianValue.getFullYear()
            ? true
            : prevDecadeCompare;
  ...
  public islamicNext(): void {
    ...
    const view: number = this.calendarInstance.getViewNumber(
      this.calendarInstance.currentView()
    );
    const islamicDate: any = this.getIslamicDate(
      this.calendarInstance.currentDate
    );
    switch (this.calendarInstance.currentView()) {
      case 'Year':
        this.calendarInstance.currentDate = this.toGregorian(
          islamicDate.year + 1,
          islamicDate.month,
          1
        );
      ...
      case 'Decade':
        ...
        if (
          islamicDate.year -
            this.calendarInstance.headerElement.textContent
              .split('-')[0]
              .trim() ===
          1
        ) {
  ...
  public islamicPrevious(): void {
    const currentView: number = this.calendarInstance.getViewNumber(
      this.calendarInstance.currentView()
    );
    this.calendarInstance.effect = '';
    const islamicDate: any = this.getIslamicDate(
  ...
  public islamicRenderYears(e?: Event, value?: Date): void {
    ...
    const islamicDate: any = this.getIslamicDate(localDate);
    const gregorianObject: Date = HijriParser.toGregorian(
      islamicDate.year,
      1,
      1
    );
    ...
    const minYr: number = (<any>this.getIslamicDate(this.calendarInstance.min))
      .year;
    const minMonth: number = (<any>(
      this.getIslamicDate(this.calendarInstance.min)
    )).month;
    const maxYr: number = (<any>this.getIslamicDate(this.calendarInstance.max))
      .year;
    const maxMonth: number = (<any>(
      this.getIslamicDate(this.calendarInstance.max)
    )).month;
    ...
    for (let month: number = 1; month <= 12; ++month) {
      ...
      const localMonth: boolean =
        value &&
        (<any>this.getIslamicDate(value)).month ===
          (<any>this.getIslamicDate(localDate)).month;
      const select: boolean =
        value && (<any>this.getIslamicDate(value)).year === yr && localMonth;
      dayLink.textContent = this.calendarInstance.globalize.formatDate(
        localDate,
        { type: 'dateTime', format: 'MMM', calendar: 'islamic' }
      );
      if (
        (this.calendarInstance.min &&
          (curYrs < minYr || (month < minMonth && curYrs === minYr))) ||
        (this.calendarInstance.max &&
          (curYrs > maxYr || (month > maxMonth && curYrs >= maxYr)))
      ) {
        addClass([tdEle], DISABLED);
        ...
        if (
          (<any>this.getIslamicDate(localDate)).month === mon &&
          (<any>this.getIslamicDate(this.calendarInstance.currentDate))
            .month === mon
        ) {
  ...
  public islamicRenderDecade(e?: Event, value?: Date): void {
    ...
    const startYr: Date = new Date(
      this.islamicInValue(localYr - (localYr % 10))
    );
    const endYr: Date = new Date(
      this.islamicInValue(localYr - (localYr % 10) + (10 - 1))
    );
    let startFullYr: number = startYr.getFullYear();
    let endFullYr: number = endYr.getFullYear();
    let startHdrYr: any = this.calendarInstance.globalize.formatDate(startYr, {
      type: 'dateTime',
      format: 'y',
      calendar: 'islamic',
    });
    let endHdrYr: any = this.calendarInstance.globalize.formatDate(endYr, {
      type: 'dateTime',
      format: 'y',
      calendar: 'islamic',
    });
    if (this.calendarInstance.locale === 'ar') {
      startHdrYr = Number(
        startHdrYr.replace(/[٠١٢٣٤٥٦٧٨٩]/g, (d: any) =>
          String.fromCharCode(d.charCodeAt(0) - 1632 + 48)
        )
      );
      endHdrYr = Number(
        endHdrYr.replace(/[٠١٢٣٤٥٦٧٨٩]/g, (d: any) =>
          String.fromCharCode(d.charCodeAt(0) - 1632 + 48)
        )
      );
    }
    const splityear: any =
      this.calendarInstance.headerElement.textContent.split('-');
    if (
      (!isNullOrUndefined(e) &&
        splityear[0] !== startHdrYr &&
        (e as KeyboardEventArgs).action === 'home') ||
      (!isNullOrUndefined(e) &&
        e.type === 'keydown' &&
        (e as KeyboardEventArgs).action === 'end')
    ) {
      startHdrYr = this.calendarInstance.headerElement.textContent
        .split('-')[0]
        .trim();
      endHdrYr = this.calendarInstance.headerElement.textContent
        .split('-')[1]
        .trim();
    }
    if (this.calendarInstance.islamicPreviousHeader) {
      startHdrYr = this.calendarInstance.islamicPreviousHeader
        .split('-')[0]
        .trim();
      endHdrYr = this.calendarInstance.islamicPreviousHeader
        .split('-')[1]
        .trim();
    ...
    if (this.calendarInstance.previousIconClicked) {
      ...
      for (i = 0; i <= splityear.length; i++) {
        endHdrYr =
          endHdrYr - splityear[i as number] === 2 ||
          splityear[i as number] - endHdrYr === 2
            ? (parseInt(endHdrYr, 10) + 1).toString()
            : endHdrYr - splityear[i as number] === 3 ||
              splityear[i as number] - endHdrYr === 3
            ? (parseInt(endHdrYr, 10) + 2).toString()
            : endHdrYr - splityear[i as number] === 4 ||
              splityear[i as number] - endHdrYr === 4
            ? (parseInt(endHdrYr, 10) + 3).toString()
            : endHdrYr - splityear[i as number] === 5 ||
              splityear[i as number] - endHdrYr === 5
            ? (parseInt(endHdrYr, 10) + 4).toString()
            : endHdrYr;
        if (
          endHdrYr - splityear[i as number] === 0 ||
          splityear[i as number] - endHdrYr === 0
        ) {
          endHdrYr = (parseInt(endHdrYr, 10) - 1).toString();
        }
      }
      if (
        endHdrYr - splityear[i as number] === 8 ||
        splityear[i as number] - endHdrYr === 8
      ) {
        endHdrYr = (parseInt(endHdrYr, 10) - 9).toString();
        startHdrYr = (parseInt(endHdrYr, 10) - 9).toString();
      }
      if (
        endHdrYr - splityear[i as number] === 7 ||
        splityear[i as number] - endHdrYr === 7
      ) {
        endHdrYr = (parseInt(endHdrYr, 10) - 8).toString();
        startHdrYr = (parseInt(endHdrYr, 10) - 9).toString();
      }
      startHdrYr =
        endHdrYr - startHdrYr === 10
          ? (parseInt(startHdrYr, 10) + 1).toString()
          : endHdrYr - startHdrYr === 11
          ? (parseInt(startHdrYr, 10) + 2).toString()
          : endHdrYr - startHdrYr === 12
          ? (parseInt(startHdrYr, 10) + 3).toString()
          : startHdrYr;
    ...
    if (this.calendarInstance.nextIconClicked) {
      for (let i: number = 0; i <= splityear.length; i++) {
        if (
          startHdrYr - splityear[i as number] === 0 ||
          splityear[i as number] - startHdrYr === 0
        ) {
          startHdrYr = (parseInt(startHdrYr, 10) + 1).toString();
        }
        if (
          startHdrYr - splityear[i as number] === 2 &&
          startHdrYr > splityear[i as number].trim()
        ) {
          startHdrYr = (parseInt(startHdrYr, 10) - 1).toString();
        }
        if (
          splityear[i as number] - startHdrYr === 1 &&
          startHdrYr < splityear[i as number].trim()
        ) {
      ...
      endHdrYr =
        endHdrYr - startHdrYr === 10
          ...
          : endHdrYr;
      endHdrYr =
        endHdrYr - startHdrYr === 7
          ? (parseInt(endHdrYr, 10) + 2).toString()
          : endHdrYr - startHdrYr === 8
          ? (parseInt(endHdrYr, 10) + 1).toString()
          : endHdrYr;
    }
    if (this.calendarInstance.locale === 'ar') {
      const startHeaderYear: any = this.calendarInstance.globalize.formatDate(
        startYr,
        { type: 'dateTime', format: 'y', calendar: 'islamic' }
      );
      const endHeaderYear: any = this.calendarInstance.globalize.formatDate(
        endYr,
        { type: 'dateTime', format: 'y', calendar: 'islamic' }
      );
      this.calendarInstance.headerTitleElement.textContent =
        startHeaderYear + ' - ' + endHeaderYear;
    } else {
      this.calendarInstance.headerTitleElement.textContent =
        startHdrYr + ' - ' + endHdrYr;
    }
    this.calendarInstance.nextIconClicked =
      this.calendarInstance.previousIconClicked = false;
    ...
    for (let rowCount: number = 1; rowCount <= yearCell; ++rowCount) {
      ...
      localDate = gregorianObject;
      if (
        islamicDate.year === parseInt(startHdrYr, 10) - 1 ||
        (islamicDate.year >= startHdrYr && islamicDate.year <= endFullYr) ||
        islamicDate.year === parseInt(endHdrYr, 10) + 1
      ) {
        const tdEle: HTMLElement = this.islamicDayCell(localDate);
        attributes(tdEle, { role: 'gridcell' });
        const dayLink: HTMLElement =
          this.calendarInstance.createElement('span');
        dayLink.textContent = this.calendarInstance.globalize.formatDate(
          localDate,
          { type: 'dateTime', format: 'y', calendar: 'islamic' }
        ...
        if (
          ...
          year <
            new Date(
              this.islamicInValue(this.calendarInstance.min)
            ).getFullYear() ||
          year >
            new Date(
              this.islamicInValue(this.calendarInstance.max)
            ).getFullYear()
        ) {
          addClass([tdEle], DISABLED);
        } else if (
          ...
          if (
            localDate.getFullYear() ===
              this.calendarInstance.currentDate.getFullYear() &&
        ...
        if (
          (!isNullOrUndefined(e) &&
            (e as KeyboardEventArgs).action === 'home' &&
            islamicDate.year.toString() === startHdrYr) ||
          (!isNullOrUndefined(e) &&
            (e as KeyboardEventArgs).action === 'end' &&
            islamicDate.year.toString() === endHdrYr)
        ) {
          addClass([tdEle], FOCUSEDDATE);
        }
        tdEles.push(tdEle);
  ...
  public islamicDayCell(localDate: Date): HTMLElement {
    ...
    const formatDate: string = this.calendarInstance.globalize.formatDate(
      localDate,
      dateFormatOptions
    );
    const date: Date = this.calendarInstance.globalize.parseDate(
  ...
  public islamicRenderTemplate(
    ...
    this.calendarInstance.tableBodyElement =
      this.calendarInstance.createElement('tbody');
    this.calendarInstance.table.appendChild(
      this.calendarInstance.tableBodyElement
    );
    removeClass(
      [
        this.calendarInstance.contentElement,
        this.calendarInstance.headerElement,
      ],
      [MONTH, DECADE, YEAR]
    );
    addClass(
    ...
    for (
      ...
      dayCell < Math.round(elements.length / count);
      ++dayCell
    ) {
      ...
      for (rowCount = 0 + rowCount; rowCount < row; rowCount++) {
        if (
          !elements[rowCount as number].classList.contains('e-week-number') &&
          !isNullOrUndefined(elements[rowCount as number].children[0])
        ) {
        ...
        if (
          this.calendarInstance.weekNumber &&
          rowCount === otherMonthCell + 1 &&
    ...
    this.calendarInstance.table.querySelector('tbody').className =
      this.calendarInstance.effect;
    this.islamicIconHandler();

    if (
      view !==
        this.calendarInstance.getViewNumber(
          this.calendarInstance.currentView()
        ) ||
      (view === 0 &&
        view !==
  ...
  public islamicCompareMonth(start: Date, end: Date): number {
    ...
    if (hijriStart.year > hijriEnd.year) {
      ...
      result =
        hijriStart.month === hijriEnd.month
          ? 0
          : hijriStart.month > hijriEnd.month
          ? 1
          : -1;
  ...
  public islamicCompare(
    ...
    if (hijriStart.year > end) {
      ...
      this.calendarInstance.currentView() === 'Decade' &&
      hijriStart.year < start &&
      !(
        startDate.getFullYear() >= minDecade &&
        startDate.getFullYear() <= maxDecade
      )
    ) {
      result = -1;
    } else if (
      hijriStart.year < start &&
      this.calendarInstance.currentView() === 'Year'
    ) {
      result = -1;
    }
    return result;
