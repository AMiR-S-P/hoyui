...
import { Input, InputObject, IInput, FloatLabelType } from '@hoyui/inputs';
import { ListBase, ListBaseOptions, createElementParams } from '@hoyui/lists';
...
export namespace TimePickerBase {
  ...
  export function createListItems(
    ...
    while (end >= start) {
      ...
      listItems.push(
        globalize.formatDate(new Date(start), {
          format: timeFormat,
          type: 'time',
        })
      );
...
export class TimePicker extends Component<HTMLElement> implements IInput {
  ...
  private initialize(): void {
    ...
    this.setProperties(
      {
        scrollTo: this.checkDateValue(
          new Date(this.checkInValue(this.scrollTo))
        ),
      },
      true
    );
    ...
    const localeText: { placeholder: string } = {
  ...
  private checkInvalidValue(value: Date): void {
    ...
    if (typeof value !== 'object' && !isNullOrUndefined(value)) {
      ...
      if (typeof value === 'number') {
        ...
        if (!/^[a-zA-Z0-9- ]*$/.test(value)) {
          ...
          if (isNullOrUndefined(valueExpression)) {
            valueExpression = this.checkDateValue(
              this.globalize.parseDate(this.getAmPmValue(valueString), {
      ...
      valueExpression =
        !isNullOrUndefined(valueExp) &&
        valueExp instanceof Date &&
        !isNaN(+valueExp)
          ? valueExp
          : null;
      if (
        isNullOrUndefined(valueExpression) &&
        valueString.replace(/\s/g, '').length
      ) {
        ...
        basicISOString =
          /^\s*((?:[+-]\d{6}|\d{4})(?:\d\d\d\d|W\d\d\d|W\d\d|\d\d\d|\d\d))(?:(T| )(\d\d(?:\d\d(?:\d\d(?:[.,]\d+)?)?)?)([\+\-]\d\d(?::?\d\d)?|\s*Z)?)?/;
        if (
          (!extISOString.test(valueString) &&
            !basicISOString.test(valueString)) ||
          /^[a-zA-Z0-9- ]*$/.test(value) ||
          isNaN(+new Date('' + valueString))
        ) {
          isInvalid = true;
        } else {
  ...
  private cldrFormat(type: string): string {
    ...
    if (this.locale === 'en' || this.locale === 'en-US') {
      cldrDateTimeString = <string>(
        getValue('timeFormats.short', getDefaultDateObject())
      );
    } else {
      cldrDateTimeString = <string>(
        this.getCultureTimeObject(cldrData, '' + this.locale)
      );
    }
  ...
  public destroy(): void {
    ...
    if (this.inputElement) {
      Input.removeAttributes(
        <{ [key: string]: string }>ariaAttribute,
        this.inputElement
      );
      if (this.angularTag === null) {
  ...
  protected ensureInputAttribute(): void {
    ...
    for (let i: number = 0; i < this.inputElement.attributes.length; i++) {
      propertyList[i as number] =
        this.inputElement.attributes[i as number].name;
    }
    for (let i: number = 0; i < propertyList.length; i++) {
      if (
        !isNullOrUndefined(
          this.cloneElement.getAttribute(propertyList[i as number])
        )
      ) {
  ...
  private generateList(): void {
    ...
    this.liCollections = <HTMLElement[] & NodeListOf<Element>>(
      this.listWrapper.querySelectorAll('.' + LISTCLASS)
    );
  }
  private renderPopup(): void {
    ...
    if (Browser.isDevice && this.fullScreenMode) {
      ...
      const modelTitleSpan: HTMLElement = this.createElement('span', {
        className: 'e-model-title',
      });
      modelTitleSpan.textContent = 'Select time';
      const modelCloseIcon: HTMLElement = this.createElement('span', {
        className: 'e-popup-close',
      });
      EventHandler.add(
        modelCloseIcon,
        'mousedown touchstart',
        this.timePopupCloseHandler,
        this
      );
    ...
    if (!isNullOrUndefined(this.htmlAttributes)) {
      for (const key of Object.keys(this.htmlAttributes)) {
        if (wrapperAttributes.indexOf(key) > -1) {
          if (key === 'class') {
            const updatedClassesValue: string = this.htmlAttributes[`${key}`]
              .replace(/\s+/g, ' ')
              .trim();
            if (updatedClassesValue !== '') {
              addClass(
                [this.inputWrapper.container],
                updatedClassesValue.split(' ')
              );
            }
          } else if (key === 'style') {
            let timeStyle: string =
              this.inputWrapper.container.getAttribute(key);
            timeStyle = !isNullOrUndefined(timeStyle)
              ? timeStyle + this.htmlAttributes[`${key}`]
  ...
  private checkErrorState(val: Date | string): void {
    ...
    if (
      (this.validateState(value) && !this.invalidValueString) ||
  ...
  private getActiveElement(): HTMLElement[] {
    if (!isNullOrUndefined(this.popupWrapper)) {
      return <NodeListOf<HTMLElement> & HTMLElement[]>(
        this.popupWrapper.querySelectorAll('.' + SELECTED)
      );
    } else {
      return null;
    }
  ...
  private setPopupWidth(width: string | number): string {
    ...
    if (width.indexOf('%') > -1) {
      const inputWidth: number =
        (this.containerStyle.width * parseFloat(width)) / 100;
  ...
  private findScrollTop(element: HTMLElement): void {
    ...
    if (height + element.offsetTop > listHeight) {
      this.popupWrapper.scrollTop = nextEle
        ? height - (listHeight / HALFPOSITION + liHeight / HALFPOSITION)
        : height;
    } else {
  ...
  private setScrollTo(): void {
    ...
    if (!isNullOrUndefined(this.popupWrapper)) {
      const items: HTMLElement[] = <NodeListOf<HTMLLIElement> & HTMLElement[]>(
        this.popupWrapper.querySelectorAll('.' + LISTCLASS)
      );
      if (items.length) {
        const initialTime: number = this.timeCollections[0];
        const scrollTime: number = this.getDateObject(
          this.checkDateValue(this.scrollTo)
        ).getTime();
        element =
          items[Math.round((scrollTime - initialTime) / (this.step * 60000))];
      }
    } else {
      this.popupWrapper.scrollTop = 0;
    }
  ...
  private getText(): string {
    return isNullOrUndefined(this.checkDateValue(this.value))
      ? ''
      : this.getValue(this.value);
  }
  private getValue(value: Date): string {
    return isNullOrUndefined(this.checkDateValue(value))
      ? null
      : this.globalize.formatDate(value, {
  ...
  private compareFormatChange(value: string): Date {
    ...
    return value !== this.getText()
      ...
      : this.getDateObject(this.value);
  }
  private updatePlaceHolder(): void {
    Input.setPlaceholder(
      this.l10n.getConstant('placeholder'),
      this.inputElement
    );
  }
  ...
  private mouseUpHandler(event: MouseEvent): void {
    if (!this.readonly) {
      ...
      if (this.enableMask) {
        ...
        if (
          !(curPos.start === 0 && curPos.end === this.inputElement.value.length)
        ) {
          if (this.inputElement.value.length > 0) {
            this.cursorDetails = this.focusSelection();
          }
          this.inputElement.setSelectionRange(
            this.cursorDetails.start,
            this.cursorDetails.end
          );
        }
      }
    }
      ...
      switch (event.action) {
        ...
        case 'enter':
          if (this.isNavigate) {
            ...
            this.valueWithMinutes = new Date(
              this.timeCollections[this.activeIndex]
            );
  ...
  private onMouseClick(event: MouseEvent): void {
    ...
    const li: HTMLElement = (this.selectedElement = <HTMLElement>(
      closest(target, '.' + LISTCLASS)
    ));
  ...
  private checkValueChange(
    event: KeyboardEventArgs | FocusEvent | MouseEvent,
    ...
    if (!this.strictMode && !this.validateState(this.valueWithMinutes)) {
      ...
      this.initValue = this.valueWithMinutes = this.compareFormatChange(
        this.inputElement.value
      );
      ...
      if (!isNavigation) {
        if (
          this.prevValue !== this.inputElement.value ||
          isNullOrUndefined(this.checkDateValue(this.value))
        ) {
          this.valueProcess(
            event,
            this.compareFormatChange(this.inputElement.value)
          );
        }
      } else {
        const value: Date = this.getDateObject(
          new Date(this.timeCollections[this.activeIndex])
        );
  ...
  private setHover(li: HTMLElement, className: string): void {
    if (
      this.enabled &&
      this.isValidLI(li) &&
      !li.classList.contains(className)
    ) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  protected validateMinMax(
    dateVal: Date | string,
    minVal: Date,
    maxVal: Date
  ): Date | string {
    const value: Date =
      dateVal instanceof Date ? dateVal : this.getDateObject(dateVal);
    if (!isNullOrUndefined(this.checkDateValue(value))) {
      dateVal = this.strictOperation(
        this.initMin,
        this.initMax,
        dateVal,
        value
      ...
      +this.createDateObj(this.getFormattedValue(this.initMin)) >
  ...
  private valueIsDisable(value: string | Date): boolean {
    if (
      !isNullOrUndefined(this.disableItemCollection) &&
      this.disableItemCollection.length > 0
    ) {
      if (this.disableItemCollection.length === this.timeCollections.length) {
        return false;
      }
      const time: string =
        value instanceof Date ? this.objToString(value) : value;
      for (
        let index: number = 0;
        index < this.disableItemCollection.length;
        index++
      ) {
      ...
      if (this.valueIsDisable(val)) {
        const value: Date =
          typeof val === 'string'
            ...
            : this.setCurrentDate(this.getDateObject(val));
        const maxValue: Date = this.setCurrentDate(
          this.getDateObject(this.initMax)
        );
        const minValue: Date = this.setCurrentDate(
          this.getDateObject(this.initMin)
        );
        if (!isNullOrUndefined(this.checkDateValue(value))) {
  ...
  protected bindEvents(): void {
    ...
    if (!Browser.isDevice) {
      this.keyConfigure = extend(this.keyConfigure, this.keyConfigs) as {
        [key: string]: string;
      };
  ...
  protected unBindEvents(): void {
    ...
    if (
      ...
      !isNullOrUndefined(this.inputWrapper.clearButton)
    ) {
      EventHandler.remove(
        this.inputWrapper.clearButton,
        'mousedown touchstart',
    ...
    for (const prop of attributes) {
      if (!isNullOrUndefined(this.inputElement.getAttribute(prop))) {
        switch (prop) {
          case 'disabled':
            if (
              isNullOrUndefined(this.timeOptions) ||
              this.timeOptions['enabled'] === undefined ||
              isDynamic
            ) {
          ...
          case 'readonly':
            if (
              isNullOrUndefined(this.timeOptions) ||
              this.timeOptions['readonly'] === undefined ||
              isDynamic
            ) {
          ...
          case 'placeholder':
            if (
              isNullOrUndefined(this.timeOptions) ||
              this.timeOptions['placeholder'] === undefined ||
              isDynamic
            ) {
              this.setProperties(
          ...
          case 'min':
            if (
              isNullOrUndefined(this.timeOptions) ||
              this.timeOptions['min'] === undefined ||
              isDynamic
            ) {
          ...
          case 'max':
            if (
              isNullOrUndefined(this.timeOptions) ||
              this.timeOptions['max'] === undefined ||
              isDynamic
            ) {
              value = new Date(this.inputElement.getAttribute(prop));
              if (!isNullOrUndefined(this.checkDateValue(value))) {
          ...
          case 'value':
            if (
              isNullOrUndefined(this.timeOptions) ||
              this.timeOptions['value'] === undefined ||
              isDynamic
            ) {
              value = new Date(this.inputElement.getAttribute(prop));
              if (!isNullOrUndefined(this.checkDateValue(value))) {
  ...
  protected getTextFormat(): number {
    let time: number = 0;
    if (
      this.cldrTimeFormat().split(' ')[0] === 'a' ||
      this.cldrTimeFormat().indexOf('a') === 0
    ) {
      time = 1;
    ...
    if (this.isNullOrEmpty(value)) {
      ...
      if (this.strictMode) {
        ...
        const inputVal: string =
          val === null && (<string>value).trim().length > 0
            ? this.previousState(this.prevDate)
            : this.inputElement.value;
  ...
  protected checkValue(value: string | Date): string {
    if (!this.isNullOrEmpty(value)) {
      const date: Date =
        value instanceof Date ? value : this.getDateObject(value);
  ...
  protected findNextElement(event: KeyboardEventArgs): void {
    ...
    const value: Date = isNullOrUndefined(this.valueWithMinutes)
      ? this.createDateObj(textVal)
    ...
    if (
      !isNullOrUndefined(this.checkDateValue(value)) ||
      !isNullOrUndefined(this.activeIndex)
    ) {
      if (event.action === 'home') {
        const index: number = this.validLiElement(0);
        timeVal = +this.createDateObj(
          new Date(this.timeCollections[index as number])
        );
  ...
  protected selectNextItem(event: KeyboardEventArgs): void {
    const index: number = this.validLiElement(
      0,
      event.action === 'down' ? false : true
    );
    this.activeIndex = index;
  ...
  private validLiElement(index: number, backward?: boolean): number {
    ...
    const items: HTMLElement[] = isNullOrUndefined(this.popupWrapper)
      ? this.liCollections
      : <NodeListOf<HTMLLIElement> & HTMLElement[]>(
          this.popupWrapper.querySelectorAll('.' + LISTCLASS)
        );
  ...
  protected keyHandler(event: KeyboardEventArgs): void {
    ...
    if (
      isNullOrUndefined(this.getActiveElement()) ||
      this.getActiveElement().length === 0
    ) {
      ...
      if (event.keyCode >= 37 && event.keyCode <= 40) {
        let index: number =
          event.keyCode === 40 || event.keyCode === 39
            ...
            : --this.activeIndex;
        this.activeIndex = index =
          this.activeIndex === count ? 0 : this.activeIndex;
        this.activeIndex = index =
          this.activeIndex < 0 ? count - 1 : this.activeIndex;
        this.activeIndex = index = this.validLiElement(
          this.activeIndex,
          event.keyCode === 40 || event.keyCode === 39 ? false : true
        );
        nextItem = isNullOrUndefined(this.timeCollections[index as number])
          ? this.timeCollections[0]
          : this.timeCollections[index as number];
  ...
  protected getCultureTimeObject(ld: Object, c: string): Object {
    return getValue(
      'main.' + c + '.dates.calendars.gregorian.timeFormats.short',
      ld
    );
  }
  protected getCultureDateObject(ld: Object, c: string): Object {
    return getValue(
      'main.' + c + '.dates.calendars.gregorian.dateFormats.short',
      ld
    );
  }
  ...
  protected valueProcess(
    event: KeyboardEventArgs | FocusEvent | MouseEvent,
    value: Date
  ): void {
    const result: Date = isNullOrUndefined(this.checkDateValue(value))
      ? null
      : value;
  ...
  protected changeEvent(e: KeyboardEventArgs | FocusEvent | MouseEvent): void {
    ...
    eventArgs.value =
      this.valueWithMinutes || this.getDateObject(this.inputElement.value);
    this.prevDate =
      this.valueWithMinutes || this.getDateObject(this.inputElement.value);
  ...
  protected updateInput(isUpdate: boolean, date: Date): void {
    ...
    if (
      typeof date !== 'number' ||
      (this.value && +new Date(+this.value).setMilliseconds(0)) !== +date
    ) {
  ...
  protected removeSelection(): void {
    this.removeHover(HOVER);
    if (!isNullOrUndefined(this.popupWrapper)) {
      const items: Element[] = <NodeListOf<Element> & Element[]>(
        this.popupWrapper.querySelectorAll('.' + SELECTED)
      );
      if (items.length) {
  ...
  protected getHoverItem(className: string): Element[] {
    ...
    if (!isNullOrUndefined(this.popupWrapper)) {
      hoveredItem = <NodeListOf<HTMLLIElement> & Element[]>(
    ...
    if (!isNullOrUndefined(this.popupWrapper)) {
      ...
      if (items.length) {
        for (let i: number = 0; i < items.length; i++) {
          if (
            this.timeCollections[i as number] ===
            +this.getDateObject(this.valueWithMinutes)
          ) {
  ...
  protected isValidLI(li: Element | HTMLElement): boolean {
    return (
      li && li.classList.contains(LISTCLASS) && !li.classList.contains(DISABLED)
    );
  }
  protected createDateObj(val: Date | string): Date {
    const formatStr: string = null;
    let today: string = this.globalize.formatDate(new Date(), {
      format: formatStr,
      skeleton: 'short',
      type: 'date',
    });
    ...
    if (typeof val === 'string') {
      if (
        val.toUpperCase().indexOf('AM') > -1 ||
        val.toUpperCase().indexOf('PM') > -1
      ) {
        ...
        value = isNaN(+new Date(today + ' ' + val))
          ? null
          : new Date(new Date(today + ' ' + val).setMilliseconds(0));
        if (isNullOrUndefined(value)) {
  ...
  protected timeParse(today: string, val: Date | string): Date {
    ...
    value = isNullOrUndefined(value)
      ? this.globalize.parseDate(this.getAmPmValue(today + ' ' + val), {
    ...
    value = isNullOrUndefined(value)
      ? value
      : new Date(value.setMilliseconds(0));
    return value;
  }
  protected createListItems(): void {
    ...
    while (end >= start) {
      ...
      listItems.push(
        this.globalize.formatDate(new Date(start), {
          format: this.cldrTimeFormat(),
          type: 'time',
        })
      );
      ...
      itemCreated: (args: { item: Element; text: string }): void => {
        ...
        this.trigger('itemRender', eventArgs, (eventArgs: ItemEventArgs) => {
          ...
          if (eventArgs.element.classList.contains(DISABLED)) {
            this.disableItemCollection.push(
              eventArgs.element.getAttribute('data-value')
            );
          }
        });
      },
    };
    this.listTag = ListBase.createList(
      this.createElement,
      listItems,
      listBaseOptions,
      true
  ...
  private documentClickHandler(event: MouseEvent): void {
    ...
    if (
      ...
      ((this.inputWrapper.container.contains(target) &&
        ...
        (this.popupObj.element && this.popupObj.element.contains(target))) &&
  ...
  protected inputBlurHandler(e: MouseEvent): void {
    ...
    if (
      ...
      if (
        this.inputElement.value === this.maskedDateValue &&
        !this.value &&
  ...
  public show(event?: KeyboardEvent | MouseEvent | Event): void {
    if ((this.enabled && this.readonly) || !this.enabled || this.popupWrapper) {
      ...
      this.trigger('open', eventArgs, (eventArgs: PopupEventArgs) => {
        ...
        if (
          !this.openPopupEventArgs.cancel &&
          !this.inputWrapper.buttons[0].classList.contains(DISABLED)
        ) {
  ...
  private setOverlayIndex(
    ...
    timePopupElement: HTMLElement,
    modal: HTMLElement,
    isDevice: boolean
  ): void {
    if (
      isDevice &&
      !isNullOrUndefined(timePopupElement) &&
      !isNullOrUndefined(modal) &&
      !isNullOrUndefined(popupWrapper)
    ) {
      const index: number = parseInt(timePopupElement.style.zIndex, 10)
        ? parseInt(timePopupElement.style.zIndex, 10)
        : 1000;
  ...
  private popupAlignment(args: PopupEventArgs): void {
    ...
    if (
      !isNaN(parseFloat(args.popup.position.X)) ||
      !isNaN(parseFloat(args.popup.position.Y))
    ) {
    ...
    if (!Browser.isDevice) {
      ...
      if (
        args.popup.position.X === 'center' &&
        args.popup.position.Y === 'center'
      ) {
  ...
  public onPropertyChanged(
    newProp: TimePickerModel,
    oldProp: TimePickerModel
  ): void {
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        case 'width':
          ...
          this.containerStyle =
            this.inputWrapper.container.getBoundingClientRect();
        ...
        case 'value':
          ...
          if (!this.invalidValueString) {
            if (typeof newProp.value === 'string') {
              ...
              if (
                (newProp.value &&
                  +new Date(+newProp.value).setMilliseconds(0)) !== +this.value
              ) {
                newProp.value = this.checkDateValue(
                  new Date('' + newProp.value)
                );
              }
            }
            this.initValue = newProp.value;
            newProp.value = this.compareFormatChange(
              this.checkValue(newProp.value)
            );
          } else {
            this.updateInputValue(this.invalidValueString);
            this.checkErrorState(this.invalidValueString);
        ...
        case 'scrollTo':
          if (
            this.checkDateValue(new Date(this.checkInValue(newProp.scrollTo)))
          ) {
            if (this.popupWrapper) {
              this.setScrollTo();
            }
            this.setProperties(
              {
                scrollTo: this.checkDateValue(
                  new Date(this.checkInValue(newProp.scrollTo))
                ),
              },
              true
            );
