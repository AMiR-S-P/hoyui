...
import { ModuleDeclaration, extend, Touch, SwipeEventArgs } from '@hoyui/base';
...
export class DateTimePicker extends DatePicker {
  ...
  private formatRegex: RegExp =
    /dddd|ddd|dd|d|MMMM|MMM|MM|M|yyyy|yyy|yy|y|'[^']*'|'[^']*'/g;
  ...
  public destroy(): void {
    ...
    if (
      this.popupObject &&
      this.popupObject.element.classList.contains(POPUP)
    ) {
  ...
  public render(): void {
    ...
    if (
      ...
      this.inputWrapper.container
        ...
        .classList.add('e-date-time-icon');
    }
    this.renderComplete();
  }
  ...
  protected isValidTime(value: Date): boolean {
    if (value != null && (this.minTime || this.maxTime)) {
      ...
      if (this.minTime) {
        minTimeValue =
          this.minTime.getHours() * 3600000 +
          this.minTime.getMinutes() * 60000 +
          this.minTime.getSeconds() * 1000 +
          this.minTime.getMilliseconds();
      }
      if (this.maxTime) {
        maxTimeValue =
          this.maxTime.getHours() * 3600000 +
          this.maxTime.getMinutes() * 60000 +
          this.maxTime.getSeconds() * 1000 +
          this.maxTime.getMilliseconds();
      }
      if (
        this.min &&
        +value.getDate() === +this.min.getDate() &&
        +value.getMonth() === +this.min.getMonth() &&
        +value.getFullYear() === +this.min.getFullYear()
      ) {
        minValue =
          this.min.getHours() * 3600000 +
          this.min.getMinutes() * 60000 +
          this.min.getSeconds() * 1000 +
          this.min.getMilliseconds();
        minTimeValue = minTimeValue < minValue ? minValue : minTimeValue;
      }
      if (
        this.max &&
        +value.getDate() === +this.max.getDate() &&
        +value.getMonth() === +this.max.getMonth() &&
        +this.max.getFullYear() === +this.max.getFullYear()
      ) {
        maxValue =
          this.max.getHours() * 3600000 +
          this.max.getMinutes() * 60000 +
          this.max.getSeconds() * 1000 +
          this.max.getMilliseconds();
      ...
      if (this.strictMode) {
        ...
        return !(
          (minTimeValue && valueTime < minTimeValue) ||
          (maxTimeValue && valueTime > maxTimeValue)
        );
      }
    }
    return true;
  ...
  private renderTimeIcon(): void {
    this.timeIcon = Input.appendSpan(
      INPUTWRAPPER + ' ' + TIMEICON + ' ' + ICONS,
      this.inputWrapper.container
    );
  }
  private bindInputEvents(): void {
    ...
    this.keyboardHandler = new KeyboardEvents(<HTMLElement>this.inputElement, {
      eventName: 'keydown',
    ...
    if (this.calendarMode === 'Gregorian') {
      ...
      return getValue(
        'main.' +
          '' +
          this.locale +
          '.dates.calendars.islamic.timeFormats.short',
        ld
      );
    }
  }
  ...
  private listCreation(): void {
    let dateObject: Date;
    if (this.calendarMode === 'Gregorian') {
      this.cldrDateTimeFormat().replace(
        this.formatRegex,
        this.TimePopupFormat()
      );
      if (this.dateFormatString === '') {
        this.dateFormatString = this.cldrDateTimeFormat();
      }
      dateObject = this.globalize.parseDate(
        this.getAmPmValue(this.inputElement.value),
        {
    ...
    const value: Date = isNullOrUndefined(this.value)
  ...
  private popupCreation(
    ...
    if (type === 'date') {
      ...
      if (!this.readonly) {
        ...
        if (Browser.isDevice) {
          this.modelWrapper.style.zIndex = (
            this.popupObject.zIndex - 1
          ).toString();
  ...
  private documentClickHandler(event: MouseEvent): void {
    ...
    if (
      !isNullOrUndefined(this.popupObject) &&
      ((this.inputWrapper.container.contains(target) &&
        event.type !== 'mousedown') ||
        (this.popupObject.element &&
          this.popupObject.element.contains(target))) &&
    ...
    if (
      !closest(
        target,
        '[id="' + (this.popupObject && this.popupObject.element.id + '"]')
      ) &&
      target !== this.inputElement &&
      target !== this.timeIcon &&
      !isNullOrUndefined(this.inputWrapper) &&
      target !== this.inputWrapper.container &&
  ...
  private isTimePopupOpen(): boolean {
    return this.dateTimeWrapper &&
      this.dateTimeWrapper.classList.contains('' + ROOT)
      ? true
      : false;
  }
  private isDatePopupOpen(): boolean {
    return this.popupWrapper &&
      this.popupWrapper.classList.contains('' + DATETIMEPOPUPWRAPPER)
      ? true
      : false;
  }
  private renderPopup(): void {
    ...
    if (Browser.isDevice) {
      ...
      const dlgOverlay: HTMLElement = createElement('div', {
    ...
    if (Browser.isDevice && this.fullScreenMode) {
      ...
      EventHandler.add(
        modelCloseIcon,
        'mousedown touchstart',
        this.dateTimeCloseHandler,
        this
      );
      modelHeader.appendChild(modelCloseIcon);
      modelHeader.appendChild(modelTitleSpan);
      this.dateTimeWrapper.insertBefore(
        modelHeader,
        this.dateTimeWrapper.firstElementChild
      );
    }
  }
  ...
  protected getPopupHeight(): number {
    ...
    const popupHeight: number =
      this.dateTimeWrapper.getBoundingClientRect().height;
    if (Browser.isDevice && this.fullScreenMode) {
      return popupHeight;
    } else {
  ...
  protected changeEvent(e: Event): void {
    ...
    if (
      ...
      (this.previousDateTime && +this.previousDateTime.valueOf())
    ) {
      this.valueWithMinutes = this.value;
  ...
  private findScrollTop(element: HTMLElement): void {
    ...
    const height: number = nextElement
      ? (<HTMLElement>nextElement).offsetTop
      : element.offsetTop;
    const lineHeight: number = element.getBoundingClientRect().height;
    if (height + element.offsetTop > listHeight) {
      if (Browser.isDevice && this.fullScreenMode) {
        const listContent: Element =
          this.dateTimeWrapper.querySelector('.e-content');
        listContent.scrollTop = nextElement
          ? height - (listHeight / HALFPOSITION + lineHeight / HALFPOSITION)
          : height;
      } else {
        this.dateTimeWrapper.scrollTop = nextElement
          ? height - (listHeight / HALFPOSITION + lineHeight / HALFPOSITION)
          : height;
      }
    } else {
  ...
  private setScrollTo(): void {
    ...
    const items: HTMLElement[] = <NodeListOf<HTMLLIElement> & HTMLElement[]>(
      this.dateTimeWrapper.querySelectorAll('.' + LISTCLASS)
    );
    if (items.length >= 0) {
  ...
  private setInputValue(type: string): void {
    if (type === 'date') {
      this.inputElement.value = this.previousElementValue =
        this.getFormattedValue(this.getFullDateTime());
      this.setProperties({ value: this.getFullDateTime() }, true);
    } else {
      const tempVal: string = this.getFormattedValue(
  ...
  private setSelection(li: HTMLElement, event: MouseEvent): void {
    if (this.isValidLI(li) && !li.classList.contains(ACTIVE)) {
      ...
      const index: number = Array.prototype.slice
  ...
  private setTimeActiveClass(): void {
    const collections: HTMLElement = isNullOrUndefined(this.dateTimeWrapper)
      ? this.listWrapper
      : this.dateTimeWrapper;
    if (!isNullOrUndefined(collections)) {
      const items: HTMLElement[] = <NodeListOf<HTMLLIElement> & HTMLElement[]>(
        collections.querySelectorAll('.' + LISTCLASS)
      );
      if (items.length) {
  ...
  protected removeTimeSelection(): void {
    ...
    if (!isNullOrUndefined(this.dateTimeWrapper)) {
      const items: Element[] = <NodeListOf<Element> & Element[]>(
        this.dateTimeWrapper.querySelectorAll('.' + ACTIVE)
      );
      if (items.length) {
  ...
  protected getTimeHoverItem(className: string): Element[] {
    ...
    if (!isNullOrUndefined(collections)) {
      hoveredItem = <NodeListOf<HTMLLIElement> & Element[]>(
        collections.querySelectorAll('.' + className)
      );
    }
    return hoveredItem;
  }
  ...
  private startTime(date: Date): Date {
    ...
    if (
      (+startValue.getDate() === +tempMin.getDate() &&
        +startValue.getMonth() === +tempMin.getMonth() &&
      ...
      if (
        ...
        +new Date(
          ...
          +new Date(
            ...
            tempMin.getMinutes(),
            tempMin.getSeconds(),
            tempMin.getMilliseconds()
          )
      ) {
        tempStartValue.setHours(
          startValue.getHours(),
          startValue.getMinutes(),
          startValue.getSeconds(),
          startValue.getMilliseconds()
  ...
  private endTime(date: Date): Date {
    ...
    if (
      (+endValue.getDate() === +tempMax.getDate() &&
        +endValue.getMonth() === +tempMax.getMonth() &&
        +endValue.getFullYear() === +tempMax.getFullYear()) ||
      +new Date(
        endValue.getUTCFullYear(),
        endValue.getMonth(),
        endValue.getDate()
      ) >=
      ...
      if (
        ...
        +new Date(
          ...
          +new Date(
            ...
            tempMax.getMinutes(),
            tempMax.getSeconds(),
            tempMax.getMilliseconds()
          )
      ) {
        tempEndValue.setHours(
          endValue.getHours(),
          endValue.getMinutes(),
          endValue.getSeconds(),
          endValue.getMilliseconds()
    ...
    for (const prop of attributes) {
      if (!isNullOrUndefined(this.inputElement.getAttribute(prop))) {
        switch (prop) {
          ...
          case 'readonly':
            if (
              isNullOrUndefined(this.dateTimeOptions) ||
              this.dateTimeOptions['readonly'] === undefined ||
              isDynamic
            ) {
          ...
          case 'placeholder':
            if (
              isNullOrUndefined(this.dateTimeOptions) ||
              this.dateTimeOptions['placeholder'] === undefined ||
              isDynamic
            ) {
              this.setProperties(
          ...
          case 'min':
            if (
              isNullOrUndefined(this.dateTimeOptions) ||
              this.dateTimeOptions['min'] === undefined ||
              isDynamic
            ) {
          ...
          case 'disabled':
            if (
              isNullOrUndefined(this.dateTimeOptions) ||
              this.dateTimeOptions['enabled'] === undefined ||
              isDynamic
            ) {
          ...
          case 'value':
            if (
              isNullOrUndefined(this.dateTimeOptions) ||
              this.dateTimeOptions['value'] === undefined ||
              isDynamic
            ) {
              value = new Date(this.inputElement.getAttribute(prop));
              if (!this.isNullOrEmpty(value) && !isNaN(+value)) {
          ...
          case 'max':
            if (
              isNullOrUndefined(this.dateTimeOptions) ||
              this.dateTimeOptions['max'] === undefined ||
              isDynamic
            ) {
              value = new Date(this.inputElement.getAttribute(prop));
              if (!this.isNullOrEmpty(value) && !isNaN(+value)) {
  ...
  private getDateObject(text: string | Date): Date {
    if (!this.isNullOrEmpty(text)) {
      ...
      if (this.checkDateValue(dateValue)) {
        ...
        const millisecond: number = status
          ? value.getMilliseconds()
          : MILLISECOND;
  ...
  protected findNextTimeElement(event: KeyboardEventArgs): void {
    ...
    if (
      ...
      if (event.action === 'home') {
        ...
        dateTimeVal = +this.createDateObj(
          new Date(this.timeCollections[this.timeCollections.length - 1])
        );
        ...
        if (event.action === 'down') {
          for (let i: number = 0; i < listCount; i++) {
            if (+value < this.timeCollections[i as number]) {
              dateTimeVal = +this.createDateObj(
                new Date(this.timeCollections[i as number])
              );
              this.activeIndex = i;
              break;
            }
      ...
      this.timeElementValue(
        isNullOrUndefined(dateTimeVal) ? null : new Date(dateTimeVal)
      );
    }
  }
  ...
  protected setTimeValue(date: Date, value: Date): Date {
    ...
    if (this.calendarMode === 'Gregorian') {
      dateString = this.globalize.formatDate(time, {
        format: !isNullOrUndefined(this.formatString)
          ? this.formatString
  ...
  protected timeKeyHandler(event: KeyboardEventArgs): void {
    ...
    if (
      isNullOrUndefined(this.getTimeActiveElement()) ||
      this.getTimeActiveElement().length === 0
    ) {
      ...
      if (event.keyCode >= 37 && event.keyCode <= 40) {
        let index: number =
          event.keyCode === 40 || event.keyCode === 39
            ...
            : --this.activeIndex;
        this.activeIndex = index =
          this.activeIndex === listCount ? 0 : this.activeIndex;
        this.activeIndex = index =
          this.activeIndex < 0 ? listCount - 1 : this.activeIndex;
  ...
  public onPropertyChanged(
    newProp: DateTimePickerModel,
    oldProp: DateTimePickerModel
  ): void {
      ...
      switch (prop) {
        case 'value':
          ...
          if (this.enableMask) {
            ...
            Input.setValue(
              this.getFormattedValue(newProp.value),
              this.inputElement,
              this.floatLabelType,
              this.showClearButton
            );
        ...
        case 'locale':
          ...
          if (
            ...
            this.dateTimeOptions.placeholder == null
          ) {
            this.setProperties(
        ...
        case 'scrollTo':
          if (
            this.checkDateValue(new Date(this.checkValue(newProp.scrollTo)))
          ) {
            if (this.dateTimeWrapper) {
              this.setScrollTo();
            }
            this.setProperties(
              {
                scrollTo: this.checkDateValue(
                  new Date(this.checkValue(newProp.scrollTo))
                ),
              },
              true
            );
  ...
  protected restoreValue(): void {
    ...
    this.previousElementValue = this.previousElementValue = isNullOrUndefined(
      this.inputValueCopy
    )
      ? ''
