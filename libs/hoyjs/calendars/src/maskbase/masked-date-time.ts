...
export class MaskedDateTime {
  ...
  private formatRegex: RegExp =
    /EEEEE|EEEE|EEE|EE|E|dddd|ddd|dd|d|MMMM|MMM|MM|M|yyyy|yyy|yy|y|HH|H|hh|h|mm|m|fff|ff|f|aa|a|ss|s|zzzz|zzz|zz|z|'[^']*'|'[^']*'/g;
  ...
  public constructor(parent?: IMaskedDateTime) {
    ...
    this.maskDateValue =
      this.parent.value != null ? new Date(+this.parent.value) : new Date();
  ...
  private createMask(mask: events): void {
    ...
    if (this.parent.maskPlaceholder.dayOfTheWeek) {
      this.defaultConstant['dayOfTheWeek'] =
        this.parent.maskPlaceholder.dayOfTheWeek.toString();
    }

    this.getCUltureMaskFormat();
    const inputValue: string = this.dateformat.replace(
      this.formatRegex,
      this.formatCheck()
    );
    this.isHiddenMask = true;
    this.hiddenMask = this.dateformat.replace(
      this.formatRegex,
      this.formatCheck()
    );
  ...
  private validCharacterCheck(): void {
    ...
    for (
      let i: number = start, j: number = start - 1;
      i < this.hiddenMask.length || j >= 0;
      i++, j--
    ) {
      if (
        i < this.hiddenMask.length &&
      ...
      if (
        j >= 0 &&
        this.validCharacters.indexOf(this.hiddenMask[j as number]) !== -1
      ) {
  ...
  private maskKeydownHandler(args: events): void {
    this.dayTypeCount =
      this.monthTypeCount =
      this.hourTypeCount =
      this.minuteTypeCount =
      this.secondTypeCount =
        0;
    ...
    if (
      !args.e.altKey &&
      !args.e.ctrlKey &&
      (args.e.key === ARROWLEFT ||
        args.e.key === ARROWRIGHT ||
        args.e.key === SHIFTTAB ||
        args.e.key === TAB ||
        args.e.action === SHIFTTAB ||
        args.e.key === END ||
        args.e.key === HOME)
    ) {
      ...
      if (
        ...
        (args.e.key === TAB || args.e.action === SHIFTTAB)
      ) {
        const index: number = args.e.action === SHIFTTAB ? end : 0;
        this.parent.inputElement.selectionStart =
          this.parent.inputElement.selectionEnd = index;
      }
      if (args.e.key === END || args.e.key === HOME) {
        const range: number = args.e.key === END ? length : 0;
        this.parent.inputElement.selectionStart =
          this.parent.inputElement.selectionEnd = range;
      }
    ...
    if (
      !args.e.altKey &&
      !args.e.ctrlKey &&
      (args.e.key === ARROWUP || args.e.key === ARROWDOWN)
    ) {
      let start: number = this.parent.inputElement.selectionStart;
      let formatText: string = '';
      if (
        this.validCharacters.indexOf(this.hiddenMask[start as number]) !== -1
      ) {
  ...
  private differenceCheck(): void {
    ...
    const previousVal: string = this.previousValue.substring(
      0,
      start + this.previousValue.length - inputValue.length
    );
    ...
    if (
      previousVal.indexOf(newVal) === 0 &&
      (newVal.length === 0 ||
        this.previousHiddenMask[newVal.length - 1] !==
          this.previousHiddenMask[newVal.length])
    ) {
      for (let i: number = newVal.length; i < previousVal.length; i++) {
        if (
          this.previousHiddenMask[i as number] !== '' &&
          this.validCharacters.indexOf(this.previousHiddenMask[i as number]) >=
            0
        ) {
          this.isDeletion = this.handleDeletion(
            this.previousHiddenMask[i as number],
            false
          );
        }
    ...
    switch (this.previousHiddenMask[start - 1]) {
      case 'd': {
        let date: number =
          (this.isDayPart &&
          newDateValue.getDate().toString().length < 2 &&
          !this.isPersist()
            ? newDateValue.getDate() * 10
            : 0) + parseInt(newVal[start - 1], 10);
        this.isDateZero = newVal[start - 1] === '0';
        this.parent.isFocused = this.parent.isFocused
          ? false
        ...
        if (date >= 1) {
          ...
          this.dayTypeCount = this.isDateZero
            ? this.dayTypeCount + 1
            : this.dayTypeCount;
        }
        break;
      ...
      case 'M': {
        let month: number;
        if (
          newDateValue.getMonth().toString().length < 2 &&
          !this.isPersist()
        ) {
          month =
            (this.isMonthPart ? (newDateValue.getMonth() + 1) * 10 : 0) +
            parseInt(newVal[start - 1], 10);
        } else {
        ...
        if (!isNaN(month)) {
          ...
          if (month >= 1) {
            ...
            if (this.isDayPart) {
              const previousMaxDate: number = new Date(
                this.previousDate.getFullYear(),
                this.previousDate.getMonth() + 1,
                0
              ).getDate();
              const currentMaxDate: number = new Date(
                newDateValue.getFullYear(),
                newDateValue.getMonth() + 1,
                0
              ).getDate();
              if (
                this.previousDate.getDate() === previousMaxDate &&
                currentMaxDate <= previousMaxDate
              ) {
            ...
            this.monthTypeCount = this.isMonthZero
              ? this.monthTypeCount + 1
              : this.monthTypeCount;
          }
        } else {
          const monthString: string[] = <string[]>(
            this.getCulturedValue('months[stand-alone].wide')
          );
          ...
          while (this.monthCharacter.length > 0) {
            ...
            for (const months of monthValue) {
              if (
                monthString[i as number]
                  .toLowerCase()
                  .indexOf(this.monthCharacter) === 0
              ) {
            ...
            this.monthCharacter = this.monthCharacter.substring(
              1,
              this.monthCharacter.length
            );
          }
        }
        break;
      ...
      case 'y': {
        let year: number =
          (this.isYearPart &&
          newDateValue.getFullYear().toString().length < 4 &&
          !this.isShortYear
            ? newDateValue.getFullYear() * 10
            : 0) + parseInt(newVal[start - 1], 10);
      ...
      case 'h':
        this.hour =
          (this.isHourPart &&
          (newDateValue.getHours() % 12 || 12).toString().length < 2 &&
          !this.isPersist()
            ? (newDateValue.getHours() % 12 || 12) * 10
            : 0) + parseInt(newVal[start - 1], 10);
        this.parent.isFocused = this.parent.isFocused
          ? false
          : this.parent.isFocused;
        ...
        newDateValue.setHours(
          Math.floor(newDateValue.getHours() / 12) * 12 + (this.hour % 12)
        );
      ...
      case 'H':
        this.hour =
          (this.isHourPart &&
          newDateValue.getHours().toString().length < 2 &&
          !this.isPersist()
            ? newDateValue.getHours() * 10
            : 0) + parseInt(newVal[start - 1], 10);
        this.parent.isFocused = this.parent.isFocused
          ? false
          : this.parent.isFocused;
      ...
      case 'm': {
        let minutes: number =
          (this.isMinutePart &&
          newDateValue.getMinutes().toString().length < 2 &&
          !this.isPersist()
            ? newDateValue.getMinutes() * 10
            : 0) + parseInt(newVal[start - 1], 10);
        this.parent.isFocused = this.parent.isFocused
          ? false
          : this.parent.isFocused;
      ...
      case 's': {
        let seconds: number =
          (this.isSecondsPart &&
          newDateValue.getSeconds().toString().length < 2 &&
          !this.isPersist()
            ? newDateValue.getSeconds() * 10
            : 0) + parseInt(newVal[start - 1], 10);
        this.parent.isFocused = this.parent.isFocused
          ? false
          : this.parent.isFocused;
      ...
      case 'a': {
        ...
        const periodString: string[] = <string[]>(
          this.getCulturedValue('dayPeriods.format.wide')
        );
        const periodkeys: string[] = Object.keys(periodString);
        for (let i: number = 0; this.periodCharacter.length > 0; i++) {
          if (
            (periodString[periodkeys[0] as unknown as number]
              .toLowerCase()
              .indexOf(this.periodCharacter) === 0 &&
              newDateValue.getHours() >= 12) ||
            (periodString[periodkeys[1] as unknown as number]
              .toLowerCase()
              .indexOf(this.periodCharacter) === 0 &&
              newDateValue.getHours() < 12)
          ) {
          ...
          this.periodCharacter = this.periodCharacter.substring(
            1,
            this.periodCharacter.length
          );
        }
        break;
      }
  ...
  private formatCheck(): any {
    ...
    function formatValueSpecifier(formattext: string): string {
      let result: string;
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const daysAbbreviated: any = proxy.getCulturedValue(
        'days[stand-alone].abbreviated'
      );
      const dayKeyAbbreviated: string[] = Object.keys(daysAbbreviated);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const daysWide: any = <string[]>(
        proxy.getCulturedValue('days[stand-alone].wide')
      );
      const dayKeyWide: string[] = Object.keys(daysWide);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const daysNarrow: any = <string[]>(
        proxy.getCulturedValue('days[stand-alone].narrow')
      );
      const dayKeyNarrow: string[] = Object.keys(daysNarrow);
      const monthAbbreviated: string[] = <string[]>(
        proxy.getCulturedValue('months[stand-alone].abbreviated')
      );
      const monthWide: string[] = <string[]>(
        proxy.getCulturedValue('months[stand-alone].wide')
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const periodString: any = <string[]>(
        proxy.getCulturedValue('dayPeriods.format.wide')
      );
      const periodkeys: string[] = Object.keys(periodString);
      ...
      switch (formattext) {
        ...
        case 'dddd':
        case 'd':
          result = proxy.isDayPart
            ? proxy.maskDateValue.getDate().toString()
            : proxy.defaultConstant['day'].toString();
        ...
        case 'dd':
          result = proxy.isDayPart
            ? proxy.roundOff(proxy.maskDateValue.getDate(), 2)
            : proxy.defaultConstant['day'].toString();
          result = proxy.zeroCheck(proxy.isDateZero, proxy.isDayPart, result);
          if (proxy.dayTypeCount === 2) {
            proxy.isNavigate = true;
        ...
        case 'EEE':
          result =
            proxy.isDayPart && proxy.isMonthPart && proxy.isYearPart
              ? daysAbbreviated[
                  dayKeyAbbreviated[proxy.maskDateValue.getDay()]
                ].toString()
              : proxy.defaultConstant['dayOfTheWeek'].toString();
          break;
        case 'EEEE':
          result =
            proxy.isDayPart && proxy.isMonthPart && proxy.isYearPart
        ...
        case 'EEEEE':
          result =
            proxy.isDayPart && proxy.isMonthPart && proxy.isYearPart
              ? daysNarrow[
                  dayKeyNarrow[proxy.maskDateValue.getDay()]
                ].toString()
              : proxy.defaultConstant['dayOfTheWeek'].toString();
          break;
        case 'M':
          result = proxy.isMonthPart
            ? (proxy.maskDateValue.getMonth() + 1).toString()
            : proxy.defaultConstant['month'].toString();
          result = proxy.zeroCheck(
            proxy.isMonthZero,
            proxy.isMonthPart,
            result
          );
        ...
        case 'MM':
          result = proxy.isMonthPart
            ? proxy.roundOff(proxy.maskDateValue.getMonth() + 1, 2)
            : proxy.defaultConstant['month'].toString();
          result = proxy.zeroCheck(
            proxy.isMonthZero,
            proxy.isMonthPart,
        ...
        case 'MMM':
          result = proxy.isMonthPart
            ? monthAbbreviated[proxy.maskDateValue.getMonth() + 1]
            : proxy.defaultConstant['month'].toString();
          break;
        case 'MMMM':
          result = proxy.isMonthPart
            ? monthWide[proxy.maskDateValue.getMonth() + 1]
            : proxy.defaultConstant['month'].toString();
          break;
        case 'yy':
          result = proxy.isYearPart
            ? proxy.roundOff(proxy.maskDateValue.getFullYear() % 100, 2)
            : proxy.defaultConstant['year'].toString();
        ...
        case 'yyyy':
          result = proxy.isYearPart
            ? proxy.roundOff(proxy.maskDateValue.getFullYear(), 4)
            : proxy.defaultConstant['year'].toString();
          result = proxy.zeroCheck(proxy.isYearZero, proxy.isYearPart, result);
          break;
        case 'h':
          result = proxy.isHourPart
            ? (proxy.maskDateValue.getHours() % 12 || 12).toString()
            : proxy.defaultConstant['hour'].toString();
        ...
        case 'hh':
          result = proxy.isHourPart
            ? proxy.roundOff(proxy.maskDateValue.getHours() % 12 || 12, 2)
            : proxy.defaultConstant['hour'].toString();
          if (proxy.hourTypeCount === 2) {
            proxy.isNavigate = true;
            proxy.hourTypeCount = 0;
        ...
        case 'H':
          result = proxy.isHourPart
            ? proxy.maskDateValue.getHours().toString()
            : proxy.defaultConstant['hour'].toString();
          if (proxy.hourTypeCount === 2) {
            proxy.isNavigate = true;
            proxy.hourTypeCount = 0;
        ...
        case 'HH':
          result = proxy.isHourPart
            ? proxy.roundOff(proxy.maskDateValue.getHours(), 2)
            : proxy.defaultConstant['hour'].toString();
          if (proxy.hourTypeCount === 2) {
            proxy.isNavigate = true;
            proxy.hourTypeCount = 0;
          ...
          break;
        case 'm':
          result = proxy.isMinutePart
            ? proxy.maskDateValue.getMinutes().toString()
            : proxy.defaultConstant['minute'].toString();
        ...
        case 'mm':
          result = proxy.isMinutePart
            ? proxy.roundOff(proxy.maskDateValue.getMinutes(), 2)
            : proxy.defaultConstant['minute'].toString();
          if (proxy.minuteTypeCount === 2) {
            proxy.isNavigate = true;
            proxy.minuteTypeCount = 0;
          ...
          break;
        case 's':
          result = proxy.isSecondsPart
            ? proxy.maskDateValue.getSeconds().toString()
            : proxy.defaultConstant['second'].toString();
        ...
        case 'ss':
          result = proxy.isSecondsPart
            ? proxy.roundOff(proxy.maskDateValue.getSeconds(), 2)
            : proxy.defaultConstant['second'].toString();
          if (proxy.secondTypeCount === 2) {
            proxy.isNavigate = true;
            proxy.secondTypeCount = 0;
        ...
        case 'f':
          result = Math.floor(
            proxy.maskDateValue.getMilliseconds() / 100
          ).toString();
          break;
        case 'ff':
          ...
          if (proxy.maskDateValue.getMilliseconds() > 99) {
            milliseconds = Math.floor(
              proxy.maskDateValue.getMilliseconds() / 10
            );
          }
        ...
        case 'aa':
          result =
            proxy.maskDateValue.getHours() < 12
              ? periodString['am']
              : periodString['pm'];
          break;
        case 'z':
        ...
        case 'zzzz':
          ...
          result = proxy.parent.globalize.formatDate(
            proxy.maskDateValue,
            dateOptions
          );
          break;
      ...
      result =
        result !== undefined
          ? result
          : formattext.slice(1, formattext.length - 1);
  ...
  private navigateSelection(isbackward: boolean): void {
    ...
    while (formatIndex < this.hiddenMask.length && formatIndex >= 0) {
      if (
        this.validCharacters.indexOf(this.hiddenMask[formatIndex as number]) >=
        0
      ) {
  ...
  private dateAlteration(isDecrement: boolean): void {
    ...
    const newDateValue: Date = new Date(
      this.maskDateValue.getFullYear(),
      this.maskDateValue.getMonth(),
      this.maskDateValue.getDate(),
      this.maskDateValue.getHours(),
    ...
    this.maskDateValue =
      newDateValue.getFullYear() > 0 ? newDateValue : this.maskDateValue;
    if (this.validCharacters.indexOf(this.hiddenMask[start as number]) !== -1) {
  ...
  private getCulturedValue(format: string): object {
    ...
    if (locale === 'en' || locale === 'en-US') {
      ...
      result = getValue(
        'main.' + '' + locale + ('.dates.calendars.gregorian.' + format),
        cldrData
      );
    }
    return result;
  ...
  private getCulturedFormat(): string {
    let formatString: string = this.getCulturedValue(
      'dateTimeFormats[availableFormats].yMd'
    ).toString();
    if (this.parent.moduleName === 'datepicker') {
      formatString = this.getCulturedValue(
        'dateTimeFormats[availableFormats].yMd'
      ).toString();
