...
import { Complex, Browser, isNullOrUndefined } from '@hoyui/base';
import {
  Event,
  EmitType,
  EventHandler,
...
import {
  ...
  getValueFromAngle,
  getRange,
} from './utils/helper-circular-gauge';
  ...
  implements INotifyPropertyChanged
{
   ...
   */
  @Complex<FontModel>(
    { size: null, color: null, fontWeight: null, fontFamily: null },
    Font
  )
  ...
  protected render(): void {
    if (!isNullOrUndefined(this.element)) {
      ...
      this.allowLoadingAnimation =
        ((this.animationDuration === 0 && animationMode === 'Enable') ||
  ...
  private unWireEvents(): void {
    ...
    EventHandler.remove(
      this.element,
      Browser.isPointer ? 'pointerleave' : 'mouseleave',
      this.mouseLeave
    );
  ...
  public mouseMove(e: PointerEvent): boolean {
    this.setMouseXY(e);
    const args: IMouseEventArgs = this.getMouseArgs(
      e,
      'touchmove',
      gaugeMouseMove
    );
    this.trigger('gaugeMouseMove', args, (observedArgs: IMouseEventArgs) => {
      ...
      if (!args.cancel) {
        if (
          (this.enablePointerDrag || this.enableRangeDrag) &&
          this.svgObject.getAttribute('cursor') !== 'grabbing'
        ) {
          if (
            (args.target.id.indexOf('_Pointer_') !== -1 &&
              this.enablePointerDrag) ||
            (this.enableRangeDrag && args.target.id.indexOf('_Range_') !== -1)
          ) {
        ...
        const svgElement: HTMLElement = <HTMLElement>(
          getElement(this.element.id + '_svg')
        );
        ...
        if (this.enablePointerDrag && this.activePointer) {
          this.isDrag = true;
          e.preventDefault();
          const dragPointInd: number = parseInt(
            this.activePointer.pathElement[0].id.slice(-1),
            10
          );
          const dragAxisInd: number = parseInt(
            this.activePointer.pathElement[0].id.split('_Axis_')[1],
            10
          );
          dragArgs = {
          ...
          this.pointerDrag(
            new GaugeLocation(args.x + extraWidth, args.y),
            dragAxisInd,
            dragPointInd
          );
          ...
          e.preventDefault();
          const dragAxisInd: number = parseInt(
            this.activeRange.pathElement[0].id.split('_Axis_')[1],
            10
          );
          const dragRangeInd: number = parseInt(
            this.activeRange.pathElement[0].id.split('Range_')[1],
            10
          );
          dragArgs = {
            axis: this.activeAxis,
  ...
  public pointerDrag(
    ...
    const value: number = getValueFromAngle(
      getAngleFromLocation(this.midPoint, location),
      range.max,
      range.min,
      axis.startAngle,
      axis.endAngle,
    ...
    if (value >= range.min && value <= range.max) {
      this.axes[axisIndex as number].pointers[pointerIndex as number].value =
        value;
      this.activePointer.currentValue = value;
      this.gaugeAxisLayoutPanel.pointerRenderer.setPointerValue(
        axis,
        this.activePointer,
        value
      );
  ...
  public rangeDrag(
    ...
    if (this.activeAxis) {
      ...
      if (value >= range.min && value <= range.max) {
        ...
        const start: number =
          typeof this.activeRange.start === 'string'
            ? parseFloat(<string>this.activeRange.start)
            : this.activeRange.start;
        const end: number =
          typeof this.activeRange.end === 'string'
            ? parseFloat(<string>this.activeRange.end)
        ...
        this.endValue =
          value < avg
            ? previousValue1 > avg
              ...
              : this.activeRange.start
            : value;
        this.axes[axisIndex as number].ranges[rangeIndex as number].start =
          this.startValue;
        this.axes[axisIndex as number].ranges[rangeIndex as number].end =
          this.endValue;
        if (this.isTouch) {
          this.setRangeValue(
            axisIndex,
    ...
    this.trigger('gaugeMouseDown', args, (observedArgs: IMouseEventArgs) => {
      if (
        ...
        this.activePointer = <Pointer>(
          this.activeAxis.pointers[currentPointer.pointerIndex]
        );
        if (isNullOrUndefined(this.activePointer.pathElement)) {
          this.activePointer.pathElement = [e.target as Element];
        }
        if (
          this.activePointer.type === 'Marker' &&
          this.activePointer.markerShape === 'Text' &&
          this.activePointer.pathElement.length === 0
        ) {
          this.activePointer.pathElement = [e.target as Element];
        }
        const pointInd: number = parseInt(
          this.activePointer.pathElement[0].id.slice(-1),
          10
        );
        const axisInd: number = parseInt(
          this.activePointer.pathElement[0].id.split('_Axis_')[1],
          10
        );
  ...
  public mouseEnd(e: PointerEvent): boolean {
    ...
    if (this.activeAxis && this.activePointer && this.enablePointerDrag) {
      this.svgObject.setAttribute('cursor', 'auto');
      const pointerInd: number = parseInt(
        this.activePointer.pathElement[0].id.slice(-1),
        10
      );
      const axisInd: number = parseInt(
      ...
      this.svgObject.setAttribute('cursor', 'auto');
      const rangeInd: number = parseInt(
        this.activeRange.pathElement[0].id.slice(-1),
        10
      );
      const axisInd: number = parseInt(
        this.activeRange.pathElement[0].id.split('_Axis_')[1],
    ...
    if (
      ...
      (args.target.id === this.element.id + '_CircularGaugeTitle' ||
        args.target.id.indexOf('_gauge_legend_') > -1)
    ) {
      const touchArg: TouchEvent = <TouchEvent & PointerEvent>e;
      titleTooltip(
        e,
        touchArg.changedTouches[0].pageX,
        touchArg.changedTouches[0].pageY,
        this,
        true
      );
    }
  ...
  private getMouseArgs(
    ...
    location.x += isTouch
      ? (<TouchEvent & PointerEvent>e).changedTouches[0].clientX
      : e.clientX;
    location.y += isTouch
      ? (<TouchEvent & PointerEvent>e).changedTouches[0].clientY
      : e.clientY;
    return {
      cancel: false,
      name: name,
    ...
    if (!this.isDestroyed) {
      ...
      if (!args.cancel) {
        ...
        if (
          ...
          this.element.classList.contains('e-circulargauge')
        ) {
          this.animatePointer = false;
          this.resizeTo = window.setTimeout((): void => {
            this.isResize = true;
  ...
  private initPrivateVariable(): void {
    if (this.element.id === '') {
      const collection: number =
        document.getElementsByClassName('e-circulargauge').length;
  ...
  private calculateSvgSize(): Size {
    ...
    const borderWidth: number =
      parseInt(this.element.style.borderWidth.split('px').join(''), 10) * 2;
    let width: number =
  ...
  private radiusAndCenterCalculation(
    ...
    radius: number,
    titleHeight: number,
    isUpperAngle: boolean,
    isLowerAngle: boolean,
    ...
    let rect: Rect;
    const bottom: number = this.margin.bottom + this.border.width;
    let minRadius: number;
    let widthRadius: number;
    let centerX: number;
    let centerY: number;
    if (
      this.moveToCenter &&
      this.axes.length === 1 &&
      isNullOrUndefined(this.centerXpoint) &&
      isNullOrUndefined(this.centerYpoint)
    ) {
      ...
      if (!this.allowMargin) {
        if (
          ...
          rect = new Rect(
            left + width / 2 - minRadius,
            top + height / 2 - minRadius,
            minRadius * 2,
            minRadius * 2
          );
        } else {
          if (
            width > height &&
            ((isLowerAngle && isLower) || (isUpperAngle && isUpper))
          ) {
            widthRadius = width / 2;
            const heightValue: number =
              isUpper && isLower ? height / 2 : height * (3 / 4);
            if (widthRadius > heightValue) {
              widthRadius = heightValue;
            }
            rect = new Rect(
              left + width / 2 - widthRadius,
              top + height / 2 - widthRadius,
              widthRadius * 2,
              widthRadius * 2
            );
          } else {
    ...
    if (!this.allowMargin) {
      if (
        ...
        this.gaugeRect.width =
          width - (this.availableSize.width - this.gaugeRect.width) / 2;
        this.gaugeRect.y = this.gaugeRect.y - difference;
        this.gaugeRect.height =
          this.gaugeRect.height +
          difference +
          (this.availableSize.height - this.gaugeRect.height) / 2;
      } else if (
        !isNullOrUndefined(this.legendModule) &&
        (isUpperAngle || isLowerAngle) &&
        width > height &&
        (this.legendSettings.position === 'Left' ||
          this.legendSettings.position === 'Right')
      ) {
        const difference: number = this.gaugeRect.height - this.gaugeRect.width;
        this.gaugeRect.x =
          this.legendSettings.position === 'Right'
            ? this.gaugeRect.x + this.margin.right
            : this.gaugeRect.x;
        this.gaugeRect.width =
          this.legendSettings.position === 'Left'
            ? Math.abs(
            ...
            : Math.abs(
                width -
                  (this.availableSize.width - this.gaugeRect.width) / 2 -
                  10
              );
      }
      centerX =
        this.centerXpoint !== null
          ? stringToNumber(this.centerXpoint, this.availableSize.width)
          : this.gaugeRect.x + this.gaugeRect.width / 2;
      if (
        (isUpperAngle || isLowerAngle) &&
        !isNullOrUndefined(this.legendModule)
      ) {
        centerX =
          ...
          this.legendSettings.position === 'Bottom'
            ...
            : this.legendSettings.position === 'Right'
            ? this.gaugeRect.width / 2 + this.margin.right
            : centerX;
      }
      centerY =
        isUpperAngle || isLowerAngle
          ? isUpperAngle
            ...
            : this.gaugeRect.height * (1 / 4) + this.gaugeRect.y
          : this.gaugeRect.y + this.gaugeRect.height / 2;
      centerY =
        !isFullPercent && (isUpperAngle || isLowerAngle)
          ? this.gaugeRect.height / 2 +
            this.gaugeRect.y +
            radiusPercent * (3 / 4) * (1 / 2)
          : centerY;
      if (
        !isNullOrUndefined(this.axes) &&
        this.axes.length > 1 &&
        !isNullOrUndefined(this.midPoint)
      ) {
      ...
      centerX =
        this.centerXpoint !== null
          ...
          : this.gaugeRect.x + this.gaugeRect.width / 2;
      centerY =
        this.centerYpoint !== null
          ? stringToNumber(this.centerYpoint, this.availableSize.height)
          : this.gaugeRect.y + this.gaugeRect.height / 2;
    }
  ...
  private calculateBounds(): void {
    ...
    const left: number = margin.left + this.border.width;
    ...
    if (!this.allowMargin) {
      for (let j: number = 0; j < this.axes.length; j++) {
        const isUpperAngle: boolean =
          270 <= this.axes[j as number].startAngle &&
          this.axes[j as number].startAngle <= 360 &&
          0 <= this.axes[j as number].endAngle &&
          this.axes[j as number].endAngle <= 90;
        const isLowerAngle: boolean =
          90 >= this.axes[j as number].startAngle &&
          this.axes[j as number].startAngle <= 180 &&
          180 <= this.axes[j as number].endAngle &&
          270 <= this.axes[j as number].endAngle &&
          0 !== this.axes[j as number].startAngle &&
          360 !== this.axes[j as number].endAngle;
        isUpper = isUpperAngle ? isUpperAngle : isUpper;
        isLower = isLowerAngle ? isLowerAngle : isLower;
        const isFullPercent: boolean =
          this.axes[j as number].radius !== null
            ? parseInt(this.axes[0].radius.split('%')[0], 10) >= 100
            : true;
        const radiusPercent: number =
          this.axes[j as number].radius !== null
            ? radius * (parseInt(this.axes[0].radius.split('%')[0], 10) / 100)
            : radius;
        this.radiusAndCenterCalculation(
          top,
          left,
          width,
          height,
  ...
  private renderAnimation(): void {
    if (this.allowLoadingAnimation) {
      for (let i: number = 0; i < this.axes.length; i++) {
        this.gaugeAxisLayoutPanel.axisLineAnimation(
          i,
          this.loadingAnimationDuration[i as number],
          this
        );
      }
    }
  ...
  private renderLegend(): void {
    ...
    if (this.legendModule.legendCollection.length) {
      this.legendModule.renderLegend(
        this.legendSettings,
        this.legendModule.legendBounds
      );
    }
  }
  /**
  ...
  private renderTitle(): void {
    if (this.title) {
      ...
      let style: FontModel = {
        ...
        fontWeight:
          this.titleStyle.fontWeight || this.themeStyle.titleFontWeight,
      ...
      const element: Element = textElement(
        options,
        style,
        style.color || this.themeStyle.titleFontColor,
        this.svgObject,
        ''
      );
    ...
    if (borderWidth > 0 || this.background || this.themeStyle.backgroundColor) {
      this.svgObject.appendChild(
        this.renderer.drawRectangle(
          new RectOption(
            this.element.id + '_CircularGaugeBorder',
            ...
            new Rect(
              borderWidth / 2,
              borderWidth / 2,
              this.availableSize.width - borderWidth,
  ...
  public setPointerValue(
    ...
    if (!this.isDestroyed && pointer.currentValue !== value) {
      const enableAnimation: boolean =
        pointer.animation.enable || animationMode === 'Enable';
      ...
      document.getElementById(
        this.element.id + '_Axis_' + axisIndex + '_Pointer_' + pointerIndex
      ...
      pointer.pathElement.map((element: Element) => {
        if (pointer.type === 'RangeBar') {
          ...
          if (element.id.indexOf('_Pointer_NeedleCap_') >= 0) {
            ...
            setStyles(
              element as HTMLElement,
              pointer.needleTail.color,
              pointer.needleTail.border
            );
          } else if (element.id.indexOf('_Pointer_NeedleRect_') >= 0) {
            setStyles(element as HTMLElement, 'transparent', {
              color: 'transparent',
              width: 0,
            });
          } else if (
            pointer.type === 'Marker' &&
            pointer.markerShape !== 'Text'
          ) {
            setStyles(element as HTMLElement, pointer.color, pointer.border);
          }
          if (enableAnimation) {
            if (pointer.type === 'Marker' && pointer.markerShape === 'Text') {
              this.gaugeAxisLayoutPanel.pointerRenderer.performTextAnimation(
                element as HTMLElement,
                pointer.currentValue,
                value,
                axis,
  ...
  public setAnnotationValue(
    ...
    if (!this.isDestroyed) {
      this.allowLoadingAnimation = false;
      const isElementExist: boolean =
        getElement(this.element.id + '_Annotations_' + axisIndex) !== null;
      const element: HTMLElement =
        <HTMLElement>(
          getElement(this.element.id + '_Annotations_' + axisIndex)
        ) ||
        createElement('div', {
          id: this.element.id + '_Annotations_' + axisIndex,
          styles: this.animationDuration > 0 ? 'opacity: 0' : 'opacity: 1',
        });
      const annotation: Annotation = <Annotation>(
        this.axes[axisIndex as number].annotations[annotationIndex as number]
      );
      if (content !== null) {
        removeElement(
          this.element.id +
        ...
        this.annotationsModule.createTemplate(
          element,
          annotationIndex,
          axisIndex,
          this
        );
        const secondaryElement: Element = getElement(
  ...
  public export(
    ...
    if (type === 'PDF' && this.allowPdfExport && this.pdfExportModule) {
      ...
      this.allowImageExport &&
      type !== 'PDF' &&
      this.imageExportModule
    ) {
      // eslint-disable-next-line
      return new Promise((resolve: any, reject: any) => {
        resolve(
  ...
  public setRangeValue(
    ...
    if (
      ...
      this.legendModule.getLegendOptions(this.axes as Axis[]);
      const height: number =
        this.legendModule.legendBounds.height +
        this.legendSettings.margin.top +
        this.legendSettings.margin.bottom +
        this.legendSettings.border.width;
      const width: number =
        this.legendModule.legendBounds.width +
        this.legendSettings.margin.left +
        this.legendSettings.margin.right +
        this.legendSettings.border.width;
      // eslint-disable-next-line prefer-const
    ...
    if (
      !isNullOrUndefined(<string>range.startWidth) &&
      (<string>range.startWidth).length > 0
    ) {
    ...
    let endWidth: number;
    if (
      !isNullOrUndefined(<string>range.endWidth) &&
      (<string>range.endWidth).length > 0
    ) {
      endWidth = toPixel(<string>range.endWidth, range.currentRadius);
    } else {
      endWidth = <number>range.endWidth;
    }
    endAngle = isClockWise
      ? endAngle
      : [startAngle, (startAngle = endAngle)][0];
    endWidth = isClockWise
      ? endWidth
      : [startWidth, (startWidth = endWidth)][0];
    ...
    setStyles(
      element as HTMLElement,
      range.color ? range.color : range.rangeColor,
      {
  ...
  public requiredModules(): ModuleDeclaration[] {
    ...
    axes.map((axis: Axis) => {
      axis.annotations.map((annotation: Annotation) => {
        if (!annotationEnable) {
          annotationEnable =
            (!isNullOrUndefined(annotation.content) &&
              annotation.content.length !== 0) ||
            typeof annotation.content === 'function';
        }
      });
    });
    if (annotationEnable) {
  ...
  public onPropertyChanged(
    newProp: CircularGaugeModel,
    oldProp: CircularGaugeModel
  ): void {
    ...
    if (!this.isDestroyed) {
      ...
      this.allowLoadingAnimation =
        this.animationDuration > 0 && !this.isOverAllAnimationComplete
          ? true
          : false;
      let refreshBounds: boolean = false;
      let refreshWithoutAnimation: boolean = false;
      const isPointerValueSame: boolean =
        Object.keys(newProp).length === 1 &&
        newProp instanceof Object &&
            ...
            for (let x: number = 0; x < axesPropertyLength; x++) {
              if (!isNullOrUndefined(newProp.axes[x as number])) {
                const collection: string[] = Object.keys(
                  newProp.axes[x as number]
                );
                for (const collectionProp of collection) {
                  if (collectionProp === 'pointers') {
                    const pointerPropertyLength: number = Object.keys(
                      newProp.axes[x as number].pointers
                    ).length;
                    for (let y: number = 0; y < pointerPropertyLength; y++) {
                      const index: number = parseInt(
                        Object.keys(newProp.axes[x as number].pointers)[
                          y as number
                        ],
                        10
                      );
                      if (
                        !isNullOrUndefined(
                          Object.keys(
                            newProp.axes[x as number].pointers[index as number]
                          )
                        )
                      ) {
                        this.allowLoadingAnimation = false;
                        this.loadingAnimationDuration = [];
                        this.isAnimationProgress =
                          this.axes[x as number].pointers[
                            index as number
                          ].animation.enable;
                        this.axes[x as number].pointers[index as number][
                          'previousValue'
                        ] =
                          this.axes[x as number].pointers[index as number][
                            'currentValue'
