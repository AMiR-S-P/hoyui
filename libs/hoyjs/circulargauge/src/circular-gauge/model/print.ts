...
export class Print {
  ...
  public print(
    ...
    const argsData: IPrintEventArgs = {
      cancel: false,
      htmlContent: this.getHTMLContent(gauge, elements),
      name: beforePrint,
    };
    // eslint-disable-next-line
    gauge.trigger(
      'beforePrint',
      argsData,
      (beforePrintArgs: IPrintEventArgs) => {
        if (!argsData.cancel) {
  ...
  public getHTMLContent(
    ...
    if (elements) {
      ...
      const exportElement: HTMLElement = gauge.element.cloneNode(
        true
      ) as HTMLElement;
        ...
        if (!isNullOrUndefined(backgroundElement)) {
          ...
          if (
            ...
            (gauge.theme === 'TailwindDark' ||
              ...
              gauge.theme === 'Fluent2HighContrast') &&
            (backgroundColor === 'rgba(255,255,255, 0.0)' ||
              backgroundColor === 'transparent')
          ) {
