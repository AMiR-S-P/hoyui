import { Component, Property, L10n } from '@hoyui/base';
...
import {
  ...
  exportAsImage,
} from '../barcode/utility/barcode-util';
 ...
 * ```
 */
export class DataMatrixGenerator
  ...
  implements INotifyPropertyChanged
{
   ...
   */
  constructor(
    options?: DataMatrixGeneratorModel,
  ...
  public onPropertyChanged(
    newProp: DataMatrixGeneratorModel,
    oldProp: DataMatrixGeneratorModel
  ): void {
    let width: number | string;
    let height: number | string;
    if (this.mode === 'Canvas' && newProp.mode !== 'Canvas') {
      refreshCanvasBarcode(
        this as DataMatrixGenerator,
    ...
    if (newProp.width) {
      width =
        this.mode === 'Canvas' && newProp.mode !== 'Canvas'
          ? (newProp.width as number) * 1.5
          : newProp.width;
