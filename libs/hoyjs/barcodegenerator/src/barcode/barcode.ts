import { Component, Property, L10n, ModuleDeclaration } from '@hoyui/base';
import { INotifyPropertyChanged, Complex, Event, EmitType } from '@hoyui/base';
import {
  RenderingMode,
  BarcodeType,
  BarcodeEvent,
  ...
  implements INotifyPropertyChanged
{
  ...
  public onPropertyChanged(
    newProp: BarcodeGeneratorModel,
    oldProp: BarcodeGeneratorModel
  ): void {
    if (this.mode === 'Canvas' && newProp.mode !== 'Canvas') {
      this.refreshCanvasBarcode();
    } else {
      this.barcodeRenderer = removeChildElements(
        newProp,
        this.barcodeCanvas,
        this.mode,
        this.element.id
    ...
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        case 'width':
          ...
          this.barcodeCanvas.setAttribute(
            'width',
            String(this.element.offsetWidth)
          );
          break;
        case 'height':
          ...
          this.barcodeCanvas.setAttribute(
            'height',
            String(this.element.offsetHeight)
          );
          break;
        case 'backgroundColor':
          this.barcodeCanvas.setAttribute(
            'style',
            'background:' + newProp.backgroundColor
          );
          break;
        case 'mode':
          this.initialize();
  ...
  public exportAsBase64Image(exportType: BarcodeExportType): Promise<string> {
    const returnValue: Promise<string> = exportAsImage(
      exportType,
      '',
      this.element,
      true,
  ...
  private renderElements(): void {
    ...
    if (this.mode === 'Canvas') {
      (this.barcodeCanvas as HTMLCanvasElement)
        .getContext('2d')
    ...
    if (
      (this.type === 'Ean8' || this.type === 'Ean13' || this.type === 'UpcA') &&
      this.displayText.text.length > 0
    ) {
  ...
  private clearCanvas(view: BarcodeGenerator): void {
    ...
    if (view.mode !== 'SVG') {
      const ctx: CanvasRenderingContext2D = BarcodeCanvasRenderer.getContext(
        this.barcodeCanvas as HTMLCanvasElement
      );
  ...
  protected preRender(): void {
    ...
    this.barcodeRenderer = new BarcodeRenderer(
      this.element.id,
      this.mode === 'SVG'
    );
    this.initialize();
    ...
    this.setCulture();
    const measureElement: HTMLCollection = document.getElementsByClassName(
      'barcodeMeasureElement'
    );
    if (measureElement.length > 0) {
      for (let i: number = measureElement.length - 1; i >= 0; i--) {
        measureElement[parseInt(i.toString(), 10)].parentNode.removeChild(
          measureElement[parseInt(i.toString(), 10)]
        );
      }
