...
describe('Code128B - Barcode', () => {
  describe('checking Code128B barcode rendering all lines check', () => {
    ...
    it('checking Code128A barcode rendering with display text margin and barcode margin', (done: Function) => {
      ...
      expect(
        Math.round(Number(children.children[0].getAttribute('x'))) === 79 ||
          (Math.round(Number(children.children[0].getAttribute('x'))) === 78 &&
            (children.children[0] as HTMLElement).style.fontSize === '20px')
      ).toBe(true);
  ...
  describe('checking Code128B barcode rendering all lines check', () => {
    ...
    it('checking Code128B barcode rendering all lines check', (done: Function) => {
      ...
      let children: HTMLElement = barcode.children[0] as HTMLElement;
      expect(
        Math.round(Number(children.children[0].getAttribute('x'))) === 110 &&
          (children.children[0] as HTMLElement).style.fontSize === '13.6px'
      ).toBe(true);
      for (let j: number = 1; j < children.children.length - 1; j++) {
        expect(
          Math.round(Number(children.children[j + 1].getAttribute('x'))) ===
