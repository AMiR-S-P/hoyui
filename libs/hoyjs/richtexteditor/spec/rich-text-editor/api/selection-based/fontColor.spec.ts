 ...
 * Font color spec
 */
import { isNullOrUndefined, detach } from '@hoyui/base';
...
describe('RTE SELECTION BASED - fontColor - ', () => {
  ...
  describe(' onPropertyChange - ', () => {
    describe(' change the fontColor.colorCode - ', () => {
      ...
      it(' Test the FontColor change dynamically ', (done) => {
        ...
        let popup: HTMLElement = document.querySelector(
          '#' + controlId + '_toolbar_FontColor-popup'
        );
    ...
    describe(' change the fontColor.columns - ', () => {
      ...
      it(' Test the columns', () => {
        ...
        expect(
          columns.querySelectorAll('.e-rte-square-palette').length === 12
        ).toBe(true);
        expect(
          (popup.querySelector('.e-container.e-color-palette') as HTMLElement)
            .style.width !== '0px'
        ).toBe(true);
      });
    });
    describe(' change the fontColor.default - ', () => {
      ...
      it(' Test the default value', () => {
        ...
        item = item.querySelector(
          '.e-rte-color-content .e-rte-font-color'
        ) as HTMLElement;
  ...
  describe(' public methods - ', () => {
    describe(' getHtml and getText method - ', () => {
      ...
      it(' Test the getHtml method after apply the font color ', (done) => {
        ...
        expect(
          rteObj.getHtml() ===
            '<p id="rte"><span style="color: rgb(255, 255, 0); text-decoration: inherit;">RTE</span></p>'
        ).toBe(true);
        done();
      });
  ...
  describe('EJ2-41061 - Issue with selecting the transparent color from the FontColor in Rich Text Editor ', () => {
    ...
    it(' Test the FontColor ', (done) => {
      ...
      let fontColorPopup: Element = document.querySelector(
        '#' + controlId + '_toolbar_FontColor-popup'
      );
      expect(fontColorPopup.classList.contains('e-popup-open')).toBe(true);
      expect(
        fontColorPopup
          .querySelector('.e-custom-palette .e-row > .e-selected')
          .getAttribute('aria-label')
      ).toBe('');
      expect(
        fontColorPopup
          .querySelector('.e-custom-palette .e-row > .e-selected')
          .classList.contains('e-nocolor-item')
      ).toBe(true);
      (
        fontColorPopup.querySelector('.e-mode-switch-btn') as HTMLElement
      ).click();
      expect(
        fontColorPopup.querySelectorAll('.e-custom-palette').length > 0
      ).toBe(false);
      expect(
        fontColorPopup.querySelectorAll('.e-color-picker').length > 0
      ).toBe(true);
      done();
    });
    it(' Test the BackgroundColor ', (done) => {
      ...
      (item.querySelector('.e-btn-icon.e-caret') as HTMLElement).click();
      let bgColorPopup: Element = document.querySelector(
        '#' + controlId + '_toolbar_BackgroundColor-popup'
      );
      ...
      expect(
        bgColorPopup.querySelectorAll('.e-custom-palette').length > 0
      ).toBe(false);
      expect(bgColorPopup.querySelectorAll('.e-color-picker').length > 0).toBe(
        true
      );
      done();
    });
