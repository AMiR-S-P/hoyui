...
describe(' Inline Quick Toolbar - ', () => {
  ...
  describe(' inlineMode property onPropertyChange- ', () => {
    ...
    it(' Test - enable as true the inline toolbar dynamically', (done) => {
      ...
      dispatchEvent(pEle, 'mouseup');
      setTimeout(() => {
        expect(document.querySelectorAll('.e-rte-quick-popup').length > 0).toBe(
          true
        );
        done();
      }, 200);
  ...
  describe('EJ2-39369 - Inline toolbar Font Size not update in toolbar status', () => {
    ...
    it('FontSize toolbar status', (done: Function) => {
      ...
      setTimeout(() => {
        let item: HTMLElement = document.querySelector(
          '#' + controlId + '_quick_FontSize'
        );
        item.click();
        let popup: HTMLElement = document.getElementById(
          controlId + '_quick_FontSize-popup'
        );
        dispatchEvent(
        ...
        setTimeout(() => {
          dispatchEvent(pEle, 'mouseup');
          setTimeout(() => {
            let sizeSpan: HTMLElement = item.querySelector(
  ...
  describe('EJ2-49293 - Inline toolbar goes outside of window', () => {
    ...
    it('Template class availability testing', (done: Function) => {
      ...
      dispatchEvent(pEle, 'mouseup');
      setTimeout(() => {
        let fontNameItem: HTMLElement = document.querySelector(
          '#' + controlId + '_quick_FontName'
        ).parentElement;
        expect(fontNameItem.classList.contains('e-rte-inline-template')).toBe(
          true
        );
        let formatsItem: HTMLElement = document.querySelector(
          '#' + controlId + '_quick_Formats'
        ).parentElement;
        expect(formatsItem.classList.contains('e-rte-inline-template')).toBe(
          true
        );
        let alignmentsItem: HTMLElement = document.querySelector(
          '#' + controlId + '_quick_Alignments'
        ).parentElement;
        expect(alignmentsItem.classList.contains('e-rte-inline-template')).toBe(
          true
        );
        let olItem: HTMLElement = document.querySelector(
          '#' + controlId + '_quick_NumberFormatList'
        ).parentElement;
        expect(olItem.classList.contains('e-rte-inline-template')).toBe(true);
        let ulItem: HTMLElement = document.querySelector(
          '#' + controlId + '_quick_BulletFormatList'
        ).parentElement;
        expect(ulItem.classList.contains('e-rte-inline-template')).toBe(true);
        let fontSizeItem: HTMLElement = document.querySelector(
          '#' + controlId + '_quick_FontSize'
        ).parentElement;
        expect(
          fontSizeItem.classList.contains('e-rte-inline-size-template')
        ).toBe(true);
        let fontColorItem: HTMLElement = document.querySelector(
          '#' + controlId + '_quick_FontColor'
        ).parentElement;
        expect(
          fontColorItem.classList.contains('e-rte-inline-color-template')
        ).toBe(true);
        let bgColorItem: HTMLElement = document.querySelector(
          '#' + controlId + '_quick_BackgroundColor'
        ).parentElement;
        expect(
          bgColorItem.classList.contains('e-rte-inline-color-template')
        ).toBe(true);
        done();
      }, 200);
    });
