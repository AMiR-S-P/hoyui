import { createElement, detach, getUniqueID, extend } from '@hoyui/base';
...
export let ieUA: string =
  'Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko';
export let androidUA: string =
  'Mozilla/5.0 (Linux; <Android Version>; <Build Tag etc.>) AppleWebKit/<WebKit Rev> (KHTML, like Gecko) Chrome/<Chrome Rev> Mobile Safari/<WebKit Rev>';
export let iPhoneUA: string =
...
export const hostURL: string =
  'https://ej2services.syncfusion.com/js/development/';

export function renderRTE(
  options: RichTextEditorModel,
  customElement: any = null
): RichTextEditor {
...
export function removeStyleElements(elements: any) {
  if (elements.length > 0) {
    [].slice.call(elements).forEach((ele: any) => {
      detach(ele);
    });
  }
}
...
export type ImageResizeGripper =
  | 'e-rte-botRight'
  | 'e-rte-botLeft'
  | 'e-rte-topRight'
  | 'e-rte-topLeft';

export type VideoResizeGripper =
  | 'e-rte-botRight'
  | 'e-rte-botLeft'
  | 'e-rte-topRight'
  | 'e-rte-topLeft';
