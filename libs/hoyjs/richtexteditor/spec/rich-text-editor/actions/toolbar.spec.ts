...
import {
  ...
  TOOLBAR_FOCUS_SHORTCUT_EVENT_INIT,
} from '../../constant.spec';
...
describe('Checking the customized NumberFormatListand BulletFormatList dropdownItems', () => {
  ...
  it(' Checking the sampleLevel numberFormatList items ', (done) => {
    let format: HTMLElement = rteObj.element.querySelector(
      '#' + controlId + '_toolbar_NumberFormatList'
    );
    ...
    setTimeout(() => {
      let items: any = document.querySelectorAll(
        '#' + controlId + '_toolbar_NumberFormatList-popup .e-item'
      );
  ...
  it(' Checking the sampleLevel bulletFormatList items ', (done) => {
    let format: HTMLElement = rteObj.element.querySelector(
      '#' + controlId + '_toolbar_BulletFormatList'
    );
    dispatchEvent(format, 'mousedown');
    dispatchEvent(format, 'mouseup');
    format.click();
    setTimeout(() => {
      let items: any = document.querySelectorAll(
        '#' + controlId + '_toolbar_BulletFormatList-popup .e-item'
      );
      expect(items[0].textContent === 'None').toBe(true);
...
describe('EJ2-59865 - css class dependency component', () => {
  ...
  it(' css class dependency initial load and dynamic change ', (done) => {
    ...
    setTimeout(() => {
      expect(
        document
          .querySelectorAll('.e-dropdown-popup')[0]
          .classList.contains('customClass')
      ).toBe(true);
      expect(
        document
          .querySelectorAll('.e-dropdown-popup')[1]
          .classList.contains('customClass')
      ).toBe(true);
      expect(
        document
          .querySelector('.e-rte-numberformatlist-dropdown')
          .classList.contains('customClass')
      ).toBe(true);
      expect(
        document
          .querySelector('.e-rte-bulletformatlist-dropdown')
          .classList.contains('customClass')
      ).toBe(true);
      ...
      expect(
        document
          .querySelectorAll('.e-dropdown-popup')[0]
          .classList.contains('changedClass')
      ).toBe(true);
      expect(
        document
          .querySelectorAll('.e-dropdown-popup')[1]
...
describe('Checking the NumberFormatList dropdownItems', () => {
  ...
  it(' Check the numberFormatList items 1', (done) => {
    ...
    let numberFormatList: HTMLElement = rteObj.element.querySelector(
      '#' + controlId + '_toolbar_NumberFormatList'
    );
...
describe('Checking the BulletFormatList  dropdownItems', () => {
  ...
  it(' Check the bulletFormatList items 1', (done) => {
    ...
    let bulletFormatList: HTMLElement = rteObj.element.querySelector(
      '#' + controlId + '_toolbar_BulletFormatList'
    );
...
describe('Applying ordered and unordered list then, advanced ol,ul to p tag', () => {
  ...
  it('Applying Ordered list then, NumberFormatList to p tag ', (done) => {
    ...
    let OL: HTMLElement = rteObj.element.querySelector(
  ...
  it('Applying UnOrdered list then, BulletFormatList to p tag ', (done) => {
    ...
    let UL: HTMLElement = rteObj.element.querySelector(
...
describe('Checking the NumberFormatListand BulletFormatList style type and style image', () => {
      ...
      numberFormatList: {
        types: [
          ...
          { text: 'Katakana', value: 'katakana' },
          {
            ...
            value: 'listImage',
            listImage:
              'url("https://mdn.mozillademos.org/files/11981/starsolid.gif")',
          },
        ],
      },
      bulletFormatList: {
        types: [
          ...
          { text: 'Square', value: 'square' },
          {
            text: 'Circle',
            value: 'listImage',
            listImage: 'linear-gradient(to left bottom, red, blue)',
          },
        ],
      },
      value: `<p id="rte">RichTextEditor</p>`,
  ...
  it(' Checking the NumberFormatListlist style image to style type', (done) => {
    ...
    items[6].click();
    expect(
      pEle.style.listStyleImage ===
        'url("https://mdn.mozillademos.org/files/11981/starsolid.gif")'
    ).toBe(true);
    expect(rteObj.element.querySelector('#rte').tagName === 'OL').toBe(true);
    let numberFormatList1: HTMLElement = rteObj.element.querySelector(
      '#' + controlId + '_toolbar_NumberFormatList'
    );
    dispatchEvent(numberFormatList1, 'mousedown');
    dispatchEvent(numberFormatList1, 'mouseup');
    numberFormatList1.click();
    let item: any = document.querySelectorAll(
      '#' + controlId + '_toolbar_NumberFormatList-popup .e-item'
    );
    item[3].click();
    let Elem: HTMLElement = rteObj.element.querySelector('#rte');
  ...
  it(' Checking the BulletFormatList list style image to style type', (done) => {
    ...
    items[2].click();
    expect(
      pEle.style.listStyleImage === 'linear-gradient(to left bottom, red, blue)'
    ).toBe(true);
    expect(rteObj.element.querySelector('#rte').tagName === 'UL').toBe(true);
    let bulletFormatList1: HTMLElement = rteObj.element.querySelector(
      '#' + controlId + '_toolbar_BulletFormatList'
    );
...
describe('Checking the LI tag', () => {
  ...
  it(' Check the numberFormatList with LI tag', (done) => {
    ...
    items[3].click();
    expect(
      (rteObj.element.querySelector('#rte').childNodes[0] as HTMLElement)
        .tagName === 'LI'
    ).toBe(true);
    expect(rteObj.element.querySelector('#rte').tagName === 'OL').toBe(true);
    expect(pEle.style.listStyleType === 'lower-roman').toBe(true);
...
describe('Toolbar - Actions Module', () => {
  ...
  describe('ToolbarSettings property testing', () => {
    ...
    it("toolbarSettings - 'items' property default value element testing", () => {
      ...
      expect(
        (<HTMLElement>tbItems.item(0)).firstElementChild.id.indexOf('Bold') > 0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(1)).firstElementChild.id.indexOf('Italic') >
          0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(2)).firstElementChild.id.indexOf(
          'Underline'
        ) > 0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(3)).classList.contains('e-separator')
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(4)).firstElementChild.id.indexOf('Formats') >
          0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(5)).firstElementChild.id.indexOf(
          'Alignments'
        ) > 0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(6)).firstElementChild.id.indexOf(
          'Blockquote'
        ) > 0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(7)).firstElementChild.id.indexOf(
          'OrderedList'
        ) > 0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(8)).firstElementChild.id.indexOf(
          'UnorderedList'
        ) > 0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(9)).classList.contains('e-separator')
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(10)).firstElementChild.id.indexOf(
          'CreateLink'
        ) > 0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(11)).firstElementChild.id.indexOf('Image') >
          0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(12)).classList.contains('e-separator')
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(13)).firstElementChild.id.indexOf(
          'SourceCode'
        ) > 0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(14)).firstElementChild.id.indexOf('Undo') > 0
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(15)).firstElementChild.id.indexOf('Redo') > 0
      ).toBe(true);
    });
  });
  ...
  describe("toolbarSettings - 'items' property customize value testing", () => {
    ...
    it('Customize value as string with element testing', () => {
      ...
      expect(
        (<HTMLElement>tbItems.item(2)).classList.contains('e-separator')
      ).toBe(true);
      expect(
        (<HTMLElement>tbItems.item(3)).firstElementChild.id.indexOf(
          'Maximize'
        ) > 0
      ).toBe(true);
    });
    ...
    it('Object collection with items list testing', () => {
      ...
      let tbItemsEle: HTMLElement[] = selectAll(
        '.e-toolbar-item',
        rteObj.element
      );
    ...
    it('Using both string and Object collection with items list testing', () => {
      ...
      expect(tbItemsEle[1].firstElementChild.id.indexOf('Italic') > 0).toBe(
        true
      );
      expect(tbItemsEle[2].classList.contains('e-separator')).toBe(true);
      expect(tbItemsEle[3].firstElementChild.id.indexOf('Maximize') > 0).toBe(
        true
      );
    ...
    it('Minimize button availability testing', () => {
      ...
      expect(
        rteEle
          ...
          .querySelector('span')
          .classList.contains('e-minimize')
      ).toBe(true);
      expect(
        rteEle
          .querySelectorAll('.e-toolbar-item')[0]
          .firstElementChild.id.indexOf('Minimize') > 0
      ).toBe(true);
    });
  });
  ...
  describe('Toolbar fullscreen item with maximize/minimize testing', () => {
    ...
    it('Maximize, minimize button click testing', () => {
      let trgEle: HTMLElement = <HTMLElement>(
        rteEle.querySelectorAll('.e-toolbar-item button')[1]
      );
      expect(rteEle.querySelectorAll('.e-toolbar-item').length).toBe(2);
      expect(
        rteEle.querySelectorAll('.e-toolbar-item')[0].getAttribute('title')
    ...
    it('Scrollable div parent element with maximize testing', () => {
      ...
      rteWrapper = createElement('div', {
        id: 'rteWrapper',
        innerHTML: rteElement.outerHTML,
      });
      ...
      (<HTMLElement>select('#rteWrapper', document.body)).style.height =
        '300px';
      (<HTMLElement>select('#rteWrapper', document.body)).style.overflow =
        'auto';
      let trgEle: HTMLElement = <HTMLElement>(
        rteEle.querySelectorAll('.e-toolbar-item button')[1]
      );
  ...
  describe('Fixed toolbar testing', () => {
    ...
    it('DIV - Class testing', () => {
      ...
      expect(rteEle.querySelectorAll('.e-toolbar').length).toBe(1);
      expect(
        rteEle.querySelector('.e-toolbar').classList.contains('e-show')
      ).toBe(false);
      destroy(rteObj);
      Browser.userAgent = defaultUA;
      rteObj = renderRTE({
    ...
    it('DIV - Focus and blur based fixed toolbar visibility testing', () => {
      ...
      rteEle = rteObj.element;
      expect(rteEle.querySelectorAll('.e-toolbar').length).toBe(1);
      let cntEle: HTMLElement = rteEle.querySelector(
    ...
    it('DIV - Content focus after toolbar interaction with focus state testing', () => {
      ...
      let trg: HTMLElement = <HTMLElement>(
        document.querySelectorAll('.e-toolbar-item > button')[3]
      );
    ...
    it('DIV - Maximize/Minimize testing', (done: Function) => {
      ...
      trg = <HTMLElement>(
        document.querySelectorAll('.e-toolbar-item > button')[1]
      );
      trg.click();
      setTimeout(() => {
        expect(
    ...
    it('IFrame - Focus and blur based fixed toolbar visibility testing', () => {
      ...
      let cntEle: HTMLElement = (
        rteObj.contentModule.getPanel() as HTMLIFrameElement
  ...
  describe('Floating toolbar testing', () => {
    ...
    it('Class testing', () => {
      expect(
        document
          .querySelector('.e-richtexteditor .e-toolbar')
          .parentElement.classList.contains('e-rte-tb-float')
      ).toBe(true);
    });
  ...
  describe('SourceCode view checking if dyncamically property is updated in DIV mode', () => {
    ...
    it('Preventing the SourceCode view while updating the enableFloating dynamically', () => {
      expect(
        document.querySelector('.e-bold').parentElement.attributes[6].value
      ).toBe('false');
      expect(
        document.querySelector('.e-italic').parentElement.attributes[6].value
      ).toBe('false');
      expect(
        document.querySelector('.e-source-code').parentElement.attributes[6]
          .value
      ).toBe('false');
      ...
      item.click();
      expect(
        rteObj.rootContainer.classList.contains('e-source-code-enabled')
      ).toBe(true);
      ...
      expect(
        document.querySelector('.e-preview').parentElement.attributes[6].value
      ).toBe('false');
      expect(
        rteObj.rootContainer.classList.contains('e-source-code-enabled')
      ).toBe(true);
      ...
      rteObj = renderRTE({
        toolbarSettings: {
          items: [
            {
              tooltipText: 'CustomButton',
              template: "<button id='customBtn'>Button</button>",
            },
            { tooltipText: 'CustomText', template: '<p></p>' },
          ],
        },
  ...
  describe("'Formats' - Dropdown button item click testing", () => {
    ...
    it('DropDownButton popup open and item click testing', () => {
      ...
      expect(
        document
          .querySelector('.e-dropdown-popup .e-item')
          .classList.contains('e-paragraph')
      ).toBe(true);
      let formatDropDownItem: HTMLElement = <HTMLElement>(
        document.querySelectorAll('.e-dropdown-popup .e-item')[0]
      );
      mouseEventArgs = {
        target: formatDropDownItem,
      };
      rteObj.toolbarModule.dropDownModule.formatDropDown.clickHandler(
        mouseEventArgs
      );
  ...
  describe('Dropdown button render testing', () => {
    ...
    it('Check id of formats toolbar', () => {
      ...
      expect(
        rteEle
          ...
          .firstElementChild.id.indexOf('FontName') > 0
      ).toBe(true);
      expect(
        rteEle
          .querySelectorAll('.e-toolbar-item')[3]
          .firstElementChild.id.indexOf('FontSize') > 0
      ).toBe(true);
    });

    it('DropDown button class testing', () => {
      expect(
        rteEle.querySelectorAll('.e-toolbar-item .e-dropdown-btn').length
      ).toBe(4);
      expect(
        rteEle.querySelectorAll('.e-toolbar-item .e-dropdown-btn.e-control')
          .length
      ).toBe(4);
    });
    ...
    it('DropDownButton popup availability testing', () => {
      expect(
        document
          .querySelectorAll('.e-rte-dropdown-popup.e-popup')[0]
          .id.indexOf('Alignments') > 0
      ).toBe(true);
      expect(
        document
          .querySelectorAll('.e-rte-dropdown-popup.e-popup')[1]
          .id.indexOf('Formats') > 0
      ).toBe(true);
    });
  ...
  describe('Expand toolbar testing', () => {
    ...
    it('Class testing', () => {
      expect(
        document
          .querySelector('.e-richtexteditor')
          .classList.contains('e-rte-tb-expand')
      ).toBe(true);
    ...
    it('Extend popup availability testing', () => {
      ...
      expect(
        document
          .querySelectorAll('.e-toolbar-extended')[0]
          .classList.contains('e-popup-close')
      ).toBe(true);
    });
  ...
  describe('Toolbar status update method testing', () => {
    ...
    it('Class testing', () => {
      ...
      expect(
        (<HTMLElement>tbItemsEle[9].firstElementChild).innerText.trim()
      ).toBe('Paragraph');
      expect(
        (<HTMLElement>tbItemsEle[10].firstElementChild).innerText.trim()
      ).toBe('Font Name');
      expect(
        (<HTMLElement>tbItemsEle[11].firstElementChild).innerText.trim()
      ).toBe('Font Size');
      expect(
        (<HTMLElement>(
          tbItemsEle[12].firstElementChild
        )).firstElementChild.classList.contains('e-justify-left')
      ).toBe(true);
      expect(
        (<HTMLElement>(
          tbItemsEle[12].firstElementChild
        )).firstElementChild.classList.contains('e-icons')
      ).toBe(true);
    ...
    it('Add toolbar status testing', () => {
      ...
      expect(
        (<HTMLElement>(
          tbItemsEle[12].firstElementChild
        )).firstElementChild.classList.contains('e-justify-right')
      ).toBe(true);
      expect(
        (<HTMLElement>(
          tbItemsEle[12].firstElementChild
  ...
  describe('To open the dropdown button in the toolbar', () => {
    ...
    it('open drop down button using keyboard', () => {
      ...
      rteObj.contentModule
        .getEditPanel()
        .dispatchEvent(TOOLBAR_FOCUS_SHORTCUT_EVENT_INIT);
      ...
      expect(document.activeElement.getAttribute('tabindex') === '0').toBe(
        true
      );
    });
  ...
  describe("EJ2-14546- toolbarSettings - 'itemConfigs' property ", () => {
    ...
    it(' Change the default icons of toolbar items', () => {
      expect(
        !isNullOrUndefined(
          rteEle
            .querySelector('#' + controlId + '_toolbar_FontColor')
            .querySelector('.e-underline')
        )
      ).toBe(true);
      expect(
        !isNullOrUndefined(
          rteEle
            .querySelector('#' + controlId + '_toolbar_Italic')
            .querySelector('.e-bold')
        )
      ).toBe(true);
      expect(
        !isNullOrUndefined(
          rteEle
            .querySelector('#' + controlId + '_toolbar_Bold')
            .querySelector('.e-italic')
        )
      ).toBe(true);
    });
  });
  ...
  describe('EJ2-31670 - Need to provide callback (event action) support for RTE custom toolbar', () => {
    ...
    let container: HTMLElement = createElement('div', {
      id: getUniqueID('container'),
      styles: 'height:800px;',
    });
    let element1: HTMLElement = createElement('div', {
      ...
      expect(
        (
          document.querySelector(
            '.e-rte-content textarea'
          ) as HTMLTextAreaElement
        ).value.indexOf('Heading 1') > 0
      ).toEqual(true);
      done();
    });
  });
  ...
  describe('Check toolbar click event in readyOnly', () => {
    ...
    it('Check style', (done) => {
      expect(
        (rteObj.element.querySelector('[title="Print"]') as HTMLElement).style
          .pointerEvents
      ).toBe('');
  ...
  describe('Scrollable toolbar', () => {
    ...
    it('Check class', () => {
      expect(rteObj.element.querySelector('.e-scroll-right-nav')).not.toBe(
        'null'
      );
      expect(rteObj.element.querySelector('.e-scroll-left-nav')).not.toBe(
        'null'
      );
    });
  });
          ...
          items: [
            {
              ...
              template:
                '<button class="e-tbar-btn e-btn" tabindex="-1" id="custom_tbar"  style="width:100%"><div class="e-tbar-btn-text" style="font-weight: 500;">_</div></button>',
              click: function () {
                ...
                if (
                  currentEle === rteObj.inputElement ||
                  closest(currentEle, '.e-content') === null
                ) {
  ...
  describe('Localization text for Formats and Font family', () => {
    ...
    it(' Check the formats dropdown items ', (done) => {
      ...
      setTimeout(() => {
        let items: any = document.querySelectorAll(
          '#' + controlId + '_toolbar_Formats-popup .e-item'
        );
    ...
    it(' Check the fontFamily dropdown items ', (done) => {
      ...
      setTimeout(() => {
        let items: any = document.querySelectorAll(
          '#' + controlId + '_toolbar_FontName-popup .e-item'
        );
  ...
  describe('Check whether the selected text changed to the selected format and font name', () => {
    ...
    it(' Check whether the selected text changed to the selected font name ', (done) => {
      ...
      expect(span.style.fontFamily === 'Arial, Helvetica, sans-serif').toBe(
        true
      );
      done();
    });
...
describe('EJ2-58542: Memory leak issue with Rich Text Editor component ', () => {
  ...
  it('When OffsetParent is null in toolbar', (done: Function) => {
    ...
    const allDropDownPopups: NodeListOf<Element> =
...
describe('849075 - The screen reader does not read the toolbar items in the Rich Text Editor', () => {
  ...
  it('Case 1: Checking the tab index on the focus action of the editor', () => {
    ...
    for (let i: number = 0; i < toolbarItems.length; i++) {
      expect(
        toolbarItems[i].firstElementChild.getAttribute('tabindex') === '-1'
      ).toBe(true);
    }
  });
  ...
  it('Case 4: Checking the tab index after the toolbar focus using shortcut keys', () => {
    editorObj.focusIn();
    editorObj.contentModule
      .getEditPanel()
      .dispatchEvent(toolbarFocusShortCutEvent);
    const toolbarItems: NodeListOf<Element> =
      document.querySelectorAll('.e-toolbar-item');
    for (let i: number = 0; i < toolbarItems.length; i++) {
      if (i == 2) {
        ...
        expect(
          toolbarItems[i].firstElementChild.getAttribute('tabindex') === null
        ).toBe(true);
      } else {
        expect(
          toolbarItems[i].firstElementChild.getAttribute('tabindex') === '-1'
  ...
  it('Case 5: Checking the tab index on Source Code view', () => {
    editorObj.focusIn();
    const sourceCodeButton: HTMLElement = editorObj.element.querySelector(
      '#' + editorObj.getID() + '_toolbar_SourceCode'
    );
    sourceCodeButton.click();
    const sourceCodeTextArea: HTMLElement =
      editorObj.element.querySelector('.e-rte-srctextarea');
    ...
    for (let i: number = 0; i < toolbarItems.length; i++) {
      ...
      if (
        toolbarItems[i].getAttribute('title') === 'Preview (Ctrl+Shift+H)' ||
        toolbarItems[i].getAttribute('title') === 'Minimize(Esc)'
      ) {
        expect(
          toolbarItems[i].firstElementChild.getAttribute('tabindex') === null
        ).toBe(true);
...
describe('849075 - Checking the tab index on navigating the toolbar items using arrow keys', () => {
  // Checking the tab index value on focus blur and toolbar navigation states.
  const rightArrowKeyUpEvent = new KeyboardEvent(
    'keyup',
    ARROWRIGHT_EVENT_INIT
  );
  const rightArrowKeyDownEvent = new KeyboardEvent(
    'keydown',
    ARROWRIGHT_EVENT_INIT
  );
  let editorObj: RichTextEditor;
  ...
  it('Case 1:', (done: DoneFn) => {
    ...
    expect(
      toolbarItems[0].firstElementChild.getAttribute('tabindex') === '-1'
    ).toBe(true);
    expect(
      toolbarItems[1].firstElementChild.getAttribute('tabindex') === '-1'
    ).toBe(true);
    expect(
      toolbarItems[2].firstElementChild.getAttribute('tabindex') === null
    ).toBe(true); // Normal item
    toolbarItems[2].dispatchEvent(rightArrowKeyDownEvent);
    toolbarItems[2].dispatchEvent(rightArrowKeyUpEvent);
    setTimeout(() => {
      expect(
        toolbarItems[2].firstElementChild.getAttribute('tabindex') === '-1'
      ).toBe(true);
      expect(
        toolbarItems[3].firstElementChild.getAttribute('tabindex') === null
      ).toBe(true); // Normal item
      done();
    }, 800);
  });
  it('Case 2:', (done: DoneFn) => {
    ...
    setTimeout(() => {
      expect(
        toolbarItems[3].firstElementChild.getAttribute('tabindex') === '-1'
      ).toBe(true);
      expect(
        toolbarItems[4].firstElementChild.getAttribute('tabindex') === '0'
      ).toBe(true); // Template item
      done();
    }, 800);
  });
});
describe('863039-Top sentence is not showing when Maximize/Minimize toolbar', () => {
  ...
  it('Test - Toolbar height testing when height after maximize/minimize', () => {
    const toolbarElement = rteEle.querySelector(
      '.e-toolbar-wrapper'
    ) as HTMLElement | null;
    const toolbarHeight: string | null = toolbarElement.style.height;
    const trgEle: HTMLElement = <HTMLElement>(
      rteEle.querySelectorAll('.e-toolbar-item')[0]
    );
    trgEle.click();
    trgEle.click();
    const newElement = rteEle.querySelector(
      '.e-toolbar-wrapper'
    ) as HTMLElement | null;
...
describe('863056-Code view shortcut key tooltip is not displaying properly', () => {
  ...
  it('check tooltip name of code view and preview', (done) => {
    rteObj.focusIn();
    let codeView: HTMLElement = <HTMLElement>(
      document.body.querySelectorAll('.e-toolbar-items')[0].childNodes[0]
    );
    expect(codeView.title === 'Code View (Ctrl+Shift+H)').toBe(true);
    codeView.click();
    setTimeout(() => {
      const previewButton: HTMLElement = <HTMLElement>(
        document.body.querySelectorAll('.e-toolbar-item')[0]
      );
...
describe('865043 - In toolbar settings, enable floating set to false the tooltip does not render', () => {
  ...
  it('Checking the tooltip is rendered when enabling floating is set to false.', (done: Function) => {
    ...
    expect(
      (toolbarItems[0] as HTMLElement).getAttribute('data-content')
    ).not.toBe(null);
    done();
  });
  afterAll((done: DoneFn) => {
...
describe('821312: Bullet list does not reverted after click on the bullet list icon', () => {
  ...
  it('Checking the bullet list revert after list type changed', (done: Function) => {
    let toolbarItem = document.querySelectorAll(
    ...
    (bulletListFristChild as HTMLElement).click();
    expect(
      rteObj.inputElement.innerHTML === `<p class="pele">Description</p>`
    ).toBe(true);
    done();
  });
  afterAll((done: DoneFn) => {
