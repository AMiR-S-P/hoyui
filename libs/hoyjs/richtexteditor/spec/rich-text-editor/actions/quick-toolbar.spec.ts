 ...
 */
import { Browser, select, isNullOrUndefined } from '@hoyui/base';
import {
  RichTextEditor,
  IRenderer,
  QuickToolbar,
...
describe('Quick Toolbar - Actions Module', () => {
  ...
  beforeAll(() => {
    ...
    let css: string =
      '.e-richtexteditor { margin-top: 100px; height: 500px; position: relative; }' +
  ...
  describe('Default value with render testing', () => {
    ...
    it('Availability testing', () => {
      ...
      expect(
        document
          .querySelectorAll('.e-rte-quick-popup')[1]
          .id.indexOf('Text_Quick_Popup') >= 0
      ).toBe(true);
      expect(
        document
          .querySelectorAll('.e-rte-quick-popup')[2]
          .id.indexOf('Image_Quick_Popup') >= 0
      ).toBe(true);
    });
    ...
    it('Text quick popup - toolbar and default items testing', () => {
      let textPop: HTMLElement = <HTMLElement>(
        document.querySelectorAll('.e-rte-quick-popup')[1]
      );
    ...
    it('Image quick popup - toolbar and default items testing', () => {
      let imgPop: HTMLElement = <HTMLElement>(
        document.querySelectorAll('.e-rte-quick-popup')[2]
      );
      ...
      expect(
        (<HTMLElement>imgTBItems.item(4)).classList.contains('e-separator')
      ).toBe(true);
  ...
  describe('EJ2-59865 - css class dependency component', () => {
    ...
    it('css class dependency initial load and dynamic change', () => {
      ...
      QTBarModule.textQTBar.showPopup(0, 0, trg);
      QTBarModule.imageQTBar.showPopup(0, 0, trg);
      let linkPop2: HTMLElement = <HTMLElement>(
        document.querySelectorAll('.e-rte-quick-popup')[0]
      );
      let textPop2: HTMLElement = <HTMLElement>(
        document.querySelectorAll('.e-rte-quick-popup')[1]
      );
      let imgPop2: HTMLElement = <HTMLElement>(
        document.querySelectorAll('.e-rte-quick-popup')[2]
      );
  ...
  describe('Quick toolbar - showPopup method with popup open testing', () => {
    ...
    it('Popup open with undo/redo disable testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          (<HTMLElement>tbItems.item(1)).classList.contains('e-overlay')
        ).toBe(true);
        expect(
          (<HTMLElement>tbItems.item(2)).classList.contains('e-overlay')
        ).toBe(true);
        done();
      }, 100);
    });
    ...
    it('Page scroll with popup hide testing', (done: Function) => {
      ...
      let trg: HTMLElement = <HTMLElement>(
        rteObj.element.querySelector('.e-toolbar-item > button')
      );
      let clickEvent: MouseEvent = document.createEvent('MouseEvents');
      clickEvent.initEvent('mousedown', true, true);
      trg.dispatchEvent(clickEvent);
  ...
  describe('Desktop DIV - Quick toolbar - Position testing', () => {
    ...
    it('Text toolbar open testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          textPop.offsetTop + textPop.offsetHeight <
            rteEle.offsetTop + rteEle.offsetHeight
        ).toBe(true);
    ...
    it('Image toolbar open testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          imgPop.offsetTop + imgPop.offsetHeight <
            rteEle.offsetTop + rteEle.offsetHeight
        ).toBe(true);
        QTBarModule.hideQuickToolbars();
        done();
    ...
    it('Link toolbar open testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          linkPop.offsetTop + linkPop.offsetHeight <
            rteEle.offsetTop + rteEle.offsetHeight
        ).toBe(true);
        QTBarModule.hideQuickToolbars();
        done();
    ...
    it("Image element 'Right' align with click testing", (done: Function) => {
      ...
      setTimeout(() => {
        ...
        // expect(imgPop.offsetLeft >= 10).toBe(true);
        expect(
          imgPop.offsetLeft + imgPop.offsetWidth <=
            rteEle.offsetLeft + rteEle.offsetWidth
        ).toBe(true);
    ...
    it('Image element bottom section click testing', (done: Function) => {
      ...
      QTBarModule.imageQTBar.showPopup(
        10,
        imgEle.offsetTop + imgEle.offsetHeight - 10,
        trg.children[0]
      );
      setTimeout(() => {
        let imgPop: HTMLElement = <HTMLElement>(
    ...
    it('Element bottom section click testing', (done: Function) => {
      rteEle.style.marginTop = '500px';
      QTBarModule.linkQTBar.showPopup(
        10,
        linkEle.offsetTop + linkEle.offsetHeight - 5,
        trg.children[0]
      );
      setTimeout(() => {
        let anchorPop: HTMLElement = <HTMLElement>(
    ...
    it('Paragraph element click with text toolbar testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          pop.offsetTop + pop.offsetHeight <
            rteEle.offsetTop + rteEle.offsetHeight
        ).toBe(true);
        done();
      }, 100);
  ...
  describe('Desktop IFrame - Quick toolbar - Position testing', () => {
    ...
    let pageY: number;
    let htmlStr: string =
      "<img id='imgTag' style='width: 200px; float: none' alt='Logo'" +
      " src='http://cdn.syncfusion.com/content/images/sales/buynow/Character-opt.png' /> <br/>" +
      "<a id='linkTag' href='http://www.syncfusion.com'>Syncfusion</a>" +
      '<p> Paragraph </p>';
    ...
    beforeAll((done: Function) => {
      ...
      let iframe: HTMLIFrameElement = <HTMLIFrameElement>(
        document.querySelector('iframe.e-rte-content')
      );
  ...
  describe('Mobile IFrame - Quick toolbar - Position testing', () => {
    ...
    let pageY: number;
    let htmlStr: string =
      "<img id='imgTag' class='e-rte-image' style='width: 200px; float: none' alt='Logo'" +
      " src='http://cdn.syncfusion.com/content/images/sales/buynow/Character-opt.png' /> <br/>" +
      "<a id='linkTag' href='http://www.syncfusion.com'>Syncfusion</a>" +
      '<p> Paragraph </p>';
    ...
    it("Image element 'Right' align with click testing", (done: Function) => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        (document.querySelector('.e-rte-content') as any).contentDocument,
        (
          document.querySelector('.e-rte-content') as any
        ).contentDocument.querySelector('.e-rte-image'),
        (
          document.querySelector('.e-rte-content') as any
        ).contentDocument.querySelector('.e-rte-image'),
        0,
      ...
      (toolbarRenderer as any).dropDownSelected({
        item: { command: 'Images' },
      });
      setTimeout(() => {
        let imgPop: HTMLElement = <HTMLElement>(
  ...
  describe('Inline Quick toolbar - showPopup method with popup open testing', () => {
    ...
    it('KeyUp handler testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          (<HTMLElement>(
            pop.querySelectorAll('.e-toolbar-item')[1]
          )).classList.contains('e-overlay')
        ).toBe(true);
        rteObj.quickToolbarModule.hideInlineQTBar();
        trg = <HTMLElement>(
          rteObj.element.querySelectorAll('.e-rte-srctextarea')[0]
        );
        let clickEvent: MouseEvent = document.createEvent('MouseEvents');
        clickEvent.initEvent('mousedown', true, true);
        trg.dispatchEvent(clickEvent);
    ...
    it('show inline popup on CTRL + A action', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          <HTMLElement>document.querySelectorAll('.e-rte-quick-popup')[0]
        ).not.toBe(undefined);
        done();
      }, 100);
    });
    ...
    it('Page scroll with popup hide testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        window.scrollTo(0, 100);
        setTimeout(() => {
          expect(document.querySelectorAll('.e-rte-quick-popup').length).toBe(
            0
          );
          document.body.style.height = '';
          done();
  ...
  describe('Destroy method testing', () => {
    ...
    beforeEach(() => {
      ...
      rteEle = rteObj.element;
      let trg: HTMLElement = <HTMLElement>(
        rteEle.querySelectorAll('.e-content')[0]
      );
      let clickEvent: MouseEvent = document.createEvent('MouseEvents');
      clickEvent.initEvent('mousedown', true, true);
      trg.dispatchEvent(clickEvent);
  ...
  describe('EJ2-14777 - Inline Quick toolbar - fontcolor, backgrount color', () => {
    ...
    it('KeyUp handler testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(pop).not.toBe(undefined);
        expect(
          pop.querySelectorAll('.e-toolbar-item .e-rte-fontcolor-dropdown')[0]
        ).not.toBe(null);
        expect(
          pop.querySelectorAll('.e-rte-backgroundcolor-dropdown')[0]
        ).not.toBe(null);
        rteObj.quickToolbarModule.hideInlineQTBar();
        done();
      }, 100);
  ...
  describe('EJ2-18674 - RTE Inline toolbar items are not changed dynamically', () => {
    ...
    it('KeyUp handler testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(pop).not.toBe(undefined);
        expect(pop.querySelectorAll('.e-toolbar-item .e-undo')[0]).not.toBe(
          null
        );
        rteObj.quickToolbarModule.hideInlineQTBar();
        done();
  ...
  describe('EJ2-18675 - RTE removeToolbarItem and addToolbarItem method does not work in inline toolbar', () => {
    ...
    it('KeyUp handler testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(pop).not.toBe(undefined);
        expect(pop.querySelectorAll('.e-toolbar-item .e-undo')[0]).toBe(
          undefined
        );
        rteObj.quickToolbarModule.hideInlineQTBar();
        done();
  ...
  describe('Desktop - Inline quick toolbar testing', () => {
    ...
    it('Toolbar availability testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          document.querySelectorAll('.e-rte-tb-fixed.e-rte-tb-mobile').length
        ).toBe(0);
        done();
      }, 100);
    });
  ...
  describe('Android device - Inline quick toolbar testing', () => {
    ...
    it('Toolbar availability testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          document
            .querySelectorAll('.e-rte-tb-fixed.e-rte-tb-mobile')[0]
            .classList.contains('e-show')
        ).toBe(true);
        done();
      }, 100);
    });
  ...
  describe('iOS device - Inline quick toolbar testing', () => {
    ...
    it('MouseEvent args with mouseUp handler testing', (done: Function) => {
      ...
      rteObj.quickToolbarModule.mouseUpHandler({
        args: args,
        touchData: touchData,
      });
      setTimeout(() => {
    ...
    it('Quick toolbar open with selection change event action to quick toolbar availability testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        rteObj.quickToolbarModule.selectionChangeHandler(args);
        setTimeout(() => {
          expect(document.querySelectorAll('.e-rte-inline-popup').length).toBe(
            1
          );
          done();
        }, 100);
    ...
    it('Selection change update with quick toolbar availability testing', (done: Function) => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        pEle.childNodes[0].childNodes[0],
        pEle.childNodes[0].childNodes[0],
        0,
        1
      );
  ...
  describe('817012-text selection Quick toolbar', () => {
    ...
    it('mouseUp handler testing', (done: Function) => {
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.element.querySelector('.test').childNodes[0],
        rteObj.element.querySelector('.test').childNodes[0],
        0,
        3
      );
  ...
  describe('817012-text selection Quick toolbar', () => {
    ...
    it('check the inline mode is enabled', (done: Function) => {
      ...
      rteObj.mouseUp(clickEvent);
      let pop: HTMLElement = <HTMLElement>(
        document.querySelectorAll('.e-rte-text-popup')[0]
      );
      expect(pop).toBe(undefined);
      rteObj.quickToolbarModule.hideQuickToolbars();
      done();
  ...
  describe('817012-text selection Quick toolbar', () => {
    ...
    it('Check text Quick toolbar hide while click Insert table', (done: Function) => {
      ...
      let pop: HTMLElement = <HTMLElement>(
        document
          .querySelectorAll('.e-rte-text-popup')[0]
          .querySelector('[title="Create Table (Ctrl+Shift+E)"]')
      );
      pop.click();
      expect(
        <HTMLElement>document.querySelectorAll('.e-rte-text-popup')[0]
      ).toBe(undefined);
      rteEle = rteObj.element;
      trg = <HTMLElement>rteEle.querySelectorAll('.e-content')[0];
    ...
    it('Check text Quick toolbar hide while click Insert link', (done: Function) => {
      ...
      let pop: HTMLElement = <HTMLElement>(
        document
          .querySelectorAll('.e-rte-text-popup')[0]
          .querySelector('[title="Insert Link (Ctrl+K)"]')
      );
      pop.click();
      expect(
        <HTMLElement>document.querySelectorAll('.e-rte-text-popup')[0]
    ...
    it('Check text Quick toolbar hide while click Insert image', (done: Function) => {
      ...
      let pop: HTMLElement = <HTMLElement>(
        document
          .querySelectorAll('.e-rte-text-popup')[0]
          .querySelector('[title="Insert Image (Ctrl+Shift+I)"]')
      );
      pop.click();
      expect(
        <HTMLElement>document.querySelectorAll('.e-rte-text-popup')[0]
    ...
    it('Check text Quick toolbar hide while click Audio', (done: Function) => {
      ...
      let pop: HTMLElement = <HTMLElement>(
        document
          .querySelectorAll('.e-rte-text-popup')[0]
          .querySelector('[title="Insert Audio (Ctrl+Shift+A)"]')
      );
      pop.click();
      expect(
        <HTMLElement>document.querySelectorAll('.e-rte-text-popup')[0]
    ...
    it('Check text Quick toolbar hide while click video', (done: Function) => {
      ...
      let pop: HTMLElement = <HTMLElement>(
        document
          .querySelectorAll('.e-rte-text-popup')[0]
          .querySelector('[title="Insert Video (Ctrl+Alt+V)"]')
      );
      pop.click();
      expect(
        <HTMLElement>document.querySelectorAll('.e-rte-text-popup')[0]
  ...
  describe('817012-text selection Quick toolbar', () => {
    ...
    it('Check text Quick toolbar hide while click format painter', (done: Function) => {
      rteObject.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObject.element.querySelector('.test').childNodes[0],
        rteObject.element.querySelector('.test').childNodes[0],
        0,
        3
      );
  ...
  describe('817012-text selection Quick toolbar ', () => {
    ...
    it('check the text quick toolbar with link', (done: Function) => {
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.inputElement.children[0].childNodes[0].childNodes[0],
        rteObj.inputElement.children[0].childNodes[0].childNodes[1]
          .childNodes[0],
        4,
        3
      );
  ...
  describe('817012-text selection Quick toolbar ', () => {
    ...
    it('check the text quick toolbar with image', (done: Function) => {
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.inputElement.children[0].childNodes[0].childNodes[0],
        rteObj.inputElement.children[0].childNodes[0],
        4,
        3
      );
      trg = <HTMLElement>(
  ...
  describe('817012-text selection Quick toolbar ', () => {
    ...
    it('check the text quick toolbar with video', (done: Function) => {
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.inputElement.children[0].childNodes[0],
        rteObj.inputElement.children[0].childNodes[2],
        2,
        3
      );
  ...
  describe('848813 - When clicking on the image to open the quick toolbar, the main toolbar icon should not be in a visible state', () => {
    ...
    it('Checking the main toolbar icon should not be in a visible state', (done: Function) => {
      ...
      expect(
        rteEle
          .querySelectorAll('.e-toolbar-item')[6]
          .classList.contains('e-overlay')
      ).toBe(true);
      expect(
        rteEle
          .querySelectorAll('.e-toolbar-item')[25]
          .classList.contains('e-overlay')
      ).toBe(false);
      done();
    });
  ...
  describe('854233 - The quick format toolbar item status is not updated.', () => {
    ...
    it('Check for the toolbar status in the Text Quick Edit toolbar', (done: Function) => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        document.querySelector('.target').firstChild,
        document.querySelector('.target').firstChild,
        0,
        5
      );
      ...
      setTimeout(() => {
        const items: NodeListOf<Element> = document
          .querySelector('.e-rte-text-quicktoolbar')
  ...
  describe('855892 - Quick format toolbar is not opened when the text is selected with Keyboard action.', () => {
    ...
    it('Should open the quick toolbar when the text is selected with Keyboard action.', (done: Function) => {
      editorObj.inputElement.dispatchEvent(
      ...
      setTimeout(() => {
        expect(
          document
            .querySelector('.e-rte-text-quicktoolbar')
            .querySelectorAll('.e-toolbar-item').length
        ).toBe(3);
        done();
      }, 100);
    });
  ...
  describe('872942 - The ALT + F10  shortcut to focus the Quick Toolbar is not working properly.', () => {
    ...
    it('Should open the quick toolbar when the ALT + F10 shortcut is pressed.', (done: Function) => {
      ...
      clickImage(image);
      const shortCutKeyDownEvent: KeyboardEvent = new KeyboardEvent(
        'keydown',
        TOOLBAR_FOCUS_SHORTCUT_EVENT_INIT
      );
      const shortCutKeyUpEvent: KeyboardEvent = new KeyboardEvent(
        'keyup',
        TOOLBAR_FOCUS_SHORTCUT_EVENT_INIT
      );
      ...
      setTimeout(() => {
        expect(
          document.activeElement ===
            editorObj.quickToolbarModule.imageQTBar.toolbarElement.querySelector(
              '.e-toolbar-item'
            )
        );
        done();
  ...
  describe('872307 - Tabel merge quick toolbar tooltip issues ', () => {
    ...
    afterAll((done: DoneFn) => {
      ...
      if (
        document
          .querySelector('.e-quick-dropdown')
          .id.indexOf('quick_TableRows-popup') > 0
      ) {
    ...
    it('check the tooltip text of table cell items', (done: Function) => {
      ...
      let tableQTBarEle: HTMLElement = <HTMLElement>(
        document.querySelector('.e-rte-quick-popup')
      );
      let tableCell: HTMLElement = (
        tableQTBarEle.querySelector('[title="Column"]')
          .nextElementSibling as HTMLElement
      ).childNodes[0] as HTMLElement;
      tableCell.click();
      tableCell.dispatchEvent(clickEvent);
      const mergeCell = document.body.querySelector(
        'li[title="Merge cells"]'
      ) as HTMLElement;
      const horizSplit = document.body.querySelector(
        'li[title="Horizontal split"]'
      ) as HTMLElement;
      const verriSplit = document.body.querySelector(
        'li[title="Vertical split"]'
      ) as HTMLElement;
