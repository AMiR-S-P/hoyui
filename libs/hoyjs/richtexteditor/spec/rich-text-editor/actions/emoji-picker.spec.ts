 ...
 */
import { addClass, createElement, detach } from '@hoyui/base';
import {
  dispatchEvent,
  RichTextEditor,
  ToolbarType,
...
describe('Emoji picker module', () => {
  describe('When we click on the emoji toolbar icon render the emoji picker ', () => {
    ...
    it('Render emoji picker module', () => {
      const element: HTMLElement = rteObj.element.querySelector(
        '#' + controlId + '_toolbar_EmojiPicker'
      );
      element.click();
      expect(rteObj.element.querySelector('.e-rte-emojipicker-popup')).not.toBe(
        null
      );
      expect(rteObj.element.querySelector('.e-rte-emoji-search')).not.toBe(
        null
      );
      expect(rteObj.element.querySelector('.e-rte-emojipicker-btn')).not.toBe(
        null
      );
      expect(rteObj.element.querySelector('.e-rte-emojipicker-group')).not.toBe(
        null
      );
      expect(rteObj.element.querySelector('.e-rte-emojipicker-name')).not.toBe(
        null
      );
      expect(
        rteObj.element.querySelector('.e-rte-emojipickerbtn-group')
      ).not.toBe(null);
    });
    it('when initial render input box should focus', () => {
      const inputEle: HTMLInputElement = rteObj.element.querySelector(
        '.e-rte-emoji-search'
      );
      expect(inputEle === document.activeElement).toBe(true);
    });
    it('when document click emoji picker should close ', () => {
      const inputEle: HTMLElement = rteObj.element.querySelector(
        '.e-rte-emoji-search'
      );
      inputEle.click();
      const element: HTMLElement = rteObj.element.querySelector(
    ...
    it('when click on the toolbar emoji correspondingly focus on the set of first emoji', () => {
      ...
      element.click();
      const toolbarEle: HTMLElement = rteObj.element.querySelector(
    ...
    it('Filter the emoji in input box', () => {
      ...
      const popupObj: HTMLElement = rteObj.element.querySelector(
        '.e-rte-emojipicker-popup'
      );
      dispatchEvent(popupObj, 'keyup');
      expect(
        rteObj.element.querySelectorAll('.e-rte-emojisearch-btn button')[0]
          .innerHTML
      ).toBe('😀');
      inputEle.value = 'sssssssssssssssss';
      dispatchEvent(popupObj, 'keyup');
      expect(
        rteObj.element.querySelectorAll('.e-rte-emojiSearch-noEmoji')
      ).not.toBe(null);
      inputEle.value = '';
      dispatchEvent(popupObj, 'keyup');
      expect(rteObj.element.querySelector('.e-rte-emoji-search')).not.toBe(
    ...
    it('space with colon (:) in rte content render the popup', () => {
      ...
      const firstP: Element = (rteObj as any).inputElement.querySelector(
        '#rte-p'
      );
      ...
      expect(
        rteObj.element.querySelector('.e-rte-emojipicker-toolbar')
      ).not.toBe(null);
      expect(rteObj.element.querySelector('.e-rte-emojipicker-name')).not.toBe(
        null
      );
    ...
    it('space with colon (:) in rte content render the popup and keyboard action - down arrow', () => {
      ...
      (<any>rteObj).keyDown(keyboardEventArgs);
      const btnGroup: NodeListOf<HTMLElement> = rteObj.element.querySelectorAll(
        '.e-rte-emojipickerbtn-group button'
      );
    ...
    it('Insert the emoji in rte content ,close the popup ', () => {
      ...
      expect(rteObj.element.querySelector('.e-rte-emojipicker-popup')).toBe(
        null
      );
    ...
    it('Filter the emoji render the popup and keyboard action - down arrow', () => {
      ...
      expect(btnGroup[0].classList.contains('e-focus')).toBe(true);
      expect(btnGroup[0] === document.activeElement).toBe(true);
      expect(btnGroup[0].getAttribute('aria-label') === 'Grinning face').toBe(
        true
      );
    });
    ...
    it('When click the toolbar emoji the corresponding emoji set of first icon focused ', () => {
      ...
      element.click();
      const toolbarEle: NodeListOf<HTMLElement> =
        rteObj.element.querySelectorAll('.e-rte-emojipicker-toolbar button');
      toolbarEle[0].click();
      const btnGroup: NodeListOf<HTMLElement> = rteObj.element.querySelectorAll(
        '.e-rte-emojipickerbtn-group button'
      );
    ...
    it('In scroll the emoji correspondingly the tollbar set has in hover state  ', () => {
      ...
      element.click();
      const element1: HTMLElement = rteObj.element.querySelector(
        '.e-rte-emojipicker-btn'
      );
    ...
    it('space with colon (:) and filter the emoji', () => {
      ...
      expect(rteObj.element.querySelector('.e-rte-emojisearch-btn')).not.toBe(
        null
      );
      expect(
        rteObj.element.querySelector('.e-rte-emojisearch-btn button').innerHTML
      ).toBe('👨');
    });
  });
  ...
  describe('When toolbar is in extended the emoji picker popup z-index has greater than toolbar', () => {
    ...
    it('emoji picker popup z-index greater than toolbar', () => {
      const extendEle: HTMLElement = rteObj.element.querySelector(
        '.e-toolbar-extended'
      );
  ...
  describe('Space follwed by colon render the popup', () => {
    ...
    it('Scroll the Editor the popup has closed', (done: Function) => {
      ...
      const height: HTMLElement = (rteObj as any).element.querySelector(
        '.e-rte-content'
      );
  ...
  describe('ArrowDown action of set of last emoji', () => {
    ...
    it('ArrowDown action of set of last emoji', () => {
      ...
      element.click();
      const emojiDiv: NodeListOf<HTMLElement> = document.querySelectorAll(
        '.e-rte-emojipickerbtn-group'
      );
      ...
      const emoji: NodeListOf<HTMLElement> = document.querySelectorAll(
        '.e-rte-emojipickerbtn-group button'
      );
      emoji[23].focus();
  ...
  describe('ArrowUp action in emoji filtering case ', () => {
    ...
    it('ArrowUp action in emoji filtering case ', () => {
      ...
      dispatchEvent(popupObj, 'keyup');
      const emojiDiv: HTMLElement = document.querySelector(
  ...
  describe('When we press the enter key insert the emoji ', () => {
    ...
    it('Insert the emoji into editor', () => {
      ...
      (<any>rteObj).emojiPickerModule.onKeyDown({
        preventDefault: function () {},
        keyCode: 13,
        target: emoji[0],
      });
      expect(document.activeElement.innerHTML).toBe(
        '<p id="rte-p">Emoji picker : : : : : : 😀</p>'
      );
    });
  });
  ...
  describe('In filtering case show the No emoji case', () => {
    ...
    it('If give correct emoji name no emoji content need to remove', () => {
      ...
      dispatchEvent(popupObj, 'keyup');
      expect(
        rteObj.element.querySelector('.e-rte-emojiSearch-noEmoji')
      ).not.toBe(null);
      inputEle.value = 'g';
      dispatchEvent(popupObj, 'keyup');
      expect(rteObj.element.querySelector('.e-rte-emojiSearch-noEmoji')).toBe(
        null
      );
    });
  });
  ...
  describe('When insert emoji without focus on button', () => {
    ...
    it('Emoji doesnot insert in editor ', () => {
      ...
      element.click();
      const headerEle: HTMLElement = rteObj.element.querySelector(
        '.e-rte-emojipicker-name'
      );
      headerEle.click();
  ...
  describe('850182-Tooltip not removed after close the emoji picker popup ', () => {
    ...
    it('check the tooltip destroy when close the popup using space key', () => {
      const firstP: HTMLElement = (rteObj as any).inputElement.querySelector(
        '#rte-p'
      );
      ...
      expect(
        rteObj.element.ownerDocument.querySelectorAll('.e-tooltip-wrap')
          .length > 0
      ).toBe(true);
      let textNode: HTMLElement = (rteObj as any).inputElement.querySelector(
        '#rte-p'
      ).childNodes[0];
  ...
  describe('850181 - The emoji is inserted on the toolbar in the Rich Text Editor.', () => {
    ...
    it('The emoji is inserted in the toolbar of the Rich Text Editor.', () => {
      (<any>rteObj).toolbarModule
        .getToolbarElement()
        .querySelector('.e-toolbar-items button')
        .click();
      setTimeout(() => {
        (<any>rteObj).toolbarModule
          .getToolbarElement()
        ...
        (<any>rteObj).toolbarModule
          ...
          .querySelector('.e-toolbar-items button')
          .click();
        var emoji: any = document.querySelectorAll(
          '.e-rte-emojipickerbtn-group button'
        );
  ...
  describe('849570 - "No result found" text was not removed from the emoji picker even after clearing the invalid content from the search bar in the emoji picker', () => {
    ...
    it('Emoji Picker input element close icon click ', () => {
      ...
      (rteObj as any).element
        .querySelector('.e-rte-emojipicker-popup .e-input-group .e-clear-icon')
        .click();
      dispatchEvent(popupObj, 'keyup');
      expect(
        rteObj.element.querySelector(
          '.e-rte-emojipicker-popup .e-rte-emojiSearch-noEmoji'
        ) == null
      ).toBe(true);
    });
  });
  ...
  describe('850067 - "Nested List element not removed while apply the emoji', () => {
    ...
    it('Check nested list element removed when apply the emoji', () => {
      const startContainer: HTMLElement =
        document.querySelector('.startContainer');
      ...
      const emojiEle: HTMLElement = document.querySelectorAll(
        '.e-toolbar-item'
      )[0] as HTMLElement;
      emojiEle.click();
      (
        document.querySelector(
          '.e-rte-emojipickerbtn-group [title="Grinning face"]'
        ) as HTMLElement
      ).click();
      let innerHtml =
  ...
  describe('When rte reach the top position', () => {
    ...
    it('- To open emoji picker popup at correct position', (done) => {
      ...
      rteObj.showEmojiPicker();
      setTimeout(() => {
        const popEle: HTMLElement = rteObj.element.querySelector(
          '.e-rte-emojipicker-popup'
        );
