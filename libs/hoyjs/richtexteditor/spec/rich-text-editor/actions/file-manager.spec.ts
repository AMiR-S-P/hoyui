 ...
 */
import { detach, isNullOrUndefined, Browser } from '@hoyui/base';
import {
  IRenderer,
  RichTextEditor,
  QuickToolbar,
...
describe('FileManager module', () => {
  describe('FileManager render test', () => {
    ...
    it('FileManager class availability testing', (done: Function) => {
      ...
      fileEle = document.body.querySelector(
    ...
    it('image - fileSelect as true', () => {
      (rteObj.fileManagerModule as any).fileObj.trigger('fileSelect', {
        fileDetails: { filterPath: '\\Pictures', isFile: true, type: '.png' },
      });
      expect(
        (
          document.body.querySelector(
            '.e-rte-file-manager-dialog .e-input.e-img-url'
          ) as HTMLInputElement
        ).value
      ).toContain(
        'https://ej2-aspcore-service.azurewebsites.net/api/FileManager/GetImage?path=/Pictures'
      );
    });
    it('image FileSelect as false', () => {
      (rteObj.fileManagerModule as any).fileObj.trigger('fileSelect', {
        fileDetails: { isFile: false, type: '.png' },
      });
      expect(
        (
          document.body.querySelector(
    ...
    it('insert image button click test', () => {
      let inputUrl: HTMLInputElement = document.body.querySelector(
        '.e-rte-file-manager-dialog .e-input.e-img-url'
      ) as HTMLInputElement;
      inputUrl.value =
        'https://ej2.syncfusion.com/javascript/demos/src/rich-text-editor/images/RTEImage-Feather.png';
      let insertBtn: HTMLButtonElement = document.body.querySelector(
        '.e-rte-file-manager-dialog button.e-primary'
      );
    ...
    it('cancel button click testing', (done: Function) => {
      ...
      setTimeout(() => {
        (
          document.body.querySelector(
            '.e-rte-file-manager-dialog button.e-cancel'
          ) as HTMLElement
        ).click();
        fileEle = document.body.querySelector(
          '.e-rte-file-manager-dialog .e-filemanager'
  ...
  describe('EJ2-59865 - css class dependency component', () => {
    ...
    it('css class dependency initial load and dynamic change', (done: Function) => {
      ...
      expect(
        document.body
          .querySelector('.e-rte-file-manager-dialog')
  ...
  describe('Open FileManager while replace image', () => {
    ...
    it('Image toolbar open test', (done: Function) => {
      let trg: HTMLElement = <HTMLElement>(
        rteObj.element.querySelectorAll('.e-content img')[0]
      );
