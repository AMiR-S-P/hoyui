...
describe('Sanitize Html Helper', () => {
  ...
  describe('xss attack while component initial rendering : ', () => {
    ...
    it('check the div element attribute while background image style set as wrong', () => {
      expect(
        rteObj.inputElement.querySelector('#background').hasAttribute('style')
      ).toBe(false);
      expect(
        rteObj.inputElement.querySelector('#expression').hasAttribute('style')
      ).toBe(false);
    });
  ...
  describe('xss attack while executeCommand createLink : ', () => {
    ...
    beforeAll(() => {
      ...
      rteObj.focusIn();
      let range =
        rteObj.formatter.editorManager.nodeSelection.getRange(document);
      let selection = rteObj.formatter.editorManager.nodeSelection.save(
        range,
        document
      );
    ...
    it('check the anchor tag href', () => {
      expect(
        (rteObj.inputElement.querySelector('a') as HTMLElement).getAttribute(
          'href'
        ) === ''
      ).toBe(true);
    });
  ...
  describe('xss attack while executeCommand image : ', () => {
    ...
    it('check the image tag src', () => {
      expect(
        (rteObj.inputElement.querySelector('img') as HTMLElement).getAttribute(
          'src'
        ) === ''
      ).toBe(true);
    });
  ...
  describe('prevent xss attack when enableHtmlSanitizer is true', () => {
    ...
    it('check the script element', () => {
      expect(
        rteObj.inputElement.querySelectorAll('script').length
      ).toBeGreaterThan(0);
    });
  ...
  describe('enableHtmlSanitizer is set to false when insertHTML is used in execueCommand', () => {
    ...
    it('check the script with args.helper null', () => {
      rteObj.executeCommand(
        'insertHTML',
        "<div style='position: relative;width: 100%;height: 0;padding-bottom: 50%;'><iframe frameborder='0' scrolling='no' marginheight='0' marginwidth='0' style='position: absolute;top: 0;left: 0;width: 100%;height: 100%;' frameborder='0' type='text/html' src='https://www.youtube.com/embed/tgbNymZ7vqY?fs=1'></iframe></div>"
      );
      expect(
        rteObj.inputElement.innerHTML ===
          '<div style="position: relative;width: 100%;height: 0;padding-bottom: 50%;"><iframe frameborder="0" scrolling="no" marginheight="0" marginwidth="0" style="position: absolute;top: 0;left: 0;width: 100%;height: 100%;" type="text/html" src="https://www.youtube.com/embed/tgbNymZ7vqY?fs=1"></iframe></div><p>syncfusion RTE</p>'
      ).toBe(true);
    });
