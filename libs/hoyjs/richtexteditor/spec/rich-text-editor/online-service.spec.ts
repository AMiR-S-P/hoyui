import { SuccessEventArgs } from '@hoyui/inputs';
...
describe('Test case with online service', () => {
  ...
  beforeEach(async () => {
    try {
      ...
      console.error(
        'There was a problem with the service. Service status offline.',
        error
      );
      isOnline = false;
    }
  ...
  describe('901364 - After image delete is called multiple times when the CTRL + A is pressed multiple times.', () => {
    ...
    it('Should call the success and then the removing event.', (done: DoneFn) => {
      ...
      setTimeout(() => {
        const dialogElem: HTMLElement =
          editor.element.querySelector('.e-rte-img-dialog');
        const inputElem: HTMLInputElement =
          dialogElem.querySelector('input.e-uploader');
        ...
        if (isOnline) {
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              expect(editor.inputElement.querySelectorAll('img').length).toBe(
                1
              );
              done();
            }, 100);
