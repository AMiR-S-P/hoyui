...
describe('Toolbar - Renderer', () => {
  describe('div content ', () => {
    ...
    it(' Open the DropDownButton with modal', () => {
      ...
      expect(document.querySelectorAll('.e-item.e-active').length == 1).toBe(
        true
      );
  ...
  describe('Checking the role attribute in div mode-', () => {
    ...
    it(' FontCOlor and Backgroundcolor role resting', () => {
      ...
      expect((trgEle.childNodes[0] as HTMLElement).hasAttribute('role')).toBe(
        true
      );
      let trgEle2: HTMLElement = <HTMLElement>(
        rteEle.querySelectorAll('.e-toolbar-item')[1]
      );
      expect((trgEle2.childNodes[0] as HTMLElement).hasAttribute('role')).toBe(
        true
      );
    });
  ...
  describe('Open dropdown button two times', function () {
    ...
    it('Open fontColor dropdown button', function () {
      ...
      (
        document.querySelectorAll('.e-rte-square-palette')[1] as HTMLElement
      ).click();
      expect(
        document.querySelector(
          '.e-dropdown-popup.e-control.e-popup-close.e-popup-modal'
        )
      ).toBe(null);
    });
    afterAll(function () {
  ...
  describe('863259: dropdown active state not working when drop down is opened', function () {
    ...
    it('Check the fontColor dropdown active element', function () {
      ...
      let activeEle = document.querySelector(
        '.e-dropdown-popup .e-default.e-active'
      ) as HTMLElement;
    ...
    it('Check the fontSize dropdown active element', function () {
      ...
      dispatchEvent(trgEle.firstElementChild, 'mousedown');
      let activeEle = document.querySelector(
        '.e-font-size-tbar-btn .e-item.e-active'
      ) as HTMLElement;
      expect(!isNullOrUndefined(activeEle)).toBe(true);
    });
    afterAll(function () {
  ...
  describe('876793: list dropdown active state not working when drop down is opened', function () {
    ...
    it('Check the active state of the list dropdown ', function () {
      let startNode = rteObj.contentModule
        .getDocument()
        .querySelector('.startFocus');
      let endNode = rteObj.contentModule
        .getDocument()
        .querySelector('.endFocus');
      ...
      dispatchEvent(trgEle.firstElementChild, 'mousedown');
      expect(
        (document.querySelector('.e-dropdown-popup .e-active') as HTMLElement)
          .innerText === `Number`
      ).toBe(true);
    });
    afterAll(function () {
  ...
  describe('876802 - In Inline mode, after applying the format type, the applied format type does not remain in the active state.', () => {
    ...
    beforeAll((done) => {
      rteObj = renderRTE({
        ...
        value:
          '<h1 id="rte-element1"><span id=\'test-span\'>Rich text Editor</span></h1><div id="rte-element2">Ri<span id=\'test-span1\' style="font-family: Impact, Charcoal, sans-serif;">ch text Edit</span>or</div>',
      });
      done();
    });
    it('Format preselect in inline mode', (done: Function) => {
      (rteObj as any).focusIn();
      let headerElement: HTMLElement = rteObj.inputElement.querySelector(
        '#rte-element1 #test-span'
      );
      (rteObj as any).formatter.editorManager.nodeSelection.setSelectionText(
        document,
        headerElement.childNodes[0],
        headerElement.childNodes[0],
        2,
        4
      );
      ...
      setTimeout(() => {
        (rteObj as any).element.ownerDocument
          .querySelectorAll(
            '.e-rte-quick-toolbar .e-toolbar-items .e-toolbar-item'
          )[2]
          .firstChild.click();
        let FormatsNameItem: HTMLElement = document.querySelector(
          '#' + rteObj.getID() + '_quick_Formats-popup'
        );
        expect(
          FormatsNameItem.querySelector(
            '.e-item.e-h1[aria-label="Heading 1"]'
    ...
    it('Font name preselect in the inline mode ', (done: Function) => {
      (rteObj as any).focusIn();
      let fontElement = (rteObj as any).inputElement.querySelector(
        '#rte-element2 #test-span1'
      );
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        fontElement.childNodes[0],
        fontElement.childNodes[0],
        5,
        7
      );
      ...
      setTimeout(() => {
        rteObj.element.ownerDocument
          .querySelectorAll(
            '.e-rte-quick-toolbar .e-toolbar-items .e-toolbar-item'
          )[0]
          .firstChild.click();
        let fontNameItem = document.querySelector(
          '#' + rteObj.getID() + '_quick_FontName-popup'
        );
        expect(
          fontNameItem
            .querySelector('.e-item.e-impact')
            .classList.contains('e-active')
        ).toBe(true);
        done();
      }, 200);
  ...
  describe('885590: dropdown active state not working when drop down is opened', function () {
    ...
    it('Check the format dropdown active element', function () {
      ...
      expect(
        document
          .querySelector('.e-item.e-paragraph')
          .classList.contains('e-active')
      ).toBe(true);
    });
    afterAll(function () {
