...
describe('Audio Module', () => {
  ...
  describe('Open audio dialog and click cancel', () => {
    ...
    it('Inserting audio and applying heading', (done: Function) => {
      ...
      expect(
        dialogEle
          .querySelector('.e-aud-uploadwrap')
          .firstElementChild.classList.contains('e-droptext')
      ).toBe(true);
      expect(
        dialogEle
          .querySelector('.audioUrl')
          .firstElementChild.classList.contains('e-audio-url')
      ).toBe(true);
  ...
  describe('div content-rte testing', () => {
    ...
    it('audio dialog Coverage', (done: Function) => {
      ...
      let args: any = {
        item: {
          url: window.origin + '/base/spec/content/audio/RTE-Audio.mp3',
          selection: save,
        },
  ...
  describe('div content-rte testing', () => {
    ...
    it('upload the audio while use save url', (done) => {
      ...
      (dialogEle.querySelector('.e-audio-url') as HTMLInputElement).value =
        window.origin + '/base/spec/content/audio/RTE-Audio.mp3';
      (
        dialogEle.querySelector('.e-audio-url') as HTMLInputElement
  ...
  describe('audio dialog - documentClick', () => {
    ...
    it('open audio dialog - click on audio item in toolbar', () => {
      ...
      expect(
        document.body.contains((<any>rteObj).audioModule.dialogObj.element)
      ).toBe(true);

      let eventsArgs: any = {
        target: rteObj.element.querySelector('.e-audio'),
        preventDefault: function () {},
      };
      ...
      eventsArgs = {
        target: document.querySelector('[title="Insert Audio (Ctrl+Shift+A)"]'),
        preventDefault: function () {},
      };
      (<any>rteObj).audioModule.onDocumentClick(eventsArgs);
      expect(
  ...
  describe('Removing the audio', () => {
    ...
    it('audio remove with quickToolbar check', (done: Function) => {
      ...
      target.dispatchEvent(clickEvent);
      (<any>rteObj).audioModule.editAreaClickHandler({ args: eventsArg });
      (<any>rteObj).audioModule.audEle = rteObj.contentModule
        .getEditPanel()
        .querySelector('.e-audio-wrap audio');
      setTimeout(function () {
  ...
  describe('document click audio coverage', () => {
    ...
    it('document click audio coverage', (done: Function) => {
      ...
      setTimeout(function () {
        let audioBtn: HTMLElement = document.getElementById(
          (rteObj as any).element.id + '_quick_AudioReplace'
        );
  ...
  describe('Audio deleting when press backspace button', () => {
    ...
    it('Audio delete action checking using backspace key', (done: Function) => {
      ...
      expect((<any>rteObj).inputElement.querySelector('.e-audio-wrap')).toBe(
        null
      );
      done();
    });
  ...
  describe('Audio with inline applied', () => {
    ...
    it('classList testing for inline', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        let audio: HTMLElement = rteObj.element.querySelector(
          '.e-rte-audio'
        ) as HTMLElement;
    ...
    it(' Test - Replace the audio ', (done) => {
      ...
      setTimeout(() => {
        ...
        let insertButton: HTMLElement = dialog.querySelector(
          '.e-insertAudio.e-primary'
        );
        urlInput.dispatchEvent(new Event('input'));
        insertButton.click();
        let updateAudio: HTMLSourceElement = rteObj.element.querySelector(
          '.e-audio-wrap source'
        );
  ...
  describe('Disable the insert audio dialog button when the audio is uploading', () => {
    ...
    beforeEach((done: Function) => {
      rteObj = renderRTE({
        insertAudioSettings: {
          allowedTypes: ['.png'],
          saveUrl:
            'https://services.syncfusion.com/angular/development/api/FileUploader/Save',
          path: '../Audios/',
        },
        toolbarSettings: {
          items: ['Audio'],
    ...
    it(' Button disabled with improper file extension', (done) => {
      ...
      let dialogEle: Element = rteObj.element.querySelector('.e-dialog');
      (dialogEle.querySelector('.e-audio-url') as HTMLInputElement).value =
        '/base/spec/content/audio/RTE-Audio.mp34';
      let fileObj: File = new File(['Horse'], 'horse.mp34', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
      ...
      setTimeout(() => {
        expect(
          (
            dialogEle.querySelector('.e-insertAudio') as HTMLButtonElement
  ...
  describe('Getting error while insert the audio after applied the  lower case or  upper case commands in Html Editor  - ', () => {
    ...
    beforeEach((done: Function) => {
      rteObj = renderRTE({
        ...
        toolbarSettings: {
          items: ['LowerCase', 'UpperCase', '|', 'Audio'],
        },
      });
      controlId = rteObj.element.id;
      done();
    ...
    it(' Apply uppercase and then insert an audio  ', (done) => {
      ...
      setTimeout(() => {
        ...
        (
          document.querySelector('.e-insertAudio.e-primary') as HTMLElement
        ).click();
  ...
  describe('Inserting Audio as Base64 - ', () => {
    ...
    it(' Test the inserted audio in the component ', (done) => {
      ...
      let fileObj: File = new File(['Horse'], 'horse.mp3', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
      ...
      setTimeout(() => {
        expect(
          rteObj
            .getContent()
            .querySelector('.e-rte-audio.e-audio-inline source')
            .getAttribute('src')
            .indexOf('blob') == -1
        ).toBe(true);
  ...
  describe('Insert audio mediaRemoving event - ', () => {
    ...
    it(' Test the component insert audio events - case 2 ', (done) => {
      ...
      setTimeout(() => {
        evnArg.selectNode = [rteObj.element];
        (<any>rteObj).audioModule.deleteAudio(evnArg);
        (<any>rteObj).audioModule.uploadObj.upload(
          (<any>rteObj).audioModule.uploadObj.filesData[0]
        );
        done();
      }, 100);
    });
  ...
  describe('Testing allowed extension in audio upload - ', () => {
    ...
    it(' Test the component insert audio with allowedExtension property', (done) => {
      ...
      (dialogEle.querySelector('.e-audio-url') as HTMLInputElement).value =
        window.origin + '/base/spec/content/audio/RTE-Audio.mp3';
      let fileObj: File = new File(['Horse'], 'horse.m4a', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
  ...
  describe('beforeMediaUpload event - ', () => {
    ...
    it(' Event and arguments test ', (done) => {
      ...
      (dialogEle.querySelector('.e-audio-url') as HTMLInputElement).value =
        window.origin + '/base/spec/content/audio/RTE-Audio.mp3';
      let fileObj: File = new File(['Header'], 'horse.mp3', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
  ...
  describe('Audio outline style testing, while focus other content or audio', () => {
    ...
    it('first audio click with focus testing', (done) => {
      ...
      dispatchEvent(
        rteObj.element.querySelectorAll(
          '.e-content .e-clickelem'
        )[0] as HTMLElement,
        'mouseup'
      );
      let eventsArgs: any = {
        target: rteObj.element.querySelectorAll('.e-audio-wrap audio')[0],
        preventDefault: function () {},
      };
      (<any>rteObj).audioModule.onDocumentClick(eventsArgs);
      (<any>rteObj).audioModule.prevSelectedAudioEle =
        rteObj.element.querySelectorAll('.e-audio-wrap audio')[0];
      setTimeout(() => {
        expect(
          (
            rteObj.element.querySelectorAll(
              '.e-content .e-audio-wrap audio'
            )[0] as HTMLElement
    ...
    it('second audio click with focus testing', (done) => {
      ...
      dispatchEvent(
        rteObj.element.querySelectorAll(
          '.e-content .e-audio-wrap'
        )[1] as HTMLElement,
        'mouseup'
      );
      let eventsArgs: any = {
        target: rteObj.element.querySelectorAll('.e-audio-wrap audio')[1],
        preventDefault: function () {},
      };
      (<any>rteObj).audioModule.onDocumentClick(eventsArgs);
      setTimeout(() => {
  ...
  describe('ShowDialog, CloseDialog method testing', () => {
    ...
    it('show/hide dialog testing', (done) => {
      ...
      setTimeout(() => {
        expect(
          document.body.querySelectorAll('.e-rte-audio-dialog.e-dialog').length
        ).toBe(1);
        rteObj.closeDialog(DialogType.InsertAudio);
        setTimeout(() => {
          expect(
            document.body.querySelectorAll('.e-rte-audio-dialog.e-dialog')
              .length
          ).toBe(0);
          done();
        }, 100);
  ...
  describe('836851 - check auido remove', () => {
    ...
    it('Remove audio using quick toolbar', (done: Function) => {
      ...
      expect(
        !isNullOrUndefined(
          document.querySelector('.e-audio-wrap') as HTMLElement
        )
      ).toBe(true);
      expect(
        !isNullOrUndefined(
  ...
  describe('Audio dialog - Short cut key', () => {
    ...
    it('close audio dialog - escape', (done: Function) => {
      ...
      expect(isNullOrUndefined((<any>rteObj).audioModule.dialogObj)).toBe(
        false
      );
      (<any>rteObj).audioModule.onKeyDown({ args: keyboardEventArgs });
  ...
  describe('836851 - Remove the audio using audio quick toolbar ', () => {
    ...
    it('Remove the audio using  audio quick toolbar ', (done: Function) => {
      ...
      (<any>rteObj).audioModule.editAreaClickHandler({ args: clickEvent });
      expect(
        !isNullOrUndefined(rteEle.querySelector('.e-rte-audio') as HTMLElement)
      ).toBe(true);
      expect(
        !isNullOrUndefined(
          document.querySelector('.e-rte-quick-popup') as HTMLElement
  ...
  describe(' Mobile audio interaction', () => {
    ...
    it('check the audio click', (done: Function) => {
      ...
      dispatchEvent(
        rteObj.element.querySelector('.e-clickelem') as HTMLElement,
        'mouseup'
      );
      let eventsArgs: any = {
        target: rteObj.element.querySelector('.e-clickelem') as HTMLElement,
        preventDefault: function () {},
      };
      (<any>rteObj).audioModule.audioClick(eventsArgs);
      expect(
        !isNullOrUndefined(
          rteObj.contentModule.getEditPanel().querySelector('.e-rte-audio')
        )
      ).toBe(true);
      done();
    });
  ...
  describe('836851 - Insert audio', function () {
    ...
    it('Close the dialog while audio insert', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          !isNullOrUndefined(document.querySelector('.e-rte-audio-dialog'))
        ).toBe(true);
  ...
  describe('836851 - Check the insert button - without input URL', function () {
    ...
    it('Check the insert button - without input URL', (done: Function) => {
      ...
      let dialogEle: any = rteObj.element.querySelector('.e-dialog');
      (
        dialogEle.querySelector('.audioUrl .e-input.e-audio-url') as HTMLElement
      ).click();
      (dialogEle.querySelector('.e-audio-url') as HTMLInputElement).value =
        window.origin + '/base/spec/content/audio/RTE-Audio.mp3';
      (dialogEle.querySelector('.e-audio-url') as HTMLElement).dispatchEvent(
        new Event('input')
      );
      (dialogEle.querySelector('.e-audio-url') as HTMLInputElement).value = '';
      (dialogEle.querySelector('.e-audio-url') as HTMLElement).dispatchEvent(
      ...
      expect(!isNullOrUndefined(document.querySelector('.e-audio-wrap'))).toBe(
        true
      );
      done();
    });
  ...
  describe('836851 - insertAudioUrl', function () {
    ...
    it('Check the insertAudioUrl', (done: Function) => {
      (<any>QTBarModule).renderQuickToolbars(rteObj.audioModule);
      (<any>rteObj).audioModule.uploadUrl = {
      ...
      (dialogEle.querySelector('.e-audio-url') as HTMLInputElement).value =
        window.origin + '/base/spec/content/audio/horse.mp3';
      (dialogEle.querySelector('.e-audio-url') as HTMLElement).dispatchEvent(
        new Event('input')
      );
      (
      ...
      expect(!isNullOrUndefined(document.querySelector('.e-rte-audio'))).toBe(
        true
      );
      done();
    });
  ...
  describe('836851 - Audio keyup', function () {
    ...
    it('check the audio keyup - backspace', function (done) {
      ...
      expect(
        !isNullOrUndefined(rteObj.element.querySelector('.e-audio-wrap'))
      ).toBe(true);
      done();
    });
  });
  ...
  describe('859306 - Opus and M4a type audios are not supported in RichTextEditor', () => {
    ...
    it('Checking the type of the audio', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        (<any>rteObj).audioModule.alignmentSelect(mouseEventArgs);
        let sourceElement = rteObj.contentModule
          .getEditPanel()
          .querySelector('.e-audio-wrap audio source');
  ...
  describe('837380: The web url is empty when trying to edit after being inserted into the Rich Text Editor', function () {
    ...
    beforeEach(function (done) {
      rteObj = renderRTE({
        value:
          '<p><span class="e-audio-wrap" contenteditable="false" title="horse.mp3"><span class="e-clickElem"><audio class="e-rte-audio e-audio-inline" controls=""><source src="/base/spec/content/audio/RTE-Audio.mp3" type="audio/mp3"></audio></span></span><br></p>',
      });
      controlId = rteObj.element.id;
      done();
    });
