...
describe('Image Module', () => {
  describe(' Quick Toolbar open testing after selecting some text', () => {
    ...
    it(' selecting some text and then clicking on image test ', () => {
      rteObj = renderRTE({
        ...
        value:
          "<div id='rte'><p><b>Syncfusion</b> Software</p>" +
          "<img id='imgTag' style='width: 200px' alt='Logo'" +
  ...
  describe('div content-rte testing', () => {
    ...
    it('image dialog', () => {
      ...
      expect(
        dialogEle.firstElementChild.querySelector('.e-dlg-header').innerHTML ===
          'Insert Image'
      ).toBe(true);
      expect(
        dialogEle
          .querySelector('.e-img-uploadwrap')
          .firstElementChild.classList.contains('e-droptext')
      ).toBe(true);
      expect(
        dialogEle
          .querySelector('.imgUrl')
          .firstElementChild.classList.contains('e-img-url')
      ).toBe(true);
      (
        (<HTMLElement>(
          rteEle.querySelectorAll('.e-toolbar-item')[0]
      ...
      let args: any = {
        item: {
          url: 'https://js.syncfusion.com/demos/web/content/images/accordion/baked-chicken-and-cheese.png',
          selection: save,
        },
      ...
      expect(
        (<any>rteObj).contentModule.getEditPanel().querySelector('a')
      ).not.toBe(null);
      args.item = {
        url: 'https://www.syncfusion.com',
        target: '_blank',
      ...
      args.item = {
        url: 'https://www.syncfusion.com',
        target: '_blank',
        insertElement: rteObj.element.querySelector(
          '.e-rte-image'
        ) as HTMLElement,
        selectParent: [rteObj.element.querySelector('a') as HTMLElement],
      };
    ...
    it('image dialog Coverage', (done: Function) => {
      (rteObj.value = '<p id="contentId">hello  </p>'), rteObj.dataBind();
      let pTag: HTMLElement = rteObj.element.querySelector(
        '#contentId'
      ) as HTMLElement;
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
  ...
  describe('div content', () => {
    ...
    it('mobile UI', () => {
      ...
      expect(
        rteObj.element.lastElementChild.classList.contains('e-dlg-container')
      ).toBe(false);
      (
        (<HTMLElement>(
          rteEle.querySelectorAll('.e-toolbar-item')[0]
  ...
  describe('image resize', () => {
    ...
    beforeAll(() => {
      rteObj = renderRTE({
        ...
        actionBegin: function (e: any) {
          ...
          if (e.item.subCommand === 'Caption') {
            expect(e.item.subCommand === 'Caption').toBe(true);
          }
          if (e.item.subCommand === 'JustifyLeft') {
            expect(e.item.subCommand === 'JustifyLeft').toBe(true);
          }
          if (e.item.subCommand === 'JustifyRight') {
            expect(e.item.subCommand === 'JustifyRight').toBe(true);
          }
          if (e.item.subCommand === 'JustifyCenter') {
            expect(e.item.subCommand === 'JustifyCenter').toBe(true);
          }
          if (e.item.subCommand === 'Inline') {
            expect(e.item.subCommand === 'Inline').toBe(true);
          }
          if (e.item.subCommand === 'Break') {
            expect(e.item.subCommand === 'Break').toBe(true);
          }
        },
        actionComplete: function (e: any) {
          ...
          if (e.requestType === 'Caption') {
            expect(e.requestType === 'Caption').toBe(true);
          }
        },
      });
      rteEle = rteObj.element;
    ...
    it('image dialog', () => {
      ...
      let args: any = {
        preventDefault: function () {},
        originalEvent: {
          currentTarget: document.getElementById('rte_toolbarItems'),
        },
      };
      let range: any = new NodeSelection().getRange(document);
      let save: any = new NodeSelection().save(range, document);
      ...
      let placeHolder: HTMLElement = rteObj.element.querySelectorAll(
        '.rte-placeholder'
      )[0] as HTMLElement;
    ...
    it('resize end', () => {
      let resizeBot: HTMLElement = rteObj.contentModule
        .getEditPanel()
        .querySelector('.e-rte-botRight') as HTMLElement;
      let trg = rteObj.element.querySelector('.e-rte-image') as HTMLElement;
      ...
      expect(
        rteObj.contentModule.getDocument().body.contains(imgResizeDiv)
      ).toBe(false);
    });
    it('resizing - mousemove - bottom right', () => {
      ...
      let width = (rteObj.element.querySelector('.e-rte-image') as HTMLElement)
        .offsetWidth;
      (<any>rteObj.imageModule).resizeBtnStat.botRight = true;
    ...
    it('resizing - mousemove - top right', () => {
      ...
      let resizeBot: HTMLElement = rteObj.contentModule
        .getEditPanel()
        .querySelector('.e-rte-topRight') as HTMLElement;
      clickEvent = document.createEvent('MouseEvents');
      clickEvent.initEvent('mousedown', false, true);
      resizeBot.dispatchEvent(clickEvent);
      (rteObj.imageModule as any).resizeStart(clickEvent);
    ...
    it('resizing - mousemove - top Left', () => {
      ...
      let resizeBot: HTMLElement = rteObj.contentModule
        .getEditPanel()
        .querySelector('.e-rte-topLeft') as HTMLElement;
      clickEvent = document.createEvent('MouseEvents');
      clickEvent.initEvent('mousedown', false, true);
      resizeBot.dispatchEvent(clickEvent);
      (rteObj.imageModule as any).resizeStart(clickEvent);
    ...
    it('resizing - mousemove - bottom Left', () => {
      ...
      let resizeBot: HTMLElement = rteObj.contentModule
        .getEditPanel()
        .querySelector('.e-rte-botLeft') as HTMLElement;
      clickEvent = document.createEvent('MouseEvents');
      clickEvent.initEvent('mousedown', false, true);
      resizeBot.dispatchEvent(clickEvent);
      (rteObj.imageModule as any).resizeStart(clickEvent);
    ...
    it('resizing - mousemove - bottom right', (done) => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          //expect(width).toEqual((rteObj.element.querySelector('.e-rte-image') as HTMLElement).offsetWidth);
          (<any>rteObj.imageModule).resizeBtnStat.botRight = true;
          (rteObj.imageModule as any).resizing({
            target: resizeBot,
            pageX: 100,
          });
          setTimeout(() => {
...
`;
    ...
    it('resizing', () => {
      ...
      (<any>rteObj.imageModule).resizeBtnStat.botRight = true;
      (rteObj.imageModule as any).resizing({ target: resizeBot, pageX: 200 });
      let width = (rteObj.element.querySelector('img') as HTMLElement)
        .offsetWidth;
      (<any>rteObj.imageModule).resizeBtnStat.botRight = true;
      (rteObj.imageModule as any).resizing({ target: resizeBot, pageX: 300 });
      width += 100;
      expect(width).toEqual(
        (rteObj.element.querySelector('img') as HTMLElement).offsetWidth + 100
      );
      (<any>rteObj.imageModule).resizeBtnStat.botRight = true;
      (rteObj.imageModule as any).resizing({ target: resizeBot, pageX: 100 });
      width -= 200;
      ...
      trg.dispatchEvent(clickEvent);
      (rteObj.imageModule as any).resizeStart(clickEvent);
      resizeBot = rteObj.contentModule
        .getEditPanel()
        .querySelector('.e-rte-botRight') as HTMLElement;
      clickEvent = document.createEvent('MouseEvents');
      clickEvent.initEvent('mousedown', false, true);
  ...
  describe('mobile resize', () => {
    ...
    it('mobile UI', () => {
      ...
      let dialogEle: any = rteObj.contentModule
        .getDocument()
        .body.querySelector('.e-dialog');
      (dialogEle.querySelector('.e-img-url') as HTMLInputElement).value =
        'https://js.syncfusion.com/demos/web/content/images/accordion/baked-chicken-and-cheese.png';
      (dialogEle.querySelector('.e-img-url') as HTMLInputElement).dispatchEvent(
        new Event('input')
      ...
      (rteObj.imageModule as any).resizeStart({
        target: resizeBot,
        preventDefault: function () {},
      ...
      (rteObj.imageModule as any).onDocumentClick({
  ...
  describe(' Mobile image interaction', () => {
    ...
    it(' contenteditable set as false while click on image to close the virtual keyboard', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        (<any>rteObj).clickPoints = { clientY: 0, clientX: 0 };
        dispatchEvent(
          rteObj.element.querySelector('.testNode') as HTMLElement,
          'mouseup'
        );
        let eventsArgs: any = {
          target: rteObj.element.querySelector('.testNode') as HTMLElement,
          preventDefault: function () {},
        };
        (<any>rteObj).imageModule.imageClick(eventsArgs);
        setTimeout(() => {
  ...
  describe('Inserting image and applying heading', () => {
    ...
    it('Inserting image and applying heading', () => {
      ...
      (<any>rteObj).formatter.editorManager.imgObj.createImage(args);
      (
        rteObj.element.querySelector('.e-rte-dropdown-btn') as HTMLElement
      ).click();
  ...
  describe('Link with image', function () {
    ...
    beforeAll(function () {
      rteObj = renderRTE({
        ...
        value:
          '<a class="e-rte-anchor" href="http://adadas">syncfu<img class="e-rte-image e-imginline e-resize" alt="image" style="">sion</a>',
        insertImageSettings: { resize: false },
      });
      rteEle = rteObj.element;
    });
  ...
  describe('div content-rte testing', () => {
    ...
    it('insert image url', () => {
      ...
      (<any>rteObj).imageModule.editAreaClickHandler({ args: eventsArg });
      let linkPop: HTMLElement = <HTMLElement>(
        document.querySelectorAll('.e-rte-quick-popup')[0]
      );
      ...
      expect(
        (<any>rteObj).contentModule
          .getEditPanel()
          .querySelector('span.e-img-caption')
      ).not.toBe(null);
      expect(
        (<any>rteObj).contentModule
          .getEditPanel()
          .querySelector('span.e-rte-img-caption')
      ).not.toBe(null);
      expect(
        (<any>rteObj).contentModule
          .getEditPanel()
          .querySelector('span.e-img-wrap')
      ).not.toBe(null);
      expect(
        (<any>rteObj).imageModule.captionEle
          .querySelector('img')
          .classList.contains('e-rte-image')
      ).toBe(true);
      rteObj.formatter.editorManager.nodeSelection.setSelectionNode(
        document,
        rteObj.element.querySelector('.e-rte-image')
      );
      trget.dispatchEvent(clickEvent);
      (<any>rteObj).imageModule.editAreaClickHandler({ args: eventsArg });
      let imagePop: HTMLElement = <HTMLElement>(
        document.querySelectorAll('.e-rte-quick-popup')[0]
      );
      ...
      let captionEle: HTMLElement = trget.querySelector(
        '.e-img-caption'
      ) as HTMLElement;
      expect(
        imagePop.offsetTop > captionEle.offsetTop + captionEle.offsetHeight
      ).toBe(true);
      (rteObj.element.querySelector('.e-rte-image') as HTMLElement).focus();
      ...
      expect(
        (<any>rteObj).element
          .querySelector('.e-rte-image')
          .classList.contains('e-imgleft')
      ).toBe(true);
      ...
      evnArg.selectNode = [(<any>rteObj).element.querySelector('.e-rte-image')];
      (<any>rteObj).imageModule.imageSize(evnArg);
      (<any>rteObj).imageModule.dialogObj.element.querySelector(
        '.e-input.e-img-width'
      ).value = 180;
      (<any>rteObj).imageModule.dialogObj.element.querySelector(
        '.e-input.e-img-height'
      ).value = 180;
      (<any>rteObj).imageModule.dialogObj.element
        .querySelector('.e-update-size')
        .click();
      expect((<any>rteObj).element.querySelector('.e-rte-image').width).toBe(
        180
      );
      expect((<any>rteObj).element.querySelector('.e-rte-image').height).toBe(
        180
      );
      let eventsArgs: any = {
        target: rteObj.element.querySelector('.e-rte-image') as HTMLElement,
      ...
      eventsArgs = {
        target: rteObj.element as HTMLElement,
        preventDefault: function () {},
      };
      rteObj.formatter.editorManager.nodeSelection.setSelectionNode(
        document,
    ...
    it('image insert link', (done) => {
      ...
      let evnArg: any = {
        args,
        selfImage: (<any>rteObj).imageModule,
        selection: save,
        selectNode: [(<any>rteObj).element.querySelector('.e-rte-image')],
        link: null,
        target: '',
      };
      evnArg.args.item = { command: 'Images', subCommand: 'JustifyLeft' };
      ...
      expect(
        (<any>rteObj).element
          .querySelector('.e-rte-image')
          .classList.contains('e-imginline')
      ).not.toBe(true);
      evnArg.args.item = { command: 'Images', subCommand: 'Inline' };
      (<any>rteObj).imageModule.inline(evnArg);
      expect(
      ...
      expect(
        (<any>rteObj).imageModule.dialogObj.element.classList.contains(
          'e-rte-img-dialog'
        )
      ).toBe(true);
      expect(
      ...
      setTimeout(() => {
        ...
        (<any>rteObj).imageModule.dialogObj.element.querySelector(
          '.e-input.e-img-link'
        ).value = 'http://www.goole.com';
        (<any>rteObj).imageModule.dialogObj.element
          .querySelector('.e-update-link')
          .click();
        (<any>rteObj).imageModule.insertImgLink(evnArg);
        (<any>rteObj).imageModule.dialogObj.element
          .querySelector('.e-update-link')
        ...
        evnArg.link = (<any>rteObj).imageModule.dialogObj.element.querySelector(
          '.e-img-linkwrap .e-img-link'
        ) as HTMLInputElement;
        evnArg.target = '';
        evnArg.args.item = { command: 'Images', subCommand: 'insertlink' };
        ...
        expect(
          (<any>rteObj).element
            .querySelector('.e-rte-image')
            .parentElement.classList.contains('e-imgleft')
        ).toBe(true);
        evnArg.args.item = { command: 'Images', subCommand: 'JustifyRight' };
        (<any>rteObj).imageModule.alignImage(evnArg, 'JustifyRight');
        expect(
          (<any>rteObj).element
            .querySelector('.e-rte-image')
            .parentElement.classList.contains('e-imgright')
        ).toBe(true);
        evnArg.args.item = { command: 'Images', subCommand: 'JustifyCenter' };
        (<any>rteObj).imageModule.alignImage(evnArg, 'JustifyCenter');
        expect(
    ...
    it('insert image upload', (done) => {
      ...
      let fileObj: File = new File(['Nice One'], 'sample.png', {
        lastModified: 0,
      ...
      (<any>rteObj).imageModule.uploadObj.upload(
        (<any>rteObj).imageModule.uploadObj.filesData[0]
      );
    ...
    it('image alternative text', () => {
      ...
      (
        (<any>rteObj).imageModule.dialogObj.element.querySelector(
          '.e-img-altwrap .e-img-alt'
        ) as HTMLInputElement
      ).value = 'image';
      (evnArg.alt as any) = (<any>(
        rteObj
      )).imageModule.dialogObj.element.querySelector(
        '.e-img-altwrap .e-img-alt'
      ) as HTMLInputElement;
      evnArg.selectNode = [
        rteObj.element.querySelector('.e-rte-image') as HTMLElement,
      ...
      (<any>rteObj).imageModule.insertAlt(evnArg);
      expect(
        (rteObj.element.querySelector('.e-rte-image') as HTMLImageElement)
          .alt === 'image'
      ).toBe(true);
    });
  });
  describe('div content-rte testing', () => {
    ...
    beforeAll(() => {
      rteObj = renderRTE({
        ...
        insertImageSettings: {
          ...
          resize: false,
          saveUrl:
            'http://aspnetmvc.syncfusion.com/services/api/uploadbox/Save',
    ...
    it('upload the image while use save url', (done) => {
      ...
      setTimeout(() => {
        ...
        expect((<any>rteObj).imageModule.uploadObj.fileList.length).toEqual(1);
        (
          document.getElementsByClassName('e-browsebtn')[0] as HTMLElement
        ).click();
        done();
      }, 100);
    });
  ...
  describe('image dialog - Short cut key', () => {
    ...
    it('close image dialog - escape', () => {
      ...
      expect(isNullOrUndefined((<any>rteObj).imageModule.dialogObj)).toBe(
        false
      );
    ...
    beforeAll(function () {
      rteObj = renderRTE({
        ...
        dialogOpen: function (e) {
          expect(
            (e as any).element.querySelector('.e-upload.e-control-wrapper')
          ).not.toBe(null);
        },
      });
      rteEle = rteObj.element;
  ...
  describe('image dialog - documentClick', () => {
    ...
    it('open image dialog - click on image item in toolbar', () => {
      ...
      (<any>rteObj).imageModule.onKeyDown({ args: keyboardEventArgs });
      expect(
        document.body.contains((<any>rteObj).imageModule.dialogObj.element)
      ).toBe(true);

      let eventsArgs: any = {
        target: rteObj.element.querySelector('.e-image'),
        preventDefault: function () {},
      };
      (<any>rteObj).imageModule.onDocumentClick(eventsArgs);
      expect(
      ...
      eventsArgs = {
        target: document.querySelector('[title="Insert Image (Ctrl+Shift+I)"]'),
        preventDefault: function () {},
      };
      (<any>rteObj).imageModule.onDocumentClick(eventsArgs);
      expect(
  ...
  describe('image edit', () => {
    ...
    it('edit image', (done) => {
      ...
      setTimeout(() => {
        ...
        var args = {
          item: {
            url: 'https://gitlab.syncfusion.com/uploads/-/system/appearance/header_logo/1/Syncfusion_logo_plain.jpg',
            selection: save,
            selectParent: [
              rteObj.element.querySelector('.e-rte-image') as HTMLElement,
            ],
        ...
        expect(
          (rteObj.element.querySelector('.e-rte-image') as HTMLImageElement)
            .src ===
            'https://gitlab.syncfusion.com/uploads/-/system/appearance/header_logo/1/Syncfusion_logo_plain.jpg'
        ).toBe(true);
        (rteObj.element.querySelector('.e-rte-image') as HTMLElement).click();
        (<any>rteObj).clickPoints = { clientY: 0, clientX: 0 };
        dispatchEvent(
        ...
        setTimeout(() => {
          ...
          (
            (<HTMLElement>imgTBItems.item(1)).firstElementChild as HTMLElement
          ).click();
  ...
  describe('image with cimbination', () => {
    ...
    it('quicktoobar actions', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        (document.querySelector('.e-img-link') as any).value =
  ...
  describe('initial load image undo redo', () => {
    ...
    it('link element check', (done: Function) => {
      ...
      setTimeout(function () {
        expect(rteObj.contentModule.getEditPanel().querySelector('a')).toBe(
          null
        );
        done();
      }, 100);
  ...
  describe('EJ2-53661- Image is not deleted when press backspace and delete button', () => {
    ...
    it('Checking image delete action being removed with no removeUrl configured', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        quickTBItem.item(3).click();
        expect(
          rteObj.contentModule.getEditPanel().querySelector('.e-rte-image')
        ).toBe(null);
        done();
      }, 200);
    });
  ...
  describe('872200 - Image delete using quick toolbar with caption', () => {
    ...
    it('Image delete using quick toolbar with caption with content in the left', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        expect(
          rteObj.getRange().startContainer.textContent === `RTE content `
        ).toBe(true);
  ...
  describe('EJ2-53661- Image is not deleted when press backspace and delete button', () => {
    ...
    it('Image delete action checking using backspace key', (done: Function) => {
      ...
      expect((<any>rteObj).inputElement.querySelector('.e-img-caption')).toBe(
        null
      );
      done();
    });
  ...
  describe('EJ2-56517- Image with caption is not deleted when press backspace button', () => {
    ...
    it('Image delete action checking using backspace key', (done: Function) => {
      expect((<any>rteObj).inputElement.querySelector('.e-imgbreak')).not.toBe(
        null
      );
      rteObj.formatter.editorManager.nodeSelection.setSelectionNode(
        document,
        (rteObj as any).inputElement.childNodes[0]
      );
      keyBoardEvent.code = 'Backspace';
      (rteObj as any).keyDown(keyBoardEvent);
      expect((<any>rteObj).inputElement.querySelector('.e-imgbreak')).toBe(
        null
      );
      done();
    });
  ...
  describe('EJ2-56517- Image with caption is not deleted when pressing delete button', () => {
    ...
    it('Image delete action checking using delete key', (done: Function) => {
      ...
      keyBoardEvent.action = 'delete';
      (rteObj as any).keyDown(keyBoardEvent);
      expect((<any>rteObj).inputElement.querySelector('.e-img-imgbreak')).toBe(
        null
      );
      done();
    });
  ...
  describe('Mouse Click for image testing when showOnRightClick enabled', () => {
    ...
    it(' Test - for mouse click to focus image element', (done) => {
      ...
      (<any>rteObj).imageModule.editAreaClickHandler({ args: eventsArg });
      setTimeout(() => {
        let expectElem: HTMLElement[] = (
          rteObj as any
        ).formatter.editorManager.nodeSelection.getSelectedNodes(document);
  ...
  describe(' quickToolbarSettings property - image quick toolbar - ', () => {
    ...
    it(' Test - Replace the image ', (done) => {
      ...
      setTimeout(() => {
        let imageBtn: HTMLElement = document.getElementById(
          controlId + '_quick_Replace'
        );
        ...
        urlInput.value = png;
        let insertButton: HTMLElement = dialog.querySelector(
          '.e-insertImage.e-primary'
        );
        urlInput.dispatchEvent(new Event('input'));
        insertButton.click();
        let updateImage: HTMLImageElement =
          rteObj.element.querySelector('#image');
  ...
  describe('EJ2-37798 - Disable the insert image dialog button when the image is uploading', () => {
    ...
    it(' Button disabled with improper file extension', (done) => {
      ...
      (
        (<HTMLElement>(
          rteEle.querySelectorAll('.e-toolbar-item button')[9]
        )) as HTMLElement
      ).click();
      let dialogEle: Element = rteObj.element.querySelector('.e-dialog');
      (dialogEle.querySelector('.e-img-url') as HTMLInputElement).value =
      ...
      setTimeout(() => {
        expect(
          (
            dialogEle.querySelector('.e-insertImage') as HTMLButtonElement
  ...
  describe(' EJ2-20297: RTE Image insert link  - ', () => {
    ...
    it(" Test - don't set the target as _blank while insert link with disable the new window option ", (done) => {
      ...
      setTimeout(() => {
        let imageBtn: HTMLElement = document.getElementById(
          controlId + '_quick_InsertLink'
        );
        imageBtn.parentElement.click();
        let dialog: HTMLElement = document.getElementById(controlId + '_image');
        let urlInput: HTMLInputElement = dialog.querySelector(
          '.e-input.e-img-link'
        );
        urlInput.value = 'http://www.google.com';
        let checkboxEle: HTMLInputElement = dialog.querySelector(
          '.e-checkbox-wrapper label'
        );
        checkboxEle.click();
        setTimeout(() => {
          let insertButton: HTMLElement = dialog.querySelector(
            '.e-update-link.e-primary'
          );
          insertButton.click();
          let updateImage: HTMLImageElement =
            rteObj.element.querySelector('#image');
    ...
    it(" Test - don't set the target as _blank while updating the image link with disable the new window option ", (done) => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          let imageBtn: HTMLElement = document.getElementById(
            controlId + '_quick_EditImageLink'
          );
          imageBtn.parentElement.click();
          ...
          setTimeout(() => {
            ...
            let updateImage: HTMLImageElement =
              rteObj.element.querySelector('#image');
            expect(updateImage.parentElement.hasAttribute('target')).toBe(
              false
            );
            done();
          }, 100);
  ...
  describe(' EJ2-23213: Getting error while insert the image after applied the  lower case or  upper case commands in Html Editor  - ', () => {
    ...
    beforeEach((done: Function) => {
      rteObj = renderRTE({
        ...
        toolbarSettings: {
          items: ['LowerCase', 'UpperCase', '|', 'Image'],
        },
      });
      controlId = rteObj.element.id;
      done();
    ...
    it(' Apply uppercase and then insert an image  ', (done) => {
      ...
      setTimeout(() => {
        ...
        (dialogEle.querySelector('.e-img-url') as HTMLInputElement).value =
          'https://js.syncfusion.com/demos/web/content/images/accordion/baked-chicken-and-cheese.png';
        (
          dialogEle.querySelector('.e-img-url') as HTMLInputElement
  ...
  describe(' Quick Toolbar showOnRightClick property testing', () => {
    ...
    it(" Android - 'rightClick' with quickpopup availability testing", (done: Function) => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionNode(
        document,
        rteObj.element.querySelector('#img-container img')
      );
    ...
    it(' Test the inserted image in the component ', (done) => {
      ...
      setTimeout(() => {
        expect(
          rteObj
            .getContent()
            .querySelector('.e-rte-image.e-imginline')
            .getAttribute('src')
            .indexOf('blob') == -1
        ).toBe(true);
        evnArg.selectNode = [rteObj.element];
  ...
  describe('Insert image imageSelected, imageUploading and imageUploadSuccess event - ', () => {
    ...
    let imageUploadSuccessSpy: jasmine.Spy = jasmine.createSpy(
        ...
        setTimeout(() => {
          ...
          const mockEvent = {
            ...
            file: {
              ...
              rawFile: new File(['Dummy content'], 'Sampleimage.png', {
                type: 'image/png',
              }),
              size: 395962,
  ...
  describe('Insert image imageRemoving event - ', () => {
    ...
    it(' Test the component insert image events - case 2 ', (done) => {
      ...
      (
        document.querySelector('.e-icons.e-file-remove-btn') as HTMLElement
      ).click();
  ...
  describe('Insert image imageUploadFailed event - ', () => {
    let rteObj: RichTextEditor;
    let imageUploadFailedSpy: jasmine.Spy = jasmine.createSpy(
      'onImageUploadFailed'
    );
    beforeEach((done: Function) => {
      rteObj = renderRTE({
  ...
  describe(' Caption image with link insert testing', () => {
    ...
    beforeAll(() => {
      rteObj = renderRTE({
        ...
        insertImageSettings: { resize: false },
        value:
          "<p>Test</p><img id='rteImg' src='https://ej2.syncfusion.com/demos/src/rich-text-editor/images/RTEImage-Feather.png' style='width:300px; height: 200px'/>",
      });
    });
    afterAll(() => {
      destroy(rteObj);
    ...
    it(' insert/remove link', (done: Function) => {
      ...
      dispatchEvent(
        rteObj.element.querySelector('#rteImg') as HTMLElement,
        'mouseup'
      );
      rteObj.formatter.editorManager.nodeSelection.setSelectionNode(
        document,
        rteObj.element.querySelector('#rteImg')
      );
      setTimeout(() => {
        (
          document.querySelectorAll(
            '.e-rte-image-popup .e-toolbar-item button'
          )[2] as HTMLElement
        ).click();
        rteObj.formatter.editorManager.nodeSelection.setSelectionNode(
          document,
        ...
        setTimeout(() => {
          ...
          (document.querySelector('.e-img-link') as HTMLInputElement).value =
          ...
          expect(
            imgEle.parentElement.parentElement.classList.contains('e-img-wrap')
          ).toBe(true);
          expect(
            imgEle.parentElement.parentElement.parentElement.classList.contains(
              'e-img-caption'
            )
          ).toBe(true);
          expect(
            document.querySelector('.e-content').childNodes.item(0).nodeName
          ).toBe('P');
          expect(
            document.querySelector('.e-content').childNodes.item(1).nodeName
          ).toBe('P');
          expect(
            document.querySelector('.e-content').childNodes[1].childNodes[0]
              .nodeName
          ).toBe('SPAN');
          expect(
            (
              document.querySelector('.e-content').childNodes[1]
                .childNodes[0] as Element
            ).classList.contains('e-img-caption')
          ).toBe(true);
          dispatchEvent(
            rteObj.element.querySelector('#rteImg') as HTMLElement,
            'mouseup'
          ...
          setTimeout(() => {
            ...
            expect(imgEle.parentElement.classList.contains('e-img-wrap')).toBe(
              true
            );
            expect(
              imgEle.parentElement.parentElement.classList.contains(
                'e-img-caption'
              )
            ).toBe(true);
            expect(
  ...
  describe('Caption image with link coverage testing', () => {
    ...
    it('Caption image with link coverage testing', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        expect(
          rteObj.element.querySelector('#rteImg').parentElement.parentElement
            .nodeName === 'SPAN'
        ).toBe(true);
        done();
      }, 100);
  ...
  describe(' EJ2-28120: IFrame - Images were not replaced when using caption to the image ', () => {
    ...
    it(' insert image & caption', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect((iframeBody.querySelector('img') as HTMLImageElement).src).toBe(
          'https://js.syncfusion.com/demos/web/content/images/accordion/baked-chicken-and-cheese.png'
        );
        (iframeBody.querySelector('img') as HTMLImageElement).style.width =
          '100px';
        (iframeBody.querySelector('img') as HTMLImageElement).style.height =
          '100px';
        (rteObj.contentModule.getEditPanel() as HTMLElement).focus();
        dispatchEvent(
          rteObj.contentModule.getEditPanel() as HTMLElement,
        ...
        dispatchEvent(
          iframeBody.querySelector('img') as HTMLElement,
          'mouseup'
        );
        setTimeout(() => {
          (
          ...
          expect(
            !isNullOrUndefined(iframeBody.querySelector('.e-img-caption'))
          ).toBe(true);
          (rteObj.contentModule.getEditPanel() as HTMLElement).focus();
          dispatchEvent(
            rteObj.contentModule.getEditPanel() as HTMLElement,
          ...
          setTimeout(() => {
            ...
            expect(
              (iframeBody.querySelector('img') as HTMLImageElement).src
            ).toBe(
              'https://ej2.syncfusion.com/demos/src/rich-text-editor/images/RTEImage-Feather.png'
            );
            done();
  ...
  describe('RTE Drag and Drop Image', () => {
    ...
    it(' Check dragstart Event', function (done: Function) {
      ...
      let event: any = {
        ...
        clientY: 294,
        target: image,
        dataTransfer: { files: [fileObj], dropEffect: '', effectAllowed: '' },
        preventDefault: function () {
          return;
        },
      };
    ...
    it(' Check uploadSuccess method', function (done: Function) {
      ...
      var popupEle = createElement('div', {
        className: 'e-rte-pop e-popup-open',
      });
      ...
      var args = {
        args: event,
        type: 'Images',
        isNotify: undefined as any,
        elements: image,
      };
      (rteObj.imageModule as any).uploadSuccess(image, event, args, {});
      setTimeout(() => {
        expect(document.querySelector('.e-rte-pop.e-popup-open')).not.toBe(
          null
        );
  ...
  describe('RTE Drag and Drop Image - Failure Test Case', () => {
    ...
    it(' Check image after drop', function (done: Function) {
      ...
      setTimeout(() => {
        ...
        expect(ele.classList.contains('e-imginline')).toBe(true);
        expect(ele.classList.contains('e-resize')).toBe(true);
        expect(document.getElementsByClassName('e-upload-files').length).toBe(
          0
        );
        done();
      }, 1000);
  ...
  describe('check resize icons - When readonly property enabled', () => {
    ...
    beforeEach((done: Function) => {
      rteObj = renderRTE({
        value:
          '<p>rich text <img alt="image.jpg" class="e-resize" style="">editor</p>',
        readonly: true,
      });
      done();
    });
    ...
    it('Check icons and quicktoolbar', (done) => {
      ...
      setTimeout(() => {
        ...
        expect(
          rteObj.element.querySelectorAll('.e-rte-quick-toolbar').length
        ).toBe(0);
        done();
      }, 100);
    });
  ...
  describe('EJ2-40774 - Deleting the image using context menu doesn’t remove the resize and borders of the image', () => {
    ...
    it('Resize element availability check', (done) => {
      dispatchEvent(
        rteObj.element.querySelector('img') as HTMLElement,
        'mousedown'
      );
      setTimeout(() => {
  ...
  describe('RTE Drag and Drop Image with paste restrictions', () => {
    ...
    beforeAll((done: Function) => {
      ...
      rteObj = new RichTextEditor({
        ...
        imageUploading: function (args) {
          expect(
            rteObj.toolbarModule.baseToolbar.toolbarObj.element.classList.contains(
              'e-overlay'
            )
          ).toBe(true);
          imgSize = size;
    ...
    it(' Check image being removed with args.cancel as true', function (done: Function) {
      ...
      setTimeout(() => {
        expect(
          (
            rteObj.inputElement.querySelectorAll('img')[0] as HTMLImageElement
          ).classList.contains('e-rte-image')
        ).toBe(false);
        done();
      }, 50);
  ...
  describe('EJ2-39317 - beforeImageUpload event - ', () => {
    let rteObj: RichTextEditor;
    let beforeImageUploadSpy: jasmine.Spy = jasmine.createSpy(
      'onBeforeImageUpload'
    );
    beforeEach((done: Function) => {
      rteObj = renderRTE({
  ...
  describe('BLAZ-5933 - Images change aspect ratio when resized to smallest possible and back larger again - ', () => {
    ...
    beforeEach((done: Function) => {
      rteObj = renderRTE({
        value:
          '<p><img id="rteImageID" style="width: 300px; height: 300px; transform: rotate(0deg);" alt="Logo" src="https://ej2.syncfusion.com/javascript/demos/src/rich-text-editor/images/RTEImage-Feather.png" class="e-rte-image e-imginline e-resize"></p>',
      });
      done();
    });
    afterEach((done: Function) => {
  ...
  describe('BLAZ-9502 - Image outline style is not removed properly, while focus other content or image', () => {
    ...
    beforeAll((done: Function) => {
      rteObj = renderRTE({
        ...
        value:
          '<p>Sample Text</p> <img alt="Logo" src="https://blazor.syncfusion.com/demos/images/rich-text-editor/rte-image-feather.png" style="width: 300px" /> <img alt="Logo" src="https://blazor.syncfusion.com/demos/images/toast/map.png" style="width: 300px" />',
      });
      done();
    });
    afterAll((done: Function) => {
    ...
    it('first image click with focus testing', (done) => {
      dispatchEvent(
        rteObj.element.querySelectorAll('.e-content img')[0] as HTMLElement,
        'mousedown'
      );
      setTimeout(() => {
        expect(
          (rteObj.element.querySelectorAll('.e-content img')[0] as HTMLElement)
            .style.outline === 'rgb(74, 144, 226) solid 2px'
        ).toBe(true);
        done();
      }, 100);
    });
    it('second image click with focus testing', (done) => {
      dispatchEvent(
        rteObj.element.querySelectorAll('.e-content img')[1] as HTMLElement,
        'mousedown'
      );
      setTimeout(() => {
        expect(
    ...
    it('first image click after p click with focus testing', (done) => {
      ...
      setTimeout(() => {
        ...
        dispatchEvent(
          rteObj.element.querySelector('.e-content p') as HTMLElement,
          'mousedown'
        );
        setTimeout(() => {
          expect(
  ...
  describe('EJ2-46971- Resize icon of the image is not positioned properly, when height is set to the Rich Text Editor', () => {
    ...
    beforeAll((done: Function) => {
      rteObj = renderRTE({
        ...
        value:
          '<p>Rich Text Editor allows to insert images from online source as well as local computer where you want to insert the image in your' +
    ...
    it('image click with focus testing before being resized', (done) => {
      ...
      expect(rteObj.element.querySelector('.e-img-resize') as HTMLElement).toBe(
        null
      );
      done();
    });
    it('image click with focus testing after being resized', (done) => {
      ...
      expect(
        rteObj.element.querySelector('.e-img-resize') as HTMLElement
      ).not.toBe(null);
      done();
    });
  });
  describe('Checking Image replace, using the Image dialog', () => {
    ...
    beforeAll(() => {
      rteObj = renderRTE({
        ...
        value:
          "<img src='https://ej2.syncfusion.com/javascript/demos/src/rich-text-editor/images/RTEImage-Feather.png'>",
      });
      rteEle = rteObj.element;
    });
    afterAll(() => {
    ...
    it('image dialog', (done) => {
      ...
      expect(
        (rteObj.contentModule.getEditPanel() as HTMLElement).querySelector(
          'img'
        )
      ).not.toBe(null);
      done();
  ...
  describe('EJ2-49981 - ShowDialog, CloseDialog method testing', () => {
    ...
    it('beforeDialogOpen event trigger testing', (done) => {
      ...
      setTimeout(() => {
        expect(
          document.body.querySelectorAll('.e-rte-img-dialog.e-dialog').length
        ).toBe(1);
  ...
  describe('EJ2-58062 - RTE Drag and Drop Image', () => {
    ...
    it('EJ2-58062 - Check insertDragImage -Internal image when File data is returned', function () {
      ...
      rteObj.focusIn();
      (rteObj.imageModule as any).insertDragImage(event);
      expect(rteObj.inputElement.querySelectorAll('img').length === 1).toBe(
        true
      );
      detach(document.getElementsByTagName('IMG')[0]);
    });
  ...
  describe('BLAZ-25362: In RTE image the image border and resize icons are unevenly aligned', () => {
    ...
    it('checking left top posistion and alignment of the resize icon', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          (rteObj.element.querySelector('.e-rte-topLeft') as HTMLElement).style
            .left
        ).toEqual('-6px');
        expect(
          (rteObj.element.querySelector('.e-rte-topLeft') as HTMLElement).style
        ...
        expect(
          (rteObj.element.querySelector('.e-rte-topRight') as HTMLElement).style
            .left
        ).toEqual('296px');
        expect(
          (rteObj.element.querySelector('.e-rte-topRight') as HTMLElement).style
        ...
        expect(
          (rteObj.element.querySelector('.e-rte-botLeft') as HTMLElement).style
            .left
        ).toEqual('-6px');
        expect(
          (rteObj.element.querySelector('.e-rte-botLeft') as HTMLElement).style
        ...
        expect(
          (rteObj.element.querySelector('.e-rte-botRight') as HTMLElement).style
            .left
        ).toEqual('296px');
        expect(
          (rteObj.element.querySelector('.e-rte-botRight') as HTMLElement).style
  ...
  describe('EJ2-66350: DisplayLayout option checking in image quicktoolbar', () => {
    ...
    it('DisplayLayout option checking in image quicktoolbar', () => {
      ...
      expect(imgPop.querySelectorAll('.e-rte-toolbar').length).toBe(1);
      (<HTMLElement>(
        document.querySelectorAll('.e-rte-dropdown-btn')[1]
      )).click();
  ...
  describe('836851 - Remove the image using image quick toolbar ', () => {
    ...
    it('Remove the image using  image quick toolbar ', (done: Function) => {
      ...
      expect(
        !isNullOrUndefined(
          document.querySelector('.e-rte-image') as HTMLElement
        )
      ).toBe(true);
      expect(
        !isNullOrUndefined(
          document.querySelector('.e-rte-quick-popup') as HTMLElement
        )
      ).toBe(true);
      let imageQTBarEle = <HTMLElement>(
        document.querySelector('.e-rte-quick-popup')
      );
      (imageQTBarEle.querySelector("[title='Remove']") as HTMLElement).click();
      expect(
        isNullOrUndefined(document.querySelector('.e-rte-image') as HTMLElement)
      ).toBe(true);
      expect(
        isNullOrUndefined(
          document.querySelector('.e-rte-quick-popup') as HTMLElement
  ...
  describe('836851 - Insert image', function () {
    ...
    it('Close the dialog while image insert', (done: Function) => {
      ...
      (<any>rteObj).imageModule.uploadUrl = {
  ...
  describe('EJ2-845426: Undo does not works properly after replace the image', () => {
    ...
    it('Check undo action after replace the image.', (done) => {
      ...
      setTimeout(() => {
        (
          document.querySelector('[title="Undo (Ctrl+Z)"]') as HTMLElement
        ).click();
        expect(
          rteEle.querySelector('img').src ==
            'http://cdn.syncfusion.com/content/images/sales/buynow/Character-opt.png'
        ).toBe(true);
        done();
      }, 100);
    });
    ...
    it('Toolbar and content-editable area', (done: Function) => {
      (
        document.querySelector(
          '.e-richtexteditor .e-toolbar-wrapper .e-expended-nav'
        ) as any
      ).click();
      var trg = (rteObj as any).element.querySelector('.e-rte-image');
      (rteObj.imageModule as any).resizeStart({ target: trg, pageX: 0 });
      var resizeBot = rteObj.contentModule
        .getEditPanel()
        .querySelector('.e-rte-botRight');
      clickEvent = document.createEvent('MouseEvents');
      clickEvent.initEvent('mousedown', false, true);
      resizeBot.dispatchEvent(clickEvent);
      (rteObj.imageModule as any).resizeStart(clickEvent);
      ...
      var toolbarEditarea =
        document
          .querySelector('.e-richtexteditor .e-toolbar-wrapper')
          .getBoundingClientRect().bottom -
          3 <
        document
          .querySelector('.e-richtexteditor .e-rte-content .e-content')
    ...
    it('checking the inserted image is aligned properly', (done) => {
      ...
      setTimeout(() => {
        ...
        expect(
          (<any>rteObj).element
            .querySelector('.e-rte-image')
            .classList.contains('e-imgcenter')
        ).toBe(true);
        done();
      }, 100);
    });
  ...
  describe('850027 - Horizontal scrollbar enabled when we resize the image in large', () => {
    ...
    it('Calculation of the image maximum width when inside the OL or UL element', (done) => {
      ...
      (rteObj.imageModule as any).resizing({ target: resizeBot, pageX: 200 });
      expect(
        parseInt(trg.style.maxWidth, 10) != rteObj.getInsertImgMaxWidth()
      ).toBe(true);
      done();
    });
  });
  ...
  describe('860425 - Image with caption not get deleted when we press delete or backspace key in RichTextEditor', () => {
    ...
    it('Should remove the image on delete key press', (done: DoneFn) => {
      ...
      rteObj.dataBind();
      rteObj.focusIn();
      const element = rteObj.inputElement.querySelector(
        '.e-img-wrap'
      ) as HTMLElement;
    ...
    it('Should remove the image on delete key press and have focus on the Paragraph', (done: DoneFn) => {
      ...
      setTimeout(() => {
        expect(rteObj.inputElement.querySelector('.e-img-caption')).toBe(null);
        expect(
          window.getSelection().getRangeAt(0).startContainer.nodeName
        ).toBe('P');
        done();
      }, 100);
    });
  ...
  describe('850034 - Content scrolls to the top, when we apply formats to the image caption', () => {
    ...
    it('The content scrolls to the top when contenteditable is set to false in the Rich Text Editor.', (done) => {
      var imageCaption = rteObj.element.querySelector(
        '.e-img-caption .e-img-inner'
      );
      (rteObj as any).formatter.editorManager.nodeSelection.setSelectionText(
        rteObj.contentModule.getDocument(),
        imageCaption,
        imageCaption,
        0,
      ...
      (rteObj as any).element
        .querySelectorAll('.e-toolbar-wrapper .e-toolbar-item')[0]
        .click();
      expect(document.activeElement.classList.contains('e-img-inner')).toBe(
        true
      );
      done();
    });
  ...
  describe('Image module code coverage', () => {
    ...
    it('image module code coverage', (done) => {
      ...
      (rteObj as any).imageModule.updateCss(myObj, {
        oldCssClass: 'imageOldClass',
        cssClass: 'imageUpdatedClass',
      });
      expect(myObj.oldCssClass === '_imageNewClass imageUpdatedClass').toBe(
        true
      );
      (rteObj as any).imageModule.updateCss(myObj, {
        oldCssClass: null,
      ...
      expect(
        myObj.oldCssClass === 'imageOldClass_imageNewClass imageUpdatedClass'
      ).toBe(true);
      (rteObj as any).imageModule.popupObj = rteObj;
      (rteObj as any).imageModule.setCssClass({
        oldCssClass: 'imageOldClass',
        cssClass: 'imageUpdatedClass',
      });
      expect(
        (rteObj as any).element.classList.contains('imageUpdatedClass')
      ).toBe(true);
      (rteObj as any).imageModule.setCssClass({
        oldCssClass: null,
        cssClass: 'imageUpdatedClassNew',
      });
      expect(
        (rteObj as any).element.classList.contains('imageUpdatedClassNew')
      ).toBe(true);
      (rteObj as any).imageModule.popupObj = null;
      let undoCount: number = (rteObj as any).formatter.getUndoRedoStack()
        .length;
      (rteObj as any).imageModule.undoStack({ subCommand: 'image' });
      expect(
        (rteObj as any).formatter.getUndoRedoStack().length === undoCount
      ).toBe(true);
      ...
      QTBarModule.imageQTBar.showPopup(
        10,
        131,
        (rteObj as any).element.querySelector('.e-rte-image')
      );
      expect(document.querySelectorAll('.e-rte-quick-popup').length).toBe(1);
      (rteObj as any).imageModule.quickToolObj = (
        rteObj as any
      ).quickToolbarModule;
      ...
      expect((rteObj as any).imageModule.imgEle.style.outline !== '').toBe(
        true
      );
  ...
  describe('872197 - Multiple anchor added to the image - ', () => {
    ...
    it('Check the insert link item added in Quick toolbar', (done) => {
      ...
      (<any>rteObj).imageModule.editAreaClickHandler({ args: eventsArg });
      let imageQTBarEle: HTMLElement = <HTMLElement>(
        document.querySelector('.e-rte-quick-popup')
      );
      let openLink: HTMLElement = imageQTBarEle.querySelector(
        "[title='Open Link']"
      ) as HTMLElement;
      setTimeout(() => {
        expect(
          (imageQTBarEle.querySelector("[title='Open Link']") as HTMLElement)
            .style.display === 'none'
        ).toBe(true);
        expect(
          (imageQTBarEle.querySelector("[title='Edit Link']") as HTMLElement)
            .style.display === 'none'
        ).toBe(true);
        expect(
          (imageQTBarEle.querySelector("[title='Remove Link']") as HTMLElement)
            .style.display === 'none'
        ).toBe(true);
        (
          imageQTBarEle.querySelector("[title='Insert Link']") as HTMLElement
        ).click();
  ...
  describe('871139 - when image removing event API is used argument is null', () => {
    ...
    it("The imageRemiving event doesn't have the args property.", (done) => {
      ...
      (
        document.querySelector(
          '.e-dlg-closeicon-btn.e-control.e-btn.e-lib.e-flat.e-icon-btn'
        ) as any
      ).click();
      setTimeout(() => {
    ...
    beforeAll((done) => {
      editor = renderRTE({
        insertImageSettings: {
          display: 'Break',
          saveUrl:
            'https://ej2services.syncfusion.com/js/development/api/RichTextEditor/SaveFile',
  ...
  describe('Insert link on image with image quick toolbar with targe set to self', () => {
    ...
    it('Should have proper target attribute.', (done: DoneFn) => {
      ...
      const imageElement: HTMLElement =
        editor.inputElement.querySelector('img');
      ...
      setTimeout(() => {
        (
          document.body
            .querySelector('.e-rte-image-popup')
            .querySelector('.e-insert-link') as HTMLElement
        ).click();
        setTimeout(() => {
          (
            document.body
              .querySelector('.e-rte-img-dialog')
              .querySelector('.e-checkbox') as HTMLElement
          ).click();
          (
            editor.element.querySelector('.e-img-link') as HTMLInputElement
          ).value =
            'https://ej2.syncfusion.com/demos/#/material/rich-text-editor/tools.html';
          (
            editor.element.querySelector('.e-img-link') as HTMLInputElement
          ).dispatchEvent(new Event('input'));
          (
            document.body
              .querySelector('.e-rte-img-dialog')
              .querySelector('.e-update-link') as HTMLElement
          ).click();
          setTimeout(() => {
            expect(
              (
    ...
    it('Should have proper target attribute. CASE 2 Edit the link', (done: DoneFn) => {
      ...
      setTimeout(() => {
        (
          document.body
            .querySelector('.e-rte-image-popup')
            .querySelector('.e-edit-link') as HTMLElement
        ).click();
        setTimeout(() => {
          expect(
            document.body
  ...
  xdescribe('Dropping images into the editor area ', () => {
      ...
      editor = renderRTE({
        insertImageSettings: {
          ...
          saveUrl:
            'https://ej2services.syncfusion.com/js/development/api/RichTextEditor/SaveFile',
          removeUrl:
            'https://ej2services.syncfusion.com/js/development/api/RichTextEditor/DeleteFile',
          path: 'https://ej2services.syncfusion.com/js/development/RichTextEditor/',
        },
        imageUploadSuccess: (e: any) => {
          if (
            editor.inputElement.querySelector('img').src === 'RTE-Landscape.png'
          ) {
            success = true;
          }
        },
    ...
    it('Should insert the image into the editor.', (done: DoneFn) => {
      ...
      fetch('/base/spec/content/image/RTE-Landscape.png')
        ...
        .then((blob) => {
          const file: File = new File([blob], 'RTE-Landscape.png', {
            type: 'image/png',
          });
          const dataTransfer: DataTransfer = new DataTransfer();
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              (
                document.body
                  .querySelector('.e-rte-image-popup')
                  .querySelector('.e-remove') as HTMLElement
              ).click();
              setTimeout(() => {
                if (removeSuccess) {
      ...
      setTimeout(() => {
        expect(
          parseInt(
            (
              document.querySelector(
                '.e-rte-image-popup.e-rte-elements.e-rte-quick-popup'
              ) as any
            ).style.left
          ) < 250
        ).toBe(true);
  ...
  describe('874686 - Image Size pop up has more empty space at below before update buttom. Can reduce the size of pop up.', () => {
    ...
    it('Check the image popup size', (done) => {
      ...
      setTimeout(() => {
        let imageBtn: HTMLElement = document.getElementById(
          controlId + '_quick_Dimension'
        );
        imageBtn.click();
        expect(
          rteObj.element
            .querySelector('.e-rte-img-size-dialog')
            .hasAttribute('height')
        ).toBe(false);
        done();
      }, 100);
  ...
  describe('Insert image with display set to break', () => {
    ...
    it('Case 1: Insert by using the image url', (done: DoneFn) => {
      ...
      setTimeout(() => {
        (editor.element.querySelector('.e-img-url') as HTMLInputElement).value =
          'https://cdn.syncfusion.com/ej2/richtexteditor-resources/RTE-Overview.png';
        editor.element
          .querySelector('.e-img-url')
          .dispatchEvent(new Event('input'));
        (
          editor.element.querySelector('.e-insertImage') as HTMLButtonElement
        ).click();
        setTimeout(() => {
          expect(
            editor.inputElement
    ...
    it('Case 2: Insert by paste action', (done: DoneFn) => {
      ...
      const clipBoardData: string =
        '<p><img alt="Sky with sun" src="https://cdn.syncfusion.com/ej2/richtexteditor-resources/RTE-Overview.png" style="width: 50%;" class="e-rte-image e-imginline" /></p>';
      const dataTransfer: DataTransfer = new DataTransfer();
      dataTransfer.setData('text/html', clipBoardData);
      const pasteEvent: ClipboardEvent = new ClipboardEvent('paste', {
        clipboardData: dataTransfer,
    ...
    it('Paste the image into the Rich Text Editor', (done: DoneFn) => {
      ...
      fetch(imageUrl)
        ...
        .then((blob) => {
          ...
          setCursorPoint(
            (editor as any).inputElement.querySelector('p').childNodes[0],
            3
          );
          editor.contentModule.getEditPanel().dispatchEvent(pasteEvent);
          setTimeout(function () {
            expect(editor.inputElement.querySelectorAll('img').length > 0).toBe(
              true
            );
            done();
          }, 1000);
