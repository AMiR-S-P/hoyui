...
import { FormValidator } from '@hoyui/inputs';
...
import {
  ...
  destroy,
  setCursorPoint,
  dispatchEvent as dispatchEve,
} from './../rich-text-editor/render.spec';
...
import {
  INSRT_IMG_EVENT_INIT,
  NUMPAD_ENTER_EVENT_INIT,
} from '../constant.spec';
...
describe('RTE CR issues ', () => {
  describe('EJ2-20672 - Full Screen not working properly when render inside the overflow element', () => {
    ...
    beforeAll(() => {
      divElem = <HTMLTextAreaElement>(
        createElement('div', { styles: 'overflow: auto; border: 1px solid;' })
      );
      elem = <HTMLTextAreaElement>(
        createElement('textarea', {
          id: 'rte_test_EJ2_20672',
          attrs: { name: 'formName' },
        })
      );
  ...
  describe('EJ2-18135 - name attribute of textarea element', () => {
    ...
    it('name attribute to textarea element', (done) => {
      ...
      rteObj.appendTo(elem);
      expect(
        (rteObj as any).valueContainer.getAttribute('name') === 'formName'
      ).toBe(true);
      done();
    });
  ...
  describe('EJ2-18212 - RTE - Edited changes are not reflect using getHTML method through console window.', () => {
    ...
    it('AutoSave the value in interval time', (done) => {
      ...
      (
        rteObj as any
      ).inputElement.innerHTML = `<div><p>First p node-1</p></div>`;
      expect(rteObj.value !== '<div><p>First p node-1</p></div>').toBe(true);
      setTimeout(() => {
        expect(rteObj.value === '<div><p>First p node-1</p></div>').toBe(true);
        (
          rteObj as any
        ).inputElement.innerHTML = `<div><p>First p node-2</p></div>`;
        expect(rteObj.value !== '<div><p>First p node-2</p></div>').toBe(true);
        setTimeout(() => {
          expect(rteObj.value === '<div><p>First p node-2</p></div>').toBe(
            true
          );
          done();
        }, 400);
  ...
  describe('EJ2-20436 - Changing font color of underlined text doesn’t changes the color of the line in RTE', () => {
    ...
    it(' Apply the underline and then apply the fontcolor', (done) => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.element.querySelector('#rte').childNodes[0],
        rteObj.element.querySelector('#rte').childNodes[0],
        0,
        3
      );
      let item: HTMLElement = rteObj.element.querySelector(
        '#' + controlId + '_toolbar_Underline'
      );
      dispatchEvent(item, 'mousedown');
      item.click();
      item = rteObj.element.querySelector(
  ...
  describe('EJ2-20463 - Change event is triggered on clicking into html source code view in Edge browser', () => {
    ...
    it(' change event trigger while click on source code with edit ', (done) => {
      ...
      (
        rteObj as any
      ).inputElement.innerHTML = `<p id="rte">RichTextEditor component</p>`;
      let item: HTMLElement = rteObj.element.querySelector(
        '#' + controlId + '_toolbar_SourceCode'
      );
      dispatchEvent(item, 'mousedown');
  ...
  describe('EJ2-21471  -  RTE data annotation validation is not worked', () => {
    ...
    it(' Set the data annotation attribute to textarea alone ', () => {
      ...
      expect((rteObj as any).valueContainer.hasAttribute('data-val')).toBe(
        true
      );
    });
  });
  ...
  describe('EJ2-21612  -  To prevent the table quick toolbar when render RTE inside the table ', () => {
    ...
    it(' click on inside of table content for prevent the quick toolbar ', (done) => {
      let firstP: Element = (rteObj as any).inputElement.querySelector(
        '#rte-p'
      );
  ...
  describe('EJ2-21470 - RichTextEditor Font Size "px" not update in toolbar status and fontFamily "veranda" style not updated properly', () => {
    ...
    it(' Check the toolbar status while click on fontsize and fontName element ', (done) => {
      ...
      spanEle.click();
      setTimeout(() => {
        let fontSize: HTMLElement = rteObj.element.querySelector(
          '#' + controlId + '_toolbar_FontSize'
        );
        let fontName: HTMLElement = rteObj.element.querySelector(
          '#' + controlId + '_toolbar_FontName'
        );
        expect(
          (fontSize.firstElementChild as HTMLElement).innerText.trim()
        ).toBe('14 px');
        expect(
          (fontName.firstElementChild as HTMLElement).innerText.trim()
        ).toBe('Verdana');
        done();
      }, 50);
    });
  ...
  describe('EJ2-21814 - Clicking on view source code with single character inside textarea removes the character.', () => {
    ...
    it(' Click the source code with single character ', (done) => {
      let sourceCode: HTMLElement = rteObj.element.querySelector(
        '#' + controlId + '_toolbar_SourceCode'
      );
      ...
      setTimeout(() => {
        let textarea: HTMLTextAreaElement = (
          rteObj as any
        ).element.querySelector('.e-rte-srctextarea');
  ...
  describe(' EJ2-218412  -  htmlAttributes "id" is not set to the validation textarea element in RTE ', () => {
    ...
    let element: HTMLElement = createElement('div', {
      id: 'form-element',
      innerHTML: ` <div class="rte-element"></div>
                `,
    });
    beforeEach((done: Function) => {
      document.body.appendChild(element);
    ...
    it(' Render the RTE without ID and set the id via htmlAttributes property ', () => {
      ...
      expect((rteObj as any).valueContainer.id === 'htmlAttr-id-value').toBe(
        true
      );
      expect(
        (rteObj as any).inputElement.id === 'htmlAttr-id_rte-edit-view'
      ).toBe(true);
    });
  });
  ...
  describe('EJ2-22404 - Setting default font styles is not maintained on typing into RTE.', () => {
    ...
    it(' Set default value to format, fontSize, fontFamily ', () => {
      ...
      rteEle = rteObj.element;
      controlId = rteEle.id;
      let fontSize: HTMLElement = rteEle.querySelector(
        '#' + controlId + '_toolbar_FontSize'
      );
      let fontName: HTMLElement = rteEle.querySelector(
        '#' + controlId + '_toolbar_FontName'
      );
      let format: HTMLElement = rteEle.querySelector(
        '#' + controlId + '_toolbar_Formats'
      );
      expect(
        fontSize.querySelector('.e-rte-dropdown-btn-text').textContent ===
          '14 pt'
      ).toBe(true);
      expect(
        fontName.querySelector('.e-rte-dropdown-btn-text').textContent ===
          'Arial'
      ).toBe(true);
      expect(
        format.querySelector('.e-rte-dropdown-btn-text').textContent ===
          'Preformatted'
      ).toBe(true);
      expect(
        ((rteObj as any).inputElement as HTMLElement).style.fontSize === '14pt'
      ).toBe(true);
      expect(
        ((rteObj as any).inputElement as HTMLElement).style.fontFamily ===
          'Arial'
      ).toBe(true);
    });
    ...
    it(' Dynamic Set the default value as null to format, fontSize, fontFamily ', () => {
      ...
      expect(
        ((rteObj as any).inputElement as HTMLElement).style.fontSize === ''
      ).toBe(true);
      expect(
        ((rteObj as any).inputElement as HTMLElement).style.fontFamily === ''
      ).toBe(true);
    ...
    describe(' reset  - ', () => {
      ...
      it(' test the reset the form ', () => {
        ...
        let element: HTMLElement =
          rteObj.element.querySelector('#defaultRTE-info');
        expect(rteObj.value === '<p>EJ2 RichTextEditor Component</p>').toBe(
          true
        );
  ...
  describe('EJ2-22972 - Editor content rendered twice in DOM when using RichTextEditorFor', () => {
    ...
    it(' Check the edit area content in wrapper element', (done) => {
      elem = <HTMLTextAreaElement>(
        createElement('textarea', {
          id: 'rte_test_EJ2-22972',
          innerHTML: '<p class="test-paragraph">RichTextEditor</p>',
        })
      );
      document.body.appendChild(elem);
      ...
      rteObj.appendTo(elem);
      expect(
        rteObj.element.querySelectorAll('.test-paragraph').length === 1
      ).toBe(true);
      done();
    });
  ...
  describe('EJ2-22988 - e-lib class not added into control root element, when render RTE using textarea element', () => {
    ...
    it(' Check the root element class', (done) => {
      elem = <HTMLTextAreaElement>(
        createElement('textarea', { id: 'rte_test_EJ2-22988' })
      );
      document.body.appendChild(elem);
      rteObj = new RichTextEditor({
        value: '<p class="test-paragraph">RichTextEditor</p>',
      ...
      expect(
        (rteObj as any).valueContainer.classList.contains('e-control')
      ).toBe(false);
      expect((rteObj as any).valueContainer.classList.contains('e-lib')).toBe(
        false
      );
      expect(
        (rteObj as any).valueContainer.classList.contains('e-richtexteditor')
      ).toBe(false);
      done();
    });
    ...
    it(' Check the alignments dropdown items ', (done) => {
      ...
      setTimeout(() => {
        let items: any = document.querySelectorAll(
          '#' + controlId + '_toolbar_Alignments-popup .e-item'
        );
  ...
  describe('EJ2-23588 - RichTextEditor inline mode error when color property is displayed in mobile view.', () => {
    ...
    it(' Check the fontColor and backgroundColor ', (done) => {
      ...
      setTimeout(() => {
        let item: HTMLElement = document.querySelector(
          '#' + controlId + '_quick_FontColor'
        );
        item.click();
        let popup: HTMLElement = document.getElementById(
          controlId + '_quick_FontColor-popup'
        );
        expect(!isNullOrUndefined(popup)).toBe(true);
        done();
      }, 200);
  ...
  describe('EJ2-29347 - RTE base refresh method testing', () => {
    ...
    it(' Check the alignments dropdown items ', (done) => {
      ...
      expect(
        document.querySelectorAll('.e-toolbar-item.e-overlay').length
      ).toEqual(3);
      expect(
        document
          .querySelectorAll('.e-toolbar-item.e-overlay')[0]
          .getAttribute('title')
      ).toEqual('Bold (Ctrl+B)');
      expect(
        document
          .querySelectorAll('.e-toolbar-item.e-overlay')[1]
          .getAttribute('title')
      ).toEqual('Undo (Ctrl+Z)');
      expect(
        document
          .querySelectorAll('.e-toolbar-item.e-overlay')[2]
          .getAttribute('title')
      ).toEqual('Redo (Ctrl+Y)');
      rteObj.refresh();
      setTimeout(() => {
  ...
  describe('Change event triggered -readOnly enabled', () => {
    ...
    it('Check change event when readonly is enabled', (done: Function) => {
      ...
      (
        (<HTMLElement>(
          rteObj.element.querySelectorAll('.e-toolbar-item')[0]
        )) as HTMLElement
      ).click();
      setTimeout(() => {
  ...
  describe('RichTextEditor databinding not working in SourceCode view', () => {
    ...
    it(' Check SourceCode view ', (done) => {
      ...
      let item: HTMLInputElement =
        rteObj.element.querySelector('.e-rte-srctextarea');
  ...
  describe('BLAZ-5932 - Cannot set font-style after inserting a table', () => {
    ...
    it(' Empty container with table insert after font style apply check ', () => {
      ...
      (
        (<HTMLElement>(
          rteEle.querySelectorAll('.e-toolbar-item')[0]
        )) as HTMLElement
      ).click();
      expect(
        rteObj.tableModule.popupObj.element.querySelectorAll('.e-rte-table-row')
          .length === 3
      ).toBe(true);
      expect(
        rteObj.tableModule.popupObj.element.querySelectorAll('.e-rte-tablecell')
          .length === 30
      ).toBe(true);
      let event: any = {
        target: (rteObj as any).tableModule.popupObj.element
          .querySelectorAll('.e-rte-table-row')[1]
          .querySelectorAll('.e-rte-tablecell')[3],
        preventDefault: function () {},
      };
      ...
      let table: HTMLElement = rteObj.contentModule
        .getEditPanel()
      ...
      rteObj.formatter.editorManager.nodeSelection.setCursorPoint(
        document,
        rteObj.contentModule.getEditPanel(),
        1
      );
      (rteObj.formatter.editorManager as any).formatObj.onKeyUp({
        event: keyboardEventArgs,
      });
      expect(
        rteObj.contentModule.getEditPanel().querySelectorAll('p').length === 1
      ).toBe(true);
    });
    it(' Text container with table insert after font style apply check ', () => {
      ...
      expect(table.querySelectorAll('td').length === 8).toBe(true);
      let brTag: Element = document.createElement('br');
      rteObj.contentModule
        .getEditPanel()
        .insertBefore(
          brTag,
          rteObj.contentModule.getEditPanel().querySelector('p')
        );
      rteObj.formatter.editorManager.nodeSelection.setCursorPoint(
        document,
        rteObj.contentModule.getEditPanel(),
      ...
      expect(
        rteObj.contentModule.getEditPanel().querySelectorAll('p').length === 2
      ).toBe(true);
      expect(
        rteObj.contentModule.getEditPanel().childNodes[1].textContent === ''
      ).toBe(true);
      expect(
        rteObj.contentModule.getEditPanel().querySelectorAll('p')[0]
          .textContent === ''
      ).toBe(true);
      expect(
        rteObj.contentModule.getEditPanel().childNodes[2].textContent ===
          'Sample content'
      ).toBe(true);
      expect(
        rteObj.contentModule.getEditPanel().querySelectorAll('p')[1]
  ...
  describe('BLAZ-7176 - Enter key press before the image in a paragraph', () => {
    ...
    it(' Enter key press before the image in a paragraph ', () => {
      rteObj = renderRTE({
        ...
        value:
          '<p id="p1">Paragraph <img src="blob:null/abfb97c2-cd30-4405-81e0-2993d05bfa35" class="e-rte-image e-imginline" alt="blazor.PNG" width="auto" height="auto" style="min-width: 0px; max-width: 1199px; min-height: 0px;"> </p>',
      });
      rteEle = rteObj.element;
      let start: HTMLElement = document.getElementById('p1');
      rteObj.formatter.editorManager.nodeSelection.setCursorPoint(
        document,
        start.childNodes[0] as Element,
        10
      );
      (rteObj.formatter.editorManager as any).formatObj.onKeyUp({
        event: keyboardEventArgs,
  ...
  describe('EJ2-37997 - Lists all item selection with delete key action not remove the list completely', () => {
    ...
    it(' Ordered list with select all item with test ', () => {
      ...
      rteObj.focusIn();
      rteObj.selectAll();
      (rteObj.formatter.editorManager as any).listObj.keyDownHandler({
        event: keyboardEventArgs,
      });
    ...
    it(' Ordered list with select some item with test ', () => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.element.querySelector('ol').childNodes[0],
        rteObj.element.querySelector('ol').childNodes[1],
        0,
        1
      );
      (rteObj.formatter.editorManager as any).listObj.keyDownHandler({
    ...
    it(' Unordered list with select some item with test ', () => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.element.querySelector('ul').childNodes[0],
        rteObj.element.querySelector('ul').childNodes[1],
        0,
        1
      );
      (rteObj.formatter.editorManager as any).listObj.keyDownHandler({
  ...
  describe('EJ2-41562 - Script error occurs with toolbar options, when placing the cursor before & after RichTextEditor table', () => {
    ...
    it(' Before the table element ', () => {
      rteObj = renderRTE({
        ...
        value:
          '<table class="e-rte-table" style="width: 100%; min-width: 0px;"><tbody><tr><td class="" style="width: 50%;"><br></td><td style="width: 50%;"><br></td></tr></tbody></table>',
      });
      rteEle = rteObj.element;
      expect(rteEle.querySelector('.e-content').childNodes.length === 1).toBe(
        true
      );
      rteObj.focusIn();
      let targetElm: HTMLElement = rteEle.querySelectorAll(
      ...
      expect(document.querySelectorAll('ol').length === 1).toBe(true);
      expect(
        rteEle.querySelector('.e-content').childNodes[0].nodeName === 'OL'
      ).toBe(true);
    });
    it(' After the table element ', () => {
      ...
      targetElm.click();
      expect(rteEle.querySelector('.e-content').childNodes.length === 2).toBe(
        true
      );
      expect(document.querySelectorAll('ol').length === 1).toBe(true);
      expect(
        rteEle.querySelector('.e-content').childNodes[0].nodeName === 'TABLE'
      ).toBe(true);
      expect(
        rteEle.querySelector('.e-content').childNodes[1].nodeName === 'OL'
      ).toBe(true);
    });
    afterEach(() => {
      destroy(rteObj);
  ...
  describe('EJ2-60381 - Image resize icon not shown properly when enabled iframe', () => {
    ...
    it('check resize element when click image in iframe mode', () => {
      ...
      ).contentWindow.document.body as HTMLElement;
      ...
      expect(
        rteObj.contentModule.getEditPanel().querySelector('.e-img-resize')
      ).not.toBe(null);
      expect(
        rteObj.contentModule
          .getEditPanel()
          .querySelectorAll('.e-rte-imageboxmark').length
      ).toBe(4);
    });
  });
  ...
  describe('EJ2-60306 - EJ2-60307 - RTE render with empty p tag element', () => {
    ...
    it('check content div element', () => {
      expect(rteObj.inputElement.innerHTML === '<div><p><br></p></div>').toBe(
        true
      );
    });
  });
  ...
  describe('EJ2-62151 - Strikethrough and underline are removed when we select and press shift key on lists in RTE', () => {
    ...
    beforeAll(() => {
      defaultRTE = renderRTE({
        height: 400,
        toolbarSettings: {
          items: ['Undo', 'Redo', '|', 'Underline', 'StrikeThrough', '|'],
        },
        value: innerHTML,
      });
    });
    ...
    it('should not remove current focus of selected text after pressing SHIFT key', () => {
      let startContainer: any = (defaultRTE as any).inputElement.querySelector(
        '.FocusNode1'
      ).childNodes[0];
      ...
      defaultRTE.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        startContainer,
        endContainer,
        0,
      ...
      let style = (defaultRTE as any).inputElement.querySelector('.FocusNode1')
      ...
      expect(defaultRTE.inputElement.textContent.length).toBe(423);
      style = (defaultRTE as any).inputElement.querySelector('.FocusNode1')
        .style.textDecoration;
      expect(style == 'line-through').toBe(true);
    });
  });
  ...
  describe('EJ2-63042 - Tooltip not shown for NumberFormat and BulletFormat List in RTE Toolbar items', () => {
    ...
    it('check the tooltip', () => {
      expect(
        document.querySelectorAll('.e-toolbar-item.e-template').length
      ).toEqual(2);
      expect(
        document
          .querySelectorAll('.e-toolbar-item.e-template')[0]
          .getAttribute('title')
      ).toEqual('Number Format List (Ctrl+Shift+O)');
      expect(
        document
          .querySelectorAll('.e-toolbar-item.e-template')[1]
          .getAttribute('title')
      ).toEqual('Bullet Format List (Ctrl+Alt+O)');
    });
  });
  ...
  describe(' EJ2-65988 - Code block doesnt work properly when pasting contents into the pre tag in RTE', () => {
    ...
    const targetElm: HTMLElement = createElement('div', {
      className: 'target',
    });
    beforeEach(() => {
      rteObj = new RichTextEditor({
  ...
  describe(' EJ2-65567 - Underline and Strikethrough toolbar styles doesnt work properly CASE 1', () => {
    ...
    it('should add span element with font size to around the text node', (done: Function) => {
      const contentElem: HTMLElement =
        rteObject.element.querySelector('.e-content');
      ...
      const toolbarButtons: NodeList =
        document.body.querySelectorAll('.e-tbar-btn');
      ...
      const dropButton: NodeList =
        document.body.querySelectorAll('.e-dropdown-btn');
      ...
      expect(rteObject.inputElement.innerHTML === correctElementString).toBe(
        true
      );
      ...
      expect(
        rteObject.inputElement.innerHTML ===
          '<p><span style="font-size: 36pt;">Testing</span></p>'
      ).toBe(true);
      done();
    });
  ...
  describe(' EJ2-65567 - Underline and Strikethrough toolbar styles doesnt work properly CASE 2', () => {
    ...
    it('should add span element with font size to around the text node', (done: Function) => {
      ...
      const tileItems: NodeList = (row[0] as HTMLElement).querySelectorAll(
        '.e-tile'
      );
      (tileItems[9] as HTMLElement).click();
      // Background color
      (
        rteObject.element.querySelector('.e-background-color') as HTMLElement
      ).click();
  ...
  describe(' EJ2-65567 - Underline and Strikethrough toolbar styles doesnt work properly CASE 3 Table Element', () => {
    let rteObject: RichTextEditor;
    let innerHTML: string =
      '<table class="e-rte-table" style="width: 100%; min-width: 0px;"><tbody><tr><td style="width: 33.3333%;" class=""><span style="text-decoration: underline;"><span style="text-decoration: line-through;">Testing</span></span></td><td style="width: 33.3333%;"><br></td><td style="width: 33.3333%;"><br></td></tr><tr><td style="width: 33.3333%;"><br></td><td style="width: 33.3333%;"><br></td><td style="width: 33.3333%;"><br></td></tr><tr><td style="width: 33.3333%;"><br></td><td style="width: 33.3333%;"><br></td><td style="width: 33.3333%;"><br></td></tr></tbody></table>';
    beforeEach(() => {
      rteObject = renderRTE({
        toolbarSettings: {
          items: [
    ...
    it('should add span element with font size to around the span node', (done: Function) => {
      const contentElem: HTMLElement = rteObject.element.querySelector(
      ...
      (fontDropItems[7] as HTMLElement).click(); // Apply Font size
      expect(
        (range.startContainer.childNodes[0] as HTMLElement).style.fontSize
      ).toEqual('36pt');
      done();
    });
  });
  ...
  describe(' EJ2-65567 - Underline and Strikethrough toolbar styles doesnt work properly CASE 4 Link Element', () => {
    let rteObject: RichTextEditor;
    let innerHTML: string =
      '<p><span><a classname="e-rte-anchor" href="https://syncfusion.atlassian.net/browse/EJ2-65567" title="https://syncfusion.atlassian.net/browse/EJ2-65567" target="_blank"><span style="text-decoration: underline;">https://syncfusion.atlassian.net/browse/EJ2-65567</span> </a></span><br></p>';
    beforeEach(() => {
      rteObject = renderRTE({
        toolbarSettings: {
          items: [
  ...
  describe(' EJ2-65567 - Underline and Strikethrough toolbar styles doesnt work properly CASE 5 Code Block', () => {
    let rteObject: RichTextEditor;
    let innerHTML: string =
      '<pre><span style="text-decoration: line-through;"><span style="text-decoration: underline;">Testing﻿﻿</span></span><br></pre>';
    beforeEach(() => {
      rteObject = renderRTE({
        toolbarSettings: {
          items: [
    ...
    it('should add span element with font size to around the span node', (done: Function) => {
      ...
      const fontDropItems: NodeList = document.body.querySelectorAll('.e-item');
      (fontDropItems[7] as HTMLElement).click(); // Apply Font size
      expect((contentElem.childNodes[0] as HTMLElement).style.fontSize).toEqual(
        '36pt'
      );
      done();
    });
  ...
  describe(' EJ2-65567 - Underline and Strikethrough toolbar styles doesnt work properly CASE 6 Heading', () => {
    let rteObject: RichTextEditor;
    let innerHTML: string =
      '<h1><span style="text-decoration: line-through;"><strong>Testing 1</strong></span></h1><h2><span style="text-decoration: underline;"><strong>Testing 2</strong></span></h2><h3><span style="text-decoration: line-through;"><em><span style="text-decoration: underline;">Testing 3</span></em></span></h3><h4><strong><em><span style="text-decoration: underline;">Testing 4</span></em></strong></h4>';
    beforeAll(() => {
      rteObject = renderRTE({
        toolbarSettings: {
          items: [
  ...
  describe(' EJ2-65567 - Underline and Strikethrough toolbar styles doesnt work properly CASE 7 Image Element Alt Text', () => {
    ...
    it('should wrap span element with font size to around the style span node', (done: Function) => {
      const contentElem: HTMLElement =
        rteObject.element.querySelector('.e-img-inner');
      let range: Range = new Range();
      range.setStart(contentElem, 0);
      range.setEnd(contentElem, 1);
      rteObject.formatter.editorManager.nodeSelection.setRange(document, range);
    ...
    it('Test for margin-left and start attribute', (done: Function) => {
      ...
      setTimeout(() => {
        if (rteObject.pasteCleanupSettings.prompt) {
          let keepFormat: any = document
            .getElementById(rteObject.getID() + '_pasteCleanupDialog')
            .getElementsByClassName('e-rte-keepformat');
          keepFormat[0].click();
          let pasteOK: any = document
            .getElementById(rteObject.getID() + '_pasteCleanupDialog')
            .getElementsByClassName('e-rte-pasteok');
          pasteOK[0].click();
        }
        expect(
          rteObject.element.querySelector('li').style.marginLeft === ''
        ).toEqual(true);
        expect(rteObject.element.querySelectorAll('ol')[1].start === 2).toEqual(
          true
        );
        done();
      }, 400);
  ...
  describe(' EJ2-68542: Font size not applied properly for the Numbered lists in RichTextEditor', () => {
    ...
    it('check the font size apply on list items', (done: Function) => {
      ...
      rteObject.formatter.editorManager.nodeSelection.setRange(document, range);
      let orderNumberListBtn: HTMLElement = document.querySelectorAll(
        '.e-toolbar-item'
      )[1] as HTMLElement;
  ...
  describe('EJ2-69957: Quick toolbar tooltip remains in the open state after close the toolbar', () => {
    ...
    it('check undo tooltip content', (done: Function) => {
      ...
      let mouseEve = new MouseEvent('mouseover', {
        bubbles: true,
        cancelable: true,
        view: window,
      });
      ...
      setTimeout(() => {
        expect(
          isVisible(document.querySelector('.e-tooltip-wrap') as HTMLElement)
        ).toBe(true);
        expect(
          (
            document.querySelector('.e-tooltip-wrap')
              .childNodes[0] as HTMLElement
          ).innerHTML === 'Undo (Ctrl+Z)'
        ).toBe(true);
  ...
  describe('841892 - CTRL + Enter triggers the enter action in the Editor', () => {
    ...
    it('Pressing Crt + enter key after ', (done: Function) => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.inputElement.childNodes[0].childNodes[0],
        rteObj.inputElement.childNodes[0].childNodes[0],
        4,
        4
      );
      ...
      (rteObj as any).keyDown(keyBoardEvent);
      expect((rteObj as any).inputElement.innerHTML === `<p>Testing</p>`).toBe(
        true
      );
      done();
    });
  ...
  describe('844614 - The enableHtmlSanitizer property is not working properly in the Rich Text Editor', () => {
    ...
    it('Sanitize the value if update dynamically ', (done: Function) => {
      ...
      rteObj.dataBind();
      expect(
        (rteObj as any).inputElement.innerHTML ===
          `<p><img src="x" class="e-rte-image e-imginline"></p>`
      ).toBe(true);
      done();
    });
    afterAll((done: DoneFn) => {
  ...
  describe('844717 - The toolbar Button Tooltip not get destroyed when the dialog is opened and closed issue resolved', () => {
    ...
    it('Tooltip hide while click fullscreen', (done: Function) => {
      ...
      let toolbarEle = document.querySelector(
        '[title="Maximize (Ctrl+Shift+F)"]'
      );
      toolbarEle.dispatchEvent(event);
      expect(
        !isNullOrUndefined(document.querySelector('.e-tooltip-wrap'))
      ).toBe(true);
      (document.querySelectorAll('.e-toolbar-item')[1] as HTMLElement).click();
      setTimeout(function () {
        (
          document.querySelectorAll('.e-toolbar-item')[1] as HTMLElement
        ).click();
        setTimeout(function () {
          expect(document.querySelector('.data-tooltip-id') === null).toBe(
            true
          );
          done();
        }, 100);
  ...
  describe('846923 - The heading colour is set to blue after copying and pasting inside the editor.', () => {
    ...
    it('CASE 1: Test for console error on paste action', (done: DoneFn) => {
      ...
      setTimeout(() => {
        expect(
          rteObj.element.querySelector('.e-dlg-header-content').textContent ===
            'Paste Format'
        );
        done();
      }, 200);
  ...
  describe('848791 - The CMD + B Shortcut not working on the Safari browser', () => {
    ...
    it('Bold action in Mac machin : Command + b', () => {
      ...
      (rteObj as any).keyDown(keyBoardEvent);
      expect(
        editNode.querySelector('.first-p').firstChild.nodeName === 'STRONG'
      ).toBe(true);
    });
  ...
  describe('846885 - NumberFormatList and BulletFormatList not apply in Safari browser', () => {
    ...
    it('list in acion in mac', () => {
      rteObj.focusIn();
      selectNode = (editNode.querySelector('.first-p') as HTMLElement)
        .firstChild as HTMLElement;
      setCursorPoint(selectNode, 1);
      let trg = document.querySelector(
        '[title="Number Format List (Ctrl+Shift+O)"]'
      ...
      (
        document.querySelector('[title="Number Format List (Ctrl+Shift+O)"]')
          .childNodes[0] as HTMLElement
      ).click();
      (
        document.querySelector('.e-dropdown-popup').childNodes[0]
  ...
  describe('847101 - The image focus and resize class names are not removed when the editor in focused out. - ', () => {
    ...
    beforeEach((done: Function) => {
      rteObj = renderRTE({
        value:
          '<p><img id="rteImageID" style="width: 300px; height: 300px;" alt="Logo" src="https://ej2.syncfusion.com/javascript/demos/src/rich-text-editor/images/RTEImage-Feather.png"></p>',
      });
      done();
    });
    afterEach((done: Function) => {
  ...
  describe('849657 - Cancelling undo and redo actions using actionBegin events cancel argument is not working in RichTextEditor', () => {
    ...
    beforeAll(() => {
      rteObj = renderRTE({
        ...
        actionBegin: function (e: any) {
          if (
            (e.requestType as string).toLowerCase() === 'undo' ||
            (e.requestType as string).toLowerCase() === 'redo'
          ) {
            e.cancel = true;
            isCancelled = true;
          }
    ...
    it('Undo and Redo actions are cancelled', () => {
      ...
      expect(
        rteObj.contentModule.getEditPanel().querySelector('strong') !== null
      ).toBe(true);
      // Undo action
  ...
  describe('851908 - When selecting multiple fonts applied texts, the font family toolbar should not show the font name as empty', () => {
    ...
    it('CASE 1 - Check the toolbar values after selecting multiple font size in singel line', (done: DoneFn) => {
      ...
      setTimeout(() => {
        expect(
          rteObj.toolbarModule
            .getToolbarElement()
            .querySelector('.e-font-size-tbar-btn').textContent
        ).toBe('');
        done();
      }, 200);
    });
      ...
      setTimeout(() => {
        if (rteObj.toolbarModule.getToolbarElement()) {
          ...
          expect(
            rteObj.toolbarModule
              .getToolbarElement()
              .querySelector('.e-font-name-tbar-btn').textContent
          ).toBe('');
          expect(
            rteObj.toolbarModule
              .getToolbarElement()
              .querySelector('.e-formats-tbar-btn').textContent
          ).toBe('');
        }
      }, 200);
    });
  ...
  describe('855271 - Toolbar status not updated properly when we dynamically enable the toolbar in RichTextEditor', () => {
    ...
    it('Testing the html toolbar status class is null or undefined', () => {
      ...
      rteObj.toolbarSettings.enable = true;
      rteObj.dataBind();
      expect((rteObj.htmlEditorModule as any).toolbarUpdate).not.toBe(
        undefined
      );
    });
  });
  ...
  describe('EJ2-858344 - The pastecleanup is to remove spaces while pasting from the document', () => {
    ...
    it('Should not remove the &nbsp and span element while pasting the content', (done: DoneFn) => {
      ...
      setTimeout(() => {
        expect(
          editor.contentModule.getEditPanel().querySelectorAll('p')[1]
            .childElementCount === 1
        ).toBe(true);
        expect(
          editor.contentModule.getEditPanel().querySelectorAll('p')[1]
            .firstElementChild.textContent.length
        ).toBe(1); // &nbsp;
        done();
      }, 100);
    });
  ...
  describe('873317: Images with source srcset not properly pasted into the RichTextEditor.', () => {
    ...
    it('Should add full URL to the Source tag srcset attribute.', (done: DoneFn) => {
      ...
      setTimeout(() => {
        const sourceElems: NodeListOf<HTMLSourceElement> =
          editor.inputElement.querySelectorAll('source');
        for (let i: number = 0; i < sourceElems.length; i++) {
          expect(
            sourceElems[i].srcset.indexOf('https://support.microsoft.com')
          ).toBe(0);
        }
        done();
      }, 100);
  ...
  describe('904051: Number Pad Enter Key Does Not Trigger actionBegin Event in the Rich Text Editor.', () => {
    ...
    it('Should trigger the action begin even on NUMPAD enter action', (done: DoneFn) => {
      ...
      editor.inputElement.dispatchEvent(
        new KeyboardEvent('keydown', NUMPAD_ENTER_EVENT_INIT)
      );
      editor.inputElement.dispatchEvent(
        new KeyboardEvent('keyup', NUMPAD_ENTER_EVENT_INIT)
      );
      setTimeout(() => {
  ...
  describe('904558: Image action begin event args does not reflect after image is inserted.', () => {
      ...
      editor = renderRTE({
        actionBegin: function (e) {
          if (e.requestType === 'Image') {
            e.itemCollection.url =
              e.itemCollection.url + 'api/UnauthorizedImage';
    ...
    it('Should able to update edit the inserted image on action begin event.', (done: DoneFn) => {
      editor.focusIn();
      editor.inputElement.dispatchEvent(
        new KeyboardEvent('keydown', INSRT_IMG_EVENT_INIT)
      );
      editor.inputElement.dispatchEvent(
        new KeyboardEvent('keyup', INSRT_IMG_EVENT_INIT)
      );
      setTimeout(() => {
        const inputElem: HTMLInputElement = editor.element.querySelector(
          '.e-rte-img-dialog .e-input.e-img-url'
        );
        inputElem.value =
  ...
  describe('EJ2-855260 - The font family is not applied properly when pasted from a Word document.', () => {
    ...
    it('Should have font family of Calibri set to the paragraph element', (done: DoneFn) => {
      ...
      setTimeout(() => {
        expect(
          (
            editor.contentModule
              .getEditPanel()
              .querySelectorAll('ul p')[0] as HTMLElement
          ).style.fontFamily
        ).toEqual('');
        done();
      }, 100);
  ...
  describe('EJ2-847108 - When pasting contents into the RichTextEditor, the focus gets lost and script error thrown', () => {
    ...
    it('Test for pasteCleanup', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          window.getSelection().getRangeAt(0).startContainer.nodeName === 'BR'
        ).toEqual(true);
        expect(
          window.getSelection().getRangeAt(0).endContainer.nodeName === 'BR'
        ).toEqual(true);
        done();
      }, 400);
    });
  ...
  describe('847097 - Image get duplicated when we press enter key next to the copy pasted image content from Word', () => {
    ...
    it('Image gets duplicate paste from ms word ', () => {
      ...
      rteEle = rteObj.element;
      let start: HTMLElement = document.getElementById(
        'msWordImg-clip_image001'
      );
      setCursorPoint(start, 1);
      (rteObj as any).keyDown(keyboardEventArgs);
      expect(
        rteObj.contentModule.getEditPanel().querySelectorAll('p').length === 5
      ).toBe(true);
    });
    afterEach(() => {
      destroy(rteObj);
  ...
  describe('853088 - Script error throws when clicking preview toolbar while using itemConfigs with ToolbarSettings in RichTextEditor', () => {
    ...
    it('click the preview toolbar while using itemConfigs with ToolbarSettings ', () => {
      ...
      rteEle = rteObj.element;
      let previewEle: HTMLElement = document.querySelector(
        '[title= "Code View (Ctrl+Shift+H)"]'
      );
      previewEle.click();
  ...
  describe('853677 - The image alternate text is not shown properly in the Rich Text Editor.', () => {
    ...
    it('ensure insert image on Alternate text', () => {
      ...
      let imgElem: HTMLElement = (rteObj as any).inputElement.querySelector(
        'img'
      );
      expect(
  ...
  describe('854718 - Need to add the aria label attribute to the link in the Rich Text Editor', () => {
    ...
    it('link with the aria-label attribute', () => {
      ...
      let args: any = {
        preventDefault: function () {},
        originalEvent: { target: rteObj.toolbarModule.getToolbarElement() },
      ...
      let selectParent: any = new NodeSelection().getParentNodeCollection(
        range
      );
      ...
      let evnArg = {
        ...
        args: args,
        event: MouseEvent,
        selfLink: (<any>rteObj).linkModule,
        selection: save,
        selectParent: selectParent,
        selectNode: selectNode,
      };
      (<any>rteObj).linkModule.linkDialog(evnArg);
      (<any>rteObj).linkModule.dialogObj.contentEle.querySelector(
        '.e-rte-linkurl'
      ).value = 'http://data';
      (<any>rteObj).linkModule.dialogObj.contentEle.querySelector(
        '.e-rte-linkText'
      ...
      (<any>rteObj).contentModule
        .getEditPanel()
        .querySelector('.e-rte-anchor')
        .focus();
      args = {
        preventDefault: function () {},
        originalEvent: { target: rteObj.toolbarModule.getToolbarElement() },
      ...
      (<any>rteObj).contentModule
        .getEditPanel()
        .querySelector('.e-rte-anchor').target = '_blank';
  ...
  describe('853959 - The anchor element was removed when removing the underline in the Rich Text Editor.', () => {
    ...
    it('The anchor element was removed', () => {
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.contentModule.getDocument().querySelector('a'),
        rteObj.contentModule.getDocument().querySelector('a'),
        0,
        1
      );
      ...
      expect(
        rteObj.contentModule.getDocument().querySelector('a').style
          .textDecoration === 'none'
      ).toBe(true);
    });
  });
  ...
  describe('849875 - Cursor position get lost when having empty span tag in RichTextEditor', () => {
    ...
    beforeEach(() => {
      ...
      toolbarEle.innerHTML =
        '<ul><li class="e-list1">Object1</li><li class="e-list2">Object2</li></ul>';
  ...
  describe('854639 - Need to remove the max row count for the table in the Rich Text Editor.', () => {
    ...
    it('table creation of row more than 50 ', (done: DoneFn) => {
      (
        (<HTMLElement>(
          rteEle.querySelectorAll('.e-toolbar-item')[1]
        )) as HTMLElement
      ).click();
      let target: HTMLElement = (
        rteObj as any
      ).tableModule.popupObj.element.querySelector('.e-insert-table-btn');
      let clickEvent: any = document.createEvent('MouseEvents');
      ...
      rteEle
        .querySelectorAll('.e-numeric-hidden')[1]
        .setAttribute('value', '51');
      (
        rteEle.querySelectorAll('.e-numeric-hidden')[1] as HTMLInputElement
      ).value = '51';
      rteEle.querySelector('.e-table-row').dispatchEvent(new Event('change'));
      (rteEle.querySelector('.e-table-row') as HTMLInputElement).blur();
      target = rteObj.tableModule.editdlgObj.element.querySelector(
        '.e-insert-table'
      ) as HTMLElement;
      target.dispatchEvent(clickEvent);
      setTimeout(() => {
    ...
    it('Check the create table popup open', () => {
      (
        document.querySelector(
          '[title="Create Table (Ctrl+Shift+E)"]'
        ) as HTMLElement
      ).click();
      ...
      target.dispatchEvent(clickEvent);
      expect(document.querySelectorAll('.e-rte-table-popup').length == 0).toBe(
        true
      );
    });
  });
  ...
  describe('857980 - The rich text editor content is removed when the enter key is in BR mode ', () => {
    ...
    it('enter key br mode', () => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setCursorPoint(
        document,
        rteObj.inputElement.querySelector('.currentStartMark')
          .childNodes[5] as Element,
        0
      );
      (rteObj as any).keyDown({
        keyCode: 13,
        which: 13,
        shiftkey: false,
        key: 'Enter',
      ...
      expect(
        rteObj.inputElement.querySelector('.currentStartMark').childNodes
          .length === 11
      ).toBe(true);
    });
  });
  ...
  describe('854667 - The table styles are not preselected in the quick toolbar in the Rich Text Editor.', function () {
    ...
    it('Dashed borders', function (done) {
      rteObj.focusIn();
      var tbElement = rteObj.contentModule
        .getEditPanel()
        .querySelector('.tdElement');
      ...
      setTimeout(function () {
        (
          document
            .querySelectorAll(
              '.e-rte-quick-toolbar .e-toolbar-items .e-toolbar-item'
            )[8]
            .querySelector('.e-btn-icon.e-caret') as any
        ).click();
        (
          document.querySelector(
            '.e-dropdown-popup .e-item.e-dashed-borders'
          ) as any
        ).click();
        detach(div);
        setCursorPoint(tbElement, 0);
        ...
        div = document.querySelector(
          '#' + controlId + '_quick_TableRows-popup'
        );
        (
          document
            .querySelectorAll(
        ...
        expect(
          document
            .querySelector('.e-dropdown-popup .e-item.e-dashed-borders')
    ...
    it('Alternate rows', function (done) {
      ...
      setTimeout(function () {
        ...
        (
          document.querySelector(
            '.e-dropdown-popup .e-item.e-alternate-rows'
          ) as any
        ).click();
        detach(div);
        setCursorPoint(tbElement, 0);
        ...
        expect(
          document
            .querySelector('.e-dropdown-popup .e-item.e-alternate-rows')
            .classList.contains('e-active')
        ).toBe(true);
        detach(div);
        done();
  ...
  describe('854667 - The table styles are not preselected in the quick toolbar in the Rich Text Editor. for image', () => {
    ...
    it('edit image', (done) => {
      ...
      (dialogEle.querySelector('.e-img-url') as HTMLInputElement).value =
        'https://ej2.syncfusion.com/demos/src/rich-text-editor/images/RTEImage-Feather.png';
      (dialogEle.querySelector('.e-img-url') as HTMLInputElement).dispatchEvent(
        new Event('input')
      );
      expect(
        rteObj.element.lastElementChild.classList.contains('e-dialog')
      ).toBe(true);
      (
        document.querySelector('.e-insertImage.e-primary') as HTMLElement
      ).click();
      (rteObj.element.querySelector('.e-rte-image') as HTMLElement).click();
      (<any>rteObj).clickPoints = { clientY: 0, clientX: 0 };
      dispatchEvent(
        rteObj.element.querySelector('.e-rte-image') as HTMLElement,
        'mouseup'
      );
      setTimeout(() => {
        ...
        (rteObj as any).formatter.editorManager.nodeSelection.setSelectionNode(
          rteObj.contentModule.getDocument(),
          target
        );
        var args = {
          item: {
            url: 'https://ej2.syncfusion.com/demos/src/rich-text-editor/images/RTEImage-Feather.png',
            selection: save,
            selectParent: [
              rteObj.element.querySelector('.e-rte-image') as HTMLElement,
            ],
        ...
        setTimeout(() => {
          ...
          QTBarModule.imageQTBar.showPopup(
            10,
            131,
            rteObj.element.querySelector('.e-rte-image') as HTMLElement
          );
          let imgPop: HTMLElement = <HTMLElement>(
            document.querySelector('.e-rte-quick-popup')
          );
          let imgTBItems: NodeList = imgPop.querySelectorAll('.e-toolbar-item');
          let popupElement: Element = curDocument.querySelectorAll(
            '.e-rte-dropdown-popup.e-popup-open'
          )[0];
          let mouseEventArgs = {
            item: { command: 'Images', subCommand: 'JustifyLeft' },
          };
          let img: HTMLElement = rteObj.element.querySelector(
            '.e-rte-image'
          ) as HTMLElement;
          (
            (<HTMLElement>imgTBItems.item(9)).firstElementChild as HTMLElement
          ).click();
          popupElement = curDocument.querySelectorAll(
            '.e-rte-dropdown-popup.e-popup-open'
          )[1];
          ...
          expect(
            document.querySelector('.e-inline').classList.contains('e-active')
          ).toBe(true);
          ...
          expect(
            document.querySelector('.e-break').classList.contains('e-active')
          ).toBe(true);
          done();
        }, 40);
      }, 40);
  ...
  describe('859382 - ImageRemoving event arguments are not properly passed in the RichTextEditor', () => {
    ...
    beforeEach((done: Function) => {
      rteObj = renderRTE({
        ...
        imageRemoving: function (args) {
          if (
            args.cancel != null &&
            args.customFormData != null &&
            args.event != null &&
            args.filesData != null &&
            args.postRawFile != null
          ) {
            propertyCheck = true;
          }
        },
    ...
    it("The imageRemiving event doesn't have the args property.", (done) => {
      ...
      let evnArg = {
        args: MouseEvent,
        self: (<any>rteObj).imageModule,
        selection: save,
        selectNode: new Array(),
      };
      (
        (<HTMLElement>(
          rteEle.querySelectorAll('.e-toolbar-item button')[0]
        )) as HTMLElement
      ).click();
      let dialogEle: Element = rteObj.element.querySelector('.e-dialog');
      (dialogEle.querySelector('.e-img-url') as HTMLInputElement).value =
        'https://js.syncfusion.com/demos/web/content/images/accordion/baked-chicken-and-cheese.png';
      let fileObj: File = new File(['Nice One'], 'sample.jpg', {
        lastModified: 0,
      ...
      (
        document.querySelector(
          '.e-richtexteditor .e-upload-files .e-file-remove-btn'
        ) as any
      ).click();
      setTimeout(() => {
  ...
  describe('855622 - Font styles are not applied to the numbered and bullet format list items in RichTextEditor', () => {
    ...
    it('check Bold', () => {
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.element.querySelectorAll('li')[0].firstChild,
        rteObj.element.querySelectorAll('li')[3].firstChild,
        3,
        3
      );
      (
        rteObj.element.querySelectorAll('.e-toolbar-item')[0] as HTMLElement
      ).click();
      expect(rteEle.querySelectorAll('li')[1].style.fontWeight === 'bold').toBe(
        true
      );
    });
    it('check Italic', () => {
      ...
      (
        rteObj.element.querySelectorAll('.e-toolbar-item')[1] as HTMLElement
      ).click();
      expect(
        rteEle.querySelectorAll('li')[1].style.fontStyle === 'italic'
      ).toBe(true);
    });
    it('check fontname', () => {
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
      ...
      let node: HTMLElement = rteObj.element.querySelectorAll(
        '.e-toolbar-item'
      )[2] as HTMLElement;
      (node.firstChild as HTMLElement).click();
      (
        document.querySelectorAll(
          '.e-dropdown-popup.e-rte-elements li'
        )[4] as HTMLElement
      ).click();
      expect(
        (document.querySelectorAll('.e-content li')[2] as HTMLElement).style
  ...
  describe('863459 - Applying different text styles format out of focus Leads to Issues in RichTextEditor.', () => {
    ...
    it('Focus leads to a console error in the Rich Text Editor.', () => {
      ...
      (
        rteObj.element.querySelectorAll(
          '.e-rte-toolbar .e-toolbar-item button'
        )[0] as any
      ).click();
      (document.querySelector('.e-rte-test-elements div') as any).click();
      (
      ...
      expect(
        rteObj.inputElement.innerHTML ==
          '<p><strong><em>​</em></strong>Rich Text Editor</p>'
      ).toBe(true);
    });
  });
  ...
  describe('863440: Too many times applying bold to a text, sometimes the text got deleted in RichTextEditor.', () => {
    ...
    it('Apply Bold tag for cursor position', () => {
      ...
      expect(rteObj.inputElement.innerHTML).toEqual(
        '<div id="div1"><p id="paragraph1">second rtec</p></div>'
      );
    });
  });
  ...
  describe('866230 - Script error throws when using click event with custom toolbar template in RichTextEditor', () => {
    ...
    it('check the value undo redo action in custom toolbar click', () => {
      ...
      expect(rteObj.inputElement.innerHTML === '<p>RichTextEditor</p>').toBe(
        true
      );
      (rteObj.element.querySelectorAll('.e-toolbar-item')[2] as any).click();
      expect(
        rteObj.inputElement.innerHTML ===
          '<div>testing</div><p>RichTextEditor</p>'
      ).toBe(true);
    });
    afterEach((done) => {
      destroy(rteObj);
  ...
  describe('865259: Script error throws and line breaks added when clicking Bold toolbar item in the RichTextEditor', () => {
    ...
    it('Checking with firefox browser', () => {
      ...
      expect(
        rteObj2.inputElement.innerHTML === '<p><strong></strong>second RTEC</p>'
      ).toBe(true);
  ...
  describe('870180: Copy pasted text to override an existing text pastes at wrong position in RichTextEditor', () => {
    ...
    it('copy and paste text', () => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.inputElement.firstChild,
        rteObj.inputElement.firstChild,
        0,
        3
      );
      ...
      expect(
        (rteObj.inputElement.firstChild as HTMLElement).innerText ===
  ...
  describe('870485: Pressing Enter Key After Pasting an Image Removes the Image in RichTextEditor', () => {
    ...
    it('img with enter key', () => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setCursorPoint(
        document,
        rteObj.inputElement.querySelector('img'),
        0
      );
      keyBoardEvent.code = 'Enter';
      keyBoardEvent.action = 'enter';
  ...
  describe('870298: Numbered list not removed when we delete the entire list using backspace key in RichTextEditor', () => {
    ...
    it('Checking the backspace on list', (done: Function) => {
      rteObj = renderRTE({
        value:
          '<ol><li id="firstli">list1</li><li>list2</li><li>list3</li><li id="lastli">list4</li></ol>',
      });
      let startNode: any = (rteObj as any).inputElement.querySelector(
        '#firstli'
      );
      let endNode: any = (rteObj as any).inputElement.querySelector('#lastli');
      let sel = new NodeSelection().setSelectionText(
        document,
      ...
      (rteObj as any).keyDown(keyBoardEvent);
      expect((rteObj as any).inputElement.querySelectorAll('ol').length).toBe(
        0
      );
      done();
    });
  ...
  describe('870298: Numbered list not removed when we delete the entire list using backspace key in RichTextEditor', () => {
    ...
    it('Checking the backspace on list', (done: Function) => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setCursorPoint(
        document,
        rteObj.inputElement.querySelectorAll('li')[2].querySelector('em'),
        0
      );
      (rteObj as any).mouseUp({ target: rteObj.inputElement, isTrusted: true });
      keyBoardEvent.keyCode = 8;
      ...
      expect(rteObj.inputElement.querySelectorAll('li')[1].innerText).toBe(
        'Report Two: an internal proposal written in Short Report formatReport Three: A comparative recommendation report written for an external client in Long Report format.'
      );
      done();
    });
    afterAll((done: DoneFn) => {
    ...
    it('copy and paste text', (done) => {
      ...
      setTimeout(function () {
        rteObj.formatter.editorManager.nodeSelection.setSelectionText(
          document,
          rteObj.inputElement.firstChild,
          rteObj.inputElement.lastChild.childNodes[0],
          0,
          4
        );
        rteObj.onPaste(keyBoardEvent);
  ...
  describe('878765 - Title attribute not added properly for the Audio element in RichTextEditor', function () {
    ...
    beforeEach(function (done) {
      rteObj = renderRTE({
        value:
          "<div><div id='videoElement'>Video Element</div><div id='audioElement'>Audio Element</div></div>",
      });
      done();
    });
    ...
    it('Use the executeCommand method to insert the video title attributes.', function (done) {
      (<any>rteObj).focusIn();
      setTimeout(function () {
        (<any>rteObj).formatter.editorManager.nodeSelection.setCursorPoint(
          document,
          rteObj.inputElement.querySelector('#videoElement'),
          0
        );
        ...
        expect(
          (<any>rteObj).inputElement
            .querySelector('.e-video-wrap')
            .getAttribute('title') === 'newVideo'
        ).toBe(true);
        done();
      }, 100);
    });
    it('Use the executeCommand method to insert the audio title attributes.', function (done) {
      ...
      setTimeout(function () {
        (<any>rteObj).formatter.editorManager.nodeSelection.setCursorPoint(
          document,
          rteObj.inputElement.querySelector('#audioElement'),
          0
        );
        ...
        expect(
          (<any>rteObj).inputElement
            .querySelector('.e-audio-wrap')
            .getAttribute('title') === 'newAudio'
        ).toBe(true);
        done();
      }, 100);
    });
  ...
  describe('878730: Bullet format list not  removed properly when we replace the content in RichTextEditor', () => {
    ...
    it('copy and paste text', (done) => {
      ...
      setTimeout(function () {
        ...
        expect(
          rteObj.inputElement.innerHTML ===
            '<p style="margin: 0px; color: rgb(51, 51, 51); font-family: Roboto, &quot;Segoe UI&quot;, GeezaPro, &quot;DejaVu Serif&quot;, &quot;sans-serif&quot;, -apple-system, BlinkMacSystemFont; font-size: 14px; font-style: normal; font-weight: 400; text-align: start; text-indent: 0px; text-transform: none; white-space: normal; background-color: rgb(255, 255, 255);">test</p>'
        ).toBe(true);
        done();
      }, 400);
    });
  ...
  describe('879007 - Pressing enter key after inserting table, freezes the RichTextEditor.', () => {
    ...
    it('Keydown in after the table element', function (done) {
      ...
      (rteObj as any).formatter.editorManager.formatObj.onKeyUp({
        event: keyboardEventArgs,
        enterAction: rteObj.enterKey,
      });
      expect(
        focusElement.nextSibling.nodeName.toLocaleLowerCase() === 'p'
      ).toBe(true);
      done();
    });
    it('Keydown with a text node', function (done) {
      ...
      expect(
        focusElement.childNodes[0].nodeName.toLocaleLowerCase() === 'p'
      ).toBe(true);
      done();
    });
    afterAll((done: DoneFn) => {
  ...
  describe('878525: Content Editable div gets deleted when we copy and paste a text into the RichTextEditor when using Enterkey as', () => {
    ...
    beforeAll(() => {
      rteObj = renderRTE({
        value:
          '* Programme de soins infirmiers à domicile (palliatifs, oncologie et SLA) <br/>' +
  ...
  describe('881308: Script error throws when inserting table into the RichTextEditor', () => {
    ...
    beforeEach(() => {
      rteObj = renderRTE({
        ...
        value:
          '<p>test<a class="e-rte-anchor" href="http://link" title="http://link" target="_blank" aria-label="Open in new window">link</a></p><p>test</p><p><br></p>',
      });
      rteEle = rteObj.element;
    });
    afterEach((done) => {
    ...
    it(' insert table ', (done) => {
      ...
      setTimeout(function () {
        ...
        setTimeout(() => {
          expect(
            document.body.querySelector('.e-rte-edit-table.e-dialog')
          ).not.toBe(null);
          expect(
            rteObj.tableModule.editdlgObj.element.querySelector('#tableColumn')
          ).not.toBe(null);
          expect(
            rteObj.tableModule.editdlgObj.element.querySelector('#tableRow')
          ).not.toBe(null);
          expect(
            (
              rteObj.tableModule.editdlgObj.element.querySelector(
                '#tableRow'
              ) as any
            ).value === '3'
          ).toBe(true);
  ...
  describe('879054: InsertHtml executeCommand not inserts into the cursor position after inserting table in RichTextEditor ', () => {
    ...
    it('insert the special character inside the table', (done: DoneFn) => {
      rteObj.dataBind();
      let start: Element = (
        document.querySelector('.e-content').childNodes[0] as HTMLElement
      ...
      (
        document.querySelector('.e-insert-table.e-primary') as HTMLElement
      ).click();
      (document.getElementById('custom_tbar') as HTMLElement).click();
      (document.querySelector('[title="^"]') as HTMLElement).click();
      (
        document.querySelector('.e-rte-elements.e-primary') as HTMLElement
      ).click();
      expect(
  ...
  describe('877787 - InsertHtml executeCommand deletes the entire content when we insert html by selection in RichTextEditor', () => {
    ...
    it('insert html to selection', () => {
      ...
      expect(
        rteObj.inputElement.innerHTML ===
          '<p>testing the <span>Test</span> text editor</p>'
      ).toBe(true);
    });
    afterEach((done) => {
      destroy(rteObj);
  ...
  describe('883844: When using the Refresh method, RichTextEditor doesnot get refreshed to initial rendering', () => {
    ...
    it('Should remove the classnames properly when using refresh method.', () => {
      editor.focusIn();
      const toolbarElems: NodeListOf<HTMLElement> =
        editor.element.querySelectorAll('.e-toolbar-item');
      toolbarElems[0].click();
      expect(editor.element.classList.contains('e-rte-full-screen')).toBe(true);
      editor.refresh();
      expect(editor.element.classList.contains('e-rte-full-screen')).not.toBe(
        true
      );
    });
  });
  describe('875856 - Using indents on Numbered or Bulleted list turns into nested list in RichTextEditor', () => {
    ...
    beforeAll(() => {
      rteObj = renderRTE({
        value:
          '<ol style="list-style-image: none; list-style-type: upper-alpha;"><li>test1</li><li>test2</li><li>test3</li></ol>',
        toolbarSettings: {
    ...
    it('indent and outdent', () => {
      ...
      const item: HTMLElement = rteObj.element.querySelector(
        '#' + controlId + '_toolbar_Indent'
      );
      item.click();
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
      ...
      const item1: HTMLElement = rteObj.element.querySelector(
        '#' + controlId + '_toolbar_Outdent'
      );
      item1.click();
      expect(
        rteObj.inputElement.innerHTML ===
          '<ol style="list-style-image: none; list-style-type: upper-alpha; margin-left: 20px;"><li>test1</li><li>test2</li><li>test3</li></ol>'
      ).toBe(true);
    });
    afterAll((done) => {
      destroy(rteObj);
  ...
  describe('882579 - Pasted Blob images are not uploaded to the server in RichTextEditor', () => {
    ...
    it('Convert the absolute link into base64 format and paste it into the Rich Text Editor.', (done: DoneFn) => {
      ...
      let clipBoardData =
        '<p><img alt="Logo" src="https://cdn.syncfusion.com/content/images/home-v1/home/webp/home-banner-v6.webp" class="e-rte-image e-imginline"></p>';
      ...
      urlToBlobUrl(imageUrl).then(function (blobUrl) {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          expect(
            editor.inputElement.querySelector('img').src.includes('base64')
          ).toBe(true);
          done();
        }, 100);
      }, 500);
  ...
  describe('885141: RichTextEditor creates P tag after pressing enter key on UL element on selection of DIV as enteraction in EnterKey Configuration', () => {
    ...
    beforeAll(() => {
      rteObj = renderRTE({
        value:
          '<ul><li><div>one</div></li><li><div>two</div></li><li><div class="test">three</div></li></ul>',
        enterKey: 'DIV',
        toolbarSettings: {
          items: ['UnorderedList'],
        },
    ...
    it(' Deselect the "Bulleted List" and add the DIV to the Deselected list item', () => {
      ...
      let targetElm: HTMLElement = rteObj.element.querySelector(
        '.e-toolbar-item button'
      );
      targetElm.click();
      rteObj.dataBind();
      ...
      expect(
        rteObj.inputElement.innerHTML ===
          '<ul><li><div>one</div></li><li><div>two</div></li></ul><div><br></div><div class="test test">three</div>'
      ).toBe(true);
    });
    afterAll((done: DoneFn) => {
      destroy(rteObj);
  ...
  describe('Bug 884738: Auto numbering or bulletin list not working with enterKey as BR in RichTextEditor', () => {
    ...
    it(' to create list when using br', () => {
      rteObj.dataBind();
      setCursorPoint(
        rteObj.inputElement.querySelector('br').nextSibling as Element,
        2
      );
      ...
      (rteObj as any).keyDown(keyBoardEvent);
      expect(
        rteObj.inputElement.innerHTML === 'test<br><ol><li></li></ol>'
      ).toBe(true);
    });
    afterAll((done: DoneFn) => {
      destroy(rteObj);
  ...
  describe('883222 - Tab key press on selected paragraph deletes the entire line in RichTextEditor', () => {
    ...
    it('Select and apply tab key and Shift tab key when enterkey as BR other than startContainer offset ', () => {
      ...
      (rteObj as any).keyDown(keyBoardEvent);
      expect(startElement.innerHTML === 'hello&nbsp;&nbsp;&nbsp;&nbsp;me').toBe(
        true
      );
    });
    ...
    it('Select and apply tab key and using undo and redo', (done) => {
      ...
      setTimeout(() => {
        ...
        (
          (<HTMLElement>(
            rteObj.element.querySelectorAll('.e-toolbar-item')[1]
          )) as HTMLElement
        ).click();
        setTimeout(() => {
          startElement = rteObj.inputElement.querySelector('p');
    ...
    it('Select and apply tab key in list', () => {
      ...
      expect(
        value.innerHTML ===
          `<li><p>Provide\n        the tool bar support, it’s also customizable.</p><ol><li><p id="one">Options\n        to get the HTML elements with styles.</p></li><li><p>Support\n        to insert image from a defined path.</p></li><li id="two"><p>Footer\n        elements and styles(tag / Element information , Action button (Upload, Cancel))</p></li></ol></li>`
      ).toBe(true);
      ...
      expect(
        rteObj.value ===
          '<p id="one"><b>Functional Specifications/Requirements:</b></p><ol><li><p>Provide the tool bar support, it’s also customizable.</p></li><li><p id="two">Options to get the HTML elements with styles.</p></li></ol>'
      ).toBe(true);
    });
    afterEach((done) => {
      destroy(rteObj);
  ...
  describe('887277 - Br tag added along with the image tag while pasting images into the RichTextEditor', () => {
    ...
    it('test for pasting image in pasteCleanup in empty RTE ', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          rteObject.inputElement.innerHTML ===
            `<p><img alt="Logo" src="https://ej2.syncfusion.com/demos/src/rich-text-editor/images/RTEImage-Feather.png" class="e-rte-image e-imginline" style=" border: 0px; vertical-align: bottom; cursor: pointer; display: inline-block; float: none; margin: auto 5px; max-width: 100%; position: relative; padding: 1px; color: rgb(28, 27, 31); font-family: Roboto, -apple-system, BlinkMacSystemFont, &quot;Segoe UI&quot;, &quot;Helvetica Neue&quot;, sans-serif; font-size: 14px; font-style: normal; font-weight: 400; text-align: start; text-indent: 0px; text-transform: none; white-space: normal; background-color: rgb(255, 255, 255); width: 300px;"> </p>`
        ).toEqual(true);
        done();
      }, 400);
    });
  ...
  describe('892829 - Setting layoutOption Break and width 100 percent in insertVideoSettings not working properly in RichTextEditor', () => {
    ...
    it('Check the iframe video element that has applied the styles and classes.', (done: Function) => {
      ...
      let dialogEle: Element = rteObj.element.querySelector('.e-dialog');
      (
        dialogEle.querySelector('.e-embed-video-url') as HTMLInputElement
      ).value = `<iframe width="560" height="315" src="https://www.youtube.com/embed/4U2ZxO7b8iM" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
      (
        dialogEle.querySelector('.e-embed-video-url') as HTMLInputElement
      ).dispatchEvent(new Event('keyup'));
      (
        document.querySelector('.e-insertVideo.e-primary') as HTMLElement
      ).click();
      expect((<any>rteObj).element.querySelector('iframe')).not.toBe(null);
      expect(
        (rteObj.element.querySelector('.e-embed-video-wrap') as HTMLElement)
          .style.display === 'block'
      ).toBe(true);
      expect(
        rteObj.element
          .querySelector('iframe')
          .classList.contains('e-video-break')
      ).toBe(true);
      done();
    });
  });
  describe('890154: Plain text in pasteCleanupSettings adding unwanted styles in the RichTextEditor', () => {
    ...
    it('copy and paste text', () => {
      ...
      rteObj.contentModule.getEditPanel().dispatchEvent(pasteEvent);
      expect(rteObj.contentModule.getEditPanel().innerHTML).toEqual(
        '<p><span>大帽：</span></p><p>檔名：</p>'
      );
    });
    afterAll(() => {
      destroy(rteObj);
  ...
  describe('895384 - The placeholder does not show up after cleaning up all the content in the Rich Text Editor.', () => {
    ...
    it('select all content in rte and press back space ', (done: DoneFn) => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.element.querySelector('h1').childNodes[0],
        rteObj.element.querySelector('h2'),
        0,
        1
      );
      keyBoardEvent.keyCode = 8;
  ...
  describe('894204: Deleting empty line removes the text in the first list in RichTextEditor', () => {
    ...
    beforeAll(() => {
      rteObj = renderRTE({
        ...
        value:
          '<p>Hello</p><p><br></p><ul><li>line 1</li><li>line 2</li><li>line 3</li></ul>',
      });
      rteEle = rteObj.element;
    });
    it('Delete with Empty br node', () => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setCursorPoint(
        document,
        rteObj.inputElement.firstChild.nextSibling as HTMLElement,
        0
      );
      rteObj.dataBind();
      (rteObj as any).keyDown(keyboardEventArgs);
      expect(
        rteObj.inputElement.innerHTML ===
          '<p>Hello</p><p></p><ul><li>line 1</li><li>line 2</li><li>line 3</li></ul>'
      ).toBe(true);
    });
    afterEach((done: DoneFn) => {
      destroy(rteObj);
  ...
  describe('902418 - Indendation does not get removed when the backspace action is performed.', () => {
    ...
    it('Press the backspace on the indent applied text', (done: DoneFn) => {
      ...
      expect(
        (rteObj as any).inputElement.innerHTML ===
          '<p style="">Rich Text Editor</p>'
      ).toBe(true);
      done();
    });
    it('Press the backspace on the indent applied', function (done) {
      (rteObj as any).inputElement.innerHTML =
        '<p style="margin-left: 20px;"><strong>Rich Text Editor</strong></p>';
      ...
      expect(
        (rteObj as any).inputElement.innerHTML ===
          '<p style=""><strong>Rich Text Editor</strong></p>'
      ).toBe(true);
      done();
    });
  });
  ...
  describe('898140 - Delete action inside the list not working properly.', () => {
    ...
    it('select all last two contents of list and press delete ', (done: DoneFn) => {
      ...
      rteObj.formatter.editorManager.nodeSelection.setSelectionText(
        document,
        rteObj.element.querySelector('#one').childNodes[0],
        rteObj.element.querySelector('#two'),
        0,
        1
      );
      keyBoardEventDel.keyCode = 46;
      ...
      setTimeout(() => {
        expect(
          rteObj.inputElement.innerHTML ===
            '<p>hello</p><ul><li>world</li></ul><p>me</p>'
        ).toBe(true);
        done();
      }, 100);
    });
    ...
    it('select all entire of list and press delete  for user case for p tag wrapped', (done: DoneFn) => {
      ...
      setTimeout(() => {
        expect(
          rteObj.inputElement.innerHTML === '<p><b>Key features:</b></p>'
        ).toBe(true);
        done();
      }, 100);
    });
  ...
  describe('900940 - Rich Text Editor not supported the pasted image.', () => {
    ...
    it('Test for pasteCleanup', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          expect(
            rteObject.inputElement
              .querySelector('img')
              .getAttribute('v:shapes')
              .indexOf('img1') <= 0
          ).toBe(true);
          done();
        }, 400);
  ...
  describe('903810 - Ordered list gets removed when list item is copied and pasted into the same list in the Rich Text Editor', () => {
    ...
    it('select all last two contents of list and press delete ', (done: DoneFn) => {
      ...
      setTimeout(() => {
        expect(
          (rteObj.inputElement.firstChild as HTMLElement).innerHTML ===
            `<li id="\\&quot;li2\\&quot;" style="">hello</li><li id="li2">hello</li>`
        ).toBe(true);
        done();
      }, 200);
    });
  ...
  describe('908611 -In localiaztion, same text are used in alternative text quick toolbar item and alternative text dialog header.', () => {
    ...
    it('image dialog quick toolbar alternative text check', (done: Function) => {
      ...
      target.dispatchEvent(clickEvent);
      target = (
        rteObj.contentModule.getEditPanel() as HTMLElement
      ).querySelector('img');
      (rteObj as any).formatter.editorManager.nodeSelection.setSelectionNode(
        rteObj.contentModule.getDocument(),
        target
      ...
      setTimeout(function () {
        let alternateButton = document.getElementById(
          rteEle.id + '_quick_AltText'
        );
        expect(
          alternateButton.getAttribute('aria-label') === 'Alternate Text'
        ).toBe(true);
        done();
      }, 200);
    });
