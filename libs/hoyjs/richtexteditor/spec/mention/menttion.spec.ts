import { Mention } from '@hoyui/dropdowns';
...
describe('Mention integration tests', () => {
  ...
  describe('Mouse action testing ', () => {
    ...
    it('Should open the popup and insert the item on click.', (done: DoneFn) => {
      ...
      const atKeyDownEvent: KeyboardEvent = new KeyboardEvent(
        'keydown',
        AT_CHARACTER_KEY_EVENT_INIT
      );
      editor.inputElement.dispatchEvent(atKeyDownEvent);
      const atKeyUpEvent: KeyboardEvent = new KeyboardEvent(
        'keyup',
        AT_CHARACTER_KEY_EVENT_INIT
      );
      ...
      setTimeout(() => {
        expect(
          document
            .querySelector('.e-mention.e-popup')
            .classList.contains('e-popup-open')
        ).toBe(true);
        (
          document.querySelector('.e-mention.e-popup li') as HTMLElement
        ).click();
        setTimeout(() => {
          expect(
            editor.inputElement.innerHTML ===
              '<p><span contenteditable="false" class="e-mention-chip">Selma Rose</span>​</p>'
          ).toBe(true);
          done();
        }, 200);
      }, 200);
  ...
  describe('Keyboard action Enter action testing ', () => {
    ...
    it('Should open the popup and insert the item using enter key.', (done: DoneFn) => {
      ...
      setTimeout(() => {
        ...
        const enterKeyDownEvent: KeyboardEvent = new KeyboardEvent(
          'keydown',
          ENTERKEY_EVENT_INIT
        );
        document.activeElement.dispatchEvent(enterKeyDownEvent);
        const enterKeyUpEvent: KeyboardEvent = new KeyboardEvent(
          'keyup',
          ENTERKEY_EVENT_INIT
        );
  ...
  describe('905363: User list popup not open when pressing Shift with @ key.', () => {
    ...
    it('Should open the popup properly when the @ character is pressed inside strong with zero width space.', (done: DoneFn) => {
      editor.focusIn();
      setCursorPoint(
        editor.inputElement.firstChild.firstChild.firstChild as Element,
        2
      );
      const atKeyDownEvent: KeyboardEvent = new KeyboardEvent(
        'keydown',
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          const tabKeyDownEvent: KeyboardEvent = new KeyboardEvent(
            'keydown',
            TAB_KEY_EVENT_INIT
          );
          document.activeElement.dispatchEvent(tabKeyDownEvent);
          const tabKeyUpEvent: KeyboardEvent = new KeyboardEvent(
            'keyup',
            TAB_KEY_EVENT_INIT
          );
          ...
          setTimeout(() => {
            expect(
              editor.inputElement.querySelectorAll('.e-mention-chip').length > 0
            ).toBe(true);
            expect(
              editor.inputElement.querySelectorAll('strong').length > 0
            ).toBe(true);
            done();
          }, 200);
        }, 200);
