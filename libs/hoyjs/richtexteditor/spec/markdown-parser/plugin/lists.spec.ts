...
Tabs and shift-tabs work too`;
  ...
  let nestedValue: string = `- Lists are a piece of cake
\t- They even auto continue as you type
\t\t- A double enter will end them
\t- Tabs and shift-tabs work too`;

  let enterValue: string = `# Lists are a piece of cake
They even auto continue as you type
...
Tabs and shift-tabs work too`;
  ...
  describe(' OL testing', () => {
    ...
    it('  apply to multi line   ', () => {
      ...
      editorObj.markdownSelection.restore(textArea);
      editorObj.execCommand('Lists', 'OL', null);
      let lines: string[] = editorObj.markdownSelection.getAllParents(
        textArea.value
      );
  ...
  describe(' OL enter and tab, shift+tab key testing', () => {
    ...
    it(' enter key navigation from nested line  ', () => {
      let textArea: HTMLTextAreaElement =
        editorObj.element as HTMLTextAreaElement;
      ...
      textArea.value =
        textArea.value.substr(0, length + 1) +
        '\n' +
    ...
    it(' shift+tab key navigation from nested line  ', () => {
      ...
      let prevSpace: string = (editorObj.listObj as any).getTabSpace(
        editorObj.markdownSelection.getLine(textArea, 1)
      );
      (editorObj as any).editorKeyDown(keyBoardEvent);
      let curSpace: string = (editorObj.listObj as any).getTabSpace(
        editorObj.markdownSelection.getLine(textArea, 1)
      );
    ...
    it(' tab key navigation with multi lines  ', () => {
      ...
      editorObj.markdownSelection.save(
        firstLine.length + 1,
    ...
    it(' shift+tab key navigation with nested last line  ', () => {
      ...
      editorObj.execCommand('Lists', 'OL', null);
      editorObj.markdownSelection.save(
        textArea.value.length - 6,
        textArea.value.length - 4
      );
      editorObj.markdownSelection.restore(textArea);
      keyBoardEvent.event.shiftKey = true;
      (editorObj as any).editorKeyDown(keyBoardEvent);
      let index: number = editorObj.markdownSelection.getLineNumber(
        textArea,
        (editorObj.element as HTMLTextAreaElement).selectionStart
      );
...
number five`;
    ...
    it('  apply to multi line   ', () => {
      ...
      for (let i: number = 0; lines[i] !== '' && i < lines.length; i++) {
        expect(new RegExp('^(' + (i + 1) + '. )', 'gim').test(lines[i])).toBe(
          true
        );
      }
    });
...
four
five`;
    let nestedValue: string = `1. one
\t1. two
\t\t1. three
4. four
5. five`;
    let shiftnestedValue: string = `1. one
\t1. two
2. three`;

    let enterTestValue: string = `1. one`;

    let enterTestValueMultiLine: string = `1. one


Line with more than one break line before`;

    let enterMultiValue: string = `1. one para`;
    let editorObj: MarkdownParser;
    let textArea: HTMLTextAreaElement = <HTMLTextAreaElement>(
      createElement('textarea', {
        id: 'markdown-editor',
    ...
    it(' enter key navigation in the middle of the list', () => {
      ...
      editorObj.markdownSelection.restore(textArea);
      textArea.value =
        textArea.value.substr(0, length - 4) +
        '\n' +
