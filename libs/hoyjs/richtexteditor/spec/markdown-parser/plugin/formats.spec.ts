...
Tabs and shift-tabs work too`;
  ...
  describe(' Pre Formats testing', () => {
    ...
    it(' apply Pre formats ', () => {
      ...
      expect(isCallBack).toBe(true);
      let lineNumber: number = editorObj.markdownSelection.getLineNumber(
        textArea,
        2
      );
      let line: string = editorObj.markdownSelection.getSelectedLine(textArea);
      let prev: string = editorObj.markdownSelection.getLine(
        textArea,
        lineNumber - 1
      );
      let next: string = editorObj.markdownSelection.getLine(
        textArea,
        lineNumber + 2
      );
