...
describe('DOMNode plugin', () => {
  ...
  describe('node public methods testing', () => {
    ...
    it('contents method', () => {
      ...
      expect(domNode.contents(pEle.childNodes[0] as Element).length === 0).toBe(
        true
      );
    ...
    it('attributes method', () => {
      let spanEle: HTMLElement = elem.querySelector('span');
      expect(
        domNode.attributes(spanEle) ===
          ' class="e-node-deletable e-node-inner" contenteditable="true" style="color:red;"'
      ).toBe(true);
      expect(domNode.attributes() === '').toBe(true);
      let olEle: HTMLElement = elem.querySelector('ol');
      let attr: string = domNode.attributes(olEle);
      let newElement: Element = domNode.parseHTMLFragment(
        '<span' + attr + '></span>'
      );
      expect(
        Object.keys(domNode.rawAttributes(newElement.firstElementChild))
          .length == 2
      ).toBe(true);
    });
    ...
    it('openTagString method', () => {
      ...
      expect(
        domNode.openTagString(spanEle) ===
          '<span class="e-node-deletable e-node-inner" contenteditable="true" style="color:red;">'
      ).toBe(true);
    });
    ...
    it('createTagString method', () => {
      let spanEle: HTMLElement = elem.querySelector('span');
      let matchStr: String =
        '<h4 class="e-node-deletable e-node-inner" contenteditable="true" style="color:red;">h4 format</h4>';
      expect(
        domNode.createTagString('h4', spanEle, 'h4 format') === matchStr
      ).toBe(true);
    });
    ...
    it('replaceWith method', () => {
      ...
      expect(
        spanEle.querySelector('span').textContent === 'replace content'
      ).toBe(true);
    });
    ...
    it('getRangePoint method', () => {
      ...
      expect(ranges[0].startOffset == 0 && ranges[0].endOffset === 0).toBe(
        true
      );

      domSelection.setSelectionNode(document, start.childNodes[0]);
      ranges = <Range[]>domNode.getRangePoint();
      expect(ranges[0].startOffset == 0 && ranges[0].endOffset === 20).toBe(
        true
      );
    });
    ...
    it('wrapInner method', () => {
      ...
      let wrapper: Element = (start.childNodes[1] as Element).querySelector(
        'li'
      ).childNodes[0] as Element;
      domNode.wrapInner(
        wrapper,
        domNode.parseHTMLFragment('<span class="e-rte-wrap-inner"></span>')
      );
    ...
    it('insertAfter method', () => {
      let edit: HTMLElement = elem.querySelector('[contenteditable="true"]');
      let start: Element = edit.querySelector('ol');
      let reference: Element = (start.childNodes[1] as Element).querySelector(
        'li'
      ).childNodes[0] as Element;
      domNode.insertAfter(
        domNode.parseHTMLFragment(
          '<span class="insert-after-tag">Insert Tag</span>'
        ),
        reference
      );
      expect(
        (reference.nextSibling as Element).classList.contains(
          'insert-after-tag'
        )
      ).toBe(true);
    });
    it('parseHTMLFragment method', () => {
      let element: Element = domNode.parseHTMLFragment(
        '<span id="new-frag">replace content</span>'
      );
      elem.appendChild(element);
      expect(
        elem.querySelector('#new-frag').textContent === 'replace content'
      ).toBe(true);
    });
  ...
  describe('850070 - Number list not applied properly to the paragraph content when the content is inside the table', () => {
    ...
    it('Should apply the number list properly CASE 1 With Selection', () => {
      let currentTable: HTMLElement = editor.inputElement.querySelectorAll(
        'table'
      )[0] as HTMLElement;
      ...
      editor.dataBind();
      currentTable = editor.inputElement.querySelectorAll(
        'table'
      )[0] as HTMLElement;
  ...
  describe('850070 - Number list not applied properly to the paragraph content when the content is inside the table - Case 2', () => {
    let editor: RichTextEditor;
    const content: string =
      '<table class="e-rte-table" style="width: 18.7166%; min-width: 0px; height: 159px;"><tbody><tr style="height: 99.3711%;"><td class="" style="width: 99.7143%;">Item 1&nbsp;<br>Item 2<br>Item 3<br><br><br><br></td></tr></tbody></table><p><br></p>';
    function applyFormat(): void {
      const wrapper: HTMLElement = editor.element.querySelectorAll(
        '.e-toolbar-item'
      )[0] as HTMLElement;
    ...
    it('Should not remove the BR element when applying format', () => {
      ...
      expect(tdElem.querySelectorAll('br').length).toBe(6);
      expect(
        tdElem.querySelectorAll('br')[5].parentElement.nodeName === 'P'
      ).toBe(true); // Issue
    });

    it('Should apply Heading format properly on Empty TD', () => {
      editor.value =
        '<table class="e-rte-table" style="width: 100%; min-width: 0px;"><tbody><tr><td class="" style="width: 100%;"><br></td></tr></tbody></table>';
      editor.dataBind();
      let currentTable: HTMLElement = editor.inputElement.querySelectorAll(
        'table'
      )[0] as HTMLElement;
    ...
    it('Should apply Unordered List properly on In between text', () => {
      editor.value =
        '<table class="e-rte-table" style="width: 100%; min-width: 0px;"><tbody><tr><td class="" style="width: 100%;">The Rich<strong> Text</strong> Editor.</td></tr></tbody></table><p><br></p>';
      editor.dataBind();
      let currentTable: HTMLElement = editor.inputElement.querySelectorAll(
        'table'
      )[0] as HTMLElement;
      ...
      expect(
        tdElem.innerHTML ===
          '<ul><li>The Rich<strong> Text</strong> Editor.</li></ul>'
      ).toBe(true);
    });
  });
  ...
  describe('875147 - Number or Bullet format list not applied properly and throws error on continuous click in RichTextEditor', () => {
    let editor: RichTextEditor;
    const content: string =
      '<ol><li><p class="textPtag">Provides an option to customize the quick toolbar for an image </p></li></ol><p class="imgPtag"><img alt="Logo" src="https://ej2.syncfusion.com/demos/src/rich-text-editor/images/RTEImage-Feather.png" style="width: 300px;" class="e-rte-image e-imginline"></p>';
    beforeEach(() => {
      editor = renderRTE({
        toolbarSettings: {
    ...
    it('Check the unwanted empty elements maintain dom', () => {
      ...
      domSelection.setSelectionText(
        document,
        textPtag.firstChild,
        imgPtag.parentElement,
        0,
        1
      );
      let numberlist: HTMLElement = editor
        .getToolbar()
      ...
      expect(
        editor.inputElement.innerHTML ===
          `<p class="textptag">Provides an option to customize the quick toolbar for an image </p><p><img alt="Logo" src="https://ej2.syncfusion.com/demos/src/rich-text-editor/images/RTEImage-Feather.png" style="width: 300px;" class="e-rte-image e-imginline"></p>`
      ).toBe(true);
    });
  });
});
