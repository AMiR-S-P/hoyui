 ...
 */
import { detach, Browser } from '@hoyui/base';
...
describe('Selection commands', () => {
  ...
  let innervalue: string =
    '<div id="div1"><p id="paragraph1"><b>Description:</b></p>' +
  ...
  it('Apply subscript tag for multiple nodes', () => {
    ...
    expect(ptag.childNodes[0].childNodes[0].nodeName.toLowerCase()).toEqual(
      'sub'
    );
  });
  it('Apply underline tag for multiple nodes', () => {
    ...
    expect(
      ptag.childNodes[0].childNodes[0].childNodes[0].nodeName.toLowerCase()
    ).toEqual('span');
    expect(
      (ptag.childNodes[0].childNodes[0].childNodes[0] as HTMLElement).style
        .textDecoration
    ).toEqual('underline');
  });
  it('Apply strikethrough tag for multiple nodes', () => {
    ...
    expect(
      ptag.childNodes[0].childNodes[0].childNodes[0].childNodes[0].nodeName.toLowerCase()
    ).toEqual('span');
    expect(
      (
        ptag.childNodes[0].childNodes[0].childNodes[0]
  ...
  it('Apply Italic tag for multiple nodes', () => {
    SelectionCommands.applyFormat(document, 'italic', parentDiv, 'P');
    expect(
      ptag.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].nodeName.toLowerCase()
    ).toEqual('em');
  });
  it('Revert Italic tag for multiple nodes', () => {
    SelectionCommands.applyFormat(document, 'italic', parentDiv, 'P');
    expect(
      ptag.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0]
        .nodeName
    ).toEqual('#text');
  });
  it('Revert superscript tag for multiple nodes', () => {
    SelectionCommands.applyFormat(document, 'superscript', parentDiv, 'P');
    expect(
      ptag.childNodes[0].childNodes[0].childNodes[0].childNodes[0].nodeName
    ).toEqual('#text');
  });
  it('Revert strikethrough tag for multiple nodes', () => {
    SelectionCommands.applyFormat(document, 'strikethrough', parentDiv, 'P');
    expect(ptag.childNodes[0].childNodes[0].childNodes[0].nodeName).toEqual(
      '#text'
    );
  });
  ...
  it('Apply fontcolor tag for text node', () => {
    ...
    expect((node1.childNodes[0] as HTMLElement).style.color).toEqual(
      'rgb(102, 102, 0)'
    );
    expect((node1.childNodes[0] as HTMLElement).nodeName.toLowerCase()).toEqual(
      'span'
    );
  });
  it('Apply fontname tag for text node', () => {
    ...
    expect((ptag.childNodes[0] as HTMLElement).style.fontFamily).toEqual(
      'Arial'
    );
    expect((ptag.childNodes[0] as HTMLElement).nodeName.toLowerCase()).toEqual(
      'span'
    );
  });
  ...
  it('Apply backgroundcolor tag for text node', () => {
    ...
    expect(
      (ptag.childNodes[0].childNodes[0] as HTMLElement).style.backgroundColor
    ).toEqual('rgb(246, 198, 206)');
    expect(
      (ptag.childNodes[0].childNodes[0] as HTMLElement).nodeName.toLowerCase()
    ).toEqual('span');
  });
  it('Apply uppercase tag for text node', () => {
    SelectionCommands.applyFormat(document, 'uppercase', parentDiv, 'P');
    expect(
      ptag.childNodes[0].childNodes[0].childNodes[0].childNodes[0].textContent
    ).toEqual('THE RICH TEXT EDITOR (RTE)');
  });
  ...
  it('Re - Apply fontname tag for text node', () => {
    ...
    expect(
      (
        ptag.childNodes[0].childNodes[0].childNodes[0] as HTMLElement
  ...
  it('Apply backgroundcolor tag for already applied specific text node', () => {
    ...
    expect(
      (
        ptag.childNodes[0].childNodes[0].childNodes[0].childNodes[1]
  ...
  it('Apply uppercase tag for already applied specific text node', () => {
    SelectionCommands.applyFormat(document, 'uppercase', parentDiv, 'P');
    expect(
      ptag.childNodes[0].childNodes[0].childNodes[0].childNodes[1].childNodes[0]
  ...
  it('Apply Bold tag for span style node', () => {
    ...
    SelectionCommands.applyFormat(document, 'bold', parentDiv, 'P');
    expect(
      document.getElementById('boldparent').childNodes[0].textContent
    ).toEqual('the');
  });
  it('Apply Italic tag for span style node', () => {
    ...
    SelectionCommands.applyFormat(document, 'italic', parentDiv, 'P');
    expect(
      document.getElementById('italicparent').childNodes[0].textContent
    ).toEqual('the');
  });
  ...
  it('Apply Bold tag for cursor position 2', () => {
    ...
    SelectionCommands.applyFormat(document, 'bold', parentDiv, 'P');
    expect(
      document.getElementById('cursor4').childNodes[1].nodeName.toLowerCase()
    ).toEqual('#text');
  });
  ...
  it('Cursor pointer multiple style with empty node applied issue', () => {
    ...
    SelectionCommands.applyFormat(document, 'bold', parentDiv, 'P');
    expect(
      (node1 as HTMLElement).children[0].textContent.match(regEx)
    ).not.toBe(null);
    SelectionCommands.applyFormat(document, 'italic', parentDiv, 'P');
    expect(
      (node1 as HTMLElement).children[0].children[0].textContent.match(regEx)
    ).not.toBe(null);
    SelectionCommands.applyFormat(document, 'underline', parentDiv, 'P');
    expect(
      (node1 as HTMLElement).children[0].children[0].textContent.match(regEx)
  ...
  it('Cursor pointer multiple style with textnode applied issue', () => {
    ...
    SelectionCommands.applyFormat(document, 'bold', parentDiv, 'P');
    expect(
      document.getElementById('format5').nextSibling.nodeName.toLowerCase()
    ).toEqual('em');
    expect(
      document
        .getElementById('format5')
        .nextSibling.childNodes[0].nodeName.toLowerCase()
    ).toEqual('u');
  });
  it('transparent background color not apllied issue', () => {
    ...
    expect(
      (document.getElementById('format6').childNodes[1] as HTMLElement).style
        .color
    ).toEqual('');
  });
  it('Apply fontsize tag for list elements', () => {
    ...
    expect(
      document.getElementById('paragraph20').parentElement.style.fontSize
    ).toEqual('36px');
    expect(
      document.getElementById('paragraph21').parentElement.style.fontSize
    ).toEqual('36px');
    expect(
      document.getElementById('paragraph22').parentElement.style.fontSize
    ).toEqual('36px');
    expect(
      document.getElementById('paragraph23').parentElement.style.fontSize
    ).toEqual('36px');
    expect(
      document.getElementById('paragraph24').parentElement.style.fontSize
    ).toEqual('36px');
    expect(
      document.getElementById('paragraph25').parentElement.style.fontSize
    ).toEqual('36px');
    expect(
      document.getElementById('paragraph26').parentElement.style.fontSize
    ).toEqual('36px');
  });
  it('Apply fontcolor tag for list elements', () => {
    ...
    expect(
      document.getElementById('paragraph30').parentElement.style.color
    ).toEqual('rgb(83, 129, 53)');
    expect(
      document.getElementById('paragraph31').parentElement.style.color
    ).toEqual('rgb(83, 129, 53)');
    expect(
      document.getElementById('paragraph32').parentElement.style.color
    ).toEqual('rgb(83, 129, 53)');
    expect(
      document.getElementById('paragraph33').parentElement.style.color
    ).toEqual('rgb(83, 129, 53)');
    expect(
      document.getElementById('paragraph34').parentElement.style.color
    ).toEqual('rgb(83, 129, 53)');
    expect(
      document.getElementById('paragraph35').parentElement.style.color
    ).toEqual('rgb(83, 129, 53)');
  });
  ...
  it('Apply fontsize tag for list elements', () => {
    ...
    expect(
      (
        document.getElementById('paragraph20').firstElementChild as HTMLElement
      ).tagName.toLowerCase() === 'span'
    ).toBe(true);
    expect(
      (document.getElementById('paragraph20').childNodes[1] as HTMLElement)
        .style.fontSize
    ).toEqual('10px');
    expect(
      document.getElementById('paragraph21').parentElement.style.fontSize
    ...
    expect(
      document
        .getElementById('paragraph26')
        .firstElementChild.tagName.toLowerCase() === 'span'
    ).toBe(true);
    expect(
      (document.getElementById('paragraph26').firstElementChild as HTMLElement)
        .style.fontSize
    ).toEqual('10px');
  });
  it('Apply fontsize tag for list elements with space', () => {
    ...
    expect(
      document.getElementById('paragraph27').parentElement.style.fontSize
    ).toEqual('36px');
    expect(
      document.getElementById('paragraph28').parentElement.style.fontSize
    ).toEqual('36px');
  });
...
describe('EJ2-52289- Textcolor is removed for the range node, when removing the formats', () => {
  ...
  beforeAll((done: Function) => {
    rteObj = renderRTE({
      ...
      created: function () {
        ...
        italicItem = document.body.querySelector(
          '#' + rteID + '_toolbar_Italic'
        );
        underlineItem = document.body.querySelector(
          '#' + rteID + '_toolbar_Underline'
        );
      },
    });
    done();
  ...
  it('Checking the fontColor for the node not being removed', (done) => {
    rteObj.formatter.editorManager.nodeSelection.setCursorPoint(
      document,
      rteObj.inputElement.lastElementChild.firstElementChild.lastElementChild
        .lastElementChild.lastElementChild.firstChild,
      rteObj.inputElement.lastElementChild.firstElementChild.lastElementChild
        .lastElementChild.lastElementChild.firstChild.textContent.length
    );
    underlineItem.click();
    italicItem.click();
    boldItem.click();
    setTimeout(() => {
      expect(
        (rteObj as any).inputElement.children[1].firstElementChild.style.color
      ).toBe('rgb(68, 114, 196)');
      done();
    }, 200);
  });
...
describe('EJ2-57778- Console error occurs and format not applied, when removing the formats', () => {
  ...
  it('when removing all formats in the editor', (done) => {
    ...
    expect((rteObj as any).inputElement.innerHTML).toBe(
      '<p><strong>​<em>​<span style="text-decoration: underline;">Testing</span></em></strong><em><span style="text-decoration: underline;">​</span></em></p>'
    );
    italicItem.click();
    expect((rteObj as any).inputElement.innerHTML).toBe(
      '<p><strong>​<em>​<span style="text-decoration: underline;">Testing</span></em></strong><span style="text-decoration: underline;">​</span></p>'
    );
    underlineItem.click();
    expect((rteObj as any).inputElement.innerHTML).toBe(
      '<p><strong>​<em>​<span style="text-decoration: underline;">Testing</span></em></strong>​</p>'
    );
    done();
  });
});
...
describe('EJ2-57778- Console error occurs, when removing the particular format', () => {
  ...
  it('when adding/removing bold format', (done) => {
    rteObj.formatter.editorManager.nodeSelection.setCursorPoint(
      document,
      rteObj.inputElement.lastElementChild.lastChild,
      7
    );
    boldItem.click();
    expect((rteObj as any).inputElement.innerHTML).toBe(
      '<p>Testing<strong>​</strong></p>'
    );
    boldItem.click();
...
describe('EJ2-59075 - The font name is not getting properly while loading custom font ', () => {
  ...
  it('The font name is not changed properly issue - EJ2-59075 ', (done) => {
    ...
    expect((rteObj as any).inputElement.innerHTML).toBe(
      `<p><span class="focusNode" style="font-family: Arial;"><br></span></p>`
    );
    done();
  });
});
...
describe('EJ2-60277 - Formatting is not maintained properly while unselecting the strikethrough style', () => {
  ...
  it('Formatting is not maintained properly while unselecting the strikethrough style', (done) => {
    ...
    expect((rteObj as any).inputElement.innerHTML).toBe(
      `<p><span style="text-decoration: underline;">​<span style="text-decoration: line-through;" class="focusNode">RTE Content</span>​</span></p>`
    );
    done();
  });
});
...
describe('EJ2-58803 - Styles format not maintain properly when applied different formats Xamarin reported', () => {
  ...
  it('Case 2 of the formating issue - EJ2-58803 ', (done) => {
    ...
    expect((rteObj as any).inputElement.innerHTML).toBe(
      `<p><strong>​<em>​<span style="text-decoration: underline;">​<span class="focusNode" style="color: rgb(255, 0, 0); text-decoration: inherit;">RTE Content</span></span></em></strong><span class="focusNode" style="color: rgb(255, 0, 0); text-decoration: inherit;"><span style="background-color: rgb(246, 198, 206);">​</span></span></p>`
    );
    done();
  });
});
...
describe('Selection Testing with Multiple nodes', () => {
  //HTML value
  let innervalue: string =
    '<p><strong>​<em>​<span style="text-decoration: underline;">​Testing</span></em>' +
  ...
  it('Checking the nodes innerHTML', (done) => {
    ...
    rteObj.formatter.editorManager.nodeSelection.setSelectionText(
      document,
      rteObj.inputElement.childNodes[1],
      rteObj.inputElement.childNodes[1],
      0,
      0
    );
    boldItem.click();
    expect(
      rteObj.inputElement.childNodes[1].firstElementChild.tagName.toLowerCase()
    ).not.toBe('strong');
    italicItem.click();
    expect(
      rteObj.inputElement.childNodes[1].firstElementChild.tagName.toLowerCase()
...
describe('Removing multiple strong node Testing', () => {
  //HTML value
  let innervalue: string =
    '<p><strong><strong>Testing</br></strong></strong></p>';
  ...
  it('Checking the multiple strong tags removal', (done) => {
    ...
    rteObj.formatter.editorManager.nodeSelection.setSelectionText(
      document,
      rteObj.inputElement.childNodes[0].childNodes[0].childNodes[0]
        .childNodes[0],
      rteObj.inputElement.childNodes[0].childNodes[0].childNodes[0]
        .childNodes[0],
      0,
...
describe('Removing multiple strong and em nodes Testing', () => {
  //HTML value
  let innervalue: string =
    '<strong><em><strong><em id="multiple">Testing</em></strong><br></em></strong>';
  ...
  it('Checking the multiple strong and em tags removal', (done) => {
    ...
    rteObj.formatter.editorManager.nodeSelection.setSelectionText(
      document,
      rteObj.inputElement.childNodes[0].querySelector('#multiple')
        .childNodes[0],
      rteObj.inputElement.childNodes[0].querySelector('#multiple')
        .childNodes[0],
      0,
    ...
    italicItem.click();
    expect(
      rteObj.inputElement.childNodes[0].querySelectorAll('em').length
    ).toBe(1);
    boldItem.click();
    ...
    expect(
      rteObj.inputElement.childNodes[0].querySelectorAll('strong').length
    ).toBe(1);
    done();
  });
});
...
describe('EJ2-52390 - When using the list which contains multiple spans inside will create a new list while applying background color', () => {
  ...
  it('Applying background color for the range li nodes', (done) => {
    rteObj.formatter.editorManager.nodeSelection.setSelectionText(
      document,
      rteObj.inputElement.children[0].firstElementChild.firstElementChild
        .firstElementChild.firstChild.firstChild,
      rteObj.inputElement.children[0].firstElementChild.lastElementChild
        .firstElementChild.firstChild.firstChild,
      0,
      125
    );
    expect(
      (rteObj as any).inputElement.children[0].firstElementChild.children[0]
    ...
    expect(
      (rteObj as any).inputElement.children[0].firstElementChild.children[0]
        .firstElementChild.style.backgroundColor
    ).toBe('rgb(246, 198, 206)');
    expect(
      (rteObj as any).inputElement.children[0].firstElementChild.children[1]
        .firstElementChild.style.backgroundColor
    ).toBe('rgb(246, 198, 206)');
    expect(
      (rteObj as any).inputElement.children[0].firstElementChild.children[2]
        .firstElementChild.style.backgroundColor
    ).toBe('rgb(246, 198, 206)');
    expect(
      (rteObj as any).inputElement.children[0].firstElementChild.children[3]
        .firstElementChild.style.backgroundColor
    ).toBe('rgb(246, 198, 206)');
    done();
  });
...
describe('Remove Br tags when applying formatting', () => {
  ...
  it('if value is empty', (done) => {
    rteObj.formatter.editorManager.nodeSelection.setSelectionText(
      document,
      rteObj.inputElement.childNodes[0],
      rteObj.inputElement.childNodes[0],
      0,
      0
    );
    let boldItem: HTMLElement = rteObj.element.querySelector(
      '#' + controlId + '_toolbar_Bold'
    );
    boldItem.click();
    expect(
      rteObj.inputElement.childNodes[0].firstElementChild.tagName.toLowerCase()
    ).not.toBe('br');
    done();
  });
});
...
describe('Bold the content inside table in fire fox', () => {
  ...
  it('Apply bold to the content inside the table testing in firefox', () => {
    ...
    SelectionCommands.applyFormat(document, 'bold', parentDiv, 'P');
    expect(node1.childNodes[0].childNodes[0].nodeName.toLowerCase()).toEqual(
      'strong'
    );
  });
});
...
describe('862912 - Bold is not applied when using shortcut key (Ctrl+B)', () => {
  ...
  it('Bold is not applied when using shortcut key (Ctrl+B) at the end of the first link', () => {
    ...
    expect(
      document
        ...
        .nextElementSibling.nodeName.toLowerCase()
    ).toEqual('strong');
  });
  ...
  it('Bold is not applied when using shortcut key (Ctrl+B) at the start of the first link', () => {
    ...
    expect(
      document
        .getElementById('lastLink')
        .previousElementSibling.nodeName.toLowerCase()
    ).toEqual('strong');
  });
});
...
describe('EJ2-46060: List not generated after enter key press and bold format changed', () => {
  ...
  it(' Apply list', () => {
    rteObj = renderRTE({
      value: '<p><strong>a</strong><br></p><p><strong><br></strong></p>',
    });
    ...
    (
      rteObj.element.querySelectorAll('.e-toolbar-item')[7] as HTMLElement
    ).click();
    expect(
      (
        rteObj.element.querySelector('.e-content') as HTMLElement
      ).innerHTML.replace(/\uFEFF/g, '')
    ).toBe('<p><strong>a</strong><br></p><ol><li><br></li></ol>');
  });
  afterEach(() => {
    destroy(rteObj);
  });
...
describe('EJ2-46956: Applying background color to multiple span element does not work properly', () => {
  ...
  it(' Apply transparent first parent', () => {
    ...
    let backgroundColorPicker: HTMLElement = <HTMLElement>(
      rteEle.querySelectorAll('.e-toolbar-item .e-dropdown-btn')[0]
    );
    backgroundColorPicker.click();
    let noColorItem: HTMLElement = <HTMLElement>(
      document.querySelector('.e-nocolor-item')
    );
    noColorItem.click();
    expect(
      rteObj.element.querySelectorAll('.e-content span')[0].style
        .backgroundColor
    ).toBe('transparent');
  });
  it(' Apply transparent to text selection', () => {
    ...
    domSelection.setSelectionText(
      document,
      startSpan.childNodes[0],
      endSpan.childNodes[0],
      1,
      1
    );
    rteObj.notify('selection-save', {});
    ...
    noColorItem.click();
    expect(
      rteObj.element.querySelectorAll('.e-content span > span')[2].style
        .backgroundColor
    ).toBe('transparent');
    expect(
      rteObj.element.querySelectorAll('.e-content span > span')[4].style
        .backgroundColor
    ).toBe('transparent');
    expect(
      rteObj.element.querySelectorAll('.e-content span > span')[6].style
        .backgroundColor
    ).toBe('transparent');
  });
  afterEach(() => {
...
describe('EJ2-55031 - Selecting some texts and applying Font and background colors alternatively, will create some new elements Issue', () => {
  ...
  it('EJ2-55031 - Selecting some texts and applying Font and background colors alternatively, will create some new elements', () => {
    ...
    domSelection.setSelectionText(
      ...
      endSpan.childNodes[0],
      0,
      endSpan.childNodes[0].textContent.length
    );
    rteObj.notify('selection-save', {});
    let fontColorPicker: HTMLElement = <HTMLElement>(
      rteEle.querySelectorAll('.e-toolbar-item .e-dropdown-btn')[0]
    );
    fontColorPicker.click();
    let blackItem: HTMLElement = <HTMLElement>(
      document.querySelector('.e-nocolor-item').nextElementSibling
    );
    blackItem.click();
    expect(rteObj.element.querySelectorAll('p')[2].textContent).toBe(
...
describe('Bug 901952: In Rich Text Editor, style is inconsistently maintained', () => {
  ...
  it(' To validate the strikethrough and fontsize combination', () => {
    rteObj = renderRTE({
      ...
      created: function () {
        rteID = document.body.querySelector('.e-richtexteditor').id;
        strikethroughItem = document.body.querySelector(
          '#' + rteID + '_toolbar_StrikeThrough'
        );
      },
    });
    rteObj.inputElement.focus();
    ...
    fontSizePicker.click();
    var fontSizeChooser: HTMLElement = <HTMLElement>(
      document.querySelectorAll('.e-item')[7]
    );
    fontSizeChooser.click();
    expect(rteObj.inputElement.innerHTML).toBe(
      '<p><span style="font-size: 36pt;"><span style="text-decoration: line-through;">​</span></span></p>'
    );
  });
  afterEach(() => {
    destroy(rteObj);
...
describe('EJ2-70405 - Background Color not applied properly when nested styles are applied', () => {
  ...
  it('Test for background color application of selected text node', () => {
    ...
    const allSpanNodes = rteObj.element
      .querySelector('.e-content')
    ...
    // Test bold
    expect(
      rteObj.element.querySelector('.e-content').querySelectorAll('strong')
        .length
    ).toEqual(1);
    // Test italic
    expect(
      rteObj.element.querySelector('.e-content').querySelectorAll('em').length
    ).toEqual(1);
    // Test underline
...
describe('EJ2-69958 - Font size fails to works properly after applying the subscript or superscript', () => {
  ...
  it('EJ2-69958 - update the font size after applying the subscript or superscript', () => {
    ...
    domSelection.setRange(document, range);
    let SubscriptPicker: HTMLElement = <HTMLElement>(
...
describe('EJ2-70405 - Background Color not applied properly when nested styles are applied', () => {
  ...
  it('Test for background color application of selected text node', () => {
    const range: Range = document.createRange();
    range.setStart(
      rteObj.element.querySelector('.e-img-inner').childNodes[0],
      0
    );
...
describe('Reverting font color to the list element.', () => {
  ...
  it('Should remove the color from link and then the span element.', () => {
    ...
    range.setEnd(
      spanELemCollection[2].firstChild,
      spanELemCollection[2].firstChild.textContent.length
    );
    domSelection.setRange(document, range);
    SelectionCommands.applyFormat(
      document,
    ...
    const updatedSpanElemCollection =
      rteObj.inputElement.querySelectorAll('span');
...
describe('872185 - Font family does not applied properly in nested list', () => {
  ...
  it('Test for font name for select all text node', () => {
    ...
    let fristLI: HTMLElement = rteObj.element.querySelector('.li1');
    expect(fristLI.style.fontFamily === 'Impact, Charcoal, sans-serif').toEqual(
      true
    );
  });
});
...
describe(' 873091 - Hyperlinks got removed when we apply font color to the link text in RichTextEditor', () => {
  ...
  it('Test for font color applied anchor element', () => {
    ...
    expect(
      (anchorNode as HTMLElement).style.color === 'rgb(102, 102, 0)'
    ).toEqual(true);
  });
  it('Test for backgroundColor applied strong element', () => {
    ...
    expect(
      (strongNode as HTMLElement).parentElement.style.color ===
        'rgb(102, 102, 0)'
    ).toEqual(true);
  });
  it('Test for background color applied italic element', () => {
    ...
    expect(
      (bgNode as HTMLElement).parentElement.style.backgroundColor ===
        'rgb(102, 102, 0)'
    ).toEqual(true);
  });
  it('Test for fontSize applied strong element', () => {
    ...
    expect(
      (fontSizeNode as HTMLElement).parentElement.style.fontSize === '16pt'
    ).toEqual(true);
  });
  it('Test for fontFamily applied italic element', () => {
    ...
    expect(
      (fontFamNode as HTMLElement).parentElement.style.fontFamily ===
        'Tahoma, Geneva, sans-serif'
    ).toEqual(true);
  });
});
...
describe('888161 - Font color applied to full line instead of selected text', () => {
  ...
  it('select text and apply background color and font color', (done) => {
    ...
    expect((rteObj as any).inputElement.innerHTML).toBe(
      `<ul><li>hello <span style="color: rgb(102, 102, 0); text-decoration: inherit;"><span style="background-color: rgb(255, 255, 0);">word </span></span>this is me</li></ul>`
    );
    done();
  });
});
...
describe('888243 - Single sentence break into double sentence', () => {
  ...
  it('select text and apply background color and font color', () => {
    ...
    expect(
      focusNode.innerHTML ===
        `<span style="font-size: 18px;">​</span>hello world this is me`
    ).toBe(true);
  });
});
describe(' - feature for code format', () => {
  ...
  it('select text and apply code format', (done) => {
    ...
    expect((rteObj as any).inputElement.innerHTML).toBe(
      `<p><code>Hello world this is the sample for code feature .</code></p>`
    );
    SelectionCommands.applyFormat(document, 'inlinecode', focusNode, 'P');
    expect((rteObj as any).inputElement.innerHTML).toBe(
      `<p>Hello world this is the sample for code feature .</p>`
    );
    done();
  });
  it('select text apply code format using and checking toolbar status', (done) => {
    ...
    let codeButton: HTMLElement = document.querySelectorAll(
      '.e-toolbar-item'
    )[0] as HTMLElement;
    codeButton.click();
