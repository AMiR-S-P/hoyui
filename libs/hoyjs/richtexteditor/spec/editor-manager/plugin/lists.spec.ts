...
<ol class='ol-first-node'>
  ...
  describe('left indent testing', () => {
    ...
    beforeEach(() => {
      elem = createElement('div', {
        id: 'dom-node',
        innerHTML: leftindentHTML.trim(),
      });
      document.body.appendChild(elem);
      editorObj = new EditorManager({
        document: document,
        editableElement: document.getElementById('content-edit'),
      });
    ...
    it(' apply the left indent to the first LI element', function () {
      ...
      startNode = startNode.childNodes[0].childNodes[0].childNodes[0]
      ...
      expect(
        (editorObj as any).nodeSelection.range.startContainer.textContent ===
          startNode.textContent
      ).toBe(true);
      expect(
        (editorObj as any).nodeSelection.range.endContainer.textContent ===
          startNode.textContent
      ).toBe(true);
    ...
    it(' apply the left indent to the multiple LI element from second element with all elements are within the same parent OL', function () {
      ...
      startNode = startNode.childNodes[0].childNodes[0].childNodes[1]
        .childNodes[0].childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[0].childNodes[0].childNodes[2].childNodes[0]
      ...
      startNode = editNode.querySelector('.ol-second-node');
      expect((startNode.childNodes[1] as HTMLElement).tagName === 'LI').toBe(
        true
      );
      expect((startNode.childNodes[2] as HTMLElement).tagName === 'LI').toBe(
        true
      );
      editorObj.nodeSelection.Clear(document);
    });
    ...
    it(' apply the left indent to the one LI element with OL as child element and selecting all elements', function () {
      ...
      startNode = startNode.childNodes[0].childNodes[0].childNodes[0]
        .childNodes[0].childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[0].childNodes[0].childNodes[0].childNodes[1]
        .childNodes[1].childNodes[0].childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
        endNode,
    ...
    it(' apply the left indent to the one LI element with OL as child element and selecting first two elements', function () {
      ...
      startNode = editNode.querySelector('.ol-third-node');
      expect(
        startNode.childNodes[0].childNodes[1].childNodes.length === 1
      ).toBe(true);
    ...
    it(' apply the left indent to the one LI element with OL as child element and selecting first(one) elements in the LI', function () {
      ...
      startNode = startNode.childNodes[0].childNodes[1]
        .childNodes[0] as HTMLElement;
    ...
    it(' apply the left indent to the one LI element with OL as child element and selecting first elements in the OL ', function () {
      ...
      endNode = editNode.querySelector('.ol-third-node');
      startNode = startNode.childNodes[0].childNodes[0].childNodes[0]
        .childNodes[1].childNodes[0].childNodes[0].childNodes[0] as HTMLElement;
      endNode = startNode;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
      ...
      expect(startNode.childNodes.length === 2).toBe(true);
      expect((startNode.childNodes[0] as HTMLElement).tagName === 'LI').toBe(
        true
      );
      expect((startNode.childNodes[1] as HTMLElement).tagName === 'LI').toBe(
        true
    ...
    it(' apply the left indent to the one LI element with OL as child element and selecting second elements in the OL ', function () {
      ...
      endNode = editNode.querySelector('.ol-third-node');
      startNode = startNode.childNodes[0].childNodes[0].childNodes[0]
        .childNodes[1].childNodes[1].childNodes[0].childNodes[0] as HTMLElement;
      endNode = startNode;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
    ...
    it(' apply the left indent to the childs of multiple nested OL element', function () {
      ...
      startNode = startNode.childNodes[0].childNodes[0].childNodes[0]
        .childNodes[1].childNodes[1].childNodes[0].childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[0].childNodes[0].childNodes[1].childNodes[1]
        .childNodes[0].childNodes[0].childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
  ...
  describe('right indent testing', () => {
    ...
    it(' apply the right indent to the LI element with two childs where the selection is the parent element and the first child', function () {
      startNode = editNode.querySelector('.ol-second-node');
      endNode = editNode.querySelector('.ol-second-node');
      startNode = startNode.childNodes[0].childNodes[0]
        .childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[0].childNodes[1].childNodes[0].childNodes[0]
        .childNodes[0].childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
    ...
    it(' apply the right indent to the child element with childs having parent having nested child where the selection is the nested child element', function () {
      startNode = editNode.querySelector('.ol-third-node');
      endNode = editNode.querySelector('.ol-third-node');
      startNode = startNode.childNodes[0].childNodes[1].childNodes[0]
        .childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[0].childNodes[1].childNodes[0].childNodes[1]
        .childNodes[0].childNodes[0].childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
      ...
      startNode = editNode.querySelector('.ol-third-node');
      startNode = startNode.childNodes[0].childNodes[1] as HTMLElement;
      let siblingListLI: NodeListOf<HTMLLIElement> = (
        startNode as Element
      ).querySelectorAll('li') as NodeListOf<HTMLLIElement>;
      let siblingListOL: Element[] = <NodeListOf<Element> & Element[]>(
        (startNode as Element).querySelectorAll('ol, ul')
      );
    ...
    it(' apply the right indent to the li element with child element right indented twice and with previous element ', function () {
      ...
      endNode = endNode.childNodes[1].childNodes[1].childNodes[0].childNodes[0]
        .childNodes[0].childNodes[0].childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
    ...
    it(' apply the right indent to multiple LI element', function () {
      ...
      startNode = startNode.childNodes[0].childNodes[0]
        .childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[2].childNodes[0]
        .childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
    ...
    it(' apply the right indent to multiple LI element with previous Element as OL', function () {
      ...
      startNode = editNode.querySelector('.ol-first-node');
      endNode = editNode.querySelector('.ol-first-node');
      startNode = startNode.childNodes[1].childNodes[0]
        .childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[3].childNodes[0]
        .childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
    ...
    it(' apply the right indent to one LI element which the Ol element as child and with previous Element as OL ', function () {
      ...
      startNode = editNode.querySelector('.ol-first-node');
      endNode = editNode.querySelector('.ol-first-node');
      startNode = startNode.childNodes[2].childNodes[0]
        .childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[3].childNodes[0]
        .childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
      ...
      startNode = startNode.childNodes[1].childNodes[0]
        .childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[1].childNodes[1].childNodes[1].childNodes[0]
        .childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
    ...
    it(' apply the right indent to childs in the multiple LI element with two li having the Ol element as child ', function () {
      ...
      startNode = editNode.querySelector('.ol-first-node');
      endNode = editNode.querySelector('.ol-first-node');
      startNode = startNode.childNodes[3].childNodes[0]
        .childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[4].childNodes[0]
        .childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
      ...
      startNode = editNode.querySelector('.ol-first-node');
      endNode = editNode.querySelector('.ol-first-node');
      startNode = startNode.childNodes[1].childNodes[1].childNodes[1]
        .childNodes[0].childNodes[0] as HTMLElement;
      endNode = endNode.childNodes[2].childNodes[1].childNodes[0].childNodes[0]
        .childNodes[0] as HTMLElement;
      editorObj.nodeSelection.setSelectionText(
        document,
        startNode,
  ...
  describe(' OL testing', () => {
    ...
    describe(' basic OL format apply and revert with content with space', () => {
      ...
      it(' apply the OL format to selected "p"and revert with content with space', () => {
        ...
        expect(
          (editorObj.listObj as any).saveSelection.range.startContainer
            .textContent === startNode.textContent
        ).toBe(true);
        expect(
          (editorObj.listObj as any).saveSelection.range.endContainer
    ...
    describe('Table OL testing', () => {
      ...
      it(' apply the OL format to table with empty content at the end of the table', () => {
        ...
        expect(
          editNode.querySelectorAll('li')[7].children[0].nodeName === 'BR'
        ).toBe(true);
      });
      afterAll(() => {
        detach(elem);
    ...
    describe('EJ2-59780 - List not applied in fire fox testing', () => {
      ...
      let elem: HTMLElement = createElement('div', {
        id: 'dom-node',
        innerHTML: `<div id="content-edit" contenteditable="true"><p class="startFocus">A paragraph is a series of sentences that are organized and coherent,
    ...
    describe('832588 - console error occurs when you apply the number format when start of the selection is a empty line', () => {
      let elem: HTMLElement = createElement('div', {
        id: 'dom-node',
        innerHTML: `<div id="content-edit" contenteditable="true"><p>rich text editor</p><p class="startFocus"><br></p><p>rich text editor</p><p class="endFocus">rich text editor</p></div>`,
      });
      beforeAll(() => {
        document.body.appendChild(elem);
        editorObj = new EditorManager({
      ...
      it(' console error occurs when you apply the number format when start of the selection is a empty line', () => {
        ...
        expect(
          (
            editorObj.nodeSelection.range.startContainer as HTMLElement
          ).classList.contains('startfocus')
        ).toBe(true);
      });
      afterAll(() => {
        detach(elem);
    ...
    describe('873315 - Applying list to the blockquotes element', () => {
      let elem: HTMLElement = createElement('div', {
        id: 'dom-node',
        innerHTML: `<div id="content-edit" contenteditable="true"><p class="startFocus">Line 1</p><p>Line 2</p><blockquote>Line 3 with quotation</blockquote><p>Line 4</p><p>Line 5</p><blockquote>Line 6&nbsp;<span style="background-color: unset; text-align: inherit;">with quotation</span></blockquote><p>Line 7</p><p class="endFocus">Line 8</p></div>`,
      });
      beforeAll(() => {
        document.body.appendChild(elem);
        editorObj = new EditorManager({
      ...
      it(' Applying list to the blockquotes content', () => {
        ...
        editorObj.execCommand('Lists', 'OL', null);
        expect(editNode.querySelectorAll('blockquote')[0].closest('OL')).toBe(
          null
        );
        expect(editNode.querySelectorAll('blockquote')[1].closest('OL')).toBe(
          null
        );
        expect(
          editNode.innerHTML ===
            `<ol><li class="startfocus">Line 1</li><li>Line 2</li></ol><blockquote><ol><li>Line 3 with quotation</li></ol></blockquote><ol><li>Line 4</li><li>Line 5</li></ol><blockquote><ol><li>Line 6&nbsp;<span style="background-color: unset; text-align: inherit;">with quotation</span></li></ol></blockquote><ol><li>Line 7</li><li class="endfocus">Line 8</li></ol>`
        ).toBe(true);
      });
      afterAll(() => {
        detach(elem);
    ...
    describe(' basic OL format apply and revert', () => {
      ...
      it(' apply the OL format to selected "p" node without class', () => {
        startNode = (
          editNode.querySelectorAll('p') as NodeListOf<HTMLParagraphElement> &
            Element[]
        )[0] as HTMLElement;
        endNode = (
          editNode.querySelectorAll('p') as NodeListOf<HTMLParagraphElement> &
      ...
      it(' apply the OL format to already applied "OL" format elements', () => {
        ...
        editorObj.nodeSelection.setSelectionText(
          document,
          startNode.childNodes[0],
          endNode.childNodes[1].childNodes[0].childNodes[0],
          0,
          4
        );
        ...
        let replaceNodes: Element[] = <Element[] & NodeListOf<Element>>(
          editNode.querySelectorAll('p.ol-first-node')
        );
      ...
      it(' revert from parent LI node and nested LI selection point', () => {
        ...
        startNode = startNode.childNodes[1].childNodes[0] as HTMLElement;
        endNode = editNode.querySelector('.ol-second-node');
        endNode = endNode.childNodes[1].childNodes[1]
          .childNodes[1] as HTMLElement;
        editorObj.nodeSelection.setSelectionText(
          document,
          startNode,
        ...
        expect(
          (editorObj.listObj as any).saveSelection.range.endContainer
            .textContent === endNode.childNodes[0].textContent
        ).toBe(true);

        expect(editNode.querySelector('p.ol-second-node')).not.toBeNull();
        expect(editNode.querySelectorAll('.ol-second-node').length === 3).toBe(
          true
        );
        editorObj.nodeSelection.Clear(document);
      });
    ...
    describe(' tab and shift+tab and revert', () => {
      ...
      it(' revert from parent LI node and nested LI selection point', () => {
        startNode = editNode.querySelector('.ol-second-node');
        endNode = startNode.childNodes[1].childNodes[1].childNodes[2]
          .childNodes[0] as HTMLElement;
        ...
        expect(editNode.querySelector('p.ol-second-node')).not.toBeNull();
        expect(editNode.querySelectorAll('p.ol-second-node').length === 2).toBe(
          true
        );
        editorObj.nodeSelection.Clear(document);
      });
    ...
    describe(' OL format with tab and shift+tab key', () => {
      ...
      it(' tab key navigation from first li start point', () => {
        ...
        startNode = editNode
          .querySelector('.ol-third-node')
          .querySelector('ol');
        expect(startNode.tagName === 'OL').toBe(true);
        expect(startNode.childNodes.length === 1).toBe(true);
        expect((startNode.childNodes[0] as Element).tagName === 'LI').toBe(
          true
        );
        editorObj.nodeSelection.Clear(document);
      });
      ...
      it(' tab key navigation from second li with nested OL element start point', () => {
        ...
        startNode = editNode
          .querySelector('.ol-second-node')
          .querySelector('ol');
        expect(startNode).not.toBeNull();
        expect(
          editNode.querySelector('.ol-second-node').querySelectorAll('ol')
            .length === 1
        ).toBe(true);
        editorObj.nodeSelection.Clear(document);
      });
      ...
      it(' tab key navigation from second li start point with \n line', () => {
        editNode.innerHTML =
          "<ol class='ol-third-node'><li>one-node</li>\n<li>two-node</li>\n<li>three-node</li></ol>";
        startNode = editNode.querySelector('.ol-third-node');
        expect(startNode.querySelector('ol')).toBeNull();
    ...
    xdescribe(' ul format with tab and shift+tab key', () => {
      ...
      it(' tab key navigation from second li with nested ul element start point', () => {
        ...
        startNode = editNode
          .querySelector('.ul-second-node')
          .querySelector('ul');
        expect(startNode).not.toBeNull();
        expect(
          editNode.querySelector('.ul-second-node').querySelectorAll('ul')
            .length === 1
        ).toBe(true);
        editorObj.nodeSelection.Clear(document);
      });
    ...
    describe(' OL to UL ', () => {
      ...
      it(' convert the OL list to UL list', () => {
        ...
        expect(
          (editorObj.listObj as any).saveSelection.range.startContainer
            .textContent === startNode.childNodes[0].textContent
        ).toBe(true);
        expect(
          (editorObj.listObj as any).saveSelection.range.endContainer
            .textContent === endNode.childNodes[0].textContent
      ...
      it(' convert the OL list to UL list while render with inner HTML tag', () => {
        ...
        expect(editNode.querySelector('.ol-fourth-node').tagName === 'UL').toBe(
          true
        );
        editorObj.nodeSelection.Clear(document);
      });
    ...
    describe(' OL format apply with toolbar icon, when applying the new list with OL ', () => {
      ...
      it(' apply the OL format to selected "p"and revert with content with space', () => {
        ...
        startNode = editNode.querySelector('.revertPara-1');
        expect(
          startNode.parentElement.parentElement.hasAttribute('style')
        ).toBe(false);
        editorObj.nodeSelection.Clear(document);
      });
      afterAll(() => {
    ...
    describe('865019 - console error occurs when you apply the number format when end of the selection is a empty line', () => {
      let elem: HTMLElement = createElement('div', {
        id: 'dom-node',
        innerHTML: `<div id="content-edit" contenteditable="true"><p>rich text editor</p><p class="startFocus">rich text editor</p><p>rich text editor</p><p class="endFocus"><br></p></div>`,
      });
      beforeAll(() => {
        document.body.appendChild(elem);
        editorObj = new EditorManager({
      ...
      it('console error occurs when you apply the number format when end of the selection is a empty line', () => {
        ...
        expect(
          (
            editorObj.nodeSelection.range.endContainer as HTMLElement
          ).classList.contains('endfocus')
        ).toBe(true);
      });
      afterAll(() => {
        detach(elem);
    ...
    describe('876790 - applying list to the content with indentation and other style applied in block element', () => {
      let elem: HTMLElement = createElement('div', {
        id: 'dom-node',
        innerHTML: `<div id="content-edit" contenteditable="true"><p class="startFocus" style="margin-left: 120px; color: red; background-color: yellow;">sdvsdvsdv</p><p style="margin-left: 80px; color: yellow; background-color: red;">sdvdsvdsvsdvsdv</p><p>sdv</p><p class="endFocus" style="margin-left: 40px;">sdvsdvdsvsdv</p></div>`,
      });
      beforeAll(() => {
        document.body.appendChild(elem);
        editorObj = new EditorManager({
      ...
      it('console error occurs when you apply the number format when end of the selection is a empty line', () => {
        ...
        expect(
          editNode.innerHTML ===
            `<ol style="margin-left: 120px;"><li class="startfocus" style="color: red; background-color: yellow;">sdvsdvsdv</li><li style="color: yellow; background-color: red;">sdvdsvdsvsdvsdv</li><li>sdv</li><li class="endfocus" style="">sdvsdvdsvsdv</li></ol>`
        ).toBe(true);
      });
      afterAll(() => {
        detach(elem);
  ...
  describe(' UL testing', () => {
    ...
    xdescribe(' basic ul format apply and revert', () => {
      ...
      it(' apply the ul format to already applied "ul" format elements', () => {
        ...
        startNode = startNode.childNodes[0] as HTMLElement;
        endNode = endNode.childNodes[1].childNodes[0]
          .childNodes[0] as HTMLElement;
        editorObj.nodeSelection.setSelectionText(
          document,
          startNode,
        ...
        let replaceNodes: Element[] = <Element[] & NodeListOf<Element>>(
          editNode.querySelectorAll('p.ul-first-node')
        );
      ...
      it(' revert from parent LI node and nested LI selection point', () => {
        ...
        expect(editNode.querySelectorAll('.ul-second-node').length === 3).toBe(
          true
        );
        editorObj.nodeSelection.Clear(document);
      });
    ...
    describe('Space key press testing', () => {
      ...
      it(' space key press after 1.', () => {
        ...
        (editorObj as any).editorKeyDown(keyBoardEvent);
        expect(
          editNode.querySelector('.space-two-node').parentElement.tagName
        ).toBe('OL');
      });
      ...
      it(' space key press after *', () => {
        ...
        (editorObj as any).editorKeyDown(keyBoardEvent);
        expect(
          editNode.querySelector('.space-four-node').parentElement.tagName
        ).toBe('UL');
      });
      ...
      it(' space key press after 1. when previous element starting with *', () => {
        ...
        (editorObj as any).editorKeyDown(keyBoardEvent);
        expect(
          editNode.querySelector('.space-six-node').parentElement.tagName
        ).not.toBe('OL');
      ...
      it(' space key press after * when previous element starting with *', () => {
        ...
        (editorObj as any).editorKeyDown(keyBoardEvent);
        expect(
          editNode.querySelector('.space-five-node').parentElement.tagName
        ).not.toBe('OL');
      ...
      it(' space key press after a.', () => {
        ...
        (editorObj as any).editorKeyDown(keyBoardEvent);
        expect(
          editNode.querySelector('.space-seven-node').parentElement.tagName
        ).toBe('OL');
      });
      ...
      it(' space key press after i.', () => {
        ...
        (editorObj as any).editorKeyDown(keyBoardEvent);
        expect(
          editNode.querySelector('.space-eight-node').parentElement.tagName
        ).toBe('OL');
      });
    ...
    describe('Space key press testing bold applied', () => {
      ...
      it(' space key press after 1. when bold is applied', () => {
        ...
        (editorObj as any).editorKeyDown(keyBoardEvent);
        expect(
          editNode.querySelector('.selectnode').parentElement.tagName
        ).toBe('OL');
      });
    ...
    describe('872830 - Space key press cursor position test ', () => {
      ...
      it(' Space key press cursor position test', () => {
        ...
        expect(
          editNode.querySelector('.selectnode').parentElement.childNodes[0]
    ...
    describe('872421 - Enter key press testing in nested list', () => {
      ...
      it(' Enter key press testing in nested list after OL 1.', () => {
        ...
        expect(
          editNode.innerHTML ===
            `<ol><li>List 1</li><li>List 2<ol><li style="list-style-type: none;"><ol><li style=""></li></ol></li></ol></li></ol>`
        ).toBe(true);
      });
    ...
    describe('Backspace key press at the start of the list when no content is previous to the list', () => {
      ...
      it(' Backspace key press at start of the normal list', () => {
        ...
        expect(editNode.querySelectorAll('.removeList').length === 0).toBe(
          true
        );
  ...
  describe('874955 - List style type got changed when we try to remove a single list in RichTextEditor', () => {
    ...
    it('Check the list style type changing after revert the single list', () => {
      startNode = editorObj.inputElement
        .querySelector('ol')
        .querySelectorAll('li')[1];
      endNode = editorObj.inputElement
        .querySelector('ol')
        .querySelectorAll('li')[1];
      ...
      let numberlist: HTMLElement = editorObj
        .getToolbar()
        .querySelector('[title="Numbered List (Ctrl+Shift+O)"]');
      numberlist.click();
  ...
  describe('883337 - List type got changed for Nested list while applying increase indents in RichTextEditor.', () => {
    ...
    it('Select the entire list and apply the indent.', (done) => {
      ...
      expect(
        startNode.firstElementChild.querySelector('ol').style.listStyleType ===
          'upper-alpha'
      ).toBe(true);
      done();
    });
