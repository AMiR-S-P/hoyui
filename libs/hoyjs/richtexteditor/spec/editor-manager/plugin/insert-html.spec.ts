...
describe('Insert HTML', () => {
  ...
  let innervalue: string =
    '<div id="parentDiv"><p id="paragraph1"><b>Description:</b><span id="span1">Span1 Element</span>' +
  ...
  it('Insert HTML in  cursor position', () => {
    ...
    expect(
      domSelection.getParentNodeCollection(domSelection.getRange(document))[0]
    ).toEqual(node);
  });
  ...
  it('Insert table next to image cursor position', () => {
    ...
    expect(
      document.querySelectorAll('#parentDiv > #imgParagraph > img').length
    ).toEqual(1);
    expect(document.querySelectorAll('#parentDiv > table').length).toEqual(1);
    expect(document.querySelector('#parentDiv > table').id).toEqual(
      'testTable'
    );
  });
  ...
  it('Insert table next to table cursor position', () => {
    ...
    expect(document.querySelectorAll('#parentDiv > table')[0].id).toEqual(
      'testTable1'
    );
    expect(document.querySelectorAll('#parentDiv > table')[1].id).toEqual(
      'testTable'
    );
  ...
  it('Insert table by selecting all the content', () => {
    ...
    InsertHtml.Insert(document, table1, editNode);
    expect(document.getElementById('divElement').children.length === 1).toBe(
      true
    );
    expect(
      document.getElementById('divElement').children[0].tagName === 'TABLE'
    ).toBe(true);
  });
});
...
describe('EJ2-49169-InsertHtml for the pasted elements not inserted properly', function () {
  ...
  it('Inserting pasted element in the empty node', function () {
    ...
    (InsertHtml as any).insertTempNode(
      range,
      pasteElement,
      rangeNodes,
      nodeCutter,
    ...
    expect(
      (divElement as any).childNodes[4].childNodes[0].childNodes.length
    ).toBe(5);
  });
});
...
describe('BLAZ-13456 - Pasting on the content by selecting "ctrl+a" throws console errors', function () {
  ...
  it('Inserting pasted element for empty blocknodes', function () {
    ...
    expect(
      (divElement as any).childNodes[1].childNodes[0].childNodes.length
    ).toBe(2);
  });
});
...
describe('Testing the insert method for html content paste', function () {
  ...
  it('Pasting html string content', function () {
    ...
    domSelection.setSelectionText(
      document,
      divElement.childNodes[0].firstChild,
      divElement.childNodes[2],
      0,
      0
    );
...
describe('EJ2-55078 - Insert HTML insert content outside', function () {
  let innervalue: string =
    '<div style="font-family: Calibri, Arial, Helvetica, sans-serif; font-size: 12pt; color: rgb(0, 0, 0);">test auto reply<br /><p class="focusElement"><br /><br /></p></div>';
  let range: Range;
  let divElement: HTMLElement = document.createElement('div');
  divElement.id = 'divElement';
  divElement.contentEditable = 'true';
  ...
  it('Insert HTML insert content outside when P has two br tags', function () {
    ...
    expect(
      (divElement as any).innerHTML ===
        '<div style="font-family: Calibri, Arial, Helvetica, sans-serif; font-size: 12pt; color: rgb(0, 0, 0);">test auto reply<br><p class="focusElement"><br></p><p>Inserted Content</p></div>'
    ).toBe(true);
  });
});
...
describe('EJ2-53098- Numbered List order in the Rich Text Editor goes incorrect when copying and pasting a list from MS word', function () {
  let innervalue: string =
    '<ol><li>Initial 1</li><li>Initial 2</li><li>Initial 3<br></li></ol>';
  let rangeNodes: Node[] = [];
  let range: Range;
  let nodeCutter: NodeCutter = new NodeCutter();
  let divElement: HTMLElement = document.createElement('div');
  ...
  it('Inserting li element at last in the existing OL', function () {
    ...
    expect(
      divElement.childNodes[0].childNodes[2].childNodes[1].childNodes[1]
        .textContent
    ).toBe('Initial 1');
    expect(
      divElement.childNodes[0].childNodes[2].childNodes[1].childNodes[2]
        .textContent
    ).toBe('Initial 2');
    expect(
      divElement.childNodes[0].childNodes[2].childNodes[1].childNodes[3]
        .textContent
    ).toBe('Initial 3');
  });
  it('Inserting li element at middle in the existing OL', function () {
    ...
    expect(
      divElement.childNodes[0].childNodes[1].childNodes[1].childNodes[1]
        .textContent
    ).toBe('Initial 1');
    expect(
      divElement.childNodes[0].childNodes[1].childNodes[1].childNodes[2]
        .textContent
    ).toBe('Initial 2');
    expect(
      divElement.childNodes[0].childNodes[1].childNodes[1].childNodes[3]
        .textContent
    ).toBe('Initial 3');
  });
});
...
describe('EJ2-53098- Unordered List order in the Rich Text Editor goes incorrect when copying and pasting a list from MS word', function () {
  let innervalue: string =
    '<ul><li>Initial 1</li><li>Initial 2</li><li>Initial 3<br></li></ul>';
  let rangeNodes: Node[] = [];
  let range: Range;
  let nodeCutter: NodeCutter = new NodeCutter();
  let divElement: HTMLElement = document.createElement('div');
