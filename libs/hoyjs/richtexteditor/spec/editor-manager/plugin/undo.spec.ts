 ...
 * Undo Redo spec
 */
import { selectAll, removeClass } from '@hoyui/base';
import {
  RichTextEditor,
  NodeSelection,
  IToolbarStatus,
...
describe('Undo and Redo module', () => {
  describe('div content-rte testing', () => {
    ...
    it('undo', () => {
      ...
      (<any>rteObj).formatter.editorManager.undoRedoManager.keyUp({
        event: keyboardEventArgs,
      });
      (<any>rteObj).formatter.editorManager.execCommand(
        'Actions',
        'Undo',
        null
      );
      expect(
        (rteObj.getContent().firstElementChild as HTMLElement).innerText ===
          'data'
      ).toBe(true);
  ...
  describe(' Toolbar click testing', () => {
    ...
    it('undo', () => {
      ...
      expect(
        (
          rteObj.getContent().firstElementChild as HTMLElement
        ).innerText.trim() === 'data'
      ).toBe(true);
    });
    it('redo', () => {
  ...
  describe('rte Iframe content- testing', () => {
    ...
    it('Content iframe testing', () => {
      expect(
        rteObj.element.querySelectorAll('iframe.e-rte-content').length
      ).toBe(1);
    });
    it('iframe-undo', () => {
      expect(rteObj.element.querySelectorAll('.e-rte-content').length).toBe(1);
      ...
      expect(
        (rteObj.contentModule.getEditPanel() as HTMLElement).innerText ===
          'data'
      ).toBe(true);
    });
    it('iframe-redo', () => {
