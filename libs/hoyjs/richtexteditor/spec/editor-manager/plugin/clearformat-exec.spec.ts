...
describe('Clear Format Exec plugin', () => {
  describe('apply Clear testing', () => {
    ...
    it('paragraph clear format', () => {
      ...
      editorObj.execCommand('Clear', 'ClearFormat', null, (): boolean => {
        callback = true;
        return true;
      });
      expect(
        document.getElementById('paragraph1').querySelectorAll('a').length
      ).toBe(0);
    ...
    it('label clear format', () => {
      ...
      expect(
        document
          .getElementById('last-p-node')
          .childNodes[0].nodeName.toLowerCase()
      ).toBe('#text');
    });
    it('Cursor clear format', () => {
      ...
      editorObj.execCommand('Clear', 'ClearFormat', null, null);
      expect(
        document.getElementById('label3').childNodes[0].nodeName.toLowerCase()
      ).toBe('#text');
    });
    it('No Command clear format', () => {
      let callback: boolean = false;
      editorObj.execCommand('Clear', 'ClearFormat1', null, (): boolean => {
        callback = true;
        return true;
      });
    ...
    it('Clear bold element', () => {
      ...
      editorObj.execCommand('Clear', 'ClearFormat', null, null);
      expect(
        document.getElementById('paragraph4').querySelectorAll('b').length
      ).toEqual(0);
    });
    afterAll(() => {
      detach(elem);
