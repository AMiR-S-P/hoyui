import { debounce, KeyboardEventArgs, isNullOrUndefined } from '@hoyui/base';
import { MarkdownParser } from './../base/markdown-parser';
import {
  IMarkdownSubCommands,
  IMDKeyboardEvent,
  MarkdownUndoRedoData,
} from './../base/interface';
...
export class UndoRedoCommands {
  ...
  private keyUp(e: IMDKeyboardEvent): void {
    if (
      ...
      !(e.event as KeyboardEvent).ctrlKey
    ) {
      this.saveData(e);
    }
  }
  ...
  public saveData(e?: KeyboardEvent | MouseEvent | IUndoCallBack): void {
    ...
    const textValue: string = (this.parent.element as HTMLTextAreaElement)
      .value;
    const changEle: { [key: string]: string | Object } = {
      text: textValue,
      start: start,
      end: end,
    };
    ...
    if (
      ...
      this.undoRedoStack[this.undoRedoStack.length - 1].start === start &&
  ...
  public undo(e?: IMarkdownSubCommands | IMDKeyboardEvent): void {
    if (this.steps > 0) {
      ...
      const removedContent: string = this.undoRedoStack[this.steps - 1]
        .text as string;
  ...
  public redo(e?: IMarkdownSubCommands | IMDKeyboardEvent): void {
    if (this.undoRedoStack[this.steps + 1] != null) {
      ...
      const start: number = this.undoRedoStack[this.steps + 1].start;
      const end: number = this.undoRedoStack[this.steps + 1].end;
      (this.parent.element as HTMLTextAreaElement).value = this.undoRedoStack[
        this.steps + 1
      ].text as string;
      (this.parent.element as HTMLTextAreaElement).focus();
      this.steps++;
