...
import { extend, KeyboardEventArgs } from '@hoyui/base';
...
export class MDLists {
  ...
  private tabKey(event: IMDKeyboardEvent): void {
    const textArea: HTMLTextAreaElement = this.parent
      .element as HTMLTextAreaElement;
    this.selection.save(textArea.selectionStart, textArea.selectionEnd);
    let start: number = textArea.selectionStart;
    const end: number = textArea.selectionEnd;
    const parents: { [key: string]: string | number }[] =
      this.selection.getSelectedParentPoints(textArea);
    ...
    for (let i: number = 0; i < parents.length; i++) {
      ...
      let prevIndex: number = event.event.shiftKey
        ? (parents[i as number].line as number) - 1
        : (parents[i as number].line as number) - 1;
      let prevLine: string = this.selection.getLine(textArea, prevIndex);
      if (
        prevLine &&
        ((!event.event.shiftKey && isNotFirst) || event.event.shiftKey)
      ) {
        const prevLineSplit: string[] = prevLine.split('. ');
        const tabSpace: string = '\t';
        const tabSpaceLength: number = event.event.shiftKey
          ? -tabSpace.length
          : tabSpace.length;
        ...
        if (
          this.currentAction === 'OL' &&
          /^\d+$/.test(prevLineSplit[0].trim()) &&
          listFormat
        ) {
          event.event.preventDefault();
          parents[i as number].text = event.event.shiftKey
            ? splitTab.splice(1, splitTab.length).join('\t')
            : tabSpace + parents[i as number].text;
          const curTabSpace: string = this.getTabSpace(
          ...
          if (curTabSpace === prevTabSpace) {
            this.changeTextAreaValue(
              splitText,
              this.nextOrderedListValue(prevLineSplit[0].trim()),
              event,
              textArea,
              parents,
              i,
            ...
            for (; prevTabSpace.length > curTabSpace.length; null) {
              ...
              if (/^\d+$/.test(prevLineSplit[0])) {
                ...
                if (prevTabSpace.length <= curTabSpace.length) {
                  this.changeTextAreaValue(
                    splitText,
                    this.nextOrderedListValue(prevLineSplit[0]),
                    event,
                    textArea,
                    parents,
                    i,
          ...
          (this.currentAction === 'UL' && regex.test(prevLine.trim())) ||
          !listFormat
        ) {
          event.event.preventDefault();
          parents[i as number].text = event.event.shiftKey
            ...
            : tabSpace + parents[i as number].text;
          textArea.value =
            textArea.value.substr(0, parents[i as number].start as number) +
            parents[i as number].text +
            '\n' +
            textArea.value.substr(
              parents[i as number].end as number,
        ...
        if (parents.length !== 1) {
          for (let j: number = i; j < parents.length; j++) {
            // eslint-disable-next-line max-len
            parents[j as number].start =
              j !== 0
                ? (parents[j as number].start as number) + tabSpaceLength
                : parents[j as number].start;
            parents[j as number].end =
              (parents[j as number].end as number) + tabSpaceLength;
          }
        }
      }
    }
  ...
  private changeTextAreaValue(
    ...
    const textAreaLength: number = this.selection.getAllParents(
      textArea.value
    ).length;
    let changedList: string = '';
    const curTabSpace: string = this.getTabSpace(
      parents[k as number].text as string
    );
    // eslint-disable-next-line
    let prefixNumber: number = parseInt(prefix.split('.')[0], null);
    let nestedTabSpace: string = this.getTabSpace(
      parents[k as number].text as string
    );
    ...
    changedList =
      this.selection.getLine(textArea, (parents[0].line as number) + 1) !== ''
        ? ''
        : changedList +
          textArea.value.substr(
            parents[0].end as number,
            textArea.value.length
          );
    for (
      let i: number = 1;
      i < textAreaLength &&
      !isNullOrUndefined(
        this.selection.getLine(textArea, (parents[0].line as number) + i)
      ) &&
      this.selection.getLine(textArea, (parents[0].line as number) + i) !== '';
      i++
    ) {
      const nextLine: string = this.selection.getLine(
        textArea,
        (parents[0].line as number) + i
      );
      const nextTabSpace: string = this.getTabSpace(nextLine);
      const nextLineSplit: string[] = nextLine.split('. ');
      if (nextLineSplit.length === 1) {
        changedList += textArea.value.substr(
          (parents[0].end as number) + nextLineLength,
          textArea.value.length
        );
        break;
      } else {
        ...
        curTabSpaceLength = event.event.shiftKey
          ? curTabSpace.length + 1
          : curTabSpace.length - 1;
        if (nextTabSpace.length > nestedTabSpace.length) {
          traversIncreased = false;
        }
        if (
          curTabSpace.length !== nextTabSpace.length &&
          nextTabSpace.length < nestedTabSpace.length
        ) {
          nestedListStart = true;
          nestedlistorder = false;
          shiftTabTargetList =
            event.event.shiftKey && curTabSpace.length === nextTabSpace.length
              ? ((nestedListStart = false), true)
              : false;
        } else if (
          traversIncreased &&
        ...
        if (curTabSpaceLength === nextTabSpace.length && nestedListStart) {
          const nextPrefix: string = event.event.shiftKey
            ? (nextPrefixValue++,
              this.nextOrderedListValue(nextPrefixValue.toString()))
          ...
          const nextPrefix: string = this.nextOrderedListValue(
    ...
    parents[k as number].text =
      this.getTabSpace(parents[k as number].text as string) +
      prefix +
      splitText.join('. ') +
      '\n';
    textArea.value =
      textArea.value.substr(0, parents[k as number].start as number) +
      parents[k as number].text +
      changedList;
  }
  ...
  private isNotFirstLine(
    ...
    if (prevLine && regexFirstCondition) {
      ...
      for (; prevTabSpace.length > curTabSpace.length; null) {
        ...
        prevLine = this.selection.getLine(textArea, prevIndex);
        const prevLineSplit: string[] = prevLine.trim().split('. ');
        const regexSecondCondition: boolean =
          this.currentAction === 'OL'
            ? /^\d+$/.test(prevLineSplit[0])
            : regex.test(prevLine.trim());
  ...
  private getAction(line: string): string {
    ...
    // eslint-disable-next-line
    const ul: string = line
      .trim()
      .split(
        new RegExp(
          '^(' + this.selection.replaceSpecialChar(this.syntax.UL).trim() + ')'
        )
      )[1];
  ...
  private enterKey(event: IMDKeyboardEvent): void {
    ...
    const parents: { [key: string]: string | number }[] =
      this.selection.getSelectedParentPoints(textArea);
    const prevLine: string = this.selection.getLine(
      textArea,
      (parents[0].line as number) - 1
    );
    const listFormat: number = this.olListType();
    const regex: RegExp = this.getListRegex();
    ...
    if (
      ...
      for (
        ...
        if (isNullOrUndefined(nextLine)) {
          ...
          if (
            nextLineSplit.length > 1 &&
            /^\d+$/.test(nextLineSplit[0].trim()) &&
            curTabSpace.length === nextTabSpace.length &&
            nestedListOrder
          ) {
            const nextPrefix: string = this.nextOrderedListValue(
      ...
      textArea.value =
        textArea.value.substr(0, parents[0].start as number) +
        curTabSpace +
        prefix +
        this.selection.getLine(textArea, parents[0].line as number) +
        changedList;
      start = start + prefix.length + tabSpace.length;
      addedLength += prefix.length + tabSpace.length;
    } else if (
      (this.currentAction === 'UL' &&
        prevLine &&
        regex.test(prevLine.trim()) &&
        prevLine.trim().replace(regex, '') !== '') ||
      (this.currentAction === 'OL' && !listFormat)
    ) {
      const tabSpace: string = this.getTabSpace(prevLine);
  ...
  private olListType(): number {
    ...
    const listType: number =
      olSyntaxList.length === 1
        ? null
        : // eslint-disable-next-line
          parseInt(olSyntaxList[2].trim(), null) -
          parseInt(olSyntaxList[0].trim(), null);
    if (listType) {
      return 1;
    } else {
      return 0;
    ...
    for (let i: number = 0; i < parents.length; i++) {
      if (listFormat) {
        regex =
          this.currentAction === 'OL'
            ? i + listFormat + '. '
            : this.syntax[this.currentAction];
      } else {
        regex =
          this.currentAction === 'OL'
            ? this.syntax.OL
      ...
      if (
        !this.selection.isStartWith(parents[i as number].text as string, regex)
      ) {
        if (parents[i as number].text === '' && i === 0) {
          ...
          if (parents.length !== 1) {
            for (let j: number = i; j < parents.length; j++) {
              parents[j as number].start =
                j !== 0
                  ? 1 + (parents[j as number].start as number)
                  : parents[j as number].start;
              parents[j as number].end =
                1 + (parents[j as number].end as number);
            }
          }
        }
        const preLineTabSpaceLength: number = !isNullOrUndefined(parents[i - 1])
          ? this.getTabSpace(parents[i - 1].text as string).length
          : 0;
        const replace: { [key: string]: number | string } = this.appliedLine(
          parents[i as number].text as string,
          regex,
          perfixObj,
          preLineTabSpaceLength
        );
        prefix = replace.line ? prefix : regex;
        parents[i as number].text = replace.line
          ? replace.line
          : prefix + parents[i as number].text;
        replace.space = replace.space ? replace.space : 0;
        textArea.value =
          textArea.value.substr(
            0,
            (parents[i as number].start as number) + endLength
          ) +
          parents[i as number].text +
          '\n' +
          textArea.value.substr(
        ...
        start =
          i === 0
            ? start + prefix.length + (replace.space as number) > 0
              ? start + prefix.length + (replace.space as number)
              : 0
            : start;
        ...
        if (parents.length !== 1) {
          for (let j: number = i; j < parents.length; j++) {
            parents[j as number].start =
              j !== 0
                ? prefix.length +
                  (parents[j as number].start as number) +
                  (replace.space as number)
                : parents[j as number].start;
            parents[j as number].end =
              prefix.length +
              (parents[j as number].end as number) +
              (replace.space as number);
          }
        }
        ...
        textArea.value =
          ...
          '\n' +
          textArea.value.substr(
            (parents[i as number].end as number) + endLength,
            textArea.value.length
          );
  ...
  private appliedLine(
    ...
    perfixObj: { [key: string]: number },
    preTabSpaceLength: number
    ...
    const isExist: boolean =
      regex.test(line.trim()) ||
      line.trim() === this.syntax.OL.trim() ||
      line.trim() === this.syntax.UL.trim() ||
      /^\d+$/.test(lineSplit[0].trim());
    const listFormat: number = this.olListType();
    const curTabSpaceLength: number = this.getTabSpace(line).length;
    if (this.currentAction === 'OL' && listFormat) {
      perfixObj[curTabSpaceLength.toString()] = !isNullOrUndefined(
        perfixObj[curTabSpaceLength.toString()]
      )
        ? perfixObj[curTabSpaceLength.toString()].valueOf() + 1
        : 1;
      prefixPattern =
        perfixObj[curTabSpaceLength.toString()].valueOf().toString() + '. ';
      if (
        !isNullOrUndefined(preTabSpaceLength) &&
        preTabSpaceLength > curTabSpaceLength
      ) {
    ...
    if (isExist) {
      ...
      if (regex.test(line.trim())) {
        ...
        points.space =
          prefixPattern.trim().length - this.syntax.UL.trim().length;
      } else if (/^\d+$/.test(lineSplit[0].trim()) && listFormat) {
        pattern = lineSplit[0].trim() + '. ';
        replace = prefixPattern;
        points.space =
          this.syntax.UL.trim().length - currentPrefix.trim().length;
  ...
  private restore(
    ...
    event?: IMarkdownSubCommands | IMDKeyboardEvent
  ): void {
  ...
  private getListRegex(): RegExp {
    ...
    for (let j: number = 0; j < configKey.length; j++) {
      const syntax: string = this.selection.replaceSpecialChar(
        this.syntax[configKey[j as number]]
      );
      regex +=
        regex === ''
          ? '^(' + syntax + ')|^(' + syntax.trim() + ')'
