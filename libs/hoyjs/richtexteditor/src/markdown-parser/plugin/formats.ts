...
export class MDFormats {
  ...
  private applyFormats(e: IMarkdownSubCommands): void {
    ...
    let parents: { [key: string]: string | number }[] =
    ...
    if (e.subCommand === 'p') {
      ...
      if (
        (e.subCommand === 'pre' && parents.length !== 1) ||
        e.subCommand !== 'pre'
      ) {
    ...
    for (let i: number = 0; i < parents.length; i++) {
      if (
        ...
        !this.selection.isStartWith(
          parents[i as number].text as string,
          '\\' + this.syntax[e.subCommand]
        )
      ) {
        parents[i as number].text =
          this.syntax[e.subCommand] + parents[i as number].text;
        textArea.value =
  ...
  private clearRegex(): string {
    ...
    for (
      let j: number = 0;
      j < configKey.length &&
      configKey[j as number] !== 'pre' &&
      configKey[j as number] !== 'p';
      j++
    ) {
      regex +=
        regex === ''
          ? '^(' +
            this.selection.replaceSpecialChar(
              this.syntax[configKey[j as number]].trim()
            ) +
            ')'
          : '|^(' +
            this.selection.replaceSpecialChar(
  ...
  private cleanFormat(textArea: HTMLTextAreaElement, command?: string): void {
    ...
    if (this.selection.isClear(parents, this.clearRegex())) {
      for (let i: number = 0; i < parents.length; i++) {
        ...
        for (
          let j: number = 0;
          parents[i as number].text !== '' && j < configKey.length;
          j++
        ) {
          ...
          const regex: RegExp = new RegExp(
            '^(' + this.selection.replaceSpecialChar(removeText) + ')',
            'gim'
          );
          if (regex.test(parents[i as number].text as string)) {
            ...
            start =
              i === 0
                ? start - removeText.length > 0
                  ? start - removeText.length
                  : 0
                : start;
            removeLength += removeText.length;
            if (parents.length !== 1) {
              for (let k: number = 0; k < parents.length; k++) {
                parents[k as number].start =
                  k !== 0
                    ? (parents[k as number].start as number) - removeText.length
                    : parents[k as number].start;
                parents[k as number].end =
                  (parents[k as number].end as number) - removeText.length;
              }
            }
            break;
          }
  ...
  private applyCodeBlock(
    ...
    if (
      ...
      textArea.value =
        textArea.value.substr(0, start as number) +
        this.syntax[`${command}`] +
      ...
      const startNo: number = textArea.value
        .substr(0, textArea.selectionStart as number)
        .lastIndexOf(cmd);
      let endNo: number = textArea.value
        .substr(
          textArea.selectionEnd as number,
          textArea.selectionEnd as number
        )
        .indexOf(cmd);
  ...
  private isAppliedFormat(
    ...
    const direction: string = (this.parent.element as HTMLTextAreaElement)
      .selectionDirection;
    const checkLine: string =
      direction === 'backward'
        ? (lines[0].text as string)
        : (lines[lines.length - 1].text as string);
    for (let i: number = 0; !documentNode && i < keys.length; i++) {
      if (
        keys[i as number] !== 'pre' &&
        this.selection.isStartWith(checkLine, this.syntax[keys[i as number]])
      ) {
        format = keys[i as number];
        break;
      } else if (keys[i as number] === 'pre') {
        const parentLines: string[] = this.selection.getAllParents(
          (this.parent.element as HTMLTextAreaElement).value
        );
        const firstPrevText: string =
          parentLines[(lines[0].line as number) - 1];
        const lastNextText: string = parentLines[lines.length + 1];
        if (
          this.selection.isStartWith(
            firstPrevText,
            this.syntax[keys[i as number]].split('\n')[0]
          ) &&
          this.selection.isStartWith(
            lastNextText,
            this.syntax[keys[i as number]].split('\n')[0]
