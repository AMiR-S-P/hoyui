import { isNullOrUndefined, KeyboardEventArgs } from '@hoyui/base';
...
export class MDSelectionFormats {
  ...
  public isAppliedCommand(cmd?: string): boolean {
    ...
    const splitAt: Function = (index: number) => (x: string) =>
    ...
    const selectedText: string =
      this.parent.markdownSelection.getSelectedText(textArea);
    if (
      selectedText !== '' &&
      ...
      const beforeText: string = textArea.value.substr(
      ...
      if (
        beforeText !== '' &&
        afterText !== '' &&
        beforeText.match(/[a-z]/i) &&
        beforeText === beforeText.toLocaleUpperCase() &&
        afterText === afterText.toLocaleUpperCase() &&
        cmd === 'UpperCase'
      ) {
        return true;
      }
    ...
    if (
      !this.isBold(splitText[0], cmdB) &&
      !this.isItalic(splitText[0], cmdI) &&
      !this.isBold(splitText[1], cmdB) &&
      !this.isItalic(splitText[1], cmdI)
    ) {
      if (
        !isNullOrUndefined(
          this.isMatch(splitText[0], this.syntax.StrikeThrough)
        ) &&
        !isNullOrUndefined(
          this.isMatch(splitText[1], this.syntax.StrikeThrough)
        ) &&
        this.isMatch(splitText[0], this.syntax.StrikeThrough).length % 2 ===
          1 &&
        this.isMatch(splitText[1], this.syntax.StrikeThrough).length % 2 ===
          1 &&
        cmd === 'StrikeThrough'
      ) {
        isFormat = true;
      ...
      if (
        !isNullOrUndefined(
          this.isMatch(splitText[0], this.syntax.InlineCode)
        ) &&
        !isNullOrUndefined(
          this.isMatch(splitText[1], this.syntax.InlineCode)
        ) &&
        this.isMatch(splitText[0], this.syntax.InlineCode).length % 2 === 1 &&
        this.isMatch(splitText[1], this.syntax.InlineCode).length % 2 === 1 &&
        cmd === 'InlineCode'
      ) {
        isFormat = true;
      }
      /* eslint-disable */
      if (
        !isNullOrUndefined(splitText[0].match(/\<sub>/g)) &&
        !isNullOrUndefined(splitText[1].match(/\<\/sub>/g)) &&
        splitText[0].match(/\<sub>/g).length % 2 === 1 &&
        splitText[1].match(/\<\/sub>/g).length % 2 === 1 &&
        cmd === 'SubScript'
      ) {
        isFormat = true;
      }
      if (
        !isNullOrUndefined(splitText[0].match(/\<sup>/g)) &&
        !isNullOrUndefined(splitText[1].match(/\<\/sup>/g)) &&
        splitText[0].match(/\<sup>/g).length % 2 === 1 &&
        splitText[1].match(/\<\/sup>/g).length % 2 === 1 &&
        cmd === 'SuperScript'
      ) {
        isFormat = true;
      }
    ...
    if (
      ...
      splitText[1].match(this.multiCharRegx(cmdB)).length % 2 === 1 &&
      cmd === 'Bold'
    ) {
      isFormat = true;
    }
    splitText[0] = this.isBold(splitText[0], cmdB)
      ? splitText[0].replace(this.multiCharRegx(cmdB), '$%@')
      : splitText[0];
    splitText[1] = this.isBold(splitText[1], cmdB)
      ? splitText[1].replace(this.multiCharRegx(cmdB), '$%@')
      : splitText[1];
    if (
      !isNullOrUndefined(this.isMatch(splitText[0], this.syntax.Italic)) &&
      !isNullOrUndefined(this.isMatch(splitText[1], this.syntax.Italic)) &&
      this.isMatch(splitText[0], this.syntax.Italic).length % 2 === 1 &&
      this.isMatch(splitText[1], this.syntax.Italic).length % 2 === 1 &&
      cmd === 'Italic'
    ) {
      isFormat = true;
    }
    if (
      ...
      this.isMatch(splitText[1], this.syntax.StrikeThrough).length % 2 === 1 &&
      cmd === 'StrikeThrough'
    ) {
      isFormat = true;
    }
    if (
      !isNullOrUndefined(this.isMatch(splitText[0], this.syntax.InlineCode)) &&
  ...
  private applyCommands(e: IMarkdownSubCommands): void {
    ...
    const selection: { [key: string]: string | number } =
      this.parent.markdownSelection.getSelectedInlinePoints(textArea);
    if (this.isAppliedCommand(e.subCommand) && selection.text !== '') {
      const startCmd: string = this.syntax[e.subCommand];
      const endCmd: string =
        e.subCommand === 'SubScript'
          ? '</sub>'
          : e.subCommand === 'SuperScript'
          ? '</sup>'
          : this.syntax[e.subCommand];
      const startLength: number =
        e.subCommand === 'UpperCase' || e.subCommand === 'LowerCase'
          ? 0
          : startCmd.length;
      const startNo: number = textArea.value
        .substr(0, selection.start as number)
        .lastIndexOf(startCmd);
      let endNo: number = textArea.value
        .substr(selection.end as number, textArea.value.length)
        .indexOf(endCmd);
    ...
    if (selection.text !== '' && !this.isApplied(selection, e.subCommand)) {
      ...
      const repStart: string = textArea.value.substr(
        (selection.start as number) - this.syntax[e.subCommand].length,
        this.syntax[e.subCommand].length
      );
      let repEnd: string;
      if (
        repStart === e.subCommand ||
        ((selection.start as number) - this.syntax[e.subCommand].length ===
          textArea.value.indexOf(this.syntax[e.subCommand]) &&
          ((selection.end as number) ===
            textArea.value.lastIndexOf(this.syntax[e.subCommand]) ||
            (selection.end as number) ===
        ...
        if (e.subCommand === 'SubScript' || e.subCommand === 'SuperScript') {
          repEnd = textArea.value.substr(
            selection.end as number,
            this.syntax[e.subCommand].length + 1
          );
        } else {
          repEnd = textArea.value.substr(
            selection.end as number,
        ...
        if (e.subCommand === 'SubScript' || e.subCommand === 'SuperScript') {
          ...
          e.subCommand === 'UpperCase' ||
          e.subCommand === 'LowerCase'
        ) {
          selection.text =
            e.subCommand === 'UpperCase'
              ? (selection.text as string).toUpperCase()
      ...
      if (e.subCommand === 'SubScript' || e.subCommand === 'SuperScript') {
        selection.text = this.textReplace(
    ...
    switch (command) {
      ...
      case 'Italic':
        if (!this.isBold(text, this.syntax[`${command}`].substr(0, 1))) {
          ...
          const regxB: RegExp = this.multiCharRegx(
            this.syntax[`${command}`].substr(0, 1)
          );
