...
export class MDTable {
  ...
  private syntaxTag: {
    [key in MarkdownTableFormat]: { [key: string]: string };
  };
  ...
  private insertTable(
    ...
    const parentText: { [key: string]: string | number }[] =
      this.selection.getSelectedParentPoints(this.element);
    const lastLineSplit: string[] = (
      parentText[parentText.length - 1].text as string
    ).split(' ', 2);
    ...
    if (lastLineSplit.length < 2) {
      ...
      if (
        this.ensureFormatApply(parentText[parentText.length - 1].text as string)
      ) {
        this.checkValid(
          start,
          end,
  ...
  private ensureFormatApply(line: string): boolean {
    ...
    for (let i: number = 0; i < formatTags.length; i++) {
      if (
        formatSplitZero === formatTags[i as number] ||
        /^[\d.]+[ ]+$/.test(formatSplitZero)
      ) {
        return true;
      }
    }
  ...
  private ensureStartValid(
    firstLine: number,
    parentText: { [key: string]: string | number }[]
  ): boolean {
    const firstLineSplit: string[] = (parentText[0].text as string).split(
      ' ',
      2
    );
    for (
      let i: number = firstLine + 1;
      i <= firstLine + firstLineSplit[0].length + 1;
      i++
    ) {
      if (
        this.element.selectionStart === i ||
        this.element.selectionEnd === i
      ) {
        return false;
      }
    }
  ...
  private ensureEndValid(lastLine: number, formatSplitLength: number): boolean {
    for (
      let i: number = lastLine + 1;
      i <= lastLine + formatSplitLength + 1;
      i++
    ) {
  ...
  private updateValueWithFormat(formatSplit: string[], text: string): string {
    const textApplyFormat: string = this.element.value.substring(
      this.element.selectionEnd,
      this.element.value.length
    );
    text += textApplyFormat.replace(
      textApplyFormat,
      formatSplit[0] + ' ' + textApplyFormat
    );
  ...
  private updateValue(text: string): string {
    return (
      this.element.value.substr(0, this.element.selectionStart) +
      text +
  ...
  private checkValid(
    ...
    if (
      this.ensureStartValid(parentText[0].start as number, parentText) &&
      this.ensureEndValid(
        parentText[parentText.length - 1].start as number,
        formatSplit[0].length
      )
    ) {
      if (start === (parentText[0].start as number)) {
        if (
          start !== end &&
          end !== (parentText[parentText.length - 1].end as number) - 1
        ) {
        ...
        (parentText[parentText.length - 1].end as number) - 1
      ) {
        this.element.value = this.updateValue(text);
      } else {
        this.element.value = this.updateValueWithFormat(formatSplit, text);
