  ...
  if (childNodes.length > 0) {
    ...
    while (tempNode.firstChild) {
      if (
        ...
        inlineNode.indexOf(tempNode.firstChild.nodeName.toLocaleLowerCase()) >=
          0
      ) {
    ...
    const tableElm: NodeListOf<HTMLElement> =
      resultElm.querySelectorAll('table');
    for (let i: number = 0; i < tableElm.length; i++) {
      if (
        tableElm[i as number].classList.length > 0 &&
        !tableElm[i as number].classList.contains('e-rte-table')
      ) {
        tableElm[i as number].classList.add('e-rte-paste-table');
        if (
          tableElm[i as number].classList.contains('e-rte-paste-word-table')
        ) {
          ...
          tableElm[i as number].classList.contains('e-rte-paste-excel-table')
        ) {
          ...
          const tdElm: NodeListOf<HTMLElement> =
            tableElm[i as number].querySelectorAll('td');
          ...
          tableElm[i as number].classList.contains('e-rte-paste-onenote-table')
        ) {
          tableElm[i as number].classList.remove('e-rte-paste-onenote-table');
          continue;
        } else if (
          tableElm[i as number].classList.contains('e-rte-paste-html-table')
        ) {
    ...
    for (let i: number = 0; i < imageElm.length; i++) {
      ...
      if (
        !(
          imageElm[i as number].classList.contains('e-imginline') ||
...
export function nestedListCleanUp(range: Range): void {
  if (
    ...
    while (
      ...
      ).querySelectorAll('li :empty').length > 0 ||
      (range.startContainer.nodeName === '#text'
        ? range.startContainer.parentElement
        : (range.startContainer as HTMLElement)
      ).querySelectorAll('ol :empty').length > 0
    ) {
      const emptyLI: NodeListOf<Element> = (
        range.startContainer.nodeName === '#text'
          ? range.startContainer.parentElement
          : (range.startContainer as HTMLElement)
    ...
    const liElem: NodeListOf<HTMLLIElement> = (
      range.startContainer.nodeName === '#text'
        ? range.startContainer.parentElement
        : (range.startContainer as HTMLElement)
    ).querySelectorAll('li');
    if (liElem.length > 0) {
      liElem.forEach((item: HTMLLIElement) => {
        if (
          !isNullOrUndefined(item.firstChild) &&
          (item.firstChild.nodeName === 'OL' ||
            item.firstChild.nodeName === 'UL')
        ) {
...
export function scrollToCursor(
  document: Document,
  inputElement: HTMLElement | HTMLBodyElement
): void {
  const rootElement: HTMLElement =
    inputElement.nodeName === 'BODY'
      ? (inputElement.ownerDocument.defaultView.frameElement.closest(
          '.e-richtexteditor'
        ) as HTMLElement)
  ...
  const finalFocusElement: HTMLElement =
    range.startContainer.nodeName === '#text'
  ...
  const hasMargin: boolean =
    rootElement.querySelectorAll('.e-count-enabled, .e-resize-enabled').length >
    0;
  if (inputElement.nodeName === 'BODY') {
    if (height === 'auto') {
      ...
      if (cursorTop > inputElement.getBoundingClientRect().height) {
        finalFocusElement.scrollIntoView({
          block: 'nearest',
          inline: 'nearest',
        });
      }
    ...
    if (height === 'auto') {
      ...
      if (cursorBottom > rootRect.bottom) {
        rootElement.querySelector('.e-rte-content').scrollTop +=
          cursorBottom - rootRect.bottom + (hasMargin ? 20 : 0);
      }
    }
  }
  const scrollVal: HTMLElement = inputElement.closest(
 ...
 * @private
 */
export function removeClassWithAttr(
  ...
  for (let i: number = 0; i < elements.length; i++) {
    if (
      (elements[i as number] as Element).classList.length === 0 &&
      (elements[i as number] as Element).getAttribute('class')
    ) {
