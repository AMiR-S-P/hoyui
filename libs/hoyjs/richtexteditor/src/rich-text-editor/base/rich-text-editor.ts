...
import { Touch as EJ2Touch, TapEventArgs } from '@hoyui/base';
import {
  getScrollableParent,
  BeforeOpenEventArgs,
  ...
  implements INotifyPropertyChanged
{
   ...
   * }
   */
  @Complex<SlashMenuSettingsModel>(
    {
      enable: false,
      items: [
        'Paragraph',
  ...
  private updateEnable(): void {
    if (this.enabled) {
      ...
      if (this.getToolbar()) {
        removeClass(
          this.getToolbar().querySelectorAll('.' + classes.CLS_ACTIVE),
          classes.CLS_ACTIVE
        );
  ...
  private setContainer(): void {
    ...
    for (let a: number = 0; a < this.element.attributes.length; a++) {
      if (
        invalidAttr.indexOf(this.element.attributes[a as number].name) === -1 &&
        !/^data-val/.test(this.element.attributes[a as number].name)
      ) {
        // data-val for asp.net core data annotation validation.
        htmlAttr[this.element.attributes[a as number].name] =
          this.element.getAttribute(this.element.attributes[a as number].name);
      }
    }
    ...
    this.rootContainer = this.createElement('div', {
      className: classes.CLS_RTE_CONTAINER,
  ...
  public executeCommand(
    ...
    if (this.editorMode === 'HTML') {
      ...
      if (this.iframeSettings.enable) {
        this.formatter.editorManager.nodeSelection.Clear(
          this.element.ownerDocument
        );
      }
      const toFocus: boolean =
        this.iframeSettings.enable && range.startContainer === this.inputElement
          ? true
          : !this.inputElement.contains(range.startContainer);
      if (toFocus) {
        this.focusIn();
      }
    }
    ...
    if (this.maxLength !== -1 && !isNOU(tool.command)) {
      ...
      if (tool.command === 'Links') {
        currentInsertContentLength =
          (value as ILinkCommandsArgs).text.length === 0
            ? (value as ILinkCommandsArgs).url.length
            : (value as ILinkCommandsArgs).text.length;
      }
      if (
        tool.command === 'Images' ||
        tool.command === 'Table' ||
      ...
      const currentLength: number = this.getText()
        .trim()
        .replace(/(\r\n|\n|\r|\t)/gm, '')
        .replace(/\u200B/g, '').length;
      const selectionLength: number = this.getSelection().length;
      const totalLength: number =
        currentLength - selectionLength + currentInsertContentLength;
    ...
    if (this.editorMode === 'HTML') {
      switch (command) {
        case 'insertHTML':
          if (this.enableHtmlSanitizer) {
            if (typeof value === 'string') {
              ...
              value = this.htmlEditorModule.sanitizeHelper(
                (value as HTMLElement).outerHTML
              );
            }
          }
          break;
        ...
        case 'insertAudio': {
          ...
          const temp: HTMLElement = this.createElement('source', {
            attrs: {
              ...
              type:
                (value as IAudioCommandsArgs).url &&
                (value as IAudioCommandsArgs).url.split('.').length > 0
                  ? 'audio/' +
                    (value as IAudioCommandsArgs).url.split('.')[
                      (value as IAudioCommandsArgs).url.split('.').length - 1
                    ]
                  : '',
            },
          });
          ...
          if (this.enableHtmlSanitizer) {
            audioValue = this.htmlEditorModule.sanitizeHelper(
              wrapTemp.outerHTML
            );
          }
          let url: string =
        ...
        case 'insertVideo': {
          ...
          const temp: HTMLElement = this.createElement('source', {
            attrs: {
              ...
              type:
                (value as IVideoCommandsArgs).url &&
                (value as IVideoCommandsArgs).url.split('.').length > 0
                  ? 'video/' +
                    (value as IVideoCommandsArgs).url.split('.')[
                      (value as IVideoCommandsArgs).url.split('.').length - 1
                    ]
                  : '',
            },
          });
        ...
        case 'createLink': {
          ...
          if (this.enableHtmlSanitizer) {
            linkValue = this.htmlEditorModule.sanitizeHelper(
              tempNode.outerHTML
            );
          }
          let href: string =
  ...
  protected render(): void {
    ...
    this.value =
      !(this.editorMode === 'Markdown') && !isNOU(this.value)
        ? this.addAnchorAriaLabel(this.value)
        : this.value;
    ...
    this.RTERender();
    // eslint-disable-next-line
    const execCommandCallBack: ExecCommandCallBack = new ExecCommandCallBack(
      this
    );
    ...
    if (
      this.toolbarSettings.enable &&
      (this.toolbarSettings.type === 'Expand' ||
        this.toolbarSettings.type === 'MultiRow' ||
        this.toolbarSettings.type === 'Scrollable') &&
      !isNOU(this.getToolbar()) &&
      this.toolbarSettings.items.indexOf('Undo') > -1 &&
      this.toolbarSettings.items.indexOf('Redo') > -1
    ) {
  ...
  public addAudioVideoWrapper(): void {
    ...
    const audioElm: NodeListOf<HTMLElement> =
      this.element.querySelectorAll('audio');
    for (let i: number = 0; i < audioElm.length; i++) {
      ...
      if (
        !audioElm[i as number].parentElement.classList.contains(
          classes.CLS_CLICKELEM
        ) &&
        !audioElm[i as number].parentElement.classList.contains(
          classes.CLS_AUDIOWRAP
        )
      ) {
        const audioWrapElem: HTMLElement = this.createElement('span', {
          className: classes.CLS_AUDIOWRAP,
        });
        audioWrapElem.setAttribute('style', 'width:300px; margin:0 auto;');
        audioWrapElem.contentEditable = 'false';
        const audioInnerWrapElem: HTMLElement = this.createElement('span', {
          className: classes.CLS_CLICKELEM,
        });
        audioWrapElem.appendChild(audioInnerWrapElem);
        audioElm[i as number].parentNode.insertBefore(
          audioWrapElem,
          audioElm[i as number].nextSibling
        );
        ...
        if (audioWrapElem.nextElementSibling === null) {
          ...
          audioWrapElem.parentNode.insertBefore(
            insertElem,
            audioWrapElem.nextSibling
          );
        }
      }
    }
    const videoElm: NodeListOf<HTMLElement> =
    ...
    for (let i: number = 0; i < videoElm.length; i++) {
      ...
      // eslint-disable-next-line max-len
      if (
        !videoElm[i as number].parentElement.classList.contains(
          classes.CLS_CLICKELEM
        ) &&
        !videoElm[i as number].parentElement.classList.contains(
          classes.CLS_VIDEOWRAP
        )
      ) {
        const videoWrapElem: HTMLElement = this.createElement('span', {
          className: classes.CLS_VIDEOWRAP,
        });
        videoWrapElem.contentEditable = 'false';
        videoElm[i as number].parentNode.insertBefore(
          videoWrapElem,
          videoElm[i as number].nextSibling
        );
        videoWrapElem.appendChild(videoElm[i as number]);
        if (videoWrapElem.nextElementSibling === null) {
          insertElem = this.createElement('br');
          videoWrapElem.parentNode.insertBefore(
            insertElem,
            videoWrapElem.nextSibling
          );
        }
      }
  ...
  public cleanList(e: KeyboardEvent): void {
    ...
    const isSameContainer: boolean =
      currentStartContainer === currentEndContainer ? true : false;
    // eslint-disable-next-line
    const currentEndOffset: number = currentEndContainer.textContent.length;
    const endNode: Element =
    ...
    if (currentEndContainer.nodeType === Node.TEXT_NODE) {
      if (
        currentEndContainer.textContent.charAt(currentRangeEndOffset - 1) ===
        '\uFEFF'
      ) {
    ...
    if (
      !isNOU(closestLI) &&
      endNode.textContent.trim().length === currentRangeEndOffset &&
      !range.collapsed &&
      isNOU(endNode.nextElementSibling)
    ) {
      for (let i: number = 0; i < closestLI.childNodes.length; i++) {
        if (
          closestLI.childNodes[i as number].nodeName === '#text' &&
          closestLI.childNodes[i as number].textContent.trim().length === 0
        ) {
      ...
      while (
        currentLastElem.lastChild !== null &&
        currentLastElem.nodeName !== '#text'
      ) {
        currentLastElem = currentLastElem.lastChild as Element;
      }
      if (isDetached) {
        const currentLast: Node =
          currentLastElem.nodeName === 'BR' &&
          !isNOU(currentLastElem.previousSibling)
  ...
  public keyDown(e: KeyboardEvent): void {
    if (this.inputElement.classList.contains('e-mention')) {
      const mentionPopup: HTMLElement =
        this.inputElement.ownerDocument.getElementById(
          this.inputElement.id + '_popup'
        );
      const slashMenuPopup: HTMLElement =
        this.inputElement.ownerDocument.getElementById(
          this.inputElement.id + '_slash_menu_popup'
        );
      const mentionKeys: string[] = mentionRestrictKeys;
      const isMentionKeys: boolean = mentionKeys.indexOf(e.key) !== -1;
      const isMentionPopupOpen: boolean =
        mentionPopup && mentionPopup.classList.contains('e-popup-open');
      const isSlashMenuPopupOpen: boolean =
        slashMenuPopup && slashMenuPopup.classList.contains('e-popup-open');
    ...
    if (
      ...
      ((e.which === 8 && e.code === 'Backspace') ||
        (e.which === 46 && e.code === 'Delete'))
    ) {
      const range: Range = this.getRange();
      ...
      if (
        (closest(startNode, 'pre') &&
          e.which === 8 &&
          range.startContainer.textContent.charCodeAt(range.startOffset - 1) ===
            8203) ||
        (e.which === 46 &&
          range.startContainer.textContent.charCodeAt(range.startOffset) ===
            8203)
      ) {
        const regEx: RegExp = new RegExp('\u200B', 'g');
        const pointer: number =
          e.which === 8 ? range.startOffset - 1 : range.startOffset;
        range.startContainer.textContent =
          range.startContainer.textContent.replace(regEx, '');
        ...
        for (i = index; i >= 0; i--) {
          // eslint-disable-next-line max-len
          if (
            parentEle.childNodes[i as number].nodeType === 3 &&
            parentEle.childNodes[i as number].textContent.charCodeAt(0) ===
              8203 &&
            bool
          ) {
    ...
    const notFormatPainterCopy: boolean = isNOU((e as KeyboardEventArgs).action)
      ? true
      : (e as KeyboardEventArgs).action !== 'format-copy'
      ? true
      : false;
    if (
      this.formatter.getUndoRedoStack().length === 0 &&
      notFormatPainterCopy &&
    ...
    if (this.editorMode === 'HTML') {
      const range: Range = this.getRange();
      preventingMention =
        !isNOU(range.startContainer) &&
        range.startContainer === range.endContainer &&
        range.endContainer.childNodes.length > 1 &&
        !isNOU(range.startContainer.childNodes[range.startOffset - 1]) &&
        range.startContainer.childNodes[range.startOffset - 1].nodeName ===
          '#text' &&
        !isNOU(
          range.startContainer.childNodes[range.startOffset - 1].previousSibling
        ) &&
        range.startContainer.childNodes[
          range.startOffset - 1
        ].textContent.charCodeAt(0) === 32 &&
        (
          range.startContainer.childNodes[range.startOffset - 1]
            .previousSibling as HTMLElement
        ).classList.contains('e-mention-chip');
    }
    const keyboardEventAction: string[] = [
      'insert-link',
      'format-copy',
      'format-paste',
      'insert-image',
    ...
    if (
      keyboardEventAction.indexOf((e as KeyboardEventArgs).action) === -1 &&
      (!(e as KeyboardEvent).target ||
        !(
          ((e as KeyboardEvent).target as Element).classList.contains(
            'e-mention'
          ) &&
          !isNOU(
            document.querySelector(
              '#' +
                ((e as KeyboardEvent).target as Element).id +
                '_popup.e-popup-open'
            )
          ) &&
          e.code === 'Tab'
        )) &&
      (((e as KeyboardEventArgs).action &&
        (e as KeyboardEventArgs).action !== 'paste' &&
        (e as KeyboardEventArgs).action !== 'space') ||
        e.which === 9 ||
      ...
      if (!isNOU(this.formatPainterModule)) {
        FormatPainterEscapeAction =
          this.formatPainterModule.previousAction === 'escape';
      }
      if (!FormatPainterEscapeAction) {
        if (
          this.editorMode === 'HTML' &&
          ((e as KeyboardEventArgs).action === 'increase-fontsize' ||
            (e as KeyboardEventArgs).action === 'decrease-fontsize')
        ) {
          this.notify(events.onHandleFontsizeChange, {
      ...
      switch ((e as KeyboardEventArgs).action) {
        case 'toolbar-focus':
          if (this.toolbarSettings.enable && this.getToolbarElement()) {
            let firstActiveItem: HTMLElement =
              this.getToolbarElement().querySelector(
                '.e-toolbar-item:not(.e-overlay)[title]'
              );
            const quickToolbarElem: HTMLElement | null =
              this.getRenderedQuickToolbarElem();
            if (quickToolbarElem) {
              firstActiveItem = quickToolbarElem.querySelector(
                '.e-toolbar-item:not(.e-overlay)[title]'
              );
            }
            if (firstActiveItem) {
              const firstChild: HTMLElement =
                firstActiveItem.firstElementChild as HTMLElement;
    ...
    if (
      ...
      const range: Range = this.contentModule
        .getDocument()
        .getSelection()
        .getRangeAt(0);
  ...
  private keyUp(e: KeyboardEvent): void {
    if (this.editorMode === 'HTML') {
      ...
      if (
        ...
        const selection: Selection = this.iframeSettings.enable
          ? this.contentModule.getPanel().ownerDocument.getSelection()
          : this.contentModule.getDocument().getSelection();
        if (this.isCopyAll) {
          this.inputElement.innerHTML =
            this.enterKey !== 'BR'
              ? '<' + this.enterKey + '><br></' + this.enterKey + '>'
              : '<br>';
          this.isCopyAll = false;
        }
        if (
          selection.rangeCount > 0 &&
          this.contentModule.getDocument().activeElement.tagName !== 'INPUT' &&
          this.inputElement.contains(
            this.contentModule.getDocument().activeElement
          ) &&
          (range.startContainer as HTMLElement).innerHTML === '<br>' &&
          (range.startContainer as HTMLElement).textContent === ''
        ) {
      ...
      if (
        ...
        this.formatter.editorManager.nodeSelection.setSelectionText(
          ...
          tempElem.childNodes[0],
          tempElem.childNodes[0].textContent.length,
          tempElem.childNodes[0].textContent.length
        );
      }
    }
    this.notify(events.keyUp, { member: 'keyup', args: e });
    this.notify(events.tableModulekeyUp, {
      member: 'tableModulekeyUp',
      args: e,
    });
    if (
      e.code === 'KeyX' &&
    ...
    const allowedKeys: boolean =
      e.which === 32 || e.which === 13 || e.which === 8 || e.which === 46;
    const formatPainterCopy: boolean = e.key === 'C' && e.altKey && e.shiftKey;
    const formatPainterPaste: boolean = e.key === 'V' && e.altKey && e.shiftKey;
    if (
      (!formatPainterCopy &&
        !formatPainterPaste &&
        ((e.key !== 'shift' && !e.ctrlKey && e.key && e.key.length === 1) ||
          allowedKeys)) ||
      (this.editorMode === 'Markdown' &&
        ((e.key !== 'shift' && !e.ctrlKey && e.key && e.key.length === 1) ||
          allowedKeys)) ||
      (this.autoSaveOnIdle && Browser.isDevice && !this.inlineMode.enable)
    ) {
      this.formatter.onKeyHandler(this, e);
    }
    if (
      (this.inputElement && this.inputElement.textContent.length !== 0) ||
  ...
  public serializeValue(value: string): string {
    if (this.editorMode === 'HTML' && !isNOU(value)) {
      if (this.enableHtmlEncode) {
        ...
        value = this.htmlEditorModule.sanitizeHelper(value);
        value = this.enableXhtml
          ? this.htmlEditorModule.xhtmlValidation.selfEncloseValidation(value)
          : value;
      }
    }
    return value;
  ...
  private triggerEditArea(e: MouseEvent | TouchEvent): void {
    if (!isIDevice()) {
      ...
      const touch: Touch = <Touch>(
        ((e as TouchEvent).touches ? (e as TouchEvent).changedTouches[0] : e)
      );
      if (
        this.clickPoints.clientX === touch.clientX &&
        this.clickPoints.clientY === touch.clientY
      ) {
        this.notify(events.editAreaClick, { member: 'editAreaClick', args: e });
      }
    }
  ...
  private notifyMouseUp(e: MouseEvent | TouchEvent): void {
    ...
    if (
      (this.inputElement &&
        ((this.editorMode === 'HTML' &&
          (this.inputElement.textContent.length !== 0 ||
            (e.target &&
              !isNOU((e.target as HTMLElement).querySelector('li'))))) ||
          (this.editorMode === 'Markdown' &&
            (this.inputElement as HTMLTextAreaElement).value.length !== 0))) ||
      (e.target && !isNOU(closest(e.target as HTMLElement, 'table'))) ||
      (e.target && !isNOU((e.target as HTMLElement).querySelector('img'))) ||
      (e.target &&
        ((e.target as HTMLElement).nodeName === 'VIDEO' ||
          (e.target as HTMLElement).querySelectorAll(
            '.' + classes.CLS_VIDEOWRAP
          ).length > 0)) ||
      (e.target &&
        (e.target as HTMLElement).nodeName !== 'BR' &&
  ...
  private mouseUp(e: MouseEvent | TouchEvent): void {
    if (this.quickToolbarSettings.showOnRightClick && Browser.isDevice) {
      ...
      if (
        ...
        (closestTable &&
          this.contentModule.getEditPanel().contains(closestTable))
      ) {
        return;
      }
    }
  ...
  public onPaste(e?: KeyboardEvent | ClipboardEvent): void {
    ...
    this.trigger(
      ...
      (pasteArgs: { [key: string]: Object }) => {
        ...
        const selectionLength: number = this.getSelection().length;
        const pastedContentLength: number =
          isNOU(e as ClipboardEvent) ||
        ...
        if (
          ...
          const isImageDialogOpen: HTMLElement = this.contentModule
            .getDocument()
          ...
          if (!isNOU(this.pasteCleanupModule)) {
            ...
            console.warn(
              '[WARNING] :: Module "pasteCleanup" is not available in RichTextEditor component! You either misspelled the module name or forgot to load it.'
            );
            const args: Object = {
              requestType: 'Paste',
              editorMode: this.editorMode,
            ...
            const file: File =
              ...
              (e as ClipboardEvent).clipboardData.items.length > 0
                ? (e as ClipboardEvent).clipboardData.items[0].getAsFile()
                : null;
            if (value !== null) {
  ...
  public destroy(): void {
    ...
    if (this.originalElement.tagName === 'TEXTAREA') {
      ...
      if (this.originalElement.innerHTML.trim() !== '') {
        ...
        this.setProperties(
          { value: !isNOU(this.initialValue) ? this.initialValue : null },
          true
        );
      } else {
    ...
    const dialogElement: Element = document.querySelector(
      '.e-dialog.e-rte-elements'
    );
    if (dialogElement) {
  ...
  private removeHtmlAttributes(): void {
    if (this.htmlAttributes) {
      ...
      for (
        let i: number = 0;
        i < keys.length && this.element.hasAttribute(keys[i as number]);
        i++
      ) {
  ...
  private removeAttributes(): void {
    ...
    if (
      ...
      this.originalElement.style.width === ''
    ) {
      this.element.style.removeProperty('width');
    }
    if (
      this.element.style.height !== '' &&
      this.originalElement.style.height === ''
    ) {
  ...
  public getSelectedHtml(): string {
    ...
    const selection: Selection = this.contentModule
      .getDocument()
      .getSelection();
  ...
  public showInlineToolbar(): void {
    if (this.inlineMode.enable) {
      ...
      const targetElm: HTMLElement =
        currentRange.endContainer.nodeName === '#text'
          ? currentRange.endContainer.parentElement
          : (currentRange.endContainer as HTMLElement);
  ...
  public onPropertyChanged(
    newProp: RichTextEditorModel,
    oldProp: RichTextEditorModel
  ): void {
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        case 'value': {
          ...
          const val: string =
            this.editorMode === 'HTML' ? getEditValue(nVal, this) : nVal;
          if ((!isNOU(nVal) && nVal !== '') || prop === 'enterKey') {
            this.setProperties(
              { value: this.enableHtmlEncode ? this.encode(decode(val)) : val },
              true
            );
          }
          ...
          this.setPlaceHolder();
          this.notify(events.xhtmlValidation, {
        ...
        case 'cssClass':
          ...
          this.notify(events.bindCssClass, {
        ...
        case 'iframeSettings': {
          ...
          if (frameSetting.resources) {
            ...
            if (frameSetting.resources.scripts) {
              files = <NodeListOf<Element> & Element[]>(
                header.querySelectorAll('.' + classes.CLS_SCRIPT_SHEET)
              );
              this.removeSheets(files);
            }
            if (frameSetting.resources.styles) {
              files = <NodeListOf<Element> & Element[]>(
                header.querySelectorAll('.' + classes.CLS_STYLE_SHEET)
              );
              this.removeSheets(files);
            }
          }
        ...
        case 'locale':
          ...
          break;
        case 'inlineMode':
          this.notify(events.modelChanged, {
            module: 'quickToolbar',
            newProp: newProp,
            oldProp: oldProp,
          });
        ...
        case 'undoRedoSteps':
        case 'undoRedoTimer':
          this.formatter.editorManager.observer.notify(CONSTANT.MODEL_CHANGED, {
            newProp: newProp,
            oldProp: oldProp,
          });
          break;
        ...
        case 'quickToolbarSettings':
          newProp.quickToolbarSettings.showOnRightClick
  ...
  public updateValueData(): void {
    if (this.enableHtmlEncode) {
      ...
      this.setProperties({
        value: /<[a-z][\s\S]*>/i.test(this.inputElement.innerHTML)
          ? this.inputElement.innerHTML
  ...
  private updatePanelValue(): void {
    let value: string =
      this.editorMode === 'HTML'
        ? this.listOrderCorrection(this.value)
        : this.value;
    value = this.enableHtmlEncode && this.value ? decode(value) : value;
    const getTextArea: HTMLInputElement = this.element.querySelector(
      '.' + classes.CLS_RTE_SOURCE_CODE_TXTAREA
    );
    if (value) {
      if (
        !isNOU(getTextArea) &&
        this.rootContainer.classList.contains('e-source-code-enabled')
      ) {
      ...
      if (
        this.editorMode === 'HTML' &&
        this.inputElement &&
        this.inputElement.innerHTML.trim() !== value.trim()
      ) {
  ...
  private listOrderCorrection(value: string): string {
    ...
    const listElements: NodeListOf<Element> =
      valueElementWrapper.querySelectorAll('UL, OL');
    for (let i: number = 0; i < listElements.length; i++) {
      if (
        !isNOU(listElements[i as number]) &&
        !isNOU(listElements[i as number].parentElement) &&
        !isNOU(listElements[i as number].previousElementSibling) &&
        (listElements[i as number].parentElement.nodeName === 'UL' ||
          listElements[i as number].parentElement.nodeName === 'OL')
      ) {
        listElements[i as number].previousElementSibling.appendChild(
          listElements[i as number]
        );
      }
    }
  ...
  private setHeight(height: string | number): void {
    ...
    if (
      this.toolbarSettings.type === 'Expand' &&
      ((typeof this.height === 'string' && this.height.indexOf('px') > -1) ||
  ...
  public setPlaceHolder(): void {
    if (
      this.inputElement &&
      this.placeholder &&
      this.iframeSettings.enable !== true
    ) {
      if (this.editorMode !== 'Markdown') {
        if (!this.placeHolderWrapper) {
          this.placeHolderWrapper = this.createElement('span', {
            className:
              'rte-placeholder e-rte-placeholder' + ' ' + this.getCssClass(),
          });
        ...
        if (
          this.inputElement.textContent.length === 0 &&
          this.inputElement.childNodes.length < 2 &&
          !isNOU(this.inputElement.firstChild) &&
          (this.inputElement.firstChild.nodeName === 'BR' ||
            ((this.inputElement.firstChild.nodeName === 'P' ||
              this.inputElement.firstChild.nodeName === 'DIV') &&
              !isNOU(this.inputElement.firstChild.firstChild) &&
  ...
  private updateReadOnly(): void {
    this.notify(events.readOnlyMode, {
  ...
  public enableToolbarItem(
    items: string | string[],
    muteToolbarUpdate?: boolean
  ): void {
    this.toolbarModule.enableTBarItems(
      this.getBaseToolbarObject(),
      items,
      true,
      muteToolbarUpdate
    );
  ...
  public getRange(): Range {
    return this.formatter.editorManager.nodeSelection.getRange(
      this.contentModule.getDocument()
    );
  }
  ...
  private setIframeSettings(): void {
    if (this.iframeSettings.resources) {
      ...
      if (
        !isNOU(this.iframeSettings.resources.scripts) &&
        this.iframeSettings.resources.scripts.length > 0
      ) {
        this.InjectSheet(true, scriptSrc);
      }
      if (
        !isNOU(this.iframeSettings.resources.styles) &&
        this.iframeSettings.resources.styles.length > 0
      ) {
  ...
  private setValue(isPropertyChange?: boolean): void {
    if (this.valueTemplate) {
      ...
      if (
        typeof this.valueTemplate === 'string' &&
        regEx.test(this.valueTemplate)
      ) {
        this.setProperties({ value: this.valueTemplate });
      } else {
        const compiledTemplate: NodeList = compile(this.valueTemplate)(
          '',
          this,
          'valueTemplate'
        );
        ...
        if (typeof this.valueTemplate !== 'string' && (this as any).isReact) {
          ...
          this.renderTemplates(() => {
            this.inputElement.innerHTML = (
              this.displayTempElem.childNodes[0] as HTMLElement
            ).innerHTML;
      ...
      const innerHtml: string =
        !isNOU(this.element.innerHTML) &&
        this.element.innerHTML.replace(/<(\/?|\!?)(!--!--)>/g, '').trim();
      if (innerHtml !== '') {
        if (this.element.tagName === 'TEXTAREA') {
  ...
  public getInsertImgMaxWidth(): string | number {
    ...
    if (
      this.editorMode === 'HTML' &&
      !isNOU(this.formatter.editorManager.nodeSelection) &&
      !isNOU(this.formatter.editorManager.nodeSelection.range)
    ) {
      const currentRange: Range =
        this.formatter.editorManager.nodeSelection.range;
      if (
        currentRange.startContainer.nodeType !== 3 &&
        (currentRange.startContainer as HTMLElement).closest &&
    ...
    const editEleMaxWidth: number =
      editEle.offsetWidth -
      (imgPadding +
        imgResizeBorder +
        parseFloat(eleStyle.paddingLeft.split('px')[0]) +
        parseFloat(eleStyle.paddingRight.split('px')[0]) +
        parseFloat(eleStyle.marginLeft.split('px')[0]) +
        parseFloat(eleStyle.marginRight.split('px')[0]));
  ...
  public getHtml(): string {
    const htmlValue: string = this.removeResizeElement(
  ...
  public getXhtml(): string {
    ...
    if (!isNOU(currentValue) && this.enableXhtml) {
      currentValue =
        this.htmlEditorModule.xhtmlValidation.selfEncloseValidation(
          currentValue
        );
    }
    return currentValue;
  ...
  public getCharCount(): number {
    const htmlText: string =
      this.editorMode === 'Markdown'
        ? (this.inputElement as HTMLTextAreaElement).value.trim()
  ...
  public getBaseToolbarObject(): BaseToolbar {
    ...
    if (this.inlineMode.enable && (!Browser.isDevice || isIDevice())) {
      tbObj =
        ...
        this.quickToolbarModule.getInlineBaseToolbar();
    } else {
  ...
  public getToolbar(): HTMLElement {
    return this.toolbarModule
      ? <HTMLElement>this.toolbarModule.getToolbarElement()
      : null;
  }
  ...
  public preventDefaultResize(e: FocusEvent | MouseEvent): void {
    if (Browser.info.name === 'msie') {
      this.contentModule
        .getEditPanel()
        .addEventListener('mscontrolselect', this.preventImgResize);
  ...
  private defaultResize(e: FocusEvent): void {
    if (Browser.info.name === 'msie') {
      this.contentModule
        .getEditPanel()
        .removeEventListener('mscontrolselect', this.preventImgResize);
    } else if (Browser.info.name === 'mozilla') {
      this.contentModule
        .getDocument()
        .execCommand('enableObjectResizing', true, 'true');
      this.contentModule
        .getDocument()
        .execCommand('enableInlineTableEditing', true, 'true');
    }
  }
  ...
  private focusHandler(e: FocusEvent): void {
    if (!this.isRTE || this.isFocusOut) {
      ...
      if (this.editorMode === 'HTML') {
        this.cloneValue =
          ...
          this.inputElement.innerHTML === '<br>'
            ? null
            : this.enableHtmlEncode
            ? this.encode(decode(this.inputElement.innerHTML))
            : this.inputElement.innerHTML;
      } else {
        this.cloneValue =
          (this.inputElement as HTMLTextAreaElement).value === ''
      ...
      if (
        active === this.element ||
        active === this.getToolbarElement() ||
        active === this.contentModule.getEditPanel() ||
        (this.iframeSettings.enable &&
          active === this.contentModule.getPanel() &&
          e.target &&
        ...
        if (!isNOU(this.getToolbarElement())) {
          ...
          const items: NodeList =
            this.getToolbarElement().querySelectorAll('[tabindex="0"]');
      ...
      if (
        !isNOU(this.saveInterval) &&
        this.saveInterval > 0 &&
        !this.autoSaveOnIdle &&
        isNOU(this.timeInterval)
      ) {
        this.timeInterval = setInterval(
          this.updateValueOnIdle.bind(this),
          this.saveInterval
        );
      }
    ...
    if (!this.readonly) {
      ...
      if (
        currentFocus === 'editArea' ||
        currentFocus === 'textArea' ||
        currentFocus === 'sourceCode'
      ) {
    ...
    if (this.editorMode === 'HTML') {
      value =
        ...
        this.inputElement.innerHTML === '<br>'
          ...
          : this.enableHtmlEncode
          ? this.encode(
              decode(this.removeResizeElement(this.inputElement.innerHTML))
            )
          : this.inputElement.innerHTML;
      if (
        !isNOU(getTextArea) &&
  ...
  private updateValueOnIdle(): void {
    if (
      !isNOU(this.tableModule) &&
      !isNOU(this.inputElement.querySelector('.e-table-box.e-rbox-select'))
    ) {
      return;
    }
  ...
  public addAnchorAriaLabel(value: string): string {
    ...
    if (item.length > 0) {
      for (let i: number = 0; i < item.length; i++) {
        if (
          item[i as number].hasAttribute('target') &&
          item[i as number].getAttribute('target') === '_blank'
        ) {
          item[i as number].setAttribute(
            'aria-label',
            (
              this.serviceLocator.getService<L10n>('rteLocale') as L10n
            ).getConstant('linkAriaLabel')
          );
        }
      }
    }
  ...
  private removeResizeElement(value: string): string {
    ...
    if (this.editorMode === 'HTML') {
      valueElementWrapper.innerHTML = value;
      const item: NodeListOf<Element> = valueElementWrapper.querySelectorAll(
    ...
    return this.editorMode === 'Markdown'
  ...
  private updateStatus(e: StatusArgs): void {
    if (!isNOU(e.html) || !isNOU(e.markdown)) {
      const status: { [key: string]: boolean } =
        this.formatter.editorManager.undoRedoManager.getUndoStatus();
  ...
  private onDocumentClick(e: MouseEvent): void {
    ...
    const hideQuickToolbarChecker: boolean =
      this.quickToolbarModule &&
      !this.inlineMode.enable &&
      isNOU(this.quickToolbarModule.inlineQTBar);
    if (
      (hideQuickToolbarChecker &&
        !isNOU(closest(target, '.' + 'e-toolbar-wrapper'))) ||
      (hideQuickToolbarChecker &&
        (!isNOU(closest(target, '.e-rte-table-resize')) ||
          !isNOU(closest(target, '.e-table-box'))))
    ) {
  ...
  private blurHandler(e: FocusEvent): void {
    ...
    if (trg) {
      ...
      if (!rteElement && this.iframeSettings.enable) {
        const iframeElement: HTMLIFrameElement = this.element.querySelector(
          '#' + this.getID() + '_rte-view'
        );
        if (
          iframeElement &&
          iframeElement.contentWindow.document.body.contains(trg)
        ) {
      ...
      if (rteElement && rteElement === this.element) {
        ...
        closest(trg, '[aria-owns="' + this.getID() + '"]') ||
        closest(trg, '.' + classes.CLS_RTE_ELEMENTS)
      ) {
        this.isBlur = false;
      } else {
        this.isBlur = true;
    ...
    if (this.isBlur && isNOU(trg)) {
      ...
      if (
        !isNOU(this.placeHolderWrapper) &&
        this.element.querySelector('[title = Preview]')
      ) {
    ...
    if (this.autoSaveOnIdle) {
      if (!isNOU(this.saveInterval)) {
        ...
        this.autoSaveTimeOut = setTimeout(
          this.updateIntervalValue.bind(this),
          this.saveInterval
        );
      }
    }
  ...
  private wireContextEvent(): void {
    if (this.quickToolbarSettings.showOnRightClick) {
      ...
      if (Browser.isDevice) {
        this.touchModule = new EJ2Touch(this.inputElement, {
  ...
  private contextHandler(e: MouseEvent): void {
    let closestElem: Element = closest(
      e.target as HTMLElement,
      'a, table, img, video, audio'
    );
    if (
      !closestElem &&
      (e.target as HTMLElement) &&
    ...
    if (
      this.inlineMode.onSelection === false ||
      (!isNOU(closestElem) &&
        this.inputElement.contains(closestElem) &&
        (closestElem.tagName === 'IMG' ||
          closestElem.tagName === 'TABLE' ||
          closestElem.tagName === 'A' ||
          closestElem.tagName.toLowerCase() === 'video' ||
          closestElem.tagName.toLowerCase() === 'audio'))
    ) {
      e.preventDefault();
    }
  }
  ...
  public autoResize(): void {
    ...
    if (this.height === 'auto') {
      ...
      if (this.editorMode === 'Markdown') {
        ...
        const otherElemHeight: number =
          this.enableResize || this.showCharCount ? 20 : 0;
        ...
        if (textArea) {
          textArea.style.height =
            ...
            (this.toolbarModule.getToolbarHeight() + otherElemHeight + 3) +
            'px';
        }
      } else if (this.iframeSettings.enable) {
        const iframe: HTMLIFrameElement = this.element.querySelector(
          '#' + this.getID() + '_rte-view'
        );
        const otherElemHeight: number =
          this.enableResize || this.showCharCount ? 20 : 0;
  ...
  private wireEvents(): void {
    ...
    if (this.iframeSettings.enable) {
      ...
      this.contentModule
        .getEditPanel()
        .addEventListener('load', this.onLoadHandler, true);
    }
    if (this.readonly && this.enabled) {
      return;
    }
  ...
  private restrict(e: MouseEvent | KeyboardEvent): void {
    if (this.maxLength >= 0) {
      ...
      if (
        ...
        this.maxLength !== -1 &&
        (e as MouseEvent).which !== arrayKey
      ) {
  ...
  private bindEvents(): void {
    this.keyboardModule = new KeyboardEvents(this.inputElement, {
      ...
      keyConfigs: { ...this.formatter.keyConfig, ...this.keyConfig },
      eventName: 'keydown',
    });
    ...
    this.wireContextEvent();
    this.formatter.editorManager.observer.on(
      CONSTANT.KEY_DOWN_HANDLER,
      this.editorKeyDown,
      this
    );
    ...
    if (this.iframeSettings.enable) {
      ...
      EventHandler.add(
        this.inputElement.ownerDocument,
        'scroll',
        this.contentScrollHandler,
        this
      );
  ...
  private unWireEvents(): void {
    ...
    if (this.iframeSettings.enable) {
      this.contentModule
        .getEditPanel()
        .removeEventListener('load', this.onLoadHandler, true);
  ...
  private unbindEvents(): void {
    ...
    this.unWireContextEvent();
    if (this.formatter) {
      this.formatter.editorManager.observer.off(
        CONSTANT.KEY_DOWN_HANDLER,
        this.editorKeyDown
      );
    }
  ...
  private getCurrentFocus(e: FocusEvent): string {
    if (
      ...
      e.target === this.getToolbarElement() ||
      (!isNOU(e.relatedTarget) &&
        closest(e.relatedTarget as Element, '.e-rte-toolbar') ===
          this.getToolbarElement())
    ) {
      return 'toolbar';
    } else if (
      e.target === this.valueContainer &&
      document.activeElement === this.valueContainer
    ) {
      return 'textArea';
    } else if (
      !isNOU(e.target) &&
  ...
  private resetToolbarTabIndex(): void {
    if (this.getToolbarElement()) {
      const toolbarItem: NodeList = this.getToolbarElement().querySelectorAll(
        'input,select,button,a,[tabindex]'
      );
      for (let i: number = 0; i < toolbarItem.length; i++) {
        if (
          !(toolbarItem[i as number] as HTMLElement).classList.contains(
            'e-rte-dropdown-btn'
          ) &&
          !(toolbarItem[i as number] as HTMLElement).classList.contains(
            'e-insert-table-btn'
          ) &&
          (!(toolbarItem[i as number] as HTMLElement).hasAttribute(
            'tabindex'
          ) ||
            (toolbarItem[i as number] as HTMLElement).getAttribute(
              'tabindex'
            ) !== '-1')
        ) {
          (toolbarItem[i as number] as HTMLElement).setAttribute(
            'tabindex',
            '-1'
          );
        }
  ...
  private removeSelectionClassStates(element: HTMLElement): void {
    const classNames: string[] = [
      classes.CLS_IMG_FOCUS,
      classes.CLS_TABLE_SEL,
      classes.CLS_TABLE_MULTI_CELL,
      classes.CLS_TABLE_SEL_END,
      classes.CLS_VID_FOCUS,
      classes.CLS_AUD_FOCUS,
    ];
    for (let i: number = 0; i < classNames.length; i++) {
      const item: NodeListOf<Element> = element.querySelectorAll(
        '.' + classNames[i as number]
      );
  ...
  private getRenderedQuickToolbarElem(): HTMLElement | null {
    const quickToolbars: IBaseQuickToolbar[] =
      this.quickToolbarModule.getQuickToolbarInstance();
