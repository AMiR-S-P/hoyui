...
import { Browser, detach, SanitizeHtmlHelper, extend } from '@hoyui/base';
...
import {
  IToolbarItems,
  IDropDownItemModel,
  ISetToolbarStatusArgs,
  IToolbarItemModel,
} from './interface';

const undoRedoItems: string[] = ['Undo', 'Redo'];
const inlineNode: string[] = [
  'a',
  'abbr',
  'acronym',
  'audio',
...
export function getDropDownValue(
  ...
  for (let k: number = 0; k < items.length; k++) {
    if (
      type === 'value' &&
      items[k as number].value.toLocaleLowerCase() === value.toLocaleLowerCase()
    ) {
      data = items[k as number];
      break;
    } else if (
      type === 'text' &&
      items[k as number].text.toLocaleLowerCase() === value.toLocaleLowerCase()
    ) {
      data = items[k as number];
      break;
    } else if (
      type === 'subCommand' &&
      items[k as number].subCommand.toLocaleLowerCase() ===
 ...
 * @hidden
 */
export function setToolbarStatus(
  e: ISetToolbarStatusArgs,
  isPopToolbar: boolean,
  self: IRichTextEditor
): void {
  ...
  for (const key of keys) {
    for (let j: number = 0; j < e.tbItems.length; j++) {
      ...
      if (
        (item && itemStr === key) ||
        (item === 'UL' && key === 'unorderedlist') ||
        (item === 'OL' && key === 'orderedlist') ||
        (itemStr === 'pre' && key === 'insertcode') ||
        (item === 'NumberFormatList' && key === 'numberFormatList') ||
        ...
        if (typeof data[`${key}`] === 'boolean') {
          ...
          switch (key) {
            case 'formats': {
              if (
                isNOU(dropDown.formatDropDown) ||
                isPopToolbar ||
                (!isNOU(dropDown.formatDropDown) &&
                  dropDown.formatDropDown.isDestroyed)
              ) {
                break;
              }
              const formatItems: IDropDownItemModel[] = e.parent.format.types;
              const formatContent: string = isNOU(e.parent.format.default)
                ? formatItems[0].text
                : e.parent.format.default;
              result =
                value === 'empty'
                  ? ''
                  : getDropDownValue(formatItems, value, 'subCommand', 'text');
              dropDown.formatDropDown.content =
                '<span style="display: inline-flex;' +
                'width:' +
                e.parent.format.width +
                '" >' +
                '<span class="e-rte-dropdown-btn-text' +
                (isNOU(e.parent.cssClass) ? '' : ' ' + e.parent.cssClass) +
                '">' +
            ...
            case 'alignments': {
              if (
                isNOU(dropDown.alignDropDown) ||
                (!isNOU(dropDown.alignDropDown) &&
                  dropDown.alignDropDown.isDestroyed)
              ) {
                break;
              }
              ...
              dropDown.alignDropDown.iconCss = isNOU(result)
                ? 'e-icons e-justify-left'
                : result;
            ...
            case 'fontname': {
              if (
                isNOU(dropDown.fontNameDropDown) ||
                isPopToolbar ||
                (!isNOU(dropDown.fontNameDropDown) &&
                  dropDown.fontNameDropDown.isDestroyed)
              ) {
                break;
              }
              const fontNameItems: IDropDownItemModel[] =
                e.parent.fontFamily.items;
              result =
                value === 'empty'
                  ? ''
                  : getDropDownValue(fontNameItems, value, 'value', 'text');
              const fontNameContent: string = isNOU(e.parent.fontFamily.default)
                ? fontNameItems[0].text
                : e.parent.fontFamily.default;
              const name: string =
                (isNOU(result) ? fontNameContent : result) === 'Default'
                  ? self.serviceLocator
                      .getService<L10n>('rteLocale')
                      .getConstant('fontName')
                  : isNOU(result)
                  ? fontNameContent
                  : result;
            ...
            case 'fontsize': {
              if (
                isNOU(dropDown.fontSizeDropDown) ||
                (!isNOU(dropDown.fontSizeDropDown) &&
                  dropDown.fontSizeDropDown.isDestroyed)
              ) {
                break;
              }
              const fontSizeItems: IDropDownItemModel[] =
                e.parent.fontSize.items;
              const fontSizeContent: string = isNOU(e.parent.fontSize.default)
                ? fontSizeItems[0].text
                : e.parent.fontSize.default;
              const fontSizeToolbarText: string = getDropDownValue(
                fontSizeItems,
                value === '' ? fontSizeContent.replace(/\s/g, '') : value,
                fontSizeContent.replace(/\s/g, '') === fontSizeItems[0].text &&
                  value === ''
                  ? 'text'
                  : 'value',
                'text'
              ...
              result =
                value === 'empty'
                  ? ''
                  : fontSizeToolbarText === 'Default'
                  ? self.serviceLocator
                      .getService<L10n>('rteLocale')
                      .getConstant('fontSize')
                  : fontSizeToolbarText;
              dropDown.fontSizeDropDown.content =
                '<span style="display: inline-flex;' +
                'width:' +
                e.parent.fontSize.width +
                '" >' +
                '<span class="e-rte-dropdown-btn-text' +
                (isNOU(e.parent.cssClass) ? '' : ' ' + e.parent.cssClass) +
                '">' +
 ...
 * @hidden
 */
export function getTBarItemsIndex(
  items: string[],
  toolbarItems: IToolbarItemModel[]
): number[] {
  ...
  for (let i: number = 0; i < items.length; i++) {
    for (let j: number = 0; j < toolbarItems.length; j++) {
      if (toolbarItems[j as number].type === 'Separator') {
        ...
        if (
          (items[i as number] === 'OrderedList' ||
            items[i as number] === 'NumberFormatList') &&
          toolbarItems[j as number].subCommand === 'OL'
        ) {
          itemsIndex.push(j);
          break;
        } else if (
          (items[i as number] === 'UnorderedList' ||
            items[i as number] === 'BulletFormatList') &&
          toolbarItems[j as number].subCommand === 'UL'
        ) {
          itemsIndex.push(j);
          break;
        } else if (
          items[i as number] === 'InsertCode' &&
          toolbarItems[j as number].subCommand === 'Pre'
        ) {
          itemsIndex.push(j);
          break;
        } else if (
          items[i as number] === 'Blockquote' &&
          toolbarItems[j as number].subCommand === 'blockquote'
        ) {
          itemsIndex.push(j);
          break;
        } else if (
          items[i as number] === 'FileManager' &&
          toolbarItems[j as number].subCommand === 'File'
        ) {
          itemsIndex.push(j);
          break;
        } else if (
          typeof items[i as number] === 'object' &&
          (items[i as number] as IToolbarItems).command === 'Custom'
        ) {
          itemsIndex.push(i);
          break;
        } else if (
          items[i as number] === toolbarItems[j as number].subCommand
        ) {
          itemsIndex.push(j);
          break;
        }
 ...
 * @hidden
 */
export function updateUndoRedoStatus(
  baseToolbar: BaseToolbar,
  undoRedoStatus: { [key: string]: boolean }
): void {
  let i: number = 0;
  const trgItems: number[] = getTBarItemsIndex(
    getCollection(undoRedoItems),
    baseToolbar.toolbarObj.items
  );
  const tbItems: HTMLElement[] = selectAll(
    '.' + classes.CLS_TB_ITEM,
    baseToolbar.toolbarObj.element
  );
...
export function parseHtml(value: string): DocumentFragment {
  const tempNode: HTMLTemplateElement = <HTMLTemplateElement>(
 ...
 * @hidden
 */
export function toObjectLowerCase(obj: { [key: string]: IToolsItemConfigs }): {
  ...
  for (let i: number = 0; i < Object.keys(obj).length; i++) {
    convertedValue[keys[i as number].toLocaleLowerCase()] =
      obj[keys[i as number]];
  }
...
export function getEditValue(value: string, rteObj: IRichTextEditor): string {
  let val: string;
  if (value !== null && value !== '') {
    val = rteObj.enableHtmlEncode
      ? updateTextNode(decode(value), rteObj)
      : updateTextNode(value, rteObj);
...
export function updateTextNode(
  ...
  if (childNodes.length > 0) {
    ...
    while (tempNode.firstChild) {
      const emptyBlockElem: NodeListOf<Element> = tempNode.querySelectorAll(
        CONSTANT.blockEmptyNodes
      );
      ...
      for (let i: number = 0; i < allPNodes.length; i++) {
        if (
          allPNodes[i as number].textContent.trim().length === 0 &&
          allPNodes[i as number].childNodes.length === 1 &&
          allPNodes[i as number].childNodes[0].nodeName === '#text' &&
          isNOU(
            allPNodes[i as number].childNodes[0].textContent.match(/\u00a0/g)
          )
        ) {
          allPNodes[i as number].innerHTML = '<br>';
        }
      }
      const emptyInlineElem: NodeListOf<Element> = tempNode.querySelectorAll(
        CONSTANT.inlineEmptyNodes
      );
      ...
      if (
        rteObj.enterKey !== 'BR' &&
        ((tempNode.firstChild.nodeName === '#text' &&
          (tempNode.firstChild.textContent.indexOf('\n') < 0 ||
            tempNode.firstChild.textContent.trim() !== '')) ||
          inlineNode.indexOf(
            tempNode.firstChild.nodeName.toLocaleLowerCase()
          ) >= 0)
      ) {
        ...
        tempNode.firstChild.nodeName === '#text' &&
        (tempNode.firstChild.textContent === '\n' ||
          (tempNode.firstChild.textContent.indexOf('\n') >= 0 &&
            tempNode.firstChild.textContent.trim() === ''))
      ) {
    ...
    for (let i: number = 0; i < imageElm.length; i++) {
      if (
        (imageElm[i as number] as HTMLImageElement).classList.contains(
      ...
      if (
        !(
          imageElm[i as number].classList.contains(classes.CLS_IMGINLINE) ||
          imageElm[i as number].classList.contains(classes.CLS_IMGBREAK)
        ) &&
        !(
          imageElm[i as number].classList.contains('e-imgleft') ||
          imageElm[i as number].classList.contains('e-imgright') ||
          imageElm[i as number].classList.contains('e-imgcenter')
        )
      ) {
...
export function getDefaultValue(rteObj: IRichTextEditor): string {
  ...
  if (rteObj.enterKey === 'DIV') {
    currentVal = rteObj.enableHtmlEncode
      ? '&lt;div&gt;&lt;br/&gt;&lt;/div&gt;'
      : '<div><br/></div>';
  } else if (rteObj.enterKey === 'BR') {
...
export function sanitizeHelper(value: string, parent: IRichTextEditor): string {
  if (parent.enableHtmlSanitizer) {
    ...
    if (
      item.selectors.tags[2] &&
      item.selectors.tags[2].indexOf('iframe') > -1
    ) {
 ...
 * @hidden
 */
export function getLocaleFontFormat(
  self: IRichTextEditor,
  localeItems: { [ket: string]: string }[],
  item: IDropDownItemModel
): string {
  for (let i: number = 0; localeItems.length > i; i++) {
    if (
      localeItems[i as number].value === item.value ||
      localeItems[i as number].value === item.subCommand
    ) {
...
export function updateDropDownFontFormatLocale(self: IRichTextEditor): void {
  model.fontFamily.forEach((item: IDropDownItemModel, i: number) => {
    model.fontFamily[i as number].text = getLocaleFontFormat(
      self,
      fontNameLocale,
      model.fontFamily[i as number]
    );
  });
  model.formatItems.forEach((item: IDropDownItemModel, i: number) => {
    model.formatItems[i as number].text = getLocaleFontFormat(
      self,
      formatsLocale,
      model.formatItems[i as number]
    );
  });
  model.numberFormatList.forEach((item: IDropDownItemModel, i: number) => {
    model.numberFormatList[i as number].text = getLocaleFontFormat(
      self,
      numberFormatListLocale,
      model.numberFormatList[i as number]
    );
  });
  model.bulletFormatList.forEach((item: IDropDownItemModel, i: number) => {
    model.bulletFormatList[i as number].text = getLocaleFontFormat(
      self,
      bulletFormatListLocale,
      model.bulletFormatList[i as number]
    );
  });
}
