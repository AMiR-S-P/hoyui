...
import {
  IHtmlFormatterCallBack,
  IMarkdownFormatterCallBack,
  IUndoCallBack,
...
export class Formatter {
  ...
  public process(
    ...
    args: ActionBeginEventArgs,
    event: MouseEvent | KeyboardEvent,
    value: IItemCollectionArgs
  ): void {
    const selection: Selection = self.contentModule
      .getDocument()
      .getSelection();
    const range: Range =
    ...
    const isKeyboardVideoInsert: boolean =
      !isNOU(value) &&
      !isNOU((value as IVideoCommandsArgs).cssClass) &&
      (value as IVideoCommandsArgs).cssClass !== 'e-video-inline';
    if (self.editorMode === 'HTML') {
      if (!isNOU(args) && !isKeyboardVideoInsert) {
        if (
          isNOU(args.name) ||
          (!isNOU(args.name) && args.name !== 'showDialog')
        ) {
          if (newRange) {
            saveSelection = this.editorManager.nodeSelection.save(
              newRange,
              self.contentModule.getDocument()
            );
          } else {
            saveSelection = this.editorManager.nodeSelection.save(
              range,
    ...
    if (
      ...
      !(
        self.contentModule
          .getEditPanel()
          .contains(this.getAncestorNode(range.commonAncestorContainer)) ||
    ...
    if (!isNOU(args) && self.maxLength !== -1 && !isNOU(args.item.command)) {
      ...
      if (args.item.command === 'Links') {
        currentInsertContentLength =
          value.text.length === 0 ? value.url.length : value.text.length;
      }
      if (
        args.item.command === 'Images' ||
        args.item.command === 'Videos' ||
      ...
      const currentLength: number = self
        .getText()
        .trim()
        .replace(/(\r\n|\n|\r|\t)/gm, '')
        .replace(/\u200B/g, '').length;
    ...
    if (isNOU(args)) {
      ...
      if (!args.cancel) {
        const isTableModule: boolean = isNOU(self.tableModule)
          ? true
          : self.tableModule
          ? self.tableModule.ensureInsideTableList
          : false;
        if ((event.which === 9 && isTableModule) || event.which !== 9) {
          ...
          this.editorManager.observer.notify(
            event.type === 'keydown' ? KEY_DOWN : KEY_UP,
            {
              event: event,
      ...
      ((args.item.command !== args.item.subCommand &&
        args.item.command !== 'Font' &&
        args.item.command !== 'Export') ||
        ((args.item.subCommand === 'FontName' ||
          args.item.subCommand === 'FontSize') &&
          args.name === 'dropDownSelect') ||
        ((args.item.subCommand === 'BackgroundColor' ||
          args.item.subCommand === 'FontColor') &&
          args.name === 'colorPickerChanged') ||
        args.item.subCommand === 'FormatPainter' ||
        args.item.subCommand === 'EmojiPicker')
    ) {
      extend(
        args,
        args,
      ...
      self.trigger(
        ...
        (actionBeginArgs: ActionBeginEventArgs) => {
          if (!actionBeginArgs.cancel) {
            const formatPainterCopy: boolean =
              !isNOU(actionBeginArgs.requestType) &&
              actionBeginArgs.requestType === 'FormatPainter' &&
              actionBeginArgs.name === 'format-copy';
            const formatPainterPaste: boolean =
              !isNOU(actionBeginArgs.requestType) &&
              actionBeginArgs.requestType === 'FormatPainter' &&
              actionBeginArgs.name === 'format-paste';
            if (
              (this.getUndoRedoStack().length === 0 &&
                actionBeginArgs.item.command !== 'Links' &&
            ...
            self.isBlur = false;
            const quickToolbarAction: boolean =
              !isNOU(event) &&
              !isNOU(event.target) &&
              (!isNOU(
                closest(
                  event.target as HTMLElement,
                  '.e-rte-elements.e-dropdown-popup.e-rte-dropdown-popup.e-quick-dropdown.e-popup-open'
                )
              ) ||
                !isNOU(
                  closest(
                    event.target as HTMLElement,
                    '.e-rte-elements.e-rte-quick-popup.e-popup-open'
                  )
                ));
            if (
              isNOU(saveSelection) ||
              (!quickToolbarAction &&
                (isNOU(
                  closest(
                    saveSelection.range.startContainer.parentElement,
                    '.e-img-caption'
                  )
                )
                  ? true
            ...
            if (self.editorMode === 'HTML' && !isKeyboardVideoInsert) {
              if (
                isNOU(args.selectType) ||
                (!isNOU(args.selectType) && args.selectType !== 'showDialog')
              ) {
                saveSelection.restore();
              }
            }
            const command: string =
              actionBeginArgs.item.subCommand.toLocaleLowerCase();
            if (command === 'image') {
            ...
            if (
              ...
              this.editorManager.observer.notify(CONSTANT.checkUndo, {
              ...
              this.editorManager.execCommand(
                ...
                actionBeginArgs.item.subCommand === 'Pre' &&
                  actionBeginArgs.selectType === 'dropDownSelect'
    ...
    if (
      isNOU(event) ||
      (event && (event as KeyboardEventArgs).action !== 'copy')
    ) {
  ...
  public onSuccess(
    self: IRichTextEditor,
    events: IMarkdownFormatterCallBack | IHtmlFormatterCallBack
  ): void {
    self.notify(CONSTANT.contentChanged, {});
    if (
      events &&
      (isNOU(events.event) ||
        (events.event as KeyboardEventArgs).action !== 'copy')
    ) {
      this.enableUndo(self);
      self.notify(CONSTANT.execCommandCallBack, events);
    }
    self.trigger(
      CONSTANT.actionComplete,
      events,
      (callbackArgs: IMarkdownFormatterCallBack | IHtmlFormatterCallBack) => {
        self.setPlaceHolder();
        if (
          (callbackArgs.requestType === 'Images' ||
            callbackArgs.requestType === 'Links' ||
            callbackArgs.requestType === 'Audios' ||
            callbackArgs.requestType === 'Videos') &&
          self.editorMode === 'HTML'
        ) {
          const args: IHtmlFormatterCallBack =
            callbackArgs as IHtmlFormatterCallBack;
          if (
            callbackArgs.requestType === 'Links' &&
            callbackArgs.event &&
  ...
  public enableUndo(self: IRichTextEditor): void {
    ...
    if (self.inlineMode.enable && (!Browser.isDevice || isIDevice())) {
      updateUndoRedoStatus(
        self.quickToolbarModule.inlineQTBar.quickTBarObj,
        status
      );
    } else {
