...
export class QuickToolbar {
  ...
  public createQTBar(
    ...
    const qTBar: BaseQuickToolbar = new BaseQuickToolbar(
      this.parent,
      this.locator
    );
    qTBar.render(
      this.getQTBarOptions(popupType, mode, this.formatItems(items), type)
    );
    return qTBar;
  }
  ...
  private initializeQuickToolbars(): void {
    ...
    if (
      ...
      this.parent.inlineMode.onSelection &&
      isIDevice()
    ) {
      EventHandler.add(
  ...
  private renderQuickToolbars(): void {
    ...
    this.linkQTBar = this.createQTBar(
      'Link',
      'Scrollable',
      this.parent.quickToolbarSettings.link,
      RenderType.LinkToolbar
    );
    ...
    if (
      ...
      this.textQTBar = this.createQTBar(
        'Text',
        'MultiRow',
        this.parent.quickToolbarSettings.text,
        RenderType.TextToolbar
      );
  ...
  private renderInlineQuickToolbar(): void {
    if (this.parent.inlineMode.enable && (!Browser.isDevice || isIDevice())) {
      ...
      this.inlineQTBar = this.createQTBar(
        'Inline',
        'MultiRow',
        this.parent.toolbarSettings.items,
        RenderType.InlineToolbar
      );
      this.renderFactory.addRenderer(
        RenderType.InlineToolbar,
        this.inlineQTBar
  ...
  public hideInlineQTBar(): void {
    if (
      this.inlineQTBar &&
      !hasClass(this.inlineQTBar.element, 'e-popup-close')
    ) {
  ...
  public hideQuickToolbars(): void {
    if (
      this.linkQTBar &&
      !hasClass(this.linkQTBar.element, 'e-popup-close') &&
    ...
    if (
      this.textQTBar &&
      !hasClass(this.textQTBar.element, 'e-popup-close') &&
    ...
    if (
      this.imageQTBar &&
      !hasClass(this.imageQTBar.element, 'e-popup-close') &&
    ...
    if (
      this.audioQTBar &&
      !hasClass(this.audioQTBar.element, 'e-popup-close') &&
    ...
    if (
      this.videoQTBar &&
      !hasClass(this.videoQTBar.element, 'e-popup-close') &&
    ...
    if (
      this.tableQTBar &&
      !hasClass(this.tableQTBar.element, 'e-popup-close') &&
    ...
    if (this.parent.inlineMode.enable && (!Browser.isDevice || isIDevice())) {
      ...
      if (isNullOrUndefined(select('.' + CLS_INLINE_POP, document.body))) {
        ...
        if (target.nodeName === 'TEXTAREA') {
          ...
          const closestAnchor: HTMLElement = closest(
            target,
            'a'
          ) as HTMLElement;
          ...
          if (
            (isNOU(closest(startNode, 'A')) || isNOU(closest(endNode, 'A'))) &&
            (!closest(target, 'td,th') || !range.collapsed) &&
            (target.tagName !== 'IMG' ||
              this.parent.getRange().startOffset !==
                this.parent.getRange().endOffset)
          ) {
    ...
    if (
      ...
      if (
        (range.endContainer.parentElement.tagName ===
          ...
          range.startContainer.parentElement.tagName === 'A' &&
          range.endContainer.parentElement.tagName === 'A') ||
        target.tagName === 'IMG' ||
        target.tagName === 'VIDEO' ||
        this.isEmbedVidElem(target) ||
        target.tagName === 'AUDIO' ||
        (target.childNodes[0] &&
          target.childNodes[0].nodeType === 1 &&
          (target.childNodes[0 as number] as HTMLElement).classList.contains(
            'e-rte-audio'
          )) ||
  ...
  private keyDownHandler(e: NotifyArgs): void {
    ...
    if (
      this.parent.inlineMode.enable &&
      (e.args as KeyboardEvent).metaKey &&
      (e.args as KeyboardEvent).keyCode === 65
    ) {
      this.showInlineQTBar(
        this.offsetX,
        this.offsetY,
        (e.args as KeyboardEvent).target as HTMLElement
      );
      return;
    }
    if (!preventHide) {
  ...
  private onSelectionChange(e: Event): void {
    ...
    const selection: Selection = this.contentRenderer
      .getDocument()
      .getSelection();
  ...
  private updateCss(baseQTObj: BaseQuickToolbar, e: ICssClassArgs): void {
    if (baseQTObj && e.cssClass) {
      if (
        isNullOrUndefined(
          e.oldCssClass &&
            baseQTObj.quickTBarObj.toolbarObj.cssClass !== e.cssClass
        )
      ) {
        baseQTObj.quickTBarObj.toolbarObj.setProperties({
          cssClass: (
            baseQTObj.quickTBarObj.toolbarObj.cssClass +
            ' ' +
            e.cssClass
          ).trim(),
        });
