...
import { SuccessEventArgs, Uploader } from '@hoyui/inputs';
  ...
  private onImport(): void {
    this.uploaderObj = new Uploader({
      ...
      success: (args: SuccessEventArgs) => {
        (this.parent as RichTextEditor).executeCommand(
          'insertHTML',
          ((args.e as MouseEvent).currentTarget as XMLHttpRequest).response,
          { undo: true }
        );
      },
    });
    ...
    const uploadParentEle: HTMLElement = this.parent.createElement('div', {
      className:
        'e-import-uploadwrap e-droparea' + this.parent.getCssClass(true),
    });
    ...
    (
      this.uploaderObj.element.closest('.e-upload') as HTMLElement
    ).style.display = 'none';
  }
