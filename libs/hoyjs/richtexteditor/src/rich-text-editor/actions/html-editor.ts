...
import { isNullOrUndefined as isNOU } from '@hoyui/base';
...
export class HtmlEditor {
  ...
  private addEventListener(): void {
    ...
    this.parent.on(
      events.onHandleFontsizeChange,
      this.onHandleFontsizeChange,
      this
    );
  }
  ...
  private isTableClassAdded(): void {
    const tableElement: NodeListOf<HTMLElement> =
      this.parent.inputElement.querySelectorAll('table');
    for (let i: number = 0; i < tableElement.length; i++) {
      if (
        !tableElement[i as number].classList.contains('e-rte-table') &&
        !tableElement[i as number].classList.contains('e-rte-paste-table')
      ) {
  ...
  private onHandleFontsizeChange(e: NotifyArgs): void {
    ...
    const items: IDropDownItemModel[] = (this.parent.fontSize as FontSizeModel)
      .items;
    let activeElem: string;
    if (
      this.parent.toolbarModule &&
      (this.parent.toolbarModule as Toolbar).dropDownModule &&
      (this.parent.toolbarModule as Toolbar).dropDownModule.fontSizeDropDown &&
      !isNOU(
        (this.parent.toolbarModule as Toolbar).dropDownModule.fontSizeDropDown
          .activeElem[0].textContent
      ) &&
      (this.parent.toolbarModule as Toolbar).dropDownModule.fontSizeDropDown
        .activeElem[0].textContent !== ''
    ) {
      activeElem =
        this.parent.toolbarModule.dropDownModule.fontSizeDropDown.activeElem[0]
          .textContent;
    } else {
      let fontSizeValue: string;
      const selection: Selection = this.parent.contentModule
        .getDocument()
        .getSelection();
      if (
        selection &&
        ...
        fontSizeValue = document.defaultView
          .getComputedStyle(selection.focusNode.parentElement, null)
          .getPropertyValue('font-size');
      } else {
        fontSizeValue = (this.parent.fontSize as FontSizeModel).width;
      }
      fontSizeValue = isNOU(fontSizeValue)
        ? (this.parent.fontSize as FontSizeModel).width
        : fontSizeValue;
      const actualTxtFontValues: RegExpMatchArray =
        fontSizeValue.match(/^([\d.]+)(\D+)$/);
      const size: number = parseInt(actualTxtFontValues[1], 10);
      const unit: string = actualTxtFontValues[2];
      const defaultFontValues: RegExpMatchArray =
        items[1].value.match(/^([\d.]+)(\D+)$/);
      if (defaultFontValues[2] === unit) {
        const index: number = items.findIndex(
          ({ value }: { value: string }) => parseInt(value, 10) >= size
        );
        activeElem = items[index as number].text;
      } else {
        const convertedSize: number = this.convertFontSize(
          size,
          unit,
          defaultFontValues[2]
        );
        const index: number = items.findIndex(
          ({ value }: { value: string }) => parseInt(value, 10) >= convertedSize
        );
        activeElem = items[index as number].text;
      }
    }
    const fontIndex: number = items.findIndex(
      (size: IDropDownItemModel) =>
        size.text === (activeElem === 'Font Size' ? 'Default' : activeElem)
    );
    if (keyboardArgs.action === 'increase-fontsize' && fontIndex !== -1) {
      if (fontIndex >= items.length - 1) {
        const fontValues: RegExpMatchArray =
          items[fontIndex as number].value.match(/^([\d.]+)(\D+)$/);
        if (fontValues) {
          const size: number = parseInt(fontValues[1], 10);
          const unit: string = fontValues[2];
          const roundedSize: number =
            size % 10 === 0
              ? Math.ceil((size + 1) / 10) * 10
              : Math.ceil(size / 10) * 10;
          (args.item as IDropDownItemModel).value =
            roundedSize.toLocaleString() + unit;
          (args.item as IDropDownItemModel).text =
            roundedSize.toLocaleString() + ' ' + unit;
        }
      ...
      keyboardArgs.action === 'decrease-fontsize' &&
      fontIndex !== -1 &&
      fontIndex > 0
    ) {
      (args.item as IDropDownItemModel).value = items[fontIndex - 1].value;
      ...
      if (
        fontIndex >= 0 &&
        fontIndex < items.length &&
        items[fontIndex as number]
      ) {
        (args.item as IDropDownItemModel).value =
          items[fontIndex as number].value;
        (args.item as IDropDownItemModel).text =
          items[fontIndex as number].text;
      }
    }
  ...
  private convertFontSize(
    ...
    if (
      CONSTANT.supportedUnits.indexOf(originalUnit) !== -1 ||
      CONSTANT.supportedUnits.indexOf(targetUnit) !== -1
    ) {
      originalUnit = 'px';
    }
    const convertedValue: number =
      value *
      CONSTANT.conversionFactors[originalUnit as string][targetUnit as string];
    return convertedValue;
  }

  private onKeyUp(e: NotifyArgs): void {
    const args: KeyboardEvent = e.args as KeyboardEvent;
    const restrictKeys: number[] = [
      8, 9, 13, 17, 18, 20, 27, 37, 38, 39, 40, 44, 45, 46, 91, 112, 113, 114,
    ...
    if (
      range.startContainer === range.endContainer &&
      range.startOffset === range.endOffset &&
      range.startContainer !== this.parent.inputElement &&
      range.startOffset !== 0
    ) {
      const mentionStartNode: Node =
      ...
      isMention =
        ...
        !isNOU(mentionStartNode.previousSibling) &&
        (mentionStartNode.previousSibling as HTMLElement).contentEditable ===
          'false';
    }
    if (this.isCopyAll) {
      return;
    ...
    if (
      restrictKeys.indexOf(args.keyCode) < 0 &&
      !args.shiftKey &&
      !args.ctrlKey &&
      !args.altKey &&
      !isEmptyNode &&
      ...
      range.startContainer.nodeName === '#text'
        ? range.startContainer.parentElement !== this.parent.inputElement
          ? range.startContainer.parentElement.classList.add('currentStartMark')
          : (isRootParent = true)
        : (range.startContainer as Element).classList.add('currentStartMark');
      if (range.startContainer.textContent.charCodeAt(0) === 8203) {
        ...
        pointer =
          previousRange === 0
            ? previousRange
            : previousRange -
              (previousLength - range.startContainer.textContent.length);
      ...
      const currentLength: number = this.parent.inputElement.innerHTML.replace(
        regEx,
        ''
      ).length;
      ...
      if (previousLength > currentLength && !isRootParent) {
        if (
          focusNode.textContent.trim().length !== 0 &&
        ...
        let currentChild: Element = this.parent.inputElement
        ...
        while (!isNOU(currentChild)) {
          ...
          if (
            currentChild.textContent.replace(regEx, '').trim().length > 0 &&
            currentChild.textContent.includes('\u200B')
          ) {
            currentChild.innerHTML = currentChild.innerHTML.replace(regEx, '');
          }
          currentChild = currentChild.nextElementSibling;
        }
        let tempSpanToRemove: Element =
          this.parent.inputElement.querySelector('.tempSpan');
        if (
          tempSpanToRemove &&
          tempSpanToRemove.previousSibling &&
          focusNode.textContent.trim().length !== 0
        ) {
          ...
          const parentElement: HTMLElement | null =
            tempSpanToRemove.parentNode as HTMLElement | null;
          parentElement.removeChild(tempSpanToRemove);
          tempSpanToRemove = null;
        }
        const currentChildNode: NodeListOf<ChildNode> =
          this.parent.inputElement.querySelector(
            '.currentStartMark'
          ).childNodes;
        if (currentChildNode.length > 1) {
          for (let i: number = 0; i < currentChildNode.length; i++) {
            if (
              currentChildNode[i as number].nodeName === '#text' &&
              currentChildNode[i as number].textContent.length === 0
            ) {
              detach(currentChildNode[i as number]);
              i--;
            }
            if (
              !isNOU(currentChildNode[i as number]) &&
              focusNode.textContent.replace(regEx, '') ===
                currentChildNode[i as number].textContent
            ) {
              pointer =
                  ...
                  ? focusNode.textContent ===
                    ...
                    : pointer -
                      (focusNode.textContent.length -
                        focusNode.textContent.replace(regEx, '').length)
                  : focusNode.textContent.length;
          ...
          if (
            focusNode.textContent.replace(regEx, '') ===
            currentChildNode[0].textContent
          ) {
      ...
      const currentElem: Element =
        this.parent.inputElement.querySelector('.currentStartMark');
      ...
      if (
        ...
        !isNOU(range.startContainer.previousSibling.parentElement) &&
        range.startContainer.parentElement ===
          range.startContainer.previousSibling.parentElement &&
        range.startContainer.previousSibling.textContent.charCodeAt(0) ===
          8203 &&
        range.startContainer.previousSibling.textContent.length <= 1
      ) {
        range.startContainer.previousSibling.textContent =
          range.startContainer.previousSibling.textContent.replace(regEx, '');
      }
      if (range.endContainer.textContent.charCodeAt(range.endOffset) === 8203) {
        pointer = range.startOffset;
        range.endContainer.textContent = range.endContainer.textContent.replace(
          regEx,
          ''
        );
        this.parent.formatter.editorManager.nodeSelection.setCursorPoint(
  ...
  private onKeyDown(e: NotifyArgs): void {
    if (
      (e.args as KeyboardEventArgs).ctrlKey &&
      (e.args as KeyboardEventArgs).keyCode === 65
    ) {
      this.isCopyAll = true;
    } else {
      this.isCopyAll = false;
    ...
    const args: KeyboardEvent = e.args as KeyboardEvent;
    if (
      this.parent.inputElement.querySelectorAll('.e-cell-select').length > 1 &&
      (args.keyCode === 8 || args.keyCode === 32 || args.keyCode === 13)
    ) {
    ...
    if (args.keyCode === 9 && this.parent.enableTabKey) {
      ...
      if (
        !isNOU(args.target) &&
        isNullOrUndefined(closest(args.target as Element, '.e-rte-toolbar'))
      ) {
        const range: Range = this.nodeSelectionObj.getRange(
          this.contentRenderer.getDocument()
        );
        const parentNode: Node[] =
          this.nodeSelectionObj.getParentNodeCollection(range);
        if (
          !(
            parentNode[0].nodeName === 'LI' ||
            closest(parentNode[0] as HTMLElement, 'li') ||
            closest(parentNode[0] as HTMLElement, 'table')
          )
        ) {
          args.preventDefault();
          const selection: Range = this.contentRenderer
            .getDocument()
            .getSelection()
            .getRangeAt(0);
          let alignmentNodes: Node[] =
            this.parent.formatter.editorManager.domNode.blockNodes();
          if (this.parent.enterKey === 'BR') {
            if (
              selection.startOffset !== selection.endOffset &&
              selection.startOffset === 0
            ) {
              let save: NodeSelection = this.nodeSelectionObj.save(
                range,
                this.contentRenderer.getDocument()
              );
              ...
              ).domNode.convertToBlockNodes(alignmentNodes, false);
              this.marginTabAdd(args.shiftKey, alignmentNodes);
              save = (
                this.parent.formatter.editorManager as EditorManager
              ).domNode.saveMarker(save);
              save.restore();
            } else {
              InsertHtml.Insert(
                this.contentRenderer.getDocument(),
              ...
              this.rangeCollection.push(
                this.nodeSelectionObj.getRange(
                  this.contentRenderer.getDocument()
                )
              );
            }
            ...
            if (!args.shiftKey) {
              ...
              this.rangeCollection.length > 0 &&
              this.rangeCollection[this.rangeCollection.length - 1]
                .startContainer.textContent.length === 4
            ) {
              const textCont: Node =
                this.rangeCollection[this.rangeCollection.length - 1]
                  .startContainer;
    ...
    if (
      ((e as NotifyArgs).args as KeyboardEventArgs).action === 'space' ||
      ...
      if (this.parent.editorMode === 'HTML' && !this.parent.readonly) {
        const currentLength: number = this.parent
          .getText()
          .trim()
          .replace(/(\r\n|\n|\r|\t)/gm, '')
          .replace(/\u200B/g, '').length;
        ...
        if (
          !(
            this.parent.maxLength === -1 || totalLength <= this.parent.maxLength
          ) &&
          ((e as NotifyArgs).args as KeyboardEventArgs).keyCode === 13
        ) {
    ...
    if (((e as NotifyArgs).args as KeyboardEventArgs).action === 'space') {
      ...
      const editorValue: string = currentRange.startContainer.textContent.slice(
        0,
      ...
      if (
        !hasSplitedText &&
        ((orderedList && !unOrderedList) || (unOrderedList && !orderedList))
      ) {
        ...
        this.parent.trigger(
          ...
          (actionBeginArgs: ActionBeginEventArgs) => {
            if (!actionBeginArgs.cancel) {
              this.parent.formatter.editorManager.observer.notify(
                ON_BEGIN,
                eventArgs
              );
    ...
    if (
      ...
      ((!isNullOrUndefined(this.rangeElement) &&
        !isNullOrUndefined(this.oldRangeElement)) ||
        (!isNullOrUndefined(this.deleteRangeElement) &&
          !isNullOrUndefined(this.deleteOldRangeElement))) &&
      currentRange.startContainer.parentElement.tagName !== 'TD' &&
  ...
  private backSpaceCleanup(e: NotifyArgs, currentRange: Range): void {
    ...
    if (
      !isNOU(currentRange.startContainer.previousSibling) &&
      currentRange.startContainer.previousSibling.nodeName === 'SPAN'
    ) {
      isPreviousNotContentEditable =
        (currentRange.startContainer.previousSibling as HTMLElement)
          .contentEditable === 'false'
          ? false
          : true;
    }
    const checkNode: Node =
      currentRange.startContainer.nodeName === '#text'
        ? currentRange.startContainer.parentElement
        : currentRange.startContainer;
    const isSelectedPositionNotStart: boolean = closest(
      currentRange.startContainer.nodeName === '#text'
        ? currentRange.startContainer.parentElement
        : currentRange.startContainer,
      'li'
    )
      ? checkNode.nodeName !== 'li' && isNOU(checkNode.previousSibling)
      : true;
    if (
      ((e as NotifyArgs).args as KeyboardEventArgs).code === 'Backspace' &&
      ...
      currentRange.startContainer.textContent.length > 0 &&
      currentRange.startContainer.parentElement.tagName !== 'TD' &&
      currentRange.startContainer.parentElement.tagName !== 'TH' &&
      isPreviousNotContentEditable &&
      isSelectedPositionNotStart
    ) {
      if (
        (!this.parent.formatter.editorManager.domNode.isBlockNode(
          checkNode as Element
        ) &&
          !isNOU(checkNode.previousSibling) &&
          checkNode.previousSibling.nodeName === 'BR') ||
        (!isNOU(currentRange.startContainer.previousSibling) &&
          currentRange.startContainer.previousSibling.nodeName === 'BR')
      ) {
        return;
      }
      this.rangeElement = this.getRootBlockNode(
        currentRange.startContainer
      ) as HTMLElement;
      if (
        this.rangeElement.tagName === 'OL' ||
        this.rangeElement.tagName === 'UL'
      ) {
        const liElement: HTMLElement = this.getRangeLiNode(
          currentRange.startContainer
        ) as HTMLElement;
        if (
          ...
          this.oldRangeElement =
            liElement.previousElementSibling.lastElementChild.nodeName === 'BR'
              ? liElement.previousElementSibling
              : (liElement.previousElementSibling.lastChild as HTMLElement);
          if (
            !isNOU(liElement.lastElementChild) &&
            liElement.lastElementChild.nodeName !== 'BR' &&
            isNOU(liElement.lastElementChild.previousSibling) &&
            liElement.lastChild.nodeName !== '#text'
          ) {
        ...
        (!isNOU(this.rangeElement.previousElementSibling) &&
          this.rangeElement.previousElementSibling.tagName === 'TABLE')
      ) {
        return;
      } else {
        this.oldRangeElement = this.rangeElement
          .previousElementSibling as HTMLElement;
      }
      const findBlockElement: Node[] =
        this.parent.formatter.editorManager.domNode.blockNodes();
      if (
        !isNOU(findBlockElement[0]) &&
        currentRange.collapsed &&
        currentRange.startOffset === 0 &&
        currentRange.endOffset === 0 &&
        (findBlockElement[0] as HTMLElement).style.marginLeft !== ''
      ) {
        (findBlockElement[0] as HTMLElement).style.marginLeft =
          parseInt((findBlockElement[0] as HTMLElement).style.marginLeft, 10) <=
          20
            ? ''
            : parseInt(
                (findBlockElement[0] as HTMLElement).style.marginLeft,
      ...
      if (isNOU(this.oldRangeElement)) {
        ...
        if (
          this.oldRangeElement.tagName === 'OL' ||
          this.oldRangeElement.tagName === 'UL'
        ) {
          this.oldRangeElement = this.oldRangeElement.lastElementChild
            .lastElementChild
            ? this.oldRangeElement.lastElementChild.lastElementChild
            : this.oldRangeElement.lastElementChild;
        }
        let lastNode: Node = this.oldRangeElement.lastChild
          ? this.oldRangeElement.lastChild
          : this.oldRangeElement;
        while (
          lastNode.nodeType !== 3 &&
          lastNode.nodeName !== '#text' &&
          lastNode.nodeName !== 'BR' &&
          !isNOU(lastNode.lastChild)
        ) {
        ...
        if (
          !isNOU(this.rangeElement) &&
          this.oldRangeElement !== this.rangeElement
        ) {
          while (this.rangeElement.firstChild) {
            if (this.oldRangeElement.nodeName === '#text') {
              this.oldRangeElement.parentElement.appendChild(
                this.rangeElement.childNodes[0]
              );
            } else {
  ...
  private deleteCleanup(e: NotifyArgs, currentRange: Range): void {
    ...
    if (
      ((e as NotifyArgs).args as KeyboardEventArgs).code === 'Delete' &&
      ((e as NotifyArgs).args as KeyboardEventArgs).keyCode === 46 &&
      ...
      this.deleteRangeElement = rootElement = this.getRootBlockNode(
        currentRange.startContainer
      ) as HTMLElement;
      if (
        this.deleteRangeElement.tagName === 'OL' ||
        this.deleteRangeElement.tagName === 'UL'
      ) {
        liElement = this.getRangeLiNode(
          currentRange.startContainer
        ) as HTMLElement;
        if (
          liElement.nextElementSibling &&
          liElement.nextElementSibling.childElementCount > 0 &&
        ...
        (!isNullOrUndefined(this.deleteRangeElement.nextElementSibling) &&
      ...
      if (
        this.getCaretIndex(currentRange, this.deleteRangeElement) ===
          this.deleteRangeElement.textContent.length &&
        !isImgWithEmptyBlockNode
      ) {
        if (!isNullOrUndefined(liElement)) {
          if (isLiElement || !isNullOrUndefined(liElement.nextElementSibling)) {
            this.deleteOldRangeElement = this.getRangeElement(
              liElement.nextElementSibling
            );
          } else {
          ...
          this.deleteOldRangeElement =
            this.deleteRangeElement.nextElementSibling;
        }
        if (isNullOrUndefined(this.deleteOldRangeElement)) {
          ...
          const brNode: HTMLElement =
            this.deleteRangeElement.querySelector('BR');
          ...
          if (
            ...
            while (this.deleteOldRangeElement.firstChild) {
              this.deleteRangeElement.appendChild(
                this.deleteOldRangeElement.childNodes[0]
              );
            }
            if (!isLiElement) {
  ...
  private getCaretIndex(currentRange: Range, element: Element): number {
    ...
    if (
      this.parent.contentModule.getDocument().getSelection().rangeCount !== 0
    ) {
  ...
  private getRangeElement(element: Element): Element {
    const rangeElement: Element = element.lastElementChild
      ? element.lastElementChild.tagName === 'BR'
        ? element.lastElementChild.previousElementSibling
          ? element.lastElementChild.previousElementSibling
          : element
  ...
  private getRootBlockNode(rangeBlockNode: Node): Node {
    ...
    for (
      ...
      rangeBlockNode &&
      this.parent &&
      this.parent.inputElement !== rangeBlockNode;
      rangeBlockNode = rangeBlockNode
    ) {
  ...
  private onPaste(e: NotifyArgs): void {
    ...
    if (e.text.match(regex)) {
      ...
      // eslint-disable-next-line
      const saveSelection: NodeSelection =
      ...
      const paraElem: NodeListOf<HTMLParagraphElement> =
        divElement.querySelectorAll('span, p');
      for (let i: number = 0; i < paraElem.length; i++) {
        const splitTextContent: string[] =
          paraElem[i as number].innerHTML.split(' ');
        let resultSplitContent: string = '';
        for (let j: number = 0; j < splitTextContent.length; j++) {
          if (
            splitTextContent[j as number].match(httpRegex) ||
            splitTextContent[j as number].match(wwwRegex)
          ) {
            resultSplitContent +=
              '<a class="e-rte-anchor" href="' +
              splitTextContent[j as number] +
              '" title="' +
              splitTextContent[j as number] +
              '" target="_blank"' +
              ' aria-label="' +
              this.parent.serviceLocator
                .getService<L10n>('rteLocale')
      ...
      if (
        !isNullOrUndefined(anchorElement) &&
        !isNullOrUndefined(anchorElement.childNodes[0]) &&
        anchorElement.nodeName === 'SPAN' &&
        anchorElement.childNodes[0].nodeName === 'A'
      ) {
  ...
  private spaceLink(e?: KeyboardEvent): void {
    ...
    const selectNodeEle: Node[] =
      this.nodeSelectionObj.getParentNodeCollection(range);
    const text: string = range.startContainer.textContent.substr(
      0,
      range.endOffset
    );
    const splitText: string[] = text.split(' ');
    let urlText: string = splitText[splitText.length - 1];
    const urlTextRange: number =
      range.startOffset -
    ...
    if (selectNodeEle[0].nodeName !== 'A' && urlText.match(regex)) {
      ...
      const url: string =
        urlText.indexOf('http') > -1 ? urlText : 'http://' + urlText;
  ...
  private onToolbarClick(args: ClickEventArgs): void {
    ...
    const closestElement: Element = closest(
      args.originalEvent.target as Element,
      '.e-rte-quick-popup'
    );
    ...
    if (item.command !== 'FormatPainter') {
      if (
        closestElement &&
        !closestElement.classList.contains('e-rte-inline-popup') &&
        !closestElement.classList.contains('e-rte-text-popup')
      ) {
        if (
          !(
            item.subCommand === 'SourceCode' ||
            item.subCommand === 'Preview' ||
            item.subCommand === 'FontColor' ||
            item.subCommand === 'BackgroundColor'
          )
        ) {
          ...
          save = this.nodeSelectionObj.save(
            range,
            this.parent.contentModule.getDocument()
          );
        ...
        const linkDialog: Element = document.getElementById(
          this.parent.getID() + '_rtelink'
        );
        const imageDialog: Element = document.getElementById(
          this.parent.getID() + '_image'
        );
        if (
          !(
            item.subCommand === 'SourceCode' ||
            ...
            item.subCommand === 'FontColor' ||
            item.subCommand === 'BackgroundColor' ||
            item.subCommand === 'NumberFormatList' ||
            item.subCommand === 'BulletFormatList'
          )
        ) {
          const range: Range = this.nodeSelectionObj.getRange(
            this.parent.contentModule.getDocument()
  ...
  private instantiateRenderer(): void {
    if (this.parent.iframeSettings.enable) {
      this.renderFactory.addRenderer(
        RenderType.Content,
        new IframeContentRender(this.parent)
      );
    } else {
      this.renderFactory.addRenderer(
        RenderType.Content,
        new ContentRender(this.parent)
      );
    }
  }
  ...
  private render(): void {
    ...
    const editElement: HTMLTextAreaElement =
      this.contentRenderer.getEditPanel() as HTMLTextAreaElement;
  ...
  protected onPropertyChanged(e: { [key: string]: RichTextEditorModel }): void {
    ...
    if (
      !isNOU(e) &&
      !isNOU(e.newProp.toolbarSettings) &&
      !isNOU(e.newProp.toolbarSettings.enable)
    ) {
      this.toolbarUpdate = new HtmlToolbarStatus(this.parent);
    }
  ...
  private getSelectedHtml(e: NotifyArgs): void {
    e.callBack(
      this.parent.formatter.editorManager.nodeSelection
        .getRange(this.parent.contentModule.getDocument())
        .toString()
    );
  }
  ...
  private tableSelectionKeyAction(e: NotifyArgs): void {
    ...
    const tableCellSelectNodes: NodeListOf<Element> =
      this.parent.inputElement.querySelectorAll('.e-cell-select');
    for (let i: number = 0; i < tableCellSelectNodes.length; i++) {
      ...
      if (i === 0) {
        ...
        this.nodeSelectionObj.setCursorPoint(
          this.parent.contentModule.getDocument(),
          currentCell.firstChild as Element,
          0
        );
      } else {
        currentCell.innerHTML = '<br>';
  ...
  private marginTabAdd(val: boolean, alignmentNodes: Node[]): void {
    for (let index: number = 0; index < alignmentNodes.length; index++) {
      const element: HTMLElement = alignmentNodes[
        index as number
      ] as HTMLElement;
