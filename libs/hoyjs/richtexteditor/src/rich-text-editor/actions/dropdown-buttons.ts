...
export class DropDownButtons {
        ...
        switch (item) {
          case 'numberformatlist': {
            targetElement = select(
              '#' + this.parent.getID() + '_' + type + '_NumberFormatList',
              tbElement
            );
            if (
              isNullOrUndefined(targetElement) ||
              targetElement.classList.contains(classes.CLS_DROPDOWN_BTN)
            ) {
              return;
            }
            const formatOLItem: IListDropDownModel[] =
              this.parent.numberFormatList.types.slice();
          ...
          case 'bulletformatlist': {
            targetElement = select(
              '#' + this.parent.getID() + '_' + type + '_BulletFormatList',
              tbElement
            );
            if (
              isNullOrUndefined(targetElement) ||
            ...
            const formatULItem: IListDropDownModel[] =
              this.parent.bulletFormatList.types.slice();
          ...
          case 'formats': {
            targetElement = select(
              '#' + this.parent.getID() + '_' + type + '_Formats',
              tbElement
            );
            if (
              isNullOrUndefined(targetElement) ||
            ...
            const formatItem: IDropDownItemModel[] =
              this.parent.format.types.slice();
            formatItem.forEach((item: IDropDownItemModel): void => {
              Object.defineProperties(item as object, {
                command: { value: 'Formats', enumerable: true },
                subCommand: { value: item.value, enumerable: true },
              });
            });
            const formatContent: string = isNullOrUndefined(
            ...
            this.formatDropDown = this.toolbarRenderer.renderDropDownButton({
              ...
              content: this.dropdownContent(
                ...
                type === 'quick'
                  ? ''
                  : getDropDownValue(formatItem, formatContent, 'text', 'text')
              ),
              cssClass:
          ...
          case 'fontname': {
            targetElement = select(
              '#' + this.parent.getID() + '_' + type + '_FontName',
              tbElement
            );
            if (
              isNullOrUndefined(targetElement) ||
            ...
            const fontItem: IDropDownItemModel[] =
              this.parent.fontFamily.items.slice();
            ...
            const fontNameContent: string = isNullOrUndefined(
            ...
            this.fontNameDropDown = this.toolbarRenderer.renderDropDownButton({
              ...
              content: this.dropdownContent(
                ...
                type === 'quick'
                  ...
                  : getDropDownValue(fontItem, fontNameContent, 'text', 'text')
              ),
              cssClass:
                classes.CLS_DROPDOWN_POPUP +
                ' ' +
                classes.CLS_DROPDOWN_ITEMS +
                ' ' +
                classes.CLS_FONT_NAME_TB_BTN,
              itemName: 'FontName',
              items: fontItem,
              element: targetElement,
            } as IDropDownModel);
            if (!isNullOrUndefined(this.parent.fontFamily.default)) {
              this.getEditNode().style.fontFamily =
                this.parent.fontFamily.default;
            }
            break;
          }
          case 'fontsize': {
            targetElement = select(
              '#' + this.parent.getID() + '_' + type + '_FontSize',
              tbElement
            );
            if (
              isNullOrUndefined(targetElement) ||
            ...
            const fontsize: IDropDownItemModel[] =
              !isNullOrUndefined(this.fontSizeDropDown) &&
              !isNullOrUndefined(this.fontSizeDropDown.items) &&
              this.fontSizeDropDown.items.length > 0
                ? this.fontSizeDropDown.items
                : JSON.parse(
                    JSON.stringify(this.parent.fontSize.items.slice())
                  );
            ...
            const fontSizeContent: string = isNullOrUndefined(
              this.parent.fontSize.default
            )
              ? fontsize[0].text
              : this.parent.fontSize.default;
            const fontSizeDropDownContent: string =
              fontSizeContent === 'Default'
                ? getDropDownValue(
                    fontsize,
            ...
            this.fontSizeDropDown = this.toolbarRenderer.renderDropDownButton({
              content: this.dropdownContent(
                ...
                getFormattedFontSize(
                  fontSizeDropDownContent === 'Default'
                    ? this.i10n.getConstant('fontSize')
                    : fontSizeDropDownContent
                )
              ),
              cssClass:
                classes.CLS_DROPDOWN_POPUP +
          ...
          case 'alignments':
            targetElement = select(
              '#' + this.parent.getID() + '_' + type + '_Alignments',
              tbElement
            );
            if (
              isNullOrUndefined(targetElement) ||
          ...
          if (this.fontNameDropDown) {
            for (const fontFamily of Object.keys(newProp.fontFamily)) {
              switch (fontFamily) {
                case 'default':
                case 'width': {
                  const fontItems: IDropDownItemModel[] =
                    this.fontNameDropDown.items;
                  type = !isNullOrUndefined(
                    closest(
                      this.fontNameDropDown.element,
                      '.' + classes.CLS_QUICK_TB
                    )
                  )
                    ? 'quick'
                    : 'toolbar';
                ...
                case 'items':
                  this.fontNameDropDown.setProperties({
                    items: this.getUpdateItems(
                      newProp.fontFamily.items,
                      'FontName'
                    ),
                  });
                  break;
            ...
            for (const fontSize of Object.keys(newProp.fontSize)) {
              switch (fontSize) {
                ...
                case 'width': {
                  ...
                  if (!isNullOrUndefined(this.parent.fontSize.default)) {
                    this.getEditNode().style.fontSize =
                      this.parent.fontSize.default;
                  } else {
          ...
          if (this.formatDropDown) {
            for (const format of Object.keys(newProp.format)) {
              switch (format) {
                case 'default':
                case 'width': {
                  const formatItems: IDropDownItemModel[] =
                    this.formatDropDown.items;
                  type = !isNullOrUndefined(
                    closest(
                      this.formatDropDown.element,
                      '.' + classes.CLS_QUICK_TB
                    )
                  )
                    ? 'quick'
  ...
  private rowDropDown(
    type: string,
    tbElement: HTMLElement,
    targetElement: Element
  ): void {
    targetElement = select(
      '#' + this.parent.getID() + '_' + type + '_TableRows',
      tbElement
    );
  ...
  private columnDropDown(
    ...
    targetElement = select(
      '#' + this.parent.getID() + '_' + type + '_TableColumns',
      tbElement
    );
    if (targetElement.classList.contains(classes.CLS_DROPDOWN_BTN)) {
      return;
  ...
  private cellDropDown(
    ...
    targetElement = select(
      '#' + this.parent.getID() + '_' + type + '_TableCell',
      tbElement
    );
    if (targetElement.classList.contains(classes.CLS_DROPDOWN_BTN)) {
      return;
  ...
  private verticalAlignDropDown(
    ...
    targetElement = select(
      '#' + this.parent.getID() + '_' + type + '_TableCellVerticalAlign',
      tbElement
    );
    if (targetElement.classList.contains(classes.CLS_DROPDOWN_BTN)) {
      return;
    ...
    this.tableCellVerticalAlignDropDown =
      this.toolbarRenderer.renderDropDownButton({
  ...
  private renderDisplayDropDown(
    ...
    this.displayDropDown = this.toolbarRenderer.renderDropDownButton({
      ...
      items:
        item === 'display'
          ? model.imageDisplayItems
          : item === 'videolayoutoption'
          ? model.videoLayoutOptionItems
          : model.audioLayoutOptionItems,
      element: targetElement,
    } as IDropDownModel);
  }
  ...
  private tableStylesDropDown(
    ...
    targetElement = select(
      '#' + this.parent.getID() + '_' + type + '_Styles',
      tbElement
    );
    if (targetElement.classList.contains(classes.CLS_DROPDOWN_BTN)) {
      return;
  ...
  private updateCss(dropDownObj: DropDownButton, e: ICssClassArgs): void {
    if (dropDownObj && e.cssClass) {
      if (isNullOrUndefined(e.oldCssClass)) {
        ...
        dropDownObj.setProperties({
          cssClass: (
            dropDownObj.cssClass.replace(e.oldCssClass, '').trim() +
            ' ' +
            e.cssClass
          ).trim(),
        });
  ...
  private onIframeMouseDown(): void {
    if (
      !isNullOrUndefined(this.parent.getToolbarElement()) &&
      (this.parent
        .getToolbarElement()
        .querySelectorAll('.e-rte-dropdown-btn[aria-expanded="true"]').length >
        0 ||
        this.parent
          .getToolbarElement()
          .querySelectorAll(
            '.e-dropdown-btn.e-rte-inline-dropdown[aria-expanded="true"]'
          ).length > 0)
    ) {
