...
export class FormatPainter implements IFormatPainter {
  ...
  private addEventListener(): void {
    ...
    this.parent.on(
      events.formatPainterDoubleClick,
      this.toolbarDoubleClick,
      this
    );
  ...
  private toolbarClick(clickargs: NotifyArgs): void {
    ...
    if (
      ...
      this.parent.element.ownerDocument.contains(
  ...
  private onKeyDown(event: NotifyArgs): void {
    ...
    if (
      (!isNOU(originalEvent) &&
        !isNOU(originalEvent.action) &&
        (originalEvent.action === 'format-copy' ||
          originalEvent.action === 'format-paste')) ||
      (originalEvent.action === 'escape' &&
        (this.previousAction === 'format-copy' ||
          this.previousAction === 'format-paste'))
    ) {
      if (
        originalEvent.action === 'format-copy' ||
        originalEvent.action === 'format-paste'
  ...
  private actionHandler(event: NotifyArgs, type: string): void {
    ...
    const enable: boolean =
      type === 'docClick' || action === 'escape' ? false : true;
  ...
  private updateCursor(isKeyboard: boolean): void {
    if (
      !this.parent.inputElement.classList.contains('e-rte-cursor-brush') &&
      !isKeyboard
    ) {
  ...
  private updateToolbarBtn(enable: boolean): void {
    if (!isNOU(this.parent.element.querySelector('.e-rte-format-painter'))) {
      const toolbarBtn: HTMLElement = this.parent.element.querySelector(
        '.e-rte-format-painter'
