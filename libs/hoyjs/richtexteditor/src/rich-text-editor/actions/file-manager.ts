...
import { ButtonPropsModel, Dialog, DialogModel } from '@hoyui/popups';
import {
  ContextMenu,
  DetailsView,
  FileManager as EJ2FileManager,
} from '@hoyui/filemanager';
import {
  FileSelectEventArgs,
  NavigationPane,
  Toolbar,
...
export class FileManager {
  ...
  private constructor(parent?: IRichTextEditor, locator?: ServiceLocator) {
    ...
    this.dialogRenderObj =
      locator.getService<DialogRenderer>('dialogRenderObject');
    this.rendererFactory =
      locator.getService<RendererFactory>('rendererFactory');
  ...
  private setCssClass(e: ICssClassArgs) {
    if (this.dialogObj && e.cssClass) {
      if (isNullOrUndefined(e.oldCssClass)) {
        ...
        this.dialogObj.setProperties({
          cssClass: (
            this.dialogObj.cssClass.replace(e.oldCssClass, '').trim() +
            ' ' +
            e.cssClass
          ).trim(),
        });
  ...
  private dialogClosed(e: any): void {
    ...
    this.parent.element.ownerDocument.removeEventListener(
      'mousedown',
      this.onDocumentClickBoundFn
    );
  ...
  private renderFileManager(): void {
    this.fileObj = new EJ2FileManager({
      ...
      navigationPaneSettings:
        this.parent.fileManagerSettings.navigationPaneSettings,
      beforeSend: this.parent.fileManagerSettings.beforeSend,
      fileSelect: (e: FileSelectEventArgs) => {
        const selectedFile: { [key: string]: string } = e.fileDetails as {
          [key: string]: string;
        };
        if (
          selectedFile.isFile &&
          this.parent.insertImageSettings.allowedTypes.indexOf(
            selectedFile.type
          ) > -1
        ) {
          this.inputUrl.value =
            this.parent.fileManagerSettings.ajaxSettings.getImageUrl +
            '?path=' +
            (selectedFile.filterPath &&
              selectedFile.filterPath.replace(/\\/g, '/')) +
            selectedFile.name;
    ...
    this.parent.element.ownerDocument.addEventListener(
      'mousedown',
      this.onDocumentClickBoundFn
    );
  }
  ...
  private getInputUrlElement(): HTMLElement {
    ...
    const urlLabel: HTMLElement = this.parent.createElement('div', {
      className: 'e-rte-label',
    });
    urlLabel.innerHTML =
      '<label for="rteSample_img_url">' +
  ...
  private insertImageUrl(e: MouseEvent): void {
    ...
    if (url !== '') {
      if (
        ...
        isNOU(
          closest(
            this.selectObj.selection.range.startContainer.parentNode,
            '#' + this.contentModule.getPanel().id
          )
        )
      ) {
      ...
      const matchUrl: string =
        !isNOU(url.match(regex)) && this.parent.editorMode === 'HTML'
          ? url.match(regex)[0]
          : '';
      ...
      this.parent.formatter.process(
        this.parent,
        this.selectObj.args,
        (this.selectObj.args as ClickEventArgs).originalEvent,
        value
      );
  ...
  private onDocumentClick(e: MouseEvent): void {
    ...
    const prevEle: Element =
      target.nodeName !== '#document' &&
      !isNOU(target.previousElementSibling) &&
    ...
    if (
      ...
      !closest(
        target,
        '#' + this.parent.getID() + '_rte-file-manager_tb_sortby-popup'
      ) &&
      !closest(
        target,
        '#' + this.parent.getID() + '_rte-file-manager_tb_view-popup'
      ) &&
      !closest(
        target,
        '#' + this.parent.getID() + '_rte-file-manager_contextmenu'
      ) &&
      !(
        !isNOU(closest(target, '.e-contextmenu-wrapper')) &&
        closest(target, '.e-contextmenu-wrapper').querySelector(
          '#' + this.parent.getID() + '_rte-file-manager_contextmenu'
        )
      ) &&
      !isNOU(prevEle) &&
      !prevEle.classList.contains('e-rte-file-manager-dialog') &&
      !isNOU(prevEle) &&
      prevEle.id !== this.parent.getID() + '_rte-file-manager_contextmenu'
    ) {
      this.dialogObj.hide({ returnValue: true } as Event);
