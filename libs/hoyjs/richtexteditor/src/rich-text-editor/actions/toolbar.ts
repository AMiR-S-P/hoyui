...
import { setStyleAttribute, extend } from '@hoyui/base';
import { Toolbar as tool, OverflowMode } from '@hoyui/navigations';
...
export class Toolbar {
  ...
  private toolbarBindEvent(): void {
    if (!this.parent.inlineMode.enable) {
      this.keyBoardModule = new KeyboardEvents(
        this.getToolbarElement() as HTMLElement,
        {
      ...
      case 'enter':
        if (
          ...
          this.parent.notify(events.showColorPicker, {
            toolbarClick: (e.target as Element).classList.contains(
              'e-rte-fontcolor-dropdown'
            )
              ? 'fontcolor'
              : 'backgroundcolor',
  ...
  private getToolbarMode(): OverflowMode {
    ...
    if (
      isIDevice() &&
      this.parent.toolbarSettings.type === ToolbarType.Expand
    ) {
  ...
  private checkIsTransformChild(): void {
    this.isTransformChild = false;
    const transformElements: HTMLElement[] = <HTMLElement[]>(
      selectAll('[style*="transform"]', document)
    );
    for (let i: number = 0; i < transformElements.length; i++) {
      // eslint-disable-next-line max-len
      if (
        !isNullOrUndefined(transformElements[i as number].contains) &&
        transformElements[i as number].contains(this.parent.element)
      ) {
  ...
  private renderToolbar(): void {
    ...
    if (
      this.parent.rootContainer &&
      this.parent.rootContainer.classList.contains('e-source-code-enabled')
    ) {
  ...
  private hideFixedTBar(): void {
    ...
    !this.isToolbar
      ? removeClass(
          [this.tbElement],
          [classes.CLS_SHOW, classes.CLS_TB_IOS_FIX]
        )
      : (this.isToolbar = false);
  }
  ...
  public updateItem(args: IUpdateItemsModel): void {
    const item: IToolsItems =
      this.tools[args.updateItem.toLocaleLowerCase() as ToolbarItems];
    if (
      (getTooltipText(args.updateItem.toLocaleLowerCase(), this.locator) !==
        'Code View' &&
        getTooltipText(args.updateItem.toLocaleLowerCase(), this.locator) !==
          'Preview') ||
      this.parent.locale !== 'en-US'
    ) {
      item.tooltip = getTooltipText(
        args.updateItem.toLocaleLowerCase(),
        this.locator
      );
    }
    const trgItem: IToolsItems =
      this.tools[args.targetItem.toLocaleLowerCase() as ToolbarItems];
    const index: number = getTBarItemsIndex(
      getCollection(trgItem.subCommand),
      args.baseToolbar.toolbarObj.items
    )[0];
    if (!isNOU(index)) {
      const prefixId: string = this.parent.inlineMode.enable
        ? '_quick_'
        : '_toolbar_';
      args.baseToolbar.toolbarObj.items[index as number].id =
        this.parent.getID() + prefixId + item.id;
      args.baseToolbar.toolbarObj.items[index as number].prefixIcon = item.icon;
      args.baseToolbar.toolbarObj.items[index as number].tooltipText =
        item.tooltip;
      (args.baseToolbar.toolbarObj.items as IToolbarItemModel[])[
        index as number
  ...
  private updateToolbarStatus(args: IToolbarStatus): void {
    if (
      !this.tbElement ||
      (this.parent.inlineMode.enable && (isIDevice() || !Browser.isDevice))
    ) {
      return;
    }
    ...
    setToolbarStatus(
      options,
      this.parent.inlineMode.enable ? true : false,
      this.parent
    );
  }
  ...
  public enableTBarItems(
    ...
    if (
      !select('.' + classes.CLS_RTE_SOURCE_CODE_TXTAREA, this.parent.element) &&
      !muteToolbarUpdate
    ) {
      updateUndoRedoStatus(
        baseToolbar,
  ...
  public removeTBarItems(items: string | string[]): void {
    ...
    const trgItems: number[] = getTBarItemsIndex(
      getCollection(items),
      this.baseToolbar.toolbarObj.items
    );
    this.tbItems = this.parent.inlineMode.enable
      ? selectAll(
          '.' + classes.CLS_TB_ITEM,
          this.baseToolbar.toolbarObj.element
        )
      : selectAll('.' + classes.CLS_TB_ITEM, this.parent.element);
    for (let i: number = 0; i < trgItems.length; i++) {
      this.baseToolbar.toolbarObj.removeItems(
        this.tbItems[trgItems[i as number]]
      );
    }
  }
  ...
  public getExpandTBarPopHeight(): number {
    ...
    if (
      this.parent.toolbarSettings.type === ToolbarType.Expand &&
      this.tbElement.classList.contains('e-extended-toolbar')
    ) {
      const expandPopup: HTMLElement = <HTMLElement>(
        select('.e-toolbar-extended', this.tbElement)
      );
      if (
        (expandPopup && this.tbElement.classList.contains('e-expand-open')) ||
  ...
  public getToolbarElement(): Element {
    return this.parent && this.parent.element
      ? select('.' + classes.CLS_TOOLBAR, this.parent.element)
      : null;
  }
  ...
  private isToolbarDestroyed(): boolean {
    return this.baseToolbar &&
      this.baseToolbar.toolbarObj &&
      this.baseToolbar.toolbarObj.isDestroyed
      ? true
      : false;
  }
  ...
  private destroyToolbar(): void {
    if (!this.isToolbarDestroyed()) {
      ...
      if (
        this.parent.emojiPickerModule &&
        !this.parent.emojiPickerModule.isPopupDestroyed
      ) {
      ...
      const tbWrapper: HTMLElement = <HTMLElement>(
        select('.' + classes.CLS_TB_WRAP, this.parent.element)
      );
      const tbElement: HTMLElement = <HTMLElement>(
        select('.' + classes.CLS_TOOLBAR, this.parent.element)
      );
  ...
  private tbFocusHandler(e: Event): void {
    ...
    const isToolbaractive: Element = closest(
      activeElm as Element,
      '.e-rte-toolbar'
    );
    if (
      activeElm === this.parent.getToolbarElement() ||
      isToolbaractive === this.parent.getToolbarElement()
    ) {
      const toolbarItem: NodeList = this.parent
        .getToolbarElement()
        .querySelectorAll('.e-expended-nav');
      for (let i: number = 0; i < toolbarItem.length; i++) {
        if (
          isNOU(
            this.parent.getToolbarElement().querySelector('.e-insert-table-btn')
          )
        ) {
  ...
  protected addEventListener(): void {
    ...
    this.parent.on(
      events.beforeDropDownOpen,
      this.dropDownBeforeOpenHandler,
      this
    );
  ...
  protected removeEventListener(): void {
    ...
    this.parent.off(
      events.enableFullScreen,
      this.parent.fullScreenModule.showFullScreen
    );
    this.parent.off(
      events.disableFullScreen,
      this.parent.fullScreenModule.hideFullScreen
    );
  ...
  private refreshToolbar(): void {
    ...
    const tbWrapper: Element = select(
      '.' + classes.CLS_TB_WRAP,
      this.parent.element
    );
    const tbElement: Element = select(
      '.' + classes.CLS_TOOLBAR,
      this.parent.element
    );
