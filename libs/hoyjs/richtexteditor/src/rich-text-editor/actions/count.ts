...
export class Count {
  ...
  private appendCount(): void {
    const htmlText: string =
      this.parent.editorMode === 'Markdown'
        ? (this.editPanel as HTMLTextAreaElement).value
    ...
    const string: string | number =
      this.parent.maxLength === -1
        ? this.htmlLength
        : this.htmlLength + ' / ' + this.parent.maxLength;
  ...
  public destroy(): void {
    ...
    if (
      ...
      this.parent.rootContainer.classList.contains('e-count-enabled')
    ) {
    ...
    if (
      this.element &&
      !isNullOrUndefined(this.parent.element.querySelector('.' + CLS_COUNT))
    ) {
      detach(this.element);
    }
