...
export class FullScreen {
  ...
  public showFullScreen(event?: MouseEvent | KeyboardEventArgs): void {
    if (
      this.parent.toolbarSettings.enable === true &&
    ...
    if (
      ...
      !isNOU(document.querySelector('.e-tooltip-wrap'))
    ) {
    ...
    if (!this.parent.element.classList.contains(classes.CLS_FULL_SCREEN)) {
      ...
      this.parent.trigger(
        events.actionBegin,
        evenArgs,
        (beginEventArgs: { [key: string]: Object }) => {
          if (!beginEventArgs.cancel) {
            if (
              ...
              (this.parent.getToolbarElement() as HTMLElement).style.width =
                '100%';
              (this.parent.getToolbarElement() as HTMLElement).style.top =
                '0px';
            }
            ...
            if (this.parent.toolbarModule) {
              if (
                !(
                  this.parent.getBaseToolbarObject().toolbarObj.items[0] as {
                    [key: string]: string;
                  }
                ).properties
              ) {
        ...
        (beginEventArgs: { [key: string]: Object }) => {
          if (!beginEventArgs.cancel) {
            ...
            const elem: NodeListOf<Element> =
              document.querySelectorAll('.e-rte-overflow');
