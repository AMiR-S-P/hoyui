...
import { detach, isNullOrUndefined } from '@hoyui/base';
...
export class XhtmlValidation {
   ...
   * @deprecated
   */
  public selfEncloseValidation(
    currentValue: string,
    valueLength?: number
  ): string {
    if (
      valueLength === 0 &&
      currentValue.indexOf('table') < 0 &&
      currentValue.indexOf('img') < 0 &&
    ...
    for (let j: number = 0; j < imgRegexp.length; j++) {
      ...
      for (let i: number = 0; i < valueDupe.length; i++) {
        if (
          valueDupe[i as number].indexOf('/') === -1 ||
          valueDupe[i as number].lastIndexOf('/') !==
            valueDupe[i as number].length - 2
        ) {
          valueDupe[i as number] =
            valueDupe[i as number].substr(
              0,
              valueDupe[i as number].length - 1
            ) +
            ' /' +
            valueDupe[i as number].substr(
              valueDupe[i as number].length - 1,
              valueDupe[i as number].length
            );
        }
      }
      for (let g: number = 0; g <= valueDupe.length - 1; g++) {
        currentValue = currentValue.replace(
          valueOriginal[g as number],
          valueDupe[g as number]
        );
      }
    }
    return currentValue;
  ...
  private ImageTags(): void {
    const imgNodes: NodeListOf<HTMLElement> =
      this.currentElement.querySelectorAll('IMG');
  ...
  private removeTags(): void {
    ...
    for (let i: number = 0; i < removeAttribute.length; i++) {
      this.RemoveElementNode(
        removeAttribute[i as number][0],
        removeAttribute[i as number][1]
      );
    }
  }

  private RemoveElementNode(rmvNode: string, parentNode: string): void {
    const parentArray: NodeListOf<HTMLElement> =
      this.currentElement.querySelectorAll(parentNode);
    for (let i: number = 0; i < parentArray.length; i++) {
      const rmvArray: NodeListOf<HTMLElement> =
        parentArray[i as number].querySelectorAll(rmvNode);
  ...
  private RemoveUnsupported(): void {
    const underlineEle: NodeListOf<HTMLElement> = <NodeListOf<HTMLElement>>(
      this.currentElement.querySelectorAll('u')
    );
    for (let i: number = underlineEle.length - 1; i >= 0; i--) {
      ...
      underlineEle[i as number].parentNode.insertBefore(
        spanEle,
        underlineEle[i as number]
      );
      detach(underlineEle[i as number]);
    }
    const strongEle: NodeListOf<HTMLElement> =
      this.currentElement.querySelectorAll('strong');
    for (let i: number = strongEle.length - 1; i >= 0; i--) {
      ...
      strongEle[i as number].parentNode.insertBefore(
        boldEle,
        strongEle[i as number]
      );
  ...
  private RemoveAttributeByName(attrName: string): void {
    if (this.currentElement.firstChild !== null) {
      if (this.currentElement.firstChild.nodeType !== 3) {
        for (
          let i: number = 0;
          i < this.currentElement.childNodes.length;
          i++
        ) {
          ...
          if (
            ...
            if (ele.hasChildNodes()) {
              for (let j: number = 0; j < ele.childNodes.length; j++) {
                ...
                if (
                  childEle.nodeType !== 3 &&
                  childEle.nodeName !== 'TABLE' &&
                  childEle.nodeName !== 'TBODY' &&
                  childEle.nodeName !== 'THEAD' &&
                  childEle.nodeName !== 'TH' &&
                  childEle.nodeName !== 'TR' &&
                  childEle.nodeName !== 'TD' &&
                  (childEle as HTMLElement).hasAttribute(attrName) &&
                  !(childEle as HTMLElement).classList.contains(
                    'e-mention-chip'
                  )
                ) {
                ...
                if (childEle.hasChildNodes()) {
                  for (let k: number = 0; k < childEle.childNodes.length; k++) {
                    if (
                      childEle.childNodes[k as number].nodeType !== 3 &&
                      childEle.childNodes[k as number].nodeName !== 'TABLE' &&
                      childEle.childNodes[k as number].nodeName !== 'TBODY' &&
                      childEle.childNodes[k as number].nodeName !== 'THEAD' &&
                      childEle.childNodes[k as number].nodeName !== 'TH' &&
                      childEle.childNodes[k as number].nodeName !== 'TR' &&
                      childEle.childNodes[k as number].nodeName !== 'TD' &&
                      (
                        childEle.childNodes[k as number] as HTMLElement
