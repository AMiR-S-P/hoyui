  ...
  implements INotifyPropertyChanged
{
  ...
  public onPropertyChanged(
    newProp: KeyboardEventsModel,
    oldProp?: KeyboardEventsModel
  ): void {
    // No code are needed
  }
  ...
  private keyPressHandler: EventListener = (e: KeyboardEventArgs): void => {
    ...
    for (const key of keys) {
      ...
      for (const rconfig of configCollection) {
        const rKeyObj: KeyData = KeyboardEvents.getKeyConfigData(
          rconfig.trim()
        );
        if (
          isAltKey === rKeyObj.altKey &&
          (isCtrlKey === rKeyObj.ctrlKey || isMetaKey) &&
