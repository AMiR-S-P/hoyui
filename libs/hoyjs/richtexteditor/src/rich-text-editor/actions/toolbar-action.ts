  ...
  private toolbarClick(args: IDropDownClickArgs): void {
    ...
    if (!isNOU((args.item as { [key: string]: object }).controlParent)) {
      ...
      let activeEle: HTMLElement = (
        (args.item as { [key: string]: object }).controlParent as {
          [key: string]: object;
        }
    ...
    if (
      args.item.command === 'NumberFormatList' ||
      args.item.command === 'BulletFormatList'
    ) {
      if (
        (args.originalEvent.target as HTMLElement).classList.contains(
          'e-order-list'
        ) ||
        (args.originalEvent.target as HTMLElement).classList.contains(
          'e-unorder-list'
        ...
        args.item.subCommand =
          args.item.subCommand === 'NumberFormatList' ? 'OL' : 'UL';
      }
    }
    if (args.item.command === 'Lists') {
      if (
        ...
        ).parentElement.classList.contains('e-rte-bulletformatlist-dropdown') ||
          (
            args.originalEvent.target as HTMLElement
          ).parentElement.classList.contains('e-rte-numberformatlist-dropdown'))
      ) {
        return;
      }
    }
    ...
    if (
      !(
        ...
        (e.item.command === 'Images' ||
          ...
          (e.item.command as string) === 'Table')
      )
    ) {
      const value: string =
        ...
        this.parent.quickToolbarModule.tableQTBar.element.contains(
          e.item.controlParent.element
        )
          ? 'Table'
          : null;
      if (e.item.command === 'Lists') {
        const listItem: IAdvanceListItem = {
