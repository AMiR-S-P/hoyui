...
import { ClickEventArgs, Toolbar } from '@hoyui/navigations';
import { ItemModel } from '@hoyui/navigations/src/toolbar/toolbar-model';
...
export class EmojiPicker {
  ...
  public childDestroy(): void {
    ...
    if (this.popupObj && !this.popupObj.isDestroyed) {
      if (
        ...
        this.popupObj.element.querySelector('.e-rte-emoji-search')
      ) {
        const textBox: TextBox = getComponent(
      ...
      if (
        this.popupObj.element &&
        this.popupObj.element.querySelector('.e-rte-emojipicker-toolbar')
      ) {
        const toolbar: Toolbar = getComponent(
          this.popupObj.element.querySelector(
      ...
      const closeIcon: HTMLElement =
        this.popupObj.element.querySelector('.e-clear-icon');
      if (!isNOU(closeIcon)) {
    ...
    if (this.popDiv) {
      ...
      if (this.popDiv.querySelector('.e-rte-emojipicker-btn')) {
        const btn: HTMLElement = this.popDiv.querySelector(
  ...
  private toolbarClick(args: NotifyArgs | any): void {
    ...
    if (
      !this.parent.inputElement.contains(
    ...
    const emojiPicker: EmojiIconsSet[] =
      this.parent.emojiPickerSettings.iconsSet;
    if (this.popupObj) {
    ...
    const extendEle: HTMLElement = this.parent.element.querySelector(
      '.e-toolbar-extended'
    );
    let zIndex: string;
    if (!isNOU(extendEle)) {
      const computedStyle: CSSStyleDeclaration =
        window.getComputedStyle(extendEle);
    ...
    if (
      !isNOU((args as NotifyArgs).args as ClickEventArgs) &&
      !this.parent.inlineMode.enable &&
      isNOU(this.parent.quickToolbarSettings.text)
    ) {
      target = ((args as NotifyArgs).args as ClickEventArgs).originalEvent
    ...
    const emojiToolBar: HTMLElement = this.parent.createElement('div', {
    ...
    for (
      let i: number = 0;
      i < this.parent.emojiPickerSettings.iconsSet.length;
      i++
    ) {
      if (
        !isNOU(this.parent.emojiPickerSettings.iconsSet[i as number].iconCss)
      ) {
        pushToolBar.push({
          prefixIcon:
            this.parent.emojiPickerSettings.iconsSet[i as number].iconCss,
          htmlAttributes: {
            ['title']:
              this.parent.emojiPickerSettings.iconsSet[i as number].name,
          },
        });
      } else {
        pushToolBar.push({
          text:
            '&#x' +
            this.parent.emojiPickerSettings.iconsSet[i as number].code +
            ';',
          htmlAttributes: {
            ['title']:
              this.parent.emojiPickerSettings.iconsSet[i as number].name,
    ...
    // emoji btn div
    const emojiBtnDiv: HTMLElement = this.parent.createElement('div', {
      className: 'e-rte-emojipicker-btn',
    });
    this.popDiv.appendChild(emojiBtnDiv);
    let height: string;
    const popupBorder: CSSStyleDeclaration = window.getComputedStyle(
      this.popDiv
    );
    if (
      (isNOU((args as NotifyArgs).args as ClickEventArgs) &&
      ...
      height =
        this.popDiv.getBoundingClientRect().height -
        emojiToolBar.getBoundingClientRect().height -
        2 * parseFloat(popupBorder.borderWidth) +
        'px';
      emojiBtnDiv.style.setProperty('height', height, 'important');
    } else {
      const inputELe: HTMLElement = this.parent.element.querySelector(
        '.e-rte-emoji-search'
      ).parentElement;
      const getComputedStyle: CSSStyleDeclaration =
        window.getComputedStyle(inputELe);
      height =
        this.popDiv.getBoundingClientRect().height -
        emojiToolBar.getBoundingClientRect().height -
        inputELe.getBoundingClientRect().height -
        parseFloat(getComputedStyle.marginTop) -
        parseFloat(getComputedStyle.marginBottom) -
        2 * parseFloat(popupBorder.borderWidth) +
        'px';
      emojiBtnDiv.style.setProperty('height', height, 'important');
    }
    for (let i: number = 0; i < emojiPicker.length; i++) {
      const emojiGroupDiv: HTMLElement = this.parent.createElement('div', {
        className: 'e-rte-emojipicker-group',
      });
      emojiBtnDiv.appendChild(emojiGroupDiv);
      const emojiName: HTMLElement = this.parent.createElement('div', {
        className: 'e-rte-emojipicker-name',
      });
      emojiName.innerText =
        this.parent.emojiPickerSettings.iconsSet[i as number].name;
      emojiName.setAttribute(
        'aria-label',
        this.parent.emojiPickerSettings.iconsSet[i as number].name
      );
      emojiGroupDiv.appendChild(emojiName);
      const emojiBtn: HTMLElement = this.parent.createElement('div', {
      ...
      for (let j: number = 0; j < emojiPicker[i as number].icons.length; j++) {
        const button: HTMLElement = this.parent.createElement('button', {
          className: 'e-btn ' + ' ' + 'e-control',
        });
        button.innerHTML = this.buttoncode(
          this.parent.emojiPickerSettings.iconsSet[i as number].icons[
            j as number
          ].code
        );
        button.setAttribute(
    ...
    const emojiButtons: NodeListOf<HTMLButtonElement> =
      this.parent.element.querySelectorAll('.e-rte-emojipicker-btn button');
    if (
      isNOU(this.parent.element.querySelector('.e-rte-emoji-search')) &&
      !isNOU((args as NotifyArgs).args as ClickEventArgs)
    ) {
      emojiButtons[0].focus();
      addClass([emojiButtons[0]], 'e-focus');
    }
    const popup: HTMLElement = this.parent.element.querySelector(
      '.e-rte-emojipicker-btn'
    );
    const toolbarName: NodeListOf<HTMLElement> =
      this.parent.element.querySelectorAll('.e-rte-emojipicker-toolbar button');
  ...
  private buttoncode(value: string): string {
    ...
    // eslint-disable-next-line
    const joinedEmoji: string = valueLength
  ...
  private docClick(e: { [key: string]: object }): void {
    ...
    if (
      ...
      !target.classList.contains('e-emoji') &&
  ...
  private scrollEvent(): void {
    ...
    const emojiSet: NodeListOf<HTMLElement> =
      this.parent.element.querySelectorAll('.e-rte-emojipicker-group');
    const toolbarName: NodeListOf<HTMLElement> =
      this.parent.element.querySelectorAll('.e-rte-emojipicker-toolbar button');
    let scrollTop: number;
    if (!isNOU(popup)) {
  ...
  private emojiToolbarClick(e: ClickEventArgs): void {
    ...
    const emojiGroups: NodeListOf<Element> =
      this.parent.element.querySelectorAll('.e-rte-emojipicker-group');
    const emojiButtons: NodeListOf<HTMLButtonElement> =
      this.parent.element.querySelectorAll('.e-rte-emojipicker-btn button');
    const toolbarName: NodeListOf<HTMLElement> =
    ...
    const emojiGroupsheight: HTMLElement = this.parent.element.querySelector(
      '.e-rte-emojipicker-btn'
    );
    ...
    emojiGroups.forEach((group) => {
      ...
      const focusBtn: HTMLElement = group.childNodes[1]
  ...
  private onKeyDown(e: KeyboardEvent): void {
    ...
    const emojiButtons: NodeListOf<HTMLButtonElement> =
      this.parent.element.querySelectorAll('.e-rte-emojipicker-btn button');
    const emojiGroups: NodeListOf<Element> =
      this.parent.element.querySelectorAll('.e-rte-emojipickerbtn-group');
    const searchKeyHandler: HTMLElement = this.parent.element.querySelector(
      '.e-rte-emojisearch-btn button'
    );
    if (e.keyCode === 27) {
      if (this.popupObj) {
        removeClass([this.divElement], 'e-active');
    ...
    if (!isNOU(srcElement)) {
      if (
        srcElement.classList.contains('e-rte-emoji-search') &&
        e.keyCode === 40
      ) {
        for (let i: number = 0; i < emojiButtons.length; i++) {
          if (emojiButtons[i as number].classList.contains('e-focus')) {
          ...
          if (e.keyCode === 40) {
            ...
            if (emojiButtons.length - 4 > i) {
              ...
              for (let j: number = 0; j < emojiGroups.length; j++) {
                for (
                  let k: number = 0;
                  k < emojiGroups[j as number].childNodes.length;
                  k++
                ) {
                  // eslint-disable-next-line
                  const childNodes: any = emojiGroups[j as number].childNodes[
                    k as number
                  ] as HTMLElement;
                  if (childNodes.classList.contains('e-focus') && count !== 1) {
                    const currentIndex: number = k;
                    const lastChild: HTMLElement = emojiGroups[j as number]
                      .lastChild as HTMLElement;
                    const lastRowIndex: number = Math.floor(k % 6);
                    const lastEleLength: number =
                      emojiGroups[j as number].childNodes.length - 1;
                    const lastEleIndex: number = Math.floor(lastEleLength % 6);
                    if (currentIndex !== -1) {
                      const nextRowIndex: number = currentIndex + 6;
                      if (
                        !isNOU(
                          emojiGroups[j as number].childNodes[
                            nextRowIndex as number
                          ]
                        )
                      ) {
                        // next row has six buttons
                        // handle focus change here
                        const firstFocusEle: HTMLElement = emojiButtons[
                          i as number
                        ] as HTMLElement | null;
                        removeClass([firstFocusEle], 'e-focus');
                        const focusEle: HTMLElement = emojiButtons[
                          (focusIndex += 6)
                        ] as HTMLElement | null;
                        ...
                        !lastChild.classList.contains('e-focus') &&
                        lastEleIndex < lastRowIndex
                      ) {
                        const firstFocusEle: HTMLElement = emojiButtons[
                          i as number
                        ] as HTMLElement | null;
                        removeClass([firstFocusEle], 'e-focus');
                        const focusEle: HTMLElement = emojiGroups[j as number]
                        ...
                        removeClass([firstFocusEle], 'e-focus');
                        const focusEle: HTMLElement = emojiGroups[
                          (j as number) + 1
                        ].childNodes[
                          lastRowIndex as number
                        ] as HTMLElement | null;
                        addClass([focusEle], 'e-focus');
                        focusEle.focus();
                        count = 1;
            ...
            if (i >= 6) {
              ...
              for (let j: number = 0; j < emojiGroups.length; j++) {
                for (
                  ...
                  const childNodes: HTMLElement = emojiGroups[j as number]
                    .childNodes[k as number] as HTMLElement;
                  if (childNodes.classList.contains('e-focus') && count !== 1) {
                    // eslint-disable-next-line
                    const currentIndex: any = k;
                    const previousRowLength: number = isNOU(emojiGroups[j - 1])
                      ? null
                    ...
                    if (currentIndex !== -1) {
                      ...
                      if (
                        ...
                        isNOU(
                          emojiGroups[j - 1].childNodes[
                            emojiGroups[j - 1].childNodes.length -
                              (previousRowLength - k)
                          ]
                        )
                      ) {
                        ...
                        removeClass([firstFocusEle], 'e-focus');
                        const focusEle: HTMLElement = emojiGroups[j - 1]
                          .lastChild as HTMLElement | null;
                        addClass([focusEle], 'e-focus');
                        focusEle.focus();
                        count = 1;
              ...
              const inputELe: HTMLElement = this.popupObj.element.querySelector(
                '.e-rte-emoji-search'
              );
    ...
    if (e.keyCode === 40) {
      const firstFocusEle: HTMLElement =
        this.parent.element.querySelector('.e-focus');
  ...
  private filterKeyHandler(e?: KeyboardEvent): void {
    const emojiButtons: NodeListOf<HTMLButtonElement> =
      this.parent.element.querySelectorAll('.e-rte-emojisearch-btn button');
    const firstFocusEle: HTMLElement =
      this.parent.element.querySelector('.e-focus');
    if (isNOU(firstFocusEle) && e.keyCode === 40) {
      ...
      for (let i: number = 0; i < emojiButtons.length; i++) {
        let focusIndex: number = i;
        const childNodes: HTMLElement = emojiButtons[
          i as number
        ] as HTMLElement;
  ...
  private searchFilter(e?: KeyboardEvent, value?: string): void {
    const inputElement: HTMLInputElement = this.parent.element.querySelector(
      '.e-rte-emoji-search'
    ) as HTMLInputElement;
    const contentELe: HTMLElement =
      this.parent.element.querySelector('.e-content');
    if (
      document.activeElement === inputElement ||
      document.activeElement === contentELe
    ) {
      ...
      const inputValue: string = isNOU(inputElement)
        ? trimmedStr
        : inputElement.value;
      const emojiButtons: NodeListOf<HTMLButtonElement> =
        this.parent.element.querySelectorAll('.e-rte-emojipicker-btn button');
      const emojipickerAll: HTMLElement = this.parent.element.querySelector(
        '.e-rte-emojipicker-btn'
      );
      const emojiGroups: NodeListOf<HTMLElement> =
        this.parent.element.querySelectorAll('.e-rte-emojipicker-group');
      const toolbarGroup: HTMLElement = this.parent.element.querySelector(
        '.e-rte-emojipicker-toolbar'
      ) as HTMLElement;
      const excludedDiv: HTMLElement = this.parent.element.querySelector(
      ...
      const filteredButtons = isNOU(excludedDiv)
        ? emojiButtons
        : // eslint-disable-next-line @typescript-eslint/tslint/config
          Array.from(emojiButtons).filter(
            (button) => !excludedDiv.contains(button)
          );
      if (inputValue === '' || value === ':') {
        toolbarGroup.style.display = '';
        emojipickerAll.style.height =
          this.popDiv.getBoundingClientRect().height -
          toolbarGroup.getBoundingClientRect().height -
          inputHeight -
          2 * parseFloat(popupBorder.borderWidth) +
          'px';
        // eslint-disable-next-line @typescript-eslint/tslint/config
      ...
      const emojis: HTMLElement = this.parent.element.querySelector(
        '.e-rte-emojisearch-btn'
      ) as HTMLElement;
      if (emojis) {
        emojis.remove();
      }
      const noEmojiObj: HTMLElement = this.parent.createElement('div', {
      ...
      for (let i: number = 0; i < filteredButtons.length; i++) {
        if (!isNOU(filteredButtons[i as number].getAttribute('title'))) {
          const title: string = filteredButtons[i as number]
            .getAttribute('title')
            .toLowerCase();
          const titleLength: string[] = title.split(' ');
          for (let j: number = 0; j < titleLength.length; j++) {
            if (
              titleLength[j as number].startsWith(inputValue.toLowerCase()) &&
              inputValue !== ''
            ) {
      ...
      if (
        noEMoji &&
        !this.parent.element.querySelector('.e-rte-emojiSearch-noEmoji') &&
        inputValue !== '' &&
        value !== ':' &&
        value !== ': :'
      ) {
        noEmojiObj.innerHTML =
          '<span style="color: rgba(0, 0, 0, 0.75); font-weight: 500; font-size: 16px;">' +
          this.i10n.getConstant('emojiPickerNoResultFound') +
          ' 😥 </span>' +
          '<br>' +
  ...
  private emojiBtnClick(e: Event | KeyboardEvent): void {
    const event: MouseEvent = new MouseEvent('mouseleave', {
      bubbles: true,
      cancelable: true,
    });
    // Includes the emote button element tooltip and toolbar tooltip
    const emotePickerTooltips: NodeList = this.parent.element.querySelectorAll(
      '.e-rte-emojipicker-popup [data-tooltip-id]'
    );
    ...
    const previousText: string = textContent.substring(
      startOffset,
      startOffset + 1
    );
    ...
    const originalEvent: MouseEvent | KeyboardEvent | PointerEvent = e as
      | MouseEvent
      | KeyboardEvent
      | PointerEvent;
  ...
  private onkeyPress(e: NotifyArgs): void {
    ...
    const selection: Selection = this.parent.iframeSettings.enable
      ? (
          this.parent.contentModule.getPanel() as HTMLIFrameElement
    ...
    const prevChar: string = selection.focusNode.textContent.substring(
      cursorPos - 1,
      cursorPos
    );
    const isPrevSpace: boolean = /\s/.test(prevChar);
    const isPrevColon: boolean =
      selection.focusNode.textContent.substring(cursorPos - 1, cursorPos) ===
      ':';
    const colon: boolean =
      selection.focusNode.textContent.charAt(cursorPos - 1) === ':';
    if (
      originalEvent.keyCode === 186 &&
      originalEvent.shiftKey &&
      (isPrevSpace || selection.focusOffset === 0)
    ) {
      const coordinates: { [key: string]: number } = this.getCoordinates();
    ...
    if (originalEvent.keyCode === 32 && isPrevColon && this.popupObj) {
      removeClass([this.divElement], 'e-active');
      const currentDocument: Document = this.parent.iframeSettings.enable
        ? this.parent.contentModule.getPanel().ownerDocument
        : this.parent.contentModule.getDocument();
      if (
        this.parent.showTooltip &&
        !isNOU(currentDocument.querySelector('.e-tooltip-wrap'))
      ) {
    ...
    if (
      this.popupObj &&
      (originalEvent.keyCode === 37 ||
        originalEvent.keyCode === 38 ||
        originalEvent.keyCode === 39 ||
        originalEvent.keyCode === 27 ||
  ...
  private onkeyUp(e: NotifyArgs): void {
    ...
    for (let i: number = cursorPos - 1; i >= cursorPos - 15; i--) {
      ...
      if (isPrevSpace && !isNOU(this.popDiv) && count === 0) {
        selectedValue = range.startContainer.textContent.substring(
          i - 1,
          cursorPos
        );
    ...
    const colon: string = selection.focusNode.textContent.substring(
      cursorPos - 1,
      cursorPos
    );
  ...
  private getCoordinates(): { [key: string]: number } {
    ...
    // eslint-disable-next-line
    const rect: any = isNOU(firstChild)
      ? range.getBoundingClientRect()
      : firstChild.getBoundingClientRect();
    const rteContent: HTMLElement =
      this.parent.element.querySelector('.e-rte-content');
    const rteEle: HTMLElement = this.parent.element;
    const toolbarHeight: number = rteEle.offsetHeight - rteContent.offsetHeight;
    const cursorTop: number =
      rect.top - this.parent.inputElement.getBoundingClientRect().top;
    const cursorLeft: number =
      rect.left - this.parent.inputElement.getBoundingClientRect().left;
    const screenHeight: number = window.innerHeight;
    const popupHeight: number = 330;
    let popupTop: number = cursorTop;
    const popupLeft: number = cursorLeft + rect.width;
    if (
      rteEle.getBoundingClientRect().top < 0 &&
      !this.parent.inlineMode.enable
    ) {
      popupTop =
        popupTop + rteContent.getBoundingClientRect().top - toolbarHeight;
    }
