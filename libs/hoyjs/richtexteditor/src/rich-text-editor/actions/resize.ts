import { EventHandler, Browser, isNullOrUndefined, detach } from '@hoyui/base';
...
export class Resize {
  ...
  private renderResizable(): void {
    const enableRtlClass: string = this.parent.enableRtl
      ? classes.CLS_RTE_RES_WEST
      : classes.CLS_RTE_RES_EAST;
    this.resizer = this.parent.createElement('div', {
      id: this.parent.getID() + '-resizable',
      className:
        'e-icons' + ' ' + classes.CLS_RTE_RES_HANDLE + ' ' + enableRtlClass,
    });
  ...
  private performResize(e?: MouseEvent | TouchEvent | PointerEvent): void {
    ...
    if (this.isMouseEvent(e)) {
      this.parent.element.style.height =
        (<MouseEvent>e).clientY - boundRect.top + 'px';
      this.parent.element.style.width = !this.parent.enableRtl
        ? (<MouseEvent>e).clientX - boundRect.left + 'px'
        : boundRect.right - (<MouseEvent>e).clientX + 'px';
      const toolBarEle: HTMLElement =
        this.parent.toolbarModule.getToolbarElement() as HTMLElement;
      if (
        !isNullOrUndefined(toolBarEle) &&
        !isNullOrUndefined(toolBarEle.parentElement)
      ) {
        if (
          toolBarEle.parentElement.classList.contains(classes.CLS_TB_FLOAT) &&
          this.parent.toolbarSettings.enableFloating &&
          this.parent.getToolbar() &&
          !this.parent.inlineMode.enable
        ) {
          const contentPanel: HTMLElement =
            this.parent.contentModule.getPanel() as HTMLElement;
          const contentPanelWidth: number =
            contentPanel.getBoundingClientRect().width;
          toolBarEle.style.width = contentPanelWidth + 'px';
        }
      }
    } else {
      const eventType: MouseEvent | Touch =
        Browser.info.name !== 'msie'
          ? (<TouchEvent>e).touches[0]
          : <MouseEvent>e;
      this.parent.element.style.height =
        eventType.clientY - boundRect.top + 'px';
      this.parent.element.style.width = !this.parent.enableRtl
        ? eventType.clientX - boundRect.left + 'px'
        : boundRect.right - eventType.clientX + 'px';
    }
  ...
  private isMouseEvent(e: MouseEvent | TouchEvent | PointerEvent): boolean {
    ...
    if (
      this.getEventType(e.type) === 'mouse' ||
      (!isNullOrUndefined((<PointerEvent>e).pointerType) &&
  ...
  private removeEventListener(): void {
    ...
    if (
      ...
      this.parent.rootContainer.classList.contains('e-resize-enabled')
    ) {
      this.parent.rootContainer.classList.remove('e-resize-enabled');
    }
    if (
      this.parent.iframeSettings.enable &&
      !isNullOrUndefined(this.parent.inputElement)
    ) {
