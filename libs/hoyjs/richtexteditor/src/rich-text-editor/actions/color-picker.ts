import { select, detach, extend, isNullOrUndefined } from '@hoyui/base';
import { ColorPicker } from '@hoyui/inputs';
...
export class ColorPickerInput {
    ...
    templateItems.forEach((item: string) => {
      if (getIndex(item, args.items) !== -1) {
        switch (item) {
          case 'fontcolor': {
            targetID =
              this.parent.getID() + '_' + suffixID + '_FontColor_Target';
            const fontNode: HTMLInputElement = this.parent.createElement(
              'input'
            ) as HTMLInputElement;
            ...
            options = {
              ...
              command:
                this.tools[item.toLocaleLowerCase() as ToolbarItems].command,
              subCommand:
                this.tools[item.toLocaleLowerCase() as ToolbarItems].subCommand,
              element: select(
                '#' + this.parent.getID() + '_' + suffixID + '_FontColor',
                tbElement
              ),
              target: targetID,
            } as IColorPickerModel;
            this.fontColorPicker = this.toolbarRenderer.renderColorPicker(
              options,
              'fontcolor'
            );
            this.fontColorDropDown =
              this.toolbarRenderer.renderColorPickerDropDown(
                options,
                'fontcolor',
          ...
          case 'backgroundcolor': {
            targetID =
              this.parent.getID() + '_' + suffixID + '_BackgroundColor_Target';
            const backNode: HTMLInputElement = this.parent.createElement(
              'input'
            ) as HTMLInputElement;
            ...
            options = {
              ...
              subCommand:
                this.tools[item.toLocaleLowerCase() as ToolbarItems].subCommand,
              element: select(
                '#' + this.parent.getID() + '_' + suffixID + '_BackgroundColor',
                tbElement
              ),
              target: targetID,
            } as IColorPickerModel;
            this.backgroundColorPicker = this.toolbarRenderer.renderColorPicker(
              options,
              'backgroundcolor'
            );
  ...
  public destroyColorPicker(): void {
    ...
    if (this.fontColorDropDown && !this.fontColorDropDown.isDestroyed) {
      const innerEle: HTMLElement =
        this.fontColorDropDown.element.querySelector(
          '.e-rte-color-content'
        ) as HTMLElement;
      if (innerEle) {
        detach(innerEle);
    ...
    if (
      this.backgroundColorDropDown &&
      !this.backgroundColorDropDown.isDestroyed
    ) {
      const innerEle: HTMLElement =
        this.backgroundColorDropDown.element.querySelector(
          '.e-rte-color-content'
        ) as HTMLElement;
      if (innerEle) {
        this.defaultColorPicker = (
          innerEle.children[0] as HTMLElement
  ...
  protected addEventListener(): void {
    this.parent.on(
      events.toolbarRenderComplete,
      this.renderColorPickerInput,
      this
    );
  ...
  private showColorPicker(e: { [key: string]: Object }): void {
    if (
      !isNullOrUndefined(this.fontColorPicker) &&
      (e as { [key: string]: Object }).toolbarClick === 'fontcolor'
    ) {
      this.fontColorDropDown.toggle();
    } else if (
      !isNullOrUndefined(this.backgroundColorPicker) &&
      (e as { [key: string]: Object }).toolbarClick === 'backgroundcolor'
    ) {
            ...
            for (const font of Object.keys(newProp.fontColor)) {
              switch (font) {
                case 'default': {
                  ...
                  const fontBorder: HTMLElement = element.querySelector(
                    '.' + this.tools['fontcolor' as ToolbarItems].icon
                  );
                  fontBorder.style.borderBottomColor =
                    newProp.fontColor.default;
                  break;
                }
                case 'mode':
                  this.fontColorPicker.setProperties({
            ...
            for (const background of Object.keys(newProp.backgroundColor)) {
              switch (background) {
                case 'default': {
                  ...
                  backgroundBorder.style.borderBottomColor =
                    newProp.backgroundColor.default;
                  break;
                }
                case 'mode':
                  this.backgroundColorPicker.setProperties({
