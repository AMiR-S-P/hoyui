...
import { Popup, isCollide, Tooltip, TooltipEventArgs } from '@hoyui/popups';
import { OverflowMode } from '@hoyui/navigations';
...
import {
  IToolbarItems,
  IRenderer,
  IQuickToolbarOptions,
  IShowQuickTBarOptions,
...
export class BaseQuickToolbar implements IBaseQuickToolbar {
  ...
  private setPosition(e: IShowQuickTBarOptions): void {
    let x: number;
    let y: number;
    const imgWrapper: HTMLElement = <HTMLElement>(
      closest(e.target, '.e-img-caption')
    );
    const isAligned: boolean =
      e.target.classList.contains('e-imginline') ||
      e.target.classList.contains('e-imgbreak')
        ? false
        : true;
    ...
    if (
      ...
      !target.classList.contains('e-multi-cells-select') &&
      !target.classList.contains('e-rte-image')
    ) {
      ...
      targetOffsetTop = target.classList.contains('e-rte-audio')
    ...
    if (targetOffsetTop - e.editTop > e.popHeight) {
      y =
        parentOffsetTop +
        e.tBarElementHeight +
        (targetOffsetTop - e.editTop) -
        e.popHeight -
        5;
    } else if (
      e.editTop + e.editHeight - (targetOffsetTop + target.offsetHeight) >
      e.popHeight
    ) {
      y =
        parentOffsetTop +
    ...
    if (
      ...
      targetOffsetLeft = target.classList.contains('e-rte-audio')
    ...
    this.popupObj.position.X =
      x + e.popWidth > e.parentData.right ? e.parentData.right - e.popWidth : x;
      ...
      switch (collision[i as number]) {
        ...
        case 'bottom': {
          ...
          if (viewPort === 'document') {
            if (type === 'inline' || type === 'text') {
              ...
              if (
                e.windowHeight - (parentTop + e.tBarElementHeight) >
                e.popHeight
              ) {
                if (contentTop - e.windowHeight > e.popHeight) {
                  posY =
                    contentTop + (e.windowHeight - parentTop) - e.popHeight;
                } else {
                  posY = contentTop;
                }
              } else {
            ...
            if (e.target.tagName !== 'IMG') {
              posY =
                e.parentData.bottom + window.pageYOffset - e.popHeight - 10;
            } else {
        ...
        case 'right':
          if (type === 'inline' || type === 'text') {
            x =
              window.pageXOffset +
              (e.windowWidth - (e.popWidth + e.bodyRightSpace + 10));
          } else {
            x = e.x - e.popWidth;
          }
          break;
  ...
  public showPopup(x: number, y: number, target: Element, type?: string): void {
    const eventArgs: BeforeQuickToolbarOpenArgs = {
      popup: this.popupObj,
      cancel: false,
    ...
    this.parent.trigger(
      events.beforeQuickToolbarOpen,
      eventArgs,
      (beforeQuickToolbarArgs: BeforeQuickToolbarOpenArgs) => {
        if (!beforeQuickToolbarArgs.cancel) {
          ...
          const bodyRight: number =
            parseFloat(bodyStyle.marginRight.split('px')[0]) +
            parseFloat(bodyStyle.paddingRight.split('px')[0]);
          const windowHeight: number = window.innerHeight;
          const windowWidth: number = window.innerWidth;
          const parent: HTMLElement = this.parent.element;
          const toolbarAvail: boolean = !isNullOrUndefined(
            this.parent.getToolbar()
          );
          const tbHeight: number =
            toolbarAvail && this.parent.toolbarModule.getToolbarHeight();
          const expTBHeight: number =
            toolbarAvail && this.parent.toolbarModule.getExpandTBarPopHeight();
          ...
          if (this.parent.iframeSettings.enable) {
            const cntEle: Window = (<HTMLIFrameElement>(
              this.contentRenderer.getPanel()
            )).contentWindow;
            editPanelTop = cntEle.pageYOffset;
            editPanelHeight = cntEle.innerHeight;
          } else {
            const cntEle: HTMLElement = <HTMLElement>(
              closest(target, '.' + classes.CLS_RTE_CONTENT)
            );
          ...
          if (
            ...
            (x === beforeQuickToolbarArgs.positionX ||
              y === beforeQuickToolbarArgs.positionY)
          ) {
          ...
          this.popupObj.show(
            { name: 'ZoomIn', duration: Browser.isIE ? 250 : 400 },
  ...
  public hidePopup(): void {
    const isSourceCodeEnabled: boolean =
      !isNOU(this.parent.rootContainer) &&
    ...
    if (!isNOU(this.element.querySelectorAll('[data-title]'))) {
      const removeHandEle: NodeListOf<Element> =
        this.element.querySelectorAll('[data-title]');
    ...
    if (!isNullOrUndefined(document.querySelector('.e-tooltip-wrap'))) {
      if (
        !isNullOrUndefined(
          document.querySelector('#' + this.element.id + ' [data-tooltip-id]')
        )
      ) {
        const tooltipTargetEle: HTMLElement = <HTMLElement>(
          document.querySelector('#' + this.element.id + ' [data-tooltip-id]')
        );
        const dataContent: string =
          tooltipTargetEle.getAttribute('data-content');
    ...
    if (
      !isNullOrUndefined(this.parent.getToolbar()) &&
      !this.parent.inlineMode.enable
    ) {
  ...
  public addQTBarItem(item: (string | IToolbarItems)[], index: number): void {
    this.quickTBarObj.toolbarObj.addItems(
      this.quickTBarObj.getItems(item, 'toolbar') as IToolbarItemModel[],
      index
    );
  }
  /**
  ...
  private updateStatus(args: IToolbarStatus): void {
    ...
    if (
      ...
      const options: ISetToolbarStatusArgs = {
        args: args,
        dropDownModule:
          this.parent.quickToolbarModule.textQTBar.dropDownButtons,
        parent: this.parent,
        tbElements: selectAll(
          '.' + classes.CLS_TB_ITEM,
          this.parent.quickToolbarModule.textQTBar.element
        ),
        tbItems:
          this.parent.quickToolbarModule.textQTBar.quickTBarObj.toolbarObj
            .items,
      };
      setToolbarStatus(options, true, this.parent);
      updateUndoRedoStatus(
    ...
    if (
      ...
      updateUndoRedoStatus(
        this.parent.getBaseToolbarObject(),
        this.parent.formatter.editorManager.undoRedoManager.getUndoStatus()
      );
    }
  }
