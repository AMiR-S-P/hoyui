...
import { Dialog, DialogModel, Popup } from '@hoyui/popups';
...
import { getUniqueID, Browser, closest } from '@hoyui/base';
import {
...
export class PasteCleanup {
  ...
  private pasteClean(e?: NotifyArgs): void {
    ...
    if (e.args && value !== null && this.parent.editorMode === 'HTML') {
      ...
      if (value.length === 0) {
        ...
        file =
          e &&
          (e.args as ClipboardEvent).clipboardData &&
          (e.args as ClipboardEvent).clipboardData.items.length > 0
            ? (e.args as ClipboardEvent).clipboardData.items[0].getAsFile() ===
              null
              ? !isNOU((e.args as ClipboardEvent).clipboardData.items[1])
                ? (e.args as ClipboardEvent).clipboardData.items[1].getAsFile()
                : null
              : (e.args as ClipboardEvent).clipboardData.items[0].getAsFile()
            : null;
        ...
        this.parent.formatter.editorManager.observer.notify(
          EVENTS.MS_WORD_CLEANUP,
          {
            args: e.args,
            text: e.text,
            allowedStylePropertiesArray:
              this.parent.pasteCleanupSettings.allowedStyleProps,
            callBack: (
              a: string,
              cropImageData: { [key: string]: string | boolean | number }[],
      ...
      const tempDivElem: HTMLElement = this.parent.createElement(
        'div'
      ) as HTMLElement;
      tempDivElem.innerHTML = value;
      const unsupportedImg: NodeListOf<HTMLImageElement> =
        tempDivElem.querySelectorAll('.e-rte-image-unsupported');
      for (let index: number = 0; index < unsupportedImg.length; index++) {
        unsupportedImg[index as number].setAttribute(
          'alt',
          this.i10n.getConstant('unsupportedImage')
        );
        unsupportedImg[index as number].classList.remove(
          'e-rte-image-unsupported'
        );
      }
      value = tempDivElem.innerHTML;
      const isValueNotEmpty: boolean =
        tempDivElem.textContent !== '' ||
  ...
  private splitBreakLine(value: string): string {
    ...
    for (let i: number = 0; i < enterSplitText.length; i++) {
      if (enterSplitText[i as number].trim() === '') {
        ...
        const contentWithSpace: string = this.makeSpace(
          enterSplitText[i as number]
        );
        contentInnerElem +=
          (i === 0 && this.parent.enterKey !== 'BR' ? '<span>' : startNode) +
          (contentWithSpace.trim() === '' ? '<br>' : contentWithSpace.trim()) +
          (enterSplitText.length - 1 === i && this.parent.enterKey === 'BR'
            ? ''
            : i === 0 && this.parent.enterKey !== 'BR'
            ? '</span>'
            : endNode);
      }
    }
  ...
  private imgUploading(elm: HTMLElement): void {
    ...
    if (this.parent.insertImageSettings.saveUrl && allImgElm.length > 0) {
      ...
      for (let i: number = 0; i < allImgElm.length; i++) {
        if (
          !isNOU(allImgElm[i as number].getAttribute('src')) &&
          allImgElm[i as number]
            .getAttribute('src')
            .split(',')[0]
            .indexOf('base64') >= 0
        ) {
      ...
      for (let i: number = 0; i < base64Src.length; i++) {
        fileList.push(
          this.base64ToFile(base64Src[i as number], imgName[i as number])
        );
      }
    ...
    const allImgElmId: NodeListOf<Element> =
      elm.querySelectorAll('.pasteContent_Img');
  ...
  private getBlob(allImgElm: NodeListOf<HTMLImageElement>): void {
    for (let i: number = 0; i < allImgElm.length; i++) {
      if (
        ...
        const blopUrl: string = URL.createObjectURL(
          convertToBlob(allImgElm[i as number].getAttribute('src'))
        );
  ...
  private uploadMethod(file: File, imgElem: Element): void {
    ...
    const contentEle: HTMLInputElement | HTMLElement =
    ...
    (
      this.popupObj.element.getElementsByClassName(
        'e-file-select-wrap'
      )[0] as HTMLElement
    ).style.display = 'none';
    detach(
  ...
  private popupClose(
    ...
    if (element.statusCode === '2') {
      ...
      this.parent.trigger(events.imageRemoving, e, (e: object) => {
        if (
          !isNullOrUndefined((e as { element: HTMLImageElement }).element.src)
        ) {
  ...
  private base64ToFile(base64: string, filename: string): File {
    ...
    if (Browser.isIE || navigator.appVersion.indexOf('Edge') > -1) {
      ...
      return new File(
        [decodeArr],
        filename + '.' + (!isNOU(extension) ? extension : ''),
        { type: extension }
      );
    }
  }
  ...
  private radioRender(): void {
    ...
    const keepFormatElement: HTMLElement =
      this.parent.element.querySelector('#keepFormating');
    ...
    const cleanFormatElement: HTMLElement =
      this.parent.element.querySelector('#cleanFormat');
    ...
    const plainTextElement: HTMLElement =
      this.parent.element.querySelector('#plainTextFormat');
  ...
  private selectFormatting(
    ...
    isClipboardHTMLDataNull: boolean
  ): void {
    if (keepChecked) {
      this.formatting(value, false, args);
  ...
  private pasteDialog(
    ...
    const dialogModel: DialogModel = {
      buttons: [
        {
          click: () => {
            if (!this.dialogObj.isDestroyed) {
              ...
              this.selectFormatting(
                value,
                args,
                keepChecked,
                cleanChecked,
                isClipboardHTMLDataNull
              );
            }
          },
          buttonModel: {
      ...
      content:
        ...
        '<br/><div><div style="padding-top:24px;">' +
    ...
    let rteDialogWrapper: HTMLElement = this.parent.element.querySelector(
      '#' + this.parent.getID() + '_pasteCleanupDialog'
    );
  ...
  private updateCss(
    currentObj: Dialog | Uploader | RadioButton,
    e: ICssClassArgs
  ): void {
  ...
  private destroyDialog(rteDialogWrapper: HTMLElement): void {
    const rteDialogContainer: HTMLElement = this.parent.element.querySelector(
      '.e-rte-dialog-minheight'
    );
    detach(rteDialogContainer);
    const rteDialogWrapperChildLength: number =
      rteDialogWrapper.children.length;
  ...
  private cleanAppleClass(elem: HTMLElement): HTMLElement {
    const appleClassElem: NodeListOf<Element> = elem.querySelectorAll(
  ...
  private formatting(value: string, clean: boolean, args: Object): void {
    let clipBoardElem: HTMLElement = this.parent.createElement('div', {
    ...
    const allImg: NodeListOf<HTMLImageElement> =
      clipBoardElem.querySelectorAll('img');
    ...
    if (
      clipBoardElem.textContent !== '' ||
      !isNOU(clipBoardElem.querySelector('img')) ||
      !isNOU(clipBoardElem.querySelector('table'))
    ) {
      const tempWrapperElem: HTMLElement = this.parent.createElement(
        'div'
      ) as HTMLElement;
      tempWrapperElem.innerHTML = value;
      const filesData: FileInfo[] = [];
      if (!isNOU(tempWrapperElem.querySelector('img'))) {
        const imgElem: NodeListOf<HTMLImageElement> =
          tempWrapperElem.querySelectorAll('img');
        const base64Src: string[] = [];
        const imgName: string[] = [];
        const uploadImg: Element[] = [];
        for (let i: number = 0; i < imgElem.length; i++) {
          if (
            imgElem[i as number].getAttribute('src') &&
            imgElem[i as number]
              .getAttribute('src')
              .split(',')[0]
              .indexOf('base64') >= 0
  ...
  private cropImageHandler(element: HTMLElement): void {
    const allImgElm: NodeListOf<HTMLImageElement> =
      element.querySelectorAll('.e-img-cropped');
    if (allImgElm.length > 0) {
      for (let i: number = 0; i < allImgElm.length; i++) {
        if (
          ...
          const ctx: CanvasRenderingContext2D = canvas.getContext('2d');
          image.onload = () => {
            const wGoalWidth: number =
              (this.cropImageData[i as number].goalWidth as number) /
              image.naturalWidth;
            const hGoalHeight: number =
              (this.cropImageData[i as number].goalHeight as number) /
              image.naturalHeight;
            const cropLength: number =
              (this.cropImageData[i as number].cropLength as number) /
              wGoalWidth;
            const cropTop: number =
              (this.cropImageData[i as number].cropTop as number) / hGoalHeight;
            const cropWidth: number =
              ((this.cropImageData[i as number].goalWidth as number) -
                (this.cropImageData[i as number].cropLength as number) -
                (this.cropImageData[i as number].cropR as number)) /
              wGoalWidth;
            const cropHeight: number =
              ((this.cropImageData[i as number].goalHeight as number) -
                (this.cropImageData[i as number].cropTop as number) -
                (this.cropImageData[i as number].cropB as number)) /
              hGoalHeight;
      ...
      if (
        !isNOU(this.parent.insertImageSettings.saveUrl) &&
        !isNOU(this.parent.insertImageSettings.path) &&
        this.parent.inputElement.querySelectorAll('img').length > 0 &&
        this.parent.inputElement
  ...
  private addTableClass(element: HTMLElement, source?: string): HTMLElement {
    ...
    for (let i: number = 0; i < tableElement.length; i++) {
      const isMSTeamsTable: boolean =
        tableElement[i as number].parentElement.nodeName === 'FIGURE';
      if (
        this.parent.pasteCleanupSettings.keepFormat &&
        source &&
        !isMSTeamsTable
      ) {
        tableElement[i as number].classList.add(
          'e-rte-paste-' + source + '-table'
        );
  ...
  private setImageProperties(allImg: HTMLImageElement): void {
    ...
    if (
      this.parent.insertImageSettings.minWidth !== '0' &&
      this.parent.insertImageSettings.minWidth !== 0
    ) {
      allImg.style.minWidth =
        this.parent.insertImageSettings.minWidth.toString();
    }
    ...
    if (
      this.parent.insertImageSettings.minHeight !== '0' &&
      this.parent.insertImageSettings.minHeight !== 0
    ) {
      allImg.style.minHeight =
        this.parent.insertImageSettings.minHeight.toString();
    }
    if (this.parent.insertImageSettings.maxHeight !== null) {
      allImg.style.maxHeight =
  ...
  private removeTempClass(): void {
    const classElm: NodeListOf<Element> =
      this.parent.inputElement.querySelectorAll('.pasteContent_RTE');
  ...
  private plainFormatting(
    ...
    if (clipBoardElem.textContent.trim() !== '') {
      if (
        !isNOU(clipBoardElem.firstElementChild) &&
        ...
        if (!isNOU(clipBoardElem.firstElementChild)) {
          const spanElm: HTMLElement = this.parent.createElement(
            'span'
          ) as HTMLElement;
          for (
            let i: number = 0, j: number = 0;
            i < firstElm.childNodes.length;
            i++, j++
          ) {
            if (firstElm.childNodes[i as number].nodeName === '#text') {
              spanElm.appendChild(firstElm.childNodes[i as number]);
              clipBoardElem.insertBefore(
                spanElm,
                clipBoardElem.firstElementChild
              );
              i--;
            } else if (
              firstElm.childNodes[i as number].nodeName !== '#text' &&
              j === 0
            ) {
              for (
                let k: number = 0;
                k < firstElm.childNodes[i as number].childNodes.length;
                k++
              ) {
                spanElm.appendChild(
                  firstElm.childNodes[i as number].childNodes[k as number]
                );
                clipBoardElem.insertBefore(
                  spanElm,
                  clipBoardElem.firstElementChild
  ...
  private reframeToBrContent(clipBoardElem: HTMLElement): HTMLElement {
    const newClipBoardElem: HTMLElement = this.parent.createElement('div', {
      className: 'pasteContent',
      styles: 'display:inline;',
    }) as HTMLElement;
    while (!isNOU(clipBoardElem.firstChild)) {
      const brElem: HTMLElement = this.parent.createElement(
        'br'
      ) as HTMLElement;
      const currentFirstChild: HTMLElement =
        clipBoardElem.firstChild as HTMLElement;
      if (currentFirstChild.nodeName === '#text') {
        ...
        if (
          !isNOU(currentFirstChild) &&
          !isNOU(currentFirstChild.nextSibling)
        ) {
          newClipBoardElem.appendChild(brElem);
        }
  ...
  private getTextContent(clipBoardElem: HTMLElement): void {
    for (let i: number = 0; i < this.blockNode.length; i++) {
      const inElem: NodeListOf<Element> = clipBoardElem.querySelectorAll(
        this.blockNode[i as number]
      );
      for (let j: number = 0; j < inElem.length; j++) {
        let parElem: HTMLElement;
        for (
          let k: number = 0, l: number = 0, preNode: string;
          k < inElem[j as number].childNodes.length;
          k++, l++
        ) {
          if (
            inElem[j as number].childNodes[k as number].nodeName === 'DIV' ||
            inElem[j as number].childNodes[k as number].nodeName === 'P' ||
            (inElem[j as number].childNodes[k as number].nodeName === '#text' &&
              inElem[j as number].childNodes[k as number].nodeValue.replace(
                /\u00a0/g,
                '&nbsp;'
              ) !== '&nbsp;' &&
            ...
            inElem[j as number].childNodes[
              k as number
            ].parentElement.parentElement.insertBefore(
              inElem[j as number].childNodes[k as number],
              inElem[j as number].childNodes[k as number].parentElement
            );
            k--;
          } else {
            parElem = inElem[j as number].childNodes[k as number].parentElement;
  ...
  private detachInlineElements(clipBoardElem: HTMLElement): void {
    for (let i: number = 0; i < this.inlineNode.length; i++) {
      const inElem: NodeListOf<Element> = clipBoardElem.querySelectorAll(
        this.inlineNode[i as number]
      );
      for (let j: number = 0; j < inElem.length; j++) {
        if (
          !(
            inElem[j as number] === clipBoardElem.firstChild &&
            inElem[j as number].nodeName === 'SPAN'
          )
        ) {
          let parElem: HTMLElement;
          for (
  ...
  private tagGrouping(deniedTags: string[]): string[] {
    ...
    for (let i: number = 0; i < groupingTags.length; i++) {
      ...
      if (groupingTags[i as number].split('[').length > 1) {
        groupingTags[i as number] = groupingTags[i as number]
          .split('[')[0]
          .trim();
      }
      if (keys.indexOf(groupingTags[i as number]) > -1) {
        for (
          let j: number = 0;
          j < values[keys.indexOf(groupingTags[i as number])].length;
          j++
        ) {
          if (
            groupingTags.indexOf(
              values[keys.indexOf(groupingTags[i as number])][j as number]
            ) < 0 &&
            addTags.indexOf(
              values[keys.indexOf(groupingTags[i as number])][j as number]
            ) < 0
  ...
  private attributesfilter(deniedTags: string[]): string[] {
    for (let i: number = 0; i < deniedTags.length; i++) {
      if (deniedTags[i as number].split('[').length > 1) {
        const userAttributes: string[] = deniedTags[i as number]
          .split('[')[1]
          .split(']')[0]
          .split(',');
        ...
        for (let j: number = 0; j < userAttributes.length; j++) {
          if (userAttributes[j as number].indexOf('!') < 0) {
            ...
            deniedAttributeArray.push(
              userAttributes[j as number].split('!')[1].trim()
            );
          }
        }
        const allowedAttribute: string =
          allowedAttributeArray.length > 1
            ? allowedAttributeArray.join('][')
            : allowedAttributeArray.join();
        const deniedAttribute: string =
          deniedAttributeArray.length > 1
            ? deniedAttributeArray.join('][')
            : deniedAttributeArray.join();
        if (deniedAttribute.length > 0) {
          const select: string =
            allowedAttribute !== ''
              ? deniedTags[i as number].split('[')[0] +
                '[' +
                allowedAttribute +
                ']'
              : deniedTags[i as number].split('[')[0];
  ...
  private deniedTags(clipBoardElem: HTMLElement): HTMLElement {
    let deniedTags: string[] = isNullOrUndefined(
    ...
    for (let i: number = 0; i < deniedTags.length; i++) {
      const removableElement: NodeListOf<Element> =
        clipBoardElem.querySelectorAll(deniedTags[i as number]);
  ...
  private deniedAttributes(
    ...
    const deniedAttrs: string[] = isNullOrUndefined(
    ...
    for (let i: number = 0; i < deniedAttrs.length; i++) {
      const removableAttrElement: NodeListOf<HTMLElement> =
        clipBoardElem.querySelectorAll('[' + deniedAttrs[i as number] + ']');
      for (let j: number = 0; j < removableAttrElement.length; j++) {
        removableAttrElement[j as number].removeAttribute(
          deniedAttrs[i as number]
        );
      }
    }
    return clipBoardElem;
  ...
  private allowedStyle(clipBoardElem: HTMLElement): HTMLElement {
    const allowedStyleProps: string[] = isNullOrUndefined(
      this.parent.pasteCleanupSettings.allowedStyleProps
    )
      ? []
      : [...this.parent.pasteCleanupSettings.allowedStyleProps];
    allowedStyleProps.push('list-style-type', 'list-style');
    const styleElement: NodeListOf<HTMLElement> =
      clipBoardElem.querySelectorAll('[style]');
    for (let i: number = 0; i < styleElement.length; i++) {
      let allowedStyleValue: string = '';
      const allowedStyleValueArray: string[] = [];
      const styleValue: string[] = styleElement[i as number]
        .getAttribute('style')
        .split(';');
      for (let k: number = 0; k < styleValue.length; k++) {
        if (
          allowedStyleProps.indexOf(
            styleValue[k as number].split(':')[0].trim()
          ) >= 0
        ) {
      ...
      allowedStyleValue =
        allowedStyleValueArray.join(';').trim() === ''
          ? allowedStyleValueArray.join(';')
          : allowedStyleValueArray.join(';') + ';';
  ...
  private processPictureElement(clipBoardElem: HTMLElement): void {
    const pictureElems: NodeListOf<HTMLElement> =
      clipBoardElem.querySelectorAll('picture');
    for (let i: number = 0; i < pictureElems.length; i++) {
      const imgElem: HTMLImageElement | null =
        pictureElems[i as number].querySelector('img');
      const sourceElems: NodeListOf<HTMLSourceElement> =
        pictureElems[i as number].querySelectorAll('source');
      if (imgElem && imgElem.getAttribute('src')) {
        ...
        for (let j: number = 0; j < sourceElems.length; j++) {
          const srcset: string | null =
            sourceElems[j as number].getAttribute('srcset');
          if (srcset) {
