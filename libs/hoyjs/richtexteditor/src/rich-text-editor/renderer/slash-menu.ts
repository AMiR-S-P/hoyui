...
import { isNullOrUndefined as isNOU, L10n } from '@hoyui/base';
import {
  defaultSlashMenuDataModel,
  injectibleSlashMenuDataModel,
  ISlashMenuModel,
  ModuleSlashMenuModel,
} from '../models/slash-menu-settings';
...
export class SlashMenu {
  ...
  private generateMentionModel(): MentionModel {
    ...
    const model: MentionModel = {
      ...
      itemTemplate:
        ...
        '${/if}',
      beforeOpen: () => {
        this.savedSelection = this.savedSelection.save(
          this.savedSelection.getRange(this.currentDocument),
  ...
  private handleSelect(args: SelectEventArgs): void {
    ...
    this.parent.trigger(
      'slashMenuItemSelect',
      selectEventArgs,
      (selectArgs: SlashMenuItemSelectArgs) => {
  ...
  private getItems(): { [key: string]: Object }[] {
    const items: (SlashMenuItems | ISlashMenuItem)[] =
      this.parent.slashMenuSettings.items;
    ...
    for (let i: number = 0; i < items.length; i++) {
      ...
      if (typeof items[i as number] === 'string') {
        ...
        if (isNOU(model)) {
          model = this.injectibleItems.filter(
            (item: ModuleSlashMenuModel) =>
              item.module.toLowerCase().replace(' ', '') ===
              commnadName.toLowerCase().replace(' ', '')
          )[0];
        }
        const localeVariable: { text: string; description: string } =
          slashMenuCommandsKey.get(commnadName as SlashMenuItems);
        dataSource.push({
        ...
        // Custom slash commands processing
        dataSource.push({
  ...
  public render(): void {
    if (
      ...
      this.parent.slashMenuSettings.enable
    ) {
      const options: MentionModel = this.generateMentionModel();
      this.mention = new Mention(options);
      this.mention.appendTo(
        this.parent.rootContainer.appendChild(
          this.parent.createElement('div', {
            className: 'e-rte-slash-menu',
            id: this.parent.getID() + '_slash_menu',
          })
        )
      );
    }
