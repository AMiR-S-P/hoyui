...
import { getInstance, closest, MouseEventArgs, selectAll } from '@hoyui/base';
import {
  Toolbar,
  ClickEventArgs,
  BeforeCreateArgs,
...
import { Popup, Tooltip, TooltipEventArgs } from '@hoyui/popups';
...
export class ToolbarRenderer implements IRenderer {
  ...
  private destroyTooltip(): void {
    ...
    if (
      !isNullOrUndefined(currentDocument.querySelector('.e-tooltip-wrap')) &&
      !isNullOrUndefined(currentDocument.querySelector('[data-tooltip-id]'))
    ) {
      const tooltipTargetEle: HTMLElement =
        currentDocument.querySelector('[data-tooltip-id]');
  ...
  private dropDownOpen(args: MenuEventArgs): void {
    if (
      args.element.parentElement.getAttribute('id').indexOf('TableCell') > -1 &&
      !isNOU(args.element.parentElement.querySelector('.e-cell-merge')) &&
      (!isNOU(
        args.element.parentElement.querySelector('.e-cell-horizontal-split')
      ) ||
        !isNOU(
          args.element.parentElement.querySelector('.e-cell-vertical-split')
        ))
    ) {
      const listEle: NodeListOf<HTMLElement> =
        args.element.querySelectorAll('li');
      const selectedEles: NodeListOf<HTMLElement> =
      ...
      if (selectedEles.length === 1) {
        ...
        if (
          !Array.from(selectedEles).every(
            (element: HTMLElement) =>
              element.tagName.toLowerCase() ===
              selectedEles[0].tagName.toLowerCase()
          )
        ) {
          addClass([listEle[0]], 'e-disabled');
        } else {
  ...
  public renderDropDownButton(args: IDropDownModel): DropDownButton {
      ...
      beforeOpen: (args: BeforeOpenCloseMenuEventArgs): void => {
        ...
        if (proxy.parent.editorMode !== 'Markdown') {
          const startNode: HTMLElement =
            proxy.parent.getRange().startContainer.parentElement;
          const tableEle: HTMLElement = startNode.closest('table');
          const trow: HTMLElement = startNode.closest('tr');
          if (
            !isNOU(tableEle) &&
            tableEle.classList.contains('e-dashed-border')
          ) {
            for (
              let index: number = 0;
              index < args.element.childNodes.length;
              index++
            ) {
              if (
                (
                  args.element.childNodes[index as number] as HTMLElement
                ).classList.contains('e-dashed-borders')
              ) {
                addClass(
                  [args.element.childNodes[index as number]] as Element[],
                  'e-active'
                );
              }
            }
            ...
            !isNOU(tableEle) &&
            !tableEle.classList.contains('e-dashed-border') &&
            tableEle.classList.contains('e-alternate-rows') &&
            window.getComputedStyle(trow).backgroundColor !== ''
          ) {
            for (
              let index: number = 0;
              index < args.element.childNodes.length;
              ...
              if (
                (
                  args.element.childNodes[index as number] as HTMLElement
                ).classList.contains('e-alternate-rows')
              ) {
                addClass(
                  [args.element.childNodes[index as number]] as Element[],
                  'e-active'
          ...
          while (
            alignEle !== proxy.parent.inputElement &&
            !isNOU(alignEle.parentElement)
          ) {
            if (alignEle.nodeName === '#text') {
              alignEle = alignEle.parentElement;
            }
            const alignStyle: string = window.getComputedStyle(
              alignEle as HTMLElement
            ).textAlign;
            if (
              ...
              if (
                (args.items[0 as number].text === 'Align Left' &&
                  alignStyle === 'left') ||
                alignStyle === 'start'
              ) {
                addClass(
                  [args.element.childNodes[0 as number]] as Element[],
                  'e-active'
                );
                break;
              } else if (
                args.items[1 as number].text === 'Align Center' &&
                alignStyle === 'center'
              ) {
                addClass(
                  [args.element.childNodes[1 as number]] as Element[],
                  'e-active'
                );
                break;
              } else if (
                args.items[2 as number].text === 'Align Right' &&
                alignStyle === 'right'
              ) {
                addClass(
                  [args.element.childNodes[2 as number]] as Element[],
                  'e-active'
                );
                break;
              } else if (
                args.items[3 as number].text === 'Align Justify' &&
                alignStyle === 'justify'
              ) {
                addClass(
                  [args.element.childNodes[3 as number]] as Element[],
                  'e-active'
                );
                break;
              }
          ...
          const imageEle: HTMLElement = closestNode
            ? closestNode
            : startNode.querySelector('img');
          if (
            (args.items[0 as number] as IDropDownItemModel).command === 'Images'
          ) {
            if (!isNOU(imageEle)) {
              let index: number;
              if (
                imageEle.classList.contains('e-imgleft') ||
                imageEle.classList.contains('e-imginline')
              ) {
                index = 0;
              } else if (
                imageEle.classList.contains('e-imgcenter') ||
                imageEle.classList.contains('e-imgbreak')
              ) {
                index = 1;
              ...
              if (
                !isNOU(args.element.childNodes[index as number] as HTMLElement)
              ) {
                addClass(
                  [args.element.childNodes[index as number] as Element],
                  'e-active'
          ...
          if (
            (args.items[0 as number] as IDropDownItemModel).command ===
              'Formats' ||
            (args.items[0 as number] as IDropDownItemModel).command === 'Font'
          ) {
            const fontName: string[] = [];
            const formats: string[] = [];
            let hasUpdatedActive: boolean = false;
            this.parent.format.types.forEach(
              (item: IDropDownItemModel): void => {
                formats.push(item.value.toLocaleLowerCase());
              }
            );
            this.parent.fontFamily.items.forEach(
              (item: IDropDownItemModel): void => {
            ...
            for (
              ...
              const divNode: HTMLDivElement = this.parent.createElement(
                'div'
              ) as HTMLDivElement;
              divNode.innerHTML = dropDown.content.trim();
              if (
                !hasUpdatedActive &&
                ((divNode.textContent.trim() !== '' &&
                  args.element.childNodes[
                    index as number
                  ].textContent.trim() === divNode.textContent.trim()) ||
                  ((args.items[0 as number] as IDropDownItemModel).command ===
                    'Formats' &&
                    !isNOU(toolbarStatus.formats) &&
                    this.parent.format.types[
                      index as number
                    ].value.toLowerCase() ===
                      toolbarStatus.formats.toLowerCase() &&
                    (
                      args.element.childNodes[index as number] as Element
                    ).classList.contains(
                      this.parent.format.types[index as number].cssClass
                    )) ||
                  ((args.items[0 as number] as IDropDownItemModel)
                    .subCommand === 'FontName' &&
                    (args.items[0 as number] as IDropDownItemModel).command ===
                      'Font' &&
                    !isNOU(toolbarStatus.fontname) &&
                    !isNOU(this.parent.fontFamily.items[index as number]) &&
                    this.parent.fontFamily.items[
                      index as number
                    ].value.toLowerCase() ===
                      toolbarStatus.fontname.toLowerCase() &&
                    (
                      args.element.childNodes[index as number] as Element
                    ).classList.contains(
                      this.parent.fontFamily.items[index as number].cssClass
                    )) ||
                  ((args.items[0 as number] as IDropDownItemModel)
                    .subCommand === 'FontName' &&
                    this.parent.fontFamily.items[index as number].value ===
                      '' &&
                    isNullOrUndefined(toolbarStatus.fontname) &&
                    (
                      args.element.childNodes[index as number] as Element
                    ).classList.contains(
                      this.parent.fontFamily.items[index as number].cssClass
                  ...
                  ((args.items[0 as number] as IDropDownItemModel)
                    .subCommand === 'FontSize' &&
                    args.element.childNodes[index as number].textContent ===
                      'Default' &&
                    divNode.textContent === 'Font Size' &&
                    this.parent.fontSize.items[index as number].value === '' &&
                    !isNullOrUndefined(toolbarStatus.fontsize)))
              ) {
                if (
                  !(
                    args.element.childNodes[index as number] as HTMLElement
          ...
          if (
            ...
            for (let index: number = 0; index < childNodes.length; index++) {
              ...
              if (
                !hasUpdatedActive &&
                divNode.textContent.trim() !== '' &&
                childNodes[index as number].textContent.trim() ===
                  divNode.textContent.trim()
              ) {
                if (
                  !(
                    childNodes[index as number] as HTMLElement
                  ).classList.contains('e-active')
                ) {
                  addClass(
                    [childNodes[index as number]] as Element[],
                    'e-active'
                  );
                  hasUpdatedActive = true;
                }
    ...
    const popupElement: Element = document.getElementById(
      dropDown.element.id + '-popup'
    );
  ...
  private mouseOutHandler(): void {
    if (!isNOU(this.tooltipTargetEle)) {
      this.tooltipTargetEle.setAttribute(
        'title',
        this.tooltipTargetEle.getAttribute('data-title')
      );
    } else {
      const currentDocument: Document = this.parent.iframeSettings.enable
        ? this.parent.contentModule.getPanel().ownerDocument
  ...
  private closeTooltip(args: { [key: string]: HTMLElement | boolean }): void {
    if (args.isTitle as boolean) {
      ...
      this.tooltipTargetEle.setAttribute(
        'data-title',
        this.tooltipTargetEle.getAttribute('title')
      );
      ...
      const currentDocument: Document = this.parent.iframeSettings.enable
        ? this.parent.contentModule.getPanel().ownerDocument
        : this.parent.contentModule.getDocument();
      this.tooltipTargetEle = closest(
        args.target as HTMLElement,
        '[data-tooltip-id]'
      );
      if (
        !isNOU(this.tooltipTargetEle) &&
  ...
  public renderListDropDown(args: IDropDownModel): DropDownButton {
    ...
    let css: string =
      ...
      ' ' +
      CLS_TB_BTN +
      (this.parent.inlineMode ? ' ' + CLS_INLINE_DROPDOWN : '');
    css +=
      ' ' +
      (args.itemName === 'NumberFormatList'
        ? CLS_NUMBERFORMATLIST_TB_BTN
        : CLS_BULLETFORMATLIST_TB_BTN);
    const content: HTMLElement = proxy.parent.createElement('span', {
      className: CLS_LIST_PRIMARY_CONTENT,
    });
    const inlineEle: HTMLElement = proxy.parent.createElement('span', {
    ...
    const dropDown: DropDownButton = new DropDownButton({
      ...
      beforeOpen: (args: BeforeOpenCloseMenuEventArgs): void => {
        if (proxy.parent.editorMode !== 'Markdown') {
          ...
          const currentLiElem: HTMLElement = !isNOU(listElem)
            ? listElem.parentElement
            : null;
          if (
            !isNOU(currentLiElem) &&
            (currentLiElem.nodeName === 'OL' || currentLiElem.nodeName === 'UL')
          ) {
            if (
              currentLiElem.nodeName === 'UL' &&
              (args.items[0 as number] as IDropDownItemModel).subCommand ===
                'NumberFormatList'
            ) {
              addClass(
                [args.element.childNodes[0 as number]] as Element[],
              ...
              let currentListStyle: string = currentLiElem.style.listStyleType
                .split('-')
                .join('')
                .toLocaleLowerCase();
              currentListStyle =
                currentListStyle === 'decimal' ? 'number' : currentListStyle;
              for (
                let index: number = 0;
                index < args.element.childNodes.length;
                index++
        ...
        const element: HTMLElement = args.event
        ...
        if (
          ...
          (element.classList.contains(CLS_LIST_PRIMARY_CONTENT) ||
    ...
    if (args.element.childElementCount === 1) {
      dropDown.element.insertBefore(
        content,
        dropDown.element.querySelector('.e-caret')
      );
    }
    args.element.tabIndex = -1;
  ...
  private paletteSelection(
    dropDownArgs: BeforeOpenCloseMenuEventArgs,
    currentElement: HTMLElement
  ): void {
    const ele: Element = dropDownArgs.element.querySelector(
      '.e-control.e-colorpicker'
    );
    const colorbox: HTMLElement[] = [].slice.call(
      selectAll('.e-tile', ele.parentElement)
    );
    removeClass(colorbox, 'e-selected');
    const style: string = (
      currentElement.querySelector('.' + CLS_RTE_ELEMENTS) as HTMLElement
   ...
   * @deprecated
   */
  public renderColorPickerDropDown(
    args: IColorPickerModel,
    item: string,
    ...
    let range: Range;
    const initialBackgroundColor: string = isNullOrUndefined(defaultColor)
      ? proxy.parent.backgroundColor.default
      : defaultColor;
    inlineEle.style.borderBottomColor =
      item === 'backgroundcolor'
        ? initialBackgroundColor
        : proxy.parent.fontColor.default;
    content.appendChild(inlineEle);
    const dropDown: DropDownButton = new DropDownButton({
      ...
      beforeOpen: (dropDownArgs: BeforeOpenCloseMenuEventArgs): void => {
        ...
        const element: HTMLElement = dropDownArgs.event
          ? (dropDownArgs.event.target as HTMLElement)
          : null;
        proxy.currentElement = dropDown.element;
        proxy.currentDropdown = dropDown;
        ...
        if (
          dropDownArgs.event &&
          dropDownArgs.event.type === 'click' &&
          (element.classList.contains(CLS_COLOR_CONTENT) ||
            element.parentElement.classList.contains(CLS_COLOR_CONTENT))
        ) {
          dropDownArgs.cancel = true;
          const colorpickerValue: string = element.classList.contains(
            CLS_RTE_ELEMENTS
          )
            ? element.style.borderBottomColor
            : (element.querySelector('.' + CLS_RTE_ELEMENTS) as HTMLElement)
                .style.borderBottomColor;
          proxy.parent.notify(events.selectionRestore, {});
          range = proxy.parent.formatter.editorManager.nodeSelection.getRange(
          ...
          if (
            (range.startContainer.nodeName === 'TD' ||
              range.startContainer.nodeName === 'TH' ||
            ...
            closest(closestElement, '.' + classes.CLS_RTE)
          ) {
          ...
          const ele: HTMLElement = dropDownArgs.element.querySelector(
            '.e-control.e-colorpicker'
          ) as HTMLElement;
          const inst: ColorPicker = getInstance(
            ele,
            ColorPicker
          ) as ColorPicker;
          inst.showButtons = dropDownArgs.element.querySelector(
            '.e-color-palette'
          )
            ? false
            : true;
      ...
      open: (dropDownArgs: OpenCloseMenuEventArgs): void => {
        ...
        if (dropDownArgs.element.querySelector('.e-color-palette')) {
          focusEle = ele.parentElement.querySelector(
            '.e-palette'
          ) as HTMLElement;
        } else {
          focusEle = ele.parentElement.querySelector(
  ...
  private setColorPickerContentWidth(colorPicker: ColorPicker): void {
    const colorPickerContent: HTMLElement = colorPicker.element
      .nextSibling as HTMLElement;
    if (colorPickerContent.style.width === '0px') {
      colorPickerContent.style.width = '';
      const borderWidth: number = parseInt(
        getComputedStyle(colorPickerContent).borderBottomWidth,
        10
      );
      colorPickerContent.style.width = formatUnit(
        (colorPickerContent.children[0] as HTMLElement).offsetWidth +
          borderWidth +
          borderWidth
      );
    }
  ...
  public renderColorPicker(args: IColorPickerModel, item: string): ColorPicker {
    ...
    const colorPicker: ColorPicker = new ColorPicker({
      ...
      mode:
        item === 'backgroundcolor'
          ? proxy.parent.backgroundColor.mode
          : proxy.parent.fontColor.mode,
      modeSwitcher:
        item === 'backgroundcolor'
          ? proxy.parent.backgroundColor.modeSwitcher
          : proxy.parent.fontColor.modeSwitcher,
      presetColors:
        item === 'backgroundcolor'
          ? this.parent.backgroundColor.colorCode
          : this.parent.fontColor.colorCode,
      columns:
        item === 'backgroundcolor'
          ? this.parent.backgroundColor.columns
          : this.parent.fontColor.columns,
      ...
      change: (colorPickerArgs: IColorPickerEventArgs): void => {
        ...
        proxy.parent.notify(events.selectionRestore, {});
        (
          proxy.currentElement.querySelector(
            '.' + CLS_RTE_ELEMENTS
          ) as HTMLElement
        ).style.borderBottomColor = colorpickerValue;
        const range: Range =
  ...
  public destroy(): void {
    ...
    if (this.tooltip && !this.tooltip.isDestroyed) {
      ...
      const tooltipElements: NodeListOf<Element> = document.querySelectorAll(
        '[data-rte-id="' + this.parent.getID() + '"]'
      );
      for (let i: number = 0; i < tooltipElements.length; i++) {
        const tooltipEle: Element = tooltipElements[i as number] as Element;
        if (
          this.parent.getID() ===
          (tooltipEle.getAttribute('data-rte-id') as string)
        ) {
          detach(tooltipEle);
        }
      }
