  ...
  private keyUp(e: NotifyArgs): void {
    if (this.parent.editorMode === 'HTML') {
      switch ((e.args as MouseEvent).which) {
        ...
        case 8:
          // eslint-disable-next-line
          const childNodes: Element[] = <NodeListOf<Element> & Element[]>(
            this.parent.contentModule.getEditPanel().childNodes
          );
          if (
            childNodes.length === 0 ||
            (childNodes.length === 1 &&
              childNodes[0].childNodes.length === 0 &&
              ((childNodes[0] as Element).tagName === 'BR' ||
                (((childNodes[0] as Element).tagName === 'P' ||
                  (childNodes[0] as Element).tagName === 'DIV') &&
