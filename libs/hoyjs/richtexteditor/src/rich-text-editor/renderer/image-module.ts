...
import { Button, CheckBox, ChangeEventArgs } from '@hoyui/buttons';
...
export class Image {
  ...
  protected removeEventListener(): void {
    ...
    const dropElement: HTMLElement | Document = this.parent.iframeSettings
      .enable
    ...
    if (!isNOU(this.contentModule)) {
      ...
      this.parent.formatter.editorManager.observer.off(
        events.checkUndo,
        this.undoStack
      );
      if (this.parent.insertImageSettings.resize) {
        ...
        (this.parent.element.ownerDocument as Document).removeEventListener(
          'mousedown',
          this.docClick
        );
        this.docClick = null;
  ...
  private updateCss(
    currentObj: CheckBox | TextBox | Uploader | Dialog,
    e: ICssClassArgs
  ): void {
  ...
  private afterRender(): void {
    ...
    if (this.parent.insertImageSettings.resize) {
      ...
      (this.parent.element.ownerDocument as Document).addEventListener(
        'mousedown',
        this.docClick
      );
      EventHandler.add(
  ...
  private undoStack(args?: { [key: string]: string }): void {
    if (
      (args.subCommand.toLowerCase() === 'undo' ||
        args.subCommand.toLowerCase() === 'redo') &&
      this.parent.editorMode === 'HTML'
    ) {
      for (
        let i: number = 0;
        i < this.parent.formatter.getUndoRedoStack().length;
        i++
      ) {
        const temp: Element = this.parent.createElement('div');
        const contentElem: DocumentFragment =
          this.parent.formatter.getUndoRedoStack()[i as number]
            .text as DocumentFragment;
        ...
        if (temp.querySelector('.e-img-resize') && img.length > 0) {
          ...
          const clonedElement: HTMLElement = temp.cloneNode(
            true
          ) as HTMLElement;
  ...
  private resizeEnd(e: PointerEvent | TouchEvent): void {
    ...
    this.parent.formatter.editorManager.observer.on(
      events.checkUndo,
      this.undoStack,
      this
    );
  ...
  private imageResize(e: HTMLImageElement): void {
    ...
    this.imgResizeDiv.appendChild(
      this.parent.createElement('span', {
        className:
          'e-rte-imageboxmark e-rte-topLeft' + this.parent.getCssClass(true),
    ...
    this.imgResizeDiv.appendChild(
      this.parent.createElement('span', {
        className:
          'e-rte-imageboxmark e-rte-topRight' + this.parent.getCssClass(true),
    ...
    this.imgResizeDiv.appendChild(
      this.parent.createElement('span', {
        className:
          'e-rte-imageboxmark e-rte-botLeft' + this.parent.getCssClass(true),
        styles: 'cursor: nesw-resize',
      })
    );
    this.imgResizeDiv.appendChild(
      this.parent.createElement('span', {
        className:
          'e-rte-imageboxmark e-rte-botRight' + this.parent.getCssClass(true),
        styles: 'cursor: nwse-resize',
      })
    );
    if (Browser.isDevice) {
  ...
  private imgResizePos(e: HTMLImageElement, imgResizeDiv: HTMLElement): void {
    ...
    const borWid: number = Browser.isDevice
    ...
    (imgResizeDiv.querySelector('.e-rte-botLeft') as HTMLElement).style.left =
      left - borWid + 'px';
    (imgResizeDiv.querySelector('.e-rte-botLeft') as HTMLElement).style.top =
      imgHgt - borWid + top + 'px';
    (imgResizeDiv.querySelector('.e-rte-botRight') as HTMLElement).style.left =
      imgWid - (borWid - devWid) + left + 'px';
    (imgResizeDiv.querySelector('.e-rte-botRight') as HTMLElement).style.top =
      imgHgt - borWid + top + 'px';
    (imgResizeDiv.querySelector('.e-rte-topRight') as HTMLElement).style.left =
      imgWid - (borWid - devWid) + left + 'px';
    (imgResizeDiv.querySelector('.e-rte-topRight') as HTMLElement).style.top =
      top - borWid + 'px';
    (imgResizeDiv.querySelector('.e-rte-topLeft') as HTMLElement).style.left =
      left - borWid + 'px';
    (imgResizeDiv.querySelector('.e-rte-topLeft') as HTMLElement).style.top =
      top - borWid + 'px';
  }
  ...
  private calcPos(elem: HTMLElement): OffsetPosition {
    ...
    let offsetParent: Node =
      (elem.offsetParent &&
      (elem.offsetParent.classList.contains('e-img-caption') ||
        ignoreOffset.indexOf(elem.offsetParent.tagName) > -1)
        ? closest(elem, '#' + this.parent.getID() + '_rte-edit-view')
  ...
  private setAspectRatio(
    ...
    const factor: string = this.parent.insertImageSettings.resizeByPercent
      ? '%'
      : 'px';
    const emptyStyleDimension: boolean =
  ...
  private imgDupMouseMove(
    ...
    this.parent.trigger(events.onResize, args, (resizingArgs: ResizeArgs) => {
      if (resizingArgs.cancel) {
        ...
        if (
          parseInt(this.parent.insertImageSettings.minWidth as string, 10) >=
            parseInt(width, 10) ||
          (parseInt(this.parent.getInsertImgMaxWidth() as string, 10) <=
            parseInt(width, 10) &&
        ...
        if (
          ...
          (parseInt(this.parent.insertImageSettings.minHeight as string, 10) >=
            parseInt(height, 10) ||
            parseInt(this.parent.insertImageSettings.maxHeight as string, 10) <=
              parseInt(height, 10))
        ) {
          return;
        }
  ...
  private resizing(e: PointerEvent | TouchEvent): void {
    ...
    if (
      ...
      const resizeFactor: number[] = this.getResizeFactor(
  ...
  private resizeImgDupPos(e: HTMLImageElement): void {
    this.imgDupPos = {
      width:
        e.style.width !== '' &&
        this.parent.insertImageSettings &&
        !this.parent.insertImageSettings.resizeByPercent
          ? this.imgEle.style.width
          : e.width + 'px',
      height:
        e.style.height !== '' ? this.imgEle.style.height : e.height + 'px',
    };
  }
  ...
  private openImgLink(e: NotifyArgs): void {
    const target: string =
      (e.selectParent[0].parentNode as HTMLAnchorElement).target === ''
        ? '_self'
        : '_blank';
    this.parent.formatter.process(this.parent, e.args, e.args, {
      url: (e.selectParent[0].parentNode as HTMLAnchorElement).href,
      target: target,
      selectNode: e.selectNode,
      subCommand: ((e.args as ClickEventArgs).item as IDropDownItemModel)
        .subCommand,
    });
  }

  private editImgLink(e: NotifyArgs): void {
    const selectParentEle: HTMLElement = e.selectParent[0]
    ...
    const inputDetails: { [key: string]: string } = {
      ...
      target: (selectParentEle as HTMLAnchorElement).target,
      header: 'Edit Link',
      btnText: linkUpdate,
    };
  ...
  private removeImgLink(e: NotifyArgs): void {
    ...
    const isCapLink: boolean =
      this.contentModule.getEditPanel().contains(this.captionEle) &&
      select('a', this.captionEle)
        ? true
        : false;
    const selectParent: Node[] = isCapLink
      ? [this.captionEle]
      : [e.selectNode[0].parentElement];
    this.parent.formatter.process(this.parent, e.args, e.args, {
      insertElement: e.selectNode[0] as HTMLElement,
  ...
  private onKeyDown(event: NotifyArgs): void {
    ...
    const keyCodeValues: number[] = [
      27, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122, 123, 44, 45, 9,
      16, 17, 18, 19, 20, 33, 34, 35, 36, 37, 38, 39, 40, 91, 92, 93, 144, 145,
      182, 183,
    ];
    if (this.parent.editorMode === 'HTML') {
      range = this.parent.formatter.editorManager.nodeSelection.getRange(
    ...
    if (
      !isCursor &&
      this.parent.editorMode === 'HTML' &&
      keyCodeValues.indexOf(originalEvent.which) < 0
    ) {
      const nodes: Node[] =
    ...
    if (
      this.parent.editorMode === 'HTML' &&
      ((originalEvent.which === 8 && originalEvent.code === 'Backspace') ||
      ...
      const isCursor: boolean =
        ...
        range.startOffset === range.endOffset;
      if (
        originalEvent.which === 8 &&
        originalEvent.code === 'Backspace' &&
        isCursor
      ) {
        this.checkImageBack(range);
      } else if (
        originalEvent.which === 46 &&
        originalEvent.code === 'Delete' &&
        isCursor
      ) {
    ...
    if (
      ...
      if (
        ...
        originalEvent.key &&
        (originalEvent.key.length === 1 || originalEvent.action === 'enter') &&
        !isNOU(selectParentEle[0]) &&
        (selectParentEle[0] as HTMLElement).tagName === 'IMG' &&
    ...
    if (
      originalEvent.ctrlKey &&
      (originalEvent.keyCode === 89 || originalEvent.keyCode === 90)
    ) {
    ...
    if (originalEvent.keyCode === 8 || originalEvent.keyCode === 46) {
      if (
        selectNodeEle &&
        selectNodeEle[0].nodeName === 'IMG' &&
        selectNodeEle.length < 1
      ) {
      ...
      if (
        this.parent.contentModule.getEditPanel().querySelector('.e-img-resize')
      ) {
        this.removeResizeEle();
      }
    }
      ...
      case 'delete':
        if (this.parent.editorMode !== 'Markdown') {
          if (range.startContainer.nodeType === 3) {
            if (originalEvent.code === 'Backspace') {
              if (
                (range.startContainer as HTMLElement).previousElementSibling &&
                range.startOffset === 0 &&
                (
              ...
              if (
                (range.startContainer as HTMLElement).nextElementSibling &&
                range.endContainer.textContent.length === range.endOffset &&
                (
                  range.startContainer as HTMLElement
                ).nextElementSibling.classList.contains(classes.CLS_CAPTION) &&
                (
                  range.startContainer as HTMLElement
                ).nextElementSibling.classList.contains(classes.CLS_CAPINLINE)
              ) {
                detach(
                  (range.startContainer as HTMLElement).nextElementSibling
                );
            ...
            if (
              (range.startContainer as HTMLElement).querySelector(
                '.' + classes.CLS_CAPTION + '.' + classes.CLS_CAPINLINE
              )
            ) {
              detach(
                (range.startContainer as HTMLElement).querySelector(
              ...
              (range.startContainer as HTMLElement).querySelector(
                '.' + classes.CLS_CAPTION + '.' + classes.CLS_IMGBREAK
              )
            ) {
              detach(
                (range.startContainer as HTMLElement).querySelector(
  ...
  private handleSelectAll(): void {
    this.cancelResizeAction();
    const imgFocusNodes: NodeListOf<Element> =
      this.parent.inputElement.querySelectorAll('.' + classes.CLS_IMG_FOCUS);
    ...
    if (!isNOU(this.deletedImg) && this.deletedImg.length > 0) {
      ...
      for (let i: number = deleteImages.length - 1; i >= 0; i--) {
        const args: AfterImageDeleteEventArgs = {
          ...
          src: (this.deletedImg[i as number] as HTMLElement).getAttribute(
            'src'
          ),
        };
  ...
  private checkImageBack(range: Range): void {
    if (
      ...
      range.startOffset === 0 &&
      !isNOU(range.startContainer.previousSibling) &&
  ...
  private checkImageDel(range: Range): void {
    if (
      ...
      !isNOU(range.startContainer.nextSibling) &&
  ...
  private alignmentSelect(e: ClickEventArgs): void {
    ...
    selectNodeEle =
      selectNodeEle[0].nodeName === 'IMG' ? selectNodeEle : [this.imgEle];
  ...
  private imageWithLinkQTBarItemUpdate(): void {
    const items: NodeListOf<Element> =
      this.quickToolObj.imageQTBar.toolbarElement.querySelectorAll(
        '.e-toolbar-item'
      );
    for (let i: number = 0; i < items.length; i++) {
      if (
        items[i as number].getAttribute('title') ===
          this.i10n.getConstant('openLink') ||
        items[i as number].getAttribute('title') ===
          this.i10n.getConstant('editLink') ||
        items[i as number].getAttribute('title') ===
          this.i10n.getConstant('removeLink')
      ) {
        ...
        items[i as number].classList.contains('e-rte-horizontal-separator')
      ) {
        detach(items[i as number]);
      }
    }
    const newItems: NodeListOf<Element> =
      this.quickToolObj.imageQTBar.toolbarElement.querySelectorAll(
        '.e-toolbar-item:not(.e-link-groups)'
      );
    this.quickToolObj.imageQTBar.addQTBarItem(
      ['-'],
      Math.round(newItems.length / 2)
    );
  }
  ...
  private showImageQuickToolbar(e: IShowPopupArgs): void {
    ...
    const pageY: number = this.parent.iframeSettings.enable
      ? window.pageYOffset +
    ...
    if (this.parent.quickToolbarModule.imageQTBar) {
      if (e.isNotify) {
        this.imageQTPopupTime = setTimeout(() => {
          ...
          this.quickToolObj.imageQTBar.showPopup(
            args.pageX,
            pageY,
            target as Element
          );
  ...
  private hideImageQuickToolbar(): void {
    if (
      !isNOU(this.contentModule.getEditPanel().querySelector('.e-img-focus'))
    ) {
      removeClass(
        [this.contentModule.getEditPanel().querySelector('.e-img-focus')],
        'e-img-focus'
      );
      if (
        this.quickToolObj &&
  ...
  private editAreaClickHandler(e: IImageNotifyArgs): void {
    ...
    if (
      ...
      if (
        showOnRightClick &&
        args.which === 1 &&
        !isNOU(args.target as HTMLElement) &&
    ...
    if (
      ...
      const isPopupOpen: boolean =
        this.quickToolObj.imageQTBar.element.classList.contains('e-rte-pop');
        ...
        if (closest(target, 'a')) {
          for (let i: number = 0; i < items.length; i++) {
            if (
              ...
              items[i as number].getAttribute('title') === 'Insert Link'
            ) {
              (items[i as number] as HTMLElement).style.display = 'none';
            } else if (
              items[i as number].classList.contains(
                'e-rte-horizontal-separator'
              )
            ) {
  ...
  private insertImgLink(
    ...
    this.imagDialog(e);
    if (!isNOU(this.dialogObj)) {
      const linkWrap: HTMLElement = this.parent.createElement('div', {
      ...
      const linkTarget: HTMLInputElement = linkWrap.querySelector(
        '.e-rte-linkTarget'
      ) as HTMLInputElement;
      const inputLink: HTMLElement = linkWrap.querySelector(
        '.e-img-link'
      ) as HTMLElement;
      const linkOpenLabel: string = this.i10n.getConstant(
        'linkOpenInNewWindow'
      );
      ...
      const imageLabel: string | null =
        this.i10n.getConstant('imageLinkAriaLabel');
      const linkUpdate: string = this.i10n.getConstant('dialogUpdate');
  ...
  private insertAltText(e: IImageNotifyArgs): void {
    ...
    if (!isNOU(this.dialogObj)) {
      ...
      const inputAlt: HTMLElement = altWrap.querySelector(
        '.e-img-alt'
      ) as HTMLElement;
  ...
  private insertlink(e: IImageNotifyArgs): void {
    ...
    if (e.selectNode[0].parentElement.nodeName === 'A') {
      proxy.parent.formatter.process(proxy.parent, e.args, e.args, {
        ...
        ariaLabel: proxy.checkBoxObj.checked
          ? this.i10n.getConstant('imageLinkAriaLabel')
          : null,
        selectNode: e.selectNode,
        subCommand: ((e.args as ClickEventArgs).item as IDropDownItemModel)
  ...
  private deleteImg(e: IImageNotifyArgs, keyCode?: number): void {
    ...
    if (
      ...
      (restoreStartElement as HTMLElement).parentElement.classList.contains(
        'e-img-caption'
      )
    ) {
  ...
  private caption(e: IImageNotifyArgs): void {
    ...
    const subCommand: string = (e.args as ClickEventArgs).item
      ? ((e.args as ClickEventArgs).item as IDropDownItemModel).subCommand
      : 'Caption';
    if (!isNOU(closest(selectNode, '.' + classes.CLS_CAPTION))) {
      ...
      const imgWrap: HTMLElement = this.parent.createElement('span', {
      ...
      this.parent.formatter.editorManager.nodeSelection.setSelectionText(
        this.contentModule.getDocument(),
        imgInner.childNodes[0],
        imgInner.childNodes[0],
        0,
        imgInner.childNodes[0].textContent.length
      );
    }
    if (
      this.quickToolObj &&
  ...
  private imageSize(e: IImageNotifyArgs): void {
    ...
    if (!isNOU(this.dialogObj)) {
      ...
      const selectObj: IImageNotifyArgs = {
        args: e.args,
        selfImage: this,
        selection: e.selection,
        selectNode: e.selectNode,
  ...
  private break(e: IImageNotifyArgs): void {
    ...
    const subCommand: string = (
      (e.args as ClickEventArgs).item as IDropDownItemModel
    ).subCommand;
    this.parent.formatter.process(this.parent, e.args, e.args, {
      selectNode: e.selectNode,
      subCommand: subCommand,
  ...
  private imagDialog(e: IImageNotifyArgs): void {
    ...
    const imgDialog: HTMLElement = this.parent.createElement('div', {
    ...
    const dialogModel: DialogModel = {
      ...
      close: (event: BeforeCloseEventArgs) => {
        if (
          event &&
          event.closedBy !== 'user action' &&
          this.uploadObj.filesData.length > 0
        ) {
    ...
    const dialogContent: HTMLElement = this.parent.createElement('div', {
      className: 'e-img-content' + this.parent.getCssClass(true),
    });
    if (
      (!isNOU(this.parent.insertImageSettings.path) &&
        this.parent.editorMode === 'Markdown') ||
      this.parent.editorMode === 'HTML'
    ) {
      dialogContent.appendChild(this.imgUpload(e));
    }
    const linkHeader: HTMLElement = this.parent.createElement('div', {
    ...
    if (e.selectNode && e.selectNode[0].nodeName === 'IMG') {
      ...
      dialogModel.buttons[0].buttonModel.cssClass =
        dialogModel.buttons[0].buttonModel.cssClass + ' e-updateImage';
    } else {
      dialogModel.content = dialogContent;
    }
  ...
  private onDocumentClick(e: MouseEvent): void {
    ...
    if (
      ...
      /* eslint-disable */
      if (
        e.offsetX > (e.target as HTMLImageElement).clientWidth ||
        e.offsetY > (e.target as HTMLImageElement).clientHeight
      ) {
      } else {
    ...
    if (
      !(
        ...
        !isNOU(this.parent.currentTarget) &&
      ...
      !(
        this.quickToolObj &&
        this.quickToolObj.imageQTBar &&
        this.quickToolObj.imageQTBar.element.contains(e.target as HTMLElement)
      ) &&
      this.contentModule.getEditPanel().contains(this.imgResizeDiv)
    ) {
      this.cancelResizeAction();
    ...
    if (
      ...
      if (target.tagName !== 'IMG' && !isNOU(this.imgEle)) {
        ...
        !isNOU(this.prevSelectedImgEle) &&
        this.prevSelectedImgEle !== target
      ) {
        this.prevSelectedImgEle.style.outline = '';
      }
    }
    if (target.tagName !== 'IMG') {
      const items: NodeListOf<HTMLElement> = this.contentModule
        .getEditPanel()
    ...
    if (
      ...
      target &&
      this.dialogObj &&
      !closest(target, '#' + this.dialogObj.element.id)
    ) {
      this.dialogObj.hide();
    }
  }
  ...
  private onWindowResize(): void {
    if (
      !isNOU(this.contentModule) &&
      !isNOU(this.contentModule.getEditPanel().querySelector('.e-img-resize'))
    ) {
      this.cancelResizeAction();
    }
  }
  ...
  private imageUrlPopup(e: IImageNotifyArgs): HTMLElement {
    const imgUrl: HTMLElement = this.parent.createElement('div', {
    ...
    this.inputUrl.addEventListener('input', () => {
      if (!isNOU(this.inputUrl)) {
        if ((this.inputUrl as HTMLInputElement).value.length === 0) {
          ...
          (this.dialogObj.getButtons(0) as Button).element.removeAttribute(
            'disabled'
          );
        }
      }
    ...
    if (e.selectNode && e.selectNode[0].nodeName === 'IMG') {
      ...
      (this.inputUrl as HTMLInputElement).value = (
        e.selectNode[0] as HTMLImageElement
      ).src.match(regex)
        ? (e.selectNode[0] as HTMLImageElement).src
        : '';
    }
  ...
  private insertImageUrl(e: MouseEvent): void {
    ...
    if (
      ...
      (e.target as HTMLElement).classList.contains('e-updateImage')
    ) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const element: HTMLElement =
        (this as any).selectNode &&
        (this as any).selectNode[0] &&
        (this as any).selectNode[0].nodeName === 'IMG'
          ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ((this as any).selectNode[0] as HTMLElement)
          : null;
      const args: AfterImageDeleteEventArgs = {
        element: element,
        src: url,
    ...
    if (!isNOU(proxy.uploadUrl) && proxy.uploadUrl.url !== '') {
      proxy.uploadUrl.cssClass =
        proxy.parent.insertImageSettings.display === 'inline'
          ? classes.CLS_IMGINLINE
          : classes.CLS_IMGBREAK;
      proxy.dialogObj.hide({ returnValue: false } as Event);
      proxy.parent.formatter.process(
        proxy.parent,
        (this as IImageNotifyArgs).args,
        ((this as IImageNotifyArgs).args as ClickEventArgs).originalEvent,
        proxy.uploadUrl
      );
      proxy.uploadUrl.url = '';
      ...
      if (
        ...
        isNOU(
          closest(
            // eslint-disable-next-line
            (this as IImageNotifyArgs).selection.range.startContainer
              .parentNode,
        ...
        (this as IImageNotifyArgs).selection =
          proxy.parent.formatter.editorManager.nodeSelection.save(
            range,
            proxy.contentModule.getDocument()
          );
        (this as IImageNotifyArgs).selectParent =
          proxy.parent.formatter.editorManager.nodeSelection.getParentNodeCollection(
            range
          );
      }
      const regex: RegExp = /[\w-]+.(jpg|png|jpeg|gif)/g;
      const matchUrl: string =
        !isNOU(url.match(regex)) && proxy.parent.editorMode === 'HTML'
          ? url.match(regex)[0]
          : '';
      const value: IImageCommandsArgs = {
        cssClass:
          proxy.parent.insertImageSettings.display === 'inline'
            ...
            : classes.CLS_IMGBREAK,
        url: url,
        selection: (this as IImageNotifyArgs).selection,
        altText: matchUrl,
  ...
  private imgsizeInput(e: IImageNotifyArgs): HTMLElement {
    const selectNode: HTMLImageElement = (e as IImageNotifyArgs)
      .selectNode[0] as HTMLImageElement;
    const imgHeight: string = this.i10n.getConstant('imageHeight');
    const imgWidth: string = this.i10n.getConstant('imageWidth');
    const imgSizeWrap: HTMLElement = this.parent.createElement('div', {
      className: 'e-img-sizewrap' + this.parent.getCssClass(true),
    });
    let widthVal: string =
      isNOU(this.changedWidthValue) &&
      (selectNode.style.width.toString() === 'auto' ||
        selectNode.style.width !== '')
        ? selectNode.style.width
        : !isNOU(this.changedWidthValue)
        ? this.changedWidthValue
        : parseInt(
            selectNode.getClientRects()[0].width.toString(),
            10
          ).toString();
    let heightVal: string =
      isNOU(this.changedHeightValue) &&
      (selectNode.style.height.toString() === 'auto' ||
        selectNode.style.height !== '')
        ? selectNode.style.height
        : !isNOU(this.changedHeightValue)
        ? this.changedHeightValue
        : parseInt(
            selectNode.getClientRects()[0].height.toString(),
            10
          ).toString();
    ...
    this.widthNum.appendTo(
      imgSizeWrap.querySelector('#imgwidth') as HTMLElement
    );
    ...
    this.heightNum.appendTo(
      imgSizeWrap.querySelector('#imgheight') as HTMLElement
    );
    return imgSizeWrap;
  }
  ...
  private insertSize(e: IImageNotifyArgs): void {
    ...
    const width: string = (
      dialogEle.querySelector('.e-img-width') as HTMLInputElement
    ).value;
    const height: string = (
      dialogEle.parentElement.querySelector('.e-img-height') as HTMLInputElement
    ).value;
  ...
  private insertImage(e: IImageNotifyArgs): void {
    ...
    if (!isNOU(this.dialogObj)) {
      this.dialogObj.element.style.maxHeight = 'inherit';
      const dialogContent: HTMLElement =
        this.dialogObj.element.querySelector('.e-img-content');
      if (
        (!isNOU(this.parent.insertImageSettings.path) &&
          this.parent.editorMode === 'Markdown') ||
        this.parent.editorMode === 'HTML'
        ...
        (
          document.getElementById(this.rteID + '_insertImage') as HTMLElement
        ).focus();
      } else {
  ...
  private imgUpload(e: IImageNotifyArgs): HTMLElement {
    ...
    if (
      ...
      ((!iframe &&
        isNOU(
          closest(
            e.selection.range.startContainer.parentNode,
            '[id=' +
              // eslint-disable-next-line
              "'" +
        ...
        (iframe &&
          !hasClass(
            e.selection.range.startContainer.parentNode.ownerDocument.querySelector(
              'body'
            ),
            'e-lib'
          )))
    ...
    const uploadParentEle: HTMLElement = this.parent.createElement('div', {
      className: 'e-img-uploadwrap e-droparea' + this.parent.getCssClass(true),
    });
    const deviceImgUpMsg: string = this.i10n.getConstant(
    ...
    const uploadEle: HTMLInputElement | HTMLElement = this.parent.createElement(
      'input',
      {
    ...
    this.uploadObj = new Uploader({
      ...
      cssClass: this.parent.getCssClass(),
      allowedExtensions:
        this.parent.insertImageSettings.allowedTypes.toString(),
      selected: (e: SelectedEventArgs) => {
        proxy.isImgUploaded = true;
        selectArgs = e;
        filesData = e.filesData;
        this.parent.trigger(
          events.imageSelected,
          selectArgs,
          (selectArgs: SelectedEventArgs) => {
            if (!selectArgs.cancel) {
              this.checkExtension(selectArgs.filesData[0]);
              altText = selectArgs.filesData[0].name.replace(
                /\.[a-zA-Z0-9]+$/,
                ''
              );
              if (
                ...
                reader.addEventListener('load', (e: MouseEvent) => {
                  const url: string =
                    this.parent.insertImageSettings.saveFormat === 'Base64'
                      ? (reader.result as string)
                      : URL.createObjectURL(
                          convertToBlob(reader.result as string)
                        );
                  proxy.uploadUrl = {
                    url: url,
                    selection: save,
                  ...
                  if (
                    isNOU(proxy.parent.insertImageSettings.saveUrl) &&
                    this.isAllowedTypes &&
                    !isNOU(this.dialogObj)
                  ) {
                    (
                      this.dialogObj.getButtons(0) as Button
      ...
      success: (e: ImageSuccessEventArgs) => {
        ...
        this.parent.trigger(
          ...
          e,
          (e: ImageSuccessEventArgs) => {
            if (!isNOU(this.parent.insertImageSettings.path)) {
              const url: string =
                this.parent.insertImageSettings.path + e.file.name;
              // Update the URL of the previously uploaded image
              if (
                !isNOU(previousFileInfo) &&
                (e as SuccessEventArgs).operation === 'upload'
              ) {
            ...
            if (
              (e as ProgressEventArgs).operation === 'upload' &&
              !isNOU(this.dialogObj)
            ) {
              (this.dialogObj.getButtons(0) as Button).element.removeAttribute(
                'disabled'
  ...
  private checkExtension(e: FileInfo): void {
    if (this.uploadObj.allowedExtensions) {
      if (e.type) {
        if (
          this.uploadObj.allowedExtensions
            .toLocaleLowerCase()
            .indexOf(('.' + e.type).toLocaleLowerCase()) === -1
        ) {
          (this.dialogObj.getButtons(0) as Button).element.setAttribute(
            'disabled',
            'disabled'
          );
  ...
  private dragOver(e?: DragEvent): void | boolean {
    if (
      (Browser.info.name === 'edge' &&
        e.dataTransfer.items[0].type.split('/')[0].indexOf('image') > -1) ||
  ...
  private dragDrop(args: ImageDropEventArgs): void {
    this.parent.trigger(
      events.beforeImageDrop,
      args,
      (e: ImageDropEventArgs) => {
        const imgElement: HTMLElement =
          this.parent.inputElement.ownerDocument.querySelector(
            '.' + classes.CLS_RTE_DRAG_IMAGE
          );
        const isImgOrFileDrop: boolean =
          (imgElement && imgElement.tagName === 'IMG') ||
          e.dataTransfer.files.length > 0;
        if (!e.cancel && isImgOrFileDrop) {
          if (
            closest(
              e.target as HTMLElement,
              '#' + this.parent.getID() + '_toolbar'
            ) ||
          ...
          const uploadArea: HTMLElement = this.parent.element.querySelector(
            '.' + classes.CLS_DROPAREA
          ) as HTMLElement;
          if (uploadArea) {
            return;
          }
  ...
  private getDropRange(x: number, y: number): Range {
    ...
    const elem: Element = this.contentModule
      .getDocument()
      .elementFromPoint(x, y);
    const startNode: Node =
      elem.childNodes.length > 0 ? elem.childNodes[0] : elem;
    let startCharIndexCharacter: number = 0;
    if (
      (this.parent.inputElement.firstChild as HTMLElement).innerHTML === '<br>'
    ) {
      ...
      do {
        ...
        rangeRect.left < x &&
        startCharIndexCharacter < (startNode as Text).length - 1
      );
    }
    return startRange;
  }
  ...
  private insertDragImage(e: DragEvent): void {
    e.preventDefault();
    const activePopupElement: HTMLElement = this.parent.element.querySelector(
      '' + classes.CLS_POPUP_OPEN
    );
    ...
    if (e.dataTransfer.files.length > 0 && imgElement === null) {
      ...
      const allowedTypes: string[] =
        this.parent.insertImageSettings.allowedTypes;
      for (let i: number = 0; i < allowedTypes.length; i++) {
        if (
          imgType.toLocaleLowerCase() ===
          allowedTypes[i as number].toLowerCase()
        ) {
          ...
          if (!actionBeginArgs.cancel) {
            ...
            if (imgElement && imgElement.tagName === 'IMG') {
              if (imgElement.nextElementSibling) {
                if (
                  imgElement.nextElementSibling.classList.contains(
                    classes.CLS_IMG_INNER
                  )
                ) {
              ...
              const imgArgs: ActionCompleteEventArgs = {
                elements: [imgElement],
              };
  ...
  private onSelect(args: DragEvent): void {
    ...
    const parentElement: HTMLElement = this.parent.createElement('ul', {
    ...
    const imageTag: HTMLImageElement = <HTMLImageElement>(
      this.parent.createElement('IMG')
    );
    ...
    reader.addEventListener('load', () => {
      ...
      imageTag.src =
        proxy.parent.insertImageSettings.saveFormat === 'Blob'
          ? url
          : (reader.result as string);
    });
    if (file) {
    ...
    const imageCommand: IImageCommandsArgs = {
      cssClass:
        this.parent.insertImageSettings.display === 'inline'
          ? classes.CLS_IMGINLINE
          : classes.CLS_IMGBREAK,
    ...
    this.parent.trigger(
      ...
      (actionBeginArgs: ActionBeginEventArgs) => {
        if (!actionBeginArgs.cancel) {
          const command: IImageCommandsArgs =
            actionBeginArgs.itemCollection as IImageCommandsArgs;
  ...
  private uploadMethod(
    ...
    if (!isNOU(this.parent.cssClass)) {
      addClass(
        ...
        this.parent.cssClass.replace(/\s+/g, ' ').trim().split(' ')
      );
    }
    const range: Range =
      this.parent.formatter.editorManager.nodeSelection.getRange(
    ...
    const timeOut: number =
      dragEvent.dataTransfer.files[0].size > 1000000 ? 300 : 100;
    ...
    this.uploadObj = new Uploader({
      ...
      uploading: (e: UploadingEventArgs) => {
        if (!this.parent.isServerRendered) {
          isUploading = true;
          this.parent.trigger(
            events.imageUploading,
            e,
            (imageUploadingArgs: UploadingEventArgs) => {
  ...
  private refreshPopup(imageElement: HTMLElement): void {
    const imgPosition: number = this.parent.iframeSettings.enable
      ? this.parent.element.offsetTop + imageElement.offsetTop
      : imageElement.offsetTop;
    const rtePosition: number =
      this.parent.element.offsetTop + this.parent.element.offsetHeight;
  ...
  private uploadSuccess(
    ...
    this.parent.trigger(events.imageUploadSuccess, e, (e: object) => {
      if (!isNOU(this.parent.insertImageSettings.path)) {
        const url: string =
          this.parent.insertImageSettings.path + (e as MetaData).file.name;
