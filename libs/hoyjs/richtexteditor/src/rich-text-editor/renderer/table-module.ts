...
import { Dialog, Popup } from '@hoyui/popups';
import { DialogModel } from '@hoyui/popups';
...
import { ClickEventArgs } from '@hoyui/navigations';
...
export class Table {
  ...
  private constructor(
    ...
    this.rendererFactory =
      serviceLocator.getService<RendererFactory>('rendererFactory');
    this.dialogRenderObj =
      serviceLocator.getService<DialogRenderer>('dialogRenderObject');
  ...
  private updateCss(
    currentObj: Button | Dialog | NumericTextBox,
    e: ICssClassArgs
  ): void {
    if (currentObj && e.cssClass) {
      if (isNullOrUndefined(e.oldCssClass)) {
        ...
        currentObj.setProperties({
          cssClass: (
            currentObj.cssClass.replace(e.oldCssClass, '').trim() +
            ' ' +
            e.cssClass
          ).trim(),
        });
  ...
  private selectionTable(e: ITableNotifyArgs): void {
    const target: HTMLTableCellElement = (e.args as MouseEvent)
      .target as HTMLTableCellElement;
    if (
      Browser.info.name === 'mozilla' &&
      !isNOU(closest(target, 'table')) &&
  ...
  private dropdownSelect(e: ClickEventArgs): void {
    ...
    if (
      !document.body.contains(
        document.body.querySelector('.e-rte-quick-toolbar')
      ) ||
      item.command !== 'Table'
    ) {
      return;
    ...
    const range: Range =
      this.parent.formatter.editorManager.nodeSelection.getRange(
    ...
    const args: ITableNotifyArgs = {
      ...
      selectParent:
        this.parent.formatter.editorManager.nodeSelection.getParentNodeCollection(
          range
        ),
    };
  ...
  private keyUp(e: NotifyArgs): void {
    const target: HTMLElement = <HTMLElement>(
      (e.args as KeyboardEventArgs).target
    );
    if (
      (e.args as KeyboardEventArgs).key.toLocaleLowerCase() === 'escape' &&
      target &&
      target.classList &&
      this.popupObj &&
      ...
      let createTableToolbarBtn: HTMLElement = this.popupObj
        .relateTo as HTMLElement;
      if (createTableToolbarBtn.nodeName !== 'BUTTON') {
        createTableToolbarBtn = createTableToolbarBtn.querySelector(
          'span.e-create-table'
        );
        createTableToolbarBtn =
          createTableToolbarBtn.parentElement as HTMLElement;
      }
      this.popupObj.hide();
  ...
  private keyDown(e: NotifyArgs): void {
    ...
    if (
      !isNullOrUndefined(this.parent.formatter.editorManager.nodeSelection) &&
      this.contentModule &&
      event.code !== 'KeyK'
    ) {
      ...
      ele =
        ele && ele.tagName !== 'TD' && ele.tagName !== 'TH'
          ? ele.parentElement
          : ele;
      if (
        (event as KeyboardEventArgs).keyCode === 8 ||
        (event as KeyboardEventArgs).keyCode === 46 ||
      ...
      if (ele && ele.tagName !== 'TD' && ele.tagName !== 'TH') {
        ...
        ele =
          !isNullOrUndefined(closestTd) &&
          this.parent.inputElement.contains(closestTd)
            ? closestTd
            : ele;
      }
      if (ele && (ele.tagName === 'TD' || ele.tagName === 'TH')) {
        const selectedEndCell: NodeListOf<HTMLElement> = this.contentModule
          .getEditPanel()
          .querySelectorAll('.e-cell-select-end');
        if (
          (isNOU(this.activeCell) || this.activeCell !== ele) &&
        ...
        if (
          !isNOU(this.parent.formatter.editorManager.nodeSelection) &&
          this.contentModule
        ) {
          selection = this.parent.formatter.editorManager.nodeSelection.save(
            range,
        ...
        if (
          ...
          ((event as KeyboardEventArgs).shiftKey &&
            (event as KeyboardEventArgs).keyCode === 9)
        ) {
    ...
    if (
      (event as KeyboardEventArgs).shiftKey &&
      ((event as KeyboardEventArgs).keyCode === 39 ||
        (event as KeyboardEventArgs).keyCode === 37 ||
        (event as KeyboardEventArgs).keyCode === 38 ||
        (event as KeyboardEventArgs).keyCode === 40)
    ) {
    ...
    if (
      ((event.code === 'Delete' && event.which === 46) ||
      ...
      if (
        ...
        (range.startContainer as HTMLElement).classList.contains(
        ...
        const table: HTMLElement =
          event.code === 'Delete' && event.which === 46
            ? this.getAdjacentTableElement(range, true)
            : this.getAdjacentTableElement(range, false);
        if (table) {
      ...
      const isActionKey: boolean =
        classes.ALLOWED_ACTIONKEYS.indexOf(event.key) !== -1;
      if (
        isShiftEnter ||
        isActionKey ||
        (event.key && event.key.length === 1)
      ) {
        const table: HTMLElement = this.parent.contentModule
          .getEditPanel()
          .querySelector('table.e-cell-select');
        if (table) {
          if (
            (event as KeyboardEventArgs).keyCode === 39 ||
            (event as KeyboardEventArgs).keyCode === 37
  ...
  private tableCellsKeyboardSelection(e: Event): void {
    ...
    const isMultiSelect: boolean =
      ...
      !isNullOrUndefined(range.commonAncestorContainer) &&
      range.commonAncestorContainer.nodeType === Node.ELEMENT_NODE &&
      ((range.commonAncestorContainer as Element).tagName === 'TR' ||
        (range.commonAncestorContainer as Element).tagName === 'TBODY') &&
      !isNullOrUndefined(this.activeCell);
    const selectedEndCell: NodeListOf<HTMLElement> = this.contentModule
      .getEditPanel()
      .querySelectorAll('.e-cell-select-end');
    if (!isNullOrUndefined(selectedEndCell) && selectedEndCell.length > 0) {
      this.parent.formatter.editorManager.nodeSelection.Clear(
        this.parent.contentModule.getDocument()
      );
      this.parent.formatter.editorManager.nodeSelection.setSelectionText(
        this.parent.contentModule.getDocument(),
    ...
    if (
      isMultiSelect ||
      (!isNullOrUndefined(selectedEndCell) && selectedEndCell.length > 0)
    ) {
      ...
      if ((event as KeyboardEventArgs).keyCode === 39) {
        if (activeCellColIndex < cells[0].length - 1) {
          target =
            cells[activeCellRowIndex as number][
              (activeCellColIndex + 1) as number
            ];
        } else if (activeCellRowIndex < cells.length - 1) {
          target =
            cells[(activeCellRowIndex + 1) as number][
              activeCellColIndex as number
            ];
          if (
            selectedEndCell.length === 0 &&
            activeCellRowIndex < cells.length - 1
          ) {
        ...
        if (0 < activeCellColIndex) {
          ...
          if (selectedEndCell.length === 0 && 0 < activeCellRowIndex) {
            this.activeCell =
              cells[activeCellRowIndex as number][
                (cells[activeCellRowIndex as number].length - 1) as number
              ];
          }
        } else {
          this.resetTableSelection();
  ...
  private resetTableSelection(): void {
    const selectedEndCell: NodeListOf<HTMLElement> = this.contentModule
      ...
      .querySelectorAll('.e-cell-select-end');
    if (!isNullOrUndefined(selectedEndCell) && selectedEndCell.length > 0) {
      this.parent.formatter.editorManager.nodeSelection.setSelectionNode(
        this.parent.contentModule.getDocument(),
        this.curTable
      );
    }
  ...
  private getCorrespondingColumns(): HTMLElement[][] {
    ...
    for (let i: number = 0; i <= allRows.length - 1; i++) {
      ...
      for (let j: number = 0; j <= ele.children.length - 1; j++) {
        ...
        for (
          let ele: HTMLElement = colEle as HTMLElement,
            colspan: number = parseInt(ele.getAttribute('colspan'), 10) || 1,
            rowSpan: number = parseInt(ele.getAttribute('rowspan'), 10) || 1,
            rowIndex: number = i;
          rowIndex < i + rowSpan;
          rowIndex++
        ) {
          for (
            ...
            if (elementArray[rowIndex as number][colIndex as number]) {
              ...
              elementArray[rowIndex as number][colIndex as number] =
                colEle as HTMLElement;
            }
          }
        }
        index += colspan;
  ...
  private getAdjacentTableElement(
    ...
    const startContainer: HTMLElement = (
      range.collapsed &&
      this.parent.contentModule.getEditPanel() === range.startContainer &&
      nodeCollection &&
    ...
    if (startContainer && startContainer.nodeType === Node.ELEMENT_NODE) {
      if (
        startContainer.tagName === 'IMG' ||
        startContainer.querySelector('img') ||
        startContainer.tagName === 'AUDIO' ||
        startContainer.querySelector('audio') ||
        startContainer.tagName === 'VIDEO' ||
        startContainer.querySelector('video') ||
        startContainer.querySelector('.e-video-clickelem')
      ) {
        const compareRange: Range = this.contentModule
          .getDocument()
          .createRange();
        compareRange.collapse(true);
        compareRange.selectNodeContents(startContainer);
        const nodeIndex: number =
          this.parent.formatter.editorManager.nodeSelection.getIndex(
            startContainer
          );
        if (
          (isdelKey && compareRange.startOffset >= range.startOffset) ||
          (!isdelKey &&
            ((startContainer.tagName !== 'IMG' &&
              compareRange.startOffset !== range.startOffset) ||
              (startContainer.tagName === 'IMG' &&
                nodeIndex !== range.startOffset)))
        ) {
          return null;
        }
      }
    ...
    if (startContainer && startContainer.nodeType === Node.TEXT_NODE) {
      if (isdelKey) {
        if (range.endOffset !== range.endContainer.textContent.length) {
          if (
            range.endOffset !== range.endContainer.textContent.trim().length
          ) {
            return null;
          }
        }
    ...
    if (
      ...
      startContainer.tagName === 'TABLE'
    ) {
      adjacentElement = startContainer;
    }
    if (adjacentElement) {
      const currentEleIndex: number =
        this.parent.formatter.editorManager.nodeSelection.getIndex(
          adjacentElement
        );
      if (
    ...
    if (!adjacentElement && startContainer) {
      adjacentElement = this.getAdjacentElementFromDom(
        startContainer,
        isBrEle,
        isdelKey
      );
    ...
    if (
      adjacentElement &&
      adjacentElement.nodeType === Node.ELEMENT_NODE &&
  ...
  private getAdjacentElementFromDom(
    ...
    let parentElement: HTMLElement = (
      isBrEle ? isBrEle : startContainer.parentNode
    ) as HTMLElement;
    ...
    while (parentElement && !adjacentElement && parentElement.parentNode) {
      ...
      if (
        ...
        ((isdelKey && startContainerIndex < childNodes.length - 1) ||
          (!isdelKey && startContainerIndex > 0))
      ) {
        adjacentElement = childNodes[
          isdelKey ? startContainerIndex + 1 : startContainerIndex - 1
        ] as HTMLElement;
      } else {
        adjacentElement = (
          isdelKey ? parentElement.nextSibling : parentElement.previousSibling
        ) as HTMLElement;
      ...
      if (
        ...
        !isNullOrUndefined(adjacentElement.textContent) &&
        !adjacentElement.textContent.trim()
      ) {
        currentElement = parentElement =
          adjacentElement.parentNode as HTMLElement;
        adjacentElement = null;
      }
      if (
        adjacentElement &&
        ...
        adjacentElement = this.getAdjacentElementFromList(
          adjacentElement,
          isdelKey
        );
        if (!adjacentElement) {
      ...
      if (
        ...
        parentElement.tagName.toUpperCase() === 'LI' &&
        !isdelKey
      ) {
    ...
    while (adjacentElement) {
      if (
        ...
        if (
          ...
          adjacentElement = (
            isdelKey ? adjacentElement.firstChild : adjacentElement.lastChild
          ) as HTMLElement;
        }
        break;
      }
  ...
  private getNodeCollection(range: Range): Node[] {
    ...
    if (
      ...
      nodes =
        this.parent.formatter.editorManager.nodeSelection.getNodeCollection(
          range
        );
    }
    return nodes;
  ...
  private setSelection(nextElement: HTMLElement, isBrEle: HTMLElement): void {
    if (!nextElement.classList.contains('e-cell-select')) {
      ...
      const fakeSelectionEle: HTMLElement = this.parent.createElement('div', {
  ...
  private removeAllFakeSelectionEles(): void {
    const fakeSelectionEles: NodeListOf<HTMLElement> = this.parent.contentModule
      .getEditPanel()
      .querySelectorAll('.e-table-fake-selection');
  ...
  private handleSelectAll(): void {
    ...
    const selectedCells: NodeListOf<Element> =
      this.parent.inputElement.querySelectorAll('.' + classes.CLS_TABLE_SEL);
  ...
  private tableModulekeyUp(e: NotifyArgs): void {
    if (
      ...
      if (
        ...
        !isNullOrUndefined(this.previousTableElement) &&
        !eventArgs.shiftKey &&
  ...
  private openDialog(isInternal?: boolean, e?: NotifyArgs): void {
    ...
    if (this.parent.editorMode === 'HTML') {
      ...
      const range: Range =
        this.parent.formatter.editorManager.nodeSelection.getRange(docElement);
      const selection: NodeSelection =
        this.parent.formatter.editorManager.nodeSelection.save(
          range,
          docElement
        );
  ...
  private onToolbarAction(args: ITableNotifyArgs): void {
    const item: IToolbarItemModel = (args.args as ClickEventArgs)
      .item as IToolbarItemModel;
    switch (item.subCommand) {
      case 'TableHeader':
  ...
  private verticalAlign(args: ITableNotifyArgs, e: ClickEventArgs): void {
    const tdEle: Element =
      closest(args.selectParent[0], 'th') ||
      closest(args.selectParent[0], 'td');
    if (tdEle) {
      this.parent.formatter.process(this.parent, e, e, {
        tableCell: tdEle,
        subCommand: (e.item as IDropDownItemModel).subCommand,
  ...
  private tableStyles(args: ITableNotifyArgs, command: string): void {
    const table: HTMLTableElement = closest(
      args.selectParent[0],
      'table'
    ) as HTMLTableElement;
    if (command === 'Dashed') {
      ...
      this.parent.element.classList.contains(classes.CLS_TB_DASH_BOR)
        ? removeClassWithAttr([this.parent.element], classes.CLS_TB_DASH_BOR)
        : this.parent.element.classList.add(classes.CLS_TB_DASH_BOR);
      table.classList.contains(classes.CLS_TB_DASH_BOR)
        ? removeClassWithAttr([this.parent.element], classes.CLS_TB_DASH_BOR)
    ...
    if (command === 'Alternate') {
      this.parent.element.classList.contains(classes.CLS_TB_ALT_BOR)
        ? removeClassWithAttr([this.parent.element], classes.CLS_TB_ALT_BOR)
        : this.parent.element.classList.add(classes.CLS_TB_ALT_BOR);
      table.classList.contains(classes.CLS_TB_ALT_BOR)
        ? removeClassWithAttr([table], classes.CLS_TB_ALT_BOR)
        : table.classList.add(classes.CLS_TB_ALT_BOR);
      /* eslint-enable */
    }
    if (
      (args.args as ClickEventArgs) &&
      (args.args as ClickEventArgs).item.cssClass
    ) {
  ...
  private insideList(range: Range): boolean {
    const blockNodes: Element[] = <Element[]>(
      (
        this.parent.formatter.editorManager as EditorManager
      ).domNode.blockNodes()
    );
    const nodes: Element[] = [];
    for (let i: number = 0; i < blockNodes.length; i++) {
      if ((blockNodes[i as number].parentNode as Element).tagName === 'LI') {
        ...
        blockNodes[i as number].tagName === 'LI' &&
        (blockNodes[i as number].childNodes[0] as Element).tagName !== 'P' &&
    ...
    if (
      nodes.length > 1 ||
      (nodes.length && range.startOffset === 0 && range.endOffset === 0)
    ) {
  ...
  private tabSelection(
    ...
    const allHeadBodyTRElements: NodeListOf<HTMLTableRowElement> = ele
      .closest('table')
      .querySelectorAll('thead, tbody, tr');
    ...
    if (
      ((event.keyCode === 37 || event.keyCode === 39) &&
        selection.range.startContainer.nodeType === 3) ||
      insideList
    ) {
      return;
    }
    ...
    if (!event.shiftKey && event.keyCode !== 37) {
      let nextElement: HTMLElement | Element | Node = !isNullOrUndefined(
        ele.nextSibling
      )
        ? ele.nextSibling
        : !isNullOrUndefined(closest(ele, 'tr').nextSibling)
        ? closest(ele, 'tr').nextSibling.childNodes[0]
        : !isNullOrUndefined(closest(ele, 'table').nextSibling)
        ? closest(ele, 'table').nextSibling.nodeName.toLowerCase() === 'td'
          ? closest(ele, 'table').nextSibling
          : ele
        : ele;
      if (ele === nextElement && ele.nodeName === 'TH') {
        nextElement = (closest(ele, 'table') as HTMLTableElement).rows[1]
          .cells[0];
      }
      ...
      if (ele === nextElement && event.keyCode !== 39 && nextElement) {
        ...
        nextElement = nextElement.parentElement.nextSibling
          ? (nextElement.parentElement.nextSibling.firstChild as HTMLElement)
      ...
      let prevElement: HTMLElement | Node = !isNullOrUndefined(
        ele.previousSibling
      )
        ? ele.previousSibling
        : !isNullOrUndefined(closest(ele, 'tr').previousSibling)
        ? closest(ele, 'tr').previousSibling.childNodes[
            closest(ele, 'tr').previousSibling.childNodes.length - 1
          ]
        : !isNullOrUndefined(closest(ele, 'table').previousSibling)
        ? closest(ele, 'table').previousSibling.nodeName.toLowerCase() === 'td'
          ? closest(ele, 'table').previousSibling
          : ele
        : ele;
      if (
        ele === prevElement &&
        (ele as HTMLTableDataCellElement).cellIndex === 0 &&
        (closest(ele, 'table') as HTMLTableElement).tHead
      ) {
        const clsTble: HTMLTableElement = closest(
          ele,
          'table'
        ) as HTMLTableElement;
  ...
  private tableArrowNavigation(
    ...
    if (
      ...
      (event.keyCode === 38 &&
        selText.nodeType === 3 &&
        ((selText.previousSibling &&
          selText.previousSibling.nodeName === 'BR') ||
          (selText.parentNode && selText.parentNode.nodeName !== 'TD')))
    ) {
      return;
    }
    ...
    if (event.keyCode === 40) {
      ele = !isNullOrUndefined(closest(ele, 'tr').nextElementSibling)
        ? ((closest(ele, 'tr').nextElementSibling as Element).children[
            (ele as HTMLTableDataCellElement).cellIndex
          ] as HTMLElement)
        : (closest(ele, 'table') as HTMLTableElement).tHead &&
          ele.nodeName === 'TH'
        ? (closest(ele, 'table') as HTMLTableElement).rows[1].cells[
            (ele as HTMLTableDataCellElement).cellIndex
          ]
        : !isNullOrUndefined(closest(ele, 'table').nextSibling)
        ? (closest(ele, 'table').nextSibling as HTMLElement)
        : (ele as HTMLElement);
    } else {
      ele = !isNullOrUndefined(closest(ele, 'tr').previousElementSibling)
        ? ((closest(ele, 'tr').previousElementSibling as Element).children[
            (ele as HTMLTableDataCellElement).cellIndex
          ] as HTMLElement)
        : (closest(ele, 'table') as HTMLTableElement).tHead &&
          ele.nodeName !== 'TH'
        ? (closest(ele, 'table') as HTMLTableElement).tHead.rows[0].cells[
            (ele as HTMLTableDataCellElement).cellIndex
          ]
        : !isNullOrUndefined(closest(ele, 'table').previousSibling)
        ? (closest(ele, 'table').previousSibling as HTMLElement)
        : (ele as HTMLElement);
    }
    if (ele) {
      selection.setSelectionText(
  ...
  private setBGColor(args: IColorPickerEventArgs): void {
    ...
    for (let i: number = 0; i < selectedCells.length; i++) {
      (selectedCells[i as number] as HTMLElement).style.backgroundColor =
        args.item.value;
    }
    this.parent.formatter.saveData();
    this.hideTableQuickToolbar();
  ...
  private tableHeader(
    selection: NodeSelection,
    e: ClickEventArgs | KeyboardEvent
  ): void {
    this.parent.formatter.process(
      ...
      {
        selection: selection,
        subCommand: ((e as ClickEventArgs).item as IDropDownItemModel)
          .subCommand,
      }
    );
  }
  ...
  private editAreaClickHandler(e: ITableNotifyArgs): void {
    if (
      ...
      !isNOU(
        closest((e.args as MouseEvent).target as Element, '.e-img-caption')
      )
    ) {
      return;
    }
    const args: MouseEvent = e.args as MouseEvent;
    const showOnRightClick: boolean =
      this.parent.quickToolbarSettings.showOnRightClick;
    if (
      args.which === 2 ||
      (showOnRightClick && args.which === 1) ||
      (!showOnRightClick && args.which === 3)
    ) {
      return;
    }
    if (
      ...
      const isPopupOpen: boolean =
        this.quickToolObj.tableQTBar.element.classList.contains('e-rte-pop');
      if (isPopupOpen) {
        return;
      }
      const range: Range =
      ...
      const startNode: HTMLElement =
        this.parent.getRange().startContainer.parentElement;
      const endNode: HTMLElement =
        this.parent.getRange().endContainer.parentElement;
      ...
      if (
        ...
        if (Browser.isDevice && (e.args as TouchEvent).touches) {
          pageX = this.parent.iframeSettings.enable
            ? window.pageXOffset +
              this.parent.element.getBoundingClientRect().left +
              (e.args as TouchEvent).changedTouches[0].clientX
            : (e.args as TouchEvent).changedTouches[0].pageX;
          pageY = this.parent.iframeSettings.enable
            ? window.pageYOffset +
              this.parent.element.getBoundingClientRect().top +
              (!this.parent.inlineMode.enable
                ? this.parent.toolbarModule.getToolbarHeight()
                : 0) +
              (e.args as TouchEvent).changedTouches[0].clientY
            : (e.args as TouchEvent).changedTouches[0].pageY;
        } else {
          pageX = this.parent.iframeSettings.enable
            ? window.pageXOffset +
              this.parent.element.getBoundingClientRect().left +
  ...
  private tableCellSelect(e?: MouseEvent): void {
    ...
    const row: number = Array.prototype.slice
      .call((target as HTMLElement).parentElement.parentElement.children)
      .indexOf((target as HTMLElement).parentElement);
    const col: number = Array.prototype.slice
      .call((target as HTMLElement).parentElement.children)
      .indexOf(target);
    const list: Element[] = <NodeListOf<Element> & Element[]>(
      this.dlgDiv.querySelectorAll('.e-rte-tablecell')
    );
    Array.prototype.forEach.call(list, (item: HTMLElement): void => {
      const parentIndex: number = Array.prototype.slice
        .call(item.parentElement.parentElement.children)
        .indexOf(item.parentElement);
      const cellIndex: number = Array.prototype.slice
  ...
  private tableCellClick(e: MouseEvent): void {
    ...
    const row: number =
      Array.prototype.slice
        .call((target as HTMLElement).parentElement.parentElement.children)
        .indexOf((target as HTMLElement).parentElement) + 1;
    const col: number =
      Array.prototype.slice
        .call((target as HTMLElement).parentElement.children)
        .indexOf(target) + 1;
    (this as ITableNotifyArgs).self.tableInsert(
      row,
      col,
      e,
  ...
  private tableInsert(
    ...
    if (
      proxy.parent.editorMode === 'HTML' &&
      ((proxy.parent.iframeSettings.enable &&
      ...
      const range: Range =
        proxy.parent.formatter.editorManager.nodeSelection.getRange(
          proxy.contentModule.getDocument()
        );
      selectionObj.selection =
        proxy.parent.formatter.editorManager.nodeSelection.save(
          range,
          proxy.contentModule.getDocument()
        );
    }
    ...
    if (proxy.popupObj) {
      ...
      for (let i: number = 0; i < rows.length; i++) {
        ...
        const cells: HTMLElement[] = Array.prototype.slice.call(
          rows[i as number].children
        );
    ...
    const selection: Selection =
      proxy.parent.formatter.editorManager.nodeSelection.get(
        proxy.contentModule.getDocument()
      );
    if (
      ...
      ((selection.anchorNode as HTMLElement).tagName === 'TD' ||
        (selection.anchorNode as HTMLElement).tagName === 'TH')
    ) {
      proxy.curTable = closest(
  ...
  private cellSelect(e: ITableNotifyArgs): void {
    let target: HTMLTableCellElement = (e.args as MouseEvent)
      .target as HTMLTableCellElement;
    const tdNode: Element = closest(target, 'td,th') as HTMLTableCellElement;
    target =
      target.nodeName !== 'TD' &&
      tdNode &&
      this.parent.contentModule.getEditPanel().contains(tdNode)
        ? (tdNode as HTMLTableCellElement)
        : target;
    if (
      !isNOU(this.activeCell) &&
      (e.args as MouseEvent).shiftKey &&
      !isNOU(target) &&
    ...
    if (target && (target.tagName === 'TD' || target.tagName === 'TH')) {
      ...
      this.curTable = this.curTable
        ? this.curTable
        : (closest(target, 'table') as HTMLTableElement);
      this.wireTableSelectionEvents();
      this.isTableMoveActive = true;
      this.removeResizeElement();
      if (
        this.helper &&
        this.contentModule.getEditPanel().contains(this.helper)
      ) {
        detach(this.helper);
      }
    }
  ...
  private removeCellSelectClasses(): void {
    removeClassWithAttr(
      this.contentModule.getEditPanel().querySelectorAll('table td, table th'),
      classes.CLS_TABLE_SEL_END
    );
    removeClassWithAttr(
      this.contentModule.getEditPanel().querySelectorAll('table td, table th'),
      classes.CLS_TABLE_MULTI_CELL
    );
    removeClassWithAttr(
      this.contentModule.getEditPanel().querySelectorAll('table td, table th'),
      classes.CLS_TABLE_SEL
    );
  }
  ...
  private resizeHelper(e: PointerEvent | TouchEvent): void {
    ...
    const target: HTMLElement =
      ...
      ((e as TouchEvent).targetTouches[0].target as HTMLElement);
    const closestTable: Element = closest(
      target,
      'table.e-rte-table, table.e-rte-paste-table'
    );
    ...
    if (
      ...
      this.curTable =
        ...
        (target.nodeName === 'TD' || target.nodeName === 'TH')
          ? (closestTable as HTMLTableElement)
  ...
  private tableResizeEleCreation(table: HTMLTableElement, e: MouseEvent): void {
    ...
    for (let i: number = 0; i < table.rows.length; i++) {
      for (let j: number = 0; j < table.rows[i as number].cells.length; j++) {
        if (
          !table.rows[i as number].cells[j as number].hasAttribute('rowspan')
        ) {
          rows.push(
            Array.prototype.slice.call(
              table.rows[i as number].cells,
              0,
              table.rows[i as number].cells.length
            )[j as number]
          );
          break;
        }
    ...
    for (let i: number = 0; columns.length >= i; i++) {
      ...
      colReEle.classList.add(
        classes.CLS_RTE_TABLE_RESIZE,
        classes.CLS_TB_COL_RES
      );
      if (columns.length === i) {
        colReEle.style.cssText =
          'height: ' +
          height +
          'px; width: 4px; top: ' +
          pos.top +
          'px; left:' +
          ((columns[i - 1].classList.contains('e-multi-cells-select')
            ? 0
            : pos.left) +
            this.calcPos(columns[i - 1] as HTMLElement).left +
            (columns[i - 1] as HTMLElement).offsetWidth -
            2) +
          'px;';
      } else {
        colReEle.style.cssText =
          ...
          pos.top +
          'px; left:' +
          ((columns[i as number].classList.contains('e-multi-cells-select')
            ? 0
            : pos.left) +
            this.calcPos(columns[i as number] as HTMLElement).left -
            2) +
          'px;';
      }
    ...
    for (let i: number = 0; rows.length > i; i++) {
      ...
      rowReEle.classList.add(
        classes.CLS_RTE_TABLE_RESIZE,
        classes.CLS_TB_ROW_RES
      );
      const rowPosLeft: number =
        !isNOU(table.getAttribute('cellspacing')) ||
        table.getAttribute('cellspacing') !== ''
          ? 0
          : this.calcPos(rows[i as number] as HTMLElement).left;
      rowReEle.style.cssText =
        'width: ' +
        width +
        'px; height: 4px; top: ' +
        (this.calcPos(rows[i as number] as HTMLElement).top +
          (rows[i as number].classList.contains('e-multi-cells-select')
            ? 0
            : pos.top) +
          (rows[i as number] as HTMLElement).offsetHeight -
          2) +
        'px; left:' +
        (rowPosLeft + pos.left) +
        'px;';
  ...
  public removeResizeElement(): void {
    const item: NodeListOf<Element> = this.parent.contentModule
      .getEditPanel()
  ...
  private calcPos(elem: HTMLElement): OffsetPosition {
    ...
    if (isNestedTable) {
      ...
      const topValue: number =
        this.parent.inputElement && this.parent.inputElement.scrollTop > 0
          ? this.parent.inputElement.scrollTop + offset.top - parentOffset.top
          : offset.top - parentOffset.top;
      const leftValue: number =
        this.parent.inputElement && this.parent.inputElement.scrollLeft > 0
          ? this.parent.inputElement.scrollLeft +
            offset.left -
            parentOffset.left
  ...
  private resizeStart(e: PointerEvent | TouchEvent): void {
    ...
    if (
      target.classList.contains(classes.CLS_TB_COL_RES) ||
      ...
      removeClassWithAttr(
        this.curTable.querySelectorAll('td,th'),
        classes.CLS_TABLE_SEL
      );
      this.removeTableSelection();
      ...
      if (target.classList.contains(classes.CLS_TB_COL_RES)) {
        ...
        if (
          ...
          if (parseInt(target.getAttribute('data-col'), 10) === 0) {
            ...
            const cellColl: HTMLCollectionOf<HTMLTableDataCellElement> =
              this.curTable.rows[0].cells;
            ...
            const rowSpanCells: Map<string, HTMLTableDataCellElement> =
              new Map();
            for (let i: number = 0; i < this.curTable.rows.length; i++) {
              let currentColIndex: number = 0;
              for (
                let k: number = 0;
                k < this.curTable.rows[i as number].cells.length;
                k++
              ) {
                for (
                  let l: number = 1;
                  l <
                  this.curTable.rows[i as number].cells[k as number].rowSpan;
                  l++
                ) {
                  const key: string = `${i + l}${currentColIndex}`;
                  rowSpanCells.set(
                    key,
                    this.curTable.rows[i as number].cells[k as number]
                  );
                }
                ...
                const width: number =
                  this.curTable.rows[i as number].cells[k as number]
                    .offsetWidth;
                if (
                  !sizes[currentColIndex as number] ||
                  width < sizes[currentColIndex as number]
                ) {
                  sizes[currentColIndex as number] = width;
                }
                currentColIndex +=
                  1 +
                  this.curTable.rows[i as number].cells[k as number].colSpan -
                  1;
              }
            }
            for (let size: number = 0; size < sizes.length; size++) {
              ...
              cell.style.width =
                this.convertPixelToPercentage(
                  sizes[size as number],
                  parseInt(getComputedStyle(this.curTable).width as string, 10)
                ) + '%';
      ...
      if (target.classList.contains(classes.CLS_TB_ROW_RES)) {
        this.rowEle = this.curTable.rows[
      ...
      if (
        Browser.isDevice &&
        this.helper &&
        !this.helper.classList.contains('e-reicon')
      ) {
      ...
      if (this.isResizeBind) {
        EventHandler.add(
          this.contentModule.getDocument(),
          Browser.touchMoveEvent,
          this.resizing,
          this
        );
        EventHandler.add(
          this.contentModule.getDocument(),
          Browser.touchEndEvent,
          this.resizeEnd,
          this
        );
  ...
  private setHelperHeight(): void {
    ...
    if (this.resizeBtnStat.column) {
      ...
      (this.helper as HTMLElement).style.cssText =
        'height: ' +
        getComputedStyle(this.curTable).height +
        '; top: ' +
        pos.top +
        'px; left:' +
        (pos.left +
      ...
      (this.helper as HTMLElement).style.cssText =
        'width: ' +
        getComputedStyle(this.curTable).width +
        '; top: ' +
        (this.calcPos(this.rowEle).top +
          pos.top +
          (this.rowEle as HTMLElement).offsetHeight -
          1) +
        'px; left:' +
        (this.calcPos(this.rowEle).left + pos.left) +
        'px;';
    }
  }
  ...
  private updateHelper(): void {
    ...
    if (this.resizeBtnStat.column) {
      const left: number =
        pos.left +
        this.calcPos(this.columnEle as HTMLElement).left +
        (this.currentColumnResize === 'last' ? this.columnEle.offsetWidth : 0) -
        1;
  ...
  private calMaxCol(curTable: HTMLTableElement): HTMLTableDataCellElement[] {
    ...
    for (let i: number = 0; i < curTable.rows.length; i++) {
      ...
      for (
        let k: number = 0;
        k < curTable.rows[i as number].cells.length;
        k++
      ) {
        for (
          let l: number = 1;
          l < curTable.rows[i as number].cells[k as number].rowSpan;
          l++
        ) {
          const key: string = `${i + l}${currentColIndex}`;
        ...
        const width: number =
          curTable.rows[i as number].cells[k as number].offsetWidth;
        if (
          !cells[currentColIndex as number] ||
          width < cells[currentColIndex as number].offsetWidth
        ) {
          cells[currentColIndex as number] =
            curTable.rows[i as number].cells[k as number];
        }
        currentColIndex +=
          1 + curTable.rows[i as number].cells[k as number].colSpan - 1;
      }
    }
    return cells;
  }
  ...
  private resizing(e: PointerEvent | TouchEvent): void {
    ...
    let mouseX: number = this.parent.enableRtl
      ? -(pageX - this.pageX)
      : pageX - this.pageX;
    const mouseY: number = this.parent.enableRtl
      ? -(pageY - this.pageY)
      : pageY - this.pageY;
    this.pageX = pageX;
    this.pageY = pageY;
    ...
    this.parent.trigger(events.onResize, args, (resizingArgs: ResizeArgs) => {
      if (resizingArgs.cancel) {
        ...
        const tableReBox: HTMLElement = this.contentModule
          .getEditPanel()
          .querySelector('.e-table-box') as HTMLElement;
        const tableWidth: number = parseInt(
          getComputedStyle(this.curTable).width as string,
          10
        );
        const tableHeight: number = parseInt(
          getComputedStyle(this.curTable).height as string,
          10
        );
        const paddingSize: number = +getComputedStyle(
          this.contentModule.getEditPanel()
        )
          .paddingRight.match(/\d/g)
          .join('');
        const rteWidth: number =
          (this.contentModule.getEditPanel() as HTMLElement).offsetWidth -
          ((this.contentModule.getEditPanel() as HTMLElement).offsetWidth -
            (this.contentModule.getEditPanel() as HTMLElement).clientWidth) -
          paddingSize * 2;
        let widthCompare: number;
        if (
          !isNOU(this.curTable.parentElement.closest('table')) &&
          !isNOU(this.curTable.closest('td')) &&
          (this.contentModule.getEditPanel() as HTMLElement).contains(
            this.curTable.closest('td')
          )
        ) {
          const currentTd: HTMLElement = this.curTable.closest('td');
          const currentTDPad: number = +getComputedStyle(currentTd)
            .paddingRight.match(/\d/g)
            .join('');
          // Padding of the current table with the parent element multiply with 2.
          widthCompare =
            currentTd.offsetWidth -
            (currentTd.offsetWidth - currentTd.clientWidth) -
            currentTDPad * 2;
        } else {
        ...
        if (this.resizeBtnStat.column) {
          ...
          const colGroup: NodeListOf<HTMLTableColElement> =
            this.curTable.querySelectorAll('colgroup > col');
          let currentTableWidth: number;
          if (
            this.curTable.style.width !== '' &&
            this.curTable.style.width.includes('%')
          ) {
            currentTableWidth = parseFloat(
              this.curTable.style.width.split('%')[0]
            );
          } else {
            currentTableWidth = this.getCurrentTableWidth(
              this.curTable.offsetWidth,
              this.parent.inputElement.offsetWidth
            );
          }
          const currentCol: HTMLTableDataCellElement = this.calMaxCol(
            this.curTable
          )[this.colIndex];
          const currentColResizableWidth: number = this.getCurrentColWidth(
            currentCol,
            tableWidth
          );
          if (this.currentColumnResize === 'first') {
            ...
            if (currentTdElement) {
              maxiumWidth =
                this.curTable.getBoundingClientRect().right -
                this.calcPos(currentTdElement).left;
              this.curTable.style.maxWidth = maxiumWidth + 'px';
            }
            // Below the value '100' is the 100% width of the parent element.
            if (
              ((mouseX !== 0 && 5 < currentColResizableWidth) || mouseX < 0) &&
              currentTableWidth <= 100 &&
              this.convertPixelToPercentage(
                tableWidth - mouseX,
                widthCompare
              ) <= 100
            ) {
              const firstColumnsCell: HTMLTableDataCellElement[] =
                this.findFirstLastColCells(this.curTable, true);
              this.curTable.style.width =
                this.convertPixelToPercentage(
                  tableWidth - mouseX,
                  widthCompare
              ...
              const widthType: boolean =
                this.curTable.style.width.indexOf('%') > -1;
              if (
                !widthType &&
                this.curTable.offsetWidth >
                  (this.contentModule.getEditPanel() as HTMLElement).offsetWidth
              ...
              if (
                widthType &&
                parseFloat(this.curTable.style.width.split('%')[0]) > 100
              ) {
                this.curTable.style.width = '100%';
                return;
              }
              if (
                !isNOU(this.curTable.style.marginLeft) &&
                this.curTable.style.marginLeft !== ''
              ) {
                const regex: RegExp = /[-+]?\d*\.\d+|\d+/;
                const value: RegExpMatchArray | null =
                  this.curTable.style.marginLeft.match(regex);
                if (!isNOU(value)) {
              ...
              if (
                !this.curTable.classList.contains('e-rte-paste-table') &&
                currentMarginLeft &&
                currentMarginLeft < 1
              ) {
              ...
              this.curTable.style.marginLeft =
                'calc(' +
                (this.curTable.style.width === '100%' ? 0 : currentMarginLeft) +
                '%)';
              for (let i: number = 0; i < firstColumnsCell.length; i++) {
                const currentColumnCellWidth: number = this.getCurrentColWidth(
                  firstColumnsCell[i as number],
                  tableWidth
                );
                (
                  firstColumnsCell[i as number] as HTMLTableDataCellElement
                ).style.width = currentColumnCellWidth - differenceWidth + '%';
              }
            }
            ...
            if (currentTdElement) {
              maxiumWidth =
                currentTdElement.getBoundingClientRect().right -
                this.curTable.getBoundingClientRect().left;
              this.curTable.style.maxWidth = maxiumWidth + 'px';
            }
            // Below the value '100' is the 100% width of the parent element.
            if (
              ...
              const lastColumnsCell: HTMLTableDataCellElement[] =
                this.findFirstLastColCells(this.curTable, false);
              this.curTable.style.width =
                this.convertPixelToPercentage(
                  tableWidth + mouseX,
                  widthCompare
              ...
              for (let i: number = 0; i < lastColumnsCell.length; i++) {
                const currentColumnCellWidth: number = this.getCurrentColWidth(
                  lastColumnsCell[i as number],
                  tableWidth
                );
                (
                  lastColumnsCell[i as number] as HTMLTableDataCellElement
                ).style.width = currentColumnCellWidth - differenceWidth + '%';
              }
            }
          } else {
            const actualwid: number =
              colGroup[this.colIndex].offsetWidth - mouseX;
            // eslint-disable-next-line
            const totalwid: number =
              (colGroup[this.colIndex] as HTMLTableColElement).offsetWidth +
              colGroup[this.colIndex - 1].offsetWidth;
            if (totalwid - actualwid > 20 && actualwid > 20) {
              const leftColumnWidth: number = totalwid - actualwid;
              const rightColWidth: number = actualwid;
              colGroup[this.colIndex - 1].style.width =
                this.convertPixelToPercentage(leftColumnWidth, tableWidth) +
                '%';
              colGroup[this.colIndex].style.width =
          ...
          const currentTableTrElement: NodeListOf<Element> =
            this.curTable.querySelectorAll('tr');
          for (let i: number = 0; i < currentTableTrElement.length; i++) {
            if (this.rowEle !== currentTableTrElement[i as number]) {
              tableTrElementPixel[i as number] = parseFloat(
                currentTableTrElement[i as number].clientHeight.toString()
              );
            }
          }
          this.curTable.style.height =
            parseFloat(this.curTable.clientHeight.toString()) +
            (mouseY > 0 ? 0 : mouseY) +
            'px';
          for (let i: number = 0; i < currentTableTrElement.length; i++) {
            if (this.rowEle === currentTableTrElement[i as number]) {
              (currentTableTrElement[i as number] as HTMLElement).style.height =
                parseFloat(
                  currentTableTrElement[i as number].clientHeight.toString()
                ) +
                mouseY +
              ...
              (currentTableTrElement[i as number] as HTMLElement).style.height =
                tableTrElementPixel[i as number] + 'px';
            }
          }
          if (!isNOU(tableReBox)) {
            tableReBox.style.cssText =
              'top: ' +
              (this.calcPos(this.curTable).top + tableHeight - 4) +
              'px; left:' +
              (this.calcPos(this.curTable).left + tableWidth - 4) +
              'px;';
          }
          this.updateHelper();
        } else if (this.resizeBtnStat.tableBox) {
          if (currentTdElement) {
            const tableBoxPosition: number =
              this.curTable.getBoundingClientRect().left -
              currentTdElement.getBoundingClientRect().left;
            maxiumWidth =
              Math.abs(
                tableBoxPosition -
                  currentTdElement.getBoundingClientRect().width
              ) - 5;
            this.curTable.style.maxWidth = maxiumWidth + 'px';
          }
  ...
  private getCurrentColWidth(
    ...
    if (col.style.width !== '' && col.style.width.includes('%')) {
      ...
      currentColWidth = this.convertPixelToPercentage(
        col.offsetWidth,
        tableWidth
      );
    }
  ...
  private findFirstLastColCells(
    ...
    for (let i: number = 0; i < rows.length; i++) {
      const cellIndex: number = isFirst
        ? 0
        : rows[i as number].cells.length - 1;
      const column: HTMLTableCellElement =
        rows[i as number].cells[cellIndex as number];
      ...
      if (
        rowSpanCellIndexs.length === 0 ||
        (isFirst && rowSpanCellIndexs.indexOf(spannedCellKey) === -1) ||
        (!isFirst &&
          rowSpanCellIndexs.indexOf(spannedCellKey) === -1 &&
          rowSpanCellIndexs.every(
            (key: string) => key.split('-')[0] !== i.toString()
          ))
      ) {
  ...
  private resizeEnd(e: PointerEvent | TouchEvent): void {
    ...
    if (
      this.contentModule.getEditPanel().querySelector('.e-table-box') &&
      this.contentModule
        .getEditPanel()
        .contains(
          this.contentModule.getEditPanel().querySelector('.e-table-box')
    ...
    for (let i: number = 0; i < currentTableTrElement.length; i++) {
      if (
        (currentTableTrElement[i as number] as HTMLElement).parentElement
          .nodeName === 'THEAD'
      ) {
        (
          currentTableTrElement[i as number] as HTMLElement
        ).parentElement.style.height = tableTrPercentage[i as number] + '%';
        (currentTableTrElement[i as number] as HTMLElement).style.height =
          tableTrPercentage[i as number] + '%';
      } else {
        (currentTableTrElement[i as number] as HTMLElement).style.height =
          tableTrPercentage[i as number] + '%';
      }
  ...
  private resetResizeHelper(curTable: HTMLTableElement): void {
    const colHelper: NodeListOf<Element> = this.parent.element.querySelectorAll(
    ...
    const rowHelper: NodeListOf<Element> = this.parent.element.querySelectorAll(
      ...
      for (let i: number = 0; i < curTable.rows.length; i++) {
        for (
          ...
          const width: string =
            this.convertPixelToPercentage(
              curTable.rows[i as number].cells[k as number].offsetWidth,
              parseInt(getComputedStyle(curTable).width, 10)
            ) + '%';
  ...
  private addRow(
    ...
    const value: ITableArgs = {
      selection: selectCell,
      subCommand: tabkey
        ? (cmd.item as ITableArgs).subCommand
        : ((e as ClickEventArgs).item as IDropDownItemModel).subCommand,
    };
  ...
  private removeTable(
    selection: NodeSelection,
    args?: ClickEventArgs | KeyboardEventArgs,
    delKey?: boolean
  ): void {
    let cmd: { [key: string]: object };
    if (delKey) {
    ...
    const value: ITableArgs = {
      ...
      subCommand: delKey
        ? (cmd.item as ITableArgs).subCommand
        : ((args as ClickEventArgs).item as IDropDownItemModel).subCommand,
    };
    this.parent.formatter.process(
      this.parent,
      delKey ? cmd : args,
      (args as ClickEventArgs).originalEvent,
      value
    );
    (this.contentModule.getEditPanel() as HTMLElement).focus();
    if (
      this.parent.inputElement.innerHTML === null ||
  ...
  private renderDlgContent(args?: ITableNotifyArgs): void {
    const argsTarget: HTMLElement = (args.args as ClickEventArgs).originalEvent
      .target as HTMLElement;
    if (
      Browser.isDevice ||
      this.parent.inlineMode.enable ||
      !isNullOrUndefined(closest(argsTarget, '.e-rte-text-popup'))
    ) {
    ...
    const tableDiv: HTMLElement = this.parent.createElement('div', {
    ...
    if (
      !isNOU(
        this.parent
          .getToolbarElement()
          .querySelector('.e-expended-nav') as HTMLElement
      )
    ) {
      (
        this.parent
    ...
    let target: HTMLElement = (
      (args as ITableNotifyArgs).args as ClickEventArgs
    ).originalEvent.target as HTMLElement;
    target = target.classList.contains('e-toolbar-item')
  ...
  private onIframeMouseDown(): void {
    ...
    if (
      !isNOU(this.parent) &&
      !isNOU(this.parent.contentModule) &&
      !isNOU(this.parent.contentModule.getEditPanel())
    ) {
      this.removeResizeElement();
    }
  }
  ...
  private docClick(e: { [key: string]: object }): void {
    ...
    if (
      ...
      ((this.popupObj &&
        !closest(
          target,
          '[id=' + "'" + this.popupObj.element.id + "'" + ']'
        )) ||
        (this.editdlgObj &&
          !closest(target, '#' + this.editdlgObj.element.id))) &&
      !target.classList.contains('e-create-table') &&
      target.offsetParent &&
      !target.offsetParent.classList.contains('e-rte-backgroundcolor-dropdown')
    ) {
      if (this.popupObj) {
        this.popupObj.hide();
      }
    ...
    const isExist: boolean =
      closestEle &&
      this.parent.contentModule.getEditPanel().contains(closestEle)
        ? true
        : false;
    if (
      target &&
      ...
      target.tagName !== 'TH' &&
      !isExist &&
      closest(target, '.e-rte-quick-popup') === null &&
      target.offsetParent &&
      ...
      !closest(target, '.e-rte-dropdown-popup') &&
      !closest(target, '.e-rte-elements')
    ) {
      const isToolbarClick: boolean =
        target && target.closest('.e-toolbar') ? true : false;
      if (!isToolbarClick) {
        this.removeCellSelectClasses();
      }
      this.removeTableSelection();
    ...
    for (let row: number = 0; row < 3; row++) {
      ...
      for (let col: number = 0; col < 10; col++) {
        tableCell = this.parent.createElement('div', {
          className:
            'e-rte-tablecell e-default' + this.parent.getCssClass(true),
          attrs: { 'data-cell': '' + col },
        });
  ...
  private insertTableDialog(args: MouseEvent | NotifyArgs): void {
    const proxy: Table = (this as ITableNotifyArgs).self
      ? (this as ITableNotifyArgs).self
      : this;
    if (proxy.popupObj) {
      proxy.popupObj.hide();
    }
  ...
  private tableCellDlgContent(): HTMLElement {
    ...
    const tableWrap: HTMLElement = this.parent.createElement('div', {
    ...
    this.columnTextBox.appendTo(
      tableWrap.querySelector('#tableColumn') as HTMLElement
    );
    ...
    this.rowTextBox.appendTo(
      tableWrap.querySelector('#tableRow') as HTMLElement
    );
    return tableWrap;
  }
  ...
  private createDialog(args: ITableArgs | ClickEventArgs | MouseEvent): void {
    ...
    if (
      ...
      this.parent.element.ownerDocument.body.contains(
        this.quickToolObj.textQTBar.element
      )
    ) {
  ...
  private customTable(args: ITableNotifyArgs, e: MouseEvent): void {
    ...
    if (
      proxy &&
      proxy.rowTextBox &&
      proxy.rowTextBox.value &&
      proxy.columnTextBox &&
      proxy.columnTextBox.value
    ) {
      const argument: ITableNotifyArgs =
        Browser.isDevice ||
        (!isNullOrUndefined(args.args as ClickEventArgs) &&
          !isNullOrUndefined((args.args as ClickEventArgs).originalEvent) &&
          ((args.args as ClickEventArgs).originalEvent as KeyboardEventArgs)
            .action === 'insert-table') ||
        proxy.parent.inlineMode.enable ||
        (!isNullOrUndefined(proxy.parent.quickToolbarSettings.text) &&
          !(args instanceof PointerEvent))
          ? args
          : (this as ITableNotifyArgs);
      proxy.tableInsert(
        proxy.rowTextBox.value,
  ...
  private applyProperties(args: ITableNotifyArgs, e: MouseEvent): void {
    ...
    const table: HTMLTableElement = closest(
      (args as ITableNotifyArgs).selectNode[0] as HTMLElement,
      'table'
    ) as HTMLTableElement;
    table.style.width = dialogEle.querySelector('.e-table-width')
      ? (dialogEle.querySelector('.e-table-width') as HTMLInputElement).value +
        'px'
      : table.style.width;
    if (
      dialogEle.querySelector('.e-cell-padding') &&
      (dialogEle.querySelector('.e-cell-padding') as HTMLInputElement).value !==
        ''
    ) {
      ...
      for (let i: number = 0; i < tdElm.length; i++) {
        ...
        if (tdElm[i as number].style.padding === '') {
          padVal =
            tdElm[i as number].getAttribute('style') +
            ' padding:' +
            (dialogEle.querySelector('.e-cell-padding') as HTMLInputElement)
              .value +
            'px;';
        } else {
          tdElm[i as number].style.padding =
            (dialogEle.querySelector('.e-cell-padding') as HTMLInputElement)
              .value + 'px';
    ...
    table.cellSpacing = dialogEle.querySelector('.e-cell-spacing')
      ? (dialogEle.querySelector('.e-cell-spacing') as HTMLInputElement).value
      : table.cellSpacing;
  ...
  private tableDlgContent(e: ITableNotifyArgs): HTMLElement {
    const selectNode: HTMLElement = (e as ITableNotifyArgs)
      .selectParent[0] as HTMLElement;
    ...
    const widthVal: string | number = closest(
      selectNode,
      'table'
    ).getClientRects()[0].width;
    const padVal: string | number = (closest(selectNode, 'td') as HTMLElement)
      .style.padding;
    const brdSpcVal: string | number = (
      closest(selectNode, 'table') as HTMLElement
    ).getAttribute('cellspacing');
    const content: string =
      '<div class="e-rte-field' +
      this.parent.getCssClass(true) +
      '"><input type="text" data-role ="none" id="tableWidth" class="e-table-width' +
      this.parent.getCssClass(true) +
      '" ' +
      ' /></div>' +
      '<div class="e-rte-field' +
      this.parent.getCssClass(true) +
      '"><input type="text" data-role ="none" id="cellPadding" class="e-cell-padding' +
      this.parent.getCssClass(true) +
      '" />' +
      ' </div><div class="e-rte-field' +
      this.parent.getCssClass(true) +
      '"><input type="text" data-role ="none" id="cellSpacing" class="e-cell-spacing' +
      this.parent.getCssClass(true) +
      '" /></div>';
    const contentElem: DocumentFragment = parseHtml(content);
    tableWrap.appendChild(contentElem);
    ...
    this.tableWidthNum.appendTo(
      tableWrap.querySelector('#tableWidth') as HTMLElement
    );
    ...
    this.tableCellPadding.appendTo(
      tableWrap.querySelector('#cellPadding') as HTMLElement
    );
    this.tableCellSpacing = new NumericTextBox({
      ...
      // eslint-disable-next-line
      value:
        brdSpcVal !== '' && !isNOU(brdSpcVal) ? parseInt(brdSpcVal, null) : 0,
    ...
    this.tableCellSpacing.appendTo(
      tableWrap.querySelector('#cellSpacing') as HTMLElement
    );
    return tableWrap;
  }
  ...
  private updateResizeIconPosition(): void {
    const tableReBox: HTMLElement = this.parent.contentModule
      .getEditPanel()
      .querySelector('.e-table-box');
    if (!isNOU(tableReBox)) {
      ...
      tableReBox.style.cssText =
        ...
        'px; left:' +
        (tablePosition.left +
          parseInt(getComputedStyle(this.curTable).width, 10) -
          4) +
        'px;';
    }
  }
