...
export class Video {
    ...
    if (!isNOU(this.deletedVid) && this.deletedVid.length > 0) {
      for (let i: number = 0; i < this.deletedVid.length; i++) {
        const args: AfterMediaDeleteEventArgs = {
          ...
          src:
            (this.deletedVid[i as number] as HTMLElement).tagName !== 'IFRAME'
              ? (this.deletedVid[i as number] as HTMLVideoElement)
  ...
  private videoSize(e: IImageNotifyArgs): void {
    if (
      e.selectNode[0].nodeName !== 'VIDEO' &&
      !this.isEmbedVidElem(e.selectNode[0] as HTMLElement)
    ) {
      return;
    }
    this.insertVideo(e);
  ...
  private vidsizeInput(e: IImageNotifyArgs): HTMLElement {
    const selectNode: HTMLVideoElement = (e as IImageNotifyArgs)
      .selectNode[0] as HTMLVideoElement;
    const vidHeight: string = this.i10n.getConstant('videoHeight');
    const vidWidth: string = this.i10n.getConstant('videoWidth');
    const vidSizeWrap: HTMLElement = this.parent.createElement('div', {
      className: 'e-video-sizewrap',
    });
    const widthVal: string =
      isNullOrUndefined(this.changedWidthValue) &&
    ...
    const heightVal: string =
      isNullOrUndefined(this.changedHeightValue) &&
    ...
    this.widthNum.appendTo(
      vidSizeWrap.querySelector('#vidwidth') as HTMLElement
    );
    ...
    this.heightNum.appendTo(
      vidSizeWrap.querySelector('#vidheight') as HTMLElement
    );
    return vidSizeWrap;
  }
  ...
  private insertSize(e: IImageNotifyArgs): void {
    ...
    const width: string = (
      dialogEle.querySelector('.e-vid-width') as HTMLInputElement
    ).value;
    const height: string = (
      dialogEle.parentElement.querySelector('.e-vid-height') as HTMLInputElement
    ).value;
    ...
    if (this.vidResizeDiv) {
      e.selectNode[0] =
        (e.selectNode[0] as HTMLElement).tagName === 'VIDEO' ||
        (e.selectNode[0] as HTMLElement).tagName === 'IFRAME'
          ? e.selectNode[0]
          : (e.selectNode[0] as HTMLElement).querySelector('iframe');
      proxy.vidResizePos(
        e.selectNode[0] as HTMLVideoElement,
        this.vidResizeDiv
      );
    }
  ...
  private resizeEnd(e: PointerEvent | TouchEvent): void {
    ...
    let pageY: number = this.parent.iframeSettings.enable
      ? window.pageYOffset +
  ...
  private resizeStart(e: PointerEvent | TouchEvent, ele?: Element): void {
    ...
    const target: HTMLElement = ele
      ? (ele as HTMLElement)
      : !this.isEmbedVidElem(e.target as HTMLElement)
      ? (e.target as HTMLElement)
      : (e.target as HTMLElement).querySelector('iframe');
    this.prevSelectedVidEle = this.videoEle;
    if (
      (target as HTMLElement).tagName === 'VIDEO' ||
  ...
  private vidResizePos(
    e: HTMLVideoElement | HTMLIFrameElement,
    ...
    const vidWid: string | number =
      e.width !== 0 && e.width !== 'auto' && e.width !== 'NaN'
        ? e.width
        : e.getBoundingClientRect().width;
    const vidHgt: string | number =
      e.height !== 0 && e.height !== 'auto' && e.height !== 'NaN'
        ? e.height
    ...
    (vidResizeDiv.querySelector('.e-rte-botLeft') as HTMLElement).style.left =
      left - borWid + 'px';
    (vidResizeDiv.querySelector('.e-rte-botLeft') as HTMLElement).style.top =
      parseInt(vidHgt.toString(), 10) - borWid + top + 'px';
    (vidResizeDiv.querySelector('.e-rte-botRight') as HTMLElement).style.left =
      parseInt(vidWid.toString(), 10) - (borWid - devWid) + left + 'px';
    (vidResizeDiv.querySelector('.e-rte-botRight') as HTMLElement).style.top =
      parseInt(vidHgt.toString(), 10) - borWid + top + 'px';
    (vidResizeDiv.querySelector('.e-rte-topRight') as HTMLElement).style.left =
      parseInt(vidWid.toString(), 10) - (borWid - devWid) + left + 'px';
    (vidResizeDiv.querySelector('.e-rte-topRight') as HTMLElement).style.top =
      top - borWid + 'px';
    (vidResizeDiv.querySelector('.e-rte-topLeft') as HTMLElement).style.left =
      left - borWid + 'px';
    (vidResizeDiv.querySelector('.e-rte-topLeft') as HTMLElement).style.top =
      top - borWid + 'px';
  }
  ...
  private calcPos(elem: HTMLElement): OffsetPosition {
    ...
    let offsetParent: Node =
      (elem.offsetParent &&
      (elem.offsetParent.classList.contains('e-video-clickelem') ||
  ...
  private setAspectRatio(
    vid: HTMLVideoElement | HTMLIFrameElement,
    expectedX: number,
    expectedY: number,
    e: ResizeArgs
  ): void {
    const vidEleStyle: CSSStyleDeclaration | HTMLIFrameElement =
      getComputedStyle(vid);
    ...
    let width: string | number =
      vidEleStyle.width !== ''
        ? vidEleStyle.width.match(new regExp('^\\d+(\\.\\d*)?%$', 'g'))
          ? parseFloat(vidEleStyle.width)
          : parseInt(vidEleStyle.width, 10)
        : vid.style.width !== ''
        ? vid.style.width
        : vid.width;
    let height: string | number =
      vidEleStyle.height !== ''
        ? parseInt(vidEleStyle.height, 10)
        : vid.style.height !== ''
        ? vid.style.height
        : vid.height;
    width = width.toString().match(new regExp('\\b\\d+(\\.\\d*)?(%|$)\\b', 'g'))
      ? parseFloat(width.toString())
      : parseInt(width.toString(), 10);
    height = height
      .toString()
      .match(new regExp('\\b\\d+(\\.\\d*)?(%|$)\\b', 'g'))
      ? parseFloat(height.toString())
      : parseInt(height.toString(), 10);
    /* eslint-enable */
    if (width > height) {
      vid.style.minWidth =
        this.parent.insertVideoSettings.minWidth === 0
          ? '200px'
          : formatUnit(this.parent.insertVideoSettings.minWidth);
      vid.style.minHeight =
        this.parent.insertVideoSettings.minHeight === 0
          ? '90px'
          : formatUnit(this.parent.insertVideoSettings.minHeight);
      if (this.parent.insertVideoSettings.resizeByPercent) {
        ...
        (vidEleStyle.width === '' && vidEleStyle.height !== '')
      ) {
        ...
        const currentWidth: number =
          (width / height) * expectedY + width / height <
        ...
        vid.setAttribute(
          'width',
          parseInt(
            ((width / height) * expectedY + width / height).toString(),
            10
          ).toString()
        );
      }
      ...
      if (this.parent.insertVideoSettings.resizeByPercent) {
        ...
        vid.setAttribute(
          ...
          this.resizeBtnStat.botRight
            ? (
                this.getPointX(e.event as PointerEvent) -
                vid.getBoundingClientRect().left
              ).toString()
            : expectedX.toString()
        );
      }
  ...
  private updateVidEleWidth(
    ...
    if (
      parseInt('' + vid.getBoundingClientRect().width + '', 10) !== 0 &&
      parseInt('' + width + '', 10) !== 0
    ) {
      const original: number = vid.offsetWidth + this.mouseX;
      const finalWidthByPerc: number =
        (original / vid.offsetWidth) *
        (parseFloat(vid.style.width).toString() === 'NaN'
          ? (vid.offsetWidth /
              parseFloat(getComputedStyle(this.parent.element).width)) *
            100
          : parseFloat(vid.style.width));
  ...
  private vidDupMouseMove(
    ...
    this.parent.trigger(events.onResize, args, (resizingArgs: ResizeArgs) => {
      if (resizingArgs.cancel) {
        ...
        if (
          parseInt(this.parent.insertVideoSettings.minWidth as string, 10) >=
            parseInt(width, 10) ||
          (parseInt(this.parent.getInsertVidMaxWidth() as string, 10) <=
            parseInt(width, 10) &&
        ...
        if (
          !this.parent.insertVideoSettings.resizeByPercent &&
          (parseInt(this.parent.insertVideoSettings.minHeight as string, 10) >=
            parseInt(height, 10) ||
            parseInt(this.parent.insertVideoSettings.maxHeight as string, 10) <=
              parseInt(height, 10))
        ) {
          return;
        }
  ...
  private resizing(e: PointerEvent | TouchEvent): void {
    ...
    const mouseX: number =
      this.resizeBtnStat.botLeft || this.resizeBtnStat.topLeft
        ? -(pageX - this.pageX)
        : pageX - this.pageX;
    const mouseY: number =
      this.resizeBtnStat.topLeft || this.resizeBtnStat.topRight
        ? -(pageY - this.pageY)
        : pageY - this.pageY;
    const width: number = parseInt(this.vidDupPos.width as string, 10) + mouseX;
    const height: number =
      parseInt(this.vidDupPos.height as string, 10) + mouseY;
    this.pageX = pageX;
    this.pageY = pageY;
    this.mouseX = mouseX;
  ...
  private resizeVidDupPos(e: HTMLVideoElement | HTMLIFrameElement): void {
    this.vidDupPos = {
      width:
        e.style.width !== '' &&
        this.parent.insertVideoSettings &&
        !this.parent.insertVideoSettings.resizeByPercent
          ? this.videoEle.style.width
          : e.width !== 'auto' && e.width !== 0 && e.width !== 'NaN'
          ? e.width + 'px'
          : parseInt(getComputedStyle(e).width, 10) + 'px',
      height:
        e.style.height !== ''
          ? this.videoEle.style.height
          : e.height !== 'auto' && e.height !== 0 && e.height !== 'NaN'
          ? e.height + 'px'
          : parseInt(getComputedStyle(e).height, 10) + 'px',
    };
  }
  ...
  private onKeyDown(event: NotifyArgs): void {
    ...
    if (
      ...
      for (let i: number = 0; i < nodes.length; i++) {
        if (
          nodes[i as number].nodeName === 'VIDEO' ||
          nodes[i as number].nodeName === 'IFRAME'
        ) {
    ...
    if (
      ...
      if (
        ...
        ((!isNOU(selectParentEle[0]) &&
          (selectParentEle[0] as HTMLElement).tagName === 'VIDEO') ||
          this.isEmbedVidElem(selectParentEle[0] as HTMLElement)) &&
        (selectParentEle[0] as HTMLElement).parentElement
      ) {
        const prev: Node = (
          (selectParentEle[0] as HTMLElement).parentElement as HTMLElement
        ).childNodes[0];
    ...
    if (originalEvent.keyCode === 8 || originalEvent.keyCode === 46) {
      if (
        selectNodeEle &&
        (selectNodeEle[0].nodeName === 'VIDEO' ||
      ...
      if (
        this.parent.contentModule.getEditPanel().querySelector('.e-vid-resize')
      ) {
        this.removeResizeEle();
      }
    }
    ...
    switch (originalEvent.action) {
      ...
      case 'delete':
        for (let i: number = 0; i < this.deletedVid.length; i++) {
          const src: string = (this.deletedVid[i as number] as HTMLVideoElement)
            .src;
          ...
          if (range.startContainer.nodeType === 3) {
            if (originalEvent.code === 'Backspace') {
              ...
              if (
                ...
                (
                  range.startContainer as HTMLElement
                ).nextElementSibling.classList.contains(classes.CLS_VIDEOWRAP)
              ) {
                detach(
                  (range.startContainer as HTMLElement).nextElementSibling
                );
            ...
            (range.startContainer as HTMLElement).classList &&
  ...
  private handleSelectAll(): void {
    this.cancelResizeAction();
    const videoFocusNodes: NodeList = this.parent.inputElement.querySelectorAll(
  ...
  private checkVideoBack(range: Range): void {
    if (
      ...
      (range.startContainer.previousSibling.nodeName === 'VIDEO' ||
        this.isEmbedVidElem(
          range.startContainer.previousSibling as HTMLElement
        ))
    ) {
  ...
  private checkVideoDel(range: Range): void {
    if (
      ...
      (range.startContainer.nextSibling.nodeName === 'VIDEO' ||
      ...
      (range.startContainer.childNodes[range.startOffset].nodeName ===
        'VIDEO' ||
        this.isEmbedVidElem(
  ...
  private alignmentSelect(e: ClickEventArgs): void {
    ...
    selectNodeEle =
      selectNodeEle[0].nodeName === 'VIDEO' ? selectNodeEle : [this.videoEle];
    ...
    if (
      this.quickToolObj &&
      document.body.contains(this.quickToolObj.videoQTBar.element)
    ) {
      this.quickToolObj.videoQTBar.hidePopup();
  ...
  private deleteVideo(e: IImageNotifyArgs, keyCode?: number): void {
    ...
    if (this.isEmbedVidElem(e.selectNode[0] as HTMLElement)) {
      e.selectNode[0] = (e.selectNode[0] as HTMLElement).classList.contains(
    ...
    const args: AfterMediaDeleteEventArgs = {
      element: !this.isEmbedVidElem(e.selectNode[0] as HTMLElement)
        ? (e.selectNode[0] as HTMLElement).querySelector('iframe')
        : (e.selectNode[0] as HTMLElement),
      src: !this.isEmbedVidElem(e.selectNode[0] as HTMLElement)
        ? (e.selectNode[0] as HTMLElement)
            .querySelector('source')
            .getAttribute('src')
        : (e.selectNode[0] as HTMLElement)
    ...
    this.parent.formatter.process(
      this.parent,
      e.args,
      (e.args as ClickEventArgs).originalEvent,
      {
        selectNode: e.selectNode,
        subCommand: ((e.args as ClickEventArgs).item as IDropDownItemModel)
          .subCommand,
  ...
  private videoRemovePost(src: string): void {
    ...
    if (
      isNOU(this.parent.insertVideoSettings.removeUrl) ||
      this.parent.insertVideoSettings.removeUrl === ''
    ) {
      return;
    }
  ...
  private onDocumentClick(e: MouseEvent): void {
    ...
    if (target.nodeName === 'VIDEO' || this.isEmbedVidElem(target)) {
      this.videoEle = !this.isEmbedVidElem(target)
    ...
    if (
      ...
      /* eslint-disable */
      if (
        e.offsetX > (e.target as HTMLVideoElement).clientWidth ||
        e.offsetY > (e.target as HTMLVideoElement).clientHeight
      ) {
      } else {
    ...
    if (this.contentModule.getEditPanel().querySelector('.e-vid-resize')) {
      ...
      if (
        ...
        !this.isEmbedVidElem(target) &&
        !isNOU(this.videoEle)
      ) {
        this.videoEle.style.outline = '';
      } else if (
        !isNOU(this.prevSelectedVidEle) &&
        this.prevSelectedVidEle !==
          (target.tagName === 'IFRAME' || target.tagName === 'VIDEO'
            ? target
            : target.querySelector('iframe'))
      ) {
  ...
  private onWindowResize(): void {
    if (
      ...
      !isNOU(this.contentModule.getEditPanel().querySelector('.e-vid-resize'))
    ) {
      this.cancelResizeAction();
    }
  }
  ...
  private editAreaClickHandler(e: IImageNotifyArgs): void {
    ...
    if (
      ...
      if (
        showOnRightClick &&
        args.which === 1 &&
        !isNullOrUndefined(args.target as HTMLElement) &&
        ((args.target as HTMLElement).tagName === 'VIDEO' ||
          this.isEmbedVidElem(args.target as HTMLElement))
      ) {
    ...
    if (
      ...
      this.parent.quickToolbarModule.videoQTBar
    ) {
      this.quickToolObj = this.parent.quickToolbarModule;
      const target: HTMLElement = args.target as HTMLElement;
      this.contentModule = this.rendererFactory.getRenderer(RenderType.Content);
      const isPopupOpen: boolean =
        this.quickToolObj.videoQTBar.element.classList.contains('e-rte-pop');
      if (
        (target.nodeName === 'VIDEO' || this.isEmbedVidElem(target)) &&
        this.parent.quickToolbarModule
      ) {
        if (isPopupOpen) {
          return;
  ...
  private showVideoQuickToolbar(e: IShowPopupArgs): void {
    if (
      e.type !== 'Videos' ||
      (!isNullOrUndefined(e.args) && e.args.detail === 2) ||
      isNullOrUndefined(this.parent.quickToolbarModule) ||
      isNullOrUndefined(this.parent.quickToolbarModule.videoQTBar) ||
      isNullOrUndefined(e.args)
    ) {
      return;
    }
    this.quickToolObj = this.parent.quickToolbarModule;
    ...
    if (target.tagName === 'VIDEO' || this.isEmbedVidElem(target)) {
      addClass(
        [
          !this.isEmbedVidElem(target) || target.tagName === 'IFRAME'
            ? target
            : target.querySelector('iframe'),
        ],
        [classes.CLS_VID_FOCUS]
    ...
    if (this.parent.quickToolbarModule.videoQTBar) {
      if (e.isNotify) {
        ...
        this.quickToolObj.videoQTBar.showPopup(
          args.pageX - 50,
          pageY + target.getBoundingClientRect().height / 2 - target.offsetTop,
          target as Element
        );
      }
    }
  ...
  public hideVideoQuickToolbar(): void {
    if (
      !isNullOrUndefined(
        this.contentModule
          .getEditPanel()
          .querySelector('.' + classes.CLS_VID_FOCUS)
      )
    ) {
      removeClass(
        [
      ...
      if (
        ...
        detach(
          this.contentModule.getEditPanel().querySelector('.e-vid-resize')
        );
      }
      if (
        this.quickToolObj &&
        this.quickToolObj.videoQTBar &&
        document.body.contains(this.quickToolObj.videoQTBar.element)
      ) {
        this.quickToolObj.videoQTBar.hidePopup();
      }
  ...
  private insertingVideo(e: IImageNotifyArgs): void {
    ...
    if (!isNullOrUndefined(this.dialogObj)) {
      ...
      const dialogContent: HTMLElement =
        this.dialogObj.element.querySelector('.e-video-content');
      if (
        !isNullOrUndefined(this.parent.insertVideoSettings.path) ||
        this.parent.editorMode === 'HTML'
      ) {
        (
          document.getElementById(this.rteID + '_insertVideo') as HTMLElement
        ).focus();
      } else {
  ...
  public insertVideo(e: IImageNotifyArgs): void {
    ...
    const videoDialog: HTMLElement = this.parent.createElement('div', {
    ...
    const dialogModel: DialogModel = {
      ...
      close: (event: { [key: string]: object }) => {
        ...
        if (
          event &&
          !isNOU(event.event) &&
          (event.event as { [key: string]: string }).returnValue
        ) {
          if (this.parent.editorMode === 'HTML') {
            selection.restore();
          }
    ...
    const embedLinkHeader: HTMLElement = this.parent.createElement('div', {
      className: 'e-embed-videoheader',
    });
    const linkHeaderText: string = this.i10n.getConstant('videoLinkHeader');
    const embedLinkHeaderText: string = this.i10n.getConstant(
      'embedVideoLinkHeader'
    );
    if (this.parent.editorMode === 'HTML') {
    ...
    if (
      ...
      this.dialogObj.element.querySelector('.e-insertVideo').textContent =
    ...
    if (this.quickToolObj) {
      if (
        ...
        if (!isNullOrUndefined(e.selectParent as Node[])) {
          removeClass(
            [e.selectParent[0] as HTMLElement],
            classes.CLS_VID_FOCUS
          );
        }
      }
  ...
  private urlPopup(e: IImageNotifyArgs): HTMLElement {
    const videoUrl: HTMLElement = this.parent.createElement('div', {
      className: 'e-video-url-wrap',
    });
    const urlContent: HTMLElement = this.parent.createElement('div', {
      id: 'urlcontent',
    });
    const placeUrl: string = this.i10n.getConstant('videoUrl');
    const content: string =
      '<input id="embedURL" type="radio">' +
      '<input id="webURL" type="radio" >';
    const contentElem: DocumentFragment = parseHtml(content);
    ...
    if (
      e.selectNode &&
      (e.selectNode[0] as HTMLElement) &&
      ((e.selectNode[0] as HTMLElement).nodeName === 'VIDEO' ||
        this.isEmbedVidElem(e.selectNode[0] as HTMLElement))
    ) {
      if ((e.selectNode[0] as HTMLElement).nodeName === 'VIDEO') {
        ...
        (this.inputUrl as HTMLInputElement).value =
          sourceElement && sourceElement.src && sourceElement.src.match(regex)
            ? sourceElement.src
            : '';
      } else {
        (this.embedInputUrl as HTMLInputElement).value =
          (e.selectNode[0] as HTMLElement).nodeName === 'IFRAME'
            ? (e.selectNode[0] as HTMLElement).outerHTML
            : (e.selectNode[0] as HTMLElement).querySelector('iframe')
                .outerHTML;
      }
    }
    const isWebUrl: boolean = (this.inputUrl as HTMLInputElement).value
      ? true
      : false;
    ...
    this.embedUrlBtn.appendTo(
      videoUrl.querySelector('#embedURL') as HTMLElement
    );
  ...
  private videoUpload(e: IImageNotifyArgs): HTMLElement {
    ...
    const deviceVideoUpMsg: string = this.i10n.getConstant(
      'videoDeviceUploadMessage'
    );
    ...
    this.uploadObj = new Uploader({
      ...
      enableRtl: this.parent.enableRtl,
      allowedExtensions:
        this.parent.insertVideoSettings.allowedTypes.toString(),
            ...
            if (!selectArgs.cancel) {
              ...
              if (
                ...
                reader.addEventListener('load', (e: MouseEvent) => {
                  const url: string =
                    this.parent.insertVideoSettings.saveFormat === 'Base64'
                  ...
                  if (
                    isNullOrUndefined(
                      proxy.parent.insertVideoSettings.saveUrl
                    ) &&
                    this.isAllowedTypes &&
                    !isNullOrUndefined(this.dialogObj)
                  ) {
      ...
      success: (e: Object) => {
        this.parent.trigger(events.fileUploadSuccess, e, (e: object) => {
          if (!isNullOrUndefined(this.parent.insertVideoSettings.path)) {
            const url: string =
              this.parent.insertVideoSettings.path + (e as MetaData).file.name;
            // eslint-disable-next-line
  ...
  private insertVideoUrl(e: MouseEvent): void {
    ...
    if (!isNullOrUndefined(proxy.uploadUrl) && proxy.uploadUrl.url !== '') {
      proxy.uploadUrl.cssClass =
        proxy.parent.insertVideoSettings.layoutOption === 'Inline'
          ? classes.CLS_VIDEOINLINE
          : classes.CLS_VIDEOBREAK;
      ...
      const webUrlBtn: HTMLInputElement = document.getElementById(
        'webURL'
      ) as HTMLInputElement;
      const name: string = webUrlBtn.checked
