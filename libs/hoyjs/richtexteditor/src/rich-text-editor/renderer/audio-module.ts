...
export class Audio {
  ...
  private checkAudioBack(range: Range): void {
    if (
      ...
      this.isAudioElem(range.startContainer.previousSibling as HTMLElement)
    ) {
  ...
  private checkAudioDel(range: Range): void {
    if (
      ...
      range.startContainer.nextSibling.nodeName === 'AUDIO'
    ) {
    ...
    if (!isNOU(this.deletedAudio) && this.deletedAudio.length > 0) {
      for (let i: number = 0; i < this.deletedAudio.length; i++) {
        const args: AfterMediaDeleteEventArgs = {
          element: (
            this.deletedAudio[i as number] as HTMLElement
          ).querySelector('audio'),
          src: (this.deletedAudio[i as number] as HTMLElement)
  ...
  private onKeyDown(event: NotifyArgs): void {
    ...
    if (
      ...
      if (
        ...
        this.isAudioElem(selectParentEle[0] as HTMLElement) &&
    ...
    if (originalEvent.keyCode === 8 || originalEvent.keyCode === 46) {
      if (
        selectNodeEle &&
        this.isAudioElem(selectNodeEle[0] as HTMLElement) &&
    ...
    switch (originalEvent.action) {
      ...
      case 'delete':
        for (let i: number = 0; i < this.deletedAudio.length; i++) {
          const src: string = (
            this.deletedAudio[i as number] as HTMLAudioElement
          ).src;
          ...
          if (range.startContainer.nodeType === 3) {
            if (originalEvent.code === 'Backspace') {
              ...
              if (
                ...
                (
                  range.startContainer as HTMLElement
                ).nextElementSibling.classList.contains(classes.CLS_AUDIOWRAP)
              ) {
                detach(
                  (range.startContainer as HTMLElement).nextElementSibling
                );
  ...
  private handleSelectAll(): void {
    const audioFocusNodes: NodeListOf<Element> =
      this.parent.inputElement.querySelectorAll('.' + classes.CLS_AUD_FOCUS);
  ...
  private deleteAudio(e: IImageNotifyArgs, keyCode?: number): void {
    ...
    const args: AfterMediaDeleteEventArgs = {
      ...
      src: (e.selectNode[0] as HTMLElement)
        .querySelector('source')
        .getAttribute('src'),
    };
    ...
    if (
      this.quickToolObj &&
      document.body.contains(this.quickToolObj.audioQTBar.element)
    ) {
      this.quickToolObj.audioQTBar.hidePopup();
    }
  ...
  private audioRemovePost(src: string): void {
    ...
    if (
      isNOU(this.parent.insertAudioSettings.removeUrl) ||
      this.parent.insertAudioSettings.removeUrl === ''
    ) {
      return;
    }
  ...
  private audioClick(e: MouseEvent): void {
    ...
    if (this.isAudioElem(e.target as HTMLElement)) {
      this.audEle = (e.target as HTMLElement).querySelector(
        'audio'
      ) as HTMLAudioElement;
      e.preventDefault();
    }
  ...
  private onDocumentClick(e: MouseEvent): void {
    ...
    if (this.isAudioElem(target)) {
      this.audEle = (target as HTMLElement).querySelector(
        'audio'
      ) as HTMLAudioElement;
    }
    if (
    ...
    if (
      this.contentModule
        .getEditPanel()
        .querySelector('.' + classes.CLS_AUD_FOCUS)
    ) {
      if (!this.isAudioElem(e.target as HTMLElement) && !isNOU(this.audEle)) {
        this.audEle.style.outline = '';
      } else if (
        !isNOU(this.prevSelectedAudEle) &&
        this.prevSelectedAudEle !== target
      ) {
  ...
  private editAreaClickHandler(e: IImageNotifyArgs): void {
    ...
    if (
      ...
      this.parent.quickToolbarModule.audioQTBar
    ) {
      this.quickToolObj = this.parent.quickToolbarModule;
      const target: HTMLElement = args.target as HTMLElement;
      this.contentModule = this.rendererFactory.getRenderer(RenderType.Content);
      const isPopupOpen: boolean =
        this.quickToolObj.audioQTBar.element.classList.contains('e-rte-pop');
  ...
  private isAudioElem(target: HTMLElement): boolean {
    if (
      ...
      target.nodeName !== 'BR' &&
      target.classList &&
      (target.classList.contains(classes.CLS_AUDIOWRAP) ||
  ...
  private showAudioQuickToolbar(e: IShowPopupArgs): void {
    if (
      ...
      isNullOrUndefined(this.parent.quickToolbarModule.audioQTBar) ||
    ...
    if (this.parent.quickToolbarModule.audioQTBar) {
      if (e.isNotify) {
        this.showPopupTime = setTimeout(() => {
          ...
          this.quickToolObj.audioQTBar.showPopup(
            args.pageX - 50,
            target.getBoundingClientRect().top + 34,
            target as Element
          );
        }, 400);
      } else {
  ...
  private insertingAudio(e: IImageNotifyArgs): void {
    ...
    if (!isNullOrUndefined(this.dialogObj)) {
      ...
      const dialogContent: HTMLElement =
        this.dialogObj.element.querySelector('.e-audio-content');
      if (
        !isNullOrUndefined(this.parent.insertAudioSettings.path) ||
        this.parent.editorMode === 'HTML'
      ) {
        (
          document.getElementById(this.rteID + '_insertAudio') as HTMLElement
        ).focus();
      } else {
  ...
  public insertAudio(e: IImageNotifyArgs): void {
    ...
    const audioDialog: HTMLElement = this.parent.createElement('div', {
    ...
    if (
      ...
      this.isAudioElem(e.selectNode[0] as HTMLElement)
    ) {
    ...
    if (
      ...
      this.dialogObj.element.querySelector('.e-insertAudio').textContent =
  ...
  private audioUrlPopup(e: IImageNotifyArgs): HTMLElement {
    const audioUrl: HTMLElement = this.parent.createElement('div', {
      className: 'audioUrl',
    });
    ...
    if (e.selectNode && this.isAudioElem(e.selectNode[0] as HTMLElement)) {
      ...
      (this.inputUrl as HTMLInputElement).value = sourceElement.src.match(regex)
        ? sourceElement.src
        : '';
    }
  ...
  private audioUpload(e: IImageNotifyArgs): HTMLElement {
    ...
    const deviceAudioUpMsg: string = this.i10n.getConstant(
      'audioDeviceUploadMessage'
    );
    ...
    this.uploadObj = new Uploader({
      ...
      enableRtl: this.parent.enableRtl,
      allowedExtensions:
        this.parent.insertAudioSettings.allowedTypes.toString(),
            ...
            if (!selectArgs.cancel) {
              ...
              if (
                ...
                reader.addEventListener('load', (e: MouseEvent) => {
                  const url: string =
                    this.parent.insertAudioSettings.saveFormat === 'Base64'
                  ...
                  if (
                    isNullOrUndefined(
                      proxy.parent.insertAudioSettings.saveUrl
                    ) &&
                    this.isAllowedTypes &&
                    !isNullOrUndefined(this.dialogObj)
                  ) {
      ...
      success: (e: Object) => {
        this.parent.trigger(events.fileUploadSuccess, e, (e: object) => {
          if (!isNullOrUndefined(this.parent.insertAudioSettings.path)) {
            const url: string =
              this.parent.insertAudioSettings.path + (e as MetaData).file.name;
            // eslint-disable-next-line
  ...
  private insertAudioUrl(e: MouseEvent): void {
    ...
    if (!isNullOrUndefined(proxy.uploadUrl) && proxy.uploadUrl.url !== '') {
      proxy.uploadUrl.cssClass =
        proxy.parent.insertAudioSettings.layoutOption === 'Inline'
          ? classes.CLS_AUDIOINLINE
          : classes.CLS_AUDIOBREAK;
