...
export class ViewSource {
    ...
    switch (event.action) {
      ...
      case 'toolbar-focus':
        if (
          ...
          const firstActiveItem: HTMLElement = this.parent
            .getToolbarElement()
            .querySelector('.e-toolbar-item:not(.e-overlay)[title]');
          (firstActiveItem.firstElementChild as HTMLElement).removeAttribute(
            'tabindex'
          );
    ...
    this.parent.trigger(
      ...
      (actionBeginArgs: ActionBeginEventArgs) => {
        if (!actionBeginArgs.cancel) {
          ...
          this.contentModule = this.rendererFactory.getRenderer(
            RenderType.Content
          );
          const height: number =
            this.parent.inputElement.getBoundingClientRect().height;
          ...
          if (
            isNOU(
              this.parent.element.querySelector(
                '#' + this.parent.getID() + '_source-view'
              )
            )
          ) {
            rteContent = this.parent.createElement('div', {
              className: 'e-source-content',
              id: this.parent.getID() + '_source-view',
              attrs: { style: 'height:' + height + 'px' },
            });
          } else {
            rteContent = this.parent.element.querySelector(
              '#' + this.parent.getID() + '_source-view'
            ) as HTMLElement;
          ...
          (this.getPanel() as HTMLTextAreaElement).value =
          ...
          if (
            !isNOU(this.parent.saveInterval) &&
            this.parent.saveInterval > 0 &&
            this.parent.autoSaveOnIdle
          ) {
            this.codeViewTimeInterval = setInterval(() => {
              this.parent.notify(events.updateValueOnIdle, {});
    ...
    this.parent.trigger(
      ...
      (actionBeginArgs: ActionBeginEventArgs) => {
        if (!actionBeginArgs.cancel) {
          ...
          const editHTML: HTMLTextAreaElement =
            this.getPanel() as HTMLTextAreaElement;
          this.parent.notify(events.updateToolbarItem, {
            targetItem: 'Preview',
            updateItem: 'SourceCode',
            baseToolbar: this.parent.getBaseToolbarObject(),
          });
          const serializeValue: string = this.parent.serializeValue(
            editHTML.value
          );
          let value: string;
  ...
  private getTextAreaValue(): string {
    return this.contentModule.getEditPanel().innerHTML === '<p><br></p>' ||
      ...
      (this.contentModule.getEditPanel().childNodes.length === 1 &&
        (this.contentModule.getEditPanel().childNodes[0] as HTMLElement)
          .tagName === 'P' &&
        this.contentModule.getEditPanel().innerHTML.length === 7) ||
      ((this.contentModule.getEditPanel().childNodes[0] as HTMLElement)
        .tagName === 'DIV' &&
        this.contentModule.getEditPanel().innerHTML.length === 11)
      ? ''
      : this.parent.value;
  }
  ...
  public getPanel(): HTMLTextAreaElement {
    return (
      this.parent.element &&
      this.parent.element.querySelector('.e-rte-srctextarea')
    );
  }
