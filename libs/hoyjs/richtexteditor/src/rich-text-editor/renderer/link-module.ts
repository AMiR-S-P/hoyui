...
import { closest, addClass, removeClass, Browser } from '@hoyui/base';
import {
  IRichTextEditor,
  NotifyArgs,
  IRenderer,
...
export class Link {
  ...
  private showLinkQuickToolbar(e: IShowPopupArgs): void {
    if (
      ...
      (e.args as KeyboardEventArgs).action !== 'enter' &&
      ...
      [].forEach.call(e.elements, (element: Element, index: number) => {
        if (index === 0) {
          target = (
            element.nodeName === '#text' ? element.parentNode : element
          ) as HTMLElement;
        }
      });
      if (e.isNotify) {
        const tbElement: HTMLElement =
          this.parent.toolbarModule.getToolbarElement() as HTMLElement;
        ...
        const tbHeight: number = tbElement
          ? tbElement.offsetHeight +
        ...
        pageY =
          ...
          (this.parent.iframeSettings.enable
            ? parentTop + tbHeight + linkTop
            : parentTop + linkPos);
      } else {
        let args: Touch | MouseEvent;
        args = (e.args as TouchEvent).touches
          ? (e.args as TouchEvent).changedTouches[0]
          : (args = e.args as MouseEvent);
        pageX = this.parent.iframeSettings.enable
          ? window.pageXOffset + parentLeft + args.clientX
          : args.pageX;
        pageY = this.parent.iframeSettings.enable
          ? window.pageYOffset + parentTop + args.clientY
          : args.pageY;
      }
      if (this.quickToolObj.linkQTBar) {
        this.quickToolObj.linkQTBar.showPopup(
          pageX,
          pageY,
  ...
  private hideLinkQuickToolbar(): void {
    if (
      this.quickToolObj &&
      this.quickToolObj.linkQTBar &&
      document.body.contains(this.quickToolObj.linkQTBar.element)
    ) {
  ...
  private editAreaClickHandler(e: IImageNotifyArgs): void {
    ...
    if (
      ...
      this.parent.quickToolbarModule.linkQTBar
    ) {
      this.quickToolObj = this.parent.quickToolbarModule;
      ...
      const isPopupOpen: boolean =
        this.quickToolObj.linkQTBar.element.classList.contains('e-rte-pop');
      if (
        target.nodeName === 'A' &&
        target.childNodes.length > 0 &&
        target.childNodes[0].nodeName !== 'IMG' &&
        ((e.args as MouseEvent).target as HTMLElement).nodeName !== 'IMG' &&
        !isNOU(
          closest(this.parent.getRange().startContainer.parentElement, 'A')
        ) &&
        !isNOU(closest(this.parent.getRange().endContainer.parentElement, 'A'))
      ) {
        if (isPopupOpen) {
          return;
        }
        if ((e.args as MouseEvent).ctrlKey === false) {
          ...
          const range: Range = selection.getRange(
            this.parent.contentModule.getDocument()
          );
          const args: object = {
            args: {
  ...
  private openDialog(isInternal?: boolean, event?: NotifyArgs): void {
    ...
    if (this.parent.editorMode === 'HTML') {
      ...
      this.parent.formatter.editorManager.markdownSelection.save(
  ...
  private linkDialog(
    ...
    if (
      this.parent.editorMode === 'HTML' &&
      e.selectParent.length > 0 &&
      !isNullOrUndefined((e.selectParent[0] as HTMLElement).classList) &&
      (e.selectParent[0] as HTMLElement).classList.contains('e-rte-anchor') &&
      isNullOrUndefined(inputDetails)
    ) {
      this.editLink(e);
      return;
    }
    ...
    const htmlTextbox: string =
      this.parent.editorMode === 'HTML'
        ? '<label>' +
          ...
          '">' +
          '<input type="text" data-role ="none" spellcheck="false" placeholder = "' +
          title +
          '"aria-label="' +
          this.i10n.getConstant('linkTitle') +
    ...
    const content: string =
      ...
      urlPlace +
      '"aria-label="' +
      this.i10n.getConstant('linkWebUrl') +
      '" class="e-input e-rte-linkurl' +
      this.parent.getCssClass(true) +
      '"/></div>' +
      '<div class="e-rte-label' +
      this.parent.getCssClass(true) +
      ...
      this.parent.getCssClass(true) +
      '"aria-label="' +
      this.i10n.getConstant('linkText') +
      '" placeholder="' +
      textPlace +
      '">' +
    ...
    const linkTarget: HTMLInputElement = linkContent.querySelector(
      '.e-rte-linkTarget'
    ) as HTMLInputElement;
    const linkUrl: HTMLInputElement = linkContent.querySelector(
      '.e-rte-linkurl'
    ) as HTMLInputElement;
    const linkText: HTMLInputElement = linkContent.querySelector(
      '.e-rte-linkText'
    ) as HTMLInputElement;
    const linkTitle: HTMLInputElement = linkContent.querySelector(
      '.e-rte-linkTitle'
    ) as HTMLInputElement;
    ...
    if (!isNullOrUndefined(inputDetails)) {
      ...
      inputDetails.target
        ...
        : (this.checkBoxObj.checked = false);
      this.dialogObj.header = inputDetails.header;
      this.dialogObj.element.querySelector('.e-insertLink').textContent =
        inputDetails.btnText;
    }
    this.checkUrl(false);
    if (
      (this.parent.editorMode === 'HTML' &&
        isNullOrUndefined(inputDetails) &&
        ((!isNullOrUndefined(selectText) &&
          selectText !== '' &&
          e.selection.range.startOffset === 0) ||
          e.selection.range.startOffset !== e.selection.range.endOffset)) ||
      e.module === 'Markdown'
    ) {
  ...
  private insertlink(e: MouseEvent | KeyboardEvent): void {
    const linkEle: HTMLElement = (this as NotifyArgs).selfLink.dialogObj
      .element as HTMLElement;
    let linkUrl: string = (
      linkEle.querySelector('.e-rte-linkurl') as HTMLInputElement
    ).value;
    let linkText: string = (
      linkEle.querySelector('.e-rte-linkText') as HTMLInputElement
    ).value;
    let linkTitle: string;
    if ((this as NotifyArgs).selfLink.parent.editorMode === 'HTML') {
      linkTitle = (
        linkEle.querySelector('.e-rte-linkTitle') as HTMLInputElement
      ).value;
    }
    const target: string = (this as NotifyArgs).selfLink.checkBoxObj.checked
      ? '_blank'
      : null;
    const linkLabel: string | null = (this as NotifyArgs).selfLink.checkBoxObj
      .checked
      ? (this as NotifyArgs).selfLink.i10n.getConstant('linkAriaLabel')
      : null;
    if (
      (this as NotifyArgs).selfLink.parent.editorMode === 'Markdown' &&
      linkUrl === ''
    ) {
      linkUrl = 'http://';
    }
    ...
    if (
      ...
      isNullOrUndefined(
        closest(
          (this as IImageNotifyArgs).selection.range.startContainer.parentNode,
          '[id=' +
            // eslint-disable-next-line
            "'" +
      ...
      (this as NotifyArgs).selection =
        proxy.parent.formatter.editorManager.nodeSelection.save(
          range,
          proxy.parent.contentModule.getDocument()
        );
      (this as NotifyArgs).selectParent =
    ...
    const value: NotifyArgs = {
      ...
      target: target,
      ariaLabel: linkLabel,
      selection: (this as NotifyArgs).selection,
      selectParent: (this as NotifyArgs).selectParent,
    };
    if (document.body.contains(proxy.dialogObj.element)) {
      (this as NotifyArgs).selfLink.dialogObj.hide({
        returnValue: false,
      } as Event);
    }
    if (isIDevice() && proxy.parent.iframeSettings.enable) {
      (<HTMLIFrameElement>(
        select('iframe', proxy.parent.element)
      )).contentWindow.focus();
    }
    ...
    let argsValue:
      ...
      | Object;
    if (
      !isNullOrUndefined((this as NotifyArgs).args as KeyboardEvent) &&
      ((this as NotifyArgs).args as KeyboardEvent).code === 'KeyK'
    ) {
      const originalEvent: KeyboardEventArgs = (this as NotifyArgs)
        .args as KeyboardEventArgs;
      extend(
    ...
    (this as NotifyArgs).selfLink.parent.formatter.process(
      (this as NotifyArgs).selfLink.parent,
      argsValue,
      !isNullOrUndefined((this as NotifyArgs).args as ClickEventArgs) &&
        ((this as NotifyArgs).args as ClickEventArgs).originalEvent,
      value
    );
    (
      (
        this as NotifyArgs
      ).selfLink.parent.contentModule.getEditPanel() as HTMLElement
    ).focus();
  }
  ...
  private checkUrl(e: boolean): void {
    ...
    const linkUrl: HTMLElement = linkEle.querySelector(
      '.e-rte-linkurl'
    ) as HTMLElement;
    if (e) {
      addClass([linkUrl], 'e-error');
      (linkUrl as HTMLInputElement).setSelectionRange(
        0,
        (linkUrl as HTMLInputElement).value.length
      );
  ...
  private removeLink(e: NotifyArgs): void {
    ...
    const selectParentEle: HTMLElement = this.getAnchorNode(
    ...
    if (isIDevice() && this.parent.iframeSettings.enable) {
      (<HTMLIFrameElement>(
        select('iframe', this.parent.element)
      )).contentWindow.focus();
    } else {
      (this.parent.contentModule.getEditPanel() as HTMLElement).focus();
    }
  ...
  private openLink(e: NotifyArgs): void {
    ...
    if (
      selectParentEle.classList.contains('e-rte-anchor') ||
      selectParentEle.tagName === 'A'
    ) {
      this.parent.formatter.process(this.parent, e.args, e.args, {
        url: (selectParentEle as HTMLAnchorElement).href,
        text: selectParentEle.innerText,
        target:
          (selectParentEle as HTMLAnchorElement).target === ''
            ? '_self'
            : '_blank',
        selectNode: e.selectNode,
        subCommand: ((e.args as ClickEventArgs).item as IDropDownItemModel)
  ...
  private editLink(e: NotifyArgs): void {
    const selectedNode: HTMLElement = this.getAnchorNode(
      e.selectNode[0] as HTMLElement
    );
    let selectParentEle: HTMLElement = this.getAnchorNode(
      e.selectParent[0] as HTMLElement
    );
    selectParentEle =
      selectedNode.nodeName === 'A' ? selectedNode : selectParentEle;
    if (
      selectParentEle.classList.contains('e-rte-anchor') ||
      selectParentEle.tagName === 'A'
    ) {
      ...
      const inputDetails: { [key: string]: string } = {
        url: (selectParentEle as HTMLAnchorElement).getAttribute('href'),
        text: selectParentEle.innerText,
      ...
      (!closest(target, '[id=' + "'" + this.dialogObj.element.id + "'" + ']') &&
        ...
        (((this.parent.getToolbarElement() &&
          ...
          !target.querySelector(
            '#' + this.parent.getID() + '_toolbar_CreateLink'
          )))
    ) {
