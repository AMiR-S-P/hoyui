...
import { closest, isNullOrUndefined as isNOU } from '@hoyui/base';
...
export class DialogRenderer {
  ...
  protected addEventListener(): void {
    ...
    this.parent.on(
      events.documentClickClosedBy,
      this.documentClickClosedBy,
      this
    );
  }
  ...
  private beforeOpen(args: BeforeOpenEventArgs): void {
    ...
    this.parent.trigger(
      events.beforeDialogOpen,
      args,
      this.beforeOpenCallback.bind(this, args)
    );
  }
  ...
  private beforeClose(args: BeforeCloseEventArgs): void {
    ...
    args.closedBy =
      this.outsideClickClosedBy === 'outside click'
        ? this.outsideClickClosedBy
        : args.closedBy;
    this.parent.trigger(
      events.beforeDialogClose,
      args,
      (closeArgs: BeforeCloseEventArgs) => {
        if (!closeArgs.cancel) {
  ...
  private getDialogPosition(): string {
    let distanceFromVisibleTop: number =
