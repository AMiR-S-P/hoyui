...
export class ImageCommand {
  ...
  private createImage(e: IHtmlItem): void {
    ...
    e.item.url =
      isNOU(e.item.url) || e.item.url === 'undefined' ? e.item.src : e.item.url;
    if (
      !isNOU(e.item.selectParent) &&
      (e.item.selectParent[0] as HTMLElement).tagName === 'IMG'
    ) {
    ...
    if (
      ...
      const imgElm: Element =
        e.value === 'Replace' || isReplaced
          ? (e.item.selectParent[0] as Element)
          : Browser.isIE
          ? (selectedNode.previousSibling as Element)
          : (selectedNode as Element).previousElementSibling;
      const onImageLoadEvent: () => void = () => {
        if (!isNOU(this.parent.currentDocument)) {
          e.callBack({
            requestType:
              e.value === 'Replace'
                ? ((e.item.subCommand = 'Replace'), 'Replace')
                : 'Images',
            editorMode: 'HTML',
            event: e.event,
            range: this.parent.nodeSelection.getRange(
  ...
  private setStyle(
    ...
    imgElement.setAttribute(
      'class',
      'e-rte-image' +
        (isNOU(e.item.cssClass) ? '' : ' ' + e.item.cssClass) +
        (isNOU(alignClassName) ? '' : ' ' + alignClassName)
    );
    if (!isNOU(e.item.altText)) {
      imgElement.setAttribute(
        'alt',
        e.item.altText.replace(/\.[a-zA-Z0-9]+$/, '')
      );
    }
    if (!isNOU(e.item.width) && !isNOU(e.item.width.width)) {
      imgElement.setAttribute(
        'width',
        this.calculateStyleValue(e.item.width.width)
      );
    }
    if (!isNOU(e.item.height) && !isNOU(e.item.height.height)) {
      imgElement.setAttribute(
        'height',
        this.calculateStyleValue(e.item.height.height)
      );
    }
    if (!isNOU(e.item.width) && !isNOU(e.item.width.minWidth)) {
      imgElement.style.minWidth = this.calculateStyleValue(
        e.item.width.minWidth
      );
    }
    if (!isNOU(e.item.width) && !isNOU(e.item.width.maxWidth)) {
      imgElement.style.maxWidth = this.calculateStyleValue(
        e.item.width.maxWidth
      );
    }
    if (!isNOU(e.item.height) && !isNOU(e.item.height.minHeight)) {
      imgElement.style.minHeight = this.calculateStyleValue(
        e.item.height.minHeight
      );
    }
    if (!isNOU(e.item.height) && !isNOU(e.item.height.maxHeight)) {
      imgElement.style.maxHeight = this.calculateStyleValue(
        e.item.height.maxHeight
      );
    }
  }
  ...
  private editImageLink(e: IHtmlItem): void {
    ...
    if (isNOU(e.item.target)) {
      (e.item.selectNode[0].parentElement as HTMLAnchorElement).removeAttribute(
        'target'
      );
      (e.item.selectNode[0].parentElement as HTMLAnchorElement).removeAttribute(
        'aria-label'
      ...
      (e.item.selectNode[0].parentElement as HTMLAnchorElement).target =
        e.item.target;
      (e.item.selectNode[0].parentElement as HTMLElement).setAttribute(
        'aria-label',
        e.item.ariaLabel
      );
    }
  ...
  private removeImage(e: IHtmlItem): void {
    if (closest(e.item.selectNode[0], 'a')) {
      if (
        ...
        !isNOU(e.item.selectNode[0].parentElement.innerText)
      ) {
        if (
          !isNOU(closest(e.item.selectNode[0], '.' + classes.CLASS_CAPTION))
        ) {
  ...
  private imageDimension(e: IHtmlItem): void {
    const selectNode: HTMLImageElement = e.item
  ...
  private imageJustifyLeft(e: IHtmlItem): void {
    ...
    if (!isNOU(selectNode)) {
      ...
      if (!isNOU(closest(selectNode, '.' + classes.CLASS_CAPTION))) {
        removeClass(
          [closest(selectNode, '.' + classes.CLASS_CAPTION)],
          classes.CLASS_IMAGE_RIGHT
        );
        addClass(
          [closest(selectNode, '.' + classes.CLASS_CAPTION)],
          classes.CLASS_IMAGE_LEFT
        );
      }
