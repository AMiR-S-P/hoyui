import { createElement, detach, isNullOrUndefined as isNOU } from '@hoyui/base';
...
export class TableSelection {
  ...
  public getBlockNodes(): HTMLElement[] {
    ...
    const currentTable: HTMLTableElement = this.root
      .querySelector('.e-cell-select')
      .closest('table') as HTMLTableElement;
    const cellSelectNode: NodeListOf<HTMLElement> =
      currentTable.querySelectorAll('.e-cell-select');
  ...
  private addBlockNodes(node: Node, blockNodes: HTMLElement[]): void {
    ...
    for (let j: number = 0; j < nodes.length; j++) {
      ...
      if (
        ...
        (currentNode.parentElement.nodeName === 'TD' ||
          currentNode.parentElement.nodeName === 'TH') &&
  ...
  private addTextNodes(parent: HTMLElement, textNodes: Node[]): void {
    ...
    if (
      nodes.length === 0 &&
      (parent.nodeName === 'TD' || parent.nodeName === 'TH')
    ) {
  ...
  private wrapParagraphNodes(blockNodes: HTMLElement[]): void {
    ...
    for (let i: number = 0; i < blockNodes.length; i++) {
      ...
      if (
        ...
        const firstChild: HTMLElement = currentNode
          .childNodes[0] as HTMLElement;
        if (
          firstChild.nodeType === Node.ELEMENT_NODE &&
          this.BASIC_FORMATS.indexOf(firstChild.nodeName.toLocaleLowerCase()) >=
            0 &&
    ...
    while (child) {
      ...
      if (child.nodeName === 'BR') {
        ...
        if (
          ...
          tempBlockNodes.indexOf(wrapperElement) < 0
        ) {
          tempBlockNodes.push(wrapperElement);
        }
        ...
        this.BLOCK_TAGS.indexOf(child.nodeName.toLocaleLowerCase()) >= 0
      ) {
        ...
        if (wrapperElement.childNodes.length > 0) {
          ...
          if (
            child &&
            child.nodeName === 'LI' &&
            (child as HTMLElement).querySelectorAll('li').length > 0
          ) {
            const listNodes: NodeListOf<HTMLElement> = (
