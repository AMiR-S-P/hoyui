...
export class AudioCommand {
  ...
  public audioCommand(e: IHtmlItem): void {
    ...
    switch (value) {
      ...
      case 'inline':
        ...
        (
          selectNode.closest('.' + classes.CLASS_AUDIO_WRAP) as HTMLElement
        ).style.display = 'inline-block';
        addClass(
          [selectNode],
          [
            classes.CLASS_AUDIO,
            classes.CLASS_AUDIO_INLINE,
            classes.CLASS_AUDIO_FOCUS,
          ]
        );
        this.callBack(e);
        break;
  ...
  private createAudio(e: IHtmlItem): void {
    ...
    if (
      ...
      ((e.item.selectParent[0] as HTMLElement).classList.contains(
        classes.CLASS_CLICK_ELEM
      ) ||
        (e.item.selectParent[0] as HTMLElement).classList.contains(
          classes.CLASS_AUDIO_WRAP
        ))
    ) {
      const audioEle: HTMLSourceElement = (
      ...
      wrapElement = createElement('span', {
        className: classes.CLASS_AUDIO_WRAP,
        attrs: {
      ...
      const audElement: HTMLElement = createElement('audio', {
        className: classes.CLASS_AUDIO + ' ' + classes.CLASS_AUDIO_INLINE,
        attrs: { controls: '' },
      });
      const sourceElement: HTMLElement = createElement('source');
      const clickElement: HTMLElement = createElement('span', {
        className: classes.CLASS_CLICK_ELEM,
      });
    ...
    if (
      ...
      const audioElm: Element =
        e.value === 'AudioReplace' || isReplaced
          ? (e.item.selectParent[0] as Element).querySelector('audio')
          : Browser.isIE
          ? (selectedNode as Element)
          : (selectedNode as Element).querySelector('audio');
  ...
  private setStyle(sourceElement: HTMLSourceElement, e: IHtmlItem): void {
    ...
    const fileExtension: string = e.item.fileName
      ? e.item.fileName.split('.').pop().toLowerCase()
      : e.item.url
      ? e.item.url.split('.').pop().toLowerCase()
      : '';
    if (fileExtension === 'opus') {
      ...
      sourceElement.type =
        e.item.fileName && e.item.fileName.split('.').length > 0
          ...
          ? 'audio/' + e.item.url.split('.')[e.item.url.split('.').length - 1]
          : '';
    }
  }
