...
export class Indents {
  ...
  private applyIndents(e: IHtmlSubCommands): void {
    ...
    if (e.enterAction === 'BR') {
      indentsNodes = this.parent.domNode.convertToBlockNodes(
        indentsNodes,
        false
      );
    }
    ...
    for (let i: number = 0; i < parentNodes.length; i++) {
      if (
        (parentNodes[i as number] as Element).tagName !== 'LI' &&
        'LI' === (parentNodes[i as number].parentNode as Element).tagName
      ) {
    ...
    for (let i: number = 0; i < indentsNodes.length; i++) {
      ...
      const marginLeftOrRight: string = isRtl
      ...
      if (e.subCommand === 'Indent') {
        ...
        indentsValue =
          marginLeftOrRight === ''
            ? this.indentValue + 'px'
            : parseInt(marginLeftOrRight, null) + this.indentValue + 'px';
        isRtl
          ? (parentNode.style.marginRight = indentsValue)
          : (parentNode.style.marginLeft = indentsValue);
      } else {
        indentsValue =
          marginLeftOrRight === '' ||
          marginLeftOrRight === '0px' ||
          marginLeftOrRight === '0in'
            ? ''
            : parseInt(marginLeftOrRight, null) - this.indentValue + 'px';
        isRtl
          ? (parentNode.style.marginRight = indentsValue)
