...
export class InsertHtml {
   ...
   * @deprecated
   */
  public static inlineNode: string[] = [
    'a',
    'abbr',
    'acronym',
    'audio',
  ...
  public static Insert(
    ...
    if (
      ...
      range.startOffset === 0 &&
      range.startOffset === range.endOffset &&
      editNode.textContent.trim().length > 0
    ) {
      const focusNode: Node | null = this.findFirstTextNode(
    ...
    if (
      ...
      const currentIndex: number = Array.prototype.slice
        .call(range.startContainer.parentElement.childNodes)
    ...
    const closestParentNode: Node =
      node.nodeName.toLowerCase() === 'table'
        ? !isNOU(nodes[0])
          ? this.closestEle(nodes[0].parentNode, editNode)
          : range.startContainer
        : nodes[0];
    if (
      closestParentNode &&
      closestParentNode.nodeName === 'LI' &&
      node.nodeName.toLowerCase() === 'table'
    ) {
    ...
    if (
      ...
      enterAction !== 'BR' &&
      node.nodeName !== '#text' &&
      !isNOU((node as HTMLElement).children[0]) &&
      !isNOU((node as HTMLElement).children[0].tagName) &&
      (node as HTMLElement).children[0].tagName === 'IMG' &&
      (node as HTMLElement).children.length === 1
    ) {
    ...
    if (
      isExternal ||
      (!isNOU(node) &&
        !isNOU((node as HTMLElement).classList) &&
    ...
    if (
      editNode !== range.startContainer &&
      ((!isCollapsed &&
        !(
          closestParentNode.nodeType === Node.ELEMENT_NODE &&
          CONSTANT.TABLE_BLOCK_TAGS.indexOf(
            (closestParentNode as Element).tagName.toLocaleLowerCase()
          ) !== -1
        )) ||
        (node.nodeName.toLowerCase() === 'table' &&
          closestParentNode &&
      ...
      const preNode: Node = nodeCutter.GetSpliceNode(
        range,
        closestParentNode as HTMLElement
      );
      ...
      if (
        nodes.length === 1 ||
        (node.nodeName.toLowerCase() === 'table' &&
          (preNode as HTMLElement).childElementCount === 0)
      ) {
        nodeSelection.setSelectionContents(docElement, preNode);
        range = nodeSelection.getRange(docElement);
      } else {
        let lasNode: Node = nodeCutter.GetSpliceNode(
          range,
          nodes[nodes.length - 1].parentElement as HTMLElement
        );
        ...
        nodeSelection.setSelectionText(
          ...
          0,
          lasNode.nodeType === 3
            ? lasNode.textContent.length
            : lasNode.childNodes.length
        );
        range = nodeSelection.getRange(docElement);
      }
      if (
        range.startContainer.parentElement.closest('ol,ul') !== null &&
      ...
      for (let index: number = 0; index < nodes.length; index++) {
        if (
          ...
          nodes[index as number].parentNode != null
        ) {
      ...
      if (!isNOU(sibNode) && !isNOU(sibNode.parentNode)) {
        InsertMethods.AppendBefore(
          node as HTMLElement,
          sibNode as HTMLElement,
          true
        );
      } else {
        ...
        if (
          parentNode.firstChild &&
          ((parentNode as HTMLElement) !== editNode ||
            (node.nodeName === 'TABLE' &&
              isCursor &&
          ...
          if (
            ...
            InsertMethods.AppendBefore(
              node as HTMLElement,
              parentNode.firstChild as HTMLElement,
              false
            );
          }
        } else if (
          isNOU(preNode.previousSibling) &&
          (insertNode as HTMLElement).tagName === 'TABLE'
        ) {
      ...
      if (Browser.isIE) {
        ...
        const paraElem: Element = (range.startContainer as HTMLElement)
          .nextElementSibling;
        if (paraElem) {
  ...
  private static findFirstTextNode(node: Node | null): Node | null {
    ...
    for (let i: number = 0; i < node.childNodes.length; i++) {
      const textNode: Node = this.findFirstTextNode(
        node.childNodes[i as number]
      );
      if (!isNOU(textNode)) {
        return textNode;
      }
  ...
  private static pasteInsertHTML(
    ...
    if (
      isCursor &&
      range.startContainer === editNode &&
      editNode.textContent === ''
    ) {
      const currentBlockNode: Node = this.getImmediateBlockNode(
    ...
    if (
      ...
      preNode = nodeCutter.GetSpliceNode(
        range,
        closestParentNode as HTMLElement
      );
      if (!isNOU(preNode)) {
        sibNode = isNOU(preNode.previousSibling)
          ? preNode.parentNode.previousSibling
        ...
        if (nodes.length === 1) {
          ...
          lasNode = nodeCutter.GetSpliceNode(
            range,
            nodes[nodes.length - 1].parentElement as HTMLElement
          );
          lasNode = isNOU(lasNode) ? preNode : lasNode;
    ...
    for (let i: number = 0; i < allChildNodes.length; i++) {
      if (
        CONSTANT.BLOCK_TAGS.indexOf(
          allChildNodes[i as number].nodeName.toLocaleLowerCase()
        ) >= 0
      ) {
    ...
    if (!containsBlockNode) {
      if (!isCursor) {
        ...
        if (isSingleNode) {
          ...
          const startContainerParent: Node =
            editNode === range.startContainer
              ? range.startContainer
        ...
        const nearestAnchor: Node = closest(
        ...
        if (
          ...
          nearestAnchor &&
          closest(nearestAnchor, 'span')
        ) {
          const immediateBlockNode: Node = this.getImmediateBlockNode(
            range.startContainer,
            editNode
          );
          if (
            (immediateBlockNode as HTMLElement).querySelectorAll('br').length >
            0
          ) {
      ...
      if (!isNOU(node) && !isNOU(parentElem) && parentElem.nodeName === 'PRE') {
        ...
        while (node.firstChild) {
          ...
          if (
            (node.firstChild.nodeName === '#text' && isFirstTextNode) ||
            (this.inlineNode.indexOf(
              node.firstChild.nodeName.toLocaleLowerCase()
            ) >= 0 &&
              isFirstTextNode)
          ) {
            lastSelectionNode = node.firstChild;
            if (isNOU((node as HTMLElement).previousElementSibling)) {
              const firstParaElm: HTMLElement =
                enterAction === 'DIV'
                  ? createElement('div')
                  : createElement('p');
              (node as HTMLElement).parentElement.insertBefore(
                firstParaElm,
                node
              );
            }
            if (
              (node as HTMLElement).previousElementSibling.nodeName === 'BR'
            ) {
              (node as HTMLElement).parentElement.insertBefore(
                node.firstChild,
                node
              ...
              (node as HTMLElement).previousElementSibling.appendChild(
                node.firstChild
              );
            }
          } else {
    ...
    if (lastSelectionNode.nodeName === 'TABLE') {
      ...
      lastSelectionNode.parentElement.insertBefore(
        pTag,
        lastSelectionNode.nextSibling
      );
  ...
  private static placeCursorEnd(
    ...
    lastSelectionNode =
      lastSelectionNode.nodeName === 'BR'
        ? isNOU(lastSelectionNode.previousSibling)
          ? lastSelectionNode.parentNode
          : lastSelectionNode.previousSibling
        : lastSelectionNode;
    while (
      ...
      if (
        !isNOU(lastSelectionNode.lastChild) &&
        lastSelectionNode.lastChild.nodeName === 'P' &&
  ...
  private static insertTempNode(
    ...
    if (
      range.startContainer === editNode &&
      !isNOU(range.startContainer.childNodes[range.endOffset - 1]) &&
      range.startContainer.childNodes[range.endOffset - 1].nodeName === 'TABLE'
    ) {
      if (
        isNOU(range.startContainer.childNodes[range.endOffset - 1].nextSibling)
      ) {
        range.startContainer.appendChild(node);
      } else {
        range.startContainer.insertBefore(
          node,
          range.startContainer.childNodes[range.endOffset - 1].nextSibling
        );
      }
    } else if (
      range.startContainer === editNode &&
      ...
      if (
        (range.startContainer as HTMLElement).classList.contains('e-content') ||
      ...
      let blockNode: Node = this.getImmediateBlockNode(
        nodes[nodes.length - 1],
        editNode
      );
      if (
        (isNOU(blockNode) || isNOU(blockNode.parentElement)) &&
      ...
      if (
        ...
        ((node as HTMLElement).firstElementChild.tagName === 'OL' ||
          (node as HTMLElement).firstElementChild.tagName === 'UL')
      ) {
        let liNode: HTMLElement;
        while (
          (node as HTMLElement).firstElementChild.lastElementChild &&
          (node as HTMLElement).firstElementChild.lastElementChild.tagName ===
            'LI'
        ) {
          liNode = (node as HTMLElement).firstElementChild
          ...
          (node as HTMLElement).firstElementChild.insertAdjacentElement(
            'afterend',
            liNode
          );
        }
      ...
      if (
        ...
        const nodeSelection: NodeSelection = new NodeSelection();
        const currentNode: Node = this.getNodeCollection(
          range,
          nodeSelection,
          node
        )[this.getNodeCollection(range, nodeSelection, node).length - 1];
        let splitedElm: Node;
        if (
          ...
          (currentNode.nodeName === '#text' || currentNode.nodeName === 'BR') &&
          !isNOU(currentNode.parentElement) &&
          (currentNode.parentElement.nodeName === 'LI' ||
            currentNode.parentElement.closest('LI') ||
            (blockNode === editNode &&
              currentNode.parentElement === blockNode)) &&
          currentNode.parentElement.textContent.trim().length > 0
        ) {
          splitedElm = currentNode;
          if (
          ...
          splitedElm = nodeCutter.GetSpliceNode(
            range,
  ...
  private static getImmediateBlockNode(node: Node, editNode: Node): Node {
    do {
      ...
      node &&
      CONSTANT.BLOCK_TAGS.indexOf(node.nodeName.toLocaleLowerCase()) < 0
    );
    return node;
  }
  ...
  private static findDetachEmptyElem(element: Element): HTMLElement {
    ...
    if (!isNOU(element.parentElement)) {
      ...
      if (
        ...
        element.parentElement.nodeName !== 'TD' &&
        element.parentElement.nodeName !== 'TH'
      ) {
  ...
  private static removeEmptyElements(element: HTMLElement): void {
    ...
    const nonSvgEmptyElements: Element[] = Array.from(emptyElements).filter(
      (element: Element) => {
    ...
    for (let i: number = 0; i < nonSvgEmptyElements.length; i++) {
      ...
      if (nonSvgEmptyElements[i as number].tagName === 'DIV') {
        lineWithDiv =
          (nonSvgEmptyElements[i as number] as HTMLElement).style
            .borderBottom === 'none' ||
          (nonSvgEmptyElements[i as number] as HTMLElement).style
            .borderBottom === ''
            ? true
      ...
      if (
        CONSTANT.SELF_CLOSING_TAGS.indexOf(
          nonSvgEmptyElements[i as number].tagName.toLowerCase()
        ) < 0 &&
        lineWithDiv
      ) {
        const detachableElement: HTMLElement = this.findDetachEmptyElem(
          nonSvgEmptyElements[i as number]
        );
  ...
  private static closestEle(
    ...
    while (el && el.nodeType === 1) {
      if (
        el.parentNode === editNode ||
        (!isNOU((el.parentNode as Element).tagName) &&
          CONSTANT.IGNORE_BLOCK_TAGS.indexOf(
            (el.parentNode as Element).tagName.toLocaleLowerCase()
          ) !== -1)
      ) {
        return el;
      }
  ...
  private static insertTableInList(
    ...
    if (range.collapsed) {
      ...
      const isEnd: boolean =
        (range.startContainer.textContent as any).trimEnd().length ===
        range.startOffset;
      if (isStart || isEnd) {
        if (isStart) {
          InsertMethods.AppendBefore(
        ...
        const preNode: Node = nodeCutter.SplitNode(
          range,
          parentNode as HTMLElement,
          true
        );
  ...
  private static alignCheck(editNode: HTMLElement): void {
    const spanAligns: NodeListOf<Element> = editNode.querySelectorAll(
    ...
    for (let i: number = 0; i < spanAligns.length; i++) {
      ...
      if (spanAlign) {
        const blockAlign: HTMLElement = this.getImmediateBlockNode(
          spanAlign,
          null
        ) as HTMLElement;
        if (blockAlign) {
  ...
  private static removeListfromPaste(range: Range): void {
    ...
    if (
      !isNOU(value) &&
      value.nodeName === 'LI' &&
      !isNOU(value.parentElement) &&
      (value.parentElement.nodeName === 'OL' ||
        value.parentElement.nodeName === 'UL') &&
      value.textContent.trim() === ''
    ) {
      (value.parentElement as HTMLElement)
        .querySelectorAll('li')
