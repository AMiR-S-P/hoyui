...
export class LinkCommand {
  ...
  private createLink(e: IHtmlItem): void {
    let closestAnchor: Element =
      !isNOU(e.item.selectParent) &&
      e.item.selectParent.length > 0 &&
      closest(e.item.selectParent[0], 'a');
    closestAnchor = !isNOU(closestAnchor)
      ? closestAnchor
      : !isNOU(e.item.selectParent) && e.item.selectParent.length > 0
      ? (e.item.selectParent[0] as Element)
      : null;
    if (
      !isNOU(closestAnchor) &&
      (closestAnchor as HTMLElement).tagName === 'A'
    ) {
      ...
      if (linkText === e.item.text) {
        ...
        const startIndex: number =
          e.item.action === 'Paste'
            ? anchorEle.childNodes[0].textContent.length
            : 0;
        const endIndex: number =
          anchorEle.firstChild.nodeName === '#text'
            ? anchorEle.childNodes[0].textContent.length
            : anchorEle.childNodes.length;
        e.item.selection.setSelectionText(
          this.parent.currentDocument,
      ...
      if (
        ...
        range.startContainer.textContent.length === range.endOffset + 1 &&
        range.endContainer.textContent.charAt(range.endOffset) === ' ' &&
        !isNOU(range.endContainer.nextSibling) &&
        range.endContainer.nextSibling.nodeName === 'A'
      ) {
      ...
      const text: boolean = isNOU(e.item.text)
        ? true
        : e.item.text
            .replace(/ /g, '')
            .localeCompare(
      ...
      if (
        (e.event &&
          (e.event as KeyboardEvent).type === 'keydown' &&
          ((e.event as KeyboardEvent).keyCode === 32 ||
        ...
        const modifiedText: string = text.replace(
          /  +/g,
        ...
        if (
          ...
          startContainer.textContent = this.removeText(
          ...
          const startIndex: number =
            e.item.action === 'Paste'
              ? anchor.childNodes[0].textContent.length
              : 0;
          e.item.selection.setSelectionText(
            this.parent.currentDocument,
            anchor.childNodes[0],
  ...
  private createLinkNode(e: IHtmlItem): void {
    ...
    const nodes: Node[] = this.getSelectionNodes(
      domSelection.getNodeCollection(range)
    );
    const save: NodeSelection = domSelection.save(
      range,
      this.parent.currentDocument
    );
    const txtArray: Node[] = [];
      ...
      while (check === true) {
        if (currentNode.parentNode.nodeName === 'A') {
          ...
          currentNode.parentNode.parentNode.insertBefore(
            anchorEle.firstChild,
            anchorEle
          );
          currentNode.parentNode.removeChild(anchorEle);
        }
        if (
          this.isBlockNode(currentNode.parentNode as Element) ||
          txtArray.length === 0 ||
          i === 0 ||
          i === txtArray.length - 1 ||
    ...
    for (let i: number = 0, j: number = 0; i < inlineNodes.length; i++) {
      ...
      if (inlineNodes.length > 1 && i < inlineNodes.length - 1) {
        if (
          inlineNodes[i as number].parentElement ===
            inlineNodes[i + 1].parentElement &&
    ...
    for (let i: number = 0; i < finalinlineNodes.length; i++) {
      ...
      if (i < finalinlineNodes.length - 1) {
        if (
          finalinlineNodes[i as number].parentNode ===
          finalinlineNodes[i + 1].parentNode
        ) {
    ...
    for (
      ...
      i <= finalinlineNodes.length;
      i++
    ) {
      if (i === 0) {
        finalinlineNodes[i as number].parentNode.insertBefore(
          anchorNodes[j as number],
          finalinlineNodes[i as number].nextSibling
        );
        if (
          this.parent.domNode.blockNodes().length === 1 &&
      ...
      if (i < finalinlineNodes.length - 1) {
        if (
          ...
          j = j + 1;
          finalinlineNodes[i + 1].parentNode.insertBefore(
            anchorNodes[j as number],
            finalinlineNodes[i + 1]
          );
          removeNodes[k as number] = finalinlineNodes[i + 1];
          k++;
        }
    ...
    for (let i: number = 0; i < removeNodes.length; i++) {
      if (removeNodes[i as number].parentNode) {
        removeNodes[i as number].parentNode.removeChild(
          removeNodes[i as number]
        );
      }
    }
  }
  ...
  private createAchorNode(e: IHtmlItem): HTMLElement {
    const anchorEle: HTMLElement = createElement('a', {
      ...
      attrs: {
        href: e.item.url,
        title:
          isNOU(e.item.title) || e.item.title === ''
            ? e.item.url
            : e.item.title,
      },
    });
  ...
  private getSelectionNodes(nodeCollection: Node[]): Node[] {
    ...
    for (let index: number = 0; index < nodeCollection.length; index++) {
      if (
        ...
        nodeCollection[index as number].textContent.trim() === ''
      ) {
  ...
  private removeLink(e: IHtmlItem): void {
    ...
    if (blockNodes.length < 2) {
      ...
      const selectParent: Node = closestAnchor
        ? closestAnchor
      ...
      for (; selectParent.firstChild; null) {
        if (parent) {
          child.push(
            parent.insertBefore(selectParent.firstChild, selectParent)
          );
        } else {
          break;
        }
      ...
      if (child && child.length === 1) {
        e.item.selection.startContainer = e.item.selection.getNodeArray(
      ...
      for (let i: number = 0; i < blockNodes.length; i++) {
        const linkNode: NodeListOf<HTMLAnchorElement> = (
          blockNodes[i as number] as HTMLElement
        ).querySelectorAll('a');
        for (let j: number = 0; j < linkNode.length; j++) {
          if (
            this.parent.currentDocument
              .getSelection()
              .containsNode(linkNode[j as number], true)
          ) {
