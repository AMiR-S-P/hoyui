...
export class NodeCutter {
      ...
      if (isCollapsed) {
        ...
        if (fragment && fragment.childNodes.length > 0) {
          const isEmpty: boolean =
            ...
            fragment.childNodes[0].nodeName !== 'IMG' &&
            !(fragment.querySelectorAll('img').length > 0) &&
            this.isRteElm(fragment) &&
  ...
  private isRteElm(fragment: DocumentFragment): boolean {
    ...
    if (
      ...
      for (
        let i: number = 0;
        !isNOU(firstChild.childNodes) && i < firstChild.childNodes.length;
        i++
      ) {
        if (
          firstChild.childNodes[i as number].nodeName === 'IMG' ||
          (firstChild.childNodes[i as number].nodeName === 'SPAN' &&
            ((
              firstChild.childNodes[i as number] as HTMLElement
            ).classList.contains('e-video-wrap') ||
              (
                firstChild.childNodes[i as number] as HTMLElement
              ).classList.contains('e-embed-video-wrap') ||
              (
                firstChild.childNodes[i as number] as HTMLElement
  ...
  private spliceEmptyNode(
    ...
    if (
      (fragment.childNodes.length === 1 &&
        fragment.childNodes[0].nodeName === '#text' &&
    ...
    if (len > -1 && !isStart) {
      ...
      fragment.nodeName !== 'IMG' &&
      !((fragment as Element).querySelectorAll('img').length > 0) &&
      !(fragment as Element).classList.contains('e-video-wrap') &&
      !(fragment as Element).classList.contains('e-audio-wrap')
    ) {
  ...
  public GetCursorRange(docElement: Document, range: Range, node: Node): Range {
    ...
    while (index !== -1) {
      ...
      if (
        new RegExp('\\s').test(str[index - 1]) &&
        indexes.indexOf(index - 1) < 0
      ) {
    ...
    if (
      indexes.indexOf(range.startOffset) >= 0 ||
      (indexes.indexOf(range.startOffset - 1) >= 0 &&
        (range.startOffset !== 1 ||
          (range.startOffset === 1 && new RegExp('\\s').test(str[0])))) ||
      (indexes[indexes.length - 1] - 1 === range.startOffset &&
        range.endOffset !== str.length - 1 &&
        !new RegExp('\\s').test(str[0]))
    ) {
      cursorRange = range;
      this.position = 1;
    } else {
      let startOffset: number = this.GetCursorStart(
        indexes,
        range.startOffset,
        true
      );
      ...
      cursorRange.setEnd(
        ...
        this.GetCursorStart(indexes, range.startOffset, false)
      );
    }
    return cursorRange;
  }
  ...
  public GetCursorNode(docElement: Document, range: Range, node: Node): Node {
    return this.GetSpliceNode(
      this.GetCursorRange(docElement, range, node),
      node as HTMLElement
    );
  }
