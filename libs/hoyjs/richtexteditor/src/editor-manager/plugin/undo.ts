import { debounce, isNullOrUndefined, detach } from '@hoyui/base';
...
export class UndoRedoManager {
  ...
  public constructor(
    ...
    this.parent = parent;
    this.undoRedoSteps = !isNullOrUndefined(options)
      ? options.undoRedoSteps
      : 30;
    this.undoRedoTimer = !isNullOrUndefined(options)
      ? options.undoRedoTimer
      : 300;
  ...
  private onPropertyChanged(props: { [key: string]: Object }): void {
    for (const prop of Object.keys(props.newProp)) {
      switch (prop) {
        case 'undoRedoSteps':
          this.undoRedoSteps = (
            props.newProp as { [key: string]: number }
          ).undoRedoSteps;
          break;
        case 'undoRedoTimer':
          this.undoRedoTimer = (
  ...
  private keyUp(e: IHtmlKeyboardEvent): void {
    if (
      (e.event as KeyboardEvent).keyCode !== 17 &&
      !(e.event as KeyboardEventArgs).ctrlKey
    ) {
      this.saveData(e);
    }
  }
  private getTextContentFromFragment(
    ...
    for (let i: number = 0; i < fragment.childNodes.length; i++) {
      ...
      if (childNode.nodeType === Node.TEXT_NODE) {
        ...
        textContent += this.getTextContentFromFragment(
  ...
  private isElementStructureEqual(
    previousFragment: DocumentFragment | HTMLElement,
    currentFragment: DocumentFragment | HTMLElement
  ): boolean {
    if (
      previousFragment.childNodes.length !== currentFragment.childNodes.length
    ) {
      return false;
    }
    for (let i: number = 0; i < previousFragment.childNodes.length; i++) {
      const previousFragmentNode: ChildNode =
        previousFragment.childNodes[i as number];
      const currentFragmentNode: ChildNode =
        currentFragment.childNodes[i as number];
      ...
      if (
        (previousFragmentNode as Element).outerHTML !==
        (currentFragmentNode as Element).outerHTML
      ) {
        return false;
      }
    }
  ...
  public saveData(e?: KeyboardEvent | MouseEvent | IUndoCallBack): void {
    ...
    let range: Range = new NodeSelection().getRange(
      this.parent.currentDocument
    );
    const currentContainer: Node =
      this.parent.editableElement === range.startContainer.parentElement
        ? range.startContainer.parentElement
        : range.startContainer;
    for (let i: number = currentContainer.childNodes.length - 1; i >= 0; i--) {
      if (
        !isNullOrUndefined(currentContainer.childNodes[i as number]) &&
        currentContainer.childNodes[i as number].nodeName === '#text' &&
        currentContainer.childNodes[i as number].textContent.length === 0 &&
        currentContainer.childNodes[i as number].nodeName !== 'IMG' &&
        currentContainer.childNodes[i as number].nodeName !== 'BR' &&
        currentContainer.childNodes[i as number].nodeName &&
        'HR'
      ) {
    ...
    const save: NodeSelection = new NodeSelection().save(
      range,
      this.parent.currentDocument
    );
    const clonedElement: HTMLElement = this.parent.editableElement.cloneNode(
      true
    ) as HTMLElement;
    ...
    const changEle: { [key: string]: DocumentFragment | Object } = {
      text: fragment,
      range: save,
    };
    ...
    if (
      ...
      this.undoRedoStack[this.undoRedoStack.length - 1].range.range
        .collapsed === range.collapsed &&
      this.undoRedoStack[this.undoRedoStack.length - 1].range.startOffset ===
        save.range.startOffset &&
      this.undoRedoStack[this.undoRedoStack.length - 1].range.endOffset ===
        save.range.endOffset &&
      this.undoRedoStack[this.undoRedoStack.length - 1].range.range
        .startContainer === save.range.startContainer &&
      this.getTextContentFromFragment(
        this.undoRedoStack[this.undoRedoStack.length - 1].text
      ).trim() ===
        this.getTextContentFromFragment(
          changEle.text as DocumentFragment
        ).trim() &&
      this.isElementStructureEqual(
        this.undoRedoStack[this.undoRedoStack.length - 1].text,
        changEle.text as DocumentFragment
      )
    ) {
  ...
  public undo(e?: IHtmlSubCommands | IHtmlKeyboardEvent): void {
    if (this.steps > 0) {
      ...
      const removedContent: DocumentFragment =
        this.undoRedoStack[this.steps - 1].text;
      ...
      scrollToCursor(
        this.parent.currentDocument,
        this.parent.editableElement as HTMLElement
      );
      if (isIDevice()) {
        setEditFrameFocus(
          this.parent.editableElement,
          (e as IHtmlSubCommands).selector
        );
      }
  ...
  public redo(e?: IHtmlSubCommands | IHtmlKeyboardEvent): void {
    if (this.undoRedoStack[this.steps + 1] != null) {
      const range: string | object = this.undoRedoStack[this.steps + 1].range;
      const addedContent: DocumentFragment =
        this.undoRedoStack[this.steps + 1].text;
      this.parent.editableElement.innerHTML = '';
