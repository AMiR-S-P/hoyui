...
export class ClearFormat {
  private static BLOCK_TAGS: string[] = [
    'address',
    'article',
    'aside',
    'blockquote',
  ...
  private static NONVALID_PARENT_TAGS: string[] = [
    'thead',
    'tbody',
    'ul',
    'ol',
  ...
  private static NONVALID_TAGS: string[] = [
    'thead',
    'tbody',
    'figcaption',
    'td',
  ...
  public static clear(
    ...
    const nodes: Node[] = range.collapsed
      ? nodeSelection.getSelectionNodeCollection(range)
    ...
    if (!isCollapsed) {
      ...
      if (
        nodes.length > 0 &&
        nodes[0].nodeName === 'BR' &&
        closest(nodes[0], 'table')
      ) {
        preNode = nodeCutter.GetSpliceNode(
          range,
          closest(nodes[0], 'table') as HTMLElement
        );
      } else {
        preNode = nodeCutter.GetSpliceNode(
          range,
          nodes[
            nodes.length > 1 && nodes[0].nodeName === 'IMG' ? 1 : 0
          ] as HTMLElement
        );
      }
      if (nodes.length === 1) {
        ...
        const lasNode: Node = nodeCutter.GetSpliceNode(
          range,
          lastText as HTMLElement
        );
        if (lasNode) {
  ...
  private static reSelection(
    ...
    save.endOffset =
      endIndexNode.nodeType === 3
        ? endIndexNode.textContent.length
  ...
  private static clearBlocks(
    ...
    for (let index: number = 0; index < nodes.length; index++) {
      if (
        this.BLOCK_TAGS.indexOf(
          nodes[index as number].nodeName.toLocaleLowerCase()
        ) > -1 &&
        ...
        this.BLOCK_TAGS.indexOf(
          nodes[index as number].parentNode.nodeName.toLocaleLowerCase()
        ) > -1 &&
  ...
  private static spliceParent(parentNodes: Node[], nodes: Node[]): Node[][] {
    for (let index1: number = 0; index1 < parentNodes.length; index1++) {
      ...
      for (let index2: number = 0; index2 < len; index2++) {
        if (
          nodes.indexOf(
            parentNodes[index1 as number].childNodes[index2 as number]
          ) > 0 &&
          parentNodes[index1 as number].childNodes[index2 as number].childNodes
            .length > 0
        ) {
          nodes = this.spliceParent(
            [parentNodes[index1 as number].childNodes[index2 as number]],
            nodes
          )[1];
        }
        if (
          ...
          parentNodes[index1 as number].childNodes[
            index2 as number
          ].textContent.trim() !== ''
        ) {
          for (let index3: number = 0; index3 < len; index3++) {
            if (
              nodes.indexOf(
                parentNodes[index1 as number].childNodes[index3 as number]
              ) > -1
            ) {
              nodes.splice(
                nodes.indexOf(
          ...
          const parentIndex: number = parentNodes.indexOf(
            parentNodes[index1 as number].parentNode
          );
          const nodeIndex: number = nodes.indexOf(
            parentNodes[index1 as number].parentNode
          );
          if (parentIndex > -1) {
          ...
          const elementIndex: number = nodes.indexOf(
    ...
    if (count > 0) {
      for (let index: number = 0; index < count; index++) {
        if (parentNodes.indexOf(parentNode.childNodes[index as number]) > -1) {
          parentNodes = this.removeChild(
            parentNodes,
            parentNode.childNodes[index as number]
          );
          parentNodes.splice(
            parentNodes.indexOf(parentNode.childNodes[index as number]),
            1
          );
        }
      }
  ...
  private static unWrap(
    ...
    for (let index1: number = 0; index1 < parentNodes.length; index1++) {
      if (
        this.NONVALID_TAGS.indexOf(
          parentNodes[index1 as number].nodeName.toLowerCase()
        ) > -1 &&
        parentNodes[index1 as number].parentNode &&
        this.NONVALID_PARENT_TAGS.indexOf(
          parentNodes[index1 as number].parentNode.nodeName.toLowerCase()
        ) > -1
      ) {
        nodeSelection.setSelectionText(
          docElement,
      ...
      const blockquoteNode: Node = closest(
        parentNodes[index1 as number],
        'blockquote'
      );
      if (
        parentNodes[index1 as number].nodeName.toLocaleLowerCase() !==
          'blockquote' &&
        !isNullOrUndefined(blockquoteNode) &&
        blockquoteNode.textContent === parentNodes[index1 as number].textContent
      ) {
      ...
      if (parentNodes[index1 as number].nodeName.toLocaleLowerCase() !== 'p') {
        if (
          ...
          !(
            (parentNodes[index1 as number].nodeName.toLocaleLowerCase() ===
              ...
              parentNodes[index1 as number].nodeName.toLocaleLowerCase() ===
                'li') &&
            this.IGNORE_PARENT_TAGS.indexOf(
              parentNodes[
                index1 as number
              ].childNodes[0].nodeName.toLocaleLowerCase()
            ) > -1
          ) &&
          !(
            ...
            parentNodes[
              index1 as number
            ].childNodes[0].nodeName.toLocaleLowerCase() === 'p'
          )
        ) {
          InsertMethods.Wrap(
            parentNodes[index1 as number] as HTMLElement,
            docElement.createElement(this.defaultTag)
          );
        }
        const childNodes: Node[] = InsertMethods.unwrap(
          parentNodes[index1 as number]
        );
        if (
          childNodes.length === 1 &&
        ...
        for (let index2: number = 0; index2 < childNodes.length; index2++) {
          if (
            this.NONVALID_TAGS.indexOf(
              childNodes[index2 as number].nodeName.toLowerCase()
            ) > -1
          ) {
            this.unWrap(
              docElement,
              [childNodes[index2 as number]],
              nodeCutter,
              nodeSelection
            );
          } else if (
            ...
            const blockNodes: Node[] = this.removeParent([
              childNodes[index2 as number],
            ]);
            this.unWrap(docElement, blockNodes, nodeCutter, nodeSelection);
          } else if (
            ...
            if (
              childNodes[
                index2 as number
              ].parentNode.nodeName.toLocaleLowerCase() === 'p'
            ) {
              InsertMethods.unwrap(
                childNodes[index2 as number].parentNode as HTMLElement
              );
            }
            InsertMethods.Wrap(
              childNodes[index2 as number] as HTMLElement,
              docElement.createElement(this.defaultTag)
            );
            ...
            childNodes[
              index2 as number
            ].parentNode.nodeName.toLocaleLowerCase() ===
              childNodes[index2 as number].nodeName.toLocaleLowerCase()
          ) {
            InsertMethods.unwrap(childNodes[index2 as number]);
          }
  ...
  private static clearInlines(
    ...
    for (let index: number = 0; index < textNodes.length; index++) {
      ...
      while (
        !this.domNode.isBlockNode(currentInlineNode as Element) &&
        currentInlineNode.parentElement &&
        !currentInlineNode.parentElement.classList.contains('e-img-inner')
      ) {
  ...
  private static removeInlineParent(textNodes: Node): void {
    ...
    for (let index: number = 0; index < nodes.length; index++) {
      if (
        nodes[index as number].parentNode.childNodes.length === 1 &&
        !(nodes[index as number].parentNode as HTMLElement).classList.contains(
          'e-img-inner'
        ) &&
        IsFormatted.inlineTags.indexOf(
          nodes[index as number].parentNode.nodeName.toLocaleLowerCase()
