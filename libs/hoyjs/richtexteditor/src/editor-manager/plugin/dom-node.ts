...
export class DOMNode {
  ...
  public contents(element: Element): Node[] {
    return element && 'IFRAME' !== element.tagName
  ...
  public isBlockNode(element: Element): boolean {
    return (
      ...
      CONSTANT.BLOCK_TAGS.indexOf(element.tagName.toLowerCase()) >= 0
    );
  }
  ...
  public blockParentNode(element: Element): Element {
    for (
      ...
      (!element.parentNode ||
        !this.hasClass(element.parentNode as Element, 'e-node-inner'));
      null
    ) {
  ...
  public clearAttributes(element: Element): void {
    for (
      let attr: NamedNodeMap = element.attributes, c: number = attr.length - 1;
      c >= 0;
      c--
    ) {
  ...
  public createTagString(
    ...
    return (
      '<' +
      tagName.toLowerCase() +
      this.attributes(relativeElement) +
      '>' +
      innerHTML +
      '</' +
      tagName.toLowerCase() +
  ...
  public parseHTMLFragment(value: string): Element {
    /* eslint-disable */
    let temp: HTMLTemplateElement = <HTMLTemplateElement>(
  ...
  public unWrap(element: Element): Element[] {
    ...
    unWrapNode =
      unWrapNode.length > 0 ? unWrapNode : [element.parentNode as Element];
  ...
  public getSelectedNode(element: Element, index: number): Element {
    if (
      ...
      element.childNodes[index - 1].nodeType === Node.ELEMENT_NODE &&
      ((element.childNodes[index - 1] as Element).classList.contains(
      ...
      element.nodeType === Node.ELEMENT_NODE &&
      element.childNodes.length > 0 &&
      element.childNodes[index as number]
    ) {
  ...
  public nodeFinds(element: Element, elements: Element[]): Element[] {
    ...
    for (let i: number = 0; i < elements.length; i++) {
      if (
        element.contains(elements[i as number]) &&
        element !== elements[i as number]
      ) {
  ...
  public isEditorArea(): boolean {
    ...
    for (
      ...
      element && !this.isElement(element);
      null
    ) {
      element = element.parentNode as Element;
    }
  ...
  public saveMarker(save: NodeSelection): NodeSelection {
    let start: Element = this.parent.querySelector(
      '.' + markerClassName.startSelection
    );
    let end: Element = this.parent.querySelector(
      '.' + markerClassName.endSelection
    );
    let startTextNode: Element;
    let endTextNode: Element;
    if (
      this.hasClass(start, markerClassName.startSelection) &&
      start.classList.length > 1
    ) {
      const replace: string = this.createTagString(
        CONSTANT.DEFAULT_TAG,
        start,
        this.encode(start.textContent)
      );
    ...
    if (
      this.hasClass(end, markerClassName.endSelection) &&
      end.classList.length > 1
    ) {
      const replace: string = this.createTagString(
        CONSTANT.DEFAULT_TAG,
        end,
        this.encode(end.textContent)
      );
  ...
  public setMarker(save: NodeSelection): void {
    ...
    const isTableStart: boolean =
      startChildNodes.length > 1 && startChildNodes[0].nodeName === 'TABLE';
    const isImgOnlySelected: boolean =
      startChildNodes.length > 1 &&
      startChildNodes[0].nodeName === 'IMAGE' &&
      range.endOffset === 1 &&
      range.endContainer.nodeName === '#text' &&
      range.endContainer.textContent.length === 0;
    let start: Element = <Element>(
      ((isTableStart
        ? getLastTextNode(startChildNodes[range.startOffset + 1])
        : startChildNodes[
            range.startOffset > 0 ? range.startOffset - 1 : range.startOffset
          ]) || range.startContainer)
    );
    let end: Element = <Element>(
          ...
          if (
            ...
            (node.nodeType === Node.TEXT_NODE ||
              (CONSTANT.IGNORE_BLOCK_TAGS.indexOf(
                (node.parentNode as Element).tagName.toLocaleLowerCase()
              ) >= 0 &&
    ...
    if (start !== end) {
      if (
        ...
        ((start.tagName === 'BR' &&
          CONSTANT.IGNORE_BLOCK_TAGS.indexOf(
            (start.parentNode as Element).tagName.toLocaleLowerCase()
          ) >= 0) ||
          start.tagName === 'IMG')
      ) {
        this.replaceWith(
        ...
        const markerStart: Element = (
          range.startContainer as HTMLElement
        ).querySelector('.' + markerClassName.startSelection);
      ...
      if (
        ...
        CONSTANT.IGNORE_BLOCK_TAGS.indexOf(
          (end.parentNode as Element).tagName.toLocaleLowerCase()
        ) >= 0
      ) {
        this.replaceWith(
          end,
        ...
        ).querySelector('.' + markerClassName.endSelection);
  ...
  public ensureSelfClosingTag(
    ...
    if (start.nodeType === 3) {
      this.replaceWith(
        start,
        this.marker(className, this.encode(start.textContent))
      );
      ...
      const tagName: string = !isNOU(start.parentElement)
        ? start.parentElement.tagName.toLocaleLowerCase()
        : '';
      ...
      if (start.tagName === 'TABLE') {
        ...
        if (start.textContent === '') {
          ...
          start = !isNOU(start.childNodes[0])
      ...
      for (let i: number = 0; i < selfClosingTags.length; i++) {
        start =
          start.tagName === selfClosingTags[i as number] && !isTable
            ? (start.parentNode as Element)
            : start;
      }
  ...
  public createTempNode(element: Element): Element {
    ...
    if (element.tagName === 'BR') {
      const wrapper: string =
        '<' + CONSTANT.DEFAULT_TAG + '></' + CONSTANT.DEFAULT_TAG + '>';
      const node: Element = element.parentNode as Element;
      if (
        CONSTANT.IGNORE_BLOCK_TAGS.indexOf(node.tagName.toLocaleLowerCase()) >=
        0
      ) {
      ...
      element = this.wrap(element, this.parseHTMLFragment(wrapper));
      const ignoreBr: boolean =
        target.nodeType === Node.ELEMENT_NODE &&
        target.firstChild &&
        target.firstChild.nodeName === 'BR' &&
  ...
  public gatherElementsAround(
    ...
    while (
      ...
      const prevSibling: HTMLElement =
        currentNode.previousSibling as HTMLElement;
    ...
    currentNode = (newWrapElem.nextSibling as HTMLElement)
      ? (newWrapElem.nextSibling as HTMLElement)
    ...
    while (
      ...
      const nextSibling: HTMLElement | null =
        (currentNode.nextSibling as HTMLElement)
          ? (currentNode.nextSibling as HTMLElement)
          : (currentNode.parentElement.nextSibling as HTMLElement);
      if (currentNode.nodeType === 3 || currentNode.nodeType === 1) {
        newWrapElem.appendChild(currentNode);
      }
  ...
  public convertToBlockNodes(selectedNodes: Node[], fromList: boolean): Node[] {
    if (selectedNodes.length > 1) {
        ...
        if (!isNOU(currentSelectedNode)) {
          if (fromList) {
            selectedNodes[i as number] =
              currentSelectedNode.nodeName === 'LI' ||
              this.isBlockNode(currentSelectedNode)
                ? currentSelectedNode
                : this.gatherElementsAround(
                    currentSelectedNode as HTMLElement,
                    fromList ? 'p' : 'div'
                  );
          } else {
            selectedNodes[i as number] = !this.isBlockNode(
              selectedNodes[i as number] as HTMLElement
            )
              ? this.gatherElementsAround(
                  currentSelectedNode as HTMLElement,
                  fromList ? 'p' : 'div'
          ...
          const currentProcessNode: Node =
            selectedNodes[i as number].nodeName === 'LI'
              ? selectedNodes[i as number].parentElement
              : selectedNodes[i as number];
          const currentElementCheckNode: HTMLElement =
            currentProcessNode.nodeName === '#text'
              ? currentProcessNode.parentElement
              : (currentProcessNode as HTMLElement);
          currentSelectedNode =
            !isNOU(
              currentElementCheckNode.querySelector('.e-editor-select-end')
            ) || !isNOU(closest(currentSelectedNode, '.e-editor-select-end'))
              ? null
              : (currentProcessNode.nextSibling as HTMLElement);
          if (
            currentSelectedNode === null &&
            !isNOU(currentProcessNode.nextSibling) &&
            currentProcessNode.nextSibling.nodeName === 'BR'
          ) {
      ...
      if (!this.isBlockNode(selectedNodes[0] as HTMLElement)) {
        selectedNodes[0] = this.gatherElementsAround(
          selectedNodes[0] as HTMLElement,
          fromList ? 'p' : 'div'
        );
        if (
          !isNOU(selectedNodes[0].nextSibling) &&
          selectedNodes[0].nextSibling.nodeName === 'BR'
        ) {
  ...
  public blockNodes(action?: boolean): Node[] {
    ...
    if (this.isEditorArea() && selection.rangeCount) {
      ...
      for (let j: number = 0; j < ranges.length; j++) {
        ...
        const startNode: Element = this.getSelectedNode(
        ...
        if (
          this.isBlockNode(startNode) &&
          collectionNodes.indexOf(startNode) < 0
        ) {
        ...
        if (parentNode && collectionNodes.indexOf(parentNode) < 0) {
          if (!isNOU(action) && action) {
            if (
              ...
              endParentNode.nodeName === 'TD'
            ) {
              const processedNodes: Node[] =
                this.getPreBlockNodeCollection(range);
              if (processedNodes.length > 1) {
                ...
                if (startNode !== endNode && startNode.nodeName !== 'BR') {
                  ...
                  ((startNode as HTMLElement).classList.contains(
                    markerClassName.startSelection
                  ) ||
                    (startNode as HTMLElement).classList.contains(
                      markerClassName.endSelection
            ...
            if (
              CONSTANT.IGNORE_BLOCK_TAGS.indexOf(
                parentNode.tagName.toLocaleLowerCase()
              ) >= 0 &&
              (startNode.tagName === 'BR' ||
              ...
              const tempNode: Element =
                startNode.previousSibling &&
                (startNode.previousSibling as Element).nodeType ===
                  Node.TEXT_NODE
                  ? (startNode.previousSibling as Element)
                  : startNode;
              if (
        ...
        const nodes: Element[] = [];
        for (
          let node: Element = startNode;
          node !== endNode && node !== this.parent;
          null
        ) {
          if (
            nodes.indexOf(node) < 0 &&
            ...
            CONSTANT.IGNORE_BLOCK_TAGS.indexOf(
              (
                node.nextSibling.parentNode as Element
          ...
          if (
            collectionNodes.indexOf(node) < 0 &&
          ...
          if (
            ...
            this.ignoreTableTag(node as Element) &&
            nodes.indexOf(node) < 0 &&
            collectionNodes.indexOf(node) < 0 &&
            (node !== endNode || range.endOffset > 0)
          ) {
        ...
        if (
          parentNode &&
          this.ignoreTableTag(parentNode) &&
          collectionNodes.indexOf(parentNode) < 0 &&
          !isNOU(parentNode.previousElementSibling) &&
    ...
    for (let i: number = collectionNodes.length - 1; i > 0; i--) {
      const nodes: Element[] = this.nodeFinds(
        collectionNodes[i as number],
        collectionNodes
      );
      if (nodes.length) {
        ...
        if (
          collectionNodes[i as number].tagName === 'LI' &&
          listNodes.length > 0
        ) {
          continue;
        } else {
  ...
  private ignoreTableTag(element: Element): boolean {
    return !(
      CONSTANT.TABLE_BLOCK_TAGS.indexOf(element.tagName.toLocaleLowerCase()) >=
      0
    );
  }
  ...
  private getPreBlockNodeCollection(range: Range): Node[] {
    ...
    const processedStart: Node = this.getClosestInlineParent(
      startNode,
      rootNode,
      true
    );
    const processedEnd: Node = this.getClosestInlineParent(
      endNode,
      rootNode,
      false
    );
    ...
    while (currentNode) {
      ...
      if (rootFlag) {
        ...
        if (isStart && currentNode.previousSibling) {
          if (
            this.isBlockNode(currentNode.previousSibling as Element) ||
          ...
          if (
            this.isBlockNode(currentNode.nextSibling as Element) ||
  ...
  private wrapWithBlockNode(nodes: Node[], collectionNodes: Node[]): void {
    ...
    if (
      wrapperElement.childNodes.length > 0 &&
      collectionNodes.indexOf(wrapperElement) < 0
    ) {
      collectionNodes.push(wrapperElement);
    }
  }
