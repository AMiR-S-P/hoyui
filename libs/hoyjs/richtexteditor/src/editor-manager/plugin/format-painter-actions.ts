...
import {
  ...
  FormatPainterCollection,
  FormatPainterValue,
  IFormatPainterEditor,
  IFormatPainterSettings,
  IHtmlItem,
} from '../base';
...
export class FormatPainterActions implements IFormatPainterEditor {
  ...
  private addEventListener(): void {
    this.parent.observer.on(
      EVENTS.FORMAT_PAINTER_ACTIONS,
      this.actionHandler,
      this
    );
    this.parent.observer.on(
      EVENTS.MODEL_CHANGED_PLUGIN,
  ...
  private actionHandler(args: IHtmlItem): void {
    ...
    if (
      !isNOU(args) &&
      !isNOU(args.item) &&
      !isNOU(args.item.formatPainterAction)
    ) {
  ...
  private generateElement(): void {
    const copyCollection: FormatPainterCollection[] =
      this.copyCollection.slice(); // To clone without reversing the collcection array
    copyCollection.reverse();
    const length: number = copyCollection.length;
    const elemCollection: HTMLElement = createElement('div', {
    ...
    const elemChild: HTMLElement = this.removeDeniedFormats(
      elemCollection as HTMLElement
    );
  ...
  private removeDeniedFormats(parentElement: HTMLElement): HTMLElement {
    if (
      !isNOU(this.deniedFormatsCollection) &&
      this.deniedFormatsCollection.length > 0
    ) {
      const deniedPropArray: DeniedFormatsCollection[] =
          ...
          for (let j: number = 0; j < elementsList.length; j++) {
            if (deniedPropArray[i as number].classes.length > 0) {
              ...
              for (let k: number = 0; k < classLength; k++) {
                if (
                  (elementsList[j as number] as HTMLElement).classList.contains(
                    classes[k as number]
                  )
                ) {
                  removeClass(
                    [elementsList[j as number] as HTMLElement],
                    classes[k as number].trim()
                  );
                }
              }
              if (
                (elementsList[j as number] as HTMLElement).classList.length ===
                0
              ) {
                (elementsList[j as number] as HTMLElement).removeAttribute(
                  'class'
                );
              }
            }
            if (deniedPropArray[i as number].styles.length > 0) {
              ...
              for (let k: number = 0; k < styleLength; k++) {
                (elementsList[j as number] as HTMLElement).style.removeProperty(
                  styles[k as number].trim()
                );
              }
              if (
                (elementsList[j as number] as HTMLElement).style.length === 0
              ) {
                (elementsList[j as number] as HTMLElement).removeAttribute(
                  'style'
                );
  ...
  private copyAction(): void {
    ...
    let nodes: Node[] = range.collapsed
      ? domSelection.getSelectionNodeCollection(range)
      : domSelection.getSelectionNodeCollectionBr(range);
    if (
      nodes.length === 0 &&
      domSelection.getSelectionNodeCollectionBr(range).length === 0
    ) {
      return;
    } else {
      nodes =
    ...
    const allowedRulesArray: string[] =
      this.settings.allowedFormats.indexOf(';') > -1
    ...
    const [rangeParentElem, context] = this.getRangeParentElem(
      currentContext,
      parentElem
    );
    ...
    if (
      !isNOU(currentContext) &&
      this.settings.allowedContext.indexOf(
        currentContext as IFormatPainterContext
      ) > -1
    ) {
      ...
      do {
        if (allowedRulesArray.indexOf(parentElem.nodeName.toLowerCase()) > -1) {
          ...
          for (let i: number = 0; i < allAttributes.length; i++) {
            if (
              allAttributes[i as number].name !== 'class' &&
              allAttributes[i as number].name !== 'style'
            ) {
        ...
        !isNOU(parentElem) ||
        parentElem === this.parent.editableElement
      );
  ...
  private getRangeParentElem(
    ...
    if (isNOU(rangeParentELem)) {
      const nearBlockParentName: string | null =
        this.getNearestBlockParentElement(rangeParent);
      if (
        !isNOU(nearBlockParentName) &&
        nearBlockParentName !== 'UL' &&
        nearBlockParentName !== 'OL' &&
        nearBlockParentName !== 'LI'
      ) {
  ...
  private paintPlainTextFormat(): void {
    ...
    const domSelection: NodeSelection = this.parent.nodeSelection;
    const nodes: Node[] = range.collapsed
      ? domSelection.getSelectionNodeCollection(range)
      : domSelection.getSelectionNodeCollectionBr(range);
    ...
    if (nodes.length > 1) {
      ...
      isInValid =
        this.validateELementTag(range.startContainer) &&
        this.validateELementTag(range.endContainer);
    }
    if (!isInValid) {
  ...
  private findCurrentContext(parentElem: HTMLElement): string | null {
    ...
    if (closestParagraph && !closestList) {
      ...
      if (
        !isNOU(closestParagraph) &&
        !isNOU(closestList) &&
        closestParagraph.textContent.trim() !== closestList.textContent.trim()
      ) {
        return 'Text';
      }
      return 'List';
    } else if (
      closest(parentElem, 'td') ||
      closest(parentElem, 'tr') ||
      closest(parentElem, 'th')
    ) {
      return 'Table';
    }
    return null;
  ...
  private insertFormatNode(elem: HTMLElement, lastChild: HTMLElement): void {
    ...
    if (blockNodes.length === 1) {
      isFullNodeSelected =
        blockNodes[0].textContent.trim() === range.toString().trim();
    }
    if (
      (this.isBlockElement(clonedElem) && isCollapsed) ||
      blockNodes.length > 1 ||
      isFullNodeSelected
    ) {
  ...
  private isListCopied(): boolean {
    ...
    for (let i: number = 0; i < this.copyCollection.length; i++) {
      if (
        this.copyCollection[i as number].tagName === 'OL' ||
        this.copyCollection[i as number].tagName === 'UL'
      ) {
        isListCopied = true;
        break;
      }
  ...
  private insertBlockNode(
    ...
    const cloneElementNode: Node = isNOU(cloneListParentNode)
      ? element
      : element.firstChild;
    for (let index: number = 0; index < nodes.length; index++) {
      if (
        this.INVALID_TAGS.indexOf(nodes[index as number].nodeName) > -1 ||
        (nodes[index as number] as HTMLElement).querySelectorAll(
      ...
      if (
        nodes[index as number].nodeName === 'TD' ||
        nodes[index as number].nodeName === 'TH'
      ) {
      ...
      if (cloneParentNode.nodeName === 'LI' && !sameListType) {
        ...
        nodes[index as number].parentNode.replaceChild(
          cloneParentNode,
      ...
      if (
        cloneParentNode.nodeName !== 'LI' &&
        (cloneParentNode.parentElement.nodeName === 'OL' ||
        ...
        if (!isNOU(parent.previousElementSibling)) {
          ...
          if (cloneParentNode.childNodes.length > 1) {
            for (
              let j: number = 0;
              j < cloneParentNode.childNodes.length;
              j++
            ) {
              const currentChild: Node =
                cloneParentNode.childNodes[j as number];
              if (
                currentChild.nodeName === 'OL' ||
                currentChild.nodeName === 'UL'
              ) {
  ...
  private insertNewList(
    ...
    nodes: Node[],
    index: number,
    cloneListParentNode: Node,
    cloneParentNode: Node
  ): void {
    // Appending the li nodes to the ol or ul node
    if (index === 0) {
      const nodeName: string = nodes[index as number].nodeName;
      nodes[index as number] = nodes[index as number].parentNode.replaceChild(
        cloneListParentNode,
        nodes[index as number]
      );
      const parent: HTMLElement =
        nodeName === 'LI'
          ? cloneListParentNode.parentElement
      ...
      if (nodes[index as number].nodeName === 'LI' && !isNOU(parent)) {
        ...
        if (!isNOU(parent)) {
          if (
            !isNOU(parent.previousElementSibling) &&
            parent.previousElementSibling.nodeName ===
              cloneListParentNode.nodeName
          ) {
            const currentParent: Element = parent.previousElementSibling;
            currentParent.append(cloneParentNode);
            while (currentParent.firstChild) {
              (cloneListParentNode as HTMLElement).append(
                currentParent.firstChild
              );
            }
          } else if (
            !isNOU(parent.nextElementSibling) &&
            parent.nextElementSibling.nodeName === cloneListParentNode.nodeName
          ) {
  ...
  private insertSameList(
    ...
    if (index === 0) {
      if (
        !isNOU(nodes[index as number].parentNode) &&
        (nodes[index as number].parentNode.nodeName === 'UL' ||
          nodes[index as number].parentNode.nodeName === 'OL')
      ) {
        ...
        if (nodes.length === 1) {
          ...
          while (cloneParentNode.firstChild) {
            (nodes[index as number] as HTMLElement).append(
              cloneParentNode.firstChild
            );
          }
          for (
            let i: number = 0;
            i < nodes[index as number].parentNode.childNodes.length;
            i++
          ) {
            const currentChild: Node =
              nodes[index as number].parentNode.childNodes[i as number];
            (cloneListParentNode as HTMLElement).append(
              currentChild.cloneNode(true)
            );
          }
        } else {
          (cloneListParentNode as HTMLElement).append(cloneParentNode);
        ...
        nodes[index as number].parentNode.parentNode.replaceChild(
          cloneListParentNode,
          nodes[index as number].parentNode
        );
      }
    } else {
      (cloneListParentNode as HTMLElement).append(cloneParentNode);
  ...
  private isSameListType(element: HTMLElement, node: Node): boolean {
    ...
    if (
      !isNOU(nearestListNode) &&
      (nearestListNode as Element).querySelectorAll('li').length > 0
    ) {
  ...
  private setDeniedFormats(): void {
    ...
    const deniedFormats: string[] =
      this.settings.deniedFormats.indexOf(';') > -1
        ? this.settings.deniedFormats.split(';')
        : [this.settings.deniedFormats];
    ...
    for (let i: number = 0; i < length; i++) {
      ...
      if (formatString !== '') {
        formatString.trim();
        const collection: DeniedFormatsCollection =
          this.makeDeniedFormatsCollection(formatString);
        if (!isNOU(collection)) {
