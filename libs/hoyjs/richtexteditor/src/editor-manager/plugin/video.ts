...
export class VideoCommand {
  ...
  public videoCommand(e: IHtmlItem): void {
    ...
    if (value !== 'video' && value !== 'videoreplace') {
      ...
      videoWrapNode = selectNode.closest(
        '.' + classes.CLASS_EMBED_VIDEO_WRAP
      ) as HTMLElement;
      videoClickElem = selectNode.closest(
        '.' + classes.CLASS_VIDEO_CLICK_ELEM
      ) as HTMLElement;
    }
    switch (value) {
      case 'video':
      ...
      case 'inline':
        ...
        addClass(
          ...
          [
            classes.CLASS_VIDEO,
            classes.CLASS_VIDEO_INLINE,
            classes.CLASS_VIDEO_FOCUS,
          ]
        );
        this.callBack(e);
        break;
      ...
      case 'justifycenter':
        ...
        addClass(
          [selectNode],
          [classes.CLASS_VIDEO, classes.CLASS_VIDEO_CENTER]
        );
        this.callBack(e);
        break;
      case 'justifyright':
        ...
        addClass(
          [selectNode],
          [classes.CLASS_VIDEO, classes.CLASS_VIDEO_RIGHT]
        );
        this.callBack(e);
        break;
      case 'videoremove':
  ...
  private wrapVideo(e: IHtmlItem): HTMLElement {
    ...
    if (e.item.isEmbedUrl) {
      ...
      const clickElement: HTMLElement = createElement('span', {
      ...
      this.vidElement = sourceElement =
        clickElement.firstElementChild as HTMLElement;
      this.setStyle(sourceElement as HTMLSourceElement, e, this.vidElement);
      wrapElement.style.display =
        e.item.cssClass === classes.CLASS_VIDEO_INLINE
          ? 'inline-block'
          : 'block';
      ...
      wrapElement = createElement('span', {
        ...
        attrs: {
          ...
          title: !isNOU(e.item.title)
            ? e.item.title
            : !isNOU(e.item.fileName)
            ? e.item.fileName
            : '',
        },
      });
  ...
  private createVideo(e: IHtmlItem): void {
    ...
    let wrapElement: HTMLElement;
    if (
      e.value === 'VideoReplace' &&
      !isNOU(e.item.selectParent) &&
      (e.item.selectParent[0] as HTMLElement).tagName === 'VIDEO'
    ) {
      if (e.item.isEmbedUrl) {
        wrapElement = this.wrapVideo(e);
        const oldEle: HTMLElement = e.item.selection.range
          .startContainer as HTMLElement;
        oldEle.parentNode.replaceChild(wrapElement, oldEle);
      } else {
        const videoEle: HTMLSourceElement = (
          e.item.selectParent[0] as HTMLElement
      ...
      e.value === 'VideoReplace' &&
      !isNOU(e.item.selectParent) &&
      isNOU((e.item.selectParent[0] as HTMLElement).querySelector('iframe')) &&
      (e.item.selectParent[0] as HTMLElement).classList &&
      (e.item.selectParent[0] as HTMLElement).classList.contains(
        classes.CLASS_VIDEO_CLICK_ELEM
      )
    ) {
      ...
      if (wrapElement.nextElementSibling === null) {
        ...
        wrapElement.parentNode.insertBefore(
          insertElem,
          wrapElement.nextSibling
        );
      }
    }
    if (
      e.callBack &&
      (isNOU(e.selector) ||
        (!isNOU(e.selector) && e.selector !== 'pasteCleanupModule'))
    ) {
      const selectedNode: Node = this.parent.nodeSelection.getSelectedNodes(
        this.parent.currentDocument
      )[0];
      ...
      if (e.value === 'VideoReplace' || isReplaced) {
        if (!e.item.isEmbedUrl) {
          ...
          videoElm = (e.item.selectParent[0] as Element).querySelector(
            'iframe'
          );
        }
      } else {
        videoElm = !e.item.isEmbedUrl
          ? (selectedNode as Element).lastElementChild
          : (selectedNode as Element).querySelector('iframe');
      }
      videoElm.addEventListener(
        videoElm.tagName !== 'IFRAME' ? 'loadeddata' : 'load',
        () => {
          if (e.value !== 'VideoReplace' || !isReplaced) {
      ...
      if (Browser.userAgent.indexOf('Firefox') !== -1) {
        this.vidElement.addEventListener('play', () => {
          this.editAreaVideoClick(e);
        });
        this.vidElement.addEventListener('pause', () => {
          this.editAreaVideoClick(e);
        });
      }
    }
  ...
  private setStyle(
    sourceElement: HTMLSourceElement | HTMLElement | HTMLIFrameElement,
    e: IHtmlItem,
    videoEle: HTMLSourceElement | HTMLElement | HTMLIFrameElement
  ): void {
    if (
      e.item.url !== '' && !isNOU(e.item.url) && isNOU(sourceElement)
        ? false
        : sourceElement.nodeName.toLowerCase() !== 'iframe'
    ) {
      sourceElement.setAttribute('src', e.item.url);
    }
    if (!e.item.isEmbedUrl) {
      (sourceElement as HTMLSourceElement).type =
        e.item.fileName && e.item.fileName.split('.').length > 0
          ? 'video/' +
            e.item.fileName.split('.')[e.item.fileName.split('.').length - 1]
          : e.item.url && e.item.url.split('.').length > 0
          ? 'video/' + e.item.url.split('.')[e.item.url.split('.').length - 1]
          : '';
    }
  ...
  private videoDimension(e: IHtmlItem): void {
    const selectNode: HTMLVideoElement | HTMLIFrameElement = !(
      e.item.selectNode[0] as HTMLVideoElement
    ).classList.contains(classes.CLASS_VIDEO_CLICK_ELEM)
      ? (e.item.selectNode[0] as HTMLVideoElement)
