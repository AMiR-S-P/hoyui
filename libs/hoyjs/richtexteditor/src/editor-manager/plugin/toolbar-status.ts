...
export class ToolbarStatus {
  ...
  public static get(
    ...
    let formatCollection: IToolbarStatus = JSON.parse(
      JSON.stringify(statusCollection)
    );
    const nodeCollection: IToolbarStatus = JSON.parse(
      JSON.stringify(statusCollection)
    );
    const nodeSelection: NodeSelection = new NodeSelection();
    const range: Range = nodeSelection.getRange(docElement);
    const nodes: Node[] = documentNode
      ? [documentNode]
      : range.collapsed
      ? nodeSelection.getNodeCollection(range)
    ...
    for (let index: number = 0; index < nodes.length; index++) {
      while (
        ...
        nodes[index as number].parentNode.lastElementChild &&
        nodes[index as number].parentNode.lastElementChild.nodeName !== 'BR' &&
        this.getImmediateBlockNode(
          nodes[index as number].parentNode as Node
        ).textContent.replace(/\u200B/g, '').length === 0 &&
        ...
        nodes[index as number] =
          nodes[index as number].parentNode.lastElementChild.firstChild;
        isNodeChanged = true;
      }
      if (isNodeChanged && nodes[index as number]) {
        nodeSelection.setCursorPoint(
          docElement,
          nodes[index as number] as Element,
          nodes[index as number].textContent.length
        );
        isNodeChanged = false;
      }
      if (
        (nodes[index as number].nodeName !== 'BR' &&
          nodes[index as number].nodeType !== 3) ||
        (nodesLength > 1 &&
          nodes[index as number].nodeType === 3 &&
    ...
    for (let index: number = 0; index < nodes.length; index++) {
      ...
      if (
        (index === 0 && formatCollection.insertcode) ||
        !formatCollection.insertcode
      ) {
        nodeCollection.insertcode = formatCollection.insertcode;
      }
      if (
        (index === 0 && formatCollection.blockquote) ||
        !formatCollection.blockquote
      ) {
        nodeCollection.blockquote = formatCollection.blockquote;
      }
      if (
        (index === 0 && formatCollection.italic) ||
        !formatCollection.italic
      ) {
        nodeCollection.italic = formatCollection.italic;
      }
      if (
        (index === 0 && formatCollection.underline) ||
        !formatCollection.underline
      ) {
        nodeCollection.underline = formatCollection.underline;
      }
      if (
        (index === 0 && formatCollection.strikethrough) ||
        !formatCollection.strikethrough
      ) {
        nodeCollection.strikethrough = formatCollection.strikethrough;
      }
      if (
        (index === 0 && formatCollection.superscript) ||
        !formatCollection.superscript
      ) {
        nodeCollection.superscript = formatCollection.superscript;
      }
      if (
        (index === 0 && formatCollection.subscript) ||
        !formatCollection.subscript
      ) {
        nodeCollection.subscript = formatCollection.subscript;
      }
      if (
        (index === 0 && formatCollection.fontcolor) ||
        !formatCollection.fontcolor
      ) {
      ...
      if (index === 0 && formatCollection.fontname) {
        ...
        nodeCollection.fontname =
          formatCollection.fontname === nodeCollection.fontname
      ...
      if (index === 0 && formatCollection.fontsize) {
        ...
        nodeCollection.fontsize =
          formatCollection.fontsize === nodeCollection.fontsize
            ? formatCollection.fontsize
            : 'empty';
      }
      if (
        (index === 0 && formatCollection.backgroundcolor) ||
        !formatCollection.backgroundcolor
      ) {
        nodeCollection.backgroundcolor = formatCollection.backgroundcolor;
      }
      if (
        (index === 0 && formatCollection.orderedlist) ||
        !formatCollection.orderedlist
      ) {
        nodeCollection.orderedlist = formatCollection.orderedlist;
      }
      if (
        (index === 0 && formatCollection.unorderedlist) ||
        !formatCollection.unorderedlist
      ) {
        nodeCollection.unorderedlist = formatCollection.unorderedlist;
      }
      if (
        (index === 0 && formatCollection.alignments) ||
        !formatCollection.alignments
      ) {
      ...
      if (index === 0 && formatCollection.formats) {
        ...
        nodeCollection.formats =
          formatCollection.formats === nodeCollection.formats
            ? formatCollection.formats
            : 'empty';
      }
      if (
        (index === 0 && formatCollection.createlink) ||
        !formatCollection.createlink
      ) {
        nodeCollection.createlink = formatCollection.createlink;
      }
      if (
        (index === 0 && formatCollection.numberFormatList) ||
        !formatCollection.numberFormatList
      ) {
        nodeCollection.numberFormatList = formatCollection.numberFormatList;
      }
      if (
        (index === 0 && formatCollection.bulletFormatList) ||
        !formatCollection.bulletFormatList
      ) {
        nodeCollection.bulletFormatList = formatCollection.bulletFormatList;
      }
      if (
        (index === 0 && formatCollection.inlinecode) ||
        !formatCollection.inlinecode
      ) {
  ...
  private static getFormatParent(
    ...
    if (
      targetNode.contains(node) ||
      (node.nodeType === 3 &&
        targetNode.nodeType !== 3 &&
        targetNode.contains(node.parentNode))
    ) {
      do {
  ...
  private static isFormattedNode(
    ...
    if (!formatCollection.numberFormatList && !isComplexListUpdated) {
      formatCollection.numberFormatList = this.isNumberFormatList(
        node
      ) as string;
    }
    if (!formatCollection.bulletFormatList && !isComplexListUpdated) {
      formatCollection.bulletFormatList = this.isBulletFormatList(
        node
      ) as string;
    }
  ...
  private static isFontColor(docElement: Document, node: Node): string {
    let color: string =
      (node as HTMLElement).style && (node as HTMLElement).style.color;
    if (
  ...
  private static isFontName(
    ...
    let name: string =
      (node as HTMLElement).style && (node as HTMLElement).style.fontFamily;
    if (
      (name === null || name === undefined || name === '') &&
      node.nodeType !== 3
    ) {
      name = this.getComputedStyle(
        docElement,
        node as HTMLElement,
        'font-family'
      );
    ...
    if (
      ...
      (fontName === null ||
        fontName === undefined ||
        (fontName.filter((value: string, pos: number) => {
          ...
          if (
            ...
            (value.split(',')[0] &&
              !isNullOrUndefined(value.split(',')[0].trim().match(pattern)) &&
              value.split(',')[0].trim() ===
                value.split(',')[0].trim().match(pattern)[0])
          ) {
            index = pos;
          }
        }) &&
  ...
  private static isAlignment(node: Node): string {
    const align: string =
      (node as HTMLElement).style && (node as HTMLElement).style.textAlign;
    if (align === 'left') {
      return 'justifyleft';
  ...
  private static isFormats(node: Node, formatNode?: string[]): string {
    if (
      ((formatNode === undefined || formatNode === null) &&
        CONSTANT.BLOCK_TAGS.indexOf(
          (node as Node).nodeName.toLocaleLowerCase()
        ) > -1) ||
      (formatNode !== null &&
  ...
  private static getComputedStyle(
    ...
    return docElement.defaultView
      .getComputedStyle(node, null)
      .getPropertyValue(prop);
  }
  private static isNumberFormatList(node: Node): string | boolean {
    const list: string =
      (node as HTMLElement).style && (node as HTMLElement).style.listStyleType;
    if (list === 'lower-alpha') {
      return 'Lower Alpha';
    } else if (list === 'number') {
      return 'Number';
