...
export class Formats {
  ...
  private addEventListener(): void {
    ...
    this.parent.observer.on(
      EVENTS.BLOCKQUOTE_LIST_HANDLE,
      this.blockQuotesHandled,
      this
    );
  ...
  private getParentNode(node: Node): Node {
    for (
      ;
      node.parentNode && node.parentNode !== this.parent.editableElement;
      null
    ) {
  ...
  private onKeyUp(e: IHtmlSubCommands): void {
    ...
    if (
      ...
      if (
        ...
        lastChild.previousSibling.nodeName === 'TABLE'
      ) {
        ...
        const brNode: Node =
          this.parent.nodeSelection.getSelectionNodeCollectionBr(range)[0];
        if (
          !isNOU(brNode) &&
          brNode.nodeName === 'BR' &&
          brNode.previousSibling &&
          brNode.previousSibling.nodeName === 'TABLE'
        ) {
    ...
    if (
      ...
      !isNOU(range.startContainer.parentElement) &&
      range.startContainer === range.endContainer &&
      range.startContainer.nodeName === '#text' &&
      range.startContainer.parentElement.classList.contains('e-content') &&
      range.startContainer.parentElement.isContentEditable
    ) {
  ...
  private onKeyDown(e: IHtmlSubCommands): void {
    if (e.event.which === 13 && !this.blockquotePrevent) {
      ...
      const startCon: Node =
        ...
        range.startContainer.nodeName === 'PRE'
          ? range.startContainer
      ...
      const liParent: boolean =
        !isNOU(preElem) &&
        !isNOU(preElem.parentElement) &&
        preElem.parentElement.tagName === 'LI';
      if (liParent) {
        return;
      }
      if (
        (isNOU(preElem) && !isNOU(endPreElem)) ||
        (!isNOU(preElem) && isNOU(endPreElem))
      ) {
      ...
      if (
        ...
        const startParent: Node = this.getBlockParent(
          range.startContainer,
          blockquoteEle
        );
        if (
          (startParent.textContent.charCodeAt(0) === 8203 &&
            startParent.textContent.length === 1) ||
          (startParent.textContent.length === 0 &&
          ...
          if (isNOU((startParent as HTMLElement).nextElementSibling)) {
            this.paraFocus(
              startParent.parentElement === this.parent.editableElement
                ? (startParent as HTMLElement)
                : startParent.parentElement
            ); //Revert from blockquotes while pressing enter key
          } else {
            const nodeCutter: NodeCutter = new NodeCutter();
            const newElem: Node = nodeCutter
              .SplitNode(range, startParent.parentElement as HTMLElement, false)
              .cloneNode(true);
            this.paraFocus(
              startParent.parentElement === this.parent.editableElement
                ? (startParent as HTMLElement)
      ...
      if (e.event.which === 13 && !isNOU(preElem) && !isNOU(endPreElem)) {
        ...
        this.removeCodeContent(range);
        range = this.parent.nodeSelection.getRange(this.parent.currentDocument);
        const lastEmpty: Node =
          range.startContainer.childNodes[range.endOffset];
        const lastBeforeBr: Node =
          range.startContainer.childNodes[range.endOffset - 1];
        let startParent: Node = range.startContainer;
        if (
          !isNOU(lastEmpty) &&
          !isNOU(lastBeforeBr) &&
          isNOU(lastEmpty.nextSibling) &&
          lastEmpty.nodeName === 'BR' &&
          lastBeforeBr.nodeName === 'BR'
        ) {
          ...
          while (
            startParent.parentElement.nodeName !== 'PRE' &&
            (startParent.textContent.length === 1 ||
              startParent.textContent.length === 0)
          ) {
          ...
          if (
            !isNOU(startParent.previousSibling) &&
            startParent.previousSibling.nodeName === 'BR' &&
  ...
  private removeCodeContent(range: Range): void {
    ...
    if (!isNOU(range.endContainer.textContent.match(regEx))) {
      const pointer: number =
        range.endContainer.textContent.charCodeAt(range.endOffset - 1) === 65279
          ? range.endOffset - 2
          : range.endOffset;
  ...
  private isNotEndCursor(preElem: Element, range: Range): void {
    const nodeCutter: NodeCutter = new NodeCutter();
    const isEnd: boolean =
      range.startOffset === preElem.lastChild.textContent.length &&
      preElem.lastChild.textContent === range.startContainer.textContent;
    //Cursor at start point
    if (
      preElem.textContent.indexOf(range.startContainer.textContent) === 0 &&
      ((range.startOffset === 0 && range.endOffset === 0) ||
        range.startContainer.nodeName === 'PRE')
    ) {
      this.insertMarker(preElem, range);
      const brTag: HTMLElement = createElement('br');
      preElem.childNodes[range.endOffset].parentElement.insertBefore(
        brTag,
        preElem.childNodes[range.endOffset]
      );
    } else {
      //Cursor at middle
      const cloneNode: HTMLElement = nodeCutter.SplitNode(
        range,
        preElem as HTMLElement,
        true
      ) as HTMLElement;
      this.insertMarker(preElem, range);
      ...
      if (previousSib.tagName === 'PRE') {
        previousSib.insertAdjacentHTML(
          'beforeend',
          '<br>' + (cloneNode as HTMLElement).innerHTML
        );
        detach(preElem);
      }
    }
  ...
  private setCursorPosition(isEnd: boolean, preElem: Element): void {
    ...
    const markerElem: Element =
      this.parent.editableElement.querySelector('.tempSpan');
    const mrkParentElem: HTMLElement = markerElem.parentElement;
    // eslint-disable-next-line
    markerElem.parentNode.textContent === ''
      ? (isEmpty = true)
      : this.parent.nodeSelection.setCursorPoint(
          this.parent.currentDocument,
          markerElem,
  ...
  private insertMarker(preElem: Element, range: Range): void {
    const tempSpan: HTMLElement = createElement('span', {
      className: 'tempSpan',
    });
  ...
  private applyFormats(e: IHtmlSubCommands): void {
    ...
    if (
      this.parent.editableElement === range.endContainer &&
      !isNOU(this.parent.editableElement.children[range.endOffset - 1]) &&
      this.parent.editableElement.children[range.endOffset - 1].tagName ===
        'TABLE' &&
      !range.collapsed
    ) {
      isSelectAll = true;
    }
    let save: NodeSelection = this.parent.nodeSelection.save(
      range,
      this.parent.currentDocument
    );
    ...
    if (e.enterAction === 'BR') {
      ...
      const allSelectedNode: Node[] =
        this.parent.nodeSelection.getSelectedNodes(this.parent.currentDocument);
      const selectedNodes: Node[] =
        this.parent.nodeSelection.getSelectionNodes(allSelectedNode);
      ...
      for (let i: number = 0; i < selectedNodes.length; i++) {
        ...
        while (
          !this.parent.domNode.isBlockNode(currentNode as Element) &&
      ...
      for (let i: number = 0; i < currentFormatNodes.length; i++) {
        if (
          !this.parent.domNode.isBlockNode(
            currentFormatNodes[i as number] as Element
          )
        ) {
          ...
          if (
            this.parent.domNode.isBlockNode(previousNode.parentElement) &&
          ...
          while (
            ...
            tempElem.firstChild.parentElement.insertBefore(
              preNode,
              tempElem.firstChild
            );
    ...
    for (let i: number = 0; i < formatsNodes.length; i++) {
      if (
        isNOU(closest(formatsNodes[0], 'blockquote')) ||
    ...
    for (let i: number = 0; i < formatsNodes.length; i++) {
      ...
      if (e.subCommand.toLowerCase() === 'blockquote') {
        ...
        if (e.enterAction === 'BR') {
          ...
          if (
            !isNOU(closest(formatsNodes[i as number], 'table')) &&
            this.parent.editableElement.contains(
              closest(formatsNodes[i as number], 'table')
            )
          ) {
            replaceHTML = !isNOU(
              closest(formatsNodes[i as number], 'blockquote')
            )
      ...
      const replaceNode: string =
        e.subCommand.toLowerCase() === 'pre' &&
        parentNode.tagName.toLowerCase() === 'pre'
          ? 'p'
          : e.subCommand;
      const isToggleBlockquoteList: boolean =
        e.subCommand.toLowerCase() === parentNode.tagName.toLowerCase() &&
        e.subCommand.toLowerCase() === 'blockquote' &&
        this.parent.domNode.isList(
          (parentNode as HTMLElement).firstElementChild
        );
      const ensureNode: Element =
        parentNode.tagName === 'TABLE'
          ? !isNOU(closest(formatsNodes[i as number], 'blockquote'))
            ? closest(formatsNodes[i as number], 'blockquote')
            : parentNode
          : parentNode;
      const isToggleBlockquote: boolean =
        e.subCommand.toLowerCase() === ensureNode.tagName.toLowerCase() &&
        e.subCommand.toLowerCase() === 'blockquote';
      let replaceTag: string;
      if (isToggleBlockquoteList) {
        replaceTag = replaceHTML
          .replace('<blockquote>', '')
          .replace('</blockquote>', '');
      } else if (
        isToggleBlockquote &&
        closest(formatsNodes[0], 'blockquote') &&
        closest(formatsNodes[formatsNodes.length - 1], 'blockquote')
      ) {
        if (
          isNOU(formatsNodes[0].previousSibling) &&
          isNOU(formatsNodes[formatsNodes.length - 1].nextSibling)
        ) {
          replaceTag = replaceHTML.replace(
          ...
          if (i === 0) {
            ...
            formatsNodes[i as number].parentNode.insertBefore(
              tempCloseSpanElem,
              formatsNodes[i as number]
            );
          }
        ...
        replaceTag = this.parent.domNode.createTagString(
          replaceNode,
          e.subCommand.toLowerCase() === 'blockquote' ? null : parentNode,
    ...
    if (isWholeBlockquoteNotSelected) {
      const blockquoteElem: NodeListOf<Element> =
        this.parent.editableElement.querySelectorAll(
          '.e-rte-blockquote-open, .e-rte-blockquote-close'
        );
      for (let i: number = 0; i < blockquoteElem.length; i++) {
        const blockquoteNode: Element =
          blockquoteElem[i as number].parentElement;
        let blockquoteContent: string = blockquoteNode.innerHTML;
        blockquoteContent = blockquoteContent.replace(
          /<span class="e-rte-blockquote-open"><\/span>/g,
          '<blockquote>'
        );
        blockquoteContent = blockquoteContent.replace(
          /<span class="e-rte-blockquote-close"><\/span>/g,
          '</blockquote>'
        );
        if (
          blockquoteElem[0].parentElement === blockquoteElem[1].parentElement
        ) {
          this.parent.domNode.replaceWith(
            blockquoteNode,
            this.parent.domNode.openTagString(blockquoteNode) +
              blockquoteContent.trim() +
              this.parent.domNode.closeTagString(blockquoteNode)
          );
          break;
        } else if (
          i === blockquoteElem.length - 1 &&
          !isNOU(blockquoteElem[i as number]) &&
          !isNOU(blockquoteElem[i - 1]) &&
          blockquoteElem[i as number].parentElement !==
            blockquoteElem[i - 1].parentElement
        ) {
          this.parent.domNode.replaceWith(
            blockquoteNode,
            blockquoteContent.trim()
    ...
    let startNode: Node = this.parent.editableElement.querySelector(
      '.' + markerClassName.startSelection
    );
    let endNode: Node = this.parent.editableElement.querySelector(
  ...
  private preFormatMerge(): void {
    const preNodes: NodeListOf<Element> =
      this.parent.editableElement.querySelectorAll('PRE');
    if (!isNOU(preNodes)) {
      for (let i: number = 0; i < preNodes.length; i++) {
        const previousSib: Element = (preNodes[i as number] as HTMLElement)
          .previousElementSibling;
        if (!isNOU(previousSib) && previousSib.tagName === 'PRE') {
          previousSib.insertAdjacentHTML(
            'beforeend',
            '<br>' + (preNodes[i as number] as HTMLElement).innerHTML
          );
  ...
  private blockquotesFormatMerge(enterAction: string): void {
    const blockquoteNodes: NodeListOf<Element> =
      this.parent.editableElement.querySelectorAll('BLOCKQUOTE');
    if (!isNOU(blockquoteNodes)) {
      for (let i: number = 0; i < blockquoteNodes.length; i++) {
        if (
          (blockquoteNodes[i as number] as HTMLElement).innerHTML.trim() === ''
        ) {
          detach(blockquoteNodes[i as number]);
        }
        const previousSib: Element = (
          blockquoteNodes[i as number] as HTMLElement
        ).previousElementSibling;
        if (!isNOU(previousSib) && previousSib.tagName === 'BLOCKQUOTE') {
          previousSib.insertAdjacentHTML(
            'beforeend',
            (enterAction === 'BR' ? '<br>' : '') +
              (blockquoteNodes[i as number] as HTMLElement).innerHTML
          );
          detach(blockquoteNodes[i as number]);
        }
      }
