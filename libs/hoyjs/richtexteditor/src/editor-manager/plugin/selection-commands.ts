...
export class SelectionCommands {
  ...
  public static applyFormat(
    ...
    endNode: Node,
    enterAction: string,
    tableCellSelection?: ITableSelection,
    value?: string,
    selector?: string,
    ...
    if (validFormats.indexOf(format) > -1 || value === 'formatPainter') {
      ...
      if (
        range.collapsed &&
        !isNOU(currentAnchorNode) &&
        currentAnchorNode.tagName === 'A' &&
        (range.startOffset === currentAnchorNode.textContent.length ||
          range.startOffset === 0)
      ) {
        const emptyTextNode: Node = document.createTextNode('');
        if (range.startOffset === 0) {
          currentAnchorNode.parentNode.insertBefore(
            emptyTextNode,
            currentAnchorNode
          );
        } else {
          if (!isNOU(currentAnchorNode.nextSibling)) {
            currentAnchorNode.parentElement.insertBefore(
              emptyTextNode,
              currentAnchorNode.nextSibling
            );
          } else {
      ...
      if (
        ...
        const startChildNodes: NodeListOf<Node> =
        ...
        let lastSelectionNode: Element = <Element>(
      ...
      if (nodes && nodes.length > 0) {
        ...
        nodes = range.collapsed
          ? domSelection.getSelectionNodeCollection(range)
      ...
      const isFontStyle: boolean =
        ['fontcolor', 'fontname', 'fontsize', 'backgroundcolor'].indexOf(
          format
        ) > -1;
      if (!isTableSelect && range.collapsed) {
        const currentFormatNode: Node = isFormatted.getFormattedNode(
          range.startContainer,
          format,
          endNode
        );
        const currentSelector: string = !isNOU(currentFormatNode)
          ? (currentFormatNode as HTMLElement).getAttribute('style') === null
            ? currentFormatNode.nodeName
            : currentFormatNode.nodeName +
              "[style='" +
              (currentFormatNode as HTMLElement).getAttribute('style') +
              "']"
          : null;
        if (nodes.length > 0) {
          isCollapsed = true;
          ...
          ((range.startContainer.parentElement.childElementCount > 0 &&
            range.startOffset > 0 &&
            range.startContainer.parentElement.firstElementChild.tagName.toLowerCase() !==
              'br') ||
            (!isNOU(currentFormatNode) &&
              (currentFormatNode as HTMLElement) ===
          ...
          isCollapsed = true;
          range = nodeCutter.GetCursorRange(
            docElement,
            range,
            range.startContainer
          );
          ...
          domSelection.endContainer = domSelection.startContainer =
            domSelection.getNodeArray(cursorNode, true);
          const childNodes: NodeListOf<Node> =
            cursorNode.nodeName === 'BR' && cursorNode.parentNode.childNodes;
          if (
            !isNOU(childNodes) &&
            childNodes.length === 1 &&
          ...
          if (
            ...
            cursorNode.parentNode.textContent.length === 0
          ) {
            preventRestore = true;
          }
        }
      ...
      for (let index: number = 0; index < nodes.length; index++) {
        let formatNode: Node = isFormatted.getFormattedNode(
        ...
        if (formatNode === null) {
          if (format === 'subscript') {
            formatNode = isFormatted.getFormattedNode(
              nodes[index as number],
              'superscript',
              endNode
            );
        ...
        if (!isTableSelect) {
          domSelection = this.applySelection(
            nodes,
            domSelection,
            nodeCutter,
            index,
  ...
  private static insertCursorNode(
    ...
    const cursorFormat: Node =
      cursorNodes.length > 0
        ? cursorNodes.length > 1 && range.startContainer === range.endContainer
          ...
          : value === '' &&
            format === 'fontsize' &&
            isFormatted.getFormattedNode(cursorNodes[0], format, endNode) ==
              null &&
            cursorNodes[0].parentElement.nodeName === 'SPAN'
          ? cursorNodes[0].parentElement
          : isFormatted.getFormattedNode(cursorNodes[0], format, endNode)
        : null;
    ...
    if (cursorFormat) {
      ...
      if (
        cursorFormat.firstChild.textContent.charCodeAt(0) === 8203 &&
        cursorFormat.firstChild.nodeType === 3
      ) {
        ...
        if (cursorFormat.firstChild === cursorNode) {
          cursorNode.textContent =
            cursorFormat.parentElement &&
            (domNode.isBlockNode(cursorFormat.parentElement) &&
            cursorFormat.parentElement.textContent.length <= 1
              ? cursorFormat.parentElement.childElementCount > 1
              : (cursorFormat as HTMLElement).childElementCount === 0) &&
            (cursorFormat.parentElement.textContent.length > 1 ||
              (cursorFormat.parentElement.firstChild &&
                cursorFormat.parentElement.firstChild.nodeType === 1))
              ? cursorNode.textContent
              : cursorNode.textContent.replace(regEx, '');
          emptySpaceNode = cursorNode;
        } else {
          cursorFormat.firstChild.textContent =
            cursorFormat.firstChild.textContent.replace(regEx, '');
        ...
        if (emptySpaceNode.textContent.length === 0) {
          if (!isNOU(emptySpaceNode.previousSibling)) {
            ...
            domSelection.setCursorPoint(
              docElement,
              emptySpaceNode as Element,
              pointer
            );
          } else if (
            !isNOU(emptySpaceNode.parentElement) &&
            emptySpaceNode.parentElement.textContent.length === 0
          ) {
      ...
      if (
        cursorNodes.length > 1 &&
        range.startOffset > 0 &&
        (cursorNodes[0] as HTMLElement).firstElementChild &&
        (
          cursorNodes[0] as HTMLElement
        ).firstElementChild.tagName.toLowerCase() === 'br'
      ) {
      ...
      if (
        cursorNodes.length === 1 &&
        range.startOffset === 0 &&
        (cursorNodes[0].nodeName === 'BR' ||
          (isNOU(cursorNodes[0].nextSibling)
            ? false
            : cursorNodes[0].nextSibling.nodeName === 'BR'))
      ) {
        detach(
          cursorNodes[0].nodeName === '#text'
            ? cursorNodes[0].nextSibling
            : cursorNodes[0]
        );
      }
      if (
        !isNOU(cursorNodes[0].parentElement) &&
        IsFormatted.inlineTags.indexOf(
          cursorNodes[0].parentElement.tagName.toLowerCase()
        ) !== -1 &&
        cursorNodes[0].textContent.includes('\u200B')
      ) {
        ...
        if (format === 'fontsize') {
          ...
          while (currentFormatNode) {
            const isSameTextContent: boolean =
              currentFormatNode.parentElement.textContent.trim() ===
              cursorNodes[0].textContent.trim();
            const previousElement: HTMLElement =
              currentFormatNode.parentElement;
            if (
              !domNode.isBlockNode(previousElement) &&
              isSameTextContent &&
              !(
                previousElement.nodeName === 'SPAN' &&
                (previousElement as HTMLElement).classList.contains(
                  'e-img-inner'
                )
              )
            ) {
  ...
  private static getCursorFormat(
    ...
    for (let index: number = 0; index < cursorNodes.length; index++) {
      currentNode = (cursorNodes[index as number] as HTMLElement)
        .lastElementChild
        ? (cursorNodes[index as number] as HTMLElement).lastElementChild
        : cursorNodes[index as number];
    }
    return format === 'fontsize' &&
      isFormatted.getFormattedNode(currentNode, format, endNode) == null &&
      currentNode.parentElement.nodeName === 'SPAN'
      ? currentNode.parentElement
      : isFormatted.getFormattedNode(currentNode, format, endNode);
  }
  ...
  private static removeFormat(
    ...
    if (
      ...
      if (nodes[index as number].nodeName !== 'BR') {
        if (
          clonedElement.nodeName === '#text' &&
          clonedElement.textContent.includes('\u200B')
        ) {
          (clonedElement as Element).remove();
        }
        if (!isTableCell) {
          cloneNode = splitNode =
            isCursor && formatNode.textContent.length - 1 === range.startOffset
      ...
      if (!isCursor) {
        while (
          cloneNode &&
          cloneNode.childNodes.length > 0 &&
          nodeIndex.length - 1 >= 0 &&
        ...
        if (nodes[index as number].nodeName !== 'BR') {
          if (
            cloneNode.nodeType === 3 &&
            !(isCursor && cloneNode.nodeValue === '')
          ) {
        ...
        for (
          ;
          lastNode.firstChild !== null && lastNode.firstChild.nodeType !== 3;
          null
        ) {
      ...
      isFontStyle &&
      !nodes[index as number].contains(formatNode) &&
      nodes[index as number].nodeType === 3 &&
      nodes[index as number].textContent !== formatNode.textContent
    ) {
      // If the selection is within the format node .
      const isFullNodeSelected: boolean =
        nodes[index as number].textContent ===
        (nodes[index as number] as Text).wholeText;
      ...
      while (
        nodeTraverse &&
        nodeTraverse.textContent === nodeTraverse.parentElement.textContent
      ) {
        nodeTraverse = nodeTraverse.parentElement;
      }
      if (
        isFullNodeSelected &&
        formatNode.textContent !== nodeTraverse.textContent
      ) {
        const nodeArray: Node[] = [];
        const priorityNode: Node = this.getPriorityFormatNode(
          nodeTraverse,
          endNode
        );
        if (
          priorityNode &&
          priorityNode.textContent === nodeTraverse.textContent
        ) {
    ...
    let bgStyle: string;
    if (format === 'fontsize') {
      const bg: Element = closest(
        nodes[index as number].parentElement,
    ...
    const formatNodeStyles: string = (formatNode as HTMLElement).getAttribute(
      'style'
    );
    ...
    if (formatNodeTagName === 'A' && format === 'underline') {
      ...
      IsFormatted.inlineTags.indexOf(formatNodeTagName.toLowerCase()) !== -1 &&
      isFontStyle &&
      formatNodeTagName.toLocaleLowerCase() !== 'span'
    ) {
      const fontNodeStyle: CSSStyleDeclaration = (formatNode as HTMLElement)
        .style;
      ...
      if (
        !isNOU(liElement) &&
        liElement.tagName.toLowerCase() === 'li' &&
        ...
        SelectionCommands.conCatenateTextNode(
          liElement,
          format,
          '',
          'normal',
    ...
    if (child[0] && !isFontStyle) {
      let nodeTraverse: Node = child[index as number]
        ? child[index as number]
        : child[0];
      ...
      for (
        ...
        nodeTraverse.parentElement !== endNode;
        // eslint-disable-next-line
        nodeTraverse = nodeTraverse
      ) {
        let nodeTraverseCondition: boolean;
        if (formatNode.nodeName === 'SPAN') {
          nodeTraverseCondition =
            nodeTraverse.parentElement.tagName.toLocaleLowerCase() ===
              (formatNode as HTMLElement).tagName.toLocaleLowerCase() &&
            nodeTraverse.parentElement.getAttribute('style') ===
              formatNodeStyles;
        } else {
          nodeTraverseCondition =
            nodeTraverse.parentElement.tagName.toLocaleLowerCase() ===
        ...
        if (
          nodeTraverse.parentElement &&
          nodeTraverseCondition &&
          (nodeTraverse.parentElement.childElementCount > 1 ||
            range.startOffset > 1)
        ) {
          if (
            textNode.parentElement &&
            textNode.parentElement.tagName.toLocaleLowerCase() ===
              (formatNode as HTMLElement).tagName.toLocaleLowerCase()
          ) {
            if (
              range.startOffset === range.endOffset &&
              textNode.nodeType !== 1 &&
              !isNOU(textNode.textContent) &&
              textNode.parentElement.childElementCount > 1
            ) {
          ...
          if (
            nodeTraverse.parentElement.tagName.toLocaleLowerCase() === 'span'
          ) {
            if (
              (formatNode as HTMLElement).style.textDecoration ===
                'underline' &&
          ...
          nodeTraverse =
            !isNOU(nodeTraverse.parentElement) &&
            !domNode.isBlockNode(nodeTraverse.parentElement)
              ? textNode
              : nodeTraverse.parentElement;
        } else {
          nodeTraverse = nodeTraverse.parentElement;
    ...
    if (
      ...
      for (let num: number = 0; num < child.length; num++) {
        if (
          child[num as number].nodeType !== 3 ||
          (child[num as number].textContent &&
      ...
      if (
        ...
        curParentElem.parentElement.insertBefore(
          currentNodeElem,
          curParentElem
        );
      ...
      if (format === 'fontsize' || format === 'fontcolor') {
        ...
        while (
          ...
          !isNOU(liElement) &&
          liElement.tagName.toLowerCase() === 'li' &&
          (liElement as Node).contains(nodes[num as number]) &&
          liElement.textContent
            .replace('/\u200B/g', '')
            .trim()
            .includes(nodes[num as number].textContent.trim())
        ) {
          /* eslint-enable security/detect-object-injection */
          liChildContent =
            ' ' + nodes[num as number].textContent.trim() + liChildContent;
          num--;
        }
        ...
        if (!isNOU(liElement) && liElement.childNodes) {
          for (let num: number = 0; num < liElement.childNodes.length; num++) {
            if (
              liElement.childNodes[num as number].nodeName === ('OL' || 'UL')
            ) {
              nestedListCount++;
              isNestedList = true;
            }
        ...
        if (
          !isNOU(liElement) &&
          liElement.tagName.toLowerCase() === 'li' &&
          liElement.textContent.split('\u200B').join('').trim() ===
            liChildContent.split('\u200B').join('').trim()
        ) {
          if (format === 'fontsize') {
          ...
          if (isNestedList && nestedListCount > 0) {
            for (
              let num: number = 0;
              num < liElement.childNodes.length;
              num++
            ) {
              if (
                nodes[index as number].textContent ===
                  liElement.childNodes[num as number].textContent &&
                nodes[index as number].textContent === nodeText &&
                liElement.textContent
                  .replace('/\u200B/g', '')
                  .trim()
                  .includes(liChildContent.split('\u200B').join('').trim())
              ) {
          ...
          if (isNestedListItem) {
            for (
              ...
              if (
                ...
                (
                  liElement.childNodes[num as number] as HTMLElement
    ...
    if (!isCursor) {
      if ((formatNode === null && isFormat) || isFontStyle) {
        if (!isTableSelect && nodes[index as number].nodeName !== 'BR') {
          nodes[index as number] = nodeCutter.GetSpliceNode(
            range,
            nodes[index as number] as HTMLElement
          );
          nodes[index as number].textContent = nodeCutter.TrimLineBreak(
            (nodes[index as number] as Text).textContent
          );
        }
        if (format === 'uppercase' || format === 'lowercase') {
          nodes[index as number].textContent =
            format === 'uppercase'
              ? nodes[index as number].textContent.toLocaleUpperCase()
          ...
          if (value === 'formatPainter' || isFontStyle) {
            ...
            if (
              ...
              if (!isNOU(parentElem) && parentElem.childNodes) {
                for (let i: number = 0; i < parentElem.childNodes.length; i++) {
                  if (
                    this.concatenateTextExcludingList(nodes, index) ===
                    nodes[index as number].textContent
                  ) {
                  ...
                  const childElement: HTMLElement = parentElem.childNodes[
                    i as number
                  ] as HTMLElement;
                  if (
            ...
            if (value === 'formatPainter') {
              return this.insertFormatPainterElem(
                nodes,
                index,
                range,
                nodeCutter,
            ...
            if (!isNOU(priorityNode) || isFontStyle) {
              let currentFormatNode: Node = isNOU(priorityNode)
                ? currentNode
                : priorityNode;
              currentFormatNode =
                !isNOU(priorityNode) &&
                (priorityNode as HTMLElement).style.fontSize !== ''
                  ? (currentFormatNode.firstChild as Node)
                  : currentFormatNode;
              if (isNOU(priorityNode) || format === 'fontsize') {
                while (currentFormatNode) {
                  ...
                  if (
                    !domNode.isBlockNode(parent) &&
                    isSameTextContent &&
                    !(
                      parent.nodeName === 'SPAN' &&
                      (parent as HTMLElement).classList.contains('e-img-inner')
                    )
                  ) {
              ...
              if (format === 'fontcolor') {
                const closestAnchor: Node = closest(
                  nodes[index as number].parentElement,
                  'A'
                );
                if (
                  !isNOU(closestAnchor) &&
                  closestAnchor.firstChild.textContent.trim() ===
                    nodes[index as number].textContent.trim()
                ) {
              ...
              if (
                nodes[index as number].textContent.trim() !==
                currentFormatNode.textContent.trim()
              ) {
                currentFormatNode = nodes[index as number];
              }
      ...
      if (format !== 'uppercase' && format !== 'lowercase') {
        const element: HTMLElement = this.getInsertNode(
          docElement,
          range,
          format,
          value
  ...
  private static applyStyles(
    ...
    if (
      !(nodes[index as number].nodeName === 'BR' && this.enterAction === 'BR')
    ) {
      nodes[index as number] =
        index === nodes.length - 1 || nodes[index as number].nodeName === 'BR'
          ? InsertMethods.Wrap(nodes[index as number] as HTMLElement, element)
          : InsertMethods.WrapBefore(
              nodes[index as number] as Text,
              element,
              true
            );
      nodes[index as number] = this.getChildNode(
        nodes[index as number],
        element
      );
    }
  ...
  private static getPriorityFormatNode(node: Node, endNode: Node): Node | null {
    const isFormatted: IsFormatted = new IsFormatted();
    const fontSizeNode: Node = isFormatted.getFormattedNode(
      node,
      'fontsize',
      endNode
    );
    ...
    if (isNOU(fontSizeNode)) {
      backgroundColorNode = isFormatted.getFormattedNode(
        node,
        'backgroundcolor',
        endNode
      );
      if (isNOU(backgroundColorNode)) {
        ...
        if (isNOU(fontNameNode)) {
          fontColorNode = isFormatted.getFormattedNode(
            node,
            'fontcolor',
            endNode
          );
  ...
  private static insertFormatPainterElem(
    ...
    let parent: HTMLElement = !domNode.isBlockNode(
      nodes[index as number].parentElement
    )
      ? (nodes[index as number].parentElement as HTMLElement)
      : (nodes[index as number] as HTMLElement);
    if (!domNode.isBlockNode(parent)) {
      while (
        parent.textContent.trim() === parent.parentElement.textContent.trim() &&
        !domNode.isBlockNode(parent.parentElement)
      ) {
    ...
    if (
      ...
      parent.nodeType === 1 &&
      !(
        parent.classList.contains('e-rte-img-caption') ||
        parent.classList.contains('e-img-inner')
      )
    ) {
      this.formatPainterCleanup(
        index,
        nodes,
        parent,
        range,
    ...
    if (!isNOU(elem)) {
      ...
      ).querySelectorAll(painterValues.lastChild.nodeName);
      ...
      if (elemList.length > 0) {
        ...
        if (
          !isNOU(clonedElement) &&
          clonedElement.nodeName === painterValues.lastChild.nodeName
        ) {
      ...
      nodes[index as number] = InsertMethods.Wrap(
        nodes[index as number] as HTMLElement,
        clonedElement as HTMLElement
      );
      nodes[index as number].textContent = '';
  ...
  private static formatPainterCleanup(
    ...
    if (
      index === 0 &&
      parent.textContent.trim() !== nodes[index as number].textContent.trim()
    ) {
      nodeCutter.SplitNode(range, parent, true);
      const childELemList: NodeList =
        nodes[index as number].parentElement.childNodes;
      for (let i: number = 0; i < childELemList.length; i++) {
        if (
          childELemList[i as number].textContent.trim() ===
          nodes[i as number].textContent.trim()
        ) {
      ...
      for (let k: number = 0; k < blockChildNodes.length; k++) {
        if (
          (blockChildNodes[k as number].textContent.trim() === '' ||
            blockChildNodes[k as number].textContent.length === 0) &&
      ...
      while (
        ...
        for (let i: number = 0; i < parent.childNodes.length; i++) {
          ...
          if (
            currentChild.textContent.trim().length !== 0 &&
  ...
  private static concatenateTextExcludingList(
    ...
    const parentNode: HTMLElement =
      nodes[index as number].nodeName === '#text'
        ? (closest(nodes[index as number].parentElement, 'li') as HTMLElement)
        : (closest(nodes[index as number], 'li') as HTMLElement);
    if (!isNOU(parentNode)) {
      for (let i: number = 0; i < parentNode.childNodes.length; i++) {
        const childNode: HTMLElement = parentNode.childNodes[
          i as number
        ] as HTMLElement;
        if (
    ...
    switch (format) {
      case 'bold':
        liElement
          ...
          .forEach(function (e: HTMLElement): void {
      ...
      case 'fontcolor':
        liElement
          ...
          .forEach(function (span: HTMLElement): void {
