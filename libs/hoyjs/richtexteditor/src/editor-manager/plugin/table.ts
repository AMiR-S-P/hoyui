...
export class TableCommand {
  ...
  private addEventListener(): void {
    ...
    this.parent.observer.on(
      CONSTANT.TABLE_VERTICAL_ALIGN,
      this.tableVerticalAlign,
      this
    );
  ...
  private createTable(e: IHtmlItem): HTMLElement {
    ...
    const tdWid: number =
      parseInt(e.item.width.width as string, 10) > 100
        ? 100 / e.item.columns
        : parseInt(e.item.width.width as string, 10) / e.item.columns;
    ...
    if (
      ...
      !table.classList.contains('ignore-table')
    ) {
  ...
  private getSelectedCellMinMaxIndex(e: HTMLElement[][]): MinMax {
    const selectedCells: NodeListOf<HTMLElement> =
      this.curTable.querySelectorAll('.e-cell-select');
    let a: number = 0;
    ...
    for (let i: number = 0; a < selectedCells.length; a++) {
      const selectedCellIndex: number[] = this.getCorrespondingIndex(
        selectedCells[a as number],
        e
      );
      const minMaxIndex: number[] = this.FindIndex(
        selectedCellIndex[0],
        selectedCellIndex[1],
        e
      );
  ...
  private insertRow(e: IHtmlItem): void {
    const isBelow: boolean =
      e.item.subCommand === 'InsertRowBefore' ? false : true;
    let selectedCell: HTMLElement = e.item.selection.range
    ...
    if (
      selectedCell.nodeName.toLowerCase() === 'th' &&
      e.item.subCommand === 'InsertRowBefore'
    ) {
      return;
    }
    this.curTable = closest(
      this.parent.nodeSelection.range.startContainer.parentElement,
      'table'
    ) as HTMLTableElement;
    if (this.curTable.querySelectorAll('.e-cell-select').length === 0) {
      const lastRow: Element =
        this.curTable.rows[this.curTable.rows.length - 1];
      ...
      const minVal: number = isBelow
        ? minMaxIndex.endRow
        : minMaxIndex.startRow;
      const newRow: Element = createElement('tr');
      const isHeaderSelect: boolean =
        this.curTable.querySelectorAll('th.e-cell-select').length > 0;
      for (let i: number = 0; i < allCells[minVal as number].length; i++) {
        // eslint-disable-next-line max-len
        if (
          (isBelow &&
            minVal < allCells.length - 1 &&
            allCells[minVal as number][i as number] ===
              allCells[minVal + 1][i as number]) ||
          (!isBelow &&
            0 < minVal &&
            allCells[minVal as number][i as number] ===
              allCells[minVal - 1][i as number])
          ...
          if (
            ...
            (0 < i &&
              allCells[minVal as number][i as number] !==
                allCells[minVal as number][i - 1])
          ) {
            allCells[minVal as number][i as number].setAttribute(
              'rowspan',
              (
                parseInt(
                  allCells[minVal as number][i as number].getAttribute(
                    'rowspan'
                  ),
                  10
                ) + 1
          ...
          tdElement.setAttribute(
            'style',
            allCells[
              isHeaderSelect && isBelow
                ? allCells[minVal + 1]
                  ? minVal + 1
                  : minVal
                : minVal
            ][i as number].getAttribute('style')
          );
        }
      }
      // eslint-disable-next-line
    ...
    if (e.callBack) {
      e.callBack({
        ...
        event: e.event,
        range: this.parent.nodeSelection.getRange(this.parent.currentDocument),
        elements: this.parent.nodeSelection.getSelectedNodes(
          this.parent.currentDocument
        ) as Element[],
      });
    }
  ...
  private insertColumn(e: IHtmlItem): void {
    ...
    const curRow: HTMLElement = closest(
      selectedCell as HTMLElement,
      'tr'
    ) as HTMLElement;
    let curCell: Element;
    const allRows: HTMLCollectionOf<HTMLTableRowElement> = (
      closest(curRow, 'table') as HTMLTableElement
    ).rows;
    const colIndex: number = Array.prototype.slice
      .call(
        (curRow as HTMLElement).querySelectorAll(':scope > td, :scope > th')
      )
      .indexOf(selectedCell);
    const previousWidth: number =
      parseInt(e.item.width as string, 10) /
      curRow.querySelectorAll(':scope > td, :scope > th').length;
    const currentWidth: number =
      parseInt(e.item.width as string, 10) /
      (curRow.querySelectorAll(':scope > td, :scope > th').length + 1);
    const currentTabElm: Element = closest(curRow as HTMLElement, 'table');
    const thTdElm: NodeListOf<HTMLElement> = closest(
      curRow as HTMLElement,
      'table'
    ).querySelectorAll('th,td');
    for (let i: number = 0; i < thTdElm.length; i++) {
      thTdElm[i as number].dataset.oldWidth =
        (thTdElm[i as number].offsetWidth /
          (currentTabElm as HTMLElement).offsetWidth) *
          100 +
        '%';
    }
    for (let i: number = 0; i < allRows.length; i++) {
      curCell = allRows[i as number].querySelectorAll(
        ':scope > td, :scope > th'
      )[colIndex as number];
    ...
    for (let i: number = 0; i < thTdElm.length; i++) {
      thTdElm[i as number].style.width =
        (
          (Number(thTdElm[i as number].dataset.oldWidth.split('%')[0]) *
            currentWidth) /
          previousWidth
        ).toFixed(4) + '%';
  ...
  private deleteColumn(e: IHtmlItem): void {
    ...
    const tBodyHeadEle: Element = closest(
      selectedCell,
      selectedCell.tagName === 'TH' ? 'thead' : 'tbody'
    );
    const rowIndex: number =
      tBodyHeadEle &&
      Array.prototype.indexOf.call(
        tBodyHeadEle.childNodes,
        selectedCell.parentNode
      );
    this.curTable = closest(selectedCell, 'table') as HTMLTableElement;
    const curRow: HTMLTableRowElement = closest(
      selectedCell as HTMLElement,
      'tr'
    ...
    if (curRow.querySelectorAll('th,td').length === 1) {
      ...
      const allCells: HTMLElement[][] = this.getCorrespondingColumns();
      //eslint-disable-next-line
      const selectedMinMaxIndex: any =
        this.getSelectedCellMinMaxIndex(allCells);
      ...
      for (let i: number = 0; i < allCells.length; i++) {
        ...
        for (let j: number = 0; j < currentRow.length; j++) {
          ...
          //eslint-disable-next-line
          const currentCellIndex: any = this.getCorrespondingIndex(
            currentCell,
            allCells
          );
          const colSpanVal: number =
            parseInt(currentCell.getAttribute('colspan'), 10) || 1;
          if (
            currentCellIndex[1] + (colSpanVal - 1) >= minCol &&
            currentCellIndex[1] <= maxCol
          ) {
            if (colSpanVal > 1) {
              ...
              if (Browser.isIE) {
                e.item.selection.setSelectionText(
                  this.parent.currentDocument,
                  this.curTable.querySelector('td'),
                  this.curTable.querySelector('td'),
                  0,
                  0
                );
      ...
      if (deleteIndex > -1) {
        ...
        const nextFocusCell: HTMLElement =
          rowHeadEle &&
          (rowHeadEle.children[
            deleteIndex <= rowHeadEle.children.length - 1
              ? deleteIndex
              : deleteIndex - 1
          ] as HTMLElement);
        if (nextFocusCell) {
    ...
    if (e.callBack) {
      const sContainer: Node = this.parent.nodeSelection.getRange(
        this.parent.currentDocument
      ).startContainer;
      if (sContainer.nodeName !== 'TD') {
        const startChildLength: number = this.parent.nodeSelection.getRange(
          this.parent.currentDocument
        ).startOffset;
        const focusNode: Element = (sContainer as HTMLElement).children[
          startChildLength as number
        ];
        if (focusNode) {
  ...
  private deleteRow(e: IHtmlItem): void {
    ...
    const colIndex: number = Array.prototype.indexOf.call(
      selectedCell.parentNode.childNodes,
      selectedCell
    );
    this.curTable = closest(selectedCell, 'table') as HTMLTableElement;
    ...
    if (this.curTable.rows.length === 1) {
      ...
      for (maxI = minMaxIndex.endRow; maxI >= minMaxIndex.startRow; maxI--) {
        ...
        for (j = 0; j < allCells[maxI as number].length; j++) {
          if (
            j === 0 ||
            allCells[maxI as number][j as number] !==
              allCells[maxI as number][j - 1]
          ) {
            if (
              1 <
              parseInt(
                allCells[maxI as number][j as number].getAttribute('rowspan'),
                10
              )
            ) {
              const rowSpanVal: number =
                parseInt(
                  allCells[maxI as number][j as number].getAttribute('rowspan'),
                  10
                ) - 1;
              if (1 === rowSpanVal) {
                allCells[maxI as number][j as number].removeAttribute(
                  'rowspan'
                );
                const cell: HTMLElement = this.getMergedRow(
                  this.getCorrespondingColumns() as HTMLElement[][]
                )[j as number];
                if (cell) {
                ...
                allCells[maxI as number][j as number].setAttribute(
                  'rowspan',
                  rowSpanVal.toString()
                );
              }
          ...
          // eslint-disable-next-line max-len
          if (
            maxI < allCells.length - 1 &&
            allCells[maxI as number][j as number] ===
              allCells[maxI + 1][j as number] &&
            (0 === maxI ||
              allCells[maxI as number][j as number] !==
                allCells[maxI - 1][j as number])
          ) {
            ...
            for (
              index = j;
              0 < index &&
              allCells[maxI as number][index as number] ===
                allCells[maxI as number][index - 1];
              index--
            ) {
              if (index === 0) {
                (this.curTable.rows[maxI + 1] as HTMLElement).prepend(element);
              } else {
                allCells[maxI + 1][index - 1].insertAdjacentElement(
                  'afterend',
                  element
                );
              }
        ...
        const focusTrEle: Element = !isNOU(
        ...
        const nextFocusCell: HTMLElement =
          focusTrEle && focusTrEle.querySelectorAll('td')[colIndex as number];
        if (nextFocusCell) {
          e.item.selection.setSelectionText(
            this.parent.currentDocument,
            nextFocusCell,
  ...
  private getMergedRow(cells: HTMLElement[][]): HTMLElement[] {
    ...
    for (let i: number = 0; i < cells.length; i++) {
      if (
        cells[i as number].length !== this.curTable.rows[0].childNodes.length
      ) {
  ...
  private removeTable(e: IHtmlItem): void {
    ...
    selectedCell =
      selectedCell.nodeType === 3 ? selectedCell.parentNode : selectedCell;
    const selectedTable: HTMLElement = closest(
      selectedCell.parentElement,
      'table'
    ) as HTMLElement;
    if (selectedTable) {
      ...
      for (let i: number = 0; i < cellCount; i++) {
        colSpanCount =
          ...
          (parseInt(
            table
              .querySelector('tr')
              .children[i as number].getAttribute('colspan'),
            10
          ) || 1);
      }
  ...
  private cellMerge(e: IHtmlItem): void {
    ...
    const minMaxIndexes: MinMax = this.getSelectedMinMaxIndexes(
      this.getCorrespondingColumns()
    );

    const firstCell: Node = selectedCells[0];
    const rowSelectedCells: NodeListOf<HTMLElement> =
      firstCell.parentElement.querySelectorAll('.e-cell-select');
    ...
    if (minMaxIndexes.startColumn < minMaxIndexes.endColumn) {
      (firstCell as HTMLElement).setAttribute(
        'colspan',
        (minMaxIndexes.endColumn - minMaxIndexes.startColumn + 1).toString()
      );
    }
    if (minMaxIndexes.startRow < minMaxIndexes.endRow) {
      (firstCell as HTMLElement).setAttribute(
        'rowspan',
        (minMaxIndexes.endRow - minMaxIndexes.startRow + 1).toString()
      );
    }
    ...
    for (let j: number = rowSelectedCells.length - 1; j >= 0; j--) {
      if (
        !isNOU((rowSelectedCells[j as number] as HTMLElement).style.width) &&
        (rowSelectedCells[j as number] as HTMLElement).style.width !== ''
      ) {
        if (!unit) {
          const match: RegExpMatchArray =
            rowSelectedCells[j as number].style.width.match(
              /^([\d.]+)([a-z%]+)$/i
            );
          unit = match ? match[2] : '%';
        }
        ...
        totalWidth =
          totalWidth +
          ((rowSelectedCells[j as number] as HTMLElement).offsetWidth /
    ...
    removeClassWithAttr(
      this.curTable.querySelectorAll('table td, table th'),
      'e-multi-cells-select'
    );
    removeClassWithAttr(
      this.curTable.querySelectorAll('table td, table th'),
      'e-cell-select-end'
    );
    this.updateRowSpanStyle(
      minMaxIndexes.startRow,
      minMaxIndexes.endRow,
      this.getCorrespondingColumns()
    );
    this.updateColSpanStyle(
      minMaxIndexes.startColumn,
      minMaxIndexes.endColumn,
      this.getCorrespondingColumns()
    );
    e.item.selection.setSelectionText(
      this.parent.currentDocument,
  ...
  private updateColSpanStyle(
    ...
    if (min < (max = Math.min(max, eleArray[0].length - 1))) {
      for (colIndex = min; colIndex <= max; colIndex++) {
        index = Math.min(
          parseInt(
            eleArray[0][colIndex as number].getAttribute('colspan'),
            10
          ) || 1,
          max - min + 1
        );
        if (
          ...
          for (
            count = index - 1, colValue = 1;
            colValue < eleArray.length;
            colValue++
          ) {
            if (
              eleArray[colValue as number][colIndex as number] !==
              eleArray[colValue - 1][colIndex as number]
            ) {
              for (
                colMin = colIndex as number;
                colMin < colIndex + index;
                colMin++
              ) {
                attrValue =
                  parseInt(
                    eleArray[colValue as number][colMin as number].getAttribute(
                      'colspan'
                    ),
                    10
                  ) || 1;
                if (
                  1 < attrValue &&
                  eleArray[colValue as number][colMin as number] ===
                    eleArray[colValue as number][colMin + 1]
                ) {
    ...
    if (min < (max = Math.min(max, eleArray.length - 1))) {
      for (rowValue = min; rowValue <= max; rowValue++) {
        if (
          ...
          eleArray[rowValue as number][0] &&
          1 <
            (index = Math.min(
              parseInt(
                eleArray[rowValue as number][0].getAttribute('rowspan'),
                10
              ) || 1,
              max - min + 1
            )) &&
          ...
          for (
            count = index - 1, colIndex = 1;
            colIndex < eleArray[0].length;
            colIndex++
          ) {
            if (
              eleArray[rowValue as number][colIndex as number] !==
              eleArray[rowValue as number][colIndex - 1]
            ) {
              for (rowMin = rowValue; rowMin < rowValue + index; rowMin++) {
                attrValue =
                  parseInt(
                    eleArray[rowMin as number][colIndex as number].getAttribute(
                      'rowspan'
                    ),
                    10
                  ) || 1;
                if (
                  1 < attrValue &&
                  eleArray[rowMin as number][colIndex as number] ===
  ...
  private updateCellAttribute(
    ...
    for (rowIndex = min; rowIndex <= max; rowIndex++) {
      for (colIndex = firstIndex; colIndex <= length; colIndex++) {
        spanCount =
          parseInt(
            elements[rowIndex as number][colIndex as number].getAttribute(attr),
            10
          ) || 1;
        if (
          (min < rowIndex &&
            elements[rowIndex as number][colIndex as number] ===
              elements[rowIndex - 1][colIndex as number]) ||
          (firstIndex < colIndex &&
            elements[rowIndex as number][colIndex as number] ===
              elements[rowIndex as number][colIndex - 1]) ||
          1 < spanCount
        ) {
          if (1 < spanCount - index) {
            elements[rowIndex as number][colIndex as number].setAttribute(
              attr,
              (spanCount - index).toString()
            );
          } else {
            elements[rowIndex as number][colIndex as number].removeAttribute(
              attr
            );
          }
        }
  ...
  private mergeCellContent(): void {
    ...
    let innerHtml: string =
      selectedCells[0].innerHTML === '<br>' ? '' : selectedCells[0].innerHTML;
    for (let i: number = 1; i < selectedCells.length; i++) {
      if ('<br>' !== selectedCells[i as number].innerHTML) {
        innerHtml = innerHtml
          ? innerHtml + '<br>' + selectedCells[i as number].innerHTML
          : innerHtml + selectedCells[i as number].innerHTML;
      }
    }
  ...
  private getSelectedMinMaxIndexes(
    ...
    if (0 < selectedCells.length) {
      ...
      for (let i: number = 0; i < selectedCells.length; i++) {
        const currentRowCol: number[] = this.getCorrespondingIndex(
          selectedCells[i as number] as HTMLElement,
          correspondingCells
        );
        const targetRowCol: number[] = this.FindIndex(
          currentRowCol[0],
          currentRowCol[1],
          correspondingCells
        );
  ...
  private HorizontalSplit(e: IHtmlItem): void {
    ...
    if (
      (this.curTable as HTMLElement).querySelectorAll('.e-cell-select').length >
      1
    ) {
      return;
    }
    ...
    const activeCellIndex: number[] = this.getCorrespondingIndex(
      this.activeCell,
      this.getCorrespondingColumns()
    );
    const correspondingCells: HTMLElement[][] = this.getCorrespondingColumns();
    const activeCellRowSpan: number = this.activeCell.getAttribute('rowspan')
      ? parseInt(this.activeCell.getAttribute('rowspan'), 10)
      : 1;
    if (activeCellRowSpan > 1) {
      const avgCount: number = Math.ceil(activeCellRowSpan / 2);
      // eslint-disable-next-line
      1 < avgCount
        ? this.activeCell.setAttribute('rowspan', avgCount.toString())
        : this.activeCell.removeAttribute('rowspan');
      // eslint-disable-next-line
      1 < activeCellRowSpan - avgCount
        ? newCell.setAttribute(
            'rowspan',
            (activeCellRowSpan - avgCount).toString()
          )
        : newCell.removeAttribute('rowspan');
      let avgRowIndex: number;
      let colIndex: number;
      for (
        avgRowIndex = activeCellIndex[0] + Math.ceil(activeCellRowSpan / 2),
          colIndex =
            0 === activeCellIndex[1]
              ? activeCellIndex[1]
              : activeCellIndex[1] - 1;
        0 <= colIndex &&
        (correspondingCells[avgRowIndex as number][colIndex as number] ===
          // eslint-disable-next-line max-len
          correspondingCells[avgRowIndex as number][colIndex - 1] ||
          (0 < avgRowIndex &&
            correspondingCells[avgRowIndex as number][colIndex as number] ===
      ...
      if (colIndex === -1) {
        if (this.curTable.rows[avgRowIndex as number].firstChild) {
          (this.curTable.rows[avgRowIndex as number] as HTMLElement).prepend(
            newCell
          );
        } else {
        ...
        correspondingCells[avgRowIndex as number][
          colIndex as number
        ].insertAdjacentElement('afterend', newCell);
      }
    } else {
      ...
      for (let j: number = 0; j <= selectedRow.length - 1; j++) {
        if (
          selectedRow[j as number] !== selectedRow[j - 1] &&
          selectedRow[j as number] !== this.activeCell
        ) {
          selectedRow[j as number].setAttribute(
            'rowspan',
            (
              (parseInt(selectedRow[j as number].getAttribute('rowspan'), 10)
                ? parseInt(selectedRow[j as number].getAttribute('rowspan'), 10)
                : 1) + 1
            ).toString()
          );
      ...
      (this.activeCell.parentNode as HTMLElement).insertAdjacentElement(
        'afterend',
        newTrEle
      );
    }
  ...
  private VerticalSplit(e: IHtmlItem): void {
    ...
    const correspondingColumns: HTMLElement[][] =
      this.getCorrespondingColumns();
    const activeCellcolSpan: number =
      parseInt(this.activeCell.getAttribute('colspan'), 10) || 1;
    if (activeCellcolSpan > 1) {
      ...
      const activeCellUpdatedWidth: number = this.getSplitColWidth(
        activeCellIndex[1],
        activeCellIndex[1] + colSpan - 1,
        getColSizes
      );
      let newCellWidth: number = this.getSplitColWidth(
        activeCellIndex[1] + colSpan,
        activeCellIndex[1] + activeCellcolSpan - 1,
        getColSizes
      );
      const activeCellWidth: number = this.convertPixelToPercentage(
        this.activeCell.offsetWidth,
        this.curTable.offsetWidth
      );
      newCellWidth =
        activeCellWidth - activeCellUpdatedWidth < newCellWidth
          ? activeCellWidth - activeCellUpdatedWidth
          : newCellWidth;
      if (1 < colSpan) {
      ...
      if (1 < activeCellcolSpan - colSpan) {
        newCell.setAttribute(
          'colspan',
          (activeCellcolSpan - colSpan).toString()
        );
      } else {
      ...
      for (let i: number = 0; i <= allRows.length - 1; i++) {
        if (
          0 === i ||
          correspondingColumns[i as number][activeCellIndex[1]] !==
            correspondingColumns[i - 1][activeCellIndex[1]]
        ) {
          const currentCell: HTMLElement =
            correspondingColumns[i as number][activeCellIndex[1]];
    ...
    this.activeCell.parentNode.insertBefore(
      newCell,
      this.activeCell.nextSibling
    );
    if (e.callBack) {
      e.callBack({
        requestType: e.item.subCommand,
  ...
  private FindIndex(
    ...
    let nextCol: number;
    for (
      nextIndex = rowIndex + 1, nextCol = columnIndex + 1;
      nextIndex < cells.length;

    ) {
      if (
        cells[nextIndex as number][columnIndex as number] !==
        cells[rowIndex as number][columnIndex as number]
      ) {
        nextIndex--;
        break;
      }
    ...
    for (
      nextIndex === cells.length && nextIndex--;
      nextCol < cells[rowIndex as number].length;

    ) {
      if (
        cells[rowIndex as number][nextCol as number] !==
        cells[rowIndex as number][columnIndex as number]
      ) {
        nextCol--;
        break;
    ...
    return (
      nextCol === cells[rowIndex as number].length && nextCol--,
      [nextIndex, nextCol]
    );
  }
  ...
  private highlightCells(
    ...
    for (j = minRowIndex; j <= maxRowIndex; j++) {
      startCell = this.getCorrespondingIndex(
        eleArray[j as number][minColIndex as number],
        eleArray
      );
      ...
      if (
        (1 <
          (parseInt(
            eleArray[j as number][minColIndex as number].getAttribute(
              'rowspan'
            ),
            10
          ) || 1) ||
        ...
        1 <
          (parseInt(
            eleArray[j as number][maxColIndex as number].getAttribute(
              'rowspan'
            ),
            10
          ) || 1) ||
        ...
        startCell = this.getCorrespondingIndex(
          eleArray[j as number][maxColIndex as number],
          eleArray
        );
        endCell = this.FindIndex(startCell[0], startCell[1], eleArray);
        if (endCell) {
      ...
      for (k = minColIndex; k <= maxColIndex; k++) {
        startCell = this.getCorrespondingIndex(
          eleArray[minRowIndex as number][k as number],
          eleArray
        );
        endCell = this.FindIndex(startCell[0], startCell[1], eleArray);
        if (
          (1 <
            (parseInt(
              eleArray[minRowIndex as number][k as number].getAttribute(
                'rowspan'
              ),
              10
            ) || 1) ||
          ...
          1 <
            (parseInt(
              eleArray[maxRowIndex as number][k as number].getAttribute(
                'rowspan'
              ),
              10
            ) || 1) ||
          ...
          startCell = this.getCorrespondingIndex(
            eleArray[maxRowIndex as number][k as number],
            eleArray
          );
          endCell = this.FindIndex(startCell[0], startCell[1], eleArray);
          if (endCell) {
  ...
  private restoreRange(target: HTMLElement): void {
    if (
      this.parent.currentDocument.getSelection().rangeCount &&
  ...
  private tableMove(e: IHtmlItem): void {
    ...
    if (
      target.tagName !== 'TD' &&
      target.tagName !== 'TH' &&
      activeCellTag !== targetCellTag
    ) {
      return;
    }
    const activeCellTable: HTMLTableElement = closest(
      this.activeCell,
      'table'
    ) as HTMLTableElement;
    if (
      isNOU(this.curTable) ||
      isNOU(activeCellTable) ||
      activeCellTable !== this.curTable
    ) {
      return;
    }
    const correspondingCells: HTMLElement[][] = this.getCorrespondingColumns();
    const activeIndexes: number[] = this.getCorrespondingIndex(
      this.activeCell,
      correspondingCells
    );
    const targetIndexes: number[] = this.getCorrespondingIndex(
      target as HTMLElement,
      correspondingCells
    );
    const activeCellList: NodeListOf<HTMLElement> =
      this.curTable.querySelectorAll(
    ...
    for (
      let rowIndex: number = minMaxIndexes.startRow;
      rowIndex <= minMaxIndexes.endRow;
      rowIndex++
    ) {
      for (
        let colIndex: number = minMaxIndexes.startColumn;
        colIndex <= minMaxIndexes.endColumn;
        colIndex++
      ) {
        correspondingCells[rowIndex as number][
          colIndex as number
        ].classList.add('e-cell-select');
        correspondingCells[rowIndex as number][
          colIndex as number
        ].classList.add('e-multi-cells-select');
      }
    }
