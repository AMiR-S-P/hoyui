...
export class Lists {
  ...
  private testList(elem: Element): boolean {
    const olListRegex: RegExp[] = [
      /^[\d]+[.]+$/,
      /^(?=[MDCLXVI])M*(C[MD]|D?C{0,3})(X[CL]|L?X{0,3})(I[XV]|V?I{0,3})[.]$/gi,
      /^[a-zA-Z][.]+$/,
    ];
    const elementStart: string = !isNullOrUndefined(elem)
      ? (elem as HTMLElement).innerText.trim().split('.')[0] + '.'
      : null;
  ...
  private testCurrentList(range: Range): boolean {
    ...
    if (
      ...
      const currentContent: string = range.startContainer.textContent
        .replace(/\u200B/g, '')
        .slice(0, range.startOffset)
        .trim();
      for (let i: number = 0; i < olListStartRegex.length; i++) {
        if (
          olListStartRegex[i as number].test(currentContent) &&
  ...
  private spaceList(e: IHtmlKeyboardEvent): void {
    ...
    this.saveSelection = this.parent.nodeSelection.save(
      range,
      this.parent.currentDocument
    );
    const startNode: Element = this.parent.domNode.getSelectedNode(
      range.startContainer as Element,
      range.startOffset
    );
    // eslint-disable-next-line
    const endNode: Element = this.parent.domNode.getSelectedNode(
    ...
    const preElemULStart: string = !isNullOrUndefined(preElement)
      ? (preElement as HTMLElement).innerText.trim().substring(0, 1)
      : null;
    const nextElemULStart: string = !isNullOrUndefined(nextElement)
      ? (nextElement as HTMLElement).innerText.trim().substring(0, 1)
      : null;
    ...
    if (
      ...
      if (startElementOLTest) {
        ...
        if (
          ...
          this.parent.domNode.insertAfter(
            brElement,
            range.startContainer as Element
          );
        }
  ...
  private enterList(e: IHtmlKeyboardEvent): void {
    ...
    const startNode: Element =
      range.startContainer.nodeName === 'LI'
        ? (range.startContainer as Element)
        : range.startContainer.parentElement.closest('LI');
    const endNode: Element =
      range.endContainer.nodeName === 'LI'
        ? (range.endContainer as Element)
        : range.endContainer.parentElement.closest('LI');
    // Checks for Image, Audio , Video Element inside List Element
    let hasMediaElem: boolean = false;
    if (!isNOU(startNode)) {
      const videoElemList: NodeList =
        startNode.querySelectorAll('.e-video-clickelem');
      const embedVideoElem: boolean =
        videoElemList.length > 0 &&
        videoElemList[0].childNodes[0].nodeName === 'IFRAME';
      hasMediaElem =
        startNode.querySelectorAll('IMG').length > 0 ||
        startNode.querySelectorAll('AUDIO').length > 0 ||
        startNode.querySelectorAll('VIDEO').length > 0 ||
        embedVideoElem;
    }
    if (
      !isNOU(startNode) &&
      ...
      if (
        isNOU(parentOfCurrentOLUL.closest('UL')) &&
        isNOU(parentOfCurrentOLUL.closest('OL'))
      ) {
        if (!isNOU(startNode.nextElementSibling)) {
          const nearBlockNode: Element =
            this.parent.domNode.blockParentNode(startNode);
          this.parent.nodeCutter.GetSpliceNode(
            range,
            nearBlockNode as HTMLElement
          );
        }
        ...
        if (
          !isNOU(parentOfCurrentOLUL) &&
          parentOfCurrentOLUL.nodeName === 'BLOCKQUOTE'
        ) {
        ...
        if (
          startNodeParent.textContent === '' &&
          startNodeParent.querySelectorAll('audio,video').length === 0
        ) {
      ...
      if (
        ...
        (!isNOU(parentOfCurrentOLUL.closest('UL')) ||
          !isNOU(parentOfCurrentOLUL.closest('OL'))) &&
        parentOfCurrentOLUL.nodeName === 'LI' &&
        parentOfCurrentOLUL.style.listStyleType === 'none' &&
        parentOfCurrentOLUL.textContent === '' &&
        startNode.textContent === '' &&
        startNode === startNodeParent.firstElementChild &&
  ...
  private backspaceList(e: IHtmlKeyboardEvent): void {
    ...
    let startNode: Element = this.parent.domNode.getSelectedNode(
      range.startContainer as Element,
      range.startOffset
    );
    let endNode: Element = this.parent.domNode.getSelectedNode(
      range.endContainer as Element,
      range.endOffset
    );
    startNode =
      startNode.nodeName === 'BR' ? startNode.parentElement : startNode;
    endNode = endNode.nodeName === 'BR' ? endNode.parentElement : endNode;
    if (
      startNode === endNode &&
      !isNullOrUndefined(closest(startNode, 'li')) &&
      ((startNode.textContent.trim() === '' &&
        startNode.textContent.charCodeAt(0) === 65279) ||
        (startNode.textContent.length === 1 &&
          startNode.textContent.charCodeAt(0) === 8203))
    ) {
      startNode.textContent = '';
    }
    if (
    ...
    if (startNode === endNode && startNode.textContent === '') {
      if (
        startNode.parentElement.tagName === 'LI' &&
        endNode.parentElement.tagName === 'LI'
      ) {
        detach(startNode);
      } else if (startNode.closest('ul') || startNode.closest('ol')) {
        const parentList: HTMLElement = !isNOU(startNode.closest('ul'))
          ? startNode.closest('ul')
          : startNode.closest('ol');
        if (
          parentList.firstElementChild === startNode &&
          !isNOU(parentList.children[1]) &&
          (parentList.children[1].tagName === 'OL' ||
            parentList.children[1].tagName === 'UL')
        ) {
          if (parentList.tagName === parentList.children[1].tagName) {
            while (parentList.children[1].lastChild) {
              this.parent.domNode.insertAfter(
                parentList.children[1].lastChild as Element,
                parentList.children[1]
              );
            }
            detach(parentList.children[1]);
          } else {
            parentList.parentElement.insertBefore(
              parentList.children[1],
              parentList
            );
          }
      ...
      !isNOU(startNode.firstChild) &&
      startNode.firstChild.nodeName === 'BR' &&
      !isNullOrUndefined(startNode.childNodes[1]) &&
      (startNode.childNodes[1].nodeName === 'UL' ||
      ...
      if (parentList.tagName === startNode.childNodes[1].nodeName) {
        while (startNode.childNodes[1].lastChild) {
          this.parent.domNode.insertAfter(
            startNode.children[1].lastChild as Element,
            startNode
          );
        }
  ...
  private removeList(range: Range, e: IHtmlKeyboardEvent): void {
    ...
    let endNode: Element =
      !isNOU(range.endContainer.parentElement.closest('li')) &&
      range.endContainer.parentElement.closest('li').childElementCount > 1 &&
      range.endContainer.nodeName === '#text'
        ? (range.endContainer as Element)
        : this.parent.domNode.getSelectedNode(
            range.endContainer as Element,
            range.endOffset
          );
    ...
    endNode = endNode.nodeName === 'BR' ? endNode.parentElement : endNode;
    startNode =
      startNode.nodeName !== 'LI' && !isNOU(startNode.closest('LI'))
        ? startNode.closest('LI')
        : startNode;
    endNode =
      endNode.nodeName !== 'LI' &&
      endNode.nodeName !== '#text' &&
      !isNOU(endNode.closest('LI'))
        ? endNode.closest('LI')
        : endNode;
    if (
      ((range.commonAncestorContainer.nodeName === 'OL' ||
        range.commonAncestorContainer.nodeName === 'UL' ||
        range.commonAncestorContainer.nodeName === 'LI') &&
        isNOU(endNode.nextElementSibling) &&
        endNode.textContent.length === range.endOffset &&
        isNOU(startNode.previousElementSibling) &&
        range.startOffset === 0) ||
      (Browser.userAgent.indexOf('Firefox') !== -1 &&
        range.startContainer === range.endContainer &&
        range.startContainer === this.parent.editableElement &&
        range.startOffset === 0 &&
        range.endOffset === 1)
    ) {
      ...
      !isNOU(parentList) &&
      !range.collapsed &&
      parentList.textContent === fullContent
    ) {
  ...
  private onKeyUp(e: IHtmlKeyboardEvent): void {
    if (
      !isNOU(this.commonLIParent) &&
      !isNOU(this.commonLIParent.querySelector('.removeList'))
    ) {
      const currentLIElem: Element =
        this.commonLIParent.querySelector('.removeList');
      while (!isNOU(currentLIElem.firstChild)) {
        this.parent.domNode.insertAfter(
          currentLIElem.firstChild as Element,
          currentLIElem
        );
      }
    ...
    if (e.event.keyCode === 13) {
      const listElements: NodeListOf<Element> =
        this.parent.editableElement.querySelectorAll('UL, OL');
      for (let i: number = 0; i < listElements.length; i++) {
        if (
          !isNullOrUndefined(listElements[i as number]) &&
          !isNOU(listElements[i as number].parentElement) &&
          !isNullOrUndefined(
            listElements[i as number].previousElementSibling
          ) &&
  ...
  private firstListBackSpace(range: Range, _e: IHtmlKeyboardEvent): void {
    ...
    if (
      startNode.nodeName === 'LI' &&
      range.startOffset === 0 &&
      range.endOffset === 0 &&
      isNOU(startNode.previousSibling) &&
      !isNOU(this.commonLIParent) &&
      isNOU(this.commonLIParent.previousSibling) &&
      isNOU(this.commonLIParent.parentElement.closest('OL')) &&
      isNOU(this.commonLIParent.parentElement.closest('UL')) &&
      ...
      this.commonLIParent.parentElement.insertBefore(
        currentElem,
        this.commonLIParent
      );
    }
  ...
  private keyDownHandler(e: IHtmlKeyboardEvent): void {
    ...
    if (
      (e.event.which === 46 && e.event.action === 'delete') ||
      (e.event.which === 88 && e.event.action === 'cut')
    ) {
      const range: Range = this.parent.nodeSelection.getRange(
        this.parent.currentDocument
      );
      ...
      const startNode: Node =
        startEle.nodeType === 3
          ? this.domNode.blockParentNode(startEle as Element)
          : startEle;
      const endNode: Node =
        endEle.nodeType === 3
          ? this.domNode.blockParentNode(endEle as Element)
          : endEle;
      if (
        (commonAncestor.nodeName === 'UL' ||
          commonAncestor.nodeName === 'OL') &&
        startNode !== endNode &&
        (!isNullOrUndefined(closest(startNode, 'ul')) ||
          !isNullOrUndefined(closest(startNode, 'ol'))) &&
        (!isNullOrUndefined(closest(endNode, 'ul')) ||
          !isNullOrUndefined(closest(endNode, 'ol'))) &&
        (commonAncestor as HTMLElement).lastElementChild ===
          closest(endNode, 'li') &&
        commonAncestor.lastChild !== endNode &&
        !range.collapsed
      ) {
        if (
          this.areAllListItemsSelected(commonAncestor as HTMLElement, range)
        ) {
    ...
    if (e.event.which === 9) {
      ...
      if (
        startNode === endNode &&
        (startNode.nodeName === 'BR' || startNode.nodeName === '#text') &&
        CONSTANT.IGNORE_BLOCK_TAGS.indexOf(
          (startNode.parentNode as Element).tagName.toLocaleLowerCase()
        ) >= 0
      ) {
        return;
      } else {
        ...
        if (!(e.event.action && e.event.action === 'indent')) {
          ...
          if (e.callBack) {
            e.callBack({
              ...
              editorMode: 'HTML',
              range: this.parent.nodeSelection.getRange(
                this.parent.currentDocument
              ),
  ...
  private revertClean(): void {
    const collectionNodes: Element[] = <NodeListOf<Element> & Element[]>(
      this.parent.editableElement.querySelectorAll('ul, ol')
    );
    for (let i: number = 0; i < collectionNodes.length; i++) {
      const listNodes: Element[] = <NodeListOf<Element> & Element[]>(
        collectionNodes[i as number].querySelectorAll('ul, ol')
      );
      if (listNodes.length > 0) {
        for (let j: number = 0; j < listNodes.length; j++) {
          const prevSibling: Element = listNodes[j as number]
  ...
  private noPreviousElement(elements: Node): void {
    ...
    const siblingListOL: Element[] = <NodeListOf<Element> & Element[]>(
      (elements as Element).querySelectorAll('ol, ul')
    );
    const siblingListLI: NodeListOf<HTMLLIElement> = (
      elements as Element
    ).querySelectorAll('li') as NodeListOf<HTMLLIElement>;
    const siblingListLIFirst: Node = this.domNode.contents(
      siblingListLI[0] as Element
    )[0];
    if (
      siblingListLI.length > 0 &&
      (siblingListLIFirst.nodeName === 'OL' ||
        siblingListLIFirst.nodeName === 'UL')
    ) {
    ...
    if (firstNode) {
      for (
        let h: Node = this.domNode.contents(elements as Element)[0];
        h && !this.domNode.isList(h as Element);
        null
      ) {
        const nextSibling: Element = h.nextSibling as Element;
        prepend([h as Element], firstNode);
        setStyleAttribute(elements as HTMLElement, {
      ...
      detach(elements);
      const nestedElementLI: Element = createElement('li', {
      ...
      const nestedElement: Element = createElement(
        (elements.parentNode as Element).tagName
      );
  ...
  private nestedList(elements: Node[]): boolean {
    ...
    for (let i: number = 0; i < elements.length; i++) {
      const prevSibling: Element = this.domNode.getPreviousNode(
      ...
      if (prevSibling) {
        ...
        const siblingListOL: Element[] = <NodeListOf<Element> & Element[]>(
          (elements[i as number] as Element).querySelectorAll('ol, ul')
        );
        const siblingListLI: NodeListOf<HTMLLIElement> = (
          elements[i as number] as Element
        ).querySelectorAll('li') as NodeListOf<HTMLLIElement>;
        ...
        if (firstNode) {
          ...
          for (
            let h: Node = this.domNode.contents(
              elements[i as number] as Element
            )[0];
            h && !this.domNode.isList(h as Element);
            null
          ...
          if (prevSibling.tagName === 'LI') {
            for (
              ...
              setStyleAttribute(elements[i as number] as HTMLElement, {
                'list-style-type': 'none',
              });
              setStyleAttribute(firstNodeLI as HTMLElement, {
                'list-style-type': '',
              });
              h = nextSibling;
            }
          ...
          if (prevSibling.tagName === 'LI') {
            const nestedElement: Element = createElement(
              (elements[i as number].parentNode as Element).tagName
            );
            (nestedElement as HTMLElement).style.listStyleType = (
              elements[i as number].parentNode as HTMLElement
            ).style.listStyleType;
  ...
  private applyListsHandler(e: IHtmlSubCommands): void {
    let range: Range = this.parent.nodeSelection.getRange(
      this.parent.currentDocument
    );
    if (
      Browser.userAgent.indexOf('Firefox') !== -1 &&
      ...
      const startNode: Element = <Element>(
        (startChildNodes[
          range.startOffset > 0 ? range.startOffset - 1 : range.startOffset
        ] || range.startContainer)
      );
      const endNode: Element = <Element>(
        (range.endContainer.childNodes[
          range.endOffset > 0 ? range.endOffset - 1 : range.endOffset
        ] || range.endContainer)
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      let lastSelectionNode: any =
        endNode.lastChild.nodeName === 'BR'
          ? isNOU(endNode.lastChild.previousSibling)
            ? endNode
            : endNode.lastChild.previousSibling
          : endNode.lastChild;
      while (
        !isNOU(lastSelectionNode) &&
        lastSelectionNode.nodeName !== '#text' &&
        lastSelectionNode.nodeName !== 'IMG' &&
        lastSelectionNode.nodeName !== 'BR' &&
        lastSelectionNode.nodeName !== 'HR'
      ) {
    ...
    if (e.enterAction === 'BR') {
      ...
      this.parent.domNode.convertToBlockNodes(
    ...
    for (let i: number = 0; i < listsNodes.length; i++) {
      if (
        (listsNodes[i as number] as Element).tagName === 'TABLE' &&
        !range.collapsed
      ) {
      ...
      if (
        listsNodes.length > 0 &&
        (listsNodes[i as number] as Element).tagName !== 'LI' &&
  ...
  private setSelectionBRConfig(): void {
    const startElem: Element = this.parent.editableElement.querySelector(
      '.' + markerClassName.startSelection
    );
    const endElem: Element = this.parent.editableElement.querySelector(
      '.' + markerClassName.endSelection
    );
    if (isNOU(endElem)) {
      this.parent.nodeSelection.setCursorPoint(
        this.parent.currentDocument,
  ...
  private applyLists(
    ...
    if (this.isRevert(elements, type, item) && isNOU(item)) {
      ...
      if (elements[0].style.marginLeft !== '') {
        marginLeftAttribute =
          ' style = "margin-left: ' + elements[0].style.marginLeft + ';"';
      }
      for (let i: number = 0; i < elements.length; i++) {
        if (!isNOU(item) && !isNOU(item.listStyle)) {
          if (item.listStyle === 'listImage') {
            setStyleAttribute(elements[i as number], {
            ...
            setStyleAttribute(elements[i as number], {
              'list-style-type': item.listStyle
        ...
        const elemAtt: string =
          elements[i as number].tagName === 'IMG' ||
          elements[i as number].classList.contains('e-editor-select-start')
            ? ''
            : this.domNode.attributes(elements[i as number]);
        if (
          elements[i as number].getAttribute('contenteditable') === 'true' &&
          elements[i as number].childNodes.length === 1 &&
          elements[i as number].childNodes[0].nodeName === 'TABLE'
        ) {
          const listEle: Element = document.createElement(type);
          listEle.innerHTML = '<li><br/></li>';
          elements[i as number].appendChild(listEle);
        } else if (
          'LI' !== elements[i as number].tagName &&
          isNOU(item) &&
          ...
          const replaceHTML: string =
            elements[i as number].tagName.toLowerCase() === CONSTANT.DEFAULT_TAG
              ? elements[i as number].innerHTML
              : elements[i as number].outerHTML;
          const innerHTML: string = this.domNode.createTagString(
            newTag,
            null,
            replaceHTML
    ...
    this.cleanNode();
    if (e.enterAction === 'BR') {
      const spansToRemove: NodeListOf<Element> =
        document.querySelectorAll('span#removeSpan');
  ...
  private removeEmptyListElements(): void {
    const listElem: NodeListOf<Element> =
      this.parent.editableElement.querySelectorAll('ol, ul');
  ...
  private isRevert(
    ...
    for (let i: number = 0; i < nodes.length; i++) {
      ...
      if (
        (nodes[i as number].parentNode as Element).tagName !== tagName ||
        (isNOU(item) &&
          (nodes[i as number].parentNode as HTMLElement).style.listStyleType !==
            '')
      ) {
        isRevert = false;
      }
  ...
  private checkLists(
    ...
    for (let i: number = 0; i < nodes.length; i++) {
      ...
      if (
        !isNOU(item) &&
        'LI' === nodes[i as number].tagName &&
        !isNOU(item.listStyle)
      ) {
        if (item.listStyle === 'listImage') {
          setStyleAttribute(node as HTMLElement, {
            'list-style-image': item.listImage,
          });
        } else {
          setStyleAttribute(node as HTMLElement, {
      ...
      if (
        (nodes[i as number].tagName === 'LI' &&
          node.tagName !== tagName &&
          nodesTemp.indexOf(node) < 0) ||
        (nodes[i as number].tagName === 'LI' &&
          node.tagName === tagName &&
          nodesTemp.indexOf(node) < 0 &&
          item !== null)
  ...
  private cleanNode(): void {
    const liParents: Element[] = <Element[] & NodeListOf<Element>>(
      this.parent.editableElement.querySelectorAll('ol + ol, ul + ul')
    );
    ...
    for (let c: number = 0; c < liParents.length; c++) {
      ...
      while (containsListElements.parentElement) {
        if (
          containsListElements.parentElement &&
          containsListElements.parentElement.tagName !== 'LI' &&
          containsListElements.parentElement.tagName !== 'OL' &&
          containsListElements.parentElement.tagName !== 'UL'
        ) {
          break;
        }
      ...
      if (
        toFindtopOlUl &&
        (liParents[c as number].parentElement.parentElement.nodeName === 'OL' ||
          liParents[c as number].parentElement.parentElement.nodeName === 'UL')
      ) {
        toFindtopOlUl = false;
        const preElement: HTMLElement =
          liParents[c as number].parentElement.parentElement;
        listStyleType = preElement.style.listStyleType;
        firstNodeOL = node.previousElementSibling;
      }
      if (
        this.domNode.isList(node.previousElementSibling as Element) &&
        this.domNode.openTagString(node) ===
          this.domNode.openTagString(node.previousElementSibling as Element)
      ) {
        ...
        this.domNode.isList(node.previousElementSibling) &&
        containsListElements.contains(node.previousElementSibling) &&
        (node.tagName === 'OL' || node.tagName === 'UL') &&
        (node.previousElementSibling.nodeName === 'OL' ||
          node.previousElementSibling.nodeName === 'UL')
      ) {
        const contentNodes: Node[] = this.domNode.contents(node);
        for (let f: number = 0; f < contentNodes.length; f++) {
          node.previousElementSibling.appendChild(contentNodes[f as number]);
    ...
    if (firstNodeOL) {
      ...
      if (
        ...
        listOlUlElements = listOlUlElements.concat(
          Array.from(
        ...
        listOlUlElements = Array.from(
          (range.commonAncestorContainer as Element).querySelectorAll('ol, ul')
        );
      }
      for (let k: number = 0; k < listOlUlElements.length; k++) {
        let listStyle: string;
        let listElements: HTMLElement | Element = listOlUlElements[
          k as number
        ] as HTMLElement;
        while (listElements) {
          if (
            listElements.nodeName === 'OL' ||
            listElements.nodeName === 'OL'
          ) {
            if (
              (listElements as HTMLElement).style.listStyleType !== '' &&
              (listElements as HTMLElement).style.listStyleType !== 'none' &&
              (listElements as HTMLElement).nodeName !== 'LI'
            ) {
              ...
              ((listElements as HTMLElement).style.listStyleType === '' ||
                (listElements as HTMLElement).style.listStyleType === 'none') &&
              (listElements as HTMLElement).nodeName !== 'LI' &&
              ((listElements as HTMLElement).nodeName === 'UL' ||
                (listElements as HTMLElement).nodeName === 'OL')
            ) {
  ...
  private findUnSelected(temp: HTMLElement[], elements: HTMLElement[]): void {
    ...
    if (temp.length > 0) {
      ...
      lastElementChild = <NodeListOf<Element> & Element[]>(
        lastElement.childNodes
      );
      for (let z: number = 0; z < lastElementChild.length; z++) {
        if (
          lastElementChild[z as number].tagName === 'OL' ||
          lastElementChild[z as number].tagName === 'UL'
        ) {
          const childLI: NodeListOf<HTMLLIElement> = (
            lastElementChild[z as number] as Element
          ).querySelectorAll('li') as NodeListOf<HTMLLIElement>;
          if (childLI.length > 0) {
      ...
      if (indentElements.length > 0) {
        for (let x: number = 0; x < indentElements.length; x++) {
          if (
            this.domNode.contents(indentElements[x as number])[0].nodeName !==
              'OL' &&
            this.domNode.contents(indentElements[x as number])[0].nodeName !==
              'UL'
          ) {
  ...
  private revertList(
    elements: HTMLElement[],
    e?: IHtmlSubCommands | IHtmlKeyboardEvent
  ): void {
    const temp: Element[] = [];
    for (let i: number = elements.length - 1; i >= 0; i--) {
      for (let j: number = i - 1; j >= 0; j--) {
        if (
          elements[j as number].contains(elements[i as number]) ||
          elements[j as number] === elements[i as number]
        ) {
    ...
    for (let i: number = 0; i < elements.length; i++) {
      ...
      if (
        this.domNode.contents(element)[0].nodeType === 3 &&
        this.domNode.contents(element)[0].textContent.trim().length === 0
      ) {
      ...
      if (temp.length === 0) {
        const siblingList: Element[] = <NodeListOf<Element> & Element[]>(
          (elements[i as number] as Element).querySelectorAll('ul, ol')
        );
        const firstNode: Element = siblingList[0];
        if (firstNode) {
          const child: NodeListOf<HTMLLIElement> = firstNode.querySelectorAll(
            'li'
      ...
      if (
        (element.parentNode.insertBefore(
          this.closeTag(parentNode.tagName) as Element,
          element
        ),
        'LI' === (parentNode.parentNode as Element).tagName ||
          'OL' === (parentNode.parentNode as Element).tagName ||
          'UL' === (parentNode.parentNode as Element).tagName)
      ) {
        element.parentNode.insertBefore(
          this.closeTag('LI') as Element,
          element
        );
      } else {
        ...
        if (
          ...
          0 === element.querySelectorAll(CONSTANT.BLOCK_TAGS.join(', ')).length
        ) {
          const wrapperclass: string = isNullOrUndefined(className)
            ? ' class="e-rte-wrap-inner"'
          ...
          if (elements.length === parentElement.querySelectorAll('li').length) {
            if (!isNOU(parentElement.style.listStyleType)) {
              (parentNode as HTMLElement).style.removeProperty(
                'list-style-type'
              );
            }
          ...
          const wrapper: string =
            ...
            e.enterAction +
            wrapperclass +
            this.domNode.attributes(element) +
            '></' +
            e.enterAction +
            '>';
          if (e.enterAction !== 'BR') {
            this.domNode.wrapInner(
              element,
              this.domNode.parseHTMLFragment(wrapper)
            );
          } else {
            const wrapperSpan: string =
              '<span class=e-rte-wrap-inner id=removeSpan></span>';
            const br: HTMLElement = document.createElement('br');
            this.domNode.wrapInner(
              element,
              this.domNode.parseHTMLFragment(wrapperSpan)
            );
          ...
          const replace: string = this.domNode.createTagString(
            CONSTANT.DEFAULT_TAG,
            parentNode,
            this.parent.domNode.encode(
              this.domNode.contents(element)[0].textContent
            )
          );
          this.domNode.replaceWith(
            this.domNode.contents(element)[0] as Element,
            replace
          );
        } else if (
          (this.domNode.contents(element)[0] as HTMLElement).classList.contains(
            markerClassName.startSelection
          ) ||
          (this.domNode.contents(element)[0] as HTMLElement).classList.contains(
            markerClassName.endSelection
          ...
          if (childNode) {
            ...
            if (attributes.length > 0) {
              for (let d: number = 0; d < attributes.length; d++) {
                ...
                const existingValue: string = childNode.getAttribute(
                  e.nodeName
                );
                const parentValue: string = element.parentElement.getAttribute(
                  e.nodeName
                );
                childNode.setAttribute(
                  e.nodeName,
                  existingValue
                    ? parentValue + ' ' + existingValue
                    : parentValue
                );
              }
            }
    ...
    for (let i: number = 0; i < viewNode.length; i++) {
      ...
      const closeTag: RegExp =
        /<span class="e-rte-list-close-([a-z]*)"><\/span>/g;
      const openTag: RegExp =
        /<span class="e-rte-list-open-([a-z]*)"><\/span>/g;
      nodeInnerHtml = nodeInnerHtml.replace(closeTag, '</$1>');
      nodeInnerHtml = nodeInnerHtml.replace(
        openTag,
        '<$1 ' + this.domNode.attributes(node) + '>'
      );
      this.domNode.replaceWith(
        node,
        this.domNode.openTagString(node) +
          nodeInnerHtml.trim() +
          this.domNode.closeTagString(node)
      );
    }
    const emptyUl: Element[] = <NodeListOf<Element> & Element[]>(
      this.parent.editableElement.querySelectorAll('ul:empty, ol:empty')
    );
    for (let i: number = 0; i < emptyUl.length; i++) {
      detach(emptyUl[i as number]);
    }
    const emptyLi: Element[] = <NodeListOf<Element> & Element[]>(
      this.parent.editableElement.querySelectorAll('li:empty')
    );
  ...
  private openTag(type: string): Element {
    return this.domNode.parseHTMLFragment(
      '<span class="e-rte-list-open-' + type.toLowerCase() + '"></span>'
    );
  }

  private closeTag(type: string): Element {
    return this.domNode.parseHTMLFragment(
      '<span class="e-rte-list-close-' + type.toLowerCase() + '"></span>'
    );
  }
  public destroy(): void {
