...
import { setStyleAttribute, KeyboardEventArgs, closest } from '@hoyui/base';
...
export class Alignments {
  ...
  private applyAlignment(e: IHtmlSubCommands): void {
    ...
    if (!isTableAlign) {
      ...
      if (e.enterAction === 'BR') {
        alignmentNodes = this.parent.domNode.convertToBlockNodes(
          alignmentNodes,
          false
        );
      }
      for (let i: number = 0; i < alignmentNodes.length; i++) {
        ...
        setStyleAttribute(parentNode as HTMLElement, {
          'text-align': this.alignments[e.subCommand],
        });
      }
      const imageTags: NodeListOf<HTMLImageElement> =
        this.parent.domNode.getImageTagInSelection();
      ...
      setStyleAttribute(this.getTableNode(range)[0] as HTMLElement, {
        'text-align': this.alignments[e.subCommand],
      });
    }
    if (e.callBack) {
      e.callBack({
        ...
        elements: (isTableAlign
          ? this.getTableNode(range)
          : this.parent.domNode.blockNodes()) as Element[],
      });
    }
  }
