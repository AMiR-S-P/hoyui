import { Observer } from '@hoyui/base';
import {
  ICommandModel,
...
export class EditorManager {
  ...
  public constructor(options: ICommandModel) {
    ...
    this.formatPainterEditor = new FormatPainterActions(
      this,
      options.formatPainterSettings
    );
    this.emojiPickerObj = new EmojiPickerAction(this);
    this.tableCellSelection = new TableSelection(
  ...
  private tripleClickSelection(e: MouseEvent): void {
    ...
    if (selection.rangeCount > 0 && selection.toString() !== '') {
      const startBlockNode: Node = this.getParentBlockNode(
    ...
    const treeWalker: TreeWalker = this.currentDocument.createTreeWalker(
      ...
      NodeFilter.SHOW_ELEMENT, // whatToShow
      {
        ...
        acceptNode: function (currentNode: Node): number {
          ...
          const displayStyle: string = window.getComputedStyle(
  ...
  public beforeSlashMenuApplyFormat(): void {
    ...
    const slashNode: Node = this.nodeCutter.GetSpliceNode(
      slashRange,
      node as HTMLElement
    );
    const previouNode: Node = slashNode.previousSibling;
    if (
      slashNode.parentElement &&
      slashNode.parentElement.innerHTML.length === 1
    ) {
    ...
    if (previouNode) {
      this.nodeSelection.setCursorPoint(
        document,
        previouNode as Element,
        previouNode.textContent.length
      );
    }
  }
}
