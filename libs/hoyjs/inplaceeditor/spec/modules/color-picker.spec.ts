...
describe('ColorPicker module', () => {
  ...
  describe('Basic testing', () => {
    ...
    it('Value property testing', () => {
      ...
      expect(
        (<HTMLInputElement>select('input', ele)).value === '#003355'
      ).toEqual(true);
    });
  ...
  describe('Duplicate ID availability testing', () => {
    ...
    it('Popup - ID testing', () => {
      ...
      valueEle.click();
      expect(valueWrapper.classList.contains(classes.OPEN)).toEqual(true);
      expect(selectAll('.e-colorpicker', document.body).length === 1).toEqual(
        true
      );
      expect(selectAll('#' + ele.id, document.body).length === 1).toEqual(true);
    });
  ...
  describe('Value formatting related testing', () => {
    ...
    it('Default value with initial render testing', () => {
      ...
      expect(editorObj.value).toEqual(null);
      expect(
        (<HTMLInputElement>select('.e-colorpicker', document.body)).value
      ).toEqual('#008000');
      editorObj.save();
  ...
  describe('BLAZ-4764 - New change event testing', () => {
    ...
    it('Without initial value - Check changed value', (done) => {
      ...
      setTimeout(() => {
        ...
        (
          ele.querySelector(
            '.e-colorpicker-wrapper .e-dropdown-btn'
          ) as HTMLElement
        ).click();
        (
          document.body.querySelector(
            '.e-colorpicker-popup .e-btn.e-primary'
          ) as HTMLElement
        ).click();
        setTimeout(() => {
  ...
  describe('EJ2-48274 - New `enableHtmlParse` API testing', () => {
    ...
    it('Editor DOM value testing', (done) => {
      ...
      valueWrapper = <HTMLElement>select('.' + classes.VALUE_WRAPPER, ele);
      valueEle = <HTMLElement>select('.' + classes.VALUE, ele);
      expect(valueEle.innerText).toEqual(
        '<img src=x onerror=console.log("test")><script>alert("test")</scrip'
      );
      valueEle.click();
      setTimeout(() => {
        expect(valueWrapper.classList.contains(classes.OPEN)).toEqual(true);
