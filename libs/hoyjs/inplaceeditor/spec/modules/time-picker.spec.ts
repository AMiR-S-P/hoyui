...
import { HtmlEditor } from '@hoyui/richtexteditor';
...
describe('TimePicker module', () => {
  ...
  describe('Basic testing', () => {
    ...
    it('cssClass availability testing', () => {
      expect(editorObj.model.cssClass === 'customCSS e-editable-elements').toBe(
        true
      );
    });
    ...
    it('Value property testing', () => {
      expect(
        editorObj.timeModule.compObj.value.toString() === currentDate.toString()
      ).toEqual(true);
      expect(
        (<HTMLInputElement>select('input', ele)).value ===
          timeString.replace(/(.*)\D\d+/, '$1')
      ).toEqual(true);
    });
    it('save method with value property testing', () => {
      ...
      expect(
        valueEle.innerHTML ===
          newDate.toLocaleTimeString().replace(/(.*)\D\d+/, '$1')
      ).toEqual(true);
    });
    ...
    it('Without compObj data to update value method testing', () => {
      ...
      expect(editorObj.timeModule.compObj.value).toEqual(
        new Date('01/02/2018 10:10 AM')
      );
      editorObj.timeModule.compObj.value = new Date('01/03/2018 10:10 AM');
      expect(editorObj.timeModule.compObj.value).toEqual(
        new Date('01/03/2018 10:10 AM')
      );
  ...
  describe('Value as "" with testing', () => {
    ...
    it('Clear icon availability testing', () => {
      expect(editorObj.timeModule.compObj.showClearButton === true).toEqual(
        true
      );
    ...
    it('Value property testing', () => {
      ...
      expect((<HTMLInputElement>select('input', ele)).value === '').toEqual(
        true
      );
    });
  ...
  describe('Duplicate ID availability testing', () => {
    ...
    it('Popup - ID testing', () => {
      ...
      valueEle.click();
      expect(valueWrapper.classList.contains(classes.OPEN)).toEqual(true);
      expect(selectAll('.e-timepicker', document.body).length === 1).toEqual(
        true
      );
      expect(selectAll('#' + ele.id, document.body).length === 1).toEqual(true);
    });
    ...
    it('validation with rules testing', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        editorError = <HTMLElement>(
          select('.' + classes.EDITABLE_ERROR, ctrlGroup)
        );
        expect(editorError.childElementCount).toBe(1);
        let errorContainer: HTMLElement = <HTMLElement>(
          select('.' + classes.ERROR, editorError)
        );
  ...
  describe('BLAZ-4764 - New change event testing', () => {
    ...
    it('Without initial value - Check changed value', (done) => {
      ...
      setTimeout(() => {
        expect(editorObj.value).toEqual(null);
        editorObj.timeModule.compObj.value = modifiedDate;
  ...
  describe('EJ2-48274 - New `enableHtmlParse` API testing', () => {
    ...
    it('Editor DOM value testing', (done) => {
      editorObj = renderEditor({
        ...
        type: 'Time',
        value:
          '<img src=x onerror=console.log("test")><script>alert("test")</script>',
      });
      ele = editorObj.element;
      valueWrapper = <HTMLElement>select('.' + classes.VALUE_WRAPPER, ele);
      valueEle = <HTMLElement>select('.' + classes.VALUE, ele);
