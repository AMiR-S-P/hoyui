...
import { Encoding } from '@hoyui/file-utils';
/**
 * CsvHelper class
 * @private
 */
...
export class CsvHelper {
  ...
  constructor(json: any, separator: string) {
    ...
    if (json.styles !== null && json.styles !== undefined) {
      ...
      for (let i: number = 0; i < json.styles.length; i++) {
        if (
          json.styles[i].name !== undefined &&
          json.styles[i].numberFormat !== undefined
        ) {
          this.globalStyles.set(
            json.styles[i].name,
            json.styles[i].numberFormat
          );
        }
      }
    }
    ...
    if (cell.value !== undefined) {
      if (cell.value instanceof Date) {
        if (cell.style !== undefined && cell.style.numberFormat !== undefined) {
          ...
          try {
            csv += this.parseCellValue(
              this.formatter.displayText(
                cell.value,
                { type: 'dateTime', skeleton: cell.style.numberFormat },
            ...
            csv += this.parseCellValue(
              this.formatter.displayText(
                cell.value,
                { type: 'dateTime', format: cell.style.numberFormat },
                this.isServerRendered
              )
            );
          }
          ...
          cell.style.name !== undefined &&
          this.globalStyles.has(cell.style.name)
        ) {
          /* tslint:disable-next-line:max-line-length */
          try {
            csv += this.parseCellValue(
        ...
        if (cell.style !== undefined && cell.style.numberFormat !== undefined) {
          ...
          csv += this.parseCellValue(
            this.formatter.displayText(
              cell.value,
              { format: cell.style.numberFormat, ignoreCurrency: true },
              this.isServerRendered
            )
          );
        } else if (
  ...
  private parseCellValue(value: String): any {
    ...
    value = val;
    if (
      value.indexOf(this.separator) !== -1 ||
      value.indexOf('\n') !== -1 ||
      value.indexOf('"') !== -1
    ) {
      return (value = '"' + value + '"');
  ...
  public save(fileName: string): void {
    ...
    if (this.isMicrosoftBrowser) {
      ...
      let dwlLink: HTMLAnchorElement = document.createElementNS(
