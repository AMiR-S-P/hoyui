...
import { Internationalization, isNullOrUndefined } from '@hoyui/base';
...
export class Workbook {
  ...
  public mergedCellsStyle: Map<
    string,
    { x: number; y: number; styleIndex: number }
  >;
  ...
  public constructor(
    ...
    if (saveType === 'xlsx') {
        ...
        for (let i: number = 0; i < json.styles.length; i++) {
          if (json.styles[i].name !== undefined) {
            if (!this.cellStyles.has(json.styles[i].name)) {
              ...
              throw Error(
                'Style name ' + json.styles[i].name + ' is already existed'
              );
            }
          }
        }
      ...
      if (
        json.builtInProperties !== null &&
        json.builtInProperties !== undefined
      ) {
        this.builtInProperties = new BuiltInProperties();
        this.parserBuiltInProperties(
          json.builtInProperties,
  ...
  private parserBuiltInProperties(
    jsonBuiltInProperties: any,
    builtInProperties: BuiltInProperties
  ): void {
    //Author
    if (
      jsonBuiltInProperties.author !== null &&
      jsonBuiltInProperties.author !== undefined
    ) {
      builtInProperties.author = jsonBuiltInProperties.author;
    }
    //Comments
    if (
      jsonBuiltInProperties.comments !== null &&
      jsonBuiltInProperties.comments !== undefined
    ) {
      builtInProperties.comments = jsonBuiltInProperties.comments;
    }
    //Category
    if (
      jsonBuiltInProperties.category !== null &&
      jsonBuiltInProperties.category !== undefined
    ) {
      builtInProperties.category = jsonBuiltInProperties.category;
    }
    //Company
    if (
      jsonBuiltInProperties.company !== null &&
      jsonBuiltInProperties.company !== undefined
    ) {
      builtInProperties.company = jsonBuiltInProperties.company;
    }
    //Manager
    if (
      jsonBuiltInProperties.manager !== null &&
      jsonBuiltInProperties.manager !== undefined
    ) {
      builtInProperties.manager = jsonBuiltInProperties.manager;
    }
    //Subject
    if (
      jsonBuiltInProperties.subject !== null &&
      jsonBuiltInProperties.subject !== undefined
    ) {
      builtInProperties.subject = jsonBuiltInProperties.subject;
    }
    //Title
    if (
      jsonBuiltInProperties.title !== null &&
      jsonBuiltInProperties.title !== undefined
    ) {
      builtInProperties.title = jsonBuiltInProperties.title;
    }
    //Creation date
    if (
      jsonBuiltInProperties.createdDate !== null &&
      jsonBuiltInProperties.createdDate !== undefined
    ) {
      builtInProperties.createdDate = jsonBuiltInProperties.createdDate;
    }
    //Modified date
    if (
      jsonBuiltInProperties.modifiedDate !== null &&
      jsonBuiltInProperties.modifiedDate !== undefined
    ) {
      builtInProperties.modifiedDate = jsonBuiltInProperties.modifiedDate;
    }
    //Tags
    if (
      jsonBuiltInProperties.tags !== null &&
      jsonBuiltInProperties.tags !== undefined
    ) {
      builtInProperties.tags = jsonBuiltInProperties.tags;
    }
    //Status
    if (
      jsonBuiltInProperties.status !== null &&
      jsonBuiltInProperties.status !== undefined
    ) {
  ...
  private parserWorksheets(json: any): void {
    ...
    for (let i: number = 0; i < length; i++) {
      ...
      //showGridLines
      if (
        jsonSheet.showGridLines !== null &&
        jsonSheet.showGridLines !== undefined
      ) {
      ...
      if (
        jsonSheet.autoFilters !== null &&
        jsonSheet.autoFilters !== undefined
      ) {
  ...
  private parserPrintTitle(json: any, sheet: Worksheet): void {
    ...
    if (json.fromColumn !== null && json.fromColumn !== undefined) {
      ...
      titleColName =
        '$' +
        this.getColumnName(fromColumn) +
        ':$' +
        this.getColumnName(toColumn);
    }
  ...
  private parserFreezePanes(json: any, sheet: Worksheet): void {
    ...
    sheet.freezePanes.leftCell = this.getCellName(
      sheet.freezePanes.row + 1,
      sheet.freezePanes.column + 1
    );
  }
  /* tslint:disable:no-any */
  ...
  private insertMergedCellsStyle(sheet: Worksheet): void {
    if (this.mergeCells.length > 0) {
      this.mergedCellsStyle.forEach(
        (value: { x: number; y: number; styleIndex: number }, key: string) => {
  ...
  private createCell(
    value: { x: number; y: number; styleIndex: number },
    key: string
  ): Cell {
  ...
  private parseCells(json: any, row: Row): void {
    ...
    for (let cellId: number = 0; cellId < cellsLength; cellId++) {
      ...
      if (jsonCell.hyperlink !== null && jsonCell.hyperlink !== undefined) {
        ...
        if (jsonCell.hyperlink.target !== undefined) {
          ...
          this.parserCellStyle(
            jsonCell.style !== undefined
              ? this.mergeOptions(jsonCell.style, this.hyperlinkStyle)
              : this.hyperlinkStyle,
            cell.cellStyle,
            'string'
          );
      ...
      this.mergeCells = this.processMergeCells(
        cell,
        row.index,
        this.mergeCells
      );
      ...
      if (startindex >= 0 && endIndex >= 0) {
        if (startindex !== 0) {
          processedVal +=
            ...
            this.processString(value.substring(0, startindex)) +
            '</t></r>';
        }
                ...
                for (let element of subSplit) {
                  ...
                  switch (start) {
                    case 'size=':
                      processedVal +=
                        '<sz val="' +
                        element.substring(6, element.length - 1) +
                        '"/>';
                      break;
                    case 'face=':
                      processedVal +=
                    ...
                    case 'color':
                      processedVal +=
                        '<color rgb="' +
                        this.processColor(
                          element.substring(7, element.length - 1)
                        ) +
                        '"/>';
                      break;
                    case 'href=':
                      let hyperLink: HyperLink = new HyperLink();
                      hyperLink.target = element
                        .substring(6, element.length - 1)
                        .trim();
                      hyperLink.ref = cell.refName;
                      hyperLink.rId = this.mHyperLinks.length + 1;
                      this.mHyperLinks.push(hyperLink);
  ...
  private applyGlobalStyle(json: any, cellStyle: CellStyle): void {
    ...
    if (this.cellStyles.has(json.name)) {
      cellStyle.index = this.mStyles.filter(
        (a: CellStyle) => a.name === json.name
      )[0].index;
  ...
  private parserCellStyle(
    ...
    if (json.numberFormat !== null && json.numberFormat !== undefined) {
      if (json.type !== null && json.type !== undefined) {
        cellStyle.numberFormat = this.getNumberFormat(
          json.numberFormat,
          json.type
        );
      } else {
  ...
  private getNumberFormat(numberFormat: string, type: string): string {
    ...
    switch (type) {
      ...
      case 'datetime':
        try {
          returnFormat = this.intl.getDatePattern(
            { skeleton: numberFormat, type: 'dateTime' },
            true
          );
        } catch (error) {
          try {
            returnFormat = this.intl.getDatePattern(
              { format: numberFormat, type: 'dateTime' },
              true
            );
          } catch (error) {
            returnFormat = numberFormat;
      ...
      case 'date':
        try {
          returnFormat = this.intl.getDatePattern(
            { skeleton: numberFormat, type: 'date' },
            true
          );
        } catch (error) {
          try {
            returnFormat = this.intl.getDatePattern(
              { format: numberFormat, type: 'date' },
              true
            );
          } catch (error) {
            returnFormat = numberFormat;
      ...
      case 'time':
        try {
          returnFormat = this.intl.getDatePattern(
            { skeleton: numberFormat, type: 'time' },
            true
          );
        } catch (error) {
          try {
            returnFormat = this.intl.getDatePattern(
              { format: numberFormat, type: 'time' },
              true
            );
          } catch (error) {
            returnFormat = numberFormat;
  ...
  private processCellStyle(style: CellStyle): number {
    if (style.isGlobalStyle) {
      ...
      let compareResult: { index: number; result: boolean } =
  ...
  private isNewBorder(toCompareStyle: CellStyle): boolean {
    ...
    if (this.isAllBorder(toCompareStyle.borders)) {
      return (
        bStyle.borders.all.color === toCompareStyle.borders.all.color &&
        bStyle.borders.all.lineStyle === toCompareStyle.borders.all.lineStyle
      );
    } else {
      return (
        bStyle.borders.left.color === toCompareStyle.borders.left.color &&
  ...
  private isAllBorder(toCompareBorder: Borders): boolean {
    ...
    return (
      allBorderStyle.borders.all.color !== toCompareBorder.all.color &&
      allBorderStyle.borders.all.lineStyle !== toCompareBorder.all.lineStyle
    );
  }
  private compareStyle(toCompareStyle: CellStyle): {
    index: number;
    result: boolean;
  } {
    ...
    for (let baseStyle of this.mStyles) {
      result = baseStyle.isGlobalStyle
        ? false
        : baseStyle.backColor === toCompareStyle.backColor &&
          ...
          baseStyle.borders.left.lineStyle ===
            toCompareStyle.borders.left.lineStyle &&
          baseStyle.borders.right.color ===
            toCompareStyle.borders.right.color &&
          baseStyle.borders.right.lineStyle ===
            toCompareStyle.borders.right.lineStyle &&
          baseStyle.borders.top.color === toCompareStyle.borders.top.color &&
          baseStyle.borders.top.lineStyle ===
            toCompareStyle.borders.top.lineStyle &&
          baseStyle.borders.bottom.color ===
            toCompareStyle.borders.bottom.color &&
          baseStyle.borders.bottom.lineStyle ===
            toCompareStyle.borders.bottom.lineStyle;
      if (result) {
  ...
  private parseCellType(cell: Cell): void {
    ...
    switch (type) {
      ...
      case 'number':
        ...
        if (cell.cellStyle !== undefined && cell.cellStyle.name !== undefined) {
          if (this.globalStyles.has(cell.cellStyle.name)) {
            this.switchNumberFormat(
              this.globalStyles.get(cell.cellStyle.name).format,
              'number'
            );
          }
        }
  ...
  private parseFilters(json: any, sheet: Worksheet): void {
    ...
    if (json.lastRow !== null && json.lastRow !== undefined)
      ...
      throw new Error(
        'Argument Null Exception: lastRow cannot be null or empty'
      );
    if (json.column !== null && json.column !== undefined)
      sheet.autoFilters.column = json.column;
    else
      throw new Error(
        'Argument Null Exception: column cannot be null or empty'
      );
    if (json.lastColumn !== null && json.row !== undefined)
      sheet.autoFilters.lastColumn = json.lastColumn;
    else
      throw new Error(
        'Argument Null Exception: lastColumn cannot be null or empty'
      );
  }
  ...
  /* tslint:disable:no-any */
  public saveAsBlob(
    blobSaveType: BlobSaveType,
    encodingType?: string
  ): Promise<{ blobData: Blob }> {
    switch (blobSaveType) {
      case 'text/csv':
  ...
  public save(fileName: string, proxyUrl?: string): void {
    if (fileName === null || fileName === undefined || fileName === '') {
      throw new Error(
        'Argument Null Exception: fileName cannot be null or empty'
      );
    }
  ...
  private saveWorksheet(sheet: Worksheet, index: number): void {
      ...
      for (let column of sheet.columns) {
        ...
        if (column.width !== undefined) {
          colString +=
            ...
            column.index +
            '" width="' +
            this.pixelsToColumnWidth(column.width) +
            '" customWidth="1" />';
        } else {
          colString +=
            '<col min="' +
    ...
    if (sheet.rows !== undefined) {
      for (let row of sheet.rows) {
        ...
        if (row.height !== undefined) {
          rowString +=
            'ht="' + this.pixelsToRowHeight(row.height) + '" customHeight="1" ';
        }
        ...
        for (let cell of row.cells) {
          if (
            cell !== undefined &&
            (cell.value !== undefined || cell.cellStyle !== undefined)
          ) {
    ...
    if (sheet.autoFilters !== null && sheet.autoFilters !== undefined)
      sheetString +=
        '<autoFilter ref="' +
        this.getCellName(sheet.autoFilters.row, sheet.autoFilters.column) +
        ':' +
        this.getCellName(
          sheet.autoFilters.lastRow,
          sheet.autoFilters.lastColumn
        ) +
        '"/>';
    ...
    if (sheet.hyperLinks.length > 0) {
      ...
      for (let hLink of sheet.hyperLinks) {
        sheetString +=
          '<hyperlink ref="' + hLink.ref + '" r:id="rId' + hLink.rId + '" />';
      }
      sheetString += '</hyperlinks>';
    }
    /* tslint:disable-next-line:max-line-length */
    ...
    if (sheet.images != undefined && sheet.images.length > 0) {
      ...
      sheetString +=
        '<drawing r:id="rId' + (sheet.hyperLinks.length + 1) + '"/>';
    }
    this.addToArchive(
      this.saveSheetRelations(sheet),
      'xl/worksheets/_rels/sheet' + sheet.index + '.xml.rels'
    );
  ...
  private saveDrawings(sheet: Worksheet, index: number): void {
    ...
    if (sheet.images !== undefined) {
      ...
      for (let pic of sheet.images) {
        ...
        this.addToArchive(
          imageData,
          'xl/media/image' + this.imageCount + '.png'
        );
      }
      ...
      this.addToArchive(
        drawings.getBlob(),
        'xl/drawings/drawing' + this.drawingCount + '.xml'
      );
    }
  }
  private updatelastRowOffset(sheet: Worksheet, picture: Image): void {
    ...
    while (iCurHeight >= 0) {
      ...
      if (sheet.rows !== undefined && sheet.rows[iCurRow - 1] !== undefined)
        iRowHeight = this.convertToPixels(
          sheet.rows[iCurRow - 1].height === undefined
            ? 15
      ...
      if (iSpaceInCell > iCurHeight) {
        ...
        if (sheet.rows !== undefined && sheet.rows[iCurRow - 1] !== undefined)
          rowHiddenHeight = this.convertToPixels(
            sheet.rows[iCurRow - 1].height === undefined
              ? 15
              : sheet.rows[iCurRow - 1].height
          );
        ...
        picture.lastRowOffset = Math.round(
          picture.lastRowOffset / this.unitsProportions[7]
        );
        break;
      } else {
  ...
  private updatelastColumnOffSet(sheet: Worksheet, picture: Image): void {
    ...
    while (iCurWidth >= 0) {
      let iColWidth = 0;
      if (
        sheet.columns !== undefined &&
        sheet.columns[iCurCol - 1] !== undefined
      )
        iColWidth = this.ColumnWidthToPixels(
          sheet.columns[iCurCol - 1].width === undefined
            ? 8.43
      ...
      if (iSpaceInCell > iCurWidth) {
        ...
        if (
          ...
          colHiddenWidth = this.ColumnWidthToPixels(
            sheet.columns[iCurCol - 1].width === undefined
              ? 8.43
              : sheet.columns[iCurCol - 1].width
          );
        ...
        picture.lastColOffset = Math.round(
          picture.lastColOffset / this.unitsProportions[7]
        );
  ...
  private saveDrawingRelations(sheet: Worksheet) {
    ...
    this.addToArchive(
      drawingRelation + '</Relationships>',
      'xl/drawings/_rels/drawing' + this.drawingCount + '.xml.rels'
    );
  }
  ...
  private ColumnWidthToPixels(val: number): number {
    ...
    let fileWidth =
      val > 1
        ...
        : (((val * (dDigitWidth + 5)) / dDigitWidth) * 256.0) / 256.0;
    return this.trunc(
      ((256 * fileWidth + this.trunc(128 / dDigitWidth)) / 256) * dDigitWidth
    );
  }
  ...
  private saveSheetRelations(sheet: Worksheet): string {
    ...
    for (let hLink of sheet.hyperLinks) {
      ...
      relStr +=
        ...
        this.processString(hLink.target) +
        '" TargetMode="External" />';
    }
    if (sheet.images != undefined && sheet.images.length > 0) {
      /* tslint:disable-next-line:max-line-length */
  ...
  private updateCellXfsStyleXfs(): void {
    for (let style of this.mStyles) {
      ...
      //Add font
      let compareFontResult: { index: number; result: boolean } =
        this.isNewFont(style);
      ...
      if (style.numberFormat !== 'GENERAL') {
        if (this.mNumFmt.has(style.numberFormat)) {
          ...
          this.mNumFmt.set(
            style.numberFormat,
            new NumFmt(id, style.numberFormat)
          );
          cellXfs.numFmtId = id;
        }
      } else {
    ...
    if (this.mBorders.length >= 1) {
      for (let borders of this.mBorders) {
        if (this.isAllBorder(borders)) {
          ...
          bordersStyle +=
            '<border><left style="' +
            borders.left.lineStyle +
            '"><color rgb="FF' +
            borders.left.color.replace('#', '') +
            '" /></left><right style="' +
            borders.right.lineStyle +
            '"><color rgb="FF' +
            borders.right.color.replace('#', '') +
            '" /></right><top style="' +
            borders.top.lineStyle +
            '"><color rgb="FF' +
            borders.top.color.replace('#', '') +
            '" /></top><bottom style="' +
            borders.bottom.lineStyle +
            '"><color rgb="FF' +
            borders.bottom.color.replace('#', '') +
            '" /></bottom></border>';
        }
      }
    }
  ...
  private saveCellStyles(): string {
    let cellStyleString: string =
      '<cellStyles  count="' + this.cellStyles.size + '">';
    this.cellStyles.forEach((value: CellStyles, key: string) => {
      cellStyleString +=
        '<cellStyle name="' +
        key +
        '" xfId="' +
        this.cellStyles.get(key).xfId +
        '"';
      if (key === 'Normal') {
  ...
  private saveCellStyleXfs(): string {
    ...
    if (this.mCellStyleXfs.length >= 1) {
      for (let cellStyleXf of this.mCellStyleXfs) {
        ...
        cellXfsStyle +=
          '<xf numFmtId="' +
          cellStyleXf.numFmtId +
          '" fontId="' +
          cellStyleXf.fontId +
          '" fillId="' +
          cellStyleXf.fillId +
          '" borderId="' +
          cellStyleXf.borderId +
          '" ';
  ...
  private saveAlignment(cellXf: any): string {
    ...
    if (
      cellXf.alignment.indent !== undefined &&
      cellXf.alignment.indent !== 0
    ) {
      alignString += 'indent="' + cellXf.alignment.indent + '" ';
    } else if (
      cellXf.alignment.rotation !== undefined &&
      cellXf.alignment.rotation !== 0
    ) {
  ...
  private saveCore(builtInProperties: BuiltInProperties): void {
    ...
    if (this.builtInProperties !== undefined) {
      if (builtInProperties.author !== undefined) {
        coreString +=
          '<dc:creator>' + builtInProperties.author + '</dc:creator>';
      }
      if (builtInProperties.subject !== undefined) {
        coreString +=
          '<dc:subject>' + builtInProperties.subject + '</dc:subject>';
      }
      if (builtInProperties.category !== undefined) {
        coreString +=
          '<cp:category>' + builtInProperties.category + '</cp:category>';
      }
      if (builtInProperties.comments !== undefined) {
        coreString +=
          '<dc:description>' + builtInProperties.comments + '</dc:description>';
      }
      ...
      if (builtInProperties.tags !== undefined) {
        coreString +=
          '<cp:keywords>' + builtInProperties.tags + '</cp:keywords>';
      }
      ...
      if (builtInProperties.createdDate !== undefined) {
        ...
        coreString +=
          ...
          builtInProperties.createdDate.toISOString() +
          '</dcterms:created>';
      } else {
        coreString +=
          '<dcterms:created xsi:type="dcterms:W3CDTF">' +
      ...
      if (builtInProperties.modifiedDate !== undefined) {
        ...
        coreString +=
          ...
          builtInProperties.modifiedDate.toISOString() +
          '</dcterms:modified>';
      } else {
        coreString +=
          '<dcterms:modified xsi:type="dcterms:W3CDTF">' +
  ...
  private saveContentType(): void {
    ...
    for (let i: number = 0; i < length; i++) {
      ...
      if (
        this.worksheets[i].images != undefined &&
        this.worksheets[i].images.length > 0
      ) {
        drawingIndex++;
        /* tslint:disable-next-line:max-line-length */
        sheetsOverride +=
          '<Override PartName="/xl/drawings/drawing' +
          drawingIndex.toString() +
    ...
    this.addToArchive(
      contentTypeString + sheetsOverride + '</Types>',
      '[Content_Types].xml'
    );
  }
  ...
  private processMergeCells(
    ...
    if (cell.rowSpan !== 0 || cell.colSpan !== 0) {
      ...
      let endCell: string = this.getCellName(
        rowIndex + mCell.height,
        cell.index + mCell.width
      );
  ...
  private updatedMergedCellStyles(
    sCell: { x: number; y: number },
    eCell: { x: number; y: number },
    cell: Cell
  ): void {
  ...
  private dateToTicks(year: number, month: number, day: number): number {
    ...
    let daysToMonth365: number[] = [
      0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365,
    ];
    let daysToMonth366: number[] = [
      0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335, 366,
    ];
    if (year >= 1 && year <= 9999 && month >= 1 && month <= 12) {
      let days: number[] = this.isLeapYear(year)
        ? daysToMonth366
        : daysToMonth365;
      let y: number = year - 1;
      let n: number =
  ...
  private toOADate(date: Date): number {
    ...
    ticks =
      ...
      this.timeToTicks(date.getHours(), date.getMinutes(), date.getSeconds());
    if (ticks === 0) {
      return 0.0;
    }
    let ticksPerDay: number = 10000 * 1000 * 60 * 60 * 24;
    let daysTo1899: number =
      (((365 * 4 + 1) * 25 - 1) * 4 + 1) * 4 +
      ((365 * 4 + 1) * 25 - 1) * 3 -
      367;
