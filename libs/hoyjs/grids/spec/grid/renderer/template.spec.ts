...
describe('Template render module', () => {
  describe('column template render', () => {
      ...
      gridObj = createGrid(
        {
          ...
          columns: [
            { field: 'EmployeeID' },
            {
              template: '<div>${EmployeeID}</div><div>${index}</div>',
    ...
    it('template render testing', () => {
      ...
      expect(trs[0].querySelector('.e-templatecell').innerHTML).toBe(
        '<div>5</div><div>0</div>'
      );
      expect(trs[1].querySelector('.e-templatecell').innerHTML).toBe(
        '<div>6</div><div>1</div>'
      );
      expect(
        gridObj.getHeaderTable().querySelectorAll('.e-headercelldiv')[1]
          .innerHTML
      ).toBe('<span class="e-headertext">Template column</span>');
    });
  ...
  describe('column template element render', () => {
    ...
    describe('column template element with column object', () => {
      ...
      it('cell value testing with colum object', () => {
        ...
        expect(trs[0].querySelectorAll('td')[0].innerHTML).toBe(
          'Template column'
        );
      });
      ...
      gridObj = createGrid(
        {
          ...
          columns: [{ field: 'EmployeeID', headerText: 'Employee ID' }],
        },
        done
      );
    });
  ...
  describe('caption template render', () => {
    ...
    it('check caption template', () => {
      let rows: HTMLTableRowElement = (<any>(
        (gridObj.getContentTable() as HTMLTableElement).querySelector(
          '.e-summaryrow'
        )
      ...
      gridObj = createGrid(
        {
          ...
          allowGrouping: true,
          groupSettings: {
            captionTemplate: '<div>${EmployeeID}</div>',
            columns: ['EmployeeID'],
          },
          columns: [
            { field: 'EmployeeID', headerText: 'Employee ID' },
