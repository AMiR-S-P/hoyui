...
import { EmitType, createElement, extend } from '@hoyui/base';
...
describe('Virtual-freeze-renderer --- row virtualization', () => {
  describe('Freeze Column', () => {
    ...
    it('Collect html elements', () => {
      fCont = gridObj.getContent();
      // mCont = gridObj.getContent().querySelector('.e-movablecontent');
      // mHdr = gridObj.getHeaderContent();
      fHdr = gridObj.getHeaderContent();
    ...
    it('Frozen column testing', () => {
      expect(
        fCont.querySelector('.e-row').querySelectorAll('.e-leftfreeze').length
      ).toBe(gridObj.frozenColumns);
      let colsLength: number = gridObj.getColumns().length;
      expect(
        fCont.querySelector('.e-row').querySelectorAll('.e-unfreeze').length
      ).toBe(colsLength - gridObj.frozenColumns);
    });

    it('Ensure tables, virtualtracks and its count', () => {
      expect(
        gridObj.getContent().querySelectorAll('.e-virtualtable').length
      ).toBe(1);
      // expect(parentsUntil(mCont, 'e-virtualtable')).toBeTruthy();
      expect(
        gridObj.getHeaderContent().querySelectorAll('.e-virtualtable').length
      ).toBe(0);
      // expect(parentsUntil(mHdr, 'e-virtualtable')).toBeFalsy();
      expect(
        gridObj.getContent().querySelectorAll('.e-virtualtrack').length
      ).toBe(1);
      expect(
        gridObj.getHeaderContent().querySelectorAll('.e-virtualtrack').length
      ).toBe(0);
    });
    ...
    it('Ensure transform values before scroll', () => {
      let mVTblTrans: { width: number; height: number } =
        getTransformValues(virtualTable);
      expect(mVTblTrans.height).toBe(0);
    });

    it('Ensure position absolute', () => {
      expect(virtualTable.style.position).toBe('absolute');
    });
    ...
    it('Check rows count after scrolling', () => {
      // let fcnt: Element = gridObj.getContent();
      let mVTblTrans: { width: number; height: number } =
        getTransformValues(virtualTable);
  ...
  describe('Freeze Row and Column', () => {
    ...
    it('Change dynamically', () => {
      (gridObj.dataBound = function (done: Function) {
        ...
        expect(
          fcnt.querySelector('.e-row').querySelectorAll('.e-leftfreeze').length
        ).toBe(gridObj.frozenColumns);
        let colsLength: number = gridObj.getColumns().length;
        expect(
          fcnt.querySelector('.e-row').querySelectorAll('.e-unfreeze').length
        ).toBe(colsLength - gridObj.frozenColumns);
        let fHdr: Element = gridObj.getHeaderContent();
  ...
  describe('Ensure new methods', () => {
    ...
    it('Ensure getRowByIndex methods', () => {
      let fcnt: Element = gridObj.getContent();
      // let mcnt: Element = gridObj.getContent().querySelector('.e-movablecontent');
      ...
      let fRow1: Element = gridObj.contentModule.getRowByIndex(
        gridObj.frozenRows
      );
