...
describe('LazyLoadGroup module', () => {
  describe('Grouping test', () => {
    ...
    it('Group ProductName column', (done: Function) => {
      let actionComplete = function (args: any) {
        ...
        expect(
          gridObj.contentModule.groupCache[gridObj.pageSettings.currentPage]
            .length
        ).toBe(gridObj.pageSettings.pageSize);
        let scrollEle: Element = gridObj.getContent().firstElementChild;
        let blockSize: number =
          Math.floor(
            (scrollEle as HTMLElement).offsetHeight / gridObj.getRowHeight()
          ) - 1;
        ...
        expect(
          gridObj.contentModule.initialGroupCaptions[
            gridObj.pageSettings.currentPage
          ].length
        ).toBe(gridObj.pageSettings.pageSize);
        expect(gridObj.contentModule.initialPageRecords.length).toBe(
          gridObj.pageSettings.pageSize
        );
        expect(gridObj.getContent().querySelector('.e-row')).toBeNull();
        expect(gridObj.getContent().querySelectorAll('tr').length).toBe(
          gridObj.pageSettings.pageSize
        );
        expect(gridObj.contentModule.getInitialCaptionIndexes().length).toBe(
          gridObj.pageSettings.pageSize
        );
        expect(
          Object.keys(
            gridObj.contentModule.rowsByUid[gridObj.pageSettings.currentPage]
          ).length
        ).toBe(gridObj.pageSettings.pageSize);
        expect(
          Object.keys(
            gridObj.contentModule.objIdxByUid[gridObj.pageSettings.currentPage]
          ).length
        ).toBe(gridObj.pageSettings.pageSize);
        expect(
          gridObj.contentModule.startIndexes[gridObj.pageSettings.currentPage]
        ).toBeUndefined();
    ...
    it('caption expand', (done: Function) => {
      let expandElem = gridObj
        .getContent()
        .querySelectorAll('.e-recordpluscollapse');
    ...
    it('caption collapse', (done: Function) => {
      ...
      expect(gridObj.getContent().querySelectorAll('tr').length).not.toBe(
        gridObj.pageSettings.pageSize
      );
      ...
      expect(
        gridObj.contentModule.groupCache[gridObj.pageSettings.currentPage]
          .length
      ).toBeGreaterThan(gridObj.pageSettings.pageSize);
      expect(
        gridObj.contentModule.startIndexes[gridObj.pageSettings.currentPage]
          .length
      ).toBe(gridObj.pageSettings.pageSize);
      ...
      ).not.toBe(gridObj.pageSettings.pageSize);
      expect(
        Object.keys(
          gridObj.contentModule.objIdxByUid[gridObj.pageSettings.currentPage]
        ).length
    ...
    it('expand shortcut ProductName', (done: Function) => {
      ...
      let keyDownArgs = {
        altKey: true,
        keyCode: 74,
        target: gridObj.getContent().querySelectorAll('tr')[0].cells[0],
      };
      let lazyLoadGroupExpand = function (args: any) {
        expandShortcut = true;
        gridObj.lazyLoadGroupExpand = null;
    ...
    it('expand shortcut ProductID', (done: Function) => {
      ...
      let keyDownArgs = {
        altKey: true,
        keyCode: 74,
        target: gridObj.getContent().querySelectorAll('tr')[1].cells[0],
      };
      let lazyLoadGroupExpand = function (args: any) {
        gridObj.lazyLoadGroupExpand = null;
        setTimeout(done, 100);
  ...
  describe('LazyLoadGroup with aggregate and edit', () => {
      ...
      gridObj = createGrid(
        {
          ...
          groupSettings: {
            ...
            columns: ['ProductName', 'CustomerID'],
          },
          editSettings: {
            allowEditing: true,
            allowAdding: true,
    ...
    it('caption expand', (done: Function) => {
      const expandElem: NodeListOf<Element> = gridObj
        .getContent()
        .querySelectorAll('.e-recordpluscollapse');
      gridObj.groupModule.expandCollapseRows(expandElem[0]);
      setTimeout(done, 200);
    ...
    it('edit first row of first caption', () => {
      const dataRow: HTMLElement = gridObj
        .getContentTable()
        .querySelector('.e-row');
    ...
    it('edit group caption aggregate', (done: Function) => {
      const groupLazyLoadRenderer: GroupLazyLoadRenderer =
        gridObj.contentModule as GroupLazyLoadRenderer;
      ...
      max =
        parseInt(
          (currentPageGroupCache[0].data as any).aggregates['ProductID - max'],
          10
        ) + 1;
      (
        select(
          '#' + gridObj.element.id + (gridObj.columns[5] as Column).field,
          gridObj.element
        ) as any
      ).value = max.toString();
      gridObj.endEdit();
  ...
  describe('Lazy Load Group with Infinite scroll => ', () => {
    ...
    it('check initial render', function () {
      ...
      let captionRows: NodeListOf<HTMLTableRowElement> = gridObj
        .getContentTable()
        .querySelectorAll('tr');
    ...
    it('check datas after scroll and expand', function () {
      ...
      let initialBlocks: number = gridObj.infiniteScrollSettings.initialBlocks;
      let currentPage: number = gridObj.pageSettings.currentPage;
      let totalTr: NodeListOf<HTMLTableRowElement> = gridObj
        .getContentTable()
        .querySelectorAll('tr');
      let level1CaptionRows: NodeListOf<HTMLTableRowElement> = gridObj
        .getContent()
    ...
    it('expand level2 group', function () {
      let dataRows: NodeListOf<HTMLTableRowElement> = gridObj
        .getContentTable()
    ...
    it('Check edit state, edit and save', function () {
      ...
      expect(document.activeElement.id).toBe(
        gridObj.element.id + (gridObj.columns[2] as Column).field
      );
      ...
      expect(
        (
          select(
            '#' + gridObj.element.id + (gridObj.columns[3] as Column).field,
            gridObj.element
          ) as any
        ).value
      ).toBe(
        gridObj.dataSource[rowindex][(gridObj.columns[3] as Column).field]
      );
      (
        select(
          '#' + gridObj.element.id + (gridObj.columns[3] as Column).field,
    ...
    it('start add', function (done: Function) {
      ...
      let actionComplete = function (args: NotifyArgs) {
        if (args.requestType === 'add') {
          ...
          expect(gridObj.getContent().firstElementChild.scrollTop).toBe(0);
          data1 =
            gridObj.dataSource[4999][(gridObj.columns[0] as Column).field];
          gridObj.actionComplete = null;
          done();
        }
      };
    ...
    it('Save add', function (done: Function) {
      let actionComplete = function (args: NotifyArgs) {
        if (args.requestType === 'save') {
          ...
          expect(
            gridObj.dataSource[0][(gridObj.columns[3] as Column).field]
          ).toBe('updated');
    ...
    it('check data row, select and delete', function (done: Function) {
      ...
      gridObj.selectRow(rowindex);
      rowindex =
        gridObj.selectionModule['data'][(gridObj.columns[2] as Column).field];
  ...
  describe('Lazy Load Group with Virtual scroll => ', () => {
    ...
    it('check current page, data append and expand level1 group', () => {
      let blockSize: number = (<VirtualContentRenderer>(
        gridObj.contentModule
      )).getBlockSize();
      let blockLen: number = (<VirtualContentRenderer>gridObj.contentModule)
        .currentInfo.blockIndexes.length;
      let captionRows: NodeListOf<HTMLTableRowElement> = gridObj
        .getContentTable()
        .querySelectorAll('tr');
  ...
  describe('Lazy Load Group code coverage => ', () => {
    ...
    it('coevarge the getCacheRowsOnDownScroll and getCacheRowsOnUpScroll', function () {
      ...
      (groupLazyLoadRenderer as any).getCacheRowsOnUpScroll(
        rows[0].uid,
        rows[5].uid,
        0
      );
    ...
    it('coevarge the scrollDownHandler', function () {
      (groupLazyLoadRenderer as any).scrollDownHandler(
        gridObj.getDataRows()[gridObj.getDataRows().length - 1]
      );
    });

    it('coevarge the removeTopRows and removeBottomRows', function () {
      (groupLazyLoadRenderer as any).removeTopRows(
        rows[0].uid,
        rows[1].uid,
        rows[2].uid
      );
      (groupLazyLoadRenderer as any).removeBottomRows(
        rows[4].uid,
        rows[5].uid,
        rows[6].uid
      );
    ...
    it('coevarge the clearLazyGroupCache', function () {
      ...
      (groupLazyLoadRenderer as any).moveCells(
        gridObj.getRowsObject()[0].cells,
        3,
        1
      );
    });
