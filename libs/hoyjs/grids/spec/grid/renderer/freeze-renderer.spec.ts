...
describe('Freeze render module', () => {
  describe('Freeze Row and Column', () => {
    ...
    it('Frozen Header testing', () => {
      expect(
        gridObj.getHeaderContent().querySelector('tbody').childElementCount
      ).toBe(2);
      ...
      expect(
        gridObj.getFrozenRightHeader().querySelector('tbody').childElementCount
      ).toBe(2);
      expect(
        gridObj
          .getHeaderContent()
          .querySelector('tbody')
          .children[0].querySelectorAll('.e-leftfreeze').length
      ).toBe(2);
      expect(
        gridObj
          .getHeaderContent()
          .querySelector('tr')
          .querySelectorAll('.e-leftfreeze').length
      ).toBe(2);
    });
    ...
    it('Frozen Content testing', () => {
      expect(
        gridObj.getContent().querySelector('tbody').childElementCount
      ).toBeGreaterThan(1);
      expect(
        gridObj.getFrozenLeftContentTbody().childElementCount
      ).toBeGreaterThan(1);
      expect(
        gridObj.getFrozenRightContentTbody().childElementCount
      ).toBeGreaterThan(1);
      expect(
        gridObj.getMovableContentTbody().childElementCount
      ).toBeGreaterThan(1);
      expect(
        gridObj
          .getContent()
  ...
  describe('Freeze Row', () => {
    ...
    it('Movable Content testing', () => {
      ...
      expect(
        gridObj.getFrozenRightContent().querySelector('tbody').childElementCount
      ).toBeGreaterThan(1);
    });
  ...
  describe('Freeze Row with virtualization', () => {
    ...
    it('execute methods', () => {
      let rows: any = (gridObj.contentModule as any).getReorderedRows({
        virtualInfo: (gridObj.contentModule as any).prevInfo,
      });
      rows = null;
      expect(1).toBe(1);
    });
  ...
  describe('Freeze Column', () => {
    ...
    it('Ensure aggregate row visiblity', () => {
      let isHide: boolean = gridObj.element
        .querySelector('.e-summaryrow')
  ...
  describe('Freeze Column', () => {
    ...
    it('Aggregate checking', () => {
      expect(
        gridObj.element.querySelector('.e-summarycontent').childNodes.length
      ).toBe(1);
    });
    ...
    it('clip board testing', () => {
      gridObj.selectRow(2);
      gridObj.copy();
      expect((<any>gridObj.clipboardModule).copyContent).toBe(
    ...
    it('copy with header', () => {
      gridObj.copy(true);
      expect((<any>gridObj.clipboardModule).copyContent).toBe(
        'CustomerID	OrderID	EmployeeID	ShipCountry	ShipCity\nHANAR	10250	4	Brazil	Rio de Janeiro'
      );
    });
    it('memory leak', () => {
      profile.sample();
  ...
  describe('EJ2-39341 - Row height is not set properly in the grid when having frozen column', () => {
    ...
    it('Ensure Rows Height', () => {
      let fHead: HTMLElement = gridObj.getHeaderContent().querySelector('table')
        .rows[2];
      // let mHead: HTMLElement = gridObj.element.querySelector('.e-movableheader').querySelector('table').rows[2];
      let fContent: HTMLElement = gridObj.getContent().querySelector('table')
        .rows[0];
  ...
  describe('EJ2-45329 => Horizontal scroll bar is not displayed while using Frozen columns', () => {
    ...
    it('Ensure e-content element height', () => {
      expect(
        (gridObj.getContent().firstElementChild as HTMLElement).style.height
      ).toBe('100%');
    });
    afterAll(() => {
      // gridObj['freezeModule'].destroy();
  ...
  describe('Frozen Grid Mask Row', () => {
    ...
    it('Show Mask Row', () => {
      ...
      expect(
        frozenLeftHeaderElement.querySelector('.e-masked-table')
      ).toBeTruthy();
      ...
      expect(
        frozenLeftContentElement.querySelector('.e-masked-table')
      ).toBeTruthy();
      ...
      expect(
        frozenLeftFooterElement.querySelector('.e-masked-table')
      ).toBeTruthy();
    ...
    it('Refresh Mask Row', () => {
      ...
      expect(
        frozenLeftHeaderElement.querySelector('.e-masked-table').clientWidth
      ).toBe(
        frozenLeftHeaderElement.querySelector('.e-table:not(.e-masked-table)')
          .clientWidth
      );
      ...
      expect(
        frozenLeftContentElement.querySelector('.e-masked-table').clientWidth
      ).toBe(
        frozenLeftContentElement.querySelector('.e-table:not(.e-masked-table)')
          .clientWidth
      );
    ...
    afterAll(() => {
      destroy(gridObj);
      gridObj =
        frozenLeftHeaderElement =
        movableHeaderElement =
        frozenRightHeaderElement =
          null;
      frozenLeftContentElement =
        movableContentElement =
        frozenRightContentElement =
          null;
      frozenLeftFooterElement =
        movableFooterElement =
        frozenRightFooterElement =
          null;
    });
  });
  ...
  describe('EJ2-67886 => colSpan is not working with Frozen Grid', () => {
    ...
    it('Ensure colSpan for the cell', () => {
      expect(
        (
          gridObj.getContentTable() as HTMLTableElement
        ).rows[0].cells[1].getAttribute('colSpan')
      ).toBe('2');
    });
    // code coverage
  ...
  describe('EJ2-900729 - When freeze property is enabled the minWidth and maxWidth set to a column is not working properly', () => {
    ...
    it('check th width', () => {
      expect(
        gridObj.getHeaderTable().querySelector('th').getBoundingClientRect()
          .width
      ).toBe(30);
    });
  ...
  describe('EJ2-900822 - The Grid is getting autofitted when column chooser is used', () => {
    ...
    it('check table style width', () => {
      expect((gridObj.getHeaderTable() as HTMLElement).style.width).toBe(
        '100%'
      );
    });
