...
describe('header renderer module', () => {
  describe('grid header element testing', () => {
    ...
    it('Header table testing', () => {
      expect(
        gridObj.headerModule.getPanel().querySelectorAll('.e-table').length
      ).toBe(1);
    });

    it('Column header testing', () => {
      expect(
        gridObj.headerModule.getPanel().querySelectorAll('.e-columnheader')
          .length
      ).toBe(1);
    });
    ...
    it('Column count testing', () => {
      ...
      //for coverage
      let hRender = (<any>gridObj).renderModule.locator
        .getService('cellRendererFactory')
        .getCellRenderer(CellType.Header);
      hRender.refresh(
        { column: gridObj.getColumns()[1] } as any,
        createElement('div')
      );
    });
  ...
  describe('Header template element render', () => {
    ...
    it('header testing', () => {
      ...
      ).headerRenderer.draggable.currentStateTarget = target;
  ...
  describe('Autogenerate columns', () => {
    ...
    it('Checking the header and content table', () => {
      expect(
        gridObj.element.querySelector('.e-gridheader').querySelectorAll('table')
          .length
      ).toBe(1);
      expect(
        gridObj.element
  ...
  describe('Value accessor for header content', () => {
          ...
          columns: [
            {
              field: 'ShipCity',
              headerText: 'shipcity',
              headerValueAccessor: (field: string, column: object): string => {
                return 'HeaderName';
              },
            },
            { field: 'EmployeeID' },
    ...
    it('header text testing', () => {
      let innerTxt = gridObj
        .getHeaderContent()
        .querySelectorAll('th')[0].innerText;
