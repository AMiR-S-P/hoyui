...
describe('Render module', () => {
  describe('Grid render', () => {
    ...
    it('Row count testing', () => {
      ...
      (<any>gridObj.renderModule).updateColumnType({
        ...
        EmployeeID: new Date(2017, 2, 13, 0, 0, 10, 0),
        CustomerID: new Date(2017, 2, 13, 0, 10, 0, 0),
        ShipCity: new Date(2017, 2, 13, 10, 0, 0, 0),
        ShipCountry: new Date(2017, 2, 13, 0, 0, 0, 0),
        OrderDate: new Date(2017, 2, 13, 0, 10, 0, 10),
      });
      (<any>gridObj.renderModule).data.removeRows({
        indexes: [4, 5],
        records: data.slice(4, 5),
      });
      ...
      gridObj = createGrid(
        {
          dataSource: [],
          allowPaging: false,
          columns: [{ field: 'Column1', type: 'string' }, { field: 'Column2' }],
        },
        done
      );
    });
  ...
  describe('Row height checking', () => {
    ...
    it('Row height API checking  checking', () => {
      expect(
        (gridObj.element.querySelectorAll('.e-row')[0] as HTMLElement).style
          .height
      ).toBe('50px');
    });
    ...
    it('Row height on property change checking', () => {
      ...
      gridObj.dataBind();
      expect(
        gridObj.element.classList.contains('e-grid-min-height')
      ).toBeTruthy();
