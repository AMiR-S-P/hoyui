...
describe('Adaptive renderer', () => {
  describe('Ensure adaptive dialogs', () => {
    ...
    it('Ensure initial rendered elements', () => {
      expect(
        document
          .querySelector('.e-toolbar-item[title=Edit]')
          .classList.contains('e-hidden')
      ).toBeTruthy();
      expect(
        document
          .querySelector('.e-toolbar-item[title=Delete]')
          .classList.contains('e-hidden')
      ).toBeTruthy();
      expect(
        document
          .querySelector('.e-toolbar-item[title=Update]')
          .classList.contains('e-hidden')
      ).toBeTruthy();
      expect(
        document
          .querySelector('.e-toolbar-item[title=Cancel]')
          .classList.contains('e-hidden')
      ).toBeTruthy();
      // expect(document.querySelector('.e-toolbar-item[title=Print]').classList.contains('e-hidden')).toBeFalsy();
      expect(
        document
          .querySelector('.e-gridresponsiveicons')
          .classList.contains('e-hidden')
      ).toBeTruthy();
      expect(
        document
      ...
      expect(
        document
          .getElementsByClassName('e-summaryrow')[0]
          .querySelectorAll('.e-summarycell:not([style="display: none;"])')
          .length
      ).toBe(5);
      expect(
        document.querySelector('.e-rowcell').getAttribute('data-cell')
      ).toBeNull();
    ...
    it('Adaptive filter dialog check', (done: Function) => {
      let actionComplete = (args?: any): void => {
        if (args.requestType === 'filterchoicerequest') {
          expect(
            document.getElementsByClassName('e-responsive-dialog').length
          ).not.toBe(0);
          expect(
            document.getElementsByClassName('e-resfilterdiv').length
          ).not.toBe(0);
          expect(
            document
              .getElementsByClassName('e-responsive-dialog')[0]
              .classList.contains('e-bigger')
          ).toBeTruthy();
          expect(
            document
              .getElementsByClassName('e-resfilterdiv')[0]
              .querySelector('.e-res-custom-element')
          ).not.toBeNull();
      ...
      gridObj.filterModule.responsiveDialogRenderer.showResponsiveDialog(
        gridObj.getColumns()[0]
      );
    });
  ...
  describe('Ensure vertical dialog rendering', () => {
    ...
    it('Ensure initial rendered elements', () => {
      ...
      expect(
        document.querySelectorAll('.e-gridresponsiveicons:not(.e-hidden)')
          .length
      ).toBe(2);
      expect(
        document.querySelector(
          '.e-gridresponsiveicons.e-hidden .e-resback-icon'
        )
      ).not.toBeNull();
      expect(
        document
    ...
    it('Open custom filter dialog', () => {
      expect(gridObj.filterModule.responsiveDialogRenderer.action).toBe(
      ...
      expect(document.getElementsByClassName('e-customfilterdiv').length).toBe(
        1
      );
    });
    ...
    it('Adaptive filter dialog check', (done: Function) => {
      let actionComplete = (args?: any): void => {
        if (args.requestType === 'filterchoicerequest') {
          expect(
            (
              document
                .querySelector('.e-resfilterdiv')
                .querySelector('.e-dlg-custom-header') as HTMLElement
            ).innerText
          ).toBe(args.filterModel.options.column.headerText);
          expect(
            document.getElementsByClassName('e-responsive-dialog').length
          ).toBe(2);
          gridObj.actionComplete = null;
      ...
      gridObj.actionComplete = actionComplete;
      let customDlgCnt: HTMLElement = document.querySelector(
        '.e-customfilterdiv > .e-dlg-content'
      );
    ...
    it('Open excel filter text filters', () => {
      ...
      let filterContent: HTMLElement = document.querySelector(
        '.e-resfilterdiv > .e-dlg-content'
      );
      expect(
        filterContent.querySelector('.e-res-contextmenu-wrapper')
      ).not.toBeNull();
    });
    ...
    it('Ensure custom excel filter', () => {
      ...
      (
        filterContent.querySelector('.e-submenu.e-menu-item') as HTMLElement
      ).click();
      (
        filterContent.querySelector(
          '.e-excel-menu > .e-menu-item '
        ) as HTMLElement
      ).click();
      let filterCloseBtn: HTMLElement = document.querySelector(
        '.e-resfilterdiv > .e-dlg-header-content'
      );
      (
        filterCloseBtn.querySelector('.e-dlg-closeicon-btn') as HTMLElement
      ).click();
      expect(
        document.getElementsByClassName('e-responsive-dialog').length
      ).toBe(0);
      expect(
        gridObj.filterModule.responsiveDialogRenderer.isRowResponsive
      ).toBeFalsy();
    });
    ...
    it('Filter reset button check', (done: Function) => {
      ...
      let filterReset: HTMLElement = document
        .querySelector('.e-customfilterdiv > .e-dlg-content')
        .querySelector('.e-filterset');
    ...
    it('Open custom sort dialog', (done: Function) => {
      expect(gridObj.sortModule.responsiveDialogRenderer.action).toBe(
        ResponsiveDialogAction.isSort
      );
      ...
      expect(document.getElementsByClassName('e-ressortbutton').length).toBe(
        gridObj.getColumns().length
      );
      (
        document.getElementsByClassName('e-ressortbutton')[0] as HTMLElement
      ).click();
      (
        document.getElementsByClassName('e-ressortbutton')[0] as HTMLElement
      ).click();
      let actionComplete = (args: NotifyArgs) => {
        if (args.requestType === 'sorting') {
          expect(gridObj.currentViewData[0]['OrderID']).toBe(
            gridObj.dataSource[gridObj.dataSource.length - 1]['OrderID']
          );
          gridObj.actionComplete = null;
          done();
        }
      ...
      gridObj.actionComplete = actionComplete;
      (
        document.getElementsByClassName('e-res-apply-btn')[0] as HTMLElement
      ).click();
    });
    ...
    it('Reopen custom sort dialog', (done: Function) => {
      ...
      gridObj.actionComplete = actionComplete;
      let filterCloseBtn: HTMLElement = document.querySelector(
        '.e-ressortdiv > .e-dlg-header-content'
      );
    ...
    it('Ensure search', () => {
      expect(
        gridObj.element.querySelector('.e-grid .e-toolbar .e-input-group')
      ).toBeNull();
      expect(
        gridObj.element.querySelector(
          '.e-grid .e-toolbar .e-input-group-icon.e-search-icon'
        )
      ).not.toBeNull();
    });
    ...
    it('Check Custom toolbar', () => {
      ...
      expect(
        document
          .getElementsByClassName('e-res-toolbar')[0]
          .querySelectorAll('.e-gridresponsiveicons').length
      ).toBe(3);
    });
      ...
      gridObj = createGrid(
        {
          dataSource: [
            {
              OrderID: 10248,
              CustomerID: 'column shows ellipsis with tooltip',
              EmployeeID: 1,
            },
          ],
          enableAdaptiveUI: true,
  ...
  describe('Ensure onproperty change', () => {
    ...
    it('Filter reset button check', (done: Function) => {
      ...
      let filterClear: HTMLElement = document
        .querySelector('.e-customfilterdiv > .e-dlg-header-content')
        .querySelector('.e-res-filter-clear-btn');
      let actionComplete = (args?: NotifyArgs): void => {
    ...
    it('Clear filter by using excel header clear button', (done: Function) => {
      ...
      (
        filterContent.querySelector('.e-res-filter-clear-btn') as HTMLElement
      ).click();
    });
  ...
  describe('Ensure vertical dialog rendering', () => {
    ...
    it('Ensure custom sort dialog columns', () => {
      ...
      expect(
        document.querySelector(
          '.e-responsivecoldiv[data-e-mappingname="CustomerID"]'
        )
      ).toBeNull();
      expect(
        document.querySelector(
          '.e-responsivecoldiv[data-e-mappingname="ShipCountry"]'
        )
      ).toBeNull();
    ...
    it('Ensure custom filter dialog columns', () => {
      ...
      expect(
        document.querySelector(
          '.e-responsivecoldiv[data-e-mappingname="EmployeeID"]'
        )
      ).toBeNull();
      (document.querySelector('.e-dlg-closeicon-btn') as HTMLElement).click();
    });
  ...
  describe('EJ2-49530 - Grid’s adaptive view filter function is not working properly ', () => {
    ...
    it('Open the Adaptive dialog ', (done: Function) => {
      ...
      gridObj.element
        .querySelector('.e-grid .e-toolbar .e-resfilter-icon')
        .click();
    });
  ...
  describe('EJ2-824777 - Textwrap is not working properly with vertical row rendering', () => {
    ...
    it('Ensuring the CSS class', () => {
      ...
      expect(
        gridObj.getRows()[0].classList.contains('e-verticalwrap')
      ).toBeTruthy();
    });
  ...
  describe('Ensure vertical dialog rendering with toolbar and column chooser', () => {
    ...
    it('ToolbarMenu popup enure', (done: Function) => {
      (<any>gridObj.toolbarModule).toolbarClickHandler({
        item: { id: gridObj.element.id + '_responsivetoolbaritems' },
      });
      gridObj.toolbarModule.toolbarMenuElement
        .querySelector('.e-pdfexport')
        .click();
      (<any>gridObj.toolbarModule).toolbarClickHandler({
        item: { id: gridObj.element.id + '_responsivetoolbaritems' },
      });
      gridObj.toolbarModule.toolbarMenuElement
        .querySelector('.e-excelexport')
        .click();
      (<any>gridObj.toolbarModule).toolbarClickHandler({
        item: { id: gridObj.element.id + '_responsivetoolbaritems' },
      });
      gridObj.toolbarModule.toolbarMenuElement
        .querySelector('.e-csvexport')
        .click();
      (<any>gridObj.toolbarModule).toolbarClickHandler({
        item: { id: gridObj.element.id + '_responsivetoolbaritems' },
      });
      gridObj.toolbarModule.toolbarMenuElement
        .querySelector('.e-columnchooser')
        .click();
      done();
    });

    it('ToolbarMenu popup column chooser popup close', () => {
      let columnChooseHeader: HTMLElement = document.querySelector(
        '.e-rescolumnchooserdiv > .e-dlg-header-content'
      );
      (
        columnChooseHeader.querySelector('.e-dlg-closeicon-btn') as HTMLElement
      ).click();
      expect(
        document.getElementsByClassName('e-responsive-dialog').length
      ).toBe(0);
  ...
  describe('Code Coverage - 1', () => {
    ...
    it('Adaptive coverage - 3', () => {
      ...
      gridObj.filterModule.responsiveDialogRenderer.removeCustomDlgFilterEle(
        document.querySelector('.e-rowcell')
      );
      gridObj.filterModule.responsiveDialogRenderer.keyHandler({
        keyCode: 13,
        target: document.querySelector('.e-searchinput'),
      });
    });
    ...
    it('Adaptive coverage - 6', () => {
      (gridObj as any).headerModule.updateCustomResponsiveToolbar({
        module: 'grid',
      });
      gridObj.element
        .querySelector('.e-toolbar')
        .classList.add('e-responsive-toolbar');
      (gridObj as any).headerModule.updateCustomResponsiveToolbar({
        module: 'toolbar',
      });
    });
    ...
    it('Adaptive coverage - 7', () => {
      gridObj.filterModule.responsiveDialogRenderer.editComplate({
        requestType: 'cancel',
      });
    });
  ...
  describe('EJ2-899558-Responsive Back Element not showing when search is aligned left in Toolbar with AdaptiveUI', () => {
    ...
    it('Ensure responsive back icon', (done: Function) => {
      expect(
        document.querySelector('.e-gridresponsiveicons .e-resback-icon')
      ).not.toBeNull();
      done();
    });
