...
describe('Custom Atrributes and html encode module', () => {
  ...
  describe('html encode check', () => {
    ...
    it('content testing', () => {
      ...
      expect(
        gridObj.getContent().querySelectorAll('.e-rowcell')[2].innerHTML
      ).toBe('&lt;i&gt;TOMSP&lt;/i&gt;');
    });
  ...
  describe('Custom Formatter as Function', () => {
    ...
    it('Check custom Formatter return value', () => {
      rows = (grid.getContentTable() as any).tBodies[0]
        .rows[0] as HTMLTableRowElement;
      expect(rows.cells[1].innerHTML).toBe('5.00');
      let rows1 = (grid.getContentTable() as any).tBodies[0]
    ...
    it('Row Rendeder functionality object to attribute conversion checking', () => {
      let render: RowRenderer<Column> = new RowRenderer<Column>(
        grid.serviceLocator,
        null,
        grid
      );
      let tr: HTMLElement = <HTMLElement>(
        render.render(
          <Row<Column>>{
            isSelected: true,
            visible: false,
