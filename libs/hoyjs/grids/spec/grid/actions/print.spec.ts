  ...
  describe('Print without paging', () => {
    ...
    it('basic feature testing', (done: Function) => {
      let printComplete = (args?: { element: Element }): void => {
        ...
        expect(
          args.element.classList.contains('e-print-grid-layout')
        ).toBeTruthy();
        expect(args.element.querySelectorAll('.e-row').length).toBe(15);
        let contentDiv: HTMLElement = args.element.querySelector(
          '.e-content'
        ) as HTMLElement;
        ...
        expect(
          (args.element.querySelector('.e-groupdroparea') as HTMLElement).style
            .display
        ).toBe('none');
  ...
  describe('Print with paging', () => {
    ...
    it('current page testing and group column', (done: Function) => {
      let printComplete = (args?: { element: Element }): void => {
        ...
        expect(
          args.element.querySelector('.e-grouptopleftcell').clientWidth
        ).toBe(0);
        expect(args.element.querySelector('.e-indentcell').clientWidth).toBe(0);
        expect(
          args.element.querySelector('.e-recordplusexpand').clientWidth
        ).toBe(0);
  ...
  describe('Print with hierarchy grid All mode=>', () => {
    ...
    it('Check hierarchy in before print', (done: Function) => {
      ...
      gridObj.beforePrint = (args) => {
        ...
        expect(args.element.querySelectorAll('[aria-busy=true]').length).toBe(
          0
        );
        expect(
          (args.element as any).ej2_instances[0]['isPrinting']
        ).toBeTruthy();
        expect(args.element.querySelectorAll('.e-grid').length).toBe(c);
        expect(trigger).toBe(0);
        expect(
          (<{ printGridObj?: IGrid }>window).printGridObj.expandedRows
        ).not.toBeUndefined();
      ...
      (<Grid>(<{ printGridObj?: IGrid }>window).printGridObj).actionFailure =
        () => trigger++;
    });
