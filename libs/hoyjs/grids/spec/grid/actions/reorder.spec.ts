...
describe('Reorder module', () => {
  describe('Reorder functionalities', () => {
    ...
    it('Dynamic allowReordering set', () => {
      ...
      gridObj.dataBind();
      expect(
        gridObj.getHeaderContent().classList.contains('e-draggable')
      ).toBeTruthy();
    });
    ...
    it('Reorder Column simulate testing', (done: Function) => {
      ...
      gridObj.dataBound = dataBound;
      gridObj.dataBind();
      let dropClone = createElement('div', {
        attrs: { 'e-mappinguid': gridObj.getUidByColumnField('OrderID') },
      });
      document.body.appendChild(dropClone);
      (
        gridObj.renderModule as any
      ).headerRenderer.draggable.currentStateTarget =
        gridObj.getColumnHeaderByField('OrderID');
      (gridObj.headerModule as any).helper({
        target: gridObj.getHeaderTable().querySelector('tr'),
        sender: {
          clientX: 10,
          clientY: 10,
          target: gridObj.getColumnHeaderByField('OrderID'),
        },
      });
      (gridObj.headerModule as any).dragStart({
        target: gridObj.getColumnHeaderByField('OrderID').children[0],
        event: {
          clientX: 10,
          clientY: 10,
          target: gridObj.getColumnHeaderByField('OrderID').children[0],
      ...
      (gridObj.headerModule as any).drag({
        ...
        event: {
          clientX: 10,
          clientY: 10,
          target: gridObj.getColumnHeaderByField('EmployeeID').children[0],
        },
      });
      (gridObj.headerModule as any).dragStop({
        target: gridObj.getColumnHeaderByField('EmployeeID'),
        element: gridObj.getHeaderTable().querySelector('tr'),
        helper: dropClone,
        event: {
          clientX: 10,
          clientY: 10,
    ...
    it('Reorder disable and enable testing', (done: Function) => {
      let dataBound = (args: Object): void => {
        ...
        expect(headers[4].querySelector('.e-headercelldiv').textContent).toBe(
          'Freight'
        );
        expect(columns[0].field).toBe('OrderID');
  ...
  describe('Reorder functionalities', () => {
    ...
    it('EJ2-6054-script error thrown when render a sample with reorder and filter menu', () => {
      ...
      let tarele: Element =
        gridObj.element.querySelectorAll('.e-filtermenudiv')[1];
      let e: {} = { target: tarele };
      ...
      expect(
        (<any>gridObj.element.querySelectorAll('.e-filtered')).length
      ).toBe(1);
    });
    ...
    it('Reorder Column simulate testing', (done: Function) => {
      ...
      (gridObj.headerModule as any).drag({
        ...
        event: {
          clientX: 10,
          clientY: 10,
          target: gridObj.getColumnHeaderByField('CustomerID').children[0],
        },
      });
      (gridObj.headerModule as any).dragStop({
        target: gridObj.getColumnHeaderByField('CustomerID'),
        element: gridObj.getHeaderContent().querySelector('tr'),
        helper: dropClone,
        event: {
          clientX: 10,
          clientY: 10,
  ...
  describe('Reorder stackedheader after hiding issue', () => {
    ...
    it('EJ2-69396-visible false setting column reorder issue', () => {
      const dropClone: HTMLElement = createElement('div', {
        attrs: { 'e-mappinguid': gridObj.getUidByColumnField('Extension') },
      });
      document.body.appendChild(dropClone);
      (
        gridObj.renderModule as any
      ).headerRenderer.draggable.currentStateTarget =
        gridObj.getColumnHeaderByField('Extension');
      (gridObj.headerModule as any).helper({
        target: gridObj.getHeaderTable().querySelector('tr'),
        sender: {
          clientX: 10,
          clientY: 10,
          target: gridObj.getColumnHeaderByField('Extension'),
        },
      });
      (gridObj.headerModule as any).dragStart({
        target: gridObj.getColumnHeaderByField('Extension').children[0],
        event: {
          clientX: 10,
          clientY: 10,
          target: gridObj.getColumnHeaderByField('Extension').children[0],
      ...
      (gridObj.headerModule as any).drag({
        ...
        event: {
          clientX: 10,
          clientY: 10,
          target: gridObj.getColumnHeaderByField('HomePhone').children[0],
        },
      });
      (gridObj.headerModule as any).dragStop({
    ...
    it('EJ2-69396-check reorder element dropped index', () => {
      const stackedHdrElem: NodeListOf<Element> = document.querySelectorAll(
        '.e-headercell.e-firstcell'
      );
    ...
    it('hide and simulate reorder action', () => {
      ...
      gridObj.dataBind();
      let dropClone = createElement('div', {
        attrs: { 'e-mappinguid': gridObj.getUidByColumnField('LastName') },
      });
      document.body.appendChild(dropClone);
      (
        gridObj.renderModule as any
      ).headerRenderer.draggable.currentStateTarget =
        gridObj.getColumnHeaderByField('LastName');
      (gridObj.headerModule as any).helper({
        target: gridObj.getHeaderTable().querySelector('tr'),
        sender: {
          clientX: 10,
          clientY: 10,
          target: gridObj.getColumnHeaderByField('LastName'),
        },
      });
      (gridObj.headerModule as any).dragStart({
        target: gridObj.getColumnHeaderByField('LastName').children[0],
        event: {
          clientX: 10,
          clientY: 10,
          target: gridObj.getColumnHeaderByField('LastName').children[0],
      ...
      (gridObj.headerModule as any).drag({
        ...
        event: {
          clientX: 10,
          clientY: 10,
          target: gridObj.getColumnHeaderByField('Title').children[0],
        },
      });
      (gridObj.headerModule as any).dragStop({
        target: gridObj.getColumnHeaderByField('Title'),
    ...
    it('hide history column and simulate stacked header reorder with normal header', () => {
      ...
      let srcHeaderCell: Element =
        document.querySelectorAll('.e-headercell')[1];
      let destHeaderCell: Element =
        document.querySelectorAll('.e-headercell')[0];
      let dropClone = createElement('div', {
        attrs: {
          'e-mappinguid': (srcHeaderCell.lastChild as Element).getAttribute(
            'e-mappinguid'
          ),
        },
      });
      ...
      (
        gridObj.renderModule as any
      ).headerRenderer.draggable.currentStateTarget = srcHeaderCell;
      (gridObj.headerModule as any).helper({
        target: gridObj.getHeaderTable().querySelector('tr'),
        sender: { clientX: 10, clientY: 10, target: srcHeaderCell },
      });
      (gridObj.headerModule as any).dragStart({
        target: srcHeaderCell.children[0],
        event: { clientX: 10, clientY: 10, target: srcHeaderCell.children[0] },
      });
      (gridObj.headerModule as any).dragStart({
        target: srcHeaderCell,
        event: { clientX: 10, clientY: 10, target: srcHeaderCell.children[0] },
      });
      (gridObj.headerModule as any).drag({
        target: destHeaderCell,
        event: { clientX: 10, clientY: 10, target: destHeaderCell.children[0] },
      });
      (gridObj.headerModule as any).dragStop({
        target: destHeaderCell,
        element: gridObj.getHeaderTable().querySelector('tr'),
    ...
    it('check stacked header reorder element dropped index', () => {
      const stackedHdrElem: NodeListOf<Element> =
        document.querySelectorAll('.e-headercell');
      expect(stackedHdrElem[0].textContent).toBe('Employee Details');
      expect(stackedHdrElem[0].classList.contains('e-stackedheadercell')).toBe(
        true
      );
    });
  ...
  describe('Column reordering index is wrong when Grid has hidden columns', () => {
    ...
    it('simulate Reorder Column action with set visible false columns', () => {
      const headers: NodeListOf<HTMLElement> = gridObj
        .getHeaderContent()
      ...
      (gridObj.reorderModule as any).element = srcHeaderCell;
      (gridObj.reorderModule as any).chkDropPosition = function () {
        return true;
      };
      (gridObj.reorderModule as any).chkDropAllCols = function () {
        return true;
      };
      (gridObj.headerModule as any).drop({
        target: destHeaderCell,
    ...
    it('check reorder index', () => {
      ...
      expect(headers[3].querySelector('.e-headercelldiv').textContent).toBe(
        'Customer Name'
      );
    });
    afterAll(() => {
  ...
  describe('Reorder functionalities-column level', () => {
    ...
    it('Reorder Column simulate testing', () => {
      (gridObj.reorderModule as any).dragStart({
        target: gridObj.getColumnHeaderByField('OrderID').children[0],
  ...
  describe('Reorder Multiple Columns by using method', () => {
    ...
    it('Reorder Column method testing - 4', () => {
      gridObj.reorderColumns(
        ['Freight', 'CustomerID', 'ShipCity'],
        'EmployeeID'
      );
      columns = gridObj.getColumns() as Column[];
      headers = gridObj.getHeaderContent().querySelectorAll('.e-headercell');
    ...
    it('Reorder Column method testing - 5', () => {
      ...
      expect(headers[5].querySelector('.e-headercelldiv').textContent).toBe(
        'Verified'
      );
      expect(columns[2].field).toBe('CustomerID');
  ...
  describe('Reorder Column by Target', () => {
    ...
    it('Reorder Column by target testing - 5', () => {
      gridObj.reorderColumnByTargetIndex(
  ...
  describe('Reorder file code coverage - 1', () => {
    ...
    it('Reorder file -  headerDrop coverage', () => {
      (gridObj.reorderModule as any).element =
        gridObj.element.querySelector('.e-headercell');
      (gridObj.reorderModule as any).element.firstElementChild.removeAttribute(
        'e-mappinguid'
      );
      (gridObj.reorderModule as any).headerDrop({
        target: gridObj.element.querySelectorAll('.e-headercell')[1],
      });
    });
    ...
    it('Reorder file branches coverage - 1', () => {
      ...
      (gridObj.reorderModule as any).enableAfterRender({
        module: 'reorder',
        enable: true,
      });
      ...
      (gridObj.reorderModule as any).setScrollLeft(
        gridObj.getContent().firstElementChild,
        100
      );
      (gridObj as any).getContent().querySelector('.e-rowcell').click();
      (gridObj as any).keyActionHandler({
        action: 'ctrlLeftArrow',
        target: gridObj.getHeaderTable().querySelector('.e-rowcell'),
        preventDefault: () => {},
      });
      (gridObj as any).getHeaderTable().querySelector('.e-headercell').click();
      (gridObj as any).keyActionHandler({
        action: 'ctrlLeftArrow',
        target: gridObj.getHeaderTable().querySelector('.e-headercell'),
        preventDefault: () => {},
      });
      (gridObj as any).keyActionHandler({
        action: 'ctrlRightArrow',
        target: gridObj.getHeaderTable().querySelector('.e-headercell'),
        preventDefault: () => {},
      });
    });
    ...
    it('Reorder file branches coverage - 2', () => {
      (gridObj.reorderModule as any).chkDropPosition(
        gridObj.element.querySelector('.e-headercell'),
        gridObj.element.querySelectorAll('.e-headercell')[1]
      );
    });
    ...
    it('Reorder file branches coverage - 3', () => {
      gridObj.element
        ...
        .removeChild((gridObj.reorderModule as any).upArrow);
      gridObj.element
        .querySelector('.e-headercontent')
        .removeChild((gridObj.reorderModule as any).downArrow);
