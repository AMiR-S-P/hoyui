...
describe('Paging module', () => {
  describe('Paging functionalities', () => {
    ...
    it('page count testing', () => {
      expect(
        gridObj.getPager().getElementsByClassName('e-numericcontainer')[0]
          .childNodes.length
      ).toBe(4);
      expect(
        (<Element>(
    ...
    it('pageDown shortcut testing', (done: Function) => {
      actionComplete = (args: Object): void => {
        expect(
          gridObj.getPager().querySelectorAll('.e-active')[0].textContent
        ).toBe('2');
        done();
      };
      gridObj.actionComplete = actionComplete;
  ...
  describe('dropDownEvent test', () => {
    ...
    it('pagesizes value changed to 30 and check currentpage', (done: Function) => {
      ...
      (<any>gridObj.pagerModule).pagerObj.element.querySelector(
        '.e-dropdownlist'
      ).ej2_instances[0].value = 30;
    });
  ...
  describe('Paging & Scrolling - PageDown case', () => {
    ...
    let raiseEvt: Function = (code: number) => {
      ...
      (<any>grid.keyboardModule).keyAction({
        action: p[code + ''],
        preventDefault: () => 0,
      });
    };
  ...
  describe('EJ2-9910 dropDown All option', () => {
    ...
    it('dropDownChanged to All', () => {
      (<any>gridInstance.pagerModule).pagerObj.element.querySelector(
        '.e-dropdownlist'
      ).ej2_instances[0].value = 'All';
    });
    ...
    it('EJ2-57784 - check pagesize value when set to `All` ', () => {
      expect(gridInstance.pageSettings.pageSize).toBe(
  ...
  describe('EJ2-43015 Not able to cancel paging action in the Grid', () => {
    ...
    it('cancel the paging', (done: Function) => {
      ...
      (
        gridInstance.getPager().getElementsByClassName('e-numericcontainer')[0]
          .childNodes[1].childNodes[0] as HTMLElement
      ).click();
    });
    it('check the active paging element', () => {
      expect(
        gridInstance.getPager().querySelectorAll('.e-active')[0].textContent
      ).toBe('1');
    });
  ...
  describe('Pager Focus Test Case', () => {
    ...
    it('Set pager focus', () => {
      ...
      const focusablePagerElements: Element[] = (
        gridObj.pagerModule.pagerObj as any
      ).getFocusablePagerElements(gridObj.pagerModule.pagerObj.element, []);
      expect(
        focusablePagerElements[0].classList.contains('e-focused')
      ).toBeTruthy();
      expect(
        (gridObj.pagerModule.pagerObj as any).getFocusedElement()
      ).toBeTruthy();
    });
    it('Set focus class', function () {
      ...
      (gridObj.pagerModule.pagerObj as any).addFocus(
        focusablePagerElements[0],
        true
      );
      expect(
        focusablePagerElements[0].classList.contains('e-focused')
    ...
    it('Remove focus class', function () {
      ...
      (gridObj.pagerModule.pagerObj as any).removeFocus(
        focusablePagerElements[0],
        true
      );
      expect(
    ...
    it('Get element class', function () {
      ...
      expect(
        (gridObj.pagerModule.pagerObj as any).getClass(
          gridObj.pagerModule.pagerObj.element.querySelector('.e-first')
        )
      ).toBe('e-first');
    });
    it('Get element by class name', function () {
      expect(
        (gridObj.pagerModule.pagerObj as any).getElementByClass('e-first')
      ).toBeTruthy();
    });
    it('Alt J testing', () => {
      let args: any = {
    ...
    it('Checking onKeyPress for changing focus with tab with no parent', () => {
      ...
      expect(
        focusablePagerElements[1].classList.contains('e-focused')
      ).toBeFalsy();
    });
    ...
    it('check checkFocusInAdaptiveMode, changeFocusInAdaptiveMode Function', function () {
      const adaptiveElement = (
        gridObj.pagerModule.pagerObj as any
      ).element.querySelector('.e-mfirst');
      expect(
        (gridObj.pagerModule.pagerObj as any).checkFocusInAdaptiveMode(
          adaptiveElement
        )
      ).toBeTruthy();
      (gridObj.pagerModule.pagerObj as any).changeFocusInAdaptiveMode(
        adaptiveElement
      );
      expect(
        (gridObj.pagerModule.pagerObj as any).element.querySelector(
          '.e-focused'
        )
      ).toBeFalsy();
    });
...
describe('BUG-830382 - Page count is not increased while adding new records if the Grid has pager dropdown', () => {
  ...
  describe('Feature - EJ2-850006 - Implementation of pager dropdownlist in adaptive / mobile pager', () => {
    ...
    it('Setting adaptive mode CSS to trigger mimic media query css for code coverage purpose', function () {
      (
        gridObj.element.querySelector('.e-pager') as HTMLElement
      ).style.borderStyle = 'solid'; //code to trigger pager resizing.
      (
        gridObj.element.querySelector('.e-mfirst') as HTMLElement
      ).style.display = 'block';
      (
        gridObj.element.querySelector('.e-mfirst') as HTMLElement
      ).style.setProperty('--mq-type', 'small');
      gridObj.pagerModule.refresh();
    });
  ...
  describe('code coverage - Page', () => {
    ...
    it('Edit record', () => {
      ...
      (
        gridObj.pagerModule.pagerObj.element.querySelector(
          '.e-dropdownlist'
        ) as any
  ...
  describe('code coverage - Page - React', () => {
    ...
    it('Render pager template in React ', () => {
      ...
      (gridObj.pagerModule as any).enableAfterRender({
        module: 'pager',
        enable: true,
      });
    });
