...
import { DataManager, ODataV4Adaptor, DataUtil } from '@hoyui/data';
...
describe('Filtering module => ', () => {
    ...
    it('Filter date column testing', (done: Function) => {
      ...
      filterColumn(
        ...
        valueFormatter
          .toView(
            new Date(8364186e5),
            (gridObj.getColumnByField('OrderDate') as Column).getFormatter()
          )
          .toString()
      );
    });
  ...
  describe('Filterbar showFilterBarStatus false testing => ', () => {
    ...
    it('showFilterBarStatus testing', (done: Function) => {
      actionComplete = (args?: Object): void => {
        expect(
          (gridObj.element.querySelector('.e-pagerexternalmsg') as HTMLElement)
            .style.display
        ).toBe('none');
        done();
      };
    ...
    it('Ensure filterbar input element value', (done: Function) => {
      gridObj.clearFiltering();
      actionComplete = (args?: Object): void => {
        let elementVal: string = (
          document.getElementById('OrderID_filterBarcell') as any
        ).value;
  ...
  describe('Filterbar template => ', () => {
    ...
    it('element classname testing', () => {
      expect(
        gridObj.getHeaderTable().querySelectorAll('.e-fltrtemp').length
      ).toBe(1);
      expect(
        (
          gridObj
            .getHeaderTable()
            .querySelector('#EmployeeID_filterBarcell') as HTMLInputElement
        ).querySelectorAll('.customfiltertemplate').length
      ).toBe(1);
      expect(
        (
          gridObj
            .getHeaderTable()
            .querySelector('#OrderID_filterBarcell') as HTMLInputElement
        ).disabled
      ).toBeFalsy();
    });
          ...
          columns: [
            { field: 'OrderID', type: 'number', visible: true },
            {
              ...
              filterBarTemplate: {
                write: (args: any) => {
                  (
                    document.getElementById('EmployeeID_filterBarcell') as any
                  ).value = '5';
                },
                read: (args: any) => {
                  gridObj.filterByColumn(
  ...
  describe('Filter a column and clear filtering => ', () => {
    ...
    it('showFilterBarStatus testing initial filter', () => {
      expect(
        (<any>(
          gridObj
            .getHeaderContent()
            .querySelectorAll('#EmployeeID_filterBarcell')[0]
        )).value
      ).toBe('5');
      expect(gridObj.currentViewData.length).toBe(4);
      expect(
        gridObj.getPager().querySelectorAll('.e-pagerexternalmsg')[0].innerHTML
      ).toBe('EmployeeID: 5');
    });
      ...
      actionComplete = (args?: Object): void => {
        expect(
          (<any>(
            gridObj
              .getHeaderContent()
              .querySelectorAll('#OrderID_filterBarcell')[0]
          )).value
        ).toBe('10248');
        expect(
          (<any>(
        ...
        expect(
          gridObj.getPager().querySelectorAll('.e-pagerexternalmsg')[0]
            .innerHTML
        ).toBe('EmployeeID: 5 &amp;&amp; OrderID: 10248');
        done();
      };
      gridObj.actionComplete = actionComplete;
      gridObj.filterByColumn('OrderID', 'equal', 10248, 'and', false);
    ...
    it('group a column', (done: Function) => {
      actionComplete = () => {
        expect(
          gridObj.element.querySelectorAll('.e-groupdroparea')[0].children
            .length
        ).toBe(1);
        expect(
  ...
  describe('Filterbar with Freeze Row and column => ', () => {
    ...
    it('on property change', (done: Function) => {
      ...
      gridObj.allowFiltering = false;
      gridObj.dataBind();
      expect(gridObj.getHeaderContent().querySelector('.e-filterbar')).toBe(
        null
      );
      gridObj.allowFiltering = true;
      gridObj.dataBind();
      expect(gridObj.getHeaderContent().querySelector('.e-filterbar')).not.toBe(
        null
      );
    });
  ...
  describe('Filterbar with Freeze Row => ', () => {
    ...
    it('Filter on frozen rows', (done: Function) => {
      dBound = (args?: Object): void => {
        expect(
          gridObj.getHeaderContent().querySelector('tbody').children[0]
    ...
    it('Render empty', (done: Function) => {
      dBound = (args?: Object): void => {
        expect(
          gridObj.getContent().querySelector('tbody').children[0].classList
        ).toContain('e-emptyrow');
        done();
      };
      gridObj.dataBound = dBound;
  ...
  describe('EJ2-6573, EJ2-6562 Filterbar with paging no records display => ', () => {
    ...
    it('current page testing', (done: Function) => {
      actionComplete = (args: any): any => {
        expect(
          gridObj
            .getPager()
            .getElementsByClassName('e-active')[0]
            .getAttribute('index')
        ).toBe('6');
        done();
      };
    ...
    it('filter bar input value testing', (done: Function) => {
      actionComplete = (args?: Object): void => {
        expect(
          (gridObj.element.querySelector('#EmployeeID_filterBarcell') as any)
            .value
        ).toBe('=234');
        gridObj.actionComplete = null;
        done();
  ...
  describe('Excel Filter dialog not updated while programmatically filter the column=> ', () => {
    ...
    it('EJ2-10000(case 1) - checking excel filter icon and whether value gets updated in Excel filter popup', (done: Function) => {
      expect(
        gridObj.element
          .querySelector('.e-columnheader')
          .firstElementChild.querySelector('.e-filtered')
      ).toBeTruthy();
      expect(
        (gridObj.filterModule as any).actualPredicate['CustomerID'].length
      ).toBe(1);
      actionComplete = (args?: any): void => {
        gridObj.actionComplete = null;
        done();
    ...
    it('EJ2-10000(case 1) - Checking whether values in excel filter popup is cleared', () => {
      expect((gridObj.filterModule as any).actualPredicate['CustomerID']).toBe(
        undefined
      );
    });
    ...
    it('EJ2-10000(case 3) - checking filterbar cell value after clearing in Menu type', (done: Function) => {
      ...
      expect(
        (gridObj.element.querySelector('#CustomerID_filterBarcell') as any)
          .value
      ).toBe('');
      let headerRefreshed = (): void => {
        gridObj.off(events.headerRefreshed, headerRefreshed);
    ...
    it('EJ2-10000(case 3) - clearing in Excel type', (done: Function) => {
      ...
      gridObj.actionComplete = actionComplete;
      (gridObj.filterModule as any).filterHandler({
        action: 'clear-filter',
        field: 'CustomerID',
      });
    });
  ...
  describe('Initial filtering => ', () => {
    ...
    it('Filter a column', () => {
      //EJ2-25278 is logged as an issue and once fixed need to uncomment the below expect
      //expect((gridObj as any).getHeaderContent().querySelectorAll('.e-filtertext')[5].value).toBe('7/12/1996');
      expect(
        (gridObj as any).getHeaderContent().querySelectorAll('.e-filtertext')[1]
          .value
      ).toBe('5');
    });
    afterAll(() => {
  ...
  describe('EJ2CORE-133- Filtering when foreignKeyValue and grid field are same', () => {
    ...
    it('Menu filter check with foreignkey', (done: Function) => {
      actionComplete = (args: any) => {
        if (args.requestType == 'filterAfterOpen') {
          (
            args.filterModel.dlgDiv.querySelector('.e-flmenu-input') as any
          ).ej2_instances[0].value = 'F';
          (
    ...
    it('Filter menu icon for foreignkey column check', (done: Function) => {
      expect(
        gridObj.element.querySelectorAll('.e-filtermenudiv')[1].classList
      ).toContain('e-filtered');
      expect(
        gridObj.element
          .querySelectorAll('.e-filtermenudiv')[3]
  ...
  describe('EJ2-26559 Api support enable case sensitivity menu filter check', () => {
    ...
    it('Filter ShipCountry testing for matchcase default value true', (done: Function) => {
      ...
      gridObj.actionComplete = actionComplete;
      (
        gridObj.element.querySelectorAll('.e-filtermenudiv')[3] as HTMLElement
      ).click();
    });
    afterAll(() => {
      destroy(gridObj);
  ...
  describe('Filter Menu Opertor test case', () => {
    ...
    it('field and foreignkeyvalue as same- multiple filtering', (done: Function) => {
      actionComplete = (e: any) => {
        expect(e.filterModel.flMuiObj.dropOptr.value).toBe(
          (gridObj.columns[1] as any).filter.operator
        );
        done();
      };
      gridObj.actionComplete = actionComplete;
  ...
  describe('Filter bar test case => ', () => {
    ...
    it('action complete', (done: Function) => {
      ...
      actionComplete = (args: any) => {
        expect(args.currentFilterObject.operator).toBe(
          (gridObj.columns[1] as any).filter.operator
        );
        done();
      };
  ...
  describe('Filtering with ColumnVirtualization', () => {
    let ctr: number = 0;
    let count: string[] = Array.apply(null, Array(60)).map(
      () => 'Column' + ++ctr + ''
    );
    let data1: Object[] = (() => {
      let arr: Object[] = [];
  ...
  describe('Check for case sensitive ', () => {
    ...
    it('filterbyColumn Method checking with case sensitive', (done: Function) => {
      actionComplete = () => {
        ...
        expect(
          (gridObj.element.querySelectorAll('.e-row')[0] as any).cells[3]
            .innerHTML
        ).toBe('Switzerland');
        done();
      };
      gridObj.filterModule.filterByColumn(
        'ShipCountry',
        'contains',
        'S',
        'and',
  ...
  describe('EJ2-38480 - Filterbar focusing is not workig properly, while using checkboxmode as resetonrowclick', () => {
    ...
    it('Ensure filterbar input element focus', (done: Function) => {
      actionComplete = function (args) {
        ...
        let input = (gridObj.filterModule as any).getFilterBarElement(
          fieldName
        );
...
describe('Filter Bar Operator feature render checking and behaviour checking', () => {
  ...
  it('check for filterbar operator render', () => {
    ...
    inptLen = gridObj
      ...
      .querySelectorAll('.e-filterbaroperator').length;
  ...
  it('checking altdown key', () => {
    let tar: any = gridObj
      .getHeaderContent()
      .querySelector('.e-filterbar')
      .querySelector('.e-filtertext') as any;
...
describe('checking for operator', () => {
  ...
  it('initial operator checking', () => {
    let ordeIdDrpdown: Element = gridObj.element.querySelectorAll(
      '.e-filterbaroperator'
    )[0];
  ...
  it('checking for operator value after grouping', () => {
    let freightDrpdown: Element = gridObj.element.querySelectorAll(
      '.e-filterbaroperator'
    )[2];
    ...
    expect((gridObj.filterModule as any).getOperatorName('Freight')).toBe(
      'lessthanorequal'
    );
  });
...
describe('EJ2-52526 - Boolean custom menu filter close dialog script error', () => {
  ...
  it('close the dialog', (done: Function) => {
    (
      gridObj.element.querySelectorAll('.e-filtermenudiv')[0] as HTMLElement
    ).click();
    done();
  });
  ...
  let data1: object[] = [
    {
      OrderID: 10248,
      CustomerID: 'VINET',
      OrderDate: '2021-10-06T13:04:10.0456283+00:00',
    },
    {
      OrderID: 10249,
      CustomerID: 'TOMSP',
      OrderDate: '2021-10-12T09:22:05.8896886+00:00',
    },
    {
      OrderID: 10251,
      CustomerID: 'VICTE',
      OrderDate: '2021-09-24T12:52:15.2898138+00:00',
    },
  ];
...
describe('EJ2-58920 - Incorrect filterOperator for initial filter', () => {
  ...
  it('initial operator for initial filter checking', () => {
    let shipCountryDrpdown: Element = gridObj.element.querySelectorAll(
      '.e-filterbaroperator'
    )[4];
...
describe('EJ2-57916 Initial filter value is not displayed => ', () => {
  ...
  it('Filter a column', () => {
    expect(
      (gridObj as any).getHeaderContent().querySelectorAll('.e-filtertext')[3]
        .value
    ).toBe('Brazil1');
  });
  afterAll(() => {
  ...
  it('checking customComponent', () => {
    (
      (
        gridObj
          .getHeaderTable()
          .querySelector('#OrderDate_filterBarcell') as HTMLElement
      ).querySelector('.e-input-group-icon') as HTMLElement
    ).click();
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('EJ2-60560 - Scroller thumb is shown when empty record displayed in the Virtualization Grid => ', () => {
  ...
  it('filter invalid data', (done: Function) => {
    actionComplete = (args: any): void => {
      expect(
        gridObj.element.querySelectorAll('.e-virtualtrack')[0].clientHeight
      ).toBe(0);
      done();
    };
    gridObj.actionComplete = actionComplete;
...
describe('EJ2-62502 - clear filter testing with ColumnMenu ', () => {
  ...
  it('Clear filter - checking', (done: Function) => {
    var ShipCountryColumn = gridObj
      .getHeaderContent()
      .querySelectorAll('th')[4];
    dBound = (args?: Object): void => {
...
describe('EJ2-69040 - Filter Menu dialog is not opening on ForeignKey column when it has filterTemplate', () => {
  ...
  it('Check foreign key filter template is open or not?', () => {
    expect(
      document.querySelector('.e-grid-popup').querySelector('.e-popup-open')
    ).toBeTruthy();
  });
...
describe('EJ2-68692 - Grid Component menu filtering behaves incorrectly ', () => {
  ...
  it('Query checking', () => {
    const autoComplete = (
      document.getElementsByClassName('e-autocomplete')[0] as any
    ).ej2_instances[0];
...
describe('EJ2-849870 - Checkbox filtering is not working properly while canceling the filtering ', () => {
  ...
  it('Filtering a records ', function () {
    (
      document
        .querySelector('.e-grid-popup')
        .getElementsByClassName('e-frame e-icons e-check')[1] as HTMLElement
    ).click();
    (
      document
        .querySelector('.e-grid-popup')
  ...
  it('lick the checkbox filter after filtering', function (done) {
    var actionComplete = function (args: any) {
      if (args.requestType == 'filterAfterOpen') {
        expect(
          document.getElementsByClassName('e-frame e-icons e-check').length
        ).toBe(6);
        done();
      }
    };
...
describe('EJ2-880709 - In ODataV4Adaptor, the filter menu shows "No matches found" after searching a record in Grid search bar.', () => {
  ...
  it('Close the filter dialog', function () {
    (
      gridObj.element.getElementsByClassName('e-rowcell')[1] as HTMLElement
    ).click();
  });
  it('Execute the query generate method', function () {
    (<any>gridObj).filterModule.filterModule.checkBoxBase.options.dataManager =
      remoteData;
    (<any>gridObj).filterModule.filterModule.checkBoxBase.queryGenerate(
      gridObj.query
    );
  });
  afterAll(() => {
