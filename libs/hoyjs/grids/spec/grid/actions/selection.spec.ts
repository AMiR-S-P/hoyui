 ...
 */
import { Browser, EmitType, select } from '@hoyui/base';
import { EventHandler, isNullOrUndefined, closest } from '@hoyui/base';
...
function getEventObject(
  ...
  if (!isNullOrUndefined(target)) {
    returnObject.target =
      returnObject.srcElement =
      returnObject.toElement =
      returnObject.currentTarget =
        target;
  }
...
let ctr: number = 0;
let count500: string[] = Array.apply(null, Array(5)).map(
  () => 'Column' + ++ctr + ''
);
let virtualData: Object[] = (() => {
  let arr: Object[] = [];
  for (
    let i: number = 0, o: Object = {}, j: number = 0;
    i < 1000;
    i++, j++, o = {}
  ) {
...
describe('Selection Shortcuts testing', () => {
        ...
        for (let j: number = 0; j < len; j++) {
          expect(
            rows[i]
              ...
              [j].classList.contains('e-cellselectionbackground')
          ).toBeTruthy();
        }
      }
      expect(
        rows[0].firstElementChild.classList.contains('e-selectionbackground')
      ).toBeTruthy();
  ...
  it('downarrow shortcut testing', () => {
    ...
    expect(
      gridObj.element
        .querySelectorAll('tr[aria-selected="true"]')[0]
        .getAttribute('data-rowindex')
    ).toBe('2');
    expect(
      rows[2].firstElementChild.classList.contains('e-selectionbackground')
    ).toBeTruthy();
    expect(gridObj.getSelectedRecords().length).toBe(1);
    expect(gridObj.getSelectedRowIndexes().length).toBe(1);
    expect(
      rows[2].firstElementChild.classList.contains('e-cellselectionbackground')
    ).toBeTruthy();
  ...
  it('upArrow shortcut testing', () => {
    ...
    expect(
      rows[1].firstElementChild.classList.contains('e-selectionbackground')
    ).toBeTruthy();
    expect(selectionModule.selectedRecords.length).toBe(1);
    expect(selectionModule.selectedRowIndexes.length).toBe(1);
    expect(
      rows[1].firstElementChild.classList.contains('e-cellselectionbackground')
    ).toBeTruthy();
  ...
  it('ctrlEnd shortcut testing', () => {
    ...
    expect(
      rows[7].firstElementChild.classList.contains('e-selectionbackground')
    ).toBeTruthy();
    expect(
      (
        rows[7].querySelector(
          '.e-cellselectionbackground'
        ) as HTMLTableCellElement
      ).cellIndex
    ).toBe(4);
  ...
  it('shiftUp row shortcut testing', (done: Function) => {
    ...
    gridObj.rowSelected = () => {
      ...
      expect(
        gridObj.element
          .querySelectorAll('tr[aria-selected="true"]')[1]
          .getAttribute('data-rowindex')
      ).toBe('3');
      expect(
        rows[2].firstElementChild.classList.contains('e-selectionbackground')
      ...
      expect(
        rows[3].firstElementChild.classList.contains('e-selectionbackground')
      ).toBeTruthy();
      expect(
        rows[7]
          .querySelectorAll('.e-rowcell')[4]
          .classList.contains('e-cellselectionbackground')
      ).toBeTruthy();
      done();
    };
    gridObj.selectRow(3);
  ...
  it('shiftUp row shortcut reverse testing', () => {
    ...
    expect(
      rows[4].firstElementChild.classList.contains('e-selectionbackground')
    ).toBeFalsy();
    expect(
      rows[7]
        .querySelectorAll('.e-rowcell')[4]
  ...
  it('shiftLeft cell shortcut testing', () => {
    ...
    expect(
      rows[1]
        .querySelectorAll('.e-rowcell')[1]
        .classList.contains('e-cellselectionbackground')
    ).toBeTruthy();
    expect(
      rows[1]
        .querySelectorAll('.e-rowcell')[0]
        .classList.contains('e-cellselectionbackground')
    ).toBeTruthy();
  });
  ...
  it('ctrl + A shortcut testing', () => {
    ...
    gridObj.keyboardModule.keyAction(args);
    expect(
      gridObj.element.querySelectorAll('.e-cellselectionbackground').length
    ).toBe(gridObj.element.querySelectorAll('.e-rowcell').length);
    expect(
      gridObj.element.querySelectorAll('.e-selectionbackground').length
    ).toBe(gridObj.element.querySelectorAll('.e-rowcell').length);
  });

  it('shiftDown - header', () => {
    (
      gridObj.getHeaderContent().querySelector('.e-headercell') as HTMLElement
    ).click();
    let args: any = { action: 'shiftDown', preventDefault: preventDefault };
    gridObj.keyboardModule.keyAction(args);
    expect((<any>gridObj).focusModule.active.constructor.name).toBe(
      'HeaderFocus'
    );
  });
...
describe('Grid Selection module', () => {
  describe('grid single seletion functionalities', () => {
    ...
    it('single cell testing', () => {
      ...
      expect(selectionModule.selectedRowCellIndexes.length).toBe(1);
      expect(
        rows[0].querySelectorAll('.e-cellselectionbackground').length
      ).toBe(0);
    });
...
describe('Mode and Type changes', () => {
  ...
  it('multi row - selectRow test with mode change ', () => {
    ...
    expect(
      rows[8].firstElementChild.classList.contains('e-selectionbackground')
    ).toBeTruthy();
  });
  ...
  it('cell selection with mode change', () => {
    ...
    expect(gridObj.selectionModule.selectedRowCellIndexes.length).toBe(1);
    expect(
      rows[4].firstElementChild.classList.contains('e-cellselectionbackground')
    ).toBeTruthy;
...
describe('Grid Selection module', () => {
  describe('grid row multiple seletion functionalities', () => {
    ...
    it('multi row - shift click testing', () => {
      ...
      expect(
        rows[5].firstElementChild.classList.contains(
          'e-cellselectionbackground'
        )
      ).toBeFalsy();
      expect(selectionModule.selectedRecords.length).toBe(2);
  ...
  describe('grid cell multiple seletion functionalities', () => {
    ...
    it('multi cell - selectCellsByRange testing', () => {
      ...
      for (let i: number = 0; i <= 1; i++) {
        ...
        for (let j: number = 0; j < len; j++) {
          expect(
            cells[j].classList.contains('e-cellselectionbackground')
          ).toBeTruthy();
        }
      }
    ...
    it('multi cell - selectCells testing', () => {
      ...
      selectionModule.selectCells([
        { rowIndex: 0, cellIndexes: [0] },
        { rowIndex: 1, cellIndexes: [1] },
        { rowIndex: 2, cellIndexes: [2] },
      ]);
      for (let i: number = 0; i <= 2; i++) {
        cells = rows[i].querySelectorAll('.e-rowcell');
        expect(
          cells[i].classList.contains('e-cellselectionbackground')
        ).toBeTruthy();
      }
      expect(selectionModule.selectedRowCellIndexes.length).toBe(3);
    });
    ...
    it('Muliple Cell selection while refreshing grid testing', function (done) {
      gridObj.dataBound = function () {
        ...
        gridObj.refresh();
        gridObj.dataBound = function () {
          gridObj.selectCellsByRange(
            { rowIndex: 0, cellIndex: 0 },
            { rowIndex: 0, cellIndex: 2 }
          );
          expect(
            gridObj
              .getCellFromIndex(0, 2)
              .classList.contains('e-cellselectionbackground')
          ).toBeTruthy;
          expect(
            gridObj.getSelectedRowCellIndexes()[0].cellIndexes.length
          ).toBe(3);
          selectionModule.clearCellSelection();
          done();
        };
...
describe('Grid Selection module', () => {
  describe('clear selection cases', () => {
    ...
    it('select cell and clear row selection testing', () => {
      ...
      expect(
        rows[1].firstElementChild.classList.contains(
          'e-cellselectionbackground'
        )
      ).toBeTruthy();
      cell.click();
    ...
    it('Row select false testing', () => {
      (
        gridObj.element.querySelectorAll('.e-row')[0].firstChild as HTMLElement
      ).click();
      expect(
        gridObj.element
    ...
    it('keydown selection false testing', () => {
      ...
      //for coverage
      (<any>gridObj.selectionModule).mouseDownHandler({
      ...
      gridObj.selectionModule.selectCell({ rowIndex: 1, cellIndex: 1 }, true);
      (<any>gridObj.selectionModule).prevECIdxs = undefined;
      gridObj.selectionModule.selectCellsByRange(
        { rowIndex: 0, cellIndex: 0 },
        { rowIndex: 1, cellIndex: 1 }
      );
      (<any>gridObj.selectionModule).prevECIdxs = undefined;
      ...
      (<any>gridObj.selectionModule).prevECIdxs = undefined;
      gridObj.selectionModule.addCellsToSelection([
        { rowIndex: 0, cellIndex: 0 },
      ]);
      gridObj.selectionSettings.mode = 'Row';
      gridObj.dataBind();
      ...
      (<any>gridObj.selectionModule).mouseMoveHandler({
        target: gridObj.element,
        preventDefault: () => {},
        clientX: 10,
        clientY: 10,
  ...
  describe('Model changes', () => {
    ...
    it('select a row with wrong row index', () => {
      ...
      gridObj.dataBind();
      expect(
        gridObj.getContent().querySelectorAll('.e-selectionbackground').length
      ).toBe(0);
    });
    it('selction type change row testing', () => {
      gridObj.selectionSettings.type = 'Multiple';
    ...
    it('select a cell with wrong object ', () => {
      ...
      gridObj.dataBind();
      expect(
        gridObj.getContent().querySelectorAll('.e-cellselectionbackground')
          .length
      ).toBe(0);
    });
    it('select cells with wrong object ', () => {
      gridObj.selectionModule.selectCells([
        { rowIndex: 0, cellIndexes: [12, 15] },
        { rowIndex: 5, cellIndexes: [1, 2] },
      ]);
      gridObj.dataBind();
      expect(
        gridObj.getContent().querySelectorAll('.e-cellselectionbackground')
    ...
    it('select cells with selectCellsByRange method ', () => {
      gridObj.selectionModule.selectCellsByRange(
        { rowIndex: 0, cellIndex: 12 },
        { rowIndex: 6, cellIndex: 12 }
      );
      gridObj.dataBind();
      expect(
        gridObj.getContent().querySelectorAll('.e-cellselectionbackground')
...
describe('Grid Touch Selection', () => {
  ...
  describe('select Row/cell after grouping', () => {
    ...
    it('initial check', () => {
      expect(gridObj.element.querySelectorAll('.e-groupdroparea').length).toBe(
        1
      );
      expect(
        gridObj.element.querySelectorAll('.e-groupdroparea')[0].children.length
      ).toBe(2);
      expect(
        gridObj.element
          .querySelectorAll('.e-groupdroparea')[0]
          .querySelectorAll('.e-ungroupbutton').length
      ).toBe(2);
      expect(
        gridObj.getHeaderContent().querySelectorAll('.e-ascending').length
      ).toBe(2);
      expect(
        gridObj.getHeaderContent().querySelectorAll('.e-grouptopleftcell')
          .length
      ).toBe(2);
      expect(
        gridObj.getContentTable().querySelectorAll('.e-indentcell').length > 0
      ).toBeTruthy();
      ...
      let rowSelected = (args: Object) => {
        expect(
          (<HTMLTableCellElement>(
            gridObj.getContent().querySelectorAll('.e-selectionbackground')[0]
          )).innerHTML
        ).toEqual(gridObj.currentViewData['records'][0]['OrderID'].toString());
        expect(JSON.stringify(args['data'])).toEqual(
          JSON.stringify(gridObj.getSelectedRecords()[0])
        );
        ...
        expect(
          gridObj.getRows()[0].children[2].hasAttribute('aria-selected')
        ).toBeTruthy();
        done();
      };
    ...
    it('press up arrow', () => {
      ...
      expect(
        gridObj
          .getRows()[0]
          .children[3].classList.contains('e-selectionbackground')
      ).toBeFalsy();
      expect(
        gridObj.getRows()[0].children[3].hasAttribute('aria-selected')
      ).toBeFalsy();
    });
    ...
    it('press ctrl+home arrow', () => {
      ...
      expect(
        gridObj.getRows()[1].children[3].hasAttribute('aria-selected')
      ).toBeFalsy();
    });
    ...
    it('press ctrl+end arrow', () => {
      ...
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 1].children[3].classList.contains(
      ...
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 1].children[
            gridObj.columns.length - 1
          ].hasAttribute('aria-selected')
      ).toBeTruthy();
      expect(
        gridObj.getRows()[0].children[2].hasAttribute('aria-selected')
    ...
    it('Shift plus click event', () => {
      gridObj
        .getRows()
        [gridObj.getRows().length - 4].children[3].dispatchEvent(shiftEvt);
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 1].querySelectorAll(
            '.e-selectionbackground'
          ).length
      ).toBe(6);
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 2].querySelectorAll(
            '.e-selectionbackground'
          ).length
      ).toBe(6);
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 3].querySelectorAll(
            '.e-selectionbackground'
          ).length
      ).toBe(6);
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 4].querySelectorAll(
            '.e-selectionbackground'
          ).length
      ).toBe(6);
      expect(
    ...
    it('ctrl plus click', () => {
      ...
      expect(
        gridObj.getRows()[1].querySelectorAll('.e-selectionbackground').length
      ).toBe(6);
      expect(
        gridObj.element.querySelectorAll('.e-selectionbackground').length
      ).toBe(30);
    ...
    it('select a cell', (done: Function) => {
      cellSelecting = (args: Object) => {
        expect(JSON.stringify(args['data'])).not.toBeUndefined();
        expect(JSON.stringify(args['cellIndex'])).toEqual(
          JSON.stringify({ rowIndex: 0, cellIndex: 0 })
        );
      ...
      cellSelected = (args: Object) => {
        ...
        expect(args['previousRowCell']).toBeUndefined();
        expect(JSON.stringify(args['selectedRowCellIndex'])).toEqual(
          JSON.stringify([{ rowIndex: 0, cellIndexes: [0] }])
        );
        expect(
          gridObj
            .getRows()[0]
            .children[2].classList.contains('e-cellselectionbackground')
        ).toBeTruthy();
    ...
    it('press right arrow', (done: Function) => {
      cellSelecting = (args: Object) => {
        expect(JSON.stringify(args['data'])).not.toBeUndefined();
        expect(JSON.stringify(args['cellIndex'])).toEqual(
          JSON.stringify({ rowIndex: 0, cellIndex: 1 })
        );
        expect(args['currentCell']).toEqual(gridObj.getRows()[0].children[3]);
        expect(JSON.stringify(args['previousRowCellIndex'])).toEqual(
          JSON.stringify({ rowIndex: 0, cellIndex: 0 })
        );
        expect(args['previousRowCell']).toEqual(
          gridObj.getRows()[0].children[2]
        );
        expect(args['isCtrlPressed']).toBeFalsy();
        expect(args['isShiftPressed']).toBeFalsy();
      };
      cellSelected = (args: Object) => {
        ...
        expect(JSON.stringify(args['selectedRowCellIndex'])).toEqual(
          JSON.stringify([{ rowIndex: 0, cellIndexes: [1] }])
        );
        expect(
          gridObj
            .getRows()[0]
            .children[3].classList.contains('e-cellselectionbackground')
        ).toBeTruthy();
        done();
      };
      gridObj.cellSelected = cellSelected;
    ...
    it('press ctrl+end arrow', () => {
      ...
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 1].lastElementChild.classList.contains(
            'e-cellselectionbackground'
          )
      ).toBeTruthy();
    });
    ...
    it('press shiftUp arrow', () => {
      ...
      expect(
        gridObj.getContent().querySelectorAll('.e-cellselectionbackground')
          .length
      ).toBe(gridObj.getColumns().length + 1);
    });
    ...
    it('press shift+click arrow', () => {
      gridObj
        .getRows()
        [gridObj.getRows().length - 2].children[3].dispatchEvent(shiftEvt);
      expect(
        gridObj.getContent().querySelectorAll('.e-cellselectionbackground')
          .length
      ).toBe(11);
    ...
    it('press ctrl+click arrow', () => {
      gridObj
        .getRows()
        [gridObj.getRows().length - 4].children[3].dispatchEvent(ctrlEvt);
      expect(
        gridObj.getContent().querySelectorAll('.e-cellselectionbackground')
          .length
      ).toBe(12);
    ...
    it('press ctrlPlusA ', () => {
      ...
      expect(
        gridObj.getContent().querySelectorAll('.e-cellselectionbackground')
          .length
      ).toBe(gridObj.getColumns().length * gridObj.getRows().length);
    });
    ...
    it('clear cell Selection', () => {
      ...
      (gridObj.selectionModule as any).addRemoveClassesForRow(
        null,
        false,
        false
      );
      ...
      (gridObj.selectionModule as any).beforeFragAppend({
        requrestType: 'virtualscroll',
      });
      (gridObj.selectionModule as any).clearSelAfterRefresh({
        requrestType: 'virtualscroll',
      });
    });
    ...
    it('ctrl cell selection', () => {
      ...
      expect(
        gridObj
          .getRows()[0]
          .children[5].classList.contains('e-cellselectionbackground')
      ).toBeTruthy();
    });
  ...
  describe('select Row/cell in show/hide', () => {
      ...
      gridObj = createGrid(
        {
          dataSource: data,
          columns: [
            { field: 'OrderID', headerText: 'Order ID', visible: false },
            { field: 'CustomerID', headerText: 'CustomerID' },
    ...
    it('initial check', () => {
      expect(
        gridObj.getHeaderContent().querySelectorAll('.e-headercell.e-hide')
          .length
      ).toBe(3);
      expect(gridObj.getContentTable().querySelectorAll('.e-hide').length).toBe(
        36
      );
    });
    ...
    it('select a cell', (done: Function) => {
      cellSelected = (args: Object) => {
        expect(
          gridObj
            .getRows()[0]
            .children[1].classList.contains('e-cellselectionbackground')
        ).toBeTruthy();
        expect(
          gridObj.getRows()[0].children[1].hasAttribute('aria-selected')
        ).toBeTruthy();
        done();
      };
      gridObj.cellSelected = cellSelected;
    ...
    it('press ctrl+end arrow', () => {
      ...
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 1].children[4].classList.contains(
            'e-cellselectionbackground'
          )
      ).toBeTruthy();
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 1].children[4].hasAttribute(
            'aria-selected'
          )
      ).toBeTruthy();
    });
    ...
    it('press shiftUp arrow', () => {
      ...
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 2].children[4].hasAttribute(
            'aria-selected'
          )
      ).toBeTruthy();
    });
    ...
    it('press shiftLeft arrow', () => {
      ...
      expect(
        gridObj
          .getRows()
          [gridObj.getRows().length - 1].children[3].hasAttribute(
            'aria-selected'
          )
      ).toBeTruthy();
    });
  ...
  describe('cell span selection', () => {
      ...
      gridObj = createGrid(
        {
          ...
          queryCellInfo: function (args: QueryCellInfoEventArgs) {
            if (
              ...
              args.data['OrderID'] === 10248
            ) {
              args.colSpan = 2;
            }
            if (
              args.column.field === 'ShipCity' &&
              args.data['ShipCity'] === 'Münster'
            ) {
              args.colSpan = 2;
            }
          },
    ...
    it('select after spanned cell', (done: Function) => {
      cellSelected = (args: Object) => {
        ...
        expect(
          gridObj.getRows()[0].children[1].getAttribute('data-colindex')
        ).toBe('2');
        gridObj.cellSelected = undefined;
        done();
      };
    ...
    it('press left arrow from adjacent cell of spanned cell', () => {
      ...
      expect(
        gridObj
          .getRows()[0]
          .children[0].classList.contains('e-cellselectionbackground')
      ).toBeTruthy();
      expect(
        gridObj.getRows()[0].children[0].hasAttribute('aria-selected')
      ).toBeTruthy();
    });
    it('press right arrow to select span cell from left adjacent cell', () => {
      ...
      expect(
        gridObj
          .getRows()[1]
          .children[4].classList.contains('e-cellselectionbackground')
      ).toBeTruthy();
      expect(
        gridObj.getRows()[1].children[4].hasAttribute('aria-selected')
      ).toBeTruthy();
    });
    it('press right arrow from spanned cell', () => {
      ...
      expect(
        gridObj.getRows()[2].children[0].hasAttribute('aria-selected')
      ).toBeFalsy();
    });
    ...
    it('press up arrow', () => {
      ...
      gridObj.keyboardModule.keyAction(args);
      expect(
        gridObj.getRows()[0].querySelector('.e-cellselectionbackground')
      ).not.toBeNull();
    });
    it('press down arrow', () => {
      let args: any = { action: 'downArrow', preventDefault: preventDefault };
      gridObj.keyboardModule.keyAction(args);
      expect(
        gridObj.getRows()[1].querySelector('.e-cellselectionbackground')
      ).not.toBeNull();
    });
    ...
    it('press shiftDown arrow', () => {
      ...
      gridObj.keyboardModule.keyAction(args);
      expect(
        gridObj.getRows()[2].querySelectorAll('.e-cellselectionbackground')
      ).not.toBeNull();
    });
    ...
    it('press shiftLeft arrow', () => {
      ...
      gridObj.keyboardModule.keyAction(args);
      expect(
        gridObj.getRows()[1].querySelectorAll('.e-cellselectionbackground')
          .length
      ).toBe(2);
      expect(
        gridObj.getRows()[1].querySelectorAll('[aria-selected]')
      ).not.toBeNull();
    });
    ...
    it('ctrl click', () => {
      ...
      expect(
        gridObj.getRows()[0].querySelectorAll('.e-cellselectionbackground')
          .length
      ).toBe(2);
      expect(
        gridObj.getRows()[1].querySelectorAll('.e-cellselectionbackground')
  ...
  describe('Grid Selection Issue Fixes', () => {
    ...
    it('change selectedrowindex', () => {
      (gridObj.selectedRowIndex = 3), gridObj.dataBind();
    ...
    it('EJ2-6930-Check all checkbox issue', () => {
      ...
      expect(
        (<any>(
          gridObj.element.querySelector('.e-checkselectall').nextSibling
    ...
    it('EJ2-7517- Group Toogle button check', () => {
      ...
      expect(
        <any>(
          gridObj.element
            .querySelector('.e-headerchkcelldiv')
            .querySelector('.e-grptogglebtn')
        )
      ).toBeNull();
    });
  ...
  describe('Grid checkboxOnly keyboard Selection Issue Fixes', () => {
    ...
    it('check select all datas', () => {
      expect(gridObj.selectionModule.selectedRowIndexes.length).toBe(
        (gridObj.dataSource as object[]).length
      );
    });
  ...
  describe('Cell Selection Issue Fixes', () => {
    ...
    it('select the cell', () => {
      expect(
        gridObj.getRows()[0].children[0].classList.contains('e-gridchkbox')
      ).toBeTruthy();
    });
    it('select the cell', () => {
      cellSelected = (args: { currentCell: any }) => {
        expect(
          args.currentCell.classList.contains('e-cellselectionbackground')
        ).toBeTruthy();
        gridObj.cellSelected = undefined;
      };
      gridObj.cellSelected = cellSelected;
  ...
  describe('Grid Selection Issue Fixes', () => {
    ...
    it('Selecting Third row', (done: Function) => {
      ...
      (<HTMLElement>(
        gridObj.element
          .querySelectorAll('.e-row')[2]
          .querySelector('.e-gridchkbox')
          .querySelector('.e-checkbox-wrapper')
      )).click();
    });
    ...
    it('multi row - ctrl click testing- selecting Fourth row', (done: Function) => {
      ...
      gridObj.rowSelected = rowSelected;
      (gridObj.selectionModule as any).clickHandler({
        target: gridObj.element
          .querySelectorAll('.e-row')[3]
          .querySelectorAll('.e-rowcell')[2],
        ctrlKey: true,
      });
    });
...
describe('Row,cell Selecting in batch edit while adding record => ', () => {
  ...
  it('rowSelect event while adding new records in batchedit', () => {
    ...
    gridObj.rowSelected = rowSelected;
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.element.id + '_add' },
    });
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.element.id + '_add' },
    });
    gridObj.editModule.saveCell();
    expect(gridObj.selectionModule.getCurrentBatchRecordChanges().length).toBe(
      14
    );
  });
  ...
  it('recordClick event in Grid', () => {
    ...
    (
      gridObj
        .getContent()
        .querySelectorAll('.e-row')[2]
        .querySelectorAll('.e-rowcell')[1] as HTMLElement
    ).click();
  });
...
describe('grid checkbox selection functionalities with Freeze pane', () => {
  ...
  it('Checkbox select all', () => {
    (
      gridObj.element
        ...
        .querySelectorAll('.e-frame.e-icons')[0] as any
    ).click();
    expect(gridObj.getSelectedRowIndexes().length).toBe(6);
    expect(
      gridObj.element.querySelectorAll('.e-frame.e-icons.e-check').length
    ).toBe(7);
  });
  ...
  it('checkbox deselect check', () => {
    (
      gridObj.element
        ...
        .querySelectorAll('.e-rowcell')[0] as any
    ).click();
    expect(gridObj.getSelectedRowIndexes().length).toBe(5);
    expect(
      gridObj.element
        .querySelector('.e-headercelldiv')
        .querySelectorAll('.e-stop').length
    ).toBe(1);
    (
      gridObj.element
        .querySelector('.e-headercelldiv')
  ...
  it('checkbox unselect all check', () => {
    ...
    expect(
      gridObj.element
        .querySelector('.e-headercelldiv')
        .querySelectorAll('.e-uncheck').length
    ).toBe(1);
  });
...
describe('grid selection functionalities with Frozen rows', function () {
  ...
  it('get rowIndex from rowdata', function () {
    gridObj.selectRow(
      gridObj.getRowIndexByPrimaryKey({
        OrderID: 10253,
        CustomerID: 'HANAR',
        EmployeeID: 3,
...
describe('selectRow with virtualScrolling', () => {
  ...
  it('check cell select args multiselect', (done: Function) => {
    let cellSelecting = (args: any) => {
      ...
      expect(args['previousRowCellIndex']).toEqual({
        rowIndex: 0,
        cellIndex: 0,
      });
    };
...
describe('EJ2-33599 selection does not maintain while calling setrowdata method', () => {
  ...
  it('selectrowdata', (done: Function) => {
    ...
    expect(
      (<any>gridObj.getRows()[0]).cells[0].classList.contains(
        'e-selectionbackground'
      )
    ).toBeTruthy();
    done();
...
describe('Checkbox state when Selecting in batch edit while adding record => ', () => {
  ...
  it('Initial header checkbox state', () => {
    let cBox: HTMLElement = gridObj.element.querySelector('.e-checkselectall')
  ...
  it('Add new records, cancel and check state', (done: Function) => {
      ...
      if (args.name === 'batchCancel') {
        expect(
          gridObj.element
            ...
            .nextElementSibling.classList.contains('e-stop')
        ).toBeFalsy();
    ...
    select('#' + gridObj.element.id + 'EditConfirm', gridObj.element)
...
describe('AutoFill Feature', () => {
  ...
  it('update cell', () => {
    ...
    (gridObj.selectionModule as any).startAFCell = gridObj
      .getRowByIndex(0)
      .querySelectorAll('td')[1];
    (gridObj.selectionModule as any).endAFCell = gridObj
      .getRowByIndex(2)
    ...
    gridObj.copy();
    let value1: string = (
      document.querySelector('.e-clipboard') as HTMLInputElement
    ).value.split(/\r?\n/)[0];
    let value2: string = (
      document.querySelector('.e-clipboard') as HTMLInputElement
    ).value.split(/\r?\n/)[1];
...
describe('rowdeselect checking with persist selection and ResetOnRowClick', () => {
  ...
  it('selecting more than one row in checkbox and clicking in row element', () => {
    (
      gridObj.element.querySelectorAll('.e-frame.e-icons')[1] as HTMLElement
    ).click();
    (
      gridObj.element.querySelectorAll('.e-frame.e-icons')[2] as HTMLElement
    ).click();
    ...
    gridObj.rowDeselected = rowDeSelected;
    (
      gridObj.element.querySelectorAll(
...
describe('rowdeselect checking with persist selection and ResetOnRowClick', () => {
  ...
  it('selecting checkselectall ', () => {
    (
      gridObj.element.querySelectorAll('.e-frame.e-icons')[0] as HTMLElement
    ).click(); //selecting all row
    let rowDeSelecting = (e: any) => {
  ...
  describe('Ensure selection event arguments', () => {
    ...
    it('header checkbox selection', (done: Function) => {
      let rowSelecting = (args: RowSelectingEventArgs) => {
        ...
        expect((args.data as Object[]).length).toBe(
          gridObj.currentViewData.length
        );
        expect((args.row as Element[]).length).toBe(
          gridObj.currentViewData.length
        );
        expect((args.foreignKeyData as Object[]).length).toBe(
          gridObj.currentViewData.length
        );
        gridObj.rowSelecting = null;
      };
  ...
  describe('EJ2-67278 - Row deselection is not working in Virtualization with checkbox column', () => {
    ...
    it('check select all selection and header checkbox de-selection action', () => {
      expect(gridObj.selectionModule.selectedRecords.length).toBe(
        gridObj.currentViewData.length
      );
      (<HTMLElement>gridObj.element.querySelector('.e-checkselectall')).click();
    });
  ...
  describe('EJ2-41468 - row data in rowSelected event args is not maintained properly with Batch edit mode', () => {
    ...
    it('Check selected data', () => {
      ...
      gridObj.rowSelected = rowSelected;
      let dataCell: HTMLElement = gridObj
        .getRows()[1]
        .querySelector('.e-rowcell') as HTMLElement;
      dataCell.click();
    });
...
describe(' EJ242851 => Column Selection testing', () => {
  ...
  it('select Column by range method testing', () => {
    ...
    expect(
      gridObj.element
        .querySelector('.e-row')
        .querySelectorAll('.e-columnselection').length
    ).toBe(3);
    expect(
      gridObj.getHeaderContent().querySelectorAll('.e-columnselection').length
    ).toBe(3);
  });
  ...
  it('check select event', (done: Function) => {
    let columnSelecting: EmitType<Object> = (args: Object) => {
      ...
      expect(args['headerCell']).toEqual(
        gridObj.getHeaderContent().querySelector('.e-headercell')
      );
    };
...
describe('EJ2-44995 - isInteracted property in rowDeselecting and rowDeselected events with Selectall and paging', () => {
  ...
  it('isInteracted property testing - Click', (done: Function) => {
    ...
    gridObj.rowDeselecting = rowDeselecting;
    let selectAll: HTMLElement = <HTMLElement>(
      gridObj.element.querySelector('.e-checkselectall')
    );
    selectAll.click();
    selectAll.click();
  });
...
describe('EJ2-45406 - rowDeselect events with persistSelection', () => {
  ...
  it('rowDeselecting event testing - Click', (done: Function) => {
    ...
    gridObj.rowDeselecting = rowDeselecting;
    (<HTMLElement>(
      gridObj.element.querySelector('.e-row .e-checkselect')
    )).click();
    gridObj.clearSelection();
  });
...
describe('EJ2-45650 - Persist Checkbox selection not working properly with frozen column', () => {
  ...
  it('checking Deselect row with persistSelection and frozen column', () => {
    let chkBox: any = gridObj.element.querySelectorAll(
      '.e-checkselect'
    )[0] as HTMLElement;
    chkBox.click();
    chkBox.click();
...
describe('EJ2-56885 - Previously selected data is not returned in selection events when select rows with CTRL key', () => {
  ...
  it('multi row - ctrl click testing- selecting second row', (done: Function) => {
    ...
    gridObj.rowSelected = rowSelected;
    (gridObj.selectionModule as any).clickHandler({
      target: gridObj.getRows()[1].querySelector('.e-rowcell'),
      ctrlKey: true,
    });
  });
  afterAll(() => {
...
describe('EJ2-53014 - specific row selection with virtualization feature', () => {
  ...
  it('do scroll with partal selection', (done: Function) => {
    ...
    gridObj.dataBound = dataBound;
    (gridObj.selectionModule as any).rowsRemoved({
      records: gridObj.getSelectedRecords(),
    });
    (
      gridObj.element.querySelector('.e-gridcontent .e-content') as HTMLElement
    ).scrollTop = 10;
  });
...
describe('EJ2-59308 Preventing keyboard actions when allow keyboard set to false', () => {
  ...
  it('checkbox selection through ctrl click testing', (done: Function) => {
    ...
    gridObj.rowSelected = rowSelected;
    (<HTMLElement>(
      gridObj.element.querySelectorAll('.e-row')[0].querySelector('.e-rowcell')
    )).click();
    (gridObj.selectionModule as any).clickHandler({
      target: gridObj.element
        .querySelectorAll('.e-row')[3]
  ...
  it('checkbox selection through space key testing', () => {
    let args: any = { action: 'space', preventDefault: preventDefault };
    rows = gridObj.getRows();
    let chkBox: HTMLElement = rows[2].querySelector(
      '.e-checkselect'
    ) as HTMLElement;
    chkBox.click();
    args.target = chkBox;
    gridObj.keyboardModule.keyAction(args);
    expect(
      chkBox.nextElementSibling.classList.contains('e-check')
    ).toBeTruthy();
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('Focus strategy coverage', () => {
  ...
  it('focus navigation by Tab and ShiftTab', () => {
    const firstHeaderCell: HTMLElement = gridObj
      .getHeaderContent()
      .querySelector('.e-headercell');
    ...
    let args: any = {
      action: 'tab',
      preventDefault: preventDefault,
      target: (rows[0] as HTMLTableRowElement).cells[0],
    };
    (gridObj.focusModule as any).onKeyPress(args);
    expect(
      (rows[0] as HTMLTableRowElement).cells[1].classList.contains('e-focused')
    ).toBeTruthy();
    args = {
      action: 'shiftTab',
      preventDefault: preventDefault,
      target: (rows[0] as HTMLTableRowElement).cells[1],
    };
    (gridObj.focusModule as any).onKeyPress(args);
    expect(
      (rows[0] as HTMLTableRowElement).cells[0].classList.contains('e-focused')
    ).toBeTruthy();
  });
  it('focus navigation from header to content', () => {
    rows = gridObj.getRows();
    (
      gridObj.element.querySelectorAll('.e-headercell')[2] as HTMLElement
    ).focus();
    ...
    let args: any = {
      action: 'tab',
      preventDefault: preventDefault,
      target: gridObj.element.querySelectorAll('.e-headercell')[2],
    };
    (gridObj.focusModule as any).onKeyPress(args);
    expect(
      (rows[0] as HTMLTableRowElement).cells[0].classList.contains('e-focused')
    ...
    expect(
      gridObj.element
        .querySelectorAll('.e-headercell')[2]
        .classList.contains('e-focused')
    ).toBeTruthy();
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('EJ2-59499 - implementation on Persist Selection misbehaves while sort, filter and search with virtualization => ', () => {
  ...
  let clearFilter: Function = (gridObj: Grid, done: Function) => {
    let actionComplete: any = (args?: Object): void => {
      if (
        gridObj.element.querySelectorAll('.e-row').length ===
  ...
  it('Check Single sort orderID dsc action', (done: Function) => {
    actionComplete = (args: any): any => {
      ...
      expect(
        gridObj
          .getHeaderContent()
          .querySelectorAll('.e-columnheader')[0]
          .querySelectorAll('.e-sortnumber').length
      ).toBe(0);
      done();
    };
    gridObj.actionComplete = actionComplete;
  ...
  it('check persist selection after sorting operation', () => {
    ...
    expect(rows[1].querySelectorAll('.e-selectionbackground').length).not.toBe(
      6
    );
    expect(
      gridObj.element
...
describe('EJ2-60999 Row selection is not updated properly inside the rowDeselected event while data actions', () => {
  ...
  it('EJ2-60026 - Ensuring Selected records to be null in rowDeselected event', (done: Function) => {
    ...
    gridObj.rowDeselected = rowDeselected;
    (<HTMLElement>(
      gridObj.element.querySelectorAll('.e-row')[4].querySelector('.e-rowcell')
    )).click();
    (
      gridObj
        .getHeaderContent()
        .querySelectorAll('.e-headercell')[0] as HTMLElement
    ).click();
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('EJ2-65110 - Enter and shiftEnter key functionality with template column', () => {
  ...
  it('focus navigation by enter and ShiftEnter', () => {
    ...
    let args: any = {
      action: 'enter',
      preventDefault: preventDefault,
      target: (rows[0] as HTMLTableRowElement).cells[2],
    };
    (gridObj.focusModule as any).onKeyPress(args);
    expect(
      (rows[1] as HTMLTableRowElement).cells[2].classList.contains('e-focused')
    ).toBeTruthy();
    args = {
      action: 'shiftEnter',
      preventDefault: preventDefault,
      target: (rows[1] as HTMLTableRowElement).cells[2],
    };
    (gridObj.focusModule as any).onKeyPress(args);
    expect(
      (rows[0] as HTMLTableRowElement).cells[2].classList.contains('e-focused')
    ).toBeTruthy();
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('BUG 837180 - Selection moved to first row when newRowPosition given as Bottom', () => {
            ...
            if (
              ...
              args.index =
                gridObj.pageSettings.currentPage *
                  gridObj.pageSettings.pageSize -
                gridObj.pageSettings.pageSize;
            } else if (gridObj.editSettings.newRowPosition === 'Bottom') {
              args.index = gridObj.dataSource['length']
                ? gridObj.currentViewData.length
                : 0;
            }
          }
  ...
  it('1st Add complete', function (done) {
    ...
    gridObj.actionComplete = actionComplete;
    gridObj.editModule.editModule.addRecord({
      OrderID: 10247,
      CustomerID: 'updated',
      Freight: 12,
    });
...
describe('BUG 839515 - Selection moved to first row while addRecord( ) with index value', () => {
  ...
  it('Add new row using addRecord', function (done) {
    ...
    gridObj.actionComplete = actionComplete;
    gridObj.editModule.editModule.addRecord(
      { OrderID: 10001, CustomerID: 'New Record', Freight: 10 },
      5
    );
  });
...
describe('BUG 842498 - In a checkbox with a specified field, if you click the intermediate checkbox, all checkboxes should be selected', () => {
  ...
  it('Check selectAll checkbox', function () {
    (<HTMLElement>gridObj.element.querySelector('.e-checkselectall')).click();
    expect(
      (document.getElementsByClassName('e-checkselectall')[0] as HTMLElement)[
        'checked'
      ]
    ).toBeTruthy();
  });
...
describe('Code Coverage - autofill with scroll', () => {
  ...
  it('for coverage - 2', () => {
    (<any>gridObj.selectionModule).setScrollPosition(
      gridObj.getContent(),
      'right',
      null
    );
    ...
    (<any>gridObj.selectionModule).setScrollPosition(
      gridObj.getContent(),
      'up',
      { target: gridObj.getColumnHeaderByIndex(2) }
    );
  });
  afterAll(function () {
    destroy(gridObj);
...
describe('Code Coverage - left freeze with autofill', () => {
  ...
  it('1,3 => for coverage - 2', () => {
    ...
    (<any>gridObj.selectionModule).mouseDownHandler({
      target: elem,
      preventDefault: () => {},
      clientX: elemClient.x,
      clientY: elemClient.y,
    });
    (<any>gridObj.selectionModule).mouseUpHandler({
      target: elem,
      preventDefault: () => {},
      clientX: elemClient.x,
      clientY: elemClient.y,
...
describe('Code Coverage - Left - Right freeze with autofill', () => {
  ...
  describe('on property change', () => {
    ...
    it('execute methods 1', (done: Function) => {
      (gridObj.selectionModule as any).isFirstRow(
        gridObj.element.querySelector('.e-rowcell')
      );
      expect(1).toBe(1);
      done();
    });
  ...
  describe('283715 => Checkbox cell selection is not properly cleared on clearSelection method call.', () => {
    ...
    it('checkbox column cell selection', (done: Function) => {
      (<any>gridObj.selectionModule).selectCell(
    ...
    it('checkbox column cell de-selection', (done: Function) => {
      ...
      expect((<any>gridObj.selectionModule).selectedRowCellIndexes.length).toBe(
        0
      );
      done();
    });
...
describe('EJ2-855396 - SelectionSettings mode as Both shows wrong selected items', () => {
  ...
  it('Selecting with checkbox', (done: Function) => {
    ...
    expect(
      gridObj.selectionModule.selectedRowCellIndexes[0].cellIndexes[0]
    ).toBe(1);
  });
...
describe('EJ2-885652 - Programmatic row deselect with toggle as true, not trigger rowDeselecting, rowDeselected events and not deselect row when the grid has existing selected rows', () => {
  ...
  it('Selecting with checkbox', (done: Function) => {
    ...
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.element.id + '_select' },
    });
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.element.id + '_select' },
    });
...
describe('EJ2-891988 - SelectRow method with toggle set to true does not selects the rows properly.', () => {
  ...
  it('Selecting with checkbox', (done: Function) => {
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.element.id + '_select123' },
    });
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.element.id + '_select1' },
    });
    expect(gridObj.selectionModule.selectedRecords.length).toBe(2);
    done();
  });
...
describe('coverage for selection file.', () => {
  ...
  it('selection file code coverage - 2', () => {
    ...
    (
      gridObj.selectionModule as any
    ).selectionSettings.enableSimpleMultiRowSelection = false;
  ...
  it('selection file code coverage - 3', () => {
    ...
    (gridObj.selectionModule as any).keyDownHandler({
      target: gridObj.element,
    });
    (gridObj.selectionModule as any).keyDownHandler({
      keyCode: 93,
  ...
  it('selection file code coverage - 4', () => {
    ...
    (gridObj.selectionModule as any).actionComplete({
      selectedRow: 0,
      requestType: 'save',
      action: 'add',
    });
  ...
  it('selection file code coverage - 5', () => {
    ...
    (gridObj.selectionModule as any).selectionSettings.allowColumnSelection =
      true;
