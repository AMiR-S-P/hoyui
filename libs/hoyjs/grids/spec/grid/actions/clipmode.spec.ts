...
describe('ClipMode module', () => {
      ...
      gridObj = createGrid(
        {
          ...
          columns: [
            ...
            { headerText: 'EmployeeID', field: 'EmployeeID' },
            {
              ...
              field: 'Shipping Address of the order',
      ...
      row.forEach((element: HTMLElement) => {
        ...
        for (let i: number = 0; i < td.length; i++) {
          if (gridObj.getColumns()[i].clipMode === 'Clip') {
            ...
            gridObj.getColumns()[i].clipMode === 'EllipsisWithTooltip'
          ) {
            expect(td[i].classList.contains('e-gridclip')).toBeFalsy();
  ...
  describe('clipmode with Resizing', () => {
    ...
    it('Class testing', () => {
      ...
      let td: NodeListOf<Element> =
        gridObj.element.querySelectorAll('e-rowcell');
      let td1: Element[] = [];
      for (let i: number = 0; i < td.length; i++) {
  ...
  describe('EJ2-36833 Checkbox column has empty tooltips when grid level clipMode is EllipsisWithTooltip', () => {
    ...
    it('Checkbox class testing', () => {
      row = [gridObj.contentModule.getTable().children['1'].children];
      expect(
        row[0][0].cells[0].classList.contains('e-ellipsistooltip')
      ).toBeFalsy();
      expect(
        row[0][1].cells[0].classList.contains('e-ellipsistooltip')
      ).toBeFalsy();
      expect(
        row[0][2].cells[0].classList.contains('e-ellipsistooltip')
      ).toBeFalsy();
      expect(
        gridObj
          .getHeaderTable()
          .querySelector('.e-columnheader')
          .children[0].classList.contains('e-ellipsistooltip')
      ).toBeFalsy();
    });
    afterAll(() => {
      destroy(gridObj);
  ...
  describe('EJ2-66976- Ellipsis with tooltip is not working with batch editing', () => {
    ...
    it('checking tooltip', () => {
      row = [gridObj.contentModule.getTable().children['1'].children];
      expect(
        row[0][2].cells[5].classList.contains('e-ellipsistooltip')
      ).toBeTruthy();
    });
