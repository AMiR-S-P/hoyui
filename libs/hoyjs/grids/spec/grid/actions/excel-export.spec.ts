...
describe('excel Export =>', () => {
  ...
  describe('Single Grid excel Export =>', () => {
    ...
    it('check cell with value 0', () => {
      ...
      let excelRows: ExcelRow[] = (<any>(
        gridObj.excelExportModule
      )).processGridExport(gridObj, undefined, { result: data });
    });
  ...
  describe('Custom font in Grid excel Export =>', () => {
    ...
    it('grid exporting(Check with multiple exporting)', (done) => {
      gridObj
        .excelExport({ dataSource: new DataManager(data) }, true)
        .then((doc: Workbook) => {
          expect(doc).not.toBeUndefined();
          done();
        });
    ...
    it('Excel grid Check column length', () => {
      ...
      let style: any = (<any>gridObj.excelExportModule).styles[
        (<any>gridObj.excelExportModule).getColumnStyle(gridObj, 1)
      ];
    ...
    it('Expand a detail row', (done) => {
      gridObj.childGrid.dataBound = () => {
        expect(
          gridObj
            .getRowsObject()
            .filter((row: any) => row.isExpand && !row.isDetailRow).length
        ).toBe(1);
        done();
      };
  ...
  describe('EJ2-40200 internal exportDataBound event in excel export =>', () => {
    ...
    it('grid excel exportGroupCaption check', (done) => {
      gridObj.on(
        'export-DataBound',
        function (args: { excelRows: ExcelRow[] }) {
          expect(args.excelRows.length).toBe(
            (gridObj.dataSource as any).length + 1
          );
          done();
        }
      );
  ...
  describe('export with custom columns', () => {
    ...
    it('export new columns', (done) => {
      let newColumns: any = [
        ...
        {
          headerText: 'Ship Details',
          columns: { field: 'ShipRegion', width: 140 },
        },
      ];
      ...
      gridObj = createGrid(
        {
          ...
          groupSettings: {
            ...
            columns: ['ShipRegion', 'ShipCountry'],
          },
          columns: [
            {
              field: 'OrderID',
              ...
              rows: [
                {
                  ...
                  cells: [
                    /* tslint:disable-next-line:max-line-length */
                    {
                      ...
                      colSpan: 5,
                      value: 'INVOICE',
                      style: { fontColor: '#C25050', fontSize: 25, bold: true },
                    },
                  ],
                },
                {
        ...
        .excelExport(
          {
            header: {
              ...
              rows: [
                ...
                {
                  index: 3,
                  cells: [{ index: 1, colSpan: 2, value: 'Adventure Traders' }],
                },
              ],
            },
            footer: {
        ...
        .excelExport(
          {
            ...
            header: {
              ...
              rows: [
                {
                  index: 1,
                  cells: [{ index: 1, colSpan: 5, value: 'INVOICE' }],
                },
                {
                  index: 3,
                  cells: [
            ...
            footer: {
              footerRows: 4,
              rows: [{ cells: [{ colSpan: 6, value: 'Thank you' }] }],
            },
          },
          true
        )
  ...
  describe('Detail template Excel export => ', () => {
    ...
    it('process detail template', (done) => {
      gridObj.exportDetailTemplate = (args: ExportDetailTemplateEventArgs) => {
        if (args.parentRow.data['EmployeeID'] === 1) {
          ...
          args.value = {
            hyperLink: {
              target:
                'mailto:' + args.parentRow.data['FirstName'] + '@domain.com',
              displayText: args.parentRow.data['FirstName'],
            },
          };
        } else if (args.parentRow.data['EmployeeID'] === 4) {
          args.value = {
            columnHeader: [{ cells: [{ colSpan: 2, value: 'INVOICE' }] }],
            rows: [
              {
                cells: [
                  {
      ...
      gridObj
        .excelExport({ hierarchyExportMode: 'All' }, true)
        .then((Doc: Workbook) => {
          expect(Doc).not.toBeUndefined();
          done();
        });
  ...
  describe('Excel export file code coverage', () => {
    ...
    it('Excel export methodfile code coverage', () => {
      ...
      (gridObj as any).excelExportModule.processExcelHeader({
        rows: (gridObj as any).headerModule.rows,
      });
...
//  * Grid Excel Export spec document
//  */
// import { EmitType, EventHandler } from '@hoyui/base';
...
// import { DataManager, ODataV4Adaptor } from '@hoyui/data';
// import { ExcelExport } from '../../../src/grid/actions/excel-export';
