 ...
 */
import { EmitType, select } from '@hoyui/base';
...
import { DatePicker } from '@hoyui/calendars';
...
describe('Inline Editing module', () => {
  ...
  describe('Inline editing render => ', () => {
    ...
    it('Edit start - args.cancel', (done: Function) => {
      ...
      (<any>gridObj.toolbarModule).toolbarClickHandler({
        item: { id: gridObj.element.id + '_edit' },
      });
    });
    ...
    it('Edit start', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'beginEdit') {
          ...
          expect(gridObj.element.querySelectorAll('.e-editedrow').length).toBe(
            1
          );
          expect(gridObj.element.querySelectorAll('.e-normaledit').length).toBe(
            1
          );
          expect(gridObj.element.querySelectorAll('.e-gridform').length).toBe(
            1
          );
          expect(gridObj.element.querySelectorAll('form').length).toBe(1);
          let cells = gridObj.element
    ...
    it('Edit complete', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'save') {
          expect(
            (gridObj.editModule as any).editModule.editRowIndex
          ).toBeNull();
          expect(gridObj.element.querySelectorAll('.e-normaledit').length).toBe(
            0
          );
          ...
          expect((gridObj.currentViewData[0] as any).CustomerID).toBe(
            'updated'
          );
          //row count check
          expect(gridObj.getContent().querySelectorAll('.e-row').length).toBe(
            11
          );
          //record count check
      ...
      gridObj.actionBegin = actionBegin;
      (
        select('#' + gridObj.element.id + 'CustomerID', gridObj.element) as any
      ).value = 'updated';
      (<any>gridObj.toolbarModule).toolbarClickHandler({
        item: { id: gridObj.element.id + '_update' },
      });
    });
    ...
    it('Add start', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          expect(gridObj.element.querySelectorAll('.e-addedrow').length).toBe(
            1
          );
          expect(gridObj.element.querySelectorAll('.e-normaledit').length).toBe(
            1
    ...
    it('Add complete - args.cancel', (done: Function) => {
      ...
      gridObj.actionBegin = actionBegin;
      (
        select('#' + gridObj.element.id + 'OrderID', gridObj.element) as any
      ).value = 10247;
      (
        select('#' + gridObj.element.id + 'CustomerID', gridObj.element) as any
      ).value = 'updated';
    ...
    it('Delete action', (done: Function) => {
      ...
      gridObj.selectRow(0, true);
      (<any>gridObj.toolbarModule).toolbarClickHandler({
        item: { id: gridObj.element.id + '_delete' },
      });
    });
    ...
    it('Edit-cancel complete', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'cancel') {
          ...
          expect(gridObj.editModule.formObj.isDestroyed).toBeTruthy();
          //updatated data cehck
          expect((gridObj.currentViewData[0] as any).CustomerID).not.toBe(
            'updatednew'
          );
          //row count check
          expect(gridObj.getContent().querySelectorAll('.e-row').length).toBe(
      ...
      (<any>gridObj.toolbarModule).toolbarClickHandler({
        item: { id: gridObj.element.id + '_cancel' },
      });
    });
    ...
    it('dbl edit start', (done: Function) => {
      ...
      (gridObj as any).dblClickHandler({
        target: gridObj.element.querySelectorAll('.e-row')[1].firstElementChild,
      });
    });
    ...
    it('click another row edit complete', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'save') {
          expect((gridObj.currentViewData[1] as any).CustomerID).toBe(
            'updated'
          );
          //row count check
          expect(gridObj.getContent().querySelectorAll('.e-row').length).toBe(
    ...
    it('Add complete method with data testing', (done: Function) => {
      ...
      gridObj.actionComplete = actionComplete;
      (<any>gridObj.editModule).editModule.addRecord({
        OrderID: 10246,
        CustomerID: 'updated',
      });
    });
    ...
    it('check validation msg width', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          ...
          let validationEle: HTMLElement = gridObj.element.querySelectorAll(
            '.e-tooltip-wrap'
          )[0] as HTMLElement;
  ...
  describe('Keyboard shortcuts => ', () => {
    ...
    it('Edit start', (done: Function) => {
      ...
      gridObj.keyboardModule.keyAction({
        ...
        target: gridObj.getContent().querySelector('.e-row'),
      } as any);
    });
  ...
  describe('Disable editing, edit mode change and delete alert => ', () => {
    ...
    it('deleting - no record alert', () => {
      ...
      expect(
        select(
          '#' + gridObj.element.id + 'EditAlert',
          gridObj.element
        ).classList.contains('e-popup-open')
      ).toBeTruthy();
      select('#' + gridObj.element.id + 'EditAlert', gridObj.element)
    ...
    it('deleting - delete confirm - cancel', () => {
      ...
      expect(
        select(
          '#' + gridObj.element.id + 'EditConfirm',
          gridObj.element
        ).classList.contains('e-popup-open')
      ).toBeTruthy();
  ...
  describe('Group, and text align => ', () => {
    ...
    it('text align check', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'beginEdit') {
          let row: Element =
            gridObj.element.querySelectorAll('.e-editedrow')[0];
          expect(
            (select('#' + gridObj.element.id + 'CustomerID', row) as any).style
              .textAlign
          ).toBe('right');
          expect(
            (select('#' + gridObj.element.id + 'ShipCity', row) as any).style
              .textAlign
          ).toBe('center');
          expect(
            (select('#' + gridObj.element.id + 'ShipRegion', row) as any).style
              .textAlign
          ).toBe('left');
          expect(
            select(
          ...
          expect(
            select(
              '#' + gridObj.element.id + 'ShipCity',
              row
            ).classList.contains('e-textbox')
          ).toBeTruthy();
          expect(
            select(
              '#' + gridObj.element.id + 'ShipRegion',
              row
            ).classList.contains('e-textbox')
          ).toBeTruthy();
          done();
    ...
    it('Edit complete', (done: Function) => {
      ...
      gridObj.keyboardModule.keyAction({
        action: 'enter',
        preventDefault: preventDefault,
        target: (<any>gridObj.getContent().querySelector('.e-row')).cells[0],
      } as any);
    });
      ...
      actionComplete = (args?: any): void => {
        if (args.requestType === 'beginEdit') {
          expect(
            gridObj.element
              .querySelector('.e-editedrow')
              .querySelectorAll('.e-indentcell').length
          ).toBe(1);
          done();
        }
      };
    ...
    it('Edit complete', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'save') {
          expect((gridObj.getCurrentViewRecords()[0] as any).CustomerID).toBe(
            'updated'
          );
          done();
        }
      ...
      actionComplete = (args?: any): void => {
        if (args.requestType === 'beginEdit') {
          expect(
            gridObj.element
              .querySelector('.e-editedrow')
              .querySelectorAll('.e-detailrowcollapse').length
          ).toBe(1);
          done();
        }
      };
  ...
  describe('validation testing => ', () => {
    ...
    it('Edit with invalid data', () => {
      ...
      expect(
        gridObj.editModule.formObj.element.querySelectorAll('.e-griderror')
          .length
      ).toBe(1);
    ...
    it('Add with invalid data', () => {
      ...
      expect(document.querySelectorAll('.e-griderror').length).toBeGreaterThan(
        0
      );
      // EJ2-50953 - When double-clicking inline editing validation message, JavaScript error is thrown
      (gridObj as any).dblClickHandler({
        target: document.querySelectorAll('.e-griderror')[0],
      });
    });
    ...
    it('render datetimepicker component while adding testing', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          expect(
            gridObj.element
              .querySelector('.e-gridform')
              .querySelectorAll('td')[6]
              .querySelector('input')
              .classList.contains('e-datetimepicker')
          ).toBeTruthy();
          done();
        }
      };
  ...
  describe('Inline editing render with Freeze => ', () => {
    ...
    it('Edit complete', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'save') {
          ...
          //row count check
          expect(
            gridObj.getContent().querySelectorAll('.e-row').length +
              gridObj
                .getHeaderContent()
                .querySelector('tbody')
                .querySelectorAll('.e-row').length
          ).toBe(11);
          //record count check
          expect(gridObj.currentViewData.length).toBe(11);
          expect(gridObj.isEdit).toBeFalsy();
    ...
    it('Delete action', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'delete') {
          ...
          expect(gridObj.element.querySelectorAll('.e-overlay').length).toBe(3);
          expect(gridObj.isEdit).toBeFalsy();
          gridObj.dblClickHandler({
            target: gridObj.getContent().querySelectorAll('.e-row')[0],
          });
          setTimeout(() => {
            done();
          }, 100);
    ...
    it('Ensure added row position', (done: Function) => {
      ...
      actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          let editFormIndex: number =
            gridObj.element.querySelectorAll('.e-addedrow')[0].rowIndex;
          let totalRowsCount: number = gridObj
            .getContent()
            .querySelectorAll('.e-row').length;
  ...
  describe('EJ2-6484 hierarchy testing => ', () => {
    ...
    it('Expand method testing', (done: Function) => {
      detailDataBound = (args?: any): void => {
        childGridObj = (gridObj.element.querySelector('.e-grid') as any)
          .ej2_instances[0];
        done();
      };
      gridObj.detailDataBound = detailDataBound;
      gridObj.detailRowModule.expand(
        gridObj.getDataRows()[0].querySelector('.e-detailrowcollapse')
      );
    });
  ...
  describe('Editing with empty grid => ', () => {
    ...
    it('closing record', () => {
      ...
      expect(gridObj.getContentTable().querySelector('.e-emptyrow')).not.toBe(
        null
      );
      expect(gridObj.getContentTable().querySelector('.e-hide')).toBe(null);
    });
    ...
    it('Add start && EJ2-7012-script error on numeric text box click', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          ...
          expect(
            gridObj
              .getRowInfo(<Element | EventTarget>gridObj.element)
              .toString()
          ).toBe({}.toString());
  ...
  describe('EJ2-22823 Add and Delete operations current view check in virtualization => ', () => {
    ...
    it('Delete operation in current view check', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType == 'delete') {
          expect((gridObj.getCurrentViewRecords()[0] as any).OrderID).toBe(
            10249
          );
          done();
        }
  ...
  describe('Keyboard shortcuts => ', () => {
    ...
    it('Edit complete', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'save') {
          expect((gridObj.getCurrentViewRecords()[1] as any).CustomerID).toBe(
            'updated'
          );
          ...
          for (let i: number = 0; i < td.length; i++) {
            expect(
              td[i]
                ...
                .classList.contains('e-lastrowcell')
            ).toBeTruthy();
          }
          gridObj.actionComplete = null;
          (<any>gridObj.toolbarModule).toolbarClickHandler({
    ...
    it('Edit complete', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'save') {
          ...
          expect(
            gridObj.getContent().querySelectorAll('.e-lastrowcell').length
          ).toBe(gridObj.getColumns().length);
          gridObj.actionComplete = null;
          (<any>gridObj.toolbarModule).toolbarClickHandler({
            item: { id: gridObj.element.id + '_save' },
          });
          done();
        }
      };
      ...
      actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          expect(
            args.row
              .querySelector('.e-rowcell')
              .classList.contains('e-lastrowadded')
          ).toBeTruthy();
          gridObj.actionComplete = null;
          (<any>gridObj.toolbarModule).toolbarClickHandler({
            item: { id: gridObj.element.id + '_cancel' },
  ...
  describe('Row virtualization with inline edit support', () => {
    ...
    it('ensure edit form', (done: Function) => {
      ...
      expect(gridObj.element.querySelectorAll('form').length).toBe(0);
      expect(gridObj.isEdit).toBeTruthy();
      let data: Object = (<{ virtualData?: Object }>(
        (gridObj as Grid).contentModule
      )).virtualData;
    ...
    it('edit complete - 2', (done: Function) => {
      ...
      gridObj.actionBegin = actionBegin;
      (
        select('#' + gridObj.element.id + 'FIELD1', gridObj.element) as any
      ).value = 'updated';
      (<any>gridObj.toolbarModule).toolbarClickHandler({
        item: { id: gridObj.element.id + '_update' },
      });
    ...
    it('edit complete - 4', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'save') {
          //updatated data cehck
          expect(gridObj.contentModule.vgenerator.cache[1][2].data.FIELD1).toBe(
            'updated'
          );
          expect(gridObj.isEdit).toBeFalsy();
          done();
    ...
    it('Edit-cancel complete', (done: Function) => {
      ...
      actionBegin = (args?: any): void => {
        if (args.requestType === 'cancel') {
          ...
          expect(gridObj.element.querySelectorAll('form').length).toBe(0);
          expect(
            gridObj.contentModule.vgenerator.cache[7][15].data.FIELD1
          ).not.toBe('updated');
          expect(gridObj.isEdit).toBeTruthy();
        }
      };
    ...
    it('Edit-cancel complete', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'cancel') {
          ...
          expect(gridObj.isEdit).toBeFalsy();
          expect(
            gridObj.contentModule.vgenerator.cache[2][19].data.FIELD1
          ).not.toBe('updated');
          done();
        }
      };
    ...
    it('Add start - 1', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          ...
          expect(gridObj.isEdit).toBeTruthy();
          (
            select('#' + gridObj.element.id + 'FIELD2', gridObj.element) as any
          ).value = 1234567;
          (
            select('#' + gridObj.element.id + 'FIELD1', gridObj.element) as any
          ).value = 'updated';
    ...
    it('Add start - 2', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          expect((<HTMLElement>gridObj.getContent().firstChild).scrollTop).toBe(
            0
          );
          expect(gridObj.element.querySelectorAll('.e-addedrow').length).toBe(
            1
  ...
  describe('EJ2-36215 Script error in empty dropDownList', () => {
    ...
    it('Adding Null values', () => {
      ...
      expect(
        (gridObj.element.querySelector('.e-dropdownlist') as any)
          .ej2_instances[0].dataSource.dataSource.json.length
      ).toBe(0);
      (
        gridObj.element.querySelector('.e-dropdownlist') as any
      ).ej2_instances[0].dataSource.dataSource.json[0] = { ShipCountry: null };
      (
        gridObj.element.querySelector('.e-dropdownlist') as any
    ...
    it('editing row', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'beginEdit') {
          ...
          expect(
            args.form
              .querySelector('#' + gridObj.element.id + 'Verified')
              .nextSibling.classList.contains('e-check')
          ).toBe(false);
          gridObj.actionComplete = undefined;
        }
        done();
  ...
  describe('EJ2-39811 - Need to maintain the edit form actionBegin Event', () => {
    ...
    it('Cancel botton check', (done: Function) => {
      ...
      let cell: HTMLElement = gridObj.element
        .querySelector('.e-content')
        .querySelector('table').rows[0].childNodes[1] as any;
      cell.click();
      gridObj.keyboardModule.keyAction({
        action: 'f2',
        preventDefault: preventDefault,
      ...
      gridObj.element
        .querySelectorAll('.e-valid-input')[1]
        .querySelector('input').innerText = 'JHON';
      let cancel = select(
        '#' + gridObj.element.id + '_cancel',
        gridObj.element
      ) as HTMLElement;
      gridObj.actionBegin = actionBegin;
      cancel.click();
    ...
    it('Check rowData value with editing a row ', (done: Function) => {
      let actionComplete = (args?: any): void => {
        if (args.requestType === 'beginEdit') {
          ...
          expect(args.rowData.CustomerID).toBe(
            gridObj.dataSource[0]['CustomerID']
          );
          done();
        }
      };
  ...
  describe('EJ2-57848 - Inline editing does not work with Drag and drop and frozen columns', () => {
    ...
    it('Edit start', (done: Function) => {
      ...
      (<any>grid.toolbarModule).toolbarClickHandler({
        item: { id: grid.element.id + '_edit' },
      });
    });
    ...
    it('Edit complete', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'save') {
          expect((grid.getCurrentViewRecords()[0] as any).CustomerID).toBe(
            'updated'
          );
          done();
        }
      ...
      (
        select('#' + grid.element.id + 'CustomerID', grid.element) as any
      ).value = 'updated';
      (<any>grid.toolbarModule).toolbarClickHandler({
        item: { id: grid.element.id + '_update' },
      });
    });
    it('EJ2-60088 - Alignment issue while adding a new row with frozen columns with RowDD', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          expect(
            (grid.element.querySelector('.e-normaledit') as any).colSpan
          ).toBe(grid.getVisibleColumns().length + 1);
          done();
        }
      };
      grid.actionComplete = actionComplete;
      (<any>grid.toolbarModule).toolbarClickHandler({
        item: { id: grid.element.id + '_add' },
      });
    });
  ...
  describe('foreignKey column virtualization with inline edit support', () => {
    ...
    it('dropdown input check', (done: Function) => {
      var elem = document.getElementById(
        gridObj.element.id + 'CustomerID_hidden'
      );
...
describe('Editing on ForeignKey column is not working in frozen grid when allowEditOnDblClick is false', () => {
  ...
  it('Edit complete', () => {
    actionComplete = (args?: any): void => {
      if (args.requestType === 'save') {
        expect(
          gridObj.getContent().querySelectorAll('td')[1].innerHTML as any
        ).toBe('Andrew');
      }
    };
    gridObj.actionComplete = actionComplete;
...
describe('EJ2-70349 - Last row gets removed, after adding a new row when page size set to ALL', () => {
  ...
  it('Add start when page size set to ALL', (done: Function) => {
    gridObj.pagerModule.pagerObj.pagerdropdownModule.setDropDownValue(
      'value',
      gridObj.pageSettings.totalRecordsCount
    );
    done();
  });
  ...
  it('Add complete', function () {
    ...
    document
      .querySelector('#' + gridObj.element.id + 'EditConfirm')
      .querySelectorAll('button')[0]
      .click();
  });
...
describe('EJ2-835777 - Custom data source is not  assigned to the dropdown edit cell with Virtualization', () => {
  ...
  it('dropdown input value', (done: Function) => {
    var elem = document.getElementById(
      gridObj.element.id + 'ShipCountry_hidden'
    );
...
describe('EJ2-851524 - Grouped complex data Row Reorder by edit functionalities', () => {
  ...
  it('save complex data row', (done: Function) => {
    (
      select(
        '#' + gridObj.element.id + 'project___description',
        gridObj.element
      ) as any
    ).value += 'updated';
    let actionComplete = (args?: any): void => {
  ...
  it('check not to be reorder', () => {
    expect(
      (gridObj.getRowByIndex(1).previousSibling as HTMLElement).classList[0]
    ).toBe('e-groupcaptionrow');
    expect(
      (gridObj.getRowByIndex(1).nextSibling as HTMLElement).classList[0]
    ).toBe('e-groupcaptionrow');
  });
...
describe('Grouped showgroupedcolumn complex data Row Reorder by edit functionalities', () => {
  ...
  it('save grouped column complex data', (done: Function) => {
    (
      select(
        '#' + gridObj.element.id + 'project___name',
        gridObj.element
      ) as any
    ).value = 'Project C';
    let actionComplete = (args?: any): void => {
  ...
  it('check reordered successfully', () => {
    expect(
      (gridObj.getRowByIndex(2).previousSibling as HTMLElement).classList[0]
    ).toBe('e-row');
    expect(
      (gridObj.getRowByIndex(2).nextSibling as HTMLElement).classList[0]
    ).toBe('e-row');
  });
...
describe('Coverage Improvement', () => {
  ...
  it('Edit complete', (done: Function) => {
    ...
    gridObj.actionBegin = actionBegin;
    (
      select('#' + gridObj.element.id + 'Verified', gridObj.element) as any
    ).value = 'false';
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.element.id + '_update' },
    });
...
describe('Show Add New Row feature coverage - 1', () => {
  ...
  it('check validatoin message', () => {
    ...
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.element.id + 'cancel' },
    });
  });
...
describe('EJ2- 871212 - grid’s edit type datepicker with enable mask for date field not applied', () => {
    ...
    gridObj = createGrid(
      {
        ...
        columns: [
          ...
          },
          {
            ...
            type: 'date',
            width: 170,
            edit: { params: { enableMask: true, format: 'dd/MM/yyyy' } },
            validationRules: { date: [true, 'Enter valid date'] },
          },
          {
            field: 'ShipCountry',
            headerText: 'Ship Country',
  ...
  it('Check the datepicker column has proper masked value or not', () => {
    ...
    expect(
      (
        gridObj.element
          .querySelectorAll('.e-addedrow td')[4]
          .querySelector('.e-datepicker') as HTMLInputElement
      ).value
    ).toBe('day/month/year');
  });
  ...
  describe('EJ2-890600: Issue When numeric editType is set before the primaryKey upon tab navigation  =>', () => {
    ...
    it('In Edit method isWidgetsDestroyed false testing  while calling the currertRowFocus', () => {
      ...
      const curretRowFocusSpy = spyOn(
        editModule,
        'curretRowFocus' as keyof typeof editModule
      ).and.callFake(() => {
  ...
  it('DropDown Boolean Edit', (done: Function) => {
    actionComplete = (args?: any): void => {
      if (args.requestType === 'beginEdit') {
        dropdown = document
          .querySelector('.e-gridform')
          .querySelectorAll('.e-dropdownlist')[0]['ej2_instances'][0];
        done();
      }
    };
    gridObj.actionComplete = actionComplete;
...
describe('code coverage for Add New record navigation', () => {
  ...
  it('closeForm coevarge', () => {
    ...
    (gridObj as any).editModule.editModule.cloneRow =
      gridObj.element.querySelector('.e-normaledit');
    (gridObj as any).editModule.editModule.originalRow =
      gridObj.element.querySelector('.e-row');
...
describe('code coverage for Normal Edit file', () => {
  ...
  it('show add new row with command column  Edit complete coverage', (done: Function) => {
    ...
    gridObj.actionComplete = actionComplete;
    (
      select('#' + gridObj.element.id + 'ShipCountry', gridObj.element) as any
    ).value = 'updated';
    gridObj.keyboardModule.keyAction({
      action: 'enter',
      preventDefault: preventDefault,
...
describe('code coverage - Edit - onPropertyChanged', () => {
  ...
  it('Edit settings property changed case', () => {
    gridObj.editSettings.allowAdding = false;
    gridObj.editModule.onPropertyChanged({
      module: 'edit',
      properties: { allowAdding: false },
    });
  });
...
describe('code coverage - Edit - startEdit, deleteRow, RTL in dialog alert', () => {
  ...
  it('Delete row for no row object', () => {
    const tr: HTMLTableRowElement = gridObj
      .getContentTable()
      .querySelector('tr');
  ...
  it('Curret row focus', () => {
    ...
    (gridObj.editModule as any).curretRowFocus({
      target: contentTable.querySelectorAll('tr')[2],
    });
    const addedRow: HTMLElement = contentTable.querySelector('.e-addedrow');
    const focusableEditCells = addedRow.querySelectorAll(
      '.e-input:not(.e-disabled)'
    );
    parentsUntil(
      focusableEditCells[focusableEditCells.length - 1],
      'e-rowcell'
    ).classList.add('e-unboundcell');
    (gridObj.editModule as any).curretRowFocus({
      target: addedRow,
      action: 'tab',
    });
    (gridObj.editModule as any).curretRowFocus({
      target: focusableEditCells[0],
      action: 'shiftTab',
    });
    gridObj.editSettings.showAddNewRow = true;
    (focusableEditCells[0] as HTMLInputElement).value = '1';
    (gridObj.editModule as any).curretRowFocus({
      target: focusableEditCells[focusableEditCells.length - 1],
      action: 'tab',
    });
  });
...
describe('EJ2-895689: Issue with Grid Column Multiline Edit Params in Chrome and Edge =>', () => {
  ...
  it('Updating the row', function (done) {
    expect((gridObj.getCurrentViewRecords()[1] as any).ShipAddress).toBe(
      'Updated'
    );
    done();
  });
...
describe('EJ2-907682: The focus moves to the previous row on tabbing an editable row =>', () => {
  ...
  it('Edit the row', function (done) {
    (gridObj as any).dblClickHandler({
      target: (gridObj.element.querySelectorAll('.e-row')[3] as any).cells[2],
    });
    (
      select('#' + gridObj.element.id + 'CustomerID', gridObj.element) as any
    ).value = 'updated';
    gridObj.keyboardModule.keyAction({
      action: 'tab',
      preventDefault: preventDefault,
      target: gridObj.element.querySelectorAll('.e-input')[1],
    } as any);
    done();
  });
  ...
  it('Update the row', function (done) {
    ...
    expect(
      (
        gridObj.getContent().querySelectorAll('.e-row')[3] as any
      ).cells[2].classList.contains('e-focused')
    ).toBeTruthy();
    done();
  });
