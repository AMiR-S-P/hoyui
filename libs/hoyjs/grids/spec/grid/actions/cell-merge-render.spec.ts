      ...
      gridObject = createGrid(
        {
          ...
          queryCellInfo: function (args: any) {
            if (
              args.data.EmployeeID == 5 &&
              args.column.field == 'EmployeeID'
            ) {
              args.colSpan = 2;
            }
            if (
              args.data.EmployeeID == 3 &&
              args.column.field == 'EmployeeID'
            ) {
              args.colSpan = 3;
              args.rowSpan = 2;
  ...
  describe('with Column virtualization', () => {
    describe('scroll left continous', () => {
      ...
      it('cell merge check on cell recreate', () => {
        ...
        expect(
          Object.getOwnPropertyNames(grid.mergeCells).length
        ).toBeGreaterThan(0);
        let td = grid
          .getContentTable()
    ...
    it('Rowspan and Colspan with edting', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'beginEdit') {
          expect(
            gridObj.element
              .querySelector('.e-inline-edit')
              .querySelectorAll('.e-rowcell').length
          ).toBe(8);
          done();
        }
      };
