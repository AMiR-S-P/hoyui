...
describe('pdf Export =>', () => {
  ...
  describe('Single Grid Pdf Export =>', () => {
    ...
    it('Pdf grid Check column length', () => {
      let pdfGrid: PdfGrid = (<any>gridObj.pdfExportModule).processGridExport(
        gridObj,
  ...
  describe('Stacked Header Grid Pdf Export =>', () => {
    ...
    it('grid exporting(Check with multiple exporting)', (done) => {
      gridObj
        .pdfExport({ dataSource: data }, true)
    ...
    it('Pdf grid header length', () => {
      ...
      expect((<any>pdfGrid.headers).rows[0].gridCells.cells[1].columnSpan).toBe(
        2
      );
  ...
  describe('Custom font in Grid Pdf Export =>', () => {
    ...
    it('grid exporting(Check with multiple exporting)', (done) => {
      gridObj
        .pdfExport({ dataSource: new DataManager(data) }, true)
        .then((pdfDoc: PdfDocument) => {
          expect(pdfDoc instanceof PdfDocument).toBeTruthy();
          done();
        });
    ...
    it('Pdf grid Check column length', () => {
      ...
      expect((<any>pdfGrid.headers).rows[0].style.font.fontStyle).toBe(
        PdfFontStyle.Italic
      );
      expect((<any>pdfGrid.headers).rows[0].style.font.fontFamily).toBe(
        PdfFontFamily.TimesRoman
      );
      expect((pdfGrid.rows as any).rows[0].style.font.fontFamily).toBe(
        PdfFontFamily.TimesRoman
      );
      expect((pdfGrid.rows as any).rows[0].style.font.fontSize).toBe(12);
      expect((pdfGrid.rows as any).rows[0].style.font.fontStyle).toBe(
        PdfFontStyle.Regular
      );
    });
  ...
  describe('Hierarchy pdf export => ', () => {
    ...
    it('pdf-export -> catch method', (done) => {
      gridObj
        .pdfExport({ footer: { fromBottom: -300 } })
        .then((doc) => {})
        .catch((e) => {
          catchEvent = false;
          done();
  ...
  describe('pdf export with export properties => ', () => {
    ...
    it('export options', (done) => {
      ...
      for (let i = 0; i < pagenumberstyle.length; i++) {
        numberStyle = (gridObj.pdfExportModule as any).getPageNumberStyle(
          pagenumberstyle[i]
        );
      }
  ...
  describe('Detail template pdf export => ', () => {
    ...
    it('export normal table', (done) => {
      gridObj.exportDetailTemplate = (args: ExportDetailTemplateEventArgs) => {
        args.value = {
          columnHeader: [{ cells: [{ value: 'INVOICE' }] }],
          rows: [
            {
              cells: [
                {
  ...
  describe('Lazy load group pdf export', () => {
    ...
    it('Hierarchy export none mode', (done: Function) => {
      gridObj
        .pdfExport({ hierarchyExportMode: 'None' }, true)
        .then((pdfDoc: PdfDocument) => {
          expect(pdfDoc instanceof PdfDocument).toBeTruthy();
          done();
        });
