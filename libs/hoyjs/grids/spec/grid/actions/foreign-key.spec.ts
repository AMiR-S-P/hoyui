...
describe('Foreign Key =>', () => {
    ...
    let options: Object = {
      ...
      columns: [
        ...
        { field: 'ShipCity', width: 120, dataSource: [] },
        {
          field: 'EmployeeID',
          width: 150,
          foreignKeyValue: 'BirthDate',
          dataSource: new DataManager(employeeSelectData),
          format: 'yMd',
        },
        {
          field: 'CustomerID',
  ...
  describe('Default Rendering functions =>', () => {
    it('Initial Rendering in grid.ts', () => {
      expect((<any>gridObj).isForeignKeyEnabled(gridObj.getColumns())).toBe(
        true
      );
    ...
    it('Initial Rendering in foreign key.ts', () => {
      ...
      (<any>gridObj).foreignKeyModule.initForeignKeyColumns(
        gridObj.getColumns()
      );
      expect(column[0].dataSource instanceof DataManager).toBeTruthy();
      expect(column[0].isForeignColumn()).toBeTruthy();

      let args: { column: Column; predicate: { predicate?: Predicate } } = {
        column: column[0],
        predicate: {},
      };
      ...
      let query: Query = (<any>gridObj).foreignKeyModule.genarateQuery(
        column[0],
        column[0].columnData,
        false,
        true
      ...
      expect(
        (<any>gridObj).foreignKeyModule.genarateColumnQuery() instanceof Query
      ).toBeTruthy();

      let object: { column: PredicateModel[]; isTrue: boolean } = (<any>(
        gridObj
      )).foreignKeyModule.isFiltered(column[0]);
      expect(object.isTrue).toBeFalsy();
      expect(object.column.length).toBe(0);
    });
    it('data query generation', () => {
      expect(
        getColumnByForeignKeyValue('BirthDate', gridObj.getForeignKeyColumns())
          .field
      ).toBe('EmployeeID');
    });
  ...
  describe('Sorting with Foreign key =>', () => {
    it('sort a column', (done: Function) => {
      gridObj.actionComplete = () => {
        ...
        expect(
          gridObj.getRowsObject()[0].foreignKeyData['EmployeeID'][0]
        ).not.toBeUndefined();
        done();
      };
  ...
  describe('Filtering with Foreign key =>', () => {
    ...
    it('Check box filtering', (done: Function) => {
      ...
      gridObj.actionComplete = (args) => {
        if (args.requestType === 'filtering') {
          expect(gridObj.filterSettings.columns.length).toBeGreaterThanOrEqual(
            2
          );
          done();
        }
      };
      (<any>gridObj.filterModule).filterDialogOpen(
        gridObj.getForeignKeyColumns()[1],
        gridObj.getHeaderTable().querySelectorAll('.e-filtermenudiv.e-icons')[2]
      );
      (<any>gridObj.filterModule).filterModule.excelFilterBase.initiateFilter(
    ...
    it('Excel Filtering', (done: Function) => {
      ...
      (<any>gridObj.filterModule).filterModule.filterByColumn(
        'EmployeeID',
        'equal',
        new Date(-664743600000),
        'or',
    ...
    it('clear Filtering', () => {
      ...
      !(<any>gridObj.filterModule).filterModule.excelFilterBase.dialogObj
        .isDestroyed && (<any>gridObj.filterModule).filterModule.closeDialog();
    });
  });
  ...
  describe('Grouping with Foreign key', () => {
    ...
    afterAll((done: Function) => {
      gridObj.actionComplete = (args) => {
        if (args.requestType === 'ungrouping') done();
      };
  ...
  describe('Aggregate with Foreign key =>', () => {
    it('Normal Aggregate testing', () => {
      let customAggregateFn: CustomSummaryType = (
        data: Object[],
        ...
        let value: number = data.filter((dObj: Object) => {
          return (
            getValue(
              'City',
              getForeignData(
                gridObj.getColumnByField(column.columnName),
                dObj
              )[0]
            ) === 'Bern'
          );
  ...
  describe('Edit with Foreign key =>', () => {
    ...
    it('Normal Edit', () => {
      ...
      (<any>dropDownObj).obj = {
        fields: { value: (<any>dropDownObj).column.foreignKeyField },
      };
      (<any>dropDownObj).ddActionComplete({
        result: (<any>dropDownObj).column.columnData,
      });
      expect(
        (<DataManager>(<Column>(<any>dropDownObj).column).dataSource).dataSource
          .json.length
      ).toBeGreaterThan(5);
      gridObj.endEdit();
    });
  ...
  describe('Template Column with Foreign key =>', () => {
    it('Template Testing', () => {
      ...
      expect(
        (<HTMLTableRowElement>gridObj.getDataRows()[0]).cells[3].innerText
      ).toBe('Rio de Janeiro');
    });
  });
  ...
  describe('Exporting Grid with Foreign key =>', () => {
    it('Excel Export', (done: Function) => {
      ...
      (<any>gridObj.excelExportModule).helper
        .getColumnData(gridObj)
        .then(() => {
          expect(
            (<any>gridObj.excelExportModule).helper.foreignKeyData['EmployeeID']
          ).not.toBeUndefined();
          done();
        });
    });
  ...
  describe('Util Functions =>', () => {
    it('Refresh Foreign Data', () => {
      ...
      refreshForeignData(
        rowObj,
        gridObj.getForeignKeyColumns(),
        gridObj.getRowsObject()[2].data
      );
      expect(rowObj.foreignKeyData['CustomerID'][0]['City']).not.toBe(a);
      expect(
        rowObj.foreignKeyData['EmployeeID'][0]['BirthDate'].toString()
      ).not.toBeUndefined();
    });

    it('Get Foreign Data', () => {
      expect(
        getForeignData(gridObj.getForeignKeyColumns()[0], {}, 5)[0]['FirstName']
      ).toBe('Steven');
    });
  });
  ...
  describe('Check failed cases =>', () => {
    ...
    it('Data Source getting error', () => {
      ...
      (<any>gridObj).foreignKeyModule.getForeignKeyData({
        column: gridObj.getForeignKeyColumns()[0],
        isComplex: true,
        promise: deferred.promise,
      });
      gridObj.allowPaging = false;
      (<any>gridObj).foreignKeyModule.genarateQuery({
        needQuery: true,
        fromData: false,
      });
  ...
  describe('Custom Data Source =>', () => {
    ...
    it('setting custom data to column', (done: Function) => {
      ((gridObj.columns[1] as Column).dataSource = {
        result: customerData,
  ...
  it('EJ2-38633 - Add params is not sent in the request while on filtering on foreignKey column', (done: Function) => {
    gridObj.actionComplete = (args: any) => {
      if (args.requestType === 'filterchoicerequest') {
        expect(
          (gridObj.filterModule as any).filterModule.excelFilterBase
            .foreignKeyQuery.params.length
        ).toBe(1);
  ...
  it('EJ2-38633 - Add params is not sent in the request while on editing on foreignKey column', (done: Function) => {
    gridObj.actionComplete = (args: any) => {
      if (args.requestType === 'beginEdit') {
        expect(
          args.form.querySelector('.e-dropdownlist').ej2_instances[0].query
            .params.length
        ).toBe(1);
        gridObj.actionComplete = null;
        done();
...
describe('EJ2-53020 - When editing the row throws script error if virtualization is enabled =>', () => {
  let gridObj: Grid;
  let actionComplete: (e?: any) => void;
  beforeAll((done: Function) => {
    let options: Object = {
  ...
  it('Edit start the foreignKeyColumn - ', (done: Function) => {
...
describe('EJ2-867015 - Coverage for foreign key file =>', () => {
  ...
  it('Genarate Column Query', () => {
    (gridObj as any).foreignKeyModule.genarateColumnQuery(
      gridObj.getColumns()[2]
    );
  });
