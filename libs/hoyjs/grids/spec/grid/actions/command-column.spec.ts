 ...
 * Command Column spec
 */
import { EmitType, L10n, select } from '@hoyui/base';
...
describe('Command Column ', () => {
  describe('Command Column Rendering feature', () => {
      ...
      grid = createGrid(
        {
          ...
          dataSource: [
            { data: { a: 1 }, b: 5, c: true, d: new Date() },
    ...
    it('Command Column initial render checking', () => {
      ...
      expect(
        rows
          .querySelector('.e-unboundcelldiv')
          .children[0].classList.contains('e-edit-delete')
      ).toBeTruthy();
      expect(
        rows
          .querySelector('.e-unboundcelldiv')
          .children[1].classList.contains('e-edit-delete')
      ).toBeTruthy();
      expect(
        rows
          .querySelector('.e-unboundcelldiv')
          .children[2].classList.contains('e-save-cancel')
      ).toBeTruthy();
      expect(
        rows
          .querySelector('.e-unboundcelldiv')
          .children[3].classList.contains('e-save-cancel')
      ).toBeTruthy();
      expect(
        rows
          .querySelector('.e-unboundcelldiv')
          .children[2].classList.contains('e-hide')
      ).toBeTruthy();
      expect(
        rows
          .querySelector('.e-unboundcelldiv')
          .children[3].classList.contains('e-hide')
      ).toBeTruthy();
      expect(
        rows
          .querySelector('.e-unboundcelldiv')
          .children[0].classList.contains('e-editbutton')
      ).toBeTruthy();
      expect(
        rows
          .querySelector('.e-unboundcelldiv')
          .children[1].classList.contains('e-deletebutton')
      ).toBeTruthy();
      expect(
        rows
          .querySelector('.e-unboundcelldiv')
          .children[2].classList.contains('e-savebutton')
      ).toBeTruthy();
      expect(
        rows
          .querySelector('.e-unboundcelldiv')
          .children[3].classList.contains('e-cancelbutton')
      ).toBeTruthy();
    });
    ...
    it('Render Command Column with single string', (done: Function) => {
      grid.dataBound = () => {
        ...
        expect(
          rows
            .querySelector('.e-unboundcelldiv')
            .children[0].classList.contains('e-deletebutton')
        ).toBeTruthy();
        done();
      };
      (<Column>grid.columns[0]).commands = [
        { type: 'Delete', buttonOption: { content: 'delete' } },
      ];
      grid.refresh();
    });
    it('Render Command Column with predefined and custom buttons', (done: Function) => {
      let buttonClick = (args: Event) => {
        expect(
          (<HTMLElement>args.target).classList.contains('e-details')
        ).toBeTruthy();
        done();
      };
      grid.dataBound = () => {
        ...
        expect(
          (<HTMLElement>(
            rows.querySelector('.e-unboundcelldiv').children[1]
          )).innerText.toLowerCase()
        ).toBe('details');
        expect(
          (<HTMLElement>(
            rows.querySelector('.e-unboundcelldiv').children[0]
          )).innerText.toLowerCase()
        ).toBe('edit');
  ...
  describe('Command column with Editing enable', () => {
    ...
    it('Editing feature with command edit', (done: Function) => {
        ...
        if (args.requestType === 'beginEdit') {
          expect(
            grid
              .getRows()[0]
              .querySelector('.e-unboundcelldiv')
              .children[0].classList.contains('e-hide')
          ).toBeTruthy();
          (<any>grid).commandColumnModule.commandClickHandler({
            target: grid.getRows()[0].querySelector('.e-unboundcelldiv')
              .children[3],
          });
        }
      ...
      expect(
        grid.getRows()[0].querySelector('.e-unboundcelldiv').children.length
      ).toBe(4);
      (<any>grid).commandColumnModule.commandClickHandler({
        target: grid.getRows()[0].querySelector('.e-unboundcelldiv')
          .children[0],
    ...
    it('Editing feature with edit new row', (done: Function) => {
      grid.actionComplete = (args) => {
        if (args.requestType === 'beginEdit') {
          ...
          (<any>grid).commandColumnModule.commandClickHandler({
            target: grid.getRows()[2].querySelector('.e-unboundcelldiv')
              .children[0],
          });
        }
        if (args.requestType === 'save') {
      ...
      (<any>grid).commandColumnModule.commandClickHandler({
        target: grid.getRows()[4].querySelector('.e-unboundcelldiv')
          .children[0],
      });
    });
    ...
    it('keyboard Testing', (done) => {
      ...
      (<any>grid).commandColumnModule.keyPressHandler({
        action: 'enter',
        target: grid.getRows()[0].querySelector('.e-unboundcelldiv')
          .children[0],
  ...
  describe('Command column with deleting', () => {
    ...
    it('Editing feature with delete command', (done: Function) => {
      ...
      (<any>grid).commandColumnModule.commandClickHandler({
        target: grid
          .getRows()
          [grid.getRows().length - 1].querySelector('.e-unboundcelldiv')
          .children[1],
      });
    });
  ...
  describe('Command column with feature combinations', () => {
    ...
    it('Reordering the command column', (done: Function) => {
      ...
      grid.actionComplete = () => {
        expect((<Column>grid.columns[0]).headerText).toBe(
    ...
    it('Selection with command column', (done: Function) => {
      grid.rowSelected = () => {
        expect(
          grid
            .getRows()[0]
            .children[0].classList.contains('e-selectionbackground')
        ).toBeTruthy();
        done();
      };
      grid.actionComplete = undefined;
      (<HTMLElement>(
        grid.getContent().querySelector('tr').firstElementChild
      )).click();
    });
    ...
    it('CommandColumn with print', (done: Function) => {
      grid.beforePrint = (args: { element: Element; cancel?: boolean }) => {
        ...
        expect(
          row.firstElementChild.classList.contains('e-unboundcell')
        ).toBeTruthy();
        args.cancel = true;
        done();
      };
  ...
  describe('EJ2-7743 ShowConfirmDialog is not showing in Command Column => ', () => {
    ...
    it('Editing feature with delete command', () => {
      ...
      let dlg: any = select(
        '#' + grid.element.id + 'EditConfirm',
        grid.element
      );
      expect(dlg).not.toBeUndefined();
      select('#' + grid.element.id + 'EditConfirm', grid.element)
  ...
  describe('Inline editing custom command => ', function () {
          ...
          columns: [
            ...
            },
            {
              ...
              commands: [
                {
                  type: 'Save',
                  buttonOption: { content: 'done', cssClass: 'e-flat' },
                },
              ],
            },
            { field: 'Freight', width: 120, format: 'C2', textAlign: 'Right' },
            {
              commands: [
                {
                  type: 'Edit',
                  buttonOption: { content: 'Verified', cssClass: 'e-flat' },
                },
              ],
            },
    ...
    it('Custom command column checking', function () {
      ...
      (gridObj as any).dblClickHandler({
        target: gridObj.element.querySelectorAll('.e-row')[1].lastElementChild,
      });
      expect(gridObj.isEdit).toBeTruthy();
      expect(
        rows
          .querySelectorAll('.e-unboundcelldiv')[0]
          .querySelector('.e-btn')
          .getAttribute('title')
      ).toBe('done');
      expect(
        rows
          .querySelectorAll('.e-unboundcelldiv')[1]
          .querySelector('.e-btn')
          .getAttribute('title')
      ).toBe('Verified');
  ...
  describe('EJ2-34867 - Custom command column command button id checking => ', function () {
      ...
      gridObj = createGrid(
        {
          ...
          columns: [
            {
              headerText: 'Command Column 1',
              width: 160,
              commands: [
                { buttonOption: { content: 'ADD', cssClass: 'e-flat' } },
              ],
            },
            {
              headerText: 'Command Column 2',
              width: 160,
              commands: [
                { buttonOption: { content: 'SUB', cssClass: 'e-flat' } },
              ],
            },
            {
              headerText: 'Command Column 3',
              width: 160,
              commands: [
                { buttonOption: { content: 'MUL', cssClass: 'e-flat' } },
              ],
            },
            {
              field: 'OrderID',
    ...
    it('Custom command column command button id checking', function () {
      ...
      let uid3 = gridObj
        .getRows()[0]
        .querySelectorAll('.e-unboundcelldiv')[2]
        .querySelector('.e-btn')
        .getAttribute('data-uid');
  ...
  describe('CommandClickEventArgs receives an incorrect command column title => ', function () {
    ...
    it('Map Command Column Button Click Action and check title', (done: Function) => {
      const mapBtn: HTMLElement = gridObj
        .getRowByIndex(0)
        .querySelector('button.e-flat');
      commandClick = (args?: any): void => {
        expect(args.commandColumn.title).toBe('Map');
        done();
      };
      gridObj.commandClick = commandClick;
      (gridObj as any).commandColumnModule.commandClickHandler({
        target: mapBtn,
      });
    });
  ...
  describe('851553 - Column freeze is not working properly for the command columns', () => {
    ...
    it('checking for frozen command column row and header cells', () => {
      expect(
        (grid.getRows()[0] as any).cells[5].classList.contains('e-rightfreeze')
      ).toBe(true);
      expect(
        grid
          .getHeaderContent()
          .querySelector('tr')
          .cells[5].classList.contains('e-rightfreeze')
      ).toBe(true);
    });
  ...
  describe('command column code coverage => ', () => {
    ...
    it('commandColumnFocusElement', () => {
      ...
      let button: HTMLElement = (
        gridObj.focusModule as any
      ).commandColumnFocusElement(command, true);
      button = (gridObj.focusModule as any).commandColumnFocusElement(
        command,
        false
      );
      command = button = null;
      ...
      gridObj = createGrid(
        {
          ...
          columns: [
...

            {
              headerText: 'Manage Records',
              width: 160,
              commandsTemplate: '<button class="e-edit">Edit</button>',
              commands: [],
            },
            {
              headerText: 'Manage Records',
  ...
  describe('Code coverage for command columns => ', function () {
    ...
    it('commandColumnFocusElement coverage', () => {
      ...
      gridObj.focusModule.currentInfo.skipAction = true;
      (gridObj as any).focusModule.skipOn({
        target: gridObj.element.querySelector('.e-editbutton'),
        action: 'shiftTab',
      });
      (gridObj as any).focusModule.focusOutFromHeader({
        preventDefault: preventDefault,
      });
