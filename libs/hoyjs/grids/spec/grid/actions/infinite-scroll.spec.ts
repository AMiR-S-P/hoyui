...
import { select } from '@hoyui/base';
import {
  infiniteGroupData,
  filterData,
} from '../../../spec/grid/base/datasource.spec';
...
let filterColumn: Function = (
  ...
  let filterElement: any = gridObj.element.querySelector(
    "[id='" + colName + "_filterBarcell']"
  );
  filterElement.value = value;
  filterElement.focus();
  (gridObj.filterModule as any).keyUpHandler(
    getKeyUpObj(keyCode ? keyCode : 13, filterElement)
  );
};

let checkFilterObj: Function = (
  obj: any,
  field?: string,
  operator?: string,
  value?: string,
...
describe('Infinite scroll normal mode => ', () => {
  ...
  it('initial render', () => {
    ...
    expect(
      parseInt(gridObj.getRows()[0].getAttribute('data-rowindex'), 10)
    ).toBe(0);
    ...
    expect(
      Object.keys((gridObj.infiniteScrollModule as any).infiniteCache).length
    ).toBe(0);
    expect(
      Object.keys((gridObj.infiniteScrollModule as any).infiniteCurrentViewData)
        .length
    ).toBe(gridObj.infiniteScrollSettings.initialBlocks);
  });
  ...
  it('scroll bottom', () => {
    ...
    expect(
      parseInt(gridObj.getRows()[150].getAttribute('data-rowindex'), 10)
    ).toBe(150);
    expect(gridObj.getRows().length).toBe(200);
    expect(
      Object.keys((gridObj.infiniteScrollModule as any).infiniteCurrentViewData)
        .length
    ).toBe(gridObj.infiniteScrollSettings.initialBlocks + 1);
  });
  it('Filter string column testing', (done: Function) => {
    actionComplete = (args?: Object): void => {
      expect(
        checkFilterObj(
          gridObj.filterSettings.columns[0],
          'FIELD1',
          'startswith',
          'VINET',
          'and',
...
describe('Infinite scroll cache mode basic scroll => ', () => {
  ...
  it('Ensure page size', function () {
    let visibleRowsCount: number =
      gridObj.infiniteScrollSettings.initialBlocks *
    ...
    expect(
      (gridObj.infiniteScrollModule as any).infiniteCache[4]
    ).toBeUndefined();
    expect(
      (gridObj.infiniteScrollModule as any).infiniteCache[3]
    ).toBeDefined();
  ...
  it('select row', function (done) {
    let rowSelected = function (args: any) {
      expect(gridObj.getSelectedRows()[0].getAttribute('data-rowindex')).toBe(
        '1'
      );
      done();
    };
  ...
  it('ensure rows count after scroll', function () {
    let totalRowsCount: number =
      gridObj.pageSettings.currentPage * gridObj.pageSettings.pageSize;
    let visibleRowsCount: number =
      gridObj.infiniteScrollSettings.initialBlocks *
      gridObj.pageSettings.pageSize;
    let rowElements: Element[] = gridObj.getRows();
  ...
  it('ensure row selection and methods after scroll', function (done: Function) {
    gridObj.on('append-infinite-content', function () {
      ...
      expect(gridObj.getRowByIndex(visibleRowsCount - 1)).toBe(
        gridObj.getRows()[visibleRowsCount - 1]
      );
    });
    setTimeout(done, 400);
  });
...
describe('Infinite scroll cache mode compare with other features => ', () => {
  ...
  it('Reorder Column method testing', (done: Function) => {
    let dataBound = (args: Object): void => {
      ...
      expect(headers[0].querySelector('.e-headercelldiv').textContent).toBe(
        'FIELD2'
      );
      expect(headers[1].querySelector('.e-headercelldiv').textContent).toBe(
        'FIELD3'
      );
      expect(headers[2].querySelector('.e-headercelldiv').textContent).toBe(
        'FIELD1'
      );
...
describe('Infinite scroll cache mode grouping => ', () => {
  ...
  it('Ensure caption rows', (done: Function) => {
    let actionComplete = (args?: Object): void => {
      ...
      expect(
        gridObj.getContent().querySelectorAll('tr:not(.e-row)').length
      ).not.toBe(0);
      expect(gridObj.getRowByIndex(initalRows - 1)).toBe(
...
describe('Infinite scroll cache mode scroll settings property check => ', () => {
  ...
  it('Ensure rows count at initial rendering', function () {
    let initialRowsCount: number =
      gridObj.infiniteScrollSettings.initialBlocks *
      gridObj.pageSettings.pageSize;
    let rowElements: Element[] = gridObj.getRows();
    expect(rowElements.length).toBe(initialRowsCount);
    expect(
      (gridObj.infiniteScrollModule as any).infiniteCache[5]
    ).toBeDefined();
    expect(
      (gridObj.infiniteScrollModule as any).infiniteCache[6]
    ).toBeUndefined();
  });
  ...
  it('Ensure rows count after first scroll', function () {
    ...
    expect(rowElements.length).toBe(
      initialRowsCount + gridObj.pageSettings.pageSize
    );
    expect(rowElements[0].getAttribute('data-rowindex')).toBe('0');
    expect(
      parseInt(
        rowElements[rowElements.length - 1].getAttribute('data-rowindex')
      )
    ).toBe(value);
    expect(
      (gridObj.infiniteScrollModule as any).infiniteCache[6]
    ...
    expect(
      (gridObj.infiniteScrollModule as any).infiniteCache[7]
    ).toBeUndefined();
  });
  it('Ensure rows count at initial rendering', function (done) {
  ...
  it('Ensure rows count after second scroll', function () {
    let initialRowsCount: number =
      gridObj.infiniteScrollSettings.maxBlocks * gridObj.pageSettings.pageSize;
    let rowElements: Element[] = gridObj.getRows();
    expect(
      rowElements[rowElements.length - 1].getAttribute('data-rowindex')
    ).toBe('349');
...
describe('Infinite scroll cache mode scroll max block property => ', () => {
  ...
  it('Ensure rows count at initial rendering', function () {
    let initialRowsCount: number =
      ...
      gridObj.pageSettings.pageSize;
    let rowElements: Element[] = gridObj.getRows();
    expect(gridObj.infiniteScrollSettings.initialBlocks).toBe(
      gridObj.infiniteScrollSettings.maxBlocks
    );
    expect(rowElements.length).toBe(initialRowsCount);
  });
  it('scroll to bottom', function (done) {
  ...
  it('Ensure rows count after scroll', function () {
    ...
    let value: number = initialRowsCount + gridObj.pageSettings.pageSize - 1;
    expect(rowElements.length).toBe(initialRowsCount);
    expect(parseInt(rowElements[0].getAttribute('data-rowindex'))).toBe(
      gridObj.pageSettings.pageSize
    );
    expect(
      parseInt(
...
describe('Infinite scroll with frozen columns => ', () => {
  ...
  it('ensure intinial rendering rows count', () => {
    ...
    expect((gridObj as any).contentModule.rowElements.length).toBe(
      visibleRowsCount
    );
  });
...
describe('Infinite scroll with frozen rows => ', () => {
  ...
  it('ensure intinial rendering rows count', () => {
    ...
    let rows: Element[] = [].slice.call(
      gridObj.getContent().querySelectorAll('.e-row')
    );
    let frozenRows: Element[] = [].slice.call(
      gridObj.getHeaderContent().querySelectorAll('.e-row')
    );
...
describe('Infinite scroll cache mode with frozen columns => ', () => {
  ...
  it('ensure rows count after scroll', () => {
    ...
    expect(gridObj.getRows()[0].getAttribute('data-rowindex')).toBe(
      gridObj.pageSettings.pageSize.toString()
    );
    // EJ2-850288 - Performance Improvement -  Dynamically changing grid height with infinite scrolling
    expect(
      (gridObj as any).infiniteScrollModule.infiniteCache[2].length
    ).toBeTruthy();
  });
...
describe('Infinite scroll cache mode with frozen rows => ', () => {
  ...
  it('ensure intinial rendering rows count', () => {
    let visibleRowsCount: number =
      gridObj.infiniteScrollSettings.initialBlocks *
      gridObj.pageSettings.pageSize;
    let contentRows: Element[] = [].slice.call(
      gridObj.getContent().querySelectorAll('.e-row')
    );
    let hdrRows: Element[] = [].slice.call(
      gridObj.getHeaderContent().querySelectorAll('.e-row')
    );
    ...
    expect(contentRows[0].getAttribute('data-rowindex')).toBe(
      gridObj.frozenRows.toString()
    );
    expect(
      contentRows[contentRows.length - 1].getAttribute('data-rowindex')
    ).toBe((visibleRowsCount - 1).toString());
  ...
  it('ensure rows count after scroll', () => {
    ...
    expect(hdrRows.length).toBe(gridObj.frozenRows);
    expect(gridObj.getRows().length).toBe(
      visibleRowsCount + gridObj.frozenRows
    );
    expect((gridObj as any).contentModule.rowElements.length).toBe(
      visibleRowsCount + gridObj.frozenRows
    );
    ...
    expect(
      contentRows[contentRows.length - 1].getAttribute('data-rowindex')
    ).toBe((visibleRowsCount + gridObj.pageSettings.pageSize - 1).toString());
  });
...
describe('EJ2-40801 - Infinite scrolling is not working properly while enabling editSettings => ', () => {
  ...
  it('check no records to display issue  while scroll', function () {
    ...
    expect(gridObj.pageSettings.currentPage).toBe(
      gridObj.infiniteScrollSettings.initialBlocks
    );
  });
...
describe('Infinite scroll normal mode with edit feature teating => ', () => {
  ...
  it('collect row data before editing', function (done: Function) {
    ...
    let rowSelected = function (args: RowSelectEventArgs) {
      expect(
        gridObj.getSelectedRecords()[0][(gridObj.columns[1] as Column).field]
      ).toBe(data1[(gridObj.columns[1] as Column).field]);
      gridObj.rowSelected = null;
      done();
    };
    gridObj.rowSelected = rowSelected;
  ...
  it('start edit', function (done: Function) {
    let actionComplete = function (args: NotifyArgs) {
      if (args.requestType === 'beginEdit') {
        expect((gridObj.editModule as any).editModule.editRowIndex).toBe(
          rowindex
        );
        ...
        expect(document.activeElement.id).toBe(
          gridObj.element.id + (gridObj.columns[rowindex] as Column).field
        );
  ...
  it('Ensure grid edit form after scroll', function (done: Function) {
    ...
    expect(
      Object.keys((gridObj as any).infiniteScrollModule.virtualInfiniteData)
        .length
    ).toBe(0);
    let actionComplete = function (args: NotifyArgs) {
      if (args.requestType === 'save') {
        expect(
          gridObj.dataSource[rowindex][
            (gridObj.columns[rowindex] as Column).field
          ]
        ).toBe('updated');
        gridObj.actionComplete = null;
        done();
...
describe('Infinite scroll normal mode with edit feature teating => ', () => {
  let gridObj: Grid;
  let dataLength = virtualData.length;
  let data1: number;
  beforeAll((done: Function) => {
    gridObj = createGrid(
      {
  ...
  it('start add', function (done: Function) {
    ...
    let actionComplete = function (args: NotifyArgs) {
      if (args.requestType === 'add') {
        ...
        //focus check
        expect(document.activeElement.id).toBe(
          gridObj.element.id + (gridObj.columns[0] as Column).field
        );
        //toolbar status check
        expect(gridObj.element.querySelectorAll('.e-overlay').length).toBe(4);
        expect(gridObj.isEdit).toBeTruthy();
  ...
  it('Save add', function (done: Function) {
    let actionComplete = function (args: NotifyArgs) {
      if (args.requestType === 'save') {
        expect(
          gridObj.dataSource[0][(gridObj.columns[0] as Column).field]
        ).toBe(897654);
        expect(
          gridObj.dataSource[0][(gridObj.columns[1] as Column).field]
        ).toBe('updated');
        expect((gridObj.dataSource as any).length).toBe(dataLength + 1);
        expect(gridObj.isEdit).toBeFalsy();
        expect(
          gridObj.dataSource[199][(gridObj.columns[0] as Column).field]
        ).not.toBe(data1);
        expect(
          gridObj.dataSource[199][(gridObj.columns[0] as Column).field]
        ).toBe(data1 - 1);
        expect(
          (gridObj.element.querySelector('.e-row') as any).getAttribute(
            'data-rowindex'
          )
        ).toBe('0');
        gridObj.actionComplete = null;
    ...
    (
      select(
        '#' + gridObj.element.id + (gridObj.columns[1] as Column).field,
        gridObj.element
      ) as any
    ).value = 'updated';
...
describe('Infinite scroll normal mode with edit feature teating => ', () => {
  ...
  it('select row for delete', function (done: Function) {
    let rowSelected = function (args: RowSelectEventArgs) {
      ...
      data1 =
        gridObj.dataSource[rowIndex][(gridObj.columns[0] as Column).field];
  ...
  it('delete record', function (done: Function) {
    let actionComplete = function (args: NotifyArgs) {
      if (args.requestType === 'delete') {
        expect((gridObj.dataSource as any).length).toBe(dataLength - 1);
        expect((gridObj.element.querySelectorAll('.e-row') as any).length).toBe(
          rowsCount
        );
        expect(
          (
            gridObj.element.querySelectorAll('.e-row')[rowIndex] as any
          ).getAttribute('data-rowindex')
        ).toBe(rowIndex.toString());
        expect(
          gridObj.dataSource[rowIndex][(gridObj.columns[0] as Column).field]
        ).toBe(data1 + 1);
        expect(
        ...
        expect(gridObj.getContent().firstElementChild.scrollTop).toBe(
          scrollTop
        );
        gridObj.actionComplete = null;
        done();
    ...
    let actionComplete = function (args: NotifyArgs) {
      if (args.requestType === 'beginEdit') {
        ...
        expect(
          ...
          gridObj.dataSource[rowindex][(gridObj.columns[1] as Column).field]
        );
        (
          select(
            '#' + gridObj.element.id + (gridObj.columns[1] as Column).field,
...
describe('EJ2-42591-Infinite scroll with grouping => ', () => {
  ...
  beforeAll((done: Function) => {
    gridObj = createGrid(
      {
        dataSource: [
          { FIELD2: '10001', FIELD1: 'Test' },
          { FIELD2: '10002', FIELD1: 'Test' },
        ],
        enableInfiniteScrolling: true,
        allowGrouping: true,
        height: 400,
  ...
  it('Grouping the FIELD1 column', (done: Function) => {
    actionComplete = function (args) {
      expect(gridObj.element.querySelectorAll('.e-groupcaption').length).toBe(
        1
      );
      gridObj.actionComplete = null;
      done();
...
describe('Infinite scroll with grouping collapse testing => ', () => {
  ...
  it('Initial check =>', () => {
    ...
    expect(
      gridObj.getContent().querySelectorAll('.e-groupcaption').length
    ).toBe(1);
    expect(gridObj.getContent().querySelectorAll('.e-summaryrow').length).toBe(
      0
    );
  });
  ...
  it('Collapse caption =>', (done: Function) => {
    ...
    const dataBound = () => {
      ...
      if (triggerCount === 2) {
        ...
        expect(
          gridObj.getContent().querySelectorAll('.e-summaryrow').length
        ).toBe(1);
        const skip: number = parseInt(
          gridObj
            .getRows()
            [gridObj.getRows().length - 1].getAttribute('data-rowindex'),
          10
        );
        ...
        expect((gridObj.infiniteScrollModule as any).groupCaptionAction).toBe(
          'refresh'
        );
  ...
  it('Complex caption collapse =>', (done: Function) => {
    const dataBound = () => {
      ...
      expect(skip).toBe(
        gridObj.pageSettings.pageSize *
          (gridObj.infiniteScrollSettings.initialBlocks * 2)
      );
      gridObj.dataBound = undefined;
      done();
    };
    gridObj.dataBound = dataBound;
    gridObj.groupModule.expandCollapseRows(
      gridObj.getContent().querySelectorAll('.e-recordplusexpand')[2]
    );
  });
...
describe('EJ2-0000 - Need to maintain scroll position with the updateRow method => ', () => {
            ...
            commands: [
              {
                buttonOption: {
                  content: 'View',
                  cssClass: 'command-button chi-button -sm',
                },
              },
            ],
          },
  ...
  it('called updateRow method', () => {
    expect(
      gridObj.getRows()[0].querySelector('.e-unboundcelldiv').children.length
    ).toBe(1);
  });
...
describe('EJ2-63081 - Group by fails for 1 record on collapse => ', () => {
  ...
  it('Scroll bottom', (done: Function) => {
    gridObj.getContent().firstElementChild.scrollTop =
      gridObj.getContent().firstElementChild.scrollHeight;
    setTimeout(done, 200);
  });
  ...
  it('Collapse Last Caption Row and check', (done: Function) => {
    ...
    expect(gridObj.pageSettings.currentPage).toBe(
      (gridObj.infiniteScrollModule as any).maxPage
    );
    setTimeout(done, 200);
  });
  afterAll(() => {
    ...
    gridObj = createGrid(
      {
        ...
        toolbar: ['Add', 'Edit', 'Delete', 'Update', 'Cancel'],
        height: 400,
        columns: [{ field: 'fieldName', defaultValue: 'Demo Text' }],
      },
      () => {
        setTimeout(done, 200);
      }
...
describe('EJ2-72231- Cannot edit newly added row when infiniteScrolling enabled => ', () => {
  ...
  it('Update the row in grid', function () {
    (select('#' + gridObj.element.id + 'City', gridObj.element) as any).value =
      'New updated';
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.element.id + '_update' },
    });
...
describe('Infinite Scroll file code coverage - 2', () => {
  ...
  it('infinite scroll file method coverage - 1', () => {
    (gridObj as any).infiniteScrollModule.captionActionComplete({
      isCollapse: false,
    });
    ...
    (gridObj as any).infiniteScrollModule.actionComplete({
      requestType: 'cancel',
    });
    (gridObj as any).infiniteScrollModule.resetInfiniteEdit();
    (gridObj as any).infiniteScrollModule.infiniteEditHandler({
      e: { requestType: 'edit' },
    });
  ...
  it('coverage for infinite scroll navigation', () => {
    let cells: NodeListOf<Element> = gridObj
      .getContent()
      .querySelectorAll('.e-groupcaption');
  ...
  it('infinite scroll file method coverage - 1', () => {
    ...
    (gridObj as any).infiniteScrollModule.ensureRowAvailability(
      gridObj.getRowsObject()[0],
      gridObj.getRowsObject()[0].data
    );
    gridObj.infiniteScrollSettings.enableCache = true;
  });

  it('infinite scroll file method coverage - 3', () => {
    (gridObj as any).infiniteScrollModule.setDisplayNone({
      index: 1,
      vissible: true,
    });
    (gridObj as any).infiniteScrollModule.infiniteCache['1'][1].visible = false;
    (gridObj as any).infiniteScrollModule.refreshInfiniteCacheRowVisibleLength(
      (gridObj as any).infiniteScrollModule.infiniteCache,
      1
    );
    gridObj.infiniteScrollSettings.enableCache = false;
    (gridObj as any).infiniteScrollModule.refreshInfiniteCacheRowVisibleLength(
...
describe('EJ2-907977 - Focus is lost when enableColumnVirtualization is enabled with Freeze columns - coverage 1 => ', () => {
  ...
  it('focus on the cell element - 1', () => {
    (
      gridObj.getRows()[2].querySelectorAll('.e-rowcell')[2] as HTMLElement
    ).click();
  });
