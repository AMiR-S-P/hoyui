 ...
 */
import { EmitType, select, isNullOrUndefined } from '@hoyui/base';
...
describe('ShowHide module testing', () => {
  ...
  describe('Hide column at initial rendering', () => {
    ...
    it('check TH/TD visiblity', () => {
      rows = (grid.getHeaderTable() as any).tHead
        .rows[0] as HTMLTableRowElement;
      expect(rows.cells[0].classList.contains('e-hide')).toBeTruthy();
      rows = ((grid.getContentTable() as any).tBodies[0] as HTMLTableElement)
        .rows[0] as HTMLTableRowElement;
      expect(rows.cells[0].classList.contains('e-hide')).toBeTruthy();
    });
  ...
  describe('Show Column using headerText', () => {
    ...
    it('check colgroup->col visiblity', () => {
      let col: HTMLTableColElement = <HTMLTableColElement>(
        (<HTMLTableElement>grid.getHeaderTable()).children[0].children[0]
      );
      expect(col.style.display).toBe('');
      col = <HTMLTableColElement>(
        (<HTMLTableElement>grid.getContentTable()).children[0].children[0]
      );
      expect(col.style.display).toBe('');
    });
  ...
  describe('SetVisible function', () => {
    ...
    it('check colgroup->col visiblity', () => {
      ...
      expect(col.style.display).toBe('');
      col = <HTMLTableColElement>(
        (<HTMLTableElement>grid.getHeaderTable()).children[0].children[1]
      );
      expect(col.style.display).toBe('none');
      col = <HTMLTableColElement>(
        (<HTMLTableElement>grid.getContentTable()).children[0].children[1]
      );
      expect(col.style.display).toBe('none');
    });
  ...
  describe('show/hide with filtering', () => {
    ...
    it('render grid with filter enable', () => {
      expect(
        grid.getHeaderContent().querySelectorAll('.e-filterbarcell.e-hide')
          .length
      ).toBe(
        grid.getHeaderContent().querySelectorAll('.e-headercell.e-hide').length
      );
    });
    it('hide a column with filter enabled', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          grid.getHeaderContent().querySelectorAll('.e-headercell.e-hide')
            .length
        ).toBe(2);
        expect(
          grid.getHeaderContent().querySelectorAll('.e-filterbarcell.e-hide')
        ...
        expect(
          grid
            .getContent()
            .querySelectorAll('.e-rowcell.e-hide.e-gridchkbox-cell').length
        ).toBe(grid.currentViewData.length);
        expect(
          (grid.getContent().querySelectorAll('.e-rowcell')[1] as HTMLElement)
            .style.display
        ).toBe('none');
        done();
      }, 1000);
    ...
    it('show hidden columns', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          grid.getContent().querySelectorAll('.e-rowcell.e-hide').length
        ).toBe(0);
        expect(
          (grid.getContent().querySelectorAll('.e-rowcell')[0] as HTMLElement)
            .style.display
        ).toBe('');
        expect(
          (grid.getContent().querySelectorAll('.e-rowcell')[1] as HTMLElement)
  ...
  describe('Show / Hide Test with Freeze pane', () => {
    ...
    it('Hide column at initial rendering', () => {
      rows = (gridObj.getHeaderTable() as any).tHead
        .rows[0] as HTMLTableRowElement;
      expect(rows.cells[0].classList.contains('e-hide')).toBeTruthy();
      rows = ((gridObj.getContentTable() as any).tBodies[0] as HTMLTableElement)
        .rows[0] as HTMLTableRowElement;
      expect(rows.cells[0].classList.contains('e-hide')).toBeTruthy();
      let col: HTMLTableColElement = <HTMLTableColElement>(
        (<HTMLTableElement>gridObj.getHeaderTable()).children[0].children[0]
      );
      expect(col.style.display).toBe('none');
      col = <HTMLTableColElement>(
        (<HTMLTableElement>gridObj.getContentTable()).children[0].children[0]
      );
      expect(col.style.display).toBe('none');
    });
    ...
    it('Hide Column using field', () => {
      ...
      let col: HTMLTableColElement = <HTMLTableColElement>(
        (<HTMLTableElement>gridObj.getHeaderTable()).children[0].children[2]
      );
      expect(col.style.display).toBe('none');
      col = <HTMLTableColElement>(
        (<HTMLTableElement>gridObj.getContentTable()).children[0].children[2]
      );
      expect(col.style.display).toBe('none');
    });
    ...
    it('SetVisible function', () => {
      ...
      expect(col.style.display).toBe('');
      col = <HTMLTableColElement>(
        (<HTMLTableElement>gridObj.getHeaderTable()).children[0].children[1]
      );
      expect(col.style.display).toBe('none');
      col = <HTMLTableColElement>(
        (<HTMLTableElement>gridObj.getContentTable()).children[0].children[1]
      );
      expect(col.style.display).toBe('none');
    });
    afterAll(() => {
  ...
  describe('EJ2-26051===>script error throws when invoke hidecolumns with empty data', () => {
    ...
    it('checking after invoking the hide column method(with empty datasource)', () => {
      ...
      expect(
        gridObj.getContent().querySelectorAll('.e-rowcell.e-hide').length
    ...
    it('Check form is ceated', (done: Function) => {
      gridObj.actionComplete = (e) => {
        if (e.requestType === 'beginedit') {
          expect(isNullOrUndefined(document.querySelector('.e-gridform'))).toBe(
            false
          );
        }
        done();
  ...
  describe('Frozen Revamp feature', () => {
    ...
    it('Hide a Freeze Left Columns', () => {
      ...
      let rows: HTMLTableRowElement = (gridObj.getHeaderTable() as any).tHead
        .rows[0] as HTMLTableRowElement;
      expect(rows.cells[1].classList.contains('e-hide')).toBeTruthy();
    });
