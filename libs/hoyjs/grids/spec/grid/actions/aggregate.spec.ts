 ...
 * Data spec
 */
import { createElement, selectAll, select } from '@hoyui/base';
...
import { DataManager, RemoteSaveAdaptor } from '@hoyui/data';
...
describe('Aggregates Functionality testing', () => {
  describe('Default functionality', () => {
    ...
    it('check summary value', () => {
      let rows = (grid.getFooterContentTable() as HTMLTableElement).tFoot
        .rows[0] as HTMLTableRowElement;
      expect(rows.cells[2].innerHTML).toBe(
        '$' + DataUtil.aggregates.average(grid.dataSource, 'Freight').toFixed(2)
      );
    });
    afterAll(() => {
      destroy(grid);
  ...
  describe('Default functionality - Empty DataSource', () => {
    ...
    it('check content and table visiblity', () => {
      expect(
        grid.getFooterContentTable().querySelector('.e-summarycell')
      ).toBeNull();
    });
    afterAll(() => {
      destroy(grid);
  ...
  describe('Group and caption summary functionality enabled', () => {
    ...
    it('Footer disabled check', () => {
      expect(
        (grid.getFooterContentTable() as HTMLTableElement).tFoot.rows.length
      ).toBe(0);
    });
    it('check caption summary value', () => {
      let rows: HTMLTableRowElement = <any>(
        (grid.getContentTable() as HTMLTableElement).querySelector(
          '.e-summarycell.e-templatecell'
        )
      );
      let val: string | Object = grid.valueFormatterService.toView(
        DataUtil.aggregates.max(
          (<{ items: Object[] }>grid.currentViewData[0]).items,
          'OrderDate'
        ),
  ...
  describe('Custom summary functionality', () => {
    let grid: Grid;
    let rows: HTMLTableRowElement;
    let customSum: CustomSummaryType = (data: Object[], column: Object) =>
      'Best Employee: 1';
    beforeAll((done: Function) => {
      grid = createGrid(
        {
    ...
    it('check custom footer summary', () => {
      let rows: HTMLTableRowElement = <any>(
        (grid.getFooterContentTable() as HTMLTableElement).tFoot.rows[0]
      );
  ...
  describe('Fisrt column with caption aggreate function template', () => {
    ...
    let customSum: CustomSummaryType = (data: Object[], column: Object) =>
      'Best Employee: 1';
    let groupCaption: Function = (data: Object[], column: Object) =>
      'Best Employee: 1';
    beforeAll((done: Function) => {
      grid = createGrid(
        {
  ...
  describe('Aggregation with detailTemplate', () => {
    ...
    it('check colgroup length', () => {
      expect(
        grid.getFooterContentTable().querySelectorAll('colgroup col').length
      ).toBe(grid.getColumns().length + 1);
    });
    afterAll(() => {
      destroy(grid);
      grid = rows = null;
  ...
  describe('Reactive aggregates in footer with batch editing', () => {
      ...
      'result' in data
        ? 'records' in data.result
          ? data.result.records.filter(
              (item: Object) => item['CustomerID'] === 'HANAR'
            ).length
          : data.result.filter((item: Object) => item['CustomerID'] === 'HANAR')
              .length
        : 'items' in data
        ? data.items.filter((item: Object) => item['CustomerID'] === 'HANAR')
            .length
        : data.filter((item: Object) => item['CustomerID'] === 'HANAR').length;
    beforeAll((done: Function) => {
      grid = createGrid(
        {
          editSettings: {
    ...
    it('tab key to update aggregates in footer ', (done: Function) => {
      ...
      let AggregateRow = (grid.getFooterContentTable() as HTMLTableElement)
        .tFoot.rows[0] as HTMLTableRowElement;
    ...
    it('tab key to update custom aggregates in footer', (done: Function) => {
      let AggregateRow: HTMLTableRowElement = (
        grid.getFooterContentTable() as HTMLTableElement
      ).tFoot.rows[0] as HTMLTableRowElement;
      let afterSumVal: string = AggregateRow.cells[2].innerHTML;
      let isUpdateSummary: boolean =
        updateAggregatesVal !== afterSumVal ? true : false;
      ...
      grid.element
        .querySelector('.e-editedbatchcell')
        .querySelector('input').value = 'HANAR';
      grid.keyboardModule.keyAction({
        action: 'tab',
        preventDefault: preventDefault,
        target: cell,
    ...
    it('Update aggregates in footer using enter key', (done: Function) => {
      let CustomAggregateRow: HTMLTableRowElement = (
        grid.getFooterContentTable() as HTMLTableElement
      ).tFoot.rows[1] as HTMLTableRowElement;
    ...
    it('cancel the bulk edited value ', (done: Function) => {
      ...
      selectAll(
        '#' + grid.element.id + 'EditConfirm button',
        grid.element
      )[0].click();
    });
  ...
  describe('Aggregate in inline edit mode', () => {
    ...
    it('update the group aggregates using tab key press', (done: Function) => {
      ...
      (
        select('#' + grid.element.id + 'Freight', grid.element) as any
      ).value = 100;
      grid.keyboardModule.keyAction({
        action: 'enter',
        preventDefault: preventDefault,
        target: grid.getContent().querySelector('.e-row'),
      } as any);
      done();
    });
    ...
    it('Reactive aggregate with remote save adaptor ', () => {
      expect(
        (
          grid
            .getFooterContent()
            .querySelector('.e-templatecell') as HTMLElement
        ).innerText
      ).toBe('Sum: $696.45');
      grid.editModule.editCell(0, 'Freight');
  ...
  describe('Frozen columns with aggregates', () => {
    ...
    it('Footer content  scroll check', () => {
      ...
      expect(grid.element.querySelector('.e-summarycontent').scrollLeft).toBe(
        left
      );
    });
  ...
  describe('Reactive aggregates in footer with batch editing and frozen columns', () => {
    ...
    it('Add a new row with data in footer', (done: Function) => {
      expect(
        (grid.aggregateModule as any).footerRenderer.aggregates.aggregates[
          'Freight - sum'
        ].toFixed(2)
      ).toBe('696.45');
    ...
    it('checking index and mapping uid of the column', () => {
      ...
      expect(
        grid
          .getFooterContent()
          .querySelector('.e-summarycell')
          .hasAttribute('e-mappinguid')
      ).toBeTruthy();
    });

    it('Remove padding when using hideScroll in Footer content', () => {
      expect(
        grid.getFooterContent().classList.contains('e-footerpadding')
      ).toBeTruthy();
      grid.hideScroll();
      expect(
        grid.getFooterContent().classList.contains('e-footerpadding')
  ...
  describe('EJ2-35825-Aggregates with empty Grid batch adding', () => {
    ...
    it('cancel the added value ', (done: Function) => {
      grid.editSettings.showConfirmDialog = false;
      grid.closeEdit();
      let AggregateRow: HTMLTableSectionElement = (
        grid.getFooterContentTable() as HTMLTableElement
      ).tFoot;
  ...
  describe('EJ2-37046 when combining Aggregate, Resize and Freeze and calling autoFitColumns() on the same Grid throws script error', () => {
    ...
    it('Query selector check', (done: Function) => {
      ...
      actionComplete = (args: any): void => {
        expect(
          gridObj.getFooterContent().querySelector('.e-summarycontent')
            .children[0] as any
        ).toBeDefined();
        done();
      };
  ...
  describe('EJ2-39551 - Show column names for aria-label in the summary row', () => {
    ...
    it('check aria-label summary-cell', () => {
      let rows = (grid.getFooterContentTable() as HTMLTableElement).tFoot
        .rows[0] as HTMLTableRowElement;
      expect(
        rows.cells[0].getAttribute('aria-label').toString().indexOf('Order ID')
      ).toBeGreaterThan(0);
      expect(
        rows.cells[1].getAttribute('aria-label').toString().indexOf('Freight')
      ).toBeGreaterThan(0);
      expect(
        rows.cells[2]
  ...
  describe('EJ2-57936 - Aggregates not shown when group expanded after collapsing using `groupCollapseAll` method', () => {
    ...
    it('collapseAll method', () => {
      let rows: any = grid
        .getRowsObject()
        .filter((row: any) => row.isAggregateRow);
    ...
    it('checking the summary row', () => {
      ...
      expect(
        (grid.getContentTable().querySelector('.e-summaryrow') as HTMLElement)
          .style.display
      ).toBe('');
    });
    afterAll(() => {
  ...
  describe('Code coverage improvement', () => {
    ...
    it('check the addEventListener Binding', () => {
      ...
      expect((<AggregateColumn>grid.aggregates[0].columns[0]).format).toBe(
        'c2'
      );
    });
    afterAll(() => {
  ...
  describe('EJ2-867939: Grid aggregate value does not update properly after filter and edit', () => {
    ...
    it('Edit the Freight data', () => {
      (grid as any)
        ...
        .querySelectorAll('input')[1].value = 2;
      grid.endEdit();
    });
    it('Check the aggregate data', () => {
      expect(
        (grid as any).getFooterContentTable().querySelector('.e-templatecell')
          .innerText
      ).toBe('Sum: $2.00');
    });
    afterAll(() => {
  ...
  describe('EJ2-884166: Need to show Aggregate row when Grid is empty', () => {
    ...
    it('check summary cell visibility visiblity', () => {
      expect(
        grid.getFooterContentTable().querySelectorAll('.e-summarycell').length
      ).toBeGreaterThan(0);
    });
    afterAll(() => {
      destroy(grid);
  ...
  describe('EJ2-898870: Editing a record of column that contains multiple aggregates throws a script error.', () => {
    ...
    it('Check the aggregate data', () => {
      expect(
        (grid as any).getFooterContentTable().querySelector('.e-templatecell')
          .innerText
      ).toBe('Sum: $3.00, Min:$1.00, Max:$2.00');
    });
    afterAll(() => {
      destroy(grid);
      grid = null;
