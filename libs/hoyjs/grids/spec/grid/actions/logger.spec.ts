...
describe('Logger module', () => {
  ...
  let primary_column_missing: ItemDetails =
    detailLists['primary_column_missing'];
  ...
  let action_disabled_column: ItemDetails =
    detailLists['action_disabled_column'];
  let limitation: ItemDetails = detailLists['limitation'];
  let check_datasource_columns: ItemDetails =
    detailLists['check_datasource_columns'];
  ...
  let datasource_syntax_mismatch: ItemDetails =
    detailLists['datasource_syntax_mismatch'];
  ...
  describe('Logger render => ', () => {
    ...
    it('Chceck action_disabled_column test', () => {
      ...
      expect(
        action_disabled_column.check(
          { moduleName: 'group', columnName: 'CustomerID' },
          gridObj
        ).success
      ).toBeTruthy();
      expect(
        action_disabled_column.check(
          { moduleName: 'reorder', column: 'CustomerID' },
          gridObj
        ).success
      ).toBeTruthy();
      expect(
      ...
      expect(
        action_disabled_column.check(
          { moduleName: 'filter', columnName: 'CustomerID' },
          gridObj
        ).success
      ).toBeTruthy();
      expect(
        action_disabled_column.check(
          { moduleName: 'sort', columnName: 'CustomerID' },
          gridObj
        ).success
      ).toBeTruthy();
    });
    ...
    it('Check actionfailure test', () => {
      expect(
        actionfailure.generateMessage({ error: 'Format options' }, gridObj)
      ).not.toBeNull();
      expect(
        actionfailure.generateMessage({ error: 'Format' }, gridObj)
      ).not.toBeNull();
      expect(
        actionfailure.generateMessage({ error: { error: {} } }, gridObj)
      ).not.toBeNull();
    ...
    it('Check limitation test', () => {
      expect(
        limitation.generateMessage({ error: {} }, gridObj, { name: 'freeze' })
      ).not.toBeNull();
      expect(
        limitation.generateMessage({ error: {} }, gridObj, {
          name: 'virtualization',
        })
      ).not.toBeNull();
      expect(
        limitation.generateMessage({ name: 'scroll' }, gridObj, {
          name: 'scroll',
        })
      ).not.toBeNull();
    ...
    it('Check exporting_complete test', () => {
      ...
      expect(
        exporting_complete.generateMessage({}, gridObj, {})
      ).not.toBeNull();
    });
    ...
    it('Check grid_remote_edit test', () => {
      expect(
        grid_remote_edit.check({ result: [] }, gridObj).success
      ).toBeTruthy();
    ...
    it('Chceck initial_action test', () => {
      ...
      expect(
        initial_action.check(
          { moduleName: 'sort', column: 'CustomerID' },
          gridObj
        ).success
      ).toBeTruthy();
      expect(
        initial_action.check(
          { moduleName: 'filter', column: 'CustomerID', destColumn: 'OrderID' },
          gridObj
        ).success
      ).toBeTruthy();

      expect(
        initial_action.generateMessage({}, gridObj, { fn: 'checking' })
      ).not.toBeNull();
    });
    ...
    it('Chceck column_type_missing test', () => {
      expect(
        column_type_missing.check(
          { column: gridObj.getColumnByField('OrderID') },
          gridObj
        ).success
      ).toBeFalsy();
      expect(
        column_type_missing.generateMessage({}, gridObj, 'OrderID')
      ).not.toBeNull();
    });
    it('Check check_datasource_columns test', () => {
      ...
      expect(
        check_datasource_columns.generateMessage({}, gridObj)
      ).not.toBeNull();
    });
    it('datasource_format_mismatch test', () => {
      expect(
        datasource_syntax_mismatch.check({ dataState: gridObj }, gridObj)
          .success
      ).toBeTruthy();
      expect(
        datasource_syntax_mismatch.generateMessage(
          { dataState: gridObj },
          gridObj
        )
      ).not.toBeNull();
