 ...
 */
import { extend, getValue, select } from '@hoyui/base';
...
describe('Dialog Editing module', () => {
  ...
  describe('Dialog editing render => ', () => {
    ...
    it('Edit complete', (done: Function) => {
      ...
      gridObj.actionComplete = actionComplete;
      gridObj.actionBegin = actionBegin;
      (select('#' + gridObj.element.id + 'CustomerID', document) as any).value =
        'updated';
      (
        select(
          '#' + gridObj.element.id + '_dialogEdit_wrapper',
          document
    ...
    it('Add complete', (done: Function) => {
      ...
      gridObj.actionBegin = actionBegin;
      (
        select('#' + gridObj.element.id + 'OrderID', document) as any
      ).value = 10247;
      (select('#' + gridObj.element.id + 'CustomerID', document) as any).value =
        'updated';
      (
        ...
        if (args.requestType === 'beginEdit') {
          let td = (
            select(
              '#' + gridObj.element.id + '_dialogEdit_wrapper',
              document
            ).querySelectorAll('td.e-rowcell') as any
          )[0];
          ...
          (
            select(
              '#' + gridObj.element.id + '_dialogEdit_wrapper',
              document
            ).querySelectorAll('.e-dlg-closeicon-btn') as any
          )[0].click();
        }
        if (args.requestType === 'cancel') {
  ...
  describe('grid css class functionalities', () => {
    ...
    it('context menu CSS class Test', (done: Function) => {
      ...
      expect(
        (
          gridObj.contextMenuModule as any
        ).element.parentElement.classList.contains('e-bigger')
      ).toBeTruthy();
    ...
    it('dialog-css class Test', () => {
      ...
      expect(
        document
          .getElementById(gridObj.element.id + '_dialogEdit_wrapper')
          .parentElement.classList.contains('e-bigger')
      ).toBeTruthy();
  ...
  describe('Dialog editing render => ', () => {
      ...
      gridObj = createGrid(
        {
          ...
          editSettings: {
            ...
            allowDeleting: true,
            mode: 'Dialog',
            dialog: { params: { height: '500px', width: '400px' } },
          },
          toolbar: ['Add', 'Edit', 'Delete', 'Update', 'Cancel'],
          columns: [
            {
    ...
    it('Check the dialog height', () => {
      expect(
        (document.querySelectorAll('.e-popup-open')[0] as any).style.height
      ).toBe('500px');
      expect(
        (document.querySelectorAll('.e-popup-open')[0] as any).style.width
      ).toBe('400px');
    });
    it('memory leak', () => {
      profile.sample();
  ...
  describe('Coverage Imrovement => ', () => {
    ...
    it('called updateRow method', (done: Function) => {
      ...
      gridObj.actionComplete = actionComplete;
      (<any>gridObj.editModule).editModule.updateRow(2, {
        OrderID: 10250,
        CustomerID: 'ALFKI',
      });
    });
