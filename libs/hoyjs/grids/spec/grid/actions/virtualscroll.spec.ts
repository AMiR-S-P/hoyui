 ...
 */
import { EmitType, EventHandler, select } from '@hoyui/base';
...
let count5000: string[] = Array.apply(null, Array(500)).map(
  () => 'Column' + ++ctr + ''
);
let data: Object[] = (() => {
  let arr: Object[] = [];
...
describe('Virtualization testing', () => {
  ...
  describe('virtualization with grouping enabled', () => {
      ...
      grid = createGrid(
        ...
        () => {
          ...
          EventHandler.trigger(
            <HTMLElement>grid.getContent().firstChild,
            'wheel'
          );
          content.scrollTop = 10;
    ...
    it('check pageSize', () => {
      ...
      let contentModule: VirtualContentRenderer = <VirtualContentRenderer>(
        grid.contentModule
      );
      let fn: Function = (<any>(
        (contentModule as any).observer
      ...
      (<any>contentModule).getInfoFromView(
        'down',
        (contentModule as any).observer.sentinelInfo.up,
        { top: 100, left: 50 }
      );
  ...
  describe('Row height checking in Virtual mode', () => {
    ...
    it('API Checking', () => {
      expect(
        (grid.element.querySelectorAll('.e-row')[0] as HTMLElement).style.height
      ).toBe('70px');
  ...
  describe('Check scroll position after grid sort actions', () => {
    ...
    it('check scroll top after filter - should change to 0', () => {
      expect((<HTMLElement>grid.getContent().firstChild).scrollTop).toEqual(
        500
      );
    });
    afterAll(() => {
  ...
  describe('Selection feature testing', () => {
    describe('without scrolling', () => {
      ...
      it('check selection by click in first page', (done: Function) => {
        ...
        let row: HTMLTableRowElement = <HTMLTableRowElement>(
          (<HTMLTableElement>grid.getContentTable()).rows[0]
        );
    ...
    describe('after scrolling', () => {
      ...
      it('check selection by selectRow method', (done: Function) => {
        let row: HTMLTableRowElement = <HTMLTableRowElement>(
          (<HTMLTableElement>grid.getContentTable()).rows[5]
        );
        let index: number = parseInt(row.getAttribute('data-rowindex'), 10);
        grid.rowSelected = <EmitType<{}>>done;
...
describe('Column virtualization', () => {
  describe('scroll left continous', () => {
    ...
    it('check first column index', () => {
      ...
      expect(grid.getColumnIndexesInView()[0]).toBeGreaterThanOrEqual(
        parseInt(row.cells[0].getAttribute('data-colindex'), 10)
      );
    });
    it('check last column index', () => {
      ...
      expect(indexes[indexes.length - 1]).toBeGreaterThanOrEqual(
        parseInt(
          row.cells[indexes.length - 1].getAttribute('data-colindex'),
          10
        )
      );
    });
  ...
  describe('Grouping enabled', () => {
    ...
    it('check expand group cell', () => {
      ...
      expect(
        row.cells[0].classList.contains('e-recordplusexpand')
      ).toBeTruthy();
    ...
    it('horizontal scroll the grouped grid content', (done: Function) => {
      ...
      expect(row.querySelectorAll('.e-groupcaption').length).toEqual(
    ...
    it('check group merge', () => {
      let gen: VirtualRowModelGenerator = <VirtualRowModelGenerator>(
        (<VirtualContentRenderer>grid.contentModule).getModelGenerator()
      );
      gen.cache[1] = [
        new Row<Column>({
          data: { field: 'test1', key: 'test1' },
          isDataRow: false,
        }),
      ];
      let res: Row<Column>[] = gen.updateGroupRow(
        [
          new Row<Column>({
            data: { field: 'test1', key: 'test1' },
            isDataRow: false,
      ...
      let no: Row<Column>[] = gen.updateGroupRow(
        [
          new Row<Column>({
            data: { field: 'test1', key: 'test1' },
            isDataRow: true,
    ...
    it('for coverage', () => {
      ...
      (gen as any).cache = {
        1: [{ data: { field: 'Column9' }, field: 'Column9' }],
        2: [{ data: { field: 'Column9' }, field: 'Column8' }],
      };
      ...
      contentModule.ensureBlocks({
        block: 0,
        blockIndexes: [124, 125],
        page: 63,
        direction: 'up',
      });
    });
  ...
  describe('Ensure the virtual table while rendering the Grid', () => {
    ...
    it('Ensured the virtual table while rendering', (done: Function) => {
      let dataBound = () => {
        expect(
          grid.element.querySelectorAll('.e-virtualtable').length
        ).toBeTruthy(1);
        grid.dataBound = null;
        done();
      };
  ...
  describe('EJ2-37789 - Summary has an empty when collapse the record', () => {
    ...
    it('collapse first row', (done: Function) => {
      gObj.groupModule.expandCollapseRows(
        gObj.getContent().querySelector('.e-gdiagonaldown')
      );
      setTimeout(done, 200);
    });
    it('check aggregare value after the collapse action', (done: Function) => {
      let text: string = (gObj as any).footerElement.querySelector(
        '.e-templatecell'
      ).textContent;
  ...
  describe('Column virtualization with inline editing testing', () => {
    ...
    it('added record', (done: Function) => {
      let actionComplete = (args: EditEventArgs) => {
        expect(args.form.getElementsByTagName('td').length).toBe(
          gObj.getColumns().length
        );
        (
          select('#' + gObj.element.id + 'FIELD1', gObj.element) as any
        ).value = 11013456;
    ...
    it('Check editforms count after horizontal scroll', (done: Function) => {
      let dataBound = (args: EditEventArgs) => {
        expect(
          gObj.element.querySelector('.e-gridform').getElementsByTagName('td')
            .length
        ).toBe(gObj.getColumns().length);
        gObj.dataBound = null;
        done();
      };
    ...
    it('Save added record', (done: Function) => {
      ...
      gObj.actionComplete = actionComplete;
      (
        select('#' + gObj.element.id + columns[0].field, gObj.element) as any
      ).value = 123;
      gObj.endEdit();
    });
    ...
    it('edit record', (done: Function) => {
      let actionComplete = (args: EditEventArgs) => {
        (
          select(
            '#' + gObj.element.id + gObj.getColumns()[3].field,
            gObj.element
          ) as any
        ).value = 456789;
        gObj.actionComplete = null;
    ...
    it('Save edited record', (done: Function) => {
      let actionComplete = (args: EditEventArgs) => {
        ...
        expect(gObj.getRowsObject()[0].data[gObj.getColumns()[3].field]).toBe(
          456789
        );
        gObj.actionComplete = null;
        done();
    ...
    it('check validation msg width', (done: Function) => {
      let actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          gObj.element.focus();
          (<any>gObj.toolbarModule).toolbarClickHandler({
            item: { id: gObj.element.id + '_update' },
          });
  ...
  describe('column virtualization with inline editing validation check', () => {
    ...
    it('add record to check horizontal scroll action', (done: Function) => {
      ...
      let actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          ...
          expect(
            gObj.editModule.virtualFormObj.element.getElementsByClassName(
              'e-field'
            ).length
          ).toBe(gObj.columns.length);
          gObj.element.focus();
          (<any>gObj.toolbarModule).toolbarClickHandler({
            item: { id: gObj.element.id + '_update' },
          });
          expect(
            gObj.editModule.virtualFormObj.element.querySelector(
    ...
    it('check column virtualization horizontal scroll move', (done: Function) => {
      ...
      (
        gObj.element.querySelector(
          '#' + gObj.element.id + columns[0].field
        ) as HTMLInputElement
      ).value = '567843212345674';
      ...
      let dataBound = () => {
        ...
        expect(
          gObj.editModule.virtualFormObj.element.querySelectorAll(
            '.e-griderror:not([style*="display: none"])'
          ).length
        ).toBe(1);
        ...
        (
          gObj.element.querySelector(
            '#' + gObj.element.id + columns[columns.length - 1].field
          ) as HTMLInputElement
        ).value = 'updated';
        gObj.actionComplete = actionComplete;
        gObj.dataBound = null;
    ...
    it('Check auto vertical scroll action', (done: Function) => {
      let dataBound = () => {
        ...
        expect(
          gObj.editModule.formObj.element.querySelectorAll(
            '.e-griderror:not([style*="display: none"])'
          ).length
        ).toBe(1);
        gObj.dataBound = null;
  ...
  describe('EJ2-53627 -> Grid column virtualization does not support dynamic column changes', () => {
    ...
    it('Check the column visibility', (done: Function) => {
      let dataBound = () => {
        expect(
          (gObj as any).columnModel[(gObj as any).columnModel.length - 1]
            .visible
        ).toBe((gObj as any).columnModel[3].visible);
        expect(
          (gObj as any).columnModel[(gObj as any).columnModel.length - 1]
            .visible
        ).toBe(false);
        expect(
          document
            .getElementsByClassName('e-headercell')[0]
  ...
  describe('EJ2-828707- Editing not working properly when virtual scroll is enabled and the primary key is a complex data => ', () => {
    ...
    it('Edit the row in grid', (done: Function) => {
      let actionComplete = (args?: any): void => {
        if (args.requestType === 'save') {
          expect(gridObj.currentViewData[1]['customer']['CustomerID']).toBe(
            'TOMSP'
          );
          done();
        }
      ...
      (
        select(
          '#' + gridObj.element.id + 'customer___CustomerID',
          gridObj.element
        ) as any
      ).value = 'BRAMP';
      (<any>gridObj.toolbarModule).toolbarClickHandler({
  ...
  describe('EJ2-847398 - changeDataSource not working properly when virtualization is enabled => ', () => {
      ...
      gridObj = createGrid(
        {
          ...
          enableColumnVirtualization: true,
          height: 300,
          columns: [{ field: 'OrderID', headerText: 'OrderID', width: 120 }],
        },
        done
      );
    });
    ...
    it('changedatasource call', (done: Function) => {
      let dataBound = () => {
        ...
        expect(gridObj.headerModule['virtualEle'].wrapper.style.width).toBe(
          '100%'
        );
...
describe('EJ2-859411-Scroll using the down arrow key by focusing the template, the Grid lose focus', () => {
  ...
  it('check selection by click in last viewable row', (done: Function) => {
    ...
    row = <HTMLTableRowElement>(
      (<HTMLTableElement>grid.getContentTable()).rows[7]
    );
    let index: number = parseInt(row.getAttribute('data-rowindex'), 10);
  ...
  it('down key action and check scrolltop set properly for 1 row height', (done: Function) => {
    ...
    dataBound = (args?: any): void => {
      dataBound = null;
      expect(
        (<HTMLElement>grid.getContent().firstChild).scrollTop
      ).toBeLessThan(40);
      expect((<HTMLElement>grid.getContent().firstChild).scrollTop).not.toBe(0);
      done();
    };
    grid.dataBound = dataBound;
    grid.keyboardModule.keyAction({
      action: 'downArrow',
...
describe('VirtualScroll code coverage', () => {
  ...
  it('execute setBlockForManualRefresh', () => {
    ...
    (contentModule.vgenerator as any).setBlockForManualRefresh(
      contentModule.vgenerator.cache,
      [1, 2],
      rows
    );
  });
...
describe('Grouping VirtualScroll code coverage', () => {
  ...
  it('coverage for grouping with hide/show column in virtualscroll', () => {
    let captions: NodeListOf<Element> =
      gObj.element.querySelectorAll('.e-groupcaption');
    gObj.keyboardModule.keyAction({
      action: 'downArrow',
...
describe('enableColumnVirtualization code coverage', () => {
  ...
  it('coverage for editActionBegin', () => {
    ...
    (contentModule as any).editActionBegin({
      data: { OrderID: 101 },
      index: 1,
      isScroll: false,
    });
    ...
    (contentModule as any).refreshCache({});
    (gObj as any).virtualscrollModule.refreshVirtualElement({
      module: 'resize',
    });
