 ...
 */
import { EventHandler, select } from '@hoyui/base';
...
describe('Toolbar functionalities', () => {
  ...
  it('initial checck', () => {
    expect(
      gridObj.toolbarModule
        .getToolbar()
        .firstElementChild.querySelector('.e-hscroll-content').childElementCount
    ).toBe(5);
    expect(
      gridObj.toolbarModule
        .getToolbar()
        .firstElementChild.querySelectorAll('.e-toolbar-item')[4]
        .getAttribute('title')
    ).toBe('Add');
    expect(
      gridObj.element.firstElementChild.classList.contains('e-groupdroparea')
    ).toBeTruthy();
  });
  ...
  it('enable Rtl', () => {
    ...
    expect(
      gridObj.toolbarModule.getToolbar()['ej2_instances'][0]['enableRtl']
    ).toBeTruthy();
    expect(
      gridObj.toolbarModule.getToolbar().classList.contains('e-rtl')
    ).toBeTruthy();
  });
  it('disable Rtl', () => {
  ...
  it('change toolbar value', () => {
    ...
    gridObj.dataBind();
    expect(
      gridObj.toolbarModule.getToolbar().querySelector('.e-toolbar-left')
        .children.length
    ).toBe(4);
    expect(
      gridObj.toolbarModule.getToolbar().querySelector('.e-toolbar-right')
        .children.length
    ).toBe(1);
    //expect(gridObj.toolbarModule.getToolbar().querySelectorAll('.e-overlay').length).toBe(2);
  });
  it('check aria-attribute', () => {
    let search: Element = gridObj.toolbarModule
      .getToolbar()
      .querySelector('.e-search');
    expect(
      search.querySelector('.e-search-icon').hasAttribute('tabindex')
    ).toBeTruthy();
  });
  ...
  it('render all predefined items', () => {
    ...
    gridObj.dataBind();
    expect(
      gridObj.toolbarModule.getToolbar().querySelectorAll('.e-toolbar-item')
        .length
    ).toBe(11);
  ...
  it('Check keyPressHandler by Tab and Shift Tab Action', () => {
    const focusableToolbarItems: NodeListOf<Element> =
      gridObj.toolbarModule.toolbar.element.querySelectorAll(
        '.e-toolbar-item:not(.e-overlay):not(.e-hidden)'
      );
    let args: any = {
      action: 'tab',
      preventDefault: preventDefault,
      target: focusableToolbarItems[0].querySelector('.e-btn'),
    };
    (gridObj.toolbarModule as any).keyPressedHandler(args);
    expect(
      focusableToolbarItems[1].querySelector('.e-btn').getAttribute('tabindex')
    ).toBe('0');
    args = {
      action: 'shiftTab',
      preventDefault: preventDefault,
      target: focusableToolbarItems[1].querySelector('.e-btn'),
    };
    (gridObj.toolbarModule as any).keyPressedHandler(args);
    expect(
      focusableToolbarItems[0].querySelector('.e-btn').getAttribute('tabindex')
    ).toBe('0');
  });
  ...
  it('check search', (done: Function) => {
    ...
    let searchElement: HTMLInputElement = select(
      '#' + gridObj.element.id + '_searchbar',
  ...
  it('check search with searchbutton', (done: Function) => {
    ...
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: (<any>gridObj.toolbarModule).toolbar.items[9],
      originalEvent: {
        target: document.getElementById(gridObj.element.id + '_searchbutton'),
      },
    });
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      item: (<any>gridObj.toolbarModule).toolbar.items[9],
      originalEvent: { target: searchElement },
    });
  });
  ...
  it('check print', (done: Function) => {
    ...
    gridObj.beforePrint = (args: { element: Element }) => {
      expect(args.element.querySelector('.e-toolbar') as HTMLElement).toBe(
        null
      );
    };
    select(
      '#' + gridObj.element.id + '_print',
      gridObj.toolbarModule.getToolbar()
    ).click();
    //forcoverage
    (<any>gridObj.toolbarModule).toolbarClickHandler({
      target: (<any>gridObj.toolbarModule).element,
    });
    (gridObj.toolbarModule as any).keyUpHandler({ keyCode: 12 });

    keyup.target = gridObj.toolbarModule.getToolbar();
    EventHandler.trigger(
      gridObj.toolbarModule.getToolbar() as HTMLElement,
      'keyup',
      keyup
    );
  ...
  describe('Toolbar functionalities', () => {
    ...
    beforeAll((done: Function) => {
      let templete: string =
        '<div><div style="padding: 12px" title="search" ><input id="txt" type="search" style="padding: 0 5px"placeholder="search"></input><span id="searchbutton" class="e-search e-icons"></span></div></div>';
      document.body.appendChild(
        createElement('div', { innerHTML: templete, id: 'search' })
      );
      gridObj = createGrid(
        {
          dataSource: data,
...
describe('code coverage - Toolbar - Adaptive UI', () => {
  ...
  it('Clear search', function () {
    const clear: HTMLElement = gridObj.element.querySelector(
      '#' + gridObj.element.id + '_clearbutton'
    ) as HTMLElement;
  ...
  it('Back search wrapper', function () {
    (
      gridObj.element.querySelector(
        '#' + gridObj.element.id + '_responsiveback'
      ) as HTMLElement
    ).click();
  });
  ...
  it('Click sort', function () {
    (
      gridObj.element.querySelector(
        '#' + gridObj.element.id + '_responsivesort'
      ) as HTMLElement
    ).click();
  });
...
describe('Code Coverage - childGrid - toolbarTemplate and emptyRecordTemplate => ', () => {
  ...
  it('Case 1', () => {
    (
      gridObj
        .getContentTable()
        .querySelector('.e-dtdiagonalright') as HTMLElement
    ).click();
  });
...
describe('EJ2-899326 => Script error occurs on selecting records in Adaptive UI when the toolbar have template elements => ', () => {
    ...
    gridObj = createGrid(
      {
        ...
        toolbar: [
          'Add',
          { id: 'test', template: '<span id="test">Test</span>' },
        ],
        pageSettings: { pageSize: 12, pageSizes: true },
        columns: [
          {
  ...
  it('Selecting records and check the script error', function () {
    ...
    if (document.querySelectorAll('.e-toolbar-item.e-template').length) {
      gridObj.selectRow(0);
      expect(
        document.querySelector('.e-toolbar-item.e-template')
      ).not.toBeNull();
    }
  });
