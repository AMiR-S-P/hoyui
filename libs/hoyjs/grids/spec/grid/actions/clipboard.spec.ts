...
describe('Grid clipboard copy testing - row type selection => ', () => {
  ...
  it('Check hidden clipboard textarea', () => {
    let clipArea: HTMLElement = gridObj.element.querySelectorAll(
      '.e-clipboard'
    )[0] as HTMLElement;
    expect(
      gridObj.element.querySelectorAll('.e-clipboard').length > 0
    ).toBeTruthy();
  ...
  it('Check with row type selection', () => {
    ...
    gridObj.copy();
    expect(
      (document.querySelector('.e-clipboard') as HTMLInputElement).value ===
        '1	Nancy	Sales Representative\n2	Andrew	Vice President, Sales'
    ).toBeTruthy();
  });
...
describe('EJ2-7314/7299===>Grid clipboard => ', () => {
  ...
  it('EJ2-7299===>Hiding one column and copying the rows', () => {
    ...
    gridObj.copy();
    expect(
      (gridObj.element.querySelector('.e-clipboard') as HTMLInputElement)
        .value === '1	Nancy	WA	USA'
    ).toBeTruthy();
    gridObj.copy(true);
    expect(
      (gridObj.element.querySelector('.e-clipboard') as HTMLInputElement)
        .value === 'Employee ID	Name	Region	Country\n1	Nancy	WA	USA'
    ).toBeTruthy();
  });
  it('memory leak', () => {
    profile.sample();
...
describe('EJ2-826272 - Copy-Paste problem while adding a new row in grid', () => {
  ...
  it('Ensuring the copied content', () => {
    ...
    gridObj.copy();
    expect(
      (gridObj.element.querySelector('.e-clipboard') as HTMLInputElement).value
    ).toBe('TOMSP');
  });
...
describe('Coverage Improvemnet - Lazy Load Grouping with Virtual Scroll  => ', () => {
  ...
  it('Ensuring the copied content - Lazy Load Grouping ', () => {
    let cell: HTMLElement = gridObj.getContent().querySelector('.e-row')
...
describe('Coverage Improvemnet - Infinite Scroll  => ', () => {
  ...
  it('clipboard coverage', () => {
    ...
    (gridObj as any).selectionModule.selectedRowCellIndexes = [
      { rowIndex: 1, cellIndexes: [] },
    ];
