 ...
 */
import { Browser, EmitType } from '@hoyui/base';
...
describe('Resize module', () => {
  describe('Resize functionalities for columns', () => {
    ...
    it('Auto fit with Reorder', () => {
      ...
      expect(headers[3].querySelector('.e-headercelldiv').textContent).toEqual(
        'EmployeeID'
      );
  ...
  describe('Resize functionalities for Hidden columns', () => {
    ...
    it('Resize for Hidden column', () => {
      ...
      headers = (<HTMLElement>(
        gridObj.getHeaderTable().querySelectorAll('th')[4]
      )).style.width;
      expect(headers).toBeFalsy();
    });
  ...
  describe('allowResizing functionality', () => {
    ...
    // });
    it('resize start', () => {
      let handler: HTMLElement = gridObj
        .getHeaderTable()
        .querySelectorAll('.' + resizeClassList.root)[1];
    ...
    it('resize end', () => {
      ...
      expect(
        gridObj.element.classList.contains(resizeClassList.cursor)
      ).toBeFalsy();
      ...
      expect(
        gridObj.element.querySelector('.' + resizeClassList.helper)
      ).toBeFalsy();
    });
    ...
    it('resizing - mousemove', () => {
      ...
      expect(width).toEqual(
        gridObj.getHeaderTable().querySelectorAll('th')[1].offsetWidth
      );
    ...
    it('min move', () => {
      ...
      let handler: HTMLElement = gridObj
        .getHeaderTable()
        .querySelectorAll('.' + resizeClassList.root)[2];
    ...
    it('check width', () => {
      let width: string =
        gridObj.widthService.getTableWidth(gridObj.getColumns()) + 'px';
    ...
    it('refreshColumnWidth method', () => {
      ...
      for (let ele of [].slice.apply(
        gridObj.getHeaderTable().querySelectorAll('th.e-headercell')
      )) {
  ...
  describe('Events', () => {
    let resizeStartevent: EmitType<ResizeArgs> =
  ...
  describe('Resize with footer table', () => {
    ...
    it('width test case', () => {
      ...
      expect(
        (grid.getFooterContentTable() as HTMLElement).style.width
      ).toBeDefined();
      let handler: HTMLElement = <HTMLElement>(
        grid.getHeaderTable().querySelectorAll('.' + resizeClassList.root)[1]
      );
  ...
  describe('Resize module with Freeze pane', () => {
    ...
    describe('allowResizing functionality', () => {
      ...
      it('autoFit from hander', () => {
        let handler: HTMLElement = gridObj
          .getHeaderTable()
          .querySelectorAll('.' + resizeClassList.root)[1];
        let column: Column = gridObj.resizeModule.getTargetColumn({
          target: handler,
        });
        expect(column.field).toEqual('CustomerID');
        let width: string = gridObj
          .getHeaderTable()
          .querySelectorAll('th')[1].offsetWidth;
        gridObj.resizeModule.callAutoFit({ target: handler });
        expect(
    ...
    describe('EJ2-66907 - autoFitColumns does not work with Frozen Grid', () => {
      ...
      it('check frozen autofit table width', () => {
        let tables: NodeListOf<HTMLElement> =
          gridObj.element.querySelectorAll('table');
    ...
    describe('Events', () => {
      let resizeStartevent: EmitType<ResizeArgs> =
        jasmine.createSpy('resizeStartevent');
      let resizeStop: EmitType<ResizeArgs> =
        jasmine.createSpy('resizeStartStop');
      let resize: EmitType<ResizeArgs> = jasmine.createSpy('resize');
      let gridObj: any;
      beforeAll((done) => {
        jasmine.Ajax.install();
  ...
  describe('Resize functionalities for all columns', () => {
    ...
    it('Resize all columns', () => {
      ...
      expect(headers.width).toBeTruthy();
      headers = (
        gridObj.getHeaderContent().querySelector('.e-headercontent')
      ...
      gridObj = createGrid(
        {
          ...
          columns: [
            ...
            { field: 'CustomerID', headerText: 'CustomerID', width: 180 },
            {
              field: 'EmployeeID',
              headerText: 'EmployeeID',
              width: 150,
  ...
  describe('Resize functionalities for stacked columns', () => {
    ...
    it('check stacked header resize handler', () => {
      expect(
        gridObj
          .getHeaderContent()
          .querySelectorAll('.e-stackedheadercell .e-rhandler.e-rcursor').length
      ).toBe(1);
    });

    it('check auto fit', () => {
      expect(
        (gridObj.resizeModule as any).callAutoFit({
          target: gridObj
            .getHeaderContent()
    ...
    it('calculate column width', () => {
      let columns = (gridObj.resizeModule as any).getSubColumns(
        gridObj.columns[1],
        []
      );
      expect(columns.length).toBe(3);
      columns = (gridObj.resizeModule as any).calulateColumnsWidth(
        columns,
        false,
        4
      );
  ...
  describe('Resize functionalities with row drag and drop', () => {
    ...
    it('Row drag and drop header element', () => {
      expect(gridObj.element.querySelectorAll('.e-rowdragheader').length).toBe(
        1
      );
    });
    ...
    it('Column width after resize', () => {
      let width = gridObj
        .getHeaderTable()
        .querySelectorAll('th')[3].offsetWidth;
      expect(
        gridObj.getHeaderTable().querySelectorAll('th')[3].offsetWidth
      ).toBe(width);
      let handler: HTMLElement = gridObj
        .getHeaderTable()
        .querySelectorAll('.' + resizeClassList.root)[1];
      ...
      width += 150;
      expect(
        gridObj.getHeaderTable().querySelectorAll('th')[2].offsetWidth
      ).toEqual(width);
      expect(
        gridObj.getHeaderTable().querySelectorAll('th')[3].offsetWidth
      ).not.toBe(width);
      ...
      gridObj = createGrid(
        {
          ...
          dataSource: data1,
          columns: [
            { field: 'OrderID', headerText: 'OrderID', width: 140 },
            {
              field: 'About',
              headerText: 'About',
              width: 140,
  ...
  describe('EJ2-42233-right border width in stacked header last column', () => {
    ...
    it('column right width checking', () => {
      let hdrele: Element = gridObj.element
        .querySelectorAll('.e-columnheader:nth-child(2)')[0]
        .querySelectorAll('.e-headercell')[1];
      let lastcell: Element = gridObj.element
        .querySelectorAll('.e-columnheader:nth-child(2)')[0]
  ...
  describe('EJ2-44181-Autofit columns in databound is not working properly for hierarchy grid', () => {
    ...
    it('col group width checking', () => {
      [].slice
        .call(gridObj.getHeaderTable().querySelectorAll('col'))
        .forEach((ele: HTMLElement) => {
          expect(ele.style.width).not.toBe('');
        });
      [].slice
        .call(gridObj.getContentTable().querySelectorAll('col'))
        .forEach((ele: HTMLElement) => {
          expect(ele.style.width).not.toBe('');
        });
    });
  ...
  describe('Frozen Revamp feature', () => {
    ...
    it('freeze right column resizing - mousemove', () => {
      let handler: Element = gridObj
        .getHeaderTable()
        .querySelectorAll('.' + resizeClassList.root)[6];
      (gridObj.resizeModule as any).resizeStart({ target: handler, pageX: 0 });
      (gridObj.resizeModule as any).resizing({ target: handler, pageX: 200 });
      let width = gridObj
        .getHeaderTable()
        .querySelectorAll('th')[6].offsetWidth;
      (gridObj.resizeModule as any).resizing({ target: handler, pageX: 300 });
      width += 100;
      expect(width).toEqual(
        gridObj.getHeaderTable().querySelectorAll('th')[6].offsetWidth
      );
      (gridObj.resizeModule as any).refresh();
    });
    it('freeze right column resizing - mousemove', () => {
      let handler: Element = gridObj
        .getHeaderTable()
        .querySelectorAll('.' + resizeClassList.root)[10];
      (gridObj.resizeModule as any).resizeStart({ target: handler, pageX: 0 });
      (gridObj.resizeModule as any).resizing({ target: handler, pageX: 200 });
      let width = gridObj
        .getHeaderTable()
        .querySelectorAll('th')[10].offsetWidth;
      (gridObj.resizeModule as any).resizing({ target: handler, pageX: 300 });
      width += 100;
      expect(width).toEqual(
        gridObj.getHeaderTable().querySelectorAll('th')[10].offsetWidth
      );
      (gridObj.resizeModule as any).refresh();
    });
    ...
    it('freeze left column resizing - mousemove', () => {
      let handler: Element = gridObj
        .getHeaderTable()
        .querySelectorAll('.' + resizeClassList.root)[0];
      (gridObj.resizeModule as any).resizeStart({ target: handler, pageX: 0 });
      (gridObj.resizeModule as any).resizing({ target: handler, pageX: 200 });
      let width = gridObj
        .getHeaderTable()
    ...
    it('freeze right column resizing - mousemove', () => {
      ...
      let width = gridObj
        .getHeaderTable()
        .querySelectorAll('th')[11].offsetWidth;
      (gridObj.resizeModule as any).resizing({ target: handler, pageX: 300 });
      width += 100;
      expect(width).toEqual(
        gridObj.getHeaderTable().querySelectorAll('th')[11].offsetWidth
      );
      (gridObj.resizeModule as any).refresh();
    });
    afterAll(() => {
  ...
  describe('EJ2-867174 - Implementation the `autoFitColumns()` method feature request for better performance with start and end row Index', () => {
    ...
    let data1: object[] = [
      ...
      {
        OrderID: 10249,
        About: 'Anne has a BA degree in English',
        EmployeeID: 5,
        ShipCity: 'Reims',
        Freight: 32.38,
      },
    ...
    it('Check whether the width for the columns has been properly set with respect to start and end row index', () => {
      expect(
        parseInt(
          (gridObj.resizeModule as any).widthService.getWidth(
  ...
  describe('resize file code coverage improvement - 1', () => {
    ...
    it('resize file method coverage - 1', () => {
      ...
      (gridObj as any).resizeModule.calculateColspanWidth(
        gridObj.getColumns(),
        cell,
        1
      );
  ...
  describe('EJ2-904603 - column autoFit property not considered even resizeSettings mode as normal when applying other column width property value as auto', () => {
    ...
    it('check maxWidth col width', () => {
      expect(
        (gridObj.getContentTable() as HTMLElement).querySelectorAll('col')[1]
          .style.width
      ).toBe('auto');
    });
