 ...
 */
import { createElement, isNullOrUndefined } from '@hoyui/base';
...
describe('Grouping module => ', () => {
  describe('Grouping functionalites => ', () => {
    ...
    it('group drop area testing', () => {
      ...
      expect(dropArea[0].textContent).toBe(
    ...
    it('Single column group testing', (done: Function) => {
      actionComplete = (args?: Object): void => {
        let grpHIndent = gridObj
          .getHeaderContent()
          .querySelectorAll('.e-grouptopleftcell');
        ...
        expect(
          grpHIndent[0]
            ...
            .classList.contains('e-emptycell')
        ).toBeTruthy();
        expect(content[0].querySelectorAll('.e-recordplusexpand').length).toBe(
          1
        );
        expect(
          content[0]
            .querySelectorAll('.e-recordplusexpand')[0]
            .firstElementChild.classList.contains('e-gdiagonaldown')
        ).toBeTruthy();

        expect(content[0].querySelectorAll('.e-groupcaption').length).toBe(1);
        expect(
          content[0]
            .querySelectorAll('.e-groupcaption')[0]
            .getAttribute('colspan')
        ).toBe('6');
        expect(
          content[0].querySelectorAll('.e-groupcaption')[0].textContent
        ).toBe('Ship City: Albuquerque - 5 items');

        expect(content[1].querySelectorAll('.e-indentcell').length).toBe(1);

        expect(
          gridObj.getContent().querySelectorAll('.e-recordplusexpand').length
        ).toBe(6);

        expect(gHeader.length).toBe(1);
        expect(gHeader[0].querySelectorAll('.e-grouptext').length).toBe(1);
        expect(gHeader[0].querySelectorAll('.e-grouptext')[0].textContent).toBe(
          'Ship City'
        );
        expect(gHeader[0].querySelectorAll('.e-groupsort').length).toBe(1);
        expect(
          gHeader[0]
            .querySelectorAll('.e-groupsort')[0]
            .classList.contains('e-ascending')
        ).toBeTruthy();
    ...
    it('Expandcollase row shortcut testing', () => {
      ...
      expect(
        gridObj
          .getContent()
          .querySelectorAll('tr:not([style*="display: none"])').length
      ).toBe(13);
      (<any>gridObj.groupModule).keyPressHandler(getKeyActionObj('altUpArrow'));
      expect(
        gridObj
      ...
      (<any>gridObj.groupModule).keyPressHandler(
        getKeyActionObj('altDownArrow')
      );
      expect(
        gridObj
          .getContent()
    ...
    it('multiple column group testing', (done: Function) => {
      actionComplete = (args?: Object): void => {
        ...
        expect(content[1].querySelectorAll('.e-recordplusexpand').length).toBe(
          1
        );
        expect(
          (
            content[1].querySelectorAll(
              '.e-recordplusexpand'
            )[0] as HTMLTableCellElement
          ).cellIndex
        ).toBe(1);
        ...
        expect(content[2].querySelectorAll('.e-recordplusexpand').length).toBe(
          1
        );
        expect(
          (
            content[2].querySelectorAll(
              '.e-recordplusexpand'
            )[0] as HTMLTableCellElement
          ).cellIndex
        ).toBe(2);
        ...
        expect(
          content[0].querySelectorAll('.e-groupcaption')[0].textContent
        ).toBe('Ship City: Albuquerque - 1 item');
        expect(
          content[1].querySelectorAll('.e-groupcaption')[0].textContent
        ).toBe('Ship Country: USA - 1 item');
        expect(
          content[2].querySelectorAll('.e-groupcaption')[0].textContent
        ).toBe('CustomerID: RATTC - 5 items');
    ...
    it('Sort column with sorting disabled testing', (done: Function) => {
      actionComplete = (args?: Object): void => {
        expect(
          gridObj.element
            .querySelector('.e-groupdroparea')
            .querySelectorAll('.e-descending').length
        ).toBe(0);
        expect(
          gridObj.element
            .querySelector('.e-groupdroparea')
            .querySelectorAll('.e-ascending').length
        ).toBe(2);
        ...
        expect(
          gridObj
            .getColumnHeaderByField('CustomerID')
            .querySelectorAll('.e-ascending').length
        ).toBe(1);
        expect(
          gridObj
            .getColumnHeaderByField('ShipCity')
            .querySelectorAll('.e-ascending').length
        ).toBe(1);
        done();
      };
    ...
    it('ungroup from button click testing', (done: Function) => {
      ...
      gridObj.actionComplete = actionComplete;
      (gridObj.groupModule as any).clickHandler(
        getClickObj(
          gridObj.element
            .getElementsByClassName('e-groupheadercell')[0]
            .querySelector('.e-ungroupbutton')
        )
      );
    });
    ...
    it('expandcollapse rows method testing', () => {
      ...
      expect(expandElem[0].firstElementChild.getAttribute('title')).toBe(
        'Expanded'
      );
  ...
  describe('Grouping columns hide => ', () => {
    ...
    it('Single column group testing', (done: Function) => {
      actionComplete = (args?: Object): void => {
        ...
        expect(gridObj.sortSettings.columns.length).toBe(1);
        expect(
          gridObj.element.querySelectorAll('.e-headercell:not(.e-hide)').length
        ).toBe(5);
        done();
      };
      gridObj.actionComplete = actionComplete;
  ...
  describe('Grouping set model test case and reorder => ', () => {
    ...
    it('check default group property rendering', () => {
      ...
      expect(
        gridObj.getHeaderTable().querySelectorAll('.e-grptogglebtn').length
      ).toBe(gridObj.columns.length);
    });
    it('disable Grouping', (done: Function) => {
      actionComplete = () => {
        expect(
          gridObj.element.querySelectorAll('.e-groupdroparea').length
        ).toBe(0);
        expect(
          gridObj.getHeaderTable().querySelectorAll('.e-grptogglebtn').length
        ).toBe(0);
    ...
    it('group a column', (done: Function) => {
      actionComplete = () => {
        ...
        expect(
          gridObj
            .getHeaderTable()
            .querySelectorAll('.e-grptogglebtn.e-toggleungroup').length
        ).toBe(1);
        expect(
          gridObj.element
        ...
        expect(
          gridObj.getHeaderTable().querySelectorAll('.e-ascending').length
        ).toBe(1);
        done();
      };
      gridObj.actionComplete = actionComplete;
    ...
    it('reOrder the grouped column', (done: Function) => {
      actionComplete = () => {
        ...
        expect(
          gridObj.getHeaderContent().querySelectorAll('.e-headercell')[5]
  ...
  describe('Group settings apis => ', () => {
    ...
    it('sort after group testing', (done: Function) => {
      ...
      (gridObj.groupModule as any).clickHandler(
        getClickObj(headers[0].querySelector('.e-grptogglebtn'))
      );
    });
    ...
    it('hide drop area', () => {
      ...
      gridObj.dataBind();
      expect(
        (gridObj.element.querySelectorAll('.e-groupdroparea')[0] as HTMLElement)
          .style.display
      ).toBe('none');
    });
    ...
    it('hide ungroup button', () => {
      ...
      expect(
        (
          gridObj.element
            .querySelectorAll('.e-groupdroparea')[0]
            .querySelectorAll('.e-ungroupbutton')[0] as HTMLElement
        ).style.display
      ).toBe('none');
    });
  ...
  describe('Stacked header with grouping => ', () => {
    ...
    it('group a column', (done: Function) => {
      let actionComplete = (args: Object) => {
        expect(
          gridObj.element.querySelectorAll('.e-groupheadercell').length
        ).toBe(1);
        expect(
          gridObj.getHeaderContent().querySelectorAll('.e-ascending').length
        ).toBe(1);
        expect(
          gridObj.getHeaderContent().querySelectorAll('.e-emptycell').length
        ).toBe(3);
        done();
      };
      gridObj.groupModule.groupColumn('EmployeeID');
  ...
  describe('Grouping a column in default =>', () => {
    ...
    it('check default group property rendering', () => {
      ...
      expect(
        gridObj.getContent().querySelectorAll('.e-indentcell').length > 0
      ).toBeTruthy();
      expect(
        gridObj.getContent().querySelectorAll('.e-rowcell')[0].innerHTML
      ).toBe('10258');
      expect(gridObj.groupSettings.columns.length).toBe(1);
    });
  ...
  describe('Grouping and sorting same column and aria => ', () => {
    ...
    it('initial render testing', () => {
      expect(gridObj.groupSettings.columns.length).toBe(
        gridObj.sortSettings.columns.length
      );
      expect(
        gridObj.getHeaderContent().querySelectorAll('.e-sortnumber').length
      ).toBe(0);
      expect(
        gridObj.getHeaderContent().querySelectorAll('.e-ascending').length
      ).toBe(1);
    ...
    it('check aria attribute', () => {
      let groupDropArea: Element =
        gridObj.element.querySelector('.e-groupdroparea');
      expect(
        groupDropArea.querySelector('.e-grouptext').hasAttribute('tabindex')
      ).toBeTruthy();
      expect(
        groupDropArea.querySelector('.e-groupsort').hasAttribute('tabindex')
      ).toBeTruthy();
      expect(
        groupDropArea.querySelector('.e-ungroupbutton').hasAttribute('tabindex')
      ).toBeTruthy();
      expect(
        groupDropArea.querySelector('.e-groupsort').hasAttribute('aria-label')
      ).toBeTruthy();
      expect(
        groupDropArea
          .querySelector('.e-ungroupbutton')
      ...
      expect(
        gridObj.element
          .querySelector('.e-recordplusexpand')
  ...
  describe('Grouping remote data => ', () => {
    let gridObj: Grid;
    let old: (e: ReturnType) => Promise<Object> =
      Render.prototype.validateGroupRecords;
    let fetchSpy: any;
        ...
        Promise.resolve({
          ...
          json: function () {
            return Promise.resolve({
              d: { results: filterData, __count: 100 },
            });
          },
        })
      );
    ...
    it('multi grouping with group by format', (done: Function) => {
      let actionComplete: any = (args: Object) => {
        expect(
          gridObj.element
            .querySelectorAll('.e-groupcaption')[0]
            .innerHTML.indexOf('Order Date: 1996 ') > -1
        ).toBeTruthy();
        expect(
          gridObj.element.querySelectorAll('.e-groupheadercell').length
        ).toBe(2);
  ...
  describe('Grouping disablePageWiseAggregates with empty datasource => ', () => {
    ...
    it('EJ2-7165-complex data group', () => {
      expect(
        (<any>gridObj.groupModule).getGHeaderCell('Name.FirstName')
      ).toBeNull();
    });
    afterAll((done) => {
      destroy(gridObj);
  ...
  describe('MT-870619 - After grouping the column, the focus is not working properly => ', () => {
    ...
    it('Single column group testing', (done: Function) => {
      actionComplete = (args?: Object): void => {
        expect(
          gridObj.element
            .querySelector('.e-gdiagonaldown')
            .classList.contains('e-focus')
        ).toBeTruthy();
        done();
      };
      gridObj.actionComplete = actionComplete;
  ...
  describe('Grouping functionalites with empty grid => ', () => {
    ...
    it('group drop area - header present testing', (done: Function) => {
      ...
      setTimeout(function () {
        let dropArea: any =
          gridObj.element.querySelectorAll('.e-groupdroparea');
        expect(dropArea.length).toBe(1);
  ...
  describe('EJ2-35816-Grouping with animation => ', () => {
    ...
    it('Check whether grouped columns added in the droparea', (done) => {
      gridObj.actionComplete = (args?: Object): void => {
        expect(
          gridObj.element.querySelectorAll('.e-group-animator').length
        ).toBe(3);
        expect(gridObj.element.querySelectorAll('.e-icon-drag').length).toBe(3);
        expect(
          gridObj.element.querySelectorAll('.e-group-animator .e-icon-next')
            .length
        ).toBe(3);
        done();
      };
...
describe('EJ2-49647- Column grouping with complex binding is broken when allowReordering is set => ', () => {
  ...
  let complexData: Object[] = [
    {
      OrderID: { ID: { ordID: 10248 } },
      CustomerID: 'VINET',
      Freight: 32.38,
      ShipCountry: 'France',
    },
    {
      OrderID: { ID: { ordID: 10249 } },
      CustomerID: 'TOMSP',
      Freight: 11.61,
      ShipCountry: 'Germany',
    },
    {
      OrderID: { ID: { ordID: 10250 } },
      CustomerID: 'HANAR',
      Freight: 65.83,
      ShipCountry: 'Brazil',
    },
    {
      OrderID: { ID: { ordID: 10251 } },
      CustomerID: 'VICTE',
      Freight: 41.34,
      ShipCountry: 'France',
    },
      ...
      expect(
        gridObj.element.querySelector(
          '.e-groupdroparea div[ej-complexname=' +
            getComplexFieldID(args.columnName) +
            ']'
        )
      ).toBeDefined();
      done();
...
describe('Group caption 1st column template =>', () => {
  ...
  it('check 1st column template', () => {
    expect(
      gridObj.getContent().querySelectorAll('.e-groupcaption')[0].innerHTML
    ).toBe('Order ID: 10248 - 1 item   Max: $32.38');
  });
...
describe('EJ2-62134 Disabling expand and collapse support for infinite scrolling cache with grouping enabled grid. => ', () => {
  ...
  it('Expand collapse icon check', () => {
    expect(
      gridObj.getContent().querySelectorAll('tr')[0].querySelector('td')
        .firstElementChild
    ).toBe(null);
  });
  afterAll(() => {
...
describe('EJ2-67363 - Bottom borderline is not shown when collapsing the last grouped row => ', () => {
  ...
  it('Checking last border in collapseAll method', () => {
    ...
    gridObj.groupModule.collapseAll();
    expect(
      capElem[2].children[0].classList.contains('e-lastrowcell')
    ).toBeTruthy();
    expect(
      capElem[2].children[1].classList.contains('e-lastrowcell')
    ).toBeTruthy();
  });
...
describe('Code Coverage - group by drag and drop simulate => ', () => {
  ...
  it('for coverage - 1', () => {
    const dragHeaderElem: Element = gridObj
      .getHeaderContent()
    ...
    const dropClone: HTMLElement = document.querySelector(
      '.e-cloneproperties.e-dragclone.e-headerclone'
    );
  ...
  it('for coverage - 2', () => {
    const dragHeaderElem: Element = gridObj
      .getHeaderContent()
      .querySelectorAll('.e-headercell')[1];
  ...
  it('for coverage - 3', () => {
    let dragGroupedElem: Element = gridObj.groupModule.element
      .querySelectorAll('.e-groupheadercell')[0]
      .querySelector('.e-drag');
    let dropGroupedElem: Element =
      gridObj.groupModule.element.querySelectorAll('.e-groupheadercell')[1];
    (gridObj.groupModule as any).helper({
      element: dragGroupedElem,
      sender: { target: dragGroupedElem },
    });
    const dropClone: HTMLElement = document.querySelector(
      '.e-cloneproperties.e-dragclone.e-gdclone'
    );
    ...
    dragGroupedElem =
      gridObj.groupModule.element.querySelectorAll('.e-groupheadercell')[0];
    dropGroupedElem =
      gridObj.groupModule.element.querySelectorAll('.e-groupheadercell')[1];
    (gridObj.groupModule as any).drag({
      element: gridObj.groupModule.element,
  ...
  it('for coverage - 4', () => {
    const dragGroupedElem: Element = gridObj.groupModule.element
      .querySelectorAll('.e-groupheadercell')[0]
      .querySelector('.e-drag');
    (gridObj.groupModule as any).helper({
      element: dragGroupedElem,
  ...
  it('for coverage - 5', () => {
    const dragHeaderElem: Element = gridObj
      .getHeaderContent()
      .querySelectorAll('.e-headercell')[3];
  ...
  it('for coverage - 6', () => {
    let dragGroupedElem: Element =
      gridObj.groupModule.element.querySelectorAll('.e-groupheadercell')[0];
    (gridObj.groupModule as any).helper({
      element: dragGroupedElem,
      sender: { target: dragGroupedElem },
    });
    const dropClone: Element = document.querySelector(
      '.e-cloneproperties.e-dragclone.e-gdclone'
    );
    (gridObj.groupModule as any).dragStart({
      dragElement: dropClone,
...
describe('Code Coverage - show grouped column edit grouped row reorder and Key board interaction => ', () => {
  ...
  it('for coverage - 1', () => {
    const dragRowObject: any = gridObj.getRowObjectFromUID(
      gridObj.getRowByIndex(0).getAttribute('data-uid')
    );
    const dropRowObject: any = gridObj.getRowObjectFromUID(
      gridObj.getRowByIndex(1).getAttribute('data-uid')
    );
    gridObj.groupModule.groupedRowReorder(dragRowObject, dropRowObject);
    gridObj.groupModule.updateExpand({
      uid: gridObj.getRowByIndex(1).getAttribute('data-uid'),
      isExpand: true,
    });
  });
  ...
  it('for coverage - 2', () => {
    ...
    const innerTarget: HTMLCollection =
      target.querySelectorAll('.e-groupheadercell')[0].children[0].children;
    const expandCollapseTarget: Element = gridObj
      .getContentTable()
      .querySelector('tr.e-groupcaptionrow td.e-recordplusexpand a');
    const groupHeaderElem: Element = gridObj
      .getHeaderContent()
      .querySelectorAll('.e-headercell')[2];
    (<any>gridObj.groupModule).keyPressHandler(
      getKeyActionObj('tab', innerTarget[2])
    );
    (<any>gridObj.groupModule).keyPressHandler(
      getKeyActionObj('shiftTab', innerTarget[3])
    );
    (<any>gridObj.groupModule).keyPressHandler(
      getKeyActionObj('enter', innerTarget[2])
    );
    (<any>gridObj.groupModule).keyPressHandler(
      getKeyActionObj('enter', innerTarget[3])
    );
    (<any>gridObj.focusModule).currentInfo.elementToFocus =
      expandCollapseTarget;
    (<any>gridObj.groupModule).keyPressHandler(
      getKeyActionObj('enter', expandCollapseTarget)
    );
    (<any>gridObj.groupModule).keyPressHandler(
      getKeyActionObj('enter', expandCollapseTarget)
    );
    ...
    (<any>gridObj.groupModule).keyPressHandler(
      getKeyActionObj('ctrlSpace', groupHeaderElem)
    );
    (<any>gridObj.groupModule).keyPressHandler(
      getKeyActionObj('ctrlSpace', groupHeaderElem)
    );
    (<any>gridObj.groupModule).clickHandler({
      target: expandCollapseTarget,
      preventDefault,
    });
    (<any>gridObj.groupModule).clickHandler({
      target: expandCollapseTarget,
    ...
    gridObj = createGrid(
      {
        ...
        columns: [
          ...
          },
          {
            ...
            width: 120,
            format: 'C2',
            validationRules: { required: true, min: 0, number: true },
          },
          {
            field: 'OrderDate',
            headerText: 'Order Date',
...
describe('Code Coverage => ', () => {
  ...
  it('Grouped refresh row checked', () => {
    (gridObj.groupModule as any).groupGenerator.refreshRows(
  ...
  describe('Code Coverage with grouping=> ', () => {
    ...
    it('refresh generateDataRows group', () => {
      (gridObj as any).inViewIndexes = [7, 8, 9, 10];
      (gridObj.groupModule as any).groupGenerator.generateDataRows(
        gridObj.dataSource,
        1
      );
    });
  ...
  describe('Code Coverage => ', () => {
    ...
    it('Focus onKeyPress coverage - 1', () => {
      e = {
        target: gridObj.element.querySelector('.e-groupdroparea'),
  ...
  describe('Code Coverage - 1=> ', () => {
    ...
    it('Focus skipOn coverage - 3', () => {
      e = {
        target: gridObj.element.querySelector('.e-toolbar-item'),
        preventDefault: new Function(),
        action: 'tab',
      };
    ...
    it('check groupcaption template class name', () => {
      expect(
        gridObj
          .getContent()
          .querySelectorAll('.e-groupcaption')[0]
          .classList.contains('e-freezeleftborder')
      ).toBeTruthy();
    });
  ...
  describe('EJ2-907737 - Height 100% is not working when dynamically changing properties', () => {
      ...
      gridObj = createGrid(
        {
          ...
          width: 200,
          columns: [
            { field: 'OrderID', headerText: 'Order ID', width: 180 },
            {
              field: 'CustomerID',
    ...
    it('Get the initial height', () => {
      contentHeight = (
        gridObj.element.querySelector('.e-gridcontent') as HTMLElement
      ).style.height;
  ...
  describe('Code Coverage - Group => ', () => {
    ...
    it('Click handler for text in descending', () => {
      (
        gridObj.groupModule.element.querySelector('.e-grouptext') as HTMLElement
      ).click();
    });

    it('Click handler for sort', () => {
      (
        gridObj.groupModule.element.querySelector('.e-groupsort') as HTMLElement
      ).click();
    });
  ...
  describe('Code Coverage - Group - expandCollapseRows => ', () => {
    ...
    it('Case 1', () => {
      (
        gridObj.element.querySelector('.e-detailrowcollapse') as HTMLElement
      ).click();
    });

    it('Case 2', () => {
      (
        gridObj.element.querySelector('.e-recordplusexpand') as HTMLElement
      ).click();
    });
  ...
  describe('Code Coverage - Group - expandCollapseRows showAddNewRow Bottom => ', () => {
    ...
    it('Case 2', () => {
      gridObj
        .getContentTable()
        .querySelector('tbody')
        .lastElementChild.classList.remove('e-addedrow');
      (
        gridObj.element.querySelector('.e-recordplusexpand') as HTMLElement
      ).click();
    });
  ...
  describe('Code Coverage - Group - addLabel => ', () => {
    ...
    it('columnDragStart', () => {
      const cell: HTMLElement = gridObj
        .getHeaderTable()
  ...
  describe('Code Coverage - Group - expandCollapseRows enableVirtualization => ', () => {
    ...
    it('Case 2', () => {
      (
        gridObj.element.querySelector('.e-recordpluscollapse') as HTMLElement
      ).click();
    });
  ...
  describe('Code Coverage - Group - React => ', () => {
    ...
    it('for coverage - 1', () => {
      (gridObj.groupModule as any).addOrRemoveFocus({
        target: gridObj.element,
      });
    });

    it('for coverage - 2', () => {
      const element = gridObj
        .getContentTable()
        .querySelector('.e-icon-gdownarrow');
      (gridObj.groupModule as any).auxilaryclickHandler({
        target: element,
        button: 1,
        preventDefault,
      });
    ...
    it('for coverage - 3', () => {
      ...
      (gridObj.groupModule as any).onGroupAggregates({
        OrderID: 10248,
        CustomerID: 'GG',
        Freight: 1,
      });
  ...
  describe('Code Coverage - Group - keyPressHandler => ', () => {
    ...
    it('Case 1', () => {
      (gridObj.groupModule as any).keyPressHandler({
        target: gridObj.groupModule.element.querySelector('.e-ungroupbutton'),
        action: 'tab',
      });
    });
