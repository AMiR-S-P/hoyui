 ...
 * Grid base spec
 */
import { L10n, EventHandler, select, Browser } from '@hoyui/base';
...
describe('Grid base module', () => {
  describe('Grid properties', () => {
    ...
    it('Row count testing', () => {
      expect(gridObj.element.querySelectorAll('.e-row').length).toBe(
        data.length
      );
    });

    it('Column count testing', () => {
      expect(gridObj.element.querySelectorAll('.e-headercell').length).toBe(
        gridObj.getColumns().length
      );
    });
    it('Content cell count testing', () => {
      expect(
        gridObj.element.querySelectorAll('.e-row')[0].childNodes.length
    ...
    it('enable altrow', (done: Function) => {
      let dataBound = (args: Object) => {
        expect(gridObj.getContent().querySelectorAll('.e-altrow').length).toBe(
          Math.floor(gridObj.currentViewData.length / 2)
        );
        done();
      };
      gridObj.dataBound = dataBound;
  ...
  describe('Method testing', () => {
    ...
    it('getColumnIndexByUid testing', () => {
      let col: number = gridObj.getColumnIndexByUid(
        gridObj.getColumnByField('OrderID').uid
      );
      expect(col).toBe(0);
      col = gridObj.getColumnIndexByUid(
        gridObj.getColumnByField('OrderID').uid + 'test'
      );
      expect(col).toBe(-1);
    });

    it('getUidByColumnField testing', () => {
      expect(gridObj.getUidByColumnField('OrderID')).toBe(
        gridObj.getColumnByField('OrderID').uid
      );
    });
    ...
    it('getColumnHeaderByIndex testing', () => {
      expect(
        gridObj.getColumnHeaderByIndex(1).querySelector('.e-headercelldiv')
          .textContent
      ).toBe('CustomerID');
      expect(
        gridObj
          .getMovableColumnHeaderByIndex(1)
          .querySelector('.e-headercelldiv').textContent
      ).toBe('CustomerID');
      expect(
        gridObj
          .getFrozenRightColumnHeaderByIndex(1)
          .querySelector('.e-headercelldiv').textContent
      ).toBe('CustomerID');
      expect(
        gridObj
  ...
  describe('Grid lines testing', () => {
    ...
    it('Grid line both testing', () => {
      expect(
        gridObj.element.classList.contains('e-horizontallines')
      ).toBeFalsy();
    ...
    it('Grid line vertical testing', () => {
      ...
      expect(
        gridObj.element.classList.contains('e-verticallines')
      ).toBeTruthy();
      expect(gridObj.element.classList.contains('e-hidelines')).toBeFalsy();
    });
  ...
  describe('Localization testing', () => {
    ...
    it('renderEmptyRow content testing', () => {
      expect(gridObj.element.querySelector('.e-emptyrow').textContent).toBe(
        'Geen records om te laten zien'
      );
    });
    ...
    it('get constant method testing', () => {
      expect(gridObj.localeObj.getConstant('EmptyRecord')).toBe(
        'Geen records om te laten zien'
      );
      //for coverage
  ...
  describe('grid scrollbar placeholder hide', () => {
    ...
    it('when total row height is smaller than Grid’s height', () => {
      let contentTable: HTMLElement = gridObj
        .getContent()
        .querySelector('.e-content') as HTMLElement;
      expect(contentTable.style.overflowY === 'scroll').toBeTruthy();
      if (
        gridObj.currentViewData.length * gridObj.getRowHeight() <
        gridObj.height
      ) {
        gridObj.hideScroll();
      }
  ...
  describe('Capturing keypress events', () => {
    ...
    it('pressing keycode for "a" and ctrlkey as true', (done: Function) => {
      ...
      EventHandler.trigger(gridObj.element, 'keydown', {
        keyCode: 65,
        ctrlKey: true,
      });
    });
  ...
  describe('Detail Module Functionalities', () => {
    ...
    it('Collapse method testing', () => {
      gridObj.detailRowModule.collapse(
        gridObj.getDataRows()[1].querySelector('.e-detailrowexpand')
      );
      expect(
        gridObj.getContentTable().querySelectorAll('.e-detailrow').length
      ).toBe(0);
    });

    it('Expand method testing', () => {
      gridObj.detailRowModule.expand(
        gridObj.getDataRows()[1].querySelector('.e-detailrowcollapse')
      );
      expect(
        gridObj.getContentTable().querySelectorAll('.e-detailrow').length
      ).toBe(1);
  ...
  let dataSource = [
    {
      ...
      Customer: {
        ID: 'VINET',
        Name: { FirstName: 'Dijin', LastName: 'Jones' },
      },
      EmployeeID: 5,
      OrderDate: new Date(8364186e5),
    },
    {
      ...
      Customer: {
        ID: 'TOMSP',
        Name: { FirstName: 'Vins', LastName: 'Chevalier' },
      },
      EmployeeID: 6,
      OrderDate: new Date(836505e6),
    },
    {
      ...
      Customer: {
        ID: 'HANAR',
        Name: { FirstName: 'Hilarton', LastName: 'Abastos' },
      },
      EmployeeID: 4,
      OrderDate: new Date(8367642e5),
    },
  ...
  it('check stacked header clipmode', () => {
    expect(
      gridObj
        ...
        .classList.contains('e-ellipsistooltip')
    ).toBeTruthy();
    expect(
      gridObj
        .getHeaderContent()
    ...
    expect(
      gridObj.getHeaderContent().querySelectorAll('.e-ellipsistooltip').length
    ).toBe(8);
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('clipMode testing in grid level', () => {
  ...
  it('class testing', () => {
    ...
    expect(
      row[0][0].cells[2].classList.contains('e-ellipsistooltip')
    ).toBeTruthy();
    expect(
      row[0][0].cells[3].classList.contains('e-ellipsistooltip')
    ).toBeTruthy();
    expect(
      row[0][0].cells[1].classList.contains('e-ellipsistooltip')
    ).toBeFalsy();
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('EJ2-37293-double click in row-drag-n-drop icon to get row object', () => {
  ...
  it('double click in row-drag-n-drop icon', (done: Function) => {
    ...
    (gridObj as any).dblClickHandler({
      target: gridObj.element.querySelector('.e-rowdragdrop'),
    });
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('EJ2-46527 - Server export coverage', function () {
  ...
  it('Calling export functions', function () {
    ...
    getNumberFormat(
      (gridObj as any).getFormat(column.format),
      column.type,
      true
    );
  });
...
describe('Normal Grid Mask Row', () => {
  ...
  it('Show Mask Row', () => {
    gridObj.showMaskRow();
    (gridObj as any).translateMaskRow({
      target: gridObj.getContent().firstChild,
    });
  ...
  it('Refresh Mask Row', () => {
    ...
    expect(
      gridObj.getContent().querySelector('.e-masked-table').clientWidth
    ).toBe(
      gridObj.getContent().querySelector('.e-table:not(.e-masked-table)')
        .clientWidth
    );
  });
  it('Remove Mask Row', () => {
...
describe('EJ2-68404 - Missing Cancel icon in Reactive Aggregate sample in toolbar button.', () => {
  ...
  it('Select element outside grid to check cancel icon', () => {
    let inputEle: HTMLElement = document.body.appendChild(
      createElement('input', { id: 'TestInput' })
    );
    let e: any = { target: inputEle };
    (<any>gridObj).focusOutHandler(e);
    expect(
      document
        .querySelector('.e-toolbar')
        .querySelector('.e-tbar-btn')
...
describe('EJ2-35549 - When dynamically change group Settings,sortSettings and columns causes the script error', () => {
  ...
  it('check queryCellInfo class name - 1', () => {
    rows = gridObj.getRows();
    expect(
      (rows[1] as HTMLTableRowElement).cells[3].classList.contains('e-colour')
    ).toBeTruthy();
  });
  ...
  it('check queryCellInfo class name - 2', () => {
    rows = gridObj.getRows();
    expect(
      (rows[1] as HTMLTableRowElement).cells[2].classList.contains('e-colour')
    ).toBeFalsy();
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('Provide XSS- security for Grid =>', () => {
        ...
        dataSource: [
          {
            ...
            ShipCity: 'Münster',
            ShipName:
              '<img id="target" src="x" onerror="alert(document.domain)">',
          },
          {
            OrderID: 10249,
            ShipCity: 'Luisenstr',
            ShipName:
              '<p><strong>Environmentally friendly</strong> or <strong>environment-friendly</strong>',
          },
          {
            OrderID: 10250,
            ShipCity: 'Rio de Janeiro',
            ShipName:
              'from the tow at "low" altitude and turned back toward the gliderport when the nose of the glider pointed “down," and the glider descended',
          },
        ],
  ...
  it('test the html sanitizer', () => {
    expect((gridObj.getRowByIndex(0) as any).cells[1].innerHTML).toBe(
      '<img id="target" src="x">'
    );
  });
  ...
  it('Edited value check', (done: Function) => {
    actionComplete = (args?: any): void => {
      if (args.requestType === 'save') {
        expect((gridObj.currentViewData[2] as any).ShipName).toBe(
          '<img id="target" src="x">'
        );
        done();
      }
    ...
    gridObj.actionComplete = actionComplete;
    (
      select('#' + gridObj.element.id + 'ShipName', gridObj.element) as any
    ).value = '<img id="target" src="x" onerror="alert(document.domain)">';
...
describe('get edit template =>', () => {
  ...
  it('execute methods 1', (done) => {
    ...
    expect((gridObj as any).calculatePageSizeByParentHeight('100%')).not.toBe(
      0
    );
    expect(gridObj.getHeight('100%')).not.toBe('100%');
    expect(
      (gridObj as any).setHeaderText(gridObj.columns, 'field').length
...
describe('dateonly =>', () => {
    ...
    gridObj = createGrid(
      {
        dataSource: new DataManager([
          { OrderID: 10248, OrderDate: '2023-04-04' },
        ]),
        allowPaging: true,
        allowGrouping: true,
        allowFiltering: true,
  ...
  it('execute methods 1', (done: Function) => {
    (gridObj.filterModule as any).filterIconClickHandler(
      getClickObj(
        gridObj
          .getColumnHeaderByField('OrderDate')
...
describe('Focusing  code coverage =>', () => {
  ...
  it('for coverage - 1', () => {
    ...
    parents(
      gridObj.element.querySelector('.e-content') as Element,
      gridObj.element.id,
      true
    );
  });
...
describe('Focusing  code coverage =>', () => {
  ...
  it('for coverage - 10', () => {
    (gridObj.focusModule as any).header.getNextCurrent(
      null,
      null,
      null,
      'shiftRight'
    ...
    (gridObj.focusModule as any).content.getNextCurrent(
      [1, 0],
      null,
      null,
      'tab'
...
describe('Focusing  code coverage =>', () => {
  ...
  it('for coverage - 13', () => {
    ...
    (gridObj.focusModule as any).handleFilterNavigation(
      e,
      '.e-rowcell',
      '.e-rowcell'
    );
  ...
  it('for coverage - 14', () => {
    let e: object = {
      target: gridObj.element.querySelector('.e-numericitem'),
      action: 'shiftTab',
      keyCode: 38,
      preventDefault,
    };
    ...
    (gridObj.focusModule as any).onKeyPress(e);
    let elem: Element = (gridObj.pagerModule as any).element.querySelector(
...
describe('Util  code coverage =>', () => {
  ...
  it('Conetnt-renderer drop coverage', () => {
    let e = {
      target: gridObj.element.querySelector('.e-rowcell'),
      droppedElement: gridObj.element.querySelector('.e-rowcell'),
    };
    (gridObj as any).contentModule.drop(e);
  });
  it('for coverage - canSkip', () => {
    (gridObj as any).contentModule.canSkip(
      gridObj.columns[0],
...
describe('Grid Public properties null or undefined value testing', () => {
  ...
  it('selectionSettings', () => {
    ...
    expect(gridObj.selectionSettings.enableSimpleMultiRowSelection).toBe(
      undefined
    );
...
describe('Code Coverage feature - 1 =>', () => {
  ...
  describe('Code Coverage feature - 2 =>', () => {
    ...
    beforeAll((done: Function) => {
      ...
      document.body.appendChild(
        createElement('div', { innerHTML: templete, id: 'toolbar' })
      );
      grid = createGrid(
        {
          dataSource: filterData,
    ...
    it('focus focusVirtualElement and focusOutFromHeader coverage', () => {
      ...
      (grid as any).headerModule.createTable(
        grid.element.querySelector('.e-headercontent')
      );
  ...
  describe('Code Coverage feature - 3 =>', () => {
    ...
    it('onkeypress shiftTab key coverage', () => {
      (grid as any).focusModule.onKeyPress({
        target: grid.element,
        action: 'shiftTab',
        preventDefault: preventDefault,
      });
      (grid as any).focusModule.onKeyPress({
        target: grid.element.querySelector('.e-groupdroparea'),
        action: 'shiftTab',
        preventDefault: preventDefault,
      });
      grid.groupSettings.showDropArea = false;
      (grid as any).focusModule.onKeyPress({
        target: grid.element.querySelector('.e-toolbar'),
        action: 'shiftTab',
        preventDefault: preventDefault,
      });
    });
  ...
  describe('coverage improvement row-reorder file - 1', () => {
    ...
    it('row reorder file method coverage', () => {
      ...
      gridObj.isDestroyed = false;
      (gridObj.rowDragAndDropModule as any).singleRowDrop({
        target: gridObj.element.querySelector('.e-rowcell'),
        droppedElement: gridObj.element.firstElementChild,
      });
      ...
      (gridObj.rowDragAndDropModule as any).setScrollDown(
        gridObj.element.querySelector('.e-content'),
        2
      );
      (gridObj.rowDragAndDropModule as any).startedRow =
  ...
  describe('coverage improvement row-reorder file - 2', () => {
    ...
    it('row reorder file method coverage - 2', () => {
      ...
      gridObj.detailRowModule.expand(
        gridObj.getDataRows()[3].querySelector('.e-detailrowcollapse')
      );
    });
...
describe('EJ2-899559 - Column jumping issue on column resizing with minWidth and width', () => {
  ...
  it('Ensure 1st Column Width ', () => {
    const cols: HTMLCollection = gridObj
      .getHeaderTable()
