...
import { L10n, Internationalization } from '@hoyui/base';
...
describe('ValueFormatter Service', () => {
  ...
  describe('Format check - default locale', () => {
      ...
      grid = createGrid(
        {
          columns: [
            {
              field: 'OrderID',
              headerText: '<i>Order ID</i>',
              headerTextAlign: 'Right',
              disbleHtmlEncode: false,
              textAlign: 'Right',
            },
    ...
    it('Number format check', () => {
      rows = (grid.getContentTable() as any).tBodies[0]
    ...
    it('Date format check', () => {
      ...
      expect(rows.cells[3].innerHTML).toBe(
        intl.formatDate(new Date(8364186e5), { type: 'date', skeleton: 'yMd' })
      );
    });
    afterAll(() => {
      destroy(grid);
  ...
  describe('fromView method check', () => {
    ...
    it('check number format', () => {
      let fmtr: IValueFormatter =
        grid.serviceLocator.getService<IValueFormatter>('valueFormatter');
      expect(
        fmtr.fromView(
          '$32.0',
          (grid.getColumns() as Column[])[2].getParser(),
          'number'
        )
      ).toBe(32);
    });
    ...
    it('check format with no target type', () => {
      ...
      expect(
        fmtr.fromView(
          '$32.0',
          (grid.getColumns() as Column[])[1].getParser(),
          'custom'
        )
      ).toBe('$32.0');
      //for coverage
