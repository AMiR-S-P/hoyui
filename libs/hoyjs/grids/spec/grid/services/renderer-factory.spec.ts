...
import { getEnumValue } from '@hoyui/base';
  ...
  describe('Register and get service', () => {
    ...
    it('Check fallback', () => {
      expect(() =>
        gridObj.serviceLocator
          ...
          .getRenderer(RenderType.Summary)
      ).toThrow('The renderer Summary is not found');
    });
    ...
    it('Check register', () => {
      ...
      let factory: RendererFactory =
        gridObj.serviceLocator.getService<RendererFactory>('rendererFactory');
      factory.addRenderer(RenderType.Summary, new RenderMock());
      factory.addRenderer(RenderType.Summary, new DupRenderMock());
      expect(
        <string>getEnumValue(RenderType, RenderType.Summary) in
          factory.rendererMap
      ).toBeTruthy();
      expect(
        factory.getRenderer(RenderType.Summary) instanceof RenderMock
      ).toBeTruthy();
    });
    it('memory leak', () => {
      profile.sample();
