...
import { L10n, EmitType } from '@hoyui/base';
...
describe('Pager base module', () => {
  describe('Pager properties testing', () => {
    ...
    beforeAll((done: Function) => {
      ...
      pagerObj = new Pager({
        ...
        pageSize: 5,
        locale: 'de-DE',
        enablePagerMessage: true,
        enableExternalMessage: true,
    ...
    it('current page testing', () => {
      expect(
        pagerObj.element.querySelectorAll('.e-active')[0].getAttribute('index')
      ).toBe('8');
    });

    it('page count testing', () => {
      expect(
        pagerObj.element.querySelectorAll('.e-numericcontainer')[0].childNodes
          .length
      ).toBe(5);
    });

    it('enable pager message element testing', () => {
      expect(pagerObj.element.querySelectorAll('.e-parentmsgbar').length).toBe(
        1
      );
    });
    ...
    it('enable pager message testing', () => {
      expect(
        pagerObj.element.querySelectorAll('.e-parentmsgbar')[0].textContent
      ).toBe('8 van 20 pagina ( 100 items)');
    });

    it('enable pager external message element testing', () => {
      expect(
        pagerObj.element.querySelectorAll('.e-pagerexternalmsg').length
      ).toBe(1);
    });

    it('enable pager external message testing', () => {
      expect(
        pagerObj.element.querySelectorAll('.e-pagerexternalmsg')[0].textContent
    ...
    it('custom text testing', () => {
      expect(
        pagerObj.element.querySelectorAll('.e-active')[0].textContent
      ).toBe('sheet8');
    });
    ...
    it('pager aria-attribute testing', () => {
      ...
      expect(
        pagerElement.querySelector('.e-mfirst').hasAttribute('tabindex')
      ).toBeTruthy();
      expect(
        pagerElement.querySelector('.e-mprev').hasAttribute('tabindex')
      ).toBeTruthy();
      let pagerContainer: Element =
        pagerObj.element.querySelector('.e-pagercontainer');
      let numericContainer: Element = pagerObj.element.querySelector(
        '.e-numericcontainer'
      );
      expect(
        pagerContainer.querySelector('.e-first').hasAttribute('tabindex')
      ).toBeTruthy();
      expect(
        pagerContainer.querySelector('.e-prev').hasAttribute('tabindex')
      ).toBeTruthy();
      for (let i: number; i < numericContainer.children.length; i++) {
        expect(
          numericContainer.children[i].hasAttribute('aria-label')
        ).toBeTruthy();
        expect(
          numericContainer.children[i].hasAttribute('tabindex')
        ).toBeTruthy();
      }
      expect(
        pagerElement.querySelector('.e-mnext').hasAttribute('tabindex')
      ).toBeTruthy();
      expect(
        pagerElement.querySelector('.e-mlast').hasAttribute('aria-label')
      ).toBeTruthy();
    });
  ...
  describe('Method testing', () => {
    ...
    it('getLocalizedLabel testing', () => {
      expect(pagerObj.getLocalizedLabel('firstPageTooltip')).toBe(
        'Go to first page'
      );
    });
  ...
  describe('pager onproperty changed', () => {
    ...
    it('enablePagerMessage false testing', () => {
      ...
      pagerObj.dataBind();
      expect(
        (pagerObj.element.querySelectorAll('.e-parentmsgbar')[0] as HTMLElement)
          .style.display
      ).toBe('');
    });
    ...
    it('locale testing', () => {
      ...
      expect(
        pagerObj.element.querySelectorAll('.e-parentmsgbar')[0].textContent
      ).toBe('15 van 34 pagina ( 200 items)');
    });
  ...
  describe('pager template refresh', () => {
    ...
    it('pager template refresh testing', () => {
      pagerObj.template =
        '<span class ="e-pagenomsg">${currentPage} of ${totalPages} pages</span>';
      pagerObj.dataBind();
      expect(
        pagerObj.element.querySelectorAll('.e-pagenomsg')[0].textContent
      ).toBe('1 of 9 pages');
    });
  ...
  describe('Custom pager text tested', () => {
    ...
    it('pager text testing', () => {
      expect(pagerObj.element.querySelector('.e-pagenomsg').textContent).toBe(
        '8 of 20 pages - 100 '
      );
    });
  ...
  describe('EJ2-822821 - Need to render the pager based on the Dom width', () => {
    ...
    it('Code coverage case for resized method in pager component', function () {
      pagerElements = pagerObj.element.querySelectorAll(
        '.e-mfirst, .e-mprev, .e-icon-first, .e-icon-prev, .e-pp:not(.e-disable), .e-icon-next, .e-icon-last, .e-parentmsgbar, e-mnext, e-mlast, .e-pagerdropdown, .e-pagerconstant'
      );
      ...
      expect(
        pagerObj.element
          .querySelectorAll(
            '.e-numericitem:not(.e-hide):not(.e-np):not(.e-pp)'
          )[29]
    ...
    it('Code coverage case for resized method in pager component window reduced to current page', function () {
      ...
      pagerObj.dataBind();
      expect(
        pagerObj.element.querySelector('.e-active').classList.contains('e-hide')
      ).toBeFalsy();
    });
  ...
  describe('EJ2-832882 - Show and Hide Pager message elements dynamically when no numeric Items left to hide.', () => {
    ...
    it('check whether the pager message is hidden or not', function () {
      ...
      expect(
        (
          pagerObj.element.querySelector('.e-parentmsgbar') as HTMLElement
        ).classList.contains('e-hide')
      ).toBeTruthy();
    });
  ...
  describe('EJ2-832882 - Show and Hide Pager message elements dynamically when no numeric Items left to hide.', () => {
    ...
    it('check whether the pager message is shown when increasing window size or not', function () {
      pagerElements = pagerObj.element.querySelectorAll(
        '.e-mfirst, .e-mprev, .e-icon-first, .e-icon-prev, .e-pp:not(.e-disable), .e-icon-next, .e-icon-last, .e-parentmsgbar, e-mnext, e-mlast, .e-pagesizes'
      );
      pagerObj.element.querySelector('.e-np').classList.remove('e-disable');
      for (var i = 0; i < pagerElements.length; i++) {
        pagerElements[i].style.width = '25px';
      ...
      expect(
        (
          pagerObj.element.querySelector('.e-pagesizes') as HTMLElement
        ).classList.contains('e-hide')
      ).toBeFalsy();
      pagerElements = pagerObj.element.querySelectorAll(
        '.e-mfirst, .e-mprev, .e-icon-first, .e-icon-prev, .e-pp:not(.e-disable), .e-icon-next, .e-icon-last, .e-parentmsgbar, e-mnext, e-mlast, .e-pagerdropdown , .e-pagerconstant'
  ...
  describe('EJ2-838374 - Pager information is wrong while dynamically changing the Grid width.', () => {
    ...
    it('checking pager message is hidden/Shown when dynamically changing pager width', function () {
      pagerElements = pagerObj.element.querySelectorAll(
        '.e-mfirst, .e-mprev, .e-icon-first, .e-icon-prev, .e-pp:not(.e-disable), .e-icon-next, .e-icon-last, e-mnext, e-mlast'
      );
      pagerObj.element.querySelector('.e-np').classList.remove('e-disable');
      (
        pagerObj.element.querySelector('.e-mfirst') as HTMLElement
      ).style.display = 'none';
      for (var i = 0; i < pagerElements.length; i++) {
        pagerElements[i].style.width = '25px';
      }
