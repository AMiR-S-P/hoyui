...
export class PagerDropDown {
  ...
  public render(): void {
    ...
    const dropDownDiv: Element = createElement('div', {
      className: 'e-pagerdropdown',
    });
    const defaultTextDiv: Element = createElement('div', {
    ...
    const pageSizesModule: boolean | (number | string)[] =
      this.pagerModule.pageSizes;
    const pageSizesArray: string[] = (
      (<string[]>pageSizesModule).length
        ? this.convertValue(pageSizesModule as string[])
    ...
    this.dropDownListObject = new DropDownList({
      ...
      cssClass: this.pagerModule.cssClass
        ? 'e-alldrop' + ' ' + this.pagerModule.cssClass
        : 'e-alldrop',
    });
    this.dropDownListObject.appendTo(input);
    if ((<string[]>pageSizesModule).length) {
      (<HTMLInputElement>this.dropDownListObject.element).value =
        this.pagerModule.pageSize.toString();
    }
  ...
  private onChange(e: ChangeEventArgs): void {
    if (
      this.dropDownListObject.value ===
      this.pagerModule.getLocalizedLabel('All')
    ) {
      ...
      if (document.getElementsByClassName('e-popup-open e-alldrop').length) {
        (<HTMLElement>(
          document.getElementsByClassName('e-popup-open e-alldrop')[0]
        )).style.display = 'none';
      }
    } else {
      this.pagerModule.pageSize = parseInt(
        this.dropDownListObject.value as string,
        10
      );
      this.pagerModule.isAllPage = false;
      if (
        this.pagerCons.innerHTML !==
    ...
    if (!this.pagerModule.isCancel) {
      this.pagerModule.trigger('dropDownChanged', {
        pageSize:
          ...
          (this.pagerModule.isAllPage === undefined &&
            this.dropDownListObject.value ===
              this.pagerModule.getLocalizedLabel('All'))
  ...
  private convertValue(
    ...
    for (let i: number = 0; i < item.length; i++) {
      item[parseInt(i.toString(), 10)] = parseInt(
        item[parseInt(i.toString(), 10)] as string,
        10
      )
        ? item[parseInt(i.toString(), 10)].toString()
        : this.pagerModule.getLocalizedLabel(
            item[parseInt(i.toString(), 10)] as string
          ) !== ''
        ? this.pagerModule.getLocalizedLabel(
            item[parseInt(i.toString(), 10)] as string
          )
        : item[parseInt(i.toString(), 10)];
    }
    return item as string[];
  }
  private isPageSizeAll(value: string | number): boolean {
    const pageSizeNum: string | number =
      typeof value === 'string' &&
      value !== this.pagerModule.getLocalizedLabel('All')
        ? parseInt(value, 10)
        : value;
    if (
      pageSizeNum === this.pagerModule.totalRecordsCount ||
      value === this.pagerModule.getLocalizedLabel('All')
    ) {
      return true;
    } else {
  ...
  public setDropDownValue(prop: string, value: string | number): void {
    if (this.dropDownListObject) {
      ...
      this.pagerModule.checkAll =
        isbeforeAll && this.pagerModule.isAllPage ? true : false;
      this.dropDownListObject[`${prop}`] = this.pagerModule.isAllPage
        ? this.pagerModule.getLocalizedLabel('All')
        : value;
    }
  }
  public addEventListener(): void {
    this.dropDownListObject.on(
      'beforeValueChange',
