  ...
  implements INotifyPropertyChanged
{
  ...
  public onPropertyChanged(newProp: PagerModel, oldProp: PagerModel): void {
    ...
    if (
      (newProp.pageSize as number | string) === this.getLocalizedLabel('All') &&
      oldProp.pageSize === this.totalRecordsCount
    ) {
          ...
          if (newProp.pageSize !== oldProp.pageSize) {
            ...
            if (this.isCancel && this.hasParent) {
              (
                ...
                { pageSettings: { pageSize: oldProp.pageSize } },
                true
              );
            }
          } else {
    ...
    if (isNullOrUndefined(focusedTabIndexElement)) {
      ...
      if (target === this.element && !this.hasParent) {
        const focusablePagerElements: Element[] =
          this.getFocusablePagerElements(this.element, []);
  ...
  private onFocusOut(e: FocusEvent): void {
    ...
    if (
      this.pageSizes &&
      dropDownPage &&
      dropDownPage.classList.contains('e-input-focus')
    ) {
  ...
  private keyPressHandler(e: keyPressHandlerKeyboardEventArgs): void {
    const presskey: keyPressHandlerKeyboardEventArgs = <
      keyPressHandlerKeyboardEventArgs
  ...
  public setPagerFocus(): void {
    const focusablePagerElements: Element[] = this.getFocusablePagerElements(
      this.element,
      []
    );
    if (focusablePagerElements.length > 0) {
  ...
  private setTabIndexForFocusLastElement(): void {
    ...
    if (
      this.pageSizes &&
      dropDownPage &&
      !isNullOrUndefined((dropDownPage as HTMLElement).offsetParent)
    ) {
      (dropDownPage as HTMLElement).tabIndex = 0;
    } else if (focusablePagerElements.length > 0) {
      (
        focusablePagerElements[focusablePagerElements.length - 1] as HTMLElement
      ).tabIndex = 0;
    }
  }
  ...
  private getFocusedTabindexElement(): Element {
    ...
    const tabindexElements: NodeListOf<Element> = this.element.querySelectorAll(
    ...
    for (let i: number = 0; i < tabindexElements.length; i++) {
      ...
      if (
        element &&
        (element.classList.contains('e-focused') ||
          element.classList.contains('e-input-focus'))
      ) {
    ...
    if (focusablePagerElements.length > 0) {
      if (
        ...
        for (let i: number = 0; i < focusablePagerElements.length; i++) {
          if (
            currentItemPagerFocus ===
            focusablePagerElements[parseInt(i.toString(), 10)]
          ) {
            const incrementNumber: number = i + 1;
            if (incrementNumber < focusablePagerElements.length) {
              e.preventDefault();
              (
                focusablePagerElements[
                  parseInt(incrementNumber.toString(), 10)
                ] as HTMLElement
              ).focus();
            }
            break;
      ...
      for (let i: number = 0; i < focusablePagerElements.length; i++) {
        if (
          ...
          if (decrementNumber >= 0) {
            ...
            (
              focusablePagerElements[
                parseInt(decrementNumber.toString(), 10)
              ] as HTMLElement
            ).focus();
          } else if (this.hasParent) {
            const rows: Element[] = (
              this.parent as { getRows(): Element[] }
            ).getRows();
  ...
  private navigateToPageByEnterOrSpace(e: KeyboardEventArgs): void {
    ...
    if (currentItemPagerFocus) {
      ...
      if (
        (selectedClass === 'e-first' ||
          selectedClass === 'e-prev' ||
          selectedClass === 'e-next' ||
          selectedClass === 'e-last' ||
          selectedClass === 'e-pp' ||
          selectedClass === 'e-np') &&
        classElement &&
        !classElement.classList.contains('e-disable')
      ) {
  ...
  private navigateToPageByKey(e: KeyboardEventArgs): void {
    ...
    const currentItemPagerFocus: Element = this.getFocusedElement();
    if (
      !isNullOrUndefined(pagingItem) &&
      pagingItem.hasAttribute('index') &&
  ...
  private checkFocusInAdaptiveMode(element: Element): boolean {
    ...
    return selectedClass === 'e-mfirst' ||
      selectedClass === 'e-mprev' ||
      selectedClass === 'e-mnext' ||
      selectedClass === 'e-mlast'
      ? true
      : false;
  }
  ...
  public getDropDownPage(): Element {
    const dropDownPageHolder: Element =
      this.element.querySelector('.e-pagerdropdown');
  ...
  private getClass(element: Element): string {
    ...
    for (let i: number = 0; i < classList.length; i++) {
      if (
        element &&
        element.classList.contains(classList[parseInt(i.toString(), 10)])
      ) {
   ...
   * @hidden */
  public getFocusablePagerElements(
    element: Element,
    ...
    for (let i: number = 0; i < targetChildrens.length; i++) {
      ...
      if (
        ...
        !element.classList.contains('e-pagesizes')
      ) {
        pagerElements = this.getFocusablePagerElements(element, pagerElements);
      } else {
        const tabindexElement: Element =
          targetChildrens[parseInt(i.toString(), 10)];
        if (
          tabindexElement.hasAttribute('tabindex') &&
          !element.classList.contains('e-disable') &&
  ...
  private currentPageChanged(newProp: PagerModel, oldProp: PagerModel): void {
    ...
    if (
      newProp.currentPage !== oldProp.currentPage ||
      newProp.pageSize !== oldProp.pageSize
    ) {
      const args: {
        currentPage: number;
        newProp: PagerModel;
      ...
      if (!args.cancel) {
        ...
        if (oldProp && oldProp.pageSize) {
          ...
          if (this.pagerdropdownModule) {
            this.pagerdropdownModule.setDropDownValue(
              'value',
              oldProp.pageSize
            );
            this.pagerdropdownModule['dropDownListObject'].text =
              oldProp.pageSize + '';
          }
        }
      }
  ...
  public updateTotalPages(): void {
    this.totalPages = this.isAllPage
      ? 1
      : this.totalRecordsCount % this.pageSize === 0
      ...
      try {
        if (typeof template === 'function') {
          ...
          if (document.querySelectorAll(template).length) {
            this.templateFn = templateCompiler(
  ...
  public refresh(): void {
    if (this.template) {
      ...
      if (
        focusedTabIndexElement &&
        focusedTabIndexElement.classList.contains('e-disable')
      ) {
  ...
  private updateQueryString(value: number): void {
    const updatedUrl: string = this.getUpdatedURL(
  ...
  private getUpdatedURL(uri: string, key: string, value: string): string {
    ...
    if (uri.match(regx)) {
      ...
      return (
        uri + (uri.indexOf('?') !== -1 ? '&' : '?') + key + '=' + value + hash
      );
    }
  }
  ...
  private addAriaLabel(): void {
    ...
    if (!Browser.isDevice) {
      for (let i: number = 0; i < classList.length; i++) {
        const element: Element = this.element.querySelector(
          classList[parseInt(i.toString(), 10)]
        );
  ...
  private calculateActualWidth(): number {
    const pagerElements: NodeListOf<HTMLElement> =
    ...
    for (let i: number = 0; i < pagerElements.length; i++) {
      if (
        getComputedStyle(pagerElements[parseInt(i.toString(), 10)]).display !==
        'none'
      ) {
        actualWidth +=
          pagerElements[parseInt(i.toString(), 10)].offsetWidth +
          parseFloat(
            getComputedStyle(pagerElements[parseInt(i.toString(), 10)])
              .marginLeft
          ) +
          parseFloat(
            getComputedStyle(pagerElements[parseInt(i.toString(), 10)])
    ...
    const pagerContainer: HTMLElement =
      this.element.querySelector('.e-pagercontainer');
    actualWidth +=
      parseFloat(getComputedStyle(pagerContainer).marginLeft) +
  ...
  private resizePager(): void {
    ...
    if (!this.template && isStyleApplied) {
      ...
      const hiddenNumItems: NodeListOf<HTMLElement> =
        pagerContainer.querySelectorAll(
          '.e-numericitem.e-hide:not([style*="display: none"])'
        );
      const hideFrom: number = numItems.length;
      const showFrom: number = 1;
      const bufferWidth: number = !isNullOrUndefined(
        parentsUntil(this.element, 'e-bigger')
      )
        ? 10
        : 5;
      const NP: HTMLElement = pagerContainer.querySelector('.e-np');
      const PP: HTMLElement = pagerContainer.querySelector('.e-pp');
      const detailItems: NodeListOf<HTMLElement> =
        this.element.querySelectorAll(
          '.e-parentmsgbar:not(.e-hide):not([style*="display: none"]), .e-pagesizes:not(.e-hide):not([style*="display: none"])'
        );
      ...
      for (let i: number = 0; i < numItems.length; i++) {
        totalWidth +=
          numItems[parseInt(i.toString(), 10)].offsetWidth +
          parseFloat(
            getComputedStyle(numItems[parseInt(i.toString(), 10)]).marginLeft
          ) +
          parseFloat(
            getComputedStyle(numItems[parseInt(i.toString(), 10)]).marginRight
          );
      }
      ...
      if (
        pagerWidth > 0 &&
        actualWidth >= pagerWidth - (numericItemWidth ? numericItemWidth : 0)
      ) {
        this.isPagerResized = true;
        if (this.currentPage !== this.totalPages) {
          classList(
            NP,
            ['e-numericitem', 'e-pager-default'],
            ['e-nextprevitemdisabled', 'e-disable']
          );
        }
        ...
        for (let i: number = 1; i <= numToHide; i++) {
          ...
          if (
            this.currentPage !== 1 &&
            (parseInt(
              numItems[Math.abs(hideIndex)].getAttribute('index'),
              10
            ) === this.currentPage ||
              parseInt(
                numItems[numItems.length - 1].getAttribute('index'),
                10
              ) === this.currentPage)
          ) {
            hideIndex = 0;
          ...
          if (
            numItems[Math.abs(hideIndex)] &&
            !numItems[Math.abs(hideIndex)].classList.contains('e-currentitem')
          ) {
        ...
        if (
          ...
          if (pagerDetailItemsWidth > pagerWidth - bufferWidth) {
            let detailtoHide: number = Math.floor(
              (pagerWidth - (pagerDetailItemsWidth - totDetailWidth)) /
            ...
            for (
              let i: number = 0;
              i <
              (detailtoHide > detailItems.length
                ? detailItems.length
                : detailtoHide);
              i++
            ) {
       ...
       */
        ...
        const hiddenDetailItems: NodeListOf<HTMLElement> =
          this.element.querySelectorAll(
            '.e-parentmsgbar.e-hide, .e-pagesizes.e-hide'
          );
        // To show Pager message elements.
        if (
          hiddenDetailItems.length &&
          diff > this.averageDetailWidth + this.averageDetailWidth / 4
        ) {
          hiddenDetailItems[hiddenDetailItems.length - 1].classList.remove(
            'e-hide'
          );
        }
        if (
          diff > numericItemWidth * 2 &&
          !hiddenDetailItems.length &&
          window.innerWidth >= 768
        ) {
          // To calculate number of numeric items need to be shown.
          let numToShow: number = Math.floor(
            diff / (numericItemWidth + bufferWidth)
          );
          numToShow =
            numToShow > hiddenNumItems.length
              ? hiddenNumItems.length
              : numToShow - 1;
          //Seggregating hidden num items as less index and greater index values than current page value.
          const lesserIndexItems: HTMLElement[] = Array.from(hiddenNumItems)
            .filter(
              (item: HTMLElement) =>
                parseInt(item.getAttribute('index'), 10) < this.currentPage
            )
            .sort(
              (a: HTMLElement, b: HTMLElement) =>
                parseInt(b.getAttribute('index'), 10) -
                parseInt(a.getAttribute('index'), 10)
            );
          const greaterIndexItems: HTMLElement[] = Array.from(
            hiddenNumItems
          ).filter(
            (item: HTMLElement) =>
              parseInt(item.getAttribute('index'), 10) > this.currentPage
          ...
          let showItems: HTMLElement[] =
            (lesserIndexItems.length && lesserIndexItems) ||
            (greaterIndexItems.length && greaterIndexItems);

          for (let i: number = 1; i <= numToShow; i++) {
            const showItem: HTMLElement =
              showItems && showItems[Math.abs(showFrom - i)];
      ...
      if (numItems.length) {
        ...
        const isLastSet: boolean = Array.from(numItems).some(
          (item: HTMLElement) =>
            parseInt(item.getAttribute('index'), 10) === this.totalPages
        );
        const ppIndex: number =
          parseInt(numItems[0].getAttribute('index'), 10) -
          (isLastSet ? this.avgNumItems : numItems.length);
       ...
       */
      if (
        this.element.offsetWidth < 769 &&
        window
          .getComputedStyle(this.element.querySelector('.e-mfirst'))
          .getPropertyValue('display') !== 'none' &&
        this.pageSizes
      ) {
        this.element.querySelector('.e-pagesizes').classList.remove('e-hide');
        this.element
          .querySelector('.e-parentmsgbar')
          .classList.remove('e-hide');
        this.element.classList.add('e-adaptive');
        this.element.querySelector('.e-pagenomsg').innerHTML =
          this.element.offsetWidth < 481
            ? this.currentPage + ' / ' + this.totalPages
            : this.pagerMessageModule.format(
                this.getLocalizedLabel('currentPageInfo'),
                [
                  this.totalRecordsCount === 0 ? 0 : this.currentPage,
                  this.totalPages || 0,
