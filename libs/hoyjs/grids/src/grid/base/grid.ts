...
import { ItemModel, ClickEventArgs } from '@hoyui/navigations';
import {
  createSpinner,
  hideSpinner,
  showSpinner,
...
import {
  ...
  ExportDetailDataBoundEventArgs,
...
import {
  ...
  EJ2Intance,
  BeforeCopyEventArgs,
  ColumnDataStateChangeEventArgs,
} from './interface';
import {
...
import { Workbook } from '@hoyui/excel-export';
  ...
  implements INotifyPropertyChanged
{
   ...
   */
  @Event()
  public actionComplete: EmitType<
    | PageEventArgs
    | GroupEventArgs
    | FilterEventArgs
    | SearchEventArgs
  ...
  public getPersistData(): string {
    ...
    for (let i: number = 0; i < keyEntity.length; i++) {
      ...
      for (const val of ignoreOnPersist[
        keyEntity[parseInt(i.toString(), 10)]
      ]) {
        delete currentObject[`${val}`];
      }
    }
    const temp: string | Function = this.pageSettings.template;
    const settings: PageSettingsModel = Object.assign(
      { template: undefined },
      this.pageSettings
    );
    this.setProperties({ pageSettings: settings }, true);
    const captionTemplateRef: string | Object | Function =
      this.groupSettings.captionTemplate;
    const isAngularCaptionTemplate: boolean =
      captionTemplateRef && this.isAngular;
  ...
  public requiredModules(): ModuleDeclaration[] {
    ...
    if (
      !isNullOrUndefined(<Column[]>this.columns) &&
      this.isCommandColumn(<Column[]>this.columns)
    ) {
      modules.push({
  ...
  public extendRequiredModules(modules: ModuleDeclaration[]): void {
    ...
    if (
      !isNullOrUndefined(this.columns as Column[]) &&
      this.isForeignKeyEnabled(this.columns as Column[])
    ) {
      modules.push({
        member: 'foreignKey',
        args: [this, this.serviceLocator],
  ...
  private resizeCheck(): boolean {
    const autoFitColumns: boolean = this.getColumns().filter(
      (c: Column) => c.autoFit === true
    ).length
      ? true
      : false;
    if (
      !isNullOrUndefined(this.columnModel) &&
      this.columnModel.length &&
      !(this.columnModel[0] instanceof Column)
    ) {
      this.columnModel = [];
    }
    const columnMenu: boolean =
      this.showColumnMenu &&
      (!this.columnMenuItems ||
        (this.columnMenuItems as ColumnMenuItem[]).filter(
          (c: ColumnMenuItem) => c === 'AutoFit' || c === 'AutoFitAll'
        ).length)
        ? true
        : false;
    const contextMenu: boolean =
      this.contextMenuItems &&
      (this.contextMenuItems as ContextMenuItem[]).filter(
        (c: ContextMenuItem) => c === 'AutoFit' || c === 'AutoFitAll'
      ).length
        ? true
        : false;
    return (
      this.allowResizing ||
      this.autoFitColumnsResize ||
      autoFitColumns ||
      columnMenu ||
      contextMenu
    );
  ...
  protected render(): void {
    ...
    createSpinner(
      { target: this.element, cssClass: this.cssClass ? this.cssClass : null },
      this.createElement
    );
    ...
    if (this.pageSettings.enableQueryString) {
      const pageValue: string = new URL(window.location.href).searchParams.get(
        'page'
      );
      if (
        !isNullOrUndefined(pageValue) &&
        window.location.href.indexOf('?page=') > 0
      ) {
        const currentPageValue: number = parseInt(pageValue, 10);
        this.setProperties(
          { pageSettings: { currentPage: currentPageValue } },
          true
        );
    ...
    if (
      (this.getDataModule().dataManager.dataSource.offline === true ||
        this.getDataModule().dataManager.dataSource.url === undefined) &&
      !(
        !isNullOrUndefined(this.dataSource) &&
        (<DataResult>this.dataSource).result
      )
    ) {
    ...
    if (this.aggregateModule && this.aggregates.length) {
      for (let i: number = 0; i < this.aggregates.length; i++) {
        for (
          let j: number = 0;
          j < this.aggregates[parseInt(i.toString(), 10)].columns.length;
          j++
        ) {
          const column: AggregateColumnModel =
            this.aggregates[parseInt(i.toString(), 10)].columns[
  ...
  public showMaskRow(axisDirection?: string, dialogElement?: Element): void {
    if (
      isNullOrUndefined(this.headerModule) ||
      isNullOrUndefined(this.contentModule)
    ) {
      return;
    }
    ...
    if (dialogElement) {
      const dialogHolder: Element =
        dialogElement.querySelector('.e-checkboxlist');
      const maskRowCount: number = Math.floor(
        dialogHolder.getBoundingClientRect().height / this.getRowHeight()
      );
      const maskTemplate: string =
        '<div class="e-ftrchk e-mask-ftrchk" style="width: 100%;">' +
      ...
      for (let i: number = 0; i < maskRowCount; i++) {
        ...
        const maskSpan: Element[] = [].slice.call(
          dialogHolder.querySelectorAll(
            '.e-mask:not(.e-mask-checkbox-filter-intent):not(.e-mask-checkbox-filter-span-intent)'
          )
        );
    ...
    if (
      (!this.enableRtl &&
        !(this.getHeaderContent() as HTMLElement).style.paddingRight) ||
      (this.enableRtl &&
        !(this.getHeaderContent() as HTMLElement).style.paddingLeft)
    ) {
    ...
    if (!this.isInitialLoad && !this.getColumns().length) {
      ...
      const maskTableHeight: number =
        contentHeight === 0
          ? this.allowPaging
            ? this.pageSettings.pageSize * this.getRowHeight()
            : window.innerHeight
          : contentHeight;
      const contentRowCount: number = Math.ceil(
        maskTableHeight / this.getRowHeight()
      );
      if (this.rowRenderingMode !== 'Vertical') {
        this.headerMaskTable = this.createEmptyMaskTable(gridHeader, 1);
      }
      this.contentMaskTable = this.createEmptyMaskTable(
        gridContent,
        contentRowCount
      );
      return;
    }
    this.maskRowContentScroll =
      (this.enableVirtualization || this.enableColumnVirtualization) &&
      axisDirection
        ? true
        : false;
    if (!this.contentMaskTable) {
      ...
      if (!isNullOrUndefined(content.querySelector('tbody'))) {
        this.contentMaskTable = this.createMaskTable(
          content,
          this.getContentMaskColumns(),
          axisDirection
        );
      }
    }
    if (
      ...
      this.headerMaskTable = this.createMaskTable(
        content,
        this.getContentMaskColumns(),
        axisDirection
      );
  ...
  private createEmptyMaskTable(
    ...
    const table: Element = this.createElement('table', {
      className: 'e-table e-masked-table',
    });
    const tbody: Element = this.createElement('tbody', {
      className: 'e-masked-tbody',
    });
    const row: Element = this.createElement('tr', {
      className: 'e-masked-row e-row',
      attrs: {
        style: 'height: ' + this.getRowHeight() + 'px;',
      },
    });
    const cell: Element = this.createElement('td', {
  ...
  private createMaskTable(
    ...
    const header: boolean = closest(parentElement, '.e-gridheader')
      ? true
      : false;
    const content: boolean = closest(parentElement, '.e-gridcontent')
      ? true
      : false;
    const footer: boolean = closest(parentElement, '.e-gridfooter')
      ? true
      : false;
    const gridContent: Element = this.getContent().firstChild as Element;
    ...
    if (header && this.enableColumnVirtualization && axisDirection === 'X') {
      ...
      if (this.isFrozenGrid()) {
        const frzTd: HTMLElement[] = [].slice.call(
          row.querySelectorAll('.e-rowcell')
        );
        for (let i: number = 0; i < frzTd.length; i++) {
          if (i < this.frozenLeftCount) {
            if (this.frozenLeftCount - 1 === i) {
              frzTd[parseInt(i.toString(), 10)].classList.add(
                'e-freezeleftborder'
              );
            }
            frzTd[parseInt(i.toString(), 10)].classList.add('e-leftfreeze');
            frzTd[parseInt(i.toString(), 10)].setAttribute(
              'data-colindex',
              i.toString()
            );
            frzTd[parseInt(i.toString(), 10)].style.left =
              (<{ valueX?: number }>columns[parseInt(i.toString(), 10)])
                .valueX -
              this.translateX +
              'px';
          } else if (
            frzTd.length - this.frozenRightCount <= i &&
            ...
            frzTd[parseInt(i.toString(), 10)].style.right =
              this.translateX +
              (<{ valueX?: number }>columns[parseInt(i.toString(), 10)])
                .valueX +
              'px';
      ...
      for (let i: number = 0; i < rowCount; i++) {
        ...
        (
          maskTHead.childNodes[parseInt(i.toString(), 10)] as HTMLElement
        ).style.height =
    ...
    if (content || header) {
      ...
      let rowCount: number = Math.ceil(
        rowCountElement.getBoundingClientRect().height / this.getRowHeight()
      );
      if (
        this.allowPaging &&
        this.pageSettings.pageSize !== this.currentViewData.length &&
        this.pagerModule.pagerObj.totalPages !== this.pageSettings.currentPage
      ) {
        for (
          let i: number = 0;
          i < this.pageSettings.pageSize - this.currentViewData.length;
          i++
        ) {
          const row: Element = this.createMaskRow(maskColgroup, columns);
      ...
      if (
        tbody.querySelector('.e-emptyrow') ||
        !tbody.childNodes.length ||
        (content && this.childGrid)
      ) {
        const row: Element = this.createMaskRow(maskColgroup, columns);
        const altRow: Element = row.cloneNode(true) as Element;
        altRow.classList.add('e-altrow');
        ...
        const rowsQuery: string =
          'tr:not(.e-hide):not([style*="display:none"]):not([style*="display: none"])';
        const rows: Element[] = [].slice.call(
          tbody.querySelectorAll(rowsQuery)
        );
        const addEditRow: Element = tbody.querySelector(
          '.e-addedrow, .e-editedrow'
        );
        ...
        if (addEditRow) {
          ...
          if (this.isFrozenGrid() && this.enableColumnVirtualization) {
            const frzTd: HTMLElement[] = [].slice.call(
              rows[addEditRowIndex + 1].querySelectorAll('.e-rowcell')
            );
            for (let i: number = 0; i < frzTd.length; i++) {
              if (i < this.frozenLeftCount) {
                frzTd[parseInt(i.toString(), 10)].classList.add('e-leftfreeze');
        ...
        if (
          addEditRow &&
          addEditRow.classList.contains('e-editedrow') &&
          addEditRowIndex < rowCount
        ) {
          const addEditMaskRow: HTMLElement = maskTBody.childNodes[
            parseInt(addEditRowIndex.toString(), 10)
          ] as HTMLElement;
    ...
    if (header && this.isFrozenGrid() && !this.enableColumnVirtualization) {
      (
        this.getHeaderContent().querySelector(
          '.' + literals.headerContent
        ) as HTMLElement
      ).style.position = 'relative';
    }
    parentElement.insertBefore(maskTable, parentElement.firstChild);
    if (content && !(this.enableVirtualization && axisDirection)) {
      let minScrollTop: number =
        gridContentScrollHeight - maskTable.getBoundingClientRect().height;
      minScrollTop = minScrollTop < 0 ? 0 : minScrollTop;
      let scrollTop: number =
        gridContent.scrollTop <= minScrollTop
          ? gridContent.scrollTop
          : minScrollTop;
      if (this.enableVirtualization) {
        scrollTop -= getTransformValues(
          closest(parentElement, '.e-virtualtable')
        ).height;
      }
      (maskTable as HTMLElement).style.transform =
  ...
  private applyMaskRow(row: Element, rowHeight: number): Element {
    ...
    for (let i: number = 0; i < maskCells.length; i++) {
      ...
      const displayAsCheckBoxCell: boolean =
        maskCell.firstChild &&
        (maskCell.firstChild as Element).classList &&
        (maskCell.firstChild as Element).classList.contains(
          'e-checkbox-wrapper'
        );
      ...
      if (
        this.enableColumnVirtualization &&
        maskCell.classList.contains('e-fixedfreeze')
      ) {
        removeClass(
          [maskCell],
          ['e-fixedfreeze', 'e-freezeleftborder', 'e-freezerightborder']
        );
      ...
      if (
        !(
          this.enableColumnVirtualization &&
          (maskCell.classList.contains('e-leftfreeze') ||
      ...
      if (
        maskCell.classList.contains('e-recordplusexpand') ||
        maskCell.classList.contains('e-recordpluscollapse')
      ) {
        (maskCell.firstChild as HTMLElement).classList.add(
          'e-mask-group-intent'
        );
      } else if (
        maskCell.classList.contains('e-gridchkbox') ||
        displayAsCheckBoxCell
      ) {
        (maskCell.firstChild as HTMLElement).classList.add(
  ...
  private createMaskRow(refColgroup: Element, refColumns: Column[]): Element {
    ...
    const td: Element = this.createElement('td', {
      className: 'e-masked-cell e-rowcell',
    });
    for (
      let i: number = 0, colIndex: number = 0;
      i < colgroup.childNodes.length;
      i++
    ) {
      const col: HTMLElement = colgroup.childNodes[
      ...
      if (
        !(
          col.classList.contains('e-group-intent') ||
          col.classList.contains('e-detail-intent') ||
          col.classList.contains('e-drag-intent')
        )
      ) {
        if (
          this.rowRenderingMode === 'Vertical' &&
          columns[parseInt(colIndex.toString(), 10)]
        ) {
          localTD.setAttribute(
            'data-cell',
            columns[parseInt(colIndex.toString(), 10)].headerText
              ? columns[parseInt(colIndex.toString(), 10)].headerText
              : columns[parseInt(colIndex.toString(), 10)].field
          );
        }
        if (col.style.display === 'none') {
          localTD.classList.add('e-hide');
        } else {
          localTD.style.textAlign =
            columns[parseInt(colIndex.toString(), 10)] &&
            columns[parseInt(colIndex.toString(), 10)].textAlign
              ? columns[
                  parseInt(colIndex.toString(), 10)
          ...
          if (
            columns[parseInt(colIndex.toString(), 10)] &&
            (columns[parseInt(colIndex.toString(), 10)].type === 'checkbox' ||
              columns[parseInt(colIndex.toString(), 10)].displayAsCheckBox)
          ) {
            (localTD.firstChild as HTMLElement).classList.add(
              'e-mask-checkbox-intent'
            );
          }
        }
  ...
  public addShimmerEffect(): void {
    ...
    const maskSpan: Element[] = [].slice.call(
      this.element.querySelectorAll('.e-mask:not(.e-shimmer-wave)')
    );
  ...
  private translateMaskRow(e: MouseEvent): void {
    ...
    const maskTables: NodeListOf<Element> =
      target.querySelectorAll('.e-masked-table');
    for (let i: number = 0; i < maskTables.length; i++) {
      const maskTable: Element = maskTables[parseInt(i.toString(), 10)];
      if (maskTable) {
        let minScrollTop: number =
          target.scrollHeight - maskTable.getBoundingClientRect().height;
        minScrollTop = minScrollTop < 0 ? 0 : minScrollTop;
        let scrollTop: number =
          target.scrollTop <= minScrollTop ? target.scrollTop : minScrollTop;
        if (this.enableVirtualization) {
          scrollTop -= getTransformValues(
            closest(maskTable, '.e-virtualtable')
          ).height;
        }
        (maskTable as HTMLElement).style.transform =
          'translate(0px,' + scrollTop + 'px)';
  ...
  private updateStackedFilter(): void {
    if (
      ...
      this.getHeaderContent().getElementsByClassName('e-stackedheadercell')
        .length
    ) {
  ...
  public getMediaColumns(): void {
    if (!this.enableColumnVirtualization) {
      ...
      this.getShowHideService =
        this.serviceLocator.getService<ShowHide>('showHideService');
      if (!isNullOrUndefined(gcol)) {
        for (let index: number = 0; index < gcol.length; index++) {
          if (
            !isNullOrUndefined(
              gcol[parseInt(index.toString(), 10)].hideAtMedia
            ) &&
  ...
  private pushMediaColumn(col: Column, index: number): void {
    ...
    this.mediaBindInstance[parseInt(index.toString(), 10)] =
      this.mediaQueryUpdate.bind(this, index);
    this.media[col.uid].addListener(
      this.mediaBindInstance[parseInt(index.toString(), 10)] as null
    );
  }
  ...
  public mediaQueryUpdate(columnIndex: number, e?: MediaQueryList): void {
    ...
    if (
      this.mediaCol.some((mediaColumn: Column) => mediaColumn.uid === col.uid)
    ) {
  ...
  private refreshMediaCol(): void {
    ...
    const footerContent: Element = this.element.querySelector(
      '.' + literals.gridFooter
    );
    if (
      this.aggregates.length &&
      this.element.scrollHeight > parseInt(this.height.toString(), 10) &&
      footerContent
    ) {
      addClass([footerContent], ['e-footerpadding']);
    }
    const checkboxColumn: Column[] = this.getColumns().filter(
      (col: Column) => col.type === 'checkbox'
    );
    if (
      checkboxColumn.length === 0 ||
    ...
    if (this.rowRenderingMode === 'Vertical') {
      if (this.enableHover) {
        this.setProperties(
          { enableAdaptiveUI: true, enableHover: false },
          true
        );
  ...
  private removeMediaListener(): void {
    for (let i: number = 0; i < this.mediaCol.length; i++) {
      this.media[this.mediaCol[parseInt(i.toString(), 10)].uid].removeListener(
        this.mediaBindInstance[
          this.mediaCol[parseInt(i.toString(), 10)].index
        ] as null
      );
    }
  }
  ...
  public onPropertyChanged(newProp: GridModel, oldProp: GridModel): void {
    ...
    const args: Object = { requestType: 'refresh' };
    ...
    for (const prop of properties) {
      switch (prop) {
        ...
        case 'pageSettings':
          ...
          if (
            (isNullOrUndefined(newProp.pageSettings.currentPage) &&
              isNullOrUndefined(newProp.pageSettings.pageSize) &&
              isNullOrUndefined(newProp.pageSettings.totalRecordsCount)) ||
            (!isNullOrUndefined(oldProp.pageSettings) &&
              newProp.pageSettings.currentPage !==
                oldProp.pageSettings.currentPage &&
              !this.enableColumnVirtualization &&
              !this.enableVirtualization &&
              this.pageRequireRefresh &&
        ...
        case 'emptyRecordTemplate':
          this.emptyRecordTemplateFn = templateCompiler(
            this.emptyRecordTemplate
          );
          requireRefresh = true;
          break;
        case 'detailTemplate':
        ...
        case 'childGrid':
          requireRefresh = true;
          childGridParent = this.parentDetails
            ? document.querySelector(`#${this.parentDetails.parentID}`)
            : null;
          parentInstance = childGridParent
            ? (childGridParent as EJ2Intance).ej2_instances[0]
            : null;
          if (
            this.childGrid &&
            parentInstance &&
            isNullOrUndefined(parentInstance.childGrid.childGrid)
          ) {
            const childGridObject: GridModel = Object.assign(
              {},
              parentInstance.childGrid,
            ...
            while (!isNullOrUndefined(parentInstance.parentDetails)) {
              const currentParent: Element = document.querySelector(
                `#${parentInstance.parentDetails.parentID}`
              );
              const currentParentInstance: IGrid = currentParent
                ? (currentParent as EJ2Intance).ej2_instances[0]
                : null;
              if (currentParentInstance) {
                const currentChildObject: GridModel = Object.assign(
                  {},
                  currentParentInstance.childGrid
                );
    ...
    if (requireGridRefresh) {
      if (
        ...
        this.getFrozenColumns() ||
        this.frozenRows ||
        this.frozenLeftColumns.length ||
        this.frozenRightColumns.length
      ) {
        this.freezeRefresh();
      } else {
        this.refresh();
  ...
  private extendedPropertyChange(
    ...
    switch (prop) {
      case 'enableRtl':
        ...
        if (this.allowPaging) {
          (<EJ2Intance>(
            this.element.querySelector('.e-gridpager')
        ...
        if (
          this.showColumnMenu &&
          this.columnMenuModule &&
          !this.enableAdaptiveUI
        ) {
          (<EJ2Intance>(
            this.columnMenuModule.getColumnMenu()
          )).ej2_instances[0].enableRtl = newProp.enableRtl;
          (<EJ2Intance>(
            this.columnMenuModule.getColumnMenu()
          )).ej2_instances[0].dataBind();
      ...
      case 'dataSource':
        ...
        if (Object.getPrototypeOf(newProp).deepWatch) {
          ...
          for (
            let i: number = 0,
              props: string[] = Object.keys(newProp.dataSource);
            i < props.length;
            i++
          ) {
            this.setRowData(
          ...
          let gResult: Object = !isNullOrUndefined(this.dataSource)
            ? (<DataResult>this.dataSource).result
            : [];
          ...
          if (
            ...
            !((gResult as Object[]).length && gResult[0].field)
          ) {
            for (let i: number = 0; i < names.length; i++) {
              gResult = DataUtil.group(
                <Object[]>gResult,
                names[parseInt(i.toString(), 10)],
                pending.aggregates || []
              );
            }
          }
          this.dataSource = {
            result: gResult as Object[] | Group[],
            count: (<DataResult>this.dataSource).count,
          ...
          if (
            !isNullOrUndefined(this.dataSource) &&
            (<DataResult>this.dataSource).result &&
            (<DataResult>this.dataSource).count &&
            this.groupSettings.columns.length
          ) {
  ...
  public updateDefaultCursor(): void {
    const headerCells: Element[] = [].slice.call(
      this.getHeaderContent().querySelectorAll(
        '.e-headercell:not(.e-stackedheadercell)'
      )
    );
    const stdHdrCell: Element[] = [].slice.call(
      this.getHeaderContent().getElementsByClassName('e-stackedheadercell')
    );
    ...
    for (let i: number = 0; i < headerCells.length; i++) {
      ...
      if (this.allowGrouping || this.allowReordering || this.allowSorting) {
        if (
          !cols[parseInt(i.toString(), 10)].allowReordering ||
          !cols[parseInt(i.toString(), 10)].allowSorting ||
    ...
    for (let count: number = 0; count < stdHdrCell.length; count++) {
      if (this.allowReordering) {
        stdHdrCell[parseInt(count.toString(), 10)].classList.add(
          'e-mousepointer'
        );
      }
    }
  ...
  private updateColumnModel(columns: Column[], isRecursion?: boolean): void {
    for (
      let i: number = 0,
        len: number = !isNullOrUndefined(columns) ? columns.length : 0;
      i < len;
      i++
    ) {
      if (columns[parseInt(i.toString(), 10)].columns) {
        this.updateColumnModel(
          columns[parseInt(i.toString(), 10)].columns as Column[],
          true
        );
      } else {
  ...
  private checkLockColumns(cols: Column[]): void {
    for (let i: number = 0; i < cols.length; i++) {
      if (cols[parseInt(i.toString(), 10)].columns) {
        this.checkLockColumns(
          cols[parseInt(i.toString(), 10)].columns as Column[]
        );
  ...
  public leftrightColumnWidth(position?: string): number {
    const cols: Column[] =
      position === 'left'
        ? this.getFrozenLeftColumns()
        : position === 'right'
        ? this.getFrozenRightColumns()
        : [];
    let width: number = 0;
  ...
  public getColumns(isRefresh?: boolean): Column[] {
    const inview: number[] = this.inViewIndexes
      .map((v: number) => v - this.groupSettings.columns.length)
      .filter((v: number) => v > -1);
    const vLen: number = inview.length;
    if (
      !this.enableColumnVirtualization ||
      isNullOrUndefined(this.columnModel) ||
      this.columnModel.length === 0 ||
      isRefresh
    ) {
      this.columnModel = [];
      this.updateColumnModel(this.columns as Column[]);
    ...
    const isLeftRightFrozen: boolean =
      this.enableColumnVirtualization &&
      this.isPreparedFrozenColumns &&
      vLen === 0;
    ...
    for (let i: number = 0; i < columns.length; i++) {
      if (
        columns[parseInt(i.toString(), 10)].freeze === 'Left' ||
   ...
   * @returns {Column} Returns the Column
   */
  public getStackedHeaderColumnByHeaderText(
    ...
    for (let i: number = 0; i < col.length; i++) {
      ...
      if (
        individualColumn.field === stackedHeader ||
        individualColumn.headerText === stackedHeader
      ) {
        this.stackedColumn = individualColumn;
        break;
      } else if (individualColumn.columns) {
        this.getStackedHeaderColumnByHeaderText(
          stackedHeader,
          <Column[]>individualColumn.columns
        );
      }
    }
  ...
  public getFooterContent(): Element {
    this.footerElement = this.element.getElementsByClassName(
      literals.gridFooter
    )[0];
  ...
  public getRowInfo(target: Element | EventTarget): RowInfo {
    ...
    if (
      ...
      isNullOrUndefined(
        parentsUntil(ele, 'e-detailrowcollapse') &&
      ...
      if (!cell) {
        ...
        if (!isNullOrUndefined(row) && !row.classList.contains('e-addedrow')) {
          const rowObj: Row<Column> = this.getRowObjectFromUID(
            row.getAttribute('data-uid')
          );
          const rowIndex: number = parseInt(
            row.getAttribute(literals.dataRowIndex),
            10
          );
      ...
      const cellIndex: number = parseInt(
        cell.getAttribute(literals.dataColIndex),
        10
      );
      ...
      if (
        ...
        const rowsObject: Object = rows.filter(
          (r: Row<{}>) => r.uid === row.getAttribute('data-uid')
        );
        let rowData: Object = {};
        let column: Column;
        if (Object.keys(rowsObject).length) {
          rowData = rowsObject[0].data;
          column = rowsObject[0].cells[parseInt(index.toString(), 10)]
            .column as Column;
        }
        args = {
          cell: cell,
  ...
  public getAllDataRows(includeAdd?: boolean): Element[] {
    if (
      isNullOrUndefined(this.getContentTable().querySelector(literals.tbody))
    ) {
      return [];
    }
    ...
    if (this.frozenRows) {
      const hdrTbody: Element = this.getHeaderTable().querySelector(
        literals.tbody
      );
      ...
      if (
        this.editSettings.showAddNewRow &&
        freezeRows[0].classList.contains('e-addedrow')
      ) {
        freezeRows.shift();
      }
    }
  ...
  private generateDataRows(
    ...
    for (let i: number = 0, len: number = rows.length; i < len; i++) {
      if (
        rows[parseInt(i.toString(), 10)].classList.contains(literals.row) &&
        (!rows[parseInt(i.toString(), 10)].classList.contains('e-hiddenrow') ||
          includAdd)
      ) {
        if (this.isCollapseStateEnabled()) {
          dRows[
            parseInt(
              rows[parseInt(i.toString(), 10)].getAttribute('data-rowindex'),
              10
            )
  ...
  public setCellValue(
    ...
    const selectedRow: Object = (<Row<{}>[]>rowObjects).filter(
      (r: Row<{}>) => getValue(pkName, r.data) === key
    )[0];
    const tr: Element = selectedRow
      ? this.element.querySelector(
          '[data-uid=' + selectedRow[`${rowuID}`] + ']'
        )
      : null;
    if (!isNullOrUndefined(tr)) {
      ...
      let td: Element = this.enableVirtualization
        ? tr.children[parseInt(fieldIdx.toString(), 10)]
      ...
      if (!isNullOrUndefined(td)) {
        ...
        if (
          ...
          !isNullOrUndefined(selectedRow['changes'])
        ) {
        ...
        if (
          this.isReact &&
          td.getAttribute('tabindex') === '0' &&
          td.classList.contains('e-focused')
        ) {
   ...
   * @hidden
   */
  public refreshReactColumnTemplateByUid(
    columnUid: string,
    ...
    if (this.isReact) {
      ...
      (this as any).clearTemplate(['columnTemplate'], undefined, () => {
        const isChildGrid: boolean =
          this.childGrid && this.element.querySelectorAll('.e-childgrid').length
            ? true
            : false;
        const rows: Element[] | NodeListOf<Element> = isChildGrid
          ? this.getContentTable().querySelectorAll('.e-row')
          : this.getDataRows();
  ...
  public refreshReactTemplateTD(
    ...
    for (let j: number = 0; j < rows.length; j++) {
      let rowsObj: Row<Column> = this.getRowObjectFromUID(
        rows[parseInt(j.toString(), 10)].getAttribute('data-uid')
      );
      if (
        isChildGrid &&
        !rowsObj &&
        parentsUntil(rows[parseInt(j.toString(), 10)], 'e-childgrid')
      ) {
        const gridObj: IGrid = (
          parentsUntil(
        ...
        rowsObj = gridObj.getRowObjectFromUID(
          rows[parseInt(j.toString(), 10)].getAttribute('data-uid')
        );
      ...
      if (
        rowsObj &&
        rowsObj.isDataRow &&
        !isNullOrUndefined(rowsObj.index) &&
        ...
        for (let i: number = 0; i < rowsObj[`${cells}`].length; i++) {
          const cell: Cell<Column> =
            rowsObj[`${cells}`][parseInt(i.toString(), 10)];
          if (cell.isTemplate) {
            const cellRenderer: CellRenderer = new CellRenderer(
              this as IGrid,
              this.serviceLocator
            );
            const td: Element = isChildGrid
              ? rows[parseInt(j.toString(), 10)].children[
                  cell.index + (isChildRow ? childIndent : indent)
                ]
              : this.getCellFromIndex(j, isFrozen ? cell.index : i - indent);
  ...
  public refreshGroupCaptionFooterTemplate(): void {
    const isChildGrid: boolean =
      this.childGrid && this.element.querySelectorAll('.e-childgrid').length
        ? true
        : false;
    const rows: NodeListOf<Element> = this.getContentTable().querySelectorAll(
      '.e-groupcaptionrow, .e-groupfooterrow'
    );
    for (let i: number = 0; i < rows.length; i++) {
      // eslint-disable-next-line @typescript-eslint/no-this-alias
      let gridObj: IGrid = this;
      let rowsObj: Row<Column> = this.getRowObjectFromUID(
        rows[parseInt(i.toString(), 10)].getAttribute('data-uid')
      );
      if (
        isChildGrid &&
        !rowsObj &&
        parentsUntil(rows[parseInt(i.toString(), 10)], 'e-childgrid')
      ) {
        gridObj = (
          parentsUntil(
            rows[parseInt(i.toString(), 10)],
      ...
      if (rowsObj) {
        const cells: Object[] = rowsObj.cells.filter(
          (cell: Cell<{}>) => cell.isDataCell
        );
        const args: Object = {
          cells: cells,
          data: rowsObj.data,
  ...
  public refreshReactHeaderTemplateByUid(columnUid: string): void {
    if (this.isReact) {
      const cells: string = 'cells';
      const rowsObj: Row<Column>[] = (<{ rows?: Row<Column>[] }>(
        this.headerModule
      )).rows;
      const cellIndex: number = this.getNormalizedColumnIndex(columnUid);
      for (let j: number = 0; j < rowsObj.length; j++) {
        const cell: Cell<Column> =
          rowsObj[parseInt(j.toString(), 10)][`${cells}`][
            parseInt(cellIndex.toString(), 10)
          ];
        if (cell && cell.column.uid === columnUid) {
          const headerCellRenderer: HeaderCellRenderer = new HeaderCellRenderer(
            this,
            this.serviceLocator
          );
          const td: Element = parentsUntil(
            this.element.querySelectorAll(
              '[e-mappinguid=' + columnUid + ']'
            )[0],
            'e-templatecell'
          );
          headerCellRenderer.refresh(cell, td);
          const cols: SortDescriptorModel[] = this.sortSettings.columns;
          const columnIndex: number = cols.findIndex(function (
            col: SortDescriptorModel
          ): boolean {
            return col.field === cell.column.field;
          });
          if (columnIndex !== -1) {
            const header: Element = this.getColumnHeaderByField(
              cell.column.field
            );
            this.ariaService.setSort(
              <HTMLElement>header,
              cols[
                parseInt(columnIndex.toString(), 10)
            ...
            if (
              cols[parseInt(columnIndex.toString(), 10)].direction ===
              'Ascending'
            ) {
  ...
  public setRowData(key: string | number, rowData?: Object): void {
    ...
    if (this.groupSettings.columns.length > 0 && this.aggregates.length > 0) {
      rowObjects = (<Row<{}>[]>rowObjects).filter(
        (row: Row<{}>) => row.isDataRow
      );
    }
    const selectedRow: Row<Column> = (<Row<{}>[]>rowObjects).filter(
      (r: Row<{}>) => getValue(pkName, r.data) === key
    )[0] as Row<Column>;
    const selectRowEle: Element[] = selectedRow
      ? [].slice.call(
          this.element.querySelectorAll(
            '[data-uid=' + selectedRow[`${rowuID}`] + ']'
          )
  ...
  private setFrozenRowData(fTr: Row<Column>, rowData: Object): void {
    const rowRenderer: RowRenderer<Column> = new RowRenderer<Column>(
      this.serviceLocator,
      null,
      this
    );
   ...
   */
  public getMovableCellFromIndex(
    rowIndex: number,
    columnIndex: number
  ): Element {
   ...
   * @deprecated This method is deprecated. Use `getCellFromIndex()` method instead.
   */
  public getFrozenRightCellFromIndex(
    rowIndex: number,
    columnIndex: number
  ): Element {
    return this.getCellFromIndex(rowIndex, columnIndex);
  ...
  public getColumnHeaderByIndex(index: number): Element {
    return this.getHeaderTable().getElementsByClassName('e-headercell')[
  ...
  public getRowObjectFromUID(uid: string): Row<Column> {
    ...
    if (
      isNullOrUndefined(row) &&
      this.enableVirtualization &&
      this.groupSettings.columns.length > 0
    ) {
  ...
  public getColumnHeaderByUid(uid: string): Element {
    const element: Element = this.getHeaderContent().querySelector(
      '[e-mappinguid=' + uid + ']'
    );
  ...
  public getColumnByUid(uid: string): Column {
    return iterateArrayOrObject<Column, Column>(
      [
        ...(<Column[]>this.getColumns()),
        ...this.getStackedColumns(this.columns as Column[]),
      ],
      (item: Column) => {
        if (item.uid === uid) {
          return item;
  ...
  public getUidByColumnField(field: string): string {
    return iterateArrayOrObject<string, Column>(
      <Column[]>this.getColumns(),
      (item: Column) => {
        if (item.field === field) {
          return item.uid;
  ...
  public getIndentCount(): number {
    ...
    if (
      this.isRowDragable() &&
      isNullOrUndefined(this.rowDropSettings.targetID)
    ) {
      index++;
    }
    /**
  ...
  public getColumnFieldNames(): string[] {
    ...
    let column: Column;
    for (
      let i: number = 0, len: number = this.getColumns().length;
      i < len;
      i++
    ) {
  ...
  public getPrimaryKeyFieldNames(): string[] {
    ...
    for (let k: number = 0; k < this.columnModel.length; k++) {
      if (
        (this.columnModel[parseInt(k.toString(), 10)] as Column).isPrimaryKey
      ) {
        keys.push(
          (this.columnModel[parseInt(k.toString(), 10)] as Column).field
        );
      }
    }
    return keys;
  ...
  public getSelectedRowCellIndexes(): ISelectedCell[] {
    return this.selectionModule
      ? this.selectionModule.selectedRowCellIndexes
      : [];
  }
  ...
  public getSelectedRecords(): Object[] {
    return this.selectionModule
      ? this.selectionModule.getSelectedRecords()
      : [];
  }
  ...
  public getSelectedColumnsUid(): string[] {
    const uid: string[] = [];
    if (this.selectionModule) {
      this.selectionModule.selectedColumnsIndexes.filter((i: number) =>
        uid.push(this.getColumns()[parseInt(i.toString(), 10)].uid)
      );
    }
    return uid;
  }
  ...
  public getFrozenColumns(): number {
    return (
      this.frozenColumns + this.getFrozenCount(this.columns as Column[], 0, 0)
    );
  }
  ...
  private updateFrozenColumnsWidth(): void {
    if (this.isFrozenGrid() && !this.enableColumnVirtualization) {
      ...
      const isFrozenAuto: boolean = gcol.some((data: Column) => {
        return (
          (data.freeze === 'Left' ||
            data.freeze === 'Right' ||
            data.freeze === 'Fixed' ||
      ...
      for (let i: number = 0; i < gcol.length; i++) {
        ...
        if (
          ...
          col.width = Math.max(
            200,
            col.minWidth ? parseFloat(col.minWidth.toString()) : 0
          );
          if (
            !isNullOrUndefined(col.maxWidth) &&
            parseFloat(col.maxWidth.toString()) < col.width
          ) {
          ...
          if (isAutoWidth) {
            gcol.filter((cols: Column) => {
              if (cols.visible) {
                ...
                if (cols.width !== 'auto') {
                  const width: number = !isNullOrUndefined(cols.width)
                    ? parseFloat(cols.width.toString())
                    : Math.max(
                        200,
                        cols.minWidth ? parseFloat(cols.minWidth.toString()) : 0
                      );
                  tWidth += width;
                }
              }
            ...
            const gWidth: number =
              this.isPercentageWidthGrid() || this.width === 'auto'
                ? this.element.getBoundingClientRect().width
                : parseFloat(this.width.toString());
            difference =
              this.height === 'auto'
                ? gWidth - tWidth
                : gWidth - tWidth - getScrollBarWidth();
            if (difference < 0) {
              difference = 0;
            }
            ...
            gcol.filter((col: Column) => {
              if (col.visible) {
                if (
                  col.minWidth &&
                  parseFloat(col.minWidth.toString()) > autoWidth
                ) {
    ...
    for (let i: number = 0; i < columns.length; i++) {
      if (columns[parseInt(i.toString(), 10)].columns) {
        ...
        if (
          ...
          this.stackedarrayLeft.push(
            columns[parseInt(i.toString(), 10)] as Column
          );
        } else if (columns[parseInt(i.toString(), 10)].freeze === 'Right') {
  ...
  private pushallcol(
    text?: string,
    text1?: string,
    col?: { headerText?: string; columns?: Column[] },
    columns?: Column,
    isTrue?: boolean
  ): void {
      ...
      if (col.columns && col.columns.length) {
        for (let i: number = 0; i < col.columns.length; i++) {
          this.pushallcol(
            text,
            text1,
            col.columns[parseInt(i.toString(), 10)] as Object,
            columns,
            isTrue
          );
        }
      ...
      for (let j: number = 0; j < firstcol.length; j++) {
        if (!(<{ columns?: Column[] }>col).columns && i === 1) {
          ...
          if (tempHead.indexOf(firstcol[parseInt(j.toString(), 10)]) === -1) {
            this.pushallcol(
              firstcol[parseInt(j.toString(), 10)],
              firstcol[j - 1],
              col
            );
            tempHead.push(firstcol[parseInt(j.toString(), 10)]);
        ...
        if (j === firstcol.length - 1) {
          ...
          while (count !== headercol.length - 1 && i !== headercol.length) {
            if (
              firstcol.toString() ===
              headercol[parseInt(i.toString(), 10)].toString()
            ) {
              i++;
              this.pushallcol(
                null,
  ...
  private splitFrozenCount(columns: Column[]): void {
    if (
      this.frozenColumns ||
      (this.changedProperties && this.changedProperties.frozenColumns === 0)
    ) {
      const cols: Column[] =
        !this.enableColumnVirtualization ||
        (this.enableColumnVirtualization && this.isPreparedFrozenColumns)
          ? this.enableColumnVirtualization &&
            this.columnModel &&
            this.columnModel.length
            ? this.columnModel
      ...
      if (
        ...
        (this.changedProperties && !this.changedProperties.frozenColumns)
      ) {
        for (let i: number = 0; i < cols.length; i++) {
          if (this.frozenColumns > i) {
            cols[parseInt(i.toString(), 10)].freeze = 'Left';
          } else if (
            cols[parseInt(i.toString(), 10)].freeze === 'Right' ||
            cols[parseInt(i.toString(), 10)].freeze === 'Fixed'
          ) {
            cols[parseInt(i.toString(), 10)].freeze =
              cols[parseInt(i.toString(), 10)].freeze;
          } else {
    ...
    for (
      let i: number = 0;
      i < (!isNullOrUndefined(columns) ? columns.length : 0);
      i++
    ) {
      if (columns[parseInt(i.toString(), 10)].columns) {
        this.leftcount = 0;
    ...
    if (this.frozenLeftColumns.length) {
      const cols: Column =
        this.frozenLeftColumns[this.frozenLeftColumns.length - 1];
      this.removeBorder(this.frozenLeftColumns);
      this.frozenLeftBorderColumns(
        cols instanceof Column
          ? this.lastVisibleLeftCol(this.frozenLeftColumns)
          : cols
      );
    }
    if (this.frozenRightColumns.length) {
      ...
      this.frozenRightBorderColumns(
        cols instanceof Column
          ? this.firstVisibleRightCol(this.frozenRightColumns)
          : cols
      );
    }
    if (
      (this.frozenLeftColumns.length || this.frozenRightColumns.length) &&
      this.freezeColumnRefresh
    ) {
      const cols: Column[] = this.frozenLeftColumns
        .concat(this.movableColumns)
        .concat(this.frozenRightColumns);
  ...
  private removeBorder(columns?: Column[]): void {
    for (let i: number = 0; i < columns.length; i++) {
      if (
        columns[parseInt(i.toString(), 10)].columns &&
        columns[parseInt(i.toString(), 10)].columns.length
      ) {
        if (
          (<{ border?: string }>columns[parseInt(i.toString(), 10)]).border ===
            'Right' ||
          (<{ border?: string }>columns[parseInt(i.toString(), 10)]).border ===
            'Left'
        ) {
  ...
  private isVisibleColumns(column?: Column, arr?: string[]): void {
    if (column.columns && column.columns.length) {
      for (let i: number = 0; i < column.columns.length; i++) {
        this.isVisibleColumns(
          column.columns[parseInt(i.toString(), 10)] as Column,
          arr
        );
        if (
          (column.columns[parseInt(i.toString(), 10)] as Column).visible &&
          isNullOrUndefined(
            (column.columns[parseInt(i.toString(), 10)] as Column).columns
          ) &&
          (!isNullOrUndefined(
            (column.columns[parseInt(i.toString(), 10)] as Column).freeze
          ) ||
      ...
      if (
        column.visible &&
        (!isNullOrUndefined(column.freeze) || column.isFrozen)
      ) {
        arr.push('true');
      }
    }
  ...
  private frozenLeftBorderColumns(columns?: Column): void {
    ...
    if (columns.columns && columns.columns.length) {
      ...
      const cols: Column = columns.columns[
        columns.columns.length - 1
      ] as Column;
      this.frozenLeftBorderColumns(
        cols instanceof Column && cols.columns
          ? this.lastVisibleLeftCol(columns.columns as Column[])
          : cols
      );
    } else {
      (<{ border?: string }>columns).border = 'Left';
  ...
  private frozenRightBorderColumns(columns?: Column): void {
    ...
    if (columns.columns && columns.columns.length) {
      ...
      this.frozenRightBorderColumns(
        cols instanceof Column && cols.columns
          ? this.firstVisibleRightCol(columns.columns as Column[])
          : cols
      );
    } else {
      (<{ border?: string }>columns).border = 'Right';
  ...
  public getVisibleFrozenColumns(): number {
    return (
      this.getVisibleFrozenColumnsCount() +
      this.getVisibleFrozenCount(this.columns as Column[], 0)
    );
  }
  ...
  private getVisibleFrozenCount(cols: Column[], cnt: number): number {
    if (
      ...
      for (let i: number = 0, len: number = cols.length; i < len; i++) {
        if (cols[parseInt(i.toString(), 10)].columns) {
          cnt = this.getVisibleFrozenCount(
            cols[parseInt(i.toString(), 10)].columns as Column[],
            cnt
          );
        } else {
          if (
            cols[parseInt(i.toString(), 10)].isFrozen &&
            cols[parseInt(i.toString(), 10)].visible
          ) {
            cnt++;
          }
        }
  ...
  private getFrozenCount(cols: Column[], cnt: number, index?: number): number {
    for (
      let i: number = 0,
        len: number = !isNullOrUndefined(cols) ? cols.length : 0;
      i < len;
      i++
    ) {
      if (cols[parseInt(i.toString(), 10)].columns) {
        cnt = this.getFrozenCount(
          cols[parseInt(i.toString(), 10)].columns as Column[],
          cnt,
          index
        );
   ...
   * @hidden
   */
  public removeFilteredColsByField(
    field: string,
    isClearFilterBar?: boolean
  ): void {
    if (this.filterModule) {
  ...
  public recalcIndentWidth(): void {
    ...
    if (
      (!this.groupSettings.columns.length &&
        ...
        !this.isRowDragable()) ||
      this.getHeaderTable()
        .querySelector('.e-emptycell')
        .getAttribute('indentRefreshed') ||
      !this.getContentTable()
    ) {
      return;
    }
    let indentWidth: number = (
      this.getHeaderTable().querySelector('.e-emptycell')
        .parentElement as HTMLElement
    ).offsetWidth;
    const headerCol: HTMLElement[] = [].slice.call(
      this.getHeaderTable().querySelector(literals.colGroup).childNodes
    );
    const contentCol: HTMLElement[] = [].slice.call(
      this.getContentTable().querySelector(literals.colGroup).childNodes
    );
    const perPixel: number = indentWidth / 30;
    let i: number =
      this.getFrozenMode() === 'Right'
        ? this.groupSettings.columns.length + this.getColumns().length
        : 0;
    ...
    const applyWidth: Function = (index: number, width: number) => {
      if (ispercentageWidth(this)) {
        const newWidth: string =
          ((width / parentOffset) * 100).toFixed(1) + '%';
    ...
    if (
      this.enableColumnVirtualization ||
      this.isAutoGen ||
      this.columns.length === this.groupSettings.columns.length
    ) {
      indentWidth = 30;
    }
    ...
    this.getHeaderTable()
      .querySelector('.e-emptycell')
      .setAttribute('indentRefreshed', 'true');
  }
  ...
  public resetIndentWidth(): void {
    ...
    if (ispercentageWidth(this)) {
      this.getHeaderTable()
        .querySelector('.e-emptycell')
        .removeAttribute('indentRefreshed');
      this.widthService.setWidthToColumns();
    ...
    if (
      ...
      this.getColumns().filter(
        (col: Column) => (!col.width || col.width === 'auto') && col.minWidth
      ).length > 0
    ) {
      const tgridWidth: number | string = this.widthService.getTableWidth(
        this.getColumns(),
        true
      );
    ...
    if (
      ...
      this.selectionModule.selectedRowCellIndexes.length
    ) {
      this.selectionModule.updateAutoFillPosition();
   ...
   * @returns {void}
   */
  public reorderColumnByTargetIndex(
  ...
  public disableRowDD(enable: boolean): void {
    ...
    const headerRows: NodeListOf<Element> = headerTable.querySelectorAll(
      'th.e-rowdragheader, th.e-mastercell'
    );
    const rows: Element[] = this.getRows();
    const disValue: string = enable ? 'none' : '';
    setStyleAttribute(
      <HTMLElement>headerTable.querySelector(literals.colGroup).childNodes[0],
      { display: disValue }
    );
    setStyleAttribute(
      <HTMLElement>contentTable.querySelector(literals.colGroup).childNodes[0],
      { display: disValue }
    );
    ...
    for (
      let j: number = 0;
      j <
      headerTable.querySelectorAll('th.e-rowdragheader, th.e-mastercell')
        .length;
      j++
    ) {
  ...
  public autoFitColumns(
    ...
    const resize: Function = injectedModules.find(function (
      item: Function
    ): boolean {
  ...
  public preventAdjustColumns(): void {
    ...
    for (let i: number = 0; i < columns.length; i++) {
      if (columns[parseInt(i.toString(), 10)].visible) {
        if (
          this.groupSettings.columns.length &&
          this.groupSettings.columns.indexOf(
            columns[parseInt(i.toString(), 10)].field
          ) > -1
        ) {
          const headerCol: HTMLElement[] = [].slice.call(
            headerTable
              .querySelector('colgroup')
              .querySelectorAll(
                ':not(.e-group-intent):not(.e-detail-intent):not(.e-drag-intent)'
              )
          );
        ...
        if (columns[parseInt(i.toString(), 10)].width) {
          tableWidth += parseFloat(
            columns[parseInt(i.toString(), 10)].width.toString()
          );
        } else {
          tableWidth = 0;
          break;
    ...
    if (tableWidth) {
      ...
      if (
        ...
        const headerIndentCol: HTMLElement[] = [].slice.call(
          headerTable
            .querySelector('colgroup')
            .querySelectorAll(
              '.e-group-intent, .e-detail-intent, .e-drag-intent'
            )
        );
        const contentIndentCol: HTMLElement[] = [].slice.call(
          contentTable
            .querySelector('colgroup')
            .querySelectorAll(
              '.e-group-intent, .e-detail-intent, .e-drag-intent'
        ...
        for (let i: number = 0; i < headerIndentCol.length; i++) {
          headerIndentCol[parseInt(i.toString(), 10)].style.setProperty(
            'width',
            indentWidthUnitFormat
          );
          contentIndentCol[parseInt(i.toString(), 10)].style.setProperty(
            'width',
            indentWidthUnitFormat
          );
      ...
      if (
        (percentageWidth && tableWidth < 100) ||
        (!percentageWidth &&
          tableWidth < contentTable.parentElement.clientWidth)
      ) {
  ...
  private restoreAdjustColumns(): void {
    ...
    if (
      ...
      const percentageWidth: boolean = this.isPercentageWidthGrid();
      const indentWidthUnitFormat: string =
        this.defaultIndentWidth(percentageWidth).toString() +
        this.widthUnit(percentageWidth);
      const headerIndentCol: HTMLElement[] = [].slice.call(
        headerTable
          .querySelector('colgroup')
  ...
  private defaultIndentWidth(percentageWidth: boolean): number {
    return percentageWidth
      ? parseFloat(((30 / this.element.offsetWidth) * 100).toFixed(1))
      : 30;
  }
  ...
  private initializeServices(): void {
    ...
    this.serviceLocator.register(
      'valueFormatter',
      (this.valueFormatterService = new ValueFormatter(this.locale))
    );
    this.serviceLocator.register(
      'showHideService',
      (this.showHider = new ShowHide(this))
    );
    this.serviceLocator.register(
      'ariaService',
    ...
    this.serviceLocator.register(
      'focus',
      (this.focusModule = new FocusStrategy(this))
    );
  }
  ...
  private processModel(): void {
    ...
    if (this.allowGrouping) {
      for (let i: number = 0, len: number = gCols.length; i < len; i++) {
        ...
        for (let sLen: number = sCols.length; j < sLen; j++) {
          if (
            sCols[parseInt(j.toString(), 10)].field ===
            gCols[parseInt(i.toString(), 10)]
          ) {
            flag = true;
            break;
          }
        ...
        if (!this.groupSettings.showGroupedColumn) {
          const column: Column = this.enableColumnVirtualization
            ? (<Column[]>this.columns).filter(
                (c: Column) => c.field === gCols[parseInt(i.toString(), 10)]
              )[0]
    ...
    this.detailTemplateFn = templateCompiler(this.detailTemplate);
    this.editTemplateFn = templateCompiler(
      this.editSettings.template as string
    );
    this.editHeaderTemplateFn = templateCompiler(
      this.editSettings.headerTemplate as string
    );
    this.editFooterTemplateFn = templateCompiler(
      this.editSettings.footerTemplate as string
    );
    if (!isNullOrUndefined(this.parentDetails)) {
      const value: string = isNullOrUndefined(
  ...
  private gridRender(): void {
    ...
    if (
      this.editSettings.showAddNewRow &&
      (this.enableVirtualization || this.enableInfiniteScrolling)
    ) {
      this.editSettings.newRowPosition = 'Top';
    }
    classList(this.element, ['e-responsive', 'e-default'], []);
    const rendererFactory: RendererFactory =
      this.serviceLocator.getService<RendererFactory>('rendererFactory');
  ...
  private createGridPopUpElement(): void {
    const popup: Element = this.createElement('div', {
      className: 'e-gridpopup',
      styles: 'display:none;',
    });
    const content: Element = this.createElement('div', {
      className: literals.content,
      attrs: { tabIndex: '-1' },
    });
    append(
      [content, this.createElement('div', { className: 'e-uptail e-tail' })],
      popup
    );
    content.appendChild(this.createElement('span'));
    append(
      [content, this.createElement('div', { className: 'e-downtail e-tail' })],
      popup
    );
  ...
  private updateGridLines(): void {
    classList(
      this.element,
      [],
      ['e-verticallines', 'e-horizontallines', 'e-hidelines', 'e-bothlines']
    );
  ...
  public applyTextWrap(): void {
    if (this.allowTextWrap) {
      const headerRows: Element[] = [].slice.call(
        this.element.getElementsByClassName('e-columnheader')
      );
  ...
  private getTooltipStatus(element: HTMLElement): boolean {
    const headerTable: Element = this.getHeaderTable();
    const headerDivTag: string =
      this.enableAdaptiveUI && this.rowRenderingMode === 'Vertical'
        ? 'e-gridcontent'
        : 'e-gridheader';
    const htable: HTMLDivElement = this.createTable(
      headerTable,
      headerDivTag,
      'header'
    );
    const ctable: HTMLDivElement = this.createTable(
      headerTable,
      headerDivTag,
      'content'
    );
    const table: HTMLDivElement = element.classList.contains('e-headercell')
      ? htable
      : ctable;
    const ele: string = element.classList.contains('e-headercell')
      ? 'th'
      : 'tr';
    table.querySelector(ele).className = element.className;
    table.querySelector(ele).innerHTML = element.innerHTML;
    const width: number = table
      .querySelector(ele)
    ...
    if (
      (width > element.getBoundingClientRect().width &&
        !element.classList.contains('e-editedbatchcell')) ||
      (this.enableAdaptiveUI &&
        this.rowRenderingMode === 'Vertical' &&
        width >
          element.getBoundingClientRect().width * 0.55 -
            (this.height !== 'auto' ? 16 : 0))
    ) {
  ...
  private mouseMoveHandler(e: MouseEvent): void {
    if (this.isEllipsisTooltip()) {
      const element: HTMLElement = parentsUntil(
        e.target as Element,
        'e-ellipsistooltip'
      ) as HTMLElement;
      ...
      if (
        element &&
        e.type !== 'mouseout' &&
        !(Browser.isDevice && elemNames.indexOf(tagName) !== -1)
      ) {
        if (this.getTooltipStatus(element)) {
          const col: Column = this.getColumns()[
            parseInt(element.getAttribute(literals.dataColIndex), 10)
          ] as Column;
          const domSetter: string = col.disableHtmlEncode
            ? 'innerText'
            : 'innerHTML';
          const contentDiv: HTMLDivElement = this.createElement('div');
          if (element.getElementsByClassName('e-headertext').length) {
            const innerElement: HTMLElement = element.getElementsByClassName(
              'e-headertext'
            )[0] as HTMLElement;
          ...
          if (col.disableHtmlEncode) {
            (<{ enableHtmlParse?: boolean }>this.toolTipObj).enableHtmlParse =
              false;
          }
  ...
  protected createTable(
    ...
    const myTableDiv: HTMLDivElement = this.createElement(
      'div'
    ) as HTMLDivElement;
    myTableDiv.className = this.element.className;
    myTableDiv.style.cssText =
      'display: inline-block;visibility:hidden;position:absolute';
    const mySubDiv: HTMLDivElement = this.createElement(
      'div'
    ) as HTMLDivElement;
    mySubDiv.className = tag;
    const myTable: HTMLTableElement = this.createElement(
      'table'
    ) as HTMLTableElement;
    myTable.className = table.className;
    myTable.style.cssText = 'table-layout: auto;width: auto';
    const ele: string = type === 'header' ? 'th' : 'td';
    const myTr: HTMLTableRowElement = this.createElement('tr', {
      attrs: { role: 'row' },
  ...
  public wireEvents(): void {
    ...
    if (this.allowKeyboard) {
      this.element.tabIndex =
        this.element.tabIndex === -1 ? 0 : this.element.tabIndex;
    }
  ...
  public addListener(): void {
    ...
    this.on(
      events.refreshSplitFrozenColumn,
      this.refreshSplitFrozenColumn,
      this
    );
  ...
  public getCurrentViewRecords(): Object[] {
    if (isGroupAdaptive(this)) {
      return isNullOrUndefined(
        (this.currentViewData as Object[] & { records: Object[] }).records
      )
    ...
    return this.allowGrouping &&
      this.groupSettings.columns.length &&
      this.currentViewData.length &&
      (<{ records?: Object[] }>this.currentViewData).records
      ? (this.currentViewData as Object[] & { records: Object[] }).records
      : this.currentViewData;
  }
  ...
  private mouseClickHandler(e: MouseEvent & TouchEvent): void {
    if (
      this.isChildGrid(e) ||
      (parentsUntil(e.target as Element, 'e-gridpopup') && e.touches) ||
      this.element.getElementsByClassName('e-cloneproperties').length ||
      this.checkEdit(e)
    ) {
      return;
    }
    if (
      ((!this.allowRowDragAndDrop &&
        (parentsUntil(e.target as Element, literals.gridContent) ||
          (e.target as Element).tagName === 'TD')) ||
        (parentsUntil(e.target as Element, 'e-headercell') &&
          parentsUntil(e.target as Element, 'e-headercell').querySelector(
            '.e-checkselectall'
          )) ||
        (!(this.allowGrouping || this.allowReordering) &&
          parentsUntil(e.target as Element, 'e-gridheader'))) &&
      e.touches
    ) {
      return;
    }
    ...
    const args: RecordClickEventArgs = this.getRowInfo(
      e.target as Element
    ) as RecordClickEventArgs;
  ...
  private checkEdit(e: MouseEvent): boolean {
    ...
    const isEdit: boolean =
      this.editSettings.mode !== 'Batch' &&
      this.isEdit &&
      tr &&
      (tr.classList.contains(literals.editedRow) ||
        (tr.classList.contains(literals.addedRow) &&
          !this.editSettings.showAddNewRow));
    return (
      !parentsUntil(e.target as Element, 'e-unboundcelldiv') &&
      (isEdit ||
        (parentsUntil(e.target as Element, literals.rowCell) &&
          parentsUntil(
            e.target as Element,
            literals.rowCell
          ).classList.contains('e-editedbatchcell')))
    );
  }
  ...
  private dblClickHandler(e: MouseEvent | TouchEventArgs): void {
    ...
    if (
      ...
      grid.id !== this.element.id ||
      closest(<Node>e.target, '.e-unboundcelldiv')
    ) {
      return;
    }
    ...
    const args: RecordDoubleClickEventArgs = this.getRowInfo(
      e.target as Element
    ) as RecordDoubleClickEventArgs;
  ...
  private focusOutHandler(e: MouseEvent): void {
    ...
    if (!parentsUntil(e.target as Element, 'e-grid')) {
      (
        this.element.querySelector('.e-gridpopup') as HTMLElement
      ).style.display = 'None';
    }
    const filterClear: Element = this.element.querySelector(
      '.e-cancel:not(.e-hide)'
    );
    if (
      filterClear &&
      !filterClear.parentElement.classList.contains('e-tbar-btn')
    ) {
      filterClear.classList.add('e-hide');
    }
    const relatedTarget: HTMLElement = e.relatedTarget as HTMLElement;
    const ariaOwns: string = relatedTarget
      ? relatedTarget.getAttribute('aria-owns')
      : null;
    if (
      (!relatedTarget ||
        (!parentsUntil(relatedTarget, 'e-grid') &&
          (!isNullOrUndefined(ariaOwns) && ariaOwns) !==
            (e.target as Element).getAttribute('aria-owns'))) &&
      !this.keyPress &&
      this.isEdit &&
      !Browser.isDevice
    ) {
      if (
        this.editSettings.mode === 'Batch' &&
        !(
          (parentsUntil(relatedTarget, 'e-ddl') ||
            parentsUntil(relatedTarget, 'e-ddt')) &&
          (parentsUntil(relatedTarget, 'e-multi-select-list-wrapper') ||
            parentsUntil(relatedTarget, 'e-input-filter')) &&
          parentsUntil(relatedTarget, 'e-input-group')
        ) &&
        (parentsUntil(relatedTarget, 'e-uploader') ||
          !(
            relatedTarget &&
  ...
  private isChildGrid(e: MouseEvent | KeyboardEvent | TouchEvent): boolean {
    const gridElement: Element = parentsUntil(
      e.target as HTMLElement,
      'e-grid'
    );
    if (
      (gridElement && gridElement.id !== this.element.id) ||
      (parentsUntil(e.target as Element, 'e-unboundcelldiv') &&
  ...
  public mergePersistGridData(persistedData?: Object): void {
    ...
    if (
      ...
      !isNullOrUndefined(persistedData)
    ) {
      const dataObj: Grid = !isNullOrUndefined(persistedData)
        ? persistedData
        : JSON.parse(data);
  ...
  private mergeColumns(storedColumn: Column[], columns: Column[]): void {
    ...
    for (let i: number = 0; i < storedColumns.length; i++) {
      let localCol: Column = columns.filter((tCol: Column) =>
        isFrozenGrid
          ? tCol.index ===
            storedColumns[parseInt(i.toString(), 10)][
              `${literals.initialFrozenColumnIndex}`
            ]
          : tCol.index === storedColumns[parseInt(i.toString(), 10)].index
      )[0];
      if (!isNullOrUndefined(localCol)) {
        ...
        if (localCol.columns && localCol.columns.length) {
          this.mergeColumns(
            <Column[]>storedColumns[parseInt(i.toString(), 10)].columns,
            <Column[]>localCol.columns
          );
          storedColumns[parseInt(i.toString(), 10)] = <Column>(
            extend(
              localCol,
              storedColumns[parseInt(i.toString(), 10)],
              {},
  ...
  public isDetail(): boolean {
    return (
      !isNullOrUndefined(this.detailTemplate) ||
      !isNullOrUndefined(this.childGrid)
    );
  }
  ...
  private keyPressHandler(e: KeyboardEventArgs): void {
    const presskey: KeyboardEventArgs = <KeyboardEventArgs>(
      extend(e, { cancel: false })
    );
      ...
      if (e.keyCode === 74) {
        ...
        if (this.keyA) {
          ...
          if (
            ...
            this.focusModule.currentInfo.element
          ) {
            removeClass(
              [
                this.focusModule.currentInfo.element,
                this.focusModule.currentInfo.elementToFocus,
              ],
              ['e-focused', 'e-focus']
            );
  ...
  private keyActionHandler(e: KeyArg): void {
    if (
      ...
      (this.isEdit &&
        (!this.editSettings.showAddNewRow ||
          (this.editSettings.showAddNewRow &&
            this.element.querySelector(literals.editedRow))) &&
        e.action !== 'escape' &&
        e.action !== 'enter' &&
        e.action !== 'shiftEnter' &&
        e.action !== 'tab' &&
  ...
  public refreshColumns(): void {
    ...
    this.refresh();
    if (this.isFrozenGrid() && this.enableColumnVirtualization) {
      const left: number = this.getContent().querySelector(
        '.e-movablescrollbar'
      ).scrollLeft;
      this.getHeaderContent().querySelector(
        '.' + literals.headerContent
  ...
  public excelExport(
    ...
    if (this.exportGrids && this.exportGrids.length) {
      ...
      return this.exportMultipleExcelGrids(
        gridIds,
        excelExportProperties,
        isMultipleExport,
        workbook,
  ...
  public pdfExport(
    ...
    if (this.exportGrids && this.exportGrids.length) {
      const gridIds: string[] = this.exportGrids.slice();
      return this.exportMultiplePdfGrids(
        gridIds,
        pdfExportProperties,
        isMultipleExport,
        pdfDoc,
      ...
      return this.pdfExportModule
        ? this.pdfExportModule.Map(
            this,
            pdfExportProperties,
            isMultipleExport,
            pdfDoc,
  ...
  private exportMultiplePdfGrids(
    ...
    if (gridIds.length !== 0) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const currentGridInstance: Grid = (
        document.getElementById(currentGridId) as any
      ).ej2_instances[0];
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const exportPromise: any = currentGridInstance.pdfExportModule
        ? currentGridInstance.pdfExportModule.Map(
            currentGridInstance,
            pdfExportProperties,
            isMultipleExport,
            pdfDoc,
      ...
      return exportPromise.then(function (
        exportedGridResults: object
      ): Promise<any> {
        isMultipleExport = gridIds.length === 1 ? false : true;
        return _this.exportMultiplePdfGrids(
          gridIds,
          pdfExportProperties,
          isMultipleExport,
          exportedGridResults,
  ...
  private exportMultipleExcelGrids(
    ...
    if (gridIds.length !== 0) {
      ...
      const exportPromise: Promise<object> =
        currentGridInstance.excelExportModule
          ? currentGridInstance.excelExportModule.Map(
              currentGridInstance,
              excelExportProperties,
              isMultipleExport,
              workbook,
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return exportPromise.then(function (
        exportedGridResults: Workbook
      ): Promise<any> {
        isMultipleExport = gridIds.length === 1 ? false : true;
        return _this.exportMultipleExcelGrids(
          gridIds,
          excelExportProperties,
          isMultipleExport,
          exportedGridResults,
  ...
  public log(type: string | string[], args?: Object): void {
    const injectedModules: Function[] = this.getInjectedModules();
    const logger: Function = injectedModules.find(
      (item: Function) => item.name === 'Logger'
    );
    if (!logger) {
      Grid.Inject(Logger);
  ...
  public getPreviousRowData(): Object {
    const previousRowData: Object =
      this.getRowsObject()[this.getRows().length - 1].data;
  ...
  public hideScroll(): void {
    const content: HTMLElement = this.getContent().querySelector(
      '.' + literals.content
    );
  ...
  public getRowIndexByPrimaryKey(value: string | Object): number {
    ...
    for (let i: number = 0; i < rows.length; i++) {
      if (
        rows[parseInt(i.toString(), 10)].isDetailRow ||
        rows[parseInt(i.toString(), 10)].isCaptionRow
      ) {
        continue;
      }
      let pKvalue: object | string =
        rows[parseInt(i.toString(), 10)].data[`${pkName}`];
      if (pkName.split('.').length > 1) {
        pKvalue = performComplexDataOperation(
          pkName,
          rows[parseInt(i.toString(), 10)].data
        );
      }
      if (pKvalue === value) {
  ...
  public grabColumnByFieldFromAllCols(
    field: string,
    ...
    for (let i: number = 0; i < gCols.length; i++) {
      if (
        (!isForeignKey && field === gCols[parseInt(i.toString(), 10)].field) ||
        (isForeignKey &&
          gCols[parseInt(i.toString(), 10)].isForeignColumn() &&
  ...
  public getFilteredRecords(): Object[] | Promise<Object> {
    if (
      (this.allowFiltering && this.filterSettings.columns.length) ||
      this.searchSettings.key.length
    ) {
      const query: Query = this.renderModule.data.generateQuery(true);
      if (
        this.dataSource &&
        this.renderModule.data.isRemote() &&
        this.dataSource instanceof DataManager
      ) {
        return this.renderModule.data.getData(
          this.dataSource as DataOptions,
          query
        );
      } else {
  ...
  public getRowElementByUID(uid: string): Element {
    let rowEle: Element;
    let rows: Element[] = [];
    const cntRows: Element[] = [].slice.call(
      this.getContent().querySelector(literals.tbody).children
    );
    if (this.frozenRows) {
      rows = [].slice.call(
        this.getHeaderContent().querySelector(literals.tbody).children
      );
   ...
   */
  public calculatePageSizeByParentHeight(
    containerHeight: number | string
  ): number {
    if (this.allowPaging) {
      if (
        (this.allowTextWrap && this.textWrapSettings.wrapMode === 'Header') ||
        !this.allowTextWrap
      ) {
        let pagesize: number = 0;
        if ((containerHeight as string).indexOf('%') !== -1) {
          containerHeight =
            (parseInt(containerHeight as string, 10) / 100) *
            this.element.clientHeight;
        }
        const nonContentHeight: number =
          this.getNoncontentHeight() + this.getRowHeight();
        if (containerHeight > nonContentHeight) {
          let contentHeight: number = 0;
          contentHeight =
            (containerHeight as number) - this.getNoncontentHeight();
  ...
  private getNoncontentHeight(): number {
    ...
    if (
      ...
      !isNullOrUndefined(this.getHeaderContent().clientHeight)
    ) {
      height += this.getHeaderContent().clientHeight;
    }
    if (
      this.toolbar &&
      !isNullOrUndefined(this.element.querySelector('.e-toolbar').clientHeight)
    ) {
    ...
    if (
      this.allowPaging &&
      !isNullOrUndefined(
        this.element.querySelector('.e-gridpager').clientHeight
      )
    ) {
    ...
    if (
      ...
      !isNullOrUndefined(
        this.element.querySelector('.e-columnheader').clientHeight
      )
    ) {
    ...
    if (
      ...
      !isNullOrUndefined(
        this.element.querySelector('.e-groupdroparea').clientHeight
      )
    ) {
    ...
    if (
      ...
      !isNullOrUndefined(
        this.element.querySelector('.e-summaryrow').clientHeight
      )
    ) {
      for (
        let i: number = 0;
        i < this.element.getElementsByClassName('e-summaryrow').length;
        i++
      ) {
        height +=
          this.element.getElementsByClassName('e-summaryrow')[
            parseInt(i.toString(), 10)
          ].clientHeight;
      }
    }
   ...
   */
  public getSummaryValues(
    summaryCol: AggregateColumnModel,
    summaryData: Object
  ): number {
    return DataUtil.aggregates[(summaryCol.type as string).toLowerCase()](
      summaryData,
      summaryCol.field
    );
  }
  ...
  public exportGrid(url: string): void {
    ...
    const state: { data?: string; pvtData?: Object[] } =
      new UrlAdaptor().processQuery(new DataManager({ url: '' }), query);
    const queries: { where: object; search: Object[]; sorted: object } =
      JSON.parse(state.data);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const gridModel: any = JSON.parse(
      this.addOnPersist([
        'allowGrouping',
        'allowPaging',
        'pageSettings',
    ...
    gridModel.searchSettings.fields =
      (queries.search && queries.search[0]['fields']) || [];
    gridModel.sortSettings.columns = queries.sorted;
    gridModel.columns = this.setHeaderText(
      gridModel.columns as Column[],
      include
    );
    const form: HTMLFormElement = this.createElement('form', {
      id: 'ExportForm',
      styles: 'display:none;',
    });
    const gridInput: HTMLInputElement = this.createElement('input', {
      id: 'gridInput',
  ...
  public setHeaderText(columns: Column[], include: string[]): Column[] {
    for (let i: number = 0; i < columns.length; i++) {
      const column: Column = this.getColumnByUid(
        columns[parseInt(i.toString(), 10)].uid
      );
      ...
      if (columns[parseInt(i.toString(), 10)].format) {
        columns[parseInt(i.toString(), 10)].format = getNumberFormat(
          ...
          columns[parseInt(i.toString(), 10)].type,
          this.isExcel,
          defaultCurrencyCode
        );
      }
      ...
      for (let j: number = 0; j < keys.length; j++) {
        if (include.indexOf(keys[parseInt(j.toString(), 10)]) < 0) {
          delete columns[parseInt(i.toString(), 10)][
            keys[parseInt(j.toString(), 10)]
          ];
        }
      }
    }
  ...
  private getFormat(
    ...
    return typeof format === 'object'
      ? !isNullOrUndefined(format.format)
        ? format.format
        : format.skeleton
      : format;
  }
  ...
  public setForeignKeyData(): void {
    this.dataBind();
    const colpending: PendingState =
      this.getDataModule().getForeignKeyDataState();
  ...
  public resetFilterDlgPosition(field: string): void {
    ...
    if (header) {
      ...
      const filterDlg: HTMLElement =
        this.element.querySelector('.e-filter-popup');
      if (target) {
        ...
        if (filterDlg) {
          if (filterDlg.offsetWidth + fClient.right > gClient.right) {
            filterDlg.style.left =
              (
                fClient.right -
                filterDlg.offsetWidth -
                gClient.left
              ).toString() + 'px';
          } else {
            filterDlg.style.left =
              (fClient.right - gClient.left).toString() + 'px';
          }
        }
      }
    }
  ...
  public renderTemplates(callBack?: any): void {
    const isReactChild: boolean =
      this.parentDetails &&
      this.parentDetails.parentInstObj &&
      this.parentDetails.parentInstObj.isReact;
    if (isReactChild && this['portals']) {
      this.parentDetails.parentInstObj['portals'] =
        this.parentDetails.parentInstObj['portals'].concat(this['portals']);
  ...
  private processBulkRowChanges(changes: BatchChanges): void {
    if (!this.dataToBeUpdated) {
      ...
      const loopstring: string[] = [
        literals.addedRecords,
        literals.changedRecords,
        literals.deletedRecords,
      ];
      ...
      for (let i: number = 0; i < loopstring.length; i++) {
        if (changes[loopstring[parseInt(i.toString(), 10)]]) {
          compareChanges(
            this,
            changes,
            loopstring[parseInt(i.toString(), 10)],
            keyField
          );
        }
      }
  ...
  private processRowChanges(changes: BatchChanges): void {
    ...
    const promise: Promise<Object> = this.getDataModule().saveChanges(
      ...
      keyField,
      {},
      this.getDataModule().generateQuery().requiresCount()
    );
  ...
  private setNewData(): void {
    const oldValues: Object[] = JSON.parse(
      JSON.stringify(this.getCurrentViewRecords())
    );
    const getData: Promise<Object> = this.getDataModule().getData(
      {},
      this.getDataModule().generateQuery().requiresCount()
    );
  ...
  private bulkRefresh(
    ...
    for (let i: number = 0; i < rowObj.length; i++) {
      if (
        !result.filter((e: Object) => {
          return (
            e[`${keyField}`] ===
            rowObj[parseInt(i.toString(), 10)].data[`${keyField}`]
          );
        }).length
      ) {
    ...
    for (let i: number = 0; i < result.length; i++) {
      ...
      oldValues.filter((e: Object) => {
        if (
          e[`${keyField}`] === result[parseInt(i.toString(), 10)][`${keyField}`]
        ) {
          if (e !== result[parseInt(i.toString(), 10)]) {
            this.setRowData(
              result[parseInt(i.toString(), 10)][`${keyField}`],
              result[parseInt(i.toString(), 10)]
            );
          }
          isRowExist = true;
        }
    ...
    const rows: HTMLTableRowElement[] = [].slice.call(
      this.getContentTable().getElementsByClassName(literals.row)
    );
    ...
    if (this.allowPaging) {
      this.notify(events.inBoundModelChanged, {
        module: 'pager',
        properties: { totalRecordsCount: count },
      });
    }
  }

  private renderRowElement(data: Object, index: number): void {
    const row: RowRenderer<Column> = new RowRenderer<Column>(
      this.serviceLocator,
      null,
      this
    );
    ...
    const tr: HTMLTableRowElement = row.render(
      modelData[0],
      this.getColumns()
    ) as HTMLTableRowElement;
    ...
    if (tbody.querySelector('.e-emptyrow')) {
      ...
      if (this.frozenRows && this.element.querySelector('.e-frozenrow-empty')) {
        this.element
          .querySelector('.e-frozenrow-empty')
          .classList.remove('e-frozenrow-empty');
      }
    }
  ...
  public updateVisibleExpandCollapseRows(): void {
    ...
    for (let i: number = 0, len: number = rows.length; i < len; i++) {
      if (
        (rows[parseInt(i.toString(), 10)].isDataRow ||
          rows[parseInt(i.toString(), 10)].isAggregateRow) &&
        this.getRowElementByUID(rows[parseInt(i.toString(), 10)].uid) &&
        (
          this.getRowElementByUID(
            rows[parseInt(i.toString(), 10)].uid
          ) as HTMLTableRowElement
        ).style.display === 'none'
      ) {
        (<{ visible?: boolean }>rows[parseInt(i.toString(), 10)]).visible =
          false;
      } else {
        (<{ visible?: boolean }>rows[parseInt(i.toString(), 10)]).visible =
          true;
  ...
  public getHeight(height: string | number): number | string {
    if (
      !Number.isInteger(height as number) &&
      (height as string).indexOf('%') !== -1
    ) {
      height =
        (parseInt(height as string, 10) / 100) * this.element.clientHeight;
  ...
  public showResponsiveCustomFilter(isCustom?: boolean): void {
    if (this.filterModule) {
      this.filterModule.showCustomFilter(
        isCustom || this.rowRenderingMode === 'Vertical'
      );
    }
  }
  ...
  public showResponsiveCustomSort(isCustom?: boolean): void {
    if (this.sortModule) {
      this.sortModule.showCustomSort(
        isCustom || this.rowRenderingMode === 'Vertical'
      );
    }
  }
  ...
  public showResponsiveCustomColumnChooser(isCustom?: boolean): void {
    if (this.columnChooserModule) {
      this.columnChooserModule.showCustomColumnChooser(
        isCustom || this.rowRenderingMode === 'Vertical'
      );
    }
  }
  ...
  public getCurrentVisibleColumns(isColVirtualization?: boolean): Column[] {
    const cols: Column[] = [];
    const gridCols: Column[] = isColVirtualization
      ? this.getColumns()
      : this.columnModel;
  ...
  private enableInfiniteAggrgate(): void {
    if (
      ...
      this.groupSettings.columns.length &&
      !this.groupSettings.disablePageWiseAggregates &&
      !this.groupSettings.enableLazyLoading
    ) {
      this.setProperties(
