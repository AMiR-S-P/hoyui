import {
  ChildProperty,
  compile as baseTemplateComplier,
  setValue,
  Internationalization,
  isUndefined,
} from '@hoyui/base';
import {
  extend as baseExtend,
  isNullOrUndefined,
  getValue,
  classList,
  NumberFormatOptions,
...
import { isObject, IKeyValue, select, selectAll } from '@hoyui/base';
import {
  IPosition,
  IGrid,
  IValueFormatter,
 ...
 * @hidden
 */
export function doesImplementInterface(
  target: Object,
  checkFor: string
): boolean {
 ...
 * @hidden
 */
export function headerValueAccessor(
  field: string,
  column: ColumnModel
): Object {
  return isNullOrUndefined(field) || field === ''
    ? ''
    : DataUtil.getObject(field, column);
}
 ...
 */
// eslint-disable-next-line
export function getUpdateUsingRaf<T>(
 ...
 */
export function isExportColumns(
  exportProperties: PdfExportProperties | ExcelExportProperties
): boolean {
  return (
    !isNullOrUndefined(exportProperties) &&
    !isNullOrUndefined(exportProperties.columns) &&
    exportProperties.columns.length > 0
  );
}
 ...
 * @hidden
 */
export function updateColumnTypeForExportColumns(
  exportProperties: PdfExportProperties | ExcelExportProperties,
  gObj: IGrid
): void {
  ...
  for (let i: number = 0; i < exportColumns.length; i++) {
    if (gridColumns.length - 1 >= i) {
      if (gridColumns[parseInt(i.toString(), 10)].columns) {
        for (
          let j: number = 0;
          j < gridColumns[parseInt(i.toString(), 10)].columns.length;
          j++
        ) {
          (
            exportColumns[parseInt(i.toString(), 10)].columns[
              parseInt(j.toString(), 10)
            ] as Column
          ).type = (
            gridColumns[parseInt(i.toString(), 10)].columns[
              parseInt(j.toString(), 10)
            ] as Column
          ).type;
        }
      } else {
        exportColumns[parseInt(i.toString(), 10)].type =
          gridColumns[parseInt(i.toString(), 10)].type;
      }
    }
  }
}
...
export function updatecloneRow(grid: IGrid): void {
  const nRows: Row<Column>[] = [];
  const actualRows: Row<Column>[] = grid.vRows;
  for (let i: number = 0; i < actualRows.length; i++) {
    if (actualRows[parseInt(i.toString(), 10)].isDataRow) {
      ...
      nRows.push(actualRows[parseInt(i.toString(), 10)]);
      if (
        !actualRows[parseInt(i.toString(), 10)].isExpand &&
        actualRows[parseInt(i.toString(), 10)].isCaptionRow
      ) {
        i += getCollapsedRowsCount(
          actualRows[parseInt(i.toString(), 10)],
          grid
        );
      }
    }
...
export function getCollapsedRowsCount(val: Row<Column>, grid: IGrid): number {
  count = 0;
  const gSummary: string = 'gSummary';
  ...
  if (value === val.data[`${total}`]) {
    if (
      grid.groupSettings.columns.length &&
      !isNullOrUndefined(val[`${agrCnt}`]) &&
      val[`${agrCnt}`]
    ) {
      if (grid.groupSettings.columns.length !== 1) {
        count +=
          val.indent !== 0 && value < 2
            ? val[`${gSummary}`] *
              (gLen - val.indent + (gLen - val.indent) * val[`${agrCnt}`])
            : val[`${gSummary}`] *
                (gLen -
                  val.indent +
                  (gLen - val.indent - 1) * val[`${agrCnt}`]) +
              val[`${agrCnt}`];
      ...
      if (grid.groupSettings.columns.length !== 1) {
        count +=
          val[`${gSummary}`] * (grid.groupSettings.columns.length - val.indent);
      } else {
        count += val[`${gSummary}`];
      }
    }
    ...
    for (
      let i: number = 0, len: number = val.data[`${items}`].length;
      i < len;
      i++
    ) {
      ...
      count +=
        gLevel[`${items}`].length +
        (gLen !== grid.columns.length &&
        !isNullOrUndefined(gLevel[`${items}`][`${records}`])
          ? gLevel[`${items}`][`${records}`].length
          : 0);
      dataRowCnt +=
        !isNullOrUndefined(gLevel[`${items}`][`${records}`]) &&
        !isNullOrUndefined(val[`${agrCnt}`])
          ...
          : gLevel[`${items}`].length;
      if (
        gLevel[`${items}`].GroupGuid &&
        gLevel[`${items}`].childLevels !== 0
      ) {
        recursive(gLevel);
      }
    }
...
export function recursive(row: Object[]): void {
  ...
  for (
    let j: number = 0, length: number = row[`${items}`].length;
    j < length;
    j++
  ) {
 ...
 * @hidden
 */
export function iterateArrayOrObject<T, U>(
  collection: U[],
  predicate: (item: Object, index: number) => T
): T[] {
  const result: T[] = [];
...
export function iterateExtend(array: Object[]): Object[] {
  ...
  for (let i: number = 0; i < array.length; i++) {
    obj.push(
      baseExtend(
        {},
        getActualProperties(array[parseInt(i.toString(), 10)]),
        {},
        true
      )
    );
 ...
 * @hidden
 */
export function setStyleAndAttributes(
  node: Element,
  customAttributes: { [x: string]: Object }
): void {
  const copyAttr: { [x: string]: Object } = {};
  const literals: string[] = ['style', 'class'];
  ...
  if ('style' in copyAttr) {
    setStyleAttribute(
      node as HTMLElement,
      copyAttr[literals[0]] as { [x: string]: Object }
    );
...
export function setColumnIndex(columnModel: Column[], ind: number = 0): number {
  for (
    let i: number = 0,
      len: number = !isNullOrUndefined(columnModel) ? columnModel.length : 0;
    i < len;
    i++
  ) {
    if ((columnModel[parseInt(i.toString(), 10)] as Column).columns) {
      ...
      ind++;
      ind = setColumnIndex(
        <Column[]>(columnModel[parseInt(i.toString(), 10)] as Column).columns,
        ind
      );
    } else {
      (columnModel[parseInt(i.toString(), 10)] as Column).index =
...
export function prepareColumns(
  ...
  for (
    ...
    if (typeof columns[parseInt(c.toString(), 10)] === 'string') {
      column = new Column(
        { field: <string>columns[parseInt(c.toString(), 10)] },
        gObj
      );
    } else if (
      !(columns[parseInt(c.toString(), 10)] instanceof Column) ||
      (columns[parseInt(c.toString(), 10)] as Column).columns
    ) {
      if (!(columns[parseInt(c.toString(), 10)] as Column).columns) {
        column = new Column(
          columns[parseInt(c.toString(), 10)] as Column,
          gObj
        );
      } else {
        (columns[parseInt(c.toString(), 10)] as Column).columns =
    ...
    column.headerText = isNullOrUndefined(column.headerText)
      ? column.foreignKeyValue || column.field || ''
      : column.headerText;

    column.foreignKeyField = column.foreignKeyField || column.field;

    column.valueAccessor =
      (typeof column.valueAccessor === 'string'
        ? getValue(<string>column.valueAccessor, window)
        : column.valueAccessor) || valueAccessor;

    column.width =
      autoWidth && isNullOrUndefined(column.width) ? 200 : column.width;
 ...
 * @hidden
 */
export function setCssInGridPopUp(
  popUp: HTMLElement,
  ...
  const targetPosition: { top: number; left: number; right: number } = (
    e.target as HTMLElement
  ).getBoundingClientRect();
  popUpSpan.className = className;
  popUp.style.display = '';
  const isBottomTail: boolean =
    (isNullOrUndefined((e as MouseEvent).clientY)
      ? (e as TouchEvent).changedTouches[0].clientY
      : (e as MouseEvent).clientY) >
    popUp.offsetHeight + 10;
  popUp.style.top =
    targetPosition.top -
    position.top +
    (isBottomTail ? -(popUp.offsetHeight + 10) : popUp.offsetHeight + 10) +
    'px'; //10px for tail element
  popUp.style.left =
    getPopupLeftPosition(popUp, e, targetPosition, position.left) + 'px';
  if (isBottomTail) {
...
function getPopupLeftPosition(
  popup: HTMLElement,
  e: MouseEvent | TouchEvent,
  targetPosition: { top: number; left: number; right: number },
  left: number
): number {
...
export function getActualPropFromColl(collection: Object[]): Object[] {
  ...
  for (let i: number = 0, len: number = collection.length; i < len; i++) {
    ...
    if (collection[parseInt(i.toString(), 10)].hasOwnProperty('properties')) {
      coll.push(
        (collection[parseInt(i.toString(), 10)] as { properties: Object })
          .properties
      );
    } else {
...
export function getPosition(e: MouseEvent | TouchEvent): IPosition {
  ...
  position.x = isNullOrUndefined((e as MouseEvent).clientX)
    ? (e as TouchEvent).changedTouches[0].clientX
    : (e as MouseEvent).clientX;
  position.y = isNullOrUndefined((e as MouseEvent).clientY)
    ? (e as TouchEvent).changedTouches[0].clientY
    : (e as MouseEvent).clientY;
  return position;
}
...
export function calculateAggregate(
  ...
  return column.field in data || data instanceof Array
    ? DataUtil.aggregates[type.toLowerCase()](data, column.field)
    : null;
}
/** @hidden */
...
export function isEditable(col: Column, type: string, elem: Element): boolean {
  ...
  const isOldRow: boolean = !row
    ? true
    : row && !row.classList.contains('e-insertedrow');
...
export function isActionPrevent(inst: IGrid): boolean {
  const dlg: HTMLElement = select(
    '#' + inst.element.id + 'EditConfirm',
    inst.element
  ) as HTMLElement;
  return (
    inst.editSettings.mode === 'Batch' &&
    (selectAll('.e-updatedtd', inst.element).length ||
      selectAll('.e-gridform.e-formvalidator', inst.element).length) &&
    inst.editSettings.showConfirmDialog &&
...
export function setFormatter(
  ...
  const fmtr: IValueFormatter =
    serviceLocator.getService<IValueFormatter>('valueFormatter');
  ...
  switch (column.type) {
    case 'date':
      column.setFormatter(fmtr.getFormatFunction(args as DateFormatOptions));
      column.setParser(fmtr.getParserFunction(args as DateFormatOptions));
      break;
    case 'dateonly':
      column.setFormatter(fmtr.getFormatFunction(args as DateFormatOptions));
      column.setParser(fmtr.getParserFunction(args as DateFormatOptions));
 ...
 * @hidden
 */
export function addRemoveActiveClasses(
  cells: Element[],
  add: boolean,
  ...args: string[]
): void {
...
export function getFilterMenuPostion(target: Element, dialogObj: Dialog): void {
  ...
  dialogObj.element.style.top =
    newpos.top + target.getBoundingClientRect().height - 5 + 'px';
 ...
 * @hidden
 */
export function getZIndexCalcualtion(
  args: { popup: Popup },
  dialogObj: Dialog
): void {
 ...
 * @hidden
 */
export function createCboxWithWrap(
  uid: string,
  elem: Element,
  className?: string
): Element {
 ...
 * @hidden
 */
export function refreshForeignData(
  row: IRow<Column>,
  columns: Column[],
  data: Object
): void {
  for (
    ...
    setValue(
      ...
      getForeignData(columns[parseInt(i.toString(), 10)], data),
      row.foreignKeyData
    );
  }
  ...
  for (let i: number = 0; i < cells.length; i++) {
    if (cells[parseInt(i.toString(), 10)].isForeignKey) {
      setValue(
        'foreignKeyData',
        getValue(
          cells[parseInt(i.toString(), 10)].column.field,
          row.foreignKeyData
        ),
...
export function getForeignData(
  ...
  let key: string | Date = <string | Date>(
    (!isNullOrUndefined(lValue)
      ? lValue
      : valueAccessor(column.field, data, column))
  );
  key = isNullOrUndefined(key) ? '' : key;
  const query: Query = new Query();
  const fdata: Object[] =
    foreignKeyData ||
    (column.dataSource instanceof DataManager &&
    column.dataSource.dataSource.json.length
      ? (<DataManager>column.dataSource).dataSource.json
      : column.columnData);
  if ((<Date>key).getDay) {
    query.where(
      getDatePredicate({
 ...
 * @hidden
 */
export function getColumnByForeignKeyValue(
 ...
 * @hidden
 */
export function getDatePredicate(
  filterObject: PredicateModel,
  type?: string,
  ...
  let prevDate: Date;
  let nextDate: Date;
  const prevObj: PredicateModel = baseExtend(
    {},
    getActualProperties(filterObject)
  ) as PredicateModel;
  const nextObj: PredicateModel = baseExtend(
    {},
    getActualProperties(filterObject)
  ) as PredicateModel;
  if (isNullOrUndefined(filterObject.value) || filterObject.value === '') {
    datePredicate = new Predicate(
      prevObj.field,
      prevObj.operator,
      prevObj.value,
      false
  ...
  if (type === 'dateonly' && !isExecuteLocal) {
    ...
    const dateOnlyString: string =
      (prevObj.value as Date).getFullYear() +
      '-' +
      padZero((prevObj.value as Date).getMonth() + 1) +
      '-' +
      padZero((prevObj.value as Date).getDate());
    const predicates: Predicate = new Predicate(
      prevObj.field,
      prevObj.operator,
      dateOnlyString,
      false
    ...
    if (
      filterObject.operator === 'equal' ||
      filterObject.operator === 'notequal'
    ) {
      if (type === 'datetime') {
        prevDate = new Date(value.setSeconds(value.getSeconds() - 1));
        nextDate = new Date(value.setSeconds(value.getSeconds() + 2));
        filterObject.value = new Date(
          value.setSeconds(nextDate.getSeconds() - 1)
        );
      } else {
      ...
      const predicateSt: Predicate = new Predicate(
        prevObj.field,
        prevObj.operator,
        prevObj.value,
        false
      );
      const predicateEnd: Predicate = new Predicate(
        nextObj.field,
        nextObj.operator,
        nextObj.value,
        false
      ...
      datePredicate =
        filterObject.operator === 'equal'
          ? predicateSt.and(predicateEnd)
          : predicateSt.or(predicateEnd);
    } else {
      if (
        type === 'date' &&
        (filterObject.operator === 'lessthanorequal' ||
          filterObject.operator === 'greaterthan')
      ) {
  ...
  if ((<{ setProperties: Function }>filterObject).setProperties) {
    (<{ setProperties: Function }>filterObject).setProperties(
...
export function isGroupAdaptive(grid: IGrid): boolean {
  return (
    ...
    grid.groupSettings.columns.length > 0 &&
    grid.isVirtualAdaptive &&
...
export function getObject(field: string = '', object?: Object): any {
  if (field) {
    ...
    for (
      let i: number = 0;
      i < splits.length && !isNullOrUndefined(value);
      i++
    ) {
      value = value[splits[parseInt(i.toString(), 10)]];
      if (isUndefined(value)) {
        const newCase: string =
          splits[parseInt(i.toString(), 10)].charAt(0).toUpperCase() +
          splits[parseInt(i.toString(), 10)].slice(1);
        value =
          object[`${newCase}`] ||
          object[`${newCase}`.charAt(0).toLowerCase() + `${newCase}`.slice(1)];
      }
    }
...
export function getExpandedState(
  ...
  for (const row of rows) {
    if (row.isExpand && !row.isDetailRow) {
      const index: number =
        gObj.allowPaging && gObj.printMode === 'AllPages'
          ? row.index +
            gObj.pageSettings.currentPage * gObj.pageSettings.pageSize -
      ...
      if (!isNullOrUndefined(index)) {
        ...
        if (gObj.childGrid) {
          obj[parseInt(index.toString(), 10)].gridModel = getPrintGridModel(
            row.childGrid,
            hierarchyPrintMode
          );
          (<{ query: Query }>(
            obj[parseInt(index.toString(), 10)].gridModel
          )).query = gObj.childGrid.query;
        }
      }
    }
  }
 ...
 * @hidden
 */
export function getPrintGridModel(
  gObj: IGrid,
  hierarchyPrintMode: HierarchyGridPrintMode = 'Expanded'
): IGrid {
  ...
  if (
    (gObj.childGrid || gObj.detailTemplate) &&
    hierarchyPrintMode !== 'None'
  ) {
    (<IGrid>printGridModel).expandedRows = getExpandedState(
      gObj,
      hierarchyPrintMode
    );
  }
 ...
 * @hidden
 */
export function extendObjWithFn(
  copied: Object,
  first: Object,
  second?: Object,
  deep?: boolean
  ...
  for (let i: number = 1; i < len; i++) {
    ...
    // eslint-disable-next-line prefer-rest-params
    const obj1: { [key: string]: Object } =
      arguments[parseInt(i.toString(), 10)];
    const keys: string[] = Object.keys(Object.getPrototypeOf(obj1)).length
      ? Object.keys(obj1).concat(getPrototypesOfObj(obj1))
      : Object.keys(obj1);
    for (let i: number = 0; i < keys.length; i++) {
      ...
      if (deep && (isObject(cpy) || Array.isArray(cpy))) {
        if (isObject(cpy)) {
          cln = source ? source : {};
          res[keys[parseInt(i.toString(), 10)]] = baseExtend(
            {},
            cln,
            cpy,
            deep
...
export function getPrototypesOfObj(obj: Object): string[] {
  ...
  while (
    ...
    Object.keys(Object.getPrototypeOf(obj)).length
  ) {
...
export function measureColumnDepth(column: Column[]): number {
  ...
  for (
    let i: number = 0;
    i < (!isNullOrUndefined(column) ? column.length : 0);
    i++
  ) {
...
export function checkDepth(col: Column, index: number): number {
  ...
  if (col.columns) {
    ...
    for (let i: number = 0; i < col.columns.length; i++) {
      indices[parseInt(i.toString(), 10)] = checkDepth(
        <Column>col.columns[parseInt(i.toString(), 10)],
        index
      );
    }
 ...
 * @hidden
 */
export function refreshFilteredColsUid(
  gObj: IGrid,
  filteredCols: PredicateModel[]
): void {
  for (let i: number = 0; i < filteredCols.length; i++) {
    filteredCols[parseInt(i.toString(), 10)].uid = filteredCols[
      parseInt(i.toString(), 10)
    ].isForeignKey
      ? getColumnByForeignKeyValue(
          filteredCols[parseInt(i.toString(), 10)].field,
          gObj.getForeignKeyColumns()
        ).uid
      ...
      : gObj.getColumnByField(filteredCols[parseInt(i.toString(), 10)].field)
          .uid;
  }
}
...
export function getTransformValues(element: Element): {
  ...
  const transformV: string = (<{ getPropertyValue?: Function }>(
    style
...
export function getScrollWidth(gObj: IGrid): number {
  const scrollElem: HTMLElement = gObj.getContent()
    .firstElementChild as HTMLElement;
  return scrollElem.scrollWidth > scrollElem.offsetWidth
    ? getScrollBarWidth()
    : 0;
}
...
export function resetColspanGroupCaption(gObj: IGrid, idx: number): number {
  ...
  const gridWidth: number =
    (gObj.width === 'auto'
      ? gObj.element.offsetWidth
      : parseInt(gObj.width.toString(), 10)) - getScrollWidth(gObj);
...
export function groupCaptionRowLeftRightPos(tr: Element, gObj: IGrid): void {
  ...
  for (let j: number = 0; j < tr.childNodes.length; j++) {
    const td: HTMLElement = tr.childNodes[
      parseInt(j.toString(), 10)
    ] as HTMLElement;
    if (frozenCount === idx) {
      break;
    ...
    if (
      td.classList.contains('e-groupcaption') ||
      td.classList.contains('e-summarycell')
    ) {
    ...
    if (
      td.classList.contains('e-groupcaption') &&
      parseInt(td.getAttribute('colspan'), 10) === 1
    ) {
      td.classList.add('e-freezeleftborder');
    }
    applyStickyLeftRightPosition(td, width, gObj.enableRtl, 'Left');
    if (
      td.classList.contains('e-indentcell') ||
      td.classList.contains('e-recordplusexpand') ||
    ...
    if (td.classList.contains('e-groupcaption')) {
      ...
      for (let i: number = 0; i < cols.length; i++) {
        ...
        if (cols[parseInt(i.toString(), 10)].visible) {
          width += parseInt(
            cols[parseInt(i.toString(), 10)].width.toString(),
            10
          );
          colspan--;
        }
...
export function ensureLastRow(row: Element, gridObj: IGrid): boolean {
  const content: HTMLElement = gridObj.getContent()
    .firstElementChild as HTMLElement;
  return (
    row &&
...
export function isRowEnteredInGrid(index: number, gObj: IGrid): boolean {
  ...
  const startIndex: number =
    gObj.getContent().firstElementChild.scrollTop / rowHeight;
  const endIndex: number =
    startIndex +
    (gObj.getContent().firstElementChild as HTMLElement).offsetHeight /
      rowHeight;
...
export function ispercentageWidth(gObj: IGrid): boolean {
  ...
  for (let i: number = 0; i < columns.length; i++) {
    if (isUndefined(columns[parseInt(i.toString(), 10)].width)) {
      ...
      columns[parseInt(i.toString(), 10)].width.toString().indexOf('%') !== -1
    ) {
      percentageCol++;
    }
  }
  return (
    (gObj.width === 'auto' ||
      (typeof gObj.width === 'string' && gObj.width.indexOf('%') !== -1)) &&
    !gObj.groupSettings.showGroupedColumn &&
...
export function resetRowIndex(
  ...
  for (
    let i: number = startRowIndex ? startRowIndex : 0;
    i < rows.length;
    i++
  ) {
    if (
      ...
      rows[parseInt(i.toString(), 10)].isAltRow = gObj.enableAltRow
        ? startIndex % 2 !== 0
        : false;
      rowElms[parseInt(i.toString(), 10)].setAttribute(
        literals.dataRowIndex,
        startIndex.toString()
      );
      rowElms[parseInt(i.toString(), 10)].setAttribute(
        literals.ariaRowIndex,
      ...
      for (
        let j: number = 0;
        j < rowElms[parseInt(i.toString(), 10)].cells.length;
        j++
      ) {
        rowElms[parseInt(i.toString(), 10)].cells[
          parseInt(j.toString(), 10)
        ].setAttribute('index', startIndex.toString());
      }
      startIndex++;
    }
  }
...
export function resetCachedRowIndex(gObj: IGrid): void {
  ...
  for (
    let i: number = 0, startIndex: number = 0, k: number = 0;
    i < rowObjects.length;
    i++
  ) {
    ...
    if (rowObject.isDataRow) {
      ...
      const rowElement: HTMLTableRowElement | Element = gObj.getRowElementByUID(
        rowObject.uid
      );
      if (!isNullOrUndefined(rowElement)) {
        ...
        for (
          let j: number = 0;
          j < (rowElement as HTMLTableRowElement).cells.length;
          j++
        ) {
          (rowElement as HTMLTableRowElement).cells[
            parseInt(j.toString(), 10)
          ].setAttribute('index', startIndex.toString());
        }
        k++;
 ...
 * @hidden
 */
export function groupReorderRowObject(
  gObj: IGrid,
  args: RowDropEventArgs,
  tr: HTMLTableRowElement[],
  dropRObj?: Row<Column>
): void {
  const rowObjects: Row<Column>[] = gObj.enableVirtualization
    ? gObj.vRows
    : gObj.getRowsObject();
  const orderChangeRowObjects: Row<Column>[] = [];
  const dropRowObject: Row<Column> = dropRObj
    ? dropRObj
    : gObj.getRowObjectFromUID(
        args.target.closest('tr').getAttribute('data-uid')
      );
  let rowObjectDropIndex: number;
  for (let i: number = 0; i < args.rows.length; i++) {
    const orderChangeRowObject: Row<Column> = gObj.getRowObjectFromUID(
      args.rows[parseInt(i.toString(), 10)].getAttribute('data-uid')
    );
    if (dropRowObject === orderChangeRowObject) {
      rowObjectDropIndex = rowObjects.indexOf(dropRowObject);
    }
    orderChangeRowObjects.push(
      rowObjects.splice(rowObjects.indexOf(orderChangeRowObject), 1)[0]
    );
  }
  ...
  if (!gObj.enableVirtualization && !gObj.infiniteScrollSettings.enableCache) {
    ...
    for (let i: number = 0, len: number = tr.length; i < len; i++) {
      const index: number = parseInt(
        tr[parseInt(i.toString(), 10)].getAttribute(literals.dataRowIndex),
        10
      );
      record[parseInt(i.toString(), 10)] =
        currentViewData[parseInt(index.toString(), 10)];
    }
...
export function compareChanges(
  ...
  const newArray: Object[] = (<{ dataToBeUpdated?: Object }>(
    gObj
  )).dataToBeUpdated[`${type}`]
    .concat(changes[`${type}`])
    .reduce((r: Object, o: Object) => {
      r[o[`${keyField}`]] =
        r[o[`${keyField}`]] === undefined
          ? o
          : Object.assign(r[o[`${keyField}`]], o);
      return r;
    }, {});
  (<{ dataToBeUpdated?: Object }>gObj).dataToBeUpdated[`${type}`] = Object.keys(
    newArray
  ).map((k: string) => newArray[`${k}`]);
}
...
export function setRowElements(gObj: IGrid): void {
  (<{ rowElements?: Element[] }>gObj.contentModule).rowElements = [].slice.call(
    gObj.element.querySelectorAll(
      '.e-row:not(.e-addedrow):not(.e-cloneproperties .e-row)'
    )
  );
}
...
export function resetDialogAppend(gObj: IGrid, dlgObj: Dialog): void {
  let element: HTMLElement = gObj.createElement('div', {
    className: 'e-grid-popup',
    id: gObj.element.id + '_e-popup',
  });
  const pos: { left: number; top: number } = calculatePosition(
    gObj.element,
    'left',
    'Top'
  );
  ...
  if (sbPanel) {
    const sbPos: { left: number; top: number } = calculateRelativeBasedPosition(
      gObj.element,
      sbPanel
    );
...
export function frozenDirection(column: Column): string {
  if (
    (column.columns[0] as Column).freeze ||
    (column.columns[0] as Column).isFrozen
  ) {
    if (
      (column.columns[0] as Column).freeze === 'Left' ||
      (column.columns[0] as Column).isFrozen
    ) {
      return 'Left';
    ...
    if (
      (column.columns[0] as Column).columns &&
      (column.columns[0] as Column).columns.length
    ) {
...
export function addFixedColumnBorder(row: Element): void {
  if (row.querySelector('.e-fixedfreeze')) {
    ...
    for (let j: number = 0; j < cells.length; j++) {
      if (
        (cells[parseInt(j.toString(), 10)] as Element).classList.contains(
          'e-fixedfreeze'
        ) &&
        (!cells[j - 1] ||
          (cells[j - 1] &&
            !(cells[j - 1] as Element).classList.contains('e-fixedfreeze')))
      ) {
        (cells[parseInt(j.toString(), 10)] as Element).classList.add(
          'e-freezeleftborder'
        );
      }
      if (
 ...
 * @hidden
 */
export function applyStickyLeftRightPosition(
  node: HTMLElement,
  width: number,
  isRtl: boolean,
  position: string
 ...
 * @hidden
 */
export function resetColandRowSpanStickyPosition(
  gObj: IGrid,
  column: Column,
  node: Element,
  colSpan: number
  ...
  if (
    column.freeze === 'Left' &&
    (<{ border?: string }>column).border !== 'Left'
  ) {
    ...
    if (
      (<{ border?: string }>columns[parseInt(idx.toString(), 10)]).border ===
      'Left'
    ) {
    ...
    for (let j: number = index + 1; j < index + colSpan; j++) {
      ...
      if (columns[parseInt(j.toString(), 10)].visible) {
        width += parseInt(
          columns[parseInt(j.toString(), 10)].width.toString(),
          10
        );
      } else {
        colSpan++;
    ...
    if (gObj.enableRtl) {
      (node as HTMLElement).style.left =
        parseInt((node as HTMLElement).style.left, 10) - width + 'px';
    } else {
      (node as HTMLElement).style.right =
        parseInt((node as HTMLElement).style.right, 10) - width + 'px';
    }
  }
}
 ...
 * @hidden
 */
export function getCellFromRow(
  gObj: IGrid,
  rowIndex: number,
  colIndex: number
): Element {
  const row: HTMLTableRowElement = <HTMLTableRowElement>(
    gObj.getRowByIndex(rowIndex)
  );
  for (let i: number = 0; i < row.cells.length; i++) {
    if (
      row.cells[parseInt(i.toString(), 10)].getAttribute('data-colindex') ===
      colIndex.toString()
    ) {
 ...
 * @hidden
 */
export function addStickyColumnPosition(
  gObj: IGrid,
  column: Column,
  node: Element
): void {
  if (column.freeze === 'Left' || column.isFrozen) {
    ...
    if (column.index === cols[cols.length - 1].index) {
      const width: number =
        gObj.getFrozenMode() === 'Right' && gObj.isRowDragable() ? 30 : 0;
      applyStickyLeftRightPosition(
        node as HTMLElement,
        width,
        gObj.enableRtl,
 ...
 * @hidden
 */
export function getCellsByTableName(
  gObj: IGrid,
  col: Column,
  rowIndex: number
): Element[] {
  return [].slice.call(
    gObj
      .getDataRows()
      [parseInt(rowIndex.toString(), 10)].getElementsByClassName(
        literals.rowCell
      )
  );
}
 ...
 * @hidden
 */
export function getCellByColAndRowIndex(
  gObj: IGrid,
  col: Column,
  rowIndex: number,
  index: number
): Element {
  return getCellsByTableName(gObj, col, rowIndex)[
...
export function setValidationRuels(
  ...
  if (isCustom) {
    ...
    if (
      col.getFreezeTableName() === literals.frozenLeft ||
      (!index && col.getFreezeTableName() === literals.frozenRight) ||
      len === 1
    ) {
      rules[getComplexFieldID(col.field)] = col.validationRules;
    } else if (
      col.getFreezeTableName() === 'movable' ||
      !col.getFreezeTableName()
    ) {
  ...
  if (type === 'number') {
    try {
      format = intl.getNumberPattern(
        { format: numberFormat, currency: currencyCode, useGrouping: true },
        true
      );
    } catch (error) {
      format = numberFormat;
    ...
    try {
      format = intl.getDatePattern(
        { skeleton: numberFormat, type: type },
        isExcel
      );
...
export function addBiggerDialog(gObj: IGrid): void {
  if (gObj.enableAdaptiveUI) {
    const dialogs: HTMLCollectionOf<Element> = document.getElementsByClassName(
      'e-responsive-dialog'
    );
 ...
 * @hidden
 */
export function performComplexDataOperation(
  value: string,
  mapObject: Object
): Object | string {
...
export function setDisplayValue(
  ...
  for (let i: number = 0; i < trs.length; i++) {
    let td: HTMLTableCellElement =
      tr[trs[parseInt(i.toString(), 10)]].querySelectorAll('td.e-rowcell')[
        parseInt(idx.toString(), 10)
      ];
    if (parent && !parent.isFrozenGrid() && !parent.isRowDragable()) {
      td =
        !isNullOrUndefined(td) &&
        (parseInt(td.getAttribute('data-colindex'), 10) === idx ||
          (parentsUntil(td, 'e-addedrow') &&
            td.parentElement.childNodes[parseInt(idx.toString(), 10)] === td))
          ? td
          : tr[parseInt(i.toString(), 10)].querySelector(
              `td[data-colindex="${idx}"]`
            );
      if (isNullOrUndefined(td)) {
        ...
        idx = parent
          .getContentTable()
          .querySelector('.e-detailrowcollapse, .e-detailrowexpand')
          ? td.cellIndex - 1
          : td.cellIndex;
      }
    }
    if (
      ...
      if (
        tr[trs[parseInt(i.toString(), 10)]]
          .querySelectorAll('td.e-rowcell')
          [parseInt(idx.toString(), 10)].classList.contains('e-hide')
      ) {
        removeClass(
          [
            tr[trs[parseInt(i.toString(), 10)]].querySelectorAll(
              'td.e-rowcell'
            )[parseInt(idx.toString(), 10)],
          ],
          ['e-hide']
      ...
      if (
        (isContent && parent.isRowDragable()) ||
        (parent && parent.isDetail())
      ) {
        const index: number = idx + 1;
        rows[trs[parseInt(i.toString(), 10)]].cells[
          parseInt(index.toString(), 10)
        ].visible = displayVal === '' ? true : false;
      } else {
        if (!isNullOrUndefined(rows[trs[parseInt(i.toString(), 10)]])) {
          ...
          if (
            ...
            tr[trs[parseInt(i.toString(), 10)]]
              .querySelectorAll('td.e-rowcell')
              [parseInt(idx.toString(), 10)].classList.add('e-hide');
          }
        }
      }
      idx = actualIndex;
...
export function addRemoveEventListener(
  parent: IGrid,
  evt: { event: string; handler: Function }[],
  isOn: boolean,
  module?: Object
): void {
...
export function getColumnModelByFieldName(gObj: IGrid, field: string): Column {
  let column: Column;
  if (!(<{ columnModel?: Column[] }>gObj).columnModel) {
    <Column[]>gObj.getColumns();
  }
  for (const col of (<{ columnModel?: Column[] }>gObj).columnModel) {
    if (col.field === field) {
      column = col;
...
export function registerEventHandlers(
  ...
  for (let i: number = 0; i < evts.length; i++) {
    instance.eventHandlers[`${id}`][evts[parseInt(i.toString(), 10)]] =
      handlers[evts[parseInt(i.toString(), 10)]];
  }
}
...
export function removeEventHandlers(
  ...
  for (let i: number = 0; i < evts.length; i++) {
    ...
    component.removeEventListener(
      ...
      instance.eventHandlers[component.element.id][
        evts[parseInt(i.toString(), 10)]
      ]
    );
  }
 ...
 * @hidden
 */
export function clearReactVueTemplates(
  parent: IGrid | IXLFilter,
  templates: string[]
): void {
...
export function getParentIns(grid: IGrid): IGrid {
  return grid.parentDetails && grid.parentDetails.parentInstObj
    ? getParentIns(grid.parentDetails.parentInstObj)
    : grid;
}
 ...
 */
// eslint-disable-next-line
export function generateExpandPredicates(
  fields: string[],
  values: string[],
  instance: any
): Predicate {
  ...
  for (let i: number = 0; i < fields.length; i++) {
    const column: Column = instance.parent.getColumnByField(
      fields[parseInt(i.toString(), 10)]
    );
    const value: string =
      values[parseInt(i.toString(), 10)] === 'null'
        ? null
    ...
    if (value === '' || isNullOrUndefined(value)) {
      filterCols = filterCols.concat(
        CheckBoxFilterBase.generateNullValuePredicates(pred)
      );
    } else {
 ...
 */
export function getGroupKeysAndFields(
  index: number,
  rowsObject: Row<Column>[]
): { fields: string[]; keys: string[] } {
  const fields: string[] = [];
  const keys: string[] = [];
  for (let i: number = index; i >= 0; i--) {
    if (
      rowsObject[parseInt(i.toString(), 10)].isCaptionRow &&
      fields.indexOf(
        (rowsObject[parseInt(i.toString(), 10)].data as GroupedData).field
      ) === -1 &&
      (rowsObject[parseInt(i.toString(), 10)].indent <
        rowsObject[parseInt(index.toString(), 10)].indent ||
        i === index)
    ) {
      fields.push(
        (rowsObject[parseInt(i.toString(), 10)].data as GroupedData).field
      ...
      keys.push(
        (rowsObject[parseInt(i.toString(), 10)].data as GroupedData).key
      );
...
export function findCellIndex(
  checkActiveMatrix: number[][],
  checkCellIndex: number[],
  next: boolean
): number[] {
  ...
  if (next) {
    for (let i: number = cellIndex[0]; i < activeMatrix.length; i++) {
      ...
      for (let j: number = 0; j < rowCell.length; j++) {
        if (
          currentCellIndexPass &&
          activeMatrix[parseInt(i.toString(), 10)][
            parseInt(j.toString(), 10)
          ] === 1
        ) {
          cellIndex = [i, j];
        ...
        if (
          !currentCellIndexPass &&
          cellIndex.toString() === [i, j].toString()
        ) {
