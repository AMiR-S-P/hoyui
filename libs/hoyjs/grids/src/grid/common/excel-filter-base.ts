import { EventHandler, remove, Browser, KeyboardEventArgs } from '@hoyui/base';
...
import { DropDownList, AutoComplete, ChangeEventArgs } from '@hoyui/dropdowns';
import { NumericTextBox } from '@hoyui/inputs';
import { RadioButton, CheckBox } from '@hoyui/buttons';
import {
...
import { DatePicker, DateTimePicker } from '@hoyui/calendars';
import { OffsetPosition } from '@hoyui/popups';
import {
  parentsUntil,
  appendChildren,
  extend,
...
export class ExcelFilterBase extends CheckBoxFilterBase {
          ...
          model.push({
            text:
              this.getLocalizedLabel(
                options[`${type}`][parseInt(i.toString(), 10)]
              ) + '...',
            iconCss:
              'e-icons e-icon-check ' +
              (operator ===
              options[`${type}`][parseInt(i.toString(), 10)].toLowerCase()
                ? ''
                : 'e-emptyicon'),
          });
        } else {
  ...
  public destroy(): void {
    ...
    if (!this.isDevice && this.menuObj) {
      const li: HTMLElement =
        this.menuObj.element.querySelector('li.e-focused');
  ...
  private createMenu(
    ...
    const icon: string = isFiltered
      ? 'e-excl-filter-icon e-filtered'
      : 'e-excl-filter-icon';
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if (
      this.parent.allowSorting &&
      (this.parent as any).getModuleName() === 'grid' &&
      !this.options.isResponsiveFilter
    ) {
      const hdrele: string = this.parent
        .getColumnHeaderByUid(eleOptions.uid)
        .getAttribute('aria-sort');
      const colIsSort: object = this.parent.getColumnByField(
        eleOptions.field
      ).allowSorting;
      const isAsc: string =
        !colIsSort || hdrele === 'ascending'
          ? 'e-disabled e-excel-ascending'
          : 'e-excel-ascending';
      const isDesc: string =
        !colIsSort || hdrele === 'descending'
          ? 'e-disabled e-excel-descending'
          : 'e-excel-descending';
      const ascName: string =
        type === 'string'
          ? this.getLocalizedLabel('SortAtoZ')
          : type === 'datetime' || type === 'date'
          ? this.getLocalizedLabel('SortByOldest')
          : this.getLocalizedLabel('SortSmallestToLargest');
      const descName: string =
        type === 'string'
          ? this.getLocalizedLabel('SortZtoA')
          : type === 'datetime' || type === 'date'
          ? this.getLocalizedLabel('SortByNewest')
          : this.getLocalizedLabel('SortLargestToSmallest');
      ...
      const separator: Element = this.parent.createElement('li', {
    ...
    if (!this.options.isResponsiveFilter) {
      ul.appendChild(
        this.createMenuElem(
          this.getLocalizedLabel('ClearFilter'),
          isFiltered ? '' : 'e-disabled',
          icon
        )
      );
    }
    if (type !== 'boolean') {
      ul.appendChild(
        this.createMenuElem(
          this.getLocalizedLabel(options[`${type}`]),
          'e-submenu',
          isCheckIcon && this.ensureTextFilter()
            ? 'e-icon-check'
            : icon + ' e-emptyicon',
          true
        )
  ...
  private createMenuElem(
    ...
    const li: Element = this.parent.createElement('li', {
    ...
    if (isSubMenu) {
      li.appendChild(
        this.parent.createElement('span', { className: 'e-icons e-caret' })
      );
    }
    return li;
  }
  ...
  private clickExHandler(e: MouseEvent): void {
    ...
    const menuItem: HTMLElement = parentsUntil(
      e.target as Element,
      'e-menu-item'
    ) as HTMLElement;
    if (menuItem) {
      if (this.getLocalizedLabel('ClearFilter') === menuItem.innerText.trim()) {
        ...
        (this.options.isResponsiveFilter || Browser.isDevice) &&
        this.getLocalizedLabel(options[this.options.type]) ===
  ...
  private focusNextOrPrevElement(
    ...
    if (nextElement) {
      ...
      const focusClass: string = nextElement.classList.contains('e-chk-hidden')
        ? 'e-chkfocus'
        : focusClassName;
      const target: Element = nextElement.classList.contains('e-chk-hidden')
        ? parentsUntil(nextElement, 'e-ftrchk')
        : parentsUntil(nextElement, 'e-menu-item');
  ...
  private keyUp(e: KeyboardEventArgs): void {
    if ((e.key === 'Tab' && e.shiftKey) || e.key === 'Tab') {
      const focusClass: string = (e.target as HTMLElement).classList.contains(
        'e-chk-hidden'
      )
        ? 'e-chkfocus'
        : 'e-menufocus';
      const target: Element = (e.target as HTMLElement).classList.contains(
        ...
        ? parentsUntil(e.target as HTMLElement, 'e-menu-item')
        : parentsUntil(e.target as HTMLElement, 'e-ftrchk');
      this.excelSetFocus(target, focusClass);
      ...
      (e.key === 'Enter' || e.code === 'ArrowRight') &&
      (e.target as Element).classList.contains('e-menu-item')
    ) {
      e.preventDefault();
      ...
      if ((e.target as HTMLElement).classList.contains('e-submenu')) {
        this.hoverHandler(e);
        (
          this.menuObj.element.querySelector('.e-menu-item') as HTMLElement
        ).focus();
        this.excelSetFocus(
          parentsUntil(
            this.menuObj.element.querySelector('.e-menu-item'),
            'e-menu-item'
          ),
          'e-focused'
        );
  ...
  private hoverHandler(e: MouseEvent | KeyboardEventArgs): void {
    ...
    const target: Element = (e.target as Element).querySelector(
      '.e-contextmenu'
    );
    ...
    if (!this.isCMenuOpen && isSubMenu) {
      ...
      const menuOptions: ContextMenuModel = {
        items: this.getCMenuDS(
          this.options.type,
          selectedMenu ? selectedMenu.replace(/\s/g, '') : undefined
        ),
        ...
        cssClass:
          this.options.isResponsiveFilter && this.parent.cssClass
            ? 'e-res-contextmenu-wrapper' + ' ' + this.parent.cssClass
            : this.options.isResponsiveFilter
      ...
      const client: ClientRect = this.menu
        .querySelector('.e-submenu')
      ...
      if (this.options.isResponsiveFilter) {
        ...
        const header: string = this.getLocalizedLabel(
        ...
        if (Browser.isDevice) {
          ...
          pos.top = Browser.isIE
            ? window.pageYOffset + client.top
            : window.scrollY + client.top;
  ...
  private contextKeyDownHandler(e: KeyboardEventArgs): void {
    if ((e.key === 'Tab' && e.shiftKey) || e.key === 'Tab') {
      e.preventDefault();
      const focusableElements: HTMLElement[] = Array.from(
        this.menuObj.element.querySelectorAll(
      ...
      (
        document.querySelector('.e-submenu.e-menu-item') as HTMLElement
  ...
  private ensureTextFilter(): string {
    ...
    if (predicates && predicates.length === 2) {
      if (
        predicates[0].operator === 'greaterthanorequal' &&
        predicates[1].operator === 'lessthanorequal'
      ) {
      ...
      if (predicates && predicates.length === 1) {
        this.optrData =
          this.customFilterOperators[this.options.type + 'Operator'];
  ...
  private preventClose(args: BeforeOpenCloseMenuEventArgs): void {
    if (this.options && this.options.isResponsiveFilter && args.event) {
      ...
      const isFilterBack: boolean =
        target.classList.contains('e-resfilterback') ||
        target.classList.contains('e-res-back-btn') ||
        target.classList.contains('e-menu-item');
      args.cancel = !isFilterBack;
    } else {
      if (
        args.event instanceof MouseEvent &&
  ...
  private getCMenuYPosition(target: Element): number {
    ...
    const leftPos: number =
      targetPosition.right + contextWidth - this.parent.element.clientWidth;
    let targetBorder: number =
      (target as HTMLElement).offsetWidth - (target as HTMLElement).clientWidth;
    targetBorder = targetBorder ? targetBorder + 1 : 0;
    return leftPos < 1
      ? targetPosition.right + 1 - targetBorder
      : targetPosition.left - contextWidth - 1 + targetBorder;
  }
  ...
  public openDialog(options: IFilterArgs): void {
    ...
    const filterLength: number =
      (this.existingPredicate[options.field] &&
        this.existingPredicate[options.field].length) ||
    ...
    if (
      ...
      !isNullOrUndefined((this.parent as IGrid).getContent) &&
      (this.parent as IGrid).getContent() &&
    ...
    const menuItems: NodeListOf<Element> =
      this.dlg.querySelectorAll('.e-menu-item');
  ...
  public renderDialogue(e?: MenuEventArgs): void {
    ...
    const isComplex: boolean =
      !isNullOrUndefined(column) && isComplexField(column);
    const complexFieldName: string =
      !isNullOrUndefined(column) && getComplexFieldID(column);
    ...
    if (this.options.isResponsiveFilter) {
      const responsiveCnt: HTMLElement = document.querySelector(
        '.e-resfilter > .e-dlg-content > .e-xl-customfilterdiv'
      );
    ...
    this.dlgObj = new Dialog({
      ...
      open: () => {
        const rows: HTMLElement[] = [].slice.call(
          this.dlgObj.element.querySelectorAll(
            'table.e-xlfl-table tr.e-xlfl-fields'
          )
        );
        for (let i: number = 0; i < rows.length; i++) {
          const valInput: HTMLInputElement =
            rows[i as number].children[1].querySelector('.e-control');
          const dropDownList: DropDownList = rows[i as number].querySelector(
            '.e-dropdownlist.e-control'
          )['ej2_instances'][0] as DropDownList;
          if (
            dropDownList.value === 'isempty' ||
            dropDownList.value === 'isnotempty' ||
            ...
            valInput &&
            !isNullOrUndefined(valInput.getAttribute('disabled'))
          ) {
            valInput['ej2_instances'][0]['enabled'] = true;
          }
        }
        const row: HTMLTableRowElement = this.dlgObj.element.querySelector(
          'table.e-xlfl-table>tr'
        ) as HTMLTableRowElement;
        if (this.options.column.filterTemplate) {
          const templateField: string = isComplexField(
            this.options.column.field
          )
            ? getComplexFieldID(this.options.column.field)
            : this.options.column.field;
          const isReactCompiler: boolean =
            this.parent.isReact &&
            typeof this.options.column.filterTemplate !== 'string';
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      buttons: [
        {
          ...
          buttonModel: {
            ...
            cssClass: this.parent.cssClass
              ? 'e-xlfl-okbtn' + ' ' + this.parent.cssClass
              : 'e-xlfl-okbtn',
          },
        },
        {
          ...
          buttonModel: {
            ...
            cssClass: this.parent.cssClass
              ? 'e-xlfl-cancelbtn' + ' ' + this.parent.cssClass
              : 'e-xlfl-cancelbtn',
          },
        },
      ],
    ...
    this.dlgDiv.setAttribute(
      'aria-label',
      this.getLocalizedLabel('CustomFilterDialogARIA')
    );
  ...
  private renderResponsiveDialog(): void {
    if (this.options.isResponsiveFilter) {
      const rowResponsiveDlg: Element = document.querySelector(
      ...
      this.dlgObj.target = document.querySelector(
        '.e-resfilter > .e-dlg-content > .e-xl-customfilterdiv'
      ) as HTMLElement;
  ...
  private renderCustomFilter(target: Element, column: string): void {
    const dlgConetntEle: Element =
      this.dlgObj.element.querySelector('.e-xlfl-maindiv');

    const dlgFields: HTMLElement = this.parent.createElement('div', {
    ...
    //column name
    const fieldSet: HTMLElement = this.parent.createElement('div', {
  ...
  public filterBtnClick(col: string): void {
    ...
    const complexFieldName: string =
      !isNullOrUndefined(col) && getComplexFieldID(col);
    const colValue: string = isComplex ? complexFieldName : col;
    const fValue: NumericTextBox = (<EJ2Intance>(
      this.dlgDiv.querySelector('#' + colValue + '-xlfl-frstvalue')
    )).ej2_instances[0];
    const fOperator: DropDownList = (<EJ2Intance>(
      this.dlgDiv.querySelector('#' + colValue + '-xlfl-frstoptr')
    )).ej2_instances[0];
    const sValue: NumericTextBox = (<EJ2Intance>(
      this.dlgDiv.querySelector('#' + colValue + '-xlfl-secndvalue')
    )).ej2_instances[0];
    const sOperator: DropDownList = (<EJ2Intance>(
      this.dlgDiv.querySelector('#' + colValue + '-xlfl-secndoptr')
    )).ej2_instances[0];
    let checkBoxValue: boolean;
    if (this.options.type === 'string') {
      const checkBox: CheckBox = (<EJ2Intance>(
        this.dlgDiv.querySelector('#' + colValue + '-xlflmtcase')
      )).ej2_instances[0];
      checkBoxValue = checkBox.checked;
    }
    const andRadio: CheckBox = (<EJ2Intance>(
      this.dlgDiv.querySelector('#' + colValue + 'e-xlfl-frstpredicate')
    )).ej2_instances[0];
    ...
    this.filterByColumn(
      this.options.field,
      fOperator.value as string,
      fValue.value,
      predicate,
      checkBoxValue,
      this.options.ignoreAccent,
      sOperator.value as string,
      sValue.value
    );
    this.removeDialog();
  }
  ...
  public filterByColumn(
    ...
    firstValue: string | number | Date | boolean,
    predicate?: string,
    matchCase?: boolean,
    ignoreAccent?: boolean,
    secondOperator?: string,
    secondValue?: string | number | Date | boolean
  ): void {
    const col: Column = this.parent.getColumnByField
      ? this.parent.getColumnByField(fieldName)
      : this.options.column;
    const field: string = this.isForeignColumn(col)
      ? col.foreignKeyValue
      : fieldName;
    const fColl: PredicateModel[] = [];
    let mPredicate: Predicate;
    const arg: {
      instance: ExcelFilterBase;
      handler: Function;
      cancel: boolean;
      arg1: string;
      arg2: string;
      ...
      arg1: fieldName,
      arg2: firstOperator,
      arg3: firstValue as string,
      arg4: predicate,
      arg5: matchCase,
      arg6: ignoreAccent,
      arg7: secondOperator,
      arg8: secondValue as string,
      cancel: false,
    };
    ...
    mPredicate = new Predicate(
      field,
      firstOperator.toLowerCase(),
      arg.arg3,
      !matchCase,
      ignoreAccent
    );
    if (!isNullOrUndefined(secondOperator)) {
      ...
      mPredicate = (mPredicate as Object)[`${predicate}`](
        field,
        secondOperator.toLowerCase(),
  ...
  private renderOperatorUI(
    ...
    isFirst?: boolean
  ): { fieldElement: HTMLElement; operator: string } {
    const fieldElement: HTMLElement = this.parent.createElement('tr', {
      className: 'e-xlfl-fields',
      attrs: { role: 'row' },
    });
    table.appendChild(fieldElement);

    const xlfloptr: HTMLElement = this.parent.createElement('td', {
      className: 'e-xlfl-optr',
    });
    fieldElement.appendChild(xlfloptr);

    const optrDiv: HTMLElement = this.parent.createElement('div', {
    ...
    const optrInput: HTMLElement = this.parent.createElement('input', {
    ...
    const dropDatasource: { [key: string]: Object }[] =
      this.customFilterOperators[`${optr}`];
    this.optrData = dropDatasource;
    let selectedValue: string = this.dropSelectedVal(
      this.options.column as Column,
      predicates,
      isFirst
    );
    ...
    if (this.menuItem) {
      ...
      if (menuText === this.getLocalizedLabel('Between')) {
        selectedValue = this.getLocalizedLabel(
          isFirst ? 'GreaterThanOrEqual' : 'LessThanOrEqual'
        );
      }
    }
    const col: Column = this.options.column as Column;
    const dropOptr: DropDownList = new DropDownList(
      extend(
        {
    ...
    registerEventHandlers(
      optrInput.id,
      [literals.open, literals.change],
      evt,
      this
    );
    dropOptr.addEventListener(
      literals['open'],
      this.eventHandlers[optrInput.id][literals.open]
    );
    dropOptr.addEventListener(
      literals.change,
      this.eventHandlers[optrInput.id][literals.change]
    );
  ...
  private removeHandlersFromComponent(
    component: DropDownList | AutoComplete
  ): void {
    if (component.element.classList.contains('e-dropdownlist')) {
      ...
      removeEventHandlers(
        component,
        [events.actionComplete, literals.focus],
        this
      );
    }
  }
  ...
  private dropDownValueChange(args: ChangeEventArgs): void {
    ...
    const valInput: HTMLInputElement = args.element
      .closest('.e-xlfl-fields')
      .children[1].querySelector('.e-control');
    const dropDownList: DropDownList = args.element[
      'ej2_instances'
    ][0] as DropDownList;
    if (
      dropDownList.value === 'isempty' ||
  ...
  private renderFilterUI(column: string, dlgConetntEle: Element): void {
    ...
    let optr: { fieldElement: HTMLElement; operator: string } =
      this.renderOperatorUI(column, table, '-xlfl-frstoptr', predicates, true);
    this.firstOperator = optr.operator;

    //Renders first value
    this.renderFlValueUI(column, optr, '-xlfl-frstvalue', predicates, true);

    const predicate: HTMLElement = this.parent.createElement('tr', {
  ...
  private renderRadioButton(
    ...
    const radioDiv: HTMLElement = this.parent.createElement('div', {
    ...
    const complexFieldName: string =
      !isNullOrUndefined(column) && getComplexFieldID(column);
    const frstpredicate: HTMLInputElement = this.parent.createElement('input', {
      id: isComplex
        ? complexFieldName + 'e-xlfl-frstpredicate'
        : column + 'e-xlfl-frstpredicate',
      attrs: { type: 'radio' },
    }) as HTMLInputElement;

    const secndpredicate: HTMLInputElement = this.parent.createElement(
      'input',
      {
        id: isComplex
          ? complexFieldName + 'e-xlfl-secndpredicate'
          : column + 'e-xlfl-secndpredicate',
        attrs: { type: 'radio' },
      }
    ) as HTMLInputElement;
    ...
    const flValue: string =
      predicates && predicates.length === 2
        ? (predicates[1].predicate as string)
        : 'and';
    if (flValue === 'and') {
  ...
  private renderFlValueUI(
    column: string,
    optr: { fieldElement: HTMLElement; operator: string },
    elementId: string,
    predicates: PredicateModel[],
    isFirst?: boolean
  ): void {
    const value: HTMLElement = this.parent.createElement('td', {
    ...
    const valueDiv: HTMLElement = this.parent.createElement('div', {
      className: 'e-xlfl-valuediv',
    });
    const isFilteredCol: boolean = this.options.filteredColumns.some(
      (col: Column) => {
        return column === col.field;
      }
    );
    const fltrPredicates: Object[] = this.options.filteredColumns.filter(
      (col: Column) => col.field === column
    );
    if (this.options.column.filterTemplate) {
      let data: Object = {};
      const columnObj: Column = this.options.column as Column;
      if (isFilteredCol && elementId) {
        data = this.getExcelFilterData(
          elementId,
          data,
          columnObj,
          predicates,
      ...
      const isReactChild: boolean =
        ...
        (this.parent as any).parentDetails.parentInstObj.isReact;
      const tempID: string =
        this.parent.element.id + columnObj.uid + 'filterTemplate';
      if (isReactCompiler || isReactChild) {
        (this.options.column as Column).getFilterTemplate()(
          data,
          this.parent,
          'filterTemplate',
          tempID,
      ...
      if (isReactCompiler || isReactChild) {
        this.parent.renderTemplates(function (): void {
          valueDiv.querySelector('input').id = isComplex
            ? complexFieldName + elementId
            : column + elementId;
          value.appendChild(valueDiv);
        });
      } else {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ((this.parent as any).isAngular
          ? valueDiv.children[0]
          : valueDiv.querySelector('input')
        ).id = isComplex ? complexFieldName + elementId : column + elementId;
        value.appendChild(valueDiv);
      }
    } else {
      const valueInput: Element = this.parent.createElement('input', {
        id: isComplex ? complexFieldName + elementId : column + elementId,
      });
      ...
      if (predicates && predicates.length > 0) {
        ...
        flValue =
          predicate && predicate.operator === optr.operator
            ? predicate.value
            : undefined;
  ...
  private getExcelFilterData(
    ...
    columnObj?: Column,
    predicates?: PredicateModel[],
    fltrPredicates?: Object[]
  ): Object {
    const predIndex: number = elementId === '-xlfl-frstvalue' ? 0 : 1;
    if (elementId === '-xlfl-frstvalue' || fltrPredicates.length > 1) {
      data = {
        column:
          predicates instanceof Array
            ? predicates[parseInt(predIndex.toString(), 10)]
            : predicates,
      };
      const indx: number =
        this.options.column.columnData && fltrPredicates.length > 1
          ? this.options.column.columnData.length === 1
            ? 0
            : 1
          : predIndex;
      data[this.options.field] = columnObj.foreignKeyValue
        ? this.options.column.columnData[parseInt(indx.toString(), 10)][
            columnObj.foreignKeyValue
          ]
        : (
            (<HTMLInputElement>(
              fltrPredicates[parseInt(indx.toString(), 10)]
            )) as { value?: string | boolean | Date }
          ).value;
      if (this.options.foreignKeyValue) {
        data[this.options.foreignKeyValue] =
          this.options.column.columnData[parseInt(indx.toString(), 10)][
            columnObj.foreignKeyValue
          ];
      }
  ...
  private renderMatchCase(
    column: string,
    tr: HTMLElement,
    matchCase: HTMLElement,
    elementId: string,
    predicates: PredicateModel[]
  ): void {
    const matchCaseDiv: HTMLElement = this.parent.createElement('div', {
    ...
    const flValue: boolean =
      predicates && predicates.length > 0
        ? predicates && predicates.length === 2
          ? predicates[1].matchCase
          : predicates[0].matchCase
        : false;
  ...
  private renderDate(
    ...
    fValue: string | number | Date | boolean,
    isRtl: boolean
  ): void {
    const format: string =
      getCustomDateFormat(options.format, options.type) || options.format;
    const datePicker: DatePicker = new DatePicker(
      extend(
        {
          format: format,
          cssClass: this.parent.cssClass
            ? 'e-popup-flmenu' + ' ' + this.parent.cssClass
            : 'e-popup-flmenu',
  ...
  private renderAutoComplete(
    ...
    const dataSource: Object = isForeignColumn
      ? colObj.dataSource
      : options.dataSource;
    const fields: Object = {
      value: isForeignColumn ? colObj.foreignKeyValue : column,
    };
    const actObj: AutoComplete = new AutoComplete(
      extend(
        {
          dataSource:
            dataSource instanceof DataManager
              ? dataSource
              : new DataManager(dataSource as object),
          fields: fields,
          query: this.getQuery(),
          sortOrder: 'Ascending',
          locale: this.parent.locale,
    ...
    if (dataSource && 'result' in dataSource) {
      const defObj: FilterStateObj = eventPromise(
    ...
    actObj.addEventListener(
      literals.focus,
      this.eventHandlers[inputValue.id][literals.focus]
    );
    actObj.addEventListener(
      events.actionComplete,
      this.eventHandlers[inputValue.id][events.actionComplete]
    );
          ...
          return (
            arr
              .map((mapObject: Object) => {
                return isComplex
                  ? performComplexDataOperation(actObj.fields.value, mapObject)
                  : mapObject[actObj.fields.value];
              })
              .indexOf(
                isComplex
                  ? performComplexDataOperation(actObj.fields.value, obj)
                  : obj[actObj.fields.value]
              ) === index
          );
        }
      );
    ...
    return () => {
      ...
      actObj.ignoreCase =
        options.type === 'string'
          ? !(
              (<EJ2Intance>(
                this.dlgDiv.querySelector('#' + columnvalue + '-xlflmtcase')
              )).ej2_instances[0] as CheckBox
            ).checked
          : true;
      actObj.filterType = !isNullOrUndefined(actObj.filterType)
        ? actObj.filterType
