...
import { createCheckBox } from '@hoyui/buttons';
import { ReturnType } from '../base/type';
import {
  IFilterArgs,
  FilterSearchBeginEventArgs,
  CheckBoxBeforeRenderer,
  IGrid,
} from '../base/interface';
...
import { Dialog, DialogModel } from '@hoyui/popups';
import { Input } from '@hoyui/inputs';
import { createSpinner, hideSpinner, showSpinner } from '@hoyui/popups';
import {
...
export class CheckBoxFilterBase {
  ...
  constructor(parent?: IXLFilter) {
    ...
    if (this.parent.cssClass) {
      if (this.parent.cssClass.indexOf(' ') !== -1) {
        addClass(
          [this.cBoxTrue, this.cBoxFalse],
          this.parent.cssClass.split(' ')
        );
      } else {
  ...
  private wireEvents(): void {
    ...
    const elem: Element =
      this.dialogObj.element.querySelector('.e-searchinput');
    if (elem) {
  ...
  protected foreignKeyFilter(
    ...
    (<Promise<Object>>(
      (<DataManager>this.options.column.dataSource).executeQuery(
        query.where(mPredicate)
      )
    )).then((e: ReturnType) => {
      ...
      (<{ ejpredicate: Predicate[] }>args).ejpredicate =
        fPredicate.predicate.predicates;
      ...
      (<{ filterCollection: PredicateModel[] }>args).filterCollection =
        filterCollection.length
          ? filterCollection
          : fColl.filter(
              (col: PredicateModel) => (col.field = this.options.field)
            );
  ...
  private searchBoxClick(e: MouseEvent): void {
    ...
    if (target.classList.contains('e-searchclear')) {
      this.sInput.value = target.classList.contains('e-chkcancel-icon')
        ? ''
        : this.sInput.value;
  ...
  private updateSearchIcon(): void {
    if (this.sInput.value.length) {
      ...
      if (
        !isNullOrUndefined(document.body.querySelector('.e-chkcancel-icon'))
      ) {
        document.body
          .querySelector('.e-chkcancel-icon')
          .setAttribute('title', this.localeObj.getConstant('Clear'));
      }
    } else {
      classList(this.sIcon, ['e-search-icon'], ['e-chkcancel-icon']);
      if (
        !isNullOrUndefined(
          document.body.querySelector('.e-searchclear.e-search-icon')
        )
      ) {
        document.body
          .querySelector('.e-searchclear.e-search-icon')
          .setAttribute('title', this.localeObj.getConstant('Search'));
      }
    }
  }
  ...
  protected updateModel(options: IFilterArgs): void {
    ...
    this.options.dataManager = options.dataManager
      ? options.dataManager
      : (options.dataSource as DataManager);
    this.updateDataSource();
    this.options.type = options.type;
    this.options.format = options.format || '';
    this.options.ignoreAccent = options.ignoreAccent || false;
    this.options.filteredColumns =
      options.filteredColumns || this.parent.filterSettings.columns;
    ...
    this.infiniteRenderMod =
      ...
      this.parent.filterSettings.enableInfiniteScrolling
        ? true
        : false;
  ...
  protected getAndSetChkElem(options: IFilterArgs): HTMLElement {
    ...
    if (!options.hideSearchbox) {
      ...
      this.searchInputArgs = {
        element: this.sInput as HTMLInputElement,
  ...
  protected showDialog(options: IFilterArgs): void {
    ...
    this.dialogObj = new Dialog({
      visible: false,
      content: this.sBox as HTMLElement,
      close: this.closeDialog.bind(this),
      enableRtl: this.parent.enableRtl,
      width:
        !isNullOrUndefined(parentsUntil(options.target, 'e-bigger')) ||
        this.parent.element.classList.contains('e-device')
          ? 260
          : 255,
      ...
      buttons: [
        {
          ...
          buttonModel: {
            content: this.getLocalizedLabel(
              this.isExcel ? 'OKButton' : 'FilterButton'
            ),
            cssClass: this.parent.cssClass
              ? 'e-primary' + ' ' + this.parent.cssClass
              : 'e-primary',
            isPrimary: true,
          },
        },
        {
          ...
          buttonModel: {
            cssClass: this.parent.cssClass
              ? 'e-flat' + ' ' + this.parent.cssClass
              : 'e-flat',
            content: this.getLocalizedLabel(
              this.isExcel ? 'CancelButton' : 'ClearButton'
            ),
          },
        },
      ],
    ...
    const dialogLabel: string =
      ...
      (<{ type: string }>this.parent.filterSettings).type === 'CheckBox'
        ? this.getLocalizedLabel('CheckBoxFilterDialogARIA')
        : this.getLocalizedLabel('ExcelFilterDialogARIA');
    this.dlg.setAttribute('aria-label', dialogLabel);
    if (options.isResponsiveFilter) {
      const responsiveCnt: HTMLElement = document.querySelector(
        '.e-responsive-dialog > .e-dlg-content > .e-mainfilterdiv'
      );
    ...
    this.dialogObj.element.style.maxHeight = options.isResponsiveFilter
      ? 'none'
      : this.options.height + 'px';
    this.dialogObj.show();
    if (
      (this.parent as IGrid) &&
      (this.parent as IGrid).filterSettings &&
      (this.parent as IGrid).filterSettings.type === 'CheckBox' &&
      ((this.parent as IGrid).getContent().firstElementChild as HTMLElement)
        .offsetHeight < this.dialogObj.element.offsetHeight
    ) {
      resetDialogAppend(this.parent as IGrid, this.dialogObj);
    }
    const content: HTMLElement =
      this.dialogObj.element.querySelector('.e-dlg-content');
    ...
    if (!(this.parent as IGrid).enableAdaptiveUI) {
      ...
      if (
        (this.parent as IGrid).enableAdaptiveUI &&
        this.parent.filterSettings &&
        this.parent.filterSettings.loadingIndicator === 'Shimmer'
      ) {
        this.getAllData();
  ...
  private renderResponsiveFilter(options: IFilterArgs): void {
    if (options.isResponsiveFilter) {
      ...
      this.dialogObj.target = document.querySelector(
        '.e-resfilter > .e-dlg-content > .e-mainfilterdiv'
      ) as HTMLElement;
  ...
  public closeDialog(): void {
    if (
      this.infiniteRenderMod &&
      this.infinitePermenantLocalData.length &&
      !this.options.isRemote
    ) {
      (this.options.dataSource as DataManager).dataSource.json =
        this.infinitePermenantLocalData;
    }
    if (this.dialogObj && !this.dialogObj.isDestroyed) {
      this.isBlanks = false;
      const filterTemplateCol: Column[] = (
        this.options.columns as Column[]
      ).filter((col: Column) => col.getFilterItemTemplate());
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const registeredTemplate: any = (this.parent as any).registeredTemplate;
      if (
        filterTemplateCol.length &&
        !isNullOrUndefined(registeredTemplate) &&
        registeredTemplate.filterItemTemplate
      ) {
        this.parent.destroyTemplate(['filterItemTemplate']);
      }
      if (
        (this.parent.isReact || this.parent.isVue) &&
        this.parent.destroyTemplate !== undefined
      ) {
      ...
      if (
        this.searchInputArgs &&
        this.searchInputArgs.element &&
        this.searchInputArgs.element.parentElement
      ) {
      ...
      if (
        this.parent.isReact &&
        this.options.column.filter &&
        typeof this.options.column.filter.itemTemplate !== 'string' &&
        (this.options.column.filter.type === 'CheckBox' ||
          this.options.column.filter.type === 'Excel')
      ) {
  ...
  public clearFilter(col?: Column): void {
    ...
    const args: {
      instance: CheckBoxFilterBase;
      handler: Function;
      cancel: boolean;
    } = { instance: this, handler: this.clearFilter, cancel: false };
  ...
  private btnClick(e: MouseEvent): void {
    if (this.filterState) {
      if (
        ((<Element>e.target).tagName.toLowerCase() === 'input' &&
          (<Element>e.target).classList.contains('e-searchinput')) ||
        ...
        if (
          ...
          this.getLocalizedLabel('ClearButton').toLowerCase() === text
        ) {
          this.clearFilter();
        }
      }
      ...
      (e.target as HTMLElement).firstChild.textContent.toLowerCase() ===
        this.getLocalizedLabel('CancelButton').toLowerCase()
    ) {
      this.closeDialog();
  ...
  public fltrBtnHandler(): void {
    ...
    let checked: Element[] = [].slice.call(
      this.cBox.querySelectorAll(
        '.e-check:not(.e-selectall):not(.e-add-current)'
      )
    );
    const check: Element[] = checked;
    let optr: string = 'equal';
    const ddlValue: EJ2Intance = this.dialogObj.element.querySelector(
      '.e-dropdownlist'
    ) as EJ2Intance;
    if (ddlValue) {
    ...
    if (!this.options.hideSearchbox) {
      searchInput = this.searchBox.querySelector(
        '.e-searchinput'
      ) as HTMLInputElement;
    }
    ...
    if (isNotEqual && searchInput && searchInput.value === '') {
      ...
      checked = [].slice.call(
        this.cBox.querySelectorAll('.e-uncheck:not(.e-selectall)')
      );
    ...
    if (
      ...
      if (!this.infiniteRenderMod) {
        ...
        this.infiniteRenderMod &&
        (!this.infiniteSearchPred ||
          (this.infiniteSearchPred && !this.infiniteSearchPred.isComplex))
      ) {
      ...
      const addCurrSelection: HTMLElement = this.infiniteRenderMod
        ? this.sBox.querySelector('.e-add-current')
        : this.cBox.querySelector('.e-add-current');
      if (addCurrSelection && addCurrSelection.classList.contains('e-check')) {
        const existingPredicate: PredicateModel[] =
          this.existingPredicate[this.options.field];
        if (existingPredicate) {
          for (let j: number = 0; j < existingPredicate.length; j++) {
            if (
              !coll.some(function (data: PredicateModel): boolean {
                return (
                  data.value ===
                  existingPredicate[parseInt(j.toString(), 10)].value
                );
              })
            ) {
      ...
      if (!this.infiniteRenderMod) {
        ...
        this.sBox.querySelector('.e-selectall').classList.contains('e-check') &&
        !coll.length
      ) {
        const isClearFilter: boolean = this.options.filteredColumns.some(
          (value: PredicateModel) => {
  ...
  private complexQueryPredicate(
    ...
    for (let i: number = 0; i < checkBoxChecked.length; i++) {
      value =
        this.values[
          parentsUntil(
            checkBoxChecked[parseInt(i.toString(), 10)],
            'e-ftrchk'
          ).getAttribute('uid')
        ];
  ...
  private infiniteFltrBtnHandler(
    ...
    if (this.infiniteManualSelectMaintainPred.length) {
      for (
        let i: number = 0;
        i < this.infiniteManualSelectMaintainPred.length;
        i++
      ) {
        const pred: PredicateModel =
          this.infiniteManualSelectMaintainPred[i as number];
        value = pred.value + '';
        if (value === '' || isNullOrUndefined(value)) {
          const dummyDefaults: {
            predicate?: string;
          ...
          coll.push(
            ...CheckBoxFilterBase.generateNullValuePredicates(dummyDefaults)
          );
        } else {
          coll.push(this.infiniteManualSelectMaintainPred[i as number]);
        }
        this.notifyFilterPrevEvent(
          this.infiniteManualSelectMaintainPred[i as number]
        );
      }
    }
    if (
      ...
      this.sInput.value === '' &&
      !(!this.options.parentCurrentViewDataCount && coll.length)
    ) {
      for (
        let i: number = 0;
        i < this.infiniteUnloadParentExistPred.length;
        i++
      ) {
        coll.unshift(this.infiniteUnloadParentExistPred[i as number]);
        this.notifyFilterPrevEvent(
          this.existingPredicate[this.options.field][i as number]
        );
      }
    }
    if (
      this.sInput.value !== '' &&
      (!this.localInfiniteSelectAllClicked || this.infiniteLocalSelectAll)
    ) {
  ...
  private notifyFilterPrevEvent(predicate: PredicateModel): void {
    const args: {
      ...
      arg6: predicate.value as string,
      cancel: false,
    };
    this.parent.notify(events.fltrPrevent, args);
    if (args.cancel) {
  ...
  /** @hidden */
  public static generateNullValuePredicates(defaults: {
    predicate?: string;
    field?: string;
    type?: string;
    uid?: string;
  ...
  public initiateFilter(fColl: PredicateModel[]): void {
    ...
    if (!isNullOrUndefined(firstVal)) {
      predicate = firstVal.ejpredicate
        ? firstVal.ejpredicate
        : new Predicate(
            firstVal.field,
            firstVal.operator,
            firstVal.value,
            !firstVal.matchCase,
            firstVal.ignoreAccent
          );
      for (let j: number = 1; j < fColl.length; j++) {
        predicate =
          fColl[parseInt(j.toString(), 10)].ejpredicate !== undefined
            ? predicate[fColl[parseInt(j.toString(), 10)].predicate](
                fColl[parseInt(j.toString(), 10)].ejpredicate
              )
            : predicate[fColl[parseInt(j.toString(), 10)].predicate](
                fColl[parseInt(j.toString(), 10)].field,
                fColl[parseInt(j.toString(), 10)].operator,
                fColl[parseInt(j.toString(), 10)].value,
                !fColl[parseInt(j.toString(), 10)].matchCase,
  ...
  private refreshCheckboxes(): void {
    ...
    const query: Query = this.isForeignColumn(column)
      ? this.foreignKeyQuery.clone()
      : this.options.query.clone();
    const foreignQuery: Query = this.options.query.clone();
    const pred: QueryOptions = query.queries.filter((e: QueryOptions) => {
      return e && e.fn === 'onWhere';
    })[0];
    query.queries = [];
    foreignQuery.queries = [];
    let parsed: string | number | Date | boolean =
      this.options.type !== 'string' && parseFloat(val) ? parseFloat(val) : val;
    ...
    const field: string = this.isForeignColumn(column)
    ...
    const moduleName: Function = (<{ getModuleName?: Function }>(
      this.options.dataManager.adaptor
    )).getModuleName;
    if (this.options.type === 'boolean') {
      if (
        parsed !== undefined &&
        this.getLocalizedLabel('FilterTrue')
          .toLowerCase()
          .indexOf((parsed as string).toLowerCase()) !== -1
      ) {
        parsed = 'true';
      } else if (
        parsed !== undefined &&
        this.getLocalizedLabel('FilterFalse')
          .toLowerCase()
          .indexOf((parsed as string).toLowerCase()) !== -1
      ) {
        parsed = 'false';
      ...
      if (
        ...
        parsed =
          moduleName() === 'ODataAdaptor' || 'ODataV4Adaptor' ? true : 'true';
      } else if (
        parsed !== undefined &&
        this.getLocalizedLabel('FilterFalse')
          .toLowerCase()
        ...
        // eslint-disable-next-line no-constant-condition
        parsed =
          moduleName() === 'ODataAdaptor' || 'ODataV4Adaptor' ? false : 'false';
      }
      operator = 'equal';
    }
    if (
      ...
      if (this.options.type === 'dateonly') {
        parsed =
          parsed.getFullYear() +
          '-' +
          padZero(parsed.getMonth() + 1) +
          '-' +
          padZero(parsed.getDate());
      }
    }
    ...
    this.parent.trigger(
      events.actionBegin,
      args,
      (filterargs: FilterSearchBeginEventArgs) => {
        // eslint-disable-next-line no-self-assign
        filterargs.operator = filterargs.operator;
        predicte = new Predicate(
          field,
          filterargs.operator,
          args.value,
          filterargs.matchCase,
          filterargs.ignoreAccent
        );
        if (
          this.options.type === 'date' ||
          this.options.type === 'datetime' ||
        ...
        if (
          ...
          this.getLocalizedLabel('Blanks')
            .toLowerCase()
            .indexOf((val as string).toLowerCase()) >= 0
        ) {
          coll = coll.concat(
            CheckBoxFilterBase.generateNullValuePredicates(defaults)
          );
          const emptyValPredicte: Predicate =
            CheckBoxFilterBase.generatePredicate(coll);
          emptyValPredicte.predicates.push(predicte);
          predicte = emptyValPredicte;
          query.where(emptyValPredicte);
        } else if (val.length) {
          predicte = !isNullOrUndefined(pred)
            ? predicte.and(pred.e as Predicate)
            : predicte;
        ...
        filterargs.filterChoiceCount = !isNullOrUndefined(
          filterargs.filterChoiceCount
        )
          ? filterargs.filterChoiceCount
          : 1000;
        if (
          this.infiniteRenderMod &&
          this.parent.filterSettings.itemsCount !== filterargs.filterChoiceCount
        ) {
        ...
        if (this.isForeignColumn(column) && val.length) {
          const colData: DataManager =
            'result' in column.dataSource
              ? new DataManager((column.dataSource as DataResult).result)
          ...
          colData.executeQuery(query).then((e: any) => {
            ...
            if (this.infiniteRenderMod) {
              this.infiniteInitialLoad = isNullOrUndefined(this.fullData)
                ? true
                : false;
  ...
  private getPredicateFromCols(
    columns: Object[],
    isExecuteLocal?: boolean
  ): Predicate {
    const predicates: Predicate = CheckBoxFilterBase.getPredicate(
      columns,
      isExecuteLocal
    );
    ...
    const foreignColumn: Column[] = isGrid
      ? this.parent.getForeignKeyColumns()
      : [];
  ...
  private getAllData(): void {
    ...
    if (
      !(
        !isNullOrUndefined((this.parent as IGrid).getDataModule) &&
        moduleName &&
        moduleName() === 'ODataV4Adaptor'
      )
    ) {
    ...
    this.parent.trigger(
      events.actionBegin,
      args,
      (args: FilterSearchBeginEventArgs) => {
        args.filterChoiceCount = !isNullOrUndefined(args.filterChoiceCount)
  ...
  private addDistinct(query: Query): Query {
    let filteredColumn: Object[] = DataUtil.distinct(
      this.options.filteredColumns,
      'field'
    );
    ...
    if (
      ...
      this.options.dataSource =
        this.options.dataSource instanceof DataManager
          ? this.options.dataSource
          : new DataManager(this.options.dataSource as JSON[]);
      this.infinitePermenantLocalData = [
        ...this.options.dataSource.dataSource.json,
      ];
      this.options.dataSource.dataSource.json = DataUtil.distinct(
        this.options.parentFilteredLocalRecords.concat(
          ...this.infinitePermenantLocalData
        ),
      ...
      if (this.isForeignColumn(this.options.column as Column)) {
        this.options.column.dataSource =
          this.options.column.dataSource instanceof DataManager
            ? this.options.column.dataSource
            : new DataManager(this.options.column.dataSource as JSON[]);
        this.options.dataSource.dataSource.json =
          this.options.dataSource.dataSource.json.map(
            (item: Object, i: number) =>
              Object.assign(
                {},
                item,
                (this.options.column.dataSource as DataManager).dataSource.json[
                  i as number
                ]
              )
          );
      ...
      if (
        ...
        moduleName() &&
        (moduleName() === 'ODataV4Adaptor' ||
          moduleName() === 'WebApiAdaptor' ||
          moduleName() === 'CustomDataAdaptor' ||
          moduleName() === 'GraphQLAdaptor' ||
          moduleName() === 'ODataAdaptor')
      ) {
        query.distinct(filteredColumn as string[]);
      }
    }
  ...
  private getListHeight(element?: HTMLElement): number {
    const listDiv: HTMLDivElement = <HTMLDivElement>(
      createElement('div', {
        className: 'e-ftrchk',
  ...
  public showMask(): void {
    ...
    const maskList: HTMLElement = this.parent.createElement('div', {
    ...
    if (wrapperElem) {
      ...
      const height: number = wrapperElem.children.length
      ...
      if ((this.parent as IGrid).enableAdaptiveUI && this.infiniteRenderMod) {
        ...
        (
          this.dlg.querySelector('.e-dlg-content') as HTMLElement
        ).style.overflow = 'hidden';
      }
      const backgroundColor: string =
        this.isExcel &&
        !wrapperElem.children.length &&
        !this.dlg.classList.contains('e-excelfilter')
          ? ''
          : getComputedStyle(this.dlg.querySelector('.e-dlg-content'))
              .backgroundColor;
      maskList.style.cssText =
        'width: ' +
        computedStyle.width +
        '; min-height: ' +
      ...
      maskRowCount =
        wrapperElem.children.length > maskRowCount
          ? wrapperElem.children.length
          : maskRowCount;
      maskItemHeight = liHeight + 'px';
    }
    const maskTemplate: string =
      '<div class="e-ftrchk e-mask-ftrchk" style="width: 100%; height:' +
      maskItemHeight +
      ';">' +
    ...
    if (!wrapperElem.children.length) {
      ...
      const maskSpan: Element[] = [].slice.call(
        this.spinner.parentElement.querySelectorAll(
  ...
  private removeMask(): void {
    const maskLists: NodeListOf<Element> =
      this.dialogObj.element.querySelectorAll('.e-mask-ftrchk');
    if (maskLists.length) {
      for (let i: number = 0; i < maskLists.length; i++) {
        remove(maskLists[i as number]);
      }
    }
    const maskParent: HTMLElement = this.dialogObj.element.querySelector(
      '.e-checkboxlist.e-masklist'
    );
    if (maskParent) {
      remove(
        this.dialogObj.element.querySelector('.e-checkboxlist.e-masklist')
      );
    }
  }
  ...
  private infiniteScrollHandler(): void {
    ...
    if (
      target.scrollTop >= target.scrollHeight - target.offsetHeight &&
      !this.infiniteQueryExecutionPending &&
      this.infiniteLoadedElem.length <=
      ...
      this.infiniteRemoveElements(
        [].slice
          .call(this.cBox.children)
          .splice(0, this.parent.filterSettings.itemsCount)
      );
      ...
      appendChildren(
        this.cBox,
        this.infiniteLoadedElem.slice(
          this.infiniteSkipCnt,
          this.parent.filterSettings.itemsCount + this.infiniteSkipCnt
        )
      );
      this.prevInfiniteScrollDirection = 'down';
    } else if (
      ...
      this.infiniteRemoveElements(
        [].slice
          .call(this.cBox.children)
          .splice(
            this.parent.filterSettings.itemsCount * 2,
            this.parent.filterSettings.itemsCount
          )
      );
      this.infiniteSkipCnt -=
  ...
  private infiniteAppendElements(appendElem: HTMLElement[]): void {
    for (let i: number = 0; i < appendElem.length; i++) {
      this.cBox.insertBefore(
        appendElem[i as number],
        this.cBox.children[i as number]
      );
    }
  }
  ...
  private makeInfiniteScrollRequest(query?: Query): void {
    ...
    if (
      ...
      this.infiniteQuery.queries.length
    ) {
      for (let i: number = 0; i < this.infiniteQuery.queries.length; i++) {
        if (
          this.infiniteQuery.queries[i as number][`${fName}`] === 'onTake' ||
  ...
  private processDataOperation(query: Query, isInitial?: boolean): void {
    ...
    if (this.isForeignColumn(this.options.column as Column) && isInitial) {
      const colData: DataManager =
        'result' in this.options.column.dataSource
          ? new DataManager(
              (this.options.column.dataSource as DataResult).result
            )
    ...
    if (this.infiniteRenderMod) {
      ...
      if (this.infiniteInitialLoad) {
        ...
        EventHandler.add(
          this.cBox,
          'mouseup',
          this.infiniteScrollMouseKeyUpHandler,
          this
        );
        EventHandler.add(
          this.cBox,
          'mousedown',
          this.infiniteScrollMouseKeyDownHandler,
          this
        );
    ...
    if (
      ...
      const field: string = this.isForeignColumn(this.options.column as Column)
        ? this.options.foreignKeyValue
        : this.options.column.field;
      this.options.dataSource
        .executeQuery(new Query().sortBy(field, DataUtil.fnAscending))
        .then((e: ReturnOption) => {
          (this.options.dataSource as DataManager).dataSource.json =
            e.result as JSON[];
  ...
  private executeQueryOperations(
    query: Query,
    allPromise: Promise<Object>[],
    runArray: Function[]
  ): void {
    allPromise.push(
      (this.options.dataSource as DataManager).executeQuery(query)
    );
    runArray.push(this.dataSuccess.bind(this));
    let i: number = 0;
    Promise.all(allPromise)
      .then((e: ReturnType[]) => {
        this.infiniteQueryExecutionPending = this.infiniteRenderMod
          ? false
          : this.infiniteQueryExecutionPending;
        for (let j: number = 0; j < e.length; j++) {
          this.infiniteDataCount =
            this.infiniteRenderMod && !this.infiniteDataCount
              ? e[j as number].count
              : this.infiniteDataCount;
  ...
  private dataSuccess(e: Object[]): void {
    if (
      !this.infiniteInitialLoad &&
      this.infiniteDataCount &&
      ((this.infiniteSkipCnt >= this.infiniteDataCount &&
        !this.infiniteSearchValChange) ||
        e.length === 0)
    ) {
      return;
    ...
    this.fullData = e;
    const args1: CheckBoxBeforeRenderer = {
    ...
    if (args1.executeQuery) {
      ...
      const result: Object[] = new DataManager(
        args1.dataSource as JSON[]
      ).executeLocal(query);
      const col: Column = this.options.column as Column;
      this.filteredData =
        (
          CheckBoxFilterBase.getDistinct(
            result,
            this.options.field,
            col,
            this.foreignKeyData,
    ...
    const data: object[] = args1.executeQuery
      ? this.filteredData
      : args1.dataSource;
    this.processDataSource(null, true, data, args1);
    if (
      this.sInput &&
      ((this.infiniteRenderMod && this.infiniteInitialLoad) ||
        !this.infiniteRenderMod)
    ) {
      this.sInput.focus();
    }
  ...
  private queryGenerate(query: Query): void {
    if (this.parent.searchSettings && this.parent.searchSettings.key.length) {
      ...
      if (
        ...
        const fields: string[] = searchSettings.fields.length
    ...
    if (this.options.filteredColumns.length) {
      ...
      for (let i: number = 0; i < this.options.filteredColumns.length; i++) {
        const filterColumn: { uid: string; field: string } = this.options
          .filteredColumns[parseInt(i.toString(), 10)] as {
          uid: string;
          field: string;
        };
        if (this.options.uid) {
          filterColumn.uid =
            filterColumn.uid ||
            this.parent.getColumnByField(filterColumn.field).uid;
      ...
      const predicate: Predicate = this.getPredicateFromCols(
        cols,
        this.isExecuteLocal
      );
      if (predicate) {
  ...
  private processDataSource(
    ...
    this.isCheckboxFilterTemplate = (<{ isCheckboxFilterTemplate?: boolean }>(
      args1
    )).isCheckboxFilterTemplate;
    if (!this.isCheckboxFilterTemplate) {
  ...
  private updateResult(): void {
    ...
    const predicate: Predicate =
      this.infiniteRenderMod && this.existingPredicate[this.options.field]
        ? this.getPredicateFromCols(
            this.existingPredicate[this.options.field],
            this.isExecuteLocal
          )
        : this.getPredicateFromCols(
    ...
    if (elem && !this.isCheckboxFilterTemplate) {
      ...
      if (selectAll) {
        ...
        if (
          ...
          const value: string =
            this.values[parentsUntil(input, 'e-ftrchk').getAttribute('uid')];
          ...
          if (
            ...
            const predicate: Predicate = this.getPredicateFromCols(
              this.options.filteredColumns.concat(
                ...this.infiniteManualSelectMaintainPred
              ),
              true
            );
            ...
            const result: Object[] = new DataManager(
              this.infinitePermenantLocalData as JSON[]
            ).executeLocal(query);
          ...
          if (
            this.infiniteUnloadParentExistPred.length &&
            (this.options.parentTotalDataCount === this.infiniteDataCount ||
              !this.options.parentTotalDataCount)
          ) {
            this.infiniteUnloadParentExistPred = [];
          }
  ...
  private updateInfiniteManualSelectPred(
    ...
    for (
      ...
      const predmdl: PredicateModel =
        this.infiniteManualSelectMaintainPred[i as number];
      if (
        predmdl.value + '' === value + '' &&
        (predmdl.operator === 'equal' || predmdl.operator === 'notequal')
      ) {
    ...
    if (
      (defaults.predicate === 'or' &&
        (!this.localInfiniteSelectAllClicked ||
          !this.infiniteLocalSelectAll)) ||
      (defaults.predicate === 'and' &&
        (!this.localInfiniteSelectAllClicked || this.infiniteLocalSelectAll))
    ) {
      this.infiniteManualSelectMaintainPred.push(
        extend({}, { value: value }, defaults) as {
          field: string;
          predicate: string;
          operator: string;
          matchCase: boolean;
  ...
  private focusNextOrPrev(
    ...
    if (nextElement) {
      ...
      const target: Element = nextElement.classList.contains('e-chk-hidden')
        ? parentsUntil(nextElement, 'e-ftrchk')
        : nextElement;
  ...
  private updateAllCBoxes(checked: boolean): void {
    ...
    const cBoxes: Element[] = this.infiniteRenderMod
      ? this.infiniteLoadedElem.map((arr: HTMLElement) =>
          arr.querySelector('.e-frame')
        )
      : [].slice.call(
          this.cBox.querySelectorAll('.e-frame:not(.e-add-current)')
        );
  ...
  private createCheckbox(
    ...
    const elem: Element = checked
      ? (this.cBoxTrue.cloneNode(true) as Element)
    ...
    const dummyData: Object = extendObjWithFn({}, data, {
      column: this.options.column,
      parent: this.parent,
    });
    const innerText: string = this.options.disableHtmlEncode
      ? 'textContent'
      : 'innerHTML';
    label[`${innerText}`] =
      !isNullOrUndefined(value) && value.toString().length
        ? this.parent.enableHtmlSanitizer
          ? SanitizeHtmlHelper.sanitize(value)
          : value
        : this.getLocalizedLabel('Blanks');
    ...
    if (
      ...
      data[this.options.column.field] !== this.getLocalizedLabel('SelectAll') &&
      data[this.options.column.field] !==
        this.getLocalizedLabel('AddCurrentSelection')
    ) {
      label.innerHTML = '';
      const isReactCompiler: boolean =
        this.parent.isReact &&
        this.options.column.filter &&
        typeof this.options.column.filter.itemTemplate !== 'string';
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const isReactChild: boolean =
        (this.parent as any).parentDetails &&
        (this.parent as any).parentDetails.parentInstObj &&
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  private updateIndeterminatenBtn(): void {
    let cnt: number = this.infiniteRenderMod
      ? this.infiniteLoadedElem.length
      : this.cBox.children.length - 1;
    let className: string[] = [];
    let disabled: boolean = false;
    const elem: Element = this.infiniteRenderMod
      ? this.sBox.querySelector('.e-selectall')
      : this.cBox.querySelector('.e-selectall');
    let selected: number = this.infiniteRenderMod
      ? this.infiniteLoadedElem.filter((arr: HTMLElement) =>
          arr.querySelector('.e-check')
        ).length
      : this.cBox.querySelectorAll(
          '.e-check:not(.e-selectall):not(.e-add-current)'
        ).length;
    ...
    if (
      ...
      !this.localInfiniteSelectionInteracted &&
      (!this.localInfiniteSelectAllClicked ||
        (!this.infiniteLocalSelectAll && !selected)) &&
      (cnt !== selected || cnt === selected)
    ) {
      selected = 0;
    } else if (
      this.infiniteRenderMod &&
      this.infiniteLoadedElem.length < this.infiniteDataCount &&
      this.infiniteUnloadParentExistPred.length &&
  ...
  private createFilterItems(
    ...
    if (!this.infiniteRenderMod) {
      this.itemsCnt =
        nullCounter !== -1 ? data.length - nullCounter : data.length;
    }
    if (
      this.infiniteRenderMod &&
      this.parent.filterSettings &&
    ...
    if (data.length && !this.renderEmpty) {
      ...
      const checkBox: Element = this.createCheckbox(selectAllValue, false, {
      ...
      if (this.infiniteInitialLoad || !this.infiniteRenderMod) {
        ...
        if (this.infiniteRenderMod) {
          ...
          if ((this.parent as IGrid).enableAdaptiveUI) {
            this.spinner.style.height =
              this.spinner.offsetHeight - this.getListHeight(this.cBox) + 'px';
          }
        ...
        (
          this.sBox.querySelector('.e-ftrchk .e-selectall')
            .previousSibling as HTMLInputElement
        ).disabled = false;
        this.sBox
          .querySelector('.e-ftrchk .e-selectall')
          .parentElement.classList.remove('e-checkbox-disabled');
      }
      ...
      const isColFiltered: number = new DataManager(
        this.options.filteredColumns as JSON[]
      ).executeLocal(new Query().where(predicate)).length;
      if (this.sInput.value) {
        const predicateCheckBox: Element = this.createCheckbox(
          this.getLocalizedLabel('AddCurrentSelection'),
          false,
          {
        ...
        if (
          (this.infiniteRenderMod &&
            !isNullOrUndefined(this.sBox.children[2]) &&
            (this.sBox.children[2] as HTMLElement).innerText !==
              'Add current selection to filter') ||
          !this.infiniteRenderMod
        ) {
          const predicateElement: Element = createCboxWithWrap(
            getUid('cbox'),
            predicateCheckBox,
            'e-ftrchk'
          );
          ...
          if (this.infiniteRenderMod) {
            ...
            const checkBoxListElem: HTMLElement =
              this.spinner.querySelector('.e-checkboxlist');
            const reduceHeight: number = Math.ceil(
              predicateElement.getBoundingClientRect().height
            );
            checkBoxListElem.style.height =
              parseInt(
                getComputedStyle(checkBoxListElem).height.split('px')[0],
                10
              ) -
              reduceHeight +
              'px';
          ...
          (
            this.sBox.querySelector('.e-ftrchk .e-add-current')
              .previousSibling as HTMLInputElement
          ).disabled = false;
          this.sBox
            .querySelector('.e-ftrchk .e-add-current')
            .parentElement.classList.remove('e-checkbox-disabled');
        }
      } else if (
        this.infiniteRenderMod &&
        ...
        const checkBoxListElem: HTMLElement =
          this.spinner.querySelector('.e-checkboxlist');
        const increaseHeight: number = Math.ceil(
          this.sBox.children[2].getBoundingClientRect().height
        );
        checkBoxListElem.style.height =
          parseInt(
            getComputedStyle(checkBoxListElem).height.split('px')[0],
      ...
      for (let i: number = 0; i < data.length; i++) {
        ...
        if (this.options.formatFn) {
          value = this.valueFormatter.toView(
            value as number,
        ...
        if (this.infiniteRenderMod) {
          this.updateInfiniteUnLoadedCheckboxExistPred(
            value,
            this.infiniteUnloadParentExistPred
          );
        }
        const checkbox: Element = this.localInfiniteSelectAllClicked
          ? this.createCheckbox(
              value as string,
              this.infiniteLocalSelectAll,
              getValue('dataObj', data[i as number])
            )
          : this.createCheckbox(
              value as string,
              this.getCheckedState(isColFiltered, this.values[`${uid}`]),
              getValue('dataObj', data[i as number])
            );
        cBoxes.appendChild(createCboxWithWrap(uid, checkbox, 'e-ftrchk'));
        if (this.infiniteRenderMod) {
          (cBoxes.lastChild as HTMLElement).style.height =
            this.getListHeight(this.cBox) + 'px';
        }
      }
      ...
      if (this.infiniteRenderMod) {
        ...
        this.itemsCnt =
          nullCounter !== -1
            ? this.infiniteLoadedElem.length - nullCounter
            : this.infiniteLoadedElem.length;
      }
      if (
        this.infiniteUnloadParentExistPred.length &&
      ...
      if (this.infiniteRenderMod && !this.infiniteScrollAppendDiff) {
        this.infiniteScrollAppendDiff = Math.round(
          scrollTop - this.cBox.scrollTop
        );
      }
      this.updateIndeterminatenBtn();
      if (!this.infiniteRenderMod) {
      ...
      this.cBox.innerHTML = '';
      if (this.infiniteRenderMod) {
        const selectAll: HTMLElement = this.sBox.querySelector(
          '.e-ftrchk .e-selectall'
        );
        if (selectAll) {
          const selectAllParent: HTMLElement =
            selectAll.parentElement.parentElement;
        ...
        const addCurrSelection: HTMLElement = this.sBox.querySelector(
          '.e-ftrchk .e-add-current'
        );
        if (addCurrSelection) {
          const addCurrSelectionParent: HTMLElement =
            addCurrSelection.parentElement.parentElement;
          if (addCurrSelection.classList.contains('e-check')) {
            toogleCheckbox(addCurrSelectionParent);
          }
          (addCurrSelection.previousSibling as HTMLInputElement).disabled =
            true;
          addCurrSelection.parentElement.classList.add('e-checkbox-disabled');
        }
      }
      this.cBox.appendChild(
        this.parent.createElement('div', { className: 'e-checkfltrnmdiv' })
      );
  ...
  private updateInfiniteUnLoadedCheckboxExistPred(
    ...
    for (let j: number = 0; j < updatePredArr.length; j++) {
      ...
      if (
        value === predValue &&
        (pred.operator === 'equal' || pred.operator === 'notequal')
      ) {
  ...
  private getCheckedState(
    isColFiltered: number | boolean,
    value: string
  ): boolean {
    if (!this.isFiltered || !isColFiltered) {
      return true;
    } else {
      const checkState: boolean = this.sInput.value
        ? true
        : this.result[`${value}`];
      if (this.infiniteRenderMod) {
        return checkState;
  ...
  public static getDistinct(
    json: Object[],
    field: string,
    column?: Column,
    foreignKeyData?: Object[],
    checkboxFilter?: CheckBoxFilterBase
  ): Object {
    ...
    while (len--) {
      ...
      const currentFilterValue: string =
        typeof value === 'string' &&
        !isNullOrUndefined(checkboxFilter) &&
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        !isNullOrUndefined(checkboxFilter.parent as any) &&
        !isNullOrUndefined((checkboxFilter.parent as any).filterSettings) &&
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        !isNullOrUndefined(
          (checkboxFilter.parent as any).filterSettings.enableCaseSensitivity
        )
          ? value.toLowerCase()
          : value;
      if (!(currentFilterValue in lookup)) {
        ...
        if (isForeignKey) {
          const foreignDataObj: Object = getForeignData(
            column,
            {},
            value,
            foreignKeyData
          ...
          setValue(
            events.foreignKeyData,
            foreignDataObj,
            json[parseInt(len.toString(), 10)]
          );
    ...
    return DataUtil.group(
      DataUtil.sort(result, field, DataUtil.fnAscending),
      'ejValue'
    );
  }

  public static getPredicate(
    columns: PredicateModel[],
    isExecuteLocal?: boolean
  ): Predicate {
    const cols: PredicateModel[] =
      DataUtil.distinct(columns, 'field', true) || [];
    ...
    for (let i: number = 0; i < cols.length; i++) {
      ...
      if (collection.length !== 0) {
        pred[cols[parseInt(i.toString(), 10)].field] =
          CheckBoxFilterBase.generatePredicate(collection, isExecuteLocal);
      }
    }
    return pred;
  }
  ...
  private static generatePredicate(
    ...
    first.ignoreAccent = !isNullOrUndefined(first.ignoreAccent)
      ? first.ignoreAccent
      : false;
    if (
      first.type === 'date' ||
      ...
      predicate = first.ejpredicate
        ? (first.ejpredicate as Predicate)
        : (new Predicate(
            first.field,
            first.operator,
            first.value,
            !CheckBoxFilterBase.getCaseValue(first),
            first.ignoreAccent
          ) as Predicate);
    }
    for (let p: number = 1; p < len; p++) {
      cols[parseInt(p.toString(), 10)] = CheckBoxFilterBase.updateDateFilter(
        cols[parseInt(p.toString(), 10)]
      );
      if (
        len > 2 &&
        p > 1 &&
        ((cols[p as number].predicate === 'or' &&
          cols[(p as number) - 1].predicate === 'or') ||
          (cols[p as number].predicate === 'and' &&
            cols[(p as number) - 1].predicate === 'and'))
      ) {
        if (
          cols[p as number].type === 'date' ||
          cols[p as number].type === 'datetime' ||
          cols[p as number].type === 'dateonly'
        ) {
          predicate.predicates.push(
            getDatePredicate(
              cols[parseInt(p.toString(), 10)],
              cols[p as number].type,
              isExecuteLocal
            )
          );
        } else {
          predicate.predicates.push(
            new Predicate(
              cols[p as number].field,
              cols[parseInt(p.toString(), 10)].operator,
              cols[parseInt(p.toString(), 10)].value,
              !CheckBoxFilterBase.getCaseValue(
                cols[parseInt(p.toString(), 10)]
              ),
        ...
        if (
          ...
          if (
            cols[parseInt(p.toString(), 10)].predicate === 'and' &&
            cols[parseInt(p.toString(), 10)].operator === 'equal'
          ) {
            predicate = (predicate[`${operate}`] as Function)(
              getDatePredicate(
                cols[parseInt(p.toString(), 10)],
                cols[parseInt(p.toString(), 10)].type,
                isExecuteLocal
              ),
              cols[parseInt(p.toString(), 10)].type,
              cols[parseInt(p.toString(), 10)].ignoreAccent
            ...
            predicate = (
              predicate[
                (cols[parseInt(p.toString(), 10)] as Predicate)
                  .predicate as string
              ] as Function
            )(
              getDatePredicate(
          ...
          predicate = cols[parseInt(p.toString(), 10)].ejpredicate
            ? (
              ...
              )(cols[parseInt(p.toString(), 10)].ejpredicate)
            : (
                predicate[
                  cols[parseInt(p.toString(), 10)].predicate as string
                ] as Function
              )(
                cols[parseInt(p.toString(), 10)].field,
                cols[parseInt(p.toString(), 10)].operator,
                cols[parseInt(p.toString(), 10)].value,
                !CheckBoxFilterBase.getCaseValue(
                  cols[parseInt(p.toString(), 10)]
  ...
  private static getCaseValue(filter: PredicateModel): boolean {
    if (isNullOrUndefined(filter.matchCase)) {
      if (
        filter.type === 'string' ||
        (isNullOrUndefined(filter.type) && typeof filter.value === 'string')
      ) {
        return false;
      } else {
        return true;
