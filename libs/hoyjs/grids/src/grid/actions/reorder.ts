import { extend, isNullOrUndefined, KeyboardEventArgs } from '@hoyui/base';
import {
...
export class Reorder implements IAction {
  ...
  private chkDropPosition(srcElem: Element, destElem: Element): boolean {
    const col: Column = this.parent.getColumnByUid(
      destElem.firstElementChild.getAttribute('e-mappinguid')
    );
    const bool: boolean = col ? !col.lockColumn : true;
    return (
      (srcElem.parentElement.isEqualNode(destElem.parentElement) ||
        this.parent.enableColumnVirtualization ||
        (this.parent.isFrozenGrid() &&
          Array.prototype.indexOf.call(
            closestElement(srcElem, 'thead').children,
            srcElem.parentElement
          ) ===
            Array.prototype.indexOf.call(
              closestElement(destElem, 'thead').children,
              destElem.parentElement
            ))) &&
  ...
  private getColumnsModel(
    ...
    if (subCols.length) {
      columnModel = columnModel.concat(
        this.getColumnsModel(subCols as Column[])
      );
    }
    return columnModel;
  }
  ...
  private headerDrop(e: { target: Element }): void {
    ...
    const dropElement: Element =
      this.element.querySelector('.e-headercelldiv') ||
      this.element.querySelector('.e-stackedheadercelldiv');
    const uId: string = dropElement.getAttribute('e-mappinguid');
    const column: Column = gObj.getColumnByUid(uId);
    if (
      !closestElement(e.target, 'th') ||
      (!isNullOrUndefined(column) &&
        (!column.allowReordering || column.lockColumn))
    ) {
    ...
    const destElem: Element = closestElement(
      e.target as Element,
      '.e-headercell'
    );
    const destElemDiv: Element =
      destElem.querySelector('.e-headercelldiv') ||
      destElem.querySelector('.e-stackedheadercelldiv');
    ...
    if (!isNullOrUndefined(destElemUid)) {
      ...
      if (
        isNullOrUndefined(destColumn) ||
        !destColumn.allowReordering ||
        destColumn.lockColumn
      ) {
        this.parent.log('action_disabled_column', {
          moduleName: this.getModuleName(),
          column,
    ...
    if (
      ...
      if (this.parent.enableColumnVirtualization) {
        ...
        const newIndex: number = this.targetParentContainerIndex(
          this.element,
          destElem
        );
        const uid: string =
          this.element.firstElementChild.getAttribute('e-mappinguid');
        ...
        if (uid) {
          ...
          const columns: Column[] = this.getColumnsModel(
  ...
  private moveColumns(
    ...
    const parent: Column = this.getColParent(
      column,
      this.parent.columns as Column[]
    );
    const cols: Column[] = parent
      ? (parent.columns as Column[])
      : (this.parent.columns as Column[]);
    let srcIdx: number = inArray(column, cols);
    if (
      (parent || this.parent.lockcolPositionCount) &&
      !reorderByColumn &&
      !this.parent.enableColumnVirtualization
    ) {
      ...
      const col: Column = this.parent.getColumnByUid(
        this.destElement.firstElementChild.getAttribute('e-mappinguid')
      );
      if (col) {
        for (let i: number = 0; i < cols.length; i++) {
        ...
        for (let i: number = 0; i < cols.length; i++) {
          if (
            cols[parseInt(i.toString(), 10)].headerText ===
            (this.destElement as HTMLElement).innerText.trim()
          ) {
            destIndex = i;
          }
        }
    ...
    (cols as Column[]).splice(
      destIndex,
      0,
      (cols as Column[]).splice(srcIdx, 1)[0] as Column
    );
    const args: FrozenReorderArgs = {
      column: column,
      destIndex: destIndex,
      columns: cols,
      parent: parent,
    ...
    if (this.parent.isFrozenGrid()) {
      if (this.parent.frozenColumns) {
        ...
        if (this.parent.getFrozenLeftCount() > destIndex) {
          ...
          cols.length - this.parent.getFrozenRightColumnsCount() <=
          destIndex
        ) {
          column.freeze = 'Right';
        } else {
  ...
  private refreshColumnIndex(cols?: Column[]): void {
    for (let i: number = 0; i < cols.length; i++) {
      ...
      this.idx++;
      if (
        cols[parseInt(i.toString(), 10)].columns &&
        cols[parseInt(i.toString(), 10)].columns.length
      ) {
  ...
  private targetParentContainerIndex(
    ...
    const flatColumns: Column[] =
      stackedHdrColumn.length && stackedCols.length
        ? this.getColumnsModel(stackedCols)
        : this.getColumnsModel(cols, true);
    const parent: Column = this.getColParent(
      flatColumns[getElementIndex(srcElem, headers)],
      cols
    );
  ...
  private getAllStackedheaderParentColumns(headers: Element[]): Column[] {
    ...
    for (let i: number = 0; i < headers.length; i++) {
      if (headers[parseInt(i.toString(), 10)].classList.contains('e-hide')) {
        ...
        headers[parseInt(i.toString(), 10)].closest('thead').firstChild ===
        headers[parseInt(i.toString(), 10)].parentElement
      ) {
        stackedCols.push(
          this.parent.getColumnByUid(
            headers[parseInt(i.toString(), 10)].firstElementChild.getAttribute(
              'e-mappinguid'
            )
          )
        );
  ...
  private getHeaderCells(): Element[] {
    return [].slice.call(
      this.parent.element.getElementsByClassName('e-headercell')
    );
  }
  ...
  private reorderSingleColumn(fromFName: string, toFName: string): void {
    const fColumn: Column = this.parent.enableColumnVirtualization
      ? (this.parent.columns as Column[]).filter(
          (col: Column) => col.field === fromFName
        )[0]
      : this.parent.getColumnByField(fromFName);
    const toColumn: Column = this.parent.enableColumnVirtualization
      ? (this.parent.columns as Column[]).filter(
          (col: Column) => col.field === toFName
        )[0]
      : this.parent.getColumnByField(toFName);
    if (
      (!isNullOrUndefined(fColumn) &&
        (!fColumn.allowReordering || fColumn.lockColumn)) ||
      (!isNullOrUndefined(toColumn) &&
        (!toColumn.allowReordering || toColumn.lockColumn))
    ) {
      this.parent.log('action_disabled_column', {
        moduleName: this.getModuleName(),
        column: fColumn,
  ...
  private reorderMultipleColumns(fromFNames: string[], toFName: string): void {
    ...
    for (let i: number = 0; i < fromFNames.length; i++) {
      const column: Column = this.parent.getColumnByField(
        fromFNames[parseInt(i.toString(), 10)]
      );
      if (
        !isNullOrUndefined(column) &&
        (!column.allowReordering || column.lockColumn)
    ...
    for (let i: number = 0; i < fromFNames.length; i++) {
      ...
      if (destIndex > -1) {
        this.moveColumns(
          destIndex,
          this.parent.getColumnByField(fromFNames[parseInt(i.toString(), 10)]),
          true,
          true
        );
      }
  ...
  private reorderMultipleColumnByTarget(
    ...
    for (let i: number = 0; i < fieldName.length; i++) {
      this.reorderSingleColumnByTarget(
        fieldName[parseInt(i.toString(), 10)],
        toIndex
      );
    }
  }
  ...
  private createReorderElement(e?: {
    args: { isFrozen?: boolean; isXaxis?: boolean };
  }): void {
    if (e && e.args && e.args.isXaxis) {
      this.setDisplay('none');
    }
    const header: Element = this.parent.element.querySelector(
  ...
  public onActionComplete(e: NotifyArgs): void {
    this.parent.trigger(
      ...
      extend(e, { type: events.actionComplete })
    );
    const target: Element =
      this.fromCol && this.parent.getColumnHeaderByField(this.fromCol);
    if (target) {
  ...
  private drag(e: {
    ...
    const closest: Element = closestElement(
      target,
      '.e-headercell:not(.e-stackedHeaderCell)'
    );
    const cloneElement: HTMLElement = gObj.element.querySelector(
      '.e-cloneproperties'
    ) as HTMLElement;
    ...
    removeClass(
      [].slice.call(
        gObj.getHeaderTable().getElementsByClassName('e-reorderindicate')
      ),
      ['e-reorderindicate']
    );
    this.setDisplay('none');
    ...
    if (closest && !closest.isEqualNode(this.element)) {
      ...
      if (
        !(
          !this.chkDropPosition(this.element, target) ||
          !this.chkDropAllCols(this.element, target)
        ) &&
        gObj.getColumnByUid(uid).allowReordering &&
      ...
      if (
        closest &&
        closest.isEqualNode(this.element) &&
        !(
          (gObj.allowGrouping && e.column.allowGrouping) ||
          e.column.allowReordering
        )
      ) {
        classList(cloneElement, ['e-notallowedcur'], ['e-defaultcur']);
  ...
  private updateArrowPosition(target: Element, isLeft: boolean): void {
    ...
    if (
      (isLeft && cliRect.left < cliRectBase.left) ||
      (!isLeft && cliRect.right > cliRectBase.right)
    ) {
      return;
    }
    ...
    this.upArrow.style.top = isSticky
      ? cliRect.top + cliRect.height + 'px'
      : cliRect.top + cliRect.height - cliRectBase.top + 'px';
    this.downArrow.style.top = isSticky
      ? cliRect.top - 7 + 'px'
      : cliRect.top - cliRectBase.top - 7 + 'px';
    this.upArrow.style.left = this.downArrow.style.left = isSticky
      ? (isLeft ? cliRect.left : cliRect.right) - 4 + 'px'
  ...
  private dragStart(e: {
    ...
    this.element = target.classList.contains('e-headercell')
