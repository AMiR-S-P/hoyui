import { isNullOrUndefined, remove } from '@hoyui/base';
...
export class ShowHide {
  ...
  public addEventListener(): void {
    ...
    this.evtHandlers = [
      { event: events.batchCancel, handler: this.batchChanges },
  ...
  public resetColumnState(): void {
    if (this.isShowHide) {
      for (let i: number = 0; i < this.colName.length; i++) {
        this.colName[parseInt(i.toString(), 10)].visible =
          !this.colName[parseInt(i.toString(), 10)].visible;
      }
    }
  }
  ...
  public setVisible(
    ...
    changedStateColumns =
      changedStateColumns.length > 0 ? changedStateColumns : columns;
    const args: Object = {
    ...
    this.parent.trigger(events.actionBegin, args, (showHideArgs: Object) => {
      ...
      if (this.parent.enableColumnVirtualization) {
        const colsInCurrentView: Column[] = columns.filter((col1: Column) =>
          currentViewCols.some((col2: Column) => col1.field === col2.field)
        );
    ...
    if (!this.parent.groupSettings.columns.length) {
      if (this.parent.autoFit) {
        ...
        const isMaxWidth: boolean =
          (this.parent.getHeaderTable() as HTMLElement).style.width.indexOf(
            'px'
          ) === -1;
