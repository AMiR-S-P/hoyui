import { KeyboardEventArgs } from '@hoyui/base';
import { extend } from '@hoyui/base';
import { remove, isNullOrUndefined } from '@hoyui/base';
...
export class Page implements IAction {
  ...
  private addAriaAttr(): void {
    if (!this.pageSettings.template) {
      const numericContainerNew: Element = this.parent.createElement('div', {
        className: 'e-numericcontainer',
      });
      const pagerContainer: Element =
        this.element.querySelector('.e-pagercontainer');
      const frag: DocumentFragment = document.createDocumentFragment();
      const numericContainer: Element = this.element.querySelector(
        '.e-numericcontainer'
      );
      const links: NodeList = numericContainer.querySelectorAll('a');
      for (let i: number = 0; i < links.length; i++) {
        if (this.parent.getContentTable()) {
          (<Element>links[parseInt(i.toString(), 10)]).setAttribute(
            'aria-owns',
            this.parent.getContentTable().id + ' ' + (i + 1)
          );
        } else {
          (<Element>links[parseInt(i.toString(), 10)]).setAttribute(
            'aria-owns',
      ...
      for (let j: number = 0; j < classList.length; j++) {
        const element: Element = this.element.querySelector(
          classList[parseInt(j.toString(), 10)]
        );
        if (this.parent.getContentTable()) {
          element.setAttribute(
            'aria-owns',
            this.parent.getContentTable().id +
              classList[parseInt(j.toString(), 10)].replace('.e-', ' ')
          );
        }
      }
    }
  ...
  public updateModel(e?: NotifyArgs): void {
    ...
    const isAddAction: boolean =
      (e.action === 'add' && e.requestType === 'save') ||
  ...
  private clickHandler(e: {
    ...
    if (
      this.isForceCancel ||
      (isActionPrevent(gObj) && !gObj.prevPageMoving && !this.isCancel)
    ) {
      if (!this.isForceCancel) {
        if (
          !isNullOrUndefined(e.newProp) &&
          !isNullOrUndefined(e.newProp.pageSize)
        ) {
          ...
          this.parent.setProperties(
            { pageSettings: { pageSize: e.oldProp.pageSize } },
            true
          );
          this.pagerObj.setProperties(
            {
              currentPage:
                gObj.pageSettings.currentPage === this.pagerObj.currentPage
    ...
    if ((<{ cancel?: boolean }>args).cancel) {
      e.cancel = true;
      this.parent.setProperties(
        { pageSettings: { currentPage: prevPage } },
        true
      );
  ...
  public addEventListener(): void {
    ...
    if (this.parent.isReact || this.parent.isVue) {
      this.parent.addEventListener(
        literals.create,
        this.handlers.created.bind(this)
      );
    }
    this.evtHandlers = [
      { event: events.initialLoad, handler: this.handlers.load },
  ...
  private isReactTemplate(): boolean {
    return (
      ...
      this.pagerObj.template &&
      typeof this.pagerObj.template !== 'string'
    );
  }
