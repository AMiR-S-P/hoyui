import { MouseEventArgs, Draggable, isNullOrUndefined } from '@hoyui/base';
import { removeClass } from '@hoyui/base';
import {
  remove,
  closest as closestElement,
  classList,
  BlazorDragEventArgs,
  extend,
} from '@hoyui/base';
...
export class RowDD {
  ...
  private helper: Function = (e: { sender: MouseEventArgs }) => {
    ...
    if (
      ...
      gObj.rowDropSettings.targetID &&
      !isNullOrUndefined(target.classList) &&
    ...
    const table: Element = this.parent.createElement('table', {
    ...
    if (
      ...
      (gObj.rowDropSettings.targetID &&
        ((!isNullOrUndefined(target) &&
          !(target as Element).classList.contains('e-selectionbackground') &&
          gObj.selectionSettings.type !== 'Single') ||
          !parentsUntil(target as Element, 'e-rowcell'))) ||
      (!gObj.rowDropSettings.targetID &&
        !parentsUntil(target as Element, 'e-rowdragdrop'))
    ) {
      return false;
    }
    if (
      gObj.rowDropSettings.targetID &&
      gObj.selectionSettings.mode === 'Row' &&
      gObj.selectionSettings.type === 'Single'
    ) {
      gObj.selectRow(
        parseInt(
          (
            this.draggable.currentStateTarget as Element
          ).parentElement.getAttribute(literals.dataRowIndex),
          10
        )
      );
    }
    this.startedRow = closestElement(target as Element, 'tr').cloneNode(
      true
    ) as HTMLTableRowElement;
    if (this.parent.isFrozenGrid()) {
      const nodes: Element[] = [].slice.call(
        this.startedRow.querySelectorAll('.e-rowcell')
      );
    ...
    if (
      gObj.getSelectedRowIndexes().length > 1 &&
      this.startedRow.hasAttribute('aria-selected')
    ) {
    ...
    const ele: Element = closestElement(target as Element, 'tr').querySelector(
      '.e-icon-rowdragicon'
    );
    if (ele) {
  ...
  private dragStart: Function = (
    e: { target: HTMLElement; event: MouseEventArgs } & BlazorDragEventArgs
  ) => {
    const gObj: IGrid = this.parent;
    if (
      (gObj.enableVirtualization || gObj.infiniteScrollSettings.enableCache) &&
      gObj.allowGrouping &&
      ...
      const indentCells: NodeListOf<Element> = e.target
        .closest('tr')
        .querySelectorAll('.e-indentcell');
      for (let i: number = 0; i < dragTrs.length; i++) {
        for (let j: number = 0; j < indentCells.length; j++) {
          const cloneIndentCell: Node =
            indentCells[parseInt(j.toString(), 10)].cloneNode(true);
          dragTrs[parseInt(i.toString(), 10)].insertBefore(
            cloneIndentCell,
    ...
    if (gObj.element.classList.contains('e-childgrid')) {
      ...
      gObj.element.appendChild(
        gObj.createElement('div', { className: 'e-drag-ref' })
      );
    }
    ...
    const spanCssEle: HTMLSpanElement = this.parent.element.querySelector(
      '.e-dropitemscount'
    ) as HTMLSpanElement;
    if (this.parent.getSelectedRecords().length > 1 && spanCssEle) {
      spanCssEle.style.left =
        (
          this.parent.element.querySelector(
            '.e-cloneproperties table'
          ) as HTMLTableRowElement
        ).offsetWidth -
        5 +
        'px';
    ...
    gObj.trigger(events.rowDragStart, {
      ...
      data:
        this.rowData[0] && Object.keys(this.rowData[0]).length > 0
          ? (this.rowData as Object[])
          : this.currentViewData(),
    });
    this.dragStartData = this.rowData;
    const dropElem: EJ2Intance = document.getElementById(
      gObj.rowDropSettings.targetID
    ) as EJ2Intance;
    if (
      gObj.rowDropSettings.targetID &&
      dropElem &&
      dropElem.ej2_instances &&
      (<{ getModuleName?: Function }>(
        dropElem.ej2_instances[0]
      )).getModuleName() === 'grid'
    ) {
  ...
  private getParentGrid(childGrid: Element): Element {
    ...
    while (!parentGridObtained) {
      if ((<EJ2Intance>parentGrid).ej2_instances[0].parentDetails) {
        parentGrid = document.getElementById(
          (<EJ2Intance>parentGrid).ej2_instances[0].parentDetails.parentID
        );
      }
  ...
  private drag: Function = (e: {
    ...
    if (this.parent.rowDropSettings.targetID) {
      const dropElement: EJ2Intance = document.getElementById(
        gObj.rowDropSettings.targetID
      ) as EJ2Intance;
      ...
      if (parentsUntil(e.target, 'e-grid')) {
        this.istargetGrid =
          ...
          parentsUntil(e.target, 'e-grid').id;
      }
    }
    let cloneElement: HTMLElement = this.parent.element.querySelector(
      '.e-cloneproperties'
    ) as HTMLElement;
    if (gObj.element.classList.contains('e-childgrid')) {
      const parentGrid: Element = this.getParentGrid(gObj.element);
      cloneElement = parentGrid.querySelector(
        '.e-cloneproperties'
      ) as HTMLElement;
    }
    ...
    classList(
      cloneElement,
      ['e-defaultcur'],
      ['e-notallowedcur', 'e-movecur', 'e-grabcur']
    );

    this.isOverflowBorder = true;
    this.hoverState = gObj.enableHover;
    const trElement: HTMLTableRowElement = parentsUntil(target, 'e-grid')
      ? (closestElement(e.target, 'tr') as HTMLTableRowElement)
      : null;
    if (!e.target) {
      return;
    }
    ...
    if (
      trElement &&
      (parentsUntil(target, 'e-grid').id === cloneElement.parentElement.id ||
        parentsUntil(target, 'e-grid').id)
    ) {
      ...
      this.dragTarget = parseInt(
        this.startedRow.getAttribute('data-rowindex'),
        10
      );
    }
    ...
    if (gObj.rowDropSettings.targetID) {
      ...
      if (
        !parentsUntil(target, 'e-grid') ||
        (dragParentElement &&
          parentsUntil(dragParentElement.parentElement, 'e-grid').id ===
            parentsUntil(target, 'e-grid').id) ||
      ...
      if (
        element &&
        element.id === cloneElement.parentElement.id &&
        parentsUntil(target, 'e-row') &&
    ...
    if (parentsUntil(this.isDropGrid.element, 'e-grid')) {
      if (
        (!this.isDropGrid.groupSettings.columns.length ||
          this.isDropGrid.groupSettings.columns.length) &&
        !this.isDropGrid.element.querySelector('.e-emptyrow')
      ) {
        if (
          parentsUntil(target, 'e-grid') &&
          parentsUntil(target, 'e-grid').id === this.isDropGrid.element.id
        ) {
          this.updateScrollPostion(e.event);
        }
        if (
          ((this.isOverflowBorder ||
            this.parent.frozenRows > this.dragTarget) &&
            (parseInt(
              this.startedRow.getAttribute(literals.dataRowIndex),
              10
            ) !== this.dragTarget ||
              this.istargetGrid)) ||
          (this.istargetGrid &&
            trElement &&
            this.isDropGrid
              .getRowByIndex(this.isDropGrid.getCurrentViewRecords().length - 1)
          ...
          if (this.parent.enableVirtualization) {
            islastRowIndex =
              trElement &&
              parseInt(trElement.getAttribute(literals.dataRowIndex), 10) ===
          ...
          if (islastRowIndex && !this.parent.rowDropSettings.targetID) {
            const bottomborder: HTMLElement = this.parent.createElement('div', {
              className: 'e-lastrow-dragborder',
            });
            const gridcontentEle: Element = this.parent.getContent();
            bottomborder.style.width =
              (this.parent.element as HTMLElement).offsetWidth -
              this.getScrollWidth() +
              'px';
            if (this.parent.enableVirtualization) {
            ...
            if (
              !gridcontentEle.getElementsByClassName('e-lastrow-dragborder')
                .length &&
              (!(gObj.allowGrouping && gObj.groupSettings.columns.length) ||
                isNullOrUndefined(trElement.nextSibling))
            ) {
      ...
      if (
        target &&
        target.classList.contains(literals.content) &&
        !this.isDropGrid.element.querySelector('.e-emptyrow') &&
        this.istargetGrid
      ) {
        this.removeBorder(trElement);
        const rowIndex: number =
          this.isDropGrid.getCurrentViewRecords().length - 1;
        const selector: string =
          '.e-rowcell,.e-rowdragdrop,.e-detailrowcollapse';
        const groupSelector: string =
          '.e-rowcell:not(.e-hide),.e-rowdragdrop:not(.e-hide),.e-detailrowcollapse:not(.e-hide)';
        let rowElement: HTMLElement[] = [];
        if (
          this.parent.allowGrouping &&
          this.parent.groupSettings.columns &&
          this.parent.groupSettings.columns.length
        ) {
          rowElement = [].slice.call(
            this.isDropGrid
          ...
          rowElement = [].slice.call(
            this.isDropGrid.getRowByIndex(rowIndex).querySelectorAll(selector)
          );
        }
  ...
  private processDragStop: Function = (e: {
    target: HTMLTableRowElement;
    event: MouseEventArgs;
    helper: Element;
  }) => {
    ...
    const targetEle: Element = this.getElementFromPosition(
      e.helper as HTMLElement,
      e.event
    );
    const target: Element =
      targetEle && !targetEle.classList.contains('e-dlg-overlay')
        ? targetEle
        : e.target;
    ...
    if (
      gObj.rowDropSettings.targetID &&
      dropElement &&
      dropElement.ej2_instances &&
      (<{ getModuleName?: Function }>(
        dropElement.ej2_instances[0]
      )).getModuleName() === 'grid'
    ) {
      dropElement.ej2_instances[0]
        .getContent()
        .classList.remove('e-allowRowDrop');
    }
    if (parentsUntil(this.isDropGrid.element, 'e-grid')) {
      this.stopTimer();
      ...
      const stRow: Element =
        this.isDropGrid.element.querySelector('.e-dragstartrow');
      if (stRow) {
    ...
    const args: RowDropEventArgs = {
      ...
      rows: this.rows,
      data:
        Object.keys(this.dragStartData[0]).length > 0
          ? (this.dragStartData as Object[])
          : this.currentViewData(),
    };
    gObj.trigger(events.rowDrop, args, () => {
      if (
        !(
          ...
          parentsUntil(target, 'e-emptyrow') ||
          parentsUntil(target, literals.gridContent)
        ) ||
        args.cancel
      ) {
      ...
      if (gObj.isRowDragable()) {
        if (
          !this.parent.rowDropSettings.targetID &&
          this.startedRow.querySelector('td.e-selectionbackground') &&
          ...
          this.reorderRows(
            [parseInt(this.startedRow.getAttribute(literals.dataRowIndex), 10)],
            this.dragTarget
          );
        }
        this.dragTarget = null;
        if (!gObj.rowDropSettings.targetID) {
          ...
          if (
            ...
            !gObj.sortSettings.columns.length &&
            !gObj.filterSettings.columns.length &&
        ...
        if (
          this.parent.getContentTable().scrollHeight <
          this.parent.getContent().clientHeight
        ) {
  ...
  private refreshRow(
    ...
    const gObj: IGrid = this.parent;
    const tbodyContent: Element = gObj
      .getContentTable()
      .querySelector(literals.tbody);
    const tbodyHeader: Element = gObj
      .getHeaderTable()
      .querySelector(literals.tbody);
    for (let i: number = 0, len: number = args.rows.length; i < len; i++) {
      ...
      if (gObj.allowGrouping && gObj.groupSettings.columns.length) {
        ...
        const dropRowUid: string =
          args.target.parentElement.getAttribute('data-uid');
    ...
    const tr: HTMLTableRowElement[] = [].slice.call(
      gObj.editSettings.showAddNewRow
        ? tbody.querySelectorAll('.e-row:not(.e-addedrow)')
        : tbody.getElementsByClassName(literals.row)
    );
    if (gObj.allowGrouping && gObj.groupSettings.columns.length) {
      if (
        gObj.groupSettings.enableLazyLoading ||
        (gObj.enableInfiniteScrolling &&
          gObj.infiniteScrollSettings.enableCache &&
          tr.length > gObj.pageSettings.pageSize * 3)
      ) {
        gObj.refresh();
      } else {
        ...
        if (
          ...
          resetRowIndex(
            this.parent,
            gObj.getRowsObject().filter((data: Row<Column>) => data.isDataRow),
            tr
          );
        }
        this.parent.notify(events.refreshExpandandCollapse, {
          rows: gObj.enableVirtualization
            ? this.parent.vRows
  ...
  private updateFrozenRowreOrder(args: RowDropEventArgs): void {
    const gObj: IGrid = this.parent;
    const tbodyC: Element = gObj
      .getContentTable()
      .querySelector(literals.tbody);
    const tbodyH: Element = gObj.getHeaderTable().querySelector(literals.tbody);
    const tr: HTMLTableRowElement[] = [].slice
      .call(tbodyH.getElementsByClassName(literals.row))
      .concat([].slice.call(tbodyC.getElementsByClassName(literals.row)));
    const tbody: Element = gObj.createElement(literals.tbody, {
      attrs: { role: 'rowgroup' },
    });
    if (
      !gObj.selectionSettings.persistSelection &&
      Object.keys(gObj.selectionModule.selectedRowState).length === 0
    ) {
  ...
  private refreshRowTarget(args: RowDropEventArgs): Element {
    const gObj: IGrid = this.parent;
    let targetIdx: number = parseInt(
      args.target.parentElement.getAttribute(literals.dataRowIndex),
      10
    );
    if (
      gObj.enableVirtualization &&
    ...
    if (
      (args.fromIndex < args.dropIndex || args.fromIndex === args.dropIndex) &&
      (!gObj.allowGrouping || !gObj.groupSettings.columns.length)
    ) {
    ...
    const tr: Element =
      ...
      gObj.groupSettings.columns.length &&
      targetIdx !== -1 &&
      args.fromIndex < args.dropIndex &&
      targetTR
        ? (targetTR.nextSibling as Element)
        : targetTR;
    return tr;
  }
  ...
  private refreshData(tr: HTMLTableRowElement[]): void {
    ...
    for (let i: number = 0, len: number = tr.length; i < len; i++) {
      ...
      rowObj[parseInt(i.toString(), 10)] =
        rowObjects[parseInt(index.toString(), 10)];
      recordobj[parseInt(i.toString(), 10)] =
    ...
    for (let i: number = 0, len: number = tr.length; i < len; i++) {
      ...
      rowObjects[parseInt(i.toString(), 10)] =
        rowObj[parseInt(i.toString(), 10)];
  ...
  private rowOrder(args: RowDropEventArgs): void {
    ...
    if (
      args.target.classList.contains('e-rowcelldrag') ||
      args.target.classList.contains('e-dtdiagonalright') ||
    ...
    if (
      ...
      parentsUntil(args.target, 'e-rowcell')
    ) {
    ...
    if (
      ...
      if (
        ...
        for (let i: number = 0, len: number = args.rows.length; i < len; i++) {
          indexes.push(
            parseInt(
              args.rows[parseInt(i.toString(), 10)].getAttribute(
  ...
  private currentViewData(): object[] {
    ...
    const fromIdx: number = parseInt(
      this.startedRow.getAttribute(literals.dataRowIndex),
      10
    );
    for (let i: number = 0, n: number = selectedIndexes.length; i < n; i++) {
      const currentV: string = 'currentViewData';
      currentVdata[parseInt(i.toString(), 10)] =
        this.parent[`${currentV}`][selectedIndexes[parseInt(i.toString(), 10)]];
    }
    if (!this.parent.rowDropSettings.targetID && selectedIndexes.length === 0) {
      currentVdata[0] =
        this.parent.currentViewData[parseInt(fromIdx.toString(), 10)];
    }
    return currentVdata;
  }
  ...
  public reorderRows(fromIndexes: number[], toIndex: number): void {
    ...
    const currentViewData: Object[] = this.parent.getDataModule().isRemote()
    ...
    for (let i: number = 0, len: number = fromIndexes.length; i < len; i++) {
      draggedRecords[parseInt(i.toString(), 10)] =
        currentViewData[fromIndexes[parseInt(i.toString(), 10)] + skip];
    }
    for (
      let i: number = 0, len: number = selectedIndexes.length;
      i < len;
      i++
    ) {
      selectedRecords[parseInt(i.toString(), 10)] =
        currentViewData[selectedIndexes[parseInt(i.toString(), 10)] + skip];
    }
    for (let i: number = 0, len: number = draggedRecords.length; i < len; i++) {
      if (i !== 0) {
        for (
          let j: number = 0, len1: number = currentViewData.length;
          j < len1;
          j++
        ) {
          if (
            JSON.stringify(currentViewData[parseInt(j.toString(), 10)]) ===
        ...
        for (
          ...
          if (
            JSON.stringify(currentViewData[parseInt(j.toString(), 10)]) ===
            JSON.stringify(draggedRecords[i - 1])
          ) {
            if (actualIdx > j) {
              dropIdx = j + 1;
            }
    ...
    for (
      let i: number = 0, len: number = selectedRecords.length;
      i < len;
      i++
    ) {
      for (
        ...
        if (
          JSON.stringify(currentViewData[parseInt(j.toString(), 10)]) ===
          JSON.stringify(selectedRecords[parseInt(i.toString(), 10)])
        ) {
  ...
  private removeCell: Function = (
    targetRow: HTMLTableRowElement,
    className: string
  ) => {
    return [].slice
      .call(targetRow.querySelectorAll('td'))
      .filter((cell: HTMLTableCellElement) => {
        if (cell.classList.contains(className)) {
          (targetRow as HTMLTableRowElement).deleteCell(cell.cellIndex);
        }
      });
  };
  ...
  public onActionComplete(e: NotifyArgs): void {
    this.parent.trigger(
      ...
      extend(e, { type: events.actionComplete, requestType: 'rowdraganddrop' })
    );
  }
  ...
  private updateScrollPostion(e: MouseEvent | TouchEvent): void {
    ...
    const cliRect: ClientRect = this.isDropGrid
      .getContent()
    ...
    if (cliRect.top >= y) {
      ...
      cliRect.top +
        this.isDropGrid.getContent().clientHeight -
        rowHeight -
        33 -
        virtualScrollbtm <=
      y
  ...
  private moveDragRows(
    e: { target: HTMLElement; event: MouseEventArgs },
    startedRow: HTMLTableRowElement,
    targetRow: HTMLTableRowElement
  ): void {
    let cloneElement: HTMLElement = this.parent.element.querySelector(
      '.e-cloneproperties'
    ...
    const element: HTMLTableRowElement = closestElement(
      e.target,
      'tr'
    ) as HTMLTableRowElement;
    if (
      parentsUntil(element, 'e-grid') &&
      ((!this.parent.rowDropSettings.targetID &&
        parentsUntil(cloneElement.parentElement, 'e-grid').id ===
  ...
  private setBorder(
    ...
    if (
      parentsUntil(element, 'e-grid') &&
      element.classList.contains(literals.row) &&
      ((!this.parent.rowDropSettings.targetID &&
        parentsUntil(cloneElement.parentElement, 'e-grid').id ===
          parentsUntil(element, 'e-grid').id) ||
        this.istargetGrid)
      ...
      if (
        ...
        removeClass(
          ...
          ['e-dragtop', 'e-dragright', 'e-dragbottom', 'e-dragleft']
        );
      } else {
        removeClass(
          node.querySelectorAll(
      ...
      let rowElement: HTMLElement[] = [];
      let targetRowIndex: number = parseInt(
        targetRow.getAttribute(literals.dataRowIndex),
        10
      );
      if (
        targetRow &&
        ...
        if (
          ...
          rowElement = [].slice.call(
            element.querySelectorAll(
              '.e-groupcaption,.e-summarycell,.e-rowcell,.e-rowdragdrop,.e-detailrowcollapse'
            )
          );
        } else {
          ...
          if (
            !gridheaderEle.getElementsByClassName('e-firstrow-dragborder')
              .length
          ) {
            if (this.parent.frozenRows) {
              if (this.parent.isFrozenGrid()) {
        ...
        targetRow &&
        parseInt(startedRow.getAttribute(literals.dataRowIndex), 10) >
          targetRowIndex
      ) {
        if (
        ...
        if (
          ...
          element = targetRow;
          rowElement = [].slice.call(
            element.querySelectorAll(`.e-groupcaption,.e-summarycell,.e-rowcell:not(.e-hide),.e-rowdragdrop:not(.e-hide),
          ...
          rowElement = [].slice.call(
            element.querySelectorAll(
              '.e-rowcell,.e-rowdragdrop,.e-detailrowcollapse,.e-dragindentcell'
            )
          );
        }
      } else {
  ...
  private getScrollWidth(): number {
    const scrollElem: HTMLElement = this.parent.getContent()
  ...
  private removeFirstRowBorder(element: Element): void {
    if (
      this.isDropGrid.element.getElementsByClassName('e-firstrow-dragborder')
        .length > 0 &&
      element &&
      ...
      remove(
        this.isDropGrid.element.getElementsByClassName(
          'e-firstrow-dragborder'
        )[0]
      );
    } else {
      const addNewRow: HTMLElement = this.parent.element.querySelector(
        '.e-row.e-addedrow tr'
      );
      if (addNewRow && addNewRow.querySelector('.e-dragborder')) {
        const rowElement: HTMLElement[] = [].slice.call(
          addNewRow.querySelectorAll(
            '.e-rowcell,.e-rowdragdrop,.e-detailrowcollapse,.e-dragindentcell'
          )
        );
  ...
  private removeLastRowBorder(element: Element): void {
    ...
    if (this.parent.enableVirtualization) {
      ...
      islastRowIndex =
        element &&
        lastRow &&
        lastRow.getAttribute('data-uid') !== element.getAttribute('data-uid');
    }
    if (
  ...
  private removeBorder(element: Element): void {
    ...
    if (this.parent.allowGrouping && this.parent.groupSettings.columns.length) {
      element = [].slice
        .call(this.isDropGrid.getContentTable().querySelectorAll('tr'))
        .filter((row: Element) =>
          row.querySelector('td.e-dragtop.e-dragbottom')
        )[0];
    } else {
      element = this.isDropGrid
        .getRows()
    ...
    if (element) {
      const rowElement: HTMLElement[] =
        this.parent.allowGrouping && this.parent.groupSettings.columns.length
          ? [].slice.call(element.querySelectorAll('.e-dragtop.e-dragbottom'))
          : [].slice.call(element.getElementsByClassName('e-dragborder'));
      if (
        this.parent.allowGrouping &&
        this.parent.groupSettings.columns &&
        this.parent.groupSettings.columns.length
  ...
  private onDataBound(): void {
    if (
      this.selectedRowColls.length > 0 &&
      (this.parent.enableVirtualization || this.parent.allowRowDragAndDrop)
    ) {
      this.parent.selectRows(this.selectedRowColls);
      this.selectedRowColls = [];
    }
  ...
  private getTargetIdx(targetRow: Element): number {
    return targetRow
      ? parseInt(targetRow.getAttribute(literals.dataRowIndex), 10)
      : 0;
  }

  private singleRowDrop(e: {
    target: HTMLTableRowElement;
    droppedElement: HTMLElement;
  }): void {
    const targetRow: HTMLTableRowElement = closestElement(
      e.target,
      'tr'
    ) as HTMLTableRowElement;
    const srcControl: IGrid = (<EJ2Intance>e.droppedElement.parentElement)
      .ej2_instances[0];
    const currentIndex: number = targetRow
      ? targetRow.rowIndex
      : srcControl.currentViewData.length - 1;
  ...
  private columnDrop(e: {
    ...
    if (
      ...
      (parentsUntil(e.target, literals.row) ||
        parentsUntil(e.target, 'e-emptyrow') ||
      ...
      const dragParentElement: Element = document.querySelector('.e-drag-ref');
      if (
        (e.droppedElement.querySelector('tr').getAttribute('single-dragrow') !==
          'true' &&
      ...
      if (e.droppedElement.parentElement.id !== gObj.element.id) {
        if (dragParentElement) {
          srcControl = (<EJ2Intance>dragParentElement.parentElement)
            .ej2_instances[0];
          remove(dragParentElement);
        } else {
          srcControl = (<EJ2Intance>e.droppedElement.parentElement)
            .ej2_instances[0];
        }
      } else if (
      ...
      if (
        srcControl.element.id !== gObj.element.id &&
        srcControl.rowDropSettings.targetID !== gObj.element.id
      ) {
        return;
      }
      ...
      if (
        ...
        gObj.getCurrentViewRecords().length
      ) {
        const lastrow: Element = gObj
          .getContentTable()
          .querySelector('tr:last-child');
        if (lastrow) {
          targetIndex = currentIndex = parseInt(
            lastrow.getAttribute(literals.ariaRowIndex),
            10
          );
        }
      }
      ...
      const skip: number = srcControl.allowPaging
        ? srcControl.pageSettings.currentPage *
            srcControl.pageSettings.pageSize -
          srcControl.pageSettings.pageSize
        : 0;
      this.selectedRows = [];
      ...
      if (
        srcControl.dataSource instanceof DataManager &&
        srcControl.dataSource.dataSource.offline
      ) {
  ...
  private processArgs(target: Element): void {
    const gObj: IGrid = this.parent;
    const dragIdx: number = parseInt(
      this.startedRow.getAttribute(literals.dataRowIndex),
      10
    );
    if (
      (gObj.getSelectedRecords().length > 0 &&
        this.startedRow.cells[0].classList.contains('e-selectionbackground') ===
          false) ||
      gObj.getSelectedRecords().length === 0
    ) {
      if (
      ...
      this.rowData = [
        gObj.getRowInfo(this.startedRow.querySelector('.' + literals.rowCell))
          .rowData,
      ];
      if (
        (gObj.enableVirtualization ||
        ...
        this.rowData = Array.from(
          this.rows,
          (row: Element) =>
            gObj.getRowObjectFromUID(row.getAttribute('data-uid')).data
        );
      }
    } else {
      this.rows = gObj.getSelectedRows();
