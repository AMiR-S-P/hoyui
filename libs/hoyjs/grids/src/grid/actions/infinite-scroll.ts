import { isNullOrUndefined, remove, extend } from '@hoyui/base';
import { Query, Predicate } from '@hoyui/data';
import {
  IGrid,
  IAction,
  NotifyArgs,
...
export class InfiniteScroll implements IAction {
  ...
  constructor(parent?: IGrid, serviceLocator?: ServiceLocator) {
    ...
    this.widthService =
      serviceLocator.getService<ColumnWidthService>('widthService');
  ...
  private setGroupCollapsePageQuery(args: {
    query: Query;
    skipPage: boolean;
  }): void {
    ...
    if (!gObj.infiniteScrollSettings.enableCache && this.isGroupCollapse) {
      ...
      if (this.groupCaptionAction === 'collapse') {
        const captionRow: Row<Column> = gObj.getRowObjectFromUID(
          this.parentCapUid
        );
        ...
        for (let i: number = rowObjs.length - 1; i >= 0; i--) {
          if (
            rowObjs[parseInt(i.toString(), 10)].indent === captionRow.indent
          ) {
            break;
          }
        ...
        const key: { fields: string[]; keys: string[] } = getGroupKeysAndFields(
          rowObjs.indexOf(captionRow),
          rowObjs
        );
        let pred: Predicate = generateExpandPredicates(
          key.fields,
          key.keys,
          this
        );
        ...
        const size: number = gObj.pageSettings.pageSize;
        ...
        args.query.take(
          gObj.infiniteScrollSettings.initialBlocks *
  ...
  private captionActionComplete(args: {
    isCollapse: boolean;
    parentUid: string;
  }): void {
    const gObj: IGrid = this.parent;
    if (!gObj.infiniteScrollSettings.enableCache && args.isCollapse) {
      const contetRect: DOMRect | ClientRect = gObj
        .getContent()
        .firstElementChild.getBoundingClientRect();
      const tableReact: DOMRect | ClientRect = gObj.contentModule
        .getTable()
        .getBoundingClientRect();
      if (
        Math.round(tableReact.bottom - gObj.getRowHeight()) <=
        Math.round(contetRect.bottom)
      ) {
        this.parentCapUid = args.parentUid;
        this.groupCaptionAction = 'collapse';
        gObj.showSpinner();
        const caption: Row<Column> = gObj.getRowObjectFromUID(args.parentUid);
  ...
  private makeGroupCollapseRequest(parentUid?: string): void {
    const gObj: IGrid = this.parent;
    const captionRows: Element[] = [].slice.call(
      gObj.getContentTable().querySelectorAll('tr')
    );
    const rows: Element[] = gObj.groupSettings.enableLazyLoading
      ? captionRows
      : gObj.getRows();
    const index: number = !gObj.groupSettings.enableLazyLoading
      ? getRowIndexFromElement(rows[rows.length - 1])
      : gObj.contentModule['visibleRows'].length - 1;
    const prevPage: number = this.parent.pageSettings.currentPage;
    const nextPage: number =
      Math.ceil(index / this.parent.pageSettings.pageSize) + 1;
  ...
  private getCaptionChildCount(caption: Row<Column>): boolean {
    ...
    const index: number = rowObj.indexOf(caption);
    let make: boolean = false;
    for (let i: number = index; i < rowObj.length; i++) {
      if (
        (rowObj[parseInt(i.toString(), 10)].indent === caption.indent ||
          rowObj[parseInt(i.toString(), 10)].indent < caption.indent) &&
        (rowObj[parseInt(i.toString(), 10)].data as GroupedData).key !==
          (caption.data as GroupedData).key
      ) {
        break;
      }
      if (
        rowObj[parseInt(i.toString(), 10)].isCaptionRow &&
        !this.childCheck(rowObj, rowObj[parseInt(i.toString(), 10)], i)
      ) {
        make = true;
        break;
      }
  ...
  private childCheck(
    ...
    for (let i: number = index + 1; i < rowObj.length; i++) {
      ...
      if (
        rowObj[parseInt(i.toString(), 10)].indent === row.indent + 1 &&
  ...
  private updateCurrentViewData(): void {
    ...
    const isMiddlePage: boolean =
      isCache && (page > blocks || (this.isUpScroll && page > 1));
    const start: number = isMiddlePage
      ? this.isUpScroll
        ? page
        : page - blocks + 1
      : 1;
    const end: number = isMiddlePage
      ? start + blocks - 1
      : isCache
      ? blocks
      : keys.length;
    for (let i: number = start; i <= end; i++) {
      if (this.infiniteCurrentViewData[parseInt(i.toString(), 10)]) {
        gObj.currentViewData = gObj.currentViewData.concat(
  ...
  private refreshInfiniteCurrentViewData(e: {
    args: NotifyArgs;
    data: Object[];
  }): void {
    if (e.args.action === 'add' && e.args.requestType === 'save') {
      this.parent.pageSettings.currentPage = Math.ceil(
        e.args['index'] / this.parent.pageSettings.pageSize
      )
        ? Math.ceil(e.args['index'] / this.parent.pageSettings.pageSize)
        : 1;
    }
      ...
      if (!cache && (isAdd || isDelete)) {
        if (isAdd) {
          ...
          for (let i: number = 1; i <= keys.length; i++) {
            indexCount +=
              this.infiniteCurrentViewData[parseInt(i.toString(), 10)].length -
              1;
            if ((e.args as AddEventArgs).index <= indexCount) {
              this.resetCurrentViewData(i);
              this.infiniteCurrentViewData[parseInt(i.toString(), 10)].splice(
                (e.args as AddEventArgs).index,
                0,
                (e.args as AddEventArgs).data
              );
              break;
            }
          }
        ...
        if (blocks > 1 && e.data.length === blocks * size) {
          this.setInitialCache(
            e.data.slice() as Row<Column>[],
            {},
            cache && e.args.requestType === 'delete',
            true
          );
        } else {
          this.infiniteCurrentViewData[parseInt(page.toString(), 10)] =
            e.data.slice();
        }
      }
    }
  ...
  private resetCurrentViewData(startIndex: number): void {
    ...
    for (let i: number = startIndex; i <= keys.length; i++) {
      const lastViewData: Object =
        this.infiniteCurrentViewData[parseInt(i.toString(), 10)][
          this.infiniteCurrentViewData[parseInt(i.toString(), 10)].length - 1
        ];
  ...
  private modelChanged(args: InfiniteScrollArgs): void {
    ...
    if (
      ...
      (args as AddEventArgs).index &&
      (args as AddEventArgs).data
    ) {
    ...
    if (
      ...
      (args.requestType === 'delete' || this.requestType === 'add')
    ) {
      this.firstIndex = getRowIndexFromElement(rows[0]);
      this.firstBlock = Math.ceil(
        (this.firstIndex + 1) / this.parent.pageSettings.pageSize
      );
      ...
      if (args.requestType === 'delete') {
        ...
        args.startIndex = this.parent.infiniteScrollSettings.enableCache
          ? (this.firstBlock - 1) * this.parent.pageSettings.pageSize
          : rowObj[rowObj.length - 1].index;
      } else {
  ...
  private infiniteEditHandler(args: {
    e: InfiniteScrollArgs;
    result: Object[];
  }): void {
    if (
      !this.parent.infiniteScrollSettings.enableCache &&
      (args.e.requestType === 'delete' ||
      ...
      if (this.ensureRowAvailability(rows, args.result[0])) {
        if (
          rowElms.length &&
          !(this.addRowIndex && this.addRowIndex >= rowElms.length)
        ) {
          this.resetRowIndex(
            rows,
            args.e,
  ...
  private createRow(
    rows: Row<Column>[],
    args: { e: InfiniteScrollArgs; result: Object[] },
    isMovable?: boolean,
    isFrozenRows?: boolean,
    isFrozenRight?: boolean
  ): void {
    const row: Row<Column>[] = !isFrozenRows
      ? this.generateRows(args.result, args.e)
      : rows;
    ...
    if (
      (args.e as AddEventArgs).requestType === 'save' &&
      (args.e as AddEventArgs).index &&
      (args.e as AddEventArgs).data
    ) {
    ...
    if (this.parent.frozenRows) {
      tbody =
        (isFrozenRows && this.requestType !== 'add') ||
        (!isFrozenRows && this.requestType === 'add')
          ? this.parent.getHeaderContent().querySelector(literals.tbody)
          : tbody;
    }
    const notifyArgs: {
      rows: Row<Column>[];
    ...
    if (
      ...
      (this.parent.selectionModule.index < this.parent.frozenRows ||
        this.requestType === 'add')
    ) {
      const rowElems: Element[] = this.parent.getRows();
      const index: number =
        (isMovable || isFrozenRight) && this.requestType === 'add'
          ? this.parent.frozenRows
          : this.parent.frozenRows - 1;
    ...
    if (!this.parent.infiniteScrollSettings.enableCache && !isFrozenRows) {
      setRowElements(this.parent);
      (<{ visibleRows?: Row<Column>[] }>this.parent.contentModule).visibleRows =
        this.requestType === 'add' ? row.concat(rows) : rows.concat(row);
    }
  }
  ...
  private ensureRowAvailability(rows: Row<Column>[], data: Object): boolean {
    ...
    if (
      this.parent.frozenRows &&
      !this.parent.infiniteScrollSettings.enableCache &&
      this.parent.sortSettings.columns &&
      this.requestType === 'add'
    ) {
      const key: string = this.parent.getPrimaryKeyFieldNames()[0];
      for (let i: number = 0; i < rows.length; i++) {
        if (
          rows[parseInt(i.toString(), 10)].data[`${key}`] === data[`${key}`]
        ) {
          resume = false;
          break;
        }
  ...
  private resetRowIndex(
    ...
    const isRemove: boolean = !(
      rowElms.length % this.parent.pageSettings.pageSize
    );
    if (isAdd) {
      if (isRemove) {
      ...
      rows.filter((e: Row<Column>, index: number) => {
        if (
          e.data[`${keyField}`] ===
          (<{ data?: Object[] }>args).data[0][`${keyField}`]
        ) {
          if (isFrozen && !this.parent.groupSettings.columns.length) {
            const page: number = Math.ceil(
              (index + 1) / this.parent.pageSettings.pageSize
            );
    ...
    resetRowIndex(
      this.parent,
      rows,
      rowElms as HTMLTableRowElement[],
      startIndex,
      this.addRowIndex ? this.addRowIndex : 0
    );
  }

  private resetInfiniteCurrentViewData(page: number, index: number): void {
    index = index - (page - 1) * this.parent.pageSettings.pageSize;
    this.infiniteCurrentViewData[parseInt(page.toString(), 10)].splice(
      index,
      1
    );
  ...
  private swapCurrentViewData(page: number, isAdd: boolean): void {
    ...
    for (let i: number = page; i < end; i++) {
      if (this.infiniteCurrentViewData[i + 1]) {
        ...
        const index: number = isAdd
          ? this.infiniteCurrentViewData[parseInt(i.toString(), 10)].length - 1
          : 0;
        const data: Object[] = this.infiniteCurrentViewData[
          parseInt(pageIndex.toString(), 10)
        ].splice(index, 1);
        if (isAdd) {
          this.infiniteCurrentViewData[i + 1] = data.concat(
            this.infiniteCurrentViewData[i + 1]
          );
          if (i + 1 === end - 1) {
            this.infiniteCurrentViewData[i + 1].splice(
              this.infiniteCurrentViewData[i + 1].length - 1,
              1
            );
          }
        } else {
          this.infiniteCurrentViewData[parseInt(i.toString(), 10)].push(
            data[0]
          );
        }
      }
  ...
  private setDisplayNone(args: {
    ...
    if (this.parent.infiniteScrollSettings.enableCache) {
      ...
      for (let i: number = 1; i <= keys.length; i++) {
        const cache: Row<Column>[] =
          this.infiniteCache[parseInt(i.toString(), 10)];
  ...
  private refreshInfiniteCacheRowVisibleLength(
    args: { [x: number]: Row<Column>[] },
    currentPage: number
  ): number {
    const cPageRowArray: Row<Column>[] =
      args[parseInt(currentPage.toString(), 10)];
    if (
      ...
      for (let i: number = 0; i < cPageRowArray.length; i++) {
        if (
          cPageRowArray[parseInt(i.toString(), 10)].visible ||
  ...
  private updateCurrentViewRecords(data: Object): void {
    ...
    if (!isNullOrUndefined(index)) {
      this.parent.getCurrentViewRecords()[parseInt(index.toString(), 10)] =
        data;
    }
  }
  ...
  private actionComplete(args: NotifyArgs): void {
    if (
      ...
      this.isCancel =
        args.requestType === 'cancel' || args.requestType === 'save';
      this.isAdd = this.isEdit =
        false || this.parent.editSettings.showAddNewRow;
      if (this.isNormaledit) {
        this.editRowIndex = this.empty as number;
        this.virtualInfiniteData = {};
        (<{ previousVirtualData?: Object }>(
  ...
  private resetInfiniteEdit(): void {
    if (this.parent.enableInfiniteScrolling && this.isNormaledit) {
      if (
        (this.parent.editSettings.allowEditing && this.isEdit) ||
  ...
  private getVirtualInfiniteData(data: {
    virtualData: Object;
    isAdd: boolean;
    isCancel: boolean;
  }): void {
  ...
  private editActionBegin(e: { data: Object; index: number }): void {
    ...
    e.data = Object.keys(this.virtualInfiniteData).length
      ? this.virtualInfiniteData
      : rowObject;
  }
  ...
  private infiniteScrollHandler(e: {
    target: HTMLElement;
    isLeft: boolean;
  }): void {
    ...
    if (isInfinite && this.parent.enableInfiniteScrolling && !e.isLeft) {
      ...
      const rows: Element[] = this.parent.groupSettings.enableLazyLoading
        ? captionRows
        : this.parent.getRows();
      if (!rows.length) {
        return;
      ...
      const round: number = Math.round(
        targetEle.scrollHeight - targetEle.scrollTop
      );
      let floor: number =
        offset < targetEle.clientHeight
          ? Math.ceil(offset)
          : Math.floor(offset);
      ...
      const isBottom: boolean =
        floor === targetEle.clientHeight || round === targetEle.clientHeight;
      ...
      if (
        ...
        const rows: Element[] = [].slice.call(
          scrollEle.querySelectorAll('.e-row:not(.e-addedrow)')
        );
      ...
      if (
        ...
        const row: Element = [].slice.call(
          scrollEle.getElementsByClassName(literals.row)
        );
        const rowIndex: number = getRowIndexFromElement(
          row[this.parent.pageSettings.pageSize - 1]
        );
        const startIndex: number =
          getRowIndexFromElement(row[0]) - this.parent.pageSettings.pageSize;
        this.parent.pageSettings.currentPage =
          Math.ceil(rowIndex / this.parent.pageSettings.pageSize) - 1;
  ...
  private makeRequest(args: InfiniteScrollArgs): void {
    if (this.parent.pageSettings.currentPage !== args.prevPage) {
      const initBlocks: number =
        this.parent.infiniteScrollSettings.initialBlocks;
      if (
        initBlocks < this.maxPage &&
        this.parent.pageSettings.currentPage <= this.maxPage
      ) {
  ...
  private editPageQuery(query: Query): void {
    ...
    if (isCache) {
      ...
      if (
        !isNullOrUndefined(
          this.parent.getDataModule().dataManager.dataSource.url
        ) &&
        this.parent.getDataModule().dataManager.dataSource.url !== '' &&
  ...
  private intialPageQuery(query: Query): void {
    if (
      this.parent.infiniteScrollSettings.enableCache &&
      this.parent.infiniteScrollSettings.initialBlocks >
        this.parent.infiniteScrollSettings.maxBlocks
    ) {
      this.parent.infiniteScrollSettings.initialBlocks =
        this.parent.infiniteScrollSettings.maxBlocks;
    }
  ...
  private scrollToLastFocusedCell(e: CellFocusArgs): void {
    const gObj: IGrid = this.parent;
    const rowIdx: number =
      this.lastFocusInfo.rowIdx +
      (e.keyArgs.action === literals.upArrow ? -1 : 1);
    const cellIdx: number = this.lastFocusInfo.cellIdx;
    let row: HTMLTableRowElement = gObj.getRowByIndex(
      rowIdx
    ...
    if (!row) {
      ...
      const page: number =
        Math.floor(rowIdx / this.parent.pageSettings.pageSize) + 1;
      ...
      gObj
        ...
        .querySelector('table')
        .appendChild(
          gObj.createElement('tbody', { attrs: { role: 'rowgroup' } })
        );
      let focusRows: Row<Column>[] = [];
      // eslint-disable-next-line @typescript-eslint/tslint/config
      for (
        let i: number = page === 1 || this.maxPage === page ? 0 : -1, k = 0;
        k < gObj.infiniteScrollSettings.maxBlocks;
        this.maxPage === page ? i-- : i++, k++
      ) {
        ...
        if (rows) {
          ...
          for (let j: number = 0; j < rows.length; j++) {
            gObj
              ...
              .querySelector('tbody')
              .appendChild(
                rowRenderer.render(rows[parseInt(j.toString(), 10)], cols)
              );
          }
        }
      }
  ...
  private setLastCellFocusInfo(e: CellFocusArgs): void {
    const cell: Element = ((e.byClick && e.clickArgs.target) ||
      (e.byKey && e.keyArgs.target) ||
    ...
    if (cell && cell.classList.contains('e-rowcell')) {
      ...
      const rowIdx: number = parseInt(
        cell.parentElement.getAttribute('data-rowindex'),
        10
      );
  ...
  private infiniteCellFocus(e: CellFocusArgs): void {
    ...
    if (e.byKey) {
      ...
      const totalRowsCount: number =
        this.maxPage * gObj.pageSettings.pageSize - 1;
      const visibleRowCount: number = Math.floor(
        (content as HTMLElement).offsetHeight / this.parent.getRowHeight()
      );
      const contentRect: ClientRect = content.getBoundingClientRect();
      if (!isNaN(rowIndex)) {
        if (
          e.keyArgs.action === literals.downArrow ||
          e.keyArgs.action === literals.enter
        ) {
          ...
          if (
            ...
            this.isFocusScroll = false;
            content.scrollTop =
              (rowIndex - visibleRowCount + 1) * this.parent.getRowHeight();
          } else if (!cache && row) {
            if (
              rowRect &&
          ...
          e.keyArgs.action === literals.upArrow ||
          e.keyArgs.action === literals.shiftEnter
        ) {
          this.rowIndex = rowIndex -= 1;
          const row: Element = gObj.getRowByIndex(rowIndex);
          const rowRect: ClientRect = row && row.getBoundingClientRect();
      ...
      (e as KeyboardEventArgs).key === 'PageDown' ||
      (e as KeyboardEventArgs).key === 'PageUp'
    ) {
  ...
  private getVirtualInfiniteEditedData(): void {
    ...
    if (
      ...
      const rowData: Object = editForm
        ? extend({}, this.getEditedRowObject().data)
        : extend({}, this.emptyRowData);
      this.virtualInfiniteData = this.parent.editModule.getCurrentEditedData(
        gridForm,
        rowData
      );
      ...
      for (let i: number = 0; i < hiddenColumn.length; i++) {
        if (hiddenColumn[parseInt(i.toString(), 10)].defaultValue) {
          this.virtualInfiniteData[
            hiddenColumn[parseInt(i.toString(), 10)].field
  ...
  private restoreInfiniteEdit(): void {
    ...
    const frozenEdit: boolean = this.parent.frozenRows
      ? this.editRowIndex >= this.parent.frozenRows
      : true;
    if (
      this.isNormaledit &&
      this.parent.infiniteScrollSettings.enableCache &&
      ...
      if (
        ...
        !isNullOrUndefined(this.editRowIndex)
      ) {
        const row: HTMLTableRowElement = this.parent.getRowByIndex(
          this.editRowIndex
        ) as HTMLTableRowElement;
        if (
          Object.keys(this.virtualInfiniteData).length &&
          row &&
          !this.parent.getContent().querySelector('.' + literals.editedRow)
        ) {
  ...
  private restoreInfiniteAdd(): void {
    ...
    if (
      ...
      this.parent.getRowByIndex(0) &&
      this.isNormaledit &&
      this.parent.infiniteScrollSettings.enableCache &&
      this.isAdd &&
      !content.querySelector('.' + literals.addedRow)
    ) {
  ...
  private appendInfiniteRows(e: {
    ...
    const isInfiniteScroll: boolean =
      this.parent.enableInfiniteScrolling &&
      e.args.requestType === 'infiniteScroll';
    if (
      ...
      const isChildGrid: boolean =
        ...
        this.parent.element.querySelectorAll('.e-childgrid').length
          ? true
          : false;
    ...
    if (!e.args.isFrozen) {
      this.rowTop = !this.rowTop
        ? this.parent.getRows()[0].getBoundingClientRect().top
        : this.rowTop;
      if (isInfiniteScroll) {
  ...
  private selectNewRow(args: { direction: string }): void {
    ...
    if (row && this.keys.some((value: string) => value === this.pressedKey)) {
      ...
      const target: HTMLTableCellElement = (row as HTMLTableRowElement).cells[
        this.cellIndex
      ];
      if (
        this.pressedKey === literals.downArrow ||
        this.pressedKey === literals.enter ||
        (cache &&
          (this.pressedKey === literals.upArrow ||
            this.pressedKey === literals.shiftEnter))
      ) {
        if (
          !cache &&
          this.pressedKey !== literals.upArrow &&
      ...
      this.lastFocusInfo &&
      (this.pressedKey === literals.pageDown ||
        this.pressedKey === literals.pageUp)
    ) {
      const idx: number = cache ? 0 : this.lastFocusInfo.rowIdx;
      if (gObj.getRowByIndex(idx)) {
        const target: Element = gObj.getCellFromIndex(
          idx,
          this.lastFocusInfo.cellIdx
        );
        if (target) {
  ...
  private removeInfiniteCacheRows(e: { args: InfiniteScrollArgs }): void {
    ...
    if (
      ...
      if (e.args.direction === 'down') {
        ...
        const addRowCount: number = this.parent.element.querySelector(
          '.' + literals.addedRow
        )
          ? 0
          : 1;
        this.removeTopRows(
          rows,
          this.parent.pageSettings.pageSize - addRowCount
        );
      }
  ...
  private calculateScrollTop(args: InfiniteScrollArgs): number {
    ...
    if (args.direction === 'down') {
      ...
      const captionRows: Element[] = [].slice.call(
        this.parent
          .getContent()
          .firstElementChild.querySelectorAll('tr:not(.e-row)')
      );
      let captionCount: number = 0;
      if (this.isInitialCollapse && !isNullOrUndefined(captionRows)) {
        captionCount = Math.round(
          (captionRows.length - 1) / this.parent.groupSettings.columns.length
        );
      }
      const value: number = captionCount
        ? captionCount
        : this.parent.pageSettings.pageSize *
          (this.parent.infiniteScrollSettings.maxBlocks - 1);
    ...
    if (args.direction === 'up') {
      if (
        ...
        const len: number = this.refreshInfiniteCacheRowVisibleLength(
          this.infiniteCache,
          this.parent.pageSettings.currentPage
        );
        top = len * this.parent.getRowHeight();
      } else if (this.isInitialCollapse) {
        const groupedData: Row<Column>[] =
          this.infiniteCache[this.parent.pageSettings.currentPage];
        let count: number = 0;
        ...
        top +=
          Math.round(count / this.parent.groupSettings.columns.length) *
          this.parent.getRowHeight();
      } else {
        top +=
          this.parent.pageSettings.pageSize * this.parent.getRowHeight() +
          getScrollBarWidth();
      }
    }
    return top;
  ...
  private captionRowHeight(): number {
    const rows: Element[] = [].slice.call(
      this.parent.getContent().querySelectorAll('tr:not(.e-row)')
    );
  ...
  private removeBottomRows(
    ...
    if (this.infiniteCache[args.prevPage].length < pageSize) {
      cnt =
        this.parent.pageSettings.pageSize -
        this.infiniteCache[args.prevPage].length;
    }
  ...
  private removeCaptionRows(rows: Element[], args: InfiniteScrollArgs): void {
    const rowElements: Element[] = [].slice.call(
      this.parent.getContent().getElementsByClassName(literals.row)
    );
    if (args.direction === 'down') {
      const lastRow: Element =
        rowElements[this.parent.pageSettings.pageSize - 1];
    ...
    if (args.direction === 'up') {
      const lastIndex: number = getRowIndexFromElement(
        rowElements[rowElements.length - 1]
      );
      const page: number = Math.ceil(
        lastIndex / this.parent.pageSettings.pageSize
      );
      let startIndex: number = 0;
      for (
        let i: number = this.parent.pageSettings.currentPage + 1;
        i < page;
        i++
      ) {
  ...
  private resetInfiniteBlocks(
    ...
    if (
      !this.initialRender &&
      !isNullOrUndefined((this.parent as Grid).infiniteScrollModule) &&
      isInfiniteScroll
    ) {
      if (
        this.actions.some((value: string) => value === args.requestType) ||
        isDataModified ||
        (args.requestType === 'save' &&
          (this.parent.sortSettings.columns.length ||
            this.parent.filterSettings.columns.length ||
        ...
        (<{ isRemove?: boolean }>(this.parent as Grid).contentModule).isRemove =
          this.isRemove;
        (<{ isAddRows?: boolean }>(
          (this.parent as Grid).contentModule
        )).isAddRows = this.isRemove;
        (<{ visibleRows?: Row<Column>[] }>(
          (this.parent as Grid).contentModule
        )).visibleRows = [];
        (<{ visibleFrozenRows?: Row<Column>[] }>(
          (this.parent as Grid).contentModule
        )).visibleFrozenRows = [];
      }
    }
  ...
  private setCache(e: {
    ...
    if (
      ...
      if (e.isInfiniteScroll && !this.isRemove) {
        this.isRemove =
          (currentPage - 1) % this.parent.infiniteScrollSettings.maxBlocks ===
          0;
        (<{ isRemove?: boolean }>(this.parent as Grid).contentModule).isRemove =
          this.isRemove;
      }
  ...
  private setInitialCache(
    ...
    args?: InfiniteScrollArgs,
    isEdit?: boolean,
    isCurrentViewData?: boolean
  ): void {
    let k: number = !isEdit
      ? 1
      : isNullOrUndefined(this.firstBlock)
      ? 1
      : this.firstBlock;
    for (
      let i: number = 1;
      i <= this.parent.infiniteScrollSettings.initialBlocks;
      i++
    ) {
      ...
      if (
        ...
        if (isCurrentViewData) {
          this.infiniteCurrentViewData[parseInt(k.toString(), 10)] = data.slice(
            startIndex,
            endIndex
          );
        } else {
          this.infiniteCache[parseInt(k.toString(), 10)] = data.slice(
            startIndex,
            endIndex
          );
          this.resetContentModuleCache(this.infiniteCache);
  ...
  private setInitialGroupCache(
    ...
    for (let i: number = startIndex; i < data.length && k < eIndex; i++) {
      if (
        data[parseInt(i.toString(), 10)].index < eIndex ||
        data[parseInt(i.toString(), 10)].isCaptionRow
      ) {
        k = data[parseInt(i.toString(), 10)].isCaptionRow
          ? k
          : data[parseInt(i.toString(), 10)].index;
        pageData.push(data[parseInt(i.toString(), 10)]);
      }
      if (
        data[parseInt(i.toString(), 10)].index >= eIndex ||
        data[parseInt(i.toString(), 10)].index === eIndex - 1
      ) {
        break;
      }
    }
    this.infiniteCache[parseInt(index.toString(), 10)] =
      pageData as Row<Column>[];
    this.resetContentModuleCache(this.infiniteCache);
  }

  private resetContentModuleCache(
    data: { [x: number]: Row<Column>[] } | { [x: number]: Row<Column>[][] }
  ): void {
    (<
      {
        infiniteCache?:
    ...
    >(this.parent as Grid).contentModule).infiniteCache = data;
  }
  ...
  public resetInfiniteCache(rowObjects: Row<Column>[]): void {
    ...
    for (let i: number = 1; i <= blockLength; i++) {
      ...
      if (
        ...
        this.infiniteCache[parseInt(i.toString(), 10)] = rowObjects.slice(
          startIndex,
          endIndex
        );
        this.resetContentModuleCache(this.infiniteCache);
