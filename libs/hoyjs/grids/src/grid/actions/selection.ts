...
import { remove, closest, select } from '@hoyui/base';
import { Query, DataManager } from '@hoyui/data';
import {
  IGrid,
  IAction,
  IIndex,
...
export class Selection implements IAction {
  ...
  private fDataUpdate(args: {
    ...
    if (
      ...
      (!isNullOrUndefined(args.cellIndex) || !isNullOrUndefined(args.rowIndex))
    ) {
      const rowObj: Row<Column> = this.getRowObj(
        isNullOrUndefined(args.rowIndex)
          ? isNullOrUndefined(args.cellIndex)
            ? this.currentIndex
  ...
  public destroy(): void {
    ...
    if (
      !gridElement ||
      (!gridElement.querySelector('.' + literals.gridHeader) &&
    ...
    return (
      (this.parent.editSettings.mode === 'Normal' ||
        (this.parent.editSettings.mode === 'Batch' &&
          ...
          !this.parent.editModule.formObj.validate())) &&
      this.parent.isEdit &&
  ...
  public getCurrentBatchRecordChanges(): Object[] {
    ...
    if (gObj.editSettings.mode === 'Batch' && gObj.editModule) {
      let currentRecords: Object[] = iterateExtend(
        this.parent.getCurrentViewRecords()
      );
      currentRecords =
        gObj.editSettings.newRowPosition === 'Bottom'
          ? currentRecords.concat(
              this.parent.editModule.getBatchChanges()[literals.addedRecords]
            )
          : this.parent.editModule
              .getBatchChanges()
              [literals.addedRecords].concat(currentRecords);
      const deletedRecords: Object[] =
        this.parent.editModule.getBatchChanges()[literals.deletedRecords];
      ...
      for (let i: number = 0; i < deletedRecords.length; i++) {
        for (let j: number = 0; j < currentRecords.length; j++) {
          if (
            deletedRecords[parseInt(i.toString(), 10)][`${primaryKey}`] ===
            currentRecords[parseInt(j.toString(), 10)][`${primaryKey}`]
          ) {
      ...
      const selectedGroupedData: Object[] = gObj
        .getCurrentViewRecords()
        .filter((col: Column) => col['key'] === undefined);
  ...
  public selectRow(index: number, isToggle?: boolean): void {
    if (
      ...
      this.selectionSettings.enableSimpleMultiRowSelection
    ) {
    ...
    const rowObj: Row<Column> =
      selectedRow &&
      gObj.getRowObjectFromUID(selectedRow.getAttribute('data-uid'));
    if (
    ...
    if (gObj.enableVirtualization && index > -1) {
      const e: { selectedIndex: number; isAvailable: boolean } = {
        selectedIndex: index,
        isAvailable: true,
      };
      ...
      if (
        ...
        selectData = gObj.getRowObjectFromUID(
          selectedRow.getAttribute('data-uid')
        ).data;
      } else {
    ...
    if (!isToggle || !this.selectedRowIndexes.length) {
      ...
      const isCheckboxModeResetOnRowClick: boolean =
        this.selectionSettings.checkboxMode === 'ResetOnRowClick';
      const isSelectionTypeMultiple: boolean =
        !this.parent.isCheckBoxSelection &&
        this.selectionSettings.type === 'Multiple';
      if (
        (!isCheckboxModeResetOnRowClick && !isSelectionTypeMultiple) ||
        (this.selectedRowIndexes.length === 1 &&
          (isCheckboxModeResetOnRowClick || isSelectionTypeMultiple))
      ) {
        isToggle = !(this.isKeyAction && this.parent.isCheckBoxSelection)
    ...
    if (!isToggle) {
      ...
      this.parent.trigger(
        events.rowSelecting,
        this.fDataUpdate(args),
        this.rowSelectingCallBack(
          args,
          isToggle,
          index,
          selectData,
  ...
  private selectedDataUpdate(
    ...
    rowIndexes?: number[],
    selectableRowIndex?: number[]
  ): void {
    for (let i: number = 0, len: number = rowIndexes.length; i < len; i++) {
      let currentRow: Element =
        this.parent.getDataRows()[rowIndexes[parseInt(i.toString(), 10)]];
      if (this.parent.enableVirtualization) {
        currentRow = this.parent.getRowByIndex(
          rowIndexes[parseInt(i.toString(), 10)]
        );
      }
      ...
      if (rowObj && rowObj.isDataRow && rowObj.isSelectable) {
        ...
        if (this.isPartialSelection && selectableRowIndex) {
          selectableRowIndex.splice(
            selectableRowIndex.indexOf(rowIndexes[parseInt(i.toString(), 10)]),
            1
          );
        }
      }
  ...
  public selectRows(rowIndexes: number[]): void {
    ...
    const rowIndex: number = !this.isSingleSel()
      ? rowIndexes[0]
    ...
    this.parent.trigger(
      ...
      (args: Object) => {
        ...
        const selectRowFn: Function = (
          index: number,
          preventFocus?: boolean
        ) => {
          this.updateRowSelection(
            gObj.getRowByIndex(index),
            index,
            preventFocus
          );
  ...
  public addRowsToSelection(rowIndexes: number[]): void {
    ...
    const selectedRow: Element = !this.isSingleSel()
      ? gObj.getRowByIndex(rowIndexes[0])
      : gObj.getRowByIndex(rowIndexes[rowIndexes.length - 1]);
    if (
      (!this.isRowType() || this.isEditing()) &&
      !this.selectionSettings.checkboxOnly
    ) {
      return;
    }
    let args: Object;
    const checkboxColumn: Column[] = this.parent
      .getColumns()
      .filter((col: Column) => col.type === 'checkbox');
    ...
    for (const rowIndex of rowIndexes) {
      ...
      if (
        isUnSelected &&
        ((checkboxColumn.length ? true : this.selectionSettings.enableToggle) ||
        ...
        this.selectedRowIndexes.splice(
          this.selectedRowIndexes.indexOf(rowIndex),
          1
        );
        this.selectedRecords.splice(
          this.selectedRecords.indexOf(selectedRow),
          1
        );
        this.selectRowIndex(
          this.selectedRowIndexes.length
            ? this.selectedRowIndexes[this.selectedRowIndexes.length - 1]
            : -1
        );
        selectedRow.removeAttribute('aria-selected');
        this.addRemoveClassesForRow(
          selectedRow,
          false,
          null,
        ...
        args = {
          ...
          isCtrlPressed: this.isMultiCtrlRequest,
          isShiftPressed: this.isMultiShiftRequest,
          foreignKeyData: foreignKeyData.length
            ? foreignKeyData
            : rowObj.foreignKeyData,
          isInteracted: this.isInteracted,
          isHeaderCheckboxClicked: this.isHeaderCheckboxClicked,
  ...
  private getCollectionFromIndexes(
    ...
    let { i, max }: { i: number; max: number } =
      startIndex <= endIndex
        ? { i: startIndex, max: endIndex }
        : { i: endIndex, max: startIndex };
    for (; i <= max; i++) {
      indexes.push(i);
    }
  ...
  private clearSelectedRow(index: number): void {
    if (this.toggle) {
      ...
      this.selectedRecords.splice(
        this.selectedRecords.indexOf(this.parent.getRowByIndex(index)),
        1
      );
    }
  }
  ...
  private getPkValue(pkField: string, data: object): string {
    return pkField
      ? isComplexField(pkField)
        ? getObject(pkField, data)
        : data[`${pkField}`]
      : data[`${pkField}`];
  }
    ...
    if (
      (this.parent.isPersistSelection ||
        (this.parent.selectionSettings.persistSelection &&
          this.parent.getPrimaryKeyFieldNames().length > 0)) &&
      !isNullOrUndefined(selectedRow)
    ) {
      if (!this.parent.isPersistSelection) {
        this.ensureCheckboxFieldSelection();
      }
      const rowObj: Row<Column> = this.getRowObj(selectedRow);
      const pKey: string = rowObj.data
        ? this.getPkValue(this.primaryKey, rowObj.data)
        : null;
      if (pKey === null) {
        return;
      }
      ...
      if (
        (chkState && !this.isPartialSelection) ||
        (this.isPartialSelection && rowObj.isSelectable && rowObj.isSelected)
      ) {
        this.selectedRowState[`${pKey}`] = chkState;
        delete this.unSelectedRowState[`${pKey}`];
        if (
          !this.persistSelectedData.some(
            (data: Object) => this.getPkValue(this.primaryKey, data) === pKey
          )
        ) {
  ...
  private updatePersistDelete(
    ...
    let index: number;
    const isPresent: boolean = this.persistSelectedData.some(
  ...
  private updateCheckBoxes(
    ...
    if (!isNullOrUndefined(row)) {
      const chkBox: HTMLInputElement = row.querySelector(
        '.e-checkselect'
      ) as HTMLInputElement;
      if (!isNullOrUndefined(chkBox)) {
        removeAddCboxClasses(
          chkBox.nextElementSibling as HTMLElement,
          chkState
        );
        setChecked(chkBox, chkState);
        if (
          isNullOrUndefined(this.checkedTarget) ||
          (!isNullOrUndefined(this.checkedTarget) &&
  ...
  private updateRowSelection(
    ...
    if (!this.preventFocus) {
      let target: Element = this.focus.getPrevIndexes().cellIndex
        ? (<HTMLTableRowElement>selectedRow).cells[
            this.focus.getPrevIndexes().cellIndex
          ]
        : selectedRow.querySelector(
            '.e-selectionbackground:not(.e-hide):not(.e-detailrowcollapse):not(.e-detailrowexpand)'
          );
  ...
  public clearSelection(): void {
    ...
    if (
      this.selectionSettings.persistSelection &&
      this.persistSelectedData.length
    ) {
      this.deSelectedData = iterateExtend(this.persistSelectedData);
    }
    if (
      !this.parent.isPersistSelection ||
      (this.parent.isPersistSelection && !this.parent.isEdit) ||
      (!isNullOrUndefined(this.checkedTarget) &&
        this.checkedTarget.classList.contains('e-checkselectall'))
    ) {
      const span: Element = this.parent.element
        .querySelector('.e-gridpopup')
        .querySelector('span');
  ...
  public clearRowSelection(): void {
    if (this.isRowSelected) {
      ...
      for (
        let i: number = 0, len: number = this.selectedRowIndexes.length;
        i < len;
        i++
      ) {
      ...
      this.rowDeselect(
        ...
        () => {
          if (
            this.isCancelDeSelect &&
            (this.isRowClicked ||
              this.checkSelectAllClicked ||
              (this.isInteracted && !this.parent.isPersistSelection))
          ) {
            if (this.parent.isPersistSelection) {
              if (
                this.getCheckAllStatus(
                  this.parent.element.querySelector('.e-checkselectall')
                ) === 'Intermediate' ||
                this.parent.isPersistSelection
              ) {
                for (let i: number = 0; i < this.selectedRecords.length; i++) {
                  this.updatePersistCollection(
                    this.selectedRecords[parseInt(i.toString(), 10)],
                    true
                  );
                }
              } else {
          ...
          const element: HTMLElement[] = [].slice.call(
            rows.filter((record: HTMLElement) =>
              record.hasAttribute('aria-selected')
            )
          );
          for (let j: number = 0; j < element.length; j++) {
            if (!this.disableUI) {
              element[parseInt(j.toString(), 10)].removeAttribute(
                'aria-selected'
              );
              this.addRemoveClassesForRow(
                element[parseInt(j.toString(), 10)],
                false,
                true,
                'e-selectionbackground',
                'e-active'
  ...
  private rowDeselect(
    ...
    target: Element,
    mRow?: Element[],
    rowDeselectCallBack?: Function,
    frozenRightRow?: Element[]
  ): void {
    if (
      (this.selectionSettings.persistSelection &&
        (this.isRowClicked ||
          ...
          (this.focus['activeKey'] &&
            this.focus.currentInfo.element.classList.contains('e-gridchkbox') &&
            this.focus['activeKey'] === 'space'))) ||
      !this.selectionSettings.persistSelection
    ) {
      const cancl: string = 'cancel';
      const isSingleDeSel: boolean =
        rowIndex.length === 1 && this.deSelectedData.length === 1;
      ...
      const isHeaderCheckBxClick: boolean =
        this.actualTarget &&
        !isNullOrUndefined(closest(this.actualTarget, 'thead'));
      ...
      this.parent.trigger(type, rowDeselectObj, (args: Object) => {
  ...
  private getRowObj(row: Element | number = this.currentIndex): Row<Column> {
    ...
    if (row) {
      return (
        this.parent.getRowObjectFromUID(row.getAttribute('data-uid')) ||
        ({} as Row<Column>)
      );
    }
    return {} as Row<Column>;
  ...
  public selectCell(cellIndex: IIndex, isToggle?: boolean): void {
    ...
    const gObj: IGrid = this.parent;
    let args: Object;
    const selectedCell: Element = gObj.getCellFromIndex(
      cellIndex.rowIndex,
      this.getColIndex(cellIndex.rowIndex, cellIndex.cellIndex)
    );
    this.currentIndex = cellIndex.rowIndex;
    const selectedData: Object =
      this.getCurrentBatchRecordChanges()[this.currentIndex];
    if (!this.isCellType() || !selectedCell || this.isEditing()) {
      return;
    }
    const isCellSelected: boolean = selectedCell.classList.contains(
      'e-cellselectionbackground'
    );
    isToggle = !isToggle
      ? isToggle
      : !isUndefined(this.prevCIdxs) &&
        cellIndex.rowIndex === this.prevCIdxs.rowIndex &&
        cellIndex.cellIndex === this.prevCIdxs.cellIndex &&
        isCellSelected;
    ...
    if (!isToggle) {
      args = {
        ...
        previousRowCell: this.prevECIdxs
          ? this.getCellIndex(
              this.prevECIdxs.rowIndex,
              this.prevECIdxs.cellIndex
            )
          : undefined,
        cancel: false,
      };
  ...
  private successCallBack(
    ...
    return (cellSelectingArgs: Object) => {
      ...
      const currentCell: string = 'currentCell';
      if (
        !isNullOrUndefined(cellSelectingArgs) &&
        cellSelectingArgs[`${cncl}`] === true
      ) {
        return;
      }
      if (!isToggle) {
        cellSelectingArgs[`${currentCell}`] = cellSelectingArgs[
          `${currentCell}`
        ]
          ? cellSelectingArgs[`${currentCell}`]
          : selectedCell;
      }
      this.clearCell();
      if (!isToggle) {
  ...
  public selectCellsByRange(startIndex: IIndex, endIndex?: IIndex): void {
    ...
    const gObj: IGrid = this.parent;
    let selectedCell: Element = this.parent.isSpan
      ? getCellFromRow(this.parent, startIndex.rowIndex, startIndex.cellIndex)
    ...
    const args: Object = {
      ...
      isShiftPressed: this.isMultiShiftRequest,
      previousRowCell: this.prevECIdxs
        ? this.getCellIndex(this.prevECIdxs.rowIndex, this.prevECIdxs.cellIndex)
        : undefined,
    };
    const previousRowCellIndex: string = 'previousRowCellIndex';
    args[`${previousRowCellIndex}`] = this.prevECIdxs;
    this.parent.trigger(
      ...
      (cellSelectingArgs: Object) => {
        ...
        for (let i: number = startIndex.rowIndex; i <= endIndex.rowIndex; i++) {
          ...
          cellIndexes = [];
          for (
            let j: number = min < max ? min : max,
              len: number = min > max ? min : max;
            j <= len;
            j++
          ) {
            selectedCell = this.parent.isSpan
              ? getCellFromRow(gObj, i, j)
              : gObj.getCellFromIndex(i, j);
            if (!selectedCell) {
              continue;
            }
            cellIndexes.push(j);
        ...
        const cellSelectedArgs: Object = {
          data: selectedData,
          cellIndex: edIndex,
          currentCell: gObj.getCellFromIndex(
            edIndex.rowIndex,
            edIndex.cellIndex
          ),
          selectedRowCellIndex: this.selectedRowCellIndexes,
  ...
  public selectCells(rowCellIndexes: ISelectedCell[]): void {
    ...
    let selectedCell: Element = gObj.getCellFromIndex(
      rowCellIndexes[0].rowIndex,
      rowCellIndexes[0].cellIndexes[0]
    );
    ...
    for (let i: number = 0, len: number = rowCellIndexes.length; i < len; i++) {
      for (
        let j: number = 0,
          cellLen: number =
            rowCellIndexes[parseInt(i.toString(), 10)].cellIndexes.length;
        j < cellLen;
        j++
      ) {
        selectedCell = gObj.getCellFromIndex(
          rowCellIndexes[parseInt(i.toString(), 10)].rowIndex,
          rowCellIndexes[parseInt(i.toString(), 10)].cellIndexes[
  ...
  public addCellsToSelection(cellIndexes: IIndex[]): void {
    ...
    if (gObj.groupSettings.columns.length > 0) {
      rowObj = gObj.getRowObjectFromUID(
        this.target.parentElement.getAttribute('data-uid')
      );
    }
    const foreignKeyData: Object[] = [];
    for (const cellIndex of cellIndexes) {
      for (
        let i: number = 0, len: number = this.selectedRowCellIndexes.length;
        i < len;
        i++
      ) {
        if (
          this.selectedRowCellIndexes[parseInt(i.toString(), 10)].rowIndex ===
          cellIndex.rowIndex
        ) {
          index = i;
          break;
      ...
      if (gObj.groupSettings.columns.length > 0) {
        foreignKeyData.push(
          rowObj.cells[idx + gObj.groupSettings.columns.length].foreignKeyData
        );
      } else {
        foreignKeyData.push(
          rowObj.cells[parseInt(idx.toString(), 10)].foreignKeyData
        );
      }
      const args: Object = {
        cancel: false,
      ...
      if (isUnSelected) {
        const selectedCellIdx: number[] =
          this.selectedRowCellIndexes[parseInt(index.toString(), 10)]
            .cellIndexes;
        if (
          selectedCellIdx.indexOf(cellIndex.cellIndex) > -1 ||
          (this.selectionSettings.mode === 'Both' &&
            selectedCell.classList.contains('e-gridchkbox') &&
          ...
          selectedCellIdx.splice(
            selectedCellIdx.indexOf(cellIndex.cellIndex),
            1
          );
    ...
    if (cells) {
      for (let m: number = 0; m < cells.length; m++) {
        const colIndex: number = parseInt(
          cells[parseInt(m.toString(), 10)].getAttribute(literals.dataColIndex),
          10
        );
        if (colIndex === index) {
          return m;
  ...
  private getLastColIndex(rowIndex: number): number {
    const cells: NodeListOf<Element> = this.parent
      .getDataRows()
      [parseInt(rowIndex.toString(), 10)].querySelectorAll('td.e-rowcell');
    return parseInt(
      cells[cells.length - 1].getAttribute(literals.dataColIndex),
      10
    );
  }
  ...
  private addRowCellIndex(rowCellIndex: IIndex): void {
    ...
    if (isRowAvail) {
      if (
        ...
        ].cellIndexes.indexOf(rowCellIndex.cellIndex) < 0
      ) {
        this.selectedRowCellIndexes[
          parseInt(index.toString(), 10)
        ].cellIndexes.push(rowCellIndex.cellIndex);
      }
    } else {
      this.selectedRowCellIndexes.push({
  ...
  public clearCellSelection(): void {
    if (this.isCellSelected) {
      ...
      for (let i: number = 0, len: number = rowCell.length; i < len; i++) {
        data.push(
          currentViewData[rowCell[parseInt(i.toString(), 10)].rowIndex]
        );
        const rowObj: Row<Column> = this.getRowObj(
        ...
        for (
          let j: number = 0,
            cLen: number =
              rowCell[parseInt(i.toString(), 10)].cellIndexes.length;
          j < cLen;
          j++
        ) {
          if (rowObj.cells) {
            foreignKeyData.push(
              rowObj.cells[
                rowCell[parseInt(i.toString(), 10)].cellIndexes[
                  parseInt(j.toString(), 10)
                ]
              ].foreignKeyData
            );
      ...
      for (
        let i: number = 0, len: number = selectedCells.length;
        i < len;
        i++
      ) {
        selectedCells[parseInt(i.toString(), 10)].classList.remove(
          'e-cellselectionbackground'
        );
        selectedCells[parseInt(i.toString(), 10)].removeAttribute(
          'aria-selected'
        );
      }
      if (this.bdrElement) {
  ...
  private getSelectedCellsElement(): Element[] {
    ...
    for (let i: number = 0, len: number = rows.length; i < len; i++) {
      cells = cells.concat(
        [].slice.call(
          rows[parseInt(i.toString(), 10)].getElementsByClassName(
            'e-cellselectionbackground'
          )
        )
      );
  ...
  private mouseMoveHandler(e: MouseEventArgs): void {
      ...
      if (!target) {
        target = closest(
          document.elementFromPoint(
            this.parent.element.offsetLeft + 2,
            e.clientY
          ),
          'tr'
        );
    ...
    if (target && !e.ctrlKey && !e.shiftKey) {
      const rowIndex: number = parseInt(
        target.getAttribute(literals.dataRowIndex),
        10
      );
      if (!this.isCellDrag) {
        this.hideAutoFill();
        ...
        if (td) {
          ...
          if (rowIndex > -1) {
            this.selectLikeExcel(
              e,
              rowIndex,
              parseInt(td.getAttribute(literals.dataColIndex), 10)
            );
          }
        }
      }
  ...
  private updateScrollPosition(
    ...
    if (
      ...
      position.x -
        20 -
        (this.parent.enableRtl && this.parent.height !== 'auto'
          ? getScrollBarWidth()
          : 0)
    ) {
      ...
      this.timer1 = window.setInterval(() => {
        this.setScrollPosition(
          scrollElement.firstElementChild,
          this.parent.enableRtl ? 'left' : 'right',
          mouseEvent
        );
      }, 200);
    }
    if (clientRect.top >= position.y - this.parent.getRowHeight() * 0.5) {
      ...
      clientRect.top +
        ...
        this.parent.getRowHeight() * 0.5 -
        (scrollElement.firstElementChild.scrollWidth >
        (scrollElement.firstElementChild as HTMLElement).offsetWidth
          ? getScrollBarWidth()
          : 0) <=
      position.y
    ) {
  ...
  private setScrollPosition(
    ...
    if (this.endAFCell || this.prevECIdxs) {
      rowIndex = this.endAFCell
        ? parseInt(this.endAFCell.getAttribute('index'), 10)
        : this.prevECIdxs.rowIndex;
      columnIndex = this.endAFCell
        ? parseInt(this.endAFCell.getAttribute('data-colindex'), 10)
        : this.prevECIdxs.cellIndex;
    }
    switch (direction) {
      case 'up':
        if (
          mouseEvent &&
          closest(mouseEvent.target as Element, '.e-headercontent')
        ) {
          return;
        }
        if (
          ...
          ((this.selectedRowCellIndexes.length === 1 &&
            this.startAFCell !== this.startCell) ||
            (this.selectedRowCellIndexes.length > 1 &&
              this.startAFCell.getBoundingClientRect().top > 0))
        ) {
        ...
        rowIndex < 1
          ...
          : (scrollElement.scrollTop -= (
              this.parent.getRowByIndex(rowIndex) as HTMLElement
            ).offsetHeight);
        break;
      case 'down':
        if (
        ...
        if (rowIndex < this.parent.getRows().length - 1) {
          ...
          if (
            ...
            this.startAFCell = this.parent.getCellFromIndex(
              rowIndex,
              this.selectedRowCellIndexes[0].cellIndexes[0]
            );
          }
          scrollElement.scrollTop += (
            this.parent.getRowByIndex(rowIndex) as HTMLElement
      ...
      case 'left':
        if (columnIndex > 0 && rowIndex > -1) {
          if (
            ...
            this.startAFCell &&
            this.selectedRowCellIndexes.length &&
            ((this.selectedRowCellIndexes[0].cellIndexes.length > 0 &&
              this.startAFCell !== this.startCell) ||
              (this.selectedRowCellIndexes[0].cellIndexes.length > 1 &&
                ((!this.parent.enableRtl &&
                  this.startAFCell.getBoundingClientRect().left > 0) ||
                  (this.parent.enableRtl &&
                    this.startAFCell.getBoundingClientRect().left <
                      this.parent.element.offsetWidth))))
          ) {
            columnIndex = parseInt(
              this.startAFCell.getAttribute('data-colindex'),
              10
            );
          }
          const nextElement: HTMLElement = this.findNextCell(
            scrollElement,
            direction,
            columnIndex,
            rowIndex
          );
          columnIndex = nextElement
            ? parseInt(nextElement.getAttribute('data-colindex'), 10)
            : -1;
      ...
      case 'right':
        ...
        const currentElement: HTMLElement = this.parent.getCellFromIndex(
          rowIndex,
          columnIndex
        ) as HTMLElement;
        // eslint-disable-next-line no-case-declarations
        ...
        if (
          ...
          this.startAFCell = this.parent.getCellFromIndex(
            this.selectedRowCellIndexes[0].rowIndex,
            parseInt(nextElement.getAttribute('data-colindex'), 10)
          );
        }
        columnIndex = nextElement
        ...
        if (this.parent.enableRtl && nextElement) {
          // eslint-disable-next-line @typescript-eslint/no-unused-expressions
          columnIndex < this.parent.columns.length - 1
            ? (scrollElement.scrollLeft -= currentElement.offsetWidth)
            : (scrollElement.scrollLeft = -scrollElement.scrollWidth);
        } else if (nextElement) {
          // eslint-disable-next-line @typescript-eslint/no-unused-expressions
          columnIndex < this.parent.columns.length - 1
        ...
        if (
          ...
          scrollElement.scrollLeft = this.parent.enableRtl
    ...
    if (
      rowIndex > -1 &&
      rowIndex < this.parent.getRows().length &&
      columnIndex > -1
    ) {
      ...
      if (
        this.isAutoFillSel &&
        mouseEvent.target.classList.contains('e-cellselectionbackground') &&
        ((direction === 'down' &&
          parseInt(mouseEvent.target.getAttribute('index'), 10) ===
            this.parent.getRows().length - 1) ||
          (direction === 'right' &&
            parseInt(mouseEvent.target.getAttribute('data-colindex'), 10) ===
              this.parent.columns.length - 1))
      ) {
        return;
      }
  ...
  private findNextCell(
    ...
    let nextElement: HTMLElement = this.parent.getCellFromIndex(
      rowIndex,
      direction === 'left' ? columnIndex - 1 : columnIndex + 1
    ) as HTMLElement;
    if (nextElement && nextElement.classList.contains('e-hide')) {
      const siblingEles: NodeListOf<Element> = nextElement
        .closest('tr')
        .querySelectorAll('.e-rowcell:not(.e-hide)');
      const nextEleInd: number = Array.from(siblingEles).indexOf(
        nextElement.nextElementSibling
      );
      if (nextEleInd > 0 && nextEleInd < siblingEles.length - 1) {
        nextElement = siblingEles[
          nextEleInd + (direction === 'left' ? -1 : 1)
        ] as HTMLElement;
        return nextElement;
      } else {
        scrollElement.scrollLeft = 0;
  ...
  private setFrozenBorders(
    ...
    const cells: HTMLElement[] = [].slice
      .call(
        parentEle.querySelectorAll(
          '.e-cellselectionbackground' + '.' + strCell[`${bdrStr}`]
        )
      )
      .filter((ele: HTMLElement) => ele.style.display === '');
    const fixedCells: HTMLElement[] = [].slice
      .call(
        parentEle.querySelectorAll('.e-cellselectionbackground.e-fixedfreeze')
      )
      .filter((ele: HTMLElement) => ele.style.display === '');
    ...
    if (cells.length) {
      ...
      const lastColIdx: string =
        cells[cells.length - 1].getAttribute('data-colindex');
      for (let i: number = 0; i < cells.length; i++) {
        if (
          cells[parseInt(i.toString(), 10)].getAttribute('index') ===
            firstRowIdx &&
          (width.length === 1 ||
            (width.length === 3 && parseInt(width[0], 10) === 2) ||
            (width.length === 4 && parseInt(width[0], 10) === 2))
        ) {
          cells[parseInt(i.toString(), 10)].classList.add('e-xlsel-top-border');
        }
        if (
          cells[parseInt(i.toString(), 10)].getAttribute('data-colindex') ===
            firstColIdx &&
          (width.length === 1 ||
            (width.length === 3 && parseInt(width[1], 10) === 2) ||
            (width.length === 4 &&
              ((!isRtl && parseInt(width[3], 10) === 2) ||
                (isRtl && parseInt(width[1], 10) === 2))))
        ) {
          cells[parseInt(i.toString(), 10)].classList.add(
            isRtl ? 'e-xlsel-right-border' : 'e-xlsel-left-border'
          );
        }
        if (
          cells[parseInt(i.toString(), 10)].getAttribute('index') ===
            lastRowIdx &&
          (width.length === 1 ||
            (width.length === 3 && parseInt(width[2], 10) === 2) ||
            (width.length === 4 && parseInt(width[2], 10) === 2))
        ) {
          cells[parseInt(i.toString(), 10)].classList.add(
        ...
        if (
          ...
          (width.length === 1 ||
            (width.length === 3 && parseInt(width[1], 10) === 2) ||
            (width.length === 4 && !isRtl && parseInt(width[1], 10) === 2) ||
            (isRtl && parseInt(width[3], 10) === 2))
        ) {
          cells[parseInt(i.toString(), 10)].classList.add(
            isRtl ? 'e-xlsel-left-border' : 'e-xlsel-right-border'
          );
        }
      }
    }
    if (fixedCells.length) {
      ...
      const lastRowIdx: string =
        fixedCells[fixedCells.length - 1].getAttribute('index');
      const lastColIdx: string =
        fixedCells[fixedCells.length - 1].getAttribute('data-colindex');
      for (let i: number = 0; i < fixedCells.length; i++) {
        const idx: string =
          fixedCells[parseInt(i.toString(), 10)].getAttribute('index');
        const colIdx: string =
          fixedCells[parseInt(i.toString(), 10)].getAttribute('data-colindex');
        if (
          idx === firstRowIdx &&
          ((!this.parent
            .getHeaderContent()
            .querySelector('.e-cellselectionbackground.e-fixedfreeze') &&
            parentsUntil(parentEle, 'e-content')) ||
            !parentsUntil(parentEle, 'e-content'))
        ) {
          fixedCells[parseInt(i.toString(), 10)].classList.add(
            'e-xlsel-top-border'
          );
        }
        if (
          ...
          ((!this.parent
            .getContent()
            .querySelector('.e-cellselectionbackground.e-fixedfreeze') &&
            parentsUntil(parentEle, 'e-headercontent')) ||
            !parentsUntil(parentEle, 'e-headercontent'))
        ) {
          fixedCells[parseInt(i.toString(), 10)].classList.add(
            'e-xlsel-bottom-border'
        ...
        const preCell: Element =
          fixedCells[parseInt(i.toString(), 10)].parentElement.children[
            parseInt(colIdx, 10) - 1
          ];
        if (
          colIdx === firstColIdx &&
          (!preCell ||
            (preCell &&
              !preCell.classList.contains('e-cellselectionbackground')))
        ) {
          fixedCells[parseInt(i.toString(), 10)].classList.add(
            isRtl ? 'e-xlsel-right-border' : 'e-xlsel-left-border'
          );
        ...
        if (
          ...
          (!nextCell ||
            (nextCell &&
              !nextCell.classList.contains('e-cellselectionbackground')))
        ) {
          fixedCells[parseInt(i.toString(), 10)].classList.add(
            isRtl ? 'e-xlsel-left-border' : 'e-xlsel-right-border'
          );
  ...
  private isLastCell(cell: Element): boolean {
    const cells: Element[] = [].slice.call(
      cell.parentElement.querySelectorAll('.e-rowcell:not(.e-hide)')
    );
    return cells[cells.length - 1] === cell;
  }

  private isLastRow(cell: Element): boolean {
    const rows: Element[] = [].slice.call(
      closest(cell, literals.tbody).querySelectorAll('.e-row:not(.e-hiddenrow)')
    );
  ...
  private setBorders(
    ...
    let cells: HTMLElement[] = [].slice
      .call(parentEle.getElementsByClassName('e-cellselectionbackground'))
      .filter((ele: HTMLElement) => ele.style.display === '');
    ...
    if (cells.length) {
      ...
      if (
        start.offsetParent.classList.contains('e-content') ||
        start.offsetParent.classList.contains('e-headercontent')
      ) {
        parentOff = start.offsetParent
          .querySelector('table')
      ...
      const leftOffset: number =
        isFrozen &&
        (bdrStr === '2' || bdrStr === '4') &&
        this.isFirstCell(start)
          ? 1
          : 0;
      const rightOffset: number =
        ((this.parent.getFrozenMode() === 'Right' &&
          (bdrStr === '1' || bdrStr === '3')) ||
          (this.parent.getFrozenMode() === literals.leftRight &&
            (bdrStr === '5' || bdrStr === '6'))) &&
        this.isFirstCell(start)
      ...
      if (this.parent.enableRtl) {
        ...
        border.style.left =
          stOff.left - parentOff.left - leftOffset - rightOffset + 'px';
        border.style.width =
          endOff.right - stOff.left + leftOffset - rightOffset + 1 + 'px';
      }
      border.style.top = stOff.top - parentOff.top - topOffSet + 'px';
      border.style.height =
        endOff.top - stOff.top > 0
          ? endOff.top -
            parentOff.top +
            endOff.height +
            (isFrozen ? 0 : 1) -
            (stOff.top - parentOff.top) -
            rowHeight +
            topOffSet +
            'px'
  ...
  private applyBothFrozenBorders(str: string): void {
    ...
    switch (str.length) {
      case 6:
        {
          this.bdrElement.style.borderWidth = rtl
            ? this.right_bottom
            : this.bottom_left;
          this.mcBdrElement.style.borderWidth = this.bottom;
          this.fhBdrElement.style.borderWidth = rtl
            ? this.top_right
            : this.top_left;
          this.mhBdrElement.style.borderWidth = this.top;
          this.frcBdrElement.style.borderWidth = rtl
            ? this.bottom_left
            : this.right_bottom;
          this.frhBdrElement.style.borderWidth = rtl
            ? this.top_left
            : this.top_right;
        }
        break;
      case 4:
        {
          if (
            ...
            this.fhBdrElement.style.borderWidth = rtl
              ? this.top_right
              : this.top_left;
            this.mhBdrElement.style.borderWidth = rtl
              ? this.top_left
              : this.top_right;
            this.bdrElement.style.borderWidth = rtl
              ? this.right_bottom
              : this.bottom_left;
            this.mcBdrElement.style.borderWidth = rtl
              ? this.bottom_left
              : this.right_bottom;
          }
          if (
      ...
      case 3:
        {
          this.bdrElement.style.borderWidth = rtl
            ? this.top_right_bottom
            : this.top_bottom_left;
    ...
    if (this.parent.getFrozenMode() === literals.leftRight) {
      ...
      switch (str.length) {
        ...
        case 2:
          {
            if (this.parent.getFrozenMode() === 'Right') {
              this.bdrElement.style.borderWidth = str.includes('2')
                ? rtl
                  ? this.top_bottom_left
                  : this.top_right_bottom
                : this.right_bottom_left;
              this.mcBdrElement.style.borderWidth = str.includes('1')
                ? rtl
                  ? this.top_right_bottom
                  : this.top_bottom_left
                : this.right_bottom_left;
              if (this.parent.frozenRows) {
                this.fhBdrElement.style.borderWidth = str.includes('1')
                  ? this.top_right_left
                  : rtl
                  ? this.top_bottom_left
                  : this.top_right_bottom;
                this.mhBdrElement.style.borderWidth = str.includes('2')
                  ? this.top_right_left
                  : rtl
                  ? this.top_right_bottom
                  : this.top_bottom_left;
  ...
  private createBorders(): void {
    if (!this.bdrElement) {
      ...
      if (this.parent.isFrozenGrid()) {
        ...
        if (this.parent.getFrozenMode() === literals.leftRight) {
          this.frcBdrElement = this.parent
            .getContentTable()
            .parentElement.appendChild(
              createElement('div', {
                className: 'e-xlsel',
      ...
      if (this.parent.frozenRows && this.parent.isFrozenGrid()) {
        ...
        if (this.parent.getFrozenMode() === literals.leftRight) {
          this.frhBdrElement = this.parent
            .getHeaderTable()
            .parentElement.appendChild(
              createElement('div', {
                className: 'e-xlsel',
  ...
  private showHideBorders(display: string, freeze?: boolean): void {
    if (this.bdrElement) {
      ...
      if (this.parent.isFrozenGrid()) {
        ...
        removeClass(
          parentEle.querySelectorAll('.e-xlsel-top-border'),
          'e-xlsel-top-border'
        );
        removeClass(
          parentEle.querySelectorAll('.e-xlsel-left-border'),
          'e-xlsel-left-border'
        );
        removeClass(
          parentEle.querySelectorAll('.e-xlsel-right-border'),
          'e-xlsel-right-border'
        );
        removeClass(
          parentEle.querySelectorAll('.e-xlsel-bottom-border'),
          'e-xlsel-bottom-border'
        );
        if (!freeze) {
  ...
  private positionAFBorders(): void {
    ...
    const firstCellTop: number =
      endOff.top - stOff.top >= 0 &&
      (parentsUntil(this.startAFCell, literals.movableContent) ||
        parentsUntil(this.startAFCell, 'e-frozencontent')) &&
      this.isFirstRow(this.startAFCell)
        ? 1.5
        : 0;
    const firstCellLeft: number =
      (parentsUntil(this.startAFCell, literals.movableContent) ||
        parentsUntil(this.startAFCell, literals.movableHeader)) &&
      this.isFirstCell(this.startAFCell)
        ? 1
        : 0;
    const rowHeight: number =
      this.isLastRow(this.endAFCell) &&
      (parentsUntil(this.endAFCell, literals.movableContent) ||
        parentsUntil(this.endAFCell, 'e-frozencontent'))
        ? 2
        : 0;
    const parentOff: ClientRect = (
      this.startAFCell as HTMLElement
    ).offsetParent.getBoundingClientRect();
    const parentRect: ClientRect = this.parent.element.getBoundingClientRect();
    const sTop: number = (this.startAFCell as HTMLElement).offsetParent
      .parentElement.scrollTop;
    const sLeft: number = (this.startAFCell as HTMLElement).offsetParent
      .parentElement.scrollLeft;
    let scrollTop: number = sTop - (this.startAFCell as HTMLElement).offsetTop;
    let scrollLeft: number =
      sLeft - (this.startAFCell as HTMLElement).offsetLeft;
    let totalHeight: number =
      this.parent.element.clientHeight - getScrollBarWidth();
    if (this.parent.allowPaging) {
      totalHeight -= (
        this.parent.element.querySelector('.e-pager') as HTMLElement
      ).offsetHeight;
    }
    if (this.parent.aggregates.length) {
      totalHeight -= (this.parent.getFooterContent() as HTMLElement)
        .offsetHeight;
    }
    const totalWidth: number =
      this.parent.element.clientWidth -
      (this.parent.height !== 'auto' ? getScrollBarWidth() : 0);
    ...
    if (!this.parent.enableRtl) {
      ...
      this.bdrAFRight.style.left =
        endOff.left - parentRect.left - 2 + endOff.width + 'px';

      this.bdrAFRight.style.width =
        totalWidth <= parseInt(this.bdrAFRight.style.left, 10) ? '0px' : '2px';
      this.bdrAFTop.style.left = left + scrollLeft - 0.5 + 'px';
      this.bdrAFTop.style.width =
        parseInt(this.bdrAFRight.style.left, 10) -
        parseInt(this.bdrAFLeft.style.left, 10) -
        firstCellLeft +
        1 +
        'px';
      if (
        totalWidth <=
        parseInt(this.bdrAFTop.style.width, 10) +
          parseInt(this.bdrAFTop.style.left, 10)
      ) {
        const leftRemove: number =
          parseInt(this.bdrAFTop.style.width, 10) +
          parseInt(this.bdrAFTop.style.left, 10) -
          totalWidth;
        this.bdrAFTop.style.width =
          parseInt(this.bdrAFTop.style.width, 10) - leftRemove + 'px';
      }
    } else {
      const scrolloffSet: number =
        ...
        parentsUntil(this.startAFCell, literals.movableHeader)
          ? stOff.right -
            (
              this.startAFCell as HTMLElement
            ).offsetParent.parentElement.getBoundingClientRect().width -
            parentRect.left
          : 0;
      this.bdrAFLeft.style.right =
        parentRect.right - endOff.right - 2 + endOff.width + 'px';

      this.bdrAFLeft.style.width =
        totalWidth <= parseInt(this.bdrAFLeft.style.right, 10) ? '0px' : '2px';
      const borderAFRightValue: number =
        parentRect.right - stOff.right - firstCellLeft + scrolloffSet - 1;
      // eslint-disable-next-line @typescript-eslint/no-unused-expressions
      borderAFRightValue > 0
        ? (this.bdrAFRight.style.right = borderAFRightValue + 'px')
        : (this.bdrAFRight.style.right = '0px');
      this.bdrAFTop.style.left = endOff.left - parentRect.left - 0.5 + 'px';
      this.bdrAFTop.style.width =
        parseInt(this.bdrAFLeft.style.right, 10) -
        parseInt(this.bdrAFRight.style.right, 10) -
        firstCellLeft +
        1 +
        'px';
    ...
    this.bdrAFLeft.style.top =
      stOff.top - parentRect.top - firstCellTop + scrollTop + 'px';
    this.bdrAFLeft.style.height =
      endOff.top - stOff.top > 0
        ? endOff.top -
          parentOff.top +
          endOff.height +
    ...
    this.bdrAFRight.style.height =
      parseInt(this.bdrAFLeft.style.height, 10) + 'px';

    this.bdrAFTop.style.top = this.bdrAFRight.style.top;

    this.bdrAFBottom.style.left = this.bdrAFTop.style.left;
    this.bdrAFBottom.style.top =
      parseFloat(this.bdrAFLeft.style.top) +
      parseFloat(this.bdrAFLeft.style.height) -
      top -
      1 +
      'px';
    this.bdrAFBottom.style.width =
      totalHeight <= parseFloat(this.bdrAFBottom.style.top)
        ? '0px'
        : this.bdrAFTop.style.width;
    if (
      totalHeight <=
      parseInt(this.bdrAFLeft.style.height, 10) +
        parseInt(this.bdrAFLeft.style.top, 10)
    ) {
      const topRemove: number =
        parseInt(this.bdrAFLeft.style.height, 10) +
        parseInt(this.bdrAFLeft.style.top, 10) -
        totalHeight;
      this.bdrAFLeft.style.height =
        parseInt(this.bdrAFLeft.style.height, 10) - topRemove + 'px';
      this.bdrAFRight.style.height =
        parseInt(this.bdrAFLeft.style.height, 10) + 'px';
    }
  }
  ...
  private updateValue(rIdx: number, cIdx: number, cell: HTMLElement): void {
    const args: BeforeAutoFillEventArgs = this.createBeforeAutoFill(
      rIdx,
      cIdx,
      cell
    );
  ...
  private getAutoFillCells(
    ...
    const cells: HTMLElement[] = [].slice.call(
      this.parent
        .getDataRows()
        [parseInt(rowIndex.toString(), 10)].querySelectorAll(cls)
    );
    return cells;
  }

  private selectLikeAutoFill(e: MouseEvent, isApply?: boolean): void {
    const startrowIdx: number = parseInt(
      parentsUntil(this.startAFCell, literals.row).getAttribute(
        literals.dataRowIndex
      ),
      10
    );
    const startCellIdx: number = parseInt(
      this.startAFCell.getAttribute(literals.dataColIndex),
      10
    );
    let endrowIdx: number = parseInt(
      parentsUntil(this.endAFCell, literals.row).getAttribute(
        literals.dataRowIndex
      ),
      10
    );
    let endCellIdx: number = parseInt(
      this.endAFCell.getAttribute(literals.dataColIndex),
      10
    );
    ...
    switch (
      ...
      case !isApply &&
        this.endAFCell.classList.contains('e-cellselectionbackground') &&
        !!parentsUntil(e.target as Element, literals.rowCell):
        this.startAFCell = this.parent.getCellFromIndex(
          startrowIdx,
          startCellIdx
        );
        this.endAFCell = this.parent.getCellFromIndex(
          startrowIdx + rowLen,
          startCellIdx + colLen
        );
        this.drawAFBorders();
        break;
      case this.autoFillRLselection &&
        startCellIdx + colLen < endCellIdx && //right
        endCellIdx - startCellIdx - colLen + 1 >
          endrowIdx - startrowIdx - rowLen && // right bottom
        ...
        if (!isApply) {
          ...
          const cellIdx: number = parseInt(
            this.endCell.getAttribute(literals.dataColIndex),
            10
          );
      ...
      case this.autoFillRLselection &&
        ...
        if (!isApply) {
          ...
          for (let i: number = startrowIdx; i <= startrowIdx + rowLen; i++) {
            ...
            let c: number = 0;
            for (
              let j: number = this.startCellIndex - 1;
              j >= endCellIdx;
              j--
            ) {
              if (c > colLen) {
      ...
      case startrowIdx > endrowIdx: //up
        ...
        if (!isApply) {
          ...
          const trIdx: number = parseInt(
            this.endCell.parentElement.getAttribute(literals.dataRowIndex),
            10
          );
          let r: number = trIdx;
          for (let i: number = startrowIdx - 1; i >= endrowIdx; i--) {
            ...
            r--;
            for (
              let j: number = this.startCellIndex;
              j <= this.startCellIndex + colLen;
              j++
            ) {
              this.updateValue(i, j, cells[parseInt(c.toString(), 10)]);
              c++;
      ...
      default: //down
        ...
        if (!isApply) {
          ...
          for (let i: number = trIdx + 1; i <= endrowIdx; i++) {
            ...
            let c: number = 0;
            for (
              let m: number = this.startCellIndex;
              m <= this.startCellIndex + colLen;
              m++
            ) {
          ...
          this.selectCellsByRange(
            { rowIndex: trIdx - rowLen, cellIndex: startCellIdx },
            { rowIndex: endrowIdx, cellIndex: startCellIdx + colLen }
          );
        }
        break;
  ...
  private mouseUpHandler(e: MouseEventArgs): void {
    ...
    if (
      ...
      closeRowCell &&
      closeRowCell.classList.contains(literals.rowCell)
    ) {
      const rowIndex: number = parseInt(
        closeRowCell.parentElement.getAttribute(literals.dataRowIndex),
        10
      );
      const cellIndex: number = parseInt(
        closeRowCell.getAttribute(literals.dataColIndex),
        10
      );
    ...
    if (this.isAutoFillSel) {
      this.preventFocus = true;
      const lastCell: Element = parentsUntil(
        e.target as Element,
        literals.rowCell
      );
      this.endAFCell = lastCell
        ? lastCell
        : this.endCell === this.endAFCell
        ? this.startAFCell
        : this.endAFCell;
      this.startAFCell = this.startCell;
      if (
        !isNullOrUndefined(this.endAFCell) &&
        !isNullOrUndefined(this.startAFCell)
      ) {
  ...
  public updateAutoFillPosition(): void {
    if (
      this.parent.enableAutoFill &&
      !this.parent.isEdit &&
      this.selectionSettings.cellSelectionMode.indexOf('Box') > -1 &&
      !this.isRowType() &&
      !this.isSingleSel() &&
      this.selectedRowCellIndexes.length
    ) {
      const index: number = parseInt(
        this.target.getAttribute(literals.dataColIndex),
        10
      );
      const rindex: number = parseInt(this.target.getAttribute('index'), 10);
      const rowIndex: number =
        this.selectedRowCellIndexes[this.selectedRowCellIndexes.length - 1]
          .rowIndex;
      const cells: Element[] = this.getAutoFillCells(rowIndex, index).filter(
        (ele: HTMLElement) => ele.style.display === ''
      );
      const col: Column = this.parent.getColumnByIndex(index);
      const isFrozenCol: boolean = col.getFreezeTableName() === 'movable';
      const isFrozenRow: boolean = rindex < this.parent.frozenRows;
      const isFrozenRight: boolean =
        this.parent.getFrozenMode() === literals.leftRight &&
        col.getFreezeTableName() === literals.frozenRight;
      if (
        !select(
          '#' + this.parent.element.id + '_autofill',
          parentsUntil(this.target, literals.table)
        )
      ) {
        if (
          select(
        ...
        this.autofill.style.display = 'none';
        if (
          this.target.classList.contains('e-leftfreeze') ||
          this.target.classList.contains('e-rightfreeze') ||
        ...
        if (!isFrozenRow) {
          if (!isFrozenCol) {
            this.parent
              .getContentTable()
              .parentElement.appendChild(this.autofill);
          } else {
            this.parent
              .getContentTable()
              .parentElement.appendChild(this.autofill);
      ...
      if (cell && cell.offsetParent) {
        ...
        if (
          cell.offsetParent.classList.contains('e-content') ||
          cell.offsetParent.classList.contains('e-headercontent')
        ) {
          parentOff = cell.offsetParent
            .querySelector('table')
            .getBoundingClientRect();
        }
  ...
  private mouseDownHandler(e: MouseEventArgs): void {
    ...
    if (
      (gridElement && gridElement.id !== gObj.element.id) ||
      (parentsUntil(target, literals.headerContent) &&
        !this.parent.frozenRows) ||
      parentsUntil(target, 'e-editedbatchcell') ||
      parentsUntil(target, literals.editedRow)
    ) {
      return;
    }
    ...
    if (parentsUntil(target, literals.rowCell) && !e.shiftKey && !e.ctrlKey) {
      if (
        gObj.selectionSettings.cellSelectionMode.indexOf('Box') > -1 &&
        !this.isRowType() &&
        !this.isSingleSel()
      ) {
        ...
        gObj.allowRowDragAndDrop &&
        !gObj.isEdit &&
        !this.parent.selectionSettings.checkboxOnly
      ) {
        this.isRowDragSelected = false;
        if (
          !this.isRowType() ||
          this.isSingleSel() ||
          closest(target, 'td').classList.contains('e-selectionbackground')
        ) {
          this.isDragged = false;
          return;
        }
    ...
    this.updateStartEndCells();
    if (
      target.classList.contains('e-autofill') ||
      target.classList.contains('e-xlsel')
    ) {
      this.isCellDrag = true;
  ...
  private updateStartEndCells(): void {
    const cells: Element[] = [].slice.call(
      this.parent.element.getElementsByClassName('e-cellselectionbackground')
    );
    this.startCell = cells[0];
    this.endCell = cells[cells.length - 1];
    if (this.startCell) {
      this.startIndex = parseInt(
        this.startCell.parentElement.getAttribute(literals.dataRowIndex),
        10
      );
      this.startCellIndex = parseInt(
        parentsUntil(this.startCell, literals.rowCell).getAttribute(
          literals.dataColIndex
        ),
        10
      );
    }
  ...
  private enableDrag(e: MouseEventArgs, isUpdate?: boolean): void {
    ...
    if (isUpdate) {
      ...
      this.startDCellIndex = parseInt(
        parentsUntil(e.target as Element, literals.rowCell).getAttribute(
          literals.dataColIndex
        ),
        10
      );
  ...
  private clearSelAfterRefresh(e: { requestType: string }): void {
    const isInfiniteScroll: boolean =
      this.parent.enableInfiniteScrolling && e.requestType === 'infiniteScroll';
    if (
    ...
    if (
      (e.requestType === 'virtualscroll' || isInfiniteScroll) &&
      this.parent.isPersistSelection &&
      this.isPartialSelection &&
      this.isHdrSelectAllClicked
    ) {
      const rowObj: Row<Column>[] = this.parent
        .getRowsObject()
        .filter((e: Row<Column>) => e.isSelectable);
      const indexes: number[] = [];
      this.selectedRowState = {};
      this.persistSelectedData = [];
      for (let i: number = 0; i < rowObj.length; i++) {
        indexes.push(rowObj[parseInt(i.toString(), 10)].index);
        const pkValue: string = this.getPkValue(
          this.primaryKey,
          rowObj[parseInt(i.toString(), 10)].data
        );
  ...
  public removeEventListener(): void {
    ...
    this.parent.off(
      events.destroyAutoFillElements,
      this.destroyAutoFillElements
    );
  }
  ...
  private rowsRemoved(e: { records: Object[] }): void {
    for (let i: number = 0; i < e.records.length; i++) {
      const pkValue: string = this.getPkValue(
        this.primaryKey,
        e.records[parseInt(i.toString(), 10)]
      );
  ...
  private getCheckAllBox(): HTMLInputElement {
    return this.parent
      .getHeaderContent()
      .querySelector('.e-checkselectall') as HTMLInputElement;
  }
  ...
  private onPropertyChanged(e: {
    ...
    if (
      !isNullOrUndefined(e.properties.mode) ||
  ...
  private hidePopUp(): void {
    if (
      this.parent.element
        .querySelector('.e-gridpopup')
        .getElementsByClassName('e-rowselect').length
    ) {
      (
        this.parent.element.querySelector('.e-gridpopup') as HTMLElement
      ).style.display = 'none';
    }
  }
  ...
  private checkBoxSelectionChanged(): void {
    ...
    const checkboxColumn: Column[] = gobj
      .getColumns()
      .filter((col: Column) => col.type === 'checkbox');
  ...
  private ensureCheckboxFieldSelection(): void {
    ...
    if (
      !gobj.enableVirtualization &&
      this.chkField &&
      ((gobj.isPersistSelection &&
        Object.keys(this.selectedRowState).length === 0) ||
        !gobj.isPersistSelection)
    ) {
      this.dataSuccess(
  ...
  private dataSuccess(res: Object[]): void {
    for (let i: number = 0; i < res.length; i++) {
      const pkValue: string = this.getPkValue(
        this.primaryKey,
        res[parseInt(i.toString(), 10)]
      );
      const chkValue: boolean = res[parseInt(i.toString(), 10)][this.chkField];
      if (
        isNullOrUndefined(this.selectedRowState[`${pkValue}`]) &&
        chkValue &&
        typeof chkValue === 'boolean'
      ) {
  ...
  private setRowSelection(state: boolean): void {
    if (
      ...
      if (state) {
        if (
          ...
          for (const row of rowObj) {
            this.selectedRowState[this.getPkValue(this.primaryKey, row.data)] =
              true;
          }
        } else {
          const selectedData: Object[] = this.isPartialSelection
            ? this.parent.partialSelectedRecords
            : this.getData();
          if (this.parent.groupSettings.columns.length) {
            for (const data of this.isPartialSelection
              ? selectedData
              : (<{ records?: Object[] }>selectedData).records) {
              this.selectedRowState[this.getPkValue(this.primaryKey, data)] =
                true;
            }
          } else {
      ...
      if (state) {
        ...
        const unSelectedRowStateKeys: string[] = Object.keys(
        ...
        if (!this.isCheckboxReset) {
          const rowData: Object[] =
            this.parent.groupSettings.columns.length &&
            this.parent.isPersistSelection
              ? this.parent.currentViewData['records']
              : this.parent.currentViewData;
          for (const data of rowData) {
            if (!isNullOrUndefined(data[this.primaryKey])) {
              const key: string = data[this.primaryKey].toString();
              if (
                selectedStateKeys.indexOf(key) === -1 &&
                unSelectedRowStateKeys.indexOf(key) === -1
              ) {
                this.selectedRowState[data[this.primaryKey]] = true;
              }
            }
  ...
  private getAvailableSelectedData(): object[] {
    let filteredSearchedSelectedData: Object[] = new DataManager(
      this.persistSelectedData
    ).executeLocal(this.parent.getDataModule().generateQuery(true));
    if (
      this.parent.groupSettings.columns.length &&
      filteredSearchedSelectedData &&
      (<{ records?: Object[] }>filteredSearchedSelectedData).records
    ) {
      filteredSearchedSelectedData = (<{ records?: Object[] }>(
        filteredSearchedSelectedData
      )).records.slice();
    }
  ...
  private refreshPersistSelection(): void {
    ...
    if (
      ...
      for (let j: number = 0; j < rows.length; j++) {
        const rowObj: Row<Column> = this.getRowObj(
          rows[parseInt(j.toString(), 10)]
        );
        const pKey: string = rowObj
          ? rowObj.data
            ? this.getPkValue(this.primaryKey, rowObj.data)
            : null
          : null;
        ...
        const chkBox: HTMLInputElement = rows[
          parseInt(j.toString(), 10)
        ].querySelector('.e-checkselect') as HTMLInputElement;
        if (
          this.selectedRowState[`${pKey}`] ||
          (this.parent.checkAllRows === 'Check' &&
            this.selectedRowState[`${pKey}`] &&
            this.totalRecordsCount ===
              Object.keys(this.selectedRowState).length &&
            this.chkAllCollec.indexOf(pKey) < 0) ||
          (this.parent.checkAllRows === 'Uncheck' &&
            this.chkAllCollec.indexOf(pKey) > 0 &&
            !this.parent.selectedRowIndex) ||
          (this.parent.checkAllRows === 'Intermediate' &&
            !isNullOrUndefined(this.chkField) &&
            rowObj.data[this.chkField])
        ) {
          indexes.push(
            parseInt(
              rows[parseInt(j.toString(), 10)].getAttribute(
                literals.dataRowIndex
              ),
              10
            )
    ...
    if (
      (this.parent.isCheckBoxSelection ||
        this.parent.selectionSettings.checkboxMode === 'ResetOnRowClick') &&
  ...
  private actionBegin(e: { requestType: string }): void {
    if (e.requestType === 'save' && this.parent.isPersistSelection) {
      const editChkBox: HTMLInputElement = this.parent.element.querySelector(
        '.e-edit-checkselect'
      ) as HTMLInputElement;
      if (!isNullOrUndefined(editChkBox)) {
        let row: HTMLElement = closest(
          editChkBox,
          '.' + literals.editedRow
        ) as HTMLElement;
        if (row) {
          if (this.parent.editSettings.mode === 'Dialog') {
            row = this.parent.element.querySelector(
              '.e-dlgeditrow'
            ) as HTMLElement;
          }
          ...
          this.selectedRowState[this.getPkValue(this.primaryKey, rowObj.data)] =
            rowObj.isSelected = editChkBox.checked;
        } else {
    ...
    if (e.requestType === 'save' && this.parent.isPersistSelection) {
      if (e.action === 'add') {
        if (this.isCheckedOnAdd) {
          ...
          this.selectedRowState[this.getPkValue(this.primaryKey, rowObj.data)] =
            rowObj.isSelected = this.isCheckedOnAdd;
        }
        this.isHdrSelectAllClicked = false;
        this.setCheckAllState();
      }
  ...
  private onDataBound(): void {
    ...
    this.initialRowSelection =
      this.isRowType() &&
      this.parent.element.querySelectorAll('.e-selectionbackground') &&
      ...
      if (this.parent.checkAllRows === 'Uncheck') {
        ...
        this.persistSelectedData =
          !(
            ...
            ? this.parent.groupSettings.columns.length
              ? (<{ records?: Object[] }>this.getData()).records.slice()
              : this.getData().slice()
            : this.persistSelectedData;
      }
    }
  ...
  private checkSelectAllAction(checkState: boolean): void {
    ...
    const editForm: HTMLFormElement = this.parent.element.querySelector(
      '.e-gridform'
    ) as HTMLFormElement;
    ...
    if (checkState && this.getCurrentBatchRecordChanges().length) {
      ...
      this.selectRowsByRange(
        ...
        cRenderer.getVirtualRowIndex(
          this.getCurrentBatchRecordChanges().length - 1
        )
      );
    } else {
      this.parent.checkAllRows = 'Uncheck';
    ...
    if (!isNullOrUndefined(editForm)) {
      const editChkBox: HTMLElement = editForm.querySelector(
        '.e-edit-checkselect'
      ) as HTMLElement;
      if (!isNullOrUndefined(editChkBox)) {
        removeAddCboxClasses(
          editChkBox.nextElementSibling as HTMLElement,
          checkState
        );
      }
    }
  ...
  private checkSelectAll(checkBox: HTMLInputElement): void {
    ...
    if (stateStr === 'Intermediate') {
      if (!this.chkField && !this.parent.isPersistSelection) {
        state = this.getCurrentBatchRecordChanges().some(
          (data: Object) =>
      ...
      if (
        ...
        for (
          let i: number = 0;
          i < this.getCurrentBatchRecordChanges().length;
          i++
        ) {
          if (
            !isNullOrUndefined(
              this.getPkValue(
                this.primaryKey,
                this.getCurrentBatchRecordChanges()[`${i}`]
              )
            )
          ) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            if (
              (Object.keys(this.selectedRowState) as any).includes(
                this.getPkValue(
                  this.primaryKey,
                  this.getCurrentBatchRecordChanges()[`${i}`]
                ).toString()
  ...
  private checkSelect(checkBox: HTMLInputElement): void {
    const target: HTMLElement = closest(
      this.checkedTarget,
      '.' + literals.rowCell
    ) as HTMLElement;
    const gObj: IGrid = this.parent;
    ...
    if (isGroupAdaptive(gObj)) {
      ...
      if (
        ...
        rIndex = parseInt(
          target.parentElement.getAttribute(literals.dataRowIndex),
          10
        );
      } else {
        rIndex = gObj
          .getRows()
          .map((m: HTMLTableRowElement) => m.getAttribute('data-uid'))
          .indexOf(uid);
      }
    } else {
      rIndex = parseInt(
    ...
    if (
      this.parent.isPersistSelection &&
      this.parent.element.getElementsByClassName(literals.addedRow).length >
        0 &&
    ...
    this.rowCellSelectionHandler(
      rIndex,
      parseInt(target.getAttribute(literals.dataColIndex), 10)
    );
    this.moveIntoUncheckCollection(
      closest(target, '.' + literals.row) as HTMLElement
    );
    this.setCheckAllState();
    this.isMultiCtrlRequest = false;
    this.triggerChkChangeEvent(
      checkBox,
      checkBox.nextElementSibling.classList.contains('e-check')
    );
  }
  ...
  private updateSelectedRowIndex(index?: number): void {
    if (
      ...
      if (this.parent.checkAllRows === 'Check') {
        this.selectedRowIndexes = [];
        const dataLength: number = this.parent.groupSettings.columns.length
          ? this.getData()['records'].length
          : this.getData().length;
  ...
  private isAllSelected(count?: number): boolean {
    if (
      ...
      return (
        this.getAvailableSelectedData().length ===
      ...
      if (this.isPartialSelection) {
        if (
          ...
          this.parent.pageSettings.pageSize <
            this.parent.pageSettings.totalRecordsCount
        ) {
          const data: object[] = this.parent.partialSelectedRecords;
          for (let i: number = 0; i < data.length; i++) {
            const pKey: string = this.getPkValue(
              this.primaryKey,
              data[parseInt(i.toString(), 10)]
            );
        ...
        const data: object[] = this.parent.groupSettings.columns.length
  ...
  private someDataSelected(): boolean {
    if (
      ...
      const filteredSearchedSelectedData: Object[] =
        this.getAvailableSelectedData();
      for (let i: number = 0; i < filteredSearchedSelectedData.length; i++) {
        const pKey: string = this.getPkValue(
          this.primaryKey,
          filteredSearchedSelectedData[parseInt(i.toString(), 10)]
        );
        if (this.selectedRowState[`${pKey}`]) {
          return false;
        }
    ...
    const data: object[] = this.isPartialSelection
      ? this.parent.partialSelectedRecords
      : this.parent.getDataModule().isRemote() ||
        (!isNullOrUndefined(this.parent.dataSource) &&
          (<{ result: object[] }>this.parent.dataSource).result)
    ...
    if (
      ...
      if (input) {
        ...
        if (
          ...
          (!isFiltered &&
            ((checkedLen === this.totalRecordsCount &&
              ...
              (!this.parent.enableVirtualization &&
                !this.parent.enableInfiniteScrolling &&
                this.isPartialSelection &&
                (this.isSelectAllRowCount(checkedLen) ||
                  this.isHdrSelectAllClicked)) ||
              ((this.parent.enableVirtualization ||
                this.parent.enableInfiniteScrolling) &&
                !this.parent.allowPaging &&
                ((!(
                  ...
                  getRecord.length &&
                  checkedLen === getRecord.length) ||
                  ((this.parent.getDataModule().isRemote() ||
                    (!isNullOrUndefined(this.parent.dataSource) &&
                      (<{ result: object[] }>this.parent.dataSource).result)) &&
                    !this.isPartialSelection &&
                    (checkedLen === this.parent.totalDataRecordsCount ||
                      ((this.isSelectAllRowCount(checkedLen) ||
                        checkedLen === this.totalRecordsCount) &&
                        !this.parent.isPersistSelection))) ||
                  (this.isPartialSelection &&
          ...
          (((!this.selectedRowIndexes.length &&
            (!this.parent.enableVirtualization ||
              (!this.persistSelectedData.length && !isFiltered) ||
        ...
        if (this.isPartialSelection) {
          const rowCount: number = this.parent
            .getRowsObject()
            .filter((e: Row<Column>) => e.isSelectable).length;
          if (
            rowCount === 0 &&
  ...
  private isSelectAllRowCount(count?: number): boolean {
    ...
    if (
      ...
      if (
        ...
        rowCount =
          this.parent.partialSelectedRecords.length +
          this.parent.disableSelectedRecords.length;
        if (rowCount === this.totalRecordsCount) {
          return (
            this.parent.partialSelectedRecords.length &&
            count === this.parent.partialSelectedRecords.length
          );
        } else {
          return false;
        }
  ...
  private keyDownHandler(e: KeyboardEvent): void {
    ...
    if (
      (Browser.info.name === 'opera' && e.keyCode === 17) ||
      (Browser.info.name === 'mozilla' && e.keyCode === 224) ||
      ((Browser.info.name === 'safari' || Browser.info.name === 'chrome') &&
        (e.keyCode === 91 || e.keyCode === 93))
    ) {
      this.cmdKeyPressed = true;
    }
    const targetHeadCell: Element = parentsUntil(
      e.target as HTMLElement,
      'e-headercell'
    );
    const targetRowCell: Element = parentsUntil(
      e.target as HTMLElement,
      literals.rowCell
    );
    const isCheckBox: boolean = targetHeadCell
      ? targetHeadCell.children[0].classList.contains('e-headerchkcelldiv')
      : targetRowCell
      ? targetRowCell.classList.contains('e-gridchkbox')
      : false;
  ...
  private clickHandler(e: MouseEvent): void {
    ...
    if (
      parentsUntil(target, literals.row) ||
      parentsUntil(target, 'e-headerchkcelldiv') ||
    ...
    const checkWrap: HTMLElement = parentsUntil(
      target,
      'e-checkbox-wrapper'
    ) as HTMLElement;
    this.checkSelectAllClicked =
      (checkWrap && checkWrap.getElementsByClassName('e-checkselectall')) ||
      (this.selectionSettings.persistSelection &&
        parentsUntil(target, literals.row))
        ? true
        : false;
    ...
    if (
      checkWrap &&
      checkWrap.querySelectorAll('.e-checkselect,.e-checkselectall').length > 0
    ) {
      checkBox = checkWrap.querySelector(
        'input[type="checkbox"]'
      ) as HTMLInputElement;
      chkSelect = true;
    }
    ...
    if (
      ...
      target = this.parent.getCellFromIndex(
        parseInt(target.getAttribute('index'), 10),
        parseInt(target.getAttribute('data-colindex'), 10)
      ) as HTMLElement;
    }
    if (
    ...
    if (
      ((target &&
        target.parentElement.classList.contains(literals.row) &&
        !this.parent.selectionSettings.checkboxOnly) ||
        chkSelect) &&
      !this.isRowDragSelected
    ) {
      ...
      (e.target as HTMLTableCellElement).classList.contains('e-headercell') &&
      !(e.target as HTMLTableCellElement).classList.contains(
        'e-stackedheadercell'
      )
    ) {
      const uid: string = (e.target as HTMLTableCellElement)
        .querySelector('.e-headercelldiv')
        .getAttribute('e-mappinguid');
  ...
  private popUpClickHandler(e: MouseEvent): void {
    const target: Element = e.target as Element;
    if (
      closest(target, '.e-headercell') ||
      (e.target as HTMLElement).classList.contains(literals.rowCell) ||
  ...
  private showPopup(e: MouseEvent): void {
    if (!this.selectionSettings.enableSimpleMultiRowSelection) {
      setCssInGridPopUp(
        <HTMLElement>this.parent.element.querySelector('.e-gridpopup'),
        e,
  ...
  private rowCellSelectionHandler(rowIndex: number, cellIndex: number): void {
    if (
      (!this.isMultiCtrlRequest && !this.isMultiShiftRequest) ||
      this.isSingleSel()
    ) {
      if (!this.isDragged) {
      ...
      if (
        this.parent.isCheckBoxSelection ||
        (!this.parent.isCheckBoxSelection &&
          !closest(this.target, '.' + literals.rowCell).classList.contains(
            literals.gridChkBox
          ))
      ) {
        this.selectRowsByRange(
          isUndefined(this.prevRowIndex) ? rowIndex : this.prevRowIndex,
          rowIndex
        );
      } else {
      ...
      if (this.selectionSettings.mode === 'Both') {
        const checkboxColumn: Column = this.parent
          .getColumns()
          .find((col: Column) => col.type === 'checkbox');
        const checkboxColumnIndexCheck: boolean =
          checkboxColumn && checkboxColumn.index !== cellIndex;
  ...
  private onCellFocused(e: CellFocusArgs): void {
    if (this.parent.frozenRows && e.container.isHeader && e.byKey) {
      if (e.keyArgs.action === 'upArrow') {
        ...
        if (e.keyArgs.action === 'downArrow') {
          const rIdx: number = Number(
            e.element.parentElement.getAttribute(literals.dataRowIndex)
          );
    ...
    const clear: boolean =
      ((e.container.isHeader && e.isJump) ||
        (e.container.isContent && !e.container.isSelectable)) &&
      !(e.byKey && e.keyArgs.action === 'space') &&
      !(
        e.element.classList.contains('e-detailrowexpand') ||
        e.element.classList.contains('e-detailrowcollapse')
      );
    const headerAction: boolean =
      e.container.isHeader &&
      e.element.tagName !== 'TD' &&
      !closest(e.element, '.' + literals.rowCell) &&
      !(e.byKey && e.keyArgs.action === 'space');
    if (!e.byKey || clear) {
    ...
    let [rowIndex, cellIndex]: number[] = e.container.isContent
      ? e.container.indexes
      : e.indexes;
    const prev: IIndex = this.focus.getPrevIndexes();
    if (
      e.element.parentElement.querySelector('.e-rowcelldrag') ||
      e.element.parentElement.querySelector('.e-dtdiagonalright') ||
    ...
    if (this.parent.frozenRows) {
      if (
        ...
        let hdrLength: number = this.parent
          .getHeaderTable()
          .querySelector('thead').childElementCount;
        if (
          this.parent.editSettings.showAddNewRow &&
          this.parent.editSettings.newRowPosition === 'Top' &&
        ...
        rowIndex -= hdrLength;
        prev.rowIndex = !isNullOrUndefined(prev.rowIndex)
          ? prev.rowIndex - hdrLength
          : null;
      } else {
        rowIndex += this.parent.frozenRows;
        prev.rowIndex =
          prev.rowIndex === 0 || !isNullOrUndefined(prev.rowIndex)
            ? prev.rowIndex + this.parent.frozenRows
            : null;
      }
    }
    if (
      ...
      rowIndex = parseInt(
        e.element.parentElement.getAttribute('data-rowindex'),
        10
      );
    }
    if (
      (headerAction ||
        (['ctrlPlusA', 'escape'].indexOf(e.keyArgs.action) === -1 &&
          e.keyArgs.action !== 'space' &&
    ...
    if (
      this.parent.element.classList.contains('e-gridcell-read') &&
      (e.keyArgs.action === 'tab' ||
        e.keyArgs.action === 'shiftTab' ||
        e.keyArgs.action === 'rightArrow' ||
        e.keyArgs.action === 'leftArrow')
    ) {
      let targetLabel: string = this.target.getAttribute('aria-label');
      targetLabel =
        this.target.innerHTML +
        ' column header ' +
        this.parent.getColumnByIndex(cellIndex).field;
    ...
    switch (e.keyArgs.action) {
      ...
      case 'end':
        cellIndex =
          e.keyArgs.action === 'end' ? this.getLastColIndex(rowIndex) : 0;
  ...
  private getKeyColIndex(e: CellFocusArgs): number {
    ...
    const stackedHeader: HTMLElement = (
      e.element as HTMLTableCellElement
    ).querySelector('.e-stackedheadercelldiv');
    if (
      this.selectionSettings.allowColumnSelection &&
      parentsUntil(e.element, 'e-columnheader')
    ) {
      ...
      if (stackedHeader) {
        ...
        const innerColumn: Column[] = this.getstackedColumns(
          this.parent.getColumnByUid(uid).columns as Column[]
        );
        const lastIndex: number = this.parent.getColumnIndexByUid(
          innerColumn[innerColumn.length - 1].uid
        );
        const firstIndex: number = this.parent.getColumnIndexByUid(
          innerColumn[0].uid
        );
        index = this.prevColIndex >= lastIndex ? firstIndex : lastIndex;
      } else {
        index = this.parent.getColumnIndexByUid(
          (e.element as HTMLTableCellElement)
            .querySelector('.e-headercelldiv')
            .getAttribute('e-mappinguid')
        );
  ...
  private applyUpDown(rowIndex: number): void {
    ...
    if (!this.target) {
      this.target =
        this.parent.getRows()[0].children[
          this.parent.groupSettings.columns.length || 0
        ];
    }
    const cIndex: number = parseInt(
      this.target.getAttribute(literals.dataColIndex),
      10
    );
    const row: Element = this.contentRenderer.getRowByIndex(rowIndex);
    if (row) {
      this.target = row.getElementsByClassName(literals.rowCell)[
        parseInt(cIndex.toString(), 10)
      ];
    }
  ...
  public shiftDownKey(rowIndex?: number, cellIndex?: number): void {
    ...
    if (this.isCellType() && !this.isSingleSel()) {
      this.selectCellsByRange(this.prevCIdxs || { rowIndex: 0, cellIndex: 0 }, {
        rowIndex,
        cellIndex,
      });
    }
  ...
  private getstackedColumns(column: Column[]): Column[] {
    ...
    for (let i: number = 0, len: number = column.length; i < len; i++) {
      if (column[parseInt(i.toString(), 10)].columns) {
        this.getstackedColumns(
          column[parseInt(i.toString(), 10)].columns as Column[]
        );
      } else {
  ...
  private addRemoveClassesForRow(
    ...
    if (row) {
      const cells: Element[] = [].slice.call(
        row.getElementsByClassName(literals.rowCell)
      );
      const detailIndentCell: Element =
        row.querySelector('.e-detailrowcollapse') ||
        row.querySelector('.e-detailrowexpand');
    ...
    this.getRenderer().setSelection(
      row ? row.getAttribute('data-uid') : null,
      isAdd,
      clearAll
    );
  }
  ...
  private isRowType(): boolean {
    return (
      this.selectionSettings.mode === 'Row' ||
      this.selectionSettings.mode === 'Both'
    );
  }
  ...
  public getSelectedRecords(): Object[] {
    ...
    if (
      ...
      selectedData = (<Row<Column>[]>this.parent.getRowsObject())
        .filter((row: Row<Column>) => row.isSelected)
  ...
  public selectColumn(index: number): void {
    ...
    const isColSelected: boolean =
      selectedCol.classList.contains('e-columnselection');
    if (!gObj.selectionSettings.allowColumnSelection) {
      return;
    }
    const isMultiColumns: boolean =
      this.selectedColumnsIndexes.length > 1 &&
    ...
    if (
      ...
      if (
        ...
        !(
          isColSelected &&
          gObj.selectionSettings.enableToggle &&
          index === this.prevColIndex
        )
      ) {
  ...
  public selectColumnsByRange(startIndex: number, endIndex?: number): void {
    const gObj: IGrid = this.parent;
    if (
      isNullOrUndefined(gObj.getColumns()[parseInt(startIndex.toString(), 10)])
    ) {
      return;
    }
    const indexes: number[] = [];
    ...
    for (let i: number = 0, len: number = columnIndexes.length; i < len; i++) {
      this.updateColSelection(
        gObj.getColumnHeaderByUid(
          gObj.getColumnByIndex(columnIndexes[parseInt(i.toString(), 10)]).uid
        ),
  ...
  public selectColumnWithExisting(startIndex: number): void {
    ...
    const newCol: Element = gObj.getColumnHeaderByUid(
      gObj.getColumnByIndex(startIndex).uid
    );
    let selectedCol: Element[] | Element = this.getselectedCols();
    if (gObj.selectionSettings.type === 'Single') {
      this.clearColDependency();
  ...
  public clearColumnSelection(clearIndex?: number): void {
    if (this.isColumnSelected) {
      const gObj: IGrid = this.parent;
      if (
        !isNullOrUndefined(clearIndex) &&
        this.selectedColumnsIndexes.indexOf(clearIndex) === -1
      ) {
        return;
      }
      const index: number = !isNullOrUndefined(clearIndex)
        ? clearIndex
      ...
      const isCanceled: boolean = this.columnDeselect(
        deselectedArgs,
      ...
      const selectedHeader: Element[] = !isNullOrUndefined(clearIndex)
        ? [selectedCol]
        : [].slice.call(
            gObj.getHeaderContent().getElementsByClassName('e-columnselection')
          );
      const selectedCells: Element[] = this.getSelectedColumnCells(clearIndex);
      for (
        let i: number = 0, len: number = selectedHeader.length;
        i < len;
        i++
      ) {
        addRemoveActiveClasses(
          [selectedHeader[parseInt(i.toString(), 10)]],
          false,
          'e-columnselection'
        );
      }
      for (
        ...
        addRemoveActiveClasses(
          [selectedCells[parseInt(i.toString(), 10)]],
          false,
          'e-columnselection'
        );
      }
      if (!isNullOrUndefined(clearIndex)) {
        this.selectedColumnsIndexes.splice(
          this.selectedColumnsIndexes.indexOf(clearIndex),
          1
        );
        this.parent.getColumns()[
          parseInt(clearIndex.toString(), 10)
        ].isSelected = false;
      } else {
        ...
        this.parent
          .getColumns()
          .filter((col: Column) => (col.isSelected = false));
      }
    }
  }
        ...
        selectedCol.push(
          gObj.getColumnHeaderByUid(
            gObj.getColumnByIndex(
              this.selectedColumnsIndexes[parseInt(i.toString(), 10)]
            ).uid
          )
        );
      }
    } else {
      selectedCol = gObj.getColumnHeaderByUid(
        gObj.getColumnByIndex(this.selectedColumnsIndexes[0]).uid
      );
    }
  ...
  private getSelectedColumnCells(clearIndex?: number): Element[] {
    ...
    const selectionString: string = !isNullOrUndefined(clearIndex)
      ? '[data-colindex="' + clearIndex + '"]'
      : '.e-columnselection';
    for (let i: number = 0, len: number = rows.length; i < len; i++) {
      seletedcells = seletedcells.concat(
        [].slice.call(
          rows[parseInt(i.toString(), 10)].querySelectorAll(selectionString)
        )
      );
    }
    return seletedcells;
  ...
  private updateColSelection(selectedCol: Element, startIndex: number): void {
    if (
      isNullOrUndefined(
        this.parent.getColumns()[parseInt(startIndex.toString(), 10)]
      )
    ) {
      return;
    }
    const isRowTemplate: boolean = !isNullOrUndefined(this.parent.rowTemplate);
    const rows: Element[] = isRowTemplate
      ? this.parent.getRows()
      : this.parent.getDataRows();
    this.selectedColumnsIndexes.push(startIndex);
    this.parent.getColumns()[parseInt(startIndex.toString(), 10)].isSelected =
      true;
    ...
    for (let j: number = 0, len: number = rows.length; j < len; j++) {
      if (rows[parseInt(j.toString(), 10)].classList.contains(literals.row)) {
        if (
          (rows[parseInt(j.toString(), 10)].classList.contains(
            literals.editedRow
          ) ||
            rows[parseInt(j.toString(), 10)].classList.contains(
              literals.addedRow
          ...
          this.parent.editSettings.mode === 'Normal' &&
          !isNullOrUndefined(
            rows[parseInt(j.toString(), 10)].querySelector('tr').childNodes[
          ...
          if (this.parent.isSpan && this.parent.isFrozenGrid()) {
            const cells: NodeListOf<Element> =
              rows[parseInt(j.toString(), 10)].querySelectorAll('.e-rowcell');
            for (let i: number = 0; i < cells.length; i++) {
              if (
                cells[parseInt(i.toString(), 10)].getAttribute(
                  'aria-colindex'
                ) === selectedCol.getAttribute('aria-colindex')
              ) {
                addRemoveActiveClasses(
                  [cells[parseInt(i.toString(), 10)]],
                  true,
                  'e-columnselection'
                );
              }
            ...
            !isNullOrUndefined(
              rows[parseInt(j.toString(), 10)].childNodes[
                parseInt(startIndex.toString(), 10)
              ]
            )
          ) {
  ...
  private headerSelectionHandler(colIndex: number): void {
    if (
      ...
      this.selectColumnsByRange(
        isUndefined(this.prevColIndex) ? colIndex : this.prevColIndex,
        colIndex
      );
    } else {
  ...
  private addEventListener_checkbox(): void {
    ...
    this.parent.on(
      events.refreshInfinitePersistSelection,
      this.onDataBoundFunction
    );
    this.parent.on(events.contentReady, this.checkBoxSelectionChanged, this);
    this.parent.on(
      events.beforeRefreshOnDataChange,
      this.initPerisistSelection,
      this
    );
  ...
  private setCheckAllForEmptyGrid(): void {
    ...
    if (checkAllBox) {
      ...
      const spanEle: HTMLElement =
        checkAllBox.nextElementSibling as HTMLElement;
      removeClass([spanEle], ['e-check', 'e-stop', 'e-uncheck']);
      addClass([spanEle.parentElement], ['e-checkbox-disabled']);
    }
  }
  ...
  public dataReady(e: { requestType: string }): void {
    ...
    if (
      ...
      this.disableUI =
        ...
        !(e.requestType === 'save' && e['action'] === 'add');
      this.clearSelection();
      this.setCheckAllState();
  ...
  private selectRowIndex(index: number): void {
    ...
    if (
      isNullOrUndefined(this.parent.selectedRowIndex) ||
      this.parent.selectedRowIndex === -1 ||
      !this.parent.enablePersistence
    ) {
