...
import { Query, DataManager, DataResult } from '@hoyui/data';
...
export class ExportHelper {
  ...
  public constructor(
    parent: IGrid,
    foreignKeyData?: { [key: string]: Object[] }
  ) {
    this.parent = parent;
  ...
  public static getQuery(parent: IGrid, data: Data): Query {
    ...
    if (data.isRemote()) {
      if (
        ...
        parent.groupSettings.columns.length
      ) {
        query.lazyLoad = [];
      } else {
  ...
  public getFData(value: string, column: Column): Object {
    const foreignKeyData: Object = getForeignData(
      column,
      {},
      value,
      this.foreignKeyData[column.field]
    )[0];
      ...
      for (let i: number = 0; i < length; i++, startIndex++) {
        ...
        if (gObj.childGrid || gObj.detailTemplate) {
          if (gObj.hierarchyPrintMode === 'All') {
            ...
            gObj.hierarchyPrintMode === 'Expanded' &&
            this.parent.expandedRows &&
            this.parent.expandedRows[parseInt(startIndex.toString(), 10)]
          ) {
            options.isExpand =
              gObj.expandedRows[parseInt(startIndex.toString(), 10)].isExpand;
          }
        }
    ...
    if (fColumns.length) {
      for (let i: number = 0; i < fColumns.length; i++) {
        const colData: DataManager =
          'result' in fColumns[parseInt(i.toString(), 10)].dataSource
            ? new DataManager(
                (
                  fColumns[parseInt(i.toString(), 10)].dataSource as DataResult
                ).result
              )
      ...
      promise = Promise.all(columnPromise).then((e: ReturnType[]) => {
        for (let j: number = 0; j < fColumns.length; j++) {
          this.foreignKeyData[fColumns[parseInt(j.toString(), 10)].field] =
            e[parseInt(j.toString(), 10)].result;
        }
  ...
  public getHeaders(
    columns: Column[],
    isHideColumnInclude?: boolean
  ): { rows: Row<Column>[]; columns: Column[] } {
  ...
  public getConvertedWidth(input: string): number {
    ...
    return input.indexOf('%') !== -1
      ? (this.parent.element.getBoundingClientRect().width * value) / 100
      : value;
  }
  ...
  private appendGridCells(
    ...
    if (
      !cols.columns &&
      (cols.visible !== false || this.hideColumnInclude) &&
      !cols.commands
    ) {
      gridRows[parseInt(index.toString(), 10)].cells.push(
        this.generateCell(cols, CellType.Header, this.colDepth - index, index)
      );
      ...
      for (let i: number = 0, len: number = cols.columns.length; i < len; i++) {
        if (
          (cols.columns as Column[])[parseInt(i.toString(), 10)].visible &&
          !isIgnoreFirstCell
        ) {
          isIgnoreFirstCell = true;
        }
        gridRows = this.appendGridCells(
  ...
  private processColumns(rows: Row<Column>[]): Row<Column>[] {
    ...
    for (let i: number = 0, len: number = rows.length; i < len; i++) {
      if (gridObj.allowGrouping) {
        for (
          let j: number = 0,
            len: number = gridObj.groupSettings.columns.length - 1;
          j < len;
          j++
        ) {
          if (
            gridObj.enableColumnVirtualization &&
            columnIndexes.indexOf(j) === -1
          ) {
            continue;
          }
          rows[parseInt(i.toString(), 10)].cells.splice(
            0,
            0,
  ...
  private getCellCount(column: Column, count: number): number {
    if (column.columns) {
      for (let i: number = 0; i < column.columns.length; i++) {
        count = this.getCellCount(
          <Column>column.columns[parseInt(i.toString(), 10)],
          count
        );
      }
    } else {
  ...
  public createChildGrid(
    gObj: IGrid,
    row: Row<Column>,
    exportType: string,
    gridPool: Object
  ): { childGrid: IGrid; element: HTMLElement } {
    const childGridObj: IGrid = new Grid(
      this.parent.detailRowModule.getGridModel(gObj, row, exportType)
    );
  ...
  public getGridExportColumns(columns: Column[]): Column[] {
    ...
    for (
      let i: number = 0, gridColumns: Column[] = columns;
  ...
  public formatCellValue(args: ExportHelperArgs): string {
    if (args.isForeignKey) {
      args.value = getValue(
        ...
        getForeignData(args.column, {}, args.value as string | number)[0]
      );
    }
    if (
      ...
      return args.value || args.value === 0
        ? this.internationalization.getNumberFormat(
            args.column.format as NumberFormatOptions
          )(args.value)
        : '';
      ...
      if (typeof args.column.format === 'string') {
        ...
        if (
          ...
          (args.column.format as DateFormatOptions).type === undefined
        ) {
