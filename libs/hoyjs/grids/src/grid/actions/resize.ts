...
export class Resize implements IAction {
  ...
  public autoFitColumns(
    ...
    const columnName: string[] =
      fName === undefined || fName === null || fName.length <= 0
        ? this.parent.getColumns().map((x: Column) => {
            x.autoFit = true;
            return x.field;
          })
        : typeof fName === 'string'
  ...
  private autoFit(): void {
    ...
    const contentTable: HTMLElement =
      this.parent.getContentTable() as HTMLElement;
    if (
      this.parent.allowResizing &&
      isMaxWidthCount &&
      (this.parent.resizeSettings.mode === 'Auto' ||
        (this.parent.resizeSettings.mode === 'Normal' &&
          !this.parent.autoFit &&
          newarray.length === 0))
    ) {
      this.widthService.setWidthToTable(
        contentTable.style.width.indexOf('px') === -1
      );
    } else if (
      this.parent.autoFit &&
      this.parent.resizeSettings.mode === 'Auto'
    ...
    if (
      ...
      contentTable.getBoundingClientRect().width <
        contentTable.parentElement.clientWidth
    ) {
      ...
      return [].slice
        .call(
          this.parent
            .getHeaderTable()
            .querySelectorAll(
              `td.e-rowcell:nth-child(${
                columnIndex + 1
              }):not(.e-groupcaption):not(.e-detailcell)`
            )
        )
        .concat(
          [].slice.call(
  ...
  private resizeColumn(
    ...
    const headerTextClone: Element = <HTMLElement>(
      headerTable
        .querySelector('[e-mappinguid="' + uid + '"]')
        .parentElement.cloneNode(true)
    );
    const contentTextClone: HTMLTableCellElement[] =
      this.getCellElementsByColumnIndex(columnIndex);
    ...
    if (footerTable) {
      footerTextClone = footerTable.querySelectorAll(
        `td:nth-child(${columnIndex + 1}):not(.e-groupcaption)`
      );
    }
    const indentWidthClone: NodeListOf<Element> = [].slice.call(
      headerTable
        .querySelector('tr')
        .getElementsByClassName('e-grouptopleftcell')
    );
    if (indentWidthClone.length > 0) {
      for (let i: number = 0; i < indentWidthClone.length; i++) {
        indentWidth += (<HTMLElement>(
          indentWidthClone[parseInt(i.toString(), 10)]
        )).offsetWidth;
      }
    }
    const detailsElement: HTMLElement =
      <HTMLElement>contentTable.querySelector('.e-detailrowcollapse') ||
      <HTMLElement>contentTable.querySelector('.e-detailrowexpand');
    if (
      (this.parent.detailTemplate || this.parent.childGrid) &&
      detailsElement
    ) {
    ...
    if (footerTable) {
      for (let i: number = 0; i < footerTextClone.length; i++) {
        footerText[parseInt(i.toString(), 10)] = footerTextClone[
          parseInt(i.toString(), 10)
        ].cloneNode(true) as Element;
      }
    }
    for (let i: number = 0; i < contentTextClone.length; i++) {
      contentText[parseInt(i.toString(), 10)] = contentTextClone[
        parseInt(i.toString(), 10)
      ].cloneNode(true) as Element;
    }
    const wHeader: number = this.createTable(
      headerTable,
      headerText,
      headerDivTag
    );
    ...
    const columnbyindex: Column =
      gObj.getColumns()[parseInt(columnIndexByField.toString(), 10)];
    const width: string = (columnbyindex.width = formatUnit(
      Math.max(wHeader, wContent, wFooter)
    ));
    const colMaxWidth: number =
      columnbyindex.maxWidth && parseFloat(columnbyindex.maxWidth.toString());
    if (parseInt(width, 10) > colMaxWidth) {
      columnbyindex.width = colMaxWidth;
    }
    this.widthService.setColumnWidth(
      gObj.getColumns()[parseInt(columnIndexByField.toString(), 10)] as Column
    );
    const result: boolean = gObj
      .getColumns()
      .some(
        (x: Column) =>
          (x.visible || gObj.groupSettings.columns.length) &&
          (x.width === null ||
            x.width === undefined ||
            (x.width as string).length <= 0)
      );
    if (result === false) {
      ...
      for (let i: number = 0; i < element.length; i++) {
        if (element[parseInt(i.toString(), 10)].visible) {
          tWidth =
            tWidth +
            parseFloat(element[parseInt(i.toString(), 10)].width as string);
        }
      }
    }
    ...
    if (tWidth > 0) {
      ...
      if (!isNullOrUndefined(footerTable)) {
        (<HTMLTableElement>footerTable).style.width =
          formatUnit(calcTableWidth);
      }
    }
  ...
  private findColumn(
    ...
    for (let i: number = 0; i < fName.length; i++) {
      ...
      const column: Column =
        this.parent.getColumns()[parseInt(columnIndex.toString(), 10)];
      if (
        columnIndex > -1 &&
  ...
  protected createTable(
    ...
    endRowIndex: number = text.length
  ): number {
    ...
    const myTableDiv: HTMLDivElement = this.parent.createElement(
      'div'
    ) as HTMLDivElement;
    const adaptiveClass: string = this.parent.enableAdaptiveUI
      ? ' e-bigger'
      : '';
    ...
    const mySubDiv: HTMLDivElement = this.parent.createElement(
      'div'
    ) as HTMLDivElement;
    mySubDiv.className = tag;
    const myTable: HTMLTableElement = this.parent.createElement('table', {
      attrs: { role: 'grid' },
    }) as HTMLTableElement;
    myTable.className = table.className;
    myTable.classList.add('e-resizetable');
    myTable.style.cssText = 'table-layout: auto;width: auto';
    const myTr: HTMLTableRowElement = this.parent.createElement(
      'tr'
    ) as HTMLTableRowElement;
    for (
      let i: number = startRowIndex <= 0 ? 1 : startRowIndex;
      i <= (endRowIndex > text.length ? text.length : endRowIndex);
      i++
    ) {
  ...
  private refreshHeight(): void {
    if (this.parent.getHeaderTable()) {
      ...
      for (let i: number = 0; i < element.length; i++) {
        if (
          element[parseInt(i.toString(), 10)].parentElement.offsetHeight > 0
        ) {
          element[parseInt(i.toString(), 10)].style.height =
            element[parseInt(i.toString(), 10)].parentElement.offsetHeight +
            'px';
        }
      }
      this.setHandlerHeight();
  ...
  private getResizeHandlers(): HTMLElement[] {
    return [].slice.call(
      this.parent.getHeaderTable().getElementsByClassName(resizeClassList.root)
    );
  }
  ...
  private setHandlerHeight(): void {
    const element: HTMLElement[] = [].slice.call(
      this.parent
        .getHeaderTable()
        .getElementsByClassName(resizeClassList.suppress)
    );
    for (let i: number = 0; i < element.length; i++) {
      element[parseInt(i.toString(), 10)].style.height =
        element[parseInt(i.toString(), 10)].parentElement.offsetHeight + 'px';
    }
  }

  private callAutoFit(e: PointerEvent | TouchEvent): void {
    if (
      (e.target as HTMLElement).classList.contains('e-rhandler') &&
      !this.isCancelAutoFit
    ) {
      ...
      this.resizeColumn(
        col.field,
        this.parent.getNormalizedColumnIndex(col.uid),
        col.uid
      );
      if (this.parent.isFrozenGrid()) {
        this.refreshResizefrzCols(true, true);
      }
      const header: HTMLElement = <HTMLElement>(
        closest(<HTMLElement>e.target, resizeClassList.header)
      );
  ...
  private resizeStart(e: PointerEvent | TouchEvent): void {
    if ((e.target as HTMLElement).classList.contains('e-rhandler')) {
      ...
      if (!this.isCancelAutoFit) {
        if (!this.helper) {
          ...
          if (this.column.getFreezeTableName() === literals.frozenRight) {
            if (this.parent.enableRtl) {
              this.minMove =
                (this.column.minWidth
                  ? parseFloat(this.column.minWidth.toString())
                  : 0) -
                parseFloat(
                  isNullOrUndefined(this.column.width)
                    ? ''
                    : this.column.width.toString()
                );
            } else {
              this.minMove =
                parseFloat(
        ...
        if (
          Browser.isDevice &&
          !this.helper.classList.contains(resizeClassList.icon)
        ) {
  ...
  private updateResizeEleHeight(): void {
    const elements: HTMLElement[] = [].slice.call(
      this.parent.getHeaderContent().getElementsByClassName('e-rhandler')
    );
  ...
  private getColData(
    ...
    return {
      width:
        parseFloat(
          isNullOrUndefined(this.widthService.getWidth(column)) ||
            this.widthService.getWidth(column) === 'auto'
            ? '0'
            : this.widthService.getWidth(column).toString()
        ) + mousemove,
  ...
  private refreshResizeFixedCols(pos?: string): void {
    const cols: Column[] = this.parent.getColumns();
    const translateX: number = this.parent.enableColumnVirtualization
      ? this.parent.translateX
      : 0;
    const th: HTMLTableCellElement[] = [].slice
      .call(
        this.parent
          .getHeaderContent()
          .querySelector('tbody')
          .querySelectorAll('.e-fixedfreeze')
      )
      .concat(
        [].slice.call(
          this.parent.getContent().querySelectorAll('.e-fixedfreeze')
        )
      );
    for (let i: number = 0; i < th.length; i++) {
      ...
      if (pos === 'Left') {
        ...
        applyStickyLeftRightPosition(
          node,
          (width === 0 ? width : width - 1) - translateX,
      ...
      if (pos === 'Right') {
        width =
          this.parent.getFrozenMode() === 'Right' && this.parent.isRowDragable()
            ? 30
            : 0;
        for (let j: number = cols.length - 1; j >= 0; j--) {
          ...
          if (
            cols[parseInt(j.toString(), 10)].freeze === 'Right' ||
            cols[parseInt(j.toString(), 10)].freeze === 'Fixed'
          ) {
            if (cols[parseInt(j.toString(), 10)].visible) {
              width += parseFloat(
  ...
  private refreshResizefrzCols(
    ...
    if (
      ...
      const columns: Column[] = this.parent
        .getColumns()
        .filter((col: Column) => col.freeze === 'Left' || col.isFrozen);
      ...
      for (let i: number = 0; i < columns.length; i++) {
        if (
          freezeRefresh ||
          columns[parseInt(i.toString(), 10)].index > this.column.index
        ) {
          ...
          if (this.parent.frozenRows) {
            elements = [].slice
              .call(
                this.parent
                  .getHeaderContent()
                  .querySelectorAll('td[data-colindex="' + i + '"]')
              )
              .concat(
                [].slice.call(
                  this.parent
          ...
          if (this.parent.enableColumnVirtualization) {
            (<{ valueX?: number }>columns[parseInt(i.toString(), 10)]).valueX =
              width;
          }
        }
    ...
    if (
      freezeRefresh ||
      this.column.freeze === 'Right' ||
      (this.column.columns && frozenDirection(this.column) === 'Right')
    ) {
      let width: number =
        this.parent.getFrozenMode() === 'Right' && this.parent.isRowDragable()
          ? 30
      ...
      const columnsRight: Column[] = columns.filter(
        (col: Column) => col.freeze === 'Right'
      );
      for (
        let i: number = columns.length - 1;
        i >= columns.length - columnsRight.length;
        i--
      ) {
        let elements: HTMLTableCellElement[] = [];
        if (this.parent.frozenRows) {
        ...
        elements.filter((cell: HTMLTableCellElement) => {
          let colSpanwidth: number = 0;
          if (cell.colSpan > 1) {
            colSpanwidth = this.calculateColspanWidth(
              columns,
              cell,
              columns[parseInt(i.toString(), 10)].index
            );
          }
          applyStickyLeftRightPosition(
            cell,
        ...
        if (columns[parseInt(i.toString(), 10)].visible) {
          width =
            width +
            parseFloat(columns[parseInt(i.toString(), 10)].width.toString());
        }
      }
    ...
    if (
      this.column &&
      (this.column.freeze === 'Fixed' ||
        (this.column.columns && frozenDirection(this.column) === 'Fixed'))
    ) {
      this.refreshResizeFixedCols('Left');
      this.refreshResizeFixedCols('Right');
      this.frzHdrRefresh('Left');
  ...
  private refreshGroupCaptionRow(): void {
    const capRow: HTMLElement[] = [].slice.call(
      this.parent.getContent().querySelectorAll('.e-groupcaptionrow')
    );
  ...
  private frzHdrRefresh(pos?: string): void {
    ...
    if (pos === 'Left') {
      ...
      for (let i: number = 0; i < tr.length; i++) {
        const th: HTMLElement[] = [].slice.call(
          tr[parseInt(i.toString(), 10)].querySelectorAll(
            '.e-leftfreeze,.e-fixedfreeze'
          )
        );
        for (let j: number = 0; j < th.length; j++) {
          const node: HTMLElement = th[parseInt(j.toString(), 10)];
          if (
            node.classList.contains('e-rowdragheader') ||
            node.classList.contains('e-dragheadercell') ||
          ...
          if (this.parent.aggregates.length && this.parent.getFooterContent()) {
            if (
              this.parent.getContent().querySelectorAll('.e-summaryrow').length
            ) {
              const summaryRows: HTMLElement[] = [].slice.call(
                this.parent.getContent().querySelectorAll('.e-summaryrow')
              );
              summaryRows.filter((row: HTMLElement) => {
                summarycell.push(
                  row.querySelector('[e-mappinguid="' + column.uid + '"]')
                );
              });
            }
            summarycell = summarycell.concat(
              [].slice.call(
                this.parent
                  .getFooterContent()
                  .querySelectorAll('[e-mappinguid="' + column.uid + '"]')
              )
            );
          }
            ...
            if (column.index === 0) {
              ...
              for (let k: number = 0; k < cols.length; k++) {
                ...
                if (cols[parseInt(k.toString(), 10)].visible) {
                  width += parseInt(
                    cols[parseInt(k.toString(), 10)].width.toString(),
                    10
                  );
                }
              }
              ...
              if (column.index < cols[parseInt(w.toString(), 10)].index) {
                if (
                  (column.columns &&
                    isChildColumn(
                      column,
                      cols[parseInt(w.toString(), 10)].uid
                    )) ||
                ...
                if (
                  cols[parseInt(w.toString(), 10)].freeze === 'Right' ||
                  cols[parseInt(w.toString(), 10)].freeze === 'Fixed'
                ) {
                  if (cols[parseInt(w.toString(), 10)].visible) {
                    width += parseFloat(
            ...
            for (let k: number = cols.length - 1; k >= 0; k--) {
              if (
                (column.columns &&
                  isChildColumn(
                    column,
                    cols[parseInt(k.toString(), 10)].uid
                  )) ||
                column.index > cols[parseInt(k.toString(), 10)].index ||
                column.uid === cols[parseInt(k.toString(), 10)].uid
              ) {
  ...
  private getParticularCol(node?: HTMLElement): Column {
    const uid: string = node.classList.contains('e-filterbarcell')
      ? node.getAttribute('e-mappinguid')
  ...
  private resizing(e: PointerEvent | TouchEvent): void {
    ...
    if (isNullOrUndefined(this.column)) {
      offsetWidth = (parentsUntil(this.element, 'th') as HTMLTableCellElement)
        .offsetWidth;
    }
    if (this.parent.allowTextWrap) {
    ...
    const mousemove: number = this.parent.enableRtl
      ? -(pageX - this.pageX)
      : pageX - this.pageX;
    const colData: { [key: string]: number } = this.getColData(
      this.column,
      mousemove
    );
    if (!colData.width) {
    ...
    if (
      width !==
      parseFloat(
        isNullOrUndefined(this.column.width) || this.column.width === 'auto'
  ...
  private calulateColumnsWidth(
    ...
    for (const col of columns) {
      ...
      const colData: { [key: string]: number } = this.getColData(
        col,
        (parseFloat(col.width as string) * mousemove) / totalWidth
      );
      const colWidth: number = this.getWidth(
        colData.width,
        colData.minWidth,
        colData.maxWidth
      );
  ...
  private resizeEnd(e: PointerEvent): void {
    ...
    const content: HTMLElement = this.parent
      .getContent()
  ...
  private refreshColumnWidth(): Column[] {
    ...
    for (const ele of [].slice.apply(
      this.parent.getHeaderContent().querySelectorAll('th.e-headercell')
    )) {
  ...
  private refreshStackedColumnWidth(): void {
    for (const stackedColumn of this.parent.getStackedColumns(
  ...
  private getTargetColumn(e: PointerEvent | TouchEvent): Column {
    let cell: HTMLElement = <HTMLElement>(
      closest(<HTMLElement>e.target, resizeClassList.header)
    );
    cell =
      cell.querySelector('.e-headercelldiv') ||
      cell.querySelector('.e-stackedheadercelldiv');
  ...
  private updateCursor(action: string): void {
    const headerRows: Element[] = [].slice.call(
      this.parent.getHeaderContent().querySelectorAll('th')
    );
  ...
  private setHelperHeight(): void {
    let height: number =
      (<HTMLElement>this.parent.getContent()).offsetHeight -
      this.getScrollBarWidth();
    const rect: HTMLElement = closest(
      this.element,
      resizeClassList.header
    ) as HTMLElement;
    const tr: HTMLElement[] = [].slice.call(
      this.parent.getHeaderContent().querySelectorAll('tr')
    );
    for (
      let i: number = tr.indexOf(rect.parentElement);
      i < tr.length && i > -1;
      i++
    ) {
  ...
  private removeHelper(e: MouseEvent): void {
    ...
    if (
      !(
        cls.contains(resizeClassList.root) || cls.contains(resizeClassList.icon)
      ) &&
      this.helper
    ) {
      EventHandler.remove(document, Browser.touchStartEvent, this.removeHelper);
  ...
  private updateHelper(): void {
    ...
    let left: number;
    left = Math.floor(
      this.calcPos(rect).left +
        (this.parent.enableRtl ? 0 - 1 : rect.offsetWidth - 2)
    );
  ...
  private calcPos(elem: HTMLElement): OffsetPosition {
    ...
    let offsetParent: Node =
      parentsUntil(elem, 'e-grid') || doc.documentElement;
    while (
      offsetParent &&
