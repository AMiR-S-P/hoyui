...
import { PdfTrueTypeFont } from '@hoyui/pdf-export';
...
} from '../services/summary-model-generator';
...
import { DataManager, Query, Group } from '@hoyui/data';
...
export class PdfExport {
  ...
  public Map(
    ...
    this.gridPool = {};
    let query: Query =
      pdfExportProperties && pdfExportProperties.query
        ? pdfExportProperties.query
        : new Query();
    if (
      (parent.childGrid || parent.detailTemplate) &&
      !(
        !isNullOrUndefined(pdfExportProperties) &&
        pdfExportProperties.hierarchyExportMode === 'None'
      )
    ) {
    ...
    const args: Object = {
      ...
      cancel: false,
      headerPageNumbers: [],
      gridDrawPosition: { xPosition: 0, yPosition: 0 },
      generateQuery: false,
    };
    ...
    if (
      !isNullOrUndefined(pdfExportProperties) &&
      !isNullOrUndefined(pdfExportProperties.dataSource)
    ) {
      pdfExportProperties.dataSource =
        pdfExportProperties.dataSource instanceof DataManager
          ? pdfExportProperties.dataSource
          : new DataManager(pdfExportProperties.dataSource);
      if (isNullOrUndefined(query.isCountRequired) || parent.aggregates) {
        query.isCountRequired = true;
      }
      return new Promise((resolve: Function, reject: Function) => {
        (<DataManager>pdfExportProperties.dataSource)
          .executeQuery(query)
          .then((returnType: Object) => {
            this.exportWithData(
              parent,
              pdfDoc,
              resolve,
      ...
      !isNullOrUndefined(pdfExportProperties) &&
      pdfExportProperties.exportType === 'CurrentPage' &&
      !(
      ...
      if (
        ...
        pdfExportProperties.hierarchyExportMode =
          pdfExportProperties.hierarchyExportMode === 'None' ? 'None' : 'All';
        ...
        Promise.all(allPromise).then((e: ReturnType[]) => {
          ...
          this.processExport(
            ...
            .then((results: object) => {
              ...
              if (
                pdfExportProperties &&
                pdfExportProperties.multipleExport &&
                pdfExportProperties.multipleExport.type === 'AppendToPage'
              ) {
                resolve(results);
              } else {
                resolve(this.pdfDocument);
  ...
  private processExport(
    ...
    const section: PdfSection = !(
      ...
      ? (this.pdfDocument.sections.add() as PdfSection)
      : null;
    let pdfGrid: PdfGrid;
    ...
    return new Promise((resolve: Function, reject: Function) => {
      pdfGrid = <PdfGrid>(
        this.processGridExport(gObj, returnType, pdfExportProperties)
      );
      ...
      if (
        ...
        if (pdfExportProperties.multipleExport.blankSpace) {
          yPosition =
            pdfDoc['result'].bounds.y +
            pdfDoc['result'].bounds.height +
            pdfExportProperties.multipleExport.blankSpace;
        }
      } else {
  ...
  private processSectionExportProperties(
    ...
    if (
      ...
      !isNullOrUndefined(pdfExportProperties) &&
      (!isNullOrUndefined(pdfExportProperties.pageOrientation) ||
        !isNullOrUndefined(pdfExportProperties.pageSize))
    ) {
      this.pdfPageSettings.orientation =
  ...
  private processGridExport(
    ...
    if (!isNullOrUndefined(pdfExportProperties)) {
      ...
      allowHorizontalOverflow = isNullOrUndefined(
        pdfExportProperties.allowHorizontalOverflow
      )
        ? true
        : pdfExportProperties.allowHorizontalOverflow;
    }
    const helper: ExportHelper = new ExportHelper(
      gObj,
      this.helper.getForeignKeyData()
    );
    const dataSource: Object[] | Group = this.processExportProperties(
      pdfExportProperties,
      returnType.result
    );
    ...
    columns = columns.filter((columns: Column) => {
    ...
    if (
      ...
      gObj.hierarchyPrintMode =
        pdfExportProperties.hierarchyExportMode || 'Expanded';
    }
    ...
    const returnValue: { rows: Row<Column>[]; columns: Column[] } =
      helper.getHeaders(columns, this.hideColumnInclude);

    // Column collection with respect to the records in the grid
    const gridColumns: Column[] = returnValue.columns;

    // process grid header content
    pdfGrid = this.processGridHeaders(
    ...
    if (!isNullOrUndefined(dataSource) && dataSource.length) {
      if (isGrouping) {
        ...
        this.processGroupedRecords(
          ...
          border,
          0,
          captionThemeStyle.font,
          captionThemeStyle.brush,
          captionThemeStyle.backgroundBrush,
          returnType,
          pdfExportProperties,
          helper,
          0
      ...
      if (!isNullOrUndefined(returnType.aggregates)) {
        const summaryModel: SummaryModelGenerator = new SummaryModelGenerator(
          gObj
        );
        let sRows: Row<AggregateColumnModel>[];
        let column: Column[] = summaryModel.getColumns();
        column = column.filter((col: Column) => {
        ...
        if (this.customDataSource) {
          sRows = summaryModel.generateRows(
            dataSource,
            <SummaryData>returnType.aggregates,
            null,
            null,
            column
          );
  ...
  private getSummaryCaptionThemeStyle(): IThemeStyles {
    if (
      !isNullOrUndefined(this.gridTheme) &&
      !isNullOrUndefined(this.gridTheme.caption)
    ) {
      const fontSize: number = !isNullOrUndefined(
        this.gridTheme.caption.fontSize
      )
        ? this.gridTheme.caption.fontSize
        : 9.75;
      const fontFamily: number = !isNullOrUndefined(
        this.gridTheme.caption.fontName
      )
        ? this.getFontFamily(this.gridTheme.caption.fontName)
      ...
      if (!isNullOrUndefined(this.gridTheme.caption.fontColor)) {
        const penBrushColor: { r: number; g: number; b: number } =
          this.hexToRgb(this.gridTheme.caption.fontColor);
      ...
      const borderCaption: PdfBorders = this.gridTheme.caption.border
        ? this.getBorderStyle(this.gridTheme.caption.border)
        : null;
      let font: PdfStandardFont | PdfTrueTypeFont = new PdfStandardFont(
        fontFamily,
        fontSize,
        fontStyle
      );
  ...
  private getGridPdfFont(args: PdfTheme): void {
    ...
    if (args.header && args.header.font) {
      ...
      style = isNullOrUndefined(PdfFontStyle[`${headerStyle}`])
        ? 0
        : PdfFontStyle[`${headerStyle}`];
    ...
    if (args.caption && args.caption.font) {
      ...
      style = isNullOrUndefined(PdfFontStyle[`${captionStyle}`])
        ? 0
        : PdfFontStyle[`${captionStyle}`];
      if (args.caption.font[`${isTrueType}`]) {
        args.caption.font = new PdfTrueTypeFont(
          captionFont,
          captionSize,
          style
        );
        ...
        const fontFamily: number = !isNullOrUndefined(captionFont)
          ? this.getFontFamily(captionFont)
          : PdfFontFamily.Helvetica;
    ...
    if (args.record && args.record.font) {
      ...
      style = isNullOrUndefined(PdfFontStyle[`${recordStyle}`])
        ? 0
        : PdfFontStyle[`${recordStyle}`];
  ...
  private getHeaderThemeStyle(): IThemeStyles {
    ...
    if (
      !isNullOrUndefined(this.gridTheme) &&
      !isNullOrUndefined(this.gridTheme.header)
    ) {
      const fontFamily: number = !isNullOrUndefined(
        this.gridTheme.header.fontName
      )
        ? this.getFontFamily(this.gridTheme.header.fontName)
        : PdfFontFamily.Helvetica;
      const fontStyle: PdfFontStyle = this.getFontStyle(this.gridTheme.header);
      const fontSize: number = !isNullOrUndefined(
        this.gridTheme.header.fontSize
      )
        ? this.gridTheme.header.fontSize
        : 10.5;
      ...
      if (!isNullOrUndefined(this.gridTheme.header.fontColor)) {
        const penBrushColor: { r: number; g: number; b: number } =
          this.hexToRgb(this.gridTheme.header.fontColor);
        pdfColor = new PdfColor(
          penBrushColor.r,
          penBrushColor.g,
          penBrushColor.b
  ...
  private processGroupedRecords(
    ...
    for (const dataSourceItems of dataSource) {
      ...
      const value: string =
        gObj.getColumnByField(dataSourceItems.field).headerText +
        ': ' +
        (!col.enableGroupByFormat
          ? this.exportValueFormatter.formatCellValue(args)
          : dataSourceItems.key) +
        ' - ' +
        dataSourceItems.count +
        (dataSource.count > 1 ? ' items' : ' item');
      const cArgs: ExportGroupCaptionEventArgs = {
        captionText: value,
        type: 'PDF',
      ...
      this.parent.trigger(
        events.exportGroupCaption,
        cArgs,
        (cArgs: ExportGroupCaptionEventArgs) => {
          row.cells.getCell(groupIndex).value = cArgs.captionText;
          row.cells.getCell(groupIndex).style.stringFormat =
          ...
          let sRows: Row<AggregateColumnModel>[];
          const captionSummaryModel: CaptionSummaryModelGenerator =
            new CaptionSummaryModelGenerator(gObj);
          if (!isNullOrUndefined(dataSourceItems.items.records)) {
            sRows = captionSummaryModel.generateRows(
              dataSourceItems.items.records,
              dataSourceItems
            );
          } else {
            sRows = captionSummaryModel.generateRows(
          ...
          if (!isNullOrUndefined(sRows) && sRows.length === 0) {
            row.cells.getCell(groupIndex + 1).columnSpan =
              pdfGrid.columns.count - (groupIndex + 1);
          }
          if (
            !isNullOrUndefined(dataSource.childLevels) &&
            dataSource.childLevels > 0
          ) {
            this.processAggregates(
              sRows,
              pdfGrid,
            ...
            index = this.rowIndex;
            const groupSummaryModel: GroupSummaryModelGenerator =
              new GroupSummaryModelGenerator(gObj);
            sRows = groupSummaryModel.generateRows(
              dataSourceItems.items.records,
              dataSourceItems
            );
            this.processAggregates(
  ...
  private processGridHeaders(
    ...
    const applyTextAndSpan: Function = (
      rowIdx: number,
      colIdx: number,
      col: Column,
      rowSpan: number,
      ...
      const cell: Cell<Column> =
        rows[parseInt(rowIdx.toString(), 10)].cells[
          grid.groupSettings.columns.length
            ? colIdx
            : rowNumber[parseInt(rowIdx.toString(), 10)]
        ];
      rowNumber[parseInt(rowIdx.toString(), 10)] =
        rowNumber[parseInt(rowIdx.toString(), 10)] + 1;
      if (!isNullOrUndefined((col as Column).headerTextAlign)) {
        pdfCell.style.stringFormat = this.getHorizontalAlignment(
          col.headerTextAlign
        );
      }
      if (rowSpan > 0) {
      ...
      if (
        ...
        (cell.cellType === CellType.HeaderIndent ||
          cell.cellType === CellType.DetailHeader)
      ) {
        pdfCell.value = '';
        pdfCell.width = 20;
      }
      ...
      if (!isNullOrUndefined(evtArgs.style.verticalAlignment)) {
        pdfCell.style.stringFormat = this.getVerticalAlignment(
          evtArgs.style.verticalAlignment,
          pdfCell.style.stringFormat
        );
      }
    ...
    const recuHeader: Function = (
      ...
      for (let i: number = 0; i < cols.length; i++) {
        ...
        if (
          (cols[parseInt(i.toString(), 10)] as Column).columns &&
          (cols[parseInt(i.toString(), 10)] as Column).columns.length
        ) {
          const newSpanCnt: number = recuHeader(
            (cols[parseInt(i.toString(), 10)] as Column).columns,
            depth - 1,
            0,
            i + colIndex,
          ...
          applyTextAndSpan(
            rowIndex,
            i + colIndex + index,
            cols[parseInt(i.toString(), 10)] as Column,
            0,
            newSpanCnt
          );
  ...
  private processExportProperties(
    ...
    if (!isNullOrUndefined(pdfExportProperties)) {
      ...
      if (!isNullOrUndefined(pdfExportProperties.footer)) {
        ...
        const position: PointF = new PointF(
          0,
          clientSize.width - 80 - footer.fromBottom * 0.75
        );
        const size: SizeF = new SizeF(
          clientSize.width - 80,
          footer.height * 0.75
        );
        const bounds: RectangleF = new RectangleF(position, size);
        this.pdfDocument.template.bottom = this.drawPageTemplate(
          new PdfPageTemplateElement(bounds),
          footer
        );
      }
      if (
        !isNullOrUndefined(pdfExportProperties.includeHiddenColumn) &&
        !this.isGrouping
      ) {
  ...
  private drawHeader(pdfExportProperties: PdfExportProperties): void {
    ...
    if (
      !isNullOrUndefined(pdfExportProperties) &&
      !isNullOrUndefined(pdfExportProperties.header)
    ) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      if (!this.headerOnPages.length) {
        this.pdfDocument.template.top = this.drawPageTemplate(
          new PdfPageTemplateElement(bounds),
          header
        );
      } else {
        const headerTemplate: PdfPageTemplateElement = this.drawPageTemplate(
          new PdfPageTemplateElement(bounds),
          header
        );
        this.headerOnPages.filter((index: number) => {
          if (index - 1 >= 0 && index - 1 <= this.pdfDocument.pages.count - 1) {
            this.pdfDocument.pages
      ...
      switch (content.type) {
        case 'Text':
          if (
            ...
            throw new Error(
              'please enter the valid input value in text content...'
            );
          }
        ...
        case 'Image':
          if (
            ...
            throw new Error(
              'please enter the valid base64 string in image content...'
            );
          }
    ...
    if (content.type === undefined || content.type === null) {
      ...
      if (content.type === 'Line') {
        if (content.points === undefined || content.points === null) {
          throw new Error(
            'please enter valid points in ' + content.type + ' content...'
          );
        } else {
          if (
            content.points.x1 === undefined ||
            content.points.x1 === null ||
            typeof content.points.x1 !== 'number'
          ) {
            throw new Error(
              'please enter valid x1 co-ordinate in ' +
                content.type +
                ' points...'
            );
          }
          if (
            content.points.y1 === undefined ||
            content.points.y1 === null ||
            typeof content.points.y1 !== 'number'
          ) {
            throw new Error(
              'please enter valid y1 co-ordinate in ' +
                content.type +
                ' points...'
            );
          }
          if (
            content.points.x2 === undefined ||
            content.points.x2 === null ||
            typeof content.points.x2 !== 'number'
          ) {
            throw new Error(
              'please enter valid x2 co-ordinate in ' +
                content.type +
                ' points...'
            );
          }
          if (
            content.points.y2 === undefined ||
            content.points.y2 === null ||
            typeof content.points.y2 !== 'number'
          ) {
            throw new Error(
              'please enter valid y2 co-ordinate in ' +
                content.type +
                ' points...'
            );
          }
        ...
        if (content.position === undefined || content.position === null) {
          throw new Error(
            'please enter valid position in ' + content.type + ' content...'
          );
        } else {
          if (
            content.position.x === undefined ||
            content.position.x === null ||
            typeof content.position.x !== 'number'
          ) {
            throw new Error(
              'please enter valid x co-ordinate in ' +
                content.type +
                ' position...'
            );
          }
          if (
            content.position.y === undefined ||
            content.position.y === null ||
            typeof content.position.y !== 'number'
          ) {
            throw new Error(
              'please enter valid y co-ordinate in ' +
                content.type +
                ' position...'
            );
          }
  ...
  private drawText(pageTemplate: PdfPageTemplateElement, content: any): void {
    ...
    if (!isNullOrUndefined(content.style.textPenColor)) {
      const penColor: { r: number; g: number; b: number } = this.hexToRgb(
        content.style.textPenColor
      );
    ...
    const result: { format: PdfStringFormat; size: SizeF } =
      this.setContentFormat(content, format);
    if (
      result !== null &&
      !isNullOrUndefined(result.format) &&
      !isNullOrUndefined(result.size)
    ) {
      pageTemplate.graphics.drawString(
        value,
        font,
        pen,
        brush,
  ...
  private drawPageNumber(
    documentHeader: PdfPageTemplateElement,
    content: any
  ): void {
    const font: PdfFont = this.getFont(content);
    let brush: PdfSolidBrush = null;
    if (!isNullOrUndefined(content.style.textBrushColor)) {
      const brushColor: { r: number; g: number; b: number } = this.hexToRgb(
        content.style.textBrushColor
      );
      brush = new PdfSolidBrush(
        new PdfColor(brushColor.r, brushColor.g, brushColor.b)
      );
    } else {
      brush = new PdfSolidBrush(new PdfColor(0, 0, 0));
    }
    ...
    if (!isNullOrUndefined(content.format)) {
      ...
      if (
        (content.format as string).indexOf(total) !== -1 &&
        (content.format as string).indexOf(current) !== -1
      ) {
        ...
        if (
          (content.format as string).indexOf(total) >
          (content.format as string).indexOf(current)
        ) {
        ...
        compositeField = new PdfCompositeField(
          font,
          brush,
          format,
          pageNumber,
    ...
    const y: number = content.position.y * 0.75;
    const result: { format: PdfStringFormat; size: SizeF } =
      this.setContentFormat(content, compositeField.stringFormat);
    if (
      result !== null &&
      !isNullOrUndefined(result.format) &&
      !isNullOrUndefined(result.size)
      ...
      compositeField.bounds = new RectangleF(
        x,
        y,
        result.size.width,
        result.size.height
  ...
  private drawImage(
    ...
    const x: number = content.position.x * 0.75;
    const y: number = content.position.y * 0.75;
    const width: number = !isNullOrUndefined(content.size)
      ? content.size.width * 0.75
      : undefined;
    const height: number = !isNullOrUndefined(content.size)
      ? content.size.height * 0.75
      : undefined;
  ...
  private drawLine(documentHeader: PdfPageTemplateElement, content: any): void {
    ...
    if (!isNullOrUndefined(content.style) && content.style !== null) {
      if (
        !isNullOrUndefined(content.style.penSize) &&
        content.style.penSize !== null &&
        typeof content.style.penSize === 'number'
      ) {
  ...
  private processAggregates(
    sRows: Row<AggregateColumnModel>[],
    pdfGrid: PdfGrid,
    border: PdfBorders,
    font: PdfFont,
    brush: PdfSolidBrush,
    backgroundBrush: PdfSolidBrush,
    isCaption: boolean,
    captionRow?: PdfGridRow,
    groupIndex?: number,
    isGroupedFooter?: boolean,
    isAggregate?: boolean,
    gObj?: IGrid
  ): void {
    for (const row of sRows) {
      ...
      for (let i: number = 0; i < pdfGrid.columns.count + aggIdx; i++) {
        let cell: Cell<AggregateColumnModel> =
          row.cells[parseInt(index.toString(), 10)];
        ...
        if (cell.isDataCell) {
          ...
          const footerTemplate: boolean = !isNullOrUndefined(
            cell.column.footerTemplate
          );
          const groupFooterTemplate: boolean = !isNullOrUndefined(
            cell.column.groupFooterTemplate
          );
          const groupCaptionTemplate: boolean = !isNullOrUndefined(
            cell.column.groupCaptionTemplate
          );
          if (footerTemplate || groupCaptionTemplate || groupFooterTemplate) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const result: any = this.getTemplateFunction(
              templateFn,
              i,
            ...
            const data: Object =
              row.data[
                cell.column.field ? cell.column.field : cell.column.columnName
              ];
            if (
              (this.parent.isReact ||
                this.parent.isVue ||
                this.parent.isVue3 ||
                this.parent.isAngular) &&
              !(
                typeof cell.column.footerTemplate === 'string' ||
                typeof cell.column.groupFooterTemplate === 'string' ||
                typeof cell.column.groupCaptionTemplate === 'string'
              )
            ) {
              txt = data[cell.column.type as string];
              txt = !isNullOrUndefined(txt) ? txt : '';
            } else {
              txt = templateFn[getEnumValue(CellType, cell.cellType)](
                data,
                this.parent
              );
              txt =
                this.parent.isVue3 && !isNullOrUndefined(txt[1])
                  ? (<Text>txt[1]).textContent
                  : !isNullOrUndefined(txt[0])
                  ? (<Text>txt[0]).textContent
                  : '';
            }
            isEmpty = false;
            const args: AggregateQueryCellInfoEventArgs = {
              ...
              type: footerTemplate
                ...
                ? 'GroupFooter'
                : 'GroupCaption',
              style: isNullOrUndefined(captionRow)
                ? gridRow.cells
                : captionRow.cells,
              cell: cell,
              value: txt as string,
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const result: any = this.getSummaryWithoutTemplate(
              row.data[cell.column.field]
            );
        ...
        if (
          isEmpty &&
          value[parseInt(i.toString(), 10)] !== '' &&
          !isNullOrUndefined(value[parseInt(i.toString(), 10)]) &&
          value[parseInt(i.toString(), 10)] !== null
        ) {
          isEmpty = false;
        }
        index += 1;
      ...
      if (!isAggregate) {
        if (!isCaption) {
          ...
          for (
            let i: number = gObj.groupSettings.columns.length;
            i < value.length - 1;
            i++
          ) {
      ...
      if (!isEmpty) {
        if (!isCaption) {
          for (let i: number = 0; i < pdfGrid.columns.count; i++) {
            gridRow.cells.getCell(i).value =
              value[parseInt(i.toString(), 10)].toString();
          }
        } else {
          for (let i: number = 0; i < pdfGrid.columns.count; i++) {
            captionRow.cells.getCell(i).value =
              value[parseInt(i.toString(), 10)].toString();
            if (
              i === groupIndex &&
              leastCaptionSummaryIndex !== -1 &&
              leastCaptionSummaryIndex !== 1
            ) {
              captionRow.cells.getCell(i).columnSpan =
                leastCaptionSummaryIndex - 1 - groupIndex;
            } else if (i === groupIndex && leastCaptionSummaryIndex === -1) {
              captionRow.cells.getCell(i).columnSpan =
                pdfGrid.columns.count - groupIndex;
            }
          }
        }
      }
  ...
  private getTemplateFunction(
    templateFn: any,
    index: number,
    leastCaptionSummaryIndex: number,
    cell: any
  ): any {
    if (
      !isNullOrUndefined(cell.column.footerTemplate) &&
      cell.cellType === CellType.Summary
    ) {
      templateFn[getEnumValue(CellType, CellType.Summary)] = compile(
        cell.column.footerTemplate
      );
      ...
      templateFn[getEnumValue(CellType, CellType.CaptionSummary)] = compile(
        cell.column.groupCaptionTemplate
      );
    } else {
      templateFn[getEnumValue(CellType, CellType.GroupSummary)] = compile(
        cell.column.groupFooterTemplate
      );
    }
    return {
  ...
  private setColumnProperties(
    ...
    const startIndex: number = gObj.groupSettings.columns.length
      ? gObj.groupSettings.columns.length - 1
      : 0;
    ...
    for (let i: number = 0; i < gridColumns.length; i++) {
      if (
        !isNullOrUndefined(gridColumns[parseInt(i.toString(), 10)].textAlign)
      ) {
        pdfGrid.columns.getColumn(i + startIndex).format =
          this.getHorizontalAlignment(
            gridColumns[parseInt(i.toString(), 10)].textAlign
          );
      }
      // Need to add width consideration with % value
      if (
        pdfGrid.style.allowHorizontalOverflow &&
        !isNullOrUndefined(gridColumns[parseInt(i.toString(), 10)].width) &&
        allowHorizontalOverflow
      ) {
        pdfGrid.columns.getColumn(i + startIndex).width =
          typeof gridColumns[parseInt(i.toString(), 10)].width === 'number'
            ? (gridColumns[parseInt(i.toString(), 10)].width as number) * 0.75
            : helper.getConvertedWidth(
                gridColumns[parseInt(i.toString(), 10)].width as string
              ) * 0.75;
      }
    }
  }
  ...
  private setRecordThemeStyle(row: PdfGridRow, border: PdfBorders): PdfGridRow {
    if (
      !isNullOrUndefined(this.gridTheme) &&
      !isNullOrUndefined(this.gridTheme.record)
    ) {
      const fontFamily: number = !isNullOrUndefined(
        this.gridTheme.record.fontName
      )
        ? this.getFontFamily(this.gridTheme.record.fontName)
        : PdfFontFamily.Helvetica;
      const fontSize: number = !isNullOrUndefined(
        this.gridTheme.record.fontSize
      )
        ? this.gridTheme.record.fontSize
        : 9.75;
  ...
  private processRecord(
    ...
    const rows: Row<Column>[] = helper.getGridRowModel(
      columns,
      dataSource,
      gObj,
      rowIndex
    ...
    for (const row of rows) {
      ...
      const gridRow: PdfGridRow = this.setRecordThemeStyle(
        pdfGrid.rows.addRow(),
        border
      );
      ...
      for (let j: number = 0; j < cellLength; j++) {
        ...
        const cellValue: string = !isNullOrUndefined(field)
          ? (column.valueAccessor as Function)(field, row.data, column)
          : '';
        ...
        args.value =
          args.column.type === 'boolean' && typeof args.value === 'string'
            ? args.value
            : this.exportValueFormatter.formatCellValue(args);
        ...
        if (!isNullOrUndefined(args.image)) {
          ...
          args.value.height =
            (<{ height?: number }>args.image).height || args.value.height;
          args.value.width =
            (<{ width?: number }>args.image).width || args.value.width;
        }
      ...
      if (row.isExpand) {
        ...
        const startIndexVal: number =
          this.parent.childGrid || this.parent.detailTemplate ? 0 : startIndex;
        ...
        if (this.parent.childGrid) {
          ...
          (<{ actionFailure?: Function }>childGridObj).actionFailure =
            helper.failureHandler(
              this.gridPool,
              childGridObj,
              this.globalResolve
            );
          const childExportProperties: PdfExportProperties = extend(
            pdfExportProperties,
            {
              columns: null,
              dataSource: null,
          ...
          (<Grid>childGridObj).beforeDataBound = this.childGridCell(
            cell,
            childGridObj,
            childExportProperties
          );
          childGridObj.appendTo(element);
        } else if (this.parent.detailTemplate) {
          const args: ExportDetailTemplateEventArgs = {
            parentRow: row,
            row: gridRow,
            value: {},
            action: 'pdfexport',
  ...
  private processDetailTemplate(
    templateData: ExportDetailTemplateEventArgs
  ): PdfGrid | PdfTextWebLink | string | PdfBitmap {
    if (templateData.value.columnHeader || templateData.value.rows) {
      ...
      const processRow: Function = (
        ...
        for (let j: number = 0; j < row.cells.length; j++) {
          const currentCell: DetailTemplateCell =
            row.cells[parseInt(j.toString(), 10)];
          const pdfCell: PdfGridCell = gridRow.cells.getCell(
            currentCell.index ? currentCell.index : j
          );
      ...
      if (templateData.value.columnCount) {
        ...
        if (
          templateData.value.columnHeader &&
          templateData.value.columnHeader.length
        ) {
      ...
      if (templateData.value.columnHeader) {
        ...
        for (
          let i: number = 0;
          i < templateData.value.columnHeader.length;
          i++
        ) {
          const gridHeader: PdfGridRow = pdfGrid.headers.getHeader(
            parseInt(i.toString(), 10)
          );
          processRow(
            templateData.value.columnHeader[parseInt(i.toString(), 10)],
            gridHeader,
            true
          );
        }
  ...
  private childGridCell(
    ...
    return (result: ReturnType): Object => {
      ...
      cell.value = this.processGridExport(
        childGridObj,
        result,
        pdfExportProperties
      );
  ...
  private processCellStyle(
    cell: PdfGridCell,
    args: PdfQueryCellInfoEventArgs
  ): void {
    if (!isNullOrUndefined(args.style.backgroundColor)) {
      const backColor: { r: number; g: number; b: number } = this.hexToRgb(
        args.style.backgroundColor
      );
      cell.style.backgroundBrush = new PdfSolidBrush(
        new PdfColor(backColor.r, backColor.g, backColor.b)
      );
    }

    if (!isNullOrUndefined(args.style.textAlignment)) {
      cell.style.stringFormat = this.getHorizontalAlignment(
        args.style.textAlignment
      );
    }
    ...
    if (!isNullOrUndefined(args.style.verticalAlignment)) {
      cell.style.stringFormat = this.getVerticalAlignment(
        args.style.verticalAlignment,
        cell.style.stringFormat
      );
    }

    if (!isNullOrUndefined(args.style.textBrushColor)) {
      const textBrushColor: { r: number; g: number; b: number } = this.hexToRgb(
        args.style.textBrushColor
      );
      cell.style.textBrush = new PdfSolidBrush(
        new PdfColor(textBrushColor.r, textBrushColor.g, textBrushColor.b)
      );
    }

    if (!isNullOrUndefined(args.style.textPenColor)) {
      const textPenColor: { r: number; g: number; b: number } = this.hexToRgb(
        args.style.textPenColor
      );
      cell.style.textPen = new PdfPen(
        new PdfColor(textPenColor.r, textPenColor.g, textPenColor.b)
      );
    }

    if (
      !isNullOrUndefined(args.style.fontFamily) ||
      !isNullOrUndefined(args.style.fontSize) ||
      !isNullOrUndefined(args.style.bold) ||
      !isNullOrUndefined(args.style.italic) ||
      !isNullOrUndefined(args.style.underline) ||
      !isNullOrUndefined(args.style.strikeout)
    ) {
    ...
    if (!isNullOrUndefined(args.style.border)) {
      ...
      // set border width
      const width: number =
        !isNullOrUndefined(borderWidth) && typeof borderWidth === 'number'
          ? borderWidth * 0.75
          : undefined;
      // set border color
      let color: PdfColor = new PdfColor(196, 196, 196);
      if (!isNullOrUndefined(args.style.border.color)) {
        const borderColor: { r: number; g: number; b: number } = this.hexToRgb(
  ...
  private getFont(content: any): PdfFont {
    ...
    const fontSize: number = !isNullOrUndefined(content.style.fontSize)
      ? content.style.fontSize * 0.75
      : 9.75;
    const fontFamily: number = !isNullOrUndefined(content.style.fontFamily)
      ? this.getFontFamily(content.style.fontFamily)
    ...
    if (
      !isNullOrUndefined(content.style.underline) &&
    ...
    if (
      !isNullOrUndefined(content.style.strikeout) &&
      content.style.strikeout
    ) {
  ...
  private setContentFormat(
    ...
    format: PdfStringFormat
  ): { format: PdfStringFormat; size: SizeF } {
  ...
  private getPenFromContent(content: PdfHeaderFooterContent): PdfPen {
    ...
    if (
      !isNullOrUndefined(content.style) &&
      content.style !== null &&
      !isNullOrUndefined(content.style.penColor)
    ) {
      const penColor: { r: number; g: number; b: number } = this.hexToRgb(
        content.style.penColor
      );
      pen = new PdfPen(new PdfColor(penColor.r, penColor.g, penColor.b));
    }
    return pen;
