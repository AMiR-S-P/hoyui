...
import { remove, select, removeClass } from '@hoyui/base';
import { Toolbar as tool, ItemModel, ClickEventArgs } from '@hoyui/navigations';
import {
  IGrid,
  NotifyArgs,
  FocusInfo,
...
import { ContextMenu as Menu } from '@hoyui/navigations';
import { OffsetPosition, calculatePosition } from '@hoyui/popups';
...
export class Toolbar {
  ...
  private render(): void {
    ...
    for (const item of preItems) {
      ...
      if (isAdaptive) {
        ...
        this.predefinedItems[`${item}`].visible =
          excludingItems.indexOf(item) === -1;
      }
    }
    ...
    if (isAdaptive && this.isResponsiveToolbarMenuItems(false)) {
      (
        this.predefinedItems as { responsiveToolbarItems: ItemModel }
    ...
    if (this.parent.rowRenderingMode === 'Vertical') {
      if (
        ...
        (
          this.predefinedItems as { responsiveFilter: ItemModel }
        ).responsiveFilter = {
    ...
    if (
      this.parent.enableAdaptiveUI &&
      this.parent.toolbar &&
      this.parent.toolbar.some(
        (item: string | ItemModel | ToolbarItem) =>
  ...
  private isResponsiveToolbarMenuItems(isRight?: boolean): boolean {
    const items: string[] = isRight
      ? ['Add', 'Edit', 'Delete', 'Search']
      : ['Print', 'ColumnChooser', 'PdfExport', 'ExcelExport', 'CsvExport'];
    const toolbarItems: (ToolbarItems | string | ItemModel | ToolbarItem)[] =
      this.parent.toolbar || [];
    for (let i: number = 0; i < items.length; i++) {
      if (
        toolbarItems.indexOf(items[parseInt(i.toString(), 10)] as string) >= 0
      ) {
  ...
  public destroy(): void {
    if (this.toolbar && !this.toolbar.isDestroyed) {
      ...
      if (!this.toolbar.element) {
        ...
        this.toolbar.off(
          'render-react-toolbar-template',
          this.addReactToolbarPortals
        );
  ...
  private bindSearchEvents(): void {
    this.searchElement = <HTMLInputElement>(
      select('#' + this.gridID + '_searchbar', this.element)
    );
    this.wireEvent();
  ...
  private toolbarCreated(isNormal?: boolean): void {
    if (this.element.querySelector('.e-search-wrapper')) {
      if (!this.parent.enableAdaptiveUI || isNormal) {
        const classList: string = this.parent.cssClass
          ? 'e-input-group e-search ' + this.parent.cssClass
        ...
        this.element.querySelector('.e-search-wrapper').innerHTML =
          ...
          '" role="search">\
                    <input id="' +
          this.gridID +
          '_searchbar" class="e-input e-search" name="input" type="search" \
                    placeholder= "' +
  ...
  private createToolbar(): void {
    ...
    if (this.parent.toolbarTemplate) {
      ...
      if (
        ...
        !(
          isVue &&
          !document.querySelectorAll(this.parent.toolbarTemplate).length
        )
      ) {
        ...
        if (isReactCompiler || isReactChild) {
          templateCompiler(this.parent.toolbarTemplate)(
            {},
            this.parent,
            'toolbarTemplate',
            ID,
    ...
    const tlbrLeftElement: Element =
      this.element.querySelector('.e-toolbar-left');
    const tlbrCenterElement: Element =
      this.element.querySelector('.e-toolbar-center');
    const tlbrRightElement: Element =
      this.element.querySelector('.e-toolbar-right');
  ...
  private renderResponsiveSearch(isRender: boolean): void {
    if (isRender) {
      ...
      const right: HTMLElement = parentsUntil(
        this.searchElement,
        'e-toolbar-right'
      ) as HTMLElement;
      if (right) {
  ...
  private refreshResponsiveToolbarItems(action: ResponsiveToolbarAction): void {
    if (action === ResponsiveToolbarAction.isInitial) {
      ...
      for (const item of this.toolbar.items) {
        const toolbarEle: Element =
          (item.template as string) && (item.template as string).length
            ? parentsUntil(
                this.toolbar.element.querySelector('#' + item.id),
                'e-template'
              ).children[0]
            : this.toolbar.element.querySelector('#' + item.id);
        if (toolbarEle) {
    ...
    if (action === ResponsiveToolbarAction.isSearch) {
      const items: string[] = [
        this.parent.element.id + '_responsiveback',
        this.parent.element.id + '_search',
      ];
      for (const item of this.toolbar.items) {
        const toolbarEle: Element = this.toolbar.element.querySelector(
          '#' + item.id
        );
        if (toolbarEle) {
          if (items.indexOf(item.id) > -1) {
  ...
  private refreshToolbarItems(args?: {
    ...
    const addRow: boolean =
      edit.showAddNewRow && !gObj.element.querySelector('.e-editedrow');
    if (edit.allowAdding) {
    ...
    if (gObj.editSettings.mode === 'Batch') {
      if (
        gObj.element.getElementsByClassName('e-updatedtd').length &&
      ...
      if (
        (gObj.isEdit || edit.showAddNewRow) &&
        (edit.allowAdding || edit.allowEditing)
      ) {
        enableItems = addRow
          ? [
  ...
  private getItems(): ItemModel[] {
    ...
    if (
      this.parent.enableAdaptiveUI &&
      this.isResponsiveToolbarMenuItems(false)
    ) {
  ...
  private toolbarClickHandler(args: ClickEventArgs): void {
    ...
    if (
      this.parent.enableAdaptiveUI &&
      args.item.id === gID + '_responsivetoolbaritems'
    ) {
      ...
      gObj.trigger(
        events.toolbarClick,
        newArgs,
        (toolbarargs: ClickEventArgs) => {
          toolbarargs.originalEvent = toolbarargs.originalEvent
            ? toolbarargs.originalEvent
            : originalEvent;
          if (!toolbarargs.cancel) {
            switch (
              !isNullOrUndefined(toolbarargs.item) &&
              toolbarargs.item.id
            ) {
              case gID + '_print':
                gObj.print();
              ...
              case gID + '_search':
                if (
                  (<HTMLElement>toolbarargs.originalEvent.target).id ===
                    gID + '_searchbutton' &&
                  this.searchElement
                ) {
                  this.search();
                } else if (
                  gObj.enableAdaptiveUI &&
                  !this.searchElement &&
                  ((<HTMLElement>(
                    toolbarargs.originalEvent.target
                  )).classList.contains('e-search-wrapper') ||
                    (<HTMLElement>toolbarargs.originalEvent.target).id ===
                      gID + '_searchbutton')
                ) {
                  ...
                  (<HTMLElement>(
                    toolbarargs.originalEvent.target
                  )).classList.contains('e-clear-icon') &&
                  (<HTMLElement>toolbarargs.originalEvent.target).id ===
                    gID + '_clearbutton' &&
                  this.searchElement
                ) {
                  ...
                  if (this.searchElement) {
                    this.sIcon =
                      this.searchElement.parentElement.querySelector(
                        '.e-sicon'
                      );
              ...
              case gID + '_columnchooser':
                if (this.parent.enableAdaptiveUI) {
                  ...
                  /* eslint-disable */
                  const tarElement: Element =
                    this.parent.element.querySelector('.e-ccdiv');
                  let y: number = tarElement.getBoundingClientRect().top;
                  const x: number = tarElement.getBoundingClientRect().left;
                  const targetEle: Element = <HTMLElement>(
                    toolbarargs.originalEvent.target
                  );
                  /* eslint-enable */
                  y =
                    tarElement.getBoundingClientRect().top +
                    (<HTMLElement>tarElement).offsetTop;
  ...
  private openResponsiveToolbarMenuPopup(e: Event, id?: string): void {
    ...
    applyBiggerTheme(
      this.parent.element,
      this.responsiveToolbarMenu.element.parentElement
    );
  }
  ...
  private wireEvent(): void {
    if (this.searchElement) {
      this.searchBoxObj = new SearchBox(
        this.searchElement,
        this.serviceLocator
      );
  ...
  private onFocusIn(e: FocusEvent): void {
    ...
    if (currentInfo && currentInfo.element) {
      removeClass(
        [currentInfo.element, currentInfo.elementToFocus],
        ['e-focused', 'e-focus']
      );
    ...
    if (
      ...
      !(
        ...
        ((e.relatedTarget as HTMLElement).id ===
          this.parent.element.id + '_clearbutton' ||
          (e.relatedTarget as HTMLElement).id ===
            this.parent.element.id + '_searchbutton')
      )
    ) {
      this.search();
    }
  ...
  private setFocusToolbarItem(element: Element): void {
    let elementToFocus: Element = element.querySelector(
      '.e-btn,.e-input,.e-toolbar-item-focus'
    );
    if (
      !elementToFocus &&
      this.parent.enableAdaptiveUI &&
      ...
      elementToFocus = element.querySelector(
        '#' + this.gridID + '_searchbutton'
      );
    }
  ...
  public getFocusableToolbarItems(): Element[] {
    const getFocusToolbarElements: Element[] = [].slice.call(
    ...
    for (let i: number = 0; i < getFocusToolbarElements.length; i++) {
      if (
        !isNullOrUndefined(
          getFocusToolbarElements[parseInt(i.toString(), 10)].querySelector(
            '.e-btn,.e-input,.e-toolbar-item-focus'
          )
        )
      ) {
        getFocusToolbarItems.push(
          getFocusToolbarElements[parseInt(i.toString(), 10)]
        );
      }
    }
  ...
  private keyPressedHandler(e: KeyboardEventArgs): void {
    if (e.target && parentsUntil(e.target as Element, 'e-toolbar-item')) {
      const targetParent: Element = parentsUntil(
        e.target as Element,
        'e-toolbar-item'
      );
      ...
      if (e.action === 'tab' || e.action === 'shiftTab') {
        ...
        for (let i: number = 0; i < focusableToolbarItems.length; i++) {
          if (
            targetParent === focusableToolbarItems[parseInt(i.toString(), 10)]
          ) {
            e.preventDefault();
            const index: number = e.action === 'tab' ? i + 1 : i - 1;
            this.setFocusToolbarItem(
              focusableToolbarItems[parseInt(index.toString(), 10)]
            );
            return;
          }
        }
  ...
  protected addEventListener(): void {
    ...
    this.evtHandlers = [
      { event: events.setFullScreenDialog, handler: this.reRenderToolbar },
  ...
  private removeResponsiveSearch(e: MouseEventArgs): void {
    ...
    const isSearch: boolean =
      target.classList.contains('e-search-icon') ||
      target.classList.contains('e-search-wrapper');
    if (
      this.parent.enableAdaptiveUI &&
      !isSearch &&
      this.searchElement &&
