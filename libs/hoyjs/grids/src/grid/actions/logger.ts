...
import { DataUtil, DataManager, AdaptorOptions } from '@hoyui/data';
...
export class Logger implements ILogger {
  ...
  public log(types: string | string[], args: Object): void {
    ...
    for (let i: number = 0; i < type.length; i++) {
      ...
      if (cOp.success) {
        ...
        console[item.logType](
          item.generateMessage(args, this.parent, cOp.options)
        );
      }
    }
  }
  public patchadaptor(): void {
    const adaptor: AdaptorOptions =
      this.parent.getDataModule().dataManager.adaptor;
...
export const detailLists: { [key: string]: ItemDetails } = {
  ...
  module_missing: {
    ...
    logType: 'warn',
    check(args: Object, parent: IGrid): CheckOptions {
      const injected: string[] = parent
        .getInjectedModules()
        .map((m: Function) => m.prototype.getModuleName());
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const modules: string[] = (<any>parent)
        .requiredModules()
        .map((m: ModuleDeclaration) => m.member)
        .filter((name: string) => injected.indexOf(name) === -1);
      return {
        success: modules.filter((m: string) => m !== 'resize').length > 0,
        options: modules,
      };
    },
    generateMessage(args: Object, parent: IGrid, modules: Object): string {
      modules = (<string[]>modules)
        .filter((m: string) => m !== 'resize')
  ...
  primary_column_missing: {
    ...
    check(args: Object, parent: IGrid): CheckOptions {
      return {
        success: parent.enableColumnVirtualization
          ? parent.getPrimaryKeyFieldNames().length === 0
          : parent.getColumns().filter((column: Column) => column.isPrimaryKey)
              .length === 0,
      };
    },
    generateMessage(): string {
  ...
  selection_key_missing: {
    ...
    check(args: Object, parent: IGrid): CheckOptions {
      return {
        success:
          parent.selectionSettings.persistSelection &&
          parent.getColumns().filter((column: Column) => column.isPrimaryKey)
            .length === 0,
      };
    },
  ...
  locale_missing: {
    ...
    logType: 'warn',
    check(args: Object, parent: IGrid): CheckOptions {
      const lObj: Object = DataUtil.getObject(
        `locale.${parent.locale}.grid`,
        L10n
      );
  ...
  limitation: {
    ...
    check(args: Object, parent: IGrid): CheckOptions {
      ...
      switch (name) {
        case 'freeze':
          opt = {
            success:
              !isUndefined(parent.detailTemplate) ||
              !isUndefined(parent.childGrid),
            options: { name: 'freeze' },
          };
          break;
        case 'virtualization':
    ...
    generateMessage(args: Object, parent: IGrid, options: Object): string {
      ...
      switch (name) {
        case 'freeze':
          opt =
            'Frozen rows and columns do not support the following features:\n' +
            '* Details Template\n' +
            '* Hierarchy Grid\n';
          break;
        case 'virtualization':
          opt =
            'Virtualization does not support the following features.\n' +
            '* Details Template.\n' +
            '* Hierarchy Grid.\n';
          break;
        default:
    ...
    check(args: Object, parent: IGrid): CheckOptions {
      return {
        success: !(
          ...
          (!isNullOrUndefined(parent.dataSource) &&
            (<object[]>parent.dataSource).length)
        ),
      };
    },
    generateMessage(): string {
  ...
  resize_min_max: {
    ...
    check(args: { column: Column; width: number }): CheckOptions {
      return {
        success:
          (args.column.minWidth && args.column.minWidth >= args.width) ||
      ...
      switch (args.moduleName) {
        case 'reorder':
          if (isNullOrUndefined(args.destColumn)) {
            ...
            fn = `reordering action is disabled for the ${
              args.column.allowReordering
                ? args.destColumn.headerText
  ...
  initial_action: {
    ...
    check(args: { moduleName: string; columnName: string }): CheckOptions {
      ...
      switch (args.moduleName) {
        case 'group':
          fn =
            `The ${args.columnName} column is not available in the grid's column model.` +
  ...
  frozen_rows_columns: {
    ...
    check(args: string, parent: IGrid): CheckOptions {
      return {
        success:
          parent.getColumns().length <= parent.frozenColumns ||
          (parent.currentViewData.length &&
            parent.frozenRows >= parent.currentViewData.length),
      };
    },
    generateMessage(args: Object, parent: IGrid): string {
      return (
  ...
  datasource_syntax_mismatch: {
    ...
    check(args: { dataState: IGrid }): CheckOptions {
      return {
        success:
          args.dataState.dataSource &&
          !(
            args.dataState.dataSource instanceof DataManager ||
            'result' in args.dataState.dataSource ||
            args.dataState.dataSource instanceof Array
          ) &&
...
const ajaxErrorHandler: Function = (args: Object): string => {
  const error: XMLHttpRequest = <XMLHttpRequest>(
    DataUtil.getObject('error.error', args)
  );
