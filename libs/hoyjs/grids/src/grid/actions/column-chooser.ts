...
import { EventHandler, L10n, closest } from '@hoyui/base';
...
export class ColumnChooser implements IAction {
  ...
  private destroy(): void {
    ...
    this.unWireEvents();
    if (
      !isNullOrUndefined(this.dlgObj) &&
      this.dlgObj.element &&
      !this.dlgObj.isDestroyed
    ) {
      this.dlgObj.destroy();
    }
  }
  ...
  private setFullScreenDialog(): void {
    if (this.serviceLocator) {
      this.serviceLocator.registerAdaptiveService(
        ...
        ResponsiveDialogAction.isColumnChooser
      );
    }
    if (this.parent.enableAdaptiveUI) {
      this.parent.on(
        events.renderResponsiveColumnChooserDiv,
  ...
  private rtlUpdate(): void {
    if (!isNullOrUndefined(this.innerDiv)) {
      if (this.parent.enableRtl) {
        addClass(
          [].slice.call(
            this.innerDiv.getElementsByClassName('e-checkbox-wrapper')
          ),
          ['e-rtl']
        );
      } else {
  ...
  private clickHandler(e: MouseEvent): void {
    ...
    if (!this.isCustomizeOpenCC) {
      if (
        !isNullOrUndefined(closest(targetElement, '.e-cc-toolbar')) ||
        !isNullOrUndefined(closest(targetElement, '.e-cc'))
      ) {
        if (
          targetElement.classList.contains('e-columnchooser-btn') ||
          targetElement.classList.contains('e-cc-toolbar')
        ) {
          ...
          (<HTMLInputElement>(
            targetElement.parentElement.querySelector('.e-ccsearch')
          )).value = '';
        ...
        if (
          !isNullOrUndefined(this.dlgObj) &&
          this.dlgObj.visible &&
          !targetElement.classList.contains('e-toolbar-items')
        ) {
          this.dlgObj.hide();
          this.clearActions();
    ...
    if (
      ...
      (e.target as Element).classList.contains('e-cc-cancel')
    ) {
      this.refreshCheckboxState();
    }
    if (!this.parent.enableAdaptiveUI) {
  ...
  public renderColumnChooser(x?: number, y?: number, target?: Element): void {
    ...
    if (!this.dlgObj.visible) {
      ...
      const isSticky: boolean = this.parent
        .getHeaderContent()
        .classList.contains('e-sticky');
      const toolbarItem: HTMLElement = <HTMLElement>(
        closest(target, '.e-toolbar-item')
      );
      ...
      if (isSticky) {
        ...
        newpos = calculateRelativeBasedPosition(
          toolbarItem,
          this.dlgObj.element
        );
      }
      ...
      this.dlgObj.element.style.top =
        newpos.top +
        closest(target, '.e-cc-toolbar').getBoundingClientRect().height +
        'px';
      ...
      if (Browser.isDevice) {
        ...
        if (this.parent.enableRtl) {
          this.dlgObj.element.style.left =
            (<HTMLElement>target).offsetLeft + 'px';
        } else {
          this.dlgObj.element.style.left =
            newpos.left -
            dlgWidth +
            closest(target, '.e-cc-toolbar').clientWidth +
            2 +
            'px';
        }
      }
      ...
      if (
        (this.parent.getContent().firstElementChild as HTMLElement)
          .offsetHeight < this.dlgObj.element.offsetHeight &&
        !this.parent.element.classList.contains('e-drillthrough-grid')
      ) {
        resetDialogAppend(this.parent, this.dlgObj);
        (
          this.dlgObj.element.querySelector('.e-ccsearch') as HTMLInputElement
        ).select();
      }
  ...
  public openColumnChooser(X?: number, Y?: number): void {
    ...
    if (
      (this.parent.getContent().firstElementChild as HTMLElement).offsetHeight <
        this.dlgObj.element.offsetHeight &&
      !this.parent.element.classList.contains('e-drillthrough-grid')
    ) {
      resetDialogAppend(this.parent, this.dlgObj);
    }
  ...
  private customDialogOpen(): void {
    const searchElement: Element = (
      this.dlgObj.content as Element
    ).querySelector('input.e-ccsearch');
    EventHandler.add(
      searchElement,
      'keyup',
      this.columnChooserManualSearch,
      this
    );
  }
  ...
  private getColumns(): Column[] {
    const columns: Column[] = this.parent
      ...
      .filter(
        (column: Column) =>
          (column.type !== 'checkbox' && column.showInColumnChooser === true) ||
  ...
  private renderDlgContent(): void {
    ...
    this.dlgObj = new Dialog({
      header: this.parent.enableAdaptiveUI
        ? null
        : this.l10n.getConstant('ChooseColumns'),
      showCloseIcon: false,
      closeOnEscape: false,
      ...
      this.dlgObj.buttons = [
        {
          ...
          buttonModel: {
            ...
            isPrimary: true,
            cssClass: this.parent.cssClass
              ? 'e-cc e-cc_okbtn' + ' ' + this.parent.cssClass
              : 'e-cc e-cc_okbtn',
          },
        },
        {
    ...
    if (isAdaptive) {
      ...
      this.dlgObj.target = document.querySelector(
        '.e-rescolumnchooser > .e-dlg-content > .e-mainfilterdiv'
      ) as HTMLElement;
    }
    this.wireEvents();
  }
  ...
  private renderChooserList(): HTMLElement {
    ...
    const searchDiv: HTMLElement = this.parent.createElement('div', {
    ...
    const conDiv: HTMLElement = this.parent.createElement('div', {
    ...
    const innerDivContent: HTMLElement | string[] | string =
      this.refreshCheckboxList(this.parent.getColumns() as Column[]);
    this.innerDiv.appendChild(innerDivContent as Element);
    conDiv.appendChild(this.innerDiv);
    if (this.parent.enableAdaptiveUI) {
      const searchBoxDiv: HTMLElement = this.parent.createElement('div', {
  ...
  private confirmDlgBtnClick(args: Object): void {
    ...
    if (!isNullOrUndefined(args)) {
      if (uncheckedLength < this.parent.getColumns().length) {
        ...
        this.getShowHideService.setVisible(
          this.stateChangeColumns,
          this.changedStateColumns
        );
        this.clearActions();
        ...
        if (this.parent.getCurrentViewRecords().length === 0) {
          const emptyRowCell: HTMLElement = this.parent.element
            .querySelector('.e-emptyrow')
            .querySelector('td');
          emptyRowCell.setAttribute(
            'colSpan',
            this.parent.getVisibleColumns().length.toString()
          );
        }
      }
  ...
  private checkstatecolumn(
    ...
    if (isChecked) {
      ...
      if (
        this.showColumn.indexOf(coluid) === -1 &&
        !(currentCol && currentCol.visible)
      ) {
      ...
      if (
        this.hideColumn.indexOf(coluid) === -1 &&
        currentCol &&
        currentCol.visible
      ) {
  ...
  private columnChooserSearch(searchVal: string): void {
    ...
    let okButton: Button;
    const buttonEle: HTMLElement =
      this.dlgDiv.querySelector('.e-footer-content');
    const selectedCbox: number =
      this.ulElement.querySelector('.e-check') &&
      this.ulElement.querySelectorAll('.e-check:not(.e-selectall)').length;
    this.isInitialOpen = true;
    if (buttonEle) {
      okButton = (buttonEle.querySelector('.e-btn') as EJ2Intance)
    ...
    if (searchVal === '') {
      ...
      this.filterColumns = new DataManager(
        this.getColumns() as Object[] as JSON[]
      ).executeLocal(
        new Query().where(
          'headerText',
          this.searchOperator,
          searchVal,
          true,
          this.parent.columnChooserSettings.ignoreAccent
        )
      ) as Column[];
    }
    ...
    if (this.filterColumns.length) {
      ...
      this.innerDiv.appendChild(
        <HTMLElement>this.refreshCheckboxList(this.filterColumns)
      );
      if (!clearSearch) {
        this.addcancelIcon();
        this.refreshCheckboxButton();
      ...
      const nMatchele: HTMLElement = this.parent.createElement('span', {
  ...
  private checkBoxClickHandler(e: MouseEvent): void {
    ...
    const elem: Element = parentsUntil(
      e.target as Element,
      'e-checkbox-wrapper'
    );
    if (elem) {
      ...
      this.updateIntermediateBtn();
      const columnUid: string = parentsUntil(elem, 'e-ccheck').getAttribute(
        'uid'
      );
      const column: Column[] =
      ...
      if (columnUid === this.parent.element.id + '-selectAll') {
        ...
        for (let i: number = 0; i < column.length; i++) {
          if (column[parseInt(i.toString(), 10)].showInColumnChooser) {
            this.checkstatecolumn(
              checkstate,
              column[parseInt(i.toString(), 10)].uid,
              true
            );
          }
        }
  ...
  private updateIntermediateBtn(): void {
    ...
    const selected: number = this.ulElement.querySelectorAll(
      '.e-check:not(.e-selectall)'
    ).length;
    let btn: Button;
    if (!this.parent.enableAdaptiveUI) {
  ...
  private updateSelectAll(checked: boolean): void {
    const cBoxes: Element[] = [].slice.call(
      this.ulElement.getElementsByClassName('e-frame')
    );
    for (const cBox of cBoxes) {
      removeAddCboxClasses(cBox, checked);
      const cBoxInput: HTMLInputElement =
        cBox.parentElement.querySelector('input');
  ...
  private refreshCheckboxButton(): void {
    ...
    for (let i: number = 0; i < visibleCols.length; i++) {
      ...
      if (
        this.prevShowedCols.indexOf(columnUID) === -1 &&
        visibleCols[parseInt(i.toString(), 10)].type !== 'checkbox'
      ) {
        this.prevShowedCols.push(columnUID);
      }
    }
    for (let i: number = 0; i < this.hideColumn.length; i++) {
      const index: number = this.prevShowedCols.indexOf(
        this.hideColumn[parseInt(i.toString(), 10)]
      );
      if (index !== -1) {
    ...
    const selected: number =
      this.showColumn.length !== 0 ? 1 : this.prevShowedCols.length;
    let btn: Button;
    if (!this.parent.enableAdaptiveUI) {
      btn = (
        this.dlgDiv
          .querySelector('.e-footer-content')
          .querySelector('.e-btn') as EJ2Intance
      ).ej2_instances[0] as Button;
      btn.disabled = false;
    } else if (this.parent.enableAdaptiveUI && this.responsiveDialogRenderer) {
      this.parent.notify(events.refreshCustomFilterOkBtn, { disabled: false });
    ...
    const searchData: NodeListOf<HTMLInputElement> = [].slice.call(
      this.parent.element.getElementsByClassName('e-cc-chbox')
    );
    for (
      let i: number = 0, itemsLen: number = searchData.length;
      i < itemsLen;
      i++
    ) {
      const element: HTMLInputElement = searchData[
        parseInt(i.toString(), 10)
      ] as HTMLInputElement;
      const columnUID: string = parentsUntil(element, 'e-ccheck').getAttribute(
        'uid'
      );
      srchShowCols.push(columnUID);
    }
    const hideCols: string[] = this.showColumn.filter(
      (column: string) => srchShowCols.indexOf(column) !== -1
    );
  ...
  private refreshCheckboxList(gdCol: Column[]): HTMLElement {
    ...
    const cclist: HTMLElement = this.parent.createElement('li', {
      className: 'e-cclist e-cc e-cc-selectall',
    });
    const selectAll: Element = this.createCheckBox(
      selectAllValue,
      false,
      this.parent.element.id + '-selectAll'
    );
    if (gdCol.length) {
      selectAll
        .querySelector('.e-checkbox-wrapper')
        .firstElementChild.classList.add('e-selectall');
  ...
  private refreshCheckboxState(): void {
    (<HTMLInputElement>(
      this.dlgObj.element.querySelector('.e-cc.e-input')
    )).value = '';
    this.columnChooserSearch('');
    const gridObject: IGrid = this.parent;
    const currentCheckBoxColls: NodeListOf<Element> =
      this.dlgObj.element.querySelectorAll('.e-cc-chbox:not(.e-selectall)');
    for (
      let i: number = 0, itemLen: number = currentCheckBoxColls.length;
      i < itemLen;
      i++
    ) {
      const element: HTMLInputElement = currentCheckBoxColls[
        parseInt(i.toString(), 10)
      ] as HTMLInputElement;
      ...
      const uncheck: NodeListOf<Element> = [].slice.call(
        element.parentElement.getElementsByClassName('e-uncheck')
      );
  ...
  private renderCheckbox(column: Column): void {
    ...
    if (column.showInColumnChooser) {
      ...
      const cccheckboxlist: Element = this.createCheckBox(
        column.headerText,
        (column.visible && !hideColState) || showColState,
        column.uid
      );
  ...
  private addcancelIcon(): void {
    this.dlgDiv
      .querySelector('.e-cc.e-ccsearch-icon')
      .classList.add('e-cc-cancel');
    this.dlgDiv
      .querySelector('.e-cc-cancel')
      .setAttribute('title', this.l10n.getConstant('Clear'));
  }

  private removeCancelIcon(): void {
    this.dlgDiv
      .querySelector('.e-cc.e-ccsearch-icon')
      .classList.remove('e-cc-cancel');
    this.dlgDiv
      .querySelector('.e-cc.e-ccsearch-icon')
      .setAttribute('title', this.l10n.getConstant('Search'));
  }

  private mOpenDlg(): void {
    if (Browser.isDevice) {
      this.dlgObj.element
        .querySelector('.e-cc-searchdiv')
        .classList.remove('e-input-focus');
      (<HTMLElement>(
        this.dlgObj.element.querySelectorAll('.e-cc-chbox')[0]
      )).focus();
    }
    if (this.parent.enableAdaptiveUI) {
      this.dlgObj.element
        .querySelector('.e-cc-searchdiv')
        .classList.add('e-input-focus');
    }
  }
  ...
  private hideOpenedDialog(): void {
    const openCC: Element[] = [].slice
      .call(document.getElementsByClassName('e-ccdlg'))
      .filter((dlgEle: Element) => dlgEle.classList.contains('e-popup-open'));
    for (let i: number = 0, dlgLen: number = openCC.length; i < dlgLen; i++) {
      if (
        this.parent.element.id + '_ccdlg' !==
          openCC[parseInt(i.toString(), 10)].id ||
        openCC[parseInt(i.toString(), 10)].classList.contains('e-dialog')
      ) {
        (
          openCC[parseInt(i.toString(), 10)] as EJ2Intance
        ).ej2_instances[0].hide();
      }
    }
  }
