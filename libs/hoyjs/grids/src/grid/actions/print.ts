...
export class Print {
  ...
  public static printGridProp: string[] = [
    ...getCloneProperties(),
    events.beforePrint,
    events.printComplete,
    events.load,
  ];
  ...
  private isContentReady(): Function {
    if (
      this.isPrintGrid() &&
      (this.parent.hierarchyPrintMode === 'None' || !this.parent.childGrid)
    ) {
  ...
  private hierarchyPrint(): void {
    ...
    if (
      printGridObj &&
      !printGridObj.element.querySelector('[aria-busy=true')
    ) {
  ...
  private renderPrintGrid(): void {
    ...
    const printGrid: IGrid = new Grid(
      getPrintGridModel(gObj, gObj.hierarchyPrintMode) as Object
    );
    for (let i: number = 0; i < printGrid.columns.length; i++) {
      (printGrid.columns[parseInt(i.toString(), 10)] as Column) = extend(
        {},
        printGrid.columns[parseInt(i.toString(), 10)]
      ) as Column;
      if (!gObj.getFrozenColumns() && gObj.isFrozenGrid()) {
        (printGrid.columns[parseInt(i.toString(), 10)] as Column).freeze =
          undefined;
      }
    }
    ...
    if ((this.parent as any).isAngular) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (printGrid as any).viewContainerRef = (
        this.parent as any
      ).viewContainerRef;
    }
  ...
  private printGrid(): void {
    ...
    if (gObj.height !== 'auto') {
      ...
      const contentDiv: HTMLElement = gObj.element.querySelector(
        '.' + literals.content
      ) as HTMLElement;
      const headerDiv: HTMLElement = <HTMLElement>(
        gObj.element.querySelector('.' + literals.gridHeader)
      );
    ...
    if (gObj.allowGrouping) {
      if (!gObj.groupSettings.columns.length) {
        (
          gObj.element.querySelector('.e-groupdroparea') as HTMLElement
        ).style.display = 'none';
      } else {
    ...
    for (const element of [].slice.call(
      gObj.element.getElementsByClassName(literals.content)
    )) {
      element.style.overflowX = 'hidden';
    }
    // Hide the waiting popup
    const waitingPop: NodeListOf<Element> = [].slice.call(
      gObj.element.getElementsByClassName('e-spin-show')
    );
  ...
  private removeColGroup(gObj: IGrid): void {
    ...
    const colSpan: string = (<HTMLElement>groupCaption[depth - 1]).getAttribute(
      'colspan'
    );
    for (let i: number = 0; i < groupCaption.length; i++) {
      (<HTMLElement>groupCaption[parseInt(i.toString(), 10)]).setAttribute(
        'colspan',
        colSpan
      );
    }
    const colGroups: HTMLElement[] = selectAll(
      `colgroup${id}colgroup`,
      element
    );
    const contentColGroups: HTMLElement[] = selectAll(
      '.e-content colgroup',
      element
    );
    const footerColGroups: HTMLElement[] = selectAll(
      '.e-summarycontent colgroup',
      element
    );
  ...
  private hideColGroup(colGroups: HTMLElement[], depth: number): void {
    for (let i: number = 0; i < colGroups.length; i++) {
      for (let j: number = 0; j < depth; j++) {
        (<HTMLElement>(
          (<HTMLElement>colGroups[parseInt(i.toString(), 10)]).children[
  ...
  public isPrintGrid(): boolean {
    return (
      this.parent.element.id.indexOf('_print') > 0 && this.parent.isPrinting
    );
  }
