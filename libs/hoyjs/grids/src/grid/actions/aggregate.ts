import { remove, extend, getValue } from '@hoyui/base';
...
export class Aggregate implements IAction {
  ...
  private initiateRender(): void {
    ...
    const instance: ICellRenderer<{}> = new SummaryCellRenderer(
      this.parent,
      this.locator
    );
    const type: CellType[] = [
    ...
    if (
      this.parent.element.scrollHeight >=
        (this.parent as Grid).getHeight(this.parent.height) &&
      footerContent
    ) {
      ...
      if (typeof column.format === 'object') {
        ...
        column.setFormatter(
          valueFormatter.getFormatFunction(
            extend({}, column.format as DateFormatOptions)
          )
        );
  ...
  public onPropertyChanged(e: NotifyArgs): void {
    ...
    const cModel: CaptionSummaryModelGenerator =
      new CaptionSummaryModelGenerator(this.parent);
    const gModel: GroupSummaryModelGenerator = new GroupSummaryModelGenerator(
      this.parent
    );
 ...
 * @private
 */
export function summaryIterator(
  ...
  for (let i: number = 0; i < aggregates.length; i++) {
    for (
      let j: number = 0;
      j < aggregates[parseInt(i.toString(), 10)].columns.length;
      j++
    ) {
      callback(
        aggregates[parseInt(i.toString(), 10)].columns[
