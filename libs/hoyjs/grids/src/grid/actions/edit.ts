...
import { extend, getValue } from '@hoyui/base';
...
import { calculateRelativeBasedPosition, OffsetPosition } from '@hoyui/popups';
...
export class Edit implements IAction {
  ...
  private updateColTypeObj(): void {
    const cols: Column[] = (<{ columnModel?: Column[] }>this.parent)
      .columnModel;
    for (let i: number = 0; i < cols.length; i++) {
      if (
        this.parent.editSettings.template ||
        cols[parseInt(i.toString(), 10)].editTemplate
      ) {
        const templteCell: string = 'templateedit';
        cols[parseInt(i.toString(), 10)].edit = extend(
          new Edit.editCellType[`${templteCell}`](this.parent),
          cols[parseInt(i.toString(), 10)].edit || {}
        );
      } else {
        cols[parseInt(i.toString(), 10)].edit = extend(
          new Edit.editCellType[
            cols[parseInt(i.toString(), 10)].editType &&
  ...
  public onPropertyChanged(e: NotifyArgs): void {
    ...
    for (const prop of Object.keys(e.properties)) {
      switch (prop) {
        ...
        case 'allowEditing':
          if (
            gObj.editSettings.allowAdding ||
            gObj.editSettings.allowEditing ||
            gObj.editSettings.allowDeleting
          ) {
            this.initialEnd();
          }
          break;
  ...
  private updateEditObj(): void {
    ...
    this.editModule = new this.editType[this.parent.editSettings.mode](
      this.parent,
      this.serviceLocator,
      this.renderer
    );
  ...
  public startEdit(tr?: HTMLTableRowElement): void {
    ...
    if (
      ...
      (gObj.isEdit &&
        (!gObj.editSettings.showAddNewRow ||
          (gObj.editSettings.showAddNewRow &&
            !isNullOrUndefined(
              gObj.element.querySelector('.' + literals.editedRow)
            )))) ||
    ...
    if (
      ...
      isNullOrUndefined(
        this.parent.getRowByIndex(
          parseInt(
            this.parent.getSelectedRows()[0].getAttribute('data-rowindex'),
            10
          )
        )
      )
    ...
    const lastTr: HTMLTableRowElement = gObj
      .getContent()
      .querySelector('tr:last-child') as HTMLTableRowElement;
    const hdrTbody: Element = gObj.getHeaderContent().querySelector('tbody');
    if (
      gObj.frozenRows &&
      isNullOrUndefined(lastTr) &&
      hdrTbody &&
      hdrTbody.querySelector('tr:last-child')
    ) {
      this.isLastRow =
        tr.rowIndex ===
        parseInt(
    ...
    this.refreshToolbar();
    (gObj.element.querySelector('.e-gridpopup') as HTMLElement).style.display =
      'none';
  ...
  public checkLastRow(
    tr: Element,
    args?: { row?: Element; requestType?: string }
  ): void {
    ...
    if (
      ...
      (this.parent.getContent().firstElementChild as HTMLElement).offsetHeight >
        this.parent.getContentTable().scrollHeight
    ) {
      addClass(
        [].slice.call(tr.getElementsByClassName(literals.rowCell)),
        'e-lastrowadded'
      );
  ...
  public deleteRecord(fieldname?: string, data?: Object): void {
    ...
    if (!data) {
      if (
        !gObj.getSelectedRecords().length &&
        isNullOrUndefined(gObj.commandDelIndex)
      ) {
  ...
  public getBatchChanges(): Object {
    return this.editModule.getBatchChanges
      ? this.editModule.getBatchChanges()
      : {};
  }
  ...
  public getValueFromType(
    ...
    switch (col.type) {
      case 'number':
        val = !isNaN(parseFloat(value as string))
      ...
      case 'boolean':
        if (col.editType !== 'booleanedit') {
          val =
            value === this.l10n.getConstant('True') || value === true
              ? true
              : false;
        }
        break;
      ...
      case 'datetime':
        if (
          ...
          col.editType !== 'datetimepickeredit' &&
          value &&
        ...
        val =
          value && (value = new Date(value as string | Date))
            ? (value as Date).getFullYear() +
              '-' +
              padZero((value as Date).getMonth() + 1) +
              '-' +
              padZero((value as Date).getDate())
            : null;
        break;
    }
    return val;
  ...
  private destroyToolTip(): void {
    const elements: Element[] = [].slice.call(
      this.parent.element.getElementsByClassName('e-griderror')
    );
    for (const elem of elements) {
      remove(elem);
    }
    (this.parent.getContent().firstElementChild as HTMLElement).style.position =
      'relative';
    if (this.parent.isFrozenGrid()) {
      if (this.parent.element.querySelector('.e-gridheader')) {
        (
          this.parent.element.querySelector('.e-gridheader') as HTMLElement
        ).style.position = '';
      }
      (
        this.parent.element.querySelector('.e-gridcontent') as HTMLElement
      ).style.position = '';
    }
  }
  ...
  public getCurrentEditedData(form: Element, editedData: Object): Object {
    ...
    if (gObj.editSettings.template) {
      const elements: HTMLInputElement[] = [].slice.call(
        (<HTMLFormElement>form).elements
      );
      for (let k: number = 0; k < elements.length; k++) {
        if (
          ((elements[parseInt(k.toString(), 10)].hasAttribute('name') &&
            elements[parseInt(k.toString(), 10)].className !==
              'e-multi-hidden') ||
            elements[parseInt(k.toString(), 10)].classList.contains(
              'e-multiselect'
            )) &&
          !(
            elements[parseInt(k.toString(), 10)].type === 'hidden' &&
            (parentsUntil(
              elements[parseInt(k.toString(), 10)],
              'e-switch-wrapper'
            ) ||
              parentsUntil(
                elements[parseInt(k.toString(), 10)],
          ...
          const field: string = elements[
            parseInt(k.toString(), 10)
          ].hasAttribute('name')
            ? setComplexFieldID(
                elements[parseInt(k.toString(), 10)].getAttribute('name')
              )
            : setComplexFieldID(
                elements[parseInt(k.toString(), 10)].getAttribute('id')
              );
          const column: Column =
            gObj.getColumnByField(field) ||
            ({
              field: field,
          ...
          if (column.type === 'checkbox' || column.type === 'boolean') {
            ...
            if (
              (<EJ2Intance>(elements[parseInt(k.toString(), 10)] as Element))
                .ej2_instances &&
              (<Object[]>(
                (<EJ2Intance>(elements[parseInt(k.toString(), 10)] as Element))
                  .ej2_instances
              ...
              value = (
                (<EJ2Intance>(elements[parseInt(k.toString(), 10)] as Element))
                  .ej2_instances[0] as { value?: string | boolean | Date }
              ).value;
            }
          } else if (
            (<EJ2Intance>(elements[parseInt(k.toString(), 10)] as Element))
          ...
          if (column.edit && typeof column.edit.read === 'string') {
            value = getValue(column.edit.read, window)(
              elements[parseInt(k.toString(), 10)],
              value
            );
          } else if (column.edit && column.edit.read) {
            value = (column.edit.read as Function)(
              elements[parseInt(k.toString(), 10)],
              value
            );
          }
    ...
    const col: Column[] = (<{ columnModel?: Column[] }>gObj).columnModel.filter(
      (col: Column) => col.editTemplate
    );
    for (let j: number = 0; j < col.length; j++) {
      if (form[getComplexFieldID(col[parseInt(j.toString(), 10)].field)]) {
        let inputElements: HTMLInputElement[] = [].slice
          .call(form[getComplexFieldID(col[parseInt(j.toString(), 10)].field)])
          .filter(
        ...
        inputElements = inputElements.length
          ? inputElements
          : [form[getComplexFieldID(col[parseInt(j.toString(), 10)].field)]];
        let temp: HTMLInputElement[] = inputElements.filter(
          (e: HTMLInputElement) =>
            !isNullOrUndefined((<EJ2Intance>(e as Element)).ej2_instances)
        );
        if (temp.length === 0) {
          temp = inputElements.filter((e: HTMLInputElement) =>
            e.hasAttribute('name')
          );
        }
        for (let k: number = 0; k < temp.length; k++) {
          let value: number | string | Date | boolean = this.getValue(
            col[parseInt(j.toString(), 10)],
            temp[parseInt(k.toString(), 10)],
            editedData
          );
          ...
          DataUtil.setValue(
            col[parseInt(j.toString(), 10)].field,
            value,
            editedData
          );
        }
    ...
    const inputs: HTMLInputElement[] = [].slice.call(
      form.getElementsByClassName('e-field')
    );
    for (let i: number = 0, len: number = inputs.length; i < len; i++) {
      const col: Column = gObj.getColumnByUid(
        inputs[parseInt(i.toString(), 10)].getAttribute('e-mappinguid')
      );
      if (col && col.field) {
        let value: number | string | Date | boolean = this.getValue(
          col,
          inputs[parseInt(i.toString(), 10)],
          editedData
        );
        if (
          col.type === 'string' &&
          !(col.isForeignColumn() && typeof value !== 'string')
        ) {
          value = this.parent.sanitize(value as string);
        }
  ...
  private getValue(
    ...
    let value: string | boolean | number | Date = (<EJ2Intance>(
      (input as Element)
    )).ej2_instances
      ? (<EJ2Intance>(input as Element)).ej2_instances[0].value
      : input.value;
    const gObj: IGrid = this.parent;
    ...
    if (typeof temp === 'string') {
      ...
      value = gObj.editModule.getValueFromType(
        col,
        (col.edit.read as Function)(input, value)
      );
    }
  ...
  public onActionBegin(e: NotifyArgs): void {
    if (
      ...
      const editRow: Element = this.parent.element.querySelector(
        '.' + literals.editedRow
      );
      const addRow: Element = this.parent.element.querySelector(
        '.' + literals.addedRow
      );
      if (
        editRow &&
        ...
        parseInt(
          parentsUntil(editRow, literals.row).getAttribute(
      ...
      const restrictedRequestTypes: string[] = [
        'filterAfterOpen',
        'filterBeforeOpen',
        'filterchoicerequest',
        'filterSearchBegin',
  ...
  public destroyWidgets(cols?: Column[]): void {
    ...
    cols = cols
      ? cols
      : (this.parent.getCurrentVisibleColumns(
          this.parent.enableColumnVirtualization
        ) as Column[]);
    if (
      cols.some((column: Column) => !isNullOrUndefined(column.editTemplate))
    ) {
    ...
    const elements: HTMLInputElement[] = [].slice.call(
      (<HTMLFormElement>this.formObj.element).elements
    );
    for (let i: number = 0; i < elements.length; i++) {
      if (elements[parseInt(i.toString(), 10)].hasAttribute('name')) {
        const instanceElement: HTMLInputElement = elements[
          parseInt(i.toString(), 10)
        ].parentElement.classList.contains('e-ddl')
          ? elements[parseInt(i.toString(), 10)].parentElement.querySelector(
              'input'
            )
          : elements[parseInt(i.toString(), 10)];
        if (
          (<EJ2Intance>(instanceElement as Element)).ej2_instances &&
          (<Object[]>(<EJ2Intance>(instanceElement as Element)).ej2_instances)
            .length &&
          !(<EJ2Intance>(instanceElement as Element)).ej2_instances[0]
            .isDestroyed
        ) {
  ...
  public destroyForm(): void {
    ...
    const col: Column[] = (<{ columnModel?: Column[] }>(
      this.parent
    )).columnModel.filter((col: Column) => col.editTemplate);
    for (let i: number = 0; i < formObjects.length; i++) {
      if (
        formObjects[parseInt(i.toString(), 10)] &&
        formObjects[parseInt(i.toString(), 10)].element &&
  ...
  private keyPressHandler(e: KeyboardEventArgs): void {
    ...
    switch (e.action) {
      ...
      case 'delete':
        if (
          ((e.target as HTMLElement).tagName !== 'INPUT' ||
            (e.target as HTMLElement).classList.contains('e-checkselect')) &&
      ...
      case 'enter':
        if (
          !parentsUntil(e.target as HTMLElement, 'e-unboundcelldiv') &&
          this.parent.editSettings.mode !== 'Batch' &&
  ...
  private curretRowFocus(e: KeyboardEventArgs): void {
    if (this.parent.isEdit && this.parent.editSettings.mode !== 'Batch') {
      ...
      const editedRow: Element =
        parentsUntil(e.target as Element, 'e-editedrow') ||
        parentsUntil(e.target as Element, 'e-addedrow');
      if (editedRow) {
        let focusableEditCells: HTMLElement[] = [].slice.call(
          editedRow.querySelectorAll('.e-input:not(.e-disabled)')
        );
        const commandColCell: HTMLElement[] = [].slice.call(
          editedRow.querySelectorAll('.e-unboundcell')
        );
        if (commandColCell && commandColCell.length) {
          for (let i: number = 0; i < commandColCell.length; i++) {
            focusableEditCells = focusableEditCells.concat(
              [].slice.call(
                commandColCell[parseInt(i.toString(), 10)].querySelectorAll(
                  '.e-btn:not(.e-hide)'
                )
              )
            );
        ...
        rowCell =
          rowCell && rowCell.classList.contains('e-unboundcell')
            ? (e.target as Element)
            : rowCell;
        let lastCell: Element = parentsUntil(
          focusableEditCells[focusableEditCells.length - 1],
          'e-rowcell'
        );
        lastCell =
          lastCell && lastCell.classList.contains('e-unboundcell')
            ? (focusableEditCells[focusableEditCells.length - 1] as Element)
            : lastCell;
        if (
          (rowCell === lastCell && e.action === 'tab') ||
          e.action === 'escape' ||
          (rowCell === parentsUntil(focusableEditCells[0], 'e-rowcell') &&
            e.action === 'shiftTab' &&
            !this.parent.editSettings.showAddNewRow)
        ) {
          const uid: string = editedRow.getAttribute('data-uid');
          const rows: Element[] = this.parent.allowGrouping
            ? !isNullOrUndefined(this.parent.getContent())
              ? [].slice.call(this.parent.getContent().querySelectorAll('tr'))
              : []
            : this.parent.getRows();
          let rowIndex: number = rows
          ...
          if (
            ...
            (!this.parent.editSettings.showAddNewRow ||
              editedRow.classList.contains('e-editedrow') ||
              (this.parent.editSettings.showAddNewRow &&
                editedRow.classList.contains('e-addedrow') &&
                isNullOrUndefined(
            ...
            if (
              ...
              matrix.matrix[parseInt(rowIndex.toString(), 10)]
            ) {
            ...
            this.parent.focusModule.active.matrix.current = [
              rowIndex,
              firstCellIndex,
            ];
          }
  ...
  public applyFormValidation(cols?: Column[], newRule?: Object): void {
    ...
    const form: HTMLFormElement =
      this.parent.editSettings.mode !== 'Dialog'
        ? gObj.editSettings.showAddNewRow &&
          gObj.element.querySelector('.' + literals.editedRow)
          ? (gObj.element
              .querySelector('.' + literals.editedRow)
              .getElementsByClassName('e-gridform')[
              parseInt(idx.toString(), 10)
            ] as HTMLFormElement)
          : (gObj.element.getElementsByClassName('e-gridform')[
              parseInt(idx.toString(), 10)
            ] as HTMLFormElement)
        : (select(
            '#' + gObj.element.id + '_dialogEdit_wrapper .e-gridform',
            document
          ) as HTMLFormElement);
    ...
    for (let i: number = 0; i < cols.length; i++) {
      if (
        ...
        (gObj.editSettings.mode !== 'Dialog' ||
          (gObj.groupSettings.columns.indexOf(
    ...
    this.parent.editModule.formObj = this.createFormObj(
      form,
      newRule ? newRule : rules
    );
  }
      ...
      customPlacement: (inputElement: HTMLElement, error: HTMLElement) => {
        ...
        if (
          (!(
            event &&
            event['relatedTarget'] &&
            event['relatedTarget'].classList.contains('e-cancelbutton')
          ) &&
            !this.parent.editSettings.showAddNewRow) ||
          (this.parent.editSettings.showAddNewRow &&
            event &&
            ...
            (parentsUntil(
              ...
              this.parent.element.id + '_update',
              true
            ) ||
              (parentsUntil(event.target as Element, 'e-grid-menu') &&
                ((event.target as Element).classList.contains('e-save') ||
                  (event.target as Element).querySelector('.e-save'))) ||
              this.isShowAddedRowValidate ||
              (parentsUntil(event.target as Element, 'e-unboundcell') &&
                parentsUntil(event.target as Element, 'e-update')) ||
              (event['action'] === 'enter' &&
                (parentsUntil(event.target as Element, 'e-content') ||
                  parentsUntil(event.target as Element, 'e-addedrow'))))) ||
          (this.parent.editSettings.showAddNewRow &&
            !isNullOrUndefined(
              this.parent.element.querySelector('.' + literals.editedRow)
            ))
        ) {
  ...
  private valErrorPlacement(
    ...
    if (this.parent.isEdit) {
      ...
      const elem: Element = this.getElemTable(inputElement).querySelector(
        '#' + getParsedFieldID(id) + '_Error'
      );
      if (!elem) {
  ...
  private getElemTable(inputElement: Element): Element {
    ...
    if (gObj.editSettings.mode !== 'Dialog') {
      isFrozenHdr =
        gObj.frozenRows &&
        closest(inputElement, '.' + literals.row) &&
        gObj.frozenRows >
          (parseInt(
            closest(inputElement, '.' + literals.row).getAttribute(
              literals.dataRowIndex
            ),
            10
          ) || 0);
      table = this.parent.isFrozenGrid()
        ? gObj.element
        : isFrozenHdr ||
          (gObj.editSettings.showAddNewRow &&
            (gObj.enableVirtualization || gObj.enableInfiniteScrolling))
        ? gObj.getHeaderTable()
  ...
  private validationComplete(args: {
    ...
    if (this.parent.isEdit) {
      const elem: HTMLElement = this.getElemTable(args.element).querySelector(
        '#' + getParsedFieldID(args.inputName) + '_Error'
      ) as HTMLElement;
      if (this.parent.editSettings.showAddNewRow && !elem && args.element) {
        const error: Element = parentsUntil(
          args.element,
          'e-rowcell'
        ).querySelector('.e-error');
        if (error) {
  ...
  private createTooltip(
    ...
    const gcontent: HTMLElement = this.parent.getContent()
      .firstElementChild as HTMLElement;
    const isScroll: boolean =
      gcontent.scrollHeight > gcontent.clientHeight ||
      gcontent.scrollWidth > gcontent.clientWidth;
    ...
    const isAddNewRow: boolean =
      this.parent.editSettings.showAddNewRow &&
      !isNullOrUndefined(parentsUntil(element, literals.addedRow)) &&
      (this.parent.enableVirtualization || this.parent.enableInfiniteScrolling);
    const viewPortRowCount: number =
      Math.round(
        this.parent.getContent().clientHeight / this.parent.getRowHeight()
      ) - 1;
    let rows: Element[] = [].slice.call(
      this.parent.getContent().getElementsByClassName(literals.row)
    );
    if (this.parent.editSettings.mode === 'Batch') {
      rows = [].slice.call(
        this.parent.getContent().querySelectorAll('.e-row:not(.e-hiddenrow)')
      );
      if (
        viewPortRowCount >= 1 &&
        rows.length >= viewPortRowCount &&
        rows[rows.length - 1].getAttribute(literals.dataRowIndex) ===
    ...
    if (isInline) {
      if (this.parent.frozenRows || isAddNewRow) {
        const headerRows: string = this.parent.editSettings.showAddNewRow
        ...
        const fHearderRows: HTMLCollection = [].slice.call(
          this.parent
            ...
            .querySelectorAll(headerRows)
        );
        isFHdr =
          fHearderRows.length >
          (parseInt(row.getAttribute(literals.dataRowIndex), 10) || 0);
        isFHdrLastRow =
          isFHdr &&
          parseInt(row.getAttribute(literals.dataRowIndex), 10) ===
            fHearderRows.length - 1;
        const insertRow: Element[] = [].slice.call(
          this.parent
            .getHeaderTable()
            .querySelector(literals.tbody)
            .querySelectorAll('.e-row:not(.e-hiddenrow)')
        );
        if (
          insertRow.length === 1 &&
          (insertRow[0].classList.contains('e-addedrow') ||
            insertRow[0].classList.contains('e-insertedrow'))
        ) {
          isFHdrLastRow = true;
        }
      }
      if (
        ...
        (viewPortRowCount >= 1 &&
          ...
          ((this.parent.editSettings.newRowPosition === 'Bottom' &&
            ...
            (td.classList.contains('e-lastrowcell') &&
              !row.classList.contains(literals.addedRow)))) ||
        isBatchModeLastRow
      ) {
    ...
    const table: Element = isInline
      ...
      : select(
          '#' + this.parent.element.id + '_dialogEdit_wrapper .e-dlg-content',
          document
        );
    ...
    const inputClient: ClientRect = input
    ...
    const div: HTMLElement = this.parent.createElement('div', {
      ...
      styles:
        ...
        ';top:' +
        ((isFHdr
          ? inputClient.top + inputClient.height
          : inputClient.bottom - client.top) +
          table.scrollTop +
          9) +
        'px;left:' +
        (inputClient.left - left + table.scrollLeft + inputClient.width / 2) +
        'px;' +
        'max-width:' +
        inputClient.width +
    ...
    const content: Element = this.parent.createElement('div', {
    ...
    if (
      ...
      const getEditCell: HTMLElement =
        this.parent.editSettings.mode === 'Normal'
          ? (closest(element, '.e-editcell') as HTMLElement)
          : (closest(element, '.' + literals.table) as HTMLElement);
      getEditCell.style.position = 'relative';
      div.style.position = 'absolute';
      if (
        this.parent.editSettings.mode === 'Batch' ||
        closest(element, '.' + literals.frozenContent) ||
        closest(element, '.' + literals.frozenHeader) ||
        this.parent.frozenRows ||
        isAddNewRow
      ) {
        if (this.parent.isFrozenGrid()) {
          if (td.classList.contains('e-unfreeze')) {
            ...
            const elem: HTMLElement = closest(td, '.e-gridheader')
              ? this.parent.element.querySelector('.e-gridheader')
              : rows.length === 1
              ? this.parent.element
                  .querySelector('.e-gridcontent')
    ...
    if (!isNullOrUndefined(td)) {
      if (td.classList.contains('e-fixedfreeze')) {
        ...
        td.classList.contains('e-leftfreeze') ||
        td.classList.contains('e-rightfreeze')
      ) {
        div.classList.add('e-freezeerror');
      }
    }
    if (
      !validationForBottomRowPos &&
      isInline &&
      gcontent.getBoundingClientRect().bottom <
        inputClient.bottom + inputClient.height
    ) {
      const contentDiv: HTMLElement = this.parent
        .getContent()
        .querySelector('.e-content');
      if (
        ...
        gcontent.scrollTop =
          gcontent.scrollTop + div.offsetHeight + arrow.scrollHeight;
      }
    }
    ...
    if (
      div.getBoundingClientRect().width < inputClient.width &&
      div.querySelector('label').getBoundingClientRect().height /
        (lineHeight * 1.2) >=
        2
    ) {
    ...
    if (
      ...
      div.style.left =
        input.offsetLeft + (input.offsetWidth / 2 - div.offsetWidth / 2) + 'px';
    } else {
      div.style.left =
        parseInt(div.style.left, 10) - div.offsetWidth / 2 + 'px';
    }
    if (
      (isInline && !isScroll && !this.parent.allowPaging) ||
      this.parent.frozenRows ||
      isAddNewRow
    ) {
    ...
    if (validationForBottomRowPos) {
      if (
        ...
        !this.parent.enableInfiniteScrolling &&
        !(
          div.classList.contains('e-freezeerror') &&
          div.classList.contains('e-fixederror')
        )
      ) {
        const scrollWidth: number =
          gcontent.scrollWidth > gcontent.offsetWidth ? getScrollBarWidth() : 0;
        const gHeight: number | string =
          this.parent.height.toString().indexOf('%') === -1
            ? parseInt(this.parent.height as string, 10)
            : gcontent.offsetHeight;
        div.style.bottom =
          (gHeight as number) -
          gcontent.querySelector('table').offsetHeight -
          scrollWidth +
          inputClient.height +
          9 +
          'px';
      ...
      if (
        ...
        const rowsCount: number = this.parent.frozenRows
          ? (isAddNewRow
        ...
        const position: number =
          this.parent.getContent().clientHeight - rowsHeight;
  ...
  public checkColumnIsGrouped(col: Column): boolean {
    return (
      !col.visible &&
      !(this.parent.groupSettings.columns.indexOf(col.field) > -1)
    );
  }
