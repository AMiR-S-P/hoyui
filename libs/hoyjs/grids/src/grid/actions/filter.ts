...
import { remove, matches } from '@hoyui/base';
import { DataUtil, Predicate, Query, DataManager } from '@hoyui/data';
...
import { Input, InputArgs } from '@hoyui/inputs';
...
export class Filter implements IAction {
  ...
  private fltrDlgDetails: { field?: string; isOpen?: boolean } = {
    field: '',
    isOpen: false,
  };
  ...
  public render(e?: NotifyArgs): void {
    ...
    if (this.parent.filterSettings.type === 'FilterBar') {
      if (gObj.columns.length) {
        const fltrElem: Element =
          this.parent.element.querySelector('.e-filterbar');
        if (fltrElem) {
          remove(fltrElem);
        }
        ...
        const cellrender: CellRendererFactory =
          this.serviceLocator.getService<CellRendererFactory>(
            'cellRendererFactory'
          );
        cellrender.addCellRenderer(
          CellType.Filter,
          new FilterCellRenderer(this.parent, this.serviceLocator)
        );
        this.valueFormatter =
          this.serviceLocator.getService<IValueFormatter>('valueFormatter');
        ...
        this.parent
          .getHeaderContent()
          .querySelector('thead:not(.e-masked-thead)')
          .appendChild(rowRenderer.element);
        const rowdrag: Element =
          this.parent.element.querySelector('.e-rowdragheader');
        this.element = rowRenderer.render(
          row,
        ...
        if (
          ...
          const td: NodeListOf<Element> = this.element.querySelectorAll(
            '.e-indentcell, .e-grouptopleftcell'
          );
          for (let i: number = 0; i < td.length; i++) {
            td[parseInt(i.toString(), 10)].classList.add('e-leftfreeze');
            applyStickyLeftRightPosition(
              td[parseInt(i.toString(), 10)] as HTMLElement,
              i * 30,
        ...
        const detail: Element = this.element.querySelector(
          '.e-detailheadercell'
        );
        if (detail) {
        ...
        const gCells: Element[] = [].slice.call(
          this.element.getElementsByClassName('e-grouptopleftcell')
        );
        if (gCells.length) {
  ...
  public destroy(): void {
    ...
    if (
      ...
      (this.parent.isDestroyed || !this.parent.allowFiltering)
    ) {
    ...
    if (this.filterSettings.type === 'FilterBar' && !this.parent.isDestroyed) {
      if (this.filterSettings.showFilterBarOperator) {
        const dropdownlist: NodeListOf<Element> = [].slice.call(
          this.element.getElementsByClassName('e-filterbaroperator')
        );
        for (let i: number = 0; i < dropdownlist.length; i++) {
          if (
            (<EJ2Intance>dropdownlist[parseInt(i.toString(), 10)])
              .ej2_instances[0]
          ) {
            (<EJ2Intance>(
              dropdownlist[parseInt(i.toString(), 10)]
            )).ej2_instances[0].destroy();
          }
        }
      }
      this.parent.getColumns().map((column: Column) => {
        if (
          ...
          !isNullOrUndefined(column.filterBarTemplate.destroy)
        ) {
    ...
    if (this.element) {
      if (this.element.parentElement) {
        for (let i: number = 0; i < this.inputList.length; i++) {
          Input.destroy(
            ...
            this.inputList[parseInt(i.toString(), 10)].element
      ...
      const filterBarElement: Element = this.parent
        .getHeaderContent()
        .querySelector('.e-filterbar');
      if (filterBarElement) {
  ...
  private setFullScreenDialog(): void {
    if (this.serviceLocator) {
      this.serviceLocator.registerAdaptiveService(
        this,
  ...
  private generateCells(): Cell<Column>[] {
    ...
    if (this.parent.allowGrouping) {
      for (
        let c: number = 0,
          len: number = this.parent.groupSettings.columns.length;
        c < len;
        c++
      ) {
  ...
  public updateModel(): void {
    const col: Column = this.column.isForeignColumn()
    ...
    const arrayVal: (string | number | Date | boolean)[] = Array.isArray(
      this.value
    )
      ? this.value
      : [this.value];
    const moduleName: string =
      (this.parent.dataSource as DataManager).adaptor &&
    ...
    for (let i: number = 0, len: number = arrayVal.length; i < len; i++) {
      const field: string = col.isForeignColumn()
        ? col.foreignKeyValue
        : this.fieldName;
      ...
      if (index > -1 && !Array.isArray(this.value)) {
        this.filterSettings.columns[parseInt(index.toString(), 10)] =
          this.currentFilterObject;
      } else {
      ...
      if (
        ...
        for (let i: number = 0; i < this.filterSettings.columns.length; i++) {
          if (
            this.filterSettings.columns[`${i}`].field === field &&
  ...
  private getFilteredColsIndexByField(col: Column): number {
    ...
    for (let i: number = 0, len: number = cols.length; i < len; i++) {
      if (
        cols[parseInt(i.toString(), 10)].uid === col.uid ||
        (col.isForeignColumn() &&
  ...
  public onActionComplete(e: NotifyArgs): void {
    const args: Object = !this.isRemove
      ? {
          ...
          currentFilteringColumn: !isNullOrUndefined(this.column)
            ? this.column.field
            : undefined,
  ...
  public addEventListener(): void {
    ...
    this.parent.on(
      ...
      this.renderResponsiveChangeAction,
      this
    );
  ...
  private refreshClearIcon(e: Event): void {
    if (
      ...
      (e.target as Element)
        .closest('th')
        .classList.contains('e-filterbarcell') &&
      (e.target as Element).classList.contains('e-clear-icon')
    ) {
      const targetText: HTMLInputElement = (e.target as Element)
        .previousElementSibling as HTMLInputElement;
      Input.setValue(
        null,
        targetText as HTMLInputElement | HTMLTextAreaElement,
        'Never',
        true
      );
  ...
  public filterByColumn(
    ...
    if (
      ...
      isNullOrUndefined(this.column.filterTemplate)
    ) {
    ...
    if (
      filterOperator === 'like' &&
      filterValue &&
      (filterValue as string).indexOf('%') === -1
    ) {
    ...
    if (this.filterSettings.type === 'FilterBar') {
      filterCell = gObj
        .getHeaderContent()
        .querySelector(
          "[id='" + this.column.field + "_filterBarcell']"
        ) as HTMLInputElement;
    }
    if (
      !isNullOrUndefined(this.column.allowFiltering) &&
      !this.column.allowFiltering
    ) {
    ...
    this.ignoreAccent = this.ignoreAccent = !isNullOrUndefined(ignoreAccent)
      ? ignoreAccent
      : this.parent.filterSettings.ignoreAccent;
    this.fieldName = fieldName;
    this.operator = filterOperator;
    filterValue = !isNullOrUndefined(filterValue)
    ...
    if (filterCell && this.filterSettings.type === 'FilterBar') {
      if (
        (filterValue && (filterValue as string).length < 1) ||
        (!this.filterByMethod &&
        ...
        this.filterStatusMsg =
          filterValue && (filterValue as string).length < 1
            ? ''
            : this.l10n.getConstant('InvalidFilterMessage');
        this.updateFilterMsg();
        return;
      }
    ...
    const filterColumn: PredicateModel[] =
      this.parent.filterSettings.columns.filter((fColumn: PredicateModel) => {
        return fColumn.field === this.fieldName;
      });
    if (
      filterColumn.length > 1 &&
      !isNullOrUndefined(this.actualPredicate[this.fieldName])
    ) {
  ...
  private applyColumnFormat(
    ...
    const getFlvalue: Date | number | string =
      this.column.type === 'date' ||
      this.column.type === 'datetime' ||
      this.column.type === 'dateonly'
    ...
    if (
      ...
      this.values[this.column.field] = (filterValue as string)
        .split(',')
        .map((val: string) => {
          if (val === '') {
            val = null;
      ...
      this.values[this.column.field] = this.setFormatForFlColumn(
        getFlvalue,
        this.column
      );
    }
  ...
  private skipUid(col: object[]): boolean {
    ...
    for (let i: number = 0; i < colLen.length; i++) {
      const key: Object[] = Object.keys(
        col[colLen[parseInt(i.toString(), 10)]]
      );
          ...
          if (this.contentRefresh && this.skipUid(e.properties[`${col}`])) {
            ...
            if ((<{ cancel?: boolean }>args).cancel) {
              if (
                ...
                this.filterSettings.columns =
                  this.actualData.length <= 1
                    ? this.checkboxPrevFilterObject
                    : this.checkboxFilterObject;
                this.actualPredicate[this.column.field] =
                  this.filterSettings.columns;
                const col: Column = this.parent.getColumnByField(
                  this.column.field
                );
                const iconClass: string =
                  this.parent.showColumnMenu && col.showColumnMenu
                    ? '.e-columnmenu'
                    : '.e-icon-filter';
                const filterIconElement: Element = this.parent
                  .getColumnHeaderByField(this.column.field)
                ...
                if (isNullOrUndefined(this.prevFilterObject)) {
                  this.filterSettings.columns.splice(
                    this.filterSettings.columns.length - 1,
                    1
                  );
                } else {
                  this.filterSettings.columns[this.filterObjIndex] =
    ...
    if (this.filterSettings.type === 'FilterBar') {
      for (let i: number = 0; i < this.filterSettings.columns.length; i++) {
        this.column = this.parent.grabColumnByUidFromAllCols(
          this.filterSettings.columns[parseInt(i.toString(), 10)].uid
        );
        let filterValue: string | number | Date | boolean =
        ...
        if (!isNullOrUndefined(this.column.format)) {
          ...
          const key: string =
            this.filterSettings.columns[parseInt(i.toString(), 10)].field;
          this.values[`${key}`] =
            this.filterSettings.columns[parseInt(i.toString(), 10)].value;
        }
        const filterElement: HTMLInputElement = this.getFilterBarElement(
          this.column.field
        );
        if (filterElement) {
          if (
            ...
            filterElement.value = this.filterSettings.columns[
      ...
      if (this.filterSettings.columns.length === 0) {
        ...
        for (let i: number = 0; i < col.length; i++) {
          const filterElement: HTMLInputElement = this.getFilterBarElement(
            col[parseInt(i.toString(), 10)].field
          );
  ...
  private updateFilterIcon(): void {
    if (
      ...
      this.parent.element.querySelector('.e-filtered')
    ) {
      const fltrIconElement: Element[] = [].slice.call(
        this.parent.element.getElementsByClassName('e-filtered')
      );
      for (
        let i: number = 0, len: number = fltrIconElement.length;
        i < len;
        i++
      ) {
        fltrIconElement[parseInt(i.toString(), 10)].classList.remove(
          'e-filtered'
        );
      }
    }
  ...
  public clearFiltering(fields?: string[]): void {
    const cols: PredicateModel[] = getActualPropFromColl(
      this.filterSettings.columns
    );
    if (!isNullOrUndefined(fields)) {
      ...
      this.parent.setProperties(
        { filterSettings: { columns: this.filterSettings.columns } },
        true
      );
    ...
    for (let i: number = 0; i < cols.length; i++) {
      cols[parseInt(i.toString(), 10)].uid =
        cols[parseInt(i.toString(), 10)].uid ||
        this.parent.getColumnByField(cols[parseInt(i.toString(), 10)].field)
          .uid;
    }
    const colUid: string[] = cols.map((f: Column) => f.uid);
    const filteredcols: string[] = colUid.filter(
      (item: string, pos: number) => colUid.indexOf(item) === pos
    );
    this.refresh = false;
    for (let i: number = 0, len: number = filteredcols.length; i < len; i++) {
      this.removeFilteredColsByField(
        this.parent.getColumnByUid(filteredcols[parseInt(i.toString(), 10)])
          .field,
        false
      );
    }
    ...
    if (
      ...
      const fltrElement: Element[] = [].slice.call(
        this.parent.element.getElementsByClassName('e-filtered')
      );
  ...
  private checkAlreadyColFiltered(field: string): boolean {
    ...
    for (const col of columns) {
      if (
        ...
        return (
          this.checkDateColumnValue(col.value, this.value) &&
          col.operator === this.operator &&
          col.predicate === this.predicate
        );
      } else if (
        col.field === field &&
  ...
  private checkDateColumnValue(
    colDate: string | number | boolean | Date,
    filterDate: string | number | boolean | Date
  ): boolean {
  ...
  public createOptions(
    ...
    const dataSource: Object =
      col.filter.dataSource || (gObj.dataSource && 'result' in gObj.dataSource)
        ? gObj.dataSource
    ...
    const options: object = {
      ...
      hideSearchbox: isNullOrUndefined(col.filter.hideSearchbox)
        ? false
        : col.filter.hideSearchbox,
      ...
      operator:
        this.actualPredicate[col.field] && type === 'Menu'
          ? this.actualPredicate[col.field][0].operator
          : 'equal',
      parentTotalDataCount:
        gObj.getDataModule().isRemote() && gObj.allowPaging
          ? gObj.pagerModule.pagerObj.totalRecordsCount
          : gObj.getDataModule().isRemote()
          ? gObj.totalDataRecordsCount
          : (gObj.getFilteredRecords() as Object[]).length,
      parentCurrentViewDataCount: gObj.currentViewData.length,
      parentFilteredLocalRecords: !gObj.getDataModule().isRemote()
        ? (gObj.getFilteredRecords() as Object[])
        : [],
    };
    return options;
  }
  ...
  public removeFilteredColsByField(
    ...
    const colUid: string[] = cols.map((f: Column) => f.uid);
    const filteredColsUid: string[] = colUid.filter(
      (item: string, pos: number) => colUid.indexOf(item) === pos
    );
    ...
    for (
      let i: number = 0, len: number = filteredColsUid.length;
      i < len;
      i++
    ) {
      cols[parseInt(i.toString(), 10)].uid =
      ...
      const column: Column = this.parent.grabColumnByUidFromAllCols(
        filteredColsUid[parseInt(i.toString(), 10)]
      );
      if (
        column.field === field ||
        (column.field === column.foreignKeyValue && column.isForeignColumn())
      ) {
        const currentPred: PredicateModel = this.filterSettings.columns.filter(
          (e: PredicateModel) => {
        ...
        const fltrElement: Element = this.parent.getColumnHeaderByField(
          column.field
        );
        if (
          this.filterSettings.type !== 'FilterBar' ||
  ...
  private keyUpHandler(e: KeyboardEventArgs): void {
    ...
    if (target && matches(target, '.e-filterbar input')) {
      ...
      this.column = gObj.getColumnByUid(
        closeHeaderEle.getAttribute('e-mappinguid')
      );
      if (!this.column) {
        return;
      }
      if (
        ...
        const dropDownListInput: Element = closest(
          target,
          'span'
    ...
    if (
      ...
      if (
        element &&
        element.classList.contains('e-headercell') &&
        !element.classList.contains('e-stackedheadercell')
      ) {
        const column: Column = gObj.getColumnByUid(
          element.firstElementChild.getAttribute('e-mappinguid')
        );
    ...
    if (this.filterSettings.type === 'FilterBar') {
      ...
      if (
        ...
        for (let index: number = 0; index < columns.length; index++) {
          column =
            gObj.grabColumnByUidFromAllCols(
              columns[parseInt(index.toString(), 10)].uid
            ) ||
            gObj.grabColumnByFieldFromAllCols(
              columns[parseInt(index.toString(), 10)].field,
              columns[parseInt(index.toString(), 10)].isForeignKey
            );
          if (index) {
            this.filterStatusMsg += ' && ';
          }
          if (!isNullOrUndefined(column.format)) {
            const flValue: Date | number =
              column.type === 'date' ||
            ...
            if (
              ...
              const formater: IValueFormatter =
                this.serviceLocator.getService<IValueFormatter>(
                  'valueFormatter'
                );
              getFormatFlValue = formater
                .toView(flValue, column.getParser())
                .toString();
            } else {
            ...
            this.filterStatusMsg +=
              column.headerText + ': ' + this.values[column.field];
          }
        }
      }
      if (gObj.allowPaging) {
  ...
  private checkForSkipInput(column: Column, value: string): boolean {
    ...
    if (column.type === 'number') {
      if (
        DataUtil.operatorSymbols[`${value}`] ||
        this.skipNumberInput.indexOf(value) > -1
      ) {
        isSkip = true;
      }
  ...
  private onTimerTick(): void {
    ...
    let filterElement: HTMLInputElement = this.element.querySelector(
      selector
    ) as HTMLInputElement;
    if (!filterElement) {
      filterElement = this.parent
        .getHeaderContent()
        .querySelector(selector) as HTMLInputElement;
    }
    ...
    if (!isNullOrUndefined(this.column.filterBarTemplate)) {
      let templateRead: Function = this.column.filterBarTemplate
        .read as Function;
      ...
      case 'datetime':
        ...
        if (this.value !== '' && !(this.value instanceof Date)) {
          ...
          if (isNullOrUndefined(this.value)) {
            this.filterStatusMsg = this.l10n.getConstant(
  ...
  private getOperator(value: string): void {
    ...
    const operators: Object = extend(
      { '=': this.filterOperators.equal, '!': this.filterOperators.notEqual },
      DataUtil.operatorSymbols
    );
    // eslint-disable-next-line no-prototype-builtins
    if (
      operators.hasOwnProperty(singleOp) ||
      operators.hasOwnProperty(multipleOp)
    ) {
    ...
    if (
      this.operator === this.filterOperators.lessThan ||
      this.operator === this.filterOperators.greaterThan
    ) {
  ...
  public openMenuByField(field: string): void {
    ...
    this.filterDialogOpen(
      column,
      target,
      fClient.right - gClient.left,
      fClient.bottom - gClient.top
    );
  }
  ...
  private filterIconClickHandler(e: MouseEvent): void {
    ...
    if (
      target.classList.contains('e-filtermenudiv') &&
      (this.parent.filterSettings.type === 'Menu' ||
      ...
      const col: Column = gObj.getColumnByUid(
        parentsUntil(target, 'e-headercell').firstElementChild.getAttribute(
          'e-mappinguid'
        )
      );
      this.column = col;
      if (
        this.fltrDlgDetails.field === col.field &&
        this.fltrDlgDetails.isOpen
      ) {
        return;
      }
      if (this.filterModule) {
  ...
  private clickHandler(e: MouseEvent): void {
    if (
      ...
      if (
        ...
        const filterOperatorElement: HTMLElement = closest(
      ...
      if ((e.target as HTMLElement).classList.contains('e-list-item')) {
        const inputId: string = document
          .querySelector('.e-popup-open')
          .getAttribute('id')
          .replace('_popup', '');
        if (inputId.indexOf('grid-column') !== -1) {
          (
            closest(document.getElementById(inputId), 'div').querySelector(
              '.e-filtertext'
            ) as HTMLElement
          ).focus();
        }
    ...
    if (
      ...
      const hasDialogClosed: Element = this.parent.element.classList.contains(
        'e-device'
      )
        ? document.querySelector('.e-filter-popup')
        : document.getElementById(this.parent.element.id + '_e-popup')
        ? document
            .getElementById(this.parent.element.id + '_e-popup')
      ...
      if (
        this.filterModule &&
        hasDialogClosed &&
        (parentsUntil(target, 'e-excel-ascending') ||
          parentsUntil(target, 'e-excel-descending'))
      ) {
        this.filterModule.closeDialog(target);
      }
      if (
        parentsUntil(target, 'e-filter-popup') ||
        target.classList.contains('e-filtermenudiv')
      ) {
        return;
      } else if (
        this.filterModule &&
        !parentsUntil(target, 'e-date-overflow') &&
        ...
        if (
          (hasDialog &&
            !parentsUntil(target, 'e-filter-popup') &&
            !parentsUntil(target, 'e-popup-flmenu')) ||
          (!popupEle && hasDialogClosed)
        ) {
  ...
  private filterHandler(args: {
    ...
    this.checkboxPrevFilterObject = dataManager.executeLocal(query) as {
      field: string;
    }[];
    for (let i: number = 0; i < this.checkboxPrevFilterObject.length; i++) {
      ...
      for (let j: number = 0; j < this.filterSettings.columns.length; j++) {
        if (
          this.checkboxPrevFilterObject[parseInt(i.toString(), 10)].field ===
    ...
    const iconClass: string =
      this.parent.showColumnMenu && col.showColumnMenu
        ...
        : '.e-icon-filter';
    const filterIconElement: Element = this.parent
      .getColumnHeaderByField(args.field)
      .querySelector(iconClass);
    if (args.action === 'filtering') {
      this.filterSettings.columns = this.filterSettings.columns.concat(
  ...
  private updateFilter(): void {
    ...
    for (let i: number = 0; i < cols.length; i++) {
      ...
      if (
        ...
        fieldName = getColumnByForeignKeyValue(
          cols[parseInt(i.toString(), 10)].field,
          this.parent.getForeignKeyColumns()
        ).field;
      }
      this.refreshFilterIcon(
        fieldName,
        cols[parseInt(i.toString(), 10)].operator,
        cols[parseInt(i.toString(), 10)].value,
        cols[parseInt(i.toString(), 10)].type,
        cols[parseInt(i.toString(), 10)].predicate,
        cols[parseInt(i.toString(), 10)].matchCase,
        cols[parseInt(i.toString(), 10)].ignoreAccent,
        cols[parseInt(i.toString(), 10)].uid
      );
    }
  }
  ...
  private refreshFilterIcon(
    ...
    const field: string = uid
      ? this.parent.grabColumnByUidFromAllCols(uid).field
      : fieldName;
  ...
  private addFilteredClass(fieldName: string): void {
    ...
    if (this.parent.showColumnMenu && col.showColumnMenu) {
      filterIconElement = this.parent
        .getColumnHeaderByField(fieldName)
        .querySelector('.e-columnmenu');
    } else if (col) {
      filterIconElement = this.parent
        .getColumnHeaderByField(fieldName)
        .querySelector('.e-icon-filter');
    }
    if (filterIconElement) {
      filterIconElement.classList.add('e-filtered');
    }
  ...
  private getOperatorName(field: string): string {
    return (<EJ2Intance>(
      document.getElementById(this.parent.getColumnByField(field).uid)
