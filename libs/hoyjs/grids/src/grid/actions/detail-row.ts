...
import { closest, classList, isNullOrUndefined } from '@hoyui/base';
...
export class DetailRow {
  ...
  private toogleExpandcollapse(target: Element): void {
    ...
    const isExpanded: boolean =
      target && target.classList.contains('e-detailrowcollapse');
    if (
      !(
        target &&
        (target.classList.contains('e-detailrowcollapse') ||
          target.classList.contains('e-detailrowexpand'))
      ) ||
    ...
    const nextRow: HTMLElement = this.parent
      .getContentTable()
      .querySelector(literals.tbody).children[tr.rowIndex + 1] as HTMLElement;
    if (target.classList.contains('e-detailrowcollapse')) {
      ...
      if (this.isDetailRow(nextRow)) {
        ...
        const detailRow: Element = this.parent.createElement('tr', {
          className: 'e-detailrow',
          attrs: { 'data-uid': rowId, role: 'row' },
        });
        const detailCell: Element = this.parent.createElement('th', {
        ...
        for (
          let i: number = 0, len: number = gObj.groupSettings.columns.length;
          i < len;
          i++
        ) {
        ...
        if (gObj.detailTemplate) {
          isReactCompiler =
            this.parent.isReact && typeof gObj.detailTemplate !== 'string';
          isReactChild =
          ...
          childGrid[`${parent}`] = {
            ...
            parentKeyFieldValue:
              ...
              isComplexField(gObj.childGrid.queryString)
                ? getObject(gObj.childGrid.queryString, data)
                : data[gObj.childGrid.queryString],
            parentRowData: data,
          };
          if (gObj.isReact || gObj.isVue) {
            ...
            childGrid.parentDetails.parentInstObj =
              gObj.parentDetails.parentInstObj;
          }
          (<{ isLegacyTemplate?: boolean }>childGrid).isLegacyTemplate =
            gObj.isReact ||
      ...
      if (
        target.classList.contains('e-lastrowcell') &&
        this.parent.getContent().clientHeight > table.scrollHeight
      ) {
        removeClass(
          target.parentElement.querySelectorAll('td'),
          'e-lastrowcell'
        );
        const detailrowIdx: number =
          table
            .querySelector(literals.tbody)
            .getElementsByClassName('e-detailrow').length - 1;
        addClass(
          table
            .querySelector(literals.tbody)
            .getElementsByClassName('e-detailrow')[
            parseInt(detailrowIdx.toString(), 10)
          ].childNodes,
          ['e-lastrowcell']
        );
      ...
      if (
        parseInt(tr.getAttribute(literals.dataRowIndex), 10) === lastrowIdx &&
        this.lastrowcell
      ) {
  ...
  public getGridModel(
    ...
    if (
      ...
      rowObj.isExpand &&
      gObj.expandedRows &&
      gObj.expandedRows[rowObj.index] &&
      gObj.expandedRows[rowObj.index].gridModel
    ) {
      (gObj.expandedRows[rowObj.index].gridModel as IGrid).hierarchyPrintMode =
  ...
  private getTDfromIndex(index: number, className: string): Element {
    const tr: Element = !isNullOrUndefined(index)
  ...
  public expand(target: number | Element): void {
    ...
    if (
      target &&
      (target as Element).classList.contains('e-detailrowcollapse')
    ) {
  ...
  private expandCollapse(isExpand: boolean): void {
    ...
    for (let i: number = 0, len: number = rows.length; i < len; i++) {
      td = rows[parseInt(i.toString(), 10)].querySelector(
    ...
    switch (e.action) {
      ...
      case 'altDownArrow':
        ...
        if (selected.length) {
          ...
          const td: Element = dataRow.querySelector(
            '.e-detailrowcollapse, .e-detailrowexpand'
          );
      ...
      case 'enter':
        ...
        if (
          element &&
          !element.classList.contains('e-detailrowcollapse') &&
  ...
  private refreshColSpan(): void {
    const detailrows: NodeListOf<Element> = (<Grid>this.parent).contentModule
      .getTable()
      .querySelectorAll('tr.e-detailrow');
    const colSpan: number = (<Grid>this.parent).getVisibleColumns().length;
    for (let i: number = 0; i < detailrows.length; i++) {
      (<HTMLElement>detailrows[parseInt(i.toString(), 10)])
        .querySelector('.e-detailcell')
