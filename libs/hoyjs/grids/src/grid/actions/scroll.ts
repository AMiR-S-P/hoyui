...
import { addClass, removeClass } from '@hoyui/base';
import { formatUnit, isNullOrUndefined } from '@hoyui/base';
...
export class Scroll implements IAction {
  ...
  public setWidth(uiupdate?: boolean): void {
    ...
    if (
      (<Grid>this.parent).toolbarModule &&
      (<Grid>this.parent).toolbarModule.toolbar &&
      (<Grid>this.parent).toolbarModule.toolbar.element
    ) {
      const tlbrElement: Element = (<Grid>this.parent).toolbarModule.toolbar
        .element;
      const tlbrLeftElement: Element =
        tlbrElement.querySelector('.e-toolbar-left');
      const tlbrCenterElement: Element =
        tlbrElement.querySelector('.e-toolbar-center');
      const tlbrRightElement: Element =
        tlbrElement.querySelector('.e-toolbar-right');
      const tlbrItems: Element = tlbrElement.querySelector('.e-toolbar-items');
      const tlbrLeftWidth: number = tlbrLeftElement
        ? tlbrLeftElement.clientWidth
        : 0;
      const tlbrCenterWidth: number = tlbrCenterElement
        ? tlbrCenterElement.clientWidth
        : 0;
      const tlbrRightWidth: number = tlbrRightElement
        ? tlbrRightElement.clientWidth
        : 0;
      ...
      if (
        ...
        tlbrLeftWidth > tlbrWidth ||
        tlbrCenterWidth > tlbrWidth ||
        tlbrRightWidth > tlbrWidth ||
  ...
  public setHeight(): void {
    ...
    if (
      ...
      const tbody: HTMLElement = this.parent
        .getHeaderContent()
        .querySelector(literals.tbody + ':not(.e-masked-tbody)') as HTMLElement;
      mHdrHeight = tbody ? tbody.offsetHeight : 0;
      if (tbody && mHdrHeight) {
        const add: number = tbody.getElementsByClassName(
          literals.addedRow
        ).length;
        ...
        height =
          ...
          this.parent.frozenRows * this.parent.getRowHeight();
      }
  ...
  public setPadding(): void {
    ...
    const scrollWidth: number =
      Scroll.getScrollBarWidth() - this.getThreshold();
    const cssProps: ScrollCss = this.getCssProperties();
    (<HTMLElement>content.querySelector('.' + literals.headerContent)).style[
      cssProps.border
    ] = scrollWidth > 0 ? '1px' : '0px';
    content.style[cssProps.padding] =
      scrollWidth > 0 ? scrollWidth + 'px' : '0px';
  }
  ...
  public removePadding(rtl?: boolean): void {
    ...
    const hDiv: HTMLDivElement = <HTMLDivElement>(
      this.parent.getHeaderContent().querySelector('.' + literals.headerContent)
    );
    hDiv.style[cssProps.border] = '';
    hDiv.parentElement.style[cssProps.padding] = '';
    const footerDiv: HTMLDivElement = <HTMLDivElement>(
  ...
  private unwireEvents(): void {
    ...
    const mScrollBar: HTMLElement = this.parent.getContent()
      ? this.parent.getContent().querySelector('.e-movablescrollbar')
      : null;
    ...
    if (this.parent.aggregates.length && this.parent.getFooterContent()) {
      EventHandler.remove(
        <HTMLDivElement>this.parent.getFooterContent().firstChild,
        'scroll',
  ...
  private setScrollLeft(): void {
    ...
    )).scrollLeft = this.previousValues.left;
  }

  private onContentScroll(scrollTarget: HTMLElement): Function {
    const element: HTMLElement = scrollTarget;
    const isHeader: boolean = element.classList.contains(
      literals.headerContent
    );
    return (e: Event) => {
      if (
        this.content.querySelector(literals.tbody) === null ||
        this.parent.isPreventScrollEvent
      ) {
        return;
      }
      ...
      if (
        this.parent.element.querySelectorAll(
          '.e-leftfreeze,.e-fixedfreeze,.e-rightfreeze'
        ).length
      ) {
        const errorFreeze: NodeListOf<Element> = this.parent
          .getContent()
          .querySelectorAll('.e-freezeerror:not([style*="display: none"])');
        const errorFixed: NodeListOf<Element> = this.parent
          .getContent()
          .querySelectorAll('.e-fixederror:not([style*="display: none"])');
        if (
          target.scrollLeft !== 0 &&
        ...
        const widthVal: number = Math.round(
        ...
        if (
          ((rows.length === 1 && errorFreeze.length) ||
            this.parent.element.querySelector(
              '.e-freeze-autofill:not([style*="display: none"])'
            ) ||
            errorFixed.length) &&
        ...
        if (
          rows.length !== 1 &&
          (errorFreeze.length || errorFixed.length) &&
      ...
      if (this.previousValues.left === left) {
        this.previousValues.top = !isHeader
          ? this.previousValues.top
          : target.scrollTop;
        return;
      }
  ...
  private onTouchScroll(scrollTarget: HTMLElement): Function {
    ...
    return (e: PointerEvent | TouchEvent) => {
      ...
      const mHdr: Element = this.parent
        .getHeaderContent()
        .querySelector('.' + literals.headerContent) as Element;
      const mCont: Element = this.parent
        .getContent()
        .querySelector('.' + literals.content) as Element;
      if (
        this.previousValues.left === left ||
        left < 0 ||
        mHdr.scrollWidth - mHdr.clientWidth < left
      ) {
        return;
      }
      e.preventDefault();
      ...
      if (isFrozen && this.parent.enableColumnVirtualization) {
        const scrollBar: HTMLElement = this.parent
          .getContent()
          .querySelector('.e-movablescrollbar');
  ...
  private getScrollbleParent(node: HTMLElement): HTMLElement {
    ...
    const parent: HTMLElement = isNullOrUndefined(node.tagName)
      ? (node as any).scrollingElement
      : node;
    const overflowY: string = document.defaultView.getComputedStyle(
      parent,
      null
    ).overflowY;
    if (
  ...
  public addStickyListener(isAdd: boolean): void {
    this.parentElement = this.getScrollbleParent(
      this.parent.element.parentElement
    );
    if (isAdd && this.parentElement) {
      this.eventElement =
        this.parentElement.tagName === 'HTML' ||
  ...
  public resizeFrozenRowBorder(): void {
    ...
    if (!this.parent.element.querySelector('.e-frozenrow-border')) {
      ...
      div = this.parent.element.querySelector(
        '.e-frozenrow-border'
      ) as HTMLElement;
    }
    const scrollWidth: number =
      this.parent.height !== 'auto' ? Scroll.getScrollBarWidth() : 0;
    div.style.width =
      this.parent.element.offsetWidth - scrollWidth - 0.5 + 'px';
  }
  ...
  private wireEvents(): void {
    if (this.oneTimeReady) {
      ...
      const mScrollBar: HTMLElement = this.parent
        .getContent()
        .querySelector('.e-movablescrollbar');
      if (this.parent.frozenRows && this.header && this.content) {
        ...
        EventHandler.add(
          this.header,
          'touchmove pointermove',
          this.onTouchScroll(this.content),
          this
        );
      }
      if (frzCols && mScrollBar && this.parent.enableColumnVirtualization) {
        EventHandler.add(
          mScrollBar,
          'scroll',
          this.onCustomScrollbarScroll(this.content, this.header),
          this
        );
        EventHandler.add(
          this.content,
          'scroll',
          this.onCustomScrollbarScroll(mScrollBar, this.header),
          this
        );
        EventHandler.add(
          this.header,
          'scroll',
          this.onCustomScrollbarScroll(mScrollBar, this.content),
          this
        );
        EventHandler.add(
          this.header,
        ...
        if (
          ...
          EventHandler.add(
            this.content,
            'touchmove pointermove',
            this.onTouchScroll(this.header),
            this
          );
        }
      }
      ...
      if (this.parent.aggregates.length) {
        EventHandler.add(
          <HTMLDivElement>this.parent.getFooterContent().firstChild,
          'scroll',
          this.onContentScroll(this.content),
          this
        );
      }
  ...
  public setLastRowCell(): void {
    ...
    if (table.querySelector('tr:nth-last-child(2)')) {
      if (
        ...
        addClass(
          table.querySelector('tr:nth-last-child(2)').querySelectorAll('td'),
          'e-lastrowcell'
        );
      }
    }
    ...
    if (this.parent.isSpan) {
      addClass(
        table.querySelectorAll('.e-row-span-lastrowcell'),
        'e-lastrowcell'
      );
    }
  }
  ...
  public getCssProperties(rtl?: boolean): ScrollCss {
    const css: ScrollCss = {};
    const enableRtl: boolean = isNullOrUndefined(rtl)
      ? this.parent.enableRtl
      : rtl;
  ...
  public makeStickyHeader(): void {
    if (
      this.parent.enableStickyHeader &&
      this.parent.element &&
      this.parent.getContent()
    ) {
      const contentRect: ClientRect = this.parent
        .getContent()
        .getClientRects()[0];
      if (contentRect) {
        const headerEle: HTMLElement =
          this.parent.getHeaderContent() as HTMLElement;
        const toolbarEle: HTMLElement = this.parent.element.querySelector(
          '.e-toolbar'
        ) as HTMLElement;
        const groupHeaderEle: HTMLElement = this.parent.element.querySelector(
          '.e-groupdroparea'
        ) as HTMLElement;
        const height: number =
          headerEle.offsetHeight +
        ...
        const colMenuEle: HTMLElement = document.body.querySelector(
          '#' + this.parent.element.id + '_columnmenu'
        );
        if (top < height && contentRect.bottom > 0) {
          ...
          if (headerEle.classList.contains('e-sticky')) {
            ...
            const ccDlg: HTMLElement =
              this.parent.element.querySelector('.e-ccdlg');
            if (ccDlg) {
  ...
  private setSticky(
    ...
    ele.style.left =
      left !== null
        ? parseInt(ele.style.left, 10) !== left
          ? left + 'px'
          : ele.style.left
        : '';
  }
  ...
  public destroy(): void {
    ...
    //Remove Dom event
    const cont: Element = this.parent
      .getContent()
