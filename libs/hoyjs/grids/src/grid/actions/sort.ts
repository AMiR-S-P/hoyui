import { Browser, KeyboardEventArgs, EventHandler } from '@hoyui/base';
import { extend, isNullOrUndefined } from '@hoyui/base';
import { closest, classList } from '@hoyui/base';
...
export class Sort implements IAction {
  ...
  public updateModel(): void {
    const sortedColumn: SortDescriptorModel = {
    ...
    if (!this.isMultiSort) {
      if (!gCols.length) {
        ...
        for (let i: number = 0, len: number = gCols.length; i < len; i++) {
          index = this.getSortedColsIndexByField(
          ...
          if (this.columnName === gCols[parseInt(i.toString(), 10)]) {
            ...
            const sCol: SortDescriptorModel = this.getSortColumnFromField(
              gCols[parseInt(i.toString(), 10)]
            );
            sortedCols.push({
              field: sCol.field,
  ...
  public sortColumn(
    ...
    if (
      this.parent.getColumnByField(columnName).allowSorting === false ||
      this.parent.isContextMenuOpen()
    ) {
  ...
  private restoreSettings(): void {
    ...
    this.parent.setProperties(
      { sortSettings: { columns: this.lastSortedCols } },
      true
    );
  ...
  private updateSortedCols(columnName: string, isMultiSort: boolean): void {
    if (!isMultiSort) {
      if (this.parent.allowGrouping) {
        for (
          let i: number = 0, len: number = this.sortedColumns.length;
          i < len;
          i++
        ) {
          if (
            this.parent.groupSettings.columns.indexOf(
              this.sortedColumns[parseInt(i.toString(), 10)]
            ) < 0
          ) {
  ...
  public clearSorting(): void {
    const cols: SortDescriptorModel[] = getActualPropFromColl(
  ...
  private getSortedColsIndexByField(
    field: string,
    sortedColumns?: SortDescriptorModel[]
  ): number {
    const cols: SortDescriptorModel[] = sortedColumns
      ? sortedColumns
      : this.sortSettings.columns;
    for (let i: number = 0, len: number = cols.length; i < len; i++) {
      if (cols[parseInt(i.toString(), 10)].field === field) {
  ...
  public addEventListener(): void {
    ...
    this.evtHandlers = [
      { event: events.setFullScreenDialog, handler: this.setFullScreenDialog },
      {
  ...
  private excelFilterSortAction(e: MouseEvent): void {
    ...
    if (
      popUp &&
      popUp.id === gridID &&
      parentsUntil(e.target as Element, 'e-excelfilter')
    ) {
  ...
  public destroy(): void {
    ...
    if (
      this.parent.element
        ...
        .getElementsByClassName('e-sortdirect').length
    ) {
      (
    ...
    if (
      ...
      (this.parent.isDestroyed || !this.parent.allowSorting)
    ) {
      this.clearSorting();
    }
    this.isModelChanged = true;
  ...
  private clickHandler(e: MouseEvent): void {
    ...
    if (
      target &&
      !(e.target as Element).classList.contains('e-grptogglebtn') &&
      ...
      !(
        gObj.allowSelection &&
        gObj.selectionSettings.allowColumnSelection &&
        (e.target as Element).classList.contains('e-headercell')
      )
    ) {
      const gObj: IGrid = this.parent;
      const colObj: Column = gObj.getColumnByUid(
        target.querySelector('.e-headercelldiv').getAttribute('e-mappinguid')
      ) as Column;
  ...
  private excelFilterSortActionHandler(e: MouseEvent): void {
    if (
      parentsUntil(e.target as Element, 'e-excel-ascending') ||
      parentsUntil(e.target as Element, 'e-excel-descending')
    ) {
      const colUid: string = (<EJ2Intance>(
        closest(e.target as Element, '.e-filter-popup')
      )).getAttribute('uid');
      const direction: SortDirection = isNullOrUndefined(
        parentsUntil(e.target as Element, 'e-excel-descending')
      )
        ? 'Ascending'
        : 'Descending';
      this.sortColumn(
        this.parent.getColumnByUid(colUid).field,
        direction,
        false
      );
    }
  ...
  private initiateSort(
    ...
    const direction: SortDirection = !target.getElementsByClassName(
      'e-ascending'
    ).length
      ? 'Ascending'
      : 'Descending';
  ...
  private showPopUp(e: MouseEvent): void {
    ...
    if (
      ...
      (!isNullOrUndefined(target) || this.parent.isContextMenuOpen())
    ) {
      setCssInGridPopUp(
        <HTMLElement>this.parent.element.querySelector('.e-gridpopup'),
        e,
        'e-sortdirect e-icons e-icon-sortdirect' +
          (this.sortedColumns.length > 1 ? ' e-spanclicked' : '')
      );
    }
  }
  ...
  private addSortIcons(): void {
    ...
    for (let i: number = 0, len: number = cols.length; i < len; i++) {
      ...
      if (
        fieldNames.indexOf(cols[parseInt(i.toString(), 10)].field) === -1 ||
  ...
  private removeSortIcons(position?: number): void {
    ...
    const fieldNames: string[] = this.parent
      ...
      .map((c: Column) => c.field);
    for (
      let i: number = position ? position : 0,
        len: number = !isNullOrUndefined(position) ? position + 1 : cols.length;
      i < len;
      i++
    ) {
      header = gObj.getColumnHeaderByField(
      ...
      if (header.querySelector('.e-sortnumber')) {
        header
          .querySelector('.e-headercelldiv')
          .removeChild(header.querySelector('.e-sortnumber'));
      }
    }
  }
  ...
  private getSortColumnFromField(field: string): SortDescriptorModel {
    for (
      let i: number = 0, len: number = this.sortSettings.columns.length;
      i < len;
      i++
    ) {
      if (
        this.sortSettings.columns[parseInt(i.toString(), 10)].field === field
      ) {
  ...
  private updateAriaAttr(): void {
    ...
    for (const col of this.sortedColumns) {
      ...
      this.aria.setSort(
        <HTMLElement>header,
        this.getSortColumnFromField(col).direction
      );
    }
  }
