...
import { Observer, isBlazor, isNullOrUndefined } from '@hoyui/base';
...
export class BlazorAction {
  ...
  public addDeleteSuccess(args: NotifyArgs): void {
    ...
    args.promise
      .then((e: ReturnType) => this.editSuccess(editArgs))
      .catch((e: Error) => {
        if (isBlazor() && this.parent.isServerRendered) {
          const error: string = 'error';
          const message: string = 'message';
          if (
            !isNullOrUndefined(e[`${error}`]) &&
            !isNullOrUndefined(e[`${error}`][`${message}`])
          ) {
        ...
        this.parent.trigger(
          ...
          isBlazor() && e instanceof Array ? e[0] : e
        );
  ...
  public onDetailRowClick(target: Element): void {
    ...
    const args: Object = {
      ...
      index: tr.getAttribute('data-rowindex'),
    ...
    if (target.classList.contains('e-detailrowcollapse')) {
      ...
      const gridRow: Row<Column> = new Row<Column>({
        ...
        cells: [
          ...
          new Cell<Column>({ isDataCell: true, visible: true }),
        ],
      });
  ...
  public setColumnVisibility(columns: Column[]): void {
    ...
    for (let i: number = 0; i < columns.length; i++) {
      visible[columns[parseInt(i.toString(), 10)].uid] =
  ...
  public dataSuccess(args: ReturnType): void {
    if (
      ...
      Object.keys(this.actionArgs).length === 0
    ) {
      this.actionArgs.requestType = 'virtualscroll';
    }
    const startIndex: string = 'startIndex';
    const endIndex: string = 'endIndex';
    this.actionArgs[`${startIndex}`] = args[`${startIndex}`];
    this.actionArgs[`${endIndex}`] = args[`${endIndex}`];
    if (this.parent.enableVirtualization) {
      this.virtualContentModule = <VirtualContentRenderer>(
        this.parent.contentModule
      );
      if (
        this.virtualContentModule.activeKey === 'downArrow' ||
        this.virtualContentModule.activeKey === 'upArrow'
      ) {
        const row: Element = this.parent.getRowByIndex(
          this.virtualContentModule.blzRowIndex
        );
        if (row) {
          this.parent.selectRow(
            parseInt(row.getAttribute('data-rowindex'), 10)
          );
          // eslint-disable-next-line
          ((<HTMLTableRowElement>row).cells[0] as any).focus({
            preventScroll: true,
          });
        }
      }
    ...
    if (args.foreignColumnsData) {
      ...
      for (let i: number = 0; i < columns.length; i++) {
        if (
          args.foreignColumnsData[columns[parseInt(i.toString(), 10)].field]
        ) {
    ...
    if (this.parent.allowGrouping && this.parent.groupSettings.columns) {
      ...
      for (let i: number = 0; i < aggRows.length; i++) {
        const aggRow: AggregateRow = aggRows[
          parseInt(i.toString(), 10)
        ] as AggregateRow;
        for (let j: number = 0; j < aggRow.columns.length; j++) {
          let aggr: Object = {};
          const type: string | AggregateType[] =
            aggRow.columns[parseInt(j.toString(), 10)].type.toString();
          aggr = {
            type: type.toLowerCase(),
            field: aggRow.columns[parseInt(j.toString(), 10)].field,
          };
          agg.push(aggr);
        }
      }
      ...
      for (let k: number = 0; k < groupedCols.length; k++) {
        ...
        data = DataUtil.group(
          aggrds,
          groupedCols[parseInt(k.toString(), 10)],
          agg,
          null,
          null
        );
    ...
    if (this.parent[`${offsetTime}`] !== new Date().getTimezoneOffset() / 60) {
      if (this.parent.editSettings.mode !== 'Batch') {
        ...
        const batchChanges: Object = this.actionArgs[`${changes}`] || {
          changedRecords: [],
          addedRecords: [],
        };
        for (
          let i: number = 0;
          i < batchChanges[`${changedRecords}`].length;
          i++
        ) {
          for (let j: number = 0; j < args.result.length; j++) {
            if (
              batchChanges[`${changedRecords}`][parseInt(i.toString(), 10)][
                `${keyField}`
              ] === args.result[parseInt(j.toString(), 10)][`${keyField}`]
            ) {
        ...
        for (
          let i: number = 0;
          i < batchChanges[`${addedRecords}`].length;
          i++
        ) {
          for (let j: number = 0; j < args.result.length; j++) {
            if (
              batchChanges[`${addedRecords}`][parseInt(i.toString(), 10)][
                `${keyField}`
              ] === args.result[parseInt(j.toString(), 10)][`${keyField}`]
            ) {
              this.setClientOffSet(args, j);
    ...
    this.parent.renderModule.dataManagerSuccess(
      args,
      <NotifyArgs>this.actionArgs
    );
  ...
  public removeDisplayNone(): void {
    const renderedContentRows: NodeListOf<HTMLElement> = this.parent
      .getContentTable()
      .querySelectorAll('tr');
    for (let i: number = 0; i < renderedContentRows.length; i++) {
      const renderedContentCells: NodeListOf<HTMLElement> =
        renderedContentRows[parseInt(i.toString(), 10)].querySelectorAll('td');
  ...
  public setVirtualTrackHeight(args: {
    VisibleGroupedRowsCount: number;
  }): void {
    this.virtualHeight =
      args.VisibleGroupedRowsCount * this.parent.getRowHeight();
    this.virtualContentModule.setVirtualHeight(this.virtualHeight);
  }
  ...
  public setColVTableWidthAndTranslate(args?: { refresh: boolean }): void {
    if (
      (this.parent.enableColumnVirtualization &&
        this.virtualContentModule.prevInfo &&
      ...
      const translateX: number = this.virtualContentModule.getColumnOffset(
        this.virtualContentModule.startColIndex - 1
      );
      const width: string =
        this.virtualContentModule.getColumnOffset(
          this.virtualContentModule.endColIndex - 1
        ) -
        translateX +
        '';
    ...
    if (this.dataSourceChanged) {
      ...
      this.parent.getContentTable().parentElement.style.transform =
        'translate(0px,0px)';
  ...
  private setClientOffSet(args: ReturnType, index: number): void {
    ...
    args.result[parseInt(index.toString(), 10)] = DataUtil.parse.parseJson(
      JSON.stringify(args.result[parseInt(index.toString(), 10)])
    );
  ...
  private setServerOffSet(args: Object): void {
    ...
    const timeZone: number =
      (new Date().getTimezoneOffset() / 60) * 2 + this.parent[`${offsetTime}`];
    DataUtil.serverTimezoneOffset = timeZone;
  ...
  public onGroupClick(args: object): void {
    const adaptor: string = 'interopAdaptor';
    const content: string = 'contentModule';
    const invokeMethodAsync: string = 'invokeMethodAsync';
    const exactTopIndex: string = 'exactTopIndex';
    args[`${exactTopIndex}`] = Math.round(
      this.parent.element.querySelector('.e-content').scrollTop /
        this.parent.getRowHeight()
    );
    const rowHeight: string = 'rowHeight';
    args[`${rowHeight}`] = this.parent.getRowHeight();
    this.parent[`${adaptor}`]
      [`${invokeMethodAsync}`]('OnGroupExpandClick', args)
      .then(() => {
        this.parent[`${content}`].rowElements = [].slice.call(
          this.parent.getContentTable().querySelectorAll('tr.e-row[data-uid]')
        );
      });
  }
  ...
  public resetPersistData(args: string): void {
    ...
    for (let i: number = 0; i < gObj.columns.length; i++) {
      if (
        gObj.groupSettings.columns.indexOf(
          (gObj.columns[parseInt(i.toString(), 10)] as Column).field
        ) > -1
      ) {
    ...
    gObj.headerModule.refreshUI();
    for (let i: number = 0; i < gObj.columns.length; i++) {
      (gObj.columns[parseInt(i.toString(), 10)] as Column).editType = (
        gObj.columns[parseInt(i.toString(), 10)] as Column
  ...
  private contentColGroup(): void {
    ...
    contentTable.insertBefore(
      contentTable.querySelector(`#content-${gObj.element.id}colGroup`),
      contentTable.querySelector('tbody')
    );
    if (gObj.frozenRows) {
      const headerTable: Element = gObj
        .getHeaderContent()
        .querySelector('.e-table');
      headerTable.insertBefore(
        headerTable.querySelector(`#${gObj.element.id}colGroup`),
        headerTable.querySelector('tbody')
      );
    }
  }

  public dataFailure(args: { result: Object[] }): void {
    this.parent.renderModule.dataManagerFailure(
      args,
      <NotifyArgs>this.actionArgs
    );
    this.actionArgs = this.parent.currentAction = {};
