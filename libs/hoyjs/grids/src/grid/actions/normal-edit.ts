import { extend, select } from '@hoyui/base';
import { remove, isNullOrUndefined, updateBlazorTemplate } from '@hoyui/base';
import {
  IGrid,
  EJ2Intance,
  NotifyArgs,
...
export class NormalEdit {
  ...
  protected clickHandler(e: MouseEvent): void {
    ...
    if (
      ...
      isNullOrUndefined(gObj.element.querySelector('.' + literals.editedRow))
    ) {
      return;
    }
    if (
      ((parentsUntil(target, literals.gridContent) &&
        parentsUntil(parentsUntil(target, literals.gridContent), 'e-grid')
          .id === gObj.element.id) ||
        (gObj.frozenRows &&
      ...
      this.rowIndex = parentsUntil(target, literals.rowCell)
        ? parseInt(target.parentElement.getAttribute(literals.dataRowIndex), 10)
        : -1;
      if (gObj.isEdit) {
  ...
  protected dblClickHandler(e: MouseEvent): void {
    if (
      ...
      this.parent.editModule.startEdit(
        parentsUntil(e.target as Element, literals.row) as HTMLTableRowElement
      );
    }
  }
  ...
  private getEditArgs(
    ...
    for (let i: number = 0; i < primaryKeys.length; i++) {
      primaryKeyValues.push(
        getObject(primaryKeys[parseInt(i.toString(), 10)], editedData)
      );
    }
  ...
  protected startEdit(tr: Element): void {
    const gObj: IGrid = this.parent;
    this.rowIndex = this.editRowIndex = parseInt(
      tr.getAttribute(literals.dataRowIndex),
      10
    );
    if (
    ...
    const e: { data: Object; index: number; isScroll: boolean } = {
      data: undefined,
      index: this.rowIndex,
      isScroll: false,
    };
    ...
    if (isGroupAdaptive(gObj)) {
      ...
      this.previousData = extend(
        {},
        {},
        this.parent.getForeignKeyColumns().length
          ? this.parent.getRowObjectFromUID(tr.getAttribute('data-uid')).data
          : gObj.getCurrentViewRecords()[this.rowIndex],
        true
      );
    }
    const editedData: Object = extend(
      {},
      {},
      e.data || this.previousData,
      true
    );
    this.uid = tr.getAttribute('data-uid');
    const rowObj: Row<Column> = gObj.getRowObjectFromUID(this.uid);
    const args: CustomEditEventArgs = this.getEditArgs(
      editedData,
      rowObj,
      e.isScroll
    );
  ...
  private disabledShowAddRow(disable?: boolean, prevent?: boolean): void {
    ...
    const inputs: Element[] = [].slice.call(
      addRow ? addRow.querySelectorAll('.e-input') : []
    );
    if (addRow && addRow.querySelector('.e-unboundcell')) {
      const buttons: Element[] = [].slice.call(
        addRow.querySelector('.e-unboundcell').querySelectorAll('.e-btn')
      );
      for (let i: number = 0; i < buttons.length; i++) {
        if (!disable) {
          buttons[parseInt(i.toString(), 10)].classList.add('e-disabled');
          buttons[parseInt(i.toString(), 10)].setAttribute(
            'disabled',
            'disabled'
          );
        } else {
    ...
    if (inputs.length) {
      for (let i: number = 0; i < inputs.length; i++) {
        ...
        const error: Element = parentsUntil(input, 'e-rowcell').querySelector(
          '.e-error'
        );
        if (error) {
        ...
        if ((<EJ2Intance>input).ej2_instances) {
          ...
          if (!isNullOrUndefined(disable)) {
            (<EJ2Intance>input).ej2_instances[0].enabled =
              disable && column.allowEditing ? true : false;
          }
        } else {
          if (
            prevent &&
  ...
  private inlineEditHandler(editargs: EditEventArgs, tr: Element): void {
    ...
    if (
      ...
      (<{ selectionModule?: { preventFocus: boolean } }>gObj).selectionModule
    ) {
      (<{ selectionModule?: { preventFocus: boolean } }>(
        gObj
      )).selectionModule.preventFocus = true;
    ...
    if (
      gObj.editSettings.showAddNewRow &&
      !tr.classList.contains('e-addedrow')
    ) {
    ...
    if (gObj.editSettings.template) {
      gObj.editModule.applyFormValidation(
        undefined,
        editargs.form.ej2_instances[0].rules
      );
    }
    this.args = editargs;
  ...
  protected updateRow(index: number, data: Object): void {
    ...
    const args: SaveEventArgs = {
      ...
      cancel: false,
      previousData:
        gObj.getCurrentViewRecords()[parseInt(index.toString(), 10)],
    ...
    if (gObj.enableInfiniteScrolling) {
      ...
      const index: number = parseInt(
        (args.row as Element).getAttribute('data-rowindex'),
        10
      );
    ...
    if (args.promise) {
      args.promise
        .then(() => gObj.refresh())
        .catch((e: ReturnType) => this.edFail(e));
    } else {
      if (!gObj.enableInfiniteScrolling) {
        gObj.refresh();
      }
  ...
  protected endEdit(): void {
    ...
    const dlgWrapper: Element = select(
      '#' + gObj.element.id + '_dialogEdit_wrapper',
      document
    );
    const dlgForm: Element = isDlg
      ? dlgWrapper.querySelector('.e-gridform')
      : gObj.editSettings.showAddNewRow &&
        gObj.element.querySelector('.' + literals.editedRow)
      ? gObj.element
          .querySelector('.' + literals.editedRow)
          .getElementsByClassName('e-gridform')[0]
    ...
    let eleLength: number = [].slice.call(
      gObj.element.getElementsByClassName(literals.editedRow)
    ).length;
    if (
      !data.isAdd &&
      Object.keys(this.currentVirtualData).length &&
      !eleLength
    ) {
      eleLength = 1;
    }
    if (
      isDlg
        ? dlgWrapper.getElementsByClassName(literals.editedRow).length
        : eleLength
    ) {
      args.action = 'edit';
  ...
  private destroyElements(): void {
    ...
    if (
      !gObj.editSettings.showAddNewRow ||
      (gObj.editSettings.showAddNewRow &&
        gObj.element.querySelector('.e-editedrow'))
    ) {
  ...
  private editHandler(args: EditArgs): void {
    if (args.promise) {
      args.promise
        .then((e: ReturnType) => this.edSucc(e, args))
        .catch((e: ReturnType) => this.edFail(e));
    } else {
  ...
  private requestSuccess(args: Object): void {
    if (
      ...
      if (
        ...
        (<{ selectionModule?: { preventFocus: boolean } }>this.parent)
          .selectionModule
      ) {
        this.parent.element
          .querySelector('.e-dlgeditrow')
          .classList.remove('e-dlgeditrow');
      }
    }
  }
  ...
  private editSuccess(e: Object, args: EditArgs): void {
    ...
    if (
      ...
      const dragRowObject: Row<Column> =
        this.parent.getRowObjectFromUID(dragRowUid);
      for (
        let i: number = 0;
        ...
        rows = rows.filter(function (data: Row<Column>): boolean {
          ...
          if (flag) {
            const groupedColumn: string[] =
              thisRef.parent.groupSettings.columns[
      ...
      if (
        !isNullOrUndefined(dragRowObject) &&
        !isNullOrUndefined(dropRowObject) &&
        dragRowObject.parentUid !== dropRowObject.parentUid
      ) {
        (this.parent['groupModule'] as Group).groupedRowReorder(
          dragRowObject,
          dropRowObject
        );
      } else if (this.parent.aggregates.length) {
        this.parent.aggregateModule.refresh(
          args.data,
          this.parent.groupSettings.enableLazyLoading ? args.row : undefined
        );
      }
    } else if (this.parent.aggregates.length) {
      this.parent.aggregateModule.refresh(
    ...
    if (
      ...
      if (this.parent.editSettings.mode !== 'Dialog') {
        this.parent.selectRow(
          this.rowIndex > -1 ? this.rowIndex : this.editRowIndex
        );
      }
    }
    if (
      ...
      this.parent.groupSettings.columns.length &&
      ((this.parent as Grid).groupModule.getGroupAggregateTemplates(true)
        .length ||
        (this.parent as Grid).groupModule.getGroupAggregateTemplates(false)
          .length)
    ) {
      return;
    }
  ...
  private blazorTemplate(): void {
    ...
    if (
      ...
      updateBlazorTemplate(
        this.parent.element.id + 'editSettingsTemplate',
        'Template',
    ...
    for (let i: number = 0; i < cols.length; i++) {
      ...
      if (col.template) {
        updateBlazorTemplate(
          this.parent.element.id + col.uid,
          'Template',
          col,
          false
        );
      ...
      if (col.editTemplate) {
        updateBlazorTemplate(
          this.parent.element.id + col.uid + 'editTemplate',
          'EditTemplate',
          col
        );
      }
      ...
      for (let i: number = 0; i < tr.length; i++) {
        ...
        if (
          ...
          for (let j: number = 0; j < leftrightCells.length; j++) {
            if (
              leftrightCells[parseInt(j.toString(), 10)].classList.contains(
                'e-leftfreeze'
              )
            ) {
              leftrightCells[parseInt(j.toString(), 10)].style.left =
                (<{ valueX?: number }>cols[parseInt(j.toString(), 10)]).valueX -
              ...
              const idx: number = parseInt(
                leftrightCells[parseInt(j.toString(), 10)].getAttribute(
                  'data-colindex'
                ),
                10
              );
              leftrightCells[parseInt(j.toString(), 10)].style.right =
                (<{ valueX?: number }>cols[parseInt(idx.toString(), 10)])
                  .valueX +
                this.parent.translateX +
                'px';
            } else {
              leftrightCells[parseInt(j.toString(), 10)].style.left =
                this.parent.leftrightColumnWidth('left') -
                this.parent.translateX +
                'px';
              leftrightCells[parseInt(j.toString(), 10)].style.right =
  ...
  protected addRecord(data?: Object, index?: number): void {
    ...
    const rowData: { virtualData: Object; isScroll: boolean } = {
      virtualData: {},
      isScroll: false,
    };
    if (!gObj.editSettings.showAddNewRow) {
    ...
    for (let i: number = 0; i < cols.length; i++) {
      ...
      if (cols[parseInt(i.toString(), 10)].field) {
        if (cols[parseInt(i.toString(), 10)].type === 'string') {
          cols[parseInt(i.toString(), 10)].defaultValue = this.parent.sanitize(
            cols[parseInt(i.toString(), 10)].defaultValue as string
          );
        }
        DataUtil.setValue(
          cols[parseInt(i.toString(), 10)].field,
          cols[parseInt(i.toString(), 10)].defaultValue,
          this.previousData
        );
      }
    }
    const args: CustomAddEventArgs = {
      cancel: false,
      foreignKeyData: {}, //foreign key support
      requestType: 'add',
      data: this.previousData,
      type: events.actionBegin,
      index: index,
  ...
  private inlineAddHandler(addArgs: AddEventArgs): void {
    ...
    if (gObj.editSettings.template) {
      gObj.editModule.applyFormValidation(
        undefined,
        addArgs.form.ej2_instances[0].rules
      );
    }
  ...
  protected deleteRecord(fieldname?: string, data?: Object): void {
    ...
    if (data) {
      ...
      for (let i: number = 0; i < dataLen; i++) {
        ...
        const contained: boolean = gObj.currentViewData.some(
          (record: Object) => {
            tmpRecord = record;
            return (
              data[parseInt(i.toString(), 10)] ===
                getObject(fieldname, record) ||
              data[parseInt(i.toString(), 10)] === record
            );
          }
        );
        data[parseInt(i.toString(), 10)] = contained
          ? tmpRecord
          : data[parseInt(i.toString(), 10)][`${fieldname}`]
          ? data[parseInt(i.toString(), 10)]
          : { [fieldname]: data[parseInt(i.toString(), 10)] };
      }
    }
    const args: object = {
      requestType: 'delete',
    ...
    if (!isNullOrUndefined(this.parent.commandDelIndex)) {
      (<{ data?: Object[] }>args).data[0] = this.parent.getRowObjectFromUID(
        this.parent
          .getRowByIndex(this.parent.commandDelIndex)
  ...
  private stopEditStatus(): void {
    const gObj: IGrid = this.parent;
    const addElements: Element[] = [].slice.call(
      gObj.editSettings.showAddNewRow
        ? []
        : gObj.element.getElementsByClassName(literals.addedRow)
    );
    const editElements: Element[] = [].slice.call(
      gObj.element.getElementsByClassName(literals.editedRow)
    );
    ...
    for (let i: number = 0; i < editElements.length; i++) {
      editElements[parseInt(i.toString(), 10)].classList.remove(
        literals.editedRow
      );
    }
  }
  ...
  public addEventListener(): void {
    ...
    this.evtHandlers = [
      { event: events.crudAction, handler: this.editHandler },
