import { closest, isNullOrUndefined, KeyboardEventArgs } from '@hoyui/base';
import {
  click,
  keyPressed,
  commandClick,
...
import { ButtonModel } from '@hoyui/buttons';
...
export class CommandColumn {
  ...
  private initiateRender(): void {
    const cellFac: CellRendererFactory =
      this.locator.getService<CellRendererFactory>('cellRendererFactory');
    cellFac.addCellRenderer(
      CellType.CommandColumn,
      new CommandColumnRenderer(this.parent, this.locator)
    );
  }
  ...
  private commandClickHandler(e: Event): void {
    ...
    const row: Row<Column> = gObj.getRowObjectFromUID(
      closest(target, '.' + literals.row).getAttribute('data-uid')
    );
    ...
    for (let i: number = 0; i < cols.length; i++) {
      if (cols[parseInt(i.toString(), 10)].commands) {
        ...
        for (let j: number = 0; j < commandCols.length; j++) {
          ...
          if (
            commandCols[parseInt(j.toString(), 10)][`${idInString}`] === uid &&
            commandCols[parseInt(j.toString(), 10)][`${typeInString}`] === type
          ) {
            commandColumn = commandCols[parseInt(j.toString(), 10)];
          } else {
            const buttons: HTMLElement[] = [].slice.call(
              closest(target, '.e-unboundcell').querySelectorAll('button')
            );
            const index: number = buttons.findIndex(
              (ele: HTMLElement) => ele === target
            );
            if (
              index < commandCols.length &&
              commandCols[parseInt(index.toString(), 10)][`${typeInString}`] ===
                type &&
              String(
                commandCols[parseInt(j.toString(), 10)][`${idInString}`]
              ) === uid
            ) {
    ...
    this.parent.trigger(
      commandClick,
      args,
      (commandclickargs: CommandClickEventArgs) => {
        ...
        switch (type) {
          case 'Edit':
            ...
            gObj.editModule.startEdit(
              <HTMLTableRowElement>closest(target, 'tr')
            );
            break;
          case 'Cancel':
          ...
          case 'Delete':
            ...
            gObj.commandDelIndex = parseInt(
              closest(target, 'tr').getAttribute(literals.dataRowIndex),
              10
            );
            gObj.clearSelection();
    ...
    for (let i: number = 0; i < col.length; i++) {
      if (col[parseInt(i.toString(), 10)].commands) {
        ...
        for (let j: number = 0; j < commandCol.length; j++) {
          commandCol[parseInt(j.toString(), 10)][`${uid}`] =
            getUid('gridcommand');
        }
      }
    }
