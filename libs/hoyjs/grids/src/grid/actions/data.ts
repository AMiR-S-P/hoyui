...
export class Data implements IDataProcessor {
  ...
  public foreignKeyDataState: PendingState = {
    isPending: false,
    resolver: null,
  };
    ...
    if (this.parent.getDataModule().isRemote()) {
      this.parent
        ...
        .splice(
          e.toIndex,
          0,
          this.parent.getCurrentViewRecords().splice(e.fromIndex, 1)[0]
        );
    } else {
      this.dataManager.dataSource.json.splice(
        e.toIndex,
        0,
        this.dataManager.dataSource.json.splice(e.fromIndex, 1)[0]
      );
    }
  }
  ...
  private initDataManager(): void {
    ...
    this.dataManager =
      gObj.dataSource instanceof DataManager
        ? <DataManager>gObj.dataSource
        : isNullOrUndefined(gObj.dataSource)
        ? new DataManager()
        : new DataManager(gObj.dataSource);
  ...
  public generateQuery(
    skipPage?: boolean,
    isAutoCompleteCall?: boolean
  ): Query {
    const gObj: IGrid = this.parent;
    const query: Query = !isNullOrUndefined(gObj.getQuery())
      ? gObj.getQuery().clone()
      : new Query();
    if (this.parent.columnQueryMode === 'ExcludeHidden') {
      query.select(
        (<Column[]>this.parent.getColumns())
          .filter(
            (column: Column) =>
              !(
                (column.isPrimaryKey !== true && column.visible === false) ||
      ...
      for (let i: number = 0; i < columns.length; i++) {
        selectQueryFields.push(
          (columns[parseInt(i.toString(), 10)] as Column).field
        );
      }
  ...
  public aggregateQuery(query: Query): Query {
    ...
    for (let i: number = 0; i < rows.length; i++) {
      ...
      for (let j: number = 0; j < row.columns.length; j++) {
        const cols: AggregateColumnModel =
          row.columns[parseInt(j.toString(), 10)];
        const types: string[] =
          cols.type instanceof Array ? cols.type : [cols.type];
        for (let k: number = 0; k < types.length; k++) {
          query.aggregate(
            types[parseInt(k.toString(), 10)].toLowerCase(),
            cols.field
          );
        }
      }
  ...
  protected virtualGroupPageQuery(query: Query): Query {
    ...
    if (query.queries.length) {
      for (let i: number = 0; i < query.queries.length; i++) {
        if (
          query.queries[parseInt(i.toString(), 10)][`${fName}`] === 'onPage'
        ) {
  ...
  protected pageQuery(query: Query, skipPage?: boolean): Query {
    const gObj: IGrid = this.parent;
    const fName: string = 'fn';
    const args: { query: Query; skipPage: boolean } = {
      query: query,
      skipPage: false,
    };
    ...
    if (
      ...
      gObj.pageSettings.currentPage = Math.max(
        1,
        gObj.pageSettings.currentPage
      );
      ...
      if (
        !isNullOrUndefined(gObj.infiniteScrollModule) &&
        gObj.enableInfiniteScrolling
      ) {
        ...
        query.page(
          gObj.pageSettings.currentPage,
          gObj.allowPaging &&
            (gObj.pagerModule as Page) &&
            ((gObj.pagerModule as Page).pagerObj as Pager).isAllPage &&
            !gObj.isManualRefresh &&
            !this.dataManager.dataSource.offline &&
            !(this.dataManager.adaptor instanceof RemoteSaveAdaptor)
            ? null
  ...
  protected groupQuery(query: Query): Query {
    ...
    if (gObj.allowGrouping && gObj.groupSettings.columns.length) {
      ...
      for (let i: number = 0, len: number = columns.length; i < len; i++) {
        const column: Column = this.getColumnByField(
        ...
        const format: string | NumberFormatOptions | DateFormatOptions =
          column.format;
        if (isGrpFmt) {
          query.group(
            columns[parseInt(i.toString(), 10)],
            this.formatGroupColumn.bind(this),
            format
          );
        } else {
  ...
  protected sortQuery(query: Query): Query {
    const gObj: IGrid = this.parent;
    if (
      (gObj.allowSorting || gObj.allowGrouping) &&
      ...
      for (let i: number = columns.length - 1; i > -1; i--) {
        const col: Column = this.getColumnByField(
        ...
        let fn: Function | string =
          columns[parseInt(i.toString(), 10)].direction;
        if (col.sortComparer) {
        ...
        if (
          ...
          if (col.isForeignColumn() || col.sortComparer) {
            query.sortByForeignKey(
              col.field,
              fn,
              undefined,
              columns[parseInt(i.toString(), 10)].direction.toLowerCase()
            );
          } else {
      ...
      for (let i: number = 0, len: number = sortGrp.length; i < len; i++) {
        if (typeof sortGrp[parseInt(i.toString(), 10)].direction === 'string') {
          query.sortBy(
            sortGrp[parseInt(i.toString(), 10)].field,
            sortGrp[parseInt(i.toString(), 10)].direction
          );
        } else {
          const col: Column = this.getColumnByField(
            sortGrp[parseInt(i.toString(), 10)].field
          ...
          query.sortByForeignKey(
            ...
            sortGrp[parseInt(i.toString(), 10)].direction,
            undefined,
            col.getSortDirection().toLowerCase()
          );
        }
      }
    }
  ...
  public searchQuery(
    ...
    let fields: string[] =
      !isNullOrUndefined(sSettings.fields) && sSettings.fields.length
        ? sSettings.fields
        : this.getSearchColumnFieldNames();
    let predicateList: Predicate[] = [];
    let needForeignKeySearch: boolean = false;
    if (
      !isNullOrUndefined(this.parent.searchSettings.key) &&
      this.parent.searchSettings.key.length
    ) {
      needForeignKeySearch = this.parent
        .getForeignKeyColumns()
        .some((col: Column) => fields.indexOf(col.field) > -1);
      const adaptor: AdaptorOptions = !isForeignKey
        ? this.dataManager.adaptor
        : (fcolumn.dataSource as DataManager).adaptor;
      if (
        needForeignKeySearch ||
        ((<{ getModuleName?: Function }>adaptor).getModuleName &&
          (<{ getModuleName?: Function }>adaptor).getModuleName() ===
            'ODataV4Adaptor')
      ) {
        fields = isForeignKey ? [fcolumn.foreignKeyValue] : fields;
        for (let i: number = 0; i < fields.length; i++) {
          const column: Column = isForeignKey
            ? fcolumn
            : this.getColumnByField(fields[parseInt(i.toString(), 10)]);
          if (column.isForeignColumn() && !isForeignKey) {
            ...
            predicateList.push(
              new Predicate(
                ...
                sSettings.operator,
                sSettings.key,
                sSettings.ignoreCase,
                sSettings.ignoreAccent
              )
            );
          }
        }
  ...
  protected filterQuery(
    ...
    if (gObj.allowFiltering && gObj.filterSettings.columns.length) {
      const columns: PredicateModel[] = column
        ? column
        : gObj.filterSettings.columns;
      const colType: Object = {};
      for (const col of gObj.getColumns() as Column[]) {
        colType[col.field] = col.filter.type
          ? col.filter.type
          : gObj.filterSettings.type;
      }
      const foreignCols: PredicateModel[] = [];
      const defaultFltrCols: PredicateModel[] = [];
      for (const col of columns) {
        const gridColumn: Column = col.isForeignKey
          ? gObj.getColumnByUid(col.uid)
          : gObj.getColumnByField(col.field);
        if (
          isNullOrUndefined(col.type) &&
          gridColumn &&
          (gridColumn.type === 'date' ||
            gridColumn.type === 'datetime' ||
            gridColumn.type === 'dateonly')
        ) {
          col.type = col.isForeignKey
            ? gObj.getColumnByUid(col.uid).type
            : gObj.getColumnByField(col.field).type;
        }
        if (col.isForeignKey) {
      ...
      if (defaultFltrCols.length) {
        for (
          let i: number = 0, len: number = defaultFltrCols.length;
          i < len;
          i++
        ) {
          defaultFltrCols[parseInt(i.toString(), 10)].uid =
            defaultFltrCols[parseInt(i.toString(), 10)].uid ||
            this.parent.grabColumnByFieldFromAllCols(
              defaultFltrCols[parseInt(i.toString(), 10)].field,
              defaultFltrCols[parseInt(i.toString(), 10)].isForeignKey
            ).uid;
        }
        const excelPredicate: Predicate =
          CheckBoxFilterBase.getPredicate(defaultFltrCols);
      ...
      if (foreignCols.length) {
        for (const col of foreignCols) {
          ...
          if (
            ...
            const excelPredicate: Predicate =
              CheckBoxFilterBase.getPredicate(columns);
            for (const prop of Object.keys(excelPredicate)) {
              predicateList.push(<Predicate>excelPredicate[`${prop}`]);
            }
          }
  ...
  public getData(
    ...
    const key: string = this.getKey(
      args.foreignKeyData && Object.keys(args.foreignKeyData).length
        ? args.foreignKeyData
        : this.parent.getPrimaryKeyFieldNames()
    );
    this.parent.log('datasource_syntax_mismatch', {
      dataState: this.parent as IGrid,
    });
    if (this.parent.dataSource && 'result' in this.parent.dataSource) {
      ...
      switch (args.requestType) {
        case 'delete':
          ...
          if (len === 1) {
            ...
            crud = this.dataManager.saveChanges(
              changes,
              key,
              query.fromTable,
              query.requiresCount()
      ...
      if (
        crud &&
        !Array.isArray(crud) &&
        !crud.hasOwnProperty('deletedRecords')
      ) {
        return crud.then(() => {
  ...
  private executeQuery(query: Query): Promise<Object> {
    if (this.dataManager.ready) {
      ...
      ready
        .then(() => {
          (<Promise<Object>>this.dataManager.executeQuery(query)).then(
            (result: ReturnType) => {
  ...
  private formatGroupColumn(
    ...
    const formatVal: string | object = ValueFormatter.prototype.toView(
      value,
      column.getFormatter()
    );
    return formatVal;
  ...
  private crudActions(args: {
    ...
    switch (args.requestType) {
      case 'save':
        promise = this.dataManager.update(
          key,
          args.data,
          query.fromTable,
          query,
  ...
  public saveChanges(
    ...
    if ('result' in this.parent.dataSource) {
      ...
      const promise: Promise<Object> = this.dataManager.saveChanges(
        changes,
        key,
        query.fromTable,
        query,
  ...
  public isRemote(): boolean {
    return (
      this.dataManager.dataSource.offline !== true &&
      this.dataManager.dataSource.url !== undefined &&
  ...
  private removeRows(e: { indexes: number[]; records: Object[] }): void {
    ...
    if (
      ...
      this.dataManager.dataSource.json = json.filter(
        (value: Object) => e.records.indexOf(value) === -1
      );
    } else if (
      !this.parent.getDataModule().isRemote() &&
      !isNullOrUndefined(this.parent.dataSource) &&
      ...
      this.parent.dataSource = json.filter(
        (value: Object) => e.records.indexOf(value) === -1
      );
    }
  }
  ...
  private getColumnByField(field: string): Column {
    let col: Column;
    return (
      (<{ columnModel?: Column[] }>this.parent).columnModel.some(
        (column: Column) => {
          col = column;
  ...
  public getStateEventArgument(query: Query): PendingState {
    ...
    const state: { data?: string; pvtData?: Object[] } = adaptr.processQuery(
      dm,
      query
    );
  ...
  private eventPromise(
    args: { requestType?: string; foreignKeyData?: string[]; data?: Object },
    query?: Query,
    key?: string
  ): Deferred {
    ...
    const deff: Deferred = new Deferred();
    if (
      (args.requestType !== undefined ||
        (this.parent.groupSettings.disablePageWiseAggregates &&
          query.queries.some(
            (query: { fn: string }) => query.fn === 'onGroup'
          ))) &&
