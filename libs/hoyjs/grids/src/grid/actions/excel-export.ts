...
import { Query, DataManager, Group } from '@hoyui/data';
...
import { AutoFilters } from '@hoyui/excel-export/src/auto-filters';
import { defaultCurrencyCode } from '@hoyui/base';
...
export class ExcelExport {
  ...
  public Map(
    ...
    this.gridPool = {};
    if (
      (grid.childGrid || grid.detailTemplate) &&
      !(
        !isNullOrUndefined(exportProperties) &&
        exportProperties.hierarchyExportMode === 'None'
      )
    ) {
    ...
    if (
      ...
      exportProperties.dataSource =
        exportProperties.dataSource instanceof DataManager
          ? exportProperties.dataSource
          : new DataManager(exportProperties.dataSource);
      const query: Query = exportProperties.query
        ? exportProperties.query
        : new Query();
      if (isNullOrUndefined(query.isCountRequired) || gObj.aggregates) {
        query.isCountRequired = true;
      }
      return new Promise((resolve: Function) => {
        const dataManager: Promise<Object> = (
          exportProperties.dataSource as DataManager
        ).executeQuery(query);
        dataManager.then((r: ReturnType) => {
          this.init(gObj);
          this.processInnerRecords(
            gObj,
            exportProperties,
            isMultipleExport,
            workbook,
      ...
      if (
        ...
        exportProperties.hierarchyExportMode =
          exportProperties.hierarchyExportMode === 'None' ? 'None' : 'All';
  ...
  private processInnerRecords(
    ...
    const isExportPropertiesPresent: boolean =
      !isNullOrUndefined(exportProperties);
    if (
      isExportPropertiesPresent &&
      !isNullOrUndefined(exportProperties.multipleExport)
    ) {
      this.expType = !isNullOrUndefined(exportProperties.multipleExport.type)
        ? exportProperties.multipleExport.type
        : 'AppendToSheet';
    ...
    if (isExportPropertiesPresent) {
      if (!isNullOrUndefined(isMultipleExport)) {
        if (
          !isNullOrUndefined(exportProperties.header) &&
          (isMultipleExport || this.expType === 'NewSheet')
        ) {
          this.processExcelHeader(
            JSON.parse(JSON.stringify(exportProperties.header))
          );
        }
    ...
    this.includeHiddenColumn = isExportPropertiesPresent
      ? exportProperties.includeHiddenColumn
      : false;
    ...
    return new Promise((resolve: Function, reject: Function) => {
      ...
      if (!isNullOrUndefined(this.footer)) {
        if (
          (this.expType === 'AppendToSheet' && !isMultipleExport) ||
      ...
      if (
        ...
        gObj.getVisibleColumns().length &&
        isExportPropertiesPresent &&
        exportProperties.enableFilter
      ) {
        const headerRowLen: number = exportProperties.header
          ? exportProperties.header.headerRows ||
            exportProperties.header.rows.length
          : 0;
        const autoFilters: AutoFilters = {
          row: colDepth + headerRowLen,
          column: this.groupedColLength
            ? this.groupedColLength + 1
            : this.sheet.columns[0].index,
      ...
      if (!isMultipleExport) {
        if (this.isCsvExport) {
          if (
            isExportPropertiesPresent &&
            !isNullOrUndefined(exportProperties.separator) &&
          ...
          if (!this.isBlob) {
            ...
            this.blobPromise = book.saveAsBlob(
  ...
  private organiseRows(
    ...
    for (let i: number = 0; i < rows.length; i++) {
      ...
      if (childRows) {
        ...
        initialIndex = this.organiseRows(
          childRows,
          initialIndex,
          organisedRows
        );
  ...
  private processGridExport(
    ...
    if (
      !isNullOrUndefined(exportProperties) &&
      !isNullOrUndefined(exportProperties.theme)
    ) {
    ...
    if (
      ...
      gObj.hierarchyPrintMode =
        exportProperties.hierarchyExportMode || 'Expanded';
    }
    ...
    const headerRow: IHeader = helper.getHeaders(
      gColumns,
      this.includeHiddenColumn
    );
    const groupIndent: number = gObj.groupSettings.columns.length
      ? gObj.groupSettings.columns.length - 1
      : 0;
    excelRows = this.processHeaderContent(
      gObj,
      headerRow,
      groupIndent,
      excelRows
    ...
    if (
      !isNullOrUndefined(exportProperties) &&
      Object.keys(exportProperties).length &&
      isNullOrUndefined(exportProperties.dataSource)
    ) {
      if (
        exportProperties.exportType === 'CurrentPage' &&
        ...
        excelRows = this.processRecordContent(
          gObj,
          r,
          headerRow,
          exportProperties,
  ...
  private processRecordContent(
    ...
    if (!isNullOrUndefined((returnType as ReturnType).aggregates)) {
      if (
        !isNullOrUndefined(currentViewRecords) &&
        !this.parent.groupSettings.enableLazyLoading
      ) {
        this.processAggregates(
          gObj,
          (returnType as ReturnType).result,
          excelRow,
          currentViewRecords
        );
      } else {
        const result: Object[] = ((returnType as ReturnType).result as Group)
          .GroupGuid
          ? ((returnType as ReturnType).result as Group).records
          : (returnType as ReturnType).result;
  ...
  private processGroupedRows(
    ...
    for (const item of dataSource) {
      ...
      let value: string =
        gObj.getColumnByField(item.field).headerText +
        ': ' +
      ...
      const captionModelGen: CaptionSummaryModelGenerator =
        new CaptionSummaryModelGenerator(gObj);
      const groupCaptionSummaryRows: Row<AggregateColumnModel>[] =
        captionModelGen.generateRows(item);
      this.fillAggregates(
        gObj,
        groupCaptionSummaryRows,
        dataSource.level + dataSource.childLevels - 1,
        excelRows,
        this.rowLength
      );
      cells.push(cell);
      if (excelRows[excelRows.length - 1].cells.length > 0) {
        let lIndex: number =
          dataSource.level +
          dataSource.childLevels +
          groupCaptionSummaryRows[0].cells.length;
        let hIndex: number = 0;
        ...
        while (
          hIndex <
          headerRow.columns.length + level + dataSource.childLevels
        ) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const sCell: any = {};
      ...
      if (
        ...
        this.processAggregates(
          ...
          excelRows,
          undefined,
          level - 1 + dataSource.childLevels,
          true
        );
      } else {
        startIndex = this.processRecordRows(
          gObj,
          item.items,
          headerRow,
          level - 1,
    ...
    for (const row of rows) {
      ...
      for (let c: number = 0, len: number = row.cells.length; c < len; c++) {
        ...
        const cellValue: string = !isNullOrUndefined(field)
          ...
          : '';
        let value: string | number | Date = !isNullOrUndefined(cellValue)
          ? cellValue
          : '';
        ...
        if (!isNullOrUndefined(value)) {
          let cell: ExcelCell = {};
          const idx: number =
            index + level + (<{ childGridLevel?: number }>gObj).childGridLevel;
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ...
          if (
            !isNullOrUndefined(excelCellArgs.image) &&
            !isNullOrUndefined(excelCellArgs.image.base64)
          ) {
          ...
          if (!isNullOrUndefined(excelCellArgs.hyperLink)) {
            (excelCellArgs.cell as ExcelCell).hyperlink = {
              target: excelCellArgs.hyperLink.target,
            };
            excelCellArgs.value =
              excelCellArgs.hyperLink.displayText || excelCellArgs.value;
          }
          ...
          if (
            excelCellArgs.data === '' &&
            (<{ childGridLevel?: number }>gObj).childGridLevel &&
            index === 1
          ) {
          ...
          if (!isNullOrUndefined(excelCellArgs.style)) {
            ...
            cell.style = this.mergeOptions(
              this.styles[parseInt(styleIndex.toString(), 10)],
              excelCellArgs.style
            );
          } else {
            cell.style = {
      ...
      if (
        (row.isExpand || this.isChild) &&
        (!isNullOrUndefined(gObj.childGrid) ||
          !isNullOrUndefined(gObj.detailTemplate))
      ) {
        if (!isNullOrUndefined(gObj.childGrid)) {
          gObj.isPrinting = true;
          const exportType: ExportType =
            !isNullOrUndefined(excelExportProperties) &&
            excelExportProperties.exportType
              ? excelExportProperties.exportType
              : 'AllPages';
          const returnVal: { childGrid: IGrid; element: HTMLElement } =
          ...
          (<{ childGridLevel?: number }>childGridObj).childGridLevel =
            (<{ childGridLevel?: number }>gObj).childGridLevel + 1;
          const childExportProperties: ExcelExportProperties = extend(
            excelExportProperties,
            {
              columns: null,
              dataSource: null,
          ...
          (<{ childRows?: ExcelRow[] }>excelRow).childRows =
            this.processDetailTemplate(args);
        }
      }
  ...
  private processDetailTemplate(
    ...
    if (templateData.value.columnHeader || templateData.value.rows) {
      const processCell: Function = (
        currentCell: DetailTemplateCell,
        isHeader?: boolean
      ) => {
        ...
        if (
          !isNullOrUndefined(currentCell.image) &&
          !isNullOrUndefined(currentCell.image.base64)
        ) {
      ...
      const processRow: Function = (
        ...
        for (let j: number = 0; j < currentRow.cells.length; j++) {
          const currentCell: DetailTemplateCell =
            currentRow.cells[parseInt(j.toString(), 10)];
  ...
  private setImage(
    ...
    this.columns[idx - 1].width =
      args.image.width || this.columns[idx - 1].width;
  ...
  private childGridCell(
    excelRow: ExcelRow,
    childGridObj: IGrid,
    excelExportProps: ExcelExportProperties,
    gRow: Row<Column>
  ): (value: Object) => Object {
    return (result: ReturnType): Object => {
      ...
      (<{ childRows?: ExcelRow[] }>excelRow).childRows = this.processGridExport(
        childGridObj,
        excelExportProps,
        result
      );
      ...
      for (let i: number = 0; i < rows.length; i++) {
        rows[parseInt(i.toString(), 10)].grouping = {
          outlineLevel:
            intent + (<{ childGridLevel?: number }>childGridObj).childGridLevel,
  ...
  private processAggregates(
    ...
    if (gObj.groupSettings.columns.length > 0 && byGroup) {
      ...
      const groupSummaryRows: Row<AggregateColumnModel>[] =
        groupSummaryModel.generateRows(<Object>data, {
          level: (<Group>data).level,
        });
      if (groupSummaryRows.length > 0) {
        excelRows = this.fillAggregates(
          gObj,
          groupSummaryRows,
          indent,
          excelRows
      ...
      indent =
        gObj.groupSettings.columns.length > 0 && !byGroup
          ...
          : indent;
      const sRows: Row<AggregateColumnModel>[] = summaryModel.generateRows(
        data,
        (<SummaryData>rec).aggregates,
        null,
        null,
        columns
      );
    ...
    for (const row of rows) {
      ...
      for (const cell of row.cells) {
        ...
        if (!isNullOrUndefined(cell.attributes.index)) {
          columnsDetails = this.parent.getColumnByIndex(
            cell.attributes.index as number
          );
        }
        if (
          cell.cellType === CellType.DetailFooterIntent ||
          columnsDetails.type === 'checkbox' ||
          columnsDetails.commands
        ) {
          continue;
        }
        if (cell.visible || this.includeHiddenColumn) {
          ...
          if (cell.isDataCell) {
              ...
              for (const key of Object.keys(row.data[cell.column.field])) {
                if (key === cell.column.type) {
                  if (!isNullOrUndefined(row.data[cell.column.field].Sum)) {
                    eCell.value =
                      row.data[cell.column.field][`${cell.column.field} - sum`];
                  } else if (
                    !isNullOrUndefined(row.data[cell.column.field].Average)
                  ) {
                    eCell.value =
                      row.data[cell.column.field][
                        `${cell.column.field} - average`
                      ];
                  } else if (
                    !isNullOrUndefined(row.data[cell.column.field].Max)
                  ) {
                    eCell.value =
                      row.data[cell.column.field][`${cell.column.field} - max`];
                  } else if (
                    !isNullOrUndefined(row.data[cell.column.field].Min)
                  ) {
                    eCell.value =
                      row.data[cell.column.field][`${cell.column.field} - min`];
                  } else if (
                    !isNullOrUndefined(row.data[cell.column.field].Count)
                  ) {
                    eCell.value =
                      row.data[cell.column.field][
                        `${cell.column.field} - count`
                      ];
                  } else if (
                    !isNullOrUndefined(row.data[cell.column.field].TrueCount)
                  ) {
                    eCell.value =
                      row.data[cell.column.field][
                        `${cell.column.field} - truecount`
                      ];
                  } else if (
                    !isNullOrUndefined(row.data[cell.column.field].FalseCount)
                  ) {
                    eCell.value =
                      row.data[cell.column.field][
                        `${cell.column.field} - falsecount`
                      ];
                  } else if (
                    !isNullOrUndefined(row.data[cell.column.field].Custom)
                  ) {
            ...
            const gridCellStyle: { textAlign?: ExcelHAlign } =
              cell.attributes.style;
            if (gridCellStyle.textAlign) {
              eCell.style.hAlign =
                gridCellStyle.textAlign.toLowerCase() as ExcelHAlign;
            }
      ...
      if (!isNullOrUndefined(customIndex)) {
        ...
        if (
          ...
          !(
            gObj.groupSettings.enableLazyLoading &&
            isNullOrUndefined(
              excelRows[excelRows.length - 1][`${dummyGrouping}`]
            )
          )
        ) {
          const level: number =
  ...
  private aggregateStyle(
    ...
    if (typeof col.format === 'object') {
      ...
      style.numberFormat = !isNullOrUndefined(format.format)
        ? format.format
        : format.skeleton;
  ...
  private getAggreateValue(
    ...
    row: Row<AggregateColumnModel>
  ): NodeList | string {
    const templateFn: { [x: string]: Function } = {};
    templateFn[getEnumValue(CellType, cell.cellType)] = compile(template);
    let txt: NodeList | string;
    const data: Object =
      row.data[cell.column.field ? cell.column.field : cell.column.columnName];
    if (
      (this.parent.isReact ||
        this.parent.isVue ||
        this.parent.isVue3 ||
  ...
  private mergeOptions(JSON1: Object, JSON2: Object): Object {
    ...
    for (let index: number = 0; index < attrname.length; index++) {
      if (attrname[parseInt(index.toString(), 10)] !== 'name') {
        result[attrname[parseInt(index.toString(), 10)]] =
          JSON1[attrname[parseInt(index.toString(), 10)]];
      }
    }
    ...
    for (let index: number = 0; index < attrname.length; index++) {
      if (attrname[parseInt(index.toString(), 10)] !== 'name') {
        result[attrname[parseInt(index.toString(), 10)]] =
          JSON2[attrname[parseInt(index.toString(), 10)]];
      }
    }
    return result;
  }
  ...
  private getColumnStyle(gObj: IGrid, columnIndex: number): number {
    ...
    for (const style of this.styles) {
      if (
        ...
        gObj.element.id + 'column' + columnIndex
      ) {
        return index;
      }
      index++;
  ...
  private processHeaderContent(
    ...
    for (let col: number = 0; col < gridColumns.length; col++) {
      this.parseStyles(
        gObj,
        gridColumns[parseInt(col.toString(), 10)],
        this.getRecordThemeStyle(this.theme) as ExcelStyle,
        indent + col + 1
      );
    }
    let templateRowHeight: number;
    for (let row: number = 0; row < gridRows.length; row++) {
      ...
      for (
        let column: number = 0;
        column < gridRows[parseInt(row.toString(), 10)].cells.length;
        column++
      ) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        let style: any = {};
        ...
        const gridCell: Cell<Column> =
          gridRows[parseInt(row.toString(), 10)].cells[
            parseInt(column.toString(), 10)
          ];

        if (
          gridCell.cellType === CellType.HeaderIndent ||
          gridCell.cellType === CellType.DetailHeader
        ) {
          continue;
        }

        let result: { contains: boolean; index: number } = {
          contains: true,
          index: 1,
        };
        while (result.contains) {
          ...
          if (!result.contains) {
            cell.index =
              result.index + (<{ childGridLevel?: number }>gObj).childGridLevel;
            break;
          }
        }
        if (!isNullOrUndefined(gridCell.rowSpan) && gridCell.rowSpan !== 1) {
          ...
          for (let i: number = rowIndex; i < gridCell.rowSpan + rowIndex; i++) {
            const spannedCell: { rowIndex: number; columnIndex: number } = {
              rowIndex: 0,
              columnIndex: 0,
            };
        ...
        if (!isNullOrUndefined(gridCell.column.headerTextAlign)) {
          style.hAlign =
            gridCell.column.headerTextAlign.toLowerCase() as ExcelHAlign;
        }

        const excelHeaderCellArgs: ExcelHeaderQueryCellInfoEventArgs = {
          cell: cell,
          gridCell: gridCell,
          style: style,
        };
        ...
        if (
          !isNullOrUndefined(excelHeaderCellArgs.image) &&
          !isNullOrUndefined(excelHeaderCellArgs.image.base64)
        ) {
          templateRowHeight = this.setImage(
            excelHeaderCellArgs,
            currentCellIndex
          );
        }
        ...
        if (!isNullOrUndefined(excelHeaderCellArgs.hyperLink)) {
          (excelHeaderCellArgs.cell as ExcelCell).hyperlink = {
  ...
  private processExcelHeader(header: ExcelHeader): void {
    if (
      !isNullOrUndefined(header.rows) &&
      (this.expType === 'NewSheet' || this.rowLength === 1)
    ) {
      let noRows: number;
  ...
  private getIndex(
    spannedCells: ISpannedCell[],
    rowIndex: number,
    columnIndex: number
  ): { contains: boolean; index: number } {
    for (const spannedCell of spannedCells) {
      if (
        spannedCell.rowIndex === rowIndex &&
        spannedCell.columnIndex === columnIndex
      ) {
  ...
  private parseStyles(
    ...
    if (!isNullOrUndefined(col.format)) {
      if (typeof col.format === 'object') {
        ...
        if (!isNullOrUndefined(format.type)) {
          style.type =
            format.type === 'dateonly' ? 'date' : format.type.toLowerCase();
        }
      } else {
        style.numberFormat = col.format;
    ...
    if (
      ...
      this.columns.push({
        index: index + (<{ childGridLevel?: number }>gObj).childGridLevel,
        width:
          typeof col.width === 'number'
            ? col.width
            : this.helper.getConvertedWidth(col.width),
      });
    }
  }
