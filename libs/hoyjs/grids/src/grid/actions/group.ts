...
import { isNullOrUndefined, extend } from '@hoyui/base';
...
import { DataUtil, Query, Group as dmGroup, Predicate } from '@hoyui/data';
...
export class Group implements IAction {
  ...
  private helper: Function = (e: { sender: MouseEvent }) => {
    ...
    const element: HTMLElement = target.classList.contains('e-groupheadercell')
      ? (target as HTMLElement)
      : (parentsUntil(target, 'e-groupheadercell') as HTMLElement);
    if (
      !element ||
      (!target.classList.contains('e-drag') &&
        this.groupSettings.allowReordering)
    ) {
      return false;
    }
    this.column = gObj.getColumnByField(
      element.firstElementChild.getAttribute('ej-mappingname')
    );
  ...
  private drag: Function = (e: {
    ...
    const cloneElement: HTMLElement = this.parent.element.querySelector(
      '.e-cloneproperties'
    ) as HTMLElement;
    this.parent.trigger(events.columnDrag, {
      target: target,
    ...
    if (!this.groupSettings.allowReordering) {
      ...
      if (
        !(
          parentsUntil(target as Element, literals.gridContent) ||
          parentsUntil(target as Element, 'e-headercell')
        )
      ) {
  ...
  private dragStop: Function = (e: {
    target: HTMLElement;
    event: MouseEventArgs;
    helper: Element;
  }) => {
    this.parent.element.classList.remove('e-ungroupdrag');
    const preventDrop: boolean = !(
      parentsUntil(e.target, literals.gridContent) ||
      parentsUntil(e.target, 'e-gridheader')
    );
    if (this.groupSettings.allowReordering && preventDrop) {
      ...
      if (parentsUntil(e.target, 'e-groupdroparea')) {
        ...
        const field: string = this.parent.getColumnByUid(
          e.helper.getAttribute('e-mappinguid')
        ).field;
  ...
  private animateDropper: Function = (e: {
    target: HTMLElement;
    event: MouseEventArgs;
    helper: Element;
  }) => {
    const uid: string = this.parent.element
      .querySelector('.e-cloneproperties')
    ...
    const grouped: string[] = [].slice
      .call(this.element.getElementsByClassName('e-groupheadercell'))
      .map((e: Element) =>
        e.querySelector('div').getAttribute('ej-mappingname')
      );
    const cols: string[] = JSON.parse(JSON.stringify(grouped));
    if (dropTarget || parent) {
      if (dropTarget) {
        const dropField: string = dropTarget
          .querySelector('div[ej-mappingname]')
          .getAttribute('ej-mappingname');
  ...
  private drop: Function = (e: DropEventArgs) => {
    const gObj: IGrid = this.parent;
    const column: Column = gObj.getColumnByUid(
      e.droppedElement.getAttribute('e-mappinguid')
    );
    this.element.classList.remove('e-hover');
    remove(e.droppedElement);
    this.aria.setDropTarget(
      <HTMLElement>this.parent.element.querySelector('.e-groupdroparea'),
      false
    );
    this.aria.setGrabbed(
      <HTMLElement>(
        this.parent.getHeaderTable().querySelector('[aria-grabbed=true]')
      ),
      false
    );
    if (
      ...
      parentsUntil(
        gObj.getColumnHeaderByUid(column.uid),
        'e-grid'
      ).getAttribute('id') !== gObj.element.getAttribute('id')
    ) {
  ...
  private columnDrag(e: { target: Element; column: Column }): void {
    ...
    if (
      !(
        e.column.allowGrouping &&
        (parentsUntil(e.target as Element, 'e-groupdroparea') ||
          (parentsUntil(e.target as Element, 'e-headercell') &&
            parentsUntil(e.target as Element, 'e-headercell').isEqualNode(
              this.parent.getColumnHeaderByField(e.column.field)
            )))
      ) &&
      !(
  ...
  private columnDragStart(e: { target: Element; column: Column }): void {
    ...
    const dropArea: HTMLElement = <HTMLElement>(
      this.parent.element.querySelector('.e-groupdroparea')
    );
    this.aria.setDropTarget(dropArea, e.column.allowGrouping);
    const element: Element = e.target.classList.contains('e-headercell')
      ? e.target
      : parentsUntil(e.target as Element, 'e-headercell');
  ...
  private keyPressHandler(e: KeyboardEventArgs): void {
    ...
    if (
      ...
      for (let i: number = 0; i < focusableGroupedItems.length; i++) {
        if (e.target === focusableGroupedItems[parseInt(i.toString(), 10)]) {
          ...
          (
            focusableGroupedItems[parseInt(index.toString(), 10)] as HTMLElement
          ).focus();
          return;
        }
      }
    ...
    switch (e.action) {
      ...
      case 'altUpArrow':
        ...
        const selected: number[] = gObj.allowSelection
          ? gObj.getSelectedRowIndexes()
          : [];
        if (selected.length) {
          e.preventDefault();
          const rows: HTMLCollection = gObj
            .getContentTable()
            .querySelector(literals.tbody).children;
          const dataRow: HTMLTableRowElement = gObj.getDataRows()[
            selected[selected.length - 1]
          ] as HTMLTableRowElement;
          let grpRow: Element;
          for (let i: number = dataRow.rowIndex; i >= 0; i--) {
            if (
              !rows[parseInt(i.toString(), 10)].classList.contains(
                literals.row
              ) &&
              !rows[parseInt(i.toString(), 10)].classList.contains(
                'e-detailrow'
      ...
      case 'enter':
        if ((e.target as Element).classList.contains('e-groupsort')) {
          ...
          (e.target as Element).classList.contains('e-ungroupbutton')
        ) {
        ...
        if (
          element &&
          (element.classList.contains('e-icon-grightarrow') ||
            element.classList.contains('e-icon-gdownarrow'))
        ) {
          element = element.parentElement;
        }
        // eslint-disable-next-line no-case-declarations
        const row: Element = element
          ? element.parentElement.querySelector('[class^="e-record"]')
          : null;
        if (!row) {
          break;
        }
        if (
          ...
          (element.children[0].classList.contains('e-icon-grightarrow') ||
      ...
      case 'ctrlSpace':
        ...
        if (elem && elem.classList.contains('e-headercell')) {
          e.preventDefault();
          const column: Column = gObj.getColumnByUid(
            elem.firstElementChild.getAttribute('e-mappinguid')
          );
          if (
            column.field &&
            gObj.groupSettings.columns.indexOf(column.field) < 0
          ) {
            this.groupColumn(column.field);
          } else {
            this.ungroupColumn(column.field);
  ...
  public getFocusableGroupedItems(): Element[] {
    ...
    if (this.groupSettings.columns.length) {
      const focusableGroupedHeaderItems: NodeListOf<Element> =
        this.element.querySelectorAll('.e-groupheadercell');
      for (let i: number = 0; i < focusableGroupedHeaderItems.length; i++) {
        focusableGroupedItems.push(
          focusableGroupedHeaderItems[parseInt(i.toString(), 10)].querySelector(
            '.e-grouptext'
          )
        );
        focusableGroupedItems.push(
  ...
  private onFocusIn(e: FocusEvent): void {
    if (
      ...
      removeClass(
        [
          this.parent.focusModule.currentInfo.element,
          this.parent.focusModule.currentInfo.elementToFocus,
        ],
        ['e-focused', 'e-focus']
      );
  ...
  private addOrRemoveFocus(e: FocusEvent): void {
    if (
      ...
      (e.target as HTMLElement).classList.contains('e-grouptext') ||
      (e.target as HTMLElement).classList.contains('e-groupsort') ||
      (e.target as HTMLElement).classList.contains('e-ungroupbutton')
    ) {
      const target: Element = (e.target as HTMLElement).classList.contains(
        'e-grouptext'
      )
        ? (e.target as Element).parentElement.parentElement
        : (e.target as Element);
      if (e.type === 'focusin') {
  ...
  private clickHandler(e: MouseEventArgs): void {
    ...
    if (
      (e.target as Element).classList.contains('e-icon-grightarrow') ||
      (e.target as Element).classList.contains('e-icon-gdownarrow')
    ) {
      e.preventDefault();
    }
    const trgtEle: HTMLTableCellElement =
      (parentsUntil(
        e.target as Element,
        'e-recordplusexpand'
    ...
    if (
      trgtEle &&
      (trgtEle.children[0].classList.contains('e-icon-gdownarrow') ||
        trgtEle.children[0].classList.contains('e-icon-grightarrow'))
    ) {
  ...
  private toogleGroupFromHeader(target: Element): void {
    if (this.groupSettings.showToggleButton) {
      if (target.classList.contains('e-grptogglebtn')) {
        if (target.classList.contains('e-toggleungroup')) {
          this.ungroupColumn(
            this.parent.getColumnByUid(
              target.parentElement.getAttribute('e-mappinguid')
            ).field
          );
        } else {
          this.groupColumn(
        ...
        if (target.classList.contains('e-toggleungroup')) {
          this.ungroupColumn(
            target.parentElement.getAttribute('ej-mappingname')
          );
        }
      }
    }
  ...
  private applySortFromTarget(target: Element): void {
    ...
    if (
      gObj.allowSorting &&
      gHeader &&
      !target.classList.contains('e-ungroupbutton') &&
      !target.classList.contains('e-toggleungroup')
    ) {
      const field: string =
        gHeader.firstElementChild.getAttribute('ej-mappingname');
      if (
        gObj.getColumnHeaderByField(field).getElementsByClassName('e-ascending')
          .length
      ) {
  ...
  public expandCollapseRows(target: Element): void {
    const trgt: HTMLTableCellElement =
      (parentsUntil(target, 'e-recordplusexpand') as HTMLTableCellElement) ||
      (parentsUntil(target, 'e-recordpluscollapse') as HTMLTableCellElement);
    if (trgt) {
      const rowNodes: HTMLElement[] = [].slice.call(
        this.parent.getContentTable().querySelector(literals.tbody).children
      );
      if (this.parent.editSettings.showAddNewRow) {
        if (rowNodes[0].classList.contains('e-addedrow')) {
          rowNodes.shift();
        } else if (
          rowNodes[rowNodes.length - 1].classList.contains('e-addedrow')
        ) {
          rowNodes.pop();
        }
      }
      ...
      const gObj: IGrid = this.parent;
      const indent: number =
        trgt.parentElement.getElementsByClassName('e-indentcell').length;
      ...
      if (trgt.classList.contains('e-recordpluscollapse')) {
        ...
        trgt.firstElementChild.className =
          'e-icons e-gdiagonaldown e-icon-gdownarrow';
        trgt.firstElementChild.setAttribute(
          'title',
          this.l10n.getConstant('Expanded')
        );
        expand = true;
        ...
        if (this.parent.groupSettings.enableLazyLoading) {
          ...
          (this.parent.enableVirtualization
            ? (this.parent.lazyLoadRender as GroupLazyLoadRenderer)
            : (this.parent.contentModule as GroupLazyLoadRenderer)
          ).captionExpand(trgt.parentElement as HTMLTableRowElement);
        }
      } else {
        isHide = true;
        ...
        trgt.firstElementChild.setAttribute(
          'title',
          this.l10n.getConstant('Collapsed')
        );
        if (isGroupAdaptive(gObj)) {
        ...
        if (this.parent.groupSettings.enableLazyLoading) {
          (this.parent.enableVirtualization
            ? (this.parent.lazyLoadRender as GroupLazyLoadRenderer)
            : (this.parent.contentModule as GroupLazyLoadRenderer)
          ).captionCollapse(trgt.parentElement as HTMLTableRowElement);
        }
      }
      this.aria.setExpand(trgt, expand);
      if (
        !isGroupAdaptive(gObj) &&
        ...
        let cacheStartIdx: number =
          gObj.enableInfiniteScrolling &&
          gObj.infiniteScrollSettings &&
          gObj.infiniteScrollSettings.enableCache &&
          rowObjs.length !== rowNodes.length
            ? Array.from(rowNodes).indexOf(trgt.parentElement)
            : undefined;
        for (let i: number = startIdx; i < rowObjs.length; i++) {
          if (
            i > startIdx &&
            rowObjs[parseInt(i.toString(), 10)].indent === indent
          ) {
            break;
          }
          if (rowObjs[parseInt(i.toString(), 10)].isDetailRow) {
            const visible: boolean =
              rowObjs[i - 1].isExpand && rowObjs[i - 1].visible;
            if (
              cacheStartIdx &&
              cacheStartIdx > 0 &&
              cacheStartIdx < rowNodes.length
            ) {
              (
                rowNodes[parseInt(cacheStartIdx.toString(), 10)] as HTMLElement
              ).style.display = visible ? '' : 'none';
            } else if (isNullOrUndefined(cacheStartIdx)) {
              (
                rowNodes[parseInt(i.toString(), 10)] as HTMLElement
              ).style.display = visible ? '' : 'none';
            }
          } else if (
            rowsState[rowObjs[parseInt(i.toString(), 10)].parentUid] === false
          ) {
            ...
            if (
              !(
                rowObjs[parseInt(i.toString(), 10)].isDataRow ||
                rowObjs[parseInt(i.toString(), 10)].isCaptionRow ||
                rowObjs[parseInt(i.toString(), 10)].isDetailRow ||
                rowObjs[parseInt(i.toString(), 10)].isAggregateRow
              )
            ) {
              const visible: boolean = rowObjs[
                parseInt(i.toString(), 10)
              ].cells.some(
                (cell: Cell<AggregateColumnModel>) =>
          ...
          if (rowObjs[parseInt(i.toString(), 10)].isCaptionRow) {
            rowsState[rowObjs[parseInt(i.toString(), 10)].uid] =
              rowObjs[parseInt(i.toString(), 10)].isExpand &&
  ...
  public lastCaptionRowBorder(): void {
    ...
    if (
      ...
      if (table.scrollHeight < clientHeight || this.isAppliedCaptionRowBorder) {
        if (
          this.isAppliedCaptionRowBorder ||
          table.querySelector('.e-lastrowcell')
        ) {
          const borderCells: NodeListOf<Element> =
            table.querySelectorAll('.e-lastrowcell');
          for (
            let i: number = 0, len: number = borderCells.length;
            i < len;
            i++
          ) {
            removeClass(
              [borderCells[parseInt(i.toString(), 10)]],
              'e-lastrowcell'
            );
          }
          this.isAppliedCaptionRowBorder = false;
        }
        const rowNodes: HTMLCollection = this.parent
          .getContentTable()
          .querySelector(literals.tbody).children;
        const lastRow: Element = rowNodes[rowNodes.length - 1];
        if (
          (lastRow as HTMLTableRowElement).style.display !== 'none' &&
          !lastRow.classList.contains('e-groupcaptionrow')
        ) {
          if (table.scrollHeight < clientHeight) {
            addClass(
              table.querySelectorAll('tr:last-child td'),
              'e-lastrowcell'
            );
          ...
          for (
            let i: number = rowNodes.length - 1, len: number = 0;
            i > len;
            i--
          ) {
            if (
              (rowNodes[parseInt(i.toString(), 10)] as HTMLTableRowElement)
                .style.display !== 'none' &&
              rowNodes[parseInt(i.toString(), 10)].classList.contains(
                'e-groupcaptionrow'
              )
            ) {
              if (
                rowNodes[parseInt(i.toString(), 10)].querySelector(
                  '.e-recordpluscollapse'
                )
              ) {
                addClass(
                  rowNodes[parseInt(i.toString(), 10)].childNodes,
                  'e-lastrowcell'
                );
                this.isAppliedCaptionRowBorder = true;
                break;
  ...
  private updateVirtualRows(
    ...
    isExpand: boolean,
    query: Query,
    dataManager: Promise<Object>
  ): void {
    const rObj: Row<Column> = gObj.getRowObjectFromUID(
      target.closest('tr').getAttribute('data-uid')
    );
    ...
    query.queries = gObj
      .getDataModule()
      .aggregateQuery(gObj.getQuery().clone()).queries;
    ...
    dataManager.then((e: ReturnType) =>
      gObj.renderModule.dataManagerSuccess(e, args)
    );
  }
  ...
  private expandCollapse(isExpand: boolean): void {
    ...
    for (let i: number = 0, len: number = rowNodes.length; i < len; i++) {
      if (
        rowNodes[parseInt(i.toString(), 10)].querySelectorAll(
          '.e-recordplusexpand, .e-recordpluscollapse'
        ).length
      ) {
        row = rowNodes[parseInt(i.toString(), 10)].querySelector(
          isExpand ? '.e-recordpluscollapse' : '.e-recordplusexpand'
        );
        if (row) {
          if (isExpand) {
            ...
            row.firstElementChild.setAttribute(
              'title',
              this.l10n.getConstant('Expanded')
            );
          } else {
        ...
        if (
          !(
            rowNodes[
              parseInt(i.toString(), 10)
            ].firstElementChild.classList.contains('e-recordplusexpand') ||
            rowNodes[
              parseInt(i.toString(), 10)
            ].firstElementChild.classList.contains('e-recordpluscollapse')
          )
        ) {
    ...
    if (this.groupSettings.showDropArea && !this.groupSettings.columns.length) {
      ...
      if (
        (this.element.innerHTML === this.l10n.getConstant('GroupDropArea') &&
          (this.groupSettings.columns.length === 1 ||
  ...
  private initializeGHeaderDrag(): void {
    ...
    const drag: Draggable = new Draggable(this.element as HTMLElement, {
      dragTarget: this.groupSettings.allowReordering
        ? '.e-drag'
        : '.e-groupheadercell',
  ...
  public groupColumn(columnName: string): void {
    ...
    if (
      ...
      column.allowGrouping === false ||
      (this.contentRefresh &&
        this.groupSettings.columns.indexOf(columnName) > -1)
    ) {
      this.parent.log('action_disabled_column', {
        moduleName: this.getModuleName(),
  ...
  public ungroupColumn(columnName: string): void {
    ...
    const column: Column = this.parent.enableColumnVirtualization
      ? (<Column[]>this.parent.columns).filter(
          (c: Column) => c.field === columnName
        )[0]
      : gObj.getColumnByField(columnName);
    if (
      isNullOrUndefined(column) ||
      column.allowGrouping === false ||
      this.groupSettings.columns.indexOf(columnName) < 0
    ) {
      return;
    }
    if (isActionPrevent(gObj)) {
    ...
    const columns: string[] = JSON.parse(
      JSON.stringify(this.groupSettings.columns)
    );
    columns.splice(columns.indexOf(this.colName), 1);
    if (this.sortedColumns.indexOf(columnName) < 0) {
      for (
        let i: number = 0, len: number = gObj.sortSettings.columns.length;
        i < len;
        i++
      ) {
        if (
          columnName ===
          gObj.sortSettings.columns[parseInt(i.toString(), 10)].field
        ) {
  ...
  public updateModel(): void {
    let columns: string[] = JSON.parse(
      JSON.stringify(this.groupSettings.columns)
    );
    columns = this.reorderingColumns.length
      ? JSON.parse(JSON.stringify(this.reorderingColumns))
      : columns;
  ...
  private groupAddSortingQuery(colName: string): void {
    ...
    while (i < this.parent.sortSettings.columns.length) {
      if (
        this.parent.sortSettings.columns[parseInt(i.toString(), 10)].field ===
        colName
      ) {
        break;
      }
    ...
    if (this.parent.sortSettings.columns.length === i) {
      ...
      this.parent.sortSettings.columns[parseInt(i.toString(), 10)].direction =
        'Ascending';
    }
  }
  private createElement(field: string): Element {
    ...
    const animator: Element = this.parent.createElement('div', {
      className: 'e-grid-icon e-group-animator',
    });
    let groupedColumn: Element = this.parent.createElement('div', {
      className: 'e-grid-icon e-groupheadercell',
    });
    const childDiv: Element = this.parent.createElement('div', {
      attrs: { 'ej-mappingname': field },
    });
    ...
    if (this.groupSettings.showToggleButton) {
      childDiv.appendChild(
        this.parent.createElement('span', {
          className:
            'e-togglegroupbutton e-icons e-icon-ungroup e-toggleungroup',
          innerHTML: '&nbsp;',
          attrs: {
            tabindex: '-1',
    ...
    if (headerCell.querySelectorAll('.e-ascending,.e-descending').length) {
      direction = headerCell.querySelector('.e-ascending')
        ? 'Ascending'
        : 'Descending';
    }
    childDiv.appendChild(
      this.parent.createElement('span', {
        ...
        attrs: {
          tabindex: '-1',
          'aria-label':
            this.l10n.getConstant('GroupedSortIcon') + column.headerText,
          role: 'button',
        },
      })
    );
    childDiv.appendChild(
      this.parent.createElement('span', {
        ...
        attrs: {
          ...
          tabindex: '-1',
          'aria-label':
            this.l10n.getConstant('UnGroupIcon') + column.headerText,
          role: 'button',
        },
  ...
  private addColToGroupDrop(field: string): void {
    const groupElem: Element = isComplexField(field)
      ? this.parent.element.querySelector(
          '.e-groupdroparea div[ej-complexname=' +
            getParsedFieldID(getComplexFieldID(field)) +
            ']'
        )
      : this.parent.element.querySelector(
          '.e-groupdroparea div[ej-mappingname=' + getParsedFieldID(field) + ']'
        );
    ...
    if (this.groupSettings.allowReordering) {
      const index: number =
        this.element.getElementsByClassName('e-group-animator').length;
    ...
    if (
      focusModule.active.matrix.matrix[firstContentCellIndex[0]][
        firstContentCellIndex[1]
      ] === 0
    ) {
      firstContentCellIndex = findCellIndex(
        focusModule.active.matrix.matrix,
  ...
  private refreshToggleBtn(isRemove?: boolean): void {
    if (this.groupSettings.showToggleButton) {
      const headers: Element[] = [].slice.call(
        this.parent.getHeaderTable().getElementsByClassName('e-headercelldiv')
      );
      for (let i: number = 0, len: number = headers.length; i < len; i++) {
        if (
          !(
            headers[parseInt(i.toString(), 10)].classList.contains(
              'e-emptycell'
            ) ||
            headers[parseInt(i.toString(), 10)].classList.contains(
              'e-headerchkcelldiv'
          ...
          const column: Column = this.parent.getColumnByUid(
            headers[parseInt(i.toString(), 10)].getAttribute('e-mappinguid')
          );
          if (
            !this.parent.showColumnMenu ||
            (this.parent.showColumnMenu && !column.showColumnMenu)
          ) {
            if (
              headers[parseInt(i.toString(), 10)].getElementsByClassName(
                'e-grptogglebtn'
              ).length
            ) {
              remove(
                headers[parseInt(i.toString(), 10)].querySelectorAll(
                  '.e-grptogglebtn'
                )[0] as Element
              );
            }
            if (!isRemove) {
              headers[parseInt(i.toString(), 10)].appendChild(
                this.parent.createElement('span', {
                  className:
                    'e-grptogglebtn e-icons ' +
                    (this.groupSettings.columns.indexOf(column.field) > -1
                      ? 'e-toggleungroup e-icon-ungroup'
                      : 'e-togglegroup e-icon-group'),
                  attrs: {
                    tabindex: '-1',
                    'aria-label': isNullOrUndefined(this.l10n)
      ...
      switch (prop) {
          ...
          if (this.contentRefresh) {
            if (!this.isAppliedUnGroup) {
              if (!this.isAppliedGroup) {
                ...
                for (
                  let j: number = 0;
                  j < this.parent.sortSettings.columns.length;
                  j++
                ) {
                  if (
                    this.parent.sortSettings.columns[parseInt(j.toString(), 10)]
                      .isFromGroup
                  ) {
                ...
                for (
                  let i: number = 0;
                  i < this.groupSettings.columns.length;
                  i++
                ) {
                  this.colName =
                    this.groupSettings.columns[parseInt(i.toString(), 10)];
                  const col: Column = this.parent.getColumnByField(
                    this.colName
                  );
                  col.visible = this.parent.groupSettings.showGroupedColumn;
                  this.groupAddSortingQuery(this.colName);
                  if (i < this.groupSettings.columns.length - 1) {
                    this.addColToGroupDrop(
                      this.groupSettings.columns[parseInt(i.toString(), 10)]
                    );
                  }
                }
              ...
              args = {
                columnName: this.colName,
                requestType: e.properties[`${prop}`].length
                  ? 'grouping'
                  : 'ungrouping',
            ...
            if (!this.groupSettings.showGroupedColumn) {
              ...
              for (let i: number = 0; i < columns.length; i++) {
                if (
                  e.properties[`${prop}`].indexOf(
                    columns[parseInt(i.toString(), 10)]
                  ) === -1
                ) {
                  this.parent.getColumnByField(
                    columns[parseInt(i.toString(), 10)]
                  ).visible = true;
                }
              }
        ...
        case 'showUngroupButton':
          this.updateButtonVisibility(
            this.groupSettings.showUngroupButton,
            'e-ungroupbutton'
          );
          break;
        case 'showToggleButton':
          this.updateButtonVisibility(
            this.groupSettings.showToggleButton,
            'e-togglegroupbutton '
          );
  ...
  private updateButtonVisibility(isVisible: boolean, className: string): void {
    const gHeader: HTMLElement[] = [].slice.call(
      this.element.getElementsByClassName(className)
    );
    for (let i: number = 0; i < gHeader.length; i++) {
      gHeader[parseInt(i.toString(), 10)].style.display = isVisible
        ? ''
        : 'none';
    }
  }
  ...
  private refreshSortIcons(): void {
    ...
    const fieldNames: string[] = this.parent
      .getColumns()
    ...
    for (let i: number = 0, len: number = cols.length; i < len; i++) {
      ...
      header = gObj.getColumnHeaderByField(
        cols[parseInt(i.toString(), 10)].field
      );
      if (
        !gObj.allowSorting &&
        (this.sortedColumns.indexOf(cols[parseInt(i.toString(), 10)].field) >
          -1 ||
        ...
        this.getGHeaderCell(cols[parseInt(i.toString(), 10)].field) &&
        this.getGHeaderCell(
          cols[parseInt(i.toString(), 10)].field
    ...
    for (let i: number = 0, len: number = gCols.length; i < len; i++) {
      ...
      gObj
        .getColumnHeaderByField(gCols[parseInt(i.toString(), 10)])
        .setAttribute('aria-grouped', 'true');
    }
  }
  ...
  private getGHeaderCell(field: string): Element {
    if (
      this.element &&
      this.element.querySelector('[ej-mappingname="' + field + '"]')
    ) {
      return this.element.querySelector('[ej-mappingname="' + field + '"]')
        .parentElement;
    }
    return null;
  }
  ...
  private onGroupAggregates(editedData: Object[]): void {
    ...
    const summaryRows: Row<Column>[] = this.parent
      .getRowsObject()
      .filter((row: Row<Column>) => !row.isDataRow);
    const updateSummaryRows: Object[] = rowData.filter(
      (data: Row<Column>) => !data.isDataRow
    );
    if (this.parent.isReact || this.parent.isVue) {
      this.parent.destroyTemplate([
        'groupFooterTemplate',
    ...
    for (let i: number = 0; i < updateSummaryRows.length; i++) {
      const row: Row<Column> = updateSummaryRows[
        parseInt(i.toString(), 10)
      ] as Row<Column>;
      const cells: Object[] = row.cells.filter(
        (cell: Cell<{}>) => cell.isDataCell
      );
      const args: Object = {
        cells: cells,
        data: row.data,
        dataUid: summaryRows[parseInt(i.toString(), 10)]
          ? summaryRows[parseInt(i.toString(), 10)].uid
          : '',
      };
  ...
  private updateLazyLoadGroupAggregates(
    ...
    const groupCaptionTemplates: Object[] =
      this.getGroupAggregateTemplates(true);
    const groupFooterTemplates: Object[] =
      this.getGroupAggregateTemplates(false);
    ...
    const groupLazyLoadRenderer: GroupLazyLoadRenderer =
      gObj.contentModule as GroupLazyLoadRenderer;
    const groupCache: { [x: number]: Row<Column>[] } =
      groupLazyLoadRenderer.getGroupCache();
    const currentPageGroupCache: Row<Column>[] =
      groupCache[gObj.pageSettings.currentPage];
    ...
    for (let i: number = 0; i < groupedCols.length; i++) {
      ...
      const groupCaptionRowObject: Row<Column> = this.getGroupCaptionRowObject(
        editedRow,
        groupedCols.length - i
      );
      if (isRemote && result.length) {
        ...
        this.updateLazyLoadGroupAggregatesRow(
          result,
          groupKey,
          groupCaptionRowObject,
          currentPageGroupCache,
          groupCaptionTemplates,
          groupFooterTemplates
        );
      } else {
        const query: Query = gObj.renderModule.data.generateQuery();
        if (i !== 0) {
          const currentLevelCaptionRowObjects: Row<Column>[] =
            currentPageGroupCache.filter((data: Row<Column>) => {
              return (
                data.isCaptionRow &&
                data.parentUid === groupCaptionRowObject.parentUid
              );
            });
          const index: number = currentLevelCaptionRowObjects.indexOf(
            groupCaptionRowObject
          );
          const fields: string[] = gObj.groupSettings.columns
            .slice(0, i)
            .reverse();
          const keys: string[] = fields.map((data: string) => {
            return updatedData[`${data}`];
          });
          const pred: Predicate = generateExpandPredicates(
            fields,
            keys,
        ...
        gObj.renderModule.data
          ...
          .then((e: ReturnType) => {
            ...
            if (i === groupedCols.length - 1 || isRemote) {
              this.destroyRefreshGroupCaptionFooterTemplate();
            }
          })
          .catch((e: ReturnType) =>
            gObj.renderModule.dataManagerFailure(e, { requestType: 'grouping' })
          );
        if (isRemote) {
          break;
        }
  ...
  private updateLazyLoadGroupAggregatesRow(
    result: Object[],
    groupKey: string,
    groupCaptionRowObject: Row<Column>,
    currentPageGroupCache: Row<Column>[],
    groupCaptionTemplates: Object[],
    groupFooterTemplates: Object[]
  ): void {
    ...
    if (groupCaptionTemplates.length) {
      this.updateLazyLoadGroupAggregatesCell(
        updatedGroupCaptionData,
        groupCaptionRowObject,
        groupCaptionTemplates
      );
    }
    if (groupFooterTemplates.length) {
      const groupFooterRowObject: Row<Column> = currentPageGroupCache.find(
        (data: Row<Column>) => {
          return (
            data.isAggregateRow && data.parentUid === groupCaptionRowObject.uid
          );
        }
      );
      this.updateLazyLoadGroupAggregatesCell(
        updatedGroupCaptionData,
        groupFooterRowObject,
        groupFooterTemplates
      );
    }
  }
  ...
  private updateLazyLoadGroupAggregatesCell(
    updatedGroupCaptionData: Object,
    captionFooterRowObject: Row<Column>,
    captionFooterTemplates: Object[]
  ): void {
    const prevCaptionFooterData: Object = captionFooterRowObject.data;
    const updatedGroupCaptionDataAggregates: Object = (<
      { aggregates?: Object }
    >updatedGroupCaptionData).aggregates;
    if (captionFooterRowObject.isCaptionRow) {
      (<{ aggregates?: Object }>prevCaptionFooterData).aggregates =
        updatedGroupCaptionDataAggregates;
    }
    for (let i: number = 0; i < captionFooterTemplates.length; i++) {
      const template: { type: string; field: string } = captionFooterTemplates[
        parseInt(i.toString(), 10)
      ...
      fieldData[capitalizeFirstLetter(template.type)] =
        updatedGroupCaptionDataAggregates[`${key}`];
  ...
  private getGroupCaptionRowObject(
    ...
    for (let i: number = 0; i < groupCaptionIndex; i++) {
      parentCaptionRowObject = gObj.getRowObjectFromUID(
        parentCaptionRowObject.parentUid
      );
    }
  ...
  public getGroupAggregateTemplates(groupCaptionTemplate: boolean): Object[] {
    ...
    for (let j: number = 0; j < aggregateRows.length; j++) {
      const row: AggregateRow = aggregateRows[
        parseInt(j.toString(), 10)
      ] as AggregateRow;
      for (let k: number = 0; k < row.columns.length; k++) {
        if (
          (groupCaptionTemplate &&
            row.columns[parseInt(k.toString(), 10)].groupCaptionTemplate) ||
          (!groupCaptionTemplate &&
            row.columns[parseInt(k.toString(), 10)].groupFooterTemplate)
        ) {
          let aggr: Object = {};
          const type: string | AggregateType[] =
            row.columns[parseInt(k.toString(), 10)].type.toString();
          aggr = {
   ...
   * @hidden
   */
  public groupedRowReorder(
    fromRowObj: Row<Column>,
    toRowObj: Row<Column>
  ): void {
    ...
    if (
      !isNullOrUndefined(fromRowObj) &&
      !isNullOrUndefined(toRowObj) &&
      ...
      if (dropRow) {
        ...
        if (dropArgs.fromIndex > dropArgs.dropIndex) {
          this.parent
            .getContentTable()
            .querySelector(literals.tbody)
            .insertBefore(dragRow, dropRow);
        } else {
          this.parent
            .getContentTable()
            .querySelector(literals.tbody)
            .insertBefore(dragRow, dropRow.nextSibling);
        }
      } else {
        remove(dragRow);
      }
      this.groupReorderHandler(fromRowObj, toRowObj);
      const tr: HTMLTableRowElement[] = [].slice.call(
        this.parent.getContentTable().getElementsByClassName(literals.row)
      );
      ...
      if (this.parent.enableVirtualization) {
        ...
        resetRowIndex(
          ...
          this.parent
            .getRowsObject()
            .filter((data: Row<Column>) => data.isDataRow),
          tr
        );
      }
      this.parent.notify(events.refreshExpandandCollapse, {
  ...
  private groupReorderHandler(
    dragRowObject: Row<Column>,
    dropRowObject: Row<Column>
  ): void {
    const gObj: IGrid = this.parent;
    const dragRowObjectData: Object = dragRowObject.data;
    const dropRowObjectData: Object = dropRowObject.data;
    const groupAggregateTemplate: Object[] =
      gObj['groupModule'].getGroupAggregateTemplates(false);
    const dropParentRowObject: Row<Column> = gObj.getRowObjectFromUID(
      dropRowObject.parentUid
    );
    const dragParentRowObject: Row<Column> = gObj.getRowObjectFromUID(
      dragRowObject.parentUid
    );
    ...
    for (let j: number = 0; j < groupColumns.length; j++) {
      dragRowObjectData[groupColumns[parseInt(j.toString(), 10)]] =
        dropRowObjectData[groupColumns[parseInt(j.toString(), 10)]];
      if (j > 0) {
        dropRootParentRowObjects.push(
          gObj.getRowObjectFromUID(dropRootParentRowObjects[j - 1].parentUid)
        );
        dragRootParentRowObjects.push(
          gObj.getRowObjectFromUID(dragRootParentRowObjects[j - 1].parentUid)
        );
      }
    }
    ...
    let updatedCurrentViewData: Object[] = this.iterateGroupAggregates([
      {
        dragRowObjects: dragRootParentRowObjects,
        dropRowObjects: dropRootParentRowObjects,
      },
    ]);
    const updatedDragCurrentViewData: Object = updatedCurrentViewData.filter(
      (object: Object) =>
        object['key'] ===
          dragRootParentRowObjects[dragRootParentRowObjects.length - 1].data[
            'key'
          ] ||
    ...
    const updatedDropCurrentViewData: Object = updatedCurrentViewData.filter(
      (object: Object) =>
        object['key'] ===
          dropRootParentRowObjects[dropRootParentRowObjects.length - 1].data[
            'key'
          ] ||
        (object['key'] instanceof Date &&
          object['key'].toString() ===
    ...
    for (let i: number = 0; i < currentViewData.length; i++) {
      if (
        isNullOrUndefined(updatedDragCurrentViewData[0]) &&
        ...
        isNullOrUndefined(updatedDropCurrentViewData[0]) &&
        currentViewData[parseInt(i.toString(), 10)]['key'] ===
          dropRootParentRowObjects[dropRootParentRowObjects.length - 1].data[
            'key'
          ]
        ...
        !isNullOrUndefined(updatedDragCurrentViewData[0]) &&
        currentViewData[parseInt(i.toString(), 10)]['key'] ===
          updatedDragCurrentViewData[0]['key']
      ) {
        currentViewData[parseInt(i.toString(), 10)] =
          updatedDragCurrentViewData[0];
      } else if (
        !isNullOrUndefined(updatedDropCurrentViewData[0]) &&
        currentViewData[parseInt(i.toString(), 10)]['key'] ===
          updatedDropCurrentViewData[0]['key']
      ) {
        currentViewData[parseInt(i.toString(), 10)] =
          updatedDropCurrentViewData[0];
      }
    }
    const updatedRowObject: Row<Column>[] = this.groupGenerator.generateRows(
      updatedCurrentViewData,
      {}
    );
    ...
    for (let i: number = 0; i < dragRootParentRowObjects.length; i++) {
      dragRootParentAggregateRowObject.push(
        ...this.getGroupParentFooterAggregateRowObject(
          dragRootParentRowObjects[parseInt(i.toString(), 10)].uid
        )
      );
    }
    for (let i: number = 0; i < dropRootParentRowObjects.length; i++) {
      dropRootParentAggregateRowObject.push(
        ...this.getGroupParentFooterAggregateRowObject(
          dropRootParentRowObjects[parseInt(i.toString(), 10)].uid
        )
      );
    }
    ...
    this.updatedRowObjChange(
      dragRootParentRowObjects,
      updatedRowObject,
      groupAggregateTemplate,
      true
    );
    this.updatedRowObjChange(
      dropRootParentRowObjects,
      updatedRowObject,
      groupAggregateTemplate
    );
  ...
  private updatedRowObjChange(
    rootParentRowObjects: Row<Column>[],
    updatedRowObjects: Row<Column>[],
    groupAggregateTemplate: Object[],
    ...
    for (let i: number = 0; i < rootParentRowObjects.length; i++) {
      ...
      const parentRowObject: Row<Column> =
        rootParentRowObjects[parseInt(i.toString(), 10)];
      for (let j: number = 0; j < updatedRowObjects.length; j++) {
        const updatedRowObject: Row<Column> =
          updatedRowObjects[parseInt(j.toString(), 10)];
        if (
          !isNullOrUndefined(updatedRowObject) &&
          !isNullOrUndefined(parentRowObject.data['key']) &&
          !isNullOrUndefined(updatedRowObject.data['key']) &&
          (parentRowObject.data['key'] === updatedRowObject.data['key'] ||
            (parentRowObject.data['key'] instanceof Date &&
              parentRowObject.data['key'].toString() ===
                updatedRowObject.data['key'].toString()))
        ) {
          let isParentKeyPresent: boolean = true;
          const nextParentObject: Row<Column> =
            rootParentRowObjects[parseInt((i + 1).toString(), 10)];
          if (
            isDraggedRow &&
            nextParentObject &&
            !nextParentObject.isAggregateRow
          ) {
            ...
            const groupedData: Object[] = updatedRowObject.data['items'].records
              ? updatedRowObject.data['items'].records
              : updatedRowObject.data['items'];
            if (
              groupedData &&
              groupedData.length &&
              groupedData[0][`${field}`] &&
          ...
          if (index !== -1) {
            rowObjects[parseInt(index.toString(), 10)].data =
              updatedRowObject.data;
            rowObjects[parseInt(index.toString(), 10)]['gSummary'] =
              updatedRowObject['gSummary'];
          }
          if (gObj.enableVirtualization) {
            const vIndex: number =
              virtualCacheRowObjects.indexOf(parentRowObject);
            if (vIndex !== -1) {
              virtualCacheRowObjects[parseInt(vIndex.toString(), 10)].data =
                updatedRowObject.data;
              virtualCacheRowObjects[parseInt(vIndex.toString(), 10)][
                'gSummary'
              ] = updatedRowObject['gSummary'];
            }
          }
      ...
      if (!keyPresent) {
        const removeElem: Element = gObj.getRowElementByUID(
          parentRowObject.uid
        );
        ...
        if (gObj.enableVirtualization) {
          virtualCacheRowObjects.splice(
            virtualCacheRowObjects.indexOf(parentRowObject),
            1
          );
          for (let k: number = 1; k <= Object.keys(cache).length; k++) {
            const vcIndex: number =
              cache[parseInt(k.toString(), 10)].indexOf(parentRowObject);
            if (vcIndex !== -1) {
              cache[parseInt(k.toString(), 10)].splice(
                [parseInt(vcIndex.toString(), 10)],
                1
              );
            }
          }
  ...
  private groupReorderRefreshHandler(parentRowObjects: Row<Column>[]): void {
    ...
    for (let j: number = 0; j < parentRowObjects.length; j++) {
      const rowObject: Row<Column> =
        parentRowObjects[parseInt(j.toString(), 10)];
      if (
        !isNullOrUndefined(rowObject.uid) &&
  ...
  private evaluateGroupAggregateValueChange(
    rowObjects: Row<Column>,
    updatedRowObject: Row<Column>,
    groupAggregateTemplate: Object
  ): boolean {
    let change: boolean = false;
    if (
      rowObjects.data[groupAggregateTemplate['field']]['field'] ===
        updatedRowObject.data[groupAggregateTemplate['field']]['field'] &&
      rowObjects.data[groupAggregateTemplate['field']]['key'] ===
        updatedRowObject.data[groupAggregateTemplate['field']]['key'] &&
      (rowObjects.data[groupAggregateTemplate['field']] as Object)
        // eslint-disable-next-line no-prototype-builtins
        .hasOwnProperty(
          groupAggregateTemplate['field'] +
            ' - ' +
            groupAggregateTemplate['type']
        ) &&
  ...
  private gettingVirtualData(
    ...
    for (let i: number = 0; i < parentRowObjs.length; i++) {
      if (
        curViewRec.indexOf(parentRowObjs[parseInt(i.toString(), 10)].data) ===
        -1
      ) {
        datas.push(parentRowObjs[parseInt(i.toString(), 10)].data);
      }
      if (
        parentRowObjs[parseInt(i.toString(), 10)].data['field'] ===
        this.parent.groupSettings.columns[0]
      ) {
        let draggedData: Object[] =
          parentRowObjs[parseInt(i.toString(), 10)].data['items'];
        ...
        for (let j: number = 0; j < draggedData.length; j++) {
          if (
            pK &&
            curViewRec.findIndex(
              (data: Object) =>
                data[pK.toString()] ===
                draggedData[parseInt(j.toString(), 10)][pK.toString()]
            ) === -1
          ) {
  ...
  private iterateGroupAggregates(editedData: Object[]): Object[] {
    ...
    const dragParentRowObjects: Row<Column>[] =
      editedData && editedData.length
        ? editedData[0] && (editedData[0]['dragRowObjects'] as Row<Column>[])
        : null;
    const dropParentRowObjects: Row<Column>[] =
      editedData && editedData.length
        ? editedData[0] && (editedData[0]['dropRowObjects'] as Row<Column>[])
        : null;
    ...
    if (
      ...
      this.parent.groupSettings.columns.length &&
      (!isNullOrUndefined(dragParentRowObjects) ||
        !isNullOrUndefined(dropParentRowObjects))
    ) {
      if (dragParentRowObjects) {
        initData.push(
          ...this.gettingVirtualData(dragParentRowObjects, initData, field)
        );
      }
      if (dropParentRowObjects) {
        initData.push(
          ...this.gettingVirtualData(dropParentRowObjects, initData, field)
        );
      }
    }
    const isInfiniteGroup: boolean =
      this.parent.enableInfiniteScrolling &&
      this.parent.allowGrouping &&
      editedData.length &&
      this.parent.groupSettings.columns.length &&
    ...
    if (isInfiniteGroup) {
      initData = [];
      dropRootKey =
        dropParentRowObjects[dropParentRowObjects.length - 1].data['key'];
      dragRootKey =
        dragParentRowObjects[dragParentRowObjects.length - 1].data['key'];
      this.parent.getRowsObject().map((row: Row<Column>) => {
        const groupKey: string | Date =
          row.data[this.parent.groupSettings.columns[0]];
        if (
          row.isDataRow &&
          (groupKey === dropRootKey ||
            groupKey === dragRootKey ||
            (groupKey instanceof Date &&
              (groupKey.toString() === dropRootKey.toString() ||
                groupKey.toString() === dragRootKey.toString())))
        ) {
    ...
    let changeds: Object[] = rows.map(
      (row: Row<AggregateColumn> | Row<Column>) => {
        if (row.edit === 'delete') {
    ...
    if (
      !(
        (<{ type: string }>eData).type &&
        (<{ type: string }>eData).type === 'cancel'
      ) &&
      deletedCols.length > 0
    ) {
      for (let i: number = 0; i < deletedCols.length; i++) {
        const index: number = mergeData.indexOf(
          deletedCols[parseInt(i.toString(), 10)]
        );
    ...
    for (let l: number = 0; l < groupedCols.length; l++) {
      ...
      result = DataUtil.group(
        aggrds,
        groupedCols[parseInt(l.toString(), 10)],
        aggregates,
        null,
        null
      );
    ...
    if (isInfiniteGroup) {
      const lastGroupKey: string | Date =
        this.parent.currentViewData[this.parent.currentViewData.length - 1][
          'key'
        ];
      if (
        (lastGroupKey instanceof Date &&
          (lastGroupKey.toString() === dropRootKey.toString() ||
            lastGroupKey.toString() === dragRootKey.toString())) ||
        lastGroupKey === dropRootKey ||
        lastGroupKey === dragRootKey
      ) {
        ...
        const addWhere: (query: Query) => void = (input: Query) => {
          for (let i: number = 0; i < groups.length; i++) {
            predicate.push(
              new Predicate(
                'field',
                '==',
                groups[parseInt(i.toString(), 10)].field
              ).and(
                this.parent.renderModule.getPredicate(
                  'key',
                  'equal',
                  groups[parseInt(i.toString(), 10)].key
                )
              )
            );
          }
        ...
        addWhere(newQuery);
        const updatedGroupData: Object[] = this.parent
          .getDataModule()
          .dataManager.executeLocal(newQuery);
  ...
  public updateExpand(args: { uid?: string; isExpand?: boolean }): void {
    ...
    const isExpand: boolean = args.isExpand;
      ...
      if (row.uid === uid || isNullOrUndefined(uid)) {
        ...
        for (let j: number = i + 1; j < rows.length; j++) {
          ...
          if (childRow.parentUid !== row.uid) {
            closestParent = rows.filter(
              (x: Row<Column>) => x.uid === childRow.parentUid
            )[0];
          }
          if (childRow.parentUid === row.uid) {
            childRow.visible = row.isExpand;
          } else if (
            !isNullOrUndefined(closestParent) &&
            childRow.parentUid === closestParent.uid
          ) {
            if (closestParent.isExpand && closestParent.visible === true) {
              childRow.visible = true;
            } else if (
              closestParent.isExpand &&
              closestParent.visible === false
            ) {
