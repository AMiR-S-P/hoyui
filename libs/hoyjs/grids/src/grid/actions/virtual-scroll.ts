...
import { extend, getValue, isNullOrUndefined, remove } from '@hoyui/base';
...
export class VirtualScroll implements IAction {
  ...
  private instantiateRenderer(): void {
    ...
    if (this.parent.enableColumnVirtualization) {
      renderer.addRenderer(
        RenderType.Header,
        new VirtualHeaderRenderer(this.parent, this.locator)
      );
    }
    renderer.addRenderer(
      RenderType.Content,
      new VirtualContentRenderer(this.parent, this.locator)
    );
    if (
      !(
  ...
  public ensurePageSize(): void {
    ...
    this.parent.setProperties(
      { pageSettings: { pageSize: size < height ? height : size } },
      true
    );
  }
  ...
  public addEventListener(): void {
    ...
    this.parent.on(
      ...
      this.createVirtualValidationForm,
      this
    );
    this.parent.on(
      events.validateVirtualForm,
      this.virtualEditFormValidation,
      this
    );
  ...
  private createVirtualValidationForm(e: {
    uid: string;
    prevData: object;
    argsCreator: Function;
    renderer: EditRender;
  }): void {
    ...
    if (gObj.enableVirtualization && gObj.editSettings.mode === 'Normal') {
      ...
      const rowObj: Row<Column> = extend(
        {},
        {},
        gObj.getRowObjectFromUID(e.uid),
        true
      ) as Row<Column>;
      gObj.notify(events.refreshVirtualEditFormCells, rowObj);
      const args: CustomEditEventArgs = e.argsCreator(
      ...
      gObj.editModule.virtualFormObj = gObj.editModule.createFormObj(
        args.form,
        rules
      );
    }
  ...
  private virtualEditFormValidation(args: {
    prevData: object;
    isValid: boolean;
    editIdx: number;
    addIdx: number;
    ...
    const gObj: IGrid = this.parent;
    const error: HTMLElement = gObj.element.querySelector(
      '.e-griderror:not([style*="display: none"])'
    );
    if (gObj.editModule.virtualFormObj) {
      if (error && error.style.display !== 'none') {
        const errorDomRect: DOMRect | ClientRect =
          error.getBoundingClientRect();
        const forms: NodeListOf<Element> =
          gObj.element.querySelectorAll('.e-gridform');
        let form: Element = forms[0];
        const contentLeft: number = gObj
          .getContent()
        ...
        if (
          errorDomRect.left < contentLeft ||
          errorDomRect.right > gObj.element.offsetWidth
        ) {
          const tooltip: Element = form.querySelector(
            '.e-tooltip-wrap:not([style*="display: none"])'
          );
          this.scrollToEdit(
            tooltip,
            { editIdx: args.editIdx, addIdx: args.addIdx },
            true
          );
        }
      } else if (
        ...
        const existingErrors: NodeListOf<Element> =
        ...
        this.setEditedDataToValidationForm(
          gObj.editModule.virtualFormObj.element,
          this.getCurrentEditedData(args.prevData)
        );
  ...
  private scrollToEdit(
    tooltip: Element,
    args: { editIdx: number; addIdx: number },
    isRenderer?: boolean
  ): void {
    const gObj: IGrid = this.parent;
    if (tooltip) {
      ...
      if (
        (!isNullOrUndefined(args.addIdx) && scrollTop > 0) ||
        (!isNullOrUndefined(args.editIdx) && !row) ||
        isRenderer ||
        !col
      ) {
  ...
  private setEditedDataToValidationForm(
    ...
    for (let i: number = 0, len: number = inputs.length; i < len; i++) {
      const col: Column = getColumnModelByUid(
        this.parent,
  ...
  private refreshVirtualElement(args: { module: string }): void {
    if (this.parent.enableColumnVirtualization && args.module === 'resize') {
      const renderer: RendererFactory =
        this.locator.getService<RendererFactory>('rendererFactory');
      (
        renderer.getRenderer(RenderType.Content) as VirtualContentRenderer
