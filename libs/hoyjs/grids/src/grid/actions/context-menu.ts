...
import { ContextMenu as Menu, MenuItemModel } from '@hoyui/navigations';
import { OpenCloseMenuEventArgs } from '@hoyui/navigations';
import {
  IGrid,
...
export class ContextMenu implements IAction {
  ...
  private render(): void {
    ...
    this.contextMenu = new Menu({
      ...
      cssClass: this.parent.cssClass
        ? 'e-grid-menu' + ' ' + this.parent.cssClass
        : 'e-grid-menu',
    });
  ...
  private getMenuItems(): ContextMenuItemModel[] {
    ...
    for (const item of this.parent.contextMenuItems) {
      if (
        typeof item === 'string' &&
        this.getDefaultItems().indexOf(item) !== -1
      ) {
    ...
    if (exportItems.length > 0) {
      const exportGroup: ContextMenuItemModel =
        this.buildDefaultItems('export');
  ...
  private getLastPage(): number {
    let totalpage: number = Math.floor(
        ...
        if (this.parent.editModule) {
          if (this.parent.editSettings.mode === 'Batch') {
            if (
              ...
              this.cell &&
              !isNaN(
                parseInt(this.cell.getAttribute(literals.dataColIndex), 10)
              )
            ) {
              this.parent.editModule.editCell(
                parseInt(this.row.getAttribute(literals.dataRowIndex), 10),
  ...
  private contextMenuBeforeOpen(args: ContextMenuOpenEventArgs): void {
    const closestGrid: Element = closest(
    ...
    if (args.event && closestGrid && closestGrid !== this.parent.element) {
      ...
      (args.event &&
        (closest(args.event.target as Element, '.' + menuClass.groupHeader) ||
      ...
      (this.parent.editSettings.showAddNewRow &&
        closest(args.event.target as Element, '.e-addedrow') &&
      ...
      if (parentsUntil(args.event.target as Element, 'e-grid')) {
        this.targetRowdata = this.parent.getRowInfo(
          args.event.target as Element
        );
      }
      if (isNullOrUndefined(args.parentItem) && this.targetColumn) {
        if (this.targetRowdata.cell) {
          ...
          this.selectRow(
            args.event,
            this.targetRowdata.cell.classList.contains(
        ...
        if (this.getDefaultItems().indexOf(key) !== -1) {
          ...
          if (
            ...
            (closest(args.event.target as Element, '.e-gridform') ||
              this.parent.editModule.getBatchChanges()[literals.changedRecords]
                .length ||
          ...
          (item as ContextMenuItemModel).target &&
          args.event &&
          !this.ensureTarget(
            args.event.target as HTMLElement,
            (item as ContextMenuItemModel).target
      ...
      if (
        args.cancel ||
        (this.hiddenItems.length === args.items.length && !args.parentItem)
      ) {
        this.updateItemStatus();
        args.cancel = true;
      }
    ...
    applyBiggerTheme(
      this.parent.element,
      this.contextMenu.element.parentElement
    );
  }
  ...
  private ensureTarget(targetElement: HTMLElement, selector: string): boolean {
    ...
    if (
      this.ensureFrozenHeader(targetElement) &&
      (selector === menuClass.header || selector === menuClass.content)
    ) {
      target = closest(
        targetElement as Element,
        selector === menuClass.header ? 'thead' : literals.tbody
      );
    } else if (
      selector === menuClass.content ||
      selector === menuClass.header
    ) {
      target = parentsUntil(
        closest(targetElement as Element, '.' + literals.table),
  ...
  private ensureFrozenHeader(targetElement: HTMLElement): boolean {
    return this.parent.frozenRows && closest(targetElement, menuClass.header)
      ? true
      : false;
  }
  ...
  private ensureDisabledStatus(item: string): boolean {
    ...
    switch (item) {
      ...
      case 'AutoFit':
        status =
          !(this.parent.ensureModuleInjected(Resize) && !this.parent.isEdit) ||
      ...
      case 'Copy':
        if (
          (this.parent.getSelectedRowIndexes().length === 0 &&
            this.parent.getSelectedRowCellIndexes().length === 0) ||
      ...
      case 'export':
        if (
          !this.parent.allowExcelExport ||
          !this.parent.excelExport ||
          (!this.parent.ensureModuleInjected(PdfExport) &&
            !this.parent.ensureModuleInjected(ExcelExport))
        ) {
          status = true;
        }
        break;
          ...
          for (let i: number = 0; i < sortColumns.length; i++) {
            if (
              ...
              sortColumns[
                ...
                item.toLowerCase().replace('sort', '').toLocaleLowerCase()
            ) {
              status = true;
            }
          }
      ...
      case 'PrevPage':
        if (
          ...
          !this.parent.ensureModuleInjected(Page) ||
          this.parent.getCurrentViewRecords().length === 0 ||
          (this.parent.ensureModuleInjected(Page) &&
  ...
  private getColumn(e: Event): Column {
    const cell: HTMLElement = <HTMLElement>(
      closest(<HTMLElement>e.target, 'th.e-headercell')
    );
    if (cell) {
      ...
      const ele: Column = this.parent.getRowInfo(e.target as Element)
        .column as Column;
      return ele || null;
    }
  }
  ...
  private selectRow(e: Event, isSelectable: boolean): void {
    ...
    this.row =
      ((<HTMLElement>(
        closest(<HTMLElement>e.target, 'tr.e-row')
      )) as HTMLTableRowElement) || this.row;
    if (
      this.row &&
      isSelectable &&
      !parentsUntil(<HTMLElement>e.target, 'e-gridpager')
    ) {
