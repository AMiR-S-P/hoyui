...
import { remove, classList } from '@hoyui/base';
import { FormValidator } from '@hoyui/inputs';
import { isNullOrUndefined, KeyboardEventArgs, isUndefined } from '@hoyui/base';
import {
  IGrid,
  BeforeBatchAddArgs,
...
export class BatchEdit {
  ...
  protected clickHandler(e: MouseEvent): void {
    if (
      !parentsUntil(e.target as Element, this.parent.element.id + '_add', true)
    ) {
      if (
        (this.parent.isEdit &&
          closest(this.form, 'td') !== closest(e.target as Element, 'td') &&
          isNullOrUndefined(this.mouseDownElement)) ||
        this.mouseDownElement === (e.target as Element)
      ) {
        this.saveCell();
      ...
      if (
        parentsUntil(e.target as HTMLElement, literals.rowCell) &&
        !this.parent.isEdit
      ) {
  ...
  protected dblClickHandler(e: MouseEvent): void {
    ...
    const rowIndex: number =
      tr && parseInt(tr.getAttribute(literals.dataRowIndex), 10);
    const colIndex: number =
      target && parseInt(target.getAttribute(literals.dataColIndex), 10);
    if (
      !isNullOrUndefined(target) &&
      !isNullOrUndefined(rowIndex) &&
      !isNaN(colIndex) &&
      !target.parentElement.classList.contains(literals.editedRow) &&
      (this.parent.getColumns()[parseInt(colIndex.toString(), 10)] as Column)
        .allowEditing
    ) {
      this.editCell(
        rowIndex,
  ...
  private onBeforeCellFocused(e: CellFocusArgs): void {
    if (
      ...
      (e.byClick ||
        ['tab', 'shiftTab', 'enter', 'shiftEnter'].indexOf(e.keyArgs.action) >
          -1)
    ) {
      e.cancel = true;
      if (e.byClick) {
  ...
  private onCellFocused(e: CellFocusArgs): void {
    const clear: boolean =
      (!e.container.isContent || !e.container.isDataCell) &&
      !(this.parent.frozenRows && e.container.isHeader);
    if (this.parent.focusModule.active) {
      this.prevEditedBatchCell =
        this.parent.focusModule.active.matrix.current.toString() ===
        this.prevEditedBatchCellMatrix().toString();
      this.crtRowIndex = [].slice
        .call(this.parent.focusModule.active.getTable().rows)
        .indexOf(closest(e.element, 'tr'));
    }
    if (!e.byKey || clear) {
    ...
    const actualIndex: number = e.element.getAttribute('data-colindex')
      ? parseInt(e.element.getAttribute('data-colindex'), 10)
      : cellIndex;
    ...
    if (this.parent.frozenRows && e.container.isContent) {
      rowIndex +=
        (this.parent.getContent().querySelector('.e-hiddenrow')
          ? 0
    ...
    if (!this.parent.element.getElementsByClassName('e-popup-open').length) {
      ...
      switch (e.keyArgs.action) {
        ...
        case 'shiftEnter':
          e.keyArgs.preventDefault();
          // eslint-disable-next-line no-case-declarations
          const args: { cancel: boolean; keyArgs: object } = {
            cancel: false,
            keyArgs: e.keyArgs,
          };
  ...
  private isAddRow(index: number): boolean {
    return this.parent
      .getDataRows()
      [parseInt(index.toString(), 10)].classList.contains('e-insertedrow');
  }
  ...
  private editCellFromIndex(rowIdx: number, cellIdx: number): void {
    ...
    this.editCell(
      rowIdx,
      (this.parent.getColumns() as Column[])[parseInt(cellIdx.toString(), 10)]
        .field,
      this.isAddRow(rowIdx)
    );
  }
  ...
  public closeEdit(): void {
    ...
    for (let i: number = 0; i < rows.length; i++) {
      ...
      isInsert = this.removeBatchElementChanges(
  ...
  public batchSave(): void {
    ...
    if (gObj.isCheckBoxSelection) {
      const checkAllBox: Element =
        gObj.element.querySelector('.e-checkselectall').parentElement;
      if (
        checkAllBox.classList.contains('e-checkbox-disabled') &&
    ...
    const original: Object = {
      changedRecords: this.parent
        ...
        .filter(
          (row: Row<Column>) =>
            row.isDirty && ['add', 'delete'].indexOf(row.edit) === -1
        )
    ...
    gObj.trigger(
      ...
      args,
      (beforeBatchSaveArgs: BeforeBatchSaveArgs) => {
  ...
  private bulkDelete(fieldname?: string, data?: Object): void {
    ...
    if (data) {
      args.row = gObj.editModule.deleteRowUid
        ? gObj.getRowElementByUID(gObj.editModule.deleteRowUid)
    ...
    gObj.trigger(
      ...
      args,
      (beforeBatchDeleteArgs: BeforeBatchDeleteArgs) => {
        ...
        beforeBatchDeleteArgs.row = beforeBatchDeleteArgs.row
          ? (beforeBatchDeleteArgs.row as Element)
          : data
          ? (gObj.getRows()[parseInt(index.toString(), 10)] as Element)
        ...
        if (selectedRows.length === 1 || data) {
          ...
          uid =
            data && this.parent.editModule.deleteRowUid
              ? (uid = this.parent.editModule.deleteRowUid)
              : uid;
          if (beforeBatchDeleteArgs.row.classList.contains('e-insertedrow')) {
            ...
            rowObj.edit = 'delete';
            classList(
              beforeBatchDeleteArgs.row as HTMLTableRowElement,
              ['e-hiddenrow', 'e-updatedtd'],
              []
            );
            if (
              gObj.frozenRows &&
              index < gObj.frozenRows &&
              gObj.getDataRows().length >= gObj.frozenRows
            ) {
              gObj
                .getHeaderTable()
          ...
          if (data) {
            index = parseInt(
              (beforeBatchDeleteArgs.row as Element).getAttribute(
          ...
          for (let i: number = 0; i < selectedRows.length; i++) {
            const uniqueid: string =
              selectedRows[parseInt(i.toString(), 10)].getAttribute('data-uid');
            if (
              selectedRows[parseInt(i.toString(), 10)].classList.contains(
                'e-insertedrow'
              )
            ) {
              ...
              classList(
                selectedRows[parseInt(i.toString(), 10)] as HTMLTableRowElement,
                ['e-hiddenrow', 'e-updatedtd'],
                []
              );
              const selectedRow: Row<Column> =
                gObj.getRowObjectFromUID(uniqueid);
  ...
  private refreshRowIdx(): void {
    ...
    const dataObjects: Row<Column>[] = gObj
      .getRowsObject()
      .filter((row: Row<Column>) => !row.isDetailRow);
    for (
      let i: number = 0, j: number = 0, len: number = rows.length;
      i < len;
      i++
    ) {
      if (
  ...
  private bulkAddRow(data?: Object): void {
    ...
    if (this.initialRender) {
      ...
      for (let i: number = 0; i < visibleColumns.length; i++) {
        if (
          visibleColumns[parseInt(i.toString(), 10)].validationRules &&
          visibleColumns[parseInt(i.toString(), 10)].validationRules['required']
        ) {
          const obj: { field: string; cellIdx: number } = {
    ...
    gObj.trigger(
      ...
      args,
      (beforeBatchAddArgs: BeforeBatchAddArgs) => {
        ...
        const model: IModelGenerator<Column> = new RowModelGenerator(
          this.parent
        );
        const modelData: Row<Column>[] = model.generateRows([
          beforeBatchAddArgs.defaultData,
        ]);
        ...
        for (
          ...
          modelData[0].cells.unshift(
            new Cell<Column>({ cellType: CellType.Indent })
          );
        }
        let tbody: Element = gObj
          .getContentTable()
          .querySelector(literals.tbody);
        ...
        if (tbody.querySelector('.e-emptyrow')) {
          ...
          if (
            gObj.frozenRows &&
            gObj.element.querySelector('.e-frozenrow-empty')
          ) {
            gObj.element
        ...
        addClass([].slice.call(tr.getElementsByClassName(literals.rowCell)), [
          'e-updatedtd',
        ]);
        ...
        if (
          ...
          (data || changes[literals.addedRecords].length)
        ) {
          this.parent.notify(events.refreshFooterRenderer, {});
        }
        const args1: BatchAddArgs = {
          ...
          cell: !isNullOrUndefined(index)
            ? tr.cells[parseInt(index.toString(), 10)]
            : undefined,
        };
  ...
  private findNextEditableCell(
    ...
    for (let i: number = columnIndex; i < endIndex; i++) {
      validation = isValOnly
        ? isNullOrUndefined(cols[parseInt(i.toString(), 10)].validationRules)
        : false;
      ...
      if (
        isAdd &&
        (!cols[parseInt(i.toString(), 10)].template ||
          cols[parseInt(i.toString(), 10)].field) &&
        cols[parseInt(i.toString(), 10)].allowEditing &&
        cols[parseInt(i.toString(), 10)].visible &&
        !(
          cols[parseInt(i.toString(), 10)].isIdentity &&
          cols[parseInt(i.toString(), 10)].isPrimaryKey
        ) &&
        !validation
      ) {
        return i;
  ...
  private getDefaultData(): Object {
    ...
    for (const col of (<{ columnModel?: Column[] }>gObj).columnModel) {
      if (col.field) {
        setValue(
          col.field,
          Object.keys(col).indexOf('defaultValue') >= 0
            ? col.defaultValue
            : dValues[col.type],
          data
        );
  ...
  public editCell(index: number, field: string, isAdd?: boolean): void {
    ...
    const checkEdit: boolean =
      gObj.isEdit &&
      !(
        this.cellDetails.column.field === field &&
        this.cellDetails.rowIndex === index &&
        this.parent.getDataRows().length - 1 !== index &&
        this.prevEditedBatchCell
      );
    if (gObj.editSettings.allowEditing) {
      if (
        ...
        (col.allowEditing ||
          (!col.allowEditing &&
            gObj.focusModule.active &&
            gObj.focusModule.active.getTable().rows[this.crtRowIndex] &&
            gObj.focusModule.active
              .getTable()
              .rows[this.crtRowIndex].classList.contains('e-insertedrow')))
      ) {
  ...
  public editCellExtend(index: number, field: string, isAdd?: boolean): void {
    ...
    if (
      (keys[0] === col.field && !row.classList.contains('e-insertedrow')) ||
      col.columns ||
    ...
    this.parent.element.classList.add('e-editing');
    const rowObj: Row<Column> = gObj.getRowObjectFromUID(
    ...
    gObj.trigger(events.cellEdit, args, (cellEditArgs: CellEditArgs) => {
      ...
      cellEditArgs.cell = cellEditArgs.cell
        ? cellEditArgs.cell
        : cells[this.getColIndex(cells, this.getCellIdx(col.uid))];
      cellEditArgs.row = cellEditArgs.row ? cellEditArgs.row : row;
      cellEditArgs.columnObject = cellEditArgs.columnObject
        ? cellEditArgs.columnObject
        : col;
      ...
      this.cellDetails = {
        ...
        rowIndex: index,
        cellIndex: parseInt(
          (cellEditArgs.cell as HTMLTableCellElement).getAttribute(
  ...
  public updateCell(
    ...
    if (col && !col.isPrimaryKey && col.allowEditing) {
      const td: Element = this.parent.isSpan
        ? getCellFromRow(gObj, rowIndex, index)
        : getCellByColAndRowIndex(this.parent, col, rowIndex, index);
      if (this.parent.isSpan && !td) {
        return;
      }
      const rowObj: Row<Column> = gObj.getRowObjectFromUID(
        td.parentElement.getAttribute('data-uid')
      );
      if (
        gObj.isEdit ||
        (!rowObj.changes &&
          ((!(value instanceof Date) && rowObj.data['' + field] !== value) ||
            (value instanceof Date &&
              new Date(rowObj.data['' + field]).toString() !==
                new Date(value).toString()))) ||
        (rowObj.changes &&
          ((!(value instanceof Date) && rowObj.changes['' + field] !== value) ||
            (value instanceof Date &&
              new Date(rowObj.changes['' + field]).toString() !==
                new Date(value).toString())))
      ) {
        ...
        if (
          ((this.parent.isReact && this.parent.requireTemplateRef) ||
            (isReactChild &&
              this.parent.parentDetails.parentInstObj.requireTemplateRef)) &&
          col.template
        ) {
  ...
  private setChanges(
    ...
    if (rowObj.data[`${field}`] !== value) {
      ...
      if (type === 'date' || type === 'datetime') {
        if (
          new Date(rowObj.data[`${field}`]).toString() !==
          new Date(value as Date).toString()
        ) {
          rowObj.isDirty = true;
        }
      } else {
  ...
  private getCellIdx(uid: string): number {
    let cIdx: number =
      this.parent.getColumnIndexByUid(uid) +
      this.parent.groupSettings.columns.length;
    if (
      !isNullOrUndefined(this.parent.detailTemplate) ||
      !isNullOrUndefined(this.parent.childGrid)
    ) {
      cIdx++;
    }
  ...
  private refreshTD(
    ...
    const cell: CellRenderer = new CellRenderer(
      this.parent,
      this.serviceLocator
    );
    value =
      column.type === 'number' && !isNullOrUndefined(value)
    ...
    if (rowObj) {
      cell.refreshTD(
        td,
        rowcell[this.getCellIdx(column.uid) - index] as Cell<Column>,
        rowObj.changes,
    ...
    if (this.parent.isReact) {
      ...
      parentElement.cells[parseInt(cellIndex.toString(), 10)].classList.add(
        'e-updatedtd'
      );
    } else {
  ...
  private editNextValCell(): void {
    const gObj: IGrid = this.parent;
    const insertedRows: NodeListOf<Element> =
      gObj.element.querySelectorAll('.e-insertedrow');
    const isSingleInsert: boolean = insertedRows.length === 1 ? true : false;
    if (isSingleInsert && this.isAdded && !gObj.isEdit) {
      const btmIdx: number = this.getBottomIndex();
      for (
        let i: number = this.cellDetails.cellIndex;
        i < gObj.getColumns().length;
        i++
      ) {
        if (gObj.isEdit) {
          return;
        }
        const index: number = this.findNextEditableCell(
          this.cellDetails.cellIndex + 1,
          true,
          true
        );
        const col: Column = gObj.getColumns()[
        ...
        if (col) {
          if (this.parent.editSettings.newRowPosition === 'Bottom') {
            ...
            const args: { index: number; column: Column } = {
              index: 0,
              column: col,
            };
        ...
        if (!gObj.isEdit) {
          for (let j: number = 0; j < this.validationColObj.length; j++) {
            if (gObj.isEdit) {
              ...
              insertedRows[parseInt(i.toString(), 10)].querySelectorAll(
                'td:not(.e-hide)'
              )[this.validationColObj[parseInt(j.toString(), 10)].cellIdx]
                .innerHTML === ''
            ) {
              this.editCell(
                parseInt(
                  insertedRows[parseInt(i.toString(), 10)].getAttribute(
                    'data-rowindex'
                  ),
                  10
                ),
                this.validationColObj[parseInt(j.toString(), 10)].field
              );
              if (this.validateFormObj()) {
                this.saveCell();
              }
  ...
  private generateCellArgs(): Object {
    ...
    let editedData: Object = gObj.editModule.getCurrentEditedData(
      this.form,
      obj
    );
  ...
  private successCallBack(
    cellSaveArgs: CellSaveArgs,
    tr: Element,
    column: Column,
    isEscapeCellEdit?: boolean
  ): Function {
    return (cellSaveArgs: CellSaveArgs) => {
      const gObj: IGrid = this.parent;
      cellSaveArgs.cell = cellSaveArgs.cell
        ? cellSaveArgs.cell
        : this.form.parentElement;
      cellSaveArgs.columnObject = cellSaveArgs.columnObject
        ? cellSaveArgs.columnObject
        : column;
      ...
      if (cellSaveArgs.cancel) {
        ...
        if (this.editNext) {
          this.editNext = false;
          if (
            this.cellDetails.rowIndex === this.index &&
            this.cellDetails.column.field === this.field
          ) {
            return;
          }
      ...
      if (
        (!isNullOrUndefined(cellSaveArgs.value) &&
          cellSaveArgs.value.toString() ===
            (!isNullOrUndefined(this.cellDetails.value)
              ? this.cellDetails.value
              : ''
            ).toString() &&
          !this.isColored) ||
        (isNullOrUndefined(cellSaveArgs.value) &&
          isNullOrUndefined(rowObj.data[column.field]) &&
          isNullOrUndefined(this.cellDetails.value) &&
      ...
      if (this.parent.aggregates.length > 0) {
        ...
        if (
          ...
          !this.isAddRow(this.cellDetails.rowIndex)
        ) {
          this.parent.notify(events.groupAggregates, {});
        }
        if (this.parent.isReact || this.parent.isVue) {
  ...
  private prevEditedBatchCellMatrix(): number[] {
    ...
    const gObj: IGrid = this.parent;
    const editedBatchCell: Element = gObj.focusModule.active
      .getTable()
      .querySelector('.e-editedbatchcell');
    if (editedBatchCell) {
      const tr: Element = editedBatchCell.parentElement;
      const rowIndex: number = [].slice
        .call(this.parent.focusModule.active.getTable().rows)
        .indexOf(tr);
      const cellIndex: number = [].slice
        .call((tr as HTMLTableRowElement).cells)
        .indexOf(editedBatchCell);
  ...
  protected getDataByIndex(index: number): Object {
    const row: Row<Column> = this.parent.getRowObjectFromUID(
      this.parent
        .getDataRows()
        [parseInt(index.toString(), 10)].getAttribute('data-uid')
    );
  ...
  private keyDownHandler(e: KeyboardEventArgs): void {
    if (
      this.addBatchRow ||
      ((e.action === 'tab' || e.action === 'shiftTab') && this.parent.isEdit)
    ) {
      const gObj: IGrid = this.parent;
      const btmIdx: number = this.getBottomIndex();
      const rowcell: Element = parentsUntil(
        e.target as Element,
        literals.rowCell
      );
      if (this.addBatchRow || (rowcell && !this.parent.isReact)) {
        ...
        if (this.addBatchRow || cell) {
          ...
          const columnIndex: number =
            e.action === 'tab' ? visibleColumns.length - 1 : 0;
          if (
            this.addBatchRow ||
            visibleColumns[parseInt(columnIndex.toString(), 10)].field ===
              cell.getAttribute('id').slice(this.parent.element.id.length)
          ) {
  ...
  private getBottomIndex(): number {
    ...
    return (
      ...
      changes[literals.addedRecords].length -
      changes[literals.deletedRecords].length -
      1
    );
  }
}
