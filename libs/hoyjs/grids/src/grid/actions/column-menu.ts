...
export class ColumnMenu implements IAction {
  ...
  private setFullScreenDialog(): void {
    if (this.serviceLocator) {
      this.serviceLocator.registerAdaptiveService(
        ...
        ResponsiveDialogAction.isColMenu
      );
    }
  }
      ...
      if (this.parent.enableAdaptiveUI) {
        ...
        if (
          (this.isOpen &&
            this.headerCell !==
              this.getHeaderCell(
                e as { target: EventTarget; preventDefault?: Function }
              )) ||
  ...
  private openColumnMenu(e: {
    ...
    if (this.parent.enableRtl) {
      pos = this.parent.enableStickyHeader
        ? calculatePosition(this.headerCell, 'left', 'bottom', true)
        : calculatePosition(this.headerCell, 'left', 'bottom');
    } else {
      pos = this.parent.enableStickyHeader
        ? calculatePosition(this.headerCell, 'right', 'bottom', true)
        : calculatePosition(this.headerCell, 'right', 'bottom');
      pos.left -= elePos.width;
      if (headerEle.classList.contains('e-sticky')) {
        pos.top =
          this.parent.element.offsetTop +
          headerElemCliRect.top +
          headerElemCliRect.height;
        if (
          headerElemCliRect.top + headerElemCliRect.height >
          contentRect.top
        ) {
          pos.top +=
            headerElemCliRect.top + headerElemCliRect.height - contentRect.top;
        }
    ...
    applyBiggerTheme(
      this.parent.element,
      this.columnMenu.element.parentElement
    );
  }

  private columnMenuHandlerDown(): void {
    this.isOpen = !(
      this.element.style.display === 'none' || this.element.style.display === ''
    );
  }

  private getColumnMenuHandlers(): HTMLElement[] {
    return [].slice.call(
      this.parent.getHeaderTable().getElementsByClassName(this.ROOT)
    );
  }
  ...
  public addEventListener(): void {
    ...
    this.parent.on(
      events.afterFilterColumnMenuClose,
      this.afterFilterColumnMenuClose,
      this
    );
  ...
  private render(): void {
    ...
    this.element.setAttribute(
      'aria-label',
      this.l10n.getConstant('ColumnMenuDialogARIA')
    );
    this.parent.element.appendChild(this.element);
    this.columnMenu = new Menu({
      cssClass: this.parent.cssClass
        ? 'e-grid-menu e-grid-column-menu' + ' ' + this.parent.cssClass
        : 'e-grid-menu e-grid-column-menu',
  ...
  private beforeMenuItemRender(args: MenuEventArgs): void {
    if (this.isChooserItem(args.item)) {
      ...
      const column: Column[] = (<{ columnModel?: Column[] }>(
        this.parent
      )).columnModel.filter((col: Column) => col.field === field);
  ...
  private columnMenuBeforeClose(args: ColumnMenuOpenEventArgs): void {
    const colChooser: Element = args.event
      ? closest(args.event.target as Node, '.e-menu-item')
      : null;
    if (
      !isNullOrUndefined(args.parentItem) &&
      this.getKeyFromId(args.parentItem.id) === 'ColumnChooser' &&
      colChooser &&
      this.isChooserItem(colChooser)
    ) {
      args.cancel =
        args.event && (args.event as KeyboardEventArgs).code === 'Escape'
          ? false
          : true;
    } else if (
      args.event &&
      (closest(args.event.target as Element, '.' + this.POP) ||
        (args.event.currentTarget &&
          (args.event.currentTarget as Document).activeElement &&
          parentsUntil(
            (args.event.currentTarget as Document).activeElement as Element,
            'e-filter-popup'
          )) ||
        (parentsUntil(args.event.target as Element, 'e-popup') &&
          parentsUntil(args.event.target as Element, 'e-colmenu-popup')) ||
      ...
      args.event &&
      args.event.target &&
      (args.event.target as Element).classList.contains('e-filter-item') &&
  ...
  private columnMenuBeforeOpen(args: ColumnMenuOpenEventArgs): void {
    ...
    for (const item of args.items) {
      ...
      if (
        this.getDefaultItems().indexOf(key) !== -1 &&
        this.ensureDisabledStatus(key) &&
        !dItem.hide
      ) {
  ...
  private columnMenuOnOpen(args: OpenCloseMenuEventArgs): void {
    if (args.element.className === 'e-menu-parent e-ul ') {
      if (
        args.element.offsetHeight > window.innerHeight ||
        this.parent.element.offsetHeight > window.innerHeight
      ) {
    ...
    switch (item) {
      case 'Group':
        if (
          ...
          (this.parent.ensureModuleInjected(Group) &&
            this.targetColumn &&
            this.parent.groupSettings.columns.indexOf(
              this.targetColumn.field
            ) >= 0) ||
      ...
      case 'Ungroup':
        if (
          ...
          (this.parent.ensureModuleInjected(Group) &&
            this.targetColumn &&
            this.parent.groupSettings.columns.indexOf(this.targetColumn.field) <
              0)
        ) {
          status = true;
        }
      ...
      case 'Filter':
        if (
          ...
          this.targetColumn &&
          this.targetColumn.allowFiltering
        ) {
          status = false;
        } else if (
          this.parent.ensureModuleInjected(Filter) &&
  ...
  private columnMenuItemClick(args: ColumnMenuClickEventArgs): void {
    const item: string = this.isChooserItem(args.item)
      ? 'ColumnChooser'
      : this.getKeyFromId(args.item.id);
    switch (item) {
      case 'AutoFit':
      ...
      case 'ColumnChooser':
        ...
        // eslint-disable-next-line no-case-declarations
        const checkbox: HTMLElement = args.element.querySelector(
  ...
  private columnMenuOnClose(args: OpenCloseMenuEventArgs): void {
    ...
    if (
      ...
      (
        this.columnMenu.element.querySelector('.e-selected') as HTMLElement
      ).focus();
    } else {
  ...
  private getItems(): ColumnMenuItemModel[] {
    ...
    const defultItems: string[] | ColumnMenuItemModel[] = this.parent
      .columnMenuItems
      ? this.parent.columnMenuItems
      : this.getDefault();
  ...
  private getColumn(): Column {
    if (this.headerCell) {
      const uid: string = this.headerCell
  ...
  private appendFilter(e: Event): void {
    ...
    if (!this.defaultItems[`${filter}`]) {
      ...
      if (
        closest((e as Event).target as Element, '#' + key) &&
  ...
  private getFilter(target: Element, id: string, isClose?: boolean): void {
    ...
    if (filterPopup) {
      filterPopup.style.display =
        !Browser.isDevice && isClose ? 'none' : 'block';
    } else {
      this.parent.notify(events.filterOpen, {
  ...
  private filterPosition(): void {
    ...
    if (!Browser.isDevice) {
      ...
      const li: HTMLElement = this.element.querySelector(
        '.' + this.FILTER
      ) as HTMLElement;
      if (li) {
  ...
  private getFilterPop(): HTMLElement {
    if (
      !isNullOrUndefined(this.targetColumn) &&
  ...
  private isFilterItemAdded(): boolean {
    return (
      (this.parent.columnMenuItems &&
        (this.parent.columnMenuItems as string[]).indexOf('Filter') >= 0) ||
      !this.parent.columnMenuItems
    );
  }
