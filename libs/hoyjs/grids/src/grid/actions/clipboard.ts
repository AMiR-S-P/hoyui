...
export class Clipboard implements IAction {
  ...
  private pasteHandler(e: KeyboardEvent): void {
    ...
    const selectedRowCellIndexes: ISelectedCell[] =
      this.parent.getSelectedRowCellIndexes();
    if (!grid.isEdit && e.keyCode === 67 && isMacLike && e.metaKey) {
      this.copy();
    }
    if (
      selectedRowCellIndexes.length &&
      e.keyCode === 86 &&
      ((!isMacLike && e.ctrlKey) || (isMacLike && e.metaKey)) &&
      !grid.isEdit
    ) {
      ...
      if (
        ...
        !target ||
        !grid.editSettings.allowEditing ||
        grid.editSettings.mode !== 'Batch' ||
        grid.selectionSettings.mode !== 'Cell' ||
        grid.selectionSettings.cellSelectionMode === 'Flow'
      ) {
        return;
      }
  ...
  public paste(data: string, rowIndex: number, colIndex: number): void {
    ...
    for (let r: number = 0; r < rows.length; r++) {
      ...
      cIdx = colIndex;
      if (
        (r === rows.length - 1 && rows[parseInt(r.toString(), 10)] === '') ||
        isUndefined(grid.getRowByIndex(rIdx))
      ) {
        cIdx++;
        break;
      }
      for (let c: number = 0; c < cols.length; c++) {
        ...
        value = col.getParser()
          ? col.getParser()(cols[parseInt(c.toString(), 10)])
          : cols[parseInt(c.toString(), 10)];
            ...
            if (grid.editModule) {
              if (col.type === 'number') {
                this.parent.editModule.updateCell(
                  rIdx,
                  col.field,
                  parseFloat(args.data as string)
                );
              } else {
    ...
    grid.selectionModule.selectCellsByRange(
      { rowIndex: rowIndex, cellIndex: colIndex },
      { rowIndex: rIdx - 1, cellIndex: cIdx - 1 }
    );
  ...
  protected setCopyData(withHeader?: boolean): void {
    if (window.getSelection().toString() === '') {
      ...
      if (this.parent.selectionSettings.mode !== 'Cell') {
        const selectedIndexes: Object[] = this.parent
        ...
        if (withHeader) {
          ...
          for (
            let i: number = 0;
            i < this.parent.getVisibleColumns().length;
            i++
          ) {
            headerTextArray[parseInt(i.toString(), 10)] =
              this.parent.getVisibleColumns()[
        ...
        for (let i: number = 0; i < selectedIndexes.length; i++) {
          ...
          let idx: number = selectedIndexes[
            parseInt(i.toString(), 10)
          ] as number;
          if (
            ...
            idx = rows
              .map((m: Element) => m.getAttribute('data-rowindex'))
              .indexOf(selectedIndexes[parseInt(i.toString(), 10)].toString());
          }
          leftCols.push(
            ...[].slice.call(
              rows[parseInt(idx.toString(), 10)].querySelectorAll(
        ...
        if (obj.status) {
          if (withHeader) {
            ...
            for (let i: number = 0; i < obj.colIndexes.length; i++) {
              const colHeader: HTMLElement = this.parent.getColumnHeaderByIndex(
                obj.colIndexes[parseInt(i.toString(), 10)]
              ) as HTMLElement;
          ...
          for (let i: number = 0; i < obj.rowIndexes.length; i++) {
            ...
            const cells: HTMLElement[] = [].slice.call(
              rows[
                obj.rowIndexes[parseInt(i.toString(), 10)] as number
  ...
  private getCopyData(
    ...
    for (let j: number = 0; j < cells.length; j++) {
      if (withHeader && isCell) {
        const colIdx: number = parseInt(
          (cells[parseInt(j.toString(), 10)] as HTMLElement).getAttribute(
        ...
        this.copyContent +=
          (this.parent.getColumns() as Column[])[
            parseInt(colIdx.toString(), 10)
          ].headerText + '\n';
      }
      if (isElement) {
        if (
          !(
            cells[parseInt(j.toString(), 10)] as HTMLElement
  ...
  private checkBoxSelection(): {
    ...
    let obj: { status: boolean; rowIndexes?: number[]; colIndexes?: number[] } =
      { status: false };
    if (gridObj.selectionSettings.mode === 'Cell') {
      const rowCellIndxes: ISelectedCell[] =
        gridObj.getSelectedRowCellIndexes();
      let str: string;
      ...
      for (i = 0; i < rowCellIndxes.length; i++) {
        ...
        if (rowCellIndxes[parseInt(i.toString(), 10)].cellIndexes.length) {
          if (!str) {
            str = JSON.stringify(
              rowCellIndxes[parseInt(i.toString(), 10)].cellIndexes.sort()
            );
          }
          if (
            str !==
      ...
      rowIndexes.sort((a: number, b: number) => {
        return a - b;
      });
