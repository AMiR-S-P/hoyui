...
import { isNullOrUndefined, extend, setValue } from '@hoyui/base';
 ...
 * @hidden
 */
export class GroupModelGenerator
  extends RowModelGenerator
  implements IModelGenerator<Column>
{
  ...
  public generateRows(
    data: { length: number },
    args?: { startIndex?: number; requestType?: Action }
  ): Row<Column>[] {
    ...
    this.rows = [];
    this.index =
      this.parent.enableVirtualization || this.isInfiniteScroll
        ? args.startIndex
        : 0;
  ...
  private getGroupedRecords(
    ...
    if (isNullOrUndefined(data.items)) {
      if (isNullOrUndefined(data.GroupGuid)) {
        ...
        for (
          let j: number = 0, len: number = (data as Object[]).length;
          j < len;
          j++
        ) {
          this.getGroupedRecords(
            index,
            data[parseInt(j.toString(), 10)],
            data.level,
            parentid,
            index,
            this.rows.length,
      ...
      const captionRow: Row<Column> = this.generateCaptionRow(
        data,
        index,
        parentid,
        childId,
      ...
      if (this.isInfiniteScroll) {
        preCaption = this.getPreCaption(
          index,
          (<{ key?: string | Date }>captionRow.data).key
        );
      }
      if (!preCaption) {
      ...
      if (this.parent.aggregates.length && this.isRenderAggregate(captionRow)) {
        ...
        this.rows.push(
          ...(<Row<Column>[]>(
            this.summaryModelGen.generateRows(<Object>data, {
              level: level,
        ...
        for (let i: number = rowCnt - 1; i >= 0; i--) {
          if (this.rows[parseInt(i.toString(), 10)].isCaptionRow) {
            this.rows[parseInt(i.toString(), 10)].aggregatesCount =
              this.rows.length - rowCnt;
          } else if (
            !this.rows[parseInt(i.toString(), 10)].isCaptionRow &&
            !this.rows[parseInt(i.toString(), 10)].isDataRow
          ) {
            break;
          }
        }
  ...
  private isRenderAggregate(data: Row<Column>): boolean {
    if (this.parent.enableInfiniteScrolling) {
      ...
      this.renderInfiniteAgg =
        (data.data as GroupedData).count === this.infiniteChildCount;
  ...
  private getPreCaption(indent: number, key: string | Date): Row<Column> {
    ...
    let preCap: Row<Column>;
    this.infiniteChildCount = 0;
    let i: number = rowObj.length;
    while (i--) {
      if (
        rowObj[parseInt(i.toString(), 10)].isCaptionRow &&
        rowObj[parseInt(i.toString(), 10)].indent === indent
      ) {
        const groupKey: string | Date = (<{ key?: string | Date }>(
          rowObj[parseInt(i.toString(), 10)].data
        )).key;
        if (
          (groupKey.toString() === key.toString() &&
  ...
  private getCaptionRowCells(
    ...
    let visibles: Cell<Column>[] = [];
    let column: Column = this.parent.getColumnByField(field);
    const indexes: number[] = this.parent.getColumnIndexesInView();
    if (this.parent.enableColumnVirtualization) {
      column = (<Column[]>this.parent.columns).filter(
        (c: Column) => c.field === field
      )[0];
    }
    const groupedLen: number = this.parent.groupSettings.columns.length;
    const gObj: IGrid = this.parent;
    if (
      !this.parent.enableColumnVirtualization ||
      indexes.indexOf(indent) !== -1
    ) {
    ...
    indent = this.parent.enableColumnVirtualization
      ? 1
      : this.parent.getVisibleColumns().length +
        groupedLen +
        (gObj.detailTemplate || gObj.childGrid ? 1 : 0) -
        indent +
        (this.parent.getVisibleColumns().length ? -1 : 0);
    //Captionsummary cells will be added here.
    if (this.parent.aggregates.length && !this.captionModelGen.isEmpty()) {
      const captionCells: Row<Column> = <Row<Column>>(
        this.captionModelGen.generateRows(data)[0]
      );
      extend(data, captionCells.data);
      let cIndex: number = 0;
      captionCells.cells.some((cell: Cell<Column>, index: number) => {
        cIndex = index;
        return cell.visible && cell.isDataCell;
      });
      visibles = captionCells.cells
        .slice(cIndex)
        .filter((cell: Cell<Column>) => cell.visible);
      if (
        captionCells.visible &&
        visibles[0].column.field === this.parent.getVisibleColumns()[0].field
      ) {
    ...
    for (let j: number = 0; j < cols.length; j++) {
      ...
      const cellType: CellType =
        !this.parent.enableColumnVirtualization || tmpFlag
          ? CellType.GroupCaption
          : CellType.GroupCaptionEmpty;
      indent =
        this.parent.enableColumnVirtualization &&
        cellType === CellType.GroupCaption
          ? indent + groupedLen
          : indent;
  ...
  public generateCaptionRow(
    ...
    options.isExpand =
      ...
      !this.parent.isCollapseStateEnabled();
    options.parentGid = parentID;
    options.childGid = childID;
    options.tIndex = tIndex;
    options.isCaptionRow = true;
    options.parentUid = parentUid;
    options.gSummary = !isNullOrUndefined(data.items[`${records}`])
      ? data.items[`${records}`].length
      : (<Object[]>data.items).length;
    ...
    if (col && col.isForeignColumn && col.isForeignColumn()) {
      const fkValue: string | Date = <string | Date>(
        (isNullOrUndefined(data.key)
          ? ''
          : (col.valueAccessor as Function)(
              col.foreignKeyValue,
              getForeignData(col, {}, <string>data.key)[0],
              col
            ))
      );
  ...
  public generateDataRows(
    ...
    for (let i: number = 0, len: number = data.length; i < len; i++, tIndex++) {
      rows[parseInt(i.toString(), 10)] = this.generateRow(
        data[parseInt(i.toString(), 10)],
        this.index,
        i ? undefined : 'e-firstchildrow',
        indent,
        childID,
        tIndex,
        parentUid
      ...
      for (let j: number = 0; j < indent; j++) {
        ...
        rows[parseInt(i.toString(), 10)].cells.unshift(
          this.generateIndentCell()
        );
      }
      this.index++;
    }
  ...
  public refreshRows(input?: Row<Column>[]): Row<Column>[] {
    ...
    for (let i: number = 0; i < input.length; i++) {
      if (input[parseInt(i.toString(), 10)].isDataRow) {
        input[parseInt(i.toString(), 10)].cells = this.generateCells(
          input[parseInt(i.toString(), 10)]
        );
        for (
          let j: number = 0;
          j < input[parseInt(i.toString(), 10)].indent;
          j++
        ) {
          if (
            this.parent.enableColumnVirtualization &&
          ...
          input[parseInt(i.toString(), 10)].cells.unshift(
            this.generateIndentCell()
          );
        }
      } else {
        const cRow: Row<Column> = this.generateCaptionRow(
          input[parseInt(i.toString(), 10)].data,
          input[parseInt(i.toString(), 10)].indent
        );
