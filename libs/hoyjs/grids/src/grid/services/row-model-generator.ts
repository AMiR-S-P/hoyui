...
export class RowModelGenerator implements IModelGenerator<Column> {
  ...
  public generateRows(
    ...
    let startIndex: number =
      this.parent.enableVirtualization && args ? args.startIndex : 0;
    startIndex =
      this.parent.enableInfiniteScrolling && args
        ? this.getInfiniteIndex(args)
        : startIndex;
    if (this.parent.enableImmutableMode && args && args.startIndex) {
      startIndex = args.startIndex;
    }
    for (
      let i: number = 0, len: number = Object.keys(data).length;
      i < len;
      i++, startIndex++
    ) {
  ...
  protected generateRow(
    ...
    if (this.parent.isPrinting) {
      if (this.parent.hierarchyPrintMode === 'All') {
        ...
        this.parent.hierarchyPrintMode === 'Expanded' &&
        this.parent.expandedRows &&
        this.parent.expandedRows[parseInt(index.toString(), 10)]
      ) {
        options.isExpand =
          this.parent.expandedRows[parseInt(index.toString(), 10)].isExpand;
      }
    }
    ...
    const row: Row<Column> = new Row<Column>(
      <{ [x: string]: Object }>options,
      this.parent
    );
    row.cells =
      this.parent.getFrozenMode() === 'Right'
        ? this.generateCells(options).concat(cells)
  ...
  protected refreshForeignKeyRow(options: IRow<Column>): void {
    ...
    for (let i: number = 0; i < foreignKeyColumns.length; i++) {
      setValue(
        ...
        getForeignData(
          foreignKeyColumns[parseInt(i.toString(), 10)],
          options.data
        ),
        options.foreignKeyData
      );
  ...
  public generateCell(
    ...
    const opt: ICell<Column> = {
      ...
      foreignKeyData:
        ...
        column.isForeignColumn() &&
        getValue(column.field, foreignKeyData),
    };
  ...
  private getInfiniteIndex(args: InfiniteScrollArgs): number {
    return args.requestType === 'infiniteScroll' ||
      ...
      ? isNullOrUndefined(args.startIndex)
        ? args['index']
        : args.startIndex
      : 0;
  }
