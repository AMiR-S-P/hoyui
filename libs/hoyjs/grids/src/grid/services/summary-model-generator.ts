...
export class SummaryModelGenerator
  implements IModelGenerator<AggregateColumnModel>
{
  ...
  public getData(): Object {
    ...
    for (let i: number = 0; i < row.length; i++) {
      const columns: AggregateColumnModel[] = row[
        parseInt(i.toString(), 10)
      ].columns.filter((column: AggregateColumnModel) => {
        return (
          !(
            column.footerTemplate ||
            column.groupFooterTemplate ||
            column.groupCaptionTemplate
  ...
  public generateRows(
    ...
    if ((input as Object[]).length === 0) {
      if (
        args === undefined ||
        !(
          (args as ReturnType).count ||
          (args as { loadSummaryOnEmpty: boolean }).loadSummaryOnEmpty
        )
      ) {
        return [];
      }
    ...
    for (let i: number = 0; i < row.length; i++) {
      rows.push(
        this.getGeneratedRow(
          ...
          args ? (<SummaryData>args).level : undefined,
          start,
          end,
  ...
  public getGeneratedRow(
    ...
    const isDetailGridAlone: boolean = !isNullOrUndefined(
    ...
    for (let i: number = 0; i < values.length; i++) {
      tmp.push(
        this.getGeneratedCell(
          ...
          i >= indentLength
            ? this.getCellType()
            : i === 0 && this.parent.childGrid
            ? CellType.DetailFooterIntent
            : CellType.Indent,
          indents[parseInt(i.toString(), 10)],
          isDetailGridAlone
        )
      );
    }

    const row: Row<AggregateColumnModel> = new Row<AggregateColumnModel>({
      data: data,
      attributes: { class: 'e-summaryrow' },
    });
    row.cells = tmp;
    row.uid = getUid('grid-row');
    row.parentUid = parentUid;
    row.isAggregateRow = true;
    row.visible = tmp.some(
      (cell: Cell<AggregateColumnModel>) => cell.isDataCell && cell.visible
    );
    return row;
  }
  ...
  public getGeneratedCell(
    ...
    const opt: { [o: string]: Object } = {
      ...
      isTemplate:
        sColumn &&
        !isNullOrUndefined(
          sColumn.footerTemplate ||
            sColumn.groupFooterTemplate ||
            sColumn.groupCaptionTemplate
        ),
      column: sColumn || {},
      attributes: attrs,
      cellType: cellType,
  ...
  private buildSummaryData(
    ...
    const dummy: Object[] = [];
    const summaryRows: AggregateRowModel[] = <AggregateRowModel[]>(
      this.getData()
    );
    let single: Object = {};
    for (let i: number = 0; i < summaryRows.length; i++) {
      single = {};
      const column: AggregateColumn[] = summaryRows[parseInt(i.toString(), 10)]
        .columns as AggregateColumn[];
      for (let j: number = 0; j < column.length; j++) {
        single = this.setTemplate(
          column[parseInt(j.toString(), 10)] as AggregateColumn,
          args && args.aggregates ? <Object[]>args : <Object[]>data,
          single
        );
      }
      dummy.push(single);
  ...
  protected setTemplate(
    ...
    for (let i: number = 0; i < types.length; i++) {
      const key: string =
        column.field + ' - ' + types[parseInt(i.toString(), 10)].toLowerCase();
      const disp: string = column.columnName;
      const disablePageWiseAggregatesGroup: boolean =
        ...
        group.items
          ...
          : false;
      const val: Object =
        (types[parseInt(i.toString(), 10)] !== 'Custom' ||
          disablePageWiseAggregatesGroup) &&
        group.aggregates &&
        key in group.aggregates
          ? group.aggregates[`${key}`]
          : calculateAggregate(
              types[parseInt(i.toString(), 10)],
              group.aggregates ? group : <Object[]>data,
              column,
              this.parent
            );
      single[`${disp}`] = single[`${disp}`] || {};
      single[`${disp}`][`${key}`] = val;
      single[`${disp}`][types[parseInt(i.toString(), 10)]] = !isNullOrUndefined(
        val
      )
        ? formatFn(val)
        : ' ';
      if (group.field) {
        (<Group>single[`${disp}`]).field = group.field;
        (<Group>single[`${disp}`]).key = group.key;
      }
    }
...
export class GroupSummaryModelGenerator
  extends SummaryModelGenerator
  implements IModelGenerator<AggregateColumnModel>
{
  public columnSelector(column: AggregateColumnModel): boolean {
  ...
  protected getIndentByLevel(
    level: number = this.parent.groupSettings.columns.length
  ): string[] {
    if (
      this.parent.allowRowDragAndDrop &&
      this.parent.groupSettings.columns.length
    ...
    return this.parent.groupSettings.columns.map((v: string, indx: number) =>
      indx <= level - 1 ? '' : 'e-indentcelltop'
    );
  }
...
export class CaptionSummaryModelGenerator
  extends SummaryModelGenerator
  implements IModelGenerator<AggregateColumnModel>
{
  public columnSelector(column: AggregateColumnModel): boolean {
  ...
  public getData(): Object {
    ...
    return [
      (<AggregateRowModel[]>super.getData()).reduce(
