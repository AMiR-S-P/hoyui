...
import { debounce } from '@hoyui/base';
...
export class InterSectionObserver {
  ...
  public sentinelInfo: SentinelInfo = {
    up: {
      check: (rect: ClientRect, info: SentinelType) => {
        ...
        const bottom: number =
          this.containerRect.bottom > rect.bottom
            ? this.containerRect.bottom - rect.bottom
            : 0;
        info.entered = top >= 0;
        return (
          top + this.options.pageHeight / 2 >= 0 ||
          (bottom > 0 && rect.bottom > 0)
        );
      },
      axis: 'Y',
    },
    down: {
      check: (rect: ClientRect, info: SentinelType) => {
        ...
        return (
          bottom - this.containerRect.top - this.options.pageHeight / 2 <=
  ...
  public observe(callback: Function, onEnterCallback: Function): void {
    ...
    EventHandler.add(
      this.options.container,
      'scroll',
      this.virtualScrollHandler(callback, onEnterCallback),
      this
    );
  }
  ...
  private virtualScrollHandler(
    ...
    return (e: Event) => {
      ...
      let direction: ScrollDirection =
        this.options.prevTop < top ? 'down' : 'up';
      direction =
