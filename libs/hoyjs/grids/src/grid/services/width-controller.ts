import { isNullOrUndefined, addClass } from '@hoyui/base';
import { IGrid, NotifyArgs } from '../base/interface';
import { formatUnit } from '@hoyui/base';
import {
  columnWidthChanged,
...
export class ColumnWidthService {
  ...
  public setWidthToColumns(): void {
    ...
    if (this.parent.allowGrouping) {
      for (
        let len: number = this.parent.groupSettings.columns.length;
        i < len;
        i++
      ) {
        if (
    ...
    for (let j: number = 0; j < columns.length; j++) {
      this.setColumnWidth(
        ...
        wFlag && this.parent.enableColumnVirtualization ? undefined : j + i
      );
    }
    if (
      this.parent.isRowDragable() &&
    ...
    if (totalColumnsWidth !== 'auto') {
      ...
      if (
        ...
        this.parent.element.getBoundingClientRect().width > totalColumnsWidth &&
        maxWidthColumns.length === columns.length
      ) {
        addClass(
          [this.parent.getHeaderTable(), this.parent.getContentTable()],
          ['e-tableborder']
        );
      }
    }
  ...
  public setMinwidthBycalculation(tWidth?: number): void {
    ...
    const collection: Column[] = this.parent
      .getColumns()
      .filter((a: Column) => {
        if (this.parent.allowResizing) {
          return (
            (isNullOrUndefined(a.width) || a.width === 'auto') &&
            isNullOrUndefined(a.maxWidth)
          );
        } else {
    ...
    if (collection.length) {
      if (
        !isNullOrUndefined(this.parent.width) &&
        this.parent.width !== 'auto' &&
        typeof this.parent.width === 'string' &&
        this.parent.width.indexOf('%') === -1
      ) {
      ...
      for (let i: number = 0; i < collection.length; i++) {
        ...
        const index: number =
          this.parent.getColumnIndexByField(
            collection[parseInt(i.toString(), 10)].field
    ...
    for (let k: number = 0; k < collection.length; k++) {
      if (k !== collection.length - 1) {
        ...
        this.setWidth(
          200,
          this.parent.getColumnIndexByField(
            collection[parseInt(k.toString(), 10)].field
          )
        );
      }
    }
  ...
  public setColumnWidth(column: Column, index?: number, module?: string): void {
    ...
    const columnIndex: number = isNullOrUndefined(index)
      ? this.parent.getNormalizedColumnIndex(column.uid)
      : index;
    const cWidth: string | number = this.getWidth(column);
    const tgridWidth: number | string = this.getTableWidth(
    ...
    if (cWidth !== null) {
      ...
      if (
        (this.parent.allowResizing && module === 'resize') ||
        (this.parent.getFrozenColumns() && this.parent.allowResizing)
      ) {
  ...
  private setWidth(
    ...
    if (
      this.parent.groupSettings.columns.length > index &&
      ispercentageWidth(this.parent)
    ) {
    ...
    const headerCol: HTMLTableColElement = <HTMLTableColElement>(
      header.querySelector(literals.colGroup).children[
    ...
    const contentCol: HTMLTableColElement = <HTMLTableColElement>(
      content.querySelector(literals.colGroup).children[
        parseInt(index.toString(), 10)
      ]
    );
    if (contentCol && !clear) {
    ...
    if (!this.parent.enableColumnVirtualization && this.parent.isEdit) {
      const edit: NodeListOf<Element> = this.parent.element.querySelectorAll(
        '.e-table.e-inline-edit'
      );
      const editTableCol: HTMLTableColElement[] = [];
      for (let i: number = 0; i < edit.length; i++) {
        if (
          parentsUntil(edit[parseInt(i.toString(), 10)], 'e-grid').id ===
          this.parent.element.id
        ) {
          for (
            let j: number = 0;
            j <
            edit[parseInt(i.toString(), 10)].querySelector('colgroup').children
              .length;
            j++
          ) {
            editTableCol.push(
              <HTMLTableColElement>(
                edit[parseInt(i.toString(), 10)].querySelector('colgroup')
                  .children[parseInt(j.toString(), 10)]
              )
            );
          }
        }
      ...
      if (
        edit.length &&
        editTableCol.length &&
        editTableCol[parseInt(index.toString(), 10)]
      ) {
  ...
  public refreshFrozenScrollbar(): void {
    ...
    const movableScrollbar: HTMLElement = this.parent.element.querySelector(
      '.e-movablescrollbar'
    );
    let movableWidth: number = this.parent
      .getContent()
      .firstElementChild.getBoundingClientRect().width;
    if (this.parent.enableColumnVirtualization) {
      const placeHolder: HTMLElement = this.parent
        .getContent()
        .querySelector('.e-virtualtrack');
      if (placeHolder) {
    ...
    (movableScrollbar.firstElementChild as HTMLElement).style.width =
      movableWidth + 'px';
  }
  ...
  private getHeightFromDirection(
    ...
    while (sibling) {
      if (
        classList.some((value: string) => sibling.classList.contains(value))
      ) {
  ...
  public isWidthUndefined(): boolean {
    const isWidUndefCount: number = this.parent
      .getColumns()
      .filter((col: Column) => {
  ...
  public getWidth(column: Column): string | number {
    if (this.parent.allowResizing && isNullOrUndefined(column.width)) {
      if (
        isNullOrUndefined(column.minWidth) &&
        isNullOrUndefined(column.maxWidth) &&
        !this.isWidthUndefined()
      ) {
        column.width = 200;
    ...
    if (column.minWidth && width < parseInt(column.minWidth.toString(), 10)) {
      ...
      column.maxWidth &&
      width > parseInt(column.maxWidth.toString(), 10)
    ) {
  ...
  public setWidthToTable(isMaxWidth?: boolean): void {
    let tWidth: string = formatUnit(
      this.getTableWidth(<Column[]>this.parent.getColumns())
    );
    if (this.parent.detailTemplate || this.parent.childGrid) {
      this.setColumnWidth(new Column({ width: '30px' }));
    }
    tWidth =
      this.isAutoResize() || tWidth === 'auto' || isMaxWidth ? '100%' : tWidth;
    ...
    const edit: HTMLTableElement = <HTMLTableElement>(
      this.parent.element.querySelector('.e-table.e-inline-edit')
    );
    if (edit) {
  ...
  private isAutoResize(): boolean {
    return (
      this.parent.allowResizing && this.parent.resizeSettings.mode === 'Auto'
    );
  }
}
