...
import { addClass, removeClass, extend, Browser } from '@hoyui/base';
import {
  IGrid,
  IFocus,
  FocusInfo,
...
export class FocusStrategy {
  ...
  public content: IFocus;
  public header: IFocus;
  public active: IFocus;
  /** @hidden */
  ...
  protected onFocus(e?: FocusEvent): void {
    if (
      ...
      this.parent.element.querySelector('.e-masked-table') ||
      (!this.parent.isInitialLoad &&
        e &&
        e.target === this.parent.element &&
        this.parent.element.querySelector('.e-spin-show'))
    ) {
      return;
    }
    this.setActive(
      !this.parent.enableHeaderFocus && this.parent.frozenRows === 0
    );
    if (
      !this.parent.enableHeaderFocus &&
      ...
      this.getContent().matrix.generate(
        this.rowModelGen.generateRows({
          rows: [new Row<Column>({ isDataRow: true })],
        }),
        this.getContent().selector,
        false
      );
    }
    const current: number[] = this.getContent().matrix.get(
      0,
      -1,
      [0, 1],
      null,
      this.getContent().validator()
    );
  ...
  protected passiveFocus(e: FocusEvent): void {
    ...
    const firstHeaderCell: Element = this.parent
      .getHeaderContent()
      .querySelector('.e-headercell:not(.e-hide)');
    if (
      e.target === firstHeaderCell &&
      e.relatedTarget &&
      ...
      if (
        this.active.matrix.matrix[firstHeaderCellIndex[0]][
          firstHeaderCellIndex[1]
        ] === 0
      ) {
        firstHeaderCellIndex = findCellIndex(
          this.active.matrix.matrix,
          firstHeaderCellIndex,
          true
        );
      }
    ...
    if (
      e.target &&
      (<HTMLElement>e.target).classList.contains('e-detailcell')
    ) {
  ...
  protected onBlur(e?: FocusEvent): void {
    if (
      ...
      this.parent.pagerModule.pagerObj.element.querySelector(
        '.e-pagercontainer'
      )
    ) {
      this.parent.pagerModule.pagerObj.element
        .querySelector('.e-pagercontainer')
        .removeAttribute('aria-hidden');
    }
    // The below boolean condition for gantt team focus fix.
    const isGantt: boolean =
      parentsUntil(e.target as Element, 'e-gantt') &&
      (e.target as Element).classList.contains('e-rowcell') &&
      !isNullOrUndefined((e.target as Element).nextElementSibling) &&
      (e.target as Element).nextElementSibling.classList.contains('e-rowcell')
        ? true
        : false;
    if (
      (this.parent.isEdit ||
        (e &&
          (!e.relatedTarget ||
            closest(<HTMLElement>e.relatedTarget, '.e-grid')) &&
          !(
            this.parent.element.classList.contains('e-childgrid') &&
            !this.parent.element.matches(':focus-within')
          ))) &&
      !(
        !isGantt &&
        isNullOrUndefined(e.relatedTarget) &&
        parseInt((e.target as Element).getAttribute('data-colindex'), 10) ===
          0 &&
        parseInt((e.target as Element).getAttribute('index'), 10) === 0
      ) &&
      !(
        !isGantt &&
        isNullOrUndefined(e.relatedTarget) &&
        !closest(document.activeElement, '.e-grid') &&
        !isNullOrUndefined(e['sourceCapabilities'])
      )
    ) {
      return;
    }
  ...
  public setFirstFocusableTabIndex(): void {
    ...
    if (gObj.allowGrouping && gObj.groupSettings.showDropArea) {
      ...
      const focusableGroupedItems: Element[] =
        groupModule.getFocusableGroupedItems();
    ...
    if (gObj.toolbar || gObj.toolbarTemplate) {
      ...
      const focusableToolbarItems: Element[] = (
        this.parent as Grid
      ).toolbarModule.getFocusableToolbarItems();
      if (
        focusableToolbarItems.length > 0 &&
        focusableToolbarItems[0].querySelector(
          '.e-toolbar-item-focus,.e-btn,.e-input'
        )
      ) {
    ...
    if (gObj.getColumns().length) {
      const firstHeaderCell: HTMLElement = gObj
        .getHeaderContent()
        .querySelector('.e-headercell:not(.e-hide)');
      firstHeaderCell.tabIndex = 0;
      this.setActive(false);
      if (
        !isNullOrUndefined(this.active) &&
        (isNullOrUndefined(this.active.target) ||
          !this.active.target.classList.contains('e-columnmenu'))
      ) {
        let firstHeaderCellIndex: number[] = [0, 0];
        if (
          this.active.matrix.matrix[firstHeaderCellIndex[0]][
  ...
  private setLastContentCellTabIndex(): void {
    const contentTable: HTMLTableElement =
      this.parent.getContentTable() as HTMLTableElement;
    if (contentTable.rows[contentTable.rows.length - 1]) {
      const lastCell: Element =
        contentTable.rows[contentTable.rows.length - 1].lastElementChild;
      (lastCell as HTMLElement).tabIndex = 0;
    }
  }

  public onClick(
    e: Event | { target: Element; type?: string },
    force?: boolean
  ): void {
    if (
      parentsUntil(e.target as HTMLElement, 'e-filterbarcell') &&
      (parentsUntil(e.target as HTMLElement, 'e-multiselect') ||
        (e.target as HTMLElement).classList.contains('e-input-group-icon'))
    ) {
      return;
    }
    let isContent: boolean = !isNullOrUndefined(
      closest(<HTMLElement>e.target, '.' + literals.gridContent)
    );
    const isHeader: boolean = !isNullOrUndefined(
      closest(<HTMLElement>e.target, '.' + literals.gridHeader)
    );
    ...
    if (
      ...
      (!isNullOrUndefined(closest(<HTMLElement>e.target, '.e-unboundcell')) &&
        !force)
    ) {
      return;
    }
    this.setActive(isContent);
    const beforeArgs: CellFocusArgs = {
      cancel: false,
      byKey: false,
    ...
    if (
      beforeArgs.cancel ||
      (closest(<Element>e.target, '.e-inline-edit') &&
    ...
    if (this.getContent()) {
      ...
      this.focus();
      if (
        this.currentInfo.element.classList.contains('e-rowcell') &&
        e.type &&
        e.type === 'click'
      ) {
        addClass([this.currentInfo.element], ['e-focused', 'e-focus']);
  ...
  private handleFilterNavigation(
    ...
    if (
      (e.target as HTMLElement) === document.querySelector(inputSelector) &&
      e.key === 'Tab' &&
      e.shiftKey
    ) {
      e.preventDefault();
      ...
      (e.target as HTMLElement) === document.querySelector(buttonSelector) &&
      e.key === 'Tab' &&
      !e.shiftKey &&
  ...
  protected onKeyPress(e: KeyboardEventArgs): void {
    ...
    if (this.parent.allowPaging) {
      ...
      const focusablePagerElements: Element[] =
        this.parent.pagerModule.pagerObj.getFocusablePagerElements(
          pagerElement,
          []
        );
      if (
        this.parent.childGrid &&
        !parentsUntil(e.target as Element, 'e-gridpager') &&
        this.allowToPaging(e) &&
        focusablePagerElements.length
      ) {
      ...
      if (this.parent.pagerModule.pagerObj.checkPagerHasFocus()) {
        if (
          e.action === 'shiftTab' &&
          focusablePagerElements.length &&
          focusablePagerElements[0] === e.target
        ) {
          this.setActive(true);
          let lastHeaderCellIndex: number[] = [
            this.active.matrix.matrix.length - 1,
            this.active.matrix.matrix[this.active.matrix.matrix.length - 1]
              .length - 1,
          ];
          if (
            this.active.matrix.matrix[lastHeaderCellIndex[0]][
              lastHeaderCellIndex[1]
            ] === 0
          ) {
            lastHeaderCellIndex = findCellIndex(
              this.active.matrix.matrix,
              lastHeaderCellIndex,
              false
            );
          }
          this.active.matrix.current =
            this.parent.editSettings.mode === 'Batch'
              ? this.isValidBatchEditCell(lastHeaderCellIndex)
                ? lastHeaderCellIndex
                : this.findBatchEditCell(lastHeaderCellIndex, false)
              : lastHeaderCellIndex;
          e.preventDefault();
          this.focus(e);
          return;
        ...
        if (
          !(
            e.action === 'tab' &&
            this.parent.element.classList.contains('e-childgrid') &&
            ((!this.parent.pageSettings.pageSizes &&
              focusablePagerElements.length &&
              focusablePagerElements[focusablePagerElements.length - 1] ===
                e.target) ||
          ...
          const parentCell: Element = parentsUntil(
            this.parent.element,
            'e-detailcell'
          );
      ...
      if (
        this.parent.pagerModule.pagerObj.element.tabIndex === 0 &&
        (e.keyCode === 38 || (e.shiftKey && e.keyCode === 9))
      ) {
        e.preventDefault();
        this.focus(e);
        return;
    ...
    if (this.parent.filterSettings.type === 'Excel') {
      this.handleFilterNavigation(
        e,
        '.e-excelfilter .e-menu-item:not(.e-disabled)',
        '.e-excelfilter .e-footer-content button:nth-child(2)'
      );
    }
    if (this.parent.filterSettings.type === 'CheckBox') {
      this.handleFilterNavigation(
        e,
        '.e-searchinput.e-input',
        '.e-checkboxfilter .e-footer-content button:nth-child(2)'
      );
    }
    if (this.parent.filterSettings.type === 'Menu') {
      this.handleFilterNavigation(
        e,
        '.e-flmenu .e-input-group.e-popup-flmenu',
        '.e-flmenu .e-footer-content button:nth-child(2)'
      );
    }
    if (this.skipOn(e)) {
      return;
    ...
    if (e.target && parentsUntil(e.target as Element, 'e-gridcontent')) {
      const rows: HTMLElement[] = [].slice.call(
        (this.parent.getContentTable() as HTMLTableElement).rows
      );
      const lastCell: HTMLElement = rows[rows.length - 1]
    ...
    if (
      ...
      e.target &&
      (e.target === this.parent.element ||
        parentsUntil(e.target as Element, 'e-toolbar') ||
      ...
      if (parentsUntil(e.target as Element, 'e-toolbar')) {
        if (
          ...
          this.parent.groupSettings.showDropArea
        ) {
          ...
          if (focusableGroupedItems.length > 0) {
            (
              focusableGroupedItems[
                focusableGroupedItems.length - 1
              ] as HTMLElement
            ).focus();
          } else {
    ...
    if (
      ...
      if (
        (this.parent.toolbar || this.parent.toolbarTemplate) &&
        (e.target === this.parent.element ||
          parentsUntil(e.target as Element, 'e-groupdroparea') ||
          (e.target as HTMLElement).classList.contains('e-toolbar'))
      ) {
        const toolbarElement: Element = (this.parent as Grid).toolbarModule
          .element;
        const focusableToolbarItems: Element[] = (
          this.parent as Grid
        ).toolbarModule.getFocusableToolbarItems();
    ...
    if (focusFirstHeaderCell) {
      if (
        this.parent.allowGrouping &&
        !isNullOrUndefined(this.parent.groupSettings.columns) &&
    ...
    const prevBatchValue: number[] =
      this.active && this.active.matrix.current
        ? [this.active.matrix.current[0], this.active.matrix.current[1]]
        : undefined;
    ...
    if (
      ...
      this.getContent().matrix.current = this.getContent().getNextCurrent(
        bValue,
        swapInfo,
        this.active,
        e.action
    ...
    let returnVal: boolean = this.content.lastIdxCell
      ? false
      : this.getContent().onKeyPress(e);
    if (e.target && parentsUntil(e.target as Element, 'e-gridheader')) {
      if (
        e.action === 'tab' &&
        bValue.toString() === this.active.matrix.current.toString()
      ) {
        const nextHeaderCellIndex: number[] = findCellIndex(
          this.active.matrix.matrix,
          this.active.matrix.current,
          true
        );
        ...
        if (
          this.active.matrix.current.toString() ===
            lastHeaderCellIndex.toString() &&
          this.content.matrix.matrix.length
        ) {
          returnVal = true;
          this.setActive(true);
          ...
          if (
            this.active.matrix.matrix[firstContentCellIndex[0]][
              firstContentCellIndex[1]
            ] === 0
          ) {
            firstContentCellIndex = findCellIndex(
              this.active.matrix.matrix,
              [0, 0],
              true
            );
          ...
          this.active.matrix.current =
            this.parent.editSettings.mode === 'Batch'
              ? this.isValidBatchEditCell(firstContentCellIndex)
                ? firstContentCellIndex
                : this.findBatchEditCell(firstContentCellIndex, true)
              : firstContentCellIndex;
        } else if (
          this.active.matrix.current.toString() !==
          nextHeaderCellIndex.toString()
        ) {
      ...
      if (
        ...
        const previousCellIndex: number[] = findCellIndex(
          this.active.matrix.matrix,
          this.active.matrix.current,
          false
        );
        if (
          previousCellIndex.toString() === this.active.matrix.current.toString()
        ) {
    ...
    if (e.target && parentsUntil(e.target as Element, 'e-gridcontent')) {
      ...
      if (
        ...
        this.active.matrix.current = this.findBatchEditCell(
          prevBatchValue,
          e.action === 'tab' ? true : false
        );
        if (
          e.action === 'tab' &&
      ...
      if (
        ...
        if (
          !returnVal &&
          (firstContentCellIndex.toString() ===
            this.active.matrix.current.toString() ||
            (this.parent.editSettings.mode === 'Batch' &&
              prevBatchValue.toString() ===
                this.active.matrix.current.toString()))
        ) {
          returnVal = true;
          this.setActive(false);
    ...
    if (returnVal === false) {
      ...
      if (
        this.parent.allowPaging &&
        !this.parent.pagerModule.pagerObj.checkPagerHasFocus() &&
        this.allowToPaging(e) &&
    ...
    if (
      e.target &&
      parentsUntil(e.target as HTMLElement, 'e-fltrtemp') &&
      (e.action === 'tab' || e.action === 'shiftTab')
    ) {
      const target: HTMLElement = e.target as HTMLElement;
      const focusElement: HTMLElement[] = [].slice.call(
        closest(target, 'th').querySelectorAll('.e-fltrtemp-focus')
      );
      if (focusElement.length) {
        const elementIndex: number = focusElement.indexOf(target);
        const resetActive: boolean =
          (e.action === 'tab' && elementIndex < focusElement.length - 1) ||
          (e.action === 'shiftTab' && elementIndex > 0)
            ? true
            : false;
        if (resetActive) {
          this.setActive(false);
    ...
    if (
      ...
      parentsUntil(this.active.getTable(), 'e-gridheader') &&
      e.target &&
      (e.target as HTMLElement).id === this.parent.element.id + '_searchbar'
    ) {
  ...
  private isValidBatchEditCell(cellIndex: number[]): boolean {
    const cell: Element =
      this.active.getTable().rows[cellIndex[0]].cells[cellIndex[1]];
    const tr: Element = closest(cell, 'tr');
    const cellColIndex: number = parseInt(
      cell.getAttribute('data-colindex'),
      10
    );
    const cellCol: Column =
      this.parent.getColumns()[parseInt(cellColIndex.toString(), 10)];
    if (
      this.active.matrix.matrix[cellIndex[0]][cellIndex[1]] === 1 &&
      (!tr.classList.contains('e-row') ||
        ((tr.classList.contains('e-insertedrow') || !cellCol.isPrimaryKey) &&
          cellCol.allowEditing))
    ) {
      return true;
    }
    return false;
  ...
  private findBatchEditCell(
    ...
    while (!cellIndexObtain) {
      ...
      tempCellIndex = findCellIndex(
        this.active.matrix.matrix,
        tempCellIndex,
        next
      );
      if (
        prevTempCellIndex.toString() === tempCellIndex.toString() ||
  ...
  private setLastContentCellActive(): void {
    let lastContentCellIndex: number[] = [
      this.active.matrix.matrix.length - 1,
      this.active.matrix.matrix[this.active.matrix.matrix.length - 1].length -
        1,
    ];
    if (
      this.active.matrix.matrix[lastContentCellIndex[0]][
        lastContentCellIndex[1]
      ] === 0
    ) {
      lastContentCellIndex = findCellIndex(
        this.active.matrix.matrix,
        lastContentCellIndex,
        false
      );
    }
  ...
  private focusOutFromChildGrid(e: KeyboardEventArgs): void {
    const parentTable: HTMLTableElement = parentsUntil(
      this.parent.element,
      'e-table'
    ) as HTMLTableElement;
    const parentGrid: Grid = (parentsUntil(parentTable, 'e-grid') as EJ2Intance)
      .ej2_instances[0] as Grid;
    const parentCell: Element = parentsUntil(
      this.parent.element,
      'e-detailcell'
    );
    const uid: string = parentsUntil(
      this.parent.element,
      'e-detailrow'
    ).getAttribute('data-uid');
    const parentRows: Element[] = [].slice.call(
      (parentGrid.getContentTable() as HTMLTableElement).rows
    );
    const parentRowIndex: number = parentRows
    ...
    if (e.action === 'shiftTab') {
      const previousRow: HTMLTableRowElement = parentRows[
        parentRowIndex - 1
      ] as HTMLTableRowElement;
      const rowCells: HTMLCollectionOf<HTMLTableCellElement> =
        previousRow.cells;
      for (let i: number = rowCells.length - 1; i >= 0; i--) {
        ...
        if (!nextFocusCell.classList.contains('e-hide')) {
          parentGrid.focusModule.active.matrix.current = [
            parentRowIndex - 1,
            i,
          ];
          break;
      ...
      nextFocusCell = (parentRows[parentRowIndex + 1] as HTMLTableRowElement)
        .cells[0];
  ...
  private focusOutFromHeader(e: KeyboardEventArgs): void {
    ...
    if (this.parent.toolbar || this.parent.toolbarTemplate) {
      ...
      if (focusableToolbarItems.length > 0) {
        (
          focusableToolbarItems[focusableToolbarItems.length - 1].querySelector(
            '.e-toolbar-item-focus,.e-btn,.e-input'
          ) as HTMLElement
        ).focus();
      } else {
    ...
    if (this.parent.allowGrouping && this.parent.groupSettings.showDropArea) {
      ...
      if (focusableGroupedItems.length > 0) {
        (
          focusableGroupedItems[focusableGroupedItems.length - 1] as HTMLElement
        ).focus();
      } else {
        groupModule.element.focus();
      }
  ...
  private skipOn(e: KeyboardEventArgs): boolean {
    ...
    if (
      !this.parent.isEdit &&
      target &&
      target.closest('.e-unboundcell') &&
      this.currentInfo.skipAction &&
      ((e.action === 'shiftTab' &&
        this.commandColumnFocusElement(
          target.closest('.e-unboundcell') as HTMLElement,
          false
        ) === target) ||
        (e.action === 'tab' &&
          this.commandColumnFocusElement(
    ...
    if (this.parent.allowGrouping) {
      ...
      if (
        parentsUntil(e.target as Element, 'e-groupheadercell') &&
        !(
          (e.target === focusableGroupedItems[0] && e.action === 'shiftTab') ||
          (e.target ===
            focusableGroupedItems[focusableGroupedItems.length - 1] &&
            e.action === 'tab')
        )
      ) {
        return true;
    ...
    if (this.parent.toolbar || this.parent.toolbarTemplate) {
      const toolbarElement: Element = (this.parent as Grid).toolbarModule
        .element;
      const focusableToolbarItems: NodeListOf<Element> =
        toolbarElement.querySelectorAll(
          '.e-toolbar-item:not(.e-overlay):not(.e-hidden)'
        );
      if (
        parentsUntil(e.target as Element, 'e-toolbar-item') &&
        !(
          focusableToolbarItems.length > 0 &&
          ((parentsUntil(e.target as Element, 'e-toolbar-item') ===
            focusableToolbarItems[0] &&
            e.action === 'shiftTab') ||
            (parentsUntil(e.target as Element, 'e-toolbar-item') ===
              focusableToolbarItems[focusableToolbarItems.length - 1] &&
              e.action === 'tab'))
        )
      ) {
        return true;
    ...
    const th: boolean =
      closest(target, 'th') && !(closest(target, 'th') as HTMLElement).tabIndex;
    if (
      (e.target as HTMLElement).classList.contains('e-filterbaroperator') &&
      (e.keyCode === 13 || e.keyCode === 27)
    ) {
      const inputTarget: Element = closest(
    ...
    const addNewRow: boolean =
      this.parent.editSettings.showAddNewRow &&
      closest(document.activeElement, '.e-addedrow') !== null;
    if (
      (th && closest(document.activeElement, '.e-filterbarcell') !== null) ||
      addNewRow
    ) {
      this.removeFocus();
    }
    let filterCell: boolean =
    ...
    if (
      ...
      !this.parent.editSettings.showAddNewRow &&
      !isNullOrUndefined(parentsUntil(target, 'e-addedrow'))
    ) {
      const inputElements: NodeListOf<Element> =
        this.parent.editModule.formObj.element.querySelectorAll(
          'input.e-field:not(.e-disabled),button:not(.e-hide)'
        );
      const inputTarget: HTMLElement = target.classList.contains('e-ddl')
        ? target.querySelector('input')
        : target;
      const firstEditCell: boolean =
        e.action === 'tab' &&
        inputTarget === inputElements[inputElements.length - 1];
      const lastEditCell: boolean =
        e.action === 'shiftTab' && inputTarget === inputElements[0];
      if (firstEditCell || lastEditCell) {
        e.preventDefault();
        let focusElement: HTMLElement = inputElements[
          firstEditCell ? 0 : inputElements.length - 1
        ] as HTMLElement;
        focusElement = focusElement.parentElement.classList.contains('e-ddl')
    ...
    return (
          ...
          (!this.parent.editSettings.showAddNewRow ||
            (this.parent.editSettings.showAddNewRow &&
              (!isNullOrUndefined(
                ...
                (!isNullOrUndefined(parentsUntil(target, 'e-addedrow')) &&
                  !isNullOrUndefined(
                    closest(e.target as HTMLElement, 'input')
                  ) &&
                  !isNullOrUndefined(
                ...
                (!isNullOrUndefined(parentsUntil(target, 'e-addedrow')) &&
                  target &&
                  !target.querySelector('.e-cancel-icon') &&
                  !isNullOrUndefined(
                    parentsUntil(target, 'e-unboundcell')
                  )))))) ||
          ['insert', 'f2'].indexOf(e.action) > -1)) ||
      (filterCell && this.parent.enableHeaderFocus) ||
      ((filterCell || addNewRow) &&
        e.action !== 'tab' &&
        e.action !== 'shiftTab') ||
      (closest(
        document.activeElement,
        '#' + this.parent.element.id + '_searchbar'
      ) !== null &&
        [
          'enter',
          'leftArrow',
      ...
      (closest(target, '.' + literals.gridContent) === null &&
        closest(target, '.' + literals.gridHeader) === null &&
        !(
          e.target === this.parent.element ||
          parentsUntil(e.target as Element, 'e-toolbar') ||
          parentsUntil(e.target as Element, 'e-groupdroparea')
      ...
      (e.action === 'space' &&
        !target.classList.contains(literals.gridChkBox) &&
        closest(target, '.' + literals.gridChkBox) === null &&
        closest(target, '.e-headerchkcelldiv') === null) ||
      closest(target, '.e-filter-popup') !== null
    );
  }
  ...
  private focusVirtualElement(e?: KeyboardEventArgs): void {
    if (
      ...
      const data: { virtualData: Object; isAdd: boolean; isCancel: boolean } = {
        virtualData: {},
        isAdd: false,
        isCancel: false,
      };
      this.parent.notify(event.getVirtualData, data);
      const isKeyFocus: boolean = this.actions.some(
        (value: string) => value === this.activeKey
      );
      const isSelected: boolean = this.parent.contentModule
        ? (<{ selectedRowIndex?: number }>this.parent.contentModule)
            .selectedRowIndex > -1
        : false;
      if (
        data.isAdd ||
        Object.keys(data.virtualData).length ||
        isKeyFocus ||
        data.isCancel ||
        isSelected
      ) {
        ...
        if (this.isVirtualScroll || this.isInfiniteScroll) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (this.currentInfo.elementToFocus as any).focus({
            preventScroll: true,
          });
        } else {
  ...
  public setFocusedElement(element: HTMLElement, e?: KeyboardEventArgs): void {
    ...
    setTimeout(() => {
      if (!isNullOrUndefined(this.currentInfo.elementToFocus)) {
        const filterMenuElement: Element = isNullOrUndefined(
          this.parent.element.querySelector('.e-flmenu')
        )
          ? document.querySelector('.e-grid-popup .e-flmenu')
          : this.parent.element.querySelector('.e-flmenu');
        if (
          this.parent.enableVirtualization ||
          this.parent.enableInfiniteScrolling
          ...
          isNullOrUndefined(filterMenuElement) ||
          parentsUntil(document.activeElement, 'e-flmenu-valuediv') !==
            filterMenuElement.querySelector('.e-flmenu-valuediv')
        ) {
          this.currentInfo.elementToFocus.focus();
        }
      }
  ...
  protected removeFocus(e?: FocusEvent): void {
    ...
    if (
      ...
      const cellElem: HTMLElement = this.parent.getCellFromIndex(
        this.prevIndexes.rowIndex,
        this.prevIndexes.cellIndex
      ) as HTMLElement;
  ...
  protected addFocus(
    info: FocusInfo,
    e?: KeyboardEventArgs | FocusEvent
  ): void {
    this.currentInfo = info;
    this.currentInfo.outline =
      info.outline && (!isNullOrUndefined(e) || this.isVirtualScroll);
    ...
    const isFocused: boolean =
      info.elementToFocus.classList.contains('e-focus');
    if (isFocused) {
      return;
    }
  ...
  protected refreshMatrix(content?: boolean): Function {
    return (e: { rows: Row<Column>[]; args?: NotifyArgs; name?: string }) => {
      ...
      if (frozenRow && this.parent.editSettings.mode === 'Batch') {
        batchLen =
          this.parent.getHeaderContent().querySelectorAll('.e-insertedrow')
            .length +
          this.parent.getHeaderContent().querySelectorAll('.e-hiddenrow')
            .length;
      }
      if (
        !isNullOrUndefined(this.parent.groupSettings.columns) &&
        ...
        for (let i: number = 0; i < e.rows.length; i++) {
          frozenRow++;
          if (
            e.rows[parseInt(i.toString(), 10)].index + 1 ===
      ...
      let rows: Row<Column>[] = content
        ? e.rows.slice(frozenRow + batchLen)
        : e.rows;
      const updateRow: Row<Column>[] = content
        ? e.rows.slice(0, frozenRow + batchLen)
        : e.rows;
      if (this.parent.isCollapseStateEnabled() && content) {
        rows = rows.filter((x: Row<Column>) => x.visible !== false);
      }
      const isRowTemplate: boolean = !isNullOrUndefined(
        this.parent.rowTemplate
      );
      if (
        frozenRow &&
        ((this.parent.editSettings.mode === 'Batch' &&
          ...
          (e.name === 'batchDelete' ||
            e.name === 'batchAdd' ||
            e.name === 'batchCancel' ||
            (e.args && e.args.requestType === 'batchsave'))) ||
          (e.args &&
            (e.args.requestType === 'delete' || e.args.requestType === 'save')))
      ) {
        const matrixcs: number[][] = this.header.matrix.matrix;
        const hdrLen: number = (<{ rows?: Row<Column>[] }>(
          this.parent.headerModule
        )).rows.length;
        matrixcs.splice(hdrLen, matrixcs.length - hdrLen);
      }
      const matrix: number[][] = cFocus.matrix.generate(
        updateRow,
        cFocus.selector,
        isRowTemplate
      );
      ...
      if (
        ...
        if (this.parent.editSettings.newRowPosition === 'Top') {
          (this.parent.frozenRows || isScroll
            ? matrix
            : cFocus.matrix.matrix
          ).unshift(
            this.refreshAddNewRowMatrix(
              tempMatrix[tempMatrix.length - lastRowHeaderIdx]
            )
          );
        } else {
          cFocus.matrix.matrix.push(
            this.refreshAddNewRowMatrix(
              tempMatrix[tempMatrix.length - lastRowHeaderIdx]
            )
          );
      ...
      if (
        !(
          ...
          e.args &&
          (e.args.requestType === 'sorting' ||
            e.args.requestType === 'batchsave' ||
            e.args.requestType === 'paging')
        ) ||
        (frozenRow &&
          this.parent.editSettings.mode === 'Batch' &&
          content &&
      ...
      if (e && e.args && e.args.requestType === 'virtualscroll') {
        if (this.currentInfo.uid) {
          ...
          if (bool) {
            ...
            this.content.matrix.current[1] =
              this.parent.getColumnIndexByUid(this.focusedColumnUid) || 0;
            const frzLeftCount: number =
              this.parent.getVisibleFrozenLeftCount();
            const frzRightCount: number =
              this.parent.getVisibleFrozenRightCount();
            if (
              this.parent.enableColumnVirtualization &&
              !(frzLeftCount && frzRightCount) &&
              this.parent.isFrozenGrid() &&
              e.args.virtualInfo.direction === 'right'
            ) {
              this.content.matrix.current[1] =
                this.content.matrix.current[1] -
                (frzLeftCount + (frzRightCount ? frzRightCount + 1 : 0));
            }
            ...
            if (isGroup) {
              ...
              if (this.virtualSelectionInfo.isPending) {
                this.content.matrix.current[0] =
                  this.virtualSelectionInfo.direction === 'downArrow'
                    ? this.content.matrix.current[0] + 1
                    : this.content.matrix.current[0] - 1;
              }
            }
            const focusElement: HTMLElement =
              this.getContent().getFocusInfo().elementToFocus;
            if (focusElement) {
              const cellPosition: ClientRect =
                focusElement.getBoundingClientRect();
              const gridPosition: ClientRect =
              ...
              if (
                ...
                if (frzLeftCount) {
                  ...
                  freezeRightColWidth =
                    this.parent.leftrightColumnWidth('right');
                }
              }
              if (
                (cellPosition.top >= 0 &&
                ...
                this.focus(
                  isGroup && this.virtualSelectionInfo.isPending
                    ? (this.virtualSelectionInfo.event as KeyboardEventArgs)
                    : undefined
                );
              }
            }
          ...
          e.args.focusElement &&
          e.args.focusElement.classList.contains('e-filtertext')
        ) {
          const focusElement: HTMLElement = <HTMLElement>(
            this.parent.element.querySelector('#' + e.args.focusElement.id)
          );
          if (focusElement) {
            focusElement.focus();
          }
  ...
  private refreshAddNewRowMatrix(matrix?: number[]): number[] {
    ...
    for (let i: number = indent; i < matrix.length - 1; i++) {
      if (
        cols[i - indent] &&
        cols[i - indent].visible &&
        cols[i - indent].allowEditing
      ) {
  ...
  public addEventListener(): void {
    ...
    this.parent.element.addEventListener(
      'focus',
      (this.passiveHandler = (e: FocusEvent) => this.passiveFocus(e)),
      true
    );
    EventHandler.add(this.parent.element, 'focusout', this.onBlur, this);
    this.evtHandlers = [
      { event: event.keyPressed, handler: this.onKeyPress },
  ...
  private showAddNewRowFocus(): void {
    if (this.parent.editSettings.showAddNewRow) {
      const startIdx: number =
        this.parent.editSettings.newRowPosition === 'Top'
          ? 0
          : this.content.matrix.matrix.length - 1;
      ...
      if (
        ...
        const headrIdx: number =
          this.header.matrix.matrix.length -
          (this.groupedFrozenRow
            ? this.groupedFrozenRow
            : this.parent.frozenRows);
        startCellIdx = this.findNextCellFocus(
          this.header.matrix.matrix[headrIdx - 1],
          startCellIdx
        );
        ...
        startCellIdx = this.findNextCellFocus(
          this.content.matrix.matrix[parseInt(startIdx.toString(), 10)],
          startCellIdx
        );
        this.content.matrix.current = [startIdx, startCellIdx];
        this.active = this.content;
      }
      const addedrow: HTMLElement =
        this.parent.element.querySelector('.e-addedrow');
      if (
        addedrow &&
        addedrow.querySelectorAll('tr') &&
        addedrow
          .querySelector('tr')
          .cells[parseInt(startCellIdx.toString(), 10)].querySelector('input')
      ) {
        (
          addedrow
            .querySelector('tr')
            .cells[parseInt(startCellIdx.toString(), 10)].querySelector(
              'input'
            ) as HTMLInputElement
        ).select();
      }
  ...
  public restoreFocus(): void {
    ...
    if (
      this.parent.allowGrouping &&
      groupModule &&
      (groupModule.groupSortFocus || groupModule.groupTextFocus)
    ) {
  ...
  public restoreFocusWithAction(e: NotifyArgs): void {
    if (!this.parent.enableInfiniteScrolling) {
      ...
      switch (e.requestType) {
        ...
        case 'ungrouping':
          current[1] =
            ...
            !isNullOrUndefined(matrix.matrix[current[0]])
              ? matrix.matrix[current[0]].indexOf(1)
              : e.requestType === 'grouping'
              ? current[1] + 1
              : current[1] - 1;
          break;
      }
      ...
      if (
        this.parent.allowGrouping &&
        groupModule &&
        groupModule.groupCancelFocus
      ) {
        const focusableGroupedItems: Element[] =
          groupModule.getFocusableGroupedItems();
        if (focusableGroupedItems.length) {
          if (
            focusableGroupedItems[0].parentElement.getAttribute(
              'ej-mappingname'
            ) === (e as GroupEventArgs).columnName
          ) {
  ...
  public getPrevIndexes(): IIndex {
    const forget: boolean = this.forget;
    this.forget = false;
    return forget || !Object.keys(this.prevIndexes).length
      ? { rowIndex: null, cellIndex: null }
      : this.prevIndexes;
  }
  ...
  public internalCellFocus(e: CellFocusArgs): void {
    if (
      !(
        e.byKey &&
        e.container.isContent &&
        ((e.keyArgs.action === 'enter' &&
          e.parent.classList.contains('e-detailcell')) ||
          ((e.keyArgs.action === 'tab' || e.keyArgs.action === 'shiftTab') &&
    ...
    if (
      e.parent &&
      e.parent.closest('.e-unboundcell') &&
  ...
  private commandColumnFocusElement(
    ...
    const commandButtons: NodeListOf<Element> = cell.querySelectorAll(
      'button:not(.e-hide)'
    );
    return isLast
      ? (commandButtons[commandButtons.length - 1] as HTMLElement)
      : (commandButtons[0] as HTMLElement);
  }
}
...
export class Matrix {
  ...
  public set(rowIndex: number, columnIndex: number, allow?: boolean): void {
    ...
    this.matrix[parseInt(rowIndex.toString(), 10)] =
      this.matrix[parseInt(rowIndex.toString(), 10)] || [];
    this.matrix[parseInt(rowIndex.toString(), 10)][
  ...
  public get(
    ...
    columnIndex = Math.max(
      0,
      Math.min(
        columnIndex + navigator[1],
        this.matrix[parseInt(rowIndex.toString(), 10)].length - 1
      )
    );
    if (
      tmp + navigator[1] >
    ...
    if (first === null) {
      for (let i: number = 0; i < this.rows; i++) {
        if (
          this.matrix[parseInt(i.toString(), 10)].some((v: number) => {
            return v === 1;
          })
        ) {
          emptyTable = false;
    ...
    return this.inValid(val) || !validator(rowIndex, columnIndex, action)
      ? this.get(rowIndex, tmp, navigator, action, validator)
  ...
  public first(
    ...
    if (
      ((index < 0 || index === vector.length) &&
        this.inValid(vector[parseInt(index.toString(), 10)]) &&
        action !== 'upArrow' &&
        action !== 'downArrow') ||
      !vector.some((v: number) => v === 1)
    ) {
      return null;
    }
    return !this.inValid(vector[parseInt(index.toString(), 10)])
      ? index
      : this.first(
          vector,
          ['upArrow', 'downArrow', 'shiftUp', 'shiftDown'].indexOf(action) !==
            -1
            ? moveTo
              ? 0
              : ++index
  ...
  public generate(
    ...
    for (let i: number = 0; i < rows.length; i++) {
      const cells: Cell<Column>[] = rows[
        parseInt(i.toString(), 10)
      ].cells.filter((c: Cell<Column>) => c.isSpanned !== true);
      this.columns = Math.max(cells.length - 1, this.columns | 0);
      let incrementNumber: number = 0;
      for (let j: number = 0; j < cells.length; j++) {
        if (
          cells[parseInt(j.toString(), 10)].column &&
          cells[parseInt(j.toString(), 10)].column.columns
        ) {
          incrementNumber = this.columnsCount(
            cells[parseInt(j.toString(), 10)].column.columns as Column[],
            incrementNumber
          );
        } else {
          incrementNumber++;
...
export class ContentFocus implements IFocus {
  ...
  public onKeyPress(e: KeyboardEventArgs): void | boolean {
    ...
    if (isMacLike && e.metaKey) {
      if (e.action === 'home') {
        ...
        ['downArrow', 'upArrow', 'leftArrow', 'rightArrow'].indexOf(
          e.action
        ) !== -1
      ) {
        return;
    ...
    let current: number[] = this.getCurrentFromAction(
      e.action,
      navigators,
      e.action in this.keyActions,
      e
    );
    if (!current) {
      return;
    ...
    if (
      (
        (['tab', 'shiftTab'].indexOf(e.action) > -1 && this.matrix.current) ||
        []
      ).toString() === current.toString() ||
      (this.parent.allowPaging &&
        !this.parent.pagerModule.pagerObj.checkPagerHasFocus() &&
        this.matrix.current[0] === this.matrix.rows &&
        ((this.parent.editSettings.mode === 'Batch' &&
          this.parent.editSettings.allowAdding &&
          e.keyCode === 40) ||
          e.keyCode === 40))
  ...
  private editNextRow(
    ...
    const editNextRow: boolean =
      gObj.editSettings.allowNextRowEdit &&
      (gObj.isEdit || gObj.isLastCellPrimaryKey);
    const visibleIndex: number = gObj.getColumnIndexByField(
      gObj.getVisibleColumns()[0].field
    );
    const row: HTMLTableRowElement =
      this.getTable().rows[parseInt(rowIndex.toString(), 10)];
    const cell: HTMLElement =
      gObj.editSettings.showAddNewRow && row.classList.contains('e-addedrow')
        ? row.querySelectorAll<HTMLElement>('td:not(.e-editcell)')[
    ...
    if (action === 'tab' && editNextRow) {
      ...
      const index: number =
        this.getTable().rows[
          parseInt(rowIndex.toString(), 10)
        ].getElementsByClassName('e-indentcell').length +
        this.getTable().rows[
          parseInt(rowIndex.toString(), 10)
        ].getElementsByClassName('e-detailrowcollapse').length;
      cellIndex = visibleIndex + index;
    }
    if (action === 'shiftTab' && editNextRow) {
      rowIndex--;
      cellIndex = gObj.getColumnIndexByField(
        gObj.getVisibleColumns()[gObj.getVisibleColumns().length - 1].field
      );
    }
    return !cell.classList.contains(literals.rowCell) &&
      !cell.classList.contains('e-headercell') &&
      !cell.classList.contains('e-groupcaption') &&
      !cell.classList.contains('e-filterbarcell')
      ? this.editNextRow(rowIndex, cellIndex, action)
      : [rowIndex, cellIndex];
  }

  public getCurrentFromAction(
    action: string,
    ...
    if (
      (!isPresent && !this.indexesByKey(action)) ||
      this.matrix.current.length === 0
    ) {
      return null;
    }
    ...
    let [rowIndex, cellIndex, rN, cN]: number[] = this.indexesByKey(action) || [
      ...this.matrix.current,
      ...navigator,
    ];
    if (
      ...
      for (let i: number = rowIndex; i < this.matrix.matrix.length; i++) {
        ...
        if (
          row &&
          row.cells[parseInt(cellIndex.toString(), 10)] &&
          row.cells[parseInt(cellIndex.toString(), 10)].classList.contains(
            'e-rowcell'
          )
        ) {
    ...
    if (action === 'ctrlEnd' || action === 'end') {
      ...
      if (action === 'end') {
        lastContentCellIndex = [
          rowIndex,
          this.matrix.matrix[parseInt(rowIndex.toString(), 10)].length - 1,
        ];
      }
      if (
        this.matrix.matrix[lastContentCellIndex[0]][lastContentCellIndex[1]] ===
        0
      ) {
        lastContentCellIndex = findCellIndex(
          this.matrix.matrix,
    ...
    const current: number[] = this.matrix.get(
      rowIndex,
      cellIndex,
      [rN, cN],
      action,
  ...
  public onClick(e: Event, force?: boolean): void | boolean {
    ...
    target = <HTMLTableCellElement>(
      (target.classList.contains(literals.rowCell)
        ? target
        : closest(target, 'td'))
    );
    target = target
      ...
      : <HTMLTableCellElement>(
          closest(<Element>e.target, 'td.e-detailrowcollapse')
        ) ||
        <HTMLTableCellElement>(
          closest(<Element>e.target, 'td.e-detailrowexpand')
        );
    target = <HTMLTableCellElement>closest(<Element>e.target, 'td.e-detailcell')
      ? isNullOrUndefined(
          closest(closest(<Element>e.target, '.e-grid'), 'td.e-detailcell')
        )
        ? null
        : target
      : target;
    target =
      target && closest(target, 'table').classList.contains(literals.table)
        ? target
        : null;
    if (!target) {
      return false;
    ...
    if (
      this.parent.editSettings.showAddNewRow &&
      parentsUntil(<Element>target, 'e-addedrow')
    ) {
      rowIdx = (<HTMLTableRowElement>(
        parentsUntil(<Element>target, 'e-addedrow')
      )).rowIndex;
    }
    ...
    if (
      this.matrix.inValid(val) ||
      (!force && oRowIndex === rowIndex && oCellIndex === cellIndex) ||
      (!parentsUntil(e.target as Element, literals.rowCell) &&
        !parentsUntil(e.target as Element, 'e-groupcaption') &&
        !parentsUntil(e.target as Element, 'e-recordpluscollapse') &&
        !parentsUntil(e.target as Element, 'e-recordplusexpand') &&
        !parentsUntil(e.target as Element, 'e-detailrowcollapse') &&
        !parentsUntil(e.target as Element, 'e-detailrowexpand') &&
  ...
  public getFocusInfo(): FocusInfo {
    ...
    const [rowIndex = 0, cellIndex = 0]: number[] = this.matrix.current;
    this.matrix.current = [rowIndex, cellIndex];
    const row: HTMLTableRowElement =
      this.getTable().rows[parseInt(rowIndex.toString(), 10)];
    info.element = !isNullOrUndefined(row)
      ? this.parent.editSettings.showAddNewRow &&
    ...
    info.elementToFocus =
      (!info.element.classList.contains('e-unboundcell') ||
        (this.parent.editSettings.showAddNewRow &&
          info.element.classList.contains('e-unboundcell') &&
          parentsUntil(info.element, 'e-addedrow'))) &&
      !info.element.classList.contains('e-detailcell')
        ? this.getFocusable(info.element)
        : info.element;
    info.elementToFocus =
      info.element.classList.contains('e-detailcell') &&
      info.element.querySelector('.e-childgrid')
        ? info.element.querySelector('.e-childgrid')
        : info.elementToFocus;
    if (
      this.parent.editSettings.mode === 'Batch' &&
      this.parent.isEdit &&
      info.elementToFocus.tagName.toLowerCase() === 'input' &&
  ...
  public getFocusable(element: HTMLElement): HTMLElement {
    ...
    const isTemplate: boolean = !isNullOrUndefined(
      closest(<HTMLElement>element, '.e-templatecell')
    );
    if (this.parent.isEdit) {
      const commandCellQuery: string =
        this.parent.editSettings.showAddNewRow &&
        parentsUntil(element, 'e-addedrow')
          ? ', button:not(.e-hide)'
          : '';
      query =
        'input:not([type="hidden"]), select:not([aria-hidden="true"]), textarea' +
        commandCellQuery;
    }
    const child: HTMLElement[] = [].slice.call(element.querySelectorAll(query));
    if (element.classList.contains('e-fltrtemp')) {
      const focusElement: HTMLElement[] = [].slice.call(
        element.querySelectorAll('.e-fltrtemp-focus')
      );
      if (
        this.focusType === 'click' &&
        this.target &&
        this.target.classList.contains('e-fltrtemp-focus')
      ) {
        return this.target;
      } else if (
        this.focusType === 'key' &&
        focusElement.length &&
        (this.action === 'tab' || this.action === 'shiftTab')
      ) {
        const elementIndex: number = focusElement.indexOf(this.currentTarget);
        return elementIndex === -1
          ? focusElement[this.action === 'tab' ? 0 : focusElement.length - 1]
          : focusElement[
              this.action === 'tab' ? elementIndex + 1 : elementIndex - 1
            ];
      }
    }
  ...
  public selector(
    ...
    const types: CellType[] = [
      CellType.Expand,
      CellType.GroupCaption,
      CellType.CaptionSummary,
      CellType.GroupSummary,
    ];
    return (
      ((row.isDataRow &&
        cell.visible &&
        (cell.isDataCell || cell.isTemplate)) ||
        (row.isDataRow &&
          cell.cellType === CellType.DetailExpand &&
          isNullOrUndefined(cell.visible)) ||
        (!row.isDataRow &&
          types.indexOf(cell.cellType) > -1 &&
          !(
            (cell.cellType === CellType.GroupSummary ||
              cell.cellType === CellType.CaptionSummary) &&
  ...
  public nextRowFocusValidate(index: number): number {
    ...
    for (let i: number = index, len: number = this.matrix.rows; i <= len; i++) {
      if (
        this.matrix.matrix[parseInt(index.toString(), 10)].indexOf(1) === -1
      ) {
        index = index + 1;
      } else {
        return index;
  ...
  public jump(action: string, current: number[]): SwapInfo {
    ...
    const enterFrozen: boolean =
      this.parent.frozenRows !== 0 && action === 'shiftEnter';
    const headerSwap: boolean =
      (action === 'upArrow' || enterFrozen) && current[0] === 0;
    if (this.matrix.matrix[current[0]]) {
      if (
        action === 'tab' &&
        this.matrix.matrix.length &&
        current[1] === this.matrix.matrix[current[0]].lastIndexOf(1) &&
        this.matrix.matrix.length - 1 !== current[0]
      ) {
        this.matrix.current[0] = this.nextRowFocusValidate(
    ...
    if (
      action === 'shiftTab' &&
      current[0] !== 0 &&
      this.matrix.matrix[current[0]].indexOf(1) === current[1]
    ) {
      this.matrix.current[0] = this.previousRowFocusValidate(
        this.matrix.current[0] - 1
      );
    ...
    const row: Element =
      parentsUntil(document.activeElement, 'e-addedrow') &&
      this.parent.editSettings.showAddNewRow
        ? parentsUntil(document.activeElement, 'e-addedrow')
  ...
  public generateRows(rows?: Row<Column>[], optionals?: Object): void {
    const {
      matrix,
      handlerInstance,
    }: { matrix?: number[][]; handlerInstance?: IFocus } = optionals;
    const len: number = handlerInstance.matrix.matrix.length;
    const defaultLen: number =
      this.parent.allowFiltering &&
      this.parent.filterSettings.type === 'FilterBar'
        ? len + 1
        : len;
    handlerInstance.matrix.matrix = handlerInstance.matrix.matrix.slice(
      0,
      defaultLen
  ...
  public getInfo(e?: KeyboardEventArgs): FocusedContainer {
    const info: FocusInfo = this.getFocusInfo();
    const [rIndex, cIndex]: number[] = this.matrix.current;
    const isData: boolean = info.element.classList.contains(literals.rowCell);
    const isSelectable: boolean =
      isData ||
      (e &&
        e.action !== 'enter' &&
        (info.element.classList.contains('e-detailrowcollapse') ||
          info.element.classList.contains('e-detailrowexpand')));
    // eslint-disable-next-line
    let [rowIndex, cellIndex]: number[] = [
      Math.min(
        parseInt(
          info.element.parentElement.getAttribute(literals.dataRowIndex),
          10
        ),
        rIndex
      ),
      Math.min(
        parseInt(info.element.getAttribute(literals.dataColIndex), 10),
        cIndex
      ),
    ];
    if (
    ...
    return (rowIndex: number, cellIndex: number, action?: string) => {
      if (!isNullOrUndefined(table.rows[parseInt(rowIndex.toString(), 10)])) {
        ...
        if (
          table.rows[
            parseInt(rowIndex.toString(), 10)
          ].cells[0].classList.contains('e-editcell')
        ) {
          cell =
            table.rows[
              parseInt(rowIndex.toString(), 10)
            ].cells[0].querySelectorAll('td')[
              parseInt(cellIndex.toString(), 10)
            ];
        } else {
          cell =
            table.rows[parseInt(rowIndex.toString(), 10)].cells[
              parseInt(cellIndex.toString(), 10)
            ];
        }
        ...
        if (
          ...
          cell.classList.contains(literals.rowCell)
        ) {
          return isCellWidth;
  ...
  protected shouldFocusChange(e: KeyboardEventArgs): boolean {
    ...
    return e.action === 'enter' || e.action === 'shiftEnter'
      ? (cell.classList.contains(literals.rowCell) &&
          !cell.classList.contains('e-unboundcell')) ||
          (cell.classList.contains('e-editedbatchcell') &&
            !cell.classList.contains('e-detailcell'))
      : true;
  }
...
export class HeaderFocus extends ContentFocus implements IFocus {
  ...
  public onClick(e: Event): void | boolean {
    ...
    if (
      !target &&
      (this.parent.frozenRows !== 0 ||
        ((this.parent.enableVirtualization ||
          this.parent.enableInfiniteScrolling) &&
          this.parent.editSettings.showAddNewRow))
    ) {
      target = <HTMLTableCellElement>(
        ((<HTMLElement>e.target).classList.contains(literals.rowCell)
          ? e.target
          : closest(<Element>e.target, 'td'))
      );
    }
    if (
      (e.target as HTMLElement).classList.contains('e-columnheader') ||
  ...
  public selector(row: Row<Column>, cell: Cell<Column>): boolean {
    return (
      (cell.visible &&
        (cell.column.field !== undefined ||
          cell.isTemplate ||
          !isNullOrUndefined(cell.column.template) ||
          !isNullOrUndefined(cell.column.commands))) ||
  ...
  public jump(action: string, current: number[]): SwapInfo {
    const enterFrozen: boolean =
      this.parent.frozenRows !== 0 && action === 'enter';
    ...
    return {
      swap:
        ((action === 'downArrow' || enterFrozen) &&
          current[0] === this.matrix.matrix.length - 1) ||
  ...
  public getNextCurrent(
    ...
    if (
      action === 'rightArrow' ||
      (action === 'shiftRight' && this.getGridSeletion()) ||
      action === 'tab'
    ) {
  ...
  public generateRows(rows?: Row<Column>[]): void {
    ...
    if (
      ...
      for (let i: number = 0; i < cells.length; i++) {
        if (
          cells[parseInt(i.toString(), 10)].column &&
          cells[parseInt(i.toString(), 10)].column.columns
        ) {
          incrementNumber = this.checkFilterColumn(
            cells[parseInt(i.toString(), 10)].column.columns as Column[],
            length,
            incrementNumber
          );
        } else {
          this.matrix.set(
            length,
            incrementNumber,
            cells[parseInt(i.toString(), 10)].visible &&
              cells[parseInt(i.toString(), 10)].column.allowFiltering !== false
          );
          incrementNumber++;
        }
      }
      ...
      if (columns[parseInt(i.toString(), 10)].columns) {
        ...
        this.matrix.set(
          ...
          columns[parseInt(i.toString(), 10)].visible &&
            columns[parseInt(i.toString(), 10)].allowFiltering !== false
        );
        incrementNumber++;
      }
    }
  ...
  protected shouldFocusChange(e: KeyboardEventArgs): boolean {
    ...
    return e.action === 'enter' || e.action === 'altDownArrow'
      ? !cell.classList.contains('e-headercell')
      : true;
  }
...
export class SearchBox {
  ...
  public searchFocus(args: { target: HTMLInputElement }): void {
    ...
    if (
      args.target.classList.contains('e-input') &&
      args.target.classList.contains('e-search') &&
      args.target.value
    ) {
      const sIcon: HTMLElement =
        args.target.parentElement.querySelector('.e-sicon');
  ...
  protected searchBlur(args: Event & FocusEvent): void {
    const relatedTarget: EventTarget = args.relatedTarget
      ? args.relatedTarget
      : null;
    if (
      ...
      if ((<HTMLElement>relatedTarget).classList.contains('e-clear-icon')) {
        (<HTMLElement>args.target).parentElement.classList.remove(
          'e-input-focus'
        );
      }
    } else {
      (<HTMLInputElement>args.target).parentElement.classList.remove(
        'e-input-focus'
      );
    }
    if (
      (<HTMLElement>args.target).classList.contains('e-search') &&
      ((relatedTarget &&
        !(<HTMLElement>relatedTarget).classList.contains(
          'e-sicon e-clear-icon'
        ) &&
        !(<HTMLElement>relatedTarget).classList.contains('e-sicon')) ||
        isNullOrUndefined(relatedTarget))
    ) {
      const sIcon: HTMLInputElement = (<HTMLInputElement>(
        args.target
      )).parentElement.querySelector('.e-sicon');
