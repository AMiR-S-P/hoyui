...
export class ValueFormatter implements IValueFormatter {
  ...
  public getFormatFunction(
    format: NumberFormatOptions | DateFormatOptions
  ): Function {
    if (
      !isNullOrUndefined(<DateFormatOptions>format) &&
      ((<DateFormatOptions>format).type === 'dateTime' ||
        (<DateFormatOptions>format).type === 'datetime' ||
        (<DateFormatOptions>format).type === 'date' ||
        (<DateFormatOptions>format).type === 'time')
    ) {
  ...
  public fromView(
    ...
    if (
      (type === 'date' || type === 'datetime' || type === 'number') &&
