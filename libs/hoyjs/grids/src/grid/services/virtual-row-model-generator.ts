import { Browser, isNullOrUndefined } from '@hoyui/base';
import { Group } from '@hoyui/data';
import {
  IModelGenerator,
  IGrid,
  VirtualInfo,
...
export class VirtualRowModelGenerator implements IModelGenerator<Column> {
  ...
  constructor(parent: IGrid) {
    ...
    this.rowModelGenerator = this.parent.allowGrouping
  ...
  private columnInfiniteRows(data: Object[], e?: NotifyArgs): Row<Column>[] {
    ...
    if (e.requestType === 'virtualscroll') {
      ...
      if (this.parent.infiniteScrollSettings.enableCache) {
        const currentRowStartIndex: number =
          this.parent.frozenRows && this.parent.pageSettings.currentPage === 1
            ? 0
            : getRowIndexFromElement(
                this.parent
                  .getContentTable()
                  .querySelector('.e-row:not(.e-addedrow)')
              );
        let newResult: Row<Column>[] = result.slice(
          currentRowStartIndex,
          currentRowStartIndex + this.parent.pageSettings.pageSize * 3
        );
        if (
          ...
          newResult = [
            ...result.slice(0, this.parent.frozenRows),
            ...newResult,
          ];
        }
        result = newResult;
  ...
  public generateRows(data: Object[], e?: NotifyArgs): Row<Column>[] {
    ...
    const info: VirtualInfo = (e.virtualInfo =
      e.virtualInfo ||
      ((e.requestType === 'sorting' || e.requestType === 'delete') &&
    ...
    const page: number =
      !xAxis && info.loadNext && !info.loadSelf
        ? info.nextInfo.page
        : info.page;
    let result: Row<Column>[] = [];
    let indexes: number[] = this.getBlockIndexes(page);
    const loadedBlocks: number[] = [];
    if (this.currentInfo.blockIndexes) {
      indexes =
        info.blockIndexes =
        e.virtualInfo.blockIndexes =
    ...
    if (isGroupAdaptive(this.parent) && this.parent.vcRows.length) {
      const dataRows: Row<Column>[] = this.parent.vcRows.filter(
        (row: Row<Column>) => row.isDataRow
      );
      if (
        (this.parent.isManualRefresh &&
          dataRows.length === data['records'].length) ||
        !this.parent.isManualRefresh
      ) {
    ...
    if (this.parent.enableColumnVirtualization) {
      for (let i: number = 0; i < info.blockIndexes.length; i++) {
        if (
          this.isBlockAvailable(info.blockIndexes[parseInt(i.toString(), 10)])
        ) {
          this.cache[info.blockIndexes[parseInt(i.toString(), 10)]] =
            this.rowModelGenerator.refreshRows(
              this.cache[info.blockIndexes[parseInt(i.toString(), 10)]]
            );
        }
      }
    ...
    for (let i: number = 0; i < values.length; i++) {
      if (!this.isBlockAvailable(values[parseInt(i.toString(), 10)])) {
        let startIdx: number = !isNullOrUndefined(this.startIndex)
          ? this.startIndex
        ...
        if (isGroupAdaptive(this.parent)) {
          ...
          if (isManualRefresh) {
            ...
            for (let i: number = 0; i < visiblePage.length; i++) {
              const indexes: number[] = this.getBlockIndexes(
                visiblePage[parseInt(i.toString(), 10)]
              );
              ...
              if (
                ...
                const dataRowObject: Row<Column>[] = rows
                  .filter((row: Row<Column>) => row.isDataRow)
                  .slice(startIndex, endIndex);
                startIndex = prevEndIndex;
                endIndex =
                  rows.indexOf(dataRowObject[dataRowObject.length - 1]) + 1;
              }
              const pageRecord: Row<Column>[] = rows.slice(
                startIndex,
                endIndex
              );
              const median: number =
                ~~Math.max(pageRecord.length, this.model.pageSize) / 2;
      ...
      if (
        ...
        this.cache[values[parseInt(i.toString(), 10)]] = this.updateGroupRow(
          this.cache[values[parseInt(i.toString(), 10)]],
          values[parseInt(i.toString(), 10)]
        );
      }
      if (
        !e.renderMovableContent &&
    ...
    if (this.parent.allowGrouping && this.parent.groupSettings.columns.length) {
      this.parent.currentViewData[`${grouping}`] = result.map(
        (m: Row<Column>) => m.data
      );
    } else {
    ...
    if (e.requestType === 'grouping') {
      this.parent.currentViewData[`${grouping}`] = this.parent.currentViewData[
        `${grouping}`
      ].filter(
  ...
  private getStartIndex(
    ...
    const page: number = this.getPage(blk);
    const even: boolean = blk % 2 === 0;
    const index: number = (page - 1) * this.model.pageSize;
    return full || !even ? index : index + ~~(this.model.pageSize / 2);
  }

  public getColumnIndexes(
    content: HTMLElement = <HTMLElement>(
      this.parent.getHeaderContent().querySelector('.' + literals.headerContent)
    )
  ): number[] {
    const indexes: number[] = [];
    let sLeft: number = content.scrollLeft | 0;
    const keys: string[] = Object.keys(this.cOffsets);
    const cWidth: number = content.getBoundingClientRect().width;
    ...
    if (this.parent.isFrozenGrid()) {
      ...
      if (diffWidth > 0) {
        for (
          let i: number = this.parent.getVisibleFrozenLeftCount() - 1;
          i >= 0;
          i--
        ) {
          if (diffWidth <= this.cOffsets[parseInt(i.toString(), 10)]) {
            frzLeftWidth =
              frzLeftWidth - this.cOffsets[parseInt(i.toString(), 10)];
            break;
          }
        }
      }
    ...
    keys.some((offset: string) => {
      const iOffset: number = Number(offset);
      const offsetVal: number = this.cOffsets[`${offset}`];
      const border: boolean =
        diffWidth + frzLeftWidth <= offsetVal && left + calWidth >= offsetVal;
      if (border) {
  ...
  public refreshColOffsets(): void {
    let col: number = 0;
    this.cOffsets = {};
    const gLen: number = this.parent.groupSettings.columns.length;
    const cols: Column[] = <Column[]>this.parent.columns;
    const cLen: number = cols.length;
    const isVisible: Function = (column: Column) =>
      column.visible &&
      (!this.parent.groupSettings.showGroupedColumn
        ? this.parent.groupSettings.columns.indexOf(column.field) < 0
        : column.visible);
    const c: string[] = this.parent.groupSettings.columns;
    for (let i: number = 0; i < c.length; i++) {
      this.cOffsets[parseInt(i.toString(), 10)] =
        (this.cOffsets[i - 1] | 0) + 30;
    }
    // eslint-disable-next-line prefer-spread
    const blocks: number[] = Array.apply(null, Array(cLen)).map(() => col++);
    for (let j: number = 0; j < blocks.length; j++) {
      blocks[parseInt(j.toString(), 10)] =
        blocks[parseInt(j.toString(), 10)] + gLen;
      this.cOffsets[blocks[parseInt(j.toString(), 10)]] =
        (this.cOffsets[blocks[parseInt(j.toString(), 10)] - 1] | 0) +
        (isVisible(cols[parseInt(j.toString(), 10)])
          ? parseInt(<string>cols[parseInt(j.toString(), 10)].width, 10)
          : 0);
    }
  }
  ...
  private iterateGroup(
    ...
    const isPresent: boolean = current.some((row: Row<Column>) => {
      return rows.some((oRow: Row<Column>, index: number) => {
        const res: boolean =
          oRow &&
          (<Group>oRow.data).field !== undefined &&
  ...
  private getGroupVirtualRecordsByIndex(rows: Row<Column>[]): void {
    ...
    for (let i: number = 1; i <= blocks; i++) {
      ...
      for (let j: number = (i - 1) * blockSize; j < rows.length; j++) {
        ...
        this.cache[parseInt(i.toString(), 10)].push(
