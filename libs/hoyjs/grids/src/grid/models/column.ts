import { merge, isNullOrUndefined, extend, Property } from '@hoyui/base';
import { NumberFormatOptions, DateFormatOptions } from '@hoyui/base';
...
import { DropDownListModel } from '@hoyui/dropdowns';
...
export class Column {
  ...
  constructor(options: ColumnModel, parent?: IGrid) {
    ...
    if (this.type === 'none') {
      ...
      this.type =
        typeof this.type === 'string' ? this.type.toLowerCase() : undefined;
    }
    if (this.editType) {
    ...
    if (
      options.format &&
      ((<DateFormatOptions>options.format).skeleton ||
        ((<DateFormatOptions>options.format).format &&
          typeof (<DateFormatOptions>options.format).format === 'string'))
    ) {
      this.setFormatter(
        valueFormatter.getFormatFunction(
          extend({}, options.format as DateFormatOptions)
        )
      );
      this.setParser(
        valueFormatter.getParserFunction(options.format as DateFormatOptions)
      );
    }
    this.toJSON = () => {
      const col: object = {};
      ...
      for (let i: number = 0; i < keys.length; i++) {
        if (keys[parseInt(i.toString(), 10)] === 'columns') {
          ...
          for (
            let j: number = 0;
            j < this[keys[parseInt(i.toString(), 10)]].length;
            j++
          ) {
            col[keys[parseInt(i.toString(), 10)]].push(
              this[keys[parseInt(i.toString(), 10)]][
    ...
    if (this.template || this.commandsTemplate) {
      this.templateFn = templateCompiler(
        this.template || this.commandsTemplate
      );
    }
    ...
    if (
      this.isForeignColumn() &&
      (isNullOrUndefined(this.editType) ||
      ...
      if (
        this.edit.params &&
        (this.edit.params as DropDownListModel).dataSource
      ) {
    ...
    if (!this.sortComparer && this.isForeignColumn()) {
      this.sortComparer = (x: number | string, y: number | string) => {
        x = getObject(
          ...
          getForeignData(this, {}, <string>x)[0]
        );
        y = getObject(
          this.foreignKeyValue,
          getForeignData(this, {}, <string>y)[0]
        );
        return this.sortDirection === 'Descending'
          ? DataUtil.fnDescending(x, y)
          : DataUtil.fnAscending(x, y);
      };
    }
  }
    ...
    for (let i: number = 0; i < keys.length; i++) {
      if (keys[parseInt(i.toString(), 10)] === 'columns') {
        ...
        for (let j: number = 0; j < cols.length; j++) {
          (
            (this.columns as Column[]).find((col: Column) => {
              return col.field === cols[parseInt(j.toString(), 10)].field;
            }) as Column
          ).setProperties(cols[parseInt(j.toString(), 10)]);
        }
      } else {
        this[keys[parseInt(i.toString(), 10)]] =
          column[keys[parseInt(i.toString(), 10)]];
      }
      ...
      if (this.parent && this.parent.isReact) {
        if (keys[parseInt(i.toString(), 10)] === 'template') {
          ...
          this.headerTemplateFn = templateCompiler(
            column[keys[parseInt(i.toString(), 10)]]
          );
