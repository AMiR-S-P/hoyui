...
export class DialogEditRender {
  ...
  private createDialogHeader(args: ResponsiveDialogArgs): Element | string {
    ...
    if (this.parent.enableAdaptiveUI) {
      const responsiveDlgRenderer: ResponsiveDialogRenderer =
        new ResponsiveDialogRenderer(this.parent, this.serviceLocator);
      responsiveDlgRenderer.action = this.isEdit
        ? ResponsiveDialogAction.isEdit
        : ResponsiveDialogAction.isAdd;
      ...
      if (gObj.editSettings.headerTemplate) {
        header = initializeCSPTemplate((): string => {
          return this.getDialogEditTemplateElement('HeaderTemplate', args)
            .outerHTML;
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
        }) as any;
      } else if (this.isEdit) {
        header =
          this.l10n.getConstant('EditFormTitle') + args.primaryKeyValue[0];
      } else {
      ...
      extend(
        {
          ...
          cssClass: this.parent.cssClass
            ? 'e-edit-dialog' + ' ' + this.parent.cssClass
            : 'e-edit-dialog',
          ...
          footerTemplate: gObj.editSettings.footerTemplate
            ? initializeCSPTemplate((): string => {
                return this.getDialogEditTemplateElement('FooterTemplate', args)
                  .outerHTML;
              })
            : null,
          buttons: [
  ...
  private btnClick(e: MouseEvent): void {
    if (
      this.l10n.getConstant('CancelButton').toLowerCase() ===
      (e.target as HTMLInputElement).innerText.trim().toLowerCase()
    ) {
      this.dialogClose();
    } else {
      this.parent.endEdit();
  ...
  private destroy(args?: { requestType: string }): void {
    const dialogEditTemplates: string[] = [
      'template',
      'headerTemplate',
      'footerTemplate',
    ];
    for (let i: number = 0; i < dialogEditTemplates.length; i++) {
      if (
        this.parent.editSettings[
          dialogEditTemplates[parseInt(i.toString(), 10)]
        ]
      ) {
        ...
        const editTemplateID: string =
          this.parent.element.id + 'editSettings' + templateName;
        updateBlazorTemplate(
          editTemplateID,
          templateName,
          this.parent.editSettings
  ...
  private getDialogEditTemplateElement(
    dialogTemp: string,
    args: { rowData?: Object; form?: Element }
  ): Element {
    const tempDiv: Element = this.parent.createElement('div', {
      className: 'e-dialog' + dialogTemp,
    });
    const dummyData: Object = extend(
      {},
      args.rowData,
      { isAdd: !this.isEdit },
      true
    ...
    const templateID: string =
      this.parent.element.id + 'editSettings' + dialogTemp;
    appendChildren(
      tempDiv,
      (dialogTemp === 'HeaderTemplate'
        ? this.parent.getEditHeaderTemplate()
        : this.parent.getEditFooterTemplate())(
        dummyData,
        this.parent,
        'editSettings' + dialogTemp,
        templateID
      )
    );
  ...
  private getEditElement(
    ...
    const form: HTMLFormElement = (args.form = this.parent.createElement(
      'form',
      { id: gObj.element.id + 'EditForm', className: 'e-gridform' }
    ) as HTMLFormElement);
    ...
    const table: Element = this.parent.createElement('table', {
      ...
      attrs: { cellspacing: '6px', role: 'grid' },
    });
    ...
    for (let i: number = 0; i < cols.length; i++) {
      if (
        this.parent.editModule.checkColumnIsGrouped(
          cols[parseInt(i.toString(), 10)]
        ) ||
        cols[parseInt(i.toString(), 10)].commands ||
        cols[parseInt(i.toString(), 10)].commandsTemplate ||
        cols[parseInt(i.toString(), 10)].type === 'checkbox'
      ) {
        continue;
      }
      ...
      elements[cols[parseInt(i.toString(), 10)].uid].classList.remove(
        'e-input'
      );
