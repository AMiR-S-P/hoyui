...
import { Switch, ChangeEventArgs } from '@hoyui/buttons';
...
export class ToggleEditCell extends EditCellBase implements IEditCell {
  ...
  public write(args: {
    ...
    const chkBoxElement: Element =
      !isNullOrUndefined(args.row) &&
      (args.row.querySelector('.e-edit-checkselect') as Element);
    ...
    if (!isNullOrUndefined(chkBoxElement)) {
      ...
      if (args.requestType !== 'add') {
        const row: Row<Column> = this.parent.getRowObjectFromUID(
          args.row.getAttribute('data-uid')
        );
        checkState = row ? row.isSelected : false;
      }
      addRemoveActiveClasses(
        [].slice.call(args.row.getElementsByClassName(literals.rowCell)),
        checkState,
        ...this.activeClasses
      );
    }
  ...
  private switchModeChange(args: ChangeEventArgs): void {
    if (this.editRow && this.editType !== 'Dialog') {
      ...
      addRemoveActiveClasses(
        [].slice.call(this.editRow.getElementsByClassName(literals.rowCell)),
        addClass,
        ...this.activeClasses
      );
    }
