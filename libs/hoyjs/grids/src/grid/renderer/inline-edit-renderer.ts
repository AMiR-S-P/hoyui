...
import { isNullOrUndefined, addClass, extend } from '@hoyui/base';
import {
  appendChildren,
...
export class InlineEditRender {
  ...
  public addNew(
    elements: Object,
    args: { row?: Element; rowData?: Object; isScroll?: boolean }
  ): void {
    this.isEdit = false;
    let tbody: Element;
    if (
    ...
    if (
      this.parent.getContentTable().querySelector('.e-emptyrow') &&
      !this.parent.editSettings.showAddNewRow
    ) {
      const emptyRow: Element = this.parent
        .getContentTable()
        .querySelector('.e-emptyrow');
      emptyRow.parentNode.removeChild(emptyRow);
      if (
        this.parent.frozenRows &&
        this.parent.element.querySelector('.e-frozenrow-empty')
      ) {
        this.parent.element
    ...
    args.row.appendChild(
      this.getEditElement(elements, false, undefined, args, true)
    );
  ...
  public update(
    elements: Object,
    args: { row?: Element; rowData?: Object }
  ): void {
    this.isEdit = true;
    const tdElement: HTMLElement[] = [].slice.call(
      args.row.querySelectorAll('td.e-rowcell')
    );
    args.row.innerHTML = '';
    args.row.appendChild(
      this.getEditElement(elements, true, tdElement, args, true)
    );
  ...
  private getEditElement(
    ...
    const td: HTMLTableCellElement = this.parent.createElement('td', {
      ...
      attrs: {
        colspan: (
          gObj.getCurrentVisibleColumns(this.parent.enableColumnVirtualization)
            .length + this.parent.getIndentCount()
        ).toString(),
      },
    }) as HTMLTableCellElement;
    const form: HTMLFormElement = ((<{ form: HTMLFormElement }>args).form =
      this.parent.createElement('form', {
    ...
    const table: Element = this.parent.createElement('table', {
      ...
      attrs: { cellspacing: '0.25', role: 'grid' },
    });
    table.appendChild(
      gObj.getContentTable().querySelector(literals.colGroup).cloneNode(true)
    );
    ...
    if (isDetail) {
      tr.insertBefore(
        this.parent.createElement('td', { className: 'e-detailrowcollapse' }),
        tr.firstChild
      );
    }
    if (gObj.isRowDragable()) {
      tr.appendChild(
        this.parent.createElement('td', { className: 'e-dragindentcell' })
      );
    }
    while (i < gLen) {
      tr.appendChild(
    ...
    const cols: Column[] = args.isCustomFormValidation
      ? (<{ columnModel?: Column[] }>this.parent).columnModel
      : gObj.getColumns();
    while (
      (isEdit && m < tdElement.length && i < cols.length) ||
      i < cols.length
    ) {
      const span: string =
        isEdit && tdElement[parseInt(m.toString(), 10)]
      ...
      if (this.parent.isFrozenGrid()) {
        ...
        if (this.parent.isSpan) {
          const colSpan: number = td.getAttribute('colspan')
            ? parseInt(td.getAttribute('colspan'), 10)
            : 1;
        ...
        if (this.parent.enableColumnVirtualization) {
          if (
            col.freeze === 'Left' &&
            !isNullOrUndefined((<{ valueX?: number }>col).valueX)
          ) {
            td.style.left =
              (<{ valueX?: number }>col).valueX - this.parent.translateX + 'px';
          } else if (
            col.freeze === 'Right' &&
            !isNullOrUndefined((<{ valueX?: number }>col).valueX)
          ) {
      ...
      td.setAttribute(
        'aria-label',
        inputValue +
          this.parent.localeObj.getConstant('ColumnHeader') +
          col.headerText
      );
      tr.appendChild(td);
  ...
  private appendChildren(form: Element, data: Object, isFrozen: boolean): void {
    const dummyData: Object = extend(
      {},
      data,
      { isAdd: !this.isEdit, isFrozen: isFrozen },
      true
    );
      ...
      for (let i: number = 0; i < cols.length; i++) {
        if ((cols[parseInt(i.toString(), 10)] as Column).validationRules) {
          this.parent.editModule.formObj.rules[
            (cols[parseInt(i.toString(), 10)] as Column).field
          ] = (cols[parseInt(i.toString(), 10)] as Column).validationRules as {
            [rule: string]: Object;
          };
        }
      }
