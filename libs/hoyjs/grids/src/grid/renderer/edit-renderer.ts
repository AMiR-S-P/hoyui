...
import { isNullOrUndefined, closest, extend } from '@hoyui/base';
...
import { attributes, classList, select } from '@hoyui/base';
...
export class EditRender {
  ...
  constructor(parent?: IGrid, serviceLocator?: ServiceLocator) {
    ...
    this.renderer = new this.editType[this.parent.editSettings.mode](
      parent,
      serviceLocator
    );
  ...
  public update(args: Object): void {
    ...
    const isCustomFormValidation: boolean = (<
      { isCustomFormValidation?: boolean }
    >args).isCustomFormValidation;
  ...
  private convertWidget(args: {
    ...
    const cols: Column[] =
      gObj.editSettings.mode !== 'Batch'
        ? (gObj.getColumns() as Column[])
        : [gObj.getColumnByField(args.columnName)];
    for (const col of cols) {
      if (this.parent.editSettings.template && !isNullOrUndefined(col.field)) {
        const cellArgs: Object = extend({}, args);
        (<{ element: Element }>cellArgs).element = form.querySelector(
      ...
      value = (col.valueAccessor as Function)(
        col.field,
        args.rowData,
        col
      ) as string;
      ...
      if (!isNullOrUndefined(cell)) {
        if (typeof temp === 'string') {
          ...
          temp({
            ...
            foreignKeyData:
              col.isForeignColumn() &&
              getObject(col.field, args.foreignKeyData),
          });
        } else {
        ...
        if (
          !isFocused &&
          isNullOrUndefined(cell.getAttribute('disabled')) &&
          !parentsUntil(cell, 'e-checkbox-disabled')
        ) {
  ...
  private focusElement(elem: HTMLInputElement, type?: string): void {
    const chkBox: HTMLInputElement = this.parent.element.querySelector(
      '.e-edit-checkselect'
    ) as HTMLInputElement;
    ...
    if (this.parent.editSettings.mode === 'Batch') {
      ...
      const isFocus: boolean =
        ...
        this.parent.editSettings.mode === 'Normal'
          ? false
          : true;
      const focusElement: HTMLElement = elem.classList.contains(
        'e-dropdownlist'
      )
        ? elem.parentElement
        : elem;
    ...
    for (let i: number = 0, len: number = cols.length; i < len; i++) {
      ...
      if (col.commands || col.commandsTemplate) {
        ...
        const cellRenderer: ICellRenderer<{}> =
          cellRendererFact.getCellRenderer(CellType.CommandColumn);
        const cells: Cell<Column>[] = model.generateRows(args.rowData)[0].cells;
        const cell: Cell<Column>[] = cells.filter(
          (cell: Cell<Column>) => cell.rowID
        );
        const td: Element = cellRenderer.render(
          cell[parseInt(i.toString(), 10)],
          args.rowData,
          <{ [x: string]: string }>{
            index: args.row ? args.row.getAttribute(literals.dataRowIndex) : 0,
          },
          this.parent.enableVirtualization
        );
      ...
      if (col.type === 'dateonly' && args.rowData[col.field] instanceof Date) {
        ...
        args.rowData[col.field] =
          cellValue.getFullYear() +
          '-' +
          padZero(cellValue.getMonth() + 1) +
          '-' +
          padZero(cellValue.getDate());
      }
      const value: string = (col.valueAccessor as Function)(
        col.field,
        args.rowData,
        col
      ) as string;
      ...
      if (col.editTemplate) {
        input = this.parent.createElement('span', {
          attrs: { 'e-mappinguid': col.uid },
        });
        const tempID: string =
          this.parent.element.id + col.uid + 'editTemplate';
        const tempData: object = extendObjWithFn({}, args.rowData, {
          column: col,
        });
        const isReactCompiler: boolean =
          this.parent.isReact && typeof col.editTemplate !== 'string';
        ...
        if (isReactCompiler || isReactChild) {
          col.getEditTemplate()(
            extend({ index: args.rowIndex }, tempData),
            this.parent,
            'editTemplate',
            tempID,
            null,
        ...
        const isInput: number =
          input.tagName !== 'input' && input.querySelectorAll('input').length;
