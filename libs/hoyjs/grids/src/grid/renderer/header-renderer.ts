...
import { classList } from '@hoyui/base';
...
export class HeaderRender implements IRenderer {
  ...
  private helper: Function = (e: { sender: MouseEvent }) => {
    ...
    const parentEle: HTMLElement = parentsUntil(
      target,
      'e-headercell'
    ) as HTMLElement;
    if (
      !(gObj.allowReordering || gObj.allowGrouping) ||
      (!isNullOrUndefined(parentEle) &&
    ...
    const element: HTMLElement = target.classList.contains('e-headercell')
      ? (target as HTMLElement)
      : parentEle;
    if (
      !element ||
      (!gObj.allowReordering &&
        element.classList.contains('e-stackedheadercell'))
    ) {
      return false;
    }
    const height: number = element.offsetHeight;
    const headercelldiv: Element =
      element.querySelector('.e-headercelldiv') ||
      element.querySelector('.e-stackedheadercelldiv');
    let col: Column;
    if (headercelldiv) {
      if (element.querySelector('.e-stackedheadercelldiv')) {
        col = gObj.getStackedHeaderColumnByHeaderText(
          (headercelldiv as HTMLElement).innerText.trim(),
          <Column[]>gObj.columns
        );
      } else {
    ...
    if (col && !isNullOrUndefined(col.headerTemplate)) {
      if (!isNullOrUndefined(col.headerTemplate)) {
        ...
        const isReactCompiler: boolean =
          gObj.isReact && typeof col.headerTemplate !== 'string';
      ...
      if (
        !isNullOrUndefined(visualElement.firstChild) &&
        visualElement.firstChild.nodeType === 1
      ) {
  ...
  private drag: Function = (e: {
    ...
    if (target) {
      ...
      if (
        !closest ||
        closest.getAttribute('id') !== gObj.element.getAttribute('id')
      ) {
        classList(cloneElement, ['e-notallowedcur'], ['e-defaultcur']);
        if (gObj.allowReordering) {
          (
            gObj.element.querySelector('.e-reorderuparrow') as HTMLElement
          ).style.display = 'none';
          (
            gObj.element.querySelector('.e-reorderdownarrow') as HTMLElement
          ).style.display = 'none';
        }
  ...
  constructor(parent?: IGrid, serviceLocator?: ServiceLocator) {
    ...
    this.widthService =
      this.serviceLocator.getService<ColumnWidthService>('widthService');
  ...
  public renderPanel(): void {
    ...
    div = isRendered
      ? div
      : this.parent.createElement('div', { className: 'e-gridheader' });
    const innerDiv: Element = isRendered
      ? div.querySelector('.' + literals.headerContent)
  ...
  private createHeaderTable(): Element {
    ...
    const innerDiv: Element = <Element>(
  ...
  public createHeader(
    ...
    const table: Element = this.parent.createElement('table', {
      ...
      attrs: { cellspacing: '0.25px', role: 'presentation' },
    });
    const findHeaderRow: { thead: Element; rows: Row<Column>[] } =
      this.createHeaderContent(tableName);
    ...
    for (let i: number = 0, len: number = rows.length; i < len; i++) {
      for (
        let j: number = 0,
          len: number = rows[parseInt(i.toString(), 10)].cells.length;
        j < len;
        j++
      ) {
  ...
  private createHeaderContent(tableName?: freezeTable): {
    thead: Element;
    rows: Row<Column>[];
  } {
    const gObj: IGrid = this.parent;
    ...
    const colHeader: Element = this.parent.createElement('tr', {
    ...
    if (gObj.isRowDragable() && this.parent.getFrozenMode() === 'Right') {
      for (let i: number = 0, len: number = rows.length; i < len; i++) {
        rows[parseInt(i.toString(), 10)].cells.push(
          this.generateCell({} as Column, CellType.RowDragHIcon)
        );
      }
    }
    const headerTemplateColumn: Column[] = columns.filter((a: Column) => {
      return a.headerTemplate;
    });
    if (
      headerTemplateColumn.length &&
  ...
  private ensureColumns(rows: Row<Column>[]): Row<Column>[] {
    ...
    for (let i: number = 0, len: number = rows.length; i < len; i++) {
      if (gObj.allowGrouping) {
        for (
          let c: number = 0, len: number = gObj.groupSettings.columns.length;
          c < len;
          c++
        ) {
          if (
          ...
          rows[parseInt(i.toString(), 10)].cells.push(
            this.generateCell({} as Column, CellType.HeaderIndent)
          );
        }
      }
      if (gObj.detailTemplate || gObj.childGrid) {
        ...
        rows[parseInt(i.toString(), 10)].cells.push(
          this.generateCell(args as Column, CellType.DetailHeader)
        );
      }
      if (gObj.isRowDragable() && this.parent.getFrozenMode() !== 'Right') {
        rows[parseInt(i.toString(), 10)].cells.push(
  ...
  private getHeaderCells(
    ...
    const thead: Element =
      ...
      this.parent.getHeaderTable().querySelector('thead');
    const cols: Column[] = this.parent.enableColumnVirtualization
      ? this.parent.getColumns(this.parent.enablePersistence)
    ...
    if (this.parent.lockcolPositionCount) {
      for (
        let i: number = 0;
        i < (!isNullOrUndefined(cols) ? cols.length : 0);
        i++
      ) {
  ...
  private appendCells(
    ...
    isMovable: Element,
    tableName: freezeTable,
    isStackLastCol: boolean
  ): Row<Column>[] {
    const lastCol: string = isLastCol
      ? isStackLastCol
        ? 'e-laststackcell'
        : 'e-lastcell'
      : '';
    const isLockColumn: boolean =
      !this.parent.lockcolPositionCount ||
      (cols.lockColumn && !this.lockColsRendered) ||
      (!cols.lockColumn && this.lockColsRendered);
    if (!cols.columns) {
      if (isLockColumn) {
        rows[parseInt(index.toString(), 10)].cells.push(
          this.generateCell(
            cols,
            CellType.Header,
            this.colDepth - index,
            (isFirstObj ? '' : isFirstCol ? 'e-firstcell' : '') + lastCol,
            index,
            this.parent.getColumnIndexByUid(cols.uid)
          )
        );
      }
      if (this.parent.lockcolPositionCount) {
        if (
          this.frzIdx + this.notfrzIdx < this.parent.frozenColumns &&
          ((cols.lockColumn && !this.lockColsRendered) ||
            (!cols.lockColumn && this.lockColsRendered))
        ) {
          this.frzIdx++;
      ...
      if (colSpan) {
        const stackedLockColsCount: number = this.getStackedLockColsCount(
          cols,
          0
        );
        const isStackedLockColumn: boolean =
          this.parent.lockcolPositionCount === 0 ||
          (!this.lockColsRendered && stackedLockColsCount !== 0) ||
          (this.lockColsRendered && colSpan - stackedLockColsCount !== 0);
        if (isStackedLockColumn) {
          rows[parseInt(index.toString(), 10)].cells.push(
            new Cell<Column>(<{ [x: string]: Object }>{
      ...
      if (this.parent.lockcolPositionCount && !this.lockColsRendered) {
        for (let i: number = 0; i < cols.columns.length; i++) {
          rows = this.appendCells(
            (cols.columns as Column[])[parseInt(i.toString(), 10)],
            rows,
            index + 1,
            isFirstObj,
            i === 0,
            i === cols.columns.length - 1 && isLastCol,
            isMovable,
            tableName,
            false
          );
      ...
      if (this.lockColsRendered) {
        for (
          let i: number = 0, len: number = cols.columns.length;
          i < len;
          i++
        ) {
          isFirstObj = isFirstObj && i === 0;
          const isFirstCol: boolean = (this.isFirstCol =
            (cols.columns[parseInt(i.toString(), 10)] as Column).visible &&
            !isFirstObj);
  ...
  private getStackedLockColsCount(col: Column, lockColsCount: number): number {
    if (col.columns) {
      for (let i: number = 0; i < col.columns.length; i++) {
        lockColsCount = this.getStackedLockColsCount(
          col.columns[parseInt(i.toString(), 10)] as Column,
          lockColsCount
        );
      }
  ...
  private getColSpan(colSpan: number, stackedLockColsCount: number): number {
    colSpan = !this.lockColsRendered
      ? stackedLockColsCount
      : colSpan - stackedLockColsCount;
    return colSpan;
  }
  ...
  public setVisible(columns?: Column[]): void {
    ...
    for (let c: number = 0, clen: number = columns.length; c < clen; c++) {
      ...
      setStyleAttribute(
        <HTMLElement>this.getColGroup().children[parseInt(idx.toString(), 10)],
        { display: displayVal }
      );
      if (
        gObj.editSettings.showAddNewRow &&
        gObj.element.querySelector('.e-addedrow')
      ) {
        setStyleAttribute(
          <HTMLElement>(
            gObj.element.querySelector('.e-addedrow').querySelector('colgroup')
              .childNodes[parseInt(idx.toString(), 10)]
          ),
          { display: displayVal }
        );
      }
  ...
  public refreshUI(): void {
    ...
    if (table) {
      ...
      if (
        this.parent.editSettings.showAddNewRow &&
        !this.parent.isAddNewRow &&
        table.querySelector('.e-addedrow') &&
        (this.parent.enableVirtualization ||
          this.parent.enableInfiniteScrolling)
      ) {
      ...
      const rows: Element[] = [].slice.call(
        headerDiv.querySelectorAll('tr.e-columnheader')
      );
      for (const row of rows) {
        const gCells: Element[] = [].slice.call(
          row.getElementsByClassName('e-grouptopleftcell')
        );
        if (gCells.length) {
      ...
      if (
        this.parent.enableColumnVirtualization &&
        parentsUntil(table, literals.movableHeader)
      ) {
        this.parent.notify(events.headerRefreshed, {
          rows: this.rows,
          args: { isFrozen: false, isXaxis: true },
        });
      }
      if (
    ...
    const firstHeaderCell: HTMLElement = this.parent
      .getHeaderContent()
  ...
  public toggleStackClass(div: Element): void {
    ...
    const stackedHdr: boolean = !isNullOrUndefined(column)
      ? column.some((column: Column) => !isNullOrUndefined(column.columns))
      : false;
    if (stackedHdr) {
  ...
  protected initializeHeaderDrag(): void {
    ...
    if (
      !(
        ...
        (this.parent.allowGrouping && this.parent.groupSettings.showDropArea)
      )
    ) {
      return;
    }
  ...
  private renderCustomToolbar(): void {
    ...
    if (
      ...
      !gObj.toolbar &&
      (gObj.allowSorting ||
        (gObj.allowFiltering && gObj.filterSettings.type !== 'FilterBar'))
    ) {
      const div: HTMLElement = gObj.createElement('div', {
        className: 'e-res-toolbar e-toolbar',
      });
      const toolbarItems: HTMLElement = gObj.createElement('div', {
        className: 'e-toolbar-items',
      });
      const toolbarLeft: HTMLElement = gObj.createElement('div', {
        className: 'e-toolbar-left',
      });
      const count: number =
        this.parent.allowFiltering && this.parent.allowSorting ? 2 : 1;
      for (let i: number = 0; i < count; i++) {
        const toolbarItem: HTMLElement = gObj.createElement('div', {
          className:
            'e-toolbar-item e-gridresponsiveicons e-icons e-tbtn-align',
        });
        const cls: string =
          count === 1
        ...
        button.onclick = (e: MouseEvent) => {
          if (
            (e.target as HTMLElement).classList.contains('e-ressort-btn') ||
      ...
      gObj.element.insertBefore(
        div,
        this.parent.element.querySelector('.' + literals.gridHeader)
      );
    } else {
  ...
  private updateCustomResponsiveToolbar(args: { module: string }): void {
    const resToolbar: HTMLElement = this.parent.element.querySelector(
