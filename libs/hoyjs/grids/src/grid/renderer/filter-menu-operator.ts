...
export class FlMenuOptrUI {
  ...
  public renderOperatorUI(
    dlgConetntEle: Element,
    target: Element,
    column: Column,
    dlgObj: Dialog,
    operator?: { [key: string]: Object }[]
  ): void {
    this.dialogObj = dlgObj;
    const optr: string = column.type + 'Operator';
    this.optrData = this.customOptr = !isNullOrUndefined(operator)
      ? operator
      : !isNullOrUndefined(this.parent.filterSettings.operators) &&
        !isNullOrUndefined(this.parent.filterSettings.operators[`${optr}`])
      ? this.parent.filterSettings.operators[`${optr}`]
    ...
    const optrInput: Element = this.parent.createElement('input', {
    ...
    this.dropOptr = new DropDownList({
      ...
      change: function () {
        const valInput: HTMLInputElement = document
          .querySelector('.e-flmenu-valuediv')
          .querySelector('input');
        if (
          (this as DropDownList).value === 'isempty' ||
          (this as DropDownList).value === 'isnotempty' ||
          (this as DropDownList).value === 'isnotnull' ||
          (this as DropDownList).value === 'isnull'
        ) {
  ...
  private dropSelectedVal(col: Column, optr: string): string {
    ...
    for (const column of columns) {
      if (
        ...
        (col.isForeignColumn() && col.foreignKeyValue === column.field)
      ) {
        const selectedField: Object = new DataManager(
          this.optrData
        ).executeLocal(new Query().where('value', 'equal', column.operator));
        selValue = !isNullOrUndefined(selectedField[0])
          ? selectedField[0].text
          : '';
      }
    }
    if (selValue === '') {
      ...
      if (col.filter.operator) {
        ...
        for (let i: number = 0; i < optrLen; i++) {
          if (
            this.optrData[parseInt(i.toString(), 10)].value ===
            col.filter.operator
          ) {
