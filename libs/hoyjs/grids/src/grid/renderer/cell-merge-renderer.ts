import { isNullOrUndefined, attributes } from '@hoyui/base';
...
export class CellMergeRender<T> {
  ...
  public render(
    ...
    let colSpan: number = row.cells[parseInt(i.toString(), 10)].cellSpan
      ? row.cells[parseInt(i.toString(), 10)].cellSpan
      : cellArgs.colSpan + i <= row.cells.length
      ? cellArgs.colSpan
      : row.cells.length - i;
    ...
    if (row.index > 0) {
      ...
      spannedCell = cells.filter(
        (cell: Cell<Column>) => cell.column.uid === targetCell.column[`${uid}`]
      )[0];
    }
    const colSpanLen: number =
      spannedCell &&
      spannedCell.colSpanRange > 1 &&
      spannedCell.rowSpanRange > 1
        ? spannedCell.colSpanRange
        : colSpan;
    for (
      let j: number = i + 1;
      j < i + colSpanLen && j < row.cells.length;
      j++
    ) {
    ...
    if (visible > 0) {
      for (
        let j: number = i + colSpan;
        j < i + colSpan + visible && j < row.cells.length;
        j++
      ) {
        row.cells[parseInt(j.toString(), 10)].isSpanned = true;
      }
    ...
    if (row.cells[parseInt(i.toString(), 10)].cellSpan) {
      row.data[cellArgs.column.field] =
        row.cells[parseInt(i.toString(), 10)].spanText;
    ...
    if (row.index > 0 && spannedCell.rowSpanRange > 1) {
      ...
      row.cells[parseInt(i.toString(), 10)].rowSpanRange = Number(
        spannedCell.rowSpanRange - 1
      );
      row.cells[parseInt(i.toString(), 10)].colSpanRange =
        spannedCell.rowSpanRange > 0 ? spannedCell.colSpanRange : 1;
    }
    if (
      this.parent.enableColumnVirtualization &&
      !row.cells[parseInt(i.toString(), 10)].cellSpan &&
      !this.containsKey(
        cellArgs.column.field,
        cellArgs.data[cellArgs.column.field]
      )
    ) {
      this.backupMergeCells(
        cellArgs.column.field,
        cellArgs.data[cellArgs.column.field],
        cellArgs.colSpan
      );
  ...
  private getIndexFromAllColumns(field: string): number {
    const index: number = iterateArrayOrObject<number, Column>(
      <Column[]>this.parent.getVisibleColumns(),
