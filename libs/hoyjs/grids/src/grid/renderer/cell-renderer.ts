import { L10n, remove, addClass } from '@hoyui/base';
...
export class CellRenderer implements ICellRenderer<Column> {
  ...
  public evaluate(
    ...
    if (cell.column.template) {
      const isReactCompiler: boolean =
        this.parent.isReact &&
        typeof cell.column.template !== 'string' &&
        cell.column.template.prototype &&
        !cell.column.template.prototype.CSPTemplate;
      ...
      if (isReactCompiler || isReactChild) {
        ...
        result = cell.column.getColumnTemplate()(
          extend({ index: attributes[literals[0]] }, dummyData),
          this.parent,
          'template',
          templateID,
          this.parent[`${str}`],
      ...
      node.setAttribute(
        ...
        (<HTMLElement>node).innerText +
          this.localizer.getConstant('TemplateCell') +
          this.localizer.getConstant('ColumnHeader') +
  ...
  public invokeFormatter(column: Column, value: Object, data: Object): Object {
    if (!isNullOrUndefined(column.formatter)) {
      if (doesImplementInterface(column.formatter, 'getValue')) {
        const formatter: { new (): ICellFormatter } = <
          { new (): ICellFormatter }
        >column.formatter;
  ...
  public refreshTD(
    ...
    const isEdit: boolean =
      ...
      td.classList.contains('e-editedbatchcell');
    if (this.parent.isReact) {
      const cellIndex: number = (td as HTMLTableCellElement).cellIndex;
      const parentRow: HTMLTableRowElement =
      ...
      if (parentRow.cells.length !== cellIndex - 1) {
        parentRow.insertBefore(
          newTD,
          parentRow.cells[parseInt(cellIndex.toString(), 10)]
        );
      } else {
  ...
  private refreshCell(
    ...
    let value: Object = cell.isForeignKey
      ? this.getValue(column.foreignKeyValue, fData, column)
    ...
    innerHtml = !isNullOrUndefined(column.formatter)
      ? isNullOrUndefined(fromFormatter)
        ? ''
        : fromFormatter.toString()
      : innerHtml;
    if (
      this.evaluate(node, cell, data, attributes, fData, isEdit) &&
      column.type !== 'checkbox'
    ) {
      this.appendHtml(
        node,
        this.parent.sanitize(innerHtml),
        column.getDomSetter ? column.getDomSetter() : 'innerHTML'
      );
      ...
      const checkWrap: Element = createCheckBox(
        this.parent.createElement,
        false,
        { checked: value as boolean, label: ' ' }
      );
    ...
    if (column.type === 'boolean' && column.displayAsCheckBox) {
      const checked: boolean = isNaN(parseInt(value.toString(), 10))
        ? value === 'true'
    ...
    if (
      node.classList.contains('e-summarycell') &&
      !(<{ key?: string }>data).key
    ) {
  ...
  public setAttributes(
    ...
    if (
      ...
      (!column.clipMode &&
        this.parent.clipMode === 'EllipsisWithTooltip' &&
        !(
          this.parent.allowTextWrap &&
          (this.parent.textWrapSettings.wrapMode === 'Content' ||
  ...
  public buildAttributeFromCell<Column>(
    node: HTMLElement,
    cell: Cell<Column>,
