import { isNullOrUndefined, extend, addClass, removeClass } from '@hoyui/base';
import { attributes as addAttributes } from '@hoyui/base';
...
export class RowRenderer<T> implements IRowRenderer<T> {
  ...
  public refresh(
    ...
    const node: Element = this.parent.element.querySelector(
      '[data-uid=' + row.uid + ']'
    );
    const tr: Element = this.refreshRow(
      row,
      columns,
      attributes,
      rowTemplate,
    ...
    const cells: HTMLTableDataCellElement[] = [].slice.call(
      (tr as HTMLTableRowElement).cells
    );
    const tempCells: HTMLTableDataCellElement[] = [].slice.call(
      node.querySelectorAll('.e-templatecell')
    );
  ...
  private refreshRow(
    ...
    const tr: Element = !isNullOrUndefined(cloneNode)
      ? cloneNode
      : (this.element.cloneNode() as Element);
    const rowArgs: RowDataBoundEventArgs = { data: row.data };
    const cellArgs: QueryCellInfoEventArgs = { data: row.data };
    const chekBoxEnable: Column = this.parent
      .getColumns()
      .filter((col: Column) => col.type === 'checkbox' && col.field)[0];
    ...
    const cellRendererFact: CellRendererFactory =
      this.serviceLocator.getService<CellRendererFactory>(
        'cellRendererFactory'
      );
    for (let i: number = 0, len: number = row.cells.length; i < len; i++) {
      const cell: Cell<T> = row.cells[parseInt(i.toString(), 10)];
      cell.isSelected = row.isSelected;
      cell.isColumnSelected = (<{ isSelected?: boolean }>(
        cell.column
      )).isSelected;
      const cellRenderer: ICellRenderer<T> = cellRendererFact.getCellRenderer(
        row.cells[parseInt(i.toString(), 10)].cellType || CellType.Data
      );
      const attrs: {} = {
      ...
      if (
        ...
        attrs['class'] = this.parent.isPrinting
          ? 'e-detailrowcollapse'
          : 'e-detailrowexpand';
      }
      let td: Element = cellRenderer.render(
        row.cells[parseInt(i.toString(), 10)],
        row.data,
        attrs,
        row.isExpand,
        isEdit
      ...
      if (row.cells[parseInt(i.toString(), 10)].cellType !== CellType.Filter) {
        if (
          row.cells[parseInt(i.toString(), 10)].cellType === CellType.Data ||
          row.cells[parseInt(i.toString(), 10)].cellType ===
          ...
          if (
            ...
            thisRef.parent.renderTemplates(function (): void {
              if (
                typeof (<{ template?: string | Function }>cell.column)
                  .template !== 'string'
              ) {
                const ariaAttr: string = td.getAttribute('aria-label');
                td.setAttribute(
                  'aria-label',
                  (td as HTMLElement).innerText + ariaAttr
                );
              }
              thisRef.parent.trigger(
                queryCellInfo,
                extend(cellArgs, <QueryCellInfoEventArgs>{
                  cell: td,
                  column: <{}>cell.column,
                  colSpan: 1,
                  rowSpan: 1,
                  foreignKeyData:
                    row.cells[parseInt(i.toString(), 10)].foreignKeyData,
          ...
          if (
            row.index > 0 &&
            (this.isSpan || (this.parent.isSpan && isEdit))
          ) {
            const rowsObject: Row<Column>[] = this.parent.getRowsObject();
            const prevRowCells: Cell<Column>[] =
              this.parent.groupSettings.columns.length > 0 &&
              !rowsObject[row.index - 1].isDataRow
                ? rowsObject[row.index].cells
                : rowsObject[row.index - 1].cells;
            const uid: string = 'uid';
            const prevRowCell: Cell<Column> = prevRowCells.filter(
              (cell: Cell<Column>) =>
                cell.column.uid ===
                row.cells[parseInt(i.toString(), 10)].column[`${uid}`]
            )[0];
            isRowSpanned = prevRowCell.isRowSpanned
              ? prevRowCell.isRowSpanned
              : prevRowCell.rowSpanRange > 1;
          }
          if (cellArgs.rowSpan > 1 || cellArgs.colSpan > 1) {
            this.resetrowSpanvalue(
              this.parent.frozenRows > row.index
                ? this.parent.frozenRows
                : this.parent.currentViewData.length,
              cellArgs,
              row.index
            );
            if (cellArgs.column.visible === false) {
              ...
              if (isFrozen) {
                ...
                if (left > cellIdx && left < cellIdx + cellArgs.colSpan) {
                  ...
                  movableCount <= cellIdx &&
                  columns.length < cellIdx + cellArgs.colSpan
                ) {
                  const colSpan: number =
                    cellIdx + cellArgs.colSpan - columns.length;
                  cellArgs.colSpan = cellArgs.colSpan - colSpan;
                  ...
                  for (
                    ...
                    j < index + cellArgs.colSpan;
                    j++
                  ) {
                    if (
                      columns[parseInt(j.toString(), 10)].freeze === 'Fixed'
                    ) {
                      colSpan++;
                    } else {
                      break;
                  ...
                  movableCount > cellIdx &&
                  movableCount < cellIdx + cellArgs.colSpan
                ) {
                  const colSpan: number =
                    cellIdx + cellArgs.colSpan - movableCount;
                  cellArgs.colSpan = cellArgs.colSpan - colSpan;
                }
              }
            }
          ...
          if (
            cellArgs.colSpan > 1 ||
            row.cells[parseInt(i.toString(), 10)].cellSpan > 1 ||
            cellArgs.rowSpan > 1 ||
            isRowSpanned
          ) {
            this.parent.isSpan = true;
            this.isSpan = true;
            const cellMerge: CellMergeRender<T> = new CellMergeRender(
              this.serviceLocator,
              this.parent
            );
        ...
        if (this.isSpan) {
          ...
          if (rowsObject[row.index - 1]) {
            ...
            const currentRowCell: Cell<Column> =
              prevRowCells[parseInt(i.toString(), 10)];
            const nextRowCell: Cell<Column> = prevRowCells[i + 1];
            const direction: string =
              prevRowCells[parseInt(i.toString(), 10)].column.freeze;
            if (
              prevRowCell &&
              (prevRowCell.isRowSpanned || prevRowCell.rowSpanRange > 1) &&
              prevRowCell.visible
            ) {
              if (
                !isRtl &&
                (!currentRowCell.isRowSpanned ||
                  prevRowCell.rowSpanRange > currentRowCell.rowSpanRange)
              ) {
                td.classList.add('e-rowcell-firstchild');
              }
              if (isFrozen) {
                if (
                  prevRowCell.column.freeze === 'Fixed' &&
                  direction === 'Fixed'
                ) {
                  td.classList.add(
                    isRtl
                      ? 'e-removefreezerightborder'
                      : 'e-removefreezeleftborder'
                  );
            ...
            if (
              nextRowCell &&
              (nextRowCell.isRowSpanned || nextRowCell.rowSpanRange > 1) &&
              nextRowCell.visible
            ) {
              if (
                isRtl &&
                (!currentRowCell.isRowSpanned ||
                  nextRowCell.rowSpanRange > currentRowCell.rowSpanRange)
              ) {
                td.classList.add('e-rowcell-lastchild');
              }
              if (
                isFrozen &&
                nextRowCell.column.freeze === 'Fixed' &&
                direction === 'Fixed' &&
                cellArgs.colSpan < 2
              ) {
                td.classList.add(
        ...
        if (
          cellArgs.rowSpan > 1 &&
          this.parent.currentViewData.length - row.index === cellArgs.rowSpan
        ) {
    ...
    if (
        ...
        if (tr.querySelector('.e-summarycell')) {
          ...
          for (let j: number = 0; j < tr.childNodes.length; j++) {
            ...
            if (td.classList.contains('e-groupcaption')) {
              const oldColspan: number = parseInt(
      ...
      if (
        (tr.querySelectorAll('.e-leftfreeze').length ||
          tr.classList.contains('e-columnheader')) &&
        (tr.querySelectorAll('.e-indentcell').length ||
          tr.querySelectorAll('.e-grouptopleftcell').length)
      ) {
        const td: NodeListOf<Element> = tr.querySelectorAll(
    ...
    if (row.isDataRow) {
      const eventArg: RowDataBoundEventArgs = extend(rowArgs, args);
      eventArg.isSelectable = true;
      const isReactChild: boolean =
        this.parent.parentDetails &&
        this.parent.parentDetails.parentInstObj &&
        this.parent.parentDetails.parentInstObj.isReact;
      const cellTemplate: NodeListOf<Element> =
        eventArg.row.querySelectorAll('.e-templatecell');
      if (
        ((this.parent.isReact && this.parent.requireTemplateRef) ||
          (isReactChild &&
            this.parent.parentDetails.parentInstObj.requireTemplateRef)) &&
      ...
      if (
        ...
        const SelectedRecords: Object[] = eventArg.isSelectable
          ? this.parent.partialSelectedRecords
          : this.parent.disableSelectedRecords;
        if (
          !SelectedRecords.some(
            (data: Object) => data[`${primaryKey}`] === pKey
          )
        ) {
    ...
    if (rowArgs.rowHeight) {
      ...
      (tr.querySelector('.e-headercell') || tr.querySelector('.e-groupcaption'))
    ) {
    ...
    if (
      ...
      this.parent.element.scrollHeight > this.parent.height
    ) {
      for (let i: number = 0; i < this.parent.aggregates.length; i++) {
        const property: string = 'properties';
        const column: string = 'columns';
        if (
          this.parent.aggregates[parseInt(i.toString(), 10)][`${property}`][
            `${column}`
          ][0].footerTemplate
        ) {
          const summarycell: NodeList = [].slice.call(
            tr.getElementsByClassName('e-summarycell')
          );
          if (summarycell.length) {
            const lastSummaryCell: Element = summarycell[
              summarycell.length - 1
            ] as Element;
  ...
  private disableRowSelection(
    thisRef: RowRenderer<T>,
    row: Row<T>,
    args: RowDataBoundEventArgs,
    eventArg: RowDataBoundEventArgs
  ): void {
    const selIndex: number[] = this.parent.getSelectedRowIndexes();
    ...
    const chkBox: NodeList = args.row.querySelectorAll(
      '.e-rowcell.e-gridchkbox'
    );
    const isDrag: Element = eventArg.row.querySelector('.e-rowdragdrop');
    const cellIdx: number =
      thisRef.parent.groupSettings.columns.length +
      (isDrag || thisRef.parent.isDetail() ? 1 : 0);
    for (let i: number = 0; i < chkBox.length; i++) {
      (
        chkBox[parseInt(i.toString(), 10)] as HTMLElement
      ).firstElementChild.classList.add('e-checkbox-disabled');
      (chkBox[parseInt(i.toString(), 10)] as HTMLElement)
    ...
    if (row.cells.length) {
      for (let i: number = cellIdx; i < row.cells.length; i++) {
        const cell: Element = eventArg.row.querySelector(
          '.e-rowcell[data-colindex="' +
            row.cells[parseInt(i.toString(), 10)].index +
            '"]'
        );
        if (cell) {
