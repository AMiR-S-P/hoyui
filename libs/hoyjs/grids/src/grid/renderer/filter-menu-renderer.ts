import { isNullOrUndefined, getValue, L10n, remove } from '@hoyui/base';
...
export class FilterMenuRenderer {
  ...
  constructor(
    ...
    this.flMuiObj = new FlMenuOptrUI(
      this.parent,
  ...
  private openDialog(args: IFilterArgs): void {
    ...
    if (
      isNullOrUndefined(this.col.filter) ||
      isNullOrUndefined(this.col.filter.type) ||
      this.col.filter.type === 'Menu'
    ) {
      ///
  ...
  private closeDialog(target?: Element): void {
    ...
    if (this.dlgObj && !this.dlgObj.isDestroyed && elem) {
      ...
      if (
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (this.parent as any).clearTemplate(
          ['filterTemplate'],
          undefined,
          () => {
            this.dlgObj.destroy();
  ...
  private renderDlgContent(target: Element, column: Column): void {
    ...
    this.dlgDiv.setAttribute(
      'aria-label',
      this.l10n.getConstant('FilterMenuDialogARIA')
    );
    ...
    this.dlgObj = new Dialog({
      ...
      position: this.parent.element.classList.contains('e-device')
        ? { X: 'center', Y: 'center' }
        : { X: '', Y: '' },
      target: this.parent.element.classList.contains('e-device')
        ? document.body
        : this.parent.element,
      buttons: [
        {
          ...
          buttonModel: {
            ...
            cssClass: this.parent.cssClass
              ? 'e-flmenu-okbtn' + ' ' + this.parent.cssClass
              : 'e-flmenu-okbtn',
          },
        },
        {
          ...
          buttonModel: {
            ...
            cssClass: this.parent.cssClass
              ? 'e-flmenu-cancelbtn' + ' ' + this.parent.cssClass
              : 'e-flmenu-cancelbtn',
          },
        },
      ],
      content: mainDiv,
      width:
        !isNullOrUndefined(parentsUntil(target, 'e-bigger')) ||
        this.parent.element.classList.contains('e-device')
          ? 260
          : 250,
      animationSettings: { effect: 'None' },
      cssClass: this.parent.cssClass
        ? 'e-filter-popup' + ' ' + this.parent.cssClass
        : 'e-filter-popup',
    });
  ...
  private dialogCreated(target: Element, column: Column): void {
    ...
    if (
      !(
        ...
        !(
          !isNullOrUndefined(column.filter) &&
          !isNullOrUndefined(column.filter.ui) &&
    ...
    const isReactCompiler: boolean =
      this.parent.isReact && typeof column.filterTemplate !== 'string';
    const isReactChild: boolean =
      this.parent.parentDetails &&
      this.parent.parentDetails.parentInstObj &&
      this.parent.parentDetails.parentInstObj.isReact;
    if (
      !isNullOrUndefined(column.filterTemplate) &&
      !(isReactCompiler || isReactChild)
    ) {
      (
        this.dlgDiv.querySelector('.e-flmenu-valuediv')
          .firstElementChild as HTMLElement
      ).focus();
      this.dlgDiv
        .querySelector('.e-flmenu-valuediv')
        .firstElementChild.classList.add('e-input-focus');
    } else if (
      !isNullOrUndefined(this.dlgDiv.querySelector('.e-flmenu-input'))
    ) {
      (this.dlgDiv.querySelector('.e-flmenu-input') as HTMLElement).focus();
      this.dlgDiv
        .querySelector('.e-flmenu-input')
        .parentElement.classList.add('e-input-focus');
    }
  }
  ...
  public afterRenderFilterUI(): void {
    ...
    const optrInput: HTMLInputElement = this.dlgObj.element
      .querySelector('.e-flm_optrdiv')
      .querySelector('input');
    const valInput: HTMLInputElement = this.dlgObj.element
      .querySelector('.e-flmenu-valuediv')
      .querySelector('input');
    if (
      optrInput.value === 'Empty' ||
      optrInput.value === 'Not Empty' ||
      optrInput.value === 'Null' ||
      optrInput.value === 'Not Null'
    ) {
      ...
      !isNullOrUndefined(valInput && valInput.getAttribute('disabled'))
    ) {
      if (!isNullOrUndefined(valInput['ej2_instances'])) {
    ...
    if (!column.filterTemplate) {
      this.writeMethod(
        column,
        this.dlgObj.element.querySelector('#' + column.uid + '-flmenu')
      );
    }
    const args: Object = {
  ...
  private renderFilterUI(target: Element, col: Column): void {
    const dlgConetntEle: Element =
      this.dlgObj.element.querySelector('.e-flmenu-maindiv');
  ...
  private renderOperatorUI(
    dlgConetntEle: Element,
    target: Element,
    column: Column
  ): void {
    this.flMuiObj.renderOperatorUI(
      dlgConetntEle,
      target,
      column,
      this.dlgObj,
      this.filterObj.menuOperator
    );
  }
  ...
  private renderFlValueUI(
    ...
    if (column.filterTemplate) {
      ...
      fltrData[column.field] = fltrData[`${valueInString}`] = (<
        { values: Object }
      >fObj).values[column.field];
      if (column.foreignKeyValue) {
        fltrData[column.foreignKeyValue] = (<{ values: Object }>fObj).values[
          column.field
        ];
      ...
      const isReactChild: boolean =
        ...
        this.parent.parentDetails.parentInstObj.isReact;
      const tempID: string =
        this.parent.element.id + column.uid + 'filterTemplate';
      if (isReactCompiler || isReactChild) {
        column.getFilterTemplate()(
          fltrData,
          this.parent,
          'filterTemplate',
          tempID,
      ...
      if (this.isMenuCheck) {
        this.menuFilterBase.cBox = this.dlgObj.element.querySelector(
  ...
  private writeMethod(col: Column, dlgContentEle: Element): void {
    ...
    if (
      !isNullOrUndefined(col.filter) &&
      !isNullOrUndefined(col.filter.ui) &&
  ...
  private filterBtnClick(col: Column): void {
    let flValue: string | number | Date | boolean;
    const targ: HTMLInputElement = this.dlgObj.element.querySelector(
      '.e-flmenu-valuediv input'
    ) as HTMLInputElement;
    ...
    if (col.filterTemplate) {
      ...
      if ((<HTMLInputElement>element.children[0]).value) {
        ...
        if (
          !isNullOrUndefined(
            (<EJ2Intance>(element.children[0] as Element)).ej2_instances
          )
        ) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          fltrValue = (
          ...
          const eControl: EJ2Intance = element.querySelector(
            '.e-control'
          ) as Element as EJ2Intance;
          if (!isNullOrUndefined(eControl)) {
            fltrValue =
              col.type === 'boolean'
                ? (eControl as { checked?: boolean }).checked
                : !isNullOrUndefined(eControl.ej2_instances)
                ? (
                    eControl.ej2_instances[0] as {
      ...
      if (
        !isNullOrUndefined(col.filter) &&
        !isNullOrUndefined(col.filter.ui) &&
        !isNullOrUndefined(col.filter.ui.read as Function)
      ) {
