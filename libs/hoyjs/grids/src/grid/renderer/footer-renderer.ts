...
import { formatUnit, removeClass } from '@hoyui/base';
...
export class FooterRenderer extends ContentRender implements IRenderer {
  ...
  public renderTable(): void {
    ...
    const tFoot: HTMLTableSectionElement = <HTMLTableSectionElement>(
      this.parent.createElement('tfoot')
    );
  ...
  private renderSummaryContent(
    ...
    const summaries: AggregateRowModel[] = <AggregateRowModel[]>(
      this.modelGenerator.getData()
    );
    const dummies: Column[] = isNullOrUndefined(cStart)
      ? this.modelGenerator.getColumns()
      : this.modelGenerator.getColumns(cStart);
    // eslint-disable-next-line max-len
    const rows: Row<AggregateColumnModel>[] = isNullOrUndefined(cStart)
      ? this.modelGenerator.generateRows(input, e || this.aggregates)
      : this.modelGenerator.generateRows(
          input,
          e || this.aggregates,
          cStart,
          cEnd
        );
    const fragment: DocumentFragment = <DocumentFragment>(
      document.createDocumentFragment()
    );

    const rowrenderer: RowRenderer<AggregateColumnModel> =
      new RowRenderer<AggregateColumnModel>(this.locator, null, this.parent);
    ...
    for (
      let srow: number = 0, len: number = summaries.length;
      srow < len;
      srow++
    ) {
      const row: Row<AggregateColumnModel> =
        rows[parseInt(srow.toString(), 10)];
      if (!row) {
        continue;
      }
      ...
      if (
        ...
        tr.querySelectorAll('.e-summarycell').length
      ) {
        removeClass([].slice.call(tr.querySelectorAll('.e-summarycell')), [
          'e-freezeleftborder',
          'e-freezerightborder',
        ]);
      }
  ...
  public refresh(e?: { aggregates?: Object }): void {
    ...
    this.renderSummaryContent(
      e,
      <HTMLTableElement>this.getTable(),
      undefined,
      undefined
    );
  ...
  public refreshCol(): void {
    // frozen table
    const mheaderCol: Node = this.parent.enableColumnVirtualization
      ? (<HeaderRender>this.parent.headerModule).getColGroup()
      : this.parent.element
  ...
  private onWidthChange(args: {
    ...
    if (this.parent.allowResizing && args.module === 'resize') {
      this.updateFooterTableWidth(this.getTable() as HTMLElement);
    }
  }
  ...
  public addEventListener(): void {
    this.evtHandlers = [
      { event: colGroupRefresh, handler: this.refreshCol },
  ...
  private updateFooterTableWidth(tFoot: HTMLElement): void {
    const tHead: HTMLTableElement =
      this.parent.getHeaderTable() as HTMLTableElement;
    if (tHead && tFoot) {
  ...
  public onAggregates(editedData: Object[]): Object {
    ...
    if (
      !(
        this.parent.renderModule.data.isRemote() ||
      ...
      currentViewData =
        this.parent.dataSource instanceof Array
          ? isFiltered
            ...
            : this.parent.dataSource
          : this.parent.dataSource[`${gridData}`].json.length
          ? isFiltered
            ? this.parent.getFilteredRecords()
            : this.parent.dataSource[`${gridData}`].json
    ...
    if (Object.keys(batchChanges).length) {
      for (let i: number = 0; i < currentViewData.length; i++) {
        ...
        // eslint-disable-next-line max-len
        if (
          batchChanges[literals.changedRecords].length &&
          this.getIndexByKey(
            currentViewData[parseInt(i.toString(), 10)],
            batchChanges[literals.changedRecords]
          ) > -1
        ) {
          isModified = true;
          // eslint-disable-next-line max-len
        ...
        // eslint-disable-next-line max-len
        if (
          batchChanges[literals.deletedRecords].length &&
          this.getIndexByKey(
            currentViewData[parseInt(i.toString(), 10)],
            batchChanges[literals.deletedRecords]
          ) > -1
        ) {
          isModified = true;
        } else if (!isModified) {
      ...
      if (batchChanges[literals.addedRecords].length) {
        for (
          let i: number = 0;
          i < batchChanges[literals.addedRecords].length;
          i++
        ) {
          dataSource.push(
            batchChanges[literals.addedRecords][parseInt(i.toString(), 10)]
          );
        }
      }
    } else {
    ...
    for (let i: number = 0; i < aggregateRows.length; i++) {
      for (
        let j: number = 0;
        j <
        (aggregateRows[parseInt(i.toString(), 10)] as AggregateRow).columns
          .length;
        j++
      ) {
        let data: Object[] = [];
        const type: string = (
          aggregateRows[parseInt(i.toString(), 10)] as AggregateRow
