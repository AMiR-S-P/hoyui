...
import {
  GroupModelGenerator,
  GroupedData,
} from '../services/group-model-generator';
import {
...
export class GroupLazyLoadRenderer extends ContentRender implements IRenderer {
  ...
  /** @hidden */
  public ignoreAccent: boolean = this.parent.allowFiltering
  ...
  public captionExpand(tr: HTMLTableRowElement): void {
    ...
    let isRowExist: boolean = rowsObject[oriIndex + 1]
      ? rowsObject[parseInt(oriIndex.toString(), 10)].indent <
        rowsObject[oriIndex + 1].indent
      : false;
    ...
    for (let i: number = 0; i < key.fields.length; i++) {
      const column: Column = this.parent.getColumnByField(
        key.fields[parseInt(i.toString(), 10)]
      );
      if (column.enableGroupByFormat) {
        key.keys[parseInt(i.toString(), 10)] = this.locator
          .getService<IValueFormatter>('valueFormatter')
          .fromView(
            key.keys[parseInt(i.toString(), 10)],
            column.getParser(),
            column.type === 'dateonly' ? 'date' : column.type
          ) as string;
      }
    }
    const e: LazyLoadGroupArgs = {
    ...
    this.parent.trigger(
      events.lazyLoadGroupExpand,
      e,
      (args: LazyLoadGroupArgs) => {
        if (args.cancel) {
          return;
        }
        args.keys = key.keys;
        ...
        if (this.rowsByUid[parseInt(page.toString(), 10)][data.uid]) {
          this.rowsByUid[parseInt(page.toString(), 10)][data.uid].isExpand =
            true;
        }
  ...
  public captionCollapse(tr: HTMLTableRowElement): void {
    const cache: Row<Column>[] =
      this.groupCache[this.parent.pageSettings.currentPage];
      ...
      (args: LazyLoadGroupArgs) => {
        ...
        if (
          ...
          this.groupCache[this.parent.pageSettings.currentPage] = extend(
            [],
            this.refRowsObj[this.parent.pageSettings.currentPage]
          ) as Row<Column>[];
  ...
  private clearCache(uids?: string[]): void {
    ...
    if (uids.length) {
      for (let i: number = 0; i < uids.length; i++) {
        const capIdx: number = this.getRowObjectIndexByUid(
          uids[parseInt(i.toString(), 10)]
        );
        ...
        if (
          this.captionCounts[this.parent.pageSettings.currentPage][capRow.uid]
        ) {
          for (let i: number = capIdx + 1; i < cache.length; i++) {
            if (
              ...
              delete this.captionCounts[this.parent.pageSettings.currentPage][
                capRow.uid
              ];
              break;
            }
            if (cache[parseInt(i.toString(), 10)].isCaptionRow) {
              delete this.captionCounts[this.parent.pageSettings.currentPage][
                cache[parseInt(i.toString(), 10)].uid
              ];
            }
          }
        }
        if (capRow.isExpand) {
          ...
          (this.parent as Grid).groupModule.expandCollapseRows(
            tr.querySelector('.e-recordplusexpand')
          );
        }
        ...
        if (child[child.length - 1].isCaptionRow) {
          subChild = this.getChildRowsByParentIndex(
            cache.indexOf(child[child.length - 1]),
            false,
            false,
            null,
            true,
        ...
        const end: number = subChild.length
          ? cache.indexOf(subChild[subChild.length - 1])
          : cache.indexOf(child[child.length - 1]);
  ...
  private getInitialCaptionIndexes(): string[] {
    ...
    for (
      let i: number = 0;
      i < this.initialGroupCaptions[parseInt(page.toString(), 10)].length;
      i++
    ) {
      uids.push(
        this.initialGroupCaptions[parseInt(page.toString(), 10)][
  ...
  public getRowObjectIndexByUid(uid: string): number {
    return this.objIdxByUid[this.parent.pageSettings.currentPage][
      `${uid}`
    ] as number;
  }
  ...
  private collapseShortcut(args: { target: Element; collapse: boolean }): void {
    if (
      ...
      args.target &&
      parentsUntil(args.target, literals.content) &&
      args.target.parentElement.tagName === 'TR'
    ) {
      ...
      if (args.collapse) {
        ...
        if (capRow.isCaptionRow && capRow.isExpand) {
          const capEle: HTMLTableRowElement = this.getRowElementByUid(
            rowObj.parentUid
          );
          (this.parent as Grid).groupModule.expandCollapseRows(
            capEle.cells[rowObj.indent - 1]
          );
        }
      } else {
  ...
  private getRowByUid(uid: string): Row<Column> {
    return this.rowsByUid[this.parent.pageSettings.currentPage][
      `${uid}`
    ] as Row<Column>;
  }
  ...
  private actionBegin(args: NotifyArgs): void {
    if (!args.cancel) {
      if (
        !this.requestType.some((value: string) => value === args.requestType)
      ) {
        this.groupCache = {};
      ...
      if (
        ...
        for (let j: number = 0; j < keys.length; j++) {
          const cache: Row<Column>[] =
            this.groupCache[keys[parseInt(j.toString(), 10)]];
          for (let i: number = 0; i < cache.length; i++) {
            if (
              cache[parseInt(i.toString(), 10)].isCaptionRow &&
              !this.captionModelGen.isEmpty()
            ) {
              this.changeCaptionRow(
                cache[parseInt(i.toString(), 10)],
                null,
            ...
            if (cache[parseInt(i.toString(), 10)].isDataRow) {
              const from: number =
                ...
                cache[parseInt(i.toString(), 10)].indent;
              const to: number =
  ...
  private removeRows(idx: number, trIdx: number, uid?: string): void {
    ...
    const trs: Element[] = [].slice.call(
      this.parent.getContent().querySelectorAll('tr')
    );
    let aggUid: string;
    let count: number = 0;
    ...
    if (
      this.rowsByUid[parseInt(page.toString(), 10)][
        rows[parseInt(idx.toString(), 10)].uid
      ]
    ) {
      this.rowsByUid[parseInt(page.toString(), 10)][
        rows[parseInt(idx.toString(), 10)].uid
    ...
    for (let i: number = idx + 1; i < rows.length; i++) {
      if (
        rows[parseInt(i.toString(), 10)].indent === indent ||
        rows[parseInt(i.toString(), 10)].indent < indent
      ) {
      ...
      if (
        ...
        rows[parseInt(i.toString(), 10)].isExpand
      ) {
    ...
    for (let i: number = trIdx + 1; i < trs.length; i++) {
      if (trs[parseInt(i.toString(), 10)].getAttribute('data-uid') === capUid) {
        ...
        trs[parseInt(i.toString(), 10)].getAttribute('data-uid') === aggUid
      ) {
    ...
    if (this.parent.enableVirtualization) {
      this.cacheRowsObj[`${uid}`] = this.groupCache[
  ...
  private lazyLoadHandler(args: {
    ...
    const tr: HTMLElement = this.parent.getContent().querySelectorAll('tr')[
      args.index
    ];
    const uid: string = tr.getAttribute('data-uid');
    const captionIndex: number = this.getRowObjectIndexByUid(uid);
    const captionRow: IRow<Column> =
      this.groupCache[this.parent.pageSettings.currentPage][
        parseInt(captionIndex.toString(), 10)
      ];
    let rows: Row<Column>[] = args.isRowExist
      ? args.isScroll
        ? this.scrollData
        : this.parent.enableVirtualization
        ? this.cacheRowsObj[`${uid}`]
      ...
      if (Object.keys(args.data).indexOf('GroupGuid') !== -1) {
        for (let i: number = 0; i < args.data.length; i++) {
          const data: Row<Column> = this.groupGenerator.generateCaptionRow(
            args.data[parseInt(i.toString(), 10)] as GroupedData,
            args.level,
            captionRow.parentGid,
            undefined,
            0,
            captionRow.uid
          );
          ...
          if (this.parent.aggregates.length) {
            rows = rows.concat(
              <Row<Column>[]>(
                this.summaryModelGen.generateRows(
        ...
        this.groupGenerator.index = this.getStartIndex(
          captionIndex,
          args.isScroll
        );
        rows = this.groupGenerator.generateDataRows(
          args.data,
          args.level,
          captionRow.parentGid,
          0,
    ...
    const lastRow: boolean = !args.up
      ? this.hasLastChildRow(args.isScroll, args.count, nxtChild.length)
      : true;
    ...
    const aggregates: Row<Column>[] =
      !args.isScroll && !args.isRowExist
        ? this.getAggregateByCaptionIndex(captionIndex)
        : [];
    if (!args.up) {
      if (
        !args.isRowExist ||
        ...
        this.refreshRowObjects(
          rows,
          args.isScroll ? this.rowObjectIndex : captionIndex
        );
      }
    }

    if (this.parent.enableVirtualization) {
      const uid: string = args.isScroll
        ? this.groupCache[this.parent.pageSettings.currentPage][this.rowIndex]
            .uid
        : captionRow.uid;
      this.parent.notify(events.refreshVirtualLazyLoadCache, {
        rows: rows,
    ...
    if (this.isFirstChildRow && !args.up) {
      this.parent.getContent().firstElementChild.scrollTop =
        rows.length * this.parent.getRowHeight();
    }
    ...
    for (let i: number = 0; i < rows.length; i++) {
      this.refRowsObj[this.parent.pageSettings.currentPage].splice(
        captionIndex + i + 1,
        0,
  ...
  private getStartIndex(capIdx: number, isScroll: boolean): number {
    ...
    let count: number = 0;
    let idx: number = 0;
    const prevCapRow: Row<Column> = this.getRowByUid(
      cache[parseInt(capIdx.toString(), 10)].parentUid
    );
    if (prevCapRow) {
    ...
    if (cache[parseInt(capIdx.toString(), 10)].indent > 0) {
      for (let i: number = capIdx - 1; i >= 0; i--) {
        if (
          cache[parseInt(i.toString(), 10)].indent <
          cache[parseInt(capIdx.toString(), 10)].indent
        ) {
          break;
        }
        if (
          ...
          count =
            count +
            (cache[parseInt(i.toString(), 10)].data as GroupedData).count;
        }
      }
    }
    const index: number =
      count +
      idx +
      this.startIndexes[parseInt(page.toString(), 10)][
        (cache[parseInt(capIdx.toString(), 10)] as IRow<Column>).parentGid
      ];
    return index;
  }
  ...
  private prevCaptionCount(prevCapRow: Row<Column>): number {
    ...
    for (let i: number = cache.indexOf(prevCapRow) - 1; i >= 0; i--) {
      ...
      if (
        cache[parseInt(i.toString(), 10)].isCaptionRow &&
        cache[parseInt(i.toString(), 10)].indent === prevCapRow.indent
      ) {
        const count: number =
          this.captionCounts[parseInt(page.toString(), 10)][
            cache[parseInt(i.toString(), 10)].uid
          ];
        idx =
          idx +
      ...
      for (let i: number = 0; i < cache.length; i++) {
        if (cache[parseInt(i.toString(), 10)].isCaptionRow) {
          if (!indexes.length) {
            ...
            indexes.push(
              (cache[parseInt(idx.toString(), 10)].data as GroupedData).count +
  ...
  private hasLastChildRow(
    ...
    return isScroll
      ? captionCount === this.childCount + rowCount
      : captionCount === rowCount;
  }
  ...
  private getAggregateByCaptionIndex(index: number): Row<Column>[] {
    ...
    for (let i: number = index + 1; i < cache.length; i++) {
      ...
      if (
        isNullOrUndefined(cache[parseInt(i.toString(), 10)].indent) &&
        cache[parseInt(i.toString(), 10)].parentUid === uid
      ) {
  ...
  private getChildRowsByParentIndex(
    ...
    data?: Row<Column>[],
    includeAgg?: boolean,
    includeCollapseAgg?: boolean
  ): Row<Column>[] {
    const cache: Row<Column>[] = data
      ? data
      : this.groupCache[this.parent.pageSettings.currentPage];
    ...
    if (
      ...
      (parentRow.isCaptionRow && !parentRow.isExpand && !includeCollapseAgg)
    ) {
      return [];
    }
    ...
    for (let i: number = index + 1; i < cache.length; i++) {
      if (cache[parseInt(i.toString(), 10)].parentUid === uid) {
        ...
        if (deep && cache[parseInt(i.toString(), 10)].isCaptionRow) {
          rows = rows.concat(
            this.getChildRowsByParentIndex(i, deep, block, data, includeAgg)
          );
        }
  ...
  public initialGroupRows(isReorder?: boolean): Row<Column>[] {
    ...
    for (let i: number = 0; i < cache.length; i++) {
      if (cache[parseInt(i.toString(), 10)].indent === 0) {
        rows.push(cache[parseInt(i.toString(), 10)]);
        rows = rows.concat(
          this.getChildRowsByParentIndex(i, true, true, cache, true)
        );
      }
    }
    return rows;
  ...
  public getRenderedRowsObject(): Row<Column>[] {
    ...
    const trs: HTMLTableRowElement[] = [].slice.call(
      this.parent.getContent().querySelectorAll('tr')
    );
    for (let i: number = 0; i < trs.length; i++) {
      rows.push(
        this.getRowByUid(
          trs[parseInt(i.toString(), 10)].getAttribute('data-uid')
        )
      );
    }
    return rows;
  ...
  private getCacheRowsOnDownScroll(index: number): Row<Column>[] {
    ...
    for (let i: number = 0; i < this.pageSize; i++) {
      if (
        !rowsObject[parseInt(k.toString(), 10)] ||
        rowsObject[parseInt(k.toString(), 10)].indent <
      ...
      if (
        ...
        if (
          rowsObject[parseInt(k.toString(), 10)].isCaptionRow &&
          rowsObject[parseInt(k.toString(), 10)].isExpand
        ) {
          rows = rows.concat(
            this.getChildRowsByParentIndex(k, true, true, null, true)
          );
        }
      }
      if (
  ...
  private getCacheRowsOnUpScroll(
    ...
    for (let i: number = 0; i < rowsObject.length; i++) {
      if (
        str &&
        (!rowsObject[parseInt(i.toString(), 10)] ||
          rowsObject[parseInt(i.toString(), 10)].indent <
            rowsObject[parseInt(index.toString(), 10)].indent ||
          rowsObject[parseInt(i.toString(), 10)].uid === end)
      ) {
        break;
      }
      ...
      if (
        ...
        if (
          ...
          rowsObject[parseInt(i.toString(), 10)].isExpand
        ) {
          rows = rows.concat(
            this.getChildRowsByParentIndex(i, true, true, null, true)
          );
        }
      }
    }
  ...
  private scrollHandler(e: { scrollDown: boolean }): void {
    ...
    const downTrs: Element[] = [].slice.call(
      this.parent.getContent().getElementsByClassName('e-lazyload-middle-down')
    );
    const upTrs: Element[] = [].slice.call(
      this.parent.getContent().getElementsByClassName('e-lazyload-middle-up')
    );
    const endTrs: Element[] = [].slice.call(
      this.parent.getContent().getElementsByClassName('e-not-lazyload-end')
    );
    let tr: Element;
    let lazyLoadDown: boolean = false;
    let lazyLoadUp: boolean = false;
    let lazyLoadEnd: boolean = false;
    if (e.scrollDown && downTrs.length) {
      const result: { entered: boolean; tr: Element } =
        this.findRowElements(downTrs);
      tr = result.tr;
    ...
    if (!e.scrollDown && endTrs) {
      for (let i: number = 0; i < endTrs.length; i++) {
        const top: number =
          endTrs[parseInt(i.toString(), 10)].getBoundingClientRect().top;
  ...
  private scrollUpEndRowHandler(tr: Element): void {
    ...
    const rows: Row<Column>[] = this.groupCache[parseInt(page.toString(), 10)];
    const uid: string = tr.getAttribute('data-uid');
    let index: number = (this.rowObjectIndex =
      this.getRowObjectIndexByUid(uid));
    ...
    const captionRowEle: Element = this.parent
      .getContent()
      .querySelector('tr[data-uid=' + parentCapRow.uid + ']');
    ...
    if (this.childCount < (parentCapRow.data as GroupedData).count) {
      ...
      const isRowExist: boolean = rows[index + 1]
        ? childRow.indent === rows[index + 1].indent
        : false;
      this.scrollData = isRowExist
        ? this.getCacheRowsOnDownScroll(index + 1)
        : [];
      const key: { fields: string[]; keys: string[] } = getGroupKeysAndFields(
        capRowObjIdx,
        rows
      ...
      if (
        this.cacheMode &&
        this.childCount >= this.pageSize * this.cacheBlockSize
      ) {
        const child: Row<Column>[] =
          this.getChildRowsByParentIndex(capRowObjIdx);
        const currenBlock: number = Math.ceil(
          child.indexOf(rows[parseInt(idx.toString(), 10)]) / this.pageSize
        );
  ...
  private scrollDownHandler(tr: Element): void {
    ...
    index = this.rowObjectIndex = this.getRowObjectIndexByUid(
      child[this.childCount - 1].uid
    );
    ...
    if (this.childCount < (parentCapRow.data as GroupedData).count) {
      ...
      if (
        isRowExist &&
        !isNullOrUndefined(this.getRowElementByUid(rows[index + 1].uid))
      ) {
        this.parent.islazyloadRequest = false;
        this.childCount = 0;
        return;
      ...
      this.scrollData = isRowExist
        ? this.getCacheRowsOnDownScroll(this.rowObjectIndex + 1)
        : [];
      const query: { fields: string[]; keys: string[] } = getGroupKeysAndFields(
        capRowObjIdx,
        rows
      );
      const args: LazyLoadGroupArgs = {
      ...
      if (
        this.cacheMode &&
        this.childCount - this.pageSize >= this.pageSize * this.cacheBlockSize
      ) {
        this.isScrollDown = true;
        const child: Row<Column>[] =
          this.getChildRowsByParentIndex(capRowObjIdx);
        const currenBlock: number =
          Math.ceil(
            child.indexOf(rows[parseInt(idx.toString(), 10)]) / this.pageSize
          ) - 1;
  ...
  private getCurrentBlockEndIndex(row: Row<Column>, index: number): number {
    ...
    if (row.isCaptionRow) {
      if (row.isExpand) {
        const childCount: number = this.getChildRowsByParentIndex(
          index,
          true
        ).length;
      ...
      for (let i: number = idx + 1; i < rows.length; i++) {
        if (
          rows[parseInt(i.toString(), 10)].indent ===
            rows[parseInt(index.toString(), 10)].indent ||
          rows[parseInt(i.toString(), 10)].indent <
            rows[parseInt(index.toString(), 10)].indent
        ) {
          index = idx;
  ...
  private removeBlock(
    ...
    removeBlock: number,
    child: Row<Column>[],
    lastchild?: Row<Column>
  ): void {
    const page: number = this.parent.pageSettings.currentPage;
    const rows: Row<Column>[] = this.groupCache[parseInt(page.toString(), 10)];
    const uid1: string =
      child[(removeBlock + 1) * this.pageSize - 1 - this.blockSize].uid;
    ...
    rows[parseInt(firstIdx.toString(), 10)].lazyLoadCssClass =
      'e-lazyload-middle-up';
  ...
  private scrollUpHandler(tr: Element): void {
    ...
    const index: number = (this.rowObjectIndex =
      this.getRowObjectIndexByUid(uid));
    ...
    const captionRowEle: Element = this.parent.getRowElementByUID(
      parentCapRow.uid
    ) as HTMLTableRowElement;
    const capRowEleIndex: number = (captionRowEle as HTMLTableRowElement)
      .rowIndex;
    const child: Row<Column>[] = this.getChildRowsByParentIndex(capRowObjIdx);
    const childIdx: number = child.indexOf(
      rows[parseInt(index.toString(), 10)]
    );
    ...
    this.scrollData = this.getCacheRowsOnUpScroll(
      start,
      end,
      index - (idx - 1)
    );
    ...
    if (
      child.length !== (parentCapRow.data as GroupedData).count &&
      removeBlock * this.pageSize > child.length
    ) {
      this.isFirstChildRow = false;
    ...
    const count: number =
      removeBlock * this.pageSize > (parentCapRow.data as GroupedData).count
        ? (parentCapRow.data as GroupedData).count
        : removeBlock * this.pageSize;
    const size: number =
      removeBlock * this.pageSize > (parentCapRow.data as GroupedData).count
        ? this.pageSize -
          (this.pageSize * removeBlock -
            (parentCapRow.data as GroupedData).count)
        : this.pageSize;
    const childRows: Row<Column>[] = this.getChildRowsByParentIndex(
      rows.indexOf(child[count - 1]),
      true,
      false,
      null,
      true
    );
    const uid1: string = childRows.length
      ? childRows[childRows.length - 1].uid
      : child[count - 1].uid;
    const uid2: string = child[count - size].uid;
    const uid3: string = child[count - size - 1].uid;
    const lastIdx: number =
      this.objIdxByUid[parseInt(page.toString(), 10)][`${uid2}`] - idx;
    if (
      rows[parseInt(lastIdx.toString(), 10)].lazyLoadCssClass ===
      'e-lazyload-middle-down'
    ) {
      const trEle: Element = this.getRowElementByUid(
        rows[parseInt(lastIdx.toString(), 10)].uid
      );
      if (trEle) {
    ...
    const key: { fields: string[]; keys: string[] } = getGroupKeysAndFields(
      this.getRowObjectIndexByUid(parentCapRow.uid),
      rows
    );
    const args: LazyLoadGroupArgs = {
      rowIndex: capRowEleIndex,
  ...
  private findRowElements(rows: Element[]): { entered: boolean; tr: Element } {
    ...
    for (let i: number = 0; i < rows.length; i++) {
      let rowIdx: number = (
        rows[parseInt(i.toString(), 10)] as HTMLTableRowElement
      ).rowIndex;
      if (this.parent.enableVirtualization) {
        const currentInfo: VirtualInfo = (
          this.parent.contentModule as VirtualContentRenderer
        ).currentInfo;
        if (
          currentInfo &&
          currentInfo.blockIndexes &&
          currentInfo.blockIndexes[0] > 1
        ) {
          rowIdx =
            rowIdx +
            (this.parent.contentModule as VirtualContentRenderer).offsets[
              currentInfo.blockIndexes[0] - 1
            ] /
  ...
  private setCache(e?: { args: NotifyArgs; data: Row<Column>[] }): void {
    ...
    if (
      ...
      this.groupCache[parseInt(page.toString(), 10)] =
        this.initialGroupCaptions[parseInt(page.toString(), 10)] =
          this.groupCache[e.args['prevPage']].concat(
            extend([], e.data) as Row<Column>[]
          );
      const groupCacheKeys: string[] = Object.keys(this.groupCache);
      for (let i: number = 0; i < groupCacheKeys.length; i++) {
        if (
          e.args['currentPage'] !==
          parseInt(groupCacheKeys[parseInt(i.toString(), 10)], 10)
        ) {
          delete this.groupCache[
            `${groupCacheKeys[parseInt(i.toString(), 10)]}`
          ];
          delete this.initialGroupCaptions[
            `${groupCacheKeys[parseInt(i.toString(), 10)]}`
          ];
        }
      }
      ...
      this.groupCache[parseInt(page.toString(), 10)] =
        this.initialGroupCaptions[parseInt(page.toString(), 10)] = extend(
          [],
          e.data
        ) as Row<Column>[];
    }
  ...
  private captionRowExpand(args: LazyLoadGroupArgs): void {
    ...
    const level: number =
      this.parent.groupSettings.columns.indexOf(
        (captionRow.data as GroupedData).field
      ) + 1;
    ...
    if (args.makeRequest) {
      ...
      this.parent.renderModule.data
        ...
        .catch((e: ReturnType) =>
          this.parent.renderModule.dataManagerFailure(e, {
      ...
      this.lazyLoadHandler({
        data: null,
        count: (args.groupInfo.data as GroupedData).count,
        level: level,
        index: args.rowIndex,
        isRowExist: true,
        isScroll: args.isScroll,
  ...
  private scrollReset(top?: number): void {
    this.parent.getContent().firstElementChild.scrollTop = top
      ? this.parent.getContent().firstElementChild.scrollTop + top
      : 0;
  }
  ...
  private updateCurrentViewData(): void {
    ...
    this.parent.currentViewData = records.length
      ? records
  ...
  public setVisible(columns?: Column[]): void {
    ...
    const contentrows: Row<Column>[] = this.getRows().filter(
      (row: Row<Column>) => !row.isDetailRow
    );
    for (let i: number = 0; i < columns.length; i++) {
      ...
      if (idx !== -1 && testRow && idx < testRow.cells.length) {
        setStyleAttribute(
          <HTMLElement>(
            this.getColGroup().childNodes[parseInt(idx.toString(), 10)]
          ),
          { display: displayVal }
        );
      }
  ...
  public setDisplayNone(
    ...
    if (!this.parent.groupSettings.columns.length) {
      ...
      const keys: string[] = Object.keys(this.groupCache);
      for (let j: number = 0; j < keys.length; j++) {
        const uids: Row<Column>[] = this.rowsByUid[
          keys[parseInt(j.toString(), 10)]
        ] as Row<Column>[];
        ...
        for (let i: number = 0; i < idxs.length; i++) {
          const tr: HTMLTableRowElement = this.parent
            .getContent()
            .querySelector(
              'tr[data-uid=' + idxs[parseInt(i.toString(), 10)] + ']'
            );
          ...
          if (row.isCaptionRow) {
            if (!this.captionModelGen.isEmpty()) {
              ...
              row.cells[row.indent + 1].colSpan =
                displayVal === ''
                  ? row.cells[row.indent + 1].colSpan + 1
                  : row.cells[row.indent + 1].colSpan - 1;
              if (tr) {
                tr.cells[row.indent + 1].colSpan =
                  row.cells[row.indent + 1].colSpan;
              }
            }
          }
          if (row.isDataRow) {
            ...
            row.cells[parseInt(oriIdx.toString(), 10)].visible =
              displayVal === '' ? true : false;
          }
          if (
            !row.isCaptionRow &&
            !row.isDataRow &&
            isNullOrUndefined(row.indent)
          ) {
            row.cells[parseInt(oriIdx.toString(), 10)].visible =
              displayVal === '' ? true : false;
            row.visible = row.cells.some(
  ...
  private changeCaptionRow(
    ...
    this.groupCache[parseInt(index.toString(), 10)][
      this.objIdxByUid[parseInt(index.toString(), 10)][row.uid]
    ] = data;
    if (tr) {
      const tbody: Element = this.parent
        .getContentTable()
        .querySelector(literals.tbody);
      tbody.replaceChild(
        this.rowRenderer.render(data, this.parent.getColumns()),
        tr
      );
    }
  }
  ...
  private showAndHideCells(
    ...
    if (tr) {
      ...
      setStyleAttribute(
        tr.querySelectorAll(cls)[parseInt(idx.toString(), 10)] as HTMLElement,
        { display: displayVal }
      );
      if (
        tr
        ...
        removeClass(
          [tr.querySelectorAll(cls)[parseInt(idx.toString(), 10)]],
          ['e-hide']
        );
      }
      if (isSummary) {
