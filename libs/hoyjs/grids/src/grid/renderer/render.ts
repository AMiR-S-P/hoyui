import { L10n, NumberFormatOptions } from '@hoyui/base';
import {
  remove,
  resetBlazorTemplate,
  blazorTemplates,
...
import { isNullOrUndefined, extend, DateFormatOptions } from '@hoyui/base';
import {
  DataManager,
  Group,
  Query,
...
} from '../renderer/caption-cell-renderer';
...
export class Render {
  ...
  public render(): void {
    ...
    this.headerRenderer = <HeaderRender>(
      this.renderer.getRenderer(RenderType.Header)
    );
    this.contentRenderer = <ContentRender>(
      this.renderer.getRenderer(RenderType.Content)
    );
  ...
  public refresh(e: NotifyArgs = { requestType: 'refresh' }): void {
    ...
    gObj.trigger(
      events.actionBegin,
      e,
      (args: NotifyArgs = { requestType: 'refresh' }) => {
        if (args.cancel) {
          ...
          if (
            ...
            this.parent.filterSettings.columns[
              this.parent.filterModule.filterObjIndex
            ] = this.parent.filterModule.prevFilterObject;
            const iconClass: string =
              this.parent.showColumnMenu &&
              this.parent.filterModule['column'].showColumnMenu
                ? '.e-columnmenu'
                : '.e-icon-filter';
            const col: Element = this.parent.element.querySelector(
              '[e-mappinguid="' + this.parent.filterModule['column'].uid + '"]'
            ).parentElement;
          ...
          if (
            ...
            this.parent.filterSettings.columns =
              this.parent.filterModule.checkboxPrevFilterObject;
          }
          if (args.requestType === 'grouping') {
            // Remove the dropped column name from groupsettings.columns if args.cancel is true
            const index: number = gObj.groupSettings.columns.indexOf(
              (args as GroupEventArgs).columnName
            );
            if (index !== -1) {
              gObj.setProperties(
                {
              ...
              const column: Column = gObj.getColumnByField(
                (args as GroupEventArgs).columnName
              );
              const headerCell: Element = gObj.getColumnHeaderByField(
                column.field
              );
              column.visible =
                !isNullOrUndefined(headerCell) &&
                !headerCell.classList.contains('e-hide');
            }
          }
          return;
        }
        if (
          ...
          (args.action === 'clearFilter' ||
            args.action === 'clear-filter' ||
            (args.requestType === 'searching' && args.searchString === '') ||
            args.action === 'add')
        ) {
        ...
        if (
          gObj.allowPaging &&
          gObj.pageSettings.pageSizes &&
          gObj.pagerModule.pagerObj.isAllPage &&
          args.action === 'add' &&
          args.requestType === ('save' as Action) &&
          gObj.pagerModule.pagerObj.checkAll
        ) {
          gObj.setProperties(
            { pageSettings: { pageSize: gObj.pageSettings.pageSize + 1 } },
            true
          );
        }
        if (args.requestType === ('delete' as Action) && gObj.allowPaging) {
          const dataLength: number = (<{ data?: NotifyArgs[] }>args).data
            .length;
          const count: number =
            gObj.pageSettings.totalRecordsCount - dataLength;
          const currentViewData: number = gObj.getCurrentViewRecords().length;
          // eslint-disable-next-line max-len
          if (
            (!(currentViewData - dataLength) &&
              count &&
        ...
        if (
          ...
          'result' in this.parent.dataSource
        ) {
  ...
  public resetTemplates(): void {
    ...
    if (gObj.groupSettings.captionTemplate) {
      resetBlazorTemplate(
        gObj.element.id + 'captionTemplate',
        'CaptionTemplate'
      );
    }
    if (gObj.rowTemplate) {
    ...
    if (gObj.toolbarTemplate) {
      resetBlazorTemplate(
        gObj.element.id + 'toolbarTemplate',
        'ToolbarTemplate'
      );
    }
    ...
    for (let i: number = 0; i < gridColumns.length; i++) {
      if (gridColumns[parseInt(i.toString(), 10)].template) {
        blazorTemplates[
          gObj.element.id + gridColumns[parseInt(i.toString(), 10)].uid
        ] = [];
        resetBlazorTemplate(
          gObj.element.id + gridColumns[parseInt(i.toString(), 10)].uid,
          'Template'
      ...
      if (gridColumns[parseInt(i.toString(), 10)].headerTemplate) {
        resetBlazorTemplate(
          gObj.element.id +
            gridColumns[parseInt(i.toString(), 10)].uid +
            'headerTemplate',
          'HeaderTemplate'
        );
      }
    ...
    for (let k: number = 0; k < gObj.aggregates.length; k++) {
      for (
        let j: number = 0;
        j < gObj.aggregates[parseInt(k.toString(), 10)].columns.length;
        j++
      ) {
        if (
          gObj.aggregates[parseInt(k.toString(), 10)].columns[
  ...
  private refreshDataManager(args: NotifyArgs = {}): void {
    ...
    const maskRow: boolean =
      (gObj.loadingIndicator.indicatorType === 'Shimmer' &&
    ...
    if (
      args.requestType !== 'virtualscroll' &&
      !(<{ isCaptionCollapse?: boolean }>args).isCaptionCollapse &&
      !maskRow
    ) {
    ...
    if (maskRow) {
      gObj.showMaskRow(
        args.requestType === 'virtualscroll'
          ? args.virtualInfo.sentinelInfo.axis
          : args.requestType === 'infiniteScroll'
    ...
    if (!dataManager) {
      if (
        gObj.allowPaging &&
        !gObj.getDataModule().dataManager.dataSource.offline &&
        gObj.pageSettings &&
        gObj.pageSettings.pageSizes &&
        gObj.pagerModule &&
        gObj.pagerModule.pagerObj &&
        gObj.pagerModule.pagerObj.isAllPage
      ) {
        gObj.pagerModule.pagerObj.isAllPage = undefined;
      }
      dataManager = this.data.getData(
        args as NotifyArgs,
        this.data.generateQuery().requiresCount()
      );
    } else {
    ...
    if (this.parent.columnQueryMode === 'ExcludeHidden' && foreignKeyLength) {
      foreignKeyLength = foreignKeyColumns.filter(
        (col: Column) => col.visible !== false
      ).length;
    }
    if (
      foreignKeyLength &&
      (!isFActon || this.parent.searchSettings.key.length)
    ) {
      const deffered: Deferred = new Deferred();
    ...
    if (
      this.parent.groupSettings.disablePageWiseAggregates &&
      this.parent.groupSettings.columns.length
    ) {
      dataManager = dataManager.then((e: ReturnType) =>
        this.validateGroupRecords(e)
      );
    }
    dataManager
      .then((e: ReturnType) => this.dataManagerSuccess(e, args))
  ...
  private isNeedForeignAction(): boolean {
    ...
    return (
      !!(
        (gObj.allowFiltering && gObj.filterSettings.columns.length) ||
        (!isNullOrUndefined(gObj.searchSettings.key) &&
  ...
  private foreignKey(columns: Column[]): boolean {
    return columns.some((col: Column) => {
      ...
      fbool = this.parent.filterSettings.columns.some(
  ...
  private sendBulkRequest(args?: NotifyArgs): void {
    ...
    if (
      ...
      if (dataLength) {
        gObj.setProperties(
          {
            pageSettings: { pageSize: gObj.pageSettings.pageSize + dataLength },
          },
          true
        );
      }
    ...
    if (
      gObj.allowPaging &&
      ((<{ addedRecords?: Object[] }>(<{ changes?: Object }>args).changes)
        .addedRecords.length ||
        (<{ deletedRecords?: Object[] }>(<{ changes?: Object }>args).changes)
          .deletedRecords.length ||
        (<{ changedRecords?: Object[] }>(<{ changes?: Object }>args).changes)
    ...
    if (this.data.dataManager.dataSource.offline) {
      ...
      promise
        .then((e: ReturnType) => {
          this.data
            .getData(args, query)
            .then((e: { result: Object[]; count: number }) =>
              this.dmSuccess(e, args)
            )
  ...
  public emptyRow(isTrigger?: boolean): void {
    const gObj: IGrid = this.parent;
    let tbody: Element = this.contentRenderer
      .getTable()
    ...
    let spanCount: number =
      ...
      isNullOrUndefined(gObj.rowDropSettings.targetID)
        ? 1
        : 0;
    ...
    if (gObj.emptyRecordTemplate) {
      const emptyRecordTemplateID: string =
        gObj.element.id + 'emptyRecordTemplate';
      ...
      if (
        ...
        td.appendChild(
          gObj.getEmptyRecordTemplate()(
            gObj.dataSource,
            gObj,
            'emptyRecordTemplate',
            emptyRecordTemplateID,
            undefined,
            undefined,
            undefined,
            this.parent['root']
      ...
      td = this.parent.createElement('td', {
        ...
        attrs: {
          colspan: (
            gObj.getVisibleColumns().length +
            spanCount +
            (!isNullOrUndefined(gObj.groupSettings.columns)
              ? gObj.groupSettings.columns.length
              : 0)
          ).toString(),
        },
    ...
    if (gObj.frozenRows && gObj.element.querySelector('.e-frozenrow-border')) {
      this.parent.element
        .querySelector('.e-frozenrow-border')
        .classList.add('e-frozenrow-empty');
    }
    tr.appendChild(td);
    ...
    if (isTrigger) {
      ...
      this.parent.notify(events.onEmpty, {
        rows: [
          new Row<Column>({
            isDataRow: true,
            cells: [new Cell<Column>({ isDataCell: true, visible: true })],
          }),
        ],
      });
  ...
  private updateColumnType(record: Object): void {
    ...
    const data: Object =
      record && (<{ items: Object[] }>record).items
        ? (<{ items: Object[] }>record).items[0]
        : record;
    const fmtr: IValueFormatter =
      this.locator.getService<IValueFormatter>('valueFormatter');
    for (let i: number = 0, len: number = columns.length; i < len; i++) {
      value = getObject(columns[parseInt(i.toString(), 10)].field || '', data);
      if (
        !isNullOrUndefined(columns[parseInt(i.toString(), 10)][`${cFormat}`])
      ) {
        columns[parseInt(i.toString(), 10)].format =
          columns[parseInt(i.toString(), 10)][`${cFormat}`];
      }
      if (
        !isNullOrUndefined(
        ...
        !isNullOrUndefined(
          columns[parseInt(i.toString(), 10)].validationRules[`${equalTo}`]
        )
      ) {
        columns[parseInt(i.toString(), 10)].validationRules[`${equalTo}`][0] =
          this.parent.element.id +
          columns[parseInt(i.toString(), 10)].validationRules[`${equalTo}`][0];
      }
      if (
        columns[parseInt(i.toString(), 10)].isForeignColumn() &&
        columns[parseInt(i.toString(), 10)].columnData
      ) {
        value = getObject(
          columns[parseInt(i.toString(), 10)].foreignKeyValue || '',
          columns[parseInt(i.toString(), 10)].columnData[0]
        );
      }
      if (!isNullOrUndefined(value)) {
        this.isColTypeDef = true;
        if (!columns[parseInt(i.toString(), 10)].type) {
          columns[parseInt(i.toString(), 10)].type = value.getDay
            ? value.getHours() > 0 ||
              value.getMinutes() > 0 ||
              value.getSeconds() > 0 ||
        ...
        columns[parseInt(i.toString(), 10)].type =
          columns[parseInt(i.toString(), 10)].type || null;
      }
      const valueFormatter: ValueFormatter = new ValueFormatter();
      if (
        columns[parseInt(i.toString(), 10)].format &&
        ((<DateFormatOptions>columns[parseInt(i.toString(), 10)].format)
          .skeleton ||
          ((<DateFormatOptions>columns[parseInt(i.toString(), 10)].format)
            .format &&
            typeof (<DateFormatOptions>(
              columns[parseInt(i.toString(), 10)].format
            )).format === 'string'))
      ) {
        columns[parseInt(i.toString(), 10)].setFormatter(
          valueFormatter.getFormatFunction(
            extend(
              {},
              columns[parseInt(i.toString(), 10)].format as DateFormatOptions
            )
          )
        );
        columns[parseInt(i.toString(), 10)].setParser(
          valueFormatter.getParserFunction(
            columns[parseInt(i.toString(), 10)].format as DateFormatOptions
          )
        );
      }
      if (typeof columns[parseInt(i.toString(), 10)].format === 'string') {
        ...
        !columns[parseInt(i.toString(), 10)].format &&
        columns[parseInt(i.toString(), 10)].type === 'number'
      ) {
        columns[parseInt(i.toString(), 10)].setParser(
      ...
      if (
        columns[parseInt(i.toString(), 10)].type === 'dateonly' &&
        !columns[parseInt(i.toString(), 10)].format
      ) {
  ...
  public dataManagerSuccess(e: ReturnType, args?: NotifyArgs): void {
    ...
    const isInfiniteDelete: boolean =
      ...
      (args.requestType === 'delete' ||
        (args.requestType === 'save' &&
          this.parent.infiniteScrollModule.requestType === 'add' &&
          !(
    ...
    gObj.trigger(events.beforeDataBound, e, (dataArgs: ReturnType) => {
      ...
      dataArgs.result = isNullOrUndefined(dataArgs.result)
        ? []
        : dataArgs.result;
      ...
      if (
        !gObj.getColumns().length &&
        !len &&
        !(gObj.columns.length && gObj.columns[0] instanceof Column)
      ) {
        gObj.hideSpinner();
        return;
      }
      if (this.isInfiniteEnd(args) && !len) {
        this.parent.notify(events.infiniteEditHandler, {
          e: args,
          result: e.result,
          count: e.count,
          agg: e.aggregates,
      ...
      if (
        ...
        this.parent.notify(events.commandColumnDestroy, {
          type: 'refreshCommandColumn',
        });
      }
      this.contentRenderer.prevCurrentView =
        !isNullOrUndefined(this.parent.currentViewData) &&
        this.parent.currentViewData.slice();
      ...
      if (
        ...
        gObj.allowPaging &&
        args &&
        args.requestType !== ('delete' as Action)
      ) {
        if (
          this.parent.groupSettings.enableLazyLoading &&
        ...
        if (args.requestType !== ('paging' as Action)) {
          gObj.pageSettings.currentPage = Math.ceil(
            dataArgs.count / gObj.pageSettings.pageSize
          );
        }
        gObj.dataBind();
        return;
      ...
      if (
        ((!gObj.getColumns().length && len) || !this.isLayoutRendered) &&
      ...
      if (!this.isColTypeDef && gObj.getCurrentViewRecords()) {
        if (
          this.data.dataManager.dataSource.offline &&
          gObj.dataSource &&
          (gObj.dataSource as object[]).length
        ) {
      ...
      if (
        (gObj.groupSettings.columns.length ||
          (args && args.requestType === 'ungrouping')) &&
        args &&
      ...
      if (len) {
        if (isGroupAdaptive(gObj)) {
          ...
          args.scrollTop = {
            top: this.contentRenderer[`${content}`].scrollTop,
          };
        }
        if (!isInfiniteDelete) {
        ...
        if (args && (<{ isCaptionCollapse?: boolean }>args).isCaptionCollapse) {
          return;
        }
        if (!gObj.getColumns().length) {
  ...
  private setRowCount(dataRowCount: number): void {
    this.ariaService.setOptions(
      (<HTMLElement>this.parent.element) as HTMLElement,
      {
  ...
  private iterateComplexColumns(
    ...
    for (let i: number = 0; i < keys.length; i++) {
      const childKeys: string[] =
        typeof obj[keys[parseInt(i.toString(), 10)]] === 'object' &&
        obj[keys[parseInt(i.toString(), 10)]] &&
        !(obj[keys[parseInt(i.toString(), 10)]] instanceof Date)
          ? Object.keys(obj[keys[parseInt(i.toString(), 10)]])
          : [];
      if (childKeys.length) {
        this.iterateComplexColumns(
          obj[keys[parseInt(i.toString(), 10)]],
          field + (keys[parseInt(i.toString(), 10)] + '.'),
          split
        );
      } else {
  ...
  private buildColumns(record: Object): void {
    ...
    const columns: string[] = Object.keys(complexCols)
      .filter((e: string) => complexCols[`${e}`] !== 'BlazId')
    ...
    for (let i: number = 0, len: number = columns.length; i < len; i++) {
      ...
      if (this.parent.enableColumnVirtualization) {
        cols[parseInt(i.toString(), 10)].width = !isNullOrUndefined(
          cols[parseInt(i.toString(), 10)].width
        )
          ? cols[parseInt(i.toString(), 10)].width
          : 200;
      }
  ...
  private instantiateRenderer(): void {
    this.renderer.addRenderer(
      RenderType.Header,
      new HeaderRender(this.parent, this.locator)
    );
    this.renderer.addRenderer(
      RenderType.Content,
      new ContentRender(this.parent, this.locator)
    );

    const cellrender: CellRendererFactory =
      this.locator.getService<CellRendererFactory>('cellRendererFactory');
    cellrender.addCellRenderer(
      CellType.Header,
      new HeaderCellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.Data,
      new CellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.StackedHeader,
      new StackedHeaderCellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.Indent,
      new IndentCellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.GroupCaption,
      new GroupCaptionCellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.GroupCaptionEmpty,
      new GroupCaptionEmptyCellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.Expand,
      new ExpandCellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.HeaderIndent,
      new HeaderIndentCellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.StackedHeader,
      new StackedHeaderCellRenderer(this.parent, this.locator)
    ...
    cellrender.addCellRenderer(
      CellType.DetailHeader,
      new DetailHeaderIndentCellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.RowDragHIcon,
      new RowDragDropHeaderRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.DetailExpand,
      new DetailExpandCellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.DetailFooterIntent,
      new IndentCellRenderer(this.parent, this.locator)
    );
    cellrender.addCellRenderer(
      CellType.RowDragIcon,
      new RowDragDropRenderer(this.parent, this.locator)
    );
  }
  ...
  public validateGroupRecords(e: ReturnType): Promise<Object> {
    ...
    const groupN: Group = <Group>e.result[parseInt(index.toString(), 10)];
    ...
    const newQuery: Query = this.data.generateQuery(true);
    const rPredicate: Predicate[] = [];
    if (this.data.isRemote()) {
      const groups: Group[] = [group0, groupN];
      for (let i: number = 0; i < groups.length; i++) {
        rPredicate.push(
          this.getPredicate(
            (groups[parseInt(i.toString(), 10)] as Group).field,
            'equal',
            (groups[parseInt(i.toString(), 10)] as Group).key
          )
        );
      }
  ...
  public updateGroupInfo(current: Object[], untouched: Object[]): Object[] {
    ...
    for (let i: number = 0; i < elements.length; i++) {
      const updatedGroup: Group = dm.executeLocal(
        new Query().where(
          new Predicate(
            'field',
            '==',
            elements[parseInt(i.toString(), 10)].field
          ).and(
            this.getPredicate(
              'key',
              'equal',
              elements[parseInt(i.toString(), 10)].key
            )
          )
        )
      )[0];
      if (!isNullOrUndefined(updatedGroup)) {
        ...
        for (let j: number = 0; j < rows.length; j++) {
          const row: AggregateRowModel = (rows as AggregateRowModel)[
            parseInt(j.toString(), 10)
          ];
          for (let k: number = 0; k < row.columns.length; k++) {
            const column: AggregateColumnModel =
              row.columns[parseInt(k.toString(), 10)];
            const types: string[] =
              column.type instanceof Array
                ? (column.type as string[])
                : ([column.type] as string[]);
            for (let l: number = 0; l < types.length; l++) {
              const key: string =
                (column as AggregateColumnModel).field +
                ' - ' +
                types[parseInt(l.toString(), 10)].toLowerCase();
              const data: Object[] = itemGroup.level
                ? updatedGroupItem.records
                : updatedGroup.items;
              const context: Object = this.parent;
              if (types[parseInt(l.toString(), 10)] === 'Custom') {
                const data: Group = itemGroup.level
                  ? updatedGroupItem
                  : updatedGroup;
                ...
                elements[parseInt(i.toString(), 10)].aggregates[`${key}`] = temp
                  ? (temp as Function).call(
                      context,
                      data,
                      row.columns[
                ...
                elements[parseInt(i.toString(), 10)].aggregates[`${key}`] =
                  DataUtil.aggregates[
                    types[parseInt(l.toString(), 10)].toLowerCase()
                  ](
                    data,
