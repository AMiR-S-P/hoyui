 ...
 * @hidden
 */
export class StackedHeaderCellRenderer
  ...
  implements ICellRenderer<Column>
{
  ...
  public render(
    ...
    if (!isNullOrUndefined(column.headerTemplate)) {
      ...
      this.appendHtml(
        div,
        this.parent.sanitize(column.headerText),
        column.getDomSetter()
      );
    }
    ...
    if (this.parent.allowResizing) {
      ...
      handler.className = cell.column.allowResizing
    ...
    if (frozenDirection(column) === 'Left') {
      ...
      for (let i: number = cols.length - 1; i >= 0; i--) {
        if (
          isChildColumn(column, cols[parseInt(i.toString(), 10)].uid) ||
