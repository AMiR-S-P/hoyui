import { Droppable, DropEventArgs } from '@hoyui/base';
import { isNullOrUndefined, extend } from '@hoyui/base';
import { setStyleAttribute, remove, updateBlazorTemplate } from '@hoyui/base';
import {
  getUpdateUsingRaf,
  appendChildren,
  setDisplayValue,
...
export class ContentRender implements IRenderer {
  ...
  private drop: Function = (e: DropEventArgs) => {
    this.parent.notify(events.columnDrop, {
      target: e.target,
  ...
  private rafCallback: Function = (args: NotifyArgs) => {
    ...
    return () => {
      this.ariaService.setBusy(
        <HTMLElement>this.getPanel().querySelector('.' + literals.content),
        false
      );
      ...
      if (this.parent.enableInfiniteScrolling) {
        if (this.parent.groupSettings.enableLazyLoading) {
          for (let i: number = 0; i < this.visibleRows.length; i++) {
            this.setRowsInLazyGroup(
              this.visibleRows[parseInt(i.toString(), 10)],
              i
            );
          }
        }
        rows = this.parent.getRowsObject();
        const prevPage: number = (<{ prevPage: number }>arg).prevPage;
        if (this.parent.infiniteScrollSettings.enableCache && prevPage) {
          const maxBlock: number = this.parent.infiniteScrollSettings.maxBlocks;
          rows = [];
          const rowIdx: number =
            parseInt(this.rowElements[0].getAttribute('data-rowindex'), 10) + 1;
          const startIdx: number = Math.ceil(
            rowIdx / this.parent.pageSettings.pageSize
          );
          for (
            let i: number = 0, count: number = startIdx;
            i < maxBlock;
            i++, count++
          ) {
            if (this.infiniteCache[parseInt(count.toString(), 10)]) {
              rows = [
                ...rows,
                ...this.infiniteCache[parseInt(count.toString(), 10)],
              ] as Row<Column>[];
            }
          }
        }
      ...
      if (this.isLoaded) {
        ...
        if (
          (this.parent.isReact || isReactChild) &&
          this.parent.element.querySelectorAll('.e-templatecell').length
        ) {
          ...
          thisRef.parent.renderTemplates(function (): void {
            thisRef.parent.trigger(events.dataBound, {}, () => {
              if (thisRef.parent.allowTextWrap) {
                thisRef.parent.notify(events.freezeRender, {
                  case: 'textwrap',
                });
              }
            });
      ...
      if (arg) {
        const action: string =
          (arg.requestType || '').toLowerCase() + '-complete';
  ...
  constructor(parent?: IGrid, serviceLocator?: ServiceLocator) {
    ...
    this.ariaService =
      this.serviceLocator.getService<AriaService>('ariaService');
    ...
    this.parent.on(
      events.refreshInfiniteModeBlocks,
  ...
  private beforeCellFocused(e: CellFocusArgs): void {
    if (
      e.byKey &&
      (e.keyArgs.action === 'upArrow' || e.keyArgs.action === 'downArrow')
    ) {
  ...
  public renderPanel(): void {
    ...
    let div: Element = this.parent.element.querySelector(
      '.' + literals.gridContent
    );
    if (div) {
      this.ariaService.setOptions(
        <HTMLElement>this.parent.element.querySelector('.' + literals.content),
        { busy: false }
      );
      this.setPanel(div);
      return;
  ...
  protected renderHorizontalScrollbar(element?: Element): void {
    const parent: Element = this.parent.createElement('div', {
      className: 'e-movablescrollbar',
    });
    const child: Element = this.parent.createElement('div', {
  ...
  public renderTable(): void {
    ...
    if (
      this.parent.enableVirtualization &&
      !isNullOrUndefined(virtualTable) &&
      !isNullOrUndefined(virtualTrack)
    ) {
      remove(virtualTable);
      remove(virtualTrack);
    }
  ...
  public createContentTable(id: string): Element {
    ...
    const table: Element = innerDiv.querySelector('.' + literals.table)
      ? innerDiv.querySelector('.' + literals.table)
    ...
    table.appendChild(
      this.parent.createElement(literals.tbody, { attrs: { role: 'rowgroup' } })
    );
  ...
  public refreshContentRows(args: NotifyArgs = {}): void {
    ...
    const isReact: boolean =
      gObj.isReact && !isNullOrUndefined(gObj.rowTemplate);
    let frag: DocumentFragment | HTMLElement = isReact
      ? gObj.createElement(literals.tbody, { attrs: { role: 'rowgroup' } })
      : document.createDocumentFragment();
    if (!this.initialPageRecords) {
      this.initialPageRecords = extend([], dataSource);
    }
    const hdrfrag: DocumentFragment = isReact
      ? gObj.createElement(literals.tbody, { attrs: { role: 'rowgroup' } })
      : document.createDocumentFragment();
    ...
    const isColumnVirtualInfiniteProcess: boolean =
      this.isInfiniteColumnvirtualization() &&
      args.requestType !== 'virtualscroll';
    ...
    let isGroupFrozenHdr: boolean =
      this.parent.frozenRows && this.parent.groupSettings.columns.length
        ? true
        : false;
    ...
    if (
      ...
      for (let i: number = 0; i < registeredTemplates.template.length; i++) {
        for (
          let j: number = 0;
          j <
          registeredTemplates.template[parseInt(i.toString(), 10)].rootNodes
            .length;
          j++
        ) {
          if (
            isNullOrUndefined(
              registeredTemplates.template[parseInt(i.toString(), 10)]
                .rootNodes[parseInt(j.toString(), 10)].parentNode
            )
          ) {
            templatetoclear.push(
              registeredTemplates.template[parseInt(i.toString(), 10)]
            );
          }
        }
    ...
    if (this.parent.enableColumnVirtualization) {
      const cellMerge: CellMergeRender<Column> = new CellMergeRender(
        this.serviceLocator,
        this.parent
      );
    ...
    if (isGroupAdaptive(gObj) && gObj.vcRows.length) {
      ...
      const scrollTop: number = !isNullOrUndefined(args.virtualInfo.offsets)
        ? args.virtualInfo.offsets.top
        : !isNullOrUndefined(args.scrollTop)
        ? args.scrollTop[`${top}`]
        : 0;
      if (scrollTop !== 0) {
        const offsets: { [x: number]: number } = gObj.vGroupOffsets;
        const bSize: number = gObj.pageSettings.pageSize / 2;
        const values: number[] = Object.keys(offsets).map(
          (key: string) => offsets[`${key}`]
        );
        for (let m: number = 0; m < values.length; m++) {
          if (scrollTop < values[parseInt(m.toString(), 10)]) {
            if (
              !isNullOrUndefined(args.virtualInfo) &&
              args.virtualInfo.direction === 'up'
            ) {
              ...
              startIndex = m * bSize;
              if (
                this.parent.contentModule.isEndBlock(m) ||
                this.parent.contentModule.isEndBlock(m + 1)
              ) {
        ...
        if (
          Math.round(
            scrollTop +
              (this.contentPanel.firstElementChild as HTMLElement).offsetHeight
          ) >= this.contentPanel.firstElementChild.scrollHeight &&
          !args.rowObject
        ) {
          blockLoad = false;
        }
    ...
    if (
      ...
      const data: Object = isVFFrozenOnly
        ? this.initialPageRecords
        : dataSource;
      const mhdrData: Row<Column>[] = (<{ generateRows?: Function }>(
        (<{ vgenerator?: Function }>this).vgenerator
    ...
    if (
      ...
      ).refRowsObj[this.parent.pageSettings.currentPage] = [];
    }
    if (
      this.parent.enableInfiniteScrolling &&
      this.parent.groupSettings.enableLazyLoading &&
    ...
    for (
      let i: number = startIndex, len: number = modelData.length;
      i < len;
      i++
    ) {
      ...
      if (
        ...
        (this.parent.enableVirtualization
          ? (this.parent.lazyLoadRender as GroupLazyLoadRenderer)
          : (this.parent.contentModule as GroupLazyLoadRenderer)
        ).refRowsObj[this.parent.pageSettings.currentPage].push(
      ...
      if (
        ...
        args.virtualInfo &&
        args.virtualInfo.blockIndexes &&
        this.rowElements.length >=
          args.virtualInfo.blockIndexes.length *
            this.parent.contentModule.getBlockSize() &&
        blockLoad
      ) {
        this.parent.currentViewData['records'] = this.rows.map(
          (m: Row<Column>) => m.data
        );
        break;
      }
      if (!gObj.rowTemplate) {
        ...
        if (
          gObj.frozenRows &&
          (i < gObj.frozenRows || isGroupFrozenHdr) &&
          !isInfiniteScroll &&
          args.requestType !== 'virtualscroll' &&
          isVFreorder &&
        ...
        if (modelData[parseInt(i.toString(), 10)].isExpand) {
          gObj.notify(
            events.expandChildGrid,
            (<HTMLTableRowElement>tr).cells[gObj.groupSettings.columns.length]
          );
        }
      } else {
        ...
        if (gObj.isReact) {
          ...
          if (gObj.requireTemplateRef) {
            ...
            thisRef.parent.renderTemplates(function (): void {
              if (gObj.frozenRows && i < gObj.frozenRows) {
                tr = refHdrfrag.childNodes[
                  parseInt(i.toString(), 10)
                ] as Element;
              } else {
                trElement = refFrag.childNodes[
                  parseInt(i.toString(), 10)
                ] as Element;
              }
              const arg: RowDataBoundEventArgs = {
                data: modelData[parseInt(i.toString(), 10)].data,
                row: trElement ? trElement : tr,
              };
              thisRef.parent.trigger(events.rowDataBound, arg);
              if (
                modelData[parseInt(i.toString(), 10)].isDataRow ||
                (thisRef.parent.enableVirtualization &&
          ...
          elements = gObj.getRowTemplate()(
            extend({ index: i }, dataSource[parseInt(i.toString(), 10)]),
            gObj,
            'rowTemplate',
            rowTemplateID,
            undefined,
        ...
        if (!gObj.isReact && (elements[0] as Element).tagName === 'TBODY') {
          for (let j: number = 0; j < elements.length; j++) {
            const isTR: boolean =
              elements[parseInt(j.toString(), 10)].nodeName.toLowerCase() ===
              'tr';
            if (
              isTR ||
              ((elements[parseInt(j.toString(), 10)] as Element)
                .querySelectorAll &&
                (
                  elements[parseInt(j.toString(), 10)] as Element
          ...
          if (gObj.frozenRows && i < gObj.frozenRows) {
            tr = !gObj.isReact
              ? appendChildren(hdrfrag, elements)
              : hdrfrag.lastElementChild;
          } else {
            // frag.appendChild(tr);
            if (!gObj.isReact) {
              tr = appendChildren(frag, elements);
            }
            trElement = gObj.isReact
      ...
      if (
        ...
        if (
          !isNullOrUndefined(gObj.rowTemplate) &&
          (gObj.isAngular || gObj.isVue3 || gObj.isVue)
        ) {
    ...
    const isReactChild: boolean =
      gObj.parentDetails &&
      gObj.parentDetails.parentInstObj &&
    ...
    if (
      ...
      (this.parent.contentModule as GroupLazyLoadRenderer).refRowsObj[
        this.parent.pageSettings.currentPage
      ] = (this.parent.contentModule as GroupLazyLoadRenderer)['groupCache'][
        this.parent.pageSettings.currentPage
      ];
    }
    if (
      ...
      this.parent.notify(events.refreshExpandandCollapse, {
        rows: (this.parent.enableVirtualization
          ? (this.parent.lazyLoadRender as GroupLazyLoadRenderer)
          : (this.parent.contentModule as GroupLazyLoadRenderer)
        ).refRowsObj[this.parent.pageSettings.currentPage],
      });
    ...
    getUpdateUsingRaf<HTMLElement>(() => {
      ...
      if (
        !this.parent.enableVirtualization &&
        (!this.parent.enableColumnVirtualization ||
          isColumnVirtualInfiniteProcess) &&
        !isInfiniteScroll
      ) {
        if (!gObj.isReact) {
      ...
      if (gObj.rowTemplate) {
        updateBlazorTemplate(
          gObj.element.id + 'rowTemplate',
          'RowTemplate',
          gObj
        );
      }
      if (
        !isNullOrUndefined(this.parent.infiniteScrollModule) &&
        ((this.parent.enableInfiniteScrolling &&
          !this.isInfiniteColumnvirtualization()) ||
          isColumnVirtualInfiniteProcess)
      ) {
        ...
        if (
          ...
          const virtualTable: Element = this.parent
            .getContent()
            .querySelector('.e-virtualtable');
          const transform: { width: number; height: number } =
            getTransformValues(virtualTable);
          (
            this.parent.contentModule as VirtualContentRenderer
          ).resetStickyLeftPos(transform.width);
      ...
      if (
        ...
        const newRow: Element =
          this.parent.element.querySelector('.e-addrow-removed');
        if (newRow) {
          remove(newRow);
        }
      }
        ...
        if (
          ...
          ((this.parent.enableVirtualization ||
            ...
            [
              ...
              'ungrouping',
              'reorder',
            ].some(function (value: string): boolean {
              return args.requestType === value;
            }))
        ) {
          this.parent.notify(events.showAddNewRowFocus, {});
  ...
  public appendContent(
    ...
    if (
      ...
      this.parent.allowTextWrap &&
      (this.parent.textWrapSettings.wrapMode === 'Header' ||
        this.parent.textWrapSettings.wrapMode === 'Both')
    ) {
      const cells: NodeListOf<HTMLTableCellElement> =
        tbody.querySelectorAll('td');
      for (let i: number = 0; i < cells.length; i++) {
        const headerCellHeight: number = parseFloat(
          document.defaultView
            .getComputedStyle(cells[parseInt(i.toString(), 10)], '::before')
            .getPropertyValue('height')
        );
        const cellHeight: number =
          cells[parseInt(i.toString(), 10)].offsetHeight;
        if (headerCellHeight > cellHeight) {
          cells[parseInt(i.toString(), 10)].style.height =
            headerCellHeight + 'px';
  ...
  private ensureFrozenHeaderRender(args: NotifyArgs): boolean {
    return !(
      ...
      (this.parent.infiniteScrollSettings.enableCache &&
        ...
        this.parent.infiniteScrollModule.requestType === 'delete' &&
  ...
  private checkCache(
    ...
    if (
      ...
      this.isAddRows = !isNullOrUndefined(
        this.infiniteCache[this.parent.pageSettings.currentPage]
      );
      if (this.isAddRows) {
        const data: Row<Column>[] = this.infiniteCache[
          this.parent.pageSettings.currentPage
        ] as Row<Column>[];
        modelData =
          this.parent.pageSettings.currentPage === 1
            ? data.slice(this.parent.frozenRows)
            : data;
      }
      return modelData;
    }
    if (
      ...
      (this.parent.enableVirtualization
        ? (this.parent.lazyLoadRender as GroupLazyLoadRenderer)
        : (this.parent.contentModule as GroupLazyLoadRenderer)
      ).getGroupCache()[this.parent.pageSettings.currentPage]
    ) {
      ...
      return this.parent.enableVirtualization
        ...
        : (this.parent.contentModule as GroupLazyLoadRenderer).initialGroupRows(
            args.requestType === 'reorder'
          );
    }
    return null;
  ...
  private getCurrentBlockInfiniteRecords(): Row<Column>[] {
    ...
    if (this.parent.infiniteScrollSettings.enableCache) {
      ...
      let index: number = parseInt(
        rows[this.parent.frozenRows].getAttribute(literals.dataRowIndex),
        10
      );
      const first: number = Math.ceil(
      ...
      const last: number = Math.ceil(
        (index + (rows.length ? 1 : 0)) / this.parent.pageSettings.pageSize
      );
      for (let i: number = first; i <= last; i++) {
        data = !data.length
          ? (this.infiniteCache[parseInt(i.toString(), 10)] as Row<Column>[])
          : data.concat(
              this.infiniteCache[parseInt(i.toString(), 10)] as Row<Column>[]
            );
      }
  ...
  private virtualFrozenHdrRefresh(
    hdrfrag: DocumentFragment,
    modelData: Row<Column>[],
    row: RowRenderer<Column>,
    args: NotifyArgs,
    dataSource: Object,
    columns: Column[]
  ): void {
    ...
    if (this.parent.enableInfiniteScrolling) {
      if (this.parent.infiniteScrollSettings.enableCache) {
        ...
        for (let i: number = 0; i < keys.length; i++) {
          rows = [
            ...rows,
            ...this.infiniteCache[keys[parseInt(i.toString(), 10)]],
          ];
        }
      } else {
  ...
  private getInfiniteMovableRows(): Row<Column>[] {
    const infiniteCacheRows: Row<Column>[] =
      this.getCurrentBlockInfiniteRecords();
    const infiniteRows: Row<Column>[] = this.parent.enableInfiniteScrolling
      ? infiniteCacheRows.length
        ? infiniteCacheRows
        : this.visibleRows
      : [];
    return infiniteRows;
  ...
  public setVisible(columns?: Column[]): void {
    ...
    const contentrows: Row<Column>[] = infiniteData
      ? infiniteData
  ...
  protected getHeaderColGroup(): Element {
    return <Element>this.parent.element
      .querySelector('.' + literals.gridHeader)
      .querySelector(literals.colGroup + ':not(.e-masked-colgroup)')
      .cloneNode(true);
  }
  ...
  public getModelGenerator(): IModelGenerator<Column> {
    return (this.generator = this.parent.allowGrouping
      ? new GroupModelGenerator(this.parent)
  ...
  public setSelection(uid: string, set: boolean, clearAll?: boolean): void {
    ...
    const row: Row<Column>[] = (<Row<Column>[]>this.getRows()).filter(
      (row: Row<Column>) => clearAll || uid === row.uid
    );
  ...
  private getInfiniteRowIndex(index: number): number {
    if (
      ...
      const firstRowIndex: number = parseInt(
        this.parent
          .getRows()
          [parseInt(idx.toString(), 10)].getAttribute(literals.dataRowIndex),
        10
      );
  ...
  public immutableModeRendering(args: NotifyArgs = {}): void {
    ...
    const newRowObjs: Row<Column>[] = [];
    const oldIndexes: Object = {};
    const oldRowObjs: Row<Column>[] = gObj.getRowsObject().slice();
    const batchChangeKeys: Object = this.getBatchEditedRecords(key, oldRowObjs);
    const newIndexes: Object = {};
    const hasBatch: boolean = Object.keys(batchChangeKeys).length !== 0;
    if (
      gObj.getContent().querySelector('.e-emptyrow') ||
      args.requestType === 'reorder' ||
      this.parent.groupSettings.columns.length
    ) {
      ...
      const trs: Element[] = [].slice.call(
        this.getTable().querySelector(literals.tbody).children
      );
        ...
        if (prevLen === currentLen) {
          for (let i: number = 0; i < currentLen; i++) {
            if (
              ...
              trs[parseInt(i.toString(), 10)].classList.contains(
                'e-insertedrow'
              )
            ) {
              trs.splice(i, 1);
            ...
            newKeys[dataSource[parseInt(i.toString(), 10)][`${key}`]] = oldKeys[
              this.prevCurrentView[parseInt(i.toString(), 10)][`${key}`]
            ] = i;
            newIndexes[parseInt(i.toString(), 10)] =
              dataSource[parseInt(i.toString(), 10)][`${key}`];
            oldRowElements[oldRowObjs[parseInt(i.toString(), 10)].uid] =
              trs[parseInt(i.toString(), 10)];
            oldIndexes[parseInt(i.toString(), 10)] =
              this.prevCurrentView[parseInt(i.toString(), 10)][`${key}`];
          }
        } else {
          for (let i: number = 0; i < currentLen; i++) {
      ...
      for (let i: number = 0; i < dataSource.length; i++) {
        const oldIndex: number =
          oldKeys[dataSource[parseInt(i.toString(), 10)][`${key}`]];
        if (!isNullOrUndefined(oldIndex)) {
          let isEqual: boolean = false;
          if (this.parent.enableDeepCompare) {
            isEqual = this.objectEqualityChecker(
              this.prevCurrentView[parseInt(oldIndex.toString(), 10)],
              dataSource[parseInt(i.toString(), 10)]
            );
          }
          const tr: HTMLTableRowElement = oldRowElements[
            oldRowObjs[parseInt(oldIndex.toString(), 10)].uid
          ] as HTMLTableRowElement;
          newRowObjs.push(oldRowObjs[parseInt(oldIndex.toString(), 10)]);
          if (
            this.rowElements[parseInt(oldIndex.toString(), 10)] &&
            this.rowElements[parseInt(oldIndex.toString(), 10)].getAttribute(
              'data-uid'
            ) === newRowObjs[parseInt(i.toString(), 10)].uid &&
            ((hasBatch &&
              isNullOrUndefined(
                batchChangeKeys[newIndexes[parseInt(i.toString(), 10)]]
              )) ||
              (!hasBatch &&
                (isEqual ||
                  this.prevCurrentView[parseInt(oldIndex.toString(), 10)] ===
            ...
            if (oldIndex !== i) {
              this.refreshImmutableContent(
                i,
                tr,
                newRowObjs[parseInt(i.toString(), 10)]
              );
            }
          ...
          if (
            ...
            oldRowObjs[parseInt(oldIndex.toString(), 10)].setRowValue(
              dataSource[parseInt(i.toString(), 10)]
            );
          }
          tbody.appendChild(tr);
          ...
          const modelData: Row<Column>[] = this.generator.generateRows(
            [dataSource[parseInt(i.toString(), 10)]],
            args
          );
      ...
      if (args) {
        const action: string =
          (args.requestType || '').toLowerCase() + '-complete';
  ...
  private objectEqualityChecker(old: Object, next: Object): boolean {
    ...
    for (let i: number = 0; i < keys.length; i++) {
      if (
        old[keys[parseInt(i.toString(), 10)]] !==
        next[keys[parseInt(i.toString(), 10)]]
      ) {
        const isDate: boolean =
          old[keys[parseInt(i.toString(), 10)]] instanceof Date &&
          next[keys[parseInt(i.toString(), 10)]] instanceof Date;
        if (
          !isDate ||
          (old[keys[parseInt(i.toString(), 10)]] as Date).getTime() !==
            (next[keys[parseInt(i.toString(), 10)]] as Date).getTime()
        ) {
          isEqual = false;
          break;
        }
  ...
  private updateCellIndex(rowEle: HTMLTableRowElement, index: number): void {
    for (let i: number = 0; i < rowEle.cells.length; i++) {
      rowEle.cells[parseInt(i.toString(), 10)].setAttribute(
        'index',
        index.toString()
      );
    }
