...
export class DateFilterUI implements IFilterMUI {
  ...
  public write(args: {
    ...
    const dateuiObj: DatePicker | DateTimePicker = (<EJ2Intance>(
      document.querySelector('#dateui-' + args.column.uid)
    )).ej2_instances[0];
    dateuiObj.value = !isNullOrUndefined(args.filteredValue)
      ? (new Date(args.filteredValue as string) as Date)
      : (null as Date);
  }
  ...
  public read(
    ...
    const dateuiObj: DatePicker | DateTimePicker = (<EJ2Intance>(
      document.querySelector('#dateui-' + column.uid)
    )).ej2_instances[0];
  ...
  private destroy(): void {
    ...
    if (
      isNullOrUndefined(this.datePickerObj) ||
      this.datePickerObj.isDestroyed
    ) {
      return;
    }
