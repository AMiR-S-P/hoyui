import { isNullOrUndefined, getValue, closest } from '@hoyui/base';
 ...
 * @hidden
 */
export class FilterCellRenderer
  ...
  implements ICellRenderer<Column>
{
  ...
  public render(cell: Cell<Column>, data: Object): Element {
    ...
    if (column.filterTemplate) {
      ...
      if (column.type !== 'checkbox') {
        if (
          (isNullOrUndefined(column.allowFiltering) || column.allowFiltering) &&
          !isNullOrUndefined(column.filterBarTemplate)
        ) {
          node.classList.add('e-fltrtemp');
          attributes(innerDIV, {
            class: 'e-fltrtempdiv',
          ...
          input = this.parent.createElement('input', {
            ...
            attrs: {
              ...
              title:
                column.headerText +
                (cell.attributes as { title: string }).title,
        ...
        if (clearIconElem) {
          clearIconElem.setAttribute(
            'title',
            this.parent.localeObj.getConstant('ClearButton')
          );
        }
        if (!column.visible) {
          node.classList.add('e-hide');
        ...
        if (
          ...
          let templateWrite: Function = column.filterBarTemplate
            .write as Function;
          const args: { element: Element; column: Column } = {
            element: input,
            column: column,
          };
  ...
  private operatorIconRender(
    ...
    if (column.filter && column.filter.operator) {
      ...
      for (
        let i: number = 0, a: PredicateModel[] = gObj.filterSettings.columns;
        i < a.length;
        i++
      ) {
    ...
    this.dropOptr = new DropDownList({
      ...
      beforeOpen: function (): void {
        ...
        this.dataSource =
          operator[gObj.getColumnByUid(this.element.id).type + 'Operator'];
        for (let i: number = 0; i < this.dataSource.length; i++) {
          if (
            column.filter &&
            column.filter.operator &&
            isNullOrUndefined(gObj.filterModule.operators[column.field]) &&
            this.dataSource[parseInt(i.toString(), 10)].value ===
              column.filter.operator
          ) {
      ...
      cssClass: this.parent.cssClass
        ? 'e-popup-flbar' + ' ' + this.parent.cssClass
        : 'e-popup-flbar',
    });
  ...
  private internalEvent(e: object): void {
    ...
    const col: Column = gObj.getColumnByUid(
      (<{ element?: HTMLElement }>e).element.getAttribute('id')
    );
