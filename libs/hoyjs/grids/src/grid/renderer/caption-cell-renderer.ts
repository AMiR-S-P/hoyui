 ...
 * @hidden
 */
export class GroupCaptionCellRenderer
  ...
  implements ICellRenderer<Column>
{
  ...
  public render(cell: Cell<Column>, data: GroupedData): Element {
    ...
    const domSetter: string = column.getDomSetter
      ? column.getDomSetter()
      : 'innerHTML';
    let result: Element[];
    ...
    if (cell.isForeignKey) {
      fKeyValue = this.format(
        cell.column,
        (cell.column.valueAccessor as Function)('foreignKey', data, cell.column)
      );
    }
    const value: string = cell.isForeignKey
      ? fKeyValue
      : cell.column.enableGroupByFormat
      ? data.key
      : this.format(
          cell.column,
          (cell.column.valueAccessor as Function)('key', data, cell.column)
        );
    for (let j: number = 0; j < gObj.aggregates.length; j++) {
      for (
        let i: number = 0;
        i < gObj.aggregates[parseInt(j.toString(), 10)].columns.length;
        i++
      ) {
        if (
          gObj.getVisibleColumns()[0].field ===
            gObj.aggregates[parseInt(j.toString(), 10)].columns[
          ...
          const gCaptionTemp: string | Function = gObj.aggregates[
          ...
          if (typeof gCaptionTemp === 'string' && gCaptionTemp.includes('$')) {
            gTemplateValue =
              (
                gObj.aggregates[parseInt(j.toString(), 10)].columns[
                  parseInt(i.toString(), 10)
                ].groupCaptionTemplate as string
              ).split('$')[0] +
              data[gObj.getVisibleColumns()[0].field][
                gObj.aggregates[parseInt(j.toString(), 10)].columns[
                  parseInt(i.toString(), 10)
                ].type
              ] +
            ...
            const column: AggregateColumn = <AggregateColumn>(
              gObj.aggregates[parseInt(j.toString(), 10)].columns[
                parseInt(i.toString(), 10)
              ]
            );
            const tempObj: { fn: Function; property: string } =
              column.getTemplate(CellType.CaptionSummary);
            const tempID: string = '';
            if (!isNullOrUndefined(tempObj)) {
              const tempValue: NodeList = tempObj.fn(
              ...
              if (
                this.parent.isReact &&
                typeof column.groupCaptionTemplate !== 'string'
              ) {
                this.parent.renderTemplates(function (): void {
                  if (tempValue && tempValue.length) {
                    if (
                      !isNullOrUndefined(gObj.groupSettings.captionTemplate)
                    ) {
                      node.appendChild(tempValue[0]);
                    } else {
                      (node as HTMLElement).innerText +=
                        ' ' + tempValue[0].textContent;
                    }
                  }
                });
              } else {
    ...
    if (!isNullOrUndefined(gObj.groupSettings.captionTemplate)) {
      const isReactCompiler: boolean =
        this.parent.isReact &&
        typeof gObj.groupSettings.captionTemplate !== 'string';
      ...
      if (isReactCompiler || isReactChild) {
        ...
        templateCompiler(gObj.groupSettings.captionTemplate as string)(
          data,
          this.parent,
          'captionTemplate',
          tempID,
      ...
      if (!isReactCompiler && !isReactChild) {
        ...
        if (
          gTemplateValue &&
          gTemplateValue.length &&
          gTemplateValue[0].textContent
        ) {
      ...
      if (gObj.groupSettings.enableLazyLoading) {
        node[`${domSetter}`] =
          this.parent.sanitize(cell.column.headerText) +
          ': ' +
        ...
        node[`${domSetter}`] =
          ...
          ' ' +
          (data.count < 2
            ? this.localizer.getConstant('Item')
            : this.localizer.getConstant('Items')) +
          (gTemplateValue ? '   ' + gTemplateValue : '');
      }
    }
    ...
    node.setAttribute(
      'aria-label',
      node.innerHTML + this.localizer.getConstant('GroupCaption')
    );
 ...
 * @hidden
 */
export class GroupCaptionEmptyCellRenderer
  extends CellRenderer
  implements ICellRenderer<Column>
{
  public element: HTMLElement = this.parent.createElement('TD', {
  ...
  public render(
    cell: Cell<Column>,
    data: { field: string; key: string; count: number }
  ): Element {
    const node: Element = this.element.cloneNode() as Element;
    node.innerHTML = '&nbsp;';
    node.setAttribute('colspan', cell.colSpan.toString());
