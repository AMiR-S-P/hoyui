...
import { EventHandler, isNullOrUndefined, remove, Browser } from '@hoyui/base';
import {
  parentsUntil,
  addBiggerDialog,
...
export class ResponsiveDialogRenderer implements IAction {
  ...
  public addEventListener(): void {
    this.evtHandlers = [
      { event: events.filterDialogClose, handler: this.closeCustomDialog },
      {
  ...
  private renderCustomFilterDiv(): void {
    const header: HTMLElement = this.customResponsiveDlg.element.querySelector(
    ...
    const content: HTMLElement =
      this.customResponsiveDlg.element.querySelector('.e-dlg-content');
  ...
  private renderResponsiveContextMenu(args: {
    target: HTMLElement;
    header: string;
    isOpen: boolean;
    col: Column;
    ...
    if (this.action === ResponsiveDialogAction.isFilter) {
      ...
      const header: HTMLElement =
        this.customResponsiveDlg.element.querySelector('.e-dlg-header-content');
      const closeBtn: HTMLElement = header.querySelector(
        '.e-dlg-closeicon-btn'
      );
      ...
      if (args.isOpen) {
        ...
        this.backBtn = new Button({
          cssClass: this.parent.cssClass
            ? 'e-res-back-btn' + ' ' + this.parent.cssClass
            : 'e-res-back-btn',
        });
  ...
  private refreshCustomFilterOkBtn(args: { disabled: boolean }): void {
    ...
    if (
      this.parent.columnChooserModule &&
      this.parent.columnChooserModule.responsiveDialogRenderer.saveBtn
    ) {
      this.parent.columnChooserModule.responsiveDialogRenderer.saveBtn.disabled =
        args.disabled;
    }
  }
  ...
  private columnMenuResponsiveContent(
    ...
    const cDiv: HTMLElement = this.parent.createElement('div', {
      className:
        'e-responsivecoldiv e-responsive' +
        str.toLowerCase() +
        'div' +
        (disabled ? ' e-disabled' : ''),
    });
    ...
    const icon: HTMLElement = this.parent.createElement('span', {
      innerHTML: locale,
  ...
  private renderResponsiveContent(col?: Column, column?: Column): HTMLElement {
    ...
    const isColumnChooser: boolean =
      this.action === ResponsiveDialogAction.isColumnChooser;
    if (col || isColumnChooser) {
      this.filterParent = this.parent.createElement('div', {
        className:
          (isColumnChooser ? 'e-maincolumnchooserdiv ' : '') +
          'e-mainfilterdiv e-default-filter',
      ...
      if (
        ...
        this.action === ResponsiveDialogAction.isColMenu
      ) {
        this.columnMenuResponsiveContent(
          'AutoFitAll',
          gObj.localeObj.getConstant('AutoFitAll')
        );
        this.columnMenuResponsiveContent(
          'AutoFit',
          gObj.localeObj.getConstant('AutoFit')
        );
        if (column.allowGrouping && gObj.allowGrouping) {
          this.columnMenuResponsiveContent(
            'Group',
            gObj.localeObj.getConstant('Group'),
            gObj.groupSettings.columns.indexOf(column.field) >= 0
          );
          this.columnMenuResponsiveContent(
            'UnGroup',
            gObj.localeObj.getConstant('Ungroup'),
            gObj.groupSettings.columns.indexOf(column.field) < 0
          );
        }
        if (column.allowSorting && gObj.allowSorting) {
          ...
          for (let i: number = 0; i < sortColumns.length; i++) {
            if (
              sortColumns[parseInt(i.toString(), 10)].field === column.field
            ) {
          ...
          this.columnMenuResponsiveContent(
            'ascending',
            gObj.localeObj.getConstant('SortAscending'),
            direction === 'Ascending'
          );
          this.columnMenuResponsiveContent(
            'descending',
            gObj.localeObj.getConstant('SortDescending'),
        ...
        if (gObj.showColumnChooser) {
          this.columnMenuResponsiveContent(
            'Column',
            gObj.localeObj.getConstant('Columnchooser')
          );
        }
        if (column.allowFiltering && gObj.allowFiltering) {
          this.columnMenuResponsiveContent(
            'Filter',
            gObj.localeObj.getConstant('FilterMenu')
          );
        }
      } else {
        const cols: Column[] = gObj.getColumns();
        const sortBtnParent: HTMLElement = gObj.createElement('div', {
        ...
        const isFilter: boolean =
          this.action === ResponsiveDialogAction.isFilter;
        if (isFilter) {
          for (let i: number = 0; i < gObj.filterSettings.columns.length; i++) {
            filteredCols.push(
              gObj.filterSettings.columns[parseInt(i.toString(), 10)].field
            );
          }
        }
        for (let i: number = 0; i < cols.length; i++) {
          if (
            ...
            (!cols[parseInt(i.toString(), 10)].allowSorting && isSort) ||
            (!cols[parseInt(i.toString(), 10)].allowFiltering && isFilter)
          ) {
            continue;
          }
          const cDiv: HTMLElement = gObj.createElement('div', {
          ...
          const span: HTMLElement = gObj.createElement('span', {
          ...
          if (isSort) {
            ...
            const button: HTMLElement = gObj.createElement('button', {
              id:
                gObj.element.id +
                cols[parseInt(i.toString(), 10)].field +
                'sortbutton',
            ...
            const btnObj: Button = new Button({
              cssClass: this.parent.cssClass
                ? 'e-ressortbutton' + ' ' + this.parent.cssClass
                : 'e-ressortbutton',
            });
            ...
            if (
              !isNullOrUndefined(
                this.parent.sortSettings.columns[parseInt(index.toString(), 10)]
              )
            ) {
              buttonInnerText =
                this.parent.sortSettings.columns[parseInt(index.toString(), 10)]
                  .direction === 'Ascending'
                  ? this.parent.localeObj.getConstant('AscendingText')
                  : this.parent.localeObj.getConstant('DescendingText');
            }
            button.innerHTML =
              index > -1
                ? buttonInnerText
          ...
          if (
            isFilter &&
            filteredCols.indexOf(cols[parseInt(i.toString(), 10)].field) > -1
          ) {
            const divIcon: HTMLElement = gObj.createElement('div', {
              className: 'e-icons e-res-icon e-filtersetdiv',
            });
            const iconSpan: HTMLElement = gObj.createElement('span', {
      ...
      EventHandler.add(
        this.customColumnDiv,
        Browser.isDevice ? 'touchend' : 'click',
        this.customFilterColumnClickHandler,
        this
      );
  ...
  private getSortedFieldsAndDirections(name: string): string[] {
    ...
    for (let i: number = 0; i < this.parent.sortSettings.columns.length; i++) {
      fields.push(
        this.parent.sortSettings.columns[parseInt(i.toString(), 10)][`${name}`]
      );
    }
    return fields;
  }
  ...
  private sortButtonClickHandler(target: Element): void {
    if (target) {
      const columndiv: Element = parentsUntil(
        target as Element,
        'e-responsivecoldiv'
      );
      ...
      const directionTxt: string =
        txt === this.parent.localeObj.getConstant('NoneText')
          ? this.parent.localeObj.getConstant('AscendingText')
          : txt === this.parent.localeObj.getConstant('AscendingText')
          ? this.parent.localeObj.getConstant('DescendingText')
          : this.parent.localeObj.getConstant('NoneText');
      const direction: string =
        directionTxt === this.parent.localeObj.getConstant('AscendingText')
          ? 'Ascending'
          : directionTxt === this.parent.localeObj.getConstant('DescendingText')
          ? 'Descending'
          : 'None';
  ...
  private resetSortButtons(target?: Element): void {
    const buttons: HTMLElement[] = [].slice.call(
      this.customColumnDiv.getElementsByClassName('e-ressortbutton')
    );
  ...
  private customFilterColumnClickHandler(e: MouseEvent): void {
    ...
    if (
      parentsUntil(target, 'columnmenudiv') &&
      this.action === ResponsiveDialogAction.isColMenu &&
      !parentsUntil(target, 'e-disabled')
    ) {
      ...
      if (parentsUntil(target, 'e-responsiveautofitalldiv')) {
        ...
        if (gObj.filterModule) {
          gObj.filterModule.responsiveDialogRenderer.showResponsiveDialog(
            column
          );
        }
      }
    ...
    if (gObj.filterSettings.type !== 'FilterBar') {
      if (
        target.classList.contains('e-responsivecoldiv') ||
        target.parentElement.classList.contains('e-responsivecoldiv')
      ) {
        ...
        target.classList.contains('e-filterset') ||
        target.parentElement.classList.contains('e-filtersetdiv')
      ) {
        ...
        if (colDiv) {
          ...
          const col: Column = gObj.getColumnByField(field);
          if (
            col.filter.type === 'Menu' ||
            (!col.filter.type && gObj.filterSettings.type === 'Menu')
          ) {
  ...
  public showResponsiveDialog(col?: Column, column?: Column): void {
    if (
      ...
      if (this.parent.enableAdaptiveUI && col) {
        ...
        this.parent.filterModule.filterModule.openDialog(
          this.parent.filterModule.createOptions(col, undefined)
        );
      }
      ...
      if (
        ...
        const filterBase: CheckBoxFilterBase =
          this.parent.filterSettings.type === 'CheckBox'
            ? this.parent.filterModule.filterModule.checkBoxBase
            : this.parent.filterModule.filterModule.excelFilterBase;
        filterBase.showMask();
      }
    }
  }
  ...
  private renderCustomFilterDialog(col?: Column, column?: Column): void {
    ...
    if (
      this.action === ResponsiveDialogAction.isFilter &&
      gObj.filterSettings.type === 'FilterBar'
    ) {
      return;
    }
    const colMenu: string = isColMenu ? 'e-customcolumnmenudiv ' : '';
    const outerDiv: HTMLElement = this.parent.createElement('div', {
      id: gObj.element.id + (isColMenu ? 'customcolumnmenu' : 'customfilter'),
      className: this.parent.cssClass
        ? colMenu +
          'e-customfilterdiv e-responsive-dialog ' +
          this.parent.cssClass
        : colMenu + 'e-customfilterdiv e-responsive-dialog',
    });
  ...
  private getDialogOptions(
    ...
    const options: Dialog = new Dialog({
      ...
      target: this.parent.adaptiveDlgTarget
        ? this.parent.adaptiveDlgTarget
        : document.body,
      visible: false,
    ...
    if (isCustomFilter) {
      ...
      options.cssClass =
        ...
        this.action === ResponsiveDialogAction.isFilter
          ? 'e-res' + id + ' e-row-responsive-filter'
          : 'e-res' + id;
    }
    return options;
  }
  ...
  private renderResponsiveDialog(col?: Column): void {
    ...
    let id: string =
      this.action === ResponsiveDialogAction.isFilter ? 'filter' : 'sort';
    id =
      this.action === ResponsiveDialogAction.isColumnChooser
        ? 'columnchooser'
        : id;
    const outerDiv: HTMLElement = this.parent.createElement('div', {
      ...
      className: this.parent.cssClass
        ? 'e-res' + id + 'div e-responsive-dialog ' + this.parent.cssClass
        : 'e-res' + id + 'div e-responsive-dialog',
    });
    this.parent.element.appendChild(outerDiv);
  ...
  private dialogOpen(): void {
    if (
      this.action === ResponsiveDialogAction.isSort &&
      this.parent.allowMultiSorting
    ) {
      for (
        let i: number = 0;
        i < this.parent.sortSettings.columns.length;
        i++
      ) {
        this.sortedCols.push(
          this.parent.sortSettings.columns[parseInt(i.toString(), 10)].field
        );
        const sortField: string =
          this.parent.sortSettings.columns[parseInt(i.toString(), 10)].field;
        const sortDirection: SortDirection =
          this.parent.sortSettings.columns[parseInt(i.toString(), 10)]
            .direction;
  ...
  private beforeDialogClose(args: { element: Element }): void {
    this.isDialogClose =
      args.element &&
      !args.element.querySelector('.e-xl-customfilterdiv') &&
  ...
  private sortColumn(): void {
    ...
    for (let i: number = 0; i < this.sortPredicate.length; i++) {
      this.parent.sortColumn(
        ...
        this.sortPredicate[parseInt(i.toString(), 10)].direction,
        this.parent.allowMultiSorting
      );
    }
    if (!this.sortPredicate.length) {
  ...
  private getHeaderTitle(args: ResponsiveDialogArgs, col: Column): string {
    ...
    if (this.action === ResponsiveDialogAction.isEdit) {
      title =
        gObj.localeObj.getConstant('EditFormTitle') + args.primaryKeyValue[0];
      ...
      title = col
        ...
        : gObj.localeObj.getConstant('FilterButton');
    } else if (this.action === ResponsiveDialogAction.isSort) {
  ...
  private getDialogName(action: ResponsiveDialogAction): string {
    let name: string;
    if (
      action === ResponsiveDialogAction.isAdd ||
      action === ResponsiveDialogAction.isEdit
    ) {
  ...
  private getButtonText(action: ResponsiveDialogAction): string {
    ...
    if (
      ...
      action === ResponsiveDialogAction.isFilter ||
      this.action === ResponsiveDialogAction.isSort ||
      action === ResponsiveDialogAction.isColumnChooser ||
      this.action === ResponsiveDialogAction.isColumnChooser
    ) {
      text = 'OKButton';
    }
   ...
   */
  public renderResponsiveHeader(
    col: Column,
    args?: ResponsiveDialogArgs,
    isCustomFilter?: boolean
  ): HTMLElement | string {
    const gObj: IGrid = this.parent;
    gObj.on(events.enterKeyHandler, this.keyHandler, this);
    const id: string = gObj.element.id + this.getDialogName(this.action);
    const header: HTMLElement | string = gObj.createElement('div', {
      className: 'e-res-custom-element',
    });
    const titleDiv: HTMLElement = gObj.createElement('div', {
    ...
    if (!isCustomFilter) {
      this.saveBtn = new Button({
        cssClass: this.parent.cssClass
          ? 'e-primary e-flat e-res-apply-btn' + ' ' + this.parent.cssClass
          : 'e-primary e-flat e-res-apply-btn',
      });
      saveBtn.innerHTML = gObj.localeObj.getConstant(
        this.getButtonText(this.action)
      );
  ...
  private removeCustomFilterElement(): void {
    const elem: Element = document.getElementById(
      this.parent.element.id + 'customcolumnmenu'
    );
    if (elem) {
      remove(elem);
  ...
  private dialogHdrBtnClickHandler(): void {
    if (
      this.action === ResponsiveDialogAction.isEdit ||
      this.action === ResponsiveDialogAction.isAdd
    ) {
      this.parent.endEdit();
    } else if (this.action === ResponsiveDialogAction.isFilter) {
  ...
  private closeCustomDialog(): void {
    if (this.isCustomDlgRender) {
      const mainfilterdiv: HTMLElement =
        this.customResponsiveDlg.element.querySelector('.e-mainfilterdiv');
  ...
  private destroyCustomFilterDialog(): void {
    ...
    const elem: Element = document.getElementById(
      this.customResponsiveDlg.element.id
    );
    if (
      this.customResponsiveDlg &&
      !this.customResponsiveDlg.isDestroyed &&
      elem
    ) {
  ...
  private closeCustomFilter(): void {
    if (!this.isDialogClose && this.customFilterDlg) {
      const customEle: Element = document.getElementById(
        this.customFilterDlg.element.id
      );
      if (
        this.customFilterDlg &&
        !this.customFilterDlg.isDestroyed &&
        customEle
      ) {
  ...
  private removeCustomDlgFilterEle(target?: Element): void {
    if (target) {
      ...
      for (let i: number = 0; i < child.length; i++) {
        target =
          child[parseInt(i.toString(), 10)].querySelector('.e-filtersetdiv');
        if (target) {
          remove(target);
          i--;
        }
