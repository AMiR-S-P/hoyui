...
import { extend, isUndefined, KeyboardEventArgs } from '@hoyui/base';
...
export class NumberFilterUI implements IFilterMUI {
  ...
  public create(args: IFilterCreate): void {
    ...
    this.numericTxtObj = new NumericTextBox(
      extend(
        {
          format:
            typeof args.column.format === 'string' ||
            isUndefined(args.column.format)
              ? args.column.format
  ...
  public write(args: {
    ...
    const numberuiObj: NumericTextBox = (<EJ2Intance>(
      document.querySelector('#numberui-' + args.column.uid)
    )).ej2_instances[0];
  ...
  public read(
    ...
    const numberuiObj: NumericTextBox = (<EJ2Intance>(
      document.querySelector('#numberui-' + column.uid)
    )).ej2_instances[0];
