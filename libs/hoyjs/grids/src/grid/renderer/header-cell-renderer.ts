import { isNullOrUndefined, extend, addClass } from '@hoyui/base';
import { attributes } from '@hoyui/base';
...
export class HeaderCellRenderer
  ...
  implements ICellRenderer<Column>
{
  public element: HTMLElement = this.parent.createElement('TH', {
    className: 'e-headercell',
    attrs: { tabindex: '-1', role: 'columnheader' },
  });
  private ariaService: AriaService = new AriaService();
  private hTxtEle: Element = this.parent.createElement('span', {
    className: 'e-headertext',
  });
  private sortEle: Element = this.parent.createElement('div', {
    className: 'e-sortfilterdiv e-icons',
    attrs: { 'aria-hidden': 'true' },
  });
  private gui: Element = this.parent.createElement('div');
  private chkAllBox: Element = this.parent.createElement('input', {
    className: 'e-checkselectall',
    attrs: {
      type: 'checkbox',
      'aria-label': this.localizer.getConstant('SelectAllCheckbox'),
    },
  });
  /**
  ...
  public render(
    ...
    const fltrMenuEle: Element = this.parent.createElement('div', {
  ...
  private prepareHeader(
    ...
    const ariaAttr: IAriaOptions<boolean> = {};
    ...
    if (column.type !== 'checkbox') {
      ...
      const checkAllWrap: Element = createCheckBox(
        this.parent.createElement,
        false,
        { checked: false, label: ' ' }
      );
      this.chkAllBox.id = 'checkbox-' + column.uid;
      checkAllWrap.insertBefore(
        this.chkAllBox.cloneNode(),
        checkAllWrap.firstChild
      );
    ...
    if (
      ...
      elementDesc = elementDesc.length
        ? elementDesc + '. ' + this.localizer.getConstant('FilterDescription')
        : this.localizer.getConstant('FilterDescription');
      node.classList.add('e-fltr-icon');
      const matchFlColumns: Object[] = [];
      if (
        this.parent.filterSettings.columns.length &&
        this.parent.filterSettings.columns.length !== matchFlColumns.length
      ) {
        const foreignColumn: Column[] = this.parent.getForeignKeyColumns();
        for (
          let index: number = 0;
          index < this.parent.columns.length;
          index++
        ) {
          for (
            let count: number = 0;
            count < this.parent.filterSettings.columns.length;
            count++
          ) {
            if (
              this.parent.filterSettings.columns[parseInt(count.toString(), 10)]
                .field === column.field ||
    ...
    if (this.parent.allowSorting && column.allowSorting) {
      ariaAttr.sort = 'none';
      elementDesc = elementDesc.length
        ? elementDesc + '. ' + this.localizer.getConstant('SortDescription')
        : this.localizer.getConstant('SortDescription');
    }
    if (
      (this.parent.allowGrouping && column.allowGrouping) ||
      (this.parent.allowReordering && column.allowReordering)
    ) {
      ariaAttr.grabbed = false;
      elementDesc = elementDesc.length
        ? elementDesc + '. ' + this.localizer.getConstant('GroupDescription')
        : this.localizer.getConstant('GroupDescription');
    }
    if (
      ...
      elementDesc = elementDesc.length
        ? elementDesc +
          '. ' +
          this.localizer.getConstant('ColumnMenuDescription')
        : this.localizer.getConstant('ColumnMenuDescription');
    }
    ...
    if (!isNullOrUndefined(column.headerTemplate)) {
      ...
      const headerTempID: string =
        gridObj.element.id + column.uid + 'headerTemplate';
      const str: string = 'isStringTemplate';
      const col: Column = column;
      const isReactCompiler: boolean =
        this.parent.isReact && typeof column.headerTemplate !== 'string';
    ...
    if (
      column.clipMode === 'Clip' ||
      (!column.clipMode && this.parent.clipMode === 'Clip')
    ) {
    ...
    if (elementDesc) {
      const titleElem: HTMLElement = this.parent.createElement('span', {
    ...
    node.setAttribute(
      'aria-rowspan',
      (!isNullOrUndefined(cell.rowSpan) ? cell.rowSpan : 1).toString()
    );
  ...
  private extendPrepareHeader(column: Column, node: Element): Element {
    if (
      ...
      const element: Element = this.parent.createElement('div', {
      ...
      if (
        this.parent.filterSettings.columns.length &&
        this.parent.filterSettings.columns.length !==
          matchFilteredColumns.length
      ) {
        for (let i: number = 0; i < this.parent.columns.length; i++) {
          for (
            let j: number = 0;
            j < this.parent.filterSettings.columns.length;
            j++
          ) {
            if (
              this.parent.filterSettings.columns[parseInt(j.toString(), 10)]
                .field === column.field
            ) {
    ...
    if (this.parent.allowResizing) {
      ...
      handler.className = column.allowResizing
