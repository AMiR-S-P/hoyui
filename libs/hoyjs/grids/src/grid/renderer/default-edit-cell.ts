import { isNullOrUndefined, extend, KeyboardEventArgs } from '@hoyui/base';
...
export class DefaultEditCell extends EditCellBase implements IEditCell {
  public create(args: {
    ...
    return createEditElement(
      this.parent,
      args.column,
      'e-field e-input e-defaultcell',
      attr
    );
  }
  ...
  public read(element: Element): string {
    if (
      (<HTMLInputElement>element).type === 'hidden' &&
      !isNullOrUndefined((<EJ2Intance>element).ej2_instances[0]) &&
  ...
  public write(args: {
    ...
    const props: Object = {
      element: args.element as HTMLInputElement,
      floatLabelType:
        this.parent.editSettings.mode !== 'Dialog' ? 'Never' : 'Always',
    ...
    if (
      ...
      !isNullOrUndefined(col.edit.params) &&
      (col.edit.params as any).multiline
    ) {
      const cellValue: string = (col.valueAccessor as Function)(
        col.field,
        args.rowData,
        col
      ) as string;
