...
import { isNullOrUndefined, removeClass } from '@hoyui/base';
...
import {
  ...
  ScrollDirection,
} from '../services/intersection-observer';
...
import { setStyleAttribute } from '@hoyui/base';
...
export class VirtualContentRenderer extends ContentRender implements IRenderer {
  ...
  public renderTable(): void {
    this.header = <VirtualHeaderRenderer>(
      this.locator
        .getService<RendererFactory>('rendererFactory')
        .getRenderer(RenderType.Header)
    );
    super.renderTable();
    this.virtualEle.table = <HTMLElement>this.getTable();
    this.virtualEle.content = this.content = <HTMLElement>(
      this.getPanel().querySelector('.' + literals.content)
    );
    ...
    const debounceEvent: boolean =
      this.parent.dataSource instanceof DataManager &&
      !this.parent.dataSource.dataSource.offline;
  ...
  public getReorderedFrozenRows(args: NotifyArgs): Row<Column>[] {
    ...
    const firstRecords: object[] =
      this.parent.renderModule.data.dataManager.dataSource.json.slice(
        0,
        recordslength
      );
    const virtualRows: Row<Column>[] = this.vgenerator.generateRows(
      firstRecords,
      args
    );
  ...
  private scrollListener(scrollArgs: ScrollArg): void {
    if (
      !this.parent.enableVirtualization &&
      this.parent.enableColumnVirtualization &&
      (scrollArgs.direction === 'up' || scrollArgs.direction === 'down')
    ) {
      return;
    }
    ...
    if (isNullOrUndefined(document.activeElement)) {
      ...
      this.isFocused =
        this.content ===
          closest(document.activeElement, '.' + literals.content) ||
        this.content === document.activeElement;
    }
    ...
    const viewInfo: VirtualInfo = (this.currentInfo = this.getInfoFromView(
      scrollArgs.direction,
      info,
      scrollArgs.offset
    ));
    if (isGroupAdaptive(this.parent)) {
      if (
        viewInfo.blockIndexes &&
        this.prevInfo.blockIndexes.toString() ===
          viewInfo.blockIndexes.toString()
      ) {
        this.parent.removeMaskRow();
        return;
      } else {
    ...
    if (
      ...
      ((info.axis === 'Y' &&
        this.prevInfo.blockIndexes.toString() ===
          viewInfo.blockIndexes.toString()) ||
        (info.axis === 'X' &&
          this.prevInfo.columnIndexes.toString() ===
            viewInfo.columnIndexes.toString()))
    ) {
      this.parent.removeMaskRow();
      if (Browser.isIE) {
      ...
      if (
        ...
        viewInfo.blockIndexes[0] === 1 &&
        scrollArgs.direction === 'up'
      ) {
        this.virtualEle.adjustTable(
          0,
          viewInfo.offsets.top < this.offsets[1] ? 0 : this.getBlockHeight()
        );
      }
      return;
    }

    this.parent.setColumnIndexesInView(
      this.parent.enableColumnVirtualization ? viewInfo.columnIndexes : []
    );
    if (
      !(
        !this.parent.enableVirtualization &&
      ...
      this.parent.pageSettings.currentPage =
        viewInfo.loadNext && !viewInfo.loadSelf
          ? viewInfo.nextInfo.page
          : viewInfo.page;
    }
    this.requestType = 'virtualscroll';
    if (this.parent.enableVirtualMaskRow) {
    ...
    if (
      this.parent.enableColumnVirtualization &&
      !this.parent.getContentTable().querySelector('tr.e-row')
    ) {
      this.parent.removeMaskRow();
  ...
  private getInfoFromView(
    ...
    infoType.blockIndexes = tempBlocks = this.vgenerator.getBlockIndexes(
      infoType.page
    );
    infoType.loadSelf = !this.vgenerator.isBlockAvailable(
      tempBlocks[infoType.block]
    );
    const blocks: number[] = this.ensureBlocks(infoType);
    if (
      this.activeKey === 'upArrow' &&
      infoType.blockIndexes.toString() !== blocks.toString()
    ) {
    ...
    infoType.loadNext = !blocks
      .filter((val: number) => tempBlocks.indexOf(val) === -1)
      .every(this.block.bind(this));
    infoType.event =
      infoType.loadNext || infoType.loadSelf
        ? modelChanged
        : refreshVirtualBlock;
    ...
    if (isBlockAdded) {
      infoType.blockIndexes = [
        infoType.blockIndexes[0] - 1,
        infoType.blockIndexes[0],
        infoType.blockIndexes[0] + 1,
      ];
    }
    if (this.activeKey === 'downArrow' && !isNaN(this.rowIndex)) {
      ...
      if (
        firstBlock !== 1 &&
        (infoType.blockIndexes[1] !== firstBlock ||
          infoType.blockIndexes.length < 3)
      ) {
        infoType.blockIndexes = [firstBlock - 1, firstBlock, firstBlock + 1];
      }
    }
    infoType.columnIndexes =
      info.axis === 'X'
        ? this.vgenerator.getColumnIndexes()
        : this.parent.getColumnIndexesInView();
  ...
  public ensureBlocks(info: VirtualInfo): number[] {
    let index: number = info.blockIndexes[info.block];
    let mIdx: number;
    const old: number = index;
    ...
    indexes = indexes.filter(
      (val: number, ind: number) => indexes.indexOf(val) === ind
    );
    let preventSelf: boolean = false;
    if (
      checkIsVirtual(this.parent) &&
      info.direction === 'up' &&
      (((info.page + 1 === this.prevInfo.page ||
        info.page === this.prevInfo.page) &&
        info.block === 1) ||
        (info.page === this.prevInfo.page && indexes.length === 2)) &&
    ...
    if (
      (info.loadSelf && !preventSelf) ||
      (info.direction === 'down' && this.isEndBlock(old))
    ) {
    ...
    if (mIdx !== undefined) {
      ...
      if (
        info.block === 0 &&
        indexes.length === 1 &&
        this.vgenerator.isBlockAvailable(indexes[0] - 1)
      ) {
  ...
  public appendContent(
    target: HTMLElement,
    newChild: DocumentFragment | HTMLElement,
    e: NotifyArgs
  ): void {
    // currentInfo value will be used if there are multiple dom updates happened due to mousewheel
    const info: VirtualInfo =
      e.virtualInfo.sentinelInfo &&
      e.virtualInfo.sentinelInfo.axis === 'Y' &&
      this.currentInfo.page &&
      this.currentInfo.page !== e.virtualInfo.page
        ? this.currentInfo
        : e.virtualInfo;
    ...
    let width: string;
    const blocks: number[] = info.blockIndexes;
    ...
    if (this.parent.enableColumnVirtualization && this.parent.isFrozenGrid()) {
      ...
      width =
        this.getColumnOffset(cIndex[cIndex.length - 1]) -
        this.getColumnOffset(cIndex[0] - 1) +
        '';
    ...
    if (
      ...
      const translate: number = this.getTranslateY(
      ...
      if (
        ...
        this.prevInfo &&
        this.prevInfo.blockIndexes[0] === 1 &&
        info.blockIndexes[0] === 1 &&
        info.direction === 'up'
      ) {
        this.virtualEle.adjustTable(
          0,
          this.content.scrollTop < this.offsets[1] ? 0 : this.getBlockHeight()
        );
      } else {
    ...
    if (this.parent.enableColumnVirtualization) {
      ...
      if (this.parent.isFrozenGrid()) {
        ...
        if (this.parent.allowResizing) {
          (this.parent.getHeaderTable() as HTMLTableElement).style.width =
            reduceWidth + 'px';
          (this.parent.getContentTable() as HTMLTableElement).style.width =
            reduceWidth + 'Px';
        }
      }
      this.header.virtualEle.setWrapperWidth(width);
    }
    this.virtualEle.setWrapperWidth(
      width,
      <boolean>Browser.isIE || Browser.info.name === 'edge'
    );
    if (
      this.parent.enableColumnVirtualization &&
      isNullOrUndefined(target) &&
      isNullOrUndefined(newChild)
    ) {
      return;
    }
    ...
    const tbody: HTMLElement = this.parent.element
      .querySelector('.' + literals.content)
    ...
    const isReact: boolean =
      this.parent.isReact && !isNullOrUndefined(this.parent.rowTemplate);
    if (!isReact) {
    ...
    if (this.parent.groupSettings.columns.length) {
      if (!isGroupAdaptive(this.parent) && info.direction === 'up') {
        const blk: number =
          this.offsets[this.getTotalBlocks()] - this.prevHeight;
      ...
      if (!this.parent.groupSettings.enableLazyLoading) {
        this.observer.setPageHeight(
          this.getOffset(blocks[blocks.length - 1]) -
            this.getOffset(blocks[0] - 1)
        );
      }
    }
    if (
    ...
    if (this.isBottom) {
      this.isBottom = false;
      this.parent.getContent().firstElementChild.scrollTop =
        this.offsets[this.offsetKeys.length - 1];
    }
    if (
      (this.parent.pageSettings.currentPage + 1 === lastPage ||
        this.parent.pageSettings.currentPage === lastPage) &&
      blocks.length === 2 &&
    ...
    if (
      ...
      this.isBottom = true;
      this.parent.getContent().firstElementChild.scrollTop =
        this.offsets[this.offsetKeys.length - 2];
    }
    if (this.isTop) {
    ...
    if (
      e.requestType === 'virtualscroll' &&
      e.virtualInfo.sentinelInfo.axis === 'X'
    ) {
    ...
    if (!this.initialRowTop) {
      ...
      if (this.parent.getRowByIndex(0)) {
        this.initialRowTop =
          this.parent.getRowByIndex(0).getBoundingClientRect().top - gridTop;
      }
    }
  }
  ...
  private validationScrollLeft(): void {
    if (this.validationCheck) {
      if (this.validationCol) {
        const offset: number =
          this.vgenerator.cOffsets[
            this.validationCol.index - this.parent.getVisibleFrozenColumns() - 1
          ];
  ...
  private ensureSelectedRowPosition(): void {
    if (
      !this.isSelection &&
      this.isSelectionScroll &&
      !isNullOrUndefined(this.selectRowIndex)
    ) {
  ...
  private focusCell(e: NotifyArgs): void {
    ...
    if (!this.parent.selectionSettings.checkboxOnly) {
      this.parent.selectRow(
        parseInt(row.getAttribute(literals.dataRowIndex), 10)
      );
    }
  ...
  private restoreEdit(e?: NotifyArgs): void {
    if (this.isNormaledit) {
      if (
        ...
        !isNullOrUndefined(this.editedRowIndex)
      ) {
        const row: HTMLTableRowElement = this.getRowByIndex(
          this.editedRowIndex
        ) as HTMLTableRowElement;
        const content: Element = this.content;
        const keys: string[] = Object.keys(this.virtualData);
        const isXaxis: boolean =
          e && e.virtualInfo && e.virtualInfo.sentinelInfo.axis === 'X';
        if (
          keys.length &&
          row &&
          !content.querySelector('.' + literals.editedRow) &&
          [
            'sorting',
            'filtering',
            'grouping',
        ...
        if (
          row &&
          this.content.querySelector('.' + literals.editedRow) &&
          !keys.length
        ) {
          const rowData: Object =
            ...
            this.parent.enableColumnVirtualization
              ? this.enableCacheOnInfiniteColumnVirtual()
                ...
                : extend(
                    {},
                    this.parent.getCurrentViewRecords()[this.editedRowIndex]
                  )
  ...
  private getVirtualEditedData(rowData: Object): Object {
    const editForms: Element[] = [].slice.call(
      this.parent.element.getElementsByClassName('e-gridform')
    );
    const isFormDestroyed: boolean =
      this.parent.editModule &&
    ...
    if (!isFormDestroyed) {
      for (let i: number = 0; i < editForms.length; i++) {
        rowData = this.parent.editModule.getCurrentEditedData(
          editForms[parseInt(i.toString(), 10)],
          rowData
        );
      }
    }
  ...
  private restoreAdd(): void {
    const startAdd: boolean = !this.parent.element.querySelector(
      '.' + literals.addedRow
    );
  ...
  protected onDataReady(e?: NotifyArgs): void {
    if (!isNullOrUndefined(e.count)) {
      ...
      this.maxPage = Math.ceil(
        (this.parent.groupSettings.columns.length && this.parent.vcRows.length
  ...
  public setVirtualHeight(height?: number): void {
    ...
    let virtualHeight: number =
      this.offsets[
        isGroupAdaptive(this.parent) && this.count !== 0
          ? this.getGroupedTotalBlocks()
          : this.getTotalBlocks()
      ];
    if (
    ...
    const placeHolderBottom: number = Math.round(
      this.virtualEle.placeholder.getBoundingClientRect().bottom
    );
    const wrapperBottom: number = Math.round(
      this.virtualEle.wrapper.getBoundingClientRect().bottom
    );
    if (
      (this.currentInfo.page === lastPage ||
        this.currentInfo.page + 1 === lastPage) &&
      this.currentInfo.direction === 'down' &&
      placeHolderBottom > wrapperBottom &&
      !this.diff
    ) {
    ...
    if (this.virtualEle && this.virtualEle.wrapper) {
      if (
        ...
        this.virtualEle.wrapper.style.minHeight = !isNullOrUndefined(
          virtualHeight
        )
          ? formatUnit(<number>this.parent.height)
          : '0px';
      }
    }
    if (this.parent.enableColumnVirtualization) {
  ...
  private getPageFromTop(sTop: number, info: VirtualInfo): number {
    const total: number = isGroupAdaptive(this.parent)
      ? this.getGroupedTotalBlocks()
      : this.getTotalBlocks();
    let page: number = 0;
    this.offsetKeys.some((offset: string) => {
      ...
      const border: boolean =
        sTop <= this.offsets[`${offset}`] ||
        (iOffset === total && sTop > this.offsets[`${offset}`]);
      if (border) {
        if (
          this.offsetKeys.length % 2 !== 0 &&
          iOffset.toString() === this.offsetKeys[this.offsetKeys.length - 2] &&
        ...
        page = Math.max(
          1,
          Math.min(this.vgenerator.getPage(iOffset), this.maxPage)
        );
      }
      return border;
    });
  ...
  protected getTranslateY(
    ...
    const endTranslate: number = this.getOffset(
      info.blockIndexes[info.blockIndexes.length - 1]
    );
    if (isOnenter) {
      info = this.prevInfo;
    }
    let result: number =
      translate > sTop
        ? this.getOffset(block - 1)
        : endTranslate < sTop + cHeight
        ? this.getOffset(block + 1)
        : translate;
    const blockHeight: number =
      this.offsets[info.blockIndexes[info.blockIndexes.length - 1]] -
      this.tmpOffsets[info.blockIndexes[0]];
    const totalBlocks: number = isGroupAdaptive(this.parent)
      ? this.getGroupedTotalBlocks()
      : this.getTotalBlocks();
    if (
      result + blockHeight >
      this.offsets[parseInt(totalBlocks.toString(), 10)]
    ) {
      result -=
        result +
        blockHeight -
  ...
  public getOffset(block: number): number {
    return Math.min(
      this.offsets[parseInt(block.toString(), 10)] | 0,
      this.offsets[this.maxBlock] | 0
    );
  }
  ...
  private onEntered(): Function {
    return (
      ...
      if (
        ...
        check &&
        !this.preventEvent &&
        !this.parent.enableVirtualMaskRow
      ) {
      ...
      if (this.parent.enableVirtualMaskRow && !this.preventEvent) {
        ...
        const lastOffSetKey: number = parseInt(
          this.offsetKeys[this.offsetKeys.length - 1],
          10
        );
        const blockIndex: number[] = this.currentInfo.blockIndexes;
        const viewInfo: VirtualInfo = this.getInfoFromView(
          direction,
          current,
          e
        );
        const disableShowMaskRow: boolean =
          this.prevInfo &&
          current.axis === 'X' &&
          this.prevInfo.columnIndexes.toString() ===
            viewInfo.columnIndexes.toString();
        if (
          !(
            (blockIndex &&
              blockIndex[0] === firstOffSetKey &&
              direction === 'up') ||
            (blockIndex &&
              blockIndex[blockIndex.length - 1] === lastOffSetKey &&
              direction === 'down') ||
            disableShowMaskRow
          )
        ) {
      ...
      const top: number = this.prevInfo.offsets
        ? this.prevInfo.offsets.top
        : null;
      const height: number = this.content.getBoundingClientRect().height;
      let x: number = this.getColumnOffset(
        xAxis
          ? this.vgenerator.getColumnIndexes()[0] - 1
      ...
      if (xAxis) {
        const idx: number =
          Object.keys(this.vgenerator.cOffsets).length -
          this.prevInfo.columnIndexes.length;
      ...
      if (
        ...
        const y: number = this.getTranslateY(
          e.top,
          height,
          xAxis && top === e.top ? this.prevInfo : undefined,
          true
        );
        this.virtualEle.adjustTable(
          x,
          Math.min(y, this.offsets[this.maxBlock])
        );
      }
      if (this.parent.enableColumnVirtualization) {
  ...
  public resetStickyLeftPos(
    ...
    const cells: HTMLElement[] = [].slice
      ...
      .concat(
        [].slice.call(
          (newChild ? newChild : this.parent.getContent()).querySelectorAll(
            '.e-leftfreeze,.e-rightfreeze,.e-fixedfreeze'
          )
        )
      );
    let frzLeftWidth: number = 0;
    ...
    if (
      this.parent.getHeaderContent().querySelectorAll('.e-fixedfreeze').length
    ) {
    ...
    if (cells.length) {
      for (let i: number = 0; i < cells.length; i++) {
        ...
        if (cell.classList.contains('e-rowcell')) {
          if (
            isNullOrUndefined(cell.getAttribute('data-colindex')) &&
            cell.querySelector('[e-mappinguid]')
          ) {
            const uid: string = cell
              .querySelector('[e-mappinguid]')
          ...
          if (
            ...
            const uid: string = cell.classList.contains('e-filterbarcell')
              ? cell.getAttribute('e-mappinguid')
              : cell
                  .querySelector('[e-mappinguid]')
                  .getAttribute('e-mappinguid');
            col = this.parent.getColumnByUid(uid);
  ...
  private rowSelected(args: RowSelectEventArgs, isSelection?: boolean): void {
    if (
      ...
      !this.isLastBlockRow(args.rowIndex)
    ) {
      const transform: { width: number; height: number } = getTransformValues(
        this.content.firstElementChild
      );
      const gridTop: number = this.parent.element.getBoundingClientRect().top;
      const rowTop: number =
        (args.row as HTMLElement).getBoundingClientRect().top - gridTop;
      const height: number = this.content.getBoundingClientRect().height;
      const isBottom: boolean = height < rowTop;
      const remainHeight: number = isBottom
        ? rowTop - height
        : this.initialRowTop - rowTop;
      let translateY: number = isBottom
        ? transform.height - remainHeight
        : transform.height + remainHeight;
      this.virtualEle.adjustTable(transform.width, translateY);
      const wrapperInfo: DOMRect | ClientRect =
        this.virtualEle.wrapper.getBoundingClientRect();
      const contentInfo: DOMRect | ClientRect =
        this.content.getBoundingClientRect();
      if (wrapperInfo.top > contentInfo.top) {
        this.virtualEle.adjustTable(
          transform.width,
          translateY - (wrapperInfo.top - contentInfo.top)
        );
      }
      const lastRowTop: number =
        this.content
          .querySelector('tbody')
          .lastElementChild.getBoundingClientRect().top - gridTop;
      if (lastRowTop < height) {
        ...
        this.virtualEle.adjustTable(
          transform.width,
          translateY - this.parent.getRowHeight() / 2
        );
      } else if (contentInfo.bottom > wrapperInfo.bottom) {
        this.virtualEle.adjustTable(
          transform.width,
          translateY + (contentInfo.bottom - wrapperInfo.bottom)
        );
      }
      if (
        this.parent.enableColumnVirtualization &&
  ...
  private isLastBlockRow(index: number): boolean {
    ...
    const visibleRowCount: number =
      Math.floor(
        (scrollEle as HTMLElement).offsetHeight / this.parent.getRowHeight()
      ) - 1;
    const startIdx: number =
      this.maxPage * this.parent.pageSettings.pageSize - visibleRowCount;
  ...
  private refreshMaxPage(): void {
    if (this.parent.groupSettings.columns.length && this.parent.vcRows.length) {
      this.maxPage = Math.ceil(
        this.parent.vcRows.length / this.parent.pageSettings.pageSize
      );
    }
  }
  ...
  private setVirtualPageQuery(args: { query: Query; skipPage: boolean }): void {
    ...
    if (
      (this.requestType === 'sorting' || this.requestType === 'delete') &&
      visiblePage.length &&
      checkIsVirtual(this.parent)
    ) {
    ...
    if (
      this.requestType === 'virtualscroll' &&
      this.vgenerator.currentInfo.blockIndexes
    ) {
    ...
    if (
      ...
      this.vgenerator.startIndex = parseInt(
        row.getAttribute('data-rowindex'),
        10
      );
      this.vgenerator.currentInfo = extend({}, this.currentInfo);
      this.vgenerator.currentInfo.blockIndexes =
        this.currentInfo.blockIndexes.slice();
      const includePrevPage: boolean = (this.vgenerator.includePrevPage =
        this.currentInfo.blockIndexes[0] % 2 === 0);
      if (includePrevPage) {
        this.vgenerator.startIndex =
          this.vgenerator.startIndex - this.getBlockSize();
        this.vgenerator.currentInfo.blockIndexes.unshift(
          this.currentInfo.blockIndexes[0] - 1
        );
      } else {
        this.vgenerator.currentInfo.blockIndexes.push(
          this.currentInfo.blockIndexes[
            this.currentInfo.blockIndexes.length - 1
          ] + 1
        );
      }
      const skip: number =
        (this.vgenerator.currentInfo.blockIndexes[0] - 1) * this.getBlockSize();
      const take: number =
        this.vgenerator.currentInfo.blockIndexes.length * this.getBlockSize();
  ...
  public eventListener(action: string): void {
    ...
    this.parent[`${action}`](
      events.refreshVirtualLazyLoadCache,
      this.refreshVirtualLazyLoadCache,
      this
    );
    this.parent[`${action}`](
      events.selectVirtualRow,
      this.selectVirtualRow,
      this
    );
    this.parent[`${action}`](
      events.virtaulCellFocus,
      this.virtualCellFocus,
      this
    );
    this.parent[`${action}`](
      events.virtualScrollEditActionBegin,
      this.editActionBegin,
      this
    );
    this.parent[`${action}`](
      events.virtualScrollAddActionBegin,
      this.addActionBegin,
      this
    );
    ...
    this.parent[`${action}`](
      events.virtualScrollEditCancel,
      this.editCancel,
      this
    );
    this.parent[`${action}`](
      events.refreshVirtualMaxPage,
      this.refreshMaxPage,
      this
    );
    this.parent[`${action}`](
      events.setVirtualPageQuery,
      this.setVirtualPageQuery,
      this
    );
    this.parent[`${action}`](
      events.selectRowOnContextOpen,
      this.selectRowOnContextOpen,
      this
    );
    this.parent[`${action}`](
      events.resetVirtualFocus,
      this.resetVirtualFocus,
      this
    );
    this.parent[`${action}`](
      events.refreshVirtualEditFormCells,
      this.refreshCells,
      this
    );
    ...
    for (let i: number = 0; i < event.length; i++) {
      this.parent[`${action}`](
        `${event[parseInt(i.toString(), 10)]}-begin`,
        this.onActionBegin,
        this
      );
    }
  ...
  private refreshVirtualLazyLoadCache(e: {
    rows: Row<Column>[];
    uid?: string;
    count?: number;
  }): void {
    ...
    for (let i: number = 0; i < blockIndex.length; i++) {
      const rows: Row<Column>[] =
        this.vgenerator.cache[blockIndex[parseInt(i.toString(), 10)]];
    ...
    if (e.count) {
      this.vgenerator.cache[parseInt(block.toString(), 10)].splice(
        index + 1,
        e.count
      );
    } else if (e.rows && e.rows.length) {
      this.vgenerator.cache[parseInt(block.toString(), 10)] = [].slice
        .call(cache.slice(0, index + 1))
        .concat([].slice.call(e.rows))
        .concat([].slice.call(cache.slice(index + 1, cache.length)));
    }
    this.refreshOffsets();
  }
  ...
  private scrollToEdit(col: Column): void {
    ...
    if (col && allowScroll) {
      let offset: number =
        this.vgenerator.cOffsets[
          col.index - this.parent.getVisibleFrozenColumns() - 1
        ];
      if (!this.parent.enableColumnVirtualization) {
        const header: Element = this.parent
          .getHeaderContent()
          .querySelector('.e-headercelldiv[e-mappinguid="' + col.uid + '"]');
      ...
      this.content.scrollLeft = this.parent.enableRtl
        ? -Math.abs(offset)
        : offset;
    }
    if (col && !allowScroll) {
  ...
  public getVirtualData(data: {
    ...
    if (this.isNormaledit) {
      const error: Element = this.parent.element.querySelector(
        '.e-griderror:not([style*="display: none"])'
      );
      const keys: string[] = Object.keys(this.virtualData);
      data.isScroll =
        keys.length !== 0 &&
        this.currentInfo.sentinelInfo &&
        this.currentInfo.sentinelInfo.axis === 'X';
      if (error) {
        return;
      }
  ...
  private editCancel(args: { data: Object }): void {
    ...
    if (!isNullOrUndefined(dataIndex)) {
      args.data =
        this.parent.getCurrentViewRecords()[parseInt(dataIndex.toString(), 10)];
    }
  }
  ...
  private virtualCellFocus(e: KeyboardEventArgs): void {
    ...
    if (
      !ele.classList.contains(literals.rowCell) &&
    ...
    if (
      ...
      ele &&
      (ele.classList.contains(literals.rowCell) ||
        !isNullOrUndefined(parentsUntil(ele, literals.groupCaptionRow))) &&
      e &&
      (e.action === 'upArrow' || e.action === 'downArrow')
    ) {
      const scrollEle: Element = this.parent.getContent().firstElementChild;
      const scrollEleInfo: DOMRect | ClientRect =
        scrollEle.getBoundingClientRect();
      const row: Element = closest(ele, 'tr');
      const nextFocusRow: Element =
        e.action === 'downArrow'
          ? row.nextElementSibling
          : row.previousElementSibling;
      const nextFocusRowInfo: DOMRect | ClientRect = !isNullOrUndefined(
        nextFocusRow
      )
        ? nextFocusRow.getBoundingClientRect()
        : undefined;
      if (
        isNullOrUndefined(nextFocusRow) ||
        (e.action === 'downArrow' &&
        ...
        const viewDifference: number = isNullOrUndefined(nextFocusRow)
    ...
    if (
      ...
      let rowIndex: number = parseInt(
        ele.parentElement.getAttribute(literals.dataRowIndex),
        10
      );
      if (e && (e.action === 'downArrow' || e.action === 'upArrow')) {
        ...
        if (isNullOrUndefined(row)) {
          emptyRow = true;
          if (
            (e.action === 'downArrow' && page === this.maxPage - 1) ||
            (e.action === 'upArrow' && page === 1)
          ) {
            emptyRow = false;
          }
        }
        if (
          emptyRow ||
          (ensureLastRow(row, this.parent) && e.action === 'downArrow') ||
          (ensureFirstRow(row, this.parent.getRowHeight() * 2) &&
            e.action === 'upArrow')
        ) {
          this.activeKey = e.action;
          scrollEle.scrollTop =
            e.action === 'downArrow'
              ? (rowIndex - visibleRowCount) * this.parent.getRowHeight()
              : rowIndex * this.parent.getRowHeight();
        } else {
          this.activeKey = this.empty as string;
        }
        if (!this.parent.selectionSettings.checkboxOnly) {
  ...
  private editActionBegin(e: {
    ...
    const rowData: Object =
      !this.parent.enableVirtualization &&
      this.parent.enableColumnVirtualization
        ? extend({}, this.parent.getCurrentViewRecords()[e.index])
        : extend({}, this.getRowObjectByIndex(e.index));
    const keys: string[] = Object.keys(this.virtualData);
    e.data =
      keys.length && !this.parent.editSettings.showAddNewRow
        ? this.virtualData
        : this.isInfiniteColumnvirtualization()
        ? e.data
        : rowData;
  ...
  private getEditedRowObject(): Row<Column> {
    ...
    for (let i: number = 0; i < rowObjects.length; i++) {
      if (
        rowObjects[parseInt(i.toString(), 10)].index === this.editedRowIndex
      ) {
  ...
  private refreshCache(args: { data: Object }): void {
    ...
    let block: number = Math.ceil(
      (this.editedRowIndex + 1) / this.getBlockSize()
    );
    ...
    const index: number =
      this.parent.allowPaging && this.parent.enableColumnVirtualization
        ...
        : this.editedRowIndex - (block - 1) * this.getBlockSize();
    if (this.parent.groupSettings.columns.length) {
      ...
      this.vgenerator.cache[parseInt(block.toString(), 10)][
  ...
  private actionComplete(args: NotifyArgs): void {
    ...
    if (
      this.isNormaledit &&
      (dataActionRequestTypes.some(
        (value: string) => value === args.requestType
      ) ||
    ...
    if (
      this.parent.enableColumnVirtualization &&
      (args.requestType as string) === 'filterAfterOpen' &&
      this.currentInfo.columnIndexes &&
      this.currentInfo.columnIndexes[0] > 0
    ) {
      (this.parent as Grid).resetFilterDlgPosition(
        (<{ columnName?: string }>args).columnName
      );
    }
  }
  ...
  private resetIsedit(): void {
    if (this.parent.enableVirtualization && this.isNormaledit) {
      if (
        (this.parent.editSettings.allowEditing &&
          Object.keys(this.virtualData).length) ||
  ...
  private scrollAfterEdit(): void {
    if (
      ...
      if (this.parent.element.querySelector('.e-gridform')) {
        const editForm: Element = this.parent.element.querySelector(
          '.' + literals.editedRow
        );
        const addForm: Element = this.parent.element.querySelector(
          '.' + literals.addedRow
        );
        if (editForm || addForm) {
  ...
  private createEmptyRowdata(): void {
    (<{ columnModel?: Column[] }>this.parent).columnModel.filter(
      (e: Column) => {
    ...
    if (this.isNormaledit) {
      ...
      if (
        ...
        this.parent.setProperties(
          { pageSettings: { currentPage: this.maxPage - 1 } },
          true
        );
        args.startEdit = false;
  ...
  public getGroupedTotalBlocks(): number {
    ...
    return Math.floor(
      rows.length / this.getBlockSize() < 1
        ? 1
        : rows.length / this.getBlockSize()
    );
  }
  ...
  public getRows(): Row<Column>[] {
    return this.isInfiniteColumnvirtualization()
      ? this.getInfiniteRows()
      : this.vgenerator.getRows();
  }
      ...
      if (
        ...
        for (let i: number = 0; i < this.parent.getDataRows().length; i++) {
          if (
            this.parent
              .getDataRows()
              [parseInt(i.toString(), 10)].getAttribute(
        ...
        row = !isNullOrUndefined(index)
          ? this.parent.getDataRows()[parseInt(index.toString(), 10)]
          : undefined;
      }
    } else if (
      !this.parent.enableVirtualization &&
      ...
      row = !isNullOrUndefined(index)
        ? this.enableCacheOnInfiniteColumnVirtual()
          ? this.parent
              ...
              .find(
                (element: Element) =>
                  parseInt(element.getAttribute(literals.dataRowIndex), 10) ===
                  index
              )
          : this.parent.getDataRows()[parseInt(index.toString(), 10)]
        : undefined;
  ...
  public getRowCollection(
    ...
    let collection: Element[] | Object[] = isRowObject
    ...
    if (
      ...
      startIdx = parseInt(
        this.parent.getRows()[0].getAttribute(literals.dataRowIndex),
        10
      );
      collection = collection.filter((m: object) => {
        return isNullOrUndefined((<{ items?: object }>m).items);
      });
    }
    if (
      !isRowObject &&
      this.parent.allowGrouping &&
      this.parent.groupSettings.columns.length &&
      rowCollection.length
    ) {
      startIdx = parseInt(
        rowCollection[0].getAttribute(literals.dataRowIndex),
        10
      );
    }
    ...
    if (this.parent.frozenRows && this.parent.pageSettings.currentPage > 1) {
      if (!isRowObject) {
        selectedRow =
          index <= this.parent.frozenRows
            ? rowCollection[parseInt(index.toString(), 10)]
            : rowCollection[index - startIdx + this.parent.frozenRows];
      } else {
        selectedRow =
          index <= this.parent.frozenRows
            ? this.parent.getRowsObject()[parseInt(index.toString(), 10)].data
            : selectedRow;
      }
    }
    return selectedRow;
  ...
  public refreshOffsets(): void {
    const gObj: IGrid = this.parent;
    let row: number = 0;
    const bSize: number = this.getBlockSize();
    const total: number = isGroupAdaptive(this.parent)
      ? this.getGroupedTotalBlocks()
      : this.getTotalBlocks();
    ...
    for (let i: number = 0; i < blocks.length; i++) {
      const tmp: number = (
        this.vgenerator.cache[blocks[parseInt(i.toString(), 10)]] || []
      ).length;
      const rem: number = !isGroupAdaptive(this.parent)
        ? this.count % bSize
        : gObj.vcRows.length % bSize;
      const size: number =
        !isGroupAdaptive(this.parent) &&
        blocks[parseInt(i.toString(), 10)] in this.vgenerator.cache
          ? tmp * this.parent.getRowHeight()
          : rem && blocks[parseInt(i.toString(), 10)] === total
          ? rem * this.parent.getRowHeight()
          : this.getBlockHeight();
      // let size: number = this.parent.groupSettings.columns.length && block in this.vgenerator.cache ?
      // tmp * getRowHeight() : this.getBlockHeight();
      this.offsets[blocks[parseInt(i.toString(), 10)]] =
        (this.offsets[blocks[parseInt(i.toString(), 10)] - 1] | 0) + size;
      this.tmpOffsets[blocks[parseInt(i.toString(), 10)]] =
        this.offsets[blocks[parseInt(i.toString(), 10)] - 1] | 0;
    }
  ...
  public setVisible(columns?: Column[]): void {
    ...
    for (let c: number = 0, clen: number = columns.length; c < clen; c++) {
      ...
      if (idx !== -1 && testRow && idx < testRow.cells.length) {
        setStyleAttribute(colGrp[parseInt(idx.toString(), 10)] as HTMLElement, {
          display: displayVal,
        });
      }
      if (!isRefresh) {
        ...
        if (column.width) {
          if (column.visible) {
            width =
              this.virtualEle.wrapper.offsetWidth +
  ...
  private selectVirtualRow(args: {
    selectedIndex: number;
    isAvailable: boolean;
  }): void {
    const count: number = isGroupAdaptive(this.parent)
      ? this.vgenerator.recordsCount
      : this.count;
    ...
    if (
      ...
      const selectedRow: Element = this.parent.getRowByIndex(
        args.selectedIndex
      );
      ...
      if (!selectedRow || !this.isRowInView(selectedRow)) {
        ...
        if (isGroupAdaptive(this.parent)) {
          const selectedRowObjectIndex: number = this.parent.vcRows.findIndex(
            (row: Row<Column>) => row.index === args.selectedIndex
          );
          scrollTop =
            selectedRowObjectIndex !== -1
              ? selectedRowObjectIndex * rowHeight
              : undefined;
          this.isSelection =
            selectedRowObjectIndex !== -1 ? this.isSelection : false;
        } else if (
          this.parent.getDataModule().isRemote() &&
          this.parent.groupSettings.columns.length
        ) {
          const page: number = Math.ceil(
            (args.selectedIndex + 1) / this.parent.pageSettings.pageSize
          );
        ...
        if (!isNullOrUndefined(scrollTop)) {
          const direction: ScrollDirection =
            this.content.scrollTop < scrollTop ? 'down' : 'up';
  ...
  private isRowInView(row: Element): boolean {
    ...
    return (
      ...
      bottom <= this.content.getBoundingClientRect().bottom
    );
  }
}
/**
...
export class VirtualHeaderRenderer extends HeaderRender implements IRenderer {
  ...
  constructor(parent: IGrid, locator: ServiceLocator) {
    ...
    this.parent.on(
      refreshVirtualBlock,
      (e?: NotifyArgs) =>
        e.virtualInfo.sentinelInfo.axis === 'X' ? this.refreshUI() : null,
      this
    );
  }
  ...
  public renderTable(): void {
    ...
    this.parent.setColumnIndexesInView(
      this.gen.getColumnIndexes(
        <HTMLElement>this.getPanel().querySelector('.' + literals.headerContent)
      )
    );
    super.renderTable();
    this.virtualEle.table = <HTMLElement>this.getTable();
    this.virtualEle.content = <HTMLElement>(
      this.getPanel().querySelector('.' + literals.headerContent)
    );
  ...
  public setVisible(columns?: Column[]): void {
    ...
    for (let c: number = 0, clen: number = columns.length; c < clen; c++) {
      ...
      const colGrp: HTMLCollection = this.getColGroup().children;
      setStyleAttribute(<HTMLElement>colGrp[parseInt(idx.toString(), 10)], {
        display: displayVal,
      });
      if (
        gObj.enableColumnVirtualization &&
  ...
  private setDisplayNone(col: Column, displayVal: string): void {
    ...
    for (const ele of [].slice.apply(
      table.querySelectorAll('th.e-headercell')
    )) {
      if (
        ele.querySelector('[e-mappinguid]') &&
        ele.querySelector('[e-mappinguid]').getAttribute('e-mappinguid') ===
          col.uid
      ) {
...
export class VirtualElementHandler {
  ...
  public setWrapperWidth(width: string, full?: boolean): void {
    if (
      width &&
      width.indexOf('%') === -1 &&
      !(this.content.getBoundingClientRect().width < parseInt(width, 10))
    ) {
      width = undefined;
      full = true;
    }
  ...
  public setVirtualHeight(height?: number, width?: string): void {
    this.placeholder.style.height = !isNullOrUndefined(height)
      ? `${height}px`
      : '0px';
    if (
      width &&
