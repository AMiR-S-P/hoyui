...
export class BooleanFilterUI implements IFilterMUI {
  ...
  public create(args: {
    ...
    const fields: string = isForeignColumn
  ...
  public write(args: {
    ...
    const drpuiObj: DropDownList = (<EJ2Intance>(
      document.querySelector('#bool-ui-' + args.column.uid)
    )).ej2_instances[0];
  ...
  public read(
    ...
    const drpuiObj: DropDownList = (<EJ2Intance>(
      document.querySelector('#bool-ui-' + column.uid)
    )).ej2_instances[0];
    const filterValue: string | number | Date | boolean = drpuiObj.value as
      | string
      | number
      | Date
      | boolean;
  ...
  private destroy(): void {
    ...
    this.dropInstance.removeEventListener(
