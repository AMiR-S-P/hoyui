...
export class CommandColumnRenderer
  extends CellRenderer
  implements ICellRenderer<Column>
{
  private buttonElement: HTMLButtonElement = <HTMLButtonElement>(
    this.parent.createElement('button', {})
  );
  private unbounDiv: HTMLElement = this.parent.createElement('div', {
  ...
  private destroyButtons(args: { type: string }): void {
    for (let i: number = 0; i < this.childRefs.length; i++) {
      if (
        this.childRefs[parseInt(i.toString(), 10)] &&
        !this.childRefs[parseInt(i.toString(), 10)].isDestroyed &&
        !(
          this.parent.editSettings.showAddNewRow &&
          this.parent.enableVirtualization &&
          parentsUntil(
            this.childRefs[parseInt(i.toString(), 10)].element,
            'e-addedrow'
          )
        )
      ) {
    ...
    if (args.type === 'refreshCommandColumn') {
      let elem: NodeListOf<Element> = this.parent.element.querySelectorAll(
        '.e-gridcontent .e-unboundcell'
      );
      if (elem.length) {
        for (let i: number = 0; i < elem.length; i++) {
          if (
            elem[parseInt(i.toString(), 10)] &&
            !(
              this.parent.editSettings.showAddNewRow &&
              this.parent.enableVirtualization &&
            ...
            if (
              elem[parseInt(i.toString(), 10)].querySelector(
                '.e-unboundcelldiv'
              )
            ) {
              elem[parseInt(i.toString(), 10)].querySelector(
  ...
  public render(
    ...
    isVirtualEdit?: boolean
  ): Element {
    let node: Element = this.element.cloneNode() as Element;
    const uid: string = 'uid';
    node.appendChild(this.unbounDiv.cloneNode());
    (<HTMLElement>node).setAttribute(
      'aria-label',
      this.localizer.getConstant('CommandColumnAria') + cell.column.headerText
    );
    if (cell.column.commandsTemplate) {
      if (
        this.parent.isReact &&
        typeof cell.column.commandsTemplate !== 'string'
      ) {
    ...
    if (
      ...
      addClass(
        [].slice.call(node.getElementsByClassName('e-edit-delete')),
        'e-hide'
      );
      removeClass(
        [].slice.call(node.getElementsByClassName('e-save-cancel')),
        'e-hide'
      );
    } else {
      addClass(
  ...
  private renderButton(
    node: Element,
    buttonOption: CommandModel,
    index: number,
    uid: string
  ): Element {
    const button: HTMLButtonElement = <HTMLButtonElement>(
      this.buttonElement.cloneNode()
    );
    attributes(button, {
      id:
        ...
        uid,
      type: 'button',
      title: !isNullOrUndefined(buttonOption.title)
        ? buttonOption.title
        : buttonOption.buttonOption.content ||
          this.localizer.getConstant(buttonOption.type) ||
          buttonOption.type,
      'data-uid': uid,
    });
    ...
    switch (buttonOption.type) {
      ...
      case 'Delete':
        addClass(
          [button],
          ['e-edit-delete', 'e-' + buttonOption.type.toLowerCase() + 'button']
        );
        break;
      case 'Cancel':
      case 'Save':
        addClass(
          [button],
          ['e-save-cancel', 'e-' + buttonOption.type.toLowerCase() + 'button']
        );
        break;
    }
    return node;
