 ...
 * @hidden
 */
export class SummaryCellRenderer
  extends CellRenderer
  implements ICellRenderer<AggregateColumnModel>
{
  ...
  public evaluate(
    ...
    this.parent.on(
      ...
      this.refreshWithAggregate(node, cell),
      this
    );
    if (
      !(
    ...
    const tempObj: { fn: Function; property: string } = column.getTemplate(
      cell.cellType
    );
    const tempID: string = '';
    const gColumn: Column = this.parent.getColumnByField(
      data[column.columnName].field
    );
    ...
    const isReactCompiler: boolean =
      this.parent.isReact &&
      (column.footerTemplate
        ? typeof column.footerTemplate !== 'string'
        : column.groupFooterTemplate
        ? typeof column.groupFooterTemplate !== 'string'
        : column.groupCaptionTemplate
    ...
    if (isReactCompiler || isReactChild) {
      ...
      if (
        tempObj.property === 'groupCaptionTemplate' ||
        tempObj.property === 'groupFooterTemplate'
      ) {
        const groupKey: string = 'groupKey';
  ...
  public refreshWithAggregate(
    ...
    return (args: { cells: Object[]; data: Object; dataUid: string }) => {
      ...
      const field: string = cell.column.columnName
        ? cell.column.columnName
        : null;
      const curCell: Cell<Column> = <Cell<Column>>(
        (!isNullOrUndefined(field)
          ? args.cells.filter(
              (cell: Cell<AggregateColumn>) => cell.column.columnName === field
            )[0]
          : null)
      );
      if (
        node.parentElement &&
        node.parentElement.getAttribute('data-uid') === args.dataUid &&
        field &&
        curCell &&
        field ===
          (<Cell<Column> & { column: AggregateColumn }>curCell).column
            .columnName
      ) {
