...
export class Animation extends ChildProperty<Animation> {
   ...
   */
  @Property<TooltipAnimationSettings>({
    effect: 'FadeIn',
    duration: 150,
    delay: 0,
  })
  ...
  implements INotifyPropertyChanged
{
  ...
  private formatPosition(): void {
    ...
    if (
      this.position.indexOf('Top') === 0 ||
      this.position.indexOf('Bottom') === 0
    ) {
      [this.tooltipPositionY, this.tooltipPositionX] =
        this.position.split(/(?=[A-Z])/);
    } else {
      [this.tooltipPositionX, this.tooltipPositionY] =
        this.position.split(/(?=[A-Z])/);
    }
  }
  private renderArrow(): void {
    this.setTipClass(this.position);
    const tip: HTMLElement = this.createElement('div', {
  ...
  private renderPopup(target: HTMLElement): void {
    const elePos: OffsetPosition = this.mouseTrail
      ? { top: 0, left: 0 }
  ...
  private getScalingFactor(target: HTMLElement): { [key: string]: number } {
    ...
    const elementsWithTransform: HTMLElement | Element = target.closest(
      '[style*="transform: scale"]'
    );
    if (
      elementsWithTransform &&
      elementsWithTransform !== this.tooltipEle &&
      elementsWithTransform.contains(this.tooltipEle)
    ) {
      ...
      const matrixValues: number[] = transformValue
        .match(/matrix\(([^)]+)\)/)[1]
        .split(',')
        .map(parseFloat);
  ...
  private getTooltipPosition(target: HTMLElement): OffsetPosition {
    ...
    const parentWithZoomStyle: HTMLElement = this.element.closest(
      '[style*="zoom"]'
    ) as HTMLElement;
    if (parentWithZoomStyle) {
      if (!parentWithZoomStyle.contains(this.tooltipEle)) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (this.tooltipEle.style as any).zoom = (
          getComputedStyle(parentWithZoomStyle) as any
        ).zoom;
      }
    }
    const pos: OffsetPosition = calculatePosition(
      ...
      !this.isBodyContainer,
      this.isBodyContainer
        ? null
        : this.containerElement.getBoundingClientRect()
    );
    const scalingFactors: { [key: string]: number } =
      this.getScalingFactor(target);
    const offsetPos: OffsetPosition = this.calculateTooltipOffset(
      this.position,
      scalingFactors.x,
      scalingFactors.y
    );
    const collisionPosition: Array<number> = this.calculateElementPosition(
      pos,
      offsetPos
    );
    const collisionLeft: number = collisionPosition[0];
    const collisionTop: number = collisionPosition[1];
    const elePos: OffsetPosition = this.collisionFlipFit(
      target,
      collisionLeft,
      collisionTop
    );
  ...
  private closePopupHandler(): void {
    ...
    if (
      (this as any).isReact &&
      !(this.opensOn === 'Click' || typeof this.content === 'function')
    ) {
      this.clearTemplate(['content']);
    }
    this.clear();
    let tooltipAfterCloseEventArgs: TooltipEventArgs = {
      type: this.tooltipEventArgs.event
        ? this.tooltipEventArgs.event.type
        : null,
      cancel: false,
  ...
  private calculateTooltipOffset(
    ...
    if (xScalingFactor !== 1 || yScalingFactor !== 1) {
      const tooltipEleRect: DOMRect | ClientRect =
        this.tooltipEle.getBoundingClientRect();
      ...
      tipAdjust = this.showTipPointer
        ? SHOW_POINTER_TIP_GAP
        : HIDE_POINTER_TIP_GAP;
      tipHeightAdjust =
        tipHeight / 2 +
        POINTER_ADJUST +
        (tooltipEleHeight - this.tooltipEle.clientHeight * yScalingFactor);
      tipWidthAdjust =
        tipWidth / 2 +
        POINTER_ADJUST +
        (tooltipEleWidth - this.tooltipEle.clientWidth * xScalingFactor);
    } else {
      ...
      tipHeightAdjust =
        tipHeight / 2 +
        POINTER_ADJUST +
        (this.tooltipEle.offsetHeight - this.tooltipEle.clientHeight);
      tipWidthAdjust =
        tipWidth / 2 +
        POINTER_ADJUST +
  ...
  private updateTipPosition(position: Position): void {
    const selEle: Element[] = selectAll(
      '.' + ARROW_TIP + ',.' + ARROW_TIP_OUTER + ',.' + ARROW_TIP_INNER,
      this.tooltipEle
    );
  ...
  private adjustArrow(
    ...
    const arrowEle: HTMLElement = select(
      '.' + ARROW_TIP,
      this.tooltipEle
    ) as HTMLElement;
    ...
    const arrowInnerELe: HTMLElement = select(
      '.' + ARROW_TIP_INNER,
      this.tooltipEle
    ) as HTMLElement;
    const tipWidth: number = arrowEle.offsetWidth;
    const tipHeight: number = arrowEle.offsetHeight;
    this.tooltipEle.style.display = '';
    if (this.tipClass === TIP_BOTTOM || this.tipClass === TIP_TOP) {
      ...
      if (target) {
        const tipPosExclude: boolean =
          tooltipPositionX !== 'Center' ||
          tooltipWidth > target.offsetWidth ||
          this.mouseTrail;
        if (
          (tipPosExclude && tooltipPositionX === 'Left') ||
          (!tipPosExclude && this.tipPointerPosition === 'End')
        ) {
          leftValue = tooltipWidth - tipWidth - POINTER_ADJUST + 'px';
        } else if (
          (tipPosExclude && tooltipPositionX === 'Right') ||
          (!tipPosExclude && this.tipPointerPosition === 'Start')
        ) {
          leftValue = POINTER_ADJUST + 'px';
        } else if (
          tipPosExclude &&
          (this.tipPointerPosition === 'End' ||
            this.tipPointerPosition === 'Start')
        ) {
          leftValue =
            this.tipPointerPosition === 'End'
              ? target.offsetWidth +
                (this.tooltipEle.offsetWidth - target.offsetWidth) / 2 -
                tipWidth / 2 -
                POINTER_ADJUST +
                'px'
              : (this.tooltipEle.offsetWidth - target.offsetWidth) / 2 -
      ...
      const tipPosExclude: boolean =
        tooltipPositionY !== 'Center' ||
        tooltipHeight > target.offsetHeight ||
        this.mouseTrail;
      if (
        (tipPosExclude && tooltipPositionY === 'Top') ||
        (!tipPosExclude && this.tipPointerPosition === 'End')
      ) {
        topValue = tooltipHeight - tipHeight - POINTER_ADJUST + 'px';
      } else if (
        (tipPosExclude && tooltipPositionY === 'Bottom') ||
        (!tipPosExclude && this.tipPointerPosition === 'Start')
      ) {
  ...
  private renderContent(target?: HTMLElement): void {
    const tooltipContent: HTMLElement = select(
      '.' + CONTENT,
      this.tooltipEle
    ) as HTMLElement;
    if (this.cssClass) {
    ...
    if (!isNullOrUndefined(this.content)) {
      ...
      if (this.content instanceof HTMLElement) {
        ...
        if (this.isAngular) {
          ...
          this.content = this.enableHtmlSanitizer
            ? SanitizeHtmlHelper.sanitize(this.content)
            : this.content;
        }
  ...
  private renderCloseIcon(): void {
    if (!this.isSticky) {
      const existingCloseIcon: HTMLElement = this.tooltipEle.querySelector(
        '.' + ICON + '.' + CLOSE
      );
    ...
    const tipClose: HTMLElement = this.createElement('div', {
  ...
  private targetHover(e: Event): void {
    ...
    if (
      ...
      (target.getAttribute('data-tooltip-id') !== null && this.closeDelay === 0)
    ) {
      return;
    }
    const targetList: Element[] = [].slice.call(
      selectAll('[data-tooltip-id= "' + this.ctrlId + '_content"]', document)
    );
    ...
    if (beforeRenderArgs.cancel) {
      ...
      if (isNullOrUndefined(this.tooltipEle)) {
        this.ctrlId = this.element.getAttribute('id')
          ? getUniqueID(this.element.getAttribute('id'))
          : getUniqueID('tooltip');
  ...
  private tooltipBeforeRender(target: HTMLElement, ctrlObj: Tooltip): void {
    if (target) {
      ...
      ctrlObj.tooltipEle.appendChild(
        ctrlObj.createElement('div', { className: CONTENT })
      );
      ...
      ctrlObj.adjustArrow(
        target,
        ctrlObj.position,
        ctrlObj.tooltipPositionX,
        ctrlObj.tooltipPositionY
      );
  ...
  private tooltipAfterRender(
    ...
    if (target) {
      ...
      if (
        ctrlObj.needTemplateReposition() &&
        !ctrlObj.mouseTrail &&
        (showAnimation.effect === 'None' ||
          showAnimation.effect === 'FadeIn' ||
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  private beforeOpenCallback(
    ...
    if (observedArgs.cancel) {
      ...
      let openAnimation: Object = {
        name:
          showAnimation.effect === 'None' && animationMode === 'Enable'
            ? 'FadeIn'
            : this.animation.open.effect,
  ...
  private needTemplateReposition(): boolean {
    ...
    return (
      (!isNullOrUndefined(tooltip.viewContainerRef) &&
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  private checkCollision(
    ...
    const affectedPos: string[] = isCollide(
      this.tooltipEle,
      this.checkCollideTarget(),
      x,
      y
    );
    if (affectedPos.length > 0) {
      elePos.horizontal =
        affectedPos.indexOf('left') >= 0
          ? 'Right'
          : affectedPos.indexOf('right') >= 0
          ? 'Left'
          : this.tooltipPositionX;
      elePos.vertical =
        affectedPos.indexOf('top') >= 0
          ? 'Bottom'
          : affectedPos.indexOf('bottom') >= 0
          ? 'Top'
          : this.tooltipPositionY;
    }
    return elePos;
  ...
  private calculateElementPosition(
    ...
    return [
      this.isBodyContainer
        ...
        : pos.left -
          (this.containerElement.getBoundingClientRect() as DOMRect).left +
          offsetPos.left +
          window.pageXOffset +
      ...
      this.isBodyContainer
        ...
        : pos.top -
          (this.containerElement.getBoundingClientRect() as DOMRect).top +
          offsetPos.top +
          window.pageYOffset +
  ...
  private collisionFlipFit(
    ...
    if (this.tooltipPositionY !== elePos.vertical) {
      newpos = (
        this.position.indexOf('Bottom') === 0 ||
        this.position.indexOf('Top') === 0
          ? elePos.vertical + this.tooltipPositionX
          : this.tooltipPositionX + elePos.vertical
      ) as Position;
    }
    if (this.tooltipPositionX !== elePos.horizontal) {
      if (newpos.indexOf('Left') === 0) {
        elePos.vertical =
          newpos === 'LeftTop' || newpos === 'LeftCenter' ? 'Top' : 'Bottom';
        newpos = (elePos.vertical + 'Left') as Position;
      }
      if (newpos.indexOf('Right') === 0) {
        elePos.vertical =
          newpos === 'RightTop' || newpos === 'RightCenter' ? 'Top' : 'Bottom';
    ...
    if (this.tooltipEventArgs.cancel) {
      ...
      if (elePos.position !== newpos) {
        ...
        offsetPos.top -= this.getOffSetPosition(
          'TopBottom',
          newpos,
          this.offsetY
        );
        offsetPos.left -= this.getOffSetPosition(
          'RightLeft',
          newpos,
          this.offsetX
        );
        elePos.position = newpos;
        const elePosition: Array<number> = this.calculateElementPosition(
          pos,
          offsetPos
        );
    ...
    if (
      this.showTipPointer &&
      arrowEle != null &&
      (newpos.indexOf('Bottom') === 0 || newpos.indexOf('Top') === 0)
    ) {
      let arrowleft: number =
        parseInt(arrowEle.style.left, 10) - (position.left - elePos.left);
      if (arrowleft < 0) {
        arrowleft = 0;
      } else if (
        arrowleft + arrowEle.offsetWidth >
  ...
  private getOffSetPosition(
    ...
    return positionString.indexOf(this.position.split(/(?=[A-Z])/)[0]) !== -1 &&
      positionString.indexOf(newPos.split(/(?=[A-Z])/)[0]) !== -1
      ? 2 * offsetType
      : 0;
  }
  ...
  private hideTooltip(
    hideAnimation: TooltipAnimationSettings,
    e?: Event,
  ...
  private popupHide(
    ...
    let closeAnimation: Object = {
      name:
        hideAnimation.effect === 'None' && animationMode === 'Enable'
          ? 'FadeOut'
          : this.animation.close.effect,
  ...
  private onMouseOut(e: Event): void {
    ...
    if (enteredElement && !this.mouseTrail) {
      ...
      if (checkForTooltipElement) {
        ...
        if (
          this.closeDelay === 0 &&
          (this.animation.close.effect === 'None' ||
            ((this as any).isReact && typeof this.content != 'string'))
        ) {
          this.clear();
        }
      }
    ...
    if (
      this.popupObj &&
      !this.popupObj.element.classList.contains(POPUP_OPEN)
    ) {
      this.clear();
    }
  }
  ...
  private onMouseMove(event: MouseEvent & TouchEvent): void {
    let eventPageX: number = 0;
    let eventPageY: number = 0;
    ...
    const pos: OffsetPosition = this.calculateTooltipOffset(
      this.position,
      scalingFactors.x,
      scalingFactors.y
    );
    ...
    const elePos: ElementPosition = this.checkCollision(
      event.target as HTMLElement,
      x,
      y
    );
    if (
      this.tooltipPositionX !== elePos.horizontal ||
      this.tooltipPositionY !== elePos.vertical
    ) {
      const newpos: string =
        this.position.indexOf('Bottom') === 0 ||
        this.position.indexOf('Top') === 0
          ? elePos.vertical + elePos.horizontal
          : elePos.horizontal + elePos.vertical;
      ...
      const colpos: OffsetPosition = this.calculateTooltipOffset(
        elePos.position,
        scalingFactors.x,
        scalingFactors.y
      );
  ...
  private touchEnd(e: TouchEvent): void {
    if (
      this.tooltipEle &&
      closest(e.target as HTMLElement, '.' + ROOT) === null &&
      !this.isSticky
    ) {
      this.close();
    }
  ...
  private wireFocusEvents(): void {
    if (!isNullOrUndefined(this.target)) {
      const targetList: Element[] = [].slice.call(
        selectAll(this.target, this.element)
      );
  ...
  private findTarget(): HTMLElement {
    const target: HTMLElement = select(
      '[data-tooltip-id= "' + this.ctrlId + '_content"]',
      document
    ) as HTMLElement;
    return target;
      ...
      switch (prop) {
        case 'width':
          if (this.tooltipEle && targetElement) {
            this.tooltipEle.style.width = this.tooltipEle.style.maxWidth =
              formatUnit(newProp.width);
        ...
        case 'offsetX':
          if (this.tooltipEle) {
            ...
            this.tooltipEle.style.left =
              (parseInt(this.tooltipEle.style.left, 10) + x).toString() + 'px';
          }
          break;
        case 'offsetY':
          if (this.tooltipEle) {
            const y: number = newProp.offsetY - oldProp.offsetY;
            this.tooltipEle.style.top =
              (parseInt(this.tooltipEle.style.top, 10) + y).toString() + 'px';
          }
          break;
        case 'cssClass':
          if (this.tooltipEle) {
  ...
  public refresh(target?: HTMLElement): void {
    ...
    if (
      !isNullOrUndefined(this.targetsList) &&
  ...
  public destroy(): void {
    ...
    this.tooltipEle = null;
    this.popupObj = null;
    const currentTarget: HTMLElement[] = selectAll(
      '[data-tooltip-id= "' + this.ctrlId + '_content"]',
      this.element
    );
