...
import { BlazorDragEventArgs, SanitizeHtmlHelper, Browser } from '@hoyui/base';
import { Button, ButtonModel } from '@hoyui/buttons';
  ...
  implements INotifyPropertyChanged
{
  ...
  protected preRender(): void {
    ...
    this.dlgOverlayClickEventHandler = (event: Object): void => {
      ...
      this.trigger(
        'overlayClick',
        event,
        (overlayClickEventArgs: { [key: string]: object }) => {
  ...
  private checkPositionData(): void {
    if (!isNullOrUndefined(this.position)) {
      if (
        !isNullOrUndefined(this.position.X) &&
      ...
      if (
        !isNullOrUndefined(this.position.Y) &&
  ...
  private getMinHeight(): number {
    ...
    if (
      !isNullOrUndefined(this.element.querySelector('.' + DLG_HEADER_CONTENT))
    ) {
      computedHeaderHeight = getComputedStyle(this.headerContent).height;
    }
    const footerEle: Element = this.getEle(
    ...
    const footerHeight: number = parseInt(
      computedFooterHeight.slice(0, computedFooterHeight.indexOf('p')),
      10
    );
  ...
  private onResizeStart(
    args: ResizeMouseEventArgs | ResizeTouchEventArgs,
    dialogObj: Dialog
  ): boolean {
    ...
    if (this.enableResize) {
      ...
      for (let i: number = 0; i < this.resizeHandles.length; i++) {
        if (this.resizeHandles[i as number] === 'All') {
          direction =
            'south north east west north-east north-west south-east south-west';
          break;
        } else {
      ...
      createResize({
        ...
        direction: direction,
        minHeight: parseInt(
          computedHeight.slice(0, computedWidth.indexOf('p')),
          10
        ),
        maxHeight: this.targetEle.clientHeight,
        minWidth: parseInt(
          computedWidth.slice(0, computedWidth.indexOf('p')),
          10
        ),
  ...
  private keyDown(event: KeyboardEvent): void {
    if (event.keyCode === 9) {
      if (this.isModal) {
        ...
        if (
          isNullOrUndefined(this.btnObj) &&
          !isNullOrUndefined(this.ftrTemplateContent)
        ) {
        ...
        if (
          isNullOrUndefined(this.btnObj) &&
          isNullOrUndefined(this.ftrTemplateContent) &&
          !isNullOrUndefined(this.contentEle)
        ) {
          buttonObj = this.getFocusElement(this.contentEle);
        }
        if (
          !isNullOrUndefined(buttonObj) &&
          document.activeElement === buttonObj.element &&
          !event.shiftKey
        ) {
        ...
        if (
          document.activeElement === this.focusableElements(this.element) &&
          event.shiftKey
        ) {
          event.preventDefault();
    ...
    if (event.keyCode === 27 && this.closeOnEscape) {
      ...
      const query: HTMLElement = <HTMLElement>(
        document.querySelector('.e-popup-open:not(.e-dialog)')
      );
      // 'document.querySelector' is used to find the elements rendered based on body
      if (
        !(
          !isNullOrUndefined(query) &&
          !query.classList.contains('e-toolbar-pop')
        )
      ) {
        this.hide(event);
      }
    ...
    if (
      (event.keyCode === 13 &&
        ...
        isTagName &&
        !isNullOrUndefined(this.primaryButtonEle)) ||
      (event.keyCode === 13 &&
        event.ctrlKey &&
        (element.tagName.toLowerCase() === 'textarea' || isContentEdit) &&
        !isNullOrUndefined(this.primaryButtonEle))
    ) {
      let buttonIndex: number;
      const firstPrimary: boolean = this.buttons.some(
        (data: { [key: string]: Object }, index: number) => {
          buttonIndex = index;
          const buttonModel: { [key: string]: Object } = data.buttonModel as {
            [key: string]: Object;
          };
          return (
            !isNullOrUndefined(buttonModel) && buttonModel.isPrimary === true
          );
        }
      );
      if (
        firstPrimary &&
        typeof this.buttons[buttonIndex as number].click === 'function'
      ) {
        setTimeout(() => {
  ...
  private initRender(): void {
    ...
    if (this.footerTemplate !== '' && !isNullOrUndefined(this.footerTemplate)) {
      ...
      !isNullOrUndefined(this.buttons[0]) &&
      !isNullOrUndefined(this.buttons[0].buttonModel)
    ) {
      this.setButton();
    }
    ...
    if (
      this.element.classList.contains(DLG_UTIL_ALERT) !== true &&
      this.element.classList.contains(DLG_UTIL_CONFIRM) !== true &&
      !isNullOrUndefined(this.element.parentElement)
    ) {
      const parentEle: HTMLElement = this.isModal
        ? this.dlgContainer.parentElement
      ...
      parentEle.insertBefore(
        this.refElement,
        this.isModal ? this.dlgContainer : this.element
      );
    }
    ...
    this.popupObj = new Popup(this.element, {
      ...
      open: (event: Event) => {
        ...
        this.trigger(
          'open',
          eventArgs,
          (openEventArgs: { [key: string]: object }) => {
      ...
      close: (event: Event) => {
        ...
        if (
          !isNullOrUndefined(activeEle) &&
          !isNullOrUndefined(activeEle.blur)
        ) {
          activeEle.blur();
        }
        if (
          !isNullOrUndefined(this.storeActiveElement) &&
          !isNullOrUndefined(this.storeActiveElement.focus)
        ) {
      ...
      if (this.isModal) {
        ...
        if (targetType instanceof Element) {
          const computedStyle: CSSStyleDeclaration =
            window.getComputedStyle(targetType);
    ...
    if (typeof targetValue === 'string') {
      if (targetValue.startsWith('#')) {
        ...
        const elements: HTMLCollectionOf<Element> =
          document.getElementsByClassName(targetValue.substring(1));
        targetElement =
          elements.length > 0 ? (elements[0] as HTMLElement) : null;
      } else {
        if (
          !((targetValue as any) instanceof HTMLElement) &&
          (targetValue as any) !== document.body
        ) {
  ...
  private resetResizeIcon(): void {
    ...
    if (this.targetEle.offsetHeight < dialogConHeight) {
      const className: string = this.enableRtl
        ? 'e-south-west'
        : 'e-south-east';
      const resizeIcon: HTMLElement = this.element.querySelector(
        '.' + className
      );
  ...
  private setOverlayZindex(zIndexValue?: number): void {
    ...
    if (isNullOrUndefined(zIndexValue)) {
      zIndex = parseInt(this.element.style.zIndex, 10)
        ? parseInt(this.element.style.zIndex, 10)
        : this.zIndex;
    } else {
      zIndex = zIndexValue;
    }
  ...
  private positionChange(): void {
    if (this.isModal) {
      if (
        !isNaN(parseFloat(this.position.X as string)) &&
        !isNaN(parseFloat(this.position.Y as string))
      ) {
        this.setPopupPosition();
      } else if (
        (!isNaN(parseFloat(this.position.X as string)) &&
        ...
        this.dlgContainer.classList.add(
          'e-dlg-' + this.position.X + '-' + this.position.Y
        );
      }
    } else {
      this.setPopupPosition();
      ...
      this.dragObj = new Draggable(this.element, {
        ...
        dragStart: (event: Object & BlazorDragEventArgs) => {
          this.trigger(
            'dragStart',
            event,
            (dragEventArgs: Object & BlazorDragEventArgs) => {
              if (isBlazor()) {
        ...
        dragStop: (event: Object) => {
          if (this.isModal) {
            ...
            const targetType: HTMLElement = this.getTargetContainer(
              this.target
            );
            if (targetType instanceof Element) {
              const computedStyle: CSSStyleDeclaration =
  ...
  private setButton(): void {
    if (!this.isBlazorServerRender()) {
      ...
      for (let i: number = 0; i < this.buttons.length; i++) {
        ...
        const buttonType: string = !isNullOrUndefined(
          this.buttons[i as number].type
        )
          ? this.buttons[i as number].type.toLowerCase()
          : 'button';
        const btn: HTMLElement = this.createElement('button', {
    ...
    for (
      let i: number = 0, childNodes: HTMLCollection = this.element.children;
      i < childNodes.length;
      i++
    ) {
      if (childNodes[i as number].classList.contains(DLG_FOOTER_CONTENT)) {
        footerBtn = <NodeListOf<Element>>(
          (childNodes[i as number] as HTMLElement).querySelectorAll('button')
        );
      }
    }
    ...
    for (let i: number = 0; i < this.buttons.length; i++) {
      ...
      if (!this.isBlazorServerRender()) {
        this.btnObj[i as number] = new Button(
          this.buttons[i as number].buttonModel
        );
      }
      if (!isNullOrUndefined(this.ftrTemplateContent) && footerBtn.length > 0) {
        if (typeof this.buttons[i as number].click === 'function') {
          EventHandler.add(
            footerBtn[i as number],
            'click',
            this.buttons[i as number].click,
            this
          );
        }
        if (typeof this.buttons[i as number].click === 'object') {
          EventHandler.add(
            footerBtn[i as number],
            'click',
            this.buttonClickHandler.bind(this, i),
            this
          );
        }
      }
      if (
        !this.isBlazorServerRender() &&
        !isNullOrUndefined(this.ftrTemplateContent)
      ) {
        this.btnObj[i as number].appendTo(
          this.ftrTemplateContent.children[i as number] as HTMLElement
        );
        if (this.buttons[i as number].isFlat) {
          this.btnObj[i as number].element.classList.add('e-flat');
        }
        this.primaryButtonEle = this.element.getElementsByClassName(
          'e-primary'
        )[0] as HTMLElement;
      }
    }
  ...
  private setContent(): void {
    ...
    if (this.innerContentElement) {
      ...
      (!isNullOrUndefined(this.content) && this.content !== '') ||
      !this.initialRender
    ) {
  ...
  private setTemplate(
    ...
    if (!isNullOrUndefined((<HTMLElement>template).outerHTML)) {
      ...
      typeof template !== 'string' ||
      (isBlazor() && !this.isStringTemplate)
    ) {
    ...
    if (!isNullOrUndefined(templateFn)) {
      const isString: boolean =
        ...
        templateValue.indexOf('<div>Blazor') === 0
          ? this.isStringTemplate
          : true;
  ...
  public sanitizeHelper(value: string): string {
    if (this.enableHtmlSanitizer) {
      const dialogItem: BeforeSanitizeHtmlArgs =
        SanitizeHtmlHelper.beforeSanitize();
  ...
  private setMaxHeight(): void {
    ...
    this.element.style.maxHeight =
      ...
      this.targetEle.offsetHeight < window.innerHeight
        ? this.targetEle.offsetHeight - 20 + 'px'
  ...
  private setTargetContent(): void {
    if (isNullOrUndefined(this.content) || this.content === '') {
      const isContent: boolean =
        this.element.innerHTML.replace(/\s|<(\/?|\/?)(!--!--)>/g, '') !== '';
  ...
  private getValidFocusNode(items: HTMLElement[]): HTMLElement {
    ...
    for (let u: number = 0; u < items.length; u++) {
      ...
      if (
        ...
        !this.disableElement(
          node,
          '[disabled],[aria-disabled="true"],[type="hidden"]'
        )
      ) {
        return node;
      } else {
  ...
  private focusableElements(content: HTMLElement): HTMLElement {
    if (!isNullOrUndefined(content)) {
      const value: string =
        'input,select,textarea,button,a,[contenteditable="true"],[tabindex]';
      const items: HTMLElement[] = <HTMLElement[] & NodeListOf<Element>>(
        content.querySelectorAll(value)
      );
  ...
  private getAutoFocusNode(container: HTMLElement): HTMLElement {
    let node: HTMLElement = <HTMLElement>(
      container.querySelector('.' + DLG_CLOSE_ICON_BTN)
    );
  ...
  private disableElement(element: HTMLElement, t: string): HTMLElement {
    const elementMatch: Function = element
      ? element.matches ||
        element.webkitMatchesSelector ||
        element.msGetRegionContent
      : null;
    if (elementMatch) {
  ...
  private focusContent(): void {
    ...
    const node: HTMLElement = !isNullOrUndefined(element)
      ? element
      : this.element;
      ...
      switch (prop) {
        case 'content':
          if (!isNullOrUndefined(this.content) && this.content !== '') {
            ...
            if (
              !isNullOrUndefined(this.contentEle) &&
              this.contentEle.getAttribute('role') !== 'dialog'
            ) {
              if (!this.isBlazorServerRender()) {
        ...
        case 'footerTemplate':
          if (
            this.footerTemplate === '' ||
            isNullOrUndefined(this.footerTemplate)
          ) {
            if (!this.ftrTemplateContent) {
              return;
            }
        ...
        case 'showCloseIcon':
          if (this.element.getElementsByClassName(DLG_CLOSE_ICON).length > 0) {
            if (
              !this.showCloseIcon &&
              (this.header === '' || isNullOrUndefined(this.header))
            ) {
        ...
        case 'position':
          ...
          if (this.isModal) {
            ...
            if (
              this.dlgContainer.classList.contains(
                'e-dlg-' + positionX + '-' + positionY
              )
            ) {
              this.dlgContainer.classList.remove(
                'e-dlg-' + positionX + '-' + positionY
  ...
  private updateIsModal(): void {
    ...
    if (this.isModal) {
      ...
      while (this.dlgContainer.firstChild) {
        this.dlgContainer.parentElement.insertBefore(
          this.dlgContainer.firstChild,
          this.dlgContainer
        );
      }
    ...
    if (this.isModal && this.dlgOverlay) {
      EventHandler.add(
        this.dlgOverlay,
        'click',
        this.dlgOverlayClickEventHandler,
        this
      );
    }
  }
  ...
  public destroy(): void {
    ...
    if (
      ...
      this.element.classList.contains(FULLSCREEN)
    ) {
      removeClass([document.body], [DLG_TARGET, SCROLL_DISABLED]);
    }
    if (this.isModal) {
      removeClass(
        [!isNullOrUndefined(this.targetEle) ? this.targetEle : document.body],
        SCROLL_DISABLED
      );
    }
    this.unWireEvents();
    ...
    if (
      !isNullOrUndefined(this.popupObj.element) &&
      this.popupObj.element.classList.contains(POPUP_ROOT)
    ) {
    ...
    if (
      !isNullOrUndefined(this.refElement) &&
      !isNullOrUndefined(this.refElement.parentElement)
    ) {
      this.refElement.parentElement.insertBefore(
        this.isModal ? this.dlgContainer : this.element,
        this.refElement
      );
    ...
    if (this.isModal) {
      detach(this.dlgOverlay);
      this.dlgContainer.parentNode.insertBefore(
        this.element,
        this.dlgContainer
      );
  ...
  private wireEvents(): void {
    if (this.showCloseIcon) {
      EventHandler.add(
        this.closeIcon,
        'click',
        this.closeIconClickEventHandler,
        this
      );
    }
    if (this.isModal && this.dlgOverlay) {
  ...
  public show(isFullScreen?: boolean): void {
    ...
    if (
      !this.element.classList.contains(DLG_SHOW) ||
      !isNullOrUndefined(isFullScreen)
    ) {
      ...
      this.trigger(
        'beforeOpen',
        eventArgs,
        (beforeOpenArgs: BeforeOpenEventArgs) => {
          if (!beforeOpenArgs.cancel) {
            ...
            const openAnimation: Object = {
              name:
                ...
                animationMode === 'Enable'
                  ? 'Zoom' + 'In'
                  : this.animationSettings.effect + 'In',
              duration: this.animationSettings.duration,
              delay: this.animationSettings.delay,
            };
            const zIndexElement: HTMLElement = this.isModal
              ? this.element.parentElement
              : this.element;
            ...
            if (this.isModal) {
              ...
              if (targetType instanceof Element) {
                ...
                if (
                  computedStyle.getPropertyValue('direction') === 'rtl' &&
                  !this.IsDragStop
                ) {
                  this.setPopupPosition();
                }
  ...
  public hide(event?: Event): void {
    ...
    if (this.preventVisibility) {
      ...
      this.trigger(
        'beforeClose',
        eventArgs,
        (beforeCloseArgs: BeforeCloseEventArgs) => {
          if (!beforeCloseArgs.cancel) {
            ...
            if (
              document.body.classList.contains(DLG_TARGET) &&
            ...
            const closeAnimation: Object = {
              name:
                ...
                animationMode === 'Enable'
                  ? 'Zoom' + 'Out'
                  : this.animationSettings.effect + 'Out',
              duration: this.animationSettings.duration,
              delay: this.animationSettings.delay,
            };
            if (
  ...
  private fullScreen(args: boolean): boolean {
    ...
    if (args) {
      ...
      this.element.style.maxHeight = !isNullOrUndefined(this.target)
        ? this.targetEle.offsetHeight + 'px'
  ...
  private unWireButtonEvents(): void {
    if (
      ...
      for (let i: number = 0; i < this.buttons.length; i++) {
        if (
          this.buttons[i as number].click &&
          typeof this.buttons[i as number].click === 'function' &&
...
export namespace DialogUtility {
  ...
  export function confirm(args?: ConfirmDialogArgs | string): Dialog {
    ...
    confirmDialogObj.close = () => {
      ...
      if (confirmDialogObj.element.classList.contains('e-dlg-modal')) {
        ...
        (confirmDialogObj.target as HTMLElement).classList.remove(
          DLG_UTIL_ROOT
        );
      } else {
  ...
  function formOptions(
    options: DialogModel,
    option: AlertDialogArgs
  ): DialogModel {
    options.header = !isNullOrUndefined(option.title) ? option.title : null;
    options.content = !isNullOrUndefined(option.content) ? option.content : '';
    options.isModal = !isNullOrUndefined(option.isModal)
      ? option.isModal
      : true;
    options.showCloseIcon = !isNullOrUndefined(option.showCloseIcon)
      ? option.showCloseIcon
      : false;
    options.allowDragging = !isNullOrUndefined(option.isDraggable)
      ? option.isDraggable
      : false;
    options.closeOnEscape = !isNullOrUndefined(option.closeOnEscape)
      ? option.closeOnEscape
      : false;
    options.position = !isNullOrUndefined(option.position)
      ? option.position
      : { X: 'center', Y: 'top' };
    options.animationSettings = !isNullOrUndefined(option.animationSettings)
      ? option.animationSettings
      : { effect: 'Fade', duration: 400, delay: 0 };
    options.cssClass = !isNullOrUndefined(option.cssClass)
      ? option.cssClass
      : '';
  ...
  function setAlertButtonModel(
    ...
    if (!isNullOrUndefined(option.okButton)) {
      options.buttons[0] = formButtonModel(
        options.buttons[0],
        option.okButton,
        alertButtonModel[0]
      );
  ...
  // eslint-disable-next-line
  function setConfirmButtonModel(
    options: DialogModel,
    option?: ConfirmDialogArgs
  ): DialogModel {
    const okButtonModel: ButtonPropsModel = {
      buttonModel: { isPrimary: true, content: 'OK' },
      click: function (): void {
    ...
    if (!isNullOrUndefined(option.cancelButton)) {
      options.buttons[1] = formButtonModel(
        options.buttons[1],
        option.cancelButton,
        cancelButtonModel
      );
  ...
  // eslint-disable-next-line
  function formButtonModel(
    buttonModel: ButtonPropsModel,
    option: ButtonArgs,
    buttonPropModel: ButtonPropsModel
  ): ButtonPropsModel {
