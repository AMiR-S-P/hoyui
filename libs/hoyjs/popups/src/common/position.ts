...
export function calculateRelativeBasedPosition(
  ...
  while (
    (element.offsetParent || fixedElement) &&
    anchor &&
...
function getElementBottom(): number {
  return fixedParent
    ? elementRect.bottom
    : elementRect.bottom + getBodyScrollTop();
}
/**
 * @returns {number} - specifies the number value
 */
...
function getElementRight(): number {
  const popupWidth: number =
    element &&
    (element.classList.contains('e-date-wrapper') ||
      element.classList.contains('e-datetime-wrapper') ||
      (element.classList.contains('e-ddl') &&
        element.classList.contains('e-rtl')) ||
      element.classList.contains('e-date-range-wrapper'))
      ? popupRect
        ? popupRect.width
        : 0
      : popupRect && elementRect.width >= popupRect.width
      ? popupRect.width
      : 0;
