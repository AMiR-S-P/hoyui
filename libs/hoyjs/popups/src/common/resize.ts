...
export function createResize(args: ResizeArgs): void {
  ...
  for (let i: number = 0; i < directions.length; i++) {
    if (
      dialogBorderResize.indexOf(directions[i as number]) >= 0 &&
      directions[i as number]
    ) {
      ...
      const resizeHandler: HTMLElement = createElement('div', {
        className:
          'e-icons ' + RESIZE_HANDLER + ' ' + 'e-' + directions[i as number],
      });
  ...
  if (
    args.proxy &&
    args.proxy.element &&
    args.proxy.element.classList.contains('e-dialog')
  ) {
...
function wireEvents(args?: any): void {
  ...
  const resizers: NodeListOf<Element> = targetElement.querySelectorAll(
    '.' + RESIZE_HANDLER
  );
  ...
  const borderResizers: NodeListOf<Element> = targetElement.querySelectorAll(
    '.e-dialog-border-resize'
  );
...
function onMouseDown(e: MouseEvent): void {
  ...
  if (
    this.targetEle &&
    targetElement &&
    targetElement.querySelector('.' + DIALOG_RESIZABLE)
  ) {
    containerElement =
      this.target === ('body' || 'document.body' || document.body)
        ? null
        : this.targetEle;
    maxWidth = this.targetEle.clientWidth;
    maxHeight = this.targetEle.clientHeight;
  }
  const target: Document | HTMLElement = isNOU(containerElement)
    ? document
    : containerElement;
...
function onMouseUp(e: MouseEvent): void {
  const touchMoveEvent: string =
    Browser.info.name === 'msie' ? 'pointermove' : 'touchmove';
  const touchEndEvent: string =
    Browser.info.name === 'msie' ? 'pointerup' : 'touchend';
  const target: Document | HTMLElement = isNOU(containerElement)
    ? document
    : containerElement;
  const eventName: string =
  ...
  if (!isNOU(document.body.querySelector('.' + FOCUSED_HANDLER))) {
    document.body
      .querySelector('.' + FOCUSED_HANDLER)
      .classList.remove(FOCUSED_HANDLER);
  }
  if (!isNOU(resizeEnd)) {
    const proxy: any = null || this;
    resizeEnd(e, proxy);
...
function onTouchStart(e: TouchEvent | MouseEvent): void {
  targetElement = (e.target as HTMLElement).parentElement;
  calculateValues();
  const dialogResizeElement: boolean =
    targetElement.classList.contains('e-dialog');
  if (
    ((e.target as HTMLElement).classList.contains(RESIZE_HANDLER) ||
      (e.target as HTMLElement).classList.contains('e-dialog-border-resize')) &&
    dialogResizeElement
  ) {
    (e.target as HTMLElement).classList.add(FOCUSED_HANDLER);
  }
  const coordinates: Touch | MouseEvent = (e as TouchEvent).touches
...
function onMouseMove(e: MouseEvent): void {
  if (
    (e.target as HTMLElement).classList.contains(RESIZE_HANDLER) &&
    (e.target as HTMLElement).classList.contains(FOCUSED_HANDLER)
  ) {
  ...
  if (!isNOU(selectedHandler)) {
    ...
    for (let i: number = 0; i < elementClass.length; i++) {
      if (
        selectedHandler.classList.contains('e-' + elementClass[i as number])
      ) {
...
function resizeSouth(e: MouseEvent | TouchEvent): void {
  ...
  if (!isNOU(containerElement)) {
    ...
    isNOU(containerElement) &&
    (documentHeight - currentpageY >= 0 || targetRectValues.top < 0)
  ) {
    calculateValue = true;
  }
  let calculatedHeight: number =
    originalHeight + (currentpageY - originalMouseY);
  calculatedHeight =
  ...
  const borderValue: number = isNOU(containerElement)
    ? 0
    : containerElement.offsetHeight - containerElement.clientHeight;
  let topWithoutborder: number =
    targetRectValues.top - containerTop - borderValue / 2;
  topWithoutborder = topWithoutborder < 0 ? 0 : topWithoutborder;
  if (
    targetRectValues.top > 0 &&
    topWithoutborder + calculatedHeight > maxHeight
  ) {
    ...
    targetElement.style.height =
      maxHeight - parseInt(topWithoutborder.toString(), 10) + 'px';
    return;
  }
  ...
  if (calculateValue) {
    if (
      targetRectValues.top < 0 &&
      documentHeight + (targetRectValues.height + targetRectValues.top) > 0
    ) {
      targetTop = targetRectValues.top;
      if (calculatedHeight + targetTop <= 30) {
        calculatedHeight =
          targetRectValues.height -
          (targetRectValues.height + targetRectValues.top) +
          30;
      }
    }
    if (calculatedHeight + targetRectValues.top >= maxHeight) {
      targetElement.style.height =
        targetRectValues.height +
        (documentHeight - (targetRectValues.height + targetRectValues.top)) +
        'px';
    }
    const calculatedTop: number = isNOU(containerElement)
      ? targetTop
      : topWithoutborder;
    if (
      calculatedHeight >= minHeight &&
      calculatedHeight + calculatedTop <= maxHeight
    ) {
...
function resizeNorth(e: MouseEvent | TouchEvent): void {
  ...
  if (
    !isNOU(containerElement) &&
    targetRectValues.top - boundaryRectValues.top > 0
  ) {
    calculateValue = true;
...
function resizeWest(e: MouseEvent | TouchEvent): void {
  ...
  if (isNOU(resizeWestWidth)) {
    if (!isNOU(containerElement)) {
      resizeWestWidth =
        targetRectValues.left - left - borderValue / 2 + targetRectValues.width;
      resizeWestWidth =
        resizeWestWidth + (containerWidth - borderValue - resizeWestWidth);
    } else {
  ...
  if (
    !isNOU(containerElement) &&
    Math.floor(
      targetRectValues.left -
        rectValues.left +
        targetRectValues.width +
        (rectValues.right - targetRectValues.right)
    ) -
      borderValue <=
      maxWidth
  ) {
  ...
  if (setLeft) {
    calculatedWidth =
      calculatedWidth > resizeWestWidth ? resizeWestWidth : calculatedWidth;
  }
  if (calculateValue) {
...
function resizeEast(e: MouseEvent | TouchEvent): void {
  ...
  const targetRectValues: ClientRect | DOMRect =
    getClientRectValues(targetElement);
  if (
    !isNOU(containerElement) &&
    (targetRectValues.left -
      containerRectValues.left +
      targetRectValues.width <=
      maxWidth ||
      targetRectValues.right - containerRectValues.left >=
        targetRectValues.width)
  ) {
    calculateValue = true;
  ...
  if (targetRectValues.left - containerLeft + calculatedWidth > maxWidth) {
    ...
    targetElement.style.width =
      maxWidth - (targetRectValues.left - containerLeft) + 'px';
  }
  if (calculateValue) {
    if (calculatedWidth >= minWidth && calculatedWidth <= maxWidth) {
      targetElement.style.width = calculatedWidth + 'px';
...
export function removeResize(): void {
  const handlers: NodeListOf<HTMLElement> = targetElement.querySelectorAll(
    '.' + RESIZE_HANDLER
  );
  for (let i: number = 0; i < handlers.length; i++) {
    detach(handlers[i as number]);
  }
  const borderResizers: NodeListOf<HTMLElement> =
    targetElement.querySelectorAll('.e-dialog-border-resize');
  if (!isNOU(borderResizers)) {
    for (let i: number = 0; i < borderResizers.length; i++) {
