...
export function fit(
  ...
  if (axis.X) {
    const containerWidth: number = targetContainer
  ...
  if (axis.Y) {
    const containerHeight: number = targetContainer
    ...
    if (elemData.height > containerHeight) {
      if (overTop > 0 && overBottom <= 0) {
        ...
        position.top =
          overTop > overBottom
            ? containerBottom - elemData.height
            : containerTop;
      }
    } else if (overTop > 0) {
...
function setPopup(
  ...
  if (
    ...
    const data: OffsetPosition = calculatePosition(
  ...
  element.style.top =
    pos.position.top / scaleY + pos.offsetY - top / scaleY + 'px';
  element.style.left =
    pos.position.left / scaleX + pos.offsetX - left / scaleX + 'px';
}
/**
 *
 * @param {HTMLElement} element - specifies the element
...
function updateElementData(
  ...
  pos.position = calculatePosition(
    target,
    pos.posX,
    pos.posY,
    fixedParent,
...
function leftFlip(
  ...
  if (
    (collideSide.leftSide && !collideSide.rightSide) ||
    (!collideSide.leftSide && collideSide.rightSide)
  ) {
    if (pos.posX === 'right') {
      pos.posX = 'left';
...
function topFlip(
  ...
  if (
    tEdge.BL.top >= ContainerBottom() &&
    target.getBoundingClientRect().bottom < window.innerHeight
  ) {
    collideSide.bottomSide = false;
  }
  if (
    (collideSide.topSide && !collideSide.bottomSide) ||
    (!collideSide.topSide && collideSide.bottomSide)
  ) {
    if (pos.posY === 'top') {
      pos.posY = 'bottom';
...
function ContainerRight(): number {
  if (targetContainer) {
    return (
      getBodyScrollLeft() + getTargetContainerLeft() + getTargetContainerWidth()
    );
  }
...
function ContainerBottom(): number {
  if (targetContainer) {
    return (
      getBodyScrollTop() + getTargetContainerTop() + getTargetContainerHeight()
    );
  }
...
function getBodyScrollTop(): number {
  ...
  return (
    parentDocument.documentElement.scrollTop || parentDocument.body.scrollTop
  );
}
/**
...
function getBodyScrollLeft(): number {
  ...
  return (
    parentDocument.documentElement.scrollLeft || parentDocument.body.scrollLeft
  );
}
...
function getViewPortWidth(): number {
  ...
  const documentReact: ClientRect =
    document.documentElement.getBoundingClientRect();
  const offsetWidth: number = isNullOrUndefined(document.documentElement)
    ? 0
    : documentReact.width;
