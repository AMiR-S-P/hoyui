...
import { CreateElementArgs } from '@hoyui/buttons';
  ...
  switch (action) {
    case 'Create':
      ...
      const element: HTMLElement = (<HTMLElement>(
        document.querySelector(options.target as string)
      )) as HTMLElement;
...
export function createSpinner(
  ...
  let radius: number;
  const makeElement: createElementParams = !isNullOrUndefined(
    internalCreateElement
  )
    ? internalCreateElement
    : createElement;
  // eslint-disable-next-line
  let container: { wrap: HTMLElement; inner_wrap: HTMLElement } =
    create_spinner_container(args.target, makeElement);
  if (!isNullOrUndefined(args.cssClass)) {
    const classNames: string[] = args.cssClass
      .split(' ')
      .filter((className: string) => className.trim() !== '');
    container.wrap.classList.add(...classNames);
  }
  if (!isNullOrUndefined(args.template) || !isNullOrUndefined(spinTemplate)) {
    const template: string = !isNullOrUndefined(args.template)
      ? args.template
      : spinTemplate;
    container.wrap.classList.add(CLS_SPINTEMPLATE);
    replaceContent(container.wrap, template, spinCSSClass);
  } else {
    const theme: string = !isNullOrUndefined(args.type)
      ? args.type
      : getTheme(container.wrap);
    const width: string | number = !isNullOrUndefined(args.width)
      ? args.width
      : undefined;
...
function createLabel(
  ...
  label: string,
  makeElement: createElementParams
): HTMLElement {
 ...
 * @returns {void}
 */
function createMaterial3Spinner(
  container: HTMLElement,
  radius: number,
  makeElement: createElementParams
): void {
 ...
 * @returns {void}
 */
function createBootstrap4Spinner(
  container: HTMLElement,
  radius: number,
  makeElement: createElementParams
): void {
 ...
 * @returns {void}
 */
function createBootstrap5Spinner(
  container: HTMLElement,
  radius: number,
  makeElement: createElementParams
): void {
 ...
 * @returns {void}
 */
function startMatAnimate(
  container: HTMLElement,
  uniqueID: string,
  radius: number
): void {
  const globalObject: { [key: string]: GlobalVariables } = {};
  const timeOutVar: number = 0;
  globalTimeOut[`${uniqueID}`].timeOut = 0;
  globalObject[`${uniqueID}`] = globalVariables(uniqueID, radius, 0, 0);
  const spinnerInfo: SpinnerInfo = {
    uniqueID: uniqueID,
    container: container,
 ...
 * @returns {void}
 */
function createTailwindSpinner(
  container: HTMLElement,
  radius: number,
  makeElement: createElementParams
): void {
 ...
 * @returns {void}
 */
function createHighContrastSpinner(
  container: HTMLElement,
  radius: number,
  makeElement: createElementParams
): void {
...
function getTheme(container: HTMLElement): string {
  const theme: string = window
    .getComputedStyle(container as Element, ':after')
...
function setTheme(
  ...
  const innerContainer: HTMLElement = container.querySelector(
    '.' + CLS_SPININWRAP
  ) as HTMLElement;
  const svg: SVGSVGElement = innerContainer.querySelector(
    'svg'
  ) as SVGSVGElement;
 ...
 * @returns {void}
 */
function createBootstrapSpinner(
 ...
 */
// eslint-disable-next-line
function create_bootstrap_element(
  innerContainer: HTMLElement,
  uniqueID: string,
  makeElement: createElementParams
): void {
  const svgBoot: SVGSVGElement = document.createElementNS(
  ...
  for (let item: number = 0; item <= 7; item++) {
    const bootCircle: SVGCircleElement = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'circle'
    );
    bootCircle.setAttribute('class', CLS_SPINCIRCLE + '_' + item);
    bootCircle.setAttribute('r', defaultRadius + '');
    bootCircle.setAttribute(
      'transform',
 ...
 */
// eslint-disable-next-line
function boot_calculate_attributes(
  innerContainer: HTMLElement,
  radius: number
): void {
  const svg: SVGSVGElement = innerContainer.querySelector(
  ...
  for (let item: number = 0; item <= 7; item++) {
    const start: { x: number; y: number } = defineArcPoints(
      x,
      y,
      rad,
      startArc
    );
    const circleEle: SVGCircleElement = svg.querySelector(
      '.' + CLS_SPINCIRCLE + '_' + item
    ) as SVGCircleElement;
...
function animateBootstrap(innerContainer: HTMLElement): void {
  ...
  for (let i: number = 1; i <= 8; i++) {
    const circleEle: SVGCircleElement = innerContainer.getElementsByClassName(
      'e-path-circle_' + (i === 8 ? 0 : i)
    )[0] as SVGCircleElement;
  ...
  function rotation(
    ...
    function boot_animate(radius: number): void {
      if (globalTimeOut[`${id}`].isAnimate) {
        ...
        // eslint-disable-next-line
        globalTimeOut[id].timeOut = setTimeout(
          boot_animate.bind(null, series[count]),
          18
        );
      }
    }
...
function replaceContent(
  ...
  const inner: HTMLElement = container.querySelector(
    '.e-spinner-inner'
  ) as HTMLElement;
 ...
 */
// eslint-disable-next-line
function create_fabric_element(
  innerCon: HTMLElement,
  uniqueID: string,
  themeClass: string,
  makeElement: createElementParams
): void {
  const svgFabric: SVGSVGElement = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'svg'
  );
  svgFabric.setAttribute('id', uniqueID);
  svgFabric.setAttribute('class', themeClass);
  const fabricCirclePath: SVGPathElement = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'path'
  );
  fabricCirclePath.setAttribute('class', CLS_SPINCIRCLE);
  const fabricCircleArc: SVGPathElement = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'path'
  );
 ...
 */
// eslint-disable-next-line
function create_material_element(
  innerContainer: HTMLElement,
  uniqueID: string,
  makeElement: createElementParams,
  cls: string
): void {
  const svgMaterial: SVGSVGElement = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'svg'
  );
  const matCirclePath: SVGPathElement = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'path'
  );
 ...
 */
// eslint-disable-next-line
function create_spinner_container(
  target: HTMLElement,
  makeElement: createElementParams
): { wrap: HTMLElement; inner_wrap: HTMLElement } {
...
function animateMaterial(spinnerInfo: SpinnerInfo): void {
  ...
  createCircle(
    ...
    easeAnimation,
    duration,
    spinnerInfo.globalInfo[spinnerInfo.uniqueID].count,
    max,
    spinnerInfo
  );
  spinnerInfo.globalInfo[spinnerInfo.uniqueID].count =
    ++spinnerInfo.globalInfo[spinnerInfo.uniqueID].count % 4;
}
/**
 *
 * @param {number} start - specifies the number
...
function createCircle(
  ...
  const diameter: number = getSize(
    spinnerInfo.globalInfo[spinnerInfo.uniqueID].radius * 2 + ''
  );
  const strokeSize: number = getStrokeSize(diameter);
  const rotate: number =
    -90 * (spinnerInfo.globalInfo[spinnerInfo.uniqueID].count || 0);
  mat_animation(spinnerInfo);
  // eslint-disable-next-line
  function mat_animation(spinnerInfo: SpinnerInfo): void {
    const currentTime: number = Math.max(
      0,
      Math.min(new Date().getTime() - startTime, duration)
    );
    updatePath(
      easing(currentTime, start, change, duration),
      spinnerInfo.container
    );
    if (
      id === spinnerInfo.globalInfo[spinnerInfo.uniqueID].previousId &&
      currentTime < duration
    ) {
      // eslint-disable-next-line
      globalTimeOut[spinnerInfo.uniqueID].timeOut = setTimeout(
        mat_animation.bind(null, spinnerInfo),
        1
      );
    } else {
      animateMaterial(spinnerInfo);
  ...
  function updatePath(value: number, container: HTMLElement): void {
    if (
      !isNullOrUndefined(container.querySelector('svg.e-spin-material')) ||
      !isNullOrUndefined(container.querySelector('svg.e-spin-material3'))
    ) {
      ...
      if (
        ...
        !isNullOrUndefined(
          container
            .querySelector('svg.e-spin-material')
            .querySelector('path.e-path-circle')
        )
      ) {
        ...
        !isNullOrUndefined(
          container
            .querySelector('svg.e-spin-material3')
            .querySelector('path.e-path-circle')
        )
      ) {
      ...
      if (!isNullOrUndefined(svg)) {
        const path: SVGPathElement = svg.querySelector(
          'path.e-path-circle'
        ) as SVGPathElement;
        path.setAttribute(
          'stroke-dashoffset',
          getDashOffset(diameter, strokeSize, value, max) + ''
        );
        path.setAttribute(
          'transform',
 ...
 */
// eslint-disable-next-line
function mat_calculate_attributes(
  radius: number,
  container: HTMLElement,
  type: string,
  cls: string
  ...
  const svg: SVGSVGElement = container.querySelector(
    'svg.' + cls
  ) as SVGSVGElement;
  const path: SVGPathElement = svg.querySelector(
    'path.e-path-circle'
  ...
  if (type === 'Material' || type === 'Material3' || type === 'Fluent2') {
    ...
    path.setAttribute(
      'stroke-dasharray',
      (diameter - strokeSize) * Math.PI * 0.75 + ''
    );
    path.setAttribute(
      'stroke-dashoffset',
      getDashOffset(diameter, strokeSize, 1, 75) + ''
    );
  }
}
/**
...
function easeAnimation(
  ...
  return (
    start +
    change *
      (6 * timecount * timestamp + -15 * timestamp * timestamp + 10 * timecount)
  );
}
 ...
 */
// eslint-disable-next-line
function fb_calculate_attributes(
  radius: number,
  innerConainer: HTMLElement,
  trgClass: string
): void {
  ...
  const svg: SVGSVGElement = innerConainer.querySelector(
    '.' + trgClass
  ) as SVGSVGElement;
  const circle: SVGPathElement = svg.querySelector(
    '.e-path-circle'
  ) as SVGPathElement;
  const path: SVGPathElement = svg.querySelector(
    '.e-path-arc'
...
function showHideSpinner(container: HTMLElement, isHide: boolean): void {
  ...
  if (container) {
    if (container.classList.contains(CLS_SPINWRAP)) {
      ...
      const spinWrapCollection: NodeListOf<HTMLElement> =
      ...
      if (Browser.isIE) {
        for (let i: number = 0; i < spinWrapCollection.length; i++) {
          if (
            spinWrapCollection[i as number].parentElement &&
            spinWrapCollection[i as number].parentElement === container
          ) {
        ...
        spinnerWrap =
          Array.from(spinWrapCollection).find(
            (wrap: HTMLElement) => wrap.parentElement === container
          ) || null;
      }
    }
  }
  if (container && spinnerWrap) {
    const inner: HTMLElement = spinnerWrap.querySelector(
      '.' + CLS_SPININWRAP
    ) as HTMLElement;
    const spinCheck: boolean = isHide
      ? !spinnerWrap.classList.contains(CLS_SPINTEMPLATE) &&
        !spinnerWrap.classList.contains(CLS_HIDESPIN)
      : !spinnerWrap.classList.contains(CLS_SPINTEMPLATE) &&
        !spinnerWrap.classList.contains(CLS_SHOWSPIN);
    if (spinCheck) {
      const svgEle: SVGSVGElement = spinnerWrap.querySelector(
        'svg'
      ) as SVGSVGElement;
...
export function setSpinner(
  ...
  const container: Element[] = <NodeListOf<Element> & Element[]>(
    document.querySelectorAll('.' + CLS_SPINWRAP)
  );
  for (let index: number = 0; index < container.length; index++) {
    ensureTemplate(
      args.template,
      <HTMLElement>container[index as number],
      args.type,
      args.cssClass,
      makeElement
    );
...
function ensureTemplate(
  ...
  theme: string,
  cssClass: string,
  makeEle: createElementParams
): void {
  if (
    isNullOrUndefined(template) &&
    !container.classList.contains(CLS_SPINTEMPLATE)
  ) {
  ...
  if (!isNullOrUndefined(svgElement)) {
    const radius: number =
      theme === 'Bootstrap'
        ...
        : parseFloat(svgElement.style.height) / 2;
