...
describe('Position Module Specs', () => {
  ...
  function updateoffset(): void {
    offset = {
      left: getTargetElement().getBoundingClientRect().left,
      top: getTargetElement().getBoundingClientRect().top,
    };
  }
  beforeAll(() => {
    ...
    element.innerHTML =
      '<div id="block"><div style="margin: 220px;"><button id="targetElement">TargetElement</button>  </div></div>';
  ...
  describe('Position calculation', () => {
    it('Params -left,bottom', () => {
      ...
      expect(calculatePosition(getTargetElement(), 'left', 'bottom')).toEqual({
        left: offset.left,
    ...
    it('Params -center,bottom', () => {
      ...
      expect(calculatePosition(getTargetElement(), 'center', 'bottom')).toEqual(
        {
          left:
            offset.left + getTargetElement().getBoundingClientRect().width / 2,
          top: offset.top + getTargetElement().getBoundingClientRect().height,
        }
      );
    });
    it('Params -right,bottom', () => {
      ...
      expect(calculatePosition(getTargetElement(), 'right', 'bottom')).toEqual({
        left: offset.left + getTargetElement().getBoundingClientRect().width,
        top: offset.top + getTargetElement().getBoundingClientRect().height,
      });
    });
    ...
    it('Params -left,center', () => {
      ...
      expect(calculatePosition(getTargetElement(), 'left', 'center')).toEqual({
        left: offset.left,
    ...
    it('Params -right,center', () => {
      ...
      expect(calculatePosition(getTargetElement(), 'right', 'center')).toEqual({
        left: offset.left + getTargetElement().getBoundingClientRect().width,
        top: offset.top + getTargetElement().getBoundingClientRect().height / 2,
      });
    });
    it('Params -center,center', () => {
      ...
      expect(calculatePosition(getTargetElement(), 'center', 'center')).toEqual(
        {
          left:
            offset.left + getTargetElement().getBoundingClientRect().width / 2,
          top:
            offset.top + getTargetElement().getBoundingClientRect().height / 2,
        }
      );
    });
    it('Params -left,top', () => {
      ...
      expect(calculatePosition(getTargetElement(), 'left', 'top')).toEqual(
        offset
      );
    });
    it('Params -center,top', () => {
      ...
      expect(calculatePosition(getTargetElement(), 'center', 'top')).toEqual({
        left:
          offset.left + getTargetElement().getBoundingClientRect().width / 2,
        top: offset.top,
      });
    ...
    it('Params -right,top', () => {
      ...
      expect(calculatePosition(getTargetElement(), 'right', 'top')).toEqual({
        left: offset.left + getTargetElement().getBoundingClientRect().width,
        top: offset.top,
      });
    });
  ...
  describe('with different Position css Values', () => {
    ...
    it('inside the scroller content', () => {
      getTargetElement().parentElement.style.position = 'relative';
      (getTargetElement() as HTMLElement).style.position = 'abosolute';
      (document.body.querySelector('#block') as HTMLElement).style.height =
        '150px';
      (document.body.querySelector('#block') as HTMLElement).style.overflow =
        'scroll';
      updateoffset();
  ...
  describe('improper function Params scenario-', () => {
    it('API Call with null parameter', () => {
      ...
      expect(calculatePosition(null, 'left', 'top')).toEqual({
        left: 0,
        top: 0,
      });
    });
    it('API Call with un attached element', () => {
      ...
      //ToEqual - left:0,top:0
      expect(
        calculatePosition(createElement('div', { id: 'sample' }), 'left', 'top')
      ).toEqual({ left: 0, top: 0 });
    });
    it('API Call with with improper positionX, positionY', () => {
      ...
      //Expected - Get the Element Position ~ left--top.
      //ToEqual - Get the Element getBoundingClientRect ~ Top--Left .
      expect(calculatePosition(getTargetElement(), null, undefined)).toEqual({
        left: offset.left,
        top: offset.top,
      });
    });
...
describe('position calculation for popup-', () => {
  ...
  beforeAll(() => {
    ...
    element = createElement('div', { id: 'dialogSample' });
    element.innerHTML =
      '<div id="block"><div style="margin: 713px;"><button id="targetElement">TargetElement</button>  </div></div>';
    document.body.appendChild(element);
    updateoffset();
  });
  afterAll(() => {
  ...
  it('if bottom and right collison occurs', () => {
    expect(calculatePosition(getTargetElement(), 'right', 'top')).toEqual({
      left: document.querySelector('#targetElement').getBoundingClientRect()
        .right,
      top: document.querySelector('#targetElement').getBoundingClientRect().top,
    });
  });
});
...
describe('Position calculation - calculateRelativeBasedPosition -', () => {
  ...
  beforeAll(() => {
    ...
    element = createElement('div', {
      id: 'dialogSample',
      styles: 'position:absolute; margin:100px',
    });
    element.innerHTML =
      '<div id="block" style="position: absolute;margin: 203px;"><div style="margin: 220px;overflow: scroll;height: 77px;width: 182px;position: absolute;"><button id="targetElement" style="margin: 48px;">TargetElement</button>  </div></div>';
    document.body.appendChild(element);
    element.appendChild(
      createElement('div', {
        id: 'sampleElement',
        innerHTML: 'Related Element',
        styles: 'margin:"100px"',
      })
    );
    updateoffset();
  ...
  it('Position Calculation with the different offset parent.', () => {
    ...
    expect(
      calculateRelativeBasedPosition(
        <HTMLElement>getTargetElement(),
        <HTMLElement>document.querySelector('#sampleElement')
      )
    ).toEqual({ left: 471, top: 471 });
  });
  it('Invalid parametter checkups', () => {
    //Expected - Relative Element Positions check ups.
    //ToEqual - {left: 471, top: 471}
    expect(calculateRelativeBasedPosition(null, null)).toEqual({
      left: 0,
      top: 0,
    });
  });
