...
import { AnimationModel } from '@hoyui/base';
import {
  calculatePosition,
  OffsetPosition as eleOffset,
} from '../../src/common/position';
...
describe('Popup # ', () => {
  ...
  describe('Dynamically change the properties ', () => {
    ...
    it('position properties changes', () => {
      ...
      target.innerHTML =
        '<div id="par" style="position: relative"><div style="" id="targetElement1"></div></div>';
    ...
    it('position properties changes', () => {
      ...
      popup.dataBind();
      expect(popup.element.style.left).toEqual(
        (paroffs.left - offs.left).toString() + 'px'
      );
    });
  });
  ...
  describe('Animation show method test cases', function () {
    ...
    it('show method test case', function (done) {
      let options: AnimationModel = {
        name: 'FadeIn',
        duration: 400,
        delay: 10,
      };
      let flip: Function = (posLeft: number, posTop: number) => {
  ...
  describe('Animation hide method test cases', function () {
    ...
    it('hide method animation test', function (done) {
      ...
      setTimeout(function () {
        expect(popupHide.element.classList.contains('e-popup-close')).toEqual(
          true
        );
        done();
      }, 900);
  ...
  describe('Position related testing ', () => {
    beforeEach(() => {
      ...
      element.innerHTML =
        '<div style="margin: 220px"><div style="position: static" id="targetElement"></div></div>';
      document.body.appendChild(element);
      let element1: HTMLElement = createElement('div1', { id: 'blocks' });
      element1.innerHTML =
        '<div id="par" style="position: relative"><div style="position: static" id="targetElement1"></div></div>';
    ...
    it('parent static position ', () => {
      ...
      document.getElementById('targetElement1').appendChild(ele);
      let offs: eleOffset = calculatePosition(
        getTargetElement(),
        'bottom',
        'left'
      );
      ...
      expect(ele.getBoundingClientRect().left).toEqual(
        calculatePosition(ele, 'bottom', 'left').left
      );
      expect(ele.getBoundingClientRect().top).toEqual(
        calculatePosition(ele, 'bottom', 'left').top
      );
    });
    /**
    ...
    it('parent static position with relateTo ', () => {
      let target: HTMLElement = document.body.querySelector(
        '#targetElement'
      ) as HTMLElement;
      popup = new Popup(ele, {
        width: '300px',
      ...
      popup.show(options);
      expect(popup.element.offsetLeft).toEqual(
        calculatePosition(ele, 'bottom', 'left').left
      );
      expect(popup.element.offsetTop).toEqual(
        calculatePosition(ele, 'bottom', 'left').top
      );
    });
    /**
    ...
    it('parent relative position with relateTo property ', () => {
      ...
      left = ((<HTMLElement>popup.relateTo).offsetParent as HTMLElement)
        .offsetLeft;
      expect(popup.element.style.left).toEqual(left.toString() + 'px');
      top = ((<HTMLElement>popup.relateTo).offsetParent as HTMLElement)
        .offsetTop;
      expect(popup.element.style.top).toEqual(top.toString() + 'px');
    });
    /**
    ...
    it('parent relative position with relateTo property ', () => {
      ...
      left =
        ((<HTMLElement>popup.relateTo).offsetParent as HTMLElement).offsetLeft +
        popup.offsetX;
      expect(popup.element.style.left).toEqual(left.toString() + 'px');
      top =
        ((<HTMLElement>popup.relateTo).offsetParent as HTMLElement).offsetTop +
        popup.offsetY;
      expect(popup.element.style.top).toEqual(top.toString() + 'px');
    });
    /**
    ...
    it('get scrollable parents', () => {
      ...
      target.innerHTML =
        '<div id="par" style="position: relative;overflow:scroll"><div style="position:absolute" id="targetElement1"></div></div>';
      document.body.innerHTML = '';
      document.body.appendChild(target);
      popup = new Popup(ele, {
        width: '300px',
      ...
      document.getElementById('targetElement1').appendChild(ele);
      let elements: HTMLElement[] = popup.getScrollableParent(
        document.getElementById('targetElement1')
      );
      expect(elements.length).toBe(2);
    });
    it('find fixed parent', () => {
      let target: HTMLElement = createElement('div', {
        id: 'blocks',
        styles: 'height:500px; width: 500px',
      });
      target.innerHTML =
        '<div id="par" style="position: fixed;overflow:scroll"><div style="position:absolute" id="targetElement1"></div></div>';
      document.body.innerHTML = '';
      document.body.appendChild(target);
      popup = new Popup(ele, {
        width: '300px',
        position: { X: 'bottom', Y: 'left' },
        collision: { X: 'none', Y: 'flip' },
      });
      document.body.appendChild(ele);
      let elements: HTMLElement[] = popup.getScrollableParent(
        document.getElementById('targetElement1')
    ...
    it('find fixed parent inside tab components', () => {
      ...
      let ele: HTMLElement = createElement('div', { id: 'tspopup' });
      document.body.appendChild(ele);
      let popup1: Popup = new Popup(ele, {
        width: '300px',
        position: { X: 'bottom', Y: 'left' },
        collision: { X: 'none', Y: 'flip' },
      });
    ...
    it('get scrollable parents with hidden parrents', () => {
      ...
      let target: HTMLElement = createElement('div', { id: 'blocks' });
      target.innerHTML =
        '<div id="par" style="position: relative; overflow:visible;height: 100px;width: 100px;"><div style="position: static" id="targetElement1"></div></div>';
      document.body.appendChild(target);
      popup = new Popup(ele, {
        width: '300px',
        position: { X: 'bottom', Y: 'left' },
    ...
    it('parent relative position for enum type with relateTo target', () => {
      let element1: HTMLElement = createElement('div1', { id: 'blocks' });
      element1.innerHTML =
        '<div id="targetElement1" style="position: relative"></div>';
      document.body.appendChild(element1);
      let target: HTMLElement = document.body.querySelector(
        '#targetElement1'
      ) as HTMLElement;
  ...
  describe('isElementOnViewport method test', function () {
    ...
    beforeEach(function () {
      ...
      flexContainer.innerHTML =
        "<div id='targetWrapper' style='overflow:scroll;height:1000px'>" +
      ...
      popObj = new Popup(popupEle, {
        actionOnScroll: 'reposition',
        relateTo: document.getElementById('target'),
      });
    });
    ...
    it(' nexted scroll actions - reposition ', function () {
      ...
      expect(
        popObj.isElementOnViewport(document.getElementById('target'))
      ).toEqual(false);
...
describe('Popup # Custom Flip Checkup with element viewport # ', () => {
  it('Popup element Test Cases- top flip', function () {
    ...
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;width: 400px;background: #4e699c;margin: 100px 0px 50px 165px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 65px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
    popObj = new Popup(element, {
      position: { X: 'left', Y: 'top' },
      viewPortElement: <HTMLElement>getElem('#targetContainer'),
      targetType: 'relative',
      collision: { X: 'none', Y: 'flip' },
      relateTo: target,
      offsetX: 0,
    ...
    popObj.show();
    expect(calculatePosition(element)).toEqual(
      calculatePosition(target, 'left', 'bottom')
    );
  ...
  it('Popup element Test Cases- no collide flip', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;width: 400px;background: #4e699c;margin: 100px 0px 50px 165px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 150px 0px 0px 153px;"></div><div id="popup" style="height: 100px;width: 100px;background: rgb(88, 82, 82);position: absolute;left: 326px;top: 152px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
    ...
    popObj.show();
    expect(calculatePosition(element, 'left', 'bottom')).toEqual(
      calculatePosition(target, 'left', 'top')
    );
    removeContainerContent();
  });
  ...
  it('Popup element Test Cases- top center flip', function () {
    ...
    popObj.show();
    expect(calculatePosition(element)).toEqual(
      calculatePosition(target, 'center', 'bottom')
    );
    removeContainerContent();
  });
  it('Popup element Test Cases- left flip', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;width: 400px;background: #4e699c;margin: 100px 0px 50px 165px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 105px 0px 0px 53px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
    ...
    popObj.show();
    expect(calculatePosition(element)).toEqual(
      calculatePosition(target, 'right', 'top')
    );
    removeContainerContent();
  });
  it('Popup element Test Cases- left center flip', function () {
    ...
    popObj.show();
    expect(calculatePosition(element)).toEqual(
      calculatePosition(target, 'right', 'center')
    );
    removeContainerContent();
  });
  it('Popup element Test Cases- bottom flip', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;width: 400px;background: #4e699c;margin: 100px 0px 50px 165px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 243px 0px 0px 138px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
  ...
  it('Popup element Test Cases- bottom center flip', function () {
    ...
    popObj.show();
    expect(calculatePosition(element, 'left', 'bottom')).toEqual(
      calculatePosition(target, 'center', 'top')
    );
    removeContainerContent();
  });
  it('Popup element Test Cases- right flip', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;width: 400px;background: #4e699c;margin: 100px 0px 50px 165px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 105px 0px 0px 253px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
    ...
    ).toEqual(calculatePosition(target, 'left', 'top'));
    removeContainerContent();
  });
  it('Popup element Test Cases- right center flip', function () {
    ...
    popObj.show();
    expect(calculatePosition(element, 'right', 'top')).toEqual(
      calculatePosition(target, 'right', 'center')
    );
    removeContainerContent();
  });
  it('Popup element Test Cases- top left flip', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;width: 400px;background: #4e699c;margin: 100px 0px 50px 165px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 16px 0px 0px 16px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
  ...
  it('Popup element Test Cases- top right flip', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;width: 400px;background: #4e699c;margin: 100px 0px 50px 65px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 16px 0px 0px 286px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
    ...
    ).toEqual(calculatePosition(target, 'left', 'bottom'));
    removeContainerContent();
  });
  it('Popup element Test Cases- bottom  right flip', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;width: 400px;background: #4e699c;margin: 100px 0px 50px 65px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 280px 0px 0px 286px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
  ...
  it('Popup element Test Cases- bottom  left flip', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;width: 400px;background: #4e699c;margin: 100px 0px 50px 65px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 280px 0px 0px 11px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
  ...
  it('Popup element Test Cases- top fit', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;position: relative;width: 400px;background: #4e699c;margin: 100px 0px 50px 165px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 65px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
    ...
    popObj.show();
    expect(calculatePosition(element).top).toEqual(
      calculatePosition(<HTMLElement>getElem('#targetContainer'), 'left', 'top')
        .top
    );
    removeContainerContent();
  });
  it('Popup element Test Cases- left fit', function () {
    ...
    popObj.show();
    expect(calculatePosition(element).left).toEqual(
      calculatePosition(<HTMLElement>getElem('#targetContainer'), 'left', 'top')
        .left
    );
    removeContainerContent();
  ...
  it('Popup element Test Cases- bottom fit', function () {
    ...
    popObj.show();
    expect(calculatePosition(element, 'left', 'bottom').top).toEqual(
      calculatePosition(
  ...
  it('Popup element Test Cases- right fit', function () {
    ...
    popObj.show();
    expect(calculatePosition(element, 'right', 'top').left).toEqual(
      calculatePosition(
        <HTMLElement>getElem('#targetContainer'),
        'right',
        'top'
  ...
  it('Popup element Test Cases- top left fit', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;position:relative; width: 400px;background: #4e699c;margin: 100px 0px 50px 165px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 16px 0px 0px 16px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
  ...
  it('Popup element Test Cases- top right fit', function () {
    ...
    let elem: HTMLDivElement = document.createElement('div');
    elem.innerHTML =
      '<div id="targetContainer" class="flex-item" style="height: 400px;position:relative; width: 400px;background: #4e699c;margin: 100px 0px 50px 65px;float: left;"><div id="target" style="height: 100px;width: 100px;background: #af0404;float: left;margin: 16px 0px 0px 286px;"></div><div id="popup" style="height: 100px; width: 100px; background: rgb(88, 82, 82); position: absolute; left: 315px;top: 85px;"></div></div>';
    document.body.appendChild(elem.firstChild);
    let target: HTMLElement = <HTMLElement>getElem('#target'),
      element: HTMLElement = <HTMLElement>getElem('#popup'),
      targetContainer = <HTMLElement>getElem('#targetContainer');
  ...
  it('Popup element Test Cases- bottom  right fit', function () {
    ...
    popObj.show();
    expect(calculatePosition(element, 'right', 'bottom')).toEqual(
      calculatePosition(
        <HTMLElement>getElem('#targetContainer'),
        'right',
        'bottom'
...
describe('EJ2-30263 Page scrolls slow when render more than 10 Rich Text Editors in a page', () => {
  let popObj: any;
