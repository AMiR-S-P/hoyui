...
export class NonWorkingDay {
  ...
  private createNonworkingContainer(): void {
    if (
      !this.parent.ganttChartModule.chartBodyContent.contains(
  ...
  public renderHolidays(): void {
    if (this.parent.holidays && this.parent.holidays.length > 0) {
      ...
      if (
        this.nonworkingContainer &&
        this.nonworkingContainer.childNodes.length === 0
      ) {
  ...
  private getHolidaysElement(): HTMLElement {
    ...
    for (let i: number = 0; i < this.parent.holidays.length; i++) {
      if (
        this.parent.holidays[i as number].from &&
        this.parent.holidays[i as number].to
      ) {
      ...
      const align: string = this.parent.enableRtl
        ? `right:${left}px;`
        : `left:${left}px;`;
      ...
      const spanTop: number =
        viewportHeight < height ? viewportHeight / 2 : height / 2;
      const spanElement: HTMLElement = createElement('span', {
        className: cls.holidayLabel,
        styles: `top:${spanTop}px;left:${width / 2}px;`,
      });
      const property: string = this.parent.disableHtmlEncode
        ? 'textContent'
        : 'innerHTML';
      spanElement[property as string] = this.parent.holidays[i as number].label
        ? this.parent.holidays[i as number].label
        : '';
      if (
        this.parent.enableHtmlSanitizer &&
        typeof spanElement[property as string] === 'string'
      ) {
        spanElement[property as string] = SanitizeHtmlHelper.sanitize(
          spanElement[property as string]
        );
      }
  ...
  private getWeekendElements(): HTMLElement {
    const container: HTMLElement = createElement('div');
    const leftValueForStartDate: number =
    ...
    const endDate: Date = this.parent.enableTimelineVirtualization
      ? new Date(this.parent.timelineModule.weekendEndDate.getTime())
    ...
    do {
      if (nonWorkingIndex.indexOf(startDate.getDay()) !== -1) {
        ...
        if (!isFirstExecution) {
          ...
          if (
            sDateOffset !==
              this.parent.timelineModule.timelineStartDate.getTimezoneOffset() &&
  ...
  private updateHolidayLabelHeight(): void {
    ...
    const gantttable: HTMLElement = document.getElementById(
    ...
    const top: number =
      viewportHeight < height ? viewportHeight / 2 : height / 2;
    const labels: NodeList = this.holidayContainer.querySelectorAll(
      '.' + cls.holidayLabel
    );
