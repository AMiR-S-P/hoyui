...
import { Tooltip as TooltipComponent, TooltipEventArgs } from '@hoyui/popups';
...
export class Tooltip {
  ...
  public createTooltip(): void {
    ...
    this.toolTipObj.target =
      '.e-header-cell-label, .e-gantt-child-taskbar,' +
    ...
    this.toolTipObj.animation = {
      open: { effect: 'None', delay: 0 },
      close: { effect: 'None', delay: 0 },
    };
  ...
  private tooltipBeforeRender(args: TooltipEventArgs): void | Deferred {
    ...
    if (args.target.classList.contains('e-header-cell-label')) {
      ...
      if (parent.tooltipSettings.showTooltip) {
        ...
        if (
          args.target.classList.contains('e-gantt-child-taskbar') ||
          ...
          (args.target.classList.contains('e-gantt-manualparenttaskbar') &&
            parent.tooltipSettings.taskbar)
        ) {
          ...
          argsData.content = this.toolTipObj.content = taskbarTemplateNode
            ? (tooltipTemplate as HTMLElement)
            : data
            ? (parent.tooltipModule.getTooltipContent(
                data.ganttProperties.isMilestone ? 'milestone' : 'taskbar',
                data,
                parent,
                args
              ) as string | HTMLElement)
          ...
          args.target.classList.contains('e-baseline-bar') ||
          ...
          if (parent.tooltipSettings.baseline) {
            baseLineTemplateNode = parent.tooltipModule.templateCompiler(
              parent.tooltipSettings.baseline,
              parent,
              data,
              'TooltipBaselineTemplate'
            );
          }
          ...
          argsData.content = this.toolTipObj.content = baseLineTemplateNode
            ? (baselineTemplate as HTMLElement)
            : data
            ? (parent.tooltipModule.getTooltipContent(
                data.ganttProperties.isMilestone ? 'milestone' : 'baseline',
                data,
                parent,
                args
              ) as string | HTMLElement)
          ...
          args.target.classList.contains('e-connector-line-container')
        ) {
          let dependencyLineTemplateNode: NodeList;
          parent.tooltipModule.predecessorTooltipData =
            parent.tooltipModule.getPredecessorTooltipData(args);
          ...
          if (parent.tooltipSettings.connectorLine) {
            dependencyLineTemplateNode = parent.tooltipModule.templateCompiler(
              parent.tooltipSettings.connectorLine,
              parent,
              parent.tooltipModule.predecessorTooltipData,
          ...
          const ganttData: IGanttData =
            this.parent.ganttChartModule.getRecordByTarget(
              args.event as PointerEvent
            );
          argsData.content = this.toolTipObj.content =
            ganttData.ganttProperties.notes;
          if (isNullOrUndefined(argsData.content)) {
            args.cancel = true;
          }
        } else if (
          ...
          args.target.classList.contains('e-gantt-manualparent-milestone')
        ) {
          argsData.content = this.toolTipObj.content =
            parent.tooltipModule.getTooltipContent(
              'manualmilestone',
    ...
    if (args.cancel === false) {
      ...
      parent.trigger(
        'beforeTooltipRender',
        argsData,
        (argData: BeforeTooltipRenderEventArgs) => {
  ...
  private updateTooltipPosition(args: TooltipEventArgs): void {
    ...
    if (isNullOrUndefined(parentWithZoomStyle)) {
      if (
        isNullOrUndefined(this.tooltipMouseEvent) ||
        args.target.classList.contains('e-notes-info')
      ) {
        return;
      }
      const postion: { x: number; y: number } = this.getPointorPosition(
        this.tooltipMouseEvent
      );
      ...
      const topEnd: number =
        containerPosition.top + this.parent.chartPane.offsetHeight;
      const leftEnd: number =
        containerPosition.left + this.parent.chartPane.offsetWidth;
  ...
  private getPointorPosition(e: Event): { x: number; y: number } {
    ...
    let posY: number;
    if (
      !isNullOrUndefined(getValue('pageX', e)) ||
      !isNullOrUndefined(getValue('pageY', e))
    ) {
      posX = getValue('pageX', e);
      posY = getValue('pageY', e);
    } else if (
      !isNullOrUndefined(getValue('clientX', e)) ||
      !isNullOrUndefined(getValue('clientY', e))
    ) {
      posX =
        getValue('clientX', e) +
  ...
  private getTooltipContent(
    ...
    switch (elementType) {
      case 'milestone': {
        ...
        const sDate: string = !isNullOrUndefined(milestoneStartDate)
          ? '<tr><td class = "e-gantt-tooltip-label"> Date</td><td>:</td>' +
      ...
      case 'taskbar': {
        const scheduledTask: boolean =
          !ganttData.hasChildRecords || data.isAutoSchedule ? true : false;
        let startDateValue: string = this.parent.getFormatedDate(
          scheduledTask ? data.startDate : data.autoStartDate,
          this.parent.getDateFormat()
        );
        let endDateValue: string = this.parent.getFormatedDate(
          scheduledTask ? data.endDate : data.autoEndDate,
          this.parent.getDateFormat()
        );
        let durationValue: string = this.parent.getDurationString(
          scheduledTask ? data.duration : data.autoDuration,
          data.durationUnit
        );
        ...
        if (this.parent.enableHtmlSanitizer) {
          startDateValue =
            typeof startDateValue === 'string'
              ? SanitizeHtmlHelper.sanitize(startDateValue)
              : startDateValue;
          endDateValue =
            typeof endDateValue === 'string'
              ? SanitizeHtmlHelper.sanitize(endDateValue)
              : endDateValue;
          durationValue = SanitizeHtmlHelper.sanitize(durationValue);
        }
        const startDate: string = data.startDate
          ? '<tr><td class = "e-gantt-tooltip-label">' +
            this.parent.localeObj.getConstant(
              scheduledTask ? 'startDate' : 'subTasksStartDate'
            ) +
            '</td><td style="padding: 2px;">:</td>' +
            '<td class = "e-gantt-tooltip-value"> ' +
            startDateValue +
            '</td></tr>'
          : '';
        const endDate: string = data.endDate
          ? '<tr><td class = "e-gantt-tooltip-label">' +
            this.parent.localeObj.getConstant(
              scheduledTask ? 'endDate' : 'subTasksEndDate'
            ) +
            '</td><td style="padding: 2px;">:</td>' +
            '<td class = "e-gantt-tooltip-value">' +
            endDateValue +
            '</td></tr>'
          : '';
        const duration: string = !isNullOrUndefined(data.duration)
          ? '<tr><td class = "e-gantt-tooltip-label">' +
      ...
      case 'baseline': {
        let baselineStartDateValue: string = this.parent.getFormatedDate(
          data.baselineStartDate,
          this.parent.getDateFormat()
        );
        let baselineEndDateValue: string = this.parent.getFormatedDate(
          data.baselineEndDate,
          this.parent.getDateFormat()
        );
        if (this.parent.enableHtmlSanitizer) {
          baselineStartDateValue = SanitizeHtmlHelper.sanitize(
            baselineStartDateValue
          );
          baselineEndDateValue =
            SanitizeHtmlHelper.sanitize(baselineEndDateValue);
        }
        const contentTemp: Function = function (): string {
          return (
            '<table class = "e-gantt-tooltiptable"><tbody>' +
            ...
            baselineStartDateValue +
            '</td></tr><tr>' +
            '<td class = "e-gantt-tooltip-label">' +
      ...
      case 'marker': {
        const markerTooltipElement: EventMarkerModel =
          parent.tooltipModule.getMarkerTooltipData(args);
        let markerTooltipElementValue: string = this.parent.getFormatedDate(
          this.parent.dateValidationModule.getDateFromFormat(
            markerTooltipElement.day
          ),
          this.parent.getDateFormat()
        );
        let markerLabel: string = markerTooltipElement.label
          ? markerTooltipElement.label
          : '';
        if (this.parent.enableHtmlSanitizer) {
          markerLabel = SanitizeHtmlHelper.sanitize(markerLabel);
          markerTooltipElementValue = SanitizeHtmlHelper.sanitize(
            markerTooltipElementValue
          );
        }
        const contentTemp: Function = function (): string {
          return (
            '<table class = "e-gantt-tooltiptable"><tbody><tr><td>' +
            markerTooltipElementValue +
            '</td></tr><tr><td>' +
            (this.parent.disableHtmlEncode
              ? markerLabel.replace(/</g, '&lt;').replace(/>/g, '&gt;')
              : markerLabel) +
            '</td></tr></tbody></table>'
          );
        };
      ...
      case 'connectorLine': {
        let fromNameValue: string =
          parent.tooltipModule.predecessorTooltipData.fromName;
        let fromIdValue: string =
          parent.tooltipModule.predecessorTooltipData.fromId;
        let toNameValue: string =
          parent.tooltipModule.predecessorTooltipData.toName;
        let toIdValue: string =
          parent.tooltipModule.predecessorTooltipData.toId;
        let linkTextValue: string =
          parent.tooltipModule.predecessorTooltipData.linkText;
        let offsetStringValue: string =
          parent.tooltipModule.predecessorTooltipData.offsetString;
        if (this.parent.enableHtmlSanitizer) {
        ...
        const contentTemp: Function = function (): string {
          return (
            '<table class = "e-gantt-tooltiptable"><tbody><tr><td class = "e-gantt-tooltip-label">' +
            this.parent.localeObj.getConstant('from') +
            '</td><td>:</td>' +
            '<td class = "e-gantt-tooltip-value">' +
            (this.parent.disableHtmlEncode
              ? fromNameValue.replace(/</g, '&lt;').replace(/>/g, '&gt;')
              : fromNameValue) +
            ' (' +
            (this.parent.disableHtmlEncode
              ? typeof fromIdValue === 'string'
                ? fromIdValue.replace(/</g, '&lt;').replace(/>/g, '&gt;')
                : fromIdValue
              : fromIdValue) +
            ')' +
            '</td></tr><tr><td class = "e-gantt-tooltip-label">' +
            this.parent.localeObj.getConstant('to') +
            '</td><td>:</td>' +
            '<td class = "e-gantt-tooltip-value">' +
            (this.parent.disableHtmlEncode
              ? toNameValue.replace(/</g, '&lt;').replace(/>/g, '&gt;')
              : toNameValue) +
            ' (' +
            toIdValue +
            ')' +
      ...
      case 'indicator':
        if (args.target.title.length) {
          ...
          const contentTemp: Function = function (): string {
            return (
              '<table class = "e-gantt-tooltiptable"><tbody><tr>' +
              titleValue +
              '</tr></tbody></table>'
            );
          };
      ...
      case 'timeline': {
        ...
        if (
          this.parent.enableHtmlSanitizer &&
          typeof timlineTitleValue === 'string'
        ) {
      ...
      case 'manualtaskbar': {
        let autoStartDateValue: string = this.parent.getFormatedDate(
          data.autoStartDate,
          this.parent.getDateFormat()
        );
        let autoEndDateValue: string = this.parent.getFormatedDate(
          data.autoEndDate,
          this.parent.getDateFormat()
        );
        let durationUnitValue: string = this.parent.getDurationString(
          data.duration,
          data.durationUnit
        );
        let manualStartDateValue: string = this.parent.getFormatedDate(
          data.startDate,
          this.parent.getDateFormat()
        );
        let manualEndDateValue: string = this.parent.getFormatedDate(
          data.endDate,
          this.parent.getDateFormat()
        );
        if (this.parent.enableHtmlSanitizer) {
          ...
          manualStartDateValue =
            SanitizeHtmlHelper.sanitize(manualStartDateValue);
          manualEndDateValue = SanitizeHtmlHelper.sanitize(manualEndDateValue);
        }
        const autoStartDate: string =
          '<tr><td class = "e-gantt-tooltip-label">' +
      ...
      case 'manualmilestone': {
        let autoStartValue: string = this.parent.getFormatedDate(
          data.autoStartDate,
          this.parent.getDateFormat()
        );
        let autoEndValue: string = this.parent.getFormatedDate(
          data.autoEndDate,
          this.parent.getDateFormat()
        );
        let dateValue: string = this.parent.getFormatedDate(
          data.startDate,
          this.parent.getDateFormat()
        );
        if (this.parent.enableHtmlSanitizer) {
  ...
  public getMarkerTooltipData(args: TooltipEventArgs): EventMarkerModel {
    ...
    const markerTooltipElement: EventMarkerModel =
      this.parent.eventMarkers[Number(markerTooltipId)];
  ...
  public getPredecessorTooltipData(args: TooltipEventArgs): PredecessorTooltip {
    ...
    if (
      this.parent.enableHtmlSanitizer &&
      typeof predeceesorParent === 'string'
    ) {
      predeceesorParent = SanitizeHtmlHelper.sanitize(predeceesorParent);
    }
    const taskIds: string[] = predeceesorParent.match(
      /ConnectorLineparent(.*)child(.*)/
    );
    taskIds.shift();
    ...
    if (this.parent.viewType === 'ResourceView') {
      fromTask =
        this.parent.flatData[
          this.parent.getTaskIds().indexOf('T' + taskIds[0])
        ];
      toTask =
        this.parent.flatData[
          this.parent.getTaskIds().indexOf('T' + taskIds[1])
        ];
    } else {
    ...
    const predecessor: IPredecessor[] =
      fromTask.ganttProperties.predecessor.filter((pdc: IPredecessor) => {
        return pdc.to === taskIds[1];
      });
    const predecessorTooltipData: PredecessorTooltip = {
      fromId:
        this.parent.viewType === 'ResourceView'
          ? fromTask.ganttProperties.taskId
          : fromTask.ganttProperties.rowUniqueID,
      toId:
        this.parent.viewType === 'ResourceView'
          ? toTask.ganttProperties.taskId
          : toTask.ganttProperties.rowUniqueID,
      ...
      offsetString: this.parent.getDurationString(
        predecessor[0].offset,
        predecessor[0].offsetUnit
      ),
    };
  ...
  public templateCompiler(
    ...
    const tooltipFunction: Function =
      parent.chartRowsModule.templateCompiler(template);
    const templateID: string = parent.chartRowsModule.getTemplateID(propName);
    const templateNode: NodeList = tooltipFunction(
      extend({ index: 0 }, data),
      parent,
      propName,
      templateID,
