...
import { getValue, Internationalization, isNullOrUndefined } from '@hoyui/base';
...
export class EditTooltip {
  ...
  private updateTooltipPosition(args: TooltipEventArgs): void {
    ...
    if (isNullOrUndefined(parentWithZoomStyle)) {
      ...
      if (leftEnd < tooltipPositionX + args.element.offsetWidth) {
        tooltipPositionX +=
          leftEnd - (tooltipPositionX + args.element.offsetWidth);
      }
    ...
    if (bool && this.parent.tooltipSettings.showTooltip) {
      ...
      if (this.taskbarEdit.connectorSecondAction === 'ConnectorPointLeftDrag') {
        this.toolTipObj.open(
          this.taskbarEdit.connectorSecondElement.querySelector(
            '.' + cls.connectorPointLeft
          )
        );
      } else if (
        this.taskbarEdit.connectorSecondAction === 'ConnectorPointRightDrag'
      ) {
        this.toolTipObj.open(
          this.taskbarEdit.connectorSecondElement.querySelector(
  ...
  public updateTooltip(segmentIndex: number): void {
    const ganttProp: ITaskData =
      this.taskbarEdit.taskBarEditRecord.ganttProperties;
    const taskWidth: number =
      isNullOrUndefined(segmentIndex) || segmentIndex === -1
        ? ganttProp.width
        : ganttProp.segments[segmentIndex as number].width;

    const progressWidth: number =
      isNullOrUndefined(segmentIndex) || segmentIndex === -1
        ? ganttProp.progressWidth
        : ganttProp.segments[segmentIndex as number].progressWidth;

    const left: number =
      isNullOrUndefined(segmentIndex) || segmentIndex === -1
        ? ganttProp.left
        : ganttProp.left + ganttProp.segments[segmentIndex as number].left;

    if (!isNullOrUndefined(this.toolTipObj)) {
      if (
        this.taskbarEdit.taskBarEditAction === 'ConnectorPointLeftDrag' ||
        ...
        if (
          ganttProp.segments &&
          ganttProp.segments.length > 0 &&
          this.taskbarEdit['mainElement'] &&
          this.taskbarEdit.taskBarEditAction === 'ProgressResizing'
        ) {
          const segments: NodeListOf<Element> = this.taskbarEdit[
            'mainElement'
          ].querySelectorAll('.e-segmented-taskbar');
          this.toolTipObj.refresh(
            segments[segmentIndex as number] as HTMLElement
          );
        } else {
        ...
        if (this.taskbarEdit.taskBarEditAction === 'LeftResizing') {
          ...
          this.taskbarEdit.taskBarEditAction === 'RightResizing' ||
  ...
  private getTooltipText(segmentIndex: number): string | HTMLElement {
    ...
    let editRecord: ITaskData = this.taskbarEdit.taskBarEditRecord
      .ganttProperties as ITaskData;
    if (
      !isNullOrUndefined(editRecord.segments) &&
      editRecord.segments.length > 0 &&
      segmentIndex !== -1 &&
      this.taskbarEdit.taskBarEditAction !== 'ProgressResizing'
    ) {
    ...
    if (this.parent.tooltipSettings.editing) {
      const templateNode: NodeList = this.parent.tooltipModule.templateCompiler(
        this.parent.tooltipSettings.editing,
        this.parent,
        this.taskbarEdit.taskBarEditRecord,
        'TooltipEditingTemplate'
      );
      ...
      if (!isNullOrUndefined(editRecord.startDate)) {
        startDate =
          ...
          instance.formatDate(editRecord.startDate, {
      ...
      if (!isNullOrUndefined(editRecord.endDate)) {
        endDate =
          ...
          '</td><td style="padding: 2px;">:</td><td class = "e-gantt-tooltip-value">' +
          instance.formatDate(editRecord.endDate, {
            format: this.parent.getDateFormat(),
          }) +
          '</td></tr>';
      }
      if (!isNullOrUndefined(editRecord.duration)) {
        duration =
          ...
          '</td><td style="padding: 2px;">:</td><td class = "e-gantt-tooltip-value">' +
          this.parent.getDurationString(
            editRecord.duration,
            (editRecord as ITaskData).durationUnit
          ) +
          '</td></tr>';
      }
      switch (this.taskbarEdit.taskBarEditAction) {
        case 'ProgressResizing': {
          const progress: string =
            ...
            '</td><td style="padding: 2px;">:</td><td class = "e-gantt-tooltip-value">' +
            (editRecord as ITaskData).progress +
            '</td></tr>';
          tooltipString =
        ...
        case 'ManualParentDrag': {
          ...
          if (
            !isNullOrUndefined(
              this.taskbarEdit.taskBarEditRecord.ganttProperties.startDate
            )
          ) {
            sDate = startDate;
          }
          if (
            !isNullOrUndefined(
              this.taskbarEdit.taskBarEditRecord.ganttProperties.endDate
            )
          ) {
            eDate = endDate;
          }
