...
import { PdfDocument, PdfSection, PdfStringFormat } from '@hoyui/pdf-export';
...
export class PdfTreeGridLayouter extends ElementLayouter {
  ...
  public layoutInternal(param: PdfLayoutParams): PdfLayoutResult {
    ...
    if (
      ...
      this.currentBounds = new RectangleF(
        new PointF(param.bounds.x, param.bounds.y),
        new SizeF(
          this.treegrid.columns.width,
          this.currentGraphics.clientSize.height
        )
      );
    } else {
      this.currentBounds = new RectangleF(
        new PointF(param.bounds.x, param.bounds.y),
        this.currentGraphics.clientSize
      );
    }
    if (this.treegrid.rows.count !== 0) {
      this.currentBounds.width =
        param.bounds.width > 0
          ? param.bounds.width
          : this.currentBounds.width -
            this.treegrid.rows.getRow(0).cells.getCell(0).style.borders.left
              .width /
              2;
    } else {
    ...
    if (
      !this.treegrid.style.allowHorizontalOverflow &&
      !this.treegrid.isFitToWidth
    ) {
  ...
  private determineColumnDrawRanges(): void {
    ...
    const availableWidth: number = this.treegrid['taskbar'].isAutoFit()
      ? pointToPixel(this.currentGraphics.clientSize.width) -
        this.currentBounds.x
      : this.currentGraphics.clientSize.width - this.currentBounds.x;
    for (let i: number = 0; i < this.treegrid.columns.count; i++) {
      ...
      if (cellWidths >= availableWidth) {
        ...
        cellWidths =
          endColumn <= i ? this.treegrid.columns.getColumn(i).width : 0;
      }
    }
  ...
  private layoutOnPage(param: PdfLayoutParams): PdfTreeGridLayoutResult {
    ...
    const layoutedPages: TemporaryDictionary<PdfPage, number[]> =
      new TemporaryDictionary<PdfPage, number[]>();
    ...
    for (let index: number = 0; index < this.columnRanges.length; index++) {
      ...
      for (let j: number = 0; j < rowsCount; j++) {
        ...
        if (
          ...
          !layoutedPages.containsKey(this.currentPage)
        ) {
      ...
      if (
        this.columnRanges.indexOf(range) < this.columnRanges.length - 1 &&
        startPage !== null &&
        format.layout !== PdfLayoutType.OnePage
      ) {
        this.currentPage = this.getNextPageFormat(format);
    ...
    if (
      this.treegrid.style.allowHorizontalOverflow &&
      this.treegrid.style.horizontalOverflowType ===
  ...
  private checkBounds(format: PdfTreeGridLayoutFormat): void {
    const location: PointF = new PointF(
      PdfBorders.default.right.width / 2,
      PdfBorders.default.top.width / 2
    );
    if (
  ...
  private reArrangePages(
    layoutPages: TemporaryDictionary<PdfPage, number[]>
  ): void {
  ...
  private checkIfDefaultFormat(format: PdfStringFormat): boolean {
    ...
    return (
      format.alignment === defaultFormat.alignment &&
      format.characterSpacing === defaultFormat.characterSpacing &&
      format.clipPath === defaultFormat.clipPath &&
      format.firstLineIndent === defaultFormat.firstLineIndent &&
      format.horizontalScalingFactor ===
        defaultFormat.horizontalScalingFactor &&
      ...
      format.rightToLeft === defaultFormat.rightToLeft &&
  ...
  private drawRow(
    ...
    if (isNullOrUndefined(layoutResult)) {
      ...
      if (height > this.currentPageBounds.height) {
        ...
        this.currentBounds.y + height > this.currentPageBounds.height ||
      ...
      for (let i: number = this.cellStartIndex; i <= this.cellEndIndex; i++) {
        ...
        const cancelSpans: boolean =
          cell.columnSpan > 1 && i > this.cellEndIndex + 1;
        if (!cancelSpans) {
        ...
        if (
          !this.checkIfDefaultFormat(column.format) &&
          this.checkIfDefaultFormat(cell.style.format)
        ) {
        ...
        /* eslint-disable-next-line */
        if (
          row.treegrid.style.allowHorizontalOverflow &&
          (cell.columnSpan > this.cellEndIndex ||
            i + cell.columnSpan > this.cellEndIndex + 1) &&
          this.cellEndIndex < row.cells.count - 1
        ) {
      ...
      /* eslint-disable-next-line */
      layoutResult.bounds = new RectangleF(
        new PointF(layoutResult.bounds.x, layoutResult.bounds.y),
        new SizeF(location.x, location.y)
      );
      return null;
    }
  }
  ...
  private drawRowWithBreak(
    ...
    for (let i: number = this.cellStartIndex; i <= this.cellEndIndex; i++) {
      ...
      const cell: PdfTreeGridCell = row.cells.getCell(i);
      const cancelSpans: boolean =
        cell.columnSpan + i > this.cellEndIndex + 1 && cell.columnSpan > 1;
      if (!cancelSpans) {
        for (let j: number = 1; j < cell.columnSpan; j++) {
          row.cells.getCell(i + j).isCellMergeContinue = true;
        }
      ...
      result.isFinish = !result.isFinish
        ? result.isFinish
        : cell.finishedDrawingCell;
      location.x += column.width;
      leftAdjustment = 0;
      this.currentBounds.y +=
        this.treegridHeight > 0 ? this.treegridHeight : height;
      result.bounds = new RectangleF(
        new PointF(result.bounds.x, result.bounds.y),
        new SizeF(location.x, location.y)
      );
    }
  }
  ...
  public reCalculateHeight(row: PdfTreeGridRow, height: number): number {
    ...
    for (let i: number = this.cellStartIndex; i <= this.cellEndIndex; i++) {
      if (
        !isNullOrUndefined(row.cells.getCell(i).remainingString) ||
