...
export class PdfTreeGridCell {
  ...
  private measureWidth(): number {
    ...
    if (typeof this.value === 'string') {
      ...
      let font: PdfStandardFont | PdfTrueTypeFont = new PdfStandardFont(
        this.row.treegrid.ganttStyle.fontFamily,
        this.style.fontSize,
      ...
      /* eslint-disable-next-line */
      const slr: PdfStringLayoutResult = layouter.layout(
        this.value as string,
        font,
        this.style.format,
        new SizeF(Number.MAX_VALUE, Number.MAX_VALUE),
        false,
        new SizeF(0, 0)
      );
      width += slr.actualSize.width;
      width +=
        (this.style.borders.left.width + this.style.borders.right.width) * 2;
    }
    if (
      typeof this.row.treegrid.style.cellPadding.left !== 'undefined' &&
      this.row.treegrid.style.cellPadding.hasLeftPad
    ) {
      width += this.row.treegrid.style.cellPadding.left;
    }
    if (
      typeof this.row.treegrid.style.cellPadding.right !== 'undefined' &&
      this.row.treegrid.style.cellPadding.hasRightPad
    ) {
  ...
  public measureHeight(): number {
    ...
    width -=
      this.row.treegrid.style.cellPadding.right +
      this.row.treegrid.style.cellPadding.left;
    width -= this.style.borders
      ? this.style.borders.left.width + this.style.borders.right.width
      : 0;
    const layouter: PdfStringLayouter = new PdfStringLayouter();
    if (
      typeof this.value === 'string' ||
      typeof this.remainingString === 'string'
    ) {
      let currentValue: string = this.value as string;
      if (!this.finishedDrawingCell) {
        currentValue = !(
          isNullOrUndefined(this.remainingString) || this.remainingString === ''
        )
      ...
      height +=
        (this.style.borders.top.width + this.style.borders.bottom.width) * 2;
    }
    height +=
      this.row.treegrid.style.cellPadding.top +
      this.row.treegrid.style.cellPadding.bottom;
  ...
  private calculateWidth(): number {
    ...
    for (let i: number = 0; i < columnSpan; i++) {
      width += this.row.treegrid.columns.getColumn(cellIndex + i)
        ? this.row.treegrid.columns.getColumn(cellIndex + i).width
        : 0;
    }
    if (
      this.row.treegrid.columns.getColumn(cellIndex) &&
      this.row.treegrid.columns.getColumn(cellIndex).isTreeColumn
    ) {
    ...
    if (this.image && this.value) {
      ...
      if (this.finishedDrawingCell) {
        ...
        graphics.drawString(
          ...
          textPen,
          customisedBrush,
          innerLayoutArea.x + leftAdjustment,
          this.isHeaderCell ? innerLayoutArea.y - 16 : innerLayoutArea.y,
          innerLayoutArea.width - leftAdjustment - padding,
          innerLayoutArea.height - padding,
          this.style.format
        );
      } else {
        /* eslint-disable-next-line */
      ...
      if (this.finishedDrawingCell) {
        ...
        graphics.drawString(
          ...
          customisedBrush,
          innerLayoutArea.x + leftAdjustment,
          this.isHeaderCell ? innerLayoutArea.y - 16 : innerLayoutArea.y - 3,
          innerLayoutArea.width - leftAdjustment - padding,
          innerLayoutArea.height - padding,
          this.style.format
        );
      ...
      this.value instanceof PdfImage ||
      this.value instanceof PdfBitmap
    ) {
      let imageBounds: RectangleF;
  ...
  public drawCellBackground(graphics: PdfGraphics, bounds: RectangleF): void {
    const backgroundBrush: PdfBrush = new PdfSolidBrush(
  ...
  private adjustContentLayoutArea(bounds: RectangleF): RectangleF {
    ...
    const returnBounds: RectangleF = new RectangleF(
    ...
    if (this.value instanceof PdfTreeGrid) {
      ...
      if (this.style.format.alignment === PdfTextAlignment.Center) {
        returnBounds.x +=
          cellPadding.left + (returnBounds.width - size.width) / 2;
        returnBounds.y +=
          cellPadding.top + (returnBounds.height - size.height) / 2;
  ...
  private drawCellBorder(graphics: PdfGraphics, bounds: RectangleF): void {
    if (
      this.row.treegrid.style.borderOverlapStyle ===
    ...
    if (this.style.borders.isAll && this.isHeaderCell) {
      ...
      pen.lineCap =
        this.style.borders.left.dashStyle === PdfDashStyle.Solid
          ? PdfLineCap.Square
          : pen.lineCap;
      ...
      if (bounds.x + bounds.width > graphics.clientSize.width - pen.width / 2) {
        ...
        p2 = new PointF(
          graphics.clientSize.width - pen.width / 2,
          bounds.y + bounds.height
        );
      }
      pen.lineCap =
        this.style.borders.right.dashStyle === PdfDashStyle.Solid
          ? PdfLineCap.Square
          : pen.lineCap;
      graphics.drawLine(pen, p1, p2);
      graphics.restore();
      ...
      pen.lineCap =
        this.style.borders.top.dashStyle === PdfDashStyle.Solid
          ? PdfLineCap.Square
          : pen.lineCap;
      graphics.drawLine(pen, p1, p2);
      graphics.restore();
      ...
      pen.lineCap =
        this.style.borders.bottom.dashStyle === PdfDashStyle.Solid
          ? PdfLineCap.Square
          : pen.lineCap;
      graphics.drawLine(pen, p1, p2);
      graphics.restore();
...
export class PdfTreeGridColumnCollection {
  ...
  public measureColumnsWidth(): number {
    ...
    for (
      let i: number = 0, count: number = this.internalColumns.length;
      i < count;
      i++
    ) {
