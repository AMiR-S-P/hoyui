...
export class PdfGantt extends PdfTreeGrid {
  ...
  private calculateRange(): void {
    const lastColumnRange: number[] =
      this.layouter.columnRanges[this.layouter.columnRanges.length - 1];
    ...
    for (
      let index: number = lastColumnRange[0];
      index <= lastColumnRange[1];
      index++
    ) {
    ...
    if (totalColumnWidth + 100 < this.result.page.getClientSize().width) {
      ...
      this.chartPageIndex = this.startPageIndex =
        this.totalPages - this.perColumnPages;
    ...
    while (Math.round(point) < Math.round(headerWidth)) {
      ...
      const range: number[] = [];
      const convertedWidth: number = pixelToPoint(
      ...
      if (
        this.parent.cloneProjectStartDate.getHours() === 0 &&
      ...
      const count: number = isNullOrUndefined(
        timelineSettings.customTimelineSettings.bottomTier.count
      )
        ? timelineSettings.customTimelineSettings.topTier.count
        : timelineSettings.customTimelineSettings.bottomTier.count;
      const scheduleType: TimelineViewMode =
        timelineSettings.customTimelineSettings.bottomTier.unit === 'None'
          ? timelineSettings.customTimelineSettings.topTier.unit
          : timelineSettings.customTimelineSettings.bottomTier.unit;
      switch (scheduleType) {
        case 'Minutes': {
          detail.startDate = new Date(timelineStartDate.getTime());
          const sDays: number = Math.floor(
            pointToPixel(detail.startPoint) /
              this.chartHeader.bottomTierCellWidth
          );
          detail.startDate.setMinutes(
            detail.startDate.getMinutes() + sDays * count
          );
          detail.startDate.setSeconds(detail.startDate.getSeconds() + 1);
          detail.endDate = new Date(detail.startDate.getTime());
          const eDays: number = Math.floor(
            pointToPixel(detail.endPoint - detail.startPoint) /
              this.chartHeader.bottomTierCellWidth
          );
          detail.endDate.setMinutes(
            detail.endDate.getMinutes() + eDays * count
          );
          break;
        }
        case 'Hour': {
          detail.startDate = new Date(timelineStartDate.getTime());
          const sDays1: number = Math.floor(
            pointToPixel(detail.startPoint) /
              this.chartHeader.bottomTierCellWidth
          );
          detail.startDate.setHours(
            detail.startDate.getHours() + sDays1 * count
          );
          detail.startDate.setMinutes(detail.startDate.getMinutes() + 1);
          detail.endDate = new Date(detail.startDate.getTime());
          const eDays1: number = Math.floor(
            pointToPixel(detail.endPoint - detail.startPoint) /
              this.chartHeader.bottomTierCellWidth
          );
        ...
        case 'Day': {
          detail.startDate = new Date(timelineStartDate.getTime());
          const startDays: number = Math.round(
            detail.startPoint /
              pixelToPoint(this.chartHeader.bottomTierCellWidth)
          );
          detail.startDate.setDate(
            detail.startDate.getDate() + startDays * count
          );
        ...
        case 'Week': {
          detail.startDate = new Date(timelineStartDate.getTime());
          const startDays1: number =
            (detail.startPoint /
              pixelToPoint(this.chartHeader.bottomTierCellWidth)) *
            7;
          detail.startDate.setDate(
            detail.startDate.getDate() + startDays1 * count
          );
          const endDays1: number =
            Math.round(
              (detail.endPoint - detail.startPoint) /
                pixelToPoint(this.chartHeader.bottomTierCellWidth)
            ) *
        ...
        case 'Month': {
          detail.startDate = new Date(timelineStartDate.getTime());
          const startDays2: number =
            (detail.startPoint /
              pixelToPoint(this.chartHeader.bottomTierCellWidth)) *
            31;
          detail.startDate.setDate(
            detail.startDate.getDate() + startDays2 * count
          );
          const endDays2: number =
            Math.round(
              (detail.endPoint - detail.startPoint) /
                pixelToPoint(this.chartHeader.bottomTierCellWidth)
            ) *
        ...
        case 'Year': {
          detail.startDate = new Date(timelineStartDate.getTime());
          const startDays3: number =
            (detail.startPoint /
              pixelToPoint(this.chartHeader.bottomTierCellWidth)) *
            365;
          detail.startDate.setDate(
            detail.startDate.getDate() + startDays3 * count
          );
          const endDays3: number =
            Math.round(
              (detail.endPoint - detail.startPoint) /
                pixelToPoint(this.chartHeader.bottomTierCellWidth)
            ) *
  ...
  private drawGantttChart(): void {
    ...
    this.headerDetails.forEach(
      (detail: TimelineDetails, index: number): void => {
        const page: PdfPage = this.result.page.section.getPages()[
          this.startPageIndex
        ] as PdfPage;
        page['contentWidth'] =
          this.parent.pdfExportModule.gantt.taskbar.isAutoFit()
            ? pointToPixel(
                this.headerDetails[index as number].endPoint -
                  this.headerDetails[index as number].startPoint
              )
            : this.headerDetails[index as number].endPoint -
              this.headerDetails[index as number].startPoint;
        ...
        taskbarPoint.y =
          taskbarPoint.y +
          pixelToPoint(this.parent.timelineModule.isSingleTier ? 45 : 60); // headerHeight
        pageStartX = taskbarPoint.x;
        cumulativeHeight = pixelToPoint(
          this.parent.timelineModule.isSingleTier ? 45 : 60
        ); // headerHeight
        this.headerDetails[this.headerDetails.indexOf(detail)].startIndex =
          this.startPageIndex;
        this.headerDetails[this.headerDetails.indexOf(detail)].pageStartPoint =
          taskbarPoint;
        this.parent.eventMarkerColloction.map(
          (eventMarker: IEventMarkerInfo) => {
            const timelimeHeight: number = pixelToPoint(
              this.parent.timelineModule.isSingleTier ? 45 : 60
            );
            const pdfPage: PdfPage = this.result.page.section.getPages()[
              this.startPageIndex
            ] as PdfPage;
            this.eventMarker.drawEventMarker(
              pdfPage,
              taskbarPoint,
              cumulativeWidth,
              detail,
        ...
        for (let i: number = 0; i < this.taskbarCollection.length; i++) {
          const task: PdfGanttTaskbarCollection =
            this.taskbarCollection[i as number];
          ...
          const isNextPage: boolean = task.drawTaskbar(
            ...
            cumulativeWidth,
            rowHeight,
            this.taskbarCollection[parseInt(i.toString(), 10)],
            lineWidth
          );
          if (isNextPage) {
            if (this.enableHeader) {
              taskbarPoint.y = pixelToPoint(
                this.parent.timelineModule.isSingleTier ? 45 : 60
              );
            } else {
              taskbarPoint.y = 0;
        ...
        if (
          this.headerDetails.indexOf(detail) !==
          this.headerDetails.length - 1
        ) {
          this.result.page.section.add();
    ...
    for (let i: number = 0; i < this.predecessorCollection.length; i++) {
      const predecessor: PdfGanttPredecessor =
        this.predecessorCollection[i as number];
