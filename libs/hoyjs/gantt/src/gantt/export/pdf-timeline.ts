...
export class PdfTimeline {
  ...
  public drawTimeline(
    ...
    let remainWidth: number =
      this.parent.pdfExportModule.gantt.taskbar.isAutoFit()
        ? pointToPixel(Math.floor(detail.totalWidth))
        : Math.round(detail.totalWidth);
      ...
      if (this.topTier.length > this.topTierIndex) {
        ...
        this.triggerQueryTimelinecell(
          ...
          this.parent.timelineModule.isSingleTier &&
            this.parent.timelineSettings.topTier.unit === 'Day'
            ? pHeader.startDate
            : null
        );
    ...
    const height: number = this.parent.timelineModule.isSingleTier
      ? 0
      : this.topTierHeight;
    this.bottomTierPoint = new PointF(
      startPoint.x,
      pixelToPoint(startPoint.y + height)
    );
    while (remainWidth > 0) {
      const secondHeader: TimelineFormat =
        this.bottomTier[this.bottomTierIndex];
      if (this.bottomTier.length > this.bottomTierIndex) {
        ...
        this.triggerQueryTimelinecell(
          page,
          this.bottomTierPoint.x,
          this.bottomTierPoint.y,
          this.bottomTierHeight,
          this.parent.pdfExportModule.gantt.taskbar.isAutoFit()
            ? width
            : pixelToPoint(width),
          secondHeader.value,
          false,
          secondHeader.startDate
        );
        this.bottomTierPoint.x =
          this.parent.pdfExportModule.gantt.taskbar.isAutoFit()
            ? this.bottomTierPoint.x + width
            : this.bottomTierPoint.x + pixelToPoint(width);
        remainWidth -= width;
        secondHeader.completedWidth = width;
        // if (isCompleted) {
  ...
  public drawPageTimeline(
    ...
    for (
      let index: number = this.prevTopTierIndex;
      index <= this.topTierIndex;
      index++
    ) {
      if (this.topTier.length > index) {
        ...
        if (pHeader.completedWidth > 0) {
          ...
          this.triggerQueryTimelinecell(
            ...
            this.topTierHeight,
            this.parent.pdfExportModule.gantt.taskbar.isAutoFit()
              ? pHeader.completedWidth
              : pixelToPoint(pHeader.completedWidth),
            pHeader.value,
            true,
            this.parent.timelineModule.isSingleTier &&
              this.parent.timelineSettings.topTier.unit === 'Day'
    ...
    for (
      let index: number = this.prevBottomTierIndex;
      index <= this.bottomTierIndex;
      index++
    ) {
      if (this.bottomTier.length > index) {
        ...
        if (secondHeader.completedWidth > 0) {
          ...
          this.bottomTierPoint.x =
            this.parent.pdfExportModule.gantt.taskbar.isAutoFit()
              ? this.bottomTierPoint.x + secondHeader.width
              : this.bottomTierPoint.x + pixelToPoint(secondHeader.width);
        }
      }
    }
  }
  ...
  private triggerQueryTimelinecell(
    ...
    timelineStyle.backgroundColor = new PdfColor(
      ganttStyle.timeline.backgroundColor
    );
    ...
    let rectPen: PdfPen;
    let rectBrush: PdfBrush = new PdfSolidBrush(
      eventArgs.timelineCell.backgroundColor
    );
    const nonWorkingDays: number[] = this.parent.nonWorkingDayIndex;
    if (
      this.parent.highlightWeekends &&
      nonWorkingDays.indexOf(days) !== -1 &&
    ...
    this.parent.holidays.map((item: HolidayModel) => {
      ...
      if (
        ...
        if (toDate.getTime() === timelinedate.getTime()) {
          ...
          const day: number = this.parent.dataOperation.getTaskWidth(
            fromDate,
            changeDate
          );
        ...
        if (
          this.detailsTimeline.startDate <= fromDate &&
          this.detailsTimeline.endDate >= fromDate
        ) {
          this.parent.timelineModule.bottomTierCollection.map(
            (items: TimelineFormat) => {
    ...
    if (
      !isTopTier &&
      (this.parent.gridLines === 'Both' || this.parent.gridLines === 'Vertical')
    ) {
    ...
    if (this.holidayCompleted) {
      ...
      const font1: PdfStandardFont = new PdfStandardFont(
      ...
      graphics.translateTransform(
        this.holidayWidth +
          width -
          (fontSize / 2) * this.holidayNumberOfDays -
          fontHieght +
          fontHieght / 2 +
          (width * this.holidayNumberOfDays) / 2,
        40
      );
    ...
    if (this.fitHolidayCompleted) {
      const holidayBrush: PdfSolidBrush = new PdfSolidBrush(
        new PdfColor(238, 238, 238)
      );
      const font1: PdfStandardFont = new PdfStandardFont(
        PdfFontFamily.Helvetica,
        10
    ...
    const textBrush: PdfBrush = new PdfSolidBrush(
      eventArgs.timelineCell.fontColor
    );
    const pLeft: PdfPaddings | number = ganttStyle.timeline.padding
      ? eventArgs.timelineCell.padding.left
      : 0;
    const pTop: PdfPaddings | number = ganttStyle.timeline.padding
      ? eventArgs.timelineCell.padding.top
      : 0;
    /* eslint-disable-next-line */
