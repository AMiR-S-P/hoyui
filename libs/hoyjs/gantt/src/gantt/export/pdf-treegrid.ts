...
export class PdfTreeGrid extends PdfLayoutElement {
  ...
  public draw(
    arg1: PdfPage,
    arg2: RectangleF | PointF | number,
    arg3?: PdfLayoutFormat | number | boolean,
    arg4?: PdfLayoutFormat
  ): PdfLayoutResult {
    if (
      arg2 instanceof PointF &&
      typeof (arg2 as RectangleF).width === 'undefined' &&
      ...
      arg2 instanceof RectangleF &&
      typeof arg2.width !== 'undefined' &&
      typeof arg3 === 'undefined'
    ) {
      ...
      arg2 instanceof PointF &&
      typeof (arg2 as RectangleF).width === 'undefined' &&
      arg3 instanceof PdfLayoutFormat
    ) {
      return this.drawHelper(arg1, arg2.x, arg2.y, arg3);
    } else if (
      typeof arg2 === 'number' &&
      typeof arg3 === 'number' &&
      (arg4 instanceof PdfLayoutFormat || arg4 === null)
    ) {
  ...
  public measureColumnsWidth(bounds?: RectangleF): void {
    if (typeof bounds !== 'undefined') {
      const widths: number[] = this.columns.getDefaultWidths(
      ...
      for (let i: number = 0; i < colCount; i++) {
        ...
        for (let j: number = 0; j < rowCount; j++) {
          ...
          const rowWidth: number =
            this.initialWidth > 0
              ? Math.min(this.initialWidth, tempWidth)
              : tempWidth;
          cellWidth = Math.max(cellWidth, rowWidth);
  ...
  public drawGrid(
    ...
    x: number,
    y: number,
    format: PdfTreeGridLayoutFormat
  ): PdfTreeGridLayoutResult {
    this.initialWidth = page.graphics.clientSize.width;
    const layout: RectangleF = new RectangleF(
      0,
      0,
  ...
  protected layout(param: PdfLayoutParams): PdfLayoutResult {
    if (this.rows.count !== 0) {
      const style: PdfGanttCellStyle = this.rows
        .getRow(0)
        .cells.getCell(0).style;
      if (style.borders.left.width !== 1) {
        const x: number = style.borders.left.width / 2;
        const y: number = style.borders.top.width / 2;
        if (
          param.bounds.x === PdfBorders.default.right.width / 2 &&
          param.bounds.y === PdfBorders.default.right.width / 2
        ) {
          const newBound: RectangleF = new RectangleF(
            new PointF(x, y),
            new SizeF(this.size.width, this.size.height)
          );
