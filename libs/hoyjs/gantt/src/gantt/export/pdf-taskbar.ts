...
export class PdfGanttTaskbarCollection {
  ...
  public drawTaskbar(
    ...
    if (yPoint > pageSize.height) {
      ...
      page['contentWidth'] = this.isAutoFit()
        ? pointToPixel(detail.endPoint - detail.startPoint)
        : detail.endPoint - detail.startPoint;
      taskGraphics = page.graphics;
      startPoint.y = 0;
      if (this.parent.pdfExportModule.gantt.enableHeader) {
        this.parent.pdfExportModule.gantt.chartHeader.drawPageTimeline(
          page,
          startPoint,
          detail
        );
        startPoint.y = pixelToPoint(
          this.parent.timelineModule.isSingleTier ? 45 : 60
        );
      }
      isNextPage = true;
      ...
      if (
        page['contentWidth'] &&
        (this.parent.gridLines === 'Both' ||
          this.parent.gridLines === 'Horizontal')
      ) {
        graphics.drawRectangle(
          pen,
          startPoint.x,
          startPoint.y,
          this.isAutoFit() && this.parent.timelineModule.bottomTier !== 'Day'
            ? page['contentWidth'] + 0.5
            : lineWidth,
          rowHeight
        );
      }
    ...
    //Draw Taskbar
    let font: PdfFont = new PdfStandardFont(
      this.fontFamily,
      9,
    ...
    customizedFont =
      !isNullOrUndefined(taskbar.taskbarTemplate.value) &&
      taskbar.taskbarTemplate.fontStyle &&
      taskbar.taskbarTemplate.fontStyle.fontFamily &&
      taskbar.taskbarTemplate.fontStyle.fontSize
        ? new PdfStandardFont(
            taskbar.taskbarTemplate.fontStyle.fontFamily,
            taskbar.taskbarTemplate.fontStyle.fontSize,
            taskbar.taskbarTemplate.fontStyle.fontStyle
          )
        : font;
    customizedFontBrush =
      !isNullOrUndefined(taskbar.taskbarTemplate.value) &&
    ...
    customizedFontColor =
      !isNullOrUndefined(taskbar.taskbarTemplate.value) &&
      taskbar.taskbarTemplate.fontStyle &&
      taskbar.taskbarTemplate.fontStyle.fontBrush
        ? new PdfPen(taskbar.taskbarTemplate.fontStyle.fontBrush)
        : fontColor;
    if (
      !isNullOrUndefined(
        this.parent.pdfExportModule['helper']['exportProps'].ganttStyle
      ) &&
    ...
    if (taskbar.labelSettings.taskLabel.fontStyle.fontSize) {
      const taskFont: PdfFont = new PdfStandardFont(
        taskbar.labelSettings.taskLabel.fontStyle.fontFamily,
        taskbar.labelSettings.taskLabel.fontStyle.fontSize,
        taskbar.labelSettings.taskLabel.fontStyle.fontStyle
      );
    ...
    if (taskbar.labelSettings.taskLabel.fontStyle.fontColor) {
      taskLabelFontBrush = new PdfSolidBrush(
    ...
    if (
      !isNullOrUndefined(this.taskLabel) &&
      /^[a-zA-Z0-9]/.test(this.taskLabel)
    ) {
    ...
    if (!taskbar.isMilestone) {
      ...
      const manuallineBrush: PdfBrush = new PdfSolidBrush(
        taskbar.manuallineColor
      );
      const splitline: PdfPen = new PdfPen(taskbar.splitLineBackground);
      const manualBrush: PdfBrush = new PdfSolidBrush(
        taskbar.manualParentBackground
      );
      const manualChildBrush: PdfBrush = new PdfSolidBrush(
        taskbar.manualChildBackground
      );
      const manualChildProgressBrush: PdfBrush = new PdfSolidBrush(
        taskbar.manualChildProgress
      );
      const manualProgressBrush: PdfBrush = new PdfSolidBrush(
        taskbar.manualParentProgress
      );
      ...
      const adjustHeightforTaskbar: number = pixelToPoint(
        (this.parent.rowHeight - this.height) / 2.0
      );
      const adjustHeightforBaseline: number = pixelToPoint(
        (this.parent.rowHeight - this.height) / 4.5
      );
      const adjustHeight = this.parent.renderBaseline
        ? adjustHeightforBaseline
        : adjustHeightforTaskbar;
      pageIndex = page.section.indexOf(page);
      const startDate: Date = isNullOrUndefined(this.unscheduleStarteDate)
        ? this.startDate
        : this.unscheduleStarteDate;
      const endDate: Date = isNullOrUndefined(this.unscheduleEndDate)
        ? this.endDate
        : this.unscheduleEndDate;
      const imageSize = 10;
        ...
        if (!this.isScheduledTask && this.unscheduledTaskBy === 'duration') {
          ...
          if (this.isAutoFit()) {
            ...
            taskGraphics.drawRectangle(
              brush1,
              startPoint.x + (this.left - cumulativeWidth) + 0.5,
            ...
            if (template.value || template.image) {
              ...
              if (!isNullOrUndefined(taskbar.taskbarTemplate.image)) {
                ...
                const image: PdfBitmap = new PdfBitmap(
                  taskbar.taskbarTemplate.image[0].base64
                );
                taskGraphics.drawImage(
                  image,
                  startPoint.x + (this.left - cumulativeWidth) + 0.5,
                  startPoint.y + adjustHeight,
                  pixelToPoint(taskbar.taskbarTemplate.image[0].width),
                  pixelToPoint(taskbar.taskbarTemplate.image[0].height)
                );
              } else {
                imageWidth = 0;
              }
            ...
            brush1 = new PdfLinearGradientBrush(
              new PointF(
                startPoint.x +
                  pixelToPoint(this.left - cumulativeWidth) +
                  0.5 +
                  pixelToPoint(taskbar.width) / 2,
                0
              ),
              new PointF(
                startPoint.x + pixelToPoint(this.left - cumulativeWidth),
                0
              ),
              new PdfColor(taskbar.unscheduledTaskBarColor),
              new PdfColor(255, 255, 255)
            ...
            taskGraphics.drawRectangle(
              brush1,
              startPoint.x + pixelToPoint(this.left - cumulativeWidth) + 0.5,
              startPoint.y + adjustHeight,
              pixelToPoint(taskbar.width),
              pixelToPoint(taskbar.height)
            );
          ...
          this.unscheduledTaskBy === 'endDate'
        ) {
          this.drawUnscheduledTask(
            taskGraphics,
            startPoint,
            cumulativeWidth,
            adjustHeight
          ...
          !this.isScheduledTask &&
          this.unscheduledTaskBy !== 'duration'
        ) {
          this.drawUnscheduledTask(
            taskGraphics,
            startPoint,
          ...
          if (taskbar.isSpliterTask) {
            ...
            if (this.isAutoFit()) {
              taskGraphics.drawLine(
                ...
                new PointF(
                  startPoint.x + (this.left - cumulativeWidth) + 0.5,
                  startPoint.y + adjustHeight + pixelToPoint(taskbar.height / 2)
                ),
                new PointF(
                  taskbar.width +
                    startPoint.x +
            ...
            taskbar.segment.map((data: any) => {
              if (this.isAutoFit()) {
                taskGraphics.drawRectangle(
                  ...
                  taskBrush,
                  startPoint.x +
                    (this.left + data.left - cumulativeWidth) +
                    0.5,
                  startPoint.y + adjustHeight,
                  data.width,
                  pixelToPoint(taskbar.height)
                ...
                taskGraphics.drawRectangle(
                  ...
                  taskBrush,
                  startPoint.x +
                    pixelToPoint(this.left + data.left - cumulativeWidth) +
                    0.5,
                  startPoint.y + adjustHeight,
                  pixelToPoint(data.width),
                  pixelToPoint(taskbar.height)
            ...
            if (this.isAutoFit()) {
              taskGraphics.translateTransform(
                startPoint.x + (this.left - cumulativeWidth) + 0.5 + this.width,
                startPoint.y + adjustHeight - 2
              );
            } else {
              taskGraphics.translateTransform(
            ...
            if (this.isAutoFit()) {
              ...
              taskGraphics.drawRectangle(
                ...
                startPoint.x + pixelToPoint(this.left - cumulativeWidth) + 0.5,
                startPoint.y + adjustHeight,
                pixelToPoint(taskbar.progressWidth),
                pixelToPoint(this.height)
              );
            }
          } else {
            if (this.isAutoFit()) {
              if (
                isNullOrUndefined(template.value) &&
                isNullOrUndefined(template.image)
              ) {
                taskGraphics.drawRectangle(
                  taskbarPen,
                  taskBrush,
                ...
                if (!isNullOrUndefined(taskbar.taskbarTemplate.image)) {
                  ...
                  taskGraphics.drawImage(
                    ...
                    startPoint.y +
                      adjustHeight +
                      (taskbar.height -
                        taskbar.taskbarTemplate.image[0].height) /
                        2.0,
                    taskbar.taskbarTemplate.image[0].width / 2.0,
                    taskbar.taskbarTemplate.image[0].height / 2.0
                  );
                } else {
                  imageWidth = 0;
                }
                if (taskbar.taskbarTemplate.value) {
                  ...
                  taskGraphics.drawString(
                    ...
                    customizedFontColor,
                    customizedFontBrush,
                    startPoint.x + (this.left - cumulativeWidth) + imageWidth,
                    startPoint.y + adjustHeight,
                    updatedWidth,
                    pixelToPoint(this.height),
                    progressFormat
          ...
          if (
            ...
            if (
              ...
              if (
                !isNullOrUndefined(this.parent.labelSettings.taskLabel) &&
                !isNullOrUndefined(this.taskLabel)
              ) {
                if (this.taskLabel !== '0' && this.taskLabel !== '0%') {
                ...
                if (this.isAutoFit()) {
                  taskGraphics.drawString(
                    this.taskLabel.toString(),
                    taskLabelFont,
                    fontColor,
                    taskLabelFontBrush,
                    startPoint.x + (this.left - cumulativeWidth),
                    startPoint.y + adjustHeight,
                    updatedWidth,
                    pixelToPoint(this.height),
                    progressFormat
              ...
              if (this.isAutoFit()) {
                ...
                taskGraphics.drawRectangle(
                  ...
                  startPoint.x +
                    ...
                    0.5,
                  startPoint.y + adjustHeight,
                  pixelToPoint(data.progressWidth),
                  pixelToPoint(taskbar.height)
                );
              }
            });
      ...
      //Task start date is in the range of header split up start and end date
        ...
        this.width =
          this.width - (detail.totalWidth - (this.left - cumulativeWidth));
        ...
        if (!this.isScheduledTask && this.unscheduledTaskBy === 'duration') {
          ...
          if (this.isAutoFit()) {
            ...
            brush2 = new PdfLinearGradientBrush(
              ...
              new PointF(
                startPoint.x + (this.left - cumulativeWidth) + renderWidth / 2,
                0
              ),
              new PdfColor(255, 255, 255),
              new PdfColor(taskbar.unscheduledTaskBarColor)
          ...
          if (taskbar.isSpliterTask) {
            ...
            if (this.isAutoFit()) {
              taskGraphics.drawLine(
                ...
                new PointF(
                  lineWidth + (this.left - cumulativeWidth) + startPoint.x,
                  startPoint.y + adjustHeight + pixelToPoint(taskbar.height / 2)
                )
              );
            } else {
            ...
            taskbar.segment.map((data: any, index: number) => {
                  ...
                  if (!isNullOrUndefined(taskbar.taskbarTemplate.image)) {
                    ...
                    taskGraphics.drawImage(
                      ...
                      startPoint.x +
                        ...
                        data.left,
                      startPoint.y + adjustHeight,
                      taskbar.taskbarTemplate.image[0].width,
                      taskbar.taskbarTemplate.image[0].height
                    );
                  } else {
                    imageWidth = 0;
              ...
              if (renderWidth >= pervwidth) {
                ...
                if (renderWidth >= pervwidth - data.width) {
                  ...
                  this.segmentCollection[parseInt(index.toString(), 10)].left =
                    splitRenderwidth - data.left;
                }
                if (renderWidth >= pervwidth) {
                  this.segmentCollection[
                    parseInt(index.toString(), 10)
            ...
            if (this.isAutoFit()) {
              if (
                ...
                if (!isNullOrUndefined(taskbar.taskbarTemplate.value)) {
                  const imageWidth: number = !isNullOrUndefined(
                    taskbar.taskbarTemplate.image
                  )
                    ? taskbar.taskbarTemplate.image[0].width
                    : 0;
                  taskGraphics.drawString(
              ...
              if (
                ...
                if (!isNullOrUndefined(taskbar.taskbarTemplate.value)) {
                  ...
                  const result: PdfStringLayoutResult = this.getWidth(
                    taskbar.taskbarTemplate.value,
                    detail.endPoint - this.stringLeft,
                    15
                  );
                  taskGraphics.drawString(
                    result.lines[0].text,
            ...
            if (taskbar.isAutoSchedule && !taskbar.isSpliterTask) {
                ...
                if (this.isAutoFit()) {
                  ...
                  taskGraphics.drawRectangle(
                    ...
                    startPoint.x +
                      ...
                      0.5,
                    startPoint.y + adjustHeight,
                    pixelToPoint(progressBoundsWidth),
                    pixelToPoint(taskbar.height)
                  );
                }
              }
              ...
              if (
                this.parent.labelSettings.taskLabel &&
                !isNullOrUndefined(this.taskLabel)
              ) {
          ...
          if (taskbar.isAutoSchedule && !taskbar.isSpliterTask) {
            ...
            taskbar.segment.map((data: any, index: number) => {
              ...
              if (
                this.segmentCollection[parseInt(index.toString(), 10)].width ===
                0
              ) {
                this.segmentCollection[
                  parseInt(index.toString(), 10)
                ...
                if (
                  data.width -
                    this.segmentCollection[parseInt(index.toString(), 10)]
                      .width <
                    data.progressWidth &&
                  data.width !== this.segmentCollection[index].width
                ) {
                  this.segmentCollection[
                    parseInt(index.toString(), 10)
                  ].progressWidth =
      ...
      //Task end date is in the range of header split up start and end date
        ...
        if (!this.isScheduledTask && this.unscheduledTaskBy === 'duration') {
          ...
          taskGraphics.drawRectangle(
            brush1,
            startPoint.x + pixelToPoint(taskbar.left + 0.5),
            startPoint.y + adjustHeight,
            pixelToPoint(taskbar.width),
            pixelToPoint(taskbar.height)
          );
          ...
          taskGraphics.drawLine(
            splitline,
            new PointF(
              startPoint.x + pixelToPoint(this.left) + 0.5,
              startPoint.y + adjustHeight + pixelToPoint(taskbar.height / 2)
            ),
            new PointF(
              pixelToPoint(taskbar.width) +
                startPoint.x +
                pixelToPoint(this.left),
              startPoint.y + adjustHeight + pixelToPoint(taskbar.height / 2)
            )
          );
          taskbar.segmentCollection.map((data: any) => {
            if (data.width !== 0) {
              taskGraphics.drawRectangle(
                taskbarPen,
                taskBrush,
                startPoint.x + pixelToPoint(taskbar.left + 0.5 + data.left),
                startPoint.y + adjustHeight,
                pixelToPoint(data.width),
                pixelToPoint(taskbar.height)
              );
              if (template.value || template.image) {
                ...
                !isNullOrUndefined(taskbar.taskbarTemplate.value)
                  ? taskGraphics.drawString(
                      ...
                      customizedFontBrush,
                      startPoint.x +
                        pixelToPoint(taskbar.left + 0.5 + data.left) +
                        imageWidth,
                      startPoint.y + adjustHeight,
                      pixelToPoint(data.width),
                      pixelToPoint(taskbar.height),
          ...
          if (!isNullOrUndefined(this.remainString)) {
            ...
            taskGraphics.drawString(
              ...
              customizedFontColor,
              customizedFontBrush,
              startPoint.x + pixelToPoint(taskbar.left),
              startPoint.y + adjustHeight,
              ...
              if (isNullOrUndefined(taskbar.taskbarTemplate.value)) {
                if (!isNullOrUndefined(this.remainString)) {
                  ...
                  taskGraphics.drawString(
                    ...
                    fontColor,
                    fontBrush,
                    startPoint.x + pixelToPoint(this.left),
                    startPoint.y + adjustHeight,
                    pixelToPoint(updatedWidth),
                    pixelToPoint(this.height),
                    progressFormat
      ...
      //So the task is takes entire width of page.
        ...
        if (!this.isScheduledTask && this.unscheduledTaskBy === 'duration') {
          ...
          brush1 = new PdfLinearGradientBrush(
            new PointF(
              startPoint.x +
                pixelToPoint(taskbar.left) +
                0.5 +
                pixelToPoint(detail.totalWidth) / 2,
              0
            ),
          ...
          taskGraphics.drawRectangle(
            ...
            startPoint.x + pixelToPoint(taskbar.left) + 0.5,
            startPoint.y + adjustHeight,
            pixelToPoint(detail.totalWidth),
            pixelToPoint(taskbar.height)
          );
          brush2 = new PdfLinearGradientBrush(
            new PointF(
      ...
      if (
        ...
        taskbar.baselineStartDate &&
        taskbar.baselineEndDate
      ) {
        if (
          detail.startDate <= taskbar.baselineStartDate &&
          taskbar.baselineEndDate <= detail.endDate
        ) {
          if (!this.isStartPoint) {
            this.taskStartPoint = { ...startPoint };
            this.isStartPoint = true;
          ...
          if (
            ...
            if (this.isAutoFit()) {
              taskGraphics.drawRectangle(
                baselinePen,
                baselineBrush,
                startPoint.x + (taskbar.baselineLeft - cumulativeWidth) + 0.5,
                startPoint.y + adjustHeight + pixelToPoint(taskbar.height + 3),
                taskbar.baselineWidth,
                pixelToPoint(this.baselineHeight)
              );
            } else {
              taskGraphics.drawRectangle(
                baselinePen,
                baselineBrush,
                startPoint.x +
                  pixelToPoint(taskbar.baselineLeft - cumulativeWidth) +
                  0.5,
                startPoint.y + adjustHeight + pixelToPoint(taskbar.height + 3),
                pixelToPoint(taskbar.baselineWidth),
                pixelToPoint(this.baselineHeight)
              );
            }
          }
          ...
          detail.startDate <= taskbar.baselineStartDate &&
          detail.endDate >= taskbar.baselineStartDate &&
          taskbar.baselineEndDate >= detail.endDate
        ) {
          if (!this.isStartPoint) {
            this.taskStartPoint = { ...startPoint };
          ...
          let renderWidth: number = 0;
          renderWidth =
            detail.totalWidth - (taskbar.baselineLeft - cumulativeWidth);
          if (
            this.parent.renderBaseline &&
            taskbar.baselineStartDate &&
            taskbar.baselineEndDate
          ...
          taskbar.baselineEndDate <= detail.endDate &&
          detail.startDate <= taskbar.baselineEndDate &&
          !this.isCompletedBaseline
        ) {
          if (
            this.parent.renderBaseline &&
            taskbar.baselineStartDate &&
            ...
            if (this.isAutoFit()) {
              taskGraphics.drawRectangle(
                baselinePen,
                baselineBrush,
                startPoint.x + (taskbar.baselineLeft + 0.5),
                startPoint.y + adjustHeight + pixelToPoint(taskbar.height + 3),
                taskbar.baselineWidth,
                pixelToPoint(this.baselineHeight)
              );
              ...
              taskGraphics.drawRectangle(
                baselinePen,
                baselineBrush,
                startPoint.x + pixelToPoint(taskbar.baselineLeft + 0.5),
                startPoint.y + adjustHeight + pixelToPoint(taskbar.height + 3),
                pixelToPoint(taskbar.baselineWidth),
                pixelToPoint(this.baselineHeight)
              );
          ...
          taskbar.baselineStartDate < detail.startDate &&
          taskbar.baselineEndDate > detail.endDate
        ) {
          if (
            this.parent.renderBaseline &&
            ...
            if (this.isAutoFit()) {
              taskGraphics.drawRectangle(
                ...
                startPoint.y + adjustHeight + pixelToPoint(taskbar.height + 3),
                detail.totalWidth,
                pixelToPoint(taskbar.baselineHeight)
              );
            } else {
              taskGraphics.drawRectangle(
                baselinePen,
      ...
      if (!this.isAutoSchedule && taskbar.isParentTask) {
        if (
          detail.startDate <= taskbar.autoStartDate &&
          taskbar.autoEndDate <= detail.endDate
        ) {
          if (!this.isStartPoint) {
            this.taskStartPoint = { ...startPoint };
            this.isStartPoint = true;
          ...
          if (!taskbar.isAutoSchedule && taskbar.isParentTask) {
            if (this.isAutoFit()) {
              taskGraphics.drawRectangle(
                manualParentBorderPen,
                null,
                startPoint.x + (this.autoLeft - cumulativeWidth) + 0.5,
                startPoint.y + adjustHeight + 10,
                this.autoWidth,
                pixelToPoint(12)
              );
            } else {
              taskGraphics.drawRectangle(
                ...
                null,
                startPoint.x +
                  pixelToPoint(this.autoLeft - cumulativeWidth) +
                  0.5,
                startPoint.y + adjustHeight + 10,
                pixelToPoint(this.autoWidth),
                pixelToPoint(12)
              );
            }
            taskGraphics.save();
          ...
          detail.startDate <= taskbar.autoStartDate &&
          detail.endDate >= taskbar.autoStartDate &&
          taskbar.autoEndDate >= detail.endDate
        ) {
          if (!this.isStartPoint) {
            this.taskStartPoint = { ...startPoint };
          ...
          taskbar.autoEndDate <= detail.endDate &&
          detail.startDate <= taskbar.autoEndDate &&
          !this.isCompletedAutotask
        ) {
          if (!this.isStartPoint) {
            this.taskStartPoint = { ...startPoint };
            this.isStartPoint = true;
            ...
            taskGraphics.drawRectangle(
              manualParentBorderPen,
              null,
              startPoint.x + pixelToPoint(this.autoLeft + 0.5),
              startPoint.y + adjustHeight + 10,
              pixelToPoint(taskbar.autoWidth),
              pixelToPoint(12)
            );
            taskGraphics.save();
            taskGraphics.setTransparency(0.87);
          ...
          taskbar.autoStartDate < detail.startDate &&
          taskbar.autoEndDate > detail.endDate
        ) {
          if (!this.isStartPoint) {
            this.taskStartPoint = { ...startPoint };
          ...
          if (!taskbar.isAutoSchedule && taskbar.isParentTask) {
            if (this.isAutoFit()) {
              taskGraphics.drawRectangle(
                null,
                manualProgressBrush,
                startPoint.x + (this.autoLeft + 0.5),
                startPoint.y + adjustHeight + 10,
                taskbar.progressWidth,
                pixelToPoint(12)
              );
      ...
      if (
        !isNullOrUndefined(taskbar.indicators) &&
        taskbar.indicators.length > 0
      ) {
        taskbar.indicators.map((items: IIndicator, index: number) => {
          const currendate = this.parent.dateValidationModule.getDateFromFormat(
            items.date,
            true
          );
          if (detail.startDate <= currendate && currendate <= detail.endDate) {
            const leftValue: number =
              this.parent.chartRowsModule.getIndicatorleft(items.date);
            if (!isNullOrUndefined(items.base64)) {
              const image: PdfBitmap = new PdfBitmap(items.base64);
              if (this.isAutoFit()) {
                taskGraphics.drawImage(
                  image,
                  startPoint.x + (leftValue - cumulativeWidth) + 0.5 + 10,
                  startPoint.y + adjustHeight,
                  imageSize,
                  imageSize
                );
                ...
                taskGraphics.drawImage(
                  image,
                  startPoint.x +
                    pixelToPoint(leftValue - cumulativeWidth) +
                    0.5 +
                    10,
                  startPoint.y + adjustHeight,
                  imageSize,
  ...
  private drawRightLabel(
    ...
    if (!isNullOrUndefined(this.rightTaskLabel.value)) {
      ...
      if (
        detail.startPoint <= left &&
        left < detail.endPoint &&
        !isNullOrUndefined(this.rightTaskLabel.value) &&
        !this.rightTaskLabel.isCompleted
      ) {
        const result: PdfStringLayoutResult = this.getWidth(
          this.rightTaskLabel.value,
          detail.endPoint - left,
          15
        );
        ...
        const adjustHeight: number =
          (pixelToPoint(this.parent.rowHeight) - result.actualSize.height) / 2;
        const point: PointF = new PointF(
          actualLeft,
          startPoint.y + adjustHeight
        );
      ...
      if (!isNullOrUndefined(this.labelSettings.rightLabel.image)) {
        ...
        for (
          let i: number = 0;
          i < this.labelSettings.rightLabel.image.length;
          i++
        ) {
          this.drawRigthlabelImage(
            page,
            startPoint,
            detail,
            cumulativeWidth,
          ...
          if (this.labelSettings.rightLabel.value) {
            const value: string[] =
              this.labelSettings.rightLabel.value.split(',');
            if (value) {
              this.rightTaskLabel.isCompleted = false;
              this.drawRightLabelValue(
                page,
                startPoint,
                detail,
                cumulativeWidth,
        ...
        this.drawRightLabelValue(
          ...
          detail,
          cumulativeWidth,
          this.labelSettings.rightLabel.value
        );
      }
    }
  }
  private drawRigthlabelImage(
    ...
    if (
      !isNullOrUndefined(this.labelSettings.rightLabel) &&
      !isNullOrUndefined(this.labelSettings.rightLabel.image)
    ) {
      if (this.labelSettings.isLeftCalculated) {
        ...
        if (this.isAutoFit()) {
          ...
          if (
            this.labelSettings.rightLabel.image.length > 1 &&
            value.length > 1
          ) {
            left =
              pixelToPoint(this.rightTaskLabel.left) +
              this.previousWidthofRightValue;
            this.previousWidthofRightImage = rightImage.width;
          } else if (this.labelSettings.rightLabel.image.length > 1) {
            left =
              pixelToPoint(this.rightTaskLabel.left) +
              this.previousWidthofRightImage;
            this.previousWidthofRightImage = rightImage.width;
          } else {
            left = pixelToPoint(this.rightTaskLabel.left);
          }
      ...
      if (
        ...
        const adjustHeight: number =
          (pixelToPoint(this.parent.rowHeight) - result.height) / 2;
        const point: PointF = new PointF(
          actualLeft,
          startPoint.y + adjustHeight
        );
    ...
    if (!isNullOrUndefined(this.labelSettings.rightLabel.value)) {
      if (this.labelSettings.isLeftCalculated) {
        ...
        if (this.isAutoFit()) {
          if (this.labelSettings.rightLabel.image) {
            if (this.labelSettings.rightLabel.image.length > 1) {
              left =
                this.rightTaskLabel.left +
                this.previousWidthofRightImage +
                this.previousWidthofRightValue;
              if (!isNullOrUndefined(rightString)) {
                const result: PdfStringLayoutResult = this.getWidthofrightLabel(
                  rightString,
                  detail.endPoint - left,
                  15
                );
                this.previousWidthofRightValue +=
                  result.actualSize.width + this.previousWidthofRightImage;
              }
            } else {
              left =
                this.rightTaskLabel.left +
                this.labelSettings.rightLabel.image[0].width;
            }
          } else {
            left = this.rightTaskLabel.left;
          }
      ...
      if (
        ...
        if (result.actualSize.width > 0) {
          const fontColor: PdfPen = null;
          const fontBrush: PdfBrush = new PdfSolidBrush(this.labelColor);
          const newFont: PdfFont = this.labelSettings.rightLabel.fontStyle
            .fontSize
            ? new PdfStandardFont(
                this.labelSettings.rightLabel.fontStyle.fontFamily,
                this.labelSettings.rightLabel.fontStyle.fontSize,
                this.labelSettings.rightLabel.fontStyle.fontStyle
              )
            : font;
          const newFontBrush: PdfBrush = this.labelSettings.rightLabel.fontStyle
            .fontColor
            ? new PdfSolidBrush(
          ...
          if (
            !isNullOrUndefined(result.remainder) &&
  ...
  private drawLeftLabel(
    ...
    if (!isNullOrUndefined(this.leftTaskLabel.value)) {
      ...
      if (!this.leftTaskLabel.isLeftCalculated) {
        const result: PdfStringLayoutResult = this.getWidth(
          this.leftTaskLabel.value,
          Number.MAX_VALUE,
          15
        );
        const reduceLeft: number = this.isMilestone
      ...
      if (this.isAutoFit()) {
        ...
        if (!taskbar.isAutoSchedule && taskbar.isParentTask) {
          const leftValue: number =
            pixelToPoint(taskbar.left) - pixelToPoint(taskbar.autoLeft);
          if (taskbar.left < taskbar.autoLeft) {
            actualLeft = left - pixelToPoint(cumulativeWidth) + startPoint.x;
          } else {
            actualLeft =
              left - pixelToPoint(cumulativeWidth) + startPoint.x - leftValue;
          }
        }
      }
      if (
        detail.startPoint <= left &&
        left < detail.endPoint &&
        !isNullOrUndefined(this.leftTaskLabel.value) &&
        ...
        const adjustHeight: number =
          (pixelToPoint(this.parent.rowHeight) - result.actualSize.height) / 2;
        const rightLabelpoint: PointF = new PointF(
          actualLeft,
          startPoint.y + adjustHeight
        );
        const rightLabelSize: SizeF = new SizeF(
          result.actualSize.width,
          result.actualSize.height
        );
        const rightLabelBounds: RectangleF = new RectangleF(
          rightLabelpoint,
          rightLabelSize
        );
      ...
      if (
        !isNullOrUndefined(this.labelSettings.leftLabel) &&
        !isNullOrUndefined(this.labelSettings.leftLabel.image)
      ) {
        ...
        for (
          let i: number = 0;
          i < this.labelSettings.leftLabel.image.length;
          i++
        ) {
          let value: string[];
          ...
          const totalstringWidth: number = !isNullOrUndefined(result)
            ? result.actualSize.width
            : 0;
          this.totalLeftWidth +=
            this.labelSettings.leftLabel.image[i as number].width +
            totalstringWidth;
        }
        for (
          let i: number = 0;
          ...
          this.drawLeftLabelImage(
            page,
            startPoint,
            detail,
            cumulativeWidth,
            this.labelSettings.leftLabel.image[i as number]
          );
          if (this.labelSettings.leftLabel.value) {
            const value: string[] =
              this.labelSettings.leftLabel.value.split(',');
            if (value) {
              this.drawLeftLabelValue(
                page,
                startPoint,
                detail,
                cumulativeWidth,
        ...
        !isNullOrUndefined(this.labelSettings.leftLabel) &&
        !isNullOrUndefined(this.labelSettings.leftLabel.value)
      ) {
        this.drawLeftLabelValue(
          page,
          startPoint,
          detail,
          cumulativeWidth,
          this.labelSettings.leftLabel.value
        );
      }
    }
  }
  private drawLeftLabelImage(
    ...
    if (
      ...
      if (!this.leftTaskLabel.isLeftCalculated) {
        const result: SizeF = new SizeF(
          leftLabelImage.width,
          leftLabelImage.height
        );
        const reduceLeft: number = this.isMilestone
          ? Math.floor(this.parent.chartRowsModule.taskBarHeight / 2) + 33
          : 33; // 33 indicates default timeline cell width
        ...
        if (this.isAutoFit()) {
          if (
            this.labelSettings.leftLabel.image.length > 0 &&
            value.length > 0
          ) {
      ...
      if (
        detail.startPoint <= left &&
        left < detail.endPoint &&
        !isNullOrUndefined(leftLabelImage) &&
        !this.leftTaskLabel.isCompleted
      ) {
        const result: SizeF = new SizeF(
          leftLabelImage.width,
  ...
  private drawLeftLabelValue(
    ...
    if (!isNullOrUndefined(leftLabelValue)) {
      ...
      labelLeft = this.left;
      if (!this.leftTaskLabel.isLeftCalculated) {
        const result: PdfStringLayoutResult = this.getWidthofLeftLabel(
          leftLabelValue,
          Number.MAX_VALUE,
          15
        );
        ...
        const value: string[] = this.labelSettings.leftLabel.value.split(',');
        if (this.isAutoFit()) {
          const isLeftImageExist: boolean = !isNullOrUndefined(
            this.labelSettings.leftLabel.image
          )
            ? true
            : false;
          const imageLength: number = isLeftImageExist
            ? this.labelSettings.leftLabel.image.length
            : 0;
          if (value.length === 1 && isLeftImageExist && imageLength === 1) {
            ...
            this.previousWidthofLeftValue +=
              this.previousWidthofLeftImage + result.actualSize.width;
      ...
      if (
        detail.startPoint <= left &&
        left < detail.endPoint &&
        !isNullOrUndefined(leftLabelValue) &&
        !this.leftTaskLabel.isCompleted
      ) {
        const result: PdfStringLayoutResult = this.getWidthofLeftLabel(
          leftLabelValue,
        ...
        if (result.actualSize.width > 0) {
          const fontColor: PdfPen = null;
          const fontBrush: PdfBrush = new PdfSolidBrush(this.labelColor);
          const newFont: PdfFont = this.labelSettings.leftLabel.fontStyle
            .fontSize
            ? new PdfStandardFont(
                this.labelSettings.leftLabel.fontStyle.fontFamily,
                this.labelSettings.leftLabel.fontStyle.fontSize,
                this.labelSettings.leftLabel.fontStyle.fontStyle
              )
            : font;
          const newFontBrush: PdfBrush = this.labelSettings.leftLabel.fontStyle
            .fontColor
            ? new PdfSolidBrush(
                this.labelSettings.leftLabel.fontStyle.fontColor
              )
            : fontBrush;
          graphics.drawString(
            leftLabelValue,
          ...
          if (
            ...
            !isNullOrUndefined(this.labelSettings.leftLabel.image) &&
            this.labelSettings.leftLabel.image.length === 1 &&
            value.length === 1
          ) {
            this.leftTaskLabel.isCompleted = true;
          }
  ...
  private getWidth(
    ...
    progressFormat.lineAlignment = PdfVerticalAlignment.Middle;
    /* eslint-disable-next-line */
    const result: PdfStringLayoutResult = layouter.layout(
      value,
      font,
      progressFormat,
  ...
  private drawMilestone(
    ...
    if (
      ...
      const adjustHeightforBaselineMilesone: number = pixelToPoint(
        (this.parent.rowHeight - this.height) / 3.0
      );
      const adjustHeightforMilesone: number = pixelToPoint(
        (this.parent.rowHeight - this.height) / 2.0
      );
      const adjustHeight: number = this.parent.renderBaseline
        ? adjustHeightforBaselineMilesone
        : adjustHeightforMilesone;
      ...
      const height: number = Math.floor(
        this.parent.chartRowsModule.taskBarHeight * 0.8
      );
      if (this.isAutoFit()) {
        taskGraphics.translateTransform(
          startPoint.x + (this.left - cumulativeWidth),
