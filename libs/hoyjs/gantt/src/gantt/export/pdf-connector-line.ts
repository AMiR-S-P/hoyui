...
export class PdfGanttPredecessor {
  ...
  public findindex(num: number): number {
    ...
    if (!this.parent.enableVirtualization) {
      this.parent.currentViewData.map((data: IGanttData, index: number) => {
        if (data.index === num) {
          dataindex = index;
        }
      });
      ...
      this.parent.flatData.map((data: IGanttData, index: number) => {
        if (data.index === num) {
          dataindex = index;
        }
      });
  ...
  public drawPredecessor(pdfGantt: PdfGantt): void {
    this.pdfGantt = pdfGantt;
    const pages: PdfPage[] =
      pdfGantt.result.page.section.getPages() as PdfPage[];
    const parentTask: PdfGanttTaskbarCollection =
      pdfGantt.taskbarCollection[this.findindex(this.parentIndex)];
    const childTask: PdfGanttTaskbarCollection =
      pdfGantt.taskbarCollection[this.findindex(this.childIndex)];
    ...
    switch (this.type) {
      case 'FS':
        if (childTask.startPage > -1 && parentTask.endPage > -1) {
          ...
          parentPageData =
            pdfGantt.pdfPageDetail[
              parentTask.endPage - pdfGantt.chartPageIndex
            ];
          childPageData =
            pdfGantt.pdfPageDetail[
              childTask.startPage - pdfGantt.chartPageIndex
            ];
          if (this.parentIndex < this.childIndex) {
            if (
              this.parentLeft < this.childLeft &&
              this.childLeft > this.parentLeft + this.parentWidth + 25
            ) {
      ...
      case 'FF':
        if (childTask.endPage > -1 && parentTask.endPage > -1) {
          ...
          childPageData =
            pdfGantt.pdfPageDetail[childTask.endPage - pdfGantt.chartPageIndex];
          if (this.parentIndex < this.childIndex) {
            if (
              this.childLeft + this.childWidth >=
              this.parentLeft + this.parentWidth
            ) {
      ...
      case 'SS':
        if (childTask.startPage > -1 && parentTask.startPage > -1) {
          ...
          parentPageData =
            pdfGantt.pdfPageDetail[
              parentTask.startPage - pdfGantt.chartPageIndex
            ];
          childPageData =
            pdfGantt.pdfPageDetail[
              childTask.startPage - pdfGantt.chartPageIndex
    ...
    const midPointManualparent: number = Math.round(
      (this.parent.rowHeight - 15) / 2.0
    );
    const midPointforTaskbar: number = Math.round(
      (this.parent.rowHeight - 1) / 2.0
    );
    const midPointforBaseline: number = Math.round(
      (this.parent.rowHeight - 10) / 2.0
    );
    let midPoint: number = this.parent.renderBaseline
      ? midPointforBaseline
      : (!parentTask.isAutoSchedule && parentTask.isParentTask) ||
        (!childTask.isAutoSchedule && childTask.isParentTask)
      ? midPointManualparent
      : midPointforTaskbar;
    ...
    if (this.parent.pdfExportModule.gantt.taskbar.isAutoFit()) {
      ...
      ffpoint4 = new PointF(
        this.childLeft - 6 - this.lineWidth - 1,
        childY + midPoint
      );
      sspoint4 = new PointF(
        this.childLeft + this.childWidth + 6 + this.lineWidth + 1,
        childY + midPoint
      );
    } else {
      ffpoint1 = new PointF(
        pixelToPoint(this.parentLeft + this.parentWidth),
        parentY + midPoint
      );
      sspoint1 = new PointF(
        pixelToPoint(this.parentLeft) - 1,
        parentY + midPoint
      );
      ffpoint3 = new PointF(
        pixelToPoint(this.childLeft - 20),
        childY + midPoint
      );
      ffpoint4 = new PointF(
        pixelToPoint(this.childLeft - 6 - this.lineWidth) - 1,
        childY + midPoint
      );
      sspoint4 = new PointF(
        pixelToPoint(this.childLeft + this.childWidth + 6 + this.lineWidth) + 1,
        childY + midPoint
      );
    }
    switch (predecessorType) {
      ...
      case 'FFType3':
        if (this.parent.pdfExportModule.gantt.taskbar.isAutoFit()) {
          point1 = new PointF(
            this.parentLeft + this.parentWidth + 1,
            parentY + midPoint
          );
          point2 = new PointF(
            this.childLeft + this.childWidth + 20,
            parentY + midPoint
          );
        } else {
          point1 = new PointF(
            pixelToPoint(this.parentLeft + this.parentWidth) + 1,
            parentY + midPoint
          );
          point2 = new PointF(
            pixelToPoint(this.childLeft + this.childWidth + 20),
            parentY + midPoint
          );
        }
      ...
      case 'FFType4':
        if (this.parent.pdfExportModule.gantt.taskbar.isAutoFit()) {
          ...
          point2 = new PointF(
            this.parentLeft + this.parentWidth + 20,
            parentY + midPoint
          );
        } else {
          point1 = new PointF(
          ...
          point2 = new PointF(
            pixelToPoint(this.parentLeft + this.parentWidth + 20),
            parentY + midPoint
          );
        }
        point3 = new PointF(point2.x, childY + midPoint);
      ...
      case 'SSType4':
        ...
        if (this.parent.pdfExportModule.gantt.taskbar.isAutoFit()) {
          ...
          point2 = new PointF(
            pixelToPoint(this.parentLeft - 20),
            parentY + midPoint
          );
        }
        point3 = new PointF(point2.x, childY + midPoint);
  ...
  private connectLines(
    ...
    point3: PointF,
    point4: PointF,
    childTask: PdfGanttTaskbarCollection,
    midPoint: number,
    point5?: PointF,
    point6?: PointF
  ): void {
  ...
  private findPageIndex(point: PointF): number {
    ...
    for (
      let index: number = 0;
      index < this.pdfGantt.pdfPageDetail.length;
      index++
    ) {
      const pageData: PageDetail = this.pdfGantt.pdfPageDetail[index as number];
      const pageRect: RectangleF = new RectangleF(
        pageData.startPoint.x,
        pageData.startPoint.y,
        pageData.width,
        pageData.height
      );
  ...
  private drawLine(page: PdfPage, startPoint: PointF, endPoint: PointF): void {
    const pdfPages: PdfPage[] =
      this.pdfGantt.result.page.section.getPages() as PdfPage[];
    ...
    let pageData: PageDetail =
      this.pdfGantt.pdfPageDetail[
        page.section.indexOf(page) - this.pdfGantt.chartPageIndex
      ];
    const pageRect: RectangleF = new RectangleF(
      pageData.startPoint.x,
      pageData.startPoint.y,
    ...
    const startPointCheck: boolean = this.contains(
      pageRect,
      startPoint.x,
      startPoint.y
    );
    const endPointCheck: boolean = this.contains(
      pageRect,
      endPoint.x,
      endPoint.y
    );
    ...
    if (
      ...
      if (pageIndex > -1) {
        ...
        this.drawLine(
          pdfPages[pageIndex + this.pdfGantt.chartPageIndex],
          newStartPoint,
          newEndPoint
        );
      }
      ...
      if (pageRectLeft > endPoint.x) {
        ...
        if (this.pdfGantt.enableHeader) {
          newStartPoint.y += this.parent.timelineModule.isSingleTier
            ? pixelToPoint(45)
            : pixelToPoint(62);
        }
        ...
        if (this.pdfGantt.enableHeader) {
          checkStartPoint.y += this.parent.timelineModule.isSingleTier
            ? pixelToPoint(45)
            : pixelToPoint(62);
          endPoint.y += this.parent.timelineModule.isSingleTier
            ? pixelToPoint(45)
            : pixelToPoint(62);
        }
      }
    ...
    if (startPointCheck) {
      ...
      const predecessorPen: PdfPen = new PdfPen(
        brush,
        pixelToPoint(this.lineWidth)
      );
  ...
  private drawArrow(
    ...
    /* eslint-disable-next-line */
    const pageRect: RectangleF = new RectangleF(
      new PointF(pageData.startPoint.x, pageData.startPoint.y),
      new SizeF(pageData.width, pageData.height)
    );
    ...
    if (this.parent.pdfExportModule.gantt.taskbar.isAutoFit()) {
      if (this.type === 'FS' || this.type === 'SS') {
        ...
        startPoint = new PointF(
          this.childLeft + this.childWidth + 1,
          childTask.taskStartPoint.y + pageData.startPoint.y
        );
      }
    } else {
      if (this.type === 'FS' || this.type === 'SS') {
        startPoint = new PointF(
          pixelToPoint(this.childLeft) - 1,
          childTask.taskStartPoint.y + pageData.startPoint.y
        );
      } else {
        startPoint = new PointF(
          pixelToPoint(this.childLeft + this.childWidth) + 1,
          childTask.taskStartPoint.y + pageData.startPoint.y
        );
      }
    }
    ...
    if (this.parent.pdfExportModule.gantt.taskbar.isAutoFit()) {
      ...
      if (this.type === 'FS' || this.type === 'SS') {
        point2 = new PointF(
          point1.x - pixelToPoint(width),
          point1.y - pixelToPoint(width)
        );
      } else {
        point2 = new PointF(
          point1.x + pixelToPoint(width),
