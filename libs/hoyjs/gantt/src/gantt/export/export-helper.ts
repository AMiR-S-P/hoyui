...
import { PdfFontFamily, PdfTextWebLink } from '@hoyui/pdf-export';
import {
  PdfStringFormat,
  PdfPageCountField,
  PdfPageNumberField,
...
import { getForeignData, PdfFooter, ValueFormatter } from '@hoyui/grids';
...
export class ExportHelper {
  ...
  public processToFit(): void {
    this.beforeSinglePageExport['zoomingProjectStartDate'] =
      this.parent.zoomingProjectStartDate;
    this.beforeSinglePageExport['zoomingProjectEndDate'] =
      this.parent.zoomingProjectEndDate;
    this.beforeSinglePageExport['cloneProjectStartDate'] =
      this.parent.cloneProjectStartDate;
    this.beforeSinglePageExport['cloneProjectEndDate'] =
      this.parent.cloneProjectEndDate;
    this.beforeSinglePageExport['customTimelineSettings'] = extend(
      {},
      this.parent.timelineModule.customTimelineSettings,
      null,
      true
    );
    this.beforeSinglePageExport['isTimelineRoundOff'] =
      this.parent.isTimelineRoundOff;
    this.beforeSinglePageExport['topTier'] = this.parent.timelineModule.topTier;
    this.beforeSinglePageExport['topTierCellWidth'] =
      this.parent.timelineModule.topTierCellWidth;
    this.beforeSinglePageExport['topTierCollection'] =
      this.parent.timelineModule.topTierCollection;
    this.beforeSinglePageExport['bottomTier'] =
      this.parent.timelineModule.bottomTier;
    this.beforeSinglePageExport['bottomTierCellWidth'] =
      this.parent.timelineModule.bottomTierCellWidth;
    this.beforeSinglePageExport['bottomTierCollection'] =
      this.parent.timelineModule.bottomTierCollection;
    this.beforeSinglePageExport['totalTimelineWidth'] =
      this.parent.timelineModule.totalTimelineWidth;
    this.beforeSinglePageExport['timelineStartDate'] =
      this.parent.timelineModule.timelineStartDate;
    this.beforeSinglePageExport['timelineEndDate'] =
      this.parent.timelineModule.timelineEndDate;
    this.beforeSinglePageExport['timelineRoundOffEndDate'] =
      this.parent.timelineModule.timelineRoundOffEndDate;
    this.beforeSinglePageExport['perDayWidth'] = this.parent.perDayWidth;
    this.beforeSinglePageExport['updatedConnectorLineCollection'] = extend(
      [],
    ...
    if (
      ...
      this.parent.cloneProjectStartDate = new Date(
    ...
    const timeDifference: number =
      this.parent.cloneProjectEndDate.getTime() -
      this.parent.cloneProjectStartDate.getTime();
    ...
    if (this.exportProps.fitToWidthSettings.gridWidth) {
      gridWidth = parseInt(
        this.exportProps.fitToWidthSettings.gridWidth.split('%')[0],
        10
      );
    }
    if (this.exportProps.fitToWidthSettings.chartWidth) {
      chartsideWidth = parseInt(
        this.exportProps.fitToWidthSettings.chartWidth.split('%')[0],
        10
      );
    } else {
      if (this.exportProps.fitToWidthSettings.gridWidth) {
    ...
    const pdfwidth: number =
      (this.parent.pdfExportModule['pdfPageDimensions'].width *
        chartsideWidth) /
      100;
    ...
    const zoomingCollections: ZoomTimelineSettings[] = [
      ...this.parent.zoomingLevels,
    ];
    const sortedCollectons: ZoomTimelineSettings[] = zoomingCollections.sort(
      (a: ZoomTimelineSettings, b: ZoomTimelineSettings) =>
        !a.perDayWidth && !b.perDayWidth
          ? 0
          : a.perDayWidth < b.perDayWidth
          ? 1
          : -1
    ...
    for (let i: number = 0; i < sortedCollectons.length; i++) {
      ...
      if (
        perDayWidth < firstValue.perDayWidth &&
        perDayWidth > secondValue.perDayWidth
      ) {
        zoomingLevel = sortedCollectons[i + 1];
        break;
      }
    }
    const newTimeline: ZoomTimelineSettings = extend(
      {},
      {},
      zoomingLevel,
      true
    );
    this.parent.timelineModule['roundOffDateToZoom'](
      this.parent.cloneProjectStartDate,
      true,
      perDayWidth,
      newTimeline.bottomTier.unit,
      zoomingLevel
    );
    this.parent.timelineModule['roundOffDateToZoom'](
      this.parent.cloneProjectEndDate,
    ...
    const numberOfCells: number =
      this.parent.timelineModule['calculateNumberOfTimelineCells'](newTimeline);
    const scrollHeight: number =
      this.parent.pdfExportModule['pdfPageDimensions'].height; //17 is horizontal scrollbar width
    const emptySpace: number = scrollHeight <= 0 ? 0 : 17;
    newTimeline.timelineUnitSize =
      Math.abs(chartWidth - emptySpace) / numberOfCells;
   ...
   * @private
   */
  public processGridExport(
    data: IGanttData[],
    gantt: PdfGantt,
    props: PdfExportProperties
  ): void {
    this.flatData = data;
    this.gantt = gantt;
    ...
    if (
      this.exportProps.fitToWidthSettings &&
      this.exportProps.fitToWidthSettings.isFitToWidth
    ) {
      this.processToFit();
    }
  ...
  private processColumnHeader(column: ColumnModel, index: number): void {
    ...
    const treeGridHeaderHeight: number = this.parent.timelineModule.isSingleTier
      ? 45
      : 60;
    ...
    if (args.headerTemplate.image && args.headerTemplate.value) {
      ...
      args.image.height =
        (<{ height?: number }>args.headerTemplate.image[0]).height ||
        args.image.height;
      args.image.width =
        (<{ width?: number }>args.headerTemplate.image[0]).width ||
        args.image.width;
  ...
  private isColumnVisible(column: ColumnModel): boolean {
    const visibleColumn: boolean =
      column.visible || this.exportProps.includeHiddenColumn;
  ...
  private processTimeline(): void {
    ...
    this.gantt.chartHeader.topTierHeight =
      this.gantt.chartHeader.bottomTierHeight = this.parent.timelineModule
        .isSingleTier
        ? 45
        : 60 / 2;
    this.gantt.chartHeader.topTierCellWidth = timelineSettings.topTierCellWidth;
    this.gantt.chartHeader.bottomTierCellWidth =
      timelineSettings.bottomTierCellWidth;
    this.gantt.chartHeader.topTier = extend(
      [],
      [],
      this.parent.enableTimelineVirtualization
        ? timelineSettings.pdfExportTopTierCollection
        : timelineSettings.topTierCollection,
      true
    ) as [];
    this.gantt.chartHeader.bottomTier = extend(
      [],
      [],
      this.parent.enableTimelineVirtualization
        ? timelineSettings.pdfExportBottomTierCollection
        : timelineSettings.bottomTierCollection,
      true
    ) as [];
    if (
      this.exportProps &&
      ...
      const tier: string =
        timelineSettings.topTier === 'None' ? 'bottomTier' : 'topTier';
      this.gantt.chartHeader.width = timelineSettings[
        'calculateWidthBetweenTwoDate'
      ](
        tier,
        timelineSettings.timelineStartDate,
        timelineSettings.timelineEndDate
      );
    } else {
      this.gantt.chartHeader.width = this.parent.enableTimelineVirtualization
    ...
    this.gantt.timelineStartDate = new Date(
      timelineSettings.timelineStartDate.getTime()
    );
  }
  /**
  ...
  private processPredecessor(): void {
    if (
      isNullOrUndefined(this.exportProps.showPredecessorLines) ||
      this.exportProps.showPredecessorLines
    ) {
      this.parent.pdfExportModule.isPdfExport = true;
      this.parent.predecessorModule.createConnectorLinesCollection();
      this.parent.updatedConnectorLineCollection.forEach(
        (data: IConnectorLineObject) => {
          ...
          predecessor.lineWidth =
            this.parent.connectorLineWidth > 5
              ? pixelToPoint(5)
              : pixelToPoint(this.parent.connectorLineWidth);
          if (data.isCritical) {
            predecessor.connectorLineColor =
              this.ganttStyle.criticalConnectorLineColor;
          } else {
  ...
  private processRecordCell(
    ...
    if (
      ...
      cell.value = this.parent.getDurationString(
        ganttProps.duration,
      ...
      cell.value = this.parent.getWorkString(
        ganttProps.work,
        ganttProps.workUnit
      );
    } else {
      cell.value = !isNullOrUndefined(data[column.field])
        ? data[column.field].toString()
        : '';
    }
    const cellValueString: string = !isNullOrUndefined(cell.value)
      ? cell.value.toString()
      : '';
    ...
    if (
      !isNullOrUndefined(args.image) &&
      !isNullOrUndefined(args.image.base64)
    ) {
      const dimension: Object = extend({}, args.image, null, true);
      args.image = new PdfBitmap(args.image.base64) as any;
      args.image.height = dimension['height']
        ? dimension['height']
        : (<{ height?: number }>args.image).height;
      args.image.width = dimension['width']
        ? dimension['width']
        : (<{ width?: number }>args.image).width;
      cell.image = args.image as any;
    }
    cell.value = args.value;
    if (
      !isNullOrUndefined(args.hyperLink) &&
      (!isNullOrUndefined(args.hyperLink.displayText) ||
    ...
    flatData.forEach((data: IGanttData) => {
      ...
      if (isScheduledTask) {
        if (
          ...
          taskbar.unscheduleStarteDate =
            this.parent.dateValidationModule.getValidStartDate(
              data.ganttProperties
            );
          taskbar.unscheduleEndDate =
            this.parent.dateValidationModule.getValidEndDate(
              data.ganttProperties
            );
        }
      }
      ...
      taskbar.isSpliterTask =
        isNullOrUndefined(ganttProp.segments) || ganttProp.segments.length === 0
          ? false
          : true;
      if (taskbar.isSpliterTask) {
        taskbar.segmentCollection = taskbar.segment.map((obj: ITaskData) => ({
          ...obj,
        }));
      }
      ...
      taskbar.milestoneColor = new PdfColor(
        this.ganttStyle.taskbar.milestoneColor
      );
      ...
      if (data[this.parent.labelSettings.leftLabel]) {
        taskbar.leftTaskLabel.value =
          data[this.parent.labelSettings.leftLabel].toString();
      }
      if (data[this.parent.labelSettings.rightLabel]) {
        taskbar.rightTaskLabel.value =
          data[this.parent.labelSettings.rightLabel].toString();
      }
      if (!isNullOrUndefined(data[this.parent.labelSettings.taskLabel])) {
        taskbar.taskLabel =
          data[this.parent.labelSettings.taskLabel].toString();
      }
      const reduceLeft: number = ganttProp.isMilestone
        ? Math.floor(this.parent.chartRowsModule.taskBarHeight / 2) + 33
        : 33; // 33 indicates default timeline cell width
      taskbar.rightTaskLabel.left =
        ganttProp.left + ganttProp.width + reduceLeft; // right label left value
      ...
      taskbar.progressFontColor = new PdfColor(
        this.ganttStyle.taskbar.progressFontColor
      );
      if (taskbar.isParentTask) {
        taskbar.taskColor = new PdfColor(
          this.ganttStyle.taskbar.parentTaskColor
        );
        taskbar.taskBorderColor = new PdfColor(
          this.ganttStyle.taskbar.parentTaskBorderColor
        );
        taskbar.progressColor = new PdfColor(
          this.ganttStyle.taskbar.parentProgressColor
        );
      } else {
        if (data.isCritical) {
          taskbar.taskColor = new PdfColor(
            this.ganttStyle.taskbar.criticalTaskColor
          );
          taskbar.progressColor = new PdfColor(
            this.ganttStyle.taskbar.criticalProgressColor
          );
          taskbar.taskBorderColor = new PdfColor(
            this.ganttStyle.taskbar.criticalTaskBorderColor
          );
          taskbar.milestoneColor = new PdfColor(
            this.ganttStyle.taskbar.criticalTaskColor
          );
          ...
          taskbar.progressColor = new PdfColor(
            this.ganttStyle.taskbar.progressColor
          );
          taskbar.taskBorderColor = new PdfColor(
            this.ganttStyle.taskbar.taskBorderColor
          );
        }
      }
      taskbar.manualParentBorder = new PdfColor(
        this.ganttStyle.taskbar.manualParentBorder
      );
      taskbar.manualChildBorder = new PdfColor(
        this.ganttStyle.taskbar.manualChildBorder
      );
      taskbar.manuallineColor = new PdfColor(
        this.ganttStyle.taskbar.manualLineColor
      );
      taskbar.unscheduledTaskBarColor = new PdfColor(
        this.ganttStyle.taskbar.unscheduledTaskBarColor
      );
      taskbar.manualParentBackground = new PdfColor(
        this.ganttStyle.taskbar.manualParentBackground
      );
      taskbar.manualParentProgress = new PdfColor(
        this.ganttStyle.taskbar.manualParentProgress
      );
      taskbar.manualChildBackground = new PdfColor(
        this.ganttStyle.taskbar.manualChildBackground
      );
      taskbar.manualChildProgress = new PdfColor(
        this.ganttStyle.taskbar.manualChildProgress
      );
      taskbar.splitLineBackground = new PdfColor(
        this.ganttStyle.taskbar.splitLineBackground
      );
      taskbar.baselineColor = new PdfColor(
        this.ganttStyle.taskbar.baselineColor
      );
      taskbar.baselineBorderColor = new PdfColor(
        this.ganttStyle.taskbar.baselineBorderColor
      );
      ...
      if (this.parent.pdfQueryTaskbarInfo) {
        ...
        if (
          !isNullOrUndefined(args.labelSettings.leftLabel.image) &&
          Array.isArray(args.labelSettings.leftLabel.image[0].base64) &&
          args.labelSettings.leftLabel.image[0].base64.length > 0
        ) {
          const baseCount: number =
            args.labelSettings.leftLabel.image[0].base64.length;
        ...
        if (
          !isNullOrUndefined(args.labelSettings.rightLabel.image) &&
          Array.isArray(args.labelSettings.rightLabel.image[0].base64) &&
          args.labelSettings.rightLabel.image[0].base64.length > 0
        ) {
          const baseCount: number =
            args.labelSettings.rightLabel.image[0].base64.length;
        ...
        const applyTemplate: any = (
          target: PdfGanttTaskbarCollection,
          source: PdfQueryTaskbarInfoEventArgs
        ) => {
          ...
          if (
            !isNullOrUndefined(source.taskbarTemplate.image) &&
            !isNullOrUndefined(source.taskbarTemplate.image[0].base64)
          ) {
            const width: number = source.taskbarTemplate.image[0].width;
            const milestoneHeight: number = taskbar.isMilestone
              ? source.taskbarTemplate.image[0].height <
                this.parent.chartRowsModule.taskBarHeight * 0.7
                ? source.taskbarTemplate.image[0].height
                : this.parent.chartRowsModule.taskBarHeight * 0.7 - 2
              : this.parent.chartRowsModule.taskBarHeight * 0.7 - 2;
            const taskbarHeight: number = !isNullOrUndefined(
              source.taskbarTemplate.image[0].height
            )
              ? source.taskbarTemplate.image[0].height < taskbar.height
                ...
                : taskbar.height - 2
              : taskbar.height - 2;
            const height: number = taskbar.isMilestone
              ? milestoneHeight
              : taskbarHeight;
          ...
          if (!isNullOrUndefined(source.taskbarTemplate.value)) {
            ...
            target.taskbarTemplate.value = source.taskbarTemplate.value;
            target.taskbarTemplate.fontStyle.fontColor =
              source.taskbarTemplate.fontStyle.fontColor;
            target.taskbarTemplate.fontStyle.fontSize =
              source.taskbarTemplate.fontStyle.fontSize;
            target.taskbarTemplate.fontStyle.fontFamily =
              source.taskbarTemplate.fontStyle.fontFamily;
            target.taskbarTemplate.fontStyle.fontStyle =
              source.taskbarTemplate.fontStyle.fontStyle;
            target.taskbarTemplate.fontStyle.fontBrush =
              source.taskbarTemplate.fontStyle.fontBrush;
          }
        };
        if (
          !args.data.hasChildRecords &&
          args.data.ganttProperties.duration !== 0
        ) {
  ...
  private renderEmptyGantt(): void {
    ...
    this.copyStyles(
      this.ganttStyle.columnHeader,
      row.cells.getCell(0),
      row.isParentRow
    );
    const count: number = this.columns.length;
    row.cells.getCell(0).value =
      this.parent.localeObj.getConstant('emptyRecord');
  ...
  private mergeCells(
    ...
    this.gantt.rows.getRow(rowIndex).cells.getCell(colIndex).columnSpan =
      lastColIndex;
  }
  /* eslint-disable-next-line */
  private copyStyles(
    ...
    /* eslint-disable-next-line */
    cell.style.format = (<any>Object).assign(
      new PdfStringFormat(),
      style.format
    );
  ...
  public initializePdf(pdfDoc: PdfDocument): void {
    ...
    if (
      ...
      const pdfwidth: number =
        (this.parent.pdfExportModule['pdfPageDimensions'].width * gridWidth) /
        100;
      const perColumnWidth: number =
        pdfwidth / this.gantt.columns.columns.length;
    ...
    if (
      this.totalColumnWidth > this.pdfDoc.pageSettings.width - 82 &&
      this.totalColumnWidth < PdfPage.getClientSize().width
    ) {
      this.gantt.style.allowHorizontalOverflow = true;
    } else if (
      tWidth / this.columns.length <
      widths[treeColumnIndex as number]
    ) {
      this.gantt.columns.getColumn(treeColumnIndex as number).width =
        widths[treeColumnIndex as number];
    }
    if (
      this.exportProps.enableFooter ||
      isNullOrUndefined(this.exportProps.enableFooter)
    ) {
      //code for draw the footer content
      const bounds: RectangleF = new RectangleF(
        0,
        0,
      ...
      const footerBrush: PdfBrush = new PdfSolidBrush(
        this.ganttStyle.footer.backgroundColor
      );
      ...
      /* eslint-disable-next-line */
      let font: PdfTrueTypeFont | PdfStandardFont = new PdfStandardFont(
        this.ganttStyle.fontFamily,
        this.ganttStyle.footer.fontSize,
        this.ganttStyle.footer.fontStyle
      );
      if (this.ganttStyle.font) {
        font = this.ganttStyle.font;
      }
      const brush: PdfBrush = new PdfSolidBrush(
        this.ganttStyle.footer.fontColor
      );
      ...
      const compositeField: PdfCompositeField = new PdfCompositeField(
        font,
        brush,
        'Page {0}',
        pageNumber,
    ...
    const clientSize: SizeF = !isNullOrUndefined(pageSize)
      ? pageSize
    ...
    if (!isNullOrUndefined(this.exportProps.header)) {
      ...
      const size: SizeF = new SizeF(
        clientSize.width * 1.1,
        headerProp && headerProp.height ? headerProp.height * 0.75 : 50
      );
      const bounds: RectangleF = new RectangleF(position, size);
      pdfDoc.template.top = this.drawPageTemplate(
    ...
    if (
      !this.exportProps.enableFooter &&
      !isNullOrUndefined(this.exportProps.footer)
    ) {
      ...
      const position: PointF = new PointF(
        ...
        clientSize.width -
          80 -
          (footer && footer.fromBottom ? footer.fromBottom * 0.75 : 0)
      );
      const size: SizeF = new SizeF(
        clientSize.width * 1.1,
        footer && footer.height ? footer.height * 0.75 : 50
      );
      const bounds: RectangleF = new RectangleF(position, size);
      this.pdfDoc.template.bottom = this.drawPageTemplate(
        new PdfPageTemplateElement(bounds),
        footer
      );
    }
  ...
  private drawImage(
    ...
    const y: number = content.position.y * 0.75;
    const width: number =
      !isNullOrUndefined(content.size) && !isNullOrUndefined(content.size.width)
        ? content.size.width * 0.5
        : undefined;
    const height: number =
      !isNullOrUndefined(content.size) &&
      !isNullOrUndefined(content.size.height)
  ...
  private setContentFormat(
    ...
    const width: number = content.size
      ? content.size.width * 0.75
      : this.pdfDoc.pageSettings.size.width;
    const height: number = content.size
      ? content.size.height * 0.75
      : !isNullOrUndefined(this.exportProps.footer)
      ? this.exportProps.footer.height * 0.5
      : 0;
    format = new PdfStringFormat(
...
export class ExportValueFormatter {
  ...
  private returnFormattedValue(
    args: any,
    customFormat: DateFormatOptions
  ): string {
  ...
  public formatCellValue(args: any): string {
    if (args.isForeignKey) {
      args.value = getValue(
        ...
        getForeignData(args.column, {}, args.value)[0]
      );
    }
    if (
      args.column.type === 'number' &&
      args.column.format !== undefined &&
      args.column.format !== ''
    ) {
      return args.value
        ? this.internationalization.getNumberFormat({
      ...
      (args.column.type === 'date' ||
        args.column.type === 'datetime' ||
        args.column.type === 'time') &&
      args.column.format !== undefined
    ) {
      ...
      if (typeof args.column.format === 'string') {
        ...
        if (
          args.column.format instanceof Object &&
          args.column.format.type === undefined
        ) {
      ...
      if (
        (!isNullOrUndefined(args.column.type) &&
          !isNullOrUndefined(args.value)) ||
        !isNullOrUndefined(args.value)
      ) {
        return args.value.toString();
      } else {
