...
import { TooltipEventArgs } from '@hoyui/popups';
...
export class GanttChart {
  ...
  public renderOverAllocationContainer(): void {
    ...
    const rangeContainer: HTMLElement = this.parent.element.querySelector(
      '.' + cls.rangeContainer
    );
    if (rangeContainer) {
  ...
  private renderChartElements(): void {
    if (this.parent.isFromOnPropertyChange) {
      ...
      this.parent.updateProjectDates(
        ...
        this.parent.isTimelineRoundOff
      );
      ...
      this.parent.connectorLineModule.renderConnectorLines(
        this.parent.updatedConnectorLineCollection
      );
      for (
        let i: number = 0;
        i < this.parent.chartRowsModule.ganttChartTableBody.children.length;
        i++
      ) {
        if (
          this.parent.chartRowsModule.ganttChartTableBody.children[i as number]
            .children[0].children[1].children[4]
        ) {
          (<HTMLElement>(
            this.parent.chartRowsModule.ganttChartTableBody.children[
              i as number
          ...
          if (this.parent.viewType === 'ProjectView') {
            const node: HTMLElement = this.parent.chartRowsModule
              .ganttChartTableBody.children[parseInt(i.toString(), 10)]
  ...
  private getTopValue(currentRecord: IGanttData): number {
    const updatedRecords: IGanttData[] = this.parent.getExpandedRecords(
      this.parent.currentViewData
    );
    let recordIndex: number = updatedRecords.indexOf(currentRecord);
    if (currentRecord.parentItem && recordIndex === -1) {
      const nestedParent: IGanttData = this.parent.getRecordByID(
        currentRecord.parentItem.taskId
      );
  ...
  private getRangeHeight(data: IGanttData): number {
    ...
    return (
      data.childRecords.length * this.parent.rowHeight -
      Math.floor(
        this.parent.rowHeight - this.parent.chartRowsModule.taskBarHeight
      )
    );
  ...
  private renderRange(
    ...
    if (!this.parent.allowTaskbarOverlap && this.parent.enableMultiTaskbar) {
      topValue = !currentRecord.expanded
        ? this.parent.getRowByIndex(rowIndex as number).offsetTop
        : this.parent.getRowByIndex(rowIndex as number).offsetTop +
          this.parent.rowHeight;
    } else {
      topValue = this.getTopValue(currentRecord);
    }
    const sameIDElement: Element = this.rangeViewContainer.querySelector(
      '.' + 'rangeContainer' + currentRecord.ganttProperties.rowUniqueID
    );
    if (sameIDElement) {
    ...
    if (
      currentRecord.level === 0 &&
      !currentRecord.expanded &&
      isNullOrUndefined(currentRecord.parentItem) &&
    ...
    if (
      currentRecord.level > 0 &&
      currentRecord.expanded &&
      !this.parent.getRecordByID(currentRecord.parentItem.taskId).expanded
    ) {
      return;
    }
    for (let i: number = 0; i < rangeCollection.length; i++) {
      let height: number;
      const node: NodeListOf<ChildNode> =
        this.parent.chartRowsModule.ganttChartTableBody.childNodes;
      if (
        ...
        height =
          parseInt((node[rowIndex as number] as HTMLElement).style.height, 10) -
          (this.parent.rowHeight - this.parent.chartRowsModule.taskBarHeight);
      } else {
        ...
        styles:
          ...
          `${
            this.parent.enableRtl
              ? rangeCollection[i as number].left +
                rangeCollection[i as number].width -
                5
              : rangeCollection[i as number].left
          }px;
                top: ${Math.floor(
                  (this.parent.rowHeight -
                    this.parent.chartRowsModule.taskBarHeight) /
                    2
                )}px;
                height: ${height + 1}px; border-right: 0px;
                z-index: ${
                  this.parent.viewType === 'ProjectView'
                    ? currentRecord.childRecords.length > 1
                      ? currentRecord.childRecords.length + 1
                      : currentRecord.childRecords.length
                    : 6
                }`,
      });
    ...
    this.parent.ganttChartModule.chartBodyContent.appendChild(
  ...
  private renderBodyContainers(): void {
    ...
    if (
      (this.parent.virtualScrollModule && this.parent.enableVirtualization) ||
    ...
    if (
      !isNullOrUndefined(this.parent.toolbarModule) &&
      !isNullOrUndefined(this.parent.toolbarModule.element)
    ) {
      toolbarHeight = this.parent.toolbarModule.element.offsetHeight;
    }
    this.scrollObject.setHeight(
      this.parent.ganttHeight -
        this.chartTimelineContainer.offsetHeight -
        toolbarHeight
    );
  }
  /**
  ...
  public updateWidthAndHeight(): void {
    ...
    const emptyHeight: number =
      this.parent.contentHeight === 0
        ? this.parent.flatData.length > 1
          ? emptydivHeight
          : 0
    ...
    if (
      emptyHeight >= contentElement['offsetHeight'] ||
      this.parent.height === 'auto' ||
      contentElement['offsetHeight'] - emptyHeight < emptydivHeight
    ) {
      this.chartBodyContent.style.height = formatUnit(emptyHeight);
    } else {
      const scrollHeight: number = this.parent.element.getElementsByClassName(
        'e-chart-rows-container'
      )[0]['offsetHeight'];
      if (contentElement['offsetHeight'] >= scrollHeight) {
        this.chartBodyContent.style.height =
          contentElement['offsetHeight'] - 17 + 'px';
      } else {
        this.chartBodyContent.style.height =
          contentElement['offsetHeight'] + 'px';
      }
  ...
  private setVirtualHeight(): void {
    if (this.parent.virtualScrollModule && this.parent.enableVirtualization) {
      const wrapper: HTMLElement = getValue(
        'virtualTrack',
        this.parent.ganttChartModule.virtualRender
      );
      wrapper.style.height = (
        this.parent.treeGrid.element.getElementsByClassName(
          'e-virtualtrack'
        )[0] as HTMLElement
      ).style.height;
      const wrapper1: HTMLElement = getValue(
        'wrapper',
        this.parent.ganttChartModule.virtualRender
      );
      const treegridVirtualHeight: string = (
        this.parent.treeGrid.element.getElementsByClassName(
          'e-virtualtable'
        )[0] as HTMLElement
      ).style.transform;
      const virtualTable: string = (
        document.getElementsByClassName('e-virtualtable')[1] as HTMLElement
      ).style.transform;
      if (this.parent.enableTimelineVirtualization) {
        let translateXValue: string;
        if (virtualTable !== '') {
          translateXValue = virtualTable
            .match(/translate.*\((.+)\)/)[1]
            .split(', ')[0];
        } else {
          const chartTransform: string = (
            this.parent.ganttChartModule.scrollElement.getElementsByClassName(
              'e-virtualtable'
            )[0] as HTMLElement
          ).style.transform;
          translateXValue = chartTransform
            .match(/translate.*\((.+)\)/)[1]
            .split(', ')[0];
        }
        const translateYValue: string = treegridVirtualHeight
          .match(/translate.*\((.+)\)/)[1]
          .split(', ')[1];
  ...
  public updateLastRowBottomWidth(): void {
    if (
      ...
      const expandedRecords: IGanttData[] =
        this.parent.virtualScrollModule && this.parent.enableVirtualization
          ? this.parent.currentViewData
          : this.parent.getExpandedRecords(this.parent.currentViewData);
      const lastExpandedRow: IGanttData =
        expandedRecords[expandedRecords.length - 1];
      const lastExpandedRowIndex: number =
        this.parent.currentViewData.indexOf(lastExpandedRow);
      const lastRow: HTMLElement =
        this.parent.getRowByIndex(lastExpandedRowIndex);
      const table: Element = this.parent.chartRowsModule.ganttChartTableBody;
      if (
        table.querySelectorAll('.e-chart-row-cell.e-chart-row-border.e-lastrow')
      ) {
        removeClass(
          table.querySelectorAll(
            '.e-chart-row-cell.e-chart-row-border.e-lastrow'
          ),
          'e-lastrow'
        );
      }
      if (
        this.chartBodyContent.clientHeight <
        this.chartBodyContainer.clientHeight
      ) {
        if (lastRow) {
  ...
  private ganttChartMouseDown(e: PointerEvent | TouchEvent): void {
    ...
    if (
      ...
      this.initPinchDistance = this.calculatePinchDistance(
        e.touches[0],
        e.touches[1]
      );
    ...
    if (
      ...
      const editAction: string = this.parent.editModule.taskbarEditModule[
        'getTaskBarAction'
      ](e as PointerEvent);
      if (
        editAction === 'ChildDrag' ||
        editAction === 'ParentDrag' ||
        editAction === 'MilestoneDrag' ||
        editAction === 'ManualParentDrag'
      ) {
        const args: Object = {
          cancel: cancel,
    ...
    if (!cancel && !this.isPinching) {
      ...
      if (
        this.parent.editSettings.allowTaskbarEditing &&
        (this.parent.element.querySelector('.e-left-resize-gripper') ||
          this.parent.element.querySelector('.e-left-connectorpoint-outer-div'))
      ) {
  ...
  private ganttChartMouseUp(e: PointerEvent): void {
    ...
    if (!this.parent.editSettings.allowEditing) {
      ...
      if (!isTaskbarEdited) {
        ...
        const isOnTaskbarElement: boolean | Element =
          (e.target as HTMLElement).classList.contains(
            cls.taskBarMainContainer
          ) || closest(e.target as Element, '.' + cls.taskBarMainContainer);
        if (
          closest(<HTMLElement>target, '.e-gantt-parent-taskbar') &&
          !this.parent.editSettings.allowEditing
        ) {
  ...
  private scrollToTarget(e: PointerEvent): void {
    ...
    if (
      row &&
      this.parent.element.contains(row) &&
      (this.parent.element
        .querySelectorAll('.e-chart-rows-container')[0]
        .contains(e.target as Element) ||
        this.parent.element
          .querySelectorAll('.e-gridcontent')[0]
          .contains(e.target as Element)) &&
      this.parent.currentViewData.length > 0
    ) {
      const rowIndex: number = getValue(
        'rowIndex',
        closest(e.target as Element, 'tr')
      );
      const dateObject: Date =
        this.parent.currentViewData[rowIndex as number].ganttProperties
          .startDate;
      const dateObjLeft: number =
        this.parent.currentViewData[rowIndex as number].ganttProperties.left;
  ...
  public updateScrollLeft(scrollLeft: number): void {
    ...
    if (
      ...
      this.scrollObject.setScrollLeft(
        scrollLeft - 50,
        this.parent.enableRtl ? -1 : 0
      );
    }
  ...
  private mouseUp(e: PointerEvent): void {
    ...
    const resizeCheck: HTMLElement = this.parent.element.querySelector(
      '.e-taskbar-resize-div'
    );
    if (!isNullOrUndefined(resizeCheck)) {
      resizeCheck.remove();
    ...
    if (this.parent.allowRowDragAndDrop) {
      const ganttDragElemet: HTMLElement =
        this.parent.element.querySelector('.e-ganttdrag');
      if (ganttDragElemet) {
  ...
  private documentMouseUp(e: PointerEvent | TouchEvent): void {
    ...
    const Check: HTMLElement = this.parent.element.getElementsByClassName(
      'e-clone-taskbar'
    )[0] as HTMLElement;
    if (!isNullOrUndefined(Check)) {
      const clonetbody: HTMLElement = Check.parentElement;
    ...
    if (this.parent.showActiveElement) {
      ...
      if (
        this.focusedElement &&
        !(e.target as HTMLElement).classList.contains('e-split-bar')
      ) {
    ...
    if (!isNullOrUndefined(this.parent.onTaskbarClick) && !isTaskbarEdited) {
      const target: EventTarget = e.target;
      const taskbarElement: Element = closest(
        <HTMLElement>target,
        '.e-gantt-parent-taskbar,.e-gantt-child-taskbar,.e-gantt-milestone'
      );
      if (taskbarElement) {
   ...
   * @returns {void}
   */
  public onTaskbarClick(
    e: PointerEvent | KeyboardEventArgs,
    target: EventTarget,
  ...
  private ganttChartMove(e: PointerEvent | TouchEvent): void {
    if (
      ...
      const currentPinchDistance: number = this.calculatePinchDistance(
        e.touches[0],
        e.touches[1]
      );
    ...
    if (
      ...
      if (
        this.parent.element.getElementsByClassName('e-clone-taskbar').length >
          0 &&
        !this.parent.enableRtl
      ) {
        let xValue: number;
        ...
        if (
          xValue <=
          this.parent.getOffsetRect(this.parent.ganttChartModule.chartElement)
            .left
        ) {
          return;
        }
      ...
      if (
        ...
        this.parent.connectorLineEditModule.updateConnectorLineEditElement(
          e as PointerEvent
        );
      }
    }
  ...
  private contextClick(e: PointerEvent): void {
    if (this.parent.allowFiltering && this.parent.filterModule) {
      this.parent.filterModule.closeFilterOnContextClick(
        e.srcElement as Element
      );
    }
  ...
  public mouseMoveHandler(e: PointerEvent): void {
    if (
      !isNullOrUndefined(this.parent.onMouseMove) &&
      ...
      const element: Element = closest(
        <HTMLElement>target,
        '.e-chart-row-cell,.e-connector-line-container,' +
      ...
      if (element) {
        ...
        if (closest(<HTMLElement>target, '.e-indicator-span')) {
          ...
          if (indicators.length > 1) {
            for (index = 0; index < indicators.length; index++) {
              if (
                indicators[index as number].name ===
                (<HTMLElement>element).innerText.trim()
              ) {
                break;
              }
            }
        ...
        if (closest(<HTMLElement>target, '.e-connector-line-container')) {
          ...
          args.predecessor =
            this.parent.tooltipModule.getPredecessorTooltipData(obj);
        }
        if (closest(<HTMLElement>target, '.e-event-markers')) {
          const obj: TooltipEventArgs = {} as TooltipEventArgs;
          obj.target = element as HTMLElement;
          args.eventMarkers =
            this.parent.tooltipModule.getMarkerTooltipData(obj);
        }
  ...
  private doubleClickHandler(e: PointerEvent): void {
    ...
    const rowData: IGanttData =
      this.parent.ganttChartModule.getRecordByTarget(e);
    if (
      this.parent.editSettings.allowEditing &&
  ...
  public getChartRows(): NodeListOf<Element> {
    if (
      document.getElementById(this.parent.element.id + 'GanttTaskTableBody') !==
      null
    ) {
      return document
        .getElementById(this.parent.element.id + 'GanttTaskTableBody')
        .querySelectorAll('.e-chart-row');
    } else {
      return null;
    }
  }
  ...
  public collapsedGanttRow(args: object): void {
    if (
      (isNullOrUndefined(args['gridRow']) &&
        this.parent.enableVirtualization) ||
      isNullOrUndefined(args['chartRow'])
    ) {
      return;
    }
    ...
    if (
      ...
      record = this.parent.currentViewData.filter(
        (item: IGanttData) =>
          item.ganttProperties[this.parent.taskFields.id] ===
          args['data'][this.parent.taskFields.id]
      )[0];
    } else {
    ...
    if (this.isExpandCollapseFromChart) {
      ...
      if (
        ...
        const gridRec: Object = this.parent.treeGrid
          .getCurrentViewRecords()
          .filter(function (item: Object): boolean {
            return item[idField as string] === args['data'][idField as string];
          })[0];
  ...
  public expandedGanttRow(args: object): void {
    ...
    if (this.isExpandCollapseFromChart) {
      ...
      if (
        ...
        const gridRec: Object = this.parent.treeGrid
          .getCurrentViewRecords()
          .filter(function (item: IGanttData): boolean {
            return item[idField as string] === args['data'][idField as string];
          })[0];
  ...
  private expandCollapseChartRows(
    ...
    if (action === 'expand') {
      ...
      const targetElement: NodeListOf<Element> = (
        rowElement as HTMLElement
      ).querySelectorAll('.e-row-collapse');
    ...
    if (!this.parent.enableVirtualization) {
      ...
      for (let i: number = 0; i < chartRows.length; i++) {
        if (
          (<HTMLElement>chartRows[i as number]).classList.contains(
            'gridrowtaskId' +
      ...
      for (let i: number = 0; i < rows.length; i++) {
        ...
        if (
          childRecords[i as number].childRecords &&
    ...
    if (action === 'expand') {
      ...
      if (this.isCollapseAll && !this.parent.allowTaskbarOverlap) {
        const treeGridContentHeight: number = this.parent.enableRtl
          ? this.parent['element'].getElementsByClassName('e-content')[2]
              .children[0]['offsetHeight']
          : this.parent['element'].getElementsByClassName('e-content')[0]
              .children[0]['offsetHeight'];
        this.parent.contentHeight = treeGridContentHeight;
        document.getElementsByClassName('e-chart-rows-container')[0][
          'style'
        ].height = this.parent.contentHeight + 'px';
      }
    }
  ...
  private wireEvents(): void {
    ...
    if (this.parent.isAdaptive) {
      EventHandler.add(
        this.parent.chartPane,
        click,
        this.ganttChartMouseClick,
        this
      );
      EventHandler.add(
        this.parent.chartPane,
        mouseUp,
        this.ganttChartMouseUp,
        this
      );
    }
    if (!this.parent.isAdaptive) {
    ...
    EventHandler.add(
      this.parent.chartRowsModule.ganttChartTableBody,
      'dblclick',
  ...
  private onWheelZoom(e: WheelEvent): void {
    if (e.ctrlKey) {
      ...
      if (this.debounceTimeout) {
        ...
        if (
          this.debounceTimeoutNext + 20 <= this.debounceTimeout ||
          !this.debounceTimeoutNext
        ) {
      ...
      do {
        ...
        currentColumn =
          this.parent.ganttColumns[
            (next as Element).getAttribute('data-colindex')
          ];
        rowIndex = this.parent.treeGrid
          .getRows()
          .indexOf(next.parentElement as HTMLTableRowElement);
      } while (!currentColumn.allowEditing);
      this.parent.treeGrid.saveCell();
      this.parent.treeGrid.editCell(
        rowIndex,
        this.parent.ganttColumns[next.getAttribute('data-colindex')].field
      );
    }
  ...
  public onTabAction(e: KeyboardEventArgs): void {
    ...
    const toolbarItems: HTMLCollectionOf<Element> =
      document.getElementsByClassName('e-toolbar-item');
    const isInEditedState: boolean =
      this.parent.editModule &&
    ...
    let $target: Element = isInEditedState
      ? (e.target as Element).closest('.e-rowcell')
      : (e.target as Element);
    ...
    if (
      ...
      !(
        ...
        $target.classList.contains('e-input') ||
    ...
    this.focusedRowIndex = $target.closest('.e-rowcell')
      ? ($target.parentElement as any).rowIndex
      : /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
      $target.closest('.e-chart-row')
      ? ($target.closest('.e-chart-row') as any).rowIndex
      : -1;
    const isTab: boolean = e.action === 'tab' ? true : false;
    let nextElement: Element | string = this.getNextElement(
      $target,
      isTab,
      isInEditedState
    );
    if (
      nextElement &&
      (nextElement === 'noNextRow' ||
        (nextElement as HTMLElement).classList.contains('e-rowdragheader'))
    ) {
      // eslint-disable-next-line
      nextElement === 'noNextRow' &&
      this.parent.treeGrid.element.getElementsByClassName('e-editedbatchcell')
    ...
    if (nextElement && $target.classList.contains('e-headercell')) {
      let colIndex: number = parseInt(
        (nextElement as HTMLElement).getAttribute('data-colindex'),
        10
      );
      if (e.action === 'shiftTab') {
        while (
          colIndex !== -1 &&
          !this.parent.treeGrid.columns[colIndex as number]['visible']
        ) {
          colIndex = colIndex - 1;
        }
        if (colIndex !== -1) {
          colIndex = this.parent.allowRowDragAndDrop ? colIndex + 1 : colIndex;
          nextElement = document.getElementsByClassName('e-columnheader')[0]
            .childNodes[colIndex as number] as HTMLElement;
        } else {
          const toolbarItems: HTMLCollectionOf<Element> =
            document.getElementsByClassName('e-toolbar-item');
          for (let i: number = toolbarItems.length - 1; i > 0; i--) {
            if (
              !document
                ...
                [i as number].classList.contains('e-hidden')
            ) {
              nextElement =
                document.getElementsByClassName('e-toolbar-item')[i as number];
      ...
      let itemIndex: number =
        this.currentToolbarIndex !== -1
          ? e.action === 'tab'
            ...
            : this.currentToolbarIndex - 1
          : e.action === 'shiftTab'
          ? toolbarItems.length - 1
          : 1;
      let isUpdated: boolean = false;
      if (
        itemIndex !== -1 &&
        (e.action === 'shiftTab' ||
          (e.action === 'tab' && itemIndex < toolbarItems.length))
      ) {
        do {
          if (
            !toolbarItems[itemIndex as number].classList.contains('e-hidden')
          ) {
    ...
    if (
      e.action === 'tab' &&
      !nextElement &&
      this.currentToolbarIndex === toolbarItems.length - 1 &&
      ($target.classList.contains('e-toolbar-item') ||
        $target.classList.contains('e-input') ||
        $target.classList.contains('e-btn'))
    ) {
      for (let i: number = 0; i < this.parent.treeGrid.columns.length; i++) {
        if (this.parent.treeGrid.columns[i as number]['visible']) {
          nextElement = document.getElementsByClassName('e-columnheader')[0]
            .childNodes[i as number] as HTMLElement;
          break;
        }
      }
    }
    ...
    if (
      ...
      !isNullOrUndefined(nextElement['cellIndex'])
    ) {
      if (this.parent.allowRowDragAndDrop) {
      ...
      if (
        ...
        !this.parent.ganttColumns[this.nextElementIndex]['allowEditing'] &&
        this.parent.ganttColumns[this.nextElementIndex]['field'] !==
    ...
    if (typeof nextElement !== 'string') {
      if (
        ...
        $target.closest('.e-chart-row-cell') ||
        $target.classList.contains('e-headercell') ||
        $target.closest('.e-segmented-taskbar') ||
        $target.classList.contains('e-timeline-header-container')
      ) {
        e.preventDefault();
      }
      ...
      if (
        ...
        if (isTab) {
          if (this.parent.editSettings.allowNextRowEdit) {
            const rowData: IGanttData =
              this.parent.currentViewData[this.focusedRowIndex];
            const columnName: string =
              this.parent.ganttColumns[
                nextElement.getAttribute('data-colindex')
              ].field;
            if (rowData.hasChildRecords) {
              if (
                columnName === this.parent.taskFields.endDate ||
                columnName === this.parent.taskFields.duration ||
                columnName === this.parent.taskFields.dependency ||
                columnName === this.parent.taskFields.progress ||
                columnName === this.parent.taskFields.work ||
                ...
                columnName === this.parent.taskFields.name ||
                columnName === this.parent.taskFields.startDate
              ) {
                this.parent.treeGrid.grid.notify('key-pressed', e);
              } else {
                this.parent.treeGrid.grid.notify('key-pressed', e);
            ...
            if (
              !nextElement ||
              (nextElement &&
                !nextElement.classList.contains('e-headercell') &&
                !nextElement.classList.contains('e-timeline-header-container'))
            ) {
              if ($target.classList.contains('e-headercell')) {
                this.manageFocus($target as HTMLElement, 'remove', false);
              }
              /* eslint-disable-next-line */
              const row: IGanttData =
                this.parent.currentViewData[
                  ($target.parentElement as any).rowIndex
                ];
              ...
              if (
                row.hasChildRecords &&
                (this.parent.ganttColumns[next.getAttribute('data-colindex')]
                  .field === this.parent.taskFields.progress ||
                  !this.parent.ganttColumns[next.getAttribute('data-colindex')]
                    .allowEditing) &&
                this.parent.ganttColumns[next.getAttribute('data-colindex')]
                  .field !== this.parent.taskFields.id &&
                $target.classList.contains('e-editedbatchcell')
              ) {
                let currentColumn: ColumnModel;
                next = this.updateElement(
                  next,
                  currentColumn,
                  isTab,
                  isInEditedState,
          ...
          if (
            ...
            if (
              ...
              parseInt(next.parentElement.getAttribute('data-rowindex'), 10) !==
                0 &&
              ...
              const rowElement: Element = this.getNextRowElement(
                rowIndex,
                isTab,
                true
              );
              ...
              const rowData: IGanttData =
                this.parent.flatData[
                  parseInt(rowElement.getAttribute('data-rowindex'), 10)
                ];
              if (
                rowData.hasChildRecords &&
                (!this.parent.ganttColumns[next.getAttribute('data-colindex')]
                  .allowEditing ||
                  this.parent.ganttColumns[next.getAttribute('data-colindex')]
                    .field === this.parent.taskFields.progress)
      ...
      if (this.parent.element.querySelectorAll('.e-focused').length > 0) {
        this.manageFocus(
          this.parent.element.querySelectorAll('.e-focused')[0] as HTMLElement,
          'remove',
          false
        );
      }
      if (
        ...
        if (nextElement) {
          ...
          if (
            ...
            if (!$target.classList.contains('e-rowcell')) {
              this.parent.treeGrid.grid.notify('key-pressed', e);
              const fmodule: FocusStrategy = getValue(
                'focusModule',
              ...
              /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
              fmodule.content.matrix.current = [
                (nextElement.parentElement as any).rowIndex,
                (nextElement as any).cellIndex,
              ];
            }
  ...
  private getNextElement(
    ...
    while (
      nextElement &&
      nextElement.parentElement.classList.contains('e-row')
    ) {
      if (
        !nextElement.matches('.e-hide') &&
        !nextElement.matches('.e-rowdragdrop')
      ) {
        return nextElement;
      }
      nextElement = isTab
      ...
      if (
        ...
        if (isTab) {
          ...
          if (
            ...
            rowElement = this.getNextRowElement(rowIndex, isTab, true);
            const childElement: Element | string = this.getChildElement(
              rowElement,
              isTab
            );
            return childElement;
            ...
            if (
              this.validateNextElement(rowElement, 'e-left-label-container')
            ) {
              return rowElement.getElementsByClassName(
                'e-left-label-container'
              )[0];
            } else if (
              this.validateNextElement(rowElement, 'e-taskbar-main-container')
            ) {
              return rowElement.getElementsByClassName(
                'e-taskbar-main-container'
              )[0];
            } else if (
              this.validateNextElement(rowElement, 'e-right-label-container')
            ) {
              return rowElement.getElementsByClassName(
        ...
        $target.parentElement.classList.contains('e-chart-row-cell') ||
  ...
  private getNextRowElement(
    ...
    const expandedRecords: IGanttData[] = this.parent.getExpandedRecords(
      this.parent.currentViewData
    );
    const currentItem: IGanttData =
      this.parent.currentViewData[rowIndex as number];
    const expandedRecordIndex: number = expandedRecords.indexOf(currentItem);
    const nextRecord: IGanttData = isTab
      ? expandedRecords[expandedRecordIndex + 1]
      : expandedRecords[expandedRecordIndex - 1];
    const nextRowIndex: number =
      this.parent.currentViewData.indexOf(nextRecord);
    if (nextRecord) {
      return isChartRow
        ? this.parent.treeGrid.getRows()[nextRowIndex as number]
        : this.parent.getRowByIndex(nextRowIndex);
    } else {
      return null;
    }
  }
  ...
  private getChildElement(
    ...
    if (rowElement) {
      childElement = isTab
        ? rowElement.children[0]
        : rowElement.children[rowElement.children.length - 1];
      while (childElement) {
        if (
          !childElement.matches('.e-hide') &&
          !childElement.matches('.e-rowdragdrop')
        ) {
          return childElement;
        }
        childElement = isTab
  ...
  public manageFocus(
    ...
    if (isChartElement) {
      ...
      if (
        element.classList.contains('e-left-label-container') ||
        element.classList.contains('e-right-label-container')
      ) {
        childElement = element.getElementsByTagName('span')[0];
      } else if (
        element.classList.contains('e-taskbar-main-container') ||
        element.classList.contains('e-gantt-child-taskbar-inner-div')
      ) {
        /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
        const rowIndex: number = (closest(element, '.e-chart-row') as any)
          .rowIndex;
        const data: IGanttData =
          this.parent.currentViewData[rowIndex as number];
        const className: string = data.hasChildRecords
      ...
      if (
        element.classList.contains('e-right-label-temp-container') ||
        element.classList.contains('e-left-label-temp-container') ||
        element.classList.contains('e-indicator-span') ||
        element.classList.contains('e-timeline-header-container')
      ) {
        if (focus === 'add') {
  ...
  public getIndexByTaskBar(target: Element): number {
    ...
    if (isNullOrUndefined(row)) {
      ...
      recordIndex = [].slice
        .call(this.parent.chartRowsModule.ganttChartTableBody.childNodes)
        .indexOf(row);
    } else {
      const id: string = row.getAttribute('rowUniqueId');
      ...
      if (this.parent.enableVirtualization && this.parent.enableMultiTaskbar) {
        ...
        if (
          ...
          recordIndex = this.parent.ids.indexOf(
            record.ganttProperties.taskId.toString()
          );
        } else {
