import { createElement, isNullOrUndefined, addClass } from '@hoyui/base';
import { Gantt } from '../base/gantt';
import * as cls from '../base/css-constants';
import {
  Splitter as SplitterLayout,
  ResizeEventArgs,
  ResizingEventArgs,
} from '@hoyui/layouts';
...
export class Splitter {
  ...
  public renderSplitter(): void {
    const splitterPosition: string = this.calculateSplitterPosition(
      this.parent.splitterSettings
    );
    ...
    this.splitterObject = new SplitterLayout({
      ...
      paneSettings: [
        {
          ...
          min: this.getSpliterPositionInPercentage(
            this.parent.splitterSettings.minimum
          ),
        },
        {
          resizable: true,
      ...
      resizeStart: (args: ResizeEventArgs) => {
        if (
          ...
          this.parent.contextMenuModule.isOpen
        ) {
        ...
        this.parent.trigger(
          'splitterResizeStart',
          args,
          (resizeStartArgs: ResizeEventArgs) => {
      ...
      resizing: (args: ResizingEventArgs) => {
        ...
        if (this.parent.timelineModule.isZoomToFit) {
          this.parent.timelineModule.updateTimelineAfterZooming(
      ...
      resizeStop: (args: ISplitterResizedEventArgs) => {
        ...
        const leftPane: HTMLElement = args.pane[0];
        this.splitterPreviousPositionGrid = leftPane.scrollWidth + 1 + 'px';
        this.splitterObject.paneSettings[0].size =
          this.getSpliterPositionInPercentage(
            this.splitterPreviousPositionGrid
          );
        this.splitterObject.paneSettings[1].size =
          this.parent.ganttWidth -
          parseInt(this.splitterPreviousPositionGrid, 10) -
          4 +
          'px';
        const callBackPromise: Deferred = new Deferred();
        this.parent.trigger(
          'splitterResized',
          args,
          (splitterResizedArgs: ISplitterResizedEventArgs) => {
            if (splitterResizedArgs.cancel === true) {
              ...
              this.splitterObject.paneSettings[1].size =
                this.getSpliterPositionInPercentage(
                  this.splitterPreviousPositionChart
                );
            }
    ...
    const splitterLeftPane: HTMLElement =
      this.splitterObject.element.querySelector('.e-split-bar');
    if (splitterLeftPane) {
      const numericValue: number = parseFloat(
        splitterPosition.replace('%', '')
      );
  ...
  public calculateSplitterPosition(splitter: SplitterSettingsModel): string {
    if (
      !isNullOrUndefined(this.splitterObject) &&
    ...
    if (splitter.view === 'Grid') {
      ...
      if (!isNullOrUndefined(splitter.position) && splitter.position !== '') {
        ...
        !isNullOrUndefined(splitter.columnIndex) &&
        splitter.columnIndex >= 0
      ) {
        if (
          splitter.columnIndex * 150 < this.parent.ganttWidth ||
          !this.parent.element.classList.contains('e-device')
        ) {
          return this.getSpliterPositionInPercentage(
            this.getTotalColumnWidthByIndex(splitter.columnIndex).toString() +
              'px'
          );
        } else {
          return this.getSpliterPositionInPercentage(
            splitter.columnIndex * 130 + 'px'
          );
        }
      } else {
        ...
        return this.parent.enableRtl
          ? this.getSpliterPositionInPercentage(widthAsString)
          : this.getSpliterPositionInPercentage('250px');
      }
    }
  }
  /**
  ...
  private getSpliterPositionInPercentage(position: string): string {
    let value: string =
      !isNullOrUndefined(position) && position !== '' ? position : null;
    if (!isNullOrUndefined(value)) {
      if (position.indexOf('px') !== -1) {
        const intValue: number = parseInt(position, 10);
        value =
          (intValue / this.parent.ganttWidth) * 100 <= 100
            ? (intValue / this.parent.ganttWidth) * 100 + '%'
            : '25%';
      } else {
        if (this.parent.enableRtl) {
  ...
  private getTotalColumnWidthByIndex(index: number): number {
    ...
    const newArray: ColumnModel[] = originalArray.filter(
      (obj: ColumnModel) => obj.visible === undefined || obj.visible === true
    );
    const deepCopiedArray: ColumnModel[] = newArray.map((obj: ColumnModel) =>
      Object.assign({}, obj)
    );
  ...
  public updateSplitterPosition(): void {
    this.splitterObject.separatorSize =
      this.parent.splitterSettings.separatorSize;
    const splitterPosition: string = this.calculateSplitterPosition(
      this.parent.splitterSettings
    );
    this.splitterObject.paneSettings[0].min =
      this.getSpliterPositionInPercentage(this.parent.splitterSettings.minimum);
  ...
  public triggerCustomResizedEvent(): void {
    const pane1: HTMLElement = this.splitterObject.element.querySelectorAll(
      '.e-pane'
    )[0] as HTMLElement;
    const pane2: HTMLElement = this.splitterObject.element.querySelectorAll(
      '.e-pane'
    )[1] as HTMLElement;
    ...
    if (eventArgs.cancel === true) {
      this.splitterObject.paneSettings[0].size =
        this.getSpliterPositionInPercentage(this.splitterPreviousPositionGrid);
      this.splitterObject.paneSettings[1].size =
        this.getSpliterPositionInPercentage(this.splitterPreviousPositionChart);
    }
  }
