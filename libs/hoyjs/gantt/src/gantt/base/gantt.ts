...
import { createSpinner, showSpinner, hideSpinner, Dialog } from '@hoyui/popups';
import { RowDragEventArgs, GridColumn } from '@hoyui/grids';
...
import { DataStateChangeEventArgs } from '@hoyui/treegrid';
import {
...
import {
  TreeGrid,
  FilterSettingsModel as TreeGridFilterSettingModel,
} from '@hoyui/treegrid';
...
import { CellDeselectEventArgs, IIndex, FailureEventArgs } from '@hoyui/grids';
import {
  HeaderCellInfoEventArgs,
  ColumnMenuClickEventArgs,
  ColumnMenuOpenEventArgs,
} from '@hoyui/grids';
import { ColumnMenuItemModel, ExcelQueryCellInfoEventArgs } from '@hoyui/grids';
import {
...
import {
  ContextMenuOpenEventArgs as CMenuOpenEventArgs,
  ContextMenuClickEventArgs as CMenuClickEventArgs,
} from './interface';
  ...
  implements INotifyPropertyChanged
{
   ...
   */
  @Collection<DayWorkingTimeModel>(
    [
      { from: 8, to: 12 },
      { from: 13, to: 17 },
    ],
  ...
  private initProperties(): void {
    ...
    this.cloneProjectStartDate =
      this.enablePersistence && this.cloneProjectStartDate
        ? this.cloneProjectStartDate
        : null;
    this.cloneProjectEndDate =
      this.enablePersistence && this.cloneProjectEndDate
        ? this.cloneProjectEndDate
        : null;
  ...
  private isUndoRedoItemPresent(action: string): boolean {
    if (
      ...
      this.undoRedoActions.indexOf(action as GanttAction) !== -1
    ) {
      return true;
    } else {
      return false;
  ...
  public getDateFormat(): string {
    if (!isNullOrUndefined(this.dateFormat)) {
      ...
      const ganttDateFormat: string = this.globalize.getDatePattern({
        skeleton: 'yMd',
      });
  ...
  private resourceFieldsMapping(): void {
    ...
    resourceSettings.id = !isNullOrUndefined(resourceSettings.id)
      ? resourceSettings.id
      : this.resourceIDMapping;
    resourceSettings.name = !isNullOrUndefined(resourceSettings.name)
  ...
  protected render(): void {
    ...
    if (this.isVue) {
      ...
      /* eslint-disable-next-line */
      (<{ vueInstance?: any }>this.treeGrid).vueInstance = (<
        { vueInstance?: any }
      >this).vueInstance;
      /* eslint-disable-next-line */
      (<{ vueInstance?: any }>this.treeGrid.grid).vueInstance = (<
        { vueInstance?: any }
      >this).vueInstance;
    }
  ...
  private actionFailures(): void {
    ...
    if (allPropertiesNull) {
      failureCases.push(
        'Gantt Task Fields are not configured properly. The task fields are crucial for both the project and resource views to function properly. Please ensure that the gantt task fields are properly configured!'
      );
    }
    if (this.viewType === 'ResourceView') {
      if (!this.resourceFields.id) {
        failureCases.push(
          'Gantt Resource Fields are not properly configured, which is crucial for the Resource View. Please ensure that the resource fields are configured correctly!'
        );
      }
    }
    if (
      this.dataSource instanceof DataManager &&
      isNullOrUndefined(this.taskFields.hasChildMapping)
    ) {
      failureCases.push(
        'hasChildMapping property is not configured for load-on-demand. Please ensure its properly configured in the Gantt Task Fields!'
      );
    }
    if (
      !isNullOrUndefined(this.timelineSettings.topTier.format) &&
      this.timelineSettings.topTier.format !== '' &&
      !lettersRegex.test(this.timelineSettings.topTier.format)
    ) {
      failureCases.push(
        'The provided top tier format is invalid. Please ensure that you provide a valid format for these tier. Make sure to use only letters and avoid numbers or special characters!'
      );
    }
    if (
      !isNullOrUndefined(this.timelineSettings.bottomTier.format) &&
      this.timelineSettings.bottomTier.format !== '' &&
      !lettersRegex.test(this.timelineSettings.bottomTier.format)
    ) {
      failureCases.push(
        'The provided bottom  tier format is invalid. Please ensure that you provide a valid format for these tier. Make sure to use only letters and avoid numbers or special characters!'
      );
    }
    if (isPrimary && this.viewType !== 'ResourceView') {
      failureCases.push(
        'Primarykey is not configured properly. The primarykey is crucial for doing CRUD operations or map taskId in column field of gantt!'
      );
    }
    if (this.eventMarkers.length > 0) {
      this.eventMarkers.forEach((markers: EventMarkerModel) => {
        if (!markers.day) {
          failureCases.push(
            'Day is not configured properly in event markers.Please ensure that the day in event markers is properly configured!'
          );
        }
      });
    }
    ...
    function checkModule(
      ...
      if (!isNullOrUndefined(injectedModules)) {
        modules = injectedModules.filter(
          (e: Function) => e.prototype.getModuleName() === moduleName
        );
      }
      if (settings && !isNullOrUndefined(modules) && modules.length === 0) {
        failureCases.push(
          `Module ${moduleName} is not available in Gantt component! You either misspelled the module name or forgot to load it!`
        );
      }
    }
    ...
    if (
      ...
      this.editSettings.allowNextRowEdit ||
      this.editSettings.allowTaskbarEditing ||
      this.editSettings.mode ||
      this.editSettings.newRowPosition ||
    ...
    if (
      this.allowFiltering ||
      (this.toolbar && this.toolbar.indexOf('Search') !== -1)
    ) {
      filter = true;
    }
  ...
  public hideMaskRow(): void {
    let isTablePresent: number =
      this.element.querySelectorAll('.e-masked-table').length;
    if (
      !isNullOrUndefined(this.contentMaskTable) &&
      (isTablePresent !== 0 || this.contentMaskTable)
    ) {
    ...
    if (
      !isNullOrUndefined(this.headerMaskTable) &&
      (isTablePresent !== 0 || this.headerMaskTable)
    ) {
    ...
    if (
      this.element.querySelectorAll('.' + cls.timelineHeaderTableContainer)
        .length !== 0
    ) {
        ...
        if (
          !isNullOrUndefined(
            this.element.querySelectorAll(
              '.' + cls.timelineHeaderTableContainer
    ...
    if (
      !isNullOrUndefined(
        this.element.querySelector('.' + cls.timelineHeaderContainer)
      )
    ) {
      this.element.querySelector('.' + cls.timelineHeaderContainer)[
        'style'
    ...
    if (
      !isNullOrUndefined(
        this.element.getElementsByClassName(cls.chartBodyContent)[0]
      )
    ) {
      this.element.getElementsByClassName(cls.chartBodyContent)[0][
        'style'
  ...
  public showMaskRow(): void {
    const ganttHeader: Element = this.chartPane.childNodes[0]
      .childNodes[0] as Element;
    this.scrollLeftValue =
      this.chartPane.childNodes[0].childNodes[0]['scrollLeft'];
    const ganttContent: Element = this.chartPane.childNodes[0]
      ...
      if (ganttContent) {
        ...
        if (
          ...
          for (let i: number = 0; i < this.singleTier; i++) {
            this.element.querySelectorAll(
              '.' + cls.timelineHeaderTableContainer
            )[parseInt(i.toString(), 10)]['style'].visibility = 'hidden';
          }
        }
        ...
        this.contentMaskTable = this.contentMaskTable =
          this.createMaskTable(content);
      }
      if (ganttHeader) {
        this.element.querySelector('.' + cls.timelineHeaderContainer)[
          'style'
        ...
        this.headerMaskTable = this.headerMaskTable =
          this.createMaskTable(content);
      }
    }
  }
  ...
  private renderHeaderBackground(element: Element): void {
    ...
    const header: boolean = closest(
      parentElement,
      '.' + cls.timelineHeaderContainer
    )
      ? true
      : false;
    if (header) {
      const div: Element = this.createElement('div', {
      ...
      let containerWidth: number = Math.round(
  ...
  private renderBackGround(element: Element): void {
    ...
    const content: boolean = closest(
      parentElement,
      '.' + cls.chartBodyContainer
    )
      ? true
      : false;
    if (content) {
  ...
  private createMaskTable(element: Element): any {
    const parentElement: Element = element;
    const shimmerContainerHeight: number =
    ...
    if (!header) {
      ...
      div.style.width =
        this.element.getElementsByClassName('e-timeline-header-container')[0][
          'offsetWidth'
        ] + 'px';
  ...
  private createEmptyTimeLineTable(timelineHeight: number): Element {
    ...
    for (let i: number = 0; i < row.length; i++) {
      tbody.appendChild(
        this.applyTimelineMaskRow(row[parseInt(i.toString(), 10)])
      );
  ...
  private applyTimelineMaskRow(row: Element): Element {
    ...
    for (let i: number = 0; i < maskRow.childNodes.length - 1; i++) {
      maskRow.childNodes[parseInt(i.toString(), 10)]['style']['width'] =
        166 + 'px';
    }
    ...
    for (let i: number = 0; i < maskCells.length; i++) {
      ...
      switch (this.topBottomHeader) {
        case 0: {
          if (this.enableRtl) {
            maskCell.innerHTML =
              '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-timelineHeader"></span>';
        ...
        case 1: {
          ...
          for (let i: number = 0; i < innerMaskCells.length; i++) {
            const htmlInner: Element =
              innerMaskCells[parseInt(i.toString(), 10)];
            if (i === 0) {
              if (this.enableRtl) {
                htmlInner.innerHTML =
                  '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-innerHTML"></span>';
              ...
              if (this.enableRtl) {
                htmlInner.innerHTML =
                  '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-innerHTML1"></span>';
              ...
              if (this.enableRtl) {
                htmlInner.innerHTML =
                  '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-innerHTML2"></span>';
  ...
  private applyMaskRow(row: Element): Element {
    ...
    if (this.columnLoop < 4) {
      ...
      for (let i: number = 0; i < num; i++) {
        maskRow.appendChild(
          this.createElement('td', { className: 'e-masked-cell e-rowcell' })
        );
      }
    } else {
      maskRow.appendChild(
      ...
      switch (this.columnLoop) {
        case 1:
          if (i === 0) {
            if (this.enableRtl) {
              maskCell.innerHTML =
                '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-maskcell01"></span>';
            ...
            maskCell.innerHTML =
              '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-maskcell02"></span>';
          }
          break;
        case 2:
          if (i === 0) {
            if (this.enableRtl) {
              maskCell.innerHTML =
                '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-maskcell03"></span>';
              maskCell.children[0]['style'].left = -14 + 'px';
            } else {
              maskCell.innerHTML =
                '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-maskcell03"></span>';
            ...
            maskCell.innerHTML =
              '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-maskcell04"></span>';
          }
          break;
        case 3:
          if (i === 0) {
            if (this.enableRtl) {
              maskCell.innerHTML =
                '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-maskcell05"></span>';
            ...
            if (this.enableRtl) {
              maskCell.innerHTML =
                '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-maskcell06"></span>';
        ...
        case 4:
          if (this.enableRtl) {
            maskCell.innerHTML =
              '<span class="e-mask e-skeleton e-skeleton-text e-shimmer-wave e-maskcell07"></span>';
  ...
  public renderGantt(isChange?: boolean): void {
    ...
    if (this.predecessorModule && this.taskFields.dependency) {
      ...
      if (
        this.isInPredecessorValidation &&
        this.enableValidation &&
        this.autoCalculateDateScheduling &&
        !(
          this.isLoad &&
          !this.treeGrid.loadChildOnDemand &&
  ...
  public removeCriticalPathStyles(): void {
    ...
    removeClass(
      ganttChartElement.querySelectorAll('.e-gantt-child-taskbar-inner-div'),
      cls.criticalChildTaskBarInnerDiv
    );
    removeClass(
      ganttChartElement.querySelectorAll(
        '.e-gantt-child-progressbar-inner-div'
      ),
      cls.criticalChildProgressBarInnerDiv
    );
    removeClass(
      ganttChartElement.querySelectorAll('.e-critical-milestone'),
      cls.criticalMilestone
    );
    removeClass(
      this.element.querySelectorAll('.e-connector-line'),
      cls.criticalConnectorLineSVG
    );
    removeClass(
      this.element.querySelectorAll('.e-connector-line-arrow'),
      cls.criticalConnectorArrowSVG
    );
  }
  ...
  private keyDownHandler(e: KeyboardEventArgs): void {
    if (e.altKey) {
      if (e.keyCode === 74) {
        //alt j
        this.ganttChartModule.manageFocus(
          this.treeGrid.element.childNodes[1] as HTMLElement,
          'remove',
          false
        );
  ...
  public windowResize(): void {
    if (!isNullOrUndefined(this.element)) {
      ...
      if (this.predecessorModule && this.taskFields.dependency) {
        this.updateRowHeightInConnectorLine(
          this.updatedConnectorLineCollection
        );
        this.connectorLineModule.renderConnectorLines(
          this.updatedConnectorLineCollection
        );
      }
      ...
      if (
        ...
        this.criticalPathModule.criticalConnectorLine(
          criticalModule.criticalPathCollection,
      ...
      this.calculateDimensions();
      const pane1: HTMLElement =
        this.splitterModule.splitterObject.element.querySelectorAll(
          '.e-pane'
        )[0] as HTMLElement;
      const pane2: HTMLElement =
        this.splitterModule.splitterObject.element.querySelectorAll(
          '.e-pane'
        )[1] as HTMLElement;
      this.splitterModule.splitterPreviousPositionGrid =
        pane1.scrollWidth + 1 + 'px';
      this.splitterModule.splitterPreviousPositionChart =
        pane2.scrollWidth + 1 + 'px';
      this.splitterModule.splitterObject.paneSettings[1].size =
        this.ganttWidth -
        parseInt(this.splitterModule.splitterPreviousPositionGrid, 10) -
        4 +
        'px';
  ...
  public keyActionHandler(e: KeyboardEventArgs): void {
    if (
      this.enableContextMenu &&
      this.contextMenuModule &&
      (e.action === 'downArrow' || e.action === 'upArrow') &&
      document.getElementById(this.element.id + '_contextmenu') &&
      this['args']
    ) {
    ...
    if (
      e.target &&
      (e.action === 'downArrow' || e.action === 'upArrow') &&
      e.target === this.element.querySelector('.e-rowcell')
    ) {
  ...
  private updateRowHeightInConnectorLine(
    collection: IConnectorLineObject[]
  ): void {
    if (collection && collection.length) {
      const rowHeight: number =
        this.ganttChartModule.getChartRows()[0] &&
  ...
  private updateCurrentViewData(): void {
    ...
    if (
      ...
      const data: object = gridData.filter((x: any) => {
        if (
          x['data'][this.taskFields.id] ===
          this.flatData[0].ganttProperties.taskId
        ) {
          return x;
        }
      })[0];
  ...
  public updateContentHeight(args?: object): void {
    if (
      !this.allowTaskbarOverlap &&
      !this.ganttChartModule.isCollapseAll &&
      !this.ganttChartModule.isExpandAll &&
      !this.isLoad
    ) {
      return;
    } else {
      if (
        ...
        const expandedRecords: IGanttData[] =
          this.virtualScrollModule && this.enableVirtualization
            ? this.currentViewData
            : this.getExpandedRecords(this.currentViewData);
        let height: number;
        const chartRow: Element = !isNullOrUndefined(
          this.ganttChartModule.getChartRows()
        )
          ? this.ganttChartModule.getChartRows()[0]
          : null;
        if (
          !isNullOrUndefined(chartRow) &&
          chartRow.getBoundingClientRect().height > 0
        ) {
  ...
  public getExpandStatus(data: IGanttData): boolean {
    ...
    if (!isNullOrUndefined(parentRecord)) {
      if (parentRecord.expanded === false) {
        ...
        const parentData: IGanttData = this.getParentTask(
          parentRecord.parentItem
        );
        if (parentData.expanded === false) {
          return false;
        } else {
          return this.getExpandStatus(
            this.getParentTask(parentRecord.parentItem)
          );
        }
      } else {
        return true;
  ...
  public getExpandedRecords(records: IGanttData[]): IGanttData[] {
    ...
    const expandedRecords: IGanttData[] = records.filter(
      (record: IGanttData) => {
    ...
    const zoomingLevels: ZoomTimelineSettings[] = [
      {
        ...
        bottomTier: { unit: 'Year', format: 'yyyy', count: 10 },
        timelineUnitSize: 99,
        level: 0,
      ...
      },
      {
        ...
        bottomTier: { unit: 'Year', format: 'yyyy', count: 5 },
        timelineUnitSize: 99,
        level: 1,
        timelineViewMode: 'Year',
        weekStartDay: _WeekStartDay,
      ...
      },
      {
        ...
        bottomTier: { unit: 'Year', format: 'yyyy', count: 1 },
        timelineUnitSize: 99,
        level: 2,
        timelineViewMode: 'Year',
        weekStartDay: _WeekStartDay,
      ...
      {
        topTier: { unit: 'Year', format: 'yyyy', count: 1 },
        bottomTier: { unit: 'Month', format: 'MMM yyyy', count: 1 },
        timelineUnitSize: 99,
        level: 7,
        timelineViewMode: 'Year',
        weekStartDay: _WeekStartDay,
      ...
      {
        topTier: { unit: 'Day', format: 'E dd yyyy', count: 1 },
        bottomTier: { unit: 'Hour', format: 'hh a', count: 2 },
        timelineUnitSize: 66,
        level: 18,
        timelineViewMode: 'Day',
        weekStartDay: _WeekStartDay,
      ...
      },
      {
        ...
        bottomTier: { unit: 'Minutes', format: 'mm', count: 30 },
        timelineUnitSize: 66,
        level: 22,
      ...
      {
        topTier: { unit: 'Hour', format: 'ddd MMM, h a', count: 1 },
        bottomTier: { unit: 'Minutes', format: 'mm', count: 15 },
        timelineUnitSize: 66,
        level: 23,
        timelineViewMode: 'Hour',
        weekStartDay: _WeekStartDay,
      ...
      {
        topTier: { unit: 'Hour', format: 'ddd MMM, h a', count: 1 },
        bottomTier: { unit: 'Minutes', format: 'mm', count: 1 },
        timelineUnitSize: 66,
        level: 24,
        timelineViewMode: 'Hour',
        weekStartDay: _WeekStartDay,
  ...
  public getDurationString(duration: number, durationUnit: string): string {
    const value: string = this.dateValidationModule.getDurationString(
      duration,
      durationUnit
    );
    return value;
  ...
  public getWorkString(work: number, workUnit: string): string {
    const value: string = this.dateValidationModule.getWorkString(
      work,
      workUnit
    );
    return value;
  ...
  private updateTreeColumns(): void {
    ...
    if (
      ...
      this.columns[this.treeColumnIndex] &&
    ...
    for (let i: number = 0; i < gridColumns.length; i++) {
      ...
      for (let j: number = 0; j < this.columns.length; j++) {
        if (
          this.columns[j as number]['field'] === gridColumns[i as number].field
        ) {
          for (const prop of Object.keys(this.columns[j as number])) {
            if (!isUndefined(this.columns[j as number][prop as string])) {
              gridColumn[prop as string] =
                gridColumns[i as number][prop as string];
            }
          ...
          if (
            this.columns[j as number]['type'] !== 'checkbox' &&
            !isNullOrUndefined(temp) &&
            temp !== ''
          ) {
  ...
  public treeDataBound(args: object): void {
    if (this.isLoad && this.undoRedoModule) {
      if (this.sortSettings.columns.length > 0) {
        this.undoRedoModule['previousSortedColumns'] =
          this.sortSettings.columns;
      } else if (this.searchSettings.key !== '') {
        this.undoRedoModule['searchString'] = this.searchSettings.key;
      }
    }
    this.element.getElementsByClassName('e-chart-root-container')[0][
      'style'
    ].height = '100%';
    // let gridHeight: string = this.element.getElementsByClassName('e-gridcontent')[0]['style'].height;
    const gridContent: HTMLElement = this.element.getElementsByClassName(
      'e-gridcontent'
    )[0].childNodes[0] as HTMLElement;
    gridContent.setAttribute('tabindex', '0');
    const treeGridrole: HTMLElement = this.element.getElementsByClassName(
      'e-gridcontent'
    )[0].childNodes[0].childNodes[0] as HTMLElement;
    treeGridrole.setAttribute('role', 'treegrid');
    const timelineContainer: number = this.element.getElementsByClassName(
    ...
    const scrollContainer: HTMLElement = this.element.getElementsByClassName(
      'e-chart-scroll-container e-content'
    )[0] as HTMLElement;
    scrollContainer['style'].height =
      'calc(100% - ' + timelineContainer + 'px)';
    scrollContainer.setAttribute('tabindex', '0');
    if (
      !isNullOrUndefined(this.toolbarModule) &&
      !isNullOrUndefined(this.toolbarModule.element)
    ) {
      this.splitterElement.style.height =
        'calc(100% - ' + this.toolbarModule.element.offsetHeight + 'px)';
    } else {
    ...
    if (this.isLoad) {
      ...
      if (!this.isTreeGridRendered) {
        ...
        if (this.timelineModule.isSingleTier) {
          addClass(
            this.treeGrid.element.querySelectorAll('.e-headercell'),
            cls.timelineSingleHeaderOuterDiv
          );
          addClass(
            this.treeGrid.element.querySelectorAll('.e-columnheader'),
            cls.timelineSingleHeaderOuterDiv
          );
        } else {
          removeClass(
    ...
    for (
      let i: number = 0;
      i <
      document.getElementsByClassName('e-timeline-header-table-container')
        .length;
      i++
    ) {
      for (
        ...
        const childElement: HTMLElement = <HTMLElement>(
          document.getElementsByClassName('e-timeline-header-table-container')[
            i as number
          ].children[0].children[0].children[j as number].children[0]
        );
        if (childElement) {
    ...
    this.isLoad = false;
    this.isExpandPerformed = !this.enableVirtualization
      ? false
      : this.isExpandPerformed;
    if (!this.isExpandPerformed) {
      this.previousFlatData = extend(
        [],
        this.flatData,
        [],
        true
  ...
  private getCurrentRecords(args: object): void {
    if (this.predecessorModule && this.taskFields.dependency) {
      this.connectorLineModule.removePreviousConnectorLines(
        this.currentViewData
      );
    }
    this.updateCurrentViewData();
    ...
    for (let prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        case 'columnMenuItems':
          this.treeGrid.grid.columnMenuItems = getActualProperties(
        ...
        case 'enableCriticalPath':
          ...
          break;
        case 'filterSettings':
          this.treeGrid.filterSettings = getActualProperties(
            this.filterSettings
          ) as TreeGridFilterSettingModel;
          this.treeGrid.dataBind();
          break;
        case 'gridLines':
        ...
        case 'selectionSettings':
          this.treeGrid.selectionSettings = getActualProperties(
            this.selectionSettings
          );
          this.treeGrid.grid.selectionSettings.enableToggle =
            this.selectionSettings.enableToggle;
          this.treeGrid.dataBind();
          break;
        case 'searchSettings':
          if (newProp.searchSettings.key !== ('' || undefined)) {
            this.treeGrid.grid.searchSettings = getActualProperties(
              this.searchSettings
            );
            this.treeGrid.grid.dataBind();
          }
          this.treeGrid.searchSettings = getActualProperties(
            this.searchSettings
          );
          this.treeGrid.dataBind();
        ...
        case 'height':
          ...
          break;
        case 'editSettings':
          this.treeGrid.editSettings.allowAdding =
            this.editSettings.allowAdding;
          this.treeGrid.editSettings.allowDeleting =
            this.editSettings.allowDeleting;
          this.treeGrid.editSettings.showDeleteConfirmDialog =
            this.editSettings.showDeleteConfirmDialog;
          this.treeGrid.editSettings.allowEditing =
            this.editSettings.allowEditing;
          this.treeGrid.editSettings.allowNextRowEdit =
            this.editSettings.allowNextRowEdit;
        ...
        case 'validateManualTasksOnLinking':
          this.validateManualTasksOnLinking =
            newProp.validateManualTasksOnLinking;
          break;
  ...
  private updateOverAllocationCotainer(): void {
    if (this.showOverAllocation) {
      ...
      const rangeContainer: HTMLElement = this.element.querySelector(
        '.' + cls.rangeContainer
      );
      if (rangeContainer) {
  ...
  public getPersistData(): string {
    ...
    for (let i: number = 0; i < keyEntity.length; i++) {
      ...
      for (
        let k: number = 0,
          val: string[] = ignoreOnPersist[keyEntity[i as number]];
        !isNullOrUndefined(val) && k < val.length;
        k++
      ) {
  ...
  public destroy(): void {
    ...
    this.unwireEvents();
    if (
      !isNullOrUndefined(this.validationDialogElement) &&
      !this.validationDialogElement.isDestroyed
    ) {
    ...
    if (
      !isNullOrUndefined(this.dayMarkersModule) &&
      !isNullOrUndefined(this.dayMarkersModule['eventMarkerRender']) &&
      !isNullOrUndefined(
        this.dayMarkersModule['eventMarkerRender'].eventMarkersContainer
      )
    ) {
  ...
  public renderChartGridLines(): void {
    ...
    const chartRowsTD: NodeListOf<HTMLTableDataCellElement> = document
      .getElementById(this.element.id + 'GanttTaskTableBody')
      .querySelectorAll('td');
    if (this.gridLines === 'Vertical') {
      ...
      if (
        !isNullOrUndefined(verticalLines) &&
        verticalLines.style.display !== 'none'
      ) {
        verticalLines.style.display = 'none';
      }
  ...
  public updateGridLineContainerHeight(): void {
    if (this.chartVerticalLineContainer) {
      this.chartVerticalLineContainer.style.height = formatUnit(
  ...
  public getContentHeight(): number {
    const scrollHeight: number =
      this.ganttChartModule.scrollElement.offsetHeight - 16; //16 is horizontal scrollbar height
    const contentHeight: number =
      this.ganttChartModule.chartBodyContent.offsetHeight;
    const height: number =
      contentHeight < scrollHeight ? contentHeight : scrollHeight;
    return height;
  }
  ...
  public reUpdateDimention(currentProp?: string): void {
    ...
    this.splitterModule.splitterObject.height = (
      this.ganttHeight - toolbarHeight
    ).toString();
    ...
    if (!isNullOrUndefined(this.dayMarkersModule)) {
      const holidayContainer: HTMLElement = getValue(
        'nonworkingDayRender.holidayContainer',
        this.dayMarkersModule
      );
      const weekendContainer: HTMLElement = getValue(
        'nonworkingDayRender.weekendContainer',
        this.dayMarkersModule
      );
      const eventMarkersContainer: HTMLElement = getValue(
        'eventMarkerRender.eventMarkersContainer',
        this.dayMarkersModule
      );
      if (holidayContainer) {
    ...
    this.ganttChartModule.scrollObject.setHeight(
      this.ganttHeight -
        this.ganttChartModule.chartTimelineContainer.offsetHeight -
        toolbarHeight
    );
  }
  ...
  private renderChartVerticalLines(): void {
    if (!this.element.contains(this.chartVerticalLineContainer)) {
      ...
      this.ganttChartModule.chartBodyContent.insertBefore(
        this.chartVerticalLineContainer,
        this.ganttChartModule.chartBodyContent.lastChild
      );
    }
    this.chartVerticalLineContainer.innerHTML = '';
    let headerTable: Element = this.element.getElementsByClassName(
      'e-timeline-header-table-container'
    )[1];
    if (isNullOrUndefined(headerTable)) {
      headerTable = this.element.getElementsByClassName(
        'e-timeline-header-table-container'
      )[0];
    }
    const thElements: HTMLCollectionOf<HTMLTableHeaderCellElement> =
      headerTable.getElementsByTagName(
        'th'
    ...
    for (let n: number = 0; n < thLength; n++) {
      ...
      const divElement: HTMLElement = createElement('div', {
        ...
        styles:
          (this.enableRtl
            ? 'right:' + (leftPos + 1)
            : 'left:' + (leftPos !== -1 ? leftPos + 0.3 : leftPos)) + 'px',
      });
  ...
  public getDefaultLocale(): Object {
    const ganttLocale: Object = {
      ...
      saveButton: 'Save',
      taskBeforePredecessorFS:
        'You moved "{0}" to start before "{1}" finishes and the two tasks are linked.' +
        'As the result, the links cannot be honored. Select one action below to perform',
      taskAfterPredecessorFS:
        'You moved "{0}" away from "{1}" and the two tasks are linked.' +
        'As the result, the links cannot be honored. Select one action below to perform',
      taskBeforePredecessorSS:
        'You moved "{0}" to start before "{1}" starts and the two tasks are linked.' +
        'As the result, the links cannot be honored. Select one action below to perform',
      taskAfterPredecessorSS:
        'You moved "{0}" to start after "{1}" starts and the two tasks are linked.' +
        'As the result, the links cannot be honored. Select one action below to perform',
      taskBeforePredecessorFF:
        'You moved "{0}" to finish before "{1}" finishes and the two tasks are linked.' +
        'As the result, the links cannot be honored. Select one action below to perform',
      taskAfterPredecessorFF:
        'You moved "{0}" to finish after "{1}" finishes and the two tasks are linked.' +
        'As the result, the links cannot be honored. Select one action below to perform',
      taskBeforePredecessorSF:
        'You moved "{0}" away from "{1}" to starts and the two tasks are linked.' +
        'As the result, the links cannot be honored. Select one action below to perform',
      taskAfterPredecessorSF:
        'You moved "{0}" to finish after "{1}" starts and the two tasks are linked.' +
        'As the result, the links cannot be honored. Select one action below to perform',
      okText: 'Ok',
  ...
  public scrollToTask(taskId: string): void {
    if (this.ids.indexOf(taskId) !== -1) {
      const left: number =
        this.flatData[this.ids.indexOf(taskId)].ganttProperties.left;
      this.ganttChartModule.updateScrollLeft(left);
    }
  }
  /**
  ...
  public updateChartScrollOffset(left: number, top: number): void {
    if (!isNullOrUndefined(left)) {
      left =
        this.ganttChartModule.scrollElement.scrollWidth <= left
          ? this.ganttChartModule.scrollElement.scrollWidth
          : left;
      this.ganttChartModule.scrollObject.setScrollLeft(
        left,
        this.enableRtl ? -1 : 0
      );
    }
    if (!isNullOrUndefined(top)) {
      top =
        this.ganttChartModule.scrollElement.scrollHeight <= top
          ? this.ganttChartModule.scrollElement.scrollHeight
          : top;
  ...
  public getParentTask(cloneParent: IParent): IGanttData | null {
    ...
    if (
      ...
      const parent: IGanttData = this.flatData.find(
        (val: IGanttData) => cloneParent.uniqueID === val.uniqueID
      );
      if (parent) {
        return parent;
      }
  ...
  public getRootParent(ganttRecord: IGanttData, level: number): IGanttData {
    ...
    return this.getRootParent(
      this.getParentTask(ganttRecord.parentItem),
      level
    );
  }
  /**
  ...
  public pdfExport(
    ...
    if (
      pdfExportProperties &&
      pdfExportProperties.fitToWidthSettings &&
      pdfExportProperties.fitToWidthSettings.isFitToWidth
    ) {
  ...
  public renderWorkingDayCell(args: RenderDayCellEventArgs): void {
    const includeWeekend: boolean =
      this.taskMode !== 'Auto'
        ? true
        : this.includeWeekend || !this.autoCalculateDateScheduling
        ? true
        : false;
    const nonWorkingDays: number[] = !includeWeekend
      ? this.nonWorkingDayIndex
      : [];
    const holidays: number[] = this.totalHolidayDates;
    if (
      nonWorkingDays.length > 0 &&
      nonWorkingDays.indexOf(args.date.getDay()) !== -1
    ) {
  ...
  public previousTimeSpan(mode?: string): void {
    if (
      this.undoRedoModule &&
      this['isUndoRedoItemPresent']('PreviousTimeSpan')
    ) {
      ...
      timeSpan['previousTimelineStartDate'] = extend(
        [],
        [],
        [this.timelineModule.timelineStartDate],
        true
      )[0];
      timeSpan['previousTimelineEndDate'] = extend(
        [],
        [],
        [this.timelineModule.timelineEndDate],
        true
      )[0];
      (this.undoRedoModule['getUndoCollection'][
        this.undoRedoModule['getUndoCollection'].length - 1
      ] as Object) = timeSpan;
    }
    this.timelineModule.performTimeSpanAction(
      'prevTimeSpan',
      'publicMethod',
      new Date(this.timelineModule.timelineStartDate.getTime()),
      new Date(this.timelineModule.timelineEndDate.getTime()),
      mode
    );
  }
  /**
  ...
  public updateProjectDates(
    ...
    if (
      !this.pdfExportModule ||
      (this.pdfExportModule && !this.pdfExportModule.isPdfExport) ||
      (this.pdfExportModule &&
        this.pdfExportModule.isPdfExport &&
        this.pdfExportModule.helper.exportProps &&
        this.pdfExportModule.helper.exportProps.fitToWidthSettings &&
        !this.pdfExportModule.helper.exportProps.fitToWidthSettings
          .isFitToWidth)
    ) {
      this.timelineModule.updateChartByNewTimeline();
      this.ganttChartModule.chartBodyContent.style.width = formatUnit(
        this.timelineModule.totalTimelineWidth
      );
  ...
  public mergeTask(
    ...
    segmentIndexes: { firstSegmentIndex: number; secondSegmentIndex: number }[]
  ): void {
  ...
  public setRecordValue(
    ...
    if (resultIf) {
      ...
      const ganttData: ITaskData = isTaskData
        ? (record as ITaskData)
        : (record as IGanttData).ganttProperties;
      ...
      isValid =
        ...
        (!isNullOrUndefined(value) &&
          !isNullOrUndefined(record[`${field}`]) &&
      ...
      if (
        task &&
        ((this.editedRecords.indexOf(task) === -1 && isValid) ||
          this.editedRecords.length === 0)
      ) {
        if (
          this.editModule['draggedRecord'] &&
          this.editModule['draggedRecord'].ganttProperties.taskId ===
            ganttData.taskId
        ) {
        ...
        if (this.undoRedoModule) {
          if (this.undoRedoModule['changedRecords'].indexOf(task) === -1) {
            this.undoRedoModule['changedRecords'].push(
              extend({}, {}, task, true)
            );
          }
          const currentAction: string =
            this.undoRedoModule['getUndoCollection'].length > 0
              ? this.undoRedoModule['getUndoCollection'][
                  this.undoRedoModule['getUndoCollection'].length - 1
                ]['action']
              : null;
          if (
            this.editModule &&
            this.editSettings.allowEditing &&
            !this.undoRedoModule['isUndoRedoPerformed'] &&
            !this.isOnDelete &&
            !this.isOnAdded &&
            currentAction !== 'outdent' &&
            currentAction !== 'indent' &&
            currentAction !== 'RowDragAndDrop' &&
            currentAction !== 'TaskbarDragAndDrop'
          ) {
            const oldRecord: IGanttData = this.previousFlatData.filter(
              (data: IGanttData) => {
                return (
                  data.ganttProperties.taskId === task.ganttProperties.taskId
                );
              }
            )[0];
            if (
  ...
  private closeGanttActions(): void {
    if (this.editModule) {
      if (
        this.editModule.cellEditModule &&
        ...
        this.editModule.dialogModule &&
        this.editModule.dialogModule.dialogObj &&
  ...
  public setSplitterPosition(value: string | number, type: string): void {
    ...
    const splitterPosition: string =
      this.splitterModule.calculateSplitterPosition(tempSplitterSettings);
    switch (type) {
      case 'view':
        this.splitterSettings.view = tempSplitterSettings[type as string];
        break;
      case 'columnIndex':
        this.splitterSettings.columnIndex =
          tempSplitterSettings[type as string];
        break;
      case 'position':
    ...
    this.splitterModule.splitterObject.paneSettings[1].size =
      parseFloat('99.75%') - parseFloat(splitterPosition) + '%';
      ...
      if (this.viewType === 'ResourceView') {
        ...
        if (!isNullOrUndefined(resources) && resources.length) {
          for (let i: number = 0; i < resources.length; i++) {
            id =
              typeof resources[i as number] === 'object'
                ? resources[i as number][this.resourceFields.id]
                : resources[0];
          ...
          if (
            parentTask &&
            (parentTask.level === 0 || parentTask.childRecords.length)
          ) {
            const dropChildRecord: IGanttData =
              parentTask.childRecords[rowIndex as number];
            if (
              this.viewType === 'ResourceView' &&
              this.undoRedoModule &&
              this.undoRedoModule['isUndoRedoPerformed']
            ) {
              ...
              const position: RowPosition =
                rowPosition === 'Above' || rowPosition === 'Below'
                  ? rowPosition
                  : 'Child';
              if (position === 'Child') {
                this.editModule.addRecord(
                  data,
                  position,
                  this.getTaskIds().indexOf('R' + id)
                );
              } else {
                this.editModule.addRecord(
                  data,
                  position,
                  this.flatData.indexOf(dropChildRecord)
                );
              }
            } else {
              this.editModule.addRecord(
        ...
        if (
          ...
          this.currentSelection = !isNullOrUndefined(data)
            ? data
  ...
  public getCriticalTasks(): IGanttData[] {
    if (
      !isNullOrUndefined(this.criticalPathModule) &&
      this.enableCriticalPath
    ) {
    ...
    if (this.editModule && this.editSettings.allowEditing) {
      ...
      const record: IGanttData =
        this.updatedRecords.length > 0
          ? !isNullOrUndefined(this.updatedRecords[index as number])
            ? this.updatedRecords[index as number]
            : null
          : null;
  ...
  public addPredecessor(id: number | string, predecessorString: string): void {
    ...
    if (
      this.editModule &&
      !isNullOrUndefined(ganttRecord) &&
      this.editSettings.allowTaskbarEditing
    ) {
      this.connectorLineEditModule.addPredecessor(
        ganttRecord,
        predecessorString
      );
    }
  ...
  public updatePredecessor(
    ...
    if (
      ...
      this.connectorLineEditModule.updatePredecessor(
        ganttRecord,
        predecessorString
      );
    }
  ...
  public getRowByID(id: string | number): HTMLElement {
    ...
    let index: number;
    if (this.loadChildOnDemand && this.taskFields.hasChildMapping) {
      index = this.updatedRecords
        .map((item: IGanttData) => item[this.taskFields.id])
        .indexOf(record.ganttProperties.taskId);
    } else {
      index = this.enableVirtualization
        ? this.currentViewData.indexOf(record)
        : this.updatedRecords.indexOf(record);
    }
    if (index !== -1) {
  ...
  public getUnscheduledTaskClass(ganttProp: ITaskData): string {
    if (
      isNullOrUndefined(ganttProp.startDate) &&
      isNullOrUndefined(ganttProp.endDate) &&
  ...
  public getPredecessorTextValue(type: string): string {
    ...
    switch (type) {
      case 'SS':
        textValue =
          this.localeObj.getConstant('start') +
          '-' +
          this.localeObj.getConstant('start');
        break;
      case 'FF':
        textValue =
          this.localeObj.getConstant('finish') +
          '-' +
          this.localeObj.getConstant('finish');
        break;
      case 'SF':
   ...
   * @hidden
   */
  public getOffsetRect(element: HTMLElement): {
    top: number;
    left: number;
    width?: number;
    height?: number;
    ...
    const left: number = this.enableRtl
      ? box.right + scrollLeft - clientLeft
    ...
    if (!isNullOrUndefined(args)) {
      for (let prop of Object.keys(args)) {
        ...
        switch (prop) {
          case 'projectStartDate':
            this.setProperties(
              { projectStartDate: args[prop as string] },
              true
            );
            break;
          case 'projectEndDate':
   ...
   * @hidden
   */
  public updateDataArgs(
    args: ITaskAddedEventArgs | IActionBeginEventArgs
  ): ITaskAddedEventArgs | IActionBeginEventArgs {
    ...
    if (!isNullOrUndefined(rowData)) {
      ...
      if (data[taskfields.startDate]) {
        this.setRecordValue(
          ...
          rowData.ganttProperties.startDate,
          data,
          true
        );
      }
      if (!isNullOrUndefined(taskfields.duration)) {
        ...
        data[taskfields.startDate] = new Date(
          rowData.ganttProperties.startDate
        );
      ...
      if (
        !isNullOrUndefined(this.contextMenuModule) &&
