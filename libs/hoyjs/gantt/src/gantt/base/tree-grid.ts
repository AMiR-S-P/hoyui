...
import { TreeGrid, ColumnModel } from '@hoyui/treegrid';
import {
  createElement,
  isNullOrUndefined,
  getValue,
...
import { setValue } from '@hoyui/base';
import { Deferred, Query } from '@hoyui/data';
import { TaskFieldsModel } from '../models/models';
import {
  ColumnModel as GanttColumnModel,
  Column as GanttColumn,
} from '../models/column';
...
export class GanttTreeGrid {
  ...
  constructor(parent: Gantt) {
    ...
    this.parent.treeGrid['${enableHtmlSanitizer}'] =
    ...
    if (!this.parent.isLocaleChanged && this.parent.isLoad) {
      this.parent.previousGanttColumns = extend(
        [],
        [],
        this.parent.columns,
        true
  ...
  private addEventListener(): void {
    ...
    this.parent.treeGrid.on(
      'renderReactTemplate',
      this.renderReactTemplate,
      this
    );
  }
  ...
  private createContainer(): void {
    ...
    const tempHeader: HTMLElement = createElement('div', {
  ...
  public renderTreeGrid(): void {
    ...
    this.parent.treeGrid[root as string] = this.parent[root as string]
      ? this.parent[root as string]
      : this.parent;
    setValue(
      'registeredTemplate',
    ...
    if (
      ...
      (this.parent as Gantt).isReact
    ) {
      (this.parent.treeGrid.grid as Grid).portals = (
        this.parent as Gantt
      ).portals;
    }
    this.wireEvents();
  }
  ...
  private composeProperties(): void {
    ...
    this.parent.treeGrid.childMapping = isNullOrUndefined(
    ...
    this.parent.treeGrid.enableVirtualMaskRow =
    ...
    this.parent.treeGrid.expandStateMapping =
      this.parent.taskFields.expandState;
    ...
    if (
      this.parent.searchSettings.fields.length !== 0 ||
    ...
    this.parent.treeGrid.height =
      ...
      )[0]['offsetHeight'] - (this.parent.flatData.length === 0 ? 0 : 19);
  }
  ...
  public ensureScrollBar(): void {
    ...
    const isMobile: boolean =
      /Android|Mac|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
        navigator.userAgent
      );
    if (scrollWidth !== 0) {
      content.style.cssText +=
        'width: calc(100% + ' + (scrollWidth + 1) + 'px);'; //actual scrollbar width 17 px but here scrollbar width set to 16px hence adding increment of 1
    } else {
  ...
  private beforeDataBound(args: object): void {
    if (
      ...
      this.parent.selectionModule.getSelectedRowIndexes().length > 0 &&
    ...
    this.parent.updatedRecords =
      this.parent.virtualScrollModule && this.parent.enableVirtualization
        ? getValue('virtualScrollModule.visualData', this.parent.treeGrid)
        : getValue('result', args);
    if (this.parent.virtualScrollModule && this.parent.enableVirtualization) {
      this.parent.updateContentHeight(args);
    }
    setValue(
      'contentModule.objectEqualityChecker',
    ...
    if (!this.parent.ganttChartModule.isExpandCollapseFromChart) {
      if (!isNullOrUndefined(recordLength)) {
        for (let i: number = 0; i < recordLength; i++) {
          collapsingArgs = this.createExpandCollapseArgs(
            args,
            record[i as number]
          );
  ...
  private expanding(args: object): void | Deferred {
    ...
    if (!this.parent.ganttChartModule.isExpandCollapseFromChart) {
      if (!isNullOrUndefined(recordLength)) {
        for (let i: number = 0; i < recordLength; i++) {
          expandingArgs = this.createExpandCollapseArgs(
            args,
            record[i as number]
          );
  ...
  private collapsed(args: object): void {
    if (
      !this.parent.ganttChartModule.isExpandCollapseFromChart &&
      !this.parent.isExpandCollapseLevelMethod
    ) {
      ...
      if (
        !this.parent.ganttChartModule.isCollapseAll &&
        collapsedArgs['gridRow'] &&
        !this.parent.allowTaskbarOverlap
      ) {
        collapsedArgs['gridRow'].style.height =
          collapsedArgs['chartRow'].style.height;
  ...
  private expanded(args: object): void {
    if (
      ...
      if (
        !this.parent.ganttChartModule.isExpandAll &&
        args['row'] &&
        !this.parent.allowTaskbarOverlap
      ) {
  ...
  private actionBegin(args: FilterEventArgs | SortEventArgs): void {
    ...
    const flag: boolean = getValue(
      'doubleClickTarget',
      this.parent.treeGrid.editModule
    );
    if (flag !== null) {
  ...
  private columnMenuOpen = (args: ColumnMenuOpenEventArgs) => {
    ...
    document.querySelector('.e-colmenu').addEventListener('mousemove', () => {
      const filPopOptions: HTMLElement =
    ...
    if (!isNullOrUndefined(recordLength)) {
      ...
      chartRow =
        this.parent.ganttChartModule.getChartRows()[
          this.parent.currentViewData.indexOf(currentRecord)
        ];
      ...
      chartRow =
        this.parent.ganttChartModule.getChartRows()[
          this.parent.currentViewData.indexOf(recordOne)
        ];
      const eventArgs: object = {
        data: recordOne,
        gridRow: gridRow,
  ...
  private objectEqualityChecker = (old: Object, current: Object) => {
    if (old) {
      ...
      for (let i: number = 0; i < keys.length; i++) {
        const oldKey: number | Date =
          old[keys[parseInt(i.toString(), 10)]] instanceof Date
            ? new Date(old[keys[parseInt(i.toString(), 10)]]).getTime()
            : old[keys[parseInt(i.toString(), 10)]];
        const currentKey: number | Date =
          current[keys[parseInt(i.toString(), 10)]] instanceof Date
            ? new Date(current[keys[parseInt(i.toString(), 10)]]).getTime()
            : current[keys[parseInt(i.toString(), 10)]];
        if (
          oldKey !== currentKey &&
          excludeKeys.indexOf(keys[i as number]) === -1
        ) {
  ...
  private treeActionComplete(args: object): void {
    ...
    if (getValue('requestType', args) === 'reorder') {
      if (
        this.parent.undoRedoModule &&
        !this.parent.undoRedoModule['isFromUndoRedo'] &&
        this.parent['isUndoRedoItemPresent']('ColumnReorder')
      ) {
        ...
        record['toColumn'] = extend(
          [],
          [],
          [this.parent.treeGrid.columns[args['toIndex']]['field']],
          true
        )[0];
        record['fromColumn'] = extend(
          [],
          [],
          [this.parent.treeGrid.columns[args['fromIndex']]['field']],
          true
        )[0];
    ...
    if (getValue('requestType', args) === 'columnstate') {
      if (
        this.parent.undoRedoModule &&
        !this.parent.undoRedoModule['isFromUndoRedo'] &&
        this.parent['isUndoRedoItemPresent']('ColumnState')
      ) {
        if (this.parent.undoRedoModule['redoEnabled']) {
          this.parent.undoRedoModule['disableRedo']();
        }
    ...
    if (getValue('requestType', args) === 'sorting') {
      if (
        this.parent.undoRedoModule &&
        this.parent['isUndoRedoItemPresent']('Sorting')
      ) {
        if (
          this.parent.undoRedoModule['currentAction'] &&
          this.parent.undoRedoModule['currentAction']['sortColumns'].length > 1
        ) {
        ...
        if (
          (!this.parent.undoRedoModule['currentAction'] ||
            this.parent.undoRedoModule['sortedColumnsLength'] !==
              this.parent.undoRedoModule['currentAction']['sortColumns']
                .length) &&
          !this.parent.undoRedoModule['isFromUndoRedo']
        ) {
          if (this.parent.undoRedoModule['redoEnabled']) {
          ...
          record['sortColumns'] = [];
          record['sortColumns'] =
            this.parent.undoRedoModule['previousSortedColumns'];
          (this.parent.undoRedoModule['getUndoCollection'][
            this.parent.undoRedoModule['getUndoCollection'].length - 1
          ] as Object) = record;
        }
        this.parent.undoRedoModule['previousSortedColumns'] =
          this.parent.treeGrid.sortSettings.columns;
      }
      ...
      if (
        this.parent.undoRedoModule &&
        !this.parent.undoRedoModule['isFromUndoRedo'] &&
        this.parent['isUndoRedoItemPresent']('Filtering')
      ) {
        if (this.parent.undoRedoModule['redoEnabled']) {
          this.parent.undoRedoModule['disableRedo']();
        }
      ...
      if (this.parent.editModule && this.parent.editModule.cellEditModule) {
        ...
        if (
          ...
          !isNullOrUndefined(this.parent.getTaskByUniqueID(data.uniqueID))
        ) {
          /* eslint-disable-next-line */
          this.parent.getTaskByUniqueID(data.uniqueID).taskData[
            this.parent.taskFields.duration
          ] = data.taskData[this.parent.taskFields.duration];
          if (
            !isNullOrUndefined(
        ...
        this.parent.editModule.cellEditModule.initiateCellEdit(
          args,
          this.currentEditRow
        );
    ...
    if (getValue('requestType', args) === 'searching') {
      if (
        this.parent.undoRedoModule &&
        this.parent['isUndoRedoItemPresent']('Search')
      ) {
        ...
        this.parent.undoRedoModule['searchString'] =
          this.parent.treeGrid.searchSettings.key;
      }
      this.parent.notify('actionComplete', args);
    }
    if (
      !isNullOrUndefined(this.parent.toolbarModule) &&
      !isNullOrUndefined(getValue('batchChanges', args))
    ) {
      this.parent.toolbarModule.refreshToolbarItems();
    }
    ...
    if (
      getValue('requestType', args) === 'refresh' &&
      isNullOrUndefined(getValue('type', args)) &&
      this.parent.addDeleteRecord
    ) {
      if (this.parent.selectedRowIndex !== -1) {
        if (
          ...
          if (
            ...
            for (let i: number = 0; i < selectedIndexes.length; i++) {
              const records: any =
                this.parent.flatData[selectedIndexes[i as number].toString()];
        ...
        if (
          ...
          for (let i: number = selectedRecords.length - 1; i >= 0; i--) {
            const selectedTaskId: any =
              selectedRecords[i as number][this.parent.taskFields.id];
            const existIndeletedRecords: boolean =
              this.parent.editModule.deletedRecord.some(
                (item: any) => item.ganttProperties.taskId === selectedTaskId
              );
        ...
        this.parent.currentViewData.map((data: Object, index: number) => {
          if (
            ...
            data['ganttProperties'].taskId ===
              this.parent.currentSelection[this.parent.taskFields.id]
          ) {
            indexvalue = index;
          }
        });
    ...
    if (this.parent.showOverAllocation && !this.parent.allowTaskbarOverlap) {
      for (let i: number = 0; i < this.parent.currentViewData.length; i++) {
        if (
          this.parent.currentViewData[i as number].hasChildRecords &&
          !this.parent.currentViewData[i as number].expanded
        ) {
          this.parent.chartRowsModule.updateDragDropRecords(
  ...
  private wireEvents(): void {
    ...
    if (this.parent.isAdaptive) {
      EventHandler.add(
        this.parent.treeGridPane,
        'click',
        this.treeGridClickHandler,
        this
      );
    }
  }
  ...
  private createTreeGridColumn(
    ...
    const previousColumn: GanttColumnModel =
      this.parent.previousGanttColumns.filter(
        (prevcolumn: GanttColumnModel) => {
          return column.field === prevcolumn.field;
        }
      )[0];
    column.disableHtmlEncode = !isNullOrUndefined(column.disableHtmlEncode)
      ? column.disableHtmlEncode
      : this.parent.disableHtmlEncode;
    if (taskSettings.id !== column.field) {
      column.clipMode = column.clipMode
        ? column.clipMode
        : 'EllipsisWithTooltip';
    }
    if (taskSettings.id === column.field) {
      ...
      if (this.parent.isLocaleChanged && previousColumn) {
        column.headerText = previousColumn.headerText
          ? previousColumn.headerText
          : this.parent.localeObj.getConstant('name');
      } else {
        column.headerText = column.headerText
          ? column.headerText
          : this.parent.localeObj.getConstant('name');
      ...
      column.editType = column.editType
        ? column.editType
        : this.parent.getDateFormat().toLowerCase().indexOf('hh') !== -1
        ? 'datetimepickeredit'
        : 'datepickeredit';
      column.format = column.format
        ? column.format
        : { type: 'date', format: this.parent.getDateFormat() };
      column.width = column.width ? column.width : 150;
      if (column.edit && column.edit.params) {
        column.edit.params['renderDayCell'] =
          this.parent.renderWorkingDayCell.bind(this.parent);
      } else {
        if (column.edit) {
          column.edit.params = {
      ...
      column.valueAccessor = column.valueAccessor
        ...
        : !isNullOrUndefined(column.edit) &&
          !isNullOrUndefined(column.edit.read)
        ? null
      ...
      if (this.parent.isLocaleChanged && previousColumn) {
        column.headerText = previousColumn.headerText
          ? previousColumn.headerText
          : this.parent.localeObj.getConstant('dependency');
      } else {
        column.headerText = column.headerText
          ? column.headerText
          : this.parent.localeObj.getConstant('dependency');
      ...
      column.valueAccessor = column.valueAccessor
        ? column.valueAccessor
        : this.dependencyValueAccessor.bind(this);
          ...
          ui: {
            create: (args: { target: Element; column: Object }) => {
              ...
              this.dropInstance = new AutoComplete({
                dataSource: this.changeLocale(
                  this.parent.treeGrid.grid.dataSource
                ),
                fields: {
                  text: this.parent.taskFields.dependency,
                  value: this.parent.taskFields.dependency,
                },
      ...
      if (previousColumn && this.parent.isLocaleChanged) {
        column.headerText = previousColumn.headerText
          ? previousColumn.headerText
          : this.parent.localeObj.getConstant('notes');
      } else {
        column.headerText = column.headerText
          ? column.headerText
          : this.parent.localeObj.getConstant('notes');
      ...
      if (!this.parent.showInlineNotes) {
        if (!column.template) {
          column.template =
            '<div class="e-ganttnotes-info">' +
            '<span class="e-icons e-notes-info"></span></div>';
        }
      }
    } else if (
      taskSettings.baselineStartDate === column.field ||
      ...
      if (previousColumn && this.parent.isLocaleChanged) {
        column.headerText = previousColumn.headerText
          ? previousColumn.headerText
          : this.parent.localeObj.getConstant('work');
      } else {
        column.headerText = column.headerText
          ? column.headerText
          : this.parent.localeObj.getConstant('work');
      ...
      column.valueAccessor = column.valueAccessor
        ? column.valueAccessor
        : this.workValueAccessor.bind(this);
      column.editType = column.editType ? column.editType : 'numericedit';
    } else if (taskSettings.type === column.field) {
      if (previousColumn && this.parent.isLocaleChanged) {
        column.headerText = previousColumn.headerText
          ? previousColumn.headerText
          : this.parent.localeObj.getConstant('taskType');
      } else {
        column.headerText = column.headerText
          ? column.headerText
          : this.parent.localeObj.getConstant('taskType');
      ...
      column.valueAccessor = column.valueAccessor
        ? column.valueAccessor
        : this.taskTypeValueAccessor.bind(this);
    } else if (
      taskSettings.manual === column.field &&
      this.parent.taskMode === 'Custom'
    ) {
      if (previousColumn && this.parent.isLocaleChanged) {
        column.headerText = previousColumn.headerText
          ? previousColumn.headerText
          : this.parent.localeObj.getConstant('taskMode');
      } else {
        column.headerText = column.headerText
          ? column.headerText
          : this.parent.localeObj.getConstant('taskMode');
      ...
      column.valueAccessor = column.valueAccessor
        ? column.valueAccessor
        : this.modeValueAccessor.bind(this);
      column.edit = {
        params: {
          query: new Query(),
          dataSource: [
  ...
  public changeLocale(data: any): any[] {
    const filter: any = [];
    const flatdatas: Object[] = data.map(
      (task: any) => task.ganttProperties.predecessorsName
    );
      ...
      if (!isNullOrUndefined(predecessorsName)) {
        ...
        splitString.map((splitvalue: string, index: number) => {
          if (splitvalue.includes('FS')) {
            value += splitvalue.replace(
              'FS',
              this.parent.localeObj.getConstant('FS')
            );
            value += splitString.length !== index + 1 ? ',' : '';
          } else if (splitvalue.includes('FF')) {
            value += splitvalue.replace(
              'FF',
              this.parent.localeObj.getConstant('FF')
            );
            value += splitString.length !== index + 1 ? ',' : '';
          } else if (splitvalue.includes('SS')) {
            value += splitvalue.replace(
              'SS',
              this.parent.localeObj.getConstant('SS')
            );
            value += splitString.length !== index + 1 ? ',' : '';
          } else if (splitvalue.includes('SF')) {
            value += splitvalue.replace(
              'SF',
              this.parent.localeObj.getConstant('SF')
            );
            value += splitString.length !== index + 1 ? ',' : '';
          }
        });
  ...
  private composeResourceColumn(column: GanttColumnModel): void {
    ...
    if (previousColumn && this.parent.isLocaleChanged) {
      column.headerText = previousColumn.headerText
        ? previousColumn.headerText
        : this.parent.localeObj.getConstant('resourceName');
    } else {
      column.headerText = column.headerText
        ? column.headerText
        : this.parent.localeObj.getConstant('resourceName');
    ...
    column.valueAccessor = column.valueAccessor
      ? column.valueAccessor
      : this.resourceValueAccessor.bind(this);
    column.allowFiltering = column.allowFiltering === false ? false : true;
  }
  /**
  ...
  public getResourceIds(data: IGanttData): object {
    ...
    if (!isNullOrUndefined(value)) {
      for (let i: number = 0; i < value.length; i++) {
        id.push(
          typeof value[i as number] === 'object'
            ? value[i as number][this.parent.resourceFields.id]
            : value[i as number]
        );
      }
      return id;
  ...
  private composeIDColumn(column: GanttColumnModel): void {
    ...
    const lengthDataSource: number = this.parent.dataSource
      ? this.parent.dataSource['length']
      : 0;
    ...
    if (this.parent.isLocaleChanged) {
      ...
      if (previousColumn) {
        column.headerText = previousColumn.headerText
          ? previousColumn.headerText
          : this.parent.localeObj.getConstant('id');
      }
    } else {
      column.headerText = column.headerText
        ? column.headerText
    ...
    for (let i: number = 0; i < lengthDataSource; i++) {
      if (
        !isNullOrUndefined(
          this.parent.dataSource[i as number][this.parent.taskFields.id]
        )
      ) {
        taskIDName =
          this.parent.dataSource[i as number][this.parent.taskFields.id];
    ...
    column.valueAccessor = isProjectView
      ? null
      : this.idValueAccessor.bind(this);
  }
  ...
  private composeProgressColumn(column: GanttColumnModel): void {
    ...
    if (this.parent.isLocaleChanged && previousColumn) {
      column.headerText = previousColumn.headerText
        ? previousColumn.headerText
        : this.parent.localeObj.getConstant('progress');
    } else {
      column.headerText = column.headerText
        ? column.headerText
        : this.parent.localeObj.getConstant('progress');
  ...
  private bindTreeGridColumnProperties(
    newGanttColumn: GanttColumnModel,
    isDefined?: boolean
  ): void {
    const treeGridColumn: ColumnModel = {};
    const ganttColumn: GanttColumnModel = {};
    for (const prop of Object.keys(newGanttColumn)) {
      treeGridColumn[prop as string] = ganttColumn[prop as string] =
        newGanttColumn[prop as string];
    }
  ...
  private durationValueAccessor(
    ...
    if (!isNullOrUndefined(data) && !isNullOrUndefined(data.ganttProperties)) {
      ...
      return this.parent.dataOperation.getDurationString(
        ganttProp.duration,
      ...
      return this.parent.dataOperation.getDurationString(
        parseInt(data[this.parent.taskFields.duration], 10),
  ...
  private workValueAccessor(
    ...
    const ganttProp: ITaskData = data.ganttProperties;
    if (!isNullOrUndefined(ganttProp)) {
      return this.parent.dataOperation.getWorkString(
        ganttProp.work,
        ganttProp.workUnit
      );
    }
  ...
  private idValueAccessor(
    ...
    // eslint-disable-line
    return data.level === 0
      ? 'R-' + data.ganttProperties.taskId
      : 'T-' + data.ganttProperties.taskId;
  }
  ...
  private updateScrollTop(args: object): void {
    ...
    this.previousScroll.top =
      this.treeGridElement.querySelector('.e-content').scrollTop;
  }
