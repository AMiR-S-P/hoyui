...
export function isRemoteData(dataSource: object): boolean {
  if (dataSource instanceof DataManager) {
    ...
    return (
      ...
      adaptor instanceof RemoteSaveAdaptor ||
  ...
  if (eventArgs) {
    ...
    for (let i: number = 0; i < records.length; i++) {
      const data: object = isNotExtend
        ? records[parseInt(i.toString(), 10)].taskData
...
export function updateDates(record: IGanttData, parent: Gantt): void {
  ...
  if (record && !isNullOrUndefined((record as IGanttData).ganttProperties)) {
    (record as IGanttData).taskData[parent.taskFields.startDate] =
      parent.dateValidationModule.remove(
        (record as IGanttData).ganttProperties.startDate,
        parent.timezone
      );
    if (parent.taskFields.endDate !== null) {
      (record as IGanttData).taskData[parent.taskFields.endDate] =
        parent.dateValidationModule.remove(
          (record as IGanttData).ganttProperties.endDate,
          parent.timezone
        );
    }
    if (
      parent.taskFields.baselineEndDate ||
      parent.taskFields.baselineStartDate
    ) {
      (record as IGanttData).taskData[parent.taskFields.baselineStartDate] =
        parent.dateValidationModule.remove(
          (record as IGanttData).ganttProperties.baselineStartDate,
          parent.timezone
        );

      (record as IGanttData).taskData[parent.taskFields.baselineEndDate] =
        parent.dateValidationModule.remove(
          (record as IGanttData).ganttProperties.baselineEndDate,
          parent.timezone
        );
    }
  }
...
export function getIndex(
  ...
  for (let i: number = 0; i < collection.length; i++) {
    if (
      (getValue(key1, collection[i as number]) === getValue(key1, value) &&
