import { isNullOrUndefined, getValue, extend, setValue } from '@hoyui/base';
import { getUid, ReturnType } from '@hoyui/grids';
import {
  IGanttData,
  ITaskData,
  IParent,
...
import { DataManager, Query, Group, ReturnOption } from '@hoyui/data';
...
export class TaskProcessor extends DateProcessor {
  ...
  public checkDataBinding(isChange?: boolean): void {
    ...
    if (isNullOrUndefined(this.parent.dataSource)) {
      ...
      if (
        this.parent.dataSource instanceof Object &&
  ...
  private constructDataSource(dataSource: Object[]): void {
    const mappingData: Object[] = new DataManager(dataSource).executeLocal(
      new Query().group(this.parent.taskFields.parentID)
    );
    ...
    for (let i: number = 0; i < mappingData.length; i++) {
      ...
      if (!isNullOrUndefined(groupData.key)) {
        ...
        if (index > -1) {
          if (!isNullOrUndefined(groupData.key)) {
            dataSource[index as number][this.parent.taskFields.child] =
              groupData.items;
            continue;
          }
        }
  ...
  public cloneDataSource(): void {
    ...
    if (
      !isNullOrUndefined(taskIdMapping) &&
      !isNullOrUndefined(parentIdMapping)
    ) {
      const data: object[] = [];
    ...
    if (this.parent.taskFields.segmentId) {
      this.segmentCollection = new DataManager(
        this.parent.segmentData
      ).executeLocal(new Query().group(this.parent.taskFields.segmentId));
    ...
    if (this.parent.viewType !== 'ProjectView') {
      ...
      this.constructResourceViewDataSource(
        resources,
        hierarchicalData,
        unassignedTasks
      );
      if (unassignedTasks.length > 0) {
        ...
        const resourceName: string =
          this.parent.resourceFields.name || 'resourceName';
        record[this.parent.resourceFields.id] = 0;
        record[resourceName as string] =
  ...
  private constructResourceViewDataSource(
    resources: Object[],
    data: Object[],
    unassignedTasks: Object[]
  ): void {
    for (let i: number = 0; i < data.length; i++) {
      const tempData: Object = data[i as number];
      const child: string =
        this.parent.taskFields.child !== null
          ? this.parent.taskFields.child
          : (this.parent.taskFields.child = 'Children');
      const resourceData: [] =
        tempData && tempData[this.parent.taskFields.resourceInfo];
      const resourceIdMapping: string = this.parent.resourceFields.id;
      if (
        (!tempData[child as string] ||
          tempData[child as string].length === 0) &&
        resourceData &&
        resourceData.length
      ) {
        if (typeof resourceData === 'string') {
          ...
          for (let j: number = 0; j < resources.length; j++) {
            if (
              resources[j as number][
                this.parent.resourceFields.name as string
          ...
          resourceData.forEach((resource: number | object) => {
            const id: string =
              typeof resource === 'object'
                ? resource[resourceIdMapping as string]
                : resource;
            for (let j: number = 0; j < resources.length; j++) {
              if (
                resources[j as number][
                  resourceIdMapping as string
                ].toString() === id.toString()
              ) {
                if (resources[j as number][child as string]) {
                  resources[j as number][child as string].push(tempData);
        ...
        !tempData[child as string] ||
        (tempData[child as string] && tempData[child as string].length === 0)
      ) {
        unassignedTasks.push(tempData);
      }
      if (
        tempData[this.parent.taskFields.child] &&
        tempData[this.parent.taskFields.child].length
      ) {
        this.constructResourceViewDataSource(
          resources,
          tempData[this.parent.taskFields.child],
  ...
  private prepareDataSource(data: Object[]): void {
    ...
    if (
      ...
      this.parent.dataMap = this.parent.flatData.reduce(
        (
          map: Map<string, IGanttData>,
          val: IGanttData
        ): Map<string, IGanttData> => {
    ...
    for (let i: number = 0; i < this.parent.getTaskIds().length; i++) {
      ...
      if (value[0] !== 'R') {
        ...
        this.parent.flatData.filter((data: IGanttData) => {
          if (
            data.ganttProperties.taskId.toString() === value[1] &&
            data.level !== 0
          ) {
  ...
  private prepareRecordCollection(
    ...
    for (let i: number = 0; i < length; i++) {
      ...
      if (!isNullOrUndefined(this.parent.taskFields.segmentId)) {
        const segmentData: Object[] = this.segmentCollection.filter(
          (x: Group) => x.key === tempData[this.parent.taskFields.id]
        );
        if (segmentData.length > 0) {
          tempData[this.parent.taskFields.segments] = (
            segmentData as Group
          )[0].items;
        }
      }
      const ganttData: IGanttData = this.createRecord(
        tempData,
        level,
        parentItem,
        true
      ...
      const childData: Object[] =
        tempData[this.parent.taskFields.child] ||
        (tempData['taskData'] &&
          tempData['taskData'][this.parent.taskFields.child]);
      if (
        this.parent.viewType === 'ResourceView' &&
        isNullOrUndefined(childData) &&
        ...
        if (!parentProp.isAutoSchedule) {
          this.parent.setRecordValue(
            'autoDuration',
            this.calculateAutoDuration(parentProp),
            parentProp,
            true
          );
    ...
    if (length) {
      for (let i: number = 0; i < length; i++) {
        ...
        if (fieldName === undefined) {
          this.parent.setRecordValue(
            columns[i as number].field,
            data[columns[i as number].field],
            ganttRecord
          );
        }
      }
  ...
  public createRecord(
    ...
    const predecessors: string | number | object[] =
      data[taskSettings.dependency];
    const baselineStartDate: Date = this.getDateFromFormat(
      data[taskSettings.baselineStartDate],
      true
    );
    let baselineEndDate: Date = this.getDateFromFormat(
      data[taskSettings.baselineEndDate],
      true
    );
    const ganttData: IGanttData = {} as IGanttData;
    const ganttProperties: ITaskData = {} as ITaskData;
    const autoSchedule: boolean =
      this.parent.taskMode === 'Auto'
        ? true
        : this.parent.taskMode === 'Manual'
        ? false
        : data[taskSettings.manual] === true
        ? false
        : true;
    ...
    if (
      ...
      data['ganttProperties'].predecessor.length > 0 &&
    ...
    if (!isNullOrUndefined(data[taskSettings.id])) {
      ...
      const shouldLoadChild: boolean =
        this.parent.loadChildOnDemand &&
        taskSettings.hasChildMapping &&
        data['taskData']
          ? true
          : false;
      if (shouldLoadChild) {
      ...
      if (
        isNullOrUndefined(data[resourceFields.name]) &&
        data['resourceName'] === 'Unassigned Task'
      ) {
    ...
    this.parent.setRecordValue(
      'baselineStartDate',
      this.checkBaselineStartDate(baselineStartDate, ganttProperties),
      ganttProperties,
      true
    );
    ...
    if (
      baselineEndDate &&
      baselineEndDate.getHours() === 0 &&
      dayEndTime !== 86400
    ) {
      this.setTime(dayEndTime, baselineEndDate);
    }
    if (
      (ganttProperties.baselineStartDate &&
        baselineEndDate &&
        ganttProperties.baselineStartDate.getTime() >
          baselineEndDate.getTime()) ||
      (!isNullOrUndefined(ganttProperties.baselineStartDate) &&
        !isNullOrUndefined(ganttProperties.startDate) &&
        ganttProperties.baselineStartDate.getTime() ===
          ganttProperties.startDate.getTime() &&
        !isNullOrUndefined(baselineEndDate) &&
        !isNullOrUndefined(ganttProperties.endDate) &&
        baselineEndDate.toLocaleDateString() ===
          ganttProperties.endDate.toLocaleDateString() &&
    ...
    this.parent.setRecordValue(
      'baselineEndDate',
      this.checkBaselineEndDate(baselineEndDate, ganttProperties),
      ganttProperties,
      true
    );
    ...
    this.parent.setRecordValue(
      'cssClass',
      data[taskSettings.cssClass],
      ganttProperties,
      true
    );
    this.parent.setRecordValue(
      'parentItem',
      this.getCloneParent(parentItem),
      ganttData
    );
    if (
      this.parent.loadChildOnDemand &&
      ...
      this.parent.setRecordValue(
        ...
        this.parent.currentViewData[
          this.taskIds.indexOf(data[taskSettings.id].toString())
        ]
      );
    }
    const parentUniqId: string = ganttData.parentItem
      ? ganttData.parentItem.uniqueID
      : null;
    this.parent.setRecordValue('parentUniqueID', parentUniqId, ganttData);
    if (
      this.parent.viewType === 'ResourceView' &&
      !isNullOrUndefined(taskSettings.parentID) &&
      !isNullOrUndefined(ganttData.parentItem)
    ) {
      this.parent.setRecordValue(
        'parentId',
        ganttData.parentItem.taskId,
        ganttProperties,
        true
      );
    }
    ...
    if (
      (!isNullOrUndefined(data[taskSettings.child]) &&
        data[taskSettings.child].length > 0) ||
      (data['taskData'] &&
        data['taskData'][taskSettings.child] &&
        data['taskData'][taskSettings.child].length > 0)
    ) {
      ...
      if (
        this.parent.loadChildOnDemand &&
        taskSettings.hasChildMapping &&
        ganttData.taskData[taskSettings.hasChildMapping]
      ) {
        this.parent.setRecordValue('hasChildRecords', true, ganttData);
      } else {
    ...
    if (ganttData.hasChildRecords) {
      this.parent.setRecordValue(
        'autoStartDate',
        ganttData.ganttProperties.startDate,
        ganttProperties
      );
      this.parent.setRecordValue(
        'autoEndDate',
    ...
    this.parent.setRecordValue(
      'expanded',
      ganttData.hasChildRecords && this.parent.collapseAllParentTasks
        ? false
        : true,
      ganttData
    );
    ...
    if (!isLoad) {
      this.parent.setRecordValue(
        'width',
        this.calculateWidth(ganttData),
        ganttProperties,
        true
      );
      this.parent.setRecordValue(
        'left',
        this.calculateLeft(ganttProperties),
        ganttProperties,
        true
      );
      this.parent.setRecordValue(
      ...
      if (
        ganttProperties.baselineEndDate &&
        ganttProperties.baselineStartDate
      ) {
        this.parent.setRecordValue(
          'baselineLeft',
          this.calculateBaselineLeft(ganttProperties),
          ganttProperties,
          true
        );
        this.parent.setRecordValue(
          'baselineWidth',
          this.calculateBaselineWidth(ganttProperties),
          ganttProperties,
          true
        );
      }
    ...
    if (this.parent.viewType === 'ProjectView') {
      ...
      const uniqueId: string = ganttData.uniqueID.replace(
    ...
    if (
      ...
      this.parent.setRecordValue(
        'segments',
        this.setSegmentsInfo(ganttData, true),
        ganttProperties,
        true
      );
    ...
    this.parent.chartRowsModule.updateSegment(
      ...
      ganttData.ganttProperties.taskId
    );
    return ganttData;
  }
  ...
  private sortSegmentsData(
    ...
    if (onLoad) {
      segments.sort((a: ITaskSegment, b: ITaskSegment) => {
        ...
        return (
          this.getDateFromFormat(a[startDate as string]).getTime() -
          this.getDateFromFormat(b[startDate as string]).getTime()
        );
      });
    } else {
      segments.sort((a: ITaskSegment, b: ITaskSegment) => {
  ...
  public setSegmentsInfo(data: IGanttData, onLoad: boolean): ITaskSegment[] {
    ...
    const predefinedProperties: string[] = [
      this.parent.taskFields.duration,
    ...
    if (!isNullOrUndefined(this.parent.taskFields.segments)) {
      segments = onLoad
        ? data.taskData[this.parent.taskFields.segments]
        : data.ganttProperties.segments;
      if (!onLoad) {
        if (
          data.taskData[this.parent.taskFields.segments] &&
          data.taskData[this.parent.taskFields.segments].length > 0
        ) {
          data.taskData[this.parent.taskFields.segments].forEach(
            (segment: Object) => {
      ...
      if (!isNullOrUndefined(segments) && segments.length > 1) {
        ...
        for (let i: number = 0; i < segments.length; i++) {
          ...
          let startDate: Date = onLoad
            ? segment[taskSettings.startDate]
            : segment.startDate;
          let endDate: Date = onLoad
            ? segment[taskSettings.endDate]
            : segment.endDate;
          let duration: number = onLoad
            ? segment[taskSettings.duration]
            : segment.duration;
          ...
          if (!isNullOrUndefined(duration)) {
            endDate = this.getEndDate(
              startDate,
              duration,
              data.ganttProperties.durationUnit,
              data.ganttProperties,
              false
            );
            if (taskSettings.duration) {
              ...
              duration =
                i === segments.length - 1
                  ? remainingDuration
                  : remainingDuration > 0 && duration > remainingDuration
                  ? remainingDuration
                  : duration;
              endDate = this.getEndDate(
                startDate,
              ...
              !taskSettings.duration &&
              taskSettings.endDate &&
              endDate
            ) {
              endDate =
                !isNullOrUndefined(data.ganttProperties.endDate) &&
                endDate.getTime() >= data.ganttProperties.endDate.getTime() &&
                i === segments.length - 1
                  ? data.ganttProperties.endDate
                  : endDate;
            ...
            if (
              endDate &&
              (isNullOrUndefined(duration) || String(duration) === '')
            ) {
              let dayEndTime: number;
            ...
            if (taskSettings.duration) {
              remainingDuration =
                data.ganttProperties.duration - sumOfDuration - 1;
              if (remainingDuration <= 0) {
                continue;
              }
              duration =
          ...
          if (!isNullOrUndefined(ganttSegments[i - 1])) {
            ...
            const offsetDuration: number = this.getDuration(
              ganttSegments[i - 1].endDate,
              ganttSegments[i as number].startDate,
              unit,
              data.ganttProperties.isAutoSchedule,
              data.ganttProperties.isMilestone
            );
            ...
            if (offsetDuration < 1) {
              if (this.parent.weekWorkingTime.length === 0) {
                ...
                const prevSegmentDate: Date = new Date(
                  ganttSegments[i - 1].endDate.getTime()
                );
                segment.startDate = new Date(
                  prevSegmentDate.setHours(prevSegmentDate.getHours() + 24)
                );
                ...
                const dayEndTime: number = this.parent['getEndTime'](
                  ganttSegments[i - 1].endDate
                );
                if (
                  this.getSecondsInDecimal(ganttSegments[i - 1].endDate) ===
                  dayEndTime
                ) {
                  this.setTime(
                    this.parent['getEndTime'](segment.startDate),
                    segment.startDate
                  );
                }
              }
              segment.startDate = this.checkStartDate(
                segment.startDate,
                data.ganttProperties,
                false
              );
              ...
              segment.endDate =
                !taskSettings.duration &&
                taskSettings.endDate &&
                segment.endDate > data.ganttProperties.endDate
                  ? data.ganttProperties.endDate
                  : segment.endDate;
          ...
          taskData.push(
            this.setSegmentTaskData(segment, segments[i as number])
          );
        }
        this.parent.setRecordValue(
          'duration',
        ...
        if (!isNullOrUndefined(ganttSegments[ganttSegments.length - 1])) {
          this.parent.setRecordValue(
            'endDate',
            ganttSegments[ganttSegments.length - 1].endDate,
            data.ganttProperties,
            true
          );
        }
        if (
          !isNullOrUndefined(taskSettings.endDate) &&
          !isNullOrUndefined(ganttSegments[ganttSegments.length - 1])
        ) {
          this.parent.setRecordValue(
            this.parent.taskFields.endDate,
            ganttSegments[ganttSegments.length - 1].endDate,
        ...
        if (!onLoad && taskData && taskData.length > 0) {
          taskData.forEach((task: Object, index: number) => {
            const mergedObject: Object = Object.assign(
              {},
              task,
              this.customSegmentProperties[index as number]
            );
    ...
    if (ganttSegments.length > 1) {
      ...
      this.parent.setRecordValue(
        this.parent.taskFields.segments,
        data.taskData[this.parent.taskFields.segments],
        data,
        true
      );
  ...
  public updateWorkWithDuration(ganttData: IGanttData): void {
    ...
    if (!isNullOrUndefined(resources) && !ganttData.hasChildRecords) {
      ...
      if (this.parent.weekWorkingTime.length > 0) {
        totSeconds = this.parent['getSecondsPerDay'](
          ganttData.ganttProperties.startDate
            ? ganttData.ganttProperties.startDate
            : ganttData.ganttProperties.endDate
        );
      ...
      const durationInDay: number = this.getDurationInDay(
        ganttData.ganttProperties.duration,
        ganttData.ganttProperties.durationUnit
      );
      for (index = 0; index < resourcesLength; index++) {
        resourceUnit =
          resources[index as number][this.parent.resourceFields.unit]; //in percentage
        resourceOneDayWork =
          resourceUnit > 0
            ? (actualOneDayWork * resourceUnit) / 100
            : ganttData.ganttProperties.taskType !== 'FixedUnit'
            ? actualOneDayWork
            : 0;
    ...
    if (ganttData.childRecords) {
      if (ganttData.childRecords.length > 0 && this.parent.isOnEdit) {
        ...
        for (let i: number = 0; i < ganttData.childRecords.length; i++) {
          childCompletedWorks +=
            ganttData.childRecords[i as number].ganttProperties.work;
        }
  ...
  public reUpdateResources(): void {
    if (this.parent.flatData.length > 0) {
      ...
      for (
        let index: number = 0;
        index < this.parent.flatData.length;
        index++
      ) {
    ...
    if (isLoad) {
      if (
        ...
        (dataManager instanceof DataManager &&
          dataManager.dataSource.json &&
          dataManager.dataSource.offline)
      ) {
        if (taskSettings.parentID) {
          ...
          const tempData: object =
            index > -1 ? this.dataArray[index as number] : {};
          if (!isNullOrUndefined(this.parent.taskFields.segmentId)) {
            const segmentDataCollection: Object[] =
              this.segmentCollection.filter(
                (x: Group) => x.key === tempData[this.parent.taskFields.id]
              );
            if (segmentDataCollection.length > 0) {
              tempData[this.parent.taskFields.segments] = (
                segmentDataCollection as Group
              )[0].items;
            }
          }
  ...
  private updateExpandStateMappingValue(
    ...
    if (expandStateMapping && ganttData.hasChildRecords) {
      if (!isNullOrUndefined(mappingValue)) {
        ...
        isNullOrUndefined(mappingValue) &&
        !this.parent.collapseAllParentTasks
      ) {
        updatableValue = true;
      } else if (
        isNullOrUndefined(mappingValue) &&
  ...
  private setValidatedDates(ganttData: IGanttData, data: Object): void {
    ...
    const startDate: Date = this.getDateFromFormat(
      data[taskSettings.startDate],
      true
    );
    const endDate: Date = this.getDateFromFormat(
      data[taskSettings.endDate],
      true
    );
   ...
   * @private
   */
  public calculateScheduledValues(
    ganttData: IGanttData,
    data: Object,
    isLoad: boolean
  ): void {
    ...
    const segments: ITaskSegment[] = taskSettings.segments
      ? data[taskSettings.segments] || ganttData.taskData[taskSettings.segments]
      : null;
    const isMileStone: boolean = taskSettings.milestone
      ? data[taskSettings.milestone]
        ? true
        : false
      : false;
    const durationMapping: string = data[taskSettings.durationUnit]
      ? data[taskSettings.durationUnit]
      : '';
    this.parent.setRecordValue(
      'durationUnit',
      this.validateDurationUnitMapping(durationMapping),
      ganttProperties,
      true
    );
    const work: number = !isNullOrUndefined(data[taskSettings.work])
      ? parseFloat(data[taskSettings.work])
      : 0;
    this.parent.setRecordValue(
      'workUnit',
      this.validateWorkUnitMapping(this.parent.workUnit),
      ganttProperties,
      true
    );
    const taskTypeMapping: string = data[taskSettings.type]
      ? data[taskSettings.type]
      : '';
    ...
    if (isLoad && !this.parent.enableValidation && startDate && endDate) {
      ...
      if (
        !endDate &&
        !startDate &&
        (isNullOrUndefined(duration) || duration === '')
      ) {
        if (this.parent.allowUnscheduledTasks) {
          ...
          this.parent.setRecordValue(
            'startDate',
            this.getProjectStartDate(ganttProperties, isLoad),
            ganttProperties,
            true
          );
        ...
        this.calculateDateFromStartDate(
          startDate,
          endDate,
          duration,
          ganttData,
    ...
    if (
      ...
      if (!isNullOrUndefined(ganttData.ganttProperties.startDate)) {
        ganttData[this.parent.taskFields.startDate] =
          ganttData.ganttProperties.startDate;
        ganttData.taskData[this.parent.taskFields.startDate] =
          ganttData.ganttProperties.startDate;
      }
      if (!isNullOrUndefined(ganttData.ganttProperties.endDate)) {
        ganttData[this.parent.taskFields.endDate] =
          ganttData.ganttProperties.endDate;
        ganttData.taskData[this.parent.taskFields.endDate] =
          ganttData.ganttProperties.endDate;
      }
    }
    ...
    if (ganttProperties.duration === 0) {
      ...
      if (
        !isNullOrUndefined(this.parent.taskFields) &&
        !isNullOrUndefined(this.parent.taskFields.milestone)
      ) {
        this.parent.setRecordValue(
          this.parent.taskFields.milestone,
          true,
          ganttData,
          true
        );
      ...
      if (isNaN(work) || isNullOrUndefined(work)) {
        ...
        switch (tType) {
          case 'FixedDuration':
            if (
              !isNullOrUndefined(
                ganttData[this.parent.taskFields.resourceInfo]
              ) &&
        ...
        if (ganttProperties.duration === 0) {
          ...
          !isNullOrUndefined(ganttProperties.startDate) &&
          !isNullOrUndefined(ganttProperties.duration)
        ) {
          this.parent.setRecordValue(
            'isMilestone',
            false,
  ...
  public updateDurationWithWork(ganttData: IGanttData): void {
    ...
    if (!isNullOrUndefined(resources)) {
      const resourcesLength: number = !isNullOrUndefined(resources)
        ? resources.length
        : 0;
      ...
      const totalHours: number = this.getWorkInHour(
        ganttProperties.work,
        ganttProperties.workUnit
      );
      if (resourcesLength > 0 && totalResourceOneDayWork > 0) {
        updatedDuration += totalHours / totalResourceOneDayWork;
      }
      if (
        ganttProperties.taskType === 'FixedUnit' &&
  ...
  public updateUnitWithWork(ganttData: IGanttData): void {
    ...
    let resources: Object[] =
      !this.parent.isLoad &&
      !isNullOrUndefined(this.parent.editModule) &&
      ...
      !this.parent.editModule.cellEditModule.isCellEdit
        ? this.parent.editModule.dialogModule.ganttResources
        : ganttProperties.resourceInfo;
    if (
      this.parent.editModule &&
    ...
    const totalUnitInPercentage: number =
      durationInDay > 0
        ? (totalHours / (durationInDay * actualOneDayWork)) * 100
        : 0;
    let individualUnit: number =
      totalUnitInPercentage === 0
        ? 0
        : totalUnitInPercentage > 0
        ? totalUnitInPercentage / resourcesLength
        : 100;
    //To check the decimal places.
    ...
    for (let index: number = 0; index < resourcesLength; index++) {
      resources[index as number][this.parent.resourceFields.unit] =
        individualUnit;
    }
  ...
  private calculateDateFromEndDate(
    ...
    const validateAsMilestone: boolean =
      parseInt(duration, 10) === 0 ? true : null;
    this.parent.setRecordValue(
      'endDate',
      this.checkEndDate(
        endDate,
  ...
  private calculateDateFromStartDate(
    ...
    if (!endDate && (isNullOrUndefined(duration) || duration === '')) {
      ...
      this.parent.setRecordValue(
        'endDate',
        this.checkEndDate(endDate, ganttData.ganttProperties),
        ganttProperties,
        true
      );
      if (
  ...
  public calculateWidth(ganttData: IGanttData, isAuto?: boolean): number {
    ...
    if (isNullOrUndefined(sDate) || isNullOrUndefined(eDate)) {
      ...
      (!ganttData.hasChildRecords || ganttProp.isAutoSchedule)
    ) {
  ...
  public calculateLeft(ganttProp: ITaskData, isAuto?: boolean): number {
    let sDate: Date = null;
    let left: number = -300;
    let startDate: Date = isAuto
      ? ganttProp.autoStartDate
      : ganttProp.startDate;
    const endDate: Date = isAuto ? ganttProp.autoEndDate : ganttProp.endDate;
    const duration: number = isAuto
      ? ganttProp.autoDuration
      : ganttProp.duration; // eslint-disable-line
    let milestone: boolean = ganttProp.isMilestone;
    const ganttRecord: IGanttData = this.parent.getTaskByUniqueID(
      ganttProp.uniqueID
    );
    if (
      ganttRecord &&
      ...
      ganttRecord.parentItem &&
      (this.parent.taskMode === 'Manual' ||
        (this.parent.taskMode === 'Custom' &&
          ganttRecord[this.parent.taskFields.manual]))
    ) {
      const parentRec: IGanttData = this.parent.getParentTask(
        ganttRecord.parentItem
      );
    ...
    if (startDate) {
      ...
      milestone =
        isNullOrUndefined(startDate) && this.parent.allowUnscheduledTasks
          ? false
          : true;
    } else {
  ...
  public calculateAutoLeft(ganttProperties: ITaskData): number {
    return this.getTaskLeft(
      ganttProperties.autoStartDate,
      ganttProperties.isMilestone
    );
  }
  /**
  ...
  public calculateBaselineLeft(ganttProperties: ITaskData): number {
    ...
    const baselineEndDate: Date = this.getDateFromFormat(
      ganttProperties.baselineEndDate
    );
  ...
  public calculateBaselineWidth(ganttProperties: ITaskData): number {
    ...
    if (
      baselineStartDate &&
      baselineEndDate &&
      baselineStartDate.getTime() !== baselineEndDate.getTime()
    ) {
  ...
  public getTaskWidth(
    ...
    const zoomOrTimeline: Object = this.parent.currentZoomingLevel
      ? this.parent.currentZoomingLevel
      : this.parent.timelineSettings;
    let countValue: number =
      zoomOrTimeline['bottomTier'] !== null
        ? zoomOrTimeline['bottomTier'].count
    ...
    if (!isNullOrUndefined(ganttData) && ganttData.durationUnit === 'hour') {
      modifiedsDate = new Date(
        modifiedsDate.getTime() + ganttData.duration * 60 * 60 * 1000
      );
    }
    if (
      (!isNullOrUndefined(ganttData) && ganttData.durationUnit === 'minute') ||
      (!isNullOrUndefined(ganttData) &&
        ganttData.durationUnit === 'day' &&
        ganttData.duration < 1)
    ) {
      modifiedsDate = new Date(
        modifiedsDate.getTime() + ganttData.duration * 60 * 1000
      );
    }
    if (this.parent.weekWorkingTime.length > 0) {
      let date: Date = new Date(startDate.getTime());
      for (
        let count: number = startDate.getDate();
        count <= endDate.getDate();
        count++
      ) {
        const day: number = date.getDay();
        let currentDay: string;
        const weekWorkingTime: WeekWorkingTimeModel[] =
        ...
        for (let i: number = 0; i < weekWorkingTime.length; i++) {
          if (
            weekWorkingTime[i as number][currentDay as string] &&
            weekWorkingTime[i as number][currentDay as string].length > 0
          ) {
            isValid = false;
            for (
              let j: number = 0;
              j < weekWorkingTime[i as number][currentDay as string].length;
              j++
            ) {
              hour =
                hour +
                weekWorkingTime[i as number][currentDay as string][j as number]
                  .to -
                weekWorkingTime[i as number][currentDay as string][j as number]
                  .from;
            }
        ...
        if (isValid) {
          for (let k: number = 0; k < this.parent.dayWorkingTime.length; k++) {
            hour =
              hour +
              this.parent.dayWorkingTime[k as number].to -
              this.parent.dayWorkingTime[k as number].from;
          }
        }
      ...
      for (let i: number = 0; i < this.parent.dayWorkingTime.length; i++) {
        hour =
          hour +
          this.parent.dayWorkingTime[i as number].to -
          this.parent.dayWorkingTime[i as number].from;
      }
    }
    ...
    if (
      (!isNullOrUndefined(ganttData) &&
        ganttData.durationUnit === 'minute' &&
        ganttData.duration < hour * 60) ||
      (!isNullOrUndefined(ganttData) &&
        ganttData.durationUnit === 'day' &&
        /^\d+\.\d+$/.test(ganttData.duration.toString()))
    ) {
      if (tierMode === 'Day') {
        if (this.parent.weekWorkingTime.length > 0) {
        ...
        if (
          Math.floor((dateDiff / (1000 * 60 * 60)) % 24) >= hour ||
          dateDiff === 0
        ) {
          isValid = true;
        }
      ...
      if (sDate.getTime() === eDate.getTime()) {
        ...
        if (isValid) {
          if (
            ganttData.durationUnit === 'day' &&
            /^\d+\.\d+$/.test(ganttData.duration.toString()) &&
            this.parent.timelineModule.bottomTier === 'Day'
          ) {
            const holidaysCount: number =
              this.parent.holidays && this.parent.holidays.length > 0
                ? this.getHolidaysCount(sDate, eDate)
                : 0;
            const weekendCount: number = !this.parent.includeWeekend
              ? this.getWeekendCount(sDate, eDate)
              : 0;
            return (
              (holidaysCount + weekendCount + ganttData.duration) *
              this.parent.perDayWidth
            );
          } else {
            return (
              (this.getTimeDifference(sDate, eDate, true) /
                (1000 * 60 * 60 * 24)) *
              this.parent.perDayWidth
            );
          }
          ...
          if (ganttData.durationUnit === 'day' && ganttData.duration < 1) {
            ...
            return (
              (this.getTimeDifference(sDate, eDate) / (1000 * 60 * 60 * hour)) *
              this.parent.perDayWidth
            );
          }
        }
      ...
      if (sDate.getTime() === eDate.getTime()) {
        ...
        if (this.parent.isInDst(sDate) || this.parent.isInDst(eDate)) {
          const timeDifference: number = this.getTimeDifference(
            sDate,
            eDate,
            true
          );
          let totalWidth: number =
            (timeDifference / (1000 * 60 * 60 * 24)) * this.parent.perDayWidth;
          const sOffset: number = sDate.getTimezoneOffset();
          const eOffset: number = eDate.getTimezoneOffset();
          const topTier: Object =
            this.parent.timelineModule.customTimelineSettings.topTier;
          ...
          return (
            (this.getTimeDifference(sDate, eDate) / (1000 * 60 * 60 * 24)) *
            this.parent.perDayWidth
          );
        }
      }
  ...
  public getDSTTransitions(
    year: number,
    timeZone: string
  ): { dstStart: Date; dstEnd: Date } {
    function findNthWeekday(
      year: number,
      month: number,
      dayOfWeek: number,
      n: number
    ...
    function convertToTimezone(date: Date, timeZone: string): Date {
      ...
      const [month, day, year, hour, minute, second] = formatted
        .match(/\d+/g)!
        .map(Number);
  ...
  public getTaskLeft(
    ...
    if (tierMode === 'Day') {
      ...
      if (this.getSecondsInDecimal(date) === dayStartTime) {
        ...
        this.getSecondsInDecimal(date) === dayEndTime &&
        this.parent.allowUnscheduledTasks
      ) {
        date.setHours(22);
        isTimeSet = true;
    ...
    if (
      ...
      ((this.parent.editModule.taskbarEditModule &&
        ...
        (this.parent.editModule.cellEditModule &&
          this.parent.editModule.cellEditModule.isCellEdit) ||
        this.parent.ganttChartModule.scrollObject['isSetScrollLeft']) &&
    ...
    if (
      ...
      leftValueForStartDate =
        this.parent.enableTimelineVirtualization &&
        this.parent.ganttChartModule.scrollObject.element.scrollLeft !== 0
          ? this.parent.ganttChartModule.scrollObject.getTimelineLeft()
          : null;
    }
    const timelineStartDate: Date =
      this.parent.enableTimelineVirtualization &&
      !isNullOrUndefined(leftValueForStartDate)
        ? new Date(
            this.parent.timelineModule['dateByLeftValue'](
              leftValueForStartDate
            ).toString()
          )
    ...
    if (timelineStartDate) {
      ...
      if (
        this.parent.isInDst(startDate) &&
        !this.parent.isInDst(timelineStartDate)
      ) {
        let newTimelineStartDate: Date;
        if (this.parent.isInDst(date)) {
          newTimelineStartDate = new Date(
            timelineStartDate.getTime() - 60 * 60 * 1000
          );
        } else {
          newTimelineStartDate = new Date(timelineStartDate.getTime());
        }
        leftValue =
          ((date.getTime() - newTimelineStartDate.getTime()) /
            (1000 * 60 * 60 * 24)) *
          this.parent.perDayWidth;
      } else {
        let width: number;
        if (
          this.parent.timelineModule.bottomTier === 'Day' &&
          this.getSecondsInDecimal(date) !== this.parent.defaultStartTime &&
          this.getSecondsInDecimal(date) !== 0 &&
          !isTimeSet
        ) {
          ...
          date.setHours(0, 0, 0, 0);
          leftValue =
            ((date.getTime() - timelineStartDate.getTime()) /
              (1000 * 60 * 60 * 24)) *
            this.parent.perDayWidth;
          if (
            this.getSecondsInDecimal(date) !== this.parent.defaultStartTime &&
          ...
          let newTimelineStartDate: Date;
          if (
            date.getTimezoneOffset() < timelineStartDate.getTimezoneOffset()
          ) {
            newTimelineStartDate = new Date(
              timelineStartDate.getTime() - 60 * 60 * 1000
            );
      ...
      if (
        hasDST &&
        this.parent.dayWorkingTime[0]['properties'].from >
          transitions['dstStart'].getHours() &&
        isBeforeOrAtDSTStart &&
        tierMode === 'Day' &&
        this.getSecondsInDecimal(date) === this.parent.defaultStartTime
      ) {
        if (leftValue % this.parent.perDayWidth !== 0) {
          const leftDifference: number =
            this.parent.perDayWidth - (leftValue % this.parent.perDayWidth);
      ...
      if (
        ...
        countValue === 1 &&
        tierMode === 'Hour' &&
        startDate >= transitions['dstStart'] &&
        isBeforeOrAtDSTStart
      ) {
  ...
  public getSplitTaskWidth(
    ...
    return (
      (this.getTimeDifference(startDate, endDate) / (1000 * 60 * 60 * 24)) *
      this.parent.perDayWidth
    );
  }
  public getSplitTaskLeft(sDate: Date, segmentTaskStartDate: Date): number {
    ...
    if (segmentTaskStartDate) {
      return (
        ((stDate.getTime() - segmentTaskStartDate.getTime()) /
          (1000 * 60 * 60 * 24)) *
        this.parent.perDayWidth
      );
    } else {
  ...
  public updateMappingData(ganttData: IGanttData, fieldName: string): void {
    ...
    const ganttProp: ITaskData = ganttData.ganttProperties;
    if (
      isNullOrUndefined(columnMapping[fieldName as string]) &&
      fieldName !== 'taskType' &&
      fieldName !== 'segments'
    ) {
      return;
    ...
    if (fieldName === 'predecessorName') {
      ...
      if (!isNullOrUndefined(resourceData)) {
        for (let i: number = 0; i < resourceData.length; i++) {
          ...
          let resName: string =
            resourceData[i as number][resourceSettings.name];
          const resourceUnit: number =
            resourceData[i as number][resourceSettings.unit];
          if (resourceUnit !== 100) {
      ...
      this.parent.setRecordValue(
        columnMapping[fieldName as string],
        resourcesName.join(','),
        ganttData
      );
      ...
      this.parent.setRecordValue(
        'taskData.' + this.parent.taskFields.segments,
        this.segmentTaskData(ganttData),
        ganttData
      );
    } else {
      this.parent.setRecordValue(
  ...
  private segmentTaskData(data: IGanttData): object[] {
    ...
    const taskData: Object[] = <Object[]>(
      extend([], [], data.taskData[taskSettings.segments], true)
    );
    for (let i: number = 0; i < segments.length; i++) {
      if (
        this.parent.isEdit ||
        (this.parent.editModule &&
          this.parent.editModule.dialogModule &&
          getValue('isEdit', this.parent.editModule.dialogModule)) ||
        (this.parent.contextMenuModule &&
          getValue('isEdit', this.parent.contextMenuModule))
      ) {
        taskData[i as number] = {};
      }
      if (!isNullOrUndefined(taskSettings.startDate)) {
        taskData[i as number][this.parent.taskFields.startDate] =
          segments[i as number].startDate;
      }
      if (!isNullOrUndefined(taskSettings.endDate)) {
        taskData[i as number][this.parent.taskFields.endDate] =
          segments[i as number].endDate;
      }
      if (!isNullOrUndefined(taskSettings.duration)) {
        taskData[i as number][this.parent.taskFields.duration] = Number(
          segments[i as number].duration
        );
      }
    }
    if (
      this.customSegmentProperties.length > 0 &&
      taskData &&
      taskData.length > 0
    ) {
      taskData.forEach((task: Object, index: number) => {
      ...
      for (let i: number = 0; i < resourceData.length; i++) {
        ...
        if (typeof preTaskResources === 'string') {
          ...
          for (let j: number = 0; j < preTaskResources.length; j++) {
            if (
              typeof preTaskResources[j as number] === 'number' ||
              typeof preTaskResources[j as number] === 'string'
            ) {
              if (
                parseInt(preTaskResources[j as number] as string, 10) ===
              ...
              preTaskResources[j as number][resourceSettings.id] ===
                resourceData[i as number][resourceSettings.id] &&
              typeof preTaskResources[j as number] !== 'number'
            ) {
              preTaskResources[j as number] = extend(
                {},
                preTaskResources[j as number],
                resourceData[i as number],
                true
              );
              isAdded = true;
              break;
      ...
      if (typeof preTaskResources !== 'string') {
        for (let k: number = 0; k < preTaskResources.length; k++) {
          resourceData.filter((resourceInfo: Object) => {
            if (
              resourceInfo[resourceSettings.id] ===
                preTaskResources[k as number][resourceSettings.id] &&
        ...
        this.parent.setRecordValue(
          'taskData.' + this.parent.taskFields.resourceInfo,
          data,
          ganttData
        );
      } else {
  ...
  private setRecordDuration(task: IGanttData, mapping: string): void {
    ...
    if (!isNullOrUndefined(duration)) {
      ...
      if (
        isNaN(durationValue) &&
        isNullOrUndefined(this.parent.taskFields.durationUnit) &&
        !isNullOrUndefined(durationValue)
      ) {
        this.parent.setRecordValue(
          'taskData.' + mapping,
          this.getDurationString(duration, durationUnit),
          task
        );
      } else {
    ...
    if (this.parent.taskFields.durationUnit) {
      task.taskData[this.parent.taskFields.durationUnit] =
        task.ganttProperties.durationUnit;
    }
  }
  public setDataSource(data: Object | Object[] | DataManager): Object[] {
    ...
    for (let i: number = 0; i < length; i++) {
      ...
      if (
        !isNullOrUndefined(
          data[i as number][this.parent.taskFields.child as string]
        )
      ) {
        this.setDataSource(
          data[i as number][this.parent.taskFields.child as string]
  ...
  private setStartDate(task: IGanttData): void {
    ...
    hierarchicalData.map((record: Object) => {
      if (
        task.ganttProperties.taskId ===
        record[this.parent.taskFields.id as string]
      ) {
        if (!isNullOrUndefined(this.parent.taskFields.startDate)) {
          task[this.parent.taskFields.startDate as string] =
            record[this.parent.taskFields.startDate as string];
        }
        if (!isNullOrUndefined(this.parent.taskFields.endDate)) {
          task[this.parent.taskFields.endDate as string] =
            record[this.parent.taskFields.endDate as string];
        }
      }
    });
  }
    ...
    if (!isNullOrUndefined(ganttData.taskData)) {
      ...
      if (dataMapping.name) {
        if (!this.parent.isLoad) {
          this.parent.setRecordValue(
            'taskData.' + dataMapping.name,
            ganttProperties.taskName,
            ganttData
          );
        }
        this.parent.setRecordValue(
      ...
      if (dataMapping.duration) {
        if (
          isNullOrUndefined(dataMapping.milestone) ||
          (!isNullOrUndefined(dataMapping.milestone) &&
      ...
      if (dataMapping.progress) {
        if (!this.parent.isLoad) {
          this.parent.setRecordValue(
            'taskData.' + dataMapping.progress,
            ganttProperties.progress,
            ganttData
          );
        }
        this.parent.setRecordValue(
          dataMapping.progress,
          ganttProperties.progress,
          ganttData
        );
      }
      ...
      if (dataMapping.baselineEndDate) {
        this.setRecordDate(
          ganttData,
          ganttProperties.baselineEndDate,
          dataMapping.baselineEndDate
        );
      }
      if (dataMapping.notes) {
        if (!this.parent.isLoad) {
          this.parent.setRecordValue(
            'taskData.' + dataMapping.notes,
            ganttProperties.notes,
            ganttData
          );
        }
        this.parent.setRecordValue(
      ...
      if (dataMapping.cssClass) {
        if (!this.parent.isLoad) {
          this.parent.setRecordValue(
            'taskData.' + dataMapping.cssClass,
            ganttProperties.cssClass,
            ganttData
          );
        }
        this.parent.setRecordValue(
          dataMapping.cssClass,
          ganttProperties.cssClass,
          ganttData
        );
      }
      if (dataMapping.indicators) {
        if (!this.parent.isLoad) {
          this.parent.setRecordValue(
            'taskData.' + dataMapping.indicators,
            ganttProperties.indicators,
            ganttData
          );
        }
        this.parent.setRecordValue(
          dataMapping.indicators,
          ganttProperties.indicators,
          ganttData
        );
      }
      if (dataMapping.parentID) {
        const parentID: string =
          this.parent.viewType === 'ProjectView'
            ? ganttProperties.parentId
            : data[dataMapping.parentID];
        this.parent.setRecordValue(
          'taskData.' + dataMapping.parentID,
          parentID,
          ganttData
        );
        this.parent.setRecordValue(
      ...
      if (dataMapping.type) {
        if (!this.parent.isLoad) {
          this.parent.setRecordValue(
            'taskData.' + dataMapping.type,
            ganttProperties.taskType,
            ganttData
          );
        }
        this.parent.setRecordValue(
  ...
  public setResourceInfo(data: Object): Object[] {
    ...
    if (this.parent.isLoad) {
      ...
      resourceIdCollection =
        data['taskData'] &&
        this.parent.taskFields.resourceInfo in data['taskData']
    ...
    const resources: Object[] = [];
    if (
      typeof resourceIdCollection === 'string' &&
      resourceIdCollection !== ''
    ) {
      const resource: Object[] = [];
      const resourceIds: string =
        data[this.parent.taskFields.resourceInfo].split(',');
      if (resourceIds) {
        resourceData.forEach((resourceInfo: Object) => {
          if (
            resourceIds.includes(
              resourceInfo[this.parent.resourceFields.name as string]
            )
          ) {
        ...
        if (
          !isNullOrUndefined(resourceUnitMapping) &&
          !isNullOrUndefined(
            resourceIdCollection[resourceUnitMapping as string]
          )
        ) {
          ganttDataResource[resourceUnitMapping as string] =
            resourceIdCollection[resourceUnitMapping as string];
        }
        if (
          !isNullOrUndefined(resourceGroup) &&
          !isNullOrUndefined(resourceIdCollection[resourceGroup as string])
        ) {
          ganttDataResource[resourceGroup as string] =
            resourceIdCollection[resourceGroup as string];
        }
      }
    } else {
      for (
        let count: number = 0;
        count < resourceIdCollection.length;
        count++
      ) {
        const resource: Object[] = resourceData.filter(
          (resourceInfo: Object) => {
            if (
              typeof resourceIdCollection[count as number] === 'object' &&
              resourceIdCollection[count as number][
                resourceIDMapping as string
              ] === resourceInfo[resourceIDMapping as string]
            ) {
              return true;
            } else {
              return (
        ...
        if (
          !isNullOrUndefined(resourceUnitMapping) &&
          !isNullOrUndefined(
            resourceIdCollection[count as number][resourceUnitMapping as string]
          )
        ) {
          ganttDataResource[resourceUnitMapping as string] =
            resourceIdCollection[count as number][
              resourceUnitMapping as string
            ];
        }
        if (
          !isNullOrUndefined(resourceGroup) &&
          !isNullOrUndefined(
            resourceIdCollection[count as number][resourceGroup as string]
          )
        ) {
          ganttDataResource[resourceGroup as string] =
            resourceIdCollection[count as number][resourceGroup as string];
  ...
  public updateResourceUnit(resourceData: Object[]): void {
    ...
    if (!isNullOrUndefined(resourceUnit)) {
      ...
      for (index = 0; index < length; index++) {
        if (
          isNullOrUndefined(
            resourceData[index as number][resourceUnit as string]
          )
        ) {
  ...
  public updateResourceName(data: IGanttData): void {
    ...
    if (resourceInfo && resourceInfo.length > 0) {
      ...
      if (
        typeof data.taskData[this.parent.taskFields.resourceInfo] === 'string'
      ) {
        ...
        const taskResources: Object = extend(
          [],
          [],
          data.taskData[this.parent.taskFields.resourceInfo],
          true
        ...
        for (let i: number = 0; i < resourceLength; i++) {
          ...
          const resourceUnit: number =
            resource[this.parent.resourceFields.unit];
          if (resourceUnit !== 100) {
            resName += '[' + resourceUnit + '%' + ']';
          }
          ...
          if (data.taskData) {
            ...
            if (
              taskResources[i as number] &&
              typeof (taskResources[parseInt(i.toString(), 10)] === 'object')
            ) {
              data.taskData[mapping as string].push(taskResources[i as number]);
            } else {
              data.taskData[mapping as string].push(
                resource[this.parent.resourceFields.id]
              );
            }
          }
        }
  ...
  public updateDurationValue(
    ...
    if (
      ...
      this.parent.setRecordValue(
        'duration',
        getValue('duration', tempDuration),
        ganttProperties,
        true
      );
    }
    if (!isNullOrUndefined(getValue('durationUnit', tempDuration))) {
      this.parent.setRecordValue(
        'durationUnit',
        getValue('durationUnit', tempDuration),
        ganttProperties,
        true
      );
    }
  ...
  public updateOverlappingValues(resourceTask: IGanttData): void {
    ...
    for (let count: number = 1; count < tasks.length; count++) {
      ...
      for (let index: number = 0; index < count; index++) {
        const tStartDate: Date =
          tasks[index as number].ganttProperties.startDate;
        ...
        if (
          ...
          if (
            (tStartDate.getTime() > cStartDate.getTime() &&
              tStartDate.getTime() < cEndDate.getTime() &&
              tEndDate.getTime() > cStartDate.getTime() &&
              tEndDate.getTime() >= cEndDate.getTime()) ||
            (cStartDate.getTime() === tStartDate.getTime() &&
              cEndDate.getTime() <= tEndDate.getTime())
          ) {
    ...
    this.parent.setRecordValue(
      'workTimelineRanges',
      this.mergeRangeCollections(ranges, true),
      resourceTask.ganttProperties,
      true
    );
    this.calculateRangeLeftWidth(
      resourceTask.ganttProperties.workTimelineRanges
    );
  }
  /**
  ...
  public calculateRangeLeftWidth(ranges: IWorkTimelineRanges[]): void {
    for (let count: number = 0; count < ranges.length; count++) {
      ranges[count as number].left = this.getTaskLeft(
        ranges[count as number].from,
        false
      );
      ranges[count as number].width = this.getTaskWidth(
        ranges[count as number].from,
        ranges[count as number].to
      );
    }
  }
  /**
   ...
   * @private
   */
  public mergeRangeCollections(
    ranges: IWorkTimelineRanges[],
    isSplit?: boolean
  ): IWorkTimelineRanges[] {
    ...
    let tStartDate: Date;
    let tEndDate: Date;
    const sortedRanges: IWorkTimelineRanges[] = new DataManager(
      ranges
    ).executeLocal(
      new Query().sortBy(this.parent.taskFields.startDate, 'Ascending')
    );
    for (let i: number = 0; i < sortedRanges.length; i++) {
      ...
      /* eslint-disable-next-line */
        (cStartDate.getTime() <= tStartDate.getTime() &&
          cEndDate.getTime() >= tStartDate.getTime() &&
          cEndDate.getTime() < tEndDate.getTime()) ||
        (cEndDate.getTime() < tStartDate.getTime() &&
          this.checkStartDate(cEndDate).getTime() === tStartDate.getTime())
      ) {
  ...
  public setSortedChildTasks(resourceTask: IGanttData): IGanttData[] {
    ...
    sortedRecords = new DataManager(resourceTask.childRecords).executeLocal(
      new Query().sortBy(this.parent.taskFields.startDate, 'Ascending')
    );
  ...
  private splitRangeCollection(
    rangeCollection: IWorkTimelineRanges[],
    fromField?: string,
    toField?: string
  ): IWorkTimelineRanges[] {
    ...
    if (unit === 'Week' || unit === 'Month' || unit === 'Year') {
      ...
      if (
        this.parent.workingTimeRanges[0].from === 0 &&
        this.parent.workingTimeRanges[0].to === 86400
      ) {
        splitArray = this.getRangeWithWeek(rangeCollection, fromField, toField);
      } else {
  ...
  private getRangeWithDay(
    ...
    for (let i: number = 0; i < ranges.length; i++) {
      splitArray.push(
        ...this.splitRangeForDayMode(
          ranges[parseInt(i.toString(), 10)],
          fromField,
          toField
        )
      );
  ...
  private splitRangeForDayMode(
    ...
    if (tempStart.getTime() < end.getTime()) {
      do {
        ...
        const sHour: number =
          this.parent.dataOperation.getSecondsInDecimal(tempStart);
        let startRangeIndex: number = -1;
        for (let i: number = 0; i < this.parent.workingTimeRanges.length; i++) {
          const val: IWorkingTimeRange =
            this.parent.workingTimeRanges[i as number];
        ...
        if (startRangeIndex !== -1) {
          nextAvailDuration = Math.round(
            this.parent.workingTimeRanges[startRangeIndex as number].to - sHour
          );
  ...
  private getRangeWithWeek(
    ...
    for (let i: number = 0; i < ranges.length; i++) {
      // eslint-disable-next-line
      splitArray.push.apply(
        splitArray,
        this.splitRangeForWeekMode(ranges[i as number], fromField, toField)
      );
    }
    return splitArray;
  }
    ...
    if (tempStart.getTime() < end.getTime()) {
      do {
        if (this.parent.dataOperation.isOnHolidayOrWeekEnd(tempStart, null)) {
          ...
          const isStartNotHolidayOrWeekend: boolean =
            !this.parent.dataOperation.isOnHolidayOrWeekEnd(start, null);
          const isTempEndDateNotHolidayOrWeekend: boolean =
            !this.parent.dataOperation.isOnHolidayOrWeekEnd(tempEndDate, null);
          const isDifferentDates: boolean =
            start.getTime() !== tempEndDate.getTime();
          if (
            isStartNotHolidayOrWeekend &&
  ...
  private updateTaskLeftWidth(data: IGanttData): void {
    ...
    this.parent.setRecordValue(
      'baselineLeft',
      this.calculateBaselineLeft(task),
      task,
      true
    );
    this.parent.setRecordValue(
      'baselineWidth',
      this.calculateBaselineWidth(task),
      task,
      true
    );
    ...
    if (
      parentItem &&
      childData.indexOf(data) === childData.length - 1 &&
      !data.hasChildRecords &&
      this.parent.enableValidation
    ) {
      if (
  ...
  public reUpdateGanttDataPosition(): void {
    ...
    if (
      ...
      flatData =
        this.parent.pdfExportModule.helper.beforeSinglePageExport[
          'cloneFlatData'
        ];
    } else {
  ...
  public updateWidthLeft(data: IGanttData): void {
    ...
    if (
      !isNullOrUndefined(ganttRecord.segments) &&
      ganttRecord.segments.length > 0
    ) {
      ...
      for (let i: number = 0; i < segments.length; i++) {
        ...
        if (
          i === 0 &&
          !isNullOrUndefined(ganttRecord.startDate) &&
          !isNullOrUndefined(segment.startDate) &&
          !isNullOrUndefined(segment.startDate.getTime()) &&
          ...
          const endDate: Date = this.parent.dataOperation.getEndDate(
            segment.startDate,
            segment.duration,
            ganttRecord.durationUnit,
            ganttRecord,
            false
          );
          segment.endDate = this.parent.dataOperation.checkEndDate(
            endDate,
            ganttRecord,
            false
          );
        ...
        segment.width = this.getSplitTaskWidth(
          segment.startDate,
          segment.duration,
          data
        );
      ...
      let setProgress: number = this.parent.dataOperation.getProgressWidth(
        totalSegmentsProgressWidth,
      ...
      for (let i: number = 0; i < segments.length; i++) {
        if (isValid) {
          ...
          if (segments[i as number].showProgress) {
            progressResizer =
              progressResizer +
              segments[i as number].left +
              segments[i as number].progressWidth;
          }
        }
      }
      this.parent.setRecordValue(
    ...
    this.parent.setRecordValue(
      'width',
      this.parent.dataOperation.calculateWidth(data),
      ganttRecord,
      true
    );
    this.parent.setRecordValue(
      'left',
      this.parent.dataOperation.calculateLeft(ganttRecord),
      ganttRecord,
      true
    );
    if (
      ...
      this.parent.setRecordValue(
        ...
        this.parent.dataOperation.getProgressWidth(
          ganttRecord.isAutoSchedule || !data.hasChildRecords
            ? ganttRecord.width
            : ganttRecord.autoWidth,
          ganttRecord.progress
        ),
        ganttRecord,
        true
  ...
  public updateAutoWidthLeft(data: IGanttData): void {
    ...
    this.parent.setRecordValue(
      'autoWidth',
      this.calculateWidth(data, true),
      ganttRecord,
      true
    );
    this.parent.setRecordValue(
      'autoLeft',
      this.calculateLeft(ganttRecord, true),
      ganttRecord,
      true
    );
    this.parent.setRecordValue(
  ...
  public getParentProgress(childGanttRecord: IGanttData): Object {
    ...
    if (this.parent.weekWorkingTime.length > 0) {
      totSeconds = this.parent['getSecondsPerDay'](
        childGanttRecord.ganttProperties.startDate
          ? childGanttRecord.ganttProperties.startDate
          : childGanttRecord.ganttProperties.endDate
      );
    } else {
      totSeconds = this.parent.secondsPerDay;
    }
    switch (childGanttRecord.ganttProperties.durationUnit) {
      case 'hour':
        durationInDay =
          childGanttRecord.ganttProperties.duration / (totSeconds / 3600);
        break;
      case 'minute':
        durationInDay =
          childGanttRecord.ganttProperties.duration / (totSeconds / 60);
        break;
      default:
    ...
    if (childGanttRecord.hasChildRecords) {
      setValue(
        'totalProgress',
        childGanttRecord.ganttProperties.totalProgress,
        progressValues
      );
      setValue(
        'totalDuration',
        childGanttRecord.ganttProperties.totalDuration
          ? childGanttRecord.ganttProperties.totalDuration
          : 0,
        progressValues
      );
      ...
      setValue(
        'totalProgress',
        childGanttRecord.ganttProperties.progress * durationInDay,
        progressValues
      );
  ...
  private isUnscheduledTask(
    ...
    if (
      count === 1 ||
      (count === 0 &&
        !isNullOrUndefined(parantData) &&
        parantData.hasChildRecords)
    ) {
  ...
  private isFromManual(childData: IGanttData): boolean {
    if (
      ...
      this.parent['oldRecords'].length > 0 &&
      isNullOrUndefined(childData.ganttProperties.startDate) &&
      isNullOrUndefined(childData.ganttProperties.endDate) &&
      !isNullOrUndefined(childData.ganttProperties.duration) &&
      !childData.hasChildRecords &&
      childData.parentItem &&
      (this.parent.taskMode === 'Manual' ||
        (this.parent.taskMode === 'Custom' &&
          childData[this.parent.taskFields.manual]))
    ) {
      return true;
    } else {
      return false;
   ...
   * @private
   */
  public updateParentItems(
    cloneParent: IParent | IGanttData,
    isParent?: boolean
  ): void {
    let parentData: IGanttData = isParent
      ? cloneParent
      : this.parent.getParentTask(cloneParent);
    parentData = parentData ? parentData : cloneParent;
    let deleteUpdate: boolean = false;
    const ganttProp: ITaskData = !isNullOrUndefined(parentData)
      ? parentData.ganttProperties
      : null;
    /* eslint-disable-next-line */
    const [isParentUnschecule, propertyWithValue]: [boolean, string] =
      this.isUnscheduledTask(ganttProp, parentData);
    if (
      (this.parent.autoCalculateDateScheduling &&
        !(
          this.parent.isLoad &&
      ...
      if (
        parentData &&
        parentData.childRecords &&
        parentData.childRecords.length > 0
      ) {
        const previousStartDate: Date = ganttProp.isAutoSchedule
          ? ganttProp.startDate
          : ganttProp.autoStartDate;
        const previousEndDate: Date = ganttProp.isAutoSchedule
          ? ganttProp.endDate
        ...
        childRecords.some((childRecord: IGanttData) => {
          const [isUnscheduled, propertyWithValue]: [boolean, string] =
            this.isUnscheduledTask(childRecord['ganttProperties']);
        ...
        const isChildBoth: boolean =
          countOfScheduled > 0 && countOfUnScheduled > 0 ? true : undefined;
        for (let count: number = 0; count < childLength; count++) {
          childData =
            this.parent.loadChildOnDemand &&
            this.parent.taskFields.hasChildMapping
              ? this.parent.currentViewData.filter(
                  (item: IGanttData) =>
                    item.ganttProperties.taskId ===
                    childRecords[count as number][this.parent.taskFields.id]
                )[0]
          ...
          let endDate: Date;
          const [isUnscheduled, propertyWithValue]: [boolean, string] =
            this.isUnscheduledTask(childData.ganttProperties);
          let parentRec: IGanttData;
          if (this.isFromManual(childData)) {
            const ganttRec: IGanttData = this.parent['oldRecords'].filter(
              (record: IGanttData) =>
                record.ganttProperties.uniqueID ===
                childData.ganttProperties.uniqueID
            )[0];
            if (ganttRec) {
          ...
          const prop: ITaskData = parentRec
            ? parentRec.ganttProperties
            : ganttProp;
          startDate =
            isUnscheduled &&
            propertyWithValue !== 'startDate' &&
            propertyWithValue !== 'endDate' &&
            !isParentUnschecule
              ? ganttProp.startDate
              : this.getValidStartDate(rec.ganttProperties);
          if (
            parentData.hasChildRecords &&
            !prop.isAutoSchedule &&
          ...
          endDate =
            ...
            !isParentUnschecule
              ? ganttProp.endDate
              : this.getValidEndDate(rec.ganttProperties);
          if (
            parentData.hasChildRecords &&
            !prop.isAutoSchedule &&
          ...
          if (
            !isNullOrUndefined(endDate) &&
            maxEndDate &&
            this.compareDates(endDate, maxEndDate) === 1
          ) {
            maxEndDate = this.getDateFromFormat(endDate);
          }
          if (
            !isNullOrUndefined(startDate) &&
            minStartDate &&
            this.compareDates(startDate, minStartDate) === -1
          ) {
            minStartDate = this.getDateFromFormat(startDate);
          }
          const condition: boolean =
            !childData.ganttProperties.isMilestone &&
            isScheduledTask(childData.ganttProperties)
              ? true
              : false;
          if (condition) {
        ...
        if (!deleteUpdate) {
          ...
          const parentProgress: number =
            taskCount > 0 && totalDuration > 0
              ? Number((totalProgress / totalDuration).toFixed(2))
              : 0;
          const parentProp: ITaskData = parentData.ganttProperties;
          const milestone: boolean =
            ...
            minStartDate &&
            maxEndDate &&
            minStartDate.getTime() === maxEndDate.getTime()
              ? true
              : false;
          if (this.compareDates(previousStartDate, minStartDate) !== 0) {
            ...
            if (
              (!isNullOrUndefined(ganttProp.autoDuration)
                ? ganttProp.autoDuration === 0
                : ganttProp.duration === 0) &&
              parentData['isManual'] &&
              milestone &&
              parentData.hasChildRecords &&
              parentData.ganttProperties.isAutoSchedule &&
              this.parent.editModule.taskbarEditModule.taskbarEditedArgs
                .action !== 'TaskbarEditing'
            ) {
              this.parent.setRecordValue(
                'startDate',
                minStartDate,
          ...
          if (!isNullOrUndefined(this.parent.taskFields.milestone)) {
            ...
            this.parent.setRecordValue(
              'taskData.' + this.parent.taskFields.milestone,
              milestone,
              parentData,
              true
            );
          ...
          if (parentProp.isAutoSchedule) {
            if (this.isFromManual(childData)) {
              if (parentData.childRecords.length === 1) {
                parentData.ganttProperties.duration =
                  childData.ganttProperties.duration;
              }
          ...
          parentWork = this.parent.isOnEdit
            ? parentWork
      ...
      this.parent.setRecordValue(
        'endDate',
        parentData.taskData[this.parent.taskFields.endDate],
        parentData.ganttProperties,
        true
      );
    }
    if (
      deleteUpdate &&
      parentData.childRecords.length === 1 &&
      parentData.ganttProperties.duration === 0
    ) {
      this.parent.setRecordValue(
        'isMilestone',
        true,
    ...
    const parentItem: IGanttData = !isNullOrUndefined(parentData)
      ? (this.parent.getParentTask(parentData.parentItem) as IGanttData)
      : null;
    if (parentItem) {
      if (
        (this.parent.autoCalculateDateScheduling &&
