import { isNullOrUndefined, getValue, setValue } from '@hoyui/base';
import {
  IGanttData,
  IWorkingTimeRange,
  ITaskData,
...
export class DateProcessor {
    ...
    return (
      (!isNullOrUndefined(ganttProp) &&
        ...
        (!this.parent.includeWeekend ||
          this.parent.totalHolidayDates.length > 0)) ||
      (isNullOrUndefined(ganttProp) &&
        (!this.parent.includeWeekend ||
          this.parent.totalHolidayDates.length > 0))
    );
  ...
  public checkStartDate(
    ...
    let dayStartTime: number =
      this.parent['getCurrentDayStartTime'](currentDay);
    let dayEndTime: number = this.parent['getCurrentDayEndTime'](currentDay);
    let cloneStartDate: Date = new Date(date.getTime());
    const hour: number = this.getSecondsInDecimal(cloneStartDate);
    validateAsMilestone = isNullOrUndefined(validateAsMilestone)
      ? !isNullOrUndefined(ganttProp)
    ...
    if (hour < dayStartTime && (!validateAsMilestone || isLoad)) {
      ...
      (hour === dayEndTime && (!ganttProp || !validateAsMilestone)) ||
      hour > dayEndTime
    ) {
    ...
    if (
      ...
      do {
        ...
        if (this.isValidateNonWorkDays(ganttProp)) {
          ...
          if (ganttProp) {
            dayEndTime = this.parent['getCurrentDayEndTime'](
              ganttProp.endDate
                ? ganttProp.isAutoSchedule
                  ? ganttProp.endDate
          ...
          let startTime: number =
            !validateAsMilestone || isLoad ? dayStartTime : dayEndTime;
          if (!this.parent.includeWeekend) {
            ...
            if (
              tempDate.getTime() !== cloneStartDate.getTime() &&
              !validateAsMilestone
            ) {
          ...
          for (let count: number = 0; count < holidayLength; count++) {
            const holidayFrom: Date = this.getDateFromFormat(
              new Date(this.parent.totalHolidayDates[count as number])
            );
            const holidayTo: Date = new Date(holidayFrom.getTime());
            holidayFrom.setHours(0, 0, 0, 0);
            holidayTo.setHours(23, 59, 59, 59);
            if (
              cloneStartDate.getTime() >= holidayFrom.getTime() &&
              cloneStartDate.getTime() < holidayTo.getTime()
            ) {
              cloneStartDate.setDate(cloneStartDate.getDate() + 1);
              startTime = this.parent['getCurrentDayStartTime'](cloneStartDate);
              this.setTime(startTime, cloneStartDate);
  ...
  public checkEndDate(
    ...
    if (hour > dayEndTime) {
      ...
      if (
        ...
        !this.parent.editModule['editedRecord'][taskfields.startDate] &&
        this.parent.editModule['editedRecord'][taskfields.endDate]
      ) {
      ...
      for (
        let index: number = 0;
        index < this.parent.workingTimeRanges.length;
        index++
      ) {
        const value: IWorkingTimeRange =
          this.parent.workingTimeRanges[index as number];
        if (
          hour > value.to &&
          this.parent.workingTimeRanges[index + 1] &&
          hour <= this.parent.workingTimeRanges[index + 1].from
        ) {
          this.setTime(
            this.parent.workingTimeRanges[index as number].to,
            cloneEndDate
          );
          break;
        }
      ...
      do {
        ...
        if (this.isValidateNonWorkDays(ganttProp)) {
          ...
          for (let count: number = 0; count < holidayLength; count++) {
            ...
            holidayTo.setHours(23, 59, 59, 59);
            if (
              (cloneEndDate.getTime() >= holidayFrom.getTime() &&
                cloneEndDate.getTime() < holidayTo.getTime()) ||
              (tempHoliday.getTime() >= holidayFrom.getTime() &&
                tempHoliday.getTime() < holidayTo.getTime())
            ) {
              cloneEndDate.setDate(cloneEndDate.getDate() - 1);
              dayEndTime = this.parent['getCurrentDayEndTime'](cloneEndDate);
              if (
  ...
  public checkBaselineStartDate(date: Date, ganttProp?: ITaskData): Date {
    if (isNullOrUndefined(date)) {
      ...
      const hour: number = this.getSecondsInDecimal(cloneDate);
      if (hour < dayStartTime) {
        ...
        for (let i: number = 0; i < this.parent.workingTimeRanges.length; i++) {
          ...
          if (
            hour > value.to &&
            this.parent.workingTimeRanges[i + 1] &&
  ...
  public checkBaselineEndDate(date: Date, ganttProp?: ITaskData): Date {
    if (isNullOrUndefined(date)) {
      ...
      const dayStartTime: number = this.parent['getCurrentDayStartTime'](
        ganttProp
          ? ganttProp.startDate
            ? ganttProp.isAutoSchedule
              ? ganttProp.startDate
      ...
      if (hour > dayEndTime) {
        ...
        for (let i: number = 0; i < this.parent.workingTimeRanges.length; i++) {
          ...
          if (
            ...
            this.setTime(
              this.parent.workingTimeRanges[i as number].to,
              cloneDate
            );
            break;
          }
  ...
  public calculateStartDate(ganttData: IGanttData): void {
    ...
    if (
      ...
      tempStartDate = this.getStartDate(
        ganttProp.endDate,
        ganttProp.duration,
      ...
      if (
        ...
        if (this.compareDates(ganttProp.startDate, ganttProp.endDate) === 1) {
          this.parent.setRecordValue(
            'startDate',
            new Date(ganttProp.endDate.getTime()),
            ganttProp,
            true
          );
          dayStartTime = this.parent['getCurrentDayStartTime'](
            ganttProp.isAutoSchedule
              ? ganttProp.autoStartDate
              : ganttProp.startDate
          );
          dayEndTime = this.parent['getCurrentDayEndTime'](
            ganttProp.isAutoSchedule ? ganttProp.autoEndDate : ganttProp.endDate
          );
      ...
      if (!isNullOrUndefined(ganttProp.duration)) {
        const duration: number =
          ...
          ganttProp.segments.length > 1
            ? this.totalDuration(ganttProp.segments)
            : ganttProp.duration;
        tempEndDate = this.getEndDate(
          ganttProp.startDate,
          duration,
          ganttProp.durationUnit,
          ganttProp,
  ...
  public totalDuration(segments: ITaskSegment[]): number {
    ...
    for (let i: number = 0; i < segments.length; i++) {
      duration +=
        segments[i as number].duration + segments[i as number].offsetDuration;
    }
    return duration;
  }
  /**
  ...
  public calculateDuration(ganttData: IGanttData): void {
    ...
    if (
      !isNullOrUndefined(ganttProperties.segments) &&
      ganttProperties.segments.length > 0 &&
      !isNullOrUndefined(this.parent.editModule.taskbarEditModule)
    ) {
      tDuration = this.parent.editModule.taskbarEditModule.sumOfDuration(
        ganttProperties.segments
      );
    } else {
      if (
        ...
        ganttProperties.startDate.getTime() ===
          ganttProperties.endDate.getTime() &&
    ...
    if (
      ...
      if (!isNullOrUndefined(col.edit) && !isNullOrUndefined(col.edit.read)) {
        ...
        if (!isNullOrUndefined(dialog)) {
          const textBox: TextBox = <TextBox>(
            (<EJ2Instance>(
              dialog.querySelector('#' + this.parent.element.id + 'Duration')
            )).ej2_instances[0]
          );
          if (
            !isNullOrUndefined(textBox) &&
  ...
  private getNonworkingTime(
    ...
    isCheckTimeZone = isNullOrUndefined(isCheckTimeZone)
      ? true
      : isCheckTimeZone;
    const weekendCount: number =
      ...
      isAutoSchedule
        ...
        : 0;
    const totalHours: number = this.getNumberOfSeconds(
      sDate,
      eDate,
      isCheckTimeZone
    );
    const holidaysCount: number =
      ...
      !(
        ...
        : 0;
    const totWorkDays: number =
      (totalHours - weekendCount * 86400 - holidaysCount * 86400) / 86400; // working days between two dates
    const nonWorkHours: number = this.getNonWorkingSecondsOnDate(
      sDate,
      eDate,
      isAutoSchedule
    );
    const totalNonWorkTime: number =
      (this.parent.weekWorkingTime.length > 0
        ? this.nonWorkingSeconds(sDate, eDate, isAutoSchedule, totWorkDays)
        : totWorkDays * (86400 - this.parent.secondsPerDay)) +
      weekendCount * 86400 +
      holidaysCount * 86400 +
      nonWorkHours;
  ...
  private nonWorkingSeconds(
    ...
    let count: number = 0;
    if (fromDuration) {
      const dayStartTime: number =
        this.parent['getCurrentDayStartTime'](newStartDate);
      const dayEndTime: number =
        this.parent['getCurrentDayEndTime'](newStartDate);
      if (
        !(
          newStartDate.getHours() < dayEndTime / 3600 &&
          newStartDate.getHours() >= dayStartTime / 3600
        )
      ) {
    ...
    if (workDays > 0 || isNullOrUndefined(workDays)) {
      while (
        ...
        if (newStartDate.getTime() <= newEndDate.getTime()) {
          count++;
          const currentDaySeconds: number =
            this.parent['getSecondsPerDay'](newStartDate);
          if (fromDuration) {
  ...
  public getDuration(
    ...
    const timeDiff: number =
      this.getTimeDifference(startDate, endDate, isCheckTimeZone) / 1000;
    const nonWorkHours: number = this.getNonworkingTime(
      startDate,
      endDate,
      isAutoSchedule,
      isCheckTimeZone
    ...
    if (this.parent.weekWorkingTime.length > 0) {
      totSeconds = this.nonWorkingSeconds(
        startDate,
        endDate,
        isAutoSchedule,
        undefined,
    ...
    if (
      isMilestone &&
      this.parent.getFormatedDate(startDate) ===
        this.parent.getFormatedDate(endDate)
    ) {
      durationValue = 0;
    } else {
  ...
  public getEndDate(
    ...
    if (
      ...
      secondDuration = this.calculateSecondDuration(
        duration,
        sDate,
        secondDuration,
        startDate,
      ...
      secondDuration = this.getDurationAsSeconds(
        duration,
        durationUnit,
        startDate
      );
    ...
    while (secondDuration > 0) {
      ...
      nonWork = this.getNonworkingTime(
        tempStart,
        endDate,
        ganttProp.isAutoSchedule,
        true
  ...
  private calculateSecondDuration(
    ...
    if (duration < 1) {
      ...
      if (
        !(
          sDate.getHours() < dayEndTime / 3600 &&
          sDate.getHours() > dayStartTime / 3600
        ) &&
        this.fromSegments
      ) {
        if (fromEndDate) {
      ...
      while (durationValue > 0) {
        ...
        if (!this.parent.includeWeekend) {
          sDate = fromEndDate
            ? this.getNextWorkingDay(sDate)
            : this.getPreviousWorkingDay(sDate);
        }
        let totSeconds: number = this.parent['getSecondsPerDay'](sDate);
        let num: number = 0;
        if (
          this.getSecondsInDecimal(startDate) !==
            this.parent['getStartTime'](startDate) &&
  ...
  protected getProjectStartDate(ganttProp: ITaskData, isLoad?: boolean): Date {
    if (!isNullOrUndefined(this.parent.cloneProjectStartDate)) {
      if (typeof this.parent.cloneProjectStartDate === 'string') {
        this.parent.cloneProjectStartDate = this.getDateFromFormat(
          this.parent.cloneProjectStartDate
        );
      }
      const cloneStartDate: Date = this.checkStartDate(
        this.parent.cloneProjectStartDate
      );
      this.parent.cloneProjectStartDate = cloneStartDate;
      return new Date(cloneStartDate.getTime());
    } else if (!isNullOrUndefined(this.parent.projectStartDate)) {
      const cloneStartDate: Date = this.getDateFromFormat(
      ...
      for (let index: number = 1; index < flatData.length; index++) {
        const startDate: Date =
          flatData[index as number].ganttProperties.startDate;
        if (
      ...
      this.parent.cloneProjectStartDate = this.checkStartDate(
        minStartDate,
        ganttProp
      );
    } else {
  ...
  public getValidStartDate(ganttProp: ITaskData, isAuto?: boolean): Date {
    let sDate: Date = null;
    const startDate: Date = isAuto
      ? ganttProp.autoStartDate
      : ganttProp.startDate;
    const endDate: Date = isAuto ? ganttProp.autoEndDate : ganttProp.endDate;
    const duration: number =
      !ganttProp.isAutoSchedule && ganttProp.autoDuration
    ...
    if (isNullOrUndefined(startDate)) {
      if (!isNullOrUndefined(endDate)) {
        ...
        const dayStartTime: number =
          this.parent['getCurrentDayStartTime'](sDate);
        this.setTime(dayStartTime, sDate);
      } else if (!isNullOrUndefined(duration)) {
        const ganttTask: IGanttData = this.parent.getTaskByUniqueID(
          ganttProp.uniqueID
        );
        if (
          this.parent.allowUnscheduledTasks &&
          ganttTask &&
          ganttTask.parentItem &&
          isNullOrUndefined(startDate) &&
          ...
          sDate =
            !parentTask || !parentTask.ganttProperties.startDate
              ? this.parent.cloneProjectStartDate
  ...
  private getWorkingTime(
    day: string,
    currentRange: DayWorkingTimeModel,
    startDate: Date,
    totalSeconds: number,
    count: number,
    ...
    if (
      !isNullOrUndefined(currentRange.from) &&
      !isNullOrUndefined(currentRange.to)
    ) {
      ...
      if (
        count === this[day.toLowerCase() + 'TimeRangeLength'] - 1 ||
        day === ''
      ) {
      ...
      if (count > 0) {
        if (day === '') {
          nonWorkingHours.push(
            nonWorkingHours[nonWorkingHours.length - 1] +
              sdSeconds -
          ...
          this.parent[day.toLowerCase() + 'NonWorkingHours'].push(
            this.parent[day.toLowerCase() + 'NonWorkingHours'][
              this.parent[day.toLowerCase() + 'NonWorkingHours'].length - 1
            ] +
              sdSeconds -
              this.parent[day.toLowerCase() + 'NonWorkingTimeRanges'][count - 1]
                .to
          );
          if (
            this.parent[day.toLowerCase() + 'WorkingTimeRanges'][count - 1].to <
            sdSeconds
          ) {
            this.parent[day.toLowerCase() + 'NonWorkingTimeRanges'].push({
              from: this.parent[day.toLowerCase() + 'WorkingTimeRanges'][
                count - 1
              ].to,
              to: sdSeconds,
              isWorking: false,
              interval:
                sdSeconds -
                this.parent[day.toLowerCase() + 'WorkingTimeRanges'][count - 1]
                  .to,
            });
          }
        }
  ...
  private calculateWeekWorkTime(
    day: string,
    currentDay: DayWorkingTimeModel[],
    startDate: Date,
    totalSeconds: number,
    nonWorkingHours: number[],
    workingTimeRanges: IWorkingTimeRange[],
  ...
  public getSecondsPerDay(): number {
    ...
    for (const day of [
      ...
      this[`${day.toLowerCase()}TimeRangeLength`] =
        this.parent.dayWorkingTime.length;
    }
    if (this.parent.weekWorkingTime.length > 0) {
      for (let i: number = 0; i < this.parent.weekWorkingTime.length; i++) {
        this[
          this.parent.weekWorkingTime[i as number].dayOfWeek.toLowerCase() +
            'TimeRangeLength'
      ...
      for (const weekDay of weekWorkingTime) {
        if (weekDay.timeRange && weekDay.timeRange.length > 0) {
          const seconds: number = this.calculateWeekWorkTime(
            weekDay.dayOfWeek,
            weekDay.timeRange,
            startDate,
            totalSeconds,
      ...
      for (const day of [
        ...
        const dayDefaultStartTime: number =
          this.parent[`${day.toLowerCase()}DefaultStartTime`];
  ...
  public getDurationValue(
    ...
    if (typeof value === 'string') {
      ...
      if (values && values.length <= 2) {
        ...
        const unit: string = values[1]
          ? values[1].toString().trim().toLowerCase()
          : null;
        if (
          getValue('minute', this.parent.durationUnitEditText).indexOf(unit) !==
          -1
        ) {
          durationUnit = 'minute';
        } else if (
          getValue('hour', this.parent.durationUnitEditText).indexOf(unit) !==
          -1
        ) {
          durationUnit = 'hour';
        } else if (
          getValue('day', this.parent.durationUnitEditText).indexOf(unit) !== -1
        ) {
          durationUnit = 'day';
        }
      }
  ...
  protected getHolidaysCount(startDate: Date, endDate: Date): number {
    ...
    if (sDate.getTime() < eDate.getTime()) {
      for (let i: number = 0; i < holidays.length; i++) {
        const currentHoliday: Date = this.getDateFromFormat(
          new Date(holidays[i as number])
        );
        if (
          sDate.getTime() <= currentHoliday.getTime() &&
          eDate.getTime() > currentHoliday.getTime()
        ) {
          if (
            (!this.parent.includeWeekend &&
              this.parent.nonWorkingDayIndex.indexOf(
                currentHoliday.getDay()
              ) === -1) ||
            this.parent.includeWeekend
          ) {
            holidaysCount += 1;
  ...
  public isOnHolidayOrWeekEnd(date: Date, checkWeekEnd: boolean): boolean {
    checkWeekEnd = !isNullOrUndefined(checkWeekEnd)
      ? checkWeekEnd
      : this.parent.includeWeekend;
    if (
      !this.parent.autoCalculateDateScheduling &&
    ...
    if (
      !checkWeekEnd &&
      this.parent.nonWorkingDayIndex.indexOf(date.getDay()) !== -1
    ) {
      return true;
    }
    const holidays: number[] = this.parent.totalHolidayDates;
    for (let count: number = 0; count < holidays.length; count++) {
      const holidayFrom: Date = this.getDateFromFormat(
        new Date(holidays[count as number])
      );
      const holidayTo: Date = new Date(holidayFrom.getTime());
      holidayFrom.setHours(0, 0, 0, 0);
      holidayTo.setHours(23, 59, 59, 59);
      if (
        date.getTime() >= holidayFrom.getTime() &&
        date.getTime() < holidayTo.getTime()
      ) {
        return true;
      }
    }
  ...
  protected getNonWorkingSecondsOnDate(
    ...
    const startOnHoliday: boolean =
      ...
      !(
        ...
        ? this.isOnHolidayOrWeekEnd(startDate, null)
        : false;
    const endOnHoliday: boolean =
      isAutoSchedule &&
      this.parent.autoCalculateDateScheduling &&
      !(
        this.parent.isLoad &&
        ...
        ? this.isOnHolidayOrWeekEnd(endDate, null)
        : false;
    ...
    if (
      ...
      if (!startOnHoliday) {
        for (
          let i: number = startRangeIndex;
          i < startnonWorkingTimeRange.length;
          i++
        ) {
          if (
            !isNullOrUndefined(startnonWorkingTimeRange[i as number]) &&
            !startnonWorkingTimeRange[i as number].isWorking
          ) {
      ...
      if (startRangeIndex !== endRangeIndex) {
        if (!endOnHoliday) {
          for (let i: number = startRangeIndex; i <= endRangeIndex; i++) {
            if (
              ...
              if (i === startRangeIndex) {
                totNonWrkSecs +=
                  startnonWorkingTimeRange[i as number].to - sHour;
              } else if (i === endRangeIndex) {
                totNonWrkSecs +=
                  eHour - startnonWorkingTimeRange[i as number].from;
              } else {
                totNonWrkSecs += startnonWorkingTimeRange[i as number].interval;
              }
            }
        ...
        if (!endOnHoliday) {
          const range: IWorkingTimeRange =
            startnonWorkingTimeRange[startRangeIndex as number];
          if (!range.isWorking) {
            totNonWrkSecs = eHour - sHour;
          }
        } else {
  ...
  protected getPreviousWorkingDay(date: Date): Date {
    ...
    if (
      this.parent.nonWorkingDayIndex.indexOf(dayIndex) !== -1 ||
      (this.parent.nonWorkingDayIndex.indexOf(previousIndex) !== -1 &&
        dayEndTime === 86400 &&
  ...
  protected updateDateWithTimeZone(sDate: Date, eDate: Date): void {
    ...
    let uDate: Date;
    if (sTZ !== eTZ) {
      const standardTZ: number = new Date(
  ...
  public getDateFromFormat(date: string | Date, toConvert?: boolean): Date {
    ...
    if (isNullOrUndefined(date)) {
      ...
      const dateObject: Date = this.parent.globalize.parseDate(date, {
        format: this.parent.getDateFormat(),
        type: 'dateTime',
      });
      updatedDate =
        isNullOrUndefined(dateObject) && !isNaN(new Date(date).getTime())
          ? new Date(date)
          : dateObject;
    }
    if (!isNullOrUndefined(this.parent.timezone) && toConvert) {
      const convertedDate: Date = this.convert(
        updatedDate,
  ...
  public calculateProjectDatesForValidatedTasks(editArgs?: Object): void {
    const projectStartDate: Date =
      typeof this.parent.projectStartDate === 'string'
        ? new Date(this.parent.projectStartDate)
        : this.parent.projectStartDate;
    const projectEndDate: Date =
      typeof this.parent.projectEndDate === 'string'
        ? new Date(this.parent.projectEndDate)
        : this.parent.projectEndDate;
    let minStartDate: Date | string = null;
    let maxEndDate: Date | string = null;
    ...
    if (
      ...
      const sortedStartDate: object[] = flatData
        .slice()
        .sort(
      ...
      const sortedEndDate: object[] = flatData
        .slice()
        .sort(
          (a: ITaskData, b: ITaskData) =>
            new Date(b[this.parent.taskFields.endDate]).getTime() -
            new Date(a[this.parent.taskFields.endDate]).getTime()
        );
      minStartDate = sortedStartDate[0][this.parent.taskFields.startDate];
      maxEndDate =
        sortedEndDate[sortedEndDate.length - 1][this.parent.taskFields.endDate];
    }
    this.parent.cloneProjectStartDate = projectStartDate
      ? new Date(projectStartDate.getTime())
      : typeof minStartDate === 'string'
      ? new Date(minStartDate)
      : minStartDate;
    this.parent.cloneProjectEndDate = projectEndDate
      ? new Date(projectEndDate.getTime())
      : typeof maxEndDate === 'string'
      ? new Date(maxEndDate)
      : maxEndDate;
  }
  /**
  ...
  public calculateProjectDates(editArgs?: Object): void {
    if (
      ...
      this.parent.cloneProjectEndDate = this.getDateFromFormat(
        this.parent.cloneProjectEndDate
      );
      return;
    }
    ...
    const eDate: Date =
      typeof this.parent.projectEndDate === 'string'
        ...
        : this.parent.projectEndDate;
    const projectStartDate: Date =
      this.parent.timelineModule.isZooming && this.parent.cloneProjectStartDate
        ? this.getDateFromFormat(this.parent.cloneProjectStartDate)
        : this.getDateFromFormat(sDate);
    const projectEndDate: Date =
      this.parent.timelineModule.isZooming && this.parent.cloneProjectEndDate
        ? this.getDateFromFormat(this.parent.cloneProjectEndDate)
        : this.getDateFromFormat(eDate);
    ...
    const sortDates: Function = (dates: Date[]): void => {
      if (dates.length > 0) {
        ...
        maxEndDate =
          dates.length > 1 ? new Date(dates[dates.length - 1].getTime()) : null;
      }
    };
    if (
      ((!projectStartDate || !projectEndDate) && flatData.length > 0) ||
      editArgs ||
      ...
      viewData.forEach((data: IGanttData) => {
        ...
        if (
          isNullOrUndefined(task.startDate) &&
          isNullOrUndefined(task.endDate)
        ) {
          tempStartDate = null;
          tempEndDate = null;
        } else {
      ...
      if (
        this.parent.eventMarkers.length > 0 &&
        !this.parent.timelineModule.isZoomToFit
      ) {
      ...
      if (!minStartDate || !maxEndDate) {
        if (!minStartDate) {
          minStartDate = isNullOrUndefined(minStartDate)
            ? this.getDateFromFormat(new Date())
            : minStartDate;
          minStartDate.setHours(0, 0, 0, 0);
        } else {
          minStartDate = isNullOrUndefined(minStartDate)
    ...
    if (!editArgs) {
      ...
      this.parent.cloneProjectStartDate = minStartDate
        ? !this.parent.timelineModule.isZoomToFit
          ? !isNullOrUndefined(projectStartDate)
            ? new Date(projectStartDate.getTime())
            : minStartDate
          : minStartDate
        : new Date(projectStartDate.getTime());
      this.parent.cloneProjectEndDate = maxEndDate
        ? !this.parent.timelineModule.isZoomToFit
          ? !isNullOrUndefined(projectEndDate)
            ? new Date(projectEndDate.getTime())
            : maxEndDate
          : maxEndDate
        : new Date(projectEndDate.getTime());
  ...
  public splitTasksDuration(segments: ITaskSegment[]): number {
    ...
    for (let i: number = 0; i < segments.length; i++) {
      ...
      if (this.parent.timelineModule.bottomTier === 'Hour') {
        duration += Math.ceil(
          this.getTimeDifference(sDate, eDate) / (1000 * 60 * 60)
        );
      } else if (this.parent.timelineModule.bottomTier === 'Minutes') {
        duration += Math.ceil(
          this.getTimeDifference(sDate, eDate) / (1000 * 60)
        );
      } else {
        duration += Math.ceil(
