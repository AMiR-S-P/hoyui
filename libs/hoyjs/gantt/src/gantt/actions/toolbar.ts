...
import { RowSelectEventArgs } from '@hoyui/grids';
...
export class Toolbar {
  ...
  public renderToolbar(): void {
    const toolbarItems: (ToolbarItem | string | ItemModel)[] =
      this.parent.toolbar || [];
    if (toolbarItems.length > 0) {
      ...
      if (this.parent.treeGrid.grid.headerModule) {
        this.parent.element.insertBefore(
          this.element,
          this.parent.treeGridPane.offsetParent
        );
      } else {
        this.parent.element.appendChild(this.element);
      }
      const preItems: ToolbarItem[] = [
        'Add',
        'Edit',
        'Update',
        'Delete',
      ...
      for (const item of preItems) {
        ...
        this.predefinedItems[item as string] = {
          ...
          text: this.parent.isAdaptive
            ? ''
            : this.parent.localeObj.getConstant(localeName),
          tooltipText:
            this.parent.localeObj.getConstant(localeName) +
            (localeName === 'add' ||
            localeName === 'edit' ||
            localeName === 'delete'
              ? this.parent.localeObj.getConstant('task')
      ...
      const searchLocalText: string =
        this.parent.localeObj.getConstant('search');
      if (this.parent.isAdaptive) {
        ...
        (this.predefinedItems as { Search: ItemModel }).Search = {
          ...
          template:
            ...
            '<input id="' +
            this.id +
            '_searchbar" class="e-input" name="input" type="search"' +
            ...
            'tabindex="-1" title="' +
            searchLocalText +
            '" aria-label= "search" role="button"></span>' +
            '</div>',
          tooltipText: searchLocalText,
          align: 'Right',
  ...
  private createToolbar(): void {
    ...
    const cancelItem: Element = this.element.querySelector(
      '#' + this.parent.element.id + '_cancel'
    );
    const updateItem: Element = this.element.querySelector(
    ...
    if (this.parent.isAdaptive && template) {
      ...
      this.searchElement = this.element.querySelector(
    ...
    this.enableItems(
      [this.parent.controlId + '_redo', this.parent.controlId + '_undo'],
      false
    ); // disable toolbar items.
    if (this.parent.readOnly) {
      this.enableItems(
        [
          this.parent.element.id + '_add',
          this.parent.element.id + '_update',
          this.parent.element.id + '_delete',
          this.parent.element.id + '_cancel',
          this.parent.element.id + '_indent',
          this.parent.element.id + '_outdent',
        ],
        false
      );
    }
  ...
  private getSearchBarElement(): HTMLElement {
    ...
    const textbox: HTMLElement = createElement('input', {
      attrs: { type: 'text' },
  ...
  private getItems(): ItemModel[] {
    ...
    toolbarItems.forEach((item: string | ItemModel, index: number) => {
      if (
        (typeof item === 'string' && item === 'Search') ||
    ...
    if (searchIndex > -1) {
      const searchItem: (string | ItemModel)[] = toolbarItems.splice(
        searchIndex,
        1
      );
  ...
  private getItemObject(itemName: string): ItemModel {
    return (
      this.predefinedItems[itemName as string] || {
        text: itemName,
  ...
  private toolbarClickHandler(arg: ClickEventArgs): void {
    ...
    if (
      arg.item['properties'].id === this.parent.element.id + '_pdfexport' ||
      arg.item['properties'].id === this.parent.element.id + '_critical-path'
    ) {
      if (
    ...
    gObj.trigger(events.toolbarClick, arg, (args: ClickEventArgs) => {
      if (args.cancel) {
        ...
        if (this.parent.isAdaptive === true) {
          if (
            args.item.id === gID + '_edit' ||
            args.item.id === gID + '_add' ||
            args.item.id === gID + '_delete' ||
            args.item.id === gID + '_searchbutton' ||
            args.item.id === gID + '_expandall' ||
        ...
        switch (!isNullOrUndefined(args.item) && args.item.id) {
          ...
          case gID + '_indent':
            if (
              gObj.editModule &&
              gObj.selectionModule.getSelectedRecords().length
            ) {
              gObj.indent();
            }
            break;
          ...
          case gID + '_delete':
            if (this.parent.selectionModule && this.parent.editModule) {
              if (
                ...
                (this.parent.selectionSettings.mode === 'Cell' &&
                  this.parent.selectionModule.getSelectedRowCellIndexes()
                    .length)
              ) {
            ...
            break;
          case gID + '_search': {
            const searchButtonId: string = getValue(
              'originalEvent.target.id',
              args
            );
            if (
              searchButtonId === this.parent.element.id + '_searchbutton' &&
              this.parent.filterModule
            ) {
              const keyVal: string = (<HTMLInputElement>(
          ...
          case gID + '_searchbutton': {
            const adaptiveSearchbar: HTMLElement = this.element.querySelector(
              '.e-adaptive-searchbar'
            );
            (
              adaptiveSearchbar.parentElement.childNodes[1] as HTMLElement
            ).style.display = 'none';
  ...
  public refreshToolbarItems(args?: RowSelectEventArgs): void {
    ...
    if (gObj.selectionModule) {
      if (
        gObj.selectionModule.getSelectedRecords().length === 1 ||
    ...
    if (gObj.selectionModule && gObj.selectionSettings.persistSelection) {
      const selectedRecords: number =
        gObj.selectionModule.getSelectedRecords().length;
      if (gObj.selectionSettings.persistSelection && selectedRecords === 1) {
        for (let i: number = 0; i < selectedRecords; i++) {
          const index: number = gObj.currentViewData.indexOf(
            gObj.selectionModule.getSelectedRecords()[i as number]
          );
          if (index === -1) {
            isPersist = true;
          }
    ...
    const toolbarDefaultItems: string[] = [
      gID + '_add',
      gID + '_edit',
      gID + '_delete',
      gID + '_update',
    ...
    const isResouceParent: boolean =
      (this.parent.viewType === 'ResourceView' &&
        getValue('data.level', args) !== 0) ||
    ...
    if (!isNullOrUndefined(this.parent.editModule)) {
      const touchEdit: boolean = gObj.editModule.taskbarEditModule
        ? gObj.editModule.taskbarEditModule.touchEdit
        : false;
      const hasData: number = gObj.flatData && gObj.flatData.length;
      // eslint-disable-next-line
      edit.allowAdding && !touchEdit
        ? enableItems.push(gID + '_add')
      ...
      if (
        ...
        if (
          !isNullOrUndefined(gObj.updatedRecords[ind as number]) &&
          gObj.updatedRecords[ind as number].level === 0 &&
          hasData &&
          !touchEdit
        ) {
          ...
          if (
            !isNullOrUndefined(gObj.updatedRecords[ind as number]) &&
            gObj.updatedRecords[ind as number].level -
              previousGanttRecord.level ===
              1 &&
            ind !== -1
          ) {
            disableItems.push(gID + '_indent');
      ...
      let isDeleteSelected: boolean = gObj.selectionModule
        ? gObj.selectionModule.getSelectedRecords().length > 0 ||
          gObj.selectionModule.getSelectedRowCellIndexes().length > 0
          ? true
          : false
        : false;
      if (gObj.selectionModule && gObj.selectionSettings.persistSelection) {
        ...
        const selectedRecords: IGanttData[] =
          gObj.selectionModule.getSelectedRecords();
      ...
      if (
        gObj.editSettings.mode === 'Auto' &&
        !isNullOrUndefined(gObj.editModule.cellEditModule) &&
        ...
        for (let t: number = 0; t < toolbarItems.length; t++) {
          if (
            toolbarItems[t as number].id !== gID + '_update' &&
            toolbarItems[t as number].id !== gID + '_cancel' &&
        ...
        for (let t: number = 0; t < toolbarItems.length; t++) {
          if (
            enableItems.indexOf(toolbarItems[t as number].id) === -1 &&
