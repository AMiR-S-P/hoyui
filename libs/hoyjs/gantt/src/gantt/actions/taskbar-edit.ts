...
export class TaskbarEdit extends DateProcessor {
  ...
  private mouseDownHandler(e: PointerEvent): void {
    if (this.parent.editSettings.allowTaskbarEditing && !this.parent.readOnly) {
      ...
      if (this.taskBarEditElement) {
        ...
        const element: Element = parentsUntil(
          targetElement,
          cls.taskBarMainContainer
        );
        if (
          (element &&
            element.innerHTML === this.taskBarEditElement.innerHTML) ||
          this.taskBarEditElement.classList.contains('e-segmented-taskbar') ||
          this.taskBarEditElement.classList.contains('collpse-parent-border')
        ) {
  ...
  private mouseClickHandler(e: PointerEvent): void {
    ...
    if (
      ...
      this.parent.selectionModule.enableSelectMultiTouch
    ) {
    ...
    if (
      ...
      !isNullOrUndefined(this.taskBarEditElement) &&
      element &&
      element.innerHTML !== this.taskBarEditElement.innerHTML
    ) {
      this.connectorSecondRecord =
        this.parent.ganttChartModule.getRecordByTaskBar(element);
    ...
    if (predecessors) {
      for (let i: number = 0; i < predecessors.length; i++) {
        ...
        if (
          ...
          id.toString() === predecessor.to
        ) {
          this.applyActiveColor(predecessor.from, predecessor.to, show);
        }
      }
    }
    const chartContent: Element =
      this.parent.ganttChartModule.chartBodyContainer;
    if (show) {
  ...
  private applyActiveColor(from: string, to: string, enable?: boolean): void {
    const taskId: string =
      this.parent.viewType === 'ProjectView'
        ? this.taskBarEditRecord.ganttProperties.taskId.toString()
        : this.taskBarEditRecord.ganttProperties.rowUniqueID.toString();
    const ganttRecord: IGanttData =
      taskId === from
        ? this.parent.connectorLineModule.getRecordByID(to)
        : this.parent.connectorLineModule.getRecordByID(from);
    const $tr: Element =
      this.parent.ganttChartModule.getChartRows()[
        this.parent.currentViewData.indexOf(ganttRecord)
      ];
    if (!isNullOrUndefined($tr)) {
      const $taskbar: Element = $tr.querySelector(
        '.' + cls.taskBarMainContainer
      );
      const $connectorElement: Element = this.parent.element.querySelector(
        '#ConnectorLineparent' + from + 'child' + to
      );
      if (enable) {
  ...
  private validateConnectorPoint(): boolean {
    ...
    const parentId: string =
      this.parent.viewType === 'ResourceView'
        ? parentRecord.taskId
        : parentRecord.rowUniqueID;
    const childId: string =
      this.parent.viewType === 'ResourceView'
        ? childRecord.taskId
        : childRecord.rowUniqueID;
    if (this.connectorSecondRecord.hasChildRecords) {
      ...
      for (let i: number = 0; i < childRecord.predecessor.length; i++) {
        ...
        if (
          predecessor.from === parentId.toString() &&
          predecessor.to === childId.toString()
        ) {
          this.parent.connectorLineEditModule.childRecord =
            this.connectorSecondRecord;
          ...
          predecessor.from === childId.toString() &&
          predecessor.to === parentId.toString()
        ) {
          this.parent.connectorLineEditModule.childRecord =
            this.taskBarEditRecord;
          this.parent.connectorLineEditModule.predecessorIndex = i;
          this.parent.connectorLineEditModule.renderPredecessorDeleteConfirmDialog();
          isValid = false;
          break;
  ...
  private mouseLeaveHandler(e: PointerEvent): void {
    if (
      this.taskBarEditAction === 'ChildDrag' ||
      this.taskBarEditAction === 'ParentDrag' ||
      this.taskBarEditAction === 'ProgressResizing' ||
      this.taskBarEditAction === 'LeftResizing' ||
  ...
  public updateTaskBarEditElement(e: PointerEvent): void {
    ...
    let element: Element;
    if (
      target.classList.contains(cls.manualParentRightResizer) ||
      target.classList.contains(cls.manualParentMainContainer) ||
      ...
      if (
        ...
        !target.classList.contains('e-connectorpoint-left') &&
        !target.classList.contains('e-connectorpoint-right')
      ) {
        const currentRecord: IGanttData =
          this.parent.ganttChartModule.getRecordByTaskBar(element);
        if (
          !isNullOrUndefined(currentRecord.ganttProperties.segments) &&
          currentRecord.ganttProperties.segments.length > 0
        ) {
          if (target.classList.contains('e-progressbar-handler-after')) {
            for (
              let i: number = 0;
              i < currentRecord.ganttProperties.segments.length;
              i++
            ) {
              if (
                currentRecord.ganttProperties.segments[i as number].showProgress
              ) {
                element = element.querySelectorAll('.e-segmented-taskbar')[
                  i as number
                ];
                break;
              }
    ...
    if (this.parent.editSettings.allowTaskbarEditing && element) {
      ...
      this.taskBarEditRecord = this.parent.ganttChartModule.getRecordByTaskBar(
        this.editElement
      );
      if (
        e.type === 'mousedown' ||
        ...
        const index: string =
          this.editElement.getAttribute('data-segment-index');
        const currentRecord: IGanttData =
          this.parent.ganttChartModule.getRecordByTaskBar(element);
        ...
        const resizeElement: HTMLElement = createElement('div', {
          styles:
            'height:100%;border-style:dashed;border-bottom:none;border-top:none;border-width:1px;position:absolute;z-index:10000',
        });
        addClass([resizeElement as HTMLElement], 'e-taskbar-resize-div');
        (resizeElement as HTMLElement).style.setProperty(
          'width',
          ganttprop.width + 'px'
        );
        ...
        if (
          ...
          (resizeElement as HTMLElement).style.setProperty(
            'left',
            !isNullOrUndefined(ganttprop.segments) &&
              ganttprop.segments.length > 1
              ? parentleft +
                  ganttprop.segments[Number(currentindex)].left +
                  'px'
              : ganttprop.left + 'px'
          );
        }
        ...
        const Check: Element =
          this.parent.ganttChartModule.chartBodyContainer.querySelector(
            '.e-clone-taskbar'
          );
        ...
        if (
          ...
          if (
            ...
            cloneTaskBar = this.editElement.parentElement.cloneNode(
              true
            ) as Node;
          } else {
        ...
        if (!Check) {
          ...
          (cloneTaskBar as HTMLElement).style.setProperty(
            'position',
            'absolute'
          );
          if (
            currentRecord.hasChildRecords &&
            !currentRecord.ganttProperties.isAutoSchedule
          ) {
            const containerPosition: { top: number; left: number } =
              this.parent.getOffsetRect(
                this.parent.ganttChartModule.chartBodyContainer
              );
            const taskbarPosition: { top: number; left: number } =
              this.parent.getOffsetRect(this.editElement as HTMLElement);
            if (
              this.editElement.classList.contains(
                'e-manualparent-main-container'
              )
            ) {
              (cloneTaskBar as HTMLElement).style.setProperty(
                'left',
                currentRecord.ganttProperties.left -
                  currentRecord.ganttProperties.autoLeft +
                  'px'
              );
            } else {
              (cloneTaskBar as HTMLElement).style.setProperty(
        ...
        if (
          ...
          if (this.taskBarEditRecord.level === 0) {
            ...
            const parentRecord: IGanttData = this.parent.getTaskByUniqueID(
              this.taskBarEditRecord.parentItem.uniqueID
            );
            if (!isNullOrUndefined(parentRecord) && !parentRecord.expanded) {
              ...
              if (
                this.taskBarEditElement.querySelector('.e-gantt-child-taskbar')
              ) {
                addClass(
                  [
                    this.taskBarEditElement.querySelector(
   ...
   * @private
   */
  public showHideTaskBarEditingElements(
    element: Element,
    secondElement: Element,
    fadeConnectorLine?: boolean
  ): void {
    secondElement = secondElement ? secondElement : this.editElement;
    let isShowProgressResizer: boolean = this.parent.taskFields.progress
      ? true
      : false;
    ...
    const record: IGanttData = element
      ? this.parent.ganttChartModule.getRecordByTaskBar(element)
      : secondElement
      ? this.parent.ganttChartModule.getRecordByTaskBar(secondElement)
      : null;
    if (this.parent.enableMultiTaskbar && element) {
      const parentRecord: IGanttData = this.parent.getParentTask(
        record.parentItem
      );
    ...
    if (element) {
      if (element.querySelector('.' + cls.taskBarLeftResizer)) {
        addClass(
          [element.querySelector('.' + cls.taskBarLeftResizer)],
          [cls.leftResizeGripper]
        );
        addClass(
          [element.querySelector('.' + cls.taskBarRightResizer)],
          [cls.rightResizeGripper]
        );
        if (isShowProgressResizer) {
          const progressElement: HTMLElement =
            record &&
              ...
              ? (this.parent
                  .getRowByIndex(
                    this.parent.enableVirtualization
                      ? this.parent.currentViewData.indexOf(record)
      ...
      if (
        ...
        const connectorElement: Element = !isNullOrUndefined(
          element.querySelector('.' + cls.connectorPointLeft)
        )
          ? element
        ...
        if (
          !isNullOrUndefined(
            connectorElement.querySelector('.' + cls.connectorPointLeft)
          )
        ) {
          addClass(
            [connectorElement.querySelector('.' + cls.connectorPointLeft)],
            [cls.connectorPointLeftHover]
          );
        }
        if (
          !isNullOrUndefined(
            connectorElement.querySelector('.' + cls.connectorPointRight)
          )
        ) {
          addClass(
            [connectorElement.querySelector('.' + cls.connectorPointRight)],
            [cls.connectorPointRightHover]
          );
        }
      }
    ...
    if (secondElement && element !== secondElement) {
      if (secondElement.querySelector('.' + cls.taskBarLeftResizer)) {
        removeClass(
          [secondElement.querySelector('.' + cls.taskBarLeftResizer)],
          [cls.leftResizeGripper]
        );
        removeClass(
          [secondElement.querySelector('.' + cls.taskBarRightResizer)],
          [cls.rightResizeGripper]
        );
      ...
      if (
        ...
        (secondElement.querySelector('.' + cls.connectorPointLeft) ||
          secondElement.parentElement.querySelector(
        ...
        const leftConnector: Element = connectorElement.querySelector(
          '.' + cls.connectorPointLeft
        );
        const rightConnector: Element = connectorElement.querySelector(
          '.' + cls.connectorPointRight
        );
        if (leftConnector) {
  ...
  private getTaskBarAction(e: PointerEvent): string {
    ...
    if (mouseDownElement.classList.contains(cls.taskBarLeftResizer)) {
      ...
      (mouseDownElement.classList.contains(cls.childProgressResizer) ||
      ...
      mouseDownElement.classList.contains(cls.manualParentRightResizer)
    ) {
      action = 'ParentResizing';
    } else if (
      mouseDownElement.classList.contains(cls.manualParentTaskBar) ||
  ...
  private updateMouseDownProperties(event: PointerEvent): void {
    ...
    const parentWithZoomStyle: HTMLElement = this.parent.element.closest(
      '[style*="zoom"]'
    ) as HTMLElement;
    if (parentWithZoomStyle) {
      const zoom1: number = parseFloat(
        (getComputedStyle(parentWithZoomStyle) as CSSStyleDeclaration).zoom
      );
    ...
    if (e.pageX || e.pageY) {
      ...
      this.mouseDownY = this.dragMoveY =
        e.pageY -
  ...
  private isMouseDragCheck(): void {
    if (
      ...
      ((this.mouseDownX !== this.mouseMoveX &&
        (this.mouseDownX + 3 < this.mouseMoveX ||
          this.mouseDownX - 3 > this.mouseMoveX)) ||
        (this.mouseDownY !== this.mouseMoveY &&
          (this.mouseDownY + 3 < this.mouseMoveY ||
            this.mouseDownY - 3 > this.mouseMoveY)))
    ) {
      this.isMouseDragged = true;
      this.parent.initiateEditAction(true);
      const item: ITaskData = this.taskBarEditRecord.ganttProperties;
      this.previousItem = this.parent.timelineModule.extendFunction(
        item,
        this.previousItemProperty
      ) as ITaskData;
      if (
  ...
  public removeFirstBorder(element: any): void {
    ...
    if (
      this.parent.element.getElementsByClassName('e-firstrow-border').length >
        0 &&
      element &&
      (element.getAttribute('data-rowindex') !== 0 || canremove)
    ) {
      this.parent.element
        .getElementsByClassName('e-firstrow-border')[0]
        .remove();
    }
  }
  public removeLastBorder(element: Element): void {
    const isEmptyRow: boolean =
      element &&
      (element.classList.contains('e-emptyrow') ||
    ...
    if (this.parent.enableVirtualization) {
      islastRowIndex =
        ...
        this.parent.treeGrid
          .getRows()
          [
            this.parent.treeGrid.getCurrentViewRecords().length - 1
          ].getAttribute('data-uid') !== element.getAttribute('data-uid');
    } else {
      islastRowIndex =
        element &&
        !isEmptyRow &&
    ...
    if (
      this.parent.element.getElementsByClassName('e-lastrow-border').length >
        0 &&
      element &&
      (islastRowIndex || canremove)
    ) {
      this.parent.element
        .getElementsByClassName('e-lastrow-border')[0]
        .remove();
    }
  }
  private removetopOrBottomBorder(): void {
    const border: HTMLCollection = [].slice.call(
      this.parent.element.querySelectorAll('.e-dropbottom, .e-droptop')
    );
    if (border.length) {
  ...
  private topOrBottomBorder(target: Element): void {
    ...
    if (this.parent.enableVirtualization) {
      const gridElement: Element = this.parent.treeGrid
        .getRows()
        .filter((data: Element) => {
          return (
            data.getAttribute('data-rowindex') ===
            target.getAttribute('data-rowindex')
          );
        })[0];
      const index: number = this.parent.treeGrid
        .getRows()
        .indexOf(gridElement as HTMLTableRowElement);
      rowElement = this.parent.getRowByIndex(index).children;
    } else {
      rowElement = this.parent.getRowByIndex(
        parseInt(target.getAttribute('data-rowindex'), 10)
      ).children;
    }
    if (rowElement) {
      if (this.parent.rowDragAndDropModule['dropPosition'] === 'above') {
        ...
        if (
          this.parent.element.getElementsByClassName('e-lastrow-dragborder')
            .length > 0
        ) {
          this.parent.element
            .getElementsByClassName('e-lastrow-dragborder')[0]
            .remove();
        }
      }
  ...
  private removeChildBorder(): void {
    const borderElem: HTMLCollection = [].slice.call(
      this.parent.element.querySelectorAll('.e-childborder')
    );
  ...
  private addErrorElem(): void {
    const dragelem: Element =
      document.getElementsByClassName('e-clone-taskbar')[0];
    ...
    if (!errorelem) {
      ...
      if (
        this.taskBarEditRecord.ganttProperties.segments &&
  ...
  public ensurePosition(
    ...
    currentRecord: IGanttData
  ): void {
    draggedRecords.filter((draggedRecord: IGanttData) => {
      if (
        draggedRecord.hasChildRecords &&
        !isNullOrUndefined(draggedRecord.childRecords)
      ) {
  ...
  public mouseMoveAction(event: PointerEvent): void {
    ...
    if (
      this.taskBarEditAction === 'ChildDrag' ||
      this.taskBarEditAction === 'ParentDrag' ||
      this.taskBarEditAction === 'ManualParentDrag' ||
      this.taskBarEditAction === 'MilestoneDrag'
    ) {
      let ganttRowElement: HTMLElement;
      if (this.parent.enableVirtualization) {
        const index: number = this.parent.currentViewData.indexOf(
          this.taskBarEditRecord
        );
        ganttRowElement = this.parent.getRowByIndex(index);
      } else {
        ganttRowElement = this.parent.getRowByIndex(
          this.taskBarEditRecord.index
        );
      }
    ...
    if (this.isMouseDragged && this.taskBarEditAction) {
      ...
      if (
        !isNullOrUndefined(this.taskbarElement) &&
        !isNullOrUndefined(this.editElement) &&
        this.taskBarEditAction !== 'ConnectorPointRightDrag' &&
        this.taskBarEditAction !== 'ConnectorPointLeftDrag' &&
        !(
          this.parent.viewType === 'ResourceView' &&
          this.currentData.hasChildRecords
        )
      ) {
        ...
        if (
          (this.taskBarEditAction === 'ChildDrag' ||
            this.taskBarEditAction === 'LeftResizing') &&
          !isNullOrUndefined(this.currentIndex) &&
          !isNullOrUndefined(this.currentIndex)
            ? Number(this.currentIndex) === 0
            : false
        ) {
          (
            this.taskbarElement.childNodes[0].childNodes[0] as HTMLLIElement
          ).style.setProperty(
            'top',
            currentElement.parentElement.offsetTop + 'px'
          );
          if (
            this.parent.allowTaskbarDragAndDrop &&
            this.taskBarEditAction !== 'LeftResizing' &&
            ...
            this.parent.chartPane
              .getElementsByClassName('e-gantt-chart')[0]
              .appendChild(this.taskbarElement);
          } else {
        ...
        if (this.taskBarEditAction !== 'ProgressResizing') {
          if (
            ...
            const rootElement: NodeListOf<Element> =
      ...
      if (
        ...
        const taskbarPosition: { top: number; left: number } =
          this.parent.getOffsetRect(this.taskBarEditElement);
        const left: number =
          !isNullOrUndefined(document.body.className) &&
          document.body.className.includes('e-bigger')
            ? 12
            : 0;
        const pointerLeft: number =
          3 + this.parent.chartRowsModule.connectorPointWidth + left;
        let xValue: number;
        if (this.parent.enableRtl) {
        ...
        if (!this.isClonedElement && !this.parent.enableRtl) {
          xValue =
            xValue - this.parent.ganttChartModule.scrollElement.scrollLeft;
        ...
        if (this.parent.enableRtl) {
          target = document.elementFromPoint(
            this.parent.ganttChartModule.chartBodyContainer.offsetWidth,
            yValue
          );
        ...
        if (this.parent.enableVirtualization) {
          if (!this.draggedTreeGridRowElement) {
            ...
            this.draggedTreeGridRowHeight = (
              this.draggedTreeGridRowElement as HTMLElement
            ).offsetHeight;
          }
        } else {
          draggedTreeGridRowElement =
            this.parent.treeGrid.getRows()[
              this.parent.flatData.indexOf(this.taskBarEditRecord)
            ];
          this.draggedTreeGridRowElement = draggedTreeGridRowElement;
          if (this.parent.enableMultiTaskbar) {
            if (
              this.taskBarEditRecord.parentItem &&
              !isNullOrUndefined(
                this.parent.getRowByID(this.taskBarEditRecord.parentItem.taskId)
              )
            ) {
              this.draggedTreeGridRowHeight = this.parent.getRowByID(
                this.taskBarEditRecord.parentItem.taskId
              ).offsetHeight;
              if (
                this.parent.viewType === 'ProjectView' &&
                this.draggedTreeGridRowHeight === 0
              ) {
                let nestedParent: IGanttData = this.parent.getRecordByID(
                  this.taskBarEditRecord.parentItem.taskId
                );
                while (nestedParent.parentItem) {
                  nestedParent = this.parent.getRecordByID(
                    nestedParent.parentItem.taskId
                  );
                  this.draggedTreeGridRowHeight = this.parent.getRowByID(
                    nestedParent['TaskID']
                  ).offsetHeight;
              ...
              const currentElement: HTMLElement =
                this.editElement.parentElement;
        ...
        if (chartRowElement) {
          ...
          if (this.parent.enableVirtualization) {
            droppedTreeGridRowElement = this.parent.treeGrid
              .getRows()
              .filter((data: HTMLTableRowElement) => {
                return (
                  data.getAttribute('data-rowindex') ===
                  chartRowElement.getAttribute('data-rowindex')
                );
              })[0];
          } else {
            droppedTreeGridRowElement =
              this.parent.treeGrid.getRows()[
                parseInt(chartRowElement.getAttribute('data-rowindex'), 10)
              ];
          }
          this.removeFirstBorder(droppedTreeGridRowElement);
          this.removeLastBorder(droppedTreeGridRowElement);
          if (
            (!isNullOrUndefined(droppedTreeGridRowElement) &&
              droppedTreeGridRowElement.getAttribute('data-uid') ===
                this.draggedTreeGridRowElement.getAttribute('data-uid')) ||
          ...
          const toolHeight: number = tObj.toolbarModule
            ? this.parent.toolbarModule.element.offsetHeight
            : 0;
          ...
          const contentHeight: number =
            (tObj.treeGrid.getHeaderContent() as HTMLElement).offsetHeight +
            positionOffSet.top +
            toolHeight;
          const scrollTop: number = (tObj.treeGrid.getContent() as HTMLElement)
            .firstElementChild.scrollTop;
          if (!isNullOrUndefined(droppedTreeGridRowElement)) {
            rowPositionHeight =
              (droppedTreeGridRowElement as HTMLElement).offsetTop - scrollTop;
          }
          if (this.parent.enableVirtualization) {
            rowTop = this.parent.getOffsetRect(
              droppedTreeGridRowElement as HTMLElement
            ).top;
          } else {
          ...
          const posy: number =
            event.type === 'mousemove'
              ? event.pageY
              : !isNullOrUndefined(event['changedTouches'])
              ? event['changedTouches'][0].pageY
              : null;
          const isTopSegment: boolean = posy <= topRowSegment;
          const isMiddleRowSegment: boolean =
            posy > topRowSegment && posy <= middleRowSegment;
          const isBottomRowSegment: boolean =
            posy > middleRowSegment && posy <= bottomRowSegment;
          if (isTopSegment || isMiddleRowSegment || isBottomRowSegment) {
            let rowElement: HTMLElement;
            if (this.parent.enableVirtualization) {
              const index: number = this.parent.treeGrid
                .getRows()
                .indexOf(droppedTreeGridRowElement as HTMLTableRowElement);
              rowElement = this.parent.getRowByIndex(index);
            } else {
              rowElement = this.parent.getRowByIndex(
                parseInt(
                  droppedTreeGridRowElement.getAttribute('data-rowindex'),
                  10
                )
              );
            }
            const rowIndex: number = getValue('rowIndex', rowElement);
            const droppedTreeGridRecord: IGanttData =
              this.parent.flatData[rowIndex as number];
            let isValid: boolean = true;
            if (
              this.parent.viewType === 'ResourceView' &&
              !this.taskBarEditRecord.hasChildRecords &&
              !droppedTreeGridRecord.hasChildRecords &&
              !isNullOrUndefined(droppedTreeGridRecord.parentItem) &&
              this.taskBarEditRecord.parentItem.taskId ===
                droppedTreeGridRecord.parentItem.taskId
            ) {
              isValid = false;
            }
      ...
      this.parent.trigger('actionBegin', args, (arg: IActionBeginEventArgs) => {
        if (
          args.taskBarEditAction === 'ConnectorPointRightDrag' ||
          args.taskBarEditAction === 'ConnectorPointLeftDrag' ||
          ...
          args.taskBarEditAction === 'ManualParentDrag' ||
  ...
  public taskBarEditingAction(e: PointerEvent, isMouseClick: boolean): void {
    ...
    const recordIndex: number = this.parent.ganttChartModule.getIndexByTaskBar(
      this.taskBarEditElement
    );
    if (this.taskBarEditRecord !== null) {
      ...
      this.targetElement = args.target = closest(
        e.target as Element,
        '.e-gantt-child-taskbar'
      );
      ...
      let segmentIndex: number =
        this.segmentIndex !== -1 ? this.segmentIndex : null;
      if (this.taskBarEditAction === 'ProgressResizing') {
        this.mainElement = parentsUntil(
          this.taskBarEditElement,
          cls.taskBarMainContainer
        );
        if (
          ...
          for (
            let i: number = 0;
            i < this.taskBarEditRecord.ganttProperties.segments.length;
            i++
          ) {
            const resizeGripper: HTMLCollectionOf<Element> =
              document.getElementsByClassName(
                'e-child-progress-resizer e-progress-resize-gripper'
              )[0]['style'];
            const currentWidth: number = this.parent.enableRtl
              ? parseInt(resizeGripper['right'], 10) - 8
              : parseInt(resizeGripper['left'], 10) + 8;
            if (
              currentWidth <
                this.taskBarEditRecord.ganttProperties.segments[i as number]
                  .width &&
              i === 0
            ) {
              segmentIndex = i;
            ...
            if (
              this.taskBarEditRecord.ganttProperties.segments[i + 1] &&
              currentWidth <
                this.taskBarEditRecord.ganttProperties.segments[i + 1].width +
        ...
        this.taskBarEditAction === 'RightResizing' ||
        this.taskBarEditAction === 'ParentResizing'
      ) {
      ...
      if (
        ...
        if (
          ...
          const parentRecord: IGanttData = this.parent.getRecordByID(
            this.taskBarEditRecord.parentItem.taskId
          );
          if (
            ...
            if (
              parseInt(this.editElement['style'].marginTop, 10) >
              this.parent.rowHeight
            ) {
              for (
                let i: number = 0;
                i < parentRecord.childRecords.length;
                i++
              ) {
                if (
                  parentRecord.childRecords[i as number].ganttProperties
                    .taskId !== this.taskBarEditRecord.ganttProperties.taskId
                ) {
        ...
        if (this.parent.enableRtl) {
          this.leftValue =
            pos.left -
            Math.abs(
              xValue - this.parent.getOffsetRect(this.taskBarEditElement).left
            ) -
  ...
  private updateMouseMoveProperties(event: PointerEvent): void {
    ...
    const isConnectorLineEdit: boolean =
      this.taskBarEditAction === 'ConnectorPointLeftDrag' ||
      this.taskBarEditAction === 'ConnectorPointRightDrag'
        ? true
        : false;
    if (
      ((this.taskBarEditRecord.ganttProperties.width > 3 ||
        this.taskBarEditRecord.ganttProperties.isMilestone) &&
        !(
          this.taskBarEditAction === 'ProgressResizing' &&
          (this.taskBarEditRecord.ganttProperties.progress === 0 ||
            this.taskBarEditRecord.ganttProperties.progress === 100)
        )) ||
      isConnectorLineEdit
    ) {
      let mouseX: number = 0;
      ...
      if (
        ...
        mouseY + 80 >
        containerPosition.top +
          this.parent.ganttChartModule.chartBodyContainer.offsetHeight
      ) {
        this.timerCount =
          this.parent.ganttChartModule.scrollObject.previousScroll.top;
  ...
  private enableDragging(e: PointerEvent): void {
    ...
    if (
      ...
      const segments: ITaskSegment[] =
        this.taskBarEditRecord.ganttProperties.segments.map(
          (e: ITaskSegment) => ({ ...e })
        );
      ...
      if (this.mouseDownX > this.mouseMoveX) {
        ...
        differenceWidth = isNullOrUndefined(this.previousMouseMove)
          ...
          : this.mouseMoveX - this.previousMouseMove;
        this.previousMouseMove = this.mouseMoveX;
        segment.left = segment.left + differenceWidth;
      }
      ...
      if (this.taskBarEditElement.classList.contains('e-segment-inprogress')) {
        left =
          segment.left < previousSegment.left + previousSegment.width
            ? previousSegment.left + previousSegment.width
            : segment.width + segment.left > nextSegment.left
            ? nextSegment.left - segment.width
            : segment.left;
      } else {
        left =
          segment.left < previousSegment.left + previousSegment.width
      ...
      if (this.mouseDownX > this.mouseMoveX) {
        ...
        if (differenceWidth > 0) {
          this.parent.setRecordValue(
            'left',
            this.previousItem.left - differenceWidth,
            item,
            true
          );
        }
      ...
      const left: number =
        item.left < 0
          ? 0
          : item.left + item.width >= timelineWidth
  ...
  private validateProgressWidth(
  ...
  ): { progressWidth: number; totalTaskWidth: number } {
    let reduceWidth: number = 0;
    if (this.currentSegmentIndex !== -1 && item.segments) {
      const startIndex: number =
        this.currentSegmentIndex >= this.segmentIndex
          ? 0
      ...
      if (startIndex !== -1) {
        for (let i: number = startIndex; i < endIndex; i++) {
          reduceWidth =
            item.segments[i + 1].left -
            (item.segments[i as number].left +
              item.segments[i as number].width);
    ...
    if (item.segments && item.segments.length > 0) {
      totalTaskWidth =
        this.splitTasksDuration(item.segments) * this.parent.perDayWidth;
    }
  ...
  private performProgressResize(e: PointerEvent, segmentIndex?: number): void {
    ...
    let widthValue: number =
      item.progressWidth > item.width ? item.width : item.progressWidth;
  ...
  private enableLeftResizing(e: PointerEvent): void {
    ...
    if (
      ...
      if (this.mouseDownX > this.mouseMoveX) {
        if (this.mouseMoveX < item.left + item.width) {
          ...
          if (item.left > 0) {
            ...
            this.parent.setRecordValue(
              'width',
              this.previousItem.width + differenceWidth,
              item,
              true
            );
          }
        ...
        if (this.mouseMoveX < item.left + item.width) {
          ...
          if (
            ...
            this.previousItem.left + differenceWidth <=
              this.previousItem.left + this.previousItem.width
          ) {
            this.parent.setRecordValue(
              'left',
              this.previousItem.left + differenceWidth,
      ...
      this.parent.setRecordValue(
        'left',
        this.previousItem.left + this.previousItem.width - item.width,
        item,
        true
      );
    }
  ...
  private enableSplitTaskLeftResize(item: ITaskData): void {
    ...
    if (this.mouseDownX > this.mouseMoveX) {
      if (this.mouseMoveX < item.left + segment.width + segment.left) {
        ...
        if (segment.left > 0) {
          ...
          if (this.segmentIndex !== 0) {
            const previousSegment: ITaskSegment =
              segments[this.segmentIndex - 1];
            if (
              item.left + segment.left <
              item.left + previousSegment.left + previousSegment.width
            ) {
              const difference: number =
                item.left +
                previousSegment.left +
          ...
          this.parent.setRecordValue(
            'left',
            item.left - differenceWidth,
            item,
            true
          );
          this.parent.setRecordValue(
            'width',
            item.width + differenceWidth,
            item,
            true
          );
          segment.width = segment.width + differenceWidth;
      ...
      if (
        this.mouseMoveX <
        item.left + segment.width + segment.left - this.parent.perDayWidth
      ) {
        differenceWidth = isNullOrUndefined(this.previousMouseMove)
          ? this.mouseMoveX - this.mouseDownX
          : this.mouseMoveX - this.previousMouseMove;
        ...
        if (this.segmentIndex === 0 && segment.left <= 0) {
          ...
          segment.left < segment.left + segment.width &&
        ...
        if (this.mouseMoveX < item.left + segment.left + segment.width) {
          if (segment.width > this.parent.perDayWidth) {
            differenceWidth = isNullOrUndefined(this.previousMouseMove)
              ...
              : this.mouseMoveX - this.previousMouseMove;
            this.previousMouseMove = this.mouseMoveX;
            const singleDayDifference: number =
              segment.width - differenceWidth < this.parent.perDayWidth
  ...
  private updateEditPosition(e: PointerEvent, item: ITaskData): void {
    ...
    this.parent.setRecordValue(
      'progressWidth',
      this.parent.dataOperation.getProgressWidth(item.width, item.progress),
      item,
      true
    );
  }
  ...
  private updateIsMilestone(item: ITaskData): void {
    if (item.width <= 3) {
      ...
      if (
        !isNullOrUndefined(
          this.taskBarEditRecord[this.parent.taskFields.milestone]
        )
      ) {
        this.parent.setRecordValue(
  ...
  private enableRightResizing(e: PointerEvent): void {
    ...
    if (
      ...
      if (this.mouseDownX > this.mouseMoveX) {
        if (
          this.mouseMoveX > item.left + segment.left &&
          this.mouseDownX - this.mouseMoveX > 3
        ) {
          differenceWidth = isNullOrUndefined(this.previousMouseMove)
            ? this.mouseDownX - this.mouseMoveX
            : this.previousMouseMove - this.mouseMoveX;
          ...
          if (this.mouseMoveX < item.left + segment.left) {
            const segmentWidth: number =
              (this.parent.timelineModule.isSingleTier &&
                (this.parent.timelineModule.customTimelineSettings.bottomTier
                  .unit === 'Hour' ||
              ...
              this.parent.timelineModule.customTimelineSettings.bottomTier
                .unit === 'Minutes'
                ? this.parent.timelineModule.customTimelineSettings
                    .timelineUnitSize
      ...
      let width: number;
      const nextSegment: ITaskSegment =
      ...
      if (!isNullOrUndefined(nextSegment)) {
        if (!this.taskBarEditElement.classList.contains('e-segment-last')) {
          width =
            segment.left + segment.width > nextSegment.left
              ? nextSegment.left - segment.left
              : segment.width;
        }
        segment.width = width;
      }
  ...
  private updateEditedItem(): void {
    ...
    switch (this.taskBarEditAction) {
      case 'ProgressResizing': {
        ...
        const updatedValues: { progressWidth: number; totalTaskWidth: number } =
          this.validateProgressWidth(item, progressWidth, totalTaskWidth);
        this.parent.setRecordValue(
          'progress',
          this.getProgressPercent(
            updatedValues.totalTaskWidth,
            updatedValues.progressWidth
          ),
          item,
          true
        );
      ...
      case 'LeftResizing':
        if (this.segmentIndex === -1) {
          ...
          startDate = this.parent.dateValidationModule.checkStartDate(
            projectStartDate,
            item,
            null
          );
          ...
          if (
            this.parent.dateValidationModule.compareDates(
              item.startDate,
              item.endDate
            ) === 0 &&
            isNullOrUndefined(item.isMilestone) &&
          ...
          this.parent.editModule.updateResourceRelatedFields(
            this.taskBarEditRecord,
            'duration'
          );
        } else {
      ...
      case 'ParentResizing':
        if (this.segmentIndex === -1) {
          ...
          if (isNullOrUndefined(item.startDate)) {
            startDate =
              this.parent.dateValidationModule.getValidStartDate(item);
            this.parent.setRecordValue('startDate', startDate, item, true);
          }
          const tempdate: Date = isNullOrUndefined(item.startDate)
            ? startDate
            : item.startDate;
          endDate = item.isMilestone
            ? tempdate
            : this.parent.dateValidationModule.checkEndDate(
                tempEndDate,
                this.taskBarEditRecord.ganttProperties
              );
          this.parent.setRecordValue(
            'endDate',
      ...
      case 'ManualParentDrag':
        if (
          this.segmentIndex === -1 ||
          this.segmentIndex === 0 ||
          (this.segmentIndex !== -1 && this.parent.allowTaskbarDragAndDrop)
        ) {
  ...
  private updateChildDrag(item: ITaskData): void {
    ...
    const projectStartDate: Date = this.getDateByLeft(
      left,
      item.isMilestone,
      item
    );
    ...
    if (this.segmentIndex === 0) {
      ...
      this.parent.chartRowsModule.incrementSegments(
        item.segments,
        0,
        this.taskBarEditRecord
      );
      ...
      if (
        ...
        isNullOrUndefined(item.startDate)
      ) {
        endDate = this.parent.dateValidationModule.checkStartDate(
          projectStartDate,
          item,
          null
        );
        endDate = this.parent.dateValidationModule.checkEndDate(
          endDate,
          this.taskBarEditRecord.ganttProperties
        );
        this.parent.setRecordValue('endDate', endDate, item, true);
        ...
        if (!isNullOrUndefined(item.duration)) {
          this.parent.dateValidationModule.calculateEndDate(
            this.taskBarEditRecord
          );
        }
      }
  ...
  private updateSplitLeftResize(item: ITaskData): void {
    ...
    const left: number =
      this.segmentIndex === 0
        ? this.getRoundOffStartLeft(item, this.roundOffDuration)
        : this.getRoundOffStartLeft(segment, this.roundOffDuration);
    const projectStartDate: Date = this.getDateByLeft(left);
    const startDate: Date = this.parent.dataOperation.checkStartDate(
      projectStartDate,
      item,
      false
    );
    ...
    this.parent.setRecordValue(
      'duration',
      this.sumOfDuration(item.segments),
      item,
      true
    );
    if (this.segmentIndex === 0) {
    ...
    if (!isNullOrUndefined(item.segments[this.segmentIndex - 1])) {
      const segmentOffsetDuration: number =
        this.parent.dataOperation.getDuration(
          item.segments[this.segmentIndex - 1].endDate,
          item.segments[this.segmentIndex].startDate,
          item.durationUnit,
          item.isAutoSchedule,
          item.isMilestone
        );
  ...
  private updateSplitRightResizing(item: ITaskData): void {
    ...
    const tempEndDate: Date = this.getDateByLeft(left);
    const endDate: Date = this.parent.dataOperation.checkEndDate(
      tempEndDate,
      item,
      false
    );
    ...
    if (!isNullOrUndefined(item.segments[this.segmentIndex + 1])) {
      ...
      const segmentOffset: number = this.parent.dataOperation.getDuration(
        item.segments[this.segmentIndex].endDate,
  ...
  private setSplitTaskDrag(item: ITaskData): void {
    ...
    const left: number = this.getRoundOffStartLeft(
      segment,
      this.roundOffDuration
    );
    let projectStartDate: Date = this.getDateByLeft(left);
    projectStartDate = this.parent.dateValidationModule.checkStartDate(
      projectStartDate,
      item,
      null
    );
    segment.startDate = projectStartDate;
    const segmentDate: Date = this.parent.dataOperation.getEndDate(
      segment.startDate,
      segment.duration,
      item.durationUnit,
      item,
    ...
    if (
      item.segments[this.segmentIndex + 1] &&
      segmentDate.getTime() >
        item.segments[this.segmentIndex + 1].startDate.getTime()
    ) {
    ...
    if (!isNullOrUndefined(item.segments[this.segmentIndex - 1])) {
      let offsetDuration: number = this.parent.dataOperation.getDuration(
        item.segments[this.segmentIndex - 1].endDate,
        item.segments[this.segmentIndex as number].startDate,
        item.durationUnit,
        item.isAutoSchedule,
        item.isMilestone
      );
      if (
        segment.startDate.getDay() === 1 &&
        offsetDuration === 0 &&
    ...
    if (!isNullOrUndefined(item.segments[this.segmentIndex + 1])) {
      ...
      if (
        nextSegment.startDate.getDay() === 1 &&
        offsetDuration === 0 &&
        !this.parent.includeWeekend
      ) {
        offsetDuration = 1;
  ...
  private getRoundOffEndLeft(
    ganttRecord: ITaskData,
    isRoundOff: boolean
  ): number {
    const tierMode: string =
    ...
    let remainingContribution: number =
      1 /
      (this.parent.timelineModule.getIncrement(
        this.getDateByLeft(totalLeft),
        1,
        'Day'
      ) /
        (1000 * 60 * 60 * 24));
    let remainingLeft: number =
      this.parent.perDayWidth - this.parent.perDayWidth / remainingContribution;
      ...
      if (tierMode === 'Hour') {
        ...
        remainingContribution =
          ...
          (this.parent.timelineModule.getIncrement(
            ...
            (1000 * 60 * 60));
        remainingLeft =
          this.parent.perDayWidth / 24 -
          this.parent.perDayWidth / 24 / remainingContribution;
        if (remainingLeft !== 0) {
        ...
        remainingLeft =
          ...
          this.parent.perDayWidth / (24 * 60) / remainingContribution;
        if (remainingLeft !== 0) {
   ...
   * @private
   */
  public getRoundOffStartLeft(
    ganttRecord: ITaskData | ITaskSegment,
    isRoundOff: boolean
  ): number {
    let left: number = isNullOrUndefined(ganttRecord as ITaskData)
      ? (ganttRecord as ITaskSegment).left
      : (ganttRecord as ITaskData).left;
    if (
      this.segmentIndex !== -1 &&
      isNullOrUndefined((ganttRecord as ITaskData).segments)
    ) {
    ...
    let remainingContribution: number =
      ...
      (this.parent.timelineModule.getIncrement(
        this.getDateByLeft(
          left,
          isNullOrUndefined(ganttRecord as ITaskData)
            ? (ganttRecord as ITaskData).isMilestone
            : null,
          ganttRecord as ITaskData
        ),
        1,
        'Day'
      ) /
   ...
   * @private
   */
  public getDateByLeft(
    left: number,
    isMilestone?: boolean,
    property?: ITaskData
  ): Date {
    let pStartDate: Date = new Date(
      this.parent.timelineModule.timelineStartDate.toString()
    );
    const milliSecondsPerPixel: number =
      (24 * 60 * 60 * 1000) / this.parent.perDayWidth;
    pStartDate.setTime(pStartDate.getTime() + left * milliSecondsPerPixel);
    /* To render the milestone in proper date while editing */
    if (
      isMilestone &&
      !isNullOrUndefined(property.predecessorsName) &&
      property.predecessorsName !== ''
    ) {
      //  pStartDate.setDate(pStartDate.getDate() -1);
      const dayEndTime: number = this.parent['getCurrentDayEndTime'](
        property.isAutoSchedule ? property.autoEndDate : property.endDate
      );
      this.parent.dateValidationModule.setTime(dayEndTime, pStartDate);
      pStartDate = this.parent.dateValidationModule.checkStartDate(
        pStartDate,
        property,
        true
      );
  ...
  private setItemPosition(): void {
    ...
    const segment: ITaskSegment = !isNullOrUndefined(item.segments)
      ? item.segments[this.segmentIndex]
      : null;
    const width: number =
      this.taskBarEditAction === 'MilestoneDrag' || item.isMilestone
        ? this.parent.chartRowsModule.milestoneHeight
        : item.width;
    let rightResizer: number = this.parent.isAdaptive ? width - 2 : width - 10;
    if (!isNullOrUndefined(segment)) {
      rightResizer = this.parent.isAdaptive
        ? segment.width - 2
        : segment.width - 10;
    }
    ...
    if (
      ...
      taskBarMainContainer = !this.taskBarEditElement.classList.contains(
        cls.taskBarMainContainer
      )
        ? (this.taskBarEditAction === 'ChildDrag' ||
            this.taskBarEditAction === 'LeftResizing') &&
          this.segmentIndex === 0
          ? this.taskBarEditElement.parentElement
          : closest(
              this.taskBarEditElement,
              'tr.' + cls.chartRow
    ...
    const segmentedTaskBarContainer: boolean =
      this.taskBarEditElement.classList.contains('e-segmented-taskbar') ||
      this.taskBarEditElement.querySelector('.e-segmented-taskbar')
        ? true
        : false;
    ...
    const childProgressResizer: HTMLElement = segmentedTaskBarContainer
      ? parentsUntil(
          this.taskBarEditElement,
          cls.taskBarMainContainer
        ).querySelector('.' + cls.childProgressResizer)
    ...
    const traceConnectorPointRight: HTMLElement =
      this.taskBarEditElement.querySelector(
        '.' + cls.rightConnectorPointOuterDiv
      );
    ...
    const manualTaskbar: HTMLElement = this.taskBarEditElement.querySelector(
      '.' + cls.manualParentTaskBar
    );
    const manualParentRight: HTMLElement =
      this.taskBarEditElement.querySelector('.' + cls.manualParentRightResizer);
    const manualParentLeft: HTMLElement = this.taskBarEditElement.querySelector(
      '.' + cls.manualParentLeftResizer
    );
    const resizeLine: HTMLElement =
      this.parent.ganttChartModule.chartBodyContainer.querySelector(
    ...
    if (
      ...
      if (
        ...
        if (
          segmentedTaskBarContainer &&
          !isNullOrUndefined(item.segments) &&
          (this.taskBarEditAction === 'RightResizing' ||
            this.segmentIndex !== 0)
        ) {
          ...
          (this.taskBarEditElement as HTMLElement).style.width =
            segment.width + 'px';
          if (this.parent.enableRtl) {
            (this.taskBarEditElement as HTMLElement).style.right =
              segment.left + 'px';
          } else {
            (this.taskBarEditElement as HTMLElement).style.left =
              segment.left + 'px';
            if (!isNullOrUndefined(resizeLine)) {
              if (
                !this.parent.allowTaskbarDragAndDrop ||
                ...
                resizeLine.style.left =
                  segment.left +
                  this.editElement.parentElement.offsetLeft +
                  'px';
              }
            }
          }
        }
        taskBarMainContainer.style.setProperty(
          position,
          (this.parent.allowTaskbarDragAndDrop ? this.leftValue : item.left) +
            'px'
        );
        taskBarMainContainer.style.width = width + 'px';
        if (segmentedTaskBarContainer && segmentConnectorPointRight) {
          segmentConnectorPointRight.style.left =
            (this.parent.isAdaptive ? width + 10 : width + 2) + 'px';
        }
        if (
          this.parent.allowTaskbarDragAndDrop &&
          this.parent.rowDragAndDropModule &&
        ...
        if (
          ...
          for (let i: number = 0; i < item.segments.length; i++) {
            ...
            const segmentElement: HTMLElement = segmentedTasks[
              i as number
            ] as HTMLElement;
            ...
            if (item.segments.length === 1) {
              ...
              if (i === 0) {
                resizeLine.style.width = segment.width + 'px';
                resizeLine.style.setProperty(
                  position,
                  segment.left + item.left + 'px'
                );
              }
              if (this.parent.enableRtl) {
                (segmentElement as HTMLElement).style.right =
                  segment.left + 'px';
              } else {
                (segmentElement as HTMLElement).style.left =
                  segment.left + 'px';
              }
            }
          }
      ...
      if (traceConnectorPointRight) {
        if (this.parent.enableRtl) {
          traceConnectorPointRight.style.left =
            (this.parent.isAdaptive ? width + 10 : width - 2) + 'px';
        } else {
          traceConnectorPointRight.style.left =
            (this.parent.isAdaptive ? width + 10 : width + 2) + 'px';
      ...
      if (this.taskBarEditAction === 'MilestoneDrag' || item.isMilestone) {
        taskBarMainContainer.style.setProperty(
          ...
          (this.parent.allowTaskbarDragAndDrop
            ? this.leftValue
            : item.left - width / 2) + 'px'
        );
        resizeLine.style.setProperty(position, item.left - width / 2 + 'px');
        resizeLine.style.width = width + 'px';
      } else if (this.taskBarEditAction === 'ProgressResizing') {
        if (this.segmentIndex === -1) {
          traceChildTaskBar.style.setProperty(
            position,
            item.left + item.progressWidth - 10 + 'px'
          );
          if (!isNullOrUndefined(traceChildProgressBar)) {
            traceChildProgressBar.style.width = item.progressWidth + 'px';
            traceChildProgressBar.style.borderBottomRightRadius =
              this.progressBorderRadius + 'px';
            traceChildProgressBar.style.borderTopRightRadius =
              this.progressBorderRadius + 'px';
            const width: number = this.parent.enableRtl
          ...
          const progressElements: NodeListOf<Element> =
            taskBarMainContainer.querySelectorAll('.e-gantt-child-progressbar');
          for (
            let count: number = 0;
            ...
            if (
              item.segments[count as number] &&
              item.segments[count as number].progressWidth &&
              this.segmentIndex !== count
            ) {
              progressElements[count as number]['style'].display = 'block';
              (progressElements[count as number] as HTMLElement).style.width =
                item.segments[count as number].progressWidth + 'px';
            }
          }
          traceChildProgressBar.style.width =
            item.segments[this.segmentIndex as number].progressWidth + 'px';
          traceChildProgressBar.style.borderBottomRightRadius =
            this.progressBorderRadius + 'px';
          traceChildProgressBar.style.borderTopRightRadius =
            this.progressBorderRadius + 'px';
        ...
        this.taskBarEditAction === 'RightResizing' &&
        !isNullOrUndefined(traceChildTaskBar)
      ) {
        resizeLine.style.width = width + 'px';
        ...
        if (!isNullOrUndefined(traceChildProgressBar)) {
          ...
          if (!isNullOrUndefined(childProgressResizer)) {
            childProgressResizer.style.setProperty(
              position,
              item.progressWidth - 10 + 'px'
            );
          }
        }
        ...
        this.editElement.parentElement.style.setProperty(
          'width',
          item.width + 'px'
        );
        ...
        resizeLine.style.width = item.width + 'px';
        resizeLine.style.setProperty(position, item.left + 'px');
        manualParentTaskbar.style.setProperty(
          position,
          item.left - item.autoLeft + 'px'
        );
        ...
        if (
          ...
          this.taskBarEditElement.style.left =
            (this.parent.allowTaskbarDragAndDrop ? this.leftValue : item.left) +
            'px';
          resizeLine.style.setProperty(position, item.left + 'px');
          resizeLine.style.width = item.width + 'px';
    ...
    if (
      ...
      const resizeValue: number = this.parent.enableRtl
        ? parseInt(resizeLine.style.right, 10)
        : parseInt(resizeLine.style.left, 10);
      if (this.parent.enableRtl) {
  ...
  public mouseUpHandler(e: PointerEvent): void {
    ...
    if (this.taskBarEditAction && this.isMouseDragged) {
      if (
        (!this.dragMouseLeave && this.taskBarEditedAction) ||
        this.parent.allowTaskbarDragAndDrop
      ) {
        if (
          e.type === 'touchmove' ||
          ...
          const coordinates: MousePoint = this.getCoordinate(e);
          this['droppedTarget'] = document.elementFromPoint(
            coordinates.pageX,
            coordinates.pageY
          );
        } else {
          if (
            this.taskBarEditRecord.hasChildRecords &&
            !this.taskBarEditRecord.ganttProperties.isAutoSchedule
          ) {
            const taskbarPosition: { top: number; left: number } =
              this.parent.getOffsetRect(this.taskBarEditElement);
            const left: number =
            ...
            if (this.parent.enableRtl) {
              ...
              xValue = Math.abs(
                Math.abs(taskbarPosition.left - pointerLeft - 1) -
                  this.parent.ganttChartModule.scrollElement.scrollLeft
              );
            }
          ...
          if (
            this['droppedTarget'] &&
            this['droppedTarget'].classList.contains('e-span-label')
          ) {
            this['droppedTarget'] = document.elementFromPoint(
              e.x + this['droppedTarget']['offsetWidth'],
              e.y
            );
          }
        }
    ...
    if (
      ...
      if (
        ...
        if (
          this.taskBarEditElement
            .querySelector('.e-gantt-child-taskbar')
            .classList.contains('e-collapsed-taskbar-drag')
        ) {
          removeClass(
            [this.taskBarEditElement.querySelector('.e-gantt-child-taskbar')],
            'e-collapsed-taskbar-drag'
  ...
  public taskBarEditedAction(event: PointerEvent): void {
    ...
    const e: MousePoint = this.getCoordinate(event);
    const resMouseY: number =
      e.pageY - this.parent.ganttChartModule.chartBodyContainer.offsetTop;
    if (
      (this.taskBarEditAction === 'ConnectorPointLeftDrag' ||
        this.taskBarEditAction === 'ConnectorPointRightDrag') &&
      !this.drawPredecessor
    ) {
    ...
    if (
      (this.taskBarEditAction === 'ConnectorPointLeftDrag' ||
        this.taskBarEditAction === 'ConnectorPointRightDrag') &&
      this.drawPredecessor &&
      (!this.connectorSecondRecord.hasChildRecords ||
        (this.connectorSecondRecord.hasChildRecords &&
      ...
      if (
        ...
        this.taskBarEditAction === 'ConnectorPointRightDrag' ||
        (this.oldData &&
          JSON.stringify(item.ganttProperties) ===
            JSON.stringify(this.oldData.ganttProperties))
      ) {
        if (
          this.parent.undoRedoModule &&
          ...
          this.parent.undoRedoModule['getUndoCollection'].splice(
            this.parent.undoRedoModule['getUndoCollection'].length - 1,
            1
          );
          if (this.parent.toolbarModule) {
      ...
      if (
        x1 !== x2 ||
        Math.abs(y1 - resMouseY) >=
          (this.parent.rowHeight - this.parent.taskbarHeight) / 2
      ) {
        if (item !== null) {
  ...
  public updateSegmentProgress(taskData: ITaskData): void {
    ...
    const totalTaskWidth: number =
      this.splitTasksDuration(segments) * this.parent.perDayWidth;
    const progress: number = Math.ceil(
      (taskData.progressWidth / totalTaskWidth) * 100
    );
  ...
  public taskbarEdited(arg: ITaskbarEditedEventArgs): void {
    ...
    if (
      ...
      if (this.parent.rowDragAndDropModule) {
        ...
        if (
          this.taskBarEditRecord.parentItem &&
          flatRecordCol[this.taskBarEditRecord.parentItem.index] &&
          this.parent.editedRecords.indexOf(
            flatRecordCol[this.taskBarEditRecord.parentItem.index as number]
          ) === -1
        ) {
          this.parent.editedRecords.push(
            flatRecordCol[this.taskBarEditRecord.parentItem.index as number]
          );
        }
        this.removeChildBorder();
        ...
        if (row) {
          ...
          const droppedParentRecordIndex: number = this.parent.getRootParent(
            droppedRecord,
            0
          ).index;
          const draggedParentRecordIndex: number = this.parent.getRootParent(
            this.taskBarEditRecord,
            0
          ).index;
          const e: Object = {
          ...
          if (
            !e['cancel'] &&
            droppedRecord &&
            recordIndex !== draggedRecIndex &&
            (droppedParentRecordIndex !== draggedParentRecordIndex ||
              (this.taskBarEditRecord.hasChildRecords &&
                droppedRecord.hasChildRecords) ||
              !this.taskBarEditRecord.hasChildRecords)
          ) {
            const droppedRecordIndex: number =
              this.parent.flatData.indexOf(droppedRecord);
            let position: string;
            if (
              this.parent.viewType === 'ProjectView' &&
              !isNullOrUndefined(
                this.parent.rowDragAndDropModule['dropPosition']
              )
            ) {
              ...
              position =
                droppedRecord.hasChildRecords ||
                (!droppedRecord.parentItem &&
                  droppedRecord.childRecords.length === 0)
                  ? 'child'
                  : this.parent.rowDragAndDropModule['dropPosition'];
            }
            if (this.parent.rowDragAndDropModule) {
              if (
                ...
                position === 'child' &&
                !droppedRecord.hasChildRecords &&
                !isNullOrUndefined(droppedRecord.parentItem)
              ) {
                position = 'Invalid';
                this.parent.rowDragAndDropModule['dropPosition'] = 'Invalid';
              }
              if (position) {
                this.parent.rowDragAndDropModule.reorderRows(
                  [draggedRecIndex as number],
                  droppedRecordIndex,
                  position
                );
              }
    ...
    if (args.taskBarEditAction === 'ProgressResizing') {
      if (args.previousData.progress !== taskData.progress) {
        ...
        /* eslint-disable-next-line */
        const updatedValues: { progressWidth: number; totalTaskWidth: number } =
          this.validateProgressWidth(
            args.data.ganttProperties,
            progressWidth,
            totalTaskWidth
          );
        this.parent.setRecordValue(
      ...
      if (
        ...
        const ganttProp: ITaskData = this.taskBarEditRecord.ganttProperties;
        const previousSegment: ITaskSegment =
          this.segmentIndex === 0 ? null : segments[this.segmentIndex - 1];
        const nextSegment: ITaskSegment =
          this.segmentIndex === segments.length - 1
            ? null
            : segments[this.segmentIndex + 1];
        const sDate: Date = !isNullOrUndefined(nextSegment)
          ? new Date(nextSegment.startDate.getTime())
          : this.parent.cloneProjectEndDate;
        const eDate: Date = !isNullOrUndefined(previousSegment)
          ? new Date(previousSegment.endDate.getTime())
        ...
        if (
          this.parent.timelineModule.topTier === 'Day' &&
        ...
        if (this.parent.includeWeekend) {
          ...
          if (
            cStartDate.getTime() <= eDate.getTime() &&
            !isNullOrUndefined(previousSegment) &&
            !isNullOrUndefined(segment)
          ) {
            ...
            cEndDate.getTime() >= sDate.getTime() &&
            this.segmentIndex !== segments.length - 1
          ) {
            const segmentIndexes: {
              firstSegmentIndex: number;
            ...
            segment.endDate.setDate(
              this.parent.cloneProjectEndDate.getDate() - 1
            );
          ...
          if (
            ...
            !this.parent.includeWeekend &&
            this.parent.dataOperation.getDuration(
              this.parent.dataOperation.checkStartDate(
                cEndDate,
                taskData,
                false
              ),
  ...
  private drawFalseLine(): void {
    ...
    const transform: string =
      'rotate(' + (this.parent.enableRtl ? -angle : angle) + 'deg)';
    let left: number;
    let width: number = 0;
    if (!isNullOrUndefined(document.querySelectorAll('.e-chart-row')[0])) {
      width = (document.querySelectorAll('.e-chart-row')[0] as HTMLElement)
        .offsetWidth;
    }
      ...
      if (this.parent.enableRtl) {
        left =
          ...
          (this.elementOffsetLeft +
            this.parent.chartRowsModule.connectorPointWidth / 2) -
          Math.abs(
            this.parent.ganttChartModule.scrollObject.previousScroll.left
          );
      } else {
    ...
    const top: number =
      ...
      this.elementOffsetHeight / 2 +
      this.parent.ganttChartModule.chartBodyContainer.offsetTop -
      this.parent.ganttChartModule.scrollObject.previousScroll.top;
    ...
    this.falseLine = createElement('div', {
      ...
      styles:
        ...
        transform +
        ';' +
        'border-top-width: 1px;border-top-style: dashed;z-index: 5;width:' +
        (length - 3) +
        'px;' +
        'top:' +
        top +
  ...
  public updateConnectorLineSecondProperties(e: PointerEvent): void {
    ...
    if (
      ...
      this.taskBarEditElement !== this.highlightedSecondElement
    ) {
      ...
      if (
        (this.parent.virtualScrollModule &&
          this.parent.enableVirtualization &&
          !this.elementOffsetLeft) ||
        !this.parent.enableVirtualization
      ) {
        if (!this.parent.allowParentDependency) {
          this.elementOffsetLeft =
            (this.realTaskbarElement as HTMLElement).offsetLeft -
            (isBigger ? 10 : 0);
          this.elementOffsetTop =
            (this.realTaskbarElement as HTMLElement).parentElement.offsetTop +
            (this.realTaskbarElement as HTMLElement).offsetHeight / 3 +
            scrollTop;
        } else {
          if (
            this.taskBarEditElement.children[0].classList.contains(
              'e-manualparent-main-container'
            )
          ) {
            this.elementOffsetLeft =
              (this.realTaskbarElement as HTMLElement).children[0][
                'offsetLeft'
              ] +
              (this.realTaskbarElement as HTMLElement).offsetLeft -
              (isBigger ? 10 : 0);
            this.elementOffsetTop =
              ...
              (this.realTaskbarElement as HTMLElement).offsetHeight / 3 -
              5 +
              this.taskBarEditElement.children[0]['offsetTop'] +
              scrollTop;
          } else {
            this.elementOffsetLeft =
              (this.realTaskbarElement as HTMLElement).offsetLeft -
        ...
        this.elementOffsetWidth =
          (this.realTaskbarElement as HTMLElement).offsetWidth +
          (isBigger ? 20 : 0);
        this.elementOffsetHeight = (
          this.realTaskbarElement as HTMLElement
        ).offsetHeight;
      }
      this.showHideTaskBarEditingElements(
        element,
        this.highlightedSecondElement,
        true
      );
    }
    if (
      isNullOrUndefined(this.connectorSecondAction) &&
      !isNullOrUndefined(this.connectorSecondElement) &&
      (!this.connectorSecondRecord.hasChildRecords ||
        (this.connectorSecondRecord.hasChildRecords &&
          this.parent.allowParentDependency))
    ) {
      ...
      removeClass(
        [
          this.connectorSecondElement.querySelector(
            '.' + cls.connectorPointLeft
          ),
        ],
        [cls.connectorPointAllowBlock]
      );
      removeClass(
        [
          this.connectorSecondElement.querySelector(
    ...
    this.connectorSecondRecord = isNullOrUndefined(this.connectorSecondElement)
      ? null
      : this.parent.ganttChartModule.getRecordByTaskBar(
          this.connectorSecondElement
        );
  }
  // eslint-disable-next-line
  private triggerDependencyEvent(e: PointerEvent, mouseUp?: boolean): void {
    const parentWithZoomStyle: Element =
      this.parent.element.closest('[style*="zoom"]');
    ...
    const toItem: ITaskData = this.connectorSecondRecord
      ? this.connectorSecondRecord.ganttProperties
      : null;
    ...
    const isValidLink: boolean =
      this.parent.connectorLineEditModule.validatePredecessorRelation(
        this.connectorSecondRecord,
        this.finalPredecessor
      );
    // eslint-disable-next-line
    const predecessorArray: IPredecessor[] =
      this.parent.predecessorModule.calculatePredecessor(
        predecessor,
        this.connectorSecondRecord
      );
    ...
    args.isValidLink =
      !isValidLink && args.isValidLink ? false : args.isValidLink;
    if (args.isValidLink) {
      ...
      if (!isNullOrUndefined(this.editTooltip.toolTipObj)) {
        ...
        const table: NodeList = this.parent.connectorLineModule.tooltipTable
          .querySelector('#toPredecessor')
          .querySelectorAll('td');
        (table[1] as HTMLElement).innerText = toItem.taskName;
        (table[2] as HTMLElement).innerText =
          this.parent.localeObj.getConstant(currentTarget);
        if (
          !isNullOrUndefined(
            this.parent.connectorLineModule.tooltipTable.parentElement
          ) &&
          !isNullOrUndefined(
            this.parent.connectorLineModule.tooltipTable.parentElement
              .parentElement
          ...
          const tooltipElement: HTMLElement =
            this.parent.connectorLineModule.tooltipTable.parentElement
              .parentElement;
          if (
            tooltipElement.offsetTop + tooltipElement.offsetHeight >
            zoomedPageY
          ) {
            tooltipElement.style.top =
              e.pageY - tooltipElement.offsetHeight - 20 + 'px';
          }
        }
      }
      ...
      if (this.parent.isAdaptive) {
        if (
          target.classList.contains(cls.connectorPointLeft) ||
