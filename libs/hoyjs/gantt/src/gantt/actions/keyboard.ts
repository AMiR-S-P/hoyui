...
import { ISelectedCell, IIndex } from '@hoyui/grids';
import { ContextMenu } from '@hoyui/navigations/src/context-menu';
...
export class FocusModule {
  ...
  public getActiveElement(isPreviousActiveElement?: boolean): HTMLElement {
    return isPreviousActiveElement
  ...
  public onKeyPress(e: KeyboardEventArgs): void | boolean {
    ...
    const expandedRecords: IGanttData[] = ganttObj.getExpandedRecords(
      ganttObj.currentViewData
    );
    if (
      isNullOrUndefined(this.parent.focusModule.getActiveElement()) &&
    ...
    if (
      ...
      ele.closest('.e-headercell') &&
      e.key === 'Enter' &&
      this.parent.sortModule &&
    ...
    if (
      ...
      e.action === 'leftArrow' ||
      e.action === 'focusTask' ||
      e.action === 'focusSearch' ||
      e.action === 'expandAll' ||
      e.action === 'collapseAll' ||
      e.action === 'undo' ||
      e.action === 'redo' ||
      e.action === 'selectAll'
    ) {
      if (
        !isNullOrUndefined(ganttObj.editModule) &&
        !isNullOrUndefined(ganttObj.editModule.cellEditModule) &&
    ...
    if (ganttObj.isAdaptive) {
      if (
        e.action === 'addRowDialog' ||
        e.action === 'editRowDialog' ||
        e.action === 'delete' ||
        e.action === 'addRow'
      ) {
        if (
          ...
          (<HTMLElement>(
            document.getElementsByClassName('e-gridpopup')[0]
          )).style.display = 'none';
        }
      }
    }
    switch (e.action) {
      case 'home':
        if (
          ...
          if (
            ganttObj.selectionModule &&
            ganttObj.selectionModule.getCellSelectedRecords().length === 0
          ) {
      ...
      case 'end':
        if (
          ...
          const currentSelectingRecord: IGanttData =
            expandedRecords[expandedRecords.length - 1];
          if (
            ganttObj.selectedRowIndex ===
            ganttObj.flatData.indexOf(currentSelectingRecord)
          ) {
            return;
          }
          if (
            ...
            ganttObj.selectionModule.selectRow(
              ganttObj.flatData.indexOf(currentSelectingRecord),
              false,
              true
            );
      ...
      case 'downArrow':
      case 'upArrow': {
        const searchElement: HTMLInputElement = ganttObj.element.querySelector(
          '#' + ganttObj.element.id + '_searchbar'
        );
        if (
          searchElement &&
          searchElement.parentElement.classList.contains('e-input-focus')
        ) {
        ...
        if (!ganttObj.element.classList.contains('e-scroll-disabled')) {
          ...
          if (
            ...
            ganttObj.ganttChartModule.manageFocus(
          ...
          if (
            ...
            e.target.classList.contains('e-cellselectionbackground')
          ) {
      ...
      case 'saveRequest':
        if (
          ...
          const col: ColumnModel =
            ganttObj.editModule.cellEditModule.editedColumn;
          if (
            col.field === ganttObj.columnMapping.duration &&
            !isNullOrUndefined(col.edit) &&
            !isNullOrUndefined(col.edit.read)
          ) {
            const textBox: TextBox = <TextBox>(
              (<EJ2Instance>e.target).ej2_instances[0]
            );
          ...
          if (
            ...
            getValue('dialogOpen', ganttObj.editModule.dialogModule.dialogObj)
          ) {
            return;
          }
          ganttObj.treeGrid.grid.saveCell();
          const focussedElement: HTMLElement = <HTMLElement>(
            ganttObj.element.querySelector('.e-treegrid')
          );
          focussedElement.focus();
        }
        ...
        if (
          ...
          const target: EventTarget = e.target;
          const taskbarElement: Element = targetElement.querySelector(
            '.e-gantt-parent-taskbar,' +
              '.e-gantt-child-taskbar,.e-gantt-milestone'
          );
          if (taskbarElement) {
            this.parent.ganttChartModule.onTaskbarClick(
              e,
              target,
              taskbarElement
            );
      ...
      case 'addRow': {
        if (
          ganttObj.editModule &&
          ganttObj.editModule.cellEditModule &&
          ganttObj.editModule.cellEditModule.isCellEdit
        ) {
          e.stopPropagation();
        } else if (
          isNullOrUndefined(
            document.getElementById(this.parent.element.id + '_dialog')
          )
        ) {
          e.preventDefault();
          ganttObj.addRecord(
      ...
      case 'addRowDialog':
        ...
        if (
          ...
          ganttObj.editSettings.allowAdding
        ) {
          if (
            ganttObj.editModule.dialogModule.dialogObj &&
            getValue('dialogOpen', ganttObj.editModule.dialogModule.dialogObj)
      ...
      case 'editRowDialog': {
        e.preventDefault();
        const focussedTreeElement: HTMLElement = <HTMLElement>(
          ganttObj.element.querySelector('.e-treegrid')
        );
        ...
        if (
          ganttObj.editModule &&
          ganttObj.editModule.dialogModule &&
          ganttObj.editSettings.allowEditing
        ) {
          if (
            ganttObj.editModule.dialogModule.dialogObj &&
            getValue('dialogOpen', ganttObj.editModule.dialogModule.dialogObj)
      ...
      case 'delete':
        if (
          ...
          (!ganttObj.editModule.dialogModule.dialogObj ||
            (ganttObj.editModule.dialogModule.dialogObj &&
              !ganttObj.editModule.dialogModule.dialogObj.visible)) &&
          (!ganttObj.editSettings.allowTaskbarEditing ||
            (ganttObj.editSettings.allowTaskbarEditing &&
              !ganttObj.editModule.taskbarEditModule.touchEdit))
        ) {
          if (
            (ganttObj.selectionSettings.mode !== 'Cell' &&
              ganttObj.selectionModule.selectedRowIndexes.length) ||
            (ganttObj.selectionSettings.mode === 'Cell' &&
              ganttObj.selectionModule.getSelectedRowCellIndexes().length)
          ) {
      ...
      case 'focusTask': {
        ...
        if (ganttObj.selectionModule) {
          ...
          if (
            ...
            selectedId =
              ganttObj.currentViewData[ganttObj.selectedRowIndex]
                .ganttProperties.rowUniqueID;
          } else if (
            ganttObj.selectionSettings.mode === 'Cell' &&
            ganttObj.selectionModule.getSelectedRowCellIndexes().length > 0
          ) {
            const selectCellIndex: ISelectedCell[] =
              ganttObj.selectionModule.getSelectedRowCellIndexes();
            selectedId =
              currentViewData[
                selectCellIndex[selectCellIndex.length - 1].rowIndex
              ].ganttProperties.rowUniqueID;
          }
        }
        if (selectedId) {
      ...
      case 'focusSearch': {
        if (
          ...
          const searchElement: HTMLInputElement = <HTMLInputElement>(
            ganttObj.element.querySelector(
              '#' + ganttObj.element.id + '_searchbar'
            )
          );
      ...
      case 'contextMenu': {
        const contextMenu: ContextMenu = <ContextMenu>(
          (<EJ2Instance>(
            document.getElementById(this.parent.element.id + '_contextmenu')
          )).ej2_instances[0]
        );
        ...
        const top: number =
          containerPosition.top + containerPosition.height / 2;
        const left: number =
          containerPosition.left + containerPosition.width / 2;
      ...
      case 'redo': {
        if (
          this.parent.undoRedoModule &&
          this.parent.undoRedoModule['getRedoCollection'].length > 0
        ) {
          this.parent.redo();
        }
        break;
      ...
      case 'selectAll': {
        e.preventDefault();
        const ganttRow: HTMLElement[] = [].slice.call(
          this.parent.ganttChartModule.chartBodyContent.querySelector('tbody')
            .children
        );
        if (ganttRow.length > 0) {
          let firstRowIndex: any = ganttRow[0].getAttribute('data-rowindex');
          let lastRowIndex: any =
            ganttRow[ganttRow.length - 1].getAttribute('data-rowindex');
          ...
          if (
            !isNullOrUndefined(firstRowIndex) &&
            !isNullOrUndefined(lastRowIndex)
          ) {
            this.parent.selectionModule.selectRowsByRange(
              firstRowIndex,
              lastRowIndex
            );
          }
  ...
  private upDownKeyNavigate(e: KeyboardEventArgs): void {
    ...
    if (ganttObj.selectionModule) {
      if (
        ...
        const focussedElement: HTMLElement = <HTMLElement>(
          ganttObj.element.querySelector('.e-focused')
        );
        if (focussedElement) {
        ...
        if (document.activeElement !== activeElement) {
          if (
            ...
            ganttObj.selectionModule.selectRow(
              ganttObj.currentViewData.indexOf(currentSelectingRecord),
              false,
              true
            );
          }
        ...
        const selectCellIndex: ISelectedCell[] =
          ganttObj.selectionModule.getSelectedRowCellIndexes();
        const selectedCellItem: ISelectedCell =
          selectCellIndex[selectCellIndex.length - 1];
        const currentCellIndex: number =
          selectedCellItem.cellIndexes[selectedCellItem.cellIndexes.length - 1];
        const selectedItem: IGanttData =
          ganttObj.currentViewData[selectedCellItem.rowIndex];
        const selectingRowIndex: number = expandedRecords.indexOf(selectedItem);
        const currentSelectingRecord: IGanttData =
          e.action === 'downArrow'
            ? expandedRecords[selectingRowIndex + 1]
      ...
      this.parent.ganttChartModule.focusedRowIndex =
        this.parent.selectedRowIndex;
    }
  }
  private expandCollapseKey(e: KeyboardEventArgs): void {
    ...
    if (ganttObj.selectionModule && ganttObj.selectedRowIndex !== -1) {
      ...
      if (this.parent.virtualScrollModule && this.parent.enableVirtualization) {
        selectedRowIndex =
          this.parent.currentViewData.findIndex(
            (obj: IGanttData) =>
              obj.ganttProperties.rowUniqueID === selectedRowIndex.toString()
          ) + 1;
      }
