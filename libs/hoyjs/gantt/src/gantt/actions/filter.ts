import {
  TreeGrid,
  Filter as TreeGridFilter,
  FilterSettingsModel as TreeFilterSettingsModel,
} from '@hoyui/treegrid';
import {
  FilterEventArgs,
  filterAfterOpen,
...
export class Filter {
  ...
  constructor(gantt: Gantt) {
    ...
    this.parent.treeGrid.filterSettings = getActualProperties(
      this.parent.filterSettings
    ) as TreeFilterSettingsModel;
    ...
    for (let i: number = 0; i < this.parent.ganttColumns.length; i++) {
      ...
      if (
        ((column.editType === 'datepickeredit' ||
          ...
          (column.field === settings.startDate ||
            column.field === settings.endDate ||
            column.field === settings.baselineStartDate ||
            column.field === settings.baselineEndDate)) ||
  ...
  private updateModel(): void {
    this.parent.filterSettings = this.parent.treeGrid
  ...
  private initiateFiltering(column: ColumnModel): void {
    const treeColumn: ColumnModel = this.parent.getColumnByField(
      column.field,
  ...
  private getCustomFilterUi(column: ColumnModel): IFilterMUI {
    ...
    if (
      ...
      column.field === settings.duration &&
  ...
  private getDatePickerFilter(columnName: string): IFilterMUI {
    ...
    const timeValue: number =
      columnName === parent.taskFields.startDate ||
      columnName === parent.taskFields.baselineStartDate
        ? parent.defaultStartTime
        : parent.defaultEndTime;
    let dropDateInstance: DatePicker;
    const filterDateUI: IFilterMUI = {
      create: (args: { target: Element; column: ColumnModel }) => {
        const format: string = getCustomDateFormat(
          args.column.format,
          args.column.type
        );
  ...
  private getDurationFilter(): IFilterMUI {
    ...
    const filterDurationUI: IFilterMUI = {
      create: (args: { target: Element; column: Object }) => {
        ...
        flValInput.setAttribute(
          'placeholder',
          this.parent.localeObj.getConstant('enterValue')
        );
        args.target.appendChild(flValInput);
      ...
      write: (args: { filteredValue: string }) => {
      ...
      read: (args: {
        ...
        const durationObj: object = this.parent.dataOperation.getDurationValue(
  ...
  private columnMenuOpen(args: ColumnMenuOpenEventArgs): void {
    // eslint-disable-line
    if (
      this.filterMenuElement &&
      document.body.contains(this.filterMenuElement)
    ) {
  ...
  public closeFilterOnContextClick(element: Element): void {
    const datePickerElement: Element = document.querySelector(
      'body > div.e-datepicker'
    ) as HTMLElement;
    const dateTimePickerElement: Element = document.querySelector(
      'body > div.e-datetimepicker'
    ) as HTMLElement;
    if (
      this.filterMenuElement &&
      document.body.contains(this.filterMenuElement)
    ) {
      const ganttElement: Element =
        closest(element, '#' + this.parent.element.id) ||
        element.querySelector('#' + this.parent.element.id);
      if (
        (!this.filterMenuElement.contains(element) &&
          !isNullOrUndefined(ganttElement)) ||
        (!this.filterMenuElement.contains(element) &&
          isNullOrUndefined(datePickerElement) &&
          isNullOrUndefined(dateTimePickerElement) &&
          (element.nodeName === 'DIV' ||
            element.nodeName === 'HTML' ||
            element.nodeName === 'SPAN' ||
            element.nodeName === 'BUTTON') &&
          !element.classList.contains('e-dropdownbase'))
      ) {
        remove(this.filterMenuElement);
  ...
  private actionComplete(args: GroupEventArgs): void {
    ...
    if (args.requestType === filterAfterOpen) {
      if (this.parent.treeGrid.filterSettings.type === 'Menu') {
        ...
        this.filterMenuElement = getValue(
          'filterModel.dialogObj.element',
          args
        );
      }
      ...
      if (
        (args.columnName === predecessor &&
          isNullOrUndefined(getValue(predecessor, filterValues))) ||
        (args.columnName === resource &&
          isNullOrUndefined(getValue(resource, filterValues)))
      ) {
        const element: HTMLElement =
          this.filterMenuElement.querySelector('.e-dropdownlist');
        ...
        args.columnName === taskID &&
        isNullOrUndefined(getValue(taskID, filterValues)) &&
        this.parent.treeGrid.filterSettings.type === 'Menu'
      ) {
        const element: HTMLElement =
          this.filterMenuElement.querySelector('.e-flmenu-input');
        const instanceObj: NumericTextBox = getValue(
          'ej2_instances[0]',
          element
        );
        if (
          !isNullOrUndefined(instanceObj) &&
          isNullOrUndefined(this.parent.columnByField[args.columnName].format)
        ) {
  ...
  private setPosition(li: Element, ul: HTMLElement): void {
    ...
    if (
      !isNullOrUndefined(this.parent.element.parentNode) &&
      this.parent.element.parentNode['tagName'] !== 'BODY'
    ) {
      isNonBodyTag = true;
    ...
    let top: number = isNonBodyTag
      ? liPos.top - gridPosTop
      : liPos.top + window.scrollY;
    if (gridPos.right < left + ul.offsetWidth) {
      ...
      if (
        (!isNullOrUndefined(paddingTop) || !isNullOrUndefined(paddingLeft)) &&
        isNonBodyTag
      ) {
    ...
    if (!isNullOrUndefined(paddingTop) && !isNullOrUndefined(paddingLeft)) {
      ul.style.top =
        typeof parseInt(marginTop as string, 10) === 'string'
          ? top + paddingTop + parseInt(marginTop as string, 10) + 'px'
          : top + paddingTop + 'px';
      ul.style.left =
        typeof parseInt(marginLeft as string, 10) === 'string'
          ? left + paddingLeft + parseInt(marginLeft as string, 10) + 8 + 'px'
          : left + paddingLeft + 'px';
    } else {
      ul.style.top = top + 'px';
  ...
  private updateFilterMenuPosition(
    ...
    if (this.parent.showColumnMenu) {
      targetElement = document.querySelector(
        '#treeGrid' + this.parent.controlId + '_gridcontrol_colmenu_Filter'
      );
      element.style.zIndex = targetElement.parentElement.style.zIndex;
      if (this.parent.treeGrid.filterSettings.type === 'Menu') {
        this.setPosition(
          targetElement,
          getValue('filterModel.dlgObj.element', args)
        );
      } else {
        this.setPosition(
          targetElement,
          getValue('filterModel.dialogObj.element', args)
        );
      }
    } else {
      targetElement = this.parent.treeGrid.grid
        .getColumnHeaderByField(args.columnName)
        .querySelector('.e-filtermenudiv');
      if (this.parent.treeGrid.filterSettings.type === 'Menu') {
        getFilterMenuPostion(
          targetElement,
          getValue('filterModel.dlgObj', args)
        );
      } else {
        getFilterMenuPostion(
          targetElement,
          getValue('filterModel.dialogObj', args)
        );
      }
    }
    if (this.parent.treeGrid.filterSettings.type === 'Menu') {
    ...
    if (this.parent.treeGrid.filterSettings.type === 'Excel') {
      const inputElement: HTMLElement = document.querySelector(
        '.e-searchinput'
      ) as HTMLElement;
      inputElement.focus();
    }
