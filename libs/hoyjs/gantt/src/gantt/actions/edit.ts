...
import { Dialog } from '@hoyui/popups';
import { NumericTextBoxModel } from '@hoyui/inputs';
import { MultiSelect, CheckBoxSelection, DropDownList } from '@hoyui/dropdowns';
import { ConnectorLineEdit } from './connector-line-edit';
import { ITreeData, TreeGrid, Edit as TreeGridEdit } from '@hoyui/treegrid';
...
export class Edit {
  ...
  constructor(parent?: Gantt) {
    ...
    if (
      ...
      this.parent.editSettings.mode === 'Auto'
    ) {
    ...
    if (
      this.parent.editSettings.allowAdding ||
      (this.parent.editSettings.allowEditing &&
    ...
    this.parent.treeGrid.editSettings.allowAdding =
      this.parent.editSettings.allowAdding;
    this.parent.treeGrid.editSettings.allowDeleting =
      this.parent.editSettings.allowDeleting;
    this.parent.treeGrid.editSettings.showDeleteConfirmDialog =
      this.parent.editSettings.showDeleteConfirmDialog;
    this.parent.treeGrid.editSettings.allowNextRowEdit =
  ...
  private updateDefaultColumnEditors(): void {
    const customEditorColumns: string[] = [
      this.parent.taskFields.id,
      this.parent.taskFields.progress,
      this.parent.taskFields.resourceInfo,
      this.parent.taskFields.type,
      'taskType',
    ];
    for (let i: number = 0; i < customEditorColumns.length; i++) {
      if (
        !isNullOrUndefined(customEditorColumns[i as number]) &&
        customEditorColumns[i as number].length > 0
      ) {
        const column: ColumnModel = this.parent.getColumnByField(
          customEditorColumns[parseInt(i.toString(), 10)],
        ...
        if (column) {
          if (column.field === this.parent.taskFields.id) {
            ...
            column.field === this.parent.taskFields.progress &&
            isNullOrUndefined(column.edit)
          ) {
  ...
  private updateEditParams(column: ColumnModel, editParam: object): void {
    ...
    const ganttColumn: ColumnModel = this.parent.getColumnByField(
      column.field,
  ...
  private getResourceEditor(): IEditCell {
    ...
    editObject.write = (args: {
      ...
      editor = new MultiSelect({
        ...
        popupHeight: '350px',
        delimiterChar: ',',
        value: this.parent.treeGridModule.getResourceIds(
          args.rowData as IGanttData
        ) as number[],
      });
    ...
    editObject.read = (element: HTMLElement): string => {
      ...
      for (let i: number = 0; i < value.length; i++) {
        for (let j: number = 0; j < this.parent.resources.length; j++) {
          if (
            this.parent.resources[j as number][resourceSettings.id] ===
            value[i as number]
          ) {
            resourcesName.push(
              this.parent.resources[j as number][resourceSettings.name]
            );
            break;
          }
        }
  ...
  private getTaskTypeEditor(): IEditCell {
    ...
    const types: Object[] = [
      { ID: 1, Value: 'FixedUnit' },
      { ID: 2, Value: 'FixedWork' },
      { ID: 3, Value: 'FixedDuration' },
    ];
    editObject.write = (args: {
      rowData: Object;
      element: Element;
      ...
      editor = new DropDownList({
        ...
        value: getValue(
          'taskType',
          (args.rowData as IGanttData).ganttProperties
        ),
      });
      editor.appendTo(args.element as HTMLElement);
    };
  ...
  public reUpdateEditModules(): void {
    ...
    if (editSettings.allowEditing) {
      if (
        ...
        this.parent.treeGrid.recordDoubleClick =
          this.recordDoubleClick.bind(this);
      } else if (
        isNullOrUndefined(this.parent.editModule.cellEditModule) &&
  ...
  public updateRecordByID(data: Object): void {
    if (
      this.parent.enableImmutableMode &&
      this.parent.editSettings.allowEditing &&
      this.parent.treeGrid.element.getElementsByClassName('e-editedbatchcell')
        .length > 0
    ) {
    ...
    if (!this.parent.readOnly) {
      ...
      const ganttData: IGanttData =
        this.parent.viewType === 'ResourceView'
          ? this.parent.flatData[
              this.parent.getTaskIds().indexOf('T' + data[tasks.id])
            ]
          : this.parent.getRecordByID(data[tasks.id]);
      ...
      if (
        ...
        !this.parent.undoRedoModule['isUndoRedoPerformed'] &&
        this.parent['isUndoRedoItemPresent']('Edit') &&
        ganttData
      ) {
        this.parent.undoRedoModule['createUndoCollection']();
        const details: Object = {};
        details['requestType'] =
          this.parent.contextMenuModule && this.parent.contextMenuModule.item
            ? this.parent.contextMenuModule.item
            : 'methodUpdate';
        details['modifiedRecords'] = extend([], [ganttData], [], true);
        (this.parent.undoRedoModule['getUndoCollection'][
          this.parent.undoRedoModule['getUndoCollection'].length - 1
        ] as object) = details;
      }
      if (!isNullOrUndefined(this.parent.editModule) && ganttData) {
        ...
        if (
          ...
          if (
            ...
            data['ganttProperties'].predecessor
          ) {
            for (
              let i: number = 0;
              i < data['ganttProperties'].predecessor.length;
              i++
            ) {
              const isValid: IPredecessor[] =
                ganttData.ganttProperties.predecessor.filter(
                  (pred: IPredecessor) => {
                    if (
                      pred.from ===
                        data['ganttProperties'].predecessor[i as number].from &&
                      pred.to ===
                        data['ganttProperties'].predecessor[i as number].to &&
                      data['ganttProperties'].predecessor[i as number]
                        .offset !== pred.offset
                    ) {
                      const record: IGanttData =
                        this.parent.flatData[this.parent.ids.indexOf(pred.to)];
                      const changeValue: IPredecessor[] =
                        record.ganttProperties.predecessor.filter(
                          (pred: IPredecessor) => {
                            return (
                              pred.from ===
                                data['ganttProperties'].predecessor[i as number]
                      ...
                      changeValue[0].offset =
                        data['ganttProperties'].predecessor[i as number].offset;
                    }
                    return (
                      pred.from !==
                        data['ganttProperties'].predecessor[i as number].from &&
              ...
              if (isValid.length > 0) {
                for (let j: number = 0; j < isValid.length; j++) {
                  const record: IGanttData =
                    this.parent.flatData[
                      this.parent.ids.indexOf(isValid[j as number].from)
                    ];
                  for (
                    let k: number = 0;
                    k < record.ganttProperties.predecessor.length;
                    k++
                  ) {
                    if (
                      record.ganttProperties.predecessor[k as number].from ===
                        isValid[j as number].from &&
                      record.ganttProperties.predecessor[k as number].to ===
                        isValid[j as number].to
                    ) {
            ...
            !data['ganttProperties'].predecessor &&
            ganttData.ganttProperties.predecessor
          ) {
            for (
              let i: number = 0;
              i < ganttData.ganttProperties.predecessor.length;
              i++
            ) {
              let id: string;
              if (
                ganttData.ganttProperties.taskId.toString() ===
              ...
              const parentRec: IGanttData =
                this.parent.flatData[this.parent.ids.indexOf(id)];
              for (
                let j: number = 0;
                j < parentRec.ganttProperties.predecessor.length;
                j++
              ) {
                if (
                  parentRec.ganttProperties.predecessor[j as number].from ===
                    ganttData.ganttProperties.predecessor[i as number].from &&
          ...
          ganttData.ganttProperties.resourceInfo =
            data['ganttProperties'].resourceInfo;
        }
        ...
        if (
          keys.indexOf(tasks.startDate) !== -1 ||
          keys.indexOf(tasks.endDate) !== -1 ||
          keys.indexOf(tasks.duration) !== -1
        ) {
          this.parent.dataOperation.calculateScheduledValues(
            ganttData,
            ganttData.taskData,
            false
          );
        ...
        if (
          !isUndefined(data[this.parent.taskFields.dependency]) &&
          data[this.parent.taskFields.dependency] !==
            ganttData.ganttProperties.predecessorsName
        ) {
  ...
  public validateUpdateValues(
    ...
    for (const key of Object.keys(data)) {
      if (
        [tasks.startDate, tasks.endDate, tasks.duration].indexOf(key) !== -1
      ) {
        if (
          isNullOrUndefined(data[`${key}`]) &&
          !ganttObj.allowUnscheduledTasks
        ) {
          continue;
        }
        if (isFromDialog) {
          if (tasks.duration === key) {
            ganttObj.dataOperation.updateDurationValue(
              data[key as string],
              ganttData.ganttProperties
            );
            if (
              ganttData.ganttProperties.duration > 0 &&
            ...
            ganttObj.dataOperation.updateMappingData(
              ganttData,
              ganttPropByMapping[key as string]
            );
          } else {
            let tempDate: Date =
              typeof data[key as string] === 'string'
                ? new Date(data[key as string] as string)
                : data[key as string];
            if (
              key === tasks.endDate &&
              isNullOrUndefined(ganttData.ganttProperties.startDate) &&
              (isNullOrUndefined(data[tasks.duration]) ||
                data[tasks.duration] === '' ||
                Number.isNaN(data[tasks.duration]))
            ) {
        ...
        const resourceData: Object[] =
          ganttObj.dataOperation.setResourceInfo(data);
        if (this.parent.viewType === 'ResourceView') {
          if (
            JSON.stringify(resourceData) !==
            JSON.stringify(ganttData.ganttProperties.resourceInfo)
          ) {
        ...
        if (key === tasks.id) {
          ...
          key === tasks.segments &&
          !isNullOrUndefined(ganttData.ganttProperties.segments)
        ) {
          ganttPropKey = 'segments';
          /* eslint-disable-next-line */
          if (
            data &&
            !isNullOrUndefined(data[this.parent.taskFields.segments]) &&
            data[this.parent.taskFields.segments].length > 0
          ) {
            if (
              this.parent.undoRedoModule &&
              this.parent.undoRedoModule['isUndoRedoPerformed']
            ) {
              ganttData.ganttProperties.segments =
                data['ganttProperties'].segments;
            }
            let totDuration: number = 0;
            for (
              let i: number = 0;
              i < ganttData.ganttProperties.segments.length;
              i++
            ) {
              totDuration =
                totDuration +
                ganttData.ganttProperties.segments[i as number].duration;
            }
            const sdate: Date = ganttData.ganttProperties.startDate;
            /* eslint-disable-next-line */
            const edate: Date = this.parent.dataOperation.getEndDate(
              sdate,
              totDuration,
            ...
            ganttObj.setRecordValue(
              'endDate',
              ganttObj.dataOperation.getDateFromFormat(edate),
              ganttData.ganttProperties,
              true
            );
          }
          ...
          if (ganttPropKey === 'segments' && value && value.length > 0) {
            for (let i: number = 0; i < value.length; i++) {
              ...
              if (value[i as number][this.parent.taskFields.startDate]) {
                segment['startDate'] =
                  value[i as number][this.parent.taskFields.startDate];
              }
              if (value[i as number][this.parent.taskFields.endDate]) {
                segment['endDate'] =
                  value[i as number][this.parent.taskFields.endDate];
              }
              if (value[i as number][this.parent.taskFields.duration]) {
                segment['duration'] = parseFloat(
                  value[i as number][this.parent.taskFields.duration]
                );
              }
        ...
        if (
          (key === tasks.baselineStartDate || key === tasks.baselineEndDate) &&
          ganttData.ganttProperties.baselineStartDate &&
          ganttData.ganttProperties.baselineEndDate
        ) {
          ganttObj.setRecordValue(
            'baselineStartDate',
            ganttObj.dataOperation.checkBaselineStartDate(
              ganttData.ganttProperties.baselineStartDate,
              ganttData.ganttProperties
            ),
            ganttData.ganttProperties,
          ...
          if (
            ganttData.ganttProperties.baselineEndDate &&
            ganttData.ganttProperties.baselineEndDate.getHours() === 0 &&
            dayEndTime !== 86400
          ) {
            ganttObj.dataOperation.setTime(
              dayEndTime,
              ganttData.ganttProperties.baselineEndDate
            );
          }
          if (
            (ganttData.ganttProperties.baselineStartDate &&
              ganttData.ganttProperties.baselineEndDate &&
              ganttData.ganttProperties.baselineStartDate.getTime() >
                ganttData.ganttProperties.baselineEndDate.getTime()) ||
            (!isNullOrUndefined(ganttData.ganttProperties.baselineStartDate) &&
              !isNullOrUndefined(ganttData.ganttProperties.startDate) &&
              ganttData.ganttProperties.baselineStartDate.getTime() ===
                ganttData.ganttProperties.startDate.getTime() &&
          ...
          ganttObj.setRecordValue(
            'baselineEndDate',
            ganttObj.dataOperation.checkBaselineEndDate(
              ganttData.ganttProperties.baselineEndDate
            ),
            ganttData.ganttProperties,
            true
          ...
          ganttObj.setRecordValue(
            'baselineLeft',
            ganttObj.dataOperation.calculateBaselineLeft(
              ganttData.ganttProperties
            ),
            ganttData.ganttProperties,
            true
          ...
          ganttObj.setRecordValue(
            'baselineWidth',
            ganttObj.dataOperation.calculateBaselineWidth(
              ganttData.ganttProperties
            ),
            ganttData.ganttProperties,
            true
   ...
   * @returns {void} .
   */
  public updateResourceRelatedFields(
    ...
    const resources: Object[] =
      this.parent.editModule.dialogModule &&
      this.parent.editModule.dialogModule['currentResources']
        ? this.parent.editModule.dialogModule['currentResources']
      ...
      switch (taskType) {
        ...
        case 'FixedDuration':
          ...
          if (
            resources.length &&
            (column === 'work' ||
              (isAutoSchedule && isEffectDriven && column === 'resource'))
          ) {
            if (column === 'work') {
              this.parent.dataOperation.updateUnitWithWork(currentData);
            } else {
  ...
  private validateScheduleValues(
    ...
    if (fieldNames.length > 2) {
      ...
      this.dialogModule.validateScheduleValuesByCurrentField(
        fieldNames[0],
        data[fieldNames[0]],
        ganttData
      );
  ...
  private validateScheduleByTwoValues(
    ...
    if (
      (startDate && endDate) ||
      (isUnscheduledTask &&
        fieldNames.indexOf(tasks.startDate) !== -1 &&
        fieldNames.indexOf(tasks.endDate) !== -1)
    ) {
      ganttObj.setRecordValue(
        'startDate',
        ganttObj.dataOperation.getDateFromFormat(startDate),
        ganttProp,
        true
      );
      ganttObj.setRecordValue(
        'endDate',
        ganttObj.dataOperation.getDateFromFormat(endDate),
        ganttProp,
        true
      );
  ...
  private isTaskbarMoved(data: IGanttData): boolean {
    ...
    const prevData: IGanttData =
      this.parent.previousRecords && this.parent.previousRecords[data.uniqueID];
    if (prevData && prevData.ganttProperties) {
      const prevStart: Date = getValue(
        'ganttProperties.startDate',
        prevData
      ) as Date;
      const prevEnd: Date = getValue(
        'ganttProperties.endDate',
        prevData
      ) as Date;
      const prevDuration: number = getValue(
        'ganttProperties.duration',
        prevData
      );
      const prevDurationUnit: string = getValue(
        'ganttProperties.durationUnit',
        prevData
      );
      const keys: string[] = Object.keys(prevData.ganttProperties);
      if (
        keys.indexOf('startDate') !== -1 ||
        keys.indexOf('endDate') !== -1 ||
        keys.indexOf('duration') !== -1 ||
        keys.indexOf('durationUnit') !== -1
      ) {
        if (
          (isNullOrUndefined(prevStart) &&
            !isNullOrUndefined(taskData.startDate)) ||
          (isNullOrUndefined(prevEnd) &&
            !isNullOrUndefined(taskData.endDate)) ||
          (isNullOrUndefined(taskData.startDate) &&
            !isNullOrUndefined(prevStart)) ||
          (isNullOrUndefined(taskData.endDate) &&
            !isNullOrUndefined(prevEnd)) ||
          ...
          (!isNullOrUndefined(prevDuration) &&
            prevDuration !== taskData.duration) ||
          (!isNullOrUndefined(prevDuration) &&
            prevDuration === taskData.duration &&
  ...
  private isPredecessorUpdated(data: IGanttData): boolean {
    ...
    if (
      ...
      prevData.ganttProperties.hasOwnProperty('predecessor')
    ) {
      if (
        data.ganttProperties.predecessorsName !==
        prevData.ganttProperties.predecessorsName
      ) {
  ...
  public updateGanttProperties(data: IGanttData, updateData: IGanttData): void {
    ...
    Object.keys(data.ganttProperties).forEach((property: string) => {
      if (skipProperty.indexOf(property) === -1) {
        updateData.ganttProperties[property as string] =
          data.ganttProperties[property as string];
      }
    });
  }
  /**
  ...
  public initiateUpdateAction(args: ITaskbarEditedEventArgs): void {
    ...
    if (!isNullOrUndefined(args.data.parentItem) && !isValidatePredecessor) {
      ...
      if (
        this.isTaskbarMoved(args.data) &&
        this.parent.predecessorModule.getValidPredecessor(parentData).length >
          0 &&
        this.parent.isInPredecessorValidation
      ) {
        isValidatePredecessor = true;
      ...
      if (!isValidatePredecessor && this.isTaskbarMoved(args.data)) {
        for (let i: number = 0; i < args.data.childRecords.length; i++) {
          if (
            this.parent.predecessorModule.getValidPredecessor(
              args.data.childRecords[i as number]
            ).length > 0
          ) {
            childRecordIndex = i;
            isValidatePredecessor = true;
    ...
    if (
      !this.parent.undoRedoModule ||
      (this.parent.undoRedoModule &&
        !this.parent.undoRedoModule['currentAction']) ||
      (this.parent.undoRedoModule['currentAction'] &&
        this.parent.undoRedoModule['currentAction']['action'] !== 'indent' &&
    ...
    if (
      isValidatePredecessor &&
      this.parent.autoCalculateDateScheduling &&
      !(
        this.parent.isLoad &&
        this.parent.treeGrid.loadChildOnDemand &&
      ...
      if (!isNullOrUndefined(parentData)) {
        ...
        validateObject = this.parent.connectorLineEditModule.validateTypes(
          args.data.childRecords[childRecordIndex as number],
          args.data
        );
      } else {
        validateObject = this.parent.connectorLineEditModule.validateTypes(
      ...
      if (!isNullOrUndefined(getValue('violationType', validateObject))) {
        ...
        if (
          newArgs.validateMode.preserveLinkWithEditing === false &&
          newArgs.validateMode.removeLink === false &&
          newArgs.validateMode.respectLink === false
        ) {
          this.parent.connectorLineEditModule.openValidationDialog(
            validateObject
          );
        } else {
          if (
            ...
            this.predecessorUpdated
          ) {
            this.isValidatedEditedRecord = true;
            this.parent.predecessorModule.validatePredecessor(
              args.data,
              [],
              ''
            );
  ...
  private validateTaskEvent(
    editedEventArgs: ITaskbarEditedEventArgs
  ): IValidateArgs {
  ...
  public updateEditedTask(args: ITaskbarEditedEventArgs): void {
    ...
    if (
      (this.parent.isConnectorLineUpdate ||
        (this.parent.undoRedoModule &&
          this.parent.undoRedoModule['isUndoRedoPerformed'])) &&
      this.parent.autoCalculateDateScheduling &&
      !(
      ...
      for (
        let i: number = 0;
        i < this.parent.predecessorModule.validatedChildItems.length;
        i++
      ) {
        const child: IGanttData =
          this.parent.predecessorModule.validatedChildItems[i as number];
        if (
          child.ganttProperties.predecessor &&
          child.ganttProperties.predecessor.length > 0
        ) {
      ...
      if (
        ...
        this.parent.previousRecords[ganttRecord.uniqueID].ganttProperties
          .startDate &&
      ...
      if (
        ...
        for (let i: number = 0; i < ganttRecord.childRecords.length; i++) {
          if (
            ganttRecord.childRecords[i as number].ganttProperties.predecessor
          ) {
            this.parent.predecessorModule.validatePredecessor(
              ganttRecord.childRecords[i as number],
              [],
              ''
            );
          }
    ...
    if (ganttRecord.parentItem) {
      ...
      const parentData: IGanttData = this.parent.getRecordByID(
        ganttRecord.parentItem.taskId
      );
      if (!isNullOrUndefined(parentData)) {
    ...
    if (
      ...
      this.parent.connectorLineEditModule &&
      args.data
    ) {
  ...
  private updateParentItemOnEditing(): void {
    const childRecord: object[] = getValue(
      'parentRecord',
      this.parent.predecessorModule
    );
    ...
    if (
      ...
      this.parent.editModule['updateParentRecords'].length > 0
    ) {
      this.parent.editModule['updateParentRecords'].forEach(
        (record: IGanttData) => {
          if (
            record.ganttProperties.predecessor &&
  ...
  public updateParentChildRecord(data: IGanttData): void {
    ...
    if (
      ganttRecord.hasChildRecords &&
      this.taskbarMoved &&
      (ganttRecord[this.parent.taskFields.manual] === false ||
        this.parent.taskMode === 'Auto') &&
      !isNullOrUndefined(this.parent.editModule.cellEditModule) &&
  ...
  public updateTaskScheduleModes(data: IGanttData): void {
    ...
    if (data.hasChildRecords && ganttProp.isAutoSchedule) {
      ...
      this.parent.setRecordValue(
        'width',
        this.parent.dataOperation.calculateWidth(data, true),
        ganttProp,
        true
      );
      this.parent.setRecordValue(
        'left',
        this.parent.dataOperation.calculateLeft(ganttProp, true),
        ganttProp,
        true
      );
      this.parent.setRecordValue(
      ...
      this.parent.setRecordValue(
        'autoDuration',
        this.parent.dataOperation.calculateAutoDuration(data),
        ganttProp,
        true
      );
      this.parent.dataOperation.updateAutoWidthLeft(data);
    } else {
      const startDate: Date = this.parent.dateValidationModule.checkStartDate(
        currentValue,
        data.ganttProperties
      );
      this.parent.setRecordValue(
  ...
  public updateParentProgress(cloneParent: IParent): void {
    ...
    if (childRecords) {
      for (let i: number = 0; i < childCount; i++) {
        if (
          (!childRecords[i as number].ganttProperties.isMilestone ||
            childRecords[i as number].hasChildRecords) &&
          isScheduledTask(childRecords[i as number].ganttProperties)
        ) {
          progressValues = this.parent.dataOperation.getParentProgress(
      ...
      parentProgress =
        taskCount > 0 ? Math.round(totalProgress / totalDuration) : 0;
   ...
   * @private
   */
  public reUpdatePreviousRecords(
    ...
    for (let i: number = 0; i < keys.length; i++) {
      ...
      const rowIndex: number =
        this.parent.currentViewData.indexOf(originalData);
      if (isRefreshChart) {
        this.parent.chartRowsModule.refreshRow(rowIndex);
      }
      if (isRefreshGrid) {
        const dataId: number | string =
          this.parent.viewType === 'ProjectView'
        ...
        const row: Row<Column> = this.parent.treeGrid.grid.getRowObjectFromUID(
          this.parent.treeGrid.grid
            .getDataRows()
            [rowIndex as number].getAttribute('data-uid')
        );
  ...
  public initiateSaveAction(args: ITaskbarEditedEventArgs): void {
    ...
    if (
      ...
      this.parent.undoRedoModule &&
      !this.parent.undoRedoModule['isUndoRedoPerformed'] &&
      this.parent.undoRedoModule['getUndoCollection'].length > 0
    ) {
      this.parent.toolbarModule.enableItems(
        [this.parent.controlId + '_undo'],
        true
    ...
    if (
      args.action === 'DrawConnectorLine' ||
      args.action === 'DeleteConnectorLine'
    ) {
    ...
    const ganttObj: Gantt = this.parent;
    const currentBaselineStart: Date = {
      ...eventArgs.data.ganttProperties.baselineStartDate,
    };
    const currentBaselineEnd: Date = {
      ...eventArgs.data.ganttProperties.baselineEndDate,
    };
    const currentProgress: number = eventArgs.data.ganttProperties.progress;
    /* eslint-disable-next-line */
    const unModifiedData: any = JSON.parse(
      JSON.stringify(eventArgs.data.ganttProperties)
    );
    this.parent.trigger(
      'actionBegin',
      eventArgs,
      (eventArg: IActionBeginEventArgs) => {
        if (
          currentBaselineStart !==
            eventArg.data['ganttProperties'].baselineStartDate ||
          currentBaselineEnd !==
            eventArg.data['ganttProperties'].baselineEndDate
        ) {
          ganttObj.setRecordValue(
            'baselineLeft',
            ganttObj.dataOperation.calculateBaselineLeft(
        ...
        if (
          this.parent.autoCalculateDateScheduling &&
          unModifiedData !== eventArg.data['ganttProperties'] &&
          !isNullOrUndefined(eventArg.data['parentItem'])
        ) {
          this.updateParentItemOnEditing();
          this.parent.dataOperation.updateParentItems(
            eventArg.data as IGanttData,
            true
          );
        }
        ...
        if (startDate && endDate && !segments) {
          ...
          if (duration !== 0) {
            ...
            this.parent.dateValidationModule.calculateDuration(
              eventArg.data as IGanttData
            );
          }
        }
        ...
        if (
          !isNullOrUndefined(this.parent.taskFields.progress) &&
          currentProgress !== eventArg.data['ganttProperties'].progress
        ) {
          const width: number = eventArg.data['ganttProperties'].isAutoSchedule
            ? eventArg.data['ganttProperties'].width
            : eventArg.data['ganttProperties'].autoWidth;
          this.parent.setRecordValue(
            'progressWidth',
            this.parent.dataOperation.getProgressWidth(
              width,
              eventArg.data['ganttProperties'].progress
            ),
            eventArg.data['ganttProperties'],
            true
          );
        }
        this.parent.chartRowsModule.updateSegment(
          eventArg.data['ganttProperties'].segments,
          eventArg.data['ganttProperties'].taskId
        );
        if (
          !isNullOrUndefined(this.parent.loadingIndicator) &&
        ...
        if (eventArg.cancel) {
          ...
          eventArg.modifiedTaskData = getTaskData(
            eventArg.modifiedRecords,
            null,
            null,
            this.parent
          );
          if (isRemoteData(this.parent.dataSource)) {
            ...
            if (
              !(
                dataAdaptor instanceof WebApiAdaptor &&
                dataAdaptor instanceof ODataAdaptor
              ) ||
              data.dataSource.batchUrl
            ) {
              crud = data.saveChanges(
                updatedData,
                this.parent.taskFields.id,
                null,
                query
              ...
              crud = data.update(
                this.parent.taskFields.id,
                updatedData[changedRecords as string],
                null,
                query
              ) as Promise<Object>;
            }
            crud
              .then((e: ReturnType) => this.dmSuccess(e, args))
          ...
          if (this.parent.undoRedoModule) {
            this.parent.previousFlatData = extend(
              [],
              this.parent.flatData,
              [],
              true
  ...
  private dmSuccess(e: any, args: ITaskbarEditedEventArgs): void {
    ...
    for (let i: number = 0; i < eLength; i++) {
      ...
      for (
        let j: number = 0, _a: string[] = Object.keys(rec);
        j < _aLength;
        j++
      ) {
        const key: string = _a[parseInt(j.toString(), 10)];
        this.parent.editedRecords[parseInt(i.toString(), 10)][`${key}`] =
          rec[`${key}`];
        this.parent.editedRecords[parseInt(i.toString(), 10)].taskData[
          `${key}`
        ] = rec[`${key}`];
      }
      this.updateEditedRecordFields(
        rec,
        this.parent.editedRecords[parseInt(i.toString(), 10)]
      );
    }
    this.saveSuccess(args);
  }
  ...
  private updateEditedRecordFields(
    ...
    if (fields.duration !== null) {
      editedRecord.ganttProperties['duration'] = parseFloat(
        rec[fields.duration]
      );
    }
    ...
    if (fields.baselineEndDate !== null) {
      editedRecord.ganttProperties['baselineEndDate'] =
        rec[fields.baselineEndDate];
    }
    if (fields.baselineStartDate !== null) {
      editedRecord.ganttProperties['baselineStartDate'] =
        rec[fields.baselineStartDate];
    }
  ...
  private updateSharedTask(data: IGanttData): void {
    ...
    for (let i: number = 0; i < ids.length; i++) {
      const editRecord: IGanttData =
        this.parent.flatData[
          this.parent.ids.indexOf(ids[i as number].toString())
        ];
  ...
  private saveSuccess(args: ITaskbarEditedEventArgs): void {
    ...
    if (this.parent.timelineSettings.updateTimescaleView) {
      ...
      this.parent.timelineModule.updateTimeLineOnEditing(
        [tempArray],
        args.action
      );
    }
    if (this.parent.viewType === 'ResourceView') {
      if (
        ...
        if (this.parent.editModule.dialogModule.isResourceUpdate) {
          ...
          this.updateResoures(
            this.parent.editModule.dialogModule.previousResource,
            args.data.ganttProperties.resourceInfo,
            args.data
          );
          this.updateSharedTask(args.data);
      ...
      for (let k: number = 0; k < this.updateParentRecords.length; k++) {
        this.parent.dataOperation.updateParentItems(
          this.updateParentRecords[k as number]
        );
      }
    ...
    if (!this.isTreeGridRefresh) {
      if (
        ...
        if (
          !this.parent.treeGrid.grid.element
            .querySelector('form')
      ...
      if (!this.parent.allowTaskbarOverlap && this.parent.showOverAllocation) {
        this.parent.contentHeight =
          this.parent['element'].getElementsByClassName(
            'e-content'
          )[0].children[0]['offsetHeight'];
        this.parent.ganttChartModule.chartBodyContent.style.height =
          this.parent.contentHeight + 'px';
        this.parent.ganttChartModule.renderRangeContainer(
      ...
      if (
        (this.parent.isConnectorLineUpdate ||
          (this.parent.undoRedoModule &&
            this.parent.undoRedoModule['currentAction'] &&
            this.parent.undoRedoModule['currentAction']['connectedRecords'])) &&
        !isNullOrUndefined(this.parent.connectorLineEditModule)
      ) {
        this.parent.updatedConnectorLineCollection = [];
        this.parent.connectorLineIds = [];
        this.parent.connectorLineEditModule.refreshEditedRecordConnectorLine(
    ...
    if (this.parent.enableCriticalPath && this.parent.criticalPathModule) {
      ...
      criticalModule.criticalConnectorLine(
        criticalModule.criticalPathCollection,
    ...
    if (
      !this.parent.editSettings.allowTaskbarEditing ||
      (this.parent.editSettings.allowTaskbarEditing &&
      ...
      eventArgs.data = args.data;
      eventArgs.modifiedRecords = this.parent.editedRecords;
      eventArgs.modifiedTaskData = getTaskData(
        this.parent.editedRecords,
        null,
        null,
        this.parent
      ...
      if (
        ...
        this.parent.rowDragAndDropModule &&
  ...
  private updateResoures(
    ...
    for (let index: number = 0; index < currentLength; index++) {
      ...
      let resourceID: string | number = parseInt(
        currentResource[index as number][this.parent.resourceFields.id],
        10
      ).toString();
      if (resourceID === 'NaN') {
        resourceID =
      ...
      for (let i: number = 0; i < prevResource.length; i++) {
        let prevResourceID: string | number = parseInt(
          prevResource[i as number][this.parent.resourceFields.id],
          10
        ).toString();
      ...
      if (recordIndex.length === 0) {
        const parentRecord: IGanttData =
          flatRecords[this.parent.getTaskIds().indexOf('R' + resourceID)];
        if (parentRecord) {
    ...
    for (let index: number = 0; index < prevLength; index++) {
      ...
      const resourceID: string =
        prevResource[index as number][this.parent.resourceFields.id];
      const record: IGanttData =
        flatRecords[this.parent.getTaskIds().indexOf('R' + resourceID)];
      if (!isNullOrUndefined(record)) {
        for (let j: number = 0; j < record.childRecords.length; j++) {
          if (
            record.childRecords[j as number].ganttProperties.taskId === taskID
          ) {
    ...
    if (currentLength > 0) {
      const parentTask: IGanttData = this.parent.getParentTask(
        updateRecord.parentItem
      );
      if (parentTask) {
        if (
          parentTask.ganttProperties.taskName ===
    ...
    if (
      this.parent.undoRedoModule &&
      this.parent.undoRedoModule['isUndoRedoPerformed'] &&
      this.parent.flatData[this.parent.flatData.length - 1].ganttProperties
        .taskName === this.parent.localeObj.getConstant('unassignedTask')
    ) {
      this.parent.flatData.splice(this.parent.flatData.length - 1, 1);
      this.parent.currentViewData.splice(
        this.parent.currentViewData.length - 1,
        1
      );
  ...
  public checkWithUnassignedTask(updateRecord: IGanttData): void {
    ...
    for (let i: number = 0; i < this.parent.flatData.length; i++) {
      if (
        this.parent.flatData[i as number].ganttProperties.taskName ===
        this.parent.localeObj.getConstant('unassignedTask')
      ) {
    ...
    if (!isNullOrUndefined(unassignedTasks)) {
      ...
      unassignTaskObj[this.parent.taskFields.name] =
        this.parent.localeObj.getConstant('unassignedTask');
      const beforeEditStatus: boolean = this.parent.isOnEdit;
      this.parent.isOnEdit = false;
      const cAddedRecord: IGanttData = this.parent.dataOperation.createRecord(
        unassignTaskObj,
        0
      );
      ...
      const parentRecord: IGanttData =
        this.parent.flatData[this.parent.flatData.length - 1];
      this.addNewRecord(updateRecord, parentRecord);
      /* eslint-disable-next-line */
      const source: any = this.parent.dataSource;
      source.push(updateRecord.taskData);
    }
    const updatedData: IGanttData = this.parent.currentViewData.filter(
      (data: IGanttData) => {
        return (
          data.ganttProperties.taskId === updateRecord.ganttProperties.taskId &&
  ...
  private addRecordAsBottom(cAddedRecord: IGanttData): void {
    ...
    this.parent.ids.splice(
      recordIndex1 + 1,
      0,
      cAddedRecord.ganttProperties.rowUniqueID.toString()
    );
    const taskId: string =
      cAddedRecord.level === 0
        ? 'R' + cAddedRecord.ganttProperties.taskId
        : 'T' + cAddedRecord.ganttProperties.taskId;
  ...
  private addNewRecord(
    ...
    this.parent.setRecordValue(
      'uniqueID',
      getUid(this.parent.element.id + '_data_'),
      cAddedRecord
    );
    this.parent.setRecordValue(
      'uniqueID',
      cAddedRecord.uniqueID,
      cAddedRecord.ganttProperties,
      true
    );
    const uniqueId: string = cAddedRecord.uniqueID.replace(
      this.parent.element.id + '_data_',
      ''
    );
    ...
    if (this.parent.taskFields.parentID) {
      this.parent.setRecordValue(
        'parentId',
        parentRecord.ganttProperties.taskId,
        cAddedRecord.ganttProperties,
        true
      );
    }
    this.parent.setRecordValue(
      'parentItem',
      this.parent.dataOperation.getCloneParent(parentRecord),
      cAddedRecord
    );
    const parentUniqId: string = cAddedRecord.parentItem
      ? cAddedRecord.parentItem.uniqueID
      : null;
    ...
    cAddedRecord.ganttProperties.sharedTaskUniqueIds =
  ...
  private removeChildRecord(record: IGanttData): void {
    ...
    if (
      ...
      this.parent.dataSource.dataSource.json.length > 0
    ) {
    ...
    const flatParentData: IGanttData = this.parent.getParentTask(
      deletedRow.parentItem
    );
    if (deletedRow) {
      if (deletedRow.parentItem) {
        const deleteChildRecords: IGanttData[] = flatParentData
          ? flatParentData.childRecords
          : [];
      ...
      if (deletedRow.ganttProperties.sharedTaskUniqueIds.length) {
        const uniqueIDIndex: number =
          deletedRow.ganttProperties.sharedTaskUniqueIds.indexOf(
            deletedRow.ganttProperties.rowUniqueID
          );
      ...
      if (gObj.taskFields.parentID) {
        ...
        for (let i: number = 0; i < ganttData.length; i++) {
          if (
            ganttData[i as number].ganttProperties.rowUniqueID ===
            deletedRow.ganttProperties.rowUniqueID
          ) {
            idx = i;
          }
      ...
      if (!gObj.taskFields.parentID) {
        const deletedRecordCount: number = this.parent.editModule.getChildCount(
          deletedRow,
          0
        );
        ...
        this.parent
          .getTaskIds()
          .splice(flatRecordIndex, deletedRecordCount + 1);
      }
      if (
        deletedRow.parentItem &&
        flatParentData &&
        flatParentData.childRecords &&
        !flatParentData.childRecords.length
      ) {
  ...
  private addRecordAsChild(
    droppedRecord: IGanttData,
    draggedRecord: IGanttData
  ): void {
    const gObj: Gantt = this.parent;
    const recordIndex1: number = this.parent.flatData.indexOf(droppedRecord);
    const childRecords: number = this.parent.editModule.getChildCount(
      droppedRecord,
      0
    );
    let childRecordsLength: number;
    let spliceIndex: number;
    const parentTask: IGanttData = this.parent.getTaskByUniqueID(
      draggedRecord.parentItem.uniqueID
    );
    if (
      this.parent.undoRedoModule &&
      this.parent.undoRedoModule['isUndoRedoPerformed'] &&
      this.dialogModule['indexes'] &&
      this.dialogModule['indexes'].deletedIndexes &&
      this.dialogModule['indexes'].deletedIndexes.length > 0
    ) {
      if (
        parentTask.ganttProperties.taskName ===
        this.parent.localeObj.getConstant('unassignedTask')
        ...
        for (
          let j: number = 0;
          j < this.dialogModule['indexes'].deletedIndexes.length;
          j++
        ) {
          if (
            this.dialogModule['indexes'].deletedIndexes[j as number].data
              .parentUniqueID === draggedRecord.parentUniqueID &&
            draggedRecord.ganttProperties.taskId ===
              this.dialogModule['indexes'].deletedIndexes[j as number].data
                .ganttProperties.taskId
          ) {
            const toIndex: number =
              this.dialogModule['indexes'].deletedIndexes[j as number].index;
            if (
              this.dialogModule['indexes'].deletedIndexes[j as number]
                .position === 'above'
            ) {
            ...
            for (
              let i: number = 0;
              i < droppedRecord.childRecords.length;
              i++
            ) {
              if (
                'T' +
                  droppedRecord.childRecords[i as number].ganttProperties
                    .taskId ===
                this.dialogModule['indexes'].deletedIndexes[j as number].id
              ) {
                if (
                  this.dialogModule['indexes'].deletedIndexes[j as number]
                    .position === 'above'
            ...
            const draggedParent: IGanttData = this.parent.getTaskByUniqueID(
              draggedRecord.parentItem.uniqueID
            );
            childRecordsLength =
              draggedParent.index + draggedParent.childRecords.length + 1;
            break;
          }
        }
      }
      ...
      parentTask.ganttProperties.taskName ===
        this.parent.localeObj.getConstant('unassignedTask') &&
      this.parent.undoRedoModule['currentAction']['deletedRecordsDetails']
    ) {
      for (
        let i: number = 0;
        i <
        ...
        if (
          this.parent.undoRedoModule['currentAction']['deletedRecordsDetails'][
            i as number
          ].data.ganttProperties.taskId === draggedRecord.ganttProperties.taskId
        ) {
          if (parentTask.index) {
            childRecordsLength =
              this.parent.undoRedoModule['currentAction'][
                'deletedRecordsDetails'
              ][i as number].data.index;
            spliceIndex = childRecordsLength - parentTask.index - 1;
          } else {
            childRecordsLength =
              isNullOrUndefined(childRecords) || childRecords === 0
                ? recordIndex1 + 1
      ...
      !isNullOrUndefined(this.addRowIndex) &&
      this.addRowPosition &&
      droppedRecord.childRecords &&
      this.addRowPosition !== 'Child'
    ) {
      const dropChildRecord: IGanttData =
        droppedRecord.childRecords[this.addRowIndex];
      const position: RowPosition =
        this.addRowPosition === 'Above' || this.addRowPosition === 'Below'
          ? this.addRowPosition
          : 'Child';
      childRecordsLength = dropChildRecord
        ? this.addRowIndex + recordIndex1 + 1
        : childRecords + recordIndex1 + 1;
      childRecordsLength =
        position === 'Above' ? childRecordsLength : childRecordsLength + 1;
    } else {
      childRecordsLength =
        isNullOrUndefined(childRecords) || childRecords === 0
          ? recordIndex1 + 1
    ...
    this.parent.ids.splice(
      childRecordsLength,
      0,
      draggedRecord.ganttProperties.rowUniqueID.toString()
    );
    this.updateTreeGridUniqueID(draggedRecord, 'add');
    const recordId: string =
      draggedRecord.level === 0
        ? 'R' + draggedRecord.ganttProperties.taskId
        : 'T' + draggedRecord.ganttProperties.taskId;
    ...
    if (spliceIndex >= 0) {
      ...
      droppedRecord.childRecords.splice(
        droppedRecord.childRecords.length,
        0,
        draggedRecord
      );
    }
    if (
      !isNullOrUndefined(draggedRecord) &&
      !this.parent.taskFields.parentID &&
      !isNullOrUndefined(droppedRecord.taskData[this.parent.taskFields.child])
    ) {
      droppedRecord.taskData[this.parent.taskFields.child].splice(
        droppedRecord.childRecords.length,
        0,
        draggedRecord.taskData
      );
    }
  ...
  private resetEditProperties(args?: object): void {
    ...
    if (
      !isNullOrUndefined(this.dialogModule) &&
      ...
      if (
        ...
        !this.dialogModule.dialogObj.isDestroyed
      ) {
  ...
  private confirmDeleteOkButton(): void {
    ...
    this.confirmDialog.hide();
    const focussedElement: HTMLElement =
      this.parent.element.querySelector('.e-treegrid');
  ...
  public startDeleteAction(): void {
    if (this.parent.editSettings.allowDeleting && !this.parent.readOnly) {
      if (this.parent.editSettings.showDeleteConfirmDialog) {
        ...
        this.deleteSelectedItems();
        const focusingElement: HTMLElement =
          this.parent.element.querySelector('.e-treegrid');
  ...
  private deleteResourceRecords(selectedRecords: IGanttData[]): void {
    ...
    for (let i: number = 0; i < selectedRecords.length; i++) {
      if (selectedRecords[i as number].parentItem) {
        if (
          selectedRecords[i as number].ganttProperties.sharedTaskUniqueIds
            .length === 1
        ) {
          ...
          for (let j: number = 0; j < ids.length; j++) {
            if (this.parent.ids.indexOf(ids[j as number].toString()) !== -1) {
              deleteRecords.push(
                this.parent.flatData[
                  this.parent.ids.indexOf(ids[j as number].toString())
                ]
              );
            } else if (
              this.parent.undoRedoModule &&
              this.parent.undoRedoModule['isUndoRedoPerformed'] &&
              this.parent.undoRedoModule['currentAction'] &&
              this.parent.undoRedoModule['currentAction']['action'] === 'Delete'
            ) {
              deleteRecords.push(
                this.parent.flatData[
                  this.parent.taskIds.indexOf(
                    'T' + selectedRecords[i as number].ganttProperties.taskId
                  )
                ]
              );
            }
          ...
          if (
            this.parent.ids.indexOf(data.ganttProperties.rowUniqueID) !== -1 &&
            deleteRecords.indexOf(
              this.parent.flatData[
                this.parent.ids.indexOf(data.ganttProperties.rowUniqueID)
              ]
            ) === -1
          ) {
            deleteRecords.push(
        ...
        const resourceParent: IGanttData = this.parent.flatData.filter(
          (data: IGanttData) => {
            return (
              data.ganttProperties.taskId ===
                selectedRecords[i as number].ganttProperties.taskId &&
  ...
  public deleteRecord(
    ...
    switch (variableType) {
      ...
      case 'string': {
        ...
        if (this.parent.viewType === 'ResourceView') {
          if (
            !isNullOrUndefined(taskId) &&
            this.parent.getTaskIds().indexOf('T' + taskId) !== -1
          ) {
            this.targetedRecords.push(
              this.parent.flatData[
                this.parent.getTaskIds().indexOf('T' + taskId)
              ]
            );
          }
        } else {
          if (
            !isNullOrUndefined(taskId) &&
            this.parent.ids.indexOf(taskId) !== -1
          ) {
        ...
        for (let i: number = 0; i < taskDetailArray.length; i++) {
          ...
          if (this.parent.viewType === 'ResourceView') {
            if (
              ...
              this.parent.getTaskIds().indexOf('T' + id) !== -1
            ) {
              this.targetedRecords.push(
                this.parent.flatData[this.parent.getTaskIds().indexOf('T' + id)]
              );
            }
          } else if (
            !isNullOrUndefined(id) &&
            this.parent.ids.indexOf(id) !== -1
          ) {
  ...
  private deleteRow(tasks: IGanttData[]): void {
    ...
    if (rowItems.length) {
      ...
      if (
        this.parent.undoRedoModule &&
        !this.parent.undoRedoModule['isUndoRedoPerformed'] &&
        this.parent['isUndoRedoItemPresent']('Delete')
      ) {
        if (
          this.parent.undoRedoModule['redoEnabled'] &&
          !this.parent.undoRedoModule['isUndoRedoPerformed']
        ) {
        ...
        this.parent.undoRedoModule['findPosition'](
          extend([], [], rowItems, true) as IGanttData[],
          records,
          'deletedRecordsDetails'
        );
        (this.parent.undoRedoModule['getUndoCollection'][
      ...
      for (let i: number = 0; i < rowItems.length; i++) {
        ...
        if (
          ...
          this.parent.undoRedoModule &&
          this.parent.undoRedoModule['isUndoRedoPerformed'] &&
          rowItems[i as number].ganttProperties.resourceInfo &&
          rowItems[i as number].ganttProperties.resourceInfo.length > 1
        ) {
          deleteItems = [];
          if (!rowItems[i as number].hasChildRecords) {
            const id: string =
              'T' + rowItems[i as number].ganttProperties.taskId;
            this.parent.taskIds.reduce(
              function (e: string, j: number): void {
                if (e === id) {
        ...
        for (let j: number = 0; j < deleteItems.length; j++) {
          const parentTask: IGanttData = this.parent.getParentTask(
            deleteItems[j as number].parentItem
          );
          if (deleteItems[j as number].parentItem) {
            ...
            if (!isNullOrUndefined(parentTask)) {
              ...
              const filteredRecord: IGanttData[] =
                childRecord.length === 1
                  ? childRecord
                  : childRecord.filter((data: IGanttData): boolean => {
          ...
          const predecessor: IPredecessor[] =
            deleteItems[j as number].ganttProperties.predecessor;
  ...
  public removePredecessorOnDelete(record: IGanttData): void {
    ...
    for (let i: number = 0; i < predecessors.length; i++) {
      ...
      if (predecessor.from.toString() === recordId.toString()) {
        const toRecord: IGanttData =
          this.parent.connectorLineModule.getRecordByID(
            predecessor.to.toString()
          );
        if (!isNullOrUndefined(toRecord)) {
          const toRecordPredcessor: IPredecessor[] = extend(
            [],
            [],
            toRecord.ganttProperties.predecessor,
            true
          ) as IPredecessor[];
          let index: number;
          for (let t: number = 0; t < toRecordPredcessor.length; t++) {
            ...
            if (
              toRecordPredcessor[t as number].to.toString() ===
                toId.toString() &&
              toRecordPredcessor[t as number].from.toString() ===
                recordId.toString()
            ) {
              index = t;
              break;
        ...
        if (!isNullOrUndefined(fromRecord)) {
          const fromRecordPredcessor: IPredecessor[] = extend(
            [],
            [],
            fromRecord.ganttProperties.predecessor,
            true
          ) as IPredecessor[];
          let index: number;
          for (let t: number = 0; t < fromRecordPredcessor.length; t++) {
            ...
            if (
              fromRecordPredcessor[t as number].from.toString() ===
                fromId.toString() &&
              fromRecordPredcessor[t as number].to.toString() ===
                recordId.toString()
            ) {
              index = t;
              break;
          ...
          if (
            record.uniqueID === fromRecord.parentUniqueID ||
            record.parentUniqueID === fromRecord.uniqueID ||
            this.parent.isOnDelete
          ) {
  ...
  private updatePredecessorValues(
    ...
    const predecessorString: string =
      this.parent.predecessorModule.getPredecessorStringValue(record);
    this.parent.setRecordValue(
      'predecessorsName',
      predecessorString,
      record.ganttProperties,
  ...
  public updateTaskId(
    ...
    if (!this.parent.readOnly) {
      const cId: string =
        typeof currentId === 'number' ? currentId.toString() : currentId;
      ...
      if (!isNullOrUndefined(cId) && !isNullOrUndefined(nId)) {
        ...
        if (
          this.parent.taskFields.dependency &&
          !isNullOrUndefined(thisRecord.ganttProperties.predecessor)
        ) {
          const predecessors: IPredecessor[] =
            thisRecord.ganttProperties.predecessor;
          let currentGanttRecord: IGanttData;
          for (let i: number = 0; i < predecessors.length; i++) {
            const predecessor: IPredecessor = predecessors[i as number];
            if (predecessor.to === cId) {
              currentGanttRecord =
                this.parent.flatData[ids.indexOf(predecessor.from)];
            } else if (predecessor.from === cId) {
              currentGanttRecord =
                this.parent.flatData[ids.indexOf(predecessor.to)];
            }
  ...
  private updatePredecessorOnUpdateId(
    currentGanttRecord: IGanttData,
    cId: string,
    nId: string
  ): void {
    if (this.parent.flatData.indexOf(currentGanttRecord) > -1) {
      const pred: IPredecessor[] =
        currentGanttRecord.ganttProperties.predecessor;
  ...
  private deleteChildRecords(record: IGanttData): void {
    ...
    for (let c: number = 0; c < childRecords.length; c++) {
      ...
      const predecessor: IPredecessor[] =
        childRecord.ganttProperties.predecessor;
      let canDeletePredecessor: boolean = true;
      const parentTask: IGanttData = this.parent.getParentTask(
        childRecord.parentItem
      );
      if (
        this.parent.viewType === 'ResourceView' &&
        parentTask &&
    ...
    for (let i: number = 0; i < dataCollection.length; i++) {
      ...
      if (
        record.indexOf(
          getValue(
            this.parent.taskFields.id,
            dataCollection[i as number]
          ).toString()
        ) !== -1
      ) {
        if (dataCollection[i as number][this.parent.taskFields.child]) {
          const childRecords: ITaskData[] =
            dataCollection[i as number][this.parent.taskFields.child];
        ...
        if (eventArg.cancel) {
          ...
          if (isRemoteData(this.parent.dataSource)) {
            ...
            if (this.parent.timezone) {
              (eventArg.modifiedRecords as IGanttData[]).forEach(
                (modifiedRecord: IGanttData) => {
                  updateDates(modifiedRecord, this.parent);
                }
              );
            }
            const updatedData: object = {
              deletedRecords: getTaskData(
                eventArg.data as IGanttData[],
                null,
                null,
                this.parent
            ...
            if (
              !(
                adaptor instanceof WebApiAdaptor &&
                adaptor instanceof ODataAdaptor
              ) ||
              data.dataSource.batchUrl
            ) {
              ...
              for (
                let i: number = 0;
                i < updatedData[deletedRecords as string].length;
                i++
              ) {
                deleteCrud = data.remove(
                  this.parent.taskFields.id,
                  updatedData[deletedRecords as string][i as number],
                  null,
                  query
                ) as Promise<Object>;
              }
              deleteCrud
                .then(() => {
                  ...
                  for (
                    let i: number =
                      updatedData[changedRecords as string].length - 1;
                    i >= 0;
                    i--
                  ) {
                    if (
                      updatedData['deletedRecords' as string].some(
                        (record: string) =>
                          record[this.parent.taskFields.id] ===
                          updatedData[changedRecords as string][i as number]
                            .taskId
                      )
                    ) {
                      updatedData[changedRecords as string].splice(
                        i as number,
                        1
                      );
                    }
                  }
                  const updateCrud: Promise<Object> = data.update(
                    this.parent.taskFields.id,
                    updatedData[changedRecords as string],
                    null,
                    query
  ...
  private deleteSuccess(args: ITaskDeletedEventArgs): void {
    ...
    for (let i: number = 0; i < args.deletedRecordCollection.length; i++) {
      if (this.parent.viewType === 'ProjectView') {
        const record: IGanttData = this.parent.getRecordByID(
          args.deletedRecordCollection[i as number].ganttProperties.taskId
        );
        if (!isNullOrUndefined(record)) {
        ...
        if (
          ...
          deletedRecords.push(
            this.parent.getTaskByUniqueID(
              args.deletedRecordCollection[i as number].uniqueID
            )
          );
        } else {
          deletedRecords.push(
            this.parent.flatData[this.parent.taskIds.indexOf(id.toString())]
          );
        }
      }
    }
    ...
    if (deletedRecords.length > 0) {
      const recordIndex: number = this.parent.flatData.indexOf(
        deletedRecords[deletedRecords.length - 1]
      );
      this.parent.staticSelectedRowIndex = this.parent.selectedRowIndex =
        recordIndex;
      const record: IGanttData =
        this.parent.flatData[this.parent.selectedRowIndex + 1];
      if (!isNullOrUndefined(record)) {
        ...
        if (prevRecordIndex >= 0) {
          this.parent.currentSelection =
            this.parent.flatData[prevRecordIndex as number];
        }
      }
    }
    ...
    for (let i: number = 0; i < deletedRecords.length; i++) {
      ...
      if (this.parent.viewType === 'ResourceView') {
        if (deleteRecord.hasChildRecords) {
          currentIndex = this.parent.taskIds.indexOf(
            'R' + deleteRecord.ganttProperties.taskId.toString()
          );
          flatIndex = this.parent.taskIds.indexOf(
            'R' + deleteRecord.ganttProperties.taskId.toString()
          );
        } else {
          currentIndex = this.parent.taskIds.indexOf(
            'T' + deleteRecord.ganttProperties.taskId.toString()
          );
          flatIndex = this.parent.taskIds.indexOf(
            'T' + deleteRecord.ganttProperties.taskId.toString()
          );
      ...
      const treeGridParentIndex: number =
        this.parent.treeGrid.parentData.indexOf(deleteRecord);
      const tempData: ITaskData[] = getValue(
        'dataOperation.dataArray',
        this.parent
      );
      ...
      if (!isNullOrUndefined(deleteRecord)) {
        ...
        if (deleteRecord.parentItem) {
          const parentItem: IGanttData = this.parent.getParentTask(
            deleteRecord.parentItem
          );
          if (parentItem) {
    ...
    if (
      ...
      (!this.parent.undoRedoModule ||
        (this.parent.undoRedoModule &&
          (!this.parent.undoRedoModule['isUndoRedoPerformed'] ||
            (this.parent.undoRedoModule['isUndoRedoPerformed'] &&
              this.parent.undoRedoModule['currentAction']['action'] ===
                'Delete'))))
    ) {
      const updateUnAssignedResources: IGanttData[] = eventArgs.data.filter(
        (data: IGanttData) => {
          return !data.hasChildRecords;
        }
      );
      for (let i: number = 0; i < updateUnAssignedResources.length; i++) {
        let unassignedTask: IGanttData = this.parent.flatData.filter(
          (data: IGanttData) => {
            return (
        ...
        if (unassignedTask) {
          isDuplicate = unassignedTask.childRecords.filter(
            (data: IGanttData) => {
              return (
                data.ganttProperties.taskId ===
                updateUnAssignedResources[i as number].ganttProperties.taskId
              );
            }
          );
        }
        const parentTask: IGanttData = this.parent.getParentTask(
          updateUnAssignedResources[i as number].parentItem
        );
        if (
          parentTask &&
          parentTask.ganttProperties.taskName !==
        ...
        const parentItem: IGanttData[] = this.parent.currentViewData.filter(
          (data: IGanttData) => {
            if (
              data.ganttProperties.taskId ===
                updateUnAssignedResources[i as number].ganttProperties.taskId &&
  ...
  public getNewTaskId(): number | string {
    const ids: string[] =
      this.parent.viewType === 'ResourceView'
        ? this.parent.getTaskIds()
        : this.parent.ids;
    const maxId: number = ids.length;
    let newTaskId: number | string = maxId + 1;
    if (this.parent.viewType === 'ResourceView') {
      if (
        ids.indexOf('T' + newTaskId) !== -1 ||
        ids.indexOf('R' + newTaskId) !== -1
      ) {
  ...
  private prepareNewlyAddedData(obj: Object, rowPosition: RowPosition): void {
    ...
    if (
      ...
      taskModel.duration &&
      isNullOrUndefined(obj[taskModel.duration])
    ) {
    ...
    if (taskModel.progress) {
      obj[taskModel.progress] = obj[taskModel.progress]
    ...
    if (
      !this.parent.allowUnscheduledTasks &&
      !obj[taskModel.endDate] &&
      taskModel.endDate
    ) {
      if (!obj[taskModel.duration]) {
        const startDate: Date = this.parent.dataOperation.getDateFromFormat(
          this.parent.projectStartDate
        );
        if (!isNullOrUndefined(startDate)) {
          startDate.setDate(startDate.getDate() + 4);
          obj[taskModel.endDate] = this.parent.getFormatedDate(
            startDate,
            this.parent.getDateFormat()
          );
        }
      }
    }
  ...
  private updateNewlyAddedDataBeforeAjax(
    ...
    cAddedRecord.index =
      parseInt(cAddedRecord.ganttProperties.rowUniqueID.toString(), 10) - 1;
    if (!isNullOrUndefined(parentItem)) {
      this.parent.setRecordValue(
        'parentItem',
        this.parent.dataOperation.getCloneParent(parentItem),
        cAddedRecord
      );
      const pIndex: number = cAddedRecord.parentItem
        ? cAddedRecord.parentItem.index
        : null;
      ...
      this.parent.setRecordValue('parentUniqueID', parentUniqId, cAddedRecord);
      if (
        !isNullOrUndefined(this.parent.taskFields.id) &&
        !isNullOrUndefined(this.parent.taskFields.parentID) &&
        cAddedRecord.parentItem
      ) {
        if (this.parent.viewType === 'ProjectView') {
          this.parent.setRecordValue(
            this.parent.taskFields.parentID,
            cAddedRecord.parentItem.taskId,
            cAddedRecord.taskData,
            true
          );
        }
        this.parent.setRecordValue(
    ...
    if (
      ...
      if (obj['hasChildRecords']) {
        ...
        for (let j: number = 0; j < currentAction.length; j++) {
          if (
            obj['ganttProperties'].taskId ===
            currentAction[j as number]['data'].ganttProperties.taskId
          ) {
            ...
            this.backUpAndPushNewlyAddedRecord(
              cAddedRecord,
              rowPosition,
              parentItem
            );
            for (let i: number = 0; i < totalRecords.length; i++) {
              if (this.parent.viewType === 'ProjectView') {
                if (
                  totalRecords[i as number].parentItem.taskId ===
                  cAddedRecord.ganttProperties.taskId
                ) {
                  totalRecords[i as number].parentItem.index =
                    cAddedRecord.index;
                  totalRecords[i as number].parentItem.taskId =
                    cAddedRecord.ganttProperties.taskId;
                  totalRecords[i as number].parentItem.uniqueID =
                    cAddedRecord.ganttProperties.uniqueID;
                  totalRecords[i as number].parentUniqueID =
                    cAddedRecord.ganttProperties.uniqueID;
                }
              } else {
                totalRecords[i as number].parentItem.index = cAddedRecord.index;
                totalRecords[i as number].parentItem.taskId =
                  cAddedRecord.ganttProperties.rowUniqueID;
                totalRecords[i as number].parentItem.uniqueID =
                  cAddedRecord.ganttProperties.uniqueID;
                totalRecords[i as number].parentUniqueID =
                  cAddedRecord.ganttProperties.uniqueID;
        ...
        for (let k: number = 0; k < totalRecords.length; k++) {
          ...
          const newRecord: IGanttData = this.parent.dataOperation.createRecord(
            totalRecords[k as number],
            totalRecords[k as number].level
          );
          if (
            newRecord.childRecords.length === 0 &&
            newRecord.taskData[this.parent.taskFields.child]
          ) {
            newRecord.taskData[this.parent.taskFields.child] = [];
          }
          parentItem = this.parent.getRecordByID(
            totalRecords[k as number].parentItem.taskId
          );
          if (!isNullOrUndefined(parentItem)) {
            this.parent.setRecordValue(
              'parentItem',
            ...
            const pIndex: number = newRecord.parentItem
              ? newRecord.parentItem.index
              : null;
            this.parent.setRecordValue('parentIndex', pIndex, newRecord);
            const parentUniqId: string = newRecord.parentItem
              ? newRecord.parentItem.uniqueID
              : null;
            this.parent.setRecordValue(
              'parentUniqueID',
              parentUniqId,
            ...
            if (
              !isNullOrUndefined(this.parent.taskFields.id) &&
              !isNullOrUndefined(this.parent.taskFields.parentID) &&
              newRecord.parentItem
            ) {
              if (this.parent.viewType === 'ProjectView') {
                this.parent.setRecordValue(
                  this.parent.taskFields.parentID,
                  newRecord.parentItem.taskId,
                  newRecord.taskData,
                  true
                );
              }
          ...
          if (
            this.parent.getParentTask(newRecord.parentItem).childRecords
              .length > 0
          ) {
          ...
          for (let i: number = 0; i < totalRecords.length; i++) {
            if (this.parent.viewType === 'ProjectView') {
              if (
                totalRecords[i as number].parentItem.taskId ===
                newRecord.ganttProperties.taskId
              ) {
                totalRecords[i as number].parentItem.index = newRecord.index;
                totalRecords[i as number].parentItem.taskId =
                  newRecord.ganttProperties.taskId;
                totalRecords[i as number].parentItem.uniqueID =
                  newRecord.ganttProperties.uniqueID;
                totalRecords[i as number].parentUniqueID =
                  newRecord.ganttProperties.uniqueID;
              }
            } else {
    ...
    if (
      this.parent.taskFields.dependency &&
      cAddedRecord.ganttProperties.predecessorsName
    ) {
      if (
        !this.parent.undoRedoModule ||
        !this.parent.undoRedoModule['isUndoRedoPerformed']
      ) {
        this.parent.predecessorModule.ensurePredecessorCollectionHelper(
          cAddedRecord,
          cAddedRecord.ganttProperties
        );
      }
    ...
    if (
      cAddedRecord.parentItem &&
      this.parent.getParentTask(cAddedRecord.parentItem).ganttProperties
        .isAutoSchedule
    ) {
    ...
    if (data.hasChildRecords) {
      ...
      for (let i: number = 0; i < length; i++) {
        ...
        if (childRecords[i as number].hasChildRecords) {
          count = this.getVisibleChildRecordCount(
            childRecords[i as number],
            count,
            collection
          );
  ...
  public updatePredecessorOnIndentOutdent(parentRecord: IGanttData): void {
    ...
    const predecessorCollection: IPredecessor[] =
    ...
    for (let count: number = 0; count < len; count++) {
      const fromRecord: IGanttData = this.parent.getRecordByID(
        predecessorCollection[count as number].from
      );
      const toRecord: IGanttData = this.parent.getRecordByID(
        predecessorCollection[count as number].to
      );
      validPredecessor =
        this.parent.connectorLineEditModule.validateParentPredecessor(
          fromRecord,
          toRecord
        );
      if (!validPredecessor || !this.parent.allowParentDependency) {
        if (
          predecessorCollection[count as number].to ===
          parentRecordTaskData.rowUniqueID.toString()
        ) {
          childRecord = this.parent.getRecordByID(
            predecessorCollection[count as number].from
          );
          predecessorIndex = getIndex(
            predecessorCollection[count as number],
            'from',
            childRecord.ganttProperties.predecessor,
            'to'
          );
          // eslint-disable-next-line
          let predecessorCollections: IPredecessor[] = extend(
            [],
            childRecord.ganttProperties.predecessor,
            [],
            true
          ...
          this.parent.setRecordValue(
            'predecessor',
            predecessorCollections,
            childRecord.ganttProperties,
            true
          );
        } else if (
          predecessorCollection[count as number].from ===
          ...
          const prdcList: string[] =
            childRecord.ganttProperties.predecessorsName.toString().split(',');
          const str: string =
            predecessorCollection[count as number].from +
            predecessorCollection[count as number].type;
          ...
          // eslint-disable-next-line
          const temppredecessorCollection: IPredecessor[] = extend(
            [],
            childRecord.ganttProperties.predecessor,
            [],
            true
    ...
    switch (rowPosition) {
      ...
      case 'Bottom':
        ...
        if (this.parent.viewType === 'ResourceView') {
          const taskId: string =
            record.level === 0
              ? 'R' + record.ganttProperties.taskId
              : 'T' + record.ganttProperties.taskId;
      ...
      case 'Above':
        ...
        updatedCollectionIndex = currentViewData.indexOf(
          this.addRowSelectedItem
        );
        this.recordCollectionUpdate(
          childIndex,
          recordIndex,
      ...
      case 'Below':
        ...
        if (this.addRowSelectedItem.hasChildRecords) {
          const dataChildCount: number = this.getChildCount(
            this.addRowSelectedItem,
            0
          );
          recordIndex = currentItemIndex + dataChildCount + 1;
          updatedCollectionIndex =
            currentViewData.indexOf(this.addRowSelectedItem) +
            this.getVisibleChildRecordCount(
              this.addRowSelectedItem,
              0,
              currentViewData
            ) +
          ...
          updatedCollectionIndex =
            currentViewData.indexOf(this.addRowSelectedItem) + 1;
        }
        this.recordCollectionUpdate(
          childIndex + 1,
          recordIndex,
      ...
      case 'Child':
        ...
        if (this.addRowSelectedItem.hasChildRecords) {
          ...
          if (
            !this.addRowSelectedItem.expanded &&
            !this.parent.enableMultiTaskbar
          ) {
            this.parent.expandByID(
              this.addRowSelectedItem.ganttProperties.rowUniqueID
            );
          }
          updatedCollectionIndex =
            currentViewData.indexOf(this.addRowSelectedItem) +
          ...
          this.parent.setRecordValue(
            'isMilestone',
            false,
            this.addRowSelectedItem.ganttProperties,
            true
          );
          ...
          if (
            !isNullOrUndefined(this.addRowSelectedItem.ganttProperties.segments)
          ) {
  ...
  private recordCollectionUpdate(
    ...
    if (!isNullOrUndefined(parentItem)) {
      if (rowPosition === 'Above') {
        ...
        childIndex =
          parentItem.childRecords.indexOf(this.addRowSelectedItem) + 1;
      } else {
      ...
      if (
        (this.parent.dataSource instanceof DataManager &&
          isNullOrUndefined(
            parentItem.taskData[this.parent.taskFields.parentID]
          )) ||
        ...
        if (
          ...
          parentItem.taskData[child as string].length > 0
        ) {
          if (rowPosition === 'Above' || rowPosition === 'Below') {
            parentItem.taskData[child as string].splice(
              childIndex,
              0,
              record.taskData
            );
  ...
  private constructTaskAddedEventArgs(
    ...
    if (cAddedRecord.length > 1) {
      ...
      for (let i: number = 0; i < cAddedRecord.length; i++) {
        (eventArgs.data[i as number] as IGanttData[]) = cAddedRecord[
          i as number
        ] as IGanttData[];
        eventArgs.newTaskData[i as number] = getTaskData(
          [cAddedRecord[i as number]],
          eventArgs.data[i as number] as boolean,
          eventArgs,
          this.parent
        );
      ...
      for (let i: number = 0; i < cAddedRecord.length; i++) {
        ...
        eventArgs.newTaskData = getTaskData(
          [cAddedRecord[i as number]],
          eventArgs.data as boolean,
          eventArgs,
          this.parent
        );
  ...
  private refreshRecordInImmutableMode(data?: object, dragged?: boolean): void {
    if (!dragged && !isNullOrUndefined(data)) {
      for (
        let i: number = data[0].index + 1;
        i < this.parent.currentViewData.length;
        i++
      ) {
        if (data[0].level < this.parent.currentViewData[i as number].level) {
          this.parent.modifiedRecords.push(
            this.parent.currentViewData[i as number]
          );
        } else {
          break;
        }
    ...
    for (let i: number = 0; i < this.parent.modifiedRecords.length; i++) {
      ...
      const treeIndex: number =
        this.parent.rowDragAndDropModule && this.parent.allowRowDragAndDrop
          ? 1
          : 0;
      const uniqueTaskID: string = this.parent.taskFields.id;
      const originalIndex: number = this.parent.currentViewData.findIndex(
        (data: IGanttData) => {
          return (
            data[uniqueTaskID as string] ===
            originalData[uniqueTaskID as string]
          );
        }
      );
      if (this.parent.treeGrid.getRows()[originalIndex as number]) {
        const row: Row<Column> = this.parent.treeGrid.grid.getRowObjectFromUID(
          this.parent.treeGrid.grid
            .getDataRows()
            [originalIndex as number].getAttribute('data-uid')
        );
        this.parent.treeGrid.renderModule.cellRender({
          data: row.data,
          cell: this.parent.treeGrid.getRows()[originalIndex as number].cells[
            this.parent.treeColumnIndex + treeIndex
          ],
          column:
            this.parent.treeGrid.grid.getColumns()[this.parent.treeColumnIndex],
          requestType: 'rowDragAndDrop',
        });
        this.parent.treeGrid.renderModule.RowModifier({
          data: originalData,
          row: this.parent.treeGrid.getRows()[originalIndex as number],
   ...
   * @private
   */
  public updateRealDataSource(
    addedRecord: IGanttData | IGanttData[],
    rowPosition: RowPosition
  ): void {
    const taskFields: TaskFieldsModel = this.parent.taskFields;
    let dataSource: Object[] = isCountRequired(this.parent)
      ? getValue('result', this.parent.dataSource)
    ...
    for (let i: number = 0; i < (addedRecord as IGanttData[]).length; i++) {
      if (
        isNullOrUndefined(rowPosition) ||
        isNullOrUndefined(this.addRowSelectedItem)
      ) {
      ...
      if (rowPosition === 'Top') {
        ...
        if (
          !isNullOrUndefined(taskFields.id) &&
          !isNullOrUndefined(taskFields.parentID) &&
          rowPosition === 'Child'
        ) {
          dataSource.push(addedRecord[i as number].taskData);
        } else {
          if (!this.isNewRecordAdded) {
            this.addDataInRealDataSource(
              dataSource,
              addedRecord[i as number].taskData,
              rowPosition
            );
          }
  ...
  private addDataInRealDataSource(
    ...
    for (let i: number = 0; i < dataCollection.length; i++) {
      ...
      if (
        ...
        if (
          this.parent.rowDragAndDropModule &&
          this.parent.rowDragAndDropModule['droppedRecord'] &&
          this.parent.viewType === 'ResourceView'
        ) {
          for (let i: number = 0; i < dataCollection.length; i++) {
            if (
              dataCollection[i as number][this.parent.taskFields.id] ===
              this.parent.rowDragAndDropModule['droppedRecord'].ganttProperties
                .taskId
            ) {
              index = i;
              break;
        ...
        if (rowPosition === 'Above') {
          ...
          if (
            ...
            dataCollection[i as number][child as string].length > 0
          ) {
   ...
   * @returns {void} .
   */
  public updateClientDataFromServer(
    e: { addedRecords: Object[]; changedRecords: Object[] },
    ...
    for (
      let j: number = 0, _a: string[] = Object.keys(serverReturnedValue);
      j < _aLength;
      j++
    ) {
      const key: string = _a[parseInt(j.toString(), 10)];
    ...
    if (this.parent.taskFields.id !== null) {
      (args.data as IGanttData).ganttProperties['taskId'] =
        serverReturnedValue[this.parent.taskFields.id];
    }
    if (this.parent.taskFields.name !== null) {
      (args.data as IGanttData).ganttProperties['taskName'] =
        serverReturnedValue[this.parent.taskFields.name];
    }
    if (this.parent.taskFields.startDate !== null) {
      (args.data as IGanttData).ganttProperties['startDate'] =
        serverReturnedValue[this.parent.taskFields.startDate];
    }
    if (this.parent.taskFields.endDate !== null) {
      (args.data as IGanttData).ganttProperties['endDate'] =
        serverReturnedValue[this.parent.taskFields.endDate];
    }
    if (this.parent.taskFields.duration !== null) {
      (args.data as IGanttData).ganttProperties['duration'] = parseFloat(
        serverReturnedValue[this.parent.taskFields.duration]
      );
    }
    if (this.parent.taskFields.durationUnit !== null) {
      (args.data as IGanttData).ganttProperties['durationUnit'] =
        serverReturnedValue[this.parent.taskFields.durationUnit];
    }
    if (this.parent.taskFields.progress !== null) {
      (args.data as IGanttData).ganttProperties['progress'] =
        serverReturnedValue[this.parent.taskFields.progress];
    }
    if (this.parent.taskFields.dependency !== null) {
      (args.data as IGanttData).ganttProperties['dependency'] =
        serverReturnedValue[this.parent.taskFields.dependency];
    }
    if (this.parent.taskFields.parentID !== null) {
      (args.data as IGanttData).ganttProperties['parentID'] =
        serverReturnedValue[this.parent.taskFields.parentID];
    }
    if (this.parent.taskFields.baselineEndDate !== null) {
      (args.data as IGanttData).ganttProperties['baselineEndDate'] =
        serverReturnedValue[this.parent.taskFields.baselineEndDate];
    }
    if (this.parent.taskFields.baselineStartDate !== null) {
      (args.data as IGanttData).ganttProperties['baselineStartDate'] =
        serverReturnedValue[this.parent.taskFields.baselineStartDate];
    }
    if (this.parent.taskFields.resourceInfo !== null) {
      (args.data as IGanttData).ganttProperties['resources'] =
        serverReturnedValue[this.parent.taskFields.resourceInfo];
    }
  }
  ...
  public addRecord(
    ...
    if (this.parent.editModule && this.parent.editSettings.allowAdding) {
      ...
      args = this.constructTaskAddedEventArgs(
        cAddedRecord,
        this.parent.editedRecords,
        'beforeAdd'
      );
      ...
      this.parent.trigger('actionBegin', args, (args: ITaskAddedEventArgs) => {
        ...
        if (this.parent.viewType === 'ResourceView') {
          if (args.data['childRecords'].length > 0) {
            ganttData =
              this.parent.flatData[
                this.parent.getTaskIds().indexOf('R' + args.data[tasks.id])
              ];
          } else {
            ganttData =
              this.parent.flatData[
                this.parent.getTaskIds().indexOf('T' + args.data[tasks.id])
              ];
          }
        } else {
        ...
        if (!isNullOrUndefined(args.data[`${tempTaskID}`])) {
          if (
            args.data[tempTaskID as string] !==
            args.data['ganttProperties']['taskId']
          ) {
            for (const key of Object.keys(this.parent.ids)) {
              if (
                this.parent.ids[key as string] ===
                args.data['ganttProperties']['taskId'].toString()
              ) {
                this.parent.ids[key as string] =
                  args.data[tempTaskID as string].toString();
                break;
              }
            }
            args.data['ganttProperties']['taskId'] =
              args.data[tempTaskID as string];
            args.newTaskData[tempTaskID as string] =
              args.data[tempTaskID as string];
            args.data['ganttProperties']['rowUniqueID'] =
              args.data[tempTaskID as string].toString();
          }
        }
        if (!args.cancel) {
          ...
          if (isRemoteData(this.parent.dataSource)) {
            ...
            if (
              !(
                adaptor instanceof WebApiAdaptor ||
                adaptor instanceof ODataAdaptor ||
                adaptor instanceof ODataV4Adaptor
              ) ||
              data.dataSource.batchUrl
            ) {
              /* tslint:disable-next-line */
              ...
              crud
                .then(
                  (e: { addedRecords: Object[]; changedRecords: Object[] }) => {
                    if (
                      e.addedRecords[0][
                        this.parent.taskFields.id
                      ].toString() !== args.data['ganttProperties']['taskId']
                    ) {
                      args.data['ganttProperties']['taskId'] =
                      ...
                      args.data['ganttProperties']['rowUniqueID'] =
                        e.addedRecords[0][this.parent.taskFields.id].toString();
                      this.parent.ids.push(
                        e.addedRecords[0][this.parent.taskFields.id].toString()
                      );
                    }
                    const prevID: string = (
                      args.data as IGanttData
                    ).ganttProperties.taskId.toString();
                    if (
                      this.parent.taskFields.id &&
                      !isNullOrUndefined(
                        e.addedRecords[0][this.parent.taskFields.id]
                      ) &&
                      e.addedRecords[0][
                        this.parent.taskFields.id
                      ].toString() === prevID
                      ...
                      this.parent.setRecordValue(
                        'taskId',
                        e.addedRecords[0][this.parent.taskFields.id],
                        (args.data as IGanttData).ganttProperties,
                        true
                      );
                      this.parent.setRecordValue(
                        'taskData',
                      ...
                      if (idsIndex !== -1) {
                        this.parent.ids[idsIndex as number] =
                          e.addedRecords[0][
                            this.parent.taskFields.id
                          ].toString();
                      }
                ...
                .catch((e: { result: Object[] }) => {
                  ...
                  this.dmFailure(
                    e as { result: Object[] },
                    args as ITaskbarEditedEventArgs
                  );
              ...
              const insertCrud: Promise<Object> = data.insert(
                updatedData[addedRecords as string],
                null,
                query
              ) as Promise<Object>;
              insertCrud
            ...
            if (this.parent.viewType === 'ProjectView') {
              if (
                ...
                rowPosition === 'Child' ||
                (isNullOrUndefined(rowPosition) &&
                  !(args.data as IGanttData).parentItem)
              ) {
                if (args.data instanceof Array) {
                  this.updateRealDataSource(
                    args.data as IGanttData,
                    rowPosition
                  );
                } else {
              ...
              const dataSource: Object[] = isCountRequired(this.parent)
                ? getValue('result', this.parent.dataSource)
            ...
            if (
              ...
              if (
                this.parent.taskIds.indexOf('R0') !== -1 &&
                this.parent.undoRedoModule &&
                this.parent.undoRedoModule['isUndoRedoPerformed'] &&
                this.parent.undoRedoModule['currentAction']['action'] ===
                  'Delete' &&
                ...
                const unassignedTask: IGanttData =
                  this.parent.flatData[this.parent.taskIds.indexOf('R0')];
                let isPresent: IGanttData[] =
                  unassignedTask.childRecords.filter((data: IGanttData) => {
                    return (
                      data.ganttProperties.taskId ===
                      args.data['ganttProperties'].taskId
                    );
                ...
                for (let j: number = 0; j < isPresent.length; j++) {
                  if (unassignedTask.childRecords.length === 1) {
                    this.parent.flatData.splice(
                      this.parent.taskIds.indexOf('R0'),
                      2
                    );
                    this.parent.ids.splice(
                      this.parent.taskIds.indexOf('R0'),
                    ...
                    this.parent.taskIds.splice(
                      this.parent.taskIds.indexOf('R0'),
                      2
                    );
                  } else {
                    let index: number = this.parent.taskIds.indexOf(
                      'T' + isPresent[j as number].ganttProperties.taskId
                    );
                    const id: string =
                      'T' + isPresent[j as number].ganttProperties.taskId;
                ...
                if (unassignedTask && this.parent.viewType === 'ResourceView') {
                  const isValid: IGanttData =
                    this.parent.flatData[this.parent.taskIds.indexOf('R0') + 1];
                  if (!isValid) {
                    this.parent.flatData.splice(
                      this.parent.taskIds.indexOf('R0'),
                      1
            ...
            if (
              ...
              record['action'] = 'Add';
              const tempArray: IGanttData[] =
                (args.data as IGanttData[]).length > 0
                  ? (extend(
                      [],
                      [],
                      args.data as IGanttData[],
              ...
              for (let i: number = 0; i < tempArray.length; i++) {
                addedRec = this.parent.flatData.filter((data: IGanttData) => {
                  return (
                    tempArray[i as number].index === data.index &&
                    tempArray[i as number]['ganttProperties'].taskId ===
                      data.ganttProperties.taskId
                  );
                });
              }
  ...
  public createNewRecord(): IGanttData {
    ...
    for (let i: number = 0; i < ganttColumns.length; i++) {
      ...
      if (fieldName === taskSettingsFields.id) {
        ...
        ganttColumns[i as number].field === taskSettingsFields.startDate
      ) {
        if (isNullOrUndefined(tempRecord[taskSettingsFields.endDate])) {
          tempRecord[fieldName as string] =
            this.parent.editModule.dialogModule.getMinimumStartDate();
        } else {
          tempRecord[fieldName as string] = new Date(
            tempRecord[taskSettingsFields.endDate]
          );
        }
        if (this.parent.timezone) {
          tempRecord[fieldName as string] =
            this.parent.dateValidationModule.remove(
              tempRecord[fieldName as string],
              this.parent.timezone
            );
        }
      } else if (
        ganttColumns[i as number].field === taskSettingsFields.endDate
      ) {
        if (isNullOrUndefined(tempRecord[taskSettingsFields.startDate])) {
          ...
          tempRecord[fieldName as string] = new Date(
            tempRecord[taskSettingsFields.startDate]
          );
        }
        if (this.parent.timezone) {
          tempRecord[fieldName as string] =
        ...
        ganttColumns[i as number].field === taskSettingsFields.duration
      ) {
        tempRecord[fieldName as string] = 1;
      } else if (ganttColumns[i as number].field === taskSettingsFields.name) {
        tempRecord[fieldName as string] =
          this.parent.editModule.dialogModule['localeObj'].getConstant(
            'addDialogTitle'
          ) +
          ' ' +
          taskId;
      } else if (
        ganttColumns[i as number].field === taskSettingsFields.progress
      ) {
        tempRecord[fieldName as string] = 0;
      } else if (ganttColumns[i as number].field === taskSettingsFields.work) {
        tempRecord[fieldName as string] = 0;
      } else if (
        ganttColumns[i as number].field === taskSettingsFields.type ||
        ganttColumns[i as number].field === 'taskType'
      ) {
        tempRecord[fieldName as string] = this.parent.taskType;
      } else if (
        ganttColumns[i as number].field === taskSettingsFields.milestone
      ) {
  ...
  public validateTaskPosition(
    ...
    const selectedRowIndex: number =
      isNullOrUndefined(rowIndex) || isNaN(parseInt(rowIndex.toString(), 10))
        ? this.parent.selectionModule
          ? (this.parent.selectionSettings.mode === 'Row' ||
              this.parent.selectionSettings.mode === 'Both') &&
            ...
            : this.parent.selectionSettings.mode === 'Cell' &&
              this.parent.selectionModule.getSelectedRowCellIndexes().length ===
                1
            ? this.parent.selectionModule.getSelectedRowCellIndexes()[0]
                .rowIndex
            : null
          : null
        : rowIndex;
    this.addRowSelectedItem = isNullOrUndefined(selectedRowIndex)
      ? null
      : this.parent.updatedRecords[selectedRowIndex as number];
    rowPosition = isNullOrUndefined(rowPosition)
      ? this.parent.editSettings.newRowPosition
      : rowPosition;
    data = isNullOrUndefined(data) ? this.createNewRecord() : data;
    if (
      ((isNullOrUndefined(selectedRowIndex) ||
        selectedRowIndex < 0 ||
        isNullOrUndefined(this.addRowSelectedItem)) &&
        (rowPosition === 'Above' ||
          rowPosition === 'Below' ||
    ...
    switch (rowPosition) {
      ...
      case 'Below':
        ...
        parentItem = this.parent.getParentTask(
          this.addRowSelectedItem.parentItem
        );
        break;
      case 'Child':
    ...
    const AddRecord: IGanttData = this.updateNewlyAddedDataBeforeAjax(
      data,
      level,
      rowPosition,
      parentItem
  ...
  private updateTreeGridUniqueID(data: IGanttData, action: string): void {
    if (action === 'add') {
      setValue(
        'uniqueIDCollection.' + data.uniqueID,
        data,
        this.parent.treeGrid
      );
    } else if (action === 'delete') {
      deleteObject(
        getValue('uniqueIDCollection', this.parent.treeGrid),
        data.uniqueID
      );
    }
  }
  ...
  private refreshNewlyAddedRecord(
    ...
    if (
      ...
      this.parent.staticSelectedRowIndex = this.parent.currentViewData.indexOf(
        args.data as IGanttData
      );
    }
    if (this.parent.timelineSettings.updateTimescaleView) {
      ...
      if (args.modifiedRecords.length > 0) {
        tempArray =
          (args.data as IGanttData[]).length > 0
            ? (extend([], [], args.data as IGanttData[], true) as IGanttData[])
            : [args.data as IGanttData]; // eslint-disable-next-line
    ...
    if (
      this.dialogModule.isAddNewResource &&
      !this.parent.isEdit &&
      this.parent.taskFields.work
    ) {
    ...
    if (this.parent.viewType === 'ResourceView') {
      if (cAddedRecord.length > 1) {
        for (let i: number = 0; i < cAddedRecord.length; i++) {
          (
            args.data[i as number] as IGanttData
          ).ganttProperties.sharedTaskUniqueIds.push(
            (args.data[i as number] as IGanttData).ganttProperties.rowUniqueID
          );
          if (
            (args.data[i as number] as IGanttData).ganttProperties.resourceInfo
          ) {
            // if ((args.data[i] as IGanttData).ganttProperties.resourceInfo.length > 1) {
            const resources: Object[] = extend(
              [],
              [],
              (args.data[i as number] as IGanttData).ganttProperties
                .resourceInfo,
              true
            ) as Object[];
            ...
            if (
              !this.parent.undoRedoModule ||
              (this.parent.undoRedoModule &&
                !args.data[i as number]['hasChildRecords'] &&
                this.parent.undoRedoModule['isUndoRedoPerformed']) ||
              !this.parent.undoRedoModule['isUndoRedoPerformed']
            ) {
              this.removeChildRecord(args.data[i as number] as IGanttData);
              this.parent.editModule.checkWithUnassignedTask(
                args.data[i as number] as IGanttData
              );
            }
          }
        ...
        (args.data as IGanttData).ganttProperties.sharedTaskUniqueIds.push(
          (args.data as IGanttData).ganttProperties.rowUniqueID
        );
        // eslint-disable-next-line
        if (
          (args.data as IGanttData).ganttProperties.resourceInfo &&
          (args.data as IGanttData).ganttProperties.resourceInfo.length
        ) {
          if (
            (args.data as IGanttData).ganttProperties.resourceInfo.length > 1
          ) {
            // eslint-disable-next-line
            const resources: Object[] = extend(
              [],
  ...
  private removeAddedRecord(): void {
    ...
    const currentViewData: IGanttData[] = this.parent.currentViewData;
    const ids: string[] = this.parent.ids;
    const flatRecordsIndex: number = flatRecords.indexOf(
      this.newlyAddedRecordBackup
    );
    const currentViewDataIndex: number = currentViewData.indexOf(
      this.newlyAddedRecordBackup
    );
    const idsIndex: number = ids.indexOf(
      this.newlyAddedRecordBackup.ganttProperties.rowUniqueID.toString()
    );
    deleteObject(
      this.parent.previousRecords,
      flatRecords[flatRecordsIndex as number].uniqueID
    );
    if (this.newlyAddedRecordBackup.parentItem) {
      const parentItem: IGanttData = this.parent.getParentTask(
        this.newlyAddedRecordBackup.parentItem
      );
      const parentIndex: number = parentItem.childRecords.indexOf(
        this.newlyAddedRecordBackup
      );
  ...
  private getPrevRecordIndex(): number {
    const prevRecord: IGanttData =
      this.parent.updatedRecords[
        this.parent.selectionModule.getSelectedRowIndexes()[0] - 1
      ];
    const selectedRecord: IGanttData =
      this.parent.selectionModule.getSelectedRecords()[0];
    const parent: IGanttData = this.parent.getRootParent(
      prevRecord,
  ...
  public indent(): void {
    ...
    const isSelected: boolean = this.parent.selectionModule
      ? this.parent.selectionModule.selectedRowIndexes.length === 1 ||
    ...
    if (
      ...
      !isSelected ||
      this.parent.viewType === 'ResourceView' ||
      this.parent.updatedRecords[index as number].level - prevRecord.level === 1
    ) {
      return;
    } else {
      ...
      this.indentOutdentRow(
        [this.parent.selectionModule.getSelectedRowIndexes()[0]],
        dropIndex,
        'child'
      );
    }
  ...
  public outdent(): void {
    ...
    if (
      ...
      !isSelected ||
      this.parent.viewType === 'ResourceView' ||
      this.parent.updatedRecords[index as number].level === 0
    ) {
      return;
    } else {
      const thisParent: IGanttData = this.parent.getTaskByUniqueID(
        (this.parent.selectionModule.getSelectedRecords()[0] as IGanttData)
          .parentItem.uniqueID
      );
  ...
  private indentOutdentRow(
    ...
    if ((fromIndexes[0] !== toIndex && pos === 'above') || 'below' || 'child') {
      ...
      for (let i: number = 0; i < fromIndexes.length; i++) {
        if (
          ...
          if (this.parent.undoRedoModule['currentAction']['modifiedRecord']) {
            record[i as number] =
              this.parent.undoRedoModule['currentAction']['modifiedRecord'][
                i as number
              ].data;
          } else {
            record[i as number] =
              this.parent.undoRedoModule['currentAction']['data'][i as number];
          }
        } else {
          record[i as number] =
            this.parent.updatedRecords[fromIndexes[i as number]];
        }
      }
      ...
      this.parent.trigger(
        'actionBegin',
        actionArgs,
        (actionArg: IActionBeginEventArgs) => {
          if (
            !isNullOrUndefined(this.parent.loadingIndicator) &&
            this.parent.loadingIndicator.indicatorType === 'Shimmer'
          ) {
  ...
  private reArrangeRows(args: RowDropEventArgs, isByMethod?: boolean): void {
    ...
    if (args.dropPosition !== 'Invalid' && this.parent.editModule) {
      ...
      const action: string =
        args.dropPosition === 'middleSegment' ? 'Indent' : 'Outdent';
      if (
        this.parent.undoRedoModule &&
        !this.parent.undoRedoModule['isUndoRedoPerformed'] &&
        this.parent['isUndoRedoItemPresent'](action)
      ) {
        const record: Object = {};
        record['action'] = action;
        record['modifiedRecord'] = [];
        record['selectedRowIndexes'] = extend(
          [],
          [],
          this.parent.selectionModule.selectedRowIndexes,
          true
        );
        this.parent.undoRedoModule['findPosition'](
          extend([], [], [args.data as IGanttData], true)[0],
          record,
          'modifiedRecord'
        );
        ...
        if (this.dropPosition !== 'Invalid') {
          ...
          if (this.dropPosition === 'bottomSegment') {
            if (!droppedRec.hasChildRecords) {
              if (
                this.parent.taskFields.parentID &&
                (this.ganttData as IGanttData[]).length > 0
              ) {
                (this.ganttData as IGanttData[]).splice(
                  recordIndex1 + 1,
                  0,
                  this.draggedRecord.taskData
                );
              }
              ...
              this.parent.ids.splice(
                recordIndex1 + 1,
                0,
                this.draggedRecord.ganttProperties.rowUniqueID.toString()
              );
            } else {
              ...
              const idIndex: number = this.parent.ids.indexOf(
                this.draggedRecord[this.parent.taskFields.id].toString()
              );
            ...
            this.parent.setRecordValue(
              'parentItem',
              this.treeGridData[recordIndex1 as number].parentItem,
              draggedRec
            );
            this.parent.setRecordValue(
              'parentUniqueID',
              this.treeGridData[recordIndex1 as number].parentUniqueID,
              draggedRec
            );
            this.parent.setRecordValue(
              'level',
              this.treeGridData[recordIndex1 as number].level,
              draggedRec
            );
            ...
            if (droppedRec.parentItem) {
              const record: IGanttData[] = this.parent.getParentTask(
                droppedRec.parentItem
              ).childRecords;
              const childRecords: IGanttData[] = record;
              const droppedRecordIndex: number =
                childRecords.indexOf(droppedRec) + 1;
          ...
          if (
            !isNullOrUndefined(
              draggedRec.parentItem &&
                this.updateParentRecords.indexOf(draggedRec.parentItem) !== -1
            )
          ) {
            this.updateParentRecords.push(
              this.parent.getTaskByUniqueID(draggedRec.parentItem.uniqueID)
            );
          }
        }
        if (isNullOrUndefined(draggedRec.parentItem)) {
          const parentRecords: ITreeData[] = this.parent.treeGrid.parentData;
          const newParentIndex: number = parentRecords.indexOf(
            this.droppedRecord
          );
          if (this.dropPosition === 'bottomSegment') {
      ...
      if (this.dropPosition === 'middleSegment') {
        if (!isNullOrUndefined(droppedRec.ganttProperties.predecessor)) {
          ...
          for (let count: number = len - 1; count >= 0; count--) {
            if (!isNullOrUndefined(droppedRec.ganttProperties.predecessor)) {
              ...
              const validPredecessor: boolean =
                this.parent.connectorLineEditModule.validateParentPredecessor(
                  fromRecord,
                  toRecord
                );
              if (
                droppedRec.ganttProperties.predecessor &&
                (!validPredecessor || !this.parent.allowParentDependency)
              ) {
        ...
        if (droppedRec.ganttProperties.isMilestone) {
          this.parent.setRecordValue(
            'isMilestone',
            false,
            droppedRec.ganttProperties,
            true
          );
          if (
            !isNullOrUndefined(
              droppedRec.taskData[this.parent.taskFields.milestone]
            )
          ) {
            if (
              droppedRec.taskData[this.parent.taskFields.milestone] === true
            ) {
  ...
  public refreshRecord(args: RowDropEventArgs, isDrag?: boolean): void {
    if (isRemoteData(this.parent.dataSource)) {
      ...
      const queryValue: Query =
        this.parent.query instanceof Query ? this.parent.query : new Query();
      let crud: Promise<Object> = null;
      const adaptor: AdaptorOptions = data.adaptor;
      if (
        !(
          adaptor instanceof WebApiAdaptor && adaptor instanceof ODataAdaptor
        ) ||
        data.dataSource.batchUrl
      ) {
        crud = data.saveChanges(
      ...
      crud
        .then((e: ReturnType) => this.indentSuccess(e, args, isDrag))
        .catch((e: { result: Object[] }) =>
          this.indentFailure(e as { result: Object[] })
        );
    } else {
  ...
  private refreshDataSource(): void {
    ...
    if (
      (tempData as IGanttData[]).length > 0 &&
      !isNullOrUndefined(droppedRec) &&
      !droppedRec.parentItem
    ) {
      for (let i: number = 0; i < Object.keys(tempData).length; i++) {
        if (
          tempData[i as number][this.parent.taskFields.child] ===
          droppedRec.taskData[this.parent.taskFields.child]
        ) {
          indx = i;
        }
      }
      ...
      if (
        ...
        const rowPos: RowPosition =
          this.dropPosition === 'topSegment' ? 'Above' : 'Below';
    ...
    if (this.parent.taskFields.parentID) {
      if (draggedRec.parentItem) {
        if (
          ...
          draggedRec[this.parent.taskFields.parentID] =
            droppedRec[this.parent.taskFields.parentID];
          draggedRec.taskData[this.parent.taskFields.parentID] =
            droppedRec[this.parent.taskFields.parentID];
          draggedRec.ganttProperties['parentId'] =
            droppedRec[this.parent.taskFields.parentID];
        } else {
          draggedRec[this.parent.taskFields.parentID] =
            droppedRec[this.parent.taskFields.id];
          draggedRec.taskData[this.parent.taskFields.parentID] =
            droppedRec[this.parent.taskFields.id];
          draggedRec.ganttProperties['parentId'] =
            droppedRec[this.parent.taskFields.id];
  ...
  private deleteDragRow(): void {
    if (this.parent.dataSource instanceof DataManager) {
      ...
      this.ganttData = isCountRequired(this.parent)
        ? getValue('result', this.parent.dataSource)
        : this.parent.dataSource;
    }
    this.treeGridData = isCountRequired(this.parent)
      ? getValue('result', this.parent.treeGrid.dataSource)
      : this.parent.treeGrid.dataSource;
    const delRow: IGanttData = this.parent.getTaskByUniqueID(
      this.draggedRecord.uniqueID
    );
  ...
  private updateIndentedChildRecords(indentedRecord: IGanttData): void {
    ...
    for (let i: number = 0; i < indentedRecord.childRecords.length; i++) {
      this.parent.setRecordValue(
        'parentItem',
        createParentItem,
        indentedRecord.childRecords[i as number]
      );
      this.parent.setRecordValue(
        'parentUniqueID',
        indentedRecord.uniqueID,
        indentedRecord.childRecords[i as number]
      );
    }
  ...
  private dropMiddle(recordIndex1: number): void {
    ...
    if (
      ...
      this.parent.undoRedoModule['isUndoRedoPerformed'] &&
      this.parent.undoRedoModule['currentAction']['modifiedRecord'] &&
      this.parent.undoRedoModule['currentAction']['modifiedRecord'][0]
        .position !== 'child'
    ) {
      if (
        this.parent.undoRedoModule['currentAction']['modifiedRecord'][0]
        ...
        childRecordsLength = this.parent.ids.indexOf(
          this.parent.undoRedoModule['currentAction'][
            'modifiedRecord'
          ][0].id.toString()
        );
      ...
      childRecordsLength =
        isNullOrUndefined(childRec) || childRec === 0
          ? recordIndex1 + 1
          : childRec + recordIndex1 + 1;
    }
    if (this.dropPosition === 'middleSegment') {
      if (
        obj.taskFields.parentID &&
        (this.ganttData as IGanttData[]).length > 0
      ) {
        (this.ganttData as IGanttData[]).splice(
          childRecordsLength,
  ...
  private updateChildRecordLevel(record: IGanttData, levl: number): number {
    ...
    for (let j: number = 0; j < length; j++) {
      ...
      if (record.parentItem) {
        ...
        parentData =
          this.parent.treeGrid[id as string][record.parentItem.uniqueID];
      }
      currentRec.level = record.parentItem
        ? parentData.level + levl
        : record.level + 1;
  ...
  private updateChildRecord(
    ...
    for (let i: number = 0; i < length; i++) {
      ...
      this.parent.ids.splice(
        count,
        0,
        currentRec.ganttProperties.rowUniqueID.toString()
      );
      if (
        obj.taskFields.parentID &&
        (this.ganttData as IGanttData[]).length > 0
  ...
  private removeRecords(record: IGanttData): void {
    ...
    if (delRow) {
      const flatParent: IGanttData = this.parent.getParentTask(
        delRow.parentItem
      );
      if (delRow.parentItem) {
        const childRecords: IGanttData[] = flatParent
          ? flatParent.childRecords
          : [];
        let childIndex: number = 0;
        if (childRecords && childRecords.length > 0) {
          ...
          if (!this.parent.taskFields.parentID) {
            flatParent.taskData[this.parent.taskFields.child].splice(
              childIndex,
              1
            );
          }
      ...
      if (obj.taskFields.parentID) {
        ...
        for (let i: number = 0; i < ganttData.length; i++) {
          if (
            ganttData[i as number][this.parent.taskFields.id] ===
            delRow.taskData[this.parent.taskFields.id]
          ) {
            indx = i;
          }
        }
        if (indx !== -1) {
          ...
          for (let i: number = 0; i < this.treeGridData.length; i++) {
            if (
              this.treeGridData[i as number][this.parent.taskFields.id] ===
              delRow.taskData[this.parent.taskFields.id]
            ) {
              gridIndx = i;
            }
          ...
          if (this.parent.treeGrid.parentData.indexOf(delRow) !== -1) {
            this.parent.treeGrid.parentData.splice(
              this.parent.treeGrid.parentData.indexOf(delRow),
              1
            );
          }
        }
      ...
      if (!obj.taskFields.parentID) {
        ...
        const parentIndex: number = (this.ganttData as IGanttData[]).indexOf(
          delRow.taskData
        );
        if (parentIndex !== -1) {
      ...
      if (
        ...
        flatParent &&
        flatParent.childRecords &&
        !flatParent.childRecords.length
      ) {
  ...
  private removeChildItem(record: IGanttData): void {
    ...
    for (let i: number = 0; i < record.childRecords.length; i++) {
      ...
      for (let j: number = 0; j < (<IGanttData[]>data).length; j++) {
        if (
          data[j as number][this.parent.taskFields.id] ===
          currentRec.taskData[this.parent.taskFields.id]
        ) {
          indx = j;
        }
      }
  ...
  private recordLevel(): void {
    ...
    if (this.dropPosition === 'middleSegment') {
      ...
      droppedRec.childRecords.splice(
        droppedRec.childRecords.length,
        0,
        draggedRec
      );
      if (
        !isNullOrUndefined(draggedRec) &&
        !obj.taskFields.parentID &&
        !isNullOrUndefined(droppedRec.taskData[childItem as string])
      ) {
        droppedRec.taskData[obj.taskFields.child].splice(
          droppedRec.childRecords.length,
          0,
          draggedRec.taskData
        );
      ...
      if (
        !isNullOrUndefined(droppedRec.ganttProperties.segments) &&
        droppedRec.ganttProperties.segments.length > 0
      ) {
