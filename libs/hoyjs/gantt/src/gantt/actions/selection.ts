...
export class Selection {
  ...
  constructor(gantt: Gantt) {
    ...
    this.parent.treeGrid.grid.selectionSettings.enableToggle =
      this.parent.selectionSettings.enableToggle;
    this.parent.treeGrid.selectionSettings = getActualProperties(
  ...
  public selectRowByIndex(): void {
    if (
      ...
      this.selectRow(
        this.parent.staticSelectedRowIndex !== -1
          ? this.parent.staticSelectedRowIndex
  ...
  private rowSelecting(args: RowSelectingEventArgs): void {
    ...
    if (
      !isNullOrUndefined(args.foreignKeyData) &&
      Object.keys(args.foreignKeyData).length === 0
    ) {
  ...
  private rowSelected(args: RowSelectEventArgs): void {
    ...
    const index: number[] =
      ...
      !isNullOrUndefined(args[rowIndexes as string])
        ? args[rowIndexes as string]
        : [args.rowIndex];
    this.addRemoveClass(index, args['name']);
    this.selectedRowIndexes = extend(
      [],
      this.getSelectedRowIndexes(),
      [],
      true
    ) as number[];
    ...
    if (this.parent.autoFocusTasks) {
      ...
      if (args.data && !isNullOrUndefined(args.data['length'])) {
        for (let i: number = 0; i < args.data['length']; i++) {
          this.parent.ganttChartModule.updateScrollLeft(
            args.data[i as number].ganttProperties.left
          );
        }
      } else {
        this.parent.ganttChartModule.updateScrollLeft(
          getValue('data.ganttProperties.left', args)
        );
      }
    }
    args.target = this.actualTarget as Element;
  ...
  private cellSelecting(args: CellSelectingEventArgs): void | Deferred {
    ...
    this.parent.trigger(
      'cellSelecting',
      args,
      (cellselectingArgs: CellSelectingEventArgs) => {
  ...
  public selectRow(
    ...
    if (
      ...
      (this.parent.treeGridModule.addedRecord ||
        (this.parent.editModule && this.parent.editModule.isAdded))
    ) {
      index = this.parent
        .getExpandedRecords(this.parent.flatData)
        .indexOf(this.parent.currentViewData[index as number]);
      this.parent.treeGridModule.addedRecord = false;
      if (this.parent.editModule) {
        this.parent.editModule.isAdded = false;
      }
    }
    const selectedRow: HTMLElement = ganttRow.filter(
      (e: HTMLElement) =>
        parseInt(e.getAttribute('data-rowindex'), 10) === index
    )[0];
    let condition: boolean;
    if (
      index === -1 ||
      (isNullOrUndefined(selectedRow) && !this.parent.enableVirtualization) ||
    ...
    if (
      !isNullOrUndefined(this.selectedClass) &&
    ...
    this.parent.treeGrid.grid.selectionModule.preventFocus =
      this.parent.treeGrid.grid.selectionModule.preventFocus === true
        ? false
        : this.parent.treeGrid.grid.selectionModule.preventFocus;
  ...
  public getSelectedRecords(): Object[] {
    if (
      ...
      const selectedIndexes: number[] =
        this.parent.selectionModule.getSelectedRowIndexes();
      for (let i: number = 0; i < selectedIndexes.length; i++) {
        const rec: IGanttData = this.parent.currentViewData.filter(
          (data: IGanttData) => {
  ...
  public getCellSelectedRecords(): IGanttData[] {
    const cellDetails: ISelectedCell[] =
    ...
    for (let i: number = 0; i < cellDetails.length; i++) {
      cellSelectedRecords.push(
        this.parent.currentViewData[cellDetails[i as number].rowIndex]
      );
    }
  ...
  private highlightSelectedRows(e: PointerEvent, fromChart: boolean): void {
    ...
    if (fromChart) {
      const selectedRow: Element = closest(
        e.target as Element,
        'tr.e-chart-row'
      );
      const rIndex: number = parseInt(
        selectedRow.getAttribute('data-rowindex'),
        10
      );
      const isToggle: boolean = this.parent.selectionSettings.enableToggle;
      if (
        this.parent.selectionSettings.type === 'Single' ||
        ...
        if (this.isMultiShiftRequest) {
          this.selectRowsByRange(
            isNullOrUndefined(this.prevRowIndex) ? rIndex : this.prevRowIndex,
            rIndex
          );
        } else {
          setValue(
            'isMultiCtrlRequest',
            true,
            this.parent.treeGrid.grid.selectionModule
          );
          this.parent.treeGrid.grid.selectionModule.addRowsToSelection([
            rIndex,
          ]);
          const isUnSelected: boolean =
            this.selectedRowIndexes.indexOf(rIndex) > -1;
          if (isUnSelected) {
  ...
  private addRemoveClass(records: number[], request?: string): void {
    ...
    for (let i: number = 0; i < records.length; i++) {
      const selectedRow: HTMLElement = ganttRow.filter(
        (e: HTMLElement) =>
          parseInt(e.getAttribute('data-rowindex'), 10) ===
          records[parseInt(i.toString(), 10)]
      )[0];
      if (!isNullOrUndefined(selectedRow)) {
        let persist: boolean = false;
        const index: number = this.getSelectedRowIndexes().indexOf(
          records[parseInt(i.toString(), 10)]
        );
        ...
        if (
          ...
          index > -1 &&
          this.parent.selectionSettings.persistSelection &&
          request === 'chartMouseUp' &&
          this.isSelectionFromChart
        ) {
          this.removeClass(selectedRow);
        }
      }
  ...
  private showPopup(e: MouseEvent): void {
    if (this.isSelectionFromChart) {
      setCssInGridPopUp(
        <HTMLElement>this.parent.element.querySelector('.e-ganttpopup'),
        e,
        'e-rowselect e-icons e-icon-rowselect' +
          (this.enableSelectMultiTouch &&
          (this.getSelectedRecords().length > 1 ||
            this.getSelectedRowCellIndexes().length > 1)
            ? ' e-spanclicked'
            : '')
      );
      (<HTMLElement>(
  ...
  public hidePopUp(): void {
    if (this.openPopup) {
      (<HTMLElement>(
        document.getElementsByClassName('e-ganttpopup')[0]
      )).style.display = 'none';
  ...
  private popUpClickHandler(e: MouseEvent): void {
    ...
    const $popUpElemet: Element = closest(target, '.e-ganttpopup')
      ? closest(target, '.e-ganttpopup')
      : closest(target, '.e-gridpopup');
    if ($popUpElemet) {
      const spanElement: Element = $popUpElemet.querySelector(
        '.' + 'e-rowselect'
      );
      if (
        closest(target, '.e-ganttpopup') &&
    ...
    if (grid) {
      setValue(
        'enableSelectMultiTouch',
        this.enableSelectMultiTouch,
        grid.selectionModule
      );
    }
  }
  ...
  private mouseUpHandler(e: PointerEvent): void {
    ...
    if ((e.target as Element).closest('.e-rowcell')) {
      ...
      targetElement =
        (e.target as Element).closest('.e-left-label-container') ||
        (e.target as Element).closest('.e-taskbar-main-container') ||
        (e.target as Element).closest('.e-right-label-container');
    }
    ...
    if (
      !isTaskbarEdited &&
      this.parent.element.contains(e.target as Node) &&
      !(elements.length === 1)
    ) {
  ...
  public maintainSelectedRecords(i: number): void {
    const index: number = this.parent.selectionModule
