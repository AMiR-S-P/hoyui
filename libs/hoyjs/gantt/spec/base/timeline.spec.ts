 ...
 * Gantt base spec
 */
import { createElement, remove, extend } from '@hoyui/base';
import {
  Gantt,
  Selection,
  Toolbar,
...
describe('Gantt-Timeline', () => {
  describe('Gantt base module', () => {
    ...
    it('Default Mode', (done: Function) => {
      ...
      ganttObj.dataBound = () => {
        expect(
          ganttObj.timelineModule.customTimelineSettings.topTier.unit
        ).toBe('Week');
        expect(
          ganttObj.timelineModule.customTimelineSettings.bottomTier.unit
        ).toBe('Day');
        expect(
          ganttObj.timelineModule.customTimelineSettings.topTier.count
        ).toBe(1);
        expect(
          ganttObj.timelineModule.customTimelineSettings.bottomTier.count
        ).toBe(1);
        expect(
          ganttObj.timelineModule.customTimelineSettings.topTier.format
        ).toBe('MMM dd, yyyy');
        expect(
          ganttObj.timelineModule.customTimelineSettings.bottomTier.format
        ).toBe('');
        expect(
        ...
        expect(
          ganttObj.getFormatedDate(
            ganttObj.timelineModule.timelineRoundOffEndDate,
            'M/d/yyyy'
          )
        ).toBe('3/25/2018');
        expect(
          ganttObj.element.querySelector('.' + cls.timelineHeaderContainer)
            .childElementCount
        ).toBe(2);
        done();
      };
    ...
    it('Custom Formatting the timeline', () => {
      ...
      let element: HTMLElement = ganttObj.element.querySelector(
        '#' +
          ganttObj.element.id +
          'GanttChart > div.e-timeline-header-container > table:nth-child(1) > thead > tr > th.e-timeline-top-header-cell > div'
      ) as HTMLElement;
        ...
        expect(
          ganttObj.element
            .querySelector(
              '#' +
                ganttObj.element.id +
                'GanttChart > div.e-timeline-header-container > table:nth-child(1) > thead > tr > th'
            )
            .getAttribute('aria-label')
            .indexOf('Timeline cell 1/27/2018') > -1
        ).toBeTruthy();
        expect(
          ganttObj.element
            .querySelector(
              '#' +
                ganttObj.element.id +
                'GanttChart > div.e-timeline-header-container > table:nth-child(2) > thead > tr > th:nth-child(3)'
            )
            .getAttribute('aria-label')
            .indexOf('Timeline cell 1/29/2018') > -1
        ).toBeTruthy();
        done();
      };
      ganttObj.refresh();
  ...
  describe('Check timeline dates', () => {
    ...
    it('Check timeline start date after right resize', () => {
      ...
      let dragElement: HTMLElement = ganttObj.element.querySelector(
        '#' +
          ganttObj.element.id +
          'GanttTaskTableBody > tr:nth-child(1) > td > div.e-taskbar-main-container > div.e-taskbar-right-resizer.e-icon'
      ) as HTMLElement;
      triggerMouseEvent(
        dragElement,
        'mousedown',
  ...
  describe('Fit to project issue', () => {
    ...
    it('check fit to project works properly', () => {
      ...
      ganttObj.dataBind();
      expect(
        ganttObj.getFormatedDate(ganttObj.cloneProjectStartDate, 'MM/dd/yyyy')
      ).toEqual('04/02/2013');
    });
    afterAll(() => {
      if (ganttObj) {
  ...
  describe('Fit to project display wrong timeline', () => {
    ...
    it('check timeline renders properly', () => {
      ...
      ganttObj.dataBind();
      expect(
        ganttObj.getFormatedDate(ganttObj.cloneProjectEndDate, 'MM/dd/yyyy')
      ).toEqual('04/19/2019');
    });
    afterAll(() => {
      if (ganttObj) {
  ...
  describe('Bug-841056:console error occurs while using segment data', () => {
    ...
    it('Checking 1st segments startdate and enddate', () => {
      expect(
        ganttObj.getFormatedDate(
          ganttObj.currentViewData[0].ganttProperties.segments[0].startDate,
          'MM/dd/yyyy HH:mm'
        )
      ).toEqual('04/02/2019 08:00');
      expect(
        ganttObj.getFormatedDate(
          ganttObj.currentViewData[0].ganttProperties.segments[0].endDate,
          'MM/dd/yyyy HH:mm'
        )
      ).toEqual('04/16/2019 17:00');
      expect(
        ganttObj.getFormatedDate(
          ganttObj.currentViewData[0].ganttProperties.segments[1].endDate,
          'MM/dd/yyyy HH:mm'
        )
      ).toEqual('04/25/2019 17:00');
    });
    afterAll(() => {
      if (ganttObj) {
        destroyGantt(ganttObj);
