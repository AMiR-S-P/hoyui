...
describe('Gantt toolbar support', () => {
  describe('Gantt toolbar action', () => {
    ...
    it('Check all toolbar rendered properly', () => {
      let toolbar: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_Gantt_Toolbar'
      ) as HTMLElement;
    ...
    it('Ensuring proper toolbar display', () => {
      ...
      ganttObj.dataBind();
      expect(ganttObj.element.getElementsByClassName('e-hidden').length).toBe(
        4
      );
    });
    ...
    it('Add handler function', () => {
      ...
      let startDate: HTMLInputElement = <HTMLInputElement>(
        document.querySelector('#' + ganttObj.element.id + 'StartDate')
      );
      if (startDate) {
        let StartDateInput: any = (
          document.getElementById(ganttObj.element.id + 'StartDate') as any
        ).ej2_instances[0];
        StartDateInput.value = new Date('02/06/2017');
      }
      let save: HTMLElement = document
        .querySelector('#' + ganttObj.element.id + '_dialog')
        .getElementsByClassName('e-primary')[0] as HTMLElement;
    ...
    it('Previous Timespan handler function', () => {
      let previoustimespanToolbar: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_prevtimespan'
      ) as HTMLElement;
      triggerMouseEvent(previoustimespanToolbar, 'click');
      expect(
        ganttObj.getFormatedDate(
          ganttObj.timelineModule.timelineStartDate,
          'MM/dd/yyyy'
        )
      ).toBe('01/29/2017');
    });
    ...
    it('Next Timespan handler function', () => {
      let nexttimespanToolbar: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_nexttimespan'
      ) as HTMLElement;
    ...
    it('Edit handler function', () => {
      ...
      let editToolbar: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_edit'
      ) as HTMLElement;
      triggerMouseEvent(editToolbar, 'click');
      let progress: HTMLInputElement = <HTMLInputElement>(
        ganttObj.element.querySelector('#' + ganttObj.element.id + 'Progress')
      );
      if (progress) {
        let progressInput: any = (
          document.getElementById(ganttObj.element.id + 'Progress') as any
        ).ej2_instances[0];
    ...
    it('ExpandAll handler function', () => {
      let expandallToolbar: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_expandall'
      ) as HTMLElement;
    ...
    it('Check Zoom Out action', () => {
      let zoomOut: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_zoomout'
      ) as HTMLElement;
      triggerMouseEvent(zoomOut, 'click');
      expect(
        ganttObj.timelineModule.customTimelineSettings.timelineUnitSize
      ).toBe(99);
      expect(ganttObj.currentZoomingLevel.level).toBe(10);
    });

    it('Check Zoom In action', () => {
      let zoomIn: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_zoomin'
      ) as HTMLElement;
      ...
      expect(
        ganttObj.timelineModule.customTimelineSettings.bottomTier.unit
      ).toBe('Day');
    ...
    it('Enable toolbar on row selection', () => {
      ...
      expect(
        ganttObj.element
          .querySelector('#' + ganttObj.element.id + '_Gantt_Toolbar')
          .getElementsByClassName('e-hidden').length
      ).toBe(2);
      ganttObj.selectionModule.clearSelection();
    });
    ...
    it('On celleditsaved handler function', () => {
      ...
      triggerMouseEvent(taskName, 'dblclick');
      let taskValue: HTMLInputElement = <HTMLInputElement>(
        ganttObj.element.querySelector(
          '#treeGrid' + ganttObj.element.id + '_gridcontrolTaskName'
        )
      );
      taskValue.value = 'Update TaskName';
      let updateToolbar: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_update'
      ) as HTMLElement;
    ...
    it('Search Icon handler function', () => {
      let searchbar: HTMLInputElement = <HTMLInputElement>(
        ganttObj.element.querySelector('#' + ganttObj.element.id + '_searchbar')
      );
      searchbar.value = '';
      let searchIcon: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_searchbutton'
      ) as HTMLElement;
    ...
    it('SearchKey and SearchValue', (done: Function) => {
      ...
      let searchButton: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_searchbutton'
      ) as HTMLElement;
    ...
    it('Selection Mode Cell', (done: Function) => {
      ...
      ganttObj.dataBound = () => {
        let deleteToolbar: HTMLElement = ganttObj.element.querySelector(
          '#' + ganttObj.element.id + '_delete'
        ) as HTMLElement;
        triggerMouseEvent(deleteToolbar, 'click');
        let okElement: HTMLElement = ganttObj.element
          .querySelector('#' + ganttObj.element.id + '_deleteConfirmDialog')
          .getElementsByClassName('e-primary')[0] as HTMLElement;
        triggerMouseEvent(okElement, 'click');
        done();
      };
    ...
    it('CR-EJ2-46731: Maintaining additional fields in segments on zooming action', (done: Function) => {
      ganttObj.actionComplete = (args: any): void => {
        if (args.requestType === 'AfterZoomIn') {
          expect(
            getValue('Segments[0].Custom', ganttObj.flatData[0].taskData)
          ).toBe('Test');
        }
      };
  ...
  describe('Zoomin action and zoomtofit action', () => {
    ...
    it('Check Zoom In action', () => {
      ...
      triggerMouseEvent(zoomIn, 'click');
      let zoomToFit: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_zoomtofit'
      ) as HTMLElement;
  ...
  describe('Gantt toolbar action', () => {
    ...
    it('Update handler function', () => {
      ...
      if (taskValue) {
        ...
        triggerMouseEvent(updateToolbar, 'click');
        expect(getValue('TaskName', ganttObj.flatData[4])).toBe(
          'Update TaskName'
        );
        ganttObj.selectionModule.clearSelection();
      }
    ...
    it('Indent for children', () => {
      ganttObj.actionComplete = (args: any): void => {
        if (args.requestType === 'indented') {
          expect(
            ganttObj.treeGrid.getRows()[5].getElementsByClassName('e-icons')
              .length
          ).toBe(4);
        }
      };
      ganttObj.selectRow(4);
      let indent: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_indent'
      ) as HTMLElement;
      ...
      ganttObj = createGantt(
        {
          ...
          load: (args?: any) => {
            let gantt: any = (
              document.getElementsByClassName('e-gantt')[0] as any
            ).ej2_instances[0];
  ...
  describe('Custom Zooming levels ', () => {
    ...
    it('Zoom out', () => {
      ...
      triggerMouseEvent(zoomIn, 'click');
      expect(
        document.getElementsByClassName('e-toolbar-item e-overlay')[0]
          .firstElementChild['ariaDisabled']
      ).toBe('true');
    });
  });
  describe('Perform outdent in immutable mode', () => {
    ...
    it('Outdent performed', () => {
      ...
      let outdent: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_outdent'
      ) as HTMLElement;
  ...
  describe('Zoom To Fit - ', () => {
    ...
    it('Unscheduled Task', () => {
      ...
      triggerMouseEvent(zoomToFit, 'click');
      let date2 = ganttObj.getFormatedDate(
        ganttObj.timelineModule.timelineStartDate,
        'M/d/yyyy'
      );
  ...
  describe('Add new record ', () => {
    ...
    it('Add record in UTC', () => {
      ...
      let EndDateInput: any = (
        document.getElementById(ganttObj.element.id + 'endDate') as any
      ).ej2_instances[0];
      ...
      let BaselineStartDateInput: any = (
        document.getElementById(
          ganttObj.element.id + 'baselineStartDate'
        ) as any
      ).ej2_instances[0];
      BaselineStartDateInput.value = new Date('6/28/2022');
      let BaselineEndDateInput: any = (
        document.getElementById(ganttObj.element.id + 'baselineEndDate') as any
      ).ej2_instances[0];
...
describe('Indent and outdent issue ', () => {
  ...
  it('Editing Task Name', () => {
    ...
    expect((ganttObj.currentViewData[2] as any).StartDate.toDateString()).toBe(
      'Wed Apr 17 2019'
    );
  });
});
...
describe('Gantt toolbar action', () => {
  ...
  it('Cancel handler function', () => {
    ...
    if (taskValue) {
      taskValue.value = 'Cancel TaskName';
      let cancelToolbar: HTMLElement = ganttObj.element.querySelector(
        '#' + ganttObj.element.id + '_cancel'
      ) as HTMLElement;
      triggerMouseEvent(cancelToolbar, 'mousedown');
      expect(getValue('TaskName', ganttObj.flatData[3])).toBe('Plan budget');
      ganttObj.selectionModule.clearSelection();
...
describe('Gantt with crtical path', () => {
  ...
  it('gantt with critical path', () => {
    let toolbar: HTMLElement = ganttObj.element.querySelector(
      '#' + ganttObj.element.id + '_critical-path'
    ) as HTMLElement;
...
describe('CR:890397-When adding a record via dialog, if the startDate is set to a Friday, the endDate and duration become empty', () => {
  ...
  it('enable toolbar items', () => {
    ...
    ganttObj['updateRowHeightInConnectorLine'](
      ganttObj.updatedConnectorLineCollection
    );
    ganttObj.getCriticalTasks();
    expect(
      ganttObj.isUnscheduledTask(ganttObj.currentViewData[1].ganttProperties)
    ).toBe(false);
    expect(
      ganttObj.treeGridModule['objectEqualityChecker'](
        ganttObj.flatData[0],
        ganttObj.flatData[0]
      )
    ).toBe(true);
...
describe('B900782-Taskbar render in incorrect postion after zoom to fit', () => {
  ...
  it('Performing zoom to fit action', (done: Function) => {
    ganttObj.actionComplete = function (args: any): void {
      if (args.requestType === 'AfterZoomToProject') {
        expect(ganttObj.flatData[0].ganttProperties.startDate.getHours()).toBe(
          0
        );
        done();
      }
