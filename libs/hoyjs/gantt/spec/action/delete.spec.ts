...
import { doesImplementInterface } from '@hoyui/grids';
describe('Gantt delete support', () => {
  describe('Gantt delete action', () => {
    ...
    it('Deleting through selected items', () => {
      ...
      expect(
        getValue('TaskID', ganttObj.flatData[ganttObj.flatData.length - 3])
      ).toBe(37);
    });
    it('Select a row and delete through toolbar', (done: Function) => {
      ...
      ganttObj.dataBound = () => {
        ...
        let deleteElement: HTMLElement = ganttObj.element.querySelector(
          '#' + ganttObj.element.id + '_delete'
        ) as HTMLElement;
        triggerMouseEvent(deleteElement, 'click');
        // Click cancel button
        let cancelElement: HTMLElement = ganttObj.element.querySelectorAll(
          '#' +
            ganttObj.element.id +
        ...
        triggerMouseEvent(deleteElement, 'click');
        // Click OK button
        let okElement: HTMLElement = ganttObj.element.querySelector(
          '#' +
            ganttObj.element.id +
            '_deleteConfirmDialog div.e-footer-content button'
        ) as HTMLElement;
        ...
        expect(
          getValue('TaskID', ganttObj.flatData[ganttObj.flatData.length - 1])
        ).toBe(39);
        done();
      };
      ganttObj.refresh();
    ...
    it('Select a cell in a row and delete through toolbar', () => {
      ...
      ganttObj.dataBind();
      let rowElement: NodeListOf<Element> = ganttObj.element.querySelectorAll(
        '#' + 'treeGrid' + ganttObj.element.id + '_gridcontrol_content_table tr'
