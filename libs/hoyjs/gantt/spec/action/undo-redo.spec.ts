...
import { ResizeArgs } from '@hoyui/grids';
...
describe('Gantt undoredo support', () => {
  ...
  describe('cell edit for undo redo module', () => {
    ...
    it('edit dependency column', () => {
      let dependency: HTMLElement = ganttObj.element.querySelector(
        '#treeGrid' +
          ganttObj.element.id +
          '_gridcontrol_content_table > tbody > tr:nth-child(4) > td:nth-child(7)'
      ) as HTMLElement;
  ...
  describe('Gantt column resize action', () => {
    ...
    it('Perform Column resize', () => {
      ...
      ganttObj.dataBind();
      let resizeColumn: HTMLElement = ganttObj.element
        .getElementsByClassName('e-columnheader')[0]
        .getElementsByClassName('e-rhandler e-rcursor')[0] as HTMLElement;
  ...
  describe('Gantt undo redo action for Show Hide', () => {
    ...
    it('Hideing column', () => {
      ganttObj.hideColumn(
        ganttObj.treeGrid.getColumnByField('TaskName').headerText
      );
    });
  ...
  describe('Triggering Findposition method with resources', () => {
    ...
    it('Triggering findPosition event', () => {
      ganttObj.editModule.dialogModule.ganttResources = [
        {
  ...
  describe('Cell edit in resourceview and undo action', () => {
    ...
    it('Undo Action Cell edit in resourceview', (done: Function) => {
      ganttObj.actionComplete = function (args: any): void {
        if (args.requestType === 'save') {
          expect(ganttObj.flatData[1]['TaskName']).toBe(
            'Identify site location'
          );
          done();
        }
  ...
  describe('Stack two undo action', () => {
    ...
    it('Checking for updated record', (done: Function) => {
      ganttObj.actionComplete = function (args: any): void {
        if (args.requestType === 'save') {
          expect(ganttObj.flatData[3]['TaskName']).toBe(
            'Updated by index value'
          );
          done();
        }
      };
...
describe('Cell edit for predecessor column', () => {
  ...
  it('add predecessor for parent record', () => {
    let dependency: HTMLElement = ganttObj.element.querySelector(
      '#treeGrid' +
        ganttObj.element.id +
        '_gridcontrol_content_table > tbody > tr:nth-child(1) > td:nth-child(3)'
    ) as HTMLElement;
    triggerMouseEvent(dependency, 'dblclick');
    let input: any = ganttObj.element.querySelector(
    ...
    expect(ganttObj.currentViewData[1].ganttProperties.predecessor.length).toBe(
      1
    );
  });
  it('Undo action for parent predecessor record', () => {
    ganttObj.undoRedoModule['getUndoCollection'][
      ganttObj.undoRedoModule['getUndoCollection'].length - 1
    ]['connectedRecords'] = [ganttObj.flatData[5]];
    ganttObj.undo();
    expect(ganttObj.getUndoActions().length).toBe(0);
  });
});
