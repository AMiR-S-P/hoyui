  ...
  it('Moving next time span', () => {
    ganttObj.actionComplete = function (args: any): void {
      if (args.requestType === 'nextTimeSpan') {
        let chartLeft =
          ganttObj.chartPane.querySelector('.e-content').scrollLeft;
        let currentCount: number = Math.round(
          chartLeft / ganttObj.element.offsetWidth
        );
        ganttObj.ganttChartModule.scrollObject.previousCount = currentCount;
      }
      if (args.requestType === 'scroll') {
        expect(
          ganttObj.chartPane.querySelector('.e-content').scrollLeft > 0
        ).toBe(true);
      }
    };
...
describe('Gantt chart-scroll support', () => {
  describe('Gantt chart-scroll action', () => {
    ...
    it('Chart scroll Function', () => {
      let chartscroll: HTMLElement = ganttObj.element.querySelector(
    ...
    it('Treegrid scroll Function', () => {
      let gridscroll: HTMLElement = ganttObj.treeGrid.element.querySelector(
        '.e-content'
      ) as HTMLElement;
