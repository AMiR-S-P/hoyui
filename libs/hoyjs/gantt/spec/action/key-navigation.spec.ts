...
import { Browser, getValue } from '@hoyui/base';
import { RowSelectingEventArgs } from '@hoyui/grids';
...
describe('Gantt Selection support', () => {
  describe('Gantt selection', () => {
      ...
      ganttObj = createGantt(
        {
          ...
          actionComplete: function (args: IKeyPressedEventArgs) {
            if (
              args.requestType == 'keyPressed' &&
              args.action == 'customKey'
            ) {
    ...
    it('end key testing', () => {
      ...
      ganttObj.keyboardModule.keyAction(args);
      expect(ganttObj.selectedRowIndex).toBe(
        ganttObj.currentViewData.length - 1
      );
    });
    it('end key last data selection testing', () => {
      let args: any = { action: 'end', preventDefault: preventDefault };
      ganttObj.keyboardModule.keyAction(args);
      expect(ganttObj.currentViewData[40] !== ganttObj.selectedRowIndex).toBe(
        true
      );
    });
    ...
    it('addRowDialog key testing', () => {
      ...
      ganttObj.keyboardModule.keyAction(args);
      let dialog: any = (<EJ2Instance>(
        document.getElementById(ganttObj.element.id + '_dialog')
      )).ej2_instances[0];
      if (dialog) {
        expect(getValue('isEdit', ganttObj.editModule.dialogModule)).toBe(
          false
        );
      }
      dialog.hide();
    ...
    it('Search key testing', (done) => {
      ...
      setTimeout(function () {
        ...
        searchbar.value = '';
        (ganttObj.toolbarModule as any).keyUpHandler(
          getKeyUpObj(13, searchbar)
        );
        done();
      }, 800);
    }, 1000);
    it('cancelRequest key testing', () => {
      ...
      ganttObj.keyboardModule.keyAction(args1);
      expect(ganttObj.currentViewData[3].ganttProperties.taskName).toBe(
        'Plan budget'
      );
    });
  ...
  describe('CR-issues', function () {
    ...
    it('Escape key on dependency tab', () => {
      let Add: HTMLElement = document.querySelector(
        '#' +
          ganttObj.element.id +
          'DependencyTabContainer_toolbarItems > div > div.e-toolbar-right > div:nth-child(1)'
      ) as HTMLElement;
      triggerMouseEvent(Add, 'click');
      let args: any = { action: 'escape', preventDefault: preventDefault };
      ganttObj.keyboardModule.keyAction(args);
      let element: HTMLElement = document.querySelector(
        '#' + ganttObj.element.id + '_dialog'
      ) as HTMLElement;
  ...
  it('Sorting Column', () => {
    ganttObj.actionComplete = function (args: any): void {
      if (args.requestType === 'sorting') {
        expect(
          parseInt(ganttObj.currentViewData[1].ganttProperties.taskId)
        ).toBe(2);
      }
    };
    let headerCell: HTMLElement = document.getElementsByClassName(
      'e-headercell'
    )[0] as HTMLElement;
    let args1 = {
      action: 'saveRequest',
...
describe('Focus task keyboard action', () => {
  ...
  it('Checked focused task', () => {
    ...
    ganttObj.keyboardModule.keyAction(args);
    expect(
      document.getElementsByClassName('e-selectionbackground').length > 0
    ).toBe(true);
  });
  afterAll(() => {
    if (ganttObj) {
...
describe('Up arrow key navigation', () => {
  ...
  it('Selected cell index', (done: Function) => {
    let targetCell: HTMLElement = document.getElementsByClassName(
      'e-rowcell'
    )[25] as HTMLElement;
...
describe('shift Tab action', function () {
  ...
  it('shift Tab action after editing cell', () => {
    ...
    expect(
      ganttObj.element
        .getElementsByClassName('e-editedbatchcell')[0]
...
describe('shift Tab action', function () {
  ...
  it('Tab action after focusing header cell', () => {
    ...
    expect(
      document.getElementsByClassName(
        'e-headercell e-ellipsistooltip e-focused e-focus'
      ).length
    ).toBe(1);
  });
  ...
  it('tab action for cells', () => {
    let headerCell: HTMLElement = (
      document.getElementsByClassName('e-row')[0] as any
    ).cells[1] as HTMLElement;
    triggerMouseEvent(headerCell, 'click');
    let args: any = {
      action: 'focusTask',
      preventDefault: preventDefault,
  ...
  it('tab action for second row to first row', () => {
    let headerCell: HTMLElement = (
      document.getElementsByClassName('e-row')[1] as any
    ).cells[1] as HTMLElement;
    triggerMouseEvent(headerCell, 'click');
    let args: any = {
      action: 'focusTask',
...
describe('shift Tab action', function () {
  ...
  it('tab for non editing cell', function () {
    let headerCell = (document.getElementsByClassName('e-row')[0] as any)
      .cells[5];
...
describe('shift Tab action', function () {
  ...
  it('shift tab action from taskbar to label', function () {
    let taskbar: any = document.getElementsByClassName(
