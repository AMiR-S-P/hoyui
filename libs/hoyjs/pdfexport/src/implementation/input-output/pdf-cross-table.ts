...
import {
  ...
  ObjectInfo,
} from './pdf-main-object-collection';
import { Dictionary } from './../collections/dictionary';
import {
  PdfReference,
  PdfReferenceHolder,
} from './../primitives/pdf-reference';
...
export class PdfCrossTable {
   ...
   * @private
   */
  private objects: Dictionary<number, RegisteredObject> = new Dictionary<
    number,
    RegisteredObject
  >();
  /**
  ...
  private saveTrailer(
    ...
    trailer.items.setValue(
      this.dictionaryProperties.size,
      new PdfNumber(this.objectCount + 1)
    );
  ...
  public getItem(offset: number, genNumber: number, isFree: boolean): string {
    ...
    let addGenNumberLength: number =
      5 - genNumber.toString().length <= 0
        ? 0
        : 5 - genNumber.toString().length;
    ...
    returnString =
      returnString + (isFree ? Operators.f : Operators.n) + Operators.newLine;
    return returnString;
  }
  /**
  ...
  private getSubReference(obj: IPdfPrimitive, bNew: boolean): PdfReference {
    ...
    if (
      this.items.count > 0 &&
      obj.objectCollectionIndex > 0 &&
      this.items.count > obj.objectCollectionIndex - 1
    ) {
      let tempObj: { reference: PdfReference; wasNew: boolean } =
        this.document.pdfObjects.getReference(obj, wasNew);
    ...
    if (reference == null) {
      ...
      if (wasNew) {
        ...
        let tempkey: number =
          this.document.pdfObjects.objectCollections[tempIndex].reference
            .objNumber;
        let tempvalue: ObjectInfo =
          this.document.pdfObjects.objectCollections[
            this.document.pdfObjects.count - 1
          ];
  ...
  public registerObject(
    ...
    if (typeof free === 'boolean') {
      ...
      this.objects.setValue(
        reference.objNumber,
        new RegisteredObject(<number>offset, reference, free)
      );
      ...
      this.objects.setValue(
        reference.objNumber,
        new RegisteredObject(offset, reference)
      );
      this.maxGenNumIndex = Math.max(this.maxGenNumIndex, reference.genNumber);
    }
  }
...
export class RegisteredObject {
  ...
  public constructor(
    offset: number | PdfCrossTable,
    reference: PdfReference,
    free?: boolean | PdfReference
  ) {
