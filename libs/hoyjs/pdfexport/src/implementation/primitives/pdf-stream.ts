...
export class PdfStream extends PdfDictionary {
  ...
  protected onFontProgramBeginSave(): void {
    (
      this.fontProgramBeginSave as SaveFontProgramEventHandler
    ).sender.fontProgramBeginSave();
  }
  /**
  ...
  public addFilter(filterName: string): void {
    let obj: IPdfPrimitive = this.items.getValue(
      this.dictionaryProperties.filter
    );
  ...
  public save(writer: IPdfWriter): void {
    ...
    this.items.setValue(
      this.dictionaryProperties.length,
      new PdfNumber(length)
    );
