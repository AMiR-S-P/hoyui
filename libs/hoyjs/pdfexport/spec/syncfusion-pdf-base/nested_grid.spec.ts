...
describe('Customer bug',()=>{
    ...
    it('832052', (done) => {
        ...
        row1.cells.getCell(1).value = '$Sum10000';

        var gridResult = pdfGrid.draw(page, new PointF(0, 30));
        expect(pdfDocument.pages.indexOf(gridResult.page)).toEqual(1);
        expect(gridResult.bounds.y).toEqual(0);
        expect(gridResult.bounds.height).toEqual(342.4519999999999);
        let pdfGrid2: PdfGrid = new PdfGrid();
        pdfGrid2.columns.add(4);
        pdfGrid2.headers.add(2);
        let headerRow2_1: PdfGridRow = pdfGrid2.headers.getHeader(0);
        ...
        let row2: PdfGridRow = pdfGrid2.rows.addRow();
        row2.cells.getCell(1).value = '$Sum20000';
        var gridResult2 = pdfGrid2.draw(
            gridResult.page,
            new PointF(0, gridResult.bounds.y + gridResult.bounds.height + 30)
        );
        expect(pdfDocument.pages.indexOf(gridResult2.page)).toEqual(3);
        expect(gridResult2.bounds.y).toEqual(0);
        expect(gridResult2.bounds.height).toEqual(228.46799999999996);
        var gridResult3 = pdfGrid2.draw(
            gridResult2.page,
            new PointF(0, gridResult2.bounds.y + gridResult2.bounds.height + 30)
        );
        expect(pdfDocument.pages.indexOf(gridResult3.page)).toEqual(5);
        expect(gridResult3.bounds.y).toEqual(0);
        expect(gridResult3.bounds.height).toEqual(114.48400000000002);
        var gridResult4 = pdfGrid2.draw(
            gridResult3.page,
            new PointF(0, gridResult3.bounds.y + gridResult3.bounds.height + 30)
        );
        expect(pdfDocument.pages.indexOf(gridResult4.page)).toEqual(6);
        expect(gridResult4.bounds.y).toEqual(0);
        expect(gridResult4.bounds.height).toEqual(456.4359999999998);
        var gridResult5 = pdfGrid2.draw(
            gridResult4.page,
            new PointF(0, gridResult4.bounds.y + gridResult4.bounds.height + 30)
        );
        expect(pdfDocument.pages.indexOf(gridResult5.page)).toEqual(8);
        expect(gridResult5.bounds.y).toEqual(0);
        expect(gridResult5.bounds.height).toEqual(313.9559999999999);
        var gridResult6 = pdfGrid2.draw(
            gridResult5.page,
            new PointF(0, gridResult5.bounds.y + gridResult5.bounds.height + 30)
        );
        expect(pdfDocument.pages.indexOf(gridResult6.page)).toEqual(10);
        expect(gridResult6.bounds.y).toEqual(0);
        expect(gridResult6.bounds.height).toEqual(199.97199999999998);
        var gridResult7 = pdfGrid2.draw(
            gridResult6.page,
            new PointF(0, gridResult6.bounds.y + gridResult6.bounds.height + 30)
        );
        expect(pdfDocument.pages.indexOf(gridResult7.page)).toEqual(12);
        expect(gridResult7.bounds.y).toEqual(0);
        expect(gridResult7.bounds.height).toEqual(85.98800000000001);
        //Save the document.
        //pdfDocument.save('832052_GridIssue.pdf');
        pdfDocument.save().then((xlBlob: { blobData: Blob }) => {
            if (Utils.isDownloadEnabled) {
                Utils.download(xlBlob.blobData, '832052_GridIssue.pdf');
            }
            let reader: FileReader = new FileReader();
            reader.readAsArrayBuffer(xlBlob.blobData);
            reader.onload = (): void => {
