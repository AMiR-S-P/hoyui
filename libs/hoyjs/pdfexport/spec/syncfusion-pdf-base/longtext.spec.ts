...
describe('PDFGrid_singlecell_extend', () => {
  it('PDFGrid_cell_extend', (done) => {
    ...
    for (let i = 0; i < pdfGridHeader.cells.count; i++) {
      if (i == 0) {
        pdfGridHeader.cells.getCell(i).style.borders.right.color = new PdfColor(
          255,
          255,
          255
        );
        ...
        pdfGridHeader.cells.getCell(i).style.borders.left.color = new PdfColor(
          255,
          255,
          255
        );
    ...
    for (let i = 0; i < pdfGrid.rows.count; i++) {
      if (i % 4 != 3) {
        ...
        for (let j = 0; j < row.cells.count; j++) {
          ...
          row.cells.getCell(j).style.borders.bottom.color = new PdfColor(
            255,
            255,
            255
          );
      ...
      if (i % 4 != 0) {
        ...
        for (let j = 0; j < row.cells.count; j++) {
          ...
          row.cells.getCell(j).style.borders.top.color = new PdfColor(
            255,
            255,
            255
          );
...
describe('PDFGrid_onepage', () => {
  it('PDFGrid', (done) => {
    ...
    for (var i = 0; i < 2; i++) {
      ...
      for (var j = 0; j < 3; j++) {
        childpdfGridRow1.cells.getCell(j).value =
          'Cell ' + j + ' ' + i + ' width';
      }
    }
...
describe('PDFGrid_longtext_innercell', () => {
  it('PDFGrid_longtext_innercell_columnspan', (done) => {
    ...
    childpdfGridRow1.cells.getCell(0).value =
      'the value is 0 0 points are equal then point 5';
    ...
    childpdfGridRow2.cells.getCell(1).value =
      'the Cell value is 4 4 nested grid ';
    ...
    pdfGridRow2.cells.getCell(0).value =
      'implementation of long test support in multiple nested grid implementation of long test support in multiple nested grid implementation of long test support in multiple nested grid implementation of long test support in multiple nested grid';
    pdfGridRow2.cells.getCell(1).value =
      'first row has 2 lines and second row has 3 lines';
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_longtext_columnspan(after).pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
...
describe('PDFGrid_parent_character_wrap', () => {
  it('PDFGrid_parent_character_wrap', (done) => {
    ...
    childpdfGridRow2.cells.getCell(1).value =
      'the Cell value is ' + 4 + ' ' + 4 + ' nested grid';
    childpdfGridRow2.cells.getCell(2).value =
      'the Cell value is ' + 5 + ' ' + 5;
    pdfGridRow1.cells.getCell(0).value =
      'abcdefghijklmnopqrstuvwxyz1234567890NestedgridImplementation';

    pdfGridRow1.cells.getCell(1).value = childPdfGrid;
    pdfGridRow1.cells.getCell(2).value = 'last';
    pdfGridRow2.cells.getCell(0).value =
      'implementation of long test support in multiple nested grid';
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_parent_character_wrap.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
...
describe('PDFGrid_inner_character_wrap', () => {
  it('PDFGrid_inner_character_wrap', (done) => {
    ...
    childpdfGridRow1.cells.getCell(1).value = '1 1';
    childpdfGridRow1.cells.getCell(2).value =
      '222523 4523532533532 52523235322352';
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_inner_character_wrap.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid_long_text', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_simple_long_text.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
...
describe('PDFGrid_single_nestedgrid_longtext', () => {
  it('single-long-row', (done) => {
    ...
    childpdfGridRow1.cells.getCell(5).value =
      'Child Grid Cell 6 implementation';
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_Singlerow_Longtext_FixedWidth.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_single_nestedgrid.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
...
describe('PDFGrid_longtext_innercell', () => {
  it('PDFGrid_Multiple_longtext_innercell', (done) => {
    ...
    childpdfGridRow1.cells.getCell(1).value = '1 1';
    childpdfGridRow1.cells.getCell(2).value =
      'implementation of long test support in multiple nested grid implementation of long test support in multiple nested grid implementation of long test support';
    childpdfGridRow2.cells.getCell(0).value = '3 3';
    childpdfGridRow2.cells.getCell(1).value =
      'the Cell value is 4 4 nested grid ';
    childpdfGridRow2.cells.getCell(2).value = 'the Cell value is 5 6';
    ...
    pdfGridRow1.cells.getCell(2).value = 'last';
    pdfGridRow2.cells.getCell(0).value =
      'implementation of long test support in multiple nested grid implementation of long test support in multiple nested grid';
    pdfGridRow2.cells.getCell(1).value =
      'first row has 2 lines and second row has 3 lines';
    pdfGridRow2.cells.getCell(2).value = childPdfGrid;
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_multiple_longtext_innercell.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
...
describe('PDFGrid_longtext_innercell', () => {
  it('PDFGrid_longtext_innercell_rowspan', (done) => {
    ...
    pdfGridRow1.cells.getCell(0).value = 'Nested Table';
    pdfGridRow1.cells.getCell(1).value =
      ' value are taken as column span height based on location file';
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_longtext_rowspan.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid_longtext_innercell_columnspan', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_longtext_columnspan.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid_rowspan', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_multilevel_rowspan.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_single_grid_font.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_multiple_nestedgrid_in_row.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_grid_width_change_outer.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
...
describe('PDFGrid_Multiple_inner_grid', () => {
  it('PDFGrid', (done) => {
    ...
    pdfGridRow1.cells.getCell(2).value =
      'This is the example sample for pdf nested grid support in EJ2 pdf -library using typescript';
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_Multiple_inner_grid.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_nested_grid_greater.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
...
describe('PDFGrid_nested_grid_smaller', () => {
  it('PDFGrid', (done) => {
    ...
    pdfGridRow1.cells.getCell(1).value = childPdfGrid;
    pdfGridRow2.cells.getCell(0).value =
      'This is the example sample for pdf nested grid support in EJ2 pdf -library using typescript, Branch name-- Ej2_10212 implement nested grid Implement the basic algorithm';
    pdfGridRow2.cells.getCell(2).value = childPdfGrid;
    // drawing a grid
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_nested_grid_smaller.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_nested_grid_Multiplerow.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_Default_height_outer.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_Multiple_ParentGrid.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_ParentGrid_points.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_column_span_grid.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid_innerGrid_columnspan', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_innerGrid_columnspan.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
  ...
  it('PDFGrid_innerGrid_rowspan', (done) => {
    ...
    document.save().then((xlBlob: { blobData: Blob }) => {
      if (Utils.isDownloadEnabled) {
        Utils.download(
          xlBlob.blobData,
          'PDFGrid_EJ2-11259_innerGrid_rowspan.pdf'
        );
      }
      let reader: FileReader = new FileReader();
      reader.readAsArrayBuffer(xlBlob.blobData);
