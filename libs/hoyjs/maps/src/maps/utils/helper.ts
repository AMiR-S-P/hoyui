...
import {
  ...
  isNullOrUndefined,
  remove,
  compile as templateComplier,
  merge,
  SanitizeHtmlHelper,
} from '@hoyui/base';
import { AnimationOptions, Animation, animationMode } from '@hoyui/base';
import { SvgRenderer } from '@hoyui/svg-base';
import {
  Maps,
  FontModel,
  BorderModel,
...
export function calculateSize(maps: Maps): Size {
  maps.element.style.height = !isNullOrUndefined(maps.height)
    ? maps.height
    : 'auto';
  maps.element.style.width = !isNullOrUndefined(maps.width)
    ? maps.width
    : 'auto';
  ...
  const containerElementWidth: number = stringToNumber(
    maps.element.style.width,
    containerWidth
  );
  const containerElementHeight: number = stringToNumber(
    maps.element.style.height,
    containerHeight
  );
  ...
  if (
    ...
    availableSize = new Size(
      stringToNumber(maps.width, containerWidth) ||
        containerWidth ||
        containerElementWidth ||
        600,
      stringToNumber(maps.height, containerHeight) ||
        containerHeight ||
        containerElementHeight ||
        (maps.isDevice ? Math.min(window.innerWidth, window.innerHeight) : 450)
    );
  }
...
export function convertGeoToPoint(
  ...
  const mapSize: Size = new Size(
    mapModel.mapAreaRect.width,
    mapModel.mapAreaRect.height
  );
  let x: number;
  let y: number;
  let value: Point;
  ...
  const type: ProjectionType = !isNullOrUndefined(mapModel.projectionType)
    ? mapModel.projectionType
    : 'Mercator';
  const size: number = mapModel.isTileMap
    ? Math.pow(2, 1) * 256
    : isNullOrUndefined(factor)
    ? Math.min(mapSize.width, mapSize.height)
    : Math.min(mapSize.width, mapSize.height) * factor;
  if (layer.geometryType === 'Normal') {
    x = isNullOrUndefined(factor)
      ? longitude
      : Math.abs((longitude - longitudeMinMax.min) * factor);
    y = isNullOrUndefined(factor)
      ? latitude
      : Math.abs((latitudeMinMax.max - latitude) * factor);
  } else if (layer.geometryType === 'Geographic') {
    switch (type) {
      case 'Mercator': {
        const pixelOrigin: Point = new Point(size / 2, size / 2);
        x = pixelOrigin.x + longitude * (size / 360);
        const sinY: number = calculateBound(
        ...
        y =
          pixelOrigin.y +
          0.5 * Math.log((1 + sinY) / (1 - sinY)) * -(size / (2 * Math.PI));
        break;
      }
      case 'Winkel3':
      ...
      case 'Eckert3':
        ...
        lng =
          ...
          lngRadian *
          (1 +
            Math.sqrt(1 - (4 * latRadian * latRadian) / (Math.PI * Math.PI)));
      ...
      case 'Eckert6': {
        ...
        for (let i: number = 0; i < 10 && Math.abs(delta) > epsilon; i++) {
          delta =
            (latRadian + Math.sin(latRadian) - temp) /
            (1 + Math.cos(latRadian));
    ...
    x =
      type === 'Mercator'
        ? x
        : roundTo(xToCoordinate(mapModel, radiansToDegrees(lng)), 3);
    y =
      type === 'Mercator'
        ? y
        : -roundTo(yToCoordinate(mapModel, radiansToDegrees(lat)), 3);
  }
 ...
 * @private
 */
export function calculatePolygonPath(
  maps: Maps,
  factor: number,
  ...
  if (!isNullOrUndefined(markerData) && markerData.length > 1) {
    Array.prototype.forEach.call(
      markerData,
      (data: Coordinate, dataIndex: number) => {
        const lat: number = data.latitude;
        const lng: number = data.longitude;
        const location: Point = maps.isTileMap
          ? convertTileLatLongToPoint(
              new MapLocation(lng, lat),
              factor,
              maps.tileTranslatePoint,
              true
            )
...
export function convertTileLatLongToPoint(
  center: MapLocation,
  zoomLevel: number,
  tileTranslatePoint: MapLocation,
  isMapCoordinates: boolean
): MapLocation {
...
export function xToCoordinate(mapObject: Maps, val: number): number {
  ...
  const totalSize: number = isNullOrUndefined(mapObject.baseSize)
    ? mapObject.mapAreaRect.width
    : mapObject.mapAreaRect.width +
      Math.abs(mapObject.baseSize.width - mapObject.mapAreaRect.width) / 2;
  return (
    Math.round(
      ((totalSize * (val - longitudeMinMax.min)) /
...
export function yToCoordinate(mapObject: Maps, val: number): number {
  ...
  return (
    Math.round(
      ((mapObject.mapAreaRect.height * (val - latitudeMinMax.min)) /
        (latitudeMinMax.max - latitudeMinMax.min)) *
        100
    ) / 100
  );
}
...
export function triggerDownload(
  ...
  createElement('a', {
    ...
    new MouseEvent(isDownload ? 'click' : 'move', {
      view: window,
      bubbles: false,
      cancelable: true,
    })
...
export function measureText(text: string, font: FontModel): Size {
  ...
  measureObject.style.cssText =
    'position: absolute; font-size: ' +
    (typeof font.size === 'number' ? font.size + 'px' : font.size) +
    '; font-weight: ' +
    font.fontWeight +
    '; font-style: ' +
    font.fontStyle +
...
export class PatternOptions {
  ...
  constructor(
    ...
    patternContentUnits: patternUnits = 'userSpaceOnUse',
...
export function renderTextElement(
  ...
  let height: number;
  const htmlObject: HTMLElement = <HTMLElement>(
    renderer.createText(renderOptions, text)
  );
  ...
  if (typeof option.text !== 'string' && option.text.length > 1) {
    for (let i: number = 1, len: number = option.text.length; i < len; i++) {
      ...
      tspanElement = renderer.createTSpan(
        ...
        isMinus
          ? option.text[option.text.length - (i + 1)]
          : option.text[i as number]
      );
  ...
  if (!(mapObj as any).isReact || templateType !== 'function') {
    ...
    for (let i: number = 0; i < properties.length; i++) {
      if (typeof data[properties[i as number]] === 'object') {
        ...
        (<String>properties[i as number]).toLowerCase() !== 'latitude' &&
        (<string>properties[i as number]).toLowerCase() !== 'longitude'
      ) {
        templateHtml = templateHtml.replace(
          new regExp('{{:' + <string>properties[i as number] + '}}', 'g'),
          data[properties[i as number].toString()]
        );
      }
    }
...
export function convertStringToValue(
  ...
  for (let i: number = 0; i < templateValue.length; i++) {
    if (
      (templateValue[i as number].indexOf('}}') > -1 &&
        templateValue[i as number].indexOf('.') > -1) ||
      (templateValue[i as number].indexOf('}') > -1 &&
        templateValue[i as number].search('.') > -1)
    ) {
      const split: string[] =
        stringTemplate === ''
          ? templateValue[i as number].split('}')
          : templateValue[i as number].split('}}');
      for (let j: number = 0; j < split.length; j++) {
        if (split[j as number].indexOf('.') > -1) {
          const templateSplitValue: string = getValueFromObject(
            data,
            split[j as number]
          ).toString();
          ...
          templateHtml =
            stringTemplate === ''
              ? templateHtml
                  .split('${' + split[j as number] + '}')
                  .join(formatValue(templateSplitValue, maps))
              : templateHtml.replace(
                  new regExp('{{:' + split[j as number] + '}}', 'g'),
                  templateSplitValue
                );
        }
      }
...
export function convertElementFromLabel(
  ...
  const labelEle: Element = isNullOrUndefined(element.childElementCount)
    ? element[0]
    : element;
...
export function drawSymbols(
  ...
  if (shape === 'Circle') {
    ...
    const circleOptions: CircleOption = new CircleOption(
      markerID,
      fill,
      border,
      opacity,
    ...
    y =
      location.y -
      (markerID.indexOf('cluster') > -1 ? size.height / 2 : size.height);
    merge(pathOptions, {
      href: imageUrl,
      height: size.height,
      width: size.width,
...
export function getValueFromObject(data: object, value: string): any {
  if (!isNullOrUndefined(data) && !isNullOrUndefined(value)) {
    ...
    if (splits.length === 1) {
      ...
      for (
        let i: number = 0;
        i < splits.length && !isNullOrUndefined(data);
        i++
      ) {
 ...
 * @private
 */
export function markerColorChoose(
  eventArgs: IMarkerRenderingEventArgs,
  data: object
): IMarkerRenderingEventArgs {
  const color: string = !isNullOrUndefined(eventArgs.colorValuePath)
    ? eventArgs.colorValuePath.indexOf('.') > -1
      ? getValueFromObject(data, eventArgs.colorValuePath).toString()
      : data[eventArgs.colorValuePath]
    : data[eventArgs.colorValuePath];
  eventArgs.fill =
    !isNullOrUndefined(eventArgs.colorValuePath) && !isNullOrUndefined(color)
      ? eventArgs.colorValuePath.indexOf('.') > -1
        ? getValueFromObject(data, eventArgs.colorValuePath).toString()
        : data[eventArgs.colorValuePath]
      : eventArgs.fill;
 ...
 * @private
 */
export function markerShapeChoose(
  eventArgs: IMarkerRenderingEventArgs,
  data: object
): IMarkerRenderingEventArgs {
  if (
    !isNullOrUndefined(eventArgs.shapeValuePath) &&
    !isNullOrUndefined(data[eventArgs.shapeValuePath])
  ) {
...
function getValue(path: string, data: any): any {
  return path.indexOf('.') > -1
    ? getValueFromObject(data, path).toString()
    : data[path as string];
}
/**
 *
...
function updateShape(eventArgs: any, data: any): void {
  if (!isNullOrUndefined(eventArgs.shapeValuePath)) {
    ...
    eventArgs.shape =
      !isNullOrUndefined(shape) && shape.toString() !== ''
        ? shape
        : eventArgs.shape;
  }
}
...
function updateImageUrl(eventArgs: any, data: any): void {
  if (!isNullOrUndefined(eventArgs.imageUrlValuePath)) {
    ...
    eventArgs.imageUrl = !isNullOrUndefined(imageUrl)
      ? imageUrl
      : eventArgs.imageUrl;
  }
}
...
export function clusterTemplate(
  currentLayer: LayerSettings,
  markerTemplate: HTMLElement | Element,
  maps: Maps,
  layerIndex: number,
  markerCollection: Element,
  ...
  const clusters: MarkerClusterSettingsModel =
    currentLayer.markerClusterSettings;
  const clusterGroup: Element = maps.renderer.createGroup({
  ...
  const currentZoomFactor: number = !maps.isTileMap
    ? maps.mapScaleValue
    : maps.tileZoomLevel;
  ...
  maps.trigger(
    'markerClusterRendering',
    eventArg,
    (clusterargs: IMarkerClusterRenderingEventArgs) => {
      Array.prototype.forEach.call(
        markerTemplate.childNodes,
        (markerElement: Element, o: number) => {
          indexCollection = [];
          if (markerElement['style']['visibility'] !== 'hidden') {
            ...
            if (!isNullOrUndefined(bounds1)) {
              const list: number[] =
                (maps.markerModule.zoomedMarkerCluster.length > 0 &&
                  maps.markerModule.zoomedMarkerCluster[layerIndex as number] &&
                  maps.markerModule.zoomedMarkerCluster[layerIndex as number][
                    o as number
                  ] &&
                  maps.markerModule.zoomedMarkerCluster[layerIndex as number][
                ...
                (maps.markerModule.initialMarkerCluster.length > 0 &&
                  maps.markerModule.initialMarkerCluster[
                    layerIndex as number
                  ] &&
                  maps.markerModule.initialMarkerCluster[layerIndex as number][
                    o as number
                  ] &&
                  maps.markerModule.initialMarkerCluster[layerIndex as number][
                    o as number
                  ].length > 0)
                  ? maps.previousScale < currentZoomFactor
                    ? maps.markerModule.zoomedMarkerCluster[
                        layerIndex as number
                      ][o as number]
                    : maps.markerModule.initialMarkerCluster[
                        layerIndex as number
              ...
              if (!isNullOrUndefined(list) && list.length !== 0) {
                Array.prototype.forEach.call(list, (currentIndex: number) => {
                  if (o !== currentIndex) {
                    const otherMarkerElement: Element = document.getElementById(
                      maps.element.id +
                        '_LayerIndex_' +
                        layerIndex +
                        '_MarkerIndex_' +
                ...
                Array.prototype.forEach.call(
                  markerTemplate.childNodes,
                  (otherMarkerElement: Element, p: number) => {
                    if (
                      p >= o + 1 &&
                      otherMarkerElement['style']['visibility'] !== 'hidden'
                    ) {
              ...
              if (colloideBounds.length > 0) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                indexCollection = indexCollection.filter(
                ...
                tempY =
                  tempY -
                  (maps.availableSize.height <= containerRect['height']
                    ? containerRect['top']
                    : containerRect['bottom'] - containerRect['top']);
                const dataIndex: number = parseInt(
                  markerElement['id'].split('_dataIndex_')[1].split('_')[0],
                  10
                );
                const markerIndex: number = parseInt(
                  markerElement['id'].split('_MarkerIndex_')[1].split('_')[0],
                  10
                );
                const markerSetting: MarkerSettingsModel =
                  currentLayer.markerSettings[markerIndex as number];
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const markerData: any =
                  markerSetting.dataSource[dataIndex as number];
                let location: Point;
                const longitude: number = !isNullOrUndefined(
                  markerSetting.longitudeValuePath
                )
                  ? Number(
                      getValueFromObject(
                        markerData,
                  ...
                  : !isNullOrUndefined(markerData['longitude'])
                  ? parseFloat(markerData['longitude'])
                  : !isNullOrUndefined(markerData['Longitude'])
                  ? parseFloat(markerData['Longitude'])
                  : 0;
                const latitude: number = !isNullOrUndefined(
                  markerSetting.latitudeValuePath
                )
                  ? Number(
                      getValueFromObject(
                        markerData,
                  ...
                  : !isNullOrUndefined(markerData['latitude'])
                  ? parseFloat(markerData['latitude'])
                  : !isNullOrUndefined(markerData['Latitude'])
                  ? parseFloat(markerData['Latitude'])
                  : 0;
                if (!maps.isTileMap) {
                  location = convertGeoToPoint(
                    latitude,
                    longitude,
                    factor,
                    currentLayer,
                  ...
                  location = convertTileLatLongToPoint(
                ...
                if (eventArg.cancel) {
                  ...
                  shapeCustom['borderOpacity'] = isNullOrUndefined(
                    clusters.border.opacity
                  )
                    ? clusters.opacity
                    : clusters.border.opacity;
                } else {
                  ...
                  shapeCustom['borderOpacity'] = isNullOrUndefined(
                    eventArg.border.opacity
                  )
                    ? clusters.opacity
                    : eventArg.border.opacity;
                }
                tempX = maps.isTileMap
                  ? tempX
                  : markerTemplate.id.indexOf('_Markers_Group') > -1
                  ? tempX
                  : tempX + postionY - eventArg.width / 2;
                tempY = maps.isTileMap
                  ? tempY
                  : markerTemplate.id.indexOf('_Markers_Group') > -1
                  ? tempY
                  : tempY - eventArg.height / 2;
                ...
                if (eventArg.shape === 'Balloon') {
                  (ele.children[0] as HTMLElement).textContent =
                    indexCollection.toString();
                } else {
                ...
                options = new TextOption(
                  ...
                  postionY,
                  'middle',
                  (colloideBounds.length + 1).toString(),
                  '',
                  ''
                );
                textElement = renderTextElement(
                  options,
                  style,
                  style.color,
                  markerCollection
      ...
      if (clusters.allowDeepClustering) {
        Array.prototype.forEach.call(
          clusterGroup.childNodes,
          (clusterElement: Element, o: number) => {
            if (clusterElement['style']['visibility'] !== 'hidden') {
              tempElement = clusterElement as Element;
              bounds1 = tempElement.getBoundingClientRect() as DOMRect;
              if (
                !isNullOrUndefined(bounds1) &&
                !(tempElement.id.indexOf('_datalabel_') > -1)
              ) {
                for (
                  let p: number = o + 1;
                  p < clusterGroup.childElementCount;
                  p++
                ) {
                  if (
                    clusterGroup.childNodes[p as number]['style'][
                      'visibility'
                    ] !== 'hidden'
                  ) {
                    tempElement1 = clusterGroup.childNodes[
                      p as number
                    ] as Element;
                    bounds2 = tempElement1.getBoundingClientRect() as DOMRect;
                    if (
                      !isNullOrUndefined(bounds2) &&
                      !(tempElement1.id.indexOf('_datalabel_') > -1)
                    ) {
                      if (
                        !(
                        ...
                        clusterColloideBounds.push(
                          clusterGroup.childNodes[p - 1] as Element
                        );
                        clusterGroup.childNodes[p as number]['style'][
                          'visibility'
                        ] = 'hidden';
                        clusterGroup.childNodes[p - 1]['style']['visibility'] =
                          'hidden';
                ...
                if (clusterColloideBounds.length > 0) {
                  ...
                  for (
                    let i: number = 0;
                    i < clusterColloideBounds.length;
                    i++
                  ) {
                    if (tempElement.tagName === 'g') {
                      tempElement.childNodes[0].textContent =
                        tempElement.childNodes[0].textContent +
                        ',' +
                    ...
                    clusterGroup.childNodes[o - 1].textContent = (
                      +clusterGroup.childNodes[o - 1].textContent +
                      +clusterColloideBounds[i + 1].textContent
                    ).toString();
                    i++;
                  }
                }
      ...
      while (0 < clusterGroup.childNodes.length) {
        markerCollection.insertBefore(
          clusterGroup.childNodes[0],
          markerCollection.firstChild
        );
      }
      if (!check) {
        getElementByID(maps.element.id + '_Secondary_Element').appendChild(
          markerCollection
        );
      }
      const element: HTMLElement = document.getElementById(
        maps.element.id + '_LayerIndex_' + layerIndex + '_Polygon_Group'
      );
      const polygonElement: HTMLElement = document.getElementById(
        maps.element.id + '_LayerIndex_' + layerIndex + '_Polygons_Group'
      );
      ...
      const markerCluster: HTMLElement = document.getElementById(
        maps.element.id + '_LayerIndex_' + layerIndex + '_markerCluster'
      );
      if (!isNullOrUndefined(markerCluster)) {
        markerCluster.remove();
      }
      if (zoomCheck) {
        const layerGroupElement: HTMLElement = document.getElementById(
          maps.element.id + '_Layer_Collections'
        );
        const element: HTMLElement = document.getElementById(
          maps.element.id + '_LayerIndex_' + (layerIndex + 1)
        );
        if (
          !isNullOrUndefined(layerGroupElement) &&
 ...
 * @private
 */
export function markerClusterListHandler(
  maps: Maps,
  ...
  if (currentZoomFactor === 1) {
    const initialMarkerClusterList: number[] = isNullOrUndefined(
      maps.markerModule.initialMarkerCluster[layerIndex as number][
        index as number
      ]
    )
      ? []
      : indexCollection.length > 1
      ? indexCollection
      : [];
    maps.markerModule.initialMarkerCluster[layerIndex as number][
      index as number
    ] = initialMarkerClusterList;
    const zoomedMarkerClusterList: number[] = isNullOrUndefined(
      maps.markerModule.zoomedMarkerCluster[layerIndex as number][
        index as number
      ]
    )
    ...
    maps.markerModule.zoomedMarkerCluster[layerIndex as number][
      index as number
    ] = zoomedMarkerClusterList;
  } else if (currentZoomFactor > 1) {
    maps.markerModule.zoomedMarkerCluster[layerIndex as number][
      index as number
    ] = indexCollection.length > 1 ? indexCollection : [];
  }
}
 ...
 * @private
 */
export function markerBoundsComparer(
  tempElement: Element,
  markerBounds: ClientRect,
  colloideBounds: ClientRect[],
  indexCollection: number[],
  p: number
): void {
  // eslint-disable-next-line @typescript-eslint/tslint/config
  ...
  if (!isNullOrUndefined(currentMarkerBound)) {
    if (
      !(
        markerBounds.left > currentMarkerBound.right ||
        markerBounds.right < currentMarkerBound.left ||
        markerBounds.top > currentMarkerBound.bottom ||
        markerBounds.bottom < currentMarkerBound.top
      )
    ) {
 ...
 * @private
 */
export function mergeSeparateCluster(
  ...
  const marker: MarkerSettingsModel =
    maps.layers[layerIndex as number].markerSettings[markerIndex as number];
  const clusterId: string =
    markerId + '_dataIndex_' + dataIndex + '_cluster_' + clusterIndex;
  const clusterEle: Element =
    maps.layers[layerIndex as number].markerClusterSettings.shape === 'Balloon'
      ? getElement(clusterId + '_Group')
      : getElement(clusterId);
  const clusterEleLabel: Element = getElement(
    clusterId + '_datalabel_' + clusterIndex
  );
  ...
  for (let i: number = 0; i < markerDataLength; i++) {
    markerEle =
      marker.shape === 'Balloon' && isNullOrUndefined(marker.template)
        ? getElement(
            markerId +
              '_dataIndex_' +
 ...
 * @private
 */
export function clusterSeparate(
  sameMarkerData: MarkerClusterData[],
  maps: Maps,
  markerElement: Element | HTMLElement,
  isDom?: boolean
): void {
  const layerIndex: number = sameMarkerData[0].layerIndex;
  const markerIndex: number = sameMarkerData[0].markerIndex;
  ...
  const dataIndex: number = sameMarkerData[0].dataIndex;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const getElementFunction: any = isDom
    ? getElement
    : markerElement.querySelector.bind(markerElement);
  ...
  const clusterEle: Element =
    maps.layers[layerIndex as number].markerClusterSettings.shape === 'Balloon'
      ? getElementFunction(getQueryConnect + '' + clusterId + '_Group')
      : getElementFunction(getQueryConnect + '' + clusterId);
  const clusterEleLabel: Element = getElementFunction(
    getQueryConnect + '' + clusterId + '_datalabel_' + clusterIndex
  );
  ...
  let markerEle: Element =
    marker.shape === 'Balloon' && isNullOrUndefined(marker.template)
      ? getElementFunction(
          getQueryConnect + '' + markerId + '_dataIndex_' + dataIndex + '_Group'
        )
      : getElementFunction(
          getQueryConnect + '' + markerId + '_dataIndex_' + dataIndex
        );
  const height: number =
    markerEle.parentElement.id.indexOf('Template_Group') > -1
      ? markerEle.getBoundingClientRect().height
      : marker.height;
  const width: number =
    markerEle.parentElement.id.indexOf('Template_Group') > -1
      ? markerEle.getBoundingClientRect().width
      : marker.width;
  const centerX: number = +clusterEle
    .getAttribute('transform')
    .split('translate(')[1]
    .trim()
    .split(' ')[0];
  const centerY: number = +clusterEle
    .getAttribute('transform')
    .split('translate(')[1]
    .trim()
    .split(' ')[1]
  ...
  percent =
    markerDataLength < numberOfMarker ? 100 / markerDataLength : percent;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  for (let i: number = 0; i < markerDataLength; i++) {
    ...
    const x1: number =
      centerX + radius * Math.sin((Math.PI * 2 * newAngle) / 360);
    const y1: number =
      centerY + radius * Math.cos((Math.PI * 2 * newAngle) / 360);
    path += start + 'L ' + x1 + ' ' + y1 + ' ';
    markerEle =
      marker.shape === 'Balloon' && isNullOrUndefined(marker.template)
        ? getElementFunction(
    ...
    if (markerEle.parentElement.id.indexOf('Template_Group') > -1) {
      ...
      markerEle['style']['left'] = maps.isTileMap
        ? x1 - width / 2 + 'px'
        : x1 - width / 2 - 10 + 'px';
      markerEle['style']['top'] = maps.isTileMap
        ? y1 - height / 2 + 'px'
        : y1 - height / 2 - 10 + 'px';
  ...
  const connectorLine: ConnectorLineSettingsModel =
    maps.layers[layerIndex as number].markerClusterSettings
  ...
  markerElement = isDom
    ? getElementFunction(maps.element.id + '_Markers_Group')
    : markerElement;
  const groupEle: Element = maps.renderer.createGroup({
    id:
      maps.element.id +
      '_LayerIndex_' +
      layerIndex +
  ...
  if (marker.shape === 'Balloon') {
    markerElement.insertBefore(
      groupEle,
      markerElement.querySelector('#' + markerId + '_dataIndex_0_Group')
    );
  } else {
    markerElement.insertBefore(
      groupEle,
      markerElement.querySelector('#' + markerId + '_dataIndex_0')
    );
  }
}
...
export function marker(
  ...
  const shapeCustom: any = {
    ...
    borderOpacity: isNullOrUndefined(eventArgs.border.opacity)
      ? markerSettings.opacity
  ...
  const x: number =
    (maps.isTileMap ? location.x : (location.x + transPoint.x) * scale) +
    (!isNullOrUndefined(offset.x) ? offset.x : 0);
  const y: number =
    (maps.isTileMap ? location.y : (location.y + transPoint.y) * scale) +
    (!isNullOrUndefined(offset.y) ? offset.y : 0);
  ele.setAttribute('transform', 'translate( ' + x + ' ' + y + ' )');
  maintainSelection(
    maps.selectedMarkerElementId,
    maps.markerSelectionClass,
    ele,
    'MarkerselectionMapStyle'
  );
  if (
    maps.legendSettings.toggleLegendSettings.enable &&
    maps.legendSettings.type === 'Markers'
  ) {
    const layerIndex: number = parseInt(ele.id.split('_LayerIndex_')[1], 10);
    maintainToggleSelection(
      maps.toggledElementId,
      ele.tagName === 'g' ? ele.children[0] : ele,
      maps.legendSettings.toggleLegendSettings.applyShapeSettings
        ? maps.layers[layerIndex as number].shapeSettings
        : maps.legendSettings.toggleLegendSettings
    );
  }
  ...
  if (markerSettings.animationDuration > 0 || animationMode === 'Enable') {
    elementAnimate(
      ele,
      markerSettings.animationDelay,
      markerSettings.animationDuration,
      markerPoint,
      maps,
      element
    );
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function markerTemplate(
  eventArgs: IMarkerRenderingEventArgs,
  templateFn: any,
  markerID: string,
  data: any,
  markerIndex: number,
  markerTemplate: HTMLElement,
  location: Point,
  transPoint: Point,
  scale: number,
  offset: Point,
  ...
  if (
    ...
    templateFn(
      ...
      maps,
      eventArgs.template,
      maps.element.id + '_MarkerTemplate' + markerIndex,
      false
    ).length
  ) {
    const templateElement: HTMLCollection = templateFn(
      data,
      maps,
      eventArgs.template,
      maps.element.id + '_MarkerTemplate' + markerIndex,
    ...
    const markerElement: HTMLElement = <HTMLElement>(
      convertElement(
        templateElement,
        markerID,
        data,
        markerIndex,
        maps,
        typeof eventArgs.template
      )
    );
    for (let i: number = 0; i < markerElement.children.length; i++) {
      (<HTMLElement>markerElement.children[i as number]).style.pointerEvents =
        'auto';
    }
 ...
 * @private
 */
export function maintainSelection(
  elementId: string[],
    ...
    for (let index: number = 0; index < elementId.length; index++) {
      if (element.getAttribute('id') === elementId[index as number]) {
        if (index === 0 || element.tagName === 'g') {
          if (
            !isNullOrUndefined(elementClass) &&
            !isNullOrUndefined(elementClass.id)
          ) {
            document.body.appendChild(elementClass);
          }
          if (
            element.id.indexOf('_MarkerIndex_') > -1 &&
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function maintainToggleSelection(
  toggledElements: string[],
  element: Element,
  styleProperty: any
): void {
    ...
    for (let j: number = 0; j < toggledElements.length; j++) {
      if (toggledElements[j as number] === element.id) {
        ...
        element.setAttribute(
          'stroke-opacity',
          (isNullOrUndefined(styleProperty.border.opacity)
            ? styleProperty.opacity
            : styleProperty.border.opacity
          ).toString()
        );
        element.setAttribute(
          'stroke-width',
          (isNullOrUndefined(styleProperty.border.width)
            ? 0
 ...
 * @private
 */
export function maintainStyleClass(
  id: string,
  idClass: string,
  fill: string,
  opacity: string,
  ...
  if (!getElement(id)) {
    ...
    (styleClass as HTMLElement).innerText =
      '.' +
      idClass +
      '{fill:' +
      fill +
...
export function calculateShapes(
  ...
  if (
    ...
    options['stroke'] =
      options['stroke'] === 'transparent' ? options['fill'] : options['stroke'];
  }
...
export function drawBalloon(
  ...
  location.x -= width / 2;
  location.y -=
    options.id.indexOf('cluster') > -1
      ? height / 2
      : options.id.indexOf('Legend') > -1
      ? height / 1.25
      : height;
  options.d =
    'M15,0C8.8,0,3.8,5,3.8,11.2C3.8,17.5,9.4,24.4,15,30c5.6-5.6,11.2-12.5,11.2-18.8C26.2,5,21.2,0,15,0z M15,16' +
...
export function checkShapeDataFields(
  ...
  if (!isNullOrUndefined(properties) && !isNullOrUndefined(dataSource)) {
    for (let i: number = 0; i < dataSource.length; i++) {
      const shapeDataPath: string =
        dataPath.indexOf('.') > -1
          ? getValueFromObject(dataSource[i as number], dataPath)
          : dataSource[i as number][dataPath as string];
      const shapePath: string = checkPropertyPath(
        shapeDataPath,
        propertyPath,
        properties
      );
      const shapeDataPathValue: string =
        !isNullOrUndefined(shapeDataPath) &&
        isNaN(properties[shapePath as string])
          ? typeof shapeDataPath === 'string'
            ? shapeDataPath.toLowerCase()
            : shapeDataPath
          : shapeDataPath;
      const propertiesShapePathValue: string =
        !isNullOrUndefined(properties[shapePath as string]) &&
        isNaN(properties[shapePath as string])
          ? properties[shapePath as string].toLowerCase()
          : properties[shapePath as string];
 ...
 * @returns {string} - Returns the string value
 */
export function checkPropertyPath(
  shapeData: string,
  ...
  if (!isNullOrUndefined(shapeData) && !isNullOrUndefined(shape)) {
    if (!isNullOrUndefined(shapePropertyPath)) {
      const properties: string[] = (
        Object.prototype.toString.call(shapePropertyPath) === '[object Array]'
          ? shapePropertyPath
          : [shapePropertyPath]
      ) as string[];
      for (let i: number = 0; i < properties.length; i++) {
        const shapeDataValue: string =
          !isNullOrUndefined(shapeData) && typeof shapeData === 'string'
            ? shapeData.toLowerCase()
            : shapeData;
        const shapePropertiesValue: string =
          !isNullOrUndefined(shape[properties[i as number]]) &&
          isNaN(shape[properties[i as number]])
            ? shape[properties[i as number]].toLowerCase()
            : shape[properties[i as number]];
 ...
 * @private
 */
export function getRatioOfBubble(
  min: number,
  max: number,
  value: number,
  minValue: number,
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function findMidPointOfPolygon(
  points: MapLocation[],
  type: string,
  geometryType?: string
): any {
  ...
  for (let i: number = min; i <= max - 1; i++) {
    ...
    startY =
      type === 'Mercator' || geometryType === 'Normal'
        ? points[i as number].y
        : -points[i as number].y;
    if (i === max - 1) {
      startX1 = points[0].x;
      startY1 =
    ...
    xSum =
      xSum +
      Math.abs((startX + startX1) * (startX * startY1 - startX1 * startY));
    ySum =
      ySum +
      Math.abs((startY + startY1) * (startX * startY1 - startX1 * startY));
  }
  sum = 0.5 * sum;
  // eslint-disable-next-line @typescript-eslint/tslint/config
  const pointValue: number =
    points.some((point) => point.x < 5 && point.y < 5) &&
    geometryType === 'Normal'
      ? 6
      : 4;
  ...
  for (let i: number = min; i <= max - 1; i++) {
    const point: MapLocation = points[i as number];
    point.y =
      type === 'Mercator' || geometryType === 'Normal' ? point.y : -point.y;
    if (point.y > ySum) {
      if (point.x < xSum && xSum - point.x < xSum - bottomMinPoint.x) {
        bottomMinPoint = { x: point.x, y: point.y };
      } else if (
        point.x > xSum &&
        (bottomMaxPoint.x === 0 || point.x - xSum < bottomMaxPoint.x - xSum)
      ) {
      ...
      if (point.x < xSum && xSum - point.x < xSum - topMinPoint.x) {
        ...
        point.x > xSum &&
        (topMaxPoint.x === 0 || point.x - xSum < topMaxPoint.x - xSum)
      ) {
        topMaxPoint = { x: point.x, y: point.y };
      }
    }
    height =
      bottomMaxPoint.y - topMaxPoint.y + (bottomMaxPoint.y - topMaxPoint.y) / 4;
    if (point.x > xSum) {
      if (point.y < ySum && ySum - point.y < ySum - rightMinPoint.y) {
        rightMinPoint = { x: point.x, y: point.y };
      } else if (
        point.y > ySum &&
        (rightMaxPoint.y === 0 || point.y - ySum < rightMaxPoint.y - ySum)
      ) {
      ...
      if (point.y < ySum && ySum - point.y < ySum - leftMinPoint.y) {
        ...
        point.y > ySum &&
        (leftMaxPoint.y === 0 || point.y - ySum < leftMaxPoint.y - ySum)
      ) {
...
export function isCustomPath(layerData: any[]): boolean {
  ...
  if (Object.prototype.toString.call(layerData) === '[object Array]') {
    Array.prototype.forEach.call(layerData, (layer: LayerSettings) => {
      if (
        !isNullOrUndefined(layer['geometry']) &&
        layer['geometry']['type'] === 'Path'
      ) {
        customPath = true;
      }
    });
...
export function findPosition(
  ...
  switch (alignment) {
    ...
    case 'Center':
      x =
        type === 'title'
          ? location.width / 2 - textSize.width / 2
          : location.x + location.width / 2 - textSize.width / 2;
      break;
    case 'Far':
      x =
        type === 'title'
          ? location.width - location.y - textSize.width
  ...
  const y: number =
    type === 'title'
      ...
      : location.y + location.height / 2 + textSize.height / 2;
  return new Point(x, y);
}
/**
 ...
 * @private
 */
export function calculateCenterFromPixel(
  mapObject: Maps,
  layer: LayerSettings
): Point {
  const point1: Point = convertGeoToPoint(
    mapObject.minLatOfGivenLocation,
    mapObject.minLongOfGivenLocation,
    mapObject.mapLayerPanel.calculateFactor(layer),
    layer,
    mapObject
  );
  const point2: Point = convertGeoToPoint(
    mapObject.maxLatOfGivenLocation,
    mapObject.maxLongOfGivenLocation,
    mapObject.mapLayerPanel.calculateFactor(layer),
    layer,
    mapObject
  );
...
export function getTranslate(
  ...
  const checkMethodeZoom: boolean =
    !isNullOrUndefined(mapObject.centerLatOfGivenLocation) &&
    !isNullOrUndefined(mapObject.centerLongOfGivenLocation) &&
    mapObject.zoomNotApplied;
  if (isNullOrUndefined(mapObject.mapScaleValue)) {
    mapObject.mapScaleValue = zoomFactorValue;
  }
  if (
    mapObject.zoomSettings.shouldZoomInitially &&
    mapObject.zoomSettings.enable
  ) {
    mapObject.mapScaleValue =
      scaleFactor =
      zoomFactorValue =
        (mapObject.zoomSettings.shouldZoomInitially ||
          mapObject.enablePersistence) &&
        mapObject.scale === 1
          ? // eslint-disable-next-line radix
            mapObject.scale
          : isNullOrUndefined(mapObject.markerZoomFactor)
          ? 1
          : mapObject.markerZoomedState
          ? mapObject.markerZoomFactor
          : parseInt(mapObject.scale.toString());
    if (
      mapObject.markerZoomedState &&
      mapObject.mapScaleValue !== mapObject.markerZoomFactor &&
      !mapObject.enablePersistence
    ) {
    ...
    if (
      mapObject.markerZoomedState &&
      !isNullOrUndefined(mapObject.markerCenterLatitude) &&
      !isNullOrUndefined(mapObject.markerCenterLongitude)
    ) {
  ...
  if (checkMethodeZoom) {
    mapObject.mapScaleValue =
      scaleFactor =
      zoomFactorValue =
        mapObject.scaleOfGivenLocation;
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const min: any = !isNullOrUndefined(mapObject.baseMapRectBounds)
    ? (mapObject.baseMapRectBounds['min'] as any)
    : null;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const max: any = !isNullOrUndefined(mapObject.baseMapRectBounds)
    ? (mapObject.baseMapRectBounds['max'] as any)
    : null;
  const zoomFactor: number = animate ? 1 : mapObject.mapScaleValue;
  if (isNullOrUndefined(mapObject.currentShapeDataLength)) {
    mapObject.currentShapeDataLength = !isNullOrUndefined(
      layer.shapeData['features']
    )
      ? layer.shapeData['features'].length
      : !isNullOrUndefined(layer.shapeData['geometries'])
      ? layer.shapeData['geometries'].length
      : 0;
  }
  const size: Rect =
    mapObject.totalRect && mapObject.legendSettings.visible
      ? mapObject.totalRect
  ...
  if (!isNullOrUndefined(min) && !isNullOrUndefined(max)) {
    ...
    const factor: number = animate
      ? 1
      : mapObject.markerZoomFactor === 1
      ? mapObject.mapScaleValue
      : zoomFactorValue;
    center =
      mapObject.zoomSettings.shouldZoomInitially &&
      mapObject.markerZoomedState &&
      !mapObject.zoomPersistence
        ? mapObject.markerZoomCenterPoint
    ...
    if (
      ...
      const leftPosition: number =
        ((mapWidth + Math.abs(mapObject.mapAreaRect.width - mapWidth)) / 2 +
          mapObject.mapAreaRect.x) /
        factor;
      const topPosition: number =
        ((mapHeight + Math.abs(mapObject.mapAreaRect.height - mapHeight)) / 2 +
          mapObject.mapAreaRect.y) /
        factor;
      const point: Point = checkMethodeZoom
        ? calculateCenterFromPixel(mapObject, layer)
        : convertGeoToPoint(
            centerLatitude,
            centerLongitude,
            mapObject.mapLayerPanel.calculateFactor(layer),
      ...
      if (
        isNullOrUndefined(mapObject.previousProjection) ||
        mapObject.previousProjection !== mapObject.projectionType ||
      ...
      if (
        ...
        scaleFactor = parseFloat(
          Math.min(size.width / mapWidth, size.height / mapHeight).toFixed(2)
        );
        scaleFactor = scaleFactor > 1.05 ? 1 : scaleFactor;
        mapWidth *= scaleFactor;
        mapHeight *= scaleFactor;
        const widthDiff: number =
          min['x'] !== 0 && mapObject.translateType === 'layers'
            ? availSize.width - size.width
            : 0;
        ...
        if (
          ...
          scaleFactor =
            mapObject.mapScaleValue < 1
              ...
              : mapObject.mapScaleValue;
          mapObject.mapScaleValue =
            mapObject.zoomSettings.enable && mapObject.mapScaleValue !== 1
              ? mapObject.mapScaleValue
              : 1;
          if (
            mapObject.currentShapeDataLength !==
            ...
            const scale: number = parseFloat(
              Math.min(size.height / mapHeight, size.width / mapWidth).toFixed(
                2
              )
            );
            mapHeight *= scale;
            ...
            (mapObject.availableSize.height !== mapObject.heightBeforeRefresh ||
              mapObject.widthBeforeRefresh !== mapObject.availableSize.width) &&
            !isNullOrUndefined(mapObject.translatePoint) &&
            !isNullOrUndefined(mapObject.previousTranslate)
          ) {
            const cscaleFactor: number = parseFloat(
              Math.min(size.width / mapWidth, size.height / mapHeight).toFixed(
                2
              )
            );
            let cmapWidth: number = mapWidth;
            cmapWidth *= cscaleFactor;
            let cmapHeight: number = mapHeight;
            cmapHeight *= cscaleFactor;
            const x1: number =
              size.x + (-min['x'] + (size.width / 2 - cmapWidth / 2));
            const y1: number =
              size.y + (-min['y'] + (size.height / 2 - cmapHeight / 2));
            const xdiff: number =
              (mapObject.translatePoint.x - mapObject.previousTranslate.x) /
              mapObject.widthBeforeRefresh;
            const ydiff: number =
              (mapObject.translatePoint.y - mapObject.previousTranslate.y) /
              mapObject.heightBeforeRefresh;
            ...
            if (
              !isNullOrUndefined(mapObject.previousProjection) &&
              (mapObject.mapScaleValue === 1 ||
                mapObject.mapScaleValue <= 1.05) &&
              !mapObject.zoomModule.isDragZoom
            ) {
              scaleFactor = parseFloat(
                Math.min(
                  size.width / mapWidth,
    ...
    if (!isNullOrUndefined(mapObject.translatePoint)) {
      x =
        mapObject.enablePersistence &&
        mapObject.translatePoint.x !== 0 &&
        !mapObject.zoomNotApplied
          ? mapObject.translatePoint.x
          : x;
      y =
        mapObject.enablePersistence &&
        mapObject.translatePoint.y !== 0 &&
        !mapObject.zoomNotApplied
          ? mapObject.translatePoint.y
          : y;
    }
  }
  scaleFactor = mapObject.enablePersistence
    ? mapObject.mapScaleValue >= 1
      ? mapObject.mapScaleValue
      : 1
    : scaleFactor;
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function getZoomTranslate(
  mapObject: Maps,
  layer: LayerSettings,
  animate?: boolean
): any {
  ...
  const checkZoomMethod: boolean =
    !isNullOrUndefined(mapObject.centerLongOfGivenLocation) &&
    !isNullOrUndefined(mapObject.centerLatOfGivenLocation) &&
    mapObject.zoomNotApplied;
  if (
    isNullOrUndefined(mapObject.previousCenterLatitude) &&
  ...
  if (
    isNullOrUndefined(mapObject.mapScaleValue) ||
  ...
  mapObject.mapScaleValue =
    mapObject.zoomSettings.zoomFactor !== 1 &&
    mapObject.zoomSettings.zoomFactor === mapObject.mapScaleValue
      ? mapObject.zoomSettings.zoomFactor
      : mapObject.zoomSettings.zoomFactor !== mapObject.mapScaleValue &&
        !mapObject.centerPositionChanged
      ? mapObject.mapScaleValue
      : mapObject.zoomSettings.zoomFactor;
  if (
    mapObject.zoomSettings.shouldZoomInitially &&
    !mapObject.isZoomByPosition
  ) {
    mapObject.mapScaleValue =
      zoomFactorValue =
      scaleFactor =
  ...
  const zoomFactor: number = animate ? 1 : mapObject.mapScaleValue;
  const size: Rect = mapObject.mapAreaRect;
  let x: number;
  let y: number;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  if (
    ...
    const topPosition: number =
      (mapHeight + Math.abs(mapObject.mapAreaRect.height - mapHeight)) /
      2 /
      factor;
    const leftPosition: number =
      (mapWidth + Math.abs(mapObject.mapAreaRect.width - mapWidth)) /
      2 /
      factor;
    const point: Point = checkZoomMethod
      ? calculateCenterFromPixel(mapObject, layer)
      : convertGeoToPoint(
          latitude,
          longitude,
    ...
    if (
      (!isNullOrUndefined(mapObject.zoomTranslatePoint) ||
        !isNullOrUndefined(mapObject.previousProjection)) &&
      !mapObject.zoomNotApplied
    ) {
    ...
    let zoomFact: number =
      mapObject.zoomSettings.zoomFactor === 0
        ? 1
        : mapObject.zoomSettings.zoomFactor;
    ...
    const mapScale: number =
      mapObject.mapScaleValue === 0
        ? 1
        : mapObject.mapScaleValue > maxZoomFact
        ? maxZoomFact
        : mapObject.mapScaleValue;
    let leftPosition: number =
      size.x + (-min['x'] + (size.width / 2 - mapWidth / 2));
    let topPosition: number =
      size.y + (-min['y'] + (size.height / 2 - mapHeight / 2));
    if (
      !isNullOrUndefined(mapObject.zoomTranslatePoint) ||
      !isNullOrUndefined(mapObject.previousProjection)
    ) {
      if (mapObject.previousProjection !== mapObject.projectionType) {
        ...
        for (let i: number = 1; i < maxZoomFact; i++) {
          const translatePointX: number =
            previousPositions[i - 1]['x'] -
            (size.width / i - size.width / (i + 1)) / 2;
          const translatePointY: number =
            previousPositions[i - 1]['y'] -
            (size.height / i - size.height / (i + 1)) / 2;
...
export function fixInitialScaleForTile(map: Maps): void {
  map.tileZoomScale = map.tileZoomLevel =
    Math.floor(map.availableSize.height / 512) + 1;
  ...
  map.tileTranslatePoint.y =
    map.availableSize.height / 2 - totalSize / 2 + padding;
...
export function getShapeData(
  targetId: string,
  map: Maps
): { shapeData: any; data: any } {
  const layerIndex: number = parseInt(
    targetId.split('_LayerIndex_')[1].split('_')[0],
    10
  );
  const shapeIndex: number = parseInt(
    targetId.split('_shapeIndex_')[1].split('_')[0],
    10
  );
  const layer: LayerSettings = map.layersCollection[
    layerIndex as number
  ] as LayerSettings;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    data =
      layer.dataSource[
        checkShapeDataFields(
          <any[]>layer.dataSource,
          shapeData,
          layer.shapeDataPath,
          layer.shapePropertyPath,
          layer
        )
      ];
  }
...
export function triggerShapeEvent(
  ...
  maps.trigger(eventName, eventArgs, () => {
    eventArgs.border.opacity = isNullOrUndefined(eventArgs.border.opacity)
      ? eventArgs.opacity
      : eventArgs.border.opacity;
  });
  return eventArgs;
...
export function getTargetElement(
  ...
  let targetId: string;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const shapeData: any[] = <any[]>(
    map.layers[layerIndex as number].shapeData['features']
  );
...
export function createStyle(
  id: string,
  className: string,
  eventArgs: IShapeSelectedEventArgs | any
): Element {
  ...
  styleEle.innerText =
    ...
    ';' +
    'fill-opacity:' +
    eventArgs['opacity'].toString() +
    ';' +
    'stroke-opacity:' +
    eventArgs['border']['opacity'].toString() +
    ';' +
    'stroke-width:' +
    eventArgs['border']['width'].toString() +
    ';' +
    'stroke:' +
...
export function customizeStyle(
  ...
  if (!isNullOrUndefined(styleEle)) {
    (styleEle as HTMLElement).innerText =
      '.' +
      className +
      '{fill:' +
      eventArgs['fill'] +
 ...
 * @private
 */
export function triggerItemSelectionEvent(
  ...
  map.trigger(
    'itemSelection',
    eventArgs,
    (observedArgs: ISelectionEventArgs) => {
      eventArgs.border.opacity = isNullOrUndefined(
      ...
      if (!getElement('ShapeselectionMap')) {
        document.body.appendChild(
          createStyle('ShapeselectionMap', 'ShapeselectionMapStyle', eventArgs)
        );
      } else {
        customizeStyle(
          'ShapeselectionMap',
    ...
    progress: (args: AnimationOptions): void => {
      if (args.timeStamp > args.delay && !maps.zoomModule.isPanningInProgress) {
        if (maps.isTileMap && height === 0) {
          const layerGroupElement: HTMLElement = document.querySelector(
            '.GroupElement'
          ) as HTMLElement;
...
export function showTooltip(
  ...
  if (!tooltip) {
    ...
    tooltip.style.cssText =
      'background-color: rgb(255, 255, 255) !important; color:black !important; ' +
...
export function wordWrap(
  ...
  tooltip.style.top =
    tooltip.id.indexOf('_Legend') !== -1
      ? (parseInt(size1[0], 10) + y).toString() + 'px'
      : (parseInt(size1[0], 10) * 2).toString() + 'px';
...
export function createTooltip(
  ...
  top: number,
  left: number,
  settings: ZoomToolbarTooltipSettingsModel
): void {
...
export function getHexColor(color: string): any {
  if (color.indexOf('#') !== -1 && color.toLowerCase().indexOf('rgb') === -1) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const colorArray: any = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(
      color
    );
    return colorArray
      ? {
    ...
    const colorArray: number[] = color.match(/\d+/g).map((a: string) => {
    ...
    // eslint-disable-next-line @typescript-eslint/tslint/config
    const colorArray: number[] = window
      .getComputedStyle(document.body.appendChild(divElment))
      .color.match(/\d+/g)
      .map((a) => {
        return parseInt(a, 10);
      });
...
export function drawSymbol(
  ...
  const htmlObject: Element = renderer['draw' + temp.functionName](
    temp.renderOption
  );
  return htmlObject;
}
 ...
 * @private
 */
export function renderLegendShape(
  location: MapLocation,
  size: Size,
  shape: string,
  options: PathOption,
  ...
  switch (shape) {
    ...
    case 'Pentagon':
      ...
      for (let i: number = 0; i <= 5; i++) {
        ...
        if (i === 0) {
          renderPath =
            'M' + ' ' + (shapeX + xValue) + ' ' + (shapeY + yValue) + ' ';
        } else {
          renderPath = renderPath.concat(
            'L' + ' ' + (shapeX + xValue) + ' ' + (shapeY + yValue) + ' '
          );
        }
      }
 ...
 * @private
 */
export function changeBorderWidth(
  element: Element,
  index: number,
  scale: number,
  maps: Maps
  ...
  for (let l: number = 0; l < element.childElementCount; l++) {
    ...
    if (childNode.id.indexOf('_NavigationGroup') > -1) {
      ...
      for (let i: number = 0; i < childNode.childElementCount; i++) {
        ...
        const width: number =
          maps.layersCollection[index].polygonSettings.polygons[
            parseInt(
              childNode.children[i as number].id.split('_PolygonIndex_')[1]
            )
          ].borderWidth;
        childNode.children[i as number].setAttribute(
          'stroke-width',
      ...
      let value: number = 0;
      const borderWidthValue: string =
        maps.layersCollection[index as number].shapeSettings
          .borderWidthValuePath;
      const borderWidth: number = (<LayerSettingsModel>(
        maps.layersCollection[index as number]
      )).shapeSettings.border.width;
      const circleRadius: number = (<LayerSettingsModel>(
        maps.layersCollection[index as number]
      )).shapeSettings.circleRadius;
      if (
        maps.layersCollection[index as number].shapeSettings
          .borderWidthValuePath
      ) {
        value = checkShapeDataFields(
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          <any[]>maps.layersCollection[index as number].dataSource,
          maps.layersCollection[index as number].layerData[l as number][
            'property'
          ],
          maps.layersCollection[index as number].shapeDataPath,
          maps.layersCollection[index as number].shapePropertyPath,
          maps.layersCollection[index as number]
        );
        if (value !== null) {
          if (
            maps.layersCollection[index as number].dataSource[value as number][
              borderWidthValue as string
            ]
          ) {
            currentStroke =
              maps.layersCollection[index as number].dataSource[
                value as number
              ][borderWidthValue as string];
          } else {
      ...
      childNode.setAttribute(
        'stroke-width',
        (currentStroke / scale).toString()
      );
      if (
        element.id.indexOf('_Point') > -1 ||
        element.id.indexOf('_MultiPoint') > -1
      ) {
 ...
 * @private
 */
export function changeNavaigationLineWidth(
  element: Element,
  index: number,
  scale: number,
  maps: Maps
  ...
  for (let m: number = 0; m < element.childElementCount; m++) {
    ...
    if (node.tagName === 'path') {
      const currentStroke: number = (<LayerSettingsModel>(
        maps.layersCollection[index as number]
      )).navigationLineSettings[
        parseFloat(node.id.split('_NavigationIndex_')[1].split('_')[0])
      ].width;
...
export function targetTouches(event: PointerEvent | TouchEvent): ITouches[] {
  ...
  for (let i: number = 0; i < touches.length; i++) {
    targetTouches.push({
      pageX: touches[i as number].pageX,
      pageY: touches[i as number].pageY,
    });
  }
...
export function zoomAnimate(
  ...
  if (scale === preScale) {
    element.setAttribute(
      'transform',
      'scale( ' + scale + ' ) translate( ' + point.x + ' ' + point.y + ' )'
    );
    return;
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  animate(
    ...
    (args: AnimationOptions): void => {
      if (args.timeStamp > args.delay) {
        ...
        currentLocation.x =
          previousLocation.x +
          (delta * horizontalDifference) / (currentScale / scale);
        if (slopeFactor == null) {
          currentLocation.y = previousLocation.y + delta * verticalDifference;
        } else {
          currentLocation.y =
            slopeFactor * currentLocation.x + slopeIntersection;
        }
  ...
  const startAnimation: FrameRequestCallback = (timestamp: number) => {
    ...
    if (progress < duration) {
      ...
      if (element.id.indexOf('Marker') > -1) {
        const markerElement: Element = getElementByID(
          element.id.split('_Layer')[0] + '_Markers_Group'
        );
 ...
 * @private
 */
export function smoothTranslate(
  element: Element,
  delay: number,
  duration: number,
  point: MapLocation
  ...
  const previousLocation: MapLocation = new MapLocation(
    parseInt(transform[1], 10),
    parseInt(transform[2], 10)
  );
...
export function calculateZoomLevel(
  ...
  const maxLatValue: number =
    Math.max(Math.min(maxLatRad, Math.PI), -Math.PI) / 2;
  const minLatSin: number = Math.sin((minLat * Math.PI) / 180);
  const minLatRad: number = Math.log((1 + minLatSin) / (1 - minLatSin)) / 2;
  const minLatValue: number =
    Math.max(Math.min(minLatRad, Math.PI), -Math.PI) / 2;

  if (maps.zoomNotApplied && !maps.isTileMap) {
    const latiRatio: number = Math.abs(
      (maps.baseMapBounds.latitude.max - maps.baseMapBounds.latitude.min) /
        (maxLat - minLat)
    );
    const longiRatio: number = Math.abs(
      (maps.baseMapBounds.longitude.max - maps.baseMapBounds.longitude.min) /
        (maxLong - minLong)
    );
    applyMethodeZoom = isZoomToCoordinates
      ? (latiRatio + longiRatio) / 2
      : Math.min(latiRatio, longiRatio);
  }
  ...
  const latZoom: number =
    Math.log(mapHeight / WORLD_PX_HEIGHT / latRatio) / Math.LN2;
  const lngZoom: number =
    Math.log(mapWidth / WORLD_PX_WIDTH / lngRatio) / Math.LN2;
  const result: number =
    maps.zoomNotApplied && !maps.isTileMap
      ? applyMethodeZoom
      : isZoomToCoordinates && !maps.isTileMap
      ? (latZoom + lngZoom) / 2
      : Math.min(latZoom, lngZoom);
  scaleFactor = Math.min(result, maxZoomFact);
  scaleFactor =
    maps.isTileMap || !maps.zoomNotApplied
...
export function processResult(e: any): any {
  ...
  let dataValue: any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const resultValue: any = !isNullOrUndefined(e['result'])
    ? e['result']
    : e['actual'];
