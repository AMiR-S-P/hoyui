...
import { ModuleDeclaration } from '@hoyui/base';
...
import {
  GeoPosition,
  ITooltipRenderCompleteEventArgs,
...
import {
  IMarkerMoveEventArgs,
  ILabelRenderingEventArgs,
  IBubbleMoveEventArgs,
  ...
  implements INotifyPropertyChanged
{
  ...
  private mergePersistMapsData(): void {
    ...
    if (!isNullOrUndefined(windowData)) {
      data = window.localStorage.getItem(
        this.getModuleName() + this.element.id
      );
    }
    if (!(isNullOrUndefined(data) || data === '')) {
      ...
      for (const key of keys) {
        if (
          ...
          !isNullOrUndefined(this[key as string])
        ) {
  ...
  private renderElements(): void {
    if (!this.isDestroyed) {
      ...
      if (
        !isNullOrUndefined(this.markerModule) &&
        this.markerModule.getMapsInstance()
      ) {
    ...
    Array.prototype.forEach.call(
      ...
      (layer: LayerSettings, layerIndex: number) => {
        if (layer.shapeData instanceof DataManager) {
          ...
          queryModule =
            layer.query instanceof Query ? layer.query : new Query();
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const dataManager: Promise<any> =
            dataModule.executeQuery(queryModule);
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        if (layer.markerSettings.length > 0) {
          for (let i: number = 0; i < layer.markerSettings.length; i++) {
            if (
              layer.markerSettings[i as number].dataSource instanceof
              DataManager
            ) {
              this.serverProcess['request']++;
              dataModule = layer.markerSettings[i as number]
                .dataSource as DataManager;
              queryModule =
                layer.markerSettings[i as number].query instanceof Query
                  ? layer.markerSettings[i as number].query
                  : new Query();
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              const dataManager: Promise<any> =
                dataModule.executeQuery(queryModule);
              ...
              dataManager.then((e: any) => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                layer.markerSettings[i as number].dataSource = processResult(
                  e
                ) as any[];
              });
            }
        ...
        if (layer.bubbleSettings.length > 0) {
          for (let i: number = 0; i < layer.bubbleSettings.length; i++) {
            if (
              layer.bubbleSettings[i as number].dataSource instanceof
              DataManager
            ) {
              this.serverProcess['request']++;
              dataModule = layer.bubbleSettings[i as number]
                .dataSource as DataManager;
              queryModule =
                layer.bubbleSettings[i as number].query instanceof Query
                  ? layer.bubbleSettings[i as number].query
                  : new Query();
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              const dataManager: Promise<any> =
                dataModule.executeQuery(queryModule);
              ...
              dataManager.then((e: any) => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                layer.bubbleSettings[i as number].dataSource = processResult(
                  e
                ) as any[];
              });
            }
        ...
        if (
          layer.dataSource instanceof MapAjax ||
          (!isNullOrUndefined(layer.dataSource) &&
            !isNullOrUndefined(layer.dataSource['dataOptions']))
        ) {
          this.processAjaxRequest(layer, layer.dataSource, 'DataSource');
        }
        if (
          this.serverProcess['request'] === this.serverProcess['response'] &&
          length === layerIndex
        ) {
  ...
  private processAjaxRequest(
    layer: LayerSettings,
    localAjax: MapAjax | any,
    type: string
  ): void {
    this.serverProcess['request']++;
    const fetchApiModule: Fetch = new Fetch(
      localAjax.dataOptions,
      localAjax.type,
      localAjax.contentType
    );
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    fetchApiModule.onSuccess = (args: any) => {
      if (
        !isNullOrUndefined(args.type) &&
   ...
   * @private
   */
  public processResponseJsonData(
    processType: string,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    data?: any | string,
    layer?: LayerSettings,
    ...
    if (processType) {
      if (dataType === 'ShapeData') {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        layer.shapeData =
          processType === 'DataManager' ? processResult(data as any) : data;
      } else {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        layer.dataSource =
          processType === 'DataManager' ? processResult(data as any) : data;
  ...
  private renderMap(): void {
    ...
    for (let i: number = 0; i < this.layers.length; i++) {
      if (
        this.layers[i as number].selectionSettings &&
        this.layers[i as number].selectionSettings.enable &&
        this.layers[i as number].initialShapeSelection.length > 0 &&
        this.checkInitialRender
      ) {
        const checkSelection: boolean =
          this.layers[i as number].selectionSettings.enableMultiSelect;
        this.layers[i as number].selectionSettings.enableMultiSelect =
          checkSelection ? checkSelection : true;
        const shapeSelection: InitialShapeSelectionSettingsModel[] =
          this.layers[i as number].initialShapeSelection;
        for (
          let j: number = 0;
          j < this.layers[i as number].initialShapeSelection.length;
          j++
        ) {
          this.shapeSelection(
            i,
            shapeSelection[j as number].shapePath,
            shapeSelection[j as number].shapeValue,
            true
          );
        }
        this.layers[i as number].selectionSettings.enableMultiSelect =
      ...
      if (!this.isResize) {
        for (
          let k: number = 0;
          k < this.layers[i as number].markerSettings.length;
          k++
        ) {
          // eslint-disable-next-line max-len
          if (
            this.layers[i as number].markerSettings[k as number]
              .selectionSettings &&
            this.layers[i as number].markerSettings[k as number]
              .selectionSettings.enable &&
            this.layers[i as number].markerSettings[k as number]
              .initialMarkerSelection.length > 0
          ) {
    ...
    if (
      !isNullOrUndefined(
        document.getElementById(this.element.id + '_tile_parent')
      )
    ) {
      ...
      const tileElement: HTMLElement = document.getElementById(
        this.element.id + '_tile_parent'
      );
      const tileElement1: HTMLElement = document.getElementById(
        this.element.id + '_tiles'
      );
      ...
      top =
        bottom <= 11
          ? top
          : !isNullOrUndefined(this.legendModule) &&
      ...
      if (
        !isNullOrUndefined(this.legendModule) &&
        this.legendModule.totalPages.length > 0
      ) {
        tileElement.style.height = tileElement1.style.height =
          this.legendModule.legendTotalRect.height + 'px';
        tileElement.style.width = tileElement1.style.width =
          this.legendModule.legendTotalRect.width + 'px';
      }
    }
    ...
    for (
      let i: number = 0;
      i < document.getElementsByTagName('path').length - 1;
      i++
    ) {
      if (
        document
          .getElementsByTagName('path')
          [i as number].id.indexOf('shapeIndex') > -1
      ) {
        document.getElementsByTagName('path')[i as number].style.outline =
          'none';
      }
    }
    this.zoomingChange();
    if (this.zoomModule && this.isDevice) {
      this.zoomModule.removeToolbarOpacity(
        this.isTileMap ? Math.round(this.tileZoomLevel) : this.mapScaleValue,
        this.element.id + '_Zooming_'
      );
    }
  ...
  private triggerZoomEvent(): void {
    ...
    const minMaxLatitudeLongitude: IMinMaxLatitudeLongitude =
      this.getMinMaxLatitudeLongitude();
  ...
  public markerSelection(
    ...
    if (
      this.selectedMarkerElementId.length === 0 ||
      selectionSettings.enableMultiSelect
    ) {
      if (targetElement.tagName === 'g') {
        targetElement.children[0].setAttribute(
          'class',
          'MarkerselectionMapStyle'
        );
  ...
  private renderArea(): void {
    ...
    if (width > 0 || background || this.themeStyle.areaBackgroundColor) {
      const mapBorder: BorderModel = {
        opacity: isNullOrUndefined(this.mapsArea.border.opacity)
          ? 1
          : this.mapsArea.border.opacity,
        color: this.mapsArea.border.color,
        width: this.mapsArea.border.width,
      };
      const rect: RectOption = new RectOption(
        this.element.id + '_MapAreaBorder',
        background || this.themeStyle.areaBackgroundColor,
        mapBorder,
        1,
        this.mapAreaRect
      );
      this.svgObject.appendChild(
        this.renderer.drawRectangle(rect) as SVGRectElement
      );
    }
  }
  /**
  ...
  private setSecondaryElementPosition(): void {
    const element: HTMLDivElement = getElementByID(
  ...
  private zoomingChange(): void {
    ...
    if (
      ...
      this.zoomModule.layerCollectionEle = getElementByID(
        this.element.id + '_Layer_Collections'
      );
    }
    if (
      this.isTileMap &&
      getElementByID(this.element.id + '_Tile_SVG') &&
      getElementByID(this.element.id + '_tile_parent')
    ) {
      const tileElement: Element = getElementByID(
        this.element.id + '_tile_parent'
      );
      const tileSvgElement: Element = getElementByID(
        this.element.id + '_Tile_SVG'
      );
      const tileSvgParentElement: Element = getElementByID(
        this.element.id + '_Tile_SVG_Parent'
      );
      ...
      if (
        ...
        (tileElement as HTMLElement).style.width = (
          tileSvgElement as HTMLElement
        ).style.width = this.legendModule.legendTotalRect.width.toString();
        (tileElement as HTMLElement).style.height = (
          tileSvgElement as HTMLElement
        ).style.height = this.legendModule.legendTotalRect.height.toString();
        (tileSvgParentElement as HTMLElement).style.width =
          this.legendModule.legendTotalRect.width + 'px';
        (tileSvgParentElement as HTMLElement).style.height =
          this.legendModule.legendTotalRect.height + 'px';
      }
      const markerTemplateElements: HTMLCollectionOf<Element> =
        document.getElementsByClassName('template');
      if (
        !isNullOrUndefined(markerTemplateElements) &&
        markerTemplateElements.length > 0
      ) {
        for (let i: number = 0; i < markerTemplateElements.length; i++) {
          const templateGroupEle: HTMLElement = markerTemplateElements[
            i as number
          ] as HTMLElement;
    ...
    if (this.zoomSettings.zoomFactor >= 0) {
      if (
        this.zoomModule &&
        this.zoomModule.toolBarGroup &&
        this.zoomSettings.enable
      ) {
        this.zoomModule.alignToolBar();
      }
      const elements: Element = document.getElementById(
        this.element.id + '_Layer_Collections'
      );
      if (!isNullOrUndefined(elements) && elements.childElementCount > 0) {
        for (let i: number = 0; i < elements.childNodes.length; i++) {
          const childElement: SVGAElement = elements.childNodes[
            i as number
          ] as SVGAElement;
          if (
            childElement.tagName === 'g' &&
            childElement.id.indexOf('_Legend_Group') === -1
          ) {
            const layerIndex: number = parseFloat(
              childElement.id.split('_LayerIndex_')[1].split('_')[0]
            );
            for (let j: number = 0; j < childElement.childNodes.length; j++) {
              const childNode: Element = <Element>(
                childElement.childNodes[j as number]
              );
              if (
                !(childNode.id.indexOf('_Markers_Group') > -1) &&
  ...
  private createSecondaryElement(): void {
    if (
      isNullOrUndefined(
        document.getElementById(this.element.id + '_Secondary_Element')
      )
    ) {
      const secondaryElement: Element = createElement('div', {
        id: this.element.id + '_Secondary_Element',
      });
      (secondaryElement as HTMLElement).style.cssText =
  ...
  public getMinMaxLatitudeLongitude(): IMinMaxLatitudeLongitude {
    ...
    if (!isNullOrUndefined(mapsElement)) {
      ...
      const minPosition: GeoPosition = this.isTileMap
        ? this.pointToLatLong(
            this.mapAreaRect.x - this.margin.left,
            -this.mapAreaRect.y
          )
        : this.getGeoLocation(
            0,
            this.mapAreaRect.x + element.left,
            this.mapAreaRect.y
          );
      const maxPosition: GeoPosition = this.isTileMap
        ? this.pointToLatLong(
            this.mapAreaRect.width,
            this.mapAreaRect.height - this.mapAreaRect.y
          )
        : this.getGeoLocation(
            0,
            this.mapAreaRect.x + element.left + this.mapAreaRect.width,
  ...
  public arrangeTemplate(): void {
    if (document.getElementById(this.element.id + '_Legend_Border')) {
      document.getElementById(
        this.element.id + '_Legend_Border'
      ).style.pointerEvents = 'none';
    }
    const templateElements: HTMLCollectionOf<Element> =
      document.getElementsByClassName(this.element.id + '_template');
    if (
      !isNullOrUndefined(templateElements) &&
      ...
      Array.prototype.forEach.call(
        templateElements,
        (templateGroupEle: Element) => {
          let offSetLetValue: number = 0;
          let offSetTopValue: number = 0;
          if (
            !isNullOrUndefined(templateGroupEle) &&
            templateGroupEle.childElementCount > 0
          ) {
            const layerOffset: ClientRect = getElementByID(
              this.element.id + '_Layer_Collections'
            ).getBoundingClientRect();
            const elementOffset: ClientRect = getElementByID(
              templateGroupEle.id
            ).getBoundingClientRect();
            if (templateGroupEle.id.indexOf('Marker') === -1) {
              offSetLetValue = this.isTileMap
                ? 0
                : layerOffset.left < elementOffset.left
                ? -Math.abs(elementOffset.left - layerOffset.left)
                : Math.abs(elementOffset.left - layerOffset.left);
              offSetTopValue = this.isTileMap
                ? 0
                : layerOffset.top < elementOffset.top
                ? -Math.abs(elementOffset.top - layerOffset.top)
                : Math.abs(elementOffset.top - layerOffset.top);
            }
            Array.prototype.forEach.call(
              templateGroupEle.childNodes,
              (currentTemplate: HTMLElement) => {
                if (currentTemplate.id.indexOf('Marker') !== -1) {
                  if (currentTemplate.style.visibility !== 'hidden') {
                    ...
                    currentTemplate.style.left =
                      parseFloat(currentTemplate.style.left) -
                      (this.isTileMap ? 0 : elementOffset.width / 2) +
                      'px';
                    currentTemplate.style.top =
                      parseFloat(currentTemplate.style.top) -
                      (this.isTileMap ? 0 : elementOffset.height / 2) +
                      'px';
                  }
                } else {
                  currentTemplate.style.left =
  ...
  private createTile(): void {
    ...
    if (
      mainLayer.isBaseLayer &&
      !isNullOrUndefined(mainLayer.urlTemplate) &&
      mainLayer.urlTemplate !== '' &&
      isNullOrUndefined(mainLayer.shapeData)
    ) {
      ...
      (ele as HTMLElement).style.cssText =
        'position: absolute; left: ' +
        this.mapAreaRect.x +
        'px; right: ' +
        this.margin.right +
        'px; top: ' +
        (this.mapAreaRect.y + padding) +
        'px; height: ' +
        this.mapAreaRect.height +
        'px; width: ' +
        this.mapAreaRect.width +
        'px; overflow: hidden;';
      ...
      (ele1 as HTMLElement).style.cssText =
        'position: absolute; left: ' +
        this.mapAreaRect.x +
        'px;  right: ' +
        this.margin.right +
  ...
  private initPrivateVariable(): void {
    if (this.element.id === '') {
      const collection: number =
        document.getElementsByClassName('e-maps').length;
  ...
  private renderBorder(): void {
    ...
    const borderElement: Element = this.svgObject.querySelector(
      '#' + this.element.id + '_MapBorder'
    );
    if (
      (width > 0 || this.background || this.themeStyle.backgroundColor) &&
      isNullOrUndefined(borderElement)
    ) {
      const border: BorderModel = {
        opacity: isNullOrUndefined(this.border.opacity)
          ? 1
          : this.border.opacity,
      ...
      const borderRect: RectOption = new RectOption(
        this.element.id + '_MapBorder',
        this.background || this.themeStyle.backgroundColor,
        border,
        1,
        new Rect(
          width / 2,
      ...
      this.svgObject.appendChild(
        this.renderer.drawRectangle(borderRect) as SVGRectElement
      );
    } else {
  ...
  private renderTitle(
    ...
    const width: number = Math.abs(
      this.margin.left + this.margin.right - this.availableSize.width
    );
    style.fontFamily = !isNullOrUndefined(style.fontFamily)
      ? style.fontFamily
      : this.themeStyle.fontFamily;
    style.fontWeight =
      type === 'title'
        ? style.fontWeight || this.themeStyle.titleFontWeight
        : style.fontWeight || this.themeStyle.titleFontWeight;
    style.size =
      type === 'title'
        ? style.size || this.themeStyle.titleFontSize
        : style.size ||
          this.themeStyle.subTitleFontSize ||
          Theme.mapsSubTitleFont.size;
    if (title.text) {
      ...
      const rect: Rect = isNullOrUndefined(bounds)
        ? new Rect(
            this.margin.left,
            this.margin.top,
        ...
        : bounds;
      const location: Point = findPosition(
        rect,
        title.alignment,
        elementSize,
        type
      ...
      const options: TextOption = new TextOption(
        this.element.id + '_Map_' + type,
        location.x,
        location.y,
        'start',
        trimmedTitle
      );
      const titleBounds: Rect = new Rect(
        location.x,
        location.y,
        elementSize.width,
        elementSize.height
      ...
      const element: Element = renderTextElement(
        ...
        style.color ||
          (type === 'title'
            ...
            : this.themeStyle.subTitleFontColor),
        groupEle
      );
      element.setAttribute('aria-label', title.text);
      element.setAttribute('role', 'region');
      if (
        (type === 'title' && !title.subtitleSettings.text) ||
        type === 'subtitle'
      ) {
        height = Math.abs(
          titleBounds.y + this.margin.bottom - this.availableSize.height
        );
      ...
      height = Math.abs(
        this.margin.top + this.margin.bottom - this.availableSize.height
      );
      this.mapAreaRect = new Rect(
        this.margin.left,
        this.margin.top,
  ...
  public mouseLeaveOnMap(e: PointerEvent): void {
    if (
      document.getElementsByClassName('highlightMapStyle').length > 0 &&
      this.legendModule
    ) {
  ...
  public keyUpHandler(event: KeyboardEvent): void {
    ...
    if (
      event.code === 'Tab' &&
      id.indexOf('_LayerIndex_') > -1 &&
      id.indexOf('shapeIndex') > -1
    ) {
  ...
  private keyboardHighlightSelection(id: string, event: KeyboardEvent): void {
    ...
    const layerIndex: number = parseInt(
      id.split('_LayerIndex_')[1].split('_')[0],
      10
    );
    const shapeIndex: number = parseInt(
      id.split('_shapeIndex_')[1].split('_')[0],
      10
    );
    //eslint-disable-next-line @typescript-eslint/no-explicit-any
    const shapeData: any =
      this.layers[layerIndex as number].shapeData['features']['length'] >
      shapeIndex
        ? this.layers[layerIndex as number].shapeData['features'][
            shapeIndex as number
          ]['properties']
        : null;
    const dataIndex: number = parseInt(
      id.split('_dataIndex_')[1].split('_')[0],
      10
    );
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const data: any = isNullOrUndefined(dataIndex)
      ? null
      : this.layers[layerIndex as number].dataSource[dataIndex as number];
    if (
      this.layers[layerIndex as number].selectionSettings.enable &&
      key === 'keydown' &&
      this.selectionModule
    ) {
      this.selectionModule.selectionsettings =
        this.layers[layerIndex as number].selectionSettings;
      this.selectionModule.selectionType = 'Shape';
      this.selectionModule.selectElement(
      ...
      this.layers[layerIndex as number].highlightSettings.enable &&
      key === 'keyup' &&
      !(event.target as HTMLElement).classList.contains(
        'ShapeselectionMapStyle'
      )
    ) {
      this.highlightModule.highlightSettings =
        this.layers[layerIndex as number].highlightSettings;
      this.highlightModule.handleHighlight(
        <Element>event.target,
        layerIndex,
        data,
        shapeData
  ...
  public keyDownHandler(event: KeyboardEvent): void {
    ...
    if (
      ...
      const animatedTiles: HTMLElement = document.getElementById(
        this.element.id + '_animated_tiles'
      );
    ...
    if (
      ...
      this.zoomSettings.enable &&
      this.zoomSettings.enablePanning &&
      zoom &&
      ...
      zoom.panning(
        'None',
        zoom.mouseDownLatLong['x'],
        event.code === 'ArrowUp'
          ? -zoom.mouseMoveLatLong['y']
          : zoom.mouseMoveLatLong['y'],
        event
      );
      ...
      zoom.panning(
        'None',
        event.code === 'ArrowLeft'
          ? -zoom.mouseMoveLatLong['x']
          : zoom.mouseMoveLatLong['x'],
        zoom.mouseDownLatLong['y'],
        event
      );
      ...
      if (
        this.legendModule &&
        (id.indexOf('_Left_Page_Rect') > -1 ||
          id.indexOf('_Right_Page_Rect') > -1)
      ) {
        this.mapAreaRect = this.legendModule.initialMapAreaRect;
        this.legendModule.currentPage =
          id.indexOf('_Left_Page_') > -1
        ...
        const textContent: string = document.getElementById(
          this.element.id + '_Paging_Text'
        ).textContent;
        ...
        if (id.indexOf('_Left_Page_Rect') > -1) {
          ...
          (event.target as HTMLElement).style.outlineColor =
            text[0] - 1 !== text[1] ? '' : 'transparent';
        } else if (id.indexOf('_Right_Page_Rect') > -1) {
          if (text[0] !== text[1]) {
            (event.target as HTMLElement).focus();
          }
          (event.target as HTMLElement).style.outlineColor =
            text[0] !== text[1] + 1 ? '' : 'transparent';
        }
        if (
          querySelector(this.element.id + '_Legend_Border', this.element.id)
        ) {
          (<HTMLElement>(
            querySelector(this.element.id + '_Legend_Border', this.element.id)
          )).style.pointerEvents = 'none';
  ...
  public mapsOnClick(e: PointerEvent): void {
    ...
    if (targetEle.id.indexOf('_ToolBar') === -1) {
      const latLongValue: GeoPosition = this.getClickLocation(
        targetId,
        e.pageX,
        e.pageY,
        targetEle as HTMLElement,
        e['layerX'],
        e['layerY']
      );
  ...
  private clickHandler(
    e: PointerEvent,
    eventArgs: IMouseEventArgs,
    targetEle: Element
  ): void {
    if (targetEle.id.indexOf('shapeIndex') > -1) {
      this.mergeCluster();
      if (
        getElement(this.element.id + '_mapsTooltip') &&
  ...
  private triggerShapeSelection(targetEle: Element): void {
    const layerIndex: number = parseInt(
      targetEle.id.split('_LayerIndex_')[1].split('_')[0],
      10
    );
    ...
    if (
      !shapeSelectedEventArgs.cancel &&
      this.selectionModule &&
      !isNullOrUndefined(this.shapeSelected)
    ) {
      customizeStyle(
        this.selectionModule.selectionType + 'selectionMap',
        this.selectionModule.selectionType + 'selectionMapStyle',
  ...
  private getMarkerClickLocation(
    ...
    let latLongValue: GeoPosition = this.getClickLocation(
      marker.targetId,
      pageX,
      pageY,
    ...
    const location: Point = this.isTileMap
      ? convertTileLatLongToPoint(
          new MapLocation(latLongValue.longitude, latLongValue.latitude),
          this.tileZoomLevel,
          this.tileTranslatePoint,
          true
        )
      : convertGeoToPoint(
          latLongValue.latitude,
          latLongValue.longitude,
          this.mapLayerPanel.currentFactor,
          <LayerSettings>this.layersCollection[marker.layerIndex],
          this
        );
    const transPoint: Point = this.translatePoint;
    const translateX: number = this.isTileMap
      ? location.x
      : (location.x + transPoint.x) * this.scale;
    const translateY: number = this.isTileMap
      ? location.y
      : (location.y + transPoint.y) * this.scale;
    if (this.markerDragArgument.shape !== 'Balloon') {
      ...
      targetElement.parentElement.setAttribute(
        'transform',
        'translate( ' + translateX + ' ' + translateY + ' )'
      );
    }
    if (isDragEnd) {
      const markerSettings: MarkerSettingsModel =
        this.layers[marker.layerIndex].markerSettings[marker.markerIndex];
      latLongValue = this.getClickLocation(
        marker.targetId,
        pageX - markerSettings.offset.x,
        pageY - markerSettings.offset.y,
        targetElement as HTMLElement,
        x - markerSettings.offset.x,
  ...
  public getClickLocation(
    ...
    if (
      targetId.indexOf('_LayerIndex_') !== -1 &&
      !this.isTileMap &&
      (!isNullOrUndefined(type) ||
        (parseInt(this.mouseDownEvent['x'], 10) ===
          parseInt(this.mouseClickEvent['x'], 10) &&
          parseInt(this.mouseDownEvent['y'], 10) ===
            parseInt(this.mouseClickEvent['y'], 10)))
    ) {
      ...
      if (this.layers[layerIndex as number].geometryType === 'Normal') {
        if (targetId.indexOf('_shapeIndex_') > -1) {
          ...
          const parentElement: Element =
            immediateParent.id.indexOf('_Point_Group') > -1 ||
            immediateParent.id.indexOf('_LineString_Group') > -1 ||
            immediateParent.id.indexOf('_MultiLineString_Group') > -1 ||
            immediateParent.id.indexOf('_Polygon_Group') > -1
              ? immediateParent.parentElement
              : immediateParent;
          const location: MapLocation = getMousePosition(
            pageX,
            pageY,
            parentElement
          );
          const zoomScaleValue: number =
            this.mapLayerPanel.currentFactor * this.mapScaleValue;
          const minLongitude: number = Math.abs(
            -this.baseMapBounds.longitude.min * zoomScaleValue
          );
          const minLatitude: number = Math.abs(
            this.baseMapBounds.latitude.max * zoomScaleValue
          );
          latLongValue = {
            latitude: Math.abs(
              this.baseMapBounds.latitude.max - location.y / zoomScaleValue
            ),
            longitude: Math.abs(
              location.x / zoomScaleValue + this.baseMapBounds.longitude.min
            ),
          };
          if (
            this.baseMapBounds.longitude.min < 0 &&
            minLongitude > location.x
          ) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ...
          targetId.indexOf('_MarkerIndex_') > -1 &&
          this.markerModule &&
          !this.markerDragArgument
        ) {
          const markerIndex: number = parseInt(
            targetId.split('_MarkerIndex_')[1].split('_')[0],
            10
          );
          const dataIndex: number = parseInt(
            targetId.split('_dataIndex_')[1].split('_')[0],
            10
          );
          if (!isNaN(markerIndex) && !isNaN(dataIndex)) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const dataObject: any =
              this.layers[layerIndex as number].markerSettings[
                markerIndex as number
              ].dataSource[dataIndex as number];
            latLongValue = {
          ...
          const element: HTMLElement = document.getElementById(
            this.element.id +
              '_LayerIndex_' +
              this.markerDragArgument.layerIndex
          );
          const elementRect: ClientRect = element.getBoundingClientRect();
          const location: MapLocation = new MapLocation(
            pageX > elementRect.left ? Math.abs(elementRect.left - pageX) : 0,
            pageY > elementRect.top ? Math.abs(elementRect.top - pageY) : 0
          );
          const minLongitude: number = Math.abs(
            -this.baseMapBounds.longitude.min * this.mapLayerPanel.currentFactor
          );
          const minLatitude: number = Math.abs(
            this.baseMapBounds.latitude.max * this.mapLayerPanel.currentFactor
          );
          latLongValue = {
            latitude: Math.abs(
              this.baseMapBounds.latitude.max -
                location.y / (this.mapLayerPanel.currentFactor * this.scale)
            ),
            longitude: Math.abs(
              location.x / (this.mapLayerPanel.currentFactor * this.scale) +
  ...
  private removeTileMap(): void {
    const animateElement: HTMLElement = document.getElementById(
      this.element.id + '_animated_tiles'
    );
    if (
      !isNullOrUndefined(this.currentTiles) &&
      animateElement.childElementCount < this.currentTiles.childElementCount
    ) {
      for (
        let l: number = animateElement.childElementCount - 1;
        l >= this.currentTiles.childElementCount;
        l--
      ) {
  ...
  public mouseEndOnMap(e: PointerEvent): boolean {
    ...
    if (!isNullOrUndefined(this.markerDragArgument)) {
      ...
      const latLongValue: GeoPosition = this.getMarkerClickLocation(
        pageX,
        pageY,
        layerX,
        layerY,
      ...
      const markerObject: MarkerSettingsModel =
        this.layers[marker.layerIndex].markerSettings[marker.markerIndex];
      document.getElementById(this.element.id + '_svg').style.cursor =
        markerObject.enableDrag ? 'pointer' : 'grabbing';
      const dragEventArgs: IMarkerDragEventArgs = {
        ...
        latitude: latLongValue.latitude,
        longitude: latLongValue.longitude,
        layerIndex: marker.layerIndex,
        markerIndex: marker.markerIndex,
        dataIndex: marker.dataIndex,
      };
      if (
        isNullOrUndefined(markerObject.latitudeValuePath) &&
        isNullOrUndefined(markerObject.longitudeValuePath)
      ) {
        const data: object = markerObject.dataSource[marker.dataIndex];
        if (
          !isNullOrUndefined(data['Longitude']) &&
          !isNullOrUndefined(data['Latitude'])
        ) {
          markerObject.dataSource[marker.dataIndex].Latitude =
            dragEventArgs.latitude;
          markerObject.dataSource[marker.dataIndex].Longitude =
            dragEventArgs.longitude;
        } else {
          markerObject.dataSource[marker.dataIndex].latitude =
            dragEventArgs.latitude;
          markerObject.dataSource[marker.dataIndex].longitude =
        ...
        markerObject.dataSource[marker.dataIndex][
          markerObject.latitudeValuePath
        ] = dragEventArgs.latitude;
        markerObject.dataSource[marker.dataIndex][
          markerObject.longitudeValuePath
        ] = dragEventArgs.longitude;
      }
      this.markerDragId = '';
    ...
    if (this.zoomModule && this.isDevice) {
      this.zoomModule.removeToolbarOpacity(
        this.isTileMap ? Math.round(this.tileZoomLevel) : this.scale,
        this.element.id + '_Zooming_'
      );
      this.zoomModule.removeToolbarClass('', '', '', '', '');
    }
  ...
  public mouseDownOnMap(e: PointerEvent): void {
    ...
    if (e.type.indexOf('touch') !== -1 && (e as any).changedTouches) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      this.mouseDownEvent = {
        x: (e as any).changedTouches[0].pageX,
        y: (e as any).changedTouches[0].pageY,
      };
    }
    ...
    if (element.id.indexOf('_ToolBar') === -1) {
      ...
      if (
        element.id.indexOf('shapeIndex') > -1 ||
        element.id.indexOf('Tile') > -1
      ) {
        this.mergeCluster();
      }
      if (markerModule) {
  ...
  public mergeCluster(): void {
    if (
      this.markerModule &&
      this.markerModule.sameMarkerData.length > 0 &&
  ...
  public mapsOnDoubleClick(e: PointerEvent): void {
    ...
    if (targetElement.id.indexOf('_ToolBar') === -1) {
      if (
        targetElement.id.indexOf('_LayerIndex_') !== -1 &&
        !this.isTileMap &&
        this.mouseDownEvent['x'] === e.clientX &&
        this.mouseDownEvent['y'] === e.clientY
      ) {
        layerIndex = parseFloat(
          targetElement.id.split('_LayerIndex_')[1].split('_')[0]
        );
        latLongValue = this.getGeoLocation(
          layerIndex,
          e['layerX'],
          e['layerY']
        );
  ...
  public mouseMoveOnMap(e: PointerEvent): void {
    ...
    target =
      e.type === 'touchmove'
        ? <Element>(<TouchEvent & PointerEvent>e).target
  ...
  private legendTooltip(
    ...
    let page: number = this.legendModule.currentPage;
    let legendIndex: string = (<HTMLElement>event.target).id.split(
      '_Index_'
    )[1];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let collection: any;
    page =
      this.legendModule.totalPages.length <= this.legendModule.currentPage
        ? this.legendModule.totalPages.length - 1
        : this.legendModule.currentPage < 0
        ? 0
        : this.legendModule.currentPage;
    const count: number =
      this.legendModule.totalPages.length !== 0
        ? this.legendModule.totalPages[page as number]['Collection'].length
        : this.legendModule.totalPages.length;
    for (let i: number = 0; i < count; i++) {
      collection =
        this.legendModule.totalPages[page as number]['Collection'][i as number];
      legendText = collection['DisplayText'];
      targetId = event.target['id'];
      legendIndex = event.target['id'].split('_Index_')[1];
      if (
        targetId === this.element.id + '_Legend_Text_Index_' + legendIndex &&
        (<HTMLElement>event.target).textContent.indexOf('...') > -1 &&
        collection['idIndex'] === parseInt(legendIndex, 10)
      ) {
        showTooltip(
          legendText,
          this.legendSettings.textStyle.size,
          x,
          y,
          this.element.offsetWidth,
          this.element.offsetHeight,
          this.element.id + '_EJ2_Legend_Text_Tooltip',
  ...
  private titleTooltip(
    ...
    if (
      targetId === this.element.id + '_LegendTitle' &&
      (<HTMLElement>event.target).textContent.indexOf('...') > -1
    ) {
      showTooltip(
        this.legendSettings.title.text,
        this.legendSettings.titleStyle.size,
        x,
        y,
        this.element.offsetWidth,
        this.element.offsetHeight,
        this.element.id + '_EJ2_LegendTitle_Tooltip',
        getElement(this.element.id + '_Secondary_Element'),
        isTouch
      );
    } else {
    ...
    if (
      targetId === this.element.id + '_Map_title' &&
      (<HTMLElement>event.target).textContent.indexOf('...') > -1
    ) {
      showTooltip(
        this.titleSettings.text,
        this.titleSettings.textStyle.size || this.themeStyle.titleFontSize,
        x,
        y,
        this.element.offsetWidth,
        this.element.offsetHeight,
        this.element.id + '_EJ2_Title_Tooltip',
        getElement(this.element.id + '_Secondary_Element'),
        isTouch
      );
    } else {
    ...
    if (!this.isDestroyed && !this.isExportInitialTileMap) {
      ...
      if (!args.cancel) {
        ...
        if (
          ...
          this.element.classList.contains('e-maps')
        ) {
    ...
    if (!this.isDestroyed) {
      ...
      if (!this.isTileMap && this.zoomModule) {
        if (!isNullOrUndefined(centerPosition)) {
          ...
          isRefresh =
            this.centerPosition.latitude === centerPosition.latitude &&
            this.centerPosition.longitude === centerPosition.longitude
              ? true
              : isRefresh;
   ...
   * @returns {void}
   */
  public panByDirection(
    direction: PanDirection,
    mouseLocation?: PointerEvent | TouchEvent
  ): void {
    if (!this.isDestroyed) {
      let xDiff: number = 0;
      let yDiff: number = 0;
  ...
  public addMarker(
    ...
    if (!this.isDestroyed && !isNullOrUndefined(this.markerModule)) {
      const layerEle: Element = document.getElementById(
        this.element.id + '_LayerIndex_' + layerIndex
      );
      if (markerCollection.length > 0 && layerEle) {
        for (const newMarker of markerCollection) {
          this.layersCollection[layerIndex as number].markerSettings.push(
            new MarkerSettings(this, 'markerSettings', newMarker)
          );
        }
        this.markerModule.markerRender(
          this,
          layerEle,
          layerIndex,
          this.mapLayerPanel['currentFactor'],
  ...
  public shapeSelection(
    ...
    if (!this.isDestroyed) {
      ...
      const popertyNameArray: string[] = Array.isArray(propertyName)
        ? propertyName
        : Array(propertyName);
      ...
      if (this.layers[layerIndex as number].type === 'SubLayer') {
        for (let i: number = 0; i < this.layersCollection.length; i++) {
          if (
            this.layersCollection[i as number].shapeData ===
            this.layers[layerIndex as number].shapeData
          ) {
            subLayerIndex = i;
            break;
          }
      ...
      if (selectionsettings.enable) {
        ...
        for (let i: number = 0; i < shapeData.length; i++) {
          for (
            let j: number = 0;
            j < (<string[]>popertyNameArray).length;
            j++
          ) {
            const propertyName: string =
              !isNullOrUndefined(
                shapeData[i as number]['properties'][
                  popertyNameArray[j as number]
                ]
              ) &&
              isNaN(
                shapeData[i as number]['properties'][
            ...
            const shapeName: string =
              !isNullOrUndefined(name) && typeof name === 'string'
                ? name.toLowerCase()
                : name;
            let k: number;
            if (propertyName === shapeName) {
              if (
                !isNullOrUndefined(
                  this.layers[layerIndex as number].shapeSettings.colorValuePath
                )
              ) {
                k = checkShapeDataFields(
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  <any[]>this.layers[layerIndex as number].dataSource,
                  shapeData[i as number]['properties'],
                  this.layers[layerIndex as number].shapeDataPath,
                  this.layers[layerIndex as number].shapePropertyPath,
                  this.layers[layerIndex as number]
                );
              }
              const baseLayer: LayerSettings = <LayerSettings>(
                this.layers[layerIndex as number]
              );
              if (this.baseLayerIndex >= 0 && baseLayer.isBaseLayer) {
                indexValue = 0;
              } else if (
                this.layers[layerIndex as number].type === 'SubLayer'
              ) {
              ...
              shapeIndex = parseInt(
                targetEle.id.split('_shapeIndex_')[1].split('_')[0],
                10
              );
              shapeDataValue =
                this.layers[layerIndex as number].shapeData['features'][
                  ...
                  : null;
              dataIndex = parseInt(
                targetEle.id.split('_dataIndex_')[1].split('_')[0],
                10
              );
              data = isNullOrUndefined(dataIndex)
                ? null
                : this.layers[layerIndex as number].dataSource[
                    dataIndex as number
                  ];
              if (enable) {
                triggerItemSelectionEvent(
                ...
                if (
                  ...
                  targetEle.id.indexOf('_MarkerIndex_') === -1
                ) {
                ...
                const shapeToggled: boolean = this.legendSettings.visible
                  ? this.legendModule.shapeToggled
                  : true;
                if (shapeToggled) {
                  targetEle.setAttribute('class', 'ShapeselectionMapStyle');
                  if (
                    this.selectedElementId.indexOf(
                      targetEle.getAttribute('id')
                    ) === -1
                  ) {
                ...
                this.legendSelection =
                  !selectionsettings.enableMultiSelect && !this.legendSelection
                    ? true
                    : this.legendSelection;
                if (
                  this.legendSettings.visible &&
                ...
                if (shapeToggled) {
                  removeClass(targetEle);
                  const selectedElementIdIndex: number =
                    this.selectedElementId.indexOf(
                      targetEle.getAttribute('id')
                    );
  ...
  public zoomToCoordinates(
    ...
    if (!this.isDestroyed) {
      ...
      if (
        (isNullOrUndefined(maxLatitude) && isNullOrUndefined(maxLongitude)) ||
        ...
        maxLatitude = isNullOrUndefined(maxLatitude)
          ? minLatitude
          : maxLatitude;
        maxLongitude = isNullOrUndefined(maxLongitude)
          ? minLongitude
          : maxLongitude;
      ...
      this.scaleOfGivenLocation = calculateZoomLevel(
        minLatitude,
        maxLatitude,
        minLongitude,
        maxLongitude,
      ...
      const zoomArgs: IMapZoomEventArgs = {
        ...
        scale: !this.isTileMap
          ? { previous: this.scale, current: this.scaleOfGivenLocation }
          : {
  ...
  private removeShapeSelection(): void {
    ...
    if (this.legendSettings.visible) {
      const legendSelectedElements: number =
        this.legendSelectionCollection.length;
      for (let i: number = 0; i < legendSelectedElements; i++) {
        removeClass(
          getElementByID(
            this.legendSelectionCollection[i as number]['legendElement']['id']
          )
        );
    ...
    if (!this.isDestroyed) {
        ...
        switch (prop) {
          ...
          case 'zoomSettings':
            if (!isNullOrUndefined(oldProp.zoomSettings)) {
              if (
                newProp.zoomSettings.zoomFactor !==
                  oldProp.zoomSettings.zoomFactor &&
                !isLayer
              ) {
                render = false;
              } else if (
                newProp.zoomSettings.shouldZoomInitially !==
                oldProp.zoomSettings.shouldZoomInitially
              ) {
                this.zoomSettings.zoomFactor = 1;
                this.previousProjection = null;
                this.scale = this.isMarkerZoomCompleted ? null : this.scale;
                this.isMarkerZoomCompleted =
                  !newProp.zoomSettings.shouldZoomInitially;
                render = true;
              } else if (
                newProp.zoomSettings.enable !== oldProp.zoomSettings.enable
              ) {
                this.zoomSettings.zoomFactor = 1;
                render = true;
              } else {
      ...
      if (render) {
        if (newProp.layers && isMarker) {
          ...
          if (this.isTileMap) {
            this.mapLayerPanel.renderTileLayer(
              this.mapLayerPanel,
              this.layers['currentFactor'],
  ...
  public requiredModules(): ModuleDeclaration[] {
    ...
    if (
      this.zoomSettings.enable ||
      this.zoomSettings.zoomFactor > this.zoomSettings.minZoom
    ) {
      modules.push({
        member: 'Zoom',
        args: [this],
    ...
    if (!this.isDestroyed) {
      ...
      if (type !== 'PDF' && this.allowImageExport && this.imageExportModule) {
        ...
        return new Promise((resolve: any) => {
          resolve(
            this.imageExportModule.export(this, type, fileName, allowDownload)
          );
        });
  ...
  private findVisibleLayers(
    ...
    for (const layer of layers) {
      ...
      if (layer.visible) {
        ...
        const navigationLine: NavigationLineSettingsModel[] =
          layer.navigationLineSettings;
        for (const navigation of navigationLine) {
          if (navigation.visible) {
            isSelection =
              (!isNullOrUndefined(navigation.highlightSettings) &&
                navigation.highlightSettings.enable) ||
              isSelection;
            isHighlight =
              (!isNullOrUndefined(navigation.selectionSettings) &&
                navigation.selectionSettings.enable) ||
              isHighlight;
          }
        }
        for (const polygon of polygonSetting.polygons) {
          if (!isNullOrUndefined(polygon.points) && polygon.points.length > 0) {
            isSelection =
              layer.polygonSettings.highlightSettings.enable || isSelection;
            isHighlight =
              layer.polygonSettings.selectionSettings.enable || isHighlight;
            istooltipVisible =
              layer.polygonSettings.tooltipSettings.visible || istooltipVisible;
          }
        }
        for (const marker of markers) {
          if (marker.visible) {
            istooltipVisible =
              marker.tooltipSettings.visible || istooltipVisible;
        ...
        for (const bubble of bubbles) {
          if (bubble.visible) {
            istooltipVisible =
              bubble.tooltipSettings.visible || istooltipVisible;
  ...
  public getGeoLocation(layerIndex: number, x: number, y: number): GeoPosition {
    ...
    if (!this.isDestroyed) {
      ...
      const elementClientRect: ClientRect =
        this.element.getBoundingClientRect();
      const pageX: number =
        x -
        container.offsetLeft -
        (elementClientRect.left - container.offsetLeft) -
        window.scrollX;
      const pageY: number =
        y -
        container.offsetTop -
        (elementClientRect.top - container.offsetTop) -
        window.scrollY;
      const currentLayer: LayerSettings = <LayerSettings>(
        this.layersCollection[layerIndex as number]
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      const mapSize: number =
        Math.min(this.mapAreaRect.height, this.mapAreaRect.width) *
        this.mapLayerPanel['currentFactor'] *
        this.scale;
      const xx: number =
        this.clip(pageX - translatePointX, 0, mapSize - 1) / mapSize - 0.5;
      const yy: number =
        0.5 - this.clip(pageY - translatePointY, 0, mapSize - 1) / mapSize;
  ...
  public pointToLatLong(pageX: number, pageY: number): Object {
    ...
    if (!this.isDestroyed && !isNullOrUndefined(this.translatePoint)) {
      ...
      pageY = !isNullOrUndefined(this.markerDragArgument)
        ? pageY + padding
        : pageY;
      const mapSize: number = 256 * Math.pow(2, this.tileZoomLevel);
      const x1: number =
        this.clip(pageX - this.translatePoint.x * this.scale, 0, mapSize - 1) /
          mapSize -
        0.5;
      const y1: number =
        0.5 -
        this.clip(pageY - this.translatePoint.y * this.scale, 0, mapSize - 1) /
          mapSize;
