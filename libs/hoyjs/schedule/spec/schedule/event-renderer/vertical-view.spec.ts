...
import { MultiSelect } from '@hoyui/dropdowns';
import {
  Schedule,
  Day,
  Week,
...
describe('Vertical View Event Render Module', () => {
  ...
  describe('Vertical view appointment rendering', () => {
    ...
    it('Checking appointment element', () => {
      const dayWrapper: HTMLElement[] = [].slice.call(
      ...
      const appointment: Element =
        dayWrapper[0].querySelector('.e-appointment');
  ...
  describe('Keyboard interactions with appointments', () => {
    ...
    it('shiftUp arrow key', (done: DoneFn) => {
      ...
      const appointment: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
      );
      ...
      expect(
        schObj.element.querySelector('.e-quick-popup-wrapper').classList
      ).toContain('e-popup-open');
      (
        schObj.element.querySelector(
          '.e-quick-popup-wrapper .e-subject'
        ) as HTMLInputElement
      ).value = 'shiftUpArrow Testing';
      keyModule.keyActionHandler({
        action: 'enter',
  ...
  describe('Quick Popup and Editor Window in RTL Mode', () => {
    ...
    it('event double click when RTL enabled', () => {
      ...
      const appElement: HTMLElement = schObj.element.querySelectorAll(
    ...
    it('cell double click when RTL enabled', () => {
      ...
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[130] as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[130] as HTMLElement,
  ...
  describe('Schedule vertical view appointment template checking', () => {
    ...
    const eventTemplate: string =
      '<div>Subject: ${Subject}</div><div>StartTime: ${StartTime.toLocaleString()}</div>' +
  ...
  describe('Checking vertical view appointment click actions', () => {
    ...
    it('Checking appointment click action', () => {
      ...
      const appointment: Element[] = [].slice.call(
        dayWrapper[0].querySelectorAll('.e-appointment')
      );
      util.triggerMouseEvent(appointment[0] as HTMLElement, 'click');
      expect(appointment[0].classList.contains('e-appointment-border')).toEqual(
        true
      );
      util.triggerMouseEvent(document.body, 'mousedown');
      const appointment1: Element[] = [].slice.call(
        dayWrapper[0].querySelectorAll('.e-appointment')
      );
      expect(
        appointment1[0].classList.contains('e-appointment-border')
      ).toEqual(false);
    });
  });
  ...
  describe('Vertical view all-day row appointment rendering', () => {
    ...
    it('Checking appointment element', () => {
      const allDayWrapper: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-all-day-appointment-wrapper')
      );
      expect(allDayWrapper[2].childElementCount).toBeGreaterThan(0);
      const appointment: Element =
        allDayWrapper[2].querySelector('.e-appointment');
  ...
  describe('Vertical view more that 24 hours appointment rendering with and without spannedEventPlacement property', () => {
    ...
    it('Checking more that 24 hours appointment element with spannedEventPlacement property', () => {
      ...
      const spannedEVent: HTMLElement = schObj.element.querySelector(
        '[data-id ="Appointment_2"]'
      );
      const eventDetails: Record<string, any> =
        schObj.getEventDetails(spannedEVent);
      expect(eventDetails.IsAllDay).toEqual(false);
      expect(spannedEVent.classList.contains('e-all-day-appointment')).toEqual(
        false
      );
  ...
  describe('Checking vertical view all-day row appointment click actions', () => {
    ...
    it('Checking appointment click action', () => {
      ...
      const appointment: Element[] = [].slice.call(
        allDayWrapper[2].querySelectorAll('.e-appointment')
      );
      util.triggerMouseEvent(appointment[0] as HTMLElement, 'click');
      expect(appointment[0].classList.contains('e-appointment-border')).toEqual(
        true
  ...
  describe('Checking vertical view all-day row appointment with work days', () => {
    ...
    it('Checking appointment element', () => {
      ...
      const appointment: Element =
        allDayWrapper[1].querySelector('.e-appointment');
      expect((<HTMLElement>appointment).offsetTop).toEqual(62);
    });
  });
  ...
  describe('Checking vertical view all-day row appointment with showWeekend', () => {
    ...
    it('Checking appointment element', () => {
      ...
      const appointment: Element =
        allDayWrapper[0].querySelector('.e-appointment');
      expect((<HTMLElement>appointment).offsetTop).toEqual(62);
    });
  });
  ...
  describe('Vertical view all-day row appointment expand/collapse rendering', () => {
    ...
    it('Checking appointment element', () => {
      const allDayCountWrapper: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-row-count-wrapper')
      );
      expect(allDayCountWrapper[0].childElementCount).toBeGreaterThan(0);
      const appointmentCount: Element =
        allDayCountWrapper[0].querySelector('.e-more-indicator');
      expect(
        (<HTMLElement>appointmentCount).getAttribute('data-count')
      ).toEqual('2');
    });
  });
  ...
  describe('Vertical view all-day row appointment expand/collapse icon click', () => {
    ...
    it('Checking appointment row expand', () => {
      const expandElement: HTMLElement = schObj.element.querySelector(
        '.e-all-day-appointment-section'
      ) as HTMLElement;
      expect(expandElement.classList.contains('e-appointment-expand')).toEqual(
        true
      );
      ...
      expect(allDayCountWrapper[0].childElementCount).toBeGreaterThan(0);
      expect(allDayCountWrapper[0].classList.contains('e-disable')).toEqual(
        true
      );
    ...
    it('Checking appointment collapse', () => {
      ...
      expect(
        expandElement.classList.contains('e-appointment-collapse')
      ).toEqual(true);
      util.triggerMouseEvent(expandElement, 'click');
      const allDayCountWrapper: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-row-count-wrapper')
  ...
  describe('Vertical view resource grouping appointment rendering', () => {
    ...
    it('Public method isSlotAvailable checking with resource', () => {
      ...
      expect(schObj.isSlotAvailable(startDate, endDate, resourceIndex)).toEqual(
        false
      );
    });
    ...
    it('CRUD add actions checking', (done: DoneFn) => {
      ...
      const recObj: RecurrenceEditor = (
        dialogElement.querySelector('.e-recurrenceeditor') as EJ2Instance
    ...
    it('CRUD delete actions checking', (done: DoneFn) => {
      ...
      expect(quickDialog.classList).toContain('e-popup-open');
      util.triggerMouseEvent(
        quickDialog.querySelector('.e-quick-dialog-delete'),
        'click'
      );
      expect(quickDialog.classList).toContain('e-popup-close');
    });
  ...
  describe('Vertical view resource grouping appointment rendering byDate', () => {
    ...
    it('EJ2-60623 - ScrollTo method checking when date alone is passed', (done: DoneFn) => {
      schObj.width = '600px';
      schObj.dataBind();
      expect(schObj.element.querySelector('.e-content-wrap').scrollLeft).toBe(
        0
      );
  ...
  describe('Vertical view resource grouping appointment rendering allowGroupEdit', () => {
    ...
    it('CRUD add actions checking', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element.querySelectorAll('[data-id ="Appointment_19"]').length
        ).toEqual(3);
        done();
      };
      const workCell: HTMLElement = schObj.element.querySelectorAll(
      ...
      const endTimeObj: DateTimePicker = (
      ...
      endTimeObj.dataBind();
      const resourceElements: NodeListOf<Element> =
        dialogElement.querySelectorAll('.e-multiselect.e-control');
      const roomObj: MultiSelect = (resourceElements[0] as EJ2Instance)
        .ej2_instances[0] as MultiSelect;
      roomObj.value = [1, 2];
      roomObj.dataBind();
      const ownerObj: MultiSelect = (resourceElements[1] as EJ2Instance)
        .ej2_instances[0] as MultiSelect;
    ...
    it('CRUD edit actions checking', (done: DoneFn) => {
      ...
      (dialogElement.querySelector('.e-subject') as HTMLInputElement).value =
        'Testing';
      (dialogElement.querySelector('.e-location') as HTMLInputElement).value =
        'Paris';
      (<HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS)
      )).click();
  ...
  describe('schedule`s (start/end)Hour and appointment`s (end/start)Time is same', () => {
    ...
    it('checking appointment presents after change start and endHour', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        const app1: HTMLElement = schObj.element.querySelector(
          '[data-id ="Appointment_2"]'
        ) as HTMLElement;
        expect(app1.querySelectorAll('.e-indicator').length).toEqual(2);
        expect(
          app1
            .querySelectorAll('.e-indicator')[0]
            .classList.contains('e-up-icon')
        ).toBe(true);
        expect(
          app1
            .querySelectorAll('.e-indicator')[1]
            .classList.contains('e-down-icon')
        ).toBe(true);
        const app2: HTMLElement = schObj.element.querySelector(
        ...
        const app3: HTMLElement = schObj.element.querySelector(
  ...
  describe('EJ2-12691 - Yearly recurrence appointments are not displaying properly', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        height: '550px',
        selectedDate: new Date(2022, 11, 5),
      };
      schObj = util.createSchedule(model, eventData, done);
    });
    afterAll(() => {
  ...
  describe('EJ2-47668 - Issue in rendering of scheduler event height based on the DST timezone', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        height: '550px',
        firstDayOfWeek: 1,
        selectedDate: new Date(2021, 2, 25),
      };
      schObj = util.createSchedule(model, eventData, done);
    });
    afterAll(() => {
    ...
    it('Checking dst event height', () => {
      const eventNormalElementList: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
      );
      expect(eventNormalElementList.length).toEqual(2);
      expect((eventNormalElementList[0] as HTMLElement).style.height).toBe(
        '936px'
      );
      expect((eventNormalElementList[1] as HTMLElement).style.height).toBe(
        '504px'
      );
    });
  });
  ...
  describe('Resize handler dimension checking', () => {
    ...
    it('Checking appointment resize handler height', () => {
      const appElement: HTMLElement =
        schObj.element.querySelector('.e-appointment');
      expect(
        (
  ...
  describe('Schedule week view appointment template checking', () => {
    ...
    it('elements in DOM', () => {
      const eventBlockedElementList: Element[] = [].slice.call(
  ...
  describe('allowInline property', () => {
    ...
    it('Checking allowInline property', (done: Function) => {
      schObj.dataBound = () => {
        ...
        expect(
          (eventElementList[6].querySelector('.e-subject') as HTMLElement)
            .innerHTML
        ).toBe('Testing');
        done();
      };
      ...
      workCell.click();
      expect(
        schObj.element.querySelector('.e-inline-appointment')
      ).toBeTruthy();
      const inputElement: HTMLInputElement = schObj.element.querySelector(
        '.e-inline-subject'
      ) as HTMLInputElement;
      ...
      keyModule.keyActionHandler({
        action: 'enter',
        target: schObj.element.querySelector('.e-inline-subject'),
      });
    });

    it('Checking allowInline edit action', (done: Function) => {
      schObj.dataBound = () => {
        const eventElement: Element = schObj.element.querySelector(
          '[data-id="Appointment_44"]'
        );
        expect(eventElement).toBeTruthy();
        expect(
          (eventElement.querySelector('.e-subject') as HTMLElement).innerHTML
        ).toBe('Testing - edited');
        done();
      };
      const eventElement: Element = schObj.element.querySelector(
      ...
      expect(eventElement).toBeTruthy();
      const subjectElement: HTMLElement = eventElement.querySelector(
        '.e-subject'
      ) as HTMLElement;
      subjectElement.click();
      const inputElement: HTMLInputElement = eventElement.querySelector(
        '.e-inline-subject'
      ) as HTMLInputElement;
      expect(inputElement.classList.contains('e-inline-subject')).toBeTruthy();
    ...
    it('Checking allowInline allDay event', (done: Function) => {
      schObj.dataBound = () => {
        ...
        expect(
          (eventElementList[0].querySelector('.e-subject') as HTMLElement)
            .innerHTML
        ).toBe('week');
        done();
      };
    ...
    it('Checking allowInline event in timeline week view - cell click', (done: Function) => {
      schObj.dataBound = () => {
        ...
        expect(
          (eventElementList[2].querySelector('.e-subject') as HTMLElement)
            .innerHTML
        ).toBe('timeline');
        done();
      };
    ...
    it('Checking allowInline event in timeline week view - event click', (done: Function) => {
      schObj.dataBound = () => {
        const eventElement: Element = schObj.element.querySelector(
          '[data-id="Appointment_46"]'
        );
        expect(eventElement).toBeTruthy();
        expect(
          (eventElement.querySelector('.e-subject') as HTMLElement).innerHTML
        ).toBe('timeline - edited');
        done();
      };
      const eventElement: Element = schObj.element.querySelector(
        '[data-id="Appointment_46"]'
  ...
  describe('allowInline property', () => {
    ...
    it('Checking allowInline edit action in Month view', (done: Function) => {
      schObj.dataBound = () => {
        ...
        expect(
          (eventElement.querySelector('.e-subject') as HTMLElement).innerHTML
        ).toBe('Month view testing edited');
        done();
      };
      const eventElement: Element = schObj.element.querySelector(
        '[data-id="Appointment_44"]'
    ...
    it('Checking more popup closing after cell click when inline edit enabled', () => {
      const moreEvent: HTMLElement = schObj.element.querySelector(
        '.e-more-indicator'
      ) as HTMLElement;
      moreEvent.click();
    ...
    it('Checking inline appointment wrapper width after pressing enter key on multiple day cell selection', () => {
      ...
      expect(inputElement.classList.contains('e-inline-subject')).toBeTruthy();
      inputElement.value = 'Month view testing edited';
      const appWrapper: HTMLElement = schObj.element.querySelector(
        '.e-appointment-wrapper .e-inline-appointment'
      ) as HTMLElement;
      expect(appWrapper.style.width).toBe(
        workCells[0].clientWidth * 6 - 5 + 'px'
      );
    });
  });
  ...
  describe('allowInline property with template without default subject field', () => {
    ...
    it('Checking allowInline property in Week view', () => {
      ...
      expect(
        (
          eventElement.querySelector('.e-appointment-details') as HTMLElement
  ...
  describe('allowInline property with template ', () => {
    ...
    let keyModule: any;
    const eventTemplate: string =
      '<div class = e-subject >Subject: ${Subject}</div><div>StartTime: ${StartTime.toLocaleString()}</div>' +
      '<div>EndTime: ${EndTime.toLocaleString()</div>';
    beforeAll((done: Function) => {
      const model: ScheduleModel = {
        currentView: 'Agenda',
    ...
    it('Checking allowInline property in Agenda view', (done: DoneFn) => {
      schObj.dataBound = () => {
        const eventElementList: HTMLElement[] = [].slice.call(
          schObj.element.querySelectorAll('.e-appointment')
        );
        expect(eventElementList[0].firstElementChild.innerHTML).toBe(
          'Subject: Testing'
        );
        done();
      };
      ...
      expect(eventElementList[0].firstElementChild.className).toBe(
        'e-subject-wrap'
      );
      const inLineEdited: HTMLInputElement = eventElementList[0]
        .firstElementChild.firstChild as HTMLInputElement;
    ...
    it('Checking allowInline property in TimelineWeek view', (done: Function) => {
      schObj.dataBound = () => {
        schObj.dataBound = () => {
          const eventElement: Element = schObj.element.querySelector(
            '[data-id="Appointment_11"]'
          );
          const inLineEdited: Element =
            eventElement.children[1].firstElementChild.firstElementChild;
        ...
        expect(
          inputElement.classList.contains('e-inline-subject')
        ).toBeTruthy();
        inputElement.value = 'timeline';
        const inLineEdited: HTMLInputElement = eventElement.children[1]
    ...
    it('Checking allowInline property in Week view', (done: Function) => {
      schObj.dataBound = () => {
        schObj.dataBound = () => {
          ...
          const inLineEdited: Element =
            eventElement.children[1].firstElementChild;
        ...
        eventElement.click();
        expect(eventElement.children[1].children[1].className).toBe(
          'e-subject e-disable'
        );
        const inputElement: HTMLInputElement = schObj.element.querySelector(
          '.e-inline-subject'
    ...
    it('Checking allowInline property in TimelineMonth view', (done: Function) => {
      schObj.dataBound = () => {
        schObj.dataBound = () => {
          ...
          const inLineEdited: HTMLElement = eventElement.children[1]
  ...
  describe('EJ2-51853 - Schedule events overlapping when same start and end time', () => {
    ...
    it('Checking events overlapping', () => {
      ...
      expect((eventNormalElementList[1] as HTMLElement).style.left).toBe(
        '31.3333%'
      );
      expect((eventNormalElementList[2] as HTMLElement).style.left).toBe(
        '62.6667%'
      );
  ...
  describe('EJ2-52479 - Appointment missing when showWeekend is false', () => {
    ...
    it('checking appointment rendering', () => {
      ...
      expect(eventList.querySelector('.e-subject').innerHTML).toBe(
        'Burning Man'
      );
    });
  });
  ...
  describe('EJ2-56503 - Vertical view event rendered arguments checking', () => {
    ...
    it('checking event rendered event args', (done: DoneFn) => {
      schObj.eventRendered = (args: EventRenderedArgs) => {
        expect(
          (args.data[schObj.eventFields.startTime] as Date).getTime()
        ).toEqual(new Date(2017, 10, 8, 10).getTime());
        expect(
          (args.data[schObj.eventFields.endTime] as Date).getTime()
        ).toEqual(new Date(2017, 10, 8, 12).getTime());
  ...
  describe('EJ2-65298 - Editing recurrence series to the greater than its end date causes an issue', () => {
    ...
    it('To check recurrence appointment after changing start date', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventsData.length).toEqual(1);
        expect(schObj.eventsData[0].StartTime).toEqual(
          new Date(2022, 10, 4, 10)
        );
        expect(schObj.eventsData[0].RecurrenceRule).toEqual(
          'FREQ=DAILY;INTERVAL=1;COUNT=5;'
        );
        expect(schObj.element.querySelectorAll('.e-appointment').length).toBe(
          2
        );
        done();
      };
      const appElement: HTMLElement = schObj.element.querySelector(
        '.e-appointment'
      ) as HTMLElement;
      util.triggerMouseEvent(appElement, 'click');
      util.triggerMouseEvent(appElement, 'dblclick');
      ...
      ).ej2_instances[0].value = new Date(2022, 10, 4, 10, 0);
      (
        dialogElement.querySelector('.e-end') as EJ2Instance
      ).ej2_instances[0].value = new Date(2022, 10, 4, 11, 30);
      const saveButton: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS)
      );
      saveButton.click();
  ...
  describe('EJ2-69228 - generateEventOccurrences public method test', () => {
    ...
    it('checking generateEventOccurrences method with parent event and startDate as arguments', () => {
      expect(
        schObj.generateEventOccurrences(data[1], new Date(2023, 1, 8)).length
      ).toEqual(18);
      expect(
        schObj
          .generateEventOccurrences(data[1], new Date(2023, 1, 8))[0]
          .StartTime.getTime()
      ).toEqual(new Date(2023, 1, 7, 10, 0).getTime());
      expect(
        schObj.generateEventOccurrences(data[1], new Date(2023, 1, 20)).length
      ).toEqual(6);
    });

    it('checking generateEventOccurrences method with parent event and wrong startDate as arguments', () => {
      expect(
        schObj.generateEventOccurrences(data[1], new Date(2023, 2, 3)).length
      ).toEqual(0);
    });
  });
  ...
  describe('EJ2-71317 - schedule virtual scrolling performance', () => {
    ...
    function generateResources(
      ...
      if (parentCount === 0) {
        ...
        for (let a: number = 1; a <= count; a++) {
          ...
          data.push({
            Id: a,
            ParentId: Math.ceil((a / count) * parentCount),
            Text: 'Resource ' + a,
            Color: colors[n],
          });
        }
    ...
    it('Performance checking with events rendering', (done: DoneFn) => {
      const data: Record<string, any>[] = generateEvents(
        200,
        50,
        30,
        new Date(2023, 0, 1)
      ...
      schObj.dataBound = () => {
        expect(
          Math.floor((performance.now() - performanceStart) / 1000)
  ...
  describe('ES-826853 - schedule all day events rendering performance', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        eventSettings: {
          ...
          template:
            '<div class="template-wrapper"><div class="subject">${ Subject }</div></div>',
        },
        group: {
          byDate: true,
          resources: ['Locations'],
    ...
    it('Performance checking with events rendering', (done: DoneFn) => {
      ...
      schObj.dataBound = () => {
        expect(
          (performance.now() - performanceStart) / 1000
        ).toBeLessThanOrEqual(1);
        done();
      };
    });
  ...
  describe('EJ2-876307 - Hide appointment wrappers until template rendering is completed', () => {
    ...
    it('Checking hidden appointment wraps at initial rendering', (done: DoneFn) => {
      schObj.dataBinding = () => {
        const hiddenWraps: HTMLElement[] = [].slice.call(
  ...
  describe('Checking the allowInline property in all timeline views within the inner-wrap element', () => {
    ...
    it('Checking allowInline property in TimelineDay view', (done: Function) => {
      schObj.dataBound = () => {
        ...
        eventElement.click();
        const innerWrapElement: HTMLElement =
          schObj.element.querySelector('.e-inner-wrap');
        expect(
          innerWrapElement.classList.contains('e-inner-wrap')
        ).toBeTruthy();
        const inputElement: HTMLInputElement = schObj.element.querySelector(
          '.e-inline-subject'
        ) as HTMLInputElement;
    ...
    it('Checking allowInline property in Week view', (done: Function) => {
      ...
      eventElement.click();
      const appointmentElement: HTMLElement = schObj.element.querySelector(
        '.e-appointment-details'
      );
      expect(
        appointmentElement.classList.contains('e-appointment-details')
      ).toBeTruthy();
      const inputElement: HTMLInputElement = schObj.element.querySelector(
        '.e-inline-subject'
      ) as HTMLInputElement;
