 ...
 */
import { closest, Browser, Internationalization } from '@hoyui/base';
import {
  Schedule,
  ScheduleModel,
  Day,
...
import { DropDownList, MultiSelect } from '@hoyui/dropdowns';
...
describe('Month Event Render Module', () => {
  ...
  describe('Schedule Month view appointment rendering', () => {
    ...
    it('elements in DOM', () => {
      ...
      expect(
        (closest(eventElementList[0], '.e-work-cells') as HTMLTableCellElement)
          .cellIndex
      ).toEqual(3);
  ...
  describe('EJ2-54280 Scroll position goes back to the normal in the TimelineMonth, while dragging/resizing an event', () => {
    ...
    it('right resizing', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(
          schObj.element.querySelectorAll('.e-resize-clone').length
        ).toEqual(0);
        expect(
          (<HTMLElement>(
        ...
        expect(
          schObj.element.querySelector('.e-content-wrap').scrollLeft
        ).toEqual(0);
        done();
      };
      schObj.element.querySelector('.e-content-wrap').scrollLeft = 0;
      const resizeElement: HTMLElement = schObj.element.querySelector(
        '[data-id="Appointment_1"]'
      ) as HTMLElement;
      expect(resizeElement.offsetWidth).toEqual(68);
      const resizeHandler: HTMLElement = resizeElement.querySelector(
        '.e-right-handler'
      ) as HTMLElement;
      triggerMouseEvent(resizeHandler, 'mousedown');
      triggerMouseEvent(resizeHandler, 'mousemove', 25, 0);
      const cloneElement: HTMLElement = schObj.element.querySelector(
        '.e-resize-clone'
      ) as HTMLElement;
  ...
  describe('Mobile view', () => {
    ...
    beforeAll((done: DoneFn) => {
      ...
      const model: ScheduleModel = {
        views: [
          { option: 'Day', readonly: true },
          { option: 'Week' },
          { option: 'WorkWeek', readonly: true },
          { option: 'Month' },
        ],
        currentView: 'Month',
        height: '550px',
    ...
    it('More event click testing and read only for add icon testing', () => {
      expect(
        (schObj.element.querySelector('.e-icon-add') as HTMLElement)
          .offsetHeight
      ).toEqual(25);
      const eventElementList: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
  ...
  describe('Schedule month view appointment template checking', () => {
    ...
    it('elements in DOM', () => {
      ...
      expect(
        ...
        '<span>Spanned - Greater than 24 hour</span><div class="e-indicator e-icons e-right-icon"></div>'
      );
    });
    it('change event template through setmodel', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          ...
          '<span class="event-template">Spanned - Greater than 24 hour</span>' +
            '<div class="e-indicator e-icons e-right-icon"></div>'
        );
        done();
      };
      schObj.eventSettings.template =
        '<span class="event-template">${Subject}</span>';
      schObj.dataBind();
    });
  });
  ...
  describe('EJ2-11284 Events start and end on same time', () => {
    ...
    it('appointment element present in DOM', () => {
      ...
      expect(
        schObj.element
          .querySelectorAll('.e-content-wrap .e-appointment')[0]
          .getAttribute('aria-label')
      ).toEqual(
        'Normal event Begin From Thursday, November 2, 2017 at 10:00:00 AM GMT Ends At Thursday, November 2, 2017 at 10:00:00 AM GMT Location Chennai'
      );
      expect(
        schObj.element
          .querySelectorAll('.e-content-wrap .e-appointment')[1]
          .getAttribute('aria-label')
      ).toEqual(
        'Recurrence event Begin From Friday, November 3, 2017 at 10:00:00 AM GMT Ends At Friday, November 3, 2017 at 10:00:00 AM GMT Location Madurai Recurring Event'
      );
      expect(
        schObj.element
          .querySelectorAll('.e-content-wrap .e-appointment')[2]
          .getAttribute('aria-label')
      ).toEqual(
        'Recurrence event Begin From Saturday, November 4, 2017 at 10:00:00 AM GMT Ends At Saturday, November 4, 2017 at 10:00:00 AM GMT Location Madurai Recurring Event'
      );
      expect(
        schObj.element
          .querySelectorAll('.e-content-wrap .e-appointment')[3]
          .getAttribute('aria-label')
      ).toEqual(
        'Recurrence event Begin From Sunday, November 5, 2017 at 10:00:00 AM GMT Ends At Sunday, November 5, 2017 at 10:00:00 AM GMT Location Madurai Recurring Event'
      );
      expect(
        schObj.element
          .querySelectorAll('.e-content-wrap .e-appointment')[4]
          .getAttribute('aria-label')
      ).toEqual(
        'Recurrence event Begin From Monday, November 6, 2017 at 10:00:00 AM GMT Ends At Monday, November 6, 2017 at 10:00:00 AM GMT Location Madurai Recurring Event'
      );
      expect(
        schObj.element
          .querySelectorAll('.e-content-wrap .e-appointment')[5]
          .getAttribute('aria-label')
      ).toEqual(
        'Recurrence event Begin From Tuesday, November 7, 2017 at 10:00:00 AM GMT Ends At Tuesday, November 7, 2017 at 10:00:00 AM GMT Location Madurai Recurring Event'
      );
    });
  });
  ...
  describe('Start and end time in event template', () => {
    ...
    beforeAll((done: DoneFn) => {
      ...
      (window as TemplateFunction).getTimeString = (value: Date) =>
        instance.formatDate(value, { skeleton: 'hm' });
      ...
      const model: ScheduleModel = {
        ...
        selectedDate: new Date(2017, 10, 2),
        eventSettings: {
          template: '<div class="time">${getTimeString(data.StartTime)}</div>',
        },
      };
    ...
    it('check event start time in template', () => {
      expect(
        schObj.element.querySelector('.e-appointment .time').innerHTML
      ).toEqual('10:00 AM');
    });
  });
  ...
  describe('Month view layout checking when enable both showWeekNumber and rowAutoHeight properties on same time', () => {
    ...
    it('checking week number cell and work cell height ', () => {
      expect(
        (schObj.element.querySelector('.e-week-number') as HTMLElement).style
          .height
      ).toEqual('110px');
      expect(
        (
  ...
  describe('EJ2-57751 - Month view more indicator count is wrong', () => {
    ...
    it('checking more indicator count values', () => {
      const moreIndicators: NodeListOf<Element> =
        schObj.element.querySelectorAll('.e-more-indicator');
      ...
      expect(moreIndicators[0].getAttribute('aria-label')).toEqual(
        '2 More Events'
      );
      ...
      expect(moreIndicators[4].getAttribute('aria-label')).toEqual(
        '3 More Events'
      );
    ...
    it('Check the recurrence event occurrences count', async (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventsData[0].RecurrenceRule).toEqual(
          'FREQ=WEEKLY;BYDAY=MO,TU;INTERVAL=1;UNTIL=20220308T000000Z;'
        );
        expect(
          schObj.element.querySelectorAll('.e-appointment').length
        ).toEqual(2);
      ...
      const editor: HTMLElement = document.querySelector(
        '.e-schedule-dialog.e-popup-open'
      );
      util.triggerMouseEvent(
        editor.querySelector('.e-all-day.e-field') as HTMLElement,
        'click'
      );
      const repeatElement: DropDownList = (
        editor.querySelector('.e-repeat-element') as EJ2Instance
      ).ej2_instances[0] as DropDownList;
      repeatElement.index = 2;
      repeatElement.dataBind();
      const endOnElement: DropDownList = (
        editor.querySelector('.e-end-on-element') as EJ2Instance
      ).ej2_instances[0] as DropDownList;
      endOnElement.index = 1;
      endOnElement.dataBind();
      const recEditor: RecurrenceEditor = (
        editor.querySelector('.e-recurrenceeditor') as EJ2Instance
      ).ej2_instances[0] as RecurrenceEditor;
      expect(recEditor.getRecurrenceRule()).toEqual(
        'FREQ=WEEKLY;BYDAY=MO;INTERVAL=1;UNTIL=20220506T090000Z;'
      );
      util.triggerMouseEvent(
        editor.querySelector('.e-end-on-date .e-clear-icon'),
        'mousedown'
      );
      const dp: DatePicker = (
        editor.querySelector('.e-until-date') as EJ2Instance
      ).ej2_instances[0] as DatePicker;
      dp.value = new Date(2022, 2, 1);
      async function openUntil(done: DoneFn): Promise<void> {
        util.triggerMouseEvent(
          editor.querySelector('.e-end-on-date .e-date-icon'),
          'mousedown'
        );
        done();
      }
      ...
      util.triggerMouseEvent(
        document.querySelectorAll('.e-calendar .e-cell')[9] as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        editor.querySelector('.e-days [data-index="2"]') as HTMLElement,
        'click'
      );
      expect(recEditor.getRecurrenceRule()).toEqual(
        'FREQ=WEEKLY;BYDAY=MO,TU;INTERVAL=1;UNTIL=20220308T000000Z;'
      ...
      util.triggerMouseEvent(
        editor.querySelector('.e-event-save') as HTMLElement,
        'click'
      );
    });
  });
  ...
  describe('Multiple resource grouping rendering in adaptive mode', () => {
    ...
    it('testing for adaptive grouping sample saving event in multiple resource', (done: DoneFn) => {
      ...
      const workCell: HTMLTableCellElement =
      ...
      const saveButton: HTMLElement = editor.querySelector(
        '.e-event-save'
      ) as HTMLElement;
      saveButton.click();
    });
    ...
    it('testing for adaptive grouping sample saving event in different resource', (done: DoneFn) => {
      ...
      const workCell: HTMLTableCellElement = schObj.element.querySelectorAll(
        '.e-work-cells'
      )[1] as HTMLTableCellElement;
      util.triggerMouseEvent(workCell, 'click');
      util.triggerMouseEvent(workCell, 'dblclick');
      const editor: HTMLElement = document.querySelector(
        '.e-schedule-dialog.e-popup-open'
