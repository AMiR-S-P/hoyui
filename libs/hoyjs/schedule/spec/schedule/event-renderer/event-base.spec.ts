...
describe('Event Base Module', () => {
  ...
  describe('split event by day method testing', () => {
    ...
    it('Event on same day', () => {
      ...
      const spannedEvents: Record<string, any>[] =
        schObj.eventBase.splitEventByDay(event);
  ...
  describe('checking spanned recurrence appointment', () => {
    ...
    const eventData: Record<string, any>[] = [
      {
        ...
        RecurrenceRule:
          'FREQ=WEEKLY;BYDAY=SU,MO,TU,WE,TH,FR,SA;INTERVAL=1;UNTIL=20180905T070000Z;',
      },
    ];
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        height: '550px',
        selectedDate: new Date(2018, 7, 7),
      };
    ...
    it('spanned recurrence appointments with offsetTop', (done: DoneFn) => {
      schObj.dataBound = () => {
        const app2: HTMLElement[] = [].slice.call(
  ...
  describe('Schedule Timezone testing', () => {
    ...
    it('Set timezone to Schedule', () => {
      ...
      expect((event.StartTime as Date).getTime()).toEqual(
        new Date(2018, 5, 14, 11, 0).getTime()
      );
    });
    it('Convert timezone', () => {
      ...
      const event: Record<string, any> = schObj.eventsData[0];
      expect((event.StartTime as Date).getTime()).toEqual(
        new Date(2018, 5, 14, 20, 30).getTime()
      );
    });
  ...
  describe('checking recurrence appointment', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        height: '550px',
        selectedDate: new Date(2018, 9, 1),
        views: [
  ...
  describe('Checking events fill the full height of the cell', () => {
    ...
    const eventData: Record<string, any>[] = [
      {
        ...
        RecurrenceRule:
          'FREQ=DAILY;BYDAY=SU,MO,TU,WE,TH,FR,SA;INTERVAL=1;COUNT=6;',
      },
      {
        Id: 2,
        StartTime: new Date(2018, 8, 30, 0, 0),
    ...
    it('Checking event filled full height of the cell in the week view', () => {
      const cellElement: HTMLElement = schObj.element.querySelector(
        '.e-work-cells'
      ) as HTMLElement;
  ...
  describe('Checking custom sorting functionality', () => {
        ...
        eventSettings: {
          sortComparer: (args: any) => {
            args.sort((a: any, b: any) =>
              a.RankId.localeCompare(b.RankId, undefined, { numeric: true })
            );
            return args;
          },
        },
    ...
    it('Checking event sorting by custom field in Month View', () => {
      ...
      const eventDetails: Record<string, any> =
        schObj.getEventDetails(eventElement);
  ...
  describe('Checking events fill the full height with more indicator of the cell', () => {
    ...
    it('Checking event filled full height of the cell in the Month view', () => {
      ...
      expect(eventElement.offsetHeight).toEqual(
        cellElement.offsetHeight - 19 - headerHeight
      );
    });
  ...
  describe('checking focus on element outside schedule', () => {
    ...
    it('To check outside element focus after focussing work cell', () => {
      const workCell: HTMLTableCellElement = schObj.element.querySelectorAll(
        '.e-work-cells'
      )[170] as HTMLTableCellElement;
      workCell.click();
      keyModule.keyActionHandler({ action: 'escape' });
      const inputEle: HTMLElement = document.querySelector(
        '.e-input-element'
      ) as HTMLElement;
      inputEle.focus();
    ...
    it('To check outside element focus after focussing appointment', () => {
      const appointmentEle: HTMLTableCellElement =
  ...
  describe('checking Recurrence appointments', () => {
    ...
    it('CR issue - EJ2-60887 - To check appointment rendering in week view', () => {
      ...
      const occurrence: Record<string, any>[] = schObj.getEvents(
        null,
        null,
        true
      );
  ...
  describe('checking occurrences in dst time zone', () => {
    ...
    beforeAll((done: DoneFn) => {
      const options: ScheduleModel = {
        height: '550px',
        width: '100%',
        selectedDate: new Date(2022, 2, 20),
        timezone: 'UTC',
        currentView: 'Month',
      };
      schObj = util.createSchedule(options, eventData, done);
  ...
  describe('Check appointment rendering with timezone', () => {
    ...
    it('CR issue - EJ2-61857 - On Schedule property change', () => {
      ...
      expect(app.length).toBe(1);
      expect(app[0].querySelector('.e-time').innerHTML).toEqual(
        '2:00 AM - 5:00 AM'
      );
    });
  ...
  describe('Check Agenda view appointment focus', () => {
    ...
    it('check scrollbar after open/close quick popup', () => {
      const appEle: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
      );
      appEle[1].click();
      expect(
      ...
      expect(
        Math.floor(schObj.element.querySelector('.e-content-wrap').scrollTop)
      ).toBe(1);
      (
        schObj.element.querySelector(
  ...
  describe('ES-835930 - Checking the work cell Selection', () => {
    ...
    it('Checking the scroll position on occurrence Event save', (done: DoneFn) => {
      ...
      const quickDialog: HTMLElement =
        document.querySelector('.e-quick-dialog');
      (
      ...
      const dialogElement: HTMLElement =
        document.querySelector('.e-schedule-dialog');
      const start = (dialogElement.querySelector('.e-start') as EJ2Instance)
        .ej2_instances[0];
      start.value = new Date(2017, 9, 31, 10, 30, 0);
      start.dataBind();
      const end = (dialogElement.querySelector('.e-end') as EJ2Instance)
        .ej2_instances[0];
    ...
    it('Checking the cell Selection on deleting the event', (done: DoneFn) => {
      ...
      const quickDialog: HTMLElement =
        document.querySelector('.e-quick-dialog');
      (
        quickDialog.querySelector('.e-quick-dialog-delete') as HTMLElement
      ).click();
    });
  });
  ...
  describe('ES-843662 - Checking the Recurrence exception with timezone', () => {
    ...
    it('Checking the recurrence exception on initial loading', () => {
      ...
      expect(
        (app[0].querySelector('.e-time') as HTMLElement).innerText
  ...
  describe('ES-847447 - Checking event double click event', () => {
    ...
    it('Checking the event double click args', () => {
      schObj.eventDoubleClick = (args: EventClickArgs) => {
        ...
        expect((args.event as Record<string, any>).Subject).toEqual(
      ...
      expect(
        dialogElement.firstElementChild.classList.contains('e-popup-open')
      ).toEqual(true);
      const closeIcon: HTMLElement = dialogElement.querySelector(
        '.e-dlg-closeicon-btn'
      ) as HTMLElement;
      closeIcon.click();
    });
    it('Preventing opening of the editor window by setting up args.cancel value as true', () => {
      ...
      expect(
        dialogElement.firstElementChild.classList.contains('e-popup-close')
      ).toEqual(true);
    });
  });
  ...
  describe('ES-870037 - Checking the appointments sorting and overflow', () => {
    ...
    const generateStaticEvents = (start: Date): Record<string, any>[] => {
      ...
      var endDate = new Date(
        startDate.getTime() + (1440 + 30) * (1000 * 60) * 2
      );
      data.push({
        Id: 1,
        Subject: 'Event #' + 1,
    ...
    beforeAll((done: DoneFn) => {
      ...
      schObj = util.createSchedule(
        model,
        generateStaticEvents(new Date(2024, 3, 2)),
        done
      );
    });
    afterAll(() => {
