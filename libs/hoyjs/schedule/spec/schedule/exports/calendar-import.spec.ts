...
describe('ICS calendar import', () => {
  ...
  describe('Import checking', () => {
    ...
    it('Import checking with EJ2 scheduler exported file', (done: DoneFn) => {
      ...
      const fileObj: File = new File([iCalString], 'EJSchedule.ics', {
        lastModified: 0,
        type: 'text/calendar',
      });
  ...
  describe('Import checking in description with multi lines', () => {
    ...
    it('Import checking with description parsing', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventsData.length).toEqual(3);
        expect(schObj.eventsData[2].Description).toEqual(
      ...
      const fileObj: File = new File([iCalendarImportStr], 'EJSchedule.ics', {
        lastModified: 0,
        type: 'text/calendar',
      });
      schObj.importICalendar(fileObj);
  ...
  describe('Import checking with readonly events', () => {
    ...
    it('Import checking with readonly event', () => {
      schObj.dataBound = () => {
        expect(schObj.eventsData.length).toEqual(16);
        const events: HTMLElement[] = [].slice.call(
  ...
  describe('EJ2-51402 - Issue with importing recurrence events', () => {
    ...
    beforeAll((done: DoneFn) => {
      ...
      const options: ScheduleModel = {
        selectedDate: new Date(2017, 1, 7),
        currentView: 'Month',
      };
      schObj = createSchedule(options, events, done);
    });
    ...
    it('Import checking with EJ2 scheduler exported file', (done: DoneFn) => {
      ...
      expect(schObj.eventsData.length).toEqual(3);
      const fileObj: File = new File([icalStr], 'EJSchedule.ics', {
        lastModified: 0,
        type: 'text/calendar',
      });
      schObj.importICalendar(fileObj);
    ...
    it('Import checking with readonly event', () => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element
            .querySelectorAll('.e-appointment')[1]
    ...
    it('EJ2-64169 - Update recurrence entire series after importing an recurrence event with recurrence exception', () => {
      schObj.dataBound = () => {
        expect(
          schObj.element
            .querySelectorAll('.e-appointment')[3]
            .querySelector('.' + cls.SUBJECT_CLASS).innerHTML
        ).toEqual('My_event');
        expect(
      ...
      expect(typeof schObj.eventsData[3].Id).toEqual(
        typeof schObj.eventsData[3].RecurrenceID
      );
      expect(
        schObj.element
          .querySelector('.e-appointment')
          .querySelector('.' + cls.SUBJECT_CLASS).innerHTML
      ).toEqual('My_event');
      expect(
        schObj.element
      ...
      (
        alertDialog.querySelector('.e-quick-dialog-series-event') as HTMLElement
      ).click();
      const dialogElement: HTMLElement = document.querySelector(
        '.' + cls.EVENT_WINDOW_DIALOG_CLASS
      ) as HTMLElement;
    ...
    it('EJ2-64169 - Checking for Uid value in events data for imported appointments', () => {
      ...
      expect(
        schObj.eventsData.filter(
          (data: Record<string, any>) => !isNullOrUndefined(data.UID)
        ).length
      ).toBe(0);
    });
  });
  ...
  describe('EJ2-64169 - Import recurrence events', () => {
    ...
    beforeAll((done: DoneFn) => {
      ...
      const options: ScheduleModel = {
        selectedDate: new Date(2022, 9, 17),
        currentView: 'Week',
      };
      schObj = createSchedule(options, events, done);
    });
    ...
    it('with string type events present in scheduler', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(schObj.eventsData[1].RecurrenceID).toEqual(
          schObj.eventsData[2].Id
        );
        expect(
      ...
      const fileObj: File = new File([calString], 'EJSchedule.ics', {
        lastModified: 0,
        type: 'text/calendar',
      });
      schObj.importICalendar(fileObj);
