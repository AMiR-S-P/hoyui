...
import { extend, closest } from '@hoyui/base';
import { Query, DataManager, UrlAdaptor, ODataV4Adaptor } from '@hoyui/data';
...
describe('Schedule CRUD', () => {
  ...
  describe('EJ2-54104 Problem with error message when we use more then one validation for fields', () => {
    ...
    beforeAll((done: DoneFn) => {
      const customFn: () => boolean = () => {
        const startDate: Date = (document.querySelector('#StartTime') as any)
          .ej2_instances[0].value;
        const endDate: Date = (document.querySelector('#EndTime') as any)
          .ej2_instances[0].value;
    ...
    it('End time validation', () => {
      ...
      saveButton.click();
      expect(
        (dialogElement.querySelector('#EndTime-info') as HTMLElement).innerText
  ...
  describe('Save Actions', () => {
    ...
    beforeAll((done: DoneFn) => {
      const schOptions: ScheduleModel = {
        height: '500px',
        selectedDate: new Date(2017, 9, 19),
        eventSettings: { query: new Query() },
      };
      schObj = util.createSchedule(schOptions, events, done);
    });
  ...
  describe('Delete Actions', () => {
    ...
    it('test delete occurrence action from an edited occurrence', (done: DoneFn) => {
      ...
      let quickDialog: Element = document.querySelector(
        '.e-dialog.e-quick-dialog'
      );
      (
        quickDialog.querySelector(
      ...
      (
        quickDialog.querySelector('.e-quick-dialog-delete') as HTMLButtonElement
      ).click();
    });
  });
  ...
  describe('Remote data testing', () => {
    let schObj: Schedule;
    let dataSource: Record<string, any>[] = cloneDataSource(
      defaultData.slice(0, 10)
    );
    let fetchSpy: any;
    beforeAll((done: DoneFn) => {
      ...
      fetchSpy.and.returnValue(
        Promise.resolve(
          new Response(
            JSON.stringify({ d: dataSource, __count: dataSource.length }),
            {
              status: 200,
              headers: new Headers({
  ...
  xdescribe('Remote data testing with OdataV4 service', () => {
    ...
    it('Check the server end filtering by disabling the includeFiltersInQuery property', (done: DoneFn) => {
      schObj.dataBinding = (args: DataBindingEventArgs) => {
        ...
        expect((args as any).xhr.url).toEqual(
          'https://services.odata.org/V4/Northwind/Northwind.svc/Orders/?StartDate=1996-06-30T00:00:00.000Z&EndDate=1996-08-04T00:00:00.000Z'
        );
        done();
      };
      schObj.dataBound = () => {
    ...
    it('delete occurrence', (done: DoneFn) => {
      schObj.dataBound = () => {
        const elements: NodeListOf<Element> =
          schObj.element.querySelectorAll('.e-appointment');
        const event: Record<string, any> = schObj.getEventDetails(
          elements[0]
        ) as Record<string, any>;
      ...
      const recurrenceEle: NodeListOf<Element> =
        schObj.element.querySelectorAll('.e-recurrence-icon');
  ...
  describe('Public methods', () => {
    ...
    it('getOccurrencesByRange', () => {
      const occurrence: Record<string, any>[] = schObj.getOccurrencesByRange(
        new Date(2017, 9, 29),
        new Date(2017, 10, 4)
      );
  ...
  describe('EJ2-24382 - isSlotAvaliable method is not working properly, while using saveEvent public method', () => {
    ...
    it('slot checking using public save event method', () => {
      ...
      expect(appointment.querySelector('.e-subject').textContent).toEqual(
        'Meeting'
      );
      schObj.dataBound = () => {
        const appointment: HTMLElement = schObj.element.querySelector(
  ...
  describe('Save Actions with Future Occurrences', () => {
    ...
    it('test edit with future occurrences only option', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(dataObj[3].RecurrenceRule as string).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171030T100000Z;'
        );
        expect(dataObj[0].RecurrenceRule as string).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171018T100000Z;'
        );
        done();
      };
      const data: Record<string, any> = extend(
    ...
    it('test edit with future occurrences only option on existing edited future occurrences', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(eventObj[3].RecurrenceRule).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171030T100000Z;'
        );
        expect(dataObj[0].RecurrenceRule as string).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171018T100000Z;'
        ...
        expect(dataObj[3].Subject).toEqual('future event edit parent');
        expect(dataObj[3].RecurrenceRule as string).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171020T100000Z;'
        );
        done();
      };
      const data: Record<string, any> = extend(
        {},
        schObj.eventsProcessed[5],
        null,
        true
      ) as Record<string, any>;
      data.Id = 13;
    ...
    it('test edit occurrence on already edited occurrences with future occurence option ', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(dataObj[5].Subject).toEqual(
          'Edit occurence on future edited event'
        );
        expect(dataObj[5].RecurrenceRule as string).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171030T100000Z;'
        );
        expect(eventData[3].RecurrenceRule as string).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171030T100000Z;'
        );
        done();
      };
    ...
    it('test edit series', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(dataObj[0].RecurrenceRule as string).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171030T043000Z;'
        );
        done();
      };
      const data: Record<string, any> = extend(
        {},
        schObj.eventsProcessed[0],
        null,
        true
      ) as Record<string, any>;
    ...
    it('test edit occurrence to Ensure Ignore edited occurrence', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(dataObj[1].RecurrenceException as string).toEqual(
          '20171018T100000Z'
        );
        done();
      };
    ...
    it('test edit future occurrences with Ignore edited occurrence ', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(dataObj[1].RecurrenceRule as string).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171030T100000Z;'
        );
        done();
      };
  ...
  describe('Save & Delete Actions with Future Occurrences', () => {
    ...
    it('test edit with future occurrences only option on existing edited future occurrences', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(dataObj[3].RecurrenceRule).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171020T100000Z;'
        );
        done();
      };
    ...
    it('test edit occurrence on already edited occurrences with future occurence option ', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(eventData[3].RecurrenceException as string).toEqual(
          '20171021T100000Z'
        );
        expect(eventData[3].RecurrenceRule as string).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20171030T100000Z;'
    ...
    const testData: Record<string, any>[] = [
      {
        ...
        RecurrenceRule:
          'FREQ=DAILY;INTERVAL=1;COUNT=15;UNTIL=20171016T100000Z;',
      },
      {
        Id: 11,
        Subject: 'event 1',
      ...
      },
      {
        ...
        RecurrenceRule:
          'FREQ=DAILY;INTERVAL=1;COUNT=15;UNTIL=20171019T100000Z;',
      },
      {
        Id: 16,
  ...
  describe('Events are missing after block pop thrown ', () => {
    ...
    it('checking appointment rendering', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        (
          schObj.element.querySelector(
            '.e-schedule-toolbar .e-toolbar-right .e-week'
          ) as HTMLElement
        ).click();
        done();
      };
      ...
      const startTime: DateTimePicker = (
        document.querySelector('.e-start.e-datetimepicker') as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
      startTime.value = new Date(2018, 5, 4, 11, 0);
      startTime.dataBind();
      const endTime: DateTimePicker = (
        document.querySelector('.e-end.e-datetimepicker') as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
      ...
      const popupElement: HTMLElement = document.querySelector(
        '.e-quick-dialog.e-popup'
      ) as HTMLElement;
      expect(popupElement.classList.contains('e-popup-open'));
      const okButton: HTMLElement = popupElement.querySelector(
        '.e-footer-content .e-quick-alertok'
      ) as HTMLElement;
      okButton.click();
      (
        schObj.element.querySelector(
          '.e-schedule-toolbar .e-toolbar-right .e-timeline-month'
        ) as HTMLElement
      ).click();
    });
  ...
  describe('Es-835427 - Duplicate event created after changing recurring of the following events', () => {
    ...
    it('To check recurrence appointment after changing editing following events', (done: DoneFn) => {
      ...
      (
        quickDialog.querySelector(
          '.e-quick-dialog-following-events'
        ) as HTMLElement
      ).click();
      const dialogElement: HTMLElement = document.querySelector(
        '.' + cls.EVENT_WINDOW_DIALOG_CLASS
      ...
      saveButton.click();
      (
        document.querySelector('.e-quick-dialog-alert-btn') as HTMLElement
      ).click();
      done();
    });
  });
  ...
  describe('Es-184224 - checking server query params', () => {
    ...
    it('with timezone', () => {
      schObj.dataBinding = (args: DataBindingEventArgs) => {
        expect((args as any).query.params.length).toBe(2);
        expect((args as any).query.params[0].value).toEqual(
          '2023-08-22T23:00:00.000Z'
        );
        expect((args as any).query.params[1].value).toEqual(
          '2023-08-23T23:00:00.000Z'
        );
      };
    });
  });
  ...
  describe('ES-902399 - actionBegin event args testing', () => {
    ...
    beforeAll((done: DoneFn) => {
      const schOptions: ScheduleModel = {
        height: '500px',
        selectedDate: new Date(2024, 7, 28),
      };
    ...
    it('Checking eventRemove arguments upon DeleteSeries', (done: DoneFn) => {
      schObj.actionBegin = (args: ActionEventArgs) => {
        ...
        expect(args.deletedRecords[0].RecurrenceRule).toEqual(
    ...
    it('Binding data to Schedule to check recurrence series edit from edited recurrence event - "yes" click', (done: DoneFn) => {
      ...
      schObj.eventSettings = {
        dataSource: extend([], eventData, null, true) as Record<string, any>[],
      };
      schObj.dataBind();
    });
    ...
    it('Checking eventChange arguments upon EditSeries from edited appointment clicking "yes" on alert dialog', (done: DoneFn) => {
      schObj.actionBegin = (args: ActionEventArgs) => {
        ...
        expect((args.data as Record<string, any>).Subject).toEqual(
          'Scrum meeting'
        );
        expect((args.data as Record<string, any>).RecurrenceRule).toEqual(
          'FREQ=DAILY;INTERVAL=1;COUNT=5'
        );
        expect(args.addedRecords.length).toEqual(0);
        ...
        expect(args.deletedRecords[0].Subject).toEqual(
          'Scrum meeting - rescheduled'
        );
        expect(args.deletedRecords[0].RecurrenceID).toEqual(1);
        expect(args.changedRecords[0].Id).toEqual(1);
        expect(args.changedRecords[0].Subject).toEqual(
          'Scrum meeting restored'
        );
        expect(args.changedRecords[0].RecurrenceRule).toEqual(
          'FREQ=DAILY;INTERVAL=1;COUNT=5;'
        );
        expect(args.changedRecords[0].RecurrenceException).toEqual(
          '20240828T100000Z'
        );
      };
      schObj.dataBound = () => {
      ...
      const subjectInput: HTMLInputElement =
        dialogElement.querySelector('.e-subject.e-field');
      ...
      quickDialog = document.querySelector('.e-dialog.e-quick-dialog');
      (
        quickDialog.querySelector('.e-quick-alertok') as HTMLButtonElement
      ).click();
    });
    ...
    it('Checking eventChange arguments upon EditSeries from edited appointment clicking "no" on alert dialog', (done: DoneFn) => {
      ...
      quickDialog = document.querySelector('.e-dialog.e-quick-dialog');
      (
        quickDialog.querySelector('.e-quick-alertcancel') as HTMLButtonElement
      ).click();
    });
    ...
    it('Binding data to Schedule to check recurrence series edit from following edited recurrence event - "yes" click', (done: DoneFn) => {
      ...
      schObj.eventSettings = {
        dataSource: extend([], followingEditData, null, true) as Record<
          string,
          any
        >[],
        editFollowingEvents: true,
    ...
    it('Checking eventChange arguments upon EditSeries from followinig edited appointment clicking "yes" on alert dialog', (done: DoneFn) => {
      schObj.actionBegin = (args: ActionEventArgs) => {
        ...
        expect((args.data as Record<string, any>).RecurrenceRule).toEqual(
          'FREQ=DAILY;INTERVAL=1;UNTIL=20240830T120000Z;'
        );
        expect((args.data as Record<string, any>).RecurrenceException).toEqual(
          '20240828T120000Z'
        );
        expect(args.addedRecords.length).toEqual(0);
        expect(args.changedRecords.length).toEqual(1);
