...
import { createElement, Browser, EventHandler, extend } from '@hoyui/base';
import {
  Schedule,
  EJ2Instance,
  Day,
...
describe('Touch functionalities', () => {
  ...
  describe('swipe actions', () => {
    ...
    it('swipe right touch move different points', () => {
      ...
      const moveStart: CommonArgs = <CommonArgs>(
        extend({}, startMouseEventArs, { clientX: 50, target: target })
      );
      const moveArgs1: CommonArgs = <CommonArgs>(
        extend({}, moveMouseEventArs, {
          type: 'touchmove',
          clientX: 80,
          target: target,
        })
      );
      const moveArgs2: CommonArgs = <CommonArgs>(
        extend({}, moveMouseEventArs, {
          type: 'touchmove',
          clientX: 100,
          target: target,
      ...
      const movedEnd: CommonArgs = <CommonArgs>(
        extend({}, endMouseEventArs, { clientX: 250, target: target })
      );
      ...
      expect(schObj.selectedDate.getTime()).toEqual(
    ...
    it('swipe left touch move different points', () => {
      ...
      const target: Element = schObj.element.querySelector('.e-work-cells');
      const moveStart: CommonArgs = <CommonArgs>(
        extend({}, startMouseEventArs, { clientX: 250, target: target })
      );
      const moveArgs1: CommonArgs = <CommonArgs>(
        extend({}, moveMouseEventArs, {
          type: 'touchmove',
      ...
      const movedEnd: CommonArgs = <CommonArgs>(
        extend({}, endMouseEventArs, { clientX: 50, target: target })
      );
      touchTestObj.startEvent(moveStart);
      touchTestObj.moveEvent(moveArgs1);
      touchTestObj.moveEvent(moveArgs2);
    ...
    it('swipe left and then moved to right direction', () => {
      ...
      const target: Element = schObj.element.querySelector('.e-work-cells');
      const moveStart: CommonArgs = <CommonArgs>(
        extend({}, startMouseEventArs, { clientX: 80, target: target })
      );
      const moveArgs1: CommonArgs = <CommonArgs>(
        extend({}, moveMouseEventArs, {
          type: 'touchmove',
    ...
    it('swipe right and then moved to left direction', () => {
      ...
      const target: Element = schObj.element.querySelector('.e-work-cells');
      const moveStart: CommonArgs = <CommonArgs>(
        extend({}, startMouseEventArs, { clientX: 180, target: target })
      );
      const moveArgs1: CommonArgs = <CommonArgs>(
        extend({}, moveMouseEventArs, {
          type: 'touchmove',
    ...
    it('start swipe while before previous swipe transition end', () => {
      ...
      const target: Element = schObj.element.querySelector('.e-work-cells');
      const moveStart: CommonArgs = <CommonArgs>(
        extend({}, startMouseEventArs, { clientX: 200, target: target })
      );
      const moveArgs: CommonArgs = <CommonArgs>(
        extend({}, moveMouseEventArs, {
          type: 'touchmove',
          clientX: 100,
          target: target,
      ...
      const movedEnd: CommonArgs = <CommonArgs>(
        extend({}, endMouseEventArs, { clientX: 100, target: target })
      );
      touchTestObj.startEvent(moveStart);
      touchTestObj.moveEvent(moveArgs);
      touchTestObj.endEvent(movedEnd);
      const moveStart2: CommonArgs = <CommonArgs>(
        extend({}, startMouseEventArs, { clientX: 210, target: target })
      );
      const moveArgs2: CommonArgs = <CommonArgs>(
        extend({}, moveMouseEventArs, {
          type: 'touchmove',
      ...
      const movedEnd2: CommonArgs = <CommonArgs>(
        extend({}, endMouseEventArs, { clientX: 110, target: target })
      );
  ...
  describe('Appointment Tap and multiple select actions', () => {
    ...
    it('taphold appointment selection', () => {
      ...
      expect(
        document.body.querySelector('.e-quick-popup-wrapper')
      ).toBeTruthy();
    });
    it('Single appointment select using click or tap for multi select', () => {
      ...
      const popup: HTMLElement = document.body.querySelector(
  ...
  describe('Disable swiping', () => {
    ...
    beforeAll(() => {
      const schOptions: ScheduleModel = {
        ...
        width: 300,
        currentView: 'Day',
        selectedDate: new Date(2022, 7, 5),
        allowSwiping: false,
      };
    ...
    it('prevent date navigate swipe action', (done: DoneFn) => {
      ...
      expect(
        ...
        '<div class="e-header-day">Fri</div><div class="e-header-date e-navigate" role="link">5</div>'
      );
      setTimeout(() => {
        expect(
        ...
        expect(
          schObj.element.querySelector('.e-table-container').childNodes.length
        ).toEqual(1);
        expect(schObj.selectedDate.getTime()).toEqual(
          new Date(2022, 7, 5).getTime()
        );
        done();
      }, 400);
    });
  ...
  describe('EJ2MVC-469 Need to enable Schedule swipe action when performing swipe on the appointments', () => {
    ...
    it('swipe right touch move different points', () => {
      ...
      expect(
        ...
        '<div class="e-header-day">Wed</div><div class="e-header-date e-navigate" role="link">1</div>'
      );
    });
    it('swipe left touch move different points', () => {
      node = schObj.element.querySelector('.e-table-container');
      ...
      expect(
        ...
        '<div class="e-header-day">Thu</div><div class="e-header-date e-navigate" role="link">2</div>'
      );
    });
  });

  describe('866131 - iOS appointment tap hold operation', () => {
    const iOSUserAgent: string =
    ...
    beforeAll((done: DoneFn) => {
      ...
      const schOptions: ScheduleModel = {
        width: 300,
        currentView: 'Day',
        selectedDate: new Date(2024, 4, 10),
    ...
    it('taphold appointment selection', () => {
      ...
      const target: Element = schObj.element.querySelector('.e-appointment');
      const e: any = {};
      e.originalEvent = new TouchEvent('touchstart', {
        cancelable: true,
        bubbles: true,
      });
      ...
      e.originalEvent = new TouchEvent('touchend', {
        cancelable: true,
        bubbles: true,
      });
      target.dispatchEvent(e.originalEvent);
