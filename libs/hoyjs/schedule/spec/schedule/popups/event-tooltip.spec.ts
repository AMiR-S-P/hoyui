 ...
 */
import { isVisible } from '@hoyui/base';
import {
  Schedule,
  ScheduleModel,
  Day,
...
describe('Schedule event tooltip module', () => {
  ...
  describe('Event tooltip on template content', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        eventSettings: {
          enableTooltip: true,
          tooltipTemplate:
            '<div class="event-template" style="padding:5px;"><div>Subject : ${Subject}</div></div>',
        },
      };
    ...
    it('mouse hover open tooltip', () => {
      ...
      expect(isVisible(tooltipEle)).toBe(true);
      expect(
        [].slice.call(tooltipEle.querySelectorAll('.event-template')).length
      ).toEqual(1);
      util.triggerMouseEvent(target, 'mouseleave');
      expect(document.querySelector('.e-schedule-event-tooltip')).toBeNull();
    });
    it('change tooltip template through set model', () => {
      schObj.eventSettings.tooltipTemplate =
        '<div class="event-template1" style="padding:5px;">Subject : ${Subject}</div>';
      schObj.dataBind();
      const target: HTMLElement =
        schObj.element.querySelector('.e-appointment');
      expect(document.querySelector('.e-schedule-event-tooltip')).toBeNull();
      ...
      expect(
        [].slice.call(tooltipEle.querySelectorAll('.event-template1')).length
      ).toEqual(1);
      util.triggerMouseEvent(target, 'mouseleave');
      expect(document.querySelector('.e-schedule-event-tooltip')).toBeNull();
    });
  ...
  describe('Resource tooltip on template content', () => {
      ...
      const model: ScheduleModel = {
        ...
        group: {
          ...
          headerTooltipTemplate:
            '<div class="tWrap"><div class="rText">Name: ${getResourceName(data)}</div></div>',
    ...
    it('mouse hover open tooltip', () => {
      util.disableTooltipAnimation((schObj.eventTooltip as any).tooltipObj);
      const target: HTMLElement =
        schObj.element.querySelector('.e-resource-cells');
      expect(document.querySelector('.e-schedule-event-tooltip')).toBeNull();
      util.triggerMouseEvent(target, 'mouseover');
      const tooltipEle: HTMLElement = document.querySelector(
        '.e-schedule-event-tooltip'
      ...
      expect(isVisible(tooltipEle)).toBe(true);
      expect(
        [].slice.call(tooltipEle.querySelectorAll('.tWrap')).length
      ).toEqual(1);
      util.triggerMouseEvent(target, 'mouseleave');
      expect(document.querySelector('.e-schedule-event-tooltip')).toBeNull();
    });
    it('change tooltip template through set model', () => {
      schObj.group.headerTooltipTemplate =
        '<div class="event-template1" style="padding:5px;">Name: ${getResourceName(data)}</div>';
      schObj.dataBind();
      const target: HTMLElement =
        schObj.element.querySelector('.e-resource-cells');
      expect(document.querySelector('.e-schedule-event-tooltip')).toBeNull();
  ...
  describe('Timeline resource header and event tooltip', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        group: {
          byGroupID: false,
          headerTooltipTemplate:
            '<div class="resname">Name: ${getResourceName(data)}</div>',
          resources: ['Rooms', 'Owners'],
        },
        resources: [
          {
    ...
    it('tooltip on resource header', () => {
      ...
      expect(isVisible(tooltipEle)).toBe(true);
      expect(
        [].slice.call(tooltipEle.querySelectorAll('.resname')).length
      ).toEqual(1);
      expect(tooltipEle.innerHTML).toEqual(
        '<div class="e-tip-content"><div><div class="resname">Name: ROOM 1</div></div></div>'
      );
      util.triggerMouseEvent(target, 'mouseleave');
      expect(document.querySelector('.e-schedule-event-tooltip')).toBeNull();
    });
  ...
  describe('EJ2-48019 - Add title missing in agenda view', () => {
    ...
    beforeAll((done: DoneFn) => {
      const schOptions: ScheduleModel = {
        height: '500px',
        selectedDate: new Date(2017, 9, 31),
        currentView: 'Agenda',
        eventSettings: { enableTooltip: true },
      };
