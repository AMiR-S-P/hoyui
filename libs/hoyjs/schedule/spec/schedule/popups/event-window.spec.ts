 ...
 */
import { Browser, EmitType, closest, remove, createElement } from '@hoyui/base';
import { CheckBox, Button } from '@hoyui/buttons';
import { DateTimePicker, DatePicker } from '@hoyui/calendars';
import { RadioButton } from '@hoyui/buttons';
import { FormValidator, NumericTextBox } from '@hoyui/inputs';
import { AutoComplete, DropDownList, MultiSelect } from '@hoyui/dropdowns';
import {
  Schedule,
  Week,
  ScheduleModel,
...
import {
  RecurrenceEditor,
  RecurrenceEditorModel,
} from '../../../src/recurrence-editor/index';
import {
  EJ2Instance,
  PopupOpenEventArgs,
  ActionEventArgs,
} from '../../../src/schedule/base/interface';
...
describe('Schedule event window initial load', () => {
    ...
    it('Multilevel change event checking', () => {
      ...
      const repeatElement: MultiSelect = (
        dialogElement.querySelector(
          '.e-' + schObj.resourceBase.resourceCollection[1].field
        ) as EJ2Instance
  ...
  xdescribe('Schedule event window with group without child', () => {
    ...
    it('Multilevel change event checking', () => {
      ...
      const repeatElement: DropDownList = (
        dialogElement.querySelector(
          '.e-' + schObj.resourceBase.resourceCollection[0].field
        ) as EJ2Instance
      ...
      const technician: MultiSelect = (
        dialogElement.querySelector(
          '.e-' + schObj.resourceBase.resourceCollection[2].field
        ) as EJ2Instance
      ).ej2_instances[0] as MultiSelect;
    ...
    it('Edit through event window checking', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        const endDate: DateTimePicker = (
          (<HTMLElement>(
            dialogElement.querySelector('.' + cls.EVENT_WINDOW_END_CLASS)
          )) as EJ2Instance
      ...
      const descriptionElement: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.DESCRIPTION_CLASS)
      );
      descriptionElement.value = 'Product Demo';
      const startDate: DateTimePicker = (
        (<HTMLElement>(
          dialogElement.querySelector('.' + cls.EVENT_WINDOW_START_CLASS)
        )) as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
  ...
  describe('Schedule event window initial load', () => {
    ...
    it('End value change based on start value', () => {
      ...
      const startElement: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_START_CLASS)
      );
      ...
      const listItem: HTMLElement = document.querySelectorAll(
        '.e-datetimepicker .e-list-item'
      )[5] as HTMLElement;
      listItem.click();
      const endElement: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_END_CLASS)
      );
    ...
    it('Invalid Start time or end time validation', () => {
      ...
      const endElement: HTMLElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_END_CLASS)
      );
      ((endElement as EJ2Instance).ej2_instances[0] as DateTimePicker).value =
        null;
      (
        (endElement as EJ2Instance).ej2_instances[0] as DateTimePicker
      ).dataBind();
      ...
      expect(
        (
          schObj.quickPopup.quickDialog.element.querySelector(
            '.e-dlg-content'
          ) as HTMLElement
        ).innerText
    ...
    it('Start and End DateTime format change', () => {
      ...
      const allDayElement: HTMLElement = <HTMLInputElement>(
        dialogElement.querySelector(
          '.' + cls.EVENT_WINDOW_ALL_DAY_CLASS + ' input'
        )
      );
      allDayElement.click();
    ...
    it('Start and End DateTime format change - with date format value', () => {
      ...
      const dialogElement: HTMLElement = (<HTMLInputElement>(
        document.querySelector('.' + cls.EVENT_WINDOW_DIALOG_CLASS)
      )) as HTMLElement;
      const startElement: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_START_CLASS)
      );
    ...
    it('Timezone display', () => {
      ...
      const timezoneElement: HTMLElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.TIME_ZONE_CLASS + ' input')
      );
      timezoneElement.click();
      const timezoneDiv: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_TIME_ZONE_DIV_CLASS)
      );
    ...
    it('Recurrence Appointment Validation', () => {
      ...
      const dialogElement: HTMLElement = schObj.eventWindow.dialogObject
        .element as HTMLElement;
      const repeatElement: DropDownList = (
        dialogElement.querySelector('.e-repeat-element') as EJ2Instance
      ).ej2_instances[0] as DropDownList;
      const endOnElement: DropDownList = (
        dialogElement.querySelector('.e-end-on-element') as EJ2Instance
      ).ej2_instances[0] as DropDownList;
      const repeatInterval: NumericTextBox = (
        dialogElement.querySelector('.e-repeat-interval') as EJ2Instance
      ).ej2_instances[0] as NumericTextBox;
      repeatElement.index = 1;
      repeatElement.dataBind();
      endOnElement.index = 1;
      endOnElement.dataBind();
      const endDate: DateTimePicker = (
        dialogElement.querySelector('.e-end.e-datetimepicker') as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
      ...
      repeatElement.index = 2;
      repeatElement.dataBind();
      saveButton.click();
      okButton.click();
      endDate.value = new Date('10/30/2017');
      repeatElement.index = 3;
      repeatElement.dataBind();
      const monthDate: RadioButton = (
        dialogElement.querySelector('.e-radio-wrapper .e-radio') as EJ2Instance
      ).ej2_instances[0] as RadioButton;
      monthDate.checked = true;
      const mDate: NumericTextBox = (
        dialogElement.querySelector('.e-month-day') as EJ2Instance
      ).ej2_instances[0] as NumericTextBox;
      mDate.value = 30;
    ...
    it('Create Ocurrence for Validation', (done: DoneFn) => {
      ...
      repeatElement.index = 1;
      repeatElement.dataBind();
      endOnElement.index = 2;
      endOnElement.dataBind();
      const count: NumericTextBox = (
        dialogElement.querySelector('.e-recurrence-count') as EJ2Instance
      ).ej2_instances[0] as NumericTextBox;
      count.value = 4;
      const saveButton: HTMLElement = dialogElement.querySelector(
        '.e-event-save'
    ...
    it('Edit Occurrence Validation', (done: DoneFn) => {
      ...
      const eventDialog: HTMLElement = document.querySelector(
        '.e-quick-dialog'
      ) as HTMLElement;
      const editButton: HTMLElement = eventDialog.querySelector(
      ...
      const startDate: DateTimePicker = (
        dialogElement.querySelector('.e-start.e-datetimepicker') as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
      const endDate: DateTimePicker = (
        dialogElement.querySelector('.e-end.e-datetimepicker') as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
      ...
      cancelButton.click();
      (
        schObj.element.querySelector('[data-id="Appointment_6"]') as HTMLElement
      ).click();
      ...
      (<HTMLElement>(
        eventDialog.querySelector('.e-quick-dialog-series-event')
      )).click();
    });
    ...
    it('Reset recurrence field value checking on cell double click', () => {
      ...
      repeatInterval.value = null;
      repeatInterval.dataBind();
      repeatCount.value = null;
      repeatCount.dataBind();
      repeatElement.index = 2;
      repeatElement.dataBind();
      const cancelButton: HTMLElement = dialogElement.querySelector(
        '.e-event-cancel'
    ...
    it('Dialog time value checking on allDay cell double click and uncheck allDay element', () => {
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-all-day-cells')[1] as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-all-day-cells')[1] as HTMLElement,
      ...
      const allDayElement: HTMLElement = dialogElement.querySelector(
        '.' + cls.EVENT_WINDOW_ALL_DAY_CLASS + ' input'
      );
      const startElement: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_START_CLASS)
    ...
    it('Event window normal appointments value checking', () => {
      ...
      const startElement: HTMLInputElement = dialogElement.querySelector(
        '.' + cls.EVENT_WINDOW_START_CLASS
      );
      const endElement: HTMLInputElement = dialogElement.querySelector(
        '.' + cls.EVENT_WINDOW_END_CLASS
      );
      const formElement: HTMLInputElement = dialogElement.querySelector(
        '.' + cls.FORM_CLASS
      );
      const subjectElement: HTMLInputElement = dialogElement.querySelector(
        '.' + cls.SUBJECT_CLASS
      );
      const locationElement: HTMLInputElement = dialogElement.querySelector(
        '.' + cls.LOCATION_CLASS
      );
      const descriptionElement: HTMLInputElement = dialogElement.querySelector(
        '.' + cls.DESCRIPTION_CLASS
      );
      const allDayElement: HTMLElement = dialogElement.querySelector(
        '.' + cls.EVENT_WINDOW_ALL_DAY_CLASS + ' input'
      );
      const timezoneElement: HTMLElement = dialogElement.querySelector(
      ...
      expect(
        ((timezoneElement as EJ2Instance).ej2_instances[0] as CheckBox).checked
      ).toEqual(false);
      expect(
        ((allDayElement as EJ2Instance).ej2_instances[0] as CheckBox).checked
      ).toEqual(false);
    ...
    it('Time zone value checking', () => {
      ...
      const timezoneDiv: HTMLElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_TIME_ZONE_DIV_CLASS)
      );
      expect(
        ((timezoneElement as EJ2Instance).ej2_instances[0] as CheckBox).checked
    ...
    it('Event window timezone value checking depends upon allday appointment', () => {
      ...
      const timezoneContainer: HTMLElement = <HTMLInputElement>(
        dialogElement.querySelector('.e-time-zone-container')
      );
      expect(
        ((allDayElement as EJ2Instance).ej2_instances[0] as CheckBox).checked
      ).toEqual(true);
    ...
    it('Time zone - search value', () => {
      ...
      const startTZDropDown: HTMLElement = dialogElement.querySelector(
      ...
      const listObj: any = (startTZDropDown as EJ2Instance)
        .ej2_instances[0] as DropDownList;
      listObj.showPopup();
      listObj.filterInput.value = 'Maw';
      listObj.onInput();
      listObj.onFilterUp(keyEventArgs);
      let element: Element = document.querySelector(
    ...
    it('Delete through event window checking', (done: DoneFn) => {
      ...
      const deleteButton: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.DELETE_EVENT_CLASS)
      );
      deleteButton.click();
      const deleteButton1: HTMLInputElement = <HTMLInputElement>(
        document.querySelector('.e-quick-dialog-delete')
      );
      deleteButton1.click();
    });
    ...
    it('Event double click and Delete occurrence through window', () => {
      ...
      const recurrenceEle: Element =
        schObj.element.querySelector('.e-recurrence-icon');
      const appointmentElement: HTMLElement = closest(
        recurrenceEle,
        '.e-appointment'
      ) as HTMLElement;
      appointmentElement.click();
      (<HTMLElement>(
        (<HTMLElement>schObj.quickPopup.quickPopup.content).querySelector(
          '.e-event-edit'
        )
      )).click();
      (<HTMLElement>(
        schObj.eventWindow.dialogObject.element.querySelector('.e-event-delete')
      )).click();
      (<HTMLElement>(
        schObj.quickPopup.quickDialog.element.querySelector(
    ...
    it('Event double click and Edit occurrence', (done: DoneFn) => {
      ...
      const recurrenceEle: HTMLElement = schObj.element.querySelector(
        '.e-recurrence-icon'
      ) as HTMLElement;
      const appointmentElement: HTMLElement = closest(
        recurrenceEle,
    ...
    it('Event double click and Edit occurrence multi checking', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventsData.length).toEqual(6);
        const dataObj: Record<string, any>[] = schObj.eventsData as Record<
          string,
          any
        >[];
    ...
    it('Event double click and Delete series through window', () => {
      ...
      const recurrenceEle: Element =
        schObj.element.querySelector('.e-recurrence-icon');
      const agendaElement: HTMLElement = closest(
        recurrenceEle,
        '.e-appointment'
      ) as HTMLElement;
    ...
    it('Event double click and Edit series', (done: DoneFn) => {
      ...
      const editEvent: HTMLElement = eventDialog.querySelector(
        '.e-quick-dialog-occurrence-event'
      ) as HTMLElement;
      editEvent.click();
      const dialogElement: HTMLElement = document.querySelector(
    ...
    it('Event Click and Delete occurence', (done: DoneFn) => {
      ...
      expect(eventPopup).toBeTruthy();
      (<HTMLElement>eventPopup.querySelector('.e-event-delete')).click();
      const eventDialog: HTMLElement = document.body.querySelector(
        '.e-quick-dialog'
      ) as HTMLElement;
      (<HTMLElement>(
        eventDialog.querySelector('.e-quick-dialog-occurrence-event')
      )).click();
    });
    ...
    it('Event Click and Delete series', (done: DoneFn) => {
      ...
      (
        schObj.element.querySelector('[data-id="Appointment_5"]') as HTMLElement
      ).click();
      const eventPopup: HTMLElement = schObj.element.querySelector(
        '.e-quick-popup-wrapper'
      ) as HTMLElement;
    ...
    it('Read only checking on event double click', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          (
            (<any>schObj.eventWindow).getInstance(
              cls.EVENT_WINDOW_SAVE_BUTTON_CLASS
            ) as Button
          ).disabled
        ).toEqual(false);
        ...
        expect(
          schObj.quickPopup.quickDialog.element.classList.contains(
            'e-popup-open'
          )
        ).toEqual(false);
        expect(
  ...
  describe('Recurrence Validation for event Id as string', () => {
    ...
    it('Day validation', () => {
      ...
      saveButton.click();
      const okButton: HTMLElement = document.querySelector(
        '.e-quick-alertok'
      ) as HTMLElement;
      okButton.click();
    });
  ...
  describe('Schedule event window mobile', () => {
    ...
    describe('work cells taphold', () => {
      ...
      it('dialog checking cell click and add event', () => {
        const toolbarElement: HTMLElement = schObj.element.querySelector(
          '.e-schedule-toolbar'
        ) as HTMLElement;
        const firstWorkCell: HTMLElement = schObj.element.querySelectorAll(
          '.e-work-cells'
        )[0] as HTMLElement;
        firstWorkCell.click();
        (<HTMLElement>(
          toolbarElement.querySelector('.e-add .e-tbar-btn')
        )).click();
        const dialogElement: HTMLElement = document.querySelector(
          '.' + cls.EVENT_WINDOW_DIALOG_CLASS
        ) as HTMLElement;
        const titleElement: HTMLElement = dialogElement.querySelector(
          '.e-title-header'
        ) as HTMLElement;
        expect(titleElement.children.length).toEqual(3);
        const backIcon: HTMLElement = <HTMLInputElement>(
          dialogElement.querySelector('.' + cls.EVENT_WINDOW_BACK_ICON_CLASS)
        );
        backIcon.click();
      });
      ...
      it('Repeat status checking', () => {
        ...
        const repeatElement: HTMLElement = <HTMLInputElement>(
          dialogElement.querySelector(
        ...
        repeatElement.click();
        expect(
          ((repeatElement as EJ2Instance).ej2_instances[0] as CheckBox).checked
        ).toEqual(true);
        const backIcon: HTMLElement = <HTMLInputElement>(
          dialogElement.querySelector('.' + cls.EVENT_WINDOW_BACK_ICON_CLASS)
        );
      ...
      it('EJ2-63839 - openEditor with repeatType check', () => {
        ...
        expect(
          ((repeatElement as EJ2Instance).ej2_instances[0] as CheckBox).label
        ).toEqual('Repeats every day(s)');
        const backIcon: HTMLElement = <HTMLInputElement>(
          dialogElement.querySelector('.' + cls.EVENT_WINDOW_BACK_ICON_CLASS)
        );
        backIcon.click();
    ...
    describe('event taphold', () => {
      ...
      it('dialog checking event tapHold', () => {
        ...
        (schObj.scheduleTouchModule as any).tapHoldHandler({
          originalEvent: { target: firstEvent, type: 'touchstart' },
        });
        const eventPopup: HTMLElement = document.body.querySelector(
          '.e-quick-popup-wrapper'
        ) as HTMLElement;
        expect(eventPopup).toBeTruthy();
        ...
        expect(dialogElement.querySelector('.e-title-text').innerHTML).toEqual(
          'Edit Event'
        );
        const backIcon: HTMLElement = <HTMLInputElement>(
          dialogElement.querySelector('.' + cls.EVENT_WINDOW_BACK_ICON_CLASS)
      ...
      it('dialog checking header cells tapHold', () => {
        const headerCell: HTMLElement = schObj.element.querySelectorAll(
  ...
  describe('Schedule custom editor window', () => {
    ...
    beforeAll((done: DoneFn) => {
      const template: string =
        '<table class="custom-event-editor" width="100%" cellpadding="5"><tbody>' +
      ...
      const onPopupOpen: EmitType<PopupOpenEventArgs> = (
        ...
        if (args.type === 'Editor') {
          ...
          const recurrenceEditor: HTMLElement = args.element.querySelector(
            '#recurrenceEditor'
          ) as HTMLElement;
    ...
    it('add event', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect((schObj.eventsData[0] as Record<string, any>).Subject).toEqual(
          'add'
        );
        expect(schObj.eventsData.length).toEqual(1);
        done();
      ...
      (dialogElement.querySelector('#Subject') as HTMLInputElement).value =
        'add';
      const addButton: HTMLElement = dialogElement.querySelector(
        '.e-event-save'
      ) as HTMLElement;
      addButton.click();
      expect(schObj.eventWindow.dialogObject.visible).toEqual(false);
    ...
    it('edit event', (done: DoneFn) => {
      ...
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-appointment')[0] as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-appointment')[0] as HTMLElement,
    ...
    it('editorTemplate set model to empty content', () => {
      ...
      expect(
        dialogElement.querySelectorAll('.' + cls.SUBJECT_CLASS).length
      ).toEqual(1);
    ...
    it('editorTemplate set model to change content', () => {
      schObj.editorTemplate =
        '<div id="customDiv">Some content in dialog</div>';
      schObj.dataBind();
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[0] as HTMLElement,
        'click'
    ...
    it('EJ2-70482 - Schedule form validator instance count ', () => {
      ...
      const formElement: object[] = (
        (<HTMLElement>(
          dialogElement.querySelector('.e-schedule-form')
        )) as EJ2Instance
      ).ej2_instances;
      ...
      expect(schObj.eventWindow.dialogObject.visible).toEqual(false);
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[10] as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[10] as HTMLElement,
    ...
    beforeAll((done: DoneFn) => {
      ...
      const onPopupOpen: EmitType<PopupOpenEventArgs> = (
        ...
        if (args.type === 'Editor') {
          ...
          const recurrenceEditor: HTMLElement =
  ...
  describe('Editor changed window validation', () => {
    ...
    beforeAll((done: DoneFn) => {
      ...
      const onPopupOpen: EmitType<PopupOpenEventArgs> = (
        ...
        if (args.type === 'Editor') {
          const formElement: HTMLElement = <HTMLElement>(
          ...
          const validator: FormValidator = (formElement as EJ2Instance)
    ...
    it('event window validation checking with save', () => {
      ...
      expect(
        (
          schObj.eventWindow.dialogObject.element.querySelector(
            '.e-tip-content'
          ) as HTMLElement
        ).innerText
      ).toEqual('This field is required.');
      const cancelButton: HTMLElement = dialogElement.querySelector(
        '.' + cls.EVENT_WINDOW_CANCEL_BUTTON_CLASS
      ) as HTMLElement;
      cancelButton.click();
    });
  ...
  describe('Editor changed window validation input feild', () => {
    ...
    beforeAll((done: DoneFn) => {
      const template: string =
        '<table class="custom-event-editor" width="100%" cellpadding="5" id ="formId"><tbody>' +
      ...
      const onPopupOpen: EmitType<PopupOpenEventArgs> = (
        ...
        if (args.type === 'Editor') {
          ...
          const sessionElement: HTMLInputElement = args.element.querySelector(
            '#session2'
          ) as HTMLInputElement;
          if (!sessionElement.classList.contains('e-datetimepicker')) {
            new DateTimePicker(
              { value: new Date(sessionElement.value) || new Date() },
              sessionElement
            );
          }
          const employeeElement: HTMLInputElement = args.element.querySelector(
            '#employee'
          ) as HTMLInputElement;
          ...
          const facultyElement: HTMLInputElement = args.element.querySelector(
            '#faculty'
          ) as HTMLInputElement;
          ...
          const locationElement: HTMLInputElement = args.element.querySelector(
            '#location'
          ) as HTMLInputElement;
          ...
          const stageElement: HTMLInputElement = args.element.querySelector(
            '#Stage'
          ) as HTMLInputElement;
          if (stageElement) {
    ...
    it('event window validation checking with input Feild', () => {
      ...
      const subjectElement: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('#trainingNumber')
      );
  ...
  describe('Editor window validation', () => {
    ...
    it('event window validation checking with save', () => {
      ...
      subjectElement.focus();
      subjectElement.blur();
      const locationElement: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.LOCATION_CLASS)
      );
    ...
    it('Edit event validation checking after close icon click', () => {
      const appElements: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
      );
      ...
      expect(
        document.querySelector(
          '.e-schedule-dialog .e-dlg-content .e-tooltip-wrap'
        ) as HTMLInputElement
      ).toEqual(null);
      const saveButton: HTMLElement = dialogElement.querySelector(
        '.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS
      ...
      expect(
        document.querySelector(
          '.e-schedule-dialog .e-dlg-content .e-tooltip-wrap#Location_Error #Location-info'
        ).innerHTML
      ).toEqual('This field is required.');
      expect(schObj.eventWindow.dialogObject.visible).toEqual(true);
      const cancelButton: HTMLElement = dialogElement.querySelector(
    ...
    it('save checking after clearing validation error', (done: DoneFn) => {
      ...
      (
        (document.querySelector('.' + cls.FORM_CLASS) as EJ2Instance)
          .ej2_instances[0] as FormValidator as any
      ).errorRules = [];
      const dialogElement: HTMLElement = document.querySelector(
        '.' + cls.EVENT_WINDOW_DIALOG_CLASS
      ) as HTMLElement;
    ...
    it('custom recurrence editor instance setting', () => {
      ...
      expect(
        schObj.eventWindow.dialogObject.element.classList.contains(
          'e-popup-open'
        )
      ).toEqual(true);
      expect(
      ...
      const recObj: RecurrenceEditor = (
        schObj.eventWindow.dialogObject.element.querySelector(
          '.e-recurrenceeditor'
        ) as EJ2Instance
      ).ej2_instances[0] as RecurrenceEditor;
      expect(recObj.value).toBe('');
      schObj.setRecurrenceEditor(
        new RecurrenceEditor({ value: 'FREQ=DAILY;INTERVAL=1;COUNT=5' })
      );
      expect(recObj.value).toBe('');
      util.triggerMouseEvent(
        schObj.eventWindow.dialogObject.element.querySelector(
    ...
    it('open editor method manually testing for cell click action', () => {
      const cellData: Record<string, any> = schObj.getCellDetails(
        schObj.element.querySelectorAll('.e-work-cells').item(143)
      );
      const eventData: Record<string, any> = {
        RecurrenceRule: 'FREQ=DAILY;INTERVAL=1;COUNT=10',
      };
      (<Record<string, any>>cellData).RecurrenceRule =
        'FREQ=DAILY;INTERVAL=1;COUNT=5';
      schObj.eventWindow.convertToEventData(
        <Record<string, any>>cellData,
        eventData
      );
      expect(
        schObj.eventWindow.dialogObject.element.classList.contains(
  ...
  describe('Editor window - Recurrence Editor Mobile Interaction', () => {
    ...
    it('dialog Interaction checking.', () => {
      ...
      expect(
        (<any>schObj).eventWindow.recurrenceEditor.getRecurrenceRule()
      ).toBe('');
      (<any>schObj).eventWindow.recurrenceEditor.setProperties({
        selectedType: 1,
      });
      ...
      expect(
        (<any>schObj).eventWindow.recurrenceEditor.getRecurrenceRule()
      ).toBe('FREQ=WEEKLY;BYDAY=SU;INTERVAL=1;');
  ...
  describe('event window destroy method testing', () => {
    ...
    it('recurrence editor control destroy checking', () => {
      const model: RecurrenceEditorModel = {
        value: 'FREQ=DAILY;INTERVAL=1;COUNT=5',
      };
    ...
    it('recurrence editor rendering', () => {
      ...
      recObj = util.createRecurrenceEditor(model);
      expect(recObj.element.classList.contains('e-recurrenceeditor')).toEqual(
        true
      );
  ...
  describe('isSlotAvailable public method testing', () => {
      ...
      const model: ScheduleModel = {
        ...
        actionBegin: (args: ActionEventArgs) => {
          if (args.data) {
            ...
            args.cancel = !schObj.isSlotAvailable(
    ...
    it('event initial rendering testing', () => {
      ...
      const appointment1: HTMLElement = schObj.element.querySelector(
        '[data-id="Appointment_1"]'
      ) as HTMLElement;
      ...
      const appointment2: HTMLElement = schObj.element.querySelector(
        '[data-id="Appointment_2"]'
      ) as HTMLElement;
    ...
    it('event editing with overlapping', () => {
      ...
      const endDate: DateTimePicker = (
        (<HTMLElement>dialogElement.querySelector('.e-end')) as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
  ...
  describe('closeEditor public method testing', () => {
    ...
    it('Checking for cell double click event editor', () => {
      const workCellElements: HTMLElement[] = [].slice.call(
    ...
    it('Checking for event double click event editor', () => {
      ...
      const editButton: HTMLElement = document.querySelectorAll(
        '.e-edit'
      )[0] as HTMLElement;
        ...
        resources: [
          {
            ...
            dataSource: [
              ...
              {
                CalendarText: 'Birthday',
                CalendarId: 3,
                CalendarColor: '#AF27CD',
              },
              {
                CalendarText: 'Holiday',
                CalendarId: 4,
  ...
  describe('Recurrence Validation', () => {
    ...
    it('Create Error Alert', (done: DoneFn) => {
      ...
      const startDateElement: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_START_CLASS)
      );
      startDateElement.value = '11/19/17 11:00 AM';
      const endDateElement: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_END_CLASS)
      );
  ...
  describe('All CRUD Action Properties as false', () => {
    ...
    it('Ensure New Event Creation', () => {
      ...
      expect(
        schObj.quickPopup.quickPopup.element.classList.contains('e-popup-close')
      ).toEqual(true);
      expect(
        schObj.quickPopup.quickPopup.element.classList.contains('e-popup-open')
      ).toEqual(false);
    });

    it('Ensure Edit Event', () => {
      util.triggerMouseEvent(
        schObj.element.querySelector('[data-id="Appointment_1"]'),
        'click'
      );
      expect(
        schObj.quickPopup.quickPopup.element.classList.contains('e-popup-open')
      ...
      util.triggerMouseEvent(
        schObj.element.querySelector('.e-quick-popup-wrapper .e-edit'),
        'click'
      );
      expect(
        schObj.quickPopup.quickPopup.element.classList.contains('e-popup-open')
    ...
    it('Ensure Delete Event', () => {
      ...
      util.triggerMouseEvent(
        schObj.element.querySelector('.e-quick-popup-wrapper .e-delete'),
        'click'
      );
      expect(
        schObj.quickPopup.quickPopup.element.classList.contains('e-popup-open')
  ...
  describe('CRUD Actions with AllowEditing', () => {
    ...
    it('Ensure Edit Event', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventsData.length).toEqual(5);
        const editedObj: { [key: string]: string } = schObj.eventsData[0] as {
          [key: string]: string;
        };
      ...
      (
        eventDialog.querySelector('.' + cls.SUBJECT_CLASS) as HTMLInputElement
      ).value = 'Test';
      util.triggerMouseEvent(
        eventDialog.querySelector('.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS),
        'click'
      );
    });
  ...
  describe('CRUD Actions with AllowDeleting', () => {
    ...
    it('Ensure Edit Event', () => {
      ...
      util.triggerMouseEvent(
        schObj.quickPopup.quickPopup.element.querySelector('.e-edit'),
        'click'
      );
      expect(
        schObj.quickPopup.quickPopup.element.classList.contains('e-popup-open')
    ...
    it('Ensure Delete Event', (done: DoneFn) => {
      ...
      util.triggerMouseEvent(
        schObj.quickPopup.quickPopup.element.querySelector('.e-delete'),
        'click'
      );
      util.triggerMouseEvent(
        schObj.quickPopup.quickDialog.element.querySelector(
  ...
  describe('All CRUD Action Properties as true', () => {
    ...
    it('Ensure Edit Event', (done: DoneFn) => {
      ...
      const subjectElement: HTMLInputElement = eventDialog.querySelector(
        '.' + cls.SUBJECT_CLASS
      );
      subjectElement.value = 'Test';
      editButton.click();
    ...
    it('Ensure Delete Event', (done: DoneFn) => {
      ...
      const deleteButton: HTMLElement = eventDialog.querySelector(
        '.' + cls.DELETE_EVENT_CLASS
      ) as HTMLElement;
      deleteButton.click();
      const deleteButton1: HTMLInputElement = <HTMLInputElement>(
    ...
    beforeAll((done: DoneFn) => {
      ...
      const onPopupOpen: EmitType<PopupOpenEventArgs> = (
        ...
        if (args.type === 'Editor') {
          ...
          if (
            startElement &&
            !startElement.classList.contains('e-datetimepicker')
          ) {
            new DateTimePicker(
              { value: new Date(startElement.value) || new Date() },
              startElement
          ...
          if (
            endElement &&
            !endElement.classList.contains('e-datetimepicker')
          ) {
            new DateTimePicker(
              { value: new Date(endElement.value) || new Date() },
              endElement
  ...
  describe('EJ2-48253 - checking alert when drag and drop the occurrence event', () => {
    ...
    it('Checking occurrence and same day alert', () => {
      ...
      saveButton.click();
      expect(eventDialog.querySelector('.e-dlg-content').innerHTML).toEqual(
      ...
      const okButton: HTMLElement =
        eventDialog.querySelector('.e-quick-alertok');
      okButton.click();
  ...
  describe('EJ2-51228- Delete following events through editor window', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        currentView: 'Week',
        views: ['Week'],
        selectedDate: new Date(2019, 1, 5),
    ...
    it('Event Click and Edit following series', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element
            .querySelectorAll('.e-appointment')[0]
            .querySelector('.' + cls.SUBJECT_CLASS).innerHTML
        ).toEqual('Testing');
        done();
      };
      expect(
      ...
      (<HTMLElement>(
        eventDialog.querySelector('.e-quick-dialog-following-events')
      )).click();
      const dialogElement: HTMLElement = document.querySelector(
        '.' + cls.EVENT_WINDOW_DIALOG_CLASS
      ) as HTMLElement;
  ...
  describe('EJ2-51055 - Prevent enter key submitting the form', () => {
    ...
    it('Checking prevent enter key in event window - save button', () => {
      ...
      const dialogElement: HTMLElement = document.querySelector(
        '.e-dialog' + '.' + cls.EVENT_WINDOW_DIALOG_CLASS
      ) as HTMLElement;
      const saveButton: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS)
      );
  ...
  describe('To check timezone datasource loaded in the event window', () => {
    ...
    it('To Check if the given datasource loaded in the dropdown list', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          (
            schObj.quickPopup.quickPopup.element.querySelector(
              '.' + cls.TIME_ZONE_CLASS
            ) as HTMLElement
          ).innerText
      ...
      expect((startTZDropDown as HTMLInputElement).value).toEqual(
        schObj.tzModule.getLocalTimezoneName()
      );
      const listObj: any = (startTZDropDown as EJ2Instance)
        .ej2_instances[0] as DropDownList;
      listObj.showPopup();
      ...
      listObj.onInput();
      listObj.onFilterUp(keyEventArgs);
      let element: Element = [].slice
        .call(document.querySelectorAll('.e-dropdownbase .e-list-parent'))
        .slice(-1)[0];
  ...
  describe('EJ2-56895-Editor window check box actions', () => {
    ...
    it('To Check all day and timezone check box actions', () => {
      ...
      expect(
        dialogElement.querySelector(
          '.' + cls.EVENT_WINDOW_TIME_ZONE_DIV_CLASS + '.e-enable'
        )
      ).toBeTruthy();
      timezoneElement.click();
      expect(
      ...
      allDayElement.click();
      expect(
        dialogElement.querySelector('.e-time-zone-container.e-disable')
      ).toBeTruthy();
      allDayElement.click();
      expect(
        dialogElement.querySelector('.e-time-zone-container.e-disable')
      ).toBeFalsy();
      const closeButton: HTMLElement = dialogElement.querySelector(
        '.e-dlg-closeicon-btn'
      ) as HTMLElement;
      closeButton.click();
      schObj.width = '400px';
    ...
    it('EJ2-66573 - Editor window does not open when schedule is rendered with minimum width', () => {
      ...
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-appointment-wrapper')[4]
      ...
      expect(
        schObj.element
          .querySelector('.e-quick-popup-wrapper')
  ...
  describe('EJ2CORE-491-Edit block appointments using editor window', () => {
    ...
    beforeAll((done: DoneFn) => {
      ...
      function onBlockClick(args: Event): void {
        ...
        if (target.classList.contains('e-block-appointment')) {
          currentId = parseInt(
            target.getAttribute('data-id').split('_')[1],
            10
          );
        } else {
          currentId = parseInt(
            target.offsetParent.getAttribute('data-id').split('_')[1],
            10
          );
        }
    ...
    it('Adding a block appointment', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(blockData.RecurrenceRule).toEqual(
      ...
      (
        dialogElement.querySelector('.e-is-block') as EJ2Instance
      ).ej2_instances[0].checked = true;
      (dialogElement.querySelector('.e-subject') as HTMLInputElement).value =
        'Blocked range';
      (
        dialogElement.querySelector('.e-repeat-element') as EJ2Instance
      ).ej2_instances[0].value = 'daily';
      (
        dialogElement.querySelector('.e-end-on-element') as EJ2Instance
      ).ej2_instances[0].value = 'count';
      (
        dialogElement.querySelector('.e-recurrence-count') as EJ2Instance
      ).ej2_instances[0].value = 2;
      (
        dialogElement.querySelector('.e-event-save') as HTMLButtonElement
      ).click();
    });
    it('editing a block appointment', (done: DoneFn) => {
      ...
      util.triggerMouseEvent(
        schObj.element.querySelector('.e-block-appointment') as HTMLElement,
        'click'
      );
      const dialogElement: HTMLElement = document.querySelector(
        '.' + cls.EVENT_WINDOW_DIALOG_CLASS
      ...
      (dialogElement.querySelector('.e-subject') as HTMLInputElement).value =
        'Blocked range edited';
      (
        dialogElement.querySelector('.e-repeat-element') as EJ2Instance
      ).ej2_instances[0].value = 'none';
      (
        dialogElement.querySelector('.e-event-save') as HTMLButtonElement
      ).click();
    });
  ...
  describe('EJ2-58120-openEditor method return incorrect end date testing', () => {
    ...
    it('open editor method manually testing end time', () => {
      ...
      expect(startDate.value.getTime()).toEqual(
  ...
  describe('EJ2-58120-openEditor method return incorrect end date testing for editor template', () => {
      ...
      const model: ScheduleModel = {
        ...
        popupOpen: (args: PopupOpenEventArgs) => {
          if (schObj.editorTemplate && args.type === 'Editor') {
            ...
            if (
              startElement &&
              !startElement.classList.contains('e-datepicker')
            ) {
              new DatePicker(
                { value: new Date(startElement.value) || new Date() },
                startElement
    ...
    it('open editor method manually testing end time in editor template', () => {
      ...
      expect(dialogElement.classList.contains('e-popup-close')).toEqual(false);
      const startDate: DateTimePicker = (
        dialogElement.querySelector('#StartTime') as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
      const endDate: DateTimePicker = (
        dialogElement.querySelector('#EndTime') as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
      expect(startDate.value.getTime()).toEqual(
        new Date(2021, 0, 13).getTime()
      );
    ...
    it('ES-828699 - EndDate is returned wrongly in popup open Event args', () => {
      schObj.popupOpen = (args: PopupOpenEventArgs) => {
        expect(args.data.StartTime.getTime()).toBe(
          new Date(2021, 0, 13).getTime()
        );
        expect(args.data.EndTime.getTime()).toBe(
          new Date(2021, 0, 14).getTime()
        );
        if (schObj.editorTemplate && args.type === 'Editor') {
          const startElement: HTMLInputElement = args.element.querySelector(
            '#StartTime'
  ...
  describe('EJ2-67179 - checking possible alert cases', () => {
    ...
    it('Checking possible alert case for edit first occurrence', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventsData.length).toEqual(2);
        expect(
          schObj.element.querySelector('[data-id="Appointment_2"] .e-subject')
            .innerHTML
        ).toEqual('Edited');
        done();
      };
    ...
    it('Checking possible alert case for edit mid occurrence', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element.querySelectorAll(
            '[data-id="Appointment_3"] .e-subject'
          )[0].innerHTML
        ).toEqual('Edited Mid Occurrence');
        done();
      };
      util.triggerMouseEvent(
        schObj.element.querySelectorAll(
    ...
    it('Checking possible alert case for edit last occurrence', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventsData.length).toEqual(4);
        expect(
          schObj.element.querySelector('[data-id="Appointment_4"] .e-subject')
            .innerHTML
        ).toEqual('Edited Last Occurrence');
        done();
      };
      util.triggerMouseEvent(
        schObj.element.querySelectorAll(
    ...
    it('Checking edit start date of mid occurence, if that date not having occurrence', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventsData.length).toEqual(5);
        expect(
          schObj.element.querySelector('[data-id="Appointment_5"] .e-subject')
            .innerHTML
        ).toEqual('Start date edited for mid occurrence');
        done();
      };
      util.triggerMouseEvent(
        schObj.element.querySelectorAll(
      ...
      endDate.dataBind();
      (dialogElement.querySelector('.e-subject') as HTMLInputElement).value =
        'Start date edited for mid occurrence';
      const saveButton: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS)
      );
      saveButton.click();
    ...
    it('Checking occurrence title edit', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventsData.length).toEqual(6);
        expect(
          schObj.element.querySelector('[data-id="Appointment_6"] .e-subject')
            .innerHTML
        ).toEqual('Edited');
        done();
      };
  ...
  describe('ES-832489 - Edit Occurrence checking', () => {
    ...
    it('Occurrence event edit and check for reschedule of event', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(
          schObj.element.querySelectorAll('.e-all-day-appointment').length
        ).toBe(1);
        expect(
          schObj.element.querySelectorAll('.e-content-wrap .e-appointment')
            .length
        ).toBe(4);
        expect(
          schObj.element.querySelectorAll('.e-recurrence-icon').length
        ).toBe(4);
        expect(
          schObj.element.querySelectorAll('.e-recurrence-edit-icon').length
        ).toBe(1);
        done();
      };
      const appointments: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
      );
      ...
      expect(
        document
          .querySelector('.e-dialog.e-quick-dialog')
          .classList.contains('e-popup-open')
      ).toEqual(true);
      util.triggerMouseEvent(
        document.querySelector('.e-quick-dialog-occurrence-event'),
        'click'
      );
      expect(
        document
      ...
      expect(
        schObj.element.querySelectorAll('.e-content-wrap .e-appointment').length
      ).toBe(5);
      expect(schObj.element.querySelectorAll('.e-recurrence-icon').length).toBe(
        5
      );
      expect(
        schObj.element.querySelectorAll('.e-recurrence-edit-icon').length
      ...
      ).ej2_instances[0].value = new Date(2023, 5, 7, 10, 0);
      util.triggerMouseEvent(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS),
        'click'
      );
  ...
  describe('EJ2-69821- Add event check while customizing the recurrence editor repeat type options in the Schedule editor', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        currentView: 'Week',
        views: ['Week'],
        selectedDate: new Date(2023, 1, 5),
        popupOpen: (args: PopupOpenEventArgs) => {
          if (args.type === 'Editor') {
            (schObj.eventWindow as any).recurrenceEditor.frequencies = [
              'none',
              'daily',
              'weekly',
            ];
  ...
  describe('EJ2-70484 - Numeric text box not destroyed', () => {
    ...
    beforeAll((done: DoneFn) => {
      ...
      const onPopupOpen: EmitType<PopupOpenEventArgs> = (
        ...
        if (args.type === 'Editor') {
          ...
          const priceElement: HTMLInputElement = args.element.querySelector(
            '#Price'
          ) as HTMLInputElement;
    ...
    it('Make sure the numeric text box destroyed', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(destroyedCount).toEqual(
          popupOpenedCount === 0 ? 0 : popupOpenedCount - 1
        );
        if (destroyedCount === 2) {
          done();
        }
  ...
  describe('ES-881659 - XSS vulnerable checking for resources.', () => {
        ...
        resources: [
          {
            ...
            dataSource: [
              {
                text: '<img src=1 href=1 onerror="javascript:alert(1)"></img>',
                id: 1,
                color: '#cb6bb2',
              },
              { text: 'PROJECT 2', id: 2, color: '#56ca85' },
    ...
    it('To multi select selected value - The XSS values rendred as string in value text box', () => {
      ...
      expect(resourceElement.text).toEqual(
  ...
  describe('Customize inner elements in the recurrence editor dynamically', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        popupOpen: (args: PopupOpenEventArgs) => {
          if (args.type === 'Editor') {
            const recurrenceEditor = (schObj.eventWindow as any)
              .recurrenceEditor;
    ...
    it('Save through event window checking', (done: DoneFn) => {
      ...
      setTimeout(() => {
        ...
        expect(
          dialogElement
            .querySelector('.e-end-on-date')
            .classList.contains('e-hide-recurrence-element')
        ).toEqual(false);
        expect(
          dialogElement
            .querySelector('.e-end-on-count')
            .classList.contains('e-hide-recurrence-element')
        ).toEqual(true);
        endOnElement.index = 1;
        endOnElement.dataBind();
  ...
  describe('Checking custom editor header and footer templates', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        editorHeaderTemplate:
          '<div id="dlg-template" title="Edit">${Subject}</div>',
    ...
    it('Custom header and footer templates checking with string', () => {
      ...
      expect(
        dialogElement.querySelector('.e-dlg-header').firstElementChild.innerHTML
      ).toEqual('<div id="dlg-template" title="Edit">undefined</div>');
      expect(
        dialogElement.querySelector('.e-footer-content').firstElementChild
          .innerHTML
      ).toEqual('<div id="location">undefined</div>');
      (
        dialogElement.querySelector('.e-dlg-closeicon-btn') as HTMLElement
      ).click();
      expect(
        (
          [].slice.call(
      ...
      expect(
        dialogElement.querySelector('.e-dlg-header').firstElementChild.innerHTML
      ).toEqual('<div id="dlg-template" title="Edit">Meeting - 1</div>');
      expect(
        dialogElement.querySelector('.e-footer-content').firstElementChild
          .innerHTML
      ).toEqual('<div id="location">Chennai</div>');
      expect(
        (dialogElement.querySelector('#dlg-template') as HTMLElement).innerText
      ).toEqual('Meeting - 1');
      expect(
        (dialogElement.querySelector('#location') as HTMLElement).innerText
      ).toEqual('Chennai');
      (
        dialogElement.querySelector('.e-dlg-closeicon-btn') as HTMLElement
      ).click();
    ...
    it('On property changed in custom editor header and footer template', () => {
      schObj.editorHeaderTemplate =
        '<div id="dlg-template" title="Edit">EditAppointment</div>';
      schObj.editorFooterTemplate =
        '<div><button id="Cancel" class="e-control e-btn e-primary" data-ripple="true">Cancel</button></div>';
      schObj.dataBind();
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[1] as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[1] as HTMLElement,
      ...
      expect(
        dialogElement.querySelector('.e-dlg-header').firstElementChild.innerHTML
      ).toEqual('<div id="dlg-template" title="Edit">EditAppointment</div>');
      expect(
        dialogElement.querySelector('.e-footer-content').firstElementChild
          .innerHTML
      ).toEqual(
  ...
  describe('Checking custom editor header and footer templates with function', () => {
    ...
    const customHeaderTemplate: string =
      '<div id="dlg-template" title="Edit">${Subject}</div>';
  ...
  describe('Checking the custom editor header and footer template in script', () => {
    ...
    beforeAll((done: DoneFn) => {
      ...
      const scriptEleFooter: HTMLScriptElement =
      ...
      const model: ScheduleModel = {
        editorHeaderTemplate: '#custom-header',
        editorFooterTemplate: '#custom-footer',
        height: '500px',
        eventSettings: { dataSource: data },
        currentView: 'Month',
        views: ['Month'],
  ...
  describe('Checking custom editor header and footer template in mobile view', () => {
    ...
    describe('Checking custom editor header and footer template in moble view', () => {
      ...
      it('Custom editor header and footer template checking with string', () => {
        ...
        firstWorkCell.click();
        (
          schObj.element.querySelectorAll('.e-new-event')[0] as HTMLElement
        ).click();
        const dialogElement: HTMLElement = document.querySelector(
          '.' + cls.EVENT_WINDOW_DIALOG_CLASS
        ) as HTMLElement;
        expect(
          dialogElement.querySelector('.e-dlg-header').firstElementChild
            .innerHTML
        ).toEqual('<div id="dlg-template" title="Edit">undefined</div>');
        expect(
          dialogElement.querySelector('.e-footer-content').firstElementChild
        ...
        expect(
          (dialogElement.querySelector('#dlg-template') as HTMLElement)
            .innerText
        ).toEqual('Meeting - 1');
        expect(
          (dialogElement.querySelector('#location') as HTMLElement).innerText
      ...
      it('On property changed in custom editor header and footer template ', () => {
        schObj.editorHeaderTemplate =
          '<div id="dlg-template" title="Edit">EditAppointment</div>';
        schObj.editorFooterTemplate =
          '<div><button id="delete" class="e-control e-btn e-primary" data-ripple="true">Delete</button></div>';
        schObj.dataBind();
        const firstWorkCell: HTMLElement = schObj.element.querySelectorAll(
          '.e-work-cells'
        )[0] as HTMLElement;
      ...
      it('Setting a nullable value to verify the editor header and footer Template', () => {
        ...
        expect(
          dialogElement
            .querySelector('.e-dlg-header')
            .querySelector('.e-back-icon') as HTMLElement
        ).not.toEqual(null);
        (
          dialogElement
            .querySelector('.e-dlg-header')
    ...
    describe('Custom editor heder and footer Template checking with function template in mobile view', () => {
      ...
      const customHeaderTemplate: string =
        '<div id="dlg-template" title="Edit">${Subject}</div>';
      const customFooterTemplate: string =
    ...
    describe('Checking the custom editor header template in script in mobile view', () => {
      ...
      beforeAll((done: DoneFn) => {
        const templateHeader: string =
          '<div id="dlg-template" title="Edit">${Subject}</div>';
        const scriptEleHead: HTMLScriptElement =
          document.createElement('script');
        scriptEleHead.type = 'text/x-template';
        scriptEleHead.id = 'custom-header';
        scriptEleHead.appendChild(document.createTextNode(templateHeader));
    ...
    describe('Checking the custom editor header template in script without footer template in mobile view', () => {
      ...
      it('Custom header and with out footer template checking with script', () => {
        ...
        expect(
          ...
          '<div class="e-back-icon e-icons"></div><div><div id="dlg-template" title="Edit">Meeting - 1</div></div><div class="e-save-icon e-icons"></div>'
        );
        expect(
          (dialogElement.querySelector('#dlg-template') as HTMLElement)
            .innerText
      ...
      it('Custom header and without footer template checking onproperty changed', () => {
        ...
        expect(
          ...
          '<div class="e-back-icon e-icons"></div><div><div id="dlg-template" title="Edit">EditAppointment</div></div><div class="e-save-icon e-icons"></div>'
        );
        expect(dialogElement.querySelector('.e-back-icon')).not.toEqual(null);
        expect(dialogElement.querySelector('.e-save-icon')).not.toEqual(null);
        (dialogElement.querySelector('.e-back-icon') as HTMLElement).click();
