 ...
 */
import { createElement, closest, Browser } from '@hoyui/base';
import {
  Schedule,
  ScheduleModel,
  CellClickEventArgs,
...
describe('Schedule Month view', () => {
  ...
  describe('Initial load', () => {
    ...
    it('work cells', () => {
      ...
      expect(
        ...
        'Month Start Sunday, October 1, 2017 Ends At Saturday, November 4, 2017'
      );
      expect(firstWorkCell.getAttribute('aria-label')).toEqual(
        'Sunday, October 1, 2017'
      );
      expect(firstWorkCell.innerHTML).toEqual(
        '<div class="e-date-header e-navigate">Oct 1</div>'
      );
    });
    ...
    it('disable other month cells', () => {
      schObj.selectedDate = new Date(2017, 11, 4);
      schObj.dataBind();
      expect(schObj.element.querySelectorAll('.e-other-month').length).toEqual(
        11
      );
    });
    ...
    it('horizontal scroll', () => {
      ...
      const headerCellsArea: HTMLElement = schObj.element.querySelector(
        '.e-date-header-wrap'
      ) as HTMLElement;
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  describe('custom month view with displayDate', () => {
        ...
        views: [
          {
            option: 'Month',
            displayName: 'Month',
            displayDate: new Date(2020, 4, 20),
          },
        ],
      };
      schObj = util.createSchedule(model, []);
    ...
    it('start and end dates of custom month view', () => {
      ...
      expect(cells[0].getAttribute('data-date')).toEqual(
        new Date(2020, 4, 17).getTime().toString()
      );
      expect(cells[cells.length - 1].getAttribute('data-date')).toEqual(
        new Date(2020, 5, 27).getTime().toString()
      );
      expect(
        schObj.element.querySelector('.e-date-range .e-tbar-btn-text').innerHTML
      ).toEqual('May - June 2020');
      (
        schObj.element.querySelector('.e-toolbar-item.e-prev') as HTMLElement
      ).click();
    });
    ...
    it('previous month start and end dates of custom month view', () => {
      const cells: NodeListOf<Element> =
        schObj.element.querySelectorAll('.e-work-cells');
      expect(cells[0].getAttribute('data-date')).toEqual(
        new Date(2020, 3, 5).getTime().toString()
      );
      expect(cells[cells.length - 1].getAttribute('data-date')).toEqual(
        new Date(2020, 4, 16).getTime().toString()
      );
      expect(
        schObj.element.querySelector('.e-date-range .e-tbar-btn-text').innerHTML
      ).toEqual('April - May 2020');
      (
        schObj.element.querySelector('.e-toolbar-item.e-next') as HTMLElement
      ).click();
    });
  ...
  describe('custom month view with numberOfWeeks', () => {
    ...
    beforeAll(() => {
      const model: ScheduleModel = {
        views: [{ option: 'Month', displayName: 'Month', numberOfWeeks: 6 }],
    ...
    it('start and end dates of custom month view', () => {
      const cells: NodeListOf<Element> =
        schObj.element.querySelectorAll('.e-work-cells');
      expect(cells[0].getAttribute('data-date')).toEqual(
        new Date(2020, 3, 26).getTime().toString()
      );
      expect(cells[cells.length - 1].getAttribute('data-date')).toEqual(
        new Date(2020, 5, 6).getTime().toString()
      );
      expect(
        schObj.element.querySelector('.e-date-range .e-tbar-btn-text').innerHTML
      ).toEqual('April - June 2020');
      (
        schObj.element.querySelector('.e-toolbar-item.e-prev') as HTMLElement
      ).click();
    });
    ...
    it('previous month start and end dates of custom month view', () => {
      const cells: NodeListOf<Element> =
        schObj.element.querySelectorAll('.e-work-cells');
      expect(cells[0].getAttribute('data-date')).toEqual(
        new Date(2020, 2, 15).getTime().toString()
      );
      expect(cells[cells.length - 1].getAttribute('data-date')).toEqual(
        new Date(2020, 3, 25).getTime().toString()
      );
      expect(
        schObj.element.querySelector('.e-date-range .e-tbar-btn-text').innerHTML
      ).toEqual('March - April 2020');
      (
        schObj.element.querySelector('.e-toolbar-item.e-next') as HTMLElement
      ).click();
    });
  ...
  describe('Current Day Highlight testing', () => {
    ...
    it('checking default current day highlight', () => {
      expect(
        schObj.element
          .querySelectorAll('.e-header-cells')
          .item(new Date().getDay()).classList
    ...
    it('checking current day highlight with different firstDayOfWeek', () => {
      ...
      const index: number = schObj.activeView.renderDates
        .slice(0, 7)
        .map((date: Date) => date.getDay())
        .indexOf(new Date().getDay());
      expect(
  ...
  describe('Dependent properties', () => {
    ...
    it('show weekend', () => {
      ...
      expect(
        schObj.element.querySelector('.e-work-cells .e-date-header').innerHTML
      ).toEqual('2');
      (
        schObj.element.querySelector('.e-toolbar-item.e-next') as HTMLElement
      ).click();
    ...
    it('cell template', () => {
      ...
      const workCellEle: HTMLElement = createElement('div', {
        innerHTML:
          '<div class="e-date-header e-navigate">4</div><span>10/4/17, 12:00 AM</span>',
      });
      ...
      expect(workCell.getAttribute('aria-label')).toEqual(
    ...
    it('check current date class', () => {
      ...
      schObj = util.createSchedule(model, []);
      expect(
        schObj.element.querySelector('.e-current-day').classList
      ).toContain('e-header-cells');
      expect(
        schObj.element.querySelector('.e-current-date').classList
      ).toContain('e-work-cells');
    });
    ...
    it('header cell click day view navigation', () => {
      ...
      expect(workCell.innerHTML).toEqual(
        '<div class="e-date-header e-navigate">Oct 1</div>'
      );
      ...
      expect(
        ...
        '<div class="e-header-day">Sun</div><div class="e-header-date e-navigate" role="link">1</div>'
      );
    ...
    it('disable header bar', () => {
      ...
      schObj = util.createSchedule(model, []);
      expect(
        schObj.element.querySelectorAll('.e-schedule-toolbar-container').length
      ).toEqual(0);
    });
    ...
    it('minDate and maxDate', () => {
      ...
      expect(nextButton.getAttribute('aria-disabled')).toEqual('true');
      (
        schObj.element.querySelectorAll('.e-date-header')[7] as HTMLElement
      ).click();
      expect(schObj.currentView).toEqual('Month');
      (
        schObj.element.querySelectorAll('.e-date-header')[0] as HTMLElement
      ).click();
  ...
  describe('Client side events', () => {
    ...
    it('date navigating', () => {
      ...
      const navArgs: NavigatingEventArgs = {
        ...
        previousDate: new Date(2017, 9, 5),
      };
      let args: NavigatingEventArgs;
      let beginArgs: ActionEventArgs;
      let completeArgs: ActionEventArgs;
      const model: ScheduleModel = {
      ...
      expect(completeArgs).toEqual(
        jasmine.objectContaining(actionCompleteArgs)
      );
    });
  ...
  describe('Public methods', () => {
    ...
    it('interval count', () => {
      ...
      let workCell: HTMLElement = schObj.element.querySelectorAll(
        '.e-work-cells'
      )[6] as HTMLElement;
      expect(workCell.getAttribute('aria-label')).toEqual(
      ...
      expect(workCell.innerHTML).toEqual(
        '<div class="e-date-header e-navigate">7</div>'
      );
      (
        schObj.element.querySelector('.e-toolbar-item.e-next') as HTMLElement
      ).click();
      ...
      workCell = schObj.element.querySelectorAll(
        '.e-work-cells'
      )[6] as HTMLElement;
      expect(workCell.getAttribute('aria-label')).toEqual(
      ...
      expect(workCell.innerHTML).toEqual(
        '<div class="e-date-header e-navigate">2</div>'
      );
      (
        schObj.element.querySelector('.e-toolbar-item.e-prev') as HTMLElement
      ).click();
  ...
  describe('Resources with group', () => {
    ...
    it('resource cells count', () => {
      expect(
        schObj.element.querySelectorAll('.e-date-header-wrap .e-resource-cells')
          .length
      ).toBe(5);
    });
    ...
    it('work cells count', () => {
      expect(
        schObj.element.querySelectorAll('.e-content-wrap .e-work-cells').length
      ).toBe(105);
    });

    it('work day cells count', () => {
      expect(
        schObj.element.querySelectorAll('.e-content-wrap .e-work-days').length
      ).toBe(75);
    });
  });
  ...
  describe('Resource header template of Resources in group', () => {
    ...
    it('template text', () => {
      expect(
        schObj.element.querySelector('.e-date-header-wrap .e-resource-cells')
          .innerHTML
      ).toBe('<p>ROOM 1</p>');
    });
  ...
  describe('Events rendering with rowAutoHeight property', () => {
    ...
    describe('CR Issue EJ2- Getting Week numbers while changing firstdayofWeek', () => {
      ...
      it('Week number testing for when firstdayofWeek set to Sunday', () => {
        ...
        expect(
          schObj.element.querySelectorAll('.e-week-number')[1].innerHTML
        ).toBe('45');
      });
  ...
  describe('Getting Week numbers while changing weekRule property', () => {
    ...
    it('Week number testing for when weekRule set to FirstFourDayWeek', () => {
      ...
      schObj.dataBind();
      expect(
        schObj.element.querySelectorAll('.e-week-number')[4].innerHTML
      ).toBe('53');
    });
  ...
  describe('More indicator event rendering based on the provided template', () => {
    ...
    beforeAll((done: DoneFn) => {
      const mt: string =
        '<div class="template-wrap" style="background:#007EE3;width:-webkit-fill-available;">' +
        '<div class="subject" style="background:#007EE3">${Subject}</div></div>';
      const scriptMonthEvent: HTMLScriptElement =
      ...
      const viewCollection: ViewsModel[] = [
        { option: 'Month', eventTemplate: '#month-event-template' },
      ];
      const schOptions: ScheduleModel = {
        height: '900px',
    ...
    it('Checking the template applied to the events or not', () => {
      const moreIndicator: HTMLElement =
        document.querySelector('.e-more-indicator');
      ...
      expect(templateElement).toEqual(
        '<div class="template-wrap" style="background:#007EE3;' +
  ...
  describe('Testing month view cell header template', () => {
    ...
    it('Checking cell header template on month view', () => {
      const headerElements: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-date-header')
      );
      expect(headerElements[0].innerHTML.toString()).toEqual(
        '<span>10/29/2017</span>'
      );
      expect(headerElements[6].innerHTML.toString()).toEqual(
        '<span>11/4/2017</span>'
      );
      expect(headerElements[17].innerHTML.toString()).toEqual(
        '<span>11/15/2017</span>'
      );
      expect(headerElements[26].innerHTML.toString()).toEqual(
        '<span>11/24/2017</span>'
      );
    });
    it('Checking cell header template on month agenda view', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(headerElements[12].innerHTML.toString()).toEqual(
          '<span>11/10/2017</span>'
        );
        expect(headerElements[24].innerHTML.toString()).toEqual(
