...
describe('Schedule year view', () => {
  ...
  describe('Initial load', () => {
    ...
    it('view class on container', () => {
      ...
      expect(schObj.element.getAttribute('aria-label')).toEqual(
        'Year Start Friday, January 1, 2021 Ends At Friday, December 31, 2021'
      );
    });
    ...
    it('check header elements', () => {
      expect(
        schObj.element.querySelectorAll('.e-month-calendar').length
      ).toEqual(12);
      expect(
        (schObj.element.querySelectorAll('.e-header')[0] as HTMLElement)
          .innerHTML
      ).toEqual('<div class="e-day e-title">January 2021</div>');
      expect(schObj.element.querySelectorAll('.e-week-header').length).toEqual(
        12
      );
    });
    ...
    it('work cells', () => {
      ...
      ).toEqual(new Date(2020, 11, 27).getTime().toString());
      expect(
        schObj.element.querySelectorAll('.e-work-cells')[0].innerHTML
      ).toEqual(
        '<span class="e-day" title="Sunday, December 27, 2020" aria-disabled="true">27</span>'
      );
    });
    ...
    it('check week number', () => {
      ...
      schObj.dataBind();
      expect(schObj.element.querySelectorAll('.e-week-number').length).toEqual(
        72
      );
      expect(
        schObj.element
          .querySelectorAll('.e-week-number')[0]
          .getAttribute('title')
      ).toEqual('Week 1');
      expect(
        schObj.element.querySelectorAll('.e-week-number')[0].innerHTML
      ).toEqual('1');
    });
    ...
    it('cell click', () => {
      ...
      expect(
        morePopup.firstElementChild.classList.contains('e-more-event-popup')
      ).toBeTruthy();
      expect(
        schObj.element
          .querySelector('.e-more-event-popup')
          .firstElementChild.classList.contains('e-more-event-header')
      ).toBeTruthy();
      const moreEventHeader: HTMLElement = schObj.element.querySelector(
        '.e-more-event-header'
      );
      expect(moreEventHeader.firstElementChild.getAttribute('role')).toEqual(
        'button'
      );
      expect(
        moreEventHeader.firstElementChild.classList.contains(
          'e-more-event-close'
        )
      ).toBeTruthy();
      expect(moreEventHeader.firstElementChild.innerHTML).toEqual(
        '<span class="e-btn-icon e-icons e-close-icon"></span>'
      );
      expect(
        moreEventHeader.lastElementChild.classList.contains(
          'e-more-event-date-header'
        )
      ).toBeTruthy();
      const moreDateHeader: HTMLElement = schObj.element.querySelector(
        '.e-more-event-date-header'
      );
      expect(
        moreDateHeader.firstElementChild.classList.contains('e-header-day')
      ).toBeTruthy();
      expect(moreDateHeader.firstElementChild.getAttribute('id')).toEqual(
        morePopup.getAttribute('aria-labelledby')
      );
      expect(moreDateHeader.firstElementChild.innerHTML).toEqual('Fri');
      expect(
        moreDateHeader.lastElementChild.classList.contains('e-header-date')
      ).toBeTruthy();
      expect(moreDateHeader.lastElementChild.innerHTML).toEqual('1');
      expect(moreDateHeader.lastElementChild.getAttribute('tabindex')).toEqual(
        '0'
      );
      expect(moreDateHeader.lastElementChild.getAttribute('data-date')).toEqual(
        new Date(2021, 0, 1).getTime().toString()
      );
    });
  ...
  describe('Resource and normal Header Template in timeline year view', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        monthHeaderTemplate:
          '<div class="date-text">${(data.date).getMonth()}</div>',
        views: [
          { option: 'TimelineYear', displayName: 'Horizontal Year' },
          {
            option: 'TimelineYear',
    ...
    it('HorizontalView template checking', () => {
      ...
      expect(
        ...
        '<div class="tWrap"><div class="rText" style="background:pink">ROOM 1</div></div>'
      );
      expect(schObj.element.querySelector('.e-tbar-btn-text').innerHTML).toBe(
        'May 2018 - Apr 2019'
      );
      expect(
        schObj.element.querySelectorAll('.e-month-header')[0].innerHTML
      ).toBe('<div class="date-text">4</div>');
      expect(
        schObj.element.querySelectorAll('.e-month-header')[11].innerHTML
      ).toBe('<div class="date-text">3</div>');
      expect(
        schObj.element.querySelector('.e-content-table').getAttribute('id')
      ).toEqual(schObj.element.getAttribute('aria-labelledby'));
      const verticalViewBtn: HTMLElement = schObj.element.querySelectorAll(
        '.e-toolbar-item.e-views.e-timeline-year'
      )[1] as HTMLElement;
      verticalViewBtn.click();
    });
    it('VerticalView template checking', () => {
      expect(schObj.firstMonthOfYear).toEqual(4);
      expect(schObj.element.querySelector('.e-header-cells').innerHTML).toBe(
        '<div class="date-text">4</div>'
      );
      expect(
        schObj.element.querySelectorAll('.e-header-cells')[11].innerHTML
      ).toBe('<div class="date-text">3</div>');
      expect(
        schObj.element.querySelectorAll('.e-resource-cells')[0].innerHTML
      ).toBe(
        '<div class="tWrap"><div class="rText" style="background:pink">ROOM 1</div></div>'
      );
      expect(
        schObj.element.querySelectorAll('.e-resource-cells')[1].innerHTML
      ).toBe(
        '<div class="tWrap"><div class="rText" style="background:pink">ROOM 2</div></div>'
      );
    });
    ...
    describe('Resource and normal Header without Template in timeline year view', () => {
      ...
      it('Horizontal without template checking', () => {
        ...
        expect(
          schObj.element.querySelectorAll('.e-month-header')[0].innerHTML
        ).toBe('<span>May 2018</span>');
        expect(
          schObj.element.querySelectorAll('.e-month-header')[11].innerHTML
        ).toBe('<span>April 2019</span>');
        expect(
          schObj.element.querySelectorAll('.e-resource-cells')[1].innerHTML
        ).toBe('<div class="e-text-ellipsis">ROOM 1</div>');
        expect(
          schObj.element.querySelectorAll('.e-resource-cells')[2].innerHTML
        ).toBe('<div class="e-text-ellipsis">ROOM 2</div>');
        const verticalViewBtn: HTMLElement = schObj.element.querySelectorAll(
          '.e-toolbar-item.e-views.e-timeline-year'
        )[1] as HTMLElement;
        verticalViewBtn.click();
      ...
      it('Vertical without template checking', () => {
        ...
        expect(
          ...
          '<div class="e-resource-text" style="margin-left: 0px;">ROOM 2</div>'
        );
        expect(
          schObj.element.querySelectorAll('.e-resource-cells')[0].innerHTML
        ).toBe(
          '<div class="e-resource-text" style="margin-left: 0px;">ROOM 1</div>'
        );
        expect(
          schObj.element.querySelectorAll('.e-header-cells')[0].innerHTML
        ).toBe('<span>May 2018</span>');
        expect(
          schObj.element.querySelectorAll('.e-header-cells')[11].innerHTML
        ).toBe('<span>April 2019</span>');
    ...
    describe('with Template without Resource functionalities', () => {
      ...
      it('Horizontal view functionalities checking', () => {
        ...
        expect(
          schObj.element.querySelectorAll('.e-header-cells')[1].innerHTML
        ).toBe('<div class="date-text">Sunday</div>');
        expect(
          schObj.element.querySelectorAll('.e-header-cells')[2].innerHTML
        ).toBe('<div class="date-text">Monday</div>');
        const verticalViewBtn: HTMLElement = schObj.element.querySelectorAll(
          '.e-toolbar-item.e-views.e-timeline-year'
        )[1] as HTMLElement;
        verticalViewBtn.click();
      ...
      it('Vertical view functionalities checking', () => {
        ...
        expect(schObj.firstMonthOfYear).toEqual(4);
        expect(
          schObj.element.querySelectorAll('.e-header-cells')[12].innerHTML
        ).toBe('<div class="date-text">3</div>');
        expect(
          schObj.element.querySelectorAll('.e-header-cells')[1].innerHTML
        ).toBe('<div class="date-text">4</div>');
        expect(
          schObj.element.querySelectorAll('.e-month-header')[1].innerHTML
        ).toBe('<div class="date-text">Monday</div>');
        expect(
          schObj.element.querySelectorAll('.e-month-header')[2].innerHTML
        ).toBe('<div class="date-text">Tuesday</div>');
      });
    });
    ...
    describe('without Template without Resource functionalities', () => {
      ...
      it('Horizontal view functionalities checking', () => {
        ...
        expect(
          schObj.element.querySelectorAll('.e-month-header')[0].innerHTML
        ).toBe('<span>May 2021</span>');
        expect(
          schObj.element.querySelectorAll('.e-month-header')[11].innerHTML
        ).toBe('<span>April 2022</span>');
      ...
      it('Horizontal setMode change checking', () => {
        ...
        expect(
          schObj.element.querySelectorAll('.e-month-header')[0].innerHTML
        ).toBe('<span>June 2021</span>');
        expect(
          schObj.element.querySelectorAll('.e-month-header')[11].innerHTML
        ).toBe('<span>May 2022</span>');
        const verticalViewBtn: HTMLElement = schObj.element.querySelectorAll(
          '.e-toolbar-item.e-views.e-timeline-year'
        )[1] as HTMLElement;
        verticalViewBtn.click();
      ...
      it('Vertical view functionalities checking', () => {
        ...
        const yearViewBtn: HTMLElement = schObj.element.querySelector(
          '.e-toolbar-item.e-views.e-year'
        );
        yearViewBtn.click();
      });
      it('Year layout rendering', () => {
        expect(
          schObj.element.querySelectorAll('.e-day.e-title')[0].innerHTML
        ).toBe('June 2021');
        expect(
          schObj.element.querySelectorAll('.e-day.e-title')[6].innerHTML
        ).toBe('December 2021');
        expect(
          schObj.element.querySelectorAll('.e-day.e-title')[11].innerHTML
        ).toBe('May 2022');
      });
    });
  });
  ...
  describe('checking template supports in year', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        dayHeaderTemplate:
          '<div class="date-text">${(data.date).getDay()}</div>',
        monthHeaderTemplate:
          '<div class="date-text">${(data.date).getMonth()}</div>',
        cellTemplate: '<div class="date-text">${(data.date).getDate()}</div>',
        cellHeaderTemplate:
          '<div class="date-text">${(data.date).getDate()}</div>',
        dateRangeTemplate:
    ...
    it('All templates checking', () => {
      expect(
        schObj.element.querySelectorAll('.e-day.e-title')[0]['innerHTML']
      ).toBe('<div class="date-text">0</div>');
      expect(
        schObj.element.querySelectorAll('.e-week-header')[0]['innerHTML']
      ).toBe(
        '<tr><th><div class="date-text">0</div></th><th><div class="date-text">1</div></th><th><div class="date-text">2</div></th><th><div class="date-text">3</div></th><th><div class="date-text">4</div></th><th><div class="date-text">5</div></th><th><div class="date-text">6</div></th></tr>'
      );
      expect(
        schObj.element.querySelectorAll('.e-cell.e-work-cells')[0]['innerHTML']
      ).toBe('<div class="date-text">26</div><div class="date-text">26</div>');
      expect(
        schObj.element.querySelectorAll('.e-cell.e-work-cells')[6]['innerHTML']
      ).toBe('<div class="date-text">1</div><div class="date-text">1</div>');
      expect(
        schObj.element.querySelectorAll('.e-tbar-btn-text')[0]['innerHTML']
      ).toBe('<div class="date-text">1-31</div>');
    });
    it('remove daterange', () => {
      expect(
        schObj.element.querySelector('.e-toolbar-left').children.length
      ).toEqual(3);
  ...
  describe('Client side events', () => {
    ...
    it('events call confirmation', () => {
      ...
      schObj = util.createSchedule(model, []);
      (
        schObj.element.querySelectorAll('.e-work-cells')[6] as HTMLElement
      ).click();
    ...
    it('cell click', () => {
      ...
      schObj = util.createSchedule(model, []);
      (
        schObj.element.querySelectorAll('.e-work-cells')[7] as HTMLElement
      ).click();
  ...
  describe('EJ2-51013 - Year view Resources with group by date', () => {
    ...
    it('Checking ByDate for horizontal timeline year view', () => {
      ...
      expect(schObj.element.querySelector('.e-table-wrap').classList).toContain(
        'e-timeline-year-view'
      );
      expect(
        schObj.element.querySelectorAll(
          '.e-date-header-wrap .e-schedule-table tr'
        ).length
      ).toBe(1);
      expect(
    ...
    it('Checking ByDate for vertical timeline year view', () => {
      (
        schObj.element.querySelectorAll(
          '.e-schedule-toolbar .e-timeline-year'
        )[1] as HTMLElement
      ).click();
      expect(
        schObj.element.querySelector('.e-active-view').classList
  ...
  describe('EJ2-49947 - scheduler more popup is not updated when deleting the event in year view', () => {
    ...
    it('Checking the more popup event list after deleting an event', (done: DoneFn) => {
      ...
      (<HTMLElement>(
        schObj.element.querySelectorAll('.e-work-cells')[46]
      )).click();
      const morePopup: HTMLElement = schObj.element.querySelector(
        '.e-more-popup-wrapper'
      );
  ...
  describe('EJ2CORE-692 - Year view week number title test case', () => {
    ...
    it('vietnamese test case', () => {
      ...
      const weekNoElements: NodeListOf<Element> =
        schObj.element.querySelectorAll('.e-year-view .e-week-number');
  ...
  describe('EJ2-45830 - Display multi-month event as single event in vertical year view', () => {
    ...
    it('After changing number of months', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          appointment.firstElementChild.classList.contains('e-left-icon')
        ).toBeTruthy();
        expect(
          appointment.children[2].classList.contains('e-right-icon')
        ).toBeTruthy();
        done();
      };
  ...
  describe('EJ2-57740 - Checking the scroll position maintenance in Timeline year view', () => {
    ...
    it('After the initial rendering', () => {
      ...
      const monthWrap: HTMLElement = schObj.element.querySelector(
        '.' + cls.MONTH_HEADER_WRAPPER
      );
    ...
    it('Ensure the changed position', () => {
      ...
      const monthArea: HTMLElement = schObj.element.querySelector(
        '.' + cls.MONTH_HEADER_WRAPPER
      );
  ...
  describe('Checking week header names', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        views: [{ option: 'Year', isSelected: true }],
        selectedDate: new Date(2023, 1, 24),
      };
      schObj = util.createSchedule(model, [], done);
    });
    ...
    it('Checking headers for year view', () => {
      ...
      expect(Element.querySelector('.e-title').innerHTML).toEqual(
        'November 2023'
      );
      expect(
        Element.querySelector('.e-week-header').querySelectorAll(
          '.e-content.e-month > table > thead > tr > th'
        )[0].innerHTML
      ).toEqual('S');
      expect(
        Element.querySelector('.e-week-header').querySelectorAll(
  ...
  describe('Checking aria-label with resource group', () => {
    ...
    it('Checking aria-label in vertical orientation', () => {
      expect(
        ...
        'Wednesday, January 1, 2020 at 12:00:00 AM GMT Ends At Saturday, February 1, 2020 at 12:00:00 AM GMT'
      );
    });
  ...
  describe('ES-907744 - FirstDayOfWeek property is not working in scheduler year view', () => {
    ...
    it('Checking the firstDayOfWeek property in the year view for view-specific', () => {
      ...
      expect(
        schObj.element
          .querySelectorAll('.e-month-calendar.e-calendar')[0]
          .querySelector('.e-work-cells .e-day').outerHTML
      ).toBe(
        '<span class="e-day" title="Monday, December 28, 2020" aria-disabled="true">28</span>'
      );
      expect(
        schObj.element
          .querySelectorAll('.e-month-calendar.e-calendar')[0]
          .querySelector('.e-work-cells .e-day')
          .getAttribute('title')
