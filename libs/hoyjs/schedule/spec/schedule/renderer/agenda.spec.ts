...
describe('Agenda View', () => {
  ...
  describe('Checking without datasource', () => {
    ...
    it('Initial load testing', () => {
      ...
      expect(schObj.activeView.getStartHour().getTime()).toEqual(
        startEndHour.getTime()
      );
      expect(schObj.activeView.getEndHour().getTime()).toEqual(
        startEndHour.getTime()
      );
    });
    ...
    it('Checking Agenda elements count', () => {
      expect(
        schObj.element.querySelectorAll('.e-agenda-parent').length
      ).toEqual(0);
    });

    it('Checking selected Date', () => {
      expect(util.resetTime(schObj.selectedDate)).toEqual(
        util.resetTime(new Date(2017, 10, 1))
      );
    });
    ...
    it('Checking Header text', () => {
      expect(
        schObj.element.querySelector('.e-date-range').firstElementChild
          .textContent
    ...
    it('Checking prev next icon disabled', () => {
      expect(
        schObj.element.querySelector('.e-prev').classList.contains('e-hidden')
      ).toEqual(true);
      expect(
        schObj.element.querySelector('.e-next').classList.contains('e-hidden')
      ).toEqual(true);
    });

    it('Checking Header text', () => {
      const element: HTMLElement = schObj.element.querySelector(
        '.e-empty-event'
      ) as HTMLElement;
      ...
      expect(
        schObj.element.querySelector('.e-date-range').firstElementChild
          .textContent
      ).toEqual('November 01 - 07, 2017');
    });

    it('Checking previous navigation', () => {
      const prevEle: HTMLElement = schObj.element.querySelector(
        '.e-prev'
      ) as HTMLElement;
      prevEle.click();
      expect(
        schObj.element.querySelector('.e-date-range').firstElementChild
          .textContent
      ).toEqual('Oct 31 - Nov 06, 2017');
    });

    it('Checking next navigation', () => {
      const nextEle: HTMLElement = schObj.element.querySelector(
        '.e-next'
      ) as HTMLElement;
      nextEle.click();
      expect(
    ...
    it('Checking virtual scrolling with hideEmptyAgendaDays', (done: DoneFn) => {
      schObj.dataBound = () => {
        const scrollUp: HTMLElement = schObj.element.querySelector(
          '.e-content-wrap'
        ) as HTMLElement;
        ...
        const patentTd: Element = closest(
          schObj.element.querySelectorAll('.e-agenda-parent')[0] as Element,
          'td'
        );
        expect(patentTd.getAttribute('data-date')).toEqual(
          new Date(2017, 9, 25).getTime().toString()
        );
        done();
      };
      expect(
      ...
      const td: Element = closest(
        schObj.element.querySelectorAll('.e-agenda-parent')[0] as Element,
        'td'
      );
      expect(td.getAttribute('data-date')).toEqual(
    ...
    it('Checking Dateheader color', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element
            .querySelector('.e-current-day')
            .parentElement.getAttribute('data-date')
        ).toEqual(dateString);
        done();
      };
  ...
  describe('EJ2-54150 Checking with getCurrentViewEvents for the Agenda view', () => {
    ...
    it('current view events checking', () => {
      ...
      const currentViewEvent: Record<string, any>[] =
        schObj.getCurrentViewEvents();
  ...
  describe('Checking with datasource', () => {
    ...
    it('Checking list elements', () => {
      const agendaEle: Element =
        schObj.element.querySelector('.e-content-wrap tr');
      ...
      expect(agendaListApp.children[0].className).toEqual(
        'e-agenda-parent e-ul e-agenda-view'
      );
      expect(
        agendaListApp.children[0].childElementCount
      ).toBeGreaterThanOrEqual(1);
      expect(agendaListApp.children[0].children[0].className).toEqual(
        'e-agenda-item e-agenda-view e-level-1'
      );
      expect(agendaListApp.children[0].children[0].childElementCount).toEqual(
        1
      );
      expect(
        agendaListApp.children[0].children[0].children[0].className
      ).toEqual('e-appointment');
      expect(
        agendaListApp.children[0].children[0].children[0].childElementCount
      ).toEqual(2);
      expect(
        agendaListApp.children[0].children[0].children[0].children[0].className
      ).toEqual('e-subject-wrap');
      expect(
        agendaListApp.children[0].children[0].children[0].children[1].className
      ).toEqual('e-date-time');

      const scrollElement: HTMLElement = schObj.element.querySelector(
        '.e-content-wrap'
      ) as HTMLElement;
      const scrollUp: HTMLElement = schObj.element.querySelector(
        '.e-content-wrap'
      ...
      triggerScrollEvent(scrollUp, 0);
      const scrollElement1: HTMLElement = schObj.element.querySelector(
        '.e-content-wrap'
      ) as HTMLElement;
      expect(scrollElement1.querySelectorAll('tr').length).toBeLessThanOrEqual(
        15
      );
    });
    ...
    it('Checking virtual scrolling with hideEmptyAgendaDays', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(patentTd.getAttribute('data-date')).toEqual(
          new Date(2017, 9, 23).getTime().toString()
        );
        done();
      };
      schObj.hideEmptyAgendaDays = false;
    ...
    it('home key', () => {
      ...
      const target: Element = schObj.element.querySelector(
        '.e-content-wrap table tr td[tabindex="0"]'
      );
      expect(target.getAttribute('data-date')).toEqual(
        new Date('Fri Dec 28 2018').getTime().toString()
      );
    });
    ...
    it('Tab key functionality checking', () => {
      ...
      expect(target[0].getAttribute('data-guid')).toEqual(
        document.activeElement.getAttribute('data-guid')
      );
    });

    it('Day view navigation checking', () => {
      const firstDateHeader: HTMLElement = schObj.element.querySelector(
        '.e-m-date'
      ) as HTMLElement;
  ...
  describe('Checking Virtual scrolling with events after long days from initial date navigation', () => {
    ...
    beforeAll((done: DoneFn) => {
      ...
      const data: Record<string, any>[] =
        cloneDataSource(defaultData).concat(scheduleDatas);
    ...
    it('Checking virtual scrolling with hideEmptyAgendaDays', () => {
      const eventWraps: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-agenda-parent')
      );
      expect(eventWraps.length).toEqual(16);
      const firstEventTd: Element = closest(eventWraps[0] as Element, 'td');
      expect(parseInt(firstEventTd.getAttribute('data-date'), 10)).toEqual(
        new Date(2017, 10, 10).getTime()
      );
      expect(firstEventTd.getAttribute('data-column-index')).toEqual('-21');
      const lastEventTd: Element = closest(
        eventWraps[eventWraps.length - 1] as Element,
        'td'
      );
      expect(parseInt(lastEventTd.getAttribute('data-date'), 10)).toEqual(
        new Date(2019, 0, 16).getTime()
      );
  ...
  describe('Agenda view rendering with templates', () => {
    ...
    it('Checking date template', () => {
      const agendaEle: Element =
        schObj.element.querySelector('.e-content-wrap').children[0].children[0]
          .children[0];
      expect(agendaEle.childElementCount).toEqual(2);
      const agendaDate: HTMLElement = agendaEle.children[0] as HTMLElement;
      expect(agendaDate.className).toEqual('e-agenda-cells');
    ...
    it('Checking dateRange template', () => {
      ...
      expect(
        schObj.element.querySelector('.e-toolbar-items').children[0].className
      ).toBe('e-toolbar-left');
      const dateRangeEle: Element =
        schObj.element.querySelector('.e-tbar-btn-text');
      schObj.dateRangeTemplate =
        '<div class="date-text">${(data.startDate).getMonth()}-${(data.endDate).getMonth()}</div>';
      schObj.dataBind();
      expect(dateRangeEle.innerHTML).toEqual(
        '<div class="date-text">9-10</div>'
      );
      schObj.dateRangeTemplate =
      ...
      ).toEqual('<div>10/30/17, 12:00 AM-11/5/17, 12:00 AM</div>');
      schObj.dateRangeTemplate =
        '<div>${(data.startDate).getDate()}-${(data.endDate).getDate()}</div>';
      schObj.dataBind();
      expect(
    ...
    it('Checking appointment template', (done: DoneFn) => {
      schObj.dataBound = () => {
        const agendaEle: Element =
          schObj.element.querySelector('.e-content-wrap').children[0]
        ...
        expect(
          agendaListApp.children[0].children[0].children[0].className
        ).toEqual('e-appointment e-template');
        expect(
          agendaListApp.children[0].children[0].children[0].children[0]
            .className
        ).toEqual('e-icons e-recurrence-icon');
        expect(
          agendaListApp.children[0].children[0].children[0].childElementCount
        ).toEqual(4);
        done();
  ...
  describe('Agenda view spanned events checking', () => {
    ...
    it('Checking spanned events', () => {
      const agendaAppointment: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
      );
  ...
  describe('Checking Event loading on current view', () => {
    ...
    it('test current page event loading', () => {
      ...
      expect(
        schObj.element.querySelector('.e-date-range').firstElementChild
          .textContent
      ).toEqual('April 08 - 14, 2018');
    });

    it('test check event loading on previous click', (done: DoneFn) => {
      schObj.dataBound = () => {
        const agendaAppointments: HTMLElement[] = [].slice.call(
          schObj.element.querySelectorAll('.e-appointment')
        );
  ...
  describe('Agenda view rendering with multiple resource', () => {
    ...
    const restemplate: string =
      '<div class="template-wrap"></div><div style="background:pink">${getResourceName(data)}</div>';
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        width: '100%',
        height: '550px',
    ...
    it('Checking resource rendering with EmptyAgendaDays as false', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        const emptyDiv: HTMLElement = emptyTd.children[0].children[0]
    ...
    it('Checking resource rendering with empty agenda collection', (done: DoneFn) => {
      schObj.dataBound = () => {
        const row1: Element =
          schObj.element.querySelector('.e-content-wrap').children[0]
            .children[0].children[0];
        const emptyDiv: HTMLElement = row1.children[0]
          .children[0] as HTMLElement;
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        views: [
          {
            option: 'Day',
            group: { byGroupID: false, resources: ['Rooms', 'Owners'] },
          },
  ...
  describe('EJ2-26070-Event not rendered properly in agenda view when we give datasource and date format as string', () => {
    ...
    it('Checking event rendered or not using string datasource', () => {
      ...
      expect(
        eventElements[0].querySelector('.e-date-time').textContent
      ).toEqual('All day (Day 1/2)');
      expect(
        eventElements[1].querySelector('.e-date-time').textContent
      ).toEqual('12:00 AM - 11:59 PM (Day 2/2)');
    });
  });
  ...
  describe('Checking minDate and maxDate', () => {
    ...
    it('minDate and maxDate- date navigation testing - 1', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element
            .querySelector('.e-content-table')
            .getAttribute('aria-label')
        ).toEqual(
          'Agenda Start Sunday, October 29, 2017 Ends At Saturday, November 4, 2017'
        );
        expect(
        ...
        expect(
          schObj.element
            .querySelector('.' + cls.PREVIOUS_DATE_CLASS + ' button')
            .getAttribute('aria-disabled')
        ).toEqual('true');
        expect(
          schObj.element
            .querySelector('.' + cls.NEXT_DATE_CLASS + ' button')
            .getAttribute('aria-disabled')
        ).toEqual('false');
        done();
      };
  ...
  describe('EJ2-55921 Agenda view events edit action checking on readonly set to true', () => {
    ...
    it('Checking event edit action', () => {
      ...
      expect(
        document.querySelector('.e-schedule-dialog.e-popup-open')
      ).toBeFalsy();
      expect(
        schObj.element.querySelector('.e-popup-open .e-event-popup')
      ).toBeTruthy();
      (
        schObj.element.querySelector(
          '.e-popup-open .e-event-popup .e-close'
  ...
  describe('Current date indication in Resource group in Agenda view ', () => {
    ...
    it('Cheking Current date indication', () => {
      const agendacellElement: HTMLElement = schObj.element.querySelector(
        '.e-agenda-cells'
      ) as HTMLElement;
      expect(agendacellElement.classList.contains('e-date-column')).toEqual(
        true
      );
      expect(
        agendacellElement.firstElementChild.classList.contains('e-current-day')
      ).toEqual(true);
    });
  });
  ...
  describe('ES-870019 - Hide/Show weekend and non working days based on showWeekend', () => {
    ...
    it('Checking initial rendering', () => {
      expect(
        schObj.element.querySelector('.e-date-range').firstElementChild
          .textContent
      ).toEqual('November 06 - 12, 2017');
      expect(
        schObj.element.querySelector(
          '.e-agenda-cells[data-date="1510358400000"]'
        )
      ).toBeFalsy();
      expect(
        schObj.element.querySelector(
          '.e-agenda-cells[data-date="1510444800000"]'
        )
      ).toBeFalsy();
    });
    ...
    it('Checking previous navigation rendering', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element.querySelector(
            '.e-agenda-cells[data-date="1509840000000"]'
          )
        ).toBeFalsy();
        expect(
          schObj.element.querySelector(
    ...
    it('Checking previous navigation with virtual scrolling and showWeekend true', () => {
      ...
      expect(
        schObj.element.querySelector(
          '.e-agenda-cells[data-date="1509753600000"]'
        )
      ).toBeTruthy();
      expect(
        schObj.element.querySelector(
    ...
    it('Checking next navigation with virtual scrolling and showWeekend true', () => {
      ...
      expect(
        schObj.element.querySelector(
          '.e-agenda-cells[data-date="1510963200000"]'
        )
      ).toBeTruthy();
    });
    ...
    const eventData: Record<string, any>[] = [
      {
        ...
        RecurrenceRule:
          'FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR;INTERVAL=1;UNTIL=20181231T033000Z;',
        OwnerId: [1, 2, 3],
      },
      {
        Id: 2,
        ...
        EndTime: new Date(2017, 0, 1, 12),
        RecurrenceRule:
          'FREQ=WEEKLY;BYDAY=SU,SA;INTERVAL=1;UNTIL=20181231T033000Z;',
        OwnerId: [1, 2, 3],
      },
    ];
    beforeAll((done: DoneFn) => {
