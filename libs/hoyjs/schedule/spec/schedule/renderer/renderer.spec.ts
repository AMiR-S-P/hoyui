...
describe('Data module', () => {
  ...
  describe('Locale data testing', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        eventSettings: { query: new Query().take(5) },
      };
  ...
  describe('refreshTemplate checking', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        views: [{ option: 'TimelineDay', interval: 3 }, { option: 'Month' }],
      };
    ...
    it('refresh templates testing after scrolling in TimelineDay view', (done: DoneFn) => {
      ...
      ).toEqual(schObj.element.querySelector('.e-content-wrap').scrollLeft);
  ...
  describe('refreshTemplates checking on virtual scrolling', () => {
    let schObj: Schedule;
    const resourceHeaderTemplate: string =
      '<span>ResourceHeaderTemplate</span>';
    ...
    it('refresh templates of resourceHeader testing after scrolling in Timeline views', (done: DoneFn) => {
      ...
      expect(
        schObj.element.querySelector('.e-resource-column-wrap').scrollTop
      ).toEqual(schObj.element.querySelector('.e-content-wrap').scrollTop);
      done();
    });
  });
