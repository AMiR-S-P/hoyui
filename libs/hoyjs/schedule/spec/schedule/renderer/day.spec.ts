...
describe('Schedule day view', () => {
  ...
  describe('Initial load', () => {
    ...
    it('check date header cells text', () => {
      ...
      expect(
        ...
        '<div class="e-header-day">Wed</div><div class="e-header-date e-navigate" role="link">4</div>'
      );
    });
    ...
    it('work cells', () => {
      ...
      ).toEqual('Day of Wednesday, October 4, 2017');
    });
    ...
    it('navigate next date', () => {
      ...
      expect(
        ...
        '<div class="e-header-day">Thu</div><div class="e-header-date e-navigate" role="link">5</div>'
      );
      expect(
        schObj.element
          .querySelector('.e-content-table')
          .getAttribute('aria-label')
      ).toEqual('Day of Thursday, October 5, 2017');
    });
  ...
  describe('Dependent properties', () => {
    ...
    it('show weekend', () => {
      ...
      expect(
        ...
        '<div class="e-header-day">Fri</div><div class="e-header-date e-navigate" role="link">6</div>'
      );
      (
        schObj.element.querySelector('.e-toolbar-item.e-next') as HTMLElement
      ).click();
      ...
      expect(
        ...
        '<div class="e-header-day">Sat</div><div class="e-header-date e-navigate" role="link">7</div>'
      );
      (
        schObj.element.querySelector('.e-toolbar-item.e-next') as HTMLElement
      ).click();
  ...
  describe('Client side events', () => {
    ...
    it('validate start and end time on multi cell select', () => {
      ...
      expect(schObj.activeCellsData.startTime).toEqual(
        new Date(2017, 9, 5, 1, 30, 0)
      );
      expect(schObj.activeCellsData.endTime).toEqual(
        new Date(2017, 9, 5, 3, 0, 0)
      );
      expect(eventName).toEqual('select');
    });
  ...
  describe('Public methods', () => {
    ...
    it('getCellDetails', () => {
      ...
      schObj = util.createSchedule(model, []);
      let data: CellClickEventArgs = schObj.getCellDetails(
      ...
      expect(data.endTime.getTime()).toEqual(
        new Date(2017, 9, 5, 0, 30).getTime()
      );
      expect(data.isAllDay).toEqual(false);

      data = schObj.getCellDetails(
        schObj.element.querySelector('.e-date-header-wrap .e-all-day-cells')
      );
      expect(data.startTime.getTime()).toEqual(new Date(2017, 9, 5).getTime());
    ...
    it('setWorkHours', () => {
      ...
      expect(
        (workHourCells[0].parentElement as HTMLTableRowElement).rowIndex
      ).toEqual(10);
      expect(
        (
          workHourCells[workHourCells.length - 1]
    ...
    it('interval count', () => {
      const model: ScheduleModel = {
        ...
        width: '500px',
        currentView: 'Day',
        views: [{ option: 'Day', interval: 3 }],
        selectedDate: new Date(2017, 9, 4),
      };
      schObj = util.createSchedule(model, []);
      expect(
        schObj.element.querySelector('.e-date-range .e-tbar-btn-text').innerHTML
      ).toEqual('October 04 - 06, 2017');
      expect(
        schObj.element.querySelectorAll(
          '.e-date-header-container .e-header-cells'
        ).length
  ...
  describe('Default schedule block events', () => {
    ...
    it('add event', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        const addedAppointment: HTMLElement = schObj.element.querySelector(
    ...
    it('edit event', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventWindow.dialogObject.visible).toEqual(false);
        const editedAppointment: HTMLElement = schObj.element.querySelector(
          '[data-id="Appointment_15"]'
        ) as HTMLElement;
  ...
  describe('Multi level resource rendering in block events', () => {
    ...
    it('resource add event', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventWindow.dialogObject.visible).toEqual(false);
        const addAppointment: HTMLElement = schObj.element.querySelector(
