...
describe('Month-agenda view rendering', () => {
  ...
  describe('Schedule month agenda view', () => {
    ...
    describe('Initial load', () => {
      ...
      it('view class on container', () => {
        expect(
          schObj.element.querySelector('.e-month-agenda-view')
        ).toBeTruthy();
      });
      ...
      it('work cells', () => {
        ...
        expect(
          ...
          'Month Agenda Start Sunday, October 1, 2017 Ends At Saturday, November 4, 2017'
        );
        expect(firstWorkCell.innerHTML).toEqual(
          '<div class="e-date-header">1</div>'
        );
      });
      ...
      it('navigate next date', () => {
        util.triggerSwipeEvent(
          schObj.element.querySelector('.e-table-container'),
          100
        );
        expect(
          schObj.element.querySelectorAll(
      ...
      it('disable other month cells', () => {
        ...
        schObj.dataBind();
        expect(
          schObj.element.querySelectorAll('.e-other-month').length
        ).toEqual(11);
      });

      it('appointment details at bottom', () => {
        expect(
          schObj.element.querySelectorAll('.e-appointment-wrap').length
        ).toEqual(1);
      });
    });
    ...
    describe('Dependent properties', () => {
      ...
      it('cell template', () => {
        ...
        schObj = util.createSchedule(model, []);
        expect(
          schObj.element.querySelectorAll('.custom-element').length
        ).toEqual(schObj.getWorkCellElements().length);
        const workCellEle: HTMLElement = createElement('div', {
          innerHTML:
            '<div class="e-date-header">4</div><span>10/4/17, 12:00 AM</span>',
        });
      ...
      it('header cell click Day view navigation', () => {
        ...
        expect(schObj.element.querySelector('.e-work-cells').innerHTML).toEqual(
          '<div class="e-date-header">1</div>'
        );
      ...
      it('Checking appointment wrapper height', (done: DoneFn) => {
        ...
        schObj.dataBound = () => {
          expect(
            (<HTMLElement>schObj.element.querySelector('.e-appointment-wrap'))
              .offsetHeight
          ).toEqual(266);
          done();
        };
  ...
  describe('EJ2-52001 - Week Number rendered not properly when height is auto in MonthAgenda view', () => {
    ...
    it('Week number testing for when height is auto', () => {
      expect(
        (schObj.element.querySelector('.e-week-number-wrapper') as HTMLElement)
          .offsetHeight
      ).toBe(202);
      expect(
        (
  ...
  describe('Events rendering', () => {
    ...
    it('events icon rendering', () => {
      expect(
        schObj.element.querySelectorAll('.e-work-cells')[1].children[1]
          .classList
      ).toContain('e-appointment-indicator');
      expect(
        schObj.element.querySelectorAll(
      ...
      expect(
        schObj.element.querySelector('.e-appointment-wrap .e-agenda-item')
          .classList
      ).toContain('e-month-agenda-view');
      expect(
        schObj.element.querySelector('.e-appointment-wrap .e-agenda-parent')
          .classList
      ).toContain('e-month-agenda-view');
      expect(
        schObj.element.querySelectorAll('.e-appointment-wrap .e-agenda-item')
          .length
      ).toEqual(4);
      const lastWorkCell: HTMLElement = schObj.element.querySelectorAll(
        '.e-work-cells'
      )[
        schObj.element.querySelectorAll('.e-work-cells').length - 1
      ] as HTMLElement;
      lastWorkCell.click();
      expect(
        schObj.element.querySelector('.e-appointment-wrap .e-no-event')
          .classList
      ).toContain('e-appointment-container');
      expect(
        schObj.element.querySelectorAll('.e-appointment-wrap .e-no-event')[0]
          .innerHTML
      ).toEqual('No events');
      const workCell: HTMLElement = schObj.element.querySelectorAll(
        '.e-work-cells'
      ...
      expect(
        schObj.element.querySelectorAll(
          '.e-appointment-wrap .e-agenda-item .e-appointment .e-subject'
        )[0].innerHTML
      ).toEqual('Vacation');
    });
    ...
    it('leftArrow key testing', () => {
      ...
      expect(targetCell.classList.contains('e-appointment-border')).toEqual(
        true
      );
  ...
  describe('Month Agenda view rendering with multiple resource', () => {
    ...
    it('Checking resource rendering with resource data', () => {
      const row1: Element = schObj.element.querySelector('.e-appointment-wrap')
      ...
      const row2: Element = schObj.element.querySelector('.e-appointment-wrap')
    ...
    it('Checking resource rendering with empty data', () => {
      ...
      lastWorkCell.click();
      const emptyDiv: Element = schObj.element.querySelector(
        '.e-appointment-wrap'
      ).children[0];
    ...
    it('events list issue', (done: DoneFn) => {
      ...
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[25] as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[25] as HTMLElement,
      ...
      util.triggerMouseEvent(
        schObj.eventWindow.dialogObject.element.querySelector('.e-event-save'),
        'click'
      );
    });
  });
  ...
  describe('Month Agenda view with different interval count', () => {
    ...
    it('checking height of content wrapper and appointment wrapper with different intervals', () => {
      ...
      expect(
        (<HTMLElement>schObj.element.querySelector('.e-content-wrap'))
          .offsetHeight
      ).toEqual(426);
      const conHeight: number =
        schObj.element.querySelectorAll('.e-content-wrap tr').length === 5
          ? 202
          : 242;
      expect(
        (<HTMLElement>(
          schObj.element.querySelector('.e-content-wrap').firstElementChild
        )).offsetHeight
      ).toEqual(conHeight);
