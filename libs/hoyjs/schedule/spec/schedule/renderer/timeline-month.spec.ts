...
import { createElement, Browser, closest } from '@hoyui/base';
...
describe('Schedule Timeline Month view', () => {
  ...
  describe('Initial load', () => {
    ...
    it('view class on container', () => {
      expect(
        schObj.element.querySelector('.e-timeline-month-view')
      ).toBeTruthy();
    });
    ...
    it('check date header cells count', () => {
      expect(
        schObj.element.querySelectorAll(
          '.e-date-header-container .e-date-header'
        ).length
      ).toEqual(30);
    });
    ...
    it('check date header cells text', () => {
      expect(
        schObj.element.querySelector('.e-date-header-container .e-date-header')
          .innerHTML
      ).toEqual(
        '<span class="e-navigate" title="Wednesday, November 1, 2017">Nov 1</span>'
      );
    });
    ...
    it('work cells', () => {
      ...
      expect(firstWorkCell.getAttribute('data-date')).toEqual(
        new Date(2017, 10, 1).getTime().toString()
      );
      ...
      expect(
        ...
        'Timeline Month Start Wednesday, November 1, 2017 Ends At Thursday, November 30, 2017'
      );
    });
    ...
    it('EJ2-29887 - Checking Calendar format', () => {
      ...
      const calendarCells: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-cell')
      );
      expect(calendarCells.length).toEqual(12);
      expect(calendarCells[0].innerHTML).toEqual(
        '<span title="Jan 2018" class="e-day">Jan</span>'
      );
      expect(calendarCells[1].innerHTML).toEqual(
        '<span title="Feb 2018" class="e-day">Feb</span>'
      );
      expect(calendarCells[10].innerHTML).toEqual(
        '<span title="Nov 2018" class="e-day">Nov</span>'
      );
      expect(calendarCells[11].innerHTML).toEqual(
        '<span title="Dec 2018" class="e-day">Dec</span>'
      );
    });
  });
  ...
  describe('Work hour highlight', () => {
    ...
    it('ensure work days highlight', () => {
      expect(schObj.element.querySelectorAll('.e-work-cells').length).toEqual(
        30
      );
      expect(schObj.element.querySelectorAll('.e-work-days').length).toEqual(
        22
      );
    });

    it('ensure highlight class names', () => {
      const workCell: Element =
        schObj.element.querySelectorAll('.e-work-cells')[2];
      expect(workCell.classList.contains('e-work-days'));
    });

    it('ensure not highlight class names', () => {
      const workCell: Element =
        schObj.element.querySelectorAll('.e-work-cells')[3];
  ...
  describe('Work days', () => {
    ...
    it('ensure work days', () => {
      expect(
        schObj.element
          .querySelectorAll('.e-work-cells')[5]
          .classList.contains('e-work-days')
      );
    });
    ...
    it('ensure not work days', () => {
      expect(
        schObj.element
          .querySelectorAll('.e-work-cells')[8]
          .classList.contains('e-work-days')
      ).toBeFalsy();
    });
    ...
    it('ensure date', () => {
      expect(
        ...
        '<span class="e-navigate" title="Wednesday, January 3, 2018">3</span>'
      );
    });
  ...
  describe('Show weekend', () => {
    ...
    it('check date header cells text', () => {
      expect(
        ...
        '<span class="e-navigate" title="Monday, April 2, 2018">2</span>'
      );
    });
  ...
  describe('Current Day Highlight testing', () => {
    ...
    it('checking default current day highlight', () => {
      const index: number = schObj.activeView.renderDates
        .map((date: Date) => date.getDate())
        .indexOf(new Date().getDate());
      expect(
        schObj.element.querySelectorAll('.e-header-cells').item(index).classList
      ).toContain('e-current-day');
    });
  ...
  describe('Checking rowAutoHeight property', () => {
    ...
    it('initial load', () => {
      ...
      const moreIndicatorList: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-more-indicator')
      );
  ...
  describe('Dependent properties', () => {
    ...
    it('show weekend', () => {
      ...
      expect(
        ...
        '<span class="e-navigate" title="Monday, April 3, 2017">3</span>'
      );
      (
        schObj.element.querySelector('.e-toolbar-item.e-next') as HTMLElement
      ).click();
      expect(
        ...
        '<span class="e-navigate" title="Monday, May 1, 2017">May 1</span>'
      );
      (
        schObj.element.querySelector('.e-toolbar-item.e-next') as HTMLElement
      ).click();
      expect(
        ...
        '<span class="e-navigate" title="Thursday, June 1, 2017">Jun 1</span>'
      );
    ...
    it('cell template', () => {
      ...
      expect(schObj.element.querySelectorAll('.custom-element').length).toEqual(
        schObj.getWorkCellElements().length
      );
    ...
    it('dateRange template', () => {
      ...
      expect(
        schObj.element.querySelector('.e-tbar-btn-text').innerHTML
      ).toEqual('<div>10/1/17, 12:00 AM-10/31/17, 12:00 AM</div>');
    });
    ...
    it('events template', (done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        dataBound: () => {
          ...
          expect(schObj.eventsData.length).toEqual(21);
          expect(
            eventElementList[0].querySelector('.e-appointment-details')
              .innerHTML
          ).toEqual(
  ...
  describe('Week Number and firstDayofWeek Combination', () => {
    ...
    it('Timeline Day', () => {
      ...
      expect(
        schObj.element.querySelector('.e-active-view').classList
      ).toContain('e-timeline-day');
      let headTrs: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-date-header-wrap tr')
      );
      expect(
        schObj.element.querySelector('.e-date-range .e-tbar-btn-text').innerHTML
      ).toEqual('January 1, 2019');
    ...
    it('TimelineWorkWeek', () => {
      ...
      expect(
        schObj.element.querySelector('.e-active-view').classList
      ).toContain('e-timeline-work-week');
      let headTrs: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-date-header-wrap tr')
      );
      expect(
  ...
  describe('Public methods', () => {
    ...
    it('interval count', () => {
      ...
      expect(
        ...
        '<span class="e-navigate" title="Thursday, January 4, 2018">4</span>'
      );
      (
        schObj.element.querySelector('.e-toolbar-item.e-prev') as HTMLElement
      ).click();
    ...
    it('scrollTo', (done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        selectedDate: new Date(2017, 9, 1),
        dataBound: () => {
  ...
  describe('Client side events', () => {
    ...
    it('validate start and end time on multi cell select', () => {
      ...
      expect(schObj.activeCellsData.startTime).toEqual(
        new Date(2018, 5, 4, 0, 0, 0)
      );
      expect(schObj.activeCellsData.endTime).toEqual(
        new Date(2018, 5, 7, 0, 0, 0)
      );
      expect(eventName).toEqual('select');
    });
  ...
  describe('RTL', () => {
    ...
    it('check date header cells', () => {
      ...
      expect(
        ...
        '<span class="e-navigate" title="Friday, November 3, 2017">3</span>'
      );
    });
    ...
    it('navigate next date', () => {
      ...
      expect(
        ...
        '<span class="e-navigate" title="Saturday, December 2, 2017">2</span>'
      );
    });
    ...
    it('navigate previous date', () => {
      ...
      expect(
        ...
        '<span class="e-navigate" title="Tuesday, November 7, 2017">7</span>'
      );
    });
  ...
  describe('Single level resource rendering', () => {
    ...
    it('checking resource tree rendering', () => {
      const contentRow: HTMLElement = schObj.element.querySelector(
        '.e-timeline-month-view table tbody'
      ) as HTMLElement;
    ...
    it('Checking Left icon', () => {
      ...
      expect(
        eventElementList[1].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Spanned Event - Less than 24');
      expect(
        eventElementList[1]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
          .classList.contains('e-left-icon')
  ...
  describe('Multi level resource rendering with expanded property', () => {
    ...
    it('checking resource title rendering', () => {
      ...
      expect(headerRow.children.length).toEqual(2);
      expect(
        headerRow.children[0].classList.contains('e-resource-left-td')
      ).toEqual(true);
    ...
    it('cell single click', () => {
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[100] as HTMLElement,
        'click'
      );
      const cellPopup: HTMLElement = schObj.element.querySelector(
        '.e-quick-popup-wrapper'
    ...
    it('event save by clicking cell', (done: DoneFn) => {
      const eventWrap: HTMLElement = schObj.element.querySelector(
        '.e-event-table div:nth-child(3)'
      );
      const event: HTMLElement[] = [].slice.call(
        eventWrap.querySelectorAll('.e-appointment')
      );
      ...
      const workCell: HTMLElement = schObj.element.querySelector(
        '.e-content-table tr:nth-child(3) td:nth-child(3)'
      ) as HTMLElement;
      util.triggerMouseEvent(workCell, 'click');
      util.triggerMouseEvent(workCell, 'dblclick');
      const dialogElement: HTMLElement = document.querySelector(
      ...
      const saveButton: HTMLElement = dialogElement.querySelector(
        '.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS
      ) as HTMLElement;
      saveButton.click();
      schObj.dataBound = () => {
        const eventWrap: HTMLElement = schObj.element.querySelector(
    ...
    it('Checking resource icon click through enter key', () => {
      const resourceRow: Element = schObj.element.querySelector(
        '.e-resource-column-wrap tbody'
      );
      const beforeExpand: NodeListOf<Element> = schObj.element.querySelectorAll(
        '.e-resource-column-wrap tbody tr:not(.e-hidden)'
      ...
      const firstRow: Element =
        resourceRow.children[1].querySelector('.e-resource-cells');
  ...
  describe('Custom work days of Resources in group', () => {
    ...
    it('work days count', (done: DoneFn) => {
      ...
      expect(
        (<Element>contentTable[1].childNodes[1]).classList.contains(
          'e-work-days'
        )
      ).toEqual(false);
      expect(
        (<Element>contentTable[1].childNodes[0]).classList.contains(
          'e-work-days'
        )
      ).toEqual(true);
      (
  ...
  describe('Multiple resource grouping rendering compact view in mobile device ', () => {
    ...
    it('initial layout testing', () => {
      ...
      expect(workCells[12].getAttribute('data-date')).toEqual(
        new Date(2018, 3, 13).getTime().toString()
      );
    });
  ...
  describe('Grouped events', () => {
    ...
    it('Checking left icon', () => {
      ...
      expect(
        eventElementList[10].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Recurrence Event - Previous week');
      expect(
        eventElementList[10]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
  ...
  describe('Year, Month, Week, Day, Hour header rows', () => {
    ...
    it('check month rows', () => {
      ...
      expect(headTrs[1].children[0].innerHTML).toEqual(
        '<span class="e-header-month">May</span>'
      );
    });
    ...
    it('check day rows', () => {
      ...
      expect(headTrs[3].children[0].innerHTML).toEqual(
        '<span class="e-navigate" title="Tuesday, May 1, 2018">May 1</span>'
      );
    ...
    it('navigate next date', (done: DoneFn) => {
      ...
      schObj.dataBound = () => {
        ...
        expect(headTrs[1].children[0].innerHTML).toEqual(
          '<span class="e-header-month">June</span>'
        );
        expect(headTrs[2].children.length).toEqual(5);
        expect(headTrs[2].children[0].getAttribute('colSpan')).toEqual('2');
        expect(headTrs[2].children[0].innerHTML).toEqual(
          '<span class="e-header-week">22</span>'
        );
        expect(headTrs[3].children.length).toEqual(30);
        expect(headTrs[3].children[0].getAttribute('colSpan')).toEqual('1');
        expect(headTrs[3].children[0].innerHTML).toEqual(
          '<span class="e-navigate" title="Friday, June 1, 2018">Jun 1</span>'
        );
        expect(headTrs[3].children[0].getAttribute('data-date')).toEqual(
          new Date(2018, 5, 1).getTime().toString()
        );
        expect(schObj.getWorkCellElements().length).toEqual(30);
        const firstWorkCell: HTMLElement = schObj.element.querySelector(
          '.e-work-cells'
  ...
  describe('Year, Month, Week, Day, Hour header rows with single resource', () => {
    ...
    it('check more indicator', () => {
      ...
      expect(moreIndicators[0].getAttribute('data-start-date')).toEqual(
        new Date(2018, 4, 3).getTime().toString()
      );
      expect(moreIndicators[0].getAttribute('data-end-date')).toEqual(
        new Date(2018, 4, 4).getTime().toString()
      );
      moreIndicators[0].click();
      const morePopup: HTMLElement = schObj.element.querySelector(
        '.e-more-popup-wrapper'
  ...
  describe('Default schedule block events', () => {
    ...
    it('add event', (done: DoneFn) => {
      ...
      util.triggerMouseEvent(
        schObj.element.querySelector('.e-work-cells') as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelector('.e-work-cells') as HTMLElement,
    ...
    it('edit event', (done: DoneFn) => {
      ...
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-appointment')[2] as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-appointment')[2] as HTMLElement,
    ...
    it('checking block event with enableAdativeRows property', () => {
      const blockEventElement: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-block-appointment')
      );
      expect(blockEventElement.length).toEqual(3);
      const cellHeight: number = (
        schObj.element.querySelector('.e-content-table tbody td') as HTMLElement
      ).offsetHeight;
      expect((blockEventElement[0] as HTMLElement).offsetHeight).toBe(
        cellHeight
      );
    });
  });
  ...
  describe('Multi level resource rendering in block events', () => {
    ...
    it('checking block event with enableAdativeRows property', () => {
      ...
      const cellHeight: number = (
        schObj.element.querySelector(
          '.e-content-table tbody tr:nth-child(2) td'
        ) as HTMLElement
      ).offsetHeight;
      expect((blockEventElement[0] as HTMLElement).offsetHeight).toBe(
        cellHeight
      ...
      const resorucCellHeight: number = (
        schObj.element.querySelector(
          '.e-resource-column-table tbody tr:nth-child(2) td'
        ) as HTMLElement
      ).offsetHeight;
      expect(resorucCellHeight).toEqual(cellHeight);
      const blockIndicator: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-block-indicator')
      );
  ...
  describe('EJ2-25919 - Option to get clicked date in Month view', () => {
      ...
      const schOptions: ScheduleModel = {
        ...
        actionBegin: (args: ActionEventArgs) => {
          if (args.requestType === 'viewNavigate') {
            viewDate = new Date(
              parseInt(
                closest(args.event.target as Element, 'td').getAttribute(
                  'data-date'
                ),
                10
              )
    ...
    it('timeline month view to timeline day view change', (done: DoneFn) => {
      ...
      const dateHeader: HTMLElement = schObj.element.querySelector(
        '.e-header-cells:nth-child(5) .e-navigate'
      ) as HTMLElement;
      expect(schObj.currentView).toEqual('TimelineMonth');
  ...
  describe('Testing start and end time of longer event in timeline month view', () => {
    ...
    const eventTemplate: string =
      '<div class="time">(Start date : ${data.StartTime.getDate()}.${(data.StartTime.getMonth() + 1)})' +
    ...
    it('Checking template start and end time', () => {
      ...
      expect(event[0].getAttribute('data-id')).toEqual('Appointment_1');
      expect(event[0].children[0].children[1].innerHTML).toEqual(
        '(Start date : 1.10) - (End date : 1.12)'
      );
      expect(event[1].getAttribute('data-id')).toEqual('Appointment_2');
      expect(event[1].children[0].children[1].innerHTML).toEqual(
        '(Start date : 12.10) - (End date : 12.11)'
      );
      expect(event[2].getAttribute('data-id')).toEqual('Appointment_3');
      expect(event[2].children[1].children[0].innerHTML).toEqual(
        '(Start date : 12.11) - (End date : 12.12)'
      );
    });
  });
  ...
  describe('Ensure Scroll Left for Selected Date', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        height: '500px',
        currentView: 'TimelineMonth',
        selectedDate: new Date(2017, 8, 24),
        views: ['Day', 'Week', 'TimelineMonth', 'Month'],
      };
      schObj = util.createSchedule(model, timelineData, done);
    });
  ...
  describe('Initial load with height auto', () => {
    ...
    it('Checking vertical scrollbar', (done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        dataBound: () => {
          const contentArea: HTMLElement = schObj.element.querySelector(
            '.' + cls.CONTENT_WRAP_CLASS
          ) as HTMLElement;
          const isVerticalScrollBar: boolean =
  ...
  describe('Testing the Appointment rendering for longer appointments renders after shorter one', () => {
    ...
    it('Before rendering longer appointments', () => {
      expect(
        schObj.element.querySelectorAll('.e-more-indicator').length
      ).toEqual(0);
    });
  ...
  describe('Performance checking for TimelineMonth', () => {
    ...
    it('Checking inital load performance for 20 resource 400 events', (done: DoneFn) => {
      ...
      const res: Record<string, any>[] = generateResourceDatasource(
        1,
        20,
        'Resource'
      );
  ...
  describe('EJ2-56141-Timeline month view header rows cells misalignment issue', () => {
    ...
    it('check cell misalignment', () => {
      const dateHeaders: NodeListOf<Element> = schObj.element.querySelectorAll(
        '.e-header-cells.e-date-header'
      );
      const workCells: NodeListOf<Element> =
        schObj.element.querySelectorAll('.e-work-cells');
      expect(dateHeaders.length).toEqual(365);
      expect(workCells.length).toEqual(365);
      expect(
        (dateHeaders[dateHeaders.length - 1] as HTMLElement).offsetLeft
      ).toEqual((workCells[workCells.length - 1] as HTMLElement).offsetLeft);
    });
  });
  ...
  describe('EJ2-57740 - Checking the scroll position maintenance in Timeline month view', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        width: '500px',
        height: '550px',
        selectedDate: new Date(2020, 4, 20),
        views: [{ option: 'TimelineMonth' }],
        group: {
          byGroupID: false,
          resources: ['Owners'],
    ...
    it('After the initial rendering', () => {
      const contentWrap: HTMLElement = schObj.element.querySelector(
        '.' + cls.CONTENT_WRAP_CLASS
      );
      const resourceWrap: HTMLElement = schObj.element.querySelector(
        '.' + cls.RESOURCE_COLUMN_WRAP_CLASS
      );
    ...
    it('Ensure the changed position', () => {
      ...
      const resourceArea: HTMLElement = schObj.element.querySelector(
        '.' + cls.RESOURCE_COLUMN_WRAP_CLASS
      );
  ...
  describe('Timeline views resource rendering without horizontal scrollbar', () => {
    ...
    it('checking the horizontal scrollbar rendering', () => {
      const elementClick: HTMLElement = schObj.element.querySelector(
        '.' + cls.RESOURCE_EXPAND_CLASS
      );
      elementClick.click();
      const contentArea: HTMLElement = schObj.element.querySelector(
        '.' + cls.CONTENT_WRAP_CLASS
