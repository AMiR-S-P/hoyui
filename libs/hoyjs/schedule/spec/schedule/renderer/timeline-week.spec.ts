...
import { DateTimePicker } from '@hoyui/calendars';
...
describe('Schedule Timeline Week view', () => {
  beforeAll(() => {
    const isDef: (o: any) => boolean = (o: any) =>
      o !== undefined && o !== null;
  ...
  describe('Initial load', () => {
    ...
    it('check active view class on toolbar views', () => {
      expect(
        schObj.element.querySelector('.e-active-view').classList
      ).toContain('e-timeline-week');
    });
    ...
    it('check date header cells text', () => {
      expect(
        schObj.element.querySelectorAll(
          '.e-date-header-container .e-header-cells'
        ).length
      ).toEqual(1 * 7);
      expect(
        schObj.element.querySelector('.e-header-date.e-navigate').innerHTML
      ).toEqual('Oct 1, Sunday');
    });

    it('time cells', () => {
      expect(
        schObj.element.querySelectorAll('.e-header-row .e-time-cells').length *
          2
      ).toEqual(
        schObj.element.querySelectorAll(
          '.e-content-wrap .e-content-table tbody td'
        ).length
      );
      expect(
        schObj.element.querySelectorAll('.e-header-row td')[7].innerHTML
      ).toEqual('<span>12:00 AM</span>');
    });

    it('work cells', () => {
      const firstWorkCell: HTMLElement = schObj.element.querySelector(
        '.e-work-cells'
      ) as HTMLElement;
      expect(firstWorkCell.getAttribute('aria-selected')).toEqual('false');
      expect(schObj.element.getAttribute('aria-labelledby')).toEqual(
        schObj.element
          .querySelector('.e-schedule-table.e-content-table')
          .getAttribute('id')
      );
      expect(
        schObj.element
        ...
        'Timeline Week Start Sunday, October 1, 2017 Ends At Saturday, October 7, 2017'
      );
      expect(firstWorkCell.getAttribute('aria-label')).toEqual(
        'Sunday, October 1, 2017 at 12:00:00 AM GMT Ends At Sunday, October 1, 2017 at 12:30:00 AM GMT'
      );
      expect(firstWorkCell.getAttribute('data-date')).toEqual(
        new Date(2017, 9, 1).getTime().toString()
      );
    ...
    it('navigate next date', () => {
      (
        schObj.element.querySelector('.e-toolbar-item.e-next') as HTMLElement
      ).click();
      expect(
        schObj.element.querySelectorAll(
          '.e-date-header-container .e-header-cells'
    ...
    it('navigate previous date', () => {
      (
        schObj.element.querySelector('.e-toolbar-item.e-prev') as HTMLElement
      ).click();
      expect(
        schObj.element.querySelectorAll(
          '.e-date-header-container .e-header-cells'
    ...
    it('date header position', () => {
      const contentArea: HTMLElement = schObj.element.querySelector(
        '.e-content-wrap'
      ) as HTMLElement;
    ...
    it('work hours highlight', () => {
      expect(schObj.element.querySelectorAll('.e-work-hours').length).toEqual(
        18 * 5
      );
    });
    ...
    it('Checking events elements', (done: DoneFn) => {
      schObj.dataBound = () => {
        const eventElementList: Element[] = [].slice.call(
          schObj.element.querySelectorAll('.e-appointment')
        );
        expect(eventElementList.length).toEqual(25);
        const eventWrapperList: Element[] = [].slice.call(
          schObj.element.querySelectorAll('.e-appointment-wrapper')
        );
    ...
    it('Checking left indicator icons', () => {
      ...
      expect(
        eventElementList[1].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Spanned Event - Previous week');
      expect(
        eventElementList[1]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
          .classList.contains('e-left-icon')
      ).toBeTruthy();
    });
    ...
    it('Checking right indicator icon', () => {
      ...
      expect(
        eventElementList[21]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
          .classList.contains('e-right-icon')
      ).toBeTruthy();
    });
    ...
    it('Checking recurrence icon', () => {
      ...
      expect(
        eventElementList[2].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Recurrence Event - Previous week');
      expect(
        eventElementList[2]
          .querySelectorAll('.e-appointment-details .e-icons')[1]
          .classList.contains('e-recurrence-icon')
      ).toBeTruthy();
    });

    it('Checking more indicator', () => {
      const moreIndicatorList: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-more-indicator')
      );
      ...
      const close: HTMLElement = <HTMLElement>(
        schObj.element.querySelector('.e-more-event-close')
      );
      close.click();
    });
    it('cell single click', () => {
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[120] as HTMLElement,
        'click'
      );
      const cellPopup: HTMLElement = schObj.element.querySelector(
        '.e-quick-popup-wrapper'
      ) as HTMLElement;
      expect(cellPopup.classList).toContain('e-popup-open');
      (<HTMLInputElement>cellPopup.querySelector('.e-subject')).innerText = '';
      const moreDetail: HTMLElement = <HTMLElement>(
        cellPopup.querySelector('.e-event-details')
      );
      ...
      close.click();
      const focuesdEle: HTMLTableCellElement =
        document.activeElement as HTMLTableCellElement;
      expect(
        schObj.element.querySelectorAll('.e-selected-cell').length
      ).toEqual(1);
      ...
      expect(
        (focuesdEle.parentNode as HTMLTableRowElement).sectionRowIndex
      ).toEqual(0);
    });
    it('cell double click', () => {
      ...
      const dialogElement: HTMLElement = document.querySelector(
        '.' + cls.EVENT_WINDOW_DIALOG_CLASS
      ) as HTMLElement;
      expect(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_TITLE_TEXT_CLASS)
          .innerHTML
      ).toEqual('New Event');
      expect(
        dialogElement.querySelector('.e-subject-container label').innerHTML
      ).toEqual('Title');
      expect(
        dialogElement.querySelector('.e-location-container label').innerHTML
      ).toEqual('Location');
      expect(
        dialogElement.querySelector('.e-start-container label').innerHTML
      ).toEqual('Start');
      expect(
        dialogElement.querySelector('.e-end-container label').innerHTML
      ).toEqual('End');
      expect(
        dialogElement.querySelector('.e-start-time-zone-container label')
          .innerHTML
      ).toEqual('Start Timezone');
      expect(
        dialogElement.querySelector('.e-end-time-zone-container label')
          .innerHTML
      ).toEqual('End Timezone');
      expect(
        dialogElement.querySelector('.e-description-container label').innerHTML
      ).toEqual('Description');
      expect(
        dialogElement.querySelector('.e-all-day-container .e-label').innerHTML
      ).toEqual('All day');
      expect(
        dialogElement.querySelector('.e-time-zone-container .e-label').innerHTML
      ).toEqual('Timezone');
      const cancelButton: HTMLElement = dialogElement.querySelector(
        '.e-event-cancel'
      ) as HTMLElement;
      cancelButton.click();
      const focuesdEle: HTMLTableCellElement =
    ...
    it('event single click', () => {
      const event: HTMLElement = schObj.element.querySelectorAll(
        '.e-appointment'
      )[1] as HTMLElement;
      util.triggerMouseEvent(event, 'click');
      const eventPopup: HTMLElement = schObj.element.querySelector(
        '.e-quick-popup-wrapper'
      ) as HTMLElement;
      expect(eventPopup.classList).toContain('e-popup-open');
      (<HTMLInputElement>(
        eventPopup.querySelector('.' + cls.SUBJECT_CLASS)
      )).innerText = 'Spanned Event - Same week';
      (<HTMLInputElement>(
        eventPopup.querySelector('.e-date-time-details')
      )).innerText = 'April 30, 2018 (10:00 AM) - May 3, 2018 (1:00 PM)';
    ...
    it('event double click', () => {
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-appointment')[1] as HTMLElement,
        'dblclick'
      );
      const dialogElement: HTMLElement = document.querySelector(
        '.' + cls.EVENT_WINDOW_DIALOG_CLASS
      ...
      (<HTMLInputElement>(
        dialogElement.querySelector('.' + cls.SUBJECT_CLASS)
      )).value = 'Spanned Event - Same week';
      const cancelButton: HTMLElement = dialogElement.querySelector(
        '.e-event-cancel'
      ) as HTMLElement;
      cancelButton.click();
  ...
  describe('Start and End hour', () => {
    ...
    it('Checking elements', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(
          schObj.element.querySelectorAll('.e-header-row .e-time-cells')
            .length * 2
        ).toEqual(
          schObj.element.querySelectorAll(
            '.e-content-wrap .e-content-table tbody td'
    ...
    it('Checking right indicator icon', () => {
      ...
      expect(
        eventElementList[21].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('All Day Event - Next week');
      expect(
        eventElementList[21]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
          .classList.contains('e-right-icon')
  ...
  describe('Work Days', () => {
    ...
    it('Checking left indicator icons', () => {
      ...
      expect(
        eventElementList[0].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('All Day Event - Previous week');
      expect(
        eventElementList[0]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
          .classList.contains('e-left-icon')
    ...
    it('Checking right indicator icon', () => {
      ...
      expect(
        eventElementList[20].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Recurrence Event - Next week');
      expect(
        eventElementList[20]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
          .classList.contains('e-right-icon')
  ...
  describe('First Day of Week', () => {
    ...
    it('Checking current time indicator with different firstDayOfWeek', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(
          schObj.element.querySelectorAll('.e-current-time').length
        ).toEqual(1);
        done();
      };
  ...
  describe('Getting Week numbers while changing weekRule property', () => {
    ...
    it('Week number testing for when weekRule set to FirstDay', () => {
      ...
      expect(
        (schObj.element.querySelector('.e-header-week-cell') as HTMLElement)
          .innerText
      ).toBe('1');
    });
  ...
  describe('Dependent properties', () => {
    ...
    it('date format', () => {
      ...
      schObj = util.createSchedule(model, []);
      expect(
        schObj.element.querySelector('.e-date-range .e-tbar-btn-text').innerHTML
    ...
    it('date header template', () => {
      ...
      schObj = util.createSchedule(model, []);
      expect(
        schObj.element.querySelector('.e-date-header-container .e-header-cells')
          .innerHTML
      ).toEqual('<span>Sun, 10/1</span>');
    ...
    it('dateRange template', () => {
      const model: ScheduleModel = {
        ...
        selectedDate: new Date(2017, 9, 5),
        dateRangeTemplate:
          '<div class="date-text">${(data.startDate).getMonth()}-${(data.endDate).getMonth()}</div>',
      };
      schObj = util.createSchedule(model, []);
      expect(
        schObj.element.querySelector('.e-tbar-btn-text').innerHTML
      ).toEqual('<div class="date-text">9-9</div>');
      schObj.dateRangeTemplate =
        '<div>${getShortDateTime(data.startDate)}-${getShortDateTime(data.endDate)}</div>';
      schObj.dataBind();
      expect(
        schObj.element.querySelector('.e-tbar-btn-text').innerHTML
      ).toEqual('<div>10/1/17, 12:00 AM-10/7/17, 12:00 AM</div>');
    });
    ...
    it('cell template', () => {
      ...
      schObj = util.createSchedule(model, []);
      expect(schObj.element.querySelectorAll('.custom-element').length).toEqual(
        48 * 7
      );
      schObj.cellTemplate = '<span>${getShortDateTime(data.date)}</span>';
      schObj.dataBind();
      expect(
        schObj.element.querySelectorAll('.e-work-cells')[3].innerHTML
      ).toEqual('<span>10/1/17, 1:30 AM</span>');
    });
    ...
    it('check current date class', () => {
      ...
      schObj = util.createSchedule(model, []);
      expect(
        schObj.element.querySelector('.e-date-header-container .e-current-day')
          .classList
      ).toContain('e-header-cells');
    });
    ...
    it('current time indicator', () => {
      ...
      schObj = util.createSchedule(model, []);
      const date: string = instance.formatDate(new Date(), {
        format: schObj.activeViewOptions.timeFormat,
        type: 'time',
      });
      expect(
        schObj.element.querySelector('.e-date-header-wrap .e-current-time')
          .innerHTML
      ).toEqual(date);
      expect(schObj.element.querySelectorAll('.e-current-time').length).toEqual(
        1
      );
    ...
    it('minDate and maxDate', () => {
      ...
      schObj = util.createSchedule(model, []);
      const prevButton: HTMLElement = schObj.element.querySelector(
        '.' + cls.PREVIOUS_DATE_CLASS + ' button'
      );
      const nextButton: HTMLElement = schObj.element.querySelector(
        '.' + cls.NEXT_DATE_CLASS + ' button'
      );
  ...
  describe('Template', () => {
    ...
    it('Events template', () => {
      ...
      expect(eventElementList.length).toEqual(25);
      expect(
        eventElementList[0].querySelector('.e-appointment-details').innerHTML
      ).toEqual(
        '<div class="e-indicator e-icons e-left-icon"></div><span>All Day Event - Previous week</span>'
      );
      expect(
        eventElementList[2].querySelector('.e-appointment-details span')
          .innerHTML
      ).toEqual('Recurrence Event - Previous week');
      expect(
        eventElementList[2]
  ...
  describe('Client side events', () => {
    ...
    it('cell select', () => {
      ...
      schObj = util.createSchedule(model, []);
      const workCells: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-work-cells')
      );
      expect(
        schObj.element.querySelectorAll('.e-selected-cell').length
      ).toEqual(0);
      ...
      (
        schObj.element.querySelectorAll('.e-work-cells')[3] as HTMLElement
      ).click();
      const focuesdEle: HTMLTableCellElement =
        document.activeElement as HTMLTableCellElement;
      expect(focuesdEle.classList).toContain('e-selected-cell');
    ...
    it('validate start and end time on multi cell select', () => {
      ...
      expect(schObj.activeCellsData.startTime).toEqual(
        new Date(2018, 5, 3, 1, 30, 0)
      );
      expect(schObj.activeCellsData.endTime).toEqual(
        new Date(2018, 5, 3, 3, 0, 0)
      );
      expect(eventName).toEqual('select');
    });
    ...
    it('cancel cell click', () => {
      ...
      schObj = util.createSchedule(model, []);
      const workCell: HTMLElement = schObj.element.querySelectorAll(
        '.e-work-cells'
      )[3] as HTMLElement;
  ...
  describe('Public methods', () => {
    ...
    it('getCellDetails', () => {
      const data: CellClickEventArgs = schObj.getCellDetails(
        schObj.element.querySelector('.e-work-cells')
      );
      expect(data.startTime.getTime()).toEqual(new Date(2017, 9, 1).getTime());
      expect(data.endTime.getTime()).toEqual(
        new Date(2017, 9, 1, 0, 30).getTime()
      );
    ...
    it('setWorkHours', () => {
      ...
      const workHourCells: HTMLTableCellElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-work-hours')
      );
    ...
    it('addEvent & getEventDetails', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(
          schObj.element.querySelectorAll('.e-appointment').length
        ).toEqual(1);
    ...
    it('getCurrentViewDates', () => {
      ...
      expect(schObj.element.querySelectorAll('.e-header-cells').length).toEqual(
        dateLength
      );
    });
  ...
  describe('Scroll to workhour when currentDate set to selectedDate', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        height: '500px',
        selectedDate: new Date(2020, 0, 22, 10, 30, 30),
        currentView: 'TimelineWeek',
        views: ['TimelineDay', 'TimelineWeek', 'TimelineWorkWeek'],
        workHours: {
          highlight: true,
  ...
  describe('Public methods with resource rendering', () => {
    ...
    it('setWorkHours', () => {
      ...
      expect(
        schObj.element.querySelectorAll('.e-work-hours[data-group-index="0"]')
          .length
      ).toEqual(16);
      expect(schObj.element.querySelectorAll('.e-work-hours').length).toEqual(
        16
  ...
  describe('Single level resource rendering', () => {
    ...
    it('checking resource title rendering', () => {
      const headerRow: HTMLElement = schObj.element.querySelector(
        '.e-timeline-view tr'
      ) as HTMLElement;
      expect(headerRow.children.length).toEqual(2);
      const firstTD: HTMLElement = schObj.element.querySelector(
        '.e-timeline-view tr td.e-resource-left-td'
      ) as HTMLElement;
    ...
    it('checking resource tree rendering', () => {
      const contentRow: HTMLElement = schObj.element.querySelector(
        '.e-timeline-view table tbody'
      ) as HTMLElement;
      expect(contentRow.children[1].children.length).toEqual(2);
      expect(
        [contentRow.children[1].querySelector('td div.e-resource-column-wrap')]
          .length
      ).toEqual(1);
      const resourceRow: HTMLElement = schObj.element.querySelector(
        '.e-resource-column-wrap tbody'
      ) as HTMLElement;
      expect(resourceRow.children.length).toEqual(10);
      expect(
        [resourceRow.querySelector('tr td.e-resource-cells')].length
      ).toEqual(1);
      expect(
        [
          resourceRow.querySelector(
            'tr td.e-resource-cells div.e-resource-text'
          ),
        ].length
      ).toEqual(1);
      expect(
        resourceRow.querySelector('tr td.e-resource-cells div.e-resource-text')
          .innerHTML
      ).toEqual('Nancy');
      const contentRows: HTMLElement =
        contentRow.children[1].children[1].querySelector('tbody');
    ...
    it('Checking Left icon', () => {
      ...
      expect(
        eventElementList[13].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('All Day Event - Previous week');
      expect(
        eventElementList[13]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
    ...
    it('Checking right icon', () => {
      ...
      expect(
        eventElementList[8].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Recurrence Event - Less than 24');
      expect(
        eventElementList[8]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
          .classList.contains('e-right-icon')
    ...
    it('Checking right icon', () => {
      ...
      expect(
        eventElementList[8]
          .querySelectorAll('.e-appointment-details .e-icons')[0]
          .classList.contains('e-recurrence-icon')
      ).toBeTruthy();
    });
    ...
    it('More event element checking', () => {
      ...
      const element: HTMLElement = moreIndicatorList[0] as HTMLElement;
      element.click();
      const morePopup: HTMLElement = schObj.element.querySelector(
        '.e-more-popup-wrapper'
      ) as HTMLElement;
      const moreEventList: Element[] = [].slice.call(
        morePopup.querySelectorAll('.e-more-appointment-wrapper .e-appointment')
      );
      expect(moreEventList.length).toEqual(2);
      util.triggerMouseEvent(
        morePopup.querySelector('.e-more-event-close'),
        'click'
      );
    });
  });
  ...
  describe('Multi level resource rendering', () => {
    ...
    it('checking resource tree rendering', () => {
      ...
      expect(
        [
          resourceRow.querySelector(
            'tr td.e-resource-cells div.e-resource-tree-icon'
          ),
        ].length
      ).toEqual(1);
      expect(
        [
          resourceRow.querySelector(
            'tr td.e-resource-cells div.e-resource-collapse'
          ),
        ].length
      ).toEqual(1);
      expect([resourceRow.querySelector('tr td.e-parent-node')].length).toEqual(
        1
      );
      expect(
        resourceRow.querySelector('tr td.e-resource-cells').children.length
      ).toEqual(2);
      expect(
        resourceRow.querySelector('tr td.e-resource-cells div.e-resource-text')
          .innerHTML
      ...
      expect(
        resourceRow.children[1].querySelector('.e-resource-cells').children
          .length
      ).toEqual(2);
      expect(
        resourceRow.children[1].querySelector(
          '.e-resource-cells div.e-resource-text'
        ).innerHTML
      ).toEqual('ROOM 1');
      expect(
        [resourceRow.children[2].querySelector('.e-child-node')].length
      ).toEqual(1);
      expect(
        resourceRow.children[2].querySelector('.e-child-node').children.length
      ).toEqual(1);
      const contentRows: HTMLElement =
        contentRow.children[1].children[1].querySelector('tbody');
  ...
  describe('Multi level resource rendering with expanded property', () => {
    ...
    it('resource icon click testing', () => {
      ...
      expect(resourceRow.children[1].classList.contains('e-hidden')).toEqual(
        false
      );
      const parentRow: HTMLElement = <HTMLElement>(
        resourceRow.children[10].querySelector(
          '.e-resource-cells div.e-resource-tree-icon'
        )
      );
      parentRow.click();
      const afterExpand: NodeListOf<Element> = schObj.element.querySelectorAll(
        '.e-resource-column-wrap tbody tr:not(.e-hidden)'
      );
      expect(afterExpand.length).toEqual(11);
      expect(
        [
          resourceRow.children[10].querySelector(
            '.e-resource-cells div.e-resource-collapse'
          ),
        ].length
      ).toEqual(1);
      expect(resourceRow.children[11].classList.contains('e-hidden')).toEqual(
        false
      );
      expect(
        resourceRow.children[11].querySelector(
          '.e-resource-cells div.e-resource-text'
        ).innerHTML
      ).toEqual('ROOM 2');
      parentRow.click();
      const afterCollapse: NodeListOf<Element> =
        schObj.element.querySelectorAll(
          '.e-resource-column-wrap tbody tr:not(.e-hidden)'
        );
    ...
    it('Checking right icon', () => {
      ...
      expect(
        eventElementList[4].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Recurrence Event - Previous week');
      expect(
        eventElementList[4]
          .querySelectorAll('.e-appointment-details .e-icons')[1]
    ...
    it('cell single click', () => {
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[720] as HTMLElement,
        'click'
      );
      const cellPopup: HTMLElement = schObj.element.querySelector(
        '.e-quick-popup-wrapper'
    ...
    it('cell double click', () => {
      ...
      const hall: DropDownList = (
        dialogElement.querySelector('.e-HallId') as EJ2Instance
      ).ej2_instances[0] as DropDownList;
      expect(hall.text).toEqual('Hall 1');
      expect(hall.value).toEqual(1);
      const room: DropDownList = (
        dialogElement.querySelector('.e-RoomId') as EJ2Instance
      ).ej2_instances[0] as DropDownList;
      expect(room.text).toEqual('ROOM 1');
      const owner: DropDownList = (
        dialogElement.querySelector('.e-OwnerId') as EJ2Instance
      ).ej2_instances[0] as DropDownList;
  ...
  describe('Custom work days of Resources in group', () => {
    ...
    it('work hours count', () => {
      ...
      expect(
        (<HTMLTableCellElement>schObj.element.querySelector('.e-work-hours'))
          .cellIndex
      ).toBe(64);
      const contentTable: NodeListOf<Element> = schObj.element.querySelectorAll(
        '.e-content-table tr'
      );
      expect(
        (<Element>contentTable[1].childNodes[63]).classList.contains(
          'e-work-hours'
        )
      ).toEqual(false);
      expect(
        (<Element>contentTable[1].childNodes[64]).classList.contains(
          'e-work-hours'
        )
      ).toEqual(true);
    });
  ...
  describe('Multi level resource rendering with expanded property in RTL', () => {
    ...
    it('resource icon click testing', () => {
      ...
      const beforeExpand: NodeListOf<Element> = schObj.element.querySelectorAll(
        '.e-resource-column-wrap tbody tr:not(.e-hidden)'
      );
      expect(beforeExpand.length).toEqual(7);
      const firstRow: HTMLElement = resourceRow.children[1].querySelector(
        '.e-resource-cells div.e-resource-tree-icon'
      ) as HTMLElement;
      firstRow.click();
      const afterExpand: NodeListOf<Element> = schObj.element.querySelectorAll(
      ...
      expect(resourceRow.children[2].classList.contains('e-hidden')).toEqual(
        false
      );
      expect(
        resourceRow.children[2].querySelector(
          '.e-resource-cells div.e-resource-text'
        ).innerHTML
      ).toEqual('Nancy');
    });
  ...
  describe('Single level resource rendering with Template', () => {
    let schObj: Schedule;
    const restemplate: string =
      '<div class="tWrap"><div class="rText" style="background:pink">${getResourceName(data)}</div></div>';
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        height: '550px',
        width: '100%',
    ...
    it('checking resource template rendering', () => {
      ...
      expect(
        [resourceRow.querySelector('tr td.e-resource-cells div.tWrap')].length
      ).toEqual(1);
      expect(
        resourceRow.querySelector('tr td.e-resource-cells div.tWrap').children
          .length
      ).toEqual(1);
      const templateDiv: HTMLElement = resourceRow.querySelector(
        'tr td.e-resource-cells div.tWrap div.rText'
      ) as HTMLElement;
  ...
  describe('Group by-child multi level resource rendering with expand property', () => {
    ...
    it('checking resource tree rendering', () => {
      ...
      expect(
        resourceRow.children[2].querySelector(
          '.e-child-node div.e-resource-text'
        ).innerHTML
      ).toEqual('Nancy');
      expect(
        resourceRow.children[9].querySelector(
          'tr td.e-resource-cells div.e-resource-text'
        ).innerHTML
      ).toEqual('Hall 2');
      expect(
  ...
  describe('Multiple resource grouping rendering compact view in mobile device ', () => {
    ...
    it('initial layout testing', () => {
      ...
      expect(workCells[126].getAttribute('data-date')).toEqual(
        new Date(2018, 3, 3, 15, 0).getTime().toString()
      );
    });
    ...
    it('resource toolbar testing', () => {
      ...
      expect(
        schObj.element.querySelectorAll('.e-schedule-resource-toolbar').length
      ).toEqual(1);
      expect(
        schObj.element.querySelectorAll(
          '.e-schedule-resource-toolbar .e-resource-menu'
        ).length
      ).toEqual(1);
      expect(
        schObj.element.querySelectorAll(
      ...
      expect(
        schObj.element.querySelectorAll(
          '.e-resource-level-title .e-resource-name'
        ).length
      ).toEqual(2);
      expect(
        schObj.element.querySelectorAll('.e-resource-level-title .e-icon-next')
          .length
      ).toEqual(1);
    });
    ...
    it('resource treeview testing', () => {
      expect(
        schObj.element.querySelectorAll('.e-resource-tree-popup').length
      ).toEqual(1);
      expect(
        schObj.element.querySelectorAll('.e-resource-tree').length
      ).toEqual(1);
      expect(
        schObj.element.querySelectorAll(
          '.e-resource-tree .e-list-item.e-has-child'
        ).length
      ).toEqual(2);
      expect(
        schObj.element.querySelectorAll(
          '.e-resource-tree .e-list-item:not(.e-has-child)'
        ).length
      ).toEqual(3);
    });
    ...
    it('resource menu click testing', () => {
      expect(
        schObj.element
          .querySelector('.e-resource-tree-popup')
          .classList.contains('e-popup-close')
      ).toEqual(true);
      const menuElement: HTMLElement = schObj.element.querySelector(
        '.e-resource-menu .e-icon-menu'
      );
      menuElement.click();
      expect(
        schObj.element
      ...
      expect(
        schObj.element
          .querySelector('.e-resource-tree-popup-overlay')
          .classList.contains('e-enable')
      ).toEqual(true);
      menuElement.click();
      expect(
        schObj.element
    ...
    it('resource node click testing', () => {
      ...
      menuElement.click();
      expect(
        schObj.element.querySelector(
          '.e-resource-level-title .e-resource-name:first-child'
        ).innerHTML
      ).toEqual('Room 1');
      expect(
        schObj.element.querySelector(
          '.e-resource-level-title .e-resource-name:last-child'
        ).innerHTML
      ).toEqual('Nancy');
      const nodeElement: NodeListOf<Element> = schObj.element.querySelectorAll(
        '.e-resource-tree .e-list-item:not(.e-has-child)'
      );
    ...
    it('resource events checked for timeline week view testing', () => {
      expect(schObj.element.querySelectorAll('.e-appointment').length).toEqual(
        3
      );
    });
  ...
  describe('Grouped events', () => {
    ...
    it('Checking recurrence icon', () => {
      ...
      expect(
        eventElementList[2].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Recurrence Event - Same day');
      expect(
        eventElementList[2]
          .querySelectorAll('.e-appointment-details .e-icons')[0]
          .classList.contains('e-recurrence-icon')
  ...
  describe('Events rendering based on levels', () => {
    ...
    it('Checking events elements', () => {
      const eventContainer: Element[] = [].slice.call(
        schObj.element.querySelectorAll(
          '.e-event-table .e-appointment-container'
        )
      );
    ...
    it('Checking events in top level parent node if there is no child fields mapped', () => {
      ...
      expect(
        eventContainer[0].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Spanned Event - Greater than 24');
    });
    ...
    it('Checking events in last level parent node if there is no child fields mapped', () => {
      ...
      expect(
        eventContainer[2].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Events - Within a day');
    });
    ...
    it('Checking events in child node if all fields are mapperd properly', () => {
      ...
      expect(
        eventContainer[3].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Spanned Event - Next week');
    });
    ...
    it('Checking left icon', () => {
      ...
      expect(
        eventContainer[1].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Spanned Event - Previous week');
      expect(
        eventContainer[1]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
    ...
    it('Checking right icon', () => {
      ...
      expect(
        eventElementList[3].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Spanned Event - Next week');
      expect(
        eventElementList[3]
          .querySelectorAll('.e-appointment-details .e-indicator')[0]
    ...
    it('Checking recurrence icon', () => {
      ...
      expect(
        eventElementList[5].querySelector('.e-inner-wrap .e-subject').innerHTML
      ).toEqual('Recurrence  Event');
      expect(
        eventElementList[5]
          .querySelectorAll('.e-appointment-details .e-icons')[0]
          .classList.contains('e-recurrence-icon')
  ...
  describe('Year, Month, Week, Day, Hour header rows', () => {
    ...
    it('check header rows', () => {
      expect(
        schObj.element.querySelectorAll('.e-date-header-wrap table col').length
      ).toEqual(48 * daysLength);
      expect(
        schObj.element.querySelectorAll('.e-date-header-wrap table tr').length
      ).toEqual(5);
    });

    it('check year rows', () => {
      const headTrs: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-date-header-wrap tr')
      );
      expect(headTrs[0].children.length).toEqual(1);
      expect(headTrs[0].children[0].getAttribute('colSpan')).toEqual('336');
      expect(headTrs[0].children[0].innerHTML).toEqual(
        '<span class="e-header-year" style="left: 5700px;">2018</span>'
      );
    });
    ...
    it('check month rows', () => {
      ...
      expect(headTrs[1].children[0].innerHTML).toEqual(
        '<span class="e-header-month">April</span>'
      );
    });
    ...
    it('check week rows', () => {
      ...
      expect(headTrs[2].children[0].innerHTML).toEqual(
        '<span class="e-header-week" style="left: 5700px;">18</span>'
      );
    });
    ...
    it('check day rows', () => {
      ...
      expect(headTrs[3].children[0].innerHTML).toEqual(
        '<span class="e-header-date e-navigate">Apr 29, Sunday</span>'
      );
      expect(headTrs[3].children[0].getAttribute('data-date')).toEqual(
        new Date(2018, 3, 29).getTime().toString()
      );
    });

    it('check hour rows', () => {
      const headTrs: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-date-header-wrap tbody tr')
      );
    ...
    it('check work cells', () => {
      ...
      expect(data.endTime.getTime()).toEqual(
        new Date(2018, 3, 29, 0, 30).getTime()
      );
      expect(data.isAllDay).toEqual(false);
    ...
    it('check work hours highlight', () => {
      expect(
        schObj.element.querySelectorAll('.e-work-hours,.e-work-days').length
      ).toEqual(18 * 5);
    });
    ...
    it('check events rendering', () => {
      ...
      expect(
        eventElementList[0].querySelectorAll(
          '.e-appointment-details .e-indicator'
        )[0].classList
      ).toContain('e-left-icon');
    });

    it('check more indicator', () => {
      const moreIndicators: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-more-indicator')
      );
      expect(moreIndicators.length).toEqual(2);
      expect(moreIndicators[0].innerHTML).toEqual('+3&nbsp;more');
      expect(moreIndicators[0].getAttribute('data-start-date')).toEqual(
        new Date(2018, 4, 1, 13).getTime().toString()
      );
      expect(moreIndicators[0].getAttribute('data-end-date')).toEqual(
        new Date(2018, 4, 1, 13, 30).getTime().toString()
      );
    ...
    it('navigate next date', (done: DoneFn) => {
      ...
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element.querySelector('.e-date-range .e-tbar-btn-text')
            .innerHTML
        ).toEqual('May 06 - 12, 2018');
        expect(
          schObj.element.querySelectorAll('.e-date-header-wrap table col')
            .length
        ).toEqual(336);
        const headTrs: Element[] = [].slice.call(
          schObj.element.querySelectorAll('.e-date-header-wrap tr')
        ...
        expect(headTrs[1].children[0].innerHTML).toEqual(
          '<span class="e-header-month" style="left: 5700px;">May</span>'
        );
        expect(headTrs[2].children.length).toEqual(1);
        expect(headTrs[2].children[0].getAttribute('colSpan')).toEqual('336');
        expect(headTrs[2].children[0].innerHTML).toEqual(
          '<span class="e-header-week" style="left: 5700px;">19</span>'
        );
        expect(headTrs[3].children.length).toEqual(7);
        expect(headTrs[3].children[0].getAttribute('colSpan')).toEqual('48');
        expect(headTrs[3].children[0].innerHTML).toEqual(
          '<span class="e-header-date e-navigate">May 6, Sunday</span>'
        );
        expect(headTrs[3].children[0].getAttribute('data-date')).toEqual(
          new Date(2018, 4, 6).getTime().toString()
        );
        expect(headTrs[4].children.length).toEqual(48 * daysLength);
        expect(headTrs[4].children[0].getAttribute('colSpan')).toBeNull();
        expect(headTrs[4].children[0].innerHTML).toEqual(
          '<span>12:00 AM</span>'
        );
        expect(headTrs[4].children[1].innerHTML).toEqual('');
        expect(schObj.getWorkCellElements().length).toEqual(48 * daysLength);
  ...
  describe('Year, Month, Week, Day, Hour header rows with template', () => {
    ...
    it('check year rows', () => {
      ...
      expect(headTrs[0].children[0].getAttribute('colSpan')).toEqual('336');
      expect(headTrs[0].children[0].innerHTML).toEqual(
        '<span style="left: 5700px;">yearHeader</span>'
      );
    });
    ...
    it('check week rows', () => {
      ...
      expect(headTrs[2].children[0].getAttribute('colSpan')).toEqual('336');
      expect(headTrs[2].children[0].innerHTML).toEqual(
        '<span style="left: 5700px;">weekHeader</span>'
      );
    });
    ...
    it('check events rendering with more indicator', () => {
      ...
      expect(eventWrapperList.length).toEqual(1);
      const moreIndicators: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-more-indicator')
      );
      expect(moreIndicators.length).toEqual(2);
      expect(moreIndicators[0].innerHTML).toEqual('+3&nbsp;more');
  ...
  describe('Year, Month, Week, Day header rows', () => {
    ...
    it('check year rows', () => {
      ...
      expect(headTrs[0].children[0].innerHTML).toEqual(
        '<span class="e-header-year">2018</span>'
      );
    });
    ...
    it('check week rows', () => {
      ...
      expect(headTrs[2].children[0].innerHTML).toEqual(
        '<span class="e-header-week">18</span>'
      );
    });
    ...
    it('check events rendering with more indicator', () => {
      ...
      expect(moreIndicators[0].getAttribute('data-start-date')).toEqual(
        new Date(2018, 4, 1).getTime().toString()
      );
      expect(moreIndicators[0].getAttribute('data-end-date')).toEqual(
        new Date(2018, 4, 2).getTime().toString()
      );
    });
  });
  ...
  describe('Year, Week, Day header rows', () => {
    ...
    it('check day rows', () => {
      ...
      expect(headTrs[2].children[0].getAttribute('data-date')).toEqual(
        new Date(2018, 3, 29).getTime().toString()
      );
    });
  ...
  describe('Year, Month, Week, Day, Hour header rows with single resource', () => {
    ...
    it('check resource column', () => {
      expect(
        schObj.element.querySelectorAll(
          '.e-resource-column-wrap table tbody tr'
        ).length
      ).toEqual(resLength);
      expect(
        schObj.element.querySelectorAll(
          '.e-resource-column-wrap table tbody tr td'
        ).length
      ).toEqual(resLength);
    });
    ...
    it('check work cells', () => {
      expect(
        schObj.element.querySelectorAll('.e-content-wrap table tbody tr').length
      ).toEqual(resLength);
      expect(
        schObj.element.querySelectorAll('.e-content-wrap table col').length
      ).toEqual(336);
      expect(
        schObj.element.querySelectorAll('.e-content-wrap table td').length
      ).toEqual(336 * resLength);
      const firstWorkCell: HTMLElement = schObj.element.querySelector(
        '.e-work-cells'
      ) as HTMLElement;
      expect(firstWorkCell.getAttribute('aria-selected')).toEqual('false');
    ...
    it('check work hours highlight', () => {
      expect(
        schObj.element.querySelectorAll('.e-work-hours,.e-work-days').length
      ).toEqual(90 * resLength);
    });

    it('check events rendering', () => {
      const eventElements: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
      );
      ...
      expect(eventWrapperList.length).toEqual(10);
      expect(
        schObj.element.querySelectorAll('.e-event-table > div').length
      ).toEqual(resLength);
      expect(eventElements[0].querySelector('.e-subject').innerHTML).toEqual(
        'Events - Within a day'
      );
      expect(eventElements[0].querySelector('.e-time').innerHTML).toEqual(
        '10:00 AM - 12:30 PM'
      );
      expect(eventElements[0].getAttribute('data-group-index')).toEqual('0');
      expect(eventElements[0].style.backgroundColor).toEqual(
        'rgb(255, 170, 0)'
      );
      expect(eventElements[1].getAttribute('data-group-index')).toEqual('1');
      expect(eventElements[1].style.backgroundColor).toEqual(
        'rgb(248, 163, 152)'
      );
    });
    ...
    it('check more indicator', () => {
      ...
      expect(moreIndicators[0].getAttribute('data-start-date')).toEqual(
        new Date(2018, 4, 4, 2).getTime().toString()
      );
      expect(moreIndicators[0].getAttribute('data-end-date')).toEqual(
        new Date(2018, 4, 4, 2, 30).getTime().toString()
      );
      moreIndicators[0].click();
      const morePopup: HTMLElement = schObj.element.querySelector(
        '.e-more-popup-wrapper'
    ...
    it('navigate next date', (done: DoneFn) => {
      ...
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element.querySelectorAll('.e-content-wrap table tbody tr')
            .length
        ).toEqual(resLength);
        expect(
          schObj.element.querySelectorAll('.e-content-wrap table col').length
        ...
        expect(data.startTime.getTime()).toEqual(
          new Date(2018, 4, 6).getTime()
        );
        expect(data.endTime.getTime()).toEqual(
          new Date(2018, 4, 6, 0, 30).getTime()
        );
        expect(data.isAllDay).toEqual(false);
        expect(data.groupIndex).toBe(0);
        expect(
  ...
  describe('Default schedule block events', () => {
    ...
    it('block event initial rendering testing', () => {
      expect(
        schObj.element.querySelectorAll('.e-block-appointment').length
      ).toEqual(4);
      const blockEvent: HTMLElement = schObj.element.querySelector(
        '[data-id="Appointment_1"]'
      ) as HTMLElement;
    ...
    it('add event', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        const addedEvent: HTMLElement = schObj.element.querySelector(
          '[data-id="Appointment_15"]'
        ) as HTMLElement;
      ...
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[0] as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-work-cells')[0] as HTMLElement,
      ...
      const startObj: DateTimePicker = util.getInstance(
        cls.EVENT_WINDOW_START_CLASS
      ) as DateTimePicker;
      startObj.value = new Date(2017, 9, 30, 10, 30);
      startObj.dataBind();
      const endObj: DateTimePicker = util.getInstance(
        cls.EVENT_WINDOW_END_CLASS
      ) as DateTimePicker;
      endObj.value = new Date(2017, 9, 30, 11, 30);
      endObj.dataBind();
      const saveButton: HTMLInputElement = <HTMLInputElement>(
        dialogElement.querySelector('.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS)
      );
      saveButton.click();
      const alertDialog: HTMLElement = document.querySelector(
        '.e-quick-dialog'
      ) as HTMLElement;
      expect(schObj.quickPopup.quickDialog.visible).toBe(true);
      expect(alertDialog.querySelector('.e-dlg-content').innerHTML).toEqual(
        'Events cannot be scheduled within the blocked time range.'
      );
      const okButton: HTMLElement = alertDialog.querySelector(
        '.e-quick-alertok'
      ) as HTMLElement;
      okButton.click();
      expect(schObj.quickPopup.quickDialog.visible).toBe(false);
      const startRevisedObj: DateTimePicker = util.getInstance(
        cls.EVENT_WINDOW_START_CLASS
      ) as DateTimePicker;
      startRevisedObj.value = new Date(2017, 9, 30, 1, 0);
      startRevisedObj.dataBind();
      const endRevisedObj: DateTimePicker = util.getInstance(
        cls.EVENT_WINDOW_END_CLASS
      ) as DateTimePicker;
    ...
    it('edit event', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(schObj.eventWindow.dialogObject.visible).toEqual(false);
        const editedEvent: HTMLElement = schObj.element.querySelector(
          '[data-id="Appointment_15"]'
        ) as HTMLElement;
      ...
      util.triggerMouseEvent(
        schObj.element.querySelector('.e-appointment') as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelector('.e-appointment') as HTMLElement,
  ...
  describe('Multi level resource rendering in block events', () => {
    ...
    it('resource add event', (done: DoneFn) => {
      ...
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-content-table tr')[1]
          .childNodes[73] as HTMLElement,
        'click'
      );
      util.triggerMouseEvent(
        schObj.element.querySelectorAll('.e-content-table tr')[1]
  ...
  describe('Multi level resource rendering with Timescale -  Schedule width 500', () => {
    ...
    it('Check events offsetleft - slot count 2', () => {
      const colElement: HTMLElement = schObj.element.querySelector(
        '.' + cls.CONTENT_WRAP_CLASS + ' table colgroup col:first-child'
      ) as HTMLElement;
      const tdElement: HTMLElement = schObj.element.querySelector(
        '.' + cls.WORK_CELLS_CLASS
      ) as HTMLElement;
  ...
  describe('Multi level resource rendering with Timescale -  Schedule width 1900', () => {
    ...
    it('Check events offsetleft - slot count 2', () => {
      ...
      const tdElement: HTMLElement = schObj.element.querySelector(
        '.' + cls.CONTENT_WRAP_CLASS + ' tbody tr:first-child td:first-child'
      ) as HTMLElement;
      expect(colElement.offsetWidth).toEqual(+tdElement.offsetWidth);
    });
    it('Check events offsetleft - slot count 6', (done: DoneFn) => {
  ...
  describe('checking indent template with resources', () => {
      ...
      const model: ScheduleModel = {
        ...
        views: [
          ...
          { option: 'TimelineMonth' },
          {
            ...
            displayName: 'VerticalYear',
          },
          {
            option: 'TimelineYear',
            orientation: 'Horizontal',
            displayName: 'HorizontalYear',
          },
        ],
    ...
    it('Resource indent template', () => {
      ...
      expect(
        (schObj.element.querySelector('.e-resource-left-td') as HTMLElement)
          .innerHTML
      ).toEqual('<div class="temp-wrap">Resources</div>');
      expect(
        (
          schObj.element.querySelector('.e-resource-left-td') as HTMLElement
        ).firstElementChild.classList.contains('temp-wrap')
      ).toBeTruthy();
    });
    ...
    it('changing current view and check resource indent template', () => {
      ...
      schObj.dataBind();
      expect(
        schObj.element.querySelector('.e-resource-cells').innerHTML
      ).toEqual('<div class="temp-wrap">Resources</div>');
      expect(
        schObj.element
          .querySelector('.e-resource-cells')
          .firstElementChild.classList.contains('temp-wrap')
      ).toBeTruthy();
    });
    ...
    it('checking indentTemplate property', () => {
      ...
      expect(
        (schObj.element.querySelector('.e-resource-left-td') as HTMLElement)
          .innerHTML
      ).toEqual('<div class="template-wrap">Testing</div>');
      expect(
        (
          schObj.element.querySelector('.e-resource-left-td') as HTMLElement
        ).firstElementChild.classList.contains('template-wrap')
      ).toBeTruthy();
    });
  });
  ...
  describe('checking indent template without resources', () => {
    ...
    it('checking indent template', () => {
      expect(
        schObj.element.querySelector('.e-timeline-year-view')
      ).toBeTruthy();
      expect(
        schObj.element.querySelector('.e-active-view').classList
      ).toContain('e-timeline-year');
      expect(schObj.element.querySelector('.e-header-cells').innerHTML).toEqual(
        '<div class="temp-wrap">Resources</div>'
      );
      expect(
        schObj.element
          .querySelector('.e-header-cells')
          .firstElementChild.classList.contains('temp-wrap')
      ).toBeTruthy();
      schObj.changeCurrentView('TimelineYear', 1);
      schObj.dataBind();
  ...
  describe('minimumEventDuration property', () => {
    ...
    it('Checking appointment width by setting minimumEventDuration property to 30 minutes', () => {
      const appointmentWrapper: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment-wrapper')
      );
      expect(appointmentWrapper.length).toEqual(1);
      const firstEvent: HTMLElement = schObj.element.querySelector(
        '[data-id ="Appointment_1"]'
      );
      expect(firstEvent.style.width).toEqual('50px');
      const secondEvent: HTMLElement = schObj.element.querySelector(
        '[data-id ="Appointment_2"]'
      );
  ...
  describe('EJ2-57866-Cell height is wrong while row auto height is set to true', () => {
    ...
    it('Checking the cell height', () => {
      const cell: HTMLElement = schObj.element.querySelector(
        '.' + cls.WORK_CELLS_CLASS
      );
      const apps: NodeListOf<Element> = schObj.element.querySelectorAll(
        '.' + cls.APPOINTMENT_CLASS
      );
  ...
  describe('EJ2-62147 - CR Issue scrollTo', () => {
    ...
    beforeAll((done: DoneFn) => {
      ...
      schObj = util.createSchedule(model, timelineData, done);
      const contentWrap: HTMLElement =
        schObj.element.querySelector('.e-content-wrap');
  ...
  describe('ES-826421 - Appointment misalignment issue', () => {
    ...
    it('Checking the scroll width of the col elements', () => {
      ...
      const appEle: HTMLElement = schObj.element.querySelector(
        '.e-appointment[data-id="Appointment_45"]'
      ) as HTMLElement;
      const workCell: HTMLElement = schObj.element.querySelector(
        '.e-work-cells[data-date="1672790400000"]'
      ) as HTMLElement;
  ...
  describe('ES-844069 - Checking Current Time indicator', () => {
    ...
    it('with header rows', () => {
      ...
      const timeIndicatorLeft: number = (
        contentWrap.querySelector('.e-current-timeline') as HTMLElement
      ).offsetLeft;
      const contentTableWidth: number = (
        schObj.element.querySelector('.e-content-table') as HTMLElement
      ).offsetWidth;
  ...
  describe('Schedule TimelineWeek view with maxEventsPerRow property when the row have enough height', () => {
    ...
    it('elements in DOM based on maxEventsPerRow', () => {
      const appointmentList: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
      );
      expect(appointmentList.length).toEqual(2);
      (
        schObj.element.querySelectorAll('.e-more-indicator')[0] as HTMLElement
      ).click();
      expect(
        schObj.element
          .querySelector('.e-more-popup-wrapper')
          .classList.contains('e-popup-open')
      ).toEqual(true);
      const moreEvent: HTMLElement = schObj.element
        .querySelector('.e-more-popup-wrapper')
        .querySelector('.e-more-appointment-wrapper');
      const moreAppointmentList: Element[] = [].slice.call(
        moreEvent.querySelectorAll('.e-appointment')
      );
  ...
  describe('Schedule TimelineWeek view with maxEventsPerRow property when the row does not have enough height', () => {
    ...
    it('elements in DOM based on maxEventsPerRow', () => {
      ...
      const heightValue: string = (
        schObj.element.querySelector('.e-content-table tr td') as HTMLElement
      ).style.height;
  ...
  describe('Schedule TimelineWeek view with maxEventsPerRow property when the row does not have enough height', () => {
    ...
    it('elements in DOM based on maxEventsPerRow', () => {
      ...
      expect(heightValue).toEqual('139px');
      const resourceHeightValue: string = (
        schObj.element.querySelector(
          '.e-resource-column-table tr td'
        ) as HTMLElement
      ).style.height;
  ...
  describe('EJ2-853941 - Appointments positioned incorrectly when using firstDayOfWeek with different time slot intervals', () => {
    ...
    it('checking appointment rendering', () => {
      ...
      const eventList: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
      );
  ...
  describe('ES-867036 - More indicator count incorrect on different time slots', () => {
    ...
    it('checking appointment rendering', () => {
      ...
      const indicator: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-more-indicator')
      );
  ...
  describe('ES-866125 - Appointment disappearing issue', () => {
    ...
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        ...
        headerRows: [{ option: 'Year' }, { option: 'Week' }],
        rowAutoHeight: true,
      };
      schObj = util.createSchedule(model, data, done);
    });
    ...
    it('Checking the scroll width of the col elements and appointment position', () => {
      ...
      expect(contentWrap.offsetWidth > contentWrap.clientWidth).toBeTruthy();
      const appEle: HTMLElement = schObj.element.querySelector(
        '.e-appointment[data-id="Appointment_2"]'
      ) as HTMLElement;
      const workCell: HTMLElement = schObj.element.querySelector(
        '.e-work-cells[data-date="1707004800000"]'
      ) as HTMLElement;
      expect(appEle.offsetLeft).toEqual(workCell.offsetLeft);
    });
  });
