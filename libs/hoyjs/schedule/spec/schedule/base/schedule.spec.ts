...
describe('Schedule base module', () => {
  ...
  describe('Default functionalities', () => {
    ...
    it('control class testing', () => {
      expect(
        document.getElementById('Schedule').classList.contains('e-schedule')
      ).toEqual(true);
    });
  ...
  describe('Locale', () => {
    ...
    it('vietnamese test case', () => {
      ...
      schObj = new Schedule(
        { selectedDate: new Date(2017, 9, 4), locale: 'vi' },
        '#Schedule'
      );
      expect(
        schObj.element.querySelector(
          '.e-schedule-toolbar .e-date-range .e-tbar-btn-text'
        ).innerHTML
      ).toEqual('01 - 07 Tháng 10 2017');
      expect(
        schObj.element.querySelector(
          '.e-schedule-toolbar .e-day .e-tbar-btn-text'
        ).innerHTML
      ).toEqual('Ngày');
      expect(
        schObj.element.querySelector(
          '.e-schedule-toolbar .e-week .e-tbar-btn-text'
        ).innerHTML
      ).toEqual('Tuần');
      expect(
        schObj.element.querySelector(
          '.e-schedule-toolbar .e-work-week .e-tbar-btn-text'
        ).innerHTML
      ).toEqual('Tuần làm việc');
      expect(
        schObj.element.querySelector(
          '.e-schedule-toolbar .e-month .e-tbar-btn-text'
        ).innerHTML
      ).toEqual('tháng');
      expect(
        schObj.element.querySelector(
          '.e-schedule-toolbar .e-agenda .e-tbar-btn-text'
        ).innerHTML
      ).toEqual('chương trình nghị sự');
      expect(
        schObj.element.querySelector(
          '.e-schedule-toolbar .e-today .e-tbar-btn-text'
        ).innerHTML
      ).toEqual('Hôm nay');
    ...
    it('chinese test case', () => {
      ...
      schObj = new Schedule(
        { selectedDate: new Date(2017, 9, 4), locale: 'zh' },
        '#Schedule'
      );
      expect(
        schObj.element.querySelector(
  ...
  describe('Window resize', () => {
    ...
    it('desktop to mobile', () => {
      ...
      expect(
        schObj.element.querySelectorAll('.e-week-agenda-view').length
      ).toEqual(0);
    ...
    it('desktop to mobile - month view testing', (done: DoneFn) => {
      ...
      schObj = new Schedule({
        ...
        dataBound: () => {
          expect(
            (<HTMLElement>schObj.element.querySelector('.e-work-cells'))
              .offsetWidth
          ).toEqual(43);
          done();
        },
      ...
      expect(
        schObj.element
          .querySelector('.e-active-view')
          .classList.contains('e-month')
      ).toEqual(true);
      expect(
        (<HTMLElement>schObj.element.querySelector('.e-work-cells')).offsetWidth
  ...
  describe('Views and current view', () => {
    ...
    it('same view change in headerbar', () => {
      ...
      schObj.appendTo('#Schedule');
      let viewElement: HTMLElement = schObj.element.querySelector(
        '.e-toolbar-item.e-active-view'
      );
      expect(viewElement.classList.contains('e-week')).toEqual(true);
      viewElement.click();
      viewElement = schObj.element.querySelector(
        '.e-toolbar-item.e-active-view'
      );
      expect(viewElement.classList.contains('e-week')).toEqual(true);
    });
    ...
    it('view', () => {
      ...
      schObj.appendTo('#Schedule');
      expect(schObj.element.querySelectorAll('.e-toolbar-item').length).toEqual(
        8
      );
      expect(
    ...
    it('dateFormat - setmodel testing', () => {
      ...
      schObj.appendTo('#Schedule');
      expect(
        schObj.element.querySelectorAll('.e-schedule-toolbar').length
      ).toEqual(1);
      expect(
      ...
      expect(
        (<HTMLElement>schObj.element.querySelector('.e-header-cells')).innerHTML
      ).toEqual('<span>Sunday</span>');
    ...
    it('selectedDate - setmodel testing', () => {
      schObj = new Schedule(
        { selectedDate: new Date(2017, 9, 4), currentView: 'Month' },
        '#Schedule'
      );
      expect(
        schObj.element.querySelectorAll('.e-schedule-toolbar').length
    ...
    it('Test visibility of selected date from calendar', () => {
      ...
      const popupEle: Element = schObj.element.querySelector(
        '.e-schedule-toolbar-container .e-header-popup'
      );
      const calendarEle: Element = schObj.element.querySelector(
        '.e-schedule-toolbar-container .e-header-calendar'
      );
      ...
      expect(popupEle.classList.contains('e-popup-open')).toEqual(true);
      expect(
        calendarEle.querySelector('.e-header').classList.contains('e-month')
      ).toEqual(true);
      util.triggerMouseEvent(
        calendarEle.querySelector('.e-next') as HTMLElement,
        'mousedown'
      );
      (
        schObj.element.querySelectorAll(
          '.e-content.e-month tr:last-child td'
        )[2] as HTMLElement
      ).click();
      schObj.dataBind();
      expect(
        schObj.element.querySelector(
          '.e-content.e-month tr:last-child td:nth-last-child(5) span'
        ).innerHTML
      ).toEqual('31');
      expect(resetTime(new Date(+schObj.selectedDate))).toEqual(
        new Date(2017, 9, 31)
      );
      expect(
        (schObj.element.querySelector('.e-content-wrap') as HTMLElement)
          .scrollLeft
      ).toEqual(5700);
      (
        schObj.element.querySelectorAll(
      ...
      expect(popupEle.classList.contains('e-popup-open')).toEqual(true);
      (
        calendarEle.querySelector('.e-selected').nextSibling as HTMLElement
      ).click();
      expect(popupEle.classList.contains('e-popup-open')).toEqual(true);
      expect(
        calendarEle.querySelector('.e-header').classList.contains('e-month')
      ...
      (
        schObj.element.querySelectorAll(
          '.e-content.e-month tr:nth-last-child(4) td'
        )[5] as HTMLElement
      ).click();
      schObj.dataBind();
      expect(
        schObj.element.querySelector(
          '.e-content.e-month tr:nth-last-child(4) td:nth-last-child(2) span'
        ).innerHTML
      ).toEqual('17');
      expect(resetTime(new Date(+schObj.selectedDate))).toEqual(
        new Date(2017, 10, 17)
    ...
    it('currentView not in views list - direct rendering checking', () => {
      schObj = new Schedule({
        ...
        showHeaderBar: false,
        views: [{ option: 'Day' }, { option: 'Week' }, { option: 'Month' }],
      });
      schObj.appendTo('#Schedule');
    ...
    it('currentView not in views list - direct not rendering checking', () => {
      ...
      expect(
        schObj.element
          .querySelector('.e-active-view')
          .classList.contains('e-timeline-year')
      ).toEqual(true);
    });
  });
  ...
  describe('Current time indicator', () => {
    ...
    it('showTimeIndicator - setmodel testing', () => {
      ...
      expect(
        schObj.element.querySelectorAll('.e-current-timeline').length
      ).toEqual(1);
  ...
  describe('Persistence', () => {
    ...
    it('enablePersistence with true', () => {
      ...
      schObj.appendTo('#Schedule1');
      expect(schObj.selectedDate.toDateString()).toBe(
  ...
  describe('Css class', () => {
    ...
    it('update cssClass', () => {
      ...
      schObj.dataBind();
      expect(schObj.element.classList.contains('myCustomClass1')).toEqual(
        false
      );
      expect(schObj.element.classList.contains('myCustomClass2')).toEqual(true);
      schObj.cssClass = null;
      schObj.dataBind();
      expect(schObj.element.classList.contains('myCustomClass2')).toEqual(
        false
      );
    });
  });
  ...
  describe('Scroll', () => {
    ...
    it('check scroll content', () => {
      ...
      const timeCellsArea: HTMLElement = schObj.element.querySelector(
        '.e-time-cells-wrap'
      ) as HTMLElement;
  ...
  describe('changeCurrentView', () => {
    ...
    it('checking views change', () => {
      ...
      expect(
        (
          schObj.element.querySelector('.e-active-view') as HTMLElement
  ...
  describe('Testing event setting fields', () => {
    ...
    it('default values', (done: DoneFn) => {
      schObj.dataBound = () => {
        const eventElement: HTMLElement = schObj.element.querySelector(
          '[data-id="Appointment_1"]'
        );
        eventElement.click();
        const eventPopup: HTMLElement = document.querySelector(
          '.' + cls.EVENT_POPUP_CLASS
        );
        expect(eventPopup).toBeTruthy();
        expect(
          eventPopup.querySelector('.' + cls.SUBJECT_CLASS).innerHTML
        ).toEqual('Default Subject');
        expect(
          eventPopup.querySelector('.' + cls.LOCATION_DETAILS_CLASS).innerHTML
        ).toEqual('Default Location');
        expect(
          eventPopup.querySelector('.' + cls.DESCRIPTION_DETAILS_CLASS)
            .innerHTML
        ).toEqual('Default Description');
        (
          eventPopup.querySelector('.' + cls.CLOSE_CLASS) as HTMLElement
        ).click();
        done();
      };
      const workCell: HTMLElement =
        schObj.element.querySelector('.e-work-hours');
  ...
  describe('showWeekNumber property testing', () => {
    ...
    it('week view testing', (done: DoneFn) => {
      const dataBound: EmitType<Record<string, any>> = () => {
        done();
      };
      schObj = new Schedule({
        height: '500px',
      ...
      schObj.appendTo('#Schedule');
      const weekNumber: Element = schObj.element.querySelector(
    ...
    it('month view testing', (done: DoneFn) => {
      ...
      schObj.appendTo('#Schedule');
      expect(
        schObj.element.querySelectorAll('.e-week-number-wrapper').length
      ).toEqual(1);
      const weekNumber: Element[] = [].slice.call(
        schObj.element.querySelectorAll('.e-week-number-wrapper .e-week-number')
      );
    ...
    it('setmodel testing', (done: DoneFn) => {
      ...
      schObj.appendTo('#Schedule');
      let weekNumber: Element = schObj.element.querySelector(
        '.e-left-indent-wrap .e-header-cells'
      );
      expect(weekNumber.innerHTML).toEqual('<span title="Week 14">14</span>');
      expect(weekNumber.classList.contains('e-week-number')).toEqual(true);
      ...
      schObj.dataBind();
      weekNumber = schObj.element.querySelector(
        '.e-left-indent-wrap .e-header-cells'
      );
  ...
  describe('Public methods', () => {
    ...
    it('isSlotAvailable', () => {
      let availablity: boolean = schObj.isSlotAvailable(
        new Date(2017, 9, 30, 10, 0),
        new Date(2017, 9, 30, 11, 0)
      );
      expect(availablity).toEqual(true);
      availablity = schObj.isSlotAvailable(
        new Date(2017, 10, 6, 10, 0),
        new Date(2017, 10, 6, 11, 0)
      );
      expect(availablity).toEqual(false);
    });

    it('getStartEndTime with valid time format', () => {
      expect(schObj.getStartEndTime('08:30')).toEqual(
    ...
    it('getStartEndTime with invalid time format', () => {
      expect(schObj.getStartEndTime('08')).toEqual(
    ...
    it('openQuickInfoPopup - Event', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          eventPopup.querySelector('.e-date-time-details').innerHTML
        ).toEqual('November 5, 2017 (1:00 PM&nbsp;-&nbsp;2:00 PM)');
        expect(eventPopup.querySelector('.e-subject').innerHTML).toEqual(
          'Event'
        );
    ...
    it('openQuickInfoPopup - Recurring Event', () => {
      ...
      expect(
        eventPopup.querySelector('.e-date-time-details').innerHTML
      ).toEqual('November 6, 2017 (10:00 AM&nbsp;-&nbsp;11:30 AM)');
      expect(eventPopup.querySelector('.e-subject').innerHTML).toEqual(
        'Recurring Event'
      );
      (<HTMLElement>eventPopup.querySelector('.e-close')).click();
    ...
    it('openQuickInfoPopup - Cell', () => {
      ...
      expect(
        eventPopup.querySelector('.e-date-time-details').innerHTML
      ).toEqual('November 6, 2017 (10:00 AM&nbsp;-&nbsp;11:00 AM)');
      (<HTMLElement>eventPopup.querySelector('.e-close')).click();
    });
  ...
  describe('Testing resetWorkHours with parameteres in Vertical view', () => {
    ...
    it('Checking Work hour class is avilable or not on particular cell', () => {
      const workCells: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.' + cls.WORK_CELLS_CLASS)
      );
  ...
  describe('Testing resetWorkHours on passing group index in Vertical view', () => {
    ...
    it('Checking the particular cell contains a work hour class or not', () => {
      const tableRows: NodeListOf<HTMLTableRowElement> =
        schObj.element.querySelectorAll('tr');
      expect(
        tableRows[80].children[10].classList.contains('e-work-hours')
      ).toEqual(true);
      expect(
        tableRows[82].children[10].classList.contains('e-work-hours')
      ).toEqual(true);
    });
  ...
  describe('Testing resetWorkHours on passing group index in Timeline view', () => {
    ...
    it('Checking the particular cell contains a work hour class or not', () => {
      const tableRows: NodeListOf<HTMLTableRowElement> =
        schObj.element.querySelectorAll('tr');
      expect(
        tableRows[11].children[163].classList.contains('e-work-hours')
      ).toEqual(true);
      expect(
        tableRows[11].children[166].classList.contains('e-work-hours')
      ).toEqual(true);
    });
    it('Removing work hour class from particular cell by calling resetWorkHours', () => {
      const dates: Date[] = [new Date(2019, 5, 12)];
  ...
  describe('CR Issue EJ2-16536 Schedule within hidden element', () => {
    ...
    const hiddenEle: HTMLElement = createElement('div', {
      styles: 'display:none',
    });
  ...
  describe('EJ2-23004-24 hours format is not displaying in time cells in adaptive mode only', () => {
    ...
    it('Checking elements', () => {
      expect(
        (
          schObj.element.querySelector(
            '.e-time-cells-wrap tbody tr:nth-child(1) td'
          ) as HTMLElement
        ).innerText
      ).toEqual('0');
      expect(
        (
          schObj.element.querySelector(
            '.e-time-cells-wrap tbody tr:nth-child(3) td'
          ) as HTMLElement
        ).innerText
      ).toEqual('1');
      expect(
        (
          schObj.element.querySelector(
            '.e-time-cells-wrap tbody tr:nth-child(25) td'
          ) as HTMLElement
        ).innerText
      ).toEqual('12');
      expect(
        (
          schObj.element.querySelector(
            '.e-time-cells-wrap tbody tr:nth-child(27) td'
          ) as HTMLElement
        ).innerText
      ).toEqual('13');
      expect(
        (
          schObj.element.querySelector(
            '.e-time-cells-wrap tbody tr:nth-child(45) td'
          ) as HTMLElement
        ).innerText
      ).toEqual('22');
      expect(
        (
          schObj.element.querySelector(
            '.e-time-cells-wrap tbody tr:nth-child(47) td'
          ) as HTMLElement
        ).innerText
      ).toEqual('23');
    });
  ...
  describe('CR Issue EJ2-26338 readonly events', () => {
    ...
    it('Delete key', () => {
      ...
      const selectedElement: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment-border')
      );
      ...
      expect(
        schObj.quickPopup.quickPopup.element
          .querySelector('.e-edit')
          .getAttribute('disabled')
      ).toBe('');
      expect(
        schObj.quickPopup.quickPopup.element
          .querySelector('.e-delete')
          .getAttribute('disabled')
      ).toBe('');
      expect(
        schObj.quickPopup.quickPopup.element
          .querySelector('.e-close')
          .getAttribute('disabled')
      ).toBe(null);
      (<HTMLElement>(
        schObj.quickPopup.quickPopup.element.querySelector('.e-close')
      )).click();
    });
  });
  ...
  describe('CR Issue EJ2-28683 recurrence appoinments', () => {
    ...
    const data: Record<string, any>[] = [
      {
        Id: 1,
        Subject: 'Paris',
        StartTime: timeZone.removeLocalOffset(
          new Date('2019-02-06T04:30:00.000Z')
        ),
        EndTime: timeZone.removeLocalOffset(
          new Date('2019-02-06T06:00:00.000Z')
        ),
        IsAllDay: false,
        RecurrenceRule:
          'FREQ=MONTHLY;BYMONTHDAY=5,6,7;BYDAY=MO,TU,WE,TH,FR;BYSETPOS=1',
      },
    ];
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        height: '500px',
        width: '500px',
        selectedDate: new Date(2019, 6, 5),
        currentView: 'Month',
    ...
    it('Check Aug 2019 Appointment', (done: DoneFn) => {
      schObj.dataBound = () => {
        const selector: string = `.e-work-cells[data-date="${new Date(
          2019,
          7,
          5
        ).getTime()}"] .e-appointment`;
        expect(schObj.element.querySelector(selector)).not.toBeNull();
        done();
      };
      (
        schObj.element.querySelector(
          '.e-schedule-toolbar .e-icon-next'
        ) as HTMLElement
      ).click();
      schObj.dataBind();
    });
  ...
  describe('Testing first day of week on view basis', () => {
    ...
    it('Checking first day of week in day view.', () => {
      ...
      expect(popupEle.querySelector('.e-week-header th').textContent).toBe(
        'Mo'
      );
      (
        schObj.element.querySelectorAll(
      ...
      const startTimePicker: DateTimePicker = (
        dialogElement.querySelector('.e-start') as EJ2Instance
      ...
      const endTimePicker: DateTimePicker = (
        dialogElement.querySelector('.e-end') as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
      expect(endTimePicker.firstDayOfWeek.toString()).toEqual('1');
      const untilDatePicker: DateTimePicker = (
        dialogElement.querySelector('.e-until-date') as EJ2Instance
      ).ej2_instances[0] as DateTimePicker;
      ...
      const btnElement: HTMLElement = [].slice.call(
        document.querySelectorAll('.e-round.e-lib.e-btn.e-control')
      )[1] as HTMLElement;
      expect(btnElement.title).toEqual('Monday');
      repeatElement.index = 3;
      repeatElement.dataBind();
      const dayElement: DropDownList = (
        dialogElement.querySelector('.e-month-week') as EJ2Instance
      ).ej2_instances[0] as DropDownList;
      dayElement.index = 0;
      dayElement.dataBind();
      ...
      (
        document.querySelector(
          '.e-control.e-btn.e-lib.e-event-cancel'
        ) as HTMLElement
      ).click();
    });
    it('Checking first day of week in week view.', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        const headerElement: HTMLElement = schObj.element.querySelector(
          '.e-header-row'
    ...
    it('Checking first day of week in Month view.', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(
          (schObj.element.querySelector('.e-header-cells') as HTMLElement)
            .innerText
        ).toEqual('Thursday');
        util.triggerMouseEvent(
          schObj.element.querySelectorAll('.e-work-cells')[0] as HTMLElement,
  ...
  describe('view-based template checking', () => {
    ...
    const dateHeaderTemplate: string =
      '<div class="e-custom-date-header">${date.toLocaleString()}</div>';
    const cellHeaderTemplate: string =
      '<div class="e-custom-cell-header">${date.toLocaleString()}</div>';
    const cellTemplate: string =
      '<div class="e-custom-cell">${date.toLocaleString()}</div>';
    const dateRangeTemplate: string =
      '<div class="date-text">${(data.startDate).getDate()}-${(data.endDate).getDate()}</div>';
    beforeAll((done: DoneFn) => {
      const model: ScheduleModel = {
        selectedDate: new Date(2019, 11, 5),
        views: [
    ...
    it('templates testing in week view', () => {
      expect(
        schObj.element.querySelectorAll('.e-custom-date-header').length
      ).toEqual(7);
      expect(schObj.element.querySelectorAll('.e-custom-cell').length).toEqual(
        0
      );
      expect(
        schObj.element.querySelectorAll('.e-custom-cell-header').length
      ).toEqual(0);
    ...
    it('templates testing in month view', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element.querySelectorAll('.e-custom-cell').length
        ).toEqual(35);
        expect(
          schObj.element.querySelectorAll('.e-custom-cell-header').length
        ).toEqual(35);
        expect(
          schObj.getDeletedOccurrences(schObj.eventsData[0]).length
        ).toEqual(2);
        done();
      };
      schObj.currentView = 'Month';
    ...
    it('templates testing in year view', (done: DoneFn) => {
      schObj.dataBound = () => {
        expect(
          schObj.element.querySelectorAll('.e-tbar-btn-text').length
        ).toEqual(5);
        expect(
          schObj.element.querySelectorAll('.e-tbar-btn-text')[0].innerHTML
        ).toEqual('<div class="date-text">1-31</div>');
        done();
      };
  ...
  describe('refreshTemplate checking', () => {
    ...
    const appTemplate: string =
      '<div class="app-template"><div class="subject">~${Subject}~</div></div>';
    const dateHeaderTemplate: string =
      '<span>~${getDateHeaderText(data.date)}~</span>';
    const cellTemplate: string =
      '${if(type === "workCells")}<div class="templatewrap">${getCellText(data.date)}</div>${/if}' +
      '${if(type === "monthCells")}<div class="templatewrap">${getMonthCellText(data.date)}</div>${/if}';
    const cellHeaderTemplate: string =
      '<div class="e-custom-cell">~${getDateHeaderText(data.date)}</div>';
    const resourceHeaderTemp: string =
    ...
    it('refresh templates testing in Day view', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element.querySelectorAll('.resource-template').length
        ).toEqual(5);
        expect(
          schObj.element.querySelectorAll(
            '.resource-template .resource-name'
          )[0].innerHTML
        ).toEqual('~Room 1~');
        ...
        expect(schObj.element.querySelectorAll('.app-template').length).toEqual(
          1
        );
        expect(
          schObj.element.querySelector('.app-template .subject').innerHTML
        ).toEqual('~Testing~');
        ...
        ).toEqual('<div class="date-text">4-4</div>');
    ...
    it('refresh templates testing in Week view', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element.querySelector('.e-tbar-btn-text').innerHTML
        ).toEqual('<div class="date-text">29-4</div>');
        schObj.refreshTemplates('dateRangeTemplate');
        expect(
          schObj.element.querySelector('.e-tbar-btn-text').innerHTML
        ).toEqual('<div class="date-text">29-4</div>');
    ...
    it('refresh templates testing in Agenda views', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        // dateHeaderTemplate checking
        expect(
          schObj.element.querySelector('.e-agenda-cells .e-day-date-header')
            .innerHTML
        ).toEqual('<span>~Sat, 1/4~</span>');
        schObj.refreshTemplates('dateHeaderTemplate');
        expect(
        ...
        expect(
          schObj.element.querySelector('.e-tbar-btn-text').innerHTML
        ).toEqual('<div class="date-text">4-10</div>');
        schObj.refreshTemplates('dateRangeTemplate');
        expect(
          schObj.element.querySelector('.e-tbar-btn-text').innerHTML
        ).toEqual('<div class="date-text">4-10</div>');
    ...
    it('refresh templates testing in MonthAgenda views', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element.querySelectorAll('.e-appointment-indicator').length
        ).toEqual(2);
        expect(
          schObj.element.querySelector('.app-template .subject').innerHTML
        ).toEqual('~Testing~');
    ...
    it('refresh templates testing in TimelineWorkWeek view', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        expect(
          schObj.element.querySelector('.e-tbar-btn-text').innerHTML
        ).toEqual('<div class="date-text">30-3</div>');
        schObj.refreshTemplates('dateRangeTemplate');
        expect(
          schObj.element.querySelector('.e-tbar-btn-text').innerHTML
        ).toEqual('<div class="date-text">30-3</div>');
    ...
    it('openQuickInfoPopup with resource - Event', () => {
      ...
      expect(
        eventPopup.querySelector('.e-date-time-details').innerHTML
      ).toEqual('January 1, 2020 (10:00 AM&nbsp;-&nbsp;12:00 PM)');
      expect(eventPopup.querySelector('.e-resource-details').innerHTML).toEqual(
        'Nancy'
      );
      (<HTMLElement>eventPopup.querySelector('.e-close')).click();
    });
    it('openQuickInfoPopup with resource - Cell', () => {
      ...
      expect(cellPopup.querySelector('.e-date-time-details').innerHTML).toEqual(
        'January 4, 2020 (10:00 AM&nbsp;-&nbsp;12:00 PM)'
      );
      expect(cellPopup.querySelector('.e-resource-details').innerHTML).toEqual(
        'Steven'
      );
  ...
  describe('Capitalization checking on french culture', () => {
    ...
    const eventData: Record<string, any>[] = [
      {
        ...
        RecurrenceRule:
          'FREQ=WEEKLY;BYDAY=SU,MO,TU,WE,TH,FR,SA;INTERVAL=1;COUNT=12;',
        IsAllDay: true,
      },
    ];
    beforeEach((): void => {
    ...
    it('Checking french culture on week view', () => {
      util.loadCultureFiles('fr-CH');
      schObj = new Schedule(
        { selectedDate: new Date(2019, 10, 19), locale: 'fr-CH' },
        '#Schedule'
      );

      const headerRows: NodeListOf<Element> =
        schObj.element.querySelectorAll('.e-header-day');
    ...
    it('Checking french culture on month view', () => {
      ...
      schObj = new Schedule(
        {
          selectedDate: new Date(2019, 10, 1),
          currentView: 'Month',
          locale: 'fr-CH',
        },
        '#Schedule'
      ...
      const headerRows: HTMLElement[] = [].slice.call(
        schObj.element.querySelectorAll('.e-header-cells')
      ) as HTMLElement[];
    ...
    it('Checking french culture event popup on month view', (done: DoneFn) => {
      ...
      schObj.dataBound = () => {
        ...
        expect(eventPopup).toBeTruthy();
        expect(
          (eventPopup.querySelector('.e-date-time-details') as HTMLElement)
            .innerText
        ).toEqual('7 Novembre 2019 (All day)');
        expect(
          (eventPopup.querySelector('.e-recurrence-summary') as HTMLElement)
            .innerText
        ).toEqual(
          'Every week(s) on Dim., Lun., Mar., Mer., Jeu., Ven., Sam., 12 time(s)'
        );
  ...
  describe('EJ2-50031 - Form validator locale not working', () => {
    ...
    it('Testing chinese locale form validator', () => {
      ...
      saveButton.click();
      expect(
        (document.querySelector('.e-tip-content') as HTMLElement).innerText
      ).toEqual('此字段是必需的。');
    });
  });
  ...
  describe('EJ2-59128 - Events are not displayed when min-max date set to schedule', () => {
    ...
    it('Checking appointments rendering in min-max date range in week and Month view ', (done: DoneFn) => {
      schObj.dataBound = () => {
        ...
        const renderedAppointments: HTMLElement[] = [].slice.call(
          schObj.element.querySelectorAll('.' + cls.APPOINTMENT_CLASS)
        );
  ...
  describe('Hide Tooltip when context menu is triggered', () => {
    ...
    it('Hiding tooltip with closeTooltip api', () => {
      const target: HTMLElement = [].slice.call(
        schObj.element.querySelectorAll('.e-appointment')
      )[1];
      expect(document.querySelector('.e-schedule-event-tooltip')).toBeNull();
      util.triggerMouseEvent(target, 'mouseover');
      const tooltipEle: HTMLElement = document.querySelector(
        '.e-schedule-event-tooltip'
      ) as HTMLElement;
      expect(isVisible(tooltipEle)).toBe(true);
      expect(tooltipEle.querySelector('.e-subject').innerHTML).toBe(
        'Normal Event'
      );
      expect(tooltipEle.querySelector('.e-location').innerHTML).toBe('');
      expect(tooltipEle.querySelector('.e-details').innerHTML).toBe(
        'January 3, 2018'
      );
      expect(tooltipEle.querySelector('.e-all-day').innerHTML).toBe(
        '10:00 AM - 11:00 AM'
      );
      schObj.closeTooltip();
      schObj.dataBind();
