...
export function addResourceField(
  ...
  if (!groupField) {
    ...
    for (let a: number = groupStart; a <= groupEnd; a++) {
      const eve: Record<string, any>[] = dm.executeLocal(
        new Query().where(groupField, 'equal', a)
      ) as Record<string, any>[];
      const filteredRes: Record<string, any>[] = dm1.executeLocal(
        new Query().where('GroupID', 'equal', a).select(['Id'])
      ) as Record<string, any>[];
      const possibleIds: number[] = (filteredRes as Record<string, any>[]).map(
        (a: Record<string, any>) => a.Id as number
      );
      const addField: CallbackFunction = (
 ...
 * @private
 */
export function yearDataGenerator(
  count: number = 100,
  date: Date = new Date(),
  yearCount: number = 0
): Record<string, any>[] {
 ...
 * @private
 */
export function generateResourceDatasource(
  startId: number,
  endId: number,
  text: string
): Record<string, any>[] {
 ...
 * @private
 */
export function generateAllDayData(
  count: number = 30,
  date: Date = new Date()
): Record<string, any>[] {
  const dateCollections: Record<string, any>[] = [];
