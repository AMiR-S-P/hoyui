...
export class EventTooltip {
  ...
  private onBeforeRender(args: TooltipEventArgs): void {
    ...
    if (
      ...
      if (this.parent.activeView.isTimelineView()) {
        const index: number = parseInt(
          args.target.getAttribute('data-group-index') as string,
          10
        );
        resCollection =
        ...
        const rowIndex: number = (args.target.parentNode as HTMLTableRowElement)
          .sectionRowIndex;
        const cellIndex: number = (args.target as HTMLTableCellElement)
          .cellIndex;
        resCollection =
          this.parent.activeView.getColumnLevels()[
      ...
      const templateId: string =
        this.parent.element.id + '_headerTooltipTemplate';
    ...
    const record: Record<string, any> = this.parent.eventBase.getEventByGuid(
    ...
    if (!isNullOrUndefined(this.parent.eventSettings.tooltipTemplate)) {
      ...
      eventEnd =
        eventEnd.getTime() - eventStart.getTime() !== 0 &&
        eventEnd.getHours() === 0 &&
      ...
      const tooltipTime: string = record[fields.isAllDay]
