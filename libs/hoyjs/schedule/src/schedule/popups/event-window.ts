...
import { Query, Deferred } from '@hoyui/data';
import { CheckBox, ChangeEventArgs, Button } from '@hoyui/buttons';
import {
  Dialog,
  DialogModel,
  BeforeOpenEventArgs,
...
import {
  ...
  FilteringEventArgs,
  MultiSelect,
  ChangeEventArgs as ddlChangeEventArgs,
  MultiSelectChangeEventArgs,
} from '@hoyui/dropdowns';
import { Input, FormValidator, NumericTextBox } from '@hoyui/inputs';
import { DatePicker, DateTimePicker, ChangedEventArgs } from '@hoyui/calendars';
...
export class EventWindow {
  ...
  private renderEventWindow(): void {
    ...
    if (this.parent.isAdaptive) {
      dialogModel.cssClass =
        cls.EVENT_WINDOW_DIALOG_CLASS + ' ' + cls.DEVICE_CLASS;
      if (!this.parent.editorHeaderTemplate) {
        dialogModel.header =
          '<div class="e-title-header"><div class="e-back-icon e-icons"></div><div class="e-title-text">' +
          this.l10n.getConstant('newEvent') +
          '</div><div class="e-save-icon e-icons"></div></div>';
      }
    } else {
    ...
    if (this.dialogObject.element.querySelector('.e-dlg-closeicon-btn')) {
      this.dialogObject.element
        .querySelector('.e-dlg-closeicon-btn')
        .setAttribute('title', this.l10n.getConstant('close'));
    }
    this.addEventHandlers();
    addClass(
      [this.element.parentElement],
      cls.EVENT_WINDOW_DIALOG_CLASS + '-container'
    );
    EventHandler.add(
      this.dialogObject.element,
      'keydown',
      this.preventEventSave,
      this
    );
    ...
    dialogButton.buttons = [
      {
        buttonModel: {
          ...
          disabled:
            !this.parent.eventSettings.allowDeleting || this.parent.readonly,
        },
      ...
      },
      {
        buttonModel: {
          ...
          isPrimary: true,
          disabled:
            !this.parent.eventSettings.allowAdding || this.parent.readonly,
        },
  ...
  private addEventHandlers(): void {
    const backIcon: Element = this.element.querySelector(
      '.' + cls.EVENT_WINDOW_BACK_ICON_CLASS
    );
    const saveIcon: Element = this.element.querySelector(
      '.' + cls.EVENT_WINDOW_SAVE_ICON_CLASS
    );
    if (
      this.parent.isAdaptive &&
      !isNullOrUndefined(backIcon) &&
      !isNullOrUndefined(saveIcon)
    ) {
  ...
  public openEditor(
    ...
    if (
      !isNullOrUndefined(this.parent.editorHeaderTemplate) ||
      !isNullOrUndefined(this.parent.editorFooterTemplate)
    ) {
    ...
    if (!isNullOrUndefined(this.parent.editorTemplate)) {
      this.renderFormElements(
        this.element.querySelector('.e-schedule-form'),
        data,
        type,
        repeatType
      );
  ...
  private setEditorContent(
    ...
    if (
      !this.parent.isAdaptive &&
      isNullOrUndefined(this.parent.editorTemplate)
    ) {
      removeClass(
        [this.dialogObject.element.querySelector('.e-recurrenceeditor')],
        cls.DISABLE_CLASS
      );
    }
    if (this.recurrenceEditor) {
      this.recurrenceEditor.firstDayOfWeek =
        this.parent.activeViewOptions.firstDayOfWeek;
    }
    switch (type) {
      case 'Add':
  ...
  private createAdaptiveHeaderElement(data?: Record<string, any>): HTMLElement {
    ...
    const headerBackIcon: HTMLElement = createElement('div', {
    ...
    const headerSaveIcon: HTMLElement = createElement('div', {
  ...
  private getDialogHeader(args?: Record<string, any>): HTMLElement {
    ...
    const headerTemplateId: string =
      this.parent.element.id + '_editorHeaderTemplate';
    const temHeaderDiv: HTMLElement = document.createElement('div');
    headerTemplate = [].slice.call(
      this.parent.getEditorHeaderTemplate()(
        args || {},
        this.parent,
  ...
  private getDialogFooter(args?: Record<string, any>): HTMLElement {
    ...
    const footerTemplateId: string =
      this.parent.element.id + '_editorFooterTemplate';
    const temFooterDiv: HTMLElement = document.createElement('div');
    footerTemplate = [].slice.call(
      this.parent.getEditorFooterTemplate()(
        args || {},
        this.parent,
  ...
  private preventEventSave(e: KeyboardEventArgs): void {
    if (
      this.parent &&
      !this.parent.allowKeyboardInteraction &&
      (e as KeyboardEventArgs).code === 'Enter'
    ) {
  ...
  private onBeforeOpen(args: BeforeOpenEventArgs): Deferred {
    ...
    const eventProp: PopupOpenEventArgs = {
      ...
      target: (this.cellClickAction
        ? this.parent.activeCellsData.element
        : this.parent.activeEventData.element) as HTMLElement,
    };
    if (this.cellClickAction) {
      eventProp.duration = this.getSlotDuration();
    }
    const saveObj: Button = this.getInstance(
      cls.EVENT_WINDOW_SAVE_BUTTON_CLASS
    ) as unknown as Button;
    if (saveObj) {
      saveObj.disabled = !(this.cellClickAction
        ? this.parent.eventSettings.allowAdding
        : this.parent.eventSettings.allowEditing);
      saveObj.dataBind();
    }
    const deleteObj: Button = this.getInstance(
      cls.DELETE_EVENT_CLASS
    ) as unknown as Button;
    if (deleteObj) {
    ...
    this.parent.trigger(
      ...
      (popupArgs: PopupOpenEventArgs) => {
        ...
        if (
          ...
          this.eventData = <Record<string, any>>(
            extend({}, this.eventData, null, true)
          );
        ...
        if (
          this.cellClickAction &&
          popupArgs.duration !== this.getSlotDuration() &&
          isNullOrUndefined(this.parent.editorTemplate)
        ) {
          const startObj: DateTimePicker = this.getInstance(
            cls.EVENT_WINDOW_START_CLASS
          ) as unknown as DateTimePicker;
          const endObj: DateTimePicker = this.getInstance(
            cls.EVENT_WINDOW_END_CLASS
          ) as unknown as DateTimePicker;
          endObj.value = new Date(
            startObj.value.getTime() + util.MS_PER_MINUTE * popupArgs.duration
          );
          endObj.dataBind();
        }
        if (
          this.parent.editorTemplate &&
          this.element.querySelector('.e-recurrenceeditor') &&
          !this.recurrenceEditor
        ) {
          this.recurrenceEditor = this.getInstance(
            'e-recurrenceeditor'
          ) as unknown as RecurrenceEditor;
        }
  ...
  private renderFormElements(
    ...
    if (!isNullOrUndefined(this.parent.editorTemplate)) {
      ...
      const tempEle: HTMLElement[] = [].slice.call(
        this.parent.getEditorTemplate()(
          args || {},
          this.parent,
          'editorTemplate',
          templateId,
  ...
  private getDefaultEventWindowContent(): HTMLElement {
    const parentDiv: HTMLElement = this.createDivElement(
      cls.EVENT_WINDOW_DIALOG_PARENT_CLASS
    );
    const titleLocationDiv: HTMLElement = this.createDivElement(
      cls.EVENT_WINDOW_TITLE_LOCATION_DIV_CLASS
    );
    ...
    const startEndDateTimeDiv: HTMLElement = this.createDivElement(
      cls.EVENT_WINDOW_START_END_DIV_CLASS
    );
    parentDiv.appendChild(startEndDateTimeDiv);
    startEndDateTimeDiv.appendChild(
    ...
    startEndDateTimeDiv.appendChild(
      this.renderDateTimePicker(cls.EVENT_WINDOW_END_CLASS)
    );
    const allDayTimezoneDiv: HTMLElement = this.createDivElement(
      cls.EVENT_WINDOW_ALLDAY_TZ_DIV_CLASS
    );
    parentDiv.appendChild(allDayTimezoneDiv);
    allDayTimezoneDiv.appendChild(
      this.renderCheckBox(cls.EVENT_WINDOW_ALL_DAY_CLASS)
    );
    allDayTimezoneDiv.appendChild(this.renderCheckBox(cls.TIME_ZONE_CLASS));
    const timezoneParentDiv: HTMLElement = this.createDivElement(
      cls.EVENT_WINDOW_TIME_ZONE_DIV_CLASS
    );
    parentDiv.appendChild(timezoneParentDiv);
    timezoneParentDiv.appendChild(
      this.renderDropDown(cls.EVENT_WINDOW_START_TZ_CLASS)
    );
    timezoneParentDiv.appendChild(
      this.renderDropDown(cls.EVENT_WINDOW_END_TZ_CLASS)
    );
    const repeatParentDiv: HTMLElement = this.createDivElement(
      cls.EVENT_WINDOW_REPEAT_DIV_CLASS
    );
    parentDiv.appendChild(repeatParentDiv);
    const repeatDiv: HTMLElement = this.renderCheckBox(
      cls.EVENT_WINDOW_REPEAT_CLASS
    );
    const repeatEditContainer: HTMLElement = createElement('span', {
    ...
    if (this.parent.resourceCollection.length > 0) {
      const resourceParentDiv: HTMLElement = this.createDivElement(
        cls.EVENT_WINDOW_RESOURCES_DIV_CLASS
      );
    ...
    const description: HTMLElement = this.createDivElement(
      cls.DESCRIPTION_CLASS + '-row'
    );
    description.appendChild(this.renderTextBox(cls.DESCRIPTION_CLASS));
    parentDiv.appendChild(description);
    const submit: HTMLElement = createElement('button', {
      attrs: { type: 'hidden', title: 'submit', style: 'display:none' },
    });
  ...
  private createRecurrenceEditor(parentDiv: HTMLElement): void {
    const recurrenceEditor: HTMLElement = createElement('div', {
  ...
  private renderDateTimePicker(
    value: string,
    changeEvent?: EmitType<ChangedEventArgs>
  ): HTMLElement {
    const dateTimeDiv: HTMLElement = this.createDivElement(
      value + '-container'
    );
    const fieldName: string = this.getFieldName(value);
    const dateTimeInput: HTMLElement = this.createInputElement(
      value + ' ' + EVENT_FIELD,
      fieldName
    );
    ...
    const dateTimePicker: DateTimePicker = new DateTimePicker({
      ...
      format:
        (isNullOrUndefined(this.parent.dateFormat)
          ? this.getFormat('dateFormats')
          : this.parent.dateFormat) +
        ' ' +
        this.parent.activeViewOptions.timeFormat,
  ...
  public refreshDateTimePicker(duration?: number): void {
    const elementSelector: string =
      '.' + cls.EVENT_WINDOW_START_CLASS + ',.' + cls.EVENT_WINDOW_END_CLASS;
    const startEndElement: HTMLElement[] = [].slice.call(
      this.element.querySelectorAll(elementSelector)
    );
    for (const element of startEndElement) {
      const instance: DateTimePicker = ((<HTMLElement>element) as EJ2Instance)
  ...
  private onTimeChange(): void {
    ...
    if (startObj.element.parentElement.classList.contains('e-input-focus')) {
      ...
      if (this.cellClickAction) {
        ...
        duration =
          (<Date>this.eventData[this.fields.endTime]).getTime() -
          (<Date>this.eventData[this.fields.startTime]).getTime();
      }
      const endDate: Date = isNullOrUndefined(startObj.value)
        ? null
        : new Date(startObj.value.getTime() + duration);
      if (this.cellClickAction) {
  ...
  private renderResourceDetails(resourceData: ResourcesModel): HTMLElement {
    ...
    const resourceDiv: HTMLElement = this.createDivElement(
      value + '-container' + ' ' + 'e-resources'
    );
    const resourceInput: HTMLElement = this.createInputElement(
      value + ' ' + EVENT_FIELD,
      fieldName
    );
    resourceDiv.appendChild(resourceInput);
    const resourceTemplate: Function = function (data: any): string {
      return SanitizeHtmlHelper.sanitize(
        `<div class="e-resource-template"><div class="e-resource-color" style="background-color:${
          data[resourceData.colorField]
        }"></div><div class="e-resource-text">${
  ...
  private renderDropDown(value: string): HTMLElement {
    const fieldName: string = this.getFieldName(value);
    const timezoneDiv: HTMLElement = this.createDivElement(
      value + '-container'
    );
    const timezoneInput: HTMLElement = this.createInputElement(
      value + ' ' + EVENT_FIELD,
      fieldName
    );
  ...
  private onMultiselectResourceChange(args: MultiSelectChangeEventArgs): void {
    if (
      ...
      this.parent.resourceCollection.length <= 1
    ) {
      return;
    }
    const resourceCollection: ResourcesModel[] =
      this.parent.resourceBase.resourceCollection;
    const fieldName: string =
      args.element.getAttribute('name') ||
      this.getColumnName(args.element as HTMLInputElement);
    for (let i: number = 0; i < resourceCollection.length; i++) {
      if (
        resourceCollection[parseInt(i.toString(), 10)].field === fieldName &&
        i < resourceCollection.length - 1
      ) {
        ...
        for (let j: number = 0; j < args.value.length; j++) {
          ...
          const filter: Record<string, any> = (
            resourceModel.dataSource as Record<string, any>[]
          ).filter(
            (data: Record<string, any>) =>
              data[resourceModel.groupIDField] ===
              args.value[parseInt(j.toString(), 10)]
          )[0];
          const groupId: number = !isNullOrUndefined(filter)
            ? (filter[resourceCollection[i + 1].groupIDField] as number)
            : null;
          const filterRes: Record<string, any>[] = this.filterDatasource(
            i,
            groupId
          );
  ...
  private createInstance(index: number): MultiSelect | DropDownList {
    const resourceData: Record<string, any> = this.parent.resourceBase
      .resourceCollection[index + 1] as Record<string, any>;
    const resObject: MultiSelect | DropDownList = (
      this.element.querySelector('.e-' + resourceData.field) as EJ2Instance
  ...
  private onDropdownResourceChange(args: ddlChangeEventArgs): void {
    ...
    for (let i: number = 0; i < resourceCollection.length; i++) {
      if (
        ...
        const groupId: number = (args.itemData as Record<string, any>)[
          resourceCollection[parseInt(i.toString(), 10)].idField
        ] as number;
        resObj.dataSource = this.filterDatasource(i, groupId);
        resObj.dataBind();
        const resValue: string =
          resObj.dataSource.length > 0
            ? (resObj.dataSource[0][
                resourceCollection[i + 1].idField
              ] as string)
            : null;
  ...
  private filterDatasource(
    ...
    const resourceData: ResourcesModel =
      this.parent.resourceBase.resourceCollection[index + 1];
    return (resourceData.dataSource as Record<string, any>[]).filter(
      (data: Record<string, any>) => data[resourceData.groupIDField] === groupId
    );
  }
  ...
  private onTimezoneChange(args: ddlChangeEventArgs): void {
    ...
    if (fieldName === this.parent.eventFields.startTimezone) {
      const startTimezoneObj: DropDownList = this.getInstance(
        cls.EVENT_WINDOW_START_TZ_CLASS
      ) as unknown as DropDownList;
      const endTimezoneObj: DropDownList = this.getInstance(
        cls.EVENT_WINDOW_END_TZ_CLASS
      ) as unknown as DropDownList;
  ...
  private renderCheckBox(value: string): HTMLElement {
    const checkBoxDiv: HTMLElement = this.createDivElement(
      value + '-container'
    );
    const fieldName: string = this.getFieldName(value);
    const checkBoxInput: HTMLElement = this.createInputElement(
      value + ' ' + EVENT_FIELD,
      fieldName
    );
  ...
  private renderTextBox(value: string): HTMLElement {
    ...
    const elementType: string =
      value === cls.DESCRIPTION_CLASS ? 'textarea' : 'input';
    const textBoxInput: HTMLElement = this.createInputElement(
      value + ' ' + EVENT_FIELD,
      fieldName,
      elementType
    );
  ...
  private renderRepeatDialog(): void {
    ...
    this.createRecurrenceEditor(
      <HTMLElement>(
        this.repeatDialogObject.element.querySelector('.e-rec-editor')
      )
    );
  }
  ...
  private onRepeatChange(state: boolean): void {
    if (state) {
      ...
      this.recurrenceEditor.setProperties({
        startDate: <Date>this.repeatStartDate,
        selectedType: 0,
      });
      ...
      const element: HTMLElement = <HTMLElement>(
        this.element.querySelector('.' + REPEAT_CONTAINER_CLASS)
      );
  ...
  private onCellDetailsUpdate(
    ...
    if (isNullOrUndefined(this.parent.editorHeaderTemplate)) {
      this.element.querySelector(
        '.' + cls.EVENT_WINDOW_TITLE_TEXT_CLASS
      ).innerHTML = this.l10n.getConstant('newEvent');
    }
    ...
    if (!isNullOrUndefined(this.parent.eventSettings.fields.location.default)) {
      eventObj[this.fields.location] =
        this.parent.eventSettings.fields.location.default;
    }
    if (
      !isNullOrUndefined(this.parent.eventSettings.fields.description.default)
    ) {
      eventObj[this.fields.description] =
        this.parent.eventSettings.fields.description.default;
    }
    ...
    if (eventObj[this.fields.recurrenceRule] && this.recurrenceEditor) {
      this.recurrenceEditor.setRecurrenceRule(
        <string>eventObj[this.fields.recurrenceRule],
        <Date>eventObj[this.fields.startTime]
      );
      this.repeatRule = <string>eventObj[this.fields.recurrenceRule];
    }
    const deleteButton: Element = this.element.querySelector(
      '.' + cls.DELETE_EVENT_CLASS
    );
    if (deleteButton) {
    ...
    if (this.recurrenceEditor) {
      this.recurrenceEditor.setProperties({
        ...
        selectedType: !isNullOrUndefined(repeatType)
          ? repeatType
          : !isNullOrUndefined(eventObj[this.fields.recurrenceRule])
          ? this.recurrenceEditor.selectedType
          : 0,
      });
    ...
    if (
      ...
      const saveButton: Element = this.element.querySelector(
        '.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS
      );
  ...
  public convertToEventData(
    ...
    if (
      this.parent.resourceCollection.length > 0 ||
  ...
  private applyFormValidation(): void {
    const form: HTMLFormElement = this.element.querySelector(
      '.' + cls.FORM_CLASS
    ) as HTMLFormElement;
    if (!form) {
      return;
    ...
    const subjectRule: Record<string, any> = getValidationRule(
      this.parent.eventSettings.fields.subject.validation
    );
    if (!isNullOrUndefined(subjectRule)) {
      rules[this.parent.eventSettings.fields.subject.name] = subjectRule;
    }
    const locationRule: Record<string, any> = getValidationRule(
      this.parent.eventSettings.fields.location.validation
    );
    if (!isNullOrUndefined(locationRule)) {
      rules[this.parent.eventSettings.fields.location.name] = locationRule;
    }
    const startTimeRule: Record<string, any> = getValidationRule(
      this.parent.eventSettings.fields.startTime.validation
    );
    if (!isNullOrUndefined(startTimeRule)) {
      rules[this.parent.eventSettings.fields.startTime.name] = startTimeRule;
    }
    const endTimeRule: Record<string, any> = getValidationRule(
      this.parent.eventSettings.fields.endTime.validation
    );
    if (!isNullOrUndefined(endTimeRule)) {
      rules[this.parent.eventSettings.fields.endTime.name] = endTimeRule;
    }
    const descriptionRule: Record<string, any> = getValidationRule(
      this.parent.eventSettings.fields.description.validation
    );
    ...
    this.fieldValidator.renderFormValidator(
      form,
      rules,
      this.element,
      this.parent.locale
  ...
  private showDetails(eventData: Record<string, any>): void {
    ...
    const eventObj: Record<string, any> = <Record<string, any>>(
      extend({}, eventData, null, true)
    );
    const formElements: HTMLInputElement[] = this.getFormElements(
    ...
    if (
      (!this.cellClickAction ||
        (this.cellClickAction &&
          !isNullOrUndefined(this.parent.editorTemplate))) &&
      (<Date>eventObj[this.fields.endTime]).getHours() === 0 &&
      (<Date>eventObj[this.fields.endTime]).getMinutes() === 0
    ) {
    ...
    if (isNullOrUndefined(this.parent.editorTemplate)) {
      ...
      const timezoneObj: CheckBox = this.getInstance(
        cls.TIME_ZONE_CLASS + '.' + EVENT_FIELD
      ) as unknown as CheckBox;
      if (
        !(
          isNullOrUndefined(eventObj[this.fields.startTimezone]) &&
          isNullOrUndefined(eventObj[this.fields.endTimezone])
        )
      ) {
  ...
  private getColumnName(element: HTMLInputElement): string {
    ...
    if (attrName === '') {
      ...
      const classSelector: string = isDropDowns
        ? `.${fieldSelector}:not(.e-control)`
        : `.${fieldSelector}`;
      const control: Element =
        closest(element, classSelector) ||
        element.querySelector(`.${fieldSelector}`);
      if (control) {
  ...
  private onAllDayChange(allDayStatus: boolean): void {
    ...
    const timezoneDiv: HTMLElement = this.element.querySelector(
    ...
    if (allDayStatus) {
      format = isNullOrUndefined(this.parent.dateFormat)
        ...
        : this.parent.dateFormat;
      addClass(
        this.element.querySelectorAll('.e-time-icon'),
        cls.EVENT_WINDOW_ICON_DISABLE_CLASS
      );
      addClass([timezoneDiv], cls.DISABLE_CLASS);
      if (
        this.element.querySelector('.' + cls.EVENT_WINDOW_TIME_ZONE_DIV_CLASS)
      ) {
        removeClass(
          [
            this.element.querySelector(
      ...
      if (
        (
          this.element.querySelector(
            '.e-checkbox-wrapper .e-time-zone'
          ) as HTMLInputElement
        ).checked
      ) {
        addClass(
  ...
  private updateDateTime(
    ...
    let endDate: Date;
    if (allDayStatus) {
      startDate = util.resetTime(
        new Date(this.eventWindowTime.startTime.getTime())
      );
      if (this.parent.activeCellsData.isAllDay) {
        const temp: number = util
          .addDays(new Date((<Date>this.eventWindowTime.endTime).getTime()), -1)
          .getTime();
        endDate =
          +this.eventWindowTime.startTime > temp
            ? this.eventWindowTime.endTime
            : new Date(temp);
      } else {
        endDate = util.resetTime(
          new Date(this.eventWindowTime.endTime.getTime())
        );
      }
    } else {
      ...
      if (this.parent.activeCellsData.isAllDay) {
        const startHour: Date = this.parent.getStartEndTime(
          this.parent.workHours.start
        );
        startDate.setHours(
          startHour.getHours(),
          startHour.getMinutes(),
          startHour.getSeconds()
        );
        ...
        endDate.setMilliseconds(
          this.parent.activeCellsData.endTime.getTime() -
            this.parent.activeCellsData.startTime.getTime()
        );
      }
    }
  ...
  private getFormat(formatType: string): string {
    let format: string;
    if (
      isNullOrUndefined(this.parent.locale) ||
      this.parent.locale === 'en' ||
      this.parent.locale === 'en-US'
    ) {
      format = getValue(
        formatType + '.short',
        getDefaultDateObject(this.parent.getCalendarMode())
      );
    } else {
      format = getValue(
        `main.${
          this.parent.locale
        }.dates.calendars.${this.parent.getCalendarMode()}.${formatType}.short`,
        cldrData
      );
    }
    return format;
  ...
  private onEventDetailsUpdate(eventObj: Record<string, any>): void {
    ...
    if (
      ...
      removeClass(
        [this.element.querySelector('.' + cls.DELETE_EVENT_CLASS)],
        cls.DISABLE_CLASS
      );
    }
    if (isNullOrUndefined(this.parent.editorHeaderTemplate)) {
      this.element.querySelector(
        '.' + cls.EVENT_WINDOW_TITLE_TEXT_CLASS
      ).innerHTML = this.l10n.getConstant('editEvent');
    }
    this.element
      .querySelector('.' + cls.FORM_CLASS)
      .setAttribute('data-id', eventObj[this.fields.id].toString());
    if (isNullOrUndefined(this.parent.editorTemplate)) {
      ...
      if (
        eventObj[this.fields.startTimezone] ||
        eventObj[this.fields.endTimezone]
      ) {
        timezoneValue = true;
    ...
    const isDisable: boolean = (this.parent.readonly ||
      eventObj[this.fields.isReadonly]) as boolean;
    if (!this.parent.isAdaptive) {
      const saveButton: Element = this.element.querySelector(
        '.' + cls.EVENT_WINDOW_SAVE_BUTTON_CLASS
      );
  ...
  public updateMinMaxDateToEditor(): void {
    ...
    if (startDate && endDate) {
      const startObj: DatePicker = (startDate as EJ2Instance)
        .ej2_instances[0] as DatePicker;
      const endObj: DatePicker = (endDate as EJ2Instance)
        .ej2_instances[0] as DatePicker;
    ...
    if (this.recurrenceEditor) {
      const untilDate: Element =
        this.recurrenceEditor.element.querySelector('.e-until-date');
      if (untilDate) {
        const untilObj: DatePicker = (untilDate as EJ2Instance)
          .ej2_instances[0] as DatePicker;
  ...
  private updateRepeatLabel(repeatRule: string): void {
    ...
    const data: string = repeatRule
      ? this.l10n.getConstant('repeats') +
        ' ' +
        this.recurrenceEditor.getRuleSummary(repeatRule)
      : this.l10n.getConstant('repeat');
  ...
  private resetForm(): void {
    ...
    if (
      !this.parent.isAdaptive &&
      this.recurrenceEditor &&
      !this.recurrenceEditor.isDestroyed
    ) {
  ...
  private timezoneChangeStyle(value: boolean): void {
    ...
    const localTimezoneName: string =
      this.parent.tzModule.getLocalTimezoneName();
    if (value) {
      ...
      const timezone: Record<string, any>[] =
        startTimezoneObj.dataSource as Record<string, any>[];
      if (!startTimezoneObj.value || !this.parent.timezone) {
        const found: boolean = timezone.some((tz: Record<string, any>) => {
      ...
      startTimezoneObj.value =
        startTimezoneObj.value || this.parent.timezone || localTimezoneName;
      endTimezoneObj.value =
        endTimezoneObj.value || this.parent.timezone || localTimezoneName;
      startTimezoneObj.dataBind();
      endTimezoneObj.dataBind();
    } else {
  ...
  private resetFormFields(): void {
    const formElement: HTMLInputElement[] = this.getFormElements(
      cls.EVENT_WINDOW_DIALOG_CLASS
    );
    for (const currentElement of formElement) {
      const columnName: string =
        currentElement.name || this.getColumnName(currentElement);
      if (!isNullOrUndefined(columnName) && columnName !== '') {
  ...
  public eventSave(event: Event, alert?: string): void {
    ...
    const formElement: Element = this.element.querySelector(
      '.' + cls.FORM_CLASS
    );
    if (
      formElement &&
      formElement.classList.contains('e-formvalidator') &&
      !(
        (formElement as EJ2Instance).ej2_instances[0] as FormValidator
      ).validate()
    ) {
      return;
    }
    const dataCollection: Record<
      string,
      Record<string, any>
    > = this.getEventDataFromEditor();
  ...
  public getEventDataFromEditor(): Record<string, Record<string, any>> {
    const eventObj: Record<string, any> = extend(
      {},
      this.getObjectFromFormData(cls.EVENT_WINDOW_DIALOG_CLASS)
    ...
    eventObj[this.fields.recurrenceRule] = this.recurrenceEditor
      ? this.recurrenceEditor.getRecurrenceRule() || null
      : undefined;
    const tempObj: Record<string, any> = extend(
      {},
      eventObj,
      null,
      true
    ...
    if (eventObj[this.fields.isAllDay]) {
      eventObj[this.fields.startTime] = isNullOrUndefined(
        eventObj[this.fields.startTime]
      )
        ? null
        : util.resetTime(
            new Date((<Date>eventObj[this.fields.startTime]).getTime())
          );
      eventObj[this.fields.endTime] = isNullOrUndefined(
        eventObj[this.fields.endTime]
      )
        ? null
        : util.addDays(
            util.resetTime(
              new Date((<Date>eventObj[this.fields.endTime]).getTime())
            ),
            1
          );
    }
  ...
  private processEventValidation(
    ...
    if (
      this.recurrenceEditor &&
      this.recurrenceEditor.value &&
      this.recurrenceEditor.value !== ''
    ) {
      if (this.parent.currentAction !== 'EditOccurrence') {
        alertType = this.recurrenceValidation(
          <Date>eventObj[this.fields.startTime],
          <Date>eventObj[this.fields.endTime],
          alert
        );
      }
      let isShowAlert: boolean = true;
      if (
        alertType === 'seriesChangeAlert' &&
        this.parent.uiStateValues.isIgnoreOccurrence
      ) {
        isShowAlert = false;
      }
      if (
        !isNullOrUndefined(alertType) &&
        isShowAlert &&
        ((!this.parent.enableRecurrenceValidation &&
          alertType === 'wrongPattern') ||
          this.parent.enableRecurrenceValidation)
      ) {
  ...
  private processCrudActions(eventObj: Record<string, any>): boolean {
    ...
    const isResourceEventExpand: boolean =
      (this.parent.activeViewOptions.group.resources.length > 0 ||
        this.parent.resourceCollection.length > 0) &&
    ...
    if (!isNullOrUndefined(eventId)) {
      let eveId: number | string =
        this.parent.eventBase.getEventIDType() === 'string'
          ? eventId
          : parseInt(eventId, 10);
      let editedData: Record<string, any> = this.parent.eventsData.filter(
        (data: Record<string, any>) => data[this.fields.id] === eveId
      )[0];
      if (isNullOrUndefined(editedData)) {
        editedData = this.parent.blockData.filter(
          (data: Record<string, any>) => data[this.fields.id] === eveId
        )[0];
      }
      ...
      if (!isNullOrUndefined(editedData[this.fields.recurrenceRule])) {
        ...
        eventObj.Guid = (<Record<string, any>>(
          this.parent.activeEventData.event
        )).Guid;
        if (this.parent.currentAction === 'EditOccurrence') {
          ...
          if (
            this.parent.enableRecurrenceValidation &&
            this.editOccurrenceValidation(eveId, eventObj)
          ) {
            return true;
          }
        }
        if (
          ...
          eventObj[this.fields.id] !== editedData[this.fields.id]
        ) {
  ...
  private getResourceData(eventObj: Record<string, any>): string[] | number[] {
    ...
    if (
      !isNullOrUndefined(this.parent.resourceBase) &&
      !isNullOrUndefined(this.parent.resourceBase.resourceCollection) &&
      this.parent.resourceBase.resourceCollection.length > 0
    ) {
      const lastResourceData: ResourcesModel =
        this.parent.resourceBase.resourceCollection.slice(-1)[0];
  ...
  public getObjectFromFormData(className: string): Record<string, any> {
    ...
    for (const currentElement of formElement) {
      const columnName: string =
        currentElement.name || this.getColumnName(currentElement);
      if (!isNullOrUndefined(columnName) && columnName !== '') {
        eventObj[`${columnName}`] = this.getValueFromElement(
  ...
  private recurrenceValidation(
    ...
    const interval: number = (
      this.getInstance(
        'e-repeat-interval.e-numerictextbox'
      ) as unknown as NumericTextBox
    ).value;
    if (alert !== this.l10n.getConstant('ok')) {
      const activeEvent: Record<string, any> = <Record<string, any>>(
        this.parent.activeEventData.event
      );
      let excludedEvents: Record<string, any>[] = [];
      if (
        (this.parent.currentAction === 'EditSeries' ||
          this.parent.currentAction === 'EditFollowingEvents') &&
        !isNullOrUndefined(activeEvent)
      ) {
        const eventStartTime: string = activeEvent[
          this.parent.eventFields.startTime
        ] as string;
        const seriesEvents: Record<string, any>[] =
          this.parent.eventBase.getSeriesEvents(this.eventData, eventStartTime);
        if (seriesEvents.length > 0) {
          excludedEvents = this.parent.eventBase.getEditedOccurrences(
            seriesEvents,
            eventStartTime
          );
        } else {
        ...
        if (
          this.parent.currentAction === 'EditSeries' &&
          !isNullOrUndefined(
            this.eventData[this.parent.eventFields.recurrenceException]
          )
        ) {
      ...
      if (
        (
          this.getInstance(
            'e-end-on-left .e-ddl .e-dropdownlist'
          ) as unknown as DropDownList
        ).value === 'until' &&
        (
          this.getInstance(
            'e-end-on-date .e-datepicker'
      ...
      if (isNullOrUndefined(alertMessage)) {
        let types: string[] = recEditor.value
          .split(';')[1]
          .split('=')[1]
          .split(',');
        ...
        switch (recEditor.value.split(';')[0].split('=')[1]) {
          ...
          case 'WEEKLY':
            ...
            for (
              let index: number = 0;
              index < types.length * (interval + 1);
              index++
            ) {
              temp[parseInt(index.toString(), 10)] =
                types.length > index
                  ? <number>obj[types[parseInt(index.toString(), 10)]]
            ...
            for (let index: number = 1; index < tempValue.length; index++) {
              tempDiff.push(
                tempValue[parseInt(index.toString(), 10)] - tempValue[index - 1]
              );
            }
            if (
              (endDate.getTime() - startDate.getTime()) / (1000 * 3600) >=
                Math.min(...tempDiff) * 24 ||
          ...
          case 'MONTHLY':
            if (
              endDate.getTime() >=
              new Date(+startDate).setMonth(startDate.getMonth() + interval)
            ) {
              alertMessage = 'createError';
            }
            break;
          case 'YEARLY':
            if (
              ...
              new Date(+startDate).setFullYear(
                startDate.getFullYear() + interval
              )
            ) {
              alertMessage = 'createError';
            }
  ...
  private getRecurrenceIndex(
    recColl: Record<string, any>[],
    event: Record<string, any>
  ): number {
    let recIndex: number;
    for (let index: number = 0; index < recColl.length; index++) {
      if (
        event[this.fields.startTime].valueOf() ===
        recColl[parseInt(index.toString(), 10)][this.fields.startTime].valueOf()
      ) {
        recIndex = index;
        break;
      }
  ...
  private trimAllDay(data: Record<string, any>): void {
    if (data[this.fields.isAllDay]) {
      const temp: number = util
        ...
        .getTime();
      data[this.fields.endTime] =
        +data[this.fields.startTime] > temp
          ? data[this.fields.endTime]
          : new Date(temp);
    }
  }

  public editOccurrenceValidation(
    ...
    const recurColl: Record<string, any>[] = <Record<string, any>[]>(
      this.parent.getOccurrencesByID(eventId)
    );
    const excludedDatas: Record<string, any>[] = this.parent.eventsData.filter(
      (data: Record<string, any>) => data[this.fields.recurrenceID] === eventId
    ) as Record<string, any>[];
    excludedDatas.map((data: Record<string, any>) =>
      recurColl.push(extend({}, data) as Record<string, any>)
    );
    ...
    const currentStartTime: Date = new Date(
      +currentData[this.fields.startTime]
    );
    ...
    const lastEndTime: Date = new Date(
      +recurColl[recurColl.length - 1][this.fields.endTime]
    );
    if (index === 0) {
      if (!isNullOrUndefined(recurColl[index + 1])) {
        if (
          (!(nextStartTime.getTime() >= currentEndTime.getTime()) &&
      ...
      const previousStartTime: Date = new Date(
        +recurColl[index - 1][this.fields.startTime]
      );
      const previousEndTime: Date = new Date(
        +recurColl[index - 1][this.fields.endTime]
      );
      if (index === recurColl.length - 1) {
        if (
          util
            .resetTime(
              new Date(
                +recurColl[recurColl.length - 1 - index][this.fields.startTime]
              )
            )
            .getTime() > util.resetTime(currentStartTime).getTime()
        ) {
          this.parent.quickPopup.openRecurrenceValidationAlert(
            'occurrenceAlert'
          );
          ...
          !(
            previousEndTime.getTime() <= currentStartTime.getTime() &&
            util.resetTime(currentStartTime).getTime() >
              util.resetTime(previousStartTime).getTime()
          )
        ) {
          this.parent.quickPopup.openRecurrenceValidationAlert('sameDayAlert');
          return true;
        ...
        !(
          (util.resetTime(previousStartTime).getTime() <
            util.resetTime(currentStartTime).getTime() ||
            util
              .resetTime(new Date(+recurColl[0][this.fields.startTime]))
              .getTime() > util.resetTime(currentStartTime).getTime()) &&
          (util.resetTime(nextStartTime).getTime() >
            util.resetTime(currentStartTime).getTime() ||
        ...
        util.resetTime(nextEndTime).getTime() <
          util.resetTime(currentStartTime).getTime() ||
        util.resetTime(previousStartTime).getTime() >
          util.resetTime(currentEndTime).getTime() ||
        !(
          util.resetTime(currentStartTime).getTime() <
          util.resetTime(nextStartTime).getTime()
        )
  ...
  private resourceSaveEvent(
    ...
    const lastResourceData: ResourcesModel =
      this.parent.resourceBase.resourceCollection.slice(-1)[0];
    let resourceData: string[] | number[] = eventObj[lastResourceData.field] as
      | string[]
      | number[];
    resourceData =
      resourceData instanceof Array
    ...
    for (let i: number = 0; i < resourceData.length; i++) {
      const events: Record<string, any> = <Record<string, any>>(
        extend({}, eventObj, null, true)
      );
        ...
        if (action === 'Save' && i === resourceData.length - 1) {
          if (temp.length > 0) {
            ...
            for (let k: number = 1; k < temp.length; k++) {
              temp[parseInt(k.toString(), 10)][this.fields.id] =
                this.parent.eventBase.getEventMaxID(i);
          ...
          if (temp.length > 0) {
            for (let j: number = 0; j < temp.length; j++) {
              temp[parseInt(j.toString(), 10)][this.fields.id] =
                this.parent.eventBase.getEventMaxID(j);
      ...
      if (
        this.parent.activeViewOptions.group.byGroupID &&
        !isNullOrUndefined(lastLevel)
      ) {
        const lastResource: Record<string, any>[] =
          lastResourceData.dataSource as Record<string, any>[];
        const resCol: Record<string, any>[] = this.parent
          .resourceCollection as Record<string, any>[];
        let index: number;
        if (resCol.length > 1) {
          index = util.findIndexInData(
            lastResource,
            lastResourceData.idField,
            resourceData[parseInt(i.toString(), 10)] as string,
            events,
            resCol
          );
        } else {
        ...
        const groupId: Record<string, any> = lastResource[
          parseInt(index.toString(), 10)
        ][lastResourceData.groupIDField] as Record<string, any>;
        const filter: TdData = lastLevel
          .filter(
            (obj: TdData) =>
              obj.resourceData[lastResourceData.idField] ===
              resourceData[parseInt(i.toString(), 10)]
          )
          .filter(
            (obj: TdData) =>
              obj.resourceData[lastResourceData.groupIDField] === groupId
          )[0];
        const groupOrder: number[] | string[] = filter.groupOrder;
        for (
          let index: number = 0;
          index < this.parent.resourceBase.resourceCollection.length;
          index++
        ) {
          const field: string =
            this.parent.resourceBase.resourceCollection[
          ...
          events[`${field}`] =
            (groupOrder[parseInt(index.toString(), 10)] as any) instanceof Array
              ? groupOrder[parseInt(index.toString(), 10)][0]
        ...
        for (
          let index: number = 0;
          index < this.parent.resourceBase.resourceCollection.length - 1;
          index++
        ) {
          const field: string =
            this.parent.resourceBase.resourceCollection[
          ...
          if (
            events[`${field}`] instanceof Array &&
            (events[`${field}`] as Record<string, any>[]).length > 1
          ) {
            for (
              let k: number = 0;
              k < (events[`${field}`] as Record<string, any>[]).length;
              k++
            ) {
              const event: Record<string, any> = <Record<string, any>>(
                extend({}, events, null, true)
              );
              event[`${field}`] = (eventObj[`${field}`] as Record<string, any>)[
                parseInt(k.toString(), 10)
              ];
              event[lastResourceData.field] =
                resourceData[parseInt(i.toString(), 10)];
              temp.push(event);
            }
          } else {
            if (temp.length === 0) {
              events[`${field}`] =
                eventObj[`${field}`] instanceof Array
                  ? (eventObj[`${field}`] as Record<string, any>)[0]
                  : eventObj[`${field}`];
              events[lastResourceData.field] =
                resourceData[parseInt(i.toString(), 10)];
            } else {
              for (let l: number = 0; l < temp.length; l++) {
                temp[parseInt(l.toString(), 10)][`${field}`] =
                  eventObj[`${field}`] instanceof Array
                    ? (eventObj[`${field}`] as Record<string, any>)[0]
                    : eventObj[`${field}`];
              }
  ...
  private getFormElements(className: string): HTMLInputElement[] {
    ...
    if (className === cls.EVENT_WINDOW_DIALOG_CLASS) {
      elements = [].slice.call(
        this.element.querySelectorAll('.' + EVENT_FIELD)
      );
    } else {
      elements = [].slice.call(
        this.parent.element.querySelectorAll(
          '.' + className + ' .' + EVENT_FIELD
        )
      );
    }
    return elements;
  ...
  private getValueFromElement(
    element: HTMLElement
  ): number | string | Date | boolean | string[] | number[] {
    let value: number | string | Date | boolean | string[] | number[];
    if (element.classList.contains('e-datepicker')) {
      value = ((element as EJ2Instance).ej2_instances[0] as DatePicker)
        .value as Date;
    } else if (element.classList.contains('e-datetimepicker')) {
      value = ((element as EJ2Instance).ej2_instances[0] as DateTimePicker)
        .value as Date;
    } else if (element.classList.contains('e-dropdownlist')) {
      value = ((element as EJ2Instance).ej2_instances[0] as DropDownList)
        .value as string | number;
    } else if (element.classList.contains('e-multiselect')) {
      value = ((element as EJ2Instance).ej2_instances[0] as MultiSelect)
        .value as string[] | number[];
    } else if (element.classList.contains('e-checkbox')) {
      value = ((element as EJ2Instance).ej2_instances[0] as CheckBox)
        .checked as boolean;
    } else {
      if ((element as HTMLInputElement).type === 'checkbox') {
        ...
        value = this.parent.enableHtmlSanitizer
          ? SanitizeHtmlHelper.sanitize(
              (element as HTMLInputElement).value as string
            )
  ...
  private setValueToElement(
    element: HTMLElement,
    value: number | string | Date | boolean | number[] | string[]
  ): void {
    if (element.classList.contains('e-datepicker')) {
      const instance: DatePicker = (element as EJ2Instance)
        .ej2_instances[0] as DatePicker;
      instance.value = <Date>value;
      instance.dataBind();
    } else if (element.classList.contains('e-datetimepicker')) {
      const instance: DateTimePicker = (element as EJ2Instance)
        .ej2_instances[0] as DateTimePicker;
      ...
      const instance: DropDownList = (element as EJ2Instance)
        .ej2_instances[0] as DropDownList;
      instance.value = <string>value;
      instance.dataBind();
    } else if (element.classList.contains('e-multiselect')) {
      const instance: MultiSelect = (element as EJ2Instance)
        .ej2_instances[0] as MultiSelect;
      instance.value = [];
      instance.value = <string[]>(value instanceof Array ? value : [value]);
      instance.dataBind();
    } else if (element.classList.contains('e-checkbox')) {
      const instance: CheckBox = (element as EJ2Instance)
        .ej2_instances[0] as CheckBox;
  ...
  private getInstance(className: string): Record<string, any> {
    const element: HTMLElement = this.element.querySelector(
      '.' + className
    ) as HTMLElement;
  ...
  private eventDelete(event: Event): void {
    ...
    switch (this.parent.currentAction) {
      case 'EditOccurrence':
        if (
          !isNullOrUndefined(
            (<Record<string, any>>this.parent.activeEventData.event)[
              this.parent.eventFields.recurrenceRule
            ]
          )
        ) {
    ...
    if (this.dialogObject) {
      if (this.dialogObject.element) {
        const form: HTMLFormElement =
          this.dialogObject.element.querySelector('form');
