...
import { Input, FormValidator } from '@hoyui/inputs';
...
export class QuickPopups {
  ...
  private renderQuickPopup(): void {
    const quickPopupWrapper: HTMLElement = createElement('div', {
    ...
    this.quickPopup = new Popup(quickPopupWrapper, {
      ...
      hideAnimation: this.parent.isAdaptive
        ? { name: 'ZoomOut' }
        : { name: 'FadeOut', duration: 150 },
      showAnimation: this.parent.isAdaptive
        ? { name: 'ZoomIn' }
        : { name: 'FadeIn', duration: 150 },
      collision: this.parent.isAdaptive
        ...
        : { X: 'none', Y: 'fit' },
      position:
        this.parent.isAdaptive || this.parent.enableRtl
          ? { X: 'left', Y: 'top' }
          : { X: 'right', Y: 'top' },
      viewPortElement: this.parent.isAdaptive
        ? document.body
        : this.parent.element,
  ...
  private renderMorePopup(): void {
    const moreEventPopup: string =
      `<div class="${cls.MORE_EVENT_POPUP_CLASS}"><div class="${cls.MORE_EVENT_HEADER_CLASS}">` +
      `<div class="${
        cls.MORE_EVENT_CLOSE_CLASS
      }" title="${this.l10n.getConstant(
        'close'
    ...
    this.morePopup.element.setAttribute(
      'aria-labelledby',
      this.parent.element.id + '_more_popup'
    );
    const closeButton: HTMLButtonElement = this.morePopup.element.querySelector(
      '.' + cls.MORE_EVENT_CLOSE_CLASS
    ) as HTMLButtonElement;
    this.renderButton(
      'e-round',
      cls.ICON + ' ' + cls.CLOSE_ICON_CLASS,
      false,
      closeButton,
      this.closeClick
    );
  ...
  private renderQuickDialog(): void {
    const buttonModel: ButtonPropsModel[] = [
      {
        buttonModel: { cssClass: 'e-quick-alertok e-flat', isPrimary: true },
    ...
    const okButton: Element = this.quickDialog.element.querySelector(
      '.' + cls.QUICK_DIALOG_ALERT_OK
    );
    if (okButton) {
      okButton.setAttribute('aria-label', this.l10n.getConstant('occurrence'));
      okButton.setAttribute('aria-label', okButton.innerHTML);
    }
    const cancelButton: Element = this.quickDialog.element.querySelector(
      '.' + cls.QUICK_DIALOG_ALERT_CANCEL
    );
    if (cancelButton) {
  ...
  private quickDialogClass(action: string): void {
    const classList: string[] = [
      cls.QUICK_DIALOG_OCCURRENCE_CLASS,
      cls.QUICK_DIALOG_SERIES_CLASS,
      cls.QUICK_DIALOG_DELETE_CLASS,
      cls.QUICK_DIALOG_CANCEL_CLASS,
      cls.QUICK_DIALOG_ALERT_BTN_CLASS,
      cls.DISABLE_CLASS,
    ];
    const okButton: Element = this.quickDialog.element.querySelector(
      '.' + cls.QUICK_DIALOG_ALERT_OK
    ...
    const followingEventButton: Element =
      this.quickDialog.element.querySelector(
        '.' + cls.QUICK_DIALOG_ALERT_FOLLOWING
      );
    ...
    switch (action) {
      case 'Recurrence':
        ...
        if (this.parent.eventSettings.editFollowingEvents) {
          ...
          addClass(
            [followingEventButton],
            cls.QUICK_DIALOG_FOLLOWING_EVENTS_CLASS
          );
        }
        break;
      case 'Delete':
      ...
      case 'Alert':
        addClass(
          [okButton],
          [cls.QUICK_DIALOG_ALERT_OK, cls.QUICK_DIALOG_ALERT_BTN_CLASS]
        );
        addClass(
          [cancelButton],
          [cls.QUICK_DIALOG_ALERT_CANCEL, cls.DISABLE_CLASS]
        );
        break;
    }
  }
  ...
  private applyFormValidation(): void {
    const form: HTMLFormElement = this.quickPopup.element.querySelector(
      '.' + cls.FORM_CLASS
    ) as HTMLFormElement;
    const rules: Record<string, any> = {};
    rules[this.parent.eventSettings.fields.subject.name] =
      this.parent.eventSettings.fields.subject.validation;
    this.fieldValidator.renderFormValidator(
      form,
      rules,
      this.quickPopup.element,
      this.parent.locale
    );
  }

  public openRecurrenceAlert(): void {
    const editDeleteOnly: Element = this.quickDialog.element.querySelector(
      '.' + cls.QUICK_DIALOG_ALERT_OK
    );
    if (editDeleteOnly) {
      editDeleteOnly.innerHTML = this.l10n.getConstant(
        this.parent.currentAction === 'Delete' ? 'deleteEvent' : 'editEvent'
      );
      editDeleteOnly.setAttribute('aria-label', editDeleteOnly.innerHTML);
    }
    const editFollowingEventsOnly: Element =
      this.quickDialog.element.querySelector(
        '.' + cls.QUICK_DIALOG_ALERT_FOLLOWING
      );
    if (editFollowingEventsOnly) {
      editFollowingEventsOnly.innerHTML =
        this.l10n.getConstant('editFollowingEvent');
      editFollowingEventsOnly.setAttribute(
        'aria-label',
        editFollowingEventsOnly.innerHTML
      );
    }
    const editDeleteSeries: Element = this.quickDialog.element.querySelector(
      '.' + cls.QUICK_DIALOG_ALERT_CANCEL
    );
    if (editDeleteSeries) {
      editDeleteSeries.innerHTML = this.l10n.getConstant(
        this.parent.currentAction === 'Delete' ? 'deleteSeries' : 'editSeries'
      );
    ...
    this.quickDialog.header = this.l10n.getConstant(
      this.parent.currentAction === 'Delete' ? 'deleteTitle' : 'editTitle'
    );
  ...
  public openRecurrenceValidationAlert(type: string): void {
    ...
    switch (type) {
      ...
      case 'seriesChangeAlert':
        dialogCancel = this.quickDialog.element.querySelector(
          '.' + cls.QUICK_DIALOG_CANCEL_CLASS
        );
    ...
    if (
      (!this.parent.enableRecurrenceValidation && type === 'wrongPattern') ||
  ...
  public openDeleteAlert(): void {
    ...
    this.quickDialog.content =
      (<Record<string, any>[]>this.parent.activeEventData.event).length > 1
        ? this.l10n.getConstant('deleteMultipleContent')
        : this.l10n.getConstant('deleteContent');
    this.quickDialog.header =
      (<Record<string, any>[]>this.parent.activeEventData.event).length > 1
        ? this.l10n.getConstant('deleteMultipleEvent')
        : this.l10n.getConstant('deleteEvent');
    this.quickDialogClass('Delete');
    this.showQuickDialog('DeleteAlert');
  }

  public openValidationError(
    type: string,
  ...
  private showQuickDialog(
    ...
    const eventProp: PopupOpenEventArgs = {
      ...
      data: extend(
        {},
        eventData || this.parent.activeEventData.event,
        null,
        true
  ...
  private createMoreEventList(
    eventCollection: Record<string, any>[],
    groupOrder: string[],
    groupIndex: string
  ): HTMLElement {
    const fields: EventFieldsMapping = this.parent.eventFields;
    const moreEventContentEle: HTMLElement = createElement('div', {
      className: cls.MORE_EVENT_CONTENT_CLASS,
    });
    let moreEventWrapperEle: HTMLElement = createElement('div', {
    ...
    if (eventCollection.length === 0) {
      ...
      for (const eventData of eventCollection) {
        const eventText: string = (eventData[fields.subject] ||
        ...
        const appointmentElement: HTMLElement = createElement('div', {
          ...
          attrs: {
            ...
            'data-guid': eventData.Guid as string,
            role: 'button',
            tabindex: '0',
            'aria-disabled':
              this.parent.eventBase.getReadonlyAttribute(eventData),
        ...
        if (!isNullOrUndefined(this.parent.activeViewOptions.eventTemplate)) {
          const tempId: string =
            ...
            '_' +
            this.parent.activeViewOptions.eventTemplateName +
            'eventTemplate';
          ...
          appointmentElement.appendChild(
            createElement('div', { className: cls.SUBJECT_CLASS })
          );
          this.parent.sanitize(
            eventText,
            appointmentElement.firstElementChild as HTMLElement
          );
        }
        ...
        if (!isNullOrUndefined(eventData[fields.recurrenceRule])) {
          const iconClass: string =
            eventData[fields.id] === eventData[fields.recurrenceID]
              ? cls.EVENT_RECURRENCE_ICON_CLASS
              : cls.EVENT_RECURRENCE_EDIT_ICON_CLASS;
          appointmentElement.appendChild(
            createElement('div', { className: cls.ICON + ' ' + iconClass })
          );
        }
        ...
        this.parent.trigger(
          event.eventRendered,
          args,
          (eventArgs: EventRenderedArgs) => {
            if (!eventArgs.cancel) {
              moreEventWrapperEle.appendChild(appointmentElement);
              const isPreventCrud: boolean =
                this.parent.isAdaptive || this.parent.currentView === 'Year';
              this.parent.eventBase.wireAppointmentEvents(
                appointmentElement,
                eventData,
                isPreventCrud
              );
              this.parent.eventBase.applyResourceColor(
                appointmentElement,
                eventData,
                'backgroundColor',
                groupOrder
  ...
  public tapHoldEventPopup(e: Event): void {
    ...
    const eventObj: Record<string, any> =
      this.parent.eventBase.getEventByGuid(guid);
    if (isNullOrUndefined(eventObj)) {
      return;
    }
    const eventTitle: string = (eventObj[this.parent.eventFields.subject] ||
      this.l10n.getConstant('noTitle')) as string;
    const eventTemplate: string =
      `<div class="${cls.MULTIPLE_EVENT_POPUP_CLASS}"><div class="${cls.POPUP_HEADER_CLASS}">` +
      `<button class="${cls.CLOSE_CLASS}" title="${this.l10n.getConstant(
        'close'
      )}"></button>` +
      `<div class="${cls.SUBJECT_CLASS}">${eventTitle}</div>` +
      `<button class="${cls.EDIT_CLASS}" title="${this.l10n.getConstant(
        'edit'
      )}"></button>` +
      `<button class="${cls.DELETE_CLASS}" title="${this.l10n.getConstant(
        'delete'
      )}"></button></div></div>`;
    this.quickPopup.element.innerHTML = eventTemplate;
    const closeIcon: HTMLButtonElement = this.quickPopup.element.querySelector(
      '.' + cls.CLOSE_CLASS
    ) as HTMLButtonElement;
    this.renderButton(
      'e-flat e-round e-small',
      cls.ICON + ' ' + cls.CLOSE_ICON_CLASS,
      false,
      closeIcon,
      this.closeClick
    ...
    const readonly: boolean =
      this.parent.activeViewOptions.readonly ||
      (eventObj[this.parent.eventFields.isReadonly] as boolean);
    const editAction: boolean =
      !this.parent.eventSettings.allowEditing || readonly;
    const deleteAction: boolean =
      !this.parent.eventSettings.allowDeleting || readonly;
    const editIcon: HTMLButtonElement = this.quickPopup.element.querySelector(
      '.' + cls.EDIT_CLASS
    ) as HTMLButtonElement;
    if (editIcon) {
      this.renderButton(
        'e-flat e-round e-small',
        cls.ICON + ' ' + cls.EDIT_ICON_CLASS,
        editAction,
        editIcon,
        this.editClick
      );
    }
    const deleteIcon: HTMLButtonElement = this.quickPopup.element.querySelector(
      '.' + cls.DELETE_CLASS
    ) as HTMLButtonElement;
    if (deleteIcon) {
      this.renderButton(
        'e-flat e-round e-small',
        cls.ICON + ' ' + cls.DELETE_ICON_CLASS,
        deleteAction,
        deleteIcon,
        this.deleteClick
      );
    }
    this.beforeQuickPopupOpen(
      target,
      this.parent.eventBase.getPageCoordinates(e as MouseEvent & TouchEvent)
    );
  }

  private isCellBlocked(args: CellClickEventArgs): boolean {
    const tempObj: Record<string, any> = {};
    tempObj[this.parent.eventFields.startTime] =
      this.parent.activeCellsData.startTime;
    tempObj[this.parent.eventFields.endTime] =
      this.parent.activeCellsData.endTime;
    tempObj[this.parent.eventFields.isAllDay] =
      this.parent.activeCellsData.isAllDay;
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      const targetCell: HTMLElement =
        args.element instanceof Array ? args.element[0] : args.element;
      const groupIndex: number = parseInt(
        targetCell.getAttribute('data-group-index'),
        10
      );
      this.parent.resourceBase.setResourceValues(
        tempObj,
        isNaN(groupIndex) ? null : groupIndex
      );
    }
  ...
  private cellClick(args: CellClickEventArgs): void {
    ...
    if (
      !this.parent.showQuickInfo ||
      !this.parent.eventSettings.allowAdding ||
      this.parent.currentView === 'MonthAgenda' ||
      this.isCellBlocked(args) ||
      !this.parent.isMinMaxDate(new Date(date.setHours(0, 0, 0, 0)))
    ) {
      this.quickPopupHide();
      return;
    }
    const targetEle: Element = !isNullOrUndefined(args.event)
      ? (args.event.target as Element)
      : (args.element as Element);
    if (this.parent.isAdaptive) {
      this.quickPopupHide();
      let newEventClone: HTMLElement = this.parent.element.querySelector(
        '.' + cls.NEW_EVENT_CLASS
      ) as HTMLElement;
      if (isNullOrUndefined(newEventClone)) {
        newEventClone = createElement('div', {
          className: cls.NEW_EVENT_CLASS,
          innerHTML: `<div class="e-title">+ ${this.l10n.getConstant(
            'newEvent'
          )}</div>`,
        });
      }
      const targetCell: Element = closest(
        targetEle,
        '.' + cls.WORK_CELLS_CLASS + ',.' + cls.ALLDAY_CELLS_CLASS
      );
      if (targetCell) {
    ...
    const target: Element = closest(
      ...
      '.' +
        ...
        ',.' +
        cls.ALLDAY_CELLS_CLASS +
        ',.' +
        cls.HEADER_CELLS_CLASS
    );
    if (
      isNullOrUndefined(target) ||
      targetEle.classList.contains(cls.MORE_INDICATOR_CLASS)
    ) {
      return;
    }
    ...
    if (
      isSameTarget &&
      this.quickPopup.element.classList.contains(cls.POPUP_OPEN)
    ) {
      const subjectElement: HTMLInputElement =
    ...
    temp[this.parent.eventFields.startTime] =
      this.parent.activeCellsData.startTime;
    temp[this.parent.eventFields.endTime] = this.parent.activeCellsData.endTime;
    temp[this.parent.eventFields.isAllDay] =
      this.parent.activeCellsData.isAllDay;
    const quickCellPopup: HTMLElement = createElement('div', {
    ...
    this.quickPopup.element.setAttribute(
      'aria-label',
      this.l10n.getConstant('newEvent')
    );
    const subjectElement: HTMLInputElement = quickCellPopup.querySelector(
      '.' + cls.SUBJECT_CLASS
    ) as HTMLInputElement;
    if (subjectElement) {
      Input.createInput({
    ...
    const closeIcon: HTMLButtonElement = quickCellPopup.querySelector(
      '.' + cls.CLOSE_CLASS
    ) as HTMLButtonElement;
    if (closeIcon) {
      this.renderButton(
    ...
    const moreButton: HTMLButtonElement = quickCellPopup.querySelector(
      '.' + cls.QUICK_POPUP_EVENT_DETAILS_CLASS
    ) as HTMLButtonElement;
    if (moreButton) {
      this.renderButton('e-flat', '', false, moreButton, this.detailsClick);
    }
    const saveButton: HTMLButtonElement = quickCellPopup.querySelector(
      '.' + cls.EVENT_CREATE_CLASS
    ) as HTMLButtonElement;
    if (saveButton) {
      this.renderButton(
        'e-flat e-primary',
    ...
    this.beforeQuickPopupOpen(
      target,
      this.parent.eventBase.getPageCoordinates(
        args.event as MouseEvent & TouchEvent
      )
    );
  }
  ...
  private isSameEventClick(events: EventClickArgs): boolean {
    const isSameTarget: boolean =
      this.quickPopup.relateTo ===
      closest(<HTMLElement>events.element, '.' + cls.APPOINTMENT_CLASS);
    if (
      isSameTarget &&
      this.quickPopup.element.classList.contains(cls.POPUP_OPEN)
    ) {
  ...
  private isQuickTemplate(type: TemplateType): boolean {
    return (
      this.parent.quickInfoTemplates.templateType === 'Both' ||
      this.parent.quickInfoTemplates.templateType === type
    );
  }
  ...
  private eventClick(events: EventClickArgs): void {
    ...
    if (this.parent.isAdaptive && this.isMultipleEventSelect) {
      ...
      const quickEventPopup: HTMLElement = createElement('div', {
        className: cls.EVENT_POPUP_CLASS,
      });
      quickEventPopup.appendChild(this.getPopupHeader('Event', eventData));
      quickEventPopup.appendChild(
        this.getPopupContent('Event', events, eventData)
      );
      ...
      const readonly: boolean =
        this.parent.activeViewOptions.readonly ||
        (eventData[this.parent.eventFields.isReadonly] as boolean);
      const editAction: boolean =
        !this.parent.eventSettings.allowEditing || readonly;
      const deleteAction: boolean =
        !this.parent.eventSettings.allowDeleting || readonly;
      const editIcon: HTMLButtonElement = quickEventPopup.querySelector(
        '.' + cls.EDIT_CLASS
      ) as HTMLButtonElement;
      ...
      const deleteIcon: HTMLButtonElement = quickEventPopup.querySelector(
        '.' + cls.DELETE_CLASS
      ) as HTMLButtonElement;
      if (deleteIcon) {
        this.renderButton(
      ...
      const closeIcon: HTMLButtonElement = quickEventPopup.querySelector(
        '.' + cls.CLOSE_CLASS
      ) as HTMLButtonElement;
      if (closeIcon) {
        this.renderButton(
      ...
      const editButton: HTMLButtonElement = quickEventPopup.querySelector(
        '.' + cls.EDIT_EVENT_CLASS
      ) as HTMLButtonElement;
      if (editButton) {
        this.renderButton(
      ...
      const deleteButton: HTMLButtonElement = quickEventPopup.querySelector(
      ...
      if (
        this.morePopup &&
        !closest(<Element>events.element, '.' + cls.MORE_EVENT_WRAPPER_CLASS)
      ) {
        this.morePopup.hide();
      }
      this.quickPopup.content = quickEventPopup;
      this.quickPopup.relateTo = this.parent.isAdaptive
        ? document.body
        : (closest(
            <HTMLElement>events.element,
            '.' + cls.APPOINTMENT_CLASS
          ) as HTMLElement);
      this.quickPopup.dataBind();
      this.beforeQuickPopupOpen(
        events.element as Element,
        this.parent.eventBase.getPageCoordinates((events as any).originalEvent)
      );
    }
  }

  private getPopupHeader(
    headerType: TemplateType,
    headerData: Record<string, any>
  ): HTMLElement {
    const headerTemplate: HTMLElement = createElement('div', {
      className: cls.POPUP_HEADER_CLASS,
    });
    if (
      this.isQuickTemplate(headerType) &&
      this.parent.quickInfoTemplates.header
    ) {
      const headerArgs: Record<string, any> = extend(
        {},
        headerData,
      ...
      switch (headerType) {
        case 'Cell':
          header =
            `<div class="${cls.POPUP_HEADER_ICON_WRAPPER}"><button class="${cls.CLOSE_CLASS}" title=` +
            `"${this.l10n.getConstant('close')}"></button></div>`;
          break;
        case 'Event':
          args = this.getFormattedString(headerData);
          header =
            `<div class="${cls.POPUP_HEADER_ICON_WRAPPER}">` +
            `<button class="${
              cls.EDIT_CLASS + ' ' + cls.ICON
            }" title="${this.l10n.getConstant('edit')}"></button>` +
            `<button class="${
              cls.DELETE_CLASS + ' ' + cls.ICON
            }" title="${this.l10n.getConstant('delete')}"></button>` +
            `<button class="${cls.CLOSE_CLASS}" title="${this.l10n.getConstant(
              'close'
            )}"></button></div>` +
            ...
            `title="${
              args.eventSubject
                ? args.eventSubject.replaceAll('"', "'")
                : args.eventSubject
            }"></div></div >`;
          break;
      }
      const templateWrapper: HTMLElement = createElement('div', {
        innerHTML: header,
      });
      if (headerType === 'Event') {
        const subjectText: HTMLElement = templateWrapper.querySelector(
          '.' + cls.SUBJECT_CLASS
        );
  ...
  private getPopupContent(
    type: TemplateType,
    args: CellClickEventArgs | EventClickArgs,
    data: Record<string, any>
  ): HTMLElement {
    const contentTemplate: HTMLElement = createElement('div', {
      className: cls.POPUP_CONTENT_CLASS,
    });
    if (this.isQuickTemplate(type) && this.parent.quickInfoTemplates.content) {
      const contentArgs: Record<string, any> = extend(
        {},
        data,
      ...
      const contentTemp: HTMLElement[] = [].slice.call(
        this.parent.getQuickInfoTemplatesContent()(
          contentArgs,
          this.parent,
          'content',
          templateId + '_contentTemplate',
          false
        )
      );
      ...
      const resourceText: string = this.getResourceText(
        args,
        type.toLowerCase()
      );
      switch (type) {
        case 'Cell':
          ...
          content =
            `<table class="${cls.POPUP_TABLE_CLASS}"><tbody><tr><td><form class="${cls.FORM_CLASS}" onsubmit=` +
        ...
        case 'Event':
          ...
          content =
            ...
            cls.DATE_TIME_CLASS +
            '"><div class="' +
            cls.DATE_TIME_ICON_CLASS +
            ' ' +
            cls.ICON +
            '"></div><div class="' +
            cls.DATE_TIME_WRAPPER_CLASS +
            ' ' +
            cls.TEXT_ELLIPSIS +
            '"><div class="' +
          ...
          if (data[this.parent.eventFields.location]) {
            content +=
              ...
              cls.LOCATION_CLASS +
              '"><div class="' +
              cls.LOCATION_ICON_CLASS +
              ' ' +
              cls.ICON +
              '"></div><div class="' +
              cls.LOCATION_DETAILS_CLASS +
              ' ' +
              cls.TEXT_ELLIPSIS +
              '"></div></div>';
          }
          if (
            data[this.parent.eventFields.startTimezone] ||
            data[this.parent.eventFields.endTimezone]
          ) {
            content +=
              '<div class="' +
              cls.TIME_ZONE_CLASS +
              '"><div class="' +
              cls.TIME_ZONE_ICON_CLASS +
              ' ' +
              cls.ICON +
              '"></div><div class="' +
              cls.TIME_ZONE_DETAILS_CLASS +
              ' ' +
              cls.TEXT_ELLIPSIS +
              '">' +
          ...
          if (data[this.parent.eventFields.description]) {
            content +=
              ...
              '"><div class="' +
              cls.DESCRIPTION_ICON_CLASS +
              ' ' +
              cls.ICON +
              '"></div><div class="' +
              cls.DESCRIPTION_DETAILS_CLASS +
              ' ' +
              cls.TEXT_ELLIPSIS +
              '"></div></div>';
          }
          if (this.parent.resourceCollection.length > 0) {
            content +=
              ...
              cls.RESOURCE_CLASS +
              '"><div class="' +
              cls.RESOURCE_ICON_CLASS +
              ' ' +
              cls.ICON +
              '"></div><div class="' +
              cls.RESOURCE_DETAILS_CLASS +
              ' ' +
              cls.TEXT_ELLIPSIS +
              '"></div></div>';
          }
      ...
      if (data[this.parent.eventFields.location]) {
        const locationDetails: HTMLElement = templateWrapper.querySelector(
          '.' + cls.LOCATION_DETAILS_CLASS
        );
      ...
      if (data[this.parent.eventFields.description]) {
        const descriptionDetails: HTMLElement = templateWrapper.querySelector(
          '.' + cls.DESCRIPTION_DETAILS_CLASS
        );
      ...
      if (resourceText) {
        const resourceDetails: HTMLElement = templateWrapper.querySelector(
          '.' + cls.RESOURCE_DETAILS_CLASS
        );
  ...
  private getPopupFooter(
    ...
    const footerTemplate: HTMLElement = createElement('div', {
      className: cls.POPUP_FOOTER_CLASS,
    });
    if (
      this.isQuickTemplate(footerType) &&
      this.parent.quickInfoTemplates.footer
    ) {
      const footerArgs: Record<string, any> = extend(
        {},
        footerData,
      ...
      const footerTemp: HTMLElement[] = [].slice.call(
        this.parent.getQuickInfoTemplatesFooter()(
          footerArgs,
          this.parent,
          'footer',
          templateId + '_footerTemplate',
          false
        )
      );
      ...
      switch (footerType) {
        case 'Cell':
          footer =
            `<button class="${
              cls.QUICK_POPUP_EVENT_DETAILS_CLASS + ' ' + cls.TEXT_ELLIPSIS
            }" title=` +
            `"${this.l10n.getConstant('moreDetails')}">${this.l10n.getConstant(
              'moreDetails'
            )}</button>` +
            `<button class="${cls.EVENT_CREATE_CLASS} ${
              cls.TEXT_ELLIPSIS
            }" title="${this.l10n.getConstant('save')}">` +
        ...
        case 'Event':
          footer = `${
            this.parent.isAdaptive
              ? ''
              : `<button class="${cls.EDIT_EVENT_CLASS} ` +
                `${cls.TEXT_ELLIPSIS}" title="${this.l10n.getConstant(
                  'edit'
                )}">${this.l10n.getConstant('edit')}</button>` +
                `<button class="${cls.DELETE_EVENT_CLASS} ${
                  cls.TEXT_ELLIPSIS
                }" title="${this.l10n.getConstant('delete')}">` +
  ...
  public getResourceText(
    ...
    if (this.parent.activeViewOptions.group.resources.length === 0) {
      ...
      const resourceData: Record<string, number>[] =
        resourceCollection.dataSource as Record<string, number>[];
      if (type === 'event') {
        const eventData: Record<string, any> = args.event as Record<
          string,
          any
        >;
        for (const data of resourceData) {
          const resourceId: number | number[] = eventData[
            resourceCollection.field
          ] as number | number[];
          if (resourceId instanceof Array) {
            if (resourceId.indexOf(data[resourceCollection.idField]) > -1) {
              const id: number =
                resourceId[
                  resourceId.indexOf(data[resourceCollection.idField])
                ];
              const resource: Record<string, number> = resourceData.filter(
                (e: Record<string, number>) =>
                  e[resourceCollection.idField] === id
              )[0];
              resourceValue +=
                resourceValue === ''
        ...
        resourceValue =
          resourceData[0][resourceCollection.textField].toString();
      }
    } else {
      if (type === 'event') {
        const eventData: Record<string, any> = args.event as Record<
        ...
        for (
          let i: number =
            this.parent.resourceBase.resourceCollection.length - 1;
          i >= 0;
          i--
        ) {
        ...
        const lastResourceData: Record<string, any>[] =
          lastResource.dataSource as Record<string, any>[];
        resourceData.forEach((value: string) => {
          let text: string;
          const i: number = util.findIndexInData(
            lastResourceData,
            lastResource.idField,
            value
          );
          if (i > -1) {
            text = lastResourceData[parseInt(i.toString(), 10)][
              lastResource.textField
            ] as string;
          }
          if (text) {
            resNames.push(text);
        ...
        const groupIndex: number = !isNullOrUndefined(argsData.groupIndex)
          ? argsData.groupIndex
          : 0;
        const resourceDetails: TdData =
          this.parent.resourceBase.lastResourceLevel[
            parseInt(groupIndex.toString(), 10)
          ];
        resourceValue = resourceDetails.resourceData[
  ...
  private getFormattedString(
    ...
    const fields: EventFieldsMapping = this.parent.eventFields;
    const eventSubject: string = (eventData[fields.subject] ||
      this.l10n.getConstant('noTitle')) as string;
    ...
    const endDateDetails: string =
      eventData[fields.isAllDay] &&
      endDate.getHours() === 0 &&
      endDate.getMinutes() === 0
        ? this.getDateFormat(
            util.addDays(new Date(endDate.getTime()), -1),
            'long'
          )
    ...
    if (eventData[fields.isAllDay]) {
      ...
      if (
        (util.getUniversalTime(endDate) - util.getUniversalTime(startDate)) /
          util.MS_PER_DAY >
        1
      ) {
        details +=
      ...
      details =
        startDateDetails +
        ' (' +
        (startTimeDetail + '&nbsp;-&nbsp;' + endTimeDetail) +
        ')';
    }
    return { eventSubject: eventSubject, details: details };
  }

  public moreEventClick(
    data: EventClickArgs,
    endDate: Date,
    groupIndex?: string
  ): void {
    this.quickPopupHide(true);
    const moreEventContentEle: Element = this.morePopup.element.querySelector(
      '.' + cls.MORE_EVENT_CONTENT_CLASS
    );
    ...
    const target: Element = closest(
      ...
      '.' + cls.MORE_INDICATOR_CLASS + ',.' + cls.WORK_CELLS_CLASS
    );
    const day: string = this.parent.globalize.formatDate(data.date, {
      format: 'E',
      calendar: this.parent.getCalendarMode(),
    });
    this.morePopup.element.querySelector(
      '.' + cls.MORE_EVENT_HEADER_DAY_CLASS
    ).innerHTML = util.capitalizeFirstWord(day, 'single');
    const dateElement: Element = this.morePopup.element.querySelector(
      '.' + cls.MORE_EVENT_HEADER_DATE_CLASS
    );
    ...
    if (!isNullOrUndefined(groupIndex)) {
      ...
      groupOrder =
        this.parent.resourceBase.lastResourceLevel[parseInt(groupIndex, 10)]
          .groupOrder;
    }
    const moreEventElements: HTMLElement = this.createMoreEventList(
      data.event as Record<string, any>[],
      groupOrder,
      groupIndex
    );
    this.morePopup.element
      .querySelector('.' + cls.MORE_EVENT_POPUP_CLASS)
      .appendChild(moreEventElements);
    removeClass(
      this.morePopup.element.querySelector(
        '.' + cls.MORE_EVENT_DATE_HEADER_CLASS
      ).childNodes,
      cls.CURRENTDATE_CLASS
    );
    if (
      util.resetTime(data.date).getTime() ===
      util.resetTime(this.parent.getCurrentTime()).getTime()
    ) {
      addClass(
        this.morePopup.element.querySelector(
          '.' + cls.MORE_EVENT_DATE_HEADER_CLASS
    ...
    if (!this.parent.isAdaptive) {
      if (this.parent.currentView.indexOf('Timeline') !== -1) {
        ...
        const startDate: Date = new Date(
          parseInt(target.getAttribute('data-start-date'), 10)
        );
        ...
        if (isNullOrUndefined(gIndex)) {
          ...
          if (isNullOrUndefined(this.morePopup.relateTo)) {
            ...
            for (let i: number = 0; i < workCells.length; i++) {
              const date: string =
                workCells[parseInt(i.toString(), 10)].getAttribute('data-date');
              if (date < tdDate) {
                this.morePopup.relateTo = workCells[
                  parseInt(i.toString(), 10)
                ] as HTMLElement;
              }
            }
        ...
        this.morePopup.relateTo = closest(
          <Element>target,
          '.' + cls.WORK_CELLS_CLASS
        ) as HTMLElement;
      }
  ...
  private detailsClick(event: Event): void {
    const subjectEle: HTMLInputElement = this.quickPopup.element.querySelector(
      '.' + cls.SUBJECT_CLASS
    ) as HTMLInputElement;
    ...
    this.quickPopupHide();
    this.parent.eventWindow.openEditor(
      this.parent.activeCellsData as unknown as Record<string, any>,
      'Add'
    );
  }
  ...
  private editClick(event: Event): void {
    this.dialogEvent = event;
    this.quickPopupHide(true);
    const data: Record<string, any> = this.parent.activeEventData
  ...
  private updateMoreEventContent(): void {
    ...
    const startDate: Date = new Date(
      parseInt(dateElement.getAttribute('data-date'), 10)
    );
    const endDate: Date = new Date(
      parseInt(dateElement.getAttribute('data-end-date'), 10)
    );
    ...
    const events: Record<string, any>[] = this.parent.eventBase.filterEvents(
      startDate,
      endDate,
      this.parent.eventsProcessed,
      data
    );
    const moreElement: HTMLElement = this.createMoreEventList(
      events,
      groupOrder,
      groupIndex
    );
  ...
  private closeClick(event: Event): void {
    ...
    if (
      ...
      this.parent.selectCell(
        this.parent.activeCellsData.element as HTMLTableCellElement
      );
    }
    this.quickPopupHide();
    this.morePopup.hide();
  ...
  private dialogButtonClick(event: Event): void {
    ...
    const cancelBtn: Element = this.quickDialog.element.querySelector(
      '.' + cls.QUICK_DIALOG_ALERT_CANCEL
    );
    const eventData: Record<string, any> = this.parent.activeEventData
      .event as Record<string, any>;
    if (target.classList.contains(cls.QUICK_DIALOG_OCCURRENCE_CLASS)) {
      this.parent.currentAction =
        this.parent.currentAction === 'Delete'
          ? 'DeleteOccurrence'
          : 'EditOccurrence';
      switch (this.parent.currentAction) {
        ...
        case 'DeleteOccurrence':
          this.parent.crudModule.deleteEvent(
            eventData,
            this.parent.currentAction
          );
          break;
      ...
      target.classList.contains(cls.QUICK_DIALOG_FOLLOWING_EVENTS_CLASS)
    ) {
      this.parent.currentAction =
        this.parent.currentAction === 'Delete'
      ...
      switch (this.parent.currentAction) {
        case 'EditSeries':
          this.parent.eventWindow.openEditor(
            this.parent.eventBase.getParentEvent(eventData, true),
            this.parent.currentAction
          );
          break;
        case 'DeleteSeries':
      ...
      !cancelBtn.classList.contains(cls.DISABLE_CLASS) &&
      (target.classList.contains(cls.QUICK_DIALOG_ALERT_OK) ||
        (target.classList.contains(cls.QUICK_DIALOG_ALERT_CANCEL) &&
          !cancelBtn.classList.contains(cls.QUICK_DIALOG_CANCEL_CLASS)))
    ) {
      this.parent.uiStateValues.isIgnoreOccurrence = target.classList.contains(
        cls.QUICK_DIALOG_ALERT_CANCEL
      );
  ...
  private updateTapHoldEventPopup(target: Element): void {
    ...
    if (selectedElements.length > 0) {
      const eventObj: Record<string, any> =
        this.parent.eventBase.getEventByGuid(
          selectedElements[0].getAttribute('data-guid')
        );
      const titleContent: string =
        selectedElements.length === 1
          ? (((<Record<string, any>>eventObj)[
              this.parent.eventFields.subject
            ] || this.l10n.getConstant('noTitle')) as string)
          : '(' +
            selectedElements.length +
            ')' +
            '&nbsp;' +
            this.l10n.getConstant('selectedItems');
      this.quickPopup.element.querySelector('.' + cls.SUBJECT_CLASS).innerHTML =
        titleContent;
      if (selectedElements.length > 1) {
        addClass(
          [this.quickPopup.element.querySelector('.' + cls.EDIT_ICON_CLASS)],
          cls.HIDDEN_CLASS
        );
      } else {
        removeClass(
  ...
  private getTimezone(event: Record<string, any>): string {
    ...
    zoneDetails +=
      (event[this.parent.eventFields.startTimezone] as string) || '';
  ...
  private getRecurrenceSummary(event: Record<string, any>): string {
    const recurrenceEditor: RecurrenceEditor =
      this.parent.eventWindow.getRecurrenceEditorInstance();
    if (recurrenceEditor) {
      const ruleSummary: string = recurrenceEditor.getRuleSummary(
        <string>event[this.parent.eventFields.recurrenceRule]
      );
  ...
  private getDataFromTarget(target: Element): Record<string, any> {
    ...
    const cellsData: Record<string, any> = this.parent
      .activeCellsData as unknown as Record<string, any>;
  ...
  private beforeQuickDialogClose(e: BeforeCloseEventArgs): void {
    const args: PopupCloseEventArgs = {
      ...
      type: isNullOrUndefined(this.parent.activeEventData.event)
        ? 'ValidationAlert'
        : !isNullOrUndefined(
            (<Record<string, any>>this.parent.activeEventData.event)[
              this.parent.eventFields.recurrenceRule
        ...
        : 'DeleteAlert',
      cancel: false,
      data: this.parent.activeEventData.event as Record<string, any>,
      element: this.quickDialog.element,
    };
    this.parent.trigger(
      event.popupClose,
      args,
      (popupCloseArgs: PopupCloseEventArgs) => {
  ...
  private beforeQuickPopupOpen(
    target: Element,
    originalEvent: (MouseEvent & TouchEvent) | Touch
  ): void {
    this.parent.renderTemplates(() => {
      const isEventPopup: Element = this.quickPopup.element.querySelector(
        '.' + cls.EVENT_POPUP_CLASS
      );
      const popupType: PopupType = this.parent.isAdaptive
        ? isEventPopup
          ? 'ViewEventInfo'
          : 'EditEventInfo'
        : 'QuickInfo';
      const eventProp: PopupOpenEventArgs = {
        type: popupType,
        cancel: false,
        data: extend({}, this.getDataFromTarget(target), null, true) as Record<
          string,
          any
        >,
        target: target,
        ...
        (popupArgs: PopupOpenEventArgs) => {
          if (popupArgs.cancel) {
            ...
            if (this.parent.isAdaptive) {
              ...
              this.quickPopup.element.style.display = 'block';
              this.quickPopup.element.style.height = formatUnit(
                popupType === 'EditEventInfo' ? 65 : window.innerHeight
              );
            } else {
              const isVirtualScroll: boolean =
                this.parent.virtualScrollModule &&
                this.parent.virtualScrollModule.isHorizontalScroll &&
                !isNullOrUndefined(
                  closest(target, '.' + cls.CONTENT_TABLE_CLASS)
                );
              const conTable: HTMLElement = this.parent.element.querySelector(
                '.' + cls.CONTENT_WRAP_CLASS + ' table'
              );
              this.quickPopup.offsetX =
                isVirtualScroll && !this.parent.enableRtl
                  ? util.getTranslateX(conTable) + 10
                  : 10;
              ...
              if (
                collide.indexOf(this.parent.enableRtl ? 'left' : 'right') > -1
              ) {
                this.quickPopup.offsetX =
                  -(target as HTMLElement).offsetWidth -
                  10 -
                  this.quickPopup.element.offsetWidth;
                if (isVirtualScroll && !this.parent.enableRtl) {
                  this.quickPopup.offsetX =
                    util.getTranslateX(conTable) + this.quickPopup.offsetX;
                }
                this.quickPopup.dataBind();
                this.quickPopup.refreshPosition(null, true);
                const leftCollide: string[] = isCollide(
                  this.quickPopup.element,
                  this.parent.element
                );
                if (leftCollide.indexOf('left') > -1) {
                  ...
                  this.quickPopup.offsetX = -(
                    this.quickPopup.element.offsetWidth / 2
                  );
                  this.quickPopup.dataBind();
                }
              }
              if (
                this.parent.virtualScrollModule &&
                !this.parent.virtualScrollModule.isHorizontalScroll &&
                (collide.indexOf('top') > -1 || collide.indexOf('bottom') > -1)
              ) {
              ...
              if (
                this.quickPopup.position.X === 'center' &&
                this.quickPopup.position.Y === 'center' &&
                !isNullOrUndefined(originalEvent) &&
                ...
                const targetRect: ClientRect | DOMRect =
                  target.getBoundingClientRect();
                const offsetY: number =
                  (originalEvent as MouseEvent).offsetY ||
                  Math.ceil(clientY - (targetRect as any).y);
                const previousOffset: number = this.quickPopup.offsetY;
                let collision: string[] = isCollide(
                  this.quickPopup.element,
                  target as HTMLElement
                );
                const popupRect: ClientRect | DOMRect =
                  this.quickPopup.element.getBoundingClientRect();
                const targetEle: Element = document.elementFromPoint(
                  clientX,
                  clientY
                );
                if (
                  collision.indexOf('top') > -1 ||
                  collision.indexOf('bottom') > -1 ||
                  closest(targetEle, '.' + cls.POPUP_WRAPPER_CLASS)
                ) {
                  if (
                    popupRect.top <= clientY &&
                    clientY <= popupRect.top + popupRect.height
                  ) {
                    this.quickPopup.offsetY =
                      previousOffset - popupRect.height - 10;
                    this.quickPopup.dataBind();
                    collision = isCollide(
                      this.quickPopup.element,
                      this.parent.element
  ...
  private applyEventColor(): void {
    ...
    if (
      ...
      colorField = this.parent.enableRtl
        ? 'border-right-color'
        : 'border-left-color';
    } else {
      colorField = 'background-color';
    }
    let color: string = (<HTMLElement>this.parent.activeEventData.element)
      .style[<any>colorField];
    if (color === '') {
      return;
    }
    let colorEle: HTMLElement = this.quickPopup.element.querySelector(
      '.' + cls.POPUP_HEADER_CLASS
    ) as HTMLElement;
    const footerEle: HTMLElement = this.quickPopup.element.querySelector(
      '.' + cls.POPUP_FOOTER_CLASS
    ) as HTMLElement;
    if (footerEle && footerEle.offsetParent) {
      colorEle = this.quickPopup.element.querySelector(
        '.' + cls.SUBJECT_CLASS
      ) as HTMLElement;
      if (colorEle) {
  ...
  private quickPopupOpen(): void {
    ...
    if (this.quickPopup.element.querySelector('.' + cls.CELL_POPUP_CLASS)) {
      ...
      const editElement: HTMLInputElement =
        this.quickPopup.element.querySelector(
          '.' + cls.EDIT_EVENT_CLASS
        ) as HTMLInputElement;
      if (editElement) {
      ...
      const editIcon: HTMLInputElement = this.quickPopup.element.querySelector(
        '.' + cls.EDIT_CLASS
      ) as HTMLInputElement;
      if (editIcon) {
        editIcon.focus();
  ...
  private morePopupClose(): void {
    const moreWrapper: Element = this.parent.element.querySelector(
      '.' + cls.MORE_EVENT_WRAPPER_CLASS
    );
    if (moreWrapper) {
      remove(moreWrapper);
    }
  ...
  public quickPopupHide(hideAnimation?: boolean): void {
    ...
    const isCellPopup: Element = this.quickPopup.element.querySelector(
      '.' + cls.CELL_POPUP_CLASS
    );
    let popupData: Record<string, any>;
    if (isCellPopup) {
      const formvalidator: Element =
        this.quickPopup.element.querySelector('.e-formvalidator');
      if (
        this.isCrudAction &&
        formvalidator &&
        !(
          (formvalidator as EJ2Instance).ej2_instances[0] as FormValidator
        ).validate()
      ) {
        return;
      }
      ...
      const saveObj: Record<string, any> =
        this.parent.eventWindow.getObjectFromFormData(cls.POPUP_WRAPPER_CLASS);
    ...
    const args: PopupCloseEventArgs = {
      ...
      target: (isCellPopup
        ? this.parent.activeCellsData.element
        : this.parent.activeEventData.element) as Element,
    };
    this.parent.trigger(
      event.popupClose,
      args,
  ...
  private navigationClick(e: Event): void {
    const navigateEle: Element = closest(
      e.target as Element,
      '.' + cls.NAVIGATE_CLASS
    );
    if (!isNullOrUndefined(navigateEle)) {
      const date: Date = this.parent.getDateFromElement(
  ...
  private documentClick(e: { event: Event }): void {
    ...
    const classNames: string =
      ...
      cls.APPOINTMENT_CLASS +
      ',.e-popup';
    const popupWrap: Element = this.parent.element.querySelector(
      '.' + cls.POPUP_WRAPPER_CLASS
    );
    if (
      (popupWrap &&
        popupWrap.childElementCount > 0 &&
        !closest(target, classNames)) ||
      !closest(target, classNames)
    ) {
      this.quickPopupHide();
      this.parent.removeNewEventElement();
    }
    const tar: Element = this.parent.element.querySelector(
      '.' + cls.INLINE_SUBJECT_CLASS
    );
    ...
    if (
      closest(
        target,
        '.' + cls.APPOINTMENT_CLASS + ',.' + cls.HEADER_CELLS_CLASS
      )
    ) {
      this.parent.removeNewEventElement();
    }
    if (
      !closest(target, '.' + cls.MORE_POPUP_WRAPPER_CLASS) &&
      target.classList &&
  ...
  public onClosePopup(event?: Event): void {
    ...
    if (
      ...
      (!isNullOrUndefined(event) &&
        (event as KeyboardEventArgs).action !== 'escape') ||
      (this.parent.inlineModule &&
        this.parent.element.querySelector('.' + cls.INLINE_APPOINTMENT_CLASS))
    ) {
      this.parent.eventBase.focusElement();
    }
  }
  ...
  private destroyPopupButtons(popupName: string): void {
    const popup: Popup =
      popupName === 'quickPopup' ? this.quickPopup : this.morePopup;
    const buttonCollections: HTMLElement[] = [].slice.call(
      popup.element.querySelectorAll('.e-control.e-btn')
    );
    for (const button of buttonCollections) {
      const instance: Button = (button as EJ2Instance)
        .ej2_instances[0] as Button;
      if (instance) {
        instance.destroy();
      }
    }
    if (popupName === 'quickPopup') {
      const input: HTMLInputElement = popup.element.querySelector(
        'input.' + cls.SUBJECT_CLASS
      );
      if (input) {
        input.remove();
      }
