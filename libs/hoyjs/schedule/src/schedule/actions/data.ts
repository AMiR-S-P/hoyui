...
import { Query, DataManager, Predicate } from '@hoyui/data';
...
export class Data {
  ...
  public generateQuery(startDate?: Date, endDate?: Date): Query {
    ...
    if (this.parent && startDate && endDate) {
      if (
        ...
        this.parent.resourceBase.renderedResources.length > 0
      ) {
        const resIdCollection: string[][] = [];
        this.parent.resourceBase.resourceCollection.forEach(() =>
          resIdCollection.push([])
        );
        this.parent.resourceBase.renderedResources.forEach(
          (resource: Record<string, any>) => {
            resIdCollection.forEach((resId: string[], index: number) => {
              const groupId: string =
                resource.groupOrder[parseInt(index.toString(), 10)];
        ...
        this.parent.resourceBase.resourceCollection.forEach(
          (resource: Record<string, any>, index: number) => {
            query.addParams(
              resource.field,
              resIdCollection[parseInt(index.toString(), 10)].toString()
            );
          }
        );
      }
      if (this.parent.timezone) {
        startDate = this.parent.tzModule.remove(
          new Date(+startDate.getTime()),
          this.parent.timezone
        );
        endDate = this.parent.tzModule.remove(
      ...
      if (this.parent.eventSettings.includeFiltersInQuery) {
        ...
        const recurrenceQuery: Predicate = new Predicate(
          this.parent.eventFields.recurrenceRule,
          'notequal',
          null
        ).and(
          new Predicate(this.parent.eventFields.recurrenceRule, 'notequal', '')
        );
  ...
  public getStartEndQuery(startDate?: Date, endDate?: Date): Predicate {
    ...
    const dateQuery: Predicate = new Predicate(
      ...
      .or(
        new Predicate(fieldMapping.startTime, 'lessthanorequal', startDate).and(
          new Predicate(fieldMapping.endTime, 'greaterthan', startDate)
        )
      );
    return dateQuery;
  }
