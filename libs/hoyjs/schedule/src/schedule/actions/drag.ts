...
import { addClass, remove, removeClass, setStyleAttribute } from '@hoyui/base';
import {
  DragEventArgs,
  EventFieldsMapping,
  TdData,
...
export class DragAndDrop extends ActionBase {
  ...
  public setDragArea(): void {
    const dragElement: HTMLElement = document.querySelector(
      this.parent.eventDragArea
    );
    this.dragArea =
      this.parent.eventDragArea && dragElement
        ? dragElement
        : (this.parent.element.querySelector(
            '.' + cls.CONTENT_TABLE_CLASS
          ) as HTMLElement);
  }
  ...
  private dragHelper(e: Record<string, any>): HTMLElement {
    ...
    if (
      !this.parent.allowMultiDrag ||
      isNullOrUndefined(this.parent.selectedElements) ||
      this.parent.selectedElements.length === 0 ||
      (this.parent.selectedElements.length > 0 &&
    ...
    elements.forEach((ele: HTMLElement) => {
      ...
      if (
        ele.getAttribute('data-guid') ===
        this.actionObj.element.getAttribute('data-guid')
      ) {
        ...
        if (
          ...
          !this.parent.activeView.isTimelineView() &&
          !this.actionObj.element.classList.contains(
            cls.ALLDAY_APPOINTMENT_CLASS
          )
        ) {
        ...
        this.actionObj.clone.style.top = formatUnit(
          this.actionObj.element.offsetTop
        );
      }
  ...
  private dragPosition(e: Record<string, any>): Record<string, any> {
    ...
    const cellHeight: number =
      (this.actionObj.cellHeight / this.actionObj.slotInterval) *
      this.actionObj.interval;
    ...
    if (this.isStepDragging) {
      cloneRight =
        Math.ceil(this.actionObj.clone.getBoundingClientRect().right) +
        this.actionObj.interval;
    } else {
      cloneRight = this.actionObj.clone.getBoundingClientRect().right;
    }
    const dragArea: HTMLElement = this.parent.element.querySelector(
      '.' + cls.CONTENT_WRAP_CLASS
    ) as HTMLElement;
    const contentWrapRight: number = dragArea.getBoundingClientRect().right;
    if (
      this.parent.activeView.isTimelineView() &&
      this.parent.currentView !== 'TimelineYear' &&
      !this.parent.enableRtl &&
      this.actionObj.pageX > cloneRight &&
      !this.isMorePopupOpened &&
      !(this.actionObj.pageX > contentWrapRight)
    ) {
    ...
    if (
      this.parent.activeView.isTimelineView() ||
      !this.parent.timeScale.enable ||
      (!isNullOrUndefined(this.actionObj.clone.offsetParent) &&
        this.actionObj.clone.offsetParent.classList.contains(
          cls.MORE_EVENT_POPUP_CLASS
        ))
    ) {
      ...
      topValue = formatUnit(
        (<HTMLElement>(
          this.parent.element.querySelector('.' + cls.ALLDAY_ROW_CLASS)
        )).offsetTop
      );
      setStyleAttribute(this.actionObj.clone, {
        width: formatUnit(
      ...
      const scrollHeight: number =
        this.parent.element.querySelector('.e-content-wrap').scrollHeight;
      const cloneBottom: number =
        parseInt(topValue, 10) + this.actionObj.clone.offsetHeight;
  ...
  private setDragActionDefaultValues(): void {
    ...
    this.actionObj.slotInterval =
      this.parent.activeViewOptions.timeScale.interval /
      this.parent.activeViewOptions.timeScale.slotCount;
    this.actionObj.interval = this.actionObj.slotInterval;
    const workCell: HTMLElement = this.parent.element.querySelector(
  ...
  private dragStart(e: MouseEvent & TouchEvent & DragEventArgs): void {
    ...
    this.actionObj.event = this.parent.eventBase.getEventByGuid(
      eventGuid
    ) as Record<string, any>;
    const eventObj: Record<string, any> = extend(
      {},
      this.actionObj.event,
      null,
      true
    ) as Record<string, any>;
    if (!isNullOrUndefined(eventObj)) {
      this.startTime = (
        eventObj[this.parent.eventFields.startTime] as Date
      ).getTime();
    }
    ...
    this.parent.trigger(
      ...
      (dragEventArgs: DragEventArgs) => {
        if (
          dragEventArgs.cancel ||
          (!isNullOrUndefined(this.actionObj.element) &&
            isNullOrUndefined(this.actionObj.element.parentElement))
        ) {
          const dragObj: Draggable = (this.actionObj.element as EJ2Instance)
            .ej2_instances[0] as Draggable;
          if (!isNullOrUndefined(dragObj)) {
            dragObj.intDestroy(
              (e as DragEventArgs).event as MouseEvent & TouchEvent
            );
          }
        ...
        this.actionObj.start = eventObj[
          this.parent.eventFields.startTime
        ] as Date;
        this.actionObj.end = eventObj[this.parent.eventFields.endTime] as Date;
        this.actionObj.groupIndex = parseInt(
          this.actionObj.element.getAttribute('data-group-index') || '0',
          10
        );
        ...
        this.enableCurrentViewDrag =
          dragArgs.dragWithinRange &&
          !dragArgs.navigation.enable &&
          this.parent.allowMultiDrag;
        this.actionObj.excludeSelectors = dragEventArgs.excludeSelectors;
        const viewElement: HTMLElement = this.parent.element.querySelector(
          '.' + cls.CONTENT_WRAP_CLASS
        ) as HTMLElement;
        this.scrollArgs = {
          element: viewElement,
        ...
        this.isAllDayTarget = this.actionObj.clone.classList.contains(
          cls.ALLDAY_APPOINTMENT_CLASS
        );
        if (
          (this.parent.activeView.isTimelineView() ||
          ...
          const rows: HeaderRowsModel[] =
            this.parent.activeViewOptions.headerRows;
          this.isHeaderRows =
            rows.length > 0 &&
            rows[rows.length - 1].option !== 'Hour' &&
            rows[rows.length - 1].option !== 'Date';
          this.isTimelineDayProcess =
            !this.parent.activeViewOptions.timeScale.enable ||
            this.isHeaderRows ||
            this.parent.currentView === 'TimelineMonth' ||
            (rows.length > 0 && rows[rows.length - 1].option === 'Date');
          this.isStepDragging =
            !this.isTimelineDayProcess &&
        ...
        if (
          ...
          this.parent.currentView === 'Week' ||
          this.parent.currentView === 'WorkWeek'
        ) {
          this.verticalEvent = new VerticalEvent(this.parent);
          this.verticalEvent.initializeValues();
          const splitEvents: Record<string, any>[] = this.splitEvent(
            this.actionObj.event
          );
          splitEvents.forEach((event: Record<string, any>) => {
            let query: string = `.e-day-wrapper[data-date="${(<Date>(
              util.resetTime(event[this.parent.eventFields.startTime])
            )).getTime()}"]`;
            if (this.parent.activeViewOptions.group.resources.length > 0) {
              query = query.concat(
                '[data-group-index = "' + this.actionObj.groupIndex + '"]'
              );
            }
            const appWrap: HTMLTableCellElement =
              this.parent.element.querySelector(query) as HTMLTableCellElement;
            if (appWrap) {
              const appEle: Element = appWrap.querySelector(
                '[data-id="' +
                  this.actionObj.clone.getAttribute('data-id') +
                  '"]'
              );
              if (appEle) {
  ...
  public getSelectedData(): Record<string, any>[] {
    if (
      ...
      for (const element of this.parent.selectedElements) {
        ...
        const data: Record<string, any> = this.parent.eventBase.getEventByGuid(
          eventGuid
        ) as Record<string, any>;
        this.multiData.push(
          extend({}, data, null, true) as Record<string, any>
        );
      }
    }
  ...
  private drag(e: MouseEvent & TouchEvent): void {
    if (
      (e as Record<string, any>).event &&
      (e as Record<string, any>).event.type === 'touchmove'
    ) {
    ...
    const eventArgs: (MouseEvent & TouchEvent) | Touch =
      this.parent.eventBase.getPageCoordinates(e);
    ...
    this.widthUptoCursorPoint =
      this.widthUptoCursorPoint === 0
        ? Math.ceil(
            Math.abs(
              this.actionObj.clone.getBoundingClientRect().left -
                this.actionObj.X
            ) / this.widthPerMinute
          ) * this.widthPerMinute
        : this.widthUptoCursorPoint;
    this.widthUptoCursorPoint = this.isMorePopupOpened
      ? this.actionObj.cellWidth
      : this.widthUptoCursorPoint;
    this.heightUptoCursorPoint =
      this.heightUptoCursorPoint === 0
        ? Math.ceil(
            Math.abs(
              this.actionObj.clone.getBoundingClientRect().top -
                this.actionObj.Y
            ) / this.heightPerMinute
          ) * this.heightPerMinute
        : this.heightUptoCursorPoint;
    ...
    if (
      (this.parent.currentView === 'Month' || this.isAllDayDrag) &&
      this.daysVariation < 0
    ) {
      const date: Date = this.parent.getDateFromElement(
        this.actionObj.target as HTMLElement
      );
      if (!isNullOrUndefined(date)) {
        const currentDate: Date = util.resetTime(date);
        const startDate: Date = util.resetTime(
          new Date(
            (eventObj[this.parent.eventFields.startTime] as Date).getTime()
          )
        );
        this.daysVariation =
          (currentDate.getTime() - startDate.getTime()) / util.MS_PER_DAY;
      } else {
        this.daysVariation = 0;
      }
    } else {
    ...
    if (this.parent.eventDragArea) {
      ...
      const currentTarget: Element = <Element>(
        closest(targetElement, '.' + cls.ROOT)
      );
      if (!currentTarget) {
        ...
        if (
          ...
          this.actionObj.clone.style.width = formatUnit(
            this.actionObj.element.offsetWidth
          );
        }
      }
    }
  ...
  private calculateMinutesDiff(eventObj: Record<string, any>): void {
    if (this.parent.enableRtl) {
      ...
      this.minDiff =
        (this.widthUptoCursorPoint / this.widthPerMinute) *
        this.actionObj.interval;
    }
    ...
    const startEndHours: { [key: string]: Date } = util.getStartEndHours(
      startTime,
      this.parent.activeView.getStartHour(),
      this.parent.activeView.getEndHour()
    );
    if (startEndHours.startHour.getTime() > startDate.getTime()) {
      this.minDiff =
        this.minDiff +
        (startEndHours.startHour.getTime() - startDate.getTime()) /
          util.MS_PER_MINUTE;
    }
  }
  ...
  private dragStop(e: MouseEvent): void {
    ...
    this.parent.uiStateValues.action = this.parent.uiStateValues.isTapHold =
      false;
    ...
    const target: HTMLElement = (
      ...
      !(e.target as Element).classList.contains('e-work-cells') &&
      this.parent.cellTemplate
        ? closest(e.target as Element, '.e-work-cells')
        : e.target
    ) as HTMLElement;
    const dragArgs: DragEventArgs = {
      cancel: false,
        ...
        if (this.parent.isSpecificResourceEvents()) {
          ...
          this.parent.crudModule.crudObj.sourceEvent = [
            this.parent.resourceBase.lastResourceLevel[
              parseInt(dragArgs.element.getAttribute('data-group-index'), 10)
            ],
          ];
          const currentGroupIndex: number =
            parseInt(dragArgs.target.getAttribute('data-group-index'), 10) ||
            this.actionObj.groupIndex;
          this.parent.crudModule.crudObj.targetEvent = [
            this.parent.resourceBase.lastResourceLevel[
              parseInt(currentGroupIndex.toString(), 10)
            ],
          ];
        }
        ...
        this.actionObj.navigationInterval = window.setInterval(() => {
          if (currentDate) {
            ...
            let start: number =
              ...
              this.actionObj.navigation.timeDelay / 1000;
  ...
  public updateDraggingDateTime(e: MouseEvent & TouchEvent): void {
    if (
      ...
      this.actionObj.cellWidth = this.isHeaderRows
        ? this.timelineEventModule.cellWidth
        : this.parent.getElementWidth(
            this.parent.element.querySelector('.' + cls.WORK_CELLS_CLASS)
          );
  ...
  public navigationWrapper(): void {
    if (!this.parent.activeView.isTimelineView()) {
      if (
        ...
        const outerWrapperCls: HTMLElement[] = [].slice.call(
          this.parent.element.querySelectorAll('.' + cls.WORK_CELLS_CLASS)
        );
        this.actionObj.index =
          this.parent.activeView.renderDates.length < this.actionObj.index
            ? this.parent.activeView.renderDates.length - 1
            : this.actionObj.index;
        let targetWrapper: Element = outerWrapperCls[
          this.actionObj.index
        ].querySelector('.' + cls.APPOINTMENT_WRAPPER_CLASS);
        if (!targetWrapper) {
        ...
        const wrapperClass: string = this.actionObj.clone.classList.contains(
          cls.ALLDAY_APPOINTMENT_CLASS
        )
          ? '.' + cls.ALLDAY_APPOINTMENT_WRAPPER_CLASS
          : '.' + cls.APPOINTMENT_WRAPPER_CLASS;
        this.parent.element
          .querySelectorAll(wrapperClass)
          .item(this.actionObj.index)
          .appendChild(this.actionObj.clone);
        if (wrapperClass === '.' + cls.ALLDAY_APPOINTMENT_WRAPPER_CLASS) {
          ...
          const event: HTMLElement[] = [].slice.call(
            this.parent.element.querySelectorAll(
              '.' + cls.ALLDAY_CELLS_CLASS + ':first-child'
            )
          );
      ...
      if (this.parent.activeViewOptions.group.resources.length > 0) {
        outWrapper = this.parent.element
          .querySelectorAll('.e-appointment-container:not(.e-hidden)')
          .item(this.actionObj.index);
      } else {
        outWrapper = this.parent.element.querySelector(
          '.' + cls.APPOINTMENT_CONTAINER_CLASS
        );
      }
      if (!isNullOrUndefined(outWrapper)) {
        let tarWrapper: Element = outWrapper.querySelector(
          '.' + cls.APPOINTMENT_WRAPPER_CLASS
        );
        if (!tarWrapper) {
  ...
  private viewNavigation(e: MouseEvent & TouchEvent): void {
    ...
    if (
      dragArea &&
      ((!this.scrollEdges.top && !this.scrollEdges.bottom) ||
        closest(
      ...
      if (
        dragArea.scrollLeft === 0 &&
        Math.round(this.actionObj.X) <=
          Math.round(
            dragArea.getBoundingClientRect().left +
              this.actionObj.cellWidth +
              window.pageXOffset
          )
      ) {
        ...
        Math.round(dragArea.scrollLeft) + dragArea.clientWidth ===
          dragArea.scrollWidth &&
        Math.round(this.actionObj.X) >=
          Math.round(
            dragArea.getBoundingClientRect().right -
              this.actionObj.cellWidth +
              window.pageXOffset
          )
      ) {
      ...
      if (navigationType) {
        this.parent.changeDate(
          this.parent.activeView.getNextPreviousDate(navigationType)
        );
      }
    }
  }
  ...
  private morePopupEventDragging(e: MouseEvent & TouchEvent): void {
    if (
      isNullOrUndefined(e.target) ||
      (e.target &&
        isNullOrUndefined(closest(<HTMLTableCellElement>e.target, 'td')))
    ) {
      return;
    }
    const eventObj: Record<string, any> = extend(
    ...
    const td: HTMLElement = closest(
      <HTMLTableCellElement>e.target,
      'td'
    ) as HTMLElement;
    if (
      this.parent.currentView === 'TimelineYear' &&
      (!td.classList.contains(cls.WORK_CELLS_CLASS) ||
        td.classList.contains(cls.OTHERMONTH_CLASS))
    ) {
      return;
    }
    ...
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      this.actionObj.groupIndex = parseInt(
        td.getAttribute('data-group-index'),
        10
      );
    }
    ...
    this.actionObj.clone.style.top = formatUnit(
      (<HTMLElement>td.offsetParent).offsetTop
    );
    ...
    if (this.parent.activeView.isTimelineView()) {
      const rowIndex: number = (closest(td, 'tr') as HTMLTableRowElement)
        .rowIndex;
      eventContainer = this.parent.element
        .querySelectorAll('.e-appointment-container')
        .item(rowIndex) as HTMLElement;
    }
    eventWrapper = eventContainer.querySelector(
      '.' + cls.APPOINTMENT_WRAPPER_CLASS
    ) as HTMLElement;
    if (!eventWrapper) {
  ...
  private calculateVerticalTime(e: MouseEvent & TouchEvent): void {
    if (
      isNullOrUndefined(this.actionObj.target) ||
      (this.actionObj.target &&
        isNullOrUndefined(
          closest(<HTMLTableCellElement>this.actionObj.target, 'tr')
        )) ||
      (!isNullOrUndefined(closest(this.actionObj.target as Element, 'td')) &&
        !closest(this.actionObj.target as Element, 'td').classList.contains(
          cls.WORK_CELLS_CLASS
        ) &&
        !closest(this.actionObj.target as Element, 'td').classList.contains(
    ...
    const eventStart: Date = eventObj[
      this.parent.eventFields.startTime
    ] as Date;
    const eventEnd: Date = eventObj[this.parent.eventFields.endTime] as Date;
    const eventDuration: number =
      util.getUniversalTime(eventEnd) - util.getUniversalTime(eventStart);
    let offsetTop: number =
      Math.floor(
        parseInt(this.actionObj.clone.style.top, 10) / this.actionObj.cellHeight
      ) * this.actionObj.cellHeight;
    offsetTop = offsetTop < 0 ? 0 : offsetTop;
    if (this.scrollEdges.top || this.scrollEdges.bottom) {
      offsetTop = this.scrollEdges.top
        ? dragArea.scrollTop -
          this.heightUptoCursorPoint +
          this.actionObj.cellHeight +
          window.pageYOffset
        : dragArea.scrollTop +
          dragArea.offsetHeight -
          this.actionObj.clone.offsetHeight +
          window.pageYOffset +
          (this.actionObj.clone.offsetHeight - this.heightUptoCursorPoint);
      offsetTop =
        Math.round(offsetTop / this.actionObj.cellHeight) *
        this.actionObj.cellHeight;
      if (dragArea.scrollTop > 0) {
        this.actionObj.clone.style.top = formatUnit(offsetTop);
      }
    }
    const rowIndex: number = this.parent.activeViewOptions.timeScale.enable
      ? offsetTop / this.actionObj.cellHeight
      : 0;
    const heightPerMinute: number =
      this.actionObj.cellHeight / this.actionObj.slotInterval;
    const diffInMinutes: number =
      parseInt(this.actionObj.clone.style.top, 10) - offsetTop;
    let tr: HTMLElement;
    if (this.isAllDayDrag) {
      ...
      const trCollections: HTMLTableRowElement[] = [].slice.call(
        this.parent.getContentTable().querySelectorAll('tr')
      );
    ...
    if (
      ...
      index = (
        closest(
          <HTMLTableCellElement>this.actionObj.target,
          'td'
        ) as HTMLTableCellElement
      ).cellIndex;
    }
    const colIndex: number = isNullOrUndefined(index)
      ? (<HTMLTableCellElement>closest(this.actionObj.clone, 'td')).cellIndex
      : index;
    this.actionObj.index = colIndex;
    if (isNullOrUndefined(tr)) {
      return;
    }
    const td: HTMLElement = tr.children[
    ...
    if (this.parent.activeViewOptions.timeScale.enable && !this.isAllDayDrag) {
      ...
      dragStart.setMinutes(
        dragStart.getMinutes() + diffInMinutes / heightPerMinute
      );
    ...
    const event: Record<string, any> = this.getUpdatedEvent(
      this.actionObj.start,
      this.actionObj.end,
      this.actionObj.event
    );
    const dynamicWrappers: HTMLElement[] = [].slice.call(
      this.parent.element.querySelectorAll('.e-dynamic-clone')
    );
    ...
    if (this.multiData.length > 0) {
      if (
        this.isAllDayTarget &&
        this.isAllDayDrag &&
        !isNullOrUndefined(this.actionObj.isAllDay) &&
        !this.actionObj.isAllDay
      ) {
        const targetCellTime: number = parseInt(
          closest(<HTMLElement>this.actionObj.target, 'td').getAttribute(
            'data-date'
          ),
          10
        );
        this.multiData.forEach((data: Record<string, any>) => {
          ...
          if (data[this.parent.eventFields.isAllDay]) {
            data[this.parent.eventFields.startTime] = new Date(
              (data[this.parent.eventFields.startTime] as Date).getTime() +
                (targetCellTime - this.startTime)
            );
            const startTime: Date = new Date(
              data[this.parent.eventFields.startTime] as Date
            );
            const endTime: Date = new Date(
              startTime.setMinutes(
                startTime.getMinutes() + this.actionObj.slotInterval
              )
            );
      ...
      if (this.enableCurrentViewDrag) {
        ...
        for (let i: number = 0; i < this.multiData.length; i++) {
          const eventObj: Record<string, any> = extend(
            {},
            this.multiData[parseInt(i.toString(), 10)],
            null,
            true
          ) as Record<string, any>;
          const startTime: Date = new Date(
          ...
          const dayIndex: number = this.parent.getIndexOfDate(
            renderDates,
        ...
        for (let index: number = 0; index < this.multiData.length; index++) {
          this.updatedData[parseInt(index.toString(), 10)] =
            this.updateMultipleData(
              this.multiData[parseInt(index.toString(), 10)],
              startTimeDiff
            );
          const dayIndex: number = this.getDayIndex(
            this.updatedData[parseInt(index.toString(), 10)]
          );
          if (dayIndex >= 0) {
            const isAllDay: boolean = this.updatedData[
              parseInt(index.toString(), 10)
            ][this.parent.eventFields.isAllDay] as boolean;
            const wrapper: HTMLElement = this.getEventWrapper(
              dayIndex,
              isAllDay
            );
            this.appendCloneElement(
              wrapper,
              this.actionObj.cloneElement[parseInt(index.toString(), 10)]
            );
            this.updateEventHeight(
              this.updatedData[parseInt(index.toString(), 10)],
              index,
              dayIndex
            );
  ...
  private splitEvent(event: Record<string, any>): Record<string, any>[] {
    ...
    if (
      util.resetTime(new Date(startTime.getTime())) <
      util.resetTime(new Date(endTime.getTime()))
    ) {
      let startReferenceDate: Date = util.resetTime(
        new Date(startTime.getTime())
      );
      let endReferenceDate: Date = new Date(startReferenceDate.getTime());
      for (
        let i: number = 0;
        startReferenceDate < new Date(endTime.getTime());
        i++
      ) {
        endReferenceDate = new Date(
        ...
        startReferenceDate = new Date(
          startReferenceDate.setDate(startReferenceDate.getDate() + 1)
        );
  ...
  private updateMultipleData(
    ...
    if (eventObj[this.parent.eventFields.isAllDay]) {
      ...
      eventObj[this.parent.eventFields.startTime] = util.resetTime(
        new Date(startTime.setDate(startTime.getDate() + differInDays))
      );
      eventObj[this.parent.eventFields.endTime] = util.addDays(
        eventObj[this.parent.eventFields.startTime] as Date,
        day
      );
    } else {
      eventObj[this.parent.eventFields.startTime] = new Date(
        (eventObj[this.parent.eventFields.startTime] as Date).getTime() +
          timeDifference
      );
      eventObj[this.parent.eventFields.endTime] = new Date(
        (eventObj[this.parent.eventFields.startTime] as Date).getTime() +
          endTimeDiff
      );
    }
  ...
  private getDayIndex(event: Record<string, any>): number {
    ...
    if (
      this.parent.activeViewOptions.timeScale.enable &&
      !eventObj[this.parent.eventFields.isAllDay]
    ) {
      const startHour: Date = this.parent.activeView.getStartHour();
      startDate.setMilliseconds(
        startHour.getTime() - util.resetTime(startHour).getTime()
      );
    }
    ...
    if (!this.parent.uiStateValues.isGroupAdaptive) {
      for (let i: number = 0; i < this.actionObj.groupIndex; i++) {
        if (this.verticalEvent.dateRender[parseInt(i.toString(), 10)]) {
          datesCount =
            datesCount +
            this.verticalEvent.dateRender[parseInt(i.toString(), 10)].length;
        }
      }
    }
    const indexGroup: number = this.parent.uiStateValues.isGroupAdaptive
      ? datesCount
      : this.actionObj.groupIndex;
    ...
    if (target || !this.parent.activeViewOptions.group.byDate) {
      ...
      let dayIndex: number = !this.parent.activeViewOptions.group.byDate
        ? isNullOrUndefined(index)
          ? this.actionObj.index - datesCount
          : colIndex - datesCount
        : this.parent.getIndexOfDate(
            this.verticalEvent.dateRender[parseInt(indexGroup.toString(), 10)],
            util.resetTime(
              // eslint-disable-next-line max-len
              this.parent.getDateFromElement(
                isNullOrUndefined(index)
                  ? (this.actionObj.target as HTMLElement)
                  : (this.targetTd as HTMLElement)
              )
            )
          );
      const splitEvents: Record<string, any>[] = this.splitEvent(event);
      const events: Record<string, any>[] =
        this.parent.eventBase.isAllDayAppointment(event) ||
      ...
      for (let i: number = 0; i < events.length; i++) {
        if (i > 0) {
          let filterQuery: string = `.e-day-wrapper[data-date="${(<Date>(
            util.resetTime(
              events[parseInt(i.toString(), 10)][
                this.parent.eventFields.startTime
              ]
            )
          )).getTime()}"]`;
        ...
        if (dayIndex >= 0) {
          const record: Record<string, any> = this.verticalEvent.isSpannedEvent(
            events[parseInt(i.toString(), 10)],
            dayIndex,
            indexGroup
          );
          const eStart: Date = record[
            this.verticalEvent.fields.startTime
          ] as Date;
          const eEnd: Date = record[this.verticalEvent.fields.endTime] as Date;
          let appHeight: number = this.parent.activeViewOptions.timeScale.enable
            ? this.verticalEvent.getHeight(eStart, eEnd)
            : this.actionObj.element.offsetHeight;
          let topValue: number = this.parent.activeViewOptions.timeScale.enable
            ? this.verticalEvent.getTopValue(eStart, dayIndex, indexGroup)
            : this.actionObj.element.offsetTop;
          if (isNullOrUndefined(index)) {
            ...
            if (event[this.parent.eventFields.isAllDay]) {
              topValue = (<HTMLElement>(
                this.parent.element.querySelector('.' + cls.ALLDAY_ROW_CLASS)
              )).offsetTop;
              appHeight = this.getAllDayEventHeight();
              const timeDiff: number =
                (event[this.parent.eventFields.endTime] as Date).getTime() -
                (event[this.parent.eventFields.startTime] as Date).getTime();
              const allDayDifference: number = Math.ceil(
            ...
            if (this.actionObj.cloneElement[parseInt(index.toString(), 10)]) {
              if (i === 0) {
                this.actionObj.cloneElement[
                  parseInt(index.toString(), 10)
                ].style.top = formatUnit(topValue);
                this.actionObj.cloneElement[
                  parseInt(index.toString(), 10)
                ].style.height = formatUnit(appHeight);
                this.actionObj.cloneElement[
                  parseInt(index.toString(), 10)
                ].style.width = formatUnit(appWidth);
                this.actionObj.cloneElement[
                  parseInt(index.toString(), 10)
                ].style.left = formatUnit(0);
              } else {
                this.renderSpannedEvents(
                  record,
                  dynamicIndex,
  ...
  private renderSpannedEvents(
    ...
    const startTime: number = (
      record[this.parent.eventFields.startTime] as Date
    ).getTime();
    const endTime: number = (
      record[this.parent.eventFields.endTime] as Date
    ).getTime();
    if (startTime !== endTime) {
      const appointmentElement: HTMLElement =
        this.verticalEvent.createAppointmentElement(
          record,
          false,
          record.isSpanned as Record<string, any>,
          this.actionObj.groupIndex
        );
      addClass(
        [appointmentElement],
        [cls.CLONE_ELEMENT_CLASS, 'e-dynamic-clone']
      );
      ...
      const appointmentWrap: HTMLElement[] = [].slice.call(
        this.parent.element.querySelectorAll(
          '.' + cls.APPOINTMENT_WRAPPER_CLASS
        )
      );
      appointmentWrap[parseInt(index.toString(), 10)].appendChild(
        appointmentElement
      );
    }
  }
  ...
  private getRenderedDates(): Date[] {
    ...
    this.parent.eventBase.slots.push(
      ...this.parent.activeView.renderDates.map((date: Date) => +date)
    );
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      this.parent.eventBase.slots = [];
      const resources: TdData[] =
        this.parent.resourceBase.lastResourceLevel.filter(
          (res: TdData) => res.groupIndex === this.actionObj.groupIndex
        );
      renderDates = resources[0].renderDates;
      this.parent.eventBase.slots.push(
        ...renderDates.map((date: Date) => +date)
      );
    }
    return renderDates;
  }
  ...
  private updateAllDayEvents(
    ...
    const renderDates: Date[] = this.getRenderedDates();
    const events: Record<string, any>[] = this.parent.eventBase.splitEvent(
      event,
      renderDates
    );
    let query: string = `.e-all-day-cells[data-date="${(<Date>(
      events[0][this.parent.eventFields.startTime]
    )).getTime()}"]`;
    if (
      this.parent.activeViewOptions.group.resources.length > 0 &&
      !this.parent.uiStateValues.isGroupAdaptive
    ...
    const cell: HTMLElement[] = [].slice.call(
      this.parent.element.querySelectorAll(query)
    );
    if (cell.length > 0 || !isNullOrUndefined(colIndex)) {
      const cellIndex: number = !isNullOrUndefined(colIndex)
        ? colIndex
        : (cell[0] as HTMLTableCellElement).cellIndex;
      this.appendCloneElement(this.getEventWrapper(cellIndex));
      // eslint-disable-next-line max-len
      this.actionObj.clone.style.width = formatUnit(
        ((<Record<string, any>>events[0].data).count as number) *
          this.actionObj.cellWidth
      );
    }
  }
  ...
  private swapDragging(e: MouseEvent & TouchEvent): void {
    ...
    const colIndex: number =
      !isNullOrUndefined(
        closest(<HTMLTableCellElement>this.actionObj.target, 'td')
      ) &&
      (
        closest(
          <HTMLTableCellElement>this.actionObj.target,
    ...
    if (
      closest(
        this.actionObj.target as Element,
        '.' + cls.DATE_HEADER_WRAP_CLASS
      ) &&
      ...
      const allDayElement: HTMLElement[] = [].slice.call(
        this.parent.element.querySelectorAll(
          '.' + cls.ALLDAY_CELLS_CLASS + ':first-child'
        )
      );
    ...
    if (
      closest(this.actionObj.target as Element, '.' + cls.WORK_CELLS_CLASS) &&
      ...
      // eslint-disable-next-line max-len
      const height: number =
        this.actionObj.element.offsetHeight === 0
          ? this.actionObj.height
          : this.actionObj.element.offsetHeight;
      setStyleAttribute(this.actionObj.clone, {
        left: formatUnit(0),
  ...
  private calculateVerticalDate(e: MouseEvent & TouchEvent): void {
    if (
      isNullOrUndefined(e.target) ||
      (e.target &&
        isNullOrUndefined(closest(<HTMLTableCellElement>e.target, 'tr'))) ||
    ...
    const eventDuration: number =
      util.getUniversalTime(<Date>eventObj[this.parent.eventFields.endTime]) -
      util.getUniversalTime(<Date>eventObj[this.parent.eventFields.startTime]);
    let td: HTMLTableCellElement = closest(
      <HTMLTableCellElement>this.actionObj.target,
      'td'
    ) as HTMLTableCellElement;
    if (!isNullOrUndefined(td)) {
      ...
      if (!isNullOrUndefined(currentDate)) {
        ...
        const timeString: Date = new Date(
          currentDate.setDate(currentDate.getDate() - this.daysVariation)
        );
        const dragStart: Date = new Date(timeString.getTime());
        const startTimeDiff: number =
          util.getUniversalTime(
            <Date>eventObj[this.parent.eventFields.startTime]
          ) -
          util.getUniversalTime(
            util.resetTime(
              new Date(+eventObj[this.parent.eventFields.startTime])
            )
          );
    ...
    if (isNullOrUndefined(this.parent.eventDragArea)) {
      const eventWrappers: HTMLElement[] = [].slice.call(
        this.parent.element.querySelectorAll('.' + cls.CLONE_ELEMENT_CLASS)
      );
    ...
    if (this.multiData && this.multiData.length > 0) {
      const startTime: Date = util.resetTime(
        new Date(event[this.parent.eventFields.startTime] as Date)
      );
      let startTimeDiff: number = startTime.getTime() - this.startTime;
      if (
        this.parent.currentView === 'TimelineYear' &&
        this.parent.group.resources.length > 0
      ) {
        startTimeDiff =
          (startTime.getFullYear() - new Date(this.startTime).getFullYear()) *
          12;
      ...
      for (let index: number = 0; index < this.multiData.length; index++) {
        this.updatedData[parseInt(index.toString(), 10)] =
          this.updateMultipleVerticalDate(
            this.multiData[parseInt(index.toString(), 10)],
            startTimeDiff
          );
        if (this.parent.currentView === 'TimelineYear') {
          this.dynamicYearlyEventsRendering(
            this.updatedData[parseInt(index.toString(), 10)]
          );
        } else {
          this.dynamicEventsRendering(
            this.updatedData[parseInt(index.toString(), 10)]
          );
        }
      }
  ...
  private updateMultipleVerticalDate(
    ...
    if (
      ...
      eventObj[this.parent.eventFields.startTime] = new Date(
        startDate.setMonth(startDate.getMonth() + timeDifference)
      );
    } else {
      const differInDays: number = Math.ceil(timeDifference / util.MS_PER_DAY);
      eventObj[this.parent.eventFields.startTime] = new Date(
        startDate.setDate(startDate.getDate() + differInDays)
      );
    }
    eventObj[this.parent.eventFields.endTime] = new Date(
      (eventObj[this.parent.eventFields.startTime] as Date).getTime() +
  ...
  private calculateTimelineTime(e: MouseEvent & TouchEvent): void {
    ...
    const eventDuration: number =
      util.getUniversalTime(<Date>eventObj[this.parent.eventFields.endTime]) -
      util.getUniversalTime(<Date>eventObj[this.parent.eventFields.startTime]);
    let offsetLeft: number = this.parent.enableRtl
      ? Math.abs(this.actionObj.clone.offsetLeft) -
        this.actionObj.clone.offsetWidth
      : parseInt(this.actionObj.clone.style.left, 10);
    offsetLeft =
      Math.round(offsetLeft / this.actionObj.cellWidth) *
      this.actionObj.cellWidth;
    ...
    const contentWrapRight: number = dragArea.getBoundingClientRect().right;
    const cursorDrag: boolean =
      this.parent.activeView.isTimelineView() &&
      !this.parent.enableRtl &&
      this.actionObj.pageX >
        this.actionObj.clone.getBoundingClientRect().right &&
      !this.isMorePopupOpened &&
      !(this.actionObj.pageX > contentWrapRight);
    const leftVal: number = this.parent.eventDragArea
      ? dragArea.scrollLeft - dragArea.offsetLeft
      : 0;
    if ((this.isCursorAhead || cursorDrag) && !this.isStepDragging) {
      this.isCursorAhead = true;
    }
    let cloneIndex: number = Math.floor(
      (this.actionObj.pageX -
        this.actionObj.clone.getBoundingClientRect().left +
        leftVal) /
        this.actionObj.cellWidth
    ...
    const tr: HTMLTableRowElement = this.parent
      .getContentTable()
      .querySelector('tr') as HTMLTableRowElement;
    let index: number = this.getCursorCurrentIndex(colIndex, cloneIndex, tr);
    index = index < 0 ? 0 : index;
    let eventStart: Date = this.isHeaderRows
      ? new Date(
          this.timelineEventModule.dateRender[
            parseInt(index.toString(), 10)
          ].getTime()
        )
      : this.parent.getDateFromElement(
          <HTMLElement>tr.children[parseInt(index.toString(), 10)]
        );
    if (this.isStepDragging) {
      ...
      if (widthDiff !== 0) {
        ...
        eventStart.setMinutes(
          eventStart.getMinutes() + timeDiff * this.actionObj.interval
        );
        if (this.isCursorAhead || cursorDrag) {
          ...
          const intervalInMS: number =
            this.actionObj.interval * util.MS_PER_MINUTE;
          timeDiff =
            Math.abs(eventStart.getTime() - this.actionObj.start.getTime()) /
            intervalInMS;
          ...
          if (roundTimeDiff !== timeDiff) {
            timeDiff =
              roundTimeDiff *
              intervalInMS *
              (eventStart > this.actionObj.start ? 1 : -1);
      ...
      if (this.isCursorAhead || cursorDrag) {
        const minutes: number = this.isTimelineDayProcess
          ? MINUTES_PER_DAY
          : this.actionObj.slotInterval;
        eventStart.setMinutes(eventStart.getMinutes() + minutes);
        eventStart.setMilliseconds(-eventDuration);
        if (
          eventStart.getTime() === util.resetTime(eventStart).getTime() &&
    ...
    if (this.parent.eventDragArea) {
      const targetDate: Date = this.parent.getDateFromElement(
    ...
    let eventsData: Record<string, any>[] = [
      this.getUpdatedEvent(eventStart, eventEnd, this.actionObj.event),
    ];
    if (this.multiData.length > 0) {
      const startTimeDiff: number =
        (eventsData[0][this.parent.eventFields.startTime] as Date).getTime() -
        this.startTime;
      for (let i: number = 0; i < this.multiData.length; i++) {
        this.updatedData[parseInt(i.toString(), 10)] = this.updateMultipleData(
          this.multiData[parseInt(i.toString(), 10)],
          startTimeDiff
        );
      }
    ...
    for (
      ...
      dataIndex < eventsData.length;
      dataIndex++
    ) {
      const cloneElement: HTMLElement =
        this.multiData.length > 0
          ? this.actionObj.cloneElement[parseInt(dataIndex.toString(), 10)]
          : this.actionObj.clone;
      if (isNullOrUndefined(this.parent.eventDragArea)) {
        const events: Record<string, any>[] =
          this.timelineEventModule.splitEvent(
            eventsData[parseInt(dataIndex.toString(), 10)],
            this.timelineEventModule.dateRender
          );
        const eventData: Record<string, any> = events[0].data as Record<
          string,
          any
        >;
        const startTime: Date = this.timelineEventModule.getStartTime(
          events[0],
          eventData
        );
        const endTime: Date = this.timelineEventModule.getEndTime(
          events[0],
          eventData
        );
        // eslint-disable-next-line max-len
        const width: number = this.timelineEventModule.getEventWidth(
          startTime,
          endTime,
          eventObj[this.parent.eventFields.isAllDay] as boolean,
          eventData.count as number
        );
        // eslint-disable-next-line max-len
        let day: number = this.parent.getIndexOfDate(
          this.timelineEventModule.dateRender,
          util.resetTime(new Date(startTime.getTime()))
        );
        day = day < 0 ? 0 : day;
        const left: number = this.timelineEventModule.getPosition(
          startTime,
          endTime,
          eventObj[this.parent.eventFields.isAllDay] as boolean,
          day
  ...
  private getOffsetValue(offsetLeft: number, rightOffset: number): number {
    if (this.scrollEdges.left || this.scrollEdges.right) {
      const viewEle: HTMLElement = this.parent.element.querySelector(
        '.' + cls.CONTENT_WRAP_CLASS
      ) as HTMLElement;
      if (this.parent.enableRtl) {
  ...
  private getWidthDiff(tr: HTMLTableRowElement, index: number): number {
    const pages: ClientRect | DOMRect =
      this.scrollArgs.element.getBoundingClientRect();
    if (
      pages.left <= this.actionObj.pageX &&
      pages.right >= this.actionObj.pageX
    ) {
      const targetLeft: number = (<HTMLElement>(
        tr.children[parseInt(index.toString(), 10)]
      )).offsetLeft;
      ...
      if (this.parent.enableRtl) {
        return (
          targetLeft +
          this.actionObj.cellWidth -
          (this.scrollArgs.element.scrollLeft + pageX)
        );
      } else {
  ...
  private getCursorCurrentIndex(
    ...
    if (this.isHeaderRows) {
      const dateLength: number = Math.floor(
        tr.offsetWidth / this.actionObj.cellWidth
      );
  ...
  private cursorIndex(
    ...
    const td: HTMLElement = <HTMLElement>(
      closest(e.target as Element, '.e-work-cells')
    );
    if (!isNullOrUndefined(td) && !this.isMorePopupOpened) {
      let targetDate: Date = this.parent.getDateFromElement(td);
      if (this.isHeaderRows) {
        const currentIndex: number = Math.floor(
          left / this.actionObj.cellWidth
        );
        targetDate = new Date(
          this.timelineEventModule.dateRender[currentIndex + index].getTime()
        );
      }
      const timeDiff: number =
        targetDate.getTime() -
      ...
      if (this.isTimelineDayProcess) {
        ...
        const widthDiff: number = Math.floor(
          timeDiff /
            util.MS_PER_MINUTE /
            (this.actionObj.slotInterval / this.actionObj.cellWidth)
        );
        this.cursorPointIndex = Math.floor(
          widthDiff / this.actionObj.cellWidth
        );
        this.cursorPointIndex =
          this.cursorPointIndex < 0 ? 0 : this.cursorPointIndex;
      }
    } else {
  ...
  private calculateResourceGroupingPosition(
    ...
    const trCollection: HTMLElement[] = [].slice.call(
      this.parent.element.querySelectorAll(
        '.e-content-wrap .e-content-table tr:not(.e-hidden)'
      )
    );
    let translateY: number = util.getTranslateY(
      dragArea.querySelector('table')
    );
    translateY = isNullOrUndefined(translateY) ? 0 : translateY;
    const rowHeight: number = this.parent.rowAutoHeight
      ? ~~(dragArea.querySelector('table').offsetHeight / trCollection.length)
      : this.actionObj.cellHeight;
    let rowIndex: number = Math.floor(
      Math.floor(
        this.actionObj.Y +
          ...
          util.getElementTop(dragArea, this.parent.uiStateValues.isTransformed)
      ) / rowHeight
    );
    rowIndex =
      rowIndex < 0
        ? 0
        : rowIndex > trCollection.length - 1
        ? trCollection.length - 1
        : rowIndex;
    this.actionObj.index = rowIndex;
    const eventContainer: Element = this.parent.element
      .querySelectorAll('.e-appointment-container:not(.e-hidden)')
      .item(rowIndex);
    let eventWrapper: HTMLElement = eventContainer.querySelector(
      '.' + cls.APPOINTMENT_WRAPPER_CLASS
    ) as HTMLElement;
    if (!eventWrapper) {
      eventWrapper = createElement('div', {
    ...
    const td: HTMLTableCellElement = closest(
      <HTMLTableCellElement>this.actionObj.target,
      'td'
    ) as HTMLTableCellElement;
    this.actionObj.groupIndex =
      td && !isNaN(parseInt(td.getAttribute('data-group-index'), 10))
        ? parseInt(td.getAttribute('data-group-index'), 10)
        : this.actionObj.groupIndex;
    if (!isNullOrUndefined(this.parent.eventDragArea)) {
      return;
    }
    let top: number =
      this.parent.getElementHeight(
        <HTMLElement>trCollection[parseInt(rowIndex.toString(), 10)]
      ) * rowIndex;
    if (this.parent.rowAutoHeight) {
      const cursorElement: HTMLElement = this.getCursorElement(e);
      if (cursorElement) {
        top = cursorElement.classList.contains(cls.WORK_CELLS_CLASS)
          ? cursorElement.offsetTop
          : cursorElement.offsetParent &&
            cursorElement.offsetParent.classList.contains(cls.APPOINTMENT_CLASS)
          ? (cursorElement.offsetParent as HTMLElement).offsetTop
          : top;
      }
    }
  ...
  private appendCloneElement(
    ...
    cloneElement = isNullOrUndefined(cloneElement)
      ? this.actionObj.clone
      : cloneElement;
    const dragElement: HTMLElement = document.querySelector(
      this.parent.eventDragArea
  ...
  private getEventWrapper(index: number, isAllDayDrag?: boolean): HTMLElement {
    ...
    if (isNullOrUndefined(isAllDayDrag)) {
      isAllDayDrag = this.actionObj.clone.classList.contains(
        cls.ALLDAY_APPOINTMENT_CLASS
      );
    }
    if (this.parent.activeViewOptions.timeScale.enable) {
      const wrapperClass: string = isAllDayDrag
        ? '.' + cls.ALLDAY_APPOINTMENT_WRAPPER_CLASS
        : '.' + cls.APPOINTMENT_WRAPPER_CLASS;
      eventWrapper = this.parent.element
        .querySelectorAll(wrapperClass)
        .item(index) as HTMLElement;
    } else {
      const targetWrapper: HTMLElement = this.parent.element
        .querySelectorAll('.' + cls.WORK_CELLS_CLASS)
        .item(index) as HTMLElement;
      eventWrapper = targetWrapper.querySelector(
        '.' + cls.APPOINTMENT_WRAPPER_CLASS
      ) as HTMLElement;
      if (!eventWrapper) {
        eventWrapper = createElement('div', {
  ...
  private getAllDayEventHeight(): number {
    ...
    this.parent.element
      .querySelector('.' + cls.ALLDAY_APPOINTMENT_WRAPPER_CLASS)
      .appendChild(eventWrapper);
