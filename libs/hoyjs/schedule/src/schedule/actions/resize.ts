...
export class Resize extends ActionBase {
  public wireResizeEvent(element: HTMLElement): void {
    const resizeElement: HTMLElement[] = [].slice.call(
      element.querySelectorAll('.' + cls.EVENT_RESIZE_CLASS)
    );
  ...
  private resizeHelper(): void {
    if (
      ...
      for (
        let i: number = 0, len: number = this.actionObj.originalElement.length;
        i < len;
        i++
      ) {
        const cloneElement: HTMLElement = this.createCloneElement(
          this.actionObj.originalElement[parseInt(i.toString(), 10)]
        );
  ...
  public resizeStart(e: MouseEvent & TouchEvent): void {
    ...
    const resizeTarget: HTMLElement = closest(
      e.target as Element,
      '.' + cls.EVENT_RESIZE_CLASS
    ) as HTMLElement;
    this.actionObj.element = closest(
      resizeTarget,
    ...
    this.parent.trigger(
      ...
      (resizeEventArgs: ResizeEventArgs) => {
        ...
        this.actionObj.groupIndex = this.parent.uiStateValues.isGroupAdaptive
        ...
        const hRows: string[] = this.parent.activeViewOptions.headerRows.map(
          (row: Record<string, any>) => row.option as string
        );
        if (
          this.parent.activeView.isTimelineView() &&
          hRows.length > 0 &&
          ['Date', 'Hour'].indexOf(hRows.slice(-1)[0]) < 0
        ) {
        ...
        const pages: (MouseEvent & TouchEvent) | Touch =
        ...
        if (['Month', 'TimelineYear'].indexOf(this.parent.currentView) < 0) {
          const scrollWidth: number =
            Math.round(this.scrollArgs.width / this.actionObj.cellWidth) *
            this.actionObj.cellWidth;
          this.scrollArgs.width =
            this.scrollArgs.width < scrollWidth
              ? scrollWidth
              : this.scrollArgs.width;
        }
  ...
  private resizing(e: MouseEvent & TouchEvent): void {
    ...
    if (
      this.parent.element.querySelectorAll('.' + cls.RESIZE_CLONE_CLASS)
        .length === 0
    ) {
      this.resizeHelper();
    }
  ...
  public updateResizingDirection(e: MouseEvent & TouchEvent): void {
    ...
    if (this.resizeEdges.left) {
      if (resizeValidation) {
        const leftStyles: Record<string, any> = this.getLeftRightStyles(
          e,
          true
        );
    ...
    if (this.resizeEdges.right) {
      if (resizeValidation) {
        const rightStyles: Record<string, any> = this.getLeftRightStyles(
          e,
          false
        );
    ...
    if (this.resizeEdges.bottom) {
      if (resizeValidation) {
        const bottomStyles: Record<string, any> = this.getTopBottomStyles(
          e,
          false
        );
  ...
  private monthResizing(): void {
    ...
    if (
      isNullOrUndefined(this.actionObj.pageX) ||
      isNullOrUndefined(this.actionObj.pageY)
    ) {
      return;
    }
    const td: HTMLTableCellElement = document.elementFromPoint(
      this.actionObj.pageX,
      this.actionObj.pageY
    ...
    let startTime: Date = new Date(
      (<Date>this.actionObj.event[this.parent.eventFields.startTime]).getTime()
    );
    let endTime: Date = new Date(
      (<Date>this.actionObj.event[this.parent.eventFields.endTime]).getTime()
    );
    if (
      (!this.parent.enableRtl && this.resizeEdges.left) ||
      (this.parent.enableRtl && this.resizeEdges.right) ||
  ...
  private yearEventsRendering(event: Record<string, any>): void {
    ...
    let endDate: Date = new Date(
      event[this.parent.eventFields.endTime] as Date
    );
    let monthDiff: number = 0;
    if (this.parent.activeViewOptions.group.resources.length === 0) {
      monthDiff = this.getMonthDiff(
        event[this.parent.eventFields.startTime] as Date,
        util.addDays(endDate, -1)
      );
    }
    for (let i: number = 0; i <= monthDiff; i++) {
      let eventObj: Record<string, any>;
      if (this.parent.activeViewOptions.group.resources.length === 0) {
        eventObj = this.getEventCount(
          event,
          this.actionObj.start.getMonth() + i
        );
      } else {
        eventObj = extend({}, event, null, true) as Record<string, any>;
        endDate =
          this.resizeEdges.left || this.resizeEdges.right
            ? util.addDays(endDate, -1)
            : endDate;
        eventObj.count =
          this.getMonthDiff(
  ...
  private getEventCount(
    ...
    const eventStart: Date = eventData[
    ...
    const monthEnd: Date = util.addDays(
      new Date(this.parent.selectedDate.getFullYear(), month + 1, 0),
      1
    );
    let count: number = 1;
    ...
    if (this.parent.activeViewOptions.group.resources.length === 0) {
      count = Math.ceil(
        ((eventData[this.parent.eventFields.endTime] as Date).getTime() -
          (eventData[this.parent.eventFields.startTime] as Date).getTime()) /
          util.MS_PER_DAY
      );
    }
        ...
        if (this.parent.isSpecificResourceEvents()) {
          this.parent.crudModule.crudObj.sourceEvent = [
            this.parent.resourceBase.lastResourceLevel[
              parseInt(
                resizeEventArgs.element.getAttribute('data-group-index'),
                10
              )
            ],
          ];
  ...
  private verticalResizing(isTop: boolean): void {
    ...
    const minutes: number =
      (offsetValue / this.actionObj.cellHeight) * this.actionObj.slotInterval;
    const element: Element = this.actionObj.clone.offsetParent;
    if (isNullOrUndefined(element)) {
      return;
    }
    const resizeTime: Date = util.resetTime(
      this.parent.getDateFromElement(element)
    );
    resizeTime.setHours(this.parent.activeView.getStartHour().getHours());
    resizeTime.setMinutes(
      minutes + this.parent.activeView.getStartHour().getMinutes()
    );
    if (isTop) {
  ...
  private horizontalResizing(isLeft: boolean): void {
    const eventStart: Date = new Date(
      (<Date>this.actionObj.event[this.parent.eventFields.startTime]).getTime()
    );
    ...
    const isTimeViews: boolean =
      ['TimelineDay', 'TimelineWeek', 'TimelineWorkWeek'].indexOf(
        this.parent.currentView
      ) > -1;
    const isTimelineMonth: boolean =
      this.parent.currentView === 'TimelineMonth';
    const isWithoutScale: boolean =
      isTimelineMonth ||
      (isTimeViews && !this.parent.activeViewOptions.timeScale.enable);
    if (this.parent.activeView.isTimelineView()) {
      const tr: HTMLTableRowElement = this.parent
        .getContentTable()
        .querySelector('tr') as HTMLTableRowElement;
      if (this.parent.activeViewOptions.headerRows.length > 0) {
        const rows: string[] = this.parent.activeViewOptions.headerRows.map(
          (row: Record<string, any>) => row.option as string
        );
      ...
      let offset: number = parseFloat(
        this.parent.enableRtl
          ? this.actionObj.clone.style.right
      ...
      if (['Year', 'Month', 'Week', 'Date'].indexOf(headerName) !== -1) {
        ...
        let offsetValue: number = this.parent.enableRtl
          ? parseInt(this.actionObj.clone.style.right, 10)
          : parseInt(this.actionObj.clone.style.left, 10);
        offsetValue =
          Math.round(offsetValue / this.actionObj.cellWidth) *
          this.actionObj.cellWidth;
        if (!isLeft) {
          offsetValue +=
            this.parent.getElementWidth(this.actionObj.clone) -
            this.actionObj.cellWidth;
        }
        cellIndex = !isTimelineMonth
          ? Math.round(
              offsetValue / (this.parent.getElementWidth(tr) / noOfDays)
            )
          : Math.floor(
              offsetValue /
                Math.floor(this.parent.getElementWidth(tr) / noOfDays)
            );
        ...
        cellIndex =
          cellIndex < 0 ? 0 : cellIndex >= noOfDays ? noOfDays - 1 : cellIndex;
      } else {
        const cellWidth: number = this.actionObj.cellWidth;
        cellIndex = isLeft
          ? Math.floor(offset / this.actionObj.cellWidth)
          : Math.ceil(
              (offset +
                (this.parent.getElementWidth(this.actionObj.clone) -
                  cellWidth)) /
      ...
      if (['Year', 'Month', 'Week', 'Date'].indexOf(headerName) !== -1) {
        ...
        resizeDate = this.parent.getDateFromElement(
          <HTMLElement>tr.children[parseInt(cellIndex.toString(), 10)]
        );
      }
      if (
        ['TimelineMonth', 'Year', 'Month', 'Week', 'Date'].indexOf(
          headerName
        ) !== -1 ||
        !this.parent.activeViewOptions.timeScale.enable
      ) {
        resizeTime = new Date(
          resizeDate.setHours(
            resizeTime.getHours(),
            resizeTime.getMinutes(),
            resizeTime.getSeconds()
          )
        );
      } else {
        if (!isLeft) {
        ...
        let spanMinutes: number = Math.ceil(
        ...
        spanMinutes =
          isLastCell || (!isLeft && spanMinutes === 0)
            ? this.actionObj.slotInterval
            : spanMinutes;
      ...
      const cloneIndex: number = (
        closest(this.actionObj.clone, 'td') as HTMLTableCellElement
      ).cellIndex;
      const originalWidth: number =
        Math.ceil(
          (isLeft ? this.parent.getElementWidth(this.actionObj.element) : 0) /
            this.actionObj.cellWidth
        ) * this.actionObj.cellWidth;
      const noOfDays: number = Math.ceil(
        (this.parent.getElementWidth(this.actionObj.clone) - originalWidth) /
          this.actionObj.cellWidth
      );
      const tr: HTMLTableRowElement = closest(
        this.actionObj.clone,
        'tr'
      ) as HTMLTableRowElement;
      let dayIndex: number = isLeft
        ? cloneIndex - noOfDays
        : cloneIndex + noOfDays - 1;
      dayIndex = this.getIndex(dayIndex);
      resizeTime = this.parent.getDateFromElement(
        <HTMLElement>tr.children[parseInt(dayIndex.toString(), 10)]
      );
      if (isLeft) {
        resizeTime.setHours(
          eventStart.getHours(),
    ...
    const isNotHourSlot: boolean =
      ['TimelineMonth', 'Year', 'Month', 'Week', 'Date'].indexOf(headerName) !==
        -1 || !this.parent.activeViewOptions.timeScale.enable;
    if (isLeft) {
      if (eventEnd.getTime() - resizeTime.getTime() <= 0) {
        resizeTime = isWithoutScale ? util.resetTime(eventEnd) : eventStart;
      }
      this.actionObj.start = !isNotHourSlot
        ? this.calculateIntervalTime(resizeTime)
        : resizeTime;
    } else {
      const isTimeScaleViews: boolean =
        isTimeViews && this.parent.activeViewOptions.timeScale.enable;
      let resizeEnd: Date =
        (!isTimeScaleViews ||
          isDateHeader ||
          (isTimeViews &&
            ['Week', 'Month', 'Year'].indexOf(headerName) > -1)) &&
        resizeTime.getHours() === 0 &&
        resizeTime.getMinutes() === 0
          ? util.addDays(resizeTime, 1)
          : resizeTime;
      if (isWithoutScale && resizeEnd.getTime() - eventStart.getTime() <= 0) {
        resizeEnd = util.addDays(util.resetTime(eventStart), 1);
      }
      this.actionObj.end = !isNotHourSlot
        ? this.calculateIntervalTime(resizeEnd)
        : resizeEnd;
    }
  }
  ...
  private getTopBottomStyles(
    ...
    let clnHeight: number = isTop
      ? this.actionObj.element.offsetHeight +
        (this.actionObj.Y - this.actionObj.pageY)
      : this.actionObj.element.offsetHeight +
        (this.actionObj.pageY - this.actionObj.Y);
    let clnTop: number = isTop
      ? this.actionObj.element.offsetTop -
        (this.actionObj.Y - this.actionObj.pageY)
      : this.actionObj.clone.offsetTop;
    clnHeight =
      clnTop < 0
        ? this.actionObj.clone.offsetHeight
        : this.actionObj.clone.offsetTop + this.actionObj.clone.offsetHeight >
          this.scrollArgs.height
        ? this.actionObj.clone.offsetHeight
        : clnHeight;
    clnTop = clnTop < 0 ? 0 : clnTop;
    clnTop = isTop ? Math.floor(clnTop / slotInterval) * slotInterval : clnTop;
    clnHeight =
      clnTop + clnHeight >= viewElement.scrollHeight
        ? viewElement.scrollHeight - clnTop
        : Math.ceil(clnHeight / slotInterval) * slotInterval;
    if (
      !isTop &&
      this.actionObj.clone.offsetTop + clnHeight >=
        this.parent.getContentTable().offsetHeight
    ) {
      clnHeight =
        this.parent.getContentTable().offsetHeight -
        this.actionObj.clone.offsetTop;
    ...
    const styles: Record<string, any> = {
      height: formatUnit(
        clnHeight < this.actionObj.cellHeight
          ? Math.floor(clnHeight / slotInterval) * slotInterval
          : clnHeight
      ),
      top: formatUnit(
        clnHeight < this.actionObj.cellHeight && isTop
          ? Math.ceil(clnTop / slotInterval) * slotInterval
          : clnTop
      ),
      left: '0px',
      right: '0px',
  ...
  private getLeftRightStyles(
    ...
    const slotInterval: number =
      ...
      this.actionObj.interval;
    const pageWidth: number = isLeft
      ? this.actionObj.X - this.actionObj.pageX
      : this.actionObj.pageX - this.actionObj.X;
    const targetWidth: number = isTimelineView
      ? (this.parent.getElementWidth(this.actionObj.element) /
          this.actionObj.cellWidth) *
        this.actionObj.cellWidth
      : this.parent.currentView === 'Month'
      ? this.parent.getElementWidth(this.actionObj.element)
    ...
    let offsetWidth: number =
      targetWidth +
      Math.ceil(pageWidth / this.actionObj.cellWidth) *
        this.actionObj.cellWidth;
    const left: number = this.parent.enableRtl
      ? parseInt(this.actionObj.element.style.right, 10)
      : this.actionObj.clone.offsetLeft;
    if (isTimeViews) {
      offsetWidth =
        targetWidth + Math.ceil(pageWidth / slotInterval) * slotInterval;
      if (!isLeft) {
        const roundedLeft: string = (+parseFloat(
          this.actionObj.element.style[this.parent.enableRtl ? 'right' : 'left']
        )).toFixed(1);
        const roundedWidth: number =
          Math.round(targetWidth / slotInterval) * slotInterval;
        if (roundedLeft !== left.toFixed(1) || roundedWidth !== targetWidth) {
          offsetWidth =
            Math.round((left + offsetWidth) / slotInterval) * slotInterval -
            left;
        }
      }
    ...
    let width: number =
      !isLeft &&
      offsetWidth + this.actionObj.clone.offsetLeft > this.scrollArgs.width
        ? this.parent.getElementWidth(this.actionObj.clone)
    ...
    if (this.parent.enableRtl) {
      ...
      if (isTimelineView && !isLeft) {
        rightValue =
          rightValue -
          Math.ceil((this.actionObj.pageX - this.actionObj.X) / slotInterval) *
            slotInterval;
        if (rightValue < 0) {
      ...
      rightValue =
        rightValue >= this.scrollArgs.width
          ? this.scrollArgs.width - this.actionObj.cellWidth
          : rightValue;
      styles.right = formatUnit(rightValue);
      width =
        width + rightValue > this.scrollArgs.width
          ? this.parent.getElementWidth(this.actionObj.clone)
          : width;
    } else {
      let offsetLeft: number = isLeft
      ...
      offsetLeft = isTimelineView
        ? isTimeViews
          ? isLeft
            ? this.actionObj.element.offsetLeft -
              Math.ceil(
                (this.actionObj.element.offsetLeft - offsetLeft) / slotInterval
              ) *
                slotInterval
            : offsetLeft
          : Math.floor(offsetLeft / this.actionObj.cellWidth) *
            this.actionObj.cellWidth
        : Math.ceil(Math.abs(offsetLeft) / this.actionObj.cellWidth) *
          this.actionObj.cellWidth;
      if (offsetLeft < 0) {
        ...
        styles.left = formatUnit(
          isTimelineView
            ...
            ? leftValue < 0
              ? -offsetLeft
              : Math.ceil(
                  (targetWidth - cloneWidth) / this.actionObj.cellWidth
                ) * this.actionObj.cellWidth
            : offsetLeft
        );
      }
  ...
  private resizeValidation(e: MouseEvent & TouchEvent): boolean {
    ...
    if (this.resizeEdges.left) {
      if (
        pages.pageX < viewDimension.leftOffset &&
        pages.pageX >= viewDimension.left &&
        isTimeScale
      ) {
        cellWidth = 0;
      }
    ...
    if (this.resizeEdges.right) {
      if (
        pages.pageX > viewDimension.rightOffset &&
        pages.pageX <= viewDimension.right &&
        isTimeScale
      ) {
        cellWidth = 0;
      }
