...
import { isNullOrUndefined, addClass, removeClass } from '@hoyui/base';
...
export class KeyboardInteraction {
  ...
  constructor(parent: Schedule) {
    this.parent = parent;
    this.parent.element.tabIndex =
      this.parent.element.tabIndex === -1 ? 0 : this.parent.element.tabIndex;
    ...
    switch (e.action) {
      ...
      case 'ctrlLeftArrow':
        this.parent.changeDate(
          this.parent.activeView.getNextPreviousDate('previous'),
          e
        );
        if (this.parent.headerModule) {
          (
            this.parent.headerModule.element.querySelector(
              '.e-prev button'
            ) as HTMLElement
          ).focus();
        }
      ...
      case 'ctrlRightArrow':
        this.parent.changeDate(
          this.parent.activeView.getNextPreviousDate('next'),
          e
        );
        if (this.parent.headerModule) {
          (
  ...
  private processShiftAltN(e: Event): void {
    ...
    let cellData: CellClickEventArgs = <CellClickEventArgs>(
      extend({}, null, true)
    );
    if (
      selectedCells.length > 0 &&
      (closest(target, '.' + cls.WORK_CELLS_CLASS) ||
        closest(target, '.' + cls.ALLDAY_CELLS_CLASS) ||
        closest(target, '.' + cls.HEADER_CELLS_CLASS))
    ) {
      cellData = this.getSelectedElements(target);
    } else if (
      closest(target, '.' + cls.APPOINTMENT_CLASS) &&
      !isNullOrUndefined(this.parent.activeEventData.event)
    ) {
      const event: { StartTime: Date; EndTime: Date; IsAllDay: boolean } = this
        .parent.activeEventData.event as {
        StartTime: Date;
        EndTime: Date;
        IsAllDay: boolean;
      };
      ...
      const workHour: Date = this.parent.getStartEndTime(
      ...
      cellData.startTime.setHours(
        workHour.getHours(),
        workHour.getMinutes(),
        0,
        0
      );
      cellData.endTime = new Date(
        cellData.startTime.getTime() + slotInterval * 60000
      );
    }
    const args: CellClickEventArgs = <CellClickEventArgs>(
      extend(cellData, { cancel: false, event: e })
    );
    if (args != null) {
  ...
  private processFTwelve(e: KeyboardEventArgs): void {
    ...
    if (
      ...
      const args: CellClickEventArgs = <CellClickEventArgs>(
        extend(this.parent.activeCellsData, { cancel: false, event: e })
      );
  ...
  private onCellMouseDown(e: { event: Event & MouseEvent }): void {
    ...
    if (
      (e.event.target as HTMLElement).classList.contains(
    ...
    if (
      ...
      this.parent.removeSelectedClass();
      const allDayRow: HTMLTableRowElement = <HTMLTableRowElement>(
        this.parent.getAllDayRow()
      );
  ...
  public onMouseSelection(e: MouseEvent): void {
    const appointments: Element[] = [].slice.call(
      this.parent.element.querySelectorAll('.' + cls.APPOINTMENT_CLASS)
    ) as Element[];
    addClass(appointments, 'e-allow-select');
    const selectionEdges: ResizeEdges = this.parent.boundaryValidation(
      e.pageY,
      e.pageX
    );
    if (
      ...
      const parent: HTMLElement = this.parent.element.querySelector(
        '.' + cls.CONTENT_WRAP_CLASS
      ) as HTMLElement;
      const yInBounds: boolean =
        parent.offsetHeight <= parent.scrollHeight &&
        parent.scrollTop >= 0 &&
        parent.scrollTop + parent.offsetHeight <= parent.scrollHeight;
      const xInBounds: boolean =
  ...
  private onMoveUp(e: Event): void {
    ...
    let target: HTMLTableCellElement = closest(
      e.target as Element,
      queryStr
    ) as HTMLTableCellElement;
    const selectedCells: Element[] = this.parent.getSelectedCells();
    ...
    if (
      ...
      target.classList.contains(cls.OTHERMONTH_CLASS)
    ) {
      return;
    }
    this.parent.activeCellsData = this.getSelectedElements(target);
    ...
    if (this.parent.eventWindow) {
      this.parent.eventWindow.convertToEventData(
    ...
    this.parent.trigger(event.select, args, (selectArgs: SelectEventArgs) => {
      const isPopupShow: boolean =
        selectArgs.showQuickPopup || this.parent.quickInfoOnSelectionEnd;
      if (isPopupShow && selectedCells.length > 1) {
        const cellArgs: CellClickEventArgs = <CellClickEventArgs>(
  ...
  private processEnter(e: Event): void {
    if (
      (this.parent.activeViewOptions.readonly &&
        !(e.target as Element).classList.contains(cls.APPOINTMENT_CLASS)) ||
    ...
    if (
      this.parent.currentView === 'TimelineYear' &&
      (e.target as Element).classList.contains(cls.OTHERMONTH_CLASS)
    ) {
      return;
    }
    const target: HTMLTableCellElement = e.target as HTMLTableCellElement;
    if (closest(target, '.' + cls.POPUP_WRAPPER_CLASS)) {
      if (
        target.classList.contains(cls.QUICK_POPUP_EVENT_DETAILS_CLASS) ||
    ...
    if (
      target.classList.contains(cls.RESOURCE_CELLS_CLASS) &&
      target.classList.contains(cls.RESOURCE_PARENT_CLASS)
    ) {
      const resourceIcon: HTMLElement = target.querySelector(
    ...
    if (
      target.classList.contains(cls.APPOINTMENT_CLASS) ||
      target.classList.contains(cls.MORE_EVENT_CLOSE_CLASS) ||
  ...
  private getSelectedElements(
    ...
    if (
      ...
      target.classList.contains(cls.SELECTED_CELL_CLASS)
    ) {
      const start: CellClickEventArgs = this.parent.getCellDetails(
        this.selectedCells[0]
      );
      const end: CellClickEventArgs = this.parent.getCellDetails(
        this.selectedCells.slice(-1)[0]
      );
  ...
  private getCells(
    ...
    const query: string =
      isTimelineYear && !isInverseTable
        ? '.' + cls.WORK_CELLS_CLASS + ':not(.' + cls.OTHERMONTH_CLASS + ')'
        : 'td';
    let cells: HTMLTableCellElement[] = [].slice.call(
      tableEle.querySelectorAll(query)
    );
    ...
    if (start && start.classList.contains(cls.ALLDAY_CELLS_CLASS)) {
      const allDayRow: HTMLTableRowElement =
        this.parent.getAllDayRow() as HTMLTableRowElement;
    ...
    const sCells: HTMLTableCellElement[] = isInverseTable
      ? inverseCells
      : cells;
  ...
  private focusFirstCell(): void {
    if (this.parent.currentView === 'Agenda') {
      const focusCell: HTMLElement = this.parent
        .getContentTable()
        .querySelector('.' + cls.AGENDA_CELLS_CLASS) as HTMLTableCellElement;
    ...
    if (
      ...
      const cell: Element = this.parent.element.querySelector(
        '.' +
          ...
          cls.WORK_CELLS_CLASS +
          ':not(.' +
          cls.RESOURCE_GROUP_CELLS_CLASS +
          ')'
      );
      ...
      let query: string =
        ...
        cls.WORK_CELLS_CLASS +
        ':not(.' +
        cls.OTHERMONTH_CLASS +
        ')' +
        ':not(.' +
        cls.RESOURCE_GROUP_CELLS_CLASS +
        ')';
      const isVerticalYear: boolean =
        this.parent.currentView === 'TimelineYear' &&
        this.parent.activeViewOptions.orientation === 'Vertical';
      query += isVerticalYear
        ? '[data-date="' + this.parent.activeView.startDate().getTime() + '"]'
        : '';
      this.selectCells(false, this.parent.element.querySelector(query));
    } else {
      this.selectCells(
        false,
        this.parent.getWorkCellElements()[0] as HTMLTableCellElement
      );
    }
  }
  ...
  public selectCells(
    ...
    let target: HTMLTableCellElement =
      targetCell instanceof Array ? targetCell.slice(-1)[0] : targetCell;
    if (isMultiple) {
      ...
      this.parent.trigger(event.select, args, (selectArgs: SelectEventArgs) => {
        const allowMultipleRow: boolean =
          !selectArgs.allowMultipleRow || !this.parent.allowMultiRowSelection;
        if (allowMultipleRow) {
          const isTimelineYear: boolean =
            this.parent.currentView === 'TimelineYear';
          if (
            (isTimelineYear &&
            ...
            const isGroupYear: boolean =
              isTimelineYear &&
              this.parent.activeViewOptions.group.resources.length > 0;
            target = isGroupYear
              ? this.initialTarget
              : (this.initialTarget.parentElement.children[
                  target.cellIndex
                ] as HTMLTableCellElement);
          } else if (views.indexOf(this.parent.currentView) > -1) {
            target = target.parentElement.children[
              this.initialTarget.cellIndex
            ] as HTMLTableCellElement;
          }
        }
        let selectedCells: HTMLTableCellElement[] = this.getCells(
          this.isInverseTableSelect(),
          this.initialTarget,
          target
        );
        ...
        if (
          ...
          let allDayCells: HTMLTableCellElement[] =
            this.getAllDayCells(selectedCells);
          if (this.parent.activeViewOptions.group.resources.length > 0) {
        ...
        if (
          target.getAttribute('data-group-index') !== initialId &&
          this.parent.activeViewOptions.group.resources.length > 0
        ) {
  ...
  private selectAppointmentElementFromWorkCell(
    ...
    if (
      ...
      const appointmentElements: HTMLElement[] =
        this.getUniqueAppointmentElements();
      const filteredElements: HTMLElement[] = [];
      const selectedDate: number = this.parent
        .getDateFromElement(target)
        .getTime();
      const selectedSeriesEvents: Record<string, any>[] =
        this.parent.eventsProcessed.filter(
          (eventObject: Record<string, any>) => {
            return !isReverse
              ? (<Date>(
                  eventObject[this.parent.eventFields.startTime]
      ...
      selectedSeriesEvents.filter((event: Record<string, any>) => {
        appointmentElements.filter((element: HTMLElement) => {
          if (
            JSON.stringify(event.Guid) ===
            JSON.stringify(element.getAttribute('data-guid'))
          ) {
      ...
      if (filteredElements.length > 0) {
        const selectedElement: Element = isReverse
          ? filteredElements[filteredElements.length - 1]
          : filteredElements[0];
        const focusElements: HTMLElement[] = this.getAppointmentElementsByGuid(
          selectedElement.getAttribute('data-guid')
        );
  ...
  private getAllDayCells(
    ...
    if (end.endTime.getTime() - start.startTime.getTime() >= util.MS_PER_DAY) {
      const allDayCells: HTMLTableCellElement[] = [].slice.call(
        allDayRow.cells
      );
  ...
  private getUniqueAppointmentElements(): HTMLElement[] {
    ...
    const appointmentElements: HTMLElement[] = [];
    appointments
      .map((value: HTMLElement) => value.getAttribute('data-guid'))
      .filter((value: string, index: number, self: string[]) => {
        if (self.indexOf(value) === index) {
          appointmentElements.push(
            appointments[parseInt(index.toString(), 10)]
          );
        }
      });
  ...
  private getWorkCellFromAppointmentElement(
    ...
    return this.parent.eventBase.selectWorkCellByTime([
      selectedObject,
    ]) as HTMLTableCellElement;
  }
  private processViewNavigation(e: KeyboardEventArgs): void {
    ...
    if (
      !isNaN(index) &&
      !isNullOrUndefined(this.parent.views) &&
      index < this.parent.views.length
    ) {
      const view: View =
        this.parent.viewCollections[parseInt(index.toString(), 10)].option;
  ...
  private isCalendarTarget(e: KeyboardEventArgs): boolean {
    const keyTarget: Element =
      (e.currentTarget as Element) || (e.target as Element);
    if (
      keyTarget &&
      !isNullOrUndefined(
        keyTarget.querySelector('.e-header-popup.e-popup-open')
      )
    ) {
      return true;
    }
  ...
  private cancelUpDownAction(isTimelineYear: boolean): boolean {
    ...
    const isGroup: boolean =
      this.parent.activeViewOptions.group.resources.length > 0;
    if (
      (isVerticalYear && isGroup) ||
      (isTimelineYear &&
        this.initialTarget.classList.contains(cls.OTHERMONTH_CLASS))
    ) {
      return true;
    }
    if (
      (this.parent.activeView.isTimelineView() && !isTimelineYear) ||
      this.parent.currentView === 'MonthAgenda'
    ) {
      return true;
    }
  ...
  private processUp(e: KeyboardEventArgs, isMultiple: boolean): void {
    const isTimelineYear: boolean = this.parent.currentView === 'TimelineYear';
    if (
      (isMultiple && this.cancelUpDownAction(isTimelineYear)) ||
      this.isCalendarTarget(e)
    ) {
      return;
    }
    ...
    const selectedEventElements: Element[] =
      this.parent.eventBase.getSelectedAppointments();
    const moreEventWrapper: HTMLElement = <HTMLElement>(
      this.parent.element.querySelector('.' + cls.MORE_POPUP_WRAPPER_CLASS)
    );
    ...
    if (
      selectedEventElements.length > 0 &&
      !moreEventWrapper.classList.contains(cls.POPUP_OPEN) &&
      !quickPopupWrapper.classList.contains(cls.POPUP_OPEN) &&
      ['Day', 'Week', 'WorkWeek', 'Month'].indexOf(this.parent.currentView) !==
        -1
    ) {
      target = this.getWorkCellFromAppointmentElement(
        selectedEventElements[selectedEventElements.length - 1]
      );
      this.parent.eventBase.removeSelectedAppointmentClass();
    }
    if (!target) {
    ...
    if (
      target.classList.contains(cls.WORK_CELLS_CLASS) &&
      !this.parent.element.querySelector('.' + cls.POPUP_OPEN)
    ) {
      const tableRows: HTMLTableRowElement[] =
        this.parent.getTableRows() as HTMLTableRowElement[];
      const curRowIndex: number = tableRows.indexOf(
      ...
      if (
        ...
        targetCell = this.isInverseTableSelect()
          ? this.getVerticalUpDownCell(tableRows, target, curRowIndex, true)
      ...
      if ((curRowIndex > 0 || targetCell) && curRowIndex < tableRows.length) {
        targetCell = targetCell
          ? targetCell
          : tableRows[curRowIndex - 1].cells[target.cellIndex];
        if (
          this.parent.currentView === 'Year' &&
          targetCell.classList.contains(cls.OTHERMONTH_CLASS)
        ) {
          if (
            this.parent.activeView.getStartDate().getTime() <
            +targetCell.getAttribute('data-date')
          ) {
            targetCell = this.getYearUpDownCell(
              tableRows,
              curRowIndex - 1,
              target.cellIndex,
              true
  ...
  private processDown(e: KeyboardEventArgs, isMultiple: boolean): void {
    ...
    if (
      ...
      if (
        ...
        targetCell = this.isInverseTableSelect()
          ? this.getVerticalUpDownCell(tableRows, target, curRowIndex, false)
          : this.getHorizontalUpDownCell(tableRows, target, curRowIndex, false);
      }
      if (
        curRowIndex >= 0 &&
        (curRowIndex < tableRows.length - 1 || targetCell)
      ) {
        targetCell = targetCell
          ? targetCell
          : tableRows[curRowIndex + 1].cells[target.cellIndex];
        if (
          ...
          if (
            this.parent.activeView.getEndDate().getTime() >
            +targetCell.getAttribute('data-date')
          ) {
            targetCell = this.getYearUpDownCell(
              tableRows,
  ...
  private getYearUpDownCell(
    ...
    while (
      tableRows[parseInt(rowIndex.toString(), 10)] &&
      tableRows[parseInt(rowIndex.toString(), 10)].cells[
        parseInt(cellIndex.toString(), 10)
      ].classList.contains(cls.OTHERMONTH_CLASS)
    ) {
      rowIndex = rowIndex + (isUp ? -1 : 1);
    }
    return tableRows[parseInt(rowIndex.toString(), 10)].cells[
      parseInt(cellIndex.toString(), 10)
    ];
  }
  ...
  private getHorizontalUpDownCell(
    ...
    if (cell.classList.contains(cls.OTHERMONTH_CLASS)) {
      const workCell: HTMLTableCellElement = row.querySelector(
        '.' + cls.WORK_CELLS_CLASS + ':not(.' + cls.OTHERMONTH_CLASS + ')'
      );
      ...
      const query: string =
        ...
        '"]';
      cell =
        cell.cellIndex < workCell.cellIndex
          ? workCell
  ...
  private getVerticalUpDownCell(
    ...
    const hasRow: boolean =
      (isUp && curRowIndex > 0) ||
      (!isUp && curRowIndex < tableRows.length - 1);
    let targetCell: HTMLTableCellElement = hasRow
      ? tableRows[curRowIndex + (isUp ? -1 : 1)].cells[target.cellIndex]
      : undefined;
    if (!targetCell || targetCell.classList.contains(cls.OTHERMONTH_CLASS)) {
      const column: HTMLTableCellElement =
        tableRows[parseInt(curRowIndex.toString(), 10)].cells[
          target.cellIndex - (isUp ? 1 : -1)
        ];
      if (column) {
        const dateAttr: number =
          +target.getAttribute('data-date') -
          (isUp ? util.MS_PER_DAY : -util.MS_PER_DAY);
        return this.parent
          .getContentTable()
          .querySelector(
            '.' + cls.WORK_CELLS_CLASS + '[data-date="' + dateAttr + '"]'
          );
      }
      targetCell = target;
    }
  ...
  private processLeftRight(target: HTMLTableCellElement): KeyEventArgs {
    ...
    const curRowIndex: number = (target.parentNode as HTMLTableRowElement)
      .sectionRowIndex;
    const key: KeyEventArgs = {
      ...
      maxIndex:
        tableEle.rows[parseInt(curRowIndex.toString(), 10)].cells.length,
    };
    return key;
  }
  private getQuickPopupElement(): HTMLElement {
    return (
      this.parent.isAdaptive ? document.body : this.parent.element
    ).querySelector('.' + cls.POPUP_WRAPPER_CLASS) as HTMLElement;
  }
  private isCancelLeftRightAction(
    e: KeyboardEventArgs,
    isMultiple: boolean,
    isTimelineYear: boolean
  ): boolean {
  ...
  private processRight(e: KeyboardEventArgs, isMultiple: boolean): void {
    const isTimelineYear: boolean = this.parent.currentView === 'TimelineYear';
    if (
      this.isCancelLeftRightAction(e, isMultiple, isTimelineYear) ||
      this.isCalendarTarget(e)
    ) {
      return;
    }
    const selectedCells: Element[] = this.parent.getSelectedCells();
    let targetCell: HTMLTableCellElement;
    const selectedAppointments: Element[] =
      this.parent.eventBase.getSelectedAppointments();
    let target: HTMLTableCellElement = e.target as HTMLTableCellElement;
    if (
      selectedCells.length > 0 &&
    ...
    if (
      ...
      const isMonthEnd: boolean =
        this.parent.currentView === 'Year' &&
        targetDate.getTime() === util.lastDateOfMonth(targetDate).getTime();
      if (
        key.columnIndex >= 0 &&
        key.columnIndex < key.maxIndex - 1 &&
        !isMonthEnd
      ) {
        targetCell = this.calculateNextPrevDate(
          target,
          key.element.rows[key.rowIndex].cells[target.cellIndex + 1],
          'right'
        );
        if (
          isTimelineYear &&
        ...
        if (
          !this.isInverseTableSelect() &&
          key.rowIndex < key.element.rows.length - 1 &&
          !isMonthEnd
        ) {
          targetCell = this.calculateNextPrevDate(
            target,
            key.element.rows[key.rowIndex + 1].cells[0],
            'right'
          );
          const changeTargetCell: boolean =
            isTimelineYear &&
            isMultiple &&
            targetCell.classList.contains(cls.OTHERMONTH_CLASS);
          targetCell = changeTargetCell
            ? this.getHorizontalLeftRightCell(key, target, true)
            : targetCell;
          if (!isNullOrUndefined(targetCell)) {
            this.selectCells(isMultiple, targetCell);
          }
            ...
            this.selectCells(
              ...
              this.parent.element.querySelector(
                ':not(.' +
                  cls.OTHERMONTH_CLASS +
                  ')[data-date="' +
                  (targetDate.getTime() + util.MS_PER_DAY) +
                  '"]'
              )
            );
            return;
          ...
          const rowIndex: number = this.isInverseTableSelect()
            ? key.rowIndex
            : 0;
          this.parent.changeDate(
            this.parent.activeView.getNextPreviousDate('next'),
          ...
          const tableEle: HTMLTableElement =
            this.parent.getContentTable() as HTMLTableElement;
          const cell: HTMLTableCellElement = isMonthEnd
            ? tableEle.rows[parseInt(rowIndex.toString(), 10)].querySelector(
                '.' +
                  cls.WORK_CELLS_CLASS +
                  ':not(.' +
                  cls.OTHERMONTH_CLASS +
  ...
  private processLeft(e: KeyboardEventArgs, isMultiple: boolean): void {
    ...
    if (
      ...
      const key: KeyEventArgs = this.processLeftRight(target);
      const targetDate: Date = new Date(+target.getAttribute('data-date'));
      const isMonthStart: boolean =
        this.parent.currentView === 'Year' &&
        targetDate.getTime() === util.firstDateOfMonth(targetDate).getTime();
      if (
        key.columnIndex > 0 &&
        key.columnIndex < key.maxIndex &&
        !isMonthStart
      ) {
        targetCell = this.calculateNextPrevDate(
          target,
        ...
        if (!this.isInverseTableSelect() && key.rowIndex > 0) {
          targetCell = this.calculateNextPrevDate(
            target,
            key.element.rows[key.rowIndex - 1].cells[key.maxIndex - 1],
            'left'
          );
          const otherMonthCell: boolean =
            isTimelineYear &&
            isMultiple &&
            targetCell.classList.contains(cls.OTHERMONTH_CLASS);
          targetCell = otherMonthCell
            ? this.getHorizontalLeftRightCell(key, target, false)
            : targetCell;
          if (!isNullOrUndefined(targetCell)) {
            this.selectCells(isMultiple, targetCell);
          }
          ...
          if (
            ...
            this.selectCells(
              isMultiple,
              this.parent.element.querySelector(
                '[data-date="' + (targetDate.getTime() - util.MS_PER_DAY) + '"]'
              )
            );
            return;
          }
  ...
  private getTimelineYearTargetCell(
    ...
    return this.isInverseTableSelect()
      ? this.getVerticalLeftRightCell(target, isRight)
  ...
  private getHorizontalLeftRightCell(
    ...
    const row: HTMLTableRowElement =
      key.element.rows[
        (<HTMLTableRowElement>target.parentNode).sectionRowIndex +
          (isRight ? 1 : -1)
      ];
    if (row) {
      const query: string = isRight
        ? '.' + cls.WORK_CELLS_CLASS + ':not(.' + cls.OTHERMONTH_CLASS + ')'
        : '[data-date="' +
          (+target.getAttribute('data-date') - util.MS_PER_DAY) +
          '"]';
  ...
  private getVerticalLeftRightCell(
    ...
    const start: Date = new Date(
      date.getFullYear(),
      date.getMonth() + (isRight ? 1 : -1),
      1
    );
    const tableEle: HTMLTableElement =
      this.parent.getContentTable() as HTMLTableElement;
    const targetCell: HTMLTableCellElement = tableEle.querySelector(
      '[data-date="' + start.getTime() + '"]'
    );
    if (
      (targetCell.parentNode as HTMLTableRowElement).sectionRowIndex >
      (target.parentNode as HTMLTableRowElement).sectionRowIndex
    ) {
      return targetCell;
    }
    return tableEle.querySelector(
      '[data-date="' +
  ...
  private calculateNextPrevDate(
    ...
    const initialId: string =
      this.initialTarget.getAttribute('data-group-index');
    if (
      this.parent.activeViewOptions.group.resources.length > 0 &&
      this.parent.currentView === 'Month'
    ) {
  ...
  private processTabOnPopup(e: KeyboardEventArgs, popupElement: Element): void {
    ...
    focusableElements = focusableElements.filter((element: Element) => {
      const footerEle: HTMLElement = this.parent.element.querySelector(
        '.' + cls.POPUP_FOOTER_CLASS
      ) as HTMLElement;
      if (footerEle && footerEle.offsetParent) {
        return !(
          element.classList.contains(cls.EDIT_CLASS) ||
          element.classList.contains(cls.DELETE_CLASS)
        );
      } else {
        return !(
          element.classList.contains(cls.EDIT_EVENT_CLASS) ||
          element.classList.contains(cls.DELETE_EVENT_CLASS)
        );
      }
    });
  ...
  private processTab(e: KeyboardEventArgs, isReverse: boolean): void {
    ...
    if (
      ...
      this.parent.inlineModule.inlineCrudActions(
    ...
    const popupWrapper: Element = closest(
      target,
      '.' + cls.POPUP_WRAPPER_CLASS + ',.' + cls.MORE_POPUP_WRAPPER_CLASS
    );
    ...
    if (
      ...
      const index: number = parseInt(
        target.getAttribute('data-group-index'),
        10
      );
      const appSelector: string = `.${
        cls.APPOINTMENT_CLASS
      }[data-group-index="${isReverse ? index - 1 : index}"]`;
      const appElements: HTMLElement[] = [].slice.call(
        this.parent.element.querySelectorAll(appSelector)
      );
      if (appElements.length > 0) {
        this.parent.eventBase.removeSelectedAppointmentClass();
        const focusAppointment: HTMLElement = isReverse
          ? appElements.slice(-1)[0]
          : appElements[0];
        ...
        index + 1 ===
        this.parent.resourceBase.lastResourceLevel.length
      ) {
        ...
        const averageRowHeight: number = Math.round(
          virtual.offsetHeight /
            this.parent.resourceBase.expandedResources.length
        );
        this.parent.element.querySelector('.e-content-wrap').scrollTop =
          (isReverse ? index - 1 : index + 1) * averageRowHeight;
    ...
    if (target.classList.contains(cls.APPOINTMENT_CLASS)) {
      let appElements: HTMLElement[] = [].slice.call(
        this.parent.element.querySelectorAll('.' + cls.APPOINTMENT_CLASS)
      );
      const isTimelineYear: boolean =
        this.parent.currentView === 'TimelineYear';
      ...
      if (
        ...
        appElements = [].slice.call(
          this.parent.element.querySelectorAll(
            `.${cls.APPOINTMENT_CLASS}[data-group-index="${index}"]`
          )
        );
        const resCellSelector: string = `.${
          cls.RESOURCE_CELLS_CLASS
        }[data-group-index="${isReverse ? index : index + 1}"]`;
        const resourceCell: HTMLElement = this.parent.element.querySelector(
          resCellSelector
        ) as HTMLElement;
        if (
          resourceCell &&
          ((isReverse &&
            target.getAttribute('data-guid') ===
              appElements[0].getAttribute('data-guid')) ||
            (!isReverse &&
              target.getAttribute('data-guid') ===
                appElements.slice(-1)[0].getAttribute('data-guid')))
        ) {
          this.parent.eventBase.removeSelectedAppointmentClass();
      ...
      if (
        (!isReverse &&
          target.getAttribute('data-guid') ===
            appElements[appElements.length - 1].getAttribute('data-guid')) ||
        (isReverse &&
          target.getAttribute('data-guid') ===
            appElements[0].getAttribute('data-guid'))
      ) {
  ...
  private processDelete(e: KeyboardEventArgs): void {
    ...
    if (this.parent.currentView === 'MonthAgenda') {
      const selectedEle: HTMLElement[] =
        this.parent.eventBase.getSelectedEvents().element as HTMLElement[];
      activeEle = <Element>(
        (selectedEle && isNullOrUndefined(selectedEle.length)
          ? selectedEle
          : selectedEle[0])
      );
    }
    if (activeEle && activeEle.classList.contains(cls.APPOINTMENT_CLASS)) {
      ...
      if (
        this.parent.activeViewOptions.readonly ||
        activeEle.classList.contains(cls.READ_ONLY) ||
  ...
  private processCtrlShiftNavigationArrows(e: KeyboardEventArgs): void {
    if (
      ...
      const groupIndex: number = parseInt(
        document.activeElement.getAttribute('data-group-index'),
        10
      );
      let index: number =
        e.action === 'ctrlShiftLeftArrow' || e.action === 'ctrlShiftUpArrow'
          ? groupIndex - 1
          : groupIndex + 1;
      index =
        index < 0
          ? 0
          : index > this.parent.resourceBase.lastResourceLevel.length
          ? this.parent.resourceBase.lastResourceLevel.length
          : index;
      ...
      const index: number =
        this.parent.resourceBase.renderedResources[0].groupIndex;
  ...
  private processTabOnResourceCells(target: Element, isReverse: boolean): void {
    const tabElements: Element[] = [].slice.call(
      this.parent.element.querySelectorAll('[tabIndex="0"]')
    );
    const currentTabIndex: number = tabElements.indexOf(target);
    const nextTabElement: Element = !isReverse
      ? tabElements[currentTabIndex + 1]
      : tabElements[currentTabIndex - 1];
    if (
      nextTabElement &&
      nextTabElement.classList.contains(cls.RESOURCE_CELLS_CLASS)
    ) {
      const groupIndex: number = parseInt(
        nextTabElement.getAttribute('data-group-index'),
        10
      );
      if (this.parent.virtualScrollModule) {
        const resColWrap: HTMLElement = this.parent.element.querySelector(
          '.' + cls.RESOURCE_COLUMN_WRAP_CLASS
        );
        const resCells: HTMLElement[] = [].slice.call(
          this.parent.element.querySelectorAll('.' + cls.RESOURCE_CELLS_CLASS)
        );
        resCells.forEach((element: HTMLElement) => {
          if (
            element.getBoundingClientRect().top <
            resColWrap.getBoundingClientRect().top
          ) {
