...
export class ScheduleTouch {
  ...
  private scrollHandler(e: ScrollEventArgs): void {
    ...
    if (e.scrollDirection === 'Left' || e.scrollDirection === 'Right') {
      ...
      const scrollDiv: HTMLElement = this.element.querySelector(
    ...
    if (e.scrollDirection === this.touchLeftDirection) {
      ...
      this.element.style.transform =
        'translatex(' + (this.getTranslateX(this.element) + x) + 'px)';
      ...
      const x: number = this.parent.enableRtl
        ? prevWidth - e.distanceX
        : -prevWidth + e.distanceX;
      this.element.style.transform =
        'translatex(' + (this.getTranslateX(this.element) + x) + 'px)';
    }
  ...
  private swipeHandler(e: SwipeEventArgs): void {
    ...
    const swipeDate: Date =
      e.swipeDirection === 'Left'
        ? this.parent.activeView.renderDates[0]
        : this.parent.activeView.renderDates.slice(-1)[0];
    if (
      (e.swipeDirection === 'Left' && swipeDate < this.parent.maxDate) ||
      ...
      if (
        offsetDist > time ||
        e.distanceX > this.parent.element.offsetWidth / 2
      ) {
        ...
        if (
          ...
          this.element.style.transitionDuration =
            (Browser.isDevice ? e.distanceX : offsetDist) / time / 10 + 's';
        }
      ...
      if (this.parent.isAdaptive) {
        if (Browser.isIos) {
          EventHandler.add(
            this.element,
            'touchend',
            this.preventEventClick,
            this
          );
        }
        this.parent.quickPopup.tapHoldEventPopup(e.originalEvent);
      } else if (
        ['Agenda', 'MonthAgenda', 'Year'].indexOf(this.parent.currentView) < 0
      ) {
  ...
  private confirmSwipe(swipeDirection: string): void {
    const previousDate: Date =
      swipeDirection === this.touchLeftDirection
        ? this.nextPanel.selectedDate
        : this.previousPanel.selectedDate;
        ...
        if (navArgs.cancel) {
          ...
          if (this.parent.enableRtl) {
            translateX =
              swipeDirection === this.touchLeftDirection
                ? this.previousPanel
                  ? this.previousPanel.element.offsetLeft
                  : this.currentPanel.element.offsetWidth
                : 0;
          } else {
            translateX =
              swipeDirection === this.touchLeftDirection
  ...
  private cancelSwipe(): void {
    ...
    this.parent.activeView.colLevels = this.currentPanel.colLevels;
    addClass([this.element], cls.TRANSLATE_CLASS);
    const prevWidth: number = this.previousPanel
      ? this.previousPanel.element.offsetWidth
      : 0;
    this.element.style.transform =
      'translatex(' +
