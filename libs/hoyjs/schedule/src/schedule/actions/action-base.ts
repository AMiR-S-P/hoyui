...
import { formatUnit, remove, removeClass } from '@hoyui/base';
import {
  ActionBaseArgs,
  ResizeEdges,
  DragEventArgs,
...
export class ActionBase {
  ...
  public getChangedData(
    ...
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      const originalElement: HTMLElement[] = this.getOriginalElement(
        this.actionObj.element
      );
      if (originalElement) {
        let indexCol: number[] = originalElement.map((element: HTMLElement) =>
          parseInt(element.getAttribute('data-group-index'), 10)
        );
        if (
          indexCol.indexOf(this.actionObj.groupIndex) === -1 ||
          (!isNullOrUndefined(multiData) && multiData.length > 0)
        ) {
          const cloneIndex: number = parseInt(
            this.actionObj.clone.getAttribute('data-group-index'),
            10
          );
          ...
          if (multiData && multiData.length > 0) {
            multiData.forEach((data: Record<string, any>) => {
              this.parent.resourceBase.getResourceData(
                data,
  ...
  public saveChangedData(
    eventArgs: DragEventArgs | ResizeEventArgs,
    ...
    for (const eventObj of eventsCollection) {
      const isSameResource: boolean =
        this.parent.activeViewOptions.group.resources.length > 0
          ? parseInt(
              this.actionObj.element.getAttribute('data-group-index'),
              10
            ) === this.actionObj.groupIndex
          : true;
      if (
        +eventObj[this.parent.eventFields.startTime] ===
          +this.actionObj.event[this.parent.eventFields.startTime] &&
        +eventObj[this.parent.eventFields.endTime] ===
          +this.actionObj.event[this.parent.eventFields.endTime] &&
        isSameResource
      ) {
      ...
      if (eventObj[this.parent.eventFields.recurrenceRule]) {
        const eveId: number = (eventObj[this.parent.eventFields.recurrenceID] ||
          eventObj[this.parent.eventFields.id]) as number;
        if (
          eventObj[this.parent.eventFields.id] ===
          eventObj[this.parent.eventFields.recurrenceID]
        ) {
          eventObj[this.parent.eventFields.id] =
            this.parent.eventBase.getEventMaxID();
          currentAction = 'EditOccurrence';
        }
        if (
          this.parent.enableRecurrenceValidation &&
          this.parent.eventWindow.editOccurrenceValidation(
            eveId,
            eventObj,
            this.actionObj.event
          )
      ...
      if (
        eventObj[this.parent.eventFields.startTimezone] ||
        eventObj[this.parent.eventFields.endTimezone]
      ) {
  ...
  public calculateIntervalTime(date: Date): Date {
    ...
    intervalTime.setMinutes(
      Math.floor(intervalTime.getMinutes() / this.actionObj.interval) *
  ...
  public getContentAreaDimension(): Record<string, any> {
    ...
    let trElement: HTMLElement[] = [].slice.call(
      viewElement.querySelector('tr').children
    );
    if (
    ...
    const rightOffset: ClientRect = trElement
      .slice(-1)[0]
  ...
  public getIndex(index: number): number {
    const contentElements: HTMLTableCellElement[] = [].slice.call(
      this.parent.getContentTable().querySelector('tr').children
    );
    ...
    if (
      ...
      const groupElements: HTMLTableCellElement[] =
        this.getResourceElements(contentElements);
  ...
  public updateTimePosition(
    date: Date,
    multiData?: Record<string, any>[]
  ): void {
    let index: number = 0;
    for (const cloneElement of this.actionObj.cloneElement) {
      const timeElement: Element = cloneElement.querySelector(
      ...
      if (timeElement) {
        ...
        if (multiData && multiData.length > 0) {
          startTime = multiData[parseInt(index.toString(), 10)][
            this.parent.eventFields.startTime
          ] as Date;
          endTime = multiData[parseInt(index.toString(), 10)][
            this.parent.eventFields.endTime
          ] as Date;
        }
        timeElement.innerHTML =
          this.parent.getTimeString(startTime) +
          ' - ' +
    ...
    let timeIndicator: HTMLElement = this.parent.element.querySelector(
      '.' + cls.CLONE_TIME_INDICATOR_CLASS
    ) as HTMLElement;
    if (!timeIndicator) {
      timeIndicator = createElement('div', {
        className: cls.CLONE_TIME_INDICATOR_CLASS,
      });
      const wrapperClass: string = this.parent.activeView.isTimelineView()
    ...
    if (this.parent.activeView.isTimelineView()) {
      if (this.parent.enableRtl) {
        ...
        offsetValue =
          this.actionObj.action === 'drag' || this.resizeEdges.left
            ? rightValue + this.actionObj.clone.offsetWidth
            : rightValue;
        ...
        offsetValue =
          this.actionObj.action === 'drag' || this.resizeEdges.left
            ? leftValue
            : leftValue + this.actionObj.clone.offsetWidth;
        timeIndicator.style.left = formatUnit(offsetValue);
      }
    } else {
      offsetValue =
        this.actionObj.action === 'drag' || this.resizeEdges.top
  ...
  public getOriginalElement(element: HTMLElement): HTMLElement[] {
    ...
    const isMorePopup: boolean =
      element.offsetParent &&
      element.offsetParent.classList.contains(cls.MORE_EVENT_POPUP_CLASS);
    if (
      isMorePopup ||
      this.parent.activeView.isTimelineView() ||
      (this.actionObj.action !== 'resize' && this.parent.virtualScrollModule)
    ) {
      originalElement = [].slice.call(
        this.parent.element.querySelectorAll('[data-guid="' + guid + '"]')
      );
    } else {
      const tr: HTMLElement = closest(element, 'tr') as HTMLElement;
      if (tr) {
        originalElement = [].slice.call(
          tr.querySelectorAll('[data-guid="' + guid + '"]')
        );
      }
    }
  ...
  public createCloneElement(element: HTMLElement): HTMLElement {
    const cloneWrapper: HTMLElement = createElement('div', {
    ...
    cloneClassLists.push(
      this.actionObj.action === 'drag'
        ? cls.DRAG_CLONE_CLASS
        : cls.RESIZE_CLONE_CLASS
    );
    if (
      this.parent.currentView === 'Month' ||
      this.parent.currentView === 'TimelineMonth'
  ...
  public removeCloneElementClasses(): void {
    ...
    if (
      ...
      elements = [].slice.call(
        this.parent.element.querySelectorAll('.' + cls.EVENT_ACTION_CLASS)
      );
    }
  ...
  public removeCloneElement(): void {
    ...
    const dynamicEle: HTMLElement[] = [].slice.call(
    ...
    const timeIndicator: Element = this.parent.element.querySelector(
      '.' + cls.CLONE_TIME_INDICATOR_CLASS
    );
    if (timeIndicator) {
  ...
  public autoScroll(): void {
    ...
    let xInBounds: boolean =
      xIsScrollable &&
      parent.scrollLeft >= 0 &&
      parent.scrollLeft + parent.offsetWidth <= parent.scrollWidth;
    if (
      this.actionObj.action === 'resize' &&
      this.scrollEdges.right &&
      parent.scrollLeft + parent.offsetWidth > parent.scrollWidth
    ) {
      const tdCollections: number =
        [].slice.call(
          (
            this.parent
              .getContentTable()
      ...
      const cellIndex: number = Math.ceil(
        (this.actionObj.clone.offsetLeft + this.actionObj.clone.offsetWidth) /
          this.actionObj.cellWidth
      );
      xInBounds = cellIndex === tdCollections;
    }
    if (yInBounds && (this.scrollEdges.top || this.scrollEdges.bottom)) {
      parent.scrollTop += this.scrollEdges.top
        ? -this.actionObj.scroll.scrollBy
        : this.actionObj.scroll.scrollBy;
      if (this.actionObj.action === 'resize') {
        if (
          ...
          this.actionObj.Y += this.scrollEdges.top
            ? this.actionObj.scroll.scrollBy
            : -this.actionObj.scroll.scrollBy;
        }
      }
    ...
    if (xInBounds && (this.scrollEdges.left || this.scrollEdges.right)) {
      parent.scrollLeft += this.scrollEdges.left
        ? -this.actionObj.scroll.scrollBy
        : this.actionObj.scroll.scrollBy;
      if (this.actionObj.action === 'resize') {
        if (
          parent.scrollWidth !== parent.offsetWidth + parent.scrollLeft &&
          parent.scrollLeft > 0
        ) {
          this.actionObj.X += this.scrollEdges.left
            ? this.actionObj.scroll.scrollBy
            : -this.actionObj.scroll.scrollBy;
        }
      }
  ...
  public autoScrollValidation(): boolean {
    ...
    const res: ResizeEdges = this.parent.boundaryValidation(
  ...
  public updateScrollPosition(e: MouseEvent & TouchEvent): void {
    ...
    if (
      this.actionObj.scroll.enable &&
      isNullOrUndefined(this.actionObj.scrollInterval)
    ) {
      this.actionObj.scrollInterval = window.setInterval(() => {
        if (
          ...
          if (this.actionObj.action === 'drag') {
            this.parent.dragAndDropModule.updateDraggingDateTime(
              this.scrollEventArgs
            );
          } else {
            this.parent.resizeModule.updateResizingDirection(
              this.scrollEventArgs
            );
          }
        }
  ...
  public updateOriginalElement(cloneElement: HTMLElement): void {
    let query: string =
      '[data-id="' + cloneElement.getAttribute('data-id') + '"]';
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      query = query.concat(
        '[data-group-index = "' +
          cloneElement.getAttribute('data-group-index') +
          '"]'
      );
    }
  ...
  public dynamicYearlyEventsRendering(
    ...
    if (
      ...
      const eventObj: Record<string, any> = this.yearEvent.isSpannedEvent(
        event,
      ...
      appWidth =
        (<{ [key: string]: number }>eventObj.isSpanned).count *
        this.actionObj.cellWidth;
    }
    if (
      !isResize &&
    ...
    const appointmentElement: HTMLElement = this.createAppointmentElement(
      this.actionObj.groupIndex,
      event[this.parent.eventFields.subject] as string
    );
    ...
    const date: number = util
      .resetTime(event[this.parent.eventFields.startTime] as Date)
      .getTime();
    let query: string =
      '.' + cls.WORK_CELLS_CLASS + '[data-date="' + date + '"]';
    if (
      this.parent.activeViewOptions.group.resources.length > 0 &&
      !this.parent.uiStateValues.isGroupAdaptive
    ) {
    ...
    if (isResize) {
      const dateHeader: HTMLElement = cellTd.querySelector(
        '.' + cls.DATE_HEADER_CLASS
      ) as HTMLElement;
      let appHeight: number =
        this.actionObj.cellHeight * (event.count as number) -
        (dateHeader ? dateHeader.offsetHeight : 0) -
        7;
  ...
  public dynamicEventsRendering(event: Record<string, any>): void {
    ...
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      const renderedResource: TdData[] =
        this.parent.virtualScrollModule &&
        this.parent.virtualScrollModule.isHorizontalScroll
          ? this.parent.resourceBase.renderedResources
          : this.parent.resourceBase.lastResourceLevel;
      const resources: TdData[] = renderedResource.filter(
    ...
    const spannedEvents: Record<string, any>[] = this.monthEvent.splitEvent(
      event,
      dateRender
    );
    for (const event of spannedEvents) {
      const day: number = this.parent.getIndexOfDate(
        dateRender,
        util.resetTime(event[this.monthEvent.fields.startTime] as Date)
      );
      ...
      const appointmentElement: HTMLElement =
        this.monthEvent.createAppointmentElement(
          event,
          this.actionObj.groupIndex,
          true
        );
      appointmentElement.setAttribute('drag', 'true');
      addClass([appointmentElement], cls.CLONE_ELEMENT_CLASS);
      this.monthEvent.applyResourceColor(
        appointmentElement,
        event,
        'backgroundColor',
        groupOrder
