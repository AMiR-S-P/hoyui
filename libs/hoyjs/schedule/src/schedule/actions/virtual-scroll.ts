...
export class VirtualScroll {
  ...
  public getRenderedCount(): number {
    this.setItemSize();
    const containerSize: number = this.isHorizontalScroll
      ? this.parent.element.clientWidth
      : this.parent.element.clientHeight;
    this.renderedLength =
      Math.ceil(containerSize / this.itemSize) + this.bufferCount;
  ...
  public renderVirtualTrack(contentWrap: Element): void {
    ...
    if (this.isHorizontalScroll) {
      const colCount: number =
        this.parent.activeView.colLevels[
          this.parent.activeView.colLevels.length - 1
        ].length;
      wrap.style.width = colCount * this.itemSize + 'px';
    } else {
      wrap.style.height =
        this.parent.resourceBase.expandedResources.length * this.itemSize +
        'px';
    }
  ...
  public updateVirtualScrollHeight(): void {
    ...
    const lastResourceIndex: number =
      this.parent.resourceBase.expandedResources[
        this.parent.resourceBase.expandedResources.length - 1
      ].groupIndex;
    const lastRenderIndex: number =
      this.parent.resourceBase.renderedResources[
        this.parent.resourceBase.renderedResources.length - 1
      ].groupIndex;
    if (lastRenderIndex !== lastResourceIndex) {
      ...
      if (
        conWrap.scrollHeight - conWrap.scrollTop <
        conWrap.offsetHeight * this.bufferCount
      ) {
        virtual.style.height =
          parseInt(virtual.style.height, 10) +
          conWrap.offsetHeight * this.bufferCount +
          'px';
      }
    } else {
    ...
    this.averageRowHeight =
      virtual.offsetHeight / this.parent.resourceBase.expandedResources.length;
  }

  public updateVirtualTrackHeight(wrap: HTMLElement): void {
    const resourceCount: number =
      this.parent.resourceBase.renderedResources.length;
    if (resourceCount !== this.getRenderedCount()) {
      wrap.style.height =
        (this.parent.element.querySelector('.e-content-wrap') as HTMLElement)
          .clientHeight + 'px';
      const resWrap: HTMLElement = this.parent.element.querySelector(
        '.' + cls.RESOURCE_COLUMN_WRAP_CLASS
      ) as HTMLElement;
      const conWrap: HTMLElement = this.parent.element.querySelector(
        '.' + cls.CONTENT_WRAP_CLASS
      ) as HTMLElement;
      const eventWrap: HTMLElement = this.parent.element.querySelector(
      ...
      const lastRenderIndex: number =
        this.parent.resourceBase.renderedResources[resourceCount - 1]
          .groupIndex;
      const lastCollIndex: number =
        this.parent.resourceBase.expandedResources[
          this.parent.resourceBase.expandedResources.length - 1
        ].groupIndex;
      let renderedResCount: number =
        resourceCount + (lastCollIndex - lastRenderIndex);
      renderedResCount =
        renderedResCount > this.parent.resourceBase.expandedResources.length
          ? this.parent.resourceBase.expandedResources.length
          : renderedResCount;
  ...
  public setItemSize(): void {
    if (this.isHorizontalScroll) {
      this.itemSize =
        util.getElementWidthFromClass(
          this.parent.activeView.element,
          cls.WORK_CELLS_CLASS,
          this.parent.uiStateValues.isTransformed
        ) || this.itemSize;
    } else {
      this.itemSize =
  ...
  public refreshLayout(): void {
    ...
    if (this.parent.uiStateValues.scheduleHeight !== initialHeight) {
      ...
      let firstTDIndex: number = parseInt(
        resWrap.querySelector('tbody td').getAttribute('data-group-index'),
        10
      );
      const endIndex: number = firstTDIndex + this.renderedLength;
      firstTDIndex =
        endIndex > this.parent.resourceBase.expandedResources.length
          ? this.parent.resourceBase.expandedResources.length -
            this.renderedLength
          : firstTDIndex;
      ...
      this.parent.resourceBase.renderedResources =
        this.parent.resourceBase.expandedResources.slice(
          firstTDIndex,
          endIndex
        );
  ...
  private renderEvents(): void {
    this.setTabIndex();
    const dynamicData: Record<string, any>[] = this.triggerScrollEvent(
      events.virtualScrollStop
    );
    if (
      this.parent.activeViewOptions &&
      this.parent.activeViewOptions.enableLazyLoading &&
  ...
  public virtualScrolling(): void {
    ...
    if (this.isHorizontalScroll) {
      ...
      const firstTDIndex: number = parseInt(
        resWrap.querySelector('tbody td').getAttribute('data-group-index'),
        10
      );
      const scrollHeight: number = this.parent.rowAutoHeight
        ? conTable.offsetHeight - conWrap.offsetHeight
        : this.bufferCount * this.itemSize;
      ...
      if (!isNullOrUndefined(resCollection) && resCollection.length > 0) {
        ...
        this.focusedEle =
          selectedEle[selectedEle.length - 1] || this.focusedEle;
  ...
  private horizontalScrolling(conWrap: HTMLElement): void {
    ...
    if (Math.abs(conWrap.scrollLeft) - Math.abs(this.translateY) < 0) {
      ...
      Math.abs(conWrap.scrollLeft) - Math.abs(this.translateY) >
      scrollWidth
    ) {
  ...
  private triggerScrollEvent(action: string): Record<string, any>[] {
    ...
    const eventArgs: ScrollEventArgs = {
      ...
      resourceData: this.parent.resourceBase.renderedResources.map(
        (x: TdData) => x.resourceData
      ),
      name: action,
    };
    this.parent.trigger(action, eventArgs, (args: ScrollEventArgs) => {
      if (action === events.virtualScrollStart) {
        ...
        action === events.virtualScrollStop &&
        !isNullOrUndefined(args.eventData) &&
        args.eventData.length > 0
      ) {
  ...
  private upScroll(conWrap: HTMLElement, firstTDIndex: number): TdData[] {
    let index: number = 0;
    index =
      ~~(conWrap.scrollTop / this.itemSize) +
      Math.ceil(conWrap.clientHeight / this.itemSize) -
      this.renderedLength;
    ...
    const prevSetCollection: TdData[] = this.getBufferCollection(
      index,
      index + this.renderedLength
    );
    this.parent.resourceBase.renderedResources = prevSetCollection;
    if (firstTDIndex === 0) {
      this.translateY = conWrap.scrollTop;
    } else {
      let height: number = this.parent.rowAutoHeight
        ? this.averageRowHeight
        : this.itemSize;
      height = height > 0 ? height : this.itemSize;
      this.translateY =
        conWrap.scrollTop - this.bufferCount * height > 0
          ? conWrap.scrollTop - this.bufferCount * height
          : 0;
    }
  ...
  private downScroll(conWrap: HTMLElement, firstTDIndex: number): TdData[] {
    ...
    if (this.parent.rowAutoHeight) {
      nextSetResIndex =
        ~~((conWrap.scrollTop - this.translateY) / this.averageRowHeight) +
        firstTDIndex;
      nextSetResIndex =
        nextSetResIndex > firstTDIndex + this.bufferCount
          ? nextSetResIndex
          : firstTDIndex + this.bufferCount;
    }
    let lastIndex: number = nextSetResIndex + this.renderedLength;
    lastIndex =
      lastIndex > this.parent.resourceBase.expandedResources.length
        ? nextSetResIndex +
          (this.parent.resourceBase.expandedResources.length - nextSetResIndex)
        : lastIndex;
    const nextSetCollection: TdData[] = this.getBufferCollection(
      lastIndex - this.renderedLength,
      lastIndex
    );
    this.translateY = conWrap.scrollTop;
  ...
  private leftScroll(conWrap: HTMLElement): TdData[] {
    let index: number = 0;
    index =
      ~~(Math.abs(conWrap.scrollLeft) / this.itemSize) +
      Math.ceil(conWrap.clientWidth / this.itemSize) -
      this.renderedLength;
    index = index > 0 ? index : 0;
  ...
  private rightScroll(conWrap: HTMLElement): TdData[] {
    ...
    lastIndex =
      lastIndex > lastLevel.length - 1 ? lastLevel.length - 1 : lastIndex;
  ...
  private getResCollection(startIndex: number, endIndex: number): TdData[] {
    ...
    if (this.parent.activeViewOptions.group.byDate) {
      if (
        lastLevel[parseInt(startIndex.toString(), 10)].date.getTime() ===
      ...
      resCollection = this.getByDateCollection(
        lastLevel[parseInt(startIndex.toString(), 10)],
        lastLevel[parseInt(endIndex.toString(), 10)],
        index
      );
      this.setRenderedDates(resCollection);
    } else {
      if (
        lastLevel[parseInt(startIndex.toString(), 10)].groupIndex ===
          this.parent.resourceBase.expandedResources[0].groupIndex &&
      ...
      resCollection = this.getByIdCollection(
        lastLevel[parseInt(startIndex.toString(), 10)],
        lastLevel[parseInt(endIndex.toString(), 10)],
        index
      );
  ...
  private getByDateCollection(
    ...
    const resCollection: TdData[] = this.parent.activeView.colLevels[0].filter(
      (data: TdData) =>
        firstItem.date.getTime() <= data.date.getTime() &&
        data.date.getTime() <= lastItem.date.getTime()
    );
    this.setStartEndIndex(
      this.parent.activeView.colLevels[0],
      resCollection[0],
      resCollection[resCollection.length - 1],
      index
    );
  ...
  private getByIdCollection(
    ...
    const resCollection: TdData[] =
      this.parent.resourceBase.lastResourceLevel.filter(
        (data: TdData) =>
          firstItem.groupIndex <= data.groupIndex &&
          data.groupIndex <= lastItem.groupIndex
      );
  ...
  public updateContent(
    resWrap: HTMLElement,
    conWrap: HTMLElement,
    eventWrap: HTMLElement,
    resCollection: TdData[]
  ): void {
    const renderedLength: number =
      resWrap.querySelector('tbody').children.length;
    if (
      document.activeElement &&
      document.activeElement.classList.contains(cls.RESOURCE_CELLS_CLASS)
    ) {
    ...
    const currentGroupIndices: number[] =
      this.parent.activeView.getGroupIndices(resCollection);
    const previousGroupIndices: number[] =
      this.parent.activeView.getGroupIndices(this.existingDataCollection);
    const newGroupIndices: number[] = currentGroupIndices.filter(
      (index: number) => previousGroupIndices.indexOf(index) < 0
    );
    const resWrapRows: HTMLElement[] = Array.from(
      resWrap.querySelectorAll('tbody tr')
    );
    const conWrapRows: HTMLElement[] = Array.from(
      conWrap.querySelectorAll('tbody tr')
    );
    ...
    const resourceRows: Element[] = this.parent.resourceBase.getContentRows(
      resCollection,
      true
    );
    const contentRows: Element[] = this.parent.activeView.getContentRows();
    const eventRows: Element[] = this.parent.activeView.getEventRows(
      resCollection.length
    );
    for (let i: number = 0; i < newGroupIndices.length; i++) {
      const index: number = currentGroupIndices.indexOf(
        newGroupIndices[parseInt(i.toString(), 10)]
      );
      if (index === 0) {
        ...
        resWrapBody &&
        conWrapBody &&
        resWrapBody.children[parseInt(index.toString(), 10)] &&
        conWrapBody.children[parseInt(index.toString(), 10)]
      ) {
        resWrapBody.insertBefore(
          resourceRows[parseInt(i.toString(), 10)],
          resWrapBody.children[parseInt(index.toString(), 10)]
        );
        conWrapBody.insertBefore(
          contentRows[parseInt(i.toString(), 10)],
          conWrapBody.children[parseInt(index.toString(), 10)]
        );
      } else {
  ...
  private removeObsoleteRows(
    ...
    elements.forEach((element: HTMLElement) => {
      const groupIndex: number = parseInt(
        element.firstElementChild.getAttribute('data-group-index'),
        10
      );
  ...
  private updateHorizontalContent(
    ...
    const renderedRows: HTMLTableRowElement[] = Array.from(
  ...
  private updateMonthViewContent(
    ...
    if (this.parent.activeViewOptions.group.byDate) {
      ...
      this.parent.activeView.colLevels[
        this.parent.activeView.colLevels.length - 2
      ] = resCollection;
    }
  ...
  private updateOtherViewContent(
    ...
    const newGroupIndices: Set<number> = new Set(
      resCollection.map((data: TdData) => data.groupIndex)
    );
    ...
    const col: Element[] = [].slice.call(
      conWrap.querySelector('colgroup').children
    );
    ...
    const tHead: Element[] = [].slice.call(
      conWrap.querySelector('thead').children
    );
    ...
    if (this.parent.activeViewOptions.timeScale.enable) {
      thead.appendChild(
        this.parent.eventBase.createEventWrapper('timeIndicator')
      );
    }
    prepend([thead], table);
    const contentRows: Element[] = this.parent.activeView.getContentRows();
  ...
  private mergeNewTdData(tbody: Element, contentRows: Element[]): void {
    const existingRows: HTMLTableRowElement[] = Array.from(
      tbody.querySelectorAll<HTMLTableRowElement>('tr')
    );

    existingRows.forEach(
      (existingRow: HTMLTableRowElement, rowIndex: number) => {
        if (rowIndex < contentRows.length) {
          const newRow: Element =
            contentRows[parseInt(rowIndex.toString(), 10)];
          const existingTds: HTMLTableCellElement[] = Array.from(
            existingRow.querySelectorAll<HTMLTableCellElement>('td')
          );
          const newTds: HTMLTableCellElement[] = Array.from(
            newRow.querySelectorAll<HTMLTableCellElement>('td')
          );

          newTds.forEach((newTd: HTMLTableCellElement) => {
            const newGroupIndex: number = parseInt(
              newTd.getAttribute('data-group-index').toString(),
              10
            );
            let inserted: boolean = false;

            for (const existingTd of existingTds) {
              const existingGroupIndex: number = parseInt(
                existingTd.getAttribute('data-group-index').toString(),
                10
              );
  ...
  private getBufferCollection(startIndex: number, endIndex: number): TdData[] {
    return this.parent.resourceBase.expandedResources.slice(
      startIndex,
      endIndex
    );
  }
  ...
  public updateFocusedWorkCell(): void {
    if (this.focusedEle) {
      const date: number = parseInt(
        this.focusedEle.getAttribute('data-date'),
        10
      );
      const groupIndex: number = parseInt(
        this.focusedEle.getAttribute('data-group-index'),
        10
      );
      const ele: HTMLTableCellElement = this.parent.element.querySelector(
        `.${cls.WORK_CELLS_CLASS}[data-date="${date}"][data-group-index="${groupIndex}"]`
      );
      if (ele) {
  ...
  public setRenderedDates(resCollection: TdData[]): void {
    if (this.parent.currentView !== 'Month') {
      ...
      this.parent.resourceBase.renderedResources.forEach(
        (x: TdData) => (x.renderDates = dateCol)
      );
    } else {
      const dateCol: number[] = resCollection.map((x: TdData) =>
        x.date.getDay()
      );
      const renderDates: Date[] = this.parent.activeView.renderDates.filter(
        (x: Date) => dateCol.indexOf(x.getDay()) >= 0
      );
      this.parent.resourceBase.renderedResources.forEach(
        (x: TdData) => (x.renderDates = renderDates)
      );
    }
  }
  ...
  private setTabIndex(): void {
    ...
    const focusResCell: HTMLElement = this.parent.element.querySelector(
      `.${cls.RESOURCE_CELLS_CLASS}[tabindex="${0}"]`
    );
