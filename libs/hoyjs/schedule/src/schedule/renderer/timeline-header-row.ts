...
export class TimelineHeaderRow {
  ...
  private groupByYear(dates: Date[]): { [key: number]: Date[] } {
    ...
    for (const d of dates) {
      ...
      result[parseInt(key.toString(), 10)] =
        result[parseInt(key.toString(), 10)] || [];
  ...
  private groupByWeek(dates: Date[]): { [key: number]: Date[] } {
    ...
    for (const d of dates) {
      ...
      const tzOffsetDiff: number =
        d.getTimezoneOffset() - new Date(1970, 0, 1).getTimezoneOffset();
      let key: number = Math.ceil(
        ((+d - jsDate - tzOffsetDiff * 60 * 1000) / MS_PER_DAY +
          new Date(jsDate).getDay() +
          1) /
          7
      );
      if (
        this.parent.activeViewOptions.firstDayOfWeek &&
        this.parent.activeViewOptions.firstDayOfWeek > new Date(+d).getDay()
      ) {
        key = key - 1;
      }
      result[parseInt(key.toString(), 10)] =
    ...
    for (let i: number = 0; i < keys.length; i++) {
      ...
      if (row.template) {
        ...
        const templateId: string =
          this.parent.element.id + '_headerRowTemplate';
        htmlCol = [].slice.call(
          this.parent.templateParser(row.template)(
            args,
            this.parent,
            'template',
            templateId,
        ...
        switch (row.option) {
          case 'Year':
            viewTemplate = `<span class="e-header-year">${dateParser(
              dates[0],
              'y'
            )}</span>`;
            break;
          case 'Month':
            viewTemplate = `<span class="e-header-month">${capitalizeFirstWord(
              dateParser(dates[0], 'MMMM'),
              'single'
            )}</span>`;
            break;
          case 'Week':
            viewTemplate = `<span class="e-header-week">${this.parent.getWeekNumberContent(
              dates
            )}</span>`;
        }
