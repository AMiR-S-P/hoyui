...
export class Month extends ViewBase implements IRenderer {
  ...
  public onScrollUIUpdate(args: NotifyEventArgs): void {
    ...
    const height: number =
      this.parent.element.offsetHeight - headerHeight - header.offsetHeight;
    const leftPanel: HTMLElement = this.getLeftPanelElement();
    ...
    if (content.offsetWidth - content.clientWidth > 0) {
      (<HTMLElement>header.firstElementChild).style[
    ...
    if (
      ...
      if (this.parent.uiStateValues.isInitial) {
        ...
        content.scrollLeft =
          this.parent.currentView === 'TimelineMonth'
            ? this.parent.uiStateValues.left
  ...
  private scrollToSelectedDate(): void {
    const headerCell: HTMLElement = this.element.querySelector(
      '.' +
  ...
  public setContentHeight(
    ...
    if (leftPanelElement) {
      if (this.parent.currentView === 'MonthAgenda') {
        height =
          this.parent.activeViewOptions.interval > 1
            ? (<HTMLElement>this.getContentAreaElement().firstElementChild)
                .offsetHeight
            : (<HTMLElement>(
                this.element.querySelector('.' + cls.CONTENT_TABLE_CLASS)
              )).offsetHeight;
      }
      leftPanelElement.style.height = 'auto';
      leftPanelElement.style.height = formatUnit(
        height - this.getScrollXIndent(content)
      );
    }
  }
  ...
  public generateColumnLevels(): TdData[][] {
    ...
    const level: TdData[] = this.getDateSlots(
      this.renderDates,
    ...
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      ...
      if (
        ...
        let levels: TdData[] = this.getDateSlots(
        ...
        if (
          ...
          const index: number = levels.findIndex(
            (x: TdData, index: number) =>
  ...
  public getDateSlots(renderDates: Date[], workDays: number[]): TdData[] {
    const count: number = this.parent.activeViewOptions.showWeekend
      ? util.WEEK_LENGTH
      : workDays.length;
    const dateSlots: TdData[] = [];
    const isCurrentMonth: boolean = this.isCurrentMonth(
      this.parent.selectedDate
    );
    for (let col: number = 0; col < count; col++) {
      const classList: string[] = [cls.HEADER_CELLS_CLASS];
      const currentDateIndex: number[] = renderDates
        .slice(0, count)
        .map((date: Date) => date.getDay());
      if (
        isCurrentMonth &&
        currentDateIndex.indexOf(this.parent.currentTimezoneDate.getDay()) ===
          col
      ) {
  ...
  public renderLayout(type: string): void {
    ...
    if (
      this.parent.uiStateValues.isGroupAdaptive &&
      !target.querySelector('.' + cls.RESOURCE_TOOLBAR_CONTAINER)
    ) {
  ...
  public refreshHeader(): void {
    ...
    const contentWrapper: HTMLElement = this.element.querySelector(
  ...
  public renderContent(): void {
    ...
    if (this.parent.currentView === 'Month') {
      ...
      const monthAgendaWrapper: HTMLElement = createElement('div', {
  ...
  private renderWeekNumberContent(): HTMLElement {
    const dateCol: Date[] = this.renderDates.map(
    ...
    const contentWrapTable: HTMLElement =
      this.createTableLayout() as HTMLElement;
    contentWrapper.appendChild(contentWrapTable);
    const noOfDays: number = this.parent.activeViewOptions.showWeekend
      ? util.WEEK_LENGTH
      : this.parent.activeViewOptions.workDays.length;
    for (
      let i: number = 0, length: number = this.renderDates.length / noOfDays;
      i < length;
      i++
    ) {
      ...
      contentWrapTable
        .querySelector('tbody')
        .appendChild(this.createWeekNumberElement(weekNumber));
    }
    return td;
  }
  ...
  private createHeaderCell(td: TdData): Element {
    ...
    if (td.type === 'monthDay') {
      const ele: Element = createElement('span', {
        innerHTML: util.capitalizeFirstWord(
          this.parent.getDayNames(this.dayNameFormat)[td.date.getDay()],
          'single'
        ),
      });
    ...
    if (td.type === 'dateHeader') {
      ...
      if (this.parent.activeViewOptions.dateHeaderTemplate) {
        ...
        const title: string = this.parent.globalize.formatDate(td.date, {
          skeleton: skeleton,
          calendar: this.parent.getCalendarMode(),
        });
        ele.setAttribute('title', util.capitalizeFirstWord(title, 'multiple'));
        const innerText: string =
          this.parent.calendarUtil.isMonthStart(td.date) &&
          !this.isCurrentDate(td.date) &&
          !this.parent.isAdaptive
  ...
  public getContentSlots(): TdData[][] {
    if (
      !(
        ...
        this.colLevels[this.colLevels.length - 1][0]
      )
    ) {
      return [];
    }
    const slotDatas: TdData[][] = [];
    const prepareSlots: CallbackFunction = (
      rowIndex: number,
      renderDate: Date,
      resData: TdData,
      classList?: string[]
    ...
    if (
      ...
      for (const res of this.colLevels[this.colLevels.length - 2]) {
        const dates: Date[] = res.renderDates.map(
          (date: Date) => new Date(+date)
        );
        const count: number = this.parent.activeViewOptions.showWeekend
          ? util.WEEK_LENGTH
      ...
      const dates: Date[] = this.renderDates.map(
        (date: Date) => new Date(+date)
      );
      const count: number = this.parent.activeViewOptions.showWeekend
        ? util.WEEK_LENGTH
      ...
      const startIndex: number = this.renderDates
        .map(Number)
        .indexOf(+level[0].date);
      for (let i: number = 0; i < this.renderDates.length / count; i++) {
        const colDates: Date[] = dates.splice(0, count);
        let k: number = startIndex;
        for (
          let j: number = startIndex;
          j < this.colLevels[0].length + startIndex && j < colDates.length;
          j++
        ) {
          ...
          if (includeResource) {
            ...
            if (this.parent.activeViewOptions.group.hideNonWorkingDays) {
              resourcesTd = lastRow.filter(
                (x: TdData) => x.date.getDay() === colDate.getDay()
              );
            ...
            for (
              ...
              prepareSlots(
                i,
                colDate,
                resourcesTd[parseInt(resIndex.toString(), 10)],
                clsList
              );
            }
          } else {
  ...
  private isOtherMonth(date: Date): boolean {
    return (
      date.getTime() < this.monthDates.start.getTime() ||
      date.getTime() > this.monthDates.end.getTime()
    );
  }
  ...
  public renderContentArea(): Element {
    ...
    this.monthDates = {
      ...
      end: this.parent.calendarUtil.lastDateOfMonth(
        util.addMonths(monthDate, this.parent.activeViewOptions.interval - 1)
      ),
    };
  ...
  public getContentRows(): Element[] {
    ...
    const existingGroupIndices: number[] = isTimeline
      ? this.getGroupIndices()
      : [];
    for (let row: number = 0; row < slotDatas.length; row++) {
      if (
        existingGroupIndices.length > 0 &&
        existingGroupIndices.indexOf(
          slotDatas[parseInt(row.toString(), 10)][0].groupIndex
        ) > -1
      ) {
        continue;
      }
      ...
      for (
        let col: number = 0;
        col < slotDatas[parseInt(row.toString(), 10)].length;
        col++
      ) {
        const ntd: Element = this.createContentTd(
          slotDatas[parseInt(row.toString(), 10)][parseInt(col.toString(), 10)],
          td
        );
        ntr.appendChild(ntd);
      }
  ...
  public createContentTd(data: TdData, td: Element): Element {
    ...
    if (
      ...
      this.isWorkDay(data.date, data.workDays)
    ) {
    ...
    if (
      !isNullOrUndefined(data.groupIndex) ||
      this.parent.uiStateValues.isGroupAdaptive
    ) {
  ...
  private renderDateHeaderElement(data: TdData, ntd: Element): void {
    ...
    const dateHeader: Element = createElement('div', {
    ...
    if (this.parent.activeViewOptions.cellHeaderTemplate) {
      ...
      const innerText: string =
        this.parent.calendarUtil.isMonthStart(data.date) &&
        !this.isCurrentDate(data.date) &&
        !this.parent.isAdaptive
    ...
    if (this.isCustomMonth()) {
      ...
      const numberOfDays: number =
        ...
        (this.parent.activeViewOptions.numberOfWeeks > 0
          ? this.parent.activeViewOptions.numberOfWeeks
          : util.DEFAULT_WEEKS);
      return util.addDays(start, numberOfDays - 1);
    } else {
      const endDate: Date = util.addMonths(
        currentDate,
        this.parent.activeViewOptions.interval - 1
      );
      const lastWeekOfMonth: Date = util.getWeekFirstDate(
        this.parent.calendarUtil.lastDateOfMonth(endDate),
        this.parent.activeViewOptions.firstDayOfWeek
      );
  ...
  private isCustomRange(): boolean {
    ...
    if (dates && dates.length > 0) {
      const selectedTime: number = util
        .resetTime(this.parent.selectedDate)
        .getTime();
  ...
  public getNextPreviousDate(type: string): Date {
    if (this.isCustomMonth()) {
      const dates: Date[] = this.parent.getCurrentViewDates();
      const date: Date = util.getWeekFirstDate(
        type === 'next' ? dates[dates.length - 1] : dates[0],
        this.parent.activeViewOptions.firstDayOfWeek
      );
      return util.addDays(
        date,
        type === 'next'
          ...
          : -(this.parent.activeViewOptions.numberOfWeeks > 0
              ? this.parent.activeViewOptions.numberOfWeeks
              : util.DEFAULT_WEEKS) * util.WEEK_LENGTH
      );
    } else {
      return util.addMonths(
        this.parent.selectedDate,
        (type === 'next' ? 1 : -1) * this.parent.activeViewOptions.interval
      );
    }
  }
  ...
  public getDateRangeText(): string {
    if (
      this.parent.isAdaptive ||
      isNullOrUndefined(this.parent.activeViewOptions.dateFormat)
    ) {
      ...
      const isUpdateDateRange: boolean =
        this.parent.currentView !== 'Month' || !this.isCustomMonth();
      if (
        (this.parent.activeViewOptions.interval > 1 && isUpdateDateRange) ||
        updateCustomRange
      ) {
        endDate = endDate
          ? endDate
          : util.addMonths(
              util.lastDateOfMonth(startDate),
              this.parent.activeViewOptions.interval - 1
            );
        if (startDate.getFullYear() === endDate.getFullYear()) {
          const monthNames: string =
            this.parent.globalize.formatDate(startDate, {
              format: 'MMMM',
              calendar: this.parent.getCalendarMode(),
            }) +
            ' - ' +
      ...
      const format: string = this.parent.activeViewOptions.dateFormat
  ...
  private createWeekNumberElement(text?: string): HTMLElement {
    ...
    const td: HTMLElement = createElement('td', {
      ...
      attrs: {
        title: text
          ? this.parent.localeObj.getConstant('week') + ' ' + text
          : '',
      },
      innerHTML: text || '',
    });
  ...
  public unWireEvents(): void {
    const contentScrollableEle: Element = this.element.querySelector(
      '.' + cls.CONTENT_WRAP_CLASS
    );
