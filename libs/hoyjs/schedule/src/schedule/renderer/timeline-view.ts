...
export class TimelineViews extends VerticalView {
  ...
  public scrollToWorkHour(): void {
    ...
    const currDateTime: number =
      this.isWorkDay(this.parent.selectedDate) &&
      this.parent.workHours.highlight &&
      !isNullOrUndefined(start)
        ? new Date(+this.parent.selectedDate).setHours(
            start.getHours(),
            start.getMinutes(),
            0,
            0
    ...
    const queryString: string =
      '[data-date="' + new Date(currDateTime).getTime().toString() + '"]';
    const firstWorkHourCell: HTMLElement = this.element.querySelector(
      queryString
    ) as HTMLElement;
    if (firstWorkHourCell) {
      this.getContentAreaElement().scrollLeft = !this.parent.enableRtl
        ? firstWorkHourCell.offsetLeft
        : -(
            this.parent.getContentTable().offsetWidth -
            firstWorkHourCell.offsetLeft -
            firstWorkHourCell.offsetWidth
          );
    }
  }
  ...
  public scrollToHour(hour: string, scrollDate: Date): void {
    ...
    let index: number;
    if (scrollDate) {
      index = this.parent.getIndexOfDate(
        this.renderDates,
        util.resetTime(scrollDate)
      );
      if (index >= 0) {
        date = scrollDate;
    ...
    date = isNullOrUndefined(scrollDate)
      ? this.parent.getStartEndTime(hour)
      : date;
    ...
    if (
      ...
      scrollLeft =
        ...
        this.parent.getElementWidth(
          this.element.querySelector('.e-work-cells')
        );
    } else {
      scrollLeft = isNullOrUndefined(scrollDate)
        ? this.getLeftFromDateTime(null, date)
        : this.getLeftFromDateTime([index], date);
    }
    this.getScrollableElement().scrollLeft = !this.parent.enableRtl
      ? scrollLeft
      : -scrollLeft;
  }
  ...
  public generateColumnLevels(): TdData[][] {
    ...
    if (this.parent.activeViewOptions.headerRows.length > 0) {
      const renderGn: TimelineHeaderRow = new TimelineHeaderRow(
        this.parent,
        this.renderDates
      );
  ...
  private generateTimeSlots(dateSlots: TdData[]): TdData[] {
    const handler: CallbackFunction = (r: TimeSlotData): TimeSlotData => {
      ...
      r.className = r.first
        ? [cls.TIME_SLOT_CLASS]
        : [cls.TIME_SLOT_CLASS, cls.TIME_CELLS_CLASS];
    ...
    for (const data of dateSlots) {
      ...
      const tempTimeSlots: TdData[] = <TdData[]>(
        extend([], timeSlotData, null, true)
      );
  ...
  public changeCurrentTimePosition(): void {
    ...
    const left: number = this.getLeftFromDateTime(
      currentDateIndex,
      this.parent.getCurrentTime()
    );
    const height: number = (
      this.element.querySelector('.' + cls.CONTENT_TABLE_CLASS) as HTMLElement
    ).offsetHeight;
    const headerWrap: Element = this.element.querySelector(
      '.' + cls.DATE_HEADER_WRAP_CLASS
    );
    const contentWrap: Element = this.element.querySelector(
    ...
    if (this.parent.virtualScrollModule) {
      ...
      const element: HTMLElement = this.parent.element.querySelector(
    ...
    currentTimeEle.style[this.parent.enableRtl ? 'right' : 'left'] = formatUnit(
      left - currentTimeEle.offsetWidth / 2
    );
  }
  ...
  private getLeftFromDateTime(currentDateIndex: number[], date: Date): number {
    ...
    if (!isNullOrUndefined(currentDateIndex)) {
      if (currentDateIndex[0] !== 0) {
        const index: number = this.parent.activeView.colLevels.findIndex(
          (level: TdData[]) => level[0].type === 'dateHeader'
        );
        if (
          this.parent.activeView.colLevels[parseInt(index.toString(), 10)] &&
          this.parent.activeView.colLevels[parseInt(index.toString(), 10)][0]
            .colSpan
        ) {
          diffInDates =
            currentDateIndex[0] *
  ...
  private getResourceTdData(i: number, tdData: TdData): TdData {
    const resLevel: TdData =
      this.parent.resourceBase.renderedResources[parseInt(i.toString(), 10)];
    const resSHr: string =
      (resLevel.resourceData[resLevel.resource.startHourField] as string) ||
      this.parent.workHours.start;
    const resEHr: string =
      (resLevel.resourceData[resLevel.resource.endHourField] as string) ||
      this.parent.workHours.end;
    tdData.startHour = this.parent.getStartEndTime(resSHr);
    tdData.endHour = this.parent.getStartEndTime(resEHr);
    tdData.workDays =
      (resLevel.resourceData[resLevel.resource.workDaysField] as number[]) ||
      this.parent.workDays;
  ...
  public getContentTdClass(r: TimeSlotData): string[] {
    return r.first || !this.parent.activeViewOptions.timeScale.enable
      ? [cls.WORK_CELLS_CLASS]
      : [cls.WORK_CELLS_CLASS, cls.ALTERNATE_CELLS_CLASS];
  }

  public renderEvents(): void {
    this.timelineAppointment = new TimelineEvent(
      this.parent,
  ...
  public getAdjustedDate(date: Date): Date {
    if (!this.parent.activeViewOptions.timeScale.enable) {
      ...
      for (let i: number = 0; i < timeSlots.length; i++) {
        if (
          timeSlots[parseInt(i.toString(), 10)].date.getTime() > date.getTime()
        ) {
