...
export class MonthAgenda extends Month {
  ...
  public renderAppointmentContainer(): void {
    ...
    const wrapperContainer: HTMLElement = createElement('div', {
      className: cls.WRAPPER_CONTAINER_CLASS,
    });
    contentArea.appendChild(wrapperContainer);
    const appWrap: HTMLElement = createElement('div', {
  ...
  public updateSelectedCellClass(data: TdData): void {
    if (
      ...
      util.resetTime(this.monthAgendaDate).getTime()
    ) {
  ...
  private setEventWrapperHeight(): void {
    let headerHeight: number =
      (this.parent.headerModule
        ? this.parent.headerModule.getHeaderElement().offsetHeight
        : 0) + 2;
    const resourceWrapper: HTMLElement = this.parent.element.querySelector(
    ...
    const availHeight: number =
      this.parent.element.offsetHeight - headerHeight - dateHeader.offsetHeight;
    const contentAreaHeight: number =
      this.parent.activeViewOptions.interval > 1
        ? Math.round(availHeight * 0.8)
        : contentArea.offsetHeight;
    const appContainerHeight: number = availHeight - contentAreaHeight;
    const wrapperContainer: HTMLElement = this.element.querySelector(
      '.' + cls.WRAPPER_CONTAINER_CLASS
    ) as HTMLElement;
    const eventWrapper: HTMLElement = this.element.querySelector(
      '.' + cls.APPOINTMENT_WRAP_CLASS
    ) as HTMLElement;
    if (this.parent.height !== 'auto') {
      ...
      wrapperContainer.style.height = eventWrapper.style.height =
        formatUnit(appContainerHeight);
    }
  }
  ...
  public onDataReady(args: NotifyEventArgs): void {
    ...
    this.parent.eventsProcessed =
      this.agendaBase.processAgendaEvents(eventCollection);
    let count: number = 0;
    for (const date of this.renderDates) {
      const filterData: Record<string, any>[] = this.appointmentFiltering(date);
      const workCell: Element = this.element.querySelectorAll(
      ...
      if (filterData.length > 0) {
        if (!workCell.querySelector('.' + cls.APPOINTMENT_INDICATOR_CLASS)) {
          workCell.appendChild(
            createElement('div', { className: cls.APPOINTMENT_INDICATOR_CLASS })
          );
        }
        if (
          date.getTime() ===
          util.resetTime(new Date(this.monthAgendaDate.getTime())).getTime()
        ) {
  ...
  public onCellClick(event: CellClickEventArgs): void {
    ...
    const filterData: Record<string, any>[] = this.appointmentFiltering(
      event.startTime
    );
  ...
  private onEventRender(events: Record<string, any>[], date?: Date): void {
    this.agendaBase = this.getAgendaBase();
    const appWrap: Element = this.element.querySelector(
      '.' + cls.APPOINTMENT_WRAP_CLASS
    );
    ...
    if (
      ...
      if (events.length > 0) {
        const appContainer: Element = createElement('div', {
          className: cls.APPOINTMENT_CONTAINER_CLASS,
        });
        appWrap.appendChild(
          this.agendaBase.createAgendaContentElement(
            'data',
            events,
            appContainer
          )
  ...
  private appointmentFiltering(date: Date): Record<string, any>[] {
    ...
    const dateEnd: Date = util.setTime(
  ...
  private clearElements(): void {
    const appointmentIndicators: HTMLElement[] = [].slice.call(
      this.element.querySelectorAll('.' + cls.APPOINTMENT_INDICATOR_CLASS)
    );
    for (const appointmentIndicator of appointmentIndicators) {
      remove(appointmentIndicator);
    }
    this.appendAppContainer(
      this.element.querySelector('.' + cls.APPOINTMENT_WRAP_CLASS)
    );
  }

  private appendAppContainer(appWrap: Element): void {
    const app: HTMLElement = createElement('div', {
      className: cls.APPOINTMENT_CONTAINER_CLASS,
    });
  ...
  public getNextPreviousDate(type: string): Date {
    ...
    const interval: number =
      type === 'next'
        ...
        : -this.parent.activeViewOptions.interval;
    const navigateDate: Date = util.addMonths(
    ...
    const date: number =
      lastDate >= this.monthAgendaDate.getDate()
        ? this.monthAgendaDate.getDate()
        : lastDate;
    this.monthAgendaDate = new Date(
      navigateDate.getFullYear(),
      navigateDate.getMonth(),
      date
    );
