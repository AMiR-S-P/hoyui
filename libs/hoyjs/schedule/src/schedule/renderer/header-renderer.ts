...
export class HeaderRenderer {
  ...
  private closeHeaderPopup(e: { event: Event }): void {
    const closestEle: Element = closest(
      e.event.target as HTMLElement,
      '.e-date-range,.e-header-popup,.e-day,.e-selected'
    );
    const closestPop: Element = closest(
      e.event.target as HTMLElement,
      '.e-hor-nav,.e-toolbar-pop'
    );
    ...
    if (this.parent.isAdaptive) {
      if (
        !isNullOrUndefined(closestPop) &&
        (closestPop.classList.contains('e-toolbar-pop') ||
          closestPop.classList.contains('e-hor-nav')) &&
        !(
          closestPop.classList.contains('e-hor-nav') &&
          this.element
        ...
        const popupEle: HTMLElement = this.element.querySelector(
          '.e-toolbar-pop'
        ) as HTMLElement;
        if (!isNullOrUndefined(popupEle)) {
          const popupObj: Popup = (popupEle as EJ2Instance)
  ...
  private renderToolbar(): void {
    const items: ItemModel[] =
      this.parent.toolbarItems && this.parent.toolbarItems.length > 0
        ? this.getToolbarItems()
        : this.getItems();
    this.parent.trigger(
      events.actionBegin,
      { requestType: 'toolbarItemRendering', items: items },
      (args: ActionEventArgs) => {
        ...
        this.toolbarObj.root = this.parent.root
          ? this.parent.root
          : this.parent;
        this.toolbarObj.appendTo(
          this.parent.element.querySelector(
            '.' + cls.HEADER_TOOLBAR
          ) as HTMLElement
        );
        this.toolbarObj.element.setAttribute('aria-label', 'Scheduler');
        const prevNavEle: HTMLElement = this.toolbarObj.element.querySelector(
          '.e-prev'
        ) as HTMLElement;
        if (prevNavEle) {
          (prevNavEle.firstElementChild as Element).setAttribute(
            'title',
            this.l10n.getConstant('previous')
          );
        }
        const nextNavEle: HTMLElement = this.toolbarObj.element.querySelector(
          '.e-next'
        ) as HTMLElement;
        if (nextNavEle) {
          (nextNavEle.firstElementChild as Element).setAttribute(
            'title',
            this.l10n.getConstant('next')
          );
        }
        this.updateAddIcon();
  ...
  public getCalendarView(): CalendarView {
    if (
      ['Month', 'MonthAgenda', 'TimelineMonth'].indexOf(
  ...
  public updateActiveView(): void {
    const selEle: HTMLElement[] = [].slice.call(
      this.toolbarObj.element.querySelectorAll('.e-views')
    );
  ...
  public updateDateRange(date?: Date): void {
    const selEle: Element =
      this.toolbarObj.element.querySelector('.e-date-range');
    if (!selEle) {
      return;
    }
    ...
    if (this.parent.activeViewOptions.dateRangeTemplate) {
      ...
      const viewName: string =
        this.parent.activeViewOptions.dateRangeTemplateName;
      const templateId: string =
        this.parent.element.id + '_' + viewName + 'dateRangeTemplate';
      const dateTemplate: Element[] = [].slice.call(
        this.parent.getDateRangeTemplate()(
          args,
          this.parent,
          'dateRangeTemplate',
          templateId,
  ...
  public updateAddIcon(): void {
    const addEle: HTMLElement = this.toolbarObj.element.querySelector(
      '.e-add'
    ) as HTMLElement;
    if (addEle) {
  ...
  private getDateRangeText(): string {
    const dateString: string = this.parent.globalize.formatDate(
  ...
  private getItemObject(item: View | ViewsModel): ItemModel {
    ...
    const isItemInsidePopup: boolean =
      this.parent.isAdaptive || this.parent.enableAdaptiveUI;
    switch (viewName) {
      case 'day':
        view = {
      ...
      case 'timelineyear':
        orientationClass =
          (item as ViewsModel).orientation === 'Vertical'
            ? 'vertical'
            : 'horizontal';
        view = {
          align: 'Right',
  ...
  private renderHeaderPopup(): void {
    const headerPopupEle: HTMLElement = createElement('div', {
      className: cls.HEADER_POPUP_CLASS,
    });
    const headerCalendarEle: HTMLElement = createElement('div', {
  ...
  public setCalendarTimezone(): void {
    if (this.headerCalendar) {
      (this.headerCalendar as any).timezone =
  ...
  private calculateViewIndex(args: ClickEventArgs): number {
    ...
    const views: Element[] = [].slice.call(
      this.element.querySelectorAll('.e-views')
    );
  ...
  private toolbarClickHandler(args: ClickEventArgs): void {
    ...
    switch (strClass) {
      ...
      case 'e-today':
        ...
        if (
          ...
          !this.parent.isSelectedDate(currentTime) ||
          (this.parent.currentView === 'Month' &&
            this.parent.activeViewOptions.displayDate &&
            !this.hasSelectedDate() &&
            util.resetTime(this.parent.activeViewOptions.displayDate) !==
              currentTime) ||
          (this.parent.currentView === 'Month' &&
            this.parent.activeViewOptions.numberOfWeeks > 0 &&
            !this.hasSelectedDate() &&
            util.resetTime(util.firstDateOfMonth(this.parent.selectedDate)) !==
              currentTime)
        ) {
      ...
      case 'e-add':
        ...
        if (this.parent.activeCellsData && !isSameTime) {
          ...
          const msInterval: number =
            (interval * util.MS_PER_MINUTE) / slotCount;
        ...
        this.parent.eventWindow.openEditor(
          <Record<string, any>>(
            extend(data, { cancel: false, event: args.originalEvent })
          ),
          'Add'
        );
        break;
    ...
    const toolbarPopUp: HTMLElement = <HTMLElement>(
      this.toolbarObj.element.querySelector('.e-toolbar-pop')
    );
    if (toolbarPopUp && args.item.type !== 'Input') {
      ((toolbarPopUp as EJ2Instance).ej2_instances[0] as Popup).hide({
        name: 'SlideUp',
        duration: 100,
      });
    }
  ...
  private hasSelectedDate(): boolean {
    const selectedTime: number = util
      ...
      .getTime();
    return (
      selectedTime >= this.parent.activeView.getStartDate().getTime() &&
  ...
  public updateHeaderItems(classType: string): void {
    ...
    const dateRangeEle: HTMLElement = this.toolbarObj.element.querySelector(
      '.e-date-range'
    ) as HTMLElement;
    if (prevNavEle) {
      if (classType === 'add') {
  ...
  public previousNextIconHandler(): void {
    const dates: Date[] = (
      this.parent.currentView === 'Agenda'
        ? [this.parent.getCurrentViewDates()[0]]
        : this.parent.getCurrentViewDates()
    ) as Date[];
    const prevNavEle: HTMLElement = this.toolbarObj.element.querySelector(
      '.' + cls.PREVIOUS_DATE_CLASS
    );
    const nextNavEle: HTMLElement = this.toolbarObj.element.querySelector(
      '.' + cls.NEXT_DATE_CLASS
    );
    ...
    if (
      ...
      firstDate = util.getWeekFirstDate(
        util.resetTime(this.parent.selectedDate),
        this.parent.activeViewOptions.firstDayOfWeek
      );
      lastDate = util.addDays(
        firstDate,
        7 * this.parent.activeViewOptions.interval
      );
    } else if (this.parent.currentView === 'Month') {
      const isCustomMonth: boolean =
        !isNullOrUndefined(this.parent.activeViewOptions.displayDate) ||
        this.parent.activeViewOptions.numberOfWeeks > 0;
      firstDate = isCustomMonth
        ? this.parent.activeView.getStartDate()
        : util.firstDateOfMonth(this.parent.selectedDate);
      lastDate = isCustomMonth
        ? this.parent.activeView.getEndDate()
        : util.lastDateOfMonth(
            util.addMonths(
              firstDate,
    ...
    if (!isNullOrUndefined(prevNavEle)) {
      this.toolbarObj.enableItems(
        prevNavEle,
        firstDate > util.resetTime(this.parent.minDate)
      );
    }
    if (!isNullOrUndefined(nextNavEle)) {
      this.toolbarObj.enableItems(
        nextNavEle,
        lastDate < util.resetTime(this.parent.maxDate)
      );
    }
