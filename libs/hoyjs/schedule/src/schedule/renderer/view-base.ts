...
export class ViewBase {
  ...
  public refreshResourceHeader(): void {
    const resTbl: HTMLElement = this.element.querySelector(
      '.' + cls.RESOURCE_COLUMN_TABLE_CLASS
    ) as HTMLElement;
    const resourceTd: HTMLElement[] = Array.from(
      resTbl.querySelectorAll('.' + cls.RESOURCE_CELLS_CLASS)
    ) as HTMLElement[];
    resourceTd.forEach((currentElement: HTMLElement) => {
      ...
      children.forEach((child: HTMLElement) => {
        if (
          !child.classList.contains(cls.RESOURCE_EXPAND_CLASS) &&
          !child.classList.contains(cls.RESOURCE_COLLAPSE_CLASS)
        ) {
          remove(child);
        }
      });
  ...
  public getTime(date: Date): string {
    if (this.parent.isAdaptive) {
      if (
        this.parent.activeViewOptions.timeFormat === 'HH:mm' ||
        this.parent.activeViewOptions.timeFormat === 'HH.mm'
      ) {
        return this.parent.globalize.formatDate(date, {
          format: 'H',
  ...
  public createEventTable(trCount: number): Element {
    const eventTable: Element = createElement('div', {
  ...
  public collapseRows(wrap: Element): void {
    if (
      ...
      this.parent.resourceBase.hideResourceRows(
        wrap.querySelector('.' + cls.EVENT_TABLE_CLASS)
      );
    }
  }
  ...
  public setAriaAttributes(table: Element): void {
    ...
    table.setAttribute(
      'aria-label',
      this.getLabelText(this.parent.currentView)
    );
  }
  ...
  public createColGroup(table: Element, lastRow: TdData[]): void {
    ...
    if (lastRow[0] && lastRow[0].colSpan) {
      length = lastRow
        .map((value: TdData) => value.colSpan)
  ...
  public getScrollXIndent(content: HTMLElement): number {
    return content.offsetHeight - content.clientHeight > 0
      ? util.getScrollBarWidth()
      : 0;
  }

  public scrollTopPanel(target: HTMLElement): void {
    (this.getDatesHeaderElement().firstElementChild as Element).scrollLeft =
      target.scrollLeft;
  }

  public scrollHeaderLabels(target: HTMLElement): void {
    const headerTable: HTMLElement = this.element.querySelector(
      '.e-date-header-wrap table'
    ) as HTMLElement;
    const colWidth: number =
      headerTable.offsetWidth /
      headerTable.querySelectorAll('colgroup col').length;
    const applyLeft: CallbackFunction = (
      ...
      if (!isNullOrUndefined(currentCell)) {
        (currentCell.children[0] as HTMLElement).style[
          isRtl ? 'right' : 'left'
        ] = hiddenLeft - tdLeft + 'px';
      }
    };
    ...
    for (const className of classNames) {
      const headerCells: HTMLElement[] = [].slice.call(
        this.element.querySelectorAll(className)
      );
  ...
  public getHeaderBarHeight(): number {
    ...
    if (this.parent.headerModule) {
      headerBarHeight += util.getOuterHeight(
        this.parent.headerModule.getHeaderElement()
      );
    }
    if (this.parent.uiStateValues.isGroupAdaptive) {
      const resHeader: HTMLElement = <HTMLElement>(
        this.parent.element.querySelector('.' + cls.RESOURCE_HEADER_TOOLBAR)
      );
      if (resHeader) {
  ...
  public getDatesHeaderElement(): HTMLElement {
    return this.element.querySelector(
      '.' + cls.DATE_HEADER_CONTAINER_CLASS
    ) as HTMLElement;
  }
  ...
  public getStartHour(): Date {
    let startHour: Date = this.parent.getStartEndTime(
  ...
  public getEndHour(): Date {
    let endHour: Date = this.parent.getStartEndTime(
  ...
  public isCurrentDate(date: Date): boolean {
    return (
      ...
      new Date(this.parent.currentTimezoneDate).setHours(0, 0, 0, 0)
    );
  }

  public isCurrentMonth(date: Date): boolean {
    if (
      this.parent.activeViewOptions.displayDate ||
      this.parent.activeViewOptions.numberOfWeeks > 0
    ) {
      return (
        this.parent.activeView.getStartDate().getTime() <=
          this.parent.getCurrentTime().getTime() &&
        this.parent.activeView.getEndDate().getTime() >=
          this.parent.getCurrentTime().getTime()
      );
    }
    return (
      date.getFullYear() === this.parent.getCurrentTime().getFullYear() &&
      date.getMonth() === this.parent.getCurrentTime().getMonth()
    );
  }

  public isWorkDay(
    date: Date,
    workDays: number[] = this.parent.activeViewOptions.workDays
  ): boolean {
  ...
  public isWorkHour(
    ...
    return !(
      util.getDateInMs(date) < util.getDateInMs(startHour) ||
      util.getDateInMs(date) >= util.getDateInMs(endHour) ||
  ...
  public getRenderDates(workDays?: number[]): Date[] {
    ...
    if (
      ...
      this.parent.currentView === 'TimelineWeek'
    ) {
      const selectedDate: Date = util.resetTime(this.parent.selectedDate);
      let start: Date = util.getWeekFirstDate(
        selectedDate,
      ...
      for (
        let i: number = 0,
          length: number =
            util.WEEK_LENGTH * this.parent.activeViewOptions.interval;
        i < length;
        i++
      ) {
      ...
      let start: Date = util.getWeekFirstDate(
        util.resetTime(this.parent.selectedDate),
        this.parent.activeViewOptions.firstDayOfWeek
      );
      for (
        let i: number = 0,
  ...
  public getNextPreviousDate(type: string): Date {
    ...
    const weekLength: number =
      type === 'next' ? util.WEEK_LENGTH : -util.WEEK_LENGTH;
    return util.addDays(
      this.parent.selectedDate,
      weekLength * this.parent.activeViewOptions.interval
    );
  }

  public formatViewLabel(view: string, startDate: Date, endDate: Date): string {
    const formatOptions: { type: string; skeleton: string; calendar: string } =
      {
        type: 'date',
        skeleton: 'full',
        calendar: this.parent.getCalendarMode(),
    ...
    return (
      this.parent.localeObj.getConstant(view) +
      ' ' +
      this.parent.localeObj.getConstant('start') +
      ' ' +
      this.parent.globalize.formatDate(startDate, formatOptions) +
      ' ' +
      this.parent.localeObj.getConstant('endAt') +
      ' ' +
      this.parent.globalize.formatDate(endDate, formatOptions)
    );
  }
  ...
  public getLabelText(view: string): string {
    ...
    if (view === 'Year' || view === 'TimelineYear') {
      return this.formatViewLabel(
        viewStr,
        this.parent.activeView.getStartDate(),
        this.parent.activeView.getEndDate()
      );
    } else {
      if (this.renderDates.length > 0) {
        if (
          ...
          return (
            this.parent.localeObj.getConstant(viewStr) +
            ' of ' +
          ...
          return this.formatViewLabel(
            viewStr,
            this.renderDates[0],
            this.renderDates[this.renderDates.length - 1]
          );
        }
      } else {
        return '';
  ...
  public getDateRangeText(): string {
    if (this.parent.isAdaptive) {
      const formatDate: string = this.parent.activeViewOptions.dateFormat
        ? this.parent.activeViewOptions.dateFormat
        : 'MMMM y';
  ...
  public formatDateRange(startDate: Date, endDate?: Date): string {
    ...
    if (!isNullOrUndefined(this.parent.activeViewOptions.dateFormat)) {
      ...
      text =
        globalize.formatDate(startDate, {
          format: this.parent.activeViewOptions.dateFormat,
          calendar: mode,
        }) +
        ' - ' +
    ...
    if (dateFormat.substr(0, 1) === 'd') {
      if (startDate.getFullYear() === endDate.getFullYear()) {
        if (startDate.getMonth() === endDate.getMonth()) {
          formattedStr =
            globalize.formatDate(startDate, { format: 'dd', calendar: mode }) +
            ' - ' +
            globalize.formatDate(endDate, {
              format: 'dd MMMM yyyy',
              calendar: mode,
  ...
  public setResourceHeaderContent(
    tdElement: Element,
    tdData: TdData,
    className: string = cls.TEXT_ELLIPSIS
  ): void {
    if (this.parent.activeViewOptions.resourceHeaderTemplate) {
      ...
      const viewName: string =
        this.parent.activeViewOptions.resourceHeaderTemplateName;
      const templateId: string =
        scheduleId + viewName + 'resourceHeaderTemplate';
      const quickTemplate: HTMLElement[] = [].slice.call(
        this.parent.getResourceHeaderTemplate()(
          data,
          this.parent,
      ...
      const resourceText: HTMLElement = createElement('div', {
        className: className,
      });
      this.parent.sanitize(
        tdData.resourceData[tdData.resource.textField] as string,
        resourceText
      );
  ...
  public setColWidth(content: HTMLElement): void {
    if (this.isTimelineView()) {
      ...
      const contentBody: HTMLElement = this.element.querySelector(
        '.' + cls.CONTENT_TABLE_CLASS + ' tbody'
      ) as HTMLElement;
      const colWidth: number =
        this.parent.getElementWidth(contentBody) / (colElements.length / 2);
      if (content.offsetHeight !== content.clientHeight) {
        const resourceColumn: HTMLElement = this.parent.element.querySelector(
          '.' + cls.RESOURCE_COLUMN_WRAP_CLASS
        );
        if (
          !isNullOrUndefined(resourceColumn) &&
          resourceColumn.offsetHeight !== content.clientHeight
        ) {
      ...
      const headerCellElements: HTMLElement[] = [].slice.call(
        this.element.querySelectorAll(cssClass)
      );
      headerCellElements.forEach((ele: HTMLElement) => {
        const colSpan: string = isNullOrUndefined(ele.getAttribute('colspan'))
          ? '1'
          : ele.getAttribute('colspan');
  ...
  public wireExpandCollapseIconEvents(): void {
    if (
      ...
      const treeIcons: HTMLElement[] = [].slice.call(
        this.element.querySelectorAll('.' + cls.RESOURCE_TREE_ICON_CLASS)
      );
      for (const icon of treeIcons) {
        ...
        EventHandler.add(
          icon,
          'click',
          this.parent.resourceBase.onTreeIconClick,
          this.parent.resourceBase
        );
      }
    }
  ...
  public scrollToDate(scrollDate: Date): void {
    if (
      ['Month', 'TimelineMonth'].indexOf(this.parent.currentView) === -1 ||
      isNullOrUndefined(scrollDate)
    ) {
      return;
    }
    const scrollWrap: HTMLElement = this.getContentAreaElement();
    const tdDate: number = new Date(
      util.resetTime(new Date(+scrollDate)).getTime()
    ).getTime();
    const dateElement: HTMLElement = scrollWrap.querySelector(
      `.${cls.WORK_CELLS_CLASS}[data-date="${tdDate}"]`
    ) as HTMLElement;
      ...
      if (scrollWrap.scrollWidth > scrollWrap.clientWidth) {
        if (!this.parent.enableRtl) {
          ...
          scrollWrap.scrollLeft = -(
            this.parent.getContentTable().offsetWidth -
            dateElement.offsetLeft -
            dateElement.offsetWidth
          );
        }
      }
  ...
  public setPersistence(): void {
    if (this.parent.enablePersistence) {
      const contentWrap: HTMLElement = this.element.querySelector(
        '.e-content-wrap'
      ) as HTMLElement;
  ...
  public retainScrollPosition(): void {
    if (this.parent.enablePersistence) {
      ...
      if (
        !isNullOrUndefined(conWrap) &&
        !isNullOrUndefined(this.parent.scrollLeft) &&
        !isNullOrUndefined(this.parent.scrollTop)
      ) {
  ...
  public getViewEndDate(): Date {
    ...
    if (
      ...
      endDate = util.addDays(
        this.parent.resourceBase.getResourceRenderDates().slice(-1)[0],
        1
      );
    }
    return endDate;
  ...
  public resetColLevels(): void {
    ...
    const lastLevel: TdData[] = this.parent.activeViewOptions.group.byDate
      ? this.colLevels[0]
      : this.parent.resourceBase.renderedResources;
    let index: number = 0;
    for (let i: number = 0; i < lastLevel.length; i++) {
      ...
      this.parent.resourceBase.expandedResources.push(
        lastLevel[parseInt(i.toString(), 10)]
      );
    }
    if (this.parent.activeViewOptions.group.byDate) {
      this.colLevels[0] = this.parent.resourceBase.expandedResources;
      this.parent.virtualScrollModule.setRenderedDates(
        this.parent.resourceBase.expandedResources
      );
    } else {
      this.colLevels[this.colLevels.length - 2] =
        this.parent.resourceBase.expandedResources;
      this.parent.resourceBase.renderedResources =
        this.parent.resourceBase.expandedResources;
    }
    if (this.parent.currentView !== 'Month') {
      this.colLevels[this.colLevels.length - 1] = this.colLevels[
        this.colLevels.length - 1
      ].slice(0, index);
      this.parent.resourceBase.expandedResources =
        this.colLevels[this.colLevels.length - 1];
    }
  }
  ...
  public getGroupIndices(dataCollection?: TdData[]): number[] {
    ...
    if (
      ...
      dataCollection = isNullOrUndefined(dataCollection)
