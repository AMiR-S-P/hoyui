...
export class Year extends ViewBase implements IRenderer {
  ...
  public renderLayout(className: string): void {
    ...
    const calendarTable: HTMLElement = this.createTableLayout(
      cls.OUTER_TABLE_CLASS
    ) as HTMLElement;
  ...
  public renderContent(content: HTMLElement): void {
    ...
    cTr.appendChild(cTd);
    const calendarWrapper: HTMLElement = createElement('div', {
  ...
  public renderCalendarHeader(currentDate: Date): HTMLElement {
    const headerWrapper: HTMLElement = createElement('div', {
    ...
    if (this.parent.activeViewOptions.monthHeaderTemplate) {
      ...
      this.renderTemplates(
        this.parent.getMonthHeaderTemplate(),
        args,
        'monthHeaderTemplate',
        this.parent.activeViewOptions.monthHeaderTemplateName,
        headerContent
      );
    } else {
  ...
  public renderCalendarContent(currentDate: Date): HTMLElement {
    ...
    const contentTable: HTMLElement = this.createTableLayout(
      'e-calendar-table ' + cls.CONTENT_TABLE_CLASS
    ) as HTMLElement;
    ...
    const tr: HTMLElement = createElement('tr');
    let currentWeek: Date = util.getWeekFirstDate(
      util.firstDateOfMonth(currentDate),
    ...
    for (let i: number = 0; i < util.WEEK_LENGTH; i++) {
      if (this.parent.activeViewOptions.dayHeaderTemplate) {
        ...
        this.renderTemplates(
          this.parent.getDayHeaderTemplate(),
          args,
          'dayHeaderTemplate',
          this.parent.activeViewOptions.dayHeaderTemplateName,
          th
        );
        tr.appendChild(th);
      } else {
      ...
      currentWeek =
        nextDay.getDate() === currentWeek.getDate()
          ? util.addDays(nextDay, 1)
          : nextDay;
    }
      ...
      for (const date of weekDates) {
        ...
        if (currentDate.getMonth() !== date.getMonth()) {
          ...
          if (
            ...
            !this.parent.activeViewOptions.cellTemplate
          ) {
  ...
  public generateColumnLevels(): TdData[][] {
    ...
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      ...
      if (this.parent.uiStateValues.isGroupAdaptive) {
        ...
        colLevels = [
          this.getDateSlots([this.parent.selectedDate], resourceLevel.workDays),
        ];
      }
    } else {
  ...
  public getDateSlots(
    renderDates: Date[],
    workDays: number[],
    startHour: string = this.parent.workHours.start,
    endHour: string = this.parent.workHours.end
  ): TdData[] {
    const dateCol: TdData[] = [
      {
        ...
        workDays: workDays,
        startHour: new Date(
          +this.parent.globalize.parseDate(startHour, { skeleton: 'Hm' })
        ),
        endHour: new Date(
          +this.parent.globalize.parseDate(endHour, { skeleton: 'Hm' })
        ),
      },
    ];
    return dateCol;
  ...
  public getMonthDates(date: Date): Date[] {
    const startDate: Date = util.getWeekFirstDate(
      util.firstDateOfMonth(date),
      this.parent.activeViewOptions.firstDayOfWeek
    );
  ...
  public isCurrentDate(date: Date): boolean {
    return (
      util.resetTime(new Date()).getTime() ===
  ...
  private onCellClick(e: Event): void {
    ...
    const isPrevious: boolean =
      startDate.getTime() < this.getStartDate().getTime();
    if (isPrevious || startDate.getTime() > this.getEndDate().getTime()) {
      this.parent.changeDate(
        this.parent.activeView.getNextPreviousDate(
          isPrevious ? 'previous' : 'next'
        ),
        e
      );
      const activeDate: number =
        this.parent.activeCellsData.startTime.getTime();
      const inRange: boolean =
        activeDate >= this.getStartDate().getTime() &&
        activeDate <= this.getEndDate().getTime();
      const dateAttr: number = inRange
        ? activeDate
        : (isPrevious ? this.getEndDate() : this.getStartDate()).getTime();
      const selectedCell: HTMLTableCellElement =
        this.parent.element.querySelector(
          ':not(.' + cls.OTHERMONTH_CLASS + ')[data-date="' + dateAttr + '"]'
        );
      ...
      const filteredEvents: Record<string, any>[] =
        this.parent.eventBase.filterEvents(startDate, endDate);
      const moreEventArgs: EventClickArgs = {
        date: startDate,
        event: filteredEvents,
        element: e.target,
      } as EventClickArgs;
      if (target.classList.contains(cls.OTHERMONTH_CLASS)) {
        target = this.parent.element.querySelector(
          ':not(.' +
  ...
  public onContentScroll(e: Event): void {
    ...
    if (headerWrapper) {
      (<HTMLElement>headerWrapper.firstElementChild).scrollLeft =
        target.scrollLeft;
    }
    const scrollTopSelector: string = `.${cls.MONTH_HEADER_WRAPPER},.${cls.RESOURCE_COLUMN_WRAP_CLASS}`;
    const scrollTopElement: HTMLElement = this.element.querySelector(
      scrollTopSelector
    ) as HTMLElement;
    if (scrollTopElement) {
  ...
  public onScrollUiUpdate(args: NotifyEventArgs): void {
    let height: number =
      this.parent.element.offsetHeight - this.getHeaderBarHeight();
    const headerWrapper: HTMLElement = this.element.querySelector(
    ...
    if (!this.parent.isAdaptive && headerWrapper) {
      ...
      if (contentWrapper.offsetWidth - contentWrapper.clientWidth > 0) {
        (headerWrapper.firstElementChild as HTMLElement).style[
    ...
    const leftPanelElement: HTMLElement = this.element.querySelector(
      leftPanelSelector
    ) as HTMLElement;
    if (leftPanelElement) {
      const isYScroll: boolean =
  ...
  public getStartDate(): Date {
    return new Date(
      this.parent.selectedDate.getFullYear(),
      this.parent.firstMonthOfYear % 12,
      1
    );
  }
  ...
  public getEndDate(): Date {
    return util.addDays(
      util.addMonths(this.getStartDate(), this.parent.monthsCount),
      -1
    );
  }

  public startDate(): Date {
    return this.parent.currentView === 'Year'
      ? util.getWeekFirstDate(
          this.getStartDate(),
          this.parent.activeViewOptions.firstDayOfWeek
        )
      : this.getStartDate();
  ...
  public getDateRangeText(): string {
    ...
    if (startDate.getFullYear() !== endDate.getFullYear()) {
      return (
        this.parent.globalize.formatDate(startDate, { skeleton: 'yMMM' }) +
        ' - ' +
        this.parent.globalize.formatDate(endDate, { skeleton: 'yMMM' })
      );
    } else {
  ...
  public scrollToDate(scrollDate: Date): void {
    ...
    let element: HTMLElement = this.element.querySelector(
      '.' +
        cls.WORK_CELLS_CLASS +
        ':not(.' +
        cls.OTHERMONTH_CLASS +
  ...
  public destroy(): void {
    ...
    if (this.element) {
      const contentScroll: Element = this.element.querySelector(
        '.' + cls.CONTENT_WRAP_CLASS
      );
      if (contentScroll) {
