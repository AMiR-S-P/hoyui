...
import { append, addClass, createElement, EventHandler } from '@hoyui/base';
...
export class TimelineYear extends Year {
  ...
  public renderHeader(headerWrapper: HTMLElement): void {
    ...
    if (
      ...
      const leftHeaderCells: HTMLElement = createElement('td', {
        className: cls.LEFT_INDENT_CLASS,
      });
      tr.appendChild(leftHeaderCells);
      leftHeaderCells.appendChild(
        this.renderResourceHeader(cls.LEFT_INDENT_WRAP_CLASS)
      );
    }
    ...
    if (
      ...
      container.appendChild(
        this.renderResourceHeader(cls.DATE_HEADER_WRAP_CLASS)
      );
      ...
      const dayHeaderDates: Date[] = this.getMonthDates(
        new Date(this.parent.selectedDate.getFullYear(), months[0], 1)
      );
      for (let column: number = 0; column < this.columnCount; column++) {
        const date: Date = new Date(
          this.parent.selectedDate.getFullYear(),
          months[parseInt(column.toString(), 10)],
          1
        );
        const innerTd: HTMLElement = createElement('td', {
          className: cls.HEADER_CELLS_CLASS,
        });
        if (this.parent.activeViewOptions.orientation === 'Horizontal') {
          if (this.parent.dayHeaderTemplate) {
            append(
              this.renderDayMonthHeaderTemplate(
                dayHeaderDates[parseInt(column.toString(), 10)],
                column,
                'dayHeaderTemplate'
              ),
              innerTd
            ...
            innerTd.innerHTML = `<span>${
              this.parent.getDayNames('abbreviated')[column % 7]
            }</span>`;
          }
        } else {
  ...
  private renderResourceHeader(className: string): HTMLElement {
    ...
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      ...
      const colData: TdData[] = [
        { className: [cls.HEADER_CELLS_CLASS], type: 'headerCell' },
      ];
    ...
    for (const col of this.colLevels) {
      ...
      const count: TdData[] =
        className === cls.DATE_HEADER_WRAP_CLASS ? col : [col[0]];
  ...
  public renderContent(contentWrapper: HTMLElement): void {
    ...
    if (
      ...
      tdCollection.push(firstTd);
      const monthWrapper: HTMLElement = createElement('div', {
    ...
    const contentTBody: HTMLTableSectionElement =
      contentTable.querySelector('tbody');
    if (this.parent.rowAutoHeight) {
      const addClassTable: HTMLElement[] = [contentTable];
      const monthHeader: HTMLElement = this.parent.element.querySelector(
        '.' + cls.MONTH_HEADER_WRAPPER + ' .' + cls.SCHEDULE_TABLE_CLASS
      );
      if (monthHeader) {
  ...
  private renderDefaultContent(
    wrapper: HTMLElement,
    monthBody: HTMLTableSectionElement,
    contentBody: HTMLTableSectionElement
  ): void {
    const months: number[] = this.getMonths();
    const dayHeaderDates: Date[] = this.getMonthDates(
      new Date(this.parent.selectedDate.getFullYear(), months[0], 1)
    ...
    for (let month: number = 0; month < this.rowCount; month++) {
      wrapper.appendChild(
        createElement('div', { className: cls.APPOINTMENT_CONTAINER_CLASS })
      );
      ...
      let monthStart: Date = this.parent.calendarUtil.getMonthStartDate(
        new Date(monthDate.getTime())
      );
      let monthEnd: Date = this.parent.calendarUtil.getMonthEndDate(
        new Date(monthDate.getTime())
      );
      const tr: HTMLElement = createElement('tr');
      ...
      if (this.parent.activeViewOptions.orientation === 'Horizontal') {
        ...
        if (this.parent.dayHeaderTemplate) {
          append(
            this.renderDayMonthHeaderTemplate(
              dayHeaderDates[parseInt(month.toString(), 10)],
              month,
              'dayHeaderTemplate'
            ),
            monthTd
          ...
          monthTd.innerHTML = `<span>${
            this.parent.getDayNames('abbreviated')[month % 7]
          }</span>`;
        }
      }
      ...
      for (let column: number = 0; column < this.columnCount; column++) {
        ...
        if (this.parent.activeViewOptions.orientation === 'Vertical') {
          ...
          monthStart = this.parent.calendarUtil.getMonthStartDate(
            new Date(monthDate.getTime())
          );
          monthEnd = this.parent.calendarUtil.getMonthEndDate(
            new Date(monthDate.getTime())
          );
        ...
        if (isDateAvail) {
          ...
          if (
            this.parent.activeViewOptions.workDays.indexOf(date.getDay()) > -1
          ) {
            ...
            if (this.parent.activeViewOptions.orientation === 'Horizontal') {
              tds.push(
                this.element.querySelector(
                  '.' + cls.HEADER_CELLS_CLASS + `:nth-child(${column + 1})`
                )
              );
            } else {
              tds.push(
                this.element
                  .querySelectorAll('.' + cls.MONTH_HEADER_CLASS)
                  .item(month) as HTMLElement
              );
            }
          }
  ...
  public getContentRows(): Element[] {
    ...
    for (
      let row: number = 0;
      row < this.parent.resourceBase.renderedResources.length;
      row++
    ) {
      let resData: TdData;
      if (
        ...
        if (
          ...
          existingGroupIndices.indexOf(resData.groupIndex) > -1
        ) {
          continue;
        }
      }
      ...
      let monthDate: Date = new Date(
        this.parent.selectedDate.getFullYear(),
        monthCells[parseInt(row.toString(), 10)],
        1
      );
      let date: Date = this.parent.calendarUtil.getMonthStartDate(
        new Date(monthDate.getTime())
      );
      for (let month: number = 0; month < this.columnCount; month++) {
        ...
        monthDate = new Date(
          this.parent.selectedDate.getFullYear(),
          monthCells[parseInt(month.toString(), 10)],
          1
        );
        date = this.parent.calendarUtil.getMonthStartDate(
          new Date(monthDate.getTime())
        );
        ...
        this.renderCellTemplate(
          { date: date, type: 'resourceGroupCells', groupIndex: groupIndex },
          tdELe
        );
  ...
  public renderResourceContent(
    ...
    for (let row: number = 0; row < this.rowCount; row++) {
      ...
      for (let month: number = 0; month < this.columnCount; month++) {
        ...
        if (this.parent.activeViewOptions.orientation === 'Vertical') {
          ...
          groupIndex =
            this.colLevels.slice(-1)[0][parseInt(month.toString(), 10)]
              .groupIndex;
          classList.push(cls.WORKDAY_CLASS);
        }
        const startDateText: string = this.parent.globalize.formatDate(date, {
          type: 'dateTime',
          skeleton: 'full',
          calendar: this.parent.getCalendarMode(),
        });
        const endDateText: string = this.parent.globalize.formatDate(
          this.parent.calendarUtil.getMonthEndDate(
            new Date(monthDate.getTime())
          ),
          {
            type: 'dateTime',
    ...
    if (this.parent.activeViewOptions.orientation === 'Vertical') {
      this.collapseRows(
        this.parent.element.querySelector('.' + cls.CONTENT_WRAP_CLASS)
      );
    }
  }
  ...
  public scrollToDate(scrollDate: Date): void {
    let date: number;
    if (
      this.parent.activeViewOptions.group.resources !== null &&
      this.parent.activeViewOptions.group.resources.length > 0 &&
      !this.parent.uiStateValues.isGroupAdaptive
    ) {
    ...
    if (element) {
      ...
      if (this.parent.enableRtl) {
        const conTable: HTMLElement = this.element.querySelector(
          '.' + cls.CONTENT_TABLE_CLASS
        ) as HTMLElement;
        wrap.scrollLeft = -(
          conTable.offsetWidth -
          element.offsetLeft -
          element.offsetWidth
        );
      } else {
