import { EventHandler, createElement, formatUnit } from '@hoyui/base';
...
export class TimelineMonth extends Month {
    ...
    for (let row: number = 0; row < this.getRowCount(); row++) {
      for (const data of this.colLevels[this.colLevels.length - 1]) {
        ...
        if (
          ...
          data.workDays =
            (resLevel.resourceData[
              resLevel.resource.workDaysField
            ] as number[]) || this.parent.workDays;
  ...
  public getMonthStart(currentDate: Date): Date {
    const monthStart: Date = this.parent.calendarUtil.firstDateOfMonth(
      util.resetTime(currentDate)
    );
    return new Date(
  ...
  public getMonthEnd(currentDate: Date): Date {
    ...
    return this.parent.calendarUtil.lastDateOfMonth(
      util.addMonths(
        new Date(+monthStart),
