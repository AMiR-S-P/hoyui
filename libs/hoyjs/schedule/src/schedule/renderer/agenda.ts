...
import { createElement, addClass, EventHandler } from '@hoyui/base';
import {
  IRenderer,
  NotifyEventArgs,
  EventFieldsMapping,
...
export class Agenda extends AgendaBase implements IRenderer {
  ...
  public renderLayout(): void {
    ...
    if (this.parent.resourceBase) {
      this.parent.resourceBase.generateResourceLevels([
        <TdData>{ renderDates: this.parent.activeView.renderDates },
      ]);
    }
    if (
  ...
  private eventLoad(args: NotifyEventArgs): void {
    this.dataSource = extend([], this.parent.eventsData, null, true) as Record<
      string,
      any
    >[];
    ...
    let eventCollection: Record<string, any>[] = this.parent.activeViewOptions
    ...
    if (this.parent.uiStateValues.isGroupAdaptive) {
      ...
      this.dataSource = this.parent.eventBase.filterEventsByResource(
        resource,
        this.dataSource
      );
      eventCollection = this.parent.eventBase.filterEventsByResource(
        resource,
        eventCollection
      );
    }
  ...
  private refreshEvent(refreshDate: Date): void {
    ...
    for (const data of this.dataSource) {
      if (
        isNullOrUndefined(data[fields.recurrenceID]) &&
        !isNullOrUndefined(data[fields.recurrenceRule]) &&
        !isNullOrUndefined(data.generatedDates) &&
        refreshDate >= (<Record<string, any>>data.generatedDates).end
      ) {
        processedData = processedData.concat(
          this.parent.eventBase.generateOccurrence(data, refreshDate)
        );
      }
    }
    this.parent.eventsProcessed = this.parent.eventsProcessed.concat(
      this.processAgendaEvents(processedData)
    );
  }
  ...
  public refreshHeader(): void {
    ...
    const lastDate: Date =
      ...
      this.parent.hideEmptyAgendaDays
        ? this.getEndDateFromStartDate(firstDate)
        : util.addDays(firstDate, this.parent.agendaDaysCount);
  ...
  private renderInitialContent(tBody: Element, agendaDate: Date): void {
    ...
    if (
      this.parent.activeViewOptions.allowVirtualScrolling &&
      this.parent.hideEmptyAgendaDays &&
      this.parent.eventsData.length > 0
    ) {
      const contentArea: HTMLElement = this.getContentAreaElement();
      const contentChild: HTMLElement =
        contentArea.querySelector('.e-content-table');
      while (contentArea.offsetWidth <= contentArea.clientWidth) {
        if (
          this.parent.isAdaptive &&
          contentChild.offsetHeight >= contentArea.clientHeight
        ) {
          break;
        }
        const emptyTBody: Element = createElement('tbody');
  ...
  public renderContent(tBody: Element, agendaDate: Date, lastDate: Date): void {
    ...
    const firstDate: Date = new Date(agendaDate.getTime());
    const isObject: Record<string, any>[] = this.appointmentFiltering(
      firstDate,
      lastDate
    );
    if (
      ...
      if (
        ...
        !this.isAgendaWorkDay(isObject[0][fieldMapping.startTime])
      ) {
        for (const event of isObject) {
          if (this.isAgendaWorkDay(event[fieldMapping.startTime])) {
            agendaDate = new Date(
              new Date(event[fieldMapping.startTime].getTime()).setHours(
                0,
                0,
                0,
                0
        ...
        agendaDate = new Date(
          new Date(isObject[0][fieldMapping.startTime].getTime()).setHours(
            0,
            0,
            0,
            0
    ...
    if (
      !this.parent.hideEmptyAgendaDays ||
      (this.parent.agendaDaysCount > 0 && isObject.length > 0)
    ) {
      if (
        ...
        this.calculateResourceTableElement(
          tBody,
        ...
        for (let day: number = 0; day < this.parent.agendaDaysCount; day++) {
          const nTr: HTMLElement = this.createTableRowElement(
            agendaDate,
            'data'
          ) as HTMLElement;
          const virtualContent: HTMLElement = this.element.querySelector(
            'tr[data-row-index="' + +nTr.dataset.rowIndex + '"]'
          );
          if (
            virtualContent ||
            (!this.parent.activeViewOptions.showWeekend &&
              !this.isAgendaWorkDay(agendaDate))
          ) {
          ...
          const filterData: Record<string, any>[] =
            this.appointmentFiltering(agendaDate);
          if (
            filterData.length > 0 ||
            (!this.parent.hideEmptyAgendaDays && filterData.length === 0)
          ) {
            ...
            nTr.appendChild(dTd);
            const cTd: Element = this.createAgendaContentElement(
              elementType,
              filterData,
              aTd
            );
  ...
  private virtualScrolling(event: Event): void {
    ...
    const direction: string =
      this.virtualScrollTop < scrollTop ? 'next' : 'previous';
    ...
    if (scrollTop === 0) {
      ...
      if (filterData.length > 0 || !this.parent.hideEmptyAgendaDays) {
        ...
        this.wireEventActions();
        for (
          let s: number = 0, element: HTMLCollection = tBody.children;
          s < element.length;
          s++
        ) {
          if (
            element[parseInt(s.toString(), 10)].getAttribute(
              'data-row-index'
            ) === topElement.getAttribute('data-column-index')
          ) {
            const scrollToValue: number =
              (<HTMLElement>element[parseInt(s.toString(), 10)]).offsetTop -
              (<HTMLElement>this.element.querySelector('.e-agenda-item'))
                .offsetHeight;
  ...
  private getElementFromScrollerPosition(event: Event): Element {
    ...
    const offsetHeight: number = target.clientHeight;
    const totalHeight: number = scrollTop + offsetHeight;
    const liCollection: HTMLElement[] = [].slice.call(
      target.querySelectorAll('.e-agenda-item')
    );
    let li: HTMLElement;
    ...
    if (liCollection.length > 0) {
      if (scrollTop === 0) {
        ...
        for (
          let a: number = 0, length: number = liCollection.length;
          a < length;
          a++
        ) {
  ...
  private getPreviousNextDate(date: Date, type: string): Record<string, Date> {
    ...
    do {
      if (
        ...
        const filterData: Record<string, any>[] =
          this.appointmentFiltering(currentDate);
        if (filterData.length > 0 || !this.parent.hideEmptyAgendaDays) {
          daysCount++;
        }
      }
    ...
    return type === 'next'
      ? { start: date, end: util.addDays(endDate, 1) }
      : { start: endDate, end: util.addDays(date, 1) };
  }
  ...
  private appointmentFiltering(
    ...
    let filterData: Record<string, any>[] = this.parent.eventBase.filterEvents(
      dateStart,
      dateEnd
    );
  ...
  public getStartDateFromEndDate(endDate: Date): Date {
    ...
    if (this.parent.eventsProcessed.length > 0) {
      const firstDate: number = Math.min(
        ...this.parent.eventsProcessed.map((a: Record<string, Date>) =>
          a[fields.startTime].getTime()
        )
      );
      filterDate = this.parent.hideEmptyAgendaDays
        ? new Date(firstDate)
        : this.parent.minDate;
    } else {
      filterDate = this.parent.hideEmptyAgendaDays
        ? util.addMonths(endDate, -1)
        : this.parent.minDate;
    }
  ...
  public getEndDateFromStartDate(startDate: Date): Date {
    ...
    const fieldMapping: EventFieldsMapping = this.parent.eventFields;
    if (this.parent.eventsProcessed.length > 0) {
      const lastDate: number = Math.max(
        ...this.parent.eventsProcessed.map((a: Record<string, Date>) =>
  ...
  public getDateRangeText(date?: Date): string {
    ...
    if (
      ...
      const currentDate: Date = isNullOrUndefined(date)
        ? this.parent.selectedDate
        : date;
      return util.capitalizeFirstWord(
        this.parent.globalize.formatDate(currentDate, {
          format: formatDate,
  ...
  public dayNavigationClick(e: Event): void {
    const element: HTMLTableCellElement = closest(
  ...
  private wireEvents(): void {
    EventHandler.add(
      this.element.querySelector('.' + cls.CONTENT_WRAP_CLASS),
      event.scroll,
      this.agendaScrolling,
      this
    );
  }
  ...
  private unWireEvents(): void {
    ...
    const dateHeaderElement: Element[] = [].slice.call(
      this.element.querySelectorAll('.e-m-date')
    );
  ...
  private onAgendaScrollUiUpdate(): void {
    ...
    const contentArea: HTMLElement = this.element.querySelector(
      '.' + cls.CONTENT_WRAP_CLASS
    ) as HTMLElement;
    if (this.parent.height !== 'auto') {
      contentArea.style.height = formatUnit(
        this.parent.element.offsetHeight - headerHeight
      );
    }
  }
  ...
  public scrollToDate(scrollDate: Date): void {
    ...
    if (this.parent.activeViewOptions.allowVirtualScrolling) {
      if (
        !this.parent.hideEmptyAgendaDays ||
        this.parent.getEvents(date, util.addDays(date, 1), true).length > 0
      ) {
        this.parent.changeDate(date);
      }
    } else {
      const dateElement: HTMLElement = this.element.querySelector(
        '.' + cls.AGENDA_CELLS_CLASS + '[data-date="' + date.getTime() + '"]'
      );
      if (dateElement) {
