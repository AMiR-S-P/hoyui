...
export class Render {
  ...
  private initializeLayout(viewName: View): void {
    ...
    if (isNullOrUndefined(this.parent.activeView)) {
      ...
      if (firstView) {
        ...
        console.warn(
          '[WARNING] :: Module "' +
            viewName +
            '" is not available in Schedule component!' +
    ...
    if (
      this.parent.eventSettings.enableTooltip ||
      (this.parent.activeViewOptions.group.resources.length > 0 &&
  ...
  public updateHeader(): void {
    if (this.parent.headerModule) {
      this.parent.headerModule.setDayOfWeek(
  ...
  public updateLabelText(view: string): void {
    ...
    if (view === 'Year') {
      this.parent.element.setAttribute(
        'aria-label',
        this.parent.activeView.getLabelText(view)
      );
    } else {
      this.parent.element.setAttribute(
        'aria-labelledby',
        this.parent.element.id + '_table'
      );
    }
  }
}
