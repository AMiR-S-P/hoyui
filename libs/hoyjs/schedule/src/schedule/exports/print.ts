...
export class Print {
  ...
  private printScheduler(): void {
    const clone: HTMLElement = this.parent.element.cloneNode(
      true
    ) as HTMLElement;
    ...
    const links: Element[] = [].slice.call(
      document.getElementsByTagName('head')[0].querySelectorAll('link, style')
    );
    ...
    const elementWidth: number =
      Math.round(parseInt(clone.style.width, 10) / 100) * 100;
    div.appendChild(clone);
    const printWindow: Window = window.open(
      '',
      'print',
      'height=550,width=' + elementWidth + ',tabbar=no'
    );
    ...
    setTimeout(() => {
      if ((printWindow as any).ready && scrollableEle) {
        ...
        const headerTimeCellsScroll: HTMLElement =
          printWindow.document.querySelector('.e-date-header-wrap');
        if (headerTimeCellsScroll) {
          headerTimeCellsScroll.scrollLeft = scrollableEle.scrollLeft;
        }
        const timeCellsScroll: HTMLElement =
          printWindow.document.querySelector('.e-time-cells-wrap');
        if (timeCellsScroll) {
          timeCellsScroll.scrollTop = scrollableEle.scrollTop;
        }
        const contentCellScroll: HTMLElement =
          printWindow.document.querySelector(className);
  ...
  private getPrintScheduleModel(printOptions: ScheduleModel): ScheduleModel {
    ...
    const scheduleTemplates: string[] = [
      'cellHeaderTemplate',
      'dayHeaderTemplate',
      'monthHeaderTemplate',
      'cellTemplate',
      ...
      switch (key) {
        case 'eventSettings': {
          eventSettings = Object.assign(
            {},
            (this.parent.eventSettings as Record<string, any>).properties
          );
          eventSettings.dataSource = this.parent.eventsData;
          const eventTemplate: string | Function =
            !isNullOrUndefined(printOptions.eventSettings) &&
            !isNullOrUndefined(printOptions.eventSettings.template)
              ? printOptions.eventSettings.template
              : eventSettings.template;
          eventSettings.template =
            !this.parent.isAngular && typeof eventTemplate === 'function'
              ? null
              : eventTemplate;
        ...
        case 'group':
          group = isNullOrUndefined(printOptions.group)
            ? this.parent.group
            : printOptions.group;
          ...
          break;
        case 'timeScale':
          timeScale = isNullOrUndefined(printOptions.timeScale)
            ? this.parent.timeScale
            : printOptions.timeScale;
          ...
          break;
        case 'views':
          views = isNullOrUndefined(printOptions.views)
            ? this.parent.views
            : printOptions.views;
          if (
            ...
            views &&
            views.length > 0 &&
            typeof views[0] === 'object'
          ) {
            for (const view of views) {
              scheduleTemplates.forEach((x: string) => {
                if (!isNullOrUndefined((view as Record<string, any>)[`${x}`])) {
                  (view as Record<string, any>)[`${x}`] =
                    typeof (view as Record<string, any>)[`${x}`] === 'function'
                      ? null
                      : (view as Record<string, any>)[`${x}`];
                }
              });
        ...
        default:
          if (scheduleTemplates.indexOf(key) > -1) {
            (printModel as Record<string, any>)[`${key}`] = isNullOrUndefined(
              (printOptions as Record<string, any>)[`${key}`]
            )
              ? !this.parent.isAngular &&
                typeof (this.parent as Record<string, any>)[`${key}`] ===
                  'function'
                ? null
                : (this.parent as Record<string, any>)[`${key}`]
              : !this.parent.isAngular &&
                typeof (printOptions as Record<string, any>)[`${key}`] ===
                  'function'
              ? null
              : (printOptions as Record<string, any>)[`${key}`];
            break;
          ...
          if (scheduleEvents.indexOf(key) > -1) {
            (printModel as Record<string, any>)[`${key}`] = (
              printOptions as Record<string, any>
            )[`${key}`];
            break;
          }
          (printModel as Record<string, any>)[`${key}`] = isNullOrUndefined(
