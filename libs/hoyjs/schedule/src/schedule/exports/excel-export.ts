...
import { Workbook, Worksheet, Worksheets, Column } from '@hoyui/excel-export';
...
export class ExcelExport {
  ...
  public initializeExcelExport(excelExportOptions: ExportOptions = {}): void {
    const exportColumns: ExportFieldInfo[] =
      this.getExportColumns(excelExportOptions);
    ...
    const isIncludeOccurrences: boolean =
      excelExportOptions.includeOccurrences || false;
    let separator: string;
    if (
      !isNullOrUndefined(excelExportOptions.separator) &&
    ...
    if (excelExportOptions.customData) {
      eventCollection = !isIncludeOccurrences
        ? excelExportOptions.customData
        : this.parent.eventBase.getProcessedEvents(
            excelExportOptions.customData
          );
    } else {
      eventCollection = !isIncludeOccurrences
  ...
  private processWorkbook(
    ...
    fields.forEach((field: ExportFieldInfo, i: number) => {
    ...
    fields.forEach((field: ExportFieldInfo, i: number) => {
      columnHeader.push({ index: i + 1, value: field.text, style: style });
    });
    ...
    eventCollection.forEach((event: Record<string, any>, i: number) => {
      ...
      fields.forEach((field: ExportFieldInfo, n: number) => {
        let columnRule: Record<string, any> = {
          index: n + 1,
          value: event[field.name] || '',
        };
        if (
          field.name === this.parent.eventFields.startTime ||
          field.name === this.parent.eventFields.endTime
        ) {
          const styleRule: Record<string, any> = {
            fontSize: 12,
            numberFormat: 'm/d/yyyy h:mm a',
          };
          columnRule = extend(
            {},
            columnRule,
    ...
    const workSheet: Worksheets = [
      { columns: columns, rows: rows } as Worksheet,
    ];
  ...
  private getExportColumns(exportOptions: ExportOptions): ExportFieldInfo[] {
    ...
    if (exportColumns.length === 0) {
      const fields: string[] =
        ...
        (Object.keys(this.parent.eventFields).map(
          (field: string) =>
            (<Record<string, any>>this.parent.eventFields)[`${field}`]
        ) as string[]);
