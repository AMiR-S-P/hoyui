...
export class ICalendarExport {
  ...
  public initializeCalendarExport(
    fileName: string,
  ...
  public getCalendarString(
    fileName?: string,
    customData?: Record<string, any>[]
  ): string {
    let eventsData: Record<string, any>[] = customData
      ? customData
      : (extend([], this.parent.eventsData, null, true) as Record<
          string,
          any
        >[]);
    ...
    eventsData.forEach((eventObj: Record<string, any>) => {
      let uId: string =
        eventObj[fields.id] ||
        eventObj.Guid ||
        this.parent.eventBase.generateGuid();
      const editedExDate: string[] = [];
      if (eventObj[fields.recurrenceID]) {
        const filter: Record<string, any>[] = this.filterEvents(
          filterCollection,
          fields.id,
          eventObj[fields.recurrenceID] as number
        );
        uId = filter.length > 0 ? (filter[0].UID as string) : uId;
      }
      if (
        !eventObj[fields.recurrenceID] &&
        eventObj[fields.recurrenceRule] &&
        eventObj[fields.recurrenceException]
      ) {
        const exDate: string[] = (
          eventObj[fields.recurrenceException] as string
        ).split(',');
        const editedObj: Record<string, any>[] = this.filterEvents(
          eventsData,
          fields.recurrenceID,
          eventObj[fields.id] as number
        );
        editedObj.forEach((edited: Record<string, any>) => {
          editedExDate.push(
            getRecurrenceStringFromDate(edited[fields.startTime] as Date)
          );
        });
        const exceptionDateList: string[] = exDate.filter(
          (value: string) => editedExDate.indexOf(value) === -1
        );
        eventObj[fields.recurrenceException] =
          exceptionDateList.length > 0 ? exceptionDateList.join(',') + ',' : '';
      }
      const startZone: string = (eventObj[fields.startTimezone] ||
        timeZone) as string;
      const endZone: string = (eventObj[fields.endTimezone] ||
        timeZone) as string;
      const readonly: boolean = eventObj[fields.isReadonly]
        ? eventObj[fields.isReadonly]
        : false;
      ...
      if (eventObj[fields.isAllDay]) {
        calendarEvent.splice(
          4,
          0,
          'DTEND;VALUE=DATE:' +
            this.convertDateToString(eventObj[fields.endTime] as Date, true)
        );
        calendarEvent.splice(
          4,
          0,
          'DTSTART;VALUE=DATE:' +
            this.convertDateToString(eventObj[fields.startTime] as Date, true)
        );
      } else if (
        !eventObj[fields.isAllDay] &&
        !eventObj[fields.recurrenceRule]
      ) {
        calendarEvent.splice(
          4,
          0,
          'DTEND:' + this.convertDateToString(eventObj[fields.endTime] as Date)
        );
        calendarEvent.splice(
          4,
          0,
        ...
        calendarEvent.splice(
          ...
          'DTEND;TZID="' +
            endZone +
            '":' +
            this.convertDateToString(eventObj[fields.endTime] as Date)
        );
        calendarEvent.splice(
          4,
          0,
          ...
          calendarEvent.splice(
            ...
            'EXDATE:' +
              this.convertDateToString(
                getDateFromRecurrenceDateString(
                  exDate[parseInt(i.toString(), 10)]
                ),
                eventObj[fields.isAllDay] as boolean
              )
          );
        }
      }
      ...
      if (customFields.length > 0) {
        customFields.forEach((customField: string) =>
          calendarEvent.splice(
            4,
            0,
            customField + ':' + (eventObj[`${customField}`] || '')
          )
        );
      }
      const app: Record<string, any> = <Record<string, any>>(
        extend({}, eventObj)
      );
      app.UID = uId;
    ...
    const icsString: string =
      iCalendar +
      SEPARATOR +
      iCalendarEvents.join(SEPARATOR) +
      SEPARATOR +
      'END:VCALENDAR';
    return icsString;
  }
  private customFieldFilter(
    ...
    const defaultFields: string[] = Object.keys(fields).map(
      (key: string) => (fields as Record<string, any>)[`${key}`]
    ) as string[];
    const eventFields: string[] = Object.keys(eventObj);
    return eventFields.filter(
      (value: string) => defaultFields.indexOf(value) === -1 && value !== 'Guid'
    );
  }

  private convertDateToString(eventDate: Date, allDay?: boolean): string {
    const year: string = ('0000' + eventDate.getFullYear().toString()).slice(
      -4
    );
    const month: string = ('00' + (eventDate.getMonth() + 1).toString()).slice(
      -2
    );
    const date: string = ('00' + eventDate.getDate().toString()).slice(-2);
    const hours: string = ('00' + eventDate.getHours().toString()).slice(-2);
    const minutes: string = ('00' + eventDate.getMinutes().toString()).slice(
      -2
    );
    const seconds: string = ('00' + eventDate.getSeconds().toString()).slice(
      -2
    );
    const timeString: string = allDay
      ? year + month + date
  ...
  private download(icsString: string, fileName: string): void {
    const buffer: Blob = new Blob([icsString], {
