...
import { setStyleAttribute, EventHandler } from '@hoyui/base';
...
export class YearEvent extends TimelineEvent {
  ...
  private yearViewEvents(): void {
    ...
    for (const month of months) {
      const queryString: string = `.e-month-calendar:nth-child(${
        months.indexOf(month) + 1
      }) td.e-work-cells`;
      const workCells: HTMLElement[] = [].slice.call(
        this.parent.element.querySelectorAll(queryString)
      );
      ...
      const monthStart: Date = this.parent.calendarUtil.getMonthStartDate(
        new Date(monthDate.getTime())
      );
      const monthEnd: Date = this.parent.calendarUtil.getMonthEndDate(
        new Date(monthDate.getTime())
      );
      let startDate: Date = util.getWeekFirstDate(
        monthStart,
        this.parent.activeViewOptions.firstDayOfWeek
      );
      const endDate: Date = util.addDays(
        util.getWeekLastDate(
          monthEnd,
          this.parent.activeViewOptions.firstDayOfWeek
        ),
        1
      ...
      for (
        ...
        const filterEvents: Record<string, any>[] =
          this.parent.eventBase.filterEvents(start, end);
        if (filterEvents.length > 0) {
          const workCell: HTMLElement =
            workCells[parseInt(index.toString(), 10)];
          if (workCell) {
            workCell.appendChild(
              createElement('div', { className: cls.APPOINTMENT_CLASS })
            );
          }
        }
      }
  ...
  private timelineYearViewEvents(): void {
    ...
    this.cellHeader = util.getOuterHeight(
      workCell.querySelector('.' + cls.DATE_HEADER_CLASS)
    );
    const eventTable: Element = this.parent.element.querySelector(
    ...
    const selector: string = `.${cls.MONTH_HEADER_WRAPPER} tbody tr,.${cls.RESOURCE_COLUMN_TABLE_CLASS} tbody tr,.${cls.CONTENT_TABLE_CLASS} tbody tr`;
    this.addCellHeight(
      selector,
      this.eventHeight,
      EVENT_GAP,
    ...
    const wrapperCollection: HTMLElement[] = [].slice.call(
    ...
    for (let row: number = 0; row < months.length; row++) {
      ...
      let monthStart: Date = new Date(
        this.parent.selectedDate.getFullYear(),
        months[parseInt(row.toString(), 10)],
        1
      );
      ...
      while (monthStart.getTime() <= monthEnd.getTime()) {
        ...
        if (this.parent.activeViewOptions.orientation === 'Vertical') {
          const wrapper: Element =
            wrapperCollection[parseInt(dayIndex.toString(), 10)];
          td = dayIndex + 1;
          eventWrapper = wrapper.querySelector(
        ...
        const rowTd: HTMLElement = this.parent.element.querySelector(
          `.e-content-wrap tr:nth-child(${td}) td`
        ) as HTMLElement;
        ...
        let dayEvents: Record<string, any>[] =
          this.parent.eventBase.filterEvents(
            dayStart,
            dayEnd,
            undefined,
            resource
        ...
        for (
          let index: number = 0, count: number = dayEvents.length;
          index < count;
          index++
        ) {
          const eventData: Record<string, any> = extend(
            {},
            dayEvents[parseInt(index.toString(), 10)],
            null,
            true
          ...
          const availedHeight: number =
            this.cellHeader +
            this.eventHeight * (overlapIndex + 1) +
            EVENT_GAP +
            this.moreIndicatorHeight;
          const appArea: number =
            this.cellHeight - this.cellHeader - this.moreIndicatorHeight;
          const renderedAppCount: number = Math.floor(
            appArea / (this.eventHeight + EVENT_GAP)
          );
          const eventsPerRow: number = this.parent.rowAutoHeight
            ? 1
            : this.parent.activeViewOptions.maxEventsPerRow;
          const moreIndicatorCount: number = this.parent.activeViewOptions
            .maxEventsPerRow
            ? count - eventsPerRow
            : count - renderedAppCount <= 0
            ? 1
            : count - renderedAppCount;
          if (this.parent.activeViewOptions.orientation === 'Horizontal') {
            const isRendered: Record<string, any>[] =
              this.renderedEvents.filter(
                (eventObj: Record<string, any>) =>
                  eventObj.Guid === eventData.Guid
              );
            const isSpanned: Record<string, any>[] = isSpannedCollection.filter(
              (eventObj: Record<string, any>) =>
                eventObj.Guid === eventData.Guid
            );
            if (
              (isRendered.length > 0 && isRendered[0].MoreIndicator) ||
              (isSpanned.length > 0 && isSpanned[0].MoreIndicator)
            ) {
              const moreIndex: number =
    ...
    if (
      ...
      const appContainer: HTMLElement[] = [].slice.call(
        this.parent.element.querySelectorAll(
          '.' + cls.APPOINTMENT_CONTAINER_CLASS
        )
      );
      ...
      appContainer.forEach((ele: HTMLElement, index: number) => {
        const app: HTMLElement[] = [].slice.call(
          ele.querySelectorAll('.' + cls.APPOINTMENT_CLASS)
        );
        const appTop: number =
          tr[parseInt(index.toString(), 10)].offsetTop +
          this.cellHeader +
          EVENT_GAP;
  ...
  private updateSpannedEvents(
    ...
    const isLeftRightResize: boolean =
      (this.isResource &&
        this.parent.activeViewOptions.orientation === 'Vertical') ||
      (!this.isResource &&
        this.parent.activeViewOptions.orientation === 'Horizontal');
    ...
    if (
      dayStart.getTime() <= (eventObj[this.fields.startTime] as Date).getTime()
    ) {
    ...
    if (
      dayEnd.getTime() >= (eventObj[this.fields.endTime] as Date).getTime() ||
      (isLeftRightResize &&
        !this.isResource &&
        util.addDays(dayEnd, -1).getMonth() ===
          (eventObj[this.fields.endTime] as Date).getMonth()) ||
      (isLeftRightResize &&
        this.isResource &&
        dayEnd.getTime() <= eventObj[this.fields.endTime].getTime())
    ) {
      if (isLeftRightResize) {
        data.isRight = false;
      } else {
  ...
  private timelineResourceEvents(): void {
    ...
    const isVerticalScrollbarAvail: boolean =
      contentTable.offsetWidth > contentTable.clientWidth;
    const workCell: HTMLElement = this.parent.element.querySelector(
    ...
    if (this.parent.activeViewOptions.orientation === 'Horizontal') {
      for (let month: number = 0; month < months.length; month++) {
        ...
        for (let i: number = 0, len: number = resources.length; i < len; i++) {
          this.renderedEvents = [];
          this.renderResourceEvent(
            wrapperCollection[parseInt(month.toString(), 10)],
      ...
      for (let i: number = 0, len: number = resources.length; i < len; i++) {
        ...
        for (let month: number = 0; month < months.length; month++) {
          ...
          this.renderResourceEvent(
            wrapperCollection[parseInt(i.toString(), 10)],
            resources[parseInt(i.toString(), 10)],
            month,
            i,
            monthStart
    ...
    if (
      ...
      const tr: HTMLElement[] = [].slice.call(
        conTable.querySelectorAll('tbody tr')
      );
      appContainer.forEach((ele: HTMLElement, index: number) => {
        ...
        appWrapper.forEach((appWrap: HTMLElement, cellIndex: number) => {
          const td: HTMLElement = row.querySelector(
          ...
          if (this.parent.enableRtl) {
            ...
            app.forEach((app: HTMLElement) => {
              app.style.width =
                Math.floor(parseInt(app.style.width, 10) / width) * width +
                'px';
  ...
  private renderResourceEvent(
    ...
    const monthEnd: Date = util.addDays(
      util.lastDateOfMonth(new Date(monthStart.getTime())),
      1
    );
    ...
    const rowIndex: number =
      this.parent.activeViewOptions.orientation === 'Vertical' ? index : month;
    const td: HTMLElement = this.parent.element.querySelector(
      `.e-content-wrap tr:nth-child(${rowIndex + 1}) td`
    ) as HTMLElement;
    ...
    for (let a: number = 0; a < eventDatas.length; a++) {
      ...
      if (this.parent.activeViewOptions.group.resources.length > 0) {
        const eventObj: Record<string, any> = this.isSpannedEvent(
          eventData,
          monthStart
        );
      ...
      const availedHeight: number =
        this.cellHeader +
        this.eventHeight * (a + 1) +
        EVENT_GAP +
        this.moreIndicatorHeight;
      const leftValue: number =
        this.parent.activeViewOptions.orientation === 'Vertical'
          ? month * this.cellWidth
      ...
      if (
        ...
        const isRendered: Record<string, any>[] = this.renderedEvents.filter(
          (eventObj: Record<string, any>) => eventObj.Guid === eventData.Guid
        );
        const isSpanned: Record<string, any>[] = isSpannedCollection.filter(
          (eventObj: Record<string, any>) => eventObj.Guid === eventData.Guid
        );
        ...
        if (this.parent.activeViewOptions.orientation === 'Horizontal') {
          for (let i: number = index; i < eventDatas.length; i++) {
            this.renderedEvents.push(
              extend(
                {},
                eventDatas[parseInt(i.toString(), 10)],
  ...
  private renderEvent(
    ...
    if (eventObj.isSpanned.count === 1) {
      const endTime: Date = util.addDays(
        eventObj[this.fields.endTime] as Date,
        -1
      );
      eventObj[this.fields.endTime] =
        endTime > eventObj[this.fields.startTime]
          ? endTime
          : eventObj[this.fields.endTime];
    }
    ...
    if (this.parent.activeViewOptions.orientation === 'Horizontal') {
      index = row + 1;
      if (
        (eventObj[this.fields.startTime] as Date).getTime() ===
        (eventObj[this.fields.endTime] as Date).getTime()
      ) {
      ...
      index = rowIndex + 1;
      width = this.isResource
        ? eventObj.isSpanned.count * this.cellWidth
        : this.cellWidth;
    }
    const rowTd: HTMLElement = this.parent.element.querySelector(
      `.e-content-wrap tr:nth-child(${index}) td`
    ) as HTMLElement;
    const top: number =
      rowTd.offsetTop +
      this.cellHeader +
      this.eventHeight * <number>eventObj.Index +
      EVENT_GAP;
    ...
    this.parent.trigger(
      ...
      (eventArgs: EventRenderedArgs) => {
        if (!eventArgs.cancel) {
          ...
          if (this.parent.activeViewOptions.group.resources.length > 0) {
            this.renderedEvents.push(
              extend({}, eventObj, null, true) as Record<string, any>
            );
            ...
            ((eventObj.isSpanned.isRight || eventObj.isSpanned.isLeft) &&
              this.parent.activeViewOptions.orientation === 'Horizontal') ||
  ...
  private renderMoreIndicator(
    ...
    if (
      this.parent.activeViewOptions.group.resources.length === 0 &&
      wrapper.querySelector('[data-start-date="' + startDate.getTime() + '"]')
    ) {
      return;
    }
    let endDate: Date;
    if (
      ...
      endDate = util.addDays(
        util.lastDateOfMonth(new Date(startDate.getTime())),
        1
      );
    } else {
      endDate = util.addDays(new Date(startDate.getTime()), 1);
    }
    const moreIndicator: HTMLElement = this.getMoreIndicatorElement(
      count,
      startDate,
      endDate
    );
    const rowTr: HTMLElement = this.parent.element.querySelector(
      `.e-content-wrap tr:nth-child(${row + 1})`
    ) as HTMLElement;
    const top: number =
      rowTr.offsetTop + (this.cellHeight - this.moreIndicatorHeight);
  ...
  private createEventElement(record: Record<string, any>): HTMLElement {
    ...
    if (this.parent.activeViewOptions.eventTemplate) {
      ...
      const locationEle: string = (record[this.fields.location] ||
      ...
      const startTimeEle: HTMLElement = createElement('div', {
        ...
        innerHTML: this.parent.getTimeString(
          eventObj[this.fields.startTime] as Date
        ),
      });
  ...
  public isSpannedEvent(
    ...
    const isSpanned: Record<string, any> = {
      isLeft: false,
      isRight: false,
      count: 1,
    };
    ...
    if (
      ...
      this.updateSpannedEventDetails(
        eventStart,
        eventEnd,
        yearStart,
        yearEnd,
      ...
      const originalEndTime: Date = new Date(
        eventData[this.fields.endTime] - 1
      );
      isSpanned.count =
        originalEndTime.getMonth() -
        originalStartTime.getMonth() +
        this.parent.monthsCount *
          (originalEndTime.getFullYear() - originalStartTime.getFullYear()) +
        1;
    } else {
      this.updateSpannedEventDetails(
        eventStart,
      ...
      if (
        ...
        let end: number = util
          .resetTime(eventData[this.fields.endTime])
          .getTime();
        const start: number = util
          .resetTime(eventData[this.fields.startTime])
          .getTime();
        if (
          (eventObj[this.fields.isAllDay] &&
            end === eventObj[this.fields.endTime].getTime()) ||
          isSpanned.isRight
        ) {
    ...
    if (
      util.resetTime(eventStart).getTime() <
      util.resetTime(this.parent.minDate).getTime()
    ) {
      eventData[this.fields.startTime] = this.parent.minDate;
    }
    if (
      util.resetTime(eventEnd).getTime() >
      util.resetTime(this.parent.maxDate).getTime()
    ) {
  ...
  public getOverlapEvents(
    ...
    if (
      ...
      const monthEnd: Date = util.addDays(
        this.parent.calendarUtil.getMonthEndDate(new Date(date.getTime())),
        -1
      );
      ...
      if (this.parent.rowAutoHeight) {
        ...
        dateEnd = util
          .addDays(util.resetTime(new Date(date.getTime())), 1)
          .getTime();
      } else {
        dateStart = dateEnd = util
    ...
    for (const app of appointments) {
      ...
      const timeCondition: boolean = app[this.fields.isAllDay]
        ? util.resetTime(appEnd).getTime() > dateStart
        : util.resetTime(appEnd).getTime() >= dateStart;
      if (
        (util.resetTime(appStart).getTime() <= dateStart && timeCondition) ||
        (util.resetTime(appStart).getTime() >= dateStart &&
  ...
  private getMonths(): number[] {
    ...
    return Array.apply(null, { length: this.parent.monthsCount }).map(
      (value: number, index: number) => this.parent.firstMonthOfYear + index
    );
  }

  private removeCellHeight(): void {
    const elementSelector: string = `.${cls.MONTH_HEADER_WRAPPER} tbody tr,.${cls.RESOURCE_COLUMN_TABLE_CLASS} tbody tr,.${cls.CONTENT_TABLE_CLASS} tbody tr`;
