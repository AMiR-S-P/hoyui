...
export class AgendaBase extends ViewBase {
  ...
  public createAgendaContentElement(
    ...
    if (type === 'noEvents') {
      const noEvents: Record<string, any>[] = [
      ...
      listElement = ListBase.createList(this.parent.createElement, noEvents, {
        ...
        template: `<div class="${
          cls.AGENDA_NO_EVENT_CLASS
        }">${this.parent.localeObj.getConstant('noEvents')}</div>`,
      });
      if (listElement.querySelector('.e-agenda-item').children.length === 0) {
        listElement.firstElementChild.appendChild(
          createElement('div', {
      ...
      listElements.forEach((element: HTMLElement, li: number) => {
        const appWrapper: HTMLElement = createElement('div', {
          ...
          attrs: {
            'data-id':
              'Appointment_' +
              listData[parseInt(li.toString(), 10)][
                `${this.parent.eventFields.id}`
              ],
            'data-guid': listData[parseInt(li.toString(), 10)].Guid as string,
            role: 'button',
            tabindex: '0',
            'aria-disabled': this.parent.eventBase.getReadonlyAttribute(
              listData[parseInt(li.toString(), 10)]
            ),
            'aria-label': this.parent.getAnnouncementString(
              listData[parseInt(li.toString(), 10)]
            ),
          },
        });
        ...
        if (!isNullOrUndefined(this.parent.activeViewOptions.eventTemplate)) {
          ...
          templateEle = this.parent.getAppointmentTemplate()(
            listData[parseInt(li.toString(), 10)],
            this.parent,
            'eventTemplate',
            templateId,
          ...
          if (
            !isNullOrUndefined(
              listData[parseInt(li.toString(), 10)][fieldMapping.recurrenceRule]
            )
          ) {
            ...
            appWrapper.appendChild(
              createElement('div', { className: cls.ICON + ' ' + iconClass })
            );
          }
        } else {
          templateEle = this.createAppointment(
            listData[parseInt(li.toString(), 10)]
          );
        }
  ...
  public createAppointment(event: Record<string, any>): HTMLElement[] {
    const fieldMapping: EventFieldsMapping = this.parent.eventFields;
    let eventSubject: string = (event[fieldMapping.subject] ||
      this.parent.eventSettings.fields.subject.default ||
      this.parent.localeObj.getConstant('addTitle')) as string;
    const eventLocation: string = (event[fieldMapping.location] ||
      this.parent.eventSettings.fields.location.default) as string;
    const appSubjectWrap: HTMLElement = createElement('div', {
      className: cls.SUBJECT_WRAP,
    });
    if (!isNullOrUndefined(eventLocation) && eventLocation !== '') {
      eventSubject += ',';
    }
    const appSubjectText: HTMLElement = createElement('div', {
    ...
    if (!isNullOrUndefined(event[fieldMapping.recurrenceRule])) {
      const iconClass: string =
        event[fieldMapping.id] === event[fieldMapping.recurrenceID]
          ? cls.EVENT_RECURRENCE_ICON_CLASS
          : cls.EVENT_RECURRENCE_EDIT_ICON_CLASS;
      appSubjectWrap.appendChild(
        createElement('div', { className: cls.ICON + ' ' + iconClass })
      );
    }
    ...
    const timeStr: string =
      this.parent.getTimeString(strDate) +
      ' - ' +
      this.parent.getTimeString(endDate);
    if (!isNullOrUndefined(event.data)) {
      const milliSeconds: number =
        endDate.getTimezoneOffset() !== strDate.getTimezoneOffset()
          ? endDate.getTime() - strDate.getTime() + 3600000
          : endDate.getTime() - strDate.getTime();
      const eventString: string =
        milliSeconds / util.MS_PER_DAY >= 1 ? allDayStr : timeStr;
      allDayStr =
        eventString +
        ' (' +
        this.parent.localeObj.getConstant('day') +
        ' ' +
        (event.data as Record<string, any>).index +
        '/' +
        (event.data as Record<string, any>).count +
        ')';
    }
    const displayStr: string =
      !isNullOrUndefined(event.data) || isAllDay ? allDayStr : timeStr;
    const appDateTime: HTMLElement = createElement('div', {
  ...
  public processAgendaEvents(
    ...
    for (const event of events) {
      const spanned: Record<string, any>[] =
        this.parent.eventBase.splitEventByDay(event);
      eventsProcessed = eventsProcessed.concat(
        spanned.length > 1 ? spanned : event
      );
    }
    return eventsProcessed;
  }
  ...
  public wireEventActions(): void {
    const eventElement: HTMLElement[] = [].slice.call(
    ...
    for (const element of eventElement) {
      this.parent.eventBase.wireAppointmentEvents(
        element,
        this.parent.getEventDetails(element),
        true
      );
    }
    const dateHeaderElement: Element[] = [].slice.call(
      this.parent.element.querySelectorAll('.e-m-date')
    );
    for (const element of dateHeaderElement) {
      EventHandler.add(
        element,
        'click',
        this.parent.agendaModule.dayNavigationClick,
        this
      );
    }
  }

  public calculateResourceTableElement(
    tBody: Element,
    noOfDays: number,
    agendaDate: Date,
    agendaEnd: Date = null
    ...
    let agendaLastDate: Date = util.addDays(
      new Date(agendaDate.getTime()),
      noOfDays
    );
    const days: number =
    ...
    for (let i: number = 0; i < days; i++) {
      const lastLevelInfo: TdData[][] = [];
      const tempLastLevelInfo: TdData[] = [];
      let tempIndex: number = 0;
      let eventObj: AgendaSlotData;
      let dateObj: TdData;
      ...
      const finalDate: Date =
        ...
        this.parent.currentView === 'MonthAgenda'
          ? util.addDays(firstDate, 1)
          : agendaLastDate;
      const agendaCollection: Record<string, any>[] =
        this.parent.eventBase.filterEvents(firstDate, finalDate);
      if (
        agendaCollection.length > 0 ||
        !this.parent.hideEmptyAgendaDays ||
        this.parent.currentView === 'MonthAgenda'
      ) {
        for (let res: number = 0; res < resData.length; res++) {
          ...
          // eslint-disable-next-line max-len
          const resDataCollection: Record<string, any>[] =
            this.parent.eventBase.filterEvents(
              agendaDate,
              agendaLastDate,
              agendaCollection,
              resData[parseInt(res.toString(), 10)]
            );
          if (
            resDataCollection.length > 0 ||
            !this.parent.hideEmptyAgendaDays ||
            this.parent.currentView === 'MonthAgenda'
          ) {
            for (let r: number = 0; r < noOfDays; r++) {
              // eslint-disable-next-line max-len
              const resDayCollection: Record<string, any>[] =
                this.parent.eventBase.filterEvents(
                  agendaDate,
                  util.addDays(agendaDate, 1),
                  resDataCollection,
                  undefined
                );
              if (
                ((showWeekend ||
                  (!showWeekend &&
                    (this.parent.group.byDate
                      ? this.isWorkDay(agendaDate)
                      : this.isWorkDay(
                          agendaDate,
                          resData[parseInt(res.toString(), 10)].workDays
                        )))) &&
                  (resDayCollection.length > 0 ||
                    !this.parent.hideEmptyAgendaDays)) ||
                this.parent.currentView === 'MonthAgenda'
              ) {
                ...
                eventObj = {
                  ...
                  resourceData:
                    resData[parseInt(res.toString(), 10)].resourceData,
                ...
                dateObj = {
                  ...
                  type: 'dateColumn',
                  resource:
                    resColl[parseInt((resColl.length - 1).toString(), 10)],
                  groupOrder: resData[parseInt(res.toString(), 10)].groupOrder,
                  resourceData:
                    resData[parseInt(res.toString(), 10)].resourceData,
                  date: agendaDate,
                ...
                lastLevelInfo[parseInt(tempIndex.toString(), 10)].push(
                  eventObj
                );
              ...
              if (
                agendaDate.getTime() >= agendaLastDate.getTime() ||
                this.parent.activeViewOptions.group.byDate ||
                this.parent.currentView === 'MonthAgenda'
              ) {
                if (data.length > 0) {
                  lastLevelInfo[lastLevelInfo.length - 1][1].cssClass =
                    cls.AGENDA_DAY_BORDER_CLASS;
                  ...
                  lastLevelInfo[
                    parseInt(
                      (lastLevelInfo.length - data.length).toString(),
                      10
                    )
                  ].push(tempObj);
                  tempLastLevelInfo.push(
                    <TdData>extend({}, tempObj, null, true)
                  );
                }
                break;
              }
        ...
        const topResources: ResourcesModel[] = resColl.slice(0, -1);
        let tempGroupedData: TdData[] = [];
        let totalRowSpan: number = 0;
        for (let y: number = 0; y < topResources.length; y++) {
          const data: Record<string, any>[] = topResources[
            topResources.length - (y + 1)
          ...
          for (let x: number = 0; x < data.length; x++) {
            ...
            for (let u: number = 0; u < tempLastLevelInfo.length; u++) {
              // eslint-disable-next-line max-len
              if (
                tempLastLevelInfo[parseInt(u.toString(), 10)].groupOrder[
                  topResources.length - (y + 1)
                ] ===
                ...
                totalRowSpan =
                  totalRowSpan +
                  tempLastLevelInfo[parseInt(u.toString(), 10)].rowSpan;
                tempGroupedData.push(
                  <TdData>(
                    extend(
                      {},
                      tempLastLevelInfo[parseInt(u.toString(), 10)],
                      null,
                      true
                    )
                  )
              ...
              if (
                ++z === tempLastLevelInfo.length &&
                tempGroupedData.length > 0
              ) {
                tempGroupedData[0].rowSpan = totalRowSpan;
                tempGroupedData[0].type = 'parentColumnLevel_' + (y + 1);
                tempGroupedData[0].resource =
                  topResources[topResources.length - (y + 1)];
                tempGroupedData[0].resourceData =
                  data[parseInt(x.toString(), 10)];
                tempGroupedData[0].date = agendaDate;
                lastLevelInfo[tempGroupedData[0].groupIndex].push(
                  tempGroupedData[0]
                );
                tempGroupedData = [];
                totalRowSpan = 0;
    ...
    const totalCollection: Record<string, any>[] =
      this.parent.eventBase.filterEvents(agendaStart, agendaLastDate);
    if (
  ...
  private createResourceTableRow(tContent: TdData[][], tBody: Element): void {
    ...
    if (
      ...
      const parentCollection: ResourcesModel[] =
        this.parent.resourceBase.resourceCollection.slice(0, -1);
      for (let w: number = 0; w < tContent.length; w++) {
        tContentCollection = tContentCollection.concat(
          tContent[parseInt(w.toString(), 10)]
        );
      }
      level =
        parentCollection.length > 0
          ? 'parentColumnLevel_' + parentCollection.length
          : 'resourceColumn';
      const rowSpanCollection: TdData[] = tContentCollection.filter(
        (data: TdData) => data.type === level
      );
      for (let x: number = 0; x < rowSpanCollection.length; x++) {
        rowSpan =
          rowSpan + rowSpanCollection[parseInt(x.toString(), 10)].rowSpan;
      }
    }
    for (let row: number = 0; row < tContent.length; row++) {
      ntr = tr.cloneNode() as Element;
      for (
        let col: number = tContent[parseInt(row.toString(), 10)].length - 1;
        col >= 0;
        col--
      ) {
        const data: AgendaSlotData =
          tContent[parseInt(row.toString(), 10)][parseInt(col.toString(), 10)];
        ...
        if (data.type === 'dateColumn') {
          if (
            ...
            tempData =
              tContent[parseInt(row.toString(), 10)][
                parseInt(col.toString(), 10)
              ];
            continue;
          }
          ...
          const className: string[] = [
            cls.AGENDA_CELLS_CLASS,
            cls.AGENDA_DATE_CLASS,
          ];
          if (data.cssClass) {
          ...
          const elementType: string =
            data.eventData.length === 0 ? 'noEvents' : 'data';
          ntd = this.createAgendaContentElement(
            elementType,
            data.eventData,
            ntd,
            data.groupOrder,
          ...
          if (
            ...
            addClass(
              [ntd],
              [cls.AGENDA_CELLS_CLASS, cls.AGENDA_DAY_PADDING_CLASS]
            );
          }
          ntr.appendChild(ntd);
        } else {
      ...
      if (
        ...
        addClass(
          [ntd],
          [cls.AGENDA_CELLS_CLASS, cls.AGENDA_DATE_CLASS, cls.DATE_BORDER_CLASS]
        );
        const daysCount: number = util.getDaysCount(
          this.parent.selectedDate.getTime(),
          tempData.date.getTime()
        );
        ntr.setAttribute('data-row-index', daysCount.toString());
        if (
          this.parent.element.querySelector(
            `.e-agenda-view tr[data-row-index="${daysCount}"]`
          )
        ) {
          break;
        }
  ...
  public createTableRowElement(date: Date, type: string): Element {
    ...
    const tr: Element = createElement('tr', {
      attrs: { 'data-row-index': daysCount.toString() },
    });
    const td: Element = createElement('td', {
      attrs: {
        class:
