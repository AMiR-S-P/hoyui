...
export class InlineEdit {
  ...
  private inlineEdit(args: InlineClickArgs): void {
    ...
    if (args.type === 'Cell') {
      ...
      if (
        this.parent.element.querySelector('.' + cls.INLINE_SUBJECT_CLASS) !==
  ...
  private cellEdit(args: InlineClickArgs): void {
    ...
    const start: number = util
      .resetTime(new Date('' + saveObj[this.parent.eventFields.startTime]))
      .getTime();
    const end: number = util
      .resetTime(new Date('' + saveObj[this.parent.eventFields.endTime]))
      .getTime();
  ...
  private eventEdit(args: InlineClickArgs): void {
    let inlineSubject: HTMLInputElement = args.element.querySelector(
      '.' + cls.INLINE_SUBJECT_CLASS
    ) as HTMLInputElement;
    let subject: string;
    if (inlineSubject) {
      ...
      const subEle: HTMLElement = args.element.querySelector(
        '.' + cls.SUBJECT_CLASS
      );
      ...
      if (closest(args.element, '.' + cls.MORE_POPUP_WRAPPER_CLASS)) {
        ...
        let subjectWrap: Element = args.element.querySelector(
          '.' + cls.SUBJECT_WRAP
        );
        ...
        const elementSelector: string =
          [
            ...
            'TimelineWorkWeek',
            'TimelineMonth',
          ].indexOf(this.parent.currentView) > -1
            ? '.e-inner-wrap'
            : '.e-appointment-details';
        const innerWrapElement: Element =
          args.element.querySelector(elementSelector);
        if (isNullOrUndefined(innerWrapElement)) {
          args.element
            .querySelector('.e-appointment-details')
            .prepend(inlineSubject);
        } else {
  ...
  private createVerticalViewInline(
    ...
    const index: number = verticalEvent.dateRender[
      parseInt(resIndex.toString(), 10)
    ]
      .map((date: Date) => date.getDay())
      .indexOf(dayIndex);
    if (count >= 1) {
      verticalEvent.allDayElement = [].slice.call(
        this.parent.element.querySelectorAll('.' + cls.ALLDAY_CELLS_CLASS)
      );
      verticalEvent.slots.push(
        ...this.parent.activeView.renderDates.map((date: Date) => +date)
      );
      const allDayElements: HTMLElement[] = [].slice.call(
        this.parent.element.querySelectorAll('.' + cls.ALLDAY_APPOINTMENT_CLASS)
      );
      ...
      const cellTop: number =
        verticalEvent.allDayElement.length > 0
          ? verticalEvent.allDayElement[0].offsetTop
          : 0;
      verticalEvent.renderAllDayEvents(
        saveObj,
        index,
        resIndex,
        daysCount,
        this.parent.allowInline,
        cellTop,
        appHeight
      );
    } else {
      verticalEvent.renderNormalEvents(
        saveObj,
        index,
        resIndex,
        daysCount,
  ...
  private createMonthViewInline(
    ...
    const count: number = this.getEventDaysCount(saveObj);
    const saveObject: Record<string, any> =
      this.parent.eventBase.cloneEventObject(
        saveObj,
        start,
        end,
        count,
    ...
    monthEvent.workCells = [].slice.call(
      this.parent.element.querySelectorAll(monthCellSelector)
    );
    monthEvent.cellWidth = monthEvent.workCells[0].offsetWidth;
    monthEvent.cellHeight = monthEvent.workCells[0].offsetHeight;
    monthEvent.eventHeight = this.parent.getElementHeightFromClass(
      this.parent.monthModule.element || monthEvent.element,
      cls.APPOINTMENT_CLASS
    );
    monthEvent.getSlotDates(workDays);
    const filteredDates: Date[] = monthEvent.getRenderedDates(renderDates);
    const spannedEvents: Record<string, any>[] = monthEvent.splitEvent(
      saveObject,
      filteredDates || renderDates
    );
    for (const eventData of spannedEvents) {
      monthEvent.renderEvents(eventData, index);
    }
    const inlineSubject: HTMLInputElement = this.parent.element.querySelector(
      '.' + cls.INLINE_SUBJECT_CLASS
    ) as HTMLInputElement;
    inlineSubject.focus();
  }
  ...
  private createTimelineViewInline(
    ...
    const timelineView: TimelineEvent = new TimelineEvent(
      this.parent,
      this.parent.activeViewOptions.timeScale.enable ? 'hour' : 'day'
    );
    timelineView.dateRender = this.parent.activeView.renderDates;
    timelineView.eventContainers = [].slice.call(
    ...
    timelineView.eventHeight = this.parent.getElementHeightFromClass(
  ...
  private getEventDaysCount(saveObj: Record<string, any>): number {
    ...
    const daysCount: number =
      Math.abs(endDate.getTime() - startDate.getTime()) / util.MS_PER_DAY;
    return daysCount;
  }
  ...
  private generateEventData(
    ...
    const inlineElement: HTMLInputElement = this.parent.element.querySelector(
      '.' + cls.INLINE_SUBJECT_CLASS
    ) as HTMLInputElement;
    const subject: string = inlineElement
      ? inlineElement.value
      : target
      ? target.innerHTML
      : '';
    ...
    saveObj[this.parent.eventFields.startTime] =
      this.parent.activeCellsData.startTime;
    saveObj[this.parent.eventFields.endTime] =
      this.parent.activeCellsData.endTime;
    saveObj[this.parent.eventFields.isAllDay] =
  ...
  public documentClick(): void {
    const target: HTMLInputElement = this.parent.element.querySelector(
      '.' + cls.INLINE_SUBJECT_CLASS
    ) as HTMLInputElement;
  ...
  public inlineCrudActions(target: HTMLTableCellElement): void {
    if (closest(target, '.' + cls.INLINE_APPOINTMENT_CLASS)) {
      ...
      const eventDetails: Record<string, any> = this.parent.getEventDetails(
        eventTarget
      ) as Record<string, any>;
      eventDetails[this.parent.eventFields.subject] = (
        target as HTMLTableCellElement & HTMLInputElement
      ).value;
      let currentAction: CurrentAction;
      if (
        eventDetails[this.parent.eventFields.id] ===
        eventDetails[this.parent.eventFields.recurrenceID]
      ) {
  ...
  public createInlineAppointmentElement(
    ...
    const inlineDetails: HTMLElement = createElement('div', {
      className: cls.APPOINTMENT_DETAILS,
    });
    inlineAppointmentElement.appendChild(inlineDetails);
    const inline: HTMLElement = createElement('input', {
      className: cls.INLINE_SUBJECT_CLASS,
    });
  ...
  public removeInlineAppointmentElement(): void {
    const inlineAppointment: Element[] = [].slice.call(
      this.parent.element.querySelectorAll('.' + cls.INLINE_APPOINTMENT_CLASS)
    );
    ...
    if (inlineSubject) {
      const appointmentSubject: Element = closest(
        inlineSubject,
        '.' + cls.APPOINTMENT_CLASS
      );
      const subject: Element = appointmentSubject.querySelector(
        '.' + cls.SUBJECT_CLASS
      );
