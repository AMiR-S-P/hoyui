...
export class TimelineEvent extends MonthEvent {
  ...
  constructor(parent: Schedule, type: string) {
    ...
    this.eventContainers = [].slice.call(
      this.element.querySelectorAll('.' + cls.APPOINTMENT_CONTAINER_CLASS)
    );
    const tr: HTMLElement[] = [].slice.call(
      this.element.querySelectorAll('.' + cls.CONTENT_TABLE_CLASS + ' tbody tr')
    );
    this.dayLength = tr.length === 0 ? 0 : tr[0].children.length;
    this.content = this.parent.element.querySelector(
      '.' + cls.SCHEDULE_TABLE_CLASS + '.' + cls.CONTENT_TABLE_CLASS
    );
  }
  ...
  public getSlotDates(): void {
    this.slots = [];
    this.slots.push(
      this.parent.activeView.renderDates.map((date: Date) => {
        return +date;
      }) as any
    );
    if (
      ...
      this.renderType = 'day';
      const workCell: HTMLTableCellElement = this.content.querySelector(
        '.' + cls.WORK_CELLS_CLASS
      ) as HTMLTableCellElement;
      this.cellWidth =
        this.parent.getElementWidth(workCell) /
        +(workCell.getAttribute('colspan') || 1);
      this.slotsPerDay = 1;
    } else {
      ...
      for (const app of appointments) {
        if (
          util.resetTime(<Date>app[this.fields.startTime]).getTime() <=
            util.resetTime(new Date(date.getTime())).getTime() &&
          util.resetTime(<Date>app[this.fields.endTime]).getTime() >=
            util.resetTime(new Date(date.getTime())).getTime()
        ) {
  ...
  public getSortComparerIndex(startDate: Date, endDate: Date): number {
    ...
    if (appointments.length > 0) {
      const appointmentsList: Record<string, any>[] =
        this.getOverlapSortComparerEvents(startDate, endDate, appointments);
  ...
  public getOverlapSortComparerEvents(
    startDate: Date,
    endDate: Date,
    ...
    for (const app of appointmentsCollection) {
      if (this.renderType === 'day') {
        ...
        const appStart: number = util
          .resetTime(<Date>app[this.fields.startTime])
          .getTime();
        const appEnd: number = util
          .resetTime(<Date>app[this.fields.endTime])
          .getTime();
        const isEndOverlap: () => boolean = () => {
          let endTime: number =
            end - (util.getDateInMs(endDate) <= 0 ? util.MS_PER_DAY : 0);
          endTime = start > endTime ? start : endTime;
          return appEnd >= endTime && appStart <= endTime;
        };
        if (
          (appStart <= start && appEnd >= start) ||
          isEndOverlap() ||
        ...
        const eventData: Record<string, any> = app.data as Record<string, any>;
        if (
          (eventData.trimStartTime.getTime() <= startDate.getTime() &&
            startDate.getTime() < eventData.trimEndTime.getTime()) ||
          (startDate.getTime() <= eventData.trimStartTime.getTime() &&
            eventData.trimStartTime.getTime() < endDate.getTime())
        ) {
          appointments.push(app);
        }
      }
  ...
  public renderResourceEvents(): void {
    ...
    const selector: string =
      '.' + cls.RESOURCE_COLUMN_TABLE_CLASS + ' tbody tr';
    this.addCellHeight(
      selector,
      this.eventHeight,
      EVENT_GAP,
    ...
    if (this.parent.crudModule && this.parent.crudModule.crudObj.isCrudAction) {
      for (
        ...
        if (
          ...
          (this.parent.uiStateValues.groupIndex === source.groupIndex &&
            this.parent.uiStateValues.isGroupAdaptive)
        ) {
      ...
      for (let i: number = 0; i < resources.length; i++) {
        ...
        this.renderEventsHandler(
          this.parent.activeView.renderDates,
          this.parent.activeViewOptions.workDays,
          resources[parseInt(i.toString(), 10)]
        );
      }
    }
  }
  ...
  public renderEvents(
    ...
    resIndex: number,
    appointmentsList?: Record<string, any>[]
  ): void {
    ...
    const startEndHours: Record<string, Date> = util.getStartEndHours(
      event[this.fields.startTime],
      this.startHour,
      this.endHour
    );
    ...
    this.day = this.parent.getIndexOfDate(
      this.dateRender,
    ...
    const overlapCount: number = isNullOrUndefined(
      this.parent.eventSettings.sortComparer
    )
      ? this.getIndex(startTime)
      : this.getSortComparerIndex(startTime, endTime);
    ...
    const currentDate: Date = util.resetTime(
      new Date(this.dateRender[this.day].getTime())
    );
    ...
    if (startTime <= endTime && isValidEvent) {
      ...
      if (isNullOrUndefined(this.cellTops[parseInt(resIndex.toString(), 10)])) {
        this.cellTops[parseInt(resIndex.toString(), 10)] =
          this.getRowTop(resIndex);
      }
      const top: number = this.cellTops[parseInt(resIndex.toString(), 10)];
      const appTop: number =
        top +
        (this.maxHeight ? 0 : EVENT_GAP) +
        overlapCount * (appHeight + EVENT_GAP);
      appLeft = this.parent.enableRtl ? 0 : position;
      appRight = this.parent.enableRtl ? position : 0;
      const height: number =
        (overlapCount + 1) * (appHeight + EVENT_GAP) + this.moreIndicatorHeight;
      ...
      if (this.parent.rowAutoHeight || renderApp) {
        ...
        if (this.parent.rowAutoHeight) {
          ...
          const isWithoutScroll: boolean =
            conWrap.offsetHeight === conWrap.clientHeight &&
          ...
          if (
            isWithoutScroll &&
            (conWrap.offsetWidth > conWrap.clientWidth ||
              conWidth !== this.parent.getElementWidth(conWrap))
          ) {
        ...
        for (let i: number = 0; i < diffInDays; i++) {
          const moreIndicator: HTMLElement = cellTd.querySelector(
          ...
          let startDate: Date =
            ...
            this.renderType === 'day' ||
            i !== 0
              ? new Date(this.dateRender[this.day + i].getTime())
              : new Date(startTime);
          ...
          if (
            ...
            this.renderTimelineMoreIndicator(
              startTime,
              startDate,
              endDate,
              appHeight,
            ...
            const slotCount: number =
              (((util.getUniversalTime(endTime) -
                util.getUniversalTime(startTime)) /
                util.MS_PER_MINUTE) *
                this.slotCount) /
              this.interval;
            for (let k: number = 0; k < slotCount; k++) {
              startDate =
                k === 0
                  ...
                  : new Date(startDate.getTime() + 60000 * interval);
              if (slotCount < 1) {
  ...
  private adjustToNearestTimeSlot(inputTime: Date, interval: number): Date {
    ...
    const minutesInMilliseconds: number =
      parsedTime.getHours() * 60 * 60 * 1000 +
      parsedTime.getMinutes() * 60 * 1000;

    // Calculate the adjusted time in milliseconds (nearest time slot)
    const adjustedMinutesInMilliseconds: number =
      Math.floor(minutesInMilliseconds / (interval * 60 * 1000)) *
      (interval * 60 * 1000);

    // Create a new Date object with the adjusted time
    const adjustedTime: Date = new Date(parsedTime.getTime());
    adjustedTime.setHours(
      (adjustedMinutesInMilliseconds / (60 * 60 * 1000)) % 24
    );
    adjustedTime.setMinutes(
      (adjustedMinutesInMilliseconds % (60 * 60 * 1000)) / (60 * 1000)
    );
  ...
  private renderTimelineMoreIndicator(
    ...
    appPos = Math.floor(appPos / this.cellWidth) * this.cellWidth;
    if (
      (cellTd && isNullOrUndefined(moreIndicator)) ||
      ...
      const startDateTime: Date =
        ...
        this.renderType === 'day'
          ? new Date(+startTime)
          : startDate;
      const slotStartTime: Date = new Date(
        startDateTime.setMinutes(
          Math.floor(startDateTime.getMinutes() / interval) * interval
        )
      );
      const slotEndTime: Date = new Date(
        slotStartTime.getTime() + 60000 * interval
      );
      let groupIndex: string;
      if (
        this.parent.activeViewOptions.group.resources.length > 0 &&
        !isNullOrUndefined(resIndex)
      ) {
        groupIndex = resIndex.toString();
      }
      const filterEvents: Record<string, any>[] = this.getFilterEvents(
        startDate,
        endDate,
        slotStartTime,
        slotEndTime,
      ...
      const renderedAppCount: number = Math.floor(
        appArea / (appHeight + EVENT_GAP)
      );
      const count: number =
        this.parent.activeViewOptions.maxEventsPerRow &&
        !this.parent.eventSettings.enableIndicator
          ? filterEvents.length - this.parent.activeViewOptions.maxEventsPerRow
          : filterEvents.length - renderedAppCount <= 0
          ? 1
          : filterEvents.length - renderedAppCount;
      let moreIndicatorElement: HTMLElement;
      if (this.renderType === 'day') {
        moreIndicatorElement = this.getMoreIndicatorElement(
          count,
          startDate,
          endDate
        );
      ...
      moreIndicatorElement.style.left =
        Math.floor(appLeft / this.cellWidth) * this.cellWidth + 'px';
      moreIndicatorElement.style.right =
        Math.floor(appRight / this.cellWidth) * this.cellWidth + 'px';
  ...
  public updateCellHeight(cell: HTMLElement, height: number): void {
    ...
    if (height > cellHeight) {
      ...
      const monthHeader: HTMLElement = this.parent.element.querySelector(
        '.e-month-header-wrapper table tr:nth-child(' +
  ...
  private adjustAppointments(conWidth: number): void {
    const tr: HTMLElement = this.parent.element.querySelector(
      '.' + cls.CONTENT_TABLE_CLASS + ' tbody tr'
    );
    const actualCellWidth: number = this.parent.getElementWidth(
      this.workCells[0]
    );
    this.cellWidth =
      actualCellWidth / +(this.workCells[0].getAttribute('colspan') || 1);
    const currentPercentage: number =
      (actualCellWidth * tr.children.length) / (conWidth / 100);
    ...
    apps.forEach((app: HTMLElement) => {
      if (this.parent.enableRtl && app.style.right !== '0px') {
        app.style.right =
          (parseFloat(app.style.right) / 100) * currentPercentage + 'px';
      } else if (app.style.left !== '0px') {
        app.style.left =
          (parseFloat(app.style.left) / 100) * currentPercentage + 'px';
      }
      app.style.width =
        (parseFloat(app.style.width) / 100) * currentPercentage + 'px';
    });
  }
  ...
  public updateBlockElements(): void {
    ...
    for (const element of blockElement) {
      const resIndex: number = parseInt(
        element.getAttribute('data-group-index'),
        10
      );
      const firstChild: HTMLElement = this.getFirstChild(resIndex);
    ...
    const blockIndicator: HTMLElement[] = [].slice.call(
      this.element.querySelectorAll('.' + cls.BLOCK_INDICATOR_CLASS)
    );
    for (const element of blockIndicator) {
      ...
      element.style.top =
        this.getRowTop(resIndex) +
        this.getFirstChild(resIndex).offsetHeight -
        BLOCK_INDICATOR_HEIGHT +
        'px';
    }
  }
  ...
  public getStartTime(
    ...
    if (this.isDayProcess()) {
      ...
      if (schedule.startHour.getTime() >= eventData[this.fields.startTime]) {
        ...
        schedule.endHour.getTime() <= eventData[this.fields.startTime]
      ) {
    ...
    eventData.trimStartTime = event[this.fields.isAllDay]
      ? schedule.startHour
      : eventData[this.fields.startTime];
    return startTime;
  }
  ...
  public getEndTime(
    ...
    if (this.isDayProcess()) {
      ...
      endTime = eventData[this.fields.endTime] as Date;
      if (
        schedule.endHour.getTime() <= eventData[this.fields.endTime] ||
        event[this.fields.isAllDay]
      ) {
      ...
      if (
        schedule.startHour.getTime() >=
          eventData[this.fields.endTime].getTime() &&
        !event[this.fields.isAllDay]
      ) {
        endTime = this.getPreviousDay(
          schedule.startHour,
          schedule.endHour,
          eventData
        );
    ...
    // To overcome the overflow
    eventData.trimEndTime = event[this.fields.isAllDay]
      ? schedule.endHour
      : eventData[this.fields.endTime];
    return endTime;
  }
  ...
  private getPreviousDay(
    ...
    for (let i: number = 1; i <= this.dateRender.length; i++) {
      ...
      if (
        ...
        eventData.count =
          actualEndTime.getHours() !== 0 || actualEndTime.getMinutes() !== 0
            ? count - 1
            : count;
        return endDate;
      }
  ...
  private getSameDayEventsWidth(startDate: Date, endDate: Date): number {
    ...
    if (this.slotsPerDay === 1) {
      const hoursRange: { [key: string]: Date } = util.getStartEndHours(
        util.resetTime(new Date(startDate.getTime())),
        this.startHour,
        this.endHour
      );
      intervalMins =
        (hoursRange.endHour.getTime() - hoursRange.startHour.getTime()) /
        util.MS_PER_MINUTE;
    }
    return (
      (((util.getUniversalTime(endDate) - util.getUniversalTime(startDate)) /
        util.MS_PER_MINUTE) *
        (this.cellWidth * this.slotCount)) /
      intervalMins
    );
  }
  ...
  private getSpannedEventsWidth(
    ...
    let endWidth: number;
    const start: { [key: string]: Date } = util.getStartEndHours(
      util.resetTime(new Date(startDate.getTime())),
      this.startHour,
      this.endHour
    );
    const startWidth: number = this.getSameDayEventsWidth(
      start.startHour,
      startDate
    );
    if (
      this.parent.getIndexOfDate(
        this.dateRender,
        util.resetTime(new Date(endDate.getTime()))
      ) === -1
    ) {
      endWidth = 0;
    } else {
      const end: { [key: string]: Date } = util.getStartEndHours(
        util.resetTime(new Date(endDate.getTime())),
        this.startHour,
        this.endHour
      );
  ...
  private isSameDay(startTime: Date, endTime: Date): boolean {
    const startDay: number = this.parent.getIndexOfDate(
      this.dateRender,
      util.resetTime(new Date(startTime.getTime()) as Date)
    );
    const endDay: number = this.parent.getIndexOfDate(
      this.dateRender,
      util.resetTime(new Date(endTime.getTime()) as Date)
    );
  ...
  private getAppointmentLeft(
    ...
    const slotTd: number = this.isSameDay(startTime, schedule.startHour as Date)
      ? ((util.getUniversalTime(startTime) -
          util.getUniversalTime(schedule.startHour)) /
          (util.MS_PER_MINUTE * this.interval)) *
        this.slotCount
      : 0;
    if (day === 0) {
      return slotTd;
    } else {
      const daySlot: number = Math.round(
        ((util.getUniversalTime(schedule.endHour) -
          util.getUniversalTime(schedule.startHour)) /
          util.MS_PER_MINUTE /
          this.interval) *
          this.slotCount
  ...
  public getPosition(
    ...
    const currentDate: Date = util.resetTime(
      new Date(this.dateRender[parseInt(day.toString(), 10)].getTime())
    );
    const schedule: { [key: string]: Date } = util.getStartEndHours(
      currentDate,
      this.startHour,
    ...
    if (
      schedule.endHour.getTime() <= endTime.getTime() &&
      schedule.startHour.getTime() >= startTime.getTime()
    ) {
  ...
  private isAlreadyAvail(appPos: number, cellTd: HTMLElement): boolean {
    const moreIndicator: HTMLElement[] = [].slice.call(
      cellTd.querySelectorAll('.' + cls.MORE_INDICATOR_CLASS)
    );
    for (let i: number = 0; i < moreIndicator.length; i++) {
      let indicatorPos: string;
      if (moreIndicator) {
        indicatorPos = this.parent.enableRtl
          ? moreIndicator[parseInt(i.toString(), 10)].style.right
          : moreIndicator[parseInt(i.toString(), 10)].style.left;
      }
  ...
  public getCellTd(): HTMLElement {
    const wrapIndex: number = this.parent.uiStateValues.isGroupAdaptive
      ? 0
      : this.rowIndex;
    return this.eventContainers[
      parseInt(wrapIndex.toString(), 10)
    ] as HTMLElement;
  }
  ...
  public renderBlockIndicator(
    ...
    if (
      this.parent.headerRows.length > 0 &&
      (this.parent.headerRows[this.parent.headerRows.length - 1].option !==
        'Hour' ||
        this.parent.headerRows[this.parent.headerRows.length - 1].option !==
          'Date')
    ) {
    ...
    position =
      Math.floor(position / this.cellWidth) * this.cellWidth +
      this.cellWidth -
      BLOCK_INDICATOR_WIDTH;
