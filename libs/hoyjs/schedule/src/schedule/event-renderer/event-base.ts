...
import { DataManager, Query, Predicate } from '@hoyui/data';
import {
  EventFieldsMapping,
  EventClickArgs,
  CellClickEventArgs,
...
} from '../../recurrence-editor/date-generator';
...
export class EventBase {
  ...
  public processData(
    ...
    const resourceCollection: ResourcesModel[] = this.parent.resourceBase
    ...
    for (let event of events) {
      ...
      if (timeZonePropChanged) {
        ...
        !this.parent.uiStateValues.isPreventTimezone
      ) {
        event = this.processTimezone(event);
      }
      for (let level: number = 0; level < resourceCollection.length; level++) {
        if (
          event[resourceCollection[parseInt(level.toString(), 10)].field] ===
            null ||
          event[resourceCollection[parseInt(level.toString(), 10)].field] === 0
        ) {
          event[resourceCollection[parseInt(level.toString(), 10)].field] =
            undefined;
        }
      ...
      if (
        !isNullOrUndefined(event[fields.recurrenceRule]) &&
        event[fields.recurrenceRule] === ''
      ) {
        event[fields.recurrenceRule] = null;
      }
      if (
        !isNullOrUndefined(event[fields.recurrenceRule]) &&
        isNullOrUndefined(event[fields.recurrenceID]) &&
        !(this.parent.crudModule && this.parent.crudModule.crudObj.isCrudAction)
      ) {
        processed = processed.concat(
          this.generateOccurrence(event, null, true)
        );
      } else {
        if (
          this.parent.crudModule &&
          this.parent.crudModule.crudObj.isCrudAction
        ) {
          if (
            !isNullOrUndefined(event[fields.recurrenceRule]) &&
            isNullOrUndefined(event[fields.recurrenceID])
            ...
            for (const occurrence of recurrenceEvent) {
              const app: Record<string, any>[] =
                this.parent.eventsProcessed.filter(
                  (data: Record<string, Date>) =>
                    data[fields.startTime].getTime() -
                      (<Date>occurrence[fields.startTime]).getTime() ===
                      0 && data[fields.id] === occurrence[fields.id]
                );
              occurrence.Guid =
                app.length > 0 ? app[0].Guid : this.generateGuid();
            ...
            const app: Record<string, any>[] =
              this.parent.eventsProcessed.filter(
                (data: Record<string, any>) =>
                  data[this.parent.eventFields.id] ===
                  event[this.parent.eventFields.id]
              );
    ...
    const eventData: Record<string, any>[] = processed.filter(
      (data: Record<string, any>) => !data[this.parent.eventFields.isBlock]
    );
    this.parent.eventsProcessed = this.filterEvents(start, end, eventData);
    const blockData: Record<string, any>[] = processed.filter(
      (data: Record<string, any>) => data[this.parent.eventFields.isBlock]
    );
    for (const eventObj of blockData) {
      if (eventObj[fields.isAllDay]) {
        const isDifferentDate: boolean =
          util.resetTime(
            new Date((eventObj[fields.startTime] as Date).getTime())
          ) <
          util.resetTime(
            new Date((eventObj[fields.endTime] as Date).getTime())
          );
        if (!isDifferentDate) {
          eventObj[fields.startTime] = util.resetTime(
            eventObj[fields.startTime] as Date
          );
          eventObj[fields.endTime] = util.addDays(
            util.resetTime(eventObj[fields.endTime] as Date),
            1
          );
        }
      }
  ...
  public updateEventDateTime(
    eventData: Record<string, any>
  ): Record<string, any> {
    if (typeof eventData[this.parent.eventFields.startTime] === 'string') {
      eventData[this.parent.eventFields.startTime] = util.getDateFromString(
        eventData[this.parent.eventFields.startTime]
      );
    }
    if (typeof eventData[this.parent.eventFields.endTime] === 'string') {
      eventData[this.parent.eventFields.endTime] = util.getDateFromString(
        eventData[this.parent.eventFields.endTime]
      );
    }
    return eventData;
  }

  public getProcessedEvents(
    eventCollection: Record<string, any>[] = this.parent.eventsData
  ): Record<string, any>[] {
    let processed: Record<string, any>[] = [];
    for (const event of eventCollection) {
      if (
        !isNullOrUndefined(event[this.parent.eventFields.recurrenceRule]) &&
  ...
  public timezonePropertyChange(oldTimezone: string): void {
    const data: Record<string, any>[] = this.parent.eventsData.concat(
      this.parent.blockData
    ) as Record<string, any>[];
    const processed: Record<string, any>[] = this.processData(
      data,
      true,
      oldTimezone
    );
  ...
  public timezoneConvert(eventData: Record<string, any>): void {
    const fields: EventFieldsMapping = this.parent.eventFields;
    eventData[fields.startTimezone] =
      eventData[fields.startTimezone] || eventData[fields.endTimezone];
    eventData[fields.endTimezone] =
      eventData[fields.endTimezone] || eventData[fields.startTimezone];
    if (this.parent.timezone) {
      ...
      eventData[fields.startTime] = this.parent.tzModule.convert(
        <Date>eventData[fields.startTime],
        this.parent.timezone,
        startTz
      );
      eventData[fields.endTime] = this.parent.tzModule.convert(
        <Date>eventData[fields.endTime],
        this.parent.timezone,
        endTz
      );
    }
  ...
  private processTimezoneChange(
    ...
    if (oldTimezone && this.parent.timezone) {
      ...
      event[fields.endTime] = this.parent.tzModule.convert(
        <Date>event[fields.endTime],
        oldTimezone,
        this.parent.timezone
      );
    } else if (!oldTimezone && this.parent.timezone) {
      event[fields.startTime] = this.parent.tzModule.add(
        <Date>event[fields.startTime],
        this.parent.timezone
      );
      event[fields.endTime] = this.parent.tzModule.add(
        <Date>event[fields.endTime],
        this.parent.timezone
      );
    } else if (oldTimezone && !this.parent.timezone) {
      event[fields.startTime] = this.parent.tzModule.remove(
        <Date>event[fields.startTime],
        oldTimezone
      );
      event[fields.endTime] = this.parent.tzModule.remove(
        <Date>event[fields.endTime],
        oldTimezone
      );
    }
  ...
  public processTimezone(
    ...
    if (event[fields.startTimezone] || event[fields.endTimezone]) {
      const startTimezone: string =
        <string>event[fields.startTimezone] ||
        <string>event[fields.endTimezone];
      const endTimezone: string =
        <string>event[fields.endTimezone] ||
        <string>event[fields.startTimezone];
      if (isReverse) {
        if (this.parent.timezone) {
  ...
  public filterBlockEvents(
    ...
    const blockEvents: Record<string, any>[] = <Record<string, any>[]>(
      extend([], this.parent.blockProcessed, null, true)
    );
    for (const eventObj of blockEvents) {
      if (eventObj[fields.isAllDay]) {
        const isDifferentTime: boolean =
          (eventObj[fields.endTime] as Date).getTime() >
          util
            .resetTime(new Date((eventObj[fields.endTime] as Date).getTime()))
            .getTime();
        if (isDifferentTime) {
          eventObj[fields.startTime] = util.resetTime(
            eventObj[fields.startTime] as Date
  ...
  public filterEvents(
    startDate: Date,
    endDate: Date,
    appointments: Record<string, any>[] = this.parent.eventsProcessed,
    resourceTdData?: TdData
  ): Record<string, any>[] {
    const predicate: Predicate = this.parent.dataModule.getStartEndQuery(
      startDate,
      endDate
    );
    let filter: Record<string, any>[] = new DataManager({
      json: appointments,
    }).executeLocal(new Query().where(predicate)) as Record<string, any>[];
    if (resourceTdData) {
  ...
  public filterEventsByRange(
    ...
    if (startDate && endDate) {
      ...
      filteredEvents = eventCollection.filter(
        (e: Record<string, any>) =>
          e[this.parent.eventFields.startTime] >= startDate
      );
    } else if (!startDate && endDate) {
      filteredEvents = eventCollection.filter(
        (e: Record<string, any>) =>
          e[this.parent.eventFields.endTime] <= endDate
      );
    } else {
  ...
  public filterEventsByResource(
    ...
    const resourceCollection: ResourcesModel[] =
      this.parent.resourceBase.resourceCollection;
    for (let level: number = 0; level < resourceCollection.length; level++) {
      predicate[resourceCollection[parseInt(level.toString(), 10)].field] =
        resourceTdData.groupOrder[parseInt(level.toString(), 10)];
    }
    const keys: string[] = Object.keys(predicate);
    const filteredCollection: Record<string, any>[] = appointments.filter(
      (eventObj: Record<string, any>) =>
        keys.every((key: string) => {
          if (eventObj[`${key}`] instanceof Array) {
            return (
              (<(string | number)[]>eventObj[`${key}`]).indexOf(
                predicate[`${key}`]
              ) > -1
            );
          } else {
  ...
  public sortByTime(
    appointmentsCollection: Record<string, any>[]
  ): Record<string, any>[] {
    if (
      this.parent.eventSettings.sortComparer &&
      (typeof this.parent.eventSettings.sortComparer === 'function' ||
        typeof this.parent.eventSettings.sortComparer === 'string')
    ) {
  ...
  private customSorting(
    ...
    if (typeof this.parent.eventSettings.sortComparer === 'function') {
      return this.parent.eventSettings.sortComparer.call(
        this.parent,
        appointments
      );
    } else if (typeof this.parent.eventSettings.sortComparer === 'string') {
      const splits: string[] = (
        this.parent.eventSettings.sortComparer as string
      ).split('.');
      let sortFn: Function;
  ...
  public splitEventByDay(event: Record<string, any>): Record<string, any>[] {
    ...
    const isDifferentDate: boolean =
      util.resetTime(new Date(eventStartTime.getTime())) <
      util.resetTime(new Date(eventEndTime.getTime()));
    if (isDifferentDate) {
      let start: Date = new Date(eventStartTime.getTime());
      let end: Date = util.addDays(
        util.resetTime(new Date(eventStartTime.getTime())),
        1
      );
      const endDate: Date =
        eventEndTime.getHours() === 0 && eventEndTime.getMinutes() === 0
          ? eventEndTime
          : util.addDays(eventEndTime, 1);
      let index: number = 1;
      const eventLength: number = util.getDaysCount(
      ...
      while (end <= eventEndTime && start.getTime() !== end.getTime()) {
        ...
        start = end;
        if (
          util.resetTime(new Date(start.getTime())).getTime() ===
            util.resetTime(new Date(eventEndTime.getTime())).getTime() &&
  ...
  public splitEvent(
    ...
    if (util.getDateInMs(event[fields.endTime] as Date) <= 0) {
      const temp: number = util
        .addDays(util.resetTime(event[fields.endTime]), -1)
        .getTime();
    ...
    if (start !== end) {
      ...
      for (let level: number = 0; level < this.slots.length; level++) {
        let slot: number[] = <[number]>(
          (<unknown>this.slots[parseInt(level.toString(), 10)])
        );
        if (
          this.parent.currentView === 'WorkWeek' ||
          this.parent.currentView === 'TimelineWorkWeek' ||
          this.parent.activeViewOptions.group.byDate ||
          this.parent.activeViewOptions.showWeekend
        ) {
        ...
        if (
          ...
          if (startIndex !== -1) {
            ...
            const isLeft: boolean =
              slot[parseInt(startIndex.toString(), 10)] !== orgStart;
            const isRight: boolean =
              slot[parseInt(endIndex.toString(), 10)] !== orgEnd;
            ranges.push(
              this.cloneEventObject(
                event,
                slot[parseInt(startIndex.toString(), 10)],
                slot[parseInt(endIndex.toString(), 10)],
                count,
                isLeft,
                isRight
              )
          ...
          if (this.dateInRange(cStart, slot[0], slot[slot.length - 1])) {
            ...
            for (let i: number = 0; i < slot.length; i++) {
              if (
                this.dateInRange(
                  <number>slot[parseInt(i.toString(), 10)],
                  orgStart,
                  orgEnd
                )
              ) {
            ...
            if (availSlot.length > 0) {
              if (!this.parent.activeViewOptions.group.byDate) {
                ...
                const isRight: boolean =
                  availSlot[availSlot.length - 1] !== orgEnd;
                ranges.push(
                  this.cloneEventObject(
                    event,
                    availSlot[0],
                    availSlot[availSlot.length - 1],
                    availSlot.length,
                    isLeft,
                    isRight
                  )
  ...
  public cloneEventObject(
    ...
    const fields: EventFieldsMapping = this.parent.eventFields;
    const e: Record<string, any> = extend({}, event, null, true) as Record<
      string,
      any
    >;
    const data: Record<string, any> = {
  ...
  public getSelectedEventElements(target: Element): Element[] {
    ...
    if (this.parent.selectedElements.length <= 0) {
      ...
      const isAlreadySelected: Element[] = this.parent.selectedElements.filter(
        (element: HTMLElement) =>
          element.getAttribute('data-guid') === target.getAttribute('data-guid')
      );
      if (isAlreadySelected.length <= 0) {
        const elementSelector: string =
          'div[data-guid="' + target.getAttribute('data-guid') + '"]';
        const focusElements: Element[] = [].slice.call(
          this.parent.element.querySelectorAll(elementSelector)
        );
        for (const element of focusElements) {
          this.parent.selectedElements.push(element);
        }
      } else {
        const selectedElements: Element[] = this.parent.selectedElements.filter(
          (element: HTMLElement) =>
            element.getAttribute('data-guid') !==
            target.getAttribute('data-guid')
        );
  ...
  public getSelectedEvents(): EventClickArgs {
    ...
    const selectAppointments: Element[] = [].slice.call(
      this.parent.element.querySelectorAll('.' + cls.APPOINTMENT_BORDER)
    ) as Element[];
    selectAppointments.filter((element: HTMLElement) => {
      const isAlreadyAdded: Record<string, any>[] = eventSelect.filter(
        (event: Record<string, any>) => {
      ...
      if (isAlreadyAdded.length === 0) {
        eventSelect.push(
          this.getEventByGuid(element.getAttribute('data-guid'))
        );
      }
  ...
  public addSelectedAppointments(
    cells: Element[],
  ...
  public getSelectedAppointments(): Element[] {
    return [].slice.call(
      this.parent.element.querySelectorAll(
        '.' + cls.APPOINTMENT_BORDER + ',.' + cls.APPOINTMENT_CLASS + ':focus'
      )
    );
  }
  ...
  public focusElement(isFocused?: boolean): void {
    if (
      this.parent.eventWindow.dialogObject &&
      this.parent.eventWindow.dialogObject.visible
    ) {
      return;
    }
    ...
    if (
      ...
      ((activeEle &&
        (this.parent.element.contains(activeEle) ||
          selectedCell.indexOf(activeEle) !== -1)) ||
        isFocused)
    ) {
      if (this.parent.keyboardInteractionModule) {
        const target: HTMLTableCellElement = ((!isNullOrUndefined(
          this.parent.activeCellsData
        ) &&
          this.parent.activeCellsData.element) ||
          selectedCell[selectedCell.length - 1]) as HTMLTableCellElement;
        this.parent.keyboardInteractionModule.selectCells(
    ...
    if (selectedAppointments.length > 0) {
      ...
      (
        selectedAppointments[selectedAppointments.length - 1] as HTMLElement
      ).focus();
      return;
    }
  }
  ...
  public selectWorkCellByTime(eventsData: Record<string, any>[]): Element {
    ...
    if (eventsData.length > 0) {
      ...
      const eventStartTime: Date = <Date>(
        selectedObject[this.parent.eventFields.startTime]
      );
      ...
      if (
        isAllDay &&
        ['Day', 'Week', 'WorkWeek'].indexOf(this.parent.currentView) !== -1
      ) {
      ...
      let queryString: string =
        '[data-date="' + new Date(nearestTime).getTime() + '"]';
      if (this.parent.activeViewOptions.group.resources.length > 0) {
        queryString +=
          '[data-group-index="' +
          this.getGroupIndexFromEvent(selectedObject) +
          '"]';
      }
  ...
  public getGroupIndexFromEvent(eventData: Record<string, any>): number {
    ...
    let idField: string;
    for (
      let i: number = this.parent.resourceBase.resourceCollection.length - 1;
      i >= 0;
      i--
    ) {
      const resourceData: Record<string, any> | string | number = eventData[
        this.parent.resourceBase.resourceCollection[parseInt(i.toString(), 10)]
          .field
      ] as Record<string, any>;
    ...
    if (isNullOrUndefined(levelName) && isNullOrUndefined(levelIndex)) {
      ...
      resource = this.parent.resourceCollection.filter(
        (e: ResourcesModel, index: number) => {
          if (e.name === levelName) {
            levelIndex = index;
            return e;
          }
    ...
    const id: number = (
      eventData[`${idField}`] instanceof Array
        ? (eventData[`${idField}`] as Record<string, any>)[0]
        : eventData[`${idField}`]
    ) as number;
    if (levelIndex > 0) {
      const parentField: string =
        this.parent.resourceCollection[levelIndex - 1].field;
      return this.parent.resourceBase.getIndexFromResourceId(
        id,
        levelName,
        resource,
        eventData,
  ...
  public isAllDayAppointment(event: Record<string, any>): boolean {
    ...
    const isFullDay: boolean =
      (util.getUniversalTime(<Date>event[fieldMapping.endTime]) -
        util.getUniversalTime(<Date>event[fieldMapping.startTime])) /
        util.MS_PER_DAY >=
      1;
    return isAllDay ||
      (this.parent.eventSettings.spannedEventPlacement !== 'TimeSlot' &&
        isFullDay)
      ? true
      : false;
  }
  ...
  public wireAppointmentEvents(
    element: HTMLElement,
    event?: Record<string, any>,
    isPreventCrud: boolean = false
  ): void {
    const isReadOnly: boolean = !isNullOrUndefined(event)
      ? (event[this.parent.eventFields.isReadonly] as boolean)
      : false;
  ...
  private eventTouchClick(e: Event): void {
    if (
      this.parent.uiStateValues.isTouchScroll ||
      this.parent.uiStateValues.isTapHold ||
      this.parent.uiStateValues.action
    ) {
      this.parent.uiStateValues.isTouchScroll =
        this.parent.uiStateValues.isTapHold = false;
      return;
  ...
  public renderResizeHandler(
    element: HTMLElement,
    spanEvent: Record<string, any>,
    isReadOnly: boolean
  ): void {
    if (
      !this.parent.resizeModule ||
    ...
    for (const resizeEdge of Object.keys(spanEvent)) {
      const resizeHandler: HTMLElement = createElement('div', {
        className: cls.EVENT_RESIZE_CLASS,
      });
      switch (resizeEdge) {
        case 'isLeft':
          if (!spanEvent.isLeft) {
            resizeHandler.appendChild(
              createElement('div', { className: 'e-left-right-resize' })
            );
            addClass(
              [resizeHandler],
              this.parent.enableRtl
                ? cls.RIGHT_RESIZE_HANDLER
                : cls.LEFT_RESIZE_HANDLER
            );
        ...
        case 'isTop':
          if (!spanEvent.isTop) {
            resizeHandler.appendChild(
              createElement('div', { className: 'e-top-bottom-resize' })
            );
  ...
  private eventClick(eventData: Event & MouseEvent): void {
    ...
    if (
      target.classList.contains(cls.DRAG_CLONE_CLASS) ||
      target.classList.contains(cls.RESIZE_CLONE_CLASS) ||
    ...
    if (
      ...
      eventData.which === 1 &&
      this.parent.keyboardInteractionModule
    ) {
      this.parent.quickPopup.quickPopup.hide();
      this.parent.selectedElements = [].slice.call(
        this.parent.element.querySelectorAll('.' + cls.APPOINTMENT_BORDER)
      ) as Element[];
      ...
      const args: EventClickArgs = <EventClickArgs>(
        extend(this.parent.activeEventData, {
          cancel: false,
  ...
  private eventDoubleClick(eventData: Event): void {
    ...
    this.parent.selectedElements = [];
    if (
      (this.parent.activeEventData.element as HTMLElement).classList.contains(
        cls.INLINE_APPOINTMENT_CLASS
      ) ||
      (this.parent.activeEventData.element as HTMLElement).querySelector(
    ...
    this.parent.trigger(
      ...
      args,
      (eventDoubleClickArgs: EventClickArgs) => {
        if (!eventDoubleClickArgs.cancel) {
          if (
            !isNullOrUndefined(this.parent.activeEventData.event) &&
            isNullOrUndefined(
            ...
            this.parent.eventWindow.openEditor(
              this.parent.activeEventData.event as Record<string, any>,
              'Save'
            );
          } else {
  ...
  public getEventByGuid(guid: string): Record<string, any> {
    return this.parent.eventsProcessed.filter(
      (data: Record<string, any>) => data.Guid === guid
    )[0];
  }

  public getEventById(id: number | string): Record<string, any> {
    return this.parent.eventsData.filter(
      (data: Record<string, any>) => data[this.parent.eventFields.id] === id
    )[0] as Record<string, any>;
  }

  public generateGuid(): string {
    return 'xyxxxxyx-xxxy-yxxx-xyxx-xxyxxxxyyxxx'.replace(
      /[xy]/g,
      (c: string) => {
  ...
  public getEventIDType(): string {
    if (this.parent.eventsData.length !== 0) {
      return typeof (<Record<string, any>>this.parent.eventsData[0])[
        this.parent.eventFields.id
      ];
    }
    if (this.parent.blockData.length !== 0) {
      return typeof (<Record<string, any>>this.parent.blockData[0])[
        this.parent.eventFields.id
      ];
    }
    return 'string';
  ...
  public getEventMaxID(resourceId?: number): number | string {
    ...
    if (idType === 'number') {
      const datas: Record<string, number>[] = this.parent.eventsData.concat(
        this.parent.blockData
      ) as Record<string, number>[];
      const appIds: number[] = datas.map(
        (event: Record<string, number>) => event[this.parent.eventFields.id]
      );
      let maxId: number = appIds.reduce((a: number, b: number) =>
        Math.max(a, b)
      );
  ...
  private activeEventData(eventData: Event, isMultiple: boolean): void {
    ...
    if (isMultiple) {
      this.addSelectedAppointments(
        [].slice.call(
          this.parent.element.querySelectorAll('div[data-guid="' + guid + '"]')
        ),
        true
      );
    ...
    if (eventObject && eventObject.isSpanned) {
      eventObject = this.parent.eventsData.filter(
        (obj: Record<string, any>) =>
          obj[this.parent.eventFields.id] ===
          eventObject[this.parent.eventFields.id]
      )[0];
    }
    this.parent.activeEventData = {
      event: eventObject,
  ...
  public generateOccurrence(
    ...
    const eventRule: string = event[
      this.parent.eventFields.recurrenceRule
    ] as string;
    const timeZoneDiff: number =
      endDate.getTimezoneOffset() - startDate.getTimezoneOffset();
    const duration: number =
      endDate.getTime() - startDate.getTime() - timeZoneDiff * 60000;
    viewDate = new Date(
      (viewDate || this.parent.activeView.startDate()).getTime() - duration
    );
    const exception: string = event[
      this.parent.eventFields.recurrenceException
    ] as string;
    let maxCount: number;
    if (this.parent.currentView !== 'Agenda' && isMaxCount) {
      maxCount =
        util.getDateCount(viewDate, this.parent.activeView.endDate()) + 1;
    }
    const newTimezone: string =
      this.parent.timezone || this.parent.tzModule.getLocalTimezoneName();
    ...
    if (
      event[this.parent.eventFields.recurrenceRule] &&
      event[this.parent.eventFields.recurrenceRule].includes('BYMONTHDAY') &&
      this.parent.timezone &&
      event[this.parent.eventFields.startTimezone] &&
      event[this.parent.eventFields.endTimezone]
    ) {
      startDate = this.parent.tzModule.convert(
        event[this.parent.eventFields.startTime],
        this.parent.timezone,
        event[this.parent.eventFields.startTimezone]
      );
    }
    ...
    if (
      this.parent.currentView === 'Agenda' &&
      eventRule.indexOf('COUNT') === -1 &&
      eventRule.indexOf('UNTIL') === -1
    ) {
      if (isNullOrUndefined(event.generatedDates)) {
        ...
        if (
          dates[0] <
          (<Date>(<Record<string, any>>event.generatedDates).start).getTime()
        ) {
          (<Record<string, any>>event.generatedDates).start = new Date(
            dates[0]
          );
        }
        if (
          dates[dates.length - 1] >
          (<Date>(<Record<string, any>>event.generatedDates).end).getTime()
        ) {
          (<Record<string, any>>event.generatedDates).end = new Date(
            dates[dates.length - 1]
          );
        }
      }
    ...
    for (let date of convertedDates) {
      const clonedObject: Record<string, any> = extend(
        {},
        event,
        null,
        true
      ...
      date = !isDSTAdjusted
        ? this.getDSTAdjustedTime(date, clonedObject)
        : date;
      clonedObject[this.parent.eventFields.startTime] = new Date(date);
      clonedObject[this.parent.eventFields.endTime] = new Date(
        new Date(date).setMilliseconds(duration)
      );
      clonedObject[this.parent.eventFields.recurrenceID] =
        clonedObject[this.parent.eventFields.id];
  ...
  private getDSTAdjustedTime(date: number, event: Record<string, any>): number {
    ...
    if (
      ...
      const eventOffset: number = this.getDSTDiff(
        event[this.parent.eventFields.startTime],
        new Date(date),
        event[this.parent.eventFields.startTimezone]
      );
      const schOffset: number = this.getDSTDiff(
        event[this.parent.eventFields.startTime],
        new Date(date),
        this.parent.timezone
      );
  ...
  private getDSTDiff(
    ...
    const startOffset: number = this.parent.tzModule.offset(
      new Date(startDate),
      timezone
    );
    const occurOffset: number = this.parent.tzModule.offset(
      new Date(occurDate),
      timezone
    );
  ...
  public getParentEvent(
    ...
    if (isParent && parentEvent) {
      const collection: Record<string, unknown[]> =
        this.getEventCollections(parentEvent);
      let followObj: Record<string, any> = collection.follow.slice(
        -1
      )[0] as Record<string, any>;
      if (
        collection.occurrence.length > 0 &&
        !parentEvent[this.parent.eventFields.recurrenceException]
      ) {
        followObj = collection.occurrence.slice(-1)[0] as Record<string, any>;
      }
      if (followObj) {
        parentEvent[this.parent.eventFields.recurrenceRule] =
          followObj[this.parent.eventFields.recurrenceRule];
      }
    }
    return parentEvent;
  }

  public getEventCollections(
    ...
    do {
      ...
      occurrenceCollection = occurrenceCollection.concat(
        this.getOccurrenceEvent(followingEvent || parentObj)
      );
    ...
    if (childObj) {
      const fields: EventFieldsMapping = this.parent.eventFields;
      collections = {
        follow: followingCollection.filter(
          (eventData: Record<string, any>) =>
            eventData[fields.startTime] >= childObj[fields.startTime]
        ),
        occurrence: occurrenceCollection.filter(
          (eventData: Record<string, any>) =>
            eventData[fields.startTime] >= childObj[fields.startTime]
        ),
      };
  ...
  public getFollowingEvent(
    ...
    if (isReverse) {
      ...
      fieldValue = (parentObj[fields.recurrenceID] ||
        parentObj[fields.followingID]) as string | number;
    }
    const parentApp: Record<string, any>[] = this.parent.eventsData.filter(
      (data: Record<string, any>) =>
  ...
  public isFollowingEvent(
    parentObj: Record<string, any>,
    childObj: Record<string, any>
  ): boolean {
    const parentStart: Date = parentObj[
      this.parent.eventFields.startTime
    ] as Date;
    const childStart: Date = childObj[
      this.parent.eventFields.startTime
    ] as Date;
    return (
      parentStart.getHours() === childStart.getHours() &&
      parentStart.getMinutes() === childStart.getMinutes() &&
      parentStart.getSeconds() === childStart.getSeconds()
    );
  }

  public getOccurrenceEvent(
    eventObj: Record<string, any>,
    isGuid: boolean = false,
    isFollowing: boolean = false
  ): Record<string, any>[] {
    const idField: string = isGuid
      ? 'Guid'
      : isFollowing
      ? this.parent.eventFields.followingID
      : this.parent.eventFields.recurrenceID;
    const fieldKey: string = isGuid ? 'Guid' : this.parent.eventFields.id;
    const dataSource: Record<string, any>[] = isGuid
      ? this.parent.eventsProcessed
      : this.parent.eventsData;
    return dataSource.filter(
      (data: Record<string, any>) =>
        data[`${idField}`] === eventObj[`${fieldKey}`]
    );
  }
  ...
  public getOccurrencesByID(id: number | string): Record<string, any>[] {
    const fields: EventFieldsMapping = this.parent.eventFields;
    let occurrenceCollection: Record<string, any>[] = [];
    const parentObject: Record<string, any>[] = this.parent.eventsData.filter(
      (obj: Record<string, any>) => obj[fields.id] === id
    );
  ...
  public getOccurrencesByRange(
    ...
    const filter: Record<string, any>[] = occurrenceCollection.filter(
      (obj: Record<string, any>) =>
        obj[fields.startTime] >= startTime &&
        obj[fields.endTime] <= endTime &&
        !isNullOrUndefined(obj[fields.recurrenceID])
    );
    return filter;
  }

  public getDeletedOccurrences(
    recurrenceData: string | number | Record<string, any>
  ): Record<string, any>[] {
    const fields: EventFieldsMapping = this.parent.eventFields;
    ...
    if (
      ...
      typeof recurrenceData === 'number'
    ) {
      parentObject = this.parent.eventsData.filter(
        (obj: Record<string, any>) => obj[fields.id] === recurrenceData
      )[0] as Record<string, any>;
    } else {
      parentObject = extend({}, recurrenceData, null, true) as Record<
        string,
        any
      >;
    }
    if (parentObject[fields.recurrenceException]) {
      const exDateString: string[] = (<string>(
        parentObject[fields.recurrenceException]
      )).split(',');
      for (let i: number = 0, len: number = exDateString.length; i < len; i++) {
        const edited: Record<string, any>[] = this.parent.eventsData.filter(
          (eventObj: Record<string, any>) =>
            eventObj[fields.recurrenceID] === parentObject[fields.id] &&
            eventObj[fields.recurrenceException] ===
              exDateString[parseInt(i.toString(), 10)]
        );
        if (edited.length === 0) {
          const exDate: Date = getDateFromRecurrenceDateString(
            exDateString[parseInt(i.toString(), 10)]
          );
          const childObject: Record<string, any> = extend(
            {},
            recurrenceData,
            null,
            true
          ...
          childObject[fields.recurrenceException] =
            exDateString[parseInt(i.toString(), 10)];
          const startDate: Date = new Date(exDate.getTime());
          const time: number =
            (<Date>parentObject[fields.endTime]).getTime() -
            (<Date>parentObject[fields.startTime]).getTime();
  ...
  public applyResourceColor(
    ...
    const alphaColor: CallbackFunction = (
      color: string,
      alpha: string
    ): string => {
      color = color.replace('#', '');
      ...
      const g: number = parseInt(
        color.substring(color.length / 3, (2 * color.length) / 3),
        16
      );
      const b: number = parseInt(
        color.substring((2 * color.length) / 3, (3 * color.length) / 3),
        16
      );
      return `rgba(${r}, ${g}, ${b}, ${alpha})`;
    };
    // index refers groupOrder
    const color: string = this.parent.resourceBase.getResourceColor(
      data,
      index
    );
    if (color) {
      element.style[<any>type] = !isNullOrUndefined(alpha)
        ? alphaColor(color, alpha)
        : color;
    }
  }

  public createBlockAppointmentElement(
    record: Record<string, any>,
    resIndex: number,
    isResourceEventTemplate: boolean
  ): HTMLElement {
    const eventSubject: string = (record[this.parent.eventFields.subject] ||
    ...
    if (!isNullOrUndefined(this.parent.activeViewOptions.eventTemplate)) {
      ...
      const templateName: string =
        isResourceEventTemplate &&
        this.parent.currentView.indexOf('Year') === -1
          ? this.parent.getEventTemplateName(resIndex)
          : 'eventTemplate';
  ...
  public setWrapperAttributes(
    appointmentWrapper: HTMLElement,
    resIndex: number
  ): void {
  ...
  public getReadonlyAttribute(event: Record<string, any>): string {
    return (
      event[this.parent.eventFields.isReadonly] || this.parent.readonly
    ).toString();
  }
  ...
  public isBlockRange(
    ...
    const eventCollection: Record<string, any>[] =
      eventData instanceof Array ? eventData : [eventData];
    ...
    for (const event of eventCollection as Record<string, any>[]) {
      ...
      for (const data of dataCol) {
        const filterBlockEvents: Record<string, any>[] =
          this.filterBlockEvents(data);
  ...
  public getFilterEventsList(
    dataSource: Record<string, any>[],
    query: Predicate
  ): Record<string, any>[] {
    return new DataManager(dataSource).executeLocal(
      new Query().where(query)
    ) as Record<string, any>[];
  }

  public getSeriesEvents(
    parentEvent: Record<string, any>,
    startTime?: string
  ): Record<string, any>[] {
    const fields: EventFieldsMapping = this.parent.eventFields;
    startTime = isNullOrUndefined(startTime)
      ? (parentEvent[fields.startTime] as string)
      : startTime;
    ...
    const startTimeQuery: string =
      this.parent.currentAction === 'EditSeries'
        ? 'greaterthan'
        : 'greaterthanorequal';
    do {
      deleteFutureEvent = new Predicate(fields.followingID, 'equal', delId).and(
        new Predicate(fields.startTime, startTimeQuery, startTime)
      );
      futureEvents = this.getFilterEventsList(
        this.parent.eventsData,
        deleteFutureEvent
      );
      deleteFutureEditEvents = futureEvents.slice(-1)[0];
      if (
        !isNullOrUndefined(deleteFutureEditEvents) &&
        deleteFutureEditEvents[fields.id] !== followingId
      ) {
      ...
      !isNullOrUndefined(deleteFutureEditEvents[fields.followingID])
    );
    return deleteFutureEditEventList;
  }

  public getEditedOccurrences(
    deleteFutureEditEventList: Record<string, any>[],
    startTime?: string
  ): Record<string, any>[] {
    const fields: EventFieldsMapping = this.parent.eventFields;
    ...
    for (const event of deleteFutureEditEventList) {
      let delEventQuery: Predicate = new Predicate(
        fields.recurrenceID,
        'equal',
        event[fields.id] as string
      ).or(
        new Predicate(
          fields.recurrenceID,
          'equal',
          event[fields.followingID] as string
        )
      ...
      if (
        ...
        delEventQuery = delEventQuery.and(
          new Predicate(fields.startTime, 'greaterthanorequal', startTime)
        );
      }
      delEditedEvents = this.getFilterEventsList(
        this.parent.eventsData,
        delEventQuery
      );
      deleteRecurrenceEventList =
        deleteRecurrenceEventList.concat(delEditedEvents);
    }
  ...
  public getRenderedDates(dateRender: Date[]): Date[] {
    ...
    const maxDate: Date = isNullOrUndefined(this.parent.maxDate)
      ? new Date(2099, 11, 31)
      : this.parent.maxDate;
    const minDate: Date = isNullOrUndefined(this.parent.minDate)
      ? new Date(1900, 0, 1)
      : this.parent.minDate;
    if (
      dateRender[0] < minDate &&
      dateRender[dateRender.length - 1] > maxDate
    ) {
      for (let i: number = 0; i < dateRender.length; i++) {
        if (
          util.resetTime(dateRender[parseInt(i.toString(), 10)]).getTime() ===
          util.resetTime(new Date(minDate)).getTime()
        ) {
          firstDate = i;
        }
        if (
          util.resetTime(dateRender[parseInt(i.toString(), 10)]).getTime() ===
          util.resetTime(new Date(maxDate)).getTime()
        ) {
          lastDate = i;
        }
      }
      filteredDates = dateRender.filter(
        (date: Date) =>
          date >= dateRender[parseInt(firstDate.toString(), 10)] &&
          date <= dateRender[parseInt(lastDate.toString(), 10)]
      );
    }
  ...
  public isValidEvent(
    ...
    const isHourRange: boolean =
      end.getTime() > schedule.startHour.getTime() &&
      start.getTime() < schedule.endHour.getTime();
    const isSameRange: boolean =
      schedule.startHour.getTime() <= start.getTime() &&
      (<Date>eventObj[this.parent.eventFields.startTime]).getTime() >=
        schedule.startHour.getTime() &&
      (<Date>eventObj[this.parent.eventFields.endTime]).getTime() <
        schedule.endHour.getTime() &&
  ...
  public allDayExpandScroll(dateHeader: HTMLElement): void {
    ...
    const headerRows: HTMLElement[] = [].slice.call(
      this.parent.element.querySelectorAll('.' + cls.HEADER_ROW_CLASS)
    );
    headerRows.forEach((element: HTMLElement) => {
      const tdEle: HTMLElement[] = [].slice.call(element.children);
      tdEle.forEach((ele: HTMLElement) => {
    ...
    ).style.height = indentHeight / 12 + 'em';
  ...
  public updateEventMinimumDuration(
    startEndHours: Record<string, Date>,
    startTime: Date,
    endTime: Date
  ): Record<string, Date> {
    if (
      ...
      (startTime.getTime() === endTime.getTime() &&
        startEndHours.startHour.getTime() < endTime.getTime() &&
        startEndHours.endHour.getTime() > startTime.getTime() &&
        this.parent.currentView.indexOf('Timeline') === -1)
    ) {
      const eventDuration: number =
        (util.getUniversalTime(endTime) - util.getUniversalTime(startTime)) /
        util.MS_PER_MINUTE;
      if (eventDuration < this.parent.eventSettings.minimumEventDuration) {
        const tempEnd: Date = new Date(startTime);
        tempEnd.setMinutes(
          tempEnd.getMinutes() + this.parent.eventSettings.minimumEventDuration
        );
        endTime = tempEnd;
  ...
  public createEventWrapper(type: string = '', index: number = 0): HTMLElement {
    ...
    const className: string =
      (this.parent as any).isReact &&
      this.parent.activeViewOptions.eventTemplate
    ...
    for (let i: number = 0, len: number = levels.length; i < len; i++) {
      ...
      const appointmentWrap: HTMLElement = createElement('td', {
        className:
          type === 'allDay'
            ? cls.ALLDAY_APPOINTMENT_WRAPPER_CLASS + className
            : type === 'timeIndicator'
            ? cls.TIMELINE_WRAPPER_CLASS
            : cls.DAY_WRAPPER_CLASS + className,
        attrs: { 'data-date': col.date.getTime().toString() },
      });
      if (!isNullOrUndefined(col.groupIndex)) {
        appointmentWrap.setAttribute(
          'data-group-index',
          col.groupIndex.toString()
        );
      }
      if (type === '') {
  ...
  public getPageCoordinates(
    e: MouseEvent & TouchEvent
  ): (MouseEvent & TouchEvent) | Touch {
    if (isNullOrUndefined(e)) {
      return e;
    }
    const eventArgs: TouchEvent = (
      e as Record<string, any> & MouseEvent & TouchEvent
    ).event as TouchEvent;
    return eventArgs && eventArgs.changedTouches
      ? eventArgs.changedTouches[0]
      : e.changedTouches
      ? e.changedTouches[0]
      : <MouseEvent & TouchEvent>eventArgs || e;
  }

  public renderSpannedIcon(
    element: HTMLElement,
    spanEvent: Record<string, any>
  ): void {
    const iconElement: HTMLElement = createElement('div', {
  ...
  public addCellHeight(
    ...
    if (
      this.parent.activeViewOptions.maxEventsPerRow &&
      !this.parent.rowAutoHeight
    ) {
      const rows: HTMLElement[] = [].slice.call(
        this.parent.element.querySelectorAll(selector)
      );
      const weekNumberRows: HTMLElement[] = this.parent.showWeekNumber
        ? [].slice.call(
            this.parent.element.querySelectorAll(
              '.' + cls.WEEK_NUMBER_WRAPPER_CLASS + ' tbody tr'
            )
          )
        : [];
      for (const row of rows) {
        const height: number =
          this.parent.activeViewOptions.maxEventsPerRow *
            (eventHeight +
              (this.parent.currentView === 'Month' ? eventGap : 2)) +
          headerHeight +
          indHeight;
        if (weekNumberRows.length > 0) {
          setStyleAttribute(
            weekNumberRows[rows.indexOf(row)].firstElementChild as HTMLElement,
            { height: height + 'px' }
          );
        }
        setStyleAttribute(row.firstElementChild as HTMLElement, {
          height: height + 'px',
        });
      }
  ...
  private unWireEvents(): void {
    const appElements: Element[] = [].slice.call(
      this.parent.element.querySelectorAll('.' + cls.APPOINTMENT_CLASS)
    );
