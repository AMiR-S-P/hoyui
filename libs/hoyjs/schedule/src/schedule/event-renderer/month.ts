...
import { isNullOrUndefined, setStyleAttribute, remove } from '@hoyui/base';
import {
  EventFieldsMapping,
  EventClickArgs,
  EventRenderedArgs,
...
export class MonthEvent extends EventBase {
  ...
  constructor(parent: Schedule) {
    ...
    this.maxHeight =
      this.parent.eventSettings.enableMaxHeight &&
    ...
    this.moreIndicatorHeight =
      this.parent.rowAutoHeight && this.parent.eventSettings.ignoreWhitespace
        ? 0
        : this.moreIndicatorHeight;
  ...
  public renderAppointments(): void {
    ...
    const conWrap: HTMLElement = this.parent.element.querySelector(
    ...
    const appointmentWrapper: HTMLElement[] = [].slice.call(
      this.element.querySelectorAll('.' + cls.APPOINTMENT_WRAPPER_CLASS)
    );
    if (this.parent.crudModule && this.parent.crudModule.crudObj.isCrudAction) {
      for (
        ...
        const appElement: Element[] = [].slice.call(
        ...
        if (
          ...
          this.parent.crudModule.crudObj.sourceEvent[parseInt(i.toString(), 10)]
            .groupIndex !==
            this.parent.crudModule.crudObj.targetEvent[
              parseInt(i.toString(), 10)
            ].groupIndex
        ) {
          const ele: Element[] = [].slice.call(
            this.element.querySelectorAll(
              '.e-appointment-wrapper ' +
                '[data-group-index="' +
                this.parent.crudModule.crudObj.targetEvent[
    ...
    if (this.parent.currentView === 'Month') {
      ...
      const cellTd: HTMLTableCellElement = this.parent.element.querySelector(
    ...
    this.eventHeight = this.parent.getElementHeightFromClass(
      this.element,
    ...
    this.addCellHeight(
      selector,
      this.eventHeight,
      this.parent.currentView === 'Month' ? EVENT_GAP : 2,
    ...
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      ...
      this.renderEventsHandler(
        this.parent.activeView.renderDates,
    ...
    if (this.parent.rowAutoHeight) {
      ...
      if (this.parent.currentView === 'Month' && this.parent.showWeekNumber) {
        const totalCells: HTMLElement[] = [].slice.call(
          this.parent.element.querySelectorAll(
            '.e-content-wrap table tr td:first-child'
          )
        );
        const weekNumberCells: HTMLElement[] = [].slice.call(
          this.parent.element.querySelectorAll('.' + cls.WEEK_NUMBER_CLASS)
        );
        weekNumberCells.forEach((cell: HTMLElement, i: number) => {
          const height: number =
            totalCells[parseInt(i.toString(), 10)].offsetHeight;
          setStyleAttribute(cell, { height: height + 'px' });
        });
        this.parent.element.querySelector(
          '.' + cls.WEEK_NUMBER_WRAPPER_CLASS
        ).scrollTop = this.parent.element.querySelector(
          '.' + cls.CONTENT_WRAP_CLASS
        ).scrollTop;
      }
    }
  ...
  public renderEventsHandler(
    ...
    if (resData) {
      ...
      eventsList = this.parent.eventBase.filterEventsByResource(
        resData,
        this.parent.eventsProcessed
      ) as Record<string, any>[];
      blockList = this.parent.eventBase.filterEventsByResource(
        resData,
        this.parent.blockProcessed
      ) as Record<string, any>[];
      this.workCells = [].slice.call(
        this.element.querySelectorAll(
    ...
    if (
      ...
      const height: number =
        this.parent.element.querySelector('.' + cls.CONTENT_TABLE_CLASS)
          .clientHeight / totalCells.length;
      totalCells.forEach((cell: HTMLElement) => {
        setStyleAttribute(cell, { height: height + 'px' });
      });
    }
    const cellDetail: HTMLElement =
      this.workCells[
        this.parent.activeView.isTimelineView() ? 0 : this.workCells.length - 1
      ];
    ...
    for (const event of eventsList) {
      ...
      events = events.concat(
        this.splitEvent(event, filteredDates || this.dateRender)
      );
    }
    for (let level: number = 0; level < this.slots.length; level++) {
      ...
      const spannedEvents: Record<string, any>[] = this.filterEvents(
        new Date(slot[0]),
        endDate,
        events
      );
      for (const event of spannedEvents) {
        if (this.maxHeight) {
          const sDate: Date =
            this.parent.currentView === 'Month'
              ? (event[this.fields.startTime] as Date)
  ...
  private processBlockEvents(
    ...
    for (const event of blockEvents) {
      ...
      if (this.renderType === 'day' && !event[this.fields.isAllDay]) {
        const temp: Record<string, any> = extend(
          {},
          event,
          null,
          true
        ) as Record<string, any>;
        let isSameDate: boolean = this.isSameDate(
          temp[this.fields.startTime] as Date,
          temp[this.fields.endTime] as Date
        );
        temp.isBlockIcon = isSameDate;
        if (
          !isSameDate &&
          util.getDateInMs(temp[this.fields.startTime] as Date) > 0
        ) {
          const e: Record<string, any> = extend(
            {},
            event,
            null,
            true
          ) as Record<string, any>;
          e[this.fields.endTime] = util.addDays(
            util.resetTime(new Date(event[this.fields.startTime] + '')),
            1
          );
          e.isBlockIcon = true;
        ...
        if (
          !isSameDate &&
          util.getDateInMs(temp[this.fields.endTime] as Date) > 0
        ) {
          const e: Record<string, any> = extend(
            {},
            event,
          ...
          e[this.fields.startTime] = util.resetTime(
            new Date(event[this.fields.endTime] + '')
          );
          e.isBlockIcon = true;
          blockSpannedList.push(e);
      ...
      for (const blockEvent of blockSpannedList) {
        const spannedEvents: Record<string, any>[] = this.splitEvent(
          blockEvent,
          this.dateRender
        );
        for (const event of spannedEvents) {
  ...
  public renderBlockEvents(
    ...
    const day: number = this.parent.getIndexOfDate(
      this.dateRender,
    ...
    const position: number = this.getPosition(
      startTime,
      endTime,
      event[this.fields.isAllDay] as boolean,
      day
    );
    if (!isIcon) {
      const diffInDays: number = eventData.count as number;
      let appWidth: number = this.getEventWidth(
        startTime,
        endTime,
        event[this.fields.isAllDay] as boolean,
        diffInDays
      ...
      const blockElement: HTMLElement = this.createBlockAppointmentElement(
        event,
        resIndex,
  ...
  public renderBlockIndicator(
    cellTd: HTMLElement,
    position: number,
    resIndex: number
  ): void {
    const blockIndicator: HTMLElement = createElement('div', {
      className: 'e-icons ' + cls.BLOCK_INDICATOR_CLASS,
    });
    if (
      isNullOrUndefined(cellTd.querySelector('.' + cls.BLOCK_INDICATOR_CLASS))
    ) {
  ...
  public updateIndicatorIcon(event: Record<string, any>): void {
    ...
    const startHour: Record<string, Date> = util.getStartEndHours(
      event[this.fields.startTime] as Date,
      start,
      end
    );
    const endHour: Record<string, Date> = util.getStartEndHours(
      event[this.fields.endTime] as Date,
      start,
      end
    );
    ...
    cloneData.isLeft =
      cloneData.isLeft ||
      actualStartTime.getTime() < startHour.startHour.getTime();
    cloneData.isRight =
      cloneData.isRight || actualEndTime.getTime() > endHour.endHour.getTime();
    if (
      util.resetTime(actualStartTime).getTime() !==
      util.resetTime(actualEndTime).getTime()
    ) {
      ...
      startTime.setHours(
        actualEndHour.getHours(),
        actualEndHour.getMinutes(),
        actualEndHour.getSeconds()
      );
      cloneData.isLeft =
        cloneData.isLeft ||
        actualStartTime.getTime() >=
          (actualStartHour.getDate() === startHour.endHour.getDate()
            ? startTime.getTime()
            : util.addDays(startTime, 1).getTime());
      if (actualEndTime.getTime() !== util.resetTime(actualEndTime).getTime()) {
        ...
        cloneData.isRight =
          ...
          actualEndTime.getTime() <=
            endTime.setHours(
              actualStartHour.getHours(),
              actualStartHour.getMinutes(),
              actualStartHour.getSeconds()
            );
      }
    }
  }
  ...
  public renderResourceEvents(): void {
    ...
    if (this.parent.crudModule && this.parent.crudModule.crudObj.isCrudAction) {
      for (
        ...
        const sourceRes: TdData =
          this.parent.crudModule.crudObj.sourceEvent[
            parseInt(i.toString(), 10)
          ];
        if (
          !this.parent.uiStateValues.isGroupAdaptive ||
          (this.parent.uiStateValues.groupIndex === sourceRes.groupIndex &&
            this.parent.uiStateValues.isGroupAdaptive)
        ) {
          this.renderEventsHandler(
            sourceRes.renderDates,
            sourceRes.workDays,
            sourceRes
          );
        }
        if (
          this.parent.crudModule.crudObj.sourceEvent[parseInt(i.toString(), 10)]
            .groupIndex !==
          this.parent.crudModule.crudObj.targetEvent[parseInt(i.toString(), 10)]
            .groupIndex
        ) {
          const target: TdData =
            this.parent.crudModule.crudObj.targetEvent[
  ...
  public getSlotDates(workDays?: number[]): void {
    this.slots = [];
    const dates: number[] = this.dateRender.map((date: Date) => {
      return +date;
    });
  ...
  public createAppointmentElement(
    ...
    resIndex: number,
    isCloneElement: boolean = false
  ): HTMLElement {
    const eventSubject: string = (record[this.fields.subject] ||
      this.parent.eventSettings.fields.subject.default ||
      this.parent.localeObj.getConstant('addTitle')) as string;
    const newRecord: Record<string, any> = extend(
      {},
      record,
      record.data,
      true
    ...
    if (!isNullOrUndefined(this.parent.activeViewOptions.eventTemplate)) {
      ...
      const eventTemplate: string = this.isResourceEventTemplate
      ...
      const eventLocation: string = (record[this.fields.location] ||
      ...
      this.parent.sanitize(
        eventSubject + (eventLocation ? '; ' + eventLocation : ''),
        appointmentSubject
      );
      const appointmentStartTime: HTMLElement = createElement('div', {
        ...
        innerHTML: this.parent.getTimeString(
          eventData[this.fields.startTime] as Date
        ),
      });
      const appointmentEndTime: HTMLElement = createElement('div', {
        className:
          ...
          (this.parent.isAdaptive ? ' ' + cls.DISABLE_CLASS : ''),
        innerHTML: this.parent.getTimeString(
          eventData[this.fields.endTime] as Date
        ),
      });
      if (this.parent.currentView === 'Month') {
        ...
        if (record[this.fields.isAllDay]) {
          ...
          const appLocation: HTMLElement = createElement('div', {
            className: cls.LOCATION_CLASS,
          });
          this.parent.sanitize(eventLocation, appLocation);
          innerElement = [appointmentSubject, appTime, appLocation];
        }
        const wrap: HTMLElement = createElement('div', {
  ...
  private appendEventIcons(
    record: Record<string, any>,
    appointmentDetails: HTMLElement
  ): void {
    const eventData: Record<string, any> = record.data as Record<string, any>;
    if (
      !isNullOrUndefined(record[this.fields.recurrenceRule]) ||
      !isNullOrUndefined(record[this.fields.recurrenceID])
    ) {
      const iconClass: string =
        record[this.fields.id] === record[this.fields.recurrenceID]
          ? cls.EVENT_RECURRENCE_ICON_CLASS
          : cls.EVENT_RECURRENCE_EDIT_ICON_CLASS;
      appointmentDetails.appendChild(
        createElement('div', {
          className:
            cls.ICON +
            ' ' +
    ...
    if (eventData.isLeft) {
      const iconLeft: HTMLElement = createElement('div', {
        className:
          cls.EVENT_INDICATOR_CLASS +
          ' ' +
          cls.ICON +
          ' ' +
          cls.EVENT_ICON_LEFT_CLASS,
      });
  ...
  public renderEvents(
    ...
    if (
      day < 0 ||
      startTime.getTime() < this.parent.minDate.getTime() ||
      endTime.getTime() > this.parent.maxDate.getTime()
    ) {
      return;
    }
    const eventsPerRow: number = this.parent.rowAutoHeight
      ? 1
      : this.parent.activeViewOptions.maxEventsPerRow;
    const overlapCount: number = this.getIndex(startTime);
    event.Index = overlapCount;
    const diffInDays: number = (event.data as Record<string, any>)
      .count as number;
    if (startTime.getTime() <= endTime.getTime()) {
      ...
      const height: number =
        this.monthHeaderHeight +
        (overlapCount + 1) * (this.eventHeight + EVENT_GAP) +
        this.moreIndicatorHeight;
      const enableAppRender: boolean =
        this.parent.activeViewOptions.maxEventsPerRow &&
        !this.parent.rowAutoHeight &&
        !this.parent.eventSettings.enableIndicator
          ? overlapCount < eventsPerRow
          : this.maxOrIndicator
          ? overlapCount < 1
            ? true
            : false
      ...
      if (this.parent.rowAutoHeight || enableAppRender) {
        ...
        if (this.inlineValue) {
          appointmentElement =
            this.parent.inlineModule.createInlineAppointmentElement();
        } else {
        ...
        if (this.parent.rowAutoHeight) {
          const firstChild: HTMLElement = cellTd.parentElement
        ...
        for (let i: number = 0; i < diffInDays; i++) {
          if (this.workCells[day + i]) {
            const indicator: HTMLElement = this.workCells[
              day + i
            ].querySelector('.' + cls.MORE_INDICATOR_CLASS);
            if (indicator) {
              const count: number =
                parseInt(indicator.getAttribute('data-count'), 10) + 1;
              indicator.setAttribute('data-count', count.toString());
              indicator.setAttribute(
                'aria-label',
                count + ' ' + this.parent.localeObj.getConstant('moreEvents')
              );
              indicator.innerHTML = this.getMoreIndicatorText(count);
            } else {
              const startDate: Date = new Date(
                this.dateRender[day + i].getTime()
              );
              const endDate: Date = util.addDays(this.dateRender[day + i], 1);
              const groupIndex: string =
                this.workCells[day + i].getAttribute('data-group-index');
              const moreIndicatorElement: HTMLElement =
                this.getMoreIndicatorElement(1, startDate, endDate);
              if (!isNullOrUndefined(groupIndex)) {
                moreIndicatorElement.setAttribute(
                  'data-group-index',
                  groupIndex
                );
              }
              moreIndicatorElement.style.top =
                this.cellHeight -
                this.monthHeaderHeight -
                this.moreIndicatorHeight +
                'px';
  ...
  public updateBlockElements(): void {
    const blockElement: HTMLElement[] = [].slice.call(
      this.element.querySelectorAll('.' + cls.BLOCK_APPOINTMENT_CLASS)
    );
    for (const element of blockElement) {
      const target: HTMLElement = closest(element, 'tr') as HTMLElement;
      this.monthHeaderHeight =
        (<HTMLElement>element.offsetParent).offsetTop - target.offsetTop;
      element.style.height =
        target.offsetHeight - 1 - this.monthHeaderHeight + 'px';
      const firstChild: HTMLElement = target.firstElementChild as HTMLElement;
      const width: number = Math.round(
        element.offsetWidth / firstChild.offsetWidth
      );
  ...
  public getFilteredEvents(
    ...
    if (isNullOrUndefined(groupIndex)) {
      ...
      filteredEvents = this.filterEvents(
        startDate,
        endDate,
        isNullOrUndefined(eventsList) ? undefined : eventsList,
        data
      );
    }
  ...
  public getOverlapEvents(
    ...
    for (const app of appointments) {
      if (
        util.resetTime(<Date>app[this.fields.startTime]).getTime() <=
          dateTime &&
  ...
  public getIndex(date: Date): number {
    ...
    const appointments: Record<string, any>[] = <Record<string, any>[]>(
      this.renderedEvents
    );
    if (appointments.length > 0) {
      const appointmentsList: Record<string, any>[] = this.getOverlapEvents(
        date,
        appointments
      );
      const appLevel: number[] = appointmentsList.map(
  ...
  public moreIndicatorClick(event: Event): void {
    ...
    const endDate: Date = new Date(
      parseInt(target.getAttribute('data-end-date'), 10)
    );
    ...
    this.parent.trigger(
      events.moreEventsClick,
      moreArgs,
      (clickArgs: MoreEventsClickArgs) => {
        if (!clickArgs.cancel) {
          if (clickArgs.isPopupOpen) {
            const filteredEvents: Record<string, any>[] =
              this.getFilteredEvents(startDate, endDate, groupIndex);
            const moreArgs: EventClickArgs = {
              date: startDate,
              event: filteredEvents,
              element: event.target,
            } as EventClickArgs;
            this.parent.quickPopup.moreEventClick(
              moreArgs,
              endDate,
              groupIndex
            );
  ...
  public renderEventElement(
    ...
    const eventType: string = appointmentElement.classList.contains(
      ...
      ? 'blockEvent'
      : 'event';
    const isAppointment: boolean = appointmentElement.classList.contains(
      cls.APPOINTMENT_CLASS
    );
  ...
  public getEventData(event: Record<string, any>): Record<string, any> {
    ...
    eventObj[this.fields.startTime] = (event.data as Record<string, any>)[
      this.fields.startTime
    ];
    eventObj[this.fields.endTime] = (event.data as Record<string, any>)[
      this.fields.endTime
    ];
    return eventObj;
  }
  ...
  public renderElement(
    ...
    const wrapperEle: HTMLElement = cellTd.querySelector(
  ...
  public getMoreIndicatorElement(
    count: number,
    startDate: Date,
    endDate: Date
  ): HTMLElement {
    const moreIndicatorElement: HTMLElement = createElement('div', {
      ...
      attrs: {
        ...
        tabindex: '0',
        'aria-label':
          this.parent.globalize.formatNumber(count) +
          ' ' +
          this.parent.localeObj.getConstant('moreEvents'),
  ...
  public removeHeightProperty(selector: string): void {
    const rows: HTMLElement[] = [].slice.call(
      this.element.querySelectorAll('.' + selector + ' tbody tr')
    );
  ...
  public setMaxEventHeight(event: HTMLElement, cell: HTMLElement): void {
    const headerHeight: number = util.getOuterHeight(
      cell.querySelector('.' + cls.DATE_HEADER_CLASS)
    );
    const height: number =
      cell.offsetHeight -
      headerHeight -
      (this.maxHeight ? 0 : this.moreIndicatorHeight);
