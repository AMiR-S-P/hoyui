...
export class VerticalEvent extends EventBase {
  ...
  public renderAppointments(): void {
    ...
    const wrapperElements: HTMLElement[] = [].slice.call(
      this.parent.element.querySelectorAll(
        '.' +
          cls.BLOCK_APPOINTMENT_CLASS +
          ',.' +
          cls.APPOINTMENT_CLASS +
          ',.' +
          cls.ROW_COUNT_WRAPPER_CLASS
      )
    );
    const isDragging: boolean =
      this.parent.crudModule && this.parent.crudModule.crudObj.isCrudAction
        ? true
        : false;
    const hideWrapper: (wrapper: HTMLElement) => void = (
      wrapper: HTMLElement
    ): void => {
      if (
        (this.parent as any).isReact &&
        !isNullOrUndefined(this.parent.activeViewOptions.eventTemplate)
      ) {
        const appWrapper: Element = closest(
          wrapper,
          '.' +
            cls.DAY_WRAPPER_CLASS +
            ',.' +
            cls.ALLDAY_APPOINTMENT_WRAPPER_CLASS
        );
        if (
          appWrapper &&
          !appWrapper.classList.contains(cls.APPOINTMENT_WRAPPER_HIDDEN_CLASS)
        ) {
    ...
    for (const wrapper of wrapperElements) {
      if (
        isDragging &&
        !(
          wrapper.classList.contains(cls.ALLDAY_APPOINTMENT_CLASS) ||
          wrapper.classList.contains(cls.ROW_COUNT_WRAPPER_CLASS)
        )
      ) {
        const groupIndex: number = parseInt(
          wrapper.getAttribute('data-group-index'),
          10
        );
        for (
          let j: number = 0,
            len: number = this.parent.crudModule.crudObj.sourceEvent.length;
          j < len;
          j++
        ) {
          if (
    ...
    this.allDayElement = [].slice.call(
      this.element.querySelectorAll('.' + cls.ALLDAY_CELLS_CLASS)
    );
    this.setAllDayRowHeight(0);
    if (
      this.parent.eventsProcessed.length === 0 &&
      this.parent.blockProcessed.length === 0
    ) {
      return;
    }
    const expandCollapse: HTMLElement = this.element.querySelector(
      '.' + cls.ALLDAY_APPOINTMENT_SECTION_CLASS
    ) as HTMLElement;
    ...
    if (this.allDayEvents.length > 0) {
      this.allDayEvents = this.allDayEvents.filter(
        (
          ...
          index ===
          arr.map((item: Record<string, any>) => item.Guid).indexOf(item.Guid)
      );
      removeClass(this.allDayElement, cls.ALLDAY_ROW_ANIMATE_CLASS);
      this.slots.push(
        this.parent.activeView.renderDates.map((date: Date) => +date) as any
      );
    ...
    this.parent.renderTemplates(() => {
      if (
        ...
        const wraps: Element[] = [].slice.call(
          this.parent.element.querySelectorAll(
  ...
  public initializeValues(): void {
    ...
    this.cellHeight = parseFloat(
      this.parent
        .getElementHeight(
          this.parent.element.querySelector('.e-content-wrap tbody tr')
        )
        .toFixed(2)
    );
    ...
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      for (
        let i: number = 0, len: number = this.resources.length;
        i < len;
        i++
      ) {
        this.dateRender[parseInt(i.toString(), 10)] =
          this.resources[parseInt(i.toString(), 10)].renderDates;
      }
    }
  }
  ...
  public getHeight(start: Date, end: Date): number {
    let appHeight: number =
      (((util.getUniversalTime(end) - util.getUniversalTime(start)) /
        util.MS_PER_MINUTE) *
        (this.cellHeight * this.slotCount)) /
      this.interval;
  ...
  private appendEvent(
    ...
    appointmentElement: HTMLElement,
    index: number,
    appLeft: string
  ): void {
    const appointmentWrap: HTMLElement = this.element.querySelector(
      '.' +
    ...
    const eventType: string = appointmentElement.classList.contains(
      cls.BLOCK_APPOINTMENT_CLASS
    )
      ? 'blockEvent'
      : 'event';
  ...
  private processBlockEvents(): void {
    ...
    for (const resource of resources) {
      ...
      for (
        let day: number = 0, length: number = renderDates.length;
        day < length;
        day++
      ) {
        const startDate: Date = new Date(
          renderDates[parseInt(day.toString(), 10)].getTime()
        );
        const endDate: Date = util.addDays(
          renderDates[parseInt(day.toString(), 10)],
          1
        );
        const filterEvents: Record<string, any>[] = this.filterEvents(
          startDate,
          endDate,
          this.parent.blockProcessed,
  ...
  private renderBlockEvents(
    ...
    const spannedData: Record<string, any> = this.isSpannedEvent(
      eventObj,
      dayIndex,
      resource
    );
    ...
    const currentDate: Date = util.resetTime(
      new Date(
        this.dateRender[parseInt(resource.toString(), 10)][
          parseInt(dayIndex.toString(), 10)
        ].getTime()
      )
    );
    const schedule: { [key: string]: Date } = util.getStartEndHours(
      currentDate,
      this.startHour,
      this.endHour
    );
    if (
      eStart <= eEnd &&
      this.isValidEvent(eventObj, eStart, eEnd, schedule) &&
      this.isWorkDayAvailable(resource, eStart)
    ) {
      let blockTop: string;
      let blockHeight: string;
      if (spannedData[this.fields.isAllDay]) {
        const contentWrap: HTMLElement = this.parent.element.querySelector(
      ...
      const appointmentElement: HTMLElement =
        this.createBlockAppointmentElement(
          eventObj,
          resource,
          this.isResourceEventTemplate
        );
  ...
  private renderEvents(eventType: string): void {
    ...
    const eventCollection: Record<string, any>[] =
      eventType === 'allDayEvents'
        ? this.sortByDateTime(this.allDayEvents)
        : undefined;
    const resources: number[] = this.getResourceList();
    let dateCount: number = this.getStartCount();
    let isRender: boolean;
    const appHeight: number =
      eventType === 'allDayEvents'
        ? this.parent.getElementHeightFromClass(
        ...
        : 0;
    const allDayRowTop: number =
      eventType === 'allDayEvents' && this.allDayElement.length > 0
        ? this.allDayElement[0].offsetTop
        : 0;
    for (const resource of resources) {
      isRender = true;
      if (
        ...
        if (
          this.parent.crudModule.crudObj.sourceEvent.filter(
            (data: TdData) => data.groupIndex === resource
          ).length === 0 &&
          this.parent.crudModule.crudObj.targetEvent.filter(
            (data: TdData) => data.groupIndex === resource
          ).length === 0
        ) {
          isRender = false;
      ...
      const renderedDate: Date[] =
        this.getRenderedDates(renderDates) || renderDates;
      this.slots.push(
        renderDates.map((date: Date) => {
          return +date;
        }) as any
      );
      for (
        ...
        day < length &&
        renderDates[parseInt(day.toString(), 10)] <=
          renderedDate[renderedDate.length - 1];
        day++
      ) {
        this.renderedEvents = [];
        const startDate: Date = new Date(
            ...
            if (eventType === 'allDayEvents') {
              ...
              if (this.isAllDayAppointment(event)) {
                this.allDayEvents.push(
                  extend({}, event, null, true) as Record<string, any>
                );
              } else {
  ...
  public getStartCount(): number {
    return this.parent.virtualScrollModule &&
      this.parent.activeViewOptions.allowVirtualScrolling &&
  ...
  private getDayIndex(
    ...
    if (
      ...
      !this.parent.activeViewOptions.group.hideNonWorkingDays
    ) {
    ...
    for (let i: number = 0; i < firstColumn.length; i++) {
      currentResources =
        this.parent.resourceBase.resourceDateTree[parseInt(i.toString(), 10)];
      if (
        currentDate === firstColumn[parseInt(i.toString(), 10)].date.getTime()
      ) {
        break;
      }
      dateIndex = dateIndex + firstColumn[parseInt(i.toString(), 10)].colSpan;
    }
    const resIndex: number = currentResources.findIndex(
      (x: TdData) =>
        x.groupOrder.toString() ===
  ...
  private setValues(event: Record<string, any>, resourceIndex: number): void {
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      this.cssClass =
        this.resources[parseInt(resourceIndex.toString(), 10)].cssClass;
      this.groupOrder =
        this.resources[parseInt(resourceIndex.toString(), 10)].groupOrder;
    } else {
  ...
  public createAppointmentElement(
    ...
    const recordSubject: string = (record[fieldMapping.subject] ||
    ...
    const appointmentDetails: HTMLElement = createElement('div', {
      className: cls.APPOINTMENT_DETAILS,
    });
    appointmentWrapper.appendChild(appointmentDetails);
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      const resourceIndex: number = this.parent.uiStateValues.isGroupAdaptive
        ? this.parent.uiStateValues.groupIndex
        : resource;
      appointmentWrapper.setAttribute(
        'data-group-index',
    ...
    if (!isNullOrUndefined(this.parent.activeViewOptions.eventTemplate)) {
      ...
      const resIndex: number = this.parent.uiStateValues.isGroupAdaptive
        ? this.parent.uiStateValues.groupIndex
        : resource;
      const templateName: string = this.isResourceEventTemplate
      ...
      if (isAllDay) {
        if (record[fieldMapping.isAllDay]) {
          ...
          const appointmentStartTime: HTMLElement = createElement('div', {
            className:
              cls.APPOINTMENT_TIME +
              (this.parent.isAdaptive ? ' ' + cls.DISABLE_CLASS : ''),
            innerHTML: this.parent.getTimeString(
              record[fieldMapping.startTime] as Date
            ),
          });
          const appointmentEndTime: HTMLElement = createElement('div', {
            className:
              ...
              (this.parent.isAdaptive ? ' ' + cls.DISABLE_CLASS : ''),
            innerHTML: this.parent.getTimeString(
              record[fieldMapping.endTime] as Date
            ),
          });
        ...
        const timeStr: string =
          this.parent.getTimeString(record[fieldMapping.startTime] as Date) +
          ' - ' +
    ...
    if (
      ...
      (!isNullOrUndefined(record[fieldMapping.recurrenceRule]) ||
        !isNullOrUndefined(record[fieldMapping.recurrenceID]))
    ) {
      const iconClass: string =
        record[fieldMapping.id] === record[fieldMapping.recurrenceID]
          ? cls.EVENT_RECURRENCE_ICON_CLASS
          : cls.EVENT_RECURRENCE_EDIT_ICON_CLASS;
      const recurrenceIcon: HTMLElement = createElement('div', {
    ...
    this.parent.eventBase.renderSpannedIcon(
      isAllDay ? appointmentDetails : appointmentWrapper,
      eventData
    );
    if (!isNullOrUndefined(this.cssClass)) {
      addClass([appointmentWrapper], this.cssClass);
    }
    this.applyResourceColor(
      appointmentWrapper,
      record,
      'backgroundColor',
      this.groupOrder
  ...
  private createMoreIndicator(
    ...
    const countWrapper: HTMLElement = allDayRow[
    ...
    if (countWrapper.childElementCount <= 0) {
      ...
      const moreIndicatorElement: Element = createElement('div', {
        ...
        innerHTML:
          ...
          (this.parent.isAdaptive
            ? ''
            : this.parent.localeObj.getConstant('more')),
      });
      ...
      const countCell: HTMLElement = countWrapper.querySelector(
        '.' + cls.MORE_INDICATOR_CLASS
      ) as HTMLElement;
      const moreCount: number =
        parseInt(countCell.getAttribute('data-count'), 10) + 1;
      countCell.setAttribute('data-count', moreCount.toString());
      countCell.innerHTML =
        '+' +
        this.parent.globalize.formatNumber(moreCount) +
        '&nbsp;' +
        (this.parent.isAdaptive
          ? ''
          : this.parent.localeObj.getConstant('more'));
  ...
  public isSpannedEvent(
    record: Record<string, any>,
    day: number,
    resource: number
  ): Record<string, any> {
    let currentDate: Date = util.resetTime(
      this.dateRender[parseInt(resource.toString(), 10)][
        parseInt(day.toString(), 10)
      ]
    );
    const renderedDate: Date[] = this.getRenderedDates(
      this.dateRender[parseInt(resource.toString(), 10)]
    ) || [currentDate];
    const currentDay: Date[] = renderedDate.filter(
      (date: Date) => date.getDay() === day
    );
    if (currentDay.length === 0) {
      currentDate = util.resetTime(renderedDate[0]);
    }
    const field: EventFieldsMapping = this.parent.eventFields;
    const schedule: Record<string, Date> = util.getStartEndHours(
      currentDate,
      this.startHour,
      this.endHour
    );
    const event: Record<string, any> = extend({}, record, null, true) as Record<
      string,
      any
    >;
    event.isSpanned = {
      ...
      isTop: false,
      isSameDuration:
        event[field.startTime].getTime() === event[field.endTime].getTime(),
    };
    if (
      (<Date>record[field.startTime]).getTime() < schedule.startHour.getTime()
    ) {
    ...
    const eventDates: Record<string, Date> = this.updateEventMinimumDuration(
      schedule,
  ...
  private isWorkDayAvailable(resource: number, start: Date): boolean {
    if (
      ...
      const workDays: number[] =
        this.resources[parseInt(resource.toString(), 10)].resourceData[
          this.resources[parseInt(resource.toString(), 10)].resource
            .workDaysField
  ...
  public renderAllDayEvents(
    ...
    const record: Record<string, any> = this.splitEvent(
      eventObj,
      currentDates
    )[0];
    const eStart: Date = new Date(
      (record[this.parent.eventFields.startTime] as Date).getTime()
    );
    const eEnd: Date = new Date(
      (record[this.parent.eventFields.endTime] as Date).getTime()
    );
    let appWidth: number = 0;
    let topValue: number = 1;
    const isDateRange: boolean =
      currentDates[0].getTime() <= eStart.getTime() &&
    ...
    if (
      ...
      if (this.renderedAllDayEvents[parseInt(resource.toString(), 10)]) {
        isAlreadyRendered = this.renderedAllDayEvents[
          parseInt(resource.toString(), 10)
        ].filter((event: Record<string, any>) => event.Guid === eventObj.Guid);
        if (this.parent.activeViewOptions.group.byDate) {
          isAlreadyRendered = isAlreadyRendered.filter(
            (event: Record<string, any>) =>
              event[this.parent.eventFields.startTime] >=
                currentDates[parseInt(dayIndex.toString(), 10)] &&
              event[this.parent.eventFields.endTime] <=
                util.addDays(
                  new Date(+currentDates[parseInt(dayIndex.toString(), 10)]),
                  1
                )
          );
        }
      ...
      if (isAlreadyRendered.length === 0) {
        const allDayDifference: number = (record.data as Record<string, any>)
          .count as number;
        const allDayIndex: number = this.getOverlapIndex(
          record,
          dayIndex,
          true,
          resource
        ...
        this.allDayLevel =
          this.allDayLevel < allDayIndex ? allDayIndex : this.allDayLevel;
        const widthAdjustment: number = (<Record<string, any>>record.data)
          .isRight
          ? 0
        ...
        this.renderedAllDayEvents[parseInt(resource.toString(), 10)].push(
          extend({}, record, null, true) as Record<string, any>
        );
        const allDayRow: HTMLElement[] = [].slice.call(
          this.element.querySelector('.' + cls.ALLDAY_ROW_CLASS).children
        );
        const wIndex: number = this.getDayIndex(dayIndex, resource, dayCount);
        const eventWrapper: Element = this.element.querySelector(
          '.' +
            cls.ALLDAY_APPOINTMENT_WRAPPER_CLASS +
            ':nth-child(' +
            (wIndex + 1) +
        ...
        if (inline) {
          appointmentElement =
            this.parent.inlineModule.createInlineAppointmentElement(eventObj);
        } else {
          appointmentElement = this.createAppointmentElement(
            eventObj,
            true,
        ...
        const eventData: Record<string, any> = extend(
          {},
          record.data,
          null,
          true
          ...
          (eventArgs: EventRenderedArgs) => {
            if (!eventArgs.cancel) {
              ...
              topValue +=
                (allDayIndex === 0
                  ? cellTop
                  : cellTop + allDayIndex * eventHeight) + 1;
              setStyleAttribute(appointmentElement, {
                width: appWidth + '%',
              ...
              if (allDayIndex > 1) {
                ...
                for (
                  let count: number = 0, length: number = allDayDifference;
                  count < length;
                  count++
                ) {
              ...
              this.allDayElement[0].setAttribute(
                'data-count',
                this.allDayLevel.toString()
              );
              const allDayRowHeight: number =
                (!this.parent.uiStateValues.expand && this.allDayLevel > 2
                  ? 3 * eventHeight
                  : (this.allDayLevel + 1) * eventHeight) + 4;
  ...
  public renderNormalEvents(
    ...
    const record: Record<string, any> = this.isSpannedEvent(
      eventObj,
      dayIndex,
      resource
    );
    ...
    let appWidth: string = '0%';
    const appLeft: string = '0%';
    ...
    const isValidEvent: boolean = this.isValidEvent(
      eventObj,
      eStart,
      eEnd,
      schedule
    ...
    if (
      eStart.getTime() < this.parent.minDate.getTime() ||
      eEnd.getTime() > this.parent.maxDate.getTime()
    ) {
      return;
    }
    if (
      ...
      const appHeight: number = record.isSpanned.isSameDuration
        ? this.cellHeight
        : this.getHeight(eStart, eEnd);
      if (eStart.getTime() >= schedule.startHour.getTime()) {
        topValue = this.getTopValue(eStart, dayIndex, resource);
      }
      const appIndex: number = this.getOverlapIndex(
        record,
        dayIndex,
        false,
        resource
      ...
      const tempData: Record<string, any> =
        this.adjustOverlapElements(argsData);
      ...
      this.renderedEvents[parseInt(resource.toString(), 10)].push(
        extend({}, record, null, true) as Record<string, any>
      );
      let appointmentElement: HTMLElement;
      if (inline) {
      ...
      const iconHeight: number =
        appointmentElement.querySelectorAll('.' + cls.EVENT_INDICATOR_CLASS)
          .length * 15;
      const maxHeight: number = appHeight - 40 - iconHeight;
      const subjectElement: HTMLElement = appointmentElement.querySelector(
        '.' + cls.SUBJECT_CLASS
      ) as HTMLElement;
  ...
  private getEventWidth(): string {
    ...
    const tempWidth: number =
      (width - this.overlapEvents.length) / this.overlapEvents.length;
  ...
  public getTopValue(date: Date, day: number, resource: number): number {
    const viewDate: Date = util.resetTime(
      this.dateRender[parseInt(resource.toString(), 10)][
        parseInt(day.toString(), 10)
      ]
    );
    ...
    const adjustedStartHour: number =
      util.isDaylightSavingTime(viewDate) &&
      startHour.getHours() !== this.startHour.getHours()
        ? this.startHour.getHours()
        : startHour.getHours();
    const diffInMinutes: number =
      (date.getHours() - adjustedStartHour) * 60 +
      (date.getMinutes() - startHour.getMinutes());
    return this.parent.activeViewOptions.timeScale.enable
      ? (diffInMinutes * this.cellHeight * this.slotCount) / this.interval
      : 0;
  }
  ...
  private getOverlapIndex(
    ...
    if (isAllDay) {
      if (
        ...
        const date: Date = util.resetTime(
          new Date(
            this.dateRender[parseInt(resource.toString(), 10)][
              parseInt(day.toString(), 10)
            ].getTime()
        ...
        eventsList = this.renderedAllDayEvents[
          parseInt(resource.toString(), 10)
        ].filter(
          (app: Record<string, any>) =>
            util.resetTime(<Date>app[fieldMapping.startTime]).getTime() <=
              date.getTime() &&
            util.resetTime(<Date>app[fieldMapping.endTime]).getTime() >=
              date.getTime()
        );
        if (this.parent.activeViewOptions.group.resources.length > 0) {
          eventsList = this.filterEventsByResource(
            this.resources[parseInt(resource.toString(), 10)],
            eventsList
          );
        }
      ...
      const appointmentList: Record<string, any>[] = !isNullOrUndefined(
        this.renderedEvents[parseInt(resource.toString(), 10)]
      )
        ? this.renderedEvents[parseInt(resource.toString(), 10)]
        : [];
      ...
      this.overlapList = appointmentList.filter(
        (data: Record<string, any>) =>
          (data[fieldMapping.endTime] > recordStart &&
            data[fieldMapping.startTime] <= recordEnd) ||
          (data[fieldMapping.startTime] >= recordEnd &&
            data[fieldMapping.endTime] <= recordStart) ||
          (data[fieldMapping.endTime].getTime() ===
            data[fieldMapping.startTime].getTime() &&
            data[fieldMapping.startTime].getTime() === recordStart.getTime() &&
            data[fieldMapping.endTime] < recordEnd)
      );
      if (this.parent.activeViewOptions.group.resources.length > 0) {
        this.overlapList = this.filterEventsByResource(
          this.resources[parseInt(resource.toString(), 10)],
          this.overlapList
        );
      }
      this.overlapList.forEach((obj: Record<string, any>) => {
        let filterList: Record<string, any>[] = appointmentList.filter(
          (data: Record<string, any>) =>
            data[fieldMapping.endTime] > obj[fieldMapping.startTime] &&
            data[fieldMapping.startTime] <= obj[fieldMapping.endTime]
        );
        if (this.parent.activeViewOptions.group.resources.length > 0) {
          filterList = this.filterEventsByResource(
            this.resources[parseInt(resource.toString(), 10)],
            filterList
          );
        }
        const collection: Record<string, any>[] = filterList.filter(
          (val: Record<string, any>) => this.overlapList.indexOf(val) === -1
        );
      ...
      for (let i: number = 0; i < appointment.length - 1; i++) {
        for (let j: number = i + 1; j < appointment.length; j++) {
          if (
            appointment[parseInt(i.toString(), 10)][fieldMapping.id] ===
      ...
      for (const event of eventsList) {
        ...
        if (
          isNullOrUndefined(this.overlapEvents[parseInt(index.toString(), 10)])
        ) {
    ...
    if (!isAllDay) {
      eventsList = eventsList.filter(
        (obj: Record<string, any>) =>
          (obj[fieldMapping.startTime] === record[fieldMapping.startTime] &&
          ...
          (obj[fieldMapping.endTime] === record[fieldMapping.startTime] &&
            obj[fieldMapping.startTime] === record[fieldMapping.endTime]) ||
          (obj[fieldMapping.startTime].getTime() ===
            record[fieldMapping.startTime].getTime() &&
            obj[fieldMapping.endTime].getTime() ===
    ...
    if (eventsList.length > 0) {
      const appLevel: number[] = eventsList.map(
        (obj: Record<string, any>) => obj.Index
      ) as number[];
      appIndex =
        appLevel.length > 0 ? this.getSmallestMissingNumber(appLevel) : 0;
    }
  ...
  private adjustOverlapElements(args: ElementData): Record<string, any> {
    ...
    for (
      let i: number = 0, length1: number = this.overlapEvents.length;
      i < length1;
      i++
    ) {
      if (!isNullOrUndefined(this.overlapEvents[parseInt(i.toString(), 10)])) {
        for (
          let j: number = 0,
            length2: number =
              this.overlapEvents[parseInt(i.toString(), 10)].length;
          j < length2;
          j++
        ) {
          const dayCount: number = this.getDayIndex(
            args.dayIndex,
            args.resource,
            args.day
          );
          const element: HTMLElement = this.element.querySelector(
            '#e-appointment-wrapper-' + dayCount
          ) as HTMLElement;
          if (element && element.childElementCount > 0) {
            const eleGuid: string = (<Record<string, any>>(
              this.overlapEvents[parseInt(i.toString(), 10)][
                parseInt(j.toString(), 10)
              ]
            )).Guid as string;
            if (
              element.querySelectorAll('div[data-guid="' + eleGuid + '"]')
                .length > 0 &&
              eleGuid !== args.record.Guid
            ) {
              const apps: HTMLElement = element.querySelector(
                'div[data-guid="' + eleGuid + '"]'
              ) as HTMLElement;
  ...
  private setAllDayRowHeight(height: number): void {
    const dateHeader: HTMLElement = this.parent.element.querySelector(
    ...
    const allDayRow: HTMLElement = this.parent.element.querySelector(
      '.' + cls.ALLDAY_ROW_CLASS
    ) as HTMLElement;
    allDayRow.style.height = '';
    if (
      this.parent.uiStateValues.expand &&
      this.parent.height !== 'auto' &&
      this.parent.enableAllDayScroll
    ) {
  ...
  private addOrRemoveClass(): void {
    ...
    const moreEventCount: HTMLElement = this.element.querySelector(
      '.' + cls.ALLDAY_APPOINTMENT_SECTION_CLASS
    ) as HTMLElement;
    ...
    const countCell: Element[] = [].slice.call(
      this.element.querySelectorAll('.' + cls.ROW_COUNT_WRAPPER_CLASS)
    );
  ...
  private getEventHeight(): number {
    const eventElement: HTMLElement = createElement('div', {
      className: cls.APPOINTMENT_CLASS,
      styles: 'visibility:hidden',
    });
    const eventWrapper: Element = this.element.querySelector(
      '.' + cls.ALLDAY_APPOINTMENT_WRAPPER_CLASS + ':first-child'
    );
  ...
  private rowExpandCollapse(): void {
    const target: HTMLElement = this.element.querySelector(
      '.' + cls.ALLDAY_APPOINTMENT_SECTION_CLASS
    ) as HTMLElement;
    this.parent.uiStateValues.expand = target.classList.contains(
      cls.APPOINTMENT_ROW_EXPAND_CLASS
    );
    let rowHeight: number;
    if (this.parent.uiStateValues.expand) {
      target.setAttribute(
        'title',
        this.parent.localeObj.getConstant('collapseAllDaySection')
      );
      target.setAttribute(
        'aria-label',
        this.parent.localeObj.getConstant('collapseAllDaySection')
      ...
      target.setAttribute(
        'title',
        this.parent.localeObj.getConstant('expandAllDaySection')
      );
      target.setAttribute(
        'aria-label',
        this.parent.localeObj.getConstant('expandAllDaySection')
