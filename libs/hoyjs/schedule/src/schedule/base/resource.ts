...
import { TreeView, NodeClickEventArgs } from '@hoyui/navigations';
...
export class ResourceBase {
  ...
  public renderResourceHeaderIndent(tr: Element): void {
    const resColTd: Element = createElement('td', {
      className: cls.RESOURCE_LEFT_TD_CLASS,
    });
    const resColDiv: Element = createElement('div', {
    ...
    const args: RenderCellEventArgs = {
  ...
  public hideResourceRows(tBody: Element): void {
    if (
      this.resourceCollection.length <= 1 ||
    ...
    for (let i: number = 0; i < trCount; i++) {
      const resData: Record<string, any> =
        this.lastResourceLevel[parseInt(i.toString(), 10)].resourceData;
      const res: ResourcesModel =
        this.lastResourceLevel[parseInt(i.toString(), 10)].resource;
      if (
        resData.ClassName === cls.RESOURCE_PARENT_CLASS &&
        !resData[res.expandedField] &&
        !isNullOrUndefined(resData[res.expandedField])
      ) {
        const trCollection: HTMLElement[] = [].slice.call(tBody.children);
        const slicedCollection: HTMLElement[] = trCollection.slice(
          i + 1,
          i + (parseInt(resData.Count as string, 10) + 1)
        );
  ...
  public createResourceColumn(): Element {
    ...
    const resDiv: Element = createElement('div', {
      className: cls.RESOURCE_COLUMN_WRAP_CLASS,
    });
    const tbl: Element = this.parent.activeView.createTableLayout(
      cls.RESOURCE_COLUMN_TABLE_CLASS
    );
    if (
    ...
    if (this.parent.virtualScrollModule) {
      const resourceCount: number =
        this.parent.virtualScrollModule.getRenderedCount();
  ...
  public setExpandedResources(): void {
    ...
    for (let i: number = 0; i < this.lastResourceLevel.length; i++) {
      ...
      const isExpanded: boolean = resource[
        this.lastResourceLevel[parseInt(i.toString(), 10)].resource
          .expandedField
      ] as boolean;
  ...
  public getContentRows(
    ...
    const td: Element = createElement('td', {
      attrs: { tabindex: isVirtualScroll ? '-1' : '0' },
    });
    const existingGroupIndices: number[] =
      this.parent.activeView.getGroupIndices();
    for (let i: number = 0; i < resData.length; i++) {
      if (
        existingGroupIndices.length > 0 &&
        existingGroupIndices.indexOf(
          resData[parseInt(i.toString(), 10)].groupIndex
        ) > -1
      ) {
        continue;
      }
      const ntd: Element = td.cloneNode() as Element;
      rIndex = util.findIndexInData(
        <Record<string, any>[]>resColl,
        'name',
        resData[parseInt(i.toString(), 10)].resource.name
      );
      if (rIndex === resColl.length - 1) {
        extend(resData[parseInt(i.toString(), 10)].resourceData, {
          ClassName: cls.RESOURCE_CHILD_CLASS,
        });
        this.renderedResources[parseInt(i.toString(), 10)].className = [
          cls.RESOURCE_CHILD_CLASS,
        ];
      } else {
        extend(resData[parseInt(i.toString(), 10)].resourceData, {
        ...
        this.renderedResources[parseInt(i.toString(), 10)].className = [
          cls.RESOURCE_PARENT_CLASS,
        ];
      }
      left = rIndex * this.leftPixel + 'px';
      if (
        (resData[parseInt(i.toString(), 10)].resourceData
          .ClassName as string) === cls.RESOURCE_PARENT_CLASS &&
        !isNullOrUndefined(
          resData[parseInt(i.toString(), 10)].resourceData.Count
        ) &&
        resData[parseInt(i.toString(), 10)].resourceData.Count > 0
      ) {
        let iconClass: string;
        if (
          resData[parseInt(i.toString(), 10)].resourceData[
            resColl[parseInt(rIndex.toString(), 10)].expandedField
          ] ||
          isNullOrUndefined(
            resData[parseInt(i.toString(), 10)].resourceData[
              resColl[parseInt(rIndex.toString(), 10)].expandedField
      ...
      this.parent.activeView.setResourceHeaderContent(
        ntd,
        resData[parseInt(i.toString(), 10)],
        cls.RESOURCE_TEXT_CLASS
      );
      ntd.setAttribute(
        'data-group-index',
        resData[parseInt(i.toString(), 10)].groupIndex.toString()
      );
      ntd.setAttribute(
        'aria-label',
        (resData[parseInt(i.toString(), 10)].resourceData[
          resData[parseInt(i.toString(), 10)].resource.textField
        ] as string) + ' resource'
      );
      if (!this.parent.activeViewOptions.resourceHeaderTemplate) {
        this.setMargin(
          ntd.querySelector('.' + cls.RESOURCE_TEXT_CLASS) as HTMLElement,
          left
        );
      }
      const classCollection: string[] = [
        cls.RESOURCE_CELLS_CLASS,
        resData[parseInt(i.toString(), 10)].resourceData.ClassName as string,
      ];
  ...
  private countCalculation(
    parentCollection: ResourcesModel[],
    wholeCollection: ResourcesModel[]
  ): void {
    let collection: Record<string, any>[];
    for (let y: number = 0; y < parentCollection.length; y++) {
      const data: Record<string, any>[] = parentCollection[
        parentCollection.length - (y + 1)
      ...
      for (let x: number = 0; x < data.length; x++) {
        ...
        if (this.parent.activeViewOptions.group.byGroupID) {
          const query: Query = new Query().where(
            wholeCollection[wholeCollection.length - 1].groupIDField,
            'equal',
            data[parseInt(x.toString(), 10)][
              parentCollection[parentCollection.length - (y + 1)].idField
            ] as string
          );
          collection = new DataManager(
            wholeCollection[wholeCollection.length - 1].dataSource
          ).executeLocal(query) as Record<string, any>[];
        } else {
          collection = wholeCollection[wholeCollection.length - 1]
            .dataSource as Record<string, any>[];
        }
        for (let z: number = 0; z < collection.length; z++) {
          totalCount =
            totalCount +
            parseInt(
              collection[parseInt(z.toString(), 10)].Count as string,
              10
            );
        }
        totalCount =
          totalCount +
          parseInt(data[parseInt(x.toString(), 10)].Count as string, 10);
  ...
  public onTreeIconClick(e: Event): void {
    ...
    let hide: boolean;
    const trElement: HTMLTableRowElement = closest(
      target as Element,
      '.' + cls.RESOURCE_PARENT_CLASS
    ).parentElement as HTMLTableRowElement;
    const index: number = parseInt(
      trElement.children[0].getAttribute('data-group-index'),
      10
    );
    let args: ActionEventArgs = {
      ...
      event: e,
      groupIndex: index,
      requestType: !target.classList.contains(cls.RESOURCE_COLLAPSE_CLASS)
        ? 'resourceExpand'
        : 'resourceCollapse',
    };
    this.parent.trigger(
      ...
      (actionArgs: ActionEventArgs) => {
        if (!actionArgs.cancel) {
          ...
          const eventElements: Element[] = [].slice.call(
            this.parent.element.querySelectorAll(
              '.' + cls.APPOINTMENT_CLASS + ',.' + cls.MORE_INDICATOR_CLASS
            )
          );
  ...
  public updateContent(index: number, hide: boolean): void {
    ...
    let pNode: boolean;
    const clickedRes: Record<string, any> = this.lastResourceLevel[
      parseInt(index.toString(), 10)
    ].resourceData as Record<string, any>;
    const resRows: Element[] = [].slice.call(
      this.parent.element.querySelectorAll(
        '.' + cls.RESOURCE_COLUMN_WRAP_CLASS + ' ' + 'tr'
      )
    );
    const contentRows: Element[] = [].slice.call(
      this.parent.element.querySelectorAll(
        '.' + cls.CONTENT_WRAP_CLASS + ' ' + 'tbody tr'
      )
    );
    const eventRows: Element[] = [].slice.call(
      this.parent.element.querySelectorAll(
    ...
    for (let j: number = 0; j < clickedRes.Count; j++) {
      ...
      workCellCollection.push(
        contentRows[index + j + 1] as HTMLTableRowElement
      );
    ...
    for (let i: number = 0; i < rowCollection.length; i++) {
      ...
      ].children[0].classList.contains(cls.RESOURCE_PARENT_CLASS);
      clonedCollection[parseInt(index.toString(), 10)].resourceData[
        clonedCollection[parseInt(index.toString(), 10)].resource.expandedField
      ] = !hide;
      if (hide) {
        if (pNode) {
          const trElem: Element = rowCollection[
            parseInt(i.toString(), 10)
          ].querySelector('.' + cls.RESOURCE_TREE_ICON_CLASS);
          if (trElem) {
            classList(
              trElem,
              [cls.RESOURCE_EXPAND_CLASS],
        ...
        if (
          !rowCollection[parseInt(i.toString(), 10)].classList.contains(
            cls.HIDDEN_CLASS
          )
        ) {
          addClass(
            [
              rowCollection[parseInt(i.toString(), 10)],
              workCellCollection[parseInt(i.toString(), 10)],
              headerRowCollection[parseInt(i.toString(), 10)],
            ],
            cls.HIDDEN_CLASS
          );
        }
        ...
        if (pNode) {
          const rowIndex: number =
            rowCollection[parseInt(i.toString(), 10)].rowIndex;
          if (
            !clonedCollection[parseInt(rowIndex.toString(), 10)].resourceData[
              clonedCollection[parseInt(rowIndex.toString(), 10)].resource
                .expandedField
            ] &&
            !isNullOrUndefined(
              clonedCollection[parseInt(rowIndex.toString(), 10)].resourceData[
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  public updateVirtualContent(
    index: number,
    expand: boolean,
    e: Event,
    target: Element
  ): void {
    this.lastResourceLevel[parseInt(index.toString(), 10)].resourceData[
      this.lastResourceLevel[
        parseInt(index.toString(), 10)
      ].resource.expandedField
    ] = !expand;
    this.setExpandedResources();
    const resourceCount: number =
      this.parent.virtualScrollModule.getRenderedCount();
    const startIndex: number = this.expandedResources.indexOf(
      this.renderedResources[0]
    );
    this.parent.virtualScrollModule.existingDataCollection =
      this.renderedResources;
    this.renderedResources = this.expandedResources.slice(
      startIndex,
      startIndex + resourceCount
    );
    if (this.renderedResources.length < resourceCount) {
      ...
      this.renderedResources = this.expandedResources.slice(
        sIndex,
        this.expandedResources.length
      );
    }
    const virtualTrack: HTMLElement = this.parent.element.querySelector(
      '.' + cls.VIRTUAL_TRACK_CLASS
    );
    this.parent.virtualScrollModule.updateVirtualTrackHeight(virtualTrack);
    const resTable: HTMLElement = this.parent.element.querySelector(
      '.' + cls.RESOURCE_COLUMN_WRAP_CLASS + ' ' + 'table'
    ) as HTMLElement;
    const contentTable: HTMLElement = this.parent.element.querySelector(
      '.' + cls.CONTENT_WRAP_CLASS + ' ' + 'table'
    ) as HTMLElement;
    const eventTable: HTMLElement = this.parent.element.querySelector(
      '.' + cls.EVENT_TABLE_CLASS
    ) as HTMLElement;
    this.parent.virtualScrollModule.updateContent(
      resTable,
      contentTable,
      eventTable,
      this.renderedResources
    );
    const timeIndicator: HTMLElement = this.parent.element.querySelector(
      '.' + cls.CURRENT_TIMELINE_CLASS
    ) as HTMLElement;
  ...
  public renderResourceHeader(): void {
    const resourceWrapper: HTMLElement = createElement('div', {
      className: cls.RESOURCE_TOOLBAR_CONTAINER,
    });
    resourceWrapper.innerHTML =
      '<div class="' +
      cls.RESOURCE_HEADER_TOOLBAR +
      '"><div class="' +
      cls.RESOURCE_MENU +
      '">' +
      '<div class="e-icons ' +
      cls.RESOURCE_MENU_ICON +
      '"></div></div><div class="' +
      cls.RESOURCE_LEVEL_TITLE +
      '"></div></div>';
    if (this.parent.currentView === 'MonthAgenda') {
      const target: Element = this.parent.activeView
        .getPanel()
        .querySelector('.' + cls.CONTENT_WRAP_CLASS);
      target.insertBefore(
        resourceWrapper,
        target.querySelector('.' + cls.WRAPPER_CONTAINER_CLASS)
      );
    } else {
      this.parent.element.insertBefore(
        resourceWrapper,
        this.parent.element.querySelector('.' + cls.TABLE_CONTAINER_CLASS)
      );
    }
    this.renderResourceHeaderText();
    EventHandler.add(
      resourceWrapper.querySelector('.' + cls.RESOURCE_MENU_ICON),
      'click',
      this.menuClick,
      this
    );
  ...
  public renderResourceTree(): void {
    ...
    if (this.parent.currentView === 'MonthAgenda') {
      const target: Element = this.parent.activeView
        .getPanel()
        .querySelector('.' + cls.WRAPPER_CONTAINER_CLASS);
      target.insertBefore(treeWrapper, target.children[0]);
      target.appendChild(this.popupOverlay);
    } else {
      this.parent.element
        .querySelector('.' + cls.TABLE_CONTAINER_CLASS)
        .appendChild(treeWrapper);
      this.parent.element
        .querySelector('.' + cls.TABLE_CONTAINER_CLASS)
        .appendChild(this.popupOverlay);
    }
    const resourceTree: HTMLElement = createElement('div', {
    ...
    this.treeViewObj = new TreeView({
      ...
      fields: {
        dataSource: [].slice.call(this.generateTreeData()) as Record<
          string,
          any
        >[],
        id: 'resourceId',
    ...
    this.treePopup = new Popup(treeWrapper, {
      ...
      relateTo: this.parent.element.querySelector(
        '.' + cls.TABLE_CONTAINER_CLASS
      ) as HTMLElement,
      enableRtl: this.parent.enableRtl,
      hideAnimation: { name: 'SlideLeftOut', duration: 500 },
      showAnimation: { name: 'SlideLeftIn', duration: 500 },
      viewPortElement: this.parent.element.querySelector(
        '.' +
          (this.parent.currentView === 'MonthAgenda'
            ? cls.WRAPPER_CONTAINER_CLASS
            : cls.TABLE_CONTAINER_CLASS)
      ) as HTMLElement,
    });
  ...
  private resourceTreeCreated(): void {
    if (
      this.parent.activeViewOptions.resourceHeaderTemplate &&
      (this as any).parent.portals &&
      (this.treeViewObj as any).portals
    ) {
      (this as any).parent.portals = (this as any).parent.portals.concat(
        (this.treeViewObj as any).portals
      );
  ...
  private generateTreeData(isTimeLine?: boolean): ResourceDetails[] | TdData[] {
    ...
    for (
      let i: number = 0, len: number = this.resourceTreeLevel.length;
      i < len;
      i++
    ) {
      const treeHandler: CallbackFunction = (
        treeLevel: TreeSlotData,
        index: number,
        levelId: string
      ): ResourceDetails => {
        const resource: ResourcesModel =
          this.resourceCollection[parseInt(index.toString(), 10)];
      ...
      if (!isTimeLine) {
        treeCollection.push(
          treeHandler(
            this.resourceTreeLevel[parseInt(i.toString(), 10)],
            0,
            (i + 1).toString()
          )
        );
  ...
  private renderResourceHeaderText(): void {
    const resource: TdData =
      this.lastResourceLevel[this.parent.uiStateValues.groupIndex];
    const headerCollection: HTMLElement[] = [];
    for (
      let i: number = 0, len: number = resource.groupOrder.length;
      i < len;
      i++
    ) {
      const resourceLevel: ResourcesModel =
        this.resourceCollection[parseInt(i.toString(), 10)];
      const resourceText: Record<string, any>[] = (
        resourceLevel.dataSource as Record<string, any>[]
      ).filter(
        (resData: Record<string, any>) =>
          resData[resourceLevel.idField] ===
          resource.groupOrder[parseInt(i.toString(), 10)]
      );
      const resourceName: HTMLElement = createElement('div', {
        className: cls.RESOURCE_NAME,
      });
      this.parent.sanitize(
        (<Record<string, any>>resourceText[0])[
          resourceLevel.textField
        ] as string,
        resourceName
      );
      headerCollection.push(resourceName);
      const levelIcon: HTMLElement = createElement('div', {
    ...
    const target: HTMLElement =
      this.parent.currentView === 'MonthAgenda'
        ? this.parent.activeView.getPanel()
        : this.parent.element;
    const headerWrapper: Element = target.querySelector(
      '.' + cls.RESOURCE_LEVEL_TITLE
    );
    ...
    if (this.lastResourceLevel.length === 1) {
      addClass(
        [this.parent.element.querySelector('.' + cls.RESOURCE_MENU)],
        cls.DISABLE_CLASS
      );
    }
  }
  ...
  private menuClick(event: Event): void {
    if (
      this.parent.element
        .querySelector('.' + cls.RESOURCE_TREE_POPUP)
        .classList.contains(cls.POPUP_OPEN)
    ) {
      this.treePopup.hide();
      removeClass([this.popupOverlay], cls.ENABLE_CLASS);
    } else {
      const treeNodes: HTMLElement[] = [].slice.call(
        this.treeViewObj.element.querySelectorAll(
          '.e-list-item:not(.e-has-child)'
        )
      );
  ...
  public selectResourceByIndex(groupIndex: number): void {
    if (
      this.lastResourceLevel &&
      groupIndex > -1 &&
      groupIndex < this.lastResourceLevel.length
    ) {
  ...
  private resourceClick(event: NodeClickEventArgs): void {
    if (!event.node.classList.contains('e-has-child')) {
      this.treePopup.hide();
      removeClass([this.popupOverlay], cls.ENABLE_CLASS);
      const treeNodes: HTMLLIElement[] = [].slice.call(
        this.treeViewObj.element.querySelectorAll(
          '.e-list-item:not(.e-has-child)'
        )
      );
  ...
  private documentClick(args: { event: Event }): void {
    ...
    const treeWrapper: Element = this.parent.element.querySelector(
      '.' + cls.RESOURCE_TREE_POPUP
    );
  ...
  public bindResourcesData(isSetModel: boolean): void {
    ...
    for (const resource of this.parent.resources) {
      const dataModule: Data = new Data(
        this.parent,
        resource.dataSource,
        resource.query
      );
    ...
    Promise.all(promises)
      .then((e: ReturnType[]) => this.dataManagerSuccess(e, isSetModel))
  ...
  private dataManagerSuccess(e: ReturnType[], isSetModel: boolean): void {
    ...
    for (let i: number = 0, length: number = e.length; i < length; i++) {
      const resource: ResourcesModel =
        this.parent.resources[parseInt(i.toString(), 10)];
      const resourceObj: ResourcesModel = this.getResourceModel(
        resource,
        e[parseInt(i.toString(), 10)].result
      );
  ...
  public refreshLayout(isSetModel: boolean): void {
    if (
      isNullOrUndefined(this.parent.uiStateValues.groupIndex) ||
  ...
  public setResourceCollection(): void {
    ...
    if (this.parent.activeViewOptions.group.resources.length > 0) {
      for (const resource of this.parent.activeViewOptions.group.resources) {
        const index: number = util.findIndexInData(
          <Record<string, any>[]>this.parent.resourceCollection,
          'name',
          resource
        );
        if (index >= 0) {
          requiredResources.push(
            this.parent.resourceCollection[parseInt(index.toString(), 10)]
          );
        }
      }
  ...
  public generateResourceLevels(
    ...
    const group: CallbackFunction = (
      resources: ResourcesModel[],
      index: number,
      prevResource?: ResourcesModel,
      ...
      if (resource) {
        ...
        if (prevResourceData && this.parent.activeViewOptions.group.byGroupID) {
          const id: string = (prevResourceData as Record<string, any>)[
            prevResource.idField
          ] as string;
          data = (resource.dataSource as Record<string, any>[]).filter(
            (e: Record<string, any>) => e[resource.groupIDField] === id
          );
        } else {
        ...
        for (let i: number = 0; i < data.length; i++) {
          ...
          groupOrder.push(
            (data[parseInt(i.toString(), 10)] as Record<string, any>)[
              resource.idField
            ] as string
          );
          const items: TdData[] = group(
            resources.slice(1),
            index + 1,
            resource,
          ...
          if (
            ...
            index + 1 === this.resourceCollection.length
          ) {
            const workDays: number[] = data[parseInt(i.toString(), 10)][
              resource.workDaysField
            ] as number[];
            const resStartHour: string = data[parseInt(i.toString(), 10)][
              resource.startHourField
            ] as string;
            const resEndHour: string = data[parseInt(i.toString(), 10)][
              resource.endHourField
            ] as string;
            if (workDays && workDays.length > 0) {
              renderDates = this.parent.activeView.getRenderDates(workDays);
              resWorkDays = workDays;
              dateCol = this.parent.activeView.getDateSlots(
                renderDates,
                workDays
              );
            } else {
            ...
            const dateSlots: TdData[] = this.generateCustomHours(
              dateCol,
              resStartHour,
              resEndHour,
              groupOrder
          ...
          const resCssClass: string = data[parseInt(i.toString(), 10)][
            resource.cssClassField
          ] as string;
          const slotData: TreeSlotData = {
            ...
            colSpan: this.parent.activeViewOptions.group.byDate
              ? 1
              : dateCol.length,
    ...
    return isTimeLine
      ? []
      : this.generateHeaderLevels(resTreeGroup, lastColumnDates, innerDates);
  }

  public generateCustomHours(
    renderDates: TdData[],
    startHour: string,
    endHour: string,
    groupOrder?: string[]
  ...
  private generateHeaderLevels(
    ...
    for (let i: number = resTreeGroup.length - 1; i >= 0; i--) {
      ...
      for (const currentLevelChilds of resTreeGroup[
        ...
        for (const currentLevelChild of currentLevelChilds) {
          if (resTreeGroup[i + 1] && resTreeGroup[i + 1].length > 0) {
            ...
            for (const nextLevelChild of nextLevelChilds) {
              if (
                ...
                (this.parent.activeViewOptions.group.byGroupID &&
                  nextLevelChild.resourceData[
                    nextLevelChild.resource.groupIDField
                  ] ===
                    currentLevelChild.resourceData[
                      currentLevelChild.resource.idField
                    ])
              ) {
          ...
          temp++;
          headerLevels[currentLevelChild.resourceLevelIndex] =
            headerLevels[currentLevelChild.resourceLevelIndex] || [];
          headerLevels[currentLevelChild.resourceLevelIndex].push(
            currentLevelChild
          );
        }
      }
    ...
    if (!this.parent.activeViewOptions.group.byDate) {
      ...
      for (const lastLevelResource of this.lastResourceLevel) {
        for (let i: number = 0; i < lastLevelResource.colSpan; i++) {
          lastColumnDates[parseInt(index.toString(), 10)].groupIndex =
            lastLevelResource.groupIndex;
          index++;
        }
      }
    ...
    if (this.parent.activeViewOptions.group.hideNonWorkingDays) {
      ...
      for (const headerDate of headerDates) {
        ...
        for (let j: number = 0; j < this.lastResourceLevel.length; j++) {
          let workDays: number[] =
            this.lastResourceLevel[parseInt(j.toString(), 10)].resourceData[
              this.lastResourceLevel[parseInt(j.toString(), 10)].resource
                .workDaysField
            ];
          if (!workDays) {
          ...
          if (workDays.indexOf(headerDate.date.getDay()) !== -1) {
            const resTd: TdData = <TdData>(
              extend(
                {},
                this.lastResourceLevel[parseInt(j.toString(), 10)],
                null,
                true
              )
            );
            resTd.date = headerDate.date;
            this.lastResourceLevel[parseInt(j.toString(), 10)].workDays =
              workDays;
            resTd.startHour =
              this.parent.getStartEndTime(
                resTd.resourceData[resTd.resource.startHourField] as string
              ) || headerDate.startHour;
            resTd.endHour =
              this.parent.getStartEndTime(
                resTd.resourceData[resTd.resource.endHourField] as string
              ) || headerDate.endHour;
            this.resourceDateTree[parseInt(dateIndex.toString(), 10)].push(
              resTd
            );
            for (let k: number = 0; k < resTd.groupOrder.length; k++) {
              ...
              if (k === resTd.groupOrder.length - 1) {
                ...
                const filterDates: Date[] = resTd.renderDates.filter(
                  (x: Date) => x.getDay() === headerDate.date.getDay()
                );
                renderDates[parseInt(j.toString(), 10)] =
                  renderDates[parseInt(j.toString(), 10)].concat(filterDates);
                currentDateLevels[parseInt(k.toString(), 10)].push(resTd);
                continue;
              }
              const currentLevel: TdData[] = levels[parseInt(k.toString(), 10)];
              const filteredResource: TdData[] = currentLevel.filter(
                (data: TdData) =>
                  data.resourceData[data.resource.idField] ===
                  resTd.groupOrder[parseInt(k.toString(), 10)]
              );
              if (filteredResource && filteredResource.length > 0) {
                const existedResource: TdData[] = currentDateLevels[
                ...
                if (existedResource && existedResource.length > 0) {
                  ...
                  const filteredTd: TdData = <TdData>(
                    extend({}, filteredResource[0], null, true)
                  );
                  filteredTd.colSpan = 1;
                  currentDateLevels[parseInt(k.toString(), 10)].push(
                    filteredTd
                  );
                }
              }
        ...
        if (currentDateLevels.length > 0) {
          for (let l: number = 0; l < levels.length; l++) {
            ...
            dateHeaderLevels[parseInt(l.toString(), 10)] = dateHeaderLevels[
              parseInt(l.toString(), 10)
            ].concat(currentDateLevels[parseInt(l.toString(), 10)]);
          }
          headerDate.colSpan =
            currentDateLevels[currentDateLevels.length - 1].length;
          datesColumn.push(headerDate);
        }
        dateIndex++;
      }
      this.resourceDateTree = this.resourceDateTree.filter(
        (data: TdData[]) => data.length > 0
      );
      this.lastResourceLevel.forEach((x: TdData, index: number) => {
        if (renderDates[parseInt(index.toString(), 10)]) {
          x.renderDates = renderDates[parseInt(index.toString(), 10)].sort(
    ...
    for (const headerDate of headerDates) {
      ...
      for (
        let k: number = 0, length: number = resGroup.length;
        k < length;
        k++
      ) {
        ...
        dateHeaderLevels[parseInt(k.toString(), 10)] = dateHeaderLevels[
          parseInt(k.toString(), 10)
        ].concat(resGroup[parseInt(k.toString(), 10)]);
      }
    }
    dateHeaderLevels.unshift(datesColumn);
    return dateHeaderLevels;
  ...
  public setResourceValues(
    eventObj: Record<string, any>,
    groupIndex?: number
  ): void {
    const setValues: CallbackFunction = (
      index: number,
      field: string,
      value: Record<string, any>
    ) => {
      if (
        this.resourceCollection[parseInt(index.toString(), 10)].allowMultiple &&
    ...
    if (groupIndex === void 0) {
      groupIndex = this.parent.uiStateValues.isGroupAdaptive
        ? this.parent.uiStateValues.groupIndex
        : this.parent.activeCellsData.groupIndex;
    }
    if (
      this.parent.activeViewOptions.group.resources.length > 0 &&
      !isNullOrUndefined(groupIndex)
    ) {
      const groupOrder: number[] | string[] =
        this.lastResourceLevel[parseInt(groupIndex.toString(), 10)].groupOrder;
      for (
        let index: number = 0;
        ...
        setValues(
          index,
          this.resourceCollection[parseInt(index.toString(), 10)].field,
      ...
      for (
        ...
        const data: Record<string, any> = (
          this.resourceCollection[parseInt(index.toString(), 10)] as Record<
            string,
            unknown[]
          >
        ).dataSource[0] as Record<string, any>;
        if (data) {
          setValues(
            index,
            this.resourceCollection[parseInt(index.toString(), 10)].field,
            data[
              this.resourceCollection[parseInt(index.toString(), 10)].idField
            ]
          );
        }
      }
  ...
  public getResourceColor(
    eventObj: Record<string, any>,
    groupOrder?: string[]
  ): string {
    const colorFieldIndex: number =
      !isNullOrUndefined(groupOrder) && this.colorIndex > groupOrder.length - 1
        ? groupOrder.length - 1
        : this.colorIndex;
    const resource: ResourcesModel =
      this.resourceCollection[parseInt(colorFieldIndex.toString(), 10)];
    if (
      isNullOrUndefined(groupOrder) &&
      this.parent.activeViewOptions.group.allowGroupEdit &&
      resource.allowMultiple
    ) {
      return undefined;
    }
    const id: string = isNullOrUndefined(groupOrder)
      ? <string>eventObj[resource.field]
      : <string>groupOrder[parseInt(colorFieldIndex.toString(), 10)];
    const data: Record<string, any>[] = this.filterData(
      resource.dataSource as Record<string, any>[],
      resource.idField,
      id
    );
    if (data.length > 0) {
  ...
  public getCssClass(eventObj: Record<string, any>): string {
    ...
    const data: Record<string, any>[] = this.filterData(
      resource.dataSource as Record<string, any>[],
      resource.idField,
      eventObj[resource.field] as string
    );
    if (data.length > 0) {
  ...
  public getResourceRenderDates(): Date[] {
    ...
    const resourceDates: Date[] = [].concat.apply(
      [],
      this.lastResourceLevel.map((e: TdData) => e.renderDates)
    );
    const time: number[] = resourceDates.map((dateObj: Date) =>
      dateObj.getTime()
    );
    const removeDuplicateDates: CallbackFunction = (dateColl: Date[]) =>
      dateColl.filter(
        (date: Date, index: number) => time.indexOf(date.getTime()) === index
      );
  ...
  private filterData(
    ...
    return dataSource.filter(
      (data: Record<string, any>) => data[`${field}`] === value
    );
  }
  ...
  public getResourceData(
    ...
    if (this.parent.activeViewOptions.group.allowGroupEdit) {
      ...
      for (const groupIndex of groupEditIndex) {
        const resourceLevel: Record<string, any>[] = this.lastResourceLevel[
          parseInt(groupIndex.toString(), 10)
        ].groupOrder as unknown as Record<string, any>[];
        for (
          let level: number = 0, length: number = resourceLevel.length;
          level < length;
          level++
        ) {
          const fieldName: string =
            this.resourceCollection[parseInt((level as number).toString(), 10)]
              .field;
          if (isNullOrUndefined(resourceObj[`${fieldName}`])) {
            resourceObj[`${fieldName}`] = [];
          }
          (<Record<string, any>[]>resourceObj[`${fieldName}`]).push(
            resourceLevel[parseInt(level.toString(), 10)]
          );
        }
      }
      ...
      for (
        let level: number = 0, length: number = this.resourceCollection.length;
        level < length;
        level++
      ) {
        if (this.lastResourceLevel[parseInt(index.toString(), 10)]) {
          eventObj[
            this.resourceCollection[parseInt(level.toString(), 10)].field
          ] =
            this.lastResourceLevel[parseInt(index.toString(), 10)].groupOrder[
              parseInt(level.toString(), 10)
            ];
        }
      }
    }
  ...
  public addResource(
    ...
    const resourceCollection: Record<string, any>[] =
      resources instanceof Array ? resources : [resources];
    for (const resource of this.parent.resourceCollection) {
      if (resource.name === name) {
        resourceCollection.forEach((addObj: Record<string, any>, i: number) =>
          new DataManager({
            json: resource.dataSource as Record<string, any>[],
  ...
  public removeResource(
    resourceId: string | string[] | number | number[],
    name: string
  ): void {
    const resourceCollection: (string | number)[] =
      resourceId instanceof Array ? resourceId : [resourceId];
    for (const resource of this.parent.resourceCollection) {
      if (resource.name === name) {
  ...
  public getIndexFromResourceId(
    id: string | number,
    name?: string,
    resourceData?: ResourcesModel,
    event?: Record<string, any>,
    parentField?: string
  ): number {
    name = name || this.parent.resourceCollection.slice(-1)[0].name;
    if (isNullOrUndefined(resourceData)) {
      resourceData = this.resourceCollection.filter(
        (e: ResourcesModel) => e.name === name
      )[0];
      if (isNullOrUndefined(resourceData)) {
        return null;
      }
    ...
    const resource: Record<string, any> = (
      resourceData.dataSource as Record<string, any>[]
    ...
    return this.lastResourceLevel
      .map((e: TdData) => e.resourceData)
      .indexOf(resource);
  }

  public resourceExpand(
    id: string | number,
    name: string,
    hide: boolean
  ): void {
    const resource: ResourcesModel = this.parent.resourceCollection.filter(
      (e: ResourcesModel) => {
        if (e.name === name) {
          return e;
        }
    ...
    const resourceData: Record<string, any> = (
      ...
      (e: Record<string, any>) => e[resource.idField] === id
    )[0] as Record<string, any>;
    if (!this.parent.activeViewOptions.group.byGroupID) {
      index = this.getIndexFromResourceId(id, name, resource);
    } else {
      index = this.lastResourceLevel
        .map((e: TdData) => e.resourceData)
        .indexOf(resourceData);
    }
    const target: HTMLElement = this.parent.element.querySelector(
      '.' +
    ...
    if (target) {
      if (target.classList.contains(cls.RESOURCE_EXPAND_CLASS) && !hide) {
        ...
        target.classList.contains(cls.RESOURCE_COLLAPSE_CLASS) &&
        hide
      ) {
        target.click();
      }
  ...
  public resourceScroll(id: string | number, name: string): void {
    if (
      ...
      ['Agenda', 'MonthAgenda'].indexOf(this.parent.currentView) > -1
    ) {
      return;
    }
    const levelName: string =
      name || this.parent.resourceCollection.slice(-1)[0].name;
    ...
    const scrollElement: HTMLElement = this.parent.element.querySelector(
    ...
    if (this.parent.activeView.isTimelineView()) {
      ...
      if (this.parent.virtualScrollModule) {
        const virtual: HTMLElement = this.parent.element.querySelector(
          '.' + cls.VIRTUAL_TRACK_CLASS
        ) as HTMLElement;
        const averageRowHeight: number = Math.round(
          virtual.offsetHeight / this.expandedResources.length
        );
        ...
        scrollElement.scrollTop =
          index * averageRowHeight -
          (this.parent.virtualScrollModule.bufferCount - 1) * averageRowHeight;
        this.parent.virtualScrollModule.virtualScrolling();
        if (this.parent.rowAutoHeight) {
          const td: HTMLElement = this.parent.element.querySelector(
      ...
      if (!this.parent.activeViewOptions.group.byGroupID) {
        ...
        if (levelName === this.parent.resourceCollection.slice(-1)[0].name) {
          index = this.lastResourceLevel
            .map(
              (e: TdData) =>
                (e.resourceData as Record<string, any>)[resource.idField]
            )
            .indexOf(id);
        } else {
          index = (resource.dataSource as Record<string, any>[])
            .map((e: Record<string, any>) => e[resource.idField])
            .indexOf(id);
        }
      }
      const offsetTarget: Element =
        this.parent.currentView === 'Month'
          ? this.parent.element.querySelector(
              `.${cls.DATE_HEADER_WRAP_CLASS} tbody tr:nth-child(${
                levelIndex + 1
              })`
            )
          : this.parent.element.querySelector(
              `.${cls.HEADER_ROW_CLASS}:nth-child(${levelIndex + 1})`
            );
      const offset: number[] = [].slice
        .call(offsetTarget.children)
        .map((node: HTMLElement) => node.offsetLeft);
  ...
  public destroy(): void {
    ...
    if (this.treeViewObj) {
      if (
        (this.treeViewObj as any).portals &&
        (this.treeViewObj as any).portals.length > 0
      ) {
        const treeViewTemplates: string[] = (
          this.treeViewObj as any
        ).portals.map((x: any) => x.propName);
    ...
    if (this.treePopup) {
      ...
      remove(
        this.parent.element.querySelector('.' + cls.RESOURCE_TREE_POPUP_OVERLAY)
      );
    }
    const resToolBarEle: Element = this.parent.element.querySelector(
      '.' + cls.RESOURCE_TOOLBAR_CONTAINER
    );
    if (resToolBarEle) {
