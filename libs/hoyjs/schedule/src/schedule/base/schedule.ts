  ...
  implements INotifyPropertyChanged
{
  ...
  private renderTableContainer(): void {
    if (!this.element.querySelector('.' + cls.TABLE_CONTAINER_CLASS)) {
      this.element.appendChild(
        this.createElement('div', { className: cls.TABLE_CONTAINER_CLASS })
      );
    }
  }
   ...
   * @private
   */
  public getElementHeightFromClass(
    element: Element,
    elementClass: string
  ): number {
    return util.getElementHeightFromClass(
      element,
      elementClass,
      this.uiStateValues.isTransformed
    );
  }
  ...
  private validateDate(selectedDate: Date = this.selectedDate): void {
    ...
    const minDate: Date = isNullOrUndefined(this.minDate)
      ? new Date(1900, 0, 1)
      : this.minDate instanceof Date
      ? new Date(this.minDate.getTime())
      : new Date(this.minDate);
    const maxDate: Date = isNullOrUndefined(this.maxDate)
      ? new Date(2099, 11, 31)
  ...
  private getViewIndex(viewName: View): number {
    for (let item: number = 0; item < this.viewCollections.length; item++) {
      const checkIndex: View =
        this.viewCollections[parseInt(item.toString(), 10)].option;
  ...
  private setViewOptions(isModuleLoad: boolean = false): void {
    ...
    for (const view of this.views) {
      ...
      const obj: ViewsData = extend(
        { option: viewName },
        isOptions ? view : {}
      );
      const fieldViewName: string =
      ...
      obj.resourceHeaderTemplateName = obj.resourceHeaderTemplate
        ? obj.option
        : '';
  ...
  private getActiveViewOptions(): ViewsData {
    ...
    const isYearView: boolean =
      this.viewCollections[this.viewIndex].option.indexOf('Year') > -1;
    const group: GroupModel = {
      byDate: isYearView ? false : this.group.byDate,
      byGroupID: this.group.byGroupID,
      allowGroupEdit: this.group.allowGroupEdit,
      resources: isNullOrUndefined(this.group.resources)
        ? []
        : this.group.resources,
      headerTooltipTemplate: this.group.headerTooltipTemplate,
      enableCompactView: this.group.enableCompactView,
      hideNonWorkingDays:
        ['Day', 'Week', 'WorkWeek', 'Month'].indexOf(this.currentView) > -1
          ? this.group.hideNonWorkingDays
          : false,
    };
    const workDays: number[] = this.viewCollections[this.viewIndex].workDays
      ? []
      : this.workDays;
    ...
    const viewsData: ViewsData = extend(
      scheduleOptions,
      viewOptions,
      undefined,
      true
    ...
    if (
      ...
      viewOptions.firstDayOfWeek &&
      this.firstDayOfWeek !== viewOptions.firstDayOfWeek
    ) {
      viewsData.firstDayOfWeek = this.firstDayOfWeek;
    }
    if (viewsData.displayDate) {
      viewsData.displayDate =
        viewsData.displayDate instanceof Date
          ? new Date(viewsData.displayDate.getTime())
          : new Date(viewsData.displayDate);
    }
    if (
      viewsData.enableLazyLoading &&
      !isNullOrUndefined(viewsData.group.resources) &&
      viewsData.group.resources.length > 0 &&
      (['Agenda', 'MonthAgenda', 'Year', 'TimelineYear'].indexOf(
        viewsData.option
      ) === -1 ||
        (viewsData.option === 'TimelineYear' &&
          viewsData.orientation === 'Vertical'))
    ) {
  ...
  private setEditorTitles(): void {
    this.editorTitles = {
      subject:
        this.eventSettings.fields.subject.title ||
        this.localeObj.getConstant('title'),
      startTime:
        this.eventSettings.fields.startTime.title ||
        this.localeObj.getConstant('start'),
      endTime:
        this.eventSettings.fields.endTime.title ||
        this.localeObj.getConstant('end'),
      isAllDay:
        this.eventSettings.fields.isAllDay.title ||
        this.localeObj.getConstant('allDay'),
      startTimezone:
        this.eventSettings.fields.startTimezone.title ||
        this.localeObj.getConstant('startTimezone'),
      endTimezone:
        this.eventSettings.fields.endTimezone.title ||
        this.localeObj.getConstant('endTimezone'),
      location:
        this.eventSettings.fields.location.title ||
        this.localeObj.getConstant('location'),
      description:
        this.eventSettings.fields.description.title ||
        this.localeObj.getConstant('description'),
      recurrenceRule:
        this.eventSettings.fields.recurrenceRule.title ||
        this.localeObj.getConstant('repeat'),
    };
  }
  ...
  private initializeTemplates(): void {
    this.cellHeaderTemplateFn = this.templateParser(
      this.activeViewOptions.cellHeaderTemplate
    );
    this.dayHeaderTemplateFn = this.templateParser(
      this.activeViewOptions.dayHeaderTemplate
    );
    this.monthHeaderTemplateFn = this.templateParser(
      this.activeViewOptions.monthHeaderTemplate
    );
    this.cellTemplateFn = this.templateParser(
      this.activeViewOptions.cellTemplate
    );
    this.dateHeaderTemplateFn = this.templateParser(
      this.activeViewOptions.dateHeaderTemplate
    );
    this.dateRangeTemplateFn = this.templateParser(
      this.activeViewOptions.dateRangeTemplate
    );
    this.majorSlotTemplateFn = this.templateParser(
      this.activeViewOptions.timeScale.majorSlotTemplate
    );
    this.minorSlotTemplateFn = this.templateParser(
      this.activeViewOptions.timeScale.minorSlotTemplate
    );
    this.appointmentTemplateFn = this.templateParser(
      this.activeViewOptions.eventTemplate
    );
    this.resourceHeaderTemplateFn = this.templateParser(
      this.activeViewOptions.resourceHeaderTemplate
    );
    this.headerIndentTemplateFn = this.templateParser(
      this.activeViewOptions.headerIndentTemplate
    );
    this.headerTooltipTemplateFn = this.templateParser(
      this.activeViewOptions.group.headerTooltipTemplate
    );
    this.eventTooltipTemplateFn = this.templateParser(
      this.eventSettings.tooltipTemplate
    );
    this.editorTemplateFn = this.templateParser(this.editorTemplate);
    this.editorHeaderTemplateFn = this.templateParser(
      this.editorHeaderTemplate
    );
    this.editorFooterTemplateFn = this.templateParser(
      this.editorFooterTemplate
    );
    this.quickInfoTemplatesHeaderFn = this.templateParser(
      this.quickInfoTemplates.header
    );
    this.quickInfoTemplatesContentFn = this.templateParser(
      this.quickInfoTemplates.content
    );
    this.quickInfoTemplatesFooterFn = this.templateParser(
      this.quickInfoTemplates.footer
    );
  }
  ...
  public getDayNames(type: string): string[] {
    ...
    if (
      isNullOrUndefined(this.locale) ||
      this.locale === 'en' ||
      this.locale === 'en-US'
    ) {
      nameSpace = 'days.stand-alone.';
      cldrObj = <string[]>(
        getValue(nameSpace + type, getDefaultDateObject(this.getCalendarMode()))
      );
    } else {
      nameSpace =
        'main.' +
  ...
  public getCalendarMode(): string {
    return !isNullOrUndefined(this.calendarMode)
      ? this.calendarMode.toLowerCase()
      : 'gregorian';
  }
  ...
  public changeView(
    ...
    const previousView: View = this.activeViewOptions
      ? this.activeViewOptions.option
      : this.currentView;
    ...
    this.trigger(events.actionBegin, args, (actionArgs: ActionEventArgs) => {
      if (!actionArgs.cancel) {
        ...
        this.trigger(
          events.navigating,
          navArgs,
          (navigationArgs: NavigatingEventArgs) => {
            if (!navigationArgs.cancel) {
              const isVertical: boolean =
                ['Day', 'Week', 'WorkWeek'].indexOf(view) > -1 &&
                ['Day', 'Week', 'WorkWeek'].indexOf(previousView) < 0;
              this.uiStateValues.isInitial =
                isVertical ||
                view.indexOf('Timeline') > -1 ||
                view.indexOf('Year') > -1;
              this.uiStateValues.top =
                view.indexOf('Timeline') > -1 &&
                previousView.indexOf('Timeline') < 0
                  ? 0
                  : this.uiStateValues.top;
      ...
      if (!actionArgs.cancel) {
        ...
        this.trigger(
          ...
          (navigationArgs: NavigatingEventArgs) => {
            if (!navigationArgs.cancel) {
              this.uiStateValues.isInitial =
                this.activeView.isTimelineView() &&
                this.currentView !== 'TimelineYear';
              this.validateDate(navigationArgs.currentDate);
              if (this.headerModule) {
                this.headerModule.setCalendarDate(navigationArgs.currentDate);
              }
              if (
                this.currentView === 'MonthAgenda' &&
                this.monthAgendaModule
              ) {
                this.monthAgendaModule.monthAgendaDate = new Date(
                  '' + this.selectedDate
                );
              }
              this.initializeView(this.currentView);
  ...
  public getCurrentTime(date: Date = new Date()): Date {
    if (this.timezone) {
      return this.tzModule.convert(
        date,
        this.tzModule.getLocalTimezoneName(),
        this.timezone
      );
    }
    return date;
  ...
  public getNavigateView(): View {
    if (this.activeView.isTimelineView()) {
      return this.currentView === 'TimelineMonth' ||
        this.currentView === 'TimelineYear'
        ? 'TimelineDay'
        : 'Agenda';
    }
  ...
  protected preRender(): void {
    ...
    if (
      (this && isNullOrUndefined(this.uiStateValues)) ||
  ...
  private getDefaultLocale(): void {
    this.defaultLocale = {
      ...
      seriesChangeAlert:
        'Do you want to cancel the changes made to specific ' +
        'instances of this series and match it to the whole series again?',
      createError:
        'The duration of the event must be shorter than how frequently it occurs. ' +
      ...
      occurenceAlert:
        'Cannot reschedule an occurrence of the recurring appointment if it skips over ' +
  ...
  public removeSelectedClass(): void {
    ...
    if (
      this.keyboardInteractionModule &&
      this.keyboardInteractionModule.selectedCells.length > 0
    ) {
   ...
   * @private
   */
  public addSelectedClass(
    cells: HTMLTableCellElement[],
    focusCell: HTMLTableCellElement,
  ...
  public getWorkCellElements(): Element[] {
    return [].slice.call(
      this.element.querySelectorAll('.' + cls.WORK_CELLS_CLASS)
    );
  }
  ...
  public isAllDayCell(td: Element): boolean {
    if (
      ['Month', 'TimelineMonth', 'TimelineYear', 'MonthAgenda'].indexOf(
        this.currentView
      ) > -1 ||
  ...
  public removeNewEventElement(): void {
    const eventClone: HTMLElement = this.element.querySelector(
      '.' + cls.NEW_EVENT_CLASS
    );
  ...
  public getStartEndTime(startEndTime: string): Date {
    if (!isNullOrUndefined(startEndTime) && startEndTime !== '') {
      const startEndDate: Date = util.resetTime(
        new Date(this.currentTimezoneDate) || this.getCurrentTime()
      );
      const timeString: string[] = startEndTime.split(':');
      if (timeString.length === 2) {
        startEndDate.setHours(
  ...
  private onScheduleResize(): void {
    ...
    if (
      ...
      if (
        this.activeViewOptions.enableLazyLoading &&
        this.virtualScrollModule &&
        this.virtualScrollModule.isRemoteRefresh
      ) {
      ...
      if (
        this.virtualScrollModule &&
        !this.virtualScrollModule.enableTransition
      ) {
        const resWrap: HTMLElement = this.element.querySelector(
          '.' + cls.RESOURCE_COLUMN_WRAP_CLASS
        ) as HTMLElement;
        const conWrap: HTMLElement = this.element.querySelector(
  ...
  public getSelectedCells(): Element[] {
    return [].slice.call(
      this.element.querySelectorAll('.' + cls.SELECTED_CELL_CLASS)
    );
  }
  ...
  public getAnnouncementString(
    ...
    if (this.quickPopup && this.activeViewOptions.group.resources.length > 0) {
      ...
      resourceName =
        this.quickPopup.getResourceText(
          { event: event } as EventClickArgs,
          'event'
        ) + constantText;
    }
    const recordSubject: string = (subject ||
      event[this.eventFields.subject] ||
      this.eventSettings.fields.subject.default) as string;
    const recordLocation: string = (event[this.eventFields.location] ||
      this.eventSettings.fields.location.default) as string;
    const skeleton: string = 'full';
    const startDateText: string = this.globalize.formatDate(
      event[this.eventFields.startTime] as Date,
      {
        type: 'dateTime',
        skeleton: skeleton,
        calendar: this.getCalendarMode(),
    ...
    const endDateText: string = this.globalize.formatDate(
      event[this.eventFields.endTime] as Date,
      {
        type: 'dateTime',
        skeleton: skeleton,
        calendar: this.getCalendarMode(),
    ...
    let announcementString: string =
      recordSubject +
      ' ' +
      this.localeObj.getConstant('beginFrom') +
      ' ' +
      startDateText +
      ' ' +
      this.localeObj.getConstant('endAt') +
      ' ' +
      endDateText;
    if (resourceName) {
    ...
    if (
      event[this.eventFields.recurrenceRule] &&
      event[this.eventFields.recurrenceRule] !== '' &&
      event[this.eventFields.id] === event[this.eventFields.recurrenceID]
    ) {
      announcementString =
        announcementString + ' ' + this.localeObj.getConstant('recurringEvent');
    }
  ...
  public boundaryValidation(pageY: number, pageX: number): ResizeEdges {
    ...
    const scrollEdges: ResizeEdges = {
      left: false,
      right: false,
      top: false,
      bottom: false,
    };
    const viewBoundaries: ClientRect = this.element
      .querySelector('.' + cls.CONTENT_WRAP_CLASS)
      .getBoundingClientRect();
    if (
      pageY < viewBoundaries.top + autoScrollDistance + window.pageYOffset &&
      pageY > viewBoundaries.top + window.pageYOffset
    ) {
      scrollEdges.top = true;
    }
    if (
      pageY > viewBoundaries.bottom - autoScrollDistance + window.pageYOffset &&
      pageY < viewBoundaries.bottom + window.pageYOffset
    ) {
      scrollEdges.bottom = true;
    }
    if (
      pageX < viewBoundaries.left + autoScrollDistance + window.pageXOffset &&
      pageX > viewBoundaries.left + window.pageXOffset
    ) {
      scrollEdges.left = true;
    }
    if (
      pageX > viewBoundaries.right - autoScrollDistance + window.pageXOffset &&
  ...
  public getWeekNumberContent(dates: Date[]): string {
    ...
    if (this.weekRule === 'FirstDay') {
      const weekNumberDate: Date = util.getWeekLastDate(
        dates.slice(-1)[0],
        this.firstDayOfWeek
      );
      weekNumber = this.globalize.formatNumber(
        util.getWeekNumber(weekNumberDate)
      );
    } else if (this.weekRule === 'FirstFourDayWeek') {
      const weekFirstDate: Date = util.getWeekFirstDate(
        dates.slice(-1)[0],
        this.firstDayOfWeek
      );
      const weekLastDate: Date = util.getWeekLastDate(
        dates.slice(-1)[0],
        this.firstDayOfWeek
      );
      const weekMidDate: Date = util.getWeekMiddleDate(
        weekFirstDate,
        weekLastDate
      );
      ...
      weekNumber = this.globalize.formatNumber(
        util.getWeekNumber(weekFirstDate)
      );
    }
    return weekNumber;
  }
  ...
  public isSpecificResourceEvents(): boolean {
    return (
      this.activeViewOptions.group.resources.length > 0 &&
      !this.activeViewOptions.group.allowGroupEdit &&
      !this.rowAutoHeight &&
      !this.virtualScrollModule &&
      this.activeViewOptions.group.byGroupID
    );
  }
  ...
  public onPropertyChanged(
    newProp: ScheduleModel,
    oldProp: ScheduleModel
  ): void {
    ...
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        case 'workDays':
          if (
            JSON.stringify(oldProp.workDays) !==
            JSON.stringify(newProp.workDays)
          ) {
            state.isLayout = true;
          }
          break;
        ...
        case 'resourceHeaderTemplate':
          this.activeViewOptions.resourceHeaderTemplate =
            newProp.resourceHeaderTemplate;
          this.resourceHeaderTemplateFn = this.templateParser(
            this.activeViewOptions.resourceHeaderTemplate
          );
          state.isLayout = true;
  ...
  private allDayRowScrollUpdate(): void {
    const dateHeader: HTMLElement = this.element.querySelector(
      '.' + cls.DATE_HEADER_WRAP_CLASS
    ) as HTMLElement;
    const allDayRow: HTMLElement = this.element.querySelector(
    ...
    if (this.height === 'auto' || !this.enableAllDayScroll) {
      ...
      if (this.uiStateValues.expand) {
        const allDayCells: HTMLElement[] = [].slice.call(
          this.element.querySelectorAll('.' + cls.ALLDAY_CELLS_CLASS)
        );
        allDayCells[0].style.height = allDayRow.style.height
          ? allDayRow.style.height
          : allDayCells[1].style.height;
      }
    } else {
  ...
  private extendedPropertyChange(
    ...
    switch (prop) {
      ...
      case 'agendaDaysCount':
        ...
        break;
      case 'eventSettings':
        this.onEventSettingsPropertyChanged(
          newProp.eventSettings,
          oldProp.eventSettings,
          state
        );
        break;
      ...
      case 'quickInfoTemplates':
        ...
        break;
      case 'group':
        this.onGroupSettingsPropertyChanged(
          newProp.group,
          oldProp.group,
          state
        );
      ...
      case 'resources':
        ...
        break;
      case 'timeScale':
        this.activeViewOptions.timeScale.interval =
          newProp.timeScale.interval ||
          this.activeViewOptions.timeScale.interval;
        this.activeViewOptions.timeScale.slotCount =
          newProp.timeScale.slotCount ||
          this.activeViewOptions.timeScale.slotCount;
        if (this.eventWindow) {
      ...
      case 'monthsCount':
        ...
        break;
      case 'timeFormat':
        this.internalTimeFormat =
          newProp.timeFormat || this.activeViewOptions.timeFormat;
        if (this.eventWindow) {
          this.eventWindow.refreshDateTimePicker();
        }
        state.isLayout = true;
  ...
  private onEventSettingsPropertyChanged(
    newProp: EventSettingsModel,
    oldProp: EventSettingsModel,
    state: StateArgs
  ): void {
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
   ...
   * @returns {void}
   */
  public resetWorkHours(
    dates: Date[] = this.activeView.renderDates,
    start?: string,
    end?: string,
    groupIndex?: number
  ): void {
    if (dates && start && end) {
      const cells: HTMLTableCellElement[] = this.getWorkHourCells(
        dates,
        start,
        end,
        groupIndex
      ...
      const workHourCells: HTMLElement[] = [].slice.call(
        this.element.querySelectorAll('.' + cls.WORK_HOURS_CLASS)
      );
  ...
  private getWorkHourCells(
    ...
    if (
      ['Agenda', 'MonthAgenda', 'Month', 'TimelineMonth'].indexOf(
        this.currentView
      ) > -1
    ) {
      return [];
    ...
    const tableEle: HTMLTableElement =
      this.getContentTable() as HTMLTableElement;
    if (
      isNullOrUndefined(startHour) ||
      isNullOrUndefined(endHour) ||
      !tableEle
    ) {
      return [];
    }
    ...
    const msMajorInterval: number =
      this.activeViewOptions.timeScale.interval * util.MS_PER_MINUTE;
    const msInterval: number =
      msMajorInterval / this.activeViewOptions.timeScale.slotCount;
    const offsetDiff: number =
      (viewStartHour.getTimezoneOffset() - startHour.getTimezoneOffset()) *
      util.MS_PER_MINUTE;
    let startIndex: number = Math.round(
      (startHour.getTime() - viewStartHour.getTime() + offsetDiff) / msInterval
    );
    let endIndex: number = Math.ceil(
      (endHour.getTime() - viewStartHour.getTime() + offsetDiff) / msInterval
    );
    ...
    for (let date of dates) {
      ...
      if (
        ...
        renderDates =
          this.resourceBase.lastResourceLevel[
      ...
      if (colIndex >= 0) {
        if (this.activeView.isTimelineView()) {
          const slotsPerDay: number = Math.round(
            (viewEndHour.getTime() - viewStartHour.getTime()) / msInterval
          );
        ...
        for (let i: number = startIndex; i < endIndex; i++) {
          if (this.activeView.isTimelineView()) {
            const rowIndex: number = !isNullOrUndefined(groupIndex)
              ? groupIndex
              : 0;
            cells.push(
              tableEle.rows[parseInt(rowIndex.toString(), 10)].cells[
            ...
            if (!isNullOrUndefined(groupIndex)) {
              const selector: string =
                ...
                groupIndex +
                '"]';
              const tds: HTMLTableCellElement[] = [].slice.call(
                tableEle.rows[parseInt(i.toString(), 10)].querySelectorAll(
                  selector
                )
              );
              ...
              cells.push(
                tableEle.rows[parseInt(i.toString(), 10)].cells[
  ...
  public getCellDetails(tdCol: Element | Element[]): CellClickEventArgs {
    ...
    const endDateFromColSpan: boolean =
      this.activeView.isTimelineView() &&
      !isNullOrUndefined(lastTd.getAttribute('colSpan')) &&
      this.headerRows.length > 0;
    const duration: number = endDateFromColSpan
      ? parseInt(lastTd.getAttribute('colSpan'), 10)
      : 1;
    if (
      !this.activeViewOptions.timeScale.enable ||
      endDateFromColSpan ||
      lastTd.classList.contains(cls.ALLDAY_CELLS_CLASS) ||
   ...
   * @returns {void}
   */
  public setResourceCollections(
    resourceCol: ResourcesModel[],
    isEventDataRefresh: boolean = true
  ): void {
  ...
  public getResourcesByIndex(index: number): ResourceDetails {
    if (this.resourceBase && this.resourceBase.lastResourceLevel) {
      ...
      const data: TdData =
        this.resourceBase.lastResourceLevel[parseInt(index.toString(), 10)];
  ...
  public scrollTo(hour: string, scrollDate?: Date): void {
    if (
      this.currentView.indexOf('Agenda') < 0 &&
      isNullOrUndefined(this.element.querySelector('.e-work-cells'))
    ) {
      return;
    }
  ...
  public exportToICalendar(
    ...
    if (this.iCalendarExportModule) {
      ...
      console.warn(
        '[WARNING] :: Module "ICalendarExport" is not available in Schedule component!' +
  ...
  public importICalendar(fileContent: Blob | string): void {
    if (this.iCalendarImportModule) {
      ...
      console.warn(
        '[WARNING] :: Module "ICalendarImport" is not available in Schedule component!' +
          ' You either misspelled the module name or forgot to load it.'
      );
   ...
   */
  public generateEventOccurrences(
    event: Record<string, any>,
    startDate?: Date
  ): Record<string, any>[] {
    return this.eventBase
      ? this.eventBase.generateOccurrence(event, startDate)
      : [];
  }
  ...
  public exportToExcel(excelExportOptions?: ExportOptions): void {
    if (this.excelExportModule) {
      ...
      console.warn(
        '[WARNING] :: Module "ExcelExport" is not available in Schedule component!' +
          ' You either misspelled the module name or forgot to load it.'
      );
  ...
  public print(printOptions?: ScheduleModel): void {
    if (this.printModule) {
      ...
      console.warn(
        '[WARNING] :: Module "Print" is not available in Schedule component!' +
          ' You either misspelled the module name or forgot to load it.'
      );
   ...
   */
  public deleteEvent(
    id: string | number | Record<string, any> | Record<string, any>[],
    currentAction?: CurrentAction
  ): void {
  ...
  public getEvents(
    ...
    eventCollections = this.eventBase.filterEventsByRange(
      eventCollections,
      startDate,
      endDate
    );
  ...
  public isSlotAvailable(
    startTime: Date | Record<string, any>,
    endTime?: Date,
    groupIndex?: number
  ): boolean {
    let eventStart: Date;
    let eventEnd: Date;
    let eventObj: Record<string, any> = this.activeEventData.event as Record<
      string,
      any
    >;
    if (startTime instanceof Date || typeof startTime === 'string') {
      ...
      eventStart = (<Record<string, any>>startTime)[
        this.eventFields.startTime
      ] as Date;
      eventEnd = (<Record<string, any>>startTime)[
        this.eventFields.endTime
      ] as Date;
      if (this.resourceBase) {
        groupIndex = this.eventBase.getGroupIndexFromEvent(
          startTime as Record<string, any>
        );
      }
    }
    ...
    let eventCollection: Record<string, any>[] = this.eventBase.filterEvents(
      eventStart,
      eventEnd
    );
    if (
      !isNullOrUndefined(groupIndex) &&
      this.resourceBase &&
      this.resourceBase.lastResourceLevel.length > 0
    ) {
      eventCollection = this.eventBase.filterEventsByResource(
        this.resourceBase.lastResourceLevel[
          parseInt(groupIndex.toString(), 10)
        ],
        eventCollection
    ...
    if (eventObj) {
      if (eventObj.Guid) {
        eventCollection = eventCollection.filter(
          (event: Record<string, any>) => event.Guid !== eventObj.Guid
        );
      } else {
        eventCollection = eventCollection.filter(
          (event: Record<string, any>) =>
  ...
  public openEditor(
    ...
    if (action === 'Add' && !isEventData) {
      ...
      data[this.eventFields.startTime] = this.getDateTime(
        <Date>data[this.eventFields.startTime]
      );
      data[this.eventFields.endTime] = this.getDateTime(
        <Date>data[this.eventFields.endTime]
      );
    }
  ...
  public openQuickInfoPopup(data: Record<string, any>): void {
    ...
    if (isNullOrUndefined(data[this.eventFields.id])) {
      ...
      const cellData: CellClickEventArgs = {
        startTime: (this.activeCellsData.startTime = this.getDateTime(
          <Date>data[this.eventFields.startTime]
        )),
        endTime: (this.activeCellsData.endTime = this.getDateTime(
          <Date>data[this.eventFields.endTime]
        )),
        isAllDay: (this.activeCellsData.isAllDay = !isNullOrUndefined(
          data[this.eventFields.isAllDay]
        )
          ? data[this.eventFields.isAllDay]
          : false),
      };
      const startTime: Date = this.activeView.getAdjustedDate(
      ...
      if (startTime) {
        ...
        if (
          this.activeViewOptions.group.resources.length > 0 &&
          !this.uiStateValues.isGroupAdaptive &&
          this.resourceBase &&
          this.eventBase
        ) {
          cellData.groupIndex = this.eventBase.getGroupIndexFromEvent(
      ...
      const app: Record<string, any>[] = this.getCurrentViewEvents().filter(
      ...
      if (data[this.eventFields.recurrenceRule]) {
        const occurence: Record<string, any> = app.filter(
          (x: Record<string, any>) =>
            x[this.eventFields.startTime].getTime() ===
            data[this.eventFields.startTime].getTime()
        );
      ...
      const element: HTMLElement = this.element.querySelector(
        'div[data-guid="' + selectEvent.Guid + '"]'
      );
      if (element) {
        ...
        if (
          ...
          addClass(
            [this.activeEventData.element as Element],
