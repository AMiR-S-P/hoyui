...
import { createElement, remove, isNullOrUndefined } from '@hoyui/base';
 ...
 */
export function getElementHeightFromClass(
  container: Element,
  elementClass: string,
 ...
 */
export function getElementWidthFromClass(
  container: Element,
  elementClass: string,
  isTransformed?: boolean
): number {
...
export function getTranslateY(element: HTMLElement | Element): number {
  ...
  return (<Record<string, any> & Window>(<unknown>window)).WebKitCSSMatrix
    ? new WebKitCSSMatrix(style.webkitTransform).m42
    : 0;
}
...
export function getTranslateX(element: HTMLElement | Element): number {
  const style: CSSStyleDeclaration = getComputedStyle(element);
  return (<Record<string, any> & Window>(<unknown>window)).WebKitCSSMatrix
    ? new WebKitCSSMatrix(style.webkitTransform).m41
    : 0;
}
...
export function getWeekFirstDate(date: Date, firstDayOfWeek: number): Date {
  ...
  return new Date(
    date1.setDate(
      date1.getDate() + (isNaN(firstDayOfWeek) ? 0 : firstDayOfWeek)
    )
  );
}
...
export function getWeekLastDate(date: Date, firstDayOfWeek: number): Date {
  ...
  const weekLast: Date = new Date(
    weekFirst.getFullYear(),
    weekFirst.getMonth(),
    weekFirst.getDate() + 6
  );
...
export function getWeekMiddleDate(weekFirst: Date, weekLast: Date): Date {
  return new Date(
    weekLast.valueOf() - (weekLast.valueOf() - weekFirst.valueOf()) / 2
  );
}
...
export function getDateInMs(date: Date): number {
  const localOffset: number = new Date(
    date.getFullYear(),
    date.getMonth(),
    date.getDate(),
    0,
...
export function getDateFromString(date: string): Date {
  return date.indexOf('Date') !== -1
    ? new Date(parseInt(date.match(/\d+/g).toString(), 10))
    : date.indexOf('T') !== -1
    ? new Date(date)
...
export function getScrollBarWidth(): number {
  ...
  const ratio: number = devicePixelRatio
    ? devicePixelRatio.toFixed(2) === '1.10' || devicePixelRatio <= 1
      ? Math.ceil(devicePixelRatio % 1)
      : Math.floor(devicePixelRatio % 1)
    : 0;
...
export function findIndexInData(
  ...
  resourceCollection?: Record<string, any>[]
): number {
        ...
        if (
          resData.includes(
            data[parseInt(i.toString(), 10)][
              (resourceCollection.slice(-1)[0] as any).groupIDField
            ] as string
          )
        ) {
          return i;
...
export function removeChildren(element: HTMLElement | Element): void {
  const elementChildren: HTMLElement[] | Element[] = [].slice.call(
    element.children
  );
...
export function capitalizeFirstWord(inputString: string, type: string): string {
  if (type === 'multiple') {
    inputString = inputString
      .split(' ')
      .map((e: string) => e.charAt(0).toLocaleUpperCase() + e.substring(1))
      .join(' ');
  } else if (type === 'single') {
    if (inputString[0] >= '0' && inputString[0] <= '9') {
      const array: RegExpMatchArray = inputString.match(/[a-zA-Z]/);
      inputString = isNullOrUndefined(array)
        ? inputString
        : inputString.slice(0, array.index) +
          inputString[array.index].toLocaleUpperCase() +
          inputString.slice(array.index + 1);
    }
