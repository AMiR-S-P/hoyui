...
import { HijriParser, isNullOrUndefined } from '@hoyui/base';
...
export class Islamic implements CalendarUtil {
  public firstDateOfMonth(date: Date): Date {
    ...
    const gDate: Date = HijriParser.toGregorian(
      hDate.year as number,
      hDate.month as number,
      1
    );
    return gDate;
  }
  ...
  public setValidDate(
    ...
    const firstDate: Date = !isNullOrUndefined(beginDate)
      ? this.firstDateOfMonth(beginDate)
      : date;
    const hDate: Record<string, any> = this.getHijriDate(firstDate);
    const gDate: Date = HijriParser.toGregorian(
      hDate.year as number,
      (hDate.month as number) + interval,
      startDate
    );
