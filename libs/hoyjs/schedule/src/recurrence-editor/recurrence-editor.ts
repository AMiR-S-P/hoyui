...
const MONTHEXPANDERCHECKBOXWRAPPER: string =
  'e-month-expander-checkbox-wrapper';
  ...
  implements INotifyPropertyChanged
{
    ...
    if (this.renderStatus) {
      ...
      this.trigger(
        ...
        (args: { [key: string]: string }) =>
          this.setProperties({ value: args.value }, false)
      );
    }
  }
  private resetDayButton(): void {
    ...
    elements.forEach((element: HTMLElement) =>
      removeClass([element], [ACTIVE, PRIMARY])
    );
  }
    ...
    switch (state) {
      case NONE:
        neverClassList.forEach((className: string) =>
          addClass([this.element.querySelector('.' + className)], HIDEWRAPPER)
        );
        break;
      case WEEKLY:
        weekClassList.forEach((className: string) =>
          addClass([this.element.querySelector('.' + className)], HIDEWRAPPER)
        );
        break;
      case MONTHLY:
        monthClassList.forEach((className: string) =>
          addClass([this.element.querySelector('.' + className)], HIDEWRAPPER)
        );
        break;
      case YEARLY:
        yearClassList.forEach((className: string) =>
          addClass([this.element.querySelector('.' + className)], HIDEWRAPPER)
        );
        break;
      case DAILY:
        dailyClassList.forEach((className: string) =>
          addClass([this.element.querySelector('.' + className)], HIDEWRAPPER)
        );
        break;
    }
  ...
  private updateEndOnForm(state: string): void {
    ...
    switch (state) {
      case NEVER:
        ...
        break;
      case UNTIL:
        endOnDateClassList.forEach((className: string) =>
          addClass([this.element.querySelector('.' + className)], HIDEWRAPPER)
        );
        break;
      case COUNT:
        endOnCountClassList.forEach((className: string) =>
          addClass([this.element.querySelector('.' + className)], HIDEWRAPPER)
        );
        break;
    }
  ...
  private showFormElement(): void {
    neverClassList.forEach((className: string) => {
      const hideElement: HTMLElement = this.element.querySelector(
        '.' + className
      );
      if (hideElement) {
  ...
  private renderDropdowns(): void {
    ...
    this.repeatType = new DropDownList({
      ...
      change: (args: ChangeEventArgs) => {
        self.setProperties(
          { selectedType: this.frequencies.indexOf(args.value as RepeatType) },
          false
        );
        self.element.querySelector('.' + REPEATCONTENT).innerHTML =
          self.localeObj.getConstant(
            contentType[args.value as number | string]
          );
    ...
    this.repeatType.appendTo(
      <HTMLElement>this.element.querySelector('.' + REPEATELEMENT)
    );
    ...
    this.endType.appendTo(
      <HTMLElement>this.element.querySelector('.' + ENDONELEMENT)
    );

    const renderDropDownList: Function = (
      dropDownData: [{ [key: string]: string | number }]
    ) => {
    ...
    this.monthWeekPos.appendTo(
      <HTMLElement>this.element.querySelector('.' + MONTHPOS)
    );

    this.monthWeekDays = renderDropDownList(this.getDayData('wide'));
    this.monthWeekDays.appendTo(
      <HTMLElement>this.element.querySelector('.' + MONTHWEEK)
    );
    ...
    this.monthValue.appendTo(
      <HTMLElement>this.element.querySelector('.' + YEAREXPANDERELEMENT)
    );
  }
  ...
  private setDefaultValue(): void {
    ...
    for (const element of formelement) {
      const instance: NumericTextBox = (element as EJ2Instance)
  ...
  private resetFormValues(): void {
    const recurreneElement: HTMLElement[] = [].slice.call(
  ...
  private renderDatePickers(): void {
    ...
    this.untilDateObj.appendTo(
      <HTMLElement>this.element.querySelector('.' + UNTILDATE)
    );
  }
  ...
  private dayButtonRender(): void {
    const btns: HTMLButtonElement[] = [].slice.call(
      this.element.querySelectorAll('.' + DAYWRAPPER + ' button')
    );
    // eslint-disable-next-line @typescript-eslint/no-this-alias
    const self: RecurrenceEditor = this;
    for (const btn of btns) {
      ...
      EventHandler.add(btn, 'click', (args: MouseEventArgs) => {
        const btns: HTMLElement[] = [].slice.call(
  ...
  private radioButtonRender(): void {
    ...
    this.onMonthDay.appendTo(
      <HTMLElement>this.element.querySelector('.' + MONTHEXPANDERELEMENT)
    );
    ...
    this.onWeekDay.appendTo(
      <HTMLElement>this.element.querySelector('.' + MONTHEXPANNDERELEM)
    );
  ...
  private numericTextboxRender(): void {
    ...
    this.recurrenceCount.appendTo(
      <HTMLElement>this.element.querySelector('.' + ENDONCOUNT)
    );
    ...
    this.monthDate.appendTo(
      <HTMLElement>this.element.querySelector('.' + MONTHDAYWRAPPER)
    );
    ...
    this.repeatInterval.appendTo(
      <HTMLElement>this.element.querySelector('.' + REPEATINTERVAL)
    );
  }
  ...
  private getMonthData(): { [key: string]: string }[] {
    ...
    if (this.locale === 'en' || this.locale === 'en-US') {
      ...
      cldrObj = <string[]>(
        getValue(nameSpaceString, getDefaultDateObject(this.getCalendarMode()))
      );
    } else {
      const nameSpaceString: string =
        'main.' +
  ...
  private setTemplate(): void {
    ...
    this.element.innerHTML =
      ...
      '"  role="none"><tr>' +
      '<td><input type="text" tabindex="0" id="' +
      this.element.id +
      '_' +
      REPEATINTERVAL +
      '" class="' +
      REPEATINTERVAL +
      '"title="' +
      this.localeObj.getConstant('repeatEvery') +
      '" /></td>' +
      '<td><span class="' +
      REPEATCONTENT +
      '"></span></td>' +
  ...
  private getSelectedDaysData(): string {
    ...
    for (let index: number = 0; index < elements.length; index++) {
      ruleData +=
        weekday[
          parseInt(
            elements[parseInt(index.toString(), 10)].getAttribute('data-index'),
            10
          )
  ...
  private updateWeekButton(keys: string[]): void {
    ...
    for (const obj of this.dayButtons) {
      const index: number = parseInt(
        obj.element.getAttribute('data-index'),
        10
      );
  ...
  private getUntilData(): string {
    ...
    const tempStr: string = getRecurrenceStringFromDate(
      this.untilDateObj.value
    );
  ...
  public setRecurrenceRule(
    ...
    const endon: string = this.ruleObject.count
      ? COUNT
  ...
  private detachInputs(): void {
    const inputElements: HTMLInputElement[] = [].slice.call(
  ...
  public onPropertyChanged(
    newProp: RecurrenceEditorModel,
    oldProp: RecurrenceEditorModel
  ): void {
