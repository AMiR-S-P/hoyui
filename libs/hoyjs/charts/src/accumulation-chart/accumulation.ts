 ...
 */
@NotifyPropertyChanges
export class AccumulationChart
  ...
  implements INotifyPropertyChanged
{
  ...
  @Collection<AccumulationAnnotationSettingsModel>(
    [{}],
  ...
  public animate(duration?: number): void {
    this.duration =
      duration === 0 && animationMode === 'Enable' ? 700 : duration;
    ...
    if (
      currentSeries.points.length < Object.keys(currentSeries.dataSource).length
    ) {
      this.refresh();
    } else if (
      currentSeries.points.length > Object.keys(currentSeries.dataSource).length
      ...
      const currentSeries: AccumulationSeries = this.visibleSeries[0];
      currentSeries.points = currentSeries.points.filter(
        (entry1: AccPoints) => {
      ...
      const missing: number[] = tempindex.filter(
        (item: number) => tempindex1.indexOf(item) < 0
      );
      ...
      for (let i: number = 0; i < currentSeries.points.length; i++) {
        currentSeries.points[i as number].y =
          currentSeries.dataSource[i as number].y;
      ...
      for (let i: number = 0; i < currentSeries.points.length; i++) {
        currentSeries.points[i as number].y =
          currentSeries.dataSource[i as number][currentSeries.yName];
        currentSeries.points[i as number].color =
          currentSeries.dataSource[i as number][
            currentSeries.pointColorMapping
          ] != null
            ? currentSeries.dataSource[i as number][
                currentSeries.pointColorMapping
              ]
            : currentSeries.points[i as number].color;
        currentSeries.sumOfPoints +=
          currentSeries.dataSource[i as number][currentSeries.yName];
      }
      this.redraw = this.enableAnimation;
      this.animateSeries = false;
   ...
   */
  constructor(
    options?: AccumulationChartModel,
  ...
  protected preRender(): void {
    ...
    if (this.element.id === '') {
      const collection: number = document.getElementsByClassName(
        'e-accumulationchart'
      ).length;
  ...
  protected render(): void {
    ...
    this.element.setAttribute(
      'aria-label',
      this.title + '. Syncfusion interactive chart.'
    );
    this.element.setAttribute(
      'class',
      this.element.getAttribute('class') + ' e-accumulationchart-focused'
    );
  ...
  private unWireEvents(): void {
    ...
    EventHandler.remove(
      this.element,
      'dblclick',
      this.accumulationOnDoubleClick
    );
    EventHandler.remove(
      this.element,
      'contextmenu',
      this.accumulationRightClick
    );
  ...
  public accumulationMouseEnd(e: PointerEvent): boolean {
    ...
    if (this.isTouch) {
      ...
      if (
        this.accumulationDataLabelModule &&
        this.visibleSeries[0].dataLabel.visible
      ) {
        this.accumulationDataLabelModule.move(
          e,
          this.mouseX,
          this.mouseY,
          this.isTouch
  ...
  public accumulationResize(): boolean {
    ...
    const beforeResizeArgs: IAccBeforeResizeEventArgs = {
      name: 'beforeResize',
  ...
  public print(id?: string[] | string | Element): void {
    ...
    const clippath: string = document.getElementById(
      this.element.id + '_Series_0'
    ).style.clipPath;
  ...
  private setStyle(element: HTMLElement): void {
    ...
    style.innerText =
      '.e-accumulationchart-focused:focus,path[id*=_Series_0_Point_]:focus, text[id*=_title]:focus' +
  ...
  public setAnnotationValue(annotationIndex: number, content: string): void {
    const annotation: AccumulationAnnotationSettings = <
      AccumulationAnnotationSettings
  ...
  public accumulationMouseMove(e: PointerEvent): boolean {
    ...
    if (
      this.enableBorderOnMouseMove &&
      this.type === 'Pie' &&
      this.pieSeriesModule &&
  ...
  public titleTooltip(
    ...
    const id: boolean =
      targetId === this.element.id + '_title' ||
      targetId === this.element.id + '_subTitle' ||
  ...
  public accumulationChartKeyUp(e: KeyboardEvent): boolean {
    ...
    if (legendElement) {
      ...
      if (
        className &&
        className.indexOf('e-accumulationchart-focused') === -1
      ) {
    ...
    if (e.code === 'Tab') {
      if (this.previousTargetId !== '') {
        if (
          ...
          this.setTabIndex(
            groupElement.children[this.currentPointIndex] as HTMLElement,
          ...
          targetId.indexOf('chart_legend_g_') === -1
        ) {
          this.setTabIndex(
            legendElement.children[this.currentLegendIndex] as HTMLElement,
            legendElement.firstElementChild as HTMLElement
          );
        }
      }
      ...
      if (
        ...
        targetId.indexOf('_Point_') > -1 &&
        (this.highlightMode !== 'None' || this.tooltip.enable)
      ) {
        actionKey = 'Tab';
      }
      ...
      if (targetId.indexOf('_chart_legend_page') > -1) {
        ...
        this.currentLegendIndex = this.getActualIndex(
          this.currentLegendIndex,
          legendElement.children.length
        );

        const currentLegend: Element =
        ...
        for (
          let i: number = 0;
          i < e.target['parentElement'].children.length;
          i++
        ) {
          totalLength =
            e.target['parentElement'].children[i as number].id.indexOf(
              '_Point_'
            ) > -1
              ? totalLength + 1
              : totalLength;
        ...
        targetId =
          this.element.id + '_Series_0_Point_' + this.currentPointIndex;
        this.focusTarget(getElement(targetId) as HTMLElement);
      ...
      targetId =
        targetId.indexOf('_chart_legend_g') > -1
        ...
        if (targetId.indexOf('_Point_') > -1) {
            ...
            if (!isNullOrUndefined(targetElement)) {
              if (targetElement.id.indexOf('text') > 1) {
                targetElement = getElement(
                  targetElement.id.replace('text', 'shape')
                );
              }
              if (
              ...
              this.accumulationHighlightModule.calculateSelectedElements(
                this,
                targetElement,
                'mousemove'
              );
          ...
          if (this.accumulationTooltipModule) {
            const series: AccumulationSeries =
              this.visibleSeries[seriesIndex as number];
            let data: AccPointData;
            if (series.enableTooltip) {
              data = new AccPointData(
                series.points[pointIndex as number],
                series
              );
            }
        ...
        if (this.accumulationHighlightModule && this.highlightMode !== 'None') {
          ...
          for (let i: number = 0; i < legendItemsId.length; i++) {
            ...
            if (targetId.indexOf(id) > -1) {
              ...
              this.accumulationHighlightModule.legendSelection(
                this,
                0,
      ...
      case 'Space':
        if (
          ...
          if (this.accumulationSelectionModule) {
            this.accumulationSelectionModule.calculateSelectedElements(
              this,
  ...
  private triggerPointEvent(
    ...
    if (indexes.series >= 0 && indexes.point >= 0) {
      this.trigger(event, {
        ...
        point: (<AccumulationSeries>this.series[indexes.series]).points[
          indexes.point
        ],
  ...
  public accumulationMouseLeave(e: PointerEvent): boolean {
    ...
    let borderElement: Element = document.getElementById(
      this.element.id + 'PointHover_Border'
    );
    if (borderElement) {
  ...
  private createSecondaryElement(): void {
    const element: Element =
      redrawElement(this.redraw, this.element.id + '_Secondary_Element') ||
  ...
  private calculateVisibleSeries(): void {
    this.visibleSeries = [];
    for (
      let i: number = 0, length: number = this.series.length;
      i < length;
      i++
    ) {
      (this.series[i as number] as AccumulationSeries).index = i;
      if (
        this.series[i as number].type === this.type &&
        this.visibleSeries.length === 0
      ) {
  ...
  private doGrouppingProcess(): void {
    ...
    if (
      !isNullOrUndefined(series.resultData) &&
      !isNullOrUndefined(series.lastGroupTo) &&
    ...
    if (this.subTitle) {
      ...
      subTitleHeight =
        measureText(
          ...
          this.subTitleStyle,
          this.themeStyle.chartSubTitleFont
        ).height * this.subTitleCollection.length;
    }
    subtractRect(
  ...
  private calculateLegendBounds(): void {
    ...
    this.accumulationLegendModule.getLegendOptions(
      this,
      <AccumulationSeries[]>this.visibleSeries
    );
    this.accumulationLegendModule.calculateLegendBounds(
      this.initialClipRect,
      this.availableSize,
      null
    );
  ...
  public setSecondaryElementPosition(): void {
    const tooltipParent: HTMLDivElement = getElement(
  ...
  private renderSeries(): void {
    if (!this.redraw) {
      this.svgObject.appendChild(
        this.renderer.createGroup({ id: this.element.id + '_SeriesCollection' })
      );
    }
    for (const series of this.visibleSeries) {
      if (
        series.visible &&
        this[firstToLowerCase(series.type) + 'SeriesModule']
      ) {
        this[firstToLowerCase(series.type) + 'SeriesModule'].initProperties(
          this,
          series
        );
    ...
    if (this.accumulationLegendModule.legendCollections.length) {
      if (this.visibleSeries[0].type === 'Pie') {
        this.accumulationLegendModule.getSmartLegendLocation(
          this.visibleSeries[0].labelBound,
          this.accumulationLegendModule.legendBounds,
          this.margin
        );
      }
  ...
  public processSelection(): void {
    ...
    if (this.accumulationSelectionModule && this.selectionMode !== 'None') {
      selectedDataIndexes = <Indexes[]>(
        extend(
          [],
          this.accumulationSelectionModule.selectedDataIndexes,
          null,
          true
        )
      );
  ...
  private renderTitle(): void {
    ...
    const legendHeight: number =
      this.accumulationLegendModule === undefined
        ? 0
        : this.legendSettings.position === 'Top'
        ? this.accumulationLegendModule.legendBounds.height
        : 0;
    const explode: number =
      this.explodeDistance === 0 ? 0 : this.explodeDistance;
    ...
    const space: number =
      ...
      this.visibleSeries[0].dataLabel.position === 'Outside' &&
      this.visibleSeries[0].dataLabel.connectorStyle.length
        ? stringToNumber(
            this.visibleSeries[0].dataLabel.connectorStyle.length,
            this.accBaseModule.radius
          )
        : 0;
    if (
      !this.subTitle &&
      this.series[0].type !== 'Funnel' &&
      this.series[0].type !== 'Pyramid'
    ) {
      options.y =
        parseInt(this.series[0].radius, 10) >= 80
          ? options.y
          : this.accBaseModule.center.y -
            this.accBaseModule.radius -
            padding -
            titleHeight -
            legendHeight -
      ...
      if (
        this.series[0].type === 'Pie' &&
        (parseInt(this.series[0].radius, 10) < 80 ||
          isNaN(parseInt(this.series[0].radius, 10)))
      ) {
        options.y =
          options.y < this.initialClipRect.y - legendHeight
            ? this.initialClipRect.y - legendHeight
            : options.y;
      }
    }
    ...
    if (element) {
      ...
      element.parentNode.insertBefore(
        element,
        this.svgObject.children && this.svgObject.children[1]
      );
    }
    if (this.subTitle) {
    ...
    this.format =
      data.point == null
        ...
        : this.parseFormat(
            data.point,
            this.visibleSeries[0],
            this.centerLabel.hoverTextFormat
          );
  ...
  private getPieData(e: PointerEvent | TouchEvent): AccPointData {
    ...
    if (!isNaN(dataIndex.series)) {
      return new AccPointData(
        this.visibleSeries[0].points[dataIndex.point as number],
        this.visibleSeries[0]
      );
    }
  ...
  public renderCenterLabel(
    ...
    if (pointAnimation) {
      const tspanElements: NodeListOf<SVGTSpanElement> = getElement(
    ...
    if (centerLabelSize.height * labelCollection.length > maxwidth) {
      ...
      ypos =
        this.accBaseModule.center.y -
        (centerLabelSize.height * labelCollection.length) / 2 +
        (centerLabelSize.height + padding) / 2 -
        this.pieSeriesModule.innerRadius / 2 +
        (this.pieSeriesModule.innerRadius ? padding : 0);
      if (
        centerLabelSize.height * labelCollection.length +
          this.pieSeriesModule.innerRadius / 2 +
          padding >
        maxwidth
      ) {
      ...
      ypos =
        labelCollection.length > 1
          ? this.accBaseModule.center.y -
            (centerLabelSize.height * labelCollection.length) / 2 +
            (centerLabelSize.height + padding) / 2
          : this.accBaseModule.center.y + centerLabelSize.height / 4;
    }
    const options: TextOption = new TextOption(
      this.element.id + '_centerLabel',
      ((this.series[0].animation.enable && animationMode !== 'Disable') ||
        animationMode === 'Enable') &&
      isanimate
        ? this.pieSeriesModule.center.x - 1
        : this.pieSeriesModule.center.x,
      ypos,
      getAnchor,
      '',
      '',
    ...
    const element: Element = textElement(
      ...
      options,
      this.centerLabel.textStyle,
      this.centerLabel.textStyle.color || this.themeStyle.chartTitleFont.color,
      this.svgObject,
      false,
      this.redraw,
      null,
    ...
    for (let i: number = 0; i < labelCollection.length; i++) {
      const tspanOption: Object = {
        x: options.x,
        y: options.y + i * centerLabelSize.height,
        fill: '',
      };
      const tspanElement: HTMLElement = <HTMLElement>(
        (this.renderer as SvgRenderer).createTSpan(tspanOption, '')
      );
      tspanElement.style.fontFamily = 'inherit';
      tspanElement.style.fontStyle = 'inherit';
      tspanElement.style.fontSize = 'inherit';
      tspanElement.style.fontWeight =
        labelCollection[i as number].indexOf('<b>') > -1 ||
        labelCollection[i as number].indexOf('</b>') > -1
          ? 'bold'
          : 'inherit';
      tspanElement.textContent = labelCollection[i as number].replace(
        /<\/?b>/g,
        ''
      );
  ...
  private renderSubTitle(options: TextOption): void {
    ...
    const subTitleOption: TextOption = new TextOption(
      ...
      options.y * options.text.length +
        (subTitleElementSize.height * 3) / 4 +
        padding,
  ...
  public destroy(): void {
    ...
    if (this.element) {
      ...
      const element: HTMLElement = document.getElementById(
        this.element.id + 'Keyboard_accumulationchart_focus'
      );
      if (element) {
        element.remove();
      }
  ...
  public requiredModules(): ModuleDeclaration[] {
    ...
    enableAnnotation = this.annotations.some(
      (value: AccumulationAnnotationSettings) => {
  ...
  public onPropertyChanged(
    newProp: AccumulationChartModel,
    oldProp: AccumulationChartModel
  ): void {
        ...
        case 'series':
          if (!this.animateselected) {
            ...
            for (let i: number = 0; i < len; i++) {
              ...
              if (
                newProp.series[i as number] &&
                (newProp.series[i as number].dataSource ||
                  newProp.series[i as number].yName ||
                  newProp.series[i as number].xName ||
                  series.type ||
                  newProp.series[i as number].dataLabel ||
                  blazorProp)
              ) {
                extend(
                  this.changeVisibleSeries(this.visibleSeries, i),
                  series,
                  null,
                  true
                );
              ...
              if (
                newProp.series[i as number] &&
                !isNullOrUndefined(newProp.series[i as number].explodeIndex) &&
                newProp.series[i as number].explodeIndex >= 0 &&
                newProp.series[i as number].explodeIndex !==
                  oldProp.series[i as number].explodeIndex
              ) {
                this.accBaseModule.explodePoints(
                  newProp.series[i as number].explodeIndex,
                  this
                );
                this.accBaseModule.deExplodeAll(
                  newProp.series[i as number].explodeIndex,
        ...
        case 'selectionMode':
          if (this.accumulationSelectionModule) {
            if (
              ...
              this.accumulationSelectionModule.selectedDataIndexes = this
