...
export class AccumulationAnnotation extends AnnotationBase {
  ...
  public renderAnnotations(element: Element): void {
    ...
    this.parentElement = (redrawElement(
      redraw,
      this.pie.element.id + '_Annotation_Collections'
    ) ||
      createElement('div', {
        id: this.pie.element.id + '_Annotation_Collections',
      })) as HTMLElement;
    this.annotations.map(
      (annotation: AccumulationAnnotationSettings, index: number) => {
        this.processAnnotation(annotation, index, this.parentElement);
      }
    );
    if (
      this.pie.series[0].animation.enable &&
      this.pie.series[0].type === 'Pie' &&
      this.pie.animateSeries
    ) {
