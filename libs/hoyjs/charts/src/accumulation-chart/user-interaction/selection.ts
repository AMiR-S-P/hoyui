...
export class AccumulationSelection extends BaseSelection {
  ...
  public invokeSelection(accumulation: AccumulationChart): void {
    ...
    this.series = <AccumulationSeries[]>(
      extend({}, accumulation.visibleSeries, null, true)
    );
    this.seriesStyles();
  ...
  private generateStyle(
    ...
    return (
      ...
      this.styleId +
        '_series_' +
        (<AccumulationSeries>series).index +
        '_point_' +
        point
    );
  }
  ...
  public isAlreadySelected(targetElement: Element, eventType: string): boolean {
    ...
    if (
      this.accumulation.highlightMode !== 'None' &&
      this.accumulation.selectionMode === 'None'
    ) {
      if (eventType === 'click') {
        return false;
      }
    ...
    if (
      ...
      const parentNodeId: string = (<Element>targetElement.parentNode).id;
      let isValidElement: boolean;
        ...
        if (this.previousSelectedElement[i as number].hasAttribute('class')) {
          if (
            this.previousSelectedElement[i as number]
              .getAttribute('class')
              .indexOf('highlight') > -1 &&
            (isValidElement || eventType === 'click')
          ) {
            ...
            this.performSelection(
              indexFinder(this.previousSelectedElement[i as number].id),
              this.accumulation,
              this.previousSelectedElement[i as number]
            );
          }
  ...
  public calculateSelectedElements(
    ...
    if (
      (accumulation.highlightMode === 'None' &&
    ...
    if (eventType === 'mousemove') {
      if (
        !isNullOrUndefined(targetEle.parentNode) &&
        (<Element>targetEle.parentNode).hasAttribute('class') &&
        ((<Element>targetEle.parentNode)
          .getAttribute('class')
          .indexOf('highlight') > 0 ||
          (<Element>targetEle.parentNode)
            .getAttribute('class')
    ...
    if (targetEle.getAttribute('id').indexOf('_connector_') > -1) {
      ...
      if (
        targetEle.id.indexOf('_Series_') > -1 ||
        targetEle.id.indexOf('_datalabel_') > -1
      ) {
        this.performSelection(
          indexFinder(targetEle.id),
          accumulation,
          targetEle
        );
      }
  ...
  private performSelection(
    ...
    element =
      element.id.indexOf('datalabel') > -1
        ? <Element>(
            accumulation.getSeriesElement().childNodes[index.series].childNodes[
              index.point
            ]
          )
        : element;
      ...
      case 'Point':
        if (!isNaN(index.point)) {
          ...
          this.selectionComplete(
            accumulation,
            <AccumulationSeries>accumulation.series[0]
          );
  ...
  private selection(
    ...
    if (
      !accumulation.isMultiSelect &&
      this.styleId.indexOf('highlight') === -1 &&
      accumulation.selectionMode !== 'None'
    ) {
      this.removeMultiSelectEelments(
        accumulation,
        this.selectedDataIndexes,
        index,
        accumulation.series
    ...
    if (
      selectedElements[0] &&
      className.indexOf(
        this.getSelectionClass(selectedElements[0].id, index.point)
      ) > -1
    ) {
      ...
      if (accumulation.enableBorderOnMouseMove) {
        const borderElement: Element = document.getElementById(
          selectedElements[0].id.split('_')[0] + 'PointHover_Border'
        );
        if (!isNullOrUndefined(borderElement)) {
          this.removeSvgClass(
            borderElement,
            borderElement.getAttribute('class')
          );
        }
      }
    } else {
      this.previousSelectedElement =
        accumulation.highlightMode !== 'None' ? selectedElements : [];
  ...
  public legendSelection(
    ...
    if (isPreSelected) {
      ...
      const seriesElements: Element = <Element>(
        accumulation.getSeriesElement().childNodes[series as number].childNodes[
          pointIndex as number
        ]
      );
      this.selection(accumulation, new Index(series, pointIndex), [
        seriesElements,
      ]);
      this.blurEffect(accumulation.element.id, accumulation.visibleSeries);
    }
  ...
  private removeMultiSelectEelments(
    ...
    index: Index[],
    currentIndex: Index,
    seriesCollection: AccumulationSeriesModel[]
  ): void {
    ...
    for (let i: number = 0; i < index.length; i++) {
      ...
      if (!this.checkEquals(index[i as number], currentIndex)) {
        this.removeStyles(
          this.findElements(accumulation, series, index[i as number]),
          index[i as number]
        );
        index.splice(i, 1);
        i--;
    ...
    for (const series of visibleSeries) {
      if (series.visible) {
        this.checkSelectionElements(
          document.getElementById(pieId + '_SeriesCollection'),
  ...
  private checkSelectionElements(
    ...
    for (let i: number = 0; i < children.length; i++) {
      elementClass =
        (children[i as number] as HTMLElement).getAttribute('class') || '';
      parentClass =
      ...
      if (
        ...
        className =
          elementClass.indexOf('selection') > 0 ||
          elementClass.indexOf('highlight') > 0
            ? elementClass
            : className;
        className =
          parentClass.indexOf('selection') > 0 ||
          parentClass.indexOf('highlight') > 0
            ? parentClass
            : className;
      }
      if (
        elementClass.indexOf(className) === -1 &&
        parentClass.indexOf(className) === -1 &&
        visibility
      ) {
      ...
      if (
        (this.control as AccumulationChart).accumulationLegendModule &&
        (this.control as AccumulationChart).legendSettings.visible
      ) {
        legendShape = document.getElementById(
          this.control.element.id + '_chart_legend_shape_' + i
        );
        if (legendShape) {
          if (
            elementClass.indexOf(className) === -1 &&
  ...
  private applyStyles(elements: Element[], index: Index): void {
    const accumulationTooltip: AccumulationTooltip = (
      this.control as AccumulationChart
    ...
    for (const element of elements) {
      ...
      if (element) {
        if (
          ...
          this.addSvgClass(
            legendShape,
            this.getSelectionClass(legendShape.id, index.point)
          );
        }
        ...
        const opacity: number =
          accumulationTooltip &&
          accumulationTooltip.previousPoints.length > 0 &&
          accumulationTooltip.previousPoints[0].point.index !== index.point
            ? accumulationTooltip.svgTooltip.opacity
            : this.series[index.series].opacity;
        element.setAttribute('opacity', opacity.toString());
        this.addSvgClass(
          element,
          this.getSelectionClass(element.id, index.point)
        );
      }
    }
  }
  ...
  private getSelectionClass(id: string, point?: number): string {
    return this.generateStyle(
      (this.control as AccumulationChart).series[indexFinder(id).series],
      point
    );
  }
  /**
