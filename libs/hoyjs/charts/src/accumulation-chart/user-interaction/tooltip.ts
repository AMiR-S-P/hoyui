...
export class AccumulationTooltip extends BaseTooltip {
  ...
  private mouseUpHandler(e: PointerEvent | TouchEvent): void {
    ...
    if (
      ...
      control.isTouch &&
      withInBounds(control.mouseX, control.mouseY, control.initialClipRect)
    ) {
      this.tooltip(e);
  ...
  public renderSeriesTooltip(
    ...
    if (
      data.point &&
      (!this.previousPoints[0] || this.previousPoints[0].point !== data.point)
    ) {
      if (
  ...
  private triggerTooltipRender(
    ...
    const argsData: ITooltipRenderEventArgs = {
      ...
      series: this.accumulation.isBlazor
        ? ({} as AccumulationSeries)
        : point.series,
          ...
          isFirst,
          {
            x:
              tooltip.location.x !== null
                ? tooltip.location.x
                : point.point.symbolLocation.x,
            y:
              tooltip.location.y !== null
                ? tooltip.location.y
                : point.point.symbolLocation.y,
          },
          point.series.clipRect,
          point.point,
          !tooltip.enableMarker ? [] : ['Circle'],
          0,
          this.chart.initialClipRect,
          false,
          null,
          point.point,
  ...
  private getPieData(
    ...
    if (!isNaN(id.series)) {
      ...
      if (
        ...
        const series: AccumulationSeries = this.getSeriesFromIndex(
          seriesIndex,
          chart.visibleSeries
        );
  ...
  private getTooltipText(
    ...
    let format: string = tooltip.format
      ? tooltip.format
      : '${point.x} : <b>${point.y}</b>';
    format = this.accumulation.useGroupingSeparator
    ...
    for (const dataValue of Object.keys(point)) {
      ...
      const pointValue: string =
        ...
        typeof point[dataValue as string] === 'number'
          ? this.accumulation.intl.formatNumber(point.x as number, {
              useGrouping: true,
            })
