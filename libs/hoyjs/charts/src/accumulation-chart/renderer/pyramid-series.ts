...
import { PathOption, Size, removeElement } from '@hoyui/svg-base';
...
export class PyramidSeries extends TriangularBase {
  ...
  private getSegmentData(
    ...
    //top of th series
    const seriesTop: number =
      chart.initialClipRect.y +
      (chart.initialClipRect.height - area.height) / 2;
  ...
  private calculateSurfaceSegments(series: AccumulationSeries): void {
    ...
    const gapHeight: number =
      series.points.length > 1 ? gapRatio / (count - 1) : 0;
    ...
    for (let i: number = 0; i < count; i++) {
      if (series.points[i as number].visible) {
        y[i as number] = currY;
        height[i as number] = this.getSurfaceHeight(
          currY,
          Math.abs(series.points[i as number].y)
        );
  ...
  private renderPoint(
    ...
    appendChildElement(
      ...
      element,
      redraw,
      pointAnimation ? pointAnimation : undefined,
      pointAnimation ? 'x' : undefined,
      pointAnimation ? 'y' : undefined,
      undefined,
      pointAnimation ? previousDirection : undefined,
      undefined,
      undefined,
      undefined,
