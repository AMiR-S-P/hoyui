...
export class AccumulationDataLabel extends AccumulationBase {
  ...
  public getDataLabelPosition(
    ...
    const radius: number = this.isCircular()
      ? !this.isVariousRadius()
        ? this.accumulation.pieSeriesModule.labelRadius
        : this.accumulation.pieSeriesModule.getLabelRadius(
            this.accumulation.visibleSeries[0],
            point
          )
    ...
    if (this.accumulation.title) {
      const titleSize: Size = measureText(
        this.accumulation.title,
  ...
  private getLabelRegion(
    ...
    const location: ChartLocation = degreeToLocation(
      labelAngle,
      labelRadius,
      this.isCircular() ? this.center : this.getLabelLocation(point, position)
    );

    location.y =
      position === 'Inside' ? location.y - textSize.height / 2 : location.y;
    location.x =
      position === 'Inside' ? location.x - textSize.width / 2 : location.x;
   ...
   * @private
   */
  public calculateLabelCollection(
    point: AccPoints,
    dataLabel: AccumulationDataLabelSettingsModel
  ): void {
    ...
    const position: AccumulationLabelPosition =
      point.labelPosition || dataLabel.position;
    const labelRadius: number = this.isCircular()
      ? !this.isVariousRadius()
        ? this.accumulation.pieSeriesModule.labelRadius
        : this.accumulation.pieSeriesModule.getLabelRadius(
            this.accumulation.visibleSeries[0],
    ...
    const radius: number = !this.isVariousRadius()
      ? this.accumulation.pieSeriesModule.radius -
        this.accumulation.pieSeriesModule.innerRadius
      : this.accumulation.pieSeriesModule.getLabelRadius(
          this.accumulation.visibleSeries[0],
          point
        );
    ...
    if (!maxWidth) {
      if (position === 'Outside') {
        maxWidth = this.isCircular()
          ? location.x >= this.center.x
            ? this.areaRect.x + this.areaRect.width - location.x
            : location.x - this.areaRect.x
          : location.x >= point.region.x
          ? this.areaRect.x + this.areaRect.width - location.x
          : location.x - this.areaRect.x;
      } else {
    ...
    if (point.label.indexOf('<br>') !== -1) {
      ...
      dataLabel.textWrap === 'Normal' &&
      dataLabel.textOverflow === 'Ellipsis'
    ) {
      point.labelCollection[0] = textTrim(
        maxWidth,
        point.label,
        point.argsData.font,
        this.accumulation.enableRtl,
        this.accumulation.themeStyle.datalabelFont
      );
    } else if (
      dataLabel.textWrap === 'Wrap' ||
      dataLabel.textWrap === 'AnyWhere'
    ) {
      point.labelCollection = textWrap(
        point.label,
        maxWidth,
        point.argsData.font,
        this.accumulation.enableRtl,
        dataLabel.textWrap === 'AnyWhere',
        dataLabel.textOverflow === 'Clip',
        this.accumulation.themeStyle.datalabelFont
      );
    } else {
  ...
  public getTextSize(
    ...
    let height: number = 0;
    const font: FontModel = dataLabel.font;
    let width: number = dataLabel.maxWidth ? dataLabel.maxWidth : 0;
    let textSize: Size;
    for (let i: number = 0; i < labelCollection.length; i++) {
      textSize = measureText(
        labelCollection[i as number],
        font,
        this.accumulation.themeStyle.datalabelFont
      );
  ...
  private getSmartLabel(
    ...
    let labelRadius: number = circular
      ? this.radius
      : this.getLabelDistance(point, dataLabel);

    const connectorLength: string = circular
      ? dataLabel.connectorStyle.length || '4%'
      : '0px';

    labelRadius += stringToNumber(connectorLength, labelRadius);

    let previousPoint: AccPoints = this.findPreviousPoint(
      points,
      point.index,
      point.labelPosition
    );
    ...
    if (dataLabel.position === 'Inside') {
      ...
      if (
        ...
        previousPoint = this.findPreviousPoint(
          points,
          point.index,
          point.labelPosition
        );
        if (
          ...
          this.setOuterSmartLabel(
            previousPoint,
            point,
            dataLabel.border.width,
            labelRadius,
            textSize,
            this.marginValue
          );
    ...
    if (
      this.isOverlapping(point, points) &&
      (this.accumulation.type === 'Pyramid' ||
        this.accumulation.type === 'Funnel')
    ) {
      const position: string = 'OutsideLeft';
      const space: number = 20;
      const labelAngle: number = point.midAngle || 0;
      const labelRadius: number = circular
        ? this.radius
        : this.getLabelDistance(point, dataLabel);
      const location: ChartLocation = degreeToLocation(
        labelAngle,
      ...
      point.labelRegion = new Rect(
        location.x,
        location.y,
        textSize.width + this.marginValue * 2,
        textSize.height + this.marginValue * 2
      );
      point.labelRegion.y -= point.labelRegion.height / 2;
      point.labelRegion.x =
        point.labelRegion.x - space - point.labelRegion.width;
      if (
        previousPoint &&
        previousPoint.labelRegion &&
        (isOverlap(point.labelRegion, previousPoint.labelRegion) ||
  ...
  public move(e: Event, x: number, y: number, isTouch?: boolean): void {
    if ((<HTMLElement>e.target).textContent.indexOf('...') > -1) {
      ...
      if (targetId.length === 2) {
        const seriesIndex: number = parseInt(
          targetId[1].split('_text_')[0],
          10
        );
        ...
        if (!isNaN(seriesIndex) && !isNaN(pointIndex)) {
          ...
          const point: AccPoints = getSeriesFromIndex(
            seriesIndex,
            this.accumulation.visibleSeries
          ).points[pointIndex as number];
          showTooltip(
            point.text || point.y.toString(),
            x,
            y,
            this.areaRect.width,
            this.accumulation.element.id + '_EJ2_Datalabel_Tooltip',
            ...
            null,
            null,
            this.accumulation.initialClipRect
          );
        }
      }
    } else {
  ...
  private isOverlapping(currentPoint: AccPoints, points: AccPoints[]): boolean {
    for (let i: number = currentPoint.index - 1; i >= 0; i--) {
      if (
        points[i as number].visible &&
        points[i as number].labelVisible &&
        points[i as number].labelRegion &&
        currentPoint.labelRegion &&
        currentPoint.labelVisible &&
        isOverlap(currentPoint.labelRegion, points[i as number].labelRegion)
      ) {
        return true;
      }
    }
    ...
    if (isOverlap(point.labelRegion, rect)) {
      ...
      if (point.labelVisible && point.labelRegion) {
        ...
        for (let i: number = 0; i < point.labelCollection.length; i++) {
          if (
            point.labelCollection[i as number].length === 3 &&
            point.labelCollection[i as number].indexOf('...') > -1
          ) {
            this.setPointVisibileFalse(point);
            break;
          }
  ...
  private setOuterSmartLabel(
    ...
    if (!this.isCircular()) {
      ...
      let labelAngle: number = this.getOverlappedAngle(
        previousPoint.labelRegion,
        point.labelRegion,
        point.midAngle,
        border * 2
      ...
      while (
        point.labelVisible &&
        (isOverlap(previousPoint.labelRegion, point.labelRegion) ||
          labelAngle <= previousPoint.labelAngle ||
          labelAngle <= point.midAngle * 0.9 ||
          this.isConnectorLineOverlapping(point, previousPoint))
      ) {
        if (labelAngle > point.endAngle) {
          //this.setPointVisibileFalse(point);
  ...
  private setSmartLabelForSegments(
    ...
    const overlapHeight: number =
      this.accumulation.type === 'Funnel'
        ...
        : point.labelRegion.y -
          (prevPoint.labelRegion.y + prevPoint.labelRegion.height);
    if (overlapHeight < 0) {
      point.labelRegion.y +=
        this.accumulation.type === 'Funnel' ? overlapHeight : -overlapHeight;
    }
  }
  ...
  private isConnectorLineOverlapping(
    ...
    return (
      this.isLineRectangleIntersect(start, end, point.labelRegion) ||
      this.isLineRectangleIntersect(start, end, previous.labelRegion) ||
      this.isLineRectangleIntersect(
        previousstart,
        previousend,
        point.labelRegion
      )
  ...
  private isLinesIntersect(
    point1: ChartLocation,
    point2: ChartLocation,
    point11: ChartLocation,
    point12: ChartLocation
  ): boolean {
    ...
    if (delta !== 0) {
      ...
      let lies: boolean =
        Math.min(point1.x, point2.x) <= x && x <= Math.max(point1.x, point2.x);
      lies =
        lies &&
      ...
      lies =
        lies &&
        Math.min(point11.x, point12.x) <= x &&
        x <= Math.max(point11.x, point12.x);
      lies =
        lies &&
        Math.min(point11.y, point12.y) <= y &&
        y <= Math.max(point11.y, point12.y);
      return lies;
    }
    return false;
  }
  ...
  private getConnectorPath(
    ...
    const labelAngle: number = this.accumulation.enableSmartLabels
      ? point.midAngle
      : end || point.midAngle;

    let middle: ChartLocation = new ChartLocation(0, 0);

    const endPoint: ChartLocation = this.getEdgeOfLabel(
      label,
      labelAngle,
      middle,
      connector.width,
      ...
      if (this.isCircular()) {
        const r: number =
          ...
          (this.isVariousRadius()
            ? stringToNumber(
                point.sliceRadius,
                this.accumulation.pieSeriesModule.size / 2
              )
            : this.radius);
  ...
  private getBezierPoint(
    t: number,
    controlPoints: ChartLocation[],
    index: number,
    count: number
  ): ChartLocation {
    if (count === 1) {
      return controlPoints[index as number];
    }
    const p0: ChartLocation = this.getBezierPoint(
      t,
      controlPoints,
      index,
      count - 1
    );
    const p1: ChartLocation = this.getBezierPoint(
      t,
      controlPoints,
      index + 1,
      count - 1
  ...
  private getLabelDistance(
    ...
    if (
      (point.labelPosition && dataLabel.position !== point.labelPosition) ||
      (dataLabel.connectorStyle.length && dataLabel.position === 'Outside')
    ) {
      const length: number = stringToNumber(
        dataLabel.connectorStyle.length || '70px',
    ...
    const extraSpace: number =
      (this.accumulation.initialClipRect.width - series.triangleSize.width) / 2;
    ...
    switch (position) {
      ...
      case 'Outside':
        ...
        return (
          this.accumulation.initialClipRect.width - labelLocation - extraSpace
        );
    }
  }
  ...
  private getLabelLocation(
    point: AccPoints,
    position: AccumulationLabelPosition | string = 'Outside'
  ): ChartLocation {
    if (this.accumulation.type !== 'Pie') {
      ...
      return degreeToLocation(
        ...
        this.isVariousRadius()
          ? stringToNumber(
              point.sliceRadius,
              this.accumulation.pieSeriesModule.seriesRadius
            )
          : this.radius,
        this.center
      );
  ...
  public findAreaRect(): void {
    ...
    subtractThickness(
      this.areaRect,
      new Thickness(margin.left, margin.right, margin.top, margin.bottom)
    );
  }
  /**
  ...
  public renderDataLabel(
    ...
    const id: string =
      this.accumulation.element.id + '_datalabel_Series_' + series + '_';
    const datalabelGroup: Element = this.accumulation.renderer.createGroup({
    ...
    const argsFont: FontModel = <FontModel>(
      extend({}, getValue('properties', dataLabel.font), null, true)
    );
    point.label = this.getDatalabelText(
      dataLabel.format,
      this.accumulation,
      point.originalText || point.y.toString()
    );
  ...
  private getDatalabelText(
    ...
    if (Number(labelText)) {
      ...
      labelText = customLabelFormat
        ? labelFormat.replace('{value}', format(parseFloat(labelText)))
        : format(parseFloat(labelText));
    }
    return labelText;
  }
  /**
  ...
  public calculateLabelSize(
    ...
    this.calculateLabelCollection(point, dataLabel);
    const textSize: Size = isTemplate
      ? isReactCallback
      ...
      : this.getTextSize(point.labelCollection, dataLabel);
    textSize.height += 4; // 4 for calculation with padding for smart label shape
    textSize.width += 4;
  ...
  public drawDataLabels(
    ...
    let angle: number;
    let degree: number;
    const modifiedPoints: AccPoints[] = series.leftSidePoints.concat(
    ...
    for (const point of modifiedPoints) {
      if (!isNullOrUndefined(point.argsData) && !isNullOrUndefined(point.y)) {
        ...
        const id: string =
          this.accumulation.element.id + '_datalabel_Series_' + 0 + '_';
        const datalabelGroup: Element = this.accumulation.renderer.createGroup({
          id: id + 'g_' + point.index,
        });
        ...
        if (point.visible && point.labelVisible) {
          ...
          if (point.argsData.template) {
            ...
            location = new ChartLocation(
              point.labelRegion.x + this.marginValue,
              point.labelRegion.y +
                (point.textSize.height * 3) /
                  (point.labelCollection.length * 4) +
                this.marginValue
            );
            element = getElement(id + 'shape_' + point.index);
            const startLocation: ChartLocation = element
              ? new ChartLocation(
            ...
            dataLabelElement = this.accumulation.renderer.drawRectangle(
              new RectOption(
                id + 'shape_' + point.index,
                point.argsData.color,
                point.argsData.border,
                1,
                point.labelRegion,
                dataLabel.rx,
                dataLabel.ry,
                rotate,
                series.dataLabel.border.dashArray
              )
            );
            appendChildElement(
              false,
              ...
              false,
              null,
              this.accumulation.duration
            );
            textElement(
              this.accumulation.renderer,
              new TextOption(
                id + 'text_' + point.index,
                location.x,
                location.y,
                this.accumulation.enableRtl ? 'end' : 'start',
                point.labelCollection,
                rotate,
                'auto',
                degree
              ...
              point.argsData.font,
              point.argsData.font.color ||
                this.getSaturatedColor(point, point.argsData.color),
              datalabelGroup,
              false,
              redraw,
              true,
          ...
          if (
            ...
            this.accumulation.legendSettings.visible &&
            !this.accumulation.redraw &&
            (dataLabel.position === 'Outside' ||
              this.accumulation.enableSmartLabels)
          ) {
            this.accumulation.visibleSeries[0].findMaxBounds(
              this.accumulation.visibleSeries[0].labelBound,
              point.labelRegion
            );
          }
          if (point.labelPosition === 'Outside') {
            const element: Element = getElement(
              id + 'connector_' + point.index
            );
            const previousDirection: string = element
              ? element.getAttribute('d')
              : '';
            const pathElement: Element = this.accumulation.renderer.drawPath(
              new PathOption(
                id + 'connector_' + point.index,
                'transparent',
                dataLabel.connectorStyle.width,
                dataLabel.connectorStyle.color || point.color,
                1,
                dataLabel.connectorStyle.dashArray,
                this.getConnectorPath(
                  <Rect>extend({}, point.labelRegion, null, true),
                  point,
                  dataLabel,
                  point.labelAngle
                )
          ...
          getElement(datalabelGroup.id).parentNode.removeChild(
            getElement(datalabelGroup.id)
          );
        }
      }
    }
    if (
      this.accumulation.type === 'Pie' &&
      dataLabel.textWrap === 'Normal' &&
      dataLabel.textOverflow === 'Clip'
    ) {
  ...
  private dataLabelClipPath(
    ...
    const id: string =
      this.accumulation.element.id + '_datalabel_Series_' + 0 + '_';
    const clippath: Element = this.accumulation.renderer.createClipPath({
      id: id + 'clipPath',
    });
    let clipRect: Element;
    const radius: number = this.accumulation.pieSeriesModule.pieBaseRadius;
    if (dataLabel.position === 'Inside') {
      clipRect = this.accumulation.renderer.drawCircle(
        new CircleOption(
          id + 'clipPath_rect',
          'transparent',
          { width: 0 },
      ...
      let x: number =
        this.center.x -
        radius -
        stringToNumber(dataLabel.connectorStyle.length || '4%', radius) -
        dataLabel.maxWidth;
      const y: number =
        this.center.y -
        radius -
      ...
      if (this.accumulation.legendSettings.visible) {
        const legendModule: AccumulationLegend =
          this.accumulation.accumulationLegendModule;
        if (legendModule.position === 'Left') {
          width =
            legendModule.legendBounds.x + legendModule.legendBounds.width - x >
            0
              ? width - (legendModule.legendBounds.width - x)
              : width;
          x =
            legendModule.legendBounds.x + legendModule.legendBounds.width < x
              ? x
          ...
          width =
            x + width - legendModule.legendBounds.x > 0
              ? width - (x + width - legendModule.legendBounds.x)
              : width;
        }
      }
      clipRect = this.accumulation.renderer.drawRectangle(
        new RectOption(
          id + 'clipPath_rect',
          'transparent',
          { width: 0 },
  ...
  private finalizeDatalabels(
    ...
    if (
      ...
      (this.titleRect &&
        point.labelRegion &&
        isOverlap(point.labelRegion, this.titleRect))
    ) {
      if (
        this.isCircular() &&
        point.labelPosition === 'Outside' &&
    ...
    if (
      ...
      if (
        this.accumulation.visibleSeries[0].type !== 'Pie' &&
        this.accumulation.legendSettings.position === 'Left' &&
        dataLabel.position === 'Outside'
      ) {
      ...
      this.textTrimming(
        ...
        dataLabel.font,
        this.accumulation.accumulationLegendModule.position,
        dataLabel
      );
    }
    if (
      ...
      const position: string = this.isCircular()
        ? point.labelRegion.x >= this.center.x
          ? 'InsideRight'
          : 'InsideLeft'
        : point.labelRegion.x >= point.region.x
        ? 'InsideRight'
        : 'InsideLeft';
    ...
    if (
      ...
      this.accumulation.enableSmartLabels &&
      (point.labelRegion.y + point.labelRegion.height / 2 >
        this.areaRect.y + this.areaRect.height ||
        point.labelRegion.y < this.areaRect.y ||
        point.labelRegion.x < this.areaRect.x ||
        point.labelRegion.x + point.labelRegion.width >
          this.areaRect.x + this.areaRect.width)
    ) {
      this.setPointVisibileFalse(point);
    }
  }
  ...
  private getTemplateSize(
    ...
    element = createTemplate(
      ...
      point,
      this.accumulation.visibleSeries[0],
      this.accumulation.element.id + '_DataLabel',
      0,
      argsData,
      isTemplate,
      points,
  ...
  private setTemplateStyle(
    ...
    childElement.style.color =
      labelColor ||
      this.getSaturatedColor(
        point,
        point.labelPosition === 'Inside' ? fill : this.getLabelBackground(point)
      );
  ...
  private getSaturatedColor(point: AccPoints, color: string): string {
    ...
    if (this.marginValue >= 1) {
      saturatedColor =
        color === 'transparent' ? this.getLabelBackground(point) : color;
    } else {
      saturatedColor = this.getLabelBackground(point);
    }
    saturatedColor =
      saturatedColor === 'transparent'
        ? this.accumulation.theme.indexOf('Dark') > -1 ||
          this.accumulation.theme.indexOf('HighContrast') > -1
          ? 'black'
          : 'white'
        : saturatedColor;
    const rgbValue: ColorValue = convertHexToColor(
      colorNameToHex(saturatedColor)
    );
    const contrast: number = Math.round(
      (rgbValue.r * 299 + rgbValue.g * 587 + rgbValue.b * 114) / 1000
    );
    return this.accumulation.theme === 'Bootstrap5'
      ? '#212529'
      : this.accumulation.theme === 'Bootstrap5Dark'
      ? '#DEE2E6'
      : contrast >= 128
      ? 'black'
      : 'white';
  ...
  public doTemplateAnimation(
    ...
    if (
      ((series.animation.enable && animationMode !== 'Disable') ||
        animationMode === 'Enable') &&
      accumulation.animateSeries
    ) {
  ...
  private getLabelBackground(point: AccPoints): string {
    return point.labelPosition === 'Outside'
      ? this.accumulation.background || this.accumulation.themeStyle.background
      : !point.y
      ? this.accumulation.theme.indexOf('dark')
        ? 'white'
        : 'black'
      : point.color;
  }
  ...
  private extendedLabelsCalculation(): void {
    const series: AccumulationSeries = <AccumulationSeries>(
      this.accumulation.series[0]
    );
    series.rightSidePoints.forEach(
      (point: AccPoints, index: number, halfSidePoints: AccPoints[]) => {
        point.initialLabelRegion = point.labelRegion;
        point.isLabelUpdated = 0;
        this.skipPoints(point, halfSidePoints, index);
      }
    );
    series.leftSidePoints.forEach(
      (point: AccPoints, index: number, halfSidePoints: AccPoints[]) => {
        point.initialLabelRegion = point.labelRegion;
        point.isLabelUpdated = 0;
        this.skipPoints(point, halfSidePoints, index);
  ...
  private arrangeRightSidePoints(series: AccumulationSeries): void {
    ...
    const rightSideRenderPoints: AccPoints[] = series.rightSidePoints.filter(
      (point: AccPoints) =>
        point.labelVisible && point.labelPosition === 'Outside'
    );
    this.rightSideRenderingPoints = rightSideRenderPoints;
    let checkAngle: number;
    let currentPoint: AccPoints;
    const lastPoint: AccPoints =
      rightSideRenderPoints[rightSideRenderPoints.length - 1];
    ...
    for (let i: number = rightSideRenderPoints.length - 1; i >= 0; i--) {
      currentPoint = rightSideRenderPoints[i as number];
      nextPoint = rightSideRenderPoints[i + 1];
      // A point checked for overlapping, label visibility
      if (
        (this.isOverlapWithNext(currentPoint, rightSideRenderPoints, i) &&
          currentPoint.labelVisible) ||
        ...
        if (!this.isIncreaseAngle) {
          for (let k: number = i + 1; k < rightSideRenderPoints.length; k++) {
            this.increaseAngle(
              rightSideRenderPoints[k - 1],
              rightSideRenderPoints[k as number],
              series,
              true
            );
          }
  ...
  private arrangeLeftSidePoints(series: AccumulationSeries): void {
    ...
    for (let i: number = 0; i < leftSideRenderPoints.length; i++) {
      currentPoint = leftSideRenderPoints[i as number];
      previousPoint = leftSideRenderPoints[i - 1];
      // A point checked
      if (
        (this.isOverlapWithPrevious(currentPoint, leftSideRenderPoints, i) &&
          currentPoint.labelVisible) ||
        ...
        if (!this.isIncreaseAngle) {
          for (let k: number = i; k > 0; k--) {
            this.decreaseAngle(
              leftSideRenderPoints[k as number],
              leftSideRenderPoints[k - 1],
              series,
              false
            );
            leftSideRenderPoints.filter((point: AccPoints, index: number) => {
              if (
                point.isLabelUpdated &&
                leftSideRenderPoints[index as number].labelAngle - 10 < 100
              ) {
                this.isIncreaseAngle = true;
              }
            });
  ...
  private decreaseAngle(
    ...
    if (isRightSide) {
      while (
        isOverlap(currentPoint.labelRegion, previousPoint.labelRegion) ||
        (!this.isVariousRadius() &&
          !(
            previousPoint.labelRegion.height + previousPoint.labelRegion.y <
            currentPoint.labelRegion.y
          ))
      ) {
      ...
      while (
        isOverlap(currentPoint.labelRegion, previousPoint.labelRegion) ||
        (!this.isVariousRadius() &&
          currentPoint.labelRegion.y + currentPoint.labelRegion.height >
            previousPoint.labelRegion.y)
      ) {
        let newAngle: number = previousPoint.midAngle - count;
        if (!(newAngle <= 270 && newAngle >= 90)) {
          newAngle = 90;
        ...
        if (
          isOverlap(currentPoint.labelRegion, previousPoint.labelRegion) &&
          !series.leftSidePoints.indexOf(previousPoint) &&
          newAngle - 1 < 90 &&
          newAngle - 1 > 270
        ) {
  ...
  private increaseAngle(
    ...
    if (isRightSide) {
      while (
        isOverlap(currentPoint.labelRegion, nextPoint.labelRegion) ||
        (!this.isVariousRadius() &&
          !(
            currentPoint.labelRegion.y + currentPoint.labelRegion.height <
            nextPoint.labelRegion.y
          ))
      ) {
        ...
        if (
          ...
          newAngle + 1 > 90 &&
          newAngle + 1 < 270 &&
          this.rightSideRenderingPoints.indexOf(nextPoint) ===
            this.rightSideRenderingPoints.length - 1
        ) {
          this.changeLabelAngle(currentPoint, currentPoint.labelAngle - 1);
      ...
      while (
        ...
        (!this.isVariousRadius() &&
          currentPoint.labelRegion.y <
            nextPoint.labelRegion.y + nextPoint.labelRegion.height)
      ) {
        let newAngle: number = nextPoint.midAngle + count;
        if (!(newAngle < 270 && newAngle > 90)) {
          newAngle = 270;
  ...
  private changeLabelAngle(currentPoint: AccPoints, newAngle: number): void {
    const dataLabel: AccumulationDataLabelSettingsModel =
      this.accumulation.series[0].dataLabel;
    let variableR: number;
    if (this.isVariousRadius()) {
      variableR = this.accumulation.pieSeriesModule.getLabelRadius(
        this.accumulation.visibleSeries[0],
        currentPoint
      );
    }
    ...
    const labelRadius: number =
      currentPoint.labelPosition === 'Outside' &&
      this.accumulation.enableSmartLabels &&
      dataLabel.position === 'Inside'
        ? this.radius +
          stringToNumber(
        ...
        : !this.isVariousRadius()
        ? this.accumulation.pieSeriesModule.labelRadius + 10
        : variableR;
    const radius: number = !this.isVariousRadius() ? labelRadius : variableR;
    this.getLabelRegion(
      currentPoint,
      'Outside',
      currentPoint.textSize,
      radius,
      this.marginValue,
      newAngle
    );
  ...
  private isOverlapWithNext(
    ...
    for (let i: number = pointIndex; i < points.length; i++) {
      if (
        ...
        point.labelRegion &&
        point.labelVisible &&
        isOverlap(point.labelRegion, points[i as number].labelRegion)
      ) {
        return true;
      }
    }
  ...
  private skipPoints(
    currentPoint: AccPoints,
    halfsidePoints: AccPoints[],
    pointIndex: number
  ): void {
    if (
      pointIndex > 0 &&
      ((currentPoint.midAngle < 285 && currentPoint.midAngle > 255) ||
      ...
      const angleDiff: number =
        (currentPoint.endAngle % 360) - (currentPoint.startAngle % 360);
      const prevAngleDiff: number =
        (previousPoint.endAngle % 360) - (previousPoint.startAngle % 360);
      if (
        prevAngleDiff <= angleDiff &&
        angleDiff < 5 &&
        previousPoint.labelVisible
      ) {
        this.setPointVisibleTrue(currentPoint);
      }
    } else if (
      pointIndex > 1 &&
      ((currentPoint.midAngle < 300 && currentPoint.midAngle > 240) ||
      ...
      const angleDiff: number =
        (currentPoint.endAngle % 360) - (currentPoint.startAngle % 360);
      const prevAngleDiff: number =
        (prevPoint.endAngle % 360) - (prevPoint.startAngle % 360);
      const thirdAngleDiff: number =
        (secondPrevPoint.endAngle % 360) - (secondPrevPoint.startAngle % 360);
      if (
        angleDiff < 3 &&
        prevAngleDiff < 3 &&
        thirdAngleDiff < 3 &&
        prevPoint.labelVisible &&
        currentPoint.labelVisible
      ) {
        this.setPointVisibleTrue(currentPoint);
      }
  ...
  private getPerpendicularDistance(
    ...
    const add: number = 10;
    const height: number =
      add + 10 * Math.sin((point.midAngle * Math.PI) / 360);
    if (point.midAngle > 270 && point.midAngle < 360) {
      increasedLocation = new ChartLocation(
        startPoint.x +
