...
export class PieBase extends AccumulationBase {
  ...
  public initProperties(
    ...
    if (
      (series.radius.indexOf('%') !== -1 || typeof r === 'number') &&
      !isNaN(r)
    ) {
      ...
      for (let i: number = 0; i < Object.keys(series.points).length; i++) {
        if (series.points[i as number].sliceRadius.indexOf('%') !== -1) {
          radiusCollection[i as number] = stringToNumber(
            series.points[i as number].sliceRadius,
            this.size / 2
          );
        } else {
          radiusCollection[i as number] = parseInt(
            series.points[i as number].sliceRadius,
            10
          );
        }
      ...
      this.innerRadius =
        this.innerRadius > minRadius ? this.innerRadius / 2 : this.innerRadius;
    }
    ...
    chart.explodeDistance = series.explode
      ? stringToNumber(series.explodeOffset, this.pieBaseRadius)
      : 0;
    this.findCenter(chart, series);
    this.center = this.pieBaseCenter;
    if (!chart.redraw) {
      this.defaultLabelBound(
        series,
        series.dataLabel.visible,
        series.dataLabel.position
      );
    }
   ...
   * @private
   */
  public getLabelRadius(
    series: AccumulationSeriesModel,
    point: AccPoints
  ): number {
    return series.dataLabel.position === 'Inside'
      ? (stringToNumber(point.sliceRadius, this.pieBaseRadius) -
          this.innerRadius) /
          2 +
          this.innerRadius
  ...
  public findCenter(
    ...
    this.pieBaseCenter = {
      x:
        stringToNumber(
          accumulation.center.x,
          accumulation.initialClipRect.width
        ) + accumulation.initialClipRect.x,
      y:
        stringToNumber(
          accumulation.center.y,
          accumulation.initialClipRect.height
        ) + accumulation.initialClipRect.y,
    };
    const accumulationRect: Rect = this.getSeriesBound(series);
    const accumulationRectCenter: ChartLocation = new ChartLocation(
      accumulationRect.x + accumulationRect.width / 2,
      accumulationRect.y + accumulationRect.height / 2
    );
  ...
  private initAngles(series: AccumulationSeries): void {
    const endAngle: number = isNullOrUndefined(series.endAngle)
      ? series.startAngle
      : series.endAngle;
    this.totalAngle = (endAngle - series.startAngle) % 360;
    this.startAngle = series.startAngle - 90;
    this.totalAngle =
      this.totalAngle <= 0 ? 360 + this.totalAngle : this.totalAngle;
    this.startAngle =
      (this.startAngle < 0 ? this.startAngle + 360 : this.startAngle) % 360;
  }
  ...
  public getSeriesBound(series: AccumulationSeries): Rect {
    ...
    series.findMaxBounds(
      rect,
      new Rect(this.pieBaseCenter.x, this.pieBaseCenter.y, 0, 0)
    );
  ...
  protected getPathArc(
    ...
    if (!innerRadius && innerRadius === 0) {
      ...
      return this.getDoughnutPath(
        ...
        degreeToLocation(end, radius, center),
        radius,
        degreeToLocation(start, innerRadius, center),
        degreeToLocation(end, innerRadius, center),
        innerRadius,
        flag,
        borderRadius,
        isBorder,
  ...
  protected getPiePath(
    ...
    cornerRadius = this.adjustCornerRadius(
      startAngle,
      endAngle,
      radius,
      cornerRadius
    ...
    const cx1: number =
      start.x + cornerRadius * Math.cos(Math.PI / 2 + startAngle);
    const cy1: number =
      start.y + cornerRadius * Math.sin(Math.PI / 2 + startAngle);
   ...
   */
  protected getDoughnutPath(
    center: ChartLocation,
    start: ChartLocation,
    end: ChartLocation,
    radius: number,
    innerStart: ChartLocation,
    innerEnd: ChartLocation,
    innerRadius: number,
    clockWise: number,
    cornerRadius: number,
    isBorder: boolean,
    ...
    cornerRadius = sliceCount === 1 ? 0 : cornerRadius;
    const startAngle: number = Math.atan2(
      start.y - innerStart.y,
      start.x - innerStart.x
    );
    ...
    cornerRadius =
      isBorder && this.innerRadius === 0 ? cornerRadius * -1 : cornerRadius;
    const x1: number = start.x - cornerRadius * Math.cos(startAngle);
    const y1: number = start.y - cornerRadius * Math.sin(startAngle);
    const x2: number = end.x - cornerRadius * Math.cos(Math.PI / 2 + endAngle);
    const y2: number = end.y - cornerRadius * Math.sin(Math.PI / 2 + endAngle);
    ...
    const x4: number =
      innerStart.x + cornerRadius * Math.cos(Math.PI / 2 + startAngle);
    const y4: number =
      innerStart.y + cornerRadius * Math.sin(Math.PI / 2 + startAngle);
    const cx1: number =
      start.x + cornerRadius * Math.cos(Math.PI / 2 + startAngle);
    const cy1: number =
      start.y + cornerRadius * Math.sin(Math.PI / 2 + startAngle);
    ...
    const cy2: number = end.y - cornerRadius * Math.sin(endAngle);
    const cx3: number =
      innerEnd.x - cornerRadius * Math.cos(Math.PI / 2 + endAngle);
    const cy3: number =
      innerEnd.y - cornerRadius * Math.sin(Math.PI / 2 + endAngle);
  ...
  protected doAnimation(
    ...
    const duration: number = this.accumulation.duration
      ? this.accumulation.duration
      : series.animation.duration;
    let value: number;
    ...
    new Animation({}).animate(<HTMLElement>slice, {
      ...
      progress: (args: AnimationOptions): void => {
        ...
        this.accumulation.trigger(
          animationComplete,
          this.accumulation.isBlazor
            ? {}
            : {
                series: series,
                accumulation: this.accumulation,
                chart: this.accumulation,
              }
        );
        const datalabelGroup: Element = getElement(
          this.accumulation.element.id + '_datalabel_Series_' + series.index
        );
        if (datalabelGroup) {
          (datalabelGroup as HTMLElement).style.visibility = this.accumulation
            .isDestroyed
            ? 'hidden'
            : 'visible';
        }
        (groupElement as HTMLElement).style.cssText = '';
        const annotationElement: HTMLElement = <HTMLElement>(
          getElement(this.accumulation.element.id + '_Annotation_Collections')
        );
