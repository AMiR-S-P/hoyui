...
export class TriangularBase extends AccumulationBase {
  ...
  public initProperties(
    ...
    series.triangleSize = new Size(
      stringToNumber(series.width, actualChartArea.width),
      stringToNumber(series.height, actualChartArea.height)
    );

    series.neckSize = new Size(
      stringToNumber(series.neckWidth, actualChartArea.width),
    ...
    chart.explodeDistance = stringToNumber(
      series.explodeOffset,
      actualChartArea.width
    );
  ...
  protected initializeSizeRatio(
    ...
    const coEff: number =
      sumOfPoints !== 0
        ? 1 / (sumOfPoints * (1 + gapRatio / (1 - gapRatio)))
        : 0;
    ...
    for (let i: number = points.length - 1; i >= 0; i--) {
      ...
      if (points[index as number].visible) {
        ...
        points[index as number].yRatio = y;
        points[index as number].heightRatio = height;
        y += height + spacing;
      }
    }
  }
   ...
   * @returns {void}
   */
  protected setLabelLocation(
    series: AccumulationSeries,
    point: AccPoints,
    points: ChartLocation[]
  ): void {
    const last: number = points.length - 1;
    const bottom: number =
      series.type === 'Funnel' ? points.length - 2 : points.length - 1;
    ...
    point.region = new Rect(
      ...
      points[0].y,
      right - x,
      points[bottom as number].y - points[0].y
    );
  ...
  protected getPath(
    ...
    let funnelMinimum: number = Math.min(
      series.neckSize.height,
      series.neckSize.width
    );
    if (funnelMinimum === 0) {
      funnelMinimum =
        series.neckSize.height === 0 && series.neckSize.width === 0
          ? point.region.height
          : series.neckSize.width === 0
          ? series.neckSize.height
          : series.neckSize.width;
    }
    ...
    if (series.type === 'Funnel') {
      borderRadius =
        borderRadius > funnelMinimum / 2 ? funnelMinimum / 2 : borderRadius;
    }
    const angle: number = Math.atan2(
      locations[1].x - locations[2].x,
      locations[1].y - locations[2].y
    );
    const temp: number = borderRadius;
    if (series.type === 'Pyramid') {
      borderRadius =
        point.index === lastIndex && length !== 1 && firstIndex !== lastIndex
          ? 0
          : borderRadius;
      path +=
        locations[0].x -
        temp * Math.sin(-angle) +
        ' ' +
        (locations[0].y + -temp * Math.cos(angle)) +
        ' Q' +
        locations[0].x +
        ' ' +
        locations[0].y +
        ' ' +
        (locations[0].x + borderRadius * Math.sin(-angle)) +
        ' ' +
        (locations[0].y + -borderRadius * Math.cos(-angle));
      path +=
        ' L' +
        (locations[1].x + borderRadius * Math.sin(-angle)) +
        ' ' +
        (locations[1].y + -borderRadius * Math.cos(angle));
      borderRadius = point.index === lastIndex ? temp : 0;
      path +=
        ' L' +
        (locations[2].x - temp * Math.sin(-angle)) +
        ' ' +
        (locations[2].y - -temp * Math.cos(angle)) +
        ' Q' +
        locations[2].x +
        ' ' +
        locations[2].y +
        ' ' +
        (locations[2].x - borderRadius) +
        ' ' +
        locations[2].y;
      path +=
        ' L' +
        (locations[3].x + borderRadius) +
        ' ' +
        locations[3].y +
        ' Q' +
        locations[3].x +
        ...
        locations[3].y +
        ' ' +
        (locations[3].x + temp * Math.sin(-angle)) +
        ' ' +
        (locations[3].y - -temp * Math.cos(angle));
    }
    if (series.type === 'Funnel') {
      borderRadius =
        point.index === firstIndex && length !== 1 && firstIndex !== lastIndex
          ? 0
          : borderRadius;
      path +=
        locations[0].x +
        ...
        locations[0].y +
        ' ' +
        (locations[0].x + borderRadius) +
        ' ' +
        locations[0].y;
      path +=
        ' L' +
        (locations[1].x - borderRadius) +
        ' ' +
        locations[1].y +
        ' Q' +
        locations[1].x +
      ...
      if (series.neckWidth === '0%') {
        const middle: number =
          locations[5].x + (locations[3].x - locations[5].x) / 2;
        path +=
          ' L' +
          (locations[2].x + -borderRadius * Math.sin(-angle)) +
          ' ' +
          (locations[2].y - -borderRadius * Math.cos(angle)) +
          ' Q' +
          middle +
          ' ' +
          locations[2].y +
          ' ' +
          (locations[5].x - -borderRadius * Math.sin(-angle)) +
          ' ' +
          (locations[2].y - -borderRadius * Math.cos(angle));
      } else {
        path =
          series.neckHeight !== '0%' && locations[2].y !== locations[3].y
            ? (path += ' L' + locations[2].x + ' ' + locations[2].y)
            : path;
        const tempX: number =
          series.neckHeight === '0%' ? borderRadius * Math.sin(-angle) : 0;
        const tempY: number =
          series.neckHeight === '0%'
        ...
        path +=
          ...
          locations[4].y +
          ' ' +
          (locations[4 + 1].x + tempX) +
          ' ' +
          (locations[4].y - tempY);
        path =
          series.neckHeight !== '0%' && locations[4].y !== locations[5].y
            ? (path += ' L' + locations[5].x + ' ' + locations[5].y)
            : path;
      }
    }
    return path;
  ...
  protected findPath(
    ...
    if (
      series.borderRadius &&
      (point.index === lastIndex || point.index === firstIndex)
    ) {
      path = this.getPath(
        locations,
        point,
  ...
  public defaultLabelBound(
    ...
    const x: number =
      (chart.initialClipRect.width - series.triangleSize.width) / 2;
    const y: number =
      (chart.initialClipRect.height - series.triangleSize.height) / 2;

    const accumulationBound: Rect = new Rect(
      x,
      y,
      series.triangleSize.width,
      series.triangleSize.height
    );
