...
} from '../../accumulation-chart/model/pie-interface';
...
export class AccumulationLegend extends BaseLegend {
  ...
  private mouseMove(e: MouseEvent): void {
    if (this.chart.legendSettings.visible && !this.chart.isTouch) {
      if (
        (<AccumulationChart>this.chart).accumulationHighlightModule &&
        (<AccumulationChart>this.chart).highlightMode !== 'None'
      ) {
        if (!(<AccumulationChart>this.chart).legendSettings.toggleVisibility) {
          this.click(e);
        }
        ...
        for (const id of legendItemsId) {
          if (targetId.indexOf(id) > -1) {
            ...
            (<AccumulationChart>(
              this.chart
            )).accumulationHighlightModule.legendSelection(
              <AccumulationChart>this.chart,
              0,
              index,
              e.target as Element,
              e.type
  ...
  public getLegendOptions(
    ...
    for (let i: number = 0; i < 1; i++) {
      ...
      if (seriesType === 'Pie' || seriesType === <AccumulationType>'Doughnut') {
        seriesType =
          series[i as number].innerRadius !== '0' &&
          series[i as number].innerRadius !== '0%'
            ? <AccumulationType>'Doughnut'
            : 'Pie';
      }
      for (const point of series[i as number].points) {
        if (!isNullOrUndefined(point.x) && !isNullOrUndefined(point.y)) {
          this.legendCollections.push(
            new LegendOptions(
              chart.useGroupingSeparator && typeof point.x === 'number'
                ? chart.intl.formatNumber(point.x as number, {
                    useGrouping: true,
                  })
                : point.x.toString(),
              point.color,
  ...
  public click(event: Event): void {
    ...
    for (const id of legendItemsId) {
      if (targetId.indexOf(id) > -1) {
        ...
        if (
          (this.chart as AccumulationChart).legendSettings.toggleVisibility &&
          !isNaN(pointIndex)
        ) {
          const currentSeries: AccumulationSeries = (<AccumulationChart>(
            this.chart
          )).visibleSeries[0];
          const point: AccPoints = pointByIndex(
            pointIndex,
          ...
          const legendClickArgs: IAccLegendClickEventArgs = {
            ...
            chart: chart.isBlazor ? ({} as AccumulationChart) : chart,
          ...
          (<AccumulationChart>this.chart).accumulationSelectionModule &&
          !isNaN(pointIndex)
        ) {
          (<AccumulationChart>(
            this.chart
          )).accumulationSelectionModule.legendSelection(
            <AccumulationChart>this.chart,
            0,
            pointIndex,
  ...
  private sliceVisibility(index: number, isVisible: boolean): void {
    ...
    if (
      ((this.chart as AccumulationChart).visibleSeries[0] as AccumulationSeries)
        .dataLabel.visible
    ) {
