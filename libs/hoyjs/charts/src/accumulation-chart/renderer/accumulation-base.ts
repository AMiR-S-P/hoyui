...
export class AccumulationBase {
  ...
  public get center(): ChartLocation {
    return (
      ...
      (this.accumulation.visibleSeries[0].type === 'Pie'
        ? this.accumulation.pieSeriesModule.pieBaseCenter
        : null)
    );
  }
  ...
  public get radius(): number {
    return this.pieRadius !== undefined
      ? this.pieRadius
  ...
  public get labelRadius(): number {
    return this.pieLabelRadius !== undefined
      ? this.pieLabelRadius
  ...
  public processExplode(event: Event): void {
    if (
      (<HTMLElement>event.target).id.indexOf('_Series_') > -1 ||
      (<HTMLElement>event.target).id.indexOf('_datalabel_') > -1
    ) {
      const pointIndex: number = indexFinder(
      ...
      this.deExplodeAll(
        pointIndex,
        this.accumulation.enableAnimation ? 300 : 0
      );
    }
  }
  ...
  public invokeExplode(): void {
    ...
    if (
      this.accumulation.accumulationSelectionModule &&
      this.accumulation.selectionMode !== 'None' &&
      this.accumulation.accumulationSelectionModule.selectedDataIndexes.length
    ) {
      for (const index of this.accumulation.accumulationSelectionModule
        .selectedDataIndexes) {
  ...
  public deExplodeAll(index: number, animationDuration: number): void {
    ...
    for (const currentPoint of points) {
      if (
        (index !== currentPoint.index && !currentPoint.isSliced) ||
        currentPoint.isClubbed
      ) {
  ...
  public explodePoints(
    ...
    const clubPointsExploded: boolean =
      !explode &&
      (point.isSliced ||
        (series.clubbedPoints.length &&
          points[points.length - 1].index ===
            series.clubbedPoints[series.clubbedPoints.length - 1].index));
    if (series.type === 'Pie' && (clubPointsExploded || point.isClubbed)) {
      explodePoints = this.clubPointExplode(
        index,
        point,
        series,
        points,
  ...
  private clubPointExplode(
    ...
    chart: AccumulationChart,
    duration: number,
    clubPointsExploded: boolean = false
  ): boolean {
    if (point.isClubbed) {
      ...
      chart.animateSeries = false;
      points.splice(
        points.length - series.clubbedPoints.length,
        series.clubbedPoints.length
      );
      ...
      this.deExplodeSlice(
        clubPoint.index,
        chart.element.id + '_Series_0_Point_',
        duration
      );
      if (point.isSliced) {
        return false;
  ...
  private pointExplode(
    ...
    if (!this.isCircular()) {
      translate = {
        x:
          point.labelRegion && point.labelRegion.x < point.region.x
            ? -chart.explodeDistance
            : chart.explodeDistance,
        y: 0,
      };
    } else {
      translate = degreeToLocation(
        point.midAngle,
        chart.explodeDistance,
        this.center
      );
  ...
  private isExplode(id: string): boolean {
    ...
    const transform: string = element
  ...
  private explodeSlice(
    ...
    if (
      pointAnimation &&
      getElement(
        this.accumulation.element.id + '_Series_0_Point_' + index
      ).getAttribute('transform')
    ) {
      this.setElementTransform(
        sliceId + index,
        `translate(${translate.x - center.x}, ${translate.y - center.y})`
      );
    } else {
      this.performAnimation(
        index,
  ...
  private performAnimation(
    ...
    const seriesIndex: number = parseInt(
      sliceId.split('_')[values.length - 3],
      10
    );
    const point: AccPoints =
      chart.visibleSeries[seriesIndex as number].points[index as number];
    if (duration <= 0) {
      this.setTranslate(
        index,
        sliceId,
    ...
    new Animation({}).animate(createElement('div'), {
      ...
      progress: (args: AnimationOptions): void => {
        ...
        this.setTranslate(
          ...
          sliceId,
          'translate(' +
            (isReverse ? endX - xValue : xValue) +
            ', ' +
            (isReverse ? endY - yValue : yValue) +
            ')',
          point.transform
        );
      },
        ...
        this.setTranslate(
          ...
          sliceId,
          'translate(' +
            (isReverse ? startX : endX) +
            ', ' +
            (isReverse ? startX : endY) +
            ')',
          point.transform
        );
      },
