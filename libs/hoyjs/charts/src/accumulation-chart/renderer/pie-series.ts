...
export class PieSeries extends PieBase {
  ...
  public renderPoint(
    ...
    const degree: number = sum
      ? (Math.abs(yValue) / sum) * this.totalAngle
      : null;
    const start: number = (Math.PI / 180) * (90 - (360 - this.startAngle) - 90);
    this.radius = this.isRadiusMapped
      ? stringToNumber(point.sliceRadius, this.seriesRadius)
      : this.radius;
    option.d = this.getPathOption(
      point,
      degree,
      this.startAngle % 360,
      borderRadius,
    ...
    point.symbolLocation = degreeToLocation(
      point.midAngle,
      (this.radius + this.innerRadius) / 2,
      this.center
    );
    if (!redraw) {
  ...
  public findSeries(e: PointerEvent | TouchEvent, borderRadius: number): void {
    ...
    const radius: number =
      this.innerRadius === 0
        ? this.radius + borderGap
        : this.innerRadius - borderGap;
    const innerRadius: number =
      this.innerRadius === 0 ? radius + width : radius - width;
    this.toggleInnerPoint(e, radius, innerRadius, borderRadius);
  }

  public toggleInnerPoint(
    ...
    const accumulationId: string = (event.target as Element).id.substring(
      0,
      (event.target as Element).id.indexOf('Series') - 1
    );
    let borderElement: Element = document.getElementById(
      this.accumulation.element.id + 'PointHover_Border'
    );
    let createBorderEle: Element;
    const seriesPoints: AccPoints[] = this.accumulation.visibleSeries[0].points;
    const seriesIndex: number = id.series;
    const pointIndex: number = id.point;
    const srcElem: Element = getElement(
      accumulationId + '_Series_' + seriesIndex + '_Point_' + pointIndex
    );
    if (!isNaN(id.series) && srcElem) {
      if (
        !isNullOrUndefined(seriesIndex) &&
        !isNaN(seriesIndex) &&
        !isNullOrUndefined(pointIndex) &&
        !isNaN(pointIndex)
      ) {
        const point: AccPoints =
          this.accumulation.visibleSeries[0].points[pointIndex as number];
        const opacity: number =
          srcElem.getAttribute('class') === accumulationId + '_ej2_deselected'
            ? this.accumulation.tooltip.enable
              ? 0.5
              : 0.3
            : this.accumulation.tooltip.enable
            ? 0.5
        ...
        const seriousGroup: Element = getElement(
          accumulationId + '_Series_' + seriesIndex
        );
        if (
          !borderElement &&
          (!point.isExplode || (point.isExplode && event.type !== 'click'))
        ) {
          const path: PathOption = new PathOption(
            accumulationId + 'PointHover_Border',
            point.color,
            1,
            point.color,
            opacity,
          ...
          if (
            ...
            (<Element>event.target).hasAttribute('class')
          ) {
            this.accumulation.accumulationSelectionModule.addSvgClass(
              createBorderEle,
              (<Element>event.target).getAttribute('class')
            );
          }
  ...
  private refresh(
    ...
    new Animation({}).animate(createElement('div'), {
      ...
      progress: (args: AnimationOptions): void => {
        const curentDegree: number = linear(
          args.timeStamp,
          point.degree,
          degree - point.degree,
          args.duration
        );
        let currentStartAngle: number = linear(
          args.timeStamp,
          point.start,
          start - point.start,
          args.duration
        ...
        if (previousRadius && previouCenter) {
          const currentRadius: number = linear(
            args.timeStamp,
            previousRadius,
            this.radius - previousRadius,
            args.duration
          );
          const previouCenterx: number = linear(
            args.timeStamp,
            previouCenter.x,
            this.center.x - previouCenter.x,
            args.duration
          );
          const previouCentery: number = linear(
            args.timeStamp,
            previouCenter.y,
            this.center.y - previouCenter.y,
            args.duration
          );
        ...
        if (point.isExplode) {
          chart.accBaseModule.explodePoints(
            point.index,
            chart,
            true,
            pointAnimation
        ...
        (seriesElement as HTMLElement).style.visibility = point.visible
          ? 'visible'
          : 'hidden';
  ...
  private getPathOption(
    ...
    const path: string = this.getPathArc(
      previousCenterX
        ? { x: previousCenterX, y: previousCenterY }
        : this.center,
      startAngle % 360,
      (startAngle + degree) % 360,
      this.isRadiusMapped
  ...
  public animateSeries(
    ...
    const groupId: string =
      accumulation.element.id + 'SeriesGroup' + series.index;
    if (
      ((series.animation.enable && animationMode !== 'Disable') ||
        animationMode === 'Enable') &&
      accumulation.animateSeries
    ) {
      const clippath: Element = accumulation.renderer.createClipPath({
