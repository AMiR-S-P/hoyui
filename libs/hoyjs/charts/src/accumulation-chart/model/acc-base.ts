...
import { isNullOrUndefined, getValue } from '@hoyui/base';
...
import { Rect, Size, PathOption, measureText } from '@hoyui/svg-base';
import {
  ChartLocation,
  stringToNumber,
  appendChildElement,
...
export class AccumulationSeries extends ChildProperty<AccumulationSeries> {
   ...
   */
  @Complex<AccumulationDataLabelSettingsModel>(
    {},
  ...
  public refreshDataManager(
    ...
    this.radius = this.radius
      ? this.radius
      : Browser.isDevice && this.dataLabel.position === 'Outside'
      ? '40%'
      : '80%';
    const dateSource: Object | DataManager =
      this.dataSource || accumulation.dataSource;
    ...
    dataManager.then((e: { result: Object; count: number }) =>
      this.dataManagerSuccess(e, accumulation)
    );
  }
  /**
  ...
  public dataManagerSuccess(
    ...
    if (
      (++accumulation.seriesCounts === accumulation.visibleSeries.length &&
        render) ||
  ...
  public getPoints(result: Object, accumulation: AccumulationChart): void {
    ...
    const colors: string[] = this.palettes.length
      ? this.palettes
      : getSeriesColor(accumulation.theme);
    ...
    if (
      this.clubbedPoints.length &&
      this.explode &&
      this.type === 'Pie' &&
      (this.explodeAll ||
        this.points[this.points.length - 1].index === this.explodeIndex)
    ) {
  ...
  public generateClubPoint(): AccPoints {
    ...
    clubPoint.text = clubPoint.originalText =
      clubPoint.x + ': ' + this.sumOfClub;
  ...
  private findSumOfPoints(result: Object): void {
    ...
    for (let i: number = 0; i < length; i++) {
      if (
        !isNullOrUndefined(result[i as number]) &&
        !isNullOrUndefined(result[i as number][this.yName]) &&
  ...
  public renderSeries(accumulation: AccumulationChart, redraw?: boolean): void {
    const seriesGroup: Element = redraw
      ? getElement(accumulation.element.id + '_Series_' + this.index)
      : accumulation.renderer.createGroup({
    ...
    if (this.type === 'Pie') {
      ...
      accumulation.pieSeriesModule.animateSeries(
        accumulation,
        this.animation,
        this,
        seriesGroup,
    ...
    if (!accumulation.redraw && accumulation.accumulationLegendModule) {
      ...
      this.labelBound.height +=
        accumulation.explodeDistance - this.labelBound.y;
  ...
  private renderPoints(
    ...
    seriesGroup: Element,
    redraw?: boolean,
    previouRadius?: number,
    previousCenter?: ChartLocation,
    pointAnimation?: boolean
  ): void {
    const pointId: string =
      accumulation.element.id + '_Series_' + this.index + '_Point_';
    let option: PathOption;
    let patternFill: string;
    const patterns: SelectionPattern[] = [
      'Chessboard',
      'Dots',
      'DiagonalForward',
      'Crosshatch',
    ...
    for (const point of this.points) {
      const argsData: IAccPointRenderEventArgs = {
        ...
        pattern: this.applyPattern
          ? patterns[point.index % patterns.length]
          : 'None',
      };
      ...
      option = new PathOption(
        pointId + point.index,
        patternFill,
        argsData.border.width || 1,
        argsData.border.color || point.color,
        this.opacity,
        argsData.series.dashArray,
        ''
      );
      accumulation[firstToLowerCase(this.type) + 'SeriesModule'].renderPoint(
        point,
        this,
        accumulation,
        option,
  ...
  private renderDataLabel(
    ...
    for (const point of this.points) {
      if (point.visible) {
        if (
          this.dataLabel.showZero ||
          (!this.dataLabel.showZero &&
            (point.y !== 0 ||
              (point.y === 0 && this.emptyPointSettings.mode === 'Zero')))
        ) {
    ...
    this.leftSidePoints.sort(
      (a: AccPoints, b: AccPoints) => a.midAngle - b.midAngle
    );
    this.rightSidePoints = firstQuarter.concat(secondQuarter);
    accumulation.accumulationDataLabelModule.drawDataLabels(
      this,
      this.dataLabel,
      datalabelGroup as HTMLElement,
      element,
      redraw
    );
    if (this.dataLabel.template !== null && element.childElementCount) {
      const dataLabelCallBack: Function =
        accumulation.accumulationDataLabelModule.drawDataLabels.bind(
          accumulation.accumulationDataLabelModule,
          this,
          this.dataLabel,
          datalabelGroup,
          element,
      ...
      if ((accumulation as any).isReact) {
        (accumulation as any).renderReactTemplates(dataLabelCallBack);
      }
      appendChildElement(
        false,
        getElement(accumulation.element.id + '_Secondary_Element'),
        element,
        redraw
      );
    }
  ...
  public findMaxBounds(totalbound: Rect, bound: Rect): void {
    ...
    totalbound.height =
      bound.y + bound.height > totalbound.height
  ...
  public setData(data: Object[], duration?: number): void {
    ...
    if (!samePoints) {
      ...
      this.accumulation.redraw = this.borderRadius
        ? false
        : this.accumulation.enableAnimation;
      ...
      ].initProperties(this.accumulation, this);
      this.renderPoints(
        this.accumulation,
        getElement(this.accumulation.element.id + '_Series_' + this.index),
      ...
      if (this.accumulation.annotationModule) {
        this.accumulation.annotationModule.renderAnnotations(
          getElement(this.accumulation.element.id + '_Secondary_Element')
        );
      }
      if (
        this.accumulation.accumulationDataLabelModule &&
        this.dataLabel.visible
      ) {
        this.renderDataLabel(
          this.accumulation,
          getElement(
            this.accumulation.element.id + '_datalabel_Series_' + this.index
          ),
          this.accumulation.redraw
        );
      }
  ...
  public addPoint(dataPoint: Object, duration?: number): void {
    ...
    const colors: string[] = this.palettes.length
      ? this.palettes
      : getSeriesColor(this.accumulation.theme);
    const point: AccPoints = this.setPoints(
      this.dataSource,
      pointIndex,
      colors,
      this.accumulation
    ...
    if (dataSource.length > 0 && index >= 0 && index < dataSource.length) {
      ...
      for (let i: number = 0; i < (this.dataSource as object[]).length; i++) {
        if (
          ...
          this.points[i as number].visible
        ) {
      ...
      const element: Element = getElement(
        this.accumulation.element.id + '_Series_0_Point_' + this.points.length
      );
      if (element) {
  ...
  private updateSeries(
    ...
    const previousRadius: number =
      this.accumulation[firstToLowerCase(this.type) + 'SeriesModule'].radius;
    const previousCenter: ChartLocation =
      this.accumulation[firstToLowerCase(this.type) + 'SeriesModule'].center;
    ...
    if (this.accumulation.legendSettings.visible) {
      if (updatePoint === 'addPoint') {
        this.accumulation.accumulationLegendModule.legendCollections.push(
          new LegendOptions(
            this.points[this.points.length - 1].x.toString(),
            this.points[this.points.length - 1].color,
            this.legendShape,
            this.points[this.points.length - 1].visible,
            this.type,
        ...
        this.accumulation.accumulationLegendModule.legendCollections.splice(
          index,
          1
        );
        for (
          let i: number = index;
          i <
          this.accumulation.accumulationLegendModule.legendCollections.length;
          i++
        ) {
          this.accumulation.accumulationLegendModule.legendCollections[
            i as number
          ].pointIndex = i;
        }
      }
      if (
        this.accumulation.accumulationLegendModule.legendCollections.length >= 1
      ) {
        ...
        this.accumulation.accumulationLegendModule.calculateLegendBounds(
          ...
          this.accumulation.availableSize,
          null,
          previousLegendBounds,
          true
        );
        ...
        if (
          this.dataLabel &&
          this.dataLabel.position === 'Outside' &&
          (this.accumulation.legendSettings.position === 'Bottom' ||
            this.accumulation.legendSettings.position === 'Top')
            ? previousLegendBounds.height !==
              this.accumulation.accumulationLegendModule.legendBounds.height
            : previousLegendBounds.width !==
              this.accumulation.accumulationLegendModule.legendBounds.width
        ) {
          const titleHeight: number = this.accumulation.title
            ? measureText(
                this.accumulation.title,
                this.accumulation.titleStyle,
                this.accumulation.themeStyle.chartTitleFont
              ).height * this.accumulation.titleCollection.length
            : 0;

          const subTitleHeight: number = this.accumulation.subTitle
            ? measureText(
                this.accumulation.subTitle,
                this.accumulation.subTitleStyle,
                this.accumulation.themeStyle.chartSubTitleFont
              ).height * this.accumulation.subTitleCollection.length
            : 0;

          this.accumulation.initialClipRect = new Rect(
            this.accumulation.margin.left,
          ...
          subtractRect(
            ...
            new Rect(
              ...
              this.accumulation.margin.right + this.accumulation.margin.left,
              this.accumulation.margin.bottom + this.accumulation.margin.top
            )
          );
          this.accumulation.accumulationLegendModule.calculateLegendBounds(
            this.accumulation.initialClipRect,
    ...
    if (
      ...
      this.accumulation.legendSettings.position === 'Top'
        ...
        : previousLegendBounds.width !==
          this.accumulation.accumulationLegendModule.legendBounds.width) &&
      this.accumulation.centerLabel.text
    ) {
      this.accumulation.renderCenterLabel(true, true);
    }
    if (this.accumulation.annotationModule) {
    ...
    if (this.accumulation.legendSettings.visible) {
      if (this.type === 'Pie') {
        if (
          ...
          this.points[this.points.length - 1].textSize.width > maxLabelWidth &&
          this.accumulation.legendSettings.position !== 'Top' &&
          this.accumulation.legendSettings.position !== 'Bottom'
        ) {
          ...
          this.labelBound.height +=
            this.accumulation.explodeDistance - this.labelBound.y;
          this.labelBound.width +=
            this.accumulation.explodeDistance - this.labelBound.x;
        }
        this.accumulation.accumulationLegendModule.getSmartLegendLocation(
          this.accumulation.visibleSeries[0].labelBound,
          this.accumulation.accumulationLegendModule.legendBounds,
          this.accumulation.margin
        );
      }
      this.accumulation.accumulationLegendModule.renderLegend(
        this.accumulation,
        this.accumulation.legendSettings,
        this.accumulation.accumulationLegendModule.legendBounds,
        this.accumulation.redraw,
        true
      );
 ...
 */
export function getSeriesFromIndex(
  index: number,
