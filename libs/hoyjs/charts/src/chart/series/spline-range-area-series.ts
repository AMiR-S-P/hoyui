...
export class SplineRangeAreaSeries extends SplineBase {
  ...
  public render(
    ...
    const borderColor: string = series.border.color
      ? series.border.color
      : series.interior;
    ...
    for (let i: number = 0; i < length; i++) {
      ...
      const lowPtCoordinate: ChartLocation = getPoint(
        point.xValue,
        lowPoint,
        xAxis,
        yAxis,
      ...
      const highPtCoordinate: ChartLocation = getPoint(
        point.xValue,
        highPoint,
        xAxis,
        yAxis,
      ...
      const rect1: Rect = new Rect(
        Math.min(lowPtCoordinate.x, highPtCoordinate.x),
        Math.min(lowPtCoordinate.y, highPtCoordinate.y),
        Math.max(
          Math.abs(highPtCoordinate.x - lowPtCoordinate.x),
          series.marker.width
        ),
        Math.max(
          Math.abs(highPtCoordinate.y - lowPtCoordinate.y),
          series.marker.width
        )
      );
      ...
      if (
        point.visible &&
        withInRange(
          visiblePoint[previous as number],
          point,
          visiblePoint[next as number],
          series
        )
      ) {
        if (firstPoint) {
          ...
          if (yAxis.isAxisInverse) {
            ...
            this.borderDirection +=
              'M ' + highPtCoordinate.x + ' ' + highPtCoordinate.y + ' ';
        ...
        if (
          (i + 1 < visiblePoint.length && !visiblePoint[i + 1].visible) ||
          i === visiblePoint.length - 1
        ) {
          // Path to connect the low points
          direction = this.closeSplineRangeAreaPath(
            visiblePoint,
            point,
            series,
            direction,
  ...
  protected closeSplineRangeAreaPath(
    visiblePoint: Points[],
    point: Points,
    series: Series,
    direction: string,
    ...
    for (let j: number = i; j > 0; j--) {
      if (visiblePoint[j as number].visible) {
        ...
        if (firstPoint) {
          ...
          if (yAxis.isAxisInverse) {
            direction = direction.concat(
              'L ' + highPtCoordinate.x + ' ' + highPtCoordinate.y + ' '
            );
            this.borderDirection +=
              'M ' + highPtCoordinate.x + ' ' + highPtCoordinate.y + ' ';
          } else {
            direction = direction.concat(
              'L ' + lowPtCoordinate.x + ' ' + lowPtCoordinate.y + ' '
            );
            this.borderDirection +=
              'M ' + lowPtCoordinate.x + ' ' + lowPtCoordinate.y + ' ';
          }
        }
      } else {
        break;
  ...
  public updateDirection(series: Series, point: number[]): void {
    ...
    for (let i: number = 0; i < point.length; i++) {
      if (series.marker && series.marker.visible) {
        series.points[i as number].symbolLocations.map(function (
        ...
        for (
          ...
          if (endPathCommands.length !== startPathCommands.length) {
            startPathCommands.splice(
              startPathCommands.length / 2 + 1,
              0,
              'C ' +
                startPathCommands[(startPathCommands.length - 1) / 2]
                  .split(' ')
                  .slice(5, 7)
                  .join(' ') +
                ' ' +
            ...
            startPathCommands.splice(
              startPathCommands.length / 2 + 2,
              0,
              'C ' +
                startPathCommands[startPathCommands.length / 2 + 1]
                  .split(' ')
                  .slice(1, 3)
                  .join(' ') +
                ' ' +
        ...
        for (let i: number = minLength; i < maxLength; i++) {
          if (endPathCommands.length !== startPathCommands.length) {
            endPathCommands.splice(
              ...
              0,
              'C ' +
                endPathCommands[1].split(' ').slice(-3).join(' ') +
                endPathCommands[1].split(' ').slice(-3).join(' ') +
                endPathCommands[1].split(' ').slice(-3).join(' ')
            );
            endPathCommands.splice(
              ...
              0,
              'C ' +
                endPathCommands[endPathCommands.length - 2]
                  .split(' ')
                  .slice(-4)
                  .join(' ') +
                endPathCommands[endPathCommands.length - 2]
