...
export class StackingAreaSeries extends LineBase {
  ...
  public render(
    ...
    const getCoordinate: Function = polarAreaType
      ? TransformToVisible
      : getPoint;
    ...
    if (pointsLength > 0) {
      ...
      lineDirection = lineDirection.concat(
        'M' + ' ' + point1.x + ' ' + point1.y + ' '
      );
    }
    const isPolar: boolean =
      series.chart && series.chart.chartAreaType === 'PolarRadar';
    let index: number;
    ...
    for (let i: number = 0; i < pointsLength; i++) {
      ...
      visiblePoints[i as number].symbolLocations = [];
      visiblePoints[i as number].regions = [];
      if (
        visiblePoints[i as number].visible &&
        withInRange(
          visiblePoints[i - 1],
        ...
        const startvalue: number =
          series.index > 0 && index !== undefined
            ? this.chart.visibleSeries[index as number].stackedValues.endValues[
                pointIndex as number
              ]
            : stackedvalue.startValues[pointIndex as number];
        point1 = getCoordinate(
          visiblePoints[i as number].xValue,
          !series.visible && series.isLegendClicked
            ? startvalue
            : stackedvalue.endValues[pointIndex as number],
          xAxis,
          yAxis,
          isInverted,
          series
        ...
        visiblePoints[i as number].regions.push(
          new Rect(
            visiblePoints[i as number].symbolLocations[0].x -
              series.marker.width,
            visiblePoints[i as number].symbolLocations[0].y -
              series.marker.height,
        ...
        if (!isPolar && series.emptyPointSettings.mode !== 'Drop') {
          ...
          if (
            visiblePoints[i + 1] &&
            visiblePoints[i + 1].visible &&
            (!isPolar ||
              (isPolar && this.withinYRange(visiblePoints[i + 1], yAxis)))
          ) {
            nextPointIndex = visiblePoints[i + 1].index;
            point1 = getCoordinate(
              visiblePoints[i + 1].xValue,
              stackedvalue.startValues[nextPointIndex as number],
              xAxis,
              yAxis,
              isInverted,
              series
    ...
    if (
      ...
      const connectPoints: { first: Points; last: Points } =
        this.getFirstLastVisiblePoint(series.points);
      ...
      if (
        this.chart.visible === 1 &&
        (xAxis.isAxisInverse || yAxis.isAxisInverse)
      ) {
        this.chart.enableAnimation = false;
        lineDirection =
          series.type === 'Polar'
            ? chart.polarSeriesModule.getPolarIsInversedPath(
                xAxis,
                lineDirection
              )
            : chart.radarSeriesModule.getRadarIsInversedPath(
                xAxis,
                lineDirection
              );
      }
    ...
    if (
      !isPolar ||
      (isPolar &&
        series.index !== this.getFirstSeriesIndex(series.chart.visibleSeries))
    ) {
      for (let j: number = pointsLength - 1; j >= startPoint; j--) {
        ...
        if (
          previousSeries.emptyPointSettings.mode !== 'Drop' ||
          !previousSeries.points[j as number].isEmpty
        ) {
          point2 = getCoordinate(
            visiblePoints[j as number].xValue,
            !series.visible &&
              ...
              series.index > 0 &&
              index !== undefined
              ? this.chart.visibleSeries[index as number].stackedValues
                  .endValues[pointIndex as number]
              : stackedvalue.startValues[pointIndex as number],
            xAxis,
            yAxis,
            isInverted,
          ...
          lineDirection = lineDirection.concat(
            (j === pointsLength - 1 && polarAreaType ? 'M' : 'L') +
              ' ' +
              point2.x +
              ' ' +
              point2.y +
    ...
    if (series.border.width !== 0 && series.visible) {
      emptyPointDirection = this.removeEmptyPointsBorder(
        this.getBorderDirection(lineDirection)
      );
      const options: PathOption = new PathOption(
        series.chart.element.id + '_Series_border_' + series.index,
        'transparent',
        series.visible ? series.border.width : 0,
        series.border.color ? series.border.color : series.interior,
        1,
        series.dashArray,
        emptyPointDirection
      );
  ...
  public updateDirection(series: Series, point: number[]): void {
    for (let i: number = 0; i < series.xAxis.series.length; i++) {
      ...
      this.render(
        ...
        stackSeries.xAxis,
        stackSeries.yAxis,
        stackSeries.chart.requireInvertedAxis,
        false,
        true
      );
      for (let j: number = 0; j < point.length; j++) {
        if (stackSeries.marker && stackSeries.marker.visible) {
          stackSeries.chart.markerRender.renderMarker(
            stackSeries,
            stackSeries.points[point[j as number]],
            stackSeries.points[point[j as number]].symbolLocations[0],
            null,
            true
          );
        }
        if (
          stackSeries.marker.dataLabel.visible &&
          stackSeries.chart.dataLabelModule
        ) {
          stackSeries.chart.dataLabelModule.commonId =
            stackSeries.chart.element.id +
            '_Series_' +
            stackSeries.index +
            '_Point_';
          stackSeries.chart.dataLabelModule.renderDataLabel(
            stackSeries,
            stackSeries.points[point[j as number]],
            null,
            stackSeries.marker.dataLabel
          );
        }
      }
    }
  ...
  public addAreaPath(
    ...
    if (points.previousDirection !== '' && options.d !== '') {
      ...
      if (startPathCommands.length < endPathCommands.length) {
        for (
          let i: number = startPathCommands.length;
          i < endPathCommands.length;
          i++
        ) {
          if (endPathCommands.length !== startPathCommands.length) {
            startPathCommands.splice(
              (startPathCommands.length + 1) / 2,
              0,
        ...
        for (let i: number = minLength; i < maxLength; i++) {
          if (endPathCommands.length !== startPathCommands.length) {
            ...
            endPathCommands.splice(
              ...
              0,
              endPathCommands[endPathCommands.length - 1]
            );
          }
        }
        animateAddPoints(
  ...
  private getPreviousSeries(series: Series): Series {
    ...
    for (
      let i: number = 0, length: number = seriesCollection.length;
      i < length;
      i++
    ) {
