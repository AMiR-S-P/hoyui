...
export class StackingBarSeries extends ColumnBase {
  ...
  public renderPoint(
    ...
    if (
      pointStack.visible &&
      withInRange(
        series.points[pointStack.index - 1],
        pointStack,
        series.points[pointStack.index + 1],
        series
      )
      ...
      if (!series.visible && series.isLegendClicked) {
        ...
        startvalue =
          series.index > 0 && index !== undefined
            ? series.chart.visibleSeries[index as number].stackedValues
                .endValues[pointStack.index]
            : series.stackedValues.startValues[pointStack.index];
      }
      this.rect = this.getRectangle(
        pointStack.xValue + sideBySideInfo.start,
        !series.visible && series.isLegendClicked
          ? startvalue
          : stackedValue.endValues[pointStack.index],
        pointStack.xValue + sideBySideInfo.end,
        !series.visible && series.isLegendClicked
          ? startvalue
          : stackedValue.startValues[pointStack.index],
        series
      );
      this.rect.height = series.columnWidthInPixel
        ? series.columnWidthInPixel
        : this.rect.height;
      this.rect.y = series.columnWidthInPixel
  ...
  public updateDirection(series: Series, point: number[]): void {
    ...
    for (let j: number = 0; j < seriesCollection.length; j++) {
      for (let i: number = 0; i < point.length; i++) {
        this.renderPoint(
          seriesCollection[j as number],
          seriesCollection[j as number].points[point[i as number]],
          this.sideBySideInfo[series.index],
          seriesCollection[j as number].stackedValues
        );
        ...
        if (
          seriesCollection[j as number].marker.dataLabel.visible &&
          seriesCollection[j as number].chart.dataLabelModule
        ) {
          seriesCollection[j as number].chart.dataLabelModule.commonId =
            seriesCollection[j as number].chart.element.id +
            '_Series_' +
            seriesCollection[j as number].index +
            '_Point_';
          seriesCollection[j as number].chart.dataLabelModule.renderDataLabel(
            seriesCollection[j as number],
            seriesCollection[j as number].points[point[i as number]],
            null,
            seriesCollection[j as number].marker.dataLabel
          );
        }
      }
    }
