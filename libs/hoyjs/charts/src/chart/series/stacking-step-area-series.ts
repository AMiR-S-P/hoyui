...
export class StackingStepAreaSeries extends LineBase {
  ...
  public render(
    ...
    let currentPointLocation: ChartLocation;
    ...
    const origin: number = Math.max(
      stackSeries.yAxis.visibleRange.min,
      stackedvalue.startValues[0]
    );
    const pointsLength: number = visiblePoint.length;
    let options: PathOption;
    let point: Points;
    let point2: ChartLocation;
    let point3: ChartLocation;
    let xValue: number;
    ...
    for (let i: number = 0; i < pointsLength; i++) {
      ...
      if (
        ...
        if (start === null) {
          ...
          direction +=
            'M' +
            ' ' +
            currentPointLocation.x +
            ' ' +
            currentPointLocation.y +
            ' ';
          currentPointLocation = getPoint(
            xValue - lineLength,
        ...
        if (prevPoint != null) {
          ...
          secondPoint = getPoint(
            prevPoint.xValue,
            stackedvalue.endValues[prevPoint.index],
            xAxis,
            yAxis,
            isInverted
          );
          direction += this.GetStepLineDirection(
          ...
          borderDirection += this.GetStepLineDirection(
            currentPointLocation,
            secondPoint,
            stackSeries.step,
            'L',
        ...
        visiblePoint[i as number].symbolLocations.push(
          getPoint(
            visiblePoint[i as number].xValue,
            stackedvalue.endValues[pointIndex as number],
            xAxis,
            yAxis,
            isInverted
        ...
        visiblePoint[i as number].regions.push(
          new Rect(
            visiblePoint[i as number].symbolLocations[0].x -
              stackSeries.marker.width,
            visiblePoint[i as number].symbolLocations[0].y -
              stackSeries.marker.height,
            2 * stackSeries.marker.width,
            2 * stackSeries.marker.height
          )
        );
        prevPoint = point;
      }
      ...
      if (
        visiblePoint[i + 1] &&
        !visiblePoint[i + 1].visible &&
        start !== null &&
        stackSeries.emptyPointSettings.mode !== 'Drop'
      ) {
        ...
        for (let j: number = i; j >= startPoint; j--) {
          ...
          currentPointLocation = getPoint(
            visiblePoint[pointIndex as number].xValue,
          ...
          if (
            ...
            secondPoint = getPoint(
              visiblePoint[previousPointIndex as number].xValue,
              stackedvalue.startValues[previousPointIndex as number],
              xAxis,
              yAxis,
              isInverted
    ...
    if (direction !== '') {
      ...
      if (pointsLength > 1) {
        ...
        start = {
          ...
          y: stackedvalue.endValues[pointIndex as number],
        };
        ...
        start = {
          x: visiblePoint[pointsLength - 1].xValue + lineLength,
          y: stackedvalue.startValues[pointIndex as number],
        };
        secondPoint = getPoint(start.x, start.y, xAxis, yAxis, isInverted);
        direction += 'L' + ' ' + secondPoint.x + ' ' + secondPoint.y + ' ';
      }
      ...
      for (let j: number = pointsLength - 1; j >= startPoint; j--) {
        ...
        if (visiblePoint[j as number].visible) {
          pointIndex = visiblePoint[j as number].index;
          point2 = getPoint(
            visiblePoint[j as number].xValue,
            stackedvalue.startValues[pointIndex as number],
            xAxis,
            yAxis,
            isInverted
        ...
        if (j !== 0) {
          ...
          pointIndex = index
            ? visiblePoint[index as number].index
            : visiblePoint[j - 1].index;
          point3 = getPoint(
            visiblePoint[validIndex as number].xValue,
            stackedvalue.startValues[pointIndex as number],
            xAxis,
            yAxis,
            isInverted
      ...
      this.prevStep =
        stackSeries.step === 'Right'
          ? 'Left'
          : stackSeries.step === 'Left'
          ? 'Right'
          : stackSeries.step;
      options = new PathOption(
        stackSeries.chart.element.id + '_Series_' + stackSeries.index,
        stackSeries.interior,
        0,
        'transparent',
        stackSeries.opacity,
        stackSeries.dashArray,
        direction
      );
      ...
      if (stackSeries.border.width !== 0) {
        options = new PathOption(
          stackSeries.chart.element.id + '_Series_border_' + stackSeries.index,
          'transparent',
          stackSeries.border.width,
          stackSeries.border.color
            ? stackSeries.border.color
            : stackSeries.interior,
          1,
          stackSeries.dashArray,
        ...
        for (
          ...
          if (endPathCommands.length !== startPathCommands.length) {
            startPathCommands.splice(
              Math.floor(startPathCommands.length / 2) - 1,
              0,
              startPathCommands[Math.floor(startPathCommands.length / 2) - 1],
              startPathCommands[Math.floor(startPathCommands.length / 2) - 1]
            );
            startPathCommands.splice(
              Math.floor(startPathCommands.length / 2) + 2,
              0,
        ...
        for (let i: number = minLength; i < maxLength; i++) {
          if (endPathCommands.length !== startPathCommands.length) {
            ...
            endPathCommands.splice(
              endPathCommands.length - 3,
              0,
              endPathCommands[endPathCommands.length - 3]
            );
          }
        }
        animateAddPoints(
