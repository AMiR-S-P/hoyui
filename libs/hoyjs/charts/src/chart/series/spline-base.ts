...
export class SplineBase extends LineBase {
  ...
  public findSplinePoint(series: Series): void {
    ...
    let negativePoint: boolean = false;
    ...
    if (points.length > 1) {
      ...
      for (const point of points) {
        ...
        if (point.index !== 0) {
          const previous: number = this.getPreviousIndex(
            points,
            point.index - 1,
            series
          );
          if (series.type === 'SplineRangeArea') {
            points[previous as number].yValue =
              points[previous as number].high > points[previous as number].low
                ? (points[previous as number].high as number)
                : (points[previous as number].low as number);
            point.yValue =
              point.high > point.low
                ? (point.high as number)
                : (point.low as number);
          }
          value = this.getControlPoints(
            points[previous as number],
            point,
            this.splinePoints[previous as number],
            this.splinePoints[point.index],
            series
          );
          series.drawPoints.push(value);
          if (series.type === 'SplineRangeArea') {
            points[previous as number].yValue =
              points[previous as number].low < points[previous as number].high
                ? (points[previous as number].low as number)
                : (points[previous as number].high as number);
            point.yValue =
            ...
            lowPoints = this.getControlPoints(
              points[previous as number],
              point,
              this.lowSplinePoints[previous as number],
              this.lowSplinePoints[point.index],
              series
            );
          ...
          if (
            ...
            series.yMin = Math.min(
              series.yMin,
              point.yValue,
              value.controlPoint1.y,
              value.controlPoint2.y
            );
            series.yMax = Math.ceil(
              Math.max(
                series.yMax,
                point.yValue,
                value.controlPoint1.y,
      ...
      if (series.chart.chartAreaType === 'PolarRadar' && series.isClosed) {
        value = this.getControlPoints(
          ...
          this.splinePoints[0],
          this.splinePoints[points[points.length - 1].index],
          series
        );
        series.drawPoints.push(value);
      }
  ...
  public filterEmptyPoints(series: Series, seriesPoints?: Points[]): Points[] {
    if (
      series.emptyPointSettings.mode !== 'Drop' &&
      this.isPointInRange(series.points)
    ) {
      return seriesPoints ? seriesPoints : series.points;
    }
    const points: Points[] = seriesPoints
      ? seriesPoints
      : (extend([], series.points, null, true) as Points[]);
  ...
  public findSplineCoefficients(
    ...
    let cardinalSplineTension: number = series.cardinalSplineTension
    ...
    for (let i: number = 0; i < count - 1; i++) {
      if (series.type === 'SplineRangeArea') {
        if (!isLow) {
          ...
          points[i as number].yValue =
            points[i as number].high > points[i as number].low
              ? (points[i as number].high as number)
              : (points[i as number].low as number);
        }
        if (isLow) {
          points[i + 1].yValue =
            points[i + 1].low < points[i + 1].high
              ? (points[i + 1].low as number)
              : (points[i + 1].high as number);
          points[i as number].yValue =
            points[i as number].low < points[i as number].high
              ? (points[i as number].low as number)
              : (points[i as number].high as number);
        }
      }
    ...
    for (let j: number = 0; j < dx.length; j++) {
      if (slopeLength > j + 1) {
        if (slope[j as number] * slope[j + 1] <= 0) {
          ...
          ySpline[j + 1] =
            (3 * interPoint) /
            ((interPoint + dx[j + 1]) / slope[j as number] +
    ...
    for (let i: number = 0; i < count; i++) {
      if (i === 0) {
        ySpline[i as number] =
          count > 2
            ? cardinalSplineTension *
              (points[i + 2].xValue - points[i as number].xValue)
            : 0;
      } else if (i === count - 1) {
        ySpline[i as number] =
          count > 2
            ? cardinalSplineTension *
              (points[count - 1].xValue - points[count - 3].xValue)
            : 0;
      } else {
        ySpline[i as number] =
  ...
  private clampedSplineCofficients(
    ...
    for (let i: number = 0; i < count - 1; i++) {
        ...
        if (!isLow) {
          points[1].yValue =
            points[1].high > points[1].low
              ? (points[1].high as number)
              : (points[1].low as number);
          points[0].yValue =
            points[0].high > points[0].low
              ? (points[0].high as number)
              : (points[0].low as number);
          points[points.length - 1].yValue =
            points[points.length - 1].high > points[points.length - 1].low
              ? (points[points.length - 1].high as number)
              : (points[points.length - 1].low as number);
          points[points.length - 2].yValue =
            points[points.length - 2].high > points[points.length - 2].low
              ? (points[points.length - 2].high as number)
              : (points[points.length - 2].low as number);
        }
        if (isLow) {
          points[1].yValue =
      ...
      ySpline[0] =
        (3 * (points[1].yValue - points[0].yValue)) /
          (points[1].xValue - points[0].xValue) -
        3;
      ySplineDuplicate[0] = 0.5;
      ySpline[points.length - 1] =
        (3 *
          (points[points.length - 1].yValue -
            points[points.length - 2].yValue)) /
        (points[points.length - 1].xValue - points[points.length - 2].xValue);
      ySpline[0] = ySplineDuplicate[0] =
        Math.abs(ySpline[0]) === Infinity ? 0 : ySpline[0];
      ySpline[points.length - 1] = ySplineDuplicate[points.length - 1] =
        Math.abs(ySpline[points.length - 1]) === Infinity
          ? 0
          : ySpline[points.length - 1];
    }
    return ySpline;
  ...
  private naturalSplineCoefficients(
    ...
    for (let i: number = 1; i < count - 1; i++) {
      ...
      if (coefficient1 === 0 || coefficient2 === 0 || coefficient3 === 0) {
        ...
        const p: number =
          1 / (coefficient1 * ySpline[i - 1] + 2 * coefficient2);
        ySpline[i as number] = -p * coefficient3;
        ySplineDuplicate[i as number] =
          p *
          (6 * (dy1 / coefficient3 - dy2 / coefficient1) -
            coefficient1 * ySplineDuplicate[i - 1]);
      }
    }
    for (let k: number = count - 2; k >= 0; k--) {
      ySpline[k as number] =
        ySpline[k as number] * ySpline[k + 1] + ySplineDuplicate[k as number];
    }
    return ySpline;
  }
  /**
  ...
  public getControlPoints(
    ...
    switch (series.splineType) {
      case 'Cardinal':
        ...
        controlPoint1 = new ChartLocation(
          xValue1 + ySpline1 / 3,
          yValue1 + ySplineDuplicate1 / 3
        );
        controlPoint2 = new ChartLocation(
          xValue2 - ySpline2 / 3,
          yValue2 - ySplineDuplicate2 / 3
        );
      ...
      case 'Monotonic': {
        ...
        controlPoint1 = new ChartLocation(
          xValue1 + value,
          yValue1 + ySpline1 * value
        );
        controlPoint2 = new ChartLocation(
          xValue2 - value,
          yValue2 - ySpline2 * value
        );
        point = new ControlPoints(controlPoint1, controlPoint2);
        break;
      }
      default: {
        ...
        const y1: number =
          ...
          (2 * yValue1 +
            yValue2 -
            one3 * deltaX2 * (ySpline1 + 0.5 * ySpline2));
        const y2: number =
          one3 *
          (yValue1 +
            2 * yValue2 -
            one3 * deltaX2 * (0.5 * ySpline1 + ySpline2));
