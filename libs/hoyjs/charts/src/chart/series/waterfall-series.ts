...
export class WaterfallSeries extends ColumnBase {
  ...
  public render(series: Series): void {
    ...
    for (const point of series.points) {
      ...
      if (
        ...
        if (prevRegion !== null) {
          ...
          const currentLeft: number = isInversed
            ? currentRegion.x
            : currentRegion.y;
          ...
          const beforePoint: Points =
            series.points[point.index - 1 === -1 ? 1 : point.index - 1];
          if (point.yValue === 0) {
            prevBottom = isInversed
              ? prevRegion.x + prevRegion.width
              : prevRegion.y + prevRegion.height;
            currentBottom = isInversed
              ? point.symbolLocations[0].x
              : point.symbolLocations[0].y;
          } else {
            prevBottom = isInversed
              ? beforePoint.yValue === 0
                ? beforePoint.symbolLocations[0].x
                : prevRegion.x + prevRegion.width
              : beforePoint.yValue === 0
              ? beforePoint.symbolLocations[0].y
              : prevRegion.y + prevRegion.height;
            currentBottom = isInversed
          ...
          if (
            ...
            y = isInversed
              ? currentRegion.x === 0 && prevRegion.x === 0
                ? currentBottom
                : currentRegion.x
              : currentRegion.y;
            y =
          ...
          if (isInversed) {
            ...
            if (point.yValue === 0) {
              currentYValue =
                currentRegion.y + currentRegion.height / 2 - rect.height / 2;
            }
            direction = direction.concat(
              'M' +
                ...
                (series.xAxis.isInversed
                  ? currentYValue
                  : currentYValue + currentRegion.height) +
                ' '
            );
          } else {
            if (beforePoint.yValue === 0) {
              prevRegion.x =
                prevRegion.x + prevRegion.width / 2 - rect.width / 2;
              currentXValue =
                currentRegion.x +
                currentRegion.width / 2 +
                rect.width / 2 -
                currentRegion.width;
            }
            direction = direction.concat(
              'M' +
                ...
                ' ' +
                (series.xAxis.isInversed
                  ? currentXValue + currentRegion.width
                  : currentXValue) +
                ' ' +
                y +
                ' '
    ...
    const options: PathOption = new PathOption(
      series.chart.element.id + '_Series_' + series.index + '_Connector_',
      'none',
      series.connector.width,
      series.connector.color,
      series.opacity,
      series.connector.dashArray,
      direction
    );
    ...
    const element: HTMLElement = <HTMLElement>(
      (redrawElement(redraw, options.id, options, series.chart.renderer) ||
        series.chart.renderer.drawPath(
          options,
          new Int32Array([series.clipRect.x, series.clipRect.y])
        ))
    );
    element.style.visibility = !series.chart.enableCanvas
      ? ((series.animation.enable && animationMode !== 'Disable') ||
          animationMode === 'Enable') &&
        series.chart.animateSeries
        ? 'hidden'
        : 'visible'
      : null;
    appendChildElement(
    ...
    if (series.marker.dataLabel.visible && series.chart.dataLabelModule) {
      for (let i: number = 0; i < point.length; i++) {
        ...
        for (let j: number = 0; j < dataLabelElement.length; j++) {
          series.chart.dataLabelModule.doDataLabelAnimation(
            series,
            dataLabelElement[j as number]
          );
        }
      }
    }
  ...
  private isIntermediateSum(series: Series, index: number): boolean {
    if (
      series.intermediateSumIndexes !== undefined &&
      series.intermediateSumIndexes.indexOf(index) !== -1
    ) {
      return true;
    }
    return false;
  ...
  private isSumIndex(series: Series, index: number): boolean {
    if (
      series.sumIndexes !== undefined &&
      series.sumIndexes.indexOf(index) !== -1
    ) {
      return true;
    }
    return false;
