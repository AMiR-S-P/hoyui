...
import {
  ...
  ChartSegmentModel,
  ParetoOptionsModel,
} from '../series/chart-series-model';
import {
  StackValues,
  RectOption,
  ControlPoints,
...
import { Rect, SvgRenderer, CanvasRenderer, Size } from '@hoyui/svg-base';
import {
  ChartSeriesType,
  ChartShape,
  SeriesValueType,
...
export class SeriesBase extends ChildProperty<SeriesBase> {
  ...
  public processJsonData(): void {
    ...
    const xName: string =
      this instanceof Series && this.type === 'Histogram' ? 'x' : this.xName;
    const textMappingName: string =
      this instanceof Series && this.marker.dataLabel.name
        ? this.marker.dataLabel.name
        : '';
    if (this instanceof Series) {
      if (this.type === 'Waterfall' || this.type === 'Histogram') {
        this.currentViewData = this.chart[
          firstToLowerCase(this.type) + 'SeriesModule'
        ].processInternalData(
          extend([], this.currentViewData, null, true) as Object[],
          this
        );
      }
      if (this.category === 'Pareto') {
        this.currentViewData = extend(
          [],
        ...
        if (this.type === 'Line') {
          this.currentViewData =
            this.chart.paretoSeriesModule.performCumulativeCalculation(
              this.currentViewData,
              this
            );
        }
      }
      this.isRectTypeSeries =
        this.type.indexOf('Column') > -1 ||
        this.type.indexOf('Bar') > -1 ||
      ...
      if (this.xAxis.valueType === 'DateTime') {
        ...
        if (this.chart.isBlazor) {
          this.pushCategoryData(
            point,
            index,
            Date.parse(point.x.toString()).toString()
          );
        } else {
          this.pushCategoryData(
            point,
            index,
            Date.parse(dateParser(dateFormatter(point.x))).toString()
          );
        }
      }
      this.pushData(point, index);
  ...
  public updateSplineValue(): void {
    if (
      ...
      !(this.chart.stockChart && this.xAxis.valueType === 'DateTimeCategory')
    ) {
      if (
        this.type.indexOf('Spline') > -1 ||
        (this.drawType.indexOf('Spline') > -1 &&
          this.chart.chartAreaType === 'PolarRadar')
      ) {
        const isArea: boolean =
          this.type.indexOf('Area') > -1 || this.drawType.indexOf('Area') > -1;
        const isRange: boolean = this.type.indexOf('Range') > -1;
        this.chart[
          'spline' +
            (isArea ? (isRange ? 'RangeArea' : 'Area') : '') +
            'SeriesModule'
  ...
  protected dataPoint(
    ...
    const getObjectValueByMappingString: Function = this.enableComplexProperty
      ? getValue
      : this.getObjectValue;
    ...
    point.interior = getObjectValueByMappingString(
      this.pointColorMapping,
      currentViewData
    ) as string;
    if (this instanceof Series) {
      if (this.errorBar.visible) {
        point.errorBarColor = getObjectValueByMappingString(
          this.errorBar.errorBarColorMapping,
          currentViewData
        ) as string;
        point.verticalError =
          typeof this.errorBar.verticalError == 'string'
            ? getObjectValueByMappingString(
                this.errorBar.verticalError,
                currentViewData
              )
            : this.errorBar.verticalError;
        point.horizontalError =
          typeof this.errorBar.horizontalError == 'string'
            ? getObjectValueByMappingString(
                this.errorBar.horizontalError,
                currentViewData
              )
            : this.errorBar.horizontalError;
        point.verticalNegativeError =
          typeof this.errorBar.verticalNegativeError == 'string'
            ? getObjectValueByMappingString(
                this.errorBar.verticalNegativeError,
                currentViewData
              )
            : this.errorBar.verticalNegativeError;
        point.verticalPositiveError =
          typeof this.errorBar.verticalPositiveError == 'string'
            ? getObjectValueByMappingString(
                this.errorBar.verticalPositiveError,
                currentViewData
              )
            : this.errorBar.verticalPositiveError;
        point.horizontalNegativeError =
          typeof this.errorBar.horizontalNegativeError == 'string'
            ? getObjectValueByMappingString(
                this.errorBar.horizontalNegativeError,
                currentViewData
              )
            : this.errorBar.horizontalNegativeError;
        point.horizontalPositiveError =
          typeof this.errorBar.horizontalPositiveError == 'string'
            ? getObjectValueByMappingString(
                this.errorBar.horizontalPositiveError,
                currentViewData
              )
            : this.errorBar.horizontalPositiveError;
      }
      ...
      point.text = getObjectValueByMappingString(
        textMappingName,
        currentViewData
      ) as string;
      point.tooltip = getObjectValueByMappingString(
        this.tooltipMappingName,
        currentViewData
      ) as string;
      if (this.isAdvancedColorSupported()) {
        this.rangeColorName =
          this.colorName.length > 0 ? this.colorName : this.yName;
        point.colorValue = getObjectValueByMappingString(
          this.rangeColorName,
          currentViewData
        );
        point.interior = this.getPointFillColor(
          point.interior,
          point.colorValue
        );
      }
  ...
  public setEmptyPoint(point: Points, i: number): void {
    ...
    point.isEmpty = true;
    const mode: EmptyPointMode =
      this instanceof Series && point.isPointInRange
        ? this.emptyPointSettings.mode
        : 'Drop';
    switch (mode) {
      case 'Zero':
        point.visible = true;
          ...
          if (this.seriesType.indexOf('HighLow') > -1) {
            point.high =
              isNullOrUndefined(point.high) || isNaN(+point.high)
                ? this.getAverage(this.high, i)
                : point.high;
            point.low =
              isNullOrUndefined(point.low) || isNaN(+point.low)
                ? this.getAverage(this.low, i)
                : point.low;
            if (this.seriesType.indexOf('HighLowOpenClose') > -1) {
              point.open =
                isNullOrUndefined(point.open) || isNaN(+point.open)
                  ? this.getAverage(this.open, i)
                  : point.open;
              point.close =
                isNullOrUndefined(point.close) || isNaN(+point.close)
                  ? this.getAverage(this.close, i)
                  : point.close;
            }
          } else {
            point.y =
              point.yValue =
              this.yData[i as number] =
                this.getAverage(this.yName, i);
          }
        }
        point.visible = true;
        break;
  ...
  private findVisibility(point: Points): boolean {
    const type: SeriesValueType =
      this instanceof Series ? this.seriesType : 'HighLowOpenClose';
    let yValues: number[];
    ...
    switch (type) {
      case 'XY':
        if (
          this.chart.chartAreaType === 'PolarRadar' &&
          ((!isNullOrUndefined(yAxisMin) && point.yValue < yAxisMin) ||
        ...
        if (this instanceof Series && this.type === 'Bubble') {
          this.sizeMax = Math.max(
            this.sizeMax,
            isNullOrUndefined(<number>point.size) || isNaN(+point.size)
              ? this.sizeMax
              : <number>point.size
          );
        }
        return (
          isNullOrUndefined(point.x) ||
          isNullOrUndefined(point.y) ||
          isNaN(+point.y)
        );
      case 'HighLow':
      ...
      case 'BoxPlot':
        yValues = ((point.y as number[]) || [null])
          .filter((value: number) => {
  ...
  private setXYMinMax(yValue: number): void {
    const isLogAxis: boolean =
      this.yAxis.valueType === 'Logarithmic' ||
      this.xAxis.valueType === 'Logarithmic';
    const isNegativeValue: boolean =
      yValue < 0 || this.yAxis.rangePadding === 'None';
    ...
    this.yMin = isLogAxis
      ? Math.min(
          this.yMin,
          isNullOrUndefined(seriesMinY) ||
            isNaN(seriesMinY) ||
            seriesMinY === 0 ||
            seriesMinY.toString() === '0' ||
            seriesMinY.toString() === ''
            ? this.yMin
            : seriesMinY
        )
      : Math.min(
          this.yMin,
          isNullOrUndefined(seriesMinY) || isNaN(seriesMinY)
            ? this.yMin
            : seriesMinY
        );
    this.yMax = Math.max(
      this.yMax,
      isNullOrUndefined(yValue) || isNaN(yValue) ? this.yMax : yValue
    );
  }
  /**
  ...
  private setHiloMinMax(high: number, low: number): void {
    this.yMin = Math.min(
      this.yMin,
      Math.min(
        isNullOrUndefined(low) || isNaN(low) ? this.yMin : low,
    ...
    this.yMax = Math.max(
      this.yMax,
      Math.max(
        isNullOrUndefined(low) || isNaN(low) ? this.yMax : low,
  ...
  private getSeriesType(): void {
    ...
    if (this instanceof Series) {
      const seriesType: string =
        this.chart.chartAreaType === 'PolarRadar' ? this.drawType : this.type;
      if (seriesType) {
        switch (seriesType) {
          case 'RangeColumn':
          case 'RangeArea':
  ...
  private getAverage(
    member: string,
    i: number,
    data: Object = this.currentViewData
  ): number {
    const previous: number = data[i - 1]
      ? data[i - 1][member as string] || 0
      : 0;
  ...
  public refreshDataManager(chart: Chart): void {
    ...
    if (chart[isAngular as string]) {
      dataSource = Object.keys(this.dataSource).length
        ? this.dataSource
        : chart.dataSource;
    } else {
    ...
    if (!(dataSource instanceof DataManager) && isNullOrUndefined(this.query)) {
      this.dataManagerSuccess(
        { result: dataSource, count: (dataSource as Object[]).length },
        false
      );
      return;
    }

    const dataManager: Promise<Object> = this.dataModule.getData(
      this.dataModule.generateQuery().requiresCount()
    );
    dataManager.then((e: { result: Object; count: number }) =>
      this.dataManagerSuccess(e)
    );
  }

  private dataManagerSuccess(
    e: { result: Object; count: number },
    ...
    if (this instanceof Series) {
      if (this.chart.stockChart) {
        (this.chart.stockChart.series[this.index] as StockSeries).localData =
          this.currentViewData;
      }
...
export class Series extends SeriesBase {
    ...
    for (const item of this.xAxis.series) {
      if (item.visible && item.category !== 'TrendLine') {
        ...
        for (const point of item.points) {
          item.pushCategoryData(
            point,
            point.index,
            this.xAxis.valueType === 'DateTimeCategory'
              ? Date.parse(dateParser(dateFormatter(point.x))).toString()
              : <string>point.x
          );
        ...
        for (const axis of column.axes) {
          for (const series of axis.series) {
            if (
              series === rowSeries &&
              series.visible &&
              this.rectSeriesInChart(series, isStack)
            ) {
  ...
  private rectSeriesInChart(series: Series, isStack: boolean): boolean {
    ...
    return (
      ...
      type.indexOf('bar') !== -1 ||
      type.indexOf('histogram') !== -1 ||
      type.indexOf('hiloopenclose') !== -1 ||
      type.indexOf('candle') !== -1 ||
      type.indexOf('pareto') !== -1 ||
      type.indexOf('hilo') !== -1 ||
      series.drawType.indexOf('Column') !== -1 ||
      type.indexOf('waterfall') !== -1 ||
      type.indexOf('boxandwhisker') !== -1 ||
      isStack
    );
  }
  /**
    ...
    for (const columnItem of chart.columns) {
      for (const item of chart.rows) {
        this.calculateStackingValues(
          this.findSeriesCollection(<Column>columnItem, <Row>item, true),
          isStacking100
        );
      }
    }
    ...
    for (let k: number = 0; k < keys.length; k++) {
      ...
      for (const series of seriesCollection) {
        if (
          ...
          stackingGroup =
            series.type.indexOf('StackingArea') !== -1
              ? 'StackingArea100'
              : series.type.indexOf('StackingLine') !== -1
              ? 'StackingLine100'
          ...
          for (
            let j: number = 0, pointsLength: number = visiblePoints.length;
            j < pointsLength;
            j++
          ) {
            lastValue = 0;
            ...
            if (
              lastPositive[stackingGroup as string][
                visiblePoints[j as number].xValue
              ] === undefined
            ) {
              lastPositive[stackingGroup as string][
                visiblePoints[j as number].xValue
            ...
            if (isStacking100) {
              ...
              stackedValues[j as number] = stackedValues[j as number]
                ? stackedValues[j as number] + Math.abs(value)
                : Math.abs(value);
            }
            if (value >= 0) {
              lastValue =
          ...
          const isColumnBarType: boolean =
            series.type.indexOf('Column') !== -1 ||
            series.type.indexOf('Bar') !== -1;
          series.yMin =
            isLogAxis && isColumnBarType && series.yMin < 1
              ? series.yMin
              : series.yAxis.startFromZero &&
                series.yAxis.rangePadding === 'Auto' &&
                series.yMin >= 0
              ? 0
              : parseFloat(
                  Math.min
                    .apply(0, isStacking100 ? startValues : endValues)
                    .toFixed(10)
                );
          series.yMax = Math.max.apply(0, endValues);
          if (series.yMin > Math.min.apply(0, endValues)) {
            series.yMin = isStacking100
              ? -100
              : isLogAxis && isColumnBarType && series.yMin < 1
              ? series.yMin
              : Math.min.apply(0, endValues);
          }
  ...
  private findPercentageOfStacking(
    ...
    for (const item of stackingSeies) {
      ...
      for (const point of getVisiblePoints(item)) {
        point.percentage = Math.abs(
          +((<number>point.y / values[point.index]) * 100).toFixed(2)
        );
      }
    }
  }
  private findFrequencies(seriesCollection: Series[]): number[] {
    ...
    let visiblePoints: Points[] = [];
    for (const series of seriesCollection) {
      series.yAxis.isStack100 =
        series.type.indexOf('100') !== -1 ? true : false;
      visiblePoints = getVisiblePoints(series);
  ...
  public renderSeries(chart: Chart): void {
    if (this.chart.stockChart && this.xAxis.valueType === 'DateTimeCategory') {
      for (let i: number = 0; i < this.points.length; i++) {
        const index: number = this.xAxis.labels.indexOf(
          Date.parse(this.points[i as number].x.toString()).toString()
        );
    ...
    if (chart[seriesType + 'SeriesModule']) {
      ...
      chart[seriesType + 'SeriesModule'].render(
        this,
        this.xAxis,
        this.yAxis,
  ...
  public createSeriesElements(chart: Chart): void {
    if (this.category !== 'Indicator') {
      ...
      const render: SvgRenderer | CanvasRenderer =
        this.type === 'Bubble' ||
        (!this.marker.visible && chart.tooltip.shared && chart.enableCanvas)
          ? chart.svgRenderer
          : chart.renderer;
      const index: string | number =
        this.index === undefined ? this.category : this.index;
      ...
      if (chart.chartAreaType === 'PolarRadar') {
        const markerMaxValue: number =
          this.drawType === 'Scatter'
            ? Math.max(this.marker.width, this.marker.height)
            : 0;
        options = new CircleOption(
          elementId + '_ChartSeriesClipRect_' + index,
          'transparent',
          { width: 1, color: 'Gray' },
          1,
          this.clipRect.width / 2 + this.clipRect.x,
          this.clipRect.height / 2 + this.clipRect.y,
          chart.radius + markerMaxValue
        );
        this.clipRectElement = appendClipElement(
          chart.redraw,
          options,
          render as SvgRenderer,
        ...
        options = new RectOption(
          ...
          1,
          {
            ...
            width:
              this.clipRect.width +
              (this.xAxis.columnIndex === chart.columns.length - 1
                ? markerWidth * 2
                : markerWidth),
            height:
              this.clipRect.height +
              (this.yAxis.rowIndex === 0 ? markerHeight * 2 : markerHeight),
          }
        );
        this.clipRectElement = appendClipElement(
          chart.redraw,
      ...
      const transform: string =
        chart.chartAreaType === 'Cartesian'
          ? 'translate(' + this.clipRect.x + ',' + this.clipRect.y + ')'
          : '';
      ...
      this.seriesElement = render.createGroup({
        ...
        transform: transform,
        'clip-path':
          'url(#' + elementId + '_ChartSeriesClipRect_' + index + ')',
      });
      if (this.seriesElement) {
        this.seriesElement.setAttribute('role', 'region');
        this.seriesElement.setAttribute(
          'aria-label',
          this.name +
            ',' +
            this.type +
      ...
      if (!this.chart.enableCanvas || this.type === 'Bubble') {
        this.seriesElement.setAttribute(
          ...
          index === 0
            ? '0'
            : !this.checkTabindex(chart.visibleSeries, index as number)
            ? '0'
            : ''
        );
  ...
  public performAnimation(
    ...
    if (
      ((this.animation.enable && animationMode !== 'Disable') ||
        animationMode === 'Enable') &&
      chart.animateSeries &&
      (!chart.stockChart || !chart.stockChart.isStockChartRendered)
    ) {
  ...
  public setBorderColor(point: Points, border: BorderModel): BorderModel {
    border.width = point.isEmpty
      ? this.emptyPointSettings.border.width || border.width
      : border.width;
    border.color = point.isEmpty
      ? this.emptyPointSettings.border.color || border.color
      : border.color;
    return border;
  }
  ...
  public addPoint(dataPoint: Object, duration?: number): void {
    ...
    if (this.type === 'Histogram') {
      this.currentViewData = this.chart[
        ...
        extend([], this.dataSource, null, true) as Object[],
        this
      );
      for (
        let i: number = 0;
        i < (this.currentViewData as Object[]).length;
        i++
      ) {
        this.updatePoint(i);
      }
      ...
      const pointIndex: number =
        this.points.length === 0
          ? 0
          : this.points[this.points.length - 1].index + 1;
      this.updatePoint(pointIndex);
    }
    if (this.category === 'Pareto') {
      const dataSource: Object[] = extend(
        [],
        this.dataSource,
        null,
        true
      ...
      const series: Series =
        this.chart.visibleSeries[this.index + this.chart.series.length];
      series.currentViewData =
        this.chart.paretoSeriesModule.performCumulativeCalculation(
          dataSource,
          this
        );
      for (
        let i: number = 0;
        i < (series.currentViewData as Object[]).length;
        i++
      ) {
        if (!series.points[i as number]) {
          series.updatePoint(i);
        }
        series.points[i as number].y = series.points[i as number].yValue =
          series.currentViewData[i as number][series.yName];
      }
    }
    this.updateSplineValue();
    ...
    if (
      this.chart.enableAnimation &&
      !(
        this.isRectSeries ||
        this.type === 'Bubble' ||
        this.type === 'Scatter'
      ) &&
      this.type.indexOf('step') === -1
    ) {
      if (this.marker && this.marker.visible && this.visible) {
        for (let i: number = this.points.length - 2; i >= 0; i--) {
          if (
            this.points[i as number] &&
            this.points[i as number].symbolLocations[0] !== undefined
          ) {
            this.chart.markerRender.renderMarker(
              this,
              this.points[this.points.length - 2],
              this.points[i as number].symbolLocations[0],
              null,
              true
            );
            break;
    ...
    if (this.yMin >= yMin && this.yMax <= yMax) {
      if (!setRange(this.xAxis)) {
        this.xAxis.baseModule.calculateRangeAndInterval(
          new Size(this.xAxis.rect.width, this.chart.availableSize.height),
          this.xAxis
        );
  ...
  public removePoint(index: number, duration?: number): void {
    ...
    if (dataSource.length > 0 && index >= 0 && index < dataSource.length) {
      ...
      this.xMax = -Infinity;
      if (
        this.xAxis.valueType.indexOf('Category') > -1 &&
  ...
  private removeTrackballElements(index?: number): void {
    if (this.marker.visible) {
      if (index !== undefined) {
        ...
        const symbolElement: Element = getElement(
          this.chart.element.id +
            '_Series_' +
            this.index +
            '_Point_' +
        ...
        const baseClassPattern: string =
          this.chart.element.id +
          '_EJ2-Trackball_Series_' +
          this.index +
          '_Point_';
        const elements: NodeListOf<Element> = document.querySelectorAll(
          `[class*="${baseClassPattern}"]`
        );
        if (elements[0]) {
          const pointIndexMatch: RegExpMatchArray =
            elements[0].id.match(/_Point_(\d+)_/);
          const pointIndex: number = pointIndexMatch
            ? parseInt(pointIndexMatch[1], 10)
            : null;
          elements[0].remove();
          const symbolElement: Element = getElement(
            this.chart.element.id +
    ...
    if (this.chart.tooltip.enable) {
      ...
      const tooltipElement: Element = getElement(
        this.chart.element.id + '_tooltip'
      );
      if (tooltipElement) {
  ...
  public setData(data: Object[], duration?: number): void {
        ...
        if (this.currentData.length < (this.dataSource as Object[]).length) {
          ...
          if (dataSource[i as number][this.xName] instanceof Date) {
            this.currentData.push(
              dataSource[i as number][this.xName].getTime()
            );
          } else {
    ...
    let isSameData: boolean = updatedData.every(
      (element: number | string) => this.currentData.indexOf(element) !== -1
    );
    ...
    if (
      (this.dataSource as Object[]).length === data.length &&
      !(this.type === 'Radar' || this.type === 'Polar') &&
      isSameData
    ) {
      samePoints = true;
      this.yData = [];
      if (
        ...
        const newHistogramData: Object[] = this.chart[
          firstToLowerCase(this.type) + 'SeriesModule'
        ...
        for (let i: number = 0; i < data.length; i++) {
          ...
          let existingData: number | string =
            this.dataSource[i as number][this.xName];
          if (data[i as number][this.xName] instanceof Date) {
          ...
          const getObjectValueByMappingString: Function = this
            .enableComplexProperty
            ? getValue
            : this.getObjectValue;
          const existingPoint: Object = this.dataSource[i as number];
          ...
          if (this.seriesType === 'XY' || this.seriesType === 'BoxPlot') {
            ...
            if (
              this.type === 'Bubble' &&
              existingPoint[this.size] !== newPoint[this.size]
            ) {
              point.size = getObjectValueByMappingString(this.size, newPoint);
            }
            points.push(i);
          } else if (
            existingPoint[this.high] !== newPoint[this.high] ||
            existingPoint[this.low] !== newPoint[this.low] ||
            existingPoint[this.open] !== newPoint[this.open] ||
            existingPoint[this.close] !== newPoint[this.close] ||
          ...
          if (!this.currentViewData) {
            point.index =
              this.xAxis.valueType === 'DateTime' && typeof point.x !== 'number'
                ? this.currentData.indexOf((point.x as Date).getTime())
                : this.currentData.indexOf(<number>point.x);
          }
          if (
            (this.xAxis.valueType === 'Category' ||
              this.xAxis.valueType === 'DateTimeCategory') &&
            i < this.xAxis.visibleLabels.length
          ) {
            this.xAxis.visibleLabels[i as number].value = updatedData.indexOf(
              this.currentData[i * this.xAxis.visibleRange.interval]
            );
    ...
    if (!samePoints) {
      ...
      if (yMax === this.yMax && yMin === this.yMin && this.visible) {
        ...
        this.chart[
          firstToLowerCase(
            this.category === 'Pareto' ? 'Column' : this.type.replace('100', '')
          ) + 'SeriesModule'
        ].updateDirection(this, points, this.chart.requireInvertedAxis);
        if (this.chart.annotationModule) {
          this.chart.annotationModule.renderAnnotations(
        ...
        if (maximumLabelWidth < this.yAxis.maxLabelSize.width) {
          ...
          (
            this.chart.chartAxisLayoutPanel as CartesianAxisLayoutPanel
          ).drawPaneLines(this.chart);
        ...
        if (isXAxisChanged) {
          this.chart[
            firstToLowerCase(
              this.category === 'Pareto'
                ? 'Column'
                : this.type.replace('100', '')
            ) + 'SeriesModule'
          ].updateDirection(this, points, this.chart.requireInvertedAxis);
          index = this.index;
        }
        ...
        if (this.chart.stripLineModule) {
          this.chart.stripLineModule.renderStripLine(
            this.chart,
            'Behind',
            this.chart.axisCollections
          );
        }
      }
      this.chart.redraw = false;
  ...
  private updateSeries(xAxis: boolean, yAxis: boolean, index?: number): void {
    ...
    if (xAxis && yAxis) {
      const set: Set<Series> = new Set(
        this.xAxis.series.concat(this.yAxis.series)
      );
      set.forEach((series: Series) => {
        seriesCollection.push(series);
      });
    } else {
      seriesCollection = xAxis
        ? this.xAxis.series.slice()
        : this.yAxis.series.slice();
    }
    for (const series of seriesCollection) {
      if (series.visible && series.index !== index) {
        ...
        series.chart[
          firstToLowerCase(series.type.replace('100', '')) + 'SeriesModule'
        ].render(
          series,
          series.xAxis,
          series.yAxis,
          series.chart.requireInvertedAxis,
          series.chart.enableAnimation
        );
        if (
          series.marker.visible &&
          series.chart.chartAreaType === 'Cartesian' &&
          series.type !== 'Scatter' &&
          series.type !== 'Bubble' &&
          series.type !== 'Candle' &&
          series.type !== 'Hilo' &&
          ...
          appendChildElement(
            series.chart.enableCanvas,
            series.chart.seriesElements,
            series.symbolElement,
            true
          );
        }
        if (series.marker.dataLabel.visible && series.chart.dataLabelModule) {
          ...
          series.chart.dataLabelModule.render(
            series,
            series.chart,
          ...
          if (series.textElement) {
            ...
            appendChildElement(
              series.chart.enableCanvas,
              series.chart.dataLabelElements,
              series.shapeElement,
              true
            );
        ...
        if (series.chart.annotationModule) {
          series.chart.annotationModule.renderAnnotations(
            getElement(series.chart.element.id + '_Secondary_Element')
          );
        }
      }
    }
