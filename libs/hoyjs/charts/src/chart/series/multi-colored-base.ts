...
import { PathOption, SvgRenderer } from '@hoyui/svg-base';
...
import { PathAttributes } from '@hoyui/svg-base';
/**
...
export class MultiColoredSeries extends LineBase {
  ...
  public setPointColor(
    ...
    if (series.pointColorMapping === '') {
      ...
      for (let i: number = 0; i < segments.length; i++) {
        ...
        if (
          value <=
            this.getAxisValue(
              segment.value,
              isXSegment ? series.xAxis : series.yAxis,
              series.chart
            ) ||
      ...
      if (previous) {
        return (
          series.setPointColor(currentPoint, series.interior) !==
          series.setPointColor(previous, series.interior)
        );
      } else {
        return false;
      }
  ...
  public sortSegments(
    series: Series,
    chartSegments: ChartSegmentModel[]
  ): ChartSegmentModel[] {
    ...
    return segments.sort((a: ChartSegmentModel, b: ChartSegmentModel) => {
      return (
        this.getAxisValue(a.value, axis, series.chart) -
  ...
  public applySegmentAxis(
    ...
    if (series.pointColorMapping !== '') {
      options.map((option: PathOption) => {
        this[pointAnimate ? 'addMulticolorPath' : 'appendLinePath'](
          option,
          series,
          ''
        );
    ...
    for (let index: number = 0; index < length; index++) {
      ...
      clipPath = this.createClipRect(
        index
          ? this.getAxisValue(segments[index - 1].value, axis, series.chart)
      ...
      if (clipPath) {
        options.map((option: PathOption) => {
          ...
          attributeOptions = {
            ...
            stroke:
              series.type.indexOf('Line') > -1
                ? segment.color || series.interior
                : option['stroke'],
          ...
          if (
            areaBorderCount % 2 === 0 &&
            this.chart.multiColoredAreaSeriesModule &&
            series.border.color !== 'transparent' &&
            attributeOptions['stroke-width'] !== 0
          ) {
          ...
          if (pointAnimate) {
            ...
            pathAnimation(
              getElement(attributeOptions.id),
              attributeOptions.d,
              chart.redraw
            );
          }
  ...
  private includeSegment(
    ...
    if (
      this.getAxisValue(segments[length - 1].value, axis, series.chart) <
  ...
  private addMulticolorPath(
    ...
    if (points.previousDirection === null || points.previousDirection === '') {
      points.previousDirection =
        'M ' +
        options.d.split(' ').slice(-3)[0] +
        ' ' +
        options.d.split(' ').slice(-5)[0] +
        ' L ' +
        options.d.split(' ').slice(-3)[0] +
        ' ' +
        options.d.split(' ').slice(-5)[0] +
    ...
    if (options.d === null || options.d === '') {
      options.d =
        'M ' +
        points.previousDirection.split(' ').slice(-3)[0] +
        ' ' +
        points.previousDirection.split(' ').slice(-5)[0] +
        ' L ' +
        points.previousDirection.split(' ').slice(-3)[0] +
        ' ' +
        points.previousDirection.split(' ').slice(-5)[0];
        ...
        for (let i: number = minLength; i < maxLength; i++) {
          if (endPathCommands.length !== startPathCommands.length) {
            startPathCommands.splice(
              startPathCommands.length -
                (series.type.indexOf('Line') !== -1 ? 1 : 2),
              0,
              startPathCommands[
                startPathCommands.length -
                  (series.type.indexOf('Line') !== -1 ? 1 : 2)
  ...
  public createClipRect(
    ...
    endPointLocation = isRequired
      ? [startPointLocation, (startPointLocation = endPointLocation)][0]
      : endPointLocation;
    let options: RectOption;
    if (
      (series.xAxis.isInversed || series.xAxis.isAxisInverse
        ? startPointLocation.x - endPointLocation.x > 0
        : endPointLocation.x - startPointLocation.x > 0) &&
      (series.yAxis.isInversed
        ? startPointLocation.y - endPointLocation.y > 0
        : endPointLocation.y - startPointLocation.y > 0)
    ) {
      options = new RectOption(
        ...
        1,
        {
          ...
          y: series.yAxis.isInversed
            ? endPointLocation.y
            : startPointLocation.y,
          width:
            series.xAxis.isInversed || series.xAxis.isAxisInverse
              ? startPointLocation.x - endPointLocation.x
              : endPointLocation.x - startPointLocation.x,
          height: series.yAxis.isInversed
            ? startPointLocation.y - endPointLocation.y
      ...
      if (!series.chart.enableCanvas) {
        series.seriesElement.appendChild(
          appendClipElement(
            series.chart.redraw,
            options,
            series.chart.renderer as SvgRenderer
          )
        );
      }
      return (
  ...
  public getAxisValue(segmentValue: Object, axis: Axis, chart: Chart): number {
    ...
    if (axis.valueType === 'DateTime') {
      ...
      return Date.parse(
        chart.intl.getDateParser(option)(
          chart.intl.getDateFormat(option)(
            new Date(DataUtil.parse.parseJson({ val: segmentValue }).val)
          )
        )
      );
    } else if (axis.valueType.indexOf('Category') > -1) {
      const xValue: string =
        axis.valueType === 'DateTimeCategory'
          ? (segmentValue as Date).getTime().toString()
          : <string>segmentValue;
      return axis.labels.indexOf(xValue) < 0
        ? +segmentValue
        : axis.labels.indexOf(xValue);
    } else {
      return +segmentValue;
    }
  }
