...
export class BubbleSeries {
  ...
  public render(
    ...
    const value: number = Math.max(
      series.chart.initialClipRect.height,
      series.chart.initialClipRect.width
    );
    ...
    if (series.maxRadius === null || series.minRadius === null) {
      for (const value of series.chart.visibleSeries) {
        if (
          value.type === 'Bubble' &&
          value.visible === true &&
          (value.maxRadius === null || value.minRadius === null)
        ) {
          maximumSize =
            value.sizeMax > maximumSize ? value.sizeMax : maximumSize;
        }
      }
      maxValue = value / 5 / 2;
  ...
  public renderPoint(
    ...
    let startLocation: ChartLocation =
      series.chart.redraw && bubblePoint.symbolLocations
    ...
    if (
      ...
      if (series.maxRadius === null || series.minRadius === null) {
        ...
        segmentRadius =
          minRadius + radius * Math.abs(+bubblePoint.size / maximumSize);
      }
      ...
      if (!argsData.cancel) {
        bubblePoint.symbolLocations.push(
          getPoint(
            bubblePoint.xValue,
            bubblePoint.yValue,
            series.xAxis,
        ...
        const element: Element = drawSymbol(
          ...
          shapeOption,
          bubblePoint.x.toString() + ':' + bubblePoint.yValue.toString(),
          series.chart.svgRenderer,
          series.clipRect
        );
        appendChildElement(
        ...
        startLocation =
          series.chart.redraw && !startLocation
            ? bubblePoint.symbolLocations[0]
            : startLocation;
        if (series.chart.redraw) {
          animateRedrawElement(
            element,
            series.chart.duration ? series.chart.duration : 300,
            startLocation,
            bubblePoint.symbolLocations[0],
            'cx',
            'cy'
