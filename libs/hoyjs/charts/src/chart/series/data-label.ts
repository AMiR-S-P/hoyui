...
import { createElement, getValue, extend } from '@hoyui/base';
...
export class DataLabel {
  ...
  private initPrivateVariables(
    ...
    if (series.chart.chartAreaType === 'Cartesian') {
      ...
      clipPath =
        'url(#' + this.chart.element.id + '_ChartSeriesClipRect_' + index + ')';
    }
    ...
    this.markerHeight =
      series.type === 'Scatter' || marker.visible ? marker.height / 2 : 0;
    ...
    this.chartBackground =
      this.chart.chartArea.background === 'transparent'
        ? this.chart.background || this.chart.themeStyle.background
        : this.chart.chartArea.background;
  }
    ...
    if (!series.errorBar.visible) {
      ...
      if (this.isRectSeries(series)) {
        ...
        if (position === 'Top' || position === 'Outer' || position === 'Auto') {
          if (
            (direction === 'Both' || direction === 'Plus') &&
            !series.chart.isTransposed
          ) {
            this.errorHeight = positiveHeight;
          } else {
            this.errorHeight = 0;
  ...
  public render(
    ...
    if (element.childElementCount) {
      if (!chart.enableCanvas) {
        appendChildElement(
          chart.enableCanvas,
          getElement(chart.element.id + '_Secondary_Element'),
          element,
          chart.redraw,
          false,
          'x',
        ...
        getElement(chart.element.id + '_Secondary_Element').appendChild(
          element
        );
      }
    }
  ...
  public renderDataLabel(
    ...
    if (
      !dataLabel.showZero &&
      (point.y === 0 ||
        (point.y === 0 && series.emptyPointSettings.mode === 'Zero'))
    ) {
      return null;
    }
    ...
    let rectCenterX: number;
    let rectCenterY: number;
    ...
    dataLabel.angle =
      dataLabel.labelIntersectAction === 'Rotate90' ? 90 : dataLabel.angle;
    ...
    if (
      ...
      labelText =
        point.text !== null ? getLabelText(point, series, this.chart) : [];
      ...
      for (let i: number = 0; i < labelLength; i++) {
        ...
        if (!argsData.cancel) {
          ...
          this.markerHeight =
            series.type === 'Bubble'
              ? point.regions[0].height / 2
              : this.markerHeight;
          if (argsData.template !== null) {
            this.createDataLabelTemplate(
              element,
              series,
              dataLabel,
              point,
            ...
            if (this.chart.chartAreaType === 'PolarRadar') {
              for (const rectRegion of (<PolarRadarPanel>(
                this.chart.chartAxisLayoutPanel
              )).visibleAxisLabelRect) {
                if (
                  isOverlap(
            ...
            const actualRect: Rect = new Rect(
              rect.x + clip.x,
              rect.y + clip.y,
              rect.width,
              rect.height
            ...
            if (dataLabel.enableRotation) {
              const rectCoordinates: ChartLocation[] =
                this.getRectanglePoints(rect);
              rectCenterX = rect.x + rect.width / 2;
              rectCenterY = rect.y + rect.height / 2;
              isDataLabelOverlap =
                dataLabel.labelIntersectAction === 'Rotate90' || angle === -90
                  ? false
                  : this.isDataLabelOverlapWithChartBound(
                      rectCoordinates,
                      this.chart,
                      clip
                    );
              if (!isDataLabelOverlap) {
                ...
                const currentPointIndex: number =
                  this.chart.rotatedDataLabelCollections.length - 1;
                for (
                  let index: number = currentPointIndex;
                  index >= 0;
                  index--
                ) {
                  if (
                    this.chart.rotatedDataLabelCollections[
                      currentPointIndex as number
                    ] &&
            ...
            if (
              (!isDataLabelOverlap ||
                dataLabel.labelIntersectAction === 'None') &&
              isRender
            ) {
              ...
              if (this.isShape) {
                shapeRect = this.chart.renderer.drawRectangle(
                  new RectOption(
                    ...
                    dataLabel.ry,
                    '',
                    dataLabel.border.dashArray
                  ),
                  new Int32Array([clip.x, clip.y])
                ) as HTMLElement;
              ...
              const backgroundColor: string =
                this.fontBackground === 'transparent'
                  ? this.chart.theme.indexOf('Dark') > -1 ||
                    this.chart.theme.indexOf('HighContrast') > -1
                    ? 'black'
                    : 'white'
                  : this.fontBackground;
              const rgbValue: ColorValue = convertHexToColor(
                colorNameToHex(backgroundColor)
              );
              ...
              yPos =
                ...
                this.chart.chartAreaType !== 'PolarRadar'
                  ? rect.y +
                    ...
                    textSize.height / 2 +
                    textSize.width / 4 +
                    (dataLabel.position === 'Auto'
                      ? point.regions[0].width / 10
                      : 0) +
                    labelLocation.y
                  : rect.y +
                    this.margin.top +
              ...
              if (angle !== 0 && dataLabel.enableRotation) {
                ...
                yValue = rect.y;
                xPos -=
                  this.chart.chartAreaType === 'Cartesian' &&
                  xPos + textSize.width / 2 > clip.width
                    ? !this.chart.requireInvertedAxis && xPos > clip.width
                      ? 0
                      : xPos + textSize.width / 2 - clip.width
                    : 0;
                yPos -=
              ...
              const textAnchor: string =
                dataLabel.labelIntersectAction === 'Rotate90'
                  ? dataLabel.position === 'Top'
                    ? 'start'
                    : dataLabel.position === 'Middle'
                    ? 'middle'
                    : 'end'
                  : angle === -90 && dataLabel.enableRotation
                  ? dataLabel.position === 'Top'
                    ? 'end'
                    : dataLabel.position === 'Middle'
                    ? 'middle'
              ...
              if (
                ...
                oldText = document.getElementById(
                  this.commonId + point.index + '_Text_' + i
                ).textContent;
              }
              dataLabelElement.push(
                textElement(
                  this.chart.renderer,
                  new TextOption(
                    this.commonId +
                      (series.removedPointIndex !== null &&
                      series.removedPointIndex <= point.index
                        ? point.index + 1
                        : point.index) +
                      '_Text_' +
                      i,
              ...
              if (
                ...
                (series.textElement.lastChild as HTMLElement).id =
                  this.commonId + point.index + '_Text_' + i;
              }
              if (this.chart.redraw && oldText !== argsData.text) {
                animateTextElement(
                  series.textElement.querySelector(
                    '#' + this.commonId + point.index + '_Text_' + i
                  ),
                  this.chart.duration,
                  parseFloat(oldText),
                  parseFloat(argsData.text),
                  series.marker.dataLabel.format || series.yAxis.labelFormat
                );
              }
            } else if (
              getElement(this.commonId + point.index + '_Text_0') &&
              series.chart.redraw &&
              series.currentData
            ) {
  ...
  private getRectanglePoints(rect: Rect): ChartLocation[] {
    ...
    const loc3: ChartLocation = new ChartLocation(
      rect.x + rect.width,
      rect.y + rect.height
    );
  ...
  private isDataLabelOverlapWithChartBound(
    ...
    for (let index: number = 0; index < rectCoordinates.length; index++) {
      if (
        !withInBounds(
          rectCoordinates[index as number].x + clip.x,
          rectCoordinates[index as number].y + clip.y,
    ...
    const childElement: HTMLElement = createTemplate(
      createElement('div', {
        ...
        styles:
          ...
          data.color +
          ';' +
          getFontStyle(dataLabel.font, this.chart.themeStyle.datalabelFont) +
          ';border:' +
          data.border.width +
          'px solid ' +
          data.border.color +
      ...
      point,
      series,
      this.chart.element.id + '_DataLabel',
      labelIndex
    );
    this.calculateTemplateLabelSize(
      parentElement,
      childElement,
      point,
      series,
  ...
  public calculateTemplateLabelSize(
    ...
    const elementRect: ClientRect = measureElementRect(
      childElement,
      redraw,
      isReactCallback
    );
    ...
    childElement.style.left =
      (this.chart.chartAreaType === 'PolarRadar' ? 0 : series.clipRect.x) +
      rect.x -
      clipWidth +
      'px';
    childElement.style.top =
      (this.chart.chartAreaType === 'PolarRadar' ? 0 : series.clipRect.y) +
      rect.y +
      clipHeight +
      'px';
    const backgroundColor: string =
    ...
    const vAxis: Axis = series.chart.requireInvertedAxis
      ? series.xAxis
      : series.yAxis;
    const hAxis: Axis = series.chart.requireInvertedAxis
      ? series.yAxis
      : series.xAxis;
    childElement.style.color =
      ...
      (this.chart.theme === 'Bootstrap5'
        ...
        : 'white');
    if (
      childElement.childElementCount &&
      !isOverlap &&
      (!isCollide(rect, this.chart.dataLabelCollections, clip) ||
        dataLabel.labelIntersectAction === 'None') &&
      (series.seriesType !== 'XY' ||
        point.yValue === undefined ||
        withIn(point.yValue, series.yAxis.visibleRange) ||
        series.type.indexOf('Stacking') > -1 ||
        (series.type.indexOf('100') > -1 &&
          withIn(
            series.stackedValues.endValues[point.index],
            series.yAxis.visibleRange
          ))) &&
      withIn(point.xValue, series.xAxis.visibleRange) &&
      parseFloat(childElement.style.top) >= vAxis.rect.y &&
      ...
      this.chart.dataLabelCollections.push(
        new Rect(rect.x + clip.x, rect.y + clip.y, rect.width, rect.height)
      );
      appendChildElement(
        this.chart.enableCanvas,
        parentElement,
  ...
  private calculateTextPosition(
    ...
    if (
      !this.chart.requireInvertedAxis ||
      !this.isRectSeries(series) ||
      series.type === 'BoxAndWhisker'
    ) {
      this.locationX = location.x;
      const alignmentValue: number =
        textSize.height +
        this.borderWidth * 2 +
        this.markerHeight +
        this.margin.bottom +
      ...
      location.y =
        !this.isRectSeries(series) || series.type === 'BoxAndWhisker'
          ...
          : this.calculateRectPosition(
              location.y,
              labelRegion,
              point.yValue < 0 !== this.yAxisInversed,
              dataLabel.position,
              series,
              textSize,
              labelIndex,
    ...
    if (
      !(dataLabel.enableRotation === true && dataLabel.angle !== 0) &&
      !(
      ...
      rect.y =
        rect.y < 0 && !(dataLabel.labelIntersectAction === 'None')
          ? padding
          : rect.y;
      rect.x -=
        rect.x + rect.width > clipRect.x + clipRect.width
          ? rect.x + rect.width - (clipRect.x + clipRect.width) + padding
          : 0;
      rect.y -=
        rect.y + rect.height > clipRect.y + clipRect.height
          ? rect.y + rect.height - (clipRect.y + clipRect.height) + padding
          : 0;
      this.fontBackground =
        this.fontBackground === 'transparent'
          ? this.chartBackground
    ...
    if (
      this.inverted &&
      series.isRectSeries &&
      rect.x + rect.width > labelRegion.x + labelRegion.width
    ) {
      dataLabelOutRegion = true;
    }
    this.fontBackground = dataLabelOutRegion
      ? this.chartBackground
      : this.fontBackground;
  ...
  private calculatePolarRectPosition(
    ...
    const alignmentSign: number =
      alignment === 'Center' ? 0 : alignment === 'Far' ? 1 : -1;
    const angle: number =
      point.regionData.startAngle -
      0.5 * Math.PI +
      (point.regionData.endAngle - point.regionData.startAngle) / 2;
    if (labelIndex === 0) {
      columnRadius =
        point.regionData.radius < point.regionData.innerRadius
          ? point.regionData.innerRadius
    ...
    if (position === 'Outer') {
      ...
      if (series.drawType === 'StackingColumn') {
        columnRadius =
          point.regionData.innerRadius +
          (point.regionData.radius - point.regionData.innerRadius) / 2 +
          padding -
          size.height / 2;
      }
      ...
      columnRadius +=
        series.drawType === 'StackingColumn'
          ? point.regionData.innerRadius + this.markerHeight
          : 0;
    } else {
      if (labelIndex === 0) {
        columnRadius =
          columnRadius >= series.chart.radius
    ...
    location.y =
      series.clipRect.height / 2 +
      series.clipRect.y +
      columnRadius * Math.sin(angle);
    return location;
  }
  ...
  private getLabelLocation(
    ...
    let labelRegion: Rect =
      series.type === 'Candle' && labelIndex > 1
        ? point.regions[1]
        : point.regions[0];
    ...
    if (series.type === 'BoxAndWhisker') {
      ...
      location = new ChartLocation(
        point.symbolLocations[0].x,
        point.symbolLocations[0].y
      );
    } else if (
      ...
      location = new ChartLocation(
        point.symbolLocations[1].x,
        point.symbolLocations[1].y
      );
    } else if (isInverted) {
    ...
    if (labelIndex > 1 && series.type === 'HiloOpenClose') {
      if (series.chart.requireInvertedAxis) {
        ...
        location.y =
          labelRegion.y + height / 2 + 2 * (labelIndex === 2 ? 1 : -1);
      } else {
        const width: number = labelRegion.width;
        location.x =
          labelRegion.x + width / 2 + 2 * (labelIndex === 2 ? 1 : -1);
      }
    }
    return location;
  }
  ...
  private calculateRectPosition(
    ...
    const textLength: number = series.marker.dataLabel.enableRotation
      ? textSize.width
      : !this.inverted
      ? textSize.height
      : textSize.width;
    this.extraSpace =
      ...
      Math.abs(rect.height - textSize.height) < padding
        ? 0
        : padding);
    if (
      series.type === 'StackingColumn100' ||
      series.type === 'StackingBar100'
    ) {
      position = position === 'Outer' ? 'Top' : position;
    ...
    switch (position) {
          ...
          ? isMinus
            ? labelLocation +
              (series.type === 'Waterfall'
                ? -this.extraSpace - margin.top - this.markerHeight
                : -rect.height + this.extraSpace + margin.top)
            : labelLocation + rect.height - this.extraSpace - margin.bottom
          : isMinus
          ? labelLocation +
            (series.type === 'Waterfall'
              ? +this.extraSpace + margin.left + this.markerHeight
              : +rect.width - this.extraSpace - margin.left)
      ...
      case 'Middle':
        ...
        break;
      case 'Auto':
        labelLocation = this.calculateRectActualPosition(
          labelLocation,
          rect,
          isMinus,
          series,
      ...
      default:
        ...
        labelLocation = this.calculateTopAndOuterPosition(
          labelLocation,
          rect,
          position,
          series,
    ...
    const check: boolean = !this.inverted
    ...
    if (
      ...
      for (let i: number = series.index + 1; i < seriesLength; i++) {
        ...
        if (
          nextSeries.type.indexOf('Stacking') > -1 &&
          nextSeries.type.indexOf('100') === -1
        ) {
          this.fontBackground =
            nextSeriesPoint &&
            ((nextSeriesPoint.yValue < 0 && point.yValue < 0) ||
              (nextSeriesPoint.yValue > 0 && point.yValue > 0))
              ? nextSeriesPoint
                ? nextSeriesPoint.color
                : nextSeries.interior
              : this.fontBackground;
          break;
        }
  ...
  private calculatePathPosition(
    ...
    const padding: number = 5;
    if (
      series.type.indexOf('Area') > -1 &&
      series.type !== 'RangeArea' &&
      series.type !== 'SplineRangeArea' &&
      series.type !== 'RangeStepArea' &&
      this.yAxisInversed &&
      series.marker.dataLabel.position !== 'Auto'
    ) {
      position =
        position === 'Top'
          ? 'Bottom'
  ...
  private calculateRectActualPosition(
    ...
    const finalPosition: number =
      series.type.indexOf('Range') !== -1 || series.type === 'Hilo' ? 2 : 4;
    while (isOverLap && position < finalPosition) {
      ...
      if (!this.inverted) {
        ...
        labelRect = calculateRect(
          new ChartLocation(this.locationX, location),
          size,
          this.margin
        );
        isOverLap =
          labelRect.y < 0 ||
          isCollide(labelRect, collection, series.clipRect) ||
          labelRect.y > series.clipRect.height;
        if (series.marker.dataLabel.template === null && isOverLap !== true) {
          isOverLap =
            labelRect.y / 2 +
              size.height +
              (actualPosition === 'Outer'
                ? point.regions[0].height + this.extraSpace
                : point.regions[0].height - 2 * this.extraSpace) >
            series.clipRect.height;
        }
      } else {
        labelRect = calculateRect(
          new ChartLocation(location, this.locationY),
          size,
          this.margin
        );
        isOverLap =
  ...
  private calculateTopAndOuterPosition(
    ...
    switch (series.type) {
      ...
      case 'Hilo':
        top =
          (index === 0 && !this.yAxisInversed) ||
          (index === 1 && this.yAxisInversed);
        location = this.updateLabelLocation(
          position,
          location,
          extraSpace,
          margin,
      ...
      case 'Candle':
        top =
          ((index === 0 || index === 2) && !this.yAxisInversed) ||
      ...
      case 'HiloOpenClose':
        if (index <= 1) {
          ...
          if (this.yAxisInversed) {
            location = !this.inverted
              ? location + extraSpace + margin.top
              : location - extraSpace - margin.right;
          } else {
            location = !this.inverted
              ? location - extraSpace - margin.bottom
              : location + extraSpace + margin.left;
          }
        }
        break;
      default:
        if (
          (isMinus && position === 'Top') ||
          (!isMinus && position === 'Outer') ||
          (position === 'Top' && series.visiblePoints[point.index].yValue === 0)
        ) {
          location = !this.inverted
            ? location +
              (isMinus && series.type === 'Waterfall'
                ? -rect.height + extraSpace + margin.bottom
                : -extraSpace - margin.bottom - this.markerHeight)
            : location +
              (isMinus && series.type === 'Waterfall'
                ? +rect.width - extraSpace - margin.left
                : +extraSpace + margin.left + this.markerHeight);
        } else {
          location = !this.inverted
            ? location +
              (isMinus && series.type === 'Waterfall'
                ? -rect.height - extraSpace - margin.top
                : +extraSpace + margin.top + this.markerHeight)
            : location +
              (isMinus && series.type === 'Waterfall'
                ? +rect.width + extraSpace + margin.top
                : -extraSpace - margin.right - this.markerHeight);
        }
        break;
    }
    return location;
  ...
  private updateLabelLocation(
    ...
    if (!this.inverted) {
      if (top) {
        location =
          position === 'Outer' && !inside
            ? location - extraSpace - margin.bottom - this.markerHeight
  ...
  private calculatePathActualPosition(
    ...
    if (series.type === 'Bubble') {
      ...
      if (index) {
        position =
          !previousPoint ||
          !previousPoint.visible ||
          yValue > previousPoint.yValue !== this.yAxisInversed ||
      ...
      if (labelIndex === 1 || labelIndex === 3 || labelIndex > 4) {
        ...
        isOverLap = false;
        position = 'Middle';
        yLocation = this.calculatePathPosition(
          y,
          position,
          series,
          point,
      ...
      if (index === 0) {
        position =
          ...
          (yValue < nextPoint.yValue && this.yAxisInversed)
            ? 'Top'
            : 'Bottom';
        ...
        if (!nextPoint.visible && !(previousPoint && previousPoint.visible)) {
          ...
          position =
            nextPoint.yValue > yValue ||
            (previousPoint && previousPoint.yValue > yValue)
              ? 'Bottom'
              : 'Top';
        } else {
    ...
    positionIndex = ['Outer', 'Top', 'Bottom', 'Middle', 'Auto'].indexOf(
      position
    );
    while (isOverLap && positionIndex < 4) {
      ...
      labelRect = calculateRect(
        new ChartLocation(this.locationX, yLocation),
        size,
        this.margin
      );
      isOverLap =
  ...
  public doDataLabelAnimation(series: Series, element?: Element): void {
    ...
    for (let i: number = 0; i < length; i++) {
      ...
      if (element) {
        ...
        location = new ChartLocation(
          +tempElement.getAttribute('x') +
            +tempElement.getAttribute('width') / 2,
          +tempElement.getAttribute('y') +
            +tempElement.getAttribute('height') / 2
        );
        markerAnimate(
          tempElement,
          delay,
  ...
  private getPosition(index: number): LabelPosition {
    return <LabelPosition>(
      ['Outer', 'Top', 'Bottom', 'Middle', 'Auto'][index as number]
    );
  }
