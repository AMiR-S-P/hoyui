...
export class RangeStepAreaSeries extends LineBase {
  ...
  public render(
    ...
    for (
      let i: number = 0, length: number = visiblePoints.length;
      i < length;
      i++
    ) {
      ...
      const lowPoint: ChartLocation = getPoint(
        point.xValue,
        low,
        xAxis,
        yAxis,
      ...
      const highPoint: ChartLocation = getPoint(
        point.xValue,
        high,
        xAxis,
        yAxis,
      ...
      const rect: Rect = new Rect(
        Math.min(lowPoint.x, highPoint.x),
        Math.min(lowPoint.y, highPoint.y),
      ...
      if (
        ...
        if (start === null) {
          ...
          direction +=
            command + ' ' + currentPoint.x + ' ' + currentPoint.y + ' ';
          currentPoint = getPoint(
            point.xValue - lineLength,
        ...
        if (this.prevPoint != null) {
          ...
          secondPoint = getPoint(
            this.prevPoint.xValue,
            this.prevPoint.high > this.prevPoint.low
              ? (this.prevPoint.high as number)
              : (this.prevPoint.low as number),
            xAxis,
            yAxis,
            isInverted
          );
          ...
          this.borderDirection += this.GetStepLineDirection(
            currentPoint,
            secondPoint,
            series.step,
            command,
        ...
        if (
          (i + 1 < visiblePoints.length && !visiblePoints[i + 1].visible) ||
          i === visiblePoints.length - 1
        ) {
          // Path to connect the low points.
          direction = this.closeRangeStepAreaPath(
            visiblePoints,
            point,
            series,
            direction,
    ...
    for (let j: number = i; j >= 0; j--) {
      if (visiblePoints[j as number].visible) {
        ...
        if (this.prevPoint != null) {
          ...
          secondPoint = getPoint(
            this.prevPoint.xValue,
            this.prevPoint.low < this.prevPoint.high
              ? (this.prevPoint.low as number)
              : (this.prevPoint.high as number),
            xAxis,
            yAxis,
  ...
  public updateDirection(series: Series, point: number[]): void {
    ...
    for (let i: number = 0; i < point.length; i++) {
      if (series.marker && series.marker.visible) {
        series.points[point[i as number]].symbolLocations.map(function (
