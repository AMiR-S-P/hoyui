...
import { PathOption, Rect, Size } from '@hoyui/svg-base';
...
export class ScatterSeries {
  ...
  public render(
    ...
    const scatterBorder: BorderModel = {
      width: this.isLineShapeMarker(marker.shape)
        ? series.width
        : series.border.width,
      color: this.isLineShapeMarker(marker.shape)
        ? series.interior
        : series.border.color,
    };
  ...
  public renderPoint(
    ...
    scatterBorder: BorderModel,
    visiblePoints: Points[]
  ): void {
    const redraw: boolean = series.chart.redraw;
    let argsData: IPointRenderEventArgs;
    const startLocation: ChartLocation =
      redraw && point.symbolLocations ? point.symbolLocations[0] : null;
  ...
  public enableComplexProperty(series: Series): Points[] {
    ...
    const yTolerance: number = Math.abs(
      yVisibleRange.delta / areaBounds.height
    );
    ...
    let prevXValue: number =
      seriesPoints[0] && seriesPoints[0].x > xTolerance ? 0 : xTolerance;
    let yVal: number = 0;
    let xVal: number = 0;
  ...
  private refresh(
    ...
    const shapeOption: PathOption = new PathOption(
      ...
      series.chart.enableCanvas && !argsData.border.color
        ? argsData.fill
    ...
    const element: Element = drawSymbol(
      ...
      imageURL,
      shapeOption,
      point.x.toString() + ':' + point.yValue.toString(),
