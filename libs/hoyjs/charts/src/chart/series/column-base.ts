...
import { PathOption, Rect } from '@hoyui/svg-base';
...
export class ColumnBase {
  ...
  protected getSideBySideInfo(series: Series): DoubleRange {
    ...
    const position: number =
      series.type === 'Histogram' || !series.chart.enableSideBySidePlacement
        ? 0
        : series.position;
    const rectCount: number =
      series.type === 'Histogram' || !series.chart.enableSideBySidePlacement
        ? 1
        : series.rectCount;
    ...
    const seriesSpacing: number = series.chart.enableSideBySidePlacement
      ? series.columnSpacing
      : 0; // Column Spacing
    const pointSpacing: number =
      series.columnWidth === null || isNaN(+series.columnWidth)
        ? series.type === 'Histogram'
          ? 1
          : 0.7
        : Math.min(series.columnWidth, 1); // Column width
    const minimumPointDelta: number = getMinPointsDelta(
      series.xAxis,
      visibleSeries
    );
    ...
    let doubleRange: DoubleRange = new DoubleRange(
      location,
      location + 1 / rectCount
    );
    if (!(isNaN(doubleRange.start) || isNaN(doubleRange.end))) {
      if (series.groupName && series.type.indexOf('Stacking') === -1) {
        ...
        series.chart.series.filter(function (series: SeriesModel): void {
          if (series.columnWidth > mainColumnWidth) {
            mainColumnWidth = series.columnWidth;
          }
        });
        const mainWidth: number = minimumPointDelta * mainColumnWidth;
        const mainDoubleRange: DoubleRange = new DoubleRange(
          doubleRange.start * mainWidth,
          doubleRange.end * mainWidth
        );
        const difference: number =
          (mainDoubleRange.delta -
            (doubleRange.end * width - doubleRange.start * width)) /
          2;
        doubleRange = new DoubleRange(
          mainDoubleRange.start + difference,
          mainDoubleRange.end - difference
        );
      } else {
        doubleRange = new DoubleRange(
          doubleRange.start * width,
          doubleRange.end * width
        );
      }
      radius = seriesSpacing * doubleRange.delta;
      doubleRange = new DoubleRange(
        doubleRange.start + radius / 2,
        doubleRange.end - radius / 2
      );
    }
    return doubleRange;
  }
  ...
  protected getRectangle(
    ...
    const point1: ChartLocation = getPoint(
      x1,
      y1,
      series.xAxis,
      series.yAxis,
    ...
    const point2: ChartLocation = getPoint(
      x2,
      y2,
      series.xAxis,
      series.yAxis,
   ...
   * @returns {void}
   */
  protected drawCylinder(
    options: PathOption,
    element: HTMLElement,
    cylinderSeriesOption: CylinderSeriesOption,
    rect: Rect,
    series: Series
  ): void {
    ...
    if (series.chart.enableCanvas) {
      ...
      const x: number = rect.x + series.clipRect.x;
      const y: number = rect.y + series.clipRect.y;
      let arc: number = 2 * Math.PI + 0.1;
      let rx: number;
      let ry: number;
      let cx1: number;
      let cx2: number;
      ...
      if (fillColor.indexOf('url') === -1) {
        if (!document.getElementById(id)) {
          ...
          const gradientElement: GradientStop[] = [
            { colorStop: '0%', color: gradientColor },
  ...
  private drawGradient(
    optiong: OptionGradient,
    gradientElement: Object,
    series: Series
  ): void {
    const chart: Chart = series.chart;
    const defElement: Element = chart.renderer.createDefs();
    const xmlns: string = 'http://www.w3.org/2000/svg';
    const linearGradientElement: Element = document.createElementNS(
      xmlns,
      'linearGradient'
    );
    ...
    for (
      let i: number = 0;
      i < (gradientElement as GradientStop[]).length;
      i++
    ) {
      const stopElement: Element = document.createElementNS(xmlns, 'stop');
      stopElement.setAttribute(
        'offset',
        gradientElement[i as number].colorStop
      );
      stopElement.setAttribute(
        'stop-color',
        gradientElement[i as number].color
      );
  ...
  private getSideBySidePositions(series: Series): void {
    ...
    for (const columnItem of chart.columns) {
      for (const item of chart.rows) {
        this.findRectPosition(
          series.findSeriesCollection(<Column>columnItem, <Row>item, false)
        );
      }
    }
  }
   ...
   * @returns {void}
   */
  protected updateSymbolLocation(
    point: Points,
    rect: Rect,
    series: Series
  ): void {
  ...
  protected updateXRegion(point: Points, rect: Rect, series: Series): void {
    point.symbolLocations.push({
      ...
      y:
        series.seriesType === 'BoxPlot' ||
        series.seriesType.indexOf('HighLow') !== -1 ||
        point.yValue >= 0 === !series.yAxis.isAxisInverse
          ? rect.y
          : rect.y + rect.height,
    });
  ...
  private getRegion(point: Points, rect: Rect, series: Series): void {
    if (point.y === 0) {
      const markerWidth: number =
        series.marker && series.marker.width ? series.marker.width : 0;
      const markerHeight: number =
        series.marker && series.marker.height ? series.marker.height : 0;
      point.regions.push(
        new Rect(
  ...
  protected drawRectangle(
    ...
    point: Points,
    rect: Rect,
    argsData: IPointRenderEventArgs
  ): void {
    const chart: Chart = series.chart;
    ...
    if (point.y === 0) {
      ...
      direction = this.calculateRoundedRectPath(
        rect,
        series.cornerRadius.topLeft,
        series.cornerRadius.topRight,
        series.cornerRadius.bottomLeft,
    ...
    const previousDirection: string = previousElement
      ? previousElement.getAttribute('d')
      : '';
    this.options = new PathOption(
      ...
      series.opacity,
      series.dashArray,
      series.columnFacet === 'Cylinder' ? '' : direction
    );
    this.element = chart.renderer.drawPath(
      this.options,
      new Int32Array([series.clipRect.x, series.clipRect.y])
    ...
    if (
      ...
      this.element.id =
        chart.element.id + '_Series_' + series.index + '_Point_' + point.index;
    }
    switch (series.seriesType) {
      case 'XY':
        ...
        this.element.setAttribute(
          'aria-label',
          point.x + ':' + point.yValue + ', ' + series.name
        );
        break;
      case 'HighLow':
        this.element.setAttribute('role', 'img');
        this.element.setAttribute(
          'aria-label',
          point.x + ':' + point.high + ', ' + point.low + ', ' + series.name
        );
        break;
    }
    if (
      !(
        series.columnFacet === 'Cylinder' &&
        (chart.redraw || !chart.enableAnimation) &&
        series.seriesElement.querySelector('#' + this.element.id)
      )
    ) {
      appendChildElement(
        series.chart.enableCanvas,
    ...
    for (const point of visiblePoints) {
      ...
      if (
        ...
        for (let j: number = 0; j < rectElements.length; j++) {
          this.animateRect(
            <HTMLElement>rectElements[j as number],
            series,
            point
          );
        }
        ...
        this.animateRect(
          <HTMLElement>rectElements[count as number],
          series,
          point
        );
        count++;
  ...
  private animateRect(
    ...
    if (!series.chart.requireInvertedAxis) {
      ...
      if (series.type.indexOf('Stacking') > -1) {
        y =
          (1 - valueToCoefficient(0, series.yAxis)) * series.yAxis.rect.height;
        centerX = x;
        centerY = y;
      } else {
        y = +point.regions[0].y;
        centerY =
          series.seriesType.indexOf('HighLow') !== -1 ||
          series.type.indexOf('Waterfall') !== -1
            ? y + elementHeight / 2
            : isPlot !== series.yAxis.isAxisInverse
            ? y
            : y + elementHeight;
        ...
        progress: (args: AnimationOptions): void => {
          ...
          const annotations: HTMLElement = <HTMLElement>(
            document.getElementById(
              series.chart.element.id + '_Annotation_Collections'
            )
          );
          if (annotations) {
          ...
          if (
            element === seriesElement.lastElementChild ||
            point.index === series.points.length - 1 ||
            (series.type === 'Waterfall' &&
              element ===
                seriesElement.children[seriesElement.childElementCount - 2])
          ) {
            ...
            if (series.type === 'Waterfall') {
              ...
              for (let i: number = 0; i < rectElements.length; i++) {
                if (
                  (rectElements[i as number] as HTMLElement).id.indexOf(
                    'Connector'
                  ) !== -1
                ) {
                  (rectElements[i as number] as HTMLElement).style.visibility =
                    'visible';
                  (rectElements[i as number] as HTMLElement).setAttribute(
                    'transform',
                    'translate(0,0)'
                  );
                }
  ...
  private calculateRoundedRectPath(
    ...
    return (
      ...
      (rect.x + rect.width) +
      ' ' +
      (rect.y + rect.height - bottomRight) +
      ' Q ' +
      (rect.x + rect.width) +
      ' ' +
      (rect.y + rect.height) +
      ' ' +
      (rect.x + rect.width - bottomRight) +
      ' ' +
      (rect.y + rect.height) +
      ' ' +
      'L ' +
