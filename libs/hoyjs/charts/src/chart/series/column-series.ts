...
export class ColumnSeries extends ColumnBase {
  ...
  public renderPoint(
    ...
    if (
      pointColumn.visible &&
      withInRange(
        series.points[pointColumn.index - 1],
        pointColumn,
        series.points[pointColumn.index + 1],
        series
      )
      ...
      this.rect = this.getRectangle(
        pointColumn.xValue + sideBySideInfo.start,
        pointColumn.yValue,
        pointColumn.xValue + sideBySideInfo.end,
        origin,
        series
      );
      this.rect.width = series.columnWidthInPixel
        ? series.columnWidthInPixel -
          (series.chart.enableSideBySidePlacement
            ? series.columnWidthInPixel * series.columnSpacing
            : 0)
        : this.rect.width;
      this.rect.x = series.columnWidthInPixel
        ...
        : this.rect.x;
      const color: string =
        series.category === 'Indicator' ? pointColumn.color : series.interior;
