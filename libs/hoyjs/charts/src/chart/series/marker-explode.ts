...
import { PathOption, Size } from '@hoyui/svg-base';
...
export class MarkerExplode extends ChartData {
  ...
  public mouseMoveHandler(): void {
    const chart: Chart = this.chart;
    if (
      (chart.highlightMode !== 'None' || chart.tooltip.enable) &&
      (!chart.isTouch || chart.startMove) &&
      !this.isSelected(chart) &&
      !(this.chart.zoomModule && getElement(this.elementId + '_ZoomArea'))
    ) {
  ...
  public markerMove(remove: boolean): void {
      ...
      explodeSeries =
        ...
        (series.type === 'Bubble' ||
          ...
          (series.type !== 'Candle' &&
            ...
            series.marker.width !== 0 &&
            series.marker.height !== 0));
        ...
        if (!this.chart.tooltip.showNearestPoint) {
          ...
          for (const chartSeries of chart.visibleSeries) {
            if (
              ...
              chartSeries.category === 'Indicator'
            ) {
              continue;
            }
            if (chart.chartAreaType === 'Cartesian' && chartSeries.visible) {
              ...
              data = new PointData(
                chartSeries.points[pointData.point.index],
                chartSeries
              );
            }
            if (data) {
      ...
      if (
          ...
          for (const previousPoint of this.previousPoints) {
            if (!isNullOrUndefined(previousPoint)) {
              this.removeHighlightedMarker(
                previousPoint.series as Series,
                previousPoint.point as Points
              );
            }
          }
        }
        for (const data of <PointData[]>this.currentPoints) {
          if (
            ...
            this.isRemove = true;
            data.point.symbolLocations.map(
              (location: ChartLocation, index: number) => {
                if (
                  data.series.marker.allowHighlight &&
                  (!data.series.isRectSeries || data.point.marker.visible)
                ) {
    ...
    if (
      ...
      ((this.currentPoints.length === 0 && this.isRemove) ||
      ...
      if (!isNullOrUndefined(this.previousPoints[0])) {
        this.markerExplode = +setTimeout((): void => {
          if (this.previousPoints[0]) {
            this.removeHighlightedMarker(
              this.previousPoints[0].series as Series,
              this.previousPoints[0].point as Points
            );
          }
        }, 2000);
      }
  ...
  private drawTrackBall(
    ...
    const colorValue: ColorValue = convertHexToColor(
      colorNameToHex(borderColor)
    );
    const borderWidth: number = marker.border
      ? marker.border.width
      : seriesMarker.border.width;
    const markerShadow: string =
      series.chart.themeStyle.markerShadow ||
      'rgba(' +
        colorValue.r +
        ',' +
        colorValue.g +
        ...
        colorValue.b +
        ',0.2)';
    const markerElement: Element = document.getElementById(
      this.elementId +
        '_Series_' +
        series.index +
        '_Point_' +
    ...
    if (this.chart.enableCanvas) {
      const trackElement: HTMLElement = document.getElementById(
        this.chart.element.id + '_Secondary_Element'
      );
    ...
    for (let i: number = 0; i < 2; i++) {
      const options: PathOption = new PathOption(
        ...
        marker.opacity || seriesMarker.opacity,
        series.marker.border.dashArray,
        ''
      );
      const symbol: Element = drawSymbol(
        location,
        shape,
        size,
        marker.imageUrl,
      ...
      const selectionId: string =
        element.id.indexOf('Symbol') !== -1 ? '_Symbol' : '';
      const seletionElem: Element = document.getElementById(
        this.elementId +
          '_Series_' +
          series.index +
          '_Point_' +
      ...
      if (
        className !== '' &&
        !isNullOrUndefined(className) &&
        !isNullOrUndefined(seletionElem) &&
        seletionElem.hasAttribute('class') &&
        className === seletionElem.getAttribute('class')
      ) {
  ...
  public doAnimation(
    series: Series,
    point: Points,
    endAnimate: boolean = false
  ): void {
    const duration: number = this.animationDuration();
    const delay: number = 0;
    const rectElements: HTMLCollectionOf<Element> =
    ...
    for (let i: number = 0, len: number = rectElements.length; i < len; i++) {
      this.trackballAnimate(
        <HTMLElement>rectElements[i as number],
        delay,
        duration,
        series,
        point.index,
   ...
   * @private
   */
  public removeHighlightedMarker(
    series: Series = null,
    point: Points = null,
    ...
    if (!isNullOrUndefined(series) && !isNullOrUndefined(point)) {
      ...
      const trackballElements: HTMLCollectionOf<Element> =
        document.getElementsByClassName(
          this.elementId +
            '_EJ2-Trackball_Series_' +
            series.index +
      ...
      for (
        let i: number = 0, len: number = trackballElements.length;
        i < len;
        i++
      ) {
