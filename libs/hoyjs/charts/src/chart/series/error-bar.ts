...
export class ErrorBar {
    ...
    for (const point of series.points) {
      if (point.visible && point.symbolLocations[0]) {
        ...
        const shapeOption: PathOption = new PathOption(
          ...
          '',
          errorbar.width,
          errorbar.errorBarColorMapping
            ? point.errorBarColor
            : errorbar.color || this.chart.themeStyle.errorBar,
          null,
          '',
          errorDirection[0]
        );
        let element: Element = getElement(shapeOption.id);
        let previousDirection: string = element
          ? element.getAttribute('d')
          : null;
        if (series.errorBarElement) {
          series.errorBarElement.appendChild(
            this.chart.renderer.drawPath(shapeOption)
          );
        }
        pathAnimation(
          element,
        ...
        if (series.errorBarElement) {
          appendChildElement(
            ...
            series.errorBarElement,
            this.chart.renderer.drawPath(capOption),
            redraw
          );
        }
        pathAnimation(
  ...
  private findLocation(
    ...
    const yValue: number =
      series.type.indexOf('Stacking') > -1
        ...
        : series.seriesType === 'HighLow' ||
          series.seriesType === 'HighLowOpenClose'
        ? <number>series.points[point.index].high
        : point.yValue;
    const startPoint: ChartLocation = getPoint(
      point.xValue +
        (direction === 'Plus' || direction === 'Both'
          ? errorbar.type === 'Custom' &&
            (errorbar.mode === 'Horizontal' || errorbar.mode === 'Both')
            ? (x1 = point.horizontalPositiveError)
            : x1
          : 0),
      yValue +
        (direction === 'Plus' || direction === 'Both'
          ? errorbar.type === 'Custom' &&
            (errorbar.mode === 'Vertical' || errorbar.mode === 'Both')
            ? (y1 = point.verticalPositiveError)
            : y1
          : 0),
      series.xAxis,
      series.yAxis,
    ...
    const endPoint: ChartLocation = getPoint(
      point.xValue -
        (direction === 'Minus' || direction === 'Both'
          ? errorbar.type === 'Custom' &&
            (errorbar.mode === 'Horizontal' || errorbar.mode === 'Both')
            ? (x1 = point.horizontalNegativeError)
            : x1
          : 0),
      yValue -
        (direction === 'Minus' || direction === 'Both'
          ? errorbar.type === 'Custom' &&
            (errorbar.mode === 'Vertical' || errorbar.mode === 'Both')
            ? (y1 = point.verticalNegativeError)
            : y1
          : 0),
      series.xAxis,
      series.yAxis,
    ...
    point.error =
      errorbar.mode === 'Vertical'
        ? errorbar.verticalError
        : errorbar.horizontalError;
    this.negativeHeight =
      errorbar.mode === 'Vertical' || errorbar.mode === 'Both'
        ? isInverted
          ? location[1].x - location[2].x
  ...
  private getBothDirection(
    ...
    let capDirection: string = '';
    let path: string = '';
    const pathH: string[] = this.getHorizontalDirection(
      start,
      mid,
      end,
      direction,
    ...
    const pathV: string[] = this.getVerticalDirection(
      start,
      mid,
      end,
      direction,
  ...
  private getErrorDirection(
    ...
    switch (errorMode) {
      case 'Horizontal':
        paths = this.getHorizontalDirection(
          start,
          mid,
          end,
          direction,
        ...
        break;
      case 'Vertical':
        paths = this.getVerticalDirection(
          start,
          mid,
          end,
          direction,
  ...
  public meanCalculation(series: Series, mode: ErrorBarMode): Mean {
    let sumOfX: number = 0;
    let sumOfY: number = 0;
    let verticalMean: number = 0;
    let horizontalMean: number = 0;
    const length: number = series.points.length;
  ...
  private createElement(series: Series, chart: Chart): void {
    ...
    if (chart.chartAreaType === 'Cartesian') {
      const options: RectOption = new RectOption(
        chart.element.id + '_ChartErrorBarClipRect_' + series.index,
        'transparent',
        { width: 1, color: 'Gray' },
        1,
        {
  ...
  public doErrorBarAnimation(series: Series): void {
    ...
    while (j < errorBarElements.length) {
      for (let i: number = 0; i < series.points.length; i++) {
        ...
        (<HTMLElement>errorBarElements[j as number]).style.visibility =
          'hidden';
        templateAnimate(
          errorBarElements[j as number] as HTMLElement,
          delay,
          350,
