...
import { Size, PathOption, Rect } from '@hoyui/svg-base';
...
export class BoxAndWhiskerSeries extends ColumnBase {
  ...
  private renderPoint(
    ...
    if (
      ...
      argsData = this.triggerEvent(series, point, series.interior, {
        color:
          !isNullOrUndefined(series.border.color) &&
  ...
  public updateDirection(
    ...
    for (let i: number = 0; i < point.length; i++) {
      ...
      if (
        visiblePoint.symbolLocations &&
        visiblePoint.symbolLocations.length &&
        series.marker.visible
      ) {
        series.chart.markerRender.renderMarker(
          series,
          visiblePoint,
          visiblePoint.symbolLocations[0],
          visiblePoint.symbolLocations.length - 1,
          true
        );
      }
      if (series.marker.dataLabel.visible && series.chart.dataLabelModule) {
        series.chart.dataLabelCollections = [];
        series.chart.dataLabelModule.commonId =
          series.chart.element.id + '_Series_' + series.index + '_Point_';
        ...
        series.chart.dataLabelModule.renderDataLabel(
          series,
          visiblePoint,
          null,
  ...
  public getPathString(
    point: Points,
    series: Series,
    median: ChartLocation,
    average: ChartLocation
  ): string {
    const topRect: Rect = point.regions[0];
    const midRect: Rect = point.regions[1];
    let direction: string = '';
    const center: number = series.chart.requireInvertedAxis
    ...
    if (!series.chart.requireInvertedAxis) {
      this.updateTipSize(
        series,
        point,
        { x: midRect.x, y: topRect.y, width: midWidth - midRect.x, height: 0 },
        true
      );
      this.updateTipSize(
        series,
        point,
        { x: midRect.x, y: topHeight, width: midWidth - midRect.x, height: 0 },
        true
      );
      direction +=
        'M ' +
      ...
      direction +=
        ...
        ' Z';
      direction +=
        ' M ' + center + ' ' + midHeight + ' L ' + center + ' ' + topHeight;
      direction +=
        ' M ' +
        midRect.x +
        ' ' +
        ...
        topHeight;
      direction +=
        ' M ' + midRect.x + ' ' + median.y + ' L ' + midWidth + ' ' + median.y;
      ...
      this.updateTipSize(
        series,
        point,
        { x: topRect.x, y: midRect.y, width: 0, height: midHeight - midRect.y },
        false
      );
      this.updateTipSize(
        series,
        point,
        { x: topWidth, y: midRect.y, width: 0, height: midHeight - midRect.y },
        true
      );
      direction +=
        'M ' +
      ...
      direction +=
        ...
        ' Z';
      direction +=
        ' M ' + midWidth + ' ' + center + ' L ' + topWidth + ' ' + center;
      direction +=
        ' M ' + topWidth + ' ' + midRect.y + ' L ' + topWidth + ' ' + midHeight;
      direction +=
        ' M ' +
        median.x +
        ' ' +
  ...
  public renderBoxAndWhisker(
    ...
    const symbolId: string =
      ...
      series.removedPointIndex <= point.index
        ? point.index + 1
        : point.index);
    const previusDirection: string = getElement(symbolId + '_BoxPath')
      ? getElement(symbolId + '_BoxPath').getAttribute('d')
      : '';
    ...
    element.setAttribute(
      ...
      point.x.toString() +
        ':' +
        point.maximum.toString() +
        ':' +
        point.minimum.toString() +
        ':' +
        point.lowerQuartile.toString() +
        ':' +
        point.upperQuartile.toString()
    );
  ...
  public findBoxPlotValues(
    ...
    if (mode === 'Exclusive') {
      quartile.lowerQuartile = this.getExclusiveQuartileValue(
        yValues,
        yCount,
        0.25
      );
      quartile.upperQuartile = this.getExclusiveQuartileValue(
        yValues,
        yCount,
        0.75
      );
      ...
      quartile.lowerQuartile = this.getInclusiveQuartileValue(
        yValues,
        yCount,
        0.25
      );
      quartile.upperQuartile = this.getInclusiveQuartileValue(
        yValues,
        yCount,
        0.75
      );
  ...
  private getExclusiveQuartileValue(
    ...
    if (integerRank === 0) {
      ...
      value =
        fractionRank *
          (yValues[integerRank as number] - yValues[integerRank - 1]) +
  ...
  private getInclusiveQuartileValue(
    ...
    value =
      fractionRank *
        (yValues[integerRank + 1] - yValues[integerRank as number]) +
      yValues[integerRank as number];
    return value;
  }
  /**
  ...
  private getQuartileValues(
    ...
    const upperQuartileArray: number[] = yValues.slice(
      isEvenList ? halfLength : halfLength + 1,
      count
    );
  ...
  private getMinMaxOutlier(
    ...
    const interquartile: number =
      quartile.upperQuartile - quartile.lowerQuartile;
