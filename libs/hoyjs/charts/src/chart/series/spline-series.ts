...
export class SplineSeries extends SplineBase {
  ...
  public render(
    ...
    const tempPoints: Points[] =
      series.category === 'TrendLine'
        ? series.points
        : this.enableComplexProperty(series);
      ...
      if (point.visible) {
        if (
          withInRange(
            ...
            point,
            points[this.getNextIndex(points, point.index - 1, series)],
            series
          )
        ) {
          if (firstPoint !== null) {
            direction = this.getSplineDirection(
              series.drawPoints[previous as number],
              firstPoint,
              point,
              xAxis,
              yAxis,
    ...
    if (
      points.length > 0 &&
      series.drawPoints.length > 0 &&
      series.chart.chartAreaType === 'PolarRadar' &&
      series.isClosed
    ) {
      const connectPoints: { first: Points; last: Points } =
        this.getFirstLastVisiblePoint(points);
      direction = this.getSplineDirection(
        series.drawPoints[series.drawPoints.length - 1],
        connectPoints.last,
        {
  ...
  private getSplineDirection(
    ...
    const pt1: ChartLocation = getCoordinate(
      firstPoint.xValue,
      firstPoint.yValue,
      xAxis,
      yAxis,
    ...
    const pt2: ChartLocation = getCoordinate(
      point.xValue,
      point.yValue,
      xAxis,
      yAxis,
    ...
    const bpt1: ChartLocation = getCoordinate(
      controlPoint1.x,
      controlPoint1.y,
      xAxis,
      yAxis,
    ...
    const bpt2: ChartLocation = getCoordinate(
      controlPoint2.x,
      controlPoint2.y,
      xAxis,
      yAxis,
    ...
    for (let i: number = 0; i < point.length; i++) {
      if (series.marker && series.marker.visible) {
        series.chart.markerRender.renderMarker(
          series,
          series.points[point[i as number]],
          series.points[point[i as number]].symbolLocations[0],
          null,
          true
        );
      }
      ...
      if (startPathCommands.length < endPathCommands.length) {
        for (
          ...
          if (endPathCommands.length !== startPathCommands.length) {
            if (
              startPathCommands[startPathCommands.length - 1].indexOf('C') === 0
            ) {
              startPathCommands.push(
                'L ' +
                  startPathCommands[startPathCommands.length - 1]
                    .split(' ')
                    .slice(-3)
                    .join(' ')
              );
            } else if (
              startPathCommands[startPathCommands.length - 1].indexOf('L') === 0
            ) {
              const points: string[] = startPathCommands[
                startPathCommands.length - 1
              ]
                .split(' ')
                .slice(-3);
              startPathCommands.push(
                'C ' + points.join(' ') + points.join(' ') + points.join(' ')
              );
            } else {
              const points: string = startPathCommands[
                startPathCommands.length - 1
        ...
        for (let i: number = minLength; i < maxLength; i++) {
          if (
            series.removedPointIndex === series.points.length &&
            endPathCommands.length !== startPathCommands.length
          ) {
            if (
              endPathCommands[endPathCommands.length - 1].indexOf('C') === 0
            ) {
              ...
              endPathCommands[endPathCommands.length - 1].indexOf('L') === 0
            ) {
              const points: string[] = endPathCommands[
            ...
            if (endPathCommands.length === 1) {
              ...
              endPathCommands.splice(
                ...
                0,
                'C ' +
                  endPathCommands[0].split(' ').slice(-3).join(' ') +
                  endPathCommands[0].split(' ').slice(-3).join(' ') +
                  endPathCommands[0].split(' ').slice(-3).join(' '),
