...
import { PathAttributes, PathOption, Rect } from '@hoyui/svg-base';
...
export class LineBase {
  ...
  public enableComplexProperty(series: Series): Points[] {
    ...
    const xTolerance: number =
      this.chart && this.chart.zoomRedraw && this.chart.redraw
        ? this.previousX
        : Math.abs(xVisibleRange.delta / areaBounds.width);
    const yTolerance: number =
      this.chart && this.chart.zoomRedraw && this.chart.redraw
        ? this.previousY
        : Math.abs(yVisibleRange.delta / areaBounds.height);
    let prevXValue: number =
      seriesPoints[0] && seriesPoints[0].xValue > xTolerance ? 0 : xTolerance;
    let prevYValue: number =
      seriesPoints[0] && seriesPoints[0].y > yTolerance ? 0 : yTolerance;
    ...
    for (const currentPoint of seriesPoints) {
      ...
      if (
        Math.abs(prevXValue - xVal) >= xTolerance ||
        Math.abs(prevYValue - yVal) >= yTolerance
      ) {
    ...
    for (let i: number = 0; i < tempPoints.length; i++) {
      ...
      if (
        isNullOrUndefined(tempPoint.x) ||
        (series.category === 'Indicator' &&
          (isNaN(tempPoint.xValue) || isNaN(tempPoint.yValue)))
      ) {
        continue;
      } else {
  ...
  public appendLinePath(
    ...
    pathAnimation(
      ...
      options.d,
      series.chart.redraw,
      points.previousDirection,
      points.chart.duration
    );
  }

  public appendPathElement(
    options: PathOption | PathAttributes,
    series: Series,
    clipRect: string
  ): {
    element: Element;
    ...
    const previousDirection: string = element
      ...
      : null;
    const htmlObject: HTMLElement = series.chart.renderer.drawPath(
      options,
  ...
  public addPath(options: PathOption, series: Series, clipRect: string): void {
    ...
    if (points.previousDirection !== '' && options.d !== '') {
      const startPathCommands: string[] = points.previousDirection.match(
        /[MLHVCSQTAZ][^MLHVCSQTAZ]*/g
      );
      const endPathCommands: string[] = options.d.match(
        /[MLHVCSQTAZ][^MLHVCSQTAZ]*/g
      );
      ...
      if (startPathCommands.length < endPathCommands.length) {
        for (let i: number = minLength; i < maxLength; i++) {
          if (endPathCommands.length !== startPathCommands.length) {
            startPathCommands.push(
              startPathCommands[startPathCommands.length - 1].replace('M', 'L')
            );
          }
        }
        animateAddPoints(
        ...
        for (let i: number = minLength; i < maxLength; i++) {
          if (endPathCommands.length !== startPathCommands.length) {
            if (series.removedPointIndex === series.points.length) {
              endPathCommands.push(
                endPathCommands[endPathCommands.length - 1].replace('M', 'L')
              );
            } else {
              endPathCommands.splice(
                1,
                0,
                endPathCommands[0].replace('M', 'L')
              );
            }
          }
        }
            ...
            if (endPathCommands.length !== startPathCommands.length) {
              if (startPathCommands.length === 1) {
                startPathCommands.push(
                  startPathCommands[
                    startPathCommands.length -
                      (options.id.indexOf('border') !== -1 ? 1 : 2)
                  ].replace('M', 'L')
                );
              } else {
                startPathCommands.splice(
                  startPathCommands.length - 1,
                  0,
                  startPathCommands[
                    startPathCommands.length -
                      (options.id.indexOf('border') !== -1 ? 1 : 2)
          ...
          if (endPathCommands.length !== startPathCommands.length) {
            if (series.removedPointIndex === series.points.length) {
              ...
              endPathCommands.splice(
                1,
                0,
                endPathCommands[1] ? endPathCommands[1] : endPathCommands[0]
              );
            }
          }
        }
  ...
  public doProgressiveAnimation(series: Series, option: AnimationModel): void {
    ...
    const pathLength: number = (<SVGPathElement>(
      series.pathElement
    )).getTotalLength();
    ...
    animation.animate(path, {
      duration:
        option.duration === 0 && animationMode === 'Enable'
          ? 1000
          : option.duration,
      delay: option.delay,
      progress: (args: AnimationOptions): void => {
        ...
        currentTime = Math.abs(
          Math.round((args.timeStamp * pathLength) / args.duration)
        );
        path.setAttribute('stroke-dasharray', currentTime + ',' + pathLength);
      },
      end: () => {
        const annotations: HTMLElement = <HTMLElement>(
          getElement(series.chart.element.id + '_Annotation_Collections')
        );
        if (annotations) {
  ...
  public withinYRange(point: Points, yAxis: Axis): boolean {
    return (
      point.yValue >= yAxis.visibleRange.min &&
      point.yValue <= yAxis.visibleRange.max
    );
  }

  public GetStepLineDirection(
    currentPoint: ChartLocation,
    previousPoint: ChartLocation,
    ...
    if (stepLineType === 'Right') {
      ...
      const centerX: number =
        previousPoint.x + (currentPoint.x - previousPoint.x) / 2;
      return (
        command +
        ' ' +
        centerX +
   ...
   * @private
   */
  public getFirstLastVisiblePoint(points: Points[]): {
    first: Points;
    last: Points;
  } {
    let first: Points = null;
    let last: Points = null;
   ...
   * @private
   */
  public getBorderDirection(direction: string): string {
    const coordinates: string[] = direction.split(' ');
    if (
      coordinates.length > 3 &&
      !this.chart.stackingAreaSeriesModule &&
      !this.chart.stackingStepAreaSeriesModule
    ) {
      ...
      if (
        coordinates[coordinates.length - 1] === 'L' ||
        coordinates[coordinates.length - 1] === 'M'
      ) {
   ...
   * @private
   */
  public removeEmptyPointsBorder(borderDirection: string): string {
  ...
  public doLinearAnimation(series: Series, animation: AnimationModel): void {
    const clipRect: HTMLElement = <HTMLElement>(
      series.clipRectElement.childNodes[0].childNodes[0]
    );
    ...
    const yCenter: number = series.chart.requireInvertedAxis
      ? +clipRect.getAttribute('height') + +clipRect.getAttribute('y')
    ...
    if (series.marker && series.marker.visible) {
      const markerClipRect: HTMLElement = <HTMLElement>(
        series.symbolElement.childNodes[0].childNodes[0]
      );
