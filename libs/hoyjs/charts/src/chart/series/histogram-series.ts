...
export class HistogramSeries extends ColumnSeries {
  ...
  private calculateBinInterval(yValues: number[], series: Series): void {
    ...
    series.histogramValues.sDValue = Math.sqrt(
      Math.abs(sumValue / yValues.length)
    );
    series.histogramValues.binWidth =
      series.binInterval ||
      Math.round(
        (3.5 * series.histogramValues.sDValue) / Math.pow(yValues.length, 1 / 3)
      ) ||
      1;
  }
  /**
  ...
  public processInternalData(data: Object[], series: Series): Object[] {
    ...
    for (let j: number = 0; j < data.length; ) {
      yCount = yValues.filter(
        (y: number) => y >= min && y < min + binWidth
      ).length;
      if (min + binWidth === max) {
  ...
  public calculateBinValues(series: Series): void {
    ...
    const min: number = series.xAxis.minimum
      ? parseInt(series.xAxis.minimum.toString(), 10)
      : series.xMin;
    const max: number = series.xAxis.maximum
      ? parseInt(series.xAxis.maximum.toString(), 10)
      : series.xMax;
    ...
    if (points) {
      for (let i: number = 0; i < pointsCount; i++) {
        ...
        yValue =
          (Math.exp(
            (-(xValue - mean) * (xValue - mean)) / (2 * sDValue * sDValue)
          ) /
  ...
  private renderNormalDistribution(series: Series): void {
    ...
    if (points) {
      for (let i: number = 0; i < pointsCount; i++) {
        ...
        pointLocation = getPoint(
          xValue,
          yValue * binWidth * yValuesCount,
          series.xAxis,
          series.yAxis,
          series.chart.requireInvertedAxis
        );
        direction +=
          startPoint + ' ' + pointLocation.x + ' ' + pointLocation.y + ' ';
        startPoint = 'L';
      }
    }
    const distributionLine: Element = series.chart.renderer.drawPath(
      new PathOption(
        series.chart.element.id + '_Series_' + series.index + '_NDLine',
        'transparent',
        2,
        series.chart.themeStyle.histogram || series.chart.themeStyle.errorBar,
        series.opacity,
        series.dashArray,
        direction
      ),
    ...
    (<HTMLElement>distributionLine).style.visibility = !series.chart
      .enableCanvas
  ...
  public doAnimation(series: Series): void {
    ...
    if (series.showNormalDistribution) {
      templateAnimate(
        ...
        series.animation.duration === 0 ? 1000 : series.animation.duration,
        500,
        'FadeIn'
      );
    }
