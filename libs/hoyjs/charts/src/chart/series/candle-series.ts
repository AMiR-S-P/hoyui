...
export class CandleSeries extends ColumnBase {
  ...
  public render(series: Series): void {
    ...
    for (const point of series.points) {
      this.renderPoint(
        series,
        point,
        this.sideBySideInfo[series.index],
        borderWidth
      );
    }
  }
  ...
  public renderPoint(
    ...
    if (
      ...
      tickRegion = this.getRectangle(
        ...
        point.xValue + sideBySideInfo.median,
        <number>Math.min(<number>point.high, <number>point.low),
        series
      );
      ...
      centerRegion = this.getRectangle(
        ...
        <number>Math.min(<number>point.open, <number>point.close),
        series
      );

      direction = this.getPathString(tickRegion, centerRegion, series);
      const argsData: IPointRenderEventArgs = this.triggerPointRenderEvent(
        series,
        point
      );
   ...
   */
  protected triggerPointRenderEvent(
    series: Series,
    point: Points
  ...
  private getCandleColor(point: Points, series: Series): string {
    ...
    if (series.enableSolidCandles === false) {
      if (!previousPoint) {
        ...
        return <number>previousPoint.close > <number>point.close
          ? <string>series.bullFillColor ||
              series.chart.themeStyle.bullFillColor
          : <string>series.bearFillColor ||
              series.chart.themeStyle.bearFillColor;
      }
    } else {
      return <number>point.open > <number>point.close
        ? <string>series.bullFillColor || series.chart.themeStyle.bullFillColor
        : <string>series.bearFillColor || series.chart.themeStyle.bearFillColor;
    }
  }
  ...
  public drawCandle(
    ...
    const check: number = series.chart.requireInvertedAxis
      ? rect.height
      : rect.width;
    if (check <= 0) {
      return null;
    ...
    const fill: string = !series.enableSolidCandles
      ? <number>point.open > <number>point.close
        ? argsData.fill
        : 'transparent'
      : argsData.fill;
    ...
    const previousDirection: string = element
      ...
      : null;
    const candleElement: HTMLElement = series.chart.renderer.drawPath(
      options,
