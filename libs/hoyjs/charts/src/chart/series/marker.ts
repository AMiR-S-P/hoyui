...
import { animationMode, isNullOrUndefined } from '@hoyui/base';
...
export const markerShapes: ChartShape[] = [
  'Circle',
  'Triangle',
  'Diamond',
  'Rectangle',
...
export class Marker extends MarkerExplode {
  ...
  public renderMarker(
    ...
    const seriesIndex: number | string =
      series.index === undefined ? series.category : series.index;
    const marker: MarkerSettingsModel = series.marker;
    series.marker.shape = series.marker.shape
      ? series.marker.shape
      : markerShapes[(seriesIndex as number) % 10];
    ...
    const fill: string =
      marker.fill ||
      (isBoxPlot || series.marker.isFilled
        ? point.interior || series.interior
        : '#ffffff');
    let markerElement: Element;
    const parentElement: Element =
      isBoxPlot && !this.chart.enableCanvas
        ? findlElement(
            series.seriesElement.childNodes,
            'Series_' + series.index + '_Point_' + point.index
          )
      ...
      border: {
        color:
          series.type === 'BoxAndWhisker'
            ? !isNullOrUndefined(borderColor) && borderColor !== 'transparent'
              ? borderColor
    ...
    if (!argsData.cancel) {
      ...
      const markerHeight: number =
        argsData.point.marker.height || argsData.height;
      const markerOpacity: number =
        argsData.point.marker.opacity || marker.opacity;
      const markerShape: ChartShape =
        argsData.point.marker.shape || argsData.shape;
      const imageURL: string =
        argsData.point.marker.imageUrl || marker.imageUrl;
      ...
      if (
        ...
        if (
          ...
          (parentElement.lastChild as HTMLElement).id =
            this.elementId +
            '_Series_' +
            seriesIndex +
            '_Point_' +
    ...
    if (marker.visible) {
      ...
      if (series.chart.chartAreaType === 'Cartesian') {
        options = new RectOption(
          this.elementId + '_ChartMarkerClipRect_' + index,
          'transparent',
          { width: 1, color: 'Gray' },
          1,
          {
        ...
        markerClipRect = appendClipElement(
          redraw,
          options,
        ...
        options = new CircleOption(
          ...
          { width: 1, color: 'Gray' },
          1,
          series.clipRect.width / 2 + series.clipRect.x,
          series.clipRect.height / 2 + series.clipRect.y,
  ...
  public doMarkerAnimation(series: Series): void {
    if (
      ...
      const delay: number =
        ...
        (series.animation.duration === 0 && animationMode === 'Enable'
          ? 1000
          : series.animation.duration);
      let duration: number = series.chart.animated
        ? series.chart.duration
        : 200;
      let markerDelay: number = delay;
      const pathLength: number = <SVGPathElement>series.pathElement
        ? (<SVGPathElement>series.pathElement).getTotalLength()
        : 0;
      const distances: number[] = [];
      for (
        let i: number = 1;
        series.type === 'Line' && i < series.points.length;
        i++
      ) {
        if (
          series.points[(i - 1) as number].symbolLocations[0] &&
      ...
      for (let i: number = 0; i < series.points.length; i++) {
        if (series.points[i as number].symbolLocations) {
          if (
            !series.points[i as number].symbolLocations.length ||
            !markerElements[j as number]
          ) {
            continue;
          }
          if (series.type === 'Line') {
          ...
          markerAnimate(
            markerElements[j as number] as HTMLElement,
            markerDelay,
            duration,
            series,
            i,
          ...
          if (incFactor === 2) {
            const lowPoint: ChartLocation = this.getRangeLowPoint(
              series.points[i as number].regions[0],
              series
            );
            markerAnimate(
              markerElements[j + 1] as HTMLElement,
              markerDelay,
              duration,
              series,
              i,
