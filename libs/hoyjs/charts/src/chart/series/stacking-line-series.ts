...
export class StackingLineSeries extends LineBase {
  ...
  public render(
    ...
    for (let i: number = 0; i < pointsLength; i++) {
      visiblePts[i as number].regions = [];
      visiblePts[i as number].symbolLocations = [];
      pointIndex = visiblePts[i as number].index;
      if (
        visiblePts[i as number].visible &&
        withInRange(
          visiblePts[i - 1],
          visiblePts[i as number],
          visiblePts[i + 1],
          series
        )
      ) {
        point1 = getCoordinate(
          visiblePts[i as number].xValue,
        ...
        visiblePts[i as number].regions.push(
          new Rect(
        ...
        if (series.emptyPointSettings.mode !== 'Drop') {
          if (visiblePts[i + 1] && visiblePts[i + 1].visible) {
            ...
            point1 = getCoordinate(
              visiblePts[i + 1].xValue,
              stackedvalue.endValues[nextPointIndex as number],
              xAxis,
              yAxis,
              isInverted,
              series
