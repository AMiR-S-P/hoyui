...
export class AreaSeries extends MultiColoredSeries {
  ...
  public render(
    ...
    const isDropMode: boolean =
      series.emptyPointSettings && series.emptyPointSettings.mode === 'Drop';
    ...
    for (let i: number = 0; i < visiblePoints.length; i++) {
      ...
      if (
        point.visible &&
        withInRange(visiblePoints[i - 1], point, visiblePoints[i + 1], series)
      ) {
    ...
    if (isPolar && direction !== '') {
      ...
      endPoint += this.getAreaPathDirection(
        0,
        origin,
        series,
        isInverted,
      ...
      if (xAxis.isAxisInverse || yAxis.isAxisInverse) {
        direction +=
          series.type === 'Polar'
            ? chart.polarSeriesModule.getPolarIsInversedPath(xAxis, endPoint)
    ...
    this[pointAnimate ? 'addAreaPath' : 'appendLinePath'](
      new PathOption(
        ...
        direction !== ''
          ? direction +
            this.getAreaPathDirection(
              series.points[series.points.length - 1].xValue,
              series.chart.chartAreaType === 'PolarRadar'
                ? series.points[series.points.length - 1].yValue
                : origin,
              series,
              isInverted,
              getCoordinate,
