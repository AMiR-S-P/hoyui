...
export class StackingColumnSeries extends ColumnBase {
    ...
    if (
      ...
      this.rect = this.getRectangle(
        ...
        !series.visible && series.isLegendClicked
          ? startvalue
          : stackedValue.endValues[point.index],
        point.xValue + sideBySideInfo.end,
        !series.visible && series.isLegendClicked
          ? startvalue
          : stackedValue.startValues[point.index],
        series
      );
      this.rect.width = series.columnWidthInPixel
        ? series.columnWidthInPixel
        : this.rect.width;
      this.rect.x = series.columnWidthInPixel
        ? this.rect.x -
          ((series.columnWidthInPixel / 2) * series.rectCount -
            series.columnWidthInPixel * series.position)
        : this.rect.x;
  ...
  public updateDirection(series: Series, point: number[]): void {
    ...
    for (let j: number = 0; j < seriesCollection.length; j++) {
      const visiblePoints: Points[] = getVisiblePoints(
