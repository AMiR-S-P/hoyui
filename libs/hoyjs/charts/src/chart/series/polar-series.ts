...
import { Animation, AnimationOptions, isNullOrUndefined } from '@hoyui/base';
...
export class PolarSeries extends PolarRadarPanel {
  ...
  public render(
    ...
    for (const visiblePoint of series.points) {
      visiblePoint.visible =
        visiblePoint.visible &&
        !(
          (!isNullOrUndefined(yAxisMin) && visiblePoint.yValue < yAxisMin) ||
    ...
    if (series.points.length) {
      if (series.drawType.indexOf('Column') > -1) {
        ...
        series.chart[seriesType + 'SeriesModule'].render(
          series,
          xAxis,
          yAxis,
          inverted
  ...
  public columnDrawTypeRender(series: Series, xAxis: Axis, yAxis: Axis): void {
    ...
    let endAngle: number;
    let itemCurrentXPos: number;
    let radius: number;
    let inversedValue: number;
    let pointStartAngle: number;
    let pointEndAngle: number;
    let x1: number;
    let x2: number;
    let y1: number;
    let y2: number;
    ...
    let min: number = xAxis.actualRange.min;
    const centerX: number = series.clipRect.width / 2 + series.clipRect.x;
    let dStartX: number;
    let dStartY: number;
    const centerY: number = series.clipRect.height / 2 + series.clipRect.y;
    let dEndX: number;
    let dEndY: number;
    const isRangeColumn: boolean = series.drawType === 'RangeColumn';
    const isPolar: boolean = series.type === 'Polar';
    const isLogAxis: boolean = yAxis.valueType === 'Logarithmic';
    const isStacking: boolean = series.drawType === 'StackingColumn';
    let direction: string = '';
    let sumofYValues: number = 0;
    let arcValue: string;
    const interval: number =
      (series.points[1]
        ? series.points[1].xValue
        : 2 * series.points[0].xValue) - series.points[0].xValue;
    const isInverse: boolean = xAxis.isAxisInverse;
    //customer issue ID-I249730, Polar columnSeries in OnTicks with inversed axis
    const ticks: number =
      xAxis.valueType === 'Category' && xAxis.labelPlacement === 'BetweenTicks'
        ? 0
        : isInverse
        ? -interval / 2
        : interval / 2;
    const rangeInterval: number =
      xAxis.valueType === 'DateTime' ? xAxis.dateTimeInterval : 1;
    this.getSeriesPosition(series);
    const position: number = isInverse
      ? series.rectCount - 1 - series.position
      : series.position;
    do {
      ...
      min <=
      xAxis.actualRange.max - (xAxis.valueType === 'Category' ? 0 : 1)
    );
    for (const point of series.points) {
      ...
      if (
        ...
        inversedValue = isInverse
          ? xAxis.visibleRange.max - point.xValue
          : point.xValue - xAxis.visibleRange.min;
        itemCurrentXPos =
          inversedValue +
          ((interval / series.rectCount) * position - ticks) +
          (sumofYValues / 360) * xAxis.startAngle;
        ...
        endAngle =
          ...
          (itemCurrentXPos +
            xAxis.startAngle +
            interval / series.rectCount / sumofYValues);
        ...
        endAngle =
          endAngle - 0.5 * Math.PI - 0.000001 - series.columnSpacing / 2;
        if (isStacking || isRangeColumn) {
          startValue = isRangeColumn
            ? <number>point.low
            : series.stackedValues.startValues[point.index];
          endValue = isRangeColumn
            ? <number>point.high
            : series.stackedValues.endValues[point.index];
          endValue = isLogAxis
            ? logBase(endValue === 0 ? 1 : endValue, yAxis.logBase)
            : endValue;
          endValue =
            endValue > yAxis.actualRange.max ? yAxis.actualRange.max : endValue;
        } else {
          startValue = yAxis.visibleRange.min;
          endValue =
            point.yValue > yAxis.actualRange.max
              ? yAxis.actualRange.max
              : point.yValue;
        }
        radius =
          startValue === endValue
            ? 0
            : series.chart.radius * valueToCoefficient(endValue, yAxis);
        ...
        innerRadius =
          series.chart.radius *
          valueToCoefficient(
            startValue === 0 && yAxis.visibleRange.min !== 0
        ...
        if (!argsData.cancel) {
          ...
          if (isPolar) {
            point.symbolLocations.push({
              x:
                centerX +
                radius * Math.cos(startAngle + (endAngle - startAngle) / 2),
              y:
                centerY +
                radius * Math.sin(startAngle + (endAngle - startAngle) / 2),
            });
            if (isRangeColumn) {
              point.symbolLocations.push({
                x:
  ...
  public getSeriesPosition(series: Series): void {
    ...
    for (const series of chart.visibleSeries) {
      if (
        series.visible &&
        (series.type === 'Polar' || series.type === 'Radar') &&
  ...
  public getPolarIsInversedPath(xAxis: Axis, endPoint: string): string {
    ...
    const vector: ChartLocation = CoefficientToVector(
      valueToPolarCoefficient(xAxis.visibleLabels[0].value, xAxis),
      this.startAngle
    );
