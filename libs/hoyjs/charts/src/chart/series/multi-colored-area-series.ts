...
export class MultiColoredAreaSeries extends MultiColoredSeries {
  ...
  public render(
    ...
    const segments: ChartSegmentModel[] = this.sortSegments(
      series,
      series.segments
    );
    ...
    visiblePoints.map((point: Points, i: number, seriesPoints: Points[]) => {
      ...
      if (
        point.visible &&
        withInRange(seriesPoints[i - 1], point, seriesPoints[i + 1], series)
      ) {
        ...
        if (
          ...
          this.setPointColor(
            ...
            previous,
            series,
            series.segmentAxis === 'X',
            segments
          )
        ) {
          rendered = true;
          startRegion = getPoint(
            startPoint.x,
            origin,
            xAxis,
            yAxis,
          ...
          this.generatePathOption(
            options,
            series,
            previous,
            direction,
            '_Point_' + previous.index
          );
          direction =
            'M' +
            ' ' +
  ...
  private generatePathOption(
    ...
    options.push(
      new PathOption(
        ...
        series.setPointColor(point, series.interior),
        0,
        'transparent',
        series.opacity,
        series.dashArray,
