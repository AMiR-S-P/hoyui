...
export class RangeColumnSeries extends ColumnBase {
  ...
  public renderPoint(
    ...
    if (
      rangePoint.visible &&
      withInRange(
        series.points[rangePoint.index - 1],
        rangePoint,
        series.points[rangePoint.index + 1],
        series
      )
      ...
      rect = this.getRectangle(
        rangePoint.xValue + sideBySideInfo.start,
        <number>rangePoint.high,
        rangePoint.xValue + sideBySideInfo.end,
        <number>rangePoint.low,
        series
      );
  ...
  public updateDirection(series: Series, point: number[]): void {
    for (let i: number = 0; i < point.length; i++) {
      ...
      if (series.marker && series.marker.visible) {
        series.points[point[i as number]].symbolLocations.map(
