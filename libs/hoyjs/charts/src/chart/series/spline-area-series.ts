...
export class SplineAreaSeries extends SplineBase {
  ...
  public render(
    ...
    for (let i: number = 0; i < pointsLength; i++) {
      ...
      if (
        ((i + 1 < pointsLength && !points[i + 1].visible) ||
          i === pointsLength - 1) &&
        pt2 &&
        startPoint
      ) {
    ...
    if (series.border.width !== 0) {
      emptyPointDirection = this.removeEmptyPointsBorder(
        this.getBorderDirection(direction)
      );
      this[pointAnimate ? 'addPath' : 'appendLinePath'](
        new PathOption(
  ...
  public addPath(options: PathOption, series: Series, clipRect: string): void {
    ...
    if (points.previousDirection !== '' && options.d !== '') {
      if (
        points.previousDirection.match(/[MLHVCSQTAZ][^MLHVCSQTAZ]*/g).length ===
        2
      ) {
        points.previousDirection =
          points.previousDirection.match(/[MLHVCSQTAZ][^MLHVCSQTAZ]*/g)[0] +
          'L ' +
          points.previousDirection.split(' ')[1] +
          ' ' +
          points.previousDirection.split(' ')[2] +
          ' C ' +
          points.previousDirection.split(' ')[1] +
          ' ' +
          points.previousDirection.split(' ')[2] +
      ...
      if (startPathCommands.length < endPathCommands.length) {
        for (
          ...
          if (endPathCommands.length !== startPathCommands.length) {
            ...
            if (
              startPathCommands[startPathCommands.length - 1].split(' ')
                .length === 4 &&
              options.id.indexOf('border') !== -1
            ) {
              lastPointBeforeCurve = startPathCommands[
                startPathCommands.length -
            ...
            const curveCommand: string =
              'C ' +
              lastPointBeforeCurve +
              lastPointBeforeCurve +
              lastPointBeforeCurve;
          ...
          if (endPathCommands.length !== startPathCommands.length) {
            ...
            if (series.removedPointIndex === series.points.length) {
              if (
                ...
                firstPointBeforeCurve = endPathCommands[
                  endPathCommands.length -
                    (options.id.indexOf('border') !== -1 ? 1 : 2)
                ]
                  .split(' ')
              ...
              if (
                ...
                firstPointBeforeCurve =
                  'C ' +
                  endPathCommands[options.id.indexOf('border') !== -1 ? 0 : 1]
                    .split(' ')
                    .slice(-3)
                    .join(' ') +
                  endPathCommands[options.id.indexOf('border') !== -1 ? 0 : 1]
