...
export class ChartData {
  ...
  public getData(): PointData {
    ...
    let mouseY: number;
    for (
      let len: number = chart.visibleSeries.length, i: number = len - 1;
      i >= 0;
      i--
    ) {
      ...
      width =
        ...
        series.marker.visible
          ? (series.marker.height + 5) / 2
          : 0;
      height =
        series.type === 'Scatter' ||
        series.drawType === 'Scatter' ||
        series.marker.visible
          ? (series.marker.width + 5) / 2
          : 0;
      mouseX = chart.mouseX;
      mouseY = chart.mouseY;
      if (series.dragSettings.enable && series.isRectSeries) {
        if (
          !(series.type === 'Bar' && chart.isTransposed) &&
          (chart.isTransposed || series.type === 'Bar')
        ) {
          const markerWidth: number = series.marker.width / 2;
          mouseX = series.yAxis.isAxisInverse
            ? mouseX + markerWidth
            : mouseX - markerWidth;
        } else {
          const markerHeight: number = series.marker.height / 2;
          mouseY = series.yAxis.isAxisInverse
            ? mouseY - markerHeight
            : mouseY + markerHeight;
        }
      }
      if (
        series.visible &&
        withInBounds(mouseX, mouseY, series.clipRect, width, height)
      ) {
  ...
  public isSelected(chart: Chart): boolean {
    return (
      (chart.selectionMode.indexOf('Drag') > -1 ||
        chart.selectionMode.indexOf('Lasso') > -1) &&
      chart.selectionModule &&
  ...
  private getRectPoint(
    ...
    const chart: Chart = this.chart;
    let fromCenterX: number;
    let fromCenterY: number;
    let clickAngle: number;
    ...
    for (const point of series.points) {
      ...
      if (
        ...
        arcAngle =
          2 *
          Math.PI *
          (point.regionData.currentXPosition < 0
            ? 1 + point.regionData.currentXPosition
            : point.regionData.currentXPosition);
        clickAngle =
          (Math.atan2(fromCenterY, fromCenterX) + 0.5 * Math.PI - arcAngle) %
          (2 * Math.PI);
        clickAngle = clickAngle < 0 ? 2 * Math.PI + clickAngle : clickAngle;
        clickAngle =
          clickAngle + 2 * Math.PI * series.chart.primaryXAxis.startAngle;
        ...
        distanceFromCenter = Math.sqrt(
          Math.pow(Math.abs(fromCenterX), 2) +
            Math.pow(Math.abs(fromCenterY), 2)
        );
        if (
          clickAngle >= startAngle &&
          clickAngle <= endAngle &&
          ((distanceFromCenter >= point.regionData.innerRadius &&
            distanceFromCenter <= point.regionData.radius) ||
            (distanceFromCenter <= point.regionData.innerRadius &&
              distanceFromCenter >= point.regionData.radius)) &&
      ...
      if (
        (series.dragSettings.enable && series.isRectSeries) ||
        (series.isRectSeries &&
          series.marker.visible &&
          this.chart.chartAreaType !== 'PolarRadar')
      ) {
      ...
      if (
        !this.insideRegion &&
        this.checkRegionContainsPoint(point.regions, rect, x, y)
      ) {
        return point;
      } else if (
        this.insideRegion &&
  ...
  private isPointInThresholdRegion(
    ...
    return point.regions.some((region: Rect) => {
      return withInBounds(
        ...
        y,
        new Rect(
          (this.chart.chartAreaType === 'Cartesian' ? rect.x : 0) +
            region.x +
            xValue,
          (this.chart.chartAreaType === 'Cartesian' ? rect.y : 0) +
            region.y +
            yValue,
          width,
          height
  ...
  public getClosest(series: Series, value: number, xvalues?: number[]): number {
    ...
    if (series.xAxis.valueType === 'DateTime' && series.points.length === 1) {
      ...
      for (
        let index: number = 0;
        index < series.chart.visibleSeries.length;
        index++
      ) {
        const visibleSeries: Series =
          series.chart.visibleSeries[index as number];
        if (
          visibleSeries.xMin >= leftSideNearest &&
          visibleSeries.xMin < series.xMin
        ) {
          leftSideNearest = visibleSeries.xMin + 0.1;
        }
        if (
          visibleSeries.xMax <= rightSideNearest &&
          visibleSeries.xMax > series.xMax
        ) {
          rightSideNearest = visibleSeries.xMax - 0.1;
        }
        if (visibleSeries.visible && visibleSeries.points.length > 1) {
          if (
            visibleSeries.xMax >= leftSideNearest &&
            visibleSeries.xMax < series.xMin
          ) {
            leftSideNearest = visibleSeries.xMax + 0.1;
          }
          if (
            visibleSeries.xMin <= rightSideNearest &&
            visibleSeries.xMin > series.xMax
          ) {
    ...
    if (
      value >= <number>series.xMin - leftSideNearest &&
      value <= <number>series.xMax + rightSideNearest
    ) {
      for (let i: number = 0; i < xLength; i++) {
        data = xData[i as number];
        if (
          closest == null ||
          Math.abs(data - value) < Math.abs(closest - value)
        ) {
          closest = data;
        }
      }
  ...
  public getClosestX(
    ...
    const point: Points =
      (closest || closest === 0) && series.points.length > 0
        ? this.binarySearch(
            closest,
            sort(series.points, ['xValue']) as Points[]
          )
        : null;
    if (point && point.visible) {
      if (
        !(
          ...
          ((series.category === 'Indicator' && series.name === 'Histogram') ||
            (point.symbolLocations &&
              point.symbolLocations.length > 0 &&
              point.symbolLocations[0].x >= 0 &&
  ...
  public mergeXvalues(visibleSeries: Series[]): number[] {
    if (
      ...
      (!this.commonXvalues.length ||
        this.commonXvalues.length !== visibleSeries[0].xData.length)
    ) {
      this.commonXvalues = visibleSeries[0].xData;
      for (let index: number = 1; index < visibleSeries.length; index++) {
        this.commonXvalues = this.getDistinctValues(
          this.commonXvalues,
          visibleSeries[index as number].xData
        );
      }
    }
  ...
  public commonXValue(visibleSeries: Series[]): number[] {
    ...
    for (let j: number = 0; j < visibleSeries.length; j++) {
      for (
        let i: number = 0;
        visibleSeries[j as number].points &&
        i < visibleSeries[j as number].points.length;
        i++
      ) {
        ...
        if (
          point &&
          (point.index === 0 ||
            point.index === visibleSeries[j as number].points.length - 1 ||
