...
export class Zoom {
  ...
  public renderZooming(
    ...
    if (
      ...
      (!isTouch || (chart.isDoubleTap && this.touchStartList.length === 1)) &&
  ...
  private drawZoomingRectangle(chart: Chart): void {
    ...
    const startLocation: ChartLocation = new ChartLocation(
      chart.previousMouseMoveX,
      chart.previousMouseMoveY
    );
    const endLocation: ChartLocation = new ChartLocation(
      chart.mouseX,
      chart.mouseY
    );
    const rect: Rect = (this.zoomingRect = getRectLocation(
      startLocation,
      endLocation,
      areaBounds
    ));
    if (rect.width > 0 && rect.height > 0) {
      ...
      if (chart.tooltipModule) {
        ...
        for (const series of chart.visibleSeries) {
          if (
            ...
            (series.marker.visible || chart.tooltip.shared)
          ) {
      ...
      const svg: Element = chart.enableCanvas
        ? document.getElementById(this.elementId + '_tooltip_svg')
        : chart.svgObject;
      svg.appendChild(
        chart.svgRenderer.drawRectangle(
          new RectOption(
            this.elementId + '_ZoomArea',
            chart.themeStyle.selectionRectFill,
            { color: chart.themeStyle.selectionRectStroke, width: 1 },
            1,
            rect,
            0,
            0,
  ...
  public doPan(
    ...
    for (const axis of axes as Axis[]) {
      ...
      if (axis.orientation === 'Horizontal') {
        offset =
          ...
          axis.rect.width /
          currentScale;
        argsData.currentZoomPosition = minMax(
          axis.zoomPosition + offset,
          0,
          1 - axis.zoomFactor
        );
      } else {
    ...
    const zoomingEventArgs: IZoomingEventArgs = {
      cancel: false,
  ...
  private performDefferedZoom(chart: Chart): void {
    ...
    if (this.zooming.enableDeferredZooming) {
      ...
      this.refreshAxis(
        <CartesianAxisLayoutPanel>chart.chartAxisLayoutPanel,
        chart,
        chart.axisCollections
      );
    ...
    if (this.isZoomed) {
      if (rect.width > 0 && rect.height > 0) {
        ...
        if (chart.enableCanvas) {
          ...
          const zoomToolBar: Element = getElement(
            chart.element.id + '_Zooming_KitCollection'
          );
          if (zoomToolBar) {
            zoomToolBar.remove();
          }
        ...
        if (
          !(this.isPanning && (chart.isChartDrag || this.startPanning)) &&
          !chart.enableCanvas
        ) {
          chart.duration = 600;
        ...
        if (
          ...
          (chart.legendSettings.enableHighlight ||
            chart.highlightMode !== 'None') &&
          chart.highlightModule.highlightDataIndexes
        ) {
  ...
  private refreshAxis(
    layout: CartesianAxisLayoutPanel,
    chart: Chart,
    axes: AxisModel[]
  ): void {
    ...
    layout.measureAxis(
      new Rect(
        chart.initialClipRect.x,
        chart.initialClipRect.y,
        chart.initialClipRect.width,
        chart.initialClipRect.height
      )
    );
    axes.map((axis: Axis, index: number) => {
      if (axis.orientation === 'Horizontal' && mode !== 'Y') {
        layout.drawXAxisLabels(
          axis,
          index,
          null,
  ...
  private doZoom(chart: Chart, axes: AxisModel[], bounds: Rect): void {
    ...
    const zoomedAxisCollections: IAxisData[] = [];
    this.zoomCompleteEvtCollection = [];
    for (const axis of axes as Axis[]) {
      argsData = {
        cancel: false,
      ...
      if (axis.orientation === 'Horizontal') {
        if (mode !== 'Y') {
          argsData.currentZoomPosition +=
            Math.abs((zoomRect.x - bounds.x) / bounds.width) * axis.zoomFactor;
        ...
        if (mode !== 'X') {
          argsData.currentZoomPosition +=
            (1 -
              Math.abs(
                (zoomRect.height + (zoomRect.y - bounds.y)) / bounds.height
              )) *
            axis.zoomFactor;
    ...
    const onZoomingEventArg: IZoomingEventArgs = {
      cancel: false,
  ...
  private redrawOnZooming(
    ...
    const zoomCompleteCollection: IZoomCompleteEventArgs[] = isMouseUp
      ? this.toolkit.zoomCompleteEvtCollection
      : this.zoomCompleteEvtCollection;
    if (isRedraw) {
      this.performZoomRedraw(chart);
    }
    ...
    for (let i: number = 0; i < zoomCompleteCollection.length; i++) {
      if (!zoomCompleteCollection[i as number].cancel) {
        argsData = {
          ...
          previousZoomFactor:
            zoomCompleteCollection[i as number].previousZoomFactor,
          previousZoomPosition:
            zoomCompleteCollection[i as number].previousZoomPosition,
          ...
          previousVisibleRange:
            zoomCompleteCollection[i as number].previousVisibleRange,
        };
   ...
   * @private
   */
  public performMouseWheelZooming(
    e: WheelEvent,
    mouseX: number,
    mouseY: number,
    chart: Chart,
        ...
        if (cumulative >= 1) {
          origin =
            axis.orientation === 'Horizontal'
              ? mouseX / axis.rect.width
              : 1 - mouseY / axis.rect.height;
          origin = origin > 1 ? 1 : origin < 0 ? 0 : origin;
          zoomFactor =
            cumulative === 1
              ? 1
              : minMax((direction > 0 ? 0.9 : 1.1) / cumulative, 0, 1);
          zoomPosition =
            cumulative === 1
              ? 0
              : axis.zoomPosition + (axis.zoomFactor - zoomFactor) * origin;
          if (
    ...
    const onZoomingEventArgs: IZoomingEventArgs = {
      cancel: false,
      axisCollection: zoomedAxisCollection,
      name: onZooming,
    };
  ...
  public performPinchZooming(e: TouchEvent, chart: Chart): boolean {
    if (
      (this.zoomingRect.width > 0 && this.zoomingRect.height > 0) ||
      (chart.startMove && chart.crosshair.enable)
    ) {
      return false;
    }
    this.calculateZoomAxesRange(chart);
    ...
    const scaleX: number =
      Math.abs(touch0EndX - touch1EndX) / Math.abs(touch0StartX - touch1StartX);
    const scaleY: number =
      Math.abs(touch0EndY - touch1EndY) / Math.abs(touch0StartY - touch1StartY);
    ...
    const pinchRect: Rect = new Rect(
      clipX,
      clipY,
      this.offset.width / scaleX,
      this.offset.height / scaleY
    );
    ...
    for (let index: number = 0; index < chart.axisCollections.length; index++) {
      ...
      if (
        ...
        if (axis.orientation === 'Horizontal') {
          ...
          rangeMin =
            (value * -1 + axis.rect.height) / axisTrans +
            this.zoomAxes[index as number].min;
        ...
        currentZP =
          (selectionMin - this.zoomAxes[index as number].actualMin) /
          this.zoomAxes[index as number].actualDelta;
        currentZF =
          (selectionMax - selectionMin) /
          this.zoomAxes[index as number].actualDelta;
        argsData.currentZoomPosition = currentZP < 0 ? 0 : currentZP;
        argsData.currentZoomFactor =
          currentZF > 1 ? 1 : currentZF < 0.03 ? 0.03 : currentZF;
        if (!argsData.cancel) {
          axis.zoomFactor = argsData.currentZoomFactor;
          axis.zoomPosition = argsData.currentZoomPosition;
          chart.zoomRedraw = this.zooming.enableAnimation;
  ...
  private setTransform(
    ...
    if (!chart.enableCanvas) {
      chart.seriesElements.setAttribute(
        'clip-path',
        'url(#' + this.elementId + '_ChartAreaClipRect_)'
      );
    }
    if (chart.indicatorElements) {
      chart.indicatorElements.setAttribute(
        'clip-path',
        'url(#' + this.elementId + '_ChartAreaClipRect_)'
      );
    }
    ...
    if (transX !== null && transY !== null) {
      for (const value of chart.visibleSeries) {
        xAxisLoc = chart.requireInvertedAxis
          ? value.yAxis.rect.x
          : value.xAxis.rect.x;
        yAxisLoc = chart.requireInvertedAxis
          ? value.xAxis.rect.y
          : value.yAxis.rect.y;
        translate =
          'translate(' +
          (transX + (isPinch ? scaleX * xAxisLoc : xAxisLoc)) +
          ',' +
        ...
        if (value.visible) {
          if (value.category === 'Indicator') {
            (value.seriesElement.parentNode as HTMLInputElement).setAttribute(
              'transform',
              translate
            );
          } else {
  ...
  private showZoomingToolkit(chart: Chart): boolean {
    ...
    const length: number = this.isDevice
      ? toolboxItems.length === 0
        ? 0
        : 1
      : toolboxItems.length;
    const iconSize: number = this.isDevice
      ? measureText(
          'Reset Zoom',
          { size: '12px' },
          {
      ...
      : 16;
    const height: number = this.isDevice
      ? measureText(
          'Reset Zoom',
          { size: '12px' },
          {
      ...
      : chart.theme.indexOf('Fluent2') > -1 ||
        chart.theme.indexOf('Bootstrap5') > -1
      ? 18
      : 22;
    const width: number =
      length * iconSize + (length + 1) * spacing + (length - 1) * spacing;
    ...
    const toolkit: Toolkit = this.toolkit;
    let element: Element;
    let shadowElement: string =
      '<filter id="chart_shadow" height="130%"><feGaussianBlur in="SourceAlpha" stdDeviation="5"/>';
    shadowElement +=
    ...
    const zoomFillColor: string =
      this.chart.theme === 'Tailwind'
        ...
        ? '#000000'
        : chart.theme === 'Bootstrap5'
        ? '#E9ECEF'
        : chart.theme === 'Bootstrap5Dark'
        ? '#343A40'
        : '#fafafa';
    this.toolkitElements.appendChild(
      render.drawRectangle(
        new RectOption(
          ...
          1,
          new Rect(0, 0, width, height + spacing * 2),
          this.chart.theme.indexOf('Bootstrap5') > -1 ? 1 : 4,
          this.chart.theme.indexOf('Bootstrap5') > -1 ? 1 : 4
        )
      ) as HTMLElement
    );
    const outerElement: Element = render.drawRectangle(
      new RectOption(
        this.elementId + '_Zooming_Rect',
        zoomFillColor,
        { color: 'transparent', width: 1 },
    ...
    if (
      ...
      outerElement.setAttribute(
        'rx',
        this.chart.theme.indexOf('Bootstrap5') > -1 ? '1px' : '4px'
      );
      outerElement.setAttribute(
        'ry',
        this.chart.theme.indexOf('Bootstrap5') > -1 ? '1px' : '4px'
    ...
    for (let i: number = 1; i <= length; i++) {
      ...
      switch (currentItem) {
        ...
        case 'ZoomOut':
          ...
          break;
        case 'Reset':
          toolkit.createResetButton(
            element,
            this.toolkitElements,
            chart,
            this.isDevice
          );
          break;
    ...
    this.toolkitElements.setAttribute(
      'opacity',
      this.isDevice ? '1' : '' + this.zoomkitOpacity
    );
    ...
    if (chart.enableCanvas) {
      ...
      document
        .getElementById(this.elementId + '_Secondary_Element')
        .appendChild(zoomDiv);
    } else {
    ...
    if (!this.isDevice) {
      EventHandler.add(
        ...
        this.zoomToolkitMove,
        this
      );
      EventHandler.add(
        this.toolkitElements,
        'mouseleave touchend',
        this.zoomToolkitLeave,
        this
      );
  ...
  public zoomCancel(
    axes: AxisModel[],
    zoomCompleteEventCollection: IZoomCompleteEventArgs[]
  ): void {
  ...
  public isAxisZoomed(axes: AxisModel[]): boolean {
    ...
    for (const axis of axes as Axis[]) {
      showToolkit =
        showToolkit || axis.zoomFactor !== 1 || axis.zoomPosition !== 0;
    }
    return showToolkit;
  }
  ...
  public chartMouseWheel(e: WheelEvent): boolean {
    ...
    const mouseX: number =
      e.clientX - offset.left - Math.max(svgRect.left - offset.left, 0);
    const mouseY: number =
      e.clientY - offset.top - Math.max(svgRect.top - offset.top, 0);

    if (
      this.zooming.enableMouseWheelZooming &&
      withInBounds(mouseX, mouseY, chart.chartAxisLayoutPanel.seriesClipRect)
    ) {
      e.preventDefault();
      this.performMouseWheelZooming(
        e,
        mouseX,
        mouseY,
        chart,
  ...
  private mouseMoveHandler(e: PointerEvent | TouchEvent): void {
    ...
    if (chart.isChartDrag) {
      if (chart.isTouch) {
        this.touchMoveList = this.addTouchPointer(
          <ITouches[]>this.touchMoveList,
          <PointerEvent>e,
          touches
        );
        if (
          this.zooming.enablePinchZooming &&
          this.touchMoveList.length > 1 &&
  ...
  private mouseDownHandler(e: PointerEvent): void {
    ...
    if (
      target.id.indexOf(chart.element.id + '_Zooming_') === -1 &&
      (chart.zoomSettings.enablePinchZooming ||
        chart.zoomSettings.enableSelectionZooming) &&
      withInBounds(
        chart.previousMouseMoveX,
        chart.previousMouseMoveY,
        chart.chartAxisLayoutPanel.seriesClipRect
    ...
    if (chart.isTouch) {
      this.touchStartList = this.addTouchPointer(
  ...
  private mouseUpHandler(e: PointerEvent): void {
    const chart: Chart = this.chart;
    const performZoomRedraw: boolean =
      (<Element>e.target).id.indexOf(chart.element.id + '_ZoomOut_') === -1 ||
  ...
  public addTouchPointer(
    ...
    if (touches) {
      touchList = [];
      for (
        let i: number = 0, length: number = touches.length;
        i < length;
        i++
      ) {
        touchList.push({
      ...
      if (touchList.length === 0) {
        ...
        for (
          let i: number = 0, length: number = touchList.length;
          i < length;
          i++
        ) {
