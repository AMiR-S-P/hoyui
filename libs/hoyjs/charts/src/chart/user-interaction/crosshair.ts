...
export class Crosshair {
    ...
    if (chart.stockChart && chart.stockChart.onPanning) {
      if (chart.mouseY < chart.chartAxisLayoutPanel.seriesClipRect.y) {
        ...
        chart.chartAxisLayoutPanel.seriesClipRect.y +
          chart.chartAxisLayoutPanel.seriesClipRect.height
      ) {
        chart.mouseY =
          chart.chartAxisLayoutPanel.seriesClipRect.y +
          chart.chartAxisLayoutPanel.seriesClipRect.height;
  ...
  public crosshair(): void {
    ...
    let axisTooltipGroup: Element = document.getElementById(
      this.elementID + '_crosshair_axis'
    );
    const crosshair: CrosshairSettingsModel = chart.crosshair;
    let tooltipdiv: Element = document.getElementById(
      this.elementID + '_tooltip'
    );
    const chartRect: Rect = chart.chartAxisLayoutPanel.seriesClipRect;
    const crossGroup: HTMLElement = chart.enableCanvas
      ? document.getElementById(this.elementID + '_Secondary_Element')
      : document.getElementById(this.elementID + '_UserInteraction');
    let crosshairsvg: Element;
    let cross: HTMLElement = document.getElementById(
      this.elementID + '_Crosshair'
    );
    ...
    if (
      ...
      chart.tooltipModule &&
      !withInBounds(
        chart.tooltipModule.valueX,
        chart.tooltipModule.valueY,
        chartRect
      )
    ) {
      return null;
    ...
    this.valueX =
      chart.tooltip.enable && chart.tooltipModule && chart.tooltipModule.valueX
        ? chart.tooltipModule.valueX
        : chart.mouseX;
    this.valueY =
      chart.tooltip.enable && chart.tooltipModule && chart.tooltipModule.valueY
        ? chart.tooltipModule.valueY
        : chart.mouseY;
    if (!chart.enableCanvas) {
    ...
    if (chart.enableCanvas) {
      ...
      const elementID: string = chart.tooltip.enable
        ? chart.element.id + '_tooltip_svg'
        : chart.element.id + '_svg';
      ...
      if (chart.tooltip.enable) {
        tooltipdiv = !tooltipdiv
          ? chart.tooltipModule.createElement()
          : tooltipdiv;
        tooltipdiv.appendChild(crosshairsvg);
        crossGroup.appendChild(tooltipdiv);
      }
      options = new PathOption(
        this.elementID + '_HorizontalLine',
        'none',
        crosshair.line.width,
        crosshair.horizontalLineColor ||
          crosshair.line.color ||
          chart.themeStyle.crosshairLine,
        crosshair.opacity,
        chart.theme.indexOf('Bootstrap5') > -1 ||
        chart.theme === 'Fluent2HighContrast'
          ? crosshair.dashArray || '2.5'
          : crosshair.dashArray,
        horizontalCross
      );
      this.drawCrosshairLine(
        options,
        cross,
        chartRect.x,
        this.valueY,
      ...
      if (crossGroup.childNodes.length === 0) {
        ...
        document
          .getElementById(this.elementID + '_HorizontalLine')
          .setAttribute('d', horizontalCross);
        document
          .getElementById(this.elementID + '_VerticalLine')
          .setAttribute('d', verticalCross);
        this.renderAxisTooltip(chart, chartRect, <Element>crossGroup.lastChild);
      }
    }
  }
  ...
  private renderCrosshairLine(
    options: PathOption,
    crossGroup: HTMLElement
  ): void {
    const htmlObject: HTMLElement = this.chart.renderer.drawPath(
      options
    ) as HTMLElement;
  ...
  private drawCrosshairLine(
    ...
    if (document.getElementById(options.id)) {
      ...
      const crosshairtooltip: HTMLElement = document.getElementById(
        this.elementID + '_crosshair_axis'
      );
  ...
  private renderAxisTooltip(
    ...
    let axisRect: Rect;
    for (
      let k: number = 0, length: number = chart.axisCollections.length;
      k < length;
      k++
    ) {
      ...
      if (axis.crosshairTooltip.enable) {
        if (
          axisRect &&
          ((this.valueX <= axisRect.x + axisRect.width &&
            axisRect.x <= this.valueX) ||
            (this.valueY <= axisRect.y + axisRect.height &&
              axisRect.y <= this.valueY))
        ) {
          pathElement = document.getElementById(
            this.elementID + '_axis_tooltip_' + k
          );
          textElem = document.getElementById(
            this.elementID + '_axis_tooltip_text_' + k
          );
          ...
          if (
            rect.y + rect.height / 2 > chart.availableSize.height ||
            rect.y < 0
          ) {
            continue;
          }
          if (pathElement === null) {
            if (chart.enableCanvas) {
              pathElement = this.svgRenderer.drawPath({
                ...
                fill:
                  axis.crosshairTooltip.fill || chart.themeStyle.crosshairFill,
              });
            } else {
              pathElement = chart.renderer.drawPath(
                {
                  id: this.elementID + '_axis_tooltip_' + k,
                  fill:
                    axis.crosshairTooltip.fill ||
                    chart.themeStyle.crosshairFill,
                },
                null
              );
            }
            ...
            options = new TextOption(
              ...
              0,
              0,
              chart.stockChart && chart.enableRtl ? 'end' : 'start',
              text
            );
            const render: SvgRenderer | CanvasRenderer = chart.enableCanvas
              ? this.svgRenderer
              : chart.renderer;
            textElem = textElement(
              ...
              axis.crosshairTooltip.textStyle.color ||
          ...
          textElem.setAttribute(
            ...
            (
              rect.x +
              padding +
              (chart.enableRtl ? this.elementSize.width : 0)
            ).toString()
          );
          textElem.setAttribute(
            'y',
            (rect.y + padding + (3 * this.elementSize.height) / 4).toString()
          );
          ...
          if (typeof text !== 'string' && text.length > 1) {
            ...
            textElem.setAttribute(
              ...
              (
                rect.y +
                padding +
                (3 * this.elementSize.height) / (4 * text.length)
              ).toString()
            );
            for (let i: number = 0; i < textElem.children.length; i++) {
              height += this.elementSize.height / text.length;
              textElem.children[i as number].setAttribute(
                'x',
                (
                  rect.x +
                  padding +
                  (chart.enableRtl ? this.elementSize.width : 0) +
                  this.elementSize.width / 2
                ).toString()
              );
              textElem.children[i as number].setAttribute(
                'y',
                (parseInt(textElem.getAttribute('y'), 10) + height).toString()
              );
              textElem.children[i as number].setAttribute(
                'style',
                'text-anchor: middle'
          ...
          if (
            ...
            this.chart.theme === 'FluentDark' ||
            this.chart.theme === 'Fabric' ||
            this.chart.theme === 'FabricDark' ||
            this.chart.theme === 'Fluent2HighContrast'
          ) {
            const defElement: Element = this.chart.renderer.createDefs();
            const bordercolor: string =
              this.chart.theme === 'Fluent' || this.chart.theme === 'Fabric'
                ? '#D2D0CE'
                : this.chart.theme === 'Fluent2HighContrast'
                ? '#FFFFFF'
                : null;
            ...
            defElement.setAttribute(
              'id',
              this.chart.element.id + 'SVG_tooltip_definition'
            );
            ...
            pathElement.setAttribute(
              'box-shadow',
              '0px 1.6px 3.6px 0px #00000021, 0px 0.3px 0.9px 0px #0000001A'
            );
            pathElement.setAttribute(
              'filter',
  ...
  private getAxisText(axis: Axis): string {
    ...
    if (axis.orientation === 'Horizontal') {
      value =
        getValueXByPoint(
          Math.abs(this.valueX - axis.rect.x),
          axis.rect.width,
          axis
        ) + labelValue;
      ...
      value =
        getValueYByPoint(
          Math.abs(this.valueY - axis.rect.y),
          axis.rect.height,
          axis
        ) + labelValue;
    ...
    if (axis.valueType === 'DateTime') {
      ...
      return this.chart.dateTimeCategoryModule.getIndexedAxisLabel(
        axis.labels[Math.round(<number>value)],
        axis.format
      );
    } else if (axis.valueType === 'Logarithmic') {
      return (value = axis.format(Math.pow(axis.logBase, value)));
    } else {
      const customLabelFormat: boolean =
        axis.labelFormat && axis.labelFormat.match('{value}') !== null;
      return customLabelFormat
        ? axis.labelFormat.replace('{value}', axis.format(value))
        : axis.format(value);
    }
  }
  ...
  private tooltipLocation(
    ...
    const isOpposed: boolean = axis.isAxisOpposedPosition;
    if (axis.orientation === 'Horizontal') {
      const yLocation: number = islabelInside
        ? axisRect.y - this.elementSize.height - (padding * 2 + arrowPadding)
        : axisRect.y + scrollBarHeight;
      const height: number = islabelInside
        ? axisRect.y - this.elementSize.height - arrowPadding
        : axisRect.y + arrowPadding;
      this.arrowLocation = new ChartLocation(this.valueX, yLocation);

      tooltipRect = new Rect(
        this.valueX - this.elementSize.width / 2 - padding,
        height + (!islabelInside ? scrollBarHeight : 0),
        this.elementSize.width + padding * 2,
        this.elementSize.height + padding * 2
      );
      if (isOpposed) {
        tooltipRect.y = islabelInside
          ? axisRect.y
          : axisRect.y -
            (this.elementSize.height + padding * 2 + arrowPadding) -
            scrollBarHeight;
      }
      ...
      if (tooltipRect.x + tooltipRect.width > boundsX + bounds.width) {
        tooltipRect.x -=
          tooltipRect.x + tooltipRect.width - (boundsX + bounds.width);
      }
      if (
        this.arrowLocation.x + arrowPadding / 2 >
        tooltipRect.x + tooltipRect.width - this.rx
      ) {
        this.arrowLocation.x =
          tooltipRect.x + tooltipRect.width - this.rx - arrowPadding;
      }
      ...
      const width: number = islabelInside
        ? axisRect.x - scrollBarHeight
      ...
      if (isOpposed) {
        tooltipRect.x = islabelInside
          ? axisRect.x - this.elementSize.width - arrowPadding
        ...
        if (
          ...
          this.arrowLocation.x -=
            tooltipRect.x + tooltipRect.width - this.chart.availableSize.width;
          tooltipRect.x -=
            tooltipRect.x + tooltipRect.width - this.chart.availableSize.width;
        }
      } else {
      ...
      if (tooltipRect.y + tooltipRect.height >= boundsY + bounds.height) {
        tooltipRect.y -=
          tooltipRect.y + tooltipRect.height - (boundsY + bounds.height);
      }
      if (
        this.arrowLocation.y + arrowPadding / 2 >
        tooltipRect.y + tooltipRect.height - this.ry
      ) {
        this.arrowLocation.y =
          tooltipRect.y + tooltipRect.height - this.ry - arrowPadding / 2;
      }
  ...
  public removeCrosshair(duration: number): void {
    const chart: Chart = this.chart;
    const crosshair: HTMLElement = chart.enableCanvas
      ? document.getElementById(this.elementID + '_Crosshair')
      : document.getElementById(this.elementID + '_UserInteraction');
    const crosshairtooltip: HTMLElement = chart.enableCanvas
      ? document.getElementById(this.elementID + '_crosshair_axis')
      : null;
    this.stopAnimation();
    if (crosshair && crosshair.getAttribute('opacity') !== '0') {
      this.crosshairInterval = +setTimeout((): void => {
          ...
          progress: (args: AnimationOptions): void => {
            ...
            if (!chart.enableCanvas) {
              crosshair.setAttribute(
                'opacity',
                (1 - args.timeStamp / args.duration).toString()
              );
            } else {
              crosshair.style.opacity = (
                1 -
                args.timeStamp / args.duration
              ).toString();
              crosshairtooltip.style.opacity = (
                1 -
                args.timeStamp / args.duration
              ).toString();
            }
