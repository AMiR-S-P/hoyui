 ...
 * Selection src file
 */
import { AnimationOptions, Animation, Browser } from '@hoyui/base';
import { remove } from '@hoyui/base';
import { extend, isNullOrUndefined } from '@hoyui/base';
import {
  ChartLocation,
  RectOption,
  CircleOption,
...
import { Rect, SvgRenderer, CanvasRenderer, PathOption } from '@hoyui/svg-base';
...
export class Selection extends BaseSelection {
  ...
  private addEventListener(): void {
    if (
      this.chart.isDestroyed ||
      (this.chart.stockChart && this.chart.stockChart.onPanning)
    ) {
      return;
    }
  ...
  public invokeSelection(chart: Chart): void {
    ...
    if (!(chart.selectionMode.indexOf('Drag') > -1)) {
      this.selectDataIndex(
        chart,
        this.concatIndexes(chart.selectedDataIndexes, this.selectedDataIndexes)
      );
    }
  }
  public generateStyle(series: SeriesModel): string {
    if (series) {
      if (
        this.styleId.indexOf('selection') > 1 &&
        this.chart.selectionMode !== 'None'
      ) {
        this.unselected = series.unSelectedStyle || this.unselected;
      }
      if (
        this.styleId.indexOf('highlight') > 0 &&
        (this.chart.highlightMode !== 'None' ||
          this.chart.legendSettings.enableHighlight)
      ) {
        this.unselected = series.nonHighlightStyle || this.unselected;
      }
      return (
        series.selectionStyle ||
        this.styleId + '_series_' + (<Series>series).index
      );
    }
    return 'undefined';
  }
  ...
  public selectDataIndex(chart: Chart, indexes: Index[]): void {
    for (const index of indexes) {
      this.performSelection(
        index,
        chart,
        this.getElementByIndex(
          chart,
          index,
          '',
          this.series[index.series].marker.visible
        )[0]
      );
    }
  }
  ...
  public getElementByIndex(
    ...
    elementId =
      series.type !== 'Scatter' && series.type !== 'Bubble' && marker
        ? elementId + '_Symbol' + suffix
        : elementId;
    ...
    return [
      ...
      (series.type === 'RangeArea' ||
        ...
        series.type === 'RangeStepArea') &&
      series.marker.visible
        ? getElement(elementId + '1')
        : null,
    ];
  }
  ...
  public getClusterElements(chart: Chart, index: Index): Element[] {
    ...
    for (const series of chart.visibleSeries) {
      if (series.visible) {
        ...
        clusters.push(
          this.getElementByIndex(chart, index, '', series.marker.visible)[0]
        );
        seriesStyle = this.generateStyle(chart.visibleSeries[index.series]);
        selectedElements = <NodeListOf<HTMLElement>>(
          document.querySelectorAll('.' + seriesStyle)
        );
        this.findTrackballElements(selectedElements, seriesStyle);
        const clusterIndex: number =
          series.marker.visible && series.isRectSeries ? 2 : 1;
        if (
          !chart.isMultiSelect &&
          selectedElements.length > 0 &&
          selectedElements[0].id !==
            (clusters[clusters.length - clusterIndex]
              ? clusters[clusters.length - clusterIndex].id
              : '')
        ) {
   ...
   * @returns {void}
   */
  public findTrackballElements(
    selectedElements: Element[] | NodeListOf<HTMLElement>,
    className: string
  ): void {
    let trackballElements: Element[];
    let elements: Element[];
    for (let i: number = 0; i < selectedElements.length; i++) {
      if (!isNullOrUndefined(selectedElements[i as number])) {
        trackballElements = !isNullOrUndefined(
          selectedElements[i as number].parentNode
        )
          ? <Element[]>(
              [].slice.call(
                (<Element>selectedElements[0].parentNode).querySelectorAll(
                  '.' + className
                )
              )
            )
    ...
    if (
      ...
      for (let i: number = 0; i < this.previousSelectedEle.length; i++) {
        if (
          ...
          this.previousSelectedEle[i as number].hasAttribute('class')
        ) {
          if (
            this.previousSelectedEle[i as number]
              .getAttribute('class')
            ...
            if (
              ...
              !isNullOrUndefined(this.chart.highlightColor) &&
              this.chart.highlightPattern === 'None'
            ) {
              if (
                this.previousSelectedEle[i as number].id.indexOf('Group') > 0
              ) {
                for (
                  let j: number = 0;
                  j < this.previousSelectedEle[i as number].children.length;
                  j++
                ) {
                  this.previousSelectedEle[i as number].children[
                    j as number
                  ].setAttribute(
                    'fill',
                    (this.control as Chart).visibleSeries[
                      this.indexFinder(this.previousSelectedEle[i as number].id)
                        .series
                    ].interior
                  );
                }
              } else {
                this.previousSelectedEle[i as number].setAttribute(
                  'fill',
                  (this.control as Chart).visibleSeries[
                    this.indexFinder(this.previousSelectedEle[i as number].id)
                      .series
            ...
            this.addOrRemoveIndex(
              this.highlightDataIndexes,
              this.indexFinder(
                (<HTMLElement>this.previousSelectedEle[i as number]).id
              )
            );
          } else if (
            !isElement &&
  ...
  private mouseClick(event: Event): void {
    ...
    if (
      ...
      (<Element>event.target).id.indexOf('_chart_legend_') === -1 &&
   ...
   * @returns {void}
   */
  public calculateSelectedElements(
    ...
    if (eventType === 'mousemove' || eventType === 'pointermove') {
      if (
        ...
        (targetElement.getAttribute('class').indexOf('highlight') > -1 ||
          targetElement.getAttribute('class').indexOf('selection') > -1)
      ) {
        return;
      }
      if (
        !isNullOrUndefined(targetElement.parentNode) &&
        (<Element>targetElement.parentNode).hasAttribute('class') &&
    ...
    if (
      targetElement.id &&
      targetElement.id.indexOf('_Series_') > -1 &&
      targetElement.id.indexOf('_Text_') === -1
    ) {
      let element: Element;
      if (targetElement.id.indexOf('_Trackball_1') > -1) {
        element = getElement(
          targetElement.id.split('_Trackball_')[0] + '_Symbol'
        );
        element = isNullOrUndefined(element)
          ? getElement(targetElement.id.split('_Trackball_')[0])
          : element;
      } else if (targetElement.id.indexOf('_Trackball_0') > -1) {
        return null;
      }
      this.performSelection(
        this.indexFinder(targetElement.id),
        this.chart,
  ...
  public performSelection(index: Index, chart: Chart, element?: Element): void {
    ...
    if (
      chart.visibleSeries[index.series].type === 'BoxAndWhisker' &&
      element &&
      element.id ===
        chart.element.id +
          '_Series_' +
    ...
    if (
      chart.visibleSeries[index.series].type === 'Area' &&
      (this.currentMode === 'Point' || this.currentMode === 'Cluster') &&
      element &&
      ...
      const selectionEle: NodeListOf<HTMLElement> = document.querySelectorAll(
        '.' + className
      );
    ...
    switch (this.currentMode) {
      case 'Series':
        this.selection(
          chart,
          index,
          this.getSeriesElements(chart.series[index.series])
        );
      ...
      case 'Point':
        if (!isNaN(index.point) && element) {
          ...
          if (
            pointElements[0] !== null &&
            chart.series[index.series].marker.visible &&
            (chart.series[index.series].type.indexOf('Column') !== -1 ||
              chart.series[index.series].type.indexOf('Bar') !== -1)
          ) {
            if (
              !(element.id.indexOf('_Symbol') !== -1) &&
              getElement(element.id + '_Symbol')
            ) {
              pointElements.push(getElement(element.id + '_Symbol'));
            } else if (
              element.id.indexOf('_Symbol') !== -1 &&
              getElement(element.id.replace('_Symbol', ''))
            ) {
   ...
   * @returns {void}
   */
  public selectionComplete(
    chart: Chart,
    index: Index,
    selectionMode: SelectionMode | HighlightMode
  ): void {
    let points: Points[];
    ...
    let yValue: number;
    let selectedPointX: string | number | Date;
    if (selectionMode === 'Cluster') {
      for (const series of chart.visibleSeries) {
        if (series.visible) {
          for (let i: number = 0; i < this.selectedDataIndexes.length; i++) {
            pointIndex = chart.isMultiSelect
              ? this.selectedDataIndexes[i as number].point
              : index.point;
            ...
            if (!isNaN(pointIndex) && pointIndex < points.length) {
              yValue =
                series.type !== 'RangeArea' ||
                series.type.indexOf('SplineRangeArea') > -1 ||
                series.type.indexOf('RangeStepArea') > -1
                  ? points[pointIndex as number].yValue
              ...
              if (chart.primaryXAxis.valueType === 'Category') {
                selectedPointX =
                  points[pointIndex as number].x.toLocaleString();
              ...
              if (
                ...
                selectedPointValues.push({
                  x: selectedPointX,
                  y: points[pointIndex as number].regions[0].y,
                  seriesIndex: seriesIndex,
                  pointIndex: pointIndex,
                });
              }
      ...
      if (chart.isMultiSelect) {
        ...
        seriesIndex =
          this.selectedDataIndexes.length > 0
            ? this.selectedDataIndexes[0].series
            : this.highlightDataIndexes && this.highlightDataIndexes.length > 0
            ? this.highlightDataIndexes[0].series
            : 0;
        if (
          this.selectedDataIndexes.length > 0 ||
          (this.highlightDataIndexes && this.highlightDataIndexes.length > 0)
  ...
  public selection(
    ...
    if (!(this.currentMode === 'Lasso')) {
      if (
        !chart.isMultiSelect &&
        this.currentMode.indexOf('Drag') === -1 &&
        this.styleId.indexOf('highlight') === -1 &&
        chart.selectionMode !== 'None'
      ) {
        this.removeMultiSelectElements(
          chart,
          this.selectedDataIndexes,
          index,
          chart.series
    ...
    const indexValue: number = this.rangeColorMappingEnabled()
      ? 0
      : index.series;
    if (!isNullOrUndefined(selectedElements[0])) {
        ...
        if (selectedElements[0].id) {
          if (document.getElementById(selectedElements[0].id + '_Symbol')) {
            selectedElements.push(
              getElement(selectedElements[0].id + '_Symbol')
            );
          } else if (selectedElements[0].id.indexOf('SeriesGroup') !== -1) {
            if (
              document.getElementById(
                selectedElements[0].id.replace('SeriesGroup', 'SymbolGroup')
              )
            ) {
              selectedElements.push(
                getElement(
      ...
      if (
        selectedElements[0] &&
        className.indexOf(this.getSelectionClass(selectedElements[0].id)) > -1
      ) {
        this.removeStyles(selectedElements);
      } else if (
        selectedElements[0].parentNode &&
        pClassName.indexOf(this.getSelectionClass(selectedElements[0].id)) > -1
      ) {
        this.removeStyles([<Element>selectedElements[0].parentNode]);
      } else {
        this.previousSelectedEle =
          chart.highlightMode !== 'None' || chart.legendSettings.enableHighlight
            ? selectedElements
            : [];
  ...
  public clusterSelection(chart: Chart, index: Index): void {
    this.selection(
      chart,
      index,
      this.getClusterElements(chart, new Index(index.series, index.point))
    );
  }
  /**
  ...
  public removeMultiSelectElements(
    ...
    for (let i: number = 0; i < index.length; i++) {
      ...
      if (
        ...
        (!this.isSeriesMode &&
          this.toEquals(index[i as number], currentIndex, true) &&
          !this.toEquals(index[i as number], currentIndex, false))
      ) {
        this.removeStyles(
          this.findElements(chart, series, index[i as number], '', false)
        );
        if (series.marker.visible) {
          this.removeStyles(
            this.findElements(chart, series, index[i as number], '', true)
          );
        }
        index.splice(i, 1);
        i--;
  ...
  public blurEffect(
    ...
    const visibility: boolean =
      this.checkVisibility(this.highlightDataIndexes, this.chart) ||
      this.checkVisibility(this.selectedDataIndexes, this.chart); // legend click scenario
    for (const series of visibleSeries) {
      let legendIndex: number;
      let legendStrokeColor: string;
      if (this.rangeColorMappingEnabled()) {
        if (isLegend === false) {
          legendIndex = Object.keys(series.rangeColorPoints).indexOf(
            series.points[index as number].interior
          );
          legendStrokeColor = series.points[index as number].interior;
        } else {
          legendIndex = index;
          legendStrokeColor = document
            .getElementById(chartId + '_chart_legend_shape_' + index)
            .getAttribute('fill');
        }
      } else {
      ...
      if (series.visible) {
        ...
        if (
          !isNullOrUndefined(getElement(chartId + 'SymbolGroup' + series.index))
        ) {
          this.checkSelectionElements(
  ...
  public checkSelectionElements(
    ...
    series: number = 0,
    legendStrokeColor: string = '#D3D3D3'
  ): void {
    let children: HTMLCollection | Element[] = <Element[]>(
      (this.isSeriesMode
    ...
    for (let i: number = 0; i < children.length; i++) {
      ...
      parentClassName =
        (<Element>children[i as number].parentNode).getAttribute('class') || '';
      if (
        this.chart.selectionMode !== 'None' &&
        (this.chart.highlightMode !== 'None' ||
          this.chart.legendSettings.enableHighlight)
        ...
        className =
          elementClassName.indexOf('selection') > 0 ||
          elementClassName.indexOf('highlight') > 0
            ? elementClassName
            : className;
        className =
          parentClassName.indexOf('selection') > 0 ||
          parentClassName.indexOf('highlight') > 0
            ? parentClassName
            : className;
      }
      if (
        elementClassName.indexOf(className) === -1 &&
        parentClassName.indexOf(className) === -1 &&
        visibility
      ) {
        ...
        this.removeSvgClass(
          <Element>children[i as number].parentNode,
          this.unselected
        );
        if (
          children[i as number].id !== '' &&
          elementClassName.indexOf(this.unselected) !== -1 &&
          parentClassName.indexOf(className) === -1
        ) {
          this.highlightAnimation(
            children[i as number] as HTMLElement,
            this.chart.series.length === 1
              ? 0
              : this.indexFinder(children[i as number].id).series,
            700,
            0.3
          );
        }
      ...
      if (
        children[i as number].id.indexOf('Trackball') > 0 &&
        selectElement.classList[0] === className
      ) {
        this.removeSvgClass(children[i as number], this.unselected);
        this.removeSvgClass(
          <Element>children[i as number].parentNode,
    ...
    if (element.id.indexOf('Symbol') > -1) {
      if (
        element.querySelectorAll('.' + className)[0] &&
        element.querySelectorAll('.' + className)[0].getAttribute('class') ===
          className
      ) {
        const symbolEle: Element = getElement(
          this.control.element.id +
            '_Series_' +
            element.id[element.id.length - 1]
        );
        const seriesClassName: string =
          symbolEle && symbolEle.hasAttribute('class')
            ? symbolEle.getAttribute('class')
            : '';
    ...
    if (
      (this.control as Chart).legendModule &&
      (this.control as Chart).legendSettings.visible
    ) {
      legendShape = getElement(
        this.control.element.id + '_chart_legend_shape_' + series
      );
      if (legendShape) {
        if (legendShape.hasAttribute('class')) {
          ...
          if (
            !isNullOrUndefined(
              this.chart.highlightColor && this.chart.highlightColor !== ''
            ) &&
            !this.chart.legendSettings.enableHighlight
          ) {
        ...
        parentClassName =
          (<Element>selectElement.parentNode).getAttribute('class') || '';
        if (
          elementClassName.indexOf(className) === -1 &&
          parentClassName.indexOf(className) === -1 &&
          visibility
          ...
          if (
            ...
            legendShape.setAttribute(
              'stroke',
              (this.control as Chart).visibleSeries[series as number].interior
            );
            if (this.chart.highlightPattern === 'None') {
              legendShape.setAttribute(
                'fill',
          ...
          if (
            ...
            elementClassName.trim() === 'EJ2-Trackball'
          ) {
            this.removeSvgClass(legendShape, className);
          } else {
            ...
            if (
              ...
              this.chart.highlightColor !== '' &&
              this.chart.highlightColor !== 'transparent' &&
              !isNullOrUndefined(this.chart.highlightColor)
            ) {
        ...
        if (this.rangeColorMappingEnabled()) {
          for (
            let i: number = 0;
            i < this.chart.rangeColorSettings.length;
            i++
          ) {
            legendItemsId = document.getElementById(
              this.chart.element.id + '_chart_legend_shape_' + i
            );
  ...
  public applyStyles(elements: Element[]): void {
    for (const element of elements) {
      if (element) {
        ...
        if (
          this.chart.series[0].pointColorMapping === 'fill' ||
          this.rangeColorMappingEnabled()
        ) {
          ...
          const index: number =
            className.indexOf('highlight') > -1
              ? parseInt(
                  className.split(
                    this.chart.element.id + '_ej2_chart_highlight_series_'
                  )[1],
                  10
                )
              : parseInt(
                  className.split(
                    this.chart.element.id + '_ej2_chart_selection_series_'
                  )[1],
                  10
                );
          const patternName: string =
            this.styleId.indexOf('highlight') > 0
              ? this.chart.highlightPattern
              : this.chart.selectionPattern;
          let pattern: Element;
          if (
            className.indexOf('highlight') > -1 ||
          ...
          if (
            element.id.indexOf('legend') === -1 &&
            element.id.indexOf('Group') === -1 &&
            pattern != null
          ) {
            for (let i: number = 1; i < pattern.children.length; i++) {
              pattern.children[i as number].setAttribute(
                'fill',
        ...
        if (element.id.indexOf('Group') > 0) {
          ...
          for (let i: number = 0; i < element.children.length; i++) {
            if (element.children[i as number].nodeName !== 'defs') {
              this.stopElementAnimation(
                element.children[i as number] as HTMLElement,
                seriesIndex.series
              );
            }
          }
        ...
        if (
          ...
          if (element.id.indexOf('Group') > 0) {
            for (let i: number = 0; i < element.children.length; i++) {
              element.children[i as number].setAttribute(
                'fill',
                this.chart.highlightColor
              );
            }
  ...
  public getSelectionClass(id: string): string {
    return this.generateStyle(
      (this.control as Chart).visibleSeries[this.indexFinder(id).series]
    );
  }
  /**
          ...
          if (element.id.indexOf('Group') > 0) {
            for (let i: number = 0; i < element.children.length; i++) {
              element.children[i as number].setAttribute(
                'fill',
                (this.control as Chart).visibleSeries[
                  this.indexFinder(element.id).series
                ].interior
              );
            }
          } else {
  ...
  public toEquals(
    first: Index,
    second: Index,
    checkSeriesOnly: boolean
  ): boolean {
    return (
      (first.series === second.series ||
        (this.currentMode === 'Cluster' && !checkSeriesOnly)) &&
  ...
  public redrawSelection(
    ...
    let selectedDataIndexes: Indexes[] = <Indexes[]>(
      extend([], this.selectedDataIndexes, null, true)
    );
    const highlightDataIndexes: Indexes[] = <Indexes[]>(
      extend([], this.highlightDataIndexes, null, true)
    );
    if (
      this.styleId.indexOf('highlight') > 0 &&
      highlightDataIndexes.length > 0
    ) {
  ...
  public legendSelection(
    ...
    const isPreSelected: boolean = this.isAlreadySelected(
      targetElement,
      eventType
    );
    if (isPreSelected) {
      let seriesStyle: string = this.generateStyle(
        chart.visibleSeries[series as number]
      );
      let selectedElements: NodeListOf<HTMLElement> = <NodeListOf<HTMLElement>>(
        document.querySelectorAll('.' + seriesStyle)
      );
      this.isSeriesMode = this.currentMode === 'Series';
      ...
      if (selectedElements.length > 0) {
        ...
        for (const element of chart.visibleSeries) {
          if (element.index !== series && !chart.isMultiSelect) {
            seriesStyle = this.generateStyle(
              chart.visibleSeries[element.index]
            );
        ...
        if (this.rangeColorMappingEnabled()) {
          for (
            let i: number = 0,
              a: HTMLCollection = chart.visibleSeries[0].seriesElement.children;
            i < a.length;
            i++
          ) {
            const point: Element = a[i as number];
            if (
              targetElement.getAttribute('fill') === point.getAttribute('fill')
            ) {
          ...
          if (chart.visibleSeries[series as number].visible) {
            seriesElements = this.getSeriesElements(
              chart.visibleSeries[series as number]
            );
  ...
  public rangeColorMappingEnabled(): boolean {
    if (
      this.chart.rangeColorSettings &&
      this.chart.rangeColorSettings.length > 0 &&
      this.chart.visibleSeries.length === 1 &&
      this.chart.rangeColorSettings[0].colors.length > 0 &&
      (this.chart.series[0].type === 'Column' ||
        this.chart.series[0].type === 'Bar' ||
        this.chart.series[0].type === 'Scatter' ||
        this.chart.series[0].type === 'Bubble')
    ) {
      return true;
    } else {
      return false;
   ...
   * @returns {void}
   */
  public calculateDragSelectedElements(
    chart: Chart,
    dragRect: Rect,
    isClose?: boolean
  ): void {
    ...
    const isDragResize: boolean =
      chart.allowMultiSelection && (this.rectGrabbing || this.resizing);
    this.rectPoints = this.dragRectArray[
      isDragResize ? this.targetIndex : this.count
    ] = new Rect(dragRect.x, dragRect.y, dragRect.width, dragRect.height);
    if (dragRect.width && dragRect.height && !isClose) {
      const rt: Rect = new Rect(
        dragRect.x,
        dragRect.y,
        dragRect.width,
        dragRect.height
      ...
      if (series.visible) {
        ...
        for (let j: number = 0; j < points.length; j++) {
          ...
          if (series.type === 'BoxAndWhisker') {
            isCurrentPoint = points[j as number].regions.some(
              (region: Rect): boolean => {
                return withInBounds(
                  region.x + xAxisOffset,
                  region.y + yAxisOffset,
                  rect
                );
              }
            );
            ...
            if (chart.selectionMode === 'Lasso') {
              ...
              isCurrentPoint = chart.allowMultiSelection
                ...
                : points[j as number].symbolLocations.some(
                    (location: ChartLocation) => {
                      return (
                        location &&
                        withInBounds(
                          location.x + xAxisOffset,
                          location.y + yAxisOffset,
                          rect
                        )
                      );
                    }
          ...
          if (isCurrentPoint && series.category !== 'Indicator') {
            ...
            this.selection(
              ...
              this.findElements(
                ...
                index,
                '',
                !series.isRectSeries ? series.marker.visible : false
              )
            );
          ...
          if (
            ...
            selectedPointValues.push({
              x: selectedPointX,
              y: points[j as number].regions[0].y,
            });
          }
        }
  ...
  private isPointSelect(
    ...
    for (const rect of rectCollection) {
      if (
        rect &&
        location &&
        withInBounds(location.x + xAxisOffset, location.y + yAxisOffset, rect)
      ) {
        return true;
      }
    }
  ...
  public drawDraggingRect(chart: Chart, dragRect: Rect): void {
    ...
    const isDragMode: boolean =
      chart.selectionMode.indexOf('Drag') > -1 ||
      chart.selectionMode === 'Lasso';
    if (chart.allowMultiSelection && isDragMode) {
      ...
      if (this.rectGrabbing || this.resizing) {
        const rectElement: Element = getElement(
          this.draggedRect + this.targetIndex
        );
        ...
        const svgElement: HTMLElement = document.getElementById(
          chart.element.id + '_series_svg'
        );
      ...
      if (!(chart.selectionMode === 'Lasso')) {
        element = chart.svgRenderer.drawRectangle(
          new RectOption(
            this.draggedRect + this.count,
            rectFill,
            { color: rectStroke, width: 1 },
            1,
            dragRect,
            0,
            0,
            '',
            chart.theme.indexOf('Fluent2') < 1 ? '3' : ''
          )
        );
      ...
      let element: Element = isLasso
        ? getElement(this.lassoPath)
        : getElement(this.draggedRect);
      if (this.closeIcon) {
      ...
      if (element) {
        ...
        const dragGroup: Element = chart.svgRenderer.createGroup({
  ...
  private createCloseButton(x: number, y: number): void {
    ...
    const closeIcon: Element = this.chart.svgRenderer.createGroup({
      id:
        this.closeIconId +
        (isMultiDrag ? (isDrag ? this.targetIndex : this.count) : ''),
      style: 'cursor:pointer; visibility: visible;',
    });
    closeIcon.appendChild(
      this.chart.svgRenderer.drawCircle(
        new CircleOption(
          this.closeIconId +
            '_circle' +
            (isMultiDrag ? (isDrag ? this.targetIndex : this.count) : ''),
          '#FFFFFF',
          { color: circleStroke, width: 1 },
          1,
          x,
          y,
          10
    ...
    const pathElement: Element = getElement(
      this.draggedRectGroup +
        (isMultiDrag ? (isDrag ? this.targetIndex : this.count) : '')
    );
    if (pathElement) {
  ...
  public removeDraggedElements(
    ...
    if (
      targetElement.id &&
      targetElement.id.indexOf(this.closeIconId) > -1 &&
      eventType.indexOf('move') === -1
    ) {
      ...
      if (chart.allowMultiSelection) {
        ...
        if (
          multiRectGroupElement &&
          multiRectGroupElement.childElementCount === 0
        ) {
        ...
        if (this.currentMode === 'Lasso') {
          if (this.multiDataIndexes[index as number] != null) {
            for (
              let i: number = 0;
              i < this.multiDataIndexes[index as number].length;
              i++
            ) {
              this.multiDataIndexes[index as number][i as number].isSelect =
                false;
            }
          }
          ...
          for (let j: number = 0; j < this.multiDataIndexes.length; j++) {
            if (this.multiDataIndexes[j as number] != null) {
              ...
              for (
                let k: number = 0;
                k < this.multiDataIndexes[j as number].length;
                k++
              ) {
          ...
          for (let i: number = 0; i < this.filterArray.length; i++) {
            if (this.filterArray[i as number]) {
              isSelectedvalues = false;
              this.calculateDragSelectedElements(
                chart,
                this.filterArray[i as number],
                true
              );
            }
          }
  ...
  public resizingSelectionRect(
    ...
    let rect: Rect;
    if (
      (chart.allowMultiSelection && target.id.indexOf('_ej2_drag_rect') > -1) ||
    ...
    if (!chart.allowMultiSelection) {
      rect = new Rect(
        this.rectPoints.x,
        this.rectPoints.y,
        this.rectPoints.width,
        this.rectPoints.height
      );
    }
    if (rect) {
      const resize: boolean = this.findResizeMode(
        chart.svgObject,
        rect,
        location
      );
  ...
  private findResizeMode(
    ...
    if (!this.resizing) {
      const resizeEdges: Rect[] = [
        new Rect(rect.x, rect.y, rect.width - 5, 5), // top
      ...
      for (let i: number = 0; i < resizeEdges.length; i++) {
        if (withInBounds(location.x, location.y, resizeEdges[i as number])) {
          cursorStyle =
            i === 4 ? cursorStyle : i % 2 === 0 ? 'ns-resize' : 'ew-resize';
          resize = true;
          this.resizeMode = i;
          break;
        }
    ...
    if (this.currentMode !== 'Lasso') {
      this.changeCursorStyle(
        resize,
        getElement(
          this.chart.allowMultiSelection
            ? this.draggedRect + this.targetIndex
            : this.draggedRect
        ),
        cursorStyle
  ...
  private changeCursorStyle(
    ...
    cursorStyle = isResize
      ? cursorStyle
      : this.control.svgObject === rectelement
      ? 'auto'
      : 'move';
    if (rectelement) {
  ...
  public completeSelection(target: HTMLElement, eventType: string): void {
    ...
    if (
      ...
      this.dragRect.width > 5 &&
      this.dragRect.height > 5
    ) {
      this.calculateDragSelectedElements(chart, this.dragRect);
    } else if (
      !chart.allowMultiSelection &&
      this.rectGrabbing &&
      this.rectPoints.width &&
      this.rectPoints.height
    ) {
      ...
      this.rectGrabbing &&
      this.dragRectArray[this.targetIndex].width &&
      this.dragRectArray[this.targetIndex].height
    ) {
      this.draggedRectMoved(chart, this.dragRect);
    }
    if (chart.selectionMode === 'Lasso' && this.dragging && this.path) {
      if ((this.path as string).indexOf('L') !== -1) {
        if (!chart.allowMultiSelection) {
          ...
          getElement(this.lassoPath + this.count).setAttribute(
            'd',
            this.path + 'Z'
          );
          this.pointChecking(
            getElement(this.lassoPath + this.count) as SVGPathElement
          );
        }
  ...
  public dragStart(
    ...
    this.currentMode = chart.selectionMode;
    this.dragging =
      (mode.indexOf('Drag') > -1 || mode === 'Lasso') &&
      (chart.isDoubleTap || !chart.isTouch) &&
    ...
    if (this.dragging) {
      ...
      if (
        chart.mouseDownX < seriesClipRect.x ||
        chart.mouseDownX > seriesClipRect.x + seriesClipRect.width ||
        chart.mouseDownY < seriesClipRect.y ||
        chart.mouseDownY > seriesClipRect.y + seriesClipRect.height
      ) {
        this.dragging = false;
      }
    }
    ...
    if (!(mode === 'Lasso')) {
      if (this.rectPoints && !chart.allowMultiSelection) {
        this.dragRect = new Rect(chart.mouseDownX, chart.mouseDownY, 0, 0);
        this.resizingSelectionRect(
          chart,
          new ChartLocation(mouseDownX, mouseDownY),
          true
        );
        this.rectGrabbing = withInBounds(
          mouseDownX,
          mouseDownY,
          this.rectPoints
        );
      ...
      if (chart.allowMultiSelection) {
        ...
        if (this.dragRectArray.length && this.isDragRect(target.id)) {
          ...
          this.rectGrabbing = withInBounds(
            mouseDownX,
            mouseDownY,
            this.dragRectArray[index as number]
          );
        }
      }
    }
    ...
    if (
      ...
      if (!isNullOrUndefined(target)) {
        ...
        if (
          ...
          (target.getAttribute('class').indexOf('highlight') > -1 ||
        ...
        if (
          this.chart.highlightModule.highlightDataIndexes &&
          this.chart.highlightModule.highlightDataIndexes.length > 0 &&
          target.id.indexOf('_chart_legend_') === -1 &&
   ...
   * @returns {void}
   */
  public selectionAndDrag(
    chart: Chart,
    target: Element,
    eventType: string
  ): void {
    const insideMoving: boolean = withInBounds(
      chart.mouseX,
      chart.mouseY,
      chart.chartAxisLayoutPanel.seriesClipRect
    );
    if (insideMoving && !this.chart.enableCanvas) {
      if (this.rectGrabbing && !this.resizing) {
        ...
        if (chart.selectionMode === 'Lasso') {
          ...
          this.dragRect = this.getDragRect(
            chart,
            chart.chartAxisLayoutPanel.seriesClipRect
          );
          this.drawDraggingRect(chart, this.dragRect);
      ...
      if (this.rectPoints && !chart.allowMultiSelection) {
        this.resizingSelectionRect(
          chart,
          new ChartLocation(chart.mouseX, chart.mouseY),
          null,
          target
        );
      } else if (
        (chart.allowMultiSelection && !this.dragging) ||
        this.resizing
      ) {
        this.resizingSelectionRect(
          chart,
  ...
  public removeLegendHighlightStyles(): void {
    ...
    for (let i: number = 0; i < this.chart.visibleSeries.length; i++) {
      elementCollection = document.getElementsByClassName(
        this.generateStyle(this.chart.visibleSeries[i as number])
      );
      if (this.selectedDataIndexes.length === 0) {
        elementCollection = document.getElementsByClassName(
          this.generateStyle(this.chart.visibleSeries[i as number])
        ...
        while (elementCollection.length > 0) {
          ...
          if (element) {
            ...
            if (
              element.id.indexOf('Group') > 0 &&
              !this.chart.visibleSeries[i as number].isRectSeries
            ) {
              const seriesIndex: Index = this.indexFinder(element.id);
              for (let j: number = 0; j < element.children.length; j++) {
                if (element.children[j as number].nodeName !== 'defs') {
                  this.highlightAnimation(
                    element.children[j as number] as HTMLElement,
                    seriesIndex.series,
                    700,
                    0.3,
                    true
  ...
  private highlightAnimation(
    ...
    const startWidth: number =
      parseFloat(this.chart.visibleSeries[index as number].width.toString()) +
      1;
    if (strokeWidth) {
      if (
        element.id.indexOf('border') !== -1 &&
        this.chart.visibleSeries[index as number].border.width
      ) {
        endWidth = parseFloat(
          this.chart.visibleSeries[index as number].border.width.toString()
        );
      } else if (
        element.id.indexOf('Symbol') !== -1 &&
        this.chart.visibleSeries[index as number].marker.border.width
      ) {
        endWidth = parseFloat(
          this.chart.visibleSeries[
            index as number
          ].marker.border.width.toString()
        );
      } else {
        endWidth = parseFloat(
          this.chart.visibleSeries[index as number].width.toString()
        );
      }
    } else {
      if (element.id.indexOf('border') !== -1) {
        ...
        endOpacity = parseFloat(
          this.chart.visibleSeries[index as number].marker.opacity.toString()
        );
      } else {
        endOpacity = parseFloat(
          this.chart.visibleSeries[index as number].opacity.toString()
        );
      }
      if (
        isNullOrUndefined(this.chart.selectionModule) &&
        this.chart.selectionMode === 'None' &&
        this.chart.highlightColor !== ''
      ) {
        startOpacity = 1;
  ...
  private pointChecking(path: SVGPathElement): void {
    ...
    const offsetX: number =
      chart.chartAxisLayoutPanel.seriesClipRect.x + Math.max(svgRect.left, 0);
    const offsetY: number =
      chart.chartAxisLayoutPanel.seriesClipRect.y + Math.max(svgRect.top, 0);
