...
export class DataEditing {
  ...
  public pointMouseDown(): void {
    ...
    if (
      pointData.point &&
      (data.insideRegion || !pointData.series.isRectSeries)
    ) {
      ...
      if (series.dragSettings.enable && !isZooming) {
        chart.trigger(dragStart, {
          ...
          oldValue:
            chart.visibleSeries[this.seriesIndex].yData[this.pointIndex],
          newValue:
            chart.visibleSeries[this.seriesIndex].points[this.pointIndex]
              .yValue,
        });
  ...
  public pointMouseMove(event: PointerEvent | TouchEvent): void {
    ...
    const pointData: PointData = data.getData();
    if (
      pointData.series.dragSettings.enable &&
      pointData.point &&
      (data.insideRegion || !pointData.series.isRectSeries)
    ) {
    ...
    if (chart.isPointMouseDown) {
      ...
      if (series.type.indexOf('Spline') > -1) {
        chart[firstToLowerCase(series.type) + 'SeriesModule'].findSplinePoint(
          <Series>series
        );
      }
  ...
  private pointDragging(si: number, pi: number): void {
    ...
    if (series.type === 'Bar') {
      y = chart.isTransposed
        ? axis.y + axis.height - chart.mouseY
        : chart.mouseX - axis.x;
    ...
    const minRange: number =
      yAxis.minimum !== null
        ? yAxis.visibleRange.min + extra
        : isNullOrUndefined(pointDrag.minY)
        ? yValue
        : pointDrag.minY;
    const maxRange: number =
      yAxis.maximum !== null
        ? yAxis.visibleRange.max + extra
        : isNullOrUndefined(pointDrag.maxY)
        ? yValue
        : pointDrag.maxY;
    if (maxRange >= yValue && minRange <= yValue) {
      series.points[pi as number].yValue =
        series.points[pi as number].y =
        chart.dragY =
          yAxis.valueType === 'Logarithmic'
            ? Math.pow(yAxis.logBase, yValue)
            : parseFloat(yValue.toFixed(2));
      ...
      chart.trigger(drag, {
        ...
        pointIndex: pi,
        series: series,
        point: series.points[pi as number],
        oldValue: chart.visibleSeries[this.seriesIndex].yData[this.pointIndex],
        newValue: series.points[pi as number].yValue,
      });
