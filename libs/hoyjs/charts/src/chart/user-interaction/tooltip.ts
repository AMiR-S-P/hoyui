...
import { extend, Browser, remove } from '@hoyui/base';
import { PointData, ChartLocation } from '../../common/utils/helper';
import { getElement, measureText, Rect } from '@hoyui/svg-base';
import {
  valueToCoefficient,
  removeElement,
...
export class Tooltip extends BaseTooltip {
  ...
  private mouseUpHandler(): void {
    ...
    if (
      ...
      !this.findData(data, this.previousPoints[0] as PointData) &&
      chart.tooltip.fadeOutMode === 'Click'
    ) {
   ...
   */
  public tooltip(): void {
    const elementId: string = this.chart.enableCanvas
      ? this.element.id + '_tooltip_group'
    ...
    const isStockSvg: boolean =
      this.chart.stockChart &&
      svgElement &&
      svgElement.firstChild.childNodes.length > 1;
    ...
    if (this.chart.enableCanvas && tooltipDiv) {
      document
        .getElementById(this.chart.element.id + '_Secondary_Element')
  ...
  private findHeader(data: PointData): string {
    ...
    this.header = this.parseTemplate(
      data.point,
      data.series,
      this.header,
      data.series.xAxis,
  ...
  private findShapes(): ChartShape[] {
    ...
    for (const data of this.currentPoints) {
      marker.push(
        (<PointData>data).point.marker.shape ||
          (<Series>data.series).marker.shape ||
          'Circle'
      );
    }
    return marker;
  ...
  private renderSeriesTooltip(
    ...
    if (this.findData(data, this.previousPoints[0] as PointData)) {
      if (
        !(chart.dataEditingModule && chart.dataEditingModule.isPointDragging) &&
      ...
      if (
        !data.point &&
        this.isRemove &&
        chart.tooltip.fadeOutMode === 'Move'
      ) {
        this.removeTooltip(this.chart.tooltip.fadeOutDuration);
        this.isRemove = false;
      } else {
        const commonXvalues: number[] = this.mergeXvalues(
  ...
  private triggerTooltipRender(
    ...
    const argsData: ITooltipRenderEventArgs = {
      ...
      series: this.chart.isBlazor ? ({} as Series) : point.series,
    ...
    const chartTooltipSuccess: Function = (
      argsData: ITooltipRenderEventArgs
    ) => {
      if (!argsData.cancel) {
        ...
        location.x =
          tooltip.location.x !== null ? tooltip.location.x : location.x;
        location.y =
          tooltip.location.y !== null ? tooltip.location.y : location.y;
        location = location.x === null && location.y === null ? null : location;
        this.createTooltip(
          this.chart,
          isFirst,
          location,
          point.series.clipRect,
          point.point,
          this.findShapes(),
          ...
          this.chart.crosshair.enable,
          null,
          this.getTemplateText(point),
  ...
  private findMarkerHeight(pointData: PointData): number {
    ...
    markerHeight =
      (series.marker.visible ||
        (this.chart.tooltip.shared &&
          (!series.isRectSeries || series.marker.visible)) ||
        series.type === 'Scatter' ||
  ...
  private findData(data: PointData, previous: PointData): boolean {
    return (
      ...
      (!previous ||
        ...
        (previous &&
          previous.lierIndex > 3 &&
          previous.lierIndex !== this.lierIndex) ||
        previous.point === data.point)
    );
  }
  ...
  private getRangeArea(
    ...
    if (data.point.regions[0]) {
      ...
      if (data.series.type === 'RangeStepArea') {
        location.y =
          data.point.regions[0].y +
          data.point.regions[0].height / 2 +
          data.point.regions[0].width;
      }
    }
    return location;
  }
  ...
  private getWaterfallRegion(
    ...
    if (!this.inverted) {
      location.y =
        data.point.y < 0
          ? location.y - data.point.regions[0].height
          : location.y;
    } else {
      location.x =
        data.point.y < 0
  ...
  private getTooltipText(pointData: PointData): string {
    ...
    return this.parseTemplate(
      pointData.point,
      series,
      this.getFormat(this.chart, series),
      series.xAxis,
      series.yAxis
    );
  }
  ...
  private getTemplateText(data: any): Points | Points[] {
    if (this.template && this.chart.tooltip.shared) {
      ...
      for (let i: number = 0; i < data.length; i++) {
        ...
        point[i as number].x = this.formatPointValue(
          data[i as number].point,
          data[i as number].series.xAxis,
          'x',
          true,
          false
        );
        if (data[i as number].series.seriesType === 'XY') {
          point[i as number].y = this.formatPointValue(
            data[i as number].point,
            data[i as number].series.yAxis,
            'y',
            false,
            true
          );
        } else {
          point[i as number].low = this.formatPointValue(
            data[i as number].point,
            data[i as number].series.yAxis,
            'low',
            false,
          ...
          point[i as number].high = this.formatPointValue(
            data[i as number].point,
            data[i as number].series.yAxis,
            'high',
            false,
      ...
      if (data.series.seriesType === 'XY') {
        ...
        point.low = this.formatPointValue(
          data.point,
          data.series.yAxis,
          'low',
          false,
        ...
        point.high = this.formatPointValue(
          data.point,
          data.series.yAxis,
          'high',
          false,
  ...
  private findMouseValue(data: PointData, chart: Chart): void {
    if (!chart.requireInvertedAxis) {
      if (chart.chartAreaType === 'PolarRadar') {
        this.valueX =
          valueToPolarCoefficient(data.point.xValue, data.series.xAxis) *
            data.series.xAxis.rect.width +
          data.series.xAxis.rect.x;
      } else {
        this.valueX =
          data.series.category === 'TrendLine' && chart.tooltip.shared
            ? this.valueX
            : valueToCoefficient(data.point.xValue, data.series.xAxis) *
                data.series.xAxis.rect.width +
              data.series.xAxis.rect.x;
      }
      ...
      this.valueY =
        (1 - valueToCoefficient(data.point.xValue, data.series.xAxis)) *
          data.series.xAxis.rect.height +
  ...
  private renderGroupedTooltip(
    ...
    const pointData: PointData =
      chart.chartAreaType === 'PolarRadar' ? this.getData() : null;
    this.stopAnimation();
    this.removeHighlight();
    this.currentPoints = [];
    ...
    if (isFirst) {
      if (!chart.stockChart) {
        if (tooltipDiv) {
          document
            .getElementById(this.element.id + '_Secondary_Element')
            .appendChild(tooltipDiv);
        }
      } else {
        if (tooltipDiv && !getElement(tooltipDiv.id)) {
          document
            .getElementById(chart.stockChart.element.id + '_Secondary_Element')
            .appendChild(tooltipDiv);
        }
      }
    }
    ...
    for (const series of chart.visibleSeries) {
      ...
      if (chart.chartAreaType === 'Cartesian' && series.visible) {
        data = this.getClosestX(
          chart,
          series,
          this.commonXValue(this.chart.visibleSeries)
        );
      } else if (
      ...
      if (data) {
        ...
        if (
          ...
          data.point.symbolLocations.length &&
          Math.abs(pointXValue - data.point.symbolLocations[0].x) <=
            closestXValue &&
          Math.abs(data.point.symbolLocations[0].y - pointYValue) <
            Math.abs(closetYValue - pointYValue)
        ) {
          closestXValue = Math.abs(
            pointXValue - data.point.symbolLocations[0].x
          );
    ...
    if (!chart.tooltip.showNearestPoint) {
      ...
      for (const data of dataCollection) {
        if (
          data.point.symbolLocations[0].x ===
            lastData.point.symbolLocations[0].x ||
  ...
  private triggerSharedTooltip(
    argument: ISharedTooltipRenderEventArgs,
    point: PointData,
    extraPoints: PointData[],
    chart: Chart,
    isFirst: boolean,
    ...
    if (chart.stockChart) {
      toolbarHeight = chart.stockChart.enablePeriodSelector
        ? chart.stockChart.toolbarHeight
        : 0;
      titleHeight =
        measureText(
          this.chart.stockChart.title,
          this.chart.stockChart.titleStyle,
          this.chart.themeStyle.tooltipLabelFont
        ).height + 10;
    }
    const sharedTooltip: Function = (
      argsData: ISharedTooltipRenderEventArgs
    ) => {
      if (!argsData.cancel) {
        if (point.series.type === 'BoxAndWhisker') {
          this.removeText();
        ...
        for (let i: number = 0; i < argsData.text.length; i++) {
          if (argsData.text[i as number]) {
            currentPoints.push(this.currentPoints[i as number] as PointData);
          }
        }
        ...
        if (typeof argsData.template != 'object') {
          ...
          if (argsData.template.length > currentPoints.length) {
            argsData.template = argsData.template.splice(
              argsData.template.length - 1
            );
          }
        } else {
          if (argsData.template.length > currentPoints.length) {
        ...
        this.createTooltip(
          ...
          this.currentPoints.length === 1
            ? this.currentPoints[0].series.clipRect
            : null,
          ...
          new Rect(
            borderWidth,
            chart.stockChart
              ? toolbarHeight + titleHeight + borderWidth
              : borderWidth,
            this.chart.availableSize.width - padding - borderWidth * 2,
            this.chart.availableSize.height - padding - borderWidth * 2
          ),
  ...
  private findSharedLocation(): ChartLocation {
    ...
    if (stockChart) {
      ...
      const toolbarHeight: number = stockChart.enablePeriodSelector
        ? stockChart.toolbarHeight
        : 0;
      const element: Element = document.getElementById(
        stockChart.element.id + '_ChartTitle'
      );
      const titleHeight: number =
        stockChart.title !== ''
          ? element.getBoundingClientRect().height + 10
          : 0;
      if (stockChart.tooltip.position === 'Nearest') {
        return new ChartLocation(
          this.valueX,
          this.valueY + toolbarHeight + titleHeight
        );
      }
      return new ChartLocation(
        this.chart.chartAxisLayoutPanel.seriesClipRect.x + 5,
        this.chart.chartAxisLayoutPanel.seriesClipRect.y +
          toolbarHeight +
          5 +
          titleHeight
      );
  ...
  private getBoxLocation(data: PointData): ChartLocation {
    const location: ChartLocation =
      this.lierIndex > 3
        ? data.point.outliers.length > 0
          ? data.point.symbolLocations[this.lierIndex - 4]
          : null
        : {
    ...
    for (const dataValue of Object.keys(point)) {
      ...
      format = format.replace(
        ...
        this.formatPointValue(
          point,
          val.source === '${point.x}' ? xAxis : yAxis,
          dataValue,
          val.source === '${point.x}',
  ...
  private formatPointValue(
    ...
    if (axis.valueType !== 'Category' && isXPoint) {
      ...
      textValue = customLabelFormat
        ? axis.labelFormat.replace(
            '{value}',
      ...
      value =
        dataValue === 'outliers'
          ? axis.format(point[dataValue as string][this.lierIndex - 4])
          : axis.format(+point[dataValue as string]);
      textValue = customLabelFormat
        ? axis.labelFormat.replace('{value}', value)
        : value;
    } else if (dataValue === 'size') {
      const format: Function = this.chart.intl.getNumberFormat({
        format: '',
        useGrouping: this.chart.useGroupingSeparator,
      });
      textValue =
        typeof point[dataValue as string] === 'number'
          ? format(point[dataValue as string])
          : point[dataValue as string];
    } else {
  ...
  private getFormat(chart: Chart, series: Series): string {
    if (series.tooltipFormat) {
      if (series.seriesType === 'XY' && series.category === 'Indicator') {
        return this.getIndicatorTooltipFormat(
          series,
          chart,
    ...
    switch (series.seriesType) {
      case 'XY':
        ...
        return (
          ...
          ' : ' +
          (series.type === 'Bubble'
            ? '<b>${point.y}</b>  Size : <b>${point.size}</b>'
            : '<b>${point.y}</b>')
        );
      case 'HighLow':
        return (
          format +
          '<br/>High : <b>${point.high}</b><br/>Low : <b>${point.low}</b>'
        );
      case 'HighLowOpenClose':
        return (
          format +
          ('<br/>High : <b>${point.high}</b><br/>Low : <b>${point.low}</b><br/>' +
