import { EventHandler, isNullOrUndefined } from '@hoyui/base';
...
export class Toolkit {
  ...
  public createPanButton(childElement: Element, parentElement: Element): void {
    ...
    const fillColor: string = this.chart.zoomModule.isPanning
      ? this.chart.themeStyle.toolkitSelectionColor
      : this.chart.themeStyle.toolkitFill;
    let direction: string =
      'M5,3h2.3L7.275,5.875h1.4L8.65,3H11L8,0L5,3z M3,11V8.7l2.875,0.025v-1.4L3,7.35V5L0,8L3,';
    direction +=
      '11z M11,13H8.7l0.025-2.875h-1.4L7.35,13H5l3,3L11,13z M13,5v2.3l-2.875-0.025v1.4L13,8.65V11l3-3L13,5z';
    //This is for setting low opacity to PAN Button
    this.elementOpacity =
      !this.chart.zoomModule.isZoomed && this.chart.zoomSettings.showToolbar
        ? '0.2'
        : '1';
    childElement.setAttribute(
      'opacity',
      this.chart.theme === 'Fluent2HighContrast' ? '1' : this.elementOpacity
    );
    ...
    childElement.appendChild(
      render.drawRectangle(
        new RectOption(
          ...
          this.chart.theme.indexOf('Fluent2') > -1 ||
  ...
  public createZoomButton(childElement: Element, parentElement: Element): void {
    ...
    const fillColor: string =
      this.chart.zoomModule.isPanning ||
      (!this.chart.zoomModule.isZoomed && this.chart.zoomSettings.showToolbar)
        ? this.chart.themeStyle.toolkitFill
        : this.chart.themeStyle.toolkitSelectionColor;
    this.elementOpacity =
      !this.chart.zoomModule.isPanning &&
      !this.chart.zoomModule.isZoomed &&
      this.chart.zoomSettings.showToolbar
        ? '0.2'
        : '1';
    const rectColor: string = this.chart.zoomModule.isPanning
      ? 'transparent'
      : this.chart.themeStyle.toolkitIconRectSelectionFill;
    let direction: string =
      'M0.001,14.629L1.372,16l4.571-4.571v-0.685l0.228-0.274c1.051,0.868,2.423,1.417,3.885,1.417c3.291,0,';
    direction +=
      '5.943-2.651,5.943-5.943S13.395,0,10.103,0S4.16,2.651,4.16,5.943c0,1.508,0.503,2.834,1.417,3.885l-0.274,0.228H4.571';
    direction =
      direction +
      'L0.001,14.629L0.001,14.629z M5.943,5.943c0-2.285,1.828-4.114,4.114-4.114s4.114,1.828,4.114,';
  ...
  public createZoomInButton(
    ...
    let direction: string =
      'M10.103,0C6.812,0,4.16,2.651,4.16,5.943c0,1.509,0.503,2.834,1.417,3.885l-0.274,0.229H4.571L0,';
    direction +=
      '14.628l0,0L1.372,16l4.571-4.572v-0.685l0.228-0.275c1.052,0.868,2.423,1.417,3.885,1.417c3.291,0,5.943-2.651,';
    direction +=
      '5.943-5.943C16,2.651,13.395,0,10.103,0z M10.058,10.058c-2.286,0-4.114-1.828-4.114-4.114c0-2.286,1.828-4.114,';
    ...
    const polygonDirection: string =
      '12.749,5.466 10.749,5.466 10.749,3.466 9.749,3.466 9.749,5.466 7.749,5.466 7.749,6.466';
    childElement.appendChild(
      render.drawRectangle(
        new RectOption(
    ...
    childElement.appendChild(
      render.drawPath(
        new PathOption(
          ...
          null,
          direction +
            '4.114-4.114c2.286,0,4.114,1.828,4.114,4.114C14.172,8.229,12.344,10.058,10.058,10.058z'
        )
      ) as HTMLElement
    );
    childElement.appendChild(
      render.drawPolygon(
        new PolygonOption(
          ...
          polygonDirection +
            ' 9.749,6.466 9.749,8.466 10.749,8.466 10.749,6.466 12.749,6.466',
          fillColor
        )
      ) as HTMLElement
    );
    ...
    this.elementOpacity =
      chart.zoomModule.isPanning ||
      (!chart.zoomModule.isZoomed &&
        !chart.zoomSettings.showToolbar &&
  ...
  public createZoomOutButton(
    ...
    const fillColor: string = this.chart.themeStyle.toolkitFill;
    let direction: string =
      'M0,14.622L1.378,16l4.533-4.533v-0.711l0.266-0.266c1.022,0.889,2.4,1.422,3.866,';
    direction +=
      '1.422c3.289,0,5.955-2.666,5.955-5.955S13.333,0,10.044,0S4.089,2.667,4.134,5.911c0,1.466,0.533,2.844,';
    direction +=
      ...
      render.drawPath(
        new PathOption(
          ...
          (chart.theme === 'Fluent2HighContrast' &&
            !chart.zoomModule.isZoomed) ||
          (chart.theme === 'Fluent2HighContrast' && chart.zoomModule.isPanning)
            ? '#3FF23F'
            : fillColor,
          null,
          null,
          ...
          null,
          direction +
            '4.133s-1.866,4.133-4.133,4.133S5.911,8.222,5.911,5.911z M12.567,6.466h-5v-1h5V6.466z'
        )
      ) as HTMLElement
    );
    ...
    childElement.setAttribute(
      'opacity',
      chart.theme === 'Fluent2HighContrast' ? '1' : this.elementOpacity
    );
    parentElement.appendChild(childElement);
  ...
  public createResetButton(
    ...
    let size: Size;
    let direction: string =
      'M12.364,8h-2.182l2.909,3.25L16,8h-2.182c0-3.575-2.618-6.5-5.818-6.5c-1.128,0-2.218,0.366-3.091,';
    direction +=
      '1.016l1.055,1.178C6.581,3.328,7.272,3.125,8,3.125C10.4,3.125,12.363,5.319,12.364,8L12.364,8z M11.091,';
    direction +=
    ...
    this.elementOpacity =
      !chart.zoomModule.isZoomed && chart.zoomSettings.showToolbar
        ? '0.2'
        : '1';
    childElement.setAttribute(
      'opacity',
      ...
      childElement.appendChild(
        render.drawPath(
          new PathOption(
            ...
            null,
            direction +
              '3.575,2.618,6.5,5.818,6.5C9.128,14.5,10.219,14.134,11.091,13.484L11.091,13.484z'
          )
        ) as HTMLElement
      );
    } else {
  ...
  private showTooltip(event: MouseEvent): void {
    const text: string = (<HTMLElement>event.currentTarget).id.split(
      '_Zooming_'
    )[1];
    const left: number =
      event.pageX -
      (measureText(
        ...
        5);
    const rect: Element = getElement(
      (<HTMLElement>event.currentTarget).id + '_1'
    );
    const icon2: Element = getElement(
      (<HTMLElement>event.currentTarget).id + '_2'
    );
    const icon3: Element = getElement(
      (<HTMLElement>event.currentTarget).id + '_3'
    );
    ...
    if (rect) {
      ...
      rect.setAttribute(
        ...
        this.chart.theme === 'Fluent2HighContrast' &&
          (
            (<HTMLElement>event.currentTarget).childNodes[1] as HTMLElement
          ).getAttribute('fill') === '#3FF23F'
          ? 'transparent'
    ...
    if (!this.chart.isTouch) {
      createTooltip(
        'EJ2_Chart_ZoomTip',
        this.chart.getLocalizedLabel(text),
        event.pageY + 10,
        left,
        '10px'
      );
  ...
  public removeTooltip(): void {
    ...
    const icon2: Element = this.hoveredID
      ? getElement(this.hoveredID.replace('_1', '_2'))
      : null;
    const icon3: Element = this.hoveredID
      ? getElement(this.hoveredID.replace('_1', '_3'))
      : null;
    if (icon2) {
      const iconColor: string = this.chart.zoomModule.isPanning
        ? this.hoveredID.indexOf('_Pan_') > -1
          ? this.chart.themeStyle.toolkitSelectionColor
          : this.elementOpacity === '0.2' &&
            this.chart.theme === 'Fluent2HighContrast' &&
            getElement(this.hoveredID).nextElementSibling.getAttribute(
              'fill'
            ) === '#3FF23F'
          ? '#3FF23F'
          : this.chart.themeStyle.toolkitFill
        ...
        ? this.chart.themeStyle.toolkitSelectionColor
        : this.chart.theme === 'Fluent2HighContrast' &&
          getElement(this.hoveredID).nextElementSibling.getAttribute('fill') ===
            '#3FF23F'
        ? '#3FF23F'
        : this.chart.themeStyle.toolkitFill;
    ...
    if (icon3) {
      ...
      const iconColor: string =
        this.chart.zoomModule.isPanning ||
        (!this.chart.isZoomed && this.chart.zoomSettings.showToolbar)
          ? this.chart.theme === 'Fluent2HighContrast' &&
            getElement(this.hoveredID).nextElementSibling.getAttribute(
              'fill'
            ) === '#3FF23F' &&
  ...
  private zoomIn(): boolean {
    this.zoomInOutCalculation(
      1,
      this.chart,
      this.chart.axisCollections,
      this.chart.zoomSettings.mode
    );
    return false;
  }
  ...
  private zoom(): boolean {
    ...
    this.applySelection(
      this.zoomElements.childNodes,
      this.chart.themeStyle.toolkitSelectionColor
    );
    if (this.chart.theme === 'Fluent2HighContrast') {
      this.applySelection(
        this.zoomInElements.childNodes,
        this.chart.themeStyle.toolkitFill
      );
      this.applySelection(
        this.zoomOutElements.childNodes,
        this.chart.themeStyle.toolkitFill
      );
    }
    ...
    getElement(this.selectedID).setAttribute(
      'fill',
      this.chart.themeStyle.toolkitIconRectSelectionFill
    );
    return false;
  ...
  public pan(): boolean {
    ...
    this.elementOpacity = '0.2';
    element = this.zoomInElements
      ? this.zoomInElements.setAttribute('opacity', this.elementOpacity)
      : null;
    element = this.zoomOutElements
      ? this.zoomOutElements.setAttribute('opacity', this.elementOpacity)
      : null;
    if (this.chart.theme === 'Fluent2HighContrast') {
      getElement(this.chart.element.id + '_Zooming_ZoomOut').setAttribute(
        'opacity',
        '1'
      );
      getElement(this.chart.element.id + '_Zooming_ZoomIn').setAttribute(
        'opacity',
        '1'
      );
      getElement(this.chart.element.id + '_Zooming_ZoomOut_2').setAttribute(
        'fill',
        '#3FF23F'
      );
      getElement(this.chart.element.id + '_Zooming_ZoomIn_2').setAttribute(
        'fill',
        '#3FF23F'
      );
      getElement(this.chart.element.id + '_Zooming_ZoomIn_3').setAttribute(
        'fill',
        '#3FF23F'
      );
    }
    element = this.panElements
      ? this.applySelection(
          this.panElements.childNodes,
          this.chart.themeStyle.toolkitSelectionColor
        )
      : null;
    element = this.zoomElements
    ...
    if (!chart.zoomModule.isPanning && this.elementOpacity !== '0.2') {
      ...
      for (const axis of axes as Axis[]) {
        ...
        if (
          ...
          cumulative = Math.max(
            Math.max(1 / minMax(axis.zoomFactor, 0, 1), 1) + 0.25 * scale,
            1
          );
          ...
          zoomPosition =
            cumulative === 1
              ? 0
              : axis.zoomPosition + (axis.zoomFactor - zoomFactor) * 0.5;
          if (
