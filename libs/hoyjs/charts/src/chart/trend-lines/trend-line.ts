...
export class Trendlines {
  ...
  public initSeriesCollection(trendline: Trendline, chart: Chart): void {
    const trendLineSeries: Series = new Series(
      trendline,
      'targetSeries',
      {},
      true
  ...
  private getDataPoint(
    ...
    trendPoint.x =
      series.xAxis.valueType === 'DateTime' ? new Date(Number(x)) : x;
    trendPoint.y = y;
  ...
  private findSlopeIntercept(
    ...
    while (index < points.length) {
      ...
      if (isNaN(yValues[index as number])) {
        yValues[index as number] =
          (yValues[index - 1] + yValues[index + 1]) / 2;
      }
    ...
    if (trendline.intercept && (type === 'Linear' || type === 'Exponential')) {
      ...
      switch (type) {
        ...
        case 'Exponential':
          slope =
            (xyAvg - Math.log(Math.abs(trendline.intercept)) * xAvg) / xxAvg;
          break;
      }
    } else {
      slope =
        (points.length * xyAvg - xAvg * yAvg) /
  ...
  public initDataSource(trendline: Trendline): void {
    ...
    if (points && points.length) {
      ...
      if (trendline.type !== 'Linear' && trendline.type !== 'MovingAverage') {
        trendlineSeries.chart.splineSeriesModule.findSplinePoint(
          trendlineSeries
        );
      }
    }
  ...
  private setExponentialRange(
    ...
    const slopeIntercept: SlopeIntercept = this.findSlopeIntercept(
      xValue,
      yValue,
      trendline,
      points
    );
    series.points = this.getExponentialPoints(
      trendline,
      points,
      xValue,
      yValue,
  ...
  private setLogarithmicRange(
    ...
    series.points = this.getLogarithmicPoints(
      trendline,
      points,
      xPointsLgr,
      yLogValue,
  ...
  private setPolynomialRange(
    ...
    series.points = this.getPolynomialPoints(
      trendline,
      points,
      xPolyValues,
      yPolyValues,
  ...
  private setPowerRange(
    ...
    series.points = this.getPowerPoints(
      trendline,
      points,
      powerPoints,
      yValues,
  ...
  private setLinearRange(
    ...
    series.points = this.getLinearPoints(
      trendline,
      points,
      xValues,
      yValues,
  ...
  private setMovingAverageRange(
    ...
    series.points = this.getMovingAveragePoints(
      trendline,
      points,
      xAvgValues,
      yValues,
  ...
  private getLogarithmicPoints(
    ...
    yValues: number[],
    series: Series,
    slopeInterceptLog: SlopeIntercept
  ): Points[] {
  ...
  private getPowerPoints(
    ...
    yValues: number[],
    series: Series,
    slopeInterceptPower: SlopeIntercept
  ): Points[] {
    const midPoint: number = Math.round(points.length / 2);
    const pts: Points[] = [];
    ...
    x1 = x1 > -1 ? x1 : 0;
    const y1: number =
      slopeInterceptPower.intercept * Math.pow(x1, slopeInterceptPower.slope);
    const x2: number = xValues[midPoint - 1];
    const y2: number =
      slopeInterceptPower.intercept * Math.pow(x2, slopeInterceptPower.slope);
    const x3: number = xValues[xValues.length - 1] + trendline.forwardForecast;
    const y3: number =
      slopeInterceptPower.intercept * Math.pow(x3, slopeInterceptPower.slope);
  ...
  private getPolynomialPoints(
    ...
    let pts: Points[] = [];
    let polynomialOrder: number =
      points.length <= trendline.polynomialOrder
        ? points.length
        : trendline.polynomialOrder;
    ...
    while (index < xValues.length) {
      ...
      while (subIndex <= trendline.polynomialOrder) {
        ...
        trendline.polynomialSlopes[subIndex as number] +=
          Math.pow(xVal, subIndex) * yVal;
        ++subIndex;
      }
      index++;
    }
    ...
    while (nnIndex <= trendline.polynomialOrder) {
      ...
      while (nnIndex2 <= trendline.polynomialOrder) {
        matrix[nnIndex as number][nnIndex2 as number] =
          numArray[nnIndex + nnIndex2];
        ++nnIndex2;
      }
      ++nnIndex;
    }
  ...
  private getMovingAveragePoints(
    ...
    const pts: Points[] = [];
    let period: number =
      trendline.period >= points.length ? points.length - 1 : trendline.period;
  ...
  private getLinearPoints(
    ...
    yValues: number[],
    series: Series,
    slopeInterceptLinear: SlopeIntercept
  ): Points[] {
    const pts: Points[] = [];
    const max: number = xValues.indexOf(Math.max.apply(null, xValues));
    const min: number = xValues.indexOf(Math.min.apply(null, xValues));
    const x1Linear: number =
      xValues[min as number] - trendline.backwardForecast;
    const y1Linear: number =
  ...
  private getExponentialPoints(
    ...
    yValues: number[],
    series: Series,
    slopeInterceptExp: SlopeIntercept
  ): Points[] {
    const midPoint: number = Math.round(points.length / 2);
    const ptsExp: Points[] = [];
    const x1: number = xValues[0] - trendline.backwardForecast;
    const y1: number =
      slopeInterceptExp.intercept * Math.exp(slopeInterceptExp.slope * x1);
    const x2: number = xValues[midPoint - 1];
    const y2: number =
      slopeInterceptExp.intercept * Math.exp(slopeInterceptExp.slope * x2);
    const x3: number = xValues[xValues.length - 1] + trendline.forwardForecast;
    const y3: number =
      slopeInterceptExp.intercept * Math.exp(slopeInterceptExp.slope * x3);
  ...
  private getPoints(
    ...
    points.sort((a: Points, b: Points) => {
      return a.xValue - b.xValue;
    });
    xValues.sort((a: number, b: number) => {
      return a - b;
    });
    while (polynomialSlopes !== null && index <= polynomialSlopes.length) {
      if (index === 1) {
        ...
        x1 +=
          (points.length +
            (series.xAxis.valueType === 'DateTime'
              ? index
              : trendline.forwardForecast)) /
          polynomialSlopes.length;
    ...
    while (index1 < length) {
      ...
      while (index4 < length) {
        if (numArray3[index4 as number] !== 1) {
          ...
          while (index5 < length) {
            if (
              numArray3[index5 as number] === 0 &&
              Math.abs(matrix[index4 as number][index5 as number]) >= num1
            ) {
      ...
      if (index2 !== index3) {
        ...
        while (index4 < length) {
          ...
          matrix[index2 as number][index4 as number] =
            matrix[index3 as number][index4 as number];
      ...
      while (iandex4 < length) {
        if (iandex4 !== index3) {
          ...
          let index5: number = 0;
          while (index5 < length) {
            matrix[iandex4 as number][index5 as number] -=
              matrix[index3 as number][index5 as number] * num2;
            ++index5;
          }
          polynomialSlopes[iandex4 as number] -=
            polynomialSlopes[index3 as number] * num2;
        }
        ++iandex4;
      }
      ++index1;
    ...
    while (iindex1 >= 0) {
      if (numArray2[iindex1 as number] !== numArray1[iindex1 as number]) {
        ...
        while (iindex2 < length) {
          const num: number =
            matrix[iindex2 as number][numArray2[iindex1 as number]];
          matrix[iindex2 as number][numArray2[iindex1 as number]] =
            matrix[iindex2 as number][numArray1[iindex1 as number]];
  ...
  public getTrendLineElements(series: Series, chart: Chart): void {
    ...
    const clipRectElement: Element = chart.renderer.drawClipPath(
      new RectOption(
        chart.element.id + '_ChartTrendlineClipRect_' + series.index,
        'transparent',
        { width: 1, color: 'Gray' },
        1,
        {
    ...
    for (const trendline of series.trendlines) {
      this.createTrendLineElements(
        chart,
        trendline as Trendline,
        (trendline as Trendline).index,
        element,
        clipRectElement
      );
    }
