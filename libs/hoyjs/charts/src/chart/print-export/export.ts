...
export class Export {
  ...
  private excelExport(
    ...
    let xValues: number[][] = [];
    const isRangeNavigator: boolean =
      controls[0].getModuleName() === 'rangeNavigator';
    const isAccumulation: boolean =
      controls[0].getModuleName() === 'accumulationchart';
    this.series = isRangeNavigator
      ? controls[0].series
      : (controls[0] as Chart | AccumulationChart | StockChart).visibleSeries;
    if (isRangeNavigator && this.series.length === 0) {
      if (controls[0].dataSource) {
        ...
        this.createRangeNavigatorExcelSheet(
          controls[0] as RangeNavigator,
          headerStyle,
          type
        );
      }
      ...
      if (isAccumulation || isRangeNavigator) {
        ...
        this.axisCollection =
          controls[0].getModuleName() === 'stockChart'
            ? (controls[0] as StockChart).chart.horizontalAxes
            : (controls[0] as Chart).horizontalAxes;
      }
      //To get the number of columns for the excel.
      requiredValues = this.getRequiredValues(isRangeNavigator);
      if (
        this.requiredValuesLength === 0 &&
        this.series.length === this.histogramSeriesCount
      ) {
        return;
      }
      //To get all x values in the series.
      xValues = this.getXValue(
        requiredValues,
        controls,
        isRangeNavigator,
        isAccumulation
    ...
    this.requiredValuesLength =
      this.requiredValuesLength === 0 ? 1 : this.requiredValuesLength;
    for (
      let columnCount: number = 0;
      columnCount < this.requiredValuesLength;
      columnCount++
    ) {
      ...
      cells: [
        {
          index: 1,
          value:
            controls.valueType.indexOf('DateTime') > -1
              ? controls.valueType
              : 'Category',
          colSpan: 1,
          rowSpan: 1,
        ...
        cells: [
          {
            ...
            value:
              type === 'CSV' &&
              dataSource[dataCount as number][xName as string] === null
                ? ''
                : dataSource[dataCount as number][xName as string],
            colSpan: 1,
            rowSpan: 1,
            style: headerStyle,
          },
          {
            ...
            value:
              type === 'CSV' &&
              dataSource[dataCount as number][yName as string] === null
                ? ''
                : dataSource[dataCount as number][yName as string],
            colSpan: 1,
            rowSpan: 1,
            style: {},
          },
  ...
  private getRequiredValues(isRangeNavigator: boolean): string[][] {
    ...
    for (
      ...
      if (
        (!isRangeNavigator &&
          !(
            this.series[seriesCount as number] as
              | SeriesModel
              | AccumulationSeriesModel
              | StockSeriesModel
          ).visible) ||
      ...
      if (seriesType === 'Histogram') {
        ...
        requiredValues.push([
          this.series[seriesCount as number]['xName'],
          this.series[seriesCount as number]['high'],
          this.series[seriesCount as number]['low'],
        ]);
        ...
        requiredValues.push([
          ...
          this.series[seriesCount as number]['high'],
          this.series[seriesCount as number]['low'],
          this.series[seriesCount as number]['open'],
          this.series[seriesCount as number]['close'],
        ]);
        if (
          seriesType === 'Candle' &&
          this.series[seriesCount as number]['volume'] !== ''
        ) {
          requiredValues[seriesCount as number].push(
            this.series[seriesCount as number]['volume']
          );
        }
        ...
        requiredValues.push([
          this.series[seriesCount as number]['xName'],
          this.series[seriesCount as number]['yName'],
        ]);
        if (this.series[seriesCount as number]['size'] !== '') {
          requiredValues[seriesCount as number].push(
            this.series[seriesCount as number]['size']
          );
        }
        ...
        if (this.series[seriesCount as number]['radius'].match(/[a-zA-Z]/)) {
          requiredValues[seriesCount as number].push(
            this.series[seriesCount as number]['radius']
          );
        }
      } else {
        requiredValues.push([
      ...
      if (
        (this.series[seriesCount as number] as SeriesModel).errorBar &&
        (this.series[seriesCount as number] as SeriesModel).errorBar.visible
      ) {
        const errorBar: ErrorBarSettingsModel = (
          this.series[seriesCount as number] as SeriesModel
        ).errorBar;
  ...
  private getTitle(
    ...
    if (
      !isRangeNavigator &&
      type === 'XLSX' &&
      (controls[0] as Chart | AccumulationChart | StockChart).title
    ) {
      this.rows.push({});
      this.actualRowCount++;
      isTitle = true;
    ...
    for (
      ...
      axisCount < this.axisCollection.length;
      axisCount++
    ) {
      ...
      for (
        ...
        const axisName: string =
          this.axisCollection[axisCount as number] !== null
            ? this.axisCollection[axisCount as number].name ===
                'primaryXAxis' ||
              (this.axisCollection[axisCount as number].name ===
                'primaryYAxis' &&
                this.series[seriesCount as number].type.indexOf('Bar') > -1)
              ? null
              : this.axisCollection[axisCount as number].name
            : '';
        if (
          ...
          ((!isAccumulation &&
            ...
            (this.series[seriesCount as number] as Series).category ===
              'TrendLine' ||
        ...
        const requiredValuesLength: number =
          this.series[seriesCount as number].type === 'BoxAndWhisker'
            ? requiredValues[seriesCount as number].length - 1
            : requiredValues[seriesCount as number].length;
        if (
          requiredValues[seriesCount as number][1] ===
          this.series[seriesCount as number].yName
        ) {
          for (
            let requiredValuesCount: number = 1;
            requiredValuesCount < requiredValuesLength;
            requiredValuesCount++
          ) {
            cells.push({
              index: index,
              value:
                requiredValuesCount === 1
                  ...
                  : requiredValues[seriesCount as number][
                      requiredValuesCount as number
                    ],
              colSpan: 1,
              rowSpan: 1,
              style: headerStyle,
          ...
          if (this.series[seriesCount as number].type === 'BoxAndWhisker') {
            cells.push({
              ...
              value:
                requiredValues[seriesCount as number][
                  requiredValuesLength as number
                ],
              colSpan: 1,
              rowSpan: 2,
              style: headerStyle,
        ...
        valueType = isAccumulation
          ? requiredValues[0][0]
          : isRangeNavigator
          ? (controls[0] as RangeNavigator).valueType
          : this.axisCollection[axisCount as number].valueType;
        valueType =
          isAccumulation || valueType.indexOf('DateTime') > -1
            ? valueType
            : 'Category';
      }
      if (isXValue) {
        cells[(currentIndex - 1) as number] = {
          index: currentIndex,
          value: valueType,
          colSpan: 1,
          rowSpan: isYName ? 2 : 1,
    ...
    if (isTitle) {
      ...
      cells.push({
        index: 1,
        value: (controls[0] as Chart | AccumulationChart | StockChart).title,
  ...
  private getXValue(
    ...
    for (
      ...
      for (
        ...
        for (
          let dataCount: number = 0;
          dataCount <
          (this.series[seriesCount as number].dataSource as Object[]).length;
          dataCount++
        ) {
          if (
            isAccumulation &&
            !(this.series[seriesCount as number] as AccumulationSeries).points[
              dataCount as number
            ].visible
          ) {
            continue;
          ...
          xValue.push(
            valueType.indexOf('DateTime') > -1
              ? new Date(
                  this.series[seriesCount as number].dataSource[
                    dataCount as number
                  ][requiredValues[seriesCount as number][0]]
                ).getTime()
              : this.series[seriesCount as number].dataSource[
                  dataCount as number
                ][requiredValues[seriesCount as number][0]]
    ...
    for (
      let xValuesLength: number = 0;
      xValuesLength < xValues.length;
      xValuesLength++
    ) {
      xValues[xValuesLength as number] = xValues[
        xValuesLength as number
      ].filter(
  ...
  private createExcelSheet(
    ...
    for (
      ...
      const valueType: string = isAccumulation
        ...
        : this.axisCollection[axisCount as number].valueType;
      for (
        let xValueLength: number = 0;
        xValueLength < xValues[axisCount as number].length;
        xValueLength++
      ) {
        ...
        for (
          ...
          const dataSource: Object[] = this.series[seriesCount as number]
          ...
          for (
            ...
            const xValue: number =
              valueType.indexOf('DateTime') > -1
                ? (this.series[seriesCount as number] as Series).category ===
                  'Pareto'
                  ? new Date(
                      (this.series[seriesCount as number] as Series).points[
                        dataCount as number
                      ][requiredValues[seriesCount as number][0]]
                    ).getTime()
                  : new Date(
            ...
            if (
              xValues[axisCount as number][xValueLength as number] === xValue
            ) {
              let usedValueCount: number = isXValue ? 0 : 1;
              const usedValueLength: number =
                this.series[seriesCount as number].type === 'BoxAndWhisker'
                  ? requiredValues[seriesCount as number].length - 1
                  : requiredValues[seriesCount as number].length;
              for (; usedValueCount < usedValueLength; usedValueCount++) {
                let value: string | boolean | number | Date =
                  usedValueCount !== 0 &&
                  (this.series[seriesCount as number].type ===
                    'BoxAndWhisker' ||
                    (this.series[seriesCount as number] as Series).category ===
                      'Pareto')
                    ? (this.series[seriesCount as number] as Series).points[
              ...
              if (this.series[seriesCount as number].type === 'BoxAndWhisker') {
                cells.push({
                  index: index,
                  value: (this.series[seriesCount as number] as Series).points[
                    dataCount as number
                  ]['outliers'][0],
                  colSpan: 1,
                  rowSpan: 1,
  ...
  public getDataUrl(chart: Chart | AccumulationChart): {
