 ...
 * Chart legend
 */
import { remove, Browser, extend, isNullOrUndefined } from '@hoyui/base';
...
export class Legend extends BaseLegend {
  ...
  private mouseMove(e: MouseEvent): void {
    if (this.chart.legendSettings.visible && !this.chart.isTouch) {
      this.move(e);
      if (
        (<Chart>this.chart).highlightModule &&
        ((<Chart>this.chart).highlightMode !== 'None' ||
          (<Chart>this.chart).legendSettings.enableHighlight)
      ) {
        ...
        for (const id of legendItemsId) {
          if (targetId.indexOf(id) > -1) {
            ...
            (<Chart>this.chart).highlightModule.legendSelection(
              <Chart>this.chart,
              index,
              e.target as Element,
              e.type
  ...
  public getLegendOptions(
    visibleSeriesCollection: Series[],
    chart: Chart
  ): void {
      ...
      if (this.legend.mode === 'Series') {
        if (series.category !== 'Indicator') {
          seriesType =
            chart.chartAreaType === 'PolarRadar'
              ? <ChartDrawType>series.drawType
              : <ChartSeriesType>series.type;
          dashArray =
            !series.marker.visible &&
            seriesType.indexOf('Line') > -1 &&
            seriesType.indexOf('Area') === -1
              ? series.dashArray
              : '';
          // To set legend color when use pointColorMapping
          fill =
            series.pointColorMapping && series.points.length > 0
              ? series.points[0].interior
                ? series.points[0].interior
                : series.interior
              : series.interior;
          this.legendCollections.push(
            new LegendOptions(
              ...
              series.category === 'TrendLine'
                ? (this.chart as Chart).series[series.sourceIndex].trendlines[
                    series.index
                  ].visible
                : series.visible,
              seriesType,
              series.legendImageUrl
                ? series.legendImageUrl
                : series.type === 'Scatter' && series.marker.shape === 'Image'
        ...
        for (const points of series.points) {
          ...
          if (colors.indexOf(fill) < 0) {
            colors.push(fill);
            if (
              chart.rangeColorSettings.length >= 1 &&
              chart.rangeColorSettings[0].colors.length === 1
            ) {
              ...
              this.legendCollections.push(
                new LegendOptions(
                  ...
                  series.legendShape,
                  series.category === 'TrendLine'
                    ? (this.chart as Chart).series[series.sourceIndex]
                        .trendlines[series.index].visible
                    : points.visible,
                  seriesType,
                  series.type === 'Scatter' && series.marker.shape === 'Image'
                    ? series.marker.imageUrl
        ...
        if (
          ...
          chart.rangeColorSettings.length > 0
        ) {
          const startLabel: string =
            chart.rangeColorSettings[0].start.toString();
          const endLabel: string =
            chart.rangeColorSettings[
              chart.rangeColorSettings.length - 1
            ].end.toString();
          this.legendCollections.push(
            new LegendOptions(
  ...
  public getLegendBounds(
    ...
    const verticalArrowSpace: number =
      this.isVertical && !legend.enablePages ? arrowHeight : 0;
    let titleSpace: number =
      this.isTitle && titlePosition === 'Top'
        ? this.legendTitleSize.height + this.fivePixel
        : 0;
    titleSpace =
      this.isTitle && this.isVertical && titlePosition !== 'Top'
        ? this.legendTitleSize.height + this.fivePixel
        : titleSpace;
    if (!this.isVertical) {
    ...
    for (let i: number = 0; i < this.legendCollections.length; i++) {
      ...
      if (legendOption.render && legendOption.text) {
        ...
        if (
          legendBounds.width < padding + rowWidth + titlePlusArrowSpace ||
          this.isVertical
        ) {
    ...
    columnHeight =
      Math.max.apply(null, this.columnHeights) + padding + titleSpace;
    columnHeight = Math.max(
      columnHeight,
      this.maxItemHeight + padding + padding + titleSpace
    );
    ...
    if (this.isPaging && !legend.enablePages) {
      if (!this.isVertical) {
        columnHeight =
          ...
          padding +
          padding +
          (titlePosition === 'Top' ? titleSpace : 0);
      }
    }
    ...
    switch (legend.textWrap) {
      ...
      case 'AnyWhere':
        if (
          ...
          legendWidth + rowWidth > legendBounds.width
        ) {
          legendOption.textCollection = textWrap(
            ...
            legend.maximumLabelWidth
              ? Math.min(
                  legend.maximumLabelWidth,
                  legendBounds.width - textPadding
                )
              : legendBounds.width - textPadding,
            legend.textStyle,
        ...
        legendOption.textSize.height =
          legendHeight * legendOption.textCollection.length;
        break;
    }
  }
  /**
  ...
  public getRenderPoint(
    ...
    const textWidth: number =
      ...
      (this.legend.maximumLabelWidth
        ? this.legend.maximumLabelWidth
        : previousLegend.textSize.width);
    const previousBound: number =
      previousLegend.location.x + (!this.isRtlEnable ? textWidth : -textWidth);
    if (
      this.isWithinBounds(
        previousBound,
        (this.legend.maximumLabelWidth
    ...
    if (!this.isVertical && this.isPaging && !this.legend.enablePages) {
      availwidth =
        this.legendBounds.width - legendOption.location.x - this.fivePixel;
    }
    availwidth = this.legend.maximumLabelWidth
      ? Math.min(this.legend.maximumLabelWidth, availwidth)
      : availwidth;
    if (
  ...
  public LegendClick(index: number, event: Event | PointerEvent): void {
    ...
    const seriesIndex: number =
      chart.legendSettings.mode === 'Series' ? index : 0;
    const legendIndex: number = !this.isReverse
      ? index
      : this.legendCollections.length - 1 - index;
    ...
    if (chart.legendSettings.mode === 'Series') {
      const legendClickArgs: ILegendClickEventArgs = {
        ...
        chart: chart.isBlazor ? ({} as Chart) : chart,
        series: series,
      ...
      if (!legendClickArgs.cancel) {
        ...
        if (chart.legendSettings.toggleVisibility) {
          ...
          if (series.category === 'TrendLine') {
            if (
              !chart.series[series.sourceIndex].trendlines[series.index]
                .visible &&
              chart.series[series.sourceIndex].visible
            ) {
              chart.series[series.sourceIndex].trendlines[
                series.index
              ].visible = true;
            } else {
              chart.series[series.sourceIndex].trendlines[
          ...
          if (chart.highlightModule && chart.legendSettings.enableHighlight) {
            chart.highlightModule.legendSelection(
              chart,
              index,
          ...
          chart.selectionModule.legendSelection(
            chart,
            index,
            event.target as Element,
            event.type
      ...
      if (chart.legendSettings.toggleVisibility && !legendClickArgs.cancel) {
        ...
        const legendOption: LegendOptions =
          this.legendCollections[index as number];
  ...
  private refreshLegendToggle(chart: Chart, series: Series): void {
    ...
    if (chart.selectionModule) {
      selectedDataIndexes = <Indexes[]>(
        extend([], chart.selectionModule.selectedDataIndexes, null, true)
      );
    }
    if (
      chart.svgObject.childNodes.length > 0 &&
    ...
    removeElement(
      getElement(chart.element.id + '_Secondary_Element').querySelectorAll(
        '.ejSVGTooltip'
      )[0]
    );
  ...
  private changeSeriesVisiblity(series: Series, visibility: boolean): void {
    ...
    if (
      this.isSecondaryAxis(series.yAxis) ||
      (series.category === 'Pareto' && series.type === 'Line')
    ) {
    ...
    for (const series of seriesCollection) {
      if (series.type.indexOf('Spline') > -1) {
        const isArea: boolean =
          ...
          series.drawType.indexOf('Area') > -1;
        ...
        pageY +
          (this.isPaging
            ? (this.currentPageNumber - 1) *
              this.translatePage((this.chart as Chart).enableCanvas, null, 1, 2)
            : 0),
        region.rect
      );
    });
