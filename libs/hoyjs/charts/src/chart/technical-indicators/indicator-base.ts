...
import { BaseAttibutes, Rect, SvgRenderer } from '@hoyui/svg-base';
/**
...
export class TechnicalAnalysis extends LineBase {
   ...
   * @returns {void}
   */
  public createIndicatorElements(
    chart: Chart,
    indicator: TechnicalIndicator,
    index: number
  ): void {
    ...
    const options: BaseAttibutes = new RectOption(
      chart.element.id + '_ChartIndicatorClipRect_' + index,
      'transparent',
      { width: 1, color: 'Gray' },
      1,
      {
    ...
    indicator.indicatorElement = chart.renderer.createGroup({
      ...
      'clip-path':
        'url(#' + chart.element.id + '_ChartIndicatorClipRect_' + index + ')',
    });
