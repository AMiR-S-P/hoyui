...
import { isNullOrUndefined, extend, getValue } from '@hoyui/base';
...
export class Double {
   ...
   */
  protected calculateNumericNiceInterval(
    axis: Axis,
    delta: number,
    size: Size
  ): number {
    const actualDesiredIntervalsCount: number = getActualDesiredIntervalsCount(
      size,
      axis
    );
      ...
      for (const series of axis.series) {
        ...
        if (axis.orientation === 'Horizontal') {
          if (this.chart.requireInvertedAxis) {
            ...
            this.findMinMax(
              <number>series.xMin - this.paddingInterval,
              <number>series.xMax + this.paddingInterval
            );
          }
        }
        // For yRange
      ...
      for (let i: number = 0; i < series.yData.length; i++) {
        if (
          !(
            ...
            series.intermediateSumIndexes.indexOf(i as number) !== -1
          ) &&
  ...
  private findNormal(
    ...
    if (minimum === 0 || (minimum < 0 && maximum < 0)) {
      interval = this.calculateNumericNiceInterval(
        axis,
        axis.doubleRange.delta,
        size
      );
    ...
    if (this.chart.chartAreaType === 'Cartesian') {
      ...
      if ((axis.zoomFactor < 1 || axis.zoomPosition > 0) && !isLazyLoad) {
        ...
        axis.visibleRange.interval = axis.enableAutoIntervalOnZooming
          ? this.calculateNumericNiceInterval(
              axis,
              axis.doubleRange.delta,
              size
            )
    ...
    if (
      ...
      if (duplicateTempInterval < axis.visibleRange.max) {
        axis.visibleRange.max =
          duplicateTempInterval + axis.visibleRange.interval;
      }
    }
  ...
  public calculateVisibleLabels(
    ...
    const isPolarRadar: boolean =
      controlName === 'chart' &&
      (chart as Chart).chartAreaType === 'PolarRadar';
    if (
      !isPolarRadar &&
      (axis.zoomFactor < 1 || axis.zoomPosition > 0 || this.paddingInterval)
    ) {
      tempInterval =
        axis.visibleRange.min -
        (axis.visibleRange.min % axis.visibleRange.interval);
    }
    ...
    if (axis.labelFormat && axis.labelFormat.indexOf('n') > -1) {
      formatDigits = parseInt(
        axis.labelFormat.substring(1, axis.labelFormat.length),
        10
      );
    }
    ...
    if (
      axis.visibleRange.interval &&
      (axis.visibleRange.interval + '').indexOf('.') >= 0
    ) {
    ...
    for (
      ...
      if (withIn(tempInterval, axis.visibleRange)) {
        triggerLabelRender(
          chart,
          tempInterval,
          this.formatValue(axis, isCustom, format, tempInterval),
          labelStyle,
          axis
        );
      }
    ...
    if (
      tempInterval &&
      (tempInterval + '').indexOf('.') >= 0 &&
      (tempInterval + '').split('.')[1].length > 10
    ) {
      tempInterval =
        (tempInterval + '').split('.')[1].length >
        (formatDigits || intervalDigits)
          ? +tempInterval.toFixed(formatDigits || intervalDigits)
          : tempInterval;
  ...
  protected getFormat(axis: Axis): string {
    if (axis.labelFormat) {
      if (
        axis.labelFormat.indexOf('p') === 0 &&
        axis.labelFormat.indexOf('{value}') === -1 &&
        axis.isStack100
      ) {
        return '{value}%';
      }
  ...
  public formatValue(
    ...
    const labelValue: number = !(tempInterval % 1)
      ? tempInterval
      : Number(tempInterval.toLocaleString('en-US').split(',').join(''));
    return isCustom
      ? format.replace('{value}', axis.format(labelValue))
      : format
      ? axis.format(tempInterval)
      : axis.format(labelValue);
  }
