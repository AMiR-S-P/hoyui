...
export class Logarithmic extends Double {
  ...
  public getActualRange(axis: Axis, size: Size): void {
    ...
    let logEnd: number =
      this.max === 1 ? 1 : logBase(<number>this.max, axis.logBase);
    logEnd = isFinite(logStart) ? logEnd : <number>this.max;
    this.min = Math.floor(logStart / 1);
    const isRectSeries: boolean =
      axis.series &&
      axis.series.some((item: Series) => {
        return (
          (item.type.indexOf('Column') !== -1 ||
            item.type.indexOf('Bar') !== -1) &&
          item.type.indexOf('Range') === -1
        );
      });
    if (isRectSeries) {
  ...
  protected calculateVisibleRange(size: Size, axis: Axis): void {
    ...
    if ((axis.zoomFactor < 1 || axis.zoomPosition > 0) && !isLazyLoad) {
      ...
      axis.visibleRange.interval =
        Math.floor(axis.visibleRange.interval) === 0
          ? 1
          : Math.floor(axis.visibleRange.interval);
   ...
   */
  protected calculateLogNiceInterval(
    delta: number,
    size: Size,
    axis: Axis
  ): number {
    ...
    for (
      let j: number = 0, len: number = axis.intervalDivs.length;
      j < len;
      j++
    ) {
      const currentInterval: number =
        minInterval * axis.intervalDivs[j as number];
  ...
  public calculateVisibleLabels(
    ...
    axis.startLabel = axis.format(
      startValue < 1 ? +startValue.toPrecision(1) : startValue
    );
      ...
      if (withIn(tempInterval, axis.visibleRange)) {
        ...
        triggerLabelRender(
          ...
          this.formatValue(
            ...
            isCustomFormat,
            axisFormat,
            value < 1 ? +value.toPrecision(1) : value
          ),
          labelStyle,
          axis
        );
