...
export class Category extends NiceInterval {
  ...
  public getActualRange(axis: Axis, size: Size): void {
    ...
    axis.actualRange = {};
    if (!axis.interval) {
      axis.actualRange.interval = Math.max(
        1,
        Math.floor(
          axis.doubleRange.delta / getActualDesiredIntervalsCount(size, axis)
        )
      );
    } else {
  ...
  public applyRangePadding(axis: Axis, size: Size): void {
    ...
    axis.series.forEach((element: Series) => {
      if (!isColumn) {
        isColumn =
          (element.type.indexOf('Column') > -1 ||
            element.type.indexOf('Bar') > -1) &&
          !(axis.zoomFactor < 1 || axis.zoomPosition > 0) &&
          isNullOrUndefined(axis.minimum) &&
          isNullOrUndefined(axis.maximum);
      }
    });
    const ticks: number =
      (axis.labelPlacement === 'BetweenTicks' || isColumn) &&
  ...
  public calculateVisibleLabels(axis: Axis): void {
    ...
    axis.visibleRange.interval =
      axis.visibleRange.interval < 1 ? 1 : axis.visibleRange.interval;
    ...
    for (
      ...
      if (withIn(tempInterval, axis.visibleRange) && axis.labels.length > 0) {
        ...
        triggerLabelRender(
          this.chart,
          position,
          axis.labels[position as number]
            ? axis.labels[position as number].toString()
            : '',
          labelStyle,
          axis
        );
