...
export class DateTimeCategory extends Category {
  ...
  public calculateVisibleLabels(axis: Axis): void {
    ...
    const isRangeNavigator: boolean =
      this.chart.getModuleName() === 'rangeNavigator';
    ...
    for (; i < axis.labels.length; i += interval) {
      ...
      if (this.chart.stockChart || isRangeNavigator) {
        if (axis.intervalType === 'Auto') {
          if (
            ((!isRangeNavigator && i === 1) ||
              this.StartOfWeek(
                axis.labels.map(Number)[i as number],
                axis.labels.map(Number)[i - 1],
                axis,
                i,
                previousIndex
              ) ||
            ...
            triggerLabelRender(
              ...
              i,
              axis.isIndexed
                ? this.getIndexedAxisLabel(
                    axis.labels[i as number],
                    axis.format
                  )
                : <string>(
                    axis.format(new Date(axis.labels.map(Number)[i as number]))
                  ),
              labelStyle,
              axis
            );
          ...
          if (
            (!isRangeNavigator && i === 1) ||
            this.isMaximum(i, previousIndex, axis)
          ) {
            triggerLabelRender(
              this.chart,
              i,
    ...
    axis.startLabel = axis.visibleLabels[0]
      ? (axis.visibleLabels[0].text as string)
      : '';
    axis.endLabel = axis.visibleLabels[axis.visibleLabels.length - 1]
      ? (axis.visibleLabels[axis.visibleLabels.length - 1].text as string)
      : '';
  ...
  public getIndexedAxisLabel(value: string, format: Function): string {
    ...
    for (let i: number = 0; i < texts.length; i++) {
      texts[i as number] = <string>(
        format(new Date(parseInt(texts[i as number], 10)))
      );
    }
    return texts.join(', ');
  }
      ...
      switch (type) {
        ...
        case 'Quarter':
          sameValue =
            new Date(currentDate).getFullYear() ===
              new Date(previousDate).getFullYear() &&
            Math.floor(new Date(currentDate).getMonth() / 3) ===
        ...
        case 'Months':
          sameValue =
            ...
            new Date(currentDate).getMonth() ===
              new Date(previousDate).getMonth();
          break;
        case 'Weeks':
          sameValue =
            new Date(currentDate).getFullYear() ===
        ...
        case 'Days':
          sameValue =
            Math.abs(currentDate - previousDate) < 24 * 60 * 60 * 1000 &&
            new Date(currentDate).getDay() === new Date(previousDate).getDay();
          break;
        case 'Hours':
          sameValue =
            Math.abs(currentDate - previousDate) < 60 * 60 * 1000 &&
            new Date(currentDate).getDay() === new Date(previousDate).getDay();
          break;
        case 'Minutes':
          sameValue =
            Math.abs(currentDate - previousDate) < 60 * 1000 &&
            new Date(currentDate).getMinutes() ===
              new Date(previousDate).getMinutes();
          break;
        case 'Seconds':
          sameValue =
            Math.abs(currentDate - previousDate) < 1000 &&
            new Date(currentDate).getDay() === new Date(previousDate).getDay();
          break;
      }
    }
  ...
  private StartOfWeek(
    ...
    if (axis.labels.length >= labelsCount) {
      ...
      isMonday =
        !(previousDay.getTime() === currentday.getTime()) &&
        this.isMaximum(index, previousIndex, axis);
    } else {
  ...
  public isMaximum(index: number, previousIndex: number, axis: Axis): boolean {
    ...
    const previousPointX: number =
      valueToCoefficient(previousIndex, axis) * axis.rect.width;
    return (
      pointX - previousPointX >=
      (axis.labels.length >= 15
        ? axisLabelMaximumLength
        : axisLabelMaximumLength / 2)
    );
  }
  /**
   * Get module name.
