...
import { Size, Rect, measureText } from '@hoyui/svg-base';
...
export class Row extends ChildProperty<Row> {
  ...
  public computeSize(
    ...
    const innerPadding: number =
      axis.labelPosition === 'Inside' && chart.axes.indexOf(axis) > -1 ? -5 : 5;
    if (axis.visible && axis.internalVisibility) {
      width +=
        axis.findTickSize(axis.crossInAxis) +
        scrollBarHeight +
        axis.findLabelSize(axis.crossInAxis, innerPadding, definition, chart) +
...
export class Axis extends ChildProperty<Axis> {
   ...
   */
  @Complex<LabelBorderModel>(
    { color: null, width: 0, type: 'Rectangle' },
    LabelBorder
  )
  ...
  public findTickSize(crossAxis: Axis): number {
    ...
    if (
      crossAxis &&
      (!crossAxis.visibleRange || this.isInside(crossAxis.visibleRange))
    ) {
      return 0;
    }
  ...
  public isInside(range: VisibleRangeModel): boolean {
    return (
      inside(this.crossAt, range) ||
      (!this.isAxisOpposedPosition && this.crossAt >= range.max) ||
      (this.isAxisOpposedPosition && this.crossAt <= range.min)
    );
  }
  ...
  public findLabelSize(
      ...
      if (isNullOrUndefined(angle)) {
        ...
        titleSize =
          (this.orientation === 'Vertical'
            ...
            : this.titleSize.height) + innerPadding;
      }
      if (this.rect.width || this.rect.height) {
        const length: number = isHorizontal
          ? this.rect.width
          : this.rect.height;
    ...
    let labelSize: number =
      ...
      this.labelPadding +
      (this.orientation === 'Vertical'
        ? this.maxLabelSize.width
        : this.maxLabelSize.height) +
      this.multiLevelLabelHeight;
    if (crossAxis && this.placeNextToAxisLine) {
      const range: VisibleRangeModel = crossAxis.visibleRange;
      const size: number =
        crossAxis.orientation === 'Horizontal'
      ...
      if (!range || !size) {
        ...
        diff =
          value *
          ((size - (diff < labelSize ? labelSize - diff : 0)) / range.delta);
  ...
  public updateCrossValue(): void {
    ...
    if (this.orientation === 'Horizontal') {
      value =
        this.crossInAxis.rect.height -
        valueToCoefficient(value, this.crossInAxis) *
          this.crossInAxis.rect.height;
      this.updatedRect.y = this.crossInAxis.rect.y + value;
    } else {
      value =
        valueToCoefficient(value, this.crossInAxis) *
        this.crossInAxis.rect.width;
  ...
  private findDifference(crossAxis: Axis): number {
    let value: number = 0;
    if (this.isAxisOpposedPosition) {
      value = crossAxis.isAxisInverse
        ? crossAxis.visibleRange.min
        : crossAxis.visibleRange.max;
    } else {
      value = crossAxis.isAxisInverse
        ? crossAxis.visibleRange.max
        : crossAxis.visibleRange.min;
  ...
  public calculateVisibleRangeOnZooming(): void {
    if (isZoomSet(this)) {
      ...
      if (!this.isAxisInverse) {
        start =
          this.actualRange.min + this.zoomPosition * this.actualRange.delta;
        end = start + this.zoomFactor * this.actualRange.delta;
      } else {
        start =
          this.actualRange.max - this.zoomPosition * this.actualRange.delta;
        end = start - this.zoomFactor * this.actualRange.delta;
      }
    ...
    switch (this.orientation) {
      case 'Horizontal':
        if (chart.requireInvertedAxis) {
          padding =
            this.isStack100 || this.baseModule.chart.stockChart
              ? 'Round'
              : 'Normal';
        } else {
          padding = 'None';
  ...
  public getMaxLabelWidth(chart: Chart): void {
    ...
    for (
      ...
      const width: number = isAxisLabelBreak
      ...
      const height: number = isAxisLabelBreak
        ? label.breakLabelSize.height
        : label.size.height;
      if (height > this.maxLabelSize.height) {
        this.maxLabelSize.height = height;
      }
      if (isAxisLabelBreak) {
        label.text = this.enableTrim
          ? label.text
          : label.originalText.split('<br>');
      }
      if (action === 'None' || action === 'Hide' || action === 'Trim') {
        continue;
      }
      if (
        (<LabelIntersectAction>action !== 'None' || this.angle % 360 === 0) &&
        this.orientation === 'Horizontal' &&
        this.rect.width > 0 &&
        !isIntersect
      ) {
        const width1: number = isAxisLabelBreak
          ? label.breakLabelSize.width
          : label.size.width;

        pointX =
          valueToCoefficient(label.value, this) * this.rect.width + this.rect.x;
        pointX -= width1 / 2;
        if (this.edgeLabelPlacement === 'Shift') {
          ...
          if (
            i === this.visibleLabels.length - 1 &&
            pointX + width1 > this.rect.x + this.rect.width
          ) {
            ...
            if (isAxisLabelBreak) {
              ...
              for (let index: number = 0; index < label.text.length; index++) {
                result = textWrap(
                  ...
                  this.rect.width / this.visibleLabels.length,
                  this.labelStyle,
                  chart.enableRtl,
                  null,
                  null,
    ...
    if (this.angle !== 0 && this.orientation === 'Horizontal') {
      ...
      this.rotatedLabel = isNullOrUndefined(this.rotatedLabel)
        ? ''
        : this.rotatedLabel;
      ...
      if (
        ...
        !isHorizontalAngle &&
        isBreakLabel(this.rotatedLabel)
      ) {
        ...
        this.maxLabelSize = rotateTextSize(
          this.labelStyle,
          this.rotatedLabel,
          this.angle,
          chart,
  ...
  private findMultiRows(
    length: number,
    currentX: number,
    currentLabel: VisibleLabels,
    isBreakLabels: boolean
  ): void {
    let label: VisibleLabels;
    let pointX: number;
    let width2: number;
    ...
    for (let i: number = length - 1; i >= 0; i--) {
      ...
      pointX =
        valueToCoefficient(label.value, this) * this.rect.width + this.rect.x;
      isMultiRows = !this.isAxisInverse
        ? currentX < pointX + width2 * 0.5
    ...
    const height: number =
      (isBreakLabels
        ? currentLabel.breakLabelSize.height
        : currentLabel.size.height) *
        currentLabel.index +
  ...
  public setIsInversedAndOpposedPosition(isPolar: boolean = false): void {
    this.isAxisOpposedPosition =
      this.opposedPosition ||
      (!isPolar && this.isRTLEnabled && this.orientation === 'Vertical');
    if (
      this.opposedPosition &&
      !isPolar &&
      this.isRTLEnabled &&
    ...
    this.isAxisInverse =
      this.isInversed ||
      (this.isRTLEnabled && this.orientation === 'Horizontal');
    if (
      this.isInversed &&
      !isPolar &&
      this.isRTLEnabled &&
  ...
  public updateAxis(): void {
    ...
    const chartAxisLayoutPanel: CartesianAxisLayoutPanel =
      chart.chartAxisLayoutPanel as CartesianAxisLayoutPanel;
    const index: number = chart.axisCollections.indexOf(this);
    const axisElement: Element = getElement(
      chart.element.id + 'AxisInsideCollection'
    );
    const axisLineElement: Element = getElement(
      chart.element.id + 'AxisOutsideCollection'
    );
    chartAxisLayoutPanel.element = chart.renderer.createGroup({
      id: chart.element.id + 'AxisGroup' + index + 'Inside',
    });
    const outsideElement: Element = chart.renderer.createGroup({
      id: chart.element.id + 'AxisGroup' + index + 'Outside',
    });
    const isInside: boolean = chartAxisLayoutPanel.findAxisPosition(this);
    chartAxisLayoutPanel.drawAxis(
      this,
      index,
      isInside,
      outsideElement,
...
export class VisibleLabels {
  ...
  constructor(
    ...
    breakLabelSize: Size = new Size(0, 0),
    index: number = 1
  ) {
    this.text = text;
