...
export class CartesianAxisLayoutPanel {
  ...
  public measureAxis(rect: Rect): void {
    ...
    const chartAreaWidth: number = chart.chartArea.width
      ? stringToNumber(chart.chartArea.width, chart.availableSize.width)
      : null;
    ...
    this.initialClipRect = subtractThickness(
      this.initialClipRect,
      new Thickness(this.leftSize, this.rightSize, 0, 0)
    );
    ...
    this.initialClipRect = subtractThickness(
      this.initialClipRect,
      new Thickness(0, 0, this.topSize, this.bottomSize)
    );
    ...
    this.seriesClipRect = subtractThickness(
      this.seriesClipRect,
      new Thickness(this.leftSize, this.rightSize, 0, 0)
    );
  ...
  private measureRowAxis(chart: Chart, rect: Rect): void {
    ...
    for (const item of chart.rows) {
      ...
      this.measureDefinition(
        row,
        chart,
        new Size(chart.availableSize.width, row.computedHeight)
      );
  ...
  private measureColumnAxis(chart: Chart, rect: Rect): void {
    ...
    for (const item of chart.columns) {
      ...
      this.measureDefinition(
        column,
        chart,
        new Size(column.computedWidth, chart.availableSize.height)
      );
  ...
  public measureDefinition(
    ...
    for (const axis of definition.axes) {
      ...
      axis.scrollBarHeight =
        chart.scrollBarModule &&
        chart.zoomModule &&
        chart.zoomSettings.enableScrollbar &&
        axis.enableScrollbarOnZooming &&
        chart.zoomModule.isZoomed &&
    ...
    for (let i: number = 0, len: number = chart.rows.length; i < len; i++) {
      ...
      for (let j: number = 0, len: number = row.axes.length; j < len; j++) {
        ...
        if (axis.rect.height === 0) {
          ...
          size = 0;
          for (
            let k: number = i + 1, len: number = i + axis.span;
            k < len;
            k++
          ) {
          ...
          axis.rect.y =
            row.computedTop -
            size +
            (axis.plotOffsetTop ? axis.plotOffsetTop : axisOffset);
          axis.rect.height =
            axis.rect.height +
            size -
            this.getAxisOffsetValue(
              axis.plotOffsetTop,
              axis.plotOffsetBottom,
              axis.plotOffset
            );
          axis.rect.width = 0;
        }
        if (axis.isAxisOpposedPosition) {
          if (
            axis.labelPosition === 'Inside' &&
            axis.orientation === 'Vertical'
          ) {
            x =
              rect.x + rect.width - sum(subArray(row.insideFarSizes, farCount));
          } else {
    ...
    for (let i: number = 0, len: number = chart.columns.length; i < len; i++) {
      ...
      for (let j: number = 0, len: number = column.axes.length; j < len; j++) {
        ...
        if (axis.rect.width === 0) {
          ...
          axis.rect.x =
            column.computedLeft +
            (axis.plotOffsetLeft ? axis.plotOffsetLeft : axisOffset);
          axis.rect.width -= this.getAxisOffsetValue(
            axis.plotOffsetLeft,
            axis.plotOffsetRight,
            axis.plotOffset
          );
          axis.rect.height = 0;
        }
        if (axis.isAxisOpposedPosition) {
          ...
          if (
            ...
            y =
              rect.y +
              rect.height -
              sum(subArray(column.insideNearSizes, nearCount));
          } else {
            y =
              rect.y + rect.height + sum(subArray(column.nearSizes, nearCount));
          }
          axis.rect.y = axis.rect.y >= y ? axis.rect.y : y;
          nearCount++;
        }
  ...
  public measure(): void {
    ...
    for (const axis of chart.axisCollections) {
      ...
      if (axis.orientation === 'Vertical') {
        ...
        span =
          actualIndex + axis.span > chart.rows.length
            ? chart.rows.length
            : actualIndex + axis.span;
        ...
        span =
          actualIndex + axis.span > chart.columns.length
  ...
  private getAxisOffsetValue(
    ...
    const rangeOffset: number = position1
      ? position1 + (position2 ? position2 : plotOffset)
      : position2
      ? position2 + plotOffset
      : 2 * plotOffset;
    return rangeOffset;
  }
  ...
  private crossAt(chart: Chart): void {
    for (const axis of chart.axisCollections) {
      ...
      if (!axis.crossesInAxis) {
        ...
        for (
          let i: number = 2, len: number = chart.axisCollections.length;
          i < len;
          i++
        ) {
    ...
    switch (axis.valueType) {
      case 'DateTime': {
        ...
        return Date.parse(
          dateParser(
            dateFormatter(
              new Date(DataUtil.parse.parseJson({ val: crossAt }).val)
            )
          )
        );
      }
      case 'Category':
        return parseFloat(<string>crossAt)
          ? parseFloat(<string>crossAt)
          : axis.labels.indexOf(<string>crossAt);
      case 'Logarithmic':
  ...
  private pushAxis(definition: Row | Column, axis: Axis): void {
    for (
      let i: number = 0, len: number = definition.axes.length;
      i <= len;
      i++
    ) {
  ...
  private getActualColumn(axis: Axis): number {
    ...
    const result: number =
      pos >= actualLength ? actualLength - 1 : pos < 0 ? 0 : pos;
    return result;
  }
  ...
  private calculateRowSize(rect: Rect): void {
    ...
    for (let i: number = 0, len: number = chart.rows.length; i < len; i++) {
      ...
      if (row.height.indexOf('%') !== -1) {
        height = Math.min(
          remainingHeight,
          (rect.height * parseInt(row.height, 10)) / 100
        );
      } else {
  ...
  private calculateColumnSize(rect: Rect): void {
    ...
    for (let i: number = 0, len: number = chart.columns.length; i < len; i++) {
      ...
      if (column.width.indexOf('%') !== -1) {
        width = Math.min(
          remainingWidth,
          (rect.width * parseInt(column.width, 10)) / 100
        );
      } else {
  ...
  public renderAxes(): Element {
    const chart: Chart = this.chart;
    let axis: Axis;
    const axisElement: Element = chart.renderer.createGroup({
      id: chart.element.id + 'AxisInsideCollection',
    });
    const axisLineElement: Element = chart.renderer.createGroup({
    ...
    for (
      let i: number = 0, len: number = chart.axisCollections.length;
      i < len;
      i++
    ) {
  ...
  private renderScrollbar(chart: Chart, axis: Axis): void {
    const isZoomed: boolean = isNullOrUndefined(chart.zoomModule)
      ? false
      : chart.zoomModule.isZoomed;
    if (
      ((isZoomed && (axis.zoomFactor < 1 || axis.zoomPosition > 0)) ||
        (axis.scrollbarSettings.enable &&
          (axis.zoomFactor <= 1 || axis.zoomPosition >= 0))) &&
      ...
      if (!chart.scrollElement) {
        chart.scrollElement =
          redrawElement(chart.redraw, chart.element.id + '_scrollElement') ||
          createElement('div', { id: chart.element.id + '_scrollElement' });
      }
      appendChildElement(
        false,
        chart.scrollElement,
        axis.zoomingScrollBar.render(true),
        true
      );
    } else if (
      axis.zoomFactor === 1 &&
      axis.zoomPosition === 0 &&
      axis.zoomingScrollBar.svgObject &&
      !axis.scrollbarSettings.enable
    ) {
      axis.zoomingScrollBar.destroy();
    } else if (axis.zoomingScrollBar.svgObject) {
      (axis.zoomingScrollBar.svgObject as SVGElement).style.top =
        (axis.isAxisOpposedPosition && axis.orientation === 'Horizontal'
          ? -16
          : 0) +
        axis.rect.y +
        Math.max(0.5, axis.lineStyle.width / 2) +
        'px';
      (axis.zoomingScrollBar.svgObject as SVGElement).style.left =
        (axis.isAxisOpposedPosition && axis.orientation !== 'Horizontal'
          ? 16
          : 0) +
        axis.rect.x -
  ...
  public drawPaneLines(chart: Chart, axisElement?: Element): void {
    ...
    axisElement = axisElement
      ? axisElement
      : getElement(chart.element.id + 'AxisInsideCollection');
  ...
  public drawAxis(
    ...
    if (axis.orientation === 'Horizontal') {
      if (axis.visible && axis.internalVisibility && axis.lineStyle.width > 0) {
        this.drawAxisLine(
          ...
          axis.plotOffsetLeft,
          axis.plotOffsetRight,
          isInside ? outsideElement : this.element,
          axis.updatedRect
        );
      }
      axisName = 'X';
    ...
    if (
      ...
      this['draw' + axisName + 'AxisGridLine'](
        axis,
        index,
        isInside || axis.tickPosition === 'Inside'
          ? outsideElement
          : this.element,
        axis.updatedRect
      );
    ...
    if (axis.visible && axis.internalVisibility) {
      this['draw' + axisName + 'AxisLabels'](
        axis,
        index,
        isInside || axis.labelPosition === 'Inside'
          ? outsideElement
          : this.element,
        axis.placeNextToAxisLine ? axis.updatedRect : axis.rect
      );

      this['draw' + axisName + 'AxisBorder'](
        axis,
        index,
        isInside || axis.labelPosition === 'Inside'
          ? outsideElement
    ...
    if (
      this.chart.scrollBarModule &&
      ((this.chart.zoomSettings.enableScrollbar &&
        axis.enableScrollbarOnZooming) ||
        axis.scrollbarSettings.enable)
    ) {
  ...
  public findAxisPosition(axis: Axis): boolean {
    return (
      axis.crossAt !== null && axis.isInside(axis.crossInAxis.visibleRange)
    );
  }
  ...
  private drawAxisLine(
    ...
    optionsLine = {
      ...
      d:
        'M ' +
        (rect.x - plotX - plotLeft) +
        ' ' +
        (rect.y - plotY - plotTop) +
        ' L ' +
        (rect.x + rect.width + plotX + plotRight) +
        ' ' +
        (rect.y + rect.height + plotY + plotBottom),
  ...
  private drawYAxisGridLine(
    ...
    const tickSize: number = isOpposed
      ? axis.majorTickLines.height
      : -axis.majorTickLines.height;
    const axisLineSize: number = isOpposed
      ? axis.lineStyle.width * 0.5
      : -axis.lineStyle.width * 0.5;
    const ticksbwtLabel: number =
      axis.valueType === 'Category' && axis.labelPlacement === 'BetweenTicks'
        ? 0.5
        : 0;
    const scrollBarHeight: number = isNullOrUndefined(axis.crossesAt)
      ? isOpposed
        ? axis.scrollBarHeight
        : -axis.scrollBarHeight
      : 0;
    ...
    let count: number = 1;
    if (
      axis.valueType.indexOf('Category') > -1 &&
    ...
    for (let i: number = 0; i < length; i++) {
      ...
      if (pointY >= rect.y && rect.y + rect.height >= pointY) {
        if (
          this.chart.redraw &&
          !this.chart.enableCanvas &&
          this.chart.zoomRedraw &&
          axis.visible &&
          axis.majorGridLines.width &&
          i !== 0 &&
          !getElement(
            this.chart.element.id + '_MajorGridLine_' + index + '_' + i
          )
        ) {
          majorGrid =
            'M ' +
            this.seriesClipRect.x +
            ' ' +
            (this.seriesClipRect.y +
              (axis.isInversed
                ? this.seriesClipRect.height +
                  (this.seriesClipRect.height / (i ? i : 1)) * count
                : -((this.seriesClipRect.height / (i ? i : 1)) * count))) +
            ' L ' +
            (this.seriesClipRect.x + this.seriesClipRect.width) +
            ' ' +
            (this.seriesClipRect.y +
              (axis.isInversed
                ? this.seriesClipRect.height +
          ...
          getElement(parent.id).appendChild(
            this.element.childNodes[this.element.childNodes.length - 1]
          );
        }
        if (
          inside(tempInterval, axis.visibleRange) ||
          this.isBorder(axis, i, pointY)
        ) {
          majorGrid =
            'M ' +
            this.seriesClipRect.x +
          ...
          this.renderGridLine(
            ...
            i,
            this.element,
            chartThemeStyle.majorGridLine,
            axis.majorGridLines.dashArray
          );
        }
        if (
          this.chart.redraw &&
          ...
          i !== 0 &&
          !getElement(
            this.chart.element.id + '_MajorTickLine_' + index + '_' + i
          ) &&
          !this.chart.enableCanvas &&
          axis.visible
        ) {
        ...
        majorTick =
          'M ' +
          (rect.x + axisLineSize + (isTickInside ? scrollBarHeight : 0)) +
          ' ' +
          pointY +
          ' L ' +
          ticks +
        ...
        this.renderGridLine(
          ...
          i,
          parent,
          chartThemeStyle.majorTickLine
        );
        if (
          (minorGridLines.width > 0 || minorTickLines.width > 0) &&
          ...
          minorGridDirection = this.drawAxisMinorLine(
            axis,
            tempInterval,
            rect,
            i
          ...
          this.renderGridLine(
            axis,
            index,
            minorGridDirection[0],
            minorGridLines,
            '_MinorGridLine_',
            i,
            this.element,
            chartThemeStyle.minorGridLine,
            minorGridLines.dashArray
          );
          this.renderGridLine(
            axis,
            index,
            minorGridDirection[1],
            minorTickLines,
            '_MinorTickLine_',
            i,
            parent,
            chartThemeStyle.minorTickLine
          );
          if (
            i === length - 1 &&
            isZoomSet(axis) &&
    ...
    if (
      ...
      for (let i: number = length; i < this.previousYLabel; i++) {
        const pointYValue: number =
          this.seriesClipRect.y +
          (axis.isInversed
            ? (this.seriesClipRect.height / length) * (i - length + 1) +
              this.seriesClipRect.height
            : -((this.seriesClipRect.height / length) * (i - length + 1)));
  ...
  private isBorder(axis: Axis, index: number, value: number): boolean {
    ...
    const startIndex: number =
      orientation === 'Horizontal' ? 0 : axis.visibleLabels.length - 1;
    const endIndex: number =
      orientation === 'Horizontal' ? axis.visibleLabels.length - 1 : 0;
  ...
  public drawYAxisLabels(
    ...
    const tickSpace: number =
      axis.labelPosition === axis.tickPosition ? axis.majorTickLines.height : 0;
    let padding: number = tickSpace + labelSpace + axis.lineStyle.width * 0.5;
    const angle: number = axis.angle % 360;
    const isVerticalAngle: boolean =
      angle === -90 || angle === 90 || angle === 270 || angle === -270;
    padding += isVerticalAngle ? (isLabelInside ? 5 : -5) : 0;
    padding = isOpposed ? padding : -padding;
    const labelElement: Element = chart.renderer.createGroup({
      id: chart.element.id + 'AxisLabels' + index,
    });
    const scrollBarHeight: number = isNullOrUndefined(axis.crossesAt)
      ? axis.scrollBarHeight * (isOpposed ? 1 : -1)
      : 0;
    ...
    let yAxisLabelX: number;
    const isLabelOnAxisLineLeft: boolean =
      (!isOpposed && !isLabelInside) || (isOpposed && isLabelInside);
    if (isLabelInside) {
      labelPadding = !isLabelOnAxisLineLeft ? -padding : padding;
    } else {
      labelPadding = !isLabelOnAxisLineLeft
        ? -padding + (chart.enableRtl ? -scrollBarHeight : scrollBarHeight)
        : padding + (chart.enableRtl ? -scrollBarHeight : scrollBarHeight);
    }
    const sizeWidth: number[] = [];
    const breakLabelSizeWidth: number[] = [];
    ...
    for (
      let i: number = 0, len: number = axis.visibleLabels.length;
      i < len;
      i++
    ) {
      label = axis.visibleLabels[i as number];
      isAxisBreakLabel = isBreakLabel(
        axis.visibleLabels[i as number].originalText
      );
      elementSize = isAxisBreakLabel
        ? axis.visibleLabels[i as number].breakLabelSize
        : axis.visibleLabels[i as number].size;
      pointY =
        valueToCoefficient(axis.visibleLabels[i as number].value, axis) *
          rect.height +
        (chart.stockChart ? 7 : 0);
      pointY = Math.floor(pointY * -1 + (rect.y + rect.height));
      textHeight =
        ((elementSize.height / 8) *
          axis.visibleLabels[i as number].text.length) /
        2;
      textPadding =
        chart.requireInvertedAxis && axis.labelPosition === 'Inside'
          ? 0
          : (elementSize.height / 4) * 3 + 3;
      intervalLength = rect.height / axis.visibleLabels.length;
      labelHeight =
        (axis.labelIntersectAction === 'Trim' ||
          axis.labelIntersectAction === 'Wrap') &&
        angle !== 0 &&
        elementSize.width > intervalLength
          ? intervalLength
          : elementSize.width;
      pointY = isAxisBreakLabel
        ? axis.labelPosition === 'Inside'
          ? pointY - elementSize.height / 2 - textHeight + textPadding
          : pointY - textHeight
        : axis.labelPosition === 'Inside'
        ? pointY + textPadding
        : pointY;
      if (
        axis.labelPosition === 'Inside' &&
        ((i === 0 && !axis.isInversed) || (i === len - 1 && axis.isInversed))
      ) {
        if (chart.stockChart) {
          pointY -= textPadding;
        } else {
      ...
      pointX = isOpposed
        ? axis.scrollBarHeight !== 0
          ? rect.x + axis.scrollBarHeight + padding - yAxisLabelX
          : rect.x - yAxisLabelX
        : rect.x + yAxisLabelX;
      if (isVerticalAngle) {
      ...
      options = new TextOption(
        chart.element.id + index + '_AxisLabel_' + i,
        pointX,
        pointY,
        'middle',
        label.text,
      ...
      switch (axis.edgeLabelPlacement) {
        ...
        case 'Hide':
          if (
            ((i === 0 || (isInverse && i === len - 1)) && options.y > rect.y) ||
            ((i === len - 1 || (isInverse && i === 0)) &&
              options.y - elementSize.height * 0.5 < rect.y)
          ) {
            options.text = '';
          }
          break;
        case 'Shift':
          if (
            (i === 0 || (isInverse && i === len - 1)) &&
            options.y > rect.y + rect.height
          ) {
      ...
      let previousYValue: number = options.y;
      let currentYValue: number = options.y - labelHeight;
      if (isAxisBreakLabel) {
      ...
      if (
        ...
        i !== 0 &&
        (!isInverse
          ? previousYValue >= previousEnd
          : currentYValue <= previousEnd)
      ) {
        continue;
      }
      ...
      textElement(
        ...
        options,
        label.labelStyle,
        label.labelStyle.color || chart.themeStyle.axisLabelFont.color,
        labelElement,
        false,
        chart.redraw,
        true,
        ...
        new ChartLocation(pointX, pointY)
      );
    }
    if (
      this.previousYLabel &&
      axis.visibleLabels.length &&
      ...
      for (
        let i: number = axis.visibleLabels.length;
        i < this.previousYLabel;
        i++
      ) {
        options.y =
          this.seriesClipRect.y +
          (axis.isInversed
            ? this.seriesClipRect.height +
              (this.seriesClipRect.height / axis.visibleLabels.length) *
  ...
  private rangeAnimate(
    ...
    if (element) {
      new Animation({}).animate(<HTMLElement>element, {
        duration: duration,
        progress: (): void => {
          const animateElement: HTMLElement = getElement(
            element.id
          ) as HTMLElement;
          ...
          if (label) {
            if (
              withInBounds(
                parseFloat(animateElement.getAttribute('x')),
                parseFloat(animateElement.getAttribute('y')),
                bounds
              )
            ) {
            ...
            const direction: string[] = animateElement
              .getAttribute('d')
              .split(' ');
            if (
              withInBounds(
                parseFloat(direction[1]),
                parseFloat(direction[2]),
                bounds
              )
            ) {
              animateElement.style.visibility = 'visible';
  ...
  private drawYAxisBorder(
    ...
    if (axis.border.width > 0) {
      ...
      let scrollBarHeight: number =
        axis.labelPosition === 'Outside' ? axis.scrollBarHeight : 0;
      const isOpposed: boolean = axis.isAxisOpposedPosition;
      scrollBarHeight = (isOpposed ? 1 : -1) * scrollBarHeight;
      const gap: number =
        (rect.height / axis.visibleRange.delta) *
      ...
      const length: number =
        ...
        10 +
        (axis.tickPosition === axis.labelPosition
          ? axis.majorTickLines.height
          : 0);
      ...
      const ticksbwtLabel: number =
        axis.valueType === 'Category' && axis.labelPlacement === 'BetweenTicks'
          ...
          : 0;
      const endX: number =
        (isOpposed && axis.labelPosition === 'Inside') ||
        (!isOpposed && axis.labelPosition === 'Outside')
          ? rect.x - length + scrollBarHeight
          : rect.x + length + scrollBarHeight;
      for (
        let i: number = 0, len: number = axis.visibleLabels.length;
        i < len;
    ...
    for (let i: number = 0; i < this.chart.visibleSeries.length; i++) {
      if (
        axis.multiLevelLabels.length > 0 &&
        this.chart.multiLevelLabelModule &&
        this.chart.visibleSeries[i as number].visible
      ) {
        this.chart.multiLevelLabelModule.renderYAxisMultiLevelLabels(
          axis,
          index,
          parent,
          rect
  ...
  private drawYAxisTitle(
    ...
    if (axis.title) {
      ...
      const labelRotation: number =
        (axis.titleRotation == null
          ? isOpposed
            ? 90
            : -90
          : axis.titleRotation) % 360;
      let padding: number =
        (axis.tickPosition === 'Inside'
          ? 0
          : axis.majorTickLines.height + axis.titlePadding) +
        (axis.labelPosition === 'Inside'
          ? 0
          : axis.maxLabelSize.width +
            axis.multiLevelLabelHeight +
            this.padding);

      padding = isOpposed
        ? padding + axis.scrollBarHeight
        : -padding - axis.scrollBarHeight;

      if (
        (labelRotation !== -90 && !isOpposed) ||
        (labelRotation !== 90 && isOpposed)
      ) {
      ...
      const titleSize: number =
        axis.titleSize.height * (axis.titleCollection.length - 1);
      const options: TextOption = new TextOption(
        chart.element.id + '_AxisTitle_' + index,
        x,
        y + (isRotated ? -titleSize : -axis.labelPadding - titleSize),
        anchor,
      ...
      const element: Element = textElement(
        ...
        options,
        axis.titleStyle,
        axis.titleStyle.color || chart.themeStyle.axisTitleFont.color,
        parent,
        null,
        null,
        null,
  ...
  private drawXAxisGridLine(
    ...
    const isTickInside: boolean = axis.tickPosition === 'Inside';
    const ticks: number = isTickInside
      ? rect.y - tickSize - axisLineSize
      : rect.y + tickSize + axisLineSize + scrollBarHeight;
    const chartThemeStyle: IThemeStyle = this.chart.themeStyle;
    if (
      axis.valueType.indexOf('Category') > -1 &&
      length > 0 &&
    ...
    const numericIDs: {
      [key: string]: { ids: number[]; isPointRemoved: boolean };
    } = this.calculateGridLineId(parent, length);
    for (let i: number = 0; i < length; i++) {
      ...
      if (pointX >= rect.x && rect.x + rect.width >= pointX) {
        if (
          ...
          if (
            this.chart.redraw &&
            ((this.chart.pointsAdded && !this.chart.pointsRemoved) ||
              this.chart.zoomRedraw) &&
            axis.majorGridLines.width &&
            i !== 0 &&
            axis.visible &&
            ...
            const pointXValue: number =
              this.seriesClipRect.x +
              (this.chart.enableRtl !== axis.isInversed
                ? -((this.seriesClipRect.width / length) * count)
                : this.seriesClipRect.width +
                  (this.seriesClipRect.width / (i ? i : 1)) * count);
            majorGrid =
              'M ' +
              pointXValue +
              ' ' +
              (this.seriesClipRect.y + this.seriesClipRect.height) +
              ' L ' +
              pointXValue +
              ' ' +
              this.seriesClipRect.y;
            this.renderGridLine(
              ...
              chartThemeStyle.majorGridLine,
              axis.majorGridLines.dashArray,
              numericIDs['MajorGridLine'] && numericIDs['MajorGridLine'].ids
                ? numericIDs['MajorGridLine'].ids[i as number]
                : null,
              numericIDs['MajorGridLine']
                ? numericIDs['MajorGridLine'].isPointRemoved
                : false
            );
            this.rangeAnimate(
              axis,
            ...
            (this.seriesClipRect.x +
              (this.chart.enableRtl !== axis.isInversed
                ...
                : this.seriesClipRect.width +
                  (this.seriesClipRect.width / (i ? i : 1)) * count)) +
            ' ' +
            (rect.y + axisLineSize + (isTickInside ? scrollBarHeight : 0)) +
            ' L ' +
            (this.seriesClipRect.x +
              (this.chart.enableRtl !== axis.isInversed
                ? -((this.seriesClipRect.width / length) * count)
        ...
        this.renderGridLine(
          ...
          chartThemeStyle.majorTickLine,
          null,
          numericIDs['MajorTickLine'] && numericIDs['MajorTickLine'].ids
            ? numericIDs['MajorTickLine'].ids[i as number]
            : null,
          numericIDs['MajorTickLine']
            ? numericIDs['MajorTickLine'].isPointRemoved
            : false
        );
        if (
          axis.minorTicksPerInterval > 0 &&
          ...
          this.renderGridLine(
            ...
            numericIDs['MinorGridLine'] && numericIDs['MinorGridLine'].ids
              ? numericIDs['MinorGridLine'].ids[i as number]
              : null,
            numericIDs['MinorGridLine']
              ? numericIDs['MinorGridLine'].isPointRemoved
              : false
          );
          this.renderGridLine(
            axis,
            ...
            chartThemeStyle.minorTickLine,
            null,
            numericIDs['MinorTickLine'] && numericIDs['MinorTickLine'].ids
              ? numericIDs['MinorTickLine'].ids[i as number]
              : null,
            numericIDs['MinorTickLine']
              ? numericIDs['MinorTickLine'].isPointRemoved
              : false
          );
          if (
            i === length - 1 &&
  ...
  private calculateGridLineId(
    ...
    if (this.chart.pointsRemoved) {
      ...
      elementTypes.forEach((type: string) => {
        ...
        const elements: NodeListOf<Element> = parentElement.querySelectorAll(
          '[id*="_' + type + '_"]'
        );
        ...
        let len: number =
          type === 'MajorGridLine' && this.chart.chartArea.border.width
            ? length - 2
            : length;
        len =
          type === 'MinorGridLine' || type === 'MinorTickLine'
            ? length - 1
            : len;
  ...
  private renderMinorGridOnZooming(
    ...
    i: number,
    index: number,
    chartThemeStyle: IThemeStyle,
    parent: Element
  ): void {
    const minorDirection: string[] = this.drawAxisMinorLine(
      axis,
      tempInterval,
      rect,
      i,
    ...
    if (axis.orientation === 'Horizontal') {
      for (let j: number = 0; j < axis.minorTicksPerInterval; j++) {
        ...
        if (inside(value, range)) {
          ...
          position = Math.ceil(
            (isInverse ? 1 - position : position) * rect.width
          );
          coor = Math.floor(position + rect.x);
          minorGird = minorGird.concat(
            'M' +
              ' ' +
              coor +
              ' ' +
      ...
      for (let j: number = 0; j < axis.minorTicksPerInterval; j++) {
        ...
        if (inside(value, range)) {
          position = (value - range.min) / (range.max - range.min);
          position =
            Math.ceil((isInverse ? 1 - position : position) * rect.height) * -1; // For inversed axis
  ...
  private findLogNumeric(
    ...
    if (axis.valueType === 'Logarithmic') {
      ...
      value +=
        ((axis.visibleLabels[labelIndex + 1]
          ? axis.visibleLabels[labelIndex + 1].value - padding
          : axis.visibleRange.max) -
          (axis.visibleLabels[labelIndex as number]
            ? axis.visibleLabels[labelIndex as number].value - padding
  ...
  public drawXAxisLabels(
    ...
    let padding: number = tickSpace + labelSpace + axis.lineStyle.width * 0.5;
    const angle: number = axis.angle % 360;
    const isHorizontalAngle: boolean =
    ...
    const scrollBarHeight: number =
      axis.scrollbarSettings.enable ||
      (!islabelInside &&
        isNullOrUndefined(axis.crossesAt) &&
        (axis.zoomFactor < 1 || axis.zoomPosition > 0))
        ? axis.scrollBarHeight
        : 0;
    const newPoints: ChartLocation[][] = [];
    let isRotatedLabelIntersect: boolean = false;
    ...
    padding +=
      angle === 90 || angle === 270 || angle === -90 || angle === -270
        ? islabelInside
          ...
          : -5
        : 0;
    const isLabelUnderAxisLine: boolean =
      (!isOpposed && !islabelInside) || (isOpposed && islabelInside);
    const axislabelElement: Element = getElement(
      chart.element.id + 'AxisLabels' + index
    );
    const pointsRemoved: boolean = this.removeAxisLabelElements(
      axis,
      axislabelElement
    );
    ...
    for (let i: number = 0, len: number = length; i < len; i++) {
      label = axis.visibleLabels[i as number];
      isAxisBreakLabel =
        isBreakLabel(label.originalText) ||
        (axis.labelIntersectAction === 'Wrap' && label.text.length > 1);
      pointX = valueToCoefficient(label.value, axis) * rect.width + rect.x;
      elementSize = label.size;
      intervalLength = rect.width / length;
      labelWidth = isAxisBreakLabel
        ? label.breakLabelSize.width
        : elementSize.width;
      width =
        (axis.labelIntersectAction === 'Trim' ||
          axis.labelIntersectAction === 'Wrap') &&
        angle === 0 &&
        labelWidth > intervalLength
          ? intervalLength
          : labelWidth;
      ...
      if (angle !== 0) {
        if (isAxisBreakLabel) {
          pointX -=
            axis.lineBreakAlignment === 'Left'
              ? label.breakLabelSize.width / 2
              : axis.lineBreakAlignment === 'Right'
              ? -(label.breakLabelSize.width / 2)
              : 0;
        } else {
          pointX -=
      ...
      if (
        isAxisBreakLabel &&
        axis.lineBreakAlignment !== 'Center' &&
        angle === 0
      ) {
      ...
      const paddingForBreakLabel: number = isAxisBreakLabel
        ? isHorizontalAngle
          ? axis.opposedPosition || islabelInside
            ? 0
            : elementSize.height
          : label.breakLabelSize.width / 2
        : 0;
      padding = isAxisBreakLabel
        ? tickSpace + labelSpace + axis.lineStyle.width * 0.5
        : padding;
      ...
      if (islabelInside && angle) {
        if (isAxisBreakLabel) {
          pointY = isOpposed
            ? rect.y + padding + paddingForBreakLabel
            : rect.y - padding - paddingForBreakLabel;
        } else {
          pointY = isOpposed
        ...
        if (isAxisBreakLabel) {
          labelPadding = !isLabelUnderAxisLine
            ? -(padding + scrollBarHeight + paddingForBreakLabel)
            : padding +
              scrollBarHeight +
              (angle ? paddingForBreakLabel : 3 * labelHeight);
        } else {
          labelPadding = !isLabelUnderAxisLine
            ? -(
                padding +
                scrollBarHeight +
                (angle ? labelHeight : label.index > 1 ? 2 * labelHeight : 0)
              )
      ...
      if (isAxisBreakLabel) {
        ...
        anchor = chart.enableRtl
          ? isEndAnchor
            ? ''
            : 'end'
          : chart.isRtlEnabled || isEndAnchor
          ? 'end'
          : '';
      }
      options = new TextOption(
      ...
      options.id =
        pointsRemoved &&
        axislabelElement &&
        axislabelElement.children[i as number]
          ? axislabelElement.children[i as number].id
          : options.id;
      if (angle !== 0) {
        rotatedLabelSize = rotateTextSize(
          label.labelStyle,
          label.originalText,
          angle,
          chart,
          ...
          case 'Hide':
            if (
              ((i === 0 || (isInverse && i === len - 1)) &&
                options.x < rect.x) ||
              ((i === len - 1 || (isInverse && i === 0)) &&
                options.x + (angle === 0 ? width : rotatedLabelSize.width) >
                  rect.x + rect.width)
            ) {
              continue;
            }
          ...
          case 'Shift':
            if (
              ...
              (options.x < rect.x ||
                (angle !== 0 &&
                  isLeft &&
                  options.x - rotatedLabelSize.width <= rect.x) ||
                (axis.lineBreakAlignment === 'Center' &&
                  options.x - label.size.width / label.text.length / 2 <
                    rect.x &&
                  angle === 0))
            ) {
              ...
              if (anchor === '' && !isInverse && angle === 0) {
                ...
                intervalLength = rect.width / length;
                if (
                  rect.x + intervalLength >
                  options.x + rotatedLabelSize.width
                ) {
                ...
                options.x = pointX = !isHorizontalAngle
                  ? rect.x + padding
                  : rect.x;
              }
            } else if (
              (i === len - 1 || (isInverse && i === 0)) &&
              ((options.x + width > rect.x + rect.width &&
                anchor !== 'end' &&
                angle === 0) ||
                (angle !== 0 &&
                  !isLeft &&
                  options.x + rotatedLabelSize.width > rect.x + rect.width) ||
                (anchor === 'end' && options.x > rect.x + rect.width))
            ) {
              if (
                elementSize.width > intervalLength &&
                axis.labelIntersectAction === 'Trim' &&
              ...
              if (angle !== 0) {
                options.x = pointX =
                  rect.x + rect.width - intervalLength + padding;
              } else {
                options.x = pointX = !isHorizontalAngle
                  ? rect.x + rect.width - intervalLength / 2
      ...
      if (angle !== 0) {
        ...
        let rect: Rect;
        if (isAxisBreakLabel) {
          let xAdjustment: number = 0;
          let yAdjustment: number = 0;
          ...
          if (isLabelUnderAxisLine) {
            yAdjustment =
              label.breakLabelSize.height / (options.text.length + 1);
          }
          rect = new Rect(
            options.x + xAdjustment,
            options.y - yAdjustment,
            label.breakLabelSize.width,
            height
          );
        } else {
          ...
          rect = new Rect(
            options.x,
            options.y - (label.size.height / 2 - 5),
            label.size.width,
            height
          );
        }
        const rectCoordinates: ChartLocation[] = this.getRectanglePoints(rect);
        const rectCenterX: number = isAxisBreakLabel
          ? rect.x + rect.width / 2
          : pointX;
        const rectCenterY: number = isAxisBreakLabel
          ? rect.y + rect.height / 2
          : pointY - height / 2;
        if (isAxisBreakLabel) {
          options.transform =
            'rotate(' + angle + ',' + rectCenterX + ',' + rectCenterY + ')';
        } else {
          options.transform =
        ...
        if (axis.labelIntersectAction !== 'None') {
          for (let index: number = i; index > 0; index--) {
            if (
              ...
              newPoints[i as number] = null;
              break;
            }
          }
        }
        const rotateAngle: boolean =
          (angle > 0 && angle < 90) ||
          (angle > 180 && angle < 270) ||
          (angle < -90 && angle > -180) ||
          (angle < -270 && angle > -360);
        const textRect: Rect = new Rect(
          options.x,
          options.y - (elementSize.height / 2 + padding / 2),
          label.size.width,
          height
        );
        const textRectCoordinates: ChartLocation[] =
          this.getRectanglePoints(textRect);
        const rectPoints: rectPoints[] = [];
        rectPoints.push(
          new ChartLocation(
            rotateAngle ? this.chart.availableSize.width : this.padding,
            axis.rect.y
          )
        );
        rectPoints.push(
          new ChartLocation(
            rotateAngle ? this.chart.availableSize.width : this.padding,
            axis.rect.y + axis.maxLabelSize.height
          )
        );
        textPoints.push(
          getRotatedRectangleCoordinates(
        ...
        const newRect: Rect = new Rect(
          0,
          axis.rect.y,
          this.chart.availableSize.width,
          axis.maxLabelSize.height * 2
        );
        for (let k: number = 0; k < textPoints[i as number].length; k++) {
          if (
            !axis.opposedPosition &&
            !withInBounds(
              textPoints[i as number][k as number].x,
              textPoints[i as number][k as number].y,
              newRect
            ) &&
            typeof options.text === 'string'
          ) {
            const interSectPoint: ChartLocation = this.calculateIntersection(
              textPoints[i as number][0],
              textPoints[i as number][1],
              rectPoints[0],
              rectPoints[1]
            );
            const rectPoint1: number = rotateAngle
              ? this.chart.availableSize.width - pointX
              : pointX;
            const rectPoint2: number = interSectPoint.y - axis.rect.y;
            const trimValue: number = Math.sqrt(
              rectPoint1 * rectPoint1 + rectPoint2 * rectPoint2
            );
            options.text = textTrim(
              trimValue,
      ...
      if (
        this.chart.redraw &&
        ((chart.pointsAdded && !chart.pointsRemoved) ||
          this.chart.zoomRedraw) &&
        !getElement(options.id) &&
        i !== 0 &&
        !this.chart.enableCanvas &&
        ...
        options.x =
          this.seriesClipRect.x +
          (chart.enableRtl !== axis.isInversed
            ? -((this.seriesClipRect.width / (i ? i : 1)) * count)
            : this.seriesClipRect.width +
              (this.seriesClipRect.width / (i ? i : 1)) * count);
        ...
        options.transform = angle
          ? 'rotate(' +
            ...
            options.x +
            ',' +
            parseFloat(options.transform.split(',')[2]) +
            ')'
          : '';
        const element: Element = textElement(
          chart.renderer,
          ...
          label.labelStyle.color || chart.themeStyle.axisLabelFont.color,
          labelElement,
          axis.isAxisOpposedPosition !== (axis.labelPosition === 'Inside'),
          chart.redraw,
          true,
          null,
          chart.duration,
      ...
      const element: Element = textElement(
        ...
        chart.themeStyle.axisLabelFont,
        new ChartLocation(
          parseFloat(options.transform.split(',')[1]),
          parseFloat(options.transform.split(',')[2])
        )
      );
      if (pointsRemoved) {
  ...
  private updateAxisElement(
    ...
    if (isLabel) {
      element = textElement(
        ...
        option,
        label.labelStyle,
        label.labelStyle.color || this.chart.themeStyle.axisLabelFont.color,
        parentElement,
        axis.isAxisOpposedPosition !== (axis.labelPosition === 'Inside'),
        this.chart.redraw,
        true,
      ...
      this.renderGridLine(
        ...
        index,
        gridDirection,
        isGrid ? axis.majorGridLines : axis.majorTickLines,
        elementId,
        i,
        this.element,
        isGrid
          ? this.chart.themeStyle.majorGridLine
          : this.chart.themeStyle.majorTickLine,
        isGrid ? axis.majorGridLines.dashArray : null
      );
      if (this.element && parentElement.childNodes) {
        element = this.element.childNodes[
    ...
    this.rangeAnimate(
      ...
      isLabel,
      new Rect(
        axis.orientation === 'Vertical' ? 0 : this.seriesClipRect.x,
        axis.orientation === 'Vertical' ? this.seriesClipRect.y : axis.rect.y,
        this.seriesClipRect.width,
        this.seriesClipRect.height
      ),
      isRemove,
  ...
  private removeAxisLabelElements(
    axis: Axis,
    axislabelElement: Element
  ): boolean {
    if (
      this.chart.pointsRemoved &&
      axislabelElement &&
      axislabelElement.childNodes.length
    ) {
      ...
      if (axis.valueType.indexOf('Category') > -1) {
        ...
        axis.visibleLabels.map((label: VisibleLabels) => {
          if (typeof label.text !== 'string') {
            ...
            visibleLabelTexts.push(label.text);
          }
        });
        for (
          let i: number = axislabelElement.childNodes.length - 1;
          i >= 0;
          i--
        ) {
        ...
        axislabelElement.childNodes.length > axis.visibleLabels.length
      ) {
        for (
          let j: number = 0;
          j < axislabelElement.childNodes.length - axis.visibleLabels.length;
          j++
        ) {
          axislabelElement.removeChild(
            axislabelElement.childNodes[j as number]
          );
          pointsRemoved = true;
        }
      }
  ...
  private getAnchor(axis: Axis): string {
    return axis.lineBreakAlignment === 'Center'
      ? 'middle'
  ...
  private getRectanglePoints(rect: Rect): ChartLocation[] {
    ...
    const point2: ChartLocation = new ChartLocation(
      rect.x + rect.width,
      rect.y
    );
    const point3: ChartLocation = new ChartLocation(
      rect.x + rect.width,
      rect.y + rect.height
    );
    const point4: ChartLocation = new ChartLocation(
      rect.x,
      rect.y + rect.height
    );
  ...
  private getLabelText(
    ...
    if (isBreakLabel(label.originalText)) {
      ...
      for (let index: number = 0; index < label.text.length; index++) {
        str = this.findAxisLabel(
          axis,
          <string>label.text[index as number],
          intervalLength
        );
        result.push(str);
      }
  ...
  private drawXAxisBorder(
    ...
    if (axis.border.width > 0) {
      ...
      let startX: number;
      const startY: number =
        axisRect.y + (isOpposed ? -1 : 1) * scrollBarHeight;
      const padding: number = 10;
      let pointX: number;
      const gap: number =
        (axisRect.width / axis.visibleRange.delta) *
        (axis.valueType === 'DateTime'
          ? axis.dateTimeInterval
          : axis.visibleRange.interval);
      let endX: number;
      const length: number =
        axis.maxLabelSize.height +
        (axis.tickPosition === axis.labelPosition
          ? axis.majorTickLines.height
          : 0);
      let labelBorder: string = '';
      ...
      const endY: number =
        (isOpposed && axis.labelPosition === 'Inside') ||
        (!isOpposed && axis.labelPosition === 'Outside')
          ? axisRect.y + length + padding + scrollBarHeight
          : axisRect.y - length - padding - scrollBarHeight;
      for (
        let i: number = 0, len: number = axis.visibleLabels.length;
        i < len;
        ...
        switch (axis.border.type) {
          ...
          case 'WithoutTopBorder':
            if (startX < axisRect.x && axis.labelPlacement !== 'OnTicks') {
              ...
              Math.floor(endX) > axisRect.width + axisRect.x &&
              ...
              labelBorder +=
                ...
                'L' +
                ' ' +
                (axisRect.width + axisRect.x) +
                ' ' +
                endY +
                ' ';
            } else {
              startX =
                i === 0 && axis.labelPlacement === 'OnTicks'
                  ? axisRect.x
                  : startX;
              endX =
                i === axis.visibleLabels.length - 1 &&
    ...
    for (let i: number = 0; i < this.chart.visibleSeries.length; i++) {
      if (
        ...
        this.chart.multiLevelLabelModule.renderXAxisMultiLevelLabels(
          axis,
          index,
          parent,
          axisRect
  ...
  private createAxisBorderElement(
    ...
    const element: Element = getElement(
      this.chart.element.id + '_BorderLine_' + index
    );
    const direction: string = element ? element.getAttribute('d') : '';
    const borderElement: Element = this.chart.renderer.drawPath(
      new PathOption(
        this.chart.element.id + '_BorderLine_' + index,
        'transparent',
        axis.border.width,
        axis.border.color || this.chart.themeStyle.axisLine,
        1,
        '',
        labelBorder
      )
  ...
  private findAxisLabel(axis: Axis, label: string, width: number): string {
    return axis.labelIntersectAction === 'Trim'
      ? axis.angle % 360 === 0 && !axis.enableTrim
        ? textTrim(
            width,
            label,
            axis.labelStyle,
  ...
  private drawXAxisTitle(
    ...
    if (axis.title) {
      ...
      const elementSize: Size = measureText(
        axis.title,
        axis.titleStyle,
        this.chart.themeStyle.axisTitleFont
      );
      const scrollBarHeight: number = isNullOrUndefined(axis.crossesAt)
        ? axis.scrollBarHeight
        : 0;
    ...
    if (
      ...
      element = getElement(
        chart.element.id +
          ...
          index +
          '_' +
          (isRemoved ? removeIndex : gridIndex)
      );
      direction = element ? element.getAttribute('d') : null;
      element = null;
      this.htmlObject = chart.renderer.drawPath(
        new PathOption(
          chart.element.id +
            gridId +
            index +
      ...
      if (isRemoved) {
        this.htmlObject.id =
          chart.element.id + gridId + index + '_' + gridIndex;
      }
    }
  }
  ...
  private findParentNode(
    ...
    if (
      document
        .getElementById(elementId + 'AxisGroup' + index + 'Inside')
        .contains(document.getElementById(label.id))
    ) {
      return document.getElementById(
        elementId + 'AxisGroup' + index + 'Inside'
      );
    } else {
      return document.getElementById(
        elementId + 'AxisGroup' + index + 'Outside'
      );
    }
  }
  ...
  private createZoomingLabel(
    ...
    const parentNode: Element = this.findParentNode(
      chart.element.id,
      labelElement,
      index
    );
    ...
    zoomElement = createZoomingLabels(
      chart,
      axis,
      zoomElement,
      index,
    ...
    parentNode.replaceChild(
      labelElement,
      document.getElementById(labelElement.id)
    );
    if (getElement(chart.element.id + 'AxisLabels_Zoom' + index)) {
      parentNode.replaceChild(
        zoomElement,
        document.getElementById(zoomElement.id)
      );
    } else {
