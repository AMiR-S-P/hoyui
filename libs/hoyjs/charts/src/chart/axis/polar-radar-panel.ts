...
export class PolarRadarPanel extends LineBase {
  ...
  private calculateAxisSize(): void {
    ...
    chart.radius =
      Math.min(this.initialClipRect.width, this.initialClipRect.height) / 2 -
      padding -
      (<Axis>chart.primaryXAxis).majorTickLines.height -
      (<Axis>chart.primaryXAxis).maxLabelSize.height;
    chart.radius =
      ((<Axis>chart.primaryXAxis).coefficient * chart.radius) / 100;
  ...
  private drawYAxisLine(axis: Axis, index: number): void {
    ...
    const vector: ChartLocation = CoefficientToVector(
    ...
    const axisLine: string =
      ...
      this.centerY +
      'L ' +
      (this.centerX + chart.radius * vector.x) +
      ' ' +
      (this.centerY + chart.radius * vector.y);
    optionsLine = {
  ...
  public drawYAxisLabels(axis: Axis, index: number): void {
    ...
    let pointY: number = 0;
    const angle: number =
      this.startAngle < 0 ? this.startAngle + 360 : this.startAngle;
    ...
    for (
      ...
      pointX =
        this.centerX +
        radius * vector.x +
        (axis.majorTickLines.height + elementSize.width / 2 + padding / 2) *
          Math.cos((angle * Math.PI) / 180) *
          (axis.labelPosition === 'Inside' && chart.enableRtl
            ? -1
            : axis.labelPosition === 'Inside'
            ? 1
            : axis.labelPosition === 'Outside' && chart.enableRtl
            ? 1
            : -1);
      pointY =
        this.centerY +
        radius * vector.y +
        (axis.majorTickLines.height + elementSize.height / 2) *
      ...
      labelRegions[i as number] = this.getLabelRegion(
        pointX,
        pointY,
      ...
      if (i !== 0 && intersectType === 'Hide') {
        for (let j: number = i; j >= 0; j--) {
          j = j === 0 ? 0 : j === i ? j - 1 : j;
          if (
            isLabelVisible[j as number] &&
            isOverlap(labelRegions[i as number], labelRegions[j as number])
          ) {
            isIntersect = true;
      ...
      options = new TextOption(
        ...
        pointY,
        anchor,
        axis.visibleLabels[i as number].text
      );
      ...
      textElement(
        ...
        options,
        axis.labelStyle,
        axis.labelStyle.color || chart.themeStyle.axisLabelFont.color,
        labelElement,
        false,
        chart.redraw,
        true,
    ...
    if (axis.majorGridLines.width > 0) {
      if (chart.visibleSeries[0].type === 'Polar') {
        for (let j: number = 0; j < axis.visibleLabels.length; j++) {
          element = getElement(
            chart.element.id + '_MajorGridLine_' + index + '_' + j
          );
          previousValue = element ? element.getAttribute('r') : null;
          radius =
            chart.radius *
            valueToCoefficient(axis.visibleLabels[j as number].value, axis);
          ...
          appendChildElement(
            chart.enableCanvas,
            this.element,
            chart.renderer.drawCircle(options),
            chart.redraw,
            true,
            'r',
            'r',
            new ChartLocation(+previousValue, +previousValue),
            null,
            true
          );
        }
        if (
          radius !== chart.radius &&
          (!axis.isInversed || inversedRadius !== chart.radius)
        ) {
          options = new CircleOption(
            chart.element.id +
              '_MajorGridLine_' +
        ...
        for (let j: number = 0; j < axis.visibleLabels.length; j++) {
          ...
          appendChildElement(
            chart.enableCanvas,
            this.element,
            chart.renderer.drawPath(options),
            chart.redraw,
            true,
            'x',
            'y',
      ...
      for (let i: number = 0; i < axis.visibleLabels.length; i++) {
        ...
        x2 =
          ...
          axis.majorTickLines.height *
            Math.cos((angle * Math.PI) / 180) *
            (axis.tickPosition === 'Inside' && chart.enableRtl
              ? -1
              : axis.tickPosition === 'Inside'
              ? 1
              : axis.tickPosition === 'Outside' && chart.enableRtl
              ? 1
              : -1);
        y2 =
          y1 +
  ...
  private renderRadarGrid(
    ...
    for (
      let i: number = 0,
        len: number = (<Axis>chart.primaryXAxis).visibleLabels.length;
      i < len;
      i++
    ) {
      vector = CoefficientToVector(
        valueToPolarCoefficient(
          (<Axis>chart.primaryXAxis).visibleLabels[i as number].value,
          <Axis>chart.primaryXAxis
        ),
        this.startAngle
      );
      if (i + 1 < len) {
        vector2 = CoefficientToVector(
          valueToPolarCoefficient(
            (<Axis>chart.primaryXAxis).visibleLabels[i + 1].value,
            <Axis>chart.primaryXAxis
          ),
          this.startAngle
        );
        ...
        vector2 = CoefficientToVector(
          valueToPolarCoefficient(
            (<Axis>chart.primaryXAxis).visibleLabels[0].value,
            <Axis>chart.primaryXAxis
          ),
          this.startAngle
        );
  ...
  private drawXAxisGridLine(axis: Axis, index: number): void {
    ...
    for (let i: number = 0; i < length; i++) {
      ...
      vector = CoefficientToVector(
        valueToPolarCoefficient(axis.visibleLabels[i as number].value, axis),
        this.startAngle
      );
      ...
      if (
        ...
        chart.visibleSeries[0].type !== 'Radar'
      ) {
        minorDirection = this.drawAxisMinorLine(
          axis,
          tempInterval,
          minorGirdLine,
          minorTickLine
  ...
  public drawXAxisLabels(axis: Axis, index: number): void {
    ...
    for (
      ...
      if (!isNaN(vector.x) && !isNaN(vector.y)) {
        pointX =
          this.centerX +
          (radius + axis.majorTickLines.height + padding) * vector.x;
        pointY =
          this.centerY +
          (radius + axis.majorTickLines.height + padding) * vector.y;
        textAnchor =
          parseFloat(pointX.toFixed(1)) === parseFloat(this.centerX.toFixed(1))
            ? 'middle'
            : (pointX < this.centerX && !islabelInside) ||
              (pointX > this.centerX && islabelInside)
            ? 'end'
            : 'start';
      }
      ...
      if (axis.enableTrim || intersectType === 'Trim') {
        ...
        for (let i: number = textLength - 1; i >= 0; --i) {
          ...
          if (
            pointX === chartWidth / 2
              ? pointX - size / 2 >= 0 && pointX + size / 2 <= chartWidth
              : (labelPosition === 'Outside' &&
                  ((pointX >= chartWidth / 2 && pointX + size <= chartWidth) ||
                    (pointX <= chartWidth / 2 && pointX - size >= 0))) ||
                (labelPosition === 'Inside' &&
                  (pointX + size <= chartWidth / 2 ||
                    pointX - size >= chartWidth / 2))
          ) {
            labelText = i === textLength - 1 ? originalText : trimText;
            label.size.width = measureText(
              labelText,
              axis.labelStyle,
              this.chart.themeStyle.axisLabelFont
            ).width;
      ...
      if (!isIntersect && legendRect) {
        ...
        if (isIntersect) {
          const width: number = this.getAvailableSpaceToTrim(
            legendRect,
            labelRegions[i as number]
          );
          if (width > 0) {
            labelText = textTrim(
              width,
  ...
  private renderTickLine(
    ...
    if (axis.majorTickLines.width > 0) {
      element = getElement(
        chart.element.id + '_MajorTickLine_' + index + '_' + gridIndex
      );
      ...
      tickOptions = new PathOption(
        ...
        axis.majorTickLines.color || chart.themeStyle.majorTickLine,
        null,
        null,
        majorTickLine
      );
    ...
    if (axis.minorTickLines.width > 0) {
      element = getElement(
        chart.element.id + '_MinorTickLine_' + index + '_' + gridIndex
      );
      direction = element ? element.getAttribute('d') : null;
      tickOptions = new PathOption(
        chart.element.id + '_MinorTickLine_' + index + '_' + gridIndex,
        ...
        axis.minorTickLines.color || chart.themeStyle.minorTickLine,
        null,
        null,
        minorTickLine
      );
  ...
  private renderGridLine(
    ...
    if (axis.majorGridLines.width > 0) {
      element = getElement(
        chart.element.id + '_MajorGridLine_' + index + '_' + gridIndex
      );
      direction = element ? element.getAttribute('d') : null;
      ...
      appendChildElement(
        chart.enableCanvas,
        this.element,
        chart.renderer.drawPath(gridOptions),
        chart.redraw,
        true,
        'x',
        'y',
    ...
    if (axis.minorGridLines.width > 0) {
      element = getElement(
        chart.element.id + '_MinorGridLine_' + index + '_' + gridIndex
      );
      direction = element ? element.getAttribute('d') : null;
      gridOptions = new PathOption(
        chart.element.id + '_MinorGridLine_' + index + '_' + gridIndex,
