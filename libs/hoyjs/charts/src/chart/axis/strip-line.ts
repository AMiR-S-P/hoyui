...
export class StripLine {
  ...
  private measureStripLine(
    axis: Axis,
    stripline: StripLineSettingsModel,
    seriesClipRect: Rect,
    startValue: number,
    segmentAxis: Axis,
    chart: Chart
      ...
      if (axis.valueType === 'DateTimeCategory') {
        ...
        actualStart =
          start != null && typeof start !== 'number'
            ? axis.labels.indexOf(
                this.dateToMilliSeconds(start, chart).toString()
              )
            : (start as number);
        actualEnd =
          end != null && typeof end !== 'number'
            ? axis.labels.indexOf(
                this.dateToMilliSeconds(end, chart).toString()
              )
            : (end as number);
      } else {
        actualStart =
          stripline.start === null
            ? null
            : isDateTimeAxis && this.isCoreDate(stripline.start)
            ? this.dateToMilliSeconds(stripline.start, chart)
            : +stripline.start;
        actualEnd =
          stripline.end === null
            ? null
            : isDateTimeAxis && this.isCoreDate(stripline.start)
            ? this.dateToMilliSeconds(stripline.end, chart)
            : +stripline.end;
      }
    }
    const rect: { from: number; to: number } = this.getFromTovalue(
      ...
      actualEnd,
      stripline.size,
      stripline.startFromAxis,
      axis,
      stripline
    );
    let height: number =
      orientation === 'Vertical'
        ? (rect.to - rect.from) * axis.rect.height
        : seriesClipRect.height;
    let width: number =
      orientation === 'Horizontal'
        ? (rect.to - rect.from) * axis.rect.width
        : seriesClipRect.width;
    let x: number =
      orientation === 'Vertical'
        ? seriesClipRect.x
        : rect.from * axis.rect.width + axis.rect.x;
    let y: number =
      orientation === 'Horizontal'
        ? seriesClipRect.y
        : axis.rect.y +
          axis.rect.height -
          (stripline.sizeType === 'Pixel' ? rect.from : rect.to) *
            axis.rect.height;
    if (
      stripline.isSegmented &&
      stripline.segmentStart != null &&
      stripline.segmentEnd != null &&
      stripline.sizeType !== 'Pixel'
    ) {
      const start: number =
        isDateTimeAxis && this.isCoreDate(stripline.segmentStart)
          ? this.dateToMilliSeconds(stripline.segmentStart, chart)
          : +stripline.segmentStart;
      const end: number =
        isDateTimeAxis && this.isCoreDate(stripline.segmentEnd)
          ? this.dateToMilliSeconds(stripline.segmentEnd, chart)
          : +stripline.segmentEnd;
      const segRect: { from: number; to: number } = this.getFromTovalue(
        start,
        end,
        null,
        null,
    ...
    if (
      (height !== 0 && width !== 0) ||
      (stripline.sizeType === 'Pixel' &&
        (stripline.start !== null || stripline.startFromAxis))
    ) {
  ...
  private getFromTovalue(
    ...
    let from: number =
      !stripline.isRepeat && startFromAxis ? axis.visibleRange.min : start;
    if (
      axis.valueType === 'Double' &&
      size !== null &&
      !startFromAxis &&
      stripline.start == null
    ) {
    ...
    let to: number = this.getToValue(
      Math.max(start, isNullOrUndefined(end) ? start : end),
      from,
      size,
      axis,
      end,
    ...
    if (axis.valueType === 'DateTime') {
      ...
      switch (sizeType) {
        case 'Years':
          return <number>(
            (isEnd
              ? new Date(fromValue.setFullYear(fromValue.getFullYear() + size))
              : to)
          );
        case 'Months':
          return <number>(
            (isEnd
              ? new Date(fromValue.setMonth(fromValue.getMonth() + size))
              : to)
          );
        case 'Days':
          return <number>(
            (isEnd
              ? new Date(fromValue.setDate(fromValue.getDate() + size))
              : to)
          );
        case 'Hours':
          return <number>(
            (isEnd
              ? new Date(fromValue.setHours(fromValue.getHours() + size))
              : to)
          );
        case 'Minutes':
          return <number>(
            (isEnd
              ? new Date(fromValue.setMinutes(fromValue.getMinutes() + size))
              : to)
          );
        case 'Seconds':
          return <number>(
            (isEnd
              ? new Date(fromValue.setSeconds(fromValue.getSeconds() + size))
              : to)
          );
        default:
          return from;
  ...
  private dateParse(value: Date | Object, chart: Chart): Date {
    const dateParser: Function = chart.intl.getDateParser({
      skeleton: 'full',
      type: 'dateTime',
    });
    const dateFormatter: Function = chart.intl.getDateFormat({
      skeleton: 'full',
      type: 'dateTime',
    });
    return new Date(
      Date.parse(
        dateParser(
          dateFormatter(new Date(DataUtil.parse.parseJson({ val: value }).val))
        )
      )
    );
  }
    ...
    for (const axis of axes) {
      ...
      for (const stripline of axis.stripLines) {
        if (stripline.visible && stripline.zIndex === position) {
          ...
          if (
            stripline.isRepeat &&
            stripline.repeatEvery != null &&
            stripline.size !== null &&
            stripline.sizeType !== 'Pixel'
          ) {
            limit =
              stripline.repeatUntil != null
                ? axis.valueType === 'DateTime'
                  ? this.dateToMilliSeconds(stripline.repeatUntil, chart)
                  : +stripline.repeatUntil
                : axis.actualRange.max;
            startValue =
              axis.valueType === 'DateTime' && this.isCoreDate(stripline.start)
                ? this.dateToMilliSeconds(stripline.start, chart)
                : (stripline.start as number);
            if (
              (stripline.startFromAxis &&
                axis.valueType === 'DateTime' &&
                stripline.sizeType === 'Auto') ||
              stripline.start < axis.visibleRange.min
            ) {
              startValue =
                axis.visibleLabels[0] &&
                axis.visibleLabels[0].value === axis.visibleRange.min
            ...
            startValue =
              stripline.startFromAxis && axis.valueType !== 'DateTime'
                ? axis.visibleRange.min
                : startValue;
            while (startValue < limit) {
                ...
                startValue +
                (axis.valueType === 'DateTime'
                  ? axis.dateTimeInterval * +stripline.size
                  : stripline.size);
              range = withIn(end, axis.visibleRange);
              if (
                (startValue >= axis.visibleRange.min &&
                  startValue < axis.visibleRange.max) ||
                range
              ) {
  ...
  private drawImage(
    ...
    const htmlObject: HTMLElement = chart.renderer.drawImage(
      image
    ) as HTMLElement;
    appendChildElement(
      chart.enableCanvas,
  ...
  private renderText(
    ...
    const rotation: number = isRotationNull
      ? axis.orientation === 'Vertical'
        ? 0
        : -90
      : stripline.rotation;
    ...
    if (axis.orientation === 'Horizontal') {
      tx = this.getTextStart(
        ...
        rect.width,
        stripline.horizontalAlignment
      );
      ty =
        this.getTextStart(
          ty - textMid,
          rect.height,
          stripline.verticalAlignment
        ) +
        (stripline.verticalAlignment === 'Start' && !isRotationNull
          ? textSize.height / 4
          : 0);
      anchor = isRotationNull
        ? this.invertAlignment(stripline.verticalAlignment)
        : stripline.horizontalAlignment;
      anchor =
        tx - textSize.width / 2 < axis.rect.x
          ? 'Start'
          : tx + textSize.width / 2 > axis.rect.width
          ? 'End'
          : anchor;
    } else {
    ...
    textElement(
      ...
      new TextOption(
        ...
        anchor,
        stripline.text,
        'rotate(' + rotation + ' ' + tx + ',' + ty + ')',
        'middle'
      ),
      stripline.textStyle,
      stripline.textStyle.color || chart.themeStyle.stripLineLabelFont.color,
      parent,
      null,
      null,
      null,
  ...
  private renderStripLineElement(
    ...
    seriesClipRect: Rect,
    id: string,
    striplineGroup: Element,
    chart: Chart,
    startValue: number,
    segmentAxis: Axis,
    count: number
  ): void {
    const rect: Rect = this.measureStripLine(
      axis,
      stripline,
      seriesClipRect,
      startValue,
    ...
    if (stripline.imageUrl) {
      ...
      this.renderPath(
        ...
        rect,
        id +
          (stripline.sizeType === 'Pixel' ? 'path_' : 'rect_') +
          axis.name +
          '_' +
          count,
        striplineGroup,
      ...
      const pixelRect: Rect = new Rect(
        axis.orientation === 'Horizontal'
          ? rect.x - stripline.size / 2
          : rect.x,
        axis.orientation === 'Vertical' ? rect.y - stripline.size / 2 : rect.y,
        rect.width ? rect.width : stripline.size,
        rect.height ? rect.height : stripline.size
      );
      this.renderRectangle(
        stripline,
        stripline.sizeType === 'Pixel' ? pixelRect : rect,
        id + 'border_' + axis.name + '_' + count,
        striplineGroup,
        chart
      );
    }
