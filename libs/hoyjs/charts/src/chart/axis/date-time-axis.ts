...
import { Size } from '@hoyui/svg-base';
...
import { extend, getValue, isNullOrUndefined } from '@hoyui/base';
...
export class DateTime extends NiceInterval {
  ...
  public getActualRange(axis: Axis, size: Size): void {
    ...
    if (axis.minimum !== null) {
      this.min = this.chart.isBlazor
        ? Date.parse(axis.minimum.toString())
        : Date.parse(
            dateParser(
              dateFormatter(
                new Date(DataUtil.parse.parseJson({ val: axis.minimum }).val)
              )
            )
          );
    ...
    if (axis.maximum !== null) {
      this.max = this.chart.isBlazor
        ? Date.parse(axis.maximum.toString())
        : Date.parse(
            dateParser(
              dateFormatter(
                new Date(DataUtil.parse.parseJson({ val: axis.maximum }).val)
              )
            )
          );
    ...
    const datetimeInterval: number = this.calculateDateTimeNiceInterval(
      axis,
      size,
      axis.doubleRange.start,
      axis.doubleRange.end
    );
        ...
        switch (intervalType) {
          ...
          case 'Minutes': {
            ...
            const endMinute: number =
              maximum.getMinutes() + (minimum.getMinutes() - minute);
          ...
          case 'Seconds': {
            ...
            const endSecond: number =
              maximum.getSeconds() + (minimum.getSeconds() - second);
            if (rangePadding === 'Round') {
              this.min = new Date(
                minimum.getFullYear(),
                minimum.getMonth(),
  ...
  private getMonth(
    ...
    if (rangePadding === 'Round') {
      ...
      this.max = new Date(
        maximum.getFullYear(),
        endMonth,
        new Date(maximum.getFullYear(), maximum.getMonth(), 0).getDate(),
        23,
        59,
        59
      ).getTime();
  ...
  protected calculateVisibleRange(size: Size, axis: Axis): void {
    ...
    const isLazyLoad: boolean = isNullOrUndefined(axis.zoomingScrollBar)
      ? false
      : axis.zoomingScrollBar.isLazyLoad;
    ...
    axis.dateTimeInterval =
      this.increaseDateTimeInterval(
        axis,
        axis.visibleRange.min,
        axis.visibleRange.interval
      ).getTime() - axis.visibleRange.min;
    axis.triggerRangeRender(
      this.chart,
      axis.visibleRange.min,
      axis.visibleRange.max,
      axis.visibleRange.interval
   ...
   * @private
   */
  public calculateVisibleLabels(
    axis: Axis,
    chart: Chart | RangeNavigator
  ): void {
    axis.visibleLabels = [];
    let tempInterval: number = axis.visibleRange.min;
    let labelStyle: Font;
    let previousValue: number;
    const isBlazor: boolean =
      chart.getModuleName() === 'chart' ? (chart as Chart).isBlazor : false;
    const axisLabels: VisibleLabels[] = axis.visibleLabels;
    if (axis.minimum === null) {
      tempInterval = this.alignRangeStart(
        axis,
        tempInterval,
        axis.visibleRange.interval
      ).getTime();
    ...
    if (
      ...
      (chart as Chart).zoomModule &&
      (chart as Chart).zoomModule.isPanning
    ) {
    ...
    while (tempInterval <= axis.visibleRange.max) {
      labelStyle = <Font>(
        extend({}, getValue('properties', axis.labelStyle), null, true)
      );
      previousValue = axisLabels.length
        ? axis.visibleLabels[axisLabels.length - 1].value
        : tempInterval;
      ...
      if (withIn(tempInterval, axis.visibleRange)) {
        const interval: number = this.increaseDateTimeInterval(
          axis,
          tempInterval,
          axis.visibleRange.interval
        ).getTime();
        ...
        triggerLabelRender(
          chart,
          tempInterval,
          axis.format(new Date(tempInterval)),
          labelStyle,
          axis
        );
      }
      const actualInterval: number = tempInterval;
      tempInterval = this.increaseDateTimeInterval(
        axis,
        tempInterval,
        axis.visibleRange.interval
      ).getTime();
    ...
    if (
      ...
      axis.format = chart.intl.getDateFormat({
        format:
          axis.labelFormat ||
          (axis.actualIntervalType === 'Months' && !axis.skeleton
            ? 'y MMM'
            : ''),
        type: firstToLowerCase(axis.skeletonType),
        skeleton:
          axis.skeleton || (axis.actualIntervalType === 'Days' ? 'MMMd' : ''),
      });
    }
  ...
  private blazorCustomFormat(axis: Axis): string {
    if (this.chart.isBlazor) {
      return axis.actualIntervalType === 'Years'
        ? axis.isIntervalInDecimal
          ? 'yyyy'
          : 'MMM y'
        : axis.actualIntervalType === 'Days' && !axis.isIntervalInDecimal
        ? 'ddd HH tt'
        : '';
    } else {
      return '';
   ...
   * @private
   */
  public increaseDateTimeInterval(
    axis: Axis,
    value: number,
    interval: number
  ): Date {
  ...
  private alignRangeStart(
    ...
    switch (axis.actualIntervalType) {
      case 'Years': {
        const year: number = Math.floor(
          Math.floor(sResult.getFullYear() / intervalSize) * intervalSize
        );
        sResult = new Date(
          year,
          sResult.getMonth(),
      ...
      case 'Months': {
        const month: number = Math.floor(
          Math.floor(sResult.getMonth() / intervalSize) * intervalSize
        );
        sResult = new Date(
          sResult.getFullYear(),
          month,
      ...
      case 'Days': {
        const day: number = Math.floor(
          Math.floor(sResult.getDate() / intervalSize) * intervalSize
        );
        sResult = new Date(
          sResult.getFullYear(),
          sResult.getMonth(),
      ...
      case 'Hours': {
        const hour: number = Math.floor(
          Math.floor(sResult.getHours() / intervalSize) * intervalSize
        );
        sResult = new Date(
          sResult.getFullYear(),
          sResult.getMonth(),
      ...
      case 'Minutes': {
        const minutes: number = Math.floor(
          Math.floor(sResult.getMinutes() / intervalSize) * intervalSize
        );
        sResult = new Date(
          sResult.getFullYear(),
          sResult.getMonth(),
      ...
      case 'Seconds': {
        const seconds: number = Math.floor(
          Math.floor(sResult.getSeconds() / intervalSize) * intervalSize
        );
        sResult = new Date(
          sResult.getFullYear(),
          sResult.getMonth(),
