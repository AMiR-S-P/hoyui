...
export class MultiLevelLabel {
  ...
  public getMultilevelLabelsHeight(axis: Axis): void {
    ...
    axis.multiLevelLabels.map((multiLevel: MultiLevelLabels, index: number) => {
      multiLevel.categories.map(
        (categoryLabel: MultiLevelCategories, i: number) => {
          if (
            categoryLabel.text !== '' &&
            categoryLabel.start !== null &&
            categoryLabel.end !== null
          ) {
            labelSize = measureText(
              categoryLabel.text,
              multiLevel.textStyle,
              this.chart.themeStyle.axisLabelFont
            );
            height = isVertical ? labelSize.width : labelSize.height;
            height +=
              2 * multiLevel.border.width +
              (multiLevel.border.type === 'CurlyBrace' ? padding : 0);
            gap =
              categoryLabel.maximumTextWidth !== null
                ? categoryLabel.maximumTextWidth
                : valueToCoefficient(
                    typeof categoryLabel.end === 'string'
                      ? Number(new Date(categoryLabel.end))
                      : <number>categoryLabel.end,
                    axis
                  ) *
                    axisValue -
                  valueToCoefficient(
                    typeof categoryLabel.start === 'string'
                      ? Number(new Date(categoryLabel.start))
                      : <number>categoryLabel.start,
                    axis
                  ) *
                    axisValue;
            const len: number =
              axis.multiLevelLabels[index as number].categories.length;
            gap =
              (i === 0 || i === len - 1) &&
              axis.labelPlacement === 'OnTicks' &&
              axis.edgeLabelPlacement === 'Shift'
                ? gap / 2
                : gap;
            if (
              labelSize.width > gap - padding &&
              gap > 0 &&
              multiLevel.overflow === 'Wrap' &&
              !isVertical
            ) {
              height =
                height *
            ...
            multiLevelLabelsHeight[index as number] = !multiLevelLabelsHeight[
              index as number
            ]
              ? height
              : multiLevelLabelsHeight[index as number] < height
              ? height
              : multiLevelLabelsHeight[index as number];
          }
        }
      );
      prevHeight[index as number] = value;
      value += multiLevelLabelsHeight[index as number]
        ? multiLevelLabelsHeight[index as number] + padding
        : 0;
    });
    axis.multiLevelLabelHeight =
      value +
      (axis.title !== '' ||
      (this.chart.legendModule && this.chart.legendSettings.visible)
        ? padding / 2
        : 0);
    if (isVertical) {
   ...
   * @returns {void}
   */
  public renderXAxisMultiLevelLabels(
    axis: Axis,
    index: number,
    parent: Element,
    axisRect: Rect
    ...
    const startY: number =
      (axis.labelPosition === axis.tickPosition
    ...
    const scrollBarHeight: number =
      ...
      (isOutside && isNullOrUndefined(axis.crossesAt))
        ? axis.scrollBarHeight
        : 0;
    const clipY: number =
      (opposedPosition && !isOutside) || (!opposedPosition && isOutside)
        ? axisRect.y + startY - axis.majorTickLines.width
        : axisRect.y - startY - axis.multiLevelLabelHeight;
    this.createClipRect(
      axisRect.x - axis.majorTickLines.width,
      clipY + scrollBarHeight,
      axis.multiLevelLabelHeight + padding,
      axisRect.width + 2 * axis.majorTickLines.width,
      this.chart.element.id + '_XAxis_Clippath_' + index,
      this.chart.element.id + 'XAxisMultiLevelLabel' + index
    );
    axis.multiLevelLabels.map((multiLevel: MultiLevelLabels, level: number) => {
      pointIndex = 0;
      this.labelElement = this.chart.renderer.createGroup({
        id: this.chart.element.id + index + '_MultiLevelLabel' + level,
      });
      multiLevel.categories.map(
        (categoryLabel: MultiLevelCategories, i: number) => {
          ...
          argsData = this.triggerMultiLabelRender(
            axis,
            categoryLabel.text,
            axis.multiLevelLabels[level as number].textStyle,
            axis.multiLevelLabels[level as number].alignment,
            categoryLabel.customAttributes
          );
          if (!argsData.cancel) {
            ...
            gap =
              (categoryLabel.maximumTextWidth === null
                ? endX - startX
                : categoryLabel.maximumTextWidth) - padding;
            x = startX + axisRect.x + padding;
            y =
              ((opposedPosition && !isOutside) ||
              (!opposedPosition && isOutside)
                ? startY +
                  axisRect.y +
                  labelSize.height / 2 +
                  padding +
                  this.xAxisPrevHeight[level as number]
                : axisRect.y -
                  startY +
                  labelSize.height / 2 -
                  this.xAxisMultiLabelHeight[level as number] -
            ...
            y =
              multiLevel.border.type === 'CurlyBrace'
                ? (!opposedPosition && isOutside) ||
                  (opposedPosition && !isOutside)
                  ? y + padding
                  : y - padding / 2
                ...
                switch (axis.edgeLabelPlacement) {
                  ...
                  case 'Shift':
                    if (i === 0 || (isInversed && i === len - 1)) {
                      if (
                        argsData.alignment === 'Center' &&
                        (options.x < axisRect.x + padding ||
                          options.x - labelSize.width / 2) < axis.rect.x
                      ) {
                        ...
                        argsData.alignment === 'Far' &&
                        (options.x < axisRect.x + padding ||
                          options.x > axisRect.x + padding)
                      ) {
                        options.x += labelSize.width / 2 - gap / 2;
                      } else if (
                        argsData.alignment === 'Near' &&
                        (options.x < axisRect.x + padding ||
                          options.x > axisRect.x + padding)
                      ) {
                        options.x = axisRect.x + padding;
                      ...
                      if (
                        argsData.alignment === 'Center' &&
                        options.x > axisRect.x + axisRect.width
                      ) {
                        options.x -= padding;
            ...
            if (this.chart.enableCanvas) {
              ...
              this.multiLevelLabelRectXRegion.push(
                new Rect(options.x, options.y, textSize.width, textSize.height)
              );
              this.xLabelCollection.push(options);
            }
            if (
              multiLevel.border.width > 0 &&
              multiLevel.border.type !== 'WithoutBorder'
            ) {
  ...
  private renderXAxisLabelBorder(
    ...
    let value: number;
    let value1: number;
    const groupLabel: MultiLevelLabels = <MultiLevelLabels>(
      axis.multiLevelLabels[labelIndex as number]
    );
    const categoryType: BorderType =
      groupLabel.categories[categoryIndex as number].type;
    ...
    const y: number =
      (!opposedPosition && isOutside) || (opposedPosition && !isOutside)
        ? startY +
          axisRect.y +
          this.xAxisPrevHeight[labelIndex as number] +
          scrollBarHeight
        : axisRect.y -
          startY -
          this.xAxisPrevHeight[labelIndex as number] -
          scrollBarHeight;
    const borderType: BorderType = categoryType
      ? categoryType
      : groupLabel.border.type;
    switch (borderType) {
      case 'WithoutTopandBottomBorder':
      case 'Rectangle':
      case 'WithoutTopBorder': {
        const len: number =
          axis.multiLevelLabels[labelIndex as number].categories.length;
        const lastX: number =
          categoryIndex === len - 1 && x + width > axisRect.width
            ? axisRect.width + axisRect.x
            : x + width;
        const initialX: number =
          categoryIndex === 0 &&
          axis.multiLevelLabels[labelIndex as number].categories[0].start <= 0
            ? axisRect.x
            : x;
        height =
          (!opposedPosition && isOutside) || (opposedPosition && !isOutside)
        ...
        path +=
          ...
          (y + height);
        path +=
          borderType !== 'WithoutTopandBottomBorder'
            ? ' L' + ' ' + initialX + ' ' + (y + height) + ' '
            : ' ';
        path +=
          borderType === 'Rectangle'
            ? 'M ' + initialX + ' ' + y + ' L ' + lastX + ' ' + y
            : ' ';
        break;
      }
      case 'Brace':
   ...
   * @returns {void}
   */
  public renderYAxisMultiLevelLabels(
    axis: Axis,
    index: number,
    parent: Element,
    rect: Rect
    ...
    let scrollBarHeight: number =
      isOutside && isNullOrUndefined(axis.crossesAt) ? axis.scrollBarHeight : 0;
    scrollBarHeight = scrollBarHeight * (opposedPosition ? 1 : -1);
    const clipX: number =
      (opposedPosition && !isOutside) || (!opposedPosition && isOutside)
        ? rect.x - axis.multiLevelLabelHeight - startX - padding
        : rect.x + startX;
    this.createClipRect(
      clipX + scrollBarHeight,
      rect.y - axis.majorTickLines.width,
      rect.height + 2 * axis.majorTickLines.width,
      axis.multiLevelLabelHeight + padding,
      this.chart.element.id + '_YAxis_Clippath_' + index,
      this.chart.element.id + 'YAxisMultiLevelLabel' + index
    );
    axis.multiLevelLabels.map((multiLevel: MultiLevelLabels, level: number) => {
      this.labelElement = this.chart.renderer.createGroup({
        id: this.chart.element.id + index + '_MultiLevelLabel' + level,
        ...
        (categoryLabel: MultiLevelCategories, i: number) => {
          ...
          if (!argsData.cancel) {
            const maximumWidth: number =
              categoryLabel.maximumTextWidth === null
                ? this.yAxisMultiLabelHeight[level as number] / 2
                : categoryLabel.maximumTextWidth / 2;
            labelSize = measureText(
              argsData.text,
              argsData.textStyle,
              this.chart.themeStyle.axisLabelFont
            ...
            options.text =
              multiLevel.overflow === 'Trim'
                ? textTrim(
                    categoryLabel.maximumTextWidth === null
                      ? this.yAxisMultiLabelHeight[level as number]
                      : categoryLabel.maximumTextWidth,
                    argsData.text,
                    argsData.textStyle,
                    this.chart.enableRtl,
  ...
  private renderYAxisLabelBorder(
    ...
    scrollBarHeight = scrollBarHeight * (opposedPosition ? 1 : -1);
    let width: number =
      (groupLabel.categories[categoryIndex as number].maximumTextWidth === null
        ? this.yAxisMultiLabelHeight[labelIndex as number]
        : groupLabel.categories[categoryIndex as number].maximumTextWidth) +
      padding;
    const x: number =
      ((!opposedPosition && isOutside) || (opposedPosition && !isOutside)
        ? rect.x - startX - this.yAxisPrevHeight[labelIndex as number]
        : rect.x + startX + this.yAxisPrevHeight[labelIndex as number]) +
      scrollBarHeight;
    const borderType: BorderType = categoryType
      ? categoryType
    ...
    switch (borderType) {
      ...
      case 'Brace':
        ...
        path +=
          ...
          (x - width / 2) +
          ' ' +
          (textOptions.y - labelSize.height / 2 - 4) +
          ' M ' +
          (x - width / 2) +
          ' ' +
  ...
  public createBorderElement(
    ...
    const borderElement: Element = this.chart.renderer.drawPath(
      new PathOption(
        this.chart.element.id +
          axisIndex +
          '_Axis_MultiLevelLabel_Rect_' +
          borderIndex +
          '_' +
          pointIndex,
        'Transparent',
        axis.multiLevelLabels[borderIndex as number].border.width,
        axis.multiLevelLabels[borderIndex as number].border.color ||
          this.chart.themeStyle.axisLine,
        1,
        '',
        path
      )
  ...
  public MultiLevelLabelClick(
    ...
    const categories: MultiLevelCategoriesModel[] =
      axis.multiLevelLabels[level as number].categories;
    const text: string = categories[textElement as number].text;
    const start: number | Date | string =
      categories[textElement as number].start;
    const end: number | Date | string = categories[textElement as number].end;
    const customAttributes: object =
      categories[textElement as number].customAttributes;
      ...
      for (let i: number = 0; i < this.multiLevelLabelRectXRegion.length; i++) {
        if (
          withInBounds(
            event['x'],
            event['y'],
            this.multiLevelLabelRectXRegion[i as number],
            this.multiLevelLabelRectXRegion[i as number].width,
            this.multiLevelLabelRectXRegion[i as number].height
          )
        ) {
