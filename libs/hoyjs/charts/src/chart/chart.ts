...
import { TapEventArgs, EmitType, ChildProperty } from '@hoyui/base';
import { remove, extend } from '@hoyui/base';
import { INotifyPropertyChanged, Browser, Touch } from '@hoyui/base';
import { Event, EventHandler, Complex, Collection } from '@hoyui/base';
import {
  findClipRect,
  showTooltip,
  ImageOption,
...
import {
  ...
  IResizeEventArgs,
} from '../chart/model/chart-interface';
import {
  ...
  implements INotifyPropertyChanged
{
  ...
  @Complex<ChartAreaModel>(
    { border: { color: null, width: 0.5 }, background: 'transparent' },
    ChartArea
  )
  ...
  private isIdHasSpecialCharacter(elementId: string): string {
    ...
    if (!regex.test(elementId)) {
      ...
      for (let i: number = start; i < elementId.length; i++) {
        if (
          !regex.test(elementId[i as number]) &&
          elementId.indexOf('-') === -1 &&
          elementId.indexOf('_') === -1 &&
          elementId.indexOf('\\') === -1 &&
          !numberRegex.test(elementId[i as number])
        ) {
  ...
  private initPrivateVariable(): void {
    ...
    this.element.setAttribute(
      'aria-label',
      this.description || this.title + '. Syncfusion interactive chart.'
    );
    if (!this.element.classList.contains('e-chart-focused')) {
      this.element.setAttribute(
        'class',
        this.element.getAttribute('class') + ' e-chart-focused'
      );
    }
    if (this.element.id === '') {
      let collection: number =
        document.getElementsByClassName('e-chart').length;
    ...
    this.svgId = this.stockChart
      ? this.stockChart.element.id + '_stockChart_chart'
  ...
  public animate(duration?: number): void {
    ...
    if (this.tooltipModule) {
      this.tooltipModule.removeHighlightedMarker(
        <PointData[]>this.tooltipModule.previousPoints,
        true
      );
    } else if (this.markerRender.previousPoints) {
      for (
        let previousPoint: number = 0;
        previousPoint < this.markerRender.previousPoints.length;
        previousPoint++
      ) {
        this.markerRender.removeHighlightedMarker(
          this.markerRender.previousPoints[previousPoint as number]
            .series as Series,
          this.markerRender.previousPoints[previousPoint as number]
            .point as Points
        );
  ...
  public refreshBound(): void {
    ...
    if (this.stockChart && !this.stockChart.rangeFound) {
      if (
        this.stockChart.enablePeriodSelector ||
        this.stockChart.enableSelector
      ) {
        return null;
      }
    }
  ...
  public calculateStackValues(): void {
    ...
    for (
      let i: number = 0, len: number = this.visibleSeries.length;
      i < len;
      i++
    ) {
      ...
      if (
        (series.type.indexOf('Stacking') !== -1 ||
          (series.drawType.indexOf('Stacking') !== -1 &&
            this.chartAreaType === 'PolarRadar')) &&
        !isCalculateStacking
      ) {
  ...
  private removeSelection(): void {
    ...
    if (getElement(this.element.id + '_ej2_drag_multi_group')) {
      ...
      this.selectionModule.calculateDragSelectedElements(
        this,
        new Rect(0, 0, 0, 0),
        true
      );
  ...
  public renderAxes(): Element {
    ...
    if (this.stripLineModule) {
      this.stripLineModule.renderStripLine(
        this,
        'Behind',
  ...
  private renderLegend(): void {
    if (
      this.legendModule &&
      this.legendModule.legendCollections.length &&
  ...
  private setSecondaryElementPosition(): void {
    ...
    element.style.left =
      Math.max((svgRect.left - rect.left) / this.scaleX, 0) + 'px';
    element.style.top =
  ...
  private renderSeriesElements(axisElement: Element): void {
    ...
    if (this.element.tagName !== 'g') {
      const tooltipDiv: Element =
        redrawElement(this.redraw, elementId + '_Secondary_Element') ||
    ...
    // For canvas
    if (this.enableCanvas) {
      let tooltipdiv: Element = document.getElementById(
        elementId + '_Secondary_Element'
      );
      tooltipdiv = !tooltipdiv
        ? this.createElement('div', {
            id: elementId + '_Secondary_Element',
            attrs: { style: 'position: relative; left:0px; top:0px' },
          })
        : tooltipdiv;
  ...
  public renderSeries(): void {
    ...
    if (this.enableCanvas) {
      ...
      (this.renderer as CanvasRenderer).canvasClip(
        this.chartAxisLayoutPanel.seriesClipRect
      );
    }
    for (const item of this.visibleSeries) {
      if (item.category === 'TrendLine') {
        visibility =
          this.series[item.sourceIndex].trendlines[item.index].visible;
      } else {
      ...
      if (visibility) {
        ...
        item.isLegendClicked &&
        (item.type.indexOf('StackingArea') > -1 ||
          item.type.indexOf('StackingBar') > -1 ||
          item.type.indexOf('StackingColumn') > -1)
      ) {
        findClipRect(item, this.enableCanvas);
        item.renderSeries(this);
      }
    ...
    if (this.chartAreaType === 'PolarRadar') {
      clipRectElement = appendClipElement(
        this.redraw,
        options,
        this.renderer as SvgRenderer,
    ...
    const seriesSvg: HTMLElement = document.getElementById(
      this.element.id + '_series_svg'
    );
    if (seriesSvg) {
  ...
  private initializeIndicator(): void {
    for (const indicator of this.indicators) {
      if (this[firstToLowerCase(indicator.type) + 'IndicatorModule']) {
        this[
          firstToLowerCase(indicator.type) + 'IndicatorModule'
        ].createIndicatorElements(
          this,
          indicator as TechnicalIndicator,
          (indicator as TechnicalIndicator).index
        );
      }
    }
  ...
  private appendElementsAfterSeries(axisElement: Element): void {
    ...
    if (
      (this.zoomModule &&
        this.zoomSettings.enableScrollbar &&
        this.scrollElement.childElementCount) ||
  ...
  private applyZoomkit(): void {
    ...
    if (
      (!this.redraw || this.zoomRedraw) &&
      this.zoomModule &&
      (!this.zoomSettings.enablePan ||
        this.zoomModule.performedUI ||
  ...
  private renderAnnotation(): void {
    if (this.annotationModule) {
      ...
      this.annotationModule.renderAnnotations(
        getElement(
          (this.stockChart ? this.stockChart.element.id : this.element.id) +
            '_Secondary_Element'
        )
      );
    }
  ...
  private performSelection(): void {
    ...
    if (this.selectionModule) {
      selectedDataIndexes = <Indexes[]>(
        extend([], this.selectionModule.selectedDataIndexes, null, true)
      );
  ...
  public processData(render: boolean = true): void {
    ...
    for (const indicator of this.indicators) {
      if (indicator.dataSource) {
        const techIndicator: TechnicalIndicator =
          indicator as TechnicalIndicator;
    ...
    if (
      ...
      (!this.visibleSeries.length ||
        (this.visibleSeriesCount === this.visibleSeries.length && check))
    ) {
      this.refreshBound();
  ...
  private initializeDataModule(series: SeriesBase): void {
    ...
    if (this[isAngular as string]) {
      dataSource = Object.keys(series.dataSource).length
        ? series.dataSource
        : this.dataSource;
    } else {
  ...
  public calculateBounds(): void {
    ...
    const padding: number =
      this.titleStyle.position === 'Top' ||
      (this.titleStyle.position === 'Bottom' && !this.legendSettings.visible)
        ? 15
        : 5;
    let left: number = margin.left + this.border.width;
    let width: number =
      this.availableSize.width - left - margin.right - this.border.width;
    ...
    if (this.title) {
      ...
      titleHeight =
        measureText(this.title, this.titleStyle, this.themeStyle.chartTitleFont)
          .height *
          this.titleCollection.length +
        padding;
      if (this.subTitle) {
        this.subTitleCollection = getTitle(
          this.subTitle,
          this.subTitleStyle,
          width,
          this.enableRtl,
        ...
        subTitleHeight =
          measureText(
            ...
            this.subTitleCollection.length +
          padding;
      }
    ...
    let top: number =
      ...
      elementSpacing +
      this.border.width +
      this.chartArea.border.width * 0.5;
    let height: number =
      this.availableSize.height - top - this.border.width - margin.bottom;
    const marginTotal: number =
      subTitleHeight +
      titleHeight +
      this.titleStyle.border.width +
      this.subTitleStyle.border.width;
    ...
    if (
      ...
      this.stockChart.stockLegendModule
    ) {
    ...
    if (this.legendModule && this.legendSettings.visible) {
      this.legendModule.calculateLegendBounds(
  ...
  private calculateAreaType(): void {
    ...
    this.chartArea.border.width = this.stockChart
      ? 0
      : this.chartArea.border.width;
    if (series) {
      this.requireInvertedAxis =
        (series.type.indexOf('Bar') !== -1 && !this.isTransposed) ||
        (series.type.indexOf('Bar') === -1 &&
          this.isTransposed &&
          this.chartAreaType !== 'PolarRadar');
    }
    this.chartAxisLayoutPanel =
      this.chartAreaType === 'PolarRadar'
        ? this.polarSeriesModule || this.radarSeriesModule
  ...
  private calculateVisibleAxis(): void {
    ...
    this.axisCollections = [];
    if (this.zoomModule) {
      this.zoomModule.isPanning =
        this.zoomModule.isAxisZoomed(axes) && this.zoomSettings.enablePan;
      this.svgObject.setAttribute(
        'cursor',
        this.zoomModule.isPanning ? 'pointer' : 'auto'
      );
  ...
  private initAxis(series: SeriesBase, axis: Axis, isSeries: boolean): void {
    if (
      ...
      (series.xAxisName == null && axis.name === 'primaryXAxis')
    ) {
      axis.orientation = this.requireInvertedAxis ? 'Vertical' : 'Horizontal';
      series.xAxis = axis;
      if (isSeries) {
        axis.series.push(series as Series);
      }
    } else if (
      series.yAxisName === axis.name ||
      (series.yAxisName == null && axis.name === 'primaryYAxis')
    ) {
  ...
  private initTechnicalIndicators(): void {
    ...
    for (const indicator of this.indicators) {
      ...
      if (this[type + 'IndicatorModule']) {
        ...
        this[type + 'IndicatorModule'].initSeriesCollection(
          techIndicator,
          this
        );
  ...
  public refreshTechnicalIndicator(series: SeriesBase): void {
    if (this.indicators.length) {
      ...
      if (series instanceof Series && series.category !== 'Indicator') {
        ...
        targetIndicator.setDataSource(
          series instanceof Series ? series : null,
          this
        );
      }
    }
  ...
  private calculateVisibleSeries(): void {
    let series: Series;
    this.visibleSeries = [];
    const colors: string[] = this.palettes.length
      ? this.palettes
      : getSeriesColor(this.theme);
    const count: number = colors.length;
    const seriesCollection: SeriesModel[] = this.series.sort(
      (a: SeriesModel, b: SeriesModel) => {
        return a.zOrder - b.zOrder;
      }
    );
    for (
      let i: number = 0, len: number = seriesCollection.length;
      i < len;
      i++
    ) {
      ...
      series.category =
        seriesCollection[0].type === 'Pareto' ? 'Pareto' : 'Series';
      series.index = i;
      ...
      if (this.isSecondaryAxis(series.xAxis)) {
        series.xAxis.internalVisibility = series.xAxis.series.some(
          (value: Series) => value.visible
        );
      }
      if (this.isSecondaryAxis(series.yAxis)) {
        series.yAxis.internalVisibility = series.yAxis.series.some(
          (value: Series) => value.visible
        );
      }
      switch (series.type) {
        ...
        case 'Radar':
          ...
          if (
            ...
            ((series.xAxisName !== null &&
              this.primaryXAxis.name !== series.xAxisName) ||
              (series.yAxisName !== null &&
                this.primaryYAxis.name !== series.yAxisName))
          ) {
            continue;
          }
          break;
        ...
        default:
          if (
            this.chartAreaType === 'PolarRadar' ||
            seriesCollection[0].type.indexOf('Bar') > -1
          ) {
            continue;
          }
          break;
  ...
  private renderTitle(): void {
    ...
    if (this.title) {
      let getAnchor: string = getTextAnchor(
        this.titleStyle.textAlignment,
        this.enableRtl
      );
      const elementSize: Size = measureText(
        this.title,
        this.titleStyle,
        this.themeStyle.chartTitleFont
      );
      ...
      let positionX: number =
        titlePositionX(
          rect,
          this.titleStyle || this.themeStyle.chartTitleFont
        ) + borderWidth;
      let rotation: string;
      const alignment: Alignment = this.titleStyle.textAlignment;
      const subtitleSize: Size = measureText(
        this.subTitle,
        this.subTitleStyle,
        this.themeStyle.chartSubTitleFont
      );
      switch (this.titleStyle.position) {
        case 'Top':
          ...
          positionX +=
            getAnchor === 'start'
              ? borderWidth * 0.5 + this.border.width
              : getAnchor === 'end'
              ? -borderWidth * 2 - this.border.width
              : 0;
          break;
        case 'Bottom':
          positionX +=
          ...
          positionY =
            ...
            borderWidth * 0.5 -
            this.subTitleStyle.border.width * 0.5;
          break;
        case 'Left':
          positionX =
            this.margin.left + (elementSize.height * 3) / 4 + borderWidth * 0.5;
          positionY =
            alignment === 'Near'
              ? margin.bottom + borderWidth * 0.5 + this.border.width
              : alignment === 'Far'
              ? this.availableSize.height -
                margin.bottom -
                borderWidth * 0.5 -
                this.border.width
              : this.availableSize.height / 2;
          getAnchor =
            alignment === 'Near'
              ? 'end'
              : alignment === 'Far'
      ...
      const borderOptions: borderOption = {
        ...
        x:
          positionX -
          (getAnchor === 'middle'
            ? elementSize.width / 2 + elementSpacing
            : getAnchor === 'end'
            ? elementSize.width + elementSpacing
            : elementSpacing),
      ...
      const htmlObject: Element =
        redrawElement(
          this.redraw,
          this.element.id + '_ChartTitleBorder',
          borderOptions,
          this.renderer
      ...
      const element: Element =
        ...
        textElement(
          ...
          options,
          this.titleStyle,
          this.titleStyle.color || this.themeStyle.chartTitleFont.color,
          this.svgObject,
          null,
          null,
          null,
  ...
  private renderSubTitle(options: TextOption): void {
    ...
    const subTitleElementSize: Size = measureText(
      this.subTitleCollection.reduce((a: string, b: string) =>
        a.length > b.length ? a : b
      ),
      this.subTitleStyle,
      this.themeStyle.chartSubTitleFont
    );
    const getAnchor: string = getTextAnchor(
      this.subTitleStyle.textAlignment,
      this.enableRtl
    );
    const rect: Rect = new Rect(
      alignment === 'Center'
        ? options.x - maxWidth * 0.5
        : alignment === 'Far'
        ? options.x - maxWidth
        : options.x,
      0,
    ...
    if (this.titleStyle.position === 'Left') {
      rect.x =
        alignment === 'Center'
          ? options.x - maxWidth * 0.5
          : alignment === 'Far'
          ? this.margin.left + (subTitleElementSize.height * 3) / 4
          : options.x - maxWidth;
    }
    const elementSize: Size = measureText(
      this.title,
    ...
    let positionY: number =
      options.y * options.text.length +
      subTitleElementSize.height +
      padding / 2 +
      this.titleStyle.border.width +
      this.subTitleStyle.border.width * 0.5;
    if (this.titleStyle.position === 'Bottom') {
      positionY =
        ...
        padding / 2 +
        elementSize.height / 2 +
        subTitleElementSize.height / 2;
    }
    const borderOptions: border = {
      id: this.element.id + '_ChartSubTitleBorder',
      x:
        titlePositionX(rect, this.subTitleStyle) -
        (getAnchor === 'middle'
          ? subTitleElementSize.width / 2 + padding / 2
          : getAnchor === 'end'
          ? subTitleElementSize.width + padding / 2
          : padding / 2),
      y:
        positionY - subTitleElementSize.height + subTitleElementSize.height / 4,
    ...
    const htmlObject: Element =
      redrawElement(
        this.redraw,
        this.element.id + '_ChartSubTitleBorder',
        borderOptions,
        this.renderer
      ) || this.renderer.drawRectangle(borderOptions);
    appendChildElement(
    ...
    const subTitleOptions: TextOption = new TextOption(
      ...
      titlePositionX(rect, this.subTitleStyle),
      positionY,
      getTextAnchor(this.subTitleStyle.textAlignment, this.enableRtl),
      this.subTitleCollection,
      options.transform,
      'auto'
    );
    const element: Element =
      ...
      textElement(
        ...
        subTitleOptions,
        this.subTitleStyle,
        this.subTitleStyle.color || this.themeStyle.chartSubTitleFont.color,
        this.svgObject,
        null,
        null,
        null,
  ...
  private renderBorder(): void {
    ...
    const fillColor: string = backGroundImage
      ? 'transparent'
      : this.background || this.themeStyle.background;
    if (
      this.stockChart &&
      this.stockChart.legendSettings.visible &&
      this.stockChart.stockLegendModule
    ...
    this.htmlObject =
      (redrawElement(
        ...
        rect,
        this.renderer
      ) as HTMLElement) || (this.renderer.drawRectangle(rect) as HTMLElement);
    ...
    if (backGroundImage) {
      ...
      this.htmlObject =
        (redrawElement(
          ...
          image,
          this.renderer
        ) as HTMLElement) || (this.renderer.drawImage(image) as HTMLElement);
      appendChildElement(
        this.enableCanvas,
        this.svgObject,
        this.htmlObject,
  ...
  public renderAreaBorder(): void {
    if (this.chartAreaType === 'PolarRadar') {
      ...
      const previousRect: Rect = element
        ...
        : null;
      const rect: RectOption = new RectOption(
        this.element.id + '_ChartAreaBorder',
        this.chartArea.background,
        {
          width: this.chartArea.border.width,
          color: this.chartArea.border.color || this.themeStyle.areaBorder,
        },
        this.chartArea.opacity,
        this.chartAxisLayoutPanel.seriesClipRect,
        0,
        0,
        '',
        this.chartArea.border.dashArray
      );
    ...
    if (backGroundImage) {
      ...
      const image: ImageOption = new ImageOption(
        ...
        backGroundImage,
        this.initialClipRect.x,
        this.initialClipRect.y,
  ...
  public addAxes(axisCollection: AxisModel[]): void {
    for (let axis of axisCollection) {
      ...
      if (this.isBlazor) {
        ...
        axis.desiredIntervals = isNaN(axis.desiredIntervals)
          ? null
  ...
  public destroy(): void {
    ...
    const element: HTMLElement = document.getElementById(
      this.element.id + 'Keyboard_chart_focus'
    );
    if (element) {
      element.remove();
    }
    const highlightElement: HTMLElement = document.getElementById(
      this.element.id + '_ej2_chart_highlight'
    );
    if (highlightElement) {
  ...
  private chartRightClick(event: MouseEvent | PointerEvent): boolean {
    if (
      ...
      this.crosshairModule &&
      (event.buttons === 2 ||
        event.which === 0 ||
        (<PointerEvent>event).pointerType === 'touch')
    ) {
  ...
  private setStyle(element: HTMLElement): void {
    ...
    const disableScroll: boolean =
      zooming.enableSelectionZooming ||
      zooming.enablePinchZooming ||
    ...
    element.style.height =
      element.style.height || (this.height && this.height.indexOf('%') === -1)
        ? element.style.height
        : 'inherit';
  }
  /**
  ...
  private setMouseXY(pageX: number, pageY: number): void {
    if (getElement(this.svgId)) {
      const svgRect: ClientRect = getElement(
        this.svgId
      ...
      this.mouseY =
        (pageY - rect.top - Math.max(svgRect.top - rect.top, 0)) / this.scaleY;
      this.mouseX =
        (pageX - rect.left - Math.max(svgRect.left - rect.left, 0)) /
        this.scaleX;
      if (this.stockChart) {
        this.mouseX +=
          this.stockChart.legendSettings.position === 'Left'
            ? this.stockChart.stockLegendModule.legendBounds.width
            : 0;
        this.mouseY +=
          this.stockChart.legendSettings.position === 'Top'
            ? this.stockChart.stockLegendModule.legendBounds.height
            : 0;
      }
    }
  }
  ...
  public chartResize(): boolean {
    ...
    const beforeResizeArgs: IBeforeResizeEventArgs = {
      name: 'beforeResize',
  ...
  public mouseMove(e: PointerEvent): boolean {
    ...
    if (e.type === 'touchmove') {
      ...
      this.isTouch =
        e.pointerType === 'touch' || e.pointerType === '2' || this.isTouch;
      pageX = e.clientX;
      pageY = e.clientY;
    }
  ...
  public chartKeyDown(e: KeyboardEvent): boolean {
    ...
    if (
      this.tooltip.enable &&
      ((e.code === 'Tab' && this.previousTargetId.indexOf('Series') > -1) ||
        e.code === 'Escape')
    ) {
      actionKey = 'ESC';
    }
    if (
      this.highlightMode !== 'None' &&
      e.code === 'Tab' &&
      this.previousTargetId.indexOf('_chart_legend_') > -1
    ) {
      if (this.highlightModule) {
    ...
    if (
      ...
      this.isZoomed =
        this.zoomModule &&
        (this.zoomSettings.enableDeferredZooming ||
          this.zoomSettings.enableSelectionZooming ||
          this.zoomSettings.enablePinchZooming ||
          this.zoomSettings.enableMouseWheelZooming);
  ...
  public chartKeyUp(e: KeyboardEvent): boolean {
    ...
    const titleElement: HTMLElement = getElement(
      this.element.id + '_ChartTitle'
    ) as HTMLElement;
    const seriesElement: HTMLElement = getElement(
      this.element.id + 'SeriesCollection'
    ) as HTMLElement;
    const legendElement: HTMLElement = getElement(
      this.element.id + '_chart_legend_translate_g'
    ) as HTMLElement;
    const pagingElement: HTMLElement = getElement(
      this.element.id + '_chart_legend_pageup'
    ) as HTMLElement;
    ...
    if (
      seriesElement &&
      seriesElement.firstElementChild &&
      seriesElement.firstElementChild.children[1]
    ) {
      const firstChild: HTMLElement = seriesElement.firstElementChild
    ...
    if (e.code === 'Tab') {
      if (this.previousTargetId !== '') {
        if (
          this.previousTargetId.indexOf('_Series_') > -1 &&
          targetId.indexOf('_Series_') === -1
        ) {
          groupElement = getElement(
            this.element.id + 'SeriesCollection'
          ) as HTMLElement;
          const previousElement: Element =
            this.previousTargetId.indexOf('_Symbol') > -1
              ? getElement(
                  this.element.id + 'SymbolGroup' + this.currentSeriesIndex
                ).children[this.currentPointIndex + 1]
              : this.previousTargetId.indexOf('_Point_') > -1
              ? groupElement.children[this.currentSeriesIndex].children[
          ...
          this.previousTargetId.indexOf('_chart_legend_page') > -1 &&
          targetId.indexOf('_chart_legend_page') === -1 &&
          targetId.indexOf('_chart_legend_g_') === -1
        ) {
          this.setTabIndex(
            e.target as HTMLElement,
            getElement(this.element.id + '_chart_legend_pageup') as HTMLElement
          );
        } else if (
          this.previousTargetId.indexOf('_chart_legend_g_') > -1 &&
          targetId.indexOf('_chart_legend_g_') === -1
        ) {
          groupElement = getElement(
            this.element.id + '_chart_legend_translate_g'
          ...
          this.setTabIndex(
            groupElement.children[this.currentLegendIndex] as HTMLElement,
            groupElement.firstElementChild as HTMLElement
          );
        }
      }
      ...
      if (targetId.indexOf('SeriesGroup') > -1) {
        ...
        if (targetElement.children[1].id.indexOf('_Point_') === -1) {
          markerGroup = getElement(
            this.element.id + 'SymbolGroup' + targetId.split('SeriesGroup')[1]
          ) as HTMLElement;
        }
        targetId = this.focusChild(
          (markerGroup != null
            ? markerGroup.children[1]
            : targetElement.children[1]) as HTMLElement
        );
      }
      actionKey =
        this.highlightMode !== 'None' || this.tooltip.enable ? 'Tab' : '';
    } else if (e.code.indexOf('Arrow') > -1) {
      e.preventDefault();
      this.previousTargetId = targetId;
      if (targetId.indexOf('_chart_legend_page') > -1) {
        if (e.code === 'ArrowLeft') {
          ...
          getElement(this.element.id + '_chart_legend_pageup').removeAttribute(
            'tabindex'
          );
          this.focusChild(
            getElement(
        ...
        const legendElement: HTMLCollection =
        ...
        this.currentLegendIndex +=
          e.code === 'ArrowUp' || e.code === 'ArrowRight' ? +1 : -1;
        this.currentLegendIndex = this.getActualIndex(
          this.currentLegendIndex,
          legendElement.length
        );
        ...
        if (e.code === 'ArrowRight' || e.code === 'ArrowLeft') {
          ...
          for (let i: number = 0; i < groupElement.children.length; i++) {
            if (
              groupElement.children[i as number].id.indexOf('SeriesGroup') > -1
            ) {
              seriesIndexes.push(
                +groupElement.children[i as number].id.split('SeriesGroup')[1]
              );
            }
          }
          this.currentSeriesIndex =
            seriesIndexes.indexOf(this.currentSeriesIndex) +
            (e.code === 'ArrowRight' ? 1 : -1);
          this.currentSeriesIndex =
            seriesIndexes[
              this.getActualIndex(this.currentSeriesIndex, seriesIndexes.length)
            ];
        } else {
        ...
        if (targetId.indexOf('_Symbol') > -1) {
          ...
          this.currentPointIndex = this.getActualIndex(
            this.currentPointIndex,
            getElement(
              this.element.id + 'SeriesGroup' + this.currentSeriesIndex
            ).childElementCount - 1
          );
          currentPoint = getElement(
            this.element.id +
        ...
        actionKey =
          this.tooltip.enable || this.highlightMode !== 'None'
            ? 'ArrowMove'
            : '';
      }
    } else if (
   ...
   * @private
   */
  public chartKeyboardNavigations(
    e: KeyboardEvent,
    targetId: string,
    actionKey: string
  ): void {
    ...
    switch (actionKey) {
      ...
      case 'ArrowMove':
        ...
        if (targetId.indexOf('_Point_') > -1) {
          const seriesIndex: number = +targetId
            .split('_Series_')[1]
            .split('_Point_')[0];
          const pointIndex: number = +targetId
            ...
            .replace('_Symbol', '')
            .split('_Point_')[1];
          const pointRegion: ChartLocation =
            this.visibleSeries[seriesIndex as number].points[
              pointIndex as number
            ].symbolLocations[0];
          this.mouseX = pointRegion.x + this.initialClipRect.x;
          this.mouseY = pointRegion.y + this.initialClipRect.y;
          if (this.highlightModule) {
            this.highlightModule.highlightChart(
              document.getElementById(targetId),
              'mousemove'
            );
            this.highlightModule.completeSelection(
              document.getElementById(targetId),
              'mousemove'
            );
          }
        ...
        if (this.highlightModule && this.highlightMode !== 'None') {
          ...
          for (let i: number = 0; i < legendItemsId.length; i++) {
            ...
            if (targetId.indexOf(id) > -1) {
              ...
              this.highlightModule.legendSelection(
                this,
                parseInt(targetId.split(id)[1], 10),
                document.getElementById(targetId),
                'mousemove'
              );
              break;
      ...
      case 'Space':
        if (targetId.indexOf('_chart_legend_') > -1) {
          ...
          this.selectionModule.calculateSelectedElements(
            document.getElementById(targetId),
            'click'
          );
        }
      ...
      case 'Minus':
        ...
        if (actionKey === 'Equal') {
          this.zoomModule.toolkit.zoomInOutCalculation(
            1,
            this,
            this.axisCollections,
            this.zoomSettings.mode
          );
        } else {
          this.zoomModule.toolkit.zoomInOutCalculation(
            -1,
      ...
      case 'ArrowLeft':
      case 'ArrowRight': {
        const yArrowPadding: number =
          actionKey === 'ArrowUp' ? 10 : actionKey === 'ArrowDown' ? -10 : 0;
        const xArrowPadding: number =
          actionKey === 'ArrowLeft' ? -10 : actionKey === 'ArrowRight' ? 10 : 0;
        this.zoomModule.isPanning = this.isChartDrag = true;
        this.zoomModule.doPan(
          this,
          this.axisCollections,
          xArrowPadding,
          yArrowPadding
  ...
  public chartOnMouseClick(e: PointerEvent | TouchEvent): boolean {
    ...
    this.clickCount++;
    const XYvalues: { [key: string]: number } = this.FindXYPointValue(
      chart.mouseX,
      chart.mouseY
    );
    ...
    if (this[isAngular as string]) {
      ...
      timeInterval =
        this.pointDoubleClick[observers as string].length > 0 ? 400 : 0;
    } else {
      timeInterval = this.pointDoubleClick ? 400 : 0;
    }
    if (this.clickCount === 1 && this.pointClick) {
      this.singleClickTimer = +setTimeout((): void => {
        this.clickCount = 0;
  ...
  public FindXYPointValue(
    ...
    if (
      ...
      for (
        let k: number = 0, length: number = this.axisCollections.length;
        k < length;
        k++
      ) {
        axis = this.axisCollections[k as number];
        if (axis.orientation === 'Horizontal') {
          Xvalue = getValueXByPoint(
            Math.abs(mouseX - axis.rect.x),
            axis.rect.width,
            axis
          );
          axisData[this.axisCollections[k as number].name] = Xvalue;
        } else {
          Yvalue = getValueYByPoint(
            Math.abs(mouseY - axis.rect.y),
            axis.rect.height,
            axis
          );
  ...
  private titleTooltip(
    ...
    const id: boolean =
      targetId === this.element.id + '_ChartTitle' ||
      targetId === this.element.id + '_ChartSubTitle' ||
      targetId.indexOf('_AxisTitle') > -1 ||
      targetId.indexOf('_legend_title') > -1;
    let index: number = 0;
    if (targetId.indexOf('_AxisTitle') > -1) {
      index = parseInt(
        targetId
          .replace(this.element.id, '')
          .replace('AxisLabel_', '')
          .split('_')[2],
        10
      );
    ...
    if (id && (<HTMLElement>event.target).textContent.indexOf('...') > -1) {
      const title: string =
        targetId === this.element.id + '_ChartTitle'
          ? this.title
          : targetId.indexOf('_AxisTitle') > -1
          ? this.axisCollections[index as number].title
          : targetId.indexOf('_ChartSubTitle') > -1
          ? this.subTitle
          : this.legendSettings.title;
      showTooltip(
        title,
        x,
        y,
  ...
  private axisTooltip(
    ...
    const targetId: string = (<HTMLElement>event.target).id;
    if (
      (targetId.indexOf('AxisLabel') > -1 ||
        targetId.indexOf('Axis_MultiLevelLabel') > -1) &&
      (<HTMLElement>event.target).textContent.indexOf('...') > -1
    ) {
      const isTitleOrLegendEnabled: boolean =
        this.legendSettings.visible || this.primaryXAxis.title === '';
      showTooltip(
        this.findAxisLabel(targetId),
        x,
        y,
        this.element.offsetWidth,
  ...
  private findAxisLabel(text: string): string {
    ...
    if (text.indexOf('AxisLabel') > -1) {
      texts = text
        ...
        .replace('AxisLabel_', '')
        .split('_');
      return this.axisCollections[parseInt(texts[0], 10)].visibleLabels[
        parseInt(texts[1], 10)
      ].originalText;
    } else {
      texts = text
        .replace(this.element.id, '')
        .replace('Axis_MultiLevelLabel_Level_', '')
        .replace('Text_', '')
        .split('_');
      return this.axisCollections[parseInt(texts[0], 10)].multiLevelLabels[
        parseInt(texts[1], 10)
  ...
  public chartOnMouseDown(e: PointerEvent): boolean {
    ...
    const svgRect: ClientRect = getElement(this.svgId).getBoundingClientRect();
    this.mouseDownX = this.previousMouseMoveX =
      pageX - rect.left - Math.max(svgRect.left - rect.left, 0);
    this.mouseDownY = this.previousMouseMoveY =
      pageY - rect.top - Math.max(svgRect.top - rect.top, 0);

    if (this.isTouch) {
      this.isDoubleTap =
        new Date().getTime() < this.threshold &&
        target.id.indexOf(this.element.id + '_Zooming_') === -1 &&
        (this.mouseDownX - offset >= this.mouseX ||
          this.mouseDownX + offset >= this.mouseX) &&
        (this.mouseDownY - offset >= this.mouseY ||
          this.mouseDownY + offset >= this.mouseY) &&
        (this.mouseX - offset >= this.mouseDownX ||
          this.mouseX + offset >= this.mouseDownX) &&
        (this.mouseY - offset >= this.mouseDownY ||
          this.mouseY + offset >= this.mouseDownY);
    }
    if (this.dataEditingModule) {
  ...
  public setAnnotationValue(annotationIndex: number, content: string): void {
    const parentNode: Element = getElement(
      this.element.id + '_Annotation_Collections'
    );
    const annotation: ChartAnnotationSettings = <ChartAnnotationSettings>(
      this.annotations[annotationIndex as number]
    );
  ...
  private setTheme(): void {
    ...
    if (!document.getElementById(this.element.id + 'Keyboard_chart_focus')) {
      ...
      style.setAttribute(
        'id',
        (<HTMLElement>this.element).id + 'Keyboard_chart_focus'
      );
      style.innerText =
        '.e-chart-focused:focus, path[class*=_ej2_chart_selection_series]:focus,' +
        ...
        'path[id*=_Point_]:focus-visible, text[id*=_ChartTitle]:focus-visible {outline: 1.5px ' +
        this.themeStyle.tabColor +
        ' solid}';
  ...
  public requiredModules(): ModuleDeclaration[] {
    ...
    this.chartAreaType =
      series.length > 0 &&
      (series[0].type === 'Polar' || series[0].type === 'Radar')
        ? 'PolarRadar'
        : 'Cartesian';
    if (this.tooltip.enable) {
      modules.push({
    ...
    series.map((value: Series) => {
      this.isLegend =
        this.legendSettings.visible && (value.name !== '' || !!this.isLegend);
      moduleName =
        value.type.indexOf('100') !== -1
          ? value.type.replace('100', '') + 'Series'
          : value.type + 'Series';
      errorBarVisible = value.errorBar.visible || errorBarVisible;
      dataLabelEnable =
        value.marker.dataLabel.visible ||
        dataLabelEnable ||
        (value.type === 'Pareto' &&
          value.paretoOptions.marker.dataLabel.visible);
      isPointDrag = value.dragSettings.enable || isPointDrag;
      if (
        !modules.some((currentModule: ModuleDeclaration) => {
    ...
    enableAnnotation = this.annotations.some(
      (value: ChartAnnotationSettings) => {
    ...
    if (
      ...
      (zooming.enableSelectionZooming ||
        zooming.enableMouseWheelZooming ||
        zooming.enablePinchZooming ||
        zooming.enablePan ||
        zooming.enableScrollbar ||
        zooming.showToolbar)
    ) {
      modules.push({
        member: 'Zoom',
  ...
  private findAxisModule(modules: ModuleDeclaration[]): ModuleDeclaration[] {
    ...
    for (const axis of axisCollections) {
      ...
      logarithmicEnabled =
        axis.valueType === 'Logarithmic' || logarithmicEnabled;
      dateTimeCategoryEnabled =
        axis.valueType === 'DateTimeCategory' || dateTimeCategoryEnabled;
      striplineEnabled =
        this.findStriplineVisibility(axis.stripLines) || striplineEnabled;
      multiLevelEnabled = axis.multiLevelLabels.length > 0 || multiLevelEnabled;
      this.scrollSettingEnabled = axis.scrollbarSettings.enable
        ? true
        : this.scrollSettingEnabled;
    }
  ...
  public removeSvg(): void {
    ...
    if (
      this.enableCanvas &&
      this.svgObject &&
      (this.svgObject as HTMLElement).tagName === 'CANVAS'
    ) {
      (this.renderer as CanvasRenderer).clearRect(
    ...
    if (!isNullOrUndefined(this.resizeTo)) {
      if (
        this.resizeTo !== this.checkResize &&
        this.isBlazor &&
        this.element.childElementCount
      ) {
        const containerCollection: NodeListOf<Element> =
          document.querySelectorAll('.e-chart');
        for (
          let index: number = 0;
          index < containerCollection.length;
          index++
        ) {
    ...
    if (this.svgObject) {
      ...
      if (
        !this.svgObject.hasChildNodes() &&
        this.svgObject.parentNode &&
        !this.stockChart
      ) {
        remove(this.svgObject);
      }
  ...
  public showTooltip(
    ...
    if (isPoint) {
      for (const series of this.visibleSeries) {
        for (const point of series.points) {
          const pointX: any =
            series.xAxis.valueType === 'DateTime' ? point.xValue : point.x;
          let xValue: any = x;
          ...
          if (x === pointX && y === point.yValue) {
            this.mouseX =
              point.regions[0].x + this.chartAxisLayoutPanel.seriesClipRect.x;
            this.mouseY =
              point.regions[0].y + this.chartAxisLayoutPanel.seriesClipRect.y;
            this.tooltipModule.tooltip();
      ...
      for (const prop of Object.keys(newProp)) {
        switch (prop) {
          case 'primaryXAxis':
            ...
            if (
              !isNullOrUndefined(axis.isInversed) ||
              !isNullOrUndefined(axis.opposedPosition)
            ) {
          ...
          case 'axes':
            for (const index of Object.keys(newProp.axes)) {
              ...
              if (
                ...
                (
                  this.axes[index as string] as Axis
          ...
          case 'titleStyle':
            if (
              newProp.titleStyle &&
              (newProp.titleStyle.size || newProp.titleStyle.textOverflow)
            ) {
              refreshBounds = true;
            } else {
              renderer = true;
          ...
          case 'subTitleStyle':
            if (
              newProp.subTitleStyle &&
              (newProp.subTitleStyle.size || newProp.subTitleStyle.textOverflow)
            ) {
              refreshBounds = true;
            } else {
              renderer = true;
          ...
          case 'series': {
            ...
            for (let i: number = 0; i < len; i++) {
              ...
              if (
                ...
                extend(
                  this.getVisibleSeries(this.visibleSeries, i),
                  series,
                  null,
                  true
                );
          ...
          case 'zoomSettings':
            if (
              newProp.zoomSettings.enableScrollbar ||
              oldProp.zoomSettings.enableScrollbar
            ) {
              refreshBounds = true;
            }
            renderer = true;
          ...
          case 'legendSettings':
            if (
              !newProp.legendSettings.background ||
              !newProp.legendSettings.opacity
            ) {
              refreshBounds = true;
            }
            renderer = true;
          ...
          case 'selectedDataIndexes':
            if (this.selectionModule) {
              ...
              this.selectionModule.selectedDataIndexes = this
                .selectedDataIndexes as Indexes[];
              this.selectionModule.styleId =
                this.element.id + '_ej2_chart_selection';
              this.selectionModule.redrawSelection(
                this,
                oldProp.selectionMode,
                true
              );
            } else if (this.highlightModule) {
              this.highlightModule.currentMode = this.highlightMode;
              this.highlightModule.highlightDataIndexes = this
                .selectedDataIndexes as Indexes[];
              this.highlightModule.styleId =
                this.element.id + '_ej2_chart_highlight';
              this.highlightModule.redrawSelection(
                this,
                oldProp.selectionMode,
                true
              );
          ...
          case 'selectionMode':
            if (
              ...
              newProp.selectionMode &&
              newProp.selectionMode.indexOf('Drag') === -1
            ) {
              this.selectionModule.currentMode = this.selectionMode;
              this.selectionModule.styleId =
                this.element.id + '_ej2_chart_selection';
          ...
          case 'isMultiSelect':
            if (
              this.selectionModule &&
              !newProp.isMultiSelect &&
              this.selectionModule.selectedDataIndexes.length > 1
            ) {
              this.selectionModule.currentMode = this.selectionMode;
              this.selectionModule.styleId =
                this.element.id + '_ej2_chart_selection';
