...
import { ChildProperty, Complex, Property, extend, remove } from '@hoyui/base';
import {
  BorderModel,
  FontModel,
  LocationModel,
...
import {
  Tooltip as SVGTooltip,
  ITooltipAnimationCompleteArgs,
  Rect,
} from '@hoyui/svg-base';
...
export class CircularChartTooltip3D extends ChildProperty<CircularChartTooltip3D> {
  ...
  public mouseUpHandler(
    ...
    const isWithinSwipeThreshold: boolean =
      Math.abs(chart.mouseX - chart.cachedX) < swipeThreshold &&
      Math.abs(chart.mouseY - chart.cachedY) < swipeThreshold;
    if (
      this.control.tooltip.enable &&
      !chart.rotateActivate &&
      isWithinSwipeThreshold &&
      withInBounds(
        this.control.mouseX,
        this.control.mouseY,
        this.control.initialClipRect
      ) &&
      this.control.isTouch
    ) {
      this.tooltip(event);
  ...
  private getTooltipElement(isTooltip: boolean): HTMLDivElement {
    this.header =
      this.control.tooltip.header === null
        ? '${series.name}'
   ...
   * @private
   */
  public renderSeriesTooltip(
    chart: CircularChart3D,
    data: CircularChart3DPointData
  ): void {
    const svgElement: HTMLElement = this.getElement(
      this.element.id + '_tooltip_svg'
    );
    const isTooltip: boolean =
      svgElement && parseInt(svgElement.getAttribute('opacity'), 10) > 0;
    ...
    if (data.point) {
      if (this.pushData(data, tooltipDiv)) {
        this.triggerTooltipRender(
          data,
          isFirst,
          this.getTooltipText(data, chart.tooltip),
  ...
  public removeTooltip(duration: number): void {
    const tooltipElement: HTMLElement = this.getElement(
      this.element.id + '_tooltip'
    );
    this.stopAnimation();
    if (tooltipElement && this.previousPoints.length > 0) {
      this.tooltipInterval = +setTimeout((): void => {
        if (this.svgTooltip) {
  ...
  private triggerTooltipRender(
    point: CircularChart3DPointData,
    isFirst: boolean,
    ...
    const argsData: CircularChart3DTooltipRenderEventArgs = {
      ...
      data: {
        ...
        pointY: point.point.y as Object,
    ...
    const tooltipSuccess: Function = (
      argsData: CircularChart3DTooltipRenderEventArgs
    ) => {
      if (!argsData.cancel) {
  ...
  private getPieData(
    ...
    if (element.id.indexOf('point') > -1 && element.id.indexOf('series') > -1) {
      ...
      series.dataLabel.position === 'Inside'
    ) {
      const index: number = parseInt(
        element.id.split('data-label-text-')[1],
        10
      );
      point = series.points[index as number];
    } else if (
      element.id.indexOf('data-label-series') > -1 &&
      series.dataLabel.position === 'Inside'
    ) {
      const index: number = parseInt(
        element.id.split('data-label-series-0-shape-')[1],
        10
      );
      point = series.points[index as number];
    }
    if (point) {
      const pointData: CircularChart3DPointData = {
        point: point,
        series: series,
        index: point.index,
      };
  ...
  private getTooltipText(
    ...
    let format: string = tooltip.format
      ? tooltip.format
      : '${point.x} : <b>${point.y}</b>';
    format = this.control.useGroupingSeparator
      ? format.replace('${point.y}', '${point.separatorY}')
      : format;
  ...
  private createTooltip(
    ...
    clipLocation: CircularChart3DLocation,
    point: CircularChart3DPoints,
    offset: number,
    bounds: Rect,
    extraPoints: CircularChart3DPointData[] = null,
    templatePoint: CircularChart3DPoints | CircularChart3DPoints[] = null,
    customTemplate?: string | Function
  ): void {
    const tooltipModule: CircularChartTooltip3D =
      chart.circularChartTooltip3DModule;
    ...
    if (isFirst) {
      this.svgTooltip = new SVGTooltip({
        opacity: chart.tooltip.opacity
          ...
          : this.control.theme === 'Material3' ||
            this.control.theme === 'Material3Dark' ||
        ...
        isCanvas: false,
        isFixed:
          this.control.tooltip.location.x !== null ||
          this.control.tooltip.location.y !== null,
    ...
    if ((this.control as any).isReact) {
  ...
  private removeHighlight(): void {
    ...
    for (
      let i: number = 0, len: number = this.previousPoints.length;
      i < len;
      i++
    ) {
  ...
  private highlightPoint(
    ...
    if (
      ...
      const datalabelElement: HTMLElement = document.getElementById(
        this.control.element.id + '-svg-data-label-text-' + pointIndex
      );
      const connectorElement: HTMLElement = document.getElementById(
        this.control.element.id + '-datalabel-series-0-connector-' + pointIndex
      );
      const shapeElement: HTMLElement = document.getElementById(
        this.control.element.id + '-svg-data-label-series-0-shape-' + pointIndex
      );
      if (datalabelElement) {
        pointElements.push(datalabelElement);
      }
      if (connectorElement) {
      ...
      if (seriesElements) {
        for (const seriesElement of seriesElements) {
          if (seriesElement.parentElement.id === this.control.groupElement.id) {
            const selection: boolean = seriesElement.hasAttribute('class')
              ? seriesElement.getAttribute('class').indexOf('_selection_') ===
                -1
              : true;
            seriesElement.setAttribute(
              'opacity',
              (highlight &&
              this.control.highlightColor !== 'transparent' &&
              selection
                ? 0.2
                : series.opacity
              ).toString()
  ...
  private updatePreviousPoint(extraPoints: CircularChart3DPointData[]): void {
    if (extraPoints) {
      this.currentPoints = (<CircularChart3DPointData[]>(
        this.currentPoints
      )).concat(extraPoints);
    }
    this.previousPoints = <CircularChart3DPointData[]>(
      extend([], this.currentPoints, null, true)
    );
  }
