...
import { Rect, SvgRenderer, CanvasRenderer } from '@hoyui/svg-base';
...
export class CircularChartSelection3D extends BaseSelection {
  ...
  public invokeSelection(circular3D: CircularChart3D): void {
    ...
    this.series = <CircularChart3DSeries[]>(
      extend({}, circular3D.visibleSeries, null, true)
    );
    this.seriesStyles();
  ...
  private generateStyle(
    ...
    return (
      this.styleId +
      '_series_' +
      (<CircularChart3DSeries>series).index +
      '_point_' +
      point
    );
  }
  ...
  public getElementByIndex(index: Index): Element[] {
    ...
    elements.forEach((pointElement: Element) => {
      if (
        pointElement.id.indexOf(this.circular3D.element.id + '-') > -1 &&
        this.indexFinder(pointElement.id).point === index.point
      ) {
        pointElements.push(pointElement as Element);
      }
    });
    const element: HTMLElement = document.getElementById(
      this.control.element.id + '-svg-data-label-text-' + index.point
    );
    const connectorElement: HTMLElement = document.getElementById(
      this.control.element.id + '-datalabel-series-0-connector-' + index.point
    );
    const shapeElement: HTMLElement = document.getElementById(
      this.control.element.id + '-svg-data-label-series-0-shape-' + index.point
    );
    if (element) {
  ...
  public isAlreadySelected(
    ...
    if (
      this.circular3D.highlightMode !== 'None' &&
      this.circular3D.selectionMode === 'None'
    ) {
      if (eventType === 'click') {
        return false;
      }
    ...
    if (
      (this.circular3D.highlightMode !== 'None' ||
        this.circular3D.legendSettings.enableHighlight) &&
      this.previousSelectedElement &&
      this.previousSelectedElement[0]
    ) {
      ...
      for (let i: number = 0; i < this.previousSelectedElement.length; i++) {
        if (this.previousSelectedElement[i as number].hasAttribute('class')) {
          if (
            ...
            this.previousSelectedElement[i as number].classList.forEach(
              (className: string) => {
            ...
            this.previousSelectedElement[i as number].classList.remove(
              this.styleId + '_series_' + index.series
            );
            this.addOrRemoveIndex(
              this.highlightDataIndexes,
              this.indexFinder(
                (<HTMLElement>this.previousSelectedElement[i as number]).id
              )
            );
          } else if (
            !isElement &&
            ...
            this.performSelection(
              this.indexFinder(this.previousSelectedElement[i as number].id),
              this.circular3D,
              this.previousSelectedElement[i as number]
            );
          }
  ...
  public calculateSelectedElements(
    ...
    if (
      ...
      (targetElement.id &&
        targetElement.id.indexOf(circular3D.element.id + '-') === -1)
    ) {
      return;
    }
  ...
  public indexFinder(id: string): Index {
    ...
    if (id.indexOf('-point-') > -1) {
      ...
      id.indexOf('data-label-series-0-shape-') > -1 &&
      (this.control as CircularChart3D).series[0].dataLabel.position ===
        'Inside'
    ) {
      ids[0] = '0';
    ...
    for (let i: number = 0; i < this.selectedDataIndexes.length; i++) {
      ...
      if (!isNaN(pointIndex)) {
        selectedPointValues.push({
          x: series.dataSource[pointIndex as number][series.xName],
          y: series.points[pointIndex as number].y,
  ...
  private selection(
    ...
    const className: string =
      selectedElements[0] && (selectedElements[0].getAttribute('class') || '');
    if (
      selectedElements[0] &&
      ...
      if (
        ...
        (circular3D.highlightMode !== 'None' ||
          circular3D.legendSettings.enableHighlight)
      ) {
      ...
      this.previousSelectedElement =
        circular3D.highlightMode !== 'None' ? selectedElements : [];
        ...
        this.getElementByIndex(index).length === 0
          ? [
              document.getElementById(
                this.control.element.id + '_chart_legend_shape_' + index.point
              ),
            ]
  ...
  public legendSelection(
    ...
    if (isPreSelected) {
      let seriesStyle: string = this.generateStyle(
        chart.visibleSeries[index.series as number],
        index.point
      );
      ...
      if (selectedElements.length > 0) {
        ...
        for (const element of chart.visibleSeries[0].points) {
          if (
            element.index !== index.point &&
            (!chart.isMultiSelect || seriesStyle.indexOf('highlight') > 0)
          ) {
            seriesStyle = this.generateStyle(
              chart.visibleSeries[index.series],
              element.index
            );
  ...
  private blurEffect(
    ...
    const visibility: boolean =
      this.checkVisibility(this.highlightDataIndexes) ||
    ...
    for (const point of visibleSeries[0].points) {
      ...
      if (point.visible) {
        ...
        this.checkSelectionElements(
          pointElements,
          this.generateStyle(visibleSeries[0], point.index),
          visibility,
          point.index
        );
      }
  ...
  public checkSelectionElements(
    ...
    for (let i: number = 0; i < children.length; i++) {
      ...
      if (elementClassName.indexOf(className) === -1 && visibility) {
        ...
        if (
          ...
          this.circular3D.highlightAnimation(
      ...
      if (
        children[i as number].id.indexOf('-data-label-text-') !== -1 ||
        children[i as number].id.indexOf('-connector-') !== -1 ||
        children[i as number].id.indexOf('-data-label-series-0-shape-') !== -1
      ) {
        if (
          this.highlightDataIndexes &&
          this.highlightDataIndexes[0] &&
          point === this.highlightDataIndexes[0].point
        ) {
          this.removeSvgClass(children[i as number], this.unselected);
        }
    ...
    if (
      (this.control as CircularChart3D).circularChartLegend3DModule &&
      (this.control as CircularChart3D).legendSettings.visible
    ) {
      legendShape = getElement(
        this.control.element.id + '_chart_legend_shape_' + point
      );
      if (legendShape) {
  ...
  private applyStyles(elements: Element[], index: Index): void {
    const circular3DTooltip: CircularChartTooltip3D = (
      this.control as CircularChart3D
    ...
    for (const element of elements) {
      if (element) {
        ...
        const opacity: number =
          circular3DTooltip &&
          circular3DTooltip.previousPoints.length > 0 &&
          circular3DTooltip.previousPoints[0].point.index !== index.point
            ? circular3DTooltip.svgTooltip.opacity
            : this.series[index.series].opacity;
        element.setAttribute('opacity', opacity.toString());
        if (
          element.id.indexOf('-data-label-text-') === -1 &&
          element.id.indexOf('-connector-') === -1 &&
          element.id.indexOf('-data-label-series-0-shape-') === -1
        ) {
    ...
    if (
      ...
      const legendShape: Element = document.getElementById(
        this.control.element.id + '_chart_legend_shape_' + index.point
      );
      this.removeSvgClass(legendShape, legendShape.getAttribute('class'));
  ...
  private getSelectionClass(id: string): string {
    return this.generateStyle(
      (this.control as CircularChart3D).series[this.indexFinder(id).series],
      this.indexFinder(id).point
    );
  }
  ...
  public mouseMove(event: PointerEvent | TouchEvent): void {
    ...
    if (
      !circular3D.rotateActivate &&
      circular3D.circularChartHighlight3DModule &&
      (circular3D.highlightMode !== 'None' ||
        circular3D.legendSettings.enableHighlight)
    ) {
      if (!isNullOrUndefined(targetElement)) {
        if (
          (<Element>event.target).id.indexOf('text') > 1 &&
          (<Element>event.target).id.indexOf('legend') > -1
        ) {
          targetElement = getElement(
            (<Element>event.target).id.replace('text', 'shape')
          );
        }
        if (
          targetElement.hasAttribute('class') &&
        ...
        if (
          this.highlightDataIndexes &&
          (targetElement.id.indexOf('_chart_legend_') === -1 ||
            targetElement.id.indexOf('_legend_element') !== -1) &&
          this.highlightDataIndexes.length > 0 &&
          targetElement.id.indexOf('-series-') === -1 &&
          targetElement.id.indexOf('-data-label-') === -1
        ) {
  ...
  public removeLegendHighlightStyles(): void {
    ...
    for (
      let i: number = 0;
      i < this.circular3D.visibleSeries[0].points.length;
      i++
    ) {
      elementCollection = document.getElementsByClassName(
        this.generateStyle(this.circular3D.visibleSeries[0], i)
      );
      if (this.selectedDataIndexes.length === 0) {
        ...
        while (elementCollection.length > 0) {
          ...
          if (element) {
            this.removeSvgClass(element, element.getAttribute('class'));
            if (
              this.generateStyle(this.circular3D.visibleSeries[0], i).indexOf(
                'highlight'
              ) > -1
            ) {
              this.circular3D.highlightAnimation(
