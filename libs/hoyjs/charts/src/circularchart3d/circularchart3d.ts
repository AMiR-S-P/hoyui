...
import {
  CircularChart3DBinaryTreeBuilder,
  CircularChart3DTransform,
  CircularChart3DGraphics,
  CircularChart3DMatrix,
  CircularChart3DPolygonModule,
  CircularChart3DSvgRenderer,
  CircularChart3DVectorModule,
} from './renderer/3d-renderer';
import {
  animationComplete,
  beforeResize,
  load,
  ...
  implements INotifyPropertyChanged
{
   ...
   */
  @Complex<CircularChart3DLegendSettingsModel>(
    {},
    CircularChart3DLegendSettings
  )
   ...
   */
  @Complex<CircularChart3DTooltipSettingsModel>(
    {},
    CircularChart3DTooltipSettings
  )
  ...
  protected preRender(): void {
    ...
    if (this.element.id === '') {
      const collection: number =
        document.getElementsByClassName('e-circular3dchart').length;
  ...
  public chartOnMouseClick(e: PointerEvent): boolean {
    ...
    if (this.selectionMode !== 'None' && this.circularChartSelection3DModule) {
      this.circularChartSelection3DModule.calculateSelectedElements(
        this,
        e.target as Element,
        e.type
      );
    ...
    if (this.visibleSeries[0].explode) {
      ...
      if (id.indexOf('-point-') > -1) {
        ...
        if (
          ...
          this.visibleSeries[0].points[pointIndex as number].isExplode
        ) {
          this.visibleSeries[0].explodeIndex = null;
          this.visibleSeries[0].isExploded = true;
          this.visibleSeries[0].points.forEach(
  ...
  public chartOnMouseMove(e: PointerEvent): boolean {
    ...
    if (this.rotateActivate) {
      ...
      if (deltaX || deltaY) {
        ...
        if (!this.isTouch) {
          const grpElement: Element = document.getElementById(
            this.groupElement.id
          );
        ...
        if (
          ...
          this.circularChartHighlight3DModule.calculateSelectedElements(
            this,
            document.getElementById(this.element.id + '-border'),
            'mousemove'
          );
  ...
  private titleTooltip(
    ...
    const id: boolean =
      targetId === this.element.id + '-title' ||
      targetId === this.element.id + '-sub-title' ||
  ...
  public chartMouseDown(e: PointerEvent): boolean {
    ...
    const bounds: ClientRect = document
      .getElementById(this.element.id + '-svg-chart-3d')
      .getBoundingClientRect();
    const rect: Rect = {
  ...
  private setStyle(element: HTMLElement): void {
    ...
    style.innerText =
      '.e-circular3dchart-focused:focus,path[id*=-series-0-point-]:focus, text[id*=-title]:focus' +
  ...
  protected render(): void {
    ...
    this.element.setAttribute(
      'class',
      this.element.getAttribute('class') + ' e-circular3dchart-focused'
    );
    ...
    for (let i: number = 0; i < this.visibleSeries[0].points.length; i++) {
      if (this.visibleSeries[0].points[i as number].visible) {
        elements = document.querySelectorAll(
          '[id*="region-series-0-point-' +
            this.visibleSeries[0].points[i as number].index +
            '"]'
        );
        break;
      }
  ...
  private processSelection(): void {
    ...
    if (this.circularChartSelection3DModule && this.selectionMode !== 'None') {
      selectedDataIndexes = <Indexes[]>(
        extend(
          [],
          this.circularChartSelection3DModule.selectedDataIndexes,
          null,
          true
        )
      );
  ...
  public highlightAnimation(
    ...
    const endOpacity: number = parseFloat(
      this.visibleSeries[0].opacity.toString()
    );
    if (endOpacity) {
  ...
  private doAnimation(): void {
    ...
    if (series.animation.enable && this.animateSeries) {
      const clippath: Element = this.renderer.createClipPath({
        id: this.element.id + 'SeriesGroup0' + '_clipPath',
      });
      const path: PathOption = new PathOption(
        this.element.id + 'SeriesGroup0' + '_slice',
        'transparent',
        1,
        'transparent',
        1,
      ...
      document
        ...
        .forEach(function (slice: Element): void {
          if (slice.parentElement.id === groupElementID) {
            (slice as HTMLElement).style.cssText =
              'clip-path:url(#' +
              clippath.id +
  ...
  private renderLegend(): void {
    ...
    if (
      this.circularChartLegend3DModule.legendCollections.length &&
      this.visibleSeries[0].labelBound
    ) {
      this.circularChartLegend3DModule.getSmartLegendLocation(
        this.visibleSeries[0].labelBound,
        this.circularChartLegend3DModule.legendBounds,
        this.margin
      );
    }
  ...
  private animationRect(slice: Element, series: CircularChart3DSeries): void {
    ...
    let value: number;
    let radius: number =
      Math.max(this.availableSize.height, this.availableSize.width) * 0.75;
    ...
    for (let i: number = 0; i < series.points.length; i++) {
      const dataLabelElement: HTMLElement = getElement(
        this.element.id + '-svg-data-label-text-' + i
      ) as HTMLElement;
      if (dataLabelElement) {
        dataLabelElement.style.visibility = 'hidden';
      }
      const shapeElement: HTMLElement = getElement(
        this.element.id + '-svg-data-label-series-0-shape-' + i
      ) as HTMLElement;
      if (shapeElement) {
        shapeElement.style.visibility = 'hidden';
      }
      const templateElement: HTMLElement = getElement(
        this.element.id + '-series-' + series.index + '-data-label-' + i
      ) as HTMLElement;
      if (templateElement) {
        templateElement.style.visibility = 'hidden';
      }
      const connectorElement: HTMLElement = getElement(
        this.element.id + '-datalabel-series-0-connector-' + i
      ) as HTMLElement;
      if (connectorElement) {
    ...
    if (!isNullOrUndefined(slice)) {
      new Animation({}).animate(<HTMLElement>slice, {
        ...
        progress: (args: AnimationOptions): void => {
          ...
          slice.setAttribute(
            'd',
            this.getPathArc(center, startAngle, value, radius)
          );
        },
        end: () => {
          slice.setAttribute(
            'd',
            this.getPathArc(center, 0, 359.99999, radius)
          );
  ...
  private getPathArc(
    ...
    return this.getPiePath(
      center,
      degreeToLocation(start, radius, center),
      degreeToLocation(end, radius, center),
      radius,
      flag
    );
  }
    ...
    if (series.segments.length > 0 && series.dataLabel.visible) {
      ...
      if (series.dataLabel.visible && series.dataLabel.position === 'Inside') {
        series.labelBound = new Rect(
          circularBound.x,
          circularBound.y,
          circularBound.width + circularBound.x,
  ...
  private getSeriesBound(series: CircularChart3DSeries): Rect {
    ...
    series.findMaxBounds(
      rect,
      new Rect(series.segments[0].center.x, series.segments[0].center.y, 0, 0)
    );
    ...
    const length: number =
      nextQuandrant === lastQuadrant ? 0 : Math.floor(total / 90);
  ...
  private getRectFromAngle(angle: number): Rect {
    const location: ChartLocation = degreeToLocation(
      ...
      this.visibleSeries[0].segments[0].center
    );
  ...
  private renderTitle(): void {
    ...
    const getAnchor: string = this.getTextAnchor(
  ...
  private renderSubTitle(options: TextOption): void {
    ...
    const subTitleOption: TextOption = new TextOption(
      ...
      options.y * options.text.length +
        ...
        padding,
      this.getTextAnchor(this.subTitleStyle.textAlignment, this.enableRtl),
  ...
  private calculateLegendBounds(): void {
    ...
    this.circularChartLegend3DModule.getLegendOptions(
      this,
      <CircularChart3DSeries[]>this.visibleSeries
    );
    this.circularChartLegend3DModule.calculateLegendBounds(
      this.initialClipRect,
      this.availableSize,
      null
    );
  ...
  public circular3DChartKeyDown(e: KeyboardEvent): boolean {
    ...
    if (
      ...
      ((e.code === 'Tab' && this.previousTargetId.indexOf('series') > -1) ||
        this.previousTargetId.indexOf('legend') > -1 ||
        e.code === 'Escape')
    ) {
      actionKey = 'ESC';
    }
    ...
    switch (actionKey) {
      ...
      case 'ArrowMove':
        if (targetId.indexOf('-point-') > -1) {
          ...
          const point: CircularChart3DPoints =
            this.visibleSeries[0].points[pointIndex as number];
          const center: CircularChart3DVector = point.symbolLocation.center;
          const dradius: number =
            point.symbolLocation.radius * this.visibleSeries[0].coefficient;
          const radius: number =
            dradius + (point.symbolLocation.radius - dradius) / 2;
          this.mouseX =
            center.x + radius * Math.cos(point.symbolLocation.angle);
          this.mouseY =
            center.y + radius * Math.sin(point.symbolLocation.angle);
      ...
      case 'Space':
        if (
          ...
          this.circularChartLegend3DModule
        ) {
  ...
  public circular3DChartKeyUp(e: KeyboardEvent): boolean {
    ...
    if (e.code === 'Tab') {
      if (this.previousTargetId !== '') {
        if (
          this.previousTargetId.indexOf('-point-') > -1 &&
          ...
          this.previousTargetId.indexOf('-title') > -1 &&
          targetId.indexOf('-point-') > -1
        ) {
      ...
      if (targetId.indexOf('_chart_legend_page') > -1) {
        ...
        for (let i: number = 0; i < this.visibleSeries[0].points.length; i++) {
          const point: CircularChart3DPoints =
            this.visibleSeries[0].points[i as number];
        ...
        this.currentPointIndex =
          seriesIndexes.indexOf(this.currentPointIndex) +
          (e.code === 'ArrowUp' || e.code === 'ArrowRight' ? 1 : -1);
        this.currentPointIndex =
          seriesIndexes[
            this.getActualIndex(this.currentPointIndex, seriesIndexes.length)
          ];
        const elements: NodeListOf<Element> = document.querySelectorAll(
          `[id*="region-series-0-point-${this.currentPointIndex}"]`
        );
        let element: Element;
        ...
        this.focusTarget(getElement(targetId) as HTMLElement);
        actionKey =
          this.tooltip.enable || this.circularChartHighlight3DModule
            ? 'ArrowMove'
            : '';
      }
    } else if (
  ...
  public destroy(): void {
    if (this.element) {
      ...
      const element: HTMLElement = document.getElementById(
        this.element.id + 'Keyboard_circular3dchart_focus'
      );
      if (element) {
        element.remove();
      }
  ...
  public chartResize(): boolean {
    ...
    const beforeResizeArgs: CircularChart3DBeforeResizeEventArgs = {
      cancel: false,
    };
    if (this.resizeTo) {
  ...
  public removeSeriesElements(chart: CircularChart3D): void {
    ...
    document
      .querySelectorAll('[id*="data-label-text-"]')
      .forEach(function (element: Element): void {
        if (element.parentElement.id === chart.groupElement.id) {
          return element.remove();
        }
    ...
    document
      .querySelectorAll('[id*="data-label-series-0-shape-"]')
      .forEach(function (element: Element): void {
        if (element.parentElement.id === chart.groupElement.id) {
          return element.remove();
        }
    ...
    document
      .querySelectorAll('[id*="datalabel-series-0-connector-"]')
      .forEach(function (element: Element): void {
        if (element.parentElement.id === chart.groupElement.id) {
          return element.remove();
        }
  ...
  public onPropertyChanged(
    newProp: CircularChart3DModel,
    oldProp: CircularChart3DModel
  ): void {
        ...
        case 'selectionMode':
          if (this.circularChartSelection3DModule) {
            if (
              ...
              this.circularChartSelection3DModule.selectedDataIndexes = this
        ...
        case 'tooltip':
          if (this.circularChartTooltip3DModule) {
            ...
            if (this.tooltip.template) {
              this.circularChartTooltip3DModule.template =
