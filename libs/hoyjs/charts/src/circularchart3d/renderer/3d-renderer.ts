...
import {
  CircularChart3DBasicTransform,
  CircularChart3DColorFormat,
  CircularChart3DDataElement,
  CircularChart3DLabelElement,
  CircularChart3DLocation,
  CircularChart3DPathOptions,
  CircularChart3DPolyAttributes,
  CircularChart3DPolyCollections,
  CircularChart3DPolygon,
  CircularChart3DStringBuilder,
  CircularChart3DVector,
} from '../model/circular3d-base';
...
export class CircularChart3DVectorModule {
   ...
   */
  constructor(
    pointX: { x: number; y: number } | number,
    vy?: number,
    vz?: number
  ) {
...
export class CircularChart3DMatrix {
  ...
  public getInterval(matrix: number[][]): number[][] {
    ...
    matrixData = this.getMatrixMultiple(
  ...
  public getMatrixMultiple(factor: number, matrix: number[][]): number[][] {
    for (let i: number = 0; i < matrix.length; i++) {
      for (let j: number = 0; j < matrix[i as number].length; j++) {
        matrix[i as number][j as number] =
          matrix[i as number][j as number] * factor;
      }
    }
    return matrix;
  }
  ...
  public getMatrixVectorMultiple(
    matrix: number[][],
  ...
  public getMatrixMultiplication(
    matrix1: number[][],
    matrix2: number[][]
  ): number[][] {
    ...
    for (let i: number = 0; i < this.matrixSize; i++) {
      for (let j: number = 0; j < this.matrixSize; j++) {
        ...
        for (let k: number = 0; k < this.matrixSize; k++) {
          value +=
            matrix1[k as number][j as number] *
            matrix2[i as number][k as number];
        }
  ...
  public getMinor(
    ...
    return (
      ((columnIndex + rowIndex) % 2 === 0 ? 1 : -1) *
      this.getDeterminant(this.getMatrix(matrix, columnIndex, rowIndex))
    );
  }
  ...
  public getMatrix(
    ...
    for (let i: number = 0; i < count; i++) {
      ...
      for (let j: number = 0; j < count; j++) {
        ...
        subMatrix[i as number][j as number] =
          matrix[matrixColumn as number][matrixRow as number];
      }
    }
    return subMatrix;
  }
  ...
  public getDeterminant(matrix: number[][]): number {
    ...
    if (count < 2) {
      ...
      for (let i: number = 0; i < count; i++) {
        ...
        determinant +=
          k * matrix[i as number][0] * this.getDeterminant(submatrix);
        k = k > 0 ? -1 : 1;
      }
    }
    return determinant;
  ...
  public transposed(matrix3D: number[][]): number[][] {
    ...
    for (let i: number = 0; i < this.matrixSize; i++) {
      for (let j: number = 0; j < this.matrixSize; j++) {
        transposedMatrix[i as number][j as number] =
          matrix3D[j as number][i as number];
      }
    }
...
export class CircularChart3DTransform {
  ...
  public transform(transform: CircularChart3DBasicTransform): void {
    this.setCenter(
      this.vector.vector3D(
        transform.viewingArea.width / 2,
        transform.viewingArea.height / 2,
        transform.depth / 2
      ),
      transform
    );
    this.setViewMatrix(
      this.matrixObj.transform(0, 0, transform.depth),
      transform
    );
    this.setViewMatrix(
      this.matrixObj.getMatrixMultiplication(
        transform.viewMatrix,
        this.matrixObj.turn(-this.toRadial * transform.rotation)
      ),
      transform
    );
    this.setViewMatrix(
      this.matrixObj.getMatrixMultiplication(
        transform.viewMatrix,
        this.matrixObj.tilt(-this.toRadial * transform.tilt)
      ),
      transform
    );
  ...
  private updatePerspective(
    ...
    const width: number =
      (transform.viewingArea.width + transform.viewingArea.height) *
        Math.tan(
          this.degreeToRadianConverter((180 - Math.abs(angle % 181)) / 2.0)
        ) +
      (transform.depth * 2) / 2;
  ...
  public toScreen(
    ...
    if (!chartObj) {
      ...
      vector3D = this.matrixObj.getMatrixVectorMultiple(
      ...
      vector3D = chartObj.getMatrixVectorMultiple(
        this.result(transform, chartObj),
        vector3D
      );
    }
  ...
  public result(
    ...
    let chartObj: CircularChart3DMatrix = transform.chartObj
      ? transform.chartObj
      : this.matrixObj;
    if (!chartObj) {
      chartObj = matrixobj;
    ...
    transform.resultMatrix = chartObj.getMatrixMultiplication(
      matrixObj.getInterval(transform.centeredMatrix),
...
export class CircularChart3DGraphics {
  /** The vector class. */
  private vector: CircularChart3DVectorModule = new CircularChart3DVectorModule(
    0,
    0,
    0
  );
  ...
  public prepareView(
    ...
    if (arguments.length === 0) {
      ...
      circular3DRender.tree[chart.groupElement.id] = bspTreeObj.build(
        null,
        chart
      );
    }
  ...
  public view(
    ...
    if (arguments.length === 2) {
      ...
      const eyeVector: CircularChart3DVector = this.vector.vector3D(
        0,
        0,
        MaxValue
      );
      this.drawNode3D(
        circular3DRender.tree[chart.groupElement.id],
        eyeVector,
        panel,
        chart
      );
   ...
   * @returns {void}
   */
  public draw3DElement(
    bspElement: CircularChart3DBspNode,
    chart: CircularChart3D
  ): void {
    if (bspElement.plane.element) {
      if (
        bspElement.plane.element.tag === 'text' ||
  ...
  public drawNode3D(
    bspElement: CircularChart3DBspNode,
    eyeVector: CircularChart3DVector,
    panel: Element,
    chart: CircularChart3D
  ): void {
    ...
    while (isVector) {
      const r: number = vector.vector3DAdd(
        polygonObj.getNormal(
          chart.transform3D.result(circular3DRender.transform),
          bspElement.plane.vectorPoints
        ),
        eyeVector
      );
...
export class CircularChart3DBinaryTreeBuilder {
   ...
   */
  public vector3DIndexClassification(
    point: CircularChart3DVector,
    index: number,
    result: string
  ): CircularChart3DPolyAttributes {
   ...
   */
  public classifyPoint(
    point: CircularChart3DVector,
    polygon: CircularChart3DPolygon
  ): string {
    let result: string = 'OnPlane';
    const signedDistance: number =
      -polygon.d - vector.vector3DAdd(point, polygon.normal);
  ...
  public classifyPolygon(
    ...
    for (let i: number = 0, len: number = points.length; i < len; i++) {
      const value: number =
        -polygonValue - vector.vector3DAdd(points[i as number], normal);
  ...
  private splitPolygon(
    splitPolygon: CircularChart3DPolygon,
    refPolygon: CircularChart3DPolygon
  ): CircularChart3DPolyCollections {
    ...
    if (splitPolygon.points != null) {
      ...
      for (let i: number = 0; i < count; i++) {
        ...
        const pointC: CircularChart3DVector =
          splitPolygon.points[this.getNext(i + 1, count)];
        const sideB: string = this.classifyPoint(pointB, refPolygon);
        const sideC: string = this.classifyPoint(pointC, refPolygon);
        const attributeB: CircularChart3DPolyAttributes =
          this.vector3DIndexClassification(pointB, polyPoints.length, sideB);
        polyPoints.push(attributeB);
        if (sideB !== sideC && sideB !== 'OnPlane' && sideC !== 'OnPlane') {
          const vectorValue: CircularChart3DVector = vector.vector3DMinus(
            pointB,
            pointC
          );
          const direction: CircularChart3DVector = vector.vector3DMinus(
            vector.vector3DStarMultiply(refPolygon.normal, -refPolygon.d),
            pointC
          );

          const signedDistance: number = vector.vector3DAdd(
            direction,
            refPolygon.normal
          );
          const intersectionParameter: number =
            signedDistance / vector.vector3DAdd(refPolygon.normal, vectorValue);
          const intersectionPoint: CircularChart3DVector = vector.vector3DPlus(
            pointC,
            vector.vector3DStarMultiply(vectorValue, intersectionParameter)
          );
          const attributeIntersection: CircularChart3DPolyAttributes =
            this.vector3DIndexClassification(
              intersectionPoint,
              polyPoints.length,
              'OnPlane'
            );
      ...
      if (frontPartPoints.length !== 0 || backPartPoints.length !== 0) {
        for (let i: number = 0; i < backPartPoints.length - 1; i += 2) {
          const backAttribute1: CircularChart3DPolyAttributes =
            backPartPoints[i as number];
          const backAttribute2: CircularChart3DPolyAttributes =
            backPartPoints[i + 1];
        ...
        for (let i: number = 0; i < frontPartPoints.length - 1; i += 2) {
          const frontAttribute1: CircularChart3DPolyAttributes =
            frontPartPoints[i as number];
          const frontAttribute2: CircularChart3DPolyAttributes =
            frontPartPoints[i + 1];
        ...
        for (let i: number = 0; i < backPartPoints.length - 1; i++) {
          ...
          if (outputs.length > 2) {
            const polygon1: CircularChart3DPolygon = polygonObj.polygon3D(
              outputs,
              splitPolygon
            );
        ...
        for (let i: number = 0; i < frontPartPoints.length - 1; i++) {
          ...
          if (inputs.length > 2) {
            const polygon2: CircularChart3DPolygon = polygonObj.polygon3D(
              inputs,
              splitPolygon
            );
  ...
  private cutOutFrontPolygon(
    polyPoints: CircularChart3DPolyAttributes[],
    initialVertex: CircularChart3DPolyAttributes
  ): CircularChart3DVector[] {
    ...
    while (isVector) {
      ...
      const currentVertexPair: CircularChart3DPolyAttributes =
        polyPoints[currentVertex.alterCuttingFrontPairIndex];

      if (currentVertex.cuttingFrontPoint) {
        if (!currentVertexPair.alreadyCutFront) {
          currentVertex = currentVertexPair;
        } else {
          const previousVertexOnBack: CircularChart3DPolyAttributes =
            polyPoints[
              this.getNext(currentVertex.index - 1, polyPoints.length)
            ];
          const nextVertexOnBack: CircularChart3DPolyAttributes =
            polyPoints[
              this.getNext(currentVertex.index + 1, polyPoints.length)
            ];

          if (
            previousVertexOnBack.result === 'OnFront' &&
            !previousVertexOnBack.alreadyCutFront
          ) {
            currentVertex = previousVertexOnBack;
          } else if (
            nextVertexOnBack.result === 'OnFront' &&
            !nextVertexOnBack.alreadyCutFront
          ) {
        ...
        const previousVertexOnBack: CircularChart3DPolyAttributes =
          polyPoints[this.getNext(currentVertex.index - 1, polyPoints.length)];
        const nextVertexOnBack: CircularChart3DPolyAttributes =
          polyPoints[this.getNext(currentVertex.index + 1, polyPoints.length)];

        if (
          previousVertexOnBack.result !== 'OnBack' &&
          !previousVertexOnBack.alreadyCutFront
        ) {
          currentVertex = previousVertexOnBack;
        } else if (
          nextVertexOnBack.result !== 'OnBack' &&
          !nextVertexOnBack.alreadyCutFront
        ) {
          currentVertex = nextVertexOnBack;
        } else {
  ...
  private cutOutBackPolygon(
    ...
    while (isVector) {
      ...
      const currentVertexPair: CircularChart3DPolyAttributes =
        polyPoints[currentVertex.alterCuttingBackPairIndex];
      ...
      if (currentVertex.cuttingBackPoint) {
        if (!currentVertexPair.alreadyCutBack) {
          ...
          if (
            previousVertexOnBack.result === 'OnBack' &&
            !previousVertexOnBack.alreadyCutBack
          ) {
            currentVertex = previousVertexOnBack;
          } else if (
            nextVertexOnBack.result === 'OnBack' &&
            !nextVertexOnBack.alreadyCutBack
          ) {
            currentVertex = nextVertexOnBack;
          } else {
            return points;
   ...
   */
  public build(
    points?: CircularChart3DPolygon[],
    chart?: CircularChart3D
  ): CircularChart3DBspNode {
    if (!points) {
      ...
      const bspNode: CircularChart3DBspNode = {
        back: null,
        front: null,
        plane: null,
      };
      ...
      for (
        let i: number = 1, len: number = inputPolygons.length;
        i < len;
        i++
      ) {
        const currentPolygon: CircularChart3DPolygon =
          inputPolygons[i as number];

        if (currentPolygon === plane) {
          continue;
        }
        const classificationResult: string = this.classifyPolygon(
          plane,
          currentPolygon
        );
        ...
        switch (classificationResult) {
          ...
          case 'Unknown':
            if (currentPolygon.element) {
              ...
              const result: CircularChart3DPolyCollections = this.splitPolygon(
                currentPolygon,
                plane
              );
              for (let k: number = 0; k < result.backPolygon.length; k++) {
                result.backPolygon[k as number].name =
                  result.backPolygon[k as number].name + '-back';
                polygonsToLeft.push(result.backPolygon[k as number]);
              }
              for (let j: number = 0; j < result.frontPolygon.length; j++) {
                result.frontPolygon[j as number].name =
                  result.frontPolygon[j as number].name + '-front';
...
export class CircularChart3DSvgRenderer {
  ...
  public drawText(
    ...
    let text: Element =
      ...
      chart.groupElement.querySelector('#' + options.id);
    if (text === null) {
  ...
  public hexColor(color: CircularChart3DColorFormat): string {
    ...
    if (color.alpha) {
      const returnColor: string = `rgba(${redValue.toString()},${greenValue.toString()},${blueValue.toString()},${
        color.alpha
      })`;
      return returnColor;
    } else {
...
export class CircularChart3DPolygonModule {
  ...
  public polygon3D(
    ...
    if (arguments.length === 2) {
      ...
      this.vectorPoints = points;
      this.calculateNormal(this.vectorPoints);
      const element: { tag: string; parent: Element } = {
        tag: 'path',
        parent: parent,
      };
  ...
  public calculateNormal(...args: any[]): void {
    if (args.length >= 3) {
      ...
      const vector4: CircularChart3DVector = this.vector.vector3DMinus(
        vector1,
        vector2
      );
      const vector5: CircularChart3DVector = this.vector.vector3DMinus(
        vector3,
        vector2
      );
      const normal: CircularChart3DVector = this.vector.vector3DMultiply(
        vector4,
        vector5
      );
      ...
      this.normal = this.vector.vector3D(
        normal.x / length,
        normal.y / length,
        normal.z / length
      ); // Calculate normalization of the vector
      this.d = -(
        this.normal.x * vector1.x +
        this.normal.y * vector1.y +
        this.normal.z * vector1.z
      ); // Normalized values * 1st coordinates Coordinates - Depth of the plan
  ...
  public getNormal(
    ...
    if (vectorPoints != null) {
      normal = this.vector.getNormal(
        this.matrixObj.getMatrixVectorMultiple(transform, vectorPoints[0]),
      ...
      for (
        let i: number = 3;
        i < vectorPoints.length &&
        !this.vector.isValid(normal) &&
        vectorPoints[i / 2];
        i++
      ) {
        const v1: CircularChart3DVector = matrixObj.getMatrixVectorMultiple(
          transform,
          vectorPoints[i as number]
        );
        const v2: CircularChart3DVector = matrixObj.getMatrixVectorMultiple(
          transform,
          vectorPoints[0]
        );
        const v3: CircularChart3DVector = matrixObj.getMatrixVectorMultiple(
          transform,
          vectorPoints[i / 2]
        );
  ...
  public createTextElement(
    position: CircularChart3DVector,
    element: CircularChart3DLabelElement,
    xLength: number,
    yLength: number
  ): CircularChart3DPolygon {
    ...
    vectorCollection[1] = this.vector.vector3D(
      x + desiredWidth,
      y + desiredHeight + yLength,
      position.z
    );
    vectorCollection[2] = this.vector.vector3D(
      x + desiredWidth + xLength,
      y + desiredHeight + yLength,
      position.z
    );
  ...
  public drawPolyLine(
    panel: CircularChart3DPolygon,
    chart: CircularChart3D
  ): void {
    const transform: CircularChart3DBasicTransform = circular3DRender.transform;
    const pathDirection: CircularChart3DStringBuilder =
      chart.svg3DRenderer.getStringBuilder();
    const startPoint: CircularChart3DLocation = chart.transform3D.toScreen(
      panel.vectorPoints[0],
      transform
    );
    pathDirection.append(`M ${startPoint.x} ${startPoint.y} `);

    const lineSegment1: CircularChart3DLocation = chart.transform3D.toScreen(
      panel.vectorPoints[1],
      transform
    );
    pathDirection.append(`L ${lineSegment1.x} ${lineSegment1.y} `);
    pathDirection.append(`M ${lineSegment1.x} ${lineSegment1.y} `);

    const lineSegment2: CircularChart3DLocation = chart.transform3D.toScreen(
      panel.vectorPoints[2],
      transform
    );
  ...
  public drawText(panel: CircularChart3DPolygon, chart: CircularChart3D): void {
    ...
    const actual3DPosition: CircularChart3DLocation =
      chart.transform3D.toScreen(panel.vectorPoints[0], transform);
    ...
    if (element.tag === 'text') {
      const options: any = {
        ...
        'font-family':
          element.font.fontFamily || chart.themeStyle.datalabelFont.fontFamily,
      ...
      if (chart.previousID && chart.isTouch) {
        const previousElement: HTMLElement = document.getElementById(
          chart.previousID
        );
      ...
      const textOffset: Size = measureText(
        pointText,
        element.point.argsData.font,
      ...
      const yValue: number =
        location.y - padding / 2 - height / padding + padding / 2;
      const seriesIndex: number = series.index;

      this.dataLabelSymbol(
        seriesIndex,
        series,
        element,
        xXalue,
  ...
  private dataLabelSymbol(
    ...
    dataElement: CircularChart3DLabelElement,
    x: number,
    y: number,
    width: number,
    height: number,
    ...
    if (series.dataLabel.enableRotation) {
      ...
      if (angle === 0) {
        const toDegrees: (angle: number) => number = (angle: number) =>
          angle * (180 / Math.PI);
        const midAngle: number = toDegrees(
          dataElement.point.symbolLocation.angle
        );
    ...
    if (chart.previousID && chart.isTouch) {
      ...
      const currentElement: HTMLElement = document.getElementById(
        rectOptions.id
      );
      if (previousElement && currentElement) {
        currentElement.parentNode.insertBefore(
  ...
  public draw(panel: CircularChart3DPolygon, chart: CircularChart3D): void {
    ...
    const figure: { StartPoint: CircularChart3DLocation } = {
      StartPoint: null,
    };
    if (transform != null) {
      figure.StartPoint = chart.transform3D.toScreen(
        panel.vectorPoints[0],
        transform
      );
      pathDirection.append(
        'M' + ' ' + figure.StartPoint.x + ' ' + figure.StartPoint.y + ' '
      );
      for (let i: number = 0; i < panel.vectorPoints.length; i++) {
        const lineSegment: CircularChart3DLocation = chart.transform3D.toScreen(
          panel.vectorPoints[i as number],
          transform
        );
        pathDirection.append(
          'L' + ' ' + lineSegment.x + ' ' + lineSegment.y + ' '
        );
      }
    }
    const direction: string = pathDirection.toString();
    let name: string = 'Light';
    const lightCoefficientZ: number =
      2 *
      (Math.abs(
        this.vector.vector3DAdd(panel.normal, this.vector.vector3D(0, 0, 1))
      ) -
        1);
    const lightCoefficientY: number =
      2 *
      (Math.abs(
        this.vector.vector3DAdd(panel.normal, this.vector.vector3D(0, 1, 0))
      ) -
        1);
    const lightCoefficientX: number =
      2 *
      (Math.abs(
        this.vector.vector3DAdd(panel.normal, this.vector.vector3D(1, 0, 0))
      ) -
        1);
    if (panel.name && !(panel.name.indexOf('-wall-brush') > -1)) {
      if (lightCoefficientZ === lightCoefficientX) {
        ...
        lightCoefficientY === lightCoefficientZ ||
        (lightCoefficientZ !== 0 && lightCoefficientY < lightCoefficientZ)
      ) {
        name = 'XLight';
  ...
  public drawTemplate(
    ...
    chart.circularChartDataLabel3DModule.createDataLabelTemplate(
      label.series.dataLabelElement,
      label.series,
      label.series.dataLabel,
  ...
  public applyXLight(color: string, chart: CircularChart3D): string {
    const RGB: CircularChart3DColorFormat =
      chart.svg3DRenderer.hexToValue(color);
...
const vector: CircularChart3DVectorModule = new CircularChart3DVectorModule(
  0,
  0,
  0
);
const matrixObj: CircularChart3DMatrix = new CircularChart3DMatrix();
const bspTreeObj: CircularChart3DBinaryTreeBuilder =
  new CircularChart3DBinaryTreeBuilder();
const polygonObj: CircularChart3DPolygonModule =
  new CircularChart3DPolygonModule();
