...
import { Rect, Size } from '@hoyui/svg-base';
...
import {
  ...
  CircularChart3DSegments,
  CircularChart3DSeriesStyle,
...
export class CircularChart3DSeries extends ChildProperty<CircularChart3DSeries> {
   ...
   */
  @Complex<CircularChart3DDataLabelSettingsModel>(
    {},
   ...
   */
  @Complex<CircularChart3DEmptyPointSettingsModel>(
    { mode: 'Drop' },
  ...
  public refreshDataManager(chart: CircularChart3D, render: boolean): void {
    ...
    dataManager.then((e: { result: Object; count: number }) =>
      this.dataManagerSuccess(e, chart)
    );
  }
  ...
  private getPoints(result: Object, chart: CircularChart3D): void {
    ...
    const colors: string[] = this.palettes.length
      ? this.palettes
      : getCircular3DSeriesColor(chart.theme);
  ...
  private setPoints(
    ...
    point.tooltip = getValue(
      this.tooltipMappingName || '',
      data[index as number]
    );
    point.sliceRadius = getValue(
  ...
  public draw(series: CircularChart3DSeries, chart: CircularChart3D): void {
    ...
    const seriesRadius: string = series.radius
      ? series.radius
      : Browser.isDevice && series.dataLabel.position === 'Outside'
      ? '40%'
      : '80%';
    ...
    if (
      !(
        (seriesRadius.indexOf('%') !== -1 || typeof radius === 'number') &&
        !isNaN(radius)
      )
    ) {
      ...
      Math.min(
        series.actualWidth / 2 -
          ...
          (chart.legendSettings.position === 'Right' ||
            ...
            ? chart.circularChartLegend3DModule.legendBounds.width
            : 0),
        series.actualHeight / 2
      ) *
      series.size;
  ...
  private calculateSize(
    ...
    const legendPosition: string = chart.circularChartLegend3DModule
      ? chart.circularChartLegend3DModule.position
      : 'Auto';
    ...
    const subTitleLocation: CircularChart3DTitlePosition =
      chart.subTitleLocation;
    ...
    const titleEnable: boolean =
      title && titleTextOverflow === 'Wrap' ? true : false;

    const subTitleEnable: boolean =
      subTitle && subTitleTextOverflow === 'Wrap' ? true : false;
    ...
    if (
      ...
      if (legendPosition === 'Right' || legendPosition === 'Left') {
        legXSpace =
          legendPosition.toLowerCase() === 'right'
            ? chart.margin.right
            : chart.margin.left +
              chart.circularChartLegend3DModule.legendBounds.width;
      } else {
        legYSpace =
          legendPosition === 'Top'
            ...
            ? chart.margin.bottom +
              chart.circularChartLegend3DModule.legendBounds.height
            : 0;
      }
    }
    ...
    const centerx: number =
      series.actualWidth * 0.5 + (legendPosition === 'Left' ? legXSpace : 0);

    if (titleEnable || subTitleEnable) {
      const yOffset: number =
        titleLocation.size.height +
        (subTitleEnable ? subTitleLocation.size.height : 0) +
        legYSpace;
      series.actualHeight =
        (chart.svgObject as SVGSVGElement).height.baseVal.value - yOffset;
      const centery: number =
        series.actualHeight * 0.5 +
        (titleLocation.size.height +
          (subTitleEnable ? subTitleLocation.size.height : 0));
      ...
      const centery: number =
        series.actualHeight * 0.5 +
        (legendPosition === 'Top' ? yOffset : title ? titleLocation.y : 0);
      return { centerX: centerx, centerY: centery };
    }
  }
  ...
  private createPolygons(
        ...
        if (polygonCollection !== null) {
          for (let k: number = 0; k < polygonCollection.length; k++) {
            ...
            if (polygonCollection[k as number] !== null) {
              for (
                let j: number = 0;
                j < polygonCollection[k as number].length;
                j++
              ) {
                polygons[k as number].push(
                  polygonCollection[k as number][j as number]
                );
              }
            }
          }
    ...
    for (let i: number = 0; i < polygons.length; i++) {
      for (let k: number = 0; k < polygons[i as number].length; k++) {
        const polygone: CircularChart3DPolygon =
          polygons[i as number][k as number];
  ...
  private createSector(
    segment: CircularChart3DSegments,
    chart: CircularChart3D,
    style: CircularChart3DSeriesStyle,
    seriesIndex: number
  ): CircularChart3DPolygon[][] {
    ...
    for (let i: number = 0; i < count + 1; i++) {
      const ox: number =
        ...
        segment.radius *
          Math.cos(
            (segment.actualStartValue + i * endValue) * degreesToRadians
          );
      const oy: number =
        segment.center.y +
        segment.radius *
    ...
    for (let i: number = 0; i < count; i++) {
      vts = [
        this.vector(
          outputPoints[i as number].x,
          outputPoints[i as number].y,
          0
        ),
        this.vector(
          outputPoints[i as number].x,
    ...
    if (segment.inSideRadius > 0) {
      ...
      for (let i: number = 0; i < count; i++) {
        vts = [
          this.vector(
            inputPoints[i as number].x,
            inputPoints[i as number].y,
            0
          ),
          this.vector(
            inputPoints[i as number].x,
    ...
    if (segment.inSideRadius > 0) {
      ...
      const lvts: CircularChart3DVector[] = [
        this.vector(
          outputPoints[count as number].x,
          outputPoints[count as number].y,
          0
        ),
        this.vector(
          outputPoints[count as number].x,
        ...
        this.vector(
          inputPoints[count as number].x,
          inputPoints[count as number].y,
          depth
        ),
        this.vector(
          inputPoints[count as number].x,
  ...
  private createPoints(
    ...
    series.segments = [];
    const size: { centerX: number; centerY: number } = this.calculateSize(
      chart,
      series
    );
    let all: number = 0;
    ...
    for (let j: number = 0; j < count; j++) {
      if (
        visiblePoints[j as number].visible &&
        !isNullOrUndefined(visiblePoints[j as number].y)
      ) {
    ...
    for (let i: number = 0; i < count; i++) {
      if (
        series.points[i as number].visible &&
        !isNullOrUndefined(yValues[i as number])
      ) {
        ...
        if (series.isRadiusMapped) {
          if (series.points[i as number].sliceRadius.indexOf('%') !== -1) {
            point.radius = stringToNumber(
              series.points[i as number].sliceRadius,
              this.size / 2
            );
          } else {
        ...
        const accessibilityText: string =
          point.x + ':' + point.y + ', ' + series.name;
        if (!isNullOrUndefined(value)) {
          if (
            series.explode &&
            (series.explodeIndex === point.index ||
            ...
            rect.x =
              ...
              point.radius *
              offset.X *
              stringToNumber(series.explodeOffset, 100);
            rect.y =
              0.01 *
              point.radius *
              offset.Y *
        ...
        this.addPieDataLabel(
          ...
          point.radius,
          this.isChartRotated(chart) ? chart.depth + 5 : 0,
          center,
          series.points[i as number]
        );
        segIndex++;
        arcStartAngle += arcEndAngle;
        current += value;
  ...
  private isChartRotated(chart: CircularChart3D): boolean {
    ...
    if (
      (actualTiltView > 90 && actualTiltView < 270) !==
      (actualRotateView > 90 && actualRotateView < 270)
    ) {
      return true;
    }
    return false;
