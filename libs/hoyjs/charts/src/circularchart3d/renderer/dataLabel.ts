...
import { Size, measureText, Rect } from '@hoyui/svg-base';
...
export class CircularChart3DDataLabelSettings extends ChildProperty<CircularChart3DDataLabelSettings> {
   ...
   */
  @Complex<CircularChart3DDataLabelFontModel>(
    {
      fontFamily: null,
      size: '12px',
      fontStyle: 'Normal',
...
export class CircularChartDataLabel3D extends ChildProperty<CircularChartDataLabel3D> {
  ...
  public renderDataLabel(
    ...
    series.labelBound = isNullOrUndefined(series.labelBound)
    ...
    for (let i: number = 0; i < series.points.length; i++) {
      ...
      const pointText: string = this.getDatalabelText(
        series.dataLabel.format,
        chart,
        point.text
          ? point.text
      ...
      const argsFont: FontModel = <FontModel>(
        extend({}, getValue('properties', series.dataLabel.font), null, true)
      );
    ...
    if (series.dataLabel.template) {
      appendChildElement(
        false,
        document.getElementById(chart.element.id + '_Secondary_Element'),
        series.dataLabelElement,
        chart.redraw,
        false,
        'x',
        'y',
  ...
  public createDataLabelTemplate(
    ...
    data: CircularChart3DTextRenderEventArgs,
    labelIndex: number,
    redraw: boolean,
    ...
    const childElement: HTMLElement = this.createTemplate(
      createElement('div', {
        ...
        styles:
          ...
          data.color +
          ';' +
          getFontStyle(dataLabel.font, chart.themeStyle.datalabelFont) +
          ';border:' +
          data.border.width +
          'px solid ' +
          data.border.color +
  ...
  private createTemplate(
    ...
    try {
      ...
      const elementData: Element[] = templateFn
        ? templateFn(tempObject, chart, templateId, dataLabelId)
        : [];
  ...
  private draw3DDataLabel(
    ...
    let connectorHeight: number | string = series.dataLabel.connectorStyle
      .length
      ? parseFloat(series.dataLabel.connectorStyle.length)
      : series.dataLabel.connectorStyle.length;
    let radius: number;
    let pointX: number;
    let pointY: number;
    let degree: number;
    ...
    pointX = location.x =
      center.x +
      parseFloat(point.argsData.font.size) / 3 +
      radius * Math.cos(point.symbolLocation.angle);
    pointY = location.y =
      center.y +
      parseFloat(point.argsData.font.size) / 3 +
      radius * Math.sin(point.symbolLocation.angle);
    ...
    const saturationColor: string = this.getSaturatedColor(
      point,
      point.argsData.color,
      chart
    );
    const element: CircularChart3DLabelElement = {
      width: size.width,
      height: size.height,
    ...
    if (
      chart.circularChartLegend3DModule &&
      chart.legendSettings.visible &&
      point.visible &&
      ...
      rect = new Rect(
        rect.x - legendpadding,
        rect.y - legendpadding,
        rect.width + 2 * legendpadding,
        rect.height + 2 * legendpadding
      );
      const labelRegion: Rect = new Rect(
        element.label.location.x + size.width / 2 + 20,
        element.label.location.y + 2.5,
        element.width,
        element.height
      );
      if (isOverlap(labelRegion, rect)) {
        if (chart.circularChartLegend3DModule.position === 'Right') {
          ...
          if (element.width < 0) {
            ...
            element.label.location.x =
              rect.x + rect.width - size.width / 2 + 20;
          }
        }
        if (labelRegion && element.width < labelRegion.width) {
          element.label.text = textTrim(
            element.width,
            element.label.text,
            series.dataLabel.font,
            chart.enableRtl,
        ...
        if (
          element.label.text.length === 3 &&
          element.label.text.indexOf('...') > -1
        ) {
          return;
        }
      }
    ...
    if (series.dataLabel.position !== 'Inside') {
      connectorPoints = this.updateConnectorLine(
        point,
        pointIndex,
        series,
        connectorHeight as number,
        chart
      );
    }
      ...
      const childElement: HTMLElement = this.createTemplate(
        createElement('div', {
          ...
          styles:
            ...
            point.argsData.color +
            ';' +
            getFontStyle(point.argsData.font, chart.themeStyle.datalabelFont) +
            ';border:' +
            point.argsData.border.width +
            'px solid ' +
            point.argsData.border.color +
            ';',
        }),
        point.argsData.template,
        chart,
        point,
        series,
        chart.element.id + '-data-label-'
    ...
    if (series.dataLabel.position !== 'Inside') {
      if (
        (textAngle > 1.5 && textAngle < 1.8) ||
        (textAngle > 1.3 && textAngle < 1.5) ||
        (textAngle > 4.5 && textAngle < 4.8) ||
        (textAngle > 4.3 && textAngle < 4.5)
      ) {
      ...
      if (textAngle < Math.PI / 2 || textAngle >= Math.PI / 2 + Math.PI) {
        ...
        padding = -(
          size.width / 2 +
          (point.argsData.color !== 'transparent' || point.argsData.border.color
            ? 25
            : 20)
        );
        heightPadding = 5 / 2;
    ...
    if (!point.argsData.template && commonEventArgs.data.text !== '') {
      ...
      if (series.dataLabel.enableRotation) {
        ...
        transform =
          'rotate(' + degree + ',' + location.x + ',' + location.y + ')';
      }
      element.transform = transform;
      const borderElement: CircularChart3DPolygon =
        chart.polygon.createTextElement(
          chart.vector.vector3D(
            pointX + padding,
            pointY + heightPadding,
            point.symbolLocation.z ? point.symbolLocation.z : 0
          ),
          element,
          0,
          -size.height
    ...
    element.angle = series.dataLabel.enableRotation
      ? series.dataLabel.angle !== 0
        ? series.dataLabel.angle
        : degree
      : 0;
    const polygon: CircularChart3DPolygon = chart.polygon.createTextElement(
      chart.vector.vector3D(
        location.x + padding,
        location.y + heightPadding,
        -1
      ),
      element,
      0,
  ...
  private getSaturatedColor(
    ...
    saturatedColor =
      color === 'transparent' ? this.getLabelBackground(point, chart) : color;

    saturatedColor =
      saturatedColor === 'transparent'
        ? chart.theme.indexOf('Dark') > -1 ||
          chart.theme.indexOf('HighContrast') > -1
          ? 'black'
          : 'white'
        : saturatedColor;
  ...
  private getLabelBackground(
    ...
    return point.argsData.series.dataLabel.position === 'Outside'
      ? chart.background || chart.themeStyle.background
      : !point.y
      ? chart.theme.indexOf('dark')
        ? 'white'
        : 'black'
      : point.color;
  }
  ...
  private updateConnectorLine(
    ...
    let x: number =
      symbolLocation.center.x +
      Math.cos(symbolLocation.angle) * symbolLocation.radius;
    let y: number =
      symbolLocation.center.y +
      Math.sin(symbolLocation.angle) * symbolLocation.radius;
    drawPoints.push({ x, y });

    const labelRadiusFromOrigin: number =
      symbolLocation.radius + connectorHeight;
  ...
  private drawLineSegment(
    drawpoints: CircularChart3DLocation[],
    pointIndex: number,
    series: CircularChart3DSeries,
    chart: CircularChart3D
  ): void {
    ...
    const stroke: string = series.dataLabel.connectorStyle.color
      ? series.dataLabel.connectorStyle.color
      : color;
    ...
    const ploygon: CircularChart3DPolygon = chart.polygon.createPolyline(
      vectorPoints,
      line
    );
