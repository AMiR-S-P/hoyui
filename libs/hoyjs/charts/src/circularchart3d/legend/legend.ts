...
export class CircularChartLegend3D extends BaseLegend {
  ...
  private mouseMove(e: MouseEvent): void {
    if (this.chart.legendSettings.visible && !this.chart.isTouch) {
      this.move(e);
      if (
        (<CircularChart3D>this.chart).circularChartHighlight3DModule &&
        ((<CircularChart3D>this.chart).highlightMode !== 'None' ||
          (<CircularChart3D>this.chart).legendSettings.enableHighlight)
      ) {
        ...
        for (const id of legendItemsId) {
          if (targetId.indexOf(id) > -1) {
            if ((<CircularChart3D>this.chart).circularChartSelection3DModule) {
              (<CircularChart3D>(
                this.chart
              )).circularChartSelection3DModule.legendSelection(
                <CircularChart3D>this.chart,
                e.target as Element,
                e.type
              );
            } else if (
              ...
              (<CircularChart3D>(
                this.chart
              )).circularChartHighlight3DModule.legendSelection(
                <CircularChart3D>this.chart,
                e.target as Element,
                e.type
              );
        ...
        if (!isNullOrUndefined(point.x) && !isNullOrUndefined(point.y)) {
          this.legendCollections.push(
            new LegendOptions(
              point.x.toString(),
              point.color,
              series[i as number].legendShape,
              point.visible,
              'Pie',
              series[0].legendImageUrl,
              null,
              null,
              point.index,
              series[i as number].index
            )
          );
        }
      }
  ...
  public getLegendBounds(
    ...
    let maximumWidth: number = legend.maximumLabelWidth
      ? legend.maximumLabelWidth
      : 0;
    ...
    for (let i: number = 0; i < this.legendCollections.length; i++) {
      ...
      legendOption.text =
        legendEventArgs.text.indexOf('&') > -1
          ? this.convertHtmlEntities(legendEventArgs.text)
      ...
      if (legendOption.render && legendOption.text !== '') {
        ...
        if (this.isVertical) {
          ...
          if (
            columnHeight +
              this.itemPadding +
              arrowHeight / this.pageButtonSize >
            legendBounds.height
          ) {
          ...
          this.columnHeights[columnCount as number] =
            (this.columnHeights[columnCount as number]
              ? this.columnHeights[columnCount as number]
              : 0) +
            legendOption.textSize.height +
          ...
          this.rowHeights[rowCount as number] = Math.max(
            this.rowHeights[rowCount as number]
              ? this.rowHeights[rowCount as number]
              : 0,
            legendOption.textSize.height
          );
          ...
          if (!legend.enablePages) {
            ...
            titlePlusArrowSpace =
              this.isTitle && titlePosition !== 'Top'
          ...
          if (legendBounds.width < padding + rowWidth + titlePlusArrowSpace) {
            maximumWidth = Math.max(
              maximumWidth,
              rowWidth + padding + titlePlusArrowSpace - legendWidth
            );
          ...
          this.rowHeights[len as number] = Math.max(
            this.rowHeights[len as number] ? this.rowHeights[len as number] : 0,
            legendOption.textSize.height
          );
          this.columnHeights[columnCount as number] =
            (this.columnHeights[columnCount as number]
    ...
    if (this.isVertical) {
      ...
      columnHeight =
        Math.max.apply(null, this.columnHeights) +
        padding +
        arrowHeight +
        titleHeight;
      columnHeight = Math.max(
        columnHeight,
        (this.totalPages || 1) * (this.maxItemHeight + padding) +
          padding +
          arrowHeight
      );
      ...
      columnHeight =
        !legend.enablePages && this.isPaging
          ? this.maxItemHeight + padding + padding + titleHeight
          : columnHeight;
      columnHeight = Math.max(
        columnHeight,
        this.maxItemHeight + padding + padding + titleHeight
      ...
      if (!this.isPaging) {
        ...
        rowWidth +=
          this.isTitle && titlePosition !== 'Top'
            ? this.fivePixel + this.legendTitleSize.width + this.fivePixel
            : 0;
      }
    }
    this.maxColumns = 0; // initialization for max columns
    const width: number = this.isVertical
      ? this.getMaxColumn(
          columnWidth,
          legendBounds.width,
          padding,
  ...
  public getLegendHeight(
    ...
    const textPadding: number =
      legend.shapePadding + padding * 2 + legend.shapeWidth;
    switch (legend.textWrap) {
      case 'Wrap':
      case 'AnyWhere':
        if (
          legendWidth > legend.maximumLabelWidth ||
          legendWidth + rowWidth > bounds.width
        ) {
          option.textCollection = textWrap(
            option.text,
            legend.maximumLabelWidth
              ? Math.min(legend.maximumLabelWidth, bounds.width - textPadding)
    ...
    if (this.isPaging && this.isVertical) {
      for (let i: number = 1; i < columnCount; i++) {
        ...
        if (columnWidth > width) {
          this.pageHeights.push(
            prevPage !== i - 1
              ? Math.max.apply(null, this.columnHeights.slice(prevPage, i - 1))
      ...
      this.pageHeights.push(
        prevPage !== columnCount - 1
          ? Math.max.apply(
              null,
              this.columnHeights.slice(prevPage, columnCount - 1)
            )
          : this.columnHeights[prevPage as number]
      );
  ...
  private getAvailWidth(tx: number, width: number): number {
    ...
    return (
      ...
      (this.legend.padding * 2 +
        this.legend.shapeWidth +
        this.legend.shapePadding)
    );
  }
  ...
  public getRenderPoint(
    ...
    start: ChartLocation,
    textPadding: number,
    prevLegend: LegendOptions,
    rect: Rect,
    count: number,
    firstLegend: number
  ): void {
    const padding: number = this.legend.padding;
    const previousLocation: number =
      prevLegend.location.y +
      this.maxItemHeight / 4 +
      (prevLegend.textCollection.length > 0
        ? (prevLegend.textCollection.length - 1) * this.maxItemHeight
        : 0);
    if (this.isVertical) {
      if (
        count === firstLegend ||
        previousLocation + legendOption.textSize.height + padding >
          rect.y + rect.height
      ) {
        legendOption.location.x =
          prevLegend.location.x +
          (count === firstLegend
            ? 0
            : !this.isRtlEnable
            ? this.maxColumnWidth
        ...
        this.pageXCollections.push(
          legendOption.location.x +
            (!this.isRtlEnable ? -textStartLoc : textStartLoc)
        );
      } else {
        legendOption.location.x = prevLegend.location.x;
        legendOption.location.y =
          prevLegend.location.y + prevLegend.textSize.height + this.itemPadding;
      }
    } else {
      const textWidth: number =
        textPadding +
        (this.legend.maximumLabelWidth
          ? this.legend.maximumLabelWidth
          : prevLegend.textSize.width);
      const previousBound: number =
        prevLegend.location.x + (!this.isRtlEnable ? textWidth : -textWidth);
      if (
        this.isWithinBounds(
          previousBound,
          (this.legend.maximumLabelWidth
        ...
        legendOption.location.y =
          count === firstLegend
            ? prevLegend.location.y
            : prevLegend.location.y +
              this.rowHeights[this.chartRowCount - 2] +
              padding;
        legendOption.location.x = start.x;
      } else {
        legendOption.location.y = prevLegend.location.y;
        legendOption.location.x =
          count === firstLegend ? prevLegend.location.x : previousBound;
      }
    }
    let availablewidth: number = this.getAvailWidth(
      legendOption.location.x,
      this.legendBounds.width
    );
    availablewidth = this.legend.maximumLabelWidth
      ? Math.min(this.legend.maximumLabelWidth, availablewidth)
      : availablewidth;
    if (
      this.legend.textOverflow === 'Ellipsis' &&
      this.legend.textWrap === 'Normal'
    ) {
  ...
  private isWithinBounds(
    ...
    if (!this.isRtlEnable) {
      return (
        previousBound + textWidth >
        legendBounds.x + legendBounds.width + shapeWidth
      );
    } else {
  ...
  public getSmartLegendLocation(
    ...
    switch (this.position) {
      case 'Left':
        ...
        legendBound.x =
          labelBound.x - legendBound.width < margin.left
            ? legendBound.x
      ...
      case 'Right':
        space =
          (this.chart.availableSize.width -
            margin.right -
            (labelBound.x + labelBound.width + legendBound.width)) /
          2;
        legendBound.x =
          labelBound.x + labelBound.width + legendBound.width >
          this.chart.availableSize.width - margin.right
            ? legendBound.x
            : labelBound.x + labelBound.width + space;
        break;
      case 'Top':
        ...
        space =
          (labelBound.y -
            legendBound.height -
            (this.titleRect.y + this.titleRect.height)) /
          2;
        legendBound.y =
          labelBound.y - legendBound.height < margin.top
            ? legendBound.y
      ...
      case 'Bottom':
        space =
          (this.chart.availableSize.height -
            margin.bottom -
            (labelBound.y + labelBound.height + legendBound.height)) /
          2;
        legendBound.y =
          labelBound.y + labelBound.height + legendBound.height >
          this.chart.availableSize.height - margin.bottom
            ? legendBound.y
            : labelBound.y + labelBound.height + space;
        break;
    }
  }
  ...
  private getTitleRect(circular: CircularChart3D): void {
    ...
    this.titleRect = new Rect(
      circular.availableSize.width / 2 - titleSize.width / 2,
      circular.margin.top,
      titleSize.width,
      titleSize.height
    );
  ...
  public click(event: Event): void {
    const targetId: string =
      (<HTMLElement>event.target).id.indexOf('_chart_legend_g_') > -1
        ? (event.target as HTMLElement).firstChild['id']
    ...
    for (const id of legendItemsId) {
      if (targetId.indexOf(id) > -1) {
        ...
        if (
          (this.chart as CircularChart3D).legendSettings.toggleVisibility &&
          !isNaN(pointIndex)
        ) {
          const currentSeries: CircularChart3DSeries = (<CircularChart3D>(
            this.chart
          )).visibleSeries[0];
          const point: CircularChart3DPoints =
            currentSeries.points[pointIndex as number];
          const legendOption: LegendOptions = this.legendByIndex(
            pointIndex,
            this.legendCollections
          );
          ...
          if (!legendClickArgs.cancel) {
            ...
            chart.removeSvg();
            const svgObject: Element | null = document.getElementById(
              this.chart.element.id + '-svg-chart-3d'
            );
            if (svgObject) {
            ...
            if (
              chart.circularChartHighlight3DModule &&
          ...
          if (
            (<CircularChart3D>this.chart).circularChartSelection3DModule &&
            !isNaN(pointIndex)
          ) {
            (<CircularChart3D>(
              this.chart
  ...
  private sliceVisibility(index: number, isVisible: boolean): void {
    ...
    if (
      (
        (this.chart as CircularChart3D)
          .visibleSeries[0] as CircularChart3DSeries
      ).dataLabel.visible
    ) {
