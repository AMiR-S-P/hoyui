...
import { isNullOrUndefined, Browser, ModuleDeclaration } from '@hoyui/base';
import {
  createElement,
  remove,
  Event,
...
import {
  ...
  SmithchartFontModel,
} from '../smithchart/utils/utils-model';
import {
  SmithchartMargin,
  SmithchartBorder,
  SmithchartFont,
} from '../smithchart/utils/utils';
...
} from '../smithchart/model/interface';
import {
...
import {
  ...
  load,
  loaded,
} from '../smithchart/model/constant';
  ...
  implements INotifyPropertyChanged
{
  ...
  private renderTitle(
    ...
    let textSize: SmithchartSize = measureText(
      title.text,
      font,
      this.themeStyle.smithchartTitleFont
    );
    let x: number;
    ...
    const maxTitleWidth: number = isNullOrUndefined(title.maximumWidth)
      ...
      : title.maximumWidth;
    const titleWidthEnable: boolean =
      textSize.width > maxTitleWidth ? true : false;
    if (textSize.width > this.availableSize.width) {
      x = this.margin.left + this.border.width;
    } else {
      x =
        textAlignment === 'Center'
          ? this.availableSize.width / 2 - textSize['width'] / 2
          : textAlignment === 'Near'
          ? this.margin.left + this.elementSpacing + this.border.width
          : this.availableSize.width -
            textSize['width'] -
            (this.margin.right + this.elementSpacing + this.border.width);
    }
    const y: number =
      this.margin.top + textSize['height'] / 2 + this.elementSpacing;
    ...
    const titleRenderSuccess: Function = (args: ITitleRenderEventArgs) => {
      if (!args.cancel) {
        options = new TextOption(
          this.element.id + '_Smithchart_' + type,
          args.x,
          args.y,
          'start',
          args.text
        ...
        const element: Element = renderTextElement(
          options,
          font,
          font.color || this.themeStyle.smithchartTitleFont.color,
          groupEle,
          this.themeStyle.smithchartTitleFont
        );
        ...
        const titleLocation: {
          x: number;
          y: number;
          textSize: SmithchartSize;
        } = { x: args.x, y: args.y, textSize: textSize };
  ...
  private renderSubtitle(
    ...
    const x: number =
      title['subtitle'].textAlignment === 'Far'
        ? titleLocation.x + titleLocation.textSize.width
    ...
    const subtitleRenderSuccess: Function = (
      args: ISubTitleRenderEventArgs
    ) => {
      if (!args.cancel) {
        ...
        const element: Element = renderTextElement(
          options,
          font,
          font.color || this.themeStyle.smithchartSubtitleFont.color,
          groupEle,
          this.themeStyle.smithchartSubtitleFont
        );
  ...
  private renderBorder(): void {
    ...
    const borderRect: RectOption = new RectOption(
      this.element.id + '_SmithchartBorder',
      this.background,
      border,
      1,
      new SmithchartRect(
    ...
    const element: Element = this.svgObject.appendChild(
  ...
  private initPrivateVariable(): void {
    ...
    this.element.setAttribute(
      ...
      this.title.description ||
        this.title.text + '. Syncfusion interactive chart.'
    );
  ...
  private setTheme(): void {
    ...
    if (
      !document.getElementById(this.element.id + 'Keyboard_smith_chart_focus')
    ) {
      const style: HTMLStyleElement = document.createElement('style');
      style.setAttribute(
        'id',
        (<HTMLElement>this.element).id + 'Keyboard_smith_chart_focus'
      );
      style.innerText =
        '.e-smith-chart-focused:focus,' +
        ...
        'div[id*=container]:focus-visible, text[id*=_Smithchart_title]:focus-visible, path[id*=_Points]:focus-visible, g[id*=_svg_seriesCollection]:focus-visible, g[id*=_svg_Legend]:focus-visible {outline: 1.5px ' +
  ...
  protected render(): void {
    ...
    if (this.smithchartLegendModule && this.legendSettings.visible) {
      this.legendBounds =
        this.smithchartLegendModule.calculateLegendBounds(this);
    }
    const areaBounds: AreaBounds = new AreaBounds();
    this.bounds = areaBounds.calculateAreaBounds(
      this,
      this.title,
      this.legendBounds
    );
  ...
  private createSecondaryElement(): void {
    if (
      ...
      const svgRect: HTMLElement = document.getElementById(
        this.element.id + '_svg'
      );
      if (svgRect) {
        const svgClientRect: ClientRect = svgRect.getBoundingClientRect();
        secondaryElement.style.left =
          Math.max(svgClientRect.left - rect.left, 0) + 'px';
        secondaryElement.style.top =
          Math.max(svgClientRect.top - rect.top, 0) + 'px';
      }
    } else {
  ...
  public destroy(): void {
    if (this.element) {
      ...
      const element: HTMLElement = document.getElementById(
        this.element.id + 'Keyboard_smith_chart_focus'
      );
      if (element) {
        element.remove();
      }
  ...
  public mouseEnd(e: PointerEvent): void {
    ...
    if (this.tooltipRenderModule && this.isTouch) {
      const tooltipElement: Tooltip =
        this.tooltipRenderModule.smithchartMouseMove(this, e);
      if (tooltipElement) {
        this.fadeoutTo = +setTimeout((): void => {
  ...
  public smithchartOnClick(e: Event | PointerEvent): void {
    ...
    const targetId: string = this.isLegendClicked
      ? targetEle.children[1].id
      : targetEle.id;
    ...
    if (
      ...
      for (
        let i: number = 0;
        i < this.smithchartLegendModule.legendSeries.length;
        i++
      ) {
        if (
          legendText ===
          this.smithchartLegendModule.legendSeries[i as number]['text']
        ) {
          seriesIndex =
            this.smithchartLegendModule.legendSeries[i as number].seriesIndex;
          fill = this.smithchartLegendModule.legendSeries[i as number].fill;
        }
      }
      const seriesElement: HTMLElement = <HTMLElement>(
  ...
  public chartKeyDown(e: KeyboardEvent): boolean {
    ...
    if (
      this.series[this.currentSeriesIndex].tooltip.visible &&
      ((e.code === 'Tab' && this.previousTargetId.indexOf('_Series') > -1) ||
        e.code === 'Escape')
    ) {
      actionKey = 'ESC';
    }
    if (actionKey !== '') {
      this.smithchartKeyboardNavigations(
        e,
  ...
  public chartKeyUp(e: KeyboardEvent): boolean {
    ...
    const titleElement: HTMLElement = getElement(
      this.element.id + '_Smithchart_title'
    ) as HTMLElement;
    const seriesElement: HTMLElement = getElement(
      this.element.id + '_svg' + '_seriesCollections'
    ) as HTMLElement;
    const legendElement: HTMLElement = getElement(
      this.element.id + 'legendItem_Group'
    ) as HTMLElement;
    ...
    if (
      ...
      seriesElement.firstElementChild.children[1].lastElementChild
    ) {
      const firstChild: HTMLElement = seriesElement.firstElementChild
        .children[1].lastElementChild as HTMLElement;
    ...
    if (e.code === 'Tab') {
      if (this.previousTargetId !== '') {
        if (
          this.previousTargetId.indexOf('_Series') > -1 &&
          targetId.indexOf('_Series') === -1
        ) {
          groupElement = getElement(
            this.element.id + '_svg_seriesCollections'
          ) as HTMLElement;
          const previousElement: Element =
            this.previousTargetId.indexOf('_Marker') > -1
              ? getElement(
                  this.element.id +
                    '_svg_series' +
                    this.currentSeriesIndex +
                    '_Marker'
                ).children[this.currentPointIndex]
              : groupElement.children[this.currentSeriesIndex];
          this.setTabIndex(
            previousElement as HTMLElement,
            document.getElementById(
              this.element.id + '_Series0_Points0_Marker0'
            ) as HTMLElement
          );
          this.currentPointIndex = 0;
          this.currentSeriesIndex = 0;
        } else if (
          this.previousTargetId.indexOf('_svg_Legend') > -1 &&
          targetId.indexOf('_svg_Legend') === -1
        ) {
          groupElement = getElement(
            this.element.id + 'legendItem_Group'
          ) as HTMLElement;
      ...
      if (targetId.indexOf('_svg_Legend') > -1) {
        ...
        if (e.code === 'ArrowRight' || e.code === 'ArrowLeft') {
          ...
          for (let i: number = 0; i < groupElement.children.length; i++) {
            if (
              groupElement.children[i as number].id.indexOf(
                '_svg_seriesCollection'
              ) > -1
            ) {
              seriesIndexes.push(
                +groupElement.children[i as number].id.split(
                  '_svg_seriesCollection'
                )[1]
              );
            }
        ...
        actionKey = this.series[this.currentSeriesIndex].tooltip.visible
          ? 'ArrowMove'
          : '';
      }
    } else if (
    ...
    switch (actionKey) {
      ...
      case 'ArrowMove':
        if (targetId.indexOf('_Points') > -1) {
          ...
          if (
            this.tooltipRenderModule &&
            this.series[seriesIndex as number].tooltip.visible
          ) {
            ...
            this.tooltipRenderModule.createTooltip(
              this,
              e,
              pointIndex,
              seriesIndex,
              this.series[seriesIndex as number]
            );
          }
        }
        break;
