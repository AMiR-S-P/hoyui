...
export class TooltipRender {
  ...
  public smithchartMouseMove(smithchart: Smithchart, e: PointerEvent): Tooltip {
    ...
    for (let i: number = 0; i < smithchart.series.length; i++) {
      ...
      closestPoint = this.closestPointXY(
        smithchart,
        this.mouseX,
        this.mouseY,
        series,
  ...
  private setMouseXY(
    ...
    const svgRectElement: Element = document.getElementById(
      smithchart.element.id + '_svg'
    );
  ...
  public createTooltip(
    ...
    const tooltip: string[] = currentPoint.tooltip
      ? [currentPoint.tooltip]
      : null;
    ...
    const smithChartTooltipSuccess: Function = (
      argsData: ISmithChartTooltipEventArgs
    ) => {
      const markerHeight: number =
        smithchart.series[seriesindex as number].marker.height / 2;
      let div: Element = document.getElementById(
        smithchart.element.id + '_smithchart_tooltip_div'
      );
      ...
      this.tooltipElement = new Tooltip({
        ...
        areaBounds: new SmithchartRect(
          smithchart.bounds.x,
          smithchart.bounds.y,
          smithchart.bounds.width,
          smithchart.bounds.height
        ),
        palette: [
          series.fill ||
            smithchart.seriesColors[
              seriesindex % smithchart.seriesColors.length
            ],
        ],
        shapes: ['Circle'],
      ...
      this.tooltipElement.opacity =
        smithchart.themeStyle.tooltipFillOpacity || this.tooltipElement.opacity;
      this.tooltipElement.textStyle.fontFamily =
        smithchart.themeStyle.fontFamily ||
        'Roboto, Segoe UI, Noto, Sans-serif';
      this.tooltipElement.textStyle.size =
        smithchart.themeStyle.tooltipFontSize || '13px';
      this.tooltipElement.textStyle.color =
        smithchart.themeStyle.tooltipBoldLabel ||
        this.tooltipElement.textStyle.color;
      this.tooltipElement.appendTo(div as HTMLElement);
      const element: HTMLElement = document.getElementById(
        smithchart.element.id + '_smithchart_tooltip_div_Trackball_0'
      );
      if (element) {
