...
} from '../../smithchart/series/series-model';
import {
  SmithchartRect,
  SmithchartSize,
  Point,
...
export class DataLabel {
  ...
  public drawDataLabel(
    ...
    const marker: SeriesMarkerModel =
      smithchart.series[seriesindex as number].marker;
    let region: Point;
    ...
    for (let i: number = 0; i < count; i++) {
      labelText =
        smithchart.series[seriesindex as number].points[
          i as number
        ].reactance.toString();
      ...
      const yPos: number =
        region.y - (textSize.height + marker['height'] + margin.top);
      const width: number = textSize.width + margin.left / 2 + margin.right / 2;
      const height: number =
        textSize.height + margin.top / 2 + margin.bottom / 2;
      font.fontFamily = font.fontFamily
        ? font.fontFamily
        : smithchart.themeStyle.dataLabelFont.fontFamily;
      pointIndex = i;
  ...
  public calculateSmartLabels(points: object, seriesIndex: number): void {
    ...
    for (let k: number = 0; k < length; k++) {
      this.allPoints[this.allPoints.length] =
        points['textOptions'][k as number];
      this.connectorFlag = false;
      this.compareDataLabels(k, points, count, seriesIndex);
      this.labelOptions[seriesIndex as number]['textOptions'][k as number] =
        points['textOptions'][k as number];
      this.labelOptions[seriesIndex as number]['textOptions'][
        k as number
      ].connectorFlag = this.connectorFlag;
    }
  }
      ...
      if (collide) {
        ...
        switch (count) {
          case 0:
            ...
            currentLabel['xPosition'] =
              this.prevLabel['xPosition'] +
              (this.prevLabel['width'] / 2 +
                currentLabel['width'] / 2 +
                padding);
          ...
          case 1:
            ...
            currentLabel['xPosition'] =
              ...
              padding;
            currentLabel['x'] = currentLabel['xPosition'] + padding / 2;
            currentLabel['yPosition'] =
              currentLabel['location'].y +
          ...
          case 5:
            ...
            currentLabel['x'] = currentLabel['xPosition'] + padding / 2;
            currentLabel['yPosition'] =
              this.prevLabel['yPosition'] - currentLabel['height'] - padding;
            currentLabel['y'] =
              currentLabel['yPosition'] +
              currentLabel['height'] / 2 +
              padding / 2;
  ...
  private isCollide(
    dataLabel1: DataLabelTextOptions,
    dataLabel2: DataLabelTextOptions
  ): boolean {
    ...
    if (dataLabel1 !== dataLabel2) {
      state = !(
        ...
        (
          ...
          dataLabel1['x'] + dataLabel1['width'] / 2 <
            dataLabel2['x'] - dataLabel2['width'] / 2 ||
          dataLabel1['x'] - dataLabel1['width'] / 2 >
            dataLabel2['x'] + dataLabel2['width'] / 2
        )
      );
  ...
  public drawConnectorLines(
    smithchart: Smithchart,
    seriesIndex: number,
    index: number,
    currentPoint: DataLabelTextOptions,
    groupElement: Element
  ): void {
    ...
    const connectorDirection: string =
      'M' +
      ' ' +
      location.x +
      ' ' +
      ...
      ' ' +
      endY;
    const connectorLineValues: SeriesMarkerDataLabelConnectorLineModel =
      smithchart.series[seriesIndex as number].marker.dataLabel.connectorLine;
    const stroke: string = connectorLineValues.color
  ...
  private drawDatalabelSymbol(
    ...
    for (
      let i: number = 0;
      i < smithchart.series[seriesindex as number].points.length;
      i++
    ) {
      if (dataLabel.template) {
        ...
        document
          .getElementById(smithchart.element.id + '_Secondary_Element')
          .appendChild(labelTemplateElement);
        const id: string =
          dataLabel.template +
          '_seriesIndex' +
          seriesindex +
        ...
        const data: object = {
          point:
            smithchart.series[seriesindex as number].points[i as number]
              .reactance,
        };
        const templateFn: Function = getTemplateFunction(dataLabel.template);
        const templateElement: Element = templateFn(smithchart);
        const labelElement: HTMLElement = <HTMLElement>(
          convertElementFromLabel(templateElement, id, data)
        );
        labelTemplateElement.appendChild(labelElement);
        labelElement.style.left =
          pointsRegion[i as number].point.x -
          labelElement.offsetWidth / 2 +
          'px';
        labelElement.style.top =
          pointsRegion[i as number].point.y -
          labelElement.offsetHeight -
          smithchart.series[seriesindex as number].marker.height / 2 +
          'px';
        ...
        const labelPosition: SmithchartLabelPosition = {
          textX: left,
          textY: top,
          x: left,
          y: top,
