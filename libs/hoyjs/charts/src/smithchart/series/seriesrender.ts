...
export class SeriesRender {
  ...
  public draw(
    ...
    const groupElement: Element = smithchart.renderer.createGroup({
    ...
    for (let m: number = 0; m < seriesLength; m++) {
      ...
      if (
        series[m as number].dataSource &&
        series[m as number].resistance &&
        series[m as number].reactance
      ) {
      ...
      for (let j: number = 0; j < series[m as number].points.length; j++) {
        this.xValues[j as number] =
          series[m as number].points[j as number]['resistance'];
        this.yValues[j as number] =
          series[m as number].points[j as number]['reactance'];
      }
      ...
      for (let k: number = 0; k < series[m as number].points.length; k++) {
        ...
        if (smithchart.renderType === 'Impedance') {
          reactanceCy =
            reactance > 0 ? chartAreaCy - reactanceR : chartAreaCy + reactanceR;
        ...
        if (Math.abs(reactance) < epsilon) {
          interSectPoint.x =
            smithchart.renderType === 'Impedance'
              ? resistantCx - resistantR
              : resistantCx + resistantR;
          interSectPoint.y = chartAreaCy;
        }
        this.pointsRegion[m as number][k as number] = new PointRegion();
        this.pointsRegion[m as number][k as number] = {
          point: interSectPoint,
          x: resistance,
          y: reactance,
        };
        this.location[m as number][k as number] = {
          x: interSectPoint.x,
          y: interSectPoint.y,
        };
      }
      for (let i: number = 0; i < series[m as number].points.length - 1; i++) {
        ...
        this.lineSegments[i as number] = {
          x1: this.xValues[i as number],
          y1: this.yValues[i as number],
          x2: this.xValues[index as number],
          y2: this.yValues[index as number],
        };
      }
    ...
    for (let j: number = 0; j < smithchart.series.length; j++) {
      if (
        smithchart.series[j as number].enableSmartLabels &&
        smithchart.series[j as number].marker.dataLabel.visible
      ) {
        const gdlcEle: Element = smithchart.renderer.createGroup({
          id:
            smithchart.element.id +
            '_svg' +
            '_series' +
            j +
            '_Datalabel' +
        ...
        const element: Element = document.getElementById(
          smithchart.element.id + '_svg' + '_seriesCollection' + j
        );
        if (element) {
          element.setAttribute(
            'aria-label',
            'Smithchart with ' + series[j as number].points.length + ' points'
          );
          element.appendChild(gdlcEle);
        }
        this.dataLabel.calculateSmartLabels(
          this.dataLabel.labelOptions[j as number],
          j
        );
        for (
          let k: number = 0;
          k < smithchart.series[j as number].points.length;
          k++
        ) {
          const currentPoint: DataLabelTextOptions =
            this.dataLabel.labelOptions[j as number]['textOptions'][
              k as number
            ];
          if (
            currentPoint.xPosition + currentPoint.width >
              smithchart.chartArea.x + smithchart.chartArea.width ||
            currentPoint.xPosition < smithchart.chartArea.x ||
            currentPoint.yPosition < smithchart.chartArea.y ||
            currentPoint.yPosition + currentPoint.height >
              smithchart.chartArea.y + smithchart.chartArea.height
          ) {
            this.dataLabel.labelOptions[j as number].textOptions[
              k as number
            ].connectorFlag = false;
          ...
          if (currentPoint['connectorFlag']) {
            this.dataLabel.drawConnectorLines(
              smithchart,
              j,
              k,
              currentPoint,
    ...
    for (let j: number = 0; j < smithchart.series.length; j++) {
      const dataLabel: SeriesMarkerDataLabelModel =
        smithchart.series[j as number].marker.dataLabel;
        ...
        for (
          ...
          if (!dataLabel.template && currentPoint.visible) {
            ...
            const id: string =
              ...
              '_symbol' +
              k;
            const fill: string = dataLabel['fill']
              ? dataLabel['fill']
              : smithchart.series[j as number].fill ||
                smithchart.seriesColors[j % smithchart.seriesColors.length];
            const border: SeriesMarkerDataLabelBorderModel =
              smithchart.series[j as number].marker.dataLabel.border;
            ...
            const rectOptions: RectOption = new RectOption(
              ...
              border,
              options['opacity'],
              new SmithchartRect(x, y, options['width'], options['height'])
            );
            const dataEle: Element = smithchart.renderer.drawRectangle(
              rectOptions
            ) as SVGRectElement;
            ...
            const textRenderSuccess: Function = (
              args: ISmithchartTextRenderEventArgs
            ) => {
              if (!args.cancel) {
                ...
                const color: string = font.color
                  ? font.color
                  : smithchart.themeStyle.dataLabelFont.color;
    ...
    for (let i: number = 0; i < smithchart.series.length; i++) {
      if (
        ((smithchart.series[i as number].enableAnimation &&
          animationMode !== 'Disable') ||
          animationMode === 'Enable') &&
        smithchart.animateSeries
      ) {
        if (smithchart.series[i as number].marker.dataLabel.template) {
          this.animateDataLabelTemplate(i, smithchart);
        }
        const element: Element = document.getElementById(
          smithchart.element.id + '_svg' + '_seriesCollection' + i
        );
        element.setAttribute(
          'aria-label',
          'Smithchart with ' + series[i as number].points.length + ' points'
        );
  ...
  private drawSeries(
    ...
    gsEle.setAttribute(
      'visibility',
      smithchart.series[seriesindex as number].visibility
    );
    gsEle.setAttribute('role', 'region');
    gsEle.setAttribute(
      'aria-label',
      'Smithchart with ' +
        smithchart.series[seriesindex as number].points.length +
        ' points'
    );
    ...
    let element: Element;
    const count: number =
      smithchart.series[seriesindex as number].points.length - 1;
    for (let i: number = 0; i < count; i++) {
      const point1: Point =
        this.pointsRegion[seriesindex as number][i as number]['point'];
      const point2: Point =
        this.pointsRegion[seriesindex as number][i + 1]['point'];
      sb =
        sb +
        ('M' +
          ' ' +
    ...
    const seriesRenderSuccess: Function = (
      args: ISmithchartSeriesRenderEventArgs
    ) => {
      if (!args.cancel) {
        const options: PathOption = new PathOption(
          smithchart.element.id + '_series' + seriesindex + '_points',
          'none',
          smithchart.series[seriesindex as number].width,
          seriesEventArgs.fill,
          smithchart.series[seriesindex as number].opacity,
          'none',
          path
        );
        this.clipRectElement = smithchart.renderer.drawClipPath(
          new RectOption(
            smithchart.element.id + '_ChartSeriesClipRect_' + seriesindex,
            'transparent',
            { width: 1, color: 'Gray' },
            1,
            {
        ...
        const gspEle: Element = smithchart.renderer.createGroup({
    ...
    markerrender.drawMarker(
      smithchart,
      seriesindex,
      gsEle,
      this.pointsRegion[seriesindex as number]
    );
    this.dataLabel.drawDataLabel(
      smithchart,
      seriesindex,
      gsEle,
      this.pointsRegion[seriesindex as number],
  ...
  private animateDataLabelTemplate(
    ...
    const duration: number = parseFloat(
      smithchart.series[seriesindex as number].animationDuration
    );
    for (let i: number = 0; i < length; i++) {
      const element: HTMLElement = <HTMLElement>(
        document.getElementById(
          smithchart.series[seriesindex as number].marker.dataLabel.template +
            '_seriesIndex' +
            seriesindex +
            '_pointIndex' +
            i +
  ...
  private performAnimation(
    ...
    const clipRect: HTMLElement = <HTMLElement>(
      gsEle.childNodes[0].childNodes[0].childNodes[0]
    );
  ...
  public getLocation(seriesindex: number, pointIndex: number): Point {
    const x: number =
      this.location[seriesindex as number][pointIndex as number].x;
    const y: number =
      this.location[seriesindex as number][pointIndex as number].y;
    return { x: x, y: y };
  }
}
