...
export function getTemplateFunction(
  templateString: string | Function
): Function {
  let templateFn: Function = null;
  try {
    if (
      typeof templateString !== 'function' &&
      document.querySelectorAll(templateString).length
    ) {
      templateFn = templateComplier(
        document.querySelector(templateString).innerHTML.trim()
      );
    } else {
...
export function reverselinear(
  ...
  return (
    -startValue * Math.sin((currentTime / duration) * (Math.PI / 2)) +
    endValue +
    startValue
  );
}
