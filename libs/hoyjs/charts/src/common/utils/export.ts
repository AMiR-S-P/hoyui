...
export class ExportUtils {
  ...
  public export(
    ...
    const isDownload: boolean = !(
      Browser.userAgent.toString().indexOf('HeadlessChrome') > -1
    );
    orientation = isNullOrUndefined(orientation)
    ...
    for (let i: number = 0; i < controlValue.length; i++) {
      ...
      let element: HTMLCanvasElement = controls[i as number]
        .svgObject as HTMLCanvasElement;
        ...
        image.onload = () => {
          ...
          if (type === 'PDF') {
            ...
            if (window.navigator.msSaveOrOpenBlob) {
              window.navigator.msSaveOrOpenBlob(
                element.toBlob(null),
                fileName + '.' + (type as string).toLocaleLowerCase()
              );
            } else {
      ...
      if (!isCanvas) {
        removeElement(
          document.getElementById(controls[i as number].element.id + '_canvas')
        );
      }
    }
  }
  ...
  public getDataUrl(
    ...
    const url: string = window.URL.createObjectURL(
      new Blob([new XMLSerializer().serializeToString(controlValue[0].svg)], {
        type: 'image/svg+xml',
      })
    );
    if (Browser.info.name === 'msie') {
  ...
  private getControlsValue(
    ...
    for (let i: number = 0; i < controls.length; i++) {
      ...
      const groupEle: Element = control.renderer.createGroup({
        style:
          isNullOrUndefined(isVertical) || isVertical
            ? 'transform: translateY(' + height + 'px)'
            : 'transform: translateX(' + width + 'px)',
      });
      backgroundColor = (svg.childNodes[0] as HTMLElement)
        ? (svg.childNodes[0] as HTMLElement).getAttribute('fill')
        : 'transparent';
      if (backgroundColor === 'transparent') {
        if (
          control.theme.indexOf('Dark') > -1 ||
          control.theme.indexOf('HighContrast') > -1
        ) {
      ...
      if (!isCanvas) {
        if (control.getModuleName() === 'stockChart') {
          (svg.childNodes[0].firstChild as HTMLElement).setAttribute(
            'fill',
            backgroundColor
          );
          for (let index: number = 1; index < svg.childNodes.length; index++) {
            (
              svg.childNodes[index as number].childNodes[0] as HTMLElement
            ).setAttribute('fill', backgroundColor);
          }
        } else if (type === 'SVG') {
          (svg.childNodes[0] as HTMLElement).setAttribute(
            'fill',
            backgroundColor
          );
        }
      ...
      let left: number = 0;
      if (
        (control as StockChart).stockLegendModule &&
        (control as StockChart).legendSettings.visible
      ) {
        if (
          (control as StockChart).legendSettings.position === 'Bottom' ||
          (control as StockChart).legendSettings.position === 'Top' ||
          (control as StockChart).legendSettings.position === 'Auto'
        ) {
          top += (control as StockChart).stockLegendModule.legendBounds.height;
        } else if (
          (control as StockChart).legendSettings.position === 'Left' ||
          (control as StockChart).legendSettings.position === 'Right'
        ) {
      ...
      width =
        isNullOrUndefined(isVertical) || isVertical
          ? Math.max(control.availableSize.width + left, width)
          : width + control.availableSize.width + left;
      height =
        isNullOrUndefined(isVertical) || isVertical
          ? height + control.availableSize.height + top
      ...
      if (isMultiPages || i === controls.length - 1) {
        if (
          (!isMultiPages && !(this.control as Chart).enableCanvas) ||
          (isMultiPages && !isCanvas)
        ) {
  ...
  private exportPdf(
    ...
    for (let i: number = 0; element.length > i; i++) {
      let imageString: string = element[i as number]
        .toDataURL('image/jpeg')
        .replace('image/jpeg', 'image/octet-stream');
      document.pageSettings.orientation = orientation;
      const exactWidth: number =
        pdfDefaultWidth < width[i as number]
          ? width[i as number] + margin.left + margin.right
          : pdfDefaultWidth;
      const exactHeight: number =
        orientation === 0
          ? width[i as number] + margin.left + margin.right
          : pdfDefaultHeight < height[i as number]
          ? height[i as number] + margin.top + margin.bottom
          : pdfDefaultHeight;
      if (header !== undefined) {
        ...
        const pdfHeader: PdfPageTemplateElement = new PdfPageTemplateElement(
          exactWidth,
          40
        );
        pdfHeader.graphics.drawString(
          header.content + '',
          font,
          null,
      ...
      if (footer !== undefined) {
        ...
        const pdfFooter: PdfPageTemplateElement = new PdfPageTemplateElement(
          exactWidth,
          40
        );
        pdfFooter.graphics.drawString(
          footer.content + '',
          font,
          null,
          new PdfSolidBrush(new PdfColor(0, 0, 0)),
  ...
  private doexport(type: ExportType, image: string, fileName: string): void {
  ...
  public exportImage(
    ...
    for (let j: number = 0; j < buffers.length; j++) {
      const b: Blob = new Blob([buffers[j as number]], {
        type: 'application/octet-stream',
      });
      if (Browser.info.name === 'msie') {
        window.navigator.msSaveOrOpenBlob(
          b,
          fileName + '.' + fileType.toLocaleLowerCase()
        );
      }
    }
  }
