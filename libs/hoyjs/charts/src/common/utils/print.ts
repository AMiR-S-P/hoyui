import { print as printWindow, createElement } from '@hoyui/base';
  ...
  public getHTMLContent(elements?: string[] | string | Element): Element {
    ...
    for (let index: number = 0; index < div.children.length; index++) {
      const backgroundColor: string =
        this.control.theme.indexOf('Dark') > -1 ||
        this.control.theme.indexOf('HighContrast') > -1
          ? 'rgba(0, 0, 0, 1)'
          : 'rgba(255, 255, 255, 1)';
      const svg: Element = div.children[index as number];
      for (
        let childIndex: number = 0;
        childIndex < svg.children.length;
        childIndex++
      ) {
        ...
        if (svg.id.indexOf('_stockChart_svg') > -1) {
          ...
          actualBackgroundColor =
            svg.children[childIndex as number].children[0].getAttribute('fill');
          isSVG = true;
        }
        if (isSVG) {
          actualBackgroundColor =
            actualBackgroundColor === 'transparent'
              ? backgroundColor
              : actualBackgroundColor;
          svg.children[childIndex as number].children[0].setAttribute(
            'fill',
            actualBackgroundColor
          );
        }
