...
import { TextAttributes } from '@hoyui/svg-base';
import {
  PathAttributes,
  RectAttributes,
  CircleAttributes,
...
import {
  AccumulationSeries,
  AccPoints,
} from '../../accumulation-chart/model/acc-base';
  ...
  for (let i: number = 0; i < sortData.length; i++) {
    for (let j: number = 0; j < fields.length; j++) {
      if (sortData[i as number][fields[j as number]] instanceof Date) {
        sortData[i as number][fields[j as number]] =
          sortData[i as number][fields[j as number]].getTime();
      }
    }
  }
...
export function rotateTextSize(
  ...
  const htmlObject: HTMLElement = renderer.createText(
    options,
    labelText
  ) as HTMLElement;
  ...
  if (
    !chart.delayRedraw &&
    !chart.redraw &&
    !(chart as Chart).stockChart &&
    !(chart as Chart).pointsAdded
  ) {
    remove(chart.svgObject);
  }
  if ((chart as Chart).enableCanvas) {
    const textWidth: number = measureText(
      text,
      font,
      chart.themeStyle.axisLabelFont
    ).width;
    const textHeight: number = measureText(
      text,
      font,
      chart.themeStyle.axisLabelFont
    ).height;
    const angleInRadians: number = (angle * Math.PI) / 180; // Convert the rotation angle to radians
    const rotatedTextWidth: number =
      Math.abs(Math.cos(angleInRadians) * textWidth) +
      Math.abs(Math.sin(angleInRadians) * textHeight);
    const rotatedTextHeight: number =
      Math.abs(Math.sin(angleInRadians) * textWidth) +
      Math.abs(Math.cos(angleInRadians) * textHeight);
...
export function showTooltip(
  ...
  element: Element,
  isTouch?: boolean,
  isTitleOrLegendEnabled?: boolean,
  bound?: Rect
): void {
...
export function logWithIn(value: number, axis: Axis): number {
  return axis.valueType === 'Logarithmic'
    ? logBase(value, axis.logBase)
    : value;
}
/**
...
export function withInRange(
  ...
  const mX1: number = previousPoint
    ? logWithIn(previousPoint.xValue, series.xAxis)
    : mX2;
  const mX3: number = nextPoint
    ? logWithIn(nextPoint.xValue, series.xAxis)
    : mX2;
...
export function getRotatedRectangleCoordinates(
  ...
  for (let i: number = 0; i < 4; i++) {
    ...
    const rotatedX: number =
      tempX * Math.cos(degreeToRadian(angle)) -
      tempY * Math.sin(degreeToRadian(angle));
    const rotatedY: number =
      tempX * Math.sin(degreeToRadian(angle)) +
      tempY * Math.cos(degreeToRadian(angle));
    // translate back
 ...
 */
export function isRotatedRectIntersect(
  a: ChartLocation[],
  b: ChartLocation[]
): boolean {
 ...
 */
function getAccumulationLegend(
  locX: number,
  locY: number,
  r: number,
  height: number,
  ...
  const cartesianlarge: ChartLocation = degreeToLocation(
    270,
    r,
    new ChartLocation(locX, locY)
  );
  const cartesiansmall: ChartLocation = degreeToLocation(
    270,
    r,
    new ChartLocation(locX + width / 10, locY)
  );
  return (
    'M' +
    ' ' +
...
export function valueToCoefficient(value: number, axis: Axis): number {
  ...
  const isInverse: boolean = axis.isChart
    ? axis.isAxisInverse
    : axis.isInversed;
 ...
 */
export function TransformToVisible(
  x: number,
  y: number,
  xAxis: Axis,
  yAxis: Axis,
  ...
  x =
    xAxis.valueType === 'Logarithmic'
      ? logBase(x > 1 ? x : 1, xAxis.logBase)
      : x;
  y =
    yAxis.valueType === 'Logarithmic'
      ? logBase(y > 1 ? y : 1, yAxis.logBase)
      : y;
  x +=
    xAxis.valueType === 'Category' &&
    xAxis.labelPlacement === 'BetweenTicks' &&
    series.type !== 'Radar'
      ? 0.5
      : 0;
  const radius: number = series.chart.radius * valueToCoefficient(y, yAxis);
  const point: ChartLocation = CoefficientToVector(
    valueToPolarCoefficient(x, xAxis),
...
export function indexFinder(id: string, isPoint: boolean = false): Index {
  ...
  if (id.indexOf('_Point_') > -1) {
    ...
    id.indexOf('_shape_') > -1 &&
    (!isPoint || (isPoint && id.indexOf('_legend_') === -1))
  ) {
 ...
 */
export function CoefficientToVector(
  coefficient: number,
  startAngle: number
): ChartLocation {
...
export function valueToPolarCoefficient(value: number, axis: Axis): number {
  ...
  if (axis.valueType !== 'Category') {
    delta =
      range.max -
      (axis.valueType === 'DateTime' ? axis.dateTimeInterval : range.interval) -
      range.min;
    ...
    delta =
      axis.visibleLabels.length === 1
        ...
        : axis.visibleLabels[axis.visibleLabels.length - 1].value -
          axis.visibleLabels[0].value;
    length = axis.visibleLabels.length;
  }
  return axis.isAxisInverse
    ? ((value - <number>range.min) / delta) * (1 - 1 / length)
...
export function createTooltip(
  ...
  const style: string =
    ...
    'position:absolute;border:1px solid #707070;font-size:' +
    fontSize +
 ...
 */
export function createZoomingLabels(
  chart: Chart,
  axis: Axis,
  parent: Element,
  index: number,
  ...
  const anchor: string = chart.enableRtl
    ? 'end'
    : isVertical
    ? 'start'
    : 'auto';
  ...
  const scrollBarHeight: number =
    ...
    (axis.zoomingScrollBar && axis.zoomingScrollBar.svgObject)
      ? axis.scrollBarHeight
      : 0;
  const isRtlEnabled: boolean =
    (chart.enableRtl && !isVertical && !axis.isInversed) ||
  ...
  for (let i: number = 0; i < 2; i++) {
    ...
    if (isVertical) {
      arrowLocation = i
        ? new ChartLocation(rect.x - scrollBarHeight, rect.y + rx)
        : new ChartLocation(
            axis.rect.x - scrollBarHeight,
            rect.y + rect.height - rx
          );
      x =
        rect.x +
        (opposedPosition
          ? rect.width + margin + scrollBarHeight
          : -(size.width + margin + margin + scrollBarHeight));
      y = rect.y + (i ? 0 : rect.height - size.height - margin);
      x +=
        x < 0 || chartRect < x + size.width + margin
          ? opposedPosition
            ? -(size.width / 2)
            : size.width / 2
          : 0;
      direction = findCrosshairDirection(
        rx,
        rx,
        new Rect(x, y, size.width + margin, size.height + margin),
        arrowLocation,
        margin,
        false,
        false,
        !opposedPosition,
        arrowLocation.x,
        arrowLocation.y + (i ? -rx : rx)
      );
    } else {
      arrowLocation = i
        ? new ChartLocation(
            rect.x + rect.width - rx,
            rect.y + rect.height + scrollBarHeight
          )
        : new ChartLocation(
            rect.x + rx,
            rect.y + rect.height + scrollBarHeight
      ...
      y = opposedPosition
        ? rect.y - size.height - 10 - scrollBarHeight
        : rect.y + rect.height + margin + scrollBarHeight;
      direction = findCrosshairDirection(
        rx,
        rx,
        new Rect(x, y, size.width + margin, size.height + margin),
    ...
    if (chart.theme === 'Fluent' || chart.theme === 'FluentDark') {
      ...
      pathElement.setAttribute(
        'filter',
        Browser.isIE ? '' : 'url(#' + shadowId + ')'
      );

      let shadow: string =
        '<filter id="' +
        shadowId +
        '" height="130%"><feGaussianBlur in="SourceAlpha" stdDeviation="3"/>';
      shadow +=
        '<feOffset dx="3" dy="3" result="offsetblur"/><feComponentTransfer><feFuncA type="linear" slope="0.5"/>';
      shadow +=
      ...
      defElement.setAttribute(
        'id',
        chart.element.id + 'SVG_tooltip_definition'
      );
    ...
    textElement(
      chart.renderer,
      new TextOption(
        chart.element.id + '_Zoom_' + index + '_AxisLabel_' + i,
        x,
        y,
        anchor,
        i
      ...
      chart.themeStyle.crosshairLabelFont.color,
      parent,
      null,
      null,
      null,
...
export function findCrosshairDirection(
  ...
  if (top) {
    ...
    if (arrowPadding !== 0) {
      ...
      direction = direction.concat(
        ' L' +
          ...
          ' L' +
          ' ' +
          (arrowLocation.x - arrowPadding / 2) +
          ' ' +
          height
      );
    }
    ...
    direction = direction.concat(
      ' L' + ' ' + (arrowLocation.x + arrowPadding / 2) + ' ' + startY
    );
    direction = direction.concat(
      ' L' +
        ' ' +
    ...
    direction = direction.concat(
      ' L' +
        ...
        width +
        ' ' +
        (arrowLocation.y - arrowPadding / 2)
    );
    direction = direction.concat(
      ' L' + ' ' + (width + arrowPadding) + ' ' + tipY
    );
    direction = direction.concat(
      ' L' + ' ' + width + ' ' + (arrowLocation.y + arrowPadding / 2)
    );
    direction = direction.concat(
      ' L' +
        ' ' +
    ...
    direction = direction.concat(
      ' L' + ' ' + (startX - arrowPadding) + ' ' + tipY
    );
    direction = direction.concat(
      ' L' + ' ' + startX + ' ' + (arrowLocation.y + arrowPadding / 2)
    );
    direction = direction.concat(
      ' L' +
        ' ' +
 ...
 */
export function getValueXByPoint(
  value: number,
  size: number,
  axis: Axis
): number {
  const actualValue: number = !axis.isAxisInverse
    ? value / size
    : 1 - value / size;
 ...
 */
export function getMinPointsDelta(
  ...
  for (let index: number = 0; index < seriesCollection.length; index++) {
    ...
    if (
      series.visible &&
      (axis.name === series.xAxisName ||
        (axis.name === 'primaryXAxis' && series.xAxisName === null) ||
        (axis.name === series.chart.primaryXAxis.name && !series.xAxisName))
    ) {
      if (
        series.type.indexOf('Stacking') > -1 &&
        stackingGroups.indexOf(series.stackingGroup) === -1
      ) {
      ...
      xValues.sort((first: Object, second: Object) => {
        return <number>first - <number>second;
      });
      if (xValues.length === 1) {
        const timeOffset: number =
          seriesCollection.length === 1 ? 25920000 : 2592000000;
        seriesMin =
          axis.valueType === 'DateTime' && series.xMin === series.xMax
            ? series.xMin - timeOffset
            : series.xMin;
        minVal =
          <number>xValues[0] -
          (!isNullOrUndefined(seriesMin) ? seriesMin : axis.visibleRange.min);
        if (minVal !== 0) {
          ...
          if (index > 0 && value) {
            minVal =
              ...
              axis.valueType === 'Category'
                ...
                : <number>value - <number>xValues[index - 1];
            if (minVal !== 0) {
              minDelta = Math.min(minDelta, minVal);
            }
          }
...
export function linear(
  ...
  return (
    -endValue * Math.cos((currentTime / duration) * (Math.PI / 2)) +
    endValue +
    startValue
  );
}
...
export function markerAnimate(
  ...
  new Animation({}).animate(<HTMLElement>element, {
    ...
    progress: (args: AnimationOptions): void => {
      ...
      const annotations: HTMLElement = <HTMLElement>(
        document.getElementById(
          (series as Series).chart.element.id + '_Annotation_Collections'
        )
      );
...
export function pathAnimation(
  ...
  const startDirections: string =
    previousDirection || element.getAttribute('d');
  ...
  new Animation({}).animate(createElement('div'), {
    ...
    progress: (args: AnimationOptions): void => {
      ...
      splitDirections.map((directions: string, index: number) => {
        ...
        endPath = endDirections[index as number]
          ? endDirections[index as number].split(' ')
          : startPath;
        if (startPath[0] === 'Z') {
          currentDireciton += 'Z' + ' ';
        } else if (startPath[0] === '') {
        ...
        if (startPath[0] === 'C' || startPath[0] === 'Q') {
          ...
          while (c < end) {
            currentDireciton +=
              linear(
                args.timeStamp,
                +startPath[c as number],
                +endPath[c as number] - +startPath[c as number],
                args.duration
              ) +
              ' ' +
              linear(
...
export function animateAddPoints(
  ...
  new Animation({}).animate(createElement('div'), {
    ...
    progress: (args: AnimationOptions): void => {
      currentDirection = '';
      const startPathCommands: string[] = startDirections.match(
        /[MLHVCSQTAZ][^MLHVCSQTAZ]*/g
      );
      const endPathCommands: string[] = endDirections.match(
        /[MLHVCSQTAZ][^MLHVCSQTAZ]*/g
      );
      const maxLength: number = Math.max(
        startPathCommands.length,
        endPathCommands.length
      );
      for (let i: number = 0; i < maxLength; i++) {
        ...
        for (let j: number = 1; j < startCoords.length; j++) {
          ...
          if (
            !isNaN(startCoord) &&
            !isNaN(endCoord) &&
            startCoords.length === endCoords.length
          ) {
            const interpolatedValue: number = linear(
              args.timeStamp,
              startCoord,
              endCoord - startCoord,
              duration
...
export function triggerLabelRender(
  ...
  if (!argsData.cancel) {
    ...
    const text: string | string[] = axis.enableTrim
      ? isLineBreakLabels
        ? lineBreakLabelTrim(
            axis.maximumLabelWidth,
            argsData.text,
            axis.labelStyle,
            chart.themeStyle.axisLabelFont
          )
 ...
 */
export function getActualDesiredIntervalsCount(
  availableSize: Size,
  axis: Axis | Chart3DAxis
): number {
  const size: number =
  ...
  if (isNullOrUndefined(axis.desiredIntervals)) {
    let desiredIntervalsCount: number =
      (axis.orientation === 'Horizontal' ? 0.533 : 1) * axis.maximumLabels;
...
export function drawSymbol(
  ...
  clipRect?: Rect,
  isChartControl?: boolean,
  control?: BulletChart
): Element {
  const chartRenderer: SvgRenderer | CanvasRenderer = renderer
    ? renderer
    : new SvgRenderer('');
  const shapeOption: IShapes = calculateShapes(
    location,
    size,
    shape,
    options,
  ...
  const drawElement: Element = chartRenderer['draw' + shapeOption.functionName](
    shapeOption.renderOption,
    clipRect ? new Int32Array([clipRect.x, clipRect.y]) : null
  );
...
export function calculateShapes(
  ...
  const width: number =
    isBulletChart && shape === 'Circle' ? size.width - 2 : size.width;
  const height: number =
    isBulletChart && shape === 'Circle' ? size.height - 2 : size.height;
    ...
    case 'Star': {
      ...
      for (let i: number = 0; i < 2 * cornerPoints; i++) {
        ...
        const currentX: number =
          lx + radius * Math.cos(i * angle - Math.PI / 2);
        const currentY: number =
          ly + radius * Math.sin(i * angle - Math.PI / 2);
  ...
  options = <PathOption>(
    calculateLegendShapes(location, new Size(width, height), shape, options)
      .renderOption
  );
 ...
 */
export function getRectLocation(
  startLocation: ChartLocation,
  endLocation: ChartLocation,
  outerRect: Rect
): Rect {
  const x: number =
    endLocation.x < outerRect.x
      ? outerRect.x
      : endLocation.x > outerRect.x + outerRect.width
      ? outerRect.x + outerRect.width
      : endLocation.x;
  const y: number =
    endLocation.y < outerRect.y
      ? outerRect.y
      : endLocation.y > outerRect.y + outerRect.height
      ? outerRect.y + outerRect.height
      : endLocation.y;
  return new Rect(
    x > startLocation.x ? startLocation.x : x,
    y > startLocation.y ? startLocation.y : y,
    Math.abs(x - startLocation.x),
    Math.abs(y - startLocation.y)
  );
}
/**
...
export function chartReactTemplate(
  ...
  const parentElement: HTMLElement = document.getElementById(
    chart.element.id +
      '_Series_' +
      (series.index === undefined ? series.category : series.index) +
...
export function createTemplate(
  ...
  try {
    ...
    const tempObject: Object = window[blazor as string]
      ? dataLabelId
        ? point
        : { point: point }
      : { chart: chart, series: series, point: point };
    const templateId: string = dataLabelId
      ? dataLabelId + '_template'
      : 'template';
    const elementData: Element[] = templateFn
      ? templateFn(
          tempObject,
          chart,
          templateId,
          dataLabelId || childElement.id.replace(/[^a-zA-Z0-9]/g, '')
        )
      : [];
    ...
    if (chart.getModuleName() === 'accumulationchart') {
      reactCallback = accReactTemplate.bind(
        ...
        points,
        argsData,
        points[pointIndex as number],
        datalabelGroup,
        id,
        dataLabel,
        redraw
      );
      if ((chart as any).isReact) {
        (chart as any).renderReactTemplates(reactCallback);
      }
    } else if (chart.getModuleName() === 'chart') {
      reactCallback =
        point && series
          ? chartReactTemplate.bind(
              this,
              childElement,
              chart,
              point,
...
export function getFontStyle(
  ...
  style =
    'font-size:' +
    (font.size || themeFontStyle.size) +
    '; font-style:' +
    (font.fontStyle || themeFontStyle.fontStyle) +
    '; font-weight:' +
    (font.fontWeight || themeFontStyle.fontWeight) +
    '; font-family:' +
    font.fontFamily +
    ';opacity:' +
    font.opacity +
...
export function getPoint(
  ...
  x =
    xAxis.valueType === 'Logarithmic'
      ? logBase(
          x > 0 ? x : Math.pow(xAxis.logBase, xAxis.visibleRange.min),
          xAxis.logBase
        )
      : x;
  y =
    yAxis.valueType === 'Logarithmic'
      ? logBase(
          y > 0 ? y : Math.pow(yAxis.logBase, yAxis.visibleRange.min),
          yAxis.logBase
        )
      : y;
...
export function appendChildElement(
  ...
  childElement: Element | HTMLElement,
  redraw?: boolean,
  ...
  animateDuration?: number,
  scatterElement: boolean = false,
  angle: number = 0,
  currentTransform?: ChartLocation,
  previousTranslate?: string
): void {
  if (isCanvas) {
    return null;
  }
  const existChild: HTMLElement = scatterElement
    ? null
    : parent.querySelector('#' + childElement.id);
  const element: HTMLElement = <HTMLElement>(
    (existChild || getElement(childElement.id))
  );
  const child: HTMLElement = <HTMLElement>childElement;
  const duration: number = !isNullOrUndefined(animateDuration)
    ? animateDuration
    : 300;
  if (redraw && isAnimate && element) {
    start =
      ...
      (element.tagName === 'DIV'
        ? new ChartLocation(
            +element.style[x as string].split('px')[0],
            +element.style[y as string].split('px')[0]
          )
        : new ChartLocation(
    ...
    if (direction && direction !== 'undefined') {
      ...
      const end: ChartLocation =
        child.tagName === 'DIV'
          ? new ChartLocation(
              +child.style[x as string].split('px')[0],
              +child.style[y as string].split('px')[0]
            )
      ...
      animateRedrawElement(
        ...
        angle,
        currentTransform,
        previousTranslate !== undefined
          ? new ChartLocation(
              previousTranslate.match(/\d+/g).map(Number)[0],
              previousTranslate.match(/\d+/g).map(Number)[1]
            )
          : previousTranform
          ? new ChartLocation(
              parseFloat(previousTranform.split(',')[1]),
              parseFloat(previousTranform.split(',')[2])
            )
          : new ChartLocation(0, 0),
        previousTranslate !== undefined
      );
    }
 ...
 * @private
 */
export function getDraggedRectLocation(
  x1: number,
  y1: number,
  x2: number,
  y2: number,
  ...
  const x: number = Math.max(
    checkBounds(Math.min(x1, x2), width, outerRect.x, outerRect.width),
    outerRect.x
  );
  const y: number = Math.max(
    checkBounds(Math.min(y1, y2), height, outerRect.y, outerRect.height),
    outerRect.y
  );
  return new Rect(
    x,
    y,
    Math.min(width, outerRect.width),
    Math.min(height, outerRect.height)
  );
}
/**
...
export function getLabelText(
  ...
  const labelFormat: string = series.marker.dataLabel.format
    ? series.marker.dataLabel.format
    : series.yAxis.labelFormat;
  const text: string[] = [];
  ...
  switch (series.seriesType) {
    case 'XY':
      ...
      if (series.chart.chartAreaType === 'PolarRadar') {
        if (series.drawType.indexOf('Stacking') !== -1) {
          if (
            (series.yAxis.valueType === 'Logarithmic' &&
              logWithIn(
                series.stackedValues.endValues[currentPoint.index],
                series.yAxis
              )) ||
            withIn(
              series.stackedValues.endValues[currentPoint.index],
              series.yAxis.visibleRange
            )
          ) {
          ...
          if (
            (series.yAxis.valueType === 'Logarithmic' &&
              logWithIn(currentPoint.yValue, series.yAxis)) ||
    ...
    case 'HighLow':
      text.push(
        currentPoint.text ||
          Math.max(
            <number>currentPoint.high,
            <number>currentPoint.low
          ).toString()
      );
      text.push(
        currentPoint.text ||
    ...
    case 'HighLowOpenClose':
      ...
      text.push(
        currentPoint.text ||
          Math.max(
            <number>currentPoint.open,
            <number>currentPoint.close
          ).toString()
      );
      text.push(
        currentPoint.text ||
...
export function isCollide(
  ...
  const currentRect: Rect = new Rect(
    rect.x + clipRect.x,
    rect.y + clipRect.y,
    rect.width,
    rect.height
  ...
  const isCollide: boolean = collections.some((rect: Rect) => {
    return (
      ...
      currentRect.height + currentRect.y > rect.y
    );
  });
  return isCollide;
}
...
export function containsRect(currentRect: Rect, rect: Rect): boolean {
  return (
    ...
    currentRect.x + currentRect.width >= rect.x + rect.width &&
    currentRect.y <= rect.y &&
    currentRect.height + currentRect.y >= rect.y + rect.height
  );
}
...
export function colorNameToHex(color: string): string {
  ...
  return convertToHexCode(
    new ColorValue(
      parseInt(isRGBValue[0], 10),
      parseInt(isRGBValue[1], 10),
      parseInt(isRGBValue[2], 10)
    )
  );
}
/**
...
export function checkColorFormat(color: string): boolean {
  if (color.indexOf('rgba(') === 0 || color.indexOf('rgb(') === 0) {
    ...
    return (
      (hex.length === 3 || hex.length === 6) && /^[0-9A-Fa-f]{3,6}$/.test(hex)
    );
  }
  return false;
}
...
export function getSaturationColor(color: string, factor: number): string {
  ...
  for (let i: number = 0; i < 3; i++) {
    ...
    colorCode = Math.round(
      Math.min(Math.max(0, colorCode + colorCode * factor), 255)
    );
    rgb += ('00' + colorCode.toString(16)).substr(
      colorCode.toString(16).length
    );
  }
  return rgb;
}
...
export function applyZLight(color: string, value: number): string {
  ...
  return (
    '#' +
    componentToHex(RGB.r).toUpperCase() +
    componentToHex(RGB.g).toUpperCase() +
    componentToHex(RGB.b).toUpperCase()
  );
}
/**
...
export function getMedian(values: number[]): number {
  ...
  return values.length % 2
    ...
    : (values[half - 1] + values[half as number]) / 2.0;
}
/**
 ...
 */
export function calculateLegendShapes(
  location: ChartLocation,
  size: Size,
  shape: string,
  options: PathOption
  ...
  switch (shape) {
    ...
    case 'Histogram':
      dir =
        ...
        'M' +
        ' ' +
        (lx + -width / 10 - width / 20) +
        ' ' +
        (ly - height / 4 - padding / 2) +
        ' ' +
        'L' +
        ' ' +
        (lx + width / 10 + width / 20) +
    ...
    case 'StackingArea100':
      dir =
        ...
        (lx + width / 4 + padding / 8) +
        ' ' +
        (ly + -height / 2 + height / 4) +
        ' ' +
        'L' +
        ' ' +
        (lx + height / 2 + padding / 4) +
        ' ' +
        (ly + height / 2) +
        ' ' +
        'Z';
 ...
 */
export function lineBreakLabelTrim(
  maxWidth: number,
  text: string,
  font: FontModel,
  themeFontStyle?: FontModel
...
export function redrawElement(
  redraw: boolean,
  id: string,
  options?: PathAttributes | RectAttributes | CircleAttributes,
  renderer?: SvgRenderer | CanvasRenderer
): Element {
  if (!redraw) {
    return null;
...
export function animateRedrawElement(
  ...
  y: string = 'y',
  angle: number = 0,
  newTransform: ChartLocation = new ChartLocation(0, 0),
  previousTransform: ChartLocation = new ChartLocation(0, 0),
    ...
    progress: (args: AnimationOptions): void => {
      setStyle(
        ...
        linear(
          args.timeStamp,
          previousTransform.x,
          newTransform.x - previousTransform.x,
          args.duration
        ),
        linear(
          args.timeStamp,
          previousTransform.y,
          newTransform.y - previousTransform.y,
          args.duration
        )
      );
    },
 ...
 * @returns {void}
 */
export function animateTextElement(
  element: HTMLElement,
  duration: number,
  start: number,
  end: number,
  ...
  if (element && start && end) {
    const formatText: (value: number) => string = (value: number): string => {
      return customLabelFormat
        ? customLabelFormat.replace('{value}', value.toString())
        : value.toString();
    };
    new Animation({}).animate(createElement('div'), {
      duration: duration,
...
export function textElement(
  ...
  seriesClipRect?: Rect,
  labelSize?: Size,
  isRotatedLabelIntersect?: boolean,
  isCanvas?: boolean,
  ...
  if (option.text.length > 1 && isDataLabelWrap) {
    for (let i: number = 0, len: number = option.text.length; i < len; i++) {
      maxWidth = Math.max(
        maxWidth,
        measureText(option.text[i as number], font, themeFontStyle).width
      );
    }
    width = measureText(option.text[0], font, themeFontStyle).width;
  }
  dx =
    option.text.length > 1 && isDataLabelWrap
      ? option.x + maxWidth / 2 - width / 2
      : option.x;
  renderOptions = {
    id: option.id,
    x: dx,
  ...
  if (typeof option.text !== 'string' && option.text.length > 1) {
    for (let i: number = 1, len: number = option.text.length; i < len; i++) {
      ...
      label = isMinus
        ...
        : option.text[i as number];
      if (isCanvas) {
        tspanElement = renderer.createText(
          renderOptions,
          label,
          null,
          null,
...
export function calculateSize(
  ...
  let containerWidth: number =
    chart.element.clientWidth || chart.element.offsetWidth;
  ...
  if (chart.getModuleName() === 'rangeNavigator') {
    ...
    const periodHeight: number = range.periodSelectorSettings.periods.length
      ? range.periodSelectorSettings.height
      : 0;
    marginHeight = range.margin.top + range.margin.bottom + tooltipSpace;
    const labelSize: number = measureText(
      'tempString',
      range.labelStyle,
      range.themeStyle.axisLabelFont
    ).height;
    ...
    height =
      (chart.series.length
        ? Browser.isDevice
          ? 80
          : 120
        : (range.enableGrouping ? 40 + labelPadding + labelSize : 40) +
          marginHeight) + periodHeight;
    if (range.disableRangeSelector) {
      height = periodHeight;
    }
    if (
      (chart as RangeNavigator).stockChart &&
      (chart as RangeNavigator).stockChart.chart.axisCollections[1]
        .labelPosition === 'Outside'
    ) {
      const padding: number =
        (chart as RangeNavigator).stockChart.chart.axisCollections[1]
          .labelPadding +
        (chart as RangeNavigator).stockChart.chart.axisCollections[1].lineStyle
          .width *
          0.5;
      chart.width = (
        (chart as RangeNavigator).stockChart.availableSize.width -
        ((chart as RangeNavigator).stockChart.chart.axisCollections[1]
          .maxLabelSize.width +
          padding)
      ).toString();
  ...
  chart.availableSize = new Size(
    ...
    stringToNumber(chart.height, containerHeight || height) ||
      containerHeight ||
      height
  );
  if (chart.getModuleName() === 'chart') {
    ...
    if (chart.width === '' || chart.width === null || chart.width === '100%') {
      if (
        containerWidth &&
        chart.element.parentElement &&
        chart.element.parentElement.style.transform.indexOf('scale') > -1
      ) {
        scaleX = 1;
      } else {
        scaleX =
          chart.element.getBoundingClientRect().width > 0
            ? chart.element.getBoundingClientRect().width /
      ...
      if (
        ...
        scaleY =
          chart.element.getBoundingClientRect().height > 0
            ? chart.element.getBoundingClientRect().height /
              chart.availableSize.height
            : 1;
      }
      const transformValue: string = chart.element.style.transform;
      if (transformValue) {
        const scaleValue: number = parseFloat(
          transformValue.match(/scale\((.*?)\)/)[1]
        );
...
export function createSvg(
  ...
  chart.renderer = (chart as Chart).enableCanvas
    ? (chart as Chart).canvasRender
    : new SvgRenderer(chart.element.id);
  calculateSize(chart);
  if ((chart as Chart).stockChart && chart.getModuleName() === 'chart') {
    chart.svgObject = (chart as Chart).stockChart.chartObject;
  } else if (
    (chart as RangeNavigator).stockChart &&
    chart.getModuleName() === 'rangeNavigator'
  ) {
  ...
  if ((chart as Chart).enableCanvas) {
    (chart.renderer as CanvasRenderer).ctx.direction = chart.enableRtl
      ? 'rtl'
      : 'ltr';
  }
}
...
export function getTitle(
  ...
  switch (style.textOverflow) {
    case 'Wrap':
      titleCollection = textWrap(
        title,
        width,
        style,
        isRtlEnabled,
        title.indexOf(' ') < 0 ? true : null,
        null,
        themeFontStyle
      );
      break;
    case 'Trim':
      titleCollection.push(
        textTrim(width, title, style, isRtlEnabled, themeFontStyle)
      );
      break;
    default:
    ...
    for (let i: number = 0, len: number = textCollection.length; i < len; i++) {
      ...
      if (
        ...
        if (label !== '') {
          ...
          labelCollection.push(
            clip
              ? text
              : textTrim(maximumWidth, text, font, isRtlEnabled, themeFontStyle)
          );
          text = '';
        }
      }
      if (label && i === len - 1) {
        labelCollection.push(
          clip
            ? label
            : textTrim(maximumWidth, label, font, isRtlEnabled, themeFontStyle)
        );
      }
    }
    return labelCollection;
 ...
 * @private
 */
export function textWrapAnyWhere(
  currentLabel: string,
  maximumWidth: number,
  font: FontModel,
  themeFontStyle?: FontModel
...
export function getUnicodeText(text: string, regexp: RegExp): string {
  ...
  for (let i: number = 0; i <= title.length - 1; i++) {
    if (title[i as number] === ' ') {
      digitSpecific =
        regexp === regSub
          ? digit[k as number].replace(/~/g, '')
          : digit[k as number].replace(/\^/g, '');
      for (let j: number = 0; j < digitSpecific.length; j++) {
        convertedText +=
          regexp === regSub
            ? unicodeSub[digitSpecific[j as number]]
            : unicodeSup[digitSpecific[j as number]];
      }
      k++;
    } else {
...
export function blazorTemplatesReset(control: Chart | AccumulationChart): void {
  for (
    let i: number = 0;
    i < (control as Chart | AccumulationChart).annotations.length;
    i++
  ) {
    resetBlazorTemplate(
      (control.element.id + '_Annotation_' + i).replace(/[^a-zA-Z0-9]/g, ''),
      'ContentTemplate'
    );
  }
  //This reset the tooltip templates
  resetBlazorTemplate(
    control.element.id + '_tooltipparent_template' + '_blazorTemplate',
    'Template'
  );
...
export function getGradientColor(
  ...
  if (colorMap.colors.length <= 2) {
    previousColor =
      colorMap.colors[0].charAt(0) === '#'
        ? colorMap.colors[0]
        : colorNameToHex(colorMap.colors[0]);
    nextColor =
      colorMap.colors[colorMap.colors.length - 1].charAt(0) === '#'
        ? colorMap.colors[colorMap.colors.length - 1]
        : colorNameToHex(colorMap.colors[colorMap.colors.length - 1]);
  } else {
    previousColor =
      colorMap.colors[0].charAt(0) === '#'
        ? colorMap.colors[0]
    ...
    nextColor =
      colorMap.colors[colorMap.colors.length - 1].charAt(0) === '#'
        ? colorMap.colors[colorMap.colors.length - 1]
        : colorNameToHex(colorMap.colors[colorMap.colors.length - 1]);
    const a: number = full / (colorMap.colors.length - 1);
    let b: number;
    let c: number;

    const length: number = colorMap.colors.length - 1;
    const splitColorValueOffset: Object[] = [];
    let splitColor: Object = {};
    ...
    for (let i: number = 0; i < splitColorValueOffset.length; i++) {
      if (
        ...
        value <= splitColorValueOffset[i as number]['b'] &&
        i === 0
      ) {
        midColor =
          splitColorValueOffset[i as number]['color'].charAt(0) === '#'
            ? splitColorValueOffset[i as number]['color']
            : colorNameToHex(splitColorValueOffset[i as number]['color']);
        nextColor = midColor;
        percent =
          value <= splitColorValueOffset[i as number]['b']
            ? 1 -
              Math.abs((value - splitColorValueOffset[i as number]['b']) / a)
            : (value - splitColorValueOffset[i as number]['b']) / a;
      } else if (
        splitColorValueOffset[i as number]['b'] <= value &&
        value <= nextOffset &&
        i === splitColorValueOffset.length - 1
      ) {
        midColor =
          splitColorValueOffset[i as number]['color'].charAt(0) === '#'
            ? splitColorValueOffset[i as number]['color']
      ...
      if (
        i !== splitColorValueOffset.length - 1 &&
        i < splitColorValueOffset.length
      ) {
        if (
          splitColorValueOffset[i as number]['b'] <= value &&
          value <= splitColorValueOffset[i + 1]['b']
        ) {
          midColor =
            splitColorValueOffset[i as number]['color'].charAt(0) === '#'
              ? splitColorValueOffset[i as number]['color']
          ...
          previousColor = midColor;
          nextColor =
            splitColorValueOffset[i + 1]['color'].charAt(0) === '#'
              ? splitColorValueOffset[i + 1]['color']
              : colorNameToHex(splitColorValueOffset[i + 1]['color']);
...
export function getPercentageColor(
  ...
  const r: number = getPercentage(
    percent,
    parseInt(prevColor.substr(0, 2), 16),
    parseInt(nextColor.substr(0, 2), 16)
  );
  const g: number = getPercentage(
    percent,
    parseInt(prevColor.substr(2, 2), 16),
    parseInt(nextColor.substr(2, 2), 16)
  );
  const b: number = getPercentage(
    percent,
    parseInt(prevColor.substr(4, 2), 16),
    parseInt(nextColor.substr(4, 2), 16)
  );
