import { EventHandler, Browser, remove, isNullOrUndefined } from '@hoyui/base';
import { DateFormatOptions } from '@hoyui/base';
import { ScrollElements, createScrollSvg } from './scrollbar-elements';
import { getElement, minMax, logBase } from '../utils/helper';
import { Chart } from '../../chart/chart';
import {
  Axis,
  IScrollbarThemeStyle,
  IScrollEventArgs,
} from '../../chart/index';
...
} from '../../chart/model/chart-base-model';
...
export class ScrollBar {
  ...
  /** @private */
  public scrollRange: VisibleRangeModel = {
    max: null,
    min: null,
    interval: null,
    delta: null,
  ...
  private getMouseXY(e: PointerEvent | MouseEvent): void {
    ...
    const svgRect: ClientRect = getElement(
      this.component.element.id + '_scrollBar_svg' + this.axis.name
  ...
  public scrollMouseDown(e: PointerEvent): void {
    ...
    this.getMouseXY(e);
    this.isResizeLeft =
      this.isExist(id, '_leftCircle_') || this.isExist(id, '_leftArrow_');
    this.isResizeRight =
      this.isExist(id, '_rightCircle_') || this.isExist(id, '_rightArrow_');
    //  this.previousXY = this.isVertical ? this.mouseY : this.mouseX;
    this.previousXY =
      this.isVertical && isInverse
        ? this.mouseY
        : this.isVertical
        ? this.width - this.mouseY
        : isInverse
    ...
    if (this.isExist(id, 'scrollBarThumb_') || this.isExist(id, 'gripCircle')) {
      ...
      const currentX: number = this.moveLength(
        this.previousXY,
        this.previousRectX
      );
      ...
      this.axis.zoomPosition =
        this.zoomPosition < 0
          ? 0
          : this.zoomPosition > 0.9
          ? 1
          : this.zoomPosition;
      if (this.isLazyLoad) {
        const thumbMove: string =
          elem.thumbRectX > this.previousRectX ? 'RightMove' : 'LeftMove';
        const args: IScrollEventArgs = this.calculateLazyRange(
          elem.thumbRectX,
          elem.thumbRectWidth,
          thumbMove
        );
    ...
    if (this.component.element.style.height === '') {
      ...
      this.component.element.style.height =
        this.component.availableSize.height + 'px';
    }
  }
  ...
  private moveLength(
    mouseXY: number,
    thumbX: number,
    circleRadius: number = this.axis.scrollbarSettings.height / 2
  ): number {
    let moveLength: number = (10 / 100) * (this.width - circleRadius * 2);
    if (mouseXY < thumbX) {
      moveLength =
        thumbX -
        (thumbX - moveLength > circleRadius ? moveLength : circleRadius);
    } else {
      moveLength =
        thumbX +
        (thumbX + this.scrollElements.thumbRectWidth + moveLength <
  ...
  private setZoomFactorPosition(currentX: number, currentWidth: number): void {
    ...
    const currentScrollWidth: number =
      currentX + currentWidth + circleRadius + circleWidth;
    const currentZPWidth: number = circleRadius + circleWidth / 2;
    const axisSize: number = this.isVertical ? axis.rect.height : this.width;
    this.zoomFactor =
      (currentWidth +
        (currentScrollWidth >= this.width ? circleRadius + circleWidth : 0)) /
      axisSize;
    this.zoomPosition =
      currentScrollWidth > axisSize
        ? 1 - axis.zoomFactor
        : currentX < circleRadius + circleWidth
        ? 0
        : (currentX - (currentX - currentZPWidth <= 0 ? currentZPWidth : 0)) /
          axisSize;
    this.zoomPosition =
      (this.component.enableRtl && !this.isVertical && !axis.isInversed) ||
      (axis.isInversed && !(this.component.enableRtl && !this.isVertical))
        ? 1 - (this.zoomPosition + axis.zoomFactor)
        : this.zoomPosition;
  }
  /**
  ...
  public scrollMouseMove(e: PointerEvent): void {
    ...
    if (
      this.isLazyLoad &&
      (this.isThumbDrag || this.isResizeLeft || this.isResizeRight)
    ) {
      args = this.calculateLazyRange(
        elem.thumbRectX,
        elem.thumbRectWidth,
        thumbMove
      );
    }
    const currentRange: ScrollbarSettingsRangeModel = args
      ? args.currentRange
      : null;
    if (this.isThumbDrag) {
  ...
  public scrollMouseWheel(e: WheelEvent): void {
    ...
    const direction: number =
      this.browserName === 'mozilla' && !this.isPointer
        ...
        ? 1
        : -1;
    const cumulative: number = Math.max(
      Math.max(1 / minMax(axis.zoomFactor, 0, 1), 1) + 0.25 * direction,
      1
    );
    const range: VisibleRangeModel = this.axis.visibleRange;
    const zoomPosition: number = this.zoomPosition;
    ...
    if (cumulative >= 1) {
      origin =
        axis.orientation === 'Horizontal'
          ? this.mouseX / axis.rect.width
          : 1 - this.mouseY / axis.rect.height;
      origin = origin > 1 ? 1 : origin < 0 ? 0 : origin;
      this.zoomFactor = cumulative === 1 ? 1 : minMax(1 / cumulative, 0, 1);
      this.zoomPosition =
        cumulative === 1
          ? 0
          : axis.zoomPosition + (axis.zoomFactor - this.zoomFactor) * origin;
    }
    elem.thumbRectX = this.isWithIn(this.zoomPosition * this.width)
      ? this.zoomPosition * this.width
      : elem.thumbRectX;
    this.isScrollUI = true;
    this.positionThumb(elem.thumbRectX, elem.thumbRectWidth);
    if (this.isLazyLoad) {
    ...
    if (this.isLazyLoad) {
      args = this.calculateMouseWheelRange(
        elem.thumbRectX,
        elem.thumbRectWidth
      );
      if (args) {
        if (
          args.currentRange.minimum !== args.previousAxisRange.minimum &&
          args.currentRange.maximum !== args.previousAxisRange.maximum
        ) {
          this.component.trigger(scrollEnd, args);
  ...
  public scrollMouseUp(): void {
    ...
    const currentScrollWidth: number =
      this.startX +
      this.scrollElements.thumbRectWidth +
      circleRadius +
      circleWidth;
    const currentZPWidth: number = circleRadius + circleWidth / 2;
    if ((this.isResizeLeft || this.isResizeRight) && !this.isLazyLoad) {
      this.axis.zoomFactor =
        currentScrollWidth >= this.width - 1 &&
        this.startX - currentZPWidth <= 0
          ? 1
          : this.zoomFactor;
    }
    if (this.isLazyLoad) {
      ...
      if ((moveLength > 0 || moveLength < 0) && this.isThumbDrag) {
        ...
        if (thumbMove === 'RightMove') {
          this.startX =
            this.startX + Math.abs(moveLength) < this.width - circleRadius
              ? this.startX
  ...
  public calculateMouseWheelRange(
    scrollThumbX: number,
    ...
    const previousRange: ScrollbarSettingsRangeModel = this.getStartEnd(
      this.previousStart,
      this.previousEnd,
      false
    );
  ...
  public calculateLazyRange(
    ...
    if (this.isResizeRight || thumbMove === 'RightMove') {
      currentScrollWidth = this.isResizeRight
        ? currentScrollWidth + circleWidth
        : currentScrollWidth;
      zoomFactor = currentScrollWidth / this.width;
      zoomPosition =
        thumbMove === 'RightMove'
          ? (scrollThumbX + circleRadius) / this.width
          : this.axis.zoomPosition;
      currentStart =
        thumbMove === 'RightMove'
          ? range.min + zoomPosition * range.delta
          : this.previousStart;
      ...
      currentEnd =
        thumbMove === 'LeftMove'
          ? currentStart + zoomFactor * range.delta
          : this.previousEnd;
  ...
  private getStartEnd(
    ...
    isCurrentStartEnd: boolean
  ): ScrollbarSettingsRangeModel {
    const valueType: string = this.valueType;
    if (
      (valueType === 'DateTime' || valueType === 'DateTimeCategory') &&
      isCurrentStartEnd
    ) {
      ...
      if (this.axis.valueType === 'Category') {
        currentEnd -=
          !this.axis.scrollbarSettings.enableZoom &&
          currentEnd - currentStart > this.previousEnd - this.previousStart
            ? currentEnd -
              currentStart -
              (this.previousEnd - this.previousStart)
            : 0;
      }
  ...
  public render(isScrollExist: boolean): Element {
    if (
      this.component.zoomModule ||
      (isScrollExist && this.axis.scrollbarSettings.enable)
    ) {
      this.getDefaults();
    }
    this.getTheme();
    ...
    this.svgObject.appendChild(
      this.scrollElements.renderElements(
        this,
        this.component.svgRenderer as SvgRenderer
      )
    );
  ...
  private setCursor(target: Element): void {
    ...
    (this.svgObject as HTMLElement).style.cursor =
      (id.indexOf('scrollBarThumb_') > -1 || id.indexOf('_gripCircle') > -1) &&
      this.axis.scrollbarSettings.height >= 12
        ? '-webkit-grab'
        : id.indexOf('Circle_') > -1 || id.indexOf('Arrow_') > -1
        ? this.isVertical
          ? 'ns-resize'
          : 'ew-resize'
        : 'auto';
  ...
  private setTheme(target: Element): void {
    const id: string = target.id;
    const isLeftHover: boolean =
      id.indexOf('_leftCircle_') > -1 || id.indexOf('_leftArrow_') > -1;
    const isRightHover: boolean =
      id.indexOf('_rightCircle_') > -1 || id.indexOf('_rightArrow_') > -1;
    const style: IScrollbarThemeStyle = this.scrollbarThemeStyle;
    const leftArrowEle: HTMLElement = this.scrollElements
      .leftArrowEle as HTMLElement;
    const rightArrowEle: HTMLElement = this.scrollElements
      .rightArrowEle as HTMLElement;
    const leftCircleEle: HTMLElement = this.scrollElements
      .leftCircleEle as HTMLElement;
    const rightCircleEle: HTMLElement = this.scrollElements
      .rightCircleEle as HTMLElement;
    const isAxis: boolean = this.isCurrentAxis(target, leftArrowEle);
    leftCircleEle.style.fill =
      isLeftHover && isAxis ? style.circleHover : style.circle;
    rightCircleEle.style.fill =
      isRightHover && isAxis ? style.circleHover : style.circle;
    leftCircleEle.style.stroke =
      isLeftHover && isAxis ? style.circleHover : style.circle;
    rightCircleEle.style.stroke =
      isRightHover && isAxis ? style.circleHover : style.circle;
    if (this.component.theme === 'HighContrastLight') {
      leftArrowEle.style.fill =
        isLeftHover && isAxis ? style.arrowHover : style.arrow;
      leftArrowEle.style.stroke =
        isLeftHover && isAxis ? style.arrowHover : style.arrow;
      rightArrowEle.style.fill =
        isRightHover && isAxis ? style.arrowHover : style.arrow;
      rightArrowEle.style.stroke =
        isRightHover && isAxis ? style.arrowHover : style.arrow;
      leftCircleEle.style.stroke =
        isLeftHover && isAxis ? style.circleHover : style.circle;
      rightCircleEle.style.stroke =
  ...
  private resizeThumb(): void {
    ...
    if (this.isResizeLeft && mouseXY >= 0) {
      let currentX: number =
        thumbX + (mouseXY > this.previousXY ? diff : -diff);
      currentWidth =
        currentX - circleRadius >= 0
          ? this.previousWidth + (mouseXY > this.previousXY ? -diff : diff)
          : this.previousWidth;
      ...
      currentWidth =
        mouseXY >= minThumbWidth + this.scrollElements.thumbRectX &&
        mouseXY <= this.width - circleRadius
          ? mouseXY - this.scrollElements.thumbRectX
          : this.previousWidth;
      this.scrollElements.thumbRectWidth = this.previousWidth = currentWidth;
      this.previousXY = mouseXY;
  ...
  private positionThumb(currentX: number, currentWidth: number): void {
    ...
    elem.slider.setAttribute(
      'x',
      this.axis.scrollbarSettings.enableZoom
        ? currentX.toString()
        : (currentX - this.axis.scrollbarSettings.height / 2).toString()
    );
    elem.slider.setAttribute(
      'width',
      this.axis.scrollbarSettings.enableZoom
        ? currentWidth.toString()
        : (currentWidth + this.axis.scrollbarSettings.height).toString()
    );
    elem.leftCircleEle.setAttribute('cx', currentX.toString());
    elem.rightCircleEle.setAttribute(
      'cx',
      (currentX + currentWidth).toString()
    );
    elem.setArrowDirection(currentX, currentWidth, this.height);
    elem.gripCircle.setAttribute(
      'transform',
      'translate(' +
        (currentX + currentWidth / 2 + (this.isVertical ? 1 : -1) * padding) +
        ',' +
        (this.isVertical
          ? this.axis.scrollbarSettings.height / 2 + padding / 2 - 0.5
          : this.axis.scrollbarSettings.height / 2 - padding / 2 - 0.5) +
        ') rotate(' +
        (this.isVertical ? '180' : '0') +
        ')'
    );
  }
  /**
  ...
  private getDefaults(): void {
    ...
    this.zoomPosition = this.isLazyLoad
      ? isRtlEnabled
        ? 1 - (this.zoomPosition + this.zoomFactor)
        : this.zoomPosition
      : isRtlEnabled
      ? 1 - (axis.zoomPosition + axis.zoomFactor)
      : axis.zoomPosition;
    let currentWidth: number =
      this.zoomFactor * (this.isVertical ? axis.rect.height : axis.rect.width);
    currentWidth =
      (this.isLazyLoad && !this.axis.scrollbarSettings.enableZoom) ||
      currentWidth > minThumbWidth
        ? currentWidth
        : minThumbWidth;
    ...
    const currentX: number =
      this.zoomPosition * (this.isVertical ? axis.rect.height : this.width);
    const minThumbX: number = this.width - minThumbWidth - circleRadius;
    this.scrollElements.thumbRectX =
      currentX > minThumbX
        ? minThumbX
        : currentX < circleRadius
        ? circleRadius
        : currentX;
    this.scrollElements.thumbRectWidth = this.isThumbDrag
      ? this.scrollElements.thumbRectWidth
      : currentWidth + this.scrollElements.thumbRectX <
        this.width - circleRadius * 2
      ? currentWidth
      : this.width - this.scrollElements.thumbRectX - circleRadius;
  }
  ...
  public getLazyDefaults(axis: Axis): void {
    ...
    this.valueType = valueType =
      (!scrollbarSettings.range.minimum || !scrollbarSettings.range.maximum) &&
      scrollbarSettings.pointsLength
        ? 'Double'
        : valueType;
    ...
    switch (valueType) {
      ...
      case 'Logarithmic':
        start = range.minimum
          ? (range.minimum as number)
          : pointsLength
          ? 0
          : visibleRange.min;
        end = range.maximum
          ? (range.maximum as number)
          : pointsLength
          ? pointsLength - 1
          : visibleRange.max;
        break;
      ...
      case 'DateTimeCategory':
        start = range.minimum
          ? Date.parse(dateParser(dateFormatter(range.minimum)))
          : visibleRange.min;
        end = range.maximum
          ? Date.parse(dateParser(dateFormatter(range.maximum)))
          : visibleRange.max;
        break;
    }
    ...
    const zoomFactor: number =
      (visibleRange.max - visibleRange.min) / (end - start);
    const zoomPosition: number = (visibleRange.min - start) / (end - start);
    this.zoomFactor =
      range.minimum || range.maximum
        ? zoomFactor
        : this.axis.maxPointLength / axis.scrollbarSettings.pointsLength;
    this.zoomPosition =
      range.minimum || range.maximum ? zoomPosition : axis.zoomPosition;
    this.zoomPosition =
      (this.component.enableRtl &&
        axis.orientation === 'Horizontal' &&
        !axis.isInversed) ||
      (axis.isInversed &&
        !(this.component.enableRtl && axis.orientation === 'Horizontal'))
        ? 1 - (this.zoomPosition + this.zoomFactor)
        : this.zoomPosition;
  ...
  private getArgs(
    ...
    zoomPosition?: number,
    zoomFactor?: number,
    currentRanges?: ScrollbarSettingsRangeModel
  ): IScrollEventArgs {
