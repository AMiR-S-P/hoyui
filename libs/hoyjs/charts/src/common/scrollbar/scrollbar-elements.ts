...
export function createScrollSvg(
  ...
  for (const tempSeries of scrollbar.axis.series) {
    if (tempSeries.visible) {
      // To avoid the console error, when the visibility of the series is false.
  ...
  scrollbar.svgObject = renderer.createSvg({
    id:
      scrollbar.component.element.id +
      '_' +
      'scrollBar_svg' +
      scrollbar.axis.name,
    ...
    style:
      ...
      ((scrollbar.axis.isAxisOpposedPosition && isHorizontalAxis
        ? -16
        : enablePadding
        ? markerHeight
        : 0) +
        rect.y +
        Math.max(0.5, scrollbar.axis.lineStyle.width / 2)) +
      'px;left: ' +
      ((scrollbar.axis.isAxisOpposedPosition && !isHorizontalAxis ? 16 : 0) +
        rect.x -
...
export class ScrollElements {
  ...
  private backRect(
    ...
    renderer: SvgRenderer,
    parent: Element,
    scrollBar: ScrollbarSettingsModel
  ): void {
    const style: IScrollbarThemeStyle = scroll.scrollbarThemeStyle;
    const backRectEle: Element = renderer.drawRectangle(
      new RectOption(
        this.chartId + 'scrollBarBackRect_' + scroll.axis.name,
        scrollBar.trackColor || style.backRect,
        { width: 1, color: scrollBar.trackColor || style.backRect },
        1,
        new Rect(0, 0, scroll.width, scroll.height),
        scrollBar.trackRadius,
        scrollBar.trackRadius
      )
    ) as HTMLElement;
  ...
  private arrows(
    ...
    const option: PathOption = new PathOption(
      this.chartId + 'scrollBar_leftArrow_' + scroll.axis.name,
      style.arrow,
      1,
      style.arrow,
      1,
  ...
  public setArrowDirection(
    ...
    const leftDirection: string =
      'M ' +
      (thumbRectX - circleRadius / 2) +
      ' ' +
      height / 2 +
      ' ' +
      'L ' +
      (thumbRectX - circleRadius / 2 + (height / 2 - circleRadius / 4)) +
      ' ' +
      ...
      circleRadius / 2 +
      ' Z';
    const rightDirection: string =
      'M ' +
      (thumbRectX + thumbRectWidth + circleRadius / 2) +
      ' ' +
      height / 2 +
      ' ' +
      'L ' +
  ...
  public thumb(
    ...
    const style: IScrollbarThemeStyle = scroll.scrollbarThemeStyle;
    this.slider = renderer.drawRectangle(
      new RectOption(
        this.chartId + 'scrollBarThumb_' + scroll.axis.name,
        scrollBar.scrollbarColor || style.thumb,
        { width: 1, color: scrollBar.scrollbarColor || style.thumb },
        1,
        new Rect(
          scrollBar.enableZoom
            ? this.thumbRectX
            : this.thumbRectX - scroll.height / 2,
          0,
          scrollBar.enableZoom
            ? this.thumbRectWidth
            : this.thumbRectWidth + scroll.height / 2,
          scroll.height
        ),
  ...
  private renderCircle(
    ...
    const option: CircleOption = new CircleOption(
      this.chartId + 'scrollBar_leftCircle_' + scroll.axis.name,
      style.circle,
      { width: 1, color: style.circle },
      1,
      this.thumbRectX,
      scroll.height / 2,
      scroll.height / 2
    );
    const scrollShadowEle: string =
      '<filter x="-25.0%" y="-20.0%" width="150.0%" height="150.0%" filterUnits="objectBoundingBox"' +
    ...
    (<HTMLElement>shadowGroup).innerText =
      '<use fill="black" fill-opacity="1" filter="url(#scrollbar_shadow)" xlink:href="#' +
      this.chartId +
      'scrollBar_leftCircle_' +
      scroll.axis.name +
      '"></use><use fill="black" fill-opacity="1" filter="url(#scrollbar_shadow)" xlink:href="#' +
      this.chartId +
      'scrollBar_rightCircle_' +
      scroll.axis.name +
      '"></use>';
  ...
  private thumbGrip(
    ...
    let sidePadding: number =
      scroll.component.theme.indexOf('Fluent2') > -1 ||
      scroll.component.theme.indexOf('Bootstrap5') > -1
        ? -5
        : 0;
    ...
    const style: IScrollbarThemeStyle = scroll.scrollbarThemeStyle;
    const option: CircleOption = new CircleOption(
      this.chartId + 'scrollBar_gripCircle0' + '_' + scroll.axis.name,
      scrollBar.gripColor || style.grip,
      { width: 1, color: scrollBar.gripColor || style.grip },
      1,
      0,
      0,
      1
    ...
    this.gripCircle = renderer.createGroup({
      ...
      transform:
        ...
        (!scrollBar.enableZoom
          ? this.thumbRectX +
            this.thumbRectWidth / 2 +
            (scroll.isVertical ? 2 : 0) * padding -
            scrollBar.height / 2
          : this.thumbRectX +
            this.thumbRectWidth / 2 +
            (scroll.isVertical ? 1 : -1) * padding) +
        ',' +
        (scroll.isVertical
          ? scroll.height / 2 + padding / 2 - 0.5
          : scroll.height / 2 - padding / 2 - 0.5) +
        ') rotate(' +
        (scroll.isVertical ? '180' : '0') +
        ')',
    });
    for (
      let i: number = 1;
      ...
      option.id =
        this.chartId + 'scrollBar_gripCircle' + i + '_' + scroll.axis.name;
