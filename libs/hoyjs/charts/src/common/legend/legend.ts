...
export class BaseLegend {
  ...
  constructor(
    ...
    this.isChartControl =
      chart.getModuleName() === 'chart' || chart.getModuleName() === 'chart3d';
    this.isAccChartControl =
      chart.getModuleName() === 'accumulationchart' ||
      chart.getModuleName() === 'circularchart3d';
    ...
    this.arrowWidth =
      2 * (this.fivePixel + this.pageButtonSize + this.fivePixel);
  ...
  public calculateLegendBounds(
    ...
    this.itemPadding = this.legend.itemPadding
      ? this.legend.itemPadding
      : this.isVertical
      ? this.legend.padding
      : 20;
    if (this.isVertical) {
      this.legendBounds.height =
        stringToNumber(
          legend.height,
          availableSize.height - (rect.y - this.chart.margin.top)
        ) || rect.height;
      this.legendBounds.width = stringToNumber(
        legend.width || defaultValue,
        availableSize.width
      );
    } else {
      this.legendBounds.width =
        stringToNumber(legend.width, availableSize.width) || rect.width;
      this.legendBounds.height = stringToNumber(
        legend.height || defaultValue,
        availableSize.height
      );
    }
    if (this.chart.getModuleName() === 'chart3d') {
      (this.library as Legend3D).get3DLegendBounds(
        availableSize,
        this.legendBounds,
        legend as Chart3DLegendSettingsModel
      );
    } else {
      (
        this.library as
          ...
          | AccumulationLegend
          | StockLegend
      ).getLegendBounds(availableSize, this.legendBounds, legend);
    }
    if (!this.isBulletChartControl) {
      this.legendBounds.width +=
        this.legend.containerPadding.left + this.legend.containerPadding.right;
      this.legendBounds.height +=
        this.legend.containerPadding.top + this.legend.containerPadding.bottom;
    }
    this.getLocation(
      this.position,
      legend.alignment,
  ...
  private getPosition(position: LegendPosition, availableSize: Size): void {
    ...
    if (
      this.isChartControl ||
      this.isBulletChartControl ||
      this.isStockChartControl
    ) {
      this.position = position !== 'Auto' ? position : 'Bottom';
    } else {
      if (
        position === 'Auto' &&
        (((chart || accumulation).visibleSeries &&
  ...
  protected setBounds(
    ...
    let titleHeight: number =
      legend.title && legend.titlePosition === 'Top'
        ? this.legendTitleSize.height + this.fivePixel
        : 0;
    if (
      this.isVertical &&
      this.isPaging &&
    ...
    if (legend.mode === 'Gradient') {
      legendBounds.width = legend.width
        ? legendBounds.width
        : this.isVertical
        ? computedWidth
        : 0.75 * legendBounds.width;
      legendBounds.height = legend.height
        ? legendBounds.height
        : this.isVertical
        ? 0.75 * legendBounds.height
        : computedHeight;
    } else {
    ...
    if (!this.isBulletChartControl) {
      if (this.isTop && legend.titleStyle.textOverflow !== 'None') {
        ...
        legendBounds.height +=
          legend.titleStyle.textOverflow === 'Wrap' &&
          this.legendTitleCollections.length > 1
            ? this.legendTitleSize.height -
    ...
    this.rowCount = Math.max(
      1,
      Math.ceil(
        (legendBounds.height - legend.padding - titleHeight) /
  ...
  private getLocation(
    ...
    const categoryFieldValue: number =
      isBulletChart && bulletChart.categoryField !== ''
        ? maxLabelSize.width + this.chart.border.width + padding * 3
        : 0;
    ...
    let legendHeight: number =
      ...
      this.legend.margin.bottom;
    let legendWidth: number =
      legendBounds.width +
      padding +
      this.legend.margin.left +
      this.legend.margin.right;
    if (position === 'Bottom') {
      legendBounds.x = this.alignLegend(
        legendBounds.x,
        availableSize.width,
        legendBounds.width,
        alignment
      ...
      legendBounds.y =
        previousLegendBounds &&
        (legendBounds.height === previousLegendBounds.height ||
          (this.chart as AccumulationChart).series[0].type !== 'Pie')
          ? previousLegendBounds.y
          : rect.y +
      ...
      legendBounds.y +=
        isBulletChart &&
        !bulletChart.opposedPosition &&
        !labelIns &&
        !ticklIns &&
        !isVertical
          ? bulletChart.majorTickLines.height +
            marginBottom +
            this.legend.border.width +
            padding * 2
          : isVertical && bulletChart.categoryField !== ''
          ? maxLabelSize.height + padding * 2
          : 0;
      if (
        !pointAnimation ||
        legendBounds.height !== previousLegendBounds.height
      ...
      if (this.isChartControl) {
        axisTextSize = measureText(
          '100',
          (this.chart as Chart).verticalAxes[0].labelStyle,
      ...
      legendBounds.y =
        ...
        (legendBounds.height === previousLegendBounds.height ||
          (this.chart as AccumulationChart).series[0].type !== 'Pie')
          ? previousLegendBounds.y
          : rect.y + padding + this.legend.margin.top;
      legendBounds.y -=
        isBulletChart &&
        bulletChart.opposedPosition &&
        !labelIns &&
        !ticklIns &&
        !isVertical
          ? bulletChart.majorTickLines.height + this.chart.margin.top
          : 0;
      legendHeight -= isBulletChart
        ? -padding * 2
        : this.isChartControl
        ? -padding * 2 - axisTextSize.height / 2
        : 0;
      if (
        !pointAnimation ||
        legendBounds.height !== previousLegendBounds.height
      ...
      legendBounds.x =
        previousLegendBounds &&
        (legendBounds.width === previousLegendBounds.width ||
          (this.chart as AccumulationChart).series[0].type !== 'Pie')
          ? previousLegendBounds.x
          : rect.x +
            (rect.width - legendBounds.width) -
            this.legend.margin.right;
      legendBounds.y =
        rect.y +
        this.alignLegend(
          0,
          availableSize.height - (rect.y + marginBottom),
          legendBounds.height,
          alignment
        );
      legendWidth +=
      ...
      legendBounds.x =
        ...
        (legendBounds.width === previousLegendBounds.width ||
          (this.chart as AccumulationChart).series[0].type !== 'Pie')
          ? previousLegendBounds.x
          : legendBounds.x + this.legend.margin.left;
      legendBounds.y =
        rect.y +
        this.alignLegend(
          0,
      ...
      legendWidth +=
        ...
        isVertical
          ? legendBounds.x -
            ...
            padding +
            bulletChart.majorTickLines.height
          : bulletChart.orientation !== 'Vertical' &&
            bulletChart.categoryField !== ''
          ? categoryFieldValue
          : 0;
      if (
        !pointAnimation ||
  ...
  public renderLegend(
    ...
    let titlePlusArrowWidth: number = 0;
    ...
    const firstLegend: number = this.findFirstLegendPosition(
      this.legendCollections
    );
    const padding: number = legend.padding;
    this.itemPadding = this.isBulletChartControl
      ? legend.padding
      : this.itemPadding;
    ...
    const upArrowHeight: number =
      this.isPaging && !legend.enablePages && this.isVertical
        ? this.pageButtonSize
        : 0;
    const legendGroup: Element = chart.renderer.createGroup({
      id: this.legendID + '_g',
    });
    const legendTranslateGroup: Element = this.createLegendElements(
      chart,
      legendBounds,
      legendGroup,
      legend,
    ...
    titleHeight = !this.isTitle
      ? 0
      : this.isTop || this.isVertical
      ? this.legendTitleSize.height
      : 0;
    if (this.isChartControl || (this.isAccChartControl && !this.isVertical)) {
      ...
      for (let i: number = 1; i < this.rowHeights.length; i++) {
        if (
          rowHeights + this.rowHeights[i as number] + padding >
          this.legendBounds.height -
            this.pageButtonSize -
            this.maxItemHeight / 2
        ) {
        ...
        rowHeights +=
          this.rowHeights[i as number] +
          (this.isVertical ? this.itemPadding : padding);
      }
      this.pageHeights[pageCount - 1] = rowHeights + titleHeight;
    ...
    for (let i: number = 0; i < this.legendCollections.length; i++) {
      if (this.legendCollections[i as number].text !== '') {
        maxHeight = Math.max(
          this.legendCollections[i as number].textSize.height,
          maxHeight
        );
        break;
      } else {
    ...
    if (!isPaging && this.isPaging && !this.isVertical) {
      titlePlusArrowWidth = !this.isTitle
        ? 0
        : titlePosition === 'Left'
        ? this.legendTitleSize.width
        : 0;
      titlePlusArrowWidth += this.pageButtonSize + 2 * this.fivePixel;
    } else if (this.isTitle && !this.isPaging && !this.isVertical) {
      titlePlusArrowWidth =
        titlePosition === (!this.isRtlEnable ? 'Left' : 'Right')
          ? this.fivePixel + this.legendTitleSize.width
          : 0;
    }
    if (
      (chart as Chart).legendSettings.mode === 'Gradient' &&
      this.legendCollections.length > 1
    ) {
      ...
      let count: number = 0;
      let previousLegend: LegendOptions =
        this.legendCollections[firstLegend as number];
      // starting shape center x,y position && to resolve lint error used new line for declaration
      const startPadding: number = this.isBulletChartControl
        ? padding
        : titlePlusArrowWidth +
          padding +
          legend.shapeWidth / 2 +
          legend.containerPadding.left;
      const xLocation: number =
        this.isBulletChartControl && !this.isRtlEnable
          ? legendBounds.x +
            titlePlusArrowWidth +
            padding +
            legend.shapeWidth / 2
          : !this.isRtlEnable
          ? legendBounds.x + startPadding
          : legendBounds.x +
            (this.chart.getModuleName() === 'accumulationchart' &&
            this.isVertical
              ? this.maxWidth
              : legendBounds.width) -
            startPadding;
      const start: ChartLocation = new ChartLocation(
        ...
        this.isBulletChartControl
          ...
          : legendBounds.y +
            ...
            padding +
            this.maxItemHeight / 2 +
            legend.containerPadding.top
      );
      const anchor: string =
        (chart as Chart).isRtlEnabled || (chart as Chart).enableRtl
          ? 'end'
          : 'start';
      ...
      const textPadding: number =
        legend.shapePadding + this.itemPadding + legend.shapeWidth;
      ...
      if (!legend.enablePages && this.isPaging) {
        ...
        const leftSpace: number =
          this.isTitle && !this.isVertical && titlePosition === 'Left'
            ? this.legendTitleSize.width + this.fivePixel
            : 0;
        const bottomSapce: number = this.isVertical
          ? this.pageButtonSize + Math.abs(y - legendBounds.y)
          : 0;
        let rightSpace: number =
          this.isTitle && !this.isVertical && titlePosition === 'Right'
            ? this.legendTitleSize.width + this.fivePixel
            : 0;
        rightSpace += this.isVertical
          ? 0
          : this.fivePixel + this.pageButtonSize + this.fivePixel;
        pagingLegendBounds = new Rect(
          x,
          y,
          legendBounds.width - rightSpace - leftSpace,
      ...
      for (let i: number = 0; i < this.legendCollections.length; i++) {
        ...
        legendIndex = !this.isReverse
          ? count
          : this.legendCollections.length - 1 - count;
        if (this.chart.getModuleName() === 'accumulationchart') {
          legendOption.fill = (
            (this.chart as Chart) ||
            (this.chart as AccumulationChart) ||
            (this.chart as StockChart) ||
            (this.chart as Chart3D)
          ).visibleSeries[0].points[legendOption.pointIndex].color;
        }
        if (this.chart.getModuleName() === 'stockChart') {
          legendOption.type = (this.chart as StockChart).visibleSeries[
            count as number
          ].type;
        }
        this.accessbilityText = this.isBulletChartControl
          ? 'Legend of bullet chart' + '' + legendOption.text
        ...
        if (
          ...
          legendSeriesGroup = chart.renderer.createGroup({
            id:
              this.legendID + this.generateId(legendOption, '_g_', legendIndex),
          });
          if (legendSeriesGroup) {
            ...
            legendSeriesGroup.setAttribute(
              ...
              legend.description ||
                legendOption.text +
                  ' series is ' +
                  (legendOption.visible
                    ? 'showing, press enter to hide the '
                    : 'hidden, press enter to show the ') +
                  legendOption.text +
                  ' series'
            );
            ...
            legendSeriesGroup.setAttribute(
              'aria-pressed',
              legendOption.visible ? 'true' : 'false'
            );
          }
          this.library.getRenderPoint(
            legendOption,
            start,
            textPadding,
            previousLegend,
          ...
          if (legendSeriesGroup) {
            (legendSeriesGroup as HTMLElement).style.cssText =
              'pointer-events: bounding-box; cursor: ' +
              ((!legend.toggleVisibility &&
                ((chart as Chart).selectionMode === 'None' ||
                  (chart as Chart).highlightMode === 'None' ||
                  (chart as AccumulationChart).selectionMode === 'None')) ||
              this.isBulletChartControl
                ? 'auto'
                : 'pointer');
          }
      ...
      this.totalPages =
        this.isPaging &&
        !this.isBulletChartControl &&
        !this.legend.enablePages &&
        !this.isVertical &&
        this.totalPages > this.chartRowCount
          ? this.chartRowCount
          : this.totalPages;
    ...
    if (pointAnimation) {
      const translateX: string = `translate(${
        this.rowCount > 1
          ? 0
          : Math.round(Number(xValue)) - Math.round(this.legendBounds.x)
      } 
              + ',' + ${
                Math.round(Number(yValue)) - Math.round(this.legendBounds.y)
              })`;
      const translateY: string = `translate(${
        Math.round(Number(xValue)) - Math.round(this.legendBounds.x)
      }, ${Math.round(Number(yValue)) - Math.round(this.legendBounds.y)})`;

      appendChildElement(
        (chart as Chart).enableCanvas,
        chart.svgObject,
        legendGroup,
        redraw,
        true,
  ...
  private getLinearLegend(
    ...
    const linerGradientEle: Element = document.createElementNS(
      xmlns,
      'linearGradient'
    );
    ...
    linerGradientEle.setAttribute(
      'id',
      this.generateId(previousLegend, 'linearGradient', gradientLegendCount)
    );
    ...
    const startTextSize: Size = measureText(
      startLabel,
      legend.textStyle,
      this.chart.themeStyle.legendLabelFont
    );
    const endTextSize: Size = measureText(
      endLabel,
      legend.textStyle,
      this.chart.themeStyle.legendLabelFont
    );
    const textWidth: number =
      startTextSize.width > endTextSize.width
        ? startTextSize.width
        : endTextSize.width;
    const textHeight: number =
      startTextSize.height > endTextSize.height
        ? startTextSize.height
        : endTextSize.height;
    ...
    if (this.isVertical) {
      ...
      startLabelX =
        legendBounds.x +
        (!this.isRtlEnable
          ? legend.padding + (textWidth - startTextSize.width)
      ...
      startLabelY =
        legendBounds.y + legendBounds.height * 0.5 + textHeight * 0.25;
    ...
    textOptions.id =
      this.legendID + this.generateId(previousLegend, '_text_', 1);
    const fontcolor: string = previousLegend.visible
      ? legend.textStyle.color || chart.themeStyle.legendLabelFont.color
      : hiddenColor;
    const isCanvas: boolean = this.isStockChartControl
      ? false
      : (this.chart as Chart).enableCanvas;
    textElement(
      chart.renderer,
      textOptions,
      legend.textStyle,
      fontcolor,
      legendTranslateGroup,
      false,
      false,
      false,
      false,
      null,
      this.currentPageNumber && isCanvas
        ? new Rect(
            0,
            -this.translatePage(
              isCanvas,
    ...
    textOptions.id =
      this.legendID + this.generateId(previousLegend, '_text_', 2);
    textElement(
      chart.renderer,
      textOptions,
      legend.textStyle,
  ...
  protected calculateLegendTitle(
    legend: LegendSettingsModel,
    legendBounds: Rect
  ): void {
    if (legend.title) {
      ...
      const padding: number =
        legend.titleStyle.textOverflow === 'Trim' ? 2 * legend.padding : 0;
      if (this.isTop || this.isVertical) {
        this.legendTitleCollections = getTitle(
          legend.title,
          legend.titleStyle,
          legendBounds.width - padding,
          this.chart.enableRtl,
          this.chart.themeStyle.legendTitleFont
        );
      } else {
        this.legendTitleCollections[0] = textTrim(
          legend.maximumTitleWidth,
          legend.title,
          legend.titleStyle,
          this.chart.enableRtl,
          this.chart.themeStyle.legendTitleFont
      ...
      const text: string = this.isTop
        ? legend.title
        : this.legendTitleCollections[0];
      this.legendTitleSize = measureText(
        text,
        legend.titleStyle,
        this.chart.themeStyle.legendTitleFont
      );
  ...
  private renderLegendTitle(
    ...
    this.isTop = legend.titlePosition === 'Top';
    let anchor: string = getTextAnchor(
      legend.titleStyle.textAlignment,
      chart.enableRtl
    );
    let x: number = titlePositionX(legendBounds, legend.titleStyle);
    anchor =
      this.isTop || this.isVertical ? anchor : chart.enableRtl ? 'end' : '';
    x =
      alignment === 'Near'
        ? x + padding
        : alignment === 'Far'
    ...
    const topPadding: number =
      legendBounds.height / 2 + this.legendTitleSize.height / 4;
    const y: number =
      legendBounds.y +
      (!this.isTop && !this.isVertical
        ? topPadding
        : this.legendTitleSize.height / this.legendTitleCollections.length);
    const legendTitleTextOptions: TextOption = new TextOption(
      this.legendID + '_title',
      x,
      y,
      anchor,
    ...
    textElement(
      chart.renderer,
      legendTitleTextOptions,
      legend.titleStyle,
      legend.titleStyle.color || this.chart.themeStyle.legendTitleFont.color,
      legendGroup,
      null,
      null,
      null,
  ...
  private createLegendElements(
    ...
    const options: RectOption = new RectOption(
      ...
      0,
      '',
      this.legend.border.dashArray
    );
    const legendItemsGroup: Element = chart.renderer.createGroup({
      id: id + '_collections',
    });
    const isCanvas: boolean = this.isStockChartControl
      ? false
      : (chart as Chart).enableCanvas;
    const clippath: Element = chart.renderer.createClipPath({
      id: id + '_clipPath',
    });
    options.width =
      this.isRtlEnable &&
    ...
    options.width =
      !this.isChartControl &&
      chart.getModuleName() !== 'bulletChart' &&
      !this.isStockChartControl &&
      this.isVertical
        ? this.maxWidth -
          padding +
          legend.containerPadding.left +
          legend.containerPadding.right
        : legendBounds.width;
    if (!isCanvas) {
    ...
    if (!isCanvas) {
      (legendItemsGroup as HTMLElement).style.cssText =
  ...
  protected renderSymbol(
    legendOption: LegendOptions,
    group: Element,
    legendIndex: number
  ): void {
    const control: BulletChart = this.isBulletChartControl
      ? (this.chart as BulletChart)
      : null;
    const symbolColor: string = legendOption.visible
      ? legendOption.fill
      : '#D3D3D3';
    const isStrokeWidth: boolean =
      (this.chart.getModuleName() === 'chart' ||
        this.chart.getModuleName() === 'stockChart') &&
      ((legendOption.shape === 'SeriesType' &&
        legendOption.type.toLowerCase().indexOf('line') > -1 &&
        legendOption.type.toLowerCase().indexOf('area') === -1) ||
        legendOption.shape === 'HorizontalLine' ||
        legendOption.shape === 'VerticalLine' ||
        legendOption.shape === 'Cross');
    ...
    let shape: string =
      legendOption.shape === 'SeriesType'
        ? legendOption.type
        : legendOption.shape;
    let strokewidth: number = isStrokeWidth
      ? this.legend.mode === 'Series'
        ? (this.chart as Chart).visibleSeries[legendIndex as number].width
        : (this.chart as Chart).visibleSeries[0].width
      : this.isBulletChartControl && legendOption.shape === 'Multiply'
      ? 4
      : 1;
    ...
    if (
      isCustomBorder &&
      legendIndex < (this.chart as Chart).visibleSeries.length
    ) {
      const series: Series = (this.chart as Chart).visibleSeries[
        legendIndex as number
      ];
      const seriesBorder: BorderModel = series.border;
      const isLineShapemarker: boolean =
        (shape as ChartShape) === 'HorizontalLine' ||
        (shape as ChartShape) === 'VerticalLine';
      borderColor = isLineShapemarker
        ? symbolColor
        : seriesBorder.color
        ? seriesBorder.color
        : symbolColor;
      strokewidth = isLineShapemarker
        ? series.width
        : seriesBorder.width
        ? seriesBorder.width
        : 1;
    ...
    const symbolOption: PathOption = new PathOption(
      this.legendID + this.generateId(legendOption, '_shape_', legendIndex),
      symbolColor,
      strokewidth,
      isCustomBorder ? borderColor : symbolColor,
      this.legend.opacity,
      legendOption.dashArray,
      ''
    );
    const textSize: Size = measureText(
      legendOption.text,
      this.legend.textStyle,
      this.chart.themeStyle.legendLabelFont
    );
    const x: number =
      this.legend.isInversed && !this.isRtlEnable
        ? legendOption.location.x + textSize.width + this.legend.shapePadding
    ...
    if (!isCanvas) {
      group.appendChild(
        drawSymbol(
          { x: x, y: y },
          shape,
          new Size(this.legend.shapeWidth, this.legend.shapeHeight),
          legendOption.url,
          symbolOption,
      ...
      regionPadding = -this.translatePage(
        isCanvas,
        null,
        this.currentPageNumber - 1,
        this.currentPageNumber
      ...
      this.legendRegions.push({
        rect: new Rect(
          legendOption.location.x - this.legend.shapeWidth,
          legendOption.location.y,
          this.legend.shapeWidth + this.legend.shapePadding,
          this.legend.shapeHeight + regionPadding
        ),
        index: legendIndex,
      });
    }
    if (
      (shape === 'Line' &&
        legendOption.markerVisibility &&
        legendOption.markerShape !== 'Image') ||
      (legendOption.type === <AccumulationType>'Doughnut' &&
        shape === 'Doughnut')
    ) {
      symbolOption.id =
        this.legendID +
        this.generateId(legendOption, '_shape_marker_', legendIndex);
      shape =
        legendOption.type === <AccumulationType>'Doughnut'
          ? 'Circle'
          : legendOption.markerShape;
      symbolOption.fill =
        legendOption.type === <AccumulationType>'Doughnut'
          ? '#FFFFFF'
          : symbolOption.fill;
      if (!isCanvas) {
        group.appendChild(
          drawSymbol(
            { x: x, y: y },
            shape,
            new Size(this.legend.shapeWidth / 2, this.legend.shapeHeight / 2),
            '',
            symbolOption,
            this.accessbilityText,
            null,
  ...
  protected renderText(
    ...
    const hiddenColor: string = '#D3D3D3';
    const fontcolor: string = legendOption.visible
      ? legend.textStyle.color || chart.themeStyle.legendLabelFont.color
      : hiddenColor;
    const isCanvas: boolean = this.isStockChartControl
      ? false
      : (this.chart as Chart).enableCanvas;
    textOptions.id =
      this.legendID + this.generateId(legendOption, '_text_', legendIndex);
    textOptions.text =
      legendOption.textCollection.length > 0
    ...
    if (legend.isInversed && !this.isRtlEnable) {
      ...
      textOptions.x =
        this.chart.getModuleName() === 'bulletChart'
          ? legendOption.location.x - legend.shapeWidth
          : legendOption.location.x -
            ((legendOption.textCollection.length > 1
              ? textWidth / legendOption.textCollection.length
              : textWidth) +
              legend.shapeWidth / 2 +
              legend.shapePadding);
    } else {
    ...
    if (isCanvas) {
      ...
      region.rect.y =
        textOptions.y < region.rect.y ? textOptions.y : region.rect.y;
  ...
  private renderPagingElements(
    ...
    const paginggroup: Element = chart.renderer.createGroup({
      id: this.legendID + '_navigation',
    });
    const isCanvas: boolean = this.isStockChartControl
      ? false
      : (chart as Chart).enableCanvas;
    const titleHeight: number = this.isBulletChartControl
      ? 0
      : this.legendTitleSize.height;
    const grayColor: string =
      this.chart.theme.indexOf('Dark') > -1 ||
      this.chart.theme.indexOf('Contrast') > -1
        ? '#FFFFFF'
        : '#545454';
    ...
    const pageUp: string =
      this.legendID + (!this.isRtlEnable ? '_pageup' : '_pagedown');
    const pageDown: string =
      this.legendID + (!this.isRtlEnable ? '_pagedown' : '_pageup');
    const symbolOption: PathOption = new PathOption(
      pageUp,
      'transparent',
      5,
    ...
    const iconSize: number =
      chart.availableSize.width < 110 || chart.availableSize.height < 190
        ? 4
        : this.pageButtonSize;
    ...
    const titleWidth: number =
      this.isTitle && legend.titlePosition === 'Left'
        ? this.legendTitleSize.width
        : 0;
    this.pagingRegions = [];
    ...
    if (!this.isChartControl && !this.isAccChartControl) {
      if (
        ...
        this.totalPages = Math.ceil(
          this.totalPages / Math.max(1, this.rowCount - 1)
        );
      } else {
    ...
    if (
      (this.isChartControl || this.isAccChartControl) &&
      !(!legend.enablePages && this.isPaging)
    ) {
    ...
    if (!isCanvas) {
      ...
      this.pagingClipRect.height =
        this.legendBounds.height -
        this.clipPathHeight -
        (this.pagingClipRect.y - this.legendBounds.y) -
        legend.border.width;
      this.pagingClipRect.y = this.pagingClipRect.y + this.clipPathHeight;
      this.pagingClipRect.x += legend.border.width;
      this.pagingClipRect.width -=
        legend.border.width + legend.border.width / 2;
      (this.chart.renderer as CanvasRenderer).clearRect(
        new Rect(
          this.pagingClipRect.x,
          this.pagingClipRect.y,
          this.pagingClipRect.width,
          this.pagingClipRect.height
        )
      );
    ...
    let y: number =
      bounds.y +
      this.clipPathHeight +
      (titleHeight + bounds.height - this.clipPathHeight) / 2;
    if (
      this.isPaging &&
      !legend.enablePages &&
      !this.isVertical &&
      ...
      y =
        legend.title && this.isTop
          ? bounds.y + padding + titleHeight + iconSize / 1 + 0.5
          : bounds.y + padding + iconSize + 0.5;
    }
    const size: Size = measureText(
      this.totalPages + '/' + this.totalPages,
      legend.textStyle,
      this.chart.themeStyle.legendLabelFont
    );
    const translateX: number = this.isRtlEnable
      ? legend.border.width + iconSize / 2
    ...
    if (!isCanvas) {
      if (
        ...
        symbolOption.opacity = this.isBulletChartControl
          ? symbolOption.opacity
          : legend.enablePages
          ? 1
          : !this.isRtlEnable
          ? this.backwardArrowOpacity
    ...
    this.pagingRegions.push(
      new Rect(
        !this.isRtlEnable
          ? x +
            bounds.width -
            (2 * (iconSize + padding) + padding + size.width) -
            iconSize * 0.5
          : x,
        y - iconSize * 0.5,
        iconSize,
    ...
    textOption.text = !this.isRtlEnable
      ? '1/' + this.totalPages
      : this.totalPages + '/1';
    const color: string =
      this.chart.theme.indexOf('Dark') > -1 ||
      this.chart.theme.indexOf('Contrast') > -1
        ? '#FFFFFF'
        : legend.textStyle.color || this.chart.themeStyle.legendLabelFont.color;
    if (isCanvas && this.totalNoOfPages) {
      textOption.text = !this.isRtlEnable
        ? this.currentPageNumber + '/' + this.totalNoOfPages
        : this.totalNoOfPages + '/' + this.currentPageNumber;
    }
    if (legend.enablePages || this.isBulletChartControl) {
      legend.textStyle.size =
        chart.availableSize.width < 110 ||
        (chart.availableSize.height < 190 && !this.isBulletChartControl)
          ? '8px'
          : legend.textStyle.size;
    ...
    if (this.isPaging && !legend.enablePages && !this.isVertical) {
      x =
        bounds.x +
        bounds.width -
        (this.isBulletChartControl ? this.fivePixel : 0) -
        this.pageButtonSize -
        (legend.title && legend.titlePosition === 'Right'
          ? this.legendTitleSize.width + this.fivePixel
          : 0);
    }
    symbolOption.id = pageDown;
    symbolOption.opacity = !legend.enablePages
      ? !this.isRtlEnable
        ? this.forwardArrowOpacity
        : this.backwardArrowOpacity
      : 1;
    ...
    if (!isCanvas && (legend.enablePages || this.isBulletChartControl)) {
      ...
      if (
        this.currentPageNumber === 1 &&
        this.calTotalPage &&
        (legend.enablePages || this.isBulletChartControl)
      ) {
  ...
  protected translatePage(
    ...
    let size: number =
      this.isChartControl || this.isAccChartControl
        ? page
          ? this.getPageHeight(this.pageHeights, page)
          : 0
        : this.clipPathHeight * page;
    if (!isCanvas && (this.isChartControl || this.isAccChartControl)) {
      this.clipRect.setAttribute(
        'height',
        this.pageHeights[page as number].toString()
      );
      if (
        this.isAccChartControl &&
        this.isPaging &&
        ...
        this.clipRect.setAttribute(
          'height',
          this.legendBounds.height.toString()
        );
      }
    }
    ...
    if (
      ...
      size = !this.isRtlEnable
        ? pageX - this.legendBounds.x
        : this.legendBounds.x + this.maxWidth - pageX;
      size = size < 0 ? 0 : size; // to avoid small pixel variation
      translate =
        (!this.isRtlEnable ? 'translate(-' : 'translate(') + size + ',0)';
    }
  ...
  protected changePage(event: Event, pageUp: boolean): void {
    ...
    const backwardArrow: Element = document.getElementById(
      this.legendID + '_pageup'
    );
    const forwardArrow: Element = document.getElementById(
      this.legendID + '_pagedown'
    );
    const isCanvas: boolean = this.isStockChartControl
      ? false
      : (this.chart as Chart).enableCanvas;
    const pageText: Element =
      legend.enablePages || this.isBulletChartControl
        ? document.getElementById(this.legendID + '_pagenumber')
        : null;
    const page: number =
      legend.enablePages || this.isBulletChartControl
        ? parseInt(pageText.textContent.split('/')[0], 10)
        : this.currentPage;
    if (pageUp && page > 1) {
  ...
  public move(event: Event): void {
    ...
    if ((<HTMLElement>event.target).textContent.indexOf('...') > -1) {
      const targetId: string[] = (<HTMLElement>event.target).id.split(
      ...
      if (targetId.length === 2) {
        ...
        if (!isNaN(index)) {
          ...
          if (this.isChartControl) {
            showTooltip(
              (<Chart>this.chart).series[index as number].name,
              x,
              y,
              element.offsetWidth,
            ...
            showTooltip(
              (<AccumulationChart>this.chart).visibleSeries[0].points[
                index as number
              ].x.toString(),
              x + 10,
              y + 10,
