...
export class AnnotationBase {
  ...
  public render(
    annotation: AccumulationAnnotationSettings | ChartAnnotationSettings,
    index: number
  ): HTMLElement {
  ...
  public setAnnotationPointValue(location: ChartLocation): boolean {
    ...
    if (this.isChart) {
      ...
      for (const axis of chart.axisCollections) {
        if (
          xAxisName === axis.name ||
          (xAxisName == null && axis.name === 'primaryXAxis')
        ) {
          xAxis = axis;
          if (xAxis.isIndexed) {
            xValue = Number(annotation.x);
          } else if (xAxis.valueType.indexOf('Category') > -1) {
            const xAnnotation: string =
              xAxis.valueType === 'DateTimeCategory'
                ? (annotation.x as Date).getTime().toString()
                : <string>annotation.x;
            if (typeof xAnnotation === 'object') {
              for (let i: number = 0; i < xAxis.labels.length; i++) {
                if (
                  xAxis.labels[i as number].toString() ===
            ...
            xValue =
              typeof this.annotation.x === 'object' ||
              typeof new Date(this.annotation.x) === 'object'
                ? Date.parse(
          ...
          yAxisName === axis.name ||
          (yAxisName == null && axis.name === 'primaryYAxis')
        ) {
          yAxis = axis;
        }
      }
      if (
        ...
        withIn(
          xAxis.valueType === 'Logarithmic'
            ? logBase(xValue, xAxis.logBase)
            : xValue,
          xAxis.visibleRange
        ) &&
        withIn(
          yAxis.valueType === 'Logarithmic'
            ? logBase(+annotation.y, yAxis.logBase)
            : +annotation.y,
          yAxis.visibleRange
        )
      ) {
        ...
        location.x =
          symbolLocation.x + (isInverted ? yAxis.rect.x : xAxis.rect.x);
        // for stockchart, stockchart's toolbar height and title size is added for annotation content
        location.y =
          symbolLocation.y +
          (isInverted ? xAxis.rect.y : yAxis.rect.y) +
          (stockChart && stockChart.enablePeriodSelector
            ? stockChart.toolbarHeight + stockChart.titleSize.height
            : 0);
      } else {
        return false;
      }
  ...
  public processAnnotation(
    ...
    if (
      this['setAnnotation' + annotation.coordinateUnits + 'Value'](location)
    ) {
    ...
    updateBlazorTemplate(
      (this.control.element.id + 'Annotation' + index).replace(
        /[^a-zA-Z0-9]/g,
        ''
      ),
      'ContentTemplate',
      chart.stockChart
        ? chart.stockChart.annotations[index as number]
        : this.control.annotations[index as number],
      undefined,
      annotationRendered
    );
  }
  ...
  public setAccumulationPointValue(location: ChartLocation): boolean {
    ...
    for (const accPoint of accumulation.visibleSeries[0].points) {
      if (typeof accPoint.x === 'object') {
        if (
          Date.parse(accPoint.x as string) ===
            Date.parse(this.annotation.x as string) &&
  ...
  public setElementStyle(
    ...
    if (!argsData.cancel) {
      argsData.content.style.left =
        this.setAlignmentValue(
          this.annotation.horizontalAlignment,
          elementRect.width,
          argsData.location.x
        ) + 'px';
      argsData.content.style.top =
        this.setAlignmentValue(
          this.annotation.verticalAlignment,
          elementRect.height,
          argsData.location.y
        ) + 'px';
      argsData.content.setAttribute(
        'aria-label',
        this.annotation.description || 'Annotation'
      );
