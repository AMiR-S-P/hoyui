...
export class BaseTooltip extends ChartData {
  ...
  public getTooltipElement(isTooltip: boolean): HTMLDivElement {
    ...
    this.header =
      this.control.tooltip.header === null
        ? (this.control as Chart | AccumulationChart).tooltip.shared
          ? '${point.x}'
          : '${series.name}'
    ...
    const isStockChart: boolean =
      this.chart.element.id.indexOf('stockChart') > -1;
  ...
  public pushData(
    data: PointData | AccPointData | Point3D,
    isFirst: boolean,
    ...
    if (data.series.enableTooltip) {
      ...
      if (tooltipDiv && !document.getElementById(tooltipDiv.id)) {
        if (!this.chart.stockChart) {
          ...
          document
            .getElementById(
              this.chart.stockChart.element.id + '_Secondary_Element'
            )
            .appendChild(tooltipDiv);
        }
      }
  ...
  public highlightPoint(
    series: Series | AccumulationSeries | Chart3DSeries,
    pointIndex: number,
    highlight: boolean
  ): void {
    const element: HTMLElement = this.getElement(
      this.element.id + '_Series_' + series.index + '_Point_' + pointIndex
    );
    const selectionModule: AccumulationSelection = (
      this.control as AccumulationChart
    ).accumulationSelectionModule;
    const isSelectedElement: boolean =
      selectionModule && selectionModule.selectedDataIndexes.length > 0
        ? true
        : false;
    if (element) {
      if (
        ...
        (isSelectedElement &&
          ...
          element
            .getAttribute('class')
            .indexOf('_ej2_chart_selection_series_') === -1)
      ) {
        if (
          ...
          element.setAttribute(
            'fill',
            highlight && this.chart.highlightColor !== 'transparent'
              ? this.chart.highlightColor
              : series.pointColorMapping !== ''
              ? (series as Series).points[0].color
              : (series as Series).points[pointIndex as number].color ||
                (series as Series).interior
          );
        } else {
          element.setAttribute(
            'opacity',
  ...
  public highlightPoints(): void {
    for (const item of this.currentPoints) {
      if (item.series.isRectSeries && item.series.category === 'Series') {
        this.highlightPoint(
          item.series as Series | AccumulationSeries,
          item.point.index,
          true
        );
      }
  ...
  public createTooltip(
    chart: Chart | AccumulationChart | Chart3D,
    isFirst: boolean,
    location: ChartLocation,
    clipLocation: ChartLocation,
    point: Points | AccPoints | Chart3DPoint,
    shapes: ChartShape[],
    offset: number,
    bounds: Rect,
    crosshairEnabled: boolean = false,
    extraPoints: PointData[] = null,
    templatePoint:
      | Points
      | Points[]
      | AccPoints
    ...
    const tooltipModule: AccumulationTooltip | Tooltip | Tooltip3D =
      (<Chart>chart).tooltipModule ||
      (<Chart3D>chart).tooltip3DModule ||
    ...
    if (isFirst) {
      this.svgTooltip = new SVGTooltip({
        ...
        arrowPadding:
          ...
          this.chart.tooltip.location.x !== null ||
          this.chart.tooltip.location.y !== null
            ? 0
            : 7,
        ...
        isFixed:
          ...
          this.chart.tooltip.location.y !== null,
        isTextWrap:
          chart.tooltip.enableTextWrap && chart.getModuleName() === 'chart',
        blazorTemplate: { name: 'Template', parent: this.chart.tooltip },
        controlInstance: this.chart,
        enableRTL: chart.enableRtl,
        controlName: 'Chart',
        allowHighlight:
          chart.getModuleName() === 'chart' && !series.marker.allowHighlight,
        tooltipRender: () => {
      ...
      if (this.svgTooltip) {
        ...
        this.svgTooltip.isNegative =
          series.isRectSeries &&
          series.type !== 'Waterfall' &&
          point &&
          point.y < 0;
        this.svgTooltip.clipBounds =
          this.chart.chartAreaType === 'PolarRadar'
            ? new ChartLocation(0, 0)
            : clipLocation;
        this.svgTooltip.arrowPadding =
          this.text.length > 1 ||
          this.chart.stockChart ||
          this.chart.tooltip.location.x !== null ||
          this.chart.tooltip.location.y !== null
            ? 0
            : 7;
        this.svgTooltip.allowHighlight =
          chart.getModuleName() === 'chart' && !series.marker.allowHighlight;
        this.svgTooltip.dataBind();
      }
    }
    if ((this.chart as any).isReact) {
  ...
  private findColor(
    ...
    if (
      ...
      return (
        (data.point.color && data.point.color !== '#ffffff'
          ? data.point.color
  ...
  public updatePreviousPoint(extraPoints: PointData[]): void {
    if (extraPoints) {
      this.currentPoints = (<PointData[]>this.currentPoints).concat(
        extraPoints
      );
    }
    this.previousPoints = <PointData[]>(
      extend([], this.currentPoints, null, true)
    );
  }

  public fadeOut(data: PointData[]): void {
    const svgElement: HTMLElement = this.chart.enableCanvas
      ? this.getElement(this.element.id + '_tooltip_group')
