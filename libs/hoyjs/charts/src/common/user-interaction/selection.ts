...
export class BaseSelection {
  ...
  protected seriesStyles(): void {
    ...
    let fill: string;
    let opacity: number;
    const selectionPattern: SelectionPattern = (<Chart>this.control)
      .selectionPattern;
    const highlightPattern: SelectionPattern = (<Chart>this.control)
      .highlightPattern;
    if (
      (this.styleId.indexOf('highlight') > 0 &&
        (<Chart>this.control).highlightColor !== '') ||
      isNullOrUndefined(style) ||
      ...
      for (const series of this.control.visibleSeries) {
        const visibleSeries: Series | AccumulationSeries =
          (this.control.visibleSeries[series.index] as Series) ||
          (this.control.visibleSeries[series.index] as AccumulationSeries);
        if (
          (this.styleId.indexOf('highlight') > 0 &&
            (<Chart>this.control).highlightColor !== '') ||
          ((!isNullOrUndefined(selectionPattern) ||
            !isNullOrUndefined(highlightPattern)) &&
            (selectionPattern !== 'None' || highlightPattern !== 'None'))
        ) {
          const patternName: SelectionPattern =
            this.styleId.indexOf('highlight') > 0
              ? highlightPattern
              : selectionPattern;
          if (
            ((visibleSeries.type as AccumulationType) === 'Pie' ||
              (visibleSeries.type as AccumulationType) === 'Funnel' ||
              (visibleSeries.type as AccumulationType) === 'Pyramid' ||
              this.control.getModuleName() === 'circularchart3d') &&
            ...
            for (let i: number = 0; i < visibleSeries.points.length; i++) {
              ...
              fill = this.pattern(
                this.control,
                this.styleId.indexOf('highlight') > 0 &&
                  (this.control as AccumulationChart).highlightColor !== ''
                  ? (this.control as AccumulationChart).highlightColor
                  : visibleSeries.points[i as number].color,
                series.points[i as number].index,
                patternName,
                opacity
              );
              pattern = '{ fill:' + fill + '}';
              seriesclass =
                (series as Series | AccumulationSeries).selectionStyle ||
                this.styleId +
                  '_series_' +
                  series.index +
                  '_point_' +
                  ...
                  '> *';
              if (
                (this.control as AccumulationChart).highlightMode === 'None' &&
                (this.control as AccumulationChart).legendSettings
                  .enableHighlight &&
                !series.isRectSeries
              ) {
                style.innerText +=
              ...
              pattern =
                pattern.indexOf('None') > -1
                  ? '{fill:' +
                    ...
                    '!important}'
                  : pattern;
              style.innerText += (series as Series | AccumulationSeries)
                .selectionStyle
                ? ''
                : '.' + seriesclass + pattern;
            }
          } else if (
            (visibleSeries.type as ChartSeriesType) &&
            (this.control as Chart).highlightColor !== 'transparent'
          ) {
            opacity = visibleSeries.opacity;
            fill = this.pattern(
              this.control,
              this.styleId.indexOf('highlight') > 0 &&
                (this.control as Chart).highlightColor !== ''
                ? (this.control as Chart).highlightColor
                : visibleSeries.pointColorMapping !== '' ||
                  ((this.control as Chart).rangeColorSettings &&
                    (this.control as Chart).rangeColorSettings.length > 1)
                ? (visibleSeries as Series).points[0].color
                : (visibleSeries as Series).interior,
              series.index,
              patternName,
              opacity
            );
        ...
        seriesclass =
          ...
          this.styleId +
            ...
            '> *';
        if (
          (this.control as Chart).highlightMode === 'None' &&
          (this.control as Chart).legendSettings.enableHighlight &&
          !series.isRectSeries
        ) {
          style.innerText +=
            '.' +
            ...
            ' { stroke-width:' +
            (parseFloat(
              (series as Series | AccumulationSeries).width
                ? (series as Series | AccumulationSeries).width.toString()
                : '0'
            ) +
              1) +
            ';} ';
      ...
      let unSelectOpacity: number =
        this.control.highlightColor !== 'transparent'
          ...
          : opacity;
      if (
        isNullOrUndefined((this.control as Chart).selectionModule) &&
        (this.control as Chart).selectionMode === 'None' &&
        (this.control as Chart).highlightColor !== ''
      ) {
        unSelectOpacity = 1;
      }
      style.innerText +=
        '.' + this.unselected + ' { opacity:' + unSelectOpacity + ';} ';
  ...
  public pattern(
    ...
    switch (patternName) {
      case 'Dots':
        patternGroup[heightStr as string] = patternGroup[widthStr as string] =
          patternNum;
    ...
    const svgRenderer: SvgRenderer = ((chart as Chart).svgRenderer ||
      (chart as AccumulationChart).renderer) as SvgRenderer;
  ...
  private loadPattern(
    ...
    for (i = 0; i < options.length; i++) {
      const path: Element = svgRenderer.createPattern(
        options[i as number],
        (<object>options)[i as number].name
      );
  ...
  protected concatIndexes(
    userIndexes: IndexesModel[],
  ...
  protected checkVisibility(
    ...
    for (const index of uniqueSeries) {
      if (
        chart != null &&
        (chart as Chart).rangeColorSettings &&
        (chart as Chart).rangeColorSettings.length > 0 &&
