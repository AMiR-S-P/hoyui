import { createElement, isNullOrUndefined, Browser, remove } from '@hoyui/base';
import { Toolbar, ItemModel, ClickEventArgs } from '@hoyui/navigations';
import { DateRangePicker, RangeEventArgs } from '@hoyui/calendars';
...
export class PeriodSelector {
  ...
  constructor(control: RangeNavigator | StockChart) {
    ...
    if (this.rootControl.getModuleName() === 'stockChart') {
      ...
      this.isDatetimeCategory = (
        this.rootControl as StockChart
      ).isDateTimeCategory;
    }
  }
  /**
  ...
  public setControlValues(control: RangeNavigator | StockChart): void {
    if (control.getModuleName() === 'rangeNavigator') {
      this.control.periods = (
        this.rootControl as RangeNavigator
      ...
      this.control.endValue = this.control.seriesXMax = (
        control as StockChart
      ).seriesXMax;
      this.control.startValue = this.control.seriesXMin = (
        control as StockChart
      ).seriesXMin;
      this.control.rangeNavigatorControl = (
        this.rootControl as StockChart
      ).rangeNavigator;
      if (this.control.rangeNavigatorControl) {
        this.control.rangeSlider = (
          this.rootControl as StockChart
        ).rangeNavigator.rangeSlider;
      }
    }
    this.control.element = control.element;
    this.control.disableRangeSelector = (
      control as RangeNavigator
  ...
  public renderSelectorElement(
    control?: RangeNavigator,
    options?: ISelectorRenderArgs,
    x?: number
  ): void {
    //render border
    this.periodSelectorSize = control
      ? this.periodSelectorSize
      : new Rect(
          x,
          (this.rootControl as StockChart).titleSize.height
            ? (this.rootControl as StockChart).titleSize.height
            : 10,
          options.width,
          options.height
    ...
    this.periodSelectorDiv = createElement('div', {
      ...
      styles:
        'width: ' +
        (this.periodSelectorSize.width - thumbSize) +
        'px;height: ' +
        this.periodSelectorSize.height +
        'px;top:' +
        this.periodSelectorSize.y +
        'px;left:' +
        (this.periodSelectorSize.x + thumbSize / 2) +
  ...
  public renderSelector(): void {
    ...
    const selectorElement: Element = createElement('div', {
    ...
    if (this.rootControl.getModuleName() === 'stockChart') {
      if ((<StockChart>this.rootControl).exportType.length) {
        const exportElement: HTMLElement = createElement('button', {
          id: controlId + '_export',
          styles: buttonStyles,
          className: 'e-dropdown-btn e-btn e-flat',
        });
        exportElement.innerText = 'Export';
        selector.push({ template: exportElement, align: 'Right' });
      }
    }
    ...
    this.toolbar = new Toolbar({
      ...
      created: () => {
        this.nodes =
          this.toolbar.element.querySelectorAll('.e-toolbar-left')[0];
          ...
          buttons.map((period: PeriodsModel, index: number) => {
            if (period.selected && this.selectedPeriod !== null) {
              ...
              this.control.startValue = this.changedRange(
                period.intervalType,
                this.control.endValue,
                period.interval
              ).getTime();
              if (this.isDatetimeCategory) {
                this.control.startValue =
                  this.rootControl.getModuleName() !== 'stockChart'
                    ? this.findStartValue(
                        this.control.startValue,
                        this.control.endValue
                      )
                    : this.rootControl.startValue;
              }
              this.control.startValue =
                period.text && period.text.toLowerCase() === 'all'
                  ? this.control.seriesXMin
                  : this.control.startValue;
              this.control.endValue =
                period.text && period.text.toLowerCase() === 'all'
                  ? this.control.seriesXMax
                  : this.control.endValue;
              this.selectedIndex =
                this.nodes.childNodes.length - buttons.length + index;
        ...
        if (
          ...
          const selectedIndex: number = this.findSelectedIndex(
            this.control.startValue,
            this.control.endValue,
            buttons
          );
    ...
    if (enableCustom) {
      this.datePicker = new DateRangePicker({
        min: this.isDatetimeCategory
          ? new Date(this.sortedData[this.control.seriesXMin])
          : new Date(this.control.seriesXMin),
        max: this.isDatetimeCategory
          ? new Date(this.sortedData[this.control.seriesXMax])
          : new Date(this.control.seriesXMax),
        format: 'dd/MM/yyyy',
        placeholder: 'Select a range',
        showClearButton: false,
        startDate: this.isDatetimeCategory
          ? new Date(this.sortedData[Math.floor(this.control.startValue)])
          : new Date(this.control.startValue),
        endDate: this.isDatetimeCategory
          ? new Date(this.sortedData[Math.floor(this.control.endValue)])
          : new Date(this.control.endValue),
        created: () => {
          if (selctorArgs.enableCustomFormat) {
            const datePicker: HTMLCollection = <HTMLCollection>(
              document.getElementsByClassName('e-date-range-wrapper')
            );
            let datePickerElement: HTMLElement;
            for (let i: number = 0; i < datePicker.length; i++) {
              if (
                datePicker[i as number].children[0].id.indexOf(controlId) !== -1
              ) {
            ...
            getElement(dateRangeId).insertAdjacentElement(
              'afterbegin',
              createElement('span', {
                id: controlId + 'dateIcon',
                className: 'e-input-group-icon e-range-icon e-btn-icon e-icons',
                styles:
                  'font-size: 16px; min-height: 0px; margin: -3px 0 0 0; outline: none; min-width: 30px',
        ...
        change: (args: RangeEventArgs) => {
          if (this.triggerChange) {
            ...
            if (this.control.rangeSlider && args.event) {
              if (this.rootControl.getModuleName() !== 'stockChart') {
                this.control.rangeNavigatorControl.startValue = this
                  .isDatetimeCategory
                  ? this.startValue
                  : args.startDate.getTime();
                this.control.rangeNavigatorControl.endValue = this
                  .isDatetimeCategory
                  ? this.endValue
              ...
              this.control.rangeSlider.performAnimation(
                this.isDatetimeCategory
                  ? this.startValue
                  : (args.startDate as Date).getTime(),
                this.isDatetimeCategory
                  ? this.endValue
              ...
              (this.rootControl as StockChart).rangeChanged(
                this.isDatetimeCategory
                  ? this.startValue
                  : (args.startDate as Date).getTime(),
                this.isDatetimeCategory
            ...
            this.nodes =
              this.toolbar.element.querySelectorAll('.e-toolbar-left')[0];
            if (
              !(this.rootControl as StockChart).resizeTo &&
              this.control.rangeSlider &&
              this.control.rangeSlider.isDrag
            ) {
              /**
              ...
              for (
                let i: number = 0,
                  length: number = this.nodes.childNodes.length;
                i < length;
                i++
              ) {
                (
                  this.nodes.childNodes[i as number].childNodes[0] as Element
  ...
  public findSelectedIndex(
    startDate: number,
    endDate: number,
    buttons: PeriodsModel[]
  ): number {
    ...
    for (let i: number = 0; i < buttons.length; i++) {
      ...
      if (
        period.intervalType === 'Years' &&
        daysDiffence / 365 === period.interval
      ) {
        selectedIndex = i;
      } else if (
        period.intervalType === 'Months' &&
        (daysDiffence / 30 === period.interval ||
          daysDiffence / 31 === period.interval)
      ) {
        selectedIndex = i;
      } else if (
        period.intervalType === 'Days' &&
        daysDiffence === period.interval
      ) {
        selectedIndex = i;
      } else if (
        period.intervalType === 'Weeks' &&
        daysDiffence / 7 === period.interval
      ) {
        selectedIndex = i;
      } else if (
        period.intervalType === 'Hours' &&
        daysDiffence * 24 === period.interval
      ) {
        selectedIndex = i;
      } else if (
        period.intervalType === 'Seconds' &&
        daysDiffence * 24 * 3600 === period.interval
      ) {
        selectedIndex = i;
      }
    }
  ...
  private updateCustomElement(): ItemModel[] {
    ...
    if (this.rootControl.getModuleName() === 'stockChart') {
      if ((<StockChart>this.rootControl).seriesType.length) {
        const SeriesElement: HTMLElement = createElement('button', {
          id: controlId + '_seriesType',
          styles: buttonStyles,
          className: className,
        });
        SeriesElement.innerText = 'Series';
        selector.push({ template: SeriesElement, align: 'Left' });
      }
      if ((<StockChart>this.rootControl).indicatorType.length) {
        ...
        selector.push({ template: indicatorElement, align: 'Left' });
      }
      if ((<StockChart>this.rootControl).trendlineType.length) {
        ...
        selector.push({ template: trendlineElement, align: 'Left' });
      }
    }
    return selector;
  }
  ...
  public setSelectedStyle(selectedIndex: number): void {
    for (
      let i: number = 0, length: number = this.nodes.childNodes.length;
      i < length;
      i++
    ) {
      (
    ...
    if (!isNullOrUndefined(selectedIndex)) {
      (
        this.nodes.childNodes[selectedIndex as number].childNodes[0] as Element
      ).classList.add('e-flat');
      (
        this.nodes.childNodes[selectedIndex as number].childNodes[0] as Element
  ...
  private buttonClick(
    ...
    const slider: RangeSlider = this.control.rangeSlider;
    const buttons: PeriodsModel[] = this.control.periods;
    const button: PeriodsModel = <PeriodsModel>(
      buttons.filter((btn: PeriodsModel) => btn.text === clickedEle.text)
    );
    ...
    if (clickedEle.text.toLowerCase() === 'all') {
      ...
      if (slider) {
        updatedStart = this.isDatetimeCategory
          ? new Date(
              new Date(this.sortedData[Math.floor(slider.currentEnd)])
                .getFullYear()
                .toString()
            ).getTime()
          : new Date(
              new Date(slider.currentEnd).getFullYear().toString()
            ).getTime();
        updatedStart = this.isDatetimeCategory
          ? this.findStartValue(updatedStart, slider.currentEnd)
          : updatedStart;
        ...
        updatedStart = this.isDatetimeCategory
          ? new Date(
              new Date(
                this.sortedData[
                  Math.floor((this.rootControl as StockChart).currentEnd)
                ]
              )
                .getFullYear()
                .toString()
          ...
          : new Date(
              new Date((this.rootControl as StockChart).currentEnd)
                .getFullYear()
                .toString()
            ).getTime();
        updatedStart = this.isDatetimeCategory
          ? this.findStartValue(
              updatedStart,
              (this.rootControl as StockChart).currentEnd
            )
          : updatedStart;
      ...
      if (slider) {
        updatedStart = this.changedRange(
          button[0].intervalType,
          slider.currentEnd,
          button[0].interval
        ).getTime();
        updatedStart = this.isDatetimeCategory
    ...
    if (getElement(this.calendarId + '_popup') && !Browser.isDevice) {
      const element: HTMLElement = getElement(
        this.calendarId + '_popup'
      ) as HTMLElement;
      (
        element.querySelectorAll('.e-range-header')[0] as HTMLElement
      ).style.display = 'none';
    }
  }
  /**
  ...
  public changedRange(
    ...
    const result: Date = this.isDatetimeCategory
      ? new Date(this.sortedData[Math.floor(end)])
      : new Date(end);
    switch (type) {
      case 'Quarter':
