 ...
 */
export function getRangeThemeColor(
  theme: ChartTheme,
  range: RangeNavigator
): IRangeStyle {
  const thumbSize: ThumbSettingsModel = range.navigatorStyleSettings.thumb;
  const thumbWidth: number = isNullOrUndefined(thumbSize.width)
    ? Browser.isDevice
      ? 15
      : 20
    : thumbSize.width;
  const thumbHeight: number = isNullOrUndefined(thumbSize.height)
    ? Browser.isDevice
      ? 15
      : 20
    : thumbSize.height;
  const darkAxisColor: string =
    theme === 'HighContrast' ? '#969696' : '#6F6C6C';
  const darkGridlineColor: string =
    theme === 'HighContrast' ? '#4A4848' : '#414040';
  let style: IRangeStyle = {
    gridLineColor: '#E0E0E0',
    axisLineColor: '#000000',
    labelFontColor: '#686868',
    unselectedRectColor: range.series.length
  ...
  switch (theme) {
    case 'Fabric':
      style.selectedRegionColor = range.series.length
        ? 'transparent'
        : '#007897';
      break;
    case 'Bootstrap':
    ...
    case 'HighContrast':
      style = {
        ...
        background: 'transparent',
        thumbHoverColor: '#BFBFBF',
        selectedRegionColor: range.series.length
          ? 'rgba(22, 22, 22, 0.6)'
          : '#FFD939',
        tooltipBackground: '#FFFFFF',
