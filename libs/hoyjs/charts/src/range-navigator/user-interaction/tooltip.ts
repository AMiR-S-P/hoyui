...
} from '../../range-navigator/index';
import { Rect, measureText } from '@hoyui/svg-base';
...
export class RangeTooltip {
  ...
  public renderLeftTooltip(rangeSlider: RangeSlider): void {
    ...
    let rect: Rect = this.control.enableRtl
      ? rangeSlider.rightRect
      : rangeSlider.leftRect;
    if (contentWidth > rect.width) {
      rect = rangeSlider.midRect;
    }
    this.leftTooltip = this.renderTooltip(
      rect,
      this.createElement('_leftTooltip'),
      rangeSlider.startX,
      content
    );
  }
  ...
  private getContentSize(value: string[]): number {
    ...
    if (this.control.tooltip.template) {
      ...
      width =
        measureText(value[0], font, this.control.themeStyle.tooltipLabelFont)
          .width + 20;
    }
    return width;
  }
  ...
  public renderRightTooltip(rangeSlider: RangeSlider): void {
    ...
    this.rightTooltip = this.renderTooltip(
      rect,
      this.createElement('_rightTooltip'),
      rangeSlider.endX,
      content
    );
  }
  ...
  private createElement(id: string): Element {
    if (getElement(this.elementId + id)) {
      ...
      if (!this.control.stockChart) {
        ...
        getElement(stockChart.element.id + '_Secondary_Element').appendChild(
          element
        );
  ...
  private renderTooltip(
    ...
    const left: number =
      control.svgObject.getBoundingClientRect().left -
      ...
      return new SVGTooltip(
        {
          ...
          fill: tooltip.fill
            ? tooltip.fill
            : this.control.themeStyle.tooltipBackground,
  ...
  private getTooltipContent(value: number): string[] {
    ...
    value =
      valueType === 'DateTimeCategory'
        ? parseInt(xAxis.labels[Math.floor(value)], 10)
        : value;
    if (valueType.indexOf('DateTime') > -1) {
      text = control.intl.getDateFormat({
        ...
        skeleton:
          valueType === 'DateTime'
            ? control.dateTimeModule.getSkeleton(xAxis, null, null)
            : control.dateTimeCategoryModule.getSkeleton(xAxis, null, null),
      })(new Date(value));
    } else {
