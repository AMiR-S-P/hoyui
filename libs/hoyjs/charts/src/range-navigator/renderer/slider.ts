...
import { animationMode, Browser, createElement } from '@hoyui/base';
...
import { PathOption, Rect, SvgRenderer } from '@hoyui/svg-base';
import { Animation, AnimationOptions } from '@hoyui/base';
import {
  IChangedEventArgs,
  IRangeStyle,
} from '../model/range-navigator-interface';
import {
  ThumbSettingsModel,
  StyleSettingsModel,
} from '../model/range-base-model';
...
export class RangeSlider {
  ...
  constructor(range: RangeNavigator) {
    ...
    this.thumbVisible =
      range.themeStyle.thumbWidth !== 0 && range.themeStyle.thumbHeight !== 0;
    this.elementId = range.element.id;
    this.thumpPadding = range.themeStyle.thumbWidth / 2;
    this.addEventListener();
    this.thumbColor = range.disableRangeSelector
      ? 'transparent'
  ...
  public render(range: RangeNavigator): void {
    ...
    const disabledColor: string = range.disableRangeSelector
      ? 'transparent'
      : null;
    ...
    const option: RectOption = new RectOption(
      ...
      disabledColor ||
        style.unselectedRegionColor ||
        range.themeStyle.unselectedRectColor,
      { width: 0 },
      1,
      {
        x: range.bounds.x,
    ...
    option.fill =
      disabledColor ||
      style.selectedRegionColor ||
      range.themeStyle.selectedRegionColor;
    ...
    this.selectedElement.setAttribute(
      'aria-label',
      'Range Slider with ' + range.rangeSlider.points.length + ' data points'
    );
  ...
  public createThump(
    ...
    const lineColor: string =
      disabledColor || thump.border.color || style.thumpLineColor;
    ...
    parent.appendChild(
      render.drawPath(
        new PathOption(
          ...
          'transparent',
          thump.border.width,
          control.series.length ? lineColor : 'transparent',
          1,
          thump.border.dashArray,
          'M' +
            ' ' +
            x +
            ' ' +
    ...
    if (sliderGroup && !control.disableRangeSelector) {
      ...
      (<HTMLElement>shadowElement).innerText =
        '<rect xmlns="http://www.w3.org/2000/svg" id="' +
        ...
        this.thumpY +
        '" width="' +
        control.themeStyle.thumbWidth +
        '" height="' +
        control.themeStyle.thumbHeight +
        '"' +
        ' rx="' +
        (thump.type === 'Circle' ? '50%' : '0%') +
        '"/>' +
      ...
      parent.appendChild(
        render.drawRectangle(
          new RectOption(
            id + '_ThumpSymbol',
            disabledColor || this.thumbColor,
            { width: thump.border.width, color: lineColor },
            1,
            {
      ...
      parent.appendChild(
        render.drawPath(
          new PathOption(
            ...
            'transparent',
            1,
            disabledColor || control.themeStyle.gripColor,
            1,
            null,
            'M' +
              ' ' +
              ...
              (y - tickLength) +
              ' ' +
              (this.control.theme.indexOf('Fluent2') > -1 ||
              this.control.theme.indexOf('Bootstrap5') > -1
                ? ''
                : 'M' +
                  ' ' +
                  x +
  ...
  public setSlider(
    ...
    this.startX =
      padding +
      getXLocation(start, axisRange, range.bounds.width, range.enableRtl);
    this.endX =
      padding +
      getXLocation(end, axisRange, range.bounds.width, range.enableRtl);
    ...
    this.rightUnSelectedElement.setAttribute(
      'width',
      range.bounds.width - (rightPadding - padding) + ''
    );
    this.leftSlider.setAttribute(
      'transform',
      'translate(' + (this.startX - this.thumpPadding) + ', 0)'
    );
    this.rightSlider.setAttribute(
      'transform',
      'translate(' + (this.endX - this.thumpPadding) + ', 0)'
    );
    const left: number =
      this.control.svgObject.getBoundingClientRect().left -
    ...
    const periodSelectorModule: PeriodSelector =
      this.control.periodSelectorModule;
    if (
      periodSelectorModule &&
      this.control.redraw &&
      this.control.getModuleName() === 'rangeNavigator'
    ) {
      const selectedIndex: number = periodSelectorModule.findSelectedIndex(
        start,
        end,
        periodSelectorModule.control.periods
      );
  ...
  public triggerEvent(range: VisibleRangeModel): void {
    ...
    const argsData: IChangedEventArgs = {
      ...
      start:
        valueType === 'DateTime'
          ? new Date(this.currentStart)
          : valueType === 'DateTimeCategory'
          ? this.currentStart
          : valueType === 'Logarithmic'
          ? Math.pow(xAxis.logBase, this.currentStart)
          : this.currentStart,
      end:
        valueType === 'DateTime'
          ? new Date(this.currentEnd)
          : valueType === 'DateTimeCategory'
          ? this.currentEnd
          : valueType === 'Logarithmic'
          ? Math.pow(xAxis.logBase, this.currentEnd)
          : this.currentEnd,
      name: 'changed',
      selectedData: getExactData(
        this.points,
        this.currentStart,
        this.currentEnd
      ),
      zoomPosition:
        (this.control.enableRtl
          ? range.max - this.currentEnd
  ...
  public mouseMoveHandler(e: PointerEvent | TouchEvent): void {
    ...
    if (this.isDrag && control.mouseX >= bounds.x) {
      switch (this.currentSlider) {
        case 'Left':
          control.startValue = this.getRangeValue(
            Math.abs(control.mouseX - bounds.x)
          );
          break;
        case 'Right':
          control.endValue = this.getRangeValue(
            Math.abs(control.mouseX - bounds.x)
          );
          break;
        case 'Middle': {
          start = Math.max(
            this.getRangeValue(
              Math.abs(
                this.startX - (this.previousMoveX - control.mouseX) - bounds.x
              )
            ),
            axisRange.min
          );
            ...
            this.getRangeValue(
              Math.abs(
                this.endX - (this.previousMoveX - control.mouseX) - bounds.x
              )
            ),
            axisRange.max
          );
          const currentWidth: number = Math.floor(
            Math.abs(
              getXLocation(
                end,
                axisRange,
  ...
  private mouseDownHandler(e: PointerEvent): void {
    ...
    this.isDrag = !(
      this.currentSlider === 'UnSelectedArea' || !this.currentSlider
    );
  ...
  private getCurrentSlider(id: string): string {
    ...
    if (id.indexOf(this.elementId + '_LeftSlider') > -1) {
      (this.leftSlider.childNodes[2] as Element).setAttribute(
        'fill',
        hoverColor
      );
      return 'Left';
    } else if (id.indexOf(this.elementId + '_RightSlider') > -1) {
      (this.rightSlider.childNodes[2] as Element).setAttribute(
        'fill',
        hoverColor
      );
      return 'Right';
      ...
      id.indexOf(this.elementId + '_AxisLabel_') > -1 &&
      (this.control.valueType === 'DateTime' ||
        this.control.valueType === 'DateTimeCategory')
    ) {
      this.labelIndex = +id.substring(id.lastIndexOf('_') + 1, id.length);
      return 'firstLevelLabels';
    } else if (
      id.indexOf(this.elementId + '_SecondaryLabel') > -1 &&
      (this.control.valueType === 'DateTime' ||
        this.control.valueType === 'DateTimeCategory')
    ) {
      this.labelIndex = +id.substring(id.lastIndexOf('_') + 1, id.length);
  ...
  private mouseUpHandler(): void {
    ...
    if (this.currentSlider === 'UnSelectedArea') {
      ...
      if (control.mouseDownX < this.startX) {
        value = Math.max(
          this.getRangeValue(
            control.mouseDownX - this.sliderWidth / 2 - control.bounds.x
          ),
          range.min
        );
      ...
      this.currentSlider === 'firstLevelLabels' ||
      this.currentSlider === 'secondLevelLabels'
    ) {
      const secondLabel: VisibleLabels =
        control.rangeAxis[this.currentSlider][this.labelIndex + 1];
    ...
    if (this.currentSlider !== null) {
      if (this.control.periodSelectorSettings.periods.length > 0) {
        ...
        this.control.periodSelectorModule.datePicker.startDate = this.control
          .periodSelectorModule.isDatetimeCategory
          ? new Date(
              this.control.periodSelectorModule.sortedData[
                Math.floor(this.currentStart)
              ]
            )
          : new Date(this.currentStart);
        this.control.periodSelectorModule.datePicker.endDate = this.control
          .periodSelectorModule.isDatetimeCategory
          ? new Date(
              this.control.periodSelectorModule.sortedData[
                Math.floor(this.currentEnd)
              ]
            )
          : new Date(this.currentEnd);
      }
  ...
  public setAllowSnapping(
    ...
    this.setSlider(
      getNearestValue(values, start),
      getNearestValue(values, end),
      trigger,
      tooltip
    );
    control.startValue = this.currentStart;
  ...
  public performAnimation(
    ...
    new Animation({}).animate(createElement('div'), {
      duration:
        this.control.animationDuration === 0 && animationMode === 'Enable'
          ? 1000
          : this.control.animationDuration,
      progress: (args: AnimationOptions): void => {
        this.setSlider(
          linear(args.timeStamp, 0, start - currentStart, args.duration) +
            currentStart,
          linear(args.timeStamp, 0, end - currentEnd, args.duration) +
            currentEnd,
          !isDeffered,
          enableTooltip
        );
