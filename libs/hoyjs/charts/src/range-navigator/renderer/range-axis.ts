...
export class RangeNavigatorAxis extends DateTime {
  ...
  public renderGridLines(): void {
    ...
    const disabledColor: string = control.disableRangeSelector
      ? 'transparent'
      : null;
    ...
    const tick: number =
      control.tickPosition === 'Outside' || control.series.length === 0
        ? rect.y + rect.height + majorTickLines.height
        : rect.y + rect.height - majorTickLines.height;
    //Gridlines
    ...
    if (control.valueType.indexOf('DateTime') > -1) {
      const interval: number = this.calculateDateTimeNiceInterval(
        chartAxis,
        rect,
        chartAxis.doubleRange.start,
        chartAxis.doubleRange.end,
        chartAxis.isChart
      );
      ...
      this.actualIntervalType =
        chartAxis.actualIntervalType as RangeIntervalType;
      if (
        control.valueType === 'DateTimeCategory' &&
        (this.actualIntervalType === 'Quarter' ||
          this.actualIntervalType === 'Weeks')
      ) {
    ...
    for (let i: number = 0; i < labelLength; i++) {
      ...
      pointX =
        valueToCoefficient(
          this.firstLevelLabels[i as number].value,
          chartAxis
        ) *
          rect.width +
        rect.x;
      if (pointX >= rect.x && rect.x + rect.width >= pointX) {
        majorGrid = majorGrid.concat(
          'M ' +
            pointX +
            ' ' +
    ...
    let options: PathOption = new PathOption(
      control.element.id + '_MajorGridLine',
      'transparent',
      majorGridLines.width,
      control.series.length
        ? disabledColor ||
          majorGridLines.color ||
          control.themeStyle.gridLineColor
        : 'transparent',
      1,
      majorGridLines.dashArray,
      majorGrid
    );
    this.gridLines.appendChild(
      control.renderer.drawPath(options) as HTMLElement
    );
    options = new PathOption(
      control.element.id + '_MajorTickLine',
      'transparent',
  ...
  public renderAxisLabels(): void {
    ...
    const control: RangeNavigator = this.rangeNavigator;
    let pointY: number;
    const labelElement: Element = control.renderer.createGroup({
      id: control.element.id + '_AxisLabels',
    });
    const firstLevelElement: Element = control.renderer.createGroup({
      id: control.element.id + '_FirstLevelAxisLabels',
    });
    const secondLevelElement: Element = control.renderer.createGroup({
      id: control.element.id + '_SecondLevelAxisLabels',
    });
    const secondaryAxis: Axis = axis;
    pointY = this.findLabelY(control, false);
    this.placeAxisLabels(
      axis,
      pointY,
      '_AxisLabel_',
      control,
    ...
    secondaryAxis.intervalType = secondaryAxis.actualIntervalType =
      (control.groupBy ||
        this.getSecondaryLabelType(axis.actualIntervalType)) as IntervalType;
    secondaryAxis.labelFormat = '';
    if (
      control.enableGrouping &&
      control.valueType.indexOf('DateTime') > -1 &&
      this.actualIntervalType !== 'Years'
    ) {
      ...
      const interval: number = this.calculateDateTimeNiceInterval(
        secondaryAxis,
        control.bounds,
        secondaryAxis.doubleRange.start,
        secondaryAxis.doubleRange.end,
        secondaryAxis.isChart
      );
      if (control.valueType === 'DateTime') {
      ...
      const border: string = this.placeAxisLabels(
        secondaryAxis,
        pointY,
        '_SecondaryLabel_',
        control,
      ...
      const path: PathOption = new PathOption(
        control.element.id + '_SecondaryMajorLines',
        'transparent',
        control.majorTickLines.width,
        control.majorTickLines.color || control.themeStyle.gridLineColor,
        1,
        control.majorGridLines.dashArray,
        border
      );
      this.gridLines.appendChild(
        control.renderer.drawPath(path) as HTMLElement
      );
    }
    control.chartSeries.xAxis.visibleLabels =
      control.chartSeries.xAxis.visibleLabels.concat(
        secondaryAxis.visibleLabels
      );
    (labelElement as HTMLElement).style.cursor =
      axis.valueType.indexOf('DateTime') > -1
        ? 'cursor: pointer'
        : 'cursor: default';
  ...
  private findSecondaryAxisLabels(axis: Axis): void {
    ...
    for (
      let i: number = Math.ceil(axis.visibleRange.min);
      i <= axis.visibleRange.max;
      i += axis.visibleRange.interval
    ) {
      if (
        (!this.rangeNavigator.dateTimeCategoryModule.sameInterval(
          axis.labels.map(Number)[i as number],
        ...
        this.rangeNavigator.dateTimeCategoryModule.isMaximum(
          i,
          previousIndex,
          axis
        )
      ) {
        const currentLabel: Date = new Date(
          axis.labels.map(Number)[i as number]
        );
        ...
        axis.visibleLabels.push(
          new VisibleLabels(
            this.dateFormats(
              this.rangeNavigator.format(currentLabel),
              axis,
              axis.visibleLabels.length
            ),
            i,
            this.rangeNavigator.labelStyle,
            this.rangeNavigator.format(currentLabel)
          )
        );
        previousIndex = i;
  ...
  private findAxisLabels(axis: Axis, interval: number): void {
    ...
    let text: string;
    interval = this.rangeNavigator.interval
      ? this.rangeNavigator.interval
      : interval;
    ...
    while (nextInterval <= axis.visibleRange.max) {
      text = this.dateFormats(
        this.rangeNavigator.format(new Date(nextInterval)),
        axis,
        axis.visibleLabels.length
      );
      axis.visibleLabels.push(
      ...
      nextInterval = this.increaseDateTimeInterval(
        axis,
        nextInterval,
        interval
      ).getTime();
  ...
  private dateFormats(text: string, axis: Axis, index: number): string {
    ...
    const isFirstLevel: boolean =
      this.rangeNavigator.enableGrouping && this.firstLevelLabels.length === 0;
    switch (axis.actualIntervalType as RangeIntervalType) {
      case 'Quarter':
        if (text.indexOf('Jan') > -1) {
          changedText = !isFirstLevel
            ? text.replace('Jan', 'Quarter1')
            : 'Quarter1';
        } else if (text.indexOf('Apr') > -1) {
          changedText = !isFirstLevel
            ? text.replace('Apr', 'Quarter2')
            : 'Quarter2';
        } else if (text.indexOf('Jul') > -1) {
          changedText = !isFirstLevel
            ? text.replace('Jul', 'Quarter3')
            : 'Quarter3';
        } else if (text.indexOf('Oct') > -1) {
          changedText = !isFirstLevel
            ? text.replace('Oct', 'Quarter4')
            : 'Quarter4';
        }
        break;
      case 'Weeks':
  ...
  private findLabelY(control: RangeNavigator, isSecondary: boolean): number {
    ...
    const textHeight: number = measureText(
      'Quarter1 2011',
      control.labelStyle,
      control.themeStyle.axisLabelFont
    ).height;
    let padding: number = control.labelPosition === 'Inside' ? 3 : 8;
    if (
      (control.labelPosition === 'Outside' &&
        control.tickPosition === 'Outside') ||
      control.series.length === 0
    ) {
      pointY = reference + tickHeight + padding + textHeight * 0.75;
    } else if (
      control.labelPosition === 'Inside' &&
      control.tickPosition === 'Inside'
    ) {
    ...
    if (isSecondary) {
      ...
      if (control.labelPosition === 'Outside' || control.series.length === 0) {
        ...
        pointY =
          control.tickPosition === 'Outside' || control.series.length === 0
            ? reference + tickHeight + padding + textHeight * 0.75
            : reference + padding + textHeight * 0.75;
      }
    }
    return pointY;
  }
  ...
  private placeAxisLabels(
    ...
    const intervalType: RangeIntervalType =
      axis.actualIntervalType as RangeIntervalType;
    const intervalInTime: number =
      (control.labelPlacement === 'Auto' && control.valueType === 'DateTime') ||
      control.labelPlacement === 'BetweenTicks'
        ? maxLabels > 1
          ? axis.visibleLabels[1].value - axis.visibleLabels[0].value
          : (axis.visibleRange.max - axis.visibleLabels[0].value) / 2
        : 0;
    if (
      control.valueType.indexOf('DateTime') > -1 &&
      (intervalType === 'Quarter' || intervalType === 'Weeks')
    ) {
      this.findSuitableFormat(axis, control);
    }
    for (let i: number = 0, len: number = maxLabels; i < len; i++) {
      label = axis.visibleLabels[i as number];
      label.size = measureText(
        <string>label.text,
        axis.labelStyle,
        control.themeStyle.axisLabelFont
      );
      if (
        (control.secondaryLabelAlignment === 'Middle' ||
          id.indexOf('_AxisLabel_') > -1) &&
        (control.labelPlacement === 'Auto' ||
          control.labelPlacement === 'BetweenTicks')
      ) {
        pointX =
          valueToCoefficient(label.value + intervalInTime / 2, axis) *
            rect.width +
          rect.x;
      ...
      if (
        control.labelPlacement === 'OnTicks' &&
        control.labelPosition === 'Inside'
      ) {
        pointX =
          valueToCoefficient(label.value, axis) * rect.width +
          rect.x +
          label.size.width / 2 +
          padding;
      } else if (
      ...
      if (
        (i === 0 ||
          (i === axis.visibleLabels.length - 1 && control.enableRtl)) &&
        pointX < rect.x
      ) {
      ...
      if (
        ...
        pointX =
          valueToCoefficient(label.value, axis) + (rect.x + rect.width / 2);
      }
      //labelrender event
      ...
      const style: FontModel = {
        ...
        color:
          disabledColor ||
          labelStyle.color ||
          control.themeStyle.axisLabelFont.color,
        ...
        textAlignment:
          labelStyle.textAlignment || control.labelStyle.textAlignment,
        textOverflow:
          labelStyle.textOverflow || control.labelStyle.textOverflow,
      };
      ...
      (
        textElement(
          ...
          new TextOption(
            this.rangeNavigator.element.id + id + i,
            pointX,
            pointY,
            'middle',
            argsData.text
          ...
          argsData.labelStyle,
          argsData.labelStyle.color || control.themeStyle.labelFontColor,
          labelElement,
          null,
          null,
          null,
  ...
  private findSuitableFormat(axis: Axis, control: RangeNavigator): void {
    ...
    const interval: number =
      control.valueType === 'DateTime'
        ? labelLength > 1
          ? labels[1].value - labels[0].value
          : axis.visibleRange.interval
        : 0;

    for (let i: number = 0; i < labelLength; i++) {
      currentX =
        valueToCoefficient(labels[i as number].value + interval / 2, axis) *
          bounds.width +
        bounds.x;
      labels[i as number].size = measureText(
        <string>labels[i as number].text,
        axis.labelStyle,
        control.themeStyle.axisLabelFont
      );
      //edgelabelPlacements
      ...
      if ((axis.actualIntervalType as RangeIntervalType) === 'Quarter') {
        if (i !== 0) {
          if (
            labels[i as number].text.indexOf('Quarter') > -1 &&
            this.isIntersect(
              axis,
              currentX,
              labels[i as number].size.width,
              prevX,
              labels[i - 1].size.width
            )
          ) {
        ...
        if (
          i !== 0 &&
          labels[i as number].text.indexOf('Week') > -1 &&
          this.isIntersect(
            axis,
            currentX,
            labels[i as number].size.width,
  ...
  private findAlignment(axis: Axis, index: number): number {
    ...
    return this.rangeNavigator.secondaryLabelAlignment === 'Near'
      ? valueToCoefficient(label.value, axis) * bounds.width +
          bounds.x +
          label.size.width / 2
      : valueToCoefficient(
          nextLabel ? nextLabel.value : axis.visibleRange.max,
          axis
        ) *
          bounds.width +
          bounds.x -
