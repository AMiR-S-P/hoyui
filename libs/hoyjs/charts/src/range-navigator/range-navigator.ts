...
import { Rect, measureText, Size, SvgRenderer } from '@hoyui/svg-base';
...
export class RangeNavigator extends Component<HTMLElement> {
  ...
  public preRender(): void {
    ...
    if (this.element.id === '') {
      const collection: number =
        document.getElementsByClassName('e-rangenavigator').length;
  ...
  private setSliderValue(): void {
    ...
    this.startValue =
      this.startValue || (isDateTimeCategory && this.startValue === 0)
        ? this.startValue
        : !this.value[0]
        ? range.min
        : isDateTime
        ? new Date(this.value[0].toString()).getTime()
        : isDateTimeCategory
        ? this.getRangeValue(new Date(this.value[0].toString()).getTime(), true)
        : +this.value[0];
    this.endValue =
      this.endValue || (isDateTimeCategory && this.endValue === 0)
        ? this.endValue
        : !this.value[1]
        ? range.max
        : isDateTime
        ? new Date(this.value[1].toString()).getTime()
        : isDateTimeCategory
        ? this.getRangeValue(
            new Date(this.value[1].toString()).getTime(),
            false
  ...
  private getRangeValue(value: number, isStart: boolean): number {
    ...
    return isStart
      ? index !== -1
        ? index
        : this.chartSeries.xAxis.actualRange.min
      : index === 0
      ? this.chartSeries.xAxis.actualRange.max
      : index;
  }
  /**
  ...
  public render(): void {
    ...
    this.element.setAttribute(
      'aria-label',
      'Range navigator' + ' Syncfusion interactive chart.'
    );
  }
  ...
  private calculateBounds(): void {
    ...
    const tooltipSpace: number =
      !this.disableRangeSelector && isLeightWeight && this.tooltip.enable
        ? 35
        : 0;
    if (
      !this.periodSelectorModule &&
      this.periodSelectorSettings.periods.length &&
      !this.stockChart
    ) {
    ...
    if (
      this.periodSelectorModule &&
      this.periodSelectorSettings.periods.length > 0
    ) {
      ...
      selector.periodSelectorSize.y =
        this.periodSelectorSettings.position === 'Bottom'
          ? this.availableSize.height - selector.periodSelectorSize.height
          : 0;
    }
    const periodSelectorY: number =
      this.periodSelectorSettings.position === 'Top' && selector
        ? selector.periodSelectorSize.y + selector.periodSelectorSize.height
        : 0;
    let left: number = 0;
    let top: number = 0;
    if (
    ...
    this.bounds = new Rect(
      ...
      this.availableSize.width -
        this.themeStyle.thumbWidth -
        thumb.border.width * 2 -
        margin.left -
        margin.right,
      this.availableSize.height -
        ...
        margin.bottom -
        tooltipSpace -
        (selector ? selector.periodSelectorSize.height : 0)
    );
    const deductHeight: number =
      (this.labelPosition === 'Outside' || isLeightWeight
        ? labelSize + labelPadding
        : 0) +
      (this.tickPosition === 'Outside' || isLeightWeight
        ? this.majorTickLines.height
        : 0);
    this.bounds.height -= deductHeight;
    if (isLeightWeight) {
      const height: number = this.enableGrouping
        ? this.bounds.height - (labelSize + labelPadding)
        : this.bounds.height;
      this.bounds.y +=
        this.themeStyle.thumbHeight > height
          ? (this.themeStyle.thumbHeight - height) / 2
          : 0;
    }
    if (this.disableRangeSelector) {
      ...
      this.bounds.height =
        this.periodSelectorSettings.periods.length > 0
          ? this.periodSelectorSettings.height
          : 0;
    }
  }
  ...
  public rangeResize(): boolean {
    ...
    const beforeResizeArgs: IRangeBeforeResizeEventArgs = {
      name: 'beforeResize',
  ...
  public mouseMove(e: PointerEvent): boolean {
    ...
    if (
      getElement(!this.stockChart ? this.element.id + '_svg' : this.element.id)
    ) {
  ...
  public mouseLeave(e: PointerEvent): boolean {
    ...
    if (rangeSlider.isDrag) {
      ...
      if (rangeSlider.control.allowSnapping) {
        ...
        rangeSlider.setAllowSnapping(
          rangeSlider.control,
          rangeSlider.currentStart,
          rangeSlider.currentEnd,
          false,
          enabledTooltip
        );
      }
  ...
  private setMouseX(e: PointerEvent): number {
    ...
    const svgRect: ClientRect = !this.stockChart
      ? getElement(this.element.id + '_svg').getBoundingClientRect()
