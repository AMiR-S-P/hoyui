...
import { Browser, EmitType, remove, Event, EventHandler } from '@hoyui/base';
  ...
  implements INotifyPropertyChanged
{
  ...
  @Complex<StockChartAxisModel>(
    { name: 'primaryXAxis', valueType: 'DateTime' },
    StockChartAxis
  )
  ...
  @Complex<StockTooltipSettingsModel>(
    { shared: true, enableMarker: false },
   ...
   */
  @Complex<CrosshairSettingsModel>(
    { dashArray: '5', lineType: 'Vertical' },
    CrosshairSettings
  )
  ...
  @Collection<StockChartAnnotationSettingsModel>(
    [{}],
    StockChartAnnotationSettings
  )
  ...
  constructor(options?: StockChartModel, element?: string | HTMLElement) {
    ...
    this.toolbarHeight = this.enablePeriodSelector
      ? Browser.isDevice
        ? 56
        : 42
      : 0;
  ...
  public rangeChanged(updatedStart: number, updatedEnd: number): void {
    ...
    this.periodSelector.datePicker.startDate = this.isDateTimeCategory
      ? new Date(this.sortedData[updatedStart as number])
      : new Date(updatedStart);
    this.periodSelector.datePicker.endDate = this.isDateTimeCategory
      ? new Date(this.sortedData[updatedEnd as number])
      : new Date(updatedEnd);
  ...
  private wireEvents(): void {
    ...
    EventHandler.add(
      ...
      this.stockChartOnMouseDown,
      this
    );
    EventHandler.add(
      this.element,
      Browser.touchMoveEvent,
      this.stockChartOnMouseMove,
      this
    );
    EventHandler.add(
      this.element,
      Browser.touchEndEvent,
      this.stockChartMouseEnd,
      this
    );
    ...
    EventHandler.add(
      this.element,
      'contextmenu',
      this.stockChartRightClick,
      this
    );
    EventHandler.add(
      this.element,
      cancelEvent,
      this.stockChartOnMouseLeave,
      this
    );
  ...
  private initPrivateVariable(): void {
    if (this.element.id === '') {
      const collection: number =
        document.getElementsByClassName('e-stockChart').length;
  ...
  protected render(): void {
    const loadEventData: IStockChartEventArgs = {
      name: 'load',
      stockChart: this,
      theme: this.theme,
    };
    this.trigger('load', loadEventData, () => {
      ...
      if (
        ...
        !(this.series[0].dataSource instanceof DataManager)
      ) {
        this.stockChartDataManagerSuccess();
  ...
  private drawSVG(): void {
    ...
    this.mainObject = this.renderer.createSvg({
      ...
      height:
        ...
        (this.enablePeriodSelector ? this.toolbarHeight : 0) -
  ...
  private calculateVisibleSeries(): void {
    ...
    const seriesCollections: SeriesModel[] = this.series.sort(
      (a: SeriesModel, b: SeriesModel) => {
        return a.zOrder - b.zOrder;
      }
    );
    for (
      let i: number = 0, len: number = seriesCollections.length;
      i < len;
      i++
    ) {
  ...
  private createSecondaryElements(): void {
    const tooltipDiv: Element =
      redrawElement(false, this.element.id + '_Secondary_Element') ||
      this.createElement('div');
    tooltipDiv.id = this.element.id + '_Secondary_Element';
    if (this.title) {
      this.titleSize = measureText(
        this.title,
        this.titleStyle,
    ...
    const height: number =
      (this.enablePeriodSelector ? this.toolbarHeight : 0) +
      this.titleSize.height;
    (tooltipDiv as HTMLElement).style.cssText =
  ...
  public findCurrentData(totalData: Object, xName: string): Object {
    let tempData: Object =
      !this.enablePeriodSelector && !this.enableSelector
        ? totalData
        : undefined;
    const start: number = this.isDateTimeCategory
      ? new Date(this.sortedData[Math.floor(this.startValue)]).getTime()
      : this.startValue;
    const end: number = this.isDateTimeCategory
      ? new Date(this.sortedData[Math.floor(this.endValue)]).getTime()
      : this.endValue;
    if (totalData && start && end) {
      tempData = (totalData as Object[]).filter((data: Object) => {
        return (
  ...
  public chartModuleInjection(): void {
    ...
    for (const modules of this.getInjectedModules()) {
      ...
      if (
        moduleName === 'datetime' ||
        moduleName === 'areaseries' ||
        moduleName === 'steplineseries' ||
        moduleName === 'datetimecategory'
      ) {
        RangeNavigator.Inject(modules);
      }
    }
  ...
  private findRange(): void {
    ...
    if (this.enablePeriodSelector) {
      ...
      this.tempPeriods = this.periods.length
        ? this.periods
        : this.toolbarSelector.calculateAutoPeriods();
        ...
        if (period.selected && period.text.toLowerCase() === 'ytd') {
          if (this.isDateTimeCategory) {
            const currentYear: number = new Date(
              this.sortedData[this.currentEnd]
            ).getFullYear();
            let index: number = this.currentEnd - 1;
            for (; index >= 0; index--) {
              if (
                new Date(this.sortedData[index as number]).getFullYear() !==
                currentYear
              ) {
            ...
            this.startValue = new Date(
              new Date(this.currentEnd).getFullYear().toString()
            ).getTime();
          }
        } else if (period.selected && period.text.toLowerCase() === 'all') {
          this.startValue = this.seriesXMin;
        } else if (period.selected) {
          this.startValue = this.periodSelector
            .changedRange(period.intervalType, this.endValue, period.interval)
            .getTime();
          this.startValue = this.isDateTimeCategory
            ? this.periodSelector.findStartValue(this.startValue, this.endValue)
            : this.startValue;
        }
      });
    } else {
  ...
  public stockChartResize(): boolean {
    ...
    this.resizeTo = +setTimeout((): void => {
      if (this.cartesianChart) {
        ...
        this.cartesianChart.cartesianChartRefresh(this);
        if (!this.legendSettings.visible) {
          this.mainObject.setAttribute(
            'width',
            this.availableSize.width.toString()
          );
        }
        if (this.enablePeriodSelector) {
          this.renderPeriodSelector();
  ...
  public stockChartOnMouseDown(e: PointerEvent): boolean {
    ...
    if (target.id.indexOf(this.element.id + '_stockChart_chart') > -1) {
      const svgRect: ClientRect = getElement(
        this.element.id + '_stockChart_chart'
      ...
      getElement(this.element.id + '_stockChart_chart').setAttribute(
        'cursor',
        'pointer'
      );
      if (this.isDateTimeCategory) {
        this.visibleRange.min = this.sortedData.indexOf(
          parseInt(
            this.referenceXAxis.labels[this.referenceXAxis.visibleRange.min],
            10
          )
        );
        this.visibleRange.max = this.sortedData.indexOf(
          parseInt(
            this.referenceXAxis.labels[this.referenceXAxis.visibleRange.max],
            10
          )
        );
      ...
      this.mouseDownXPoint = getRangeValueXByPoint(
        this.mouseX - this.referenceXAxis.rect.x,
        this.referenceXAxis.rect.width,
        this.visibleRange,
        this.referenceXAxis.isInversed
      );
      this.allowPan = true;
  ...
  private setMouseXY(pageX: number, pageY: number): void {
    const svgRectElement: Element = getElement(
      this.element.id + '_stockChart_chart'
    );
    if (this.element && svgRectElement) {
      ...
      this.mouseX =
        pageX - stockRect.left - Math.max(svgRect.left - stockRect.left, 0);
      this.mouseY =
        pageY - stockRect.top - Math.max(svgRect.top - stockRect.top, 0);
    }
  }
  ...
  public chartOnMouseMove(e: PointerEvent | TouchEvent): boolean {
    ...
    if (
      this.allowPan &&
      !this.chart.startMove &&
      this.mouseDownXPoint &&
      this.mouseX !== this.previousMouseMoveX &&
      ...
      this.mouseUpXPoint = getRangeValueXByPoint(
        this.mouseX - this.referenceXAxis.rect.x,
        this.referenceXAxis.rect.width,
        this.visibleRange,
        this.referenceXAxis.isInversed
      ...
      if (this.mouseDownXPoint < this.mouseUpXPoint) {
        if (this.seriesXMin <= this.visibleRange.min - diff) {
          ...
          if (this.enableSelector) {
            this.rangeNavigator.rangeSlider.setSlider(
              this.visibleRange.min - diff,
  ...
  public destroy(): void {
    ...
    const element: HTMLElement = document.getElementById(
      this.element.id + '_stockChart_chartKeyboard_chart_focus'
    );
    if (element) {
      element.remove();
    }
    const borderElement: HTMLElement = document.getElementById(
      this.element.id + '_stock_border'
    );
    if (borderElement) {
  ...
  private renderTitle(): void {
    ...
    if (this.title) {
      ...
      const getAnchor: string =
        alignment === 'Near' ? 'start' : alignment === 'Far' ? 'end' : 'middle';
      ...
      textElement(
        ...
        this.titleStyle.color ||
          this.themeStyle.chartTitleFont.color ||
          this.findTitleColor(),
        getElement(this.element.id + '_stockChart_Title'),
        false,
        false,
        null,
        null,
  ...
  public calculateLegendBounds(): void {
    ...
    this.tempAvailableSize = new Size(
    ...
    if (this.stockLegendModule && this.legendSettings.visible) {
      this.stockLegendModule.calculateLegendBounds(
        this.initialClipRect,
        this.tempAvailableSize,
        null
      );
    }
  }
  ...
  public renderLegend(): void {
    if (
      this.stockLegendModule &&
      this.stockLegendModule.legendCollections.length &&
      ...
      if (
        this.legendSettings.position === 'Auto' ||
  ...
  private findTitleColor(): string {
    if (
      this.theme.toLocaleLowerCase().indexOf('highcontrast') > -1 ||
      this.theme.indexOf('Dark') > -1
    ) {
      return '#ffffff';
    }
    return '#424242';
  ...
  public calculateStockEvents(): void {
    if (this.stockEvents.length) {
      ...
      appendChildElement(
        false,
        this.chartObject,
        this.stockEvent.renderStockEvents()
      );
    }
  }
}
