...
export class StockLegend extends BaseLegend {
          ...
          this.legendCollections.push(
            new LegendOptions(
              ...
              series.category === 'TrendLine'
                ? (this.chart as StockChart).series[series.sourceIndex]
  ...
  public getLegendBounds(
    availableSize: Size,
    legendBound: Rect,
    legend: StockChartLegendSettingsModel
  ): void {
    ...
    let titleSpace: number =
      this.isTitle && titlePosition === 'Top'
        ? this.fivePixel + this.legendTitleSize.height
        : 0;
    titleSpace =
    ...
    for (let i: number = 0; i < this.legendCollections.length; i++) {
      ...
      if (legendOption.render) {
        ...
        if (
          legendBound.width < padding + rowWidth + titlePlusArrowSpace ||
          this.isVertical
        ) {
  ...
  public getRenderPoint(
    ...
    const previousBound: number =
      prevLegend.location.x + textPadding + prevLegend.textSize.width;
    const padding: number = this.legend.padding;
    if (
      previousBound +
        (legendOptions.textSize.width + textPadding - this.itemPadding) >
        rect.x + rect.width + this.legend.shapeWidth / 2 ||
      this.isVertical
    ) {
      legendOptions.location.x = start.x;
      legendOptions.location.y =
        count === firstLegend
  ...
  public legendClick(index: number): void {
    ...
    if (chart.legendSettings.toggleVisibility) {
      ...
      targetLegend.visible =
        targetSeries.category === 'TrendLine'
          ? chart.series[targetSeries.sourceIndex].trendlines[
              targetSeries.index
            ].visible
        ...
        pageY +
          (this.isPaging
            ? (this.currentPageNumber - 1) *
              this.translatePage(false, null, 1, 2)
            : 0),
        region.rect
      );
    });
