...
export class StockEvents extends BaseTooltip {
  ...
  public renderStockEvents(): Element {
    ...
    const stockEventsElementGroup: Element = sChart.renderer.createGroup({
      id: this.chartId + '_StockEvents',
    });
    this.symbolLocations = initialArray(
      sChart.series.length,
      sChart.stockEvents.length,
    ...
    for (let i: number = 0; i < sChart.stockEvents.length; i++) {
      ...
      for (const series of sChart.chart.series as Series[]) {
        ...
        textSize = measureText(
          stockEvent.text + 'W',
          stockEvent.textStyle,
          this.stockChart.themeStyle.axisLabelFont
        );
        if (!argsData.cancel) {
          stockEventElement = sChart.renderer.createGroup({
            id: this.chartId + '_Series_' + series.index + '_StockEvents_' + i,
          });
          let stockEventDate: number = this.dateParse(
            stockEvent.date
          ).getTime();
          stockEventDate = this.stockChart.isDateTimeCategory
            ? series.xAxis.labels.indexOf(stockEventDate.toString())
            : stockEventDate;
          if (
            withIn(stockEventDate, series.xAxis.visibleRange) &&
            stockEventDate >= series.xMin &&
            stockEventDate <= series.xMax
          ) {
            if (stockEvent.seriesIndexes.length > 0) {
              for (
                let j: number = 0;
                j < stockEvent.seriesIndexes.length;
                j++
              ) {
  ...
  private creatEventGroup(
    stockEventElement: Element,
    series: Series,
    stockEvent: StockEventsSettingsModel,
    i: number,
    textSize: Size
  ): Element {
    const symbolLocation: ChartLocation = this.findClosePoint(
      series,
      stockEvent
    );
  ...
  private findClosePoint(
    ...
    for (let k: number = 0; k < series.points.length; k++) {
      ...
      if (closeIndex === point.xValue && point.visible) {
        ...
        if (
          closeIndex > series.points[k - 1].xValue &&
          closeIndex < series.points[k + 1].xValue
        ) {
          pointData = new PointData(point, series);
        }
    ...
    const xPixel: number =
      series.xAxis.rect.x +
      valueToCoefficient(pointData.point.xValue, series.xAxis) *
        series.xAxis.rect.width;
    yPixel =
      valueToCoefficient(
        pointData.point[sEvent.placeAt] as number,
        series.yAxis
  ...
  private createStockElements(
    stockEventElement: Element,
    stockEve: StockEventsSettingsModel,
    series: Series,
    i: number,
    ...
    const result: Size = new Size(
      ...
      textSize.height > 20 ? textSize.height : 20
    );
    let pathString: string;
    let pathOption: PathOption;
    const lx: number = symbolLocation.x;
    const ly: number = symbolLocation.y;
    const stockId: string =
      this.chartId + '_Series_' + series.index + '_StockEvents_' + i;
    ...
    switch (stockEve.type) {
      ...
      case 'Square':
        ...
        stockEventElement.appendChild(
          drawSymbol(
            new ChartLocation(
              stockEve.type !== 'Flag' ? lx : lx + result.width / 2,
              ly - result.height
            ),
            stockEve.type,
            result,
        ...
        textElement(
          ...
          new TextOption(
            ...
            stockEve.type !== 'Flag'
              ? symbolLocation.x
              : symbolLocation.x + result.width / 2,
            symbolLocation.y - result.height,
            'middle',
            stockEve.text,
            '',
            'middle'
      ...
      case 'ArrowLeft':
        pathString =
          'M' + ' ' + lx + ' ' + ly + ' ' + this.findArrowpaths(stockEve.type);
        ...
        stockEventElement.appendChild(
          this.stockChart.renderer.drawPath(pathOption)
        );
        break;
      case 'Triangle':
      case 'InvertedTriangle':
        ...
        stockEventElement.appendChild(
          drawSymbol(
            new ChartLocation(symbolLocation.x, symbolLocation.y),
            stockEve.type,
            new Size(20, 20),
            '',
            new PathOption(
  ...
  public renderStockEventTooltip(targetId: string): void {
    const seriesIndex: number = parseInt(
      targetId.split('_StockEvents_')[0].split(this.chartId + '_Series_')[1],
      10
    );
    const pointIndex: number = parseInt(
      targetId.split('_StockEvents_')[1].replace(/\D+/g, ''),
      10
    );
    const updatedLocation: ChartLocation =
      this.symbolLocations[seriesIndex as number][pointIndex as number];
    const pointLocation: ChartLocation = new ChartLocation(
      updatedLocation.x,
      updatedLocation.y +
        this.stockChart.toolbarHeight +
        this.stockChart.titleSize.height
    );
    ...
    const svgElement: HTMLElement = this.getElement(
      this.chartId + '_StockEvents_Tooltip_svg'
    );
    const isTooltip: boolean =
      this.stockEventTooltip &&
      svgElement &&
    ...
    if (!isTooltip) {
      ...
      getElement(this.chartId + '_Secondary_Element').appendChild(
        tooltipElement
      );
      this.stockEventTooltip = new Tooltip({
        ...
        header: '',
        content: [
          this.stockChart.stockEvents[pointIndex as number].description,
        ],
      ...
      this.stockEventTooltip.areaBounds.y +=
        this.stockChart.toolbarHeight + this.stockChart.titleSize.height;
      this.stockEventTooltip.appendTo('#' + tooltipElement.id);
    } else {
      this.stockEventTooltip.content = [
        this.stockChart.stockEvents[pointIndex as number].description,
      ];
  ...
  public removeStockEventTooltip(duration: number): void {
    const tooltipElement: HTMLElement = this.getElement(
      this.chartId + '_StockEvents_Tooltip'
    );
    this.stopAnimation();
  ...
  private applyHighLights(pointIndex: number, seriesIndex: number): void {
    ...
    const stockId: string =
      this.chartId + '_Series_' + seriesIndex + '_StockEvents_' + pointIndex;
  ...
  private dateParse(value: Date | string): Date {
    const dateParser: Function = this.chart.intl.getDateParser({
      skeleton: 'full',
      type: 'dateTime',
    });
    const dateFormatter: Function = this.chart.intl.getDateFormat({
      skeleton: 'full',
      type: 'dateTime',
    });
    return new Date(
