...
import { DropDownButton, MenuEventArgs } from '@hoyui/splitbuttons';
import { ItemModel } from '@hoyui/navigations';
import { Rect, TextOption, measureText, SvgRenderer } from '@hoyui/svg-base';
...
export class ToolBarSelector {
  ...
  public initializePeriodSelector(): void {
    ...
    const rect: Rect =
      this.stockChart.chart.chartAxisLayoutPanel.seriesClipRect;
    const htmlElement: HTMLElement = getElement(
      this.stockChart.element.id + '_Secondary_Element'
    ) as HTMLElement;
    const height: number = this.stockChart.toolbarHeight;
    this.stockChart.periodSelector.appendSelector(
      { thumbSize: 0, element: htmlElement, width: rect.width, height: height },
      rect.x
    );
  ...
  private getDropDownItems(
    ...
    if (type === this.stockChart.seriesType) {
      for (let i: number = 0; i < type.length; i++) {
        result.push({
          text: '&nbsp;&nbsp;&nbsp;' + type[i as number].toString(),
        });
      }
      for (let i: number = 0; i < this.stockChart.series.length; i++) {
        for (let j: number = 0; j < result.length; j++) {
          ...
          if (text === this.stockChart.series[i as number].type) {
            result[j as number].text = result[j as number].text.replace(
              '&nbsp;&nbsp;&nbsp;',
              '&#10004&nbsp;'
            );
          }
  ...
  private addedSeries(seriesType: string): void {
    ...
    for (let i: number = 0; i < series.length; i++) {
      ...
      series[i as number].type = <ChartSeriesType>(
        (seriesType.indexOf('Candle') > -1
          ? 'Candle'
          : seriesType.indexOf('OHLC') > -1
          ? 'HiloOpenClose'
          : seriesType)
      );
      ...
      for (
        let index: number = 0;
        index < series[i as number].trendlines.length;
        index++
      ) {
        const trendLine: TrendlineModel =
          series[i as number].trendlines[index as number];
  ...
  public initializeSeriesSelector(): void {
    if (this.stockChart.seriesType.indexOf('HiloOpenClose') > -1) {
      this.stockChart.seriesType[
        this.stockChart.seriesType.indexOf('HiloOpenClose')
      ] = 'OHLC' as ChartSeriesType;
    }
    if (
      this.stockChart.seriesType.indexOf('Candle') > -1 &&
      this.stockChart.seriesType.indexOf('Hollow Candle' as ChartSeriesType) ===
        -1
    ) {
      this.stockChart.seriesType[this.stockChart.seriesType.length] =
        'Hollow Candle' as ChartSeriesType;
    }
    const seriesType: DropDownButton = new DropDownButton({
      ...
      select: (args: MenuEventArgs) => {
        ...
        if (
          ...
          this.stockChart.stockLegendModule.legendCollections.length &&
          this.stockChart.legendSettings.visible
        ) {
          const bounds: Rect = this.stockChart.stockLegendModule.legendBounds;
          this.stockChart.stockLegendModule.renderLegend(
            this.stockChart,
            this.stockChart.legendSettings,
            bounds
          );
        }
      },
  ...
  public initializeTrendlineSelector(): void {
    this.trendlineDropDown = new DropDownButton({
      enableHtmlSanitizer: false,
      items: this.stockChart.resizeTo
        ? this.trendlineDropDown.items
        ...
        if (this.trendline !== type) {
          ...
          for (let i: number = 0; i < this.stockChart.series.length; i++) {
            ...
            if (this.stockChart.series[0].trendlines.length === 0) {
              ...
              this.stockChart.series[0].trendlines[0].animation.enable = this
                .stockChart.trendlinetriggered
                ? true
                : false;
            }
          }
    ...
    this.trendlineDropDown.appendTo(
      '#' + this.stockChart.element.id + '_trendType'
    );
  }
  public initializeIndicatorSelector(): void {
      ...
      select: (args: MenuEventArgs) => {
        for (let l: number = 0; l < this.stockChart.series.length; l++) {
          if (this.stockChart.series[l as number].trendlines.length !== 0) {
            this.stockChart.series[l as number].trendlines[0].animation.enable =
              false;
          }
        }
        args.item.text =
          args.item.text.indexOf('&#10004&nbsp;') >= 0
            ? args.item.text.substr(args.item.text.indexOf(';') + 1)
            : args.item.text;
        ...
        this.selectedIndicator =
          this.selectedIndicator.indexOf(type) === -1
        ...
        if (
          ...
          if (this.indicators.indexOf(type) === -1) {
            args.item.text =
              '&#10004&nbsp;' +
              args.item.text.replace('&nbsp;&nbsp;&nbsp;', '');
            const indicator: TechnicalIndicatorModel[] = this.getIndicator(
              type,
              this.stockChart.series[0].yAxisName
            );
            this.indicators.push(type);
            this.stockChart.indicators =
              this.stockChart.indicators.concat(indicator);
            this.stockChart.cartesianChart.initializeChart();
          } else {
            args.item.text = '&nbsp;&nbsp;&nbsp;' + args.item.text;
            for (
              let z: number = 0;
              z < this.stockChart.indicators.length;
              z++
            ) {
    ...
    this.indicatorDropDown.appendTo(
      '#' + this.stockChart.element.id + '_indicatorType'
    );
  }
  ...
  public createIndicatorAxes(
    type: TechnicalIndicators,
    args: MenuEventArgs
  ): void {
    if (this.indicators.indexOf(type) === -1) {
      args.item.text =
        '&#10004&nbsp;' + args.item.text.replace('&nbsp;&nbsp;&nbsp;', '');
      this.indicators.push(type);
      ...
      const axis: AxisModel[] = [
        {
          ...
          rowIndex: !this.stockChart.isSingleAxis
            ? this.stockChart.axes.length
            : 0,
          desiredIntervals: 1,
          labelFormat: 'n2',
      ...
      this.stockChart.primaryYAxis.rowIndex = !this.stockChart.isSingleAxis
        ? 0
        : len + 1;
      const indicator: TechnicalIndicatorModel[] = this.getIndicator(
        type,
      ...
      for (let z: number = 0; z < this.stockChart.axes.length; z++) {
        if (
          this.stockChart.axes[z as number].rowIndex !== 0 &&
          this.stockChart.axes[z as number].rowIndex > removedIndex
        ) {
          this.stockChart.axes[z as number].rowIndex =
            this.stockChart.axes[z as number].rowIndex - 1;
        }
      }
      this.stockChart.cartesianChart.initializeChart();
    }
  ...
  public tickMark(args: MenuEventArgs): string {
    ...
    for (let i: number = 0; i < items.length; i++) {
      items[i as number].text =
        items[i as number].text.indexOf('&#10004&nbsp;') >= 0
          ? items[i as number].text.substr(
              items[i as number].text.indexOf(';') + 1
            )
          : items[i as number].text;
    ...
    const exportChart: DropDownButton = new DropDownButton({
      ...
      select: (args: MenuEventArgs) => {
        ...
        if (this.stockChart.chart.exportModule) {
          ...
          const svgHeight: ClientRect =
            stockChart.svgObject.getBoundingClientRect();
          this.stockChart.svgObject.insertAdjacentElement(
            'afterbegin',
            this.addExportSettings(type === 'Print')
          );
          const additionalRect: ClientRect =
            stockChart.svgObject.firstElementChild.getBoundingClientRect();
          const rect: RectOption = new RectOption(
            ...
            new Rect(
              0,
              0,
              this.stockChart.availableSize.width,
              additionalRect.height
            )
          );
          stockChart.svgObject.firstElementChild.insertAdjacentElement(
            'afterbegin',
            this.stockChart.renderer.drawRectangle(rect) as HTMLElement
          );
          this.stockChart.svgObject.setAttribute(
            'height',
            (svgHeight.height + additionalRect.height).toString()
          );
          (getElement(stockID + 'chart') as HTMLElement).style.transform =
            'translateY(' + additionalRect.height + 'px)';
          if (stockChart.enableSelector) {
            (getElement(stockID + 'rangeSelector') as HTMLElement).setAttribute(
              'transform',
              'translate(' +
                0 +
                ',' +
                (stockChart.cartesianChart.cartesianChartSize.height +
                  additionalRect.height) +
                ')'
            );
          }
          if (
            ...
            (
              getElement(
                stockChart.element.id + '_chart_legend_g'
              ) as HTMLElement
            ).style.transform = 'translateY(' + additionalRect.height + 'px)';
          }
          if (type === 'Print') {
            this.stockChart.chart.print(this.stockChart.svgObject.id);
          } else {
            stockChart.chart.exportModule.export(
              type,
              'StockChart',
              null,
              [stockChart],
              null,
              stockChart.svgObject.clientHeight
            );
          }
  ...
  public calculateAutoPeriods(): PeriodsModel[] {
    ...
    const min: number = this.stockChart.isDateTimeCategory
      ? this.stockChart.sortedData[this.stockChart.seriesXMin]
      : this.stockChart.seriesXMin;
    const max: number = this.stockChart.isDateTimeCategory
      ? this.stockChart.sortedData[this.stockChart.seriesXMax]
  ...
  public findRange(min: number, max: number): PeriodsModel[] {
    ...
    } else if ((max - min) / 1.577e10 >= 1) {
      defaultPeriods.push(
        { text: '1M', interval: 1, intervalType: 'Months' },
        { text: '3M', interval: 3, intervalType: 'Months' },
        { text: '6M', interval: 6, intervalType: 'Months' }
      );
    } else if ((max - min) / 2.628e9 >= 1) {
      defaultPeriods.push(
        { text: '1D', interval: 1, intervalType: 'Days' },
        { text: '3W', interval: 3, intervalType: 'Weeks' },
        { text: '1M', interval: 1, intervalType: 'Months' }
      );
    } else if ((max - min) / 8.64e7 >= 1) {
      defaultPeriods.push(
  ...
  private addExportSettings(isPrint?: boolean): Element {
    const exportElement: Element = this.stockChart.renderer.createGroup({
      ...
      fill: this.stockChart.background
        ? this.stockChart.background
        : 'transparent',
    });
    const titleHeight: number = measureText(
      this.stockChart.title,
      this.stockChart.titleStyle,
      this.stockChart.themeStyle.chartTitleFont
    ).height;
    const options: TextOption = new TextOption(
      exportElement.id + '_Title',
      titlePositionX(
        new Rect(0, 0, this.stockChart.availableSize.width, 0),
        this.stockChart.titleStyle
      ),
      0,
      'middle',
    ...
    textElement(
      this.stockChart.renderer as SvgRenderer,
      options,
      this.stockChart.titleStyle,
      this.stockChart.titleStyle.color ||
        this.stockChart.themeStyle.chartTitleFont.color,
      exportElement,
      null,
      null,
      null,
    ...
    let x: number =
      measureText('Series: ' + this.selectedSeries, style).width / 2;
    const y: number = titleHeight;
    this.textElementSpan(
      new TextOption(
        exportElement.id + '_Series',
        x,
        y,
        'start',
    ...
    if (this.selectedIndicator !== '') {
      ...
      x += measureText(
        'Indicator: ' + this.selectedIndicator + ' Z',
        style
      ).width;
    }
    ...
    if (this.selectedTrendLine !== '') {
      this.textElementSpan(
        new TextOption(
          exportElement.id + '_TrendLine',
          x,
          y,
          'start',
          ['Trendline :', this.selectedTrendLine],
          '',
          'text-before-edge'
        ),
        style,
