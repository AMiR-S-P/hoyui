...
export class CartesianChart {
  ...
  public initializeChart(chartArgsData?: object[]): void {
    ...
    if (!stockChart.chartObject) {
      ...
      const chartElement: Element = document.getElementById(
        stockChart.chartObject.id
      );
    ...
    this.stockChart.isDateTimeCategory =
      this.stockChart.primaryXAxis.valueType === 'DateTimeCategory';
    if (
      this.stockChart.isDateTimeCategory &&
      !this.stockChart.sortedData.length
    ) {
      for (const series of this.stockChart.series) {
        ...
        for (const dataItem of dataSource) {
          const currentData: number = Date.parse(
            new Date(
              DataUtil.parse.parseJson({ val: dataItem[xName as string] }).val
            ).toString()
          );
    ...
    stockChart.chart = new Chart({
      ...
      seriesRender: (args: ISeriesRenderEventArgs) => {
        startValue =
          this.stockChart.startValue != null &&
          this.stockChart.isDateTimeCategory
            ? this.stockChart.sortedData[Math.floor(this.stockChart.startValue)]
            : this.stockChart.startValue;
        endValue =
          this.stockChart.endValue != null && this.stockChart.isDateTimeCategory
            ? this.stockChart.sortedData[Math.floor(this.stockChart.endValue)]
            : this.stockChart.endValue;
        if (args.data && startValue && endValue) {
          args.data = (args.data as Object[]).filter((data: Object) => {
            return (
              new Date(
                DataUtil.parse.parseJson({ val: data[args.series.xName] }).val
              ).getTime() >= startValue &&
              new Date(
                DataUtil.parse.parseJson({ val: data[args.series.xName] }).val
              ).getTime() <= endValue
      ...
      onZooming: (args: IZoomingEventArgs) => {
        this.stockChart.trigger(onZooming, args);
      },
      ...
      zoomComplete: (args: IZoomCompleteEventArgs) => {
        if (
          args.axis.valueType.indexOf('DateTime') !== -1 &&
          stockChart.rangeNavigator
        ) {
          this.stockChart.zoomChange = true;
          const newRange: Range = this.calculateUpdatedRange(
    ...
    if (stockChart.indicators.length !== 0) {
      if (stockChart.isSelect) {
        for (let i: number = 0; i < stockChart.indicators.length; i++) {
          ...
          stockChart.chart.indicators[i as number].dataSource = extend(
            [],
            stockChart.chart.series[0].dataSource,
            null,
            true
          );
        }
    ...
    if (stockChart.onPanning) {
      getElement(this.stockChart.element.id + '_stockChart_chart').setAttribute(
        'cursor',
        'pointer'
      );
  ...
  private findMargin(stockChart: StockChart): MarginModel {
    ...
    margin.top =
      stockChart.stockLegendModule &&
      stockChart.legendSettings.visible &&
      stockChart.legendSettings.position === 'Top'
        ? stockChart.margin.top
        : stockChart.margin.top * 2;
  ...
  private findSeriesCollection(series: StockSeriesModel[]): Series[] {
    ...
    for (let i: number = 0, len: number = series.length; i < len; i++) {
      ...
      if ((series[i as number] as StockSeries).localData) {
        chartSeries[i as number].dataSource = (
          series[i as number] as StockSeries
        ).localData;
      }
      chartSeries[i as number].yName =
        series[i as number].yName === ''
          ? series[i as number].close
          : series[i as number].yName;
      chartSeries[i as number].splineType =
        chartSeries[i as number].type === 'Spline' ? 'Cardinal' : 'Natural';
    }
    return chartSeries;
  }
  ...
  public calculateChartSize(): Size {
    const stockChart: StockChart = this.stockChart;
    return new Size(
      stockChart.availableSize.width,
      stockChart.enablePeriodSelector && stockChart.enableSelector
        ? stockChart.availableSize.height - stockChart.toolbarHeight - 51
        : stockChart.enableSelector && !stockChart.enablePeriodSelector
        ? stockChart.availableSize.height - 51
        : stockChart.enablePeriodSelector && !stockChart.enableSelector
        ? stockChart.availableSize.height - stockChart.toolbarHeight
        : stockChart.availableSize.height
    );
  }
  ...
  private calculateUpdatedRange(
    ...
    if (this.stockChart.isDateTimeCategory) {
      start = this.stockChart.sortedData.indexOf(
        parseInt(axis.labels[Math.floor(start)], 10)
      );
      end = this.stockChart.sortedData.indexOf(
        parseInt(axis.labels[Math.floor(end)], 10)
      );
    }
