...
export class RangeSelector {
  ...
  public initializeRangeNavigator(): void {
    ...
    const padding: number =
      stockChart.chart.axisCollections[1].labelPadding +
    ...
    stockChart.rangeNavigator = new RangeNavigator({
      ...
      value: [
        stockChart.isDateTimeCategory
          ? new Date(stockChart.sortedData[Math.floor(stockChart.startValue)])
          : new Date(stockChart.startValue),
        stockChart.isDateTimeCategory
          ? new Date(stockChart.sortedData[Math.floor(stockChart.endValue)])
      ...
      changed: (args: IChangedEventArgs) => {
        ...
        this.stockChart.startValue = args.start as number;
        this.stockChart.endValue = args.end as number;
        if (!this.stockChart.zoomChange) {
          this.stockChart.cartesianChart.cartesianChartRefresh(
            this.stockChart,
            arg.data
          );
        }
        if (stockChart.periodSelector && stockChart.periodSelector.datePicker) {
          stockChart.periodSelector.datePicker.startDate = this.stockChart
            .isDateTimeCategory
            ? new Date(
                this.stockChart.sortedData[Math.floor(args.start as number)]
              )
            : new Date(args.start as number);
          stockChart.periodSelector.datePicker.endDate = this.stockChart
            .isDateTimeCategory
            ? new Date(
                this.stockChart.sortedData[Math.floor(args.end as number)]
              )
            : new Date(args.end as number);
          stockChart.periodSelector.datePicker.dataBind();
        }
      },
    });
    stockChart.rangeNavigator.stockChart = stockChart;
    stockChart.rangeNavigator.appendTo(
      stockChart.selectorObject as HTMLElement
    );
  }
  ...
  private calculateChartSize(): Size {
    ...
    return new Size(
      ...
      stockChart.enableSelector ? 51 : 0
    );
  }
  ...
  public sliderChange(start: number, end: number): void {
    this.stockChart.rangeNavigator.rangeSlider.performAnimation(
      start,
      end,
      this.stockChart.rangeNavigator
    );
  }
}
