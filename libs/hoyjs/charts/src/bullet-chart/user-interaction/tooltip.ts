...
import { compile as templateComplier } from '@hoyui/base';
...
export class BulletTooltip {
   ...
   * @returns {void}
   */
  public _elementTooltip(
    e: PointerEvent,
    targetClass: string,
    targetId: string,
    mouseX: number
  ): void {
    const tooltipDiv: HTMLDivElement = <HTMLDivElement>(
    ...
    let str: string = '';
    const font: string = this.control.tooltip.textStyle.fontStyle
      ? this.control.tooltip.textStyle.fontStyle
      : this.control.themeStyle.tooltipLabelFont.fontStyle;
    const fill: string = this.control.tooltip.fill
      ? this.control.tooltip.fill
      : this.control.themeStyle.tooltipFill;
    const color: string =
      this.control.themeStyle.tooltipLabelFont.color ||
      this.control.themeStyle.tooltipBoldLabel;
    const style: string =
      'left:' +
      pageX +
      'px;' +
    ...
    if (targetClass === this.control.svgObject.id + '_Caption') {
      ...
      str =
        target.textContent === this.control.subtitle
          ? ''
          : this.control.subtitle;
    }
    if (str !== '') {
  ...
  public _displayTooltip(
    ...
    if (
      ...
      (targetClass === this.control.svgObject.id + '_FeatureMeasure' ||
      ...
      const measureId: string = targetId.substring(
        targetId.lastIndexOf('_') + 1
      );
      ...
      const currentVal: number =
        this.control.dataSource[measureId as string][this.control.valueField];
      targetVal = targetVal.concat(
        this.control.dataSource[measureId as string][this.control.targetField]
      );
      const categoryVal: number =
        this.control.dataSource[measureId as string][
          this.control.categoryField
        ];
      ...
      labelCurrentText = this.bulletAxis.formatValue(
        this.bulletAxis,
        isCustomFormat,
        format,
        +currentVal
      ...
      for (let i: number = 0; i < targetVal.length; i++) {
        targetValues = targetValues.concat(
          this.bulletAxis.formatValue(
            this.bulletAxis,
            isCustomFormat,
            format,
            +targetVal[i as number]
          )
        );
      }
      labelCategoryText = this.bulletAxis.formatValue(
        this.bulletAxis,
        isCustomFormat,
        format,
        +categoryVal
      );
      const data: IBulletTooltipContent = {
        value: labelCurrentText,
        target: targetValues,
        category: labelCategoryText,
      };
      const tooltipData: IBulletTooltipContent = {
        value: labelCurrentText,
        target: labelTargetText,
        category: labelCategoryText,
      };
      const style: string =
        'position: absolute; z-index: 13000; display: block;';
      if (
        document.getElementsByClassName('tooltipDiv' + this.control.element.id)
          .length === 0
      ) {
        ...
        tooltipdiv.style.cssText = style;
        document
          .getElementById(this.control.element.id + '_Secondary_Element')
      ...
      if (
        this.control.tooltip.template !== '' &&
        this.control.tooltip.template != null
      ) {
        this.updateTemplateFn();
        const elem: Element = this.control.createElement('div', {
        ...
        for (let i: number = 0; i < argsData.target.length; i++) {
          argsText +=
            ...
            (i === 0 ? '' : '_' + i) +
            ' : ' +
            argsData.target[i as number];
        }
        argsData.text = argsText;
        this.control.trigger(tooltipRender, argsData);
        tooltipdiv.innerHTML = argsData.text;
        tooltipdiv.style.font = this.control.tooltip.textStyle.fontStyle
          ? this.control.tooltip.textStyle.fontStyle
          : this.control.themeStyle.tooltipLabelFont.fontStyle;
      ...
      const borderWidth: number =
        this.control.theme === 'Fabric' ||
        (this.control.theme === 'Fluent' && !this.control.tooltip.border.width)
          ? 1
          : this.control.tooltip.border.width;
      const borderColor: string =
        this.control.theme === 'Fabric' ||
        (this.control.theme === 'Fluent' && !this.control.tooltip.border.color)
          ? '#D2D0CE'
          : this.control.tooltip.border.color;
      const borderDashArray: string = this.control.tooltip.border.dashArray
        ? 'dashed ' +
          borderColor +
          '; border-dasharray: ' +
          this.control.tooltip.border.dashArray +
          ';'
        : 'Solid' + ' ' + borderColor + ';';
      ...
      xPos =
        xPos +
          stringToNumber(
            tooltipdiv.getAttribute('width'),
            this.control.containerWidth
          ) <
        window.innerWidth
          ? xPos
          : stringToNumber(
      ...
      yPos =
        yPos +
          stringToNumber(
            tooltipdiv.getAttribute('height'),
            this.control.containerHeight
          ) <
        window.innerHeight
          ? yPos
          : stringToNumber(
      ...
      if (
        ...
        const fontFamily: string =
          this.control.tooltip.textStyle.fontFamily ||
          this.control.themeStyle.tooltipLabelFont.fontFamily;
        const color: string =
          this.control.tooltip.textStyle.color ||
          this.control.themeStyle.tooltipLabelFont.color;
        const divStyle: string =
          ...
          fontFamily +
          '; font-size:' +
          this.control.tooltip.textStyle.size +
        ...
        if (this.control.theme.indexOf('Fluent2') > -1) {
          ...
          const shadow: string =
            `<filter id="${shadowId}" height="130%"><feGaussianBlur in="SourceAlpha" stdDeviation="3"/>` +
          ...
          defElement.setAttribute(
            'id',
            this.control.element.id + 'SVG_tooltip_definition'
          );
  ...
  public updateTemplateFn(): void {
    if (this.control.tooltip.template) {
      try {
        if (
          typeof this.control.tooltip.template !== 'function' &&
          document.querySelectorAll(this.control.tooltip.template).length
        ) {
          this.templateFn = templateComplier(
            document
              .querySelector(this.control.tooltip.template)
              .innerHTML.trim()
          );
        } else {
