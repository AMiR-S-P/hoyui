...
export class BulletChartLegend extends BaseLegend {
  ...
  public getLegendOptions(visibleRangeCollection: Range[]): void {
    ...
    for (const range of visibleRangeCollection) {
      if (range.name !== null) {
        fill = range.color
          ? range.color
          : bulletChart.themeStyle.rangeStrokes[range.index][key as string];
        this.legendCollections.push(
          new LegendOptions(
            range.name,
            fill,
            range.shape,
            this.chart.legendSettings.visible,
            null,
            range.legendImageUrl,
            null,
            false,
            range.index,
            null
    ...
    if (bulletChart.dataSource !== null && bulletChart.valueField !== '') {
      fill =
        bulletChart.theme.indexOf('Dark') > -1
          ? 'white'
          : bulletChart.valueFill
          ? bulletChart.valueFill
          : 'black';
      const shape: LegendShape =
        bulletChart.orientation === 'Vertical' ? 'TargetRect' : 'ActualRect';
      this.legendCollections.push(
        new LegendOptions(
          'Actual',
          fill,
    ...
    if (bulletChart.dataSource !== null && bulletChart.targetField !== '') {
      fill =
        bulletChart.theme.indexOf('Dark') > -1
          ...
          : 'black';
      const shape: LegendShape =
        bulletChart.orientation === 'Vertical' ? 'ActualRect' : 'TargetRect';
      for (
        let i: number = 0;
        i < Object.keys(bulletChart.dataSource).length;
        i++
      ) {
        if (
          isNullOrUndefined(
            bulletChart.dataSource[i as number][bulletChart.targetField].length
          ) ||
          bulletChart.dataSource[i as number][bulletChart.targetField]
            .length === 1
        ) {
          while (i === 0) {
            this.legendCollections.push(
          ...
          while (i === 0) {
            for (let i: number = 0; i < targetTypeLength; i++) {
              ...
              targetType[i as number] =
                targetType[i as number] === 'Rect'
                  ? bulletChart.orientation === 'Vertical'
                    ? 'ActualRect'
                    : 'TargetRect'
                  : targetType[i as number];
              targetType[i as number] =
                targetType[i as number] === 'Cross'
                  ? 'Multiply'
                  : targetType[i as number];
              this.legendCollections.push(
                new LegendOptions(
                  'Target_' + i,
                  fill,
                  <LegendShape>targetType[i as number],
                  this.chart.legendSettings.visible,
                  null,
                  '',
                  null,
  ...
  public getLegendBounds(
    ...
    for (const bulletLegendOption of this.legendCollections) {
      ...
      bulletLegendOption.textSize = measureText(
        bulletLegendOption.text,
        legend.textStyle,
        this.chart.themeStyle.legendLabelFont
      );
      if (bulletLegendOption.render && bulletLegendOption.text !== '') {
        ...
        legendWidth =
          shapeWidth +
          shapePadding +
          bulletLegendOption.textSize.width +
          padding;
        legendRowWidth = legendRowWidth + legendWidth;
        if (
          bulletLegendBounds.width < padding + legendRowWidth ||
          this.isVertical
        ) {
          maximumWidth = Math.max(
            maximumWidth,
            legendRowWidth + padding - (this.isVertical ? 0 : legendWidth)
          );
          ...
          legendRowCount++;
          columnHeight =
            legendRowCount * (this.maxItemHeight + padding) + padding;
        }
      }
    }
    columnHeight = Math.max(
      columnHeight,
      this.maxItemHeight + padding + padding
    );
    this.isPaging = bulletLegendBounds.height < columnHeight;
    this.totalPages = legendRowCount;
    if (render) {
      this.setBounds(
        Math.max(legendRowWidth + padding, maximumWidth),
        columnHeight,
        legend,
        bulletLegendBounds
      );
  ...
  public getRenderPoint(
    bulletLegendOption: LegendOptions,
    start: ChartLocation,
    textPadding: number,
    ...
    const availwidth: number = !this.isRtlEnable
      ...
      : bulletLegendOption.location.x -
        ...
        this.legend.shapeWidth / 2 -
        this.legendBounds.x;
    bulletLegendOption.text = textTrim(
