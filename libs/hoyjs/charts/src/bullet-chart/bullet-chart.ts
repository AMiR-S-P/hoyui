  ...
  implements INotifyPropertyChanged
{
   ...
   */
  @Collection<RangeModel>(
    [
      { end: null, opacity: 1, color: '' },
      { end: null, opacity: 1, color: '' },
      { end: null, opacity: 1, color: '' },
    ],
    Range
  ...
  private initPrivateValues(): void {
    ...
    if (this.element.id === '') {
      const collection: number =
        document.getElementsByClassName('e-BulletChart').length;
  ...
  private setTheme(): void {
    ...
    if (
      (this.targetColor === null ||
        this.targetColor === '#191919' ||
        this.valueFill == null) &&
      (this.theme.indexOf('Fluent') > -1 ||
        this.theme.indexOf('Bootstrap5') > -1)
    ) {
      this.valueFill = !this.valueFill
        ? this.theme === 'FluentDark'
          ? '#797775'
    ...
    if (
      (this.targetColor === null ||
        this.targetColor === '#191919' ||
        this.valueFill == null) &&
      (this.theme.indexOf('Material3') > -1 ||
        this.theme.indexOf('Bootstrap5') > -1)
    ) {
      this.valueFill = !this.valueFill
        ? this.theme === 'Material3Dark'
          ? '#938F99'
          : this.theme === 'Bootstrap5Dark'
          ? '#343A40'
          : '#79747E'
        : this.valueFill;
      this.targetColor =
    ...
    if (
      !document.getElementById(this.element.id + 'Keyboard_bullet_chart_focus')
    ) {
      const style: HTMLStyleElement = document.createElement('style');
      style.setAttribute(
        'id',
        (<HTMLElement>this.element).id + 'Keyboard_bullet_chart_focus'
      );
      style.innerText =
        '.e-bullet-chart-focused:focus,' +
        ...
        'text[id*=_BulletChartTitle]:focus-visible, text[id*=_BulletChartSubTitle]:focus-visible, rect[id*=_svg_FeatureMeasure_]:focus-visible, g[id*=_chart_legend_g_]:focus-visible {outline: 1.5px ' +
  ...
  private findRange(): void {
    ...
    if (!this.maximum) {
      ...
      for (let i: number = 0; i < this.ranges.length; i++) {
        this.maximum =
          this.maximum > this.ranges[i as number].end
            ? this.maximum
            : this.ranges[i as number].end;
      }
    }
    if (this.maximum === null) {
      if (!isNullOrUndefined(this.dataSource)) {
        for (let i: number = 0; i < Object.keys(this.dataSource).length; i++) {
          if (
            this.dataSource[i as number][this.targetField] >
            this.dataSource[i as number][this.valueField]
          ) {
            this.maximum =
              this.maximum > this.dataSource[i as number][this.targetField]
                ? this.maximum + this.interval
                : this.dataSource[i as number][this.targetField] +
                  this.interval;
          } else {
            this.maximum =
              this.maximum > this.dataSource[i as number][this.valueField]
                ? this.maximum + this.interval
    ...
    if (!this.interval) {
      this.interval = this.calculateNumericNiceInterval(
  ...
  protected getActualDesiredIntervalsCount(availableSize: Size): number {
    ...
    let desiredIntervalsCount: number =
      (this.orientation === 'Horizontal' ? 0.533 : 1) * 3;
  ...
  protected calculateNumericNiceInterval(delta: number): number {
    const actualDesiredIntervalsCount: number =
      this.getActualDesiredIntervalsCount(this.availableSize);
    let niceInterval: number = delta / actualDesiredIntervalsCount;
    const minInterval: number = Math.pow(
      10,
      Math.floor(logBase(niceInterval, 10))
    );
  ...
  private renderBulletElements(): void {
    const scaleGroup: Element = this.renderer.createGroup({
    ...
    const size: number =
      this.orientation === 'Horizontal'
        ...
        : this.initialClipRect.height;

    const intervalValue: number =
      size / ((this.maximum - this.minimum) / this.interval);
  ...
  private renderBulletLegend(): void {
    if (
      this.bulletChartLegendModule &&
      this.bulletChartLegendModule.legendCollections.length
    ) {
  ...
  private calculatePosition(): void {
    ...
    if (this.title) {
      this.titleCollections = getTitle(
        this.title,
        this.titleStyle,
        this.titleStyle.maximumTitleWidth,
        this.enableRtl,
        this.themeStyle.titleFont
      );
      titleHeight =
        measureText(this.title, this.titleStyle, this.themeStyle.titleFont)
          .height *
          this.titleCollections.length +
        padding;
      for (const titleText of this.titleCollections) {
        ...
        maxTitlteWidth =
          titleSize.width > maxTitlteWidth ? titleSize.width : maxTitlteWidth;
        maxTitlteHeight =
          titleSize.height > maxTitlteHeight
            ? titleSize.height
            : maxTitlteHeight;
      }
      maxVerticalTitlteHeight += maxTitlteHeight;
      this.subTitleCollections = getTitle(
        this.subtitle,
        this.subtitleStyle,
        this.titleStyle.maximumTitleWidth,
        this.enableRtl
      ...
      if (this.subtitle) {
        ...
        subTitleHeight =
          measureText(
            ...
            this.subTitleCollections.length +
          padding;
        maxVerticalTitlteHeight += maxTitlteHeight;
      }
    }
    this.maxTitleSize = new Size(
      maxTitlteWidth,
      this.orientation === 'Vertical'
        ? maxVerticalTitlteHeight
        : maxTitlteHeight
    );
    ...
    if (this.bulletChartLegendModule) {
      this.bulletChartLegendModule.calculateLegendBounds(
  ...
  public getBulletBounds(
    ...
    const labelSpace: number =
      this.labelPosition === this.tickPosition ? padding : 0;
    const tickSize: number =
      this.tickPosition === 'Inside' ? 0 : this.majorTickLines.height;
    ...
    const formatted: number = measureText(
      ...
      this.labelStyle,
      this.themeStyle.axisLabelFont
    ).width;
    let categoryLabelSize: number;
    if (this.orientation === 'Horizontal') {
      categoryLabelSize = this.maxLabelSize.width;
      topAxisLabel = this.opposedPosition
        ? tickSize + labelSize + labelSpace
        : 0;
      bottomAxisLabel = !this.opposedPosition
        ? tickSize + labelSize + labelSpace
        : 0;
      ...
      rightAxisLabel +=
        this.opposedPosition && this.labelPosition !== 'Inside' ? formatted : 0;
      leftAxisLabel = !this.opposedPosition ? tickSize + labelSpace : 0;
      leftAxisLabel +=
        !this.opposedPosition && this.labelPosition !== 'Inside'
          ? formatted
          : 0;
      topCategory =
        categoryLabelSize && enableRtl ? categoryLabelSize + padding : 0;
      bottomCategory =
        categoryLabelSize && !enableRtl ? categoryLabelSize + padding : 0;
    }
  ...
  public getMaxLabelWidth(): Size {
    ...
    if (!isNullOrUndefined(this.dataSource)) {
      for (
        let i: number = 0, len: number = Object.keys(this.dataSource).length;
        i < len;
        i++
      ) {
        label = measureText(
          this.dataSource[i as number][this.categoryField] || '',
  ...
  private calculateVisibleElements(): void {
    ...
    this.visibleRanges = [];
    for (
      let i: number = 0, len: number = rangeCollection.length;
      i < len;
      i++
    ) {
  ...
  private renderBulletChartTitle(): void {
    ...
    const subTitleSize: Size = this.subtitle
      ? measureText(
          this.subtitle,
          this.subtitleStyle,
          this.themeStyle.subTitleFont
    ...
    if (this.title) {
      if (this.orientation === 'Horizontal') {
        ...
        switch (this.titlePosition) {
          ...
          case 'Right':
            x =
              this.availableSize.width - margin.right - elementSize.height / 3;
            anchor =
              alignment === 'Far'
                ? 'start'
                : alignment === 'Near'
      ...
      const options: TextOption = new TextOption(
        this.element.id + '_BulletChartTitle',
        x,
        y,
        anchor,
        this.titleCollections,
      ...
      const element: Element = textElement(
        ...
        options,
        this.titleStyle,
        this.titleStyle.color || this.themeStyle.titleFont.color,
        this.svgObject,
        null,
        null,
        null,
  ...
  private renderDataLabel(): void {
    ...
    if (this.dataLabel.enable) {
      for (
        ...
        labelText = isCustomFormat
          ? format.replace('{value}', this.format(labelText))
          : labelText;
        const labelSize: Object = measureText(
          labelText,
          this.dataLabel.labelStyle,
          this.themeStyle.axisLabelFont
        );
        ...
        if (this.orientation === 'Horizontal') {
          ...
          if (
            y + textHeight / 2 >
            this.initialClipRect.height + this.initialClipRect.y
          ) {
            y = y - textHeight / 3;
          }
        }
        ...
        textElement(
          ...
          labelOptions,
          this.dataLabel.labelStyle,
          this.dataLabel.labelStyle.color ||
            this.themeStyle.dataLabelFont.color,
          this.svgObject,
          null,
          null,
          null,
  ...
  private findTextAlignment(
    featureBounds: IFeatureBarBounds,
    textWidth: number,
    alignment: string
  ): number {
    ...
    switch (alignment) {
      ...
      case 'Near':
        x =
          ...
          (this.orientation === 'Horizontal'
            ? this.enableRtl
              ? featureBounds.width - elementSpacing / 2
              : elementSpacing / 2
            : this.enableRtl
            ? elementSpacing
            : featureBounds.width);
      ...
      case 'Far':
        x =
          ...
          (this.orientation === 'Horizontal'
            ? (this.enableRtl
                ? this.type === 'Rect'
                  ? textWidth + elementSpacing
                  : -elementSpacing
                : featureBounds.width) +
              (this.type === 'Rect' ? -elementSpacing / 2 : elementSpacing / 2)
            : (this.enableRtl
                ? featureBounds.width +
  ...
  private renderBulletChartSubTitle(
    ...
    const subTitleOptions: TextOption = new TextOption(
      this.element.id + '_BulletChartSubTitle',
      x,
      y,
      anchor,
      this.subTitleCollections,
    ...
    const element: Element = textElement(
      ...
      subTitleOptions,
      this.subtitleStyle,
      this.subtitleStyle.color || this.themeStyle.subTitleFont.color,
      this.svgObject,
      null,
      null,
      null,
  ...
  private calculateAvailableSize(bulletChart: BulletChart): void {
    const containerWidth: number =
      this.element.clientWidth || this.element.offsetWidth || 200;
    const height: number =
      this.orientation === 'Vertical'
        ? 450
  ...
  private bulletMouseMove(e: PointerEvent): void {
    ...
    if (
      targetClass !== this.svgObject.id + '_FeatureMeasure' ||
      this.svgObject.id + '_ComparativeMeasure'
    ) {
      if (!isNullOrUndefined(this.dataSource)) {
        for (let i: number = 0; i < Object.keys(this.dataSource).length; i++) {
          document
            .getElementById(this.svgObject.id + '_FeatureMeasure_' + i)
            .setAttribute('opacity', '1');
          document
            .getElementById(this.svgObject.id + '_ComparativeMeasure_' + i)
            .setAttribute('opacity', '1');
        }
      }
    }
    if (!this.isTouchEvent(e)) {
      ...
      if (this.bulletTooltipModule) {
        this.bulletTooltipModule._elementTooltip(
          e,
          targetClass,
          targetId,
          this.mouseX
        );
        this.bulletTooltipModule._displayTooltip(
          e,
          targetClass,
          targetId,
          this.mouseX,
  ...
  private setPointMouseXY(pageX: number, pageY: number): void {
    const svgClientRect: ClientRect = getElement(
      this.svgObject.id
    ).getBoundingClientRect();
    const elemntClientRect: ClientRect = this.element.getBoundingClientRect();
    this.mouseX =
      pageX -
      elemntClientRect.left -
      Math.max(svgClientRect.left - elemntClientRect.left, 0);
    this.mouseY =
      pageY -
      elemntClientRect.top -
      Math.max(svgClientRect.top - elemntClientRect.top, 0);
  }
  /**
  ...
  public bulletMouseLeave(e: PointerEvent): void {
    if (!this.isTouchEvent(e)) {
      const tooltipDiv: HTMLElement = document.getElementById(
        '.tooltipDiv' + this.element.id
      );
      if (tooltipDiv) {
        if ((this as any).isReact) {
          this.clearTemplate();
  ...
  public destroy(): void {
    ...
    const element: HTMLElement = document.getElementById(
      this.element.id + 'Keyboard_bullet_chart_focus'
    );
    if (element) {
      element.remove();
    }
