...
import { Animation, AnimationOptions, animationMode } from '@hoyui/base';
...
export class ScaleGroup {
  ...
  public drawScaleGroup(scaleGroup: Element): number[] {
    const rangeGroup: Element = this.bulletChart.renderer.createGroup({
    ...
    let locX: number =
      initialRect.x +
      (enableRtl && bullet.orientation === 'Horizontal'
        ? initialRect.width
        : 0);
    let locY: number =
      initialRect.y +
      (!enableRtl && bullet.orientation === 'Vertical'
        ? initialRect.height
        : 0);
    let area: number = 0;
    ...
    for (let i: number = 0; i < ranges.length; i++) {
      ...
      rect = new RectOption(
        ...
        ranges[i as number].color ||
          this.bulletChart.themeStyle.rangeStrokes[i as number]['color'],
        { width: 1 },
        ranges[i as number].opacity,
        new Rect(
          locX,
          locY,
          bullet.orientation === 'Horizontal' ? area : fillRange,
  ...
  private renderCommonFeatureBar(
    ...
    const initialBoundsStart: number = isHorizontal
      ? bulletChart.initialClipRect.y + bulletChart.initialClipRect.height
      : bulletChart.initialClipRect.x;
    let lPoint: number;
    const featueGroup: Element = bulletChart.renderer.createGroup({
      id: bulletChart.svgObject.id + '_featureGroup',
    });
    let data: Object;
    const featureBarSize: number =
      (isHorizontal
        ? bulletChart.initialClipRect.height
        : bulletChart.initialClipRect.width) / dataCount;
    ...
    for (let i: number = 0; i < dataCount; i++) {
      ...
      if (isHorizontal) {
        lPoint =
          initialBoundsStart -
          featureBarSize * i -
          (featureBarSize + bulletChart.valueHeight) / 2;
      } else {
        lPoint =
          initialBoundsStart +
          featureBarSize * i +
          featureBarSize / 2 -
          bulletChart.valueHeight / 2;
      }
      bounds = this.calculateFeatureMeasureBounds(
        data[bulletChart.valueField],
        categoryValue,
        isHorizontal
      );
      ...
      if (bounds) {
        const svgRect: Element = isHorizontal
          ? this.featureBar(bounds.pointX, lPoint, bounds.width, i)
        ...
        this.featureBarBounds[i as number] = {
          x: bounds.pointX,
          y: lPoint,
          width: bounds.width,
        ...
        if (!isNullOrUndefined(categoryValue)) {
          const categoryTextSize: Size = measureText(
            categoryValue,
            bulletChart.categoryLabelStyle,
            bulletChart.themeStyle.axisLabelFont
          );
          const categorySize: number = isHorizontal
            ? categoryTextSize.width
            : categoryTextSize.height;
          const initialRect: Rect = bulletChart.initialClipRect;
          let x: number;
          let categoryOptions: TextOption;
          if (isHorizontal) {
            x = bulletChart.enableRtl
              ? initialRect.x + initialRect.width + padding + categorySize / 2
              : initialRect.x - padding - categorySize / 2;
            categoryOptions = this.drawcategory(x, lPoint, categoryValue);
          } else {
            x = bulletChart.enableRtl
              ? initialRect.y - padding - categorySize / 2
              : initialRect.y + initialRect.height + padding + categorySize / 2;
            categoryOptions = this.drawcategory(
              lPoint + bulletChart.valueHeight / 2,
              x,
              categoryValue
            );
          }
          bulletChart.categoryLabelStyle.fontFamily =
            bulletChart.categoryLabelStyle.fontFamily ||
            bulletChart.themeStyle.axisLabelFont.fontFamily;
          textElement(
            categoryOptions,
            bulletChart.categoryLabelStyle,
            bulletChart.categoryLabelStyle.color ||
              bulletChart.themeStyle.axisLabelFont.color,
            this.scaleSettingsGroup
          );
        }
      }
      if (
        (bulletChart.animation.enable && animationMode !== 'Disable') ||
  ...
  private featureBar(
    ...
    const featureBarOptions: RectOption = new RectOption(
      ...
      this.bulletChart.dataSource[i as number][this.bulletChart.valueFill] ||
      ...
      0,
      '',
      this.bulletChart.valueBorder.dashArray
    );
    const svgRect: Element =
      this.bulletChart.renderer.drawRectangle(featureBarOptions);
    svgRect.setAttribute(
      'class',
      this.bulletChart.svgObject.id + '_FeatureMeasure'
    );
    ...
    svgRect.setAttribute(
      'aria-label',
      this.bulletChart.title +
        ', value: ' +
        this.bulletChart.dataSource[i as number].value +
        ', target: ' +
        this.bulletChart.dataSource[i as number].target
    );
    return svgRect;
  }
  ...
  private drawcategory(
    ...
    const fontsize: number = parseInt(
      this.bulletChart.categoryLabelStyle.size,
      10
    );
  ...
  private renderCommonComparativeSymbol(
    ...
    const comparativeGroup: Element = bulletChart.renderer.createGroup({
    ...
    const pointX: number = isHorizontal
      ? rect.x - targetWidth / 2
      : rect.y + rect.height;
    let temp: number;
    let values: number[] = [];
    ...
    const featureBarSize: number =
      (isHorizontal ? rect.height : rect.width) / dataCount;
    ...
    for (let k: number = 0; k < dataCount; k++) {
      ...
      for (let i: number = 0; i < values.length; i++) {
        ...
        if (values[i as number] >= minimum && values[i as number] <= maximum) {
          ...
          temp =
            scaleLength / (delta / (delta - (maximum - values[i as number])));
          if (isHorizontal) {
            x1 = pointX + (bulletChart.enableRtl ? scaleLength - temp : temp);
          } else {
            x1 = pointX - (bulletChart.enableRtl ? scaleLength - temp : temp);
          }
          svgElement = this.getTargetElement(
            targetType,
            isHorizontal,
            x1,
            y1,
  ...
  private getTargetElement(
    ...
    const strokeWidth: number =
      targetType === 'Cross' ? bulletChart.targetWidth - 1 : 1;
    const size: number =
      targetType === 'Circle'
        ? bulletChart.targetWidth - 1
        : bulletChart.targetWidth;
    ...
    const targetColor: string =
      bulletChart.dataSource[k as number][bulletChart.targetColor] ||
      bulletChart.targetColor;
    if (targetType === 'Rect') {
      shapeObject = isHorizontal
        ? this.compareMeasure(x1, y1, y2, k, value)
        : this.compareVMeasure(y1, y2, x1, k);
      shapeElement = bulletChart.renderer.drawLine(shapeObject);
    } else if (targetType === 'Circle') {
      shapeObject = new CircleOption(
        id,
        targetColor,
        { width: 1, color: targetColor || 'black' },
        1,
        lx,
        ly,
        size
  ...
  private compareMeasure(
    ...
    const compareMeasureOptions: object = {
      ...
      x1:
        value === bulletChart.maximum
          ? x1 - bulletChart.targetWidth / 2
          : value === bulletChart.minimum
          ? x1 + bulletChart.targetWidth / 2
          : x1,
      y1: y1,
      x2:
      ...
      stroke:
        bulletChart.dataSource[i as number][bulletChart.targetColor] ||
        bulletChart.targetColor ||
        'black',
    };
  ...
  private calculateFeatureMeasureBounds(
    ...
    if (value >= min) {
      ...
      let width: number;
      const loc: number = isHorizontal
        ? bulletChart.initialClipRect.x
        : bulletChart.initialClipRect.y;
      const scaleLength: number = isHorizontal
        ? bulletChart.initialClipRect.width
        : bulletChart.initialClipRect.height;
      const delta: number = bulletChart.maximum - bulletChart.minimum;
      const valueDiff: number = bulletChart.maximum - value;
      const orientation: string =
        (!bulletChart.enableRtl ? 'forward' : 'backward') +
        this.scaleOrientation.toLowerCase();
      categoryValue = isNullOrUndefined(categoryValue) ? '' : categoryValue;
      const stringLength: number = measureText(
        categoryValue.toString(),
        bulletChart.labelStyle,
        this.bulletChart.themeStyle.axisLabelFont
      ).width;
      switch (orientation) {
        ...
        case 'backwardvertical':
          ...
          width =
            pointX + width < loc + scaleLength
              ? width
              : loc + scaleLength - pointX;
          lastPointX =
            loc -
            (orientation === 'forwardhorizontal'
              ? stringLength / 2 + 5
              : this.labelOffset);
          break;
        default:
          pointX =
            loc + (scaleLength - scaleLength / (delta / (delta - valueDiff)));
          width =
            min > 0
              ? scaleLength / (delta / (delta - valueDiff))
              : scaleLength / (delta / value);
          if (value < 0) {
            width = Math.abs(width);
            pointX -= width;
          }
          ...
          lastPointX =
            loc +
            scaleLength +
            (orientation === 'backwardhorizontal' ? stringLength / 2 + 5 : 5);
          break;
      }
  ...
  public doValueBarAnimation(): void {
    ...
    const animateDuration: number = this.bulletChart.animateSeries
      ? this.bulletChart.animation.duration
    ...
    let valueX: number;
    let valueY: number;
    const elementBarHeight: number =
      valueBarElement.getBoundingClientRect().height;
    ...
    new Animation({}).animate(valueBarElement, {
      duration:
        animateDuration === 0 && animationMode === 'Enable'
          ? 1000
          : animateDuration,
      ...
      progress: (args: AnimationOptions): void => {
        if (args.timeStamp >= args.delay) {
          ...
          valueActual = effectType(
            args.timeStamp - args.delay,
            0,
            elementBarWidth,
            args.duration
          );
          valueBarElement.setAttribute(
            'transform',
            'translate(' +
              centerX +
              ' ' +
  ...
  public doTargetBarAnimation(index: number): void {
    ...
    let centerX: number;
    let centerY: number;
    const targetBarelement: HTMLElement = <HTMLElement>(
      this.comparative[index as number]
    );
    if (!targetBarelement) {
      return null;
    }
    if (this.bulletChart.orientation === 'Horizontal' && targetBarelement) {
      y =
        parseFloat(targetBarelement.getAttribute('y1')) +
        parseFloat(targetBarelement.getAttribute('y2'));
  ...
  private animateRect(
    ...
    const duration: number = this.bulletChart.animateSeries
      ? this.bulletChart.animation.duration
      : option.duration;
