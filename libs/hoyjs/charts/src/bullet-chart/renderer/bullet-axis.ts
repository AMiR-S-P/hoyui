...
import { measureText, TextOption, Rect, textElement } from '@hoyui/svg-base';
...
export class BulletChartAxis {
  ...
  public renderXMajorTickLines(intervalValue: number, scale: Element): void {
    ...
    const tickGroup: Element = bullet.renderer.createGroup({
    ...
    const y1: number =
      bullet.initialClipRect.y +
      (bullet.opposedPosition ? 0 : bullet.initialClipRect.height);
    const y2: number =
      y1 +
      (!bullet.opposedPosition
    ...
    let strokeColor: string =
      majorTick.color || bullet.themeStyle.majorTickLineColor;
    let options: Object;
    let condition: boolean;
    const size: number =
      bullet.initialClipRect.x +
      (bullet.enableRtl ? bullet.initialClipRect.width : 0);
    let majorPointX: number =
      bullet.initialClipRect.x +
      majorTick.width / 2 +
      (enableRtl ? bullet.initialClipRect.width : 0);
    for (let i: number = min; i <= max; i += interval) {
      ...
      if (bullet.majorTickLines.useRangeColor) {
        strokeColor = this.bindingRangeStrokes(
          majorPointX -
            (condition ? this.bulletChart.majorTickLines.width / 2 : 0),
          size,
          this.bulletChart.orientation,
          bullet.enableRtl
        );
      }
      options = this.majorTicks(
        majorPointX,
        majorPointX,
        y1,
        y2,
  ...
  public renderYMajorTickLines(intervalValue: number, scale: Element): void {
    ...
    const tickGroup: Element = bulletChart.renderer.createGroup({
    ...
    const x2: number =
      ...
      (!bulletChart.opposedPosition
        ? bulletChart.tickPosition !== 'Inside'
          ? this.majorTickSize
          : -this.majorTickSize
        : bulletChart.tickPosition !== 'Inside'
        ? -this.majorTickSize
    ...
    let strokeColor: string =
      majorTick.color || bulletChart.themeStyle.majorTickLineColor;
    let condition: boolean;
    let options: Object;
    const size: number = rect.y + (!bulletChart.enableRtl ? rect.height : 0);
    let majorPointY: number =
      rect.y + majorTick.width / 2 + (!enableRtl ? rect.height : 0);
    for (let i: number = min; i <= max; i += interval) {
  ...
  private majorTicks(
    ...
    const options: object = {
      ...
      stroke:
        this.bulletChart.majorTickLines.useRangeColor && strokeColor
          ? strokeColor
   ...
   * @returns {void}
   */
  public renderXMinorTickLines(
    intervalValue: number,
    scaleGroup: Element
  ): void {
    const minorTickGroup: Element = this.bulletChart.renderer.createGroup({
    ...
    const minorTicksPerInterval: number =
      this.bulletChart.minorTicksPerInterval;
    ...
    const y2: number =
      ...
      (!bullet.opposedPosition
        ...
        : minorTick);
    let strokeColor: string =
      bullet.minorTickLines.color || bullet.themeStyle.minorTickLineColor;
    let options: object;
  ...
  public renderYMinorTickLines(
    ...
    let majorPointY: number =
      ...
      (!bulletChart.enableRtl ? bulletChart.initialClipRect.height : 0);
    const x1: number =
      bulletChart.initialClipRect.x +
      (!bulletChart.opposedPosition ? 0 : bulletChart.initialClipRect.width);
    const x2: number =
      x1 -
      (!bulletChart.opposedPosition
        ? bulletChart.tickPosition !== 'Inside'
        ...
        : minorTick);
    let strokeColor: string =
      bulletChart.minorTickLines.color ||
      bulletChart.themeStyle.minorTickLineColor;
    let options: object;
    let minorTicks: Element;
    const size: number =
      bulletChart.initialClipRect.y +
    ...
    for (let i: number = min; i < max; i += interval) {
      ...
      for (let j: number = 1; j <= minorTicksPerInterval; j++) {
        if (!this.bulletChart.enableRtl) {
          y =
            majorPointY -
            minorPointY +
            minorPointY / (minorTicksPerInterval + 1);
        } else {
          y =
            majorPointY +
            minorPointY -
      ...
      majorPointY -= this.bulletChart.enableRtl
        ? -intervalValue
        : intervalValue;
    }
  }
  ...
  private minorXTicks(
    ...
    const options: object = {
      ...
      stroke:
        this.bulletChart.minorTickLines.useRangeColor && strokeColor
          ? strokeColor
  ...
  private forwardStrokeBinding(position: number, size: number): string {
    ...
    for (let k: number = 0; k <= bullet.rangeCollection.length - 1; k++) {
      ...
      if (
        position >= previous &&
        position < previous + bullet.rangeCollection[k as number]
      ) {
  ...
  private backwardStrokeBinding(position: number, size: number): string {
    ...
    for (let k: number = 0; k <= bullet.rangeCollection.length - 1; k++) {
      ...
      if (
        Math.round(position) >=
          Math.round(previous - bullet.rangeCollection[k as number]) &&
        position <= previous
      ) {
        return bullet.ranges[k as number].color;
      }
  ...
  public renderXAxisLabels(intervalValue: number, scaleGroup: Element): void {
    const axisLabelGroup: Element = this.bulletChart.renderer.createGroup({
    ...
    const enableRtl: boolean = bullet.enableRtl;
    const tick: number =
      (bullet.tickPosition === bullet.labelPosition
        ? bullet.majorTickLines.height
        : 0) +
      padding * 2;
    let y: number =
      bullet.initialClipRect.y +
      (bullet.opposedPosition
        ? bullet.labelPosition === 'Inside'
          ? tick
          : -tick
        : bullet.initialClipRect.height +
          (bullet.labelPosition === 'Inside' ? -tick : tick));
    let x: number =
      bullet.initialClipRect.x + (enableRtl ? bullet.initialClipRect.width : 0);
    const min: number = bullet.minimum;
    const max: number = bullet.maximum;
    const interval: number = bullet.interval;
    const localizedText: boolean =
      locale && this.bulletChart.enableGroupSeparator;
    const format: string = this.getFormat(this.bulletChart);
    let strokeColor: string =
      bullet.labelStyle.color || bullet.themeStyle.axisLabelFont.color;
    let condition: boolean;
    ...
    for (let i: number = min; i <= max; i += interval) {
      ...
      if (bullet.labelStyle.useRangeColor) {
        strokeColor = this.bindingRangeStrokes(
          x - (condition ? this.bulletChart.majorTickLines.width / 2 : 0),
          size,
          this.bulletChart.orientation,
          bullet.enableRtl
        );
      ...
      text = localizedText
        ...
        : this.formatValue(this, isCustomFormat, format, i);
      const labelOptions: TextOption = this.labelXOptions(x, y, text, i);
      this.bulletChart.labelStyle.fontFamily =
        this.bulletChart.labelStyle.fontFamily ||
  ...
  public renderYAxisLabels(intervalValue: number, scaleGroup: Element): void {
    ...
    const enableRtl: boolean = bulletChart.enableRtl;
    const tick: number =
      (bulletChart.tickPosition === bulletChart.labelPosition
        ? bulletChart.majorTickLines.height
        : 0) +
      padding * 2;
    let y: number =
      bulletChart.initialClipRect.y +
      (!enableRtl ? bulletChart.initialClipRect.height : 0);
    const x: number =
      bulletChart.initialClipRect.x +
      (!bulletChart.opposedPosition
        ? bulletChart.labelPosition === 'Inside'
          ? tick + padding * 2
          : -tick
    ...
    for (let i: number = min; i <= max; i += interval) {
      ...
      const labelOptions: TextOption = this.labelXOptions(
        x - (!this.bulletChart.opposedPosition ? labelWidth : -labelWidth),
        y,
        text,
        i
      );
  ...
  public formatValue(
    ...
    return isCustom
      ? format.replace('{value}', axis.format(tempInterval))
