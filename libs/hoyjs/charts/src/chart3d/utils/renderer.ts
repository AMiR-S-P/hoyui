import { Rect, Size, measureText } from '@hoyui/svg-base';
...
export class WallRenderer {
  ...
  public update3DWall(chart: Chart3D): void {
    ...
    for (let i: number = 0; i < chart.axisCollections.length; i++) {
      ...
      const opposedPosition: boolean =
        axis.opposedPosition || axis.isAxisOpposedPosition;
  ...
  private updateTopWall(chart: Chart3D): void {
    ...
    const topLeftFrontVector: Chart3DVector = chart.vector.vector3D(
    ...
    const topSideWallPlans: Chart3DPolygon[] = chart.polygon.createBox(
      topLeftFrontVector,
      bottomRightBackVector,
      chart,
      0,
      chart.wallColor || chart.themeStyle.leftWallColor,
      chart.wallColor || chart.themeStyle.leftWallColor,
      0,
    ...
    for (let i: number = 0; i < topSideWallPlans.length; i++) {
      chart.polygon.transform(
        chart.matrixObj.tilt(Math.PI / 2),
        topSideWallPlans[i as number]
      );
    }
  }
  ...
  private updateRightWall(chart: Chart3D): void {
    ...
    const rightRect: Chart3DWallRect = {
      left: -chart.depth,
      top: areaBounds.y,
    ...
    const rightSideWallPlans: Chart3DPolygon[] = chart.polygon.createBox(
      topLeftFrontVector,
      bottomRightBackVector,
      chart,
      0,
    ...
    for (let i: number = 0; i < rightSideWallPlans.length; i++) {
      chart.polygon.transform(
        chart.matrixObj.turn(-Math.PI / 2),
        rightSideWallPlans[i as number]
      );
    }
  }
  ...
  private updateBackWall(chart: Chart3D): void {
    ...
    const bottomRightBackVector: Chart3DVector = chart.vector.vector3D(
      ...
      chart.depth === 0 ? 1.5 : chart.depth
    );
    chart.polygon.createBox(
      topLeftFrontVector,
      bottomRightBackVector,
      chart,
      0,
      chart.wallColor || chart.themeStyle.backWallColor,
      chart.wallColor || chart.themeStyle.backWallColor,
      0,
      chart.theme.indexOf('Fluent2') > -1
        ? 0.3
  ...
  private updateLeftWall(chart: Chart3D): void {
    const areaBounds: Rect = chart.chartAxisLayoutPanel.seriesClipRect;
    const leftRect: Chart3DWallRect = {
      left: -chart.depth,
      top: areaBounds.y,
      bottom: areaBounds.height + areaBounds.y,
      right: 0,
    ...
    const bottomRightBackVector: Chart3DVector = chart.vector.vector3D(
      leftRect.right,
      leftRect.bottom,
      offset - chart.wallSize
    );
    const leftSideWallPlans: Chart3DPolygon[] = chart.polygon.createBox(
      topLeftFrontVector,
      bottomRightBackVector,
      chart,
      0,
  ...
  private updateBottomWall(chart: Chart3D): void {
    ...
    const bottomSideWallPlans: Chart3DPolygon[] = chart.polygon.createBox(
      bottomRightBackVector,
      topLeftFrontVector,
      chart,
      0,
    ...
    for (let i: number = 0; i < bottomSideWallPlans.length; i++) {
      chart.polygon.transform(
        chart.matrixObj.tilt(Math.PI / 2),
        bottomSideWallPlans[i as number]
      );
    }
  }
}
...
export class AxisRenderer {
  ...
  private drawAxisTitle(
    ...
    if (axis.title) {
      const font: Chart3DTextFontModel = {
        ...
        fontWeight:
          axis.titleStyle.fontWeight ||
          chart.themeStyle.axisTitleFont.fontWeight,
        fontStyle:
          axis.titleStyle.fontStyle || chart.themeStyle.axisTitleFont.fontStyle,
        fontFamily:
          axis.titleStyle.fontFamily ||
          chart.themeStyle.axisTitleFont.fontFamily,
      ...
      const transform: Chart3DBasicTransform =
        chart.transform3D.transform3D(size);
      ...
      if (orientation === 'horizontal') {
        ...
        if (axis.titleRotation) {
          padding =
            ...
            elementSpacing +
            axis.labelPadding +
            axis.titleSize.height / 2;
        } else {
          padding =
            axis.titlePadding + titlesize + axis.labelPadding + elementSpacing;
        }
        ...
        const y1: number = opposedPosition
          ? axis.rect.y - data.location.y
          : data.location.y + axis.rect.y;
        ...
        element.font.color = element.font.color
          ? element.font.color
          : chart.themeStyle.axisTitle;
        element.font.fontFamily = element.font.fontFamily
          ? element.font.fontFamily
          : chart.themeStyle.axisTitleFont.fontFamily;
        chart.graphics.addVisual(
          chart.polygon.createTextElement(
            chart.vector.vector3D(x1, y1, 0),
            element,
            10,
            10
          ),
        ...
        if (axis.titleRotation) {
          padding =
            axis.labelPadding + axis.titlePadding + axis.titleSize.width / 2;
        } else {
          padding = axis.titlePadding + axis.labelPadding;
        }
        const x1: number = opposedPosition
          ? axis.rect.x +
            (elementSpacing +
        ...
        const y1: number =
          data.location.y +
          (axis.rect.y + axis.rect.height) +
          (axis.rect.height / 2) * -1;
        const element: Chart3DLabelElement = {
  ...
  private multipleRows(
    ...
    for (let i: number = length - 1; i >= 0; i--) {
      ...
      if (isMultiRows) {
        ...
        currentLabel.index =
          currentLabel.index > label.index
            ? currentLabel.index
            : label.index + 1;
      } else {
        currentLabel.index =
          store.indexOf(label.index) > -1 ? currentLabel.index : label.index;
      }
    }
  }
  ...
  private drawAxisLabel(
    ...
    for (let i: number = 0; i < labelsCount; i++) {
      if (!isNullOrUndefined(axis.visibleLabels[i as number].text)) {
        ...
        let y1: number = 0;
        let pointX: number;
        axis.visibleLabels[i as number].originalText = axis.visibleLabels[
          i as number
        ].text as string;
        let textAnchor: string;

        const textSize: Size = measureText(
          axis.visibleLabels[i as number].text as string,
          axis.labelStyle,
          chart.themeStyle.axisLabelFont
        );
        let value: number =
          (axis.visibleLabels[i as number].value - axis.visibleRange.min) /
          axis.visibleRange.delta;
        value = axis.isInversed || axis.isAxisInverse ? 1 - value : value;
        value = isNaN(value) ? 0 : value;

        if (axis.orientation.toLowerCase() === 'horizontal') {
          x1 =
            Math.round(axis.rect.width * value) +
            areaBounds.x +
            axis.plotOffset;
          y1 = opposedPosition
            ? y -
          ...
          y1 = Math.round(
            axis.plotOffset +
              axis.rect.y +
              textSize.height / 4 +
              axis.rect.height * (1 - value)
          );
          let padding: number = 0;
          if (
            axis.labelRotation === 90 ||
            axis.labelRotation === -90 ||
            axis.labelRotation === 270 ||
            axis.labelRotation === -270
          ) {
        ...
        const maxWidth: number =
          axis.rect.width / axis.visibleLabels.length - 5;
        const label: Chart3DLabelRect = labels[i as number];

        if (
          ((label.x - label.size.width / 2 < axis.rect.x && i === 0) ||
            (label.x + label.size.width / 2 > axis.rect.x + axis.rect.width &&
              i === axis.visibleLabels.length - 1)) &&
          axis.labelIntersectAction !== 'Trim' &&
          axis.labelIntersectAction.indexOf('wrap') < 0
        ) {
          if (axis.edgeLabelPlacement === 'Hide') {
            ...
            if (i === 0) {
              ...
              label.x = x1 =
                axis.rect.x + axis.rect.width - label.size.width / 2;
            }
          }
        }
        ...
        if (axis.orientation.toLowerCase() === 'horizontal') {
          if (axis.labelRotation) {
            ...
            const rotatedSize: Size = rotateTextSize(
              axis.labelStyle,
              axis.visibleLabels[i as number].text as string,
              angleValue,
              chart,
            ...
            if (axis.labelIntersectAction === 'Trim') {
              axis.visibleLabels[i as number].text = this.textTrim(
                maxWidth,
                axis.visibleLabels[i as number].text as string,
                axis.labelStyle,
                chart.themeStyle.axisLabelFont
              ...
              axis.angle &&
              (axis.labelIntersectAction === 'Rotate45' ||
                axis.labelIntersectAction === 'Rotate90')
            ) {
              const rotatedSize: Size = rotateTextSize(
                axis.labelStyle,
                axis.visibleLabels[i as number].text as string,
              ...
              this.multipleRows(
                i,
                pointX,
                axis.visibleLabels[i as number],
                axis,
                chart.themeStyle.axisLabelFont
              );
              y1 = axis.visibleLabels[i as number].index
                ? y1 +
                  axis.visibleLabels[i as number].index * (textSize.height + 5)
                : y1;
            } else if (axis.labelIntersectAction === 'Hide') {
              let isAxisLabelHidden: boolean = false;
              for (let j: number = 0; j < i; j++) {
                if (
                  labels[j as number].x + labels[j as number].size.width / 2 >=
        ...
        element.font.color = element.font.color
          ? element.font.color
          : chart.themeStyle.axisLabel;
        element.font.fontFamily = element.font.fontFamily
          ? element.font.fontFamily
          : chart.themeStyle.axisLabelFont.fontFamily;
        chart.graphics.addVisual(
          chart.polygon.createTextElement(
            chart.vector.vector3D(x1, y1, 0),
            element,
  ...
  private renderTicks3D(
    ...
    for (let i: number = 0; i < labelsCount; i++) {
      if (axis.valueType !== 'DateTimeCategory') {
        labelValue = axis.visibleLabels[i as number]
          ? axis.visibleLabels[i as number].value - ticksbwtLabel
          : axis.visibleLabels[i - 1].value +
            axis.visibleRange.interval -
            ticksbwtLabel;
      } else {
        labelValue = axis.visibleLabels[i as number].value
          ? axis.visibleLabels[i as number].value - ticksbwtLabel
      ...
      let y2: number = 0;
      let value: number =
        (labelValue - axis.visibleRange.min) / axis.visibleRange.delta;
      value = axis.isInversed || axis.isAxisInverse ? 1 - value : value;
      value = isNaN(value) ? 0 : value;
      if (axis.orientation.toLowerCase() === 'horizontal') {
        x2 = x1 =
          Math.round(axis.rect.width * value) + areaBounds.x + axis.plotOffset;
      } else {
        y1 = y2 =
          Math.round(axis.plotOffset + axis.rect.height * (1 - value)) +
          axis.rect.y;
      }
      const position: Chart3DTickPosition = this.calculatePosition3D(
        axis,
        size,
        width,
        x1,
      ...
      if (
        ...
        minorTicks =
          axis.visibleRange.interval / (axis.minorTicksPerInterval + 1);
        for (let k: number = 0; k < axis.minorTicksPerInterval; k++) {
          value = valueToCoefficients(
            axis.visibleLabels[i as number].value + minorTicks * (k + 1),
            axis
          );
          value = isNaN(value) ? 0 : value;
          if (axis.orientation.toLowerCase() === 'horizontal') {
            x1 = x2 = Math.round(
              axis.plotOffset + areaBounds.width * value + areaBounds.x
            );
          } else {
            y1 = y2 =
              Math.round(axis.plotOffset + areaBounds.height * (1 - value)) +
              axis.rect.y;
          }
          const position: Chart3DTickPosition = this.calculatePosition3D(
            axis,
          ...
          line.id =
            chart.element.id + '-' + index + '-minor-tick-lines-' + i + '-' + k;
          chart.graphics.addVisual(
            chart.polygon.createLine(
              line,
              position.x1,
  ...
  private calculatePosition3D(
    ...
    if (axis.orientation.toLowerCase() === 'horizontal') {
      ...
      const screenPositionTop: number = isOpposed
        ? y - chart.wallSize - tickSize
        : y + chart.wallSize - tickSize / 2;
      ...
      const screenPositionLeft: number = isOpposed
        ? x + chart.wallSize
        : x - chart.wallSize - tickSize;
  ...
  private drawGridLines3D(
    ...
    if (orientation.toLowerCase() === 'horizontal') {
      ...
      for (i = 0; i < labelsCount; i++) {
        ...
        const bottom: number =
          areaBounds.y + (!opposedPosition && areaBounds.height);
        const line: Chart3DTickElement = {
          opacity: 1,
        ...
        chart.graphics.addVisual(
          chart.polygon.createLine(line, x1, y1, x2, y2, depth),
          chart
        );
        const parallelLine: Chart3DTickElement = {
        ...
        parallelLine.id =
          chart.element.id + '-' + index + '-parallel-grid-lines-' + i;
        const line3D: Chart3DPolygon = chart.polygon.createLine(
          parallelLine,
          x2,
          0,
          x2,
      ...
      for (let i: number = 0; i < labelsCount; i++) {
        ...
        const depthD: number =
          areaBounds.x + (opposedPosition && areaBounds.width + 1);
        const sideLine: Chart3DTickElement = {
          opacity: line.opacity,
          width: line.width,
          stroke: line.stroke,
          child: line.child,
        ...
        if (
          ...
          minorTicks =
            axis.visibleRange.interval / (axis.minorTicksPerInterval + 1);
          for (let k: number = 0; k < axis.minorTicksPerInterval; k++) {
            const value: number = valueToCoefficients(
              axis.visibleLabels[i as number].value + minorTicks * (k + 1),
              axis
            );
            x1 = areaBounds.x;
