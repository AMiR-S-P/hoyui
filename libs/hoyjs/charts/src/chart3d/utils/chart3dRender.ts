...
import {
  Chart3DPathOptions,
  Chart3DColorFormat,
  Chart3DStringBuilder,
  Chart3DVector,
  Chart3DLocation,
  Chart3DPolygon,
  Chart3DTickElement,
  Chart3DPolyAttributes,
  Chart3DPolyCollections,
  Chart3DLabelElement,
  Chart3DTextOption,
  Chart3DDataElement,
  Chart3DBasicTransform,
...
export class ChartTransform3D {
  ...
  public result(
    ...
    let chartObj: Matrix3D = transform.chartObj
      ? transform.chartObj
      : this.matrixObj;
    if (!chartObj) {
      chartObj = matrixobj;
...
export class Graphics3D {
  ...
  public view(
    ...
    if (arguments.length === 2) {
      ...
      this.drawNode3D(
        chart3DRender.tree[chart.chart3D.id],
        eyeVector,
        panel,
        chart
      );
    ...
    while (isVector) {
      const r: number = vector.vector3DAdd(
        polygonObj.getNormal(
          chart.transform3D.result(chart3DRender.transform),
...
export class BinaryTreeBuilder {
  ...
  public splitPolygon(
    ...
    if (splitPolygon.points != null) {
      ...
      for (let i: number = 0; i < count; i++) {
        ...
        const attributeB: Chart3DPolyAttributes =
        ...
        if (sideB !== sideC && sideB !== 'OnPlane' && sideC !== 'OnPlane') {
          const vectorValue: Chart3DVector = vector.vector3DMinus(
            pointB,
            pointC
          );
          const direction: Chart3DVector = vector.vector3DMinus(
          ...
          const attributeIntersection: Chart3DPolyAttributes =
      ...
      if (frontPartPoints.length !== 0 || backPartPoints.length !== 0) {
        for (let i: number = 0; i < backPartPoints.length - 1; i += 2) {
          const backAttribute1: Chart3DPolyAttributes =
        ...
        for (let i: number = 0; i < frontPartPoints.length - 1; i += 2) {
          const frontAttribute1: Chart3DPolyAttributes =
        ...
        for (let i: number = 0; i < backPartPoints.length - 1; i++) {
          ...
          if (outputs.length > 2) {
            const polygon1: Chart3DPolygon = polygonObj.polygon3D(
              outputs,
              splitPolygon
            );
        ...
        for (let i: number = 0; i < frontPartPoints.length - 1; i++) {
          const backAttribute2: Chart3DPolyAttributes =
            frontPartPoints[i as number];
          ...
          if (inputs.length > 2) {
            const polygon2: Chart3DPolygon = polygonObj.polygon3D(
              inputs,
              splitPolygon
            );
  ...
  private cutOutFrontPolygon(
    polyPoints: Chart3DPolyAttributes[],
    initialVertex: Chart3DPolyAttributes
  ): Chart3DVector[] {
    ...
    while (isVector) {
      ...
      const currentVertexPair: Chart3DPolyAttributes =
      ...
      if (currentVertex.cuttingFrontPoint) {
        if (!currentVertexPair.alreadyCutFront) {
          ...
          const previousVertexOnBack: Chart3DPolyAttributes =
            polyPoints[
              this.getNext(currentVertex.index - 1, polyPoints.length)
            ];
          const nextVertexOnBack: Chart3DPolyAttributes =
            polyPoints[
              this.getNext(currentVertex.index + 1, polyPoints.length)
            ];
        ...
        switch (classificationResult) {
          ...
          case 'Unknown':
            if (
              ...
              (currentPolygon.element.tag === 'line' ||
                currentPolygon.element.tag === 'text')
            ) {
              polygonsToLeft.push(currentPolygon);
            } else {
              const result: Chart3DPolyCollections = this.splitPolygon(
                currentPolygon,
                plane
              );
...
export class Svg3DRenderer {
   ...
   */
  public drawText(
    options: Chart3DTextOption | SVGCanvasAttributes,
    label: string | string[],
    font: FontModel,
    chart: Chart3D
  ): Element {
    ...
    if (label.length > 1 && typeof label !== 'string') {
      ...
      for (let i: number = 0; i < label.length; i++) {
        const tspanElement: Element = document.createElementNS(
    ...
    text = chart.svgRenderer.setElementAttributes(
  ...
  public transform3DToVisible(
    ...
    if (currentSeries.xAxis != null && currentSeries.yAxis != null) {
      ...
      const yIsLogarithmic: boolean =
        currentSeries.yAxis.valueType === 'Logarithmic';
      // Apply logarithmic transformation if necessary
      ...
      if (chart.requireInvertedAxis) {
        ...
        const pointX: number =
          left +
          currentSeries.yAxis.rect.width *
            valueToCoefficients(y, currentSeries.yAxis);
        const pointY: number =
          top +
          currentSeries.xAxis.rect.height *
            (1 - valueToCoefficients(x, currentSeries.xAxis));
        ...
        const newX: number =
          left +
          Math.round(
            currentSeries.xAxis.rect.width *
              valueToCoefficients(x, currentSeries.xAxis)
          );
        const newY: number =
          top +
          Math.round(
            currentSeries.yAxis.rect.height *
              (1 - valueToCoefficients(y, currentSeries.yAxis))
          );
...
export class Polygon3D {
  ...
  public createCylinder(
    ...
    const radiusB: number =
      v2.y - v1.y < v2.z - v1.z ? (v2.y - v1.y) / 2 : (v2.z - v1.z) / 2;
    const radiusC: number =
      v2.x - v1.x < v2.z - v1.z ? (v2.x - v1.x) / 2 : (v2.z - v1.z) / 2;
      ...
      case 'StackingBar100':
        ...
        while (pathCount--) {
          ...
          topVector.push(
            this.vector.vector3D(
              v1.x,
              outPoints[i as number].Y,
              outPoints[i as number].Z
            )
          );
          bottomVector.push(
            this.vector.vector3D(
  ...
  public calculateNormal(...args: any[]): void {
    if (args.length >= 3) {
      ...
      const vector4: Chart3DVector = this.vector.vector3DMinus(
        vector1,
        vector2
      );
      const vector5: Chart3DVector = this.vector.vector3DMinus(
        vector3,
        vector2
      );
      const normal: Chart3DVector = this.vector.vector3DMultiply(
        vector4,
        vector5
      );
    ...
    if (polygon.points != null) {
      for (let i: number = 0; i < polygon.points.length; i++) {
        polygon.vectorPoints[i as number] = polygon.points[i as number] =
          matrixObj.getMatrixVectorMultiple(
            matrix,
            polygon.points[i as number]
          );
      }
  ...
  public getNormal(
    ...
    if (vectorPoints != null) {
      ...
      for (
        ...
        const v1: Chart3DVector = matrixObj.getMatrixVectorMultiple(
          transform,
          vectorPoints[i as number]
        );
        const v2: Chart3DVector = matrixObj.getMatrixVectorMultiple(
          transform,
          vectorPoints[0]
        );
        const v3: Chart3DVector = matrixObj.getMatrixVectorMultiple(
          transform,
          vectorPoints[i / 2]
        );
  ...
  public drawLine(panel: Chart3DPolygon, chart: Chart3D): void {
    ...
    const actual3DPosition1: Chart3DLocation = chart.transform3D.toScreen(
      panel.vectorPoints[0],
      transform
    );
    const actual3DPosition2: Chart3DLocation = chart.transform3D.toScreen(
  ...
  public drawTemplate(panel: Chart3DPolygon, chart: Chart3D): void {
    ...
    const actual3DPosition: Chart3DLocation = chart.transform3D.toScreen(
      panel.vectorPoints[0],
      transform
    );
    chart.dataLabel3DModule.createDataLabelTemplate(
      label.series.dataLabelElement,
      label.series,
      label.series.dataLabel,
      label.series.visiblePoints[label.pointIndex],
      element.argsData,
      label.pointIndex,
      false,
      actual3DPosition
  ...
  public drawText(panel: Chart3DPolygon, chart: Chart3D): void {
    ...
    if (element.tag === 'text') {
      ...
      const labelFormat: string =
        series.yAxis && series.yAxis.labelFormat
          ? series.yAxis.labelFormat
          : '';
      const pointText: string = element.point.text
        ? element.point.text
        : element.point.y + labelFormat.substring(labelFormat.indexOf('}') + 1);
  ...
  public draw(panel: Chart3DPolygon, chart: Chart3D): void {
    ...
    const transform: Chart3DBasicTransform = chart3DRender.transform;
    const pathDirection: Chart3DStringBuilder =
    ...
    if (transform != null) {
      ...
      for (let i: number = 0; i < panel.vectorPoints.length; i++) {
        const lineSegment: Chart3DLocation = chart.transform3D.toScreen(
    ...
    if (element.id.indexOf('0-region-series-0-point-0') > -1) {
      if (this.tabIndex) {
        ...
        const elements: NodeListOf<Element> =
          panel.polygonElement.parent.querySelectorAll(
            '[id*="0-region-series-0-point-0"]'
          );
 ...
 */
export function getMinPointsDeltaValue(
  axis: Chart3DAxis,
