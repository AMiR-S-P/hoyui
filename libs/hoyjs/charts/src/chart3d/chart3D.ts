...
import { L10n, isNullOrUndefined, Touch } from '@hoyui/base';
import { Event, EmitType } from '@hoyui/base';
...
import {
  Chart3DBoderElements,
  Chart3DLoadedEventArgs,
  Chart3DThemeStyle,
  Chart3DBeforeResizeEventArgs,
  ...
  implements INotifyPropertyChanged
{
  ...
  private calculateBounds(): void {
    ...
    const padding: number =
      ...
      this.titleStyle.position === 'Bottom'
        ? 15
        : 5;
    ...
    if (this.title) {
      this.titleCollection = getTitle(
        this.title,
        this.titleStyle,
        this.titleStyle.position === 'Left' ||
          this.titleStyle.position === 'Right'
          ? height
          : width,
        this.enableRtl,
    ...
    if (this.legend3DModule && this.legendSettings.visible) {
      this.legend3DModule.calculateLegendBounds(
  ...
  private animateRect(
    ...
    const duration: number = series.chart.animated
      ? series.chart.duration
      : option.duration;
    const effect: Function = getAnimationFunction('Linear');
    let elementHeight: number = element.getAttribute('height')
      ? +element.getAttribute('height')
      : 0;
    let elementWidth: number = element.getAttribute('width')
    ...
    if (!isNullOrUndefined(element)) {
      ...
      new Animation({}).animate(element, {
        duration:
          duration === 0 && animationMode === 'Enable' ? 1000 : duration,
        delay: option.delay,
    ...
    for (let i: number = 0; i < this.visibleSeries.length; i++) {
      ...
      if (
        ...
        const options: RectOption = new RectOption(
          this.element.id + '_ChartSeriesClipRect_' + i,
          'transparent',
          { width: 1, color: 'Gray' },
          1,
          {
        ...
        const clipRectElement: Element = appendClipElement(
          this.redraw,
          options,
          this.svgRenderer
        );
        appendChildElement(
          false,
          this.chart3D,
          clipRectElement.children[0],
          this.redraw
        );
        for (
          let k: number = 0;
          series.visiblePoints && k < series.visiblePoints.length;
          k++
        ) {
          ...
          elements.forEach((element: Element) => {
            element.setAttribute(
              'clip-path',
              'url(#' + this.element.id + '_ChartSeriesClipRect_' + i + ')'
            );
          });
          if (series.dataLabel.visible) {
            dataLabelElement = getElement(
              this.element.id +
                '-svg-series-' +
                series.index +
                '-point-' +
            ...
            shapeElement = getElement(
              this.element.id +
                '-svg-data-label-series-' +
                series.index +
                '-point-' +
                k
            );
            templateElement = getElement(
              this.element.id + '-series-' + series.index + '-data-label-' + k
            );
          }
          this.animateRect(
            ...
            series,
            point,
            dataLabelElement as HTMLElement,
            shapeElement as HTMLElement,
  ...
  private performSelection(): void {
    ...
    if (this.selection3DModule) {
      selectedDataIndexes = <Indexes[]>(
        extend([], this.selection3DModule.selectedDataIndexes, null, true)
      );
    ...
    if (
      (!this.highlight3DModule ||
        (this.legendSettings.enableHighlight &&
          this.highlightMode === 'None')) &&
      this.tooltip3DModule
    ) {
  ...
  private renderLegend(): void {
    if (
      this.legend3DModule &&
      this.legend3DModule.legendCollections.length &&
  ...
  private renderTitle(): void {
    ...
    if (this.title) {
      ...
      const htmlObject: Element =
        redrawElement(
          this.redraw,
          this.element.id + '-chart-title-border',
          borderOptions,
          this.renderer
  ...
  private renderSubTitle(options: TextOption): void {
    ...
    const subTitleElementSize: Size = measureText(
      this.subTitleCollection.reduce((a: string, b: string): string =>
    ...
    const htmlObject: Element =
      redrawElement(
        this.redraw,
        this.element.id + '-chart-sub-title-border',
        borderOptions,
        this.renderer
      ) || this.renderer.drawRectangle(borderOptions);
    appendChildElement(false, this.svgObject, htmlObject, this.redraw);
    ...
    series.map((value: Chart3DSeries) => {
      ...
      moduleName =
        value.type.indexOf('100') !== -1
          ? value.type.replace('100', '') + 'Series3D'
          : value.type + 'Series3D';
  ...
  private setTheme(): void {
    ...
    if (!document.getElementById(this.element.id + 'Keyboard_chart_focus')) {
      ...
      style.innerText =
        ...
        'path[id*=-point-]:focus-visible, text[id*=-chart-title]:focus-visible {outline: 1.5px ' +
  ...
  private initAxis(
    ...
    if (
      ...
      series.xAxis = axis;
      if (isSeries) {
        axis.series.push(series as Chart3DSeries);
      }
    } else if (
  ...
  private calculateVisibleAxis(): void {
    ...
    for (let i: number = 0, len: number = axes.length; i < len; i++) {
      axis = <Chart3DAxis>axes[i as number];
      axis.series = [];
      axis.labels = [];
      axis.indexLabels = {};
      axis.orientation =
  ...
  private triggerPointEvent(
    ...
    if (targetElement) {
      ...
      if (
        (nodeName === 'path' || nodeName === 'shape') &&
        targetElement.id.indexOf('-region-') > 1
      ) {
  ...
  public chartOnMouseDown(e: PointerEvent): boolean {
    ...
    if (this.enableRotation) {
      if (
        this.isTouch &&
        this.tooltip3DModule &&
        this.tooltip3DModule.svgTooltip
      ) {
    ...
    if (
      ...
      if (difX || difY) {
        ...
        if (!this.isTouch) {
          ...
          document
            .querySelectorAll('[id*="axis-label-"]')
            .forEach(function (axisElement: Element): void {
  ...
  private titleTooltip(
    ...
    const id: boolean =
      targetId === this.element.id + '-chart-title' ||
      targetId === this.element.id + '-chart-sub-title' ||
      targetId.indexOf('-axis-title') > -1 ||
      targetId.indexOf('_legend_title') > -1;
    let index: number = 0;
    if (targetId.indexOf('-axis-title') > -1) {
      index = parseInt(
        targetId
          .replace(this.element.id + '-svg', '')
          .replace('-axis-title', '')
          .split('-')[1],
        10
      );
    ...
    if (id && (<HTMLElement>event.target).textContent.indexOf('...') > -1) {
      const title: string =
        targetId === this.element.id + '-chart-title'
          ...
          : targetId.indexOf('-chart-sub-title') > -1
          ? this.subTitle
      ...
      showTooltip(
        ...
        this.element.id + '-EJ2-title-tooltip',
  ...
  private setMouseXY(pageX: number, pageY: number): void {
    if (getElement(this.svgId)) {
      ...
      const rect: ClientRect = this.element.getBoundingClientRect();
      this.mouseY =
        pageY - rect.top - Math.max(svgRect.top - rect.top, 0) / this.scaleX;
      this.mouseX =
        pageX - rect.left - Math.max(svgRect.left - rect.left, 0) / this.scaleY;
    }
  }
  ...
  public chartKeyUp(e: KeyboardEvent): boolean {
    ...
    const seriesElement: HTMLElement = getElement(
      this.element.id + '-svg-0-region-series-0-point-0'
    ) as HTMLElement;
    ...
    if (e.code === 'Tab') {
      if (this.previousTargetId !== '') {
        if (
          this.previousTargetId.indexOf('-series-') > -1 &&
          targetId.indexOf('-series-') === -1
        ) {
          const previousElement: Element = getElement(
            this.element.id +
              '-svg-0-region-series-' +
      ...
      if (targetId.indexOf('-series-') > -1) {
        this.currentSeriesIndex = +targetId
          .split('-series-')[1]
          .split('-point-')[0];
      ...
      if (targetId.indexOf('_chart_legend_page') > -1) {
        ...
        if (e.code === 'ArrowRight' || e.code === 'ArrowLeft') {
          this.currentSeriesIndex =
            this.currentSeriesIndex + (e.code === 'ArrowRight' ? 1 : -1);
          this.currentSeriesIndex = this.getActualIndex(
            this.currentSeriesIndex,
            this.series.length
          );
        } else {
        ...
        if (targetId.indexOf('-point-') > -1) {
          this.currentPointIndex = this.getActualIndex(
            this.currentPointIndex,
            this.visibleSeries[this.currentSeriesIndex].points.length
              ? this.currentSeries.points.length
              : 1
          );
          const pointElements: NodeListOf<Element> = document.querySelectorAll(
            '[id*="svg-0-region-series-' +
              this.currentSeriesIndex +
              '-point-' +
              this.currentPointIndex +
              '"]'
          ...
          for (let i: number = 0; i < pointElements.length; i++) {
            if (
              pointElements[i as number].id
                .split('-point-')[1]
                .split('-')[0] === this.currentPointIndex.toString()
            ) {
  ...
  private axisTooltip(
    ...
    if (
      ...
      showTooltip(
        ...
        y,
        this.element.offsetWidth,
        this.element.id + '-EJ2-axis-label-tooltip',
        getElement(this.element.id + '_Secondary_Element'),
        isTouch,
    ...
    switch (actionKey) {
      ...
      case 'ArrowMove':
        ...
        if (targetId.indexOf('-point-') > -1) {
          ...
          if (this.highlight3DModule) {
            this.highlight3DModule.highlightChart(
          ...
          if (this.tooltip3DModule) {
            const data: Point3D = {
              series:
                this.visibleSeries[
                  targetId.split('-series-')[1].split('-point-')[0]
                ],
              point:
                this.visibleSeries[
                  targetId.split('-series-')[1].split('-point-')[0]
                ].points[targetId.split('-point-')[1].split('-')[0]],
            };
            const svgElement: HTMLElement = document.getElementById(
              this.element.id + '_tooltip_svg'
            );
            const isFirst: boolean =
              svgElement &&
              parseInt(svgElement.getAttribute('opacity'), 10) > 0;
            const tooltipDiv: HTMLDivElement =
              this.tooltip3DModule.getTooltipElement(isFirst);
            if (
              this.tooltip3DModule.pushData(
                data as Point3D,
                !isFirst,
                tooltipDiv,
                true
              ...
              this.tooltip3DModule.triggerTooltipRender(
                data,
                !isFirst,
        ...
        if (this.highlight3DModule && this.highlightMode !== 'None') {
          ...
          for (let i: number = 0; i < legendItemsId.length; i++) {
            ...
            if (targetId.indexOf(id) > -1) {
              ...
              this.highlight3DModule.legendSelection(
                this,
      ...
      case 'Space':
        if (targetId.indexOf('_chart_legend_') > -1) {
          ...
          this.selection3DModule.calculateSelectedElements(
            document.getElementById(targetId),
            'click'
          );
        }
  ...
  private setStyle(element: HTMLElement): void {
    ...
    element.style.touchAction =
      this.enableRotation || disableScroll ? 'none' : 'element';
    ...
    for (
      ...
      if (this.isSecondaryAxis(series.xAxis)) {
        series.xAxis.internalVisibility = series.xAxis.series.some(
          (value: Chart3DSeries) => value.visible
        );
      }
  ...
  public highlightAnimation(
    ...
    const endOpacity: number = parseFloat(
      this.visibleSeries[index as number].opacity.toString()
    );
    if (endOpacity) {
      new Animation({}).animate(element, {
        duration: duration,
        ...
        switch (prop) {
          case 'primaryXAxis':
            ...
            if (
              ...
              (
                this.primaryXAxis as Chart3DAxis
          ...
          case 'primaryYAxis':
            ...
            if (
              ...
              (
                this.primaryYAxis as Chart3DAxis
              ).setIsInversedAndOpposedPosition();
            }
            break;
          case 'axes':
            for (const index of Object.keys(newProp.axes)) {
              ...
              if (
                ...
                (
                  this.axes[index as string] as Chart3DAxis
                ).setIsInversedAndOpposedPosition();
              }
            }
            break;
          ...
          case 'selectedDataIndexes':
            if (this.selection3DModule) {
              ...
              this.selection3DModule.selectedDataIndexes = this
                .selectedDataIndexes as Indexes[];
              this.selection3DModule.styleId =
                this.element.id + '_ej2_chart_selection';
              this.selection3DModule.redrawSelection(
                this,
          ...
          case 'isMultiSelect':
            if (
              ...
              this.selection3DModule.selectedDataIndexes.length > 1
            ) {
              this.selection3DModule.currentMode = this.selectionMode;
              this.selection3DModule.styleId =
                this.element.id + '_ej2_chart_selection';
