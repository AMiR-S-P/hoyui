import { Property, Complex, ChildProperty, Browser } from '@hoyui/base';
import {
  rotateTextSize,
  firstToLowerCase,
  isBreakLabel,
...
import {
  Chart3DAxisRangeCalculatedEventArgs,
  Chart3DTextFont,
} from '../model/chart3d-Interface';
import {
  Chart3DMajorGridLinesModel,
  Chart3DMajorTickLinesModel,
  Chart3DMinorGridLinesModel,
  Chart3DMinorTickLinesModel,
} from './axis-model';
...
export class Chart3DRow extends ChildProperty<Chart3DRow> {
  ...
  public computeSize(axis: Chart3DAxis, chart: Chart3D): void {
    ...
    if (axis.visible && axis.internalVisibility) {
      width +=
        axis.majorTickLines.height + axis.findLabelSize(innerPadding, chart);
    }
...
export class Chart3DAxis extends ChildProperty<Chart3DAxis> {
  ...
  public getMaxLabelWidth(chart: Chart3D): void {
    ...
    for (
      ...
      if (
        ...
        pointX =
          valueToCoefficients(label.value, this) * this.rect.width +
          this.rect.x;
        pointX -= width1 / 2;
  ...
  private findMultiRows(
    ...
    for (let i: number = length - 1; i >= 0; i--) {
      ...
      pointX =
        valueToCoefficients(label.value, this) * this.rect.width + this.rect.x;
  ...
  public setIsInversedAndOpposedPosition(): void {
    this.isAxisOpposedPosition =
      ...
      (this.isRTLEnabled && this.orientation === 'Vertical');
    this.isAxisInverse =
      this.isInversed ||
