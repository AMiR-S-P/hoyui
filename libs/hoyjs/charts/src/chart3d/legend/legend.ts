...
import { Size, measureText, Rect, getElement } from '@hoyui/svg-base';
import {
  legendRender,
  legendClick,
  regSub,
...
export class Legend3D extends BaseLegend {
  ...
  private mouseMove(e: MouseEvent): void {
    if (this.chart.legendSettings.visible && !this.chart.isTouch) {
      this.move(e);
      if (
        (<Chart3D>this.chart).highlight3DModule &&
        ((<Chart3D>this.chart).highlightMode !== 'None' ||
          (<Chart3D>this.chart).legendSettings.enableHighlight)
      ) {
        ...
        for (const id of legendItemsId) {
          if (targetId.indexOf(id) > -1) {
            ...
            (<Chart3D>this.chart).highlight3DModule.legendSelection(
              <Chart3D>this.chart,
              index,
              e.target as Element,
              e.type
  ...
  public getLegendOptions(
    visibleSeriesCollection: Chart3DSeries[],
    chart: Chart3D
  ): void {
      ...
      if (this.legend.mode === 'Series') {
        ...
        this.legendCollections.push(
          new LegendOptions(
            ...
            series.visible,
            seriesType,
            series.legendImageUrl ? series.legendImageUrl : '',
            'None',
            false,
            null,
            null
        ...
        for (const points of series.points) {
          ...
          if (
            this.legendCollections.filter(
              (i: LegendOptions) => i.text === points.x.toString()
            ).length === 0
          ) {
            this.legendCollections.push(
              new LegendOptions(
    ...
    if (chart.legendSettings.mode === 'Series') {
      ...
      if (!legendClickArgs.cancel) {
        ...
        if (chart.legendSettings.toggleVisibility) {
          ...
          chart.highlight3DModule.legendSelection(
            chart,
            index,
          ...
          chart.selection3DModule.legendSelection(
            chart,
            index,
            event.target as Element,
            event.type
  ...
  private refreshLegendToggle(chart: Chart3D, series: Chart3DSeries): void {
    ...
    if (chart.selection3DModule) {
      selectedDataIndexes = <Indexes[]>(
        extend([], chart.selection3DModule.selectedDataIndexes, null, true)
      );
    }
    ...
    if (
      (chart.highlight3DModule && chart.highlightMode !== 'None') ||
