...
export class Tooltip3D extends BaseTooltip {
  ...
  private mouseUpHandler(event: MouseEvent | PointerEvent | TouchEvent): void {
    ...
    if (chart.isTouch) {
      ...
      !this.findData(data, this.previousPoints[0] as Point3D) &&
  ...
  public seriesStyles(): void {
    if (!this.styleAdded) {
      ...
      style.innerText +=
        ' .' +
        this.element.id +
        '_ej2_tooltipDeselected { opacity:' +
        0.2 +
        ';} ';
  ...
  private renderSeriesTooltip(
    ...
    if (
      ...
      ((this.previousPoints[0] &&
        !(
          this.previousPoints[0].point.index === data.point.index &&
          this.previousPoints[0].series.index === data.series.index &&
          this.chart3D.isRemove
        )) ||
        !this.previousPoints[0])
    ) {
  ...
  public triggerTooltipRender(
    ...
    const chartTooltipSuccess: Function = (
      argsData: Chart3DTooltipRenderEventArgs
    ) => {
      if (!argsData.cancel) {
        ...
        this.createTooltip(
          this.chart,
          isFirst,
          this.getSymbolLocation(point),
          point.series.clipRect,
          point.point,
          this.chart3D.tooltip.enableMarker ? ['Circle'] : [],
          0,
          new Rect(
            borderWidth,
            borderWidth,
  ...
  private blurEffect(
    ...
    tooltipSeries: Chart3DSeries
  ): void {
    if (
      !this.chart3D.highlight3DModule ||
      (this.chart3D.legendSettings.enableHighlight &&
        this.chart3D.highlightMode === 'None')
    ) {
      ...
      for (const series of visibleSeries) {
        if (series.visible && series.index !== tooltipSeries.index) {
          ...
          const tooltipElements: NodeListOf<HTMLElement> =
            document.querySelectorAll(`[id*="region-series-${series.index}"]`);
          for (let i: number = 0; i < tooltipElements.length; i++) {
            ...
            this.chart3D.stopElementAnimation(
      ...
      for (let i: number = 0; i < pointElements.length; i++) {
        if (pointElements[i as number]) {
          let elementClassName: string =
            pointElements[i as number].getAttribute('class') || '';
          elementClassName += elementClassName !== '' ? ' ' : '';
          if (
            elementClassName.indexOf('_selection_') === -1 &&
            elementClassName.indexOf(
              this.element.id + '_ej2_tooltipDeselected'
            ) === -1
          ) {
            pointElements[i as number].setAttribute(
              'class',
              elementClassName + this.element.id + '_ej2_tooltipDeselected'
            );
          }
        }
      }
  ...
  private removeBlurEffect(): void {
    if (
      ...
      while (elements.length > 0) {
        ...
        if (
          elementClassName.indexOf(this.element.id + '_ej2_tooltipDeselected') >
          -1
        ) {
          element.setAttribute(
            'class',
          ...
          this.chart3D.highlightAnimation(
            element as HTMLElement,
            index,
            700,
            0.2
  ...
  private getSymbolLocation(point: Point3D): ChartLocation {
    ...
    if (point.series.columnFacet === 'Cylinder') {
      ...
      upperElement = document.querySelectorAll(
        '[id*="' +
          this.element.id +
          '-svg-' +
          (point.series.type.indexOf('Column') === -1 ? '5' : '2') +
          '-region-series-' +
          point.series.index +
          '-point-' +
          point.point.index +
    ...
    if (upperElement) {
      if (upperElement.length === 1) {
        ...
        for (let i: number = 0; i < upperElement.length; i++) {
          ...
          if (
            element.id.indexOf('-' + point.point.index + '-back-front') !==
              -1 ||
            element.id.indexOf('-' + point.point.index + '-front-back') !== -1
          ) {
    ...
    const location: ChartLocation = new ChartLocation(
      this.chart3D.tooltip.location.x !== null
        ? this.chart3D.tooltip.location.x
        : tooltipElement.left - rect.left + tooltipElement.width / 2,
      this.chart3D.tooltip.location.y !== null
        ? this.chart3D.tooltip.location.y
        : tooltipElement.top - rect.top + tooltipElement.height / 2
    );
    return location;
  }
  ...
  public getTooltipText(pointData: Point3D): string {
    return this.parseTemplate(
      pointData.point,
      pointData.series,
      this.getFormat(this.chart3D, pointData.series),
      pointData.series.xAxis,
      pointData.series.yAxis
    );
  }
  ...
  private findMouseValue(data: Point3D, chart: Chart3D): void {
    if (!chart.requireInvertedAxis) {
      this.valueX =
        valueToCoefficients(data.point.xValue, data.series.xAxis) *
          data.series.xAxis.rect.width +
        data.series.xAxis.rect.x;
      this.valueY = chart.mouseY;
    } else {
      this.valueY =
        (1 - valueToCoefficients(data.point.xValue, data.series.xAxis)) *
