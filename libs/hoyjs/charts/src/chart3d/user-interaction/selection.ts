...
export class Selection3D extends BaseSelection {
  ...
  public selectDataIndex(chart: Chart3D, indexes: Index[]): void {
    for (const index of indexes) {
      this.performSelection(
        index,
        chart,
        this.getElementByIndex(chart, index)[0]
      );
    }
  }
  ...
  public getElementByIndex(chart: Chart3D, index: Index): Element[] {
    ...
    const elements: NodeListOf<HTMLElement> = document.querySelectorAll(
      '[id*="-region-series-' + index.series + '-point-' + index.point + '"]'
    );
    ...
    if (
      ...
      if (targetElem.parentNode) {
        isElement =
          ...
          targetElem.id.indexOf('region') > 1
            ? true
            : false;
      }
      for (let i: number = 0; i < this.previousSelectedEle.length; i++) {
        if (this.previousSelectedEle[i as number].hasAttribute('class')) {
          if (
            ...
            this.previousSelectedEle[i as number].classList.forEach(
              (className: string) => {
            ...
            this.previousSelectedEle[i as number].classList.remove(
              this.styleId + '_series_' + index.series
            );
            if (
              ...
              this.previousSelectedEle[i as number].setAttribute(
                'fill',
                (this.control as Chart3D).visibleSeries[
  ...
  public calculateSelectedElements(
    ...
    if (
      (this.chart.selectionMode === 'None' &&
        this.chart.highlightMode === 'None') ||
      (targetElement.id &&
        targetElement.id.indexOf(this.chart.element.id + '-') === -1)
    ) {
      return;
    }
    ...
    if (
      targetElement.id &&
      targetElement.id.indexOf('-series-') > -1 &&
    ...
    switch (this.currentMode) {
      case 'Series':
        this.selection(
          chart,
          index,
          this.getSeriesElements(chart.series[index.series as number])
        );
  ...
  public selectionComplete(
    chart: Chart3D,
    index: Index,
    selectionMode: Chart3DSelectionMode | HighlightMode
  ): void {
    let points: Chart3DPoint[];
  ...
  public selection(
    ...
    if (!isNullOrUndefined(selectedElements[0])) {
      if (
        (<Chart3DSeries>chart.visibleSeries[indexValue as number]).isRectSeries
      ) {
  ...
  public blurEffect(chartId: string, visibleSeries: Chart3DSeries[]): void {
    ...
    for (const series of visibleSeries) {
      ...
      const legendStrokeColor: string =
        this.chart.visibleSeries[series.index].interior;
      const pointElements: Element[] = [];

      if (series.visible) {
        const elements: NodeListOf<HTMLElement> = document.querySelectorAll(
          `[id*="region-series-${series.index}"]`
        );
  ...
  public checkSelectionElements(
    ...
    let children: HTMLCollection | Element[] = <Element[]>(
      (this.isSeriesMode ? element || [element] : element)
    );
    if (
      this.chart.selectionMode !== 'None' &&
      (this.chart.highlightMode !== 'None' ||
    ...
    for (let i: number = 0; i < children.length; i++) {
      ...
      if (
        ...
        if (
          ...
          this.chart.tooltip3DModule &&
          className.indexOf('highlight') > 0
        ) {
          this.chart.highlightAnimation(
    ...
    if (
      (this.control as Chart3D).legend3DModule &&
      (this.control as Chart3D).legendSettings.visible
    ) {
      ...
      if (legendShape) {
        ...
        if ((selectElement as Element[]).length > 0) {
          ...
          parentClassName =
            (<Element>selectElement[0].parentNode).getAttribute('class') || '';
        } else if (selectElement as Element) {
          elementClassName =
            (selectElement as Element).getAttribute('class') || '';
          parentClassName =
            (<Element>(selectElement as Element).parentNode).getAttribute(
              'class'
            ) || '';
        }
        if (
          ...
          if (
            ...
            legendShape.setAttribute(
              'stroke',
              (this.control as Chart3D).visibleSeries[series as number].interior
            );
            if (this.chart.highlightPattern === 'None') {
              legendShape.setAttribute(
                'fill',
                (this.control as Chart3D).visibleSeries[series as number]
                  .interior
              );
            }
          }
  ...
  public applyStyles(elements: Element[]): void {
    for (const element of elements) {
      if (element) {
        ...
        if (
          this.chart.tooltip3DModule &&
          this.getSelectionClass(element.id).indexOf('highlight') > 0
        ) {
          const index: number = parseFloat(
            element.id.split('-series-')[1].split('-point-')[0]
          );
  ...
  public getSelectionClass(id: string): string {
    return this.generateStyle(
      (this.control as Chart3D).visibleSeries[this.indexFinder(id).series]
    );
  }
    ...
    for (const element of elements) {
      if (element) {
        ...
        if (
          ...
          let color: string = (this.control as Chart3D).visibleSeries[
            this.indexFinder(element.id).series
          ].interior;
          if (element.getAttribute('name') === 'ZLight') {
            color = this.chart.polygon.applyZLight(
              color,
              this.control as Chart3D
            );
          }
          if (element.getAttribute('name') === 'XLight') {
            color = this.chart.polygon.applyXLight(
              color,
              this.control as Chart3D
            );
          }
  ...
  public redrawSelection(
    chart: Chart3D,
    oldMode: Chart3DSelectionMode | HighlightMode,
    chartRedraw?: boolean
  ): void {
   ...
   * @private
   */
  public getSeriesElements(
    series: Chart3DSeriesModel | Chart3DSeries
  ): Element[] {
    const seriesElements: Element[] = [];
    if (series.visible) {
      const elements: NodeListOf<HTMLElement> = document.querySelectorAll(
        `[id*="region-series-${(series as Chart3DSeries).index}"]`
      );
    ...
    if (
      ...
      if (!isNullOrUndefined(target)) {
        ...
        if (
          ...
          this.chart.highlight3DModule.highlightDataIndexes.length > 0 &&
          target.id.indexOf('_chart_legend_') === -1 &&
          target.id.indexOf('-series-') === -1
        ) {
      ...
      if (this.selectedDataIndexes.length === 0) {
        ...
        while (elementCollection.length > 0) {
          ...
          if (element) {
            ...
            if (
              ...
              if (
                element.id.indexOf('_chart_legend_shape') !== -1 &&
        ...
        while (elementCollection.length > 0) {
          ...
          if (element) {
            ...
            if (
              this.chart.tooltip3DModule &&
              this.generateStyle(this.chart.visibleSeries[i as number]).indexOf(
                'highlight'
              ) > -1
            ) {
              this.chart.highlightAnimation(
