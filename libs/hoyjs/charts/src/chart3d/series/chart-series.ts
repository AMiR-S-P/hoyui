...
import {
  ...
  Chart3DDataLabelSettingsModel,
} from './chart-series-model';
...
export class Chart3DSeries extends ChildProperty<Chart3DSeries> {
  ...
  public processJsonData(): void {
    ...
    const textMappingName: string =
      this instanceof Chart3DSeries && this.dataLabel.name
        ? this.dataLabel.name
        : '';
    ...
    for (const item of this.xAxis.series) {
      if (item.visible) {
        ...
        for (const point of item.points) {
          (item as Chart3DSeries).pushCategoryData(
            point,
            point.index,
            <string>point.x
          );
  ...
  private rectSeriesInChart(series: Chart3DSeries, isStack: boolean): boolean {
    ...
    return (
      type.indexOf('column') !== -1 || type.indexOf('bar') !== -1 || isStack
    );
  }
    ...
    for (const columnItem of chart.columns) {
      for (const item of chart.rows) {
        this.calculateStackingValues(
          this.findSeriesCollection(
            <Chart3DColumn>columnItem,
            <Chart3DRow>item,
            true
          ),
          isStacking100
  ...
  private calculateStackingValues(
    ...
    for (let k: number = 0; k < keys.length; k++) {
      ...
      const seriesCollection: Chart3DSeries[] =
        groupingValues[keys[k as number]];
  ...
  private getVisiblePoints(): Chart3DPoint[] {
    const points: Chart3DPoint[] = extend(
      [],
      this.points,
      null,
      true
  ...
  public getSideBySideInfo(series: Chart3DSeries): Chart3DRangeValues {
    ...
    const minimumPointDelta: number = getMinPointsDeltaValue(
      series.xAxis,
    ...
    if (!range.isEmpty) {
      ...
      range = this.getDoubleRange(
        range.start + radius / 2,
        range.end - radius / 2
      );
    }
    return range;
