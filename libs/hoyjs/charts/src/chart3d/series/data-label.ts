...
export class DataLabel3D {
  ...
  public render(
    ...
    if (series.dataLabel.template) {
      appendChildElement(
        false,
        document.getElementById(this.chart.element.id + '_Secondary_Element'),
  ...
  private draw3DDataLabel(
    ...
    const location: Chart3DLocation = chart.svg3DRenderer.transform3DToVisible(
      series,
      point.symbolLocations.x,
      point.symbolLocations.y,
      chart
    );
    pointY = location.y;
    pointX = location.x;
    ...
    this.fontBackground =
      series.dataLabel.position === 'Middle'
        ? argsData.color === 'transparent'
          ? point.color
          : argsData.color
        : argsData.color;
    ...
    if (
      ...
      chart.graphics.addVisual(
        chart.polygon.createTextElement(
          chart.vector.vector3D(pointX, pointY, point.symbolLocations.z),
          element,
          0,
          -size.height
        ),
    ...
    const tag: string = !(series.dataLabel && series.dataLabel.template)
      ? 'text'
      : 'template';
    const backgroundColor: string =
      this.fontBackground === 'transparent'
        ? this.chart.theme.indexOf('Dark') > -1 ||
          this.chart.theme === 'HighContrast'
          ? '#000000'
          : '#FFFFFF'
        : this.fontBackground;
    ...
    const font: Chart3DTextFontModel = {
      ...
      fontWeight:
        argsData.textStyle.fontWeight ||
        this.chart.themeStyle.datalabelFont.fontWeight,
      fontStyle:
        argsData.textStyle.fontStyle ||
        chart.themeStyle.datalabelFont.fontStyle,
      fontFamily:
        argsData.textStyle.fontFamily ||
        this.chart.themeStyle.datalabelFont.fontFamily,
      color:
  ...
  private getLabelText(
    currentPoint: Chart3DPoint,
    series: Chart3DSeries,
    chart: Chart3D
  ): string[] {
    const labelFormat: string = series.dataLabel.format
  ...
  public createDataLabelTemplate(
    ...
    const childElement: HTMLElement = this.createTemplate(
      ...
      point,
      series,
      this.chart.element.id + '-data-label-',
      labelIndex,
      location
    );
  ...
  public calculateTemplateLabelSize(
    ...
    const rectPosition: { left: number; top: number; right: number } =
      this.calculateTextPosition(series, point, elementRect, location);
    ...
    const vAxis: Chart3DAxis = series.chart.requireInvertedAxis
      ? series.xAxis
      : series.yAxis;
    const hAxis: Chart3DAxis = series.chart.requireInvertedAxis
      ? series.yAxis
      : series.xAxis;
    childElement.style.color =
      dataLabel.font.color || this.chart.theme === 'Bootstrap5'
        ? '#212529'
        : this.chart.theme === 'Bootstrap5Dark'
        ? '#DEE2E6'
        : Math.round(
