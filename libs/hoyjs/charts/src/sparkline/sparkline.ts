...
import { Browser, EventHandler, Touch, Collection } from '@hoyui/base';
import {
  SparklineBorder,
  ...
  implements INotifyPropertyChanged
{
  ...
  private setSecondaryElementPosition(): void {
    const element: HTMLDivElement = getIdElement(
    ...
    const svgRect: ClientRect = getIdElement(
  ...
  private renderBorder(): void {
    ...
    if (width > 0 || this.containerArea.background !== 'transparent') {
      borderRect = new RectOption(
        this.element.id + '_SparklineBorder',
        this.sparkTheme.background,
        this.containerArea.border,
        1,
        new Rect(
          width / 2,
          width / 2,
      ...
      this.svgObject.appendChild(
        drawRectangle(this, borderRect) as SVGRectElement
      );
    }
    ...
    borderRect = new RectOption(
      this.element.id + '_sparkline_clip_rect',
      'transparent',
      ...
      new Rect(
        padding.left,
        padding.top,
        this.availableSize.width - (padding.left + padding.right),
  ...
  private setTheme(): void {
    ...
    this.sparkTheme = getThemeColor(this.theme);
    if (
      !document.getElementById(this.element.id + 'Keyboard_sparkline_focus')
    ) {
      const style: HTMLStyleElement = document.createElement('style');
      style.setAttribute(
        'id',
        (<HTMLElement>this.element).id + 'Keyboard_sparkline_focus'
      );
      style.innerText =
        '.e-sparkline-focused:focus,' +
        ...
        'div[id*=container]:focus-visible, path[id*=_sparkline_]:focus-visible, circle[id*=_sparkline_]:focus-visible {outline: 1.5px ' +
        this.sparkTheme.tabColor +
        ' solid}';
  ...
  public requiredModules(): ModuleDeclaration[] {
    ...
    if (
      this.tooltipSettings.visible ||
      this.tooltipSettings.trackLineSettings.visible
    ) {
      modules.push({
  ...
  public sparklineMove(e: PointerEvent): boolean {
    ...
    const pointClick: { isPointRegion: boolean; pointIndex: number } =
      this.isPointRegion(e);
   ...
   */
  private isPointRegion(e: PointerEvent): {
    ...
    if (id[1] === this.type.toLowerCase()) {
      ...
      if (
        (isNullOrUndefined(index) || isNaN(index)) &&
        (this.type === 'Line' || this.type === 'Area')
      ) {
        this.sparklineRenderer.visiblePoints.forEach(
          (point: SparkValues, i: number): void => {
            if (
              withInBounds(
                this.mouseX,
                this.mouseY,
  ...
  public chartKeyDown(e: KeyboardEvent): boolean {
    ...
    if (
      this.tooltipSettings.visible &&
      ((e.code === 'Tab' &&
        this.previousTargetId.indexOf('_sparkline_') > -1) ||
        e.code === 'Escape')
    ) {
      actionKey = 'ESC';
    }
    if (actionKey !== '') {
      this.sparklineKeyboardNavigations(
        e,
  ...
  public chartKeyUp(e: KeyboardEvent): boolean {
    ...
    if (e.code === 'Tab') {
      if (this.previousTargetId !== '') {
        if (
          this.previousTargetId.indexOf('_sparkline_') > -1 &&
          targetId.indexOf('_sparkline_') === -1
        ) {
      ...
      if (targetId.indexOf('_sparkline_') > -1) {
        ...
        if (targetId.indexOf('_marker') > -1) {
          this.currentPointIndex = this.getActualIndex(
            ...
            getElement(this.element.id + '_sparkline_marker_g')
              .childElementCount
          );
          currentPoint = getElement(
            this.element.id + '_sparkline_marker_' + this.currentPointIndex
          );
          ...
          currentPoint = getElement(
            this.element.id + '_sparkline_column_' + this.currentPointIndex
          );
          ...
          currentPoint = getElement(
            this.element.id + '_sparkline_winloss_' + this.currentPointIndex
          );
          ...
          currentPoint = getElement(
            this.element.id + '_sparkline_pie_' + this.currentPointIndex
          );
        }
    ...
    switch (actionKey) {
      ...
      case 'ArrowMove':
        if (targetId.indexOf('_sparkline_') > -1) {
          ...
          if (
            this.type.indexOf('Line') > -1 ||
            this.type.indexOf('Area') > -1
          ) {
          ...
          if (this.sparklineTooltipModule) {
            this.sparklineTooltipModule.renderTooltip(
              this.sparklineRenderer.visiblePoints[pointIndex as number]
            );
          }
        }
        break;
  ...
  public destroy(): void {
    ...
    const element: HTMLElement = document.getElementById(
      this.element.id + 'Keyboard_sparkline_focus'
    );
    if (element) {
      element.remove();
    }
