...
import { Tooltip } from '@hoyui/svg-base';
...
export class SparklineTooltip {
  ...
  private fadeOut(): void {
    ...
    this.clearTooltip = +setTimeout(
      this.removeTooltipElements.bind(this),
      5000
    );
  }
  /**
  ...
  private processTooltip(e: PointerEvent | TouchEvent): void {
    ...
    if (spark.type !== 'Pie') {
      const locations: SparkValues[] = extend(
        [],
        [],
        visiblePoints
      ) as SparkValues[];
      const trackerPositions: number[] = locations.map(
        (point: SparkValues): number => {
          return point.location.x;
        }
      );
      let temp: number = Infinity;
      for (
        let i: number = 0, diff: number, len: number = trackerPositions.length;
        i < len;
        i++
      ) {
    ...
    if (
      ...
      !withInBounds(
        mouseX,
        mouseY,
        new Rect(0, 0, spark.availableSize.width, spark.availableSize.height)
      )
    ) {
      this.removeTracker();
      this.removeTooltip();
  ...
  private renderTrackerLine(points: SparkValues): void {
    const spark: Sparkline = this.sparkline;
    const tracker: TrackLineSettingsModel =
      spark.tooltipSettings.trackLineSettings;
    const color: string = spark.sparkTheme.trackerLineColor
      ? spark.sparkTheme.trackerLineColor
      : tracker.color;
    if (!tracker.visible || spark.type === 'Pie') {
      return;
    }
    let group: Element = getIdElement(
      spark.element.id + '_sparkline_tracker_g'
    );
    ...
    const pathEle: Element = getIdElement(
      spark.element.id + '_sparkline_tracker'
    );
    const d: string =
      'M ' +
      points.location.x +
  ...
  public renderTooltip(points: SparkValues): void {
    ...
    let div: Element = getIdElement(
      spark.element.id + '_sparkline_tooltip_div'
    );
    ...
    const text: string[] | HTMLElement = this.getFormat(
      spark.tooltipSettings.format,
      spark,
      x,
      this.formatValue(points.yVal, spark).toString()
    );
    ...
    location =
      spark.type === 'Pie'
        ? { x: points.location.x, y: points.location.y }
        : location;
    const textColor: string =
      tooltip.textStyle.color || spark.sparkTheme.tooltipFontColor;
    const backgroundColor: string =
      tooltip.fill === '' ? spark.sparkTheme.tooltipFill : tooltip.fill;
    const tooltipEvent: ITooltipRenderingEventArgs = {
      ...
      textStyle: {
        ...
        opacity:
          spark.sparkTheme.tooltipTextOpacity || tooltip.textStyle.opacity,
        fontWeight: tooltip.textStyle.fontWeight,
        fontStyle: tooltip.textStyle.fontStyle,
        fontFamily:
          tooltip.textStyle.fontFamily || spark.sparkTheme.tooltipFontFamily,
        color: textColor,
      },
    };
  ...
  private addTooltip(
    tooltipEvent: ITooltipRenderingEventArgs,
    spark: Sparkline,
    backgroundColor: string,
    tooltip: SparklineTooltipSettingsModel,
    location: { x: number; y: number },
    div: Element,
    eventArgs?: ITooltipRenderingEventArgs
  ): void {
    let cancel: boolean;
    let arg: object;
    let tootipArgs: ITooltipRenderingEventArgs;
    if (!isNullOrUndefined(tooltipEvent)) {
      const { cancel: c, ...otherArgs }: ITooltipRenderingEventArgs =
        tooltipEvent;
      cancel = c;
    ...
    const element: Tooltip = new Tooltip({
      ...
      areaBounds: new Rect(
        0,
        0,
        spark.availableSize.width,
        spark.availableSize.height
      ),
      theme: spark.theme,
    });
  ...
  private removeTracker(): void {
    const tracker: Element = this.sparkline.element.querySelector(
      '#' + this.sparkline.element.id + '_sparkline_tracker_g'
    );
  ...
  private removeTooltip(): void {
    ...
    const tooltip: Element = this.sparkline.element.querySelector(
      '#' + this.sparkline.element.id + '_sparkline_tooltip_div'
    );
