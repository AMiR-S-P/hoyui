...
export class SparklineRenderer {
  ...
  public processData(): void {
    ...
    if (isNullOrUndefined(data) || !data.length) {
      ...
      !isNaN(this.sparkline.dataSource[0] as number) ||
      this.sparkline.valueType === 'Numeric'
    ) {
  ...
  public processDataManager(): void {
    ...
    if (this.sparkline.dataSource instanceof DataManager) {
      ...
      queryModule =
        this.sparkline.query instanceof Query
          ? this.sparkline.query
          : new Query();
  ...
  private processCategory(
    data: Object[] = <object[]>this.sparkline.dataSource,
    x: string = this.sparkline.xName,
    y: string = this.sparkline.yName
  ): void {
    const temp: Object[] = [];
  ...
  public renderSeries(): void {
    ...
    const seriesRenderingSuccess: Function = (
      args: ISeriesRenderingEventArgs
    ) => {
      ...
      if (
        spark.type !== 'Pie' &&
        spark.type !== 'WinLoss' &&
        spark.rangeBandSettings.length
      ) {
        ...
        for (let i: number = 0; i < spark.rangeBandSettings.length; i++) {
          if (
            spark.axisSettings.minY <=
              spark.rangeBandSettings[i as number].startRange ||
            spark.axisSettings.maxY >=
              spark.rangeBandSettings[i as number].endRange
          ) {
  ...
  private rangeBand(
    ...
    let stHeight: number =
      height - (height / this.unitY) * (stValue - this.min) + model.padding.top;
    let edHeight: number =
      height - (height / this.unitY) * (edValue - this.min) + model.padding.top;
    const color: string =
      rangeBandSettings.color || this.sparkline.sparkTheme.rangeBandColor;
  ...
  private renderLine(
    ...
    color =
      this.sparkline.fill === '#00bdae' && this.sparkline.theme === 'Bootstrap4'
        ? this.sparkline.sparkTheme.axisLineColor
        : color;
    const pathOption: PathOption = new PathOption(
      spark.element.id + '_sparkline_line',
      'transparent',
      args.lineWidth,
      color,
      spark.opacity
    ...
    pathOption.d = d;
    pathOption['aria-label'] =
      'Line series with' + points.length + 'data points';
  ...
  private renderPie(
    ...
    const spark: Sparkline = this.sparkline;
    const height: number =
      spark.availableSize.height - (spark.padding.top + spark.padding.bottom);
    const width: number =
      spark.availableSize.width - (spark.padding.left + spark.padding.right);
    ...
    const colors: string[] = spark.palette.length
      ? spark.palette
      : getSeriesColor(this.sparkline.theme);
    const group: Element = this.sparkline.renderer.createGroup({
      id: spark.element.id + '_sparkline_g',
    });
    let low: number;
    ...
    if (spark.highPointColor || spark.lowPointColor) {
      const pointsYvalues: number[] = locations.map((a: SparkValues) => {
        return a.yVal;
      });
    ...
    for (
      let i: number = 0, stDeg: number = 90, edDeg: number, flag: string;
      i < points.length;
      i++
    ) {
      stDeg += deg;
      ...
      if (!pointArgs.cancel) {
        ...
        element.setAttribute(
          'aria-label',
          spark.dataSource[i as number][spark.xName] +
            ' : ' +
            points[i as number].yVal
        );
      ...
      points[i as number].location.x = mid.x;
      points[i as number].location.y = mid.y;
    }
    this.sparkline.svgObject.appendChild(group);
  }
  /**
  ...
  private renderArea(
    ...
    let pathOption: PathOption = new PathOption(
      spark.element.id + '_sparkline_area',
      args.fill,
      0,
      'transparent',
      spark.opacity
    ...
    pathOption.d = d;
    pathOption['aria-label'] =
      'Area series with' + points.length + 'data points';
    drawPath(this.sparkline, pathOption, group);
    pathOption = new PathOption(
      spark.element.id + '_sparkline_area_str',
      'transparent',
      args.border.width,
      args.border.color,
      spark.opacity,
  ...
  private renderColumn(
    ...
    if (this.sparkline.highPointColor || this.sparkline.lowPointColor) {
      const pointsYPos: number[] = locations.map((a: SparkValues) => {
    ...
    for (let i: number = 0; i < len; i++) {
      ...
      temp.location.y =
        temp.markerPosition <= this.axisHeight ? temp.y : temp.y + temp.height;
      temp.location.x = temp.x + temp.width / 2;
      rectOptions.stroke = args.border.color
        ? args.border.color
        : rectOptions.fill;
      ...
      this.sparkline.trigger('pointRendering', pointArgs, () => {
        ...
        temp.location.y =
          temp.markerPosition <= this.axisHeight
            ? temp.y
            : temp.y + temp.height;
  ...
  private renderWinLoss(
    ...
    for (let i: number = 0; i < len; i++) {
      temp = points[i as number];
      options.id = id + i;
      options.fill = paletteLength
        ? spark.palette[i % paletteLength]
        : temp.yVal === this.axisValue
        ? this.sparkline.tiePointColor || '#a216f3'
        : temp.yVal > this.axisValue
        ? args.fill || colors[i % colors.length]
      ...
      temp.location.x = temp.x + temp.width / 2;
      temp.location.y =
        temp.yVal >= this.axisValue ? temp.y : temp.y + temp.height;
      const pointArgs: ISparklinePointEventArgs = this.triggerPointRender(
        'pointRendering',
        i,
        options.fill,
        { color: options.stroke, width: args.border.width }
      );
  ...
  private renderMarker(points: SparkValues[]): void {
    ...
    if (
      visible.toLowerCase().indexOf('high') > -1 ||
      visible.toLowerCase().indexOf('low') > -1
    ) {
      const pointsYPos: number[] = locations.map((a: SparkValues) => {
        return a.markerPosition;
      });
    ...
    for (let i: number = 0, length: number = points.length; i < length; i++) {
      ...
      this.sparkline.trigger('markerRendering', markerArgs, () => {
        if (render && !markerArgs.cancel) {
          ...
          if (
            this.sparkline.type.indexOf('Line') > -1 ||
            this.sparkline.type.indexOf('Area') > -1
          ) {
            element.setAttribute('tabindex', i === 0 ? '0' : '-1');
          }
          group.appendChild(element);
  ...
  private renderLabel(points: SparkValues[]): void {
    ...
    const color: string =
      dataLabel.textStyle.color || spark.sparkTheme.dataLabelColor;
    ...
    const padding: number =
      dataLabel.fill !== 'transparent' || dataLabel.border.width ? 2 : 0;
    let size: Size = measureText(
      'sparkline_measure_text',
      labelStyle,
      this.sparkline.sparkTheme.dataLabelFont
    );
    const rectOptions: RectOption = new RectOption(
      '',
      dataLabel.fill,
    ...
    for (let i: number = 0, length: number = points.length; i < length; i++) {
      ...
      option.y =
        (spark.type === 'Pie'
          ? temp.location.y
          : temp.markerPosition > this.axisHeight
          ? temp.location.y + size.height / 2 + space + 2 + padding
          : temp.location.y - size.height / 2 - space - padding) +
        dataLabel.offset.y;
      option.text =
        dataLabel.format !== ''
          ? this.formatter(
              dataLabel.format,
              this.sparkline.dataSource[i as number]
            )
          : temp.yVal.toString();
      ...
      this.sparkline.trigger('dataLabelRendering', labelArgs, () => {
        ...
        let renderLabel: boolean =
          dataLabel.visible.join().toLowerCase().indexOf('all') > -1;
        renderLabel = this.getLabelVisible(
          renderLabel,
          temp,
          i,
          dataLabel,
        ...
        edgeLabelOption = this.arrangeLabelPosition(
          dataLabel.edgeLabelMode,
          renderLabel,
          labelArgs.x,
          i,
          length,
        ...
        if (renderLabel && !labelArgs.cancel && edgeLabelOption.render) {
          ...
          rectOptions.rect = new Rect(
            option.x - (size.width / 2 + padding),
            option.y - padding - size.height / 1.75,
  ...
  private arrangeLabelPosition(
    ...
    if (index === 0 && x - size.width / 2 - padding <= 0) {
      ...
      index === length - 1 &&
      x + size.width / 2 + padding >= this.sparkline.availableSize.width
    ) {
      if (edgeLabel === 'Hide') {
        render = false;
      } else {
  ...
  private getLabelVisible(
    ...
    temp: SparkValues,
    i: number,
    label: SparklineDataLabelSettingsModel,
    length: number,
    highPos: number,
    lowPos: number
  ): boolean {
  ...
  private getInterval(data: Object[], x: string): number {
    ...
    const x2: number = isNullOrUndefined(data[1])
      ? undefined
      : data[1][x as string];
    if (!isNullOrUndefined(x1) && !isNullOrUndefined(x2)) {
      ...
      validData.sort((a: object, b: object) => {
        if (
          isNullOrUndefined(a[x as string]) ||
          isNullOrUndefined(b[x as string])
        ) {
          return 0;
        }
  ...
  private findRanges(data: Object[]): void {
    ...
    if (isNumericArray) {
      ...
      if (model.type === 'Pie') {
        ...
        if (isNullOrUndefined(data[0][model.xName])) {
          const x: number[] = data.map((z: object) => {
          ...
          temp = temp.sort((a: object, b: object) => {
    ...
    let x1: number = 0;
    let y1: number;
    const padding: PaddingModel = model.padding;
    let point: SparkValues;
    ...
    if (model.type !== 'Pie') {
      ...
      const color: string =
        axis.lineSettings.color || this.sparkline.sparkTheme.axisLineColor;
    ...
    const interVal: number = this.getPaddingInterval(
      data,
      model.xName,
      model.valueType,
      delta
    ...
    for (let i: number = 0; i < maxPointsLength; i++) {
      if (
        isNullOrUndefined(data[i as number][model.xName]) &&
        isNullOrUndefined(data[i as number][model.yName]) &&
        data[i as number][model.yName] !== 0 &&
        isNumericArray
      ) {
        x = i;
        y = data[i as number] as number;
      } else if (isNullOrUndefined(data[i as number][model.xName])) {
        x = i;
        y = data[i as number][model.yName];
      } else {
        x = data[i as number][model.xName];
        y = data[i as number][model.yName];
      }
      ...
      if (model.type === 'Line' || model.type === 'Area') {
        y2 =
          min !== max && maxPointsLength !== 1
            ? height - Math.round(height * ((y - min) / this.unitY))
            : padding.top;
        point = {
          x:
            minX !== maxX
              ? Math.round(width * ((x - minX) / this.unitX))
              : width / 2,
          y: y2,
          markerPosition: y2,
        };
        ...
        if (model.type === 'WinLoss') {
          ...
          if (i === 0 && model.rangePadding !== 'None') {
            min -=
              model.rangePadding === 'Additional'
                ? interVal + padding.top
                : interVal;
            max +=
              model.rangePadding === 'Additional'
                ? interVal + padding.top
          ...
          const z1: number =
            y === min && y > value
              ? maxPointsLength !== 1 && this.unitY !== 1
                ? (height / this.unitY) * (min / 2)
                : z | 1
              : y === max &&
                y < value &&
                maxPointsLength !== 1 &&
                this.unitY !== 1
              ? (height / this.unitY) * (-max / 2)
              : z;
  ...
  private drawAxis(): void {
    ...
    if (
      spark.type !== 'WinLoss' &&
      spark.type !== 'Pie' &&
      spark.axisSettings.lineSettings.visible
    ) {
      const xAxis: object = {
