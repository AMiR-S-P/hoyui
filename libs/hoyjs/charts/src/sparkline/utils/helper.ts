...
export function calculateSize(sparkline: Sparkline): void {
  const containerWidth: number = !sparkline.element.clientWidth
    ? !sparkline.element.parentElement
      ? 100
      : !sparkline.element.parentElement.clientWidth
      ? window.innerWidth
      : sparkline.element.parentElement.clientWidth
    : sparkline.element.clientWidth;
  const containerHeight: number = !sparkline.element.clientHeight
    ? !sparkline.element.parentElement
      ? 50
      : sparkline.element.parentElement.clientHeight
    : sparkline.element.clientHeight;
  sparkline.availableSize = new Size(
    stringToNumber(sparkline.width, containerWidth) || containerWidth,
    stringToNumber(sparkline.height, containerHeight) ||
      containerHeight ||
      (sparkline.isDevice
...
export function calculateRoundedRectPath(
  ...
  return (
    ...
    (r.x + r.width) +
    ' ' +
    (r.y + r.height - bottomRight) +
    ' Q ' +
    (r.x + r.width) +
    ' ' +
    (r.y + r.height) +
    ' ' +
    (r.x + r.width - bottomRight) +
    ' ' +
    (r.y + r.height) +
    ' ' +
    'L ' +
...
export function drawRectangle(
  ...
  options.d = calculateRoundedRectPath(
    options.rect,
    options.topLeft,
    options.topRight,
    options.bottomLeft,
...
export function measureText(
  text: string,
  font: SparklineFontModel,
  themeStyle?: FontModel
): Size {
  let htmlObject: HTMLElement = document.getElementById(
    'sparklinesmeasuretext'
  );
...
export function renderTextElement(
  ...
  const htmlObject: HTMLElement = <HTMLElement>(
    renderer.createText(textOptions, options.text)
  );
