...
describe('default stock chart', () => {
  ...
  it('checking with stock events with arrow shapes', (done: Function) => {
    stockchart.loaded = (args: Object) => {
      ...
      document
        .getElementById('stockEvents_Series_0_StockEvents_1_Shape')
    ...
    stockchart.stockEvents = [
      ...
      },
      {
        ...
        text: 'C',
        type: 'ArrowRight',
        description: 'Market starts',
      },
      {
        date: new Date(2017, 5, 31),
        text: 'D',
        type: 'ArrowLeft',
        description: 'Marker ends',
      },
    ];
    stockchart.refresh();
  });
  it('checking with stock events tooltip', (done: Function) => {
    stockchart.loaded = (args: Object) => {
      ...
      trigger.mousemovetEvent(
        element.childNodes[0] as Element,
        pointLocation.x,
        pointLocation.y + 40
      );
      expect(
        getElement('stockEvents_StockEvents_Tooltip_text').textContent
      ).toEqual('Stock events tooltip');
      done();
    };
    stockchart.stockEvents = [
      {
        date: new Date(2017, 2, 31),
        text: 'Market',
        description: 'Stock events tooltip',
      },
    ];
    stockchart.tooltipRender = (args: ITooltipRenderEventArgs) => {
      args.cancel = false;
    };
    stockchart.refresh();
  });
  ...
  it('checking with successive tooltip cases', (done: Function) => {
    stockchart.loaded = (args: Object) => {
      ...
      expect(
        getElement('stockEvents_Series_0_StockEvents_13_Text').textContent
      ).toEqual('A');
      expect(
        getElement('stockEvents_StockEvents_Tooltip_text').textContent
      ).toEqual('This is event description');
      pointLocation = stockchart.stockEvent.symbolLocations[0][14];
      expect(
        getElement('stockEvents_Series_0_StockEvents_14_Text').textContent
      ).toEqual('C');
      trigger.mousemovetEvent(
    ...
    stockchart.stockEvents = [
      ...
      },
      {
        ...
        text: 'Q3',
        description: '2012 Quarter3 starts',
      },
      {
      ...
      {
        date: new Date(2012, 12, 0),
        text: 'Q1',
        description: '2013 Quarter1 starts',
        type: 'Pin',
        showOnSeries: false,
      },
      {
      ...
      {
        date: new Date(2013, 12, 0),
        text: 'Q1',
        description: '2014 Quarter1 starts',
        type: 'Pin',
        showOnSeries: false,
      },
      {
      ...
      {
        date: new Date(2014, 12, 0),
        text: 'Q1',
        description: '2015 Quarter1 starts',
        type: 'Pin',
        showOnSeries: false,
      },
      {
