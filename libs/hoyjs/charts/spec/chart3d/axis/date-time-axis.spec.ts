...
describe('Chart Control', () => {
  ...
  let data = [
    { x: new Date(2004, 3, 6, 1, 10), y: 10 },
    { x: new Date(2004, 3, 6, 1, 20), y: 30 },
    { x: new Date(2004, 3, 6, 1, 14), y: 15 },
    { x: new Date(2004, 3, 6, 1, 10), y: 65 },
    { x: new Date(2004, 3, 6, 1, 12), y: 90 },
    { x: new Date(2004, 3, 6, 1, 15), y: 85 },
  ];
  let hourData = [
    { x: new Date(2004, 3, 6, 2, 10), y: 10 },
    { x: new Date(2004, 3, 6, 1, 20), y: 30 },
    { x: new Date(2004, 3, 6, 1, 14), y: 15 },
    { x: new Date(2004, 3, 6, 5, 10), y: 65 },
    { x: new Date(2004, 3, 6, 3, 12), y: 90 },
    { x: new Date(2004, 3, 6, 4, 15), y: 85 },
  ];
  let ele: HTMLElement;
  let svg: HTMLElement;
  ...
  describe(' Date time Axis Behavior', () => {
    ...
    it('Checking Xaxis title Rotation', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(area.getAttribute('transform')).toBe(
          'rotate(90,403.6232612055641,428.86652941460585)'
        );
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Checking Yaxis title Rotation', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(area.getAttribute('transform')).toBe(
          'rotate(90,13.802936630602783,191.54195445324575)'
        );
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Checking Xaxis label rotation', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(label.getAttribute('transform')).toBe(
          'rotate(45,169.24072642967542,372.07008172997735)'
        );
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Checking Yaxis label rotation', (done: Function) => {
      loaded = (args: Object): void => {
        let label = document.getElementById('chartContainer-1-axis-label-0');
        expect(label.getAttribute('transform')).toBe(
          'rotate(45,60.52462913595727,342.68160741885623)'
        );
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('checking minor gridlines', (done: Function) => {
      chartObj.loaded = (args: Object): void => {
        let tick: Element = document.getElementById(
          'chartContainer-1-grid-lines-3'
        );
        expect(
          tick.getBoundingClientRect().top == 304.3954162597656 ||
            tick.getBoundingClientRect().top == 187.1382293701172
        ).toBe(true);
        done();
      };
    ...
    it('checking first grid lines', (done: Function) => {
      chartObj.loaded = () => {
        ...
        expect(
          path === '85.24346257889991' || path === '78.73164687177996'
        ).toBe(true);
        expect(
          path1 === '24.34625788999098' || path1 === '335.1628166915052'
        ).toBe(true);
        expect(
          path2 === '85.24346257889991' || path2 === '723.4988822652757'
        ).toBe(true);
        expect(
          path3 === '403.2551848512173' || path3 === '335.1628166915052'
        ).toBe(true);
        done();
      };
      chartObj.refresh();
    ...
    it('checking invervaltype as minutes', (done: Function) => {
      chartObj.loaded = () => {
        ...
        expect(
          label.textContent === '12:00:00' || label.textContent === '00:00:00'
        ).toBe(true);
        done();
      };
      (chartObj.primaryXAxis = {
    ...
    it('checking invervaltype as auto', (done: Function) => {
      chartObj.loaded = () => {
        ...
        expect(parseInt(label.getAttribute('y'))).toBe(243);
        expect(
          label.textContent === 'Nov 1999' || label.textContent === '1999'
        ).toBe(true);
        done();
      };
      (chartObj.primaryXAxis = {
