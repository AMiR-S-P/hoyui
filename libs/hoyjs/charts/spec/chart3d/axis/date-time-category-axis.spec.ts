  ...
  describe(' Date time category Axis Behavior', () => {
    ...
    it('Checking Xaxis title Rotation', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(area.getAttribute('transform')).toBe(
          'rotate(90,408.0363214837712,428.86652941460585)'
        );
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Checking Yaxis title Rotation', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(area.getAttribute('transform')).toBe(
          'rotate(90,13.802936630602783,198.89705491692425)'
        );
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Checking Yaxis label rotation', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(label.getAttribute('transform')).toBe(
          'rotate(45,66.76563808317346,377.98608964451313)'
        );
        done();
      };
      chartObj.loaded = loaded;
      ...
      chartObj.loaded = (args: Object): void => {
        ...
        expect(
          tick.getBoundingClientRect().top == 304.3954162597656 ||
            tick.getBoundingClientRect().top == 282.6441955566406
        ).toBe(true);
        done();
      };
    ...
    it('checking first grid lines', (done: Function) => {
      chartObj.loaded = () => {
        ...
        expect(
          path === '85.24346257889991' || path === '84.74943046171867'
        ).toBe(true);
        expect(
          path1 === '24.34625788999098' || path1 === '369.20454545454544'
        ).toBe(true);
        expect(
          path2 === '85.24346257889991' || path2 === '714.9884500745156'
        ).toBe(true);
        expect(
          path3 === '403.2551848512173' || path3 === '369.20454545454544'
        ).toBe(true);
        done();
      };
      chartObj.refresh();
    ...
    it('Checking inverval type as Minutes with date data', (done: Function) => {
      chartObj.loaded = (args: Object): void => {
        ...
        expect(parseInt(label.getAttribute('y'))).toBe(237);
        expect(
          label.textContent === '00:15:00' || label.textContent === '00:30:00'
        ).toBe(true);
        done();
      };
      chartObj.primaryXAxis = {
