...
import { EmitType, containerObject, createElement } from '@hoyui/base';
...
export const categoryData1: object[] = [
  { x: 'USA', y: 70 },
  { x: 'China', y: 60 },
  { x: 'Japan', y: 60 },
  { x: 'Australia', y: 56 },
...
describe('Chart Control', () => {
  ...
  describe('Chart direct properties and its behavior', () => {
    ...
    it('Checking with empty options', () => {
      ...
      expect(className).toEqual(
        'e-control e-chart3d e-lib e-chart-focused e-touch'
      );
    });
    ...
    it('Checking with title', () => {
      ...
      expect(
        text.getAttribute('y') === '25' || text.getAttribute('y') === '23.5'
      ).toBe(true);
    });
    ...
    it('Trimmed text and mouse over and out', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let tooltip: Element = document.getElementById(
          'Chart3d-EJ2-title-tooltip'
        );
        expect(tooltip.textContent).toBe(
          'candidate joined in a year syncfusion Chart Title'
        );
        expect(text.textContent.split('...').length).toEqual(2);
        tooltip.remove();
        chart.mouseEnd(
          <PointerEvent>trigger.onTouchEnd(text, 0, 0, null, null, 77, 25)
        );
    ...
    it('Trimmed text and mouse over and out for subtitle', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        trigger.mousemoveEvent(text, 0, 0, 77, 25);
        const tooltip: Element = document.getElementById(
          'Chart3d-EJ2-title-tooltip'
        );
    ...
    it('Checking with subtitle', function () {
      ...
      expect(
        text.getAttribute('y') === '52' || text.getAttribute('y') === '51.25'
      ).toBe(true);
    });
    ...
    it('Checking textAlignment subtitle center and subtitle is in Title width', function () {
      ...
      chart.subTitle =
        'Checking Syncfusion Chart SubTitle width should be in Chart Title width';
      chart.subTitleStyle.textOverflow = 'Trim';
      chart.titleStyle.textOverflow = 'Wrap';
      chart.dataBind();
      text = document.getElementById('Chart3d-chart-sub-title');
    ...
    it('checking with title alignment default', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        expect(text.getAttribute('text-anchor')).toBe('middle');
        expect(
          text.getAttribute('x') === text.children[0].getAttribute('x')
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
  ...
  describe('DataBind', () => {
    ...
    it('checking default fill property', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        element = document.getElementById(
          'seriesData-svg-0-region-series-0-point-0'
        );
    ...
    it('checking before the legend name chage', () => {
      ...
      expect(
        element.getAttribute('x') === '826' ||
          element.getAttribute('x') === '832'
      ).toBe(true);
    ...
    it('checking with changing name', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        element = document.getElementById('seriesData_chart_legend_element');
        expect(
          element.getAttribute('x') === '733' ||
            element.getAttribute('x') === '757'
        ).toBe(true);
        element = document.getElementById('seriesData_chart_legend_text_0');
        expect(
          element.textContent === 'Olymbic gold medal' ||
            element.textContent === ''
        ).toEqual(true);
        done();
      };
  ...
  describe('Check the RTL behaviour for title', () => {
    ...
    it('Checking the title with RTL and bottom position', (done: Function) => {
      loaded = (args: Object): void => {
        const titleElement: Element = document.getElementById(
          'RTLcontainer-chart-title'
        );
        const subTitleElement: Element = document.getElementById(
          'RTLcontainer-chart-sub-title'
        );
  ...
  describe('OnProperty Changed', () => {
    ...
    it('checking series fill', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        element = document.getElementById(
          'chart3DContainer-svg-0-region-series-0-point-0'
        );
    ...
    it('checking with legend settings', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        const legendElement: Element = document.getElementById(
          'chart3DContainer_chart_legend_g_0'
        );
    ...
    it('change palettes', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        const seriesElement: Element = document.getElementById(
          'chart3DContainer-svg-0-region-series-0-point-0'
        );
        const seriesElement1: Element = document.getElementById(
          'chart3DContainer-svg-4-region-series-0-point-2'
        );
    ...
    it('change tilt', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        expect(seriesElement.getAttribute('d')).toBe(
          'M 73.97330780218424 157.2247829905249 L 73.97330780218424 157.2247829905249 L 146.40061263184265 157.2247829905249 L 169.29937714799362 336.81329243051357 L 102.33484947414897 336.81329243051357 '
        );
        done();
      };
      chart.tilt = 30;
    ...
    it('change rotation', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        expect(seriesElement.getAttribute('d')).toBe(
          'M 163.97565130722325 151.25658195743833 L 163.97565130722325 151.25658195743833 L 212.5479146623298 150.10292875667125 L 212.5479146623298 344.4388904412149 L 163.97565130722325 341.09936801794174 '
        );
        done();
      };
      chart.tilt = 0;
    ...
    it('change depth', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        expect(
          seriesElement.getAttribute('d') ===
            'M 105.45454545454547 148.1818181818182 L 105.45454545454547 148.1818181818182 L 171.81818181818184 148.1818181818182 L 171.81818181818184 350 L 105.45454545454547 350 '
        ).toBe(true);
        done();
      };
      chart.rotation = 0;
    ...
    it('change perspective angle', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        expect(
          seriesElement.getAttribute('d') ===
            'M 76.65484154271743 143.85046429534273 L 76.65484154271743 143.85046429534273 L 148.56565042762938 143.85046429534273 L 148.56565042762938 362.53812967137634 L 76.65484154271743 362.53812967137634 '
        ).toBe(true);
        done();
      };
      chart.depth = 100;
    ...
    it('change enableSideBySidePlacement', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        expect(seriesElement.getAttribute('d')).toBe(
          'M 93.11864406779664 145.89830508474577 L 93.11864406779664 145.89830508474577 L 155.76271186440678 145.89830508474577 L 155.76271186440678 356.6101694915255 L 93.11864406779664 356.6101694915255 '
        );
        done();
      };
    ...
    it('Checking addSeries method', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        const seriesElement: Element = document.getElementById(
          'chart3DContainer-svg-0-region-series-1-point-0'
        );
        expect(seriesElement.getAttribute('d')).toBe(
          'M 126.33898305084746 145.89830508474577 L 126.33898305084746 145.89830508474577 L 157.66101694915255 145.89830508474577 L 157.66101694915255 356.6101694915255 L 126.33898305084746 356.6101694915255 '
        );
        done();
      };
      chart.loaded = loaded;
    ...
    it('change data source', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        expect(seriesElement.getAttribute('d')).toBe(
          'M 93.11864406779664 61.42372881355933 L 93.11864406779664 61.42372881355933 L 155.76271186440678 61.42372881355933 L 155.76271186440678 356.6101694915255 L 93.11864406779664 356.6101694915255 '
        );
        done();
      };
    ...
    it('Selected Data Indexes', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        expect(
          seriesElement.getAttribute('class') ===
            'chart3DContainer_ej2_chart_selection_series_0'
        ).toBe(true);
        done();
      };
  ...
  describe('Checking Wall Brush', () => {
        ...
        series: [
          {
            ...
            type: 'Column',
            animation: { enable: true, duration: 0 },
            columnWidth: 0.7,
            fill: '#E94649',
          },
        ],
    ...
    it('checking Back wall', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        element = document.getElementById(
          'chart3DContainer-svg-1-back-wall-brush'
        );
        expect(element.getAttribute('d')).toEqual(
          'M 83.68327402135232 98.15302491103205 L 83.68327402135232 98.15302491103205 L 798.3807829181495 98.15302491103205 L 798.3807829181495 352.79359430604984 L 83.68327402135232 352.79359430604984 '
        );
        done();
      };
      chart.loaded = loaded;
    ...
    it('checking chart mousedone event', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        args.chart.chartOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(element, 100, 100, 100, 100, 100, 100)
          )
        );
        args.chart.mouseLeave(
          <PointerEvent>(
            trigger.onTouchEnd(element, 100, 100, 100, 100, 100, 100)
          )
        );
        args.chart.chartOnMouseLeave(
          <PointerEvent>(
            trigger.onTouchEnd(element, 100, 100, 100, 100, 100, 100)
          )
        );
    ...
    it('checking chart 3D vector points', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        args.chart.polygon.polygon3D(
          args.chart.polygons[1].points,
          args.chart,
          0
        );
        args.chart.isTouch = false;
        (args.chart.series[0] as any).xAxis = null;
        args.chart.svg3DRenderer.transform3DToVisible(
          args.chart.series[0] as any,
          0,
          0,
          args.chart
        );
    ...
    it('checking 3d renderer color ', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        args.chart.bspTreeObj.classifyPolygon(
          args.chart.polygons[0],
          args.chart.polygons[0]
        );
  ...
  describe('Chart databind properites', () => {
    ...
    it('checking 3D chart keyboard navigations', (done: Function) => {
      loaded = (args: Chart3DLoadedEventArgs) => {
        ...
        let events: any = <KeyboardEvent>(
          trigger.onTouchEnd(element, 100, 100, 100, 100, 100, 100)
        );

        let legendElement: any = document.getElementById(
          'chart3DContainer_chart_legend_translate_g'
        );
        if (legendElement) {
          legendElement.firstElementChild.setAttribute('class', 'e-chart');
          events = <KeyboardEvent>(
            trigger.onTouchEnd(legendElement, 100, 100, 100, 100, 100, 100)
          );
          events.code = 'Tab';
        ...
        if (
          document.getElementById(
            args.chart.element.id + '-svg-0-region-series-0-point-0'
          )
        ) {
          document.getElementById(
            args.chart.element.id + '-svg-0-region-series-0-point-0'
