...
import {
  AccPoints,
  AccumulationSeries,
} from '../../../src/accumulation-chart/model/acc-base';
...
describe('Accumulation Chart Control', () => {
  ...
  describe('Funnel Series checking', () => {
    ...
    let seriesId: string = id + '_Series_0';
    ...
    it('Checking default funnel size', (done: Function) => {
      ...
      expect(
        (bounds.width === 464 || bounds.width == 468) &&
          (bounds.height === 304 || bounds.height === 308)
      ).toBe(true);
      done();
    });
    ...
    it('Checking default neck size', (done: Function) => {
      ...
      let bounds: ClientRect = document
        .getElementById('ej2container_Series_0_Point_0')
        .getBoundingClientRect();
      expect(
        bounds.width === 116 &&
          (bounds.height === 19.887859344482422 ||
            bounds.height === 23.899993896484375 ||
            bounds.height == 19.887847900390625)
      ).toBe(true);
      bounds = document
        .getElementById('ej2container_Series_0_Point_1')
        .getBoundingClientRect();
      expect(
        bounds.width === 116 &&
          (bounds.height === 29.831787109375 ||
            bounds.height === 33.83178710937)
      ).toBe(true);
      bounds = document
        .getElementById('ej2container_Series_0_Point_2')
        .getBoundingClientRect();
      expect(
        bounds.width === 169.1215057373047 &&
          (bounds.height === 61.0841064453125 ||
            bounds.height === 65.0841064453125)
      ).toBe(true);
      done();
    });
    ...
    it('Checking default legend position', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(
          group.getAttribute('x') === '75.5' ||
            group.getAttribute('x') === '85.5'
        ).toBe(true);
        expect(group.getAttribute('y')).toBe('11');
        expect(
          group.getAttribute('width') === '449' ||
            group.getAttribute('width') === '429'
        ).toBe(true);
        expect(
          group.getAttribute('height') === '33' ||
            group.getAttribute('height') === '32'
        ).toBe(true);
        done();
      };
    ...
    it('Checking default legend shape', (done: Function) => {
      ...
      expect(
        group.getAttribute('d') ===
          'M 93.5 22 L 88.5 32 L 83.5 22 L 93.5 22 z' ||
          group.getAttribute('d') ==
            'M 103.5 22 L 98.5 32 L 93.5 22 L 103.5 22 z'
      ).toBe(true);
      done();
    });
    ...
    it('Funnel series with average empty point mode', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(Math.floor(group.getBoundingClientRect().height) == 53).toBe(
          true
        );
        //reset data source
        data.splice(3, 1);
    ...
    it('Funnel segments with gaps', (done: Function) => {
      chart.loaded = () => {
        ...
        let bounds: ClientRect = group.getBoundingClientRect();
        expect(
          Math.floor(bounds.top) === 255 || Math.floor(bounds.top) == 254
        ).toBe(true);
    ...
    it('Funnel series as an inverted triangle', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(
          group.getAttribute('d') ===
            'M284.822429906542 337.0630841121495 L315.1775700934579 337.0630841121495 L300 355.25 L300 355.25 L300 355.25 L300 355.25' ||
            group.getAttribute('d') ===
              'M284.822429906542 337.0630841121495 L315.1775700934579 337.0630841121495 L300 355.25 L300 355.25 L300 355.25 L300 355.25'
        );
    ...
    it('Funnel series without neck and with a base', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(
          group.getAttribute('d') ===
            'M230.61682242990653 337.0630841121495 L369.3831775700935 337.0630841121495 L358 355.25 L358 355.25 L242 355.25 L242 355.25' ||
            group.getAttribute('d') ===
              'M230.61682242990653 337.0630841121495 L369.3831775700935 337.0630841121495 L358 355.25 L358 355.25 L242 355.25 L242 355.25'
        );
        done();
    ...
    it('Funnel series with default data label', (done: Function) => {
      chart.loaded = () => {
        //debugger
        let group: Element = getElement(
          'ej2container_datalabel_Series_0_text_3'
        );
        expect(
          group.getAttribute('x') === '291' ||
            group.getAttribute('x') === '291.5'
        );
        expect(
          group.getAttribute('y') === '220.80251617541336' ||
    ...
    it('Funnel series with outside labels', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(
          group.getAttribute('y') === '219.551509705248' ||
            group.getAttribute('y') === '218.801509705248'
        ).toBe(true);
        done();
      };
    ...
    it('Check the default connector line', (done: Function) => {
      chart.loaded = () => {
        //debugger
        let group: Element = getElement(
          'ej2container_datalabel_Series_0_connector_2'
        );
    ...
    it('Data labels with connector (custom length)', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(
          group.getAttribute('d') ==
            'M 403.9392523364486 282.31962616822426 L 445.4392523364486 282.31962616822426 L 455.4392523364486 282.31962616822426' ||
            group.getAttribute('d') ===
              'M 403.9392523364486 282.00841121495324 L 445.4392523364486 282.00841121495324 L 455.4392523364486 282.00841121495324' ||
            group.getAttribute('d') ===
              'M 403.9392523364486 282.00841121495324 L 446.4392523364486 282.00841121495324 L 456.4392523364486 282.00841121495324'
        ).toBe(true);
        done();
    ...
    it('Data labels with custom connector', (done: Function) => {
      chart.loaded = () => {
        //debugger
        let group: Element = getElement(
          'ej2container_datalabel_Series_0_connector_1'
        );
        expect(group.getAttribute('fill')).toBe('transparent');
    ...
    it('Checking smart labels for inside position', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(
          group.getAttribute('y') === '219.55988372093023' ||
    ...
    it('Checking smart labels for outside position', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(group.getAttribute('x')).toBe('545');
        expect(
          group.getAttribute('y') == '254.35996784565918' ||
            group.getAttribute('y') === '256.6099678456592'
        ).toBe(true);
        done();
      };
    ...
    it('Checking overlapped labels for inside position', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(
          group.getAttribute('y') === '265.433922829582' ||
            group.getAttribute('y') === '264.683922829582'
        ).toBe(true);
        done();
      };
      //fourth label should be outside
    ...
    it('Checking overlapped labels for outside position', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(group.getAttribute('x')).toBe('545');
        expect(
          group.getAttribute('y') === '264.433922829582' ||
            group.getAttribute('y') === '263.683922829582'
        ).toBe(true);
        done();
      };
      //a label will be hidden
    ...
    it('Checking default explode offset', (done: Function) => {
      chart.loaded = () => {
        let group: Element = getElement(sliceid + '2');
        expect(group.getAttribute('transform') == 'translate(25, 0)').toBe(
          true
        );
        done();
      };
    ...
    it('Checking custom explode offset', (done: Function) => {
      chart.loaded = () => {
        let group: Element = getElement(sliceid + '2');
        expect(group.getAttribute('transform') == 'translate(30, 0)').toBe(
          true
        );
        done();
      };
    ...
    it('Select a segment on click', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(group.getAttribute('class')).toBe(
          'ej2container_ej2_chart_selection_series_0_point_3'
        );
    ...
    it('Checking custom legend shape', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(
          group.getAttribute('d') ===
            'M 83.5 27 L 88.5 22 L 93.5 27 L 88.5 32 L 83.5 27 z' ||
            group.getAttribute('d') ===
              'M 93.5 27 L 98.5 22 L 103.5 27 L 98.5 32 L 93.5 27 z' ||
            group.getAttribute('d') ===
              'M 103.5 22 L 98.5 32 L 93.5 22 L 103.5 22 z'
        ).toBe(true);
        done();
    ...
    it('Checking custom legend position', (done: Function) => {
      chart.loaded = () => {
        ...
        let group: Element = getElement('ej2container_chart_legend_element');
        expect(
          group.getAttribute('x') == '484' || group.getAttribute('x') == '490'
        ).toBe(true);
        expect(
          group.getAttribute('y') == '133.5' || group.getAttribute('y') == '136'
        ).toBe(true);
        expect(
          group.getAttribute('width') == '106' ||
            group.getAttribute('width') == '100'
        ).toBe(true);
        expect(
          group.getAttribute('height') == '133' ||
            group.getAttribute('height') == '128'
        ).toBe(true);
        done();
      };
    ...
    it('Funnel with legend click', (done: Function) => {
      chart.loaded = () => {
        chart.loaded = null;
        let legendElement: Element = getElement(
          'ej2container_chart_legend_shape_0'
        );
    ...
    it('Legend click on Visible series', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        trigger.clickEvent(legendElement);
        let group: Element = getElement(
          'ej2container_datalabel_Series_0_text_2'
        );
    ...
    it('Checking overlapped labels are placed at outside left position for position as inside', (done: Function) => {
      loaded = (args: Object): void => {
        let group: Element = getElement(
          'ej2container_datalabel_Series_0_text_6'
        );
        expect(
          group.getAttribute('x') === '47.20472756410254' ||
            group.getAttribute('x') === '488.79527243589746'
        ).toBe(true);
        expect(
          group.getAttribute('y') === '202.79615384615386' ||
            group.getAttribute('y') === '180.75128205128203' ||
            group.getAttribute('y') === '180.51666666666668'
        ).toBe(true);
        done();
      };
    ...
    it('Checking data label with Legend position', (done: Function) => {
      loaded = (args: Object): void => {
        let group: Element = getElement(
          'ej2container_datalabel_Series_0_text_11'
        );
        expect(
          group.getAttribute('x') === '118.5' ||
            group.getAttribute('x') === '559.0679086538461'
        ).toBe(true);
        expect(
          group.getAttribute('y') === '115.57051282051282' ||
            group.getAttribute('y') === '84.35897435897436'
        ).toBe(true);
        done();
      };
    ...
    it('Check the default connector line at left outside', (done: Function) => {
      chart.loaded = () => {
        let group: Element = getElement(
          'ej2container_datalabel_Series_0_connector_11'
        );
        expect(group.getAttribute('fill')).toBe('transparent');
    ...
    it('Checking overlapped labels are placed at outside left position for position as outside ', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          group.getAttribute('x') === '458' ||
            group.getAttribute('x') === '488.79527243589746'
        ).toBe(true);
        expect(
          group.getAttribute('y') === '215.4' ||
            group.getAttribute('y') === '205.65615384615387' ||
            group.getAttribute('y') === '219.47384615384613'
        ).toBe(true);
        done();
      };
      chart.series[0].dataLabel.position = 'Outside';
    ...
    it('Checking Funnel default explode for outside left position', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          value.getAttribute('transform') == 'translate(-25, 0)' ||
            value.getAttribute('transform') == 'translate(25, 0)'
        ).toBe(true);
        done();
      };
  ...
  describe('CornerRadius', () => {
    ...
    it('Bottom', (done: Function) => {
      chart.loaded = () => {
        ...
        expect(group.getAttribute('d')).toBe(
          'M 242 316.4192926045016 Q242 316.4192926045016 242 316.4192926045016 L358 316.4192926045016 Q358 316.4192926045016 358 316.4192926045016 L358 334.2096463022508 Q358 352 340.2096463022508 352 L259.7903536977492 352 Q242 352 242 334.2096463022508 Z '
        );
        done();
      };
      chart.series[0].dataSource = data;
    ...
    it('OnePoint', (done: Function) => {
      chart.loaded = () => {
        let group: Element = getElement('ej2container_Series_0_Point_0');
        expect(group.getAttribute('d')).toBe(
          'M 86.20019958050572 71.84853738135232 Q68 48 98 48 L502 48 Q532 48 513.7998004194943 71.84853738135232 L358 276 L358 321.99999999999994 Q358 351.99999999999994 328 351.99999999999994 L272 351.99999999999994 Q242 351.99999999999994 242 321.99999999999994 L242 276 Z '
        );
        done();
      };
  ...
  describe('Funnel Series - Checking animation on data changes.', () => {
    ...
    it('checking funnel series with setdata', (done: Function) => {
      pie.loaded = (args: Object): void => {
        ...
        let element: Element = document.getElementById(
          'ej2container_Series_0_Point_1'
        );
        expect(element.getAttribute('d') !== '').toBe(true);
        console.log('funnel');
        done();
      };
      (pie.series[0] as AccumulationSeries).setData([
        {
          y: 18,
          x: 1,
          name: 'Bald Eagle',
        ...
        { y: 23, x: 2, name: 'Bison', text: 'Bison : 23', radius: '60%' },
        {
          y: 30,
          x: 3,
          name: 'Brown Bear',
        ...
        { y: 44, x: 4, name: 'Elk', text: 'Elk : 44', radius: '100%' },
        {
          y: 52,
          x: 10,
          name: 'Pronghorn',
        ...
        { y: 62, x: 6, name: 'Turkey', text: 'Turkey : 62', radius: '80%' },
        {
          y: 74,
          x: 7,
          name: 'Alligator',
        ...
        },
        {
          ...
          x: 15,
          name: 'Mountain Lion',
          text: 'Mountain Lion : 96',
          radius: '80%',
        },
        { y: 102, x: 10, name: 'Beaver', text: 'Beaver : 102', radius: '80%' },
      ]);
      pie.refresh();
    });
    ...
    it('checking funnel series with addPoint', (done: Function) => {
      ...
      (pie.series[0] as AccumulationSeries).addPoint({
        x: 'Dog',
        y: 15,
        text: '15%',
      });
