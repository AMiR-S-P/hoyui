...
describe('Accumulation Chart Control', () => {
  ...
  describe('accumulation and Doughnut Control Checking', () => {
    ...
    let query: Query = new Query()
      .take(7)
      .where('Estimate', 'greaterThan', 1, false);
    ...
    it('Checking with  subtitle', () => {
      ...
      expect(
        text.getAttribute('y') === '45.5' || text.getAttribute('y') === '41.75'
      ).toEqual(true);
    });
    ...
    it('Checking with title', () => {
      (accumulation.titleStyle.textOverflow = 'Wrap'),
        (accumulation.title =
          'Syncfusion accumulation TitleSyncfusionaccumulationTitleSyncfusionaccumulationTitleSyncfusionaccumulation Title');
      accumulation.dataBind();
      text = getElement(id + '_title');
    ...
    it('Checking with subtitle Overflow is Wrap', () => {
      ...
      accumulation.subTitle =
        'Syncfusion accumulation subTitleSyncfusionaccumulationTitleSyncfusion';
      accumulation.dataBind();
      text = getElement(id + '_subTitle');
    ...
    it('Checking the accumulation with Series datapoints', (done: Function) => {
      loaded = (args: IAccLoadedEventArgs) => {
        let points: AccPoints[] = (<AccumulationSeries>(
          args.accumulation.series[0]
        )).points;
    ...
    it('club points value change check', () => {
      ...
      let points: AccPoints[] = (<AccumulationSeries>accumulation.series[0])
        .points;
    ...
    it('remote data checking', (done: Function) => {
      accumulation.loaded = (args: IAccLoadedEventArgs) => {
        expect(
          (accumulation.series[0] as AccumulationSeries).points.length
        ).toBe(7);
        expect(
          getElement(id + '_Series_0_Point_3').getAttribute('opacity')
        ).toBe('0.2');
        done();
      };
    ...
    it('Background image checking', (done: Function) => {
      setTimeout(() => {
        let background: HTMLElement = document.getElementById(
          id + '_background'
        );
        expect(background.getAttribute('href') != null).toBe(true);
        done();
      }, 500);
      accumulation.backgroundImage =
  ...
  describe('Checking RTL Behaviour for CenterLabel', () => {
    ...
    it('Cheking CenterLabel text', (done: Function) => {
      accumulation.loaded = (args: IAccLoadedEventArgs) => {
        let centerLabel: HTMLElement = document.getElementById(
          'ej2-container_centerLabel'
        );
    ...
    it('Cheking CenterLabel text with br tag', (done: Function) => {
      accumulation.loaded = (args: IAccLoadedEventArgs) => {
        ...
        expect(
          centerLabel.children[0].innerHTML === '<b>Syncfusion</b> accumulation'
        ).toBe(false);
        done();
      };
    ...
    it('Cheking CenterLabel text with endAngle and startAngle', (done: Function) => {
      accumulation.loaded = (args: IAccLoadedEventArgs) => {
        ...
        expect(
          centerLabel.children[0].innerHTML === 'Syncfusion accumulation'
        ).toBe(false);
        done();
      };
  ...
  describe('Checking applyPattern', () => {
    ...
    it('Different Patterns', (done: Function) => {
      accumulation.loaded = (args: IAccLoadedEventArgs) => {
        accumulation.loaded = null;
        const array1: string[] = document
          .getElementById(id + '_svg')
          .querySelectorAll('pattern')[0]
          .id.split('_');
        const array2: string[] = document
          .getElementById(id + '_svg')
          .querySelectorAll('pattern')[1]
          .id.split('_');
    ...
    it('Preferred pattern', (done: Function) => {
      accumulation.loaded = (args: IAccLoadedEventArgs) => {
        accumulation.loaded = null;
        const fillURL: string = document
          .getElementById(id + '_svg')
          .querySelectorAll('pattern')[0].id;
