...
import {
  ...
  SparklineTooltip,
} from '../../src/sparkline/index';
import {
  removeElement,
  getIdElement,
  Rect,
} from '../../src/sparkline/utils/helper';
import { EmitType, createElement, isNullOrUndefined } from '@hoyui/base';
...
describe('Sparkline tooltip and tracker checking Spec', () => {
  ...
  describe('Sparkline tooltip Spec', () => {
    ...
    it('Sparkline tooltip template checking', () => {
      sparkline.tooltipSettings.template =
        '<div style="border: 2px solid green;background: #a0e99680">${id}<br>${value}$</div>';
      sparkline.dataBind();
      ele = getIdElement(id + '_sparkline_pie_4');
      sparkline.isTouch = true;
      sparkline.mouseX = 400;
      sparkline.mouseY = 150;
      sparkline.sparklineTooltipModule['mouseUpHandler']({
        target: ele,
      } as any);
      ...
      expect(
        ele.children[0].innerHTML.indexOf(
          '<div style="border: 2px solid green;background: #a0e99680">50<br>-60$</div>'
        ) > -1
      ).toBe(true);
    });
