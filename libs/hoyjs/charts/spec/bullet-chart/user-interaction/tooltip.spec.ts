  ...
  describe('with default case', () => {
    ...
    it('Checking with tooltip template', (done: Function) => {
      ...
      bullet.tooltip.template =
        '<div>Target   : ${target}K</div><br/><div>Current : ${value}K</div>';
      bullet.refresh();

      segement = document.getElementById(sliceid);
      trigger.mousemoveEvent(segement, 0, 0, 200, 35);
      let tooltip: HTMLElement = document.getElementById(
        'containerparent_template'
      );
      expect(tooltip != null).toBe(true);
      segement = document.getElementById('container_svg_ComparativeMeasure_0');
      trigger.mousemoveEvent(segement, 0, 0, 200, 200);
    ...
    it('Checking bullet target class', (done: Function) => {
      ...
      bullet.loaded = (args: IBulletLoadedEventArgs): void => {
        ...
        args.bulletChart.bulletTooltipModule._elementTooltip(
          <PointerEvent>(
            trigger.onTouchEnd(element, 100, 100, 100, 100, 100, 100)
          ),
          args.bulletChart.svgObject.id + '_Caption',
          null,
          100
        );
        args.bulletChart.bulletTooltipModule._elementTooltip(
          ...
          args.bulletChart.svgObject.id + '_SubTitle',
          null,
          100
        );
        ...
        let tooltipElement =
          args.bulletChart.svgObject.id + '_ComparativeMeasure';
        args.bulletChart.dataSource = [{ value: 0, target: 0 }];
        args.bulletChart.isReact = true;
        args.bulletChart.bulletTooltipModule._displayTooltip(
          <PointerEvent>(
            trigger.onTouchEnd(element, 100, 100, 100, 100, 100, 100)
          ),
          tooltipElement,
          'container_svg_ComparativeMeasure_0',
          0,
          0
        );
