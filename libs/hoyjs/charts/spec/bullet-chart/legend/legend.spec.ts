import {
  ...
  RangeModel,
  BulletChartLegend,
} from '../../../src/bullet-chart/index';
import { createElement, EmitType } from '@hoyui/base';
import { MouseEvents } from '../../chart/base/events.spec';
import { ObjectStatus } from '@hoyui/pdf-export';
...
let rangeCollection: RangeModel[] = [
  { end: 5.2, opacity: 1, color: 'skyblue', name: 'Poor' },
  ...
  describe('with default rendering', () => {
    ...
    it('Single Series Static Name and Multiple series legend text', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        for (
          let i: number = 0, length: number = bullet.ranges.length;
          i < length;
          i++
        ) {
    ...
    it('Style font, background, padding', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(legendElement.getAttribute('font-size')).toEqual(
          bullet.legendSettings.textStyle.size
        );
        expect(legendElement.getAttribute('fill')).toEqual(
          bullet.legendSettings.textStyle.color
        );
        expect(parseFloat(legendElement.getAttribute('opacity'))).toEqual(
          bullet.legendSettings.textStyle.opacity
        );
        expect(legendElement.getAttribute('font-style')).toEqual(
          bullet.legendSettings.textStyle.fontStyle
        );
        expect(legendElement.getAttribute('font-family')).toEqual(
          bullet.legendSettings.textStyle.fontFamily
        );
        expect(legendElement.getAttribute('font-weight')).toEqual(
          bullet.legendSettings.textStyle.fontWeight
        );
        done();
      };
      bullet.legendSettings = {
    ...
    it('Bottom Position', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          value === 439.5 || value === 439 || value === 187 || value === 219
        ).toBe(true);
    ...
    it('Custom X and Y Position', (done: Function) => {
      loaded = (args: Object): void => {
        bullet.loaded = null;
        legendElement = document.getElementById(legendId + '_element');
        let container: Element = document.getElementById(
          bulletElement.id + '_svg'
        );
    ...
    it('Checking with multiple target types', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        legendElement = document.getElementById(
          'container_chart_legend_shape_6'
        );
    ...
    it('Checking with multiple target types in horizontal mode', (done: Function) => {
      loaded = (args: Object): void => {
        bullet.loaded = null;
        legendElement = document.getElementById(
          'container_chart_legend_pagenumber'
        );
        expect(
          legendElement.textContent === '1/2' ||
            legendElement.textContent === '1/4'
        ).toBe(true);
        legendElement = document.getElementById(
          'container_chart_legend_shape_3'
        );
        expect(legendElement.getAttribute('rx')).toBe('4');
        done();
      };
    ...
    it('bulletchart legend on-property-change checking', (done: Function) => {
      ...
      bullet.height = '20';
      bullet.width = '50';
      (bullet.border = { color: 'blue', width: 1 }), bullet.dataBind();
    ...
    it('Checking with range color as empty', (done: Function) => {
      loaded = function (args) {
        bullet.loaded = null;
        legendElement = document.getElementById(
          'container_chart_legend_shape_2'
        );
