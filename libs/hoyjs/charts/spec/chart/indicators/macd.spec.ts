...
let financialData: object[] = [
  ...
  { x: 'May', low: 5.7, high: 14.4, open: 12.1, close: 7, volume: 10 },
  { x: 'Jun', low: 8.4, high: 16.9, open: 15, close: 10, volume: 10 },
  { x: 'Jul', low: 10.6, high: 19.1, open: 15.6, close: 13, volume: 10 },
  ...
  describe('Technical Indicators', () => {
    ...
    it('MACD Technical indicator for a series with valid points & primary axis', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let signalLine: HTMLElement = document.getElementById(
          'container_Indicator_0_MacdLine'
        );
    ...
    it('MACD Technical indicator for a series with negative points', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(Math.round(<number>chartObj.visibleSeries[1].points[1].y)).toBe(
          2
        );
    ...
    it('MACD Technical indicator for a series with secondary y axis', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('d').split(' ')[1] == '1222.5' ||
            svg.getAttribute('d').split(' ')[1] == '717.5' ||
            svg.getAttribute('d').split(' ')[1] == '729.5'
        ).toBe(true);
        expect(svg.getAttribute('d').split(' ')[2] == '10.25').toBe(true);
        expect(
          svg.getAttribute('d').split(' ')[4] == '1222.5' ||
            svg.getAttribute('d').split(' ')[4] == '717.5' ||
            svg.getAttribute('d').split(' ')[4] == '729.5'
        ).toBe(true);
        expect(
          svg.getAttribute('d').split(' ')[5] == '381.5' ||
            svg.getAttribute('d').split(' ')[5] == '379.5'
        ).toBe(true);
    ...
    it('MACD Technical indicator with MACD Type', (done: Function) => {
      loaded = (args: Object): void => {
        //define check condition
        let macdLine: HTMLElement = document.getElementById(
          'container_Indicator_0_MacdLine'
        );
        expect(macdLine).not.toBeNull();
        let histogram: HTMLElement = document.getElementById(
          'container_Indicator_0_Histogram_Group'
        );
    ...
    it('MACD Technical indicator with opposed position', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('d').split(' ')[1] == '57.5' ||
            svg.getAttribute('d').split(' ')[1] == '53.5'
        ).toBe(true);
        expect(
          svg.getAttribute('d').split(' ')[2] == '36.75' ||
            svg.getAttribute('d').split(' ')[2] == '35.75'
        ).toBe(true);
        expect(
          svg.getAttribute('d').split(' ')[4] == '1222.5' ||
            svg.getAttribute('d').split(' ')[4] == '717.5' ||
        ...
        expect(
          svg.getAttribute('d').split(' ')[5] == '36.75' ||
            svg.getAttribute('d').split(' ')[5] == '35.75'
        ).toBe(true);
        done();
      };
    ...
    it('MACD Technical indicator with cross hair', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let crosshair: Element = <Element>(
          document.getElementById('container_svg').childNodes[3]
        );
        ...
        expect(
          element1.textContent == 'Mar' || element1.textContent == 'Feb'
        ).toBe(true);
        element1 = <HTMLElement>crosshair.childNodes[2].childNodes[3];
        expect(
          Math.round(+element1.textContent) == 6 ||
            element1.textContent == '8.460' ||
            element1.textContent == '8.474'
        ).toBe(true);
        done();
      };
      chartObj.tooltip = { enable: true };
      ...
      loaded = (args: Object): void => {
        ...
        expect(
          chartObj.primaryYAxis.zoomPosition.toFixed(2) == '0.00' ||
            chartObj.primaryYAxis.zoomPosition.toFixed(2) == '0.06'
        ).toBe(true);
        done();
      };
    ...
    it('MACD Technical indicator using panels', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          Math.round(rect.top) === 335 || Math.round(rect.top) === 336
        ).toBe(true);
        chartObj.loaded = null;
        done();
      };
    ...
    it('Checking series animationEvent', (done: Function) => {
      animationComplete = (args: IAnimationCompleteEventArgs): void => {
        ...
        expect(
          point.getAttribute('transform') === 'translate(57.5,295.225)' ||
            point.getAttribute('transform') === 'translate(53.5,295.625)'
        ).toBe(true);
        done();
      };
