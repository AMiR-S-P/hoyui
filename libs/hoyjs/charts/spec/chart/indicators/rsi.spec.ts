  ...
  describe('RSI Indicators', () => {
    ...
    it('RSI Technical indicator for a series with 0 points', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          indicator.childNodes.length === 4 || indicator.childNodes.length === 2
        ).toBe(true);
        done();
      };
    ...
    it('RSI Technical indicator for a series with valid points & primary axis', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let signalLine: HTMLElement = document.getElementById(
          'container_Indicator_0_RSI'
        );
        expect(signalLine.getAttribute('d')).not.toBeNull();
        expect(
          chartObj.visibleSeries.length === 4 ||
            chartObj.visibleSeries.length === 2
        ).toBe(true);
        ...
        expect(Math.round(<number>chartObj.visibleSeries[1].points[5].y)).toBe(
          55
        );
        done();
      };
    ...
    it('RSI Technical indicator for a series with negative points', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(Math.round(<number>chartObj.visibleSeries[1].points[3].y)).toBe(
          84
        );
        done();
    ...
    it('RSI Technical indicator for a series with secondary y axis', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('d').split(' ')[1] == '761.5' ||
            svg.getAttribute('d').split(' ')[1] == '761.5' ||
            svg.getAttribute('d').split(' ')[1] == '767.5'
        ).toBe(true);
        expect(
          svg.getAttribute('d').split(' ')[2] == '43.25' ||
            svg.getAttribute('d').split(' ')[2] == '42.25'
        ).toBe(true);
        expect(
          svg.getAttribute('d').split(' ')[4] == '761.5' ||
            svg.getAttribute('d').split(' ')[4] == '761.5' ||
    ...
    it('stochastic indicator with disabled show zones', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let line: Element = document.getElementById(
          'container_Indicator_0_UpperLine'
        );
    ...
    it('RSI Technical indicator with cross hair', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          element1.textContent == 'Feb' || element1.textContent == 'Mar'
        ).toBe(true);
        element1 = <HTMLElement>crosshair.childNodes[2].childNodes[3];
        expect(
          element1.textContent == '8.461' || element1.textContent == '8.432'
        ).toBe(true);
        done();
      };
      chartObj.tooltip = { enable: true };
      ...
      loaded = (args: Object): void => {
        ...
        expect(
          text1.textContent.replace(/\u200E/g, '') ==
            'FebgoldHigh : 6.3Low : 1.3Open : 4.8Close : 2.5LowerLine : 80UpperLine : 20'
        ).toBe(true);
    ...
    it('checking  zooming with touch', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(content == '1.00' || content == '0.17').toBe(true);
        content = chartObj.primaryYAxis.zoomFactor.toFixed(2);
        expect(content == '1.00' || content == '0.45').toBe(true);
        content = chartObj.primaryXAxis.zoomPosition.toFixed(2);
        expect(content == '0.00' || content == '0.63').toBe(true);
        chartObj.mouseLeave(
          <PointerEvent>(
    ...
    it('Checking series animationEvent', (done: Function) => {
      animationComplete = (args: IAnimationCompleteEventArgs): void => {
        ...
        expect(
          point.getAttribute('transform') === 'translate(53.5,311.825)' ||
            point.getAttribute('transform') === 'translate(53.5,311.525)'
        ).toBe(true);
        done();
      };
