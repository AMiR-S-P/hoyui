...
describe('Chart', () => {
  ...
  describe('Polynomial Trendlines', () => {
    ...
    it('Polynomial Trendlines with series points and with trendline validcollection', (done: Function) => {
      ...
      chartObj.loaded = loaded;
      chartObj.series[0].trendlines = [
        { type: 'Polynomial', animation: { enable: false } },
      ];
      chartObj.refresh();
    });
  ...
  describe('Polynomial Trendlines with category data', () => {
    let chartObj: Chart;
    let loaded: EmitType<ILoadedEventArgs>;
    let trigger: MouseEvents = new MouseEvents();
    element = createElement('div', { id: 'container' });
    ...
    it('Polynomial Trendlines with category values', (done: Function) => {
      loaded = (args: Object): void => {
        let svg: number = document.getElementById('containerSeriesGroup0')
          .childNodes.length;
