  ...
  describe('Chart Trackball Default', () => {
      ...
      loaded = (args: Object): void => {
        ...
        expect(
          text.textContent.replace(/\u200E/g, '') ===
            'FebJapan : -$1.00Japan1 : $3.50Japan2 : $8.00Japan3 : $10.00'
        ).toBe(true);

        // Track ball elements are appended into svg instead of series group element.
        let trackSymbol: NodeListOf<Element> =
          document.querySelectorAll('.EJ2-Trackball');
    ...
    it('With Column type series', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(crosshair.childNodes[2].childNodes[1].textContent == 'Mar').toBe(
          true
        );
        expect(crosshair.childNodes[2].childNodes.length == 2).toBe(true);
        ...
        expect(
          document
            .getElementById('container_Series_1_Point_2')
            .getAttribute('opacity') == '0.5'
        ).toBe(true);
        expect(
          document
            .getElementById('container_Series_2_Point_2')
            .getAttribute('opacity') == '0.5'
        ).toBe(true);
        expect(
          document
            .getElementById('container_Series_3_Point_2')
            .getAttribute('opacity') == '0.5'
        ).toBe(true);
        done();
      };
    ...
    it('Checking the with null points', (done: Function) => {
      ...
      loaded = (args: Object): void => {
        ...
        targetElement = chartObj.element.querySelector(
          '#container_Series_0_Point_11_Symbol'
        ) as HTMLElement;
        y =
          parseFloat(targetElement.getAttribute('cy')) +
          parseFloat(chartArea.getAttribute('y')) +
      ...
      chartObj.series = [
        ...
        },
        {
          ...
          width: 2,
          type: 'Line',
          marker: { visible: true, height: 8, width: 8, shape: 'Circle' },
        },
      ];
      chartObj.loaded = loaded;
  ...
  describe('Chart Trackball Default', () => {
        ...
        series: [
          {
            dataSource: [
              { x: 'Jan', y: -1, color: 'red' },
              { x: 'Feb', y: -1, color: 'blue' },
              { x: 'Mar', y: 2, color: 'green' },
              { x: 'Apr', y: 8 },
              { x: 'May', y: 13, color: 'orange' },
              { x: 'Jun', y: 18, color: 'purple' },
              { x: 'Jul', y: 21, color: 'pink' },
              { x: 'yellow', y: 20, color: 'red' },
              { x: 'Sep', y: 16, color: 'teal' },
              { x: 'Oct', y: 10, color: 'violet' },
              { x: 'Nov', y: 4, color: 'black' },
              { x: 'Dec', y: 0, color: 'whitesmoke' },
            ],
            xName: 'x',
            yName: 'y',
            animation: { enable: false },
    ...
    it('Default Trackball with shared tooltip', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        y =
          series.points[5].symbolLocations[0].y +
          parseFloat(chartArea.getAttribute('y')) +
          elem.offsetTop;
        x =
          series.points[5].symbolLocations[0].x +
          parseFloat(chartArea.getAttribute('x')) +
          elem.offsetLeft;
        trigger.mousemovetEvent(elem, Math.ceil(x), Math.ceil(y));
        let marker: HTMLElement = document.getElementById(
          'container_Series_0_Point_5_Trackball_0'
        );
        ...
        marker = document.getElementById(
          'container_Series_0_Point_5_Trackball_1'
        );
        ...
        trigger.mousemovetEvent(elem, Math.ceil(x), Math.ceil(y));
        marker = document.getElementById(
          'container_Series_0_Point_0_Trackball_1'
        );
        expect(marker.getAttribute('fill')).toBe('#ffffff');
        expect(marker.getAttribute('stroke')).toBe('red');
        y =
          series.points[11].symbolLocations[0].y +
          parseFloat(chartArea.getAttribute('y')) +
          elem.offsetTop;
        x =
          series.points[11].symbolLocations[0].x +
          parseFloat(chartArea.getAttribute('x')) +
          elem.offsetLeft;
        trigger.mousemovetEvent(elem, Math.ceil(x), Math.ceil(y));
        marker = document.getElementById(
          'container_Series_0_Point_11_Trackball_1'
        );
        expect(marker.getAttribute('fill')).toBe('#ffffff');
    ...
    it('Default Trackball with shared tooltip', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        marker = document.getElementById(
          'container_Series_1_Point_5_Trackball_0'
        );
        expect(marker !== null);
        done();
      };
      ...
      chartObj.addSeries([
        {
          dataSource: [
            { x: 'Jan', y: 11, color: 'red' },
            { x: 'Feb', y: -1, color: 'blue' },
            { x: 'Mar', y: 2, color: 'green' },
            { x: 'Apr', y: -8 },
            { x: 'May', y: 15, color: 'orange' },
            { x: 'Jun', y: 14, color: 'purple' },
            { x: 'Jul', y: 31, color: 'pink' },
            { x: 'yellow', y: 23, color: 'red' },
            { x: 'Sep', y: 17, color: 'teal' },
            { x: 'Oct', y: 19, color: 'violet' },
            { x: 'Nov', y: 14, color: 'black' },
            { x: 'Dec', y: 10, color: 'whitesmoke' },
          ],
          xName: 'x',
          yName: 'y',
          animation: { enable: false },
