...
for (let j: number = 0; j < 20; j++) {
  ...
  seriesCollection[j] = {
    ...
    marker: {
      visible: false,
      shape: 'Circle',
      dataLabel: { visible: true, border: { color: 'red', width: 2 } },
    },
...
describe('Chart Control', () => {
  ...
  describe('Chart Control Legend Checking', () => {
    ...
    it('checking with datalabel before legend click', () => {
      dataLabel = getElement(
        'cartesianChart_Series_3_Point_0_Text_0'
      ) as HTMLElement;
      expect(
        parseFloat(dataLabel.getAttribute('y')) == 216 ||
          parseFloat(dataLabel.getAttribute('y')) == 213.75
      ).toBe(true);
    });
    ...
    it('checking with datalabel after legend click', () => {
      ...
      expect(parseFloat(dataLabel.getAttribute('y'))).toBeGreaterThanOrEqual(
        181.625
      );
    });
    ...
    it('checking with column series changed to bar series and check select', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          seriesElement.childElementCount == 7 ||
            seriesElement.childElementCount == 6
        );
        expect(
          (lastLabel.lastChild as HTMLElement).innerHTML == '100' ||
            (lastLabel.lastChild as HTMLElement).innerHTML == '80'
        );
        done();
      }, 301);
    });
    it('checking with spline series and check deselect', (done: Function) => {
      ...
      setTimeout(() => {
        expect(seriesElement.childElementCount).toEqual(6);
        expect(
          seriesElement.contains(getElement(ele.id + 'SeriesGroup4'))
        ).toEqual(false);
        expect((lastLabel.lastChild as HTMLElement).innerHTML).toEqual('80');
        done();
      }, 301);
    ...
    it('checking with area series with select', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          seriesElement.childElementCount == 5 ||
            seriesElement.childElementCount == 6
        );
        expect(
          seriesElement.contains(getElement(ele.id + 'SeriesGroup1'))
        ).toEqual(false);
        expect(
          seriesElement.childElementCount == 5 ||
            seriesElement.childElementCount == 6
    ...
    it('checking with Stacking bar', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          dataLabel.getAttribute('x') == '236.5' ||
            dataLabel.getAttribute('x') == '237'
        ).toBe(true);
        expect(seriesElement.childElementCount).toEqual(7);
        expect(lastLabel.lastElementChild.innerHTML).toEqual('300');
        done();
    ...
    it('checking with marker', () => {
      ...
      seriesElement = getElement(ele.id + 'SeriesCollection') as HTMLElement;
      dataLabel = getElement(
        'cartesianChart_Series_2_Point_0_Symbol'
      ) as HTMLElement;
      expect(
        parseFloat(dataLabel.getAttribute('cy')) == 223.475 ||
          parseFloat(dataLabel.getAttribute('cy')) == 224.87499999999997
      ).toBe(true);
      expect(seriesElement.contains(getElement(ele.id + 'SymbolGroup4'))).toBe(
        true
      );
    });
    it('checking with marker after legend click', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          seriesElement.contains(getElement(ele.id + 'SymbolGropu4'))
        ).toBe(false);
        dataLabel = getElement(
          'cartesianChart_Series_2_Point_0_Symbol'
        ) as HTMLElement;
        expect(
          parseFloat(dataLabel.getAttribute('cy')) == 199.53125 ||
            parseFloat(dataLabel.getAttribute('cy')) == 200.78125
        ).toBe(true);
        expect(seriesElement.childElementCount).toEqual(12);
        done();
      }, 301);
    ...
    it('checked with errorBar', () => {
      ...
      chart.refresh();
      seriesElement = getElement(ele.id + 'SeriesCollection') as HTMLElement;
      let errorBar: Element = getElement(
        'cartesianChart_Series__ErrorBarGroup_0_Point_0'
      );
    ...
    it('checked with error bar after legend Click', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        seriesElement = getElement(ele.id + 'SeriesCollection') as HTMLElement;
        expect(errorBar.getAttribute('d').split(' ')[2]).toBeLessThanOrEqual(
          285.9125
        );
    ...
    it('checked with stripline', () => {
      ...
      chart.refresh();
      let stripLine: Element = getElement(
        'cartesianChart_stripline_Behind_rect_primaryYAxis_0'
      );
      expect(
        stripLine.getAttribute('d') === 'M 38.5 219.0625 L 790 219.0625'
      ).toBe(true);
    });
    it('checked with stripline after legend click', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          stripLine.getAttribute('d') === 'M 32.5 190.953125 L 790 190.953125'
        ).toBe(true);
        done();
      }, 301);
    });
  ...
  describe('Financial chart', () => {
    ...
    it('checking with legend click series deselect', (done: Function) => {
      ...
      trigger.clickEvent(legendElement);
      setTimeout(() => {
        seriesCollection = getElement(
          ele.id + 'SeriesCollection'
        ) as HTMLElement;
        seriesElement = getElement('cartesianChartSeriesGroup1') as HTMLElement;
        ...
        let dataLabelCollection: HTMLElement = getElement(
          ele.id + 'DataLabelCollection'
        ) as HTMLElement;
    ...
    it('checking without series', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(shapeElement).toEqual(null);
        expect(textElement).toEqual(null);
        let lastLabel: HTMLElement = getElement(
          ele.id + 'AxisLabels1'
        ) as HTMLElement;
    ...
    it('checking multiple axes', (done: Function) => {
      ...
      chart.rows = [
        { height: '50%', border: { width: 1, color: 'red' } },
        { height: '50%', border: { width: 1, color: 'blue' } },
      ];
    ...
    it('checking with series visibilty with trendline series', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        legendElement = document.getElementById(
          'cartesianChart_chart_legend_text_0'
        );
        trigger.clickEvent(legendElement);
        expect(legendElement.textContent).toBe('Series');
        legendElement = document.getElementById(
          'cartesianChart_chart_legend_text_1'
        );
        trigger.clickEvent(legendElement);
        trigger.clickEvent(legendElement);
