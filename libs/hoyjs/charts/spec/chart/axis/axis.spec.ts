...
describe('Chart Control', () => {
  ...
  describe('Axis Behavior', () => {
    ...
    it('Checking the row definition', (done: Function) => {
      loaded = (args: Object): void => {
        let axis: HTMLElement = document.getElementById(
          'chartContainerAxisInsideCollection'
        );
        expect(axis.childNodes.length == 3).toBe(true);
        axis = document.getElementById('chartContainerAxisLine_1');
        expect(
          parseFloat(axis.getAttribute('d').split(' ')[5]) -
            parseFloat(axis.getAttribute('d').split(' ')[2]) ==
            300
        ).toBe(true);
        done();
      };
    ...
    it('Adding column definition to Axis', (done: Function) => {
      ...
      loaded = (args: Object): void => {
        ...
        expect(
          parseFloat(axis.getAttribute('d').split(' ')[4]) -
            parseFloat(axis.getAttribute('d').split(' ')[1]) ==
            350
        ).toBe(true);
        done();
      };
    ...
    it('Checking the empty definition', (done: Function) => {
      ...
      chartObj.rows = [];
      loaded = (args: Object): void => {
        let area: HTMLElement = document.getElementById(
          'chartContainer_ChartAreaBorder'
        );
    ...
    it('Checking the row definition with high value', (done: Function) => {
      ...
      loaded = (args: Object): void => {
        ...
        let axisLine: HTMLElement = document.getElementById(
          'chartContainerAxisLine_1'
        );
        expect(
          area.getAttribute('x') ==
            document
              .getElementById('chartContainerAxisLine_0')
              .getAttribute('d')
              .split(' ')[1]
        ).toBe(true);
        expect(
          parseFloat(area.getAttribute('height')) ==
            parseFloat(axisLine.getAttribute('d').split(' ')[5]) -
              parseFloat(axisLine.getAttribute('d').split(' ')[2])
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Checking Grid Line', () => {
      ...
      chartObj.primaryXAxis.majorTickLines = {
        color: '#0AA368',
        width: 1.5,
        height: 20,
      };
      chartObj.primaryYAxis.majorGridLines = { color: '#B4D072', width: 2 };
      chartObj.primaryYAxis.majorTickLines = {
        color: '#C2C924',
        width: 1.5,
        height: 20,
      };
    ...
    it('Checking Both Axis Major Ticklines Inside Position', () => {
      ...
      expect(svg.getAttribute('value1') === svg.getAttribute('value2')).toBe(
        true
      );
      ...
      expect(svg.getAttribute('value11') === svg.getAttribute('value21')).toBe(
        true
      );
    });
    ...
    it('Axis label rotation', () => {
      ...
      text = document.getElementById('chartContainer0_AxisLabel_0');
      expect(text.getAttribute('transform').indexOf('rotate(45') > -1).toBe(
        true
      );
    });
    ...
    it('Checking the label format', () => {
      ...
      let element: Element = <Element>(
        document.getElementById('chartContainer0_AxisLabel_1')
      );
      expect(element.getAttribute('transform').indexOf('rotate(45') > -1).toBe(
        true
      );
    });
    it('Checking the rotation greater than 360', () => {
      ...
      expect(element.getAttribute('transform').indexOf('rotate(27') > -1).toBe(
        true
      );
    });
  ...
  describe('Axis Behavior', () => {
    ...
    it('Checking with negative value in axis', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(text.childNodes[0].textContent.indexOf('custom') > -1).toBe(
          true
        );
        done();
      };
    ...
    it('checking minor gridlines', (done: Function) => {
      chart.loaded = (args: Object): void => {
        let tick: Element = document.getElementById(
          'chartContainer_MinorGridLine_0_0'
        );
        let border: Element = document.getElementById(
          'chartContainer_ChartAreaBorder'
        );
        expect(
          tick.getBoundingClientRect().top == border.getBoundingClientRect().top
        ).toBe(true);
        done();
      };
    ...
    it('checking y axis as inversed axis', (done: Function) => {
      loaded = (args: Object): void => {
        let firstLabel: HTMLElement = document.getElementById(
          'chartContainer1_AxisLabel_0'
        );
        expect(firstLabel.textContent).toEqual('0');
        let lastLabel: HTMLElement = document.getElementById(
          'chartContainer1_AxisLabel_10'
        );
        expect(lastLabel.textContent).toEqual('5');
        expect(
          +firstLabel.getAttribute('y') < +lastLabel.getAttribute('y')
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('checking x axis as inversed axis', (done: Function) => {
      loaded = (args: Object): void => {
        let firstLabel: HTMLElement = document.getElementById(
          'chartContainer0_AxisLabel_0'
        );
        expect(firstLabel.textContent).toEqual('0');
        let secondLabel = document.getElementById(
          'chartContainer0_AxisLabel_5'
        );
        expect(secondLabel.textContent).toEqual('2.5');
        expect(
          +firstLabel.getAttribute('x') > +secondLabel.getAttribute('x')
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('checking inversed axis with label intersect action', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let secondLabel: HTMLElement = document.getElementById(
          'chartContainer0_AxisLabel_1'
        );
        expect(secondLabel).toEqual(null);
        let thirdLabel: HTMLElement = document.getElementById(
          'chartContainer0_AxisLabel_2'
        );
        expect(thirdLabel.textContent).toEqual('3customLabels');
        expect(
          +firstLabel.getAttribute('x') >
            +thirdLabel.getAttribute('x') + +thirdLabel.getAttribute('width')
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('checcking inversed axis with labelintersect action as none', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(firstLabel.textContent).toEqual('1customLabels');
        let secondLabel: any = document.getElementById(
          'chartContainer0_AxisLabel_1'
        );
        ...
        expect(
          +firstLabel.getAttribute('x') <
            +secondLabel.getAttribute('x') +
              secondLabel.textLength.baseVal.value
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('checcking labelintersect action as rotate45 inside position', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          firstLabel.getAttribute('transform').indexOf('rotate(45') > -1
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('checcking inversed axis with labelintersect action as rotate90', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(firstLabel.textContent).toEqual('1customLabels');
        expect(
          firstLabel.getAttribute('transform').indexOf('rotate(90') > -1
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('checking with multiple axes', (done: Function) => {
      loaded = (args: Object): void => {
        let firstLabel: HTMLElement = document.getElementById(
          'chartContainer2_AxisLabel_0'
        );
        expect(firstLabel.textContent).toEqual('24');
        let secondLabel: any = document.getElementById(
          'chartContainer2_AxisLabel_1'
        );
        expect(secondLabel.textContent).toEqual('26');
        expect(
          +firstLabel.getAttribute('y') < +secondLabel.getAttribute('y')
        ).toBe(true);
        done();
      };
  ...
  describe('Axis Smartlabels', () => {
    ...
    beforeAll((): void => {
      ...
      chart = new Chart({
        ...
        series: [
          {
            dataSource: [
              { x: 'USAfas dqwasd saasdf', y: 50 },
              { x: 'Chin afsdfssa', y: 40 },
              { x: 'Japan safds afds', y: 70 },
              { x: 'Aust rali af dsasfas fasdfasd', y: 60 },
              { x: 'Fra nce fasf sdfds', y: 50 },
              { x: 'Germ any asdas s', y: 40 },
              { x: 'Ita ly fs adfsad', y: 40 },
              { x: 'Swed en', y: 30 },
            ],
    ...
    it('Checking with Trim and tooltip', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let tooltip: Element = document.getElementById(
          'chartContainer_EJ2_AxisLabel_Tooltip'
        );
        expect(tooltip.textContent).toBe('USAfas dqwasd saasdf');
        expect(text.textContent.split('...').length).toEqual(2);
        tooltip.remove();
        chart.mouseEnd(
          <PointerEvent>trigger.onTouchEnd(text, 0, 0, null, null, 35, 330)
        );
    ...
    it('Checking with MultipleRows', (done: Function) => {
      loaded = (args: Object): void => {
        text = document.getElementById('chartContainer0_AxisLabel_0');
        let text1: Element = document.getElementById(
          'chartContainer0_AxisLabel_1'
        );
        expect(
          (<Element>text1).getAttribute('y') > (<Element>text).getAttribute('y')
        ).toBe(true);
        let text4: Element = document.getElementById(
          'chartContainer0_AxisLabel_4'
        );
        expect(
          (<Element>text).getAttribute('y') ===
            (<Element>text4).getAttribute('y')
        ).toBe(true);
        let height: Element = document.getElementById(
          'chartContainer_ChartAreaBorder'
        );
        expect(parseFloat((<Element>height).getAttribute('height')) < 250).toBe(
          true
        );
        done();
      };
    ...
    it('Checking with MultipleRows with Oppossed', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(parseFloat((<Element>element).getAttribute('y')) > 50).toBe(
          true
        );
        done();
      };
    ...
    it('Checking with Wraped text in Middle Position', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          +text.children[1].getAttribute('x') == 209.84375 ||
            +text.children[1].getAttribute('x') == 209.53125
        ).toBe(true);
        expect(
          +text.children[1].getAttribute('y') == 285 ||
            +text.children[1].getAttribute('y') == 288.25
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('Checking with Wraped text in Middle Position with Opposed Position', (done: Function) => {
      loaded = (args: Object): void => {
        text = document.getElementById('chartContainer0_AxisLabel_4');
        expect(
          +text.children[1].getAttribute('x') == 177.78125 ||
            +text.children[1].getAttribute('x') == 177.34375
        ).toBe(true);
        expect(
          +text.children[1].getAttribute('y') == 42.25 ||
            +text.children[1].getAttribute('y') == 40.25
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('Checking with Wraps with labels inside', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          +text.children[1].getAttribute('y') <
            +axis.getAttribute('d').split(' ')[2]
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('Checking with Wraps with Oppossed', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(text.childNodes.length == 3).toBe(true);
        expect(
          text.childNodes[2].textContent === 'USA...' ||
            text.childNodes[2].textContent === 'US...'
        ).toBe(true);
        expect(text.childNodes[1].textContent.indexOf('...') > -1).toBe(true);
        text = document.getElementById('chartContainer0_AxisLabel_4');
        expect(text.childNodes.length == 4).toBe(true);
    ...
    it('Checking the label rotation', () => {
      ...
      chart.dataBind();
      let element: Element = <Element>(
        document.getElementById('chartContainer0_AxisLabel_0')
      );
      expect(element.getAttribute('transform').indexOf('rotate(45') > -1).toBe(
        true
      );
    ...
    it('Checking with maximum label width property', (done: Function) => {
      loaded = (args: Object): void => {
        let label1: HTMLElement = document.getElementById(
          'chartContainer0_AxisLabel_0'
        );
        expect(
          Math.round(label1.getBoundingClientRect().width)
    ...
    it('Checking with label width', (done: Function) => {
      loaded = (args: Object): void => {
        let maximumLabelWidth: HTMLElement = document.getElementById(
          'chartContainer0_AxisLabel_0'
        );
        expect(
          Math.round(maximumLabelWidth.getBoundingClientRect().width)
  ...
  describe('Checking the Axis Crossing for Numeric Axis', () => {
    ...
    it('With X-Axis Opposed Position and Crossing Value Less than Visible Range', (done: Function) => {
      loaded = (args: Object): void => {
        let insideGroup: HTMLElement = document.getElementById(
          'chartContainerAxisInsideCollection'
        );
        let outsideGroup: HTMLElement = document.getElementById(
          'chartContainerAxisOutsideCollection'
        );
        ...
        expect(
          parseInt(xLine.getAttribute('d').split(' ')[2]) - 1 ===
            parseInt(chartArea.getAttribute('y')) +
              parseInt(chartArea.getAttribute('height'))
        ).toBe(true);
        done();
      };
    ...
    it('With Y-Axis Opposed Position and Crossing Value Less than Visible Range', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          yLine.getAttribute('d').split(' ')[1] === chartArea.getAttribute('x')
        ).toBe(true);
        done();
      };
      chartEle.loaded = loaded;
    ...
    it('With X and Y Axis Inversed', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(yAxisInside.childElementCount).toBe(12);
        expect(
          xLine.getAttribute('d').split(' ')[2] === '249' &&
            xLine.getAttribute('d').split(' ')[5] === '249'
        ).toBe(true);

        expect(
          yLine.getAttribute('d').split(' ')[1] === '400' &&
            yLine.getAttribute('d').split(' ')[4] === '400'
        ).toBe(true);
        done();
      };
    ...
    it('With X-Axis Visibility to false', (done: Function) => {
      loaded = (args: Object): void => {
        let xLine: HTMLElement = document.getElementById(
          'chartContainerAxisLine_0'
        );
    ...
    it('With Y-Axis Visibility to false', (done: Function) => {
      loaded = (args: Object): void => {
        let yLine: HTMLElement = document.getElementById(
          'chartContainerAxisLine_1'
        );
  ...
  describe('Checking Axis Crossing with Category Axis', () => {
    ...
    it('By Specifying Cross value in Numeric', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          ((yLine.getAttribute('d').split(' ')[1] === '402.5' ||
            yLine.getAttribute('d').split(' ')[1] === '400') &&
            yLine.getAttribute('d').split(' ')[4] === '402.5') ||
            yLine.getAttribute('d').split(' ')[4] === '400'
        ).toBe(true);
        done();
      };
    ...
    it('By Specifying Cross value in String Value not in the Data Source', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          (yLine.getAttribute('d').split(' ')[1] === '33.5' ||
            yLine.getAttribute('d').split(' ')[1] === '32.5') &&
            (yLine.getAttribute('d').split(' ')[4] === '33.5' ||
              yLine.getAttribute('d').split(' ')[4] === '32.5')
        ).toBe(true);
        done();
      };
      chartEle.loaded = loaded;
          ...
          series: [
            {
              ...
              marker: { visible: true },
              dataSource: [
                { x: new Date(2016, 0, 1, 18, 22, 0), y: 46 },
                { x: new Date(2016, 0, 1, 18, 23, 0), y: 27 },
                { x: new Date(2016, 0, 1, 18, 24, 0), y: 26 },
                { x: new Date(2016, 0, 1, 18, 25, 0), y: 16 },
  ...
  describe('Checking Axis Crossing with Logarthmic Axis', () => {
    ...
    it('With Log values', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          yLine.getAttribute('d').split(' ')[1] === '394.0515262271122' &&
            yLine.getAttribute('d').split(' ')[4] === '394.0515262271122'
        ).toBe(true);
        done();
      };
      chartEle.loaded = loaded;
    ...
    it('With Minor Tick Lines for X-Axis', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          xLine.getAttribute('d').split(' ')[2] === '311.07500000000005' &&
            xLine.getAttribute('d').split(' ')[5] === '311.07500000000005'
        ).toBe(true);
        done();
      };
      chartEle.loaded = loaded;
    ...
    it('Minor Tick with Opposed Position for X-Axis', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let majorTickX: HTMLElement = document.getElementById(
          'chartContainer_MajorTickLine_0_0'
        );
        let minorTickX: HTMLElement = document.getElementById(
          'chartContainer_MinorTickLine_0_1'
        );
        expect(
          xLine.getAttribute('d').split(' ')[2] === '182.15000000000003' &&
            xLine.getAttribute('d').split(' ')[5] === '182.15000000000003'
        ).toBe(true);
        expect(
          majorTickX.getAttribute('d').split(' ')[1] === '10' &&
            majorTickX.getAttribute('d').split(' ')[2] === '181.65000000000003'
        ).toBe(true);
        expect(
          minorTickX.getAttribute('d').split(' ')[1] === '89' &&
            minorTickX.getAttribute('d').split(' ')[2] === '182.15000000000003L'
        ).toBe(true);
        done();
      };
      chartEle.loaded = loaded;
    ...
    it('Minor Tick with Opposed Position for Y-Axis', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let majorTickY: HTMLElement = document.getElementById(
          'chartContainer_MajorTickLine_1_0'
        );
        let minorTickY: HTMLElement = document.getElementById(
          'chartContainer_MinorTickLine_1_0'
        );
        expect(
          yLine.getAttribute('d').split(' ')[1] === '394.0515262271122' &&
            yLine.getAttribute('d').split(' ')[4] === '394.0515262271122'
        ...
        expect(
          majorTickY.getAttribute('d').split(' ')[1] === '394.5515262271122' &&
            majorTickY.getAttribute('d').split(' ')[2] === '440'
        ).toBe(true);
        expect(
          minorTickY.getAttribute('d').split(' ')[1] === '394.0515262271122' &&
            minorTickY.getAttribute('d').split(' ')[2] === '431L'
        ).toBe(true);
        done();
      };
      chartEle.loaded = loaded;
  ...
  describe('Checking Axis Crossing with Multiple Y Axis', () => {
      ...
      chartEle = new Chart(
        {
          ...
          axes: [
            { rowIndex: 0, opposedPosition: true, name: 'yAxis', crossesAt: 2 },
          ],
          series: [
            {
              type: 'Line',
    ...
    it('All Axis Moved', (done: Function) => {
      loaded = (args: Object): void => {
        let xLine1: HTMLElement = document.getElementById(
          'chartContainerAxisLine_0'
        );
        let yLine1: HTMLElement = document.getElementById(
          'chartContainerAxisLine_1'
        );
        let yLine2: HTMLElement = document.getElementById(
          'chartContainerAxisLine_2'
        );
        let chartArea: HTMLElement = document.getElementById(
          'chartContainer_ChartAreaBorder'
        );
        ...
        expect(
          parseInt(yLine1.getAttribute('d').split(' ')[1]) ===
            parseInt(chartArea.getAttribute('x')) +
              parseInt(chartArea.getAttribute('width'))
        ).toBe(true);
        expect(
          parseInt(yLine2.getAttribute('d').split(' ')[1]) ===
            parseInt(chartArea.getAttribute('x'))
        ).toBe(true);
        expect(
          (xLine1.getAttribute('d').split(' ')[2] === '183.390625' ||
            xLine1.getAttribute('d').split(' ')[2] === '183.828125') &&
            (xLine1.getAttribute('d').split(' ')[5] === '183.390625' ||
              xLine1.getAttribute('d').split(' ')[5] === '183.828125')
        ).toBe(true);
        done();
      };
      chartEle.loaded = loaded;
  ...
  describe('Checking Axis Crossing with Multiple X Axis', () => {
    ...
    it('All Axis Moved', (done: Function) => {
      loaded = (args: Object): void => {
        let line0: HTMLElement = document.getElementById(
          'chartContainerAxisLine_0'
        );
        let line1: HTMLElement = document.getElementById(
          'chartContainerAxisLine_1'
        );
        let line2: HTMLElement = document.getElementById(
          'chartContainerAxisLine_2'
        );
        let chartArea: HTMLElement = document.getElementById(
          'chartContainer_ChartAreaBorder'
        ...
        expect(
          parseInt(line0.getAttribute('d').split(' ')[2]) - 1 ===
            parseInt(chartArea.getAttribute('y')) +
              parseInt(chartArea.getAttribute('height'))
        ).toBe(true);
        expect(
          line1.getAttribute('d').split(' ')[1] === '400' &&
            line1.getAttribute('d').split(' ')[4] === '400'
        ).toBe(true);
        expect(
          line2.getAttribute('d').split(' ')[2] === '254.975' ||
            line2.getAttribute('d').split(' ')[2] === '259.655'
        ).toBe(true);
        done();
      };
      chartEle.loaded = loaded;
  ...
  describe('Checking Axis Label Click Event', () => {
      ...
      chartObj = new Chart(
        {
          ...
          axisLabelClick: (args: IAxisLabelClickEventArgs) => {
            document
              .getElementById('container0_AxisLabel_3')
    ...
    it('default axis label click event checking', (done: Function) => {
      chartObj.loaded = () => {
        let lableElement: Element = document.getElementById(
  ...
  describe('Checking minor grid lines with inversed vertical axis', () => {
    ...
    it('checking first and last minor grid lines', (done: Function) => {
      chartObj.loaded = () => {
        let gridLineElement: Element = document.getElementById(
          'container_MinorGridLine_1_0'
        );
        let path: string = gridLineElement.getAttribute('d');
        expect(
          path === 'M 42 62L 490 62 M 42 81L 490 81 ' ||
            path === 'M 42 61L 490 61 M 42 81L 490 81 '
        ).toBe(true);
        gridLineElement = document.getElementById(
          'container_MinorGridLine_1_4'
        );
        path = gridLineElement.getAttribute('d');
        expect(
          path === 'M 42 297L 490 297 M 42 317L 490 317 ' ||
            path === 'M 42 297L 490 297 M 42 317L 490 317 '
        ).toBe(true);
        done();
      };
  ...
  describe('Checking axis minor grid line while zooming', () => {
    ...
    it('X axis with double', (done: Function) => {
      chartObj.loaded = () => {
        minorGridElement = document.getElementById(
          'container_MinorGridLine_0_-1'
        );
        expect(minorGridElement !== null).toBe(true);
        minorGridElement = document.getElementById(
          'container_MinorTickLine_0_-1'
        );
        expect(minorGridElement !== null).toBe(true);
        done();
      };
    ...
    it('Checking with default alignment', (done: Function) => {
      chartObj.loaded = () => {
        let titleY: string = document
          .getElementById('container_AxisTitle_1')
          .getAttribute('y');
        let titleX: string = document
          .getElementById('container_AxisTitle_1')
          .getAttribute('x');
        expect(titleY === '180.375' || titleY === '188.375').toBe(true);
        expect(titleX === '27.5' || titleX === '26').toBe(true);
        titleY = document
          .getElementById('container_AxisTitle_0')
          .getAttribute('y');
        titleX = document
          .getElementById('container_AxisTitle_0')
          .getAttribute('x');
