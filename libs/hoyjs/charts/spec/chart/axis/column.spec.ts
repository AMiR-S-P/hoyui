  ...
  describe('Checking Column Definition', () => {
    ...
    it('Checking column Definition with percentage', (done: Function) => {
      ...
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('y') == '287.375' ||
            svg.getAttribute('y') == '288.375'
        ).toBe(true);
        svg = document.getElementById('chartContainer1_AxisLabel_0');
        expect(
          svg.getAttribute('y') == '543' || svg.getAttribute('y') == '546.75'
        ).toBe(true);
        expect(svg.getAttribute('x') == '37').toBe(true);

        svg = document.getElementById('chartContainer_AxisTitle_2');
        expect(
          svg.getAttribute('y') == '584.75' || svg.getAttribute('y') == '585.5'
        ).toBe(true);

        svg = document.getElementById('chartContainer2_AxisLabel_3');
        expect(
          svg.getAttribute('y') == '562' || svg.getAttribute('y') == '565.25'
        ).toBe(true);
        expect(
          svg.getAttribute('x') == '363.1875' ||
            svg.getAttribute('x') == '361.1875'
        ).toBe(true);
        ...
        expect(
          svg.getAttribute('x') == '681.875' ||
            svg.getAttribute('x') == '680.875'
        ).toBe(true);
        ...
        expect(
          svg.getAttribute('x') == '675.375' ||
            svg.getAttribute('x') == '674.875'
        ).toBe(true);
        ...
        expect(
          svg.getAttribute('x') == '265.625' ||
            svg.getAttribute('x') == '262.625'
        ).toBe(true);
        ...
        expect(
          svg.getAttribute('x') == '467.25' || svg.getAttribute('x') == '465.75'
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
  ...
  describe('Checking Column Definition with Spanning', () => {
    ...
    beforeAll((): void => {
      ...
      chartElem = new Chart({
        ...
        columns: [{ width: '300' }, { width: '300' }, { width: '300' }],
        height: '600',
        width: '900',
        legendSettings: { visible: false },
      });
    ...
    it('Axis Spanning', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('y') == '528' || svg.getAttribute('y') == '523.25'
        ).toBe(true);
        expect(
          svg.getAttribute('x') == '513.5' || svg.getAttribute('x') == '510.5'
        ).toBe(true);

        svg = document.getElementById('chartContainer_AxisTitle_3');

        expect(
          svg.getAttribute('y') == '584.5' || svg.getAttribute('y') == '583.75'
        ).toBe(true);
        expect(
          svg.getAttribute('x') == '626.25' || svg.getAttribute('x') == '624.75'
        ).toBe(true);

        svg = document.getElementById('chartContainer_AxisTitle_4');

        expect(
          svg.getAttribute('y') == '492.5' || svg.getAttribute('y') == '486.75'
        ).toBe(true);
        expect(
          svg.getAttribute('x') == '776.25' || svg.getAttribute('x') == '774.75'
        ).toBe(true);
        done();
      };
  ...
  describe('Checking Column Definition with oppossed position', () => {
    ...
    it('Checking the bottom line with opposed position', (done: Function) => {
      loaded = (args: Object): void => {
        svg = document.getElementById('chartContainer_AxisBottom_Column0');
        expect(
          svg.getAttribute('x2') == '57.5' || svg.getAttribute('x2') == '53.5'
        ).toBe(true);

        expect(svg.getAttribute('stroke') == 'red').toBe(true);
        svg = document.getElementById('chartContainer_AxisBottom_Column1');
        expect(
          svg.getAttribute('x2') == '457.5' || svg.getAttribute('x2') == '453.5'
        ).toBe(true);
        expect(svg.getAttribute('stroke') == 'blue').toBe(true);
        done();
      };
  ...
  describe('Checking the Calculating Column Size Method with Axis Crossing', () => {
    ...
    it('Checking the axis size with far', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          xLine1.getAttribute('d').split(' ')[2] === area.getAttribute('y')
        ).toBe(true);
        done();
      };
