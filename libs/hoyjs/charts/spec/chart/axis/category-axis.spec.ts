...
describe('Chart Control', () => {
  ...
  describe('Category Axis', () => {
      ...
      loaded = (args: Object): void => {
        ...
        expect(
          parseFloat(svg.getAttribute('x')) <
            parseFloat(svg1.getAttribute('d').split(' ')[1])
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('Checking the Label intersect action with rotate45', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          element.getAttribute('transform').indexOf('rotate(45') > -1
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('Checking the Label intersect action with rotate90', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let element: Element = <Element>svg.childNodes[1];
        expect(
          element.getAttribute('transform').indexOf('rotate(90') > -1
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('Checking the axis labels without data for series', (done: Function) => {
      ...
      chart.series = [
        { dataSource: null, name: 'Gold', fill: 'rgba(135,206,235,1)' },
      ];
      loaded = (args: Object): void => {
        let svg: HTMLElement = document.getElementById('containerAxisLabels0');
        ...
        let y =
          ...
          parseFloat(chartArea.getAttribute('height')) / 3 +
          ele.offsetTop;
        let x =
          parseFloat(chartArea.getAttribute('x')) +
          parseFloat(chartArea.getAttribute('width')) / 3 +
          ele.offsetLeft;
    ...
    it('Checking category axis with on ticks single point', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          element.getAttribute('x') == '12' ||
            element.getAttribute('x') == '12.5'
        ).toBe(true);
        expect(
          document.getElementById('containerAxisLabels0').childNodes.length == 1
        ).toBe(true);
        done();
      };
      chart.primaryXAxis.interval = null;
    ...
    it('Checking category axis with zoom position', (done: Function) => {
      ...
      loaded = (args: Object): void => {
        expect(
          document.getElementById('containerAxisLabels0').childNodes.length == 0
        ).toBe(true);
        done();
      };
      chart.primaryXAxis.interval = null;
    ...
    it('checking x axis as inversed axis', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let lastLabel: HTMLElement = document.getElementById(
          'container0_AxisLabel_7'
        );
        expect(lastLabel.textContent).toEqual('Sweden');
        expect(
          +firstLabel.getAttribute('x') > +lastLabel.getAttribute('x')
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
      ...
      chart.series = [
        {
          dataSource: [
            { x: 'South Africa', y: 5 },
            { x: 'United States of America', y: 2.5 },
            { x: 'United Kingdom', y: 5 },
            { x: 'United Arab Emirates', y: 5.5 },
            { x: 'Australia', y: 7.5 },
          ],
          xName: 'x',
          animation: { enable: false },
  ...
  describe('Category Axis labels with line break', () => {
    ...
    it('Checking line break labels with trim', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          label.childNodes[0].textContent === 'Southhhhhhhhhhhhhhhh...' ||
            label.childNodes[0].textContent === 'Southhhhhhhhhhhhhhhhhhh...'
        ).toBe(true);
        expect(
          label.childNodes[1].textContent === 'Koreaaaaaaaaaaaaaaaaaa...' ||
            label.childNodes[1].textContent === 'Koreaaaaaaaaaaaaaaaaaaaaa...'
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
      chart.series[0].dataSource[2] = {
        x: 'Southhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh<br>Koreaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
        y: 39.4,
      };
      chart.primaryXAxis.isInversed = false;
    ...
    it('Checking line break labels with wrap', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          label.childNodes[1].textContent == 'testhhhhhhhhhhhhhhhh...' ||
            label.childNodes[1].textContent == 'testhhhhhhhhhhhhhhhhhhh...'
        ).toBe(true);
        expect(
          label.childNodes[2].textContent == 'Koreaaaaaaaaaaaaaaaaaa...' ||
            label.childNodes[2].textContent == 'Koreaaaaaaaaaaaaaaaaaaaaa...'
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
      chart.series[0].dataSource[2] = {
        x: 'Southhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh testhhhhhhhhhhhhhhhhhhhhhhh<br>Koreaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
        y: 39.4,
      };
      chart.primaryXAxis.isInversed = false;
  ...
  describe('Checking rotated label intersection with category-axis-labels', () => {
    ...
    it('Label count with 0 degree', (done: Function) => {
      loaded = (args: Object): void => {
        let labelGroup: HTMLElement = document.getElementById(
          'containerAxisLabels0'
        );
    ...
    it('Label count with -280 degree', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          labelGroup.childElementCount == 50 ||
            labelGroup.childElementCount == 60
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
  ...
  describe('Label intersect actions checking with line break label Left alignment', () => {
    ...
    beforeAll((): void => {
      ele = createElement('div', {
        id: 'container',
        styles: 'width: 450px;height: 400px',
      });
      document.body.appendChild(ele);
      chart = new Chart(
        {
