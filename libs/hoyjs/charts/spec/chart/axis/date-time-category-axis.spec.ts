 ...
 */
import { createElement, EmitType } from '@hoyui/base';
...
import {
  datetimeData,
  datetimeCategoryYearData1,
  datetimeCategoryYearData,
  dateTimedataInterval,
} from '../base/data.spec';
  ...
  describe('Datetime category Axis', () => {
    ...
    it('Checking Auto interval with minutes', (done: Function) => {
      ...
      chart.series[0].dataSource = [
        { x: new Date(2000, 3, 21, 3), y: 50 },
        { x: new Date(2000, 3, 21, 4), y: 10 },
      ];
      chart.refresh();
    });
    ...
    it('Checking Auto interval with seconds', (done: Function) => {
      ...
      chart.primaryXAxis.intervalType = 'Auto';
      chart.series[0].dataSource = [
        { x: new Date(2000, 3, 21, 3, 10, 10), y: 50 },
        { x: new Date(2000, 3, 21, 4, 10, 10), y: 10 },
      ];
      chart.refresh();
    });
    ...
    it('Checking multiple series', (done: Function) => {
      ...
      chart.series = [
        {
          animation: { enable: false },
          dataSource: datetimeCategoryYearData,
          xName: 'x',
          yName: 'y',
          marker: { visible: true },
        },
        {
          animation: { enable: false },
          dataSource: datetimeCategoryYearData1,
          xName: 'x',
          yName: 'y',
          marker: { visible: true },
        },
    ...
    it('Checking with column series opposed position', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(+svg.getAttribute('x')).toEqual(
          +axisLine.getAttribute('d').split(' ')[1]
        );
        done();
      };
      chart.loaded = loaded;
    ...
    it('Checking with bar series with opposed postion', (done: Function) => {
      loaded = (args: Object): void => {
        svg = document.getElementById('container_ChartAreaBorder');
        axisLine = document.getElementById('containerAxisLine_0');
        expect(+svg.getAttribute('y')).toEqual(
          +axisLine.getAttribute('d').split(' ')[2]
        );
        done();
      };
      chart.loaded = loaded;
    ...
    it('Checking with dateTime axis on ticks', () => {
      ...
      chart.dataBind();
      let svg: any = document.getElementById('containerAxisLabels0')
        .childNodes[0];
      axisLine = document.getElementById('containerAxisLine_0');
    ...
    it('checking with  vertical axis edge label placement', (done: Function) => {
      loaded = (args: Object): void => {
        svg = document.getElementById('container_ChartAreaBorder');
        let label1: any = document.getElementById('containerAxisLabels0')
          .childNodes[0];
        let label2: any = document.getElementById('containerAxisLabels0')
          .childNodes[5];
    ...
    it('checking with label formats custom with intervalType as Year', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          label1.innerHTML ===
            'Monday, April 17, 2000 at 12:00:00 AM GMT+05:30' ||
            label1.innerHTML === 'Monday, April 17, 2000 at 12:00:00 AM GMT'
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('checking with inversed', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let label2: any = document.getElementById('containerAxisLabels0')
          .childNodes[5];
        expect(+label1.getAttribute('x') > +label2.getAttribute('x')).toBe(
          true
        );
        done();
      };
    ...
    it('checking with minor grid lines', () => {
      ...
      let path: string = document
        .getElementById('container_MinorGridLine_0_0')
        .getAttribute('d');
    ...
    it('checking datetime category axis with single points on ticks', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        axisLine = document.getElementById('containerAxisLine_0');
        expect(
          +svg.getAttribute('x') < +axisLine.getAttribute('d').split(' ')[1]
        ).toBe(true);
        done();
      };
      chart.loaded = loaded;
    ...
    it('checking with months with same type', (done: Function) => {
      ...
      chart.loaded = loaded;
      chart.series[0].dataSource = [
        { x: new Date(2017, 10, 2), y: 23 },
        { x: new Date(2017, 10, 3), y: 45 },
        { x: new Date(2017, 10, 4), y: 76 },
        { x: new Date(2018, 10, 4), y: 76 },
      ];
      chart.primaryXAxis.intervalType = 'Months';
      chart.refresh();
    });
    it('checking with days with same type', (done: Function) => {
      ...
      chart.loaded = loaded;
      chart.series[0].dataSource = [
        { x: new Date(2017, 10, 2, 3), y: 23 },
        { x: new Date(2017, 10, 2, 7), y: 45 },
        { x: new Date(2017, 10, 2, 4), y: 76 },
        { x: new Date(2018, 10, 4), y: 76 },
      ];
      chart.primaryXAxis.intervalType = 'Days';
      chart.refresh();
    ...
    it('checking with hours with same type', (done: Function) => {
      ...
      chart.loaded = loaded;
      chart.series[0].dataSource = [
        { x: new Date(2017, 10, 2, 1, 2), y: 23 },
        { x: new Date(2017, 10, 2, 3, 4), y: 45 },
        { x: new Date(2017, 10, 2, 3, 34), y: 76 },
        { x: new Date(2018, 10, 4), y: 76 },
      ];
      chart.primaryXAxis.intervalType = 'Hours';
      chart.refresh();
    ...
    it('checking with minutes with same type', (done: Function) => {
      ...
      chart.loaded = loaded;
      chart.series[0].dataSource = [
        { x: new Date(2017, 10, 2, 3, 2, 12), y: 23 },
        { x: new Date(2017, 10, 2, 3, 2, 14), y: 45 },
        { x: new Date(2017, 10, 2, 3, 2, 20), y: 76 },
        { x: new Date(2018, 10, 4), y: 76 },
      ];
    ...
    it('checking with seconds with same type', (done: Function) => {
      ...
      chart.series[0].dataSource = [
        { x: new Date(2017, 10, 2, 3, 2, 12), y: 23 },
        { x: new Date(2017, 10, 2, 3, 2, 14), y: 45 },
        { x: new Date(2017, 10, 2, 3, 2, 14), y: 76 },
        { x: new Date(2018, 10, 4), y: 76 },
      ];
    ...
    it('checking with annotation', (done: Function) => {
      ...
      chart.loaded = (args: Object): void => {
        ...
        expect(
          top == '249.167px' || top == '255.389px' || top == '246.312px'
        ).toBe(true);
        done();
      };
      chart.annotations = [
        {
          x: new Date(2017, 10, 2, 3, 2, 12),
          y: 20,
          region: 'Series',
          coordinateUnits: 'Point',
          content: '<div>AnnotationText</div>',
        },
      ];
      chart.refresh();
    });
    it('checking with strip lines', (done: Function) => {
      loaded = (args: Object) => {
        let stripLineElement = document.getElementById(
          'container_stripline_Behind_rect_primaryXAxis_0'
        );
        expect(stripLineElement).not.toEqual(null);
        stripLineElement = document.getElementById(
          'container_stripline_Over_rect_primaryXAxis_0'
        );
        expect(stripLineElement).not.toEqual(null);
        done();
      };
      ...
      chart.primaryXAxis.stripLines = [
        ...
        {
          start: new Date(2001, 3, 25),
          end: new Date(2002, 3, 30),
          opacity: 0.3,
          color: 'blue',
    ...
    it('checking with strip lines', (done: Function) => {
      loaded = (args: Object) => {
        let stripLineElement: HTMLElement = document.getElementById(
          'container_stripline_Over_rect_primaryXAxis_0'
        );
  ...
  describe('Checking Column Definition', () => {
    ...
    it('Checking the bottom line', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('x1') == '57.5' || svg.getAttribute('x1') == '53.5'
        ).toBe(true);
        expect(svg.getAttribute('stroke') == 'red').toBe(true);
        svg = document.getElementById('chartContainer_AxisBottom_Column1');

        expect(
          svg.getAttribute('x1') == '457.5' || svg.getAttribute('x1') == '453.5'
        ).toBe(true);
