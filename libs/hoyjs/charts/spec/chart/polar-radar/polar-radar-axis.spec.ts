...
describe('Chart Control', () => {
  ...
  describe('Polar Axis', () => {
    ...
    it('Checking the centerX, centerY and radius and labels', (done: Function) => {
      loaded = (args: Arg): void => {
        let centerX: number =
          args.chart.chartAxisLayoutPanel.seriesClipRect.width / 2 +
          args.chart.chartAxisLayoutPanel.seriesClipRect.x;
        let centerY: number =
          args.chart.chartAxisLayoutPanel.seriesClipRect.width / 2 +
          args.chart.chartAxisLayoutPanel.seriesClipRect.y;
        ...
        expect(
          parseInt(label.getAttribute('y')) < centerY - args.chart.radius
        ).toBe(true);
        done();
      };
    ...
    it('Checking labels with startAngle for axis', (done: Function) => {
      loaded = (args: Arg): void => {
        ...
        let vector: ChartLocation = CoefficientToVector(
          valueToPolarCoefficient(axis.visibleLabels[0].value, axis),
          args.chart.primaryXAxis.startAngle
        );
        expect(
          parseInt(label.getAttribute('x')) >
            centerX + args.chart.radius * vector.x
        ).toBe(true);
        expect(
          parseInt(label.getAttribute('y')) <
            centerY + args.chart.radius * vector.y
        ).toBe(true);
        label = document.getElementById('chartContainer1_AxisLabel_0');
    ...
    it('Checking x axis gridLines and tickLines for radar axis', (done: Function) => {
      loaded = (args: Object): void => {
        let element = document.getElementById(
          'chartContainer_MajorGridLine_0_0'
        );
    ...
    it('Checking startAngle with TickLines for y Axis for radar axis', (done: Function) => {
      loaded = (args: Object): void => {
        let element = document.getElementById(
          'chartContainer_MajorGridLine_1_1'
        );
        expect(element !== null).toBe(true);
  ...
  describe('Polar Axis: Smart Labels', () => {
    ...
    it('Check X axis labels count after hide', (done: Function) => {
      chartObj.loaded = (args: Arg): void => {
        let ele: Element = document.getElementById('containerAxisLabels0');
        expect(ele.childElementCount == 37 || ele.childElementCount == 38).toBe(
          true
        );
        done();
      };
    ...
    it('Check Y axis labels count after hide', (done: Function) => {
      chartObj.loaded = (args: Arg): void => {
        let ele: Element = document.getElementById('containerAxisLabels1');
        expect(ele.childElementCount == 9 || ele.childElementCount == 11).toBe(
          true
        );
        done();
      };
    ...
    it('Check Y axis labels count when X axis label inside and Y axis intersect action hide', (done: Function) => {
      chartObj.loaded = (args: Arg): void => {
        let ele: Element = document.getElementById('containerAxisLabels1');
        expect(ele.childElementCount == 7 || ele.childElementCount == 9).toBe(
          true
        );
        done();
      };
    ...
    it('Check X axis labels with start angle and label position outside', (done: Function) => {
      chartObj.loaded = (args: Arg): void => {
        ...
        let anchor: string = getElement('container0_AxisLabel_0').getAttribute(
          'text-anchor'
        );
  ...
  describe('Customer issue: Polar Radar Category axis gridline not split equally', () => {
          ...
          series: [
            {
              dataSource: [
                { x: 'k', y: 0.0 },
                { x: 'Alfreds Futterkiste', y: 2517.6 },
                { x: 'Ana Trujillo Emparedados y helados', y: 1839.54 },
                { x: 'Antonio Moreno Taquería', y: 4003.74 },
                { x: 'Around the Horn', y: 8682.83999999999 },
                { x: "B's Beverages", y: 3378.9 },
                { x: 'Berglunds snabbköp', y: 29011.08 },
                { x: 'Blauer See Delikatessen', y: 2108.46 },
                { x: 'Blondesddsl père et fils', y: 11880.0 },
                { x: 'Bólido Comidas preparadas', y: 2664.6 },
                { x: "Bon app'", y: 23715.9 },
                { x: 'Bottom-Dollar Markets', y: 13031.82 },
                { x: 'Cactus Comidas para llevar', y: 952.800000000001 },
                { x: 'Centro comercial Moctezuma', y: 39.0 },
                { x: 'Chop-suey Chinese', y: 5642.64 },
                { x: 'Comércio Mineiro', y: 2813.04 },
                { x: 'Consolidated Holdings', y: 698.7 },
                { x: 'Die Wandernde Kuh', y: 7244.88000000001 },
                { x: 'Drachenblut Delikatessen', y: 3575.04 },
                { x: 'Du monde entier', y: 945.66 },
                { x: 'Eastern Connection', y: 14170.62 },
                { x: 'Ernst Handel', y: 147221.52 },
                { x: 'Familia Arquibaldo', y: 3980.34 },
                { x: 'Folies gourmandes', y: 15002.7 },
                { x: 'Folk och fä HB', y: 31865.64 },
                { x: 'France restauration', y: 1508.16 },
                { x: 'Franchi S.p.A.', y: 875.280000000001 },
                { x: 'Frankenversand', y: 29587.86 },
                { x: 'Furia Bacalhau e Frutos do Mar', y: 5363.34 },
                { x: 'Galería del gastrónomo', y: 409.02 },
                { x: 'Godos Cocina Típica', y: 10209.36 },
                { x: 'Gourmet Lanchonetes', y: 5297.7 },
                { x: 'Great Lakes Food Market', y: 14732.58 },
                { x: 'GROSELLA-Restaurante', y: 813.6 },
                { x: 'Hanari Carnes', y: 9323.09999999999 },
                { x: 'HILARION-Abastos', y: 20504.28 },
                { x: 'Hungry Coyote Import Store', y: 1817.22 },
                { x: 'Hungry Owl All-Night Grocers', y: 43286.9400000001 },
                { x: 'Island Trading', y: 6848.39999999999 },
                { x: 'Königlich Essen', y: 18199.5 },
                { x: "La corne d'abondance", y: 1574.7 },
                { x: "La maison d'Asie", y: 9145.26000000001 },
                { x: 'Laughing Bacchus Wine Cellars', y: 172.92 },
                { x: 'Lazy K Kountry Store', y: 116.4 },
                { x: 'Lehmanns Marktstand', y: 17628.66 },
                { x: "Let's Stop N Shop", y: 3279.78 },
                { x: 'LILA-Supermercado', y: 13288.2 },
                { x: 'LINO-Delicateses', y: 12628.56 },
                { x: 'Lonesome Pine Restaurant', y: 1087.5 },
                { x: 'Magazzini Alimentari Riuniti', y: 7248.84 },
                { x: 'Maison Dewey', y: 6513.12 },
                { x: 'Mère Paillarde', y: 24726.66 },
                { x: 'Morgenstern Gesundkost', y: 5389.08 },
                { x: 'North/South', y: 618.72 },
                { x: 'Océano Atlántico Ltda.', y: 6535.02 },
                { x: 'Old World Delicatessen', y: 15569.28 },
                { x: 'Ottilies Käseladen', y: 15595.42 },
                { x: 'Pericles Comidas clásicas', y: 4750.92 },
                { x: 'Piccolo und mehr', y: 19167.12 },
                { x: 'Princesa Isabel Vinhos', y: 5315.96 },
                { x: 'Que Delícia', y: 6013.62 },
                { x: 'Queen Cozinha', y: 41798.64 },
                { x: 'QUICK-Stop', y: 125166.78 },
                { x: 'Rancho grande', y: 3148.2 },
                { x: 'Rattlesnake Canyon Grocery', y: 40659.6699999999 },
                { x: 'Reggiani Caseifici', y: 4529.22 },
                { x: 'Ricardo Adocicados', y: 10525.32 },
                { x: 'Richter Supermarkt', y: 17835.48 },
                { x: 'Romero y tomillo', y: 1232.52 },
                { x: 'Santé Gourmet', y: 5382.3 },
                { x: 'Save-a-lot Markets', y: 159203.1 },
                { x: 'Seven Seas Imports', y: 16545.0 },
                { x: 'Simons bistro', y: 9001.02000000001 },
                { x: 'Spécialités du monde', y: 1698.24 },
                { x: 'Split Rail Beer & Ale', y: 9911.57999999999 },
                { x: 'Suprêmes délices', y: 19113.06 },
                { x: 'The Big Cheese', y: 3119.46 },
                { x: 'The Cracker Box', y: 2217.78 },
                { x: 'Toms Spezialitäten', y: 1524.06 },
                { x: 'Tortuga Restaurante', y: 8743.86000000001 },
                { x: 'Tradição Hipermercados', y: 4510.8 },
                { x: "Trail's Head Gourmet Provisioners", y: 1443.78 },
                { x: 'Vaffeljernet', y: 16717.86 },
                { x: 'Victuailles en stock', y: 9400.56 },
                { x: 'Vins et alcools Chevalier', y: 694.02 },
                { x: 'Wartian Herkku', y: 15671.58 },
                { x: 'Wellington Importadora', y: 3281.76 },
                { x: 'White Clover Markets', y: 24103.92 },
                { x: 'Wilman Kala', y: 1767.24 },
                { x: 'Wolski  Zajazd', y: 2769.18 },
              ],
              xName: 'x',
              width: 2,
              yName: 'y',
    ...
    it('Check last gridline path in radar chart', (done: Function) => {
      chartObj.loaded = (args: Arg): void => {
        let path: Element = document.getElementById(
          'container_MajorGridLine_0_14'
        );
        let d: string = path.getAttribute('d');
        expect(
          d === 'M 300 200.125 L 257.0892841555031 103.74595421870559' ||
            d === 'M 300 199.625 L 256.9265894982728 102.88053603564855'
        ).toBe(true);
        done();
      };
      chartObj.refresh();
  ...
  describe('Polar Axis: Trim Axis Labels', () => {
    ...
    it('Check X axis labels trim with label position outside', (done: Function) => {
      chartObj.loaded = (args: Arg): void => {
        let ele: Element = document.getElementById(
          'trimContainer0_AxisLabel_0'
        );
  ...
  describe('Polar Axis: Trim Axis Labels after legend enabled', () => {
          ...
          series: [
            {
              ...
              drawType: 'Area',
              dataSource: [
                { x: '1 rue Alsace-Lorraine', y: 592792.0 },
                ...
                { x: '2817 Milton Dr.', y: 763208.0 },
                { x: '35 King George', y: 344544.0 },
                ...
                { x: 'Av. Copacabana, 267', y: 467788.0 },
                { x: 'Av. del Libertador 900', y: 215708.0 },
                ...
                {
                  x: 'Carrera 22 con Ave. Carlos Soublette #8-35',
                  y: 768888.0,
                },
                {
                  x: 'Carrera 52 con Ave. Bolívar #65-98 Llano Largo',
                  y: 595140.0,
                },
                { x: 'Cerrito 333', y: 259976.0 },
                { x: 'City Center Plaza\r\n516 Main St.', y: 209776.0 },
                { x: 'Erling Skakkes gate 78', y: 257204.0 },
                { x: 'Estrada da saúde n. 58', y: 210600.0 },
                { x: 'Fauntleroy Circus', y: 425644.0 },
                { x: 'Garden House\r\nCrowther Way', y: 424124.0 },
                { x: 'Geislweg 14', y: 424472.0 },
                { x: 'Gran Vía, 1', y: 211196.0 },
                { x: 'Hauptstr. 31', y: 342624.0 },
                { x: 'Ing. Gustavo Moncada 8585\r\nPiso 20-A', y: 215128.0 },
                { x: 'Jardim das rosas n. 32', y: 337668.0 },
                { x: 'Keskuskatu 45', y: 302600.0 },
                { x: 'Kirchgasse 6', y: 1279460.0 },
                { x: 'Mataderos  2312', y: 296780.0 },
                { x: 'P.O. Box 555', y: 378744.0 },
                { x: 'Rambla de Cataluña, 23', y: 212700.0 },
                { x: 'Rua da Panificadora, 12', y: 380356.0 },
                { x: 'Rua do Mercado, 12', y: 385028.0 },
                { x: 'Rua do Paço, 67', y: 581992.0 },
                { x: 'Rua Orós, 92', y: 294460.0 },
                { x: 'Rue Joseph-Bens 532', y: 302832.0 },
                { x: 'Sierras de Granada 9993', y: 41036.0 },
                { x: 'Smagsløget 45', y: 469944.0 },
                { x: 'South House\r\n300 Queensbridge', y: 129304.0 },
                { x: 'Starenweg 5', y: 429504.0 },
                { x: 'Strada Provinciale 124', y: 513692.0 },
                { x: 'Torikatu 38', y: 631052.0 },
                { x: 'ul. Filtrowa 68', y: 302380.0 },
                { x: 'Via Ludovico il Moro 22', y: 425304.0 },
                { x: 'Via Monte Bianco 34', y: 259112.0 },
                { x: 'Vinbæltet 34', y: 298004.0 },
              ],
              xName: 'x',
              width: 2,
              yName: 'y',
    ...
    it('Check X axis labels trim with label position outside - legend position is in right', (done: Function) => {
      chartObj.loaded = (args: Arg): void => {
        let ele0: Element = document.getElementById(
          'Trimcontainer0_AxisLabel_18'
        );
        expect(ele0.innerHTML.indexOf('...') !== -1).toBe(true);
        let ele1: Element = document.getElementById(
          'Trimcontainer0_AxisLabel_20'
        );
    ...
    it('Check X axis labels trim with label position outside - legend position is in Left', (done: Function) => {
      chartObj.loaded = (args: Arg): void => {
        let ele: Element = document.getElementById(
          'Trimcontainer0_AxisLabel_58'
        );
        expect(ele.innerHTML.indexOf('...') !== -1).toBe(true);
        let ele1: Element = document.getElementById(
          'Trimcontainer0_AxisLabel_60'
        );
        expect(ele1.innerHTML.indexOf('...') !== -1).toBe(true);
        done();
      };
