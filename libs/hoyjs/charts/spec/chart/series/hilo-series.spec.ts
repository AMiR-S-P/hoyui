  ...
  describe('HiloSeries', () => {
    ...
    it('with dateTimeRange', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          labelElement.textContent == '26 Tue' ||
  ...
  describe('checking for multiple axes', () => {
    ...
    it('checking with Plot Offset', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(point != null).toBe(true);
        expect(
          point.getAttribute('transform') == 'translate(160.5,462.5)' ||
            point.getAttribute('transform') == 'translate(151.5,471.5)'
        ).toBe(true);
        done();
        done();
      };
    ...
    it('Checking with category axis with multiple panes- column', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('d').split(' ')[1] == '101' ||
            svg.getAttribute('d').split(' ')[1] == '95'
        ).toBe(true);
        ...
        expect(
          svg.getAttribute('d').split(' ')[1] == '845.5' ||
            svg.getAttribute('d').split(' ')[1] == '854.5'
        ).toBe(true);
    ...
    it('Checking with category axis with multiple panes- rows', (done: Function) => {
      loaded = (args: Object): void => {
        let svg: HTMLElement = document.getElementById('containerAxisLine_0');
        expect(
          svg.getAttribute('d').split(' ')[2] == '851.5' ||
            svg.getAttribute('d').split(' ')[2] == '842.5'
        ).toBe(true);
        svg = document.getElementById('containerAxisLine_1');
        expect(
          svg.getAttribute('d').split(' ')[2] == '498.875' ||
            svg.getAttribute('d').split(' ')[2] == '499.375'
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('checking mouse wheel zooming', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          chartObj.primaryYAxis.zoomPosition.toFixed(2) == '0.00' ||
