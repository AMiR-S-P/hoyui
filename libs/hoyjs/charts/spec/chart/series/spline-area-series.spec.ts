...
describe('Chart Control', () => {
  ...
  describe('Chart Spline Area series', () => {
      ...
      loaded = (args: Arg): void => {
        ...
        expect(
          parseFloat(svg.getAttribute('y')) >
            series.points[4].symbolLocations[0].y
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Checking with category axis with multiple panes- column', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('d').split(' ')[4] == '457.5' ||
            svg.getAttribute('d').split(' ')[4] == '453.5'
        ).toBe(true);
        let svg1: HTMLElement = document.getElementById(
    ...
    it('Checking with category axis with multiple panes- rows', (done: Function) => {
      loaded = (args: Object): void => {
        let svg: HTMLElement = document.getElementById('containerAxisLine_0');
        expect(
          svg.getAttribute('d').split(' ')[2] == '593.5' ||
            svg.getAttribute('d').split(' ')[2] == '593.5'
        ).toBe(true);
        svg = document.getElementById('containerAxisLine_1');
        expect(
          svg.getAttribute('d').split(' ')[2] == '339.625' ||
    ...
    it('checking  zooming with touch', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          content == '0.77' || content == '0.84' || content == '0.83'
        ).toBe(true);
        chartObj.mouseLeave(
          <PointerEvent>(
  ...
  describe('checking spline area chart', () => {
    ...
    it('Empty Point with Line Series Zero mode', (done: Function) => {
      ...
      chartObj.series[0].emptyPointSettings = {
        mode: 'Zero',
        fill: 'blue',
  ...
  describe('Spline Area Series with animation', () => {
    ...
    beforeAll(() => {
      ...
      chartObj = new Chart({
        ...
        series: [
          {
            animation: { enable: true, duration: 100 },
            name: 'Gold',
            dataSource: [
              { x: 1000, y: 70 },
              { x: 2000, y: 40 },
