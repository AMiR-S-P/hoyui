...
describe('Waterfall Series', () => {
  ...
  describe('Waterfall Series', () => {
    ...
    let chartData: any[] = [
      { x: 'income', y: 4711 },
      { x: 'Marketting and Sales', y: -427 },
      { x: 'Research', y: -588 },
      { x: 'Development', y: -688 },
      { x: 'other Revenue', y: 1030 },
      { x: 'Administrative', y: -780 },
      { x: 'Other expense', y: -361 },
      { x: 'Income tax', y: -695 },
    ];
    let dateTimeData: any[] = [
      { x: new Date(1, 0, 2000), y: 4711 },
      { x: new Date(1, 0, 2001), y: -427 },
      { x: new Date(1, 0, 2002), y: -588 },
      { x: new Date(1, 0, 2003), y: -688 },
      { x: new Date(1, 0, 2004), y: 1030 },
      { x: new Date(1, 0, 2005), y: -780 },
      { x: new Date(1, 0, 2006), y: -361 },
      { x: new Date(1, 0, 2007), y: -695 },
    ];
    ...
    it('checcking with undefined values', (done: Function) => {
      loaded = (args: Object): void => {
        let label: any = document.getElementById(
    ...
    it('with connector appearence', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(seriesElements.getAttribute('stroke-dasharray') === '3').toBe(
          true
        );
        done();
      };
    ...
    it('data source with intermediatesumIndexes', (done: Function) => {
      loaded = (args: Object): void => {
        let seriesElement: HTMLCollection = document.getElementById(
          'containerSeriesGroup0'
        ).children;
    ...
    it('data source with sum Index', (done: Function) => {
      loaded = (args: Object): void => {
        let seriesElements: HTMLCollection = document.getElementById(
          'containerSeriesGroup0'
        ).children;
        expect(
          (seriesElements[8] as HTMLElement).getAttribute('fill') === '#4E81BC'
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('checking with tooltip without format', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        ).toEqual('Marketting and Sales : $-607M');
        expect(
    ...
    it('checking with tooltip with format', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        ).toEqual('series1 Marketting and Sales : $-607M');
        expect(
          parseFloat(tooltip.style.top) <
            series.points[1].regions[0].y +
              parseFloat(chartArea.getAttribute('y'))
    ...
    it('checking with track ball', (done: Function) => {
      ...
      chartObj.loaded = (args: Object): void => {
        ...
        expect(
          (<HTMLElement>text1.childNodes[0]).getAttribute('fill') ==
            'rgba(249, 250, 251, 1)'
        ).toBe(true);
        expect(
          text1.childNodes[0].textContent.replace(/\u200E/g, '') ==
            'series1 Marketting and Sales '
        ).toBe(true);
        expect(
          text1.childNodes[1].textContent.replace(/\u200E/g, '') == ':'
        ).toBe(true);
    ...
    it('Checking with template', (done: Function) => {
      ...
      loaded = (args: Object): void => {
        ...
        trigger.mousemovetEvent(target, Math.ceil(x), Math.ceil(y));
        tooltip = document.getElementById('container_tooltip');
        expect(tooltip.childNodes[0].childNodes[0].textContent).toEqual(
          'Marketting and Sales'
        );
        expect(tooltip.childNodes[0].childNodes[1].textContent).toEqual(
          '-607C'
        );
        expect(tooltip != null).toBe(true);
        y =
    ...
    it('Legend position', (done: Function) => {
      loaded = (args: Object) => {
        ...
        expect(
          parseInt(legendElement.getAttribute('x'), 10) == 325 ||
            parseInt(legendElement.getAttribute('x'), 10) == 328
        ).toBe(true);
        expect(
          parseInt(legendElement.getAttribute('y'), 10) == 46 ||
            parseInt(legendElement.getAttribute('y'), 10) == 44
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Legend alignment', (done: Function) => {
      loaded = (args: Object) => {
        ...
        expect(
          parseInt(legendElement.getAttribute('y'), 10) == 44 ||
            parseInt(legendElement.getAttribute('y'), 10) == 43
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Checking with category axis with plotoffset', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(point != null).toBe(true);
        expect(
          point.getAttribute('transform') == 'translate(78.5,85.75)' ||
            point.getAttribute('transform') == 'translate(78.5,84.75)'
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Checking with category axis with multiple panes- rows', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('transform') == 'translate(73.5,85.75)' ||
            svg.getAttribute('transform') == 'translate(73.5,84.75)'
        ).toBe(true);
        svg = document.getElementById('containerAxisLine_2');
        expect(
          svg.getAttribute('d').split(' ')[2] == '85.75' ||
            svg.getAttribute('d').split(' ')[2] == '77.25'
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
      ...
      loaded = (args: Object): void => {
        ...
        expect(
          svg
            ...
            .indexOf('translate(477.5,296.69200897216797') > -1 ||
            svg.getAttribute('transform') === 'translate(473.5,262.875)'
        ).toBe(true);
        svg = document.getElementById('container_AxisBottom_Column0');
        expect(svg.getAttribute('stroke') == 'red').toBe(true);
        svg = document.getElementById('containerAxisLine_2');
        expect(
          svg.getAttribute('d').split(' ')[1] == '477.5' ||
            svg.getAttribute('d').split(' ')[1] == '473.5' ||
            svg.getAttribute('d').split(' ')[1] == '473.5'
        ).toBe(true);
    ...
    it('Checking with fabric theme color', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          document.getElementById(prefix + 0 + suffix + 1).getAttribute('fill')
        ).toBe(fabric[1]);
        expect(
          document.getElementById(prefix + 0 + suffix + 3).getAttribute('fill')
        ).toBe(fabric[4]);
        done();
      };
    ...
    it('checking pinch zooming', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          content == '1.00' || content == '0.33' || content == '0.30'
        ).toBe(true);
  ...
  describe('Checking connector lines', () => {
    ...
    it('Checking connector line with start point value 0', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(d).toBe(
          'M 18.31875000000001 371.5 L 314.43125 371.5 M 184.69374999999997 371.5 L 341.06874999999997 371.5 M 490.80625000000003 148.6 L 507.44374999999997 148.6 '
        );
        done();
      };
      chart.loaded = loaded;
    ...
    it('Checking XAxis Isinversed', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let d: string = series.getAttribute('d');
        expect(d).toBe(
          'M 507.44374999999997 371.5 L 490.80625000000003 371.5 M 341.06874999999997 371.5 L 324.43125000000003 371.5 M 174.69374999999997 148.6 L 158.05625000000003 148.6 '
        );
        done();
      };
    ...
    it('Checking connector line with start point value 0 chart is transposed', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let d: string = series.getAttribute('d');
        expect(d).toBe(
          'M 0 296.61875 L 0 169.13125 M 0 218.99375 L 0 151.36875 M 410.09999999999997 81.50624999999998 L 410.09999999999997 73.74375000000002 '
        );
        done();
      };
    ...
    it('Checking Istransposed with XAxis Isinversed', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let d: string = series.getAttribute('d');
        expect(d).toBe(
          'M 0 73.74375000000002 L 0 81.50625 M 0 151.36875 L 0 159.13125 M 410.09999999999997 228.99375 L 410.09999999999997 236.75625 '
        );
        done();
      };
      chart.primaryXAxis.isInversed = true;
