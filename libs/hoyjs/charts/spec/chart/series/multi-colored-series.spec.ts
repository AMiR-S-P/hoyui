 ...
 */
import { createElement, EmitType, debounce } from '@hoyui/base';
...
let numericData: number[] = [
  29.9, 71.5, 106.4, 129.2, 144.0, 176.0, 135.6, 148.5, 216.4, 4.1, 95.6, 54.4,
];
let emptyData: number[] = [
  29.9,
  71.5,
  106.4,
  null,
...
function getData(
  ...
  for (var i = 0; i < 12; i++) {
    var point = {
      ...
      YValue: isEmpty ? emptyData[i] : numericData[i],
      color: interiors[i % 16],
    };
    series1.push(point);
  }
  return series1;
    ...
    it('Checking with stroke while applied border', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let svg: HTMLElement = document.getElementById(
          'container_Series_border_0_Segment_1'
        );
