...
describe('Chart Control - Box and Whisker Series', () => {
  ...
  describe('Box plot - default rendering', () => {
            ...
            dataSource: [
              {
                x: 'Development',
                yValues: [
                  22, 22, 23, 25, 25, 25, 26, 27, 27, 28, 28, 29, 30, 32, 34,
                  32, 34, 36, 35, 38,
                ],
              },
              {
                x: 'Testing',
                yValues: [22, 33, 23, 25, 26, 28, 29, 30, 34, 33, 32, 31, 50],
              },
              {
                x: 'HR',
                yValues: [
                  22, 24, 25, 30, 32, 34, 36, 38, 39, 41, 35, 36, 40, 56,
                ],
              },
              {
                x: 'Finance',
                yValues: [26, 27, 28, 30, 32, 34, 35, 37, 35, 37, 45],
              },
              {
                x: 'R&D',
                yValues: [26, 27, 29, 32, 34, 35, 36, 37, 38, 39, 41, 43, 58],
              },
              {
                x: 'Sales',
                yValues: [27, 26, 28, 29, 29, 29, 32, 35, 32, 38, 53],
              },
              {
                x: 'Inventory',
                yValues: [21, 23, 24, 25, 26, 27, 28, 30, 34, 36, 38],
              },
      ...
      chartObj.series[0].dataSource = [
        {
          x: 'Development',
          yValues: [
            22, 22, 23, 25, 25, 25, 26, 27, 27, 28, 28, 29, 30, 32, 34, 32, 34,
            36, 35, 38,
          ],
        },
      ];
    ...
    it('checking inverted axis', (done: Function) => {
      ...
      chartObj.series[0].dataSource = [
        ...
        {
          x: 3,
          yValues: [22, 24, 25, 30, 32, 34, 36, 38, 39, 41, 35, 36, 40, 56],
        },
  ...
  describe('Box plot - checking marker, datalabel', () => {
    ...
    it('Checking data label present in isTransposed true', (done: Function) => {
      chartObj.loaded = (args: Object): void => {
        let dataLabel: HTMLElement = document.getElementById(
          'container_Series_0_Point_0_Text_0'
        );
        expect(dataLabel).not.toBe(null);
        dataLabel = document.getElementById(
          'container_Series_0_Point_0_Text_1'
        );
        expect(dataLabel).not.toBe(null);
        dataLabel = document.getElementById(
          'container_Series_0_Point_0_Text_2'
        );
        expect(dataLabel).not.toBe(null);
        dataLabel = document.getElementById(
          'container_Series_0_Point_0_Text_3'
        );
        expect(dataLabel).not.toBe(null);
        dataLabel = document.getElementById(
          'container_Series_0_Point_0_Text_4'
        );
        expect(dataLabel).not.toBe(null);
        dataLabel = document.getElementById(
    ...
    it('Checking data label postioin as middle', (done: Function) => {
      chartObj.loaded = (args: Object): void => {
        svg = getElement('container_Series_0_Point_1_Symbol');
        svg1 = getElement('container_Series_0_Point_1_Text_5');
        expect(+svg1.getAttribute('y') - +svg.getAttribute('cy') < 4).toBe(
          true
        );
        svg = getElement('container_Series_0_Point_7_Symbol');
        svg1 = getElement('container_Series_0_Point_7_Text_5');
    ...
    it('Checking data label with shape', (done: Function) => {
      chartObj.loaded = (args: Object): void => {
        ...
        expect(svg.childElementCount == 32 || svg.childElementCount == 29).toBe(
          true
        );
        done();
      };
    ...
    it('Checking data label with template', (done: Function) => {
      chartObj.loaded = (args: Object): void => {
        ...
        expect(svg.childElementCount == 30 || svg.childElementCount == 28).toBe(
          true
        );
        done();
      };
      chartObj.series[0].marker.dataLabel.template =
        '<div>${point.average}</div>';
      chartObj.refresh();
      unbindResizeEvents(chartObj);
    });
  });
  describe('Box plot - checking user interaction', () => {
    ...
    it('Checking default tooltip', (done: Function) => {
      chartObj.loaded = (args: Object): void => {
        ...
        expect(
          text1.textContent.replace(/\u200E/g, '') == 'TestingOutliers : 50'
        ).toBe(true);
        svg = getElement('container_Series_0_Point_1_Trackball_0');
        expect(svg != null).toBe(true);
    ...
    it('Checking default tooltip - maximum position - overlapped', (done: Function) => {
      chartObj.loaded = (args: Object): void => {
        ...
        y =
          parseFloat(pathElements[54]) +
          parseFloat(chartArea.getAttribute('y')) +
          elem.offsetTop;
        x =
          parseFloat(pathElements[53]) +
          parseFloat(chartArea.getAttribute('x')) +
          elem.offsetLeft;
        trigger.mousemovetEvent(targetElement, Math.ceil(x), Math.ceil(y));
        let tooltip: HTMLElement = document.getElementById('container_tooltip');
        ...
        expect(
          text1.textContent.replace(/\u200E/g, '') ==
            'FinanceMaximum : 45Q3 : 37Median : 34Q1 : 28Minimum : 26'
        ).toBe(true);
        done();
      };
      chartObj.refresh();
