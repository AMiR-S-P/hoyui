...
export let doubleData: any[] = [
  { x: 1, low: -12, high: 0, open: -1.22, close: -8.44 },
  { x: 2, low: 12, high: 1, open: 5, close: 9 },
  { x: 3, low: 23, high: 10, open: 13, close: 20.44 },
  { x: 4, low: 20, high: 43, open: 33.22, close: 21.44 },
  { x: 5, low: 0, high: 10, open: 5, close: 9 },
  { x: 6, low: -22, high: 34, open: 3, close: 22 },
  { x: 7, low: -12, high: 23, open: 12, close: 4 },
  { x: 8, low: 12, high: 40, open: 32, close: 15 },
];

export let doubleData1: any[] = [
  { x: 1, low: 8, high: 20, open: 18, close: 12 },
  { x: 2, low: 2, high: 18, open: 15, close: 9 },
  { x: 3, low: 23, high: 10, open: 13, close: 20.44 },
  { x: 4, low: 20, high: 43, open: 33.22, close: 21.44 },
];

export let doubleData2: any[] = [
  { x: 1, low: 80, high: 120, open: 110, close: 90 },
  { x: 2, low: 70, high: 95, open: 89, close: 82 },
  { x: 3, low: 55, high: 77, open: 73.22, close: 63.44 },
  { x: 4, low: 60, high: 80, open: 77, close: 69 },
];

export let doubleData3: any[] = [
  { x: 11, low: 8, high: 20, open: 18, close: 12 },
  { x: 12, low: 2, high: 18, open: 15, close: 9 },
  { x: 13, low: 23, high: 10, open: 13, close: 20.44 },
  { x: 14, low: 20, high: 43, open: 33.22, close: 21.44 },
];

export let categoryData: any[] = [
  { x: 'USA', high: 125.45, low: 70.23, open: 112.22, close: 90.44 },
  { x: 'Austria', high: 150.99, low: 60.23, open: 120.55, close: 70.9 },
  { x: 'Germany', high: 200.19, low: 130.37, open: 160.13, close: 190.78 },
  { x: 'Swedan', high: 160.23, low: 90.16, open: 140.38, close: 110.24 },
  { x: 'France', high: 200.89, low: 100.23, open: 180.9, close: 120.29 },
...
let logdata: any[] = [
  ...
  { x: 3, high: 5004.89, low: 2104.23, open: 4182.9121375, close: 2999.29 },
  ...
  describe('HiloOpenClose Series', () => {
    ...
    it('checking with tooltip', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          ...
          'HiloOpenCloseAustriaHigh : $150.99Low : $60.23Open : $120.55Close : $70.9'
        );
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Tooltip with given label format', (done: Function) => {
      ...
      chartObj.tooltip.format =
        '${series.name}  <br/> x : ${point.x} <br/> High : ${point.high} <br/> Low : ${point.low} <br/> Open : ${point.open} <br/> Close: ${point.close}';
      chartObj.loaded = (args: Object): void => {
    ...
    it('Checking with category axis with plotoffset', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(point != null).toBe(true);
        expect(
          point.getAttribute('d').split(' ')[2] == '393.5' ||
            point.getAttribute('d').split(' ')[2] == '389.5'
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Cluster selection', (done: Function) => {
      loaded = () => {
        ...
        expect(element).toBe(<HTMLElement>selected[0]);
        let selected1: HTMLCollection = document.getElementsByClassName(
    ...
    it('Legend position', (done: Function) => {
      loaded = (args: Object) => {
        ...
        expect(
          legendElement.getAttribute('y') === '44.25' ||
            legendElement.getAttribute('y') === '43.25'
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Checking with category axis with multiple panes- rows', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('d').split(' ')[2] == '239.625' ||
            svg.getAttribute('d').split(' ')[2] == '239.125'
        ).toBe(true);
        svg = document.getElementById('containerAxisLine_2');
        expect(
          svg.getAttribute('d').split(' ')[2] == '85.75' ||
            svg.getAttribute('d').split(' ')[2] == '84.75'
        ).toBe(true);
    ...
    it('Checking with category axis with multiple panes- column', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          svg.getAttribute('x1') == '59.5' || svg.getAttribute('x1') == '63.5'
        ).toBe(true);
        expect(svg.getAttribute('stroke') == 'red').toBe(true);

        svg = document.getElementById('container_AxisBottom_Column1');
        expect(
          svg.getAttribute('x1') == '459.5' || svg.getAttribute('x1') == '463.5'
        ).toBe(true);
    ...
    it('checking with multiple different series ', (done: Function) => {
      ...
      loaded = (args: Object): void => {
        ...
        expect(
          series0.type === 'HiloOpenClose' || series1.type === 'Line'
        ).toBe(true);
        done();
      };
  ...
  describe('checking for multiple axes', () => {
    ...
    it('checking with Multiple Series', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        let series1: Series = <Series>chartObj.series[1];
        expect(
          series0.type === 'HiloOpenClose' || series1.type === 'Column'
        ).toBe(true);
        done();
      };
      ...
      loaded = (args: Object): void => {
        ...
        expect(
          chartObj.primaryYAxis.zoomPosition.toFixed(2) == '0.00' ||
            chartObj.primaryYAxis.zoomPosition.toFixed(2) == '0.08'
        ).toBe(true);
        done();
      };
    ...
    it('checking  zooming with touch', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          content == '0.33' || content == '0.30' || content == '0.31'
        ).toBe(true);
        content = chartObj.primaryYAxis.zoomFactor.toFixed(2);
        expect(
          content == '1.00' || content == '0.77' || content === '0.83'
        ).toBe(true);
        content = chartObj.primaryXAxis.zoomPosition.toFixed(2);
        expect(
          content == '0.72' || content == '0.69' || content == '0.70'
        ).toBe(true);
        chartObj.mouseLeave(
          <PointerEvent>(
  ...
  describe('HiloOpenCLose - Checking setData method', () => {
    ...
    it('Checking HiloOpenClose updated direction', (done: Function) => {
      ...
      let seriesData = [
        ...
        { x: 3, low: 23, high: 10, open: 13, close: 20.44 },
        { x: 4, low: 20, high: 43, open: 33.22, close: 21.44 },
        { x: 5, low: 0, high: 10, open: 8, close: 9 },
        { x: 6, low: -22, high: 34, open: 3, close: 22 },
        { x: 7, low: -12, high: 23, open: 11, close: 4 },
        { x: 8, low: 12, high: 40, open: 32, close: 15 },
      ];
