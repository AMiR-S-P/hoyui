...
describe('Chart Control', () => {
  ...
  describe('Chart Spline series', () => {
    ...
    it('checking with marker shape image', (done: Function) => {
      loaded = (args: Object): void => {
        svg = document.getElementById('container_Series_0_Point_0_Symbol');
        expect(svg.getAttribute('href') === 'base/spec/img/img1.jpg').toBe(
          true
        );
        done();
      };
    ...
    it('Default animation', (done: Function) => {
      let animate: EmitType<IAnimationCompleteEventArgs> = (
        ...
        let element: HTMLElement = document.getElementById(
          'container_ChartSeriesClipRect_0'
        ).childNodes[0] as HTMLElement;
        expect(
          series.clipRect.width === parseFloat(element.getAttribute('width'))
        ).toBe(true);
        done();
      };
  ...
  /**
   * Cheacking point drag and drop with spline series
   */
  describe('Spline series with drag and drop support', () => {
    let chartObj: Chart;
    let x: number;
    let y: number;
    let loaded: EmitType<ILoadedEventArgs>;
    ...
    it('spline series drag and drop with marker true', (done: Function) => {
      loaded = (): void => {
  ...
  describe('Spline Series - Checking animation on data changes.', () => {
    ...
    it('Spline series - checking remove point method', (done: Function) => {
      ...
      chartObj.series[0].removePoint(9);
      chartObj.series[0].removePoint(10);
      chartObj.refresh();
    });
  });
  it('memory leak', () => {
