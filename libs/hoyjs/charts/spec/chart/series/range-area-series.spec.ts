...
let data: object[] = [
  ...
  { x: 'Dec', high: 2.96, low: 1.5 },
];

export let categoryData: object[] = [
  { x: 'Jan', low: 0.7, high: 7.7 },
  { x: 'Feb', low: 1.3, high: 8.1 },
  { x: 'Mar', low: 1.9, high: 8.5 },
  { x: 'Apr', low: 3.1, high: 10.8 },
  { x: 'May', low: 5.7, high: 14.4 },
  { x: 'Jun', low: 8.4, high: 16.9 },
  ...
  describe('Range Area Series', () => {
    ...
    it('with Category dataSource', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(seriesElements == 3).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Showing default marker', (done: Function) => {
      loaded = (args: Object): void => {
        let series1: number = document.getElementById('containerSymbolGroup0')
          .childNodes.length;
    ...
    it('Checking with category axis with plotoffset', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(point != null).toBe(true);
        expect(
          point.getAttribute('d').split(' ')[2] == '355.5' ||
            point.getAttribute('d').split(' ')[2] == '360.5'
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('checking with datalabel outer position', (done: Function) => {
      loaded = (args: Object): void => {
        let element1: number = +document
          ...
          .getAttribute('y');
        expect(
          (<Points>(<Series>chartObj.series[0]).points[2]).symbolLocations[0]
            .y > element1
        ).toBe(true);
        done();
      };
    ...
    it('Selection mode DragX', (done: Function) => {
      loaded = () => {
        ...
        expect(
          element.getAttribute('y') == '423' ||
            element.getAttribute('y') == '45.25'
        );
        expect(
          element.getAttribute('height') == '567.75' ||
            element.getAttribute('height') == '568.75'
        ).toBe(true);
    ...
    it('Selection mode DragY', (done: Function) => {
      loaded = () => {
        ...
        element = document.getElementById(draggedRectGroup);
        expect(
          element.getAttribute('x') == '73.5' ||
            element.getAttribute('x') == '72.5' ||
            element.getAttribute('x') == '53.25' ||
            element.getAttribute('x') == '68.5' ||
            element.getAttribute('x') == '57.25'
        ).toBe(true);
        expect(element.getAttribute('y')).toEqual('92');
        expect(element.getAttribute('height') == '140').toBe(true);
        expect(
          element.getAttribute('width') == '1116.5' ||
            element.getAttribute('width') == '1133' ||
            element.getAttribute('width') == '1117.5' ||
            element.getAttribute('width') == '1121.5' ||
            element.getAttribute('width') == '1137'
        ).toBe(true);
        done();
      };
    ...
    it('checking with tooltip', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        ).toEqual('IndiaFebHigh : 6.3Low : 1.3');
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Tooltip with given label format', (done: Function) => {
      ...
      chartObj.tooltip.format =
        '${series.name}  <br/> x : ${point.x} <br/> High : ${point.high} <br/> Low : ${point.low} <br/>';
      chartObj.loaded = (args: Object): void => {
    ...
    it('checking with cross hair', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          element1.textContent == '8.352' ||
            element1.textContent == '8.321' ||
            element1.textContent == '8.336' ||
            element1.textContent == '8.326'
        ).toBe(true);
        done();
      };
      chartObj.loaded = loaded;
  ...
  describe('Inversed Range Area Series', () => {
    ...
    it('Checking inversed range area series', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          Math.round(bounds.top) == 81 || Math.round(bounds.top) == 82
        ).toBe(true);
        expect(bounds.left == 92.5 || bounds.left == 67.5).toBe(true);
        expect(bounds.width == 705.5 || bounds.width == 730.5).toBe(true);
        expect(
          Math.round(bounds.height) == 237 || Math.round(bounds.height) == 232
        ).toBe(true);
        let element1: number = +document
          .getElementById('container_Series_0_Point_2_Text_0')
          .getAttribute('y');
        expect(
          Math.round(element1) == 92 ||
            Math.round(element1) == 91 ||
            Math.round(element1) == 96
        ).toBe(true);
        element1 = +document
          .getElementById('container_Series_0_Point_2_Text_1')
          .getAttribute('y');
        expect(Math.round(element1) == 51 || Math.round(element1) == 52).toBe(
          true
        );
        done();
      };
    ...
    it('Checking transposed range area series', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          bounds.left == 110.62692260742188 ||
            bounds.left == 109.69615173339844 ||
            bounds.left == 106.90384674072266
        ).toBe(true);
        expect(
          bounds.width == 636.2461547851562 ||
            bounds.width == 637.107666015625 ||
            bounds.width == 639.6922607421875
        ).toBe(true);
        expect(
          bounds.height == 317.25 ||
            bounds.height == 318.25 ||
            bounds.height == 310.25
        ).toBe(true);
        let element1: number = +document
          .getElementById('container_Series_0_Point_2_Text_0')
          .getAttribute('x');
        expect(Math.round(element1) == 549 || Math.round(element1) == 550).toBe(
          true
        );
        element1 = +document
          .getElementById('container_Series_0_Point_2_Text_1')
          .getAttribute('x');
        expect(Math.round(element1) == 613 || Math.round(element1) == 613).toBe(
          true
        );
        done();
      };
    ...
    it('Checking inversed & transposed range area series', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        element1 = +document
          .getElementById('container_Series_0_Point_2_Text_1')
          .getAttribute('x');
        expect(Math.round(element1) == 611 || Math.round(element1) == 613).toBe(
          true
        );
