...
describe('3DChart Control', () => {
  ...
  describe('Chart Tooltip', () => {
    ...
    it('Default Tooltip', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        const tooltip: HTMLElement =
          document.getElementById('container_tooltip');
        expect(tooltip != null).toBe(true);
        const group: HTMLElement = tooltip.childNodes[0]
    ...
    it('Edge Tooltip', () => {
      ...
      expect(tooltip != null).toBe(true);
      const text2: HTMLElement = tooltip.childNodes[0].childNodes[0]
        .childNodes[1] as HTMLElement;
      expect(text2.textContent.replace(/\u200E/g, '')).toEqual(
        'ChartSeriesNameGold1000 : 70'
      );
      targetElement = chartObj.element.querySelector(
        '#container-svg-0-region-series-0-point-7'
      ) as HTMLElement;
      rect = targetElement.getBoundingClientRect();
      x = window.scrollX + rect.left + rect.width / 2;
      y = window.scrollY + rect.top;
      trigger.mousemovetEvent(targetElement, Math.ceil(x), Math.ceil(y));
      const text1: HTMLElement = tooltip.childNodes[0].childNodes[0]
        .childNodes[1] as HTMLElement;
      expect(
        text1.textContent.replace(/\u200E/g, '') ===
          'ChartSeriesNameGold8000 : 70'
      ).toBe(true);
      expect(
        (<HTMLElement>(
  ...
  describe('Circular3d chart Tooltip', () => {
    ...
    it('Tooltip with fill', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        trigger.mousemovetEvent(targetElement, Math.ceil(x), Math.ceil(y));
        expect(
          document.getElementById('container_tooltip_path').getAttribute('fill')
        ).toBe('red');
        done();
      };
      chartObj.loaded = loaded;
    ...
    it('Tooltip with Material3 theme', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        expect(
          document
            .getElementById('container_tooltip_text')
    ...
    it('Tooltip with template', (done: Function) => {
      ...
      chartObj.tooltip.template =
        '<div id="templateWrap"> <table style="width:100%;  border: 1px solid black;"><tr><th colspan="2" bgcolor="#00FFFF">Unemployment</th></tr><tr><td bgcolor="#00FFFF">${x}:</td><td bgcolor="#00FFFF">${y}</td></tr></table></div>';
      chartObj.refresh();
    });
    ...
    it('Donut - Tooltip with data label', (done: Function) => {
      loaded = (args: Object): void => {
        targetElement = document.getElementById(
          'container-svg-data-label-text-2'
        ) as HTMLElement;
        rect = targetElement.getBoundingClientRect();
        x = window.scrollX + rect.left + rect.width / 2;
        y = window.scrollY + rect.top;
        trigger.mousemovetEvent(targetElement, Math.ceil(x), Math.ceil(y));
        targetElement = document.getElementById(
          'container-svg-data-label-series-0-shape-1'
        );
        rect = targetElement.getBoundingClientRect();
        x = window.scrollX + rect.left + rect.width / 2;
        y = window.scrollY + rect.top;
  ...
  describe('Circular3d chart Tooltip', () => {
    ...
    beforeAll(() => {
      ...
      chartObj = new CircularChart3D({
        ...
        // Initialize the chart title
        title: 'Fruit Consumption',
        tooltip: { enable: true, location: { x: 15, y: 25 } },
        legendSettings: { visible: true, enableHighlight: true },
      });
      chartObj.appendTo('#container');
    });
    ...
    it('Without enable marker', (done: Function) => {
      loaded = (args: Object): void => {
        ...
        y = window.scrollY + rect.top;
        trigger.mousemovetEvent(targetElement, Math.ceil(x), Math.ceil(y));
        const tooltipMarker: HTMLElement = document.getElementById(
          'container_tooltip_trackball_group'
        );
