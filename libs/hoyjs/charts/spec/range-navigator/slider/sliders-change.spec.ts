...
import {
  IChangedEventArgs,
  IRangeEventArgs,
} from '../../../src/range-navigator/model/range-navigator-interface';
  ...
  describe('with Sliders double axis', () => {
    ...
    it('checking with left slider moving', (done: Function) => {
      range.loaded = (args: object) => {
        ...
        let leftElement: Element = document.getElementById(
      ...
      range.changed = (args: IChangedEventArgs) => {
        if (isCheck) {
          expect(
            Math.round(+args.start) >= 12 && Math.round(+args.start) < 15
          ).toBe(true);
    ...
    it('checking with left slider position less than startX', (done: Function) => {
      ...
      range.changed = (args: IChangedEventArgs) => {
        if (isCheck) {
          ...
          expect(Math.ceil(+args.end) > 20 && Math.ceil(+args.end) < 35).toBe(
            true
          );
          expect(getElement('container_Series_0').getAttribute('stroke')).toBe(
            '#a16ee5'
          );
          isCheck = false;
          done();
    ...
    it('checking with right slider moving out side of selected area', (done: Function) => {
      ...
      range.changed = (args: IChangedEventArgs) => {
        if (isCheck) {
          expect(
            getElement('container_RightSlider_ThumpSymbol').getAttribute('fill')
          ).toBe('#262626');
          expect(Math.floor(+args.start)).toEqual(20);
          expect(Math.ceil(+args.end) > 21 && Math.ceil(+args.end) < 38).toBe(
            true
          );
          isCheck = false;
          done();
    ...
    it('checking with right slider moving in side of selected area', (done: Function) => {
      range.loaded = (args: object) => {
        ...
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(targetElement, null, null, null, null, 200, 20)
          )
        );
        range.mouseMove(
          <PointerEvent>(
            trigger.onTouchMove(targetElement, null, null, null, null, 100, 20)
          )
        );
        range.mouseEnd(
          <PointerEvent>(
            trigger.onTouchEnd(targetElement, null, null, null, null, 400, 20)
          )
        );
      ...
      range.changed = (args: IChangedEventArgs) => {
        if (isCheck) {
          expect(
            Math.floor(+args.start) > 10 && Math.floor(+args.start) < 15
          ).toBe(true);
          expect(Math.ceil(+args.end)).toEqual(20);
          isCheck = false;
        }
    ...
    it('checking with label click', (done: Function) => {
      range.loaded = (args: Object): void => {
        ...
        let element: Element = <Element>(
          document.getElementById('container_AxisLabels').firstChild.firstChild
        );
        let pageX: number = +element.getAttribute('x');
        let pageY: number = +element.getAttribute('y');
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(element, null, null, null, null, pageX, pageY)
          )
        );
        range.mouseMove(
          <PointerEvent>(
            trigger.onTouchEnd(element, null, null, null, null, 0, pageY)
          )
        );
        range.mouseEnd(
          <PointerEvent>(
      ...
      range.changed = (args: IChangedEventArgs) => {
        if (isCheck) {
          expect(
            Math.ceil(+args.start) > 11 && Math.ceil(+args.start) < 30
          ).toBe(true);
          isCheck = false;
        }
        done();
    ...
    it('checking with right slider moving in side of unselected area', (done: Function) => {
      range.loaded = (args: object) => {
        ...
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(element, null, null, null, null, 200, 20)
          )
        );
        range.mouseMove(
          <PointerEvent>(
      ...
      range.changed = (args: IChangedEventArgs) => {
        if (isCheck) {
          ...
          expect(
            Math.ceil(+args.end) == 20 || Math.ceil(+args.end) == 21
          ).toEqual(true);
          isCheck = false;
        }
        done();
        ...
        series: [
          {
            dataSource: [
              ...
              { x: new Date(2000, 5), y: 12 },
              { x: new Date(2001, 0), y: 22 },
              { x: new Date(2001, 7), y: 16 },
            ],
            xName: 'x',
            yName: 'y',
            type: 'Line',
