  ...
  describe('with Sliders', () => {
    ...
    it('checking with slider selected area', (done: Function) => {
      range.loaded = (args: Object): void => {
        ...
        expect(
          +element.getAttribute('width') >= 507.75 &&
            +element.getAttribute('width') <= 561
        ).toBe(true);
        done();
      };
      range.series = [
    ...
    it('checking with thump fill mouse event', (done: Function) => {
      range.loaded = (args: Object): void => {
        ...
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(element, 608, 189, null, null, 504, 289)
          )
        );
        range.mouseEnd(
          <PointerEvent>(
            trigger.onTouchEnd(element, 608, 189, null, null, 504, 289)
          )
        );
        expect(element != null).toBe(true);
        done();
    ...
    it('checking with right slider mouse event', (done: Function) => {
      range.loaded = (args: Object): void => {
        let targetElement: Element = document.getElementById(
          'container_RightSlider_ThumpSymbol'
        );
        ...
        let thumbTransform: string = document
          .getElementById('container_RightSlider')
          .getAttribute('transform');
        // eslint-disable-next-line
        expect(
          thumbTransform === 'translate(380.99999999999994, 0)' ||
            thumbTransform === 'translate(381, 0)' ||
            thumbTransform === 'translate(381.00000000000006, 0)'
        ).toBe(true);
    ...
    it('checking with left slider mouse event', (done: Function) => {
      range.loaded = (args: Object): void => {
        let targetElement: Element = document.getElementById(
          'container_LeftSlider_ThumpSymbol'
        );
        // eslint-disable-next-line @typescript-eslint/indent
        range.rangeOnMouseDown(
          <PointerEvent>(
        ...
        expect(targetElement != null).toBe(true);
        let thumbTransform: string = document
          .getElementById('container_LeftSlider')
          .getAttribute('transform');
        expect(
          thumbTransform === 'translate(380.99999999999994, 0)' ||
            thumbTransform === 'translate(381, 0)' ||
    ...
    it('checking with right unselected area mouse move event', (done: Function) => {
      range.loaded = (args: Object): void => {
        ...
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(targetElement, 590, 89, null, null, 404, 189)
          )
        );
        range.mouseMove(
          <PointerEvent>(
            trigger.onTouchMove(targetElement, 628, 289, null, null, 304, 289)
          )
        );
        element = document.getElementById('container_SelectedArea');
        expect(
          +element.getAttribute('width') >= 99 &&
            +element.getAttribute('width') <= 101
        ).toBe(true);
        expect(targetElement != null).toBe(true);
        done();
      };
    ...
    it('checking with right unselected area mouse down event', (done: Function) => {
      range.loaded = (args: Object): void => {
        let targetElement: Element = document.getElementById(
          'container_rightUnSelectedArea'
        );
        // eslint-disable-next-line @typescript-eslint/indent
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(targetElement, 720, 180, null, null, 504, 280)
          )
        );
        range.mouseEnd(
          <PointerEvent>(
    ...
    it('checking with left unselected area mouse down event', (done: Function) => {
      range.loaded = (args: Object): void => {
        let targetElement: Element = document.getElementById(
          'container_leftUnSelectedArea'
        );
        // eslint-disable-next-line @typescript-eslint/indent
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(targetElement, 420, 80, null, null, 204, 180)
          )
        );
        range.mouseEnd(
          <PointerEvent>(
    ...
    it('checking with allow snapping', (done: Function) => {
      range.loaded = (args: Object): void => {
        ...
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(element, 590.25, 89, null, null, 404.5, 189)
          )
        );
        range.mouseMove(
          <PointerEvent>(
            trigger.onTouchStart(element, 720.5, 180, null, null, 604.5, 280)
          )
        );
        range.mouseEnd(
          <PointerEvent>(
        ...
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(targetElement, 630, 89, null, null, 454, 189)
          )
        );
        range.mouseLeave(
          <PointerEvent>(
            trigger.onTouchLeave(targetElement, 630, 89, null, null, 454, 189)
          )
        );
        expect(element != null).toBe(true);
        done();
    ...
    it('checking with mouse down event with period selector enabled', (done: Function) => {
      range.loaded = (args: Object): void => {
        ...
        expect(range.periodSelectorModule.control.periods.length === 7).toBe(
          true
        );
        done();
      };
  ...
  describe('RTL with date time axis', () => {
    ...
    it('checking with date time axis', (done: Function) => {
      range.loaded = (args: Object): void => {
        ...
        expect(
          element.firstChild.textContent === 'Quarter1' ||
            element.firstChild.textContent === 'Q1 2001' ||
            element.firstChild.textContent === 'Jul'
        ).toBe(true);
        // eslint-disable-next-line
    ...
    it('checking with date time axis grouping label mouse down', (done: Function) => {
      range.loaded = (args: Object): void => {
        ...
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(element, 434, 105, null, null, 470, 120)
          )
        );
        range.mouseEnd(
          <PointerEvent>(
        ...
        expect(element.getAttribute('x') < element1.getAttribute('x')).toBe(
          true
        );
        done();
      };
    ...
    it('checking with allowSnapping click with firstlevel label', (done: Function) => {
      range.loaded = (args: Object): void => {
        ...
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(element, 286, 296, null, null, 290, 386)
          )
        );
        range.mouseEnd(
          <PointerEvent>(
    ...
    it('checking with label position inside', (done: Function) => {
      range.loaded = (args: Object): void => {
        ...
        range.rangeOnMouseDown(
          <PointerEvent>(
            trigger.onTouchStart(targetElement, 286, 296, null, null, 290, 386)
          )
        );
        range.mouseEnd(
          <PointerEvent>(
        ...
        range.mouseMove(
          <PointerEvent>(
            trigger.onTouchStart(targetElement, 286, 186, null, null, 404, 286)
          )
        );
        expect(element.getAttribute('y') < element1.getAttribute('y')).toBe(
          true
        );
        done();
      };
