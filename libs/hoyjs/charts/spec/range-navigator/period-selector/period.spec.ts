...
describe('Range navigator', () => {
  ...
  describe('with default case', () => {
    ...
    beforeAll(() => {
      ...
      range = new RangeNavigator({
        ...
        periodSelectorSettings: {
          position: 'Top',
          height: 200,
          periods: [{ intervalType: 'Days', interval: 3, text: '3d' }],
        },
      });
    ...
    it('checking with selector position as bottom', (done: Function) => {
      range.loaded = (args: Object) => {
        let period: HTMLElement = document.getElementById(
    ...
    it('checking days click', (done: Function) => {
      ...
      range.changed = (args: IChangedEventArgs) => {
        if (isCheck) {
          expect(
            args.start.toString() ===
              'Wed May 02 2018 00:00:00 GMT+0530 (India Standard Time)' ||
              args.start.toString() ===
                'Wed May 02 2018 00:00:00 GMT+0000 (Coordinated Universal Time)'
          ).toBe(true);
      ...
      range.series[0].dataSource = [
        ...
        { x: new Date(2018, 4, 2), y: 10 },
        { x: new Date(2018, 4, 3), y: 8 },
        { x: new Date(2018, 4, 5), y: 12 },
      ];
      range.periodSelectorSettings.position = 'Top';
    ...
    it('checking weeks click', (done: Function) => {
      ...
      range.changed = (args: IChangedEventArgs) => {
        if (isCheck) {
          expect(
            args.start.toString().indexOf('May') > -1 ||
              args.start.toString().indexOf('Apr') > -1
          ).toBe(true);
          isCheck = false;
        }
        done();
      ...
      range.periodSelectorSettings.periods = [
        { intervalType: 'Weeks', interval: 2, text: '3w' },
      ];
      range.refresh();
    });
    it('checking months click', (done: Function) => {
      ...
      range.changed = (args: IChangedEventArgs) => {
        if (isCheck) {
          ...
          expect(
            args.start.toString().indexOf('Mar') > -1 ||
              args.start.toString().indexOf('May') > -1 ||
              args.start.toString().indexOf('Apr') > -1
          ).toBe(true);
          isCheck = false;
      ...
      range.periodSelectorSettings.periods = [
        { intervalType: 'Months', interval: 2, text: '2m' },
      ];
      range.refresh();
    });
    it('checking quarter click', (done: Function) => {
      ...
      range.periodSelectorSettings.periods = [
        { intervalType: 'Quarter', interval: 1, text: '1q' },
      ];
      range.refresh();
    });
  ...
  describe('with default case', () => {
    ...
    beforeAll(() => {
      ...
      range = new RangeNavigator({
        series: [
          {
            dataSource: [
              { x: new Date(2018, 4, 3, 4), y: 5 },
              { x: new Date(2018, 4, 3, 5), y: 5 },
            ],
            xName: 'x',
            yName: 'y',
            type: 'Line',
        ...
        periodSelectorSettings: {
          ...
          height: 200,
          periods: [
            { intervalType: 'Hours', interval: 3, text: '3h' },
            { intervalType: 'Minutes', interval: 30, text: '30m' },
            { intervalType: 'Seconds', interval: 100, text: '100sec' },
          ],
        },
      });
      range.appendTo('#container');
    ...
    it('checking hours click', (done: Function) => {
      ...
      range.changed = (args: IChangedEventArgs) => {
        if (isCheck) {
          expect(
            args.start.toString() ===
              'Thu May 03 2018 04:00:00 GMT+0530 (India Standard Time)' ||
              args.start.toString() ===
                'Thu May 03 2018 04:00:00 GMT+0000 (Coordinated Universal Time)'
          ).toBe(true);
    ...
    it('checing date range changed', (done: Function) => {
      range.loaded = (args: Object) => {
        isCheck = true;
        range.periodSelectorModule.datePicker.startDate = new Date(
          2018,
          4,
          3,
          4
    ...
    it('checking with click with custom range', (done: Function) => {
      range.loaded = (args: Object) => {
        ...
        trigger.clickEvent(list);
        list = document
          .getElementsByClassName('e-footer')[0]
          .getElementsByClassName('e-apply')[0];
        trigger.clickEvent(list);
        done();
      };
      range.changed = (args: IChangedEventArgs) => {
