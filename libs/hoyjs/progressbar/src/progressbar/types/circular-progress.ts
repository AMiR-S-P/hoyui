...
export class Circular {
  ...
  public renderCircularTrack(): void {
    ...
    const circularTrackGroup: Element = progress.renderer.createGroup({
    ...
    progress.totalAngle =
      progress.totalAngle <= 0
        ? 360 + progress.totalAngle
        : progress.totalAngle;
    progress.totalAngle -= progress.totalAngle === 360 ? 0.01 : 0;
    this.trackEndAngle = endAngle =
      (progress.startAngle +
        (progress.enableRtl ? -progress.totalAngle : +progress.totalAngle)) %
      360;
    ...
    this.maxThickness =
      Math.max(progress.trackThickness, progress.progressThickness) ||
      Math.max(
        progress.themeStyle.circularProgressThickness,
        progress.themeStyle.circularTrackThickness
      );
    this.availableSize =
      Math.min(progress.progressRect.height, progress.progressRect.width) / 2 -
      this.maxThickness / 2;
    radius = stringToNumber(progress.radius, this.availableSize);
    radius = radius === null ? 0 : radius;
    const stroke: string =
      progress.argsData.trackColor || progress.themeStyle.circularTrackColor;
    const fill: string = progress.enablePieProgress
      ? progress.argsData.trackColor || progress.themeStyle.circularTrackColor
      : 'none';
    const strokeWidth: number = progress.enablePieProgress
      ? 0
    ...
    const option: PathOption = new PathOption(
      progress.element.id + '_Circulartrack',
      fill,
      strokeWidth,
      stroke,
  ...
  public renderCircularProgress(
    ...
    if (!refresh) {
      ...
      circularProgressGroup = getElement(
        progress.element.id + '_CircularProgressGroup'
      );
    }
    radius = stringToNumber(progress.innerRadius, this.availableSize);
    radius = radius === null ? 0 : radius;
    progress.previousTotalEnd = progressEnd = progress.calculateProgressRange(
      progress.argsData.value > progress.maximum
        ? progress.maximum
        : progress.argsData.value
    );
    const progressEndAngle: number =
      (progress.startAngle +
        (progress.enableRtl ? -progressEnd : progressEnd)) %
      360;
    progress.previousEndAngle = endAngle =
    ...
    progressTotalAngle =
      progressTotalAngle <= 0 ? 360 + progressTotalAngle : progressTotalAngle;
    ...
    const option: PathOption = new PathOption(
      progress.element.id + '_Circularprogress',
      fill,
      strokeWidth,
      stroke,
    ...
    this.endPosition = degreeToLocation(
      this.centerX,
      this.centerY,
      radius,
      endAngle
    ...
    if (progress.argsData.value !== null) {
      if (
        ...
        circularProgress = this.segment.createCircularRange(
          this.centerX,
          this.centerY,
          radius,
          progress
      ...
      if (
        ...
        this.delay =
          progress.secondaryProgress !== null ? 300 : progress.animation.delay;
        linearClipPath = progress.createClipPath(
          progress.clipPath,
          null,
        ...
        if (
          ...
          (<SVGPathElement>circularProgress).style.clipPath =
          ...
          this.animation.doCircularAnimation(
            ...
            thickness,
            this.delay,
            refresh ? previousEnd : null,
            refresh ? previousTotalEnd : null
          );
        }
        if (progress.isIndeterminate) {
          if (progress.enableProgressSegments) {
            linearClipPath.setAttribute(
              'd',
              getPathArc(
                this.centerX,
                this.centerY,
  ...
  private renderCircularBuffer(
    ...
    radius: number,
    progressTotalAngle: number
  ): void {
    let bufferClipPath: Element;
    let circularBuffer: Element;
    let segmentWidth: number;
    let totalAngle: number;
    const circularBufferGroup: Element = progress.renderer.createGroup({
      id: progress.element.id + '_ CircularBufferGroup',
    });
    const bufferEnd: number = progress.calculateProgressRange(
      progress.secondaryProgress > progress.maximum
        ? progress.maximum
        : progress.secondaryProgress
    );
    const endAngle: number =
      (progress.startAngle + (progress.enableRtl ? -bufferEnd : bufferEnd)) %
      360;
    const circularPath: string = getPathArc(
      this.centerX,
      this.centerY,
    ...
    this.bufferEndPosition = degreeToLocation(
      this.centerX,
      this.centerY,
      radius,
      endAngle
    ...
    const strokeWidth: number = progress.enablePieProgress
      ...
      : progress.progressThickness ||
        progress.themeStyle.circularProgressThickness;
    const option: PathOption = new PathOption(
      progress.element.id + '_Circularbuffer',
      fill,
      strokeWidth,
      stroke,
    ...
    if (
      ...
      bufferClipPath = progress.createClipPath(
  ...
  public renderCircularLabel(isProgressRefresh: boolean = false): void {
    ...
    const circularLabelGroup: Element = progress.renderer.createGroup({
    ...
    const argsData: ITextRenderEventArgs = {
      ...
      color:
        ...
        progress.themeStyle.circularLabelFont.color,
    };
    ...
    if (!argsData.cancel) {
      textSize = measureText(
        argsData.text,
        progress.labelStyle,
        progress.themeStyle.circularLabelFont
      );
      centerY = this.centerY + textSize.height / 2;
      option = new TextOption(
        progress.element.id + '_circularLabel',
        progress.labelStyle.size || progress.themeStyle.circularLabelFont.size,
        progress.labelStyle.fontStyle ||
          progress.themeStyle.circularLabelFont.fontStyle,
        progress.labelStyle.fontFamily ||
          progress.themeStyle.circularLabelFont.fontFamily,
      ...
      if (
        ...
        this.animation.doLabelAnimation(
          circularLabel,
          isProgressRefresh ? progress.previousWidth : progress.startAngle,
          end,
          progress,
          this.delay
        );
  ...
  private renderActiveState(
    ...
    if (!refresh) {
      option = new PathOption(
        progress.element.id + '_CircularActiveProgress',
        'none',
        thickness,
        '#ffffff',
        0.5,
  ...
  private validateSegmentSize(
    ...
    const rDiff: number =
      parseInt(progress.radius, 10) - parseInt(progress.innerRadius, 10);
    if (rDiff !== 0 && !progress.enableProgressSegments) {
      progressSegment =
        progress.trackWidth +
        (rDiff < 0
          ? (progress.trackWidth * Math.abs(rDiff)) /
            parseInt(progress.radius, 10)
          : -(progress.trackWidth * Math.abs(rDiff)) /
            parseInt(progress.radius, 10));
      validSegment = progress.calculateSegmentSize(progressSegment, thickness);
    } else if (progress.enableProgressSegments) {
      validSegment = progress.calculateSegmentSize(
        progress.progressWidth,
        thickness
      );
    } else {
