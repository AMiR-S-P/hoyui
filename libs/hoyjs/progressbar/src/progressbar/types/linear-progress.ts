...
import { PathOption, getElement, Size, measureText } from '@hoyui/svg-base';
import {
  ITextRenderEventArgs,
  LinearGradient,
  StopElement,
} from '../model/progress-interface';
...
import { animationMode } from '@hoyui/base';
...
export class Linear {
  ...
  public renderLinearTrack(): void {
    ...
    const linearTrackGroup: Element = progress.renderer.createGroup({
      id: progress.element.id + '_LinearTrackGroup',
    });
    let linearTrack: Element;
    let option: PathOption;
    this.isRange =
      this.progress.rangeColors[0].color !== '' ||
      this.progress.rangeColors[0].start !== null ||
      this.progress.rangeColors[0].end !== null;
    const thickness: number =
      progress.trackThickness || progress.themeStyle.linearTrackThickness;
    const stroke: string =
      progress.argsData.trackColor || progress.themeStyle.linearTrackColor;
    if (progress.cornerRadius === 'Round4px') {
      if (progress.segmentCount > 1) {
        linearTrack = this.createRoundCornerSegment(
          '_LinearTrack_',
          stroke,
          thickness,
          true,
        ...
        option = new PathOption(
          progress.element.id + '_Lineartrack',
          stroke,
          0,
          'none',
          progress.themeStyle.trackOpacity,
          '0',
          this.cornerRadius(
            progress.progressRect.x,
            progress.progressRect.y,
      ...
      if (
        ...
        !this.isRange &&
        !progress.enableProgressSegments
      ) {
  ...
  public renderLinearProgress(
    ...
    const previousProgressWidth: number =
      progress.progressRect.width *
      progress.calculateProgressRange(
        progress.value > progress.maximum ? progress.maximum : progress.value
      );
    const progressWidth: number = progress.calculateProgressRange(
      progress.argsData.value > progress.maximum
        ? progress.maximum
        : progress.argsData.value
    );
    this.linearProgressWidth =
      progress.progressRect.width *
      (progress.isIndeterminate && !progress.enableProgressSegments
        ? 1
        : progressWidth);
    if (!refresh) {
      ...
      linearProgressGroup = getElement(
        progress.element.id + '_LinearProgressGroup'
      );
    }
    const thickness: number =
      progress.progressThickness || progress.themeStyle.linearProgressThickness;
    const stroke: string = !progress.isStriped
      ? this.checkingLinearProgressColor()
      : 'url(#' + progress.element.id + '_LinearStriped)';
    if (progress.cornerRadius === 'Round4px') {
      option = new PathOption(
        progress.element.id + '_Linearprogress',
        stroke,
        0,
        'none',
        progress.themeStyle.progressOpacity,
        '0',
        this.cornerRadius(
          progress.progressRect.x,
          progress.progressRect.y,
    ...
    progress.progressWidth = (<SVGPathElement>(
      progress.renderer.drawPath(option)
    )).getTotalLength();
    progress.segmentSize = !progress.enableProgressSegments
    ...
    if (progress.argsData.value !== null) {
      if (progress.cornerRadius === 'Round4px') {
        ...
        if (
          progress.segmentColor.length !== 0 &&
          !progress.isIndeterminate &&
          !this.isRange
        ) {
          segmentWidth = !progress.enableProgressSegments
            ? progress.trackWidth
            : progress.progressWidth;
          linearProgress = this.segment.createLinearSegment(
            progress,
            '_LinearProgressSegment',
            this.linearProgressWidth,
            progress.themeStyle.progressOpacity,
            thickness,
            segmentWidth
          ...
          linearProgress = this.segment.createLinearRange(
            this.linearProgressWidth,
            progress,
            progressWidth
          );
      ...
      if (
        ((progress.animation.enable && animationMode !== 'Disable') ||
        ...
        if (progress.secondaryProgress !== null) {
          animationdelay =
            ...
            (this.bufferWidth - this.linearProgressWidth);
        } else {
        ...
        (<SVGPathElement>linearProgress).style.clipPath =
          'url(#' + progress.element.id + '_clippath)';
        this.animation.doLinearAnimation(
          clipPathLinear,
          progress,
          animationdelay,
          refresh ? previousWidth : 0
        );
      }
      if (progress.isIndeterminate) {
        clipPathIndeterminate = progress.createClipPath(
          progress.clipPath,
          progress.enableProgressSegments ? 1 : progressWidth,
          null,
          refresh,
          thickness,
          progress.enableProgressSegments
        ...
        linearProgress.setAttribute(
          'style',
          'clip-path:url(#' + progress.element.id + '_clippath)'
        );
        this.animation.doLinearIndeterminate(
          !progress.enableProgressSegments
            ? clipPathIndeterminate
            : linearProgress,
          this.linearProgressWidth,
          thickness,
          progress,
  ...
  private renderLinearBuffer(progress: ProgressBar): void {
    ...
    const secondaryProgressWidth: number = progress.calculateProgressRange(
      progress.secondaryProgress > progress.maximum
        ? progress.maximum
        : progress.secondaryProgress
    );
    this.bufferWidth = linearBufferWidth =
      progress.progressRect.width * secondaryProgressWidth;
    const linearBufferGroup: Element = progress.renderer.createGroup({
      id: progress.element.id + '_LinearBufferGroup',
    });
    const thickness: number = progress.secondaryProgressThickness
      ? progress.secondaryProgressThickness
      : progress.progressThickness ||
        progress.themeStyle.linearProgressThickness;
    const stroke: string = progress.secondaryProgressColor
      ? progress.secondaryProgressColor
      : this.checkingLinearProgressColor();
    if (progress.cornerRadius === 'Round4px') {
      if (progress.segmentCount > 1) {
        linearBuffer = this.createRoundCornerSegment(
          ...
          linearBufferWidth,
          progress,
          progress.themeStyle.bufferOpacity
        );
      } else {
        option = new PathOption(
          progress.element.id + '_Linearbuffer',
          stroke,
          0,
          'none',
          progress.themeStyle.bufferOpacity,
    ...
    if (
      ...
      linearBuffer.setAttribute(
        'style',
        'clip-path:url(#' + progress.element.id + '_clippathBuffer)'
      );
      this.animation.doLinearAnimation(
        clipPathBuffer,
        progress,
  ...
  public renderLinearLabel(isProgressRefresh: boolean = false): void {
    ...
    const thickness: number =
      this.progress.progressThickness ||
      this.progress.themeStyle.linearProgressThickness;
    ...
    const linearLabelGroup: Element = progress.renderer.createGroup({
    ...
    const labelValue: number =
      ...
      percentage;
    const linearValue: number =
      progress.value < progress.minimum ? 0 : Math.round(labelValue);
    const argsData: ITextRenderEventArgs = {
      cancel: false,
      text: labelText ? labelText : String(linearValue) + '%',
      color:
        progress.labelStyle.color ||
        this.progress.themeStyle.linearLabelFont.color,
    };
    ...
    if (!argsData.cancel) {
      textSize = measureText(
        argsData.text,
        progress.labelStyle,
        progress.themeStyle.linearLabelFont
      );
      defaultPos = progress.enableRtl
        ? progress.progressRect.x +
          progress.progressRect.width -
          textSize.width / 2
      ...
      if (progress.labelOnTrack) {
        if (textAlignment === 'Near') {
          ...
          center = progress.enableRtl
            ? progress.progressRect.x +
              progress.progressRect.width -
              progressWidth / 2
            : progress.progressRect.x + progressWidth / 2;
          pos = progress.enableRtl
            ? center <= defaultPos
            : center >= defaultPos;
      ...
      if (this.progress.cornerRadius === 'Round4px') {
        ...
        posY =
          progress.progressRect.y +
          progress.progressRect.height / 2 +
          textSize.height / 4;
      }
      option = new TextOption(
        progress.element.id + '_linearLabel',
        progress.labelStyle.size || progress.themeStyle.linearLabelFont.size,
        progress.labelStyle.fontStyle ||
          progress.themeStyle.linearLabelFont.fontStyle,
        progress.labelStyle.fontFamily ||
          progress.themeStyle.linearLabelFont.fontFamily,
        progress.labelStyle.fontWeight,
        'middle',
        argsData.color,
        posX,
        posY
      ...
      if (
        ...
        (<SVGPathElement>linearlabel).style.clipPath =
          'url(#' + progress.element.id + '_clippathLabel)';
        this.animation.doLabelAnimation(
          linearlabel,
          isProgressRefresh ? progress.previousLabelWidth : 0,
  ...
  private renderActiveState(
    ...
    if (progress.cornerRadius === 'Round4px') {
      if (progress.segmentCount > 1) {
        ...
        option = new PathOption(
          progress.element.id + '_LinearActiveProgress',
          '#ffffff',
          0,
          'none',
          0.5,
      ...
      if (!refresh) {
        ...
        linearActive = getElement(
          progress.element.id + '_LinearActiveProgress'
        );
        linearActive.setAttribute(
          'd',
          progress.getPathLine(
            progress.progressRect.x,
            linearProgressWidth,
    ...
    const activeClip: Element = progress.createClipPath(
      progress.clipPath,
      progressWidth,
      null,
      refresh,
  ...
  private renderLinearStriped(
    ...
    let linearGradient: Element = document.createElementNS(
      svgLink,
      gradientType
    );
    ...
    stopOption = [
      { offset: '50%', 'stop-color': color, 'stop-opacity': '1' },
  ...
  private checkingLinearProgressColor(): string {
    ...
    switch (role) {
      ...
      default:
        linearColor =
          progress.argsData.progressColor ||
  ...
  public createRoundCornerSegment(
    ...
    const gapWidth: number =
      progress.gapWidth || progress.themeStyle.linearGapWidth;
    const segWidth: number =
      (width - (progress.segmentCount - 1) * gapWidth) / progress.segmentCount;
    const segmentGroup: Element = progress.renderer.createGroup({
    ...
    for (let i: number = 1; i <= progress.segmentCount; i++) {
      ...
      if (isTrack) {
        option = new PathOption(
          progress.element.id + id + i,
          stroke,
          0,
          'none',
          progress.themeStyle.trackOpacity,
          '0',
          this.cornerRadius(locX, locY, segWidth, thickness, 4, pathType)
        );
        ...
        option = new PathOption(
          ...
          opacity,
          '0',
          this.cornerRadius(locX, locY, avlWidth, thickness, 4, pathType)
        );
        segmentPath = progress.renderer.drawPath(option);
        segmentGroup.appendChild(segmentPath);
        locX += segWidth + gapWidth;
