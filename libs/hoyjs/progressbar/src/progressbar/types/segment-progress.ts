...
import { PathOption, LinearGradient, GradientColor } from '@hoyui/svg-base';
...
export class Segment {
  ...
  public createLinearSegment(
    ...
    let locX: number = progress.enableRtl
      ? progress.cornerRadius === 'Round'
        ? progress.progressRect.x +
          progress.progressRect.width -
          (lineCapRadius / 2) * thickness
        : progress.progressRect.x + progress.progressRect.width
      : progress.cornerRadius === 'Round'
      ? progress.progressRect.x + (lineCapRadius / 2) * thickness
      : progress.progressRect.x;
    const locY: number =
      progress.progressRect.y + progress.progressRect.height / 2;
    ...
    let avlSegWidth: number =
      progressWidth - (progress.segmentCount - 1) * gapWidth;
    avlSegWidth =
      (avlSegWidth -
        (progress.cornerRadius === 'Round'
          ? progress.segmentCount * (lineCapRadius * thickness)
          : 0)) /
      progress.segmentCount;
    const gap: number =
      progress.cornerRadius === 'Round'
        ? gapWidth + lineCapRadius * thickness
        : gapWidth;
    ...
    let tolWidth: number =
      progress.cornerRadius === 'Round'
        ? width - lineCapRadius * thickness
        : width;
    const linearThickness: number =
    ...
    for (let i: number = 0; i < count; i++) {
      ...
      option = new PathOption(
        ...
        opacity,
        '0',
        this.getLinearSegmentPath(locX, locY, segWidth, progress.enableRtl)
      );
  ...
  public createCircularSegment(
    ...
    let end: number = this.widthToAngle(
      progress.minimum,
      progress.maximum,
      value,
      progress.totalAngle
      ...
      progress.cornerRadius === 'Round' && progress.totalAngle === completeAngle
        ? this.widthToAngle(
            0,
            progressWidth,
            (lineCapRadius / 2) * thickness,
    ...
    let size: number =
      ...
      (progress.totalAngle === completeAngle
        ...
        (progress.gapWidth || progress.themeStyle.circularGapWidth);
    size =
      (size -
        (progress.cornerRadius === 'Round'
          ? (progress.totalAngle === completeAngle
          ...
          : 0)) /
      progress.segmentCount;
    let avlTolEnd: number = this.widthToAngle(
      0,
      progressWidth,
      progressWidth / progress.segmentCount,
      totalAngle
    );
    avlTolEnd -=
      progress.cornerRadius === 'Round' && progress.totalAngle === completeAngle
        ...
        : 0;
    const avlEnd: number = this.widthToAngle(
      0,
      progressWidth,
      size,
      totalAngle
    ...
    let avlSegEnd: number =
      (start + (progress.enableRtl ? -avlEnd : avlEnd)) % 360;
    let color: string;
    let j: number = 0;
    let option: PathOption;
    const circularThickness: number =
      progress.progressThickness ||
      progress.themeStyle.circularProgressThickness;
    for (let i: number = 0; i < count; i++) {
      segmentEnd = progress.enableRtl
        ? progress.startAngle - end > avlSegEnd
          ? progress.startAngle - end
          : avlSegEnd
        : progress.startAngle + end < avlSegEnd
        ? progress.startAngle + end
        : avlSegEnd;
  ...
  public createLinearRange(
    ...
    const posX: number =
      progress.progressRect.x +
      (progress.enableRtl ? progress.progressRect.width : 0);
    const startY: number =
      progress.progressRect.y + progress.progressRect.height / 2;
    const rangeGroup: Element = progress.renderer.createGroup({
    ...
    for (let i: number = 0; i < range.length; i++) {
      validRange =
        range[i as number].start >= rangeMin &&
        range[i as number].start <= rangeMax &&
        range[i as number].end >= rangeMin &&
        range[i as number].end <= rangeMax;
      startPos =
        totalWidth *
        progress.calculateProgressRange(
          range[i as number].start,
          rangeMin,
          rangeMax
        );
      endPos =
      ...
      startX = validRange
        ? progress.isGradient && i > 0
          ? startX + gradX
          : startX
        : posX;
      endX = validRange ? endX : posX;
      color = progress.isGradient
        ? 'url(#lineRangeGrad_' + i + ')'
        : range[i as number].color;
      option = new PathOption(
        progress.element.id + '_LinearRange_' + i,
        'none',
        thickness,
        color,
        opacity,
      ...
      if (progress.isGradient) {
        ...
        gradient = this.setLinearGradientColor(
          i,
          range[i as number].color,
          endColor,
          startX,
  ...
  public createCircularRange(
    ...
    const tolAngle: number = this.widthToAngle(
      progress.minimum,
      progress.maximum,
      progress.value,
      progress.totalAngle
    ...
    for (let i: number = 0; i < range.length; i++) {
      isValidRange =
        ...
        range[i as number].end >= rangeMin &&
        range[i as number].end <= rangeMax;
      startAngle = this.widthToAngle(
        rangeMin,
        rangeMax,
        range[i as number].start,
        tolAngle
      );
      endAngle = this.widthToAngle(
        rangeMin,
        rangeMax,
        range[i as number].end,
        tolAngle
      );
      startAngle = isValidRange
        ? (start + (progress.enableRtl ? -startAngle : startAngle)) % 360
        : start;
      endAngle = isValidRange
        ? (start + (progress.enableRtl ? -endAngle : endAngle)) % 360
        : start;
      color = progress.isGradient
        ? 'url(#circleRangeGrad_' + i + ')'
        : range[i as number].color;
      option = new PathOption(
        progress.element.id + '_CircularRange_' + i,
        'none',
        thickness,
        color,
        opacity,
  ...
  private setLinearGradientColor(
    ...
    const linearGradient: Element = progress.renderer.drawGradient(
      'linearGradient',
      option,
      stopColor
    );
    linearGradient.firstElementChild.setAttribute(
      'gradientUnits',
      'userSpaceOnUse'
    );
