...
} from './model/progress-base-model';
import {
  Margin,
  Animation,
  Font,
...
import { SvgRenderer, PathOption, getElement } from '@hoyui/svg-base';
import {
  ProgressType,
  CornerType,
  ProgressTheme,
  ...
  implements INotifyPropertyChanged
{
  ...
  protected render(): void {
    ...
    this.element.setAttribute(
      'aria-label',
      this.labelStyle.text || 'progress bar'
    );
  ...
  private calculateProgressBarSize(): void {
    const containerWidth: number =
      this.element.clientWidth || this.element.offsetWidth;
    ...
    height =
      this.type === 'Linear' && thickness > height - padding
        ? thickness + padding
        : height;
    this.progressSize.width =
      stringToNumber(this.width, containerWidth) || containerWidth || width;
    this.progressSize.height =
      stringToNumber(this.height, containerHeight) || containerHeight || height;
    this.progressRect.x = this.margin.left;
    this.progressRect.y = this.margin.top;
    this.progressRect.width =
      this.progressSize.width - (this.margin.left + this.margin.right);
    this.progressRect.height =
      this.progressSize.height - (this.margin.top + this.margin.bottom);
  ...
  private createSecondaryElement(): void {
    const secElement: Element = document.getElementById(
      this.element.id + 'Secondary_Element'
    );
    if (this.tooltip.enable) {
      this.tooltipElement = createElement('div', {
        ...
        styles:
          'pointer-events: none; position: absolute; zIndex: 1; visibility: visible',
      });
      if (secElement) {
        this.secElement.appendChild(this.tooltipElement);
      }
    }
    const tooltipElement: Element = document.getElementById(
      this.element.id + '_tooltip'
    );
    if (secElement) {
      secElement.innerHTML = '';
      this.secElement = tooltipElement
        ? (secElement.appendChild(tooltipElement) as HTMLElement)
        : (secElement as HTMLElement);
      return;
    }
  ...
  public getPathLine(x: number, width: number, thickness: number): string {
    const moveTo: number = this.enableRtl
      ? this.cornerRadius === 'Round'
        ? x + this.progressRect.width - (lineCapRadius / 2) * thickness
        : x + this.progressRect.width
      : this.cornerRadius === 'Round'
      ? x + (lineCapRadius / 2) * thickness
      : x;
    //TODO : BLAZ-14309 - ProgressBar renders improperly when corner radius is set to "Round" and the value between one to four.
    thickness =
      width < thickness && this.cornerRadius === 'Round' ? width : thickness;
    const lineTo: number = this.enableRtl
      ? this.cornerRadius === 'Round' && width
        ? moveTo - width + lineCapRadius * thickness
        : moveTo - width
      : this.cornerRadius === 'Round' && width
      ? moveTo + width - lineCapRadius * thickness
      : moveTo + width;
    return (
      ...
      moveTo +
      ' ' +
      (this.progressRect.y + this.progressRect.height / 2) +
      'L' +
      lineTo +
      ' ' +
      (this.progressRect.y + this.progressRect.height / 2)
  ...
  public calculateProgressRange(
    ...
    const endValue: number =
      ((value - min) / (max - min)) *
      (this.type === 'Linear' ? 1 : this.totalAngle);
  ...
  public calculateSegmentSize(width: number, thickness: number): string {
    const count: number =
      this.type === 'Circular' && this.totalAngle === completeAngle
        ? this.segmentCount
        : this.segmentCount - 1;
    const cornerCount: number =
      this.totalAngle === completeAngle || this.type === 'Linear'
        ? this.segmentCount
        : this.segmentCount - 1;
    let gap: number =
      this.gapWidth ||
      (this.type === 'Linear'
        ? this.themeStyle.linearGapWidth
        : this.themeStyle.circularGapWidth);
    let size: number = width - count * gap;
    size =
      (size -
        (this.cornerRadius === 'Round'
          ? cornerCount * (lineCapRadius * thickness)
          : 0)) /
      this.segmentCount;
  ...
  public createClipPath(
    ...
    if (this.type === 'Linear') {
      if (this.cornerRadius === 'Round4px') {
        ...
        //TODO : BLAZ-14309 - ProgressBar renders improperly when corner radius is set to "Round" and the value between one to four.
        posx =
          this.enableRtl && !isLabel
            ? x +
              totalWidth +
              (this.cornerRadius === 'Round' ? thickness / 10 : 0)
            : x - (this.cornerRadius === 'Round' ? thickness / 10 : 0);
        pathWidth = totalWidth * range;
        //TODO : BLAZ-14309 - ProgressBar renders improperly when corner radius is set to "Round" and the value between one to four.
        //posx += (this.cornerRadius === 'Round' && !isLabel) ?
        //    ((this.enableRtl) ? (lineCapRadius / 2) * thickness : -(lineCapRadius / 2) * thickness) : 0;
        posy =
          this.progressRect.y + this.progressRect.height / 2 - thickness / 2;
        pathWidth +=
          this.cornerRadius === 'Round' && !isLabel
            ? lineCapRadius * thickness
            : 0;
      }
      if (!refresh) {
        rect = new RectOption(
          this.element.id + '_clippathrect' + (isLabel ? 'label' : ''),
          'transparent',
          1,
          'transparent',
          1,
      ...
      if (!refresh) {
        option = new PathOption(
          this.element.id + '_clippathcircle',
          'transparent',
          10,
          'transparent',
          1,
  ...
  private setTheme(): void {
    ...
    switch (this.theme) {
      ...
      case 'Bootstrap5Dark':
        this.cornerRadius =
          this.cornerRadius === 'Auto' && this.type === 'Linear'
            ? 'Round'
            : this.cornerRadius;
        break;
      default:
        this.cornerRadius =
          this.cornerRadius === 'Auto' ? 'Square' : this.cornerRadius;
    }
  }
  ...
  private progressMouseMove(e: PointerEvent): void {
    ...
    if (this.tooltip.enable && this.tooltip.showTooltipOnHover) {
      if (
        target.id.indexOf('Circularprogress') >= 0 ||
        target.id.indexOf('Circularbuffer') >= 0 ||
        target.id.indexOf('Linearprogress') >= 0 ||
        target.id.indexOf('Linearbuffer') >= 0 ||
        target.id.indexOf('Linearbuffer') >= 0
      ) {
  ...
  public onPropertyChanged(
    newProp: ProgressBarModel,
    oldProp: ProgressBarModel
  ): void {
      ...
      switch (prop) {
        ...
        case 'value':
          ...
          if (this.type === 'Circular') {
            this.circular.renderCircularProgress(
              this.previousEndAngle,
              this.previousTotalEnd,
            ...
            if (
              this.progressAnnotationModule &&
              this.animation.enable &&
              !this.isIndeterminate
            ) {
              this.annotateAnimation.doAnnotationAnimation(
                this.clipPath,
                this,
                this.annotateEnd,
                this.annotateTotal
          ...
          this.element.setAttribute(
            'aria-valuenow',
            this.argsData.value ? this.argsData.value.toString() : '0'
          );
          break;
        case 'animation':
          this.createSVG();
  ...
  public requiredModules(): ModuleDeclaration[] {
    ...
    enableAnnotation = this.annotations.some(
      (value: ProgressAnnotationSettings) => {
