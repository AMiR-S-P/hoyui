...
export class ProgressAnimation {
  ...
  public doLinearAnimation(
    ...
    const duration: number = progress.isActive
      ? 3000
    ...
    let value: number = 0;
    const start: number =
      !progress.enableRtl || progress.cornerRadius === 'Round4px'
        ? previousWidth
        : parseInt(x, 10);
    const end: number =
      !progress.enableRtl || progress.cornerRadius === 'Round4px'
        ? parseInt(width, 10) - start
    ...
    animation.animate(linearPath, {
      duration:
        progress.animation.duration === 0 && animationMode === 'Enable'
          ? 2000
          : duration,
      delay: delay,
  ...
  public doLinearIndeterminate(
    ...
    const end: number =
      ...
      (width ? parseInt(width, 10) : progressWidth);
    const duration: number = !progress.enableProgressSegments
      ? progress.animation.duration
      : progress.animation.duration + 1000;
    animation.animate(<HTMLElement>clipPath, {
      ...
      progress: (args: AnimationOptions): void => {
        if (progress.enableRtl && !(progress.cornerRadius === 'Round4px')) {
          value = effect(
            args.timeStamp,
            parseInt(x, 10) || progress.progressRect.x + progressWidth,
            end,
            args.duration,
            true
          );
          if (!progress.enableProgressSegments) {
            ...
            linearPath.setAttribute(
              'd',
              progress.getPathLine(value, progressWidth, thickness)
            );
          }
        } else {
          value = effect(args.timeStamp, start, end, args.duration, false);
  ...
  public doCircularAnimation(
    ...
    if (
      ...
      totalEnd !== completeAngle &&
      totalEnd !== 0 &&
      progress.startAngle === progress.endAngle
    ) {
      const startPosition: number = degreeToLocation(x, y, pathRadius, start).x;
      let endPosition: number = degreeToLocation(
        x,
        y,
        pathRadius,
        progressEnd
      ...
      while (
        progress.enableRtl !== progress.startAngle >= 180
    ...
    const startPos: number = !isNullOrUndefined(startValue)
      ? startValue
      : start;
    const endPos: number = !isNullOrUndefined(startValue)
      ? totalEnd - previousTotal
      : totalEnd;
  ...
  public doCircularIndeterminate(
    ...
    const pathRadius: number =
      radius + (!progress.enableProgressSegments ? thickness / 2 : 0);
    const duration: number =
      progress.animation.duration === 0 && animationMode === 'Enable'
        ? 2000
        : progress.animation.duration;
    const value: number = !progress.enableProgressSegments
      ? 6000 / duration
      : 4000 / duration;
    animation.animate(<HTMLElement>clipPath, {
      progress: (): void => {
  ...
  public doLabelAnimation(
    ...
    if (progress.type === 'Linear') {
      startPos = progress.enableRtl
        ? progress.progressRect.x + progress.progressRect.width + textSize / 2
        : start - textSize / 2;
      ...
      progress: (args: AnimationOptions): void => {
        ...
        if (progress.type === 'Linear') {
          if (args.timeStamp >= args.delay) {
            if (labelText === '') {
              ...
              valueChanged = parseInt(
                (
                  (Math.round(value) / progress.progressRect.width) *
                  percentage
                ).toString(),
                10
              );
          ...
          if (labelText === '') {
            ...
            valueChanged = parseInt(
              (
                (Math.round(value) / progress.totalAngle) *
                percentage
              ).toString(),
              10
            );
  ...
  public doAnnotationAnimation(
    ...
    if (isAnnotation && progress.progressAnnotationModule) {
      firstAnnotatElement = document.getElementById(
        progress.element.id + 'Annotation0'
      ).children[0];
    ...
    totalEnd =
      ((progress.argsData.value - progress.minimum) /
    ...
    const startValue: number = !isNullOrUndefined(previousEnd)
      ? previousEnd
      : start;
    const endValue: number = !isNullOrUndefined(previousEnd)
      ? totalEnd - previousTotal
      : totalEnd;
      ...
      animation.animate(<HTMLElement>circularPath, {
        ...
        progress: (args: AnimationOptions): void => {
          ...
          if (isAnnotation && annotatElementChanged) {
            ...
            annotateValueChanged = parseInt(
              (
                ((Math.round(value) - start) / totalAngle) *
                percentage
              ).toString(),
              10
            );
            annotatElementChanged.innerHTML = annotateValueChanged
              ? annotateValueChanged.toString() + '%'
              : '0%';
          }
        },
        end: () => {
