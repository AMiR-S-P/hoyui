import { Tooltip as SVGTooltip } from '@hoyui/svg-base';
...
export class ProgressTooltip {
  ...
  public tooltip(e?: PointerEvent | TouchEvent): void {
    ...
    if (
      ...
      this.control.type === 'Circular' &&
      this.control.animation.enable &&
      !this.control.tooltip.showTooltipOnHover
    ) {
      svgElement.style.visibility = 'hidden';
      const delay: number = this.control.secondaryProgress
        ? this.control.circular.delay + this.control.animation.duration
    ...
    if (
      ...
      const delay: number = this.control.secondaryProgress
        ? this.control.linear.delay
        : this.control.animation.delay;
  ...
  private toolTipAnimation(
    ...
    const tooltipSVG: HTMLElement = document.getElementById(
      this.control.element.id + '_tooltip_svg'
    );
  ...
  private renderTooltip(
    ...
    this.textFormat = this.format(
      this.control.tooltip.showTooltipOnHover
        ? (e.target as Element).id.indexOf('Linearbuffer') >= 0 ||
          (e.target as Element).id.indexOf('Circularbuffer') >= 0
          ? this.control.secondaryProgress
          : this.control.value
        : this.control.value
    );
  ...
  public removeTooltip(duration: number): void {
    ...
    if (tooltipElement) {
      this.fadeInInterval = +setTimeout((): void => {
        if (this.svgTooltip) {
  ...
  private triggerTooltipRender(
    ...
    if (!argsData.cancel) {
      ...
      if (this.control.type === 'Linear') {
        const linearEndPointX: number =
          this.control.linear.linearProgressWidth -
          padding / 2 +
          this.control.progressRect.x;
        const linearEndPointY: number =
          this.control.cornerRadius === 'Round4px'
            ? this.control.progressRect.y + padding
            : this.control.progressRect.y +
              this.control.progressRect.height / 2 -
              (this.control.progressThickness
                ? this.control.progressThickness
                : this.control.themeStyle.linearProgressThickness) /
                2 +
              padding;
        this.createTooltip(
          this.control,
          ...
          (this.control.tooltip.enable &&
            !this.control.tooltip.showTooltipOnHover) ||
            !((e.target as Element).id.indexOf('Linearbuffer') >= 0)
            ? new ProgressLocation(
                this.control.cornerRadius === 'Round4px'
                  ? linearEndPointX - padding
                  : linearEndPointX,
                linearEndPointY
              )
            : new ProgressLocation(
                this.control.linear.bufferWidth -
                  padding / 2 +
                  this.control.progressRect.x,
                linearEndPointY
              ),
        ...
        const circularEndPointX: number =
          this.control.circular.endPosition.x - padding / 2;
        const circularEndPointY: number =
          this.control.circular.endPosition.y +
          this.control.progressRect.y -
          padding / 2;
        this.createTooltip(
          this.control,
          ...
          (this.control.tooltip.enable &&
            !this.control.tooltip.showTooltipOnHover) ||
            !((e.target as Element).id.indexOf('Circularbuffer') >= 0)
            ? new ProgressLocation(circularEndPointX, circularEndPointY)
            : new ProgressLocation(
                this.control.circular.bufferEndPosition.x - padding / 2,
                this.control.circular.bufferEndPosition.y +
                  this.control.progressRect.y -
                  padding / 2
              ),
          this.control.initialClipRect
  ...
  public createTooltip(
    ...
    bounds: ProgressLocation
  ): void {
    if (isFirst) {
      this.svgTooltip = new SVGTooltip(
        {
          opacity: this.control.tooltip.textStyle.opacity
            ? this.control.tooltip.textStyle.opacity
