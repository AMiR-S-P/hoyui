...
export class AnnotationBase {
  ...
  public render(
    annotation: ProgressAnnotationSettings,
    index: number
  ): HTMLElement {
  ...
  public processAnnotation(
    ...
    if (annotationElement) {
      ...
      if ((this.control as any).isReact) {
        (this.control as any).clearTemplate();
      }
    }
  }
  public setElementStyle(
    ...
    if (!argsData.cancel) {
      const result: { top: number; left: number } = this.Location(
        this.annotation.annotationRadius,
        this.annotation.annotationAngle
      );
