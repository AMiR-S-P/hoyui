...
import {
  ...
  Menu,
  MenuModel,
  BeforeOpenCloseMenuEventArgs as Menuopen,
  MenuEventArgs,
...
import {
  ...
  ItemModel,
  DropDownButtonModel,
} from '@hoyui/splitbuttons';
import { ColorPicker, Slider, ChangeEventArgs } from '@hoyui/inputs';
import {
  PdfAnnotationBaseModel,
  PdfFontModel,
} from '../drawing/pdf-annotation-model';
...
export class AnnotationToolbar {
  ...
  public createMobileAnnotationToolbar(
    isEnable: boolean,
    isPath?: boolean
  ): void {
    if (Browser.isDevice && !this.pdfViewer.enableDesktopMode) {
      if (this.toolbarElement == null && isEnable) {
        ...
        this.pdfViewerBase.viewerMainContainer.insertBefore(
          this.toolbarElement,
        ...
        this.deleteItem.firstElementChild.id =
          this.pdfViewer.element.id + '_annotation_delete';
      ...
      if (
        ...
        if (
          ...
          if (!this.toolbarCreated) {
            const editIcon: HTMLElement = document.getElementById(
              this.pdfViewer.element.id + '_annotationIcon'
            );
            if (
              editIcon &&
              editIcon.parentElement.classList.contains('e-pv-select')
            ) {
  ...
  private FreeTextForMobile(): void {
    ...
    this.pdfViewerBase.viewerMainContainer.appendChild(
      this.freetextToolbarElement
    );
    const colorTemplate: string =
      this.pdfViewer.toolbarModule.annotationToolbarModule.getTemplate(
        'span',
        '_annotation_color',
        'e-pv-annotation-color-container'
      );
    const strokeTemplate: string =
      this.pdfViewer.toolbarModule.annotationToolbarModule.getTemplate(
        'span',
        '_annotation_stroke',
        'e-pv-annotation-stroke-container'
      );
    const thicknessTemplate: string = this.getTemplate(
      'span',
      '_annotation_thickness',
      'e-pv-annotation-thickness-container'
    );
    const opacityTemplate: string = this.getTemplate(
      'span',
      '_annotation_opacity',
      'e-pv-annotation-opacity-container'
    );
    const fontFamilyTemplate: string = this.getTemplate(
      'input',
      '_annotation_fontname',
      'e-pv-annotation-fontname-container'
    );
    const fontSizeTemplate: string = this.getTemplate(
      'input',
      '_annotation_fontsize',
      'e-pv-annotation-fontsize-container'
    );
    const textColorTemplate: string = this.getTemplate(
      'span',
      '_annotation_textcolor',
      'e-pv-annotation-textcolor-container'
    );
    const alignmentTemplate: string = this.getTemplate(
      'span',
      '_annotation_textalign',
      'e-pv-annotation-textalign-container'
    );
    const textPropertiesTemplate: string = this.getTemplate(
      'span',
      '_annotation_textproperties',
    ...
    this.toolbar = new Tool({
      ...
      created: () => {
        this.pdfViewer.toolbarModule.annotationToolbarModule.mobileColorpicker(
          this.pdfViewer.element.id + '_annotation_freeTextEdit'
        );
      },
    });
  ...
  public createPropertyTools(shapeType: string): void {
    if (shapeType !== '') {
      ...
      shapeToolbarElement = document.getElementById(
        this.pdfViewer.element.id + '_propertyToolbar'
      );
      ...
      if (
        ...
        if (
          ...
          const commentIcon: HTMLElement = document.getElementById(
            this.pdfViewer.element.id + '_annotation_commentPanel'
          );
  ...
  private showPropertyTool(existingTool: Tool, id: string): void {
    ...
    if (this.pdfViewer.selectedItems.annotations[0]) {
      const selectedAnnotation: any =
        this.pdfViewer.selectedItems.annotations[0];
      if (
        selectedAnnotation.shapeAnnotationType !== 'SignatureText' &&
        selectedAnnotation.shapeAnnotationType !== 'HandWrittenSignature' &&
        selectedAnnotation.shapeAnnotationType !== 'Stamp' &&
        selectedAnnotation.shapeAnnotationType !== 'Image' &&
        selectedAnnotation.shapeAnnotationType !== 'Ink' &&
        selectedAnnotation.shapeAnnotationType !== 'Path' &&
        selectedAnnotation.shapeAnnotationType !== 'StickyNotes'
      ) {
        this.updateColorInIcon(
          this.colorDropDownElement,
          this.pdfViewer.selectedItems.annotations[0].fillColor
        );
        this.updateColorInIcon(
          this.strokeDropDownElement,
          this.pdfViewer.selectedItems.annotations[0].strokeColor
        );
        if (
          this.pdfViewer.selectedItems.annotations[0].shapeAnnotationType ===
          'FreeText'
          ...
          fontFamily.ej2_instances[0].value =
            this.pdfViewer.selectedItems.annotations[0].fontFamily;
          const fontColor: any = this.fontColorElement;
          fontColor.children[0].style.borderBottomColor =
            this.pdfViewer.selectedItems.annotations[0].fontColor;
          this.pdfViewer.annotation.modifyTextAlignment(
            this.pdfViewer.selectedItems.annotations[0].textAlign
          );
          this.updateTextAlignInIcon(
            this.pdfViewer.selectedItems.annotations[0].textAlign
          );
        }
      } else {
  ...
  private stampToolMobileForMobile(args: string): void {
    ...
    if (this.stampToolbarElement) {
      this.stampToolbarElement.parentElement.removeChild(
        this.stampToolbarElement
      );
    }
  ...
  private shapeToolMobile(args: ClickEventArgs): void {
    ...
    if (this.shapeToolbarElement) {
      this.shapeToolbarElement.parentElement.removeChild(
        this.shapeToolbarElement
      );
    }
    ...
    this.toolbar = new Tool({
      ...
      created: () => {
        this.pdfViewer.toolbarModule.annotationToolbarModule.mobileColorpicker(
          (args.originalEvent.target as HTMLElement).id
        );
      },
    });
    this.toolbar.isStringTemplate = true;
  ...
  private calibrateToolMobile(args: ClickEventArgs): void {
    ...
    if (this.calibrateToolbarElement) {
      this.calibrateToolbarElement.parentElement.removeChild(
        this.calibrateToolbarElement
      );
    }
  ...
  private textMarkupForMobile(args: ClickEventArgs): void {
    ...
    if (this.textMarkupToolbarElement) {
      this.textMarkupToolbarElement.parentElement.removeChild(
        this.textMarkupToolbarElement
      );
    }
  ...
  private showShapeTool(): void {
    const annotationToolbarItems: any =
  ...
  private signatureInkForMobile(): void {
    ...
    if (this.signatureInkToolbarElement) {
      this.signatureInkToolbarElement.parentElement.removeChild(
    ...
    this.toolbar = new Tool({
      ...
      created: () => {
        this.pdfViewer.toolbarModule.annotationToolbarModule.mobileColorpicker(
          this.pdfViewer.element.id + '_annotation_inkIcon'
        );
      },
    });
    this.toolbar.isStringTemplate = true;
  ...
  private hideExistingTool(): void {
    ...
    const mobileAnnotationToolbar: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_mobileAnnotationToolbar'
    );
  ...
  private showTextMarkupPropertiesTool(): void {
    ...
    if (annotationToolbarItems) {
      ...
      if (
        annotationToolbarItems.includes('HighlightTool') ||
        annotationToolbarItems.includes('UnderlineTool') ||
        annotationToolbarItems.includes('StrikethroughTool')
      ) {
  ...
  public createAnnotationToolbarForMobile(id?: string): any[] {
    ...
    if (hideToolbar) {
      ...
      this.pdfViewerBase.viewerMainContainer.appendChild(this.toolbarElement);
      const stampTemplate: string = this.getTemplate(
        'span',
        '_annotation_stamp',
        'e-pv-annotation-stamp-container'
      );
      const signTemplate: string = this.getTemplate(
        'span',
        '_annotation_signature',
        'e-pv-annotation-handwritten-container'
      );
      const items: any[] = [
        {
  ...
  public adjustMobileViewer(): void {
    ...
    if (
      this.toolbarElement &&
      this.toolbarElement.children.length === 0 &&
      this.propertyToolbar &&
      this.propertyToolbar.element.children.length > 0
    ) {
      toolbarHeight = this.propertyToolbar.element.clientHeight;
      if (
        this.pdfViewer.toolbarModule.toolbarElement.style.display === 'none' &&
        !(
          this.pdfViewerBase.navigationPane &&
      ...
      this.freetextToolbarElement &&
      this.freetextToolbarElement.children.length > 0
    ) {
    ...
    if (this.pdfViewer.enableToolbar && this.toolbarCreated) {
      this.pdfViewerBase.viewerContainer.style.height =
        this.updateViewerHeight(
          this.getElementHeight(this.pdfViewerBase.viewerContainer),
          toolbarHeight + toolbarHeight
        ) + 'px';
    } else {
      if (!isPrimaryTool) {
        if (
          this.pdfViewerBase.viewerContainer.style.height.split('%').length > 1
        ) {
          this.pdfViewerBase.viewerContainer.style.height =
            this.resetViewerHeight(
              this.getElementHeight(this.pdfViewerBase.viewerContainer),
              -toolbarHeight
            ) + 'px';
        } else {
  ...
  private createMobileToolbarItems(isPath: boolean): any[] {
    const colorTemplate: string = this.getTemplate(
      'span',
      '_annotation_color',
      'e-pv-annotation-color-container'
    );
  ...
  private goBackToToolbar(args: ClickEventArgs): void {
    ...
    if (Browser.isDevice || !this.pdfViewer.enableDesktopMode) {
      if (this.pdfViewerBase.action === 'Polygon') {
        (this.pdfViewerBase.tool as PolygonDrawingTool).mouseUp(
    ...
    if (page) {
      this.pdfViewer.annotationModule.textMarkupAnnotationModule.selectTextMarkupCurrentPage =
        null;
      this.pdfViewer.annotationModule.textMarkupAnnotationModule.clearAnnotationSelection(
        page
      );
    }
  }
  ...
  private createToolbarItems(): any[] {
    ...
    const strokeTemplate: string = this.getTemplate(
      'button',
      '_annotation_stroke',
      'e-pv-annotation-stroke-container'
    );
    ...
    const shapesTemplate: string = this.getTemplate(
      'button',
      '_annotation_shapes',
      'e-pv-annotation-shapes-container'
    );
    const calibrateTemplate: string = this.getTemplate(
      'button',
  ...
  private createSignContainer(): void {
    this.handWrittenSignatureItem = this.pdfViewerBase.getElement(
      '_annotation_signature'
    );
    this.primaryToolbar.createTooltip(
      this.pdfViewerBase.getElement('_annotation_signature'),
      this.pdfViewer.localeObj.getConstant('SignatureFieldDialogHeaderText')
    );
    this.handWrittenSignatureItem.setAttribute(
      'aria-label',
      this.pdfViewer.localeObj.getConstant('SignatureFieldDialogHeaderText')
    );
    ...
    let items: ItemModel[] = [];
    if (
      this.pdfViewer.handWrittenSignatureSettings ||
      ...
      if (
        this.pdfViewer.handWrittenSignatureSettings.signatureItem[0] ===
        'Signature'
      ) {
        items = [
          {
    ...
    const saveOptions: DropDownButtonModel = {
      ...
      beforeItemRender: (args: MenuEventArgs): void => {
        ...
        if (
          args.element &&
          args.element.className.indexOf('e-separator') !== -1
        ) {
        ...
        if (args.item.text === 'ADD SIGNATURE') {
          ...
          for (
            let collection: number =
              this.pdfViewerBase.signatureModule.signaturecollection.length;
            collection > 0;
            collection--
          ) {
            if (
              this.saveSignatureCount <
              this.pdfViewerBase.signatureModule.getSaveLimit(
                this.pdfViewer.handWrittenSignatureSettings.saveSignatureLimit
              )
            ) {
              args.element.style.display = 'block';
              const signatureCollection: any =
                this.pdfViewerBase.signatureModule.signaturecollection[
                  collection - 1
                ];
              const collectionKey: any = signatureCollection.image[0].imageData;
              const signatureID: string =
                signatureCollection.image[0].id.split('_')[1];
              if (!signatureCollection.isInitial && collectionKey !== '') {
                ...
                if (
                  signatureCollection.image[0].signatureType === 'SignatureText'
                ) {
                  ...
                  (signatureImage as HTMLElement).style.fontFamily =
                    signatureCollection.image[0].fontFamily;
                  ...
                  (signatureImage as HTMLElement).style.textOverflow =
                    'ellipsis';
                  ...
                  (signatureImage as HTMLImageElement).style.paddingLeft =
                    '20px';
                  (signatureImage as HTMLImageElement).style.paddingRight =
                    '18px';
                  (signatureImage as HTMLImageElement).style.paddingTop =
                    '12px';
                  (signatureImage as HTMLImageElement).style.paddingBottom =
                    '12px';
                  (signatureImage as HTMLImageElement).style.boxSizing =
                    'content-box';
                  signatureImage.setAttribute('alt', 'Saved Signature');
                }
                signatureImage.addEventListener(
                  'mouseover',
                  this.hoverSignatureImage.bind(this)
                );
                signatureImage.addEventListener(
                  'mouseleave',
                  this.leaveSignatureImage.bind(this)
                );
                ...
                this.pdfViewerBase.getElement(
                  '_annotation_signature-popup'
                ).style.width = '206px';
                this.saveSignatureCount++;
              }
            }
          ...
          addInitialSpan.textContent = this.pdfViewer.localeObj.getConstant(
            'HandwrittenSignatureDialogHeaderText'
          );
          ...
          addInitialSpan.addEventListener(
            'click',
            this.clickSignature.bind(this)
          );
          args.element.appendChild(addInitialSpan);
          args.element.addEventListener(
            'mouseover',
            this.hoverInitialBtn.bind(this)
          );
        ...
        if (args.item.text === 'ADD INITIAL') {
          ...
          addInitialSpan.textContent = this.pdfViewer.localeObj.getConstant(
            'HandwrittenInitialDialogHeaderText'
          );
          if (this.pdfViewer.locale === 'en-US') {
            addInitialSpan.style.width = '130px';
          } else {
  ...
  private updateSignatureCount(): void {
    ...
    for (
      ...
      collection < signatureCollection.length;
      collection++
    ) {
      const colletionList: any =
        signatureCollection[parseInt(collection.toString(), 10)];
      ...
      if (
        ...
        if (signatureCollection.image[0].imageData !== '') {
          if (!signatureCollection.isInitial) {
            this.saveSignatureCount++;
            const addedSignature: HTMLElement = document.getElementById(
              'sign_border_' +
                signatureCollection.image[0].id.split('_')[1] +
                ''
            );
            const deleteSignature: HTMLElement = document.getElementById(
              'delete_' + signatureCollection.image[0].id.split('_')[1] + ''
            );
            deleteSignature.style.backgroundClip = 'content-box';
            addedSignature.addEventListener(
              'click',
              this.renderAddedSignature.bind(this)
            );
            deleteSignature.addEventListener(
              'click',
              this.deleteSavedSign.bind(this)
            );
            deleteSignature.addEventListener(
              'mouseover',
              this.hoverSignatureDelete.bind(this)
            );
            deleteSignature.addEventListener(
              'mouseleave',
              this.leaveSignatureDelete.bind(this)
            );
            addedSignature.addEventListener(
              'mouseover',
              this.hoverSignatureDelete.bind(this)
  ...
  private hoverSignatureDelete(): void {
    ...
    if (
      eventTarget.id === 'sign_' + eventTarget.id.split('_')[1] ||
      eventTarget.classList.contains('e-pv-delete')
    ) {
  ...
  private hoverInitialBtn(event: any): void {
    ...
    if (
      ...
      currentFieldID = isNullOrUndefined(event.path)
        ? event.composedPath()[0].id
        : event.path[0].id;
    }
    if (
      currentFieldID !== 'sign_' + currentFieldID.split('_')[1] &&
      currentFieldID !== 'delete_' + currentFieldID.split('_')[1]
    ) {
      ...
      if (
        (liElement as HTMLLIElement) != null &&
        (eventTarget.id !== 'sign_' + eventTarget.id.split('_')[1] ||
          eventTarget.id !== 'sign_border_' + eventTarget.id.split('_')[2])
      ) {
        liElement.style.background = 'transparent';
        liElement.style.cursor = 'default';
      } else if (
        (liElement.parentElement as HTMLLIElement) != null &&
        (eventTarget.id !== 'sign_' + eventTarget.id.split('_')[1] ||
          eventTarget.id !== 'sign_border_' + eventTarget.id.split('_')[2])
      ) {
  ...
  private leaveSignatureDelete(): void {
    ...
    if (
      ...
      if (
        eventTarget.children[0] &&
        eventTarget.children[0].tagName === 'IMG'
      ) {
  ...
  public deleteSavedSign(event: any): void {
    ...
    const signaturecollection: any =
      this.pdfViewerBase.signatureModule.signaturecollection;
    for (
      let collection: number = signaturecollection.length;
      collection > 0;
      collection--
    ) {
      if (
        event.target.parentElement.children[0].id ===
        'sign_' + signaturecollection[collection - 1].image[0].id.split('_')[1]
      ) {
        const RemoveSignature: any = signaturecollection[collection - 1];
        RemoveSignature.image[0].imageData = '';
        this.pdfViewerBase.signatureModule.signaturecollection.splice(
          collection - 1,
          1
        );
        break;
  ...
  private getTemplate(
    ...
    const element: HTMLElement = createElement(elementName, {
  ...
  private createStampContainer(): HTMLElement {
    ...
    this.primaryToolbar.createTooltip(
      this.pdfViewerBase.getElement('_annotation_stamp'),
      this.pdfViewer.localeObj.getConstant('Add Stamp')
    );
    this.stampElement.setAttribute(
      'aria-label',
      this.pdfViewer.localeObj.getConstant('Add Stamp')
    );
    this.stampElement.setAttribute('role', 'combobox');
    this.stampElement.setAttribute('aria-expanded', 'false');
    const contextMenuElement: HTMLElement = createElement('ul', {
      id: this.pdfViewer.element.id + 'contextMenuElement',
    });
    this.pdfViewerBase
      .getElement('_annotation_stamp')
      .appendChild(contextMenuElement);
    const items: Object[] = [];
    if (
      this.pdfViewer.stampSettings.dynamicStamps &&
      this.pdfViewer.stampSettings.dynamicStamps.length > 0
    ) {
      ...
      this.pdfViewer.stampSettings.dynamicStamps.forEach(
        (stampItem: DynamicStampItem, index: number) => {
          // eslint-disable-next-line
          ...
          dynamicStamps.push({
            text: this.pdfViewer.localeObj.getConstant(name),
            label: name,
          });
        }
      );
    ...
    if (
      this.pdfViewer.stampSettings.signStamps &&
      this.pdfViewer.stampSettings.signStamps.length > 0
    ) {
      ...
      this.pdfViewer.stampSettings.signStamps.forEach(
        (stampItem: SignStampItem, index: number) => {
          // eslint-disable-next-line
    ...
    if (
      this.pdfViewer.stampSettings.standardBusinessStamps &&
      ...
      items.push({
        ...
        items: standardsBusinessStamps,
      });
      this.pdfViewer.stampSettings.standardBusinessStamps.forEach(
        (stampItem: StandardBusinessStampItem, index: number) => {
          // eslint-disable-next-line
    ...
    if (this.pdfViewer.customStampSettings.enableCustomStamp) {
      ...
      this.pdfViewerBase.customStampCollection = (this.pdfViewer
        .customStampSettings.customStamps as any)
    ...
    const menuOptions: MenuModel = {
      ...
      beforeOpen: (args: Menuopen): void => {
        ...
        if (
          args.parentItem.text === '' &&
          this.pdfViewer.customStampSettings.isAddToMenu &&
          args.items.length > 0
        ) {
          let currentElements: any = null;
          for (let i: number = 0; i < args.items.length; i++) {
            if (
              args.items[parseInt(i.toString(), 10)].text ===
              this.pdfViewer.localeObj.getConstant('Custom Stamp')
            ) {
          ...
          if (
            ...
            for (let n: number = 0; n < stampElements.length; n++) {
              elements.push({
                customStampName:
                  stampElements[parseInt(n.toString(), 10)].customStampName,
                customStampImageSource:
                  stampElements[parseInt(n.toString(), 10)]
                    .customStampImageSource,
              });
            }
          }
          for (let m: number = 0; m < elements.length; m++) {
            if (currentElements != null) {
              currentElements.items.push({
                text: elements[parseInt(m.toString(), 10)].customStampName,
              });
              for (let i: number = 0; i < args.items.length; i++) {
                if (
                  args.items[parseInt(i.toString(), 10)].text ===
                  ...
                  const liElem: HTMLElement = args.element.children[
        ...
        if (
          ...
          if (stampElements && stampElements.length > 10) {
            (
              closest(args.element, '.e-menu-wrapper') as HTMLElement
            ).style.height = '350px';
          }
        }
      ...
      beforeClose: (args: Menuopen) => {
        if (
          (args.parentItem &&
            args.parentItem.text !==
              this.pdfViewer.localeObj.getConstant('Custom Stamp') &&
            args.parentItem.text !== 'Standard Business' &&
            args.parentItem.text !== 'Dynamic' &&
            args.parentItem.text !== 'Sign Here') ||
          !args.parentItem
        ) {
      ...
      select: (args: MenuEventArgs): void => {
        ...
        if (
          ...
          const stampImage: HTMLElement = document.getElementById(
            this.pdfViewer.element.id + '_stampElement'
          );
          if (stampImage) {
            stampImage.click();
          }
          ...
          for (let n: number = 0; n < elements.length; n++) {
            if (
              elements[parseInt(n.toString(), 10)].customStampName ===
              args.item.text
            ) {
              this.pdfViewer.annotationModule.stampAnnotationModule.customStampName =
                args.item.text;
              this.checkStampAnnotations();
              this.pdfViewer.annotation.stampAnnotationModule.isStampAddMode =
                true;
              this.pdfViewer.annotationModule.stampAnnotationModule.isStampAnnotSelected =
                true;
              this.pdfViewerBase.stampAdded = true;
              this.pdfViewerBase.isAlreadyAdded = true;
              this.pdfViewer.annotationModule.stampAnnotationModule.createCustomStampAnnotation(
                elements[parseInt(n.toString(), 10)].customStampImageSource
              );
              this.pdfViewerBase.stampAdded = false;
            }
          }
        } else if (
          args.item.text !== this.pdfViewer.localeObj.getConstant('Dynamic') &&
          args.item.text !== '' &&
          args.item.text !== 'Standard Business' &&
          (this.stampParentID === 'Sign Here' || args.item.text !== 'Sign Here')
        ) {
          this.updateInteractionTools();
          this.checkStampAnnotations();
          this.pdfViewer.annotation.stampAnnotationModule.isStampAddMode = true;
          ...
          if (
            this.stampParentID ===
            this.pdfViewer.localeObj.getConstant('Dynamic')
          ) {
            this.pdfViewerBase.isDynamicStamp = true;
            this.pdfViewer.annotationModule.stampAnnotationModule.retrieveDynamicStampAnnotation(
              (args.item as any).label
            );
          } else {
            this.pdfViewerBase.isDynamicStamp = false;
            this.pdfViewer.annotationModule.stampAnnotationModule.retrievestampAnnotation(
              (args.item as any).label
            );
          }
          if (Browser.isDevice) {
            this.stampToolMobileForMobile(
              this.pdfViewer.element.id + '_annotation_stamp'
            );
          }
        }
      },
    ...
    this.menuItems = new Menu(
      menuOptions,
      '#' + this.pdfViewer.element.id + 'contextMenuElement'
    );
  ...
  public createCustomStampElement(): void {
    const stampImage: HTMLElement = createElement('input', {
    ...
    if (args.target.files[0] !== null) {
      ...
      if (uploadedFile.type.split('/')[0] === 'image') {
        ...
        reader.onload = (e: any): void => {
          ...
          proxy.pdfViewer.annotation.stampAnnotationModule.isStampAddMode =
            true;
          proxy.pdfViewer.annotationModule.stampAnnotationModule.isStampAnnotSelected =
            true;
  ...
  public checkStampAnnotations(): void {
    if (
      this.pdfViewer.annotation.stampAnnotationModule.isStampAddMode &&
      this.pdfViewer.selectedItems &&
      this.pdfViewer.selectedItems.annotations
    ) {
      for (
        let i: number = 0;
        i < this.pdfViewer.selectedItems.annotations.length;
        i++
      ) {
        ...
        if (
          ...
          !annotation.author &&
          (annotation.shapeAnnotationType !== 'Shape' ||
            annotation.shapeAnnotationType !== 'Image')
        ) {
          this.pdfViewer.remove(annotation);
          this.pdfViewer.annotation.renderAnnotations(
  ...
  private createDropDowns(isPath?: boolean): void {
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      ...
      const shapeToolbar: Tool = this.createShapeOptions(
        this.shapeElement.id,
        true
      );
      this.shapeDropDown = this.createDropDownButton(
        this.shapeElement,
        'e-pv-annotation-shape-icon',
        shapeToolbar.element,
        this.pdfViewer.localeObj.getConstant('Add Shapes')
      );
      this.calibrateElement = this.pdfViewerBase.getElement(
        '_annotation_calibrate'
      );
      const calibrateToolbar: Tool = this.createShapeOptions(
        this.calibrateElement.id,
        false
      );
      this.calibrateDropDown = this.createDropDownButton(
        this.calibrateElement,
        'e-pv-annotation-calibrate-icon',
        calibrateToolbar.element,
        this.pdfViewer.localeObj.getConstant('Calibrate')
      );
    }
    if (!isPath) {
      this.colorDropDownElement =
      ...
      this.colorDropDown = this.createDropDownButton(
        this.colorDropDownElement,
        'e-pv-annotation-color-icon',
        this.colorPalette.element.parentElement,
        this.pdfViewer.localeObj.getConstant('Color edit')
      );
      ...
      this.pdfViewerBase
        .getElement('_annotation_color-popup')
        .addEventListener('click', this.onColorPickerCancelClick.bind(this));
    }
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      this.strokeDropDownElement =
        this.pdfViewerBase.getElement('_annotation_stroke');
      this.strokeColorPicker = this.createColorPicker(
        this.strokeDropDownElement.id
      );
      this.strokeColorPicker.change = this.onStrokePickerChange.bind(this);
      this.strokeDropDown = this.createDropDownButton(
        this.strokeDropDownElement,
        'e-pv-annotation-stroke-icon',
        this.strokeColorPicker.element.parentElement,
        this.pdfViewer.localeObj.getConstant('Stroke edit')
      );
      ...
      this.pdfViewerBase
        .getElement('_annotation_stroke-popup')
        .addEventListener('click', this.onStrokePickerCancelClick.bind(this));
      this.thicknessElement = this.pdfViewerBase.getElement(
        '_annotation_thickness'
      );
      const thicknessContainer: HTMLElement = this.createThicknessSlider(
        this.thicknessElement.id
      );
      this.thicknessDropDown = this.createDropDownButton(
        this.thicknessElement,
        'e-pv-annotation-thickness-icon',
        thicknessContainer,
        this.pdfViewer.localeObj.getConstant('Change thickness')
      );
      this.thicknessDropDown.beforeOpen =
        this.thicknessDropDownBeforeOpen.bind(this);
    ...
    if (!isPath) {
      this.opacityDropDownElement = this.pdfViewerBase.getElement(
        '_annotation_opacity'
      );
      const sliderContainer: HTMLElement = this.createSlider(
        this.opacityDropDownElement.id
      );
      this.opacityDropDown = this.createDropDownButton(
        this.opacityDropDownElement,
        'e-pv-annotation-opacity-icon',
        sliderContainer,
        this.pdfViewer.localeObj.getConstant('Opacity edit')
      );
      this.opacityDropDown.beforeOpen =
        this.opacityDropDownBeforeOpen.bind(this);
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      this.fontFamilyElement = this.pdfViewerBase.getElement(
        '_annotation_fontname'
      );
      ...
      this.fontSizeElement = this.pdfViewerBase.getElement(
        '_annotation_fontsize'
      );
      this.createDropDownListForSize(this.fontSizeElement);
      this.fontColorElement = this.pdfViewerBase.getElement(
        '_annotation_textcolor'
      );
      ...
      this.fontColorDropDown = this.createDropDownButton(
        this.fontColorElement,
        'e-pv-annotation-textcolor-icon',
        this.fontColorPalette.element.parentElement,
        this.pdfViewer.localeObj.getConstant('Font color')
      );
      this.textAlignElement = this.pdfViewerBase.getElement(
        '_annotation_textalign'
      );
      this.alignmentToolbar = this.createShapeOptions(
        this.textAlignElement.id,
        undefined,
        true
      );
      this.textAlignDropDown = this.createDropDownButton(
        this.textAlignElement,
        'e-pv-annotation-textalign-icon',
        this.alignmentToolbar.element,
        this.pdfViewer.localeObj.getConstant('Text Align')
      );
      this.textAlignDropDown.beforeOpen =
        this.textAlignDropDownBeforeOpen.bind(this);
      this.textPropElement = this.pdfViewerBase.getElement(
        '_annotation_textproperties'
      );
      this.propertiesToolbar = this.createShapeOptions(
        this.textPropElement.id,
        undefined,
        false,
        true
      );
      this.textPropertiesDropDown = this.createPropDropDownButton(
        this.textPropElement,
        'e-pv-annotation-textprop-icon',
        this.propertiesToolbar.element,
        this.pdfViewer.localeObj.getConstant('Text Properties')
      );
      this.textPropertiesDropDown.beforeOpen =
        this.textPropertiesDropDownBeforeOpen.bind(this);
    }
  }
  ...
  private mobileColorpicker(id: string): void {
    ...
    if (
      ...
      if (
        ...
        this.colorDropDownElement =
          this.pdfViewerBase.getElement('_annotation_color');
        this.colorPalette = this.createColorPicker(
          this.colorDropDownElement.id
        );
        this.colorPalette.change = this.onColorPickerChange.bind(this);
        this.colorDropDown = this.createDropDownButton(
          this.colorDropDownElement,
      ...
      if (id === this.pdfViewer.element.id + '_annotation_freeTextEdit') {
        ...
        this.fontColorPalette = this.createColorPicker(
          this.fontColorElement.id
        );
        this.fontColorPalette.change = this.onFontColorChange.bind(this);
        this.fontColorDropDown = this.createDropDownButton(
          this.fontColorElement,
      ...
      if (
        ...
        this.strokeDropDown.beforeOpen =
          this.strokeDropDownBeforeOpen.bind(this);
        this.strokeDropDown.open = this.strokeDropDownOpen.bind(this);
        this.pdfViewerBase
          .getElement('_annotation_stroke-popup')
          .addEventListener('click', this.onStrokePickerCancelClick.bind(this));
  ...
  private opacityDropDownOpen(args: OpenCloseMenuEventArgs): void {
    if (Browser.isDevice && !this.pdfViewer.enableDesktopMode) {
      ...
      args.element.parentElement.style.top =
        this.pdfViewerBase.viewerContainer.clientHeight + 'px';
    } else {
    ...
    if (this.pdfViewer.annotationModule.textMarkupAnnotationModule) {
      if (
        ...
        this.colorPalette.value =
          this.pdfViewer.annotationModule.textMarkupAnnotationModule.currentTextMarkupAnnotation.color;
      } else {
  ...
  private colorDropDownOpen(args: any): void {
    if (Browser.isDevice && !this.pdfViewer.enableDesktopMode) {
      args.element.parentElement.style.top =
        this.pdfViewerBase.viewerContainer.clientHeight / 2 + 'px';
    }
  ...
  private strokeDropDownBeforeOpen(args: BeforeOpenCloseMenuEventArgs): void {
    if (this.pdfViewer.selectedItems.annotations.length === 1) {
      this.strokeColorPicker.value =
        this.pdfViewer.selectedItems.annotations[0].wrapper.children[0].style.strokeColor;
    } else {
  ...
  private popupPosition: any = function (
    colorElement: any,
    buttonElement: any
  ): void {
    if (colorElement && colorElement.element) {
      const buttonBounds: any = buttonElement.getBoundingClientRect();
      const elements: any = colorElement.element
        .getElementsByClassName('e-container e-color-palette')[0]
        .getElementsByClassName('e-palette')[0]
        .getBoundingClientRect();
      const mainContainerBounds: any =
        this.pdfViewerBase.mainContainer.getBoundingClientRect();
      if (elements.left + elements.width > mainContainerBounds.width) {
        colorElement.element.parentElement.style.left =
          elements.left - elements.width + buttonBounds.width + 'px';
      }
    }
  };
  ...
  private onFontColorChange(args: any): void {
    ...
    if (!isBlazor()) {
      currentColor =
        args.currentValue.hex === '' ? '#ffffff00' : args.currentValue.hex;
    } else {
      currentColor = args[0];
    }
    if (this.pdfViewer.selectedItems.annotations.length === 1) {
    ...
    if (isBlazor()) {
      this.fontColorElementInBlazor = this.pdfViewer.element.querySelector(
        '.e-pv-annotation-textcolor-container'
      );
  ...
  private onFontFamilyChange(args: any): void {
    ...
    if (!isBlazor()) {
      currentValue =
        args && args.fontFamily && args.fontFamily.value
  ...
  private onFontSizeChange(args: any, isInteracted: boolean): void {
    ...
    if (!isBlazor()) {
      currentValue =
        args && args.fontSize && args.fontSize.value ? args.fontSize.value : '';
    } else {
      currentValue = args;
    }
  ...
  private textAlignDropDownBeforeOpen(
    ...
    if (this.pdfViewer.selectedItems.annotations.length === 1) {
      if (
        args.element.getElementsByTagName('button') &&
        args.element.getElementsByTagName('button').length > 0
      ) {
        ...
        for (let n: number = 0; n < dropDownOptions.length; n++) {
          if (dropDownOptions[parseInt(n.toString(), 10)]) {
            dropDownOptions[parseInt(n.toString(), 10)].classList.remove(
              'textprop-option-active'
            );
            if (
              dropDownOptions[parseInt(n.toString(), 10)].id ===
                this.pdfViewer.element.id + '_left_align' &&
              selectedAnnotation.textAlign === 'Left'
            ) {
              dropDownOptions[parseInt(n.toString(), 10)].classList.add(
                'textprop-option-active'
              );
            }
            if (
              dropDownOptions[parseInt(n.toString(), 10)].id ===
                this.pdfViewer.element.id + '_right_align' &&
            ...
            if (
              dropDownOptions[parseInt(n.toString(), 10)].id ===
                this.pdfViewer.element.id + '_center_align' &&
            ...
            if (
              dropDownOptions[parseInt(n.toString(), 10)].id ===
                this.pdfViewer.element.id + '_justify_align' &&
              selectedAnnotation.textAlign === 'Justify'
            ) {
              dropDownOptions[parseInt(n.toString(), 10)].classList.add(
                'textprop-option-active'
              );
      ...
      if (
        ...
        for (let n: number = 0; n < dropDownOptions.length; n++) {
          if (dropDownOptions[parseInt(n.toString(), 10)]) {
            ...
            if (
              dropDownOptions[parseInt(n.toString(), 10)].id ===
                this.pdfViewer.element.id + '_bold' &&
              selectedAnnotation.font.isBold
            ) {
              dropDownOptions[parseInt(n.toString(), 10)].classList.add(
                'textprop-option-active'
              );
            ...
            if (
              dropDownOptions[parseInt(n.toString(), 10)].id ===
                this.pdfViewer.element.id + '_italic' &&
              selectedAnnotation.font.isItalic
            ) {
              dropDownOptions[parseInt(n.toString(), 10)].classList.add(
                'textprop-option-active'
              );
            ...
            if (
              dropDownOptions[parseInt(n.toString(), 10)].id ===
                this.pdfViewer.element.id + '_strikeout' &&
              selectedAnnotation.font.isStrikeout
            ) {
              dropDownOptions[parseInt(n.toString(), 10)].classList.add(
                'textprop-option-active'
              );
            ...
            if (
              dropDownOptions[parseInt(n.toString(), 10)].id ===
                this.pdfViewer.element.id + '_underline_textinput' &&
  ...
  private onClickTextAlignment(args: any): void {
    ...
    if (isBlazor()) {
      ...
      currentValue =
        args && args.item && args.item.value ? args.item.value : '';
    }
    if (this.pdfViewer.selectedItems.annotations.length === 1 && currentValue) {
  ...
  private onClickTextProperties(args: any): void {
    ...
    if (this.pdfViewer.selectedItems.annotations.length === 1 && currentValue) {
      ...
      if (currentValue === 'bold') {
        fontInfo.isBold =
          !this.pdfViewer.selectedItems.annotations[0].font.isBold;
      } else if (currentValue === 'italic') {
        fontInfo.isItalic =
          !this.pdfViewer.selectedItems.annotations[0].font.isItalic;
      } else if (currentValue === 'underline') {
        fontInfo.isUnderline =
          !this.pdfViewer.selectedItems.annotations[0].font.isUnderline;
      } else if (currentValue === 'strikeout') {
        fontInfo.isStrikeout =
          !this.pdfViewer.selectedItems.annotations[0].font.isStrikeout;
      }
      ...
      if (currentValue === 'bold') {
        if (this.pdfViewer.annotationModule.freeTextAnnotationModule.isBold) {
          this.pdfViewer.annotationModule.freeTextAnnotationModule.isBold =
            false;
        } else {
        ...
        if (this.pdfViewer.annotationModule.freeTextAnnotationModule.isItalic) {
          this.pdfViewer.annotationModule.freeTextAnnotationModule.isItalic =
            false;
        } else {
        ...
        if (
          ...
          this.pdfViewer.annotationModule.freeTextAnnotationModule.isStrikethrough =
            false;
          this.pdfViewer.annotationModule.freeTextAnnotationModule.updateTextProperties();
        }
      } else if (currentValue === 'strikeout') {
  ...
  private opacityChange(args: any): void {
      ...
      if (
        ...
        if (!isBlazor()) {
          if (this.isCurrentAnnotationOpacitySet && args.name === 'changed') {
            ...
            this.pdfViewer.annotationModule.textMarkupAnnotationModule.modifyOpacityProperty(
              args
            );
          }
        } else {
        ...
        switch (
          ...
          case 'Highlight':
            this.pdfViewer.annotationModule.textMarkupAnnotationModule.highlightOpacity =
              opacityValue / 100;
            break;
          case 'Underline':
            this.pdfViewer.annotationModule.textMarkupAnnotationModule.underlineOpacity =
              opacityValue / 100;
            break;
          case 'Strikethrough':
    ...
    if (this.pdfViewer.selectedItems.annotations.length === 1) {
      const currentAnnotations: any =
        this.pdfViewer.selectedItems.annotations[0];
      if (
        currentAnnotations != null &&
        (currentAnnotations.shapeAnnotationType === 'Stamp' ||
          currentAnnotations.shapeAnnotationType === 'Image')
      ) {
        const clonedObject: PdfAnnotationBaseModel =
          cloneObject(currentAnnotations);
        const redoClonedObject: PdfAnnotationBaseModel =
          cloneObject(currentAnnotations);
        redoClonedObject.opacity = opacityValue / 100;
        this.pdfViewer.nodePropertyChange(currentAnnotations, {
          opacity: opacityValue / 100,
        });
        this.pdfViewer.annotation.triggerAnnotationPropChange(
          currentAnnotations,
          false,
          false,
          false,
        ...
        this.pdfViewer.annotation.addAction(
          this.pdfViewer.selectedItems.annotations[0].pageIndex,
          null,
          this.pdfViewer.selectedItems.annotations[0],
          'stampOpacity',
          '',
        ...
        this.pdfViewer.annotation.stampAnnotationModule.updateSessionStorage(
          this.pdfViewer.selectedItems.annotations[0],
          null,
          'opacity'
        );
      ...
      if (this.pdfViewer.annotation.shapeAnnotationModule) {
        switch (
          ...
          case 'Line':
            this.pdfViewer.annotation.shapeAnnotationModule.lineOpacity =
              opacityValue / 100;
            break;
          case 'Arrow':
            this.pdfViewer.annotation.shapeAnnotationModule.arrowOpacity =
              opacityValue / 100;
            break;
          case 'Rectangle':
            this.pdfViewer.annotation.shapeAnnotationModule.rectangleOpacity =
              opacityValue / 100;
            break;
          case 'Circle':
            this.pdfViewer.annotation.shapeAnnotationModule.circleOpacity =
              opacityValue / 100;
            break;
          case 'Polygon':
            this.pdfViewer.annotation.shapeAnnotationModule.polygonOpacity =
              opacityValue / 100;
            break;
        }
      }
      ...
      if (this.pdfViewer.drawingObject) {
        ...
        if (this.pdfViewer.drawingObject.shapeAnnotationType === 'FreeText') {
          this.pdfViewer.annotation.freeTextAnnotationModule.opacity =
            opacityValue / 100;
        }
      }
    }
  ...
  private opacityDropDownBeforeOpen(args: BeforeOpenCloseMenuEventArgs): void {
    ...
    if (this.pdfViewer.selectedItems.annotations.length === 1) {
      this.opacitySlider.value =
        this.pdfViewer.selectedItems.annotations[0].wrapper.children[0].style
          .opacity * 100;
    } else {
      if (this.pdfViewer.annotation.shapeAnnotationModule) {
        switch (
          ...
          case 'Line':
            this.opacitySlider.value =
              this.pdfViewer.annotation.shapeAnnotationModule.lineOpacity * 100;
            break;
          case 'Arrow':
            this.opacitySlider.value =
              this.pdfViewer.annotation.shapeAnnotationModule.arrowOpacity *
  ...
  private thicknessDropDownBeforeOpen(): void {
    if (this.pdfViewer.selectedItems.annotations.length === 1) {
      this.thicknessSlider.value =
        this.pdfViewer.selectedItems.annotations[0].wrapper.children[0].style.strokeWidth;
    } else {
      if (this.pdfViewer.annotation.shapeAnnotationModule) {
        switch (
          this.pdfViewer.annotation.shapeAnnotationModule.currentAnnotationMode
          ...
          case 'Line':
            this.thicknessSlider.value =
              this.pdfViewer.annotation.shapeAnnotationModule.lineThickness;
            break;
          case 'Arrow':
            this.thicknessSlider.value =
              this.pdfViewer.annotation.shapeAnnotationModule.arrowThickness;
            break;
          case 'Rectangle':
            this.thicknessSlider.value =
              this.pdfViewer.annotation.shapeAnnotationModule.rectangleThickness;
            break;
          case 'Circle':
            this.thicknessSlider.value =
              this.pdfViewer.annotation.shapeAnnotationModule.circleThickness;
            break;
          case 'Polygon':
            this.thicknessSlider.value =
              this.pdfViewer.annotation.shapeAnnotationModule.polygonThickness;
            break;
        }
      }
    }
  ...
  private calculateToolbarPosition(args: OpenCloseMenuEventArgs): void {
    if (args.element && args.element.parentElement) {
      const leftValue: number = parseFloat(
  ...
  private thicknessChange(args: ChangeEventArgs): void {
    if (this.pdfViewer.selectedItems.annotations.length === 1) {
      if (args.name === 'changed') {
        ...
        if (
          annotationModule &&
          annotationModule.inkAnnotationModule &&
          selectedItems &&
          selectedItems.shapeAnnotationType === 'Ink'
        ) {
  ...
  private ShapeThickness(args: any): void {
    if (this.pdfViewer.annotation.shapeAnnotationModule) {
      ...
      if (
        ...
        this.pdfViewer.drawingObject.shapeAnnotationType === 'FreeText'
      ) {
        this.pdfViewer.annotation.freeTextAnnotationModule.borderWidth =
          args.value;
      }
    }
    const annotationModule: Annotation = this.pdfViewer.annotation;
  ...
  private createShapeOptions(
    ...
    const toolbarElement: HTMLElement = createElement('div', {
      id: idString + '_target',
    ...
    if (isAlign) {
      toolbar = new Tool(
        { items: this.textAlignmentToolbarItems(), overflowMode: 'MultiRow' },
        toolbarElement
      );
      ...
      toolbar = new Tool(
        { items: this.textPropertiesToolbarItems(), overflowMode: 'MultiRow' },
        toolbarElement
      );
      toolbar.isStringTemplate = true;
  ...
  private createPropertyToolbarForMobile(shapeType: any): any[] {
    ...
    if (
      ...
      if (shapeType === 'Polygon') {
        if (
          this.pdfViewer.selectedItems.annotations[0].measureType === 'Area'
        ) {
          items.push({ prefixIcon: 'e-pv-calibrate-area-icon e-pv-icon' });
          items.push({ type: 'Separator', align: 'Left' });
        } else if (
          this.pdfViewer.selectedItems.annotations[0].measureType === 'Volume'
        ) {
        ...
        if (
          this.pdfViewer.selectedItems.annotations[0].measureType ===
          'Perimeter'
        ) {
      ...
      shapeType === 'Path' ||
      shapeType === 'Ink' ||
      shapeType === 'HandWrittenSignature' ||
      shapeType === 'SignatureText'
    ) {
  ...
  private handleShapeTool(type: string): void {
    if (!isBlazor() && Browser.isDevice) {
      ...
      for (let i: number = 0; i < selectedItems.length; i++) {
        if (
          selectedItems[parseInt(i.toString(), 10)].id !== type &&
          selectedItems[parseInt(i.toString(), 10)].id !==
            this.pdfViewer.element.id + '_annotation'
        ) {
          selectedItems[parseInt(i.toString(), 10)].classList.remove(
            'e-pv-select'
          );
        }
      }
  ...
  private afterAlignmentToolbarCreation(): void {
    this.alignLeftElement = this.primaryToolbar.addClassToolbarItem(
      '_left_align',
      'e-pv-left-align',
      this.pdfViewer.localeObj.getConstant('Align left')
    );
    this.alignRightElement = this.primaryToolbar.addClassToolbarItem(
      '_right_align',
      'e-pv-right-align',
      this.pdfViewer.localeObj.getConstant('Align right')
    );
    this.alignCenterElement = this.primaryToolbar.addClassToolbarItem(
      '_center_align',
      'e-pv-center-align',
      this.pdfViewer.localeObj.getConstant('Center')
    );
    this.alignJustifyElement = this.primaryToolbar.addClassToolbarItem(
      '_justify_align',
      'e-pv-justfiy-align',
      this.pdfViewer.localeObj.getConstant('Justify')
    );
  }

  private afterPropertiesToolbarCreation(): void {
    this.boldElement = this.primaryToolbar.addClassToolbarItem(
      '_bold',
      'e-pv-bold',
      this.pdfViewer.localeObj.getConstant('Bold')
    );
    this.italicElement = this.primaryToolbar.addClassToolbarItem(
      '_italic',
      'e-pv-italic',
      this.pdfViewer.localeObj.getConstant('Italic')
    );
    this.fontStyleStrikethroughItem = this.primaryToolbar.addClassToolbarItem(
      '_strikeout',
      'e-pv-strikeout',
      this.pdfViewer.localeObj.getConstant('Strikethroughs')
    );
    this.fontStyleUnderlineItem = this.primaryToolbar.addClassToolbarItem(
      '_underline_textinput',
      'e-pv-underlinetext',
      this.pdfViewer.localeObj.getConstant('Underlines')
    );
  }
  ...
  private createDropDownListForSize(fontSelectElement: HTMLElement): void {
    ...
    if (!isNullOrUndefined(fontSelectElement)) {
      ...
      const toolTipElement: HTMLElement = fontSelectElement.parentElement
        ? fontSelectElement.parentElement
        : fontSelectElement;
      this.primaryToolbar.createTooltip(
        toolTipElement,
        this.pdfViewer.localeObj.getConstant('Font size')
      );
      fontSelectElement.setAttribute(
        'aria-label',
        this.pdfViewer.localeObj.getConstant('Font size')
      );
    }
  ...
  private createDropDownListForFamily(fontSelectElement: HTMLElement): void {
    const fontStyle: { [key: string]: Object }[] = [
      { FontName: 'Helvetica' },
      { FontName: 'Courier' },
      { FontName: 'Symbol' },
    ...
    if (!this.pdfViewer.enableRtl) {
      this.fontFamily = new ComboBox({
        ...
        cssClass: 'e-pv-prop-dropdown',
        itemTemplate: initializeCSPTemplate(function (data: any): string {
    ...
    if (!isNullOrUndefined(fontSelectElement)) {
      ...
      this.primaryToolbar.createTooltip(
        toolTipElement,
        this.pdfViewer.localeObj.getConstant('Font family')
      );
      fontSelectElement.setAttribute(
        'aria-label',
        this.pdfViewer.localeObj.getConstant('Font family')
  ...
  private onShapeToolbarClicked = (args: ClickEventArgs): void => {
    ...
    const shapeAnnotationModule: ShapeAnnotation =
      this.pdfViewer.annotation.shapeAnnotationModule;
    if (Browser.isDevice || !this.pdfViewer.enableDesktopMode) {
      if (this.pdfViewerBase.action === 'Polygon') {
        (this.pdfViewerBase.tool as PolygonDrawingTool).mouseUp(
          args as MouseEventArgs,
    ...
    switch ((args.originalEvent.target as HTMLElement).id) {
      ...
      case elementId + '_shape_lineIcon':
        ...
        this.updateColorInIcon(
          this.colorDropDownElement,
          shapeAnnotationModule.lineFillColor
        );
        this.updateColorInIcon(
          this.strokeDropDownElement,
          shapeAnnotationModule.lineStrokeColor
        );
      ...
      case elementId + '_shape_arrowIcon':
        ...
        this.updateColorInIcon(
          this.colorDropDownElement,
          shapeAnnotationModule.arrowFillColor
        );
        this.updateColorInIcon(
          this.strokeDropDownElement,
          shapeAnnotationModule.arrowStrokeColor
        );
      ...
      case elementId + '_shape_rectangleIcon':
        ...
        this.updateColorInIcon(
          this.colorDropDownElement,
          shapeAnnotationModule.rectangleFillColor
        );
        this.updateColorInIcon(
          this.strokeDropDownElement,
          shapeAnnotationModule.rectangleStrokeColor
        );
      ...
      case elementId + '_shape_circleIcon':
        ...
        this.updateColorInIcon(
          this.colorDropDownElement,
          shapeAnnotationModule.circleFillColor
        );
        this.updateColorInIcon(
          this.strokeDropDownElement,
          shapeAnnotationModule.circleStrokeColor
        );
      ...
      case elementId + '_shape_pentagonIcon':
        ...
        this.updateColorInIcon(
          this.colorDropDownElement,
          shapeAnnotationModule.polygonFillColor
        );
        this.updateColorInIcon(
          this.strokeDropDownElement,
          shapeAnnotationModule.polygonStrokeColor
        );
  ...
  private onCalibrateToolbarClicked(args: ClickEventArgs): void {
    const elementId: string = this.pdfViewer.element.id;
    const measureModule: MeasureAnnotation =
      this.pdfViewer.annotation.measureAnnotationModule;
    this.deselectAllItems();
    this.deselectAllItemsForMobile();
    this.resetFreeTextAnnot();
    ...
    switch ((args.originalEvent.target as HTMLElement).id) {
      ...
      case elementId + '_calibrate_distanceIcon':
        ...
        this.updateColorInIcon(
          this.colorDropDownElement,
          measureModule.distanceFillColor
        );
        this.updateColorInIcon(
          this.strokeDropDownElement,
          measureModule.distanceStrokeColor
        );
      ...
      case elementId + '_calibrate_perimeterIcon':
        ...
        this.updateColorInIcon(
          this.strokeDropDownElement,
          measureModule.perimeterStrokeColor
        );
      ...
      case elementId + '_calibrate_areaIcon':
        ...
        this.updateColorInIcon(
          this.colorDropDownElement,
          measureModule.areaFillColor
        );
        this.updateColorInIcon(
          this.strokeDropDownElement,
          measureModule.areaStrokeColor
        );
      ...
      case elementId + '_calibrate_radiusIcon':
        ...
        this.updateColorInIcon(
          this.colorDropDownElement,
          measureModule.radiusFillColor
        );
        this.updateColorInIcon(
          this.strokeDropDownElement,
          measureModule.radiusStrokeColor
        );
      ...
      case elementId + '_calibrate_volumeIcon':
        ...
        this.updateColorInIcon(
          this.colorDropDownElement,
          measureModule.volumeFillColor
        );
        this.updateColorInIcon(
          this.strokeDropDownElement,
          measureModule.volumeStrokeColor
        );
  ...
  private afterCalibrateToolbarCreationForMobile(): void {
    this.primaryToolbar.addClassToolbarItem(
      '_calibrate_distance',
      'e-pv-calibrate-distance',
      this.pdfViewer.localeObj.getConstant('Calibrate Distance')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_calibrate_perimeter',
      'e-pv-calibrate-perimeter',
      this.pdfViewer.localeObj.getConstant('Calibrate Perimeter')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_calibrate_area',
      'e-pv-calibrate-area',
      this.pdfViewer.localeObj.getConstant('Calibrate Area')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_calibrate_radius',
      'e-pv-calibrate-radius',
      this.pdfViewer.localeObj.getConstant('Calibrate Radius')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_calibrate_volume',
      'e-pv-calibrate-volume',
      this.pdfViewer.localeObj.getConstant('Calibrate Volume')
    );
  }
  ...
  private afterShapeToolbarCreationForMobile(): void {
    this.primaryToolbar.addClassToolbarItem(
      '_annotation_color',
      'e-pv-annotation-color-container',
      this.pdfViewer.localeObj.getConstant('Change Color')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_annotation_stroke',
      'e-pv-annotation-stroke-container',
      this.pdfViewer.localeObj.getConstant('Change Stroke Color')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_annotation_thickness',
      'e-pv-annotation-thickness-container',
      this.pdfViewer.localeObj.getConstant('Chnage Border Thickness')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_annotation_opacity',
      'e-annotation-opacity-container',
      this.pdfViewer.localeObj.getConstant('Change Opacity')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_shape_line',
      'e-pv-shape-line',
      this.pdfViewer.localeObj.getConstant('Add line')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_shape_arrow',
      'e-pv-shape-arrow',
      this.pdfViewer.localeObj.getConstant('Add arrow')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_shape_rectangle',
      'e-pv-shape-rectangle',
      this.pdfViewer.localeObj.getConstant('Add rectangle')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_shape_circle',
      'e-pv-shape-circle',
      this.pdfViewer.localeObj.getConstant('Add circle')
    );
    this.primaryToolbar.addClassToolbarItem(
      '_shape_pentagon',
      'e-pv-shape-pentagon',
      this.pdfViewer.localeObj.getConstant('Add polygon')
    );
  }

  private afterShapeToolbarCreation(): void {
    this.lineElement = this.primaryToolbar.addClassToolbarItem(
      '_shape_line',
      'e-pv-shape-line',
      this.pdfViewer.localeObj.getConstant('Add line')
    );
    this.arrowElement = this.primaryToolbar.addClassToolbarItem(
      '_shape_arrow',
      'e-pv-shape-arrow',
      this.pdfViewer.localeObj.getConstant('Add arrow')
    );
    this.rectangleElement = this.primaryToolbar.addClassToolbarItem(
      '_shape_rectangle',
      'e-pv-shape-rectangle',
      this.pdfViewer.localeObj.getConstant('Add rectangle')
    );
    this.circleElement = this.primaryToolbar.addClassToolbarItem(
      '_shape_circle',
      'e-pv-shape-circle',
      this.pdfViewer.localeObj.getConstant('Add circle')
    );
    this.polygonElement = this.primaryToolbar.addClassToolbarItem(
      '_shape_pentagon',
      'e-pv-shape-pentagon',
      this.pdfViewer.localeObj.getConstant('Add polygon')
    );
  ...
  private afterCalibrateToolbarCreation(): void {
    this.calibrateDistance = this.primaryToolbar.addClassToolbarItem(
      '_calibrate_distance',
      'e-pv-calibrate-distance',
      this.pdfViewer.localeObj.getConstant('Calibrate Distance')
    );
    this.calibratePerimeter = this.primaryToolbar.addClassToolbarItem(
      '_calibrate_perimeter',
      'e-pv-calibrate-perimeter',
      this.pdfViewer.localeObj.getConstant('Calibrate Perimeter')
    );
    this.calibrateArea = this.primaryToolbar.addClassToolbarItem(
      '_calibrate_area',
      'e-pv-calibrate-area',
      this.pdfViewer.localeObj.getConstant('Calibrate Area')
    );
    this.calibrateRadius = this.primaryToolbar.addClassToolbarItem(
      '_calibrate_radius',
      'e-pv-calibrate-radius',
      this.pdfViewer.localeObj.getConstant('Calibrate Radius')
    );
    this.calibrateVolume = this.primaryToolbar.addClassToolbarItem(
      '_calibrate_volume',
      'e-pv-calibrate-volume',
      this.pdfViewer.localeObj.getConstant('Calibrate Volume')
    );
  ...
  private afterMobileToolbarCreation(): void {
    const isMac: boolean = navigator.platform.match(/(Mac|iPhone|iPod|iPad)/i)
      ? true
      : false;
    this.highlightItem = this.primaryToolbar.addClassToolbarItem(
      '_highlight',
      'e-pv-highlight',
      this.pdfViewer.localeObj.getConstant('Highlight')
    );
    this.underlineItem = this.primaryToolbar.addClassToolbarItem(
      '_underline',
      'e-pv-underline',
      this.pdfViewer.localeObj.getConstant('Underline')
    );
    this.strikethroughItem = this.primaryToolbar.addClassToolbarItem(
      '_strikethrough',
      'e-pv-strikethrough',
      this.pdfViewer.localeObj.getConstant('Strikethrough')
    );
    this.shapesItem = this.primaryToolbar.addClassToolbarItem(
      '_annotation_shapes',
      'e-pv-annotation-shapes',
      this.pdfViewer.localeObj.getConstant('Add Shapes')
    );
    this.calibrateItem = this.primaryToolbar.addClassToolbarItem(
      '_annotation_calibrate',
      'e-pv-annotation-calibrate',
      this.pdfViewer.localeObj.getConstant('Calibrate')
    );
    this.freeTextEditItem = this.primaryToolbar.addClassToolbarItem(
      '_annotation_freeTextEdit',
      'e-pv-annotation-freeTextEdit',
      this.pdfViewer.localeObj.getConstant('Free Text')
    );
    this.commentItem = this.primaryToolbar.addClassToolbarItem(
      '_comment',
      'e-pv-comment',
      this.pdfViewer.localeObj.getConstant('Add Comments')
    );
    this.commentItem = this.primaryToolbar.addClassToolbarItem(
      '_annotation_commentPanel',
      'e-pv-annotation-comment-panel',
      this.pdfViewer.localeObj.getConstant('Comment Panel') +
        (isMac ? ' (⌘+⌥+0)' : ' (Ctrl+Alt+0)')
    );
    this.inkAnnotationItem = this.primaryToolbar.addClassToolbarItem(
      '_annotation_ink',
      'e-pv-annotation-ink',
      this.pdfViewer.localeObj.getConstant('Draw Ink')
    );
  ...
  private createColorPicker(idString: string): ColorPicker {
    ...
    const existingInput: HTMLElement = document.getElementById(
      idString + '_target'
    );
    if (existingInput) {
  ...
  private onColorPickerChange(args: any): void {
    ...
    if (this.pdfViewer.annotationModule.textMarkupAnnotationModule) {
      if (
        ...
        this.pdfViewer.annotationModule.textMarkupAnnotationModule.modifyColorProperty(
          currentColor
        );
      } else {
        switch (
          ...
          case 'Highlight':
            this.pdfViewer.annotationModule.textMarkupAnnotationModule.highlightColor =
              currentColor;
            break;
          case 'Underline':
            this.pdfViewer.annotationModule.textMarkupAnnotationModule.underlineColor =
              currentColor;
            break;
          case 'Strikethrough':
    ...
    if (this.pdfViewer.selectedItems.annotations.length === 1) {
      ...
      if (this.pdfViewer.annotation.shapeAnnotationModule) {
        switch (
          ...
          case 'Line':
            this.pdfViewer.annotation.shapeAnnotationModule.lineFillColor =
              currentColor;
            break;
          case 'Arrow':
            this.pdfViewer.annotation.shapeAnnotationModule.arrowFillColor =
              currentColor;
            break;
          case 'Rectangle':
            this.pdfViewer.annotation.shapeAnnotationModule.rectangleFillColor =
              currentColor;
            break;
          case 'Circle':
            this.pdfViewer.annotation.shapeAnnotationModule.circleFillColor =
              currentColor;
            break;
          case 'Polygon':
            this.pdfViewer.annotation.shapeAnnotationModule.polygonFillColor =
              currentColor;
            break;
        }
      }
      if (this.pdfViewer.drawingObject) {
        ...
        if (this.pdfViewer.drawingObject.shapeAnnotationType === 'FreeText') {
          this.pdfViewer.annotation.freeTextAnnotationModule.fillColor =
            currentColor;
        }
      }
    }
    if (isBlazor()) {
      this.colorDropDownElementInBlazor = this.pdfViewer.element.querySelector(
        '.e-pv-annotation-color-container'
      );
    ...
    if (this.pdfViewer.selectedItems.annotations.length === 1) {
      ...
      if (this.pdfViewer.annotation.shapeAnnotationModule) {
        switch (
          ...
          case 'Line':
            this.pdfViewer.annotation.shapeAnnotationModule.lineStrokeColor =
              currentColor;
            break;
          case 'Arrow':
            this.pdfViewer.annotation.shapeAnnotationModule.arrowStrokeColor =
              currentColor;
            break;
          case 'Rectangle':
            this.pdfViewer.annotation.shapeAnnotationModule.rectangleStrokeColor =
              currentColor;
            break;
          case 'Circle':
            this.pdfViewer.annotation.shapeAnnotationModule.circleStrokeColor =
              currentColor;
            break;
          case 'Polygon':
            this.pdfViewer.annotation.shapeAnnotationModule.polygonStrokeColor =
              currentColor;
            break;
        }
      }
      ...
      if (
        ...
        this.pdfViewer.annotation.freeTextAnnotationModule.borderColor =
          currentColor;
      }
    }
    if (isBlazor()) {
      this.strokeDropDownElementInBlazor = this.pdfViewer.element.querySelector(
        '.e-pv-annotation-stroke-container'
      );
    ...
    if (isBlazor()) {
      ...
      if (element) {
        if (element.childNodes[0] as HTMLElement) {
          (element.childNodes[0] as HTMLElement).style.borderBottomColor =
            color;
        }
      }
    }
  ...
  public updateTextPropertySelection(currentOption: string): void {
    if (currentOption === 'bold') {
      document
        .getElementById(this.pdfViewer.element.id + '_bold')
        .classList.toggle('textprop-option-active');
    } else if (currentOption === 'italic') {
      document
        .getElementById(this.pdfViewer.element.id + '_italic')
        .classList.toggle('textprop-option-active');
    } else if (currentOption === 'underline') {
      document
        .getElementById(this.pdfViewer.element.id + '_underline_textinput')
        .classList.toggle('textprop-option-active');
      document
        .getElementById(this.pdfViewer.element.id + '_strikeout')
        .classList.remove('textprop-option-active');
  ...
  public updateTextAlignInIcon(align: string): void {
    ...
    const leftAlign: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_left_align'
    );
    const rightAlign: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_right_align'
    );
    const centerAlign: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_center_align'
    );
    const justifyAlign: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_justify_align'
    );
    if (!isBlazor()) {
    ...
    document.getElementById(
      this.pdfViewer.element.id + '_annotation_textalign'
  ...
  private updateOpacityIndicator(): void {
    this.opacityIndicator.textContent =
      parseInt(Math.round(this.opacitySlider.value as number).toString(), 10) +
      '%';
  }
  ...
  private createSlider(idString: string): HTMLElement {
    ...
    const sliderElement: HTMLElement = createElement('div', {
      id: idString + '_slider',
    });
    ...
    if (!this.pdfViewer.enableRtl) {
      ...
      this.opacitySlider.element.parentElement.classList.add(
        'e-pv-annotation-opacity-slider-container'
      );
  ...
  private createThicknessSlider(idString: string): HTMLElement {
    ...
    this.thicknessSlider.element.parentElement.classList.add(
  ...
  private afterToolbarCreation(): void {
    ...
    this.deleteItem = this.primaryToolbar.addClassToolbarItem(
      '_annotation_delete',
      'e-pv-annotation-delete',
      this.pdfViewer.localeObj.getConstant('Delete') + ' (delete)'
    );
    this.freeTextEditItem = this.primaryToolbar.addClassToolbarItem(
      '_annotation_freeTextEdit',
      'e-pv-annotation-freeTextEdit',
    ...
    this.pdfViewerBase
      .getElement('_annotation_shapes')
      .setAttribute(
        'aria-label',
        this.pdfViewer.localeObj.getConstant('Add Shapes')
      );
    this.pdfViewerBase
      .getElement('_annotation_calibrate')
      .setAttribute(
        'aria-label',
        this.pdfViewer.localeObj.getConstant('Calibrate')
      );
    ...
    this.closeItem = this.primaryToolbar.addClassToolbarItem(
      '_annotation_close',
      'e-pv-annotation-tools-close',
      null
    );
    this.pdfViewerBase
      .getElement('_annotation_close')
      .setAttribute('aria-label', 'Close Annotation Toolbar');
    this.selectAnnotationDeleteItem(false);
  ...
  private onToolbarClicked(args: ClickEventArgs): void {
    ...
    const isKeyBoardEvent: boolean =
      args.originalEvent &&
      (args.originalEvent as any).pointerType !== 'mouse' &&
      (args.originalEvent as any).pointerType !== 'touch';
    ...
    switch ((args.originalEvent.target as HTMLElement).id) {
      ...
      case this.pdfViewer.element.id + '_annotation_commentPanelIcon': {
        ...
        if (
          ...
          this.pdfViewer.annotation.textMarkupAnnotationModule
        ) {
          this.pdfViewer.annotation.textMarkupAnnotationModule.showHideDropletDiv(
            true
          );
        }
        if (commentPanel.style.display === 'block') {
          ...
          if (
            isKeyBoardEvent &&
            !isNullOrUndefined(commentPanel.firstElementChild) &&
            !isNullOrUndefined(
              commentPanel.firstElementChild.lastElementChild
            ) &&
            commentPanel.firstElementChild.lastElementChild instanceof
              HTMLButtonElement
          ) {
      ...
      case this.pdfViewer.element.id + '_annotation_closeIcon': {
        this.inkAnnotationSelected = false;
        const commentsPanel: HTMLElement = document.getElementById(
          this.pdfViewer.element.id + '_commantPanel'
        );
      ...
      case this.pdfViewer.element.id + '_annotation_inkIcon':
        ...
        if (this.pdfViewer.annotationModule.inkAnnotationModule) {
          const currentPageNumber: string =
      ...
      case this.pdfViewer.element.id + '_comment': {
        ...
        const commentsButton: HTMLElement = document.getElementById(
          this.pdfViewer.element.id + '_comment'
        );
        this.deselectAllItemsForMobile();
  ...
  private addInkAnnotation(): void {
    ...
    if (this.pdfViewer.annotationModule.inkAnnotationModule) {
      const currentPageNumber: string =
        this.pdfViewer.annotationModule.inkAnnotationModule.currentPageNumber;
      if (currentPageNumber && currentPageNumber !== '') {
        this.pdfViewer.annotationModule.inkAnnotationModule.drawInkAnnotation(
          parseInt(currentPageNumber, 10)
        );
  ...
  public resetFreeTextAnnot(): void {
    if (
      ...
      this.pdfViewer.annotation.freeTextAnnotationModule.isNewFreeTextAnnot =
        false;
      this.pdfViewer.annotation.freeTextAnnotationModule.isNewAddedAnnot =
        false;
      if (!Browser.isDevice) {
  ...
  private handleFreeTextEditor(): void {
    ...
    if (annotation) {
      this.pdfViewer.fireAnnotationUnSelect(
        annotation.annotName,
        annotation.pageIndex,
        annotation
      );
      this.pdfViewer.fireSignatureUnselect(
        annotation.annotName,
        annotation.pageIndex,
        annotation
      );
    ...
    this.pdfViewer.clearSelection(this.pdfViewer.currentPageNumber - 1);
    if (this.pdfViewer.annotationModule.textMarkupAnnotationModule) {
      this.pdfViewer.annotationModule.textMarkupAnnotationModule.isTextMarkupAnnotationMode =
        false;
    }
    this.isStrikethroughEnabled = false;
    this.isHighlightEnabled = false;
    this.isUnderlineEnabled = false;
    const freeTextAnnotationModule: FreeTextAnnotation =
    ...
    this.updateColorInIcon(
      this.colorDropDownElement,
      this.pdfViewer.annotationModule.freeTextAnnotationModule.fillColor
    );
    this.updateColorInIcon(
      this.strokeDropDownElement,
      this.pdfViewer.annotationModule.freeTextAnnotationModule.borderColor
    );
    this.updateColorInIcon(
      this.fontColorElement,
      this.pdfViewer.annotationModule.freeTextAnnotationModule.fontColor
    );
    this.updateFontFamilyInIcon(
      this.pdfViewer.annotationModule.freeTextAnnotationModule.fontFamily
    );
    this.updateFontSizeInIcon(
      this.pdfViewer.annotationModule.freeTextAnnotationModule.fontSize
    );
    this.updateTextAlignInIcon(
      this.pdfViewer.annotationModule.freeTextAnnotationModule.textAlign
    );
  ...
  private updateFontFamilyIcon(fontFamily: string, isActive: boolean): void {
    const fontFamilyElement: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + fontFamily
    );
    if (isActive) {
  ...
  public showAnnotationToolbar(
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      if (!this.isToolbarHidden) {
        ...
        if (element) {
          ...
          if (this.pdfViewer.enableToolbar) {
            this.primaryToolbar.deSelectItem(
              this.primaryToolbar.annotationItem
            );
          }
        }
      ...
      if (
        ...
        this.pdfViewer.magnification.fitType === 'fitToPage'
      ) {
  ...
  private enablePropertiesTool(annotationModule: any): void {
    ...
    if (this.pdfViewerBase.isTextMarkupAnnotationModule()) {
      annotationModule.textMarkupAnnotationModule.isTextMarkupAnnotationMode =
        false;
    }
    this.primaryToolbar.deSelectItem(this.highlightItem);
    this.primaryToolbar.deSelectItem(this.underlineItem);
  ...
  private showSeparatorInMobile(): void {
    ...
    if (
      !this.isHighlightBtnVisible &&
      !this.isUnderlineBtnVisible &&
      !this.isStrikethroughBtnVisible
    ) {
  ...
  private showInkAnnotationTool(): void {
    if (
      this.pdfViewer.toolbarSettings.annotationToolbarItems.indexOf(
        'InkAnnotationTool'
      ) !== -1
    ) {
  ...
  private showSeparator(): void {
    ...
    if (
      !this.isFontFamilyToolVisible &&
      !this.isFontSizeToolVisible &&
      !this.isFontColorToolVisible &&
      !this.isFontAlignToolVisible &&
      !this.isFontStylesToolVisible
    ) {
      this.applyHideToToolbar(false, 21, 21);
    }
    if (
      (!this.isColorToolVisible &&
        !this.isStrokeColorToolVisible &&
        !this.isThicknessToolVisible &&
        !this.isOpacityToolVisible) ||
      !this.isDeleteAnnotationToolVisible
    ) {
  ...
  private applyHideToToolbar(
    ...
    for (let index: number = startIndex; index <= endIndex; index++) {
      let toolbar: Tool;
      const propertyToolbarElement: any =
        this.propertyToolbar && this.propertyToolbar.element
          ? this.propertyToolbar.element
          : null;
      const toolbarElement: any =
        this.toolbar && this.toolbar.element ? this.toolbar.element : null;
      if (
        toolbarElement &&
        toolbarElement.children &&
        toolbarElement.children.length > 0
      ) {
        toolbar = this.toolbar;
      } else if (
        Browser.isDevice &&
        propertyToolbarElement &&
        propertyToolbarElement.children &&
      ...
      if (toolbar && toolbar.items[parseInt(index.toString(), 10)]) {
        const className: string =
          toolbar.items[parseInt(index.toString(), 10)].cssClass;
        if (className && className !== '') {
          // Querying the toolbar item
          const element: Element = toolbar.element.querySelector(
            '.' + className
          );
          if (element) {
  ...
  public adjustViewer(isAdjust: boolean): void {
    ...
    if (isBlazor()) {
      splitterElement = this.pdfViewer.element.querySelector(
        '.e-pv-sidebar-toolbar-splitter'
      );
      toolbarContainer = this.pdfViewer.element.querySelector('.e-pv-toolbar');
      const annotationToolbarContainer: HTMLElement =
        this.pdfViewer.element.querySelector('.e-pv-annotation-toolbar');
      annotationToolbarHeight = this.getToolbarHeight(
        annotationToolbarContainer
      );
    } else {
      splitterElement = this.pdfViewerBase.getElement(
        '_sideBarToolbarSplitter'
      );
    ...
    const sideBarToolbar: HTMLElement =
      this.pdfViewerBase.navigationPane.sideBarToolbar;
    const sideBarContentContainer: HTMLElement =
      this.pdfViewerBase.navigationPane.sideBarContentContainer;
    const commentsContainer: HTMLElement =
      this.pdfViewerBase.navigationPane.commentPanelContainer;
    const commentPanelResizer: HTMLElement =
      this.pdfViewerBase.navigationPane.commentPanelResizer;
    let newToolbarHeight: string = '';
    if (isAdjust) {
      if (this.pdfViewer.enableToolbar) {
        if (!isNullOrUndefined(sideBarToolbar)) {
          sideBarToolbar.style.top =
            toolbarHeight + annotationToolbarHeight + 'px';
        }
        if (!isNullOrUndefined(sideBarContentContainer)) {
          sideBarContentContainer.style.top =
            toolbarHeight + annotationToolbarHeight + 'px';
        }
        ...
        if (!isNullOrUndefined(commentsContainer)) {
          commentsContainer.style.top =
            toolbarHeight + annotationToolbarHeight + 'px';
        }
        if (!isNullOrUndefined(commentPanelResizer)) {
          commentPanelResizer.style.top =
            toolbarHeight + annotationToolbarHeight + 'px';
        }
      } else {
      ...
      this.pdfViewerBase.viewerContainer.style.height =
        this.updateViewerHeight(
          this.getElementHeight(this.pdfViewerBase.viewerContainer),
          annotationToolbarHeight + toolbarHeight
        ) + 'px';
      newToolbarHeight = this.getNavigationToolbarHeight(
        annotationToolbarHeight + toolbarHeight
      );
      if (!isNullOrUndefined(sideBarToolbar)) {
      ...
      if (this.pdfViewerBase.viewerContainer.style.height === '0px') {
        this.pdfViewerBase.viewerContainer.style.height =
          parseInt(this.pdfViewer.element.style.height, 10) -
          parseInt(sideBarToolbar.style.top, 10) +
          'px';
      }
    }
  }
  ...
  private updateContentContainerHeight(
    ...
    const sideBarClientRect: ClientRect =
      this.pdfViewerBase.navigationPane.sideBarContentContainer.getBoundingClientRect();
    if (sideBarClientRect.height !== 0) {
      if (isAdjust) {
        this.pdfViewerBase.navigationPane.sideBarContentContainer.style.height =
          sideBarClientRect.height - annotationToolbarHeight + 'px';
      } else {
        this.pdfViewerBase.navigationPane.sideBarContentContainer.style.height =
          sideBarClientRect.height + annotationToolbarHeight + 'px';
      }
  ...
  private getToolbarHeight(element: HTMLElement): number {
    if (!isNullOrUndefined(element)) {
      ...
      if (
        toolbarHeight === 0 &&
        element === this.pdfViewerBase.getElement('_toolbarContainer')
      ) {
  ...
  private getNavigationToolbarHeight(toolbarHeight: number): string {
    const height: number =
      this.pdfViewer.element.getBoundingClientRect().height;
  ...
  public deselectAllItemsInBlazor(): void {
    if (this.pdfViewerBase.isTextMarkupAnnotationModule()) {
      this.pdfViewer.annotationModule.textMarkupAnnotationModule.isTextMarkupAnnotationMode =
        false;
      this.pdfViewer.annotationModule.textMarkupAnnotationModule.showHideDropletDiv(
        true
      );
    }
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      ...
      this.primaryToolbar.deSelectItem(this.FreeTextElement);
      this.primaryToolbar.deSelectItem(this.InkAnnotationElement);
      this.pdfViewer._dotnetInstance.invokeMethodAsync(
        'UpdateTextMarkupButtons',
        false,
        false,
        false
      );
    ...
    if (this.pdfViewer.annotationModule) {
      this.pdfViewer.annotationModule.freeTextAnnotationModule.isNewFreeTextAnnot =
        false;
    }
  }
  ...
  public deselectAllItems(): void {
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      if (isBlazorPlatform) {
        this.primaryToolbar.deSelectItem(
          this.pdfViewer.toolbar.SelectToolElement
        );
   ...
   * @returns {void}
   */
  public selectAnnotationDeleteItem(
    isEnable: boolean,
    ...
    if (!isBlazor() && !Browser.isDevice) {
      if (this.toolbar) {
        if (isEnable) {
          const annotation: any =
            this.pdfViewer.annotationModule.findCurrentAnnotation();
          if (annotation) {
            if (
              annotation.annotationSettings &&
              annotation.annotationSettings.isLock
            ) {
              if (
                this.pdfViewer.annotationModule.checkAllowedInteractions(
                  'Delete',
                  annotation
                )
              ) {
  ...
  public enableTextMarkupAnnotationPropertiesTools(isEnable: boolean): void {
    if (!Browser.isDevice) {
      if (!isBlazor()) {
        ...
        this.pdfViewerBase.blazorUIAdaptor.enableTextMarkupAnnotationPropertiesTools(
          isEnable
        );
      }
    }
  ...
  public enableAnnotationPropertiesTools(isEnable: boolean): void {
    if (!Browser.isDevice) {
      ...
      if (!isBlazor()) {
        ...
        this.pdfViewerBase.blazorUIAdaptor.enableAnnotationPropertiesTool(
          isEnable,
          isPropertiesChanges
        );
      }
  ...
  public enableSignaturePropertiesTools(isEnable: boolean): void {
    if (!Browser.isDevice) {
      ...
      if (!isBlazor()) {
        ...
        this.pdfViewerBase.blazorUIAdaptor.enableSignaturePropertiesTools(
          isEnable,
          isPropertiesChanges
        );
      }
  ...
  public enableStampAnnotationPropertiesTools(isEnable: boolean): void {
    ...
    if (!isBlazor()) {
      ...
      this.pdfViewerBase.blazorUIAdaptor.enableStampAnnotationPropertiesTools(
        isEnable,
        isPropertiesChanges
      );
    }
  ...
  public enableFreeTextAnnotationPropertiesTools(isEnable: boolean): void {
    ...
    if (!isBlazor()) {
      ...
      this.pdfViewerBase.blazorUIAdaptor.enableFreeTextAnnotationPropertiesTools(
        isEnable,
        isPropertiesChanges
      );
    }
  ...
  public enableAnnotationAddTools(isEnable: boolean): void {
    if (this.toolbar && !Browser.isDevice) {
      ...
      if (
        this.pdfViewer.enableMeasureAnnotation &&
        this.pdfViewerBase.isCalibrateAnnotationModule()
      ) {
  ...
  public isAnnotationButtonsEnabled(): boolean {
    ...
    if (
      ...
      this.isUnderlineEnabled ||
      this.isStrikethroughEnabled
    ) {
  ...
  private updateToolbarItems(): void {
    ...
    this.enableItems(
      this.shapeElement.parentElement,
      this.pdfViewer.enableShapeAnnotation
    );
    this.toolbar.enableItems(
      this.stampElement.parentElement,
      this.pdfViewer.enableStampAnnotations
    );
    this.enableItems(
      this.calibrateElement.parentElement,
      this.pdfViewer.enableMeasureAnnotation
    );
    this.enableItems(
      this.freeTextEditItem.parentElement,
      this.pdfViewer.enableFreeText
    );
    this.enableItems(
      this.handWrittenSignatureItem.parentElement,
      this.pdfViewer.enableHandwrittenSignature
    );
    this.enableItems(
      this.inkAnnotationItem.parentElement,
      this.pdfViewer.enableInkAnnotation
    );
  ...
  public updateAnnnotationPropertyItems(): void {
    if (!isBlazor()) {
      if (this.pdfViewer.selectedItems.annotations.length === 1) {
        this.pdfViewer.toolbar.annotationToolbarModule.updateColorInIcon(
          this.colorDropDownElement,
          this.getColorHexValue(
            this.pdfViewer.selectedItems.annotations[0].wrapper.children[0]
              .style.fill,
            'fillColor'
          )
        );
        ...
        if (
          ...
          this.pdfViewer.toolbar.annotationToolbarModule.updateFontFamilyInIcon(
            this.pdfViewer.selectedItems.annotations[0].fontFamily
          );
          this.pdfViewer.toolbar.annotationToolbarModule.updateFontSizeInIcon(
            this.pdfViewer.selectedItems.annotations[0].fontSize
          );
          this.pdfViewer.toolbar.annotationToolbarModule.updateTextAlignInIcon(
            this.pdfViewer.selectedItems.annotations[0].textAlign
          );
        }
      } else {
      ...
      if (this.pdfViewer.selectedItems.annotations.length === 1) {
        this.pdfViewer.toolbar.annotationToolbarModule.updateColorInIcon(
          this.colorDropDownElementInBlazor,
          this.getColorHexValue(
            this.pdfViewer.selectedItems.annotations[0].wrapper.children[0]
              .style.fill,
            'fillColor'
        ...
        this.pdfViewer.toolbar.annotationToolbarModule.updateColorInIcon(
          this.strokeDropDownElementInBlazor,
          this.getColorHexValue(
            this.pdfViewer.selectedItems.annotations[0].wrapper.children[0]
              .style.strokeColor,
            'strokeColor'
        ...
        if (
          ...
          this.pdfViewerBase.blazorUIAdaptor.updateFontFamilyInIcon(
            this.pdfViewer.selectedItems.annotations[0].fontFamily
          );
          ...
          this.pdfViewerBase.blazorUIAdaptor.updateFontSizeInIcon(
            this.pdfViewer.selectedItems.annotations[0].fontSize
          );
  ...
  public resetToolbar(): void {
    this.updateToolbarItems();
    if (
      (this.pdfViewer.isAnnotationToolbarOpen ||
        this.pdfViewer.isAnnotationToolbarVisible) &&
  ...
  private destroyComponent(): void {
    const componentElement: HTMLElement[] = [
      this.highlightItem,
      this.underlineItem,
      this.strikethroughItem,
      this.lineElement,
      this.arrowElement,
      this.rectangleElement,
      this.circleElement,
      this.polygonElement,
      this.calibrateDistance,
      this.calibrateArea,
      this.calibrateRadius,
      this.calibrateVolume,
      this.calibratePerimeter,
      this.freeTextEditItem,
      this.stampElement,
      this.handWrittenSignatureItem,
      this.inkAnnotationItem,
      this.fontFamilyElement,
      this.fontSizeElement,
      this.alignLeftElement,
      this.alignRightElement,
      this.alignCenterElement,
      this.alignJustifyElement,
      this.boldElement,
      this.italicElement,
      this.fontStyleStrikethroughItem,
      this.fontStyleUnderlineItem,
      this.deleteItem,
      this.commentItem,
      this.shapeDropDown ? this.shapeDropDown.activeElem[0] : null,
      this.calibrateDropDown ? this.calibrateDropDown.activeElem[0] : null,
      this.fontColorDropDown ? this.fontColorDropDown.activeElem[0] : null,
      this.textAlignDropDown ? this.textAlignDropDown.activeElem[0] : null,
      this.colorDropDown ? this.colorDropDown.activeElem[0] : null,
      this.strokeDropDown ? this.strokeDropDown.activeElem[0] : null,
      this.thicknessDropDown ? this.thicknessDropDown.activeElem[0] : null,
      this.opacityDropDown ? this.opacityDropDown.activeElem[0] : null,
      this.textPropertiesDropDown
        ? this.textPropertiesDropDown.activeElem[0]
        : null,
    ];
    for (let i: number = 0; i < componentElement.length; i++) {
      if (componentElement[parseInt(i.toString(), 10)]) {
        this.destroyDependentComponent(
          componentElement[parseInt(i.toString(), 10)]
        );
      }
    }
  }
  ...
  public afterAnnotationToolbarCreationInBlazor(): void {
    this.HighlightElement = document.getElementById(
      this.pdfViewer.element.id + '_highLight'
    ).children[0];
    this.UnderlineElement = document.getElementById(
      this.pdfViewer.element.id + '_underline'
    ).children[0];
    this.StrikethroughElement = document.getElementById(
      this.pdfViewer.element.id + '_strikethrough'
    ).children[0];
    this.InkAnnotationElement = document.getElementById(
      this.pdfViewer.element.id + '_annotation_ink'
    ).children[0];
    this.InkAnnotationElement.classList.add('e-pv-tbar-btn');
    this.FreeTextElement = document.getElementById(
      this.pdfViewer.element.id + '_annotation_freeTextEdit'
    ).children[0];
    this.HighlightElement = this.addClassToToolbarInBlazor(
      this.HighlightElement,
      'e-pv-highlight',
      '_highLight'
    );
    this.UnderlineElement = this.addClassToToolbarInBlazor(
      this.UnderlineElement,
      'e-pv-underline',
      '_underline'
    );
    this.StrikethroughElement = this.addClassToToolbarInBlazor(
      this.StrikethroughElement,
      'e-pv-strikethrough',
      '_strikethrough'
    );
  }
  ...
  private AnnotationSliderOpened(): void {
    if (
      ...
      if (
        this.pdfViewer.selectedItems.annotations[0].wrapper &&
        this.pdfViewer.selectedItems.annotations[0].wrapper.children[0]
      ) {
  ...
  private DropDownOpened(colorElement: any): void {
    if (colorElement && colorElement[0].element) {
      const colorElementBounds: any =
        colorElement[0].element.getBoundingClientRect();
      const sidebarElement: HTMLElement =
        this.pdfViewerBase.navigationPane.sideBarToolbar;
      const sideBarElementWidth: number = sidebarElement
        ? sidebarElement.getBoundingClientRect().width
        : 0;
      if (
        colorElementBounds.left >
        this.pdfViewerBase.viewerContainer.clientWidth +
          colorElementBounds.width +
          sideBarElementWidth
      ) {
        colorElement[0].element.style.left =
          colorElementBounds.left -
          this.pdfViewerBase.viewerContainer.clientHeight / 2 +
          'px';
      }
    }
  }
