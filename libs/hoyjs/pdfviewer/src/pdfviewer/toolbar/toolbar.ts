...
import { Toolbar as tool, ClickEventArgs } from '@hoyui/navigations';
import { NumericTextBox } from '@hoyui/inputs';
import { ComboBox, ChangeEventArgs } from '@hoyui/dropdowns';
import {
  PdfViewer,
  PdfViewerBase,
  AnnotationToolbar,
...
import { Tooltip, TooltipEventArgs } from '@hoyui/popups';
import {
  DropDownButton,
  ItemModel,
...
export class Toolbar {
  ...
  public intializeToolbar(width: string): HTMLElement {
    ...
    if (isIE) {
      if (isBlazor()) {
        this.pdfViewerBase.blazorUIAdaptor.totalPageElement.classList.add(
          'e-pv-total-page-ms'
        );
      } else {
        if (!Browser.isDevice) {
    ...
    if (!isBlazor()) {
      ...
      if (this.pdfViewer.annotationModule) {
        this.annotationToolbarModule = new AnnotationToolbar(
          this.pdfViewer,
          this.pdfViewerBase,
          this
        );
      ...
      if (this.pdfViewer.formDesignerModule) {
        this.formDesignerToolbarModule = new FormDesignerToolbar(
          this.pdfViewer,
          this.pdfViewerBase,
          this
        );
      ...
      this.PanElement = document.getElementById(
        this.pdfViewer.element.id + '_handTool'
      ).children[0];
      this.PanElement.classList.add('e-pv-tbar-btn');
      this.SelectToolElement = document.getElementById(
        this.pdfViewer.element.id + '_selectTool'
  ...
  private bindOpenIconEvent(): void {
    const openElement: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_open'
    );
    if (openElement) {
  ...
  private InitializeMobileToolbarInBlazor(): void {
    const toolbarDiv: HTMLElement = this.pdfViewer.element.querySelector(
      '.e-pv-mobile-toolbar'
    );
    this.createFileElement(toolbarDiv);
    this.wireEvent();
  ...
  public showToolbar(enableToolbar: boolean): void {
    ...
    if (enableToolbar) {
      ...
      const toolbarContainer: HTMLElement =
        this.pdfViewerBase.getElement('_toolbarContainer');
      if (toolbarContainer) {
        let toolbarHeight: number = toolbarContainer.clientHeight;
        if (toolbarHeight === 0) {
          toolbarHeight =
            parseFloat(window.getComputedStyle(toolbarContainer)['height']) + 1;
        }
      ...
      if (enableToolbar) {
        if (
          Browser.isDevice &&
          this.pdfViewer.toolbarModule.annotationToolbarModule.toolbar
        ) {
          this.annotationToolbarModule.toolbarCreated = false;
          this.annotationToolbarModule.adjustMobileViewer();
          this.pdfViewer.toolbarModule.annotationToolbarModule.toolbar.element.style.display =
            'none';
        }
        if (Browser.isDevice && this.annotationToolbarModule.propertyToolbar) {
          this.annotationToolbarModule.propertyToolbar.element.style.display =
            'none';
        }
      }
  ...
  public showNavigationToolbar(enableNavigationToolbar: boolean): void {
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      const navigationToolbar: HTMLElement =
        this.pdfViewerBase.navigationPane.sideBarToolbar;
      const navigationToolbarSplitter: HTMLElement =
        this.pdfViewerBase.navigationPane.sideBarToolbarSplitter;
      if (enableNavigationToolbar) {
        ...
        if (
          this.pdfViewerBase.navigationPane.isBookmarkOpen ||
          this.pdfViewerBase.navigationPane.isThumbnailOpen
        ) {
   ...
   * @returns {void}
   */
  public showToolbarItem(
    items: (CustomToolbarItemModel | ToolbarItem)[],
    isVisible: boolean
  ): void {
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      this.applyHideToToolbar(
        true,
        this.itemsIndexArray[0].endIndex + 1,
        this.itemsIndexArray[0].endIndex + 1
      );
      this.applyHideToToolbar(
        true,
        this.itemsIndexArray[1].endIndex + 1,
        this.itemsIndexArray[1].endIndex + 1
      );
      this.applyHideToToolbar(
        true,
        this.itemsIndexArray[2].endIndex + 1,
        this.itemsIndexArray[2].endIndex + 1
      );
      this.applyHideToToolbar(
        true,
        this.itemsIndexArray[4].endIndex + 1,
        this.itemsIndexArray[4].endIndex + 1
      );
      this.applyHideToToolbar(
        true,
        this.itemsIndexArray[5].endIndex + 1,
        this.itemsIndexArray[5].endIndex + 1
      );
    } else {
  ...
  public DisableToolbarItems(restrictionSummary: any, isEnable: boolean): void {
    switch (restrictionSummary) {
      ...
      case 'EditAnnotations':
        this.enableAnnotationEditTool(isEnable);
        if (
          this.annotationToolbarModule &&
          !this.annotationToolbarModule.isToolbarHidden
        ) {
          this.annotationToolbarModule.showAnnotationToolbar();
        }
        break;
  ...
  private showOpenOption(enableOpenOption: boolean): void {
    ...
    this.applyHideToToolbar(
      enableOpenOption,
      !isNullOrUndefined(this.itemsIndexArray[0])
        ? this.itemsIndexArray[0].startIndex
        : 0,
      !isNullOrUndefined(this.itemsIndexArray[0])
        ? this.itemsIndexArray[0].endIndex
        : 0
    );
  }
  ...
  private showPageNavigationTool(enablePageNavigationTool: boolean): void {
    ...
    this.applyHideToToolbar(
      enablePageNavigationTool,
      !isNullOrUndefined(this.itemsIndexArray[1])
        ? this.itemsIndexArray[1].startIndex
        : 2,
      !isNullOrUndefined(this.itemsIndexArray[1])
        ? this.itemsIndexArray[1].endIndex
        : 7
    );
  }
  ...
  private showMagnificationTool(enableMagnificationTool: boolean): void {
    ...
    this.applyHideToToolbar(
      enableMagnificationTool,
      !isNullOrUndefined(this.itemsIndexArray[2])
        ? this.itemsIndexArray[2].startIndex
        : 9,
      !isNullOrUndefined(this.itemsIndexArray[2])
        ? this.itemsIndexArray[2].endIndex
        : 11
    );
  }
  ...
  private showSelectionTool(enableSelectionTool: boolean): void {
    ...
    this.applyHideToToolbar(
      enableSelectionTool,
      !isNullOrUndefined(this.itemsIndexArray[3])
        ? this.itemsIndexArray[3].startIndex
        : 13,
      !isNullOrUndefined(this.itemsIndexArray[3])
        ? this.itemsIndexArray[3].endIndex
        : 13
    );
  }
  ...
  private showScrollingTool(enableScrollingTool: boolean): void {
    ...
    this.applyHideToToolbar(
      enableScrollingTool,
      !isNullOrUndefined(this.itemsIndexArray[4])
        ? this.itemsIndexArray[4].startIndex
        : 14,
      !isNullOrUndefined(this.itemsIndexArray[4])
        ? this.itemsIndexArray[4].endIndex
        : 14
    );
  }
  ...
  private showDownloadOption(enableDownloadOption: boolean): void {
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      ...
      this.applyHideToToolbar(
        ...
        !isNullOrUndefined(this.itemsIndexArray[12])
          ? this.itemsIndexArray[12].startIndex
          : 26,
        !isNullOrUndefined(this.itemsIndexArray[12])
          ? this.itemsIndexArray[12].endIndex
          : 26
      );
    } else {
  ...
  private showPrintOption(enablePrintOption: boolean): void {
    ...
    this.applyHideToToolbar(
      enablePrintOption,
      !isNullOrUndefined(this.itemsIndexArray[11])
        ? this.itemsIndexArray[11].startIndex
        : 25,
      !isNullOrUndefined(this.itemsIndexArray[11])
        ? this.itemsIndexArray[11].endIndex
        : 25
    );
  }
  ...
  private showSearchOption(enableSearchOption: boolean): void {
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      ...
      this.applyHideToToolbar(
        enableSearchOption,
        !isNullOrUndefined(this.itemsIndexArray[8])
          ? this.itemsIndexArray[8].startIndex
          : 22,
        !isNullOrUndefined(this.itemsIndexArray[8])
          ? this.itemsIndexArray[8].endIndex
          : 22
      );
    } else {
  ...
  private showUndoRedoTool(isEnable: boolean): void {
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      ...
      this.applyHideToToolbar(
        isEnable,
        !isNullOrUndefined(this.itemsIndexArray[5])
          ? this.itemsIndexArray[5].startIndex
          : 16,
        !isNullOrUndefined(this.itemsIndexArray[5])
          ? this.itemsIndexArray[5].endIndex
          : 17
      );
    } else {
  ...
  private showCommentOption(isEnable: boolean): void {
    if (!this.pdfViewer.enableStickyNotesAnnotation) {
      ...
      this.applyHideToToolbar(
        this.pdfViewer.enableStickyNotesAnnotation,
        !isNullOrUndefined(this.itemsIndexArray[6])
          ? this.itemsIndexArray[6].startIndex
          : 18,
        !isNullOrUndefined(this.itemsIndexArray[6])
          ? this.itemsIndexArray[6].endIndex
          : 19
      );
    } else {
      this.isCommentBtnVisible = isEnable;
  ...
  private showAnnotationEditTool(isEnable: boolean): void {
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      ...
      this.applyHideToToolbar(
        isEnable,
        !isNullOrUndefined(this.itemsIndexArray[9])
          ? this.itemsIndexArray[9].startIndex
          : 23,
        !isNullOrUndefined(this.itemsIndexArray[9])
          ? this.itemsIndexArray[9].endIndex
          : 23
      );
    } else {
  ...
  private showFormDesignerEditTool(isEnable: boolean): void {
    ...
    this.applyHideToToolbar(
      isEnable,
      !isNullOrUndefined(this.itemsIndexArray[10])
        ? this.itemsIndexArray[10].startIndex
        : 24,
      !isNullOrUndefined(this.itemsIndexArray[10])
        ? this.itemsIndexArray[10].endIndex
        : 24
    );
  }
  ...
  private showSubmitForm(isEnable: boolean): void {
    ...
    this.applyHideToToolbar(
      isEnable,
      !isNullOrUndefined(this.itemsIndexArray[7])
        ? this.itemsIndexArray[7].startIndex
        : 20,
      !isNullOrUndefined(this.itemsIndexArray[7])
        ? this.itemsIndexArray[7].endIndex
        : 21
    );
  }
  ...
  private enablePageNavigationTool(enablePageNavigationTool: boolean): void {
    this.enableItems(
      this.firstPageItem.parentElement,
      enablePageNavigationTool
    );
    this.enableItems(
      this.previousPageItem.parentElement,
      enablePageNavigationTool
    );
  ...
  private enableOrganizePagesButton(isEnable: boolean): void {
    if (
      !isNullOrUndefined(this.organizePageItem) &&
      !isNullOrUndefined(this.organizePageItem.parentElement) &&
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      ...
      if (this.annotationToolbarModule) {
        ...
        if (this.annotationToolbarModule) {
          ...
          const commentsContainer: HTMLElement = document.getElementById(
            this.pdfViewer.element.id + '_commentscontentcontainer'
          );
      ...
      if (this.toolbar) {
        if (this.pdfViewerBase.pageCount === 0) {
          ...
          this.toolbar.enableItems(
            this.pdfViewerBase.getElement('_currentPageInputContainer'),
            false
          );
          this.enableItems(
            this.pdfViewerBase.getElement('_zoomDropDownContainer'),
            false
          );
          ...
          const obj: HTMLElement = this.pdfViewerBase.getElement(
            '_currentPageInputContainer'
          );
          if (obj) {
            ...
            if (
              ...
              this.pdfViewer.enableFormDesigner
            ) {
              this.enableItems(this.formDesignerItem.parentElement, true);
            }
            if (
              this.pdfViewer.textSearchModule &&
        ...
        if (this.pdfViewer.toolbarSettings.annotationToolbarItems) {
          if (
            this.pdfViewer.toolbarSettings.annotationToolbarItems.length ===
              0 ||
            !this.pdfViewer.annotationModule ||
            !this.pdfViewer.enableAnnotationToolbar
          ) {
        ...
        if (this.pdfViewer.toolbarSettings.formDesignerToolbarItems) {
          if (
            this.pdfViewer.toolbarSettings.formDesignerToolbarItems.length ===
              0 ||
            !this.pdfViewer.formDesignerModule ||
            !this.pdfViewer.enableFormDesignerToolbar
          ) {
  ...
  public updateNavigationButtons(): void {
    if (this.pdfViewer.navigationModule && !this.isPageNavigationToolDisabled) {
      if (
        this.pdfViewerBase.pageCount === 0 ||
        ...
        this.pdfViewerBase.currentPageNumber === this.pdfViewerBase.pageCount &&
        this.pdfViewerBase.pageCount > 0
      ) {
  ...
  public updateZoomButtons(): void {
    if (
      this.pdfViewer.magnificationModule &&
      !this.isMagnificationToolDisabled &&
  ...
  public updateUndoRedoButtons(): void {
    if (this.pdfViewer.annotationModule) {
      if (this.pdfViewerBase.pageCount > 0) {
        if (isBlazor()) {
          this.enableCollectionAvailableInBlazor(
            this.pdfViewer.annotationModule.actionCollection,
            'undo'
          );
          this.enableCollectionAvailableInBlazor(
            this.pdfViewer.annotationModule.redoCollection,
            'redo'
          );
        } else {
          if (
            !isNullOrUndefined(this.undoItem) &&
            !isNullOrUndefined(this.undoItem.parentElement)
          ) {
            this.enableCollectionAvailable(
              this.pdfViewer.annotationModule.actionCollection,
              this.undoItem.parentElement
            );
          }
          if (
            !isNullOrUndefined(this.redoItem) &&
            !isNullOrUndefined(this.redoItem.parentElement)
          ) {
            this.enableCollectionAvailable(
              this.pdfViewer.annotationModule.redoCollection,
              this.redoItem.parentElement
            );
          }
        }
      } else {
  ...
  public updateCurrentPage(pageIndex: number): void {
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      if (!isBlazor()) {
        if (!isNullOrUndefined(this.currentPageBox)) {
          if (this.currentPageBox.value === pageIndex) {
            (this.currentPageBoxElement as HTMLInputElement).value =
              pageIndex.toString();
          }
      ...
      this.pdfViewerBase.mobilecurrentPageContainer.innerHTML =
        pageIndex.toString();
    }
  }
  ...
  public updateTotalPage(): void {
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      ...
      if (!isNullOrUndefined(this.totalPageItem)) {
        this.totalPageItem.textContent =
          this.pdfViewer.localeObj.getConstant('of') +
          this.pdfViewerBase.pageCount.toString();
      }
    }
  }
  ...
  private createCustomItem(startIndex: number): any {
    for (
      ...
      j < this.pdfViewer.toolbarSettings.toolbarItems.length;
      j++
    ) {
      if (
        typeof this.pdfViewer.toolbarSettings.toolbarItems[
          parseInt(j.toString(), 10)
        ] === 'object'
      ) {
        const customToolbarItem: CustomToolbarItemModel = this.pdfViewer
          .toolbarSettings.toolbarItems[
          parseInt(j.toString(), 10)
        ] as CustomToolbarItemModel;
        if (
          !isNullOrUndefined(customToolbarItem.prefixIcon) &&
          customToolbarItem.prefixIcon !== ''
        ) {
        ...
        if (
          (isNullOrUndefined(customToolbarItem.prefixIcon) ||
            customToolbarItem.prefixIcon === '') &&
          isNullOrUndefined(customToolbarItem.template)
        ) {
          const text: string = !isNullOrUndefined(customToolbarItem.text)
            ? customToolbarItem.text
            : 'Custom Toolbar Item';
          customToolbarItem.template =
            '<button id="Custom" class="e-tbar-btn" style="font-size:14px"><span>' +
            text +
            '</span></button>';
        }
        this.toolItems.push(customToolbarItem);
        if (
          isNullOrUndefined(customToolbarItem.align) ||
          customToolbarItem.align === 'left' ||
          customToolbarItem.align === 'Left'
        ) {
  ...
  private createToolbarItems(): any {
    const currentPageInputTemplate: string =
      this.createCurrentPageInputTemplate();
    ...
    const submitButton: string =
      '<button id="' +
      this.pdfViewer.element.id +
      '_submitForm" class="e-tbar-btn" style="font-size:15px"><span id="' +
      this.pdfViewer.element.id +
      '_submitFormSpan" class="e-tbar-btn-text e-pv-submitform-text">' +
      this.pdfViewer.localeObj.getConstant('SubmitForm') +
      '</span></button>';
    const defaultToolbarOrder: any = [
      'OpenOption',
      'PageNavigationTool',
      'MagnificationTool',
      'SelectionTool',
    ...
    for (let i: number = 0; i < defaultToolbarOrder.length; i++) {
      ...
      for (
        let k: number = 0;
        k < this.pdfViewer.toolbarSettings.toolbarItems.length;
        k++
      ) {
        if (
          defaultToolbarOrder[parseInt(i.toString(), 10)] ===
          this.pdfViewer.toolbarSettings.toolbarItems[
            parseInt(k.toString(), 10)
          ]
        ) {
          if (
            typeof this.pdfViewer.toolbarSettings.toolbarItems[k + 1] ===
            'object'
          ) {
  ...
  private afterToolbarCreationInMobile(): void {
    ...
    this.itemsContainer.id =
      this.pdfViewer.element.id + '_toolbarItemsContainer';
    this.openDocumentItem = this.addClassToolbarItem(
      '_open',
      'e-pv-open-document',
      this.pdfViewer.localeObj.getConstant('Open')
    );
    this.undoItem = this.addClassToolbarItem(
      '_undo',
      'e-pv-undo',
      this.pdfViewer.localeObj.getConstant('Undo')
    );
    this.redoItem = this.addClassToolbarItem(
      '_redo',
      'e-pv-redo',
      this.pdfViewer.localeObj.getConstant('Redo')
    );
    this.annotationItem = this.addClassToolbarItem(
      '_annotation',
      'e-pv-annotation',
      this.pdfViewer.localeObj.getConstant('Annotation')
    );
    this.textSearchItem = this.addClassToolbarItem(
      '_search',
      'e-pv-text-search',
      this.pdfViewer.localeObj.getConstant('Text Search')
    );
  }
  ...
  private afterToolbarCreation(): void {
    ...
    if (!this.pdfViewer.enableRtl) {
      this.firstPageItem = this.addClassToolbarItem(
        '_firstPage',
        'e-pv-first-page-navigation',
        this.pdfViewer.localeObj.getConstant('Go To First Page') +
          (isMac ? ' (⌘+← or ⌘+↑)' : ' (Ctrl+← or Ctrl+↑)')
      );
      this.previousPageItem = this.addClassToolbarItem(
        '_previousPage',
        'e-pv-previous-page-navigation',
        this.pdfViewer.localeObj.getConstant('Previous Page') + ' (←)'
      );
      this.nextPageItem = this.addClassToolbarItem(
        '_nextPage',
        'e-pv-next-page-navigation',
        this.pdfViewer.localeObj.getConstant('Next Page') + ' (→)'
      );
      this.lastPageItem = this.addClassToolbarItem(
        '_lastPage',
        'e-pv-last-page-navigation',
        this.pdfViewer.localeObj.getConstant('Go To Last Page') +
          (isMac ? ' (⌘+→ or ⌘+↓)' : ' (Ctrl+→ or Ctrl+↓)')
      );
    } else {
      this.firstPageItem = this.addClassToolbarItem(
    ...
    this.zoomOutItem = this.addClassToolbarItem(
      '_zoomOut',
      'e-pv-zoom-out',
      this.pdfViewer.localeObj.getConstant('Zoom Out') +
        (isMac ? ' (⌘+Minus)' : ' (Ctrl+Minus)')
    );
    this.zoomInItem = this.addClassToolbarItem(
      '_zoomIn',
      'e-pv-zoom-in',
      this.pdfViewer.localeObj.getConstant('Zoom In') +
        (isMac ? ' (⌘+Plus)' : ' (Ctrl+Plus)')
    );
    this.textSelectItem = this.addClassToolbarItem(
      '_selectTool',
      'e-pv-text-select-tool',
      this.pdfViewer.localeObj.getConstant('Text Selection') +
        (isMac ? ' (⇧+V)' : ' (Shift+V)')
    );
    this.panItem = this.addClassToolbarItem(
      '_handTool',
      'e-pv-pan-tool',
      this.pdfViewer.localeObj.getConstant('Panning') +
        (isMac ? ' (⇧+H)' : ' (Shift+H)')
    );
    this.commentItem = this.addClassToolbarItem(
      '_comment',
      'e-pv-comment',
    ...
    this.textSearchItem.setAttribute(
      'aria-label',
      this.pdfViewer.localeObj.getConstant('Search text')
    );
    this.annotationItem = this.addClassToolbarItem(
      '_annotation',
      'e-pv-annotation',
    ...
    this.annotationItem.setAttribute(
      'aria-label',
      this.pdfViewer.localeObj.getConstant('Annotation Edit text')
    );
    this.formDesignerItem = this.addClassToolbarItem(
      '_formdesigner',
      'e-pv-formdesigner',
      this.pdfViewer.localeObj.getConstant('FormDesigner')
    );
    this.printItem = this.addClassToolbarItem(
      '_print',
      'e-pv-print-document',
      this.pdfViewer.localeObj.getConstant('Print') +
        (isMac ? ' (⌘+P)' : ' (Ctrl+P)')
    );
    this.downloadItem = this.addClassToolbarItem(
      '_download',
      'e-pv-download-document',
      this.pdfViewer.localeObj.getConstant('Download file') +
        (isMac ? ' (⌘+S)' : ' (Ctrl+S)')
    );
    this.zoomDropdownItem = this.pdfViewerBase.getElement('_zoomDropDown');
    this.createTooltip(
      this.zoomDropdownItem,
      this.pdfViewer.localeObj.getConstant('Zoom')
    );
    this.zoomDropdownItem.setAttribute(
      'aria-label',
      this.pdfViewer.localeObj.getConstant('Zoom')
    );
    this.addPropertiesToolItemContainer(
      this.zoomDropdownItem.parentElement.parentElement,
      null,
      '_zoomDropDownContainer'
    );
    this.createTooltip(
      this.currentPageBoxElement,
      this.pdfViewer.localeObj.getConstant('Page Number') +
        (isMac ? ' (⌘+G)' : ' (Ctrl+G)')
    );
    this.currentPageBoxElement.setAttribute(
      'aria-label',
      this.pdfViewer.localeObj.getConstant('Page Number')
    );
    this.submitItem = this.pdfViewerBase.getElement('_submitForm');
    this.addPropertiesToolItemContainer(
      this.submitItem.parentElement,
      'e-pv-submit',
      '_submitFormContainer'
    );
    this.createTooltip(
      this.submitItem,
      this.pdfViewer.localeObj.getConstant('SubmitForm')
    );
    this.submitItem.setAttribute(
      'aria-label',
      this.pdfViewer.localeObj.getConstant('SubmitForm')
    );
    this.addPropertiesToolItemContainer(
      this.currentPageBoxElement.parentElement.parentElement,
      'e-pv-current-page-container',
      '_currentPageInputContainer'
    );
    this.pdfViewerBase.getElement('_currentPageInputContainer').style.minWidth =
      '20px';
    this.totalPageItem = this.pdfViewerBase.getElement('_totalPage');
    this.addPropertiesToolItemContainer(
      this.totalPageItem.parentElement,
  ...
  public addClassToolbarItem(
    ...
    element.parentElement.id =
      this.pdfViewer.element.id + idString + 'Container';
  ...
  private createZoomDropdownElement(): string {
    const zoomDropdownElement: HTMLElement = this.createToolbarItem(
      'input',
  ...
  private createZoomDropdown(): void {
    ...
    if (minZoom != null || maxZoom != null) {
      const isWithinRange: (zoom: number) => boolean = (
        zoom: number
      ): boolean => {
        return (
      ...
      if (
        minZoom != null &&
        !items.some((item: any) => parseInt(item.id, 10) === minZoom)
      ) {
      ...
      if (
        maxZoom != null &&
        !items.some((item: any) => parseInt(item.id, 10) === maxZoom) &&
        maxZoom !== minZoom
      ) {
    ...
    if (!proxy.pdfViewer.enableRtl) {
      proxy.zoomDropDown = new ComboBox({
        ...
        select: function (args: any): void {
          if (
            args.e.type === 'keydown' &&
            args.itemData.percent !== proxy.zoomDropDown.element.value
          ) {
    ...
    proxy.zoomDropDown.appendTo(
      proxy.pdfViewerBase.getElement('_zoomDropDown')
    );
  }

  private createCurrentPageInputTemplate(): string {
    const goToPageElement: HTMLElement = this.createToolbarItem(
      'input',
      this.pdfViewer.element.id + '_currentPageInput',
      null
    );
  ...
  private createTotalPageTemplate(): string {
    const totalPageElement: HTMLElement = this.createToolbarItem(
      'span',
      this.pdfViewer.element.id + '_totalPage',
      'e-pv-total-page'
    );
  ...
  private createNumericTextBox(): void {
    ...
    this.currentPageBoxElement = this.pdfViewerBase.getElement(
      '_currentPageInput'
    ) as HTMLElement;
  ...
  private onToolbarKeydown = (event: KeyboardEvent): void => {
    ...
    const customItem: any = this.toolItems.filter(
      (toolItem: any) => toolItem.id === targetId
    );
    if (
      !(
        targetId === this.pdfViewer.element.id + '_currentPageInput' ||
        targetId === this.pdfViewer.element.id + '_zoomDropDown' ||
        customItem.length > 0
      ) &&
      !isSkip
    ) {
  ...
  private createToolbarItemsForMobile(): void {
    ...
    const template: string =
      '<button id="' +
      this.pdfViewer.element.id +
      '_more_option" class="e-tbar-btn"></button>';
    this.toolbar = new tool({
      items: [
        {
          prefixIcon: 'e-pv-open-document-icon e-pv-icon',
        ...
        },
        {
          ...
          id: this.pdfViewer.element.id + '_menu_organize',
          prefixIcon: 'e-pv-organize-view-icon e-pv-icon',
          align: 'Right',
          disabled: true,
        },
        {
    ...
    this.redoItem.firstElementChild.id =
      this.pdfViewer.element.id + '_redoIcon';
    this.undoItem.firstElementChild.id =
      this.pdfViewer.element.id + '_undoIcon';
  ...
  private createMoreOption(idString: string): void {
    ...
    this.moreDropDown = new DropDownButton({
      ...
      open: (args: OpenCloseMenuEventArgs) => {
        const dropdownButtonPosition: ClientRect =
          this.moreDropDown.element.getBoundingClientRect();
        if (!this.pdfViewer.enableRtl) {
      ...
      select: (args: MenuEventArgs) => {
        switch (args.item.id) {
          ...
          case this.pdfViewer.element.id + '_menu_bookmarks':
            ...
            this.pdfViewerBase.navigationPane.createNavigationPaneMobile(
              'bookmarks'
            );
            break;
          default:
      ...
      beforeItemRender: (args: MenuEventArgs) => {
        if (args.item.id === this.pdfViewer.element.id + '_menu_bookmarks') {
          if (
            !this.pdfViewer.bookmarkViewModule ||
            !this.pdfViewer.bookmarkViewModule.bookmarks
          ) {
  ...
  private createToolbarItem(
    ...
    if (
      elementName === 'input' &&
      id !== this.pdfViewer.element.id + '_zoomDropDown'
    ) {
  ...
  private onTooltipBeforeOpen(args: TooltipEventArgs): void {
    if (
      !this.pdfViewer.toolbarSettings.showTooltip &&
      this.toolbarElement.contains(args.target)
    ) {
      args.cancel = true;
    }
    if (this.annotationToolbarModule) {
    ...
    if (this.formDesignerToolbarModule) {
      if (
        !this.pdfViewer.toolbarSettings.showTooltip &&
        this.formDesignerToolbarModule.toolbarElement &&
  ...
  private createFileElement(toolbarElement: HTMLElement): void {
    if (toolbarElement) {
      if (!isBlazor()) {
        ...
        this.fileInputElement = this.pdfViewer.element.querySelector(
    ...
    if (!isBlazor()) {
      if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
        this.toolbarElement.addEventListener(
          'mouseup',
          this.toolbarOnMouseup.bind(this)
        );
        this.currentPageBoxElement.addEventListener(
          'focusout',
        ...
        this.zoomDropDown.element.addEventListener(
          'keypress',
          this.onZoomDropDownInput.bind(this)
        );
        this.zoomDropDown.element.addEventListener(
          'click',
          this.onZoomDropDownInputClick.bind(this)
        );
      }
    }
  }
  ...
  private unWireEvent(): void {
    ...
    if (
      ...
      if (!isNullOrUndefined(this.currentPageBoxElement)) {
        this.currentPageBoxElement.removeEventListener(
          'focusout',
          this.textBoxFocusOut
        );
        this.currentPageBoxElement.removeEventListener(
      ...
      if (!isNullOrUndefined(this.zoomDropDown)) {
        ...
        this.zoomDropDown.element.removeEventListener(
          'keypress',
          this.onZoomDropDownInput
        );
        this.zoomDropDown.element.removeEventListener(
          'click',
          this.onZoomDropDownInputClick
        );
      }
    }
  }
  ...
  private toolbarOnMouseup(event: MouseEvent): void {
    if (
      event.target === this.itemsContainer ||
      event.target === this.toolbarElement
    ) {
      this.pdfViewerBase.focusViewerContainer();
    }
  }
  ...
  private applyHideToToolbar(
    ...
    for (let index: number = startIndex; index <= endIndex; index++) {
      if (
        ...
        this.toolbar.items[parseInt(index.toString(), 10)]
      ) {
        const className: string =
          this.toolbar.items[parseInt(index.toString(), 10)].cssClass;
        if (className && className !== '') {
          // Querying the toolbar item
          const element: Element = this.toolbar.element.querySelector(
            '.' + className
          );
          if (element) {
  ...
  private toolbarClickHandler = (args: ClickEventArgs): void => {
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      if (
        args.originalEvent.target ===
          this.zoomDropdownItem.parentElement.childNodes[1] ||
        args.originalEvent.target ===
          this.zoomDropdownItem.parentElement.childNodes[2]
      ) {
        args.cancel = true;
      } else if (
    ...
    if (!isNullOrUndefined(args.item)) {
      customItem = this.toolItems.filter(
        (toolItem: any) => toolItem.id === args.item.id
      );
    }
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      if (
        !(
          ...
          args.originalEvent.target === this.textSearchItem.childNodes[0] ||
          customItem.length > 0
        )
      ) {
        if (
          ...
          targetElement.parentElement.id !==
            this.pdfViewer.element.id + '_toolbarContainer_nav' &&
          targetElement.id !==
            this.pdfViewer.element.id + '_toolbarContainer_nav'
        ) {
  ...
  private handleToolbarBtnClick(
    ...
    switch (
      (args.originalEvent.target as HTMLElement).id ||
      (!isNullOrUndefined(args.item as HTMLElement) &&
  ...
  public addComments(targetElement: any): void {
    if (!isBlazor()) {
      if (
        targetElement.id === this.pdfViewer.element.id + '_comment' ||
        targetElement.id === this.pdfViewer.element.id + '_commentIcon'
      ) {
        if (
          targetElement.id === this.pdfViewer.element.id + '_commentIcon' &&
        ...
        if (this.pdfViewer.enableRtl) {
          targetElement.className =
            'e-pv-comment-selection-icon e-pv-icon e-icon-left e-right';
        } else {
          targetElement.className =
            'e-pv-comment-selection-icon e-pv-icon e-icon-left';
        }
      }
    } else {
      this.pdfViewerBase.isCommentIconAdded = true;
    ...
    document
      ...
      .addEventListener(
        'mousedown',
        this.pdfViewer.annotationModule.stickyNotesAnnotationModule.drawIcons.bind(
          this
        )
      );
  }
  ...
  public openZoomDropdown(): void {
    ...
    if (document.fullscreen) {
      if (isBlazor()) {
        ...
        if (
          fullscreenElement &&
          fullscreenElement.tagName !== 'BODY' &&
          fullscreenElement.tagName !== 'HTML'
        ) {
          setTimeout(() => {
            const popupElement: any = document.getElementById(
              toolbarData.pdfViewer.element.id + '_zoomCombo_popup'
            );
            const targetElement: HTMLElement = document.getElementById(
              toolbarData.toolbarElement.id
            );
        ...
        const popupElement: HTMLElement = document.getElementById(
          this.pdfViewer.element.id + '_zoomDropDown_popup'
        );
        const targetElement: HTMLElement = document.getElementById(
          this.toolbarElement.id
        );
        if (popupElement) {
          targetElement.appendChild(popupElement);
        }
    ...
    if (args.target.files[0] !== null) {
      ...
      if (uploadedFile) {
        ...
        reader.onload = (e: any): void => {
          ...
          this.pdfViewer.uploadedFileByteArray = new Uint8Array(
            uploadedFileUrl
          );
          if (isBlazor()) {
            this.pdfViewer._dotnetInstance.invokeMethodAsync(
              'LoadDocumentFromClient',
              uploadedFileUrl
            );
          } else {
            if (this.pdfViewerBase.clientSideRendering) {
              ...
              this.pdfViewer.documentPath = this.pdfViewer
                .uploadedFileByteArray as unknown as string;
              this.pdfViewerBase.documentPathByteArray =
  ...
  private navigateToPage = (args: KeyboardEvent): void => {
    if (args.which === 13) {
      const enteredValue: number = parseInt(
        (this.currentPageBoxElement as HTMLInputElement).value,
        10
      );
  ...
  private textBoxFocusOut = (): void => {
    if (
      this.currentPageBox.value === null ||
      this.currentPageBox.value >= this.pdfViewerBase.pageCount ||
  ...
  private onZoomDropDownInput(event: KeyboardEvent): boolean {
    if (
      ...
      if (event.which === 13) {
        event.preventDefault();
        const value: string = (
          this.zoomDropDown.element as HTMLInputElement
        ).value.trim();
  ...
  private zoomDropDownChange(zoomText: string): void {
    if (
      zoomText !== this.pdfViewer.localeObj.getConstant('Fit Width') &&
      zoomText !== this.pdfViewer.localeObj.getConstant('Fit Page') &&
      zoomText !== this.pdfViewer.localeObj.getConstant('Automatic')
    ) {
  ...
  public updateZoomPercentage(zoomFactor: number): void {
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      const currentPercent: string =
        parseInt((zoomFactor * 100).toString(), 10) + '%';
      if (isBlazor()) {
        const blazorZoomDropDown: any =
          this.pdfViewerBase.getElement('_zoomDropDown');
        ...
        if (this.zoomDropDown.text === currentPercent) {
          (this.zoomDropDown.element as HTMLInputElement).value =
            currentPercent;
        }
  ...
  public textSearchButtonHandler(iskeyboardClick?: boolean): void {
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      if (this.pdfViewer.textSearchModule && this.pdfViewerBase.pageCount > 0) {
        ...
        this.pdfViewer.textSearchModule.showSearchBox(
          this.isTextSearchBoxDisplayed
        );
        if (this.isTextSearchBoxDisplayed) {
          ...
          const searchInputElement: HTMLInputElement = document.getElementById(
            this.pdfViewer.element.id + '_search_input'
          ) as HTMLInputElement;
          ...
          if (!isBlazor()) {
            ...
            const searchItem: any = this.pdfViewerBase.getElement(
              '_search'
            ) as HTMLElement;
            if (iskeyboardClick) {
  ...
  public initiateAnnotationMode(id?: string, isKeyBoardEvent?: boolean): void {
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      if (
        ...
        if (
          isKeyBoardEvent ||
          this.pdfViewer.toolbarModule.annotationToolbarModule.toolbar.items
            .length > 0
        ) {
          document
            .getElementById(
        ...
        if (
          ...
          const formDesignerMainDiv: HTMLElement = document.getElementById(
            this.pdfViewer.element.id + '_formdesigner_toolbar'
          );
          formDesignerMainDiv.style.display = 'none';
          this.formDesignerToolbarModule.isToolbarHidden = false;
          this.formDesignerToolbarModule.showFormDesignerToolbar(
            this.formDesignerItem
          );
  ...
  private initiateFormDesignerMode(isKeyBoardEvent?: boolean): void {
    if (
      this.formDesignerToolbarModule &&
      this.pdfViewer.enableFormDesignerToolbar
    ) {
      this.formDesignerToolbarModule.showFormDesignerToolbar(
        this.formDesignerItem
      ...
      if (
        ...
        const annotationMainDiv: HTMLElement = document.getElementById(
          this.pdfViewer.element.id + '_annotation_toolbar'
        );
        ...
        if (
          !isNullOrUndefined(commentPanel) &&
          !isNullOrUndefined(this.pdfViewerBase.navigationPane)
        ) {
      ...
      if (
        isKeyBoardEvent &&
        this.pdfViewer.toolbarModule.formDesignerToolbarModule.toolbar.items
          .length > 0
      ) {
        document
          .getElementById(
            this.pdfViewer.toolbarModule.formDesignerToolbarModule.toolbar
              .items[0].id
          )
          .focus();
      }
  ...
  private showSeparator(
    ...
    if (
      !this.isOpenBtnVisible ||
      (!this.isNavigationToolVisible &&
        !this.isMagnificationToolVisible &&
        !this.isSelectionBtnVisible &&
        !this.isScrollingBtnVisible &&
        !this.isUndoRedoBtnsVisible)
    ) {
    ...
    if (
      ...
      (this.isMagnificationToolVisible &&
        ...
        !this.isScrollingBtnVisible) ||
      (!this.isMagnificationToolVisible &&
        (this.isSelectionBtnVisible || this.isScrollingBtnVisible))
    ) {
  ...
  public applyToolbarSettings(): void {
    const toolbarSettingsItems: (CustomToolbarItemModel | ToolbarItem)[] =
      this.pdfViewer.toolbarSettings.toolbarItems;
  ...
  private getStampMode(): boolean {
    if (
      ...
      this.pdfViewer.annotation.stampAnnotationModule
    ) {
        ...
        for (let n: number = 0; n < stampElements.length; n++) {
          elements.push({
            ...
            customStampImageSource:
              stampElements[parseInt(n.toString(), 10)].customStampImageSource,
          });
        }
      }
  ...
  private stampBeforeClose(args: any, showItemOnClick: boolean): void {
    if (
      (args.ParentItem &&
        args.ParentItem.Text !== 'Custom Stamp' &&
  ...
  private stampSelect(args: any, stampParentID: string): void {
    ...
    if (args.Item.Text === 'Custom Stamp') {
      ...
      stampImage.click();
      stampImage.addEventListener(
        'change',
        this.annotationToolbarModule.addStampImage
      );
      ...
      args.Item.Text !== 'Dynamic' &&
      args.Item.Text !== '' &&
      args.Item.Text !== 'Standard Business' &&
      (stampParentID === 'Sign Here' || args.Item.Text !== 'Sign Here')
    ) {
      this.annotationToolbarModule.checkStampAnnotations();
      this.pdfViewer.annotation.stampAnnotationModule.isStampAddMode = true;
      this.pdfViewer.annotationModule.stampAnnotationModule.isStampAnnotSelected =
  ...
  private enableItems(element: HTMLElement, isEnable: boolean): void {
    ...
    if (
      element &&
      element.id === this.pdfViewer.element.id + '_zoomDropDownContainer'
    ) {
      this.pdfViewerBase
        .getElement('_zoomDropDown')
        .setAttribute('tabindex', isEnable ? '0' : '-1');
      this.pdfViewerBase
        .getElement('_zoomDropDown')
        .setAttribute('data-tabindex', isEnable ? '0' : '-1');
    } else if (
