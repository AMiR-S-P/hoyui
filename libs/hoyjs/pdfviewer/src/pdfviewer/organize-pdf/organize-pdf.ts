...
import { Tooltip, TooltipEventArgs, Dialog } from '@hoyui/popups';
import { CheckBox } from '@hoyui/buttons';
import {
  Toolbar,
  ClickEventArgs,
  ContextMenu,
...
export class PageOrganizer {
  ...
  public createOrganizeWindow(isReConstruct?: boolean): void {
    ...
    if (
      !isNullOrUndefined(
        document.getElementById(elementID + '_organize_window')
      ) &&
      !isNullOrUndefined(this.organizeDialog)
    ) {
      this.organizeDialog.show(true);
      this.isOrganizeWindowOpen = true;
      return;
    }
    this.dialogDivElement = createElement('div', {
      id: elementID + '_organize_window',
      className: 'e-pv-organize-window',
    });
    ...
    this.organizeDialog = new Dialog({
      ...
      close: (args: any) => {
        if (!this.isSkipRevert) {
          this.tempOrganizePagesCollection = JSON.parse(
            JSON.stringify(this.organizePagesCollection)
          );
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      ...
      this.organizeDialog.buttons = [
        {
          buttonModel: {
            content: this.pdfViewer.localeObj.getConstant('Save As'),
            isPrimary: true,
          },
          click: this.onSaveasClicked.bind(this),
        },
        {
          buttonModel: {
            content: this.pdfViewer.localeObj.getConstant('Save'),
            isPrimary: true,
          },
          click: this.onSaveClicked.bind(this),
        },
        {
          buttonModel: {
            content:
              this.pdfViewer.localeObj.getConstant('Total') +
              ' ' +
              pagecount.toString() +
              ' ' +
              this.pdfViewer.localeObj.getConstant('Pages'),
            cssClass: 'e-pv-organize-total-page-button',
            disabled: true,
          },
        },
      ];
    ...
    this.waitingPopup = createElement('div', {
      id: elementID + '_organizeLoadingIndicator',
    });
  ...
  private initEventListeners(): void {
    ...
    this.tileAreaDiv.addEventListener(
      'mousedown',
  ...
  private removeEventListeners(): void {
    if (!isNullOrUndefined(this.tileAreaDiv)) {
      this.tileAreaDiv.removeEventListener(
        'mousedown',
        this.boundOnTileAreaMouseDown
      );
    }
  ...
  private onTileAreaMouseDown(event: MouseEvent): void {
    if (
      event.target &&
      (event.target as HTMLElement).previousElementSibling &&
      (event.target as HTMLElement).previousElementSibling.classList.contains(
        'e-pv-organize-tile-checkbox'
      )
    ) {
    ...
    if ((event.target as HTMLElement).closest('.e-pv-organize-anchor-node')) {
      ...
      if (this.shiftKey && this.startTile) {
        ...
        const currentIndex: number = Array.from(
          this.tileAreaDiv.children
        ).indexOf(targetTile.closest('.e-pv-organize-anchor-node'));
        if (this.startTile) {
          const startIndex: number = Array.from(
            this.tileAreaDiv.children
          ).indexOf(this.startTile.closest('.e-pv-organize-anchor-node'));
          this.selectRange(startIndex, currentIndex);
          tiles.forEach((tile: HTMLElement, index: number) => {
            if (
              index < Math.min(startIndex, currentIndex) ||
  ...
  private selectRange(startIndex: number, endIndex: number): void {
    ...
    for (let i: number = minIndex; i <= maxIndex; i++) {
      const tile: HTMLElement = this.tileAreaDiv.children[
  ...
  private selectTile(tile: HTMLElement): void {
    if (!isNullOrUndefined(tile)) {
      const checkbox: HTMLInputElement = tile
        .closest('.e-pv-organize-anchor-node')
        .querySelector('.e-pv-organize-tile-checkbox') as HTMLInputElement;
      if (checkbox) {
  ...
  private clearSelection(): void {
    const selectedTiles: NodeListOf<Element> = document.querySelectorAll(
  ...
  private updateOrganizeDialogSize(): void {
    ...
    const dialogWidth: number =
      this.pdfViewer.element.getBoundingClientRect().width;
  ...
  private createContentArea(): any {
    ...
    const contentDiv: HTMLElement = createElement('div', {
      id: elementID + '_content_appearance',
      className: 'e-pv-organize-content-apperance',
    });
    const toolbarDiv: HTMLElement = createElement('div', {
      id: elementID + '_toolbar_appearance',
      className: 'e-pv-organize-toolbar-apperance',
    });
    this.tileAreaWrapper = createElement('div', {
      id: this.pdfViewer.element.id + '_organize_tile_view_wrapper',
      className: 'e-pv-organize-tile-view-wrapper',
    });
    this.tileAreaDiv = createElement('div', {
      id: this.pdfViewer.element.id + '_organize_tile_view',
      className: 'e-pv-organize-tile-view e-pv-thumbnail-row',
    });
    ...
    this.selectAllCheckBox = new CheckBox({
      label:
        Browser.isDevice && !this.pdfViewer.enableDesktopMode
          ? ''
          : this.pdfViewer.localeObj.getConstant('Select All'),
      cssClass: 'e-pv-organize-select-all',
      checked: false,
      change: this.onSelectAllClick.bind(this),
    });
    const toolbarItems: ItemModel[] = [];
    const toolbarItemsForDesktop: ItemModel[] = [
      {
        type: 'Input',
        template: this.selectAllCheckBox,
      ...
      { type: 'Separator', align: 'Left' },
      {
        ...
        id: this.pdfViewer.element.id + '_undo_organize_Pages',
        align: 'Left',
      ...
      },
      {
        ...
        id: this.pdfViewer.element.id + '_redo_organize_Pages',
        align: 'Left',
        click: (args: ClickEventArgs) => {
          this.redo();
        },
      ...
      {
        prefixIcon: 'e-pv-rotate-left-icon e-pv-icon',
        visible: true,
        cssClass: 'e-pv-toolbar-rotate-left',
        id: this.pdfViewer.element.id + '_rotate_page_left',
        align: 'Center',
        click: (args: ClickEventArgs) => {
      ...
      {
        prefixIcon: 'e-pv-rotate-right-icon e-pv-icon',
        visible: true,
        cssClass: 'e-pv-toolbar-rotate-right',
        id: this.pdfViewer.element.id + '_rotate_page_right',
        align: 'Center',
        click: (args: ClickEventArgs) => {
      ...
      {
        prefixIcon: 'e-pv-copy-icon e-pv-icon',
        visible: true,
        cssClass: 'e-pv-toolbar-rotate-right',
        id: this.pdfViewer.element.id + '_copy_page',
        align: 'Center',
        click: (args: ClickEventArgs) => {
      ...
      { type: 'Separator', align: 'Center' },
      {
        prefixIcon: 'e-pv-delete-icon e-pv-icon',
        visible: true,
        cssClass: 'e-pv-delete-selected',
        id: this.pdfViewer.element.id + '_delete_selected',
        align: 'Center',
        click: (args: ClickEventArgs) => {
      ...
      {
        prefixIcon: 'e-pv-import-icon e-pv-icon',
        text: this.pdfViewer.localeObj.getConstant('Import Document'),
        visible: true,
        cssClass: 'e-pv-import-pages',
        id: this.pdfViewer.element.id + '_import_pages',
        align: 'Right',
        click: (args: ClickEventArgs) => {
    ...
    const toolbarItemsForMobile: ItemModel[] = [
      ...
      {
        prefixIcon: 'e-pv-more-icon e-pv-icon',
        visible: true,
        cssClass: 'e-pv-toolbar-rotate-right',
        id: this.pdfViewer.element.id + '_organize_more_button',
        align: 'Right',
    ...
    const rotateRightToolbarButton: HTMLElement = toolbarDiv.querySelector(
      '#' + this.pdfViewer.element.id + '_rotate_page_right'
    );
    if (!isNullOrUndefined(rotateRightToolbarButton)) {
      this.createTooltip(
        rotateRightToolbarButton,
        this.pdfViewer.localeObj.getConstant('Rotate Right')
      );
    }
    const rotateLeftToolbarButton: HTMLElement = toolbarDiv.querySelector(
      '#' + this.pdfViewer.element.id + '_rotate_page_left'
    );
    if (!isNullOrUndefined(rotateLeftToolbarButton)) {
      this.createTooltip(
        rotateLeftToolbarButton,
        this.pdfViewer.localeObj.getConstant('Rotate Left')
      );
    }
    const copyToolbarButton: HTMLElement = toolbarDiv.querySelector(
      '#' + this.pdfViewer.element.id + '_copy_page'
    );
    if (!isNullOrUndefined(copyToolbarButton)) {
      this.createTooltip(
        copyToolbarButton,
        this.pdfViewer.localeObj.getConstant('Copy Pages')
      );
    }
    const deleteToolbarButton: HTMLElement = toolbarDiv.querySelector(
      '#' + this.pdfViewer.element.id + '_delete_selected'
    );
    if (!isNullOrUndefined(deleteToolbarButton)) {
      this.createTooltip(
        deleteToolbarButton,
        this.pdfViewer.localeObj.getConstant('Delete Pages')
      );
    }
    const undoToolbarButton: HTMLElement = toolbarDiv.querySelector(
      '#' + this.pdfViewer.element.id + '_undo_organize_Pages'
    );
    if (!isNullOrUndefined(undoToolbarButton)) {
      this.createTooltip(
        undoToolbarButton,
        this.pdfViewer.localeObj.getConstant('Undo')
      );
    }
    const redoToolbarButton: HTMLElement = toolbarDiv.querySelector(
      '#' + this.pdfViewer.element.id + '_redo_organize_Pages'
    );
    if (!isNullOrUndefined(redoToolbarButton)) {
      this.createTooltip(
        redoToolbarButton,
        this.pdfViewer.localeObj.getConstant('Redo')
      );
    }
    return contentDiv;
  }
  ...
  private createMobileContextMenu(): void {
    this.mobileContextMenu = [
      {
        text: this.pdfViewer.localeObj.getConstant('Save'),
        iconCss: 'e-icons e-pv-save-icon e-pv-icon',
      },
      {
        text: this.pdfViewer.localeObj.getConstant('Save As'),
        iconCss: 'e-icons e-pv-save-as-icon e-pv-icon',
      },
      {
        separator: true,
      },
      {
        ...
        iconCss: 'e-pv-copy-icon e-pv-icon',
      },
      {
        separator: true,
      },
      {
        text: this.pdfViewer.localeObj.getConstant('Import Document'),
        id: this.pdfViewer.element.id + '_import_pages',
        iconCss: 'e-pv-import-icon e-pv-icon',
      },
    ];
    const contextMenuElement: HTMLElement = createElement('ul', {
      id: this.pdfViewer.element.id + '_organize_context_menu',
    });
    ...
    if (isNullOrUndefined(this.contextMenuObj)) {
      this.contextMenuObj = new ContextMenu({
        target: '#' + this.pdfViewer.element.id + '_organize_more_button',
        items: this.mobileContextMenu,
  ...
  private contextMenuBeforeOpen(args: BeforeOpenCloseMenuEventArgs): void {
    ...
    if (
      ...
      this.pdfViewer.pageOrganizerSettings.canCopy &&
      !this.getCopiedItems(isCopyDisabled) &&
  ...
  private getCopiedItems(isCopyDisabled: boolean): boolean {
    const selectedNodes: NodeListOf<Element> =
      this.tileAreaDiv.querySelectorAll('.e-pv-organize-node-selection-ring');
    selectedNodes.forEach((selectedElements: HTMLElement) => {
      const mainTileElement: HTMLElement = selectedElements.closest(
        '.e-pv-organize-anchor-node'
      ) as HTMLElement;
      const pageOrder: number = parseInt(
        mainTileElement.getAttribute('data-page-order'),
        10
      );
      const currentPageDetails: OrganizeDetails =
        this.tempOrganizePagesCollection.find((item: OrganizeDetails) => {
  ...
  private requestPreviewCreation(proxy: PageOrganizer): void {
    ...
    if (digitalSignaturePresent) {
      digitalSignatureList =
        proxy.pdfViewerBase.digitalSignaturePages.toString();
    }
    const jsonObject: object = {
      startPage: startIndex.toString(),
      endPage: previewLimit.toString(),
      sizeX: '99.7',
      sizeY: '141',
    ...
    if (!this.pdfViewerBase.clientSideRendering) {
      ...
      this.previewRequestHandler.url =
        proxy.pdfViewer.serviceUrl +
        '/' +
      ...
      this.previewRequestHandler.onSuccess = function (result: any): void {
        ...
        const redirect: boolean = (proxy as any).pdfViewerBase.checkRedirection(
          data
        );
        if (!redirect) {
      ...
      this.previewRequestHandler.onFailure = function (result: any): void {
        proxy.pdfViewer.fireAjaxRequestFailed(
          result.status,
          result.statusText,
          proxy.pdfViewer.serverActionSettings.renderThumbnail
        );
      ...
      for (
        let pageIndex: number = startIndex;
        pageIndex < previewLimit;
        pageIndex++
      ) {
    ...
    if (data) {
      ...
      if (typeof data !== 'object') {
        try {
          ...
          proxy.pdfViewerBase.onControlError(
            500,
            data,
            proxy.pdfViewer.serverActionSettings.renderThumbnail
          );
      ...
      if (data && data.uniqueId === proxy.pdfViewerBase.documentId) {
        proxy.pdfViewer.fireAjaxRequestSuccess(
          proxy.pdfViewer.serverActionSettings.renderThumbnail,
          data
        );
  ...
  public previewOnMessage(event: any): void {
    if (event.data.message === 'renderPreviewTileImage') {
      ...
      const { value, width, height, pageIndex, startIndex, endIndex } =
        event.data;
      canvas.width = width;
  ...
  public getData(data: any, isClientRender: boolean): void {
    ...
    if (isClientRender) {
      ...
      for (let i: number = startPage; i < endPage; i++) {
        const thumbnailImage: any =
          data.thumbnailImage[parseInt(i.toString(), 10)];
  ...
  private createImportDocElement(toolbarElement: HTMLElement): void {
    if (this.pdfViewer.pageOrganizerSettings.canImport) {
      if (toolbarElement) {
        this.importDocInputElement = createElement('input', {
          id: this.pdfViewer.element.id + '_importDocElement',
          styles: 'position:fixed; left:-100em',
          attrs: { type: 'file' },
        });
        this.importDocInputElement.setAttribute('accept', '.pdf');
        this.importDocInputElement.setAttribute(
          'aria-label',
  ...
  private pageDragDrop = (event: any): void => {
    const mainTileElement: HTMLElement = event.target.closest(
      '.e-pv-organize-anchor-node'
    ) as HTMLElement;
    const pageOrder: number = parseInt(
      mainTileElement.getAttribute('data-page-order'),
  ...
  private rearrangePages(
    ...
    this.tempOrganizePagesCollection = this.updateCollection(
    ...
    const draggedElements: any = selectedPagesIndexes.map(
      (index: number) => pages[parseInt(index.toString(), 10)]
    );
    let adjustedDropIndex: number = isRightInsertion
      ? dropIndex + 1
      : dropIndex;
    ...
    for (let i: number = 0; i < selectedPagesIndexes.length; i++) {
      if (selectedPagesIndexes[parseInt(i.toString(), 10)] >= dropIndex) {
        selectedPagesIndexes[parseInt(i.toString(), 10)] +=
  ...
  private updateCollection(
    ...
    const sortedCollection: OrganizeDetails[] = collection.sort(
      (a: any, b: any) =>
        this.sorting(a['currentPageIndex'], b['currentPageIndex'])
    );
    const nullCurrentPageIndexCount: number = sortedCollection.filter(
      (item: OrganizeDetails) => item.currentPageIndex === null
    ).length;
    ...
    for (let i: number = 0; i < sortedCollection.length; i++) {
      ...
      for (let j: number = 0; j < selectedIndexes.length; j++) {
        if (
          sortedCollection[parseInt(i.toString(), 10)].currentPageIndex ===
          selectedIndexes[parseInt(j.toString(), 10)]
        ) {
      ...
      if (
        !isCurrentPageIndexInSelectedIndexes &&
        !this.containsPageDetails(
          sortedCollection[parseInt(i.toString(), 10)],
          isAlreadyAdded
        )
      ) {
        if (
    ...
    for (
      let i: number = nullCurrentPageIndexCount;
      i < sortedCollection.length;
      i++
    ) {
      // checking the given Currentpageindex exists in the selected indexes
      let isCurrentPageIndexInSelectedIndexes: boolean = false;
  ...
  private containsPageDetails(
    value: OrganizeDetails,
    array: OrganizeDetails[]
  ): boolean {
    ...
    for (let i: number = 0; i < array.length; i++) {
      ...
      if (
        array[parseInt(i.toString(), 10)].currentPageIndex ===
  ...
  private renderThumbnailImage(): void {
    ...
    for (let i: number = 0; i < this.pdfViewer.pageCount; i++) {
      ...
      this.organizePagesCollection.push(
        new OrganizeDetails(
          i,
          i,
          null,
  ...
  private bindImportDocEvent(): void {
    if (this.pdfViewer.pageOrganizerSettings.canImport) {
      const importDocElement: HTMLElement = document.getElementById(
        this.pdfViewer.element.id + '_import_pages'
      );
      if (importDocElement) {
  ...
  public tileImageRender(
    ...
    this.pageLink = createElement('div', {
      id: 'anchor_page_' + pageIndex,
      className: 'e-pv-organize-anchor-node',
    }) as HTMLElement;
    if (isNewPage) {
    ...
    this.thumbnail = createElement('div', {
      id: this.pdfViewer.element.id + '_organize_page_' + pageIndex,
      className: 'e-pv-organize-tile e-pv-thumbnail-column',
    });
    if (isNewPage) {
      this.thumbnail.id = this.thumbnail.id + '_' + subIndex;
    }
    this.imageContainer = createElement('div', {
      id: this.pdfViewer.element.id + '_container_image_' + pageIndex,
      className: 'e-pv-image-container',
    });
    if (isNewPage) {
    ...
    if (!isNewPage && !isEmptyPage) {
      pageSize =
        this.pdfViewerBase.pageSize[parseInt(pageIndex.toString(), 10)];
    } else {
      pageSize = this.tempOrganizePagesCollection.find(
        (item: OrganizeDetails) => {
          return item.currentPageIndex === pageOrder;
        }
      ).pageSize;
      if (isBefore && pageOrder - 1 >= 0) {
        pageSize = this.tempOrganizePagesCollection.find(function (
          item: OrganizeDetails
        ): boolean {
    ...
    this.thumbnailImage = createElement('img', {
      id: this.pdfViewer.element.id + '_organize_image_' + pageIndex,
    ...
    if (isEmptyPage) {
      ...
      const pageDetail: OrganizeDetails = this.tempOrganizePagesCollection.find(
        (item: OrganizeDetails) => {
          return item.currentPageIndex === pageOrder;
        }
      );
      if (pageDetail && pageDetail.pageIndex !== -1) {
        this.thumbnailImage.src =
          this.dataDetails[parseInt(pageDetail.pageIndex.toString(), 10)].image;
      } else if (
        pageDetail &&
        pageDetail.copiedPageIndex !== null &&
        pageDetail.copiedPageIndex >= 0
      ) {
        this.thumbnailImage.src =
          this.dataDetails[
            parseInt(pageDetail.copiedPageIndex.toString(), 10)
          ].image;
      } else {
        this.thumbnailImage.src = base64Image;
      }
      ...
      this.thumbnailImage.src =
        this.dataDetails[parseInt(pageIndex.toString(), 10)].image;
    }
    this.imageContainer.addEventListener('click', (e: any) => {
      this.handleImageContainerClick(e);
    });
    this.thumbnailImage.alt =
      this.pdfViewer.element.id + '_organize_page_' + pageIndex;
    if (isNewPage) {
      this.thumbnailImage.alt =
        this.pdfViewer.element.id + '_organize_page_' + pageOrder;
    }
    if (isImportedPage) {
      const importDownloadIcon: HTMLElement = createElement('span', {
        id:
          this.pdfViewer.element.id +
          '_organize_import_download_icon_' +
          pageIndex,
        className:
          'e-pv-organize-import-download-icon e-pv-import-icon e-pv-icon',
      });
      this.importImageWrapper = createElement('div', {
        id:
          this.pdfViewer.element.id +
          '_organize_import_image_wrapper_' +
          pageIndex,
        className: 'e-pv-organize-import-image-wrapper',
      });
    ...
    if (
      isNewPage &&
      !isNullOrUndefined(
        this.tempOrganizePagesCollection.find(
          (item: OrganizeDetails): boolean => {
            return item.currentPageIndex === pageOrder;
          }
        )
      )
    ) {
      rotateAngle = this.tempOrganizePagesCollection.find(
        (item: OrganizeDetails): boolean => {
          return item.currentPageIndex === pageOrder;
        }
      ).rotateAngle;
    ...
    const thumbnailPageNumber: HTMLElement = createElement('div', {
      id: this.pdfViewer.element.id + '_tile_pagenumber_' + pageIndex,
      className: 'e-pv-tile-number',
    });
    if (isNewPage) {
    ...
    const checkBoxObj: CheckBox = new CheckBox({
      disabled: false,
      checked: false,
      change: this.onSelectClick.bind(this),
    });
    checkBoxObj.appendTo(input);
    input.parentElement.style.height = '100%';
    input.parentElement.style.width = '100%';
    input.parentElement.style.display = 'none';
    const buttondiv: any = createElement('div', {
      id: this.pdfViewer.element.id + '_organize_buttondiv_' + pageIndex,
      className: 'e-pv-organize-buttondiv',
    });
    if (isNewPage) {
      buttondiv.id = buttondiv.id + '_' + subIndex;
    }
    this.deleteButton = createElement('button', {
      id: this.pdfViewer.element.id + '_delete_page_' + pageIndex,
      attrs: {
        'aria-label': this.pdfViewer.localeObj.getConstant('Delete Page'),
        tabindex: '-1',
      },
    ...
    this.deleteButton.className =
      'e-pv-tbar-btn e-pv-delete-button e-btn e-pv-organize-pdf-tile-btn';
    this.deleteButton.setAttribute('type', 'button');
    const deleteButtonSpan: HTMLElement = createElement('span', {
      id: this.pdfViewer.element.id + '_delete' + '_icon',
      className: 'e-pv-delete-icon e-pv-icon',
    });
    this.deleteButton.appendChild(deleteButtonSpan);
    const deleteButtonTooltip: Tooltip = new Tooltip({
      content: initializeCSPTemplate(function (): string {
        return this.pdfViewer.localeObj.getConstant('Delete Page');
      }, this),
      opensOn: 'Hover',
      beforeOpen: this.onTooltipBeforeOpen.bind(this),
    });
    deleteButtonTooltip.appendTo(this.deleteButton);
    this.rotateRightButton = createElement('button', {
      id: this.pdfViewer.element.id + '_rotate_page_' + pageIndex,
      attrs: {
        'aria-label': this.pdfViewer.localeObj.getConstant('Rotate Right'),
        tabindex: '-1',
      },
    }) as HTMLButtonElement;
    if (isNewPage) {
    ...
    this.rotateRightButton.className =
      'e-pv-tbar-btn e-pv-rotate-right-button e-btn e-pv-organize-pdf-tile-btn';
    this.rotateRightButton.setAttribute('type', 'button');
    const rotateButtonSpan: HTMLElement = createElement('span', {
      id: this.pdfViewer.element.id + '_rotate-right' + '_icon',
      className: 'e-pv-rotate-right-icon e-pv-icon',
    });
    this.rotateRightButton.appendChild(rotateButtonSpan);
    const rotateButtonTooltip: Tooltip = new Tooltip({
      content: initializeCSPTemplate(function (): string {
        return this.pdfViewer.localeObj.getConstant('Rotate Right');
      }, this),
      opensOn: 'Hover',
      beforeOpen: this.onTooltipBeforeOpen.bind(this),
    });
    rotateButtonTooltip.appendTo(this.rotateRightButton);
    this.rotateLeftButton = createElement('button', {
      id: this.pdfViewer.element.id + '_rotate_page_' + pageIndex,
      attrs: {
        'aria-label': this.pdfViewer.localeObj.getConstant('Rotate Left'),
        tabindex: '-1',
      },
    }) as HTMLButtonElement;
    if (isNewPage) {
    ...
    this.rotateLeftButton.className =
      'e-pv-tbar-btn e-pv-rotate-left-button e-btn e-pv-organize-pdf-tile-btn';
    this.rotateLeftButton.setAttribute('type', 'button');
    const rotateLeftButtonSpan: HTMLElement = createElement('span', {
      id: this.pdfViewer.element.id + '_rotate_left' + '_icon',
      className: 'e-pv-rotate-left-icon e-pv-icon',
    });
    this.rotateLeftButton.appendChild(rotateLeftButtonSpan);
    const rotateLeftButtonTooltip: Tooltip = new Tooltip({
      content: initializeCSPTemplate(function (): string {
        return this.pdfViewer.localeObj.getConstant('Rotate Left');
      }, this),
      opensOn: 'Hover',
      beforeOpen: this.onTooltipBeforeOpen.bind(this),
    });
    ...
    this.copyButton = createElement('button', {
      id: this.pdfViewer.element.id + '_copy_page_' + pageIndex,
      attrs: {
        'aria-label': this.pdfViewer.localeObj.getConstant('Copy Page'),
        tabindex: '-1',
      },
    }) as HTMLButtonElement;
    if (isNewPage) {
    ...
    this.copyButton.className =
      'e-pv-tbar-btn e-pv-copy-button e-btn e-pv-organize-pdf-tile-btn';
    this.copyButton.setAttribute('type', 'button');
    const copyButtonSpan: HTMLElement = createElement('span', {
      id: this.pdfViewer.element.id + '_copy' + '_icon',
      className: 'e-pv-copy-icon e-pv-icon',
    });
    this.copyButton.appendChild(copyButtonSpan);
    const copyButtonTooltip: Tooltip = new Tooltip({
      content: initializeCSPTemplate(function (): string {
        return this.pdfViewer.localeObj.getConstant('Copy Page');
      }, this),
      opensOn: 'Hover',
      beforeOpen: this.onTooltipBeforeOpen.bind(this),
    });
    copyButtonTooltip.appendTo(this.copyButton);
    this.insertRightButton = createElement('button', {
      id: this.pdfViewer.element.id + '_insert_page_' + pageIndex,
      attrs: {
        'aria-label': this.pdfViewer.localeObj.getConstant('Insert Right'),
        tabindex: '-1',
      },
    }) as HTMLButtonElement;
    if (isNewPage) {
    ...
    this.insertRightButton.className =
      'e-pv-tbar-btn e-pv-insert-right-button e-btn e-pv-organize-pdf-tile-btn';
    this.insertRightButton.setAttribute('type', 'button');
    const insertRightButtonSpan: HTMLElement = createElement('span', {
      id: this.pdfViewer.element.id + '_insert_right' + '_icon',
    ...
    const insertRightButtonTooltip: Tooltip = new Tooltip({
      content: initializeCSPTemplate(function (): any {
        return this.pdfViewer.localeObj.getConstant('Insert Right');
      }, this),
      opensOn: 'Hover',
      beforeOpen: this.onTooltipBeforeOpen.bind(this),
    });
    insertRightButtonTooltip.appendTo(this.insertRightButton);
    this.insertLeftButton = createElement('button', {
      id: this.pdfViewer.element.id + '_insert_page_' + pageIndex,
      attrs: {
        'aria-label': this.pdfViewer.localeObj.getConstant('Insert Left'),
        tabindex: '-1',
      },
    }) as HTMLButtonElement;
    if (isNewPage) {
    ...
    this.insertLeftButton.className =
      'e-pv-tbar-btn e-pv-insert-left-button e-btn e-pv-organize-pdf-tile-btn';
    this.insertLeftButton.setAttribute('type', 'button');
    const insertLeftButtonSpan: HTMLElement = createElement('span', {
      id: this.pdfViewer.element.id + '_insert_left' + '_icon',
      className: 'e-icons e-plus',
    });
    this.insertLeftButton.appendChild(insertLeftButtonSpan);
    const insertLeftButtonTooltip: Tooltip = new Tooltip({
      content: initializeCSPTemplate(function (): any {
        return this.pdfViewer.localeObj.getConstant('Insert Left');
      }, this),
      opensOn: 'Hover',
      beforeOpen: this.onTooltipBeforeOpen.bind(this),
    });
    ...
    this.insertRightButton.addEventListener(
      'click',
      this.insertRightButtonClick
    );
  ...
  private handleImageContainerClick(event: MouseEvent | TouchEvent): void {
    const anchorNode: any = (event.target as HTMLElement).closest(
      '.e-pv-organize-anchor-node'
    );
    const nearestCheckBox: any = anchorNode.querySelector(
      '.e-pv-organize-tile-checkbox.e-control.e-checkbox.e-lib'
    );
    ...
    if (
      nearestCheckBox &&
      !isNullOrUndefined(nearestCheckBox) &&
      !isNullOrUndefined((nearestCheckBox as any).ej2_instances) &&
      (nearestCheckBox as any).ej2_instances.length > 0
    ) {
  ...
  private movePages(): void {
    ...
    this.dragObj = new Draggable(draggableElement, {
      ...
      helper: (e: {
        ...
        if (this.pdfViewer.pageOrganizerSettings.canRearrange) {
          ...
          const cloneTargetEle: HTMLElement = e.element.querySelector(
            '.e-pv-organize-image, .e-pv-organize-import-image-wrapper'
          );
          const clone: HTMLElement = cloneTargetEle.cloneNode(
            true
          ) as HTMLElement;
          clone.style.width = cloneTargetEle.clientWidth + 'px';
          clone.style.height = cloneTargetEle.clientHeight + 'px';
          this.virtualEle = createElement('div', {
            className: 'e-pv-virtual-image-container',
          });
      ...
      drag: (e: DragEventArgs) => {
        ...
        if (proxy.pdfViewer.pageOrganizerSettings.canRearrange) {
          ...
          const mainTileElement: HTMLElement =
            !isNullOrUndefined(e.target) && e.target instanceof Element
              ? (e.target.closest('.e-pv-organize-anchor-node') as HTMLElement)
              : null;
          if (!isNullOrUndefined(mainTileElement)) {
            ...
            const tileRect: DOMRect =
              mainTileElement.getBoundingClientRect() as DOMRect;
            let outerBorder: HTMLElement = document.querySelector(
              '.e-pv-organize-outer-border'
            );
            // If the outerBorder does not exist's, creating it
            if (isNullOrUndefined(outerBorder)) {
              outerBorder = createElement('div', {
                className: 'e-pv-organize-outer-border',
              });
              proxy.tileAreaWrapper.appendChild(outerBorder);
            }
            outerBorder.style.display = 'block';
            const isHoveredOnSelected: boolean = proxy.isHoveredOnSelectedPages(
            ...
            if (
              ...
              (e.target.classList.contains('e-pv-organize-image') ||
                e.target.classList.contains(
                  'e-pv-organize-import-image-wrapper'
                ) ||
                e.target.classList.contains('e-pv-image-container'))
            ) {
            ...
            proxy.handlePageMove(
              e,
              tileRect,
              proxy.gapBetweenDivs,
              outerBorder
            );
          }
        }
      ...
      dragStart: (e: DragEventArgs) => {
        ...
        if (proxy.pdfViewer.pageOrganizerSettings.canRearrange) {
          if (
            ...
            const checkedElements: any = proxy.tileAreaDiv.querySelectorAll(
              '.e-pv-organize-node-selection-ring'
            );
            for (let i: number = 0; i < checkedElements.length; i++) {
              proxy.selectedPageIndexes.push(
                parseInt(
                  checkedElements[parseInt(i.toString(), 10)].getAttribute(
                    'data-page-order'
                  ),
                  10
                )
              );
              checkedElements[parseInt(i.toString(), 10)].classList.add(
                'e-pv-organize-tile-draggedEle'
              );
              const imageElement: any = checkedElements[
                parseInt(i.toString(), 10)
              ].querySelector(
                '.e-pv-organize-image, .e-pv-organize-import-image-wrapper'
              );
            ...
            const anchorElement: any = e.element.closest(
              '.e-pv-organize-anchor-node'
            );
            if (anchorElement) {
              proxy.selectedPageIndexes.push(
                parseInt(anchorElement.getAttribute('data-page-order'), 10)
              );
              anchorElement.classList.add('e-pv-organize-tile-draggedEle');
              const imageElement: any = anchorElement.querySelector(
                '.e-pv-organize-image, .e-pv-organize-import-image-wrapper'
              );
              if (imageElement) {
                imageElement.classList.add('e-pv-organize-tile-draggedEle');
      ...
      dragStop: (e: DragEventArgs) => {
        const clearDraggedElements: any = () => {
          ...
          const draggedElements: any = proxy.tileAreaDiv.querySelectorAll(
            '.e-pv-organize-tile-draggedEle'
          );
          draggedElements.forEach((element: any) => {
            element.classList.remove('e-pv-organize-tile-draggedEle');
          });
          const outerBorder: HTMLElement = document.querySelector(
            '.e-pv-organize-outer-border'
          );
          if (!isNullOrUndefined(outerBorder)) {
    ...
    this.dropObj = new Droppable(droppableElement, {
      drop: (e: DropEventArgs) => {
        ...
        proxy.removeSelectionRingStyle();
        const outerBorder: any = document.querySelector(
          '.e-pv-organize-outer-border'
        );
  ...
  private autoScroll(e: DragEventArgs): void {
    ...
    const toolbarBottom: number =
      document
        .getElementById(this.pdfViewer.element.id + '_toolbar_appearance')
        .getBoundingClientRect().bottom + edgeSize;
    const footer: any =
      this.organizeDialog.element.getElementsByClassName('e-footer-content');
    let footerBounds: any;
    if (!isNullOrUndefined(footer) && footer.length > 0) {
      footerBounds = footer[0].getBoundingClientRect();
    }
    const footerTop: number = !isNullOrUndefined(footerBounds)
      ? footerBounds.top - edgeSize
    ...
    const isInTopEdge: boolean =
      parseFloat(this.virtualEle.getBoundingClientRect().top.toFixed(2)) <=
      edgeTop;
    const isInBottomEdge: boolean =
      this.virtualEle.getBoundingClientRect().bottom >= edgeBottom;
    ...
    const maxScrollY: number =
      this.tileAreaDiv.scrollHeight - this.tileAreaDiv.clientHeight;
    ...
    function adjustWindowScroll(): boolean {
      ...
      const dragDownDiffrence: number =
        elementBounds.bottom - (proxy.previousClientY + elementBounds.height);
      ...
      const canScrollDown: boolean =
        currentScrollY < maxScrollY && dragDownDiffrence >= 0;
      let nextScrollY: number = currentScrollY;
      const maxStep: number = 10;
      // Should we scroll up?
      if (isInTopEdge && canScrollUp) {
        const intensity: number =
          (edgeTop - proxy.virtualEle.getBoundingClientRect().top) / edgeSize;
        nextScrollY = nextScrollY - maxStep * intensity;
        // Should we scroll down?
      } else if (isInBottomEdge && canScrollDown) {
        const intensity: number =
          (proxy.virtualEle.getBoundingClientRect().bottom - edgeBottom) /
          edgeSize;
        nextScrollY = nextScrollY + maxStep * intensity;
      }
  ...
  private handlePageMove(
    ...
    outerBorder: HTMLElement
  ): void {
    const isRightInsertion: boolean = this.isTileRightInsertion(e);
    if (!isNullOrUndefined(this.isTileRightInsertion)) {
      const offset: number = isRightInsertion
        ? tileRect['width'] + gapBetweenDivs / 2
        : -gapBetweenDivs / 2;
      const parentBound: DOMRect =
        outerBorder.parentElement.getBoundingClientRect() as DOMRect;
  ...
  private isTileRightInsertion(e: DropEventArgs): boolean {
    const mainTileElement: HTMLElement = !isNullOrUndefined(e.target)
      ? (e.target.closest('.e-pv-organize-anchor-node') as HTMLElement)
      : null;
    if (!isNullOrUndefined(mainTileElement)) {
      const tileRect: DOMRect =
        mainTileElement.getBoundingClientRect() as DOMRect;
      const virtualElementClientX: number =
        e.event.type === 'mousemove'
          ? e.event.clientX
  ...
  private addSelectionRingStyle(): void {
    const anchorElements: any = this.tileAreaDiv.querySelectorAll(
      '.e-pv-organize-anchor-node'
    );
    for (let i: number = 0; i < this.selectedPageIndexes.length; i++) {
      anchorElements[
        this.selectedPageIndexes[parseInt(i.toString(), 10)]
      ].classList.add('e-pv-dragging-style');
    }
  }
  ...
  private removeSelectionRingStyle(): void {
    ...
    for (let i: number = 0; i < this.selectedPageIndexes.length; i++) {
      anchorElements[
        this.selectedPageIndexes[parseInt(i.toString(), 10)]
      ].classList.remove('e-pv-dragging-style');
    }
  }
        ...
        for (const child of childArray) {
          ...
          if (!child.classList.contains('e-pv-image-container')) {
            ...
            if (child.classList.contains('e-checkbox-wrapper')) {
              ...
              child.classList.contains('e-pv-organize-buttondiv') &&
  ...
  public addOrganizeAction(
    ...
    const action: IActionOrganizeElements = {
      UndoRedoTileActions: UndoRedoTileActions,
      action: actionString,
      toolbarActions: toolbarActions,
      selectedPagesIndexes: selectedPageIndexes,
      dropIndex: dropIndex,
  ...
  private enableDisableToolbarItems(): void {
    ...
    this.toolbar.items.forEach((item: ItemModel) => {
      if (item.id === this.pdfViewer.element.id + '_rotate_page_left') {
        ...
        this.enableToolbarItem(
          item.id,
          this.pdfViewer.pageOrganizerSettings.canImport
        );
      }
    });
  }
  ...
  private disableTileDeleteButton(): void {
    if (this.tileAreaDiv.childElementCount === 1) {
      const mainTileElement: HTMLElement = this.tileAreaDiv.querySelector(
        '.e-pv-organize-anchor-node'
      ) as HTMLElement;
      const deleteButton: HTMLButtonElement = mainTileElement.querySelector(
        '.e-pv-delete-button'
      ) as HTMLButtonElement;
      ...
      for (let i: number = 0; i < this.tileAreaDiv.childElementCount; i++) {
        ...
        if (
          (
            this.tileAreaDiv.childNodes[
              parseInt(i.toString(), 10)
            ] as HTMLElement
          ).classList.contains('e-pv-organize-anchor-node')
        ) {
          mainTileElement = this.tileAreaDiv.childNodes[
            parseInt(i.toString(), 10)
          ] as HTMLElement;
        } else {
          mainTileElement = (
          ...
          ).querySelector('.e-pv-organize-anchor-node') as HTMLElement;
        }
        const deleteButton: HTMLButtonElement = mainTileElement.querySelector(
          '.e-pv-delete-button'
        ) as HTMLButtonElement;
        if (
          !isNullOrUndefined(deleteButton) &&
          this.pdfViewer.pageOrganizerSettings.canDelete
        ) {
  ...
  private disableTileCopyRotateButton(): void {
    for (let i: number = 0; i < this.tempOrganizePagesCollection.length; i++) {
      const pageInfo: OrganizeDetails =
        this.tempOrganizePagesCollection[parseInt(i.toString(), 10)];
      if (
        pageInfo.isImportedDoc &&
        !pageInfo.isDeleted &&
        !pageInfo.currentPageIndex != null
      ) {
        const mainTileElement: HTMLElement = this.tileAreaDiv.querySelector(
          `[data-page-order="${pageInfo.currentPageIndex.toString()}"]`
        );
        const rotateLeftButton: HTMLButtonElement =
          mainTileElement.querySelector(
            '.e-pv-rotate-left-button'
          ) as HTMLButtonElement;
        const rotateRightButton: HTMLButtonElement =
          mainTileElement.querySelector(
            '.e-pv-rotate-right-button'
          ) as HTMLButtonElement;
        const copyButton: HTMLButtonElement = mainTileElement.querySelector(
          '.e-pv-copy-button'
        ) as HTMLButtonElement;
  ...
  private onSelectClick = (args: any): void => {
    ...
    const pageElement: HTMLElement = checkboxElement.closest(
      '.e-pv-organize-anchor-node'
    ) as HTMLElement;
    if (
      args.event.pointerType === 'mouse' ||
      (!this.isTouchEvent &&
        !(Browser.isDevice && !this.pdfViewer.enableDesktopMode))
    ) {
      if (
        this.isClickedOnCheckBox &&
        !isNullOrUndefined(checkboxElement) &&
        !isNullOrUndefined(pageElement)
      ) {
        if (pageElement) {
        ...
        if (!(this.ctrlKey || this.shiftKey)) {
          const previouslySelectedTiles: NodeListOf<HTMLElement> =
            document.querySelectorAll(
              '.e-pv-organize-node-selection-ring'
            ) as NodeListOf<HTMLElement>;
          if (previouslySelectedTiles.length > 0) {
            for (let i: number = 0; i < previouslySelectedTiles.length; i++) {
              const previousCheckbox: HTMLInputElement =
                previouslySelectedTiles[parseInt(i.toString(), 10)]
                  .closest('.e-pv-organize-anchor-node')
                  .querySelector(
                    '.e-pv-organize-tile-checkbox'
                  ) as HTMLInputElement;
              previousCheckbox.checked = false;
              this.setSelectionRingStyle(
                previousCheckbox,
                previouslySelectedTiles[parseInt(i.toString(), 10)]
              );
            }
    ...
    if (this.totalCheckedCount > 1) {
      for (
        let i: number = 0;
        i <
        pageElement.querySelector('.e-pv-organize-buttondiv').childElementCount;
        i++
      ) {
        const id: string = pageElement.querySelector('.e-pv-organize-buttondiv')
          .children[parseInt(i.toString(), 10)].id;
        if (id.split('_')[1] === 'insert') {
          (
            pageElement.querySelector('.e-pv-organize-buttondiv').children[
              parseInt(i.toString(), 10)
            ] as HTMLElement
          ).style.display = 'flex';
        } else {
  ...
  private updateSelectAllCheckbox(): void {
    ...
    this.totalCheckedCount = this.tileAreaDiv.querySelectorAll(
      '.e-pv-organize-node-selection-ring'
    ).length;
    ...
    for (const child of childrenArray) {
      ...
      for (const subchild of childArray) {
        if (subchild.classList.contains('e-checkbox-wrapper')) {
          const id: any = (
            subchild.getElementsByClassName(
              'e-control e-checkbox e-lib'
            )[0] as HTMLInputElement
          ).id;
          const cbObj: any = getComponent(
          ...
          if (checkbox.checked) {
            ...
            (
              (subchild as HTMLElement).children[0] as HTMLElement
            ).style.display = 'block';
            ...
            if (
              !isNullOrUndefined((subchild as HTMLElement).parentElement) &&
              !isNullOrUndefined(
                (subchild as HTMLElement).parentElement.lastElementChild
              ) &&
              (
                subchild as HTMLElement
              ).parentElement.lastElementChild.classList.contains(
                'e-pv-organize-buttondiv'
              )
            ) {
              (
                (subchild as HTMLElement).parentElement
  ...
  private rotateButtonClick = (event: MouseEvent): void => {
    if (this.pdfViewer.pageOrganizerSettings.canRotate) {
      ...
      const mainTileElement: HTMLElement = rotateButton.closest(
        '.e-pv-organize-anchor-node'
      ) as HTMLElement;
      const imageContainer: HTMLElement = mainTileElement.querySelector(
        '.e-pv-organize-image'
      ) as HTMLElement;
      const pageOrder: number = parseInt(
        mainTileElement.getAttribute('data-page-order'),
  ...
  private openContextMenu(event: any): void {
    const contextMenu: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_organize_context_menu'
    );
    if (
      !isNullOrUndefined(contextMenu) &&
      contextMenu.style.display !== 'block'
    ) {
      this.contextMenuObj.open(
  ...
  private onToolbarRightButtonClick = (): void => {
    if (this.pdfViewer.pageOrganizerSettings.canRotate) {
      ...
      for (let i: number = 0; i < proxy.tileAreaDiv.childElementCount; i++) {
        const mainTileElement: any =
          proxy.tileAreaDiv.childNodes[parseInt(i.toString(), 10)];
        // Type assertion to HTMLElement
        if (
          mainTileElement instanceof HTMLElement &&
          mainTileElement.classList.contains(
            'e-pv-organize-node-selection-ring'
          )
        ) {
          const imageContainer: HTMLElement = mainTileElement.querySelector(
          ...
          if (imageContainer) {
            ...
            // Update the rotation value in the pageDetails collection
            this.updateTempRotationDetail(pageOrder, 90);
            this.toolbarUndoRedoCollection.push(
              this.clonedCollection(
                this.tempOrganizePagesCollection.find(
                  (item: OrganizeDetails) => {
                    return item.currentPageIndex === pageOrder;
  ...
  private onToolbarLeftButtonClick = (): void => {
    ...
    for (let i: number = 0; i < proxy.tileAreaDiv.childElementCount; i++) {
      ...
      if (
        mainTileElement instanceof HTMLElement &&
        mainTileElement.classList.contains('e-pv-organize-node-selection-ring')
      ) {
        const imageContainer: HTMLElement = mainTileElement.querySelector(
          '.e-pv-organize-image'
        ) as HTMLElement;
  ...
  private onToolbarCopyButtonClick = (): void => {
    if (this.pdfViewer.pageOrganizerSettings.canCopy) {
      ...
      const proxy: PageOrganizer = this;
      for (let i: number = 0; i < proxy.tileAreaDiv.childElementCount; i++) {
        const mainTileElement: HTMLElement = proxy.tileAreaDiv.childNodes[
          parseInt(i.toString(), 10)
        ] as HTMLElement;
        if (
          mainTileElement instanceof HTMLElement &&
          ...
          const pageId: string =
            mainTileElement.id.split('anchor_page_')[
              mainTileElement.id.split('anchor_page_').length - 1
            ];
          const pageOrder: number = parseInt(
            mainTileElement.getAttribute('data-page-order'),
            10
          ...
          let pageIndex: number = parseInt(
            pageIdlist[parseInt((pageIdlist.length - 1).toString(), 10)],
            10
          );
          if (pageIdlist.length > 1) {
            pageIndex = parseInt(
              pageIdlist[parseInt((pageIdlist.length - 2).toString(), 10)],
              10
            );
          }
          subIndex = this.getNextSubIndex(
            mainTileElement.parentElement,
            pageIndex
          );
          this.copyPage(pageOrder, mainTileElement);
          this.tileImageRender(
            pageIndex,
            subIndex,
            pageOrder + 1,
            mainTileElement,
  ...
  private onToolbarDeleteButtonClick = (): void => {
    if (this.pdfViewer.pageOrganizerSettings.canDelete) {
      ...
      const proxy: PageOrganizer = this;
      const selectedNodes: NodeListOf<Element> =
        proxy.tileAreaDiv.querySelectorAll(
          '.e-pv-organize-node-selection-ring'
        );
      selectedNodes.forEach((selectedElements: HTMLElement) => {
        const mainTileElement: HTMLElement = selectedElements.closest(
      ...
      selectedNodes.forEach((selectedElement: HTMLElement) => {
        const mainTileElement: HTMLElement = selectedElement.closest(
          '.e-pv-organize-anchor-node'
        ) as HTMLElement;
  ...
  private updateTempRotationDetail(
    ...
    if (this.pdfViewer.pageOrganizerSettings.canRotate) {
      const tempIndex: number = this.tempOrganizePagesCollection.findIndex(
        (item: OrganizeDetails) => item.currentPageIndex === currentPageIndex
      );
      if (tempIndex !== -1) {
        let rotateAngle: number =
          this.tempOrganizePagesCollection[parseInt(tempIndex.toString(), 10)]
        ...
        this.tempOrganizePagesCollection[
          parseInt(tempIndex.toString(), 10)
        ].rotateAngle = Math.abs(
          (this.tempOrganizePagesCollection[parseInt(tempIndex.toString(), 10)]
            .rotateAngle +
            currentRotation +
            360) %
  ...
  private updateRotationDetailCollection(): void {
    for (let i: number = 0; i < this.tempOrganizePagesCollection.length; i++) {
      const tempPageDetail: OrganizeDetails =
        this.tempOrganizePagesCollection[parseInt(i.toString(), 10)];
      if (tempPageDetail.pageIndex !== -1) {
        const pageDetails: ISize =
          this.pdfViewerBase.pageSize[
            parseInt(tempPageDetail.pageIndex.toString(), 10)
          ];
        if (
          Math.abs(
            this.getRotation(pageDetails.rotation.toString()) -
              tempPageDetail.rotateAngle
          ) %
            180 ===
          90
        ) {
          this.updatePageSize(
            tempPageDetail.pageIndex,
            pageDetails.height,
            pageDetails.width
          );
        }
  ...
  private updatePageSize(
    ...
    for (
      let i: number = pageIndex;
      i < this.pdfViewerBase.pageSize.length;
      i++
    ) {
      if (
        !this.pdfViewerBase.pageSize[parseInt((i - 1).toString(), 10)] &&
        i - 1 < 0
      ) {
        this.pdfViewerBase.pageSize[parseInt(i.toString(), 10)].top =
          this.pdfViewerBase.pageGap;
      } else {
        this.pdfViewerBase.pageSize[parseInt(i.toString(), 10)].top =
          this.pdfViewerBase.pageSize[parseInt((i - 1).toString(), 10)].top +
          this.pdfViewerBase.pageSize[parseInt((i - 1).toString(), 10)].height +
          this.pdfViewerBase.pageGap;
      }
    }
  }
  ...
  private onSaveClicked(): void {
    ...
    if (
      JSON.stringify(this.tempOrganizePagesCollection) !==
        JSON.stringify(this.organizePagesCollection) ||
  ...
  public showOrganizeLoadingIndicator(isShow: boolean): void {
    const waitingPopup: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_organizeLoadingIndicator'
    );
    if (waitingPopup) {
      if (isShow) {
  ...
  private updateOrganizePageDetailsInViewer(): void {
    for (const pageDetail of this.organizePagesCollection) {
      ...
      const pageWidth: any =
        pageSizeDetails.width * this.pdfViewerBase.getZoomFactor();
      const pageHeight: any =
        pageSizeDetails.height * this.pdfViewerBase.getZoomFactor();
      const pageTop: number =
        pageSizeDetails.top * this.pdfViewerBase.getZoomFactor();
      // Find the corresponding pageDiv using pageIndex
      const pageDiv: HTMLElement = this.pdfViewerBase.getElement(
        '_pageDiv_' + pageIndex
      );
      const pageCanvas: HTMLImageElement = this.pdfViewerBase.getElement(
        '_pageCanvas_' + pageIndex
      ) as HTMLImageElement;
      if (pageDiv && pageCanvas) {
        ...
        if (this.pdfViewer.enableRtl) {
          pageDiv.style.right =
            this.pdfViewerBase.updateLeftPosition(pageIndex) + 'px';
        } else {
          pageDiv.style.left =
            this.pdfViewerBase.updateLeftPosition(pageIndex) + 'px';
        }
        pageDiv.style.top = pageTop + 'px';
        this.pdfViewerBase.pageContainer.style.width =
          this.pdfViewerBase.isMixedSizeDocument &&
          this.pdfViewerBase.highestWidth * this.pdfViewerBase.getZoomFactor() >
            this.pdfViewerBase.viewerContainer.clientWidth
            ? this.pdfViewerBase.highestWidth *
                this.pdfViewerBase.getZoomFactor() +
              'px'
            : this.pdfViewerBase.viewerContainer.clientWidth + 'px';
        ...
        if (rotateAngle === 90 || rotateAngle === 270) {
          ...
          pageCanvas.style.marginLeft = `${
            (pageCanvas.height - pageCanvas.width) / 2
          }px`;
          pageCanvas.style.marginTop = `${
            (pageCanvas.width - pageCanvas.height) / 2
          }px`;
        } else {
  ...
  private getNextSubIndex(
    ...
    const elementsWithAnchorId: any = parentElement.querySelectorAll(
      `[id^='anchor_page_${parentPageIndex}']`
    );
   ...
   */
  public undo = (): void => {
    const undoActionObject: IActionOrganizeElements =
      this.undoOrganizeCollection.pop();
    if (undoActionObject) {
      const actionObject: IActionOrganizeElements = JSON.parse(
        JSON.stringify(undoActionObject)
      );
      switch (actionObject.action) {
        ...
        case 'Delete':
          {
            const mainTileElement: HTMLElement = this.tileAreaDiv.childNodes[
              parseInt(
                actionObject.UndoRedoTileActions[0].currentPageIndex.toString(),
                10
              )
            ] as HTMLElement;
            if (actionObject.UndoRedoTileActions[0].isCopied) {
              this.insertRemovedPages(
                actionObject.UndoRedoTileActions[0],
                actionObject.UndoRedoTileActions[0].currentPageIndex,
                mainTileElement
              );
              this.tileImageRender(
                actionObject.UndoRedoTileActions[0].copiedPageIndex,
                0,
                actionObject.UndoRedoTileActions[0].currentPageIndex,
                mainTileElement,
                true,
              ...
              this.tileImageRender(
                ...
                false,
                true,
                actionObject.UndoRedoTileActions[0].documentName
              );
            } else if (
              !actionObject.UndoRedoTileActions[0].isCopied &&
              !actionObject.UndoRedoTileActions[0].isInserted &&
              !actionObject.UndoRedoTileActions[0].isImportedDoc
            ) {
              this.undoDeletedPage(
                actionObject.UndoRedoTileActions[0].currentPageIndex,
                actionObject.UndoRedoTileActions[0].pageIndex,
                actionObject.UndoRedoTileActions[0].rotateAngle,
                mainTileElement
              );
              this.tileImageRender(
                actionObject.UndoRedoTileActions[0].pageIndex,
        ...
        case 'Move Pages': {
          ...
          const beforeDropIndex: {
            currentPageIndex: number;
            selectedIndexes: number[];
          }[] = [];
          const afterDropIndex: {
            currentPageIndex: number;
            selectedIndexes: number[];
          }[] = [];
          ...
          for (
            let i: number = 0;
            i < actionObject.UndoRedoTileActions.length;
            i++
          ) {
            const action: OrganizeDetails =
              actionObject.UndoRedoTileActions[parseInt(i.toString(), 10)];
            const selectedItem: OrganizeDetails =
              this.tempOrganizePagesCollection.find((item: OrganizeDetails) => {
                if (action.isCopied) {
                  return (
                    item.copiedPageIndex === action.copiedPageIndex &&
                    isInRange(
          ...
          if (beforeDropIndex.length > 0) {
            ...
            beforeDropIndex.sort(
              (a: any, b: any) => a.currentPageIndex - b.currentPageIndex
            );
            for (let j: number = 0; j < beforeDropIndex.length; j++) {
              // eslint-disable-next-line max-len
              this.rearrangePages(
                beforeDropIndex[parseInt(j.toString(), 10)].selectedIndexes,
          ...
          if (afterDropIndex.length > 0) {
            ...
            afterDropIndex.sort(
              (a: any, b: any) => b.currentPageIndex - a.currentPageIndex
            );
            for (let j: number = 0; j < afterDropIndex.length; j++) {
              this.rearrangePages(
                afterDropIndex[parseInt(j.toString(), 10)].selectedIndexes,
        ...
        case 'Toolbar Copy':
          if (actionObject.toolbarActions.length > 0) {
            for (
              let i: number = actionObject.toolbarActions.length - 1;
              i >= 0;
              i--
            ) {
              const mainTileElement: HTMLElement = this.tileAreaDiv.childNodes[
                parseInt(
                  actionObject.toolbarActions[
                    parseInt(i.toString(), 10)
              ...
              this.deleteTempPage(
                actionObject.toolbarActions[parseInt(i.toString(), 10)]
                  .currentPageIndex,
                mainTileElement
              );
        ...
        case 'Toolbar Delete':
          {
            if (actionObject.toolbarActions.length > 0) {
              for (
                let i: number = 0;
                i < actionObject.toolbarActions.length;
                i++
              ) {
                const mainTileElement: HTMLElement = this.tileAreaDiv
                  .childNodes[
                  parseInt(
                    actionObject.toolbarActions[
                      parseInt(i.toString(), 10)
   ...
   */
  public redo = (): void => {
    const redoActionObject: IActionOrganizeElements =
      this.redoOrganizeCollection.pop();
    if (redoActionObject) {
      const actionObject: IActionOrganizeElements = JSON.parse(
        JSON.stringify(redoActionObject)
      );
      switch (actionObject.action) {
        case 'Insert Right':
        case 'Insert Left':
        ...
        case 'Move Pages':
          this.rearrangePages(
            actionObject.selectedPagesIndexes,
            actionObject.dropIndex,
            actionObject.isRightInsertion
          );
          break;
        case 'Toolbar Rotate Right':
          this.rotateImages(actionObject, 90);
  ...
  private removePage(pageOrder: number): void {
    const mainTileElement: HTMLElement = this.tileAreaDiv.childNodes[
      parseInt(pageOrder.toString(), 10)
    ] as HTMLElement;
  ...
  private rotateImages = (
    actionObject: IActionOrganizeElements,
  ...
  private rotateImage(currentPageIndex: number, rotationAngle: number): void {
    const mainTileElement: HTMLElement = this.tileAreaDiv.childNodes[
      parseInt(currentPageIndex.toString(), 10)
    ] as HTMLElement;
    const imageContainer: HTMLElement = mainTileElement.querySelector(
      '.e-pv-organize-image'
    ) as HTMLElement;
  ...
  private insertRightButtonClick = (event: MouseEvent): void => {
    if (this.pdfViewer.pageOrganizerSettings.canInsert) {
      ...
      const buttonId: string =
        insertRightButton.id.split('_insert_page_')[
          insertRightButton.id.split('_insert_page_').length - 1
        ];
      const mainTileElement: HTMLElement = insertRightButton.closest(
        '.e-pv-organize-anchor-node'
      ) as HTMLElement;
      const pageOrder: number = parseInt(
        mainTileElement.getAttribute('data-page-order'),
      ...
      let subIndex: number = 0;
      let buttonIndex: number = parseInt(
        buttonIdlist[parseInt((buttonIdlist.length - 1).toString(), 10)],
        10
      );
      if (buttonIdlist.length > 1) {
        buttonIndex = parseInt(
          buttonIdlist[parseInt((buttonIdlist.length - 2).toString(), 10)],
          10
        );
      }
      subIndex = this.getNextSubIndex(
  ...
  private insertLeftButtonClick = (event: MouseEvent): void => {
    if (this.pdfViewer.pageOrganizerSettings.canInsert) {
      ...
      const buttonId: string =
        insetLeftButton.id.split('_insert_page_')[
          insetLeftButton.id.split('_insert_page_').length - 1
        ];
      const mainTileElement: HTMLElement = insetLeftButton.closest(
        '.e-pv-organize-anchor-node'
      ) as HTMLElement;
      const pageOrder: number = parseInt(
        mainTileElement.getAttribute('data-page-order'),
  ...
  private copyButtonClick = (event: MouseEvent): void => {
    if (this.pdfViewer.pageOrganizerSettings.canCopy) {
      ...
      const buttonId: string =
        copyButton.id.split('_copy_page_')[
          copyButton.id.split('_copy_page_').length - 1
        ];
      const mainTileElement: HTMLElement = copyButton.closest(
        '.e-pv-organize-anchor-node'
      ) as HTMLElement;
      const pageOrder: number = parseInt(
        mainTileElement.getAttribute('data-page-order'),
  ...
  private deleteButtonClick = (event: MouseEvent): void => {
    if (this.pdfViewer.pageOrganizerSettings.canDelete) {
      ...
      const mainTileElement: HTMLElement = deleteButton.closest(
        '.e-pv-organize-anchor-node'
      ) as HTMLElement;
      const pageOrder: number = parseInt(
        mainTileElement.getAttribute('data-page-order'),
  ...
  private deletePageElement(mainTileElement: HTMLElement): void {
    if (
      this.pdfViewer.pageOrganizerSettings.canDelete &&
      this.tileAreaDiv.childElementCount > 1
    ) {
      const pageOrder: number = parseInt(
        mainTileElement.getAttribute('data-page-order'),
        10
      ...
      if (
        !isNullOrUndefined(deleteButton) &&
        !isNullOrUndefined((deleteButton as any).ej2_instances) &&
  ...
  private deleteTempPage(currentPageIndex: number, tileDiv: HTMLElement): void {
    if (
      this.pdfViewer.pageOrganizerSettings.canDelete &&
      this.tempOrganizePagesCollection.filter(
        (item: OrganizeDetails) => item.isDeleted === false
      ).length > 0
    ) {
      const index: number = this.tempOrganizePagesCollection.findIndex(
        (item: OrganizeDetails) => item.currentPageIndex === currentPageIndex
      );
      const delCurrentIndex: number =
      ...
      this.tempOrganizePagesCollection = this.tempOrganizePagesCollection.map(
        (item: OrganizeDetails) => {
      ...
      while (!isNullOrUndefined(tileDiv.nextElementSibling)) {
        const nextTileDiv: HTMLElement =
          tileDiv.nextElementSibling as HTMLElement;
        let nextTileIndex: number = parseInt(
          nextTileDiv.getAttribute('data-page-order'),
          10
        );
  ...
  private undoDeletedPage(
    ...
    if (
      this.tempOrganizePagesCollection.some(
        (item: OrganizeDetails) => item.isDeleted
      )
    ) {
      const deletedItem: OrganizeDetails =
        this.tempOrganizePagesCollection.find(
          (item: OrganizeDetails) =>
            item.currentPageIndex === null &&
            item.isDeleted &&
            item.pageIndex !== -1 &&
    ...
    this.tempOrganizePagesCollection = this.tempOrganizePagesCollection.map(
      (item: OrganizeDetails) => {
        if (item.currentPageIndex >= deletedPageIndex && !item.isDeleted) {
          ...
          item.currentPageIndex === deletedPageIndex &&
          item.isDeleted
        ) {
  ...
  private insertRemovedPages(
    ...
    if (index !== -1) {
      for (let i: number = 0; i < index; i++) {
        if (
          this.tempOrganizePagesCollection[parseInt(i.toString(), 10)].isDeleted
        ) {
          deleteCount++;
        }
      }
    ...
    this.tempOrganizePagesCollection = this.tempOrganizePagesCollection
      .slice(0, currentPageIndex + deleteCount)
      .concat(
        toolbarActions,
        this.tempOrganizePagesCollection.slice(currentPageIndex + deleteCount)
      );
    this.tempOrganizePagesCollection = this.tempOrganizePagesCollection.map(
      (item: OrganizeDetails, mapIndex: number) => {
  ...
  private clonedCollection(tempCollecion: OrganizeDetails): OrganizeDetails {
    const clonedCollection: OrganizeDetails = JSON.parse(
      JSON.stringify(tempCollecion)
    );
    return clonedCollection;
  }

  private updateTotalPageCount(): void {
    const totalPages: number = document.querySelectorAll(
      '.e-pv-organize-anchor-node'
    ).length;
    const totalPageNumberElement: Element = document.querySelector(
      '.e-pv-organize-total-page-button'
    );
    if (!isNullOrUndefined(totalPageNumberElement)) {
      (totalPageNumberElement as HTMLElement).textContent =
        this.pdfViewer.localeObj.getConstant('Total') +
        ' ' +
        totalPages.toString() +
        ' ' +
  ...
  private updatePageNumber(): void {
    const totalPages: any = document.querySelectorAll(
      '.e-pv-organize-anchor-node'
    );
    totalPages.forEach((element: any) => {
      const pageOrder: number = parseInt(
        element.getAttribute('data-page-order'),
        10
      );
      const thumbnailPageNumber: HTMLElement = element.querySelector(
        '.e-pv-tile-number'
      ) as HTMLElement;
      if (thumbnailPageNumber) {
        const currentPageNumber: OrganizeDetails =
          this.tempOrganizePagesCollection.find((item: OrganizeDetails) => {
            return item.currentPageIndex === pageOrder;
          });
        if (currentPageNumber.isImportedDoc) {
          ...
          const pageNumberTooltip: Tooltip = new Tooltip({
            content: initializeCSPTemplate(function (): any {
              return thumbnailPageNumber.textContent;
            }, this),
            opensOn: 'Hover',
            beforeOpen: this.onTooltipBeforeOpen.bind(this),
          });
  ...
  private insertTempPage(
    ...
    if (this.pdfViewer.pageOrganizerSettings.canInsert) {
      ...
      if (currentPageIndex !== 0) {
        beforeIndex = this.tempOrganizePagesCollection.findIndex(
          (item: OrganizeDetails) =>
            item.currentPageIndex === currentPageIndex - 1
        );
      } else {
        beforeIndex = index;
      }
      ...
      if (isBefore) {
        pageIndex =
          this.tempOrganizePagesCollection[parseInt(beforeIndex.toString(), 10)]
            .pageIndex;
        if (
          index - 1 >= 0 &&
          !this.tempOrganizePagesCollection[
            parseInt((index - 1).toString(), 10)
          ].isInserted
        ) {
          this.tempOrganizePagesCollection[
            parseInt((index - 1).toString(), 10)
        ...
        if (
          index <= this.tempOrganizePagesCollection.length - 1 &&
          !this.tempOrganizePagesCollection[parseInt(index.toString(), 10)]
            .isInserted
        ) {
          this.tempOrganizePagesCollection[
        ...
        pageSize = JSON.parse(
          JSON.stringify(
            this.tempOrganizePagesCollection[
              parseInt(beforeIndex.toString(), 10)
            ].pageSize
          )
        );
        if (pageIndex !== -1) {
          if (
            !isNullOrUndefined(pageSize.rotation) &&
        ...
        this.tempOrganizePagesCollection = [
          ...this.tempOrganizePagesCollection.slice(0, index),
          new OrganizeDetails(
            currentPageIndex,
            -1,
            this.tempOrganizePagesCollection[
        ...
        if (
          index + 1 <= this.tempOrganizePagesCollection.length - 1 &&
          !this.tempOrganizePagesCollection[
            parseInt((index + 1).toString(), 10)
          ].isInserted
        ) {
        ...
        this.tempOrganizePagesCollection = [
          ...this.tempOrganizePagesCollection.slice(0, index + 1),
          new OrganizeDetails(
            currentPageIndex + 1,
            -1,
            this.tempOrganizePagesCollection[
    ...
    if (this.pdfViewer.pageOrganizerSettings.canImport) {
      ...
      if (index !== -1) {
        ...
        if (isBefore) {
          ...
          this.tempOrganizePagesCollection =
            this.tempOrganizePagesCollection.map(
              (item: OrganizeDetails, mapIndex: number) => {
                if (
                  mapIndex !== index &&
                  item.currentPageIndex >= currentPageIndex &&
  ...
  private organizeWireEvent(): void {
    if (this.importDocInputElement) {
      this.importDocInputElement.addEventListener(
        'change',
        this.importDocument.bind(this)
      );
    }
  }
  ...
  private organizeUnWireEvent(): void {
    if (this.importDocInputElement) {
      this.importDocInputElement.removeEventListener(
        'change',
        this.importDocument.bind(this)
      );
    }
  ...
  public loadImportDoc(
    ...
    if (this.pdfViewer.pageOrganizerSettings.canImport) {
      ...
      const jsonObject: any = this.pdfViewerBase.constructJsonObject(
        documentData,
        password,
        isbase64
      );
      if (this.pdfViewer.serverActionSettings) {
        ...
        this.pdfViewerBase.loadRequestHandler.url =
          this.pdfViewer.serviceUrl +
          '/' +
          this.pdfViewer.serverActionSettings.validatePassword;
        ...
        if (this.pdfViewerBase.clientSideRendering) {
          this.pdfViewerBase
            .getPdfByteArray(base64DocumentData)
            .then((pdfbytearray: any) => {
              let data: any =
                this.pdfViewer.pdfRendererModule.loadImportDocument(
                  pdfbytearray,
                  documentId,
                  password,
                  jsonObject
              ...
              if (data) {
                if (typeof data !== 'object') {
                  try {
                    ...
                    this.pdfViewerBase.onControlError(
                      500,
                      data,
                      this.pdfViewer.serverActionSettings.load
                    );
                    data = null;
                  }
                }
                if (data) {
                  while (typeof data !== 'object') {
                    data = JSON.parse(data);
                    if (
                      typeof parseInt(data, 10) === 'number' &&
                  ...
                  if (
                    ...
                    if (data === 4) {
                      ...
                      isEncrypted = true;
                      this.pdfViewerBase.renderPasswordPopup(
                        documentData,
                        password,
                        this.pdfViewerBase.isImportDoc
                      );
                    } else if (data === 3) {
                      // 3 is error code for corrupted document.
                      this.pdfViewerBase.isImportDoc = true;
                      this.pdfViewerBase.renderCorruptPopup(
                        this.pdfViewerBase.isImportDoc
                      );
                    }
                  }
                ...
                if (
                  (!isEncrypted || (isPasswordCorrect && data !== 4)) &&
                  data !== 3
                ) {
                  this.importDocuments(
                    password,
                    this.importedDocumentName,
                    documentData
                  );
                }
              }
          ...
          this.pdfViewerBase.loadRequestHandler.onSuccess = function (
            result: any
          ): void {
            ...
            if (data) {
              if (typeof data !== 'object') {
                try {
                  ...
                  proxy.pdfViewerBase.onControlError(
                    500,
                    data,
                    proxy.pdfViewer.serverActionSettings.load
                  );
                  data = null;
                }
              }
              if (data) {
                ...
                if (
                  ...
                  if (data === 4) {
                    ...
                    isEncrypted = true;
                    proxy.pdfViewerBase.renderPasswordPopup(
                      documentData,
                      password,
                      proxy.pdfViewerBase.isImportDoc
                    );
                  } else if (data === 3) {
                    // 3 is error code for corrupted document.
                    proxy.pdfViewerBase.isImportDoc = true;
                    proxy.pdfViewerBase.renderCorruptPopup(
                      proxy.pdfViewerBase.isImportDoc
                    );
                  }
                }
              ...
              if (
                ...
                proxy.importDocuments(
                  password,
                  proxy.importedDocumentName,
                  documentData
                );
              }
            }
  ...
  private importDocuments(
    ...
    if (this.pdfViewer.pageOrganizerSettings.canImport) {
      ...
      if (
        ...
        const mainTileElement: HTMLElement = proxy.tileAreaDiv
  ...
  private updateOrganizePageCollection(): void {
    this.organizePagesCollection = JSON.parse(
      JSON.stringify(this.tempOrganizePagesCollection)
    );
  }
  ...
  public applyElementStyles(pageCanvas: any, pageNumber: number): void {
    if (pageCanvas) {
      const canvasElement: HTMLElement = document.getElementById(
        this.pdfViewer.element.id + '_pageCanvas_' + pageNumber
      );
      const oldCanvas: HTMLElement = document.getElementById(
        this.pdfViewer.element.id + '_oldCanvas_' + pageNumber
      );
    ...
    this.pdfViewer.saveAsBlob().then((blob: Blob) => {
      ...
      conversionPromise.then((result: any) => {
        if (!isNullOrUndefined(result) && result !== '') {
          canDownload = this.pdfViewer.firePageOrganizerSaveAsEventArgs(
            fileName,
            result
          );
          if (canDownload) {
  ...
  public rotateAllPages(pageRotateAngle: PdfPageRotateAngle): void {
    if (this.pdfViewer.pageOrganizerSettings.canRotate) {
      const rotateAngle: PdfPageRotateAngle =
        pageRotateAngle as PdfPageRotateAngle;
      // Get the total page count
      const totalPages: number = this.pdfViewer.pageCount;
      // Generate an array of page indexes
      const pageIndexes: number[] = Array.from(
        { length: totalPages },
  ...
  public rotatePages(
    pageIndexes: number[],
    pageRotateAngle: PdfPageRotateAngle
  ): void;
   ...
   */
  public rotatePages(
    arg1: number | number[] | PageRotation[],
    arg2?: number | PdfPageRotateAngle
  ): void {
    if (this.pdfViewer.pageOrganizerSettings.canRotate) {
      if (Array.isArray(arg1)) {
        ...
        // eslint-disable-next-line
        const rotateAngle: PdfPageRotateAngle =
          arguments[2] as PdfPageRotateAngle;
        this.processRotation(
          this.generateRange(pageStartIndex, pageEndIndex),
          rotateAngle
        );
      }
    }
  ...
  private processRotation(
    ...
    if (this.pdfViewer.pageOrganizerSettings.canRotate) {
      ...
      for (const pageIndex of pageIndexes) {
        ...
        const index: number = this.organizePagesCollection.findIndex(
          (item: PageDetails) => item.pageIndex === pageIndex
        );
        ...
        if (index !== -1) {
          ...
          this.organizePagesCollection[
            ...
            (this.organizePagesCollection[parseInt(index.toString(), 10)]
              .rotateAngle +
              rotateAngle +
              360) %
            360;
  ...
  private generateRange(start: number, end: number): number[] {
    return Array.from(
      { length: end - start + 1 },
      (_: any, index: number) => start + index
    );
  }
  ...
  private destroyDialogWindow(): void {
    ...
    const dialogElement: HTMLElement =
      this.pdfViewerBase.getElement('_organize_window');
...
export class PageRotation {
  ...
  constructor(
    ...
    public rotationAngle: PdfPageRotateAngle
  ) {}
}
