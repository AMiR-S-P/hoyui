...
export class ImageStructureBase {
    ...
    if (!isNullOrUndefined(imageFilter)) {
      for (let i: number = 0; i < imageFilter.length; i++) {
        ...
        switch (imageFilter[parseInt(i.toString(), 10)]) {
          case 'DCTDecode': {
            if (
              !this.mImageDictionary.has('SMask') &&
              !this.mImageDictionary.has('Mask')
            ) {
              ...
              if (
                ...
                if (
                  colorSpace.name === 'DeviceRGB' &&
                  (this.mImageDictionary.has('DecodeParms') ||
                    this.mImageDictionary.has('Decode'))
                ) {
                  break;
                }
              }
  ...
  private getImageInterpolation(imageDictionary: _PdfDictionary): void {
    if (
      !isNullOrUndefined(imageDictionary) &&
      imageDictionary.has('Interpolate')
    ) {
  ...
  private imageStream(): any {
    const content: Uint8Array = _stringToBytes(
      this.mImageStream.getString(),
      false,
      true
    ) as Uint8Array;
    return content;
