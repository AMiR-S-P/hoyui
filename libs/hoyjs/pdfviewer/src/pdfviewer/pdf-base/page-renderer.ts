...
import { Matrix, Rect, Size } from '@hoyui/drawings';
...
export class PageRenderer {
  ...
  public exportAnnotationComments(pageNumber: number, pageSize: Size): any {
    const page: PdfPage =
      this.pdfViewer.pdfRendererModule.loadedDocument.getPage(
        parseInt(pageNumber.toString(), 10)
      );
    const pageRotation: PdfRotationAngle = page.rotation;
    return this.getAnnotationFromPDF(
      pageSize.height,
      pageSize.width,
      pageNumber,
      pageRotation
  ...
  private getAnnotationFromPDF(
    ...
    const annotRenderer: AnnotationRenderer = new AnnotationRenderer(
      this.pdfViewer,
      this.pdfViewerBase
    );
    ...
    if (loadedPage != null) {
      loadedFreetextAnnotations = this.getAllFreeTextAnnotations(
        ...
        for (let i: number = 0; i < loadedPage.annotations.count; i++) {
          ...
          if (annotation instanceof PdfTextMarkupAnnotation) {
            const textMarkup: TextMarkupAnnotationBase =
              annotRenderer.loadTextMarkupAnnotation(
                annotation as PdfTextMarkupAnnotation,
                height,
                width,
                pageRotation,
                loadedPage
            ...
            this.textMarkupAnnotationList[
              this.textMarkupAnnotationList.length
            ] = textMarkup;
            this.annotationOrder[this.annotationOrder.length] = textMarkup;
            const name: string =
              this.textMarkupAnnotationList[
                this.textMarkupAnnotationList.length - 1
              ].AnnotName;
            if (isNullOrUndefined(name) || name === '') {
              this.textMarkupAnnotationList[
                this.textMarkupAnnotationList.length - 1
              ].AnnotName = this.setAnnotationName(pageNumber);
            }
          } else if (annotation instanceof PdfLineAnnotation) {
            const shapeLabel: PdfFreeTextAnnotation = this.getShapeFreeText(
              (annotation as PdfLineAnnotation).name,
              loadedFreetextAnnotations
            );
            if (!isNullOrUndefined(shapeLabel)) {
              textLabelCollection.push(shapeLabel.name);
            }
            const shapes: ShapeAnnotationBase =
              annotRenderer.loadLineAnnotation(
            ...
            if (!isNullOrUndefined(shapes)) {
              if (shapes instanceof MeasureShapeAnnotationBase) {
                this.measureAnnotationList[this.measureAnnotationList.length] =
                  shapes;
                this.annotationOrder[this.annotationOrder.length] = shapes;
              } else if (shapes instanceof ShapeAnnotationBase) {
                this.shapeAnnotationList[this.shapeAnnotationList.length] =
                  shapes;
                this.annotationOrder[this.annotationOrder.length] = shapes;
              }
            }
            ...
            const shapeLabel: PdfFreeTextAnnotation = this.getShapeFreeText(
              (annotation as PdfSquareAnnotation).name,
              loadedFreetextAnnotations
            );
            if (!isNullOrUndefined(shapeLabel)) {
              textLabelCollection.push(shapeLabel.name);
            ...
            const shapes: ShapeAnnotationBase =
              annotRenderer.loadSquareAnnotation(
            ...
            const shapeLabel: PdfFreeTextAnnotation = this.getShapeFreeText(
              (annotation as PdfCircleAnnotation).name,
              loadedFreetextAnnotations
            );
            if (!isNullOrUndefined(shapeLabel)) {
              textLabelCollection.push(shapeLabel.name);
            ...
            const shapes: ShapeAnnotationBase =
              annotRenderer.loadEllipseAnnotation(
                annotation as PdfCircleAnnotation,
                height,
                width,
                pageRotation,
                shapeLabel
            ...
            const shapeLabel: PdfFreeTextAnnotation = this.getShapeFreeText(
              (annotation as PdfPolygonAnnotation).name,
              loadedFreetextAnnotations
            );
            if (!isNullOrUndefined(shapeLabel)) {
              textLabelCollection.push(shapeLabel.name);
            ...
            const shapes: ShapeAnnotationBase =
              annotRenderer.loadPolygonAnnotation(
                annotation as PdfPolygonAnnotation,
                height,
                width,
                pageRotation,
                shapeLabel
            ...
            annotation instanceof PdfAngleMeasurementAnnotation
          ) {
            const shapeLabel: PdfFreeTextAnnotation = this.getShapeFreeText(
              (annotation as PdfPolyLineAnnotation).name,
              loadedFreetextAnnotations
            );
            if (!isNullOrUndefined(shapeLabel)) {
              textLabelCollection.push(shapeLabel.name);
            ...
            const shapes: ShapeAnnotationBase =
              annotRenderer.loadPolylineAnnotation(
                annotation as PdfPolyLineAnnotation,
                height,
                width,
                pageRotation,
                shapeLabel
          ...
          if (annotation instanceof PdfRubberStampAnnotation) {
            ...
            if (
              ...
              this.checkName(stampAnnotation)
            ) {
              this.signatureAnnotationList.push(
                annotRenderer.loadSignatureImage(stampAnnotation, pageNumber)
              );
            } else if (
              stampAnnotation._dictionary.has('M') ||
              stampAnnotation._dictionary.has('NM') ||
              (stampAnnotation._dictionary.has('Name') &&
                !stampAnnotation._dictionary.has('F')) ||
              (!stampAnnotation._dictionary.has('NM') &&
                !stampAnnotation._dictionary.has('T'))
            ) {
              const rubberStampAnnotation: StampAnnotationBase =
                new StampAnnotationBase();
              ...
              if (
                rubberStampAnnotation.AnnotName === '' ||
                rubberStampAnnotation.AnnotName === null
              ) {
                rubberStampAnnotation.AnnotName =
                  this.setAnnotationName(pageNumber);
              }
              if (annotation._dictionary.has('rotateAngle')) {
                const rotateAngle: any =
                  annotation._dictionary.get('rotateAngle');
                if (rotateAngle !== undefined) {
                  // The rotateAngle will get as 1, 2 and 3 To save in RotateAngle multiplying with 90.
                  rubberStampAnnotation.RotateAngle =
                    parseInt(rotateAngle[0], 10) * 90;
                }
              } else {
                // If the rotate angle is not specified in the annotation dictionary, then calculate it based on the page rotation.
                rubberStampAnnotation.RotateAngle =
                  360 - (Math.abs(stampAnnotation.rotate) - pageRotation * 90);
                const rubberStampAnnotationAngle: number =
                  rubberStampAnnotation.RotateAngle;
                if (rubberStampAnnotation.RotateAngle >= 360) {
                  rubberStampAnnotation.RotateAngle =
                    rubberStampAnnotationAngle - 360;
                }
              }
              let isBoundsEqual: boolean = false;
              if (rubberStampAnnotation.RotateAngle !== 0) {
                isBoundsEqual =
                  Math.ceil(stampAnnotation._innerTemplateBounds.x * 100) /
                    100 ===
                    Math.ceil(stampAnnotation.bounds.x * 100) / 100 &&
                  Math.ceil(stampAnnotation._innerTemplateBounds.y * 100) /
                    100 ===
                    Math.ceil(stampAnnotation.bounds.y * 100) / 100 &&
                  Math.ceil(stampAnnotation._innerTemplateBounds.width * 100) /
                    100 ===
                    Math.ceil(stampAnnotation.bounds.width * 100) / 100 &&
                  Math.ceil(stampAnnotation._innerTemplateBounds.height * 100) /
                    100 ===
                    Math.ceil(stampAnnotation.bounds.height * 100) / 100;
              }
              if (
                (rubberStampAnnotation.RotateAngle !== 0 && isBoundsEqual) ||
                rubberStampAnnotation.RotateAngle === 0
              ) {
                rubberStampAnnotation.Rect = this.getBounds(
                  stampAnnotation.bounds,
                  height,
                  width,
                  pageRotation
                );
              } else {
                const bounds: Rect = this.getRubberStampBounds(
                  stampAnnotation._innerTemplateBounds,
                  stampAnnotation.bounds,
                  height,
                  width,
                  pageRotation
              ...
              if (!isNullOrUndefined(stampAnnotation.modifiedDate)) {
                rubberStampAnnotation.ModifiedDate = this.formatDate(
                  stampAnnotation.modifiedDate
                );
              } else {
                rubberStampAnnotation.ModifiedDate = this.formatDate(
                  new Date()
              ...
              if (
                Object.prototype.hasOwnProperty.call(
                  stampAnnotation._dictionary,
                  'iconName'
                )
              ) {
                rubberStampAnnotation.IconName =
                  stampAnnotation.getValues('iconName')[0];
              ...
              annotRenderer.updateIsLockProperty(
              ...
              if (!isNullOrUndefined(stampAnnotation.reviewHistory)) {
                for (
                  let i: number = 0;
                  i < stampAnnotation.reviewHistory.count;
                  i++
                ) {
                  rubberStampAnnotation.State = this.getStateString(
                    stampAnnotation.reviewHistory.at(parseInt(i.toString(), 10))
                      .state
                  );
                  rubberStampAnnotation.StateModel = this.getStateModelString(
                    stampAnnotation.reviewHistory.at(parseInt(i.toString(), 10))
                      .stateModel
                  );
                }
              ...
              if (
                rubberStampAnnotation.State == null ||
                rubberStampAnnotation.StateModel == null
              ) {
              ...
              for (let i: number = 0; i < stampAnnotation.comments.count; i++) {
                const annot: PopupAnnotationBase =
                  annotRenderer.loadPopupAnnotation(
                    stampAnnotation.comments.at(i),
                    height,
                    width,
                    pageRotation
                  );
              ...
              if (annotation._dictionary.has('Name')) {
                ...
                if (
                  annotation._dictionary.get('Name').name.includes('#23D') ||
                  annotation._dictionary.get('Name').name.includes('#D')
                ) {
                  rubberStampAnnotation.IsDynamic = true;
                }
                if (
                  !isNullOrUndefined(rubberStampAnnotation.Subject) &&
                  !this.IsStampExist(rubberStampAnnotation.Subject)
                ) {
                  rubberStampAnnotation.Subject =
              ...
              if (stampAnnotation._dictionary.has('CustomData')) {
                const customData: string =
                  stampAnnotation._dictionary.get('CustomData');
              ...
              if (isNullOrUndefined(dictionary)) {
                ...
                if (
                  ...
                  !isNullOrUndefined(pdfReference.dictionary) &&
          ...
          if (annotation instanceof PdfPopupAnnotation) {
            if (!annotation._dictionary.has('IRT')) {
              const stickyNote: PopupAnnotationBase =
                annotRenderer.loadPopupAnnotation(
                  annotation as PdfPopupAnnotation,
                  height,
                  width,
                  pageRotation
                );
              this.stickyAnnotationList[this.stickyAnnotationList.length] =
                stickyNote;
          ...
          if (annotation instanceof PdfFreeTextAnnotation) {
            const freeTextAnnot: PdfFreeTextAnnotation =
              annotation as PdfFreeTextAnnotation;
            const isFreeTextAnnotation: boolean =
              this.isFreeTextAnnotationType(freeTextAnnot);
            if (isFreeTextAnnotation) {
              const isShapeLabelAnnot: boolean = textLabelCollection.some(
                (s: string) => s === freeTextAnnot.name
              );
              const freeText: FreeTextAnnotationBase =
                annotRenderer.loadFreeTextAnnotation(
                  freeTextAnnot,
                  height,
                  width,
                  pageRotation,
              ...
              if (!freeTextAnnot._dictionary.has('T')) {
                this.signatureAnnotationList[
                  this.signatureAnnotationList.length
                ] = freeText;
                this.annotationOrder[this.annotationOrder.length] = freeText;
              }
            }
          ...
          if (annotation instanceof PdfInkAnnotation) {
            const inkAnnotation: PdfInkAnnotation =
              annotation as PdfInkAnnotation;
            const signatureData: SignatureAnnotationBase =
              annotRenderer.loadSignature(
                inkAnnotation,
                height,
                width,
                pageRotation,
            ...
            const inkSignatureData: InkSignatureAnnotation =
              annotRenderer.loadInkAnnotation(
                inkAnnotation,
                height,
                width,
                pageRotation,
            ...
            if (!inkAnnotation._dictionary.has('T')) {
              ...
              if (inkAnnotation._dictionary.has('annotationSignature')) {
                ...
                canAdd =
                  inkAnnotation.getValues('annotationSignature').length > 0
                    ? false
                    : true;
                if (canAdd) {
                  this.signatureAnnotationList.push(signatureData);
            ...
            if (
              !inkAnnotation._dictionary.has('NM') &&
              !inkAnnotation._dictionary.has('annotationSignature')
            ) {
              this.signatureAnnotationList.push(signatureData);
              this.annotationOrder.push(signatureData);
            }
   ...
   * @returns {void}
   */
  public findStampTemplate(
    annotation: PdfRubberStampAnnotation,
    rubberStampAnnotation: any,
    pageRotation: number,
    ...
    const stampDocument: PdfDocument = new PdfDocument(
      this.readFromResources()
    );
    ...
    page.graphics.drawTemplate(template, {
      x: 0,
      y: 0,
    ...
    let data: string =
      'data:application/pdf;base64,' + _encode(stampDocument.save());
    ...
    if (isFormField) {
      this.pdfViewerBase.pdfViewerRunner.postMessage({
        ...
        message: 'LoadPageStampCollection',
        password: null,
        pageIndex: 0,
        zoomFactor: this.pdfViewer.magnificationModule.zoomFactor,
        isTextNeed: false,
        isZoomMode: false,
        AnnotName: rubberStampAnnotation.AnnotName,
        rubberStampAnnotationPageNumber: rubberStampAnnotation.pageNumber,
        annotationOrder: JSON.stringify(this.annotationOrder),
  ...
  public initialPagesRendered(data: any): void {
    ...
    if (JSON.parse(data.annotationOrder).length > 0) {
      ...
      if (
        ...
        this.pdfViewer.viewerBase.importedAnnotation[
          data.rubberStampAnnotationPageNumber
        ]
      ) {
        annotOrder =
          this.pdfViewer.viewerBase.importedAnnotation[
    ...
    const currentAnnot: any = annotOrder.find(
      (currentAnnotation: { AnnotName: any }) =>
        id === currentAnnotation.AnnotName
    );
    if (currentAnnot) {
    ...
    if (!isNullOrUndefined(recordCollection)) {
      for (let i: number = 0; i < recordCollection.length; i++) {
        const element: string[] =
          recordCollection[parseInt(i.toString(), 10)]._operands;
        switch (recordCollection[parseInt(i.toString(), 10)]._operator) {
          ...
          case 'Tj': {
            this.textString =
              recordCollection[parseInt(i.toString(), 10)]._operands[0];
            const Json: any = {
              type: 'string',
          ...
          case 'Tf': {
            ...
            if (dictionary.dictionary.has('Resources')) {
              ...
              if (!isNullOrUndefined(stdic)) {
                ...
                if (
                  !isNullOrUndefined(fontObject) &&
                  recordCollection[parseInt(i.toString(), 10)]._operator ===
                    'Tf'
                ) {
                  const name: string = element[0].replace('/', '');
                  const refernceholder: any = fontObject.get(name);
                  if (
                    !isNullOrUndefined(refernceholder) &&
                    !isNullOrUndefined(refernceholder.dictionary)
                  ) {
                  ...
                  if (xObject.has(name)) {
                    ...
                    if (
                      ...
                      if (sub.dictionary.get('Subtype').name === 'Image') {
                        const imageStucture: ImageStructureBase =
                          new ImageStructureBase(sub, sub.dictionary);
                        if (!isNullOrUndefined(imageStucture)) {
                          const imageStream: Uint8Array =
                            imageStucture.getImageStream();
                          const imageString: string =
                            'data:image/png;base64,' + _encode(imageStream);
                          const Json: any = { imagedata: imageString };
                          this.htmldata.push(Json);
                          this.Imagedata = imageString;
                        }
                      } else if (
                        sub.dictionary.get('Subtype').name === 'Form'
                      ) {
                        const appearance: _PdfBaseStream =
                          sub as _PdfBaseStream;
                        const data: string = appearance.getString();
                        const content: number[] = _stringToBytes(
                          data,
                          true
                        ) as number[];
                        const parser: _ContentParser = new _ContentParser(
                          content
                        );
                        const result: _PdfRecord[] = parser._readContent();
  ...
  private getRubberStampBounds(
    ...
    if (pageRotation === 0) {
      ...
      bound = new Rect(
        centerPointX - innerTemplateBounds.width / 2,
        centerPointY - innerTemplateBounds.height / 2,
        innerTemplateBounds.width,
        innerTemplateBounds.height
      );
    } else if (pageRotation === 1) {
      const boundsX: number = this.convertPixelToPoint(
        pageWidth -
          this.convertPointToPixel(
            !isNullOrUndefined(bounds.Y) ? bounds.Y : bounds.y
          ) -
          this.convertPointToPixel(bounds.height)
      );
      const boundsY: number = !isNullOrUndefined(bounds.X)
        ? bounds.X
        : bounds.x;
      ...
      const boundsX: number = this.convertPixelToPoint(
        pageWidth -
          this.convertPointToPixel(
            !isNullOrUndefined(bounds.X) ? bounds.X : bounds.x
          ) -
          this.convertPointToPixel(bounds.width)
      );
      const boundsY: number = this.convertPixelToPoint(
        pageHeight -
          this.convertPointToPixel(
            !isNullOrUndefined(bounds.Y) ? bounds.Y : bounds.y
          ) -
      ...
      const boundsX: number = !isNullOrUndefined(bounds.Y)
        ? bounds.Y
        : bounds.y;
      const boundsY: number = this.convertPixelToPoint(
        pageHeight -
  ...
  private getShapeFreeText(
    shapeName: string,
    loadedFreetextAnnotations: PdfFreeTextAnnotation[]
  ): PdfFreeTextAnnotation {
    if (!isNullOrUndefined(shapeName) && shapeName !== '') {
      // eslint-disable-next-line
      return loadedFreetextAnnotations.find(
        (annot: PdfFreeTextAnnotation) =>
          annot.name != undefined && annot.name.includes(shapeName)
      );
    }
    return null;
  }
