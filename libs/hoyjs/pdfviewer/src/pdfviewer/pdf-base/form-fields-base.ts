...
export class FormFieldsBase {
  ...
  constructor(
    ...
    digitalSignatruePresent?: boolean
  ) {
    this.pdfViewer = pdfViewer;
    this.pdfViewerBase = pdfViewerBase;
    this.formFieldLoadedDocument =
      this.pdfViewer.pdfRendererModule.loadedDocument;
   ...
   * @returns {void}
   */
  public drawFreeTextAnnotations(
    textSignature: any,
    loadedDocument: any,
    isAnnotationFlattern: boolean
  ): void {
  ...
  public drawImage(
    ...
    if (stampObjects !== '') {
      ...
      rubberStampAnnotation._dictionary.set(
        'NM',
        signatureImage.signatureName.toString()
      );
      if (isAnnotationFlattern) {
  ...
  public saveFormFieldsDesignerData(jsonObject: any): void {
    if (Object.prototype.hasOwnProperty.call(jsonObject, 'formDesigner')) {
      ...
      if (!isNullOrUndefined(formFields)) {
        ...
        const myList: number[] = [];
        const formFieldsPageList: any = Object.prototype.hasOwnProperty.call(
          jsonObject,
          'formFieldsPageList'
        )
          ? JSON.parse(jsonObject['formFieldsPageList'])
          : myList;
        //Removing form fields from the page.
        if (!isNullOrUndefined(this.formFieldLoadedDocument.form)) {
          const initialCount: number =
            this.formFieldLoadedDocument.form._fields.length;
          //Get the loaded form.
          ...
          for (let k: number = initialCount - 1; k >= 0; k--) {
            ...
            const signed: boolean = !isNullOrUndefined(signField)
              ? signField.isSigned
              : true;
        ...
        for (let i: number = 0; i < data.length; i++) {
          this.addFormFieldsToDocument(
            data[parseInt(i.toString(), 10)].FormField
          );
        }
      }
      if (!isNullOrUndefined(this.formFieldLoadedDocument.form)) {
  ...
  public saveFormFieldsData(jsonObject: any): void {
    if (Object.prototype.hasOwnProperty.call(jsonObject, 'fieldsData')) {
      ...
      if (
        ...
        !isNullOrUndefined(this.formFieldLoadedDocument.form)
      ) {
        ...
        for (
          let i: number = 0;
          i < this.formFieldLoadedDocument.form._fields.length;
          i++
        ) {
          const currentField: PdfField =
            this.formFieldLoadedDocument.form.fieldAt(i);
          ...
          if (currentField instanceof PdfTextBoxField) {
            if (!(currentField as PdfTextBoxField).password) {
              if (
                (Object.prototype.hasOwnProperty.call(data, currentFieldName) &&
                  !isNullOrUndefined(data[`${currentFieldName}`])) ||
                (Object.prototype.hasOwnProperty.call(data, actualFieldName) &&
                ...
                if (
                  ...
                  Object.prototype.hasOwnProperty.call(field, 'isReadOnly')
                ) {
                  (currentField as PdfTextBoxField).text = field['fieldValue'];
                  (currentField as PdfTextBoxField).readOnly =
                    field['isReadOnly'] === 'true' ? true : false;
                }
              }
            } else {
              if (
            ...
            if (
              ...
              if (!isNullOrUndefined(fieldValue)) {
                (currentField as PdfCheckBoxField)._dictionary.set(
                  'ExportValue',
                  fieldValue
                );
              }
              if (
                fieldValueString.toLowerCase() === 'true' ||
                fieldValueString.toLowerCase() === 'false'
              ) {
                ...
                if (fieldValueString.toLowerCase() === 'false') {
                  const checkBoxField: PdfCheckBoxField =
                    currentField as PdfCheckBoxField;
                ...
                if (checkBoxField.itemsCount > 0) {
                  if (integerValue === -1) {
                    ...
                    !isNullOrUndefined(checkBoxField.itemAt(integerValue))
                  ) {
            ...
            if (
              ...
              if (!isNullOrUndefined(table)) {
                if (
                  Object.prototype.hasOwnProperty.call(table, 'fieldValue') &&
                  !isNullOrUndefined(table['fieldValue'])
                ) {
                  fieldName = table['fieldValue'];
                }
                if (Object.prototype.hasOwnProperty.call(table, 'isReadOnly')) {
                  currentField.readOnly =
                    table['isReadOnly'] === 'true' ? true : false;
                }
              }
              fieldName = JSON.parse(fieldName)[0].replace(
                /[^0-9a-zA-Z]+/g,
                ''
              );
                  ...
                  for (let i: number = 0; i < currentField.itemsCount; i++) {
                    ...
                    if (
                      ...
                      (item.value === selectedValue ||
                        item._optionValue === selectedValue)
                    ) {
            ...
            if (
              ...
              const signatureFields: PdfSignatureField =
                currentField as PdfSignatureField;
              if (
                Object.prototype.hasOwnProperty.call(
                  data,
                  currentFieldName + 'fontName'
                )
              ) {
                this.drawFieldFreeTextAnnotations(
                  data[`${currentFieldName}`],
                  signatureFields,
                  currentFieldName,
                  data[currentFieldName + 'bounds'],
                  data[currentFieldName + 'fontName'],
                  data[currentFieldName + 'fontSize']
                );
              } else if (
                Object.prototype.hasOwnProperty.call(
                  data,
                  currentFieldName + 'ImageData'
                )
              ) {
                this.drawFieldImage(
                  data[`${currentFieldName}`],
                ...
                for (let k: number = 0; k < signatureFieldListCount; k++) {
                  if (
                    ...
                    this.drawFieldFreeTextAnnotations(
                      ...
                      signatureFields,
                      currentFieldName,
                      data[currentFieldName + 'bounds' + '_' + k],
                      data[currentFieldName + 'fontName' + '_' + k],
                      data[currentFieldName + 'fontSize' + '_' + k]
                    );
                  } else if (
                    Object.prototype.hasOwnProperty.call(
                      data,
                      currentFieldName + 'ImageData' + '_' + k
                    )
                  ) {
                    this.drawFieldImage(
                      data[`${currentFieldName}`],
            ...
            if (
              Object.prototype.hasOwnProperty.call(
                data,
                currentFieldName + 'isReadOnly'
              ) ||
              Object.prototype.hasOwnProperty.call(
                data,
                actualFieldName + 'isReadOnly'
              )
            ) {
              if (
                Object.prototype.hasOwnProperty.call(
              ...
              (currentField as PdfSignatureField).readOnly =
                data['isReadOnly'] === 'true' ? true : false;
            }
          }
        }
      }
  ...
  private addFormFieldsToDocument(formFieldAttributes: any): void {
    const loadedPage: PdfPage = this.formFieldLoadedDocument.getPage(
      formFieldAttributes.pageNumber - 1
    ) as PdfPage;
    let field: PdfField;
  ...
  private saveTextBoxField(
    ...
    const textboxName: string = isNullOrUndefined(formFieldAttributes.name)
      ? formFieldAttributes.type === 'Password'
        ? 'passwordTextbox'
        : 'textbox'
    ...
    const fieldBounds: any = this.getBounds(
      textBounds,
      loadedPage.size[1],
      loadedPage.size[0],
      rotationAngle,
    ...
    const textbox: PdfTextBoxField = new PdfTextBoxField(
      loadedPage,
      textboxName,
      bound
    );
    textbox.backColor = [
      formFieldAttributes.backgroundColor.r,
      formFieldAttributes.backgroundColor.g,
      formFieldAttributes.backgroundColor.b,
    ];
    if (
      formFieldAttributes.backgroundColor.r === 0 &&
      formFieldAttributes.backgroundColor.g === 0 &&
      formFieldAttributes.backgroundColor.b === 0 &&
      formFieldAttributes.backgroundColor.a === 0
    ) {
      textbox.backColor = [
        formFieldAttributes.backgroundColor.r,
        formFieldAttributes.backgroundColor.g,
        formFieldAttributes.backgroundColor.b,
        formFieldAttributes.backgroundColor.a,
      ];
    }
    ...
    textbox.visibility = this.getFormFieldsVisibility(
      formFieldAttributes.visibility
    );
    textbox.text = isNullOrUndefined(formFieldAttributes.value)
      ? ''
      : formFieldAttributes.value;
    textbox.toolTip = isNullOrUndefined(formFieldAttributes.tooltip)
      ? ''
      : formFieldAttributes.tooltip;
    textbox.color = [
      formFieldAttributes.fontColor.r,
      formFieldAttributes.fontColor.g,
      formFieldAttributes.fontColor.b,
    ];
    textbox.borderColor = [
      formFieldAttributes.borderColor.r,
      formFieldAttributes.borderColor.g,
      formFieldAttributes.borderColor.b,
    ];
    // eslint-disable-next-line
    if (
      formFieldAttributes.borderColor.r == 0 &&
      formFieldAttributes.borderColor.g == 0 &&
      // eslint-disable-next-line
      formFieldAttributes.borderColor.b == 0 &&
      ...
      textbox.borderColor = [
        ...
        formFieldAttributes.borderColor.g,
        formFieldAttributes.borderColor.b,
        formFieldAttributes.borderColor.a,
      ];
    }
    ...
    if (hasUnicode) {
      textbox.font = this.getTrueFont(
        formFieldAttributes.fontSize,
        pdfFontStyle
      );
    } else {
      textbox.font = new PdfStandardFont(
        this.getFontFamily(formFieldAttributes.FontFamily),
        this.convertPixelToPoint(formFieldAttributes.fontSize),
        pdfFontStyle
      );
    }
  ...
  private saveDropDownField(
    ...
    const dropdownListName: string = isNullOrUndefined(formFieldAttributes.name)
      ? 'dropdownList'
      : formFieldAttributes.name;
    const dropDownListbounds: {
      X: number;
      Y: number;
      Width: number;
      Height: number;
    } = this.convertFieldBounds(formFieldAttributes);
    const rotationAngle: number = loadedPage.rotation;
    let isFieldRotated: boolean = false;
    if (formFieldAttributes.rotation !== 0) {
      isFieldRotated = true;
    ...
    const comboBox: PdfComboBoxField = new PdfComboBoxField(
      loadedPage,
      dropdownListName,
      bound
    );
    ...
    for (let i: number = 0; i < formFieldAttributes.option.length; i++) {
      const item: PdfListFieldItem = new PdfListFieldItem(
        formFieldAttributes.option[parseInt(i.toString(), 10)].itemName,
    ...
    comboBox.textAlignment = this.getTextAlignment(
      formFieldAttributes.textAlign
    );
    const pdfFontStyle: PdfFontStyle = this.getFontStyle(formFieldAttributes);
    comboBox._dictionary.set('FontStyle', pdfFontStyle);
    if (hasUnicode) {
      comboBox.font = this.getTrueFont(
        formFieldAttributes.FontSize,
        pdfFontStyle
      );
    } else {
      comboBox.font = new PdfStandardFont(
        this.getFontFamily(formFieldAttributes.FontFamily),
        this.convertPixelToPoint(formFieldAttributes.FontSize),
        pdfFontStyle
      );
    ...
    comboBox.visibility = this.getFormFieldsVisibility(
      formFieldAttributes.visibility
    );
    comboBox.backColor = [
      formFieldAttributes.backgroundColor.r,
    ...
    comboBox.toolTip = isNullOrUndefined(formFieldAttributes.tooltip)
      ? ''
      : formFieldAttributes.tooltip;
    comboBox._font = new PdfStandardFont(
      this.getFontFamily(formFieldAttributes.fontFamily),
      this.convertPixelToPoint(formFieldAttributes.fontSize),
      pdfFontStyle
    );
  ...
  private SaveCheckBoxField(
    ...
    const checkboxFieldName: string =
      isNullOrUndefined(formFieldAttributes.name) &&
      formFieldAttributes.name === ''
        ? 'checkboxField'
        : formFieldAttributes.name;
    ...
    checkBoxField.visibility = this.getFormFieldsVisibility(
      formFieldAttributes.visibility
    );
    checkBoxField._dictionary.set('ExportValue', formFieldAttributes.value);
    checkBoxField.backColor = [
      formFieldAttributes.backgroundColor.r,
      formFieldAttributes.backgroundColor.g,
      formFieldAttributes.backgroundColor.b,
    ];
    ...
    checkBoxField.toolTip = isNullOrUndefined(formFieldAttributes.tooltip)
      ? ''
      : formFieldAttributes.tooltip;
    if (!isFieldRotated) {
  ...
  private saveListBoxField(
    ...
    const listBoxName: string = isNullOrUndefined(formFieldAttributes.name)
      ? 'listBox'
      : formFieldAttributes.name;
    ...
    const listBox: PdfListBoxField = new PdfListBoxField(
      loadedPage,
      listBoxName,
      bound
    );
    ...
    for (let i: number = 0; i < formFieldAttributes.option.length; i++) {
      ...
      if (!isNullOrUndefined(item && item._dictionary && !flag)) {
        item.textAlignment = this.getTextAlignment(
          formFieldAttributes.textAlign
        );
        flag = true;
      }
        ...
        if (count === 1) {
          ...
          for (let j: number = 0; j < count; j++) {
            selectedIndexes.push(
              formFieldAttributes.selectedIndex[parseInt(j.toString(), 10)]
            );
          }
    ...
    listBox.textAlignment = this.getTextAlignment(
      formFieldAttributes.textAlign
    );
    ...
    if (hasUnicode) {
      listBox.font = this.getTrueFont(
        formFieldAttributes.FontSize,
        pdfFontStyle
      );
    } else {
      listBox.font = new PdfStandardFont(
        this.getFontFamily(formFieldAttributes.fontFamily),
        this.convertPixelToPoint(formFieldAttributes.fontSize),
        pdfFontStyle
      );
    ...
    listBox.visibility = this.getFormFieldsVisibility(
      formFieldAttributes.visibility
    );
    listBox.toolTip = isNullOrUndefined(formFieldAttributes.tooltip)
      ? ''
      : formFieldAttributes.tooltip;
    if (!isFieldRotated) {
  ...
  private saveRadioButtonField(
    ...
    const fieldName: string = isNullOrUndefined(
      formFieldAttributes.radiobuttonItem[0].name
    )
      ? 'radiobuttonField'
      : formFieldAttributes.radiobuttonItem[0].name;
    ...
    for (
      let i: number = 0;
      i < formFieldAttributes.radiobuttonItem.length;
      i++
    ) {
      const radiobuttonItem: any =
        formFieldAttributes.radiobuttonItem[parseInt(i.toString(), 10)];
      const page: PdfPage = this.formFieldLoadedDocument.getPage(
        radiobuttonItem.pageNumber - 1
      ) as PdfPage;
      const radioButtonName: string = !(
        isNullOrUndefined(radiobuttonItem.value) || radiobuttonItem.value === ''
      )
      ...
      const radioButtonItem: PdfRadioButtonListItem =
        new PdfRadioButtonListItem(radioButtonName, bound, page);
      if (isFieldRotated) {
      ...
      radioButtonItem.borderColor = [
        radiobuttonItem.borderColor.r,
        radiobuttonItem.borderColor.g,
        radiobuttonItem.borderColor.b,
      ];
      // eslint-disable-next-line
      if (
        radiobuttonItem.borderColor.r == 0 &&
        radiobuttonItem.borderColor.g == 0 &&
        // eslint-disable-next-line
        radiobuttonItem.borderColor.b == 0 &&
        radiobuttonItem.borderColor.a == 0
      ) {
        radioButtonItem.borderColor = [
          radiobuttonItem.borderColor.r,
          radiobuttonItem.borderColor.g,
          radiobuttonItem.borderColor.b,
          radiobuttonItem.borderColor.a,
        ];
      }
      radioButtonItem.border.width = radiobuttonItem.thickness;
      radioButtonItem.backColor = [
        radiobuttonItem.backgroundColor.r,
        radiobuttonItem.backgroundColor.g,
        radiobuttonItem.backgroundColor.b,
      ];
      if (
        radiobuttonItem.backgroundColor.r === 0 &&
        radiobuttonItem.backgroundColor.g === 0 &&
        radiobuttonItem.backgroundColor.b === 0 &&
        radiobuttonItem.backgroundColor.a === 0
      ) {
        radioButtonItem.backColor = [
          radiobuttonItem.backgroundColor.r,
          radiobuttonItem.backgroundColor.g,
          radiobuttonItem.backgroundColor.b,
          radiobuttonItem.backgroundColor.a,
        ];
      }
      radioButtonItem.visibility = this.getFormFieldsVisibility(
        radiobuttonItem.visibility
      );
    ...
    field.toolTip = isNullOrUndefined(formFieldAttributes.tooltip)
      ? ''
      : formFieldAttributes.tooltip;
    if (isSelectedItem) {
    ...
    if (!isNullOrUndefined(formFieldAttributes.radiobuttonItem[0].customData)) {
      const customData: string = JSON.stringify(
        formFieldAttributes.radiobuttonItem[0].customData
      );
  ...
  private saveSignatureField(
    ...
    const signatureFieldName: string = isNullOrUndefined(
      formFieldAttributes.name
    )
      ? 'signatureField'
      : formFieldAttributes.name;
    const signatureFieldBounds: any =
      this.convertFieldBounds(formFieldAttributes);
    const rotationAngle: number = loadedPage.rotation;
    let isFieldRotated: boolean = false;
    if (formFieldAttributes.rotation !== 0) {
    ...
    const signatureField: PdfSignatureField = new PdfSignatureField(
      loadedPage,
      signatureFieldName,
      bound
    );
    ...
    if (formFieldAttributes.signatureType === 'Text') {
      this.drawDesignerFieldFreeTextAnnotations(
        signatureField,
        signatureFieldName,
        formFieldAttributes
      );
    } else if (formFieldAttributes.signatureType === 'Image') {
      this.drawDesignerFieldImage(
        signatureField,
        signatureFieldName,
        formFieldAttributes
      );
      ...
      if (
        !isNullOrUndefined(formFieldAttributes.value) &&
        formFieldAttributes.value !== ''
      ) {
        this.drawDesignerFieldPath(
          signatureField,
          signatureFieldName,
          formFieldAttributes
        );
  ...
  private drawDesignerFieldFreeTextAnnotations(
    ...
    if (!isNullOrUndefined(formFieldAttributes)) {
      ...
      // eslint-disable-next-line
      const hasUnicode: boolean = /[^\u0000-\u007F]/.test(
        formFieldAttributes.value
      );
      if (hasUnicode) {
        annotation.font = this.getTrueFont(
          this.convertPixelToPoint(font),
          fontStyle
        );
      } else {
        annotation.font = new PdfStandardFont(
  ...
  private drawDesignerFieldImage(
    ...
    if (!isNullOrUndefined(formFieldAttributes)) {
      ...
      const width: number = signBounds.Width;
      const height: number = signBounds.Height;
      const imageUrl: string = formFieldAttributes.value
        .toString()
        .split(',')[1];
      const rubberStampAnnotation: PdfRubberStampAnnotation =
        new PdfRubberStampAnnotation(left, top, width, height);
      const bitmap: PdfImage = new PdfBitmap(imageUrl);
      rubberStampAnnotation.appearance.normal.graphics.drawImage(
        bitmap,
        0,
        0,
        width,
      ...
      if (!isFieldRotated) {
        rubberStampAnnotation.rotationAngle = Math.abs(
          this.getRotateAngle(page.rotation)
        );
      }
    ...
    if (stampObjects.length > 0) {
      ...
      if (pageRotationAngle !== 0) {
        ...
        for (let z: number = 0; z < stampObjects.length; z++) {
          ...
          linePoints.push(
            (parseFloat(value.x) - minimumX) / newDifferenceX + left
          );
          linePoints.push(
            this.formFieldLoadedDocument.getPage(pageNumber).size[1] -
              (parseFloat(value.y) - minimumY) / newDifferenceY -
              top
          );
        }
      } else {
      ...
      if (pageRotationAngle !== 0) {
        ...
        for (let w: number = 0; w < pathCollection.length; w++) {
          const pointsCollections: any =
            pathCollection[parseInt(w.toString(), 10)];
          linePoints = [];
      ...
      inkAnnotation.rotationAngle = Math.abs(
        this.getRotateAngle(page.rotation)
      );
      this.formFieldLoadedDocument
        .getPage(pageNumber)
        .annotations.add(inkAnnotation);
    }
  }
  ...
  private getTrueFont(fontSize: number, fontStyle: PdfFontStyle): PdfFont {
    const font: PdfFont = new PdfTrueTypeFont(
      getArialFontData(),
  ...
  private convertFieldBounds(formFieldAttributes: any): {
    X: number;
    Y: number;
    Width: number;
    Height: number;
    ...
    return {
      ...
      Width: this.convertPixelToPoint(
        formFieldAttributes.lineBound.Width / zoomvalue
      ),
      Height: this.convertPixelToPoint(
        formFieldAttributes.lineBound.Height / zoomvalue
      ),
    };
  }
  ...
  private getFontStyle(formFieldAttributes?: any): FontStyle {
    ...
    if (
      !isNullOrUndefined(formFieldAttributes) &&
      !isNullOrUndefined(formFieldAttributes.font)
    ) {
  ...
  private fontConvert(font: PdfFont): any {
    return {
      Bold: font.isBold,
      FontFamily: this.getFontFamilyString(
        (font as PdfStandardFont).fontFamily
      ),
      Height: font.height,
      Italic: font.isItalic,
      Name: this.getFontFamilyString(
        (font as PdfStandardFont).fontFamily
      ).toString(),
      Size: font.size,
  ...
  public GetFormFields(): void {
    ...
    if (
      ...
      for (let i: number = 0; i < loadedForm.count; i++) {
        ...
        for (
          let j: number = 0;
          j < this.formFieldLoadedDocument.pageCount;
          j++
        ) {
        ...
        if (!isNullOrUndefined(field.page)) {
          if (field instanceof PdfTextBoxField) {
            ...
            const comboBoxField: PdfComboBoxField = loadedForm.fieldAt(
              i
            ) as PdfComboBoxField;
            ...
            if (
              ...
              this.showDigitalSignatureAppearance
            ) {
              this.mIsDigitalSignaturePresent = true;
              signatureField.flatten = true;
            } else if (
              !signatureField.isSigned ||
              !this.hideEmptyDigitalSignatureFields
            ) {
      ...
      if (textBoxField.itemsCount > 0) {
        for (let i: number = 0; i < textBoxField.itemsCount; i++) {
          ...
          if (!isNullOrUndefined(item)) {
            ...
            for (
              let k: number = 0;
              k < this.formFieldLoadedDocument.pageCount;
              k++
            ) {
            ...
            this.addTextBoxField(
              textBoxField,
              j,
              textBoxField.itemAt(i).bounds,
              textBoxField.itemAt(i).font
            );
          }
        }
      }
  ...
  private addComboBoxField(
    comboBoxField: PdfComboBoxField,
    pageNumber: number
  ): void {
    const formFields: PdfRenderedFields = new PdfRenderedFields();
    ...
    if (comboBoxField._dictionary.has('CustomData')) {
      formFields.customData = JSON.parse(
        comboBoxField._dictionary.get('CustomData')
      );
    }
    formFields.TextList = [];
    if (comboBoxField._dictionary.has('Opt')) {
      ...
      if (options.length > 0) {
        formFields.TextList = options.map((item: any) =>
          typeof item === 'string'
            ? item
            : typeof item === 'object'
            ? item[0]
  ...
  private addCheckBoxField(
    ...
    if (chkField._dictionary.has('CustomData')) {
      formFields.customData = JSON.parse(
        chkField._dictionary.get('CustomData')
      );
    }
    this.PdfRenderedFormFields.push(formFields);
  }
  ...
  private addListBoxField(
    ...
    if (itemCount > 0) {
      ...
      if (Array.isArray(selectedIndex)) {
        for (let i: number = 0; i < selectedIndex.length; i++) {
          formFields.SelectedList.push(
            selectedIndex[parseInt(i.toString(), 10)]
          );
        }
      } else {
    ...
    if (itemCount > 0) {
      if (
        Array.isArray(listBoxField.selectedIndex) &&
        Array.isArray(listBoxField.selectedValue)
      ) {
    ...
    if (listBoxField._dictionary.has('CustomData')) {
      formFields.customData = JSON.parse(
        listBoxField._dictionary.get('CustomData')
      );
    }
    this.PdfRenderedFormFields.push(formFields);
  }
  ...
  private addRadioButtonField(
    ...
    const parent: PdfRadioButtonListField =
      item._field as PdfRadioButtonListField;
    const formFields: PdfRenderedFields = new PdfRenderedFields();
        ...
        for (let i: number = 0; i < signatureField.itemsCount; i++) {
          if (!isNullOrUndefined(signatureField.itemAt(i).page)) {
            ...
            this.addSignatureField(
              signatureField,
              j,
              signatureField.itemAt(i).bounds
            );
          }
        }
      }
  ...
  private addSignatureField(
    ...
    if (!isNullOrUndefined(signatureField.backColor)) {
      formFields.BackColor = {
        R: signatureField.backColor[0],
        G: signatureField.backColor[1],
        B: signatureField.backColor[2],
      };
    }
    formFields.IsSignatureField = true;
    formFields.Rotation = signatureField.rotationAngle;
    formFields.RotationAngle = this.GetRotateAngle(
      signatureField.page.rotation
    );
    ...
    if (signatureField._dictionary.has('CustomData')) {
      formFields.customData = JSON.parse(
        signatureField._dictionary.get('CustomData')
      );
    }
    this.PdfRenderedFormFields.push(formFields);
  }
  ...
  private retrieveInkAnnotation(loadedDocument: any): void {
    ...
    for (let i: number = 0; i < loadedDocument.pageCount; i++) {
      ...
      const oldPageAnnotations: PdfAnnotationCollection =
        loadedPage.annotations;
      ...
      for (let j: number = 0; j < totalAnnotation; j++) {
        ...
        if (annotation instanceof PdfInkAnnotation) {
          ...
          const inkListY: any = [];
          if (
            inkAnnot._dictionary.has('T') &&
            !inkAnnot._dictionary.has('NM')
          ) {
            if (!isNullOrUndefined(inkAnnot.inkPointsCollection)) {
              for (
                let m: number = 0;
                m < inkAnnot.inkPointsCollection.length;
                m++
              ) {
                const inkList: number[] =
                  inkAnnot.inkPointsCollection[parseInt(m.toString(), 10)];
                for (let k: number = 0; k < inkList.length; k += 2) {
                  ...
                  if (loadedPage.rotation === PdfRotationAngle.angle90) {
                    ...
                    loadedPage.rotation === PdfRotationAngle.angle180
                  ) {
                    x = loadedPage.size[0] - inkList[k + 1];
                    y = inkList[k + 1];
                  } else if (
                    loadedPage.rotation === PdfRotationAngle.angle270
                  ) {
                    x = loadedPage.size[0] - inkList[k + 1];
                    y =
                      loadedPage.size[1] - inkList[parseInt(k.toString(), 10)];
                  } else {
            ...
            formFields.LineBounds = this.getBounds(
              bounds,
              loadedPage.size[1],
              loadedPage.size[0],
              rotationAngle,
          ...
          const inkAnnot: PdfFreeTextAnnotation =
            annotation as PdfFreeTextAnnotation;
          if (
            inkAnnot._dictionary.has('T') &&
            !inkAnnot._dictionary.has('NM') &&
            !inkAnnot._dictionary.has('M')
          ) {
            const formFields: PdfRenderedFields = new PdfRenderedFields();
          ...
          if (
            stampAnnotation._dictionary.has('T') &&
            !stampAnnotation._dictionary.has('NM') &&
            !stampAnnotation._dictionary.has('M')
          ) {
            const formFields: PdfRenderedFields = new PdfRenderedFields();
            formFields.FieldName =
              stampAnnotation._dictionary.get('T') + '_' + count;
            const dictionary: any = annotation._dictionary.get('AP');
            const pageRender: PageRenderer = new PageRenderer(
              this.pdfViewer,
              this.pdfViewerBase
            );
            formFields.LineBounds = {
            ...
            if (isNullOrUndefined(dictionary)) {
              ...
              if (
                !isNullOrUndefined(pdfReference) &&
                !isNullOrUndefined(pdfReference.dictionary as _PdfDictionary) &&
                pdfReference.dictionary.has('N')
              ) {
                const apDictionary: _PdfDictionary =
                  pdfReference.dictionary as _PdfDictionary;
