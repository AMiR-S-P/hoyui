...
import { Rect, Size } from '@hoyui/drawings';
import {
  PdfAnnotationBorder,
  PdfDocument,
  PdfPage,
...
export class AnnotationRenderer {
  ...
  public addShape(details: any, page: PdfPage): void {
    ...
    if (
      !isNullOrUndefined(shapeAnnotation.shapeAnnotationType) &&
      ...
      const lineAnnotation: PdfLineAnnotation = new PdfLineAnnotation(
        linePoints
      );
      if (!isNullOrUndefined(shapeAnnotation.note)) {
        lineAnnotation.text = shapeAnnotation.note.toString();
      }
      lineAnnotation.author =
        !isNullOrUndefined(shapeAnnotation.author) &&
        shapeAnnotation.author.toString() !== ''
          ? shapeAnnotation.author.toString()
          : 'Guest';
      lineAnnotation._dictionary.set(
        'NM',
        shapeAnnotation.annotName.toString()
      );
      ...
      lineAnnotation.rotationAngle = this.getRotateAngle(
        shapeAnnotation.rotateAngle
      );
      lineAnnotation.lineEndingStyle.begin = this.getLineEndingStyle(
        shapeAnnotation.lineHeadStart
      );
      lineAnnotation.lineEndingStyle.end = this.getLineEndingStyle(
        shapeAnnotation.lineHeadEnd
      );
      let dateValue: Date;
      if (
        !isNullOrUndefined(shapeAnnotation.modifiedDate) &&
        !isNaN(Date.parse(shapeAnnotation.modifiedDate))
      ) {
      ...
      if (commentsDetails.length > 0) {
        for (let i: number = 0; i < commentsDetails.length; i++) {
          lineAnnotation.comments.add(
            this.addCommentsCollection(
              commentsDetails[parseInt(i.toString(), 10)],
      ...
      lineAnnotation.reviewHistory.add(
        this.addReviewCollections(reviewDetails, lineAnnotation.bounds)
      );
      this.preserveIsLockProperty(shapeAnnotation, lineAnnotation);
      if (!isNullOrUndefined(shapeAnnotation.customData)) {
        lineAnnotation.setValues(
          'CustomData',
          JSON.stringify(shapeAnnotation.customData)
        );
      }
      if (
        shapeAnnotation.allowedInteractions &&
        shapeAnnotation['allowedInteractions'] != null
      ) {
        lineAnnotation.setValues(
          'AllowedInteractions',
          JSON.stringify(shapeAnnotation['allowedInteractions'])
        );
      }
      ...
      if (cropValues.x !== 0 && cropValues.y !== 0 && cropValues.x === left) {
        ...
        cropValues.x === 0 &&
        page.cropBox[2] === page.size[0] &&
        cropValues.y === page.size[1]
      ) {
        cropX = cropValues.x;
        cropY = cropValues.y;
      }
      ...
      squareAnnotation.author =
        !isNullOrUndefined(shapeAnnotation.author) &&
        shapeAnnotation.author.toString() !== ''
          ? shapeAnnotation.author.toString()
          : 'Guest';
      squareAnnotation._dictionary.set(
        'NM',
        shapeAnnotation.annotName.toString()
      );
      if (!isNullOrUndefined(shapeAnnotation.subject)) {
      ...
      squareAnnotation.rotationAngle = this.getRotateAngle(
        shapeAnnotation.rotateAngle
      );
      let dateValue: Date;
      if (
      ...
      const reviewDetails: any = shapeAnnotation.review;
      squareAnnotation.reviewHistory.add(
        this.addReviewCollections(reviewDetails, squareAnnotation.bounds)
      );
      if (
        !isNullOrUndefined(shapeAnnotation.isCloudShape) &&
        shapeAnnotation.isCloudShape
      ) {
        ...
        const rectDifferences: string[] = JSON.parse(
          shapeAnnotation.rectangleDifference
        );
      ...
      circleAnnotation.author =
        !isNullOrUndefined(shapeAnnotation.author) &&
        shapeAnnotation.author.toString() !== ''
          ? shapeAnnotation.author.toString()
          : 'Guest';
      circleAnnotation._dictionary.set(
        'NM',
        shapeAnnotation.annotName.toString()
      );
      if (!isNullOrUndefined(shapeAnnotation.subject)) {
      ...
      circleAnnotation.rotationAngle = this.getRotateAngle(
        shapeAnnotation.rotateAngle
      );
      let dateValue: Date;
      if (
      ...
      const reviewDetails: any = shapeAnnotation.review;
      circleAnnotation.reviewHistory.add(
        this.addReviewCollections(reviewDetails, circleAnnotation.bounds)
      );
      if (
        !isNullOrUndefined(shapeAnnotation.isCloudShape) &&
        shapeAnnotation.isCloudShape
      ...
      const width: number = this.convertPixelToPoint(bounds.width);
      const height: number = this.convertPixelToPoint(bounds.height);
      const polygonAnnotation: PdfPolygonAnnotation = new PdfPolygonAnnotation(
        linePoints
      );
      ...
      polygonAnnotation._dictionary.set(
        'NM',
        shapeAnnotation.annotName.toString()
      );
      if (!isNullOrUndefined(shapeAnnotation.strokeColor)) {
      ...
      polygonAnnotation.rotationAngle = this.getRotateAngle(
        shapeAnnotation.rotateAngle
      );
      let dateValue: Date;
      if (
      ...
      if (commentsDetails.length > 0) {
        for (let i: number = 0; i < commentsDetails.length; i++) {
          polygonAnnotation.comments.add(
            this.addCommentsCollection(
              commentsDetails[parseInt(i.toString(), 10)],
      ...
      const reviewDetails: any = shapeAnnotation.review;
      polygonAnnotation.reviewHistory.add(
        this.addReviewCollections(reviewDetails, polygonAnnotation.bounds)
      );
      if (
        !isNullOrUndefined(shapeAnnotation.isCloudShape) &&
        shapeAnnotation.isCloudShape
      ...
      const points: any = JSON.parse(shapeAnnotation.vertexPoints);
      const linePoints: number[] = this.getSaveVertexPoints(points, page);
      const polylineAnnotation: PdfPolyLineAnnotation =
        new PdfPolyLineAnnotation(linePoints);
      if (!isNullOrUndefined(shapeAnnotation.note)) {
        polylineAnnotation.text = shapeAnnotation.note.toString();
      }
      polylineAnnotation.author =
        !isNullOrUndefined(shapeAnnotation.author) &&
        shapeAnnotation.author.toString() !== ''
          ? shapeAnnotation.author.toString()
          : 'Guest';
      ...
      polylineAnnotation.rotationAngle = this.getRotateAngle(
        shapeAnnotation.rotateAngle
      );
      polylineAnnotation.beginLineStyle = this.getLineEndingStyle(
        shapeAnnotation.lineHeadStart
      );
      polylineAnnotation.endLineStyle = this.getLineEndingStyle(
        shapeAnnotation.lineHeadEnd
      );
      let dateValue: Date;
      if (
      ...
      if (commentsDetails.length > 0) {
        for (let i: number = 0; i < commentsDetails.length; i++) {
          polylineAnnotation.comments.add(
            this.addCommentsCollection(
              commentsDetails[parseInt(i.toString(), 10)],
      ...
      const reviewDetails: any = shapeAnnotation.review;
      polylineAnnotation.reviewHistory.add(
        this.addReviewCollections(reviewDetails, polylineAnnotation.bounds)
      );
      if (
        !isNullOrUndefined(shapeAnnotation.isCloudShape) &&
        shapeAnnotation.isCloudShape
    ...
    if (
      !isNullOrUndefined(shapeAnnotation.enableShapeLabel) &&
      shapeAnnotation.enableShapeLabel
    ) {
      const labelBounds: { [Key: string]: number } = JSON.parse(
        shapeAnnotation.labelBounds.toString()
      );
      ...
      annotation._dictionary.set(
        'NM',
        shapeAnnotation.annotName.toString() + 'freeText'
      );
      ...
      fontSize =
        !isNullOrUndefined(fontSize) && !isNaN(fontSize) && fontSize > 0
          ? fontSize
          : 16;
      const fontFamily: PdfFontFamily = this.getFontFamily(
        shapeAnnotation.fontFamily
      );
      ...
      annotation.rotationAngle = this.getRotateAngle(
        shapeAnnotation.rotateAngle
      );
      ...
      if (Object.prototype.hasOwnProperty.call(shapeAnnotation, 'thickness')) {
        if (!isNullOrUndefined(shapeAnnotation.thickness)) {
          const thickness: number = parseInt(
            shapeAnnotation.thickness.toString(),
            10
          );
      ...
      const color: { [key: string]: number } = JSON.parse(
        shapeAnnotation.labelBorderColor
      );
      const color1: number[] = [color.r, color.g, color.b];
      annotation.borderColor = color1;
      const fillColor: { [key: string]: number } = JSON.parse(
        shapeAnnotation.labelFillColor
      );
      const color2: number[] = [fillColor.r, fillColor.g, fillColor.b];
      annotation.color = color2;
      const textMarkupColor: { [key: string]: number } = JSON.parse(
        shapeAnnotation.fontColor
      );
      const color3: number[] = [
        textMarkupColor.r,
        textMarkupColor.g,
  ...
  public saveInkSignature(details: any, page: PdfPage): PdfInkAnnotation {
    ...
    const stampObjects: any = JSON.parse(
      inkSignatureAnnotation.data.toString()
    );
    const rotationAngle: number = this.getInkRotateAngle(
      page.rotation.toString()
    );
    const cropValues: PointBase = this.getCropBoxValue(page, false);
    ...
    const thickness: number = parseInt(
      inkSignatureAnnotation.thickness.toString(),
      10
    );
    ...
    const rotatedPath: Path = this.getRotatedPathForMinMax(
      drawingPath._points,
      rotationAngle
    );
    ...
    if (rotationAngle !== 0) {
      ...
      const rotatedPoints: PdfPath = this.getRotatedPath(
        linePoints,
        rotationAngle
      );
      linePoints = [];
      for (let z: number = 0; z < rotatedPoints._points.length; z += 2) {
        linePoints.push(
          (rotatedPoints._points[parseInt(z.toString(), 10)][0] - minimumX) /
            newDifferenceX +
            left
        );
        linePoints.push(
          page.size[1] -
            (rotatedPoints._points[parseInt(z.toString(), 10)][1] - minimumY) /
              newDifferenceY -
            top
        );
      }
    ...
    const colors: number[] = [
      inkSignatureAnnotation.color[0],
      inkSignatureAnnotation.color[1],
      inkSignatureAnnotation.color[2],
    ];
    const inkAnnotation: PdfInkAnnotation = new PdfInkAnnotation(
      [left, top, width, height],
      linePoints
    );
    let bound: Rect = new Rect();
    bound = new Rect(
      inkAnnotation.bounds.x,
      page.size[1] - (inkAnnotation.bounds.y + inkAnnotation.bounds.height),
      inkAnnotation.bounds.width,
    ...
    if (isNewValues > 0) {
      if (rotationAngle !== 0) {
        ...
        for (let g: number = 0; g < pathCollection.length; g++) {
          ...
          const pointsCollections: number[] =
            pathCollection[parseInt(g.toString(), 10)];
          if (pointsCollections.length > 0) {
            ...
            for (let z: number = 0; z < rotatedPoints._points.length; z += 2) {
              graphicsPoints.push(
                (rotatedPoints._points[parseInt(z.toString(), 10)][0] -
                  minimumX) /
                  newDifferenceX +
                  left
              );
              graphicsPoints.push(
                page.size[1] -
                  (rotatedPoints._points[parseInt(z.toString(), 10)][1] -
                    minimumY) /
                    newDifferenceY -
                  top
              );
    ...
    if (
      isNullOrUndefined(inkSignatureAnnotation.author) ||
      (isNullOrUndefined(inkSignatureAnnotation.author) &&
        inkSignatureAnnotation.author === '')
    ) {
      inkSignatureAnnotation.author = 'Guest';
    } else {
      inkAnnotation.author = !isNullOrUndefined(inkSignatureAnnotation.author)
        ? inkSignatureAnnotation.author.toString() !== ''
          ? inkSignatureAnnotation.author.toString()
          : 'Guest'
        : 'Guest';
    }
    if (
      !isNullOrUndefined(inkSignatureAnnotation.subject) &&
      inkSignatureAnnotation.subject !== ''
    ) {
    ...
    let dateValue: Date;
    if (
      !isNullOrUndefined(inkSignatureAnnotation.modifiedDate) &&
      !isNaN(Date.parse(inkSignatureAnnotation.modifiedDate))
    ) {
    ...
    inkAnnotation.reviewHistory.add(
      this.addReviewCollections(reviewDetails, inkAnnotation.bounds)
    );
    const commentsDetails: any = inkSignatureAnnotation.comments;
    if (commentsDetails.length > 0) {
      for (let i: number = 0; i < commentsDetails.length; i++) {
        inkAnnotation.comments.add(
          this.addCommentsCollection(
            commentsDetails[parseInt(i.toString(), 10)],
    ...
    inkAnnotation._dictionary.set(
      'NM',
      inkSignatureAnnotation.annotName.toString()
    );
    inkAnnotation.rotationAngle = this.getRotateAngle(
      inkSignatureAnnotation.rotationAngle
    );
    if (!isNullOrUndefined(inkSignatureAnnotation.customData)) {
      inkAnnotation.setValues(
        'CustomData',
        JSON.stringify(inkSignatureAnnotation.customData)
      );
    }
  ...
  public getRotatedPath(linePoints: number[], rotationAngle: number): PdfPath {
    ...
    for (let j: number = 0; j < docPath.points.length; j += 2) {
      graphicsPath.addLine(
        docPath.points[parseInt(j.toString(), 10)][0],
        docPath.points[parseInt(j.toString(), 10)][1],
        docPath.points[parseInt((j + 1).toString(), 10)][0],
        docPath.points[j + 1][1]
      );
    }
    return graphicsPath;
  }
  ...
  private getRotatedPoints(pointsCollection: any, rotationAngle: any): Path {
    ...
    for (let j: number = 0; j < pointsCollection.length; j += 2) {
      graphicsPath.moveTo(
        pointsCollection[parseInt(j.toString(), 10)],
        pointsCollection[parseInt((j + 1).toString(), 10)]
      );
      graphicsPath.lineTo(0, 0);
    }
    const rotationMatrix: [number, number, number][] =
      this.getRotationMatrix(rotationAngle);
   ...
   * @private
   */
  public getRotatedPathForMinMax(
    ...
    for (let j: number = 0; j < pointsCollection.length; j += 2) {
      graphicsPath.moveTo(
        pointsCollection[parseInt(j.toString(), 10)][0],
        pointsCollection[parseInt(j.toString(), 10)][1]
      );
      graphicsPath.lineTo(
        pointsCollection[parseInt((j + 1).toString(), 10)][0],
  ...
  public addTextMarkup(details: any, loadedDocument: PdfDocument): void {
    ...
    const pageNo: number = parseInt(
      markupAnnotation['pageNumber'].toString(),
      10
    );
    const page: PdfPage = loadedDocument.getPage(pageNo);
    const annotationtypes: PdfTextMarkupAnnotation =
      new PdfTextMarkupAnnotation();
    ...
    if (
      isNullOrUndefined(markupAnnotation.author) ||
      (isNullOrUndefined(markupAnnotation.author) &&
        markupAnnotation.author === '')
    ) {
      markupAnnotation.author = 'Guest';
    } else {
      annotation.author = !isNullOrUndefined(markupAnnotation.author)
        ? markupAnnotation.author.toString() !== ''
          ? markupAnnotation.author.toString()
          : 'Guest'
        : 'Guest';
    }
    if (
      !isNullOrUndefined(markupAnnotation.subject) &&
    ...
    if (!isNullOrUndefined(markupAnnotation.annotationRotation)) {
      (annotation as any).rotateAngle = this.getRotateAngle(
        markupAnnotation.annotationRotation
      );
    }
    let dateValue: Date;
    if (
      !isNullOrUndefined(markupAnnotation.modifiedDate) &&
    ...
    if (boundsCollection.length > 0) {
      ...
      for (let i: number = 0; i < boundsCollection.length; i++) {
        const { x, y, width, height } =
          boundsCollection[parseInt(i.toString(), 10)];
    ...
    annotation.reviewHistory.add(
      this.addReviewCollections(reviewDetails, annotation.bounds)
    );
    if (!isNullOrUndefined(markupAnnotation.color)) {
      const annotColor: any = JSON.parse(markupAnnotation.color);
      const color: number[] = [annotColor.r, annotColor.g, annotColor.b];
    ...
    if (!isNullOrUndefined(markupAnnotation.customData)) {
      annotation.setValues(
        'CustomData',
        JSON.stringify(markupAnnotation.customData)
      );
    }
    if (!isNullOrUndefined(markupAnnotation.allowedInteractions)) {
      annotation.setValues(
        'AllowedInteractions',
        JSON.stringify(markupAnnotation.allowedInteractions)
      );
    }
    if (!isNullOrUndefined(markupAnnotation.textMarkupContent)) {
      annotation._dictionary.set(
        'TextMarkupContent',
        markupAnnotation.textMarkupContent.toString()
      );
    }
  ...
  private preserveIsLockProperty(annotation: any, annotPDF: any): void {
    ...
    if (
      (!isNullOrUndefined(annotation.isLocked) && annotation.isLocked) ||
      isLock
    ) {
      isLock = true;
    }
  ...
  public addCustomStampAnnotation(details: any, page: PdfPage): void {
    ...
    const pageNo: number = parseInt(
      stampAnnotation['pageNumber'].toString(),
      10
    );
    const cropValues: PointBase = this.getCropBoxValue(page, false);
    let left: number = 0;
    ...
    const isTemplate: boolean =
      !isNullOrUndefined(stampAnnotation.template) &&
      stampAnnotation.template !== ''
        ? true
        : false;
    if (
      Object.prototype.hasOwnProperty.call(stampAnnotation, 'wrapperBounds') &&
      !isTemplate
    ) {
      const wrapperBounds: any = stampAnnotation.wrapperBounds;
      const boundsXY: Rect = this.calculateBoundsXY(
        wrapperBounds,
        bounds,
        pageNo,
        page
    ...
    if (
      !isNullOrUndefined(stampAnnotation.stampAnnotationType) &&
      stampAnnotation.stampAnnotationType === 'image' &&
      stampAnnotation.stampAnnotationPath !== ' ' &&
      !isTemplate
    ) {
      if (
    ...
    if (
      ...
      !isNullOrUndefined(stampAnnotation.annotationSettings)
    ) {
    ...
    if (
      ...
      const rubberStampAnnotation: PdfRubberStampAnnotation =
        new PdfRubberStampAnnotation(left, top, width, height);
      ...
      if (isTemplate) {
        ...
        template._size = [
          stampAnnotation.templateSize[0],
          stampAnnotation.templateSize[1],
        ];
        ...
        const imageUrl: string = stampAnnotation['stampAnnotationPath']
          .toString()
          .split(',')[1];
        ...
        if (
          ...
          const appearance: PdfTemplate =
            rubberStampAnnotation.appearance.normal;
          const state: PdfGraphicsState = graphics.save();
          appearance.graphics.drawImage(bitmap, 0, 0, width, height);
          appearance.graphics.restore(state);
        } else {
          const appearance: PdfAppearance = rubberStampAnnotation.appearance;
          const filterAnnot: any = this.pdfViewerBase.pngData.filter(
            (nameStamp: any) => nameStamp.name === stampAnnotation.annotName
          );
          const dictionary: _PdfDictionary =
            filterAnnot[0]._dictionary.get('AP');
          const pngDictionary: _PdfBaseStream = dictionary.get('N');
          appearance.normal = new PdfTemplate(
            pngDictionary,
            page._crossReference
          );
        }
        rubberStampAnnotation.rotationAngle = this.getRubberStampRotateAngle(
          page.rotation,
          rotateAngle
        );
      }
      ...
      rubberStampAnnotation._dictionary.set(
        'NM',
        stampAnnotation.annotName.toString()
      );
      let dateValue: Date;
      if (
        !isNullOrUndefined(stampAnnotation.modifiedDate) &&
        !isNaN(Date.parse(stampAnnotation.modifiedDate))
      ) {
      ...
      if (commentsDetails.length > 0) {
        for (let i: number = 0; i < commentsDetails.length; i++) {
          rubberStampAnnotation.comments.add(
            this.addCommentsCollection(
              commentsDetails[parseInt(i.toString(), 10)],
      ...
      rubberStampAnnotation.reviewHistory.add(
        this.addReviewCollections(reviewDetails, rubberStampAnnotation.bounds)
      );
      if (
        !isNullOrUndefined(stampAnnotation.author) &&
        stampAnnotation.author
      ) {
        rubberStampAnnotation.author = stampAnnotation.author.toString();
      }
      if (
        !isNullOrUndefined(stampAnnotation.subject) &&
        stampAnnotation.subject
      ) {
      ...
      if (!isNullOrUndefined(stampAnnotation.customData)) {
        rubberStampAnnotation.setValues(
          'CustomData',
          JSON.stringify(stampAnnotation.customData)
        );
      }
      ...
      const fillColor: string = !isNullOrUndefined(stampAnnotation.fillColor)
        ? stampAnnotation.fillColor.toString()
        : '#192760';
      ...
      if (icon.trim() === 'Accepted' || icon.trim() === 'Rejected') {
        graphicsPath = this.drawStampAsPath(
          stampAnnotation.stampAnnotationPath,
          rubberStampAnnotation,
          textBrush,
          stampBrush
      ...
      rubberStampAnnotation.opacity = opacity;
      rubberStampAnnotation.author =
        !isNullOrUndefined(stampAnnotation.author) &&
        stampAnnotation.author.toString() !== ''
          ? stampAnnotation.author.toString()
          : 'Guest';
      this.preserveIsLockProperty(stampAnnotation, rubberStampAnnotation);
      if (!isNullOrUndefined(stampAnnotation.customData)) {
      ...
      if (!isNullOrUndefined(stampAnnotation.rotateAngle)) {
        rubberStampAnnotation.setValues(
          'rotateAngle',
          stampAnnotation.rotateAngle.toString()
        );
      }
      if (!isNullOrUndefined(stampAnnotation.icon)) {
        rubberStampAnnotation.setValues(
          'iconName',
          stampAnnotation.icon.toString()
        );
      }
      page.annotations.add(rubberStampAnnotation);
      if (!isIconExists) {
  ...
  public addMeasure(details: any, page: PdfPage): void {
    ...
    if (
      !isNullOrUndefined(measureShapeAnnotation.shapeAnnotationType) &&
      measureShapeAnnotation.shapeAnnotationType === 'Line'
    ) {
      ...
      lineAnnotation.author =
        !isNullOrUndefined(measureShapeAnnotation.author) &&
        measureShapeAnnotation.author.toString() !== ''
          ? measureShapeAnnotation.author.toString()
          : 'Guest';
      ...
      if (
        !isNullOrUndefined(measureShapeAnnotation.borderStyle) &&
        measureShapeAnnotation.borderStyle !== ''
      ) {
        lineBorder.style = this.getBorderStyle(
          measureShapeAnnotation.borderStyle
        );
      }
      if (!isNullOrUndefined(measureShapeAnnotation.borderDashArray)) {
        lineBorder.dash = [
          measureShapeAnnotation.borderDashArray,
          measureShapeAnnotation.borderDashArray,
        ];
      }
      lineAnnotation.border = lineBorder;
      lineAnnotation.rotationAngle = this.getRotateAngle(
        measureShapeAnnotation.rotateAngle
      );
      lineAnnotation.lineEndingStyle.begin = this.getLineEndingStyle(
        measureShapeAnnotation.lineHeadStart
      );
      lineAnnotation.lineEndingStyle.end = this.getLineEndingStyle(
        measureShapeAnnotation.lineHeadEnd
      );
      let dateValue: Date;
      if (
        !isNullOrUndefined(measureShapeAnnotation.modifiedDate) &&
        !isNaN(Date.parse(measureShapeAnnotation.modifiedDate))
      ) {
        dateValue = new Date(Date.parse(measureShapeAnnotation.modifiedDate));
        lineAnnotation.modifiedDate = dateValue;
      }
      lineAnnotation.caption.type = this.getCaptionType(
      ...
      lineAnnotation._dictionary.update(
        'LLO',
        measureShapeAnnotation.leaderLineOffset
      );
      ...
      if (!isNullOrUndefined(measureDetail)) {
        lineAnnotation._dictionary.set(
          'Measure',
          this.setMeasureDictionary(measureDetail)
        );
      }
      if (!isNullOrUndefined(measureShapeAnnotation.customData)) {
        lineAnnotation.setValues(
          'CustomData',
          JSON.stringify(measureShapeAnnotation.customData)
        );
      }
      if (
        measureShapeAnnotation.allowedInteractions &&
        measureShapeAnnotation['allowedInteractions'] != null
      ) {
        lineAnnotation.setValues(
          'AllowedInteractions',
          JSON.stringify(measureShapeAnnotation['allowedInteractions'])
        );
      }
      lineAnnotation.setAppearance(true);
      page.annotations.add(lineAnnotation);
      ...
      !isNullOrUndefined(measureShapeAnnotation.shapeAnnotationType) &&
      measureShapeAnnotation.shapeAnnotationType === 'Polyline'
    ) {
      const points: any = JSON.parse(measureShapeAnnotation.vertexPoints);
      const linePoints: number[] = this.getSaveVertexPoints(points, page);
      const polylineAnnotation: PdfPolyLineAnnotation =
      ...
      polylineAnnotation._dictionary.set(
        'NM',
        measureShapeAnnotation.annotName.toString()
      );
      if (!isNullOrUndefined(measureShapeAnnotation.subject)) {
      ...
      polylineAnnotation._dictionary.set(
        'IT',
        _PdfName.get(measureShapeAnnotation.indent.toString())
      );
      if (
        !isNullOrUndefined(measureShapeAnnotation.isCloudShape) &&
        measureShapeAnnotation.isCloudShape
      ) {
        const dictionary: _PdfDictionary = new _PdfDictionary(
          page._crossReference
        );
        ...
        polylineAnnotation._dictionary.update('BE', dictionary);
        const rectDifferences: string[] = JSON.parse(
          measureShapeAnnotation.rectangleDifference
        );
        if (rectDifferences.length > 0) {
          const rd: number[] = this.getRDValues(rectDifferences);
          polylineAnnotation._dictionary.update('RD', rd);
      ...
      (!isNullOrUndefined(measureShapeAnnotation.shapeAnnotationType) &&
        measureShapeAnnotation.shapeAnnotationType === 'Polyline' &&
        measureShapeAnnotation.shapeAnnotationType === 'PolygonRadius') ||
      measureShapeAnnotation.shapeAnnotationType === 'Circle'
    ) {
      const circleMeasurementAnnotation: PdfCircleAnnotation =
        this.addCircleMeasurementAnnotation(measureShapeAnnotation, page);
      page.annotations.add(circleMeasurementAnnotation);
    } else if (
      !isNullOrUndefined(measureShapeAnnotation.shapeAnnotationType) &&
      measureShapeAnnotation.shapeAnnotationType === 'Polygon' &&
      measureShapeAnnotation.indent !== 'PolygonRadius'
    ) {
      const points: any = JSON.parse(measureShapeAnnotation.vertexPoints);
      const linePoints: number[] = this.getSaveVertexPoints(points, page);
      const polygonAnnotation: PdfPolygonAnnotation = new PdfPolygonAnnotation(
      ...
      polygonAnnotation.border = lineBorder;
      polygonAnnotation._dictionary.update(
        'IT',
        _PdfName.get(measureShapeAnnotation.indent.toString())
      );
      polygonAnnotation.rotationAngle = this.getRotateAngle(
      ...
      if (
        !isNullOrUndefined(measureShapeAnnotation.isCloudShape) &&
        Boolean(measureShapeAnnotation['isCloudShape'].toString())
      ) {
        polygonAnnotation.borderEffect.style = PdfBorderEffectStyle.cloudy;
        polygonAnnotation.borderEffect.intensity =
          measureShapeAnnotation['cloudIntensity'];
        const rectDifferences: string[] = JSON.parse(
          measureShapeAnnotation.rectangleDifference
        );
        if (rectDifferences.length > 0) {
      ...
      if (!isNullOrUndefined(measureDetail)) {
        ...
        if (
          measureShapeAnnotation['indent'] === 'PolygonVolume' &&
          Object.prototype.hasOwnProperty.call(measureDetail, 'depth')
        ) {
  ...
  public addStickyNotes(details: any, page: PdfPage): void {
    ...
    let dateValue: Date;
    if (
      !isNullOrUndefined(popUpAnnotation.modifiedDate) &&
      !isNaN(Date.parse(popUpAnnotation.modifiedDate))
    ) {
    ...
    if (!isNullOrUndefined(popUpAnnotation.customData)) {
      annotation.setValues(
        'CustomData',
        JSON.stringify(popUpAnnotation.customData)
      );
    }
    page.annotations.add(annotation);
  }
  ...
  public addFreeText(
    ...
    let dateValue: Date;
    if (
      !isNullOrUndefined(freeTextAnnotation.modifiedDate) &&
      !isNaN(Date.parse(freeTextAnnotation.modifiedDate))
    ) {
    ...
    fontSize =
      !isNullOrUndefined(fontSize) && !isNaN(fontSize) && fontSize > 0
        ...
        : 16; //default 16px
    const fontFamily: PdfFontFamily = this.getFontFamily(
      freeTextAnnotation.fontFamily
    );
    let fontJson: { [key: string]: boolean } = {};
    if (
      Object.prototype.hasOwnProperty.call(freeTextAnnotation, 'font') &&
      !isNullOrUndefined(freeTextAnnotation.font)
    ) {
    ...
    if (!isNullOrUndefined(textFont) && textFont.length > 0) {
      textFont.Keys.forEach((key: string) => {
        ...
        if (
          ...
          const fontAnnotation: PdfTrueTypeFont = new PdfTrueTypeFont(
            fontStream,
            this.convertPixelToPoint(fontSize),
            PdfFontStyle.regular
          );
          const format: PdfStringFormat = new PdfStringFormat();
          fontAnnotation.measureString(
            freeTextAnnotation.dynamicText.toString(),
            format
          );
          const glyphPresent: boolean = fontAnnotation._dictionary.has(
            'IsContainsFont'
          )
            ? fontAnnotation._dictionary.get('IsContainsFont')
            : false;
          if (glyphPresent) {
            annotation.font = new PdfTrueTypeFont(
              fontStream,
              this.convertPixelToPoint(fontSize)
            );
          }
    ...
    annotation.text = '';
    if (
      Object.prototype.hasOwnProperty.call(freeTextAnnotation, 'dynamicText') &&
      !isNullOrUndefined(freeTextAnnotation.dynamicText.toString())
    ) {
      // Markup Text
      annotation.text = freeTextAnnotation.dynamicText.toString();
    }
    const rotateAngle: string =
      'RotateAngle' + Math.abs(freeTextAnnotation.rotateAngle);
    annotation.rotationAngle = this.getRotateAngle(rotateAngle);
    const lineBorder: PdfAnnotationBorder = new PdfAnnotationBorder();
    lineBorder.width = !isNullOrUndefined(freeTextAnnotation.thickness)
      ? freeTextAnnotation.thickness
      : 1;
    annotation.border = lineBorder;
    annotation.border.width = lineBorder.width;
    if (
      Object.prototype.hasOwnProperty.call(freeTextAnnotation, 'padding') &&
      !isNullOrUndefined(freeTextAnnotation.padding)
    ) {
    ...
    annotation.opacity = !isNullOrUndefined(freeTextAnnotation.opacity)
      ? freeTextAnnotation.opacity
      : 1;
    if (!isNullOrUndefined(freeTextAnnotation.strokeColor)) {
      ...
      if (!this.isTransparentColor(strokeColor)) {
        annotation.border.width = !isNullOrUndefined(
          freeTextAnnotation.thickness
        )
          ? freeTextAnnotation.thickness
          : 0;
    ...
    if (!isNullOrUndefined(freeTextAnnotation.customData)) {
      annotation.setValues(
        'CustomData',
        JSON.stringify(freeTextAnnotation.customData)
      );
    }
    if (
      Object.prototype.hasOwnProperty.call(freeTextAnnotation, 'textAlign') &&
      !isNullOrUndefined(freeTextAnnotation.textAlign)
    ) {
      annotation.textAlignment = this.getPdfTextAlignment(
        freeTextAnnotation.textAlign.toString().toLowerCase()
      );
    }
    if (
      Object.prototype.hasOwnProperty.call(
      ...
      !isNullOrUndefined(freeTextAnnotation.allowedInteractions)
    ) {
      annotation.setValues(
        'AllowedInteractions',
        JSON.stringify(freeTextAnnotation.allowedInteractions)
      );
    }
    page.annotations.add(annotation);
  }
  ...
  private renderSignHereStamp(
    rubberStampAnnotation: PdfRubberStampAnnotation,
    rectangle: Rect,
    icon: string,
    textBrush: PdfBrush,
    page: PdfPage,
    pens: PdfPen,
    graphicsPath: PdfPath
  ): void {
    ...
    if (this.defaultHeight > 0 && this.defaultWidth > 0) {
      appearance.graphics.scaleTransform(
        rectangle.width / (this.defaultWidth + 4),
    ...
    const pointValues: number[] = [
      drawingPath._points[0][0],
      drawingPath._points[0][1],
      0,
      0,
    ];
    if (graphicsPath) {
      ...
      for (let i: number = 0; i < graphicsPath._points.length; i++) {
        const point: number[] =
          graphicsPath._points[parseInt(i.toString(), 10)];
      ...
      rubberStampAnnotation.appearance.normal.graphics.drawPath(
        graphicsPath,
        pens,
        textBrush
      );
  ...
  private renderDynamicStamp(
    ...
    stringFormat.lineAlignment = PdfVerticalAlignment.middle;
    const stampFont: PdfFont = new PdfStandardFont(
      PdfFontFamily.helvetica,
      this.pdfViewer.annotationModule.calculateFontSize(
        icon.toUpperCase(),
        rectangle
      ) - 5,
      PdfFontStyle.bold | PdfFontStyle.italic
    );
    const detailsFont: PdfFont = new PdfStandardFont(
      PdfFontFamily.helvetica,
    ...
    drawingPath.addLine(point1[0], point1[1], point2[0], point2[1]);
    const stampTypeBounds: number[] = [
      drawingPath._points[0][0],
      drawingPath._points[0][1],
      0,
      0,
    ];
    const stampTimeStampbounds: number[] = [
      drawingPath._points[1][0],
      drawingPath._points[1][1],
      rectangle.width - drawingPath._points[1][0],
      rectangle.height - drawingPath._points[1][1],
    ];
    appearance.graphics.drawString(
      icon.toUpperCase(),
      stampFont,
  ...
  private calculateBoundsXY(
    ...
    if (pdfPageBase.rotation === PdfRotationAngle.angle90) {
      ...
      boundsXY.y = this.convertPixelToPoint(
        pageSize.width - wrapperBounds.x - wrapperBounds.width
      );
    } else if (pdfPageBase.rotation === PdfRotationAngle.angle180) {
      boundsXY.x = this.convertPixelToPoint(
        pageSize.width - wrapperBounds.x - wrapperBounds.width
      );
      boundsXY.y = this.convertPixelToPoint(
        pageSize.height - wrapperBounds.y - wrapperBounds.height
      );
  ...
  private getFontFamily(fontFamily: string): PdfFontFamily {
    ...
    fontFamily =
      !isNullOrUndefined(fontFamily) && fontFamily !== ''
        ? fontFamily
        : 'Helvetica';
    switch (fontFamily) {
      case 'Helvetica':
  ...
  private drawStampAsPath(
    ...
    for (let index: number = 0; index < stampObjects.length; index++) {
      ...
      if (path === 'L') {
        const array: any[] = [currentPoint, { x: val.x, y: val.y }];
        this.transformPoints(array);
        const array1: PointBase[] = [
          { x: array[0].x, y: array[0].y },
          { x: array[1].x, y: array[1].y },
        ];
        graphicsPath.addLine(
          this.convertPixelToPoint(array1[0].x),
          this.convertPixelToPoint(array1[0].y),
          this.convertPixelToPoint(array1[1].x),
      ...
      if (path === 'C') {
        ...
        graphicsPath.addBezier(
          this.convertPixelToPoint(array21[0].x),
          this.convertPixelToPoint(array21[0].y),
          this.convertPixelToPoint(array21[1].x),
          this.convertPixelToPoint(array21[1].y),
          this.convertPixelToPoint(array21[2].x),
          this.convertPixelToPoint(array21[2].y),
          this.convertPixelToPoint(array21[3].x),
          this.convertPixelToPoint(array21[3].y)
        );
        currentPoint = { x: val.x, y: val.y };
      }
      if (path === 'Z' || path === 'z') {
  ...
  private transformPoints(points: any[]): void {
    if (!isNullOrUndefined(points)) {
      for (let i: number = 0; i < points.length; i++) {
        points[parseInt(i.toString(), 10)] = this.transform(
          points[parseInt(i.toString(), 10)]
        );
      }
    }
  }
  ...
  private setMeasureDictionary(measureDetail: any): _PdfDictionary {
    ...
    if (!isNullOrUndefined(measureDetail.x)) {
      const xNumberFormat: _PdfDictionary[] = this.createNumberFormat(
        measureDetail.x
      );
      measureDictionary.set('X', xNumberFormat);
    }
    if (!isNullOrUndefined(measureDetail.distance)) {
      const dNumberFormat: _PdfDictionary[] = this.createNumberFormat(
        JSON.parse(measureDetail.distance)
      );
      measureDictionary.set('D', dNumberFormat);
    }
    if (!isNullOrUndefined(measureDetail.area)) {
      const aNumberFormat: _PdfDictionary[] = this.createNumberFormat(
        JSON.parse(measureDetail.area)
      );
      measureDictionary.set('A', aNumberFormat);
    }
    if (!isNullOrUndefined(measureDetail.angle)) {
      const tNumberFormat: _PdfDictionary[] = this.createNumberFormat(
        JSON.parse(measureDetail.angle)
      );
      measureDictionary.set('T', tNumberFormat);
    }
    if (!isNullOrUndefined(measureDetail.volume)) {
      const vNumberFormat: _PdfDictionary[] = this.createNumberFormat(
        JSON.parse(measureDetail.volume)
      );
  ...
  private createNumberFormat(numberFormatList: any): _PdfDictionary[] {
    ...
    for (let index: number = 0; index < numberFormatList.length; index++) {
      ...
      const numberFormat: any =
        numberFormatList[parseInt(index.toString(), 10)];
  ...
  private getSaveVertexPoints(points: any, page: PdfPage): number[] {
    ...
    for (let index: number = 0; index < points.length; index++) {
      const x: number =
        this.convertPixelToPoint(points[parseInt(index.toString(), 10)].x) +
        cropValues.x;
      pointList.push(x);
      const y: number =
        pageHeight -
        this.convertPixelToPoint(points[parseInt(index.toString(), 10)].y) +
        cropValues.y;
      pointList.push(y);
    }
    return pointList;
  ...
  private addReviewCollections(
    ...
    if (popupAnnotation['state'] != null) {
      annotation.state = this.getReviewState(
        popupAnnotation['state'].toString()
      );
  ...
  private addCommentsCollection(
    ...
    const reviewDetails: any = popupAnnotation.review;
    annotation.reviewHistory.add(
      this.addReviewCollections(reviewDetails, bounds)
    );
    let dateValue: Date;
    if (
      !isNullOrUndefined(popupAnnotation.modifiedDate) &&
  ...
  public loadSignature(
    ...
    if (!isNullOrUndefined(inkAnnot.inkPointsCollection)) {
      for (
        let index: number = 0;
        index < inkAnnot.inkPointsCollection.length;
        index++
      ) {
        const inkList: number[] =
          inkAnnot.inkPointsCollection[parseInt(index.toString(), 10)];
    ...
    signature.Bounds = this.getBounds(
      inkAnnot.bounds,
      height,
      width,
      pageRotation
    ...
    signature.StrokeColor =
      ...
      inkAnnot.color[2] +
      ',' +
      (inkAnnot.color[3] ? inkAnnot.color[3] : 1) +
      ')';
  ...
  public loadInkAnnotation(
    ...
    for (let i: number = 0; i < inkAnnot.reviewHistory.count; i++) {
      signature.State = this.getStateString(
        inkAnnot.reviewHistory.at(parseInt(i.toString(), 10)).state
      );
      signature.StateModel = this.getStateModelString(
        inkAnnot.reviewHistory.at(parseInt(i.toString(), 10)).stateModel
      );
    }
    if (
      isNullOrUndefined(signature.State) ||
      isNullOrUndefined(signature.StateModel)
    ) {
    ...
    for (let i: number = 0; i < inkAnnot.comments.count; i++) {
      const annot: PopupAnnotationBase = this.loadPopupAnnotation(
        inkAnnot.comments.at(i),
        height,
        width,
        pageRotation
      );
    ...
    if (inkAnnot.bounds.y < 0) {
      const cropRect: Rect = new Rect(
        inkAnnot.bounds.x,
        loadedPage.cropBox[1] + inkAnnot.bounds.y,
        inkAnnot.bounds.width,
    ...
    if (
      inkAnnot._dictionary.has('CustomData') &&
      !isNullOrUndefined(inkAnnot._dictionary.get('CustomData'))
    ) {
   ...
   * @returns {void}
   */
  public loadSquareAnnotation(
    squareAnnot: PdfSquareAnnotation,
    height: number,
    width: number,
    pageRotation: number,
    shapeFreeText: PdfFreeTextAnnotation
  ): ShapeAnnotationBase {
    ...
    shapeAnnotation.BorderStyle = this.getBorderStylesString(
      squareAnnot.border.style
    );
    shapeAnnotation.BorderDashArray = squareAnnot.border.dash
      ? squareAnnot.border.dash[0]
        ? squareAnnot.border.dash[0]
        : 0
      : 0;
    ...
    for (let i: number = 0; i < squareAnnot.reviewHistory.count; i++) {
      shapeAnnotation.State = this.getStateString(
        squareAnnot.reviewHistory.at(parseInt(i.toString(), 10)).state
      );
      shapeAnnotation.StateModel = this.getStateModelString(
        squareAnnot.reviewHistory.at(parseInt(i.toString(), 10)).stateModel
      );
    }
    if (
      isNullOrUndefined(shapeAnnotation.State) ||
      isNullOrUndefined(shapeAnnotation.StateModel)
    ) {
    ...
    shapeAnnotation.Bounds = this.getBounds(
      squareAnnot.bounds,
      height,
      width,
      pageRotation
    ...
    if (
      squareAnnot._dictionary.has('RD') &&
      !isNullOrUndefined(squareAnnot._dictionary.get('RD'))
    ) {
    ...
    if (squareAnnot._dictionary.has('AllowedInteractions')) {
      const allowedInteractions: string[] = squareAnnot.getValues(
        'AllowedInteractions'
      );
      const text: any = allowedInteractions[0];
      shapeAnnotation.AllowedInteractions = JSON.parse(text);
    }
    shapeAnnotation.StrokeColor = !isNullOrUndefined(squareAnnot.color)
      ? 'rgba(' +
        squareAnnot.color[0] +
        ',' +
        squareAnnot.color[1] +
        ...
        squareAnnot.color[2] +
        ',' +
        (squareAnnot.color[3] ? squareAnnot.color[3] : 1) +
        ')'
      : 'rgba(0,0,0,1)';
    let fillOpacity: number =
      !isNullOrUndefined(squareAnnot.color) && squareAnnot.color[3]
        ? squareAnnot.color[3]
        : 1;
    if (
      squareAnnot._dictionary.has('FillOpacity') &&
      !isNullOrUndefined(squareAnnot._dictionary.get('FillOpacity'))
    ) {
      fillOpacity = parseInt(
        squareAnnot._dictionary.get('FillOpacity').toString(),
        10
      );
    }
    fillOpacity = squareAnnot.innerColor ? fillOpacity : 0;
    squareAnnot.innerColor = squareAnnot.innerColor
      ? squareAnnot.innerColor
      : [255, 255, 255];
    shapeAnnotation.FillColor =
      'rgba(' +
      squareAnnot.innerColor[0] +
      ',' +
      squareAnnot.innerColor[1] +
      ',' +
      squareAnnot.innerColor[2] +
      ',' +
      fillOpacity +
      ')';
    ...
    if (shapeFreeText != null) {
      ...
      shapeAnnotation.LabelFillColor =
        'rgba(' +
        shapeFreeText.color[0] +
        ',' +
        shapeFreeText.color[1] +
        ',' +
        shapeFreeText.color[2] +
        ',' +
        (shapeFreeText.color[3] ? shapeFreeText.color[3] : 1) +
        ')';
      shapeAnnotation.FontColor =
        'rgba(' +
        shapeFreeText.textMarkUpColor[0] +
        ',' +
        shapeFreeText.textMarkUpColor[1] +
        ',' +
        shapeFreeText.textMarkUpColor[2] +
        ',' +
        (shapeFreeText.textMarkUpColor[3]
          ? shapeFreeText.textMarkUpColor[3]
          : 1) +
        ')';
      shapeAnnotation.LabelBorderColor =
        'rgba(' +
        shapeFreeText.borderColor[0] +
        ',' +
        shapeFreeText.borderColor[1] +
        ',' +
        shapeFreeText.borderColor[2] +
        ',' +
        (shapeFreeText.borderColor[3] ? shapeFreeText.borderColor[3] : 1) +
        ')';
      shapeAnnotation.FontSize = shapeFreeText.font.size;
    }
    if (
      squareAnnot._dictionary.has('CustomData') &&
      !isNullOrUndefined(squareAnnot._dictionary.get('CustomData'))
    ) {
   ...
   * @returns {void}
   */
  public loadLineAnnotation(
    lineAnnot: PdfLineAnnotation,
    height: number,
    width: number,
    pageRotation: number,
    shapeFreeText: PdfFreeTextAnnotation
    ...
    shapeAnnotation.BorderStyle = this.getBorderStylesString(
      lineAnnot.border.style
    );
    shapeAnnotation.BorderDashArray = lineAnnot.border.dash
      ? lineAnnot.border.dash[0]
        ? lineAnnot.border.dash[0]
        : 0
      : 0;
    ...
    for (let i: number = 0; i < lineAnnot.reviewHistory.count; i++) {
      shapeAnnotation.State = this.getStateString(
        lineAnnot.reviewHistory.at(parseInt(i.toString(), 10)).state
      );
      shapeAnnotation.StateModel = this.getStateModelString(
        lineAnnot.reviewHistory.at(parseInt(i.toString(), 10)).stateModel
      );
    }
    if (
      isNullOrUndefined(shapeAnnotation.State) ||
    ...
    shapeAnnotation.LineHeadStart = this.getLineEndingStyleString(
      lineAnnot.lineEndingStyle.begin
    );
    shapeAnnotation.LineHeadEnd = this.getLineEndingStyleString(
      lineAnnot.lineEndingStyle.end
    );
    ...
    shapeAnnotation.VertexPoints = this.getLinePoints(
      lineAnnot.linePoints,
      height,
      width,
      pageRotation,
      lineAnnot._page
    ...
    if (
      lineAnnot._dictionary.has('RD') &&
      !isNullOrUndefined(lineAnnot._dictionary.get('RD'))
    ) {
    ...
    if (lineAnnot._dictionary.has('AllowedInteractions')) {
      const allowedInteractions: string[] = lineAnnot.getValues(
        'AllowedInteractions'
      );
      const text: any = allowedInteractions[0];
      shapeAnnotation.AllowedInteractions = JSON.parse(text);
    }
    const color: number[] = !isNullOrUndefined(lineAnnot.color)
      ? lineAnnot.color
      : [0, 0, 0];
    shapeAnnotation.StrokeColor =
      ...
      ')';
    let fillOpacity: number =
      lineAnnot.color && lineAnnot.color[3] ? lineAnnot.color[3] : 1;
    if (
      lineAnnot._dictionary.has('FillOpacity') &&
      !isNullOrUndefined(lineAnnot._dictionary.get('FillOpacity'))
    ) {
      fillOpacity = parseInt(
        lineAnnot._dictionary.get('FillOpacity').toString(),
        10
      );
    }
    fillOpacity = lineAnnot.innerColor ? fillOpacity : 0;
    lineAnnot.innerColor = lineAnnot.innerColor
      ? lineAnnot.innerColor
      : [255, 255, 255];
    shapeAnnotation.FillColor =
      'rgba(' +
      lineAnnot.innerColor[0] +
      ',' +
      lineAnnot.innerColor[1] +
      ',' +
      lineAnnot.innerColor[2] +
      ',' +
      fillOpacity +
      ')';
    if (
      lineAnnot._dictionary.has('CustomData') &&
      !isNullOrUndefined(lineAnnot._dictionary.get('CustomData'))
    ) {
    ...
    if (
      lineAnnot.lineIntent === PdfLineIntent.lineArrow ||
      !lineAnnot._dictionary.has('Measure')
    ) {
      return shapeAnnotation;
    } else {
      const measureShapeAnnotation: MeasureShapeAnnotationBase =
        new MeasureShapeAnnotationBase(shapeAnnotation);
      ...
      measureShapeAnnotation.ExistingCustomData =
        shapeAnnotation.ExistingCustomData;
      if (lineAnnot._dictionary.has('LLO')) {
        measureShapeAnnotation.LeaderLineOffset =
          lineAnnot._dictionary.get('LLO');
      } else {
        measureShapeAnnotation.LeaderLineOffset = 0;
      }
      measureShapeAnnotation.CaptionPosition =
        lineAnnot.caption.type.toString();
  ...
  private getLinePoints(
    ...
    if (
      !(
        cropBox[0] === 0 &&
        (page as PdfPage).cropBox[2] === page.size[2] &&
        cropBox[1] === page.size[3]
      )
    ) {
      cropBoxX = cropBox[0];
      cropBoxY = cropBox[1];
    ...
    if (pageRotation === 0) {
      startingPoint = {
        X:
          this.convertPointToPixel(points[0]) -
          this.convertPointToPixel(cropBoxX),
        Y:
          pageHeight -
          this.convertPointToPixel(points[1]) +
          this.convertPointToPixel(cropBoxY),
      };
      endingPoint = {
        X:
          this.convertPointToPixel(points[2]) -
          this.convertPointToPixel(cropBoxX),
        Y:
          pageHeight -
          this.convertPointToPixel(points[3]) +
          this.convertPointToPixel(cropBoxY),
      };
   ...
   * @returns {void}
   */
  public loadEllipseAnnotation(
    ellipseAnnot: PdfEllipseAnnotation,
    height: number,
    width: number,
    pageRotation: number,
    shapeFreeText: PdfFreeTextAnnotation
    ...
    shapeAnnotation.BorderStyle = this.getBorderStylesString(
      ellipseAnnot.border.style
    );
    shapeAnnotation.BorderDashArray = ellipseAnnot.border.dash
      ? ellipseAnnot.border.dash[0]
        ? ellipseAnnot.border.dash[0]
        : 0
      : 0;
    shapeAnnotation.Opacity = ellipseAnnot.opacity;
    shapeAnnotation.RotateAngle = this.getRotateAngleString(
      ellipseAnnot.rotate
    );
    shapeAnnotation.AnnotType = 'shape';
    for (let i: number = 0; i < ellipseAnnot.reviewHistory.count; i++) {
      shapeAnnotation.State = this.getStateString(
        ellipseAnnot.reviewHistory.at(parseInt(i.toString(), 10)).state
      );
      shapeAnnotation.StateModel = this.getStateModelString(
        ellipseAnnot.reviewHistory.at(parseInt(i.toString(), 10)).stateModel
      );
    }
    if (
      isNullOrUndefined(shapeAnnotation.State) ||
    ...
    if (
      ellipseAnnot._dictionary.has('RD') &&
      !isNullOrUndefined(ellipseAnnot._dictionary.get('RD'))
    ) {
    ...
    if (ellipseAnnot._dictionary.has('AllowedInteractions')) {
      const allowedInteractions: string[] = ellipseAnnot.getValues(
        'AllowedInteractions'
      );
      const text: any = allowedInteractions[0];
      shapeAnnotation.AllowedInteractions = JSON.parse(text);
    ...
    shapeAnnotation.StrokeColor =
      'rgba(' +
      ellipseAnnot.color[0] +
      ',' +
      ellipseAnnot.color[1] +
      ',' +
      ellipseAnnot.color[2] +
      ',' +
      (ellipseAnnot.color[3] ? ellipseAnnot.color[3] : 1) +
      ')';
    let fillOpacity: number = ellipseAnnot.color[3] ? ellipseAnnot.color[3] : 1;
    if (
      ellipseAnnot._dictionary.has('FillOpacity') &&
      !isNullOrUndefined(ellipseAnnot._dictionary.get('FillOpacity'))
    ) {
      fillOpacity = parseInt(
        ellipseAnnot._dictionary.get('FillOpacity').toString(),
        10
      );
    }
    fillOpacity = ellipseAnnot.innerColor ? fillOpacity : 0;
    ellipseAnnot.innerColor = ellipseAnnot.innerColor
      ? ellipseAnnot.innerColor
      : [255, 255, 255];
    shapeAnnotation.FillColor =
      'rgba(' +
      ellipseAnnot.innerColor[0] +
      ',' +
      ellipseAnnot.innerColor[1] +
      ',' +
      ellipseAnnot.innerColor[2] +
      ',' +
      fillOpacity +
      ')';
    shapeAnnotation.EnableShapeLabel = false;
    ...
    if (
      ellipseAnnot._dictionary.has('CustomData') &&
      !isNullOrUndefined(ellipseAnnot._dictionary.get('CustomData'))
    ) {
    ...
    if (ellipseAnnot._dictionary.has('Measure')) {
      ...
      const measureShapeAnnotation: MeasureShapeAnnotationBase =
        new MeasureShapeAnnotationBase(shapeAnnotation);
      measureShapeAnnotation.Calibrate = this.getMeasureObject(
        ellipseAnnot as PdfAnnotation
      );
   ...
   * @returns {void}
   */
  public loadPolygonAnnotation(
    polygonAnnot: PdfPolygonAnnotation,
    height: number,
    width: number,
    pageRotation: number,
    shapeFreeText: PdfFreeTextAnnotation
    ...
    shapeAnnotation.BorderStyle = this.getBorderStylesString(
      polygonAnnot.border.style
    );
    shapeAnnotation.BorderDashArray = polygonAnnot.border.dash
      ? polygonAnnot.border.dash[0]
        ? polygonAnnot.border.dash[0]
        : 0
      : 0;
    ...
    for (let i: number = 0; i < polygonAnnot.reviewHistory.count; i++) {
      shapeAnnotation.State = this.getStateString(
        polygonAnnot.reviewHistory.at(parseInt(i.toString(), 10)).state
      );
      shapeAnnotation.StateModel = this.getStateModelString(
        polygonAnnot.reviewHistory.at(parseInt(i.toString(), 10)).stateModel
      );
    }
    if (
      isNullOrUndefined(shapeAnnotation.State) ||
    ...
    if (!isNullOrUndefined(polygonAnnot._dictionary.get('Vertices'))) {
      shapeAnnotation.VertexPoints = this.getVertexPoints(
        polygonAnnot._dictionary.get('Vertices'),
        width,
        height,
        pageRotation,
        polygonAnnot._page
    ...
    if (
      !isNullOrUndefined(shapeAnnotation.VertexPoints) &&
      shapeAnnotation.VertexPoints[0] !==
    ...
    shapeAnnotation.StrokeColor =
      'rgba(' +
      polygonAnnot.color[0] +
      ',' +
      polygonAnnot.color[1] +
      ',' +
      polygonAnnot.color[2] +
      ',' +
      (polygonAnnot.color[3] ? polygonAnnot.color[3] : 1) +
      ')';
    let fillOpacity: number = polygonAnnot.color[3] ? polygonAnnot.color[3] : 1;
    if (
      polygonAnnot._dictionary.has('FillOpacity') &&
      !isNullOrUndefined(polygonAnnot._dictionary.get('FillOpacity'))
    ) {
      fillOpacity = parseInt(
        polygonAnnot._dictionary.get('FillOpacity').toString(),
        10
      );
    }
    fillOpacity = polygonAnnot.innerColor ? fillOpacity : 0;
    polygonAnnot.innerColor = polygonAnnot.innerColor
      ? polygonAnnot.innerColor
      : [255, 255, 255];
    shapeAnnotation.FillColor =
      'rgba(' +
      polygonAnnot.innerColor[0] +
      ',' +
      polygonAnnot.innerColor[1] +
      ',' +
      polygonAnnot.innerColor[2] +
      ',' +
      fillOpacity +
      ')';
    shapeAnnotation.LineHeadStart = 'None';
    ...
    if (
      polygonAnnot._dictionary.has('RD') &&
      !isNullOrUndefined(polygonAnnot._dictionary.get('RD'))
    ) {
    ...
    if (
      polygonAnnot._dictionary.has('CustomData') &&
      !isNullOrUndefined(polygonAnnot._dictionary.get('CustomData'))
    ) {
    ...
    if (polygonAnnot._dictionary.has('AllowedInteractions')) {
      const allowedInteractions: string[] = polygonAnnot.getValues(
        'AllowedInteractions'
      );
      const text: any = allowedInteractions[0];
      shapeAnnotation.AllowedInteractions = JSON.parse(text);
    ...
    if (polygonAnnot._dictionary.has('Measure')) {
      const measureShapeAnnotation: MeasureShapeAnnotationBase =
        new MeasureShapeAnnotationBase(shapeAnnotation);
      if (
        polygonAnnot._dictionary.has('IT') &&
        !isNullOrUndefined(polygonAnnot._dictionary.get('IT'))
      ) {
      ...
      if (
        measureShapeAnnotation.Indent === 'PolygonVolume' &&
        polygonAnnot._dictionary.has('Depth') &&
        !isNullOrUndefined(polygonAnnot._dictionary.get('Depth'))
      ) {
        measureShapeAnnotation.Calibrate.Depth =
          polygonAnnot._dictionary.get('Depth');
      }
      measureShapeAnnotation.Caption = false;
      measureShapeAnnotation.LeaderLength = 0;
      measureShapeAnnotation.LeaderLineExtension = 0;
   ...
   * @returns {void}
   */
  public loadPolylineAnnotation(
    polyLineAnnot: PdfPolyLineAnnotation,
    height: number,
    width: number,
    pageRotation: number,
    shapeFreeText: PdfFreeTextAnnotation
    ...
    if (!isNullOrUndefined(polyLineAnnot.modifiedDate)) {
      shapeAnnotation.ModifiedDate = this.formatDate(
        polyLineAnnot.modifiedDate
      );
    } else {
      shapeAnnotation.ModifiedDate = this.formatDate(new Date());
    }
    ...
    shapeAnnotation.BorderStyle = this.getBorderStylesString(
      polyLineAnnot.border.style
    );
    shapeAnnotation.BorderDashArray = polyLineAnnot.border.dash
      ? polyLineAnnot.border.dash[0]
        ? polyLineAnnot.border.dash[0]
        : 0
      : 0;
    shapeAnnotation.Opacity = polyLineAnnot.opacity;
    shapeAnnotation.RotateAngle = this.getRotateAngleString(
      polyLineAnnot.rotate
    );
    shapeAnnotation.AnnotType = 'shape';
    if (!isNullOrUndefined(polyLineAnnot.reviewHistory)) {
      for (let i: number = 0; i < polyLineAnnot.reviewHistory.count; i++) {
        shapeAnnotation.State = this.getStateString(
          polyLineAnnot.reviewHistory.at(parseInt(i.toString(), 10)).state
        );

        shapeAnnotation.StateModel = this.getStateModelString(
          polyLineAnnot.reviewHistory.at(parseInt(i.toString(), 10)).stateModel
        );
      }
    }
    if (
    ...
    if (!isNullOrUndefined(polyLineAnnot._dictionary.get('Vertices'))) {
      shapeAnnotation.VertexPoints = this.getVertexPoints(
        polyLineAnnot._dictionary.get('Vertices'),
        width,
        height,
        pageRotation,
        polyLineAnnot._page
    ...
    shapeAnnotation.StrokeColor =
      'rgba(' +
      polyLineAnnot.color[0] +
      ',' +
      polyLineAnnot.color[1] +
      ',' +
      polyLineAnnot.color[2] +
      ',' +
      (polyLineAnnot.color[3] ? polyLineAnnot.color[3] : 1) +
      ')';
    let fillOpacity: number = polyLineAnnot.color[3]
      ? polyLineAnnot.color[3]
      : 1;
    if (
      polyLineAnnot._dictionary.has('FillOpacity') &&
      !isNullOrUndefined(polyLineAnnot._dictionary.get('FillOpacity'))
    ) {
      fillOpacity = parseInt(
        polyLineAnnot._dictionary.get('FillOpacity').toString(),
        10
      );
    }
    fillOpacity = polyLineAnnot.innerColor ? fillOpacity : 0;
    polyLineAnnot.innerColor = polyLineAnnot.innerColor
      ? polyLineAnnot.innerColor
      : [255, 255, 255];
    shapeAnnotation.FillColor =
      'rgba(' +
      polyLineAnnot.innerColor[0] +
      ',' +
      polyLineAnnot.innerColor[1] +
      ',' +
      polyLineAnnot.innerColor[2] +
      ',' +
      fillOpacity +
      ')';
    shapeAnnotation.LineHeadStart = this.getLineEndingStyleString(
      polyLineAnnot.beginLineStyle
    );
    shapeAnnotation.LineHeadEnd = this.getLineEndingStyleString(
      polyLineAnnot.endLineStyle
    );
    shapeAnnotation.EnableShapeLabel = false;
    if (shapeFreeText != null) {
      shapeAnnotation.EnableShapeLabel = true;
    ...
    if (
      polyLineAnnot._dictionary.has('RD') &&
      !isNullOrUndefined(polyLineAnnot._dictionary.get('RD'))
    ) {
    ...
    if (
      polyLineAnnot._dictionary.has('CustomData') &&
      !isNullOrUndefined(polyLineAnnot._dictionary.get('CustomData'))
    ) {
    ...
    if (polyLineAnnot._dictionary.has('AllowedInteractions')) {
      const allowedInteractions: string[] = polyLineAnnot.getValues(
        'AllowedInteractions'
      );
      const text: any = allowedInteractions[0];
      shapeAnnotation.AllowedInteractions = JSON.parse(text);
    ...
    if (polyLineAnnot._dictionary.has('Measure')) {
      ...
      const measureShapeAnnotation: MeasureShapeAnnotationBase =
        new MeasureShapeAnnotationBase(shapeAnnotation);
      measureShapeAnnotation.Calibrate = this.getMeasureObject(
        polyLineAnnot as PdfAnnotation
      );
      if (polyLineAnnot._dictionary.has('IT')) {
        measureShapeAnnotation.Indent =
          polyLineAnnot._dictionary.get('IT').name;
      } else {
        measureShapeAnnotation.Indent = 'PolyLineDimension';
      }
      measureShapeAnnotation.Caption = false;
   ...
   * @returns {void}
   */
  public loadSignatureImage(
    annotation: PdfRubberStampAnnotation,
    pageNumber: number
  ): SignatureAnnotationBase {
    const stampAnnotation: PdfRubberStampAnnotation =
      annotation as PdfRubberStampAnnotation;
    ...
    formsFields.Bounds = new Rect(
      stampAnnotation.bounds.x,
      stampAnnotation.bounds.y,
      stampAnnotation.bounds.width,
    ...
    formsFields.StrokeColor =
      'rgba(' +
      stampAnnotation.color +
      ',' +
      stampAnnotation.color[1] +
      ',' +
      stampAnnotation.color[2] +
      ',' +
      (stampAnnotation.color[3] ? stampAnnotation.color[3] : 1) +
      ')';
    return formsFields;
  }
  ...
  private getMeasureObject(annotation: PdfAnnotation): Measure {
    ...
    if (
      !isNullOrUndefined(measureDictionary) &&
      measureDictionary.has('TargetUnitConversion')
    ) {
      measureObject.TargetUnitConversion = measureDictionary.get(
    ...
    if (!isNullOrUndefined(arrayValues)) {
      for (let index: number = 0; index < arrayValues.length; index++) {
        ...
        if (!isNullOrUndefined(measureFormat)) {
          if (
            measureFormat.has('D') &&
            !isNullOrUndefined(measureFormat.get('D'))
          ) {
            measureValue.Denominator = measureFormat.get('D');
          }
          if (
            measureFormat.has('C') &&
            !isNullOrUndefined(measureFormat.get('C'))
          ) {
            measureValue.ConversionFactor = measureFormat.get('C');
          }
          if (
            measureFormat.has('F') &&
            !isNullOrUndefined(measureFormat.get('F'))
          ) {
            measureValue.FractionalType = measureFormat.get('F');
          }
          if (
            measureFormat.has('FD') &&
            !isNullOrUndefined(measureFormat.get('FD'))
          ) {
            measureValue.FormatDenominator = measureFormat.get('FD');
          }
          if (
            measureFormat.has('U') &&
            !isNullOrUndefined(measureFormat.get('U'))
          ) {
  ...
  private getVertexPoints(
    ...
    if (pageRotation === 0) {
      for (let i: number = 0; i < vertices.length; i++) {
        const point: AnnotPoint = {
          X:
            this.convertPointToPixel(vertices[parseInt(i.toString(), 10)]) -
            this.convertPointToPixel(cropBoxX),
          Y:
            pageHeight -
            this.convertPointToPixel(vertices[i + 1]) +
            this.convertPointToPixel(cropBoxY),
        };
        i = i + 1;
  ...
  private getBounds(
    ...
    if (pageRotation === 0) {
      bound = new AnnotBounds(
        this.convertPointToPixel(bounds.x),
        this.convertPointToPixel(bounds.y),
        this.convertPointToPixel(bounds.width),
        this.convertPointToPixel(bounds.height)
      );
    } else if (pageRotation === 1) {
      bound = new AnnotBounds(
        pageWidth -
   ...
   * @returns {void}
   */
  public loadPopupAnnotation(
    popupAnnot: PdfPopupAnnotation,
    height: number,
    width: number,
    pageRotation: number
  ): PopupAnnotationBase {
    ...
    if (
      popupAnnot._dictionary.has('Subtype') &&
      !isNullOrUndefined(popupAnnot._dictionary.get('Subtype')) &&
      !isNullOrUndefined(popupAnnot._dictionary.get('Subtype').name)
    ) {
      popupAnnotation.SubType = popupAnnot._dictionary
        .get('Subtype')
        .name.toString();
    }
    if (
      popupAnnot._dictionary.has('Type') &&
      !isNullOrUndefined(popupAnnot._dictionary.get('Type')) &&
      !isNullOrUndefined(popupAnnot._dictionary.get('Type').name)
    ) {
      popupAnnotation.Type = popupAnnot._dictionary.get('Type').name.toString();
    }
    if (
      popupAnnot._dictionary.has('IRT') &&
      !isNullOrUndefined(popupAnnot._dictionary.get('IRT'))
    ) {
    ...
    popupAnnotation.StateModel = this.getStateModelString(
      popupAnnot.stateModel
    );
    popupAnnotation.Size = new SizeBase(
      popupAnnot.bounds.width,
      popupAnnot.bounds.height
    );
    popupAnnot.color = popupAnnot.color ? popupAnnot.color : [0, 0, 0];
    popupAnnotation.Color = new AnnotColor(
      popupAnnot.color[0],
      popupAnnot.color[1],
      popupAnnot.color[2]
    );
    ...
    popupAnnotation.StrokeColor =
      'rgba(' +
      popupAnnotation.Color.R +
      ',' +
      popupAnnotation.Color.G +
      ',' +
      popupAnnotation.Color.B +
      ',' +
      1 +
      ')';
    popupAnnotation.Bounds = this.getBounds(
      popupAnnot.bounds,
      height,
      width,
      pageRotation
    ...
    for (let i: number = 0; i < popupAnnot.reviewHistory.count; i++) {
      popupAnnotation.State = this.getStateString(
        popupAnnot.reviewHistory.at(parseInt(i.toString(), 10)).state
      );
      popupAnnotation.StateModel = this.getStateModelString(
        popupAnnot.reviewHistory.at(parseInt(i.toString(), 10)).stateModel
      );
    }
    if (
      isNullOrUndefined(popupAnnotation.State) ||
      popupAnnotation.State === 'None' ||
      isNullOrUndefined(popupAnnotation.StateModel)
    ) {
    ...
    if (
      popupAnnot._dictionary.has('CustomData') &&
      !isNullOrUndefined(popupAnnot._dictionary.get('CustomData'))
    ) {
    ...
    for (let i: number = 0; i < popupAnnot.comments.count; i++) {
      popupAnnotation.Comments.push(
        this.loadPopupAnnotation(
          popupAnnot.comments.at(i),
          height,
          width,
          pageRotation
        )
   ...
   * @returns {void}
   */
  public loadFreeTextAnnotation(
    freeTextAnnot: PdfFreeTextAnnotation,
    height: number,
    width: number,
    pageRotation: number,
    page: PdfPage
  ): FreeTextAnnotationBase {
    const freeTextAnnotation: FreeTextAnnotationBase =
      new FreeTextAnnotationBase();
    freeTextAnnotation.AnnotationIntent = this.getAnnotationIntentString(
      freeTextAnnot.annotationIntent
    ); // returns wrong value
    freeTextAnnotation.AnnotationFlags = this.getAnnotationFlagsString(
      freeTextAnnot.flags
    );
    freeTextAnnotation.Author = freeTextAnnot.author;
    freeTextAnnotation.AnnotName = freeTextAnnot.name;
    if (
      isNullOrUndefined(freeTextAnnotation.AnnotName) ||
      freeTextAnnotation.AnnotName === ''
    ) {
      freeTextAnnotation.AnnotName = Math.abs(Math.random())
        .toString(36)
        .substring(2);
    }
    freeTextAnnotation.AnnotType = 'Text Box';
    freeTextAnnotation.FreeTextAnnotationType = 'Text Box';
    freeTextAnnotation.BorderColor = new AnnotColor(
      freeTextAnnot.borderColor[0],
      freeTextAnnot.borderColor[1],
    ...
    const backgroundColor: number[] = freeTextAnnot.color
      ? freeTextAnnot.color
      : [0, 0, 0];
    if (isNullOrUndefined(freeTextAnnot.color)) {
      freeTextAnnotation.IsTransparentSet = true;
    }
    freeTextAnnotation.Color = new AnnotColor(
      backgroundColor[0],
      backgroundColor[1],
      backgroundColor[2]
    );
    freeTextAnnotation.Flatten = freeTextAnnot.flatten;
    freeTextAnnotation.FlattenPopups = !isNullOrUndefined(
      freeTextAnnot.flattenPopups
    )
      ? freeTextAnnot.flattenPopups
      : false; // returns undefined
    freeTextAnnotation.FontFamily = this.getFontFamilyString(
      (freeTextAnnot.font as PdfStandardFont)._fontFamily
    );
    freeTextAnnotation.FontSize = this.convertPointToPixel(
      freeTextAnnot.font.size
    );
    freeTextAnnotation.Font = new FontBase(
      freeTextAnnot.font,
      freeTextAnnotation.FontFamily
    ); // need to be checked
    freeTextAnnotation.Thickness = freeTextAnnot.border.width;
    freeTextAnnotation.StrokeColor =
      'rgba(' +
      freeTextAnnot.borderColor[0] +
      ',' +
      freeTextAnnot.borderColor[1] +
      ...
      freeTextAnnot.borderColor[2] +
      ',' +
      (freeTextAnnot.borderColor[3] ? freeTextAnnot.borderColor[3] : 1) +
      ')';
    let fillOpacity: number;
    if (
      freeTextAnnot._dictionary.has('FillOpacity') &&
      !isNullOrUndefined(freeTextAnnot._dictionary.get('FillOpacity'))
    ) {
      fillOpacity = parseInt(
        freeTextAnnot._dictionary.get('FillOpacity').toString(),
        10
      );
    }
    fillOpacity = freeTextAnnot.color
      ? !isNullOrUndefined(fillOpacity)
        ? fillOpacity
        : 1
      : 0;
    freeTextAnnotation.FillColor =
      'rgba(' +
      backgroundColor[0] +
      ',' +
      backgroundColor[1] +
      ...
      fillOpacity +
      ')';
    freeTextAnnotation.Layer = freeTextAnnot._dictionary.has('Layer')
      ? freeTextAnnot._dictionary.get('Layer')
      : null;
    ...
    freeTextAnnotation.Location = freeTextAnnot._dictionary.has('Location')
      ? freeTextAnnot._dictionary.get('Location')
      : '{X=' + freeTextAnnot.bounds.x + ',Y=' + freeTextAnnot.bounds.y + '}';
    freeTextAnnotation.MarkupText = freeTextAnnot.text;
    if (!isNullOrUndefined(freeTextAnnot.modifiedDate)) {
      freeTextAnnotation.ModifiedDate = this.formatDate(
        freeTextAnnot.modifiedDate
      );
    } else {
    ...
    if (
      freeTextAnnot._dictionary.has('Rotation') &&
      !isNullOrUndefined(freeTextAnnot._dictionary.get('Rotation'))
    ) {
      freeTextAnnotation.Rotate = parseInt(
        freeTextAnnot._dictionary.get('Rotation'),
        10
      );
    }
    if (
      freeTextAnnot._dictionary.has('Rotate') &&
      !isNullOrUndefined(freeTextAnnot._dictionary.get('Rotate'))
    ) {
      freeTextAnnotation.Rotate = parseInt(
        freeTextAnnot._dictionary.get('Rotate'),
        10
      );
    }
    ...
    freeTextAnnotation.MarkupText = freeTextAnnot.text;
    freeTextAnnotation.TextAlign = this.getTextAlignmentString(
      freeTextAnnot.textAlignment
    );
    if (
      isNullOrUndefined(freeTextAnnotation.State) ||
      freeTextAnnotation.State === 'None' ||
      isNullOrUndefined(freeTextAnnotation.StateModel)
    ) {
      freeTextAnnotation.State = 'Unmarked';
      freeTextAnnotation.StateModel = 'None';
    }
    freeTextAnnotation.FontColor = !isNullOrUndefined(
      freeTextAnnot.textMarkUpColor
    )
      ? 'rgba(' +
        freeTextAnnot.textMarkUpColor[0] +
        ',' +
        freeTextAnnot.textMarkUpColor[1] +
        ',' +
        freeTextAnnot.textMarkUpColor[2] +
        ',' +
        (freeTextAnnot.textMarkUpColor[3]
          ? freeTextAnnot.textMarkUpColor[3]
          : 1) +
        ')'
      : 'rgba(0, 0, 0, 1)';
    for (let i: number = 0; i < freeTextAnnot.reviewHistory.count; i++) {
      freeTextAnnotation.State = this.getStateString(
        freeTextAnnot.reviewHistory.at(parseInt(i.toString(), 10)).state
      );
      freeTextAnnotation.StateModel = this.getStateModelString(
        freeTextAnnot.reviewHistory.at(parseInt(i.toString(), 10)).stateModel
      );
    }
    freeTextAnnotation.Comments = new Array<PopupAnnotationBase>();
    for (let i: number = 0; i < freeTextAnnot.comments.count; i++) {
      const annot: PopupAnnotationBase = this.loadPopupAnnotation(
        freeTextAnnot.comments.at(i),
        height,
        width,
        pageRotation
      );
    ...
    freeTextAnnotation.Bounds = this.getBounds(
    ...
    if (
      freeTextAnnot._dictionary.has('CustomData') &&
      !isNullOrUndefined(freeTextAnnot._dictionary.get('CustomData'))
    ) {
    ...
    if (freeTextAnnot._dictionary.has('AllowedInteractions')) {
      const allowedInteractions: string[] = freeTextAnnot.getValues(
        'AllowedInteractions'
      );
      const text: any = allowedInteractions[0];
   ...
   * @returns {void}
   */
  public loadSignatureText(
    inkAnnot: PdfFreeTextAnnotation,
    pageNumber: number,
    height: number,
    width: number,
    pageRotation: number
    ...
    formFields.Bounds = this.getBounds(
      inkAnnot.bounds,
      width,
      height,
      pageRotation
    ...
    formFields.FontFamily = this.getFontFamilyString(
      (inkAnnot.font as PdfStandardFont)._fontFamily
    );
    ...
    formFields.StrokeColor =
      'rgba(' +
      inkAnnot.textMarkUpColor[0] +
      ',' +
      inkAnnot.textMarkUpColor[1] +
      ',' +
      inkAnnot.textMarkUpColor[2] +
      ',' +
      (inkAnnot.textMarkUpColor[3] ? inkAnnot.textMarkUpColor[3] : 1) +
      ')';
   ...
   * @returns {void}
   */
  public removeSignatureTypeAnnot(
    ...
    if (
      ...
      (Object.prototype.hasOwnProperty.call(
        jsonObject,
        'isFormFieldAnnotationsExist'
      ) &&
        JSON.parse(jsonObject['isFormFieldAnnotationsExist']))
    ) {
      const annotationPageList: any = jsonObject.annotationsPageList
        ? jsonObject.annotationsPageList
        : [];
      const formFieldsPageList: string = jsonObject.formFieldsPageList
        ? jsonObject.formFieldsPageList
        : '[]';
      if (annotationPageList.length !== 0) {
        ...
        for (let i: number = 0; i < removeAnnotList.length; i++) {
          const loadedPageNo: string =
            removeAnnotList[parseInt(i.toString(), 10)];
          // Removing annotations from the page.
          const page: PdfPage = loadedDocument.getPage(
            parseInt(loadedPageNo, 10)
          );
          const oldPageAnnotations: PdfAnnotationCollection = page.annotations;
          const totalAnnotation: number = parseInt(
            oldPageAnnotations.count.toString(),
            10
          );
   ...
   * @returns {void}
   */
  public loadTextMarkupAnnotation(
    ...
    const markupAnnotation: TextMarkupAnnotationBase =
      new TextMarkupAnnotationBase();
    markupAnnotation.TextMarkupAnnotationType = this.getMarkupAnnotTypeString(
      textMarkup.textMarkupType
    );
    ...
    markupAnnotation.Rect = new RectangleBase(
      textMarkup.bounds.x,
      textMarkup.bounds.y,
    ...
    markupAnnotation.Color =
      ...
      (
        (1 << 24) |
        (textMarkup.color[0] << 16) |
        (textMarkup.color[1] << 8) |
        textMarkup.color[2]
      )
        .toString(16)
        .slice(1);
    ...
    const quadPoints: string[] = textMarkup._dictionary.has('QuadPoints')
      ? textMarkup._dictionary.get('QuadPoints')
      : [];
    const bounds: AnnotBounds[] = this.getTextMarkupBounds(
      quadPoints,
      height,
      width,
      pageRotation,
    ...
    for (let i: number = 0; i < textMarkup.reviewHistory.count; i++) {
      markupAnnotation.State = this.getStateString(
        textMarkup.reviewHistory.at(parseInt(i.toString(), 10)).state
      );
      markupAnnotation.StateModel = this.getStateModelString(
        textMarkup.reviewHistory.at(parseInt(i.toString(), 10)).stateModel
      );
    }
    if (
      isNullOrUndefined(markupAnnotation.State) ||
      isNullOrUndefined(markupAnnotation.StateModel)
    ) {
    ...
    if (
      textMarkup._dictionary.has('CustomData') &&
      !isNullOrUndefined(textMarkup._dictionary.get('CustomData'))
    ) {
    ...
    if (textMarkup._dictionary.has('AllowedInteractions')) {
      const allowedInteractions: string[] = textMarkup.getValues(
        'AllowedInteractions'
      );
      const text: any = allowedInteractions[0];
    ...
    if (textMarkup._dictionary.has('TextMarkupContent')) {
      const textMarkupData: string[] =
        textMarkup.getValues('TextMarkupContent');
  ...
  private getTextMarkupBounds(
    ...
    if (!isNullOrUndefined(quadPoints)) {
      for (let k: number = 0; k < quadPoints.length; k++) {
        if (pageRotation === 0) {
          x = this.convertPointToPixel(
            parseInt(quadPoints[parseInt(k.toString(), 10)], 10) - cropX
          );
          y =
            pageHeight -
            this.convertPointToPixel(parseInt(quadPoints[k + 1], 10) + cropY);
          height = this.convertPointToPixel(
            parseInt(quadPoints[k + 3], 10) - parseInt(quadPoints[k + 7], 10)
          );
          width = this.convertPointToPixel(
            parseInt(quadPoints[k + 6], 10) - parseInt(quadPoints[k + 4], 10)
          );
        } else if (pageRotation === 1) {
          x = this.convertPointToPixel(parseInt(quadPoints[k + 5], 10));
          y = this.convertPointToPixel(
            parseInt(quadPoints[parseInt(k.toString(), 10)], 10)
          );
          height = this.convertPointToPixel(
            parseInt(quadPoints[k + 6], 10) - parseInt(quadPoints[k + 4], 10)
          );
          ...
          x =
            pageWidth -
            this.convertPointToPixel(parseInt(quadPoints[k + 2], 10));
          ...
          x =
            pageWidth -
            this.convertPointToPixel(parseInt(quadPoints[k + 1], 10));
          y =
            pageHeight -
            this.convertPointToPixel(parseInt(quadPoints[k + 6], 10));
          height = this.convertPointToPixel(
            parseInt(quadPoints[k + 6], 10) - parseInt(quadPoints[k + 4], 10)
          );
          width = this.convertPointToPixel(
...
export class MeasureShapeAnnotationBase {
  ...
  constructor(shapeAnnotation: ShapeAnnotationBase) {
    ...
    this.AnnotationSelectorSettings =
      shapeAnnotation.AnnotationSelectorSettings;
