...
import { Browser, isBlazor, isNullOrUndefined } from '@hoyui/base';
import {
  DataFormat,
...
export class PdfRenderer {
  ...
  public load(
    ...
    try {
      if (jsonObject.action !== 'VirtualLoad') {
        this.loadedDocument = new PdfDocument(
          documentData,
          password ? password : ''
        );
    ...
    const jsonResult: object = this.loadDocument(
      documentData,
      documentId,
      password,
      jsonObject
  ...
  public loadImportDocument(
    ...
    try {
      if (jsonObject.action !== 'VirtualLoad') {
        this.loadImportedDocument = new PdfDocument(
          documentData,
          password ? password : ''
        );
    ...
    const jsonResult: object = {
      uniqueId: documentId,
      password: this.importedDocpassword,
    };
  ...
  public loadDocument(
    ...
    if (
      ...
      this.isCompletePageSizeNotReceieved =
        !jsonObject.isCompletePageSizeNotReceived;
    }
    ...
    this.formFieldsBase = new FormFieldsBase(
      this.pdfViewer,
      this.pdfViewerBase,
      isDigitalSignaturePresent
    );
    ...
    if (!isNullOrUndefined(this.loadedDocument)) {
      if (
        Object.prototype.hasOwnProperty.call(
          jsonObject,
          'hideEmptyDigitalSignatureFields'
        )
      ) {
        this.formFieldsBase.hideEmptyDigitalSignatureFields =
          jsonObject['hideEmptyDigitalSignatureFields'];
      }
      if (
        Object.prototype.hasOwnProperty.call(
          jsonObject,
        ...
        this.formFieldsBase.showDigitalSignatureAppearance =
          jsonObject['showDigitalSignatureAppearance'];
      }
    }
    if (
      !isNullOrUndefined(this.formFieldsBase) &&
      this.pageSizes &&
      Object.keys(this.pageSizes).length <= 100
    ) {
    ...
    if (this.formFieldsBase.mIsDigitalSignaturePresent) {
      const digitalSignatureDoc: PdfDocument = new PdfDocument(
        documentData,
        ''
      );
      const loadedForm: PdfForm = digitalSignatureDoc.form;
      if (
        !isNullOrUndefined(loadedForm) &&
        !isNullOrUndefined(loadedForm._fields)
      ) {
        for (let i: number = 0; i < loadedForm.count; i++) {
          if (loadedForm.fieldAt(i) instanceof PdfSignatureField) {
            const signatureField: PdfSignatureField = loadedForm.fieldAt(
              i
            ) as PdfSignatureField;
            if (
              signatureField.isSigned &&
              this.formFieldsBase.showDigitalSignatureAppearance
            ) {
    ...
    return {
      ...
      digitialSignatureFile: this.digitialByteArray
        ? _encode(this.digitialByteArray)
        : '',
  ...
  private documentSecurity(password: string): void {
    const isOwnerPassword: boolean =
      this.loadedDocument.isEncrypted &&
      (!this.loadedDocument.isUserPassword ||
        this.loadedDocument._hasUserPasswordOnly);
  ...
  private restrictionSummary(password: string, isOwner: boolean): void {
    ...
    const permissionList: string[] = this.getPermissionArray(
      this.loadedDocument.permissions
    );
    const isEncrypted: boolean = this.loadedDocument.isEncrypted;
    if (
      !(permissionList.length === 1 && permissionList[0] === 'Default') ||
      (isEncrypted && ownerPassword === '' && userPassword === '')
    ) {
      for (let i: number = 0; i < this.securityList.length; i++) {
        let isExist: boolean = false;
        for (let j: number = 0; j < permissionList.length; j++) {
          if (
            permissionList[parseInt(j.toString(), 10)].trim() ===
            this.securityList[parseInt(i.toString(), 10)].trim()
          ) {
            isExist = true;
            break;
          }
  ...
  public getPageSize(pageNumber: number): Size {
    ...
    if (
      ...
      return new Size(
        this.convertPointToPixel(size[0]),
        this.convertPointToPixel(size[1])
      );
    } else {
      return new Size(
        this.convertPointToPixel(size[1]),
   ...
   * @returns {void}
   */
  public getDocumentAsBase64(jsonObject: {
    [key: string]: string;
  }): Uint8Array {
    ...
    if (
      Object.prototype.hasOwnProperty.call(
        jsonObject,
        'digitalSignatureDocumentEdited'
      ) &&
      !jsonObject.digitalSignatureDocumentEdited
    ) {
      return this.loadedDocument.save();
    } else {
      const annotationRenderer: AnnotationRenderer = new AnnotationRenderer(
        this.pdfViewer,
        this.pdfViewerBase
      );
      const formfields: FormFieldsBase = new FormFieldsBase(
        this.pdfViewer,
        this.pdfViewerBase
      );
      // create object for form fields signature
      annotationRenderer.removeSignatureTypeAnnot(
        jsonObject,
        this.loadedDocument
      );
  ...
  private rearrangePages(organizePages: any): number[] {
    ...
    if (organizePages.length > 0) {
      ...
      const sortedCollection: any = clonedCollection.sort((a: any, b: any) =>
        this.pdfViewer.pageOrganizer.sorting(
          a['currentPageIndex'],
          b['currentPageIndex']
        )
      );
      for (let i: number = 0; i < sortedCollection.length; i++) {
        const currentPageDetails: any =
          sortedCollection[parseInt(i.toString(), 10)];
        if (
          !currentPageDetails.isInserted &&
          !currentPageDetails.isCopied &&
          !currentPageDetails.isImportedDoc &&
          currentPageDetails['currentPageIndex'] !== null &&
          currentPageDetails['pageIndex'] !== null &&
          parseInt(currentPageDetails['pageIndex'].toString(), 10) >= 0
        ) {
          reorderedPageIndices.push(
            parseInt(currentPageDetails['pageIndex'].toString(), 10)
          );
        }
      }
      // eslint-disable-next-line
      const deltetedPages: any = sortedCollection.filter(function (item: any) {
        return item.isDeleted && item['currentPageIndex'] === null;
      });
      for (let i: number = 0; i < deltetedPages.length; i++) {
        ...
        reorderedPageIndices = [
          ...reorderedPageIndices.slice(0, deletedPage.pageIndex),
    ...
    if (organizePages.length > 0) {
      ...
      for (let i: number = 0; i < organizePages.length; i++) {
        const pageNumber: number =
          organizePages[parseInt(i.toString(), 10)].pageIndex;
        const currentPageNumber: number =
          organizePages[parseInt(i.toString(), 10)].currentPageIndex;
        const isDeleted: boolean =
          organizePages[parseInt(i.toString(), 10)].isDeleted;
        const isInserted: boolean =
          organizePages[parseInt(i.toString(), 10)].isInserted;
        const isCopied: boolean =
          organizePages[parseInt(i.toString(), 10)].isCopied;
        const isImportedDoc: boolean =
          organizePages[parseInt(i.toString(), 10)].isImportedDoc;
        const rotation: number =
          organizePages[parseInt(i.toString(), 10)].rotateAngle;
        if (isImportedDoc) {
          importPageCount++;
        }
        if (
          !isNullOrUndefined(currentPageNumber) &&
          !isNullOrUndefined(rotation) &&
          !isDeleted &&
          !isInserted &&
          !isCopied &&
          !isImportedDoc &&
          ...
          const page: PdfPage = this.loadedDocument.getPage(
            currentPageNumber - importPageCount
          );
  ...
  private insertPdfPages(organizePages: any): void {
    if (organizePages.length > 0) {
      ...
      for (let i: number = 0; i < organizePages.length; i++) {
        ...
        let pageSize: number[];
        if (
          !isNullOrUndefined(organizePages[parseInt(i.toString(), 10)].pageSize)
        ) {
          pageSize = [
            this.convertPixelToPoint(
              organizePages[parseInt(i.toString(), 10)].pageSize.width
            ),
            this.convertPixelToPoint(
              organizePages[parseInt(i.toString(), 10)].pageSize.height
            ),
          ];
        }
        ...
        if (
          ...
          this.loadedDocument.addPage(
            currentPageNumber - copiedPageCount,
            pageSettings
          );
        }
      }
  ...
  private copyPages(organizePages: any, clonedDocument: PdfDocument): void {
    if (organizePages.length > 0) {
      ...
      for (let i: number = 0; i < organizePages.length; i++) {
        ...
        const currentPageNumber: number =
          organizePages[parseInt(i.toString(), 10)].currentPageIndex;
        const copiedPageIndex: number =
          organizePages[parseInt(i.toString(), 10)].copiedPageIndex;
        const isDeleted: boolean =
          organizePages[parseInt(i.toString(), 10)].isDeleted;
        const isInserted: boolean =
          organizePages[parseInt(i.toString(), 10)].isInserted;
    ...
    if (organizePages.length > 0) {
      for (let i: number = organizePages.length - 1; i >= 0; i--) {
        ...
        let documentData: any =
          organizePages[parseInt(i.toString(), 10)].documentData;
        const password: string =
          organizePages[parseInt(i.toString(), 10)].password;
        if (
          !isNullOrUndefined(currentPageNumber) &&
          !isDeleted &&
          !isInserted &&
          ...
          options.groupFormFields = true;
          this.loadedDocument.importPageRange(
            this.document,
            0,
            this.document.pageCount - 1,
            options
          );
        }
      }
  ...
  private deletePdfPages(organizePages: any, reorderedPages: number[]): void {
    if (organizePages.length > 0) {
      ...
      for (let i: number = 0; i < clonedCollection.length; i++) {
        if (
          clonedCollection.find((item: any) => {
            return (
              item.pageIndex === reorderedPages[parseInt(i.toString(), 10)]
            );
          })
        ) {
          sortedCollection[parseInt(i.toString(), 10)] = clonedCollection.find(
            (item: any) => {
              return (
                item.pageIndex === reorderedPages[parseInt(i.toString(), 10)]
              );
      ...
      for (let i: number = sortedCollection.length - 1; i >= 0; i--) {
        let count: number = 0;
        const pageNumber: number =
          sortedCollection[parseInt(i.toString(), 10)].pageIndex;
        const isDeleted: boolean =
          sortedCollection[parseInt(i.toString(), 10)].isDeleted;
        if (!isNullOrUndefined(pageNumber) && isDeleted) {
          ...
          for (let j: number = i - 1; j >= 0; j--) {
            if (
              !(i === sortedCollection[parseInt(i.toString(), 10)].pageIndex)
            ) {
              count++;
            }
          }
          if (count > sortedCollection[parseInt(i.toString(), 10)].pageIndex) {
            count =
              count - sortedCollection[parseInt(i.toString(), 10)].pageIndex;
    ...
    try {
      ...
      if (typeof annotationDataFormat === 'string') {
        ...
        for (let i: number = 0; i < this.loadedDocument.pageCount; i++) {
          ...
          const renderer: PageRenderer = new PageRenderer(
            this.pdfViewer,
            this.pdfViewerBase
          );
          ...
          ].textMarkupAnnotation = renderer.textMarkupAnnotationList;
          this.annotationDetailCollection[
            parseInt(i.toString(), 10)
          ].shapeAnnotation = renderer.shapeAnnotationList;
          this.annotationDetailCollection[
            parseInt(i.toString(), 10)
          ].measureShapeAnnotation = renderer.measureAnnotationList;
          this.annotationDetailCollection[
            parseInt(i.toString(), 10)
          ].stampAnnotations = renderer.rubberStampAnnotationList;
          this.annotationDetailCollection[
            parseInt(i.toString(), 10)
          ].stickyNotesAnnotation = renderer.stickyAnnotationList;
          this.annotationDetailCollection[
            parseInt(i.toString(), 10)
          ].freeTextAnnotation = renderer.freeTextAnnotationList;
          this.annotationDetailCollection[
            parseInt(i.toString(), 10)
          ].signatureAnnotation = renderer.signatureAnnotationList;
          this.annotationDetailCollection[
            parseInt(i.toString(), 10)
          ].signatureInkAnnotation = renderer.signatureInkAnnotationList;
          this.annotationDetailCollection[
            parseInt(i.toString(), 10)
          ].annotationOrder = renderer.annotationOrder;
          this.removeAnnotationsFromCollection(renderer);
        }
        const returnAnnot: any = JSON.parse(
          JSON.stringify(this.annotationDetailCollection)
        );
      ...
      switch (annotationDataFormat.toLowerCase()) {
        case 'json':
          ...
          if (isObject) {
            ...
            fileName = this.changeFileExtension(
              this.pdfViewer.fileName,
              'json'
            );
  ...
  private orderAnnotations(jsonObject: { [key: string]: string }): void {
    ...
    const signatureModule: SignatureBase = new SignatureBase(
      this.pdfViewer,
      this.pdfViewerBase
    );
    if (
      Object.prototype.hasOwnProperty.call(jsonObject, 'isAnnotationsExist') &&
      jsonObject.isAnnotationsExist
    ) {
      if (
        Object.prototype.hasOwnProperty.call(jsonObject, 'annotationCollection')
      ) {
        const annotationDetails: any = JSON.parse(
          jsonObject.annotationCollection
        );
        const count: number = annotationDetails.length;
        for (let i: number = 0; i < count; i++) {
          let annotationType: any =
            annotationDetails[parseInt(i.toString(), 10)].shapeAnnotationType;
          let details: any = annotationDetails[parseInt(i.toString(), 10)];
          if (
            Object.prototype.hasOwnProperty.call(details, 'calibrate') &&
            (details['shapeAnnotationType'] === 'Circle' ||
              details['shapeAnnotationType'] === 'Line' ||
              details['shapeAnnotationType'] === 'Polygon' ||
              details['shapeAnnotationType'] === 'Polyline')
          ) {
            ...
            (details['shapeAnnotationType'] === 'Line' ||
              details['shapeAnnotationType'] === 'Circle' ||
              details['shapeAnnotationType'] === 'Polygon' ||
              details['shapeAnnotationType'] === 'Square')
          ) {
          ...
          switch (annotationType) {
            case 'textMarkup':
              if (
                ...
                const textMarkupDetails: any = JSON.parse(
                  jsonObject.textMarkupAnnotations
                );
                const pageNumber: string = details['pageNumber'].toString();
                const annotationCount: { [key: string]: object } =
                  textMarkupDetails[parseInt(pageNumber, 10)];
                const pageAnnotations: any = annotationCount;
                const textMarkup: any = pageAnnotations.find(
                  (obj: any) =>
                    obj['annotName'].toString() ===
                    details['annotationId'].toString()
                );
                if (textMarkup) {
                  details = textMarkup;
            ...
            case 'shapeAnnotation':
              if (
                ...
                const shapeDetails: any = JSON.parse(
                  jsonObject.shapeAnnotations
                );
                const pageNumber: string = details['pageNumber'].toString();
                const annotationCount: { [key: string]: object } =
                  shapeDetails[parseInt(pageNumber, 10)];
                const pageAnnotations: any = annotationCount;
                const page: PdfPage = this.loadedDocument.getPage(
                  parseInt(pageNumber, 10)
                );
                const shape: any = pageAnnotations.find(
                  (obj: any) =>
                    obj['annotName'].toString() ===
                    details['annotationId'].toString()
                );
            ...
            case 'stamp':
              if (
                ...
                const stampdetails: any = JSON.parse(
                  jsonObject.stampAnnotations
                );
                const pageNumber: string = details['pageNumber'].toString();
                const annotationCount: { [key: string]: object } =
                  stampdetails[parseInt(pageNumber, 10)];
                const pageAnnotations: any = annotationCount;
                const page: PdfPage = this.loadedDocument.getPage(
                  parseInt(pageNumber, 10)
                );
                const stamp: any = pageAnnotations.find(
                  (obj: any) =>
                    obj['annotName'].toString() ===
                    details['annotationId'].toString()
                );
            ...
            case 'measureShapes':
              if (
                ...
                const shapeDetails: any = JSON.parse(
                  jsonObject.measureShapeAnnotations
                );
                const pageNumber: string = details['pageNumber'].toString();
                const annotationCount: { [key: string]: object } =
                  shapeDetails[parseInt(pageNumber, 10)];
            ...
            case 'sticky':
              if (
                ...
                const shapeDetails: any = JSON.parse(
                  jsonObject.stickyNotesAnnotation
                );
                const pageNumber: string = details['pageNumber'].toString();
                const annotationCount: { [key: string]: object } =
                  shapeDetails[parseInt(pageNumber, 10)];
            ...
            case 'FreeText':
              if (
                ...
                const freeTextDetails: any = JSON.parse(
                  jsonObject.freeTextAnnotation
                );
                const pageNumber: string = details['pageNumber'].toString();
                const annotationCount: { [key: string]: object } =
                  freeTextDetails[parseInt(pageNumber, 10)];
                const pageAnnotations: any = annotationCount;
                const page: PdfPage = this.loadedDocument.getPage(
                  parseInt(pageNumber, 10)
                );
                const freeText: any = pageAnnotations.find(
                  (obj: any) =>
                    obj['annotName'].toString() ===
                    details['annotationId'].toString()
                );
                if (!isNullOrUndefined(freeText)) {
                  details = freeText;
                  if (
                    !isNullOrUndefined(this.FallbackFontCollection) &&
                    this.FallbackFontCollection.length !== 0
                  ) {
                    annotationRenderer.addFreeText(
                      details,
                      page,
                      this.FallbackFontCollection
                    );
                  } else {
        ...
        if (jsonObject.signatureData) {
          if (jsonObject.isSignatureEdited) {
            signatureModule.saveSignatureAsAnnotatation(
              jsonObject,
              this.loadedDocument
            );
          } else {
  ...
  public getAnnotationComments(jsonObject: any): any {
    try {
      if (
        Object.prototype.hasOwnProperty.call(jsonObject, 'pageStartIndex') &&
        !isNullOrUndefined(jsonObject.pageStartIndex)
      ) {
        ...
        for (let i: number = pageStartIndex; i < pageEndIndex; i++) {
          ...
          annotationDetails[pageNumber.toString()] =
            this.renderer.exportAnnotationComments(pageNumber, pageSize);
        }
        return {
          uniqueId: jsonObject.uniqueId,
  ...
  public getBookmarks(jsonObject: any): any {
    try {
      ...
      if (
        !isNullOrUndefined(bookmark) &&
        Object.prototype.hasOwnProperty.call(jsonObject, 'bookmarkStyles')
      ) {
      ...
      if (
        isNullOrUndefined(bookmark) ||
        isNullOrUndefined(bookmark.count) ||
        bookmark.count === 0
      ) {
        return null;
      } else {
        for (let i: number = 0; i < bookmark.count; i++) {
          ...
          const parentBookmarkDestination: BookmarkDestination =
            new BookmarkDestination();
          const bookmarkDestination: PdfDestination =
            pdfLoadedBookmark.destination
              ? pdfLoadedBookmark.destination
              : pdfLoadedBookmark.namedDestination
              ? pdfLoadedBookmark.namedDestination.destination
                ? pdfLoadedBookmark.namedDestination.destination
                : null
              : null;
          parentBookmarkDestination.X = !isNullOrUndefined(bookmarkDestination)
            ? bookmarkDestination.location[0]
            : 0;
          parentBookmarkDestination.PageIndex = !isNullOrUndefined(
            bookmarkDestination
          )
            ? bookmarkDestination.pageIndex
            : 0;
          parentBookmarkDestination.Zoom = !isNullOrUndefined(
            bookmarkDestination
          )
            ? bookmarkDestination.zoom
            : 0;
          ...
          parentBookmark.FileName = !isNullOrUndefined(
            (pdfLoadedBookmark as any).action
          )
            ? (pdfLoadedBookmark as any).action.toString()
            : '';
          if (!isNullOrUndefined(bookmarkDestination)) {
            if (
              bookmarkDestination.page.rotation === PdfRotationAngle.angle90
            ) {
              parentBookmarkDestination.Y =
                this.convertPointToPixel(bookmarkDestination.page.size[0]) -
                this.convertPointToPixel(
                  Math.abs(bookmarkDestination.location[1])
                );
            } else if (
              bookmarkDestination.page.rotation === PdfRotationAngle.angle270
            ) {
              parentBookmarkDestination.Y = this.convertPointToPixel(
                Math.abs(bookmarkDestination.location[1])
              );
            } else {
              parentBookmarkDestination.Y =
                this.convertPointToPixel(bookmarkDestination.page.size[1]) -
                this.convertPointToPixel(
                  Math.abs(bookmarkDestination.location[1])
                );
            }
          ...
          this.bookmarkDictionary[this.id.toString()] =
            parentBookmarkDestination;
      ...
      if (Object.prototype.hasOwnProperty.call(jsonObject, 'uniqueId')) {
        return {
          ...
          BookmarksDestination: JSON.parse(
            JSON.stringify(this.bookmarkDictionary)
          ),
    ...
    if (!isNullOrUndefined(currentElement)) {
      if (!isNullOrUndefined(currentElement.color)) {
        currentStyles.Color =
          ...
          ',' +
          currentElement.color[1] +
          ',' +
          currentElement.color[2] +
          ',' +
          1 +
          ')';
      }
      currentStyles.FontStyle = this.getPdfTextStyleString(
  ...
  private getChildrenBookmark(pdfLoadedBookmark: PdfBookmark): BookmarkBase[] {
    ...
    if (
      !isNullOrUndefined(pdfLoadedBookmark) &&
      !isNullOrUndefined(pdfLoadedBookmark.count) &&
      pdfLoadedBookmark.count > 0
    ) {
      for (let i: number = 0; i < pdfLoadedBookmark.count; i++) {
        const child: PdfBookmark = pdfLoadedBookmark.at(i);
        const childBookmarkDestination: PdfDestination = child.destination
          ? child.destination
          : child.namedDestination
          ? child.namedDestination.destination
            ? child.namedDestination.destination
            : null
          : null;
        this.id++;
        const title: string = child.title;
        this.pageIndex = !isNullOrUndefined(childBookmarkDestination)
          ? childBookmarkDestination.pageIndex
          : 0;
        this.x = !isNullOrUndefined(childBookmarkDestination)
          ? childBookmarkDestination.location[0]
          : 0;
        const yPosition: number = !isNullOrUndefined(childBookmarkDestination)
          ? Math.abs(childBookmarkDestination.location[1])
          : 0;
        if (!isNullOrUndefined(childBookmarkDestination)) {
          if (
            childBookmarkDestination.page.rotation === PdfRotationAngle.angle90
          ) {
            this.y =
              this.convertPointToPixel(childBookmarkDestination.page.size[0]) -
              this.convertPointToPixel(yPosition);
          } else if (
            childBookmarkDestination.page.rotation === PdfRotationAngle.angle270
          ) {
            this.y = this.convertPointToPixel(yPosition);
          } else {
            this.y =
              this.convertPointToPixel(childBookmarkDestination.page.size[1]) -
              this.convertPointToPixel(yPosition);
          }
        ...
        childrenBookmark.FileName = !isNullOrUndefined((child as any).action)
          ? (child as any).action.toString()
          : '';
        const childrenBookmarkDestination: BookmarkDestination =
          new BookmarkDestination();
  ...
  private exportHyperlinks(
    ...
    for (let i: number = 0; i < page.annotations.count; i++) {
      if (page.annotations.at(i) instanceof PdfUriAnnotation) {
        const pdfLoadedUriAnnotation: PdfUriAnnotation = page.annotations.at(
          i
        ) as PdfUriAnnotation;
        const rectangle: AnnotBounds = this.getHyperlinkBounds(
          pdfLoadedUriAnnotation.bounds as Rect,
          pageSize,
          page.rotation
        );
        if (isNullOrUndefined(this.renderer.hyperlinks)) {
        ...
        const pdfLoadedTextWebLinkAnnotation: PdfTextWebLinkAnnotation =
          page.annotations.at(i) as PdfTextWebLinkAnnotation;
        const rectangle: AnnotBounds = this.getHyperlinkBounds(
          pdfLoadedTextWebLinkAnnotation.bounds as Rect,
          pageSize,
          page.rotation
        );
        if (isNullOrUndefined(this.renderer.hyperlinks)) {
        ...
        const pdfLoadedDocumentLinkAnnotation: PdfDocumentLinkAnnotation =
          page.annotations.at(i) as PdfDocumentLinkAnnotation;
        const rectangle: AnnotBounds = this.getHyperlinkBounds(
          pdfLoadedDocumentLinkAnnotation.bounds as Rect,
          pageSize,
          page.rotation
        );
        ...
        if (!isNullOrUndefined(pdfLoadedDocumentLinkAnnotation.destination)) {
          const linkPageIndex: number =
            pdfLoadedDocumentLinkAnnotation.destination.pageIndex;
          ...
          if (page.rotation === PdfRotationAngle.angle180) {
            this.renderer.annotationYPosition.push(
              this.convertPointToPixel(
                Math.abs(
                  pdfLoadedDocumentLinkAnnotation.destination.location[1]
                )
              )
            );
          } else if (
  ...
  private getHyperlinkBounds(
    ...
    if (pageRotation === PdfRotationAngle.angle0) {
      ...
      bound = new AnnotBounds(
        pageSize.width - this.convertPointToPixel(bounds.y - bounds.height),
        this.convertPointToPixel(bounds.x),
        this.convertPointToPixel(bounds.height),
        this.convertPointToPixel(bounds.width)
      );
      ...
      bound = new AnnotBounds(
        pageSize.width - this.convertPointToPixel(bounds.x - bounds.width),
      ...
      bound = new AnnotBounds(
        this.convertPointToPixel(bounds.y),
        pageSize.height - this.convertPointToPixel(bounds.x - bounds.width),
        this.convertPointToPixel(bounds.height),
        this.convertPointToPixel(bounds.width)
      );
    }
  ...
  public exportFormFields(jsonObject: any, isObjects: boolean): Uint8Array {
    ...
    if (
      Object.prototype.hasOwnProperty.call(jsonObject, 'formFieldDataFormat')
    ) {
  ...
  public importFormFields(jsonObject: any): any {
    try {
      let formFields: FormFieldsBase = new FormFieldsBase(
        this.pdfViewer,
        this.pdfViewerBase
      );
      formFields.saveFormFieldsDesignerData(jsonObject);
      const dataFormat: DataFormat = this.exportDataFormat(
        jsonObject['formFieldDataFormat']
      );
      if (
        !this.isBase64(jsonObject['data']) &&
        !this.isUint8Array(jsonObject['data'])
      ) {
  ...
  private pdfiumExportAsImage(pageIndex: number, size?: Size): Promise<string> {
    ...
    return new Promise((resolve: Function, reject: Function) => {
      if (
        !isNullOrUndefined(this.pdfViewerBase.pdfViewerRunner) &&
        !isNullOrUndefined(this.loadedDocument)
      ) {
        size = !isNullOrUndefined(size) ? size : null;
        this.pdfViewerBase.pdfViewerRunner.postMessage({
          pageIndex: pageIndex,
          message: 'extractImage',
          zoomFactor: this.pdfViewer.magnificationModule.zoomFactor,
          isTextNeed: false,
          size: size,
        });
        this.pdfViewerBase.pdfViewerRunner.onmessage = function (
          event: any
        ): void {
          if (event.data.message === 'imageExtracted') {
            ...
            const imageData: ImageData = canvasContext.createImageData(
              width,
              height
            );
  ...
  public extractText(
    pageIndex: number,
    isLayout?: boolean
  ): Promise<{ textDataCollection: TextData[]; extractedText: string }> {
    return new Promise((resolve: Function, reject: Function) => {
      resolve(
        this.textExtraction(
          pageIndex,
          !isNullOrUndefined(isLayout) ? isLayout : true
        )
      );
    });
  }
  ...
  public textExtractionOnmessage(event: any): any {
    ...
    if (event.data.message === 'textExtracted') {
      ...
      for (let i: number = 0; i < characterDetails.length; i++) {
        if (
          !event.data.isLayout &&
          (characterDetails[parseInt(i.toString(), 10)].Text as string).indexOf(
            '\r'
          ) !== -1
        ) {
          extractedText += '';
        ...
        const cropBox: number[] = this.loadedDocument.getPage(
          event.data.pageIndex
        ).cropBox;
        const bound: AnnotBounds = new AnnotBounds(
          this.convertPixelToPoint(
            characterDetails[parseInt(i.toString(), 10)].X
          ),
          this.convertPixelToPoint(
            characterDetails[parseInt(i.toString(), 10)].Y + cropBox[1]
          ),
          this.convertPixelToPoint(
            characterDetails[parseInt(i.toString(), 10)].Width
          ),
          this.convertPixelToPoint(
            characterDetails[parseInt(i.toString(), 10)].Height
          )
        );
        textDataCollection.push(
          new TextData(characterDetails[parseInt(i.toString(), 10)].Text, bound)
        );
      }
      let result: any = {};
      if (event.data.isRenderText) {
        ...
        result = {
          textDataCollection: textDataCollection,
          extractedText: extractedText,
          isRenderText: event.data.isRenderText,
          pageIndex: event.data.pageIndex,
          jsonObject: event.data.jsonObject,
          requestType: event.data.requestType,
          annotationObject: event.data.annotationObject,
        };
      }
      const pageTextDataCollection: { [key: number]: PageTextData } =
        this.getPageTextDataCollection(result);
      const documentTextCollection: any = this.getDocumentTextCollection(
        pageTextDataCollection
      );
      if (!isNullOrUndefined(documentTextCollection)) {
        if (documentTextCollection.requestType === 'pageTextRequest') {
          this.pdfViewerBase.pageTextRequestSuccess(
            documentTextCollection,
            documentTextCollection.pageIndex
          );
        } else if (documentTextCollection.requestType === 'textRequest') {
          this.pdfViewerBase.textRequestSuccess(
            documentTextCollection,
            documentTextCollection.pageIndex,
            documentTextCollection.annotationObject
          );
        } else if (
          documentTextCollection.requestType === 'pdfTextSearchRequest'
        ) {
          this.pdfViewer.textSearchModule.pdfTextSearchRequestSuccess(
            documentTextCollection,
  ...
  public getPageTextDataCollection(textData: any): any {
    ...
    if (!isNullOrUndefined(textData)) {
      ...
      pageTextDataCollection[textData.pageIndex] = new PageTextData(
        new SizeBase(
          this.getPageSize(textData.pageIndex).width,
          this.getPageSize(textData.pageIndex).height
        ),
        textDetails.textDataCollection,
      ...
      if (textData.isRenderText) {
        return {
          ...
          jsonObject: textData.jsonObject,
          requestType: textData.requestType,
  ...
  public getDocumentTextCollection(value: any): any {
    const pageStartIndex: number = !isNullOrUndefined(
      value.jsonObject.pageStartIndex
    )
      ? value.jsonObject.pageStartIndex
      : value.jsonObject.pageIndex;
    const pageEndIndex: number = !isNullOrUndefined(
      value.jsonObject.pageEndIndex
    )
      ? value.jsonObject.pageEndIndex
    ...
    if (!isNullOrUndefined(value)) {
      ...
      if (
        this.documentTextCollection.length ===
        pageEndIndex - pageStartIndex
      ) {
        return {
          uniqueId: value.jsonObject.uniqueId,
   ...
   */
  public extractTextWithPageSize(
    pageIndex: number
  ): Promise<{ [key: number]: PageTextData }> {
    return new Promise((resolve: Function, reject: Function) => {
  ...
  public getDocumentText(
    ...
    const pageStartIndex: number = !isNullOrUndefined(jsonObject.pageStartIndex)
      ? jsonObject.pageStartIndex
      : jsonObject.pageIndex;
    const pageEndIndex: number = !isNullOrUndefined(jsonObject.pageEndIndex)
      ? jsonObject.pageEndIndex
      : jsonObject.pageIndex + 1;
    const pageCount: number = this.loadedDocument.pageCount;
    for (
      let pageIndex: number = pageStartIndex;
      pageIndex < pageEndIndex;
      pageIndex++
    ) {
      this.extractTextDetailsWithPageSize(
        pageIndex,
        true,
        jsonObject,
        requestType,
