...
import { Size } from '@hoyui/drawings';
...
export class Print {
  ...
  public print(): void {
    ...
    if (this.pdfViewerBase.pageCount > 0) {
      ...
      if (this.pdfViewer.printMode === 'Default') {
        ...
        this.frameDoc = this.iframe.contentWindow
      ...
      setTimeout(() => {
        for (
          pageIndex = 0;
          pageIndex < this.pdfViewerBase.pageCount;
          pageIndex++
        ) {
          ...
          if (
            !(
              pageWidth >= a4StdWidth + bufferWidth ||
              pageWidth <= a4StdWidth - bufferWidth
            ) &&
            !(
              pageHeight >= a4StdHeight + bufferHeight ||
              pageHeight <= a4StdHeight - bufferHeight
            )
          ) {
          ...
          this.pdfViewer.printModule.createRequestForPrint(
            pageIndex,
            pageWidth,
            pageHeight,
            this.pdfViewerBase.pageCount,
            this.pdfViewer.printScaleFactor
          );
        }
  ...
  private createRequestForPrint(
    ...
    const jsonObject: object = {
      ...
      digitalSignaturePresent:
        this.pdfViewerBase.digitalSignaturePresent(pageIndex),
      printScaleFactor:
        printScaleFactor >= 0.5 && printScaleFactor <= 5 ? printScaleFactor : 1,
    };
    ...
    proxy.printRequestHandler.url =
      proxy.pdfViewer.serviceUrl +
      '/' +
      proxy.pdfViewer.serverActionSettings.print;
    proxy.printRequestHandler.responseType = null;
    proxy.printRequestHandler.mode = false;
    if (
      this.pdfViewerBase.validateForm &&
      this.pdfViewer.enableFormFieldsValidation
    ) {
      this.pdfViewer.fireValidatedFailed(
        proxy.pdfViewer.serverActionSettings.download
      );
      ...
      if (proxy.pdfViewerBase.clientSideRendering) {
        this.pdfViewerBase.pdfViewerRunner.postMessage({
          ...
          message: 'printImage',
          printScaleFactor:
            printScaleFactor >= 0.5 && printScaleFactor <= 5
              ? printScaleFactor
              : 1,
        });
      } else {
  ...
  public printOnMessage(event: any): void {
    ...
    const { value, width, height, pageIndex, pageWidth, pageHeight } =
      event.data;
    canvas.width = width;
  ...
  private printSuccess(
    ...
    let printImage: any = this.pdfViewerBase.clientSideRendering
      ? result
      : result.data;
    const redirect: boolean = (this as any).pdfViewerBase.checkRedirection(
      printImage
    );
    if (redirect) {
      this.pdfViewerBase.showPrintLoadingIndicator(false);
          ...
          try {
            ...
            if (typeof printImage !== 'object') {
              this.pdfViewerBase.onControlError(
                500,
                printImage,
                this.pdfViewer.serverActionSettings.print
              );
              printImage = null;
            }
          } catch (error) {
      ...
      if (printImage && printImage.uniqueId === this.pdfViewerBase.documentId) {
        this.pdfViewer.fireAjaxRequestSuccess(
          this.pdfViewer.serverActionSettings.print,
          printImage
        );
        let annotationSource: string = '';
        if (!this.pdfViewer.annotationSettings.skipPrint) {
          const annotationCollections: any =
            this.pdfViewerBase.documentAnnotationCollections;
          if (
            annotationCollections &&
            annotationCollections[printImage.pageNumber] &&
            this.pdfViewerBase.isTextMarkupAnnotationModule()
          ) {
            const printCollection: any =
              annotationCollections[printImage.pageNumber];
            if (this.pdfViewerBase.isImportAction) {
              const textMarkupAnnotation: number[] =
                printCollection.textMarkupAnnotation;
              const shapeAnnotation: number[] = printCollection.shapeAnnotation;
              const measureShapeAnnotation: number[] =
                printCollection.measureShapeAnnotation;
              const stampAnnotation: number[] =
                printCollection.stampAnnotations;
              const freeTextAnnotation: number[] =
                printCollection.freeTextAnnotation;
              const stickyNoteAnnotation: any =
                printCollection.stickyNotesAnnotation;
              annotationSource =
                this.pdfViewer.annotationModule.textMarkupAnnotationModule.printTextMarkupAnnotations(
                  textMarkupAnnotation,
                  printImage.pageNumber,
                  stampAnnotation,
                  shapeAnnotation,
              ...
              annotationSource =
                this.pdfViewer.annotationModule.textMarkupAnnotationModule.printTextMarkupAnnotations(
                  ...
                  printImage.pageNumber,
                  printCollection.stampAnnotations,
                  printCollection.shapeAnnotation,
                  printCollection.measureShapeAnnotation,
                  printCollection.stickyNoteAnnotation,
                  printCollection.freeTextAnnotation
                );
            }
          }
        ...
        if (this.pdfViewerBase.clientSideRendering) {
          ...
          const whichIsBigger: 'Width' | 'Height' =
          ...
          if (pageHeight < pageWidth && this.pdfViewer.enablePrintRotation) {
            this.printCanvas.height =
              pageWidth * printScaleValue * window.devicePixelRatio;
            this.printCanvas.width =
              pageHeight * printScaleValue * window.devicePixelRatio;
          } else {
            this.printCanvas.height =
              maxHeight * printScaleValue * window.devicePixelRatio;
            this.printCanvas.width =
              maxWidth * printScaleValue * window.devicePixelRatio;
          }
        } else {
          this.printCanvas.height =
            this.printHeight * printScaleValue * window.devicePixelRatio;
          this.printCanvas.width =
            this.printWidth * printScaleValue * window.devicePixelRatio;
        }
        ...
        const context: CanvasRenderingContext2D =
          this.printCanvas.getContext('2d');
        ...
        pageImage.onload = (): void => {
          if (pageHeight > pageWidth || !this.pdfViewer.enablePrintRotation) {
            ...
            context.translate(
              this.printCanvas.width * 0.5,
              this.printCanvas.height * 0.5
            );
  ...
  private renderFieldsForPrint(
    ...
    let targetField: any;
    if (this.pdfViewer.printMode === 'Default') {
      targetField = this.frameDoc.document.getElementById(
        'fields_' + pageIndex
      );
    } else {
      targetField = this.printWindow.document.getElementById(
        'fields_' + pageIndex
      );
    }
      ...
      if (data) {
        ...
        for (let i: number = 0; i < formFieldsData.length; i++) {
          ...
          if (parseFloat(currentData['PageIndex']) === pageIndex) {
            const field: any = this.pdfViewer.formFieldsModule.createFormFields(
              currentData,
              pageIndex,
              i,
              targetField
            ...
            if (inputField) {
              ...
              if (inputField.InsertSpaces) {
                const font: number =
                  parseInt(inputField.style.width, 10) / inputField.maxLength -
              ...
              const pageDetails: any =
                this.pdfViewerBase.pageSize[parseInt(pageIndex.toString(), 10)];
              if (
                pageDetails.width > pageDetails.height &&
                this.pdfViewer.enablePrintRotation
              ) {
                /*
                                ...
                                */
                const x: number =
                  this.pdfViewer.formFieldsModule.ConvertPointToPixel(bounds.X);
                const y: number =
                  this.pdfViewer.formFieldsModule.ConvertPointToPixel(bounds.Y);
                const width: number =
                  this.pdfViewer.formFieldsModule.ConvertPointToPixel(
                    bounds.Width
                  );
                const height: number =
                  this.pdfViewer.formFieldsModule.ConvertPointToPixel(
      ...
      if (formDesignerData) {
        ...
        for (let i: number = 0; i < formDesignerFieldsData.length; i++) {
          const currentData: any =
            formDesignerFieldsData[parseInt(i.toString(), 10)].FormField;
          if (currentData.pageNumber - 1 === pageIndex && currentData.isPrint) {
            let signatureField: PdfAnnotationBaseModel = (
              this.pdfViewer.nameTable as any
            )[
              formDesignerFieldsData[parseInt(i.toString(), 10)].Key.split(
                '_'
              )[0]
            ];
            const element: DiagramHtmlElement = signatureField.wrapper
            ...
            if (element) {
              if (currentData.formFieldAnnotationType === 'RadioButton') {
                for (
                  let j: number = 0;
                  j < currentData.radiobuttonItem.length;
                  j++
                ) {
                  signatureField = (this.pdfViewer.nameTable as any)[
                    currentData.radiobuttonItem[
                      parseInt(j.toString(), 10)
                    ].id.split('_')[0]
                  ];
                  htmlElement = this.createFormDesignerFields(
                ...
                if (htmlElement) {
                  ...
                  this.applyPosition(
                    ...
                    widthRatio,
                    true,
                    currentData.zoomValue,
                    currentData.pageNumber - 1
                  );
                  if (
                    !isNullOrUndefined(bounds) &&
                    bounds.Width > 0 &&
                    bounds.Height > 0
                  ) {
                    targetField.appendChild(htmlElement);
  ...
  private createFormDesignerFields(
    ...
    const parentHtmlElement: HTMLElement =
      this.pdfViewer.formDesignerModule.createHtmlElement(
        'div',
        parentHtmlElementAttribute
      );
    ...
    if (
      currentData.formFieldAnnotationType === 'SignatureField' ||
      currentData.formFieldAnnotationType === 'InitialField'
    ) {
      this.pdfViewer.formDesignerModule.disableSignatureClickEvent = true;
      element.template = htmlElement.appendChild(
        this.pdfViewer.formDesignerModule.createSignatureDialog(
          this.pdfViewer,
          signatureField,
          null,
          true
      ...
      element.template = htmlElement.appendChild(
        this.pdfViewer.formDesignerModule.createDropDownList(
          element,
          signatureField,
          true
        )
      ...
      element.template = htmlElement.appendChild(
        this.pdfViewer.formDesignerModule.createListBox(
          element,
          signatureField,
          true
        )
      ...
      element.template = htmlElement.appendChild(
        this.pdfViewer.formDesignerModule.createInputElement(
          currentData.formFieldAnnotationType,
          signatureField,
          null,
          true
        )
  ...
  public applyPosition(
    ...
    if (bounds) {
      ...
      if (
        ...
        left = isFormDesignerField
          ? bounds.X / zoomValue / widthRatio
          : this.pdfViewer.formFieldsModule.ConvertPointToPixel(bounds.X) /
            widthRatio;
        top = isFormDesignerField
          ? bounds.Y / zoomValue / heightRatio
          : this.pdfViewer.formFieldsModule.ConvertPointToPixel(bounds.Y) /
            heightRatio;
        width = isFormDesignerField
          ? bounds.Width / zoomValue / widthRatio
          : this.pdfViewer.formFieldsModule.ConvertPointToPixel(bounds.Width) /
            widthRatio;
        height = isFormDesignerField
          ? bounds.Height / zoomValue / heightRatio
          : this.pdfViewer.formFieldsModule.ConvertPointToPixel(bounds.Height) /
            heightRatio;
      }
  ...
  private printWindowOpen(): void {
    ...
    for (
      let i: number = 0;
      i < this.printViewerContainer.children.length;
      i++
    ) {
      /*
      ...
      const canvasUrl: string = (
        this.printViewerContainer.children[
          parseInt(i.toString(), 10)
        ] as HTMLCanvasElement
      ).toDataURL('image/jpeg');
      printDocument.write(
        '<div style="margin:0mm;width:' +
          this.printWidth.toString() +
          'px;height:' +
          this.printHeight.toString() +
          'px;position:relative"><img src="' +
          canvasUrl +
          '" id="' +
          'image_' +
          i +
          ...
          'fields_' +
          i +
          '" style="margin:0px;top:0px;left:0px;position:absolute;width:' +
          this.printWidth.toString() +
          'px;height:' +
          this.printHeight.toString() +
          'px;z-index:2"></div></div>'
      );
      if (
      ...
      if (i === 0) {
        if (
          ...
          printDocument.write(
            '<html moznomarginboxes mozdisallowselectionprint><head><style>html, body { height: 100%; width:100% }' +
              ...
              ' div{ page-break-inside: avoid; }} @page{margin:0mm;  size:' +
              this.printWidth.toString() +
              'px ' +
              this.printHeight.toString() +
              'px; }</style></head><body>'
