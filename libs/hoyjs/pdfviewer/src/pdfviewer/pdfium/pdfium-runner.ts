...
export function PdfiumRunner(): void {
  ...
  const PDFiumModule: any =
    typeof (pdfiumWindow as any)[`${moduleString}`] !== 'undefined'
      ? (pdfiumWindow as any)[`${moduleString}`]
      : {};
  const F64: Float64ArrayConstructor = Float64Array;
  const H: any =
    (t: Float64ArrayConstructor, s: number, d: number[]) => (f: any) => {
  ...
  function initializeFPDF(): void {
    ...
    (FPDF as any).RenderPageBitmap = PDFiumModule.cwrap(
    ...
    // eslint-disable-next-line
    (FPDF as any).Bitmap_FillRect = PDFiumModule.cwrap(
      'FPDFBitmap_FillRect',
      '',
      ['number', 'number', 'number', 'number', 'number', 'number']
    );
    // eslint-disable-next-line
    (FPDF as any).Bitmap_CreateEx = PDFiumModule.cwrap(
      'FPDFBitmap_CreateEx',
      'number',
      ['number', 'number', 'number', 'number', 'number']
    );
    // eslint-disable-next-line
    (FPDF as any).Bitmap_Destroy = PDFiumModule.cwrap(
      'FPDFBitmap_Destroy',
      '',
      ['number']
    );
    (FPDF as any).LoadPage = PDFiumModule.cwrap('FPDF_LoadPage', 'number', [
      'number',
      'number',
    ]);
    (FPDF as any).ClosePage = PDFiumModule.cwrap('FPDF_ClosePage', '', [
      'number',
    ]);
    (FPDF as any).LoadMemDocument = PDFiumModule.cwrap(
      'FPDF_LoadMemDocument',
      'number',
      ['number', 'number', 'string']
    );
    (FPDF as any).GetPageSizeByIndex = PDFiumModule.cwrap(
      'FPDF_GetPageSizeByIndex',
      'number',
      ['number', 'number', 'number', 'number']
    );
    (FPDF as any).GetLastError = PDFiumModule.cwrap(
      'FPDF_GetLastError',
      'number'
    );
    (FPDF as any).GetPageCount = PDFiumModule.cwrap(
      'FPDF_GetPageCount',
      'number',
      ['number']
    );
    (FPDF as any).CloseDocument = PDFiumModule.cwrap('FPDF_CloseDocument', '', [
      'number',
    ]);
    (FPDF as any).DestroyLibrary = PDFiumModule.cwrap('FPDF_DestroyLibrary');
    (FPDF as any).LoadTextPage = PDFiumModule.cwrap(
      'FPDFText_LoadPage',
      'number',
      ['number']
    );
    (FPDF as any).CloseTextPage = PDFiumModule.cwrap('FPDFText_ClosePage', '', [
      'number',
    ]);
    (FPDF as any).TextCountChars = PDFiumModule.cwrap(
      'FPDFText_CountChars',
      'number',
      ['number']
    );
    (FPDF as any).GetUnicodeChar = PDFiumModule.cwrap(
      'FPDFText_GetUnicode',
      'number',
      ['number']
    );
    (FPDF as any).GetCharBox = PDFiumModule.cwrap(
      'FPDFText_GetCharBox',
      'number',
      ['number', 'number', 'number', 'number', 'number']
    );
    (FPDF as any).GetPageRotation = PDFiumModule.cwrap(
      'FPDFPage_GetRotation',
      'number',
      ['number']
    );
    (FPDF as any).GetCharAngle = PDFiumModule.cwrap(
    ...
    fetch(fontFile)
      ...
      .then((value: ArrayBuffer) => {
        if (value) {
          ...
          PDFiumModule.FS.createDataFile(
            name,
            null,
            new Int8Array(value),
            true,
  ...
  ctx.onmessage = function (event: any): void {
    if (event.data.message === 'initialLoading') {
      ...
      PDFiumModule.onRuntimeInitialized = function (): void {
        ...
        if (event.data.fonts && event.data.fonts.length > 0) {
          ...
          for (let i: number = 0; i < event.data.fonts.length; i++) {
            let fontpath: string;
            if (
              event.data.fonts[parseInt(i.toString(), 10)].startsWith(
                'http://'
              ) ||
              event.data.fonts[parseInt(i.toString(), 10)].startsWith(
                'https://'
              ...
              fontpath =
                PDFiumModule.url +
                '/' +
                event.data.fonts[parseInt(i.toString(), 10)];
            }
      ...
      const pages: number = FPDF.GetPageCount(
        documentDetails.processor.wasmData.wasm
      );
      ...
      const pages: number = FPDF.GetPageCount(
        documentDetailsNew.processor.wasmData.wasm
      );
      ...
      const data: object = firstPage.render(
        null,
        ImageData.zoomFactor,
        false,
        null,
        null,
        null,
      ...
      (data as any).rubberStampAnnotationPageNumber =
        event.data.rubberStampAnnotationPageNumber;
    ...
    if (documentDetails) {
      if (event.data.message === 'renderPage') {
        ...
        const data: object = firstPage.render(
          null,
          ImageData.zoomFactor,
          ImageData.isTextNeed,
          null,
          null,
          ImageData.textDetailsId,
          null,
          event.data.cropBoxRect,
          event.data.mediaBoxRect
        );
        ctx.postMessage(data);
        ...
        const thumbnail: Promise<any> = new Promise((resolve, reject) => {
          ...
          setTimeout(() => {
            try {
              const firstPage: Page = documentDetails.getPage(
                event.data.pageIndex
              );
              if (
                firstPage.processor !== null &&
                firstPage.processor !== undefined
              ) {
                const data: object = firstPage.render(
                  'thumbnail',
                  null,
        ...
        const data: object = firstPage.render(
          ...
          null,
          false,
          event.data.printScaleFactor,
          event.data.printDevicePixelRatio
        );
        ctx.postMessage(data);
        ...
        const firstPage: Page = documentDetails.getPage(event.data.pageIndex);
        const data: object = firstPage.renderTileImage(
          values.tileX,
          values.tileY,
          values.tileXCount,
          values.tileYCount,
          values.zoomFactor,
          event.data.isTextNeed,
  ...
  class Page {
    ...
    public renderTileImage(
      ...
      return this.processor.renderTileImage(
        this.index,
        x,
        y,
        tileX,
  ...
  class Processor {
    ...
    public getPageSize(i: number = 0): number[] {
      return H(
        ...
        (FPDF as any).GetPageSizeByIndex(this.wasmData.wasm, i, w, h)
    ...
    public getCharBounds(pagePointer: any, i: number = 0): number[] {
      return H(
        ...
        4,
        [-1, -1, -1, -1]
      )((left: any, right: any, bottom: any, top: any) =>
        (FPDF as any).GetCharBox(pagePointer, i, left, right, bottom, top)
      );
    }
    ...
    public getRender(
      ...
      const bmap: any = (FPDF as any).Bitmap_CreateEx(
        w,
        h,
        (FPDF as any).Bitmap_BGRA,
        heap,
        w * 4
      );
      const page: any = (FPDF as any).LoadPage(this.wasmData.wasm, i);
      (FPDF as any).Bitmap_FillRect(
        bmap,
        0,
        0,
        w,
        h,
        isTransparent ? 0x00ffffff : 0xffffffff
      );
    ...
    public textExtraction(
      ...
      if (isTextNeed) {
        ...
        pageHeight =
          pageHeight +
          this.pointerToPixelConverter(
            mediaBoxRect && mediaBoxRect.y ? mediaBoxRect.y : 0
          );
        const textPage: any = (FPDF as any).LoadTextPage(
          pagePointer,
          pageIndex
        );
        ...
        for (
          ...
          let rotationRadian: any = (FPDF as any).GetCharAngle(
            textPage,
            charCount
          );
          const character: string = String.fromCharCode(result);
          let [charLeft, charRight, charBottom, charTop] = this.getCharBounds(
            textPage,
            charCount
          );
          let X: number =
            ...
            this.pointerToPixelConverter(
              cropBoxRect && cropBoxRect.x ? cropBoxRect.x : 0
            );
          let Y: number =
            pageHeight +
            this.pointerToPixelConverter(
              cropBoxRect && cropBoxRect.y ? cropBoxRect.y : 0
            ) -
            this.pointerToPixelConverter(charTop);
          let Width: number = this.pointerToPixelConverter(
            charRight - charLeft
          );
          let Height: number = this.pointerToPixelConverter(
            charTop - charBottom
          );
          let rotationAngle: number = parseInt(
            ((rotationRadian * 180) / Math.PI).toString(),
            10
          );
          if (charCount < totalCharacterCount) {
            pageText += character;
            const currentCharacterBounds: RectAngle = new RectAngle(
              X,
              Y,
              Width,
              Height,
          ...
          switch (character) {
            case '\0': {
              ...
              const newWordBounds: RectAngle = new RectAngle(
                wordMinLeft,
                wordMinTop,
                wordMaxRight - wordMinLeft,
                wordMaxBottom - wordMinTop,
                word,
                wordRotation
              );
              wordBounds.push(newWordBounds);
              this.textBoundsCalculation(
                wordBounds,
                minTop,
                maxBottom,
                maxRight,
            ...
            case '\r':
              if (charCount < totalCharacterCount) {
                const characterBounds: RectAngle = new RectAngle(
                  X,
                  Y,
                  Width,
                  Height,
                ...
                maxRight = 0;
                pageText += '\n';
                rotationRadian = (FPDF as any).GetCharAngle(
                  textPage,
                  charCount
                );
                [charLeft, charRight, charBottom, charTop] = this.getCharBounds(
                  textPage,
                  charCount
                );
            ...
            case '\ufffe':
              {
                const characterBounds: any = new RectAngle(
                  X,
                  Y,
                  Width,
                  Height,
            ...
            default:
              if (Width === 0 || Height === 0) {
                ...
                // eslint-disable-next-line
                maxRight = Math.max.apply(Math, right);
                let newWordBounds: any = new RectAngle(
                  wordMinLeft,
                  wordMinTop,
                  wordMaxRight - wordMinLeft,
                  wordMaxBottom - wordMinTop,
                ...
                if (character !== ' ') {
                  if (
                    isPreviousSpace &&
                    wordBounds.length > 0 &&
                    rotationAngle === wordBounds[0].Rotation
                  ) {
                    if (
                      (rotationAngle === 180 || rotationAngle === 0) &&
                      Math.abs(characterBounds.Y - wordBounds[0].Y) >
                        maximumSpaceForNewLine
                    ) {
                      startNewLine = true;
                    }
                    if (
                      (rotationAngle === 270 || rotationAngle === 90) &&
                      Math.abs(characterBounds.X - wordBounds[0].X) >
                        maximumSpaceForNewLine
                    ) {
                      startNewLine = true;
                    }
                  ...
                  if (
                    (isZeroWidthSpace &&
                      wordBounds.length >= 1 &&
                      wordBounds[wordBounds.length - 1].Rotation !==
                        characterBounds.Rotation) ||
                    startNewLine
                  ) {
                  ...
                  let newWordBounds: RectAngle = new RectAngle(
                    wordMinLeft,
                    wordMinTop,
                    wordMaxRight - wordMinLeft,
                    wordMaxBottom - wordMinTop,
    ...
    public textBoundsCalculation(
      ...
      for (let count: number = 0; count < wordBounds.length; count++) {
        ...
        if (textRotation === 0 || textRotation === 180) {
          ...
          if (pageRotation === 0) {
            newWordBounds = new RectAngle(
              wordBounds[parseInt(count.toString(), 10)].Left,
              minTop,
            ...
            newWordBounds = new RectAngle(
              pageWidth - wordBounds[parseInt(count.toString(), 10)].Left,
              pageHeight - minTop,
              wordBounds[parseInt(count.toString(), 10)].Width,
              maxBottom - minTop,
              wordBounds[parseInt(count.toString(), 10)].Text,
            ...
            newWordBounds = new RectAngle(
              minTop,
              pageHeight - wordBounds[parseInt(count.toString(), 10)].Left,
              wordBounds[parseInt(count.toString(), 10)].Width,
              maxBottom - minTop,
              wordBounds[parseInt(count.toString(), 10)].Text,
              textRotation
          ...
          if (pageRotation === 0) {
            newWordBounds = new RectAngle(
              minLeft,
              wordBounds[parseInt(count.toString(), 10)].Top,
              maxRight - minLeft,
            ...
            newWordBounds = new RectAngle(
              pageWidth - wordBounds[parseInt(count.toString(), 10)].Top,
              minLeft,
              maxRight - minLeft,
              wordBounds[parseInt(count.toString(), 10)].Height,
              wordBounds[parseInt(count.toString(), 10)].Text,
            ...
            newWordBounds = new RectAngle(
              pageWidth - minLeft,
              pageHeight - wordBounds[parseInt(count.toString(), 10)].Top,
              maxRight - minLeft,
              wordBounds[parseInt(count.toString(), 10)].Height,
              wordBounds[parseInt(count.toString(), 10)].Text,
              textRotation
        ...
        if (
          !hasInBetweenRotation &&
          wordBounds[parseInt(count.toString(), 10)].Text === ' ' &&
          count !== 0 &&
          count + 1 <= wordBounds.length
        ) {
          if (!isRTLText) {
            if (count + 1 !== wordBounds.length) {
              ...
              switch (textRotation) {
                case 0:
                  spaceWidth =
                    wordBounds[count + 1].Left -
                    (wordBounds[count - 1].Left + wordBounds[count - 1].Width);
    ...
    public checkIsRtlText(text: string): boolean {
      const ltrChars: string =
        'A-Za-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02B8\\u0300-\\u0590\\u0800-\\u1FFF' +
        '\\u2C00-\\uFB1C\\uFDFE-\\uFE6F\\uFEFD-\\uFFFF';
      const rtlChars: string = '\\u0591-\\u07FF\\uFB1D-\\uFDFD\\uFE70-\\uFEFC';
      // eslint-disable-next-line
      const rtlDirCheck: any = new RegExp(
        '^[^' + ltrChars + ']*[' + rtlChars + ']'
      );
      return rtlDirCheck.test(text);
    }

    public getPageRender(
      n: number = 0,
      w: any,
      h: any,
      isTextNeed: boolean,
      ...
      const pageRenderPtr: any = this.getRender(
        n,
        w,
        h,
        isTextNeed,
      ...
      pageRenderData = PDFiumModule.HEAPU8.slice(
        pageRenderPtr,
        pageRenderPtr + w * h * 4
      );
    ...
    public render(
      ...
      if (message === 'thumbnail') {
        ...
        const newWidth: number = Math.round(
          maxWidth * printScaleFactor * scaleFactor
        );
        const newHeight: number = Math.round(
          maxHeight * printScaleFactor * scaleFactor
        );
        const data: any = this.getPageRender(n, newWidth, newHeight, false);
        return {
          value: data,
        ...
        let newWidth: number = Math.round(
          (size && size !== null ? size.width : w) * scaleFactor * zoomFactor
        );
        let newHeight: number = Math.round(
          (size && size !== null ? size.height : h) * scaleFactor * zoomFactor
        );
        ...
        const data: any = this.getPageRender(
          n,
          newWidth,
          newHeight,
          isTextNeed,
    ...
    public renderTileImage(
      ...
      let data: any[] = [];
      data = PDFiumModule.HEAPU8.slice(
        pageRenderPtr,
        pageRenderPtr + w1 * h1 * 4
      );
      PDFiumModule.asm.free(pageRenderPtr);
      if (tileX === 0 && tileY === 0) {
        return {
  ...
  class DocumentInfo {
    ...
    public createAllPages(): void {
      for (let i: number = 0; i < this.pages.length; i++) {
        this.pages[parseInt(i.toString(), 10)] = new Page(
