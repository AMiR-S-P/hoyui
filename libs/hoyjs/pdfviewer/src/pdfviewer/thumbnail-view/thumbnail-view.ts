...
import { createElement, isNullOrUndefined, Browser } from '@hoyui/base';
...
export class ThumbnailView {
  ...
  public createThumbnailContainer(): void {
    ...
    this.pdfViewerBase.navigationPane.sideBarContent.appendChild(
      this.thumbnailView
    );
    this.pdfViewerBase.navigationPane.sideBarContent.addEventListener(
      'scrollend',
      this.thumbnailOnScroll
    );
  }
  ...
  private thumbnailOnScroll = (event: any): void => {
    for (let i: number = 0; i < this.thumbnailPageSize.length; i++) {
      const scrollPosition: number =
        this.pdfViewerBase.navigationPane.sideBarContent.scrollTop;
      const index: number = this.thumbnailPageSize.findIndex(
        (page: any) => page.top >= scrollPosition
      );
      if (index !== -1) {
        const number: number =
          Math.floor(index / this.thumbnailRequestsBatch) *
          this.thumbnailRequestsBatch;
  ...
  public createRequestForThumbnails(): void {
    ...
    if (proxy.pdfViewer.isThumbnailViewOpen) {
      ...
      proxy.pdfViewerBase.navigationPane.sideBarTitle.textContent =
        proxy.pdfViewer.localeObj.getConstant('Page Thumbnails');
      document.getElementById(
        proxy.pdfViewer.element.id + '_thumbnail_view'
      ).style.display = 'flex';
      const bookmarkContent: any = proxy.pdfViewer.element.querySelector(
        '.e-pv-bookmark-view'
      );
      if (bookmarkContent) {
  ...
  private isThumbnailViewOpen(): void {
    ...
    if (proxy.pdfViewer.isThumbnailViewOpen) {
      ...
      this.pdfViewerBase.navigationPane.sideBarContentContainer.style.display =
        'block';
      if (this.pdfViewer.enableRtl) {
        proxy.pdfViewerBase.viewerContainer.style.right =
          this.pdfViewerBase.navigationPane.getViewerContainerLeft() + 'px';
      } else {
        proxy.pdfViewerBase.viewerContainer.style.left =
          this.pdfViewerBase.navigationPane.getViewerContainerLeft() + 'px';
      }
      proxy.pdfViewerBase.viewerContainer.style.width =
        proxy.pdfViewer.element.clientWidth -
        this.pdfViewerBase.navigationPane.getViewerContainerLeft() -
        this.pdfViewerBase.navigationPane.getViewerContainerRight() +
        'px';
      proxy.pdfViewerBase.pageContainer.style.width =
        proxy.pdfViewerBase.viewerContainer.clientWidth + 'px';
  ...
  private renderThumbnailEmptyPage(pageIndex: number): void {
    if (this.thumbnailView) {
      const pageLink: HTMLAnchorElement = createElement('a', {
        id: 'page_' + pageIndex,
        attrs: {
      ...
      const thumbnail: HTMLElement = createElement('div', {
      ...
      this.thumbnailSelectionRing = createElement('div', {
        id:
          this.pdfViewer.element.id + '_thumbnail_Selection_Ring_' + pageIndex,
      ...
      if (pageIndex !== 0) {
        this.thumbnailTop =
          this.thumbnailPageSize[pageIndex - 1].top +
          this.thumbnailPageSize[pageIndex - 1].height;
        const thumbnailSize: any = { height: height, top: this.thumbnailTop };
        this.thumbnailPageSize[parseInt(pageIndex.toString(), 10)] =
          thumbnailSize;
      } else {
        const thumbnailSize: any = { height: height, top: this.thumbnailTop };
        this.thumbnailPageSize[parseInt(pageIndex.toString(), 10)] =
  ...
  public renderViewPortThumbnailImage(prox?: ThumbnailView): void {
    ...
    if (
      this.pdfViewerBase.pageSize.length === this.pdfViewerBase.pageCount &&
      !this.isRendered
    ) {
      this.renderDiv();
    }
    proxy.thumbnailLimit =
      proxy.startIndex + proxy.thumbnailThreshold < proxy.pdfViewer.pageCount
        ? proxy.startIndex + proxy.thumbnailThreshold
        : proxy.pdfViewer.pageCount;
    if (!this.pdfViewerBase.clientSideRendering) {
      ...
      this.thumbnailRequestHandler.url =
      ...
      if (
        proxy.startIndex.toString() !== proxy.thumbnailLimit.toString() &&
        proxy.thumbnailLimit > 0 &&
      ...
      for (
        let count: number = proxy.startIndex;
        count < proxy.thumbnailLimit;
        count++
      ) {
        const currentPageImage: HTMLImageElement =
          this.getThumbnailImageElement(count);
        if (
          (currentPageImage && currentPageImage.src === '') ||
          (isNullOrUndefined(currentPageImage) &&
  ...
  public thumbnailOnMessage(event: any): void {
    if (event.data.message === 'renderThumbnail') {
      ...
      const currentPageImage: HTMLImageElement =
        this.getThumbnailImageElement(pageIndex);
      if (currentPageImage) {
  ...
  private renderDiv(data?: any): void {
    if (
      ...
      for (let i: number = 100; i < this.pdfViewer.pageCount; i++) {
        const thumbnail: HTMLElement = document.getElementById(
          this.pdfViewer.element.id + '_thumbnail_' + i
        );
        const pageLink: HTMLAnchorElement = document.getElementById(
          'page_' + i
        ) as HTMLAnchorElement;
        const thumbnailPageNumber: HTMLElement = document.getElementById(
          this.pdfViewer.element.id + '_thumbnail_pagenumber_' + i
        );
        const currentPageImage: HTMLImageElement =
          this.getThumbnailImageElement(i);
        let height: number = 180;
        if (
          ...
          (data &&
            (data.pageRotation[parseInt(i.toString(), 10)] === 1 ||
        ...
        if (this.thumbnailPageSize.length > 0) {
          this.thumbnailTop =
            this.thumbnailPageSize[i - 1].top +
            this.thumbnailPageSize[i - 1].height;
          const thumbnailSize: any = { height: height, top: this.thumbnailTop };
  ...
  public gotoThumbnailImage(pageNumber: number): void {
    ...
    if (this.thumbnailView) {
      const thumbnailChild: HTMLAnchorElement = this.thumbnailView.children[
        parseInt(pageNumber.toString(), 10)
      ] as HTMLAnchorElement;
      if (thumbnailChild) {
        const thumbnailDiv: HTMLElement = thumbnailChild
          .children[0] as HTMLElement;
        let offsetTop: number;
        if (shouldScroll) {
          if (
            this.pdfViewerBase.pageSize.length ===
          ...
          if (thumbnailDiv.offsetTop <= 0) {
            offsetTop =
              thumbnailDiv.parentElement.offsetTop +
              thumbnailDiv.clientTop -
              this.thumbnailTopMargin;
          } else {
            offsetTop =
              thumbnailDiv.offsetTop +
              thumbnailDiv.clientTop -
              this.thumbnailTopMargin;
          }
          this.pdfViewerBase.navigationPane.sideBarContent.scrollTop =
  ...
  private determineThumbnailsRequest(currentPageNumber: number): number {
    ...
    return currentPageNumber !== undefined && currentPageNumber < pageCount
      ? currentPageNumber
      : pageCount;
  }
  ...
  private checkThumbnailScroll(pageNumber: number): boolean {
    ...
    if (this.thumbnailView) {
      ...
      if (numVisibleThumbs > 0) {
        const visibleFirstPageID: number = this.getPageNumberFromID(
          visibleThumbs.first.id
        );
        // account for only one thumbnail being visible.
        const visibleLastPageID: number =
          numVisibleThumbs > 1
            ? this.getPageNumberFromID(visibleThumbs.last.id)
            : visibleFirstPageID;
        if (
          pageNumber <= visibleFirstPageID ||
          pageNumber >= visibleLastPageID
        ) {
          shouldScroll = true;
        } else {
          visibleThumbs.views.some((view: any) => {
            ...
            shouldScroll =
              view.percent < 100 &&
              view.view.offsetWidth > view.view.offsetHeight &&
              view.percent < 97;
            return true;
          });
        }
  ...
  private setFocusStyle(thumbnail: HTMLElement, pageNumber: number): void {
    if (
      thumbnail.children[0].id ===
      this.pdfViewer.element.id + '_thumbnail_Selection_Ring_' + pageNumber
    ) {
  ...
  private renderThumbnailImage(data: any, pageNumber?: number): void {
    ...
    if (
      ...
      this.pdfViewer.pageOrganizer.getData(
        data,
        this.pdfViewerBase.clientSideRendering
      );
    }
    this.thumbnailLimit = this.determineThumbnailsRequest(
      !isNullOrUndefined(pageNumber) ? pageNumber : this.thumbnailLimit
    );
    if (
      this.thumbnailLimit !== this.pdfViewerBase.pageCount &&
      (this.thumbnailView || !isNullOrUndefined(this.pdfViewer.pageOrganizer))
    ) {
  ...
  private renderServerThumbnailImage(data: any): void {
    const startPage: number = !isNullOrUndefined(data && data.startPage)
      ? data.startPage
      : this.startIndex;
    const endPage: number = !isNullOrUndefined(data && data.endPage)
      ? data.endPage
      : this.thumbnailLimit;
  ...
  private thumbnailImageRender(pageIndex: number, data: any): void {
    if (
      !isNullOrUndefined(data.pageRotation) &&
      Object.keys(data.pageRotation).length > 0 &&
      !this.isRendered
    ) {
      this.renderDiv(data);
    }
    const thumbnail: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_thumbnail_' + pageIndex
    );
    const pageLink: HTMLAnchorElement = document.getElementById(
      'page_' + pageIndex
    ) as HTMLAnchorElement;
    const thumbnailPageNumber: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_thumbnail_pagenumber_' + pageIndex
    );
    const currentPageImage: HTMLImageElement =
      this.getThumbnailImageElement(pageIndex);
    if (!isNullOrUndefined(thumbnail) && !isNullOrUndefined(currentPageImage)) {
      currentPageImage.src =
        ...
        data.thumbnailImage instanceof String
          ? data.thumbnailImage
          : data.thumbnailImage[parseInt(pageIndex.toString(), 10)];
      currentPageImage.alt =
        this.pdfViewer.element.id + '_thumbnail_page_' + pageIndex;
  ...
  private wireUpEvents(): void {
    if (this.thumbnailSelectionRing) {
      this.thumbnailSelectionRing.addEventListener(
        'click',
        this.thumbnailClick
      );
      ...
      this.thumbnailSelectionRing.addEventListener(
        'mouseover',
        this.thumbnailMouseOver
      );
      this.thumbnailSelectionRing.addEventListener(
        'mouseleave',
        this.thumbnailMouseLeave
      );
    }
  }
  ...
  private unwireUpEvents(): void {
    if (this.thumbnailSelectionRing && this.thumbnailImage) {
      this.thumbnailSelectionRing.removeEventListener(
        'click',
        this.thumbnailClick
      );
  ...
  private goToThumbnailPage(pageNumber: number): void {
    if (
      pageNumber > 0 &&
      pageNumber <= this.pdfViewerBase.pageCount &&
      this.pdfViewerBase.currentPageNumber !== pageNumber
    ) {
  ...
  public thumbnailMouseOver = (event: MouseEvent): void => {
    ...
    if (
      ...
      target.id ===
      proxy.pdfViewer.element.id + '_thumbnail_image_' + pageNumber
    ) {
  ...
  public thumbnailMouseLeave = (event: MouseEvent): void => {
    ...
    if (
      ...
      target.parentElement.id ===
      proxy.pdfViewer.element.id + '_thumbnail_' + pageNumber
    ) {
  ...
  private setMouseFocusToFirstPage(): void {
    const thumbnailChild: HTMLAnchorElement = this.thumbnailView
      .children[0] as HTMLAnchorElement;
    if (thumbnailChild) {
      const thumbnailDiv: HTMLElement = thumbnailChild.children[0]
        .children[0] as HTMLElement;
  ...
  private getVisibleThumbs(): IVisibleThumbnail {
    return this.getVisibleElements(
      this.pdfViewerBase.navigationPane.sideBarContent,
      this.thumbnailView.children
    );
  }

  private getVisibleElements(
    scrollElement: HTMLElement,
    thumbnailViewChildren: HTMLCollection
  ): IVisibleThumbnail {
     ...
     */
    function isThumbnailElementBottomAfterViewTop(
      thumbnailViewChildrenElement: HTMLElement
    ): boolean {
      const elementBottom: number =
        thumbnailViewChildrenElement.offsetTop +
        thumbnailViewChildrenElement.clientTop +
        thumbnailViewChildrenElement.clientHeight;
    ...
    let firstVisibleElementInd: number =
      thumbnailViewChildren.length === 0
        ? 0
        : this.binarySearchFirstItem(
            thumbnailViewChildren,
            isThumbnailElementBottomAfterViewTop
          );
    ...
    for (
      let i: number = firstVisibleElementInd,
        ii: number = thumbnailViewChildren.length;
      i < ii;
      i++
    ) {
  ...
  private backtrackBeforeAllVisibleElements(
    ...
    let pageTop: number =
      thumbnailElement.offsetTop + thumbnailElement.clientTop;
    if (pageTop >= top) {
    ...
    for (let i: number = index - 2; i >= 0; --i) {
      ...
      if (
        thumbnailElement.offsetTop +
          thumbnailElement.clientTop +
          thumbnailElement.clientHeight <=
        pageTop
      ) {
        break;
      }
  ...
  private getThumbnailImageElement(index: number): HTMLImageElement {
    ...
    if (thumbnailChild) {
      return thumbnailChild.children[0].children[0]
