import { PointModel, randomId, Point, TextAlign } from '@hoyui/drawings';
import { IElement } from '@hoyui/drawings';
import { rotatePoint } from '@hoyui/drawings';
import { Rect } from '@hoyui/drawings';
import {
  Matrix,
...
import { TextElement } from '@hoyui/drawings';
...
export class ToolBase {
  ...
  protected updateSize(
    ...
    let difx: number =
      this.currentPosition.x / zoom - this.startPosition.x / zoom;
    let dify: number =
      this.currentPosition.y / zoom - this.startPosition.y / zoom;
    const rotateAngle: number =
      shape instanceof TextElement ? angle : shape.rotateAngle;
    ...
    let diff: PointModel;
    const width: number =
      shape instanceof TextElement
        ? shape.actualSize.width
        : shape.wrapper.bounds.width;
    const height: number =
      shape instanceof TextElement
        ? shape.actualSize.height
        : shape.wrapper.bounds.height;
    let obj: any = shape;
...
export class SelectTool extends ToolBase {
  ...
  private mouseEventHelper(args: MouseEventArgs): void {
    ...
    const object: IElement = findActiveElement(
      args as any,
      this.pdfViewerBase,
      this.commandHandler
    );
    ...
    if (object && (object as any).shapeAnnotationType === 'StickyNotes') {
      if (
        (object as any).annotationSettings &&
        (object as any).annotationSettings.isLock
      ) {
        if (
          this.commandHandler.annotationModule.checkAllowedInteractions(
            'Select',
            object
          )
        ) {
    ...
    if (!isLock) {
      ...
      if (
        (args.source as PdfAnnotationBaseModel) &&
        (args as PdfAnnotationBaseModel).annotationSelectorSettings !== null
      ) {
        currentSelctor = (args.source as PdfAnnotationBaseModel)
      ...
      if (this.commandHandler) {
        ...
        if (selectedObject) {
          ...
          const currentSource: PdfAnnotationBaseModel =
            args.source as PdfAnnotationBaseModel;
          if (
            ...
            this.commandHandler.annotationModule.freeTextAnnotationModule
              .isInuptBoxInFocus === false
          ) {
            this.commandHandler.clearSelection(
              this.pdfViewerBase.activeElements.activePageID
            );
          } else if (
            args.info &&
            args.info.ctrlKey &&
            ((currentSource &&
              currentSource.shapeAnnotationType === 'FreeText') ||
              (currentAnnot && currentAnnot.shapeAnnotationType === 'FreeText'))
          ) {
            this.commandHandler.clearSelection(
              this.pdfViewerBase.activeElements.activePageID
            );
            ...
            this.commandHandler.annotationModule &&
            !isNullOrUndefined(
              this.commandHandler.annotation.textMarkupAnnotationModule
            ) &&
            isNullOrUndefined(
              this.commandHandler.annotation.textMarkupAnnotationModule
                .currentTextMarkupAnnotation
            ) &&
            this.commandHandler.formDesignerModule &&
            !(
              (currentSource &&
                currentSource.shapeAnnotationType === 'FreeText') ||
              (currentAnnot &&
                (currentAnnot.shapeAnnotationType === 'FreeText' ||
                  currentAnnot.shapeAnnotationType === 'Image' ||
                  currentAnnot.shapeAnnotationType === 'StickyNotes'))
            )
          ) {
            this.commandHandler.clearSelection(
              this.pdfViewerBase.activeElements.activePageID
          ...
          if (object) {
            if (
              ...
              !isNullOrUndefined(this.pdfViewerBase.isFreeTextSelected) &&
              !this.pdfViewerBase.isFreeTextSelected
            ) {
              this.commandHandler.select(
                [(object as PdfAnnotationBaseModel).id],
                currentSelctor
              );
          ...
          if (
            selectedObject.annotations.length === 0 &&
            annotation &&
            ...
            annotation.shapeAnnotationType !== 'Path' &&
            !annotation.formFieldAnnotationType
          ) {
            if (
              this.commandHandler.enableToolbar &&
              Browser.isDevice &&
              !this.commandHandler.enableDesktopMode
            ) {
              this.commandHandler.toolbarModule.showToolbar(true);
            }
            this.commandHandler.fireAnnotationUnSelect(
          ...
          if (
            ...
            this.commandHandler.fireSignatureUnselect(
          ...
          if (
            selectedObject.formFields.length === 0 &&
            this.commandHandler.formDesignerModule &&
            formField &&
            ...
            const field: IFormField = {
              ...
              rotation: (formField as any).rotation,
            ...
            this.commandHandler.fireFormFieldUnselectEvent(
              'formFieldUnselect',
              field,
              formField.pageIndex
            );
            ...
            this.pdfViewerBase.currentTarget.id &&
            this.commandHandler.formFields &&
            event.type === 'mousedown'
          ) {
            for (
              let i: number = 0;
              i < this.commandHandler.formFields.length;
              i++
            ) {
              const formField: PdfFormFieldBaseModel =
                this.commandHandler.formFields[parseInt(i.toString(), 10)];
              if (
                this.pdfViewerBase.currentTarget &&
                this.pdfViewerBase.currentTarget.id === formField.id
              ) {
                const field: any = {
                  value: (formField as any).value,
                  fontFamily: formField.fontFamily,
                ...
                if (!object) {
                  this.commandHandler.fireFocusOutFormField(
                    field as any,
                    (formField as any).pageIndex
                  );
                  ...
                  if (
                    ...
                    (event.target as any).className !== 'e-pv-text-layer'
                  ) {
                    this.commandHandler.fireFocusOutFormField(
                      field as any,
                      (formField as any).pageIndex
...
export class MoveTool extends ToolBase {
  ...
  public mouseDown(args: MouseEventArgs): void {
    ...
    if (isLineShapes(nodeMouseDown)) {
      this.redoElement.vertexPoints = (
        nodeMouseDown as PdfAnnotationBaseModel
      ).vertexPoints;
      this.redoElement.leaderHeight = (
        nodeMouseDown as PdfAnnotationBaseModel
      ).leaderHeight;
    }
    this.inAction = true;
  ...
  public mouseUp(args: any): void {
    if (this.commandHandler && args.source) {
      ...
      this.commandHandler.select(
        [(args.source as PdfAnnotationBaseModel).id],
        currentSelctor
      );
      if (
        this.pdfViewerBase.activeElements.activePageID === args.source.pageIndex
      ) {
        this.commandHandler.dragSelectedObjects(
          this.calculateMouseActionXDiff(args),
      ...
      if (
        args.source &&
        (args.source.formFieldAnnotationType === 'Textbox' ||
          args.source.formFieldAnnotationType === 'Checkbox' ||
          args.source.formFieldAnnotationType === 'RadioButton' ||
          args.source.formFieldAnnotationType === 'ListBox' ||
          args.source.formFieldAnnotationType === 'SignatureField' ||
          args.source.formFieldAnnotationType === 'InitialField' ||
          args.source.formFieldAnnotationType === 'DropdownList' ||
          args.source.formFieldAnnotationType === 'PasswordField')
      ) {
        this.commandHandler.formDesignerModule.updateHTMLElement(
          args.source as PdfAnnotationBaseModel
        );
      }
      this.commandHandler.renderSelector(
        this.pdfViewerBase.activeElements.activePageID,
        currentSelctor
      );
      ...
      if (isLineShapes(args.source)) {
        newShapeObject.vertexPoints = (
          args.source as PdfAnnotationBaseModel
        ).vertexPoints;
        newShapeObject.leaderHeight = (
          args.source as PdfAnnotationBaseModel
        ).leaderHeight;
      }
      if (
        args.target &&
        (args.target as PdfAnnotationBaseModel).formFieldAnnotationType
      ) {
        ...
        const field: IFormField = {
          ...
          thickness: (node as PdfFormFieldBaseModel).thickness,
        ...
        const currentPosition: IFormFieldBound = {
          X: args.source.wrapper.offsetX,
          Y: args.source.wrapper.offsetY,
          Width: args.source.wrapper.actualSize.width,
          Height: args.source.wrapper.actualSize.height,
        };
        const previousPosition: IFormFieldBound = {
          X: this.offset.x,
          Y: this.offset.y,
          Width: args.source.wrapper.actualSize.width,
          Height: args.source.wrapper.actualSize.height,
        };
        this.commandHandler.fireFormFieldMoveEvent(
          'formFieldMove',
          field,
          node.pageIndex,
          previousPosition,
      ...
      if (
        !isNullOrUndefined(this.redoElement) &&
        (this.redoElement.bounds.height !== newShapeObject.bounds.height ||
          this.redoElement.bounds.width !== newShapeObject.bounds.width ||
          this.redoElement.bounds.x !== newShapeObject.bounds.x ||
          this.redoElement.bounds.y !== newShapeObject.bounds.y)
      ) {
        isDragged = true;
      }
      if (this.commandHandler.annotation && isDragged) {
        this.commandHandler.annotation.addAction(
          (this as any).pageIndex,
          null,
          args.source,
          'Drag',
          '',
          this.redoElement as any,
          newShapeObject
        );
        this.commandHandler.annotation.stampAnnotationModule.updateSessionStorage(
          args.source,
          null,
          'Drag'
        );
        this.commandHandler.annotation.stickyNotesAnnotationModule.updateStickyNotes(
          args.source,
          null
        );
      }
    }
    const shapeAnnotationType: any =
      this.commandHandler &&
      this.commandHandler.selectedItems &&
      this.commandHandler.selectedItems.annotations &&
      this.commandHandler.selectedItems.annotations.length > 0
        ? this.commandHandler.selectedItems.annotations[0].shapeAnnotationType
        : null;
    if (
      shapeAnnotationType &&
  ...
  private calculateMouseActionXDiff(args: MouseEventArgs): number {
    const x: number =
      this.calculateMouseXDiff() /
      this.commandHandler.viewerBase.getZoomFactor();
  ...
  public mouseMove(
    ...
    if (this.inAction) {
      ...
      const selectedItem: PdfAnnotationBaseModel =
        this.commandHandler.selectedItems.annotations[0];
      let cobject: any;
      if (!this.helper) {
        cobject =
        ...
        if (
          ...
          this.helper = cobject = this.commandHandler.add(
            cobject as PdfAnnotationBase
          );
        } else {
      ...
      if (selectedItem && selectedItem.annotName) {
        this.commandHandler.annotation.triggerAnnotationMove(
          selectedItem,
          true
        );
      }
  ...
  private checkisAnnotationMove(args: any): void {
    if (
      ...
      if (
        this.commandHandler.selectedItems.annotations[0].annotName ===
        args.source.annotName
      ) {
    ...
    if (
      ...
      if (
        this.commandHandler.selectedItems.formFields[0].name ===
        args.source.name
      ) {
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (!this.inAction) {
      ...
      const nodeElement: PdfAnnotationBaseModel =
        this.commandHandler.annotation.stampAnnotationModule.moveStampElement(
          args.position.x,
          args.position.y,
          pageIndex
        );
      if (nodeElement.shapeAnnotationType === 'SignatureText') {
        const textWidth: number = this.getTextWidth(
          nodeElement.data,
          nodeElement.fontSize,
          nodeElement.fontFamily
        );
        let widthRatio: number = 1;
        if (textWidth > nodeElement.bounds.width) {
          widthRatio = nodeElement.bounds.width / textWidth;
        }
        nodeElement.fontSize = this.getFontSize(
          Math.floor(nodeElement.fontSize * widthRatio)
        );
        const defaultFontSize: number = 32; // default font size.
        nodeElement.bounds.height =
          nodeElement.fontSize < defaultFontSize
            ? nodeElement.fontSize * 2
            : nodeElement.bounds.height;
        nodeElement.thickness = 0;
      }
      newObject = this.commandHandler.add(nodeElement as PdfAnnotationBase);
      args.source = this.commandHandler.annotations[
        this.commandHandler.annotations.length - 1
      ] as IElement;
      ...
      if (
        currentSource &&
        (currentSource.shapeAnnotationType === 'HandWrittenSignature' ||
          currentSource.shapeAnnotationType === 'SignatureText' ||
          currentSource.shapeAnnotationType === 'SignatureImage')
      ) {
        this['offset'] = {
          x: args.source.wrapper.offsetX - args.source.wrapper.bounds.width / 2,
          y:
            args.source.wrapper.offsetY - args.source.wrapper.bounds.height / 2,
        };
      } else {
        this['offset'] = {
          x: args.source.wrapper.offsetX,
    ...
    this.commandHandler.renderSelector(
      (args.source as PdfAnnotationBaseModel).pageIndex,
      currentSelctor
    );
...
export class InkDrawingTool extends ToolBase {
  ...
  public mouseDown(args: MouseEventArgs): void {
    ...
    this.commandHandler.annotation.inkAnnotationModule.drawInkInCanvas(
      node,
      this.pdfViewerBase.activeElements.activePageID
    );
  }
...
export class ConnectTool extends ToolBase {
  ...
  public mouseDown(args: MouseEventArgs): void {
    ...
    if (isLineShapes(nodeElement)) {
      this.redoElement.vertexPoints = (
        nodeElement as PdfAnnotationBaseModel
      ).vertexPoints;
      this.redoElement.leaderHeight = (
        nodeElement as PdfAnnotationBaseModel
      ).leaderHeight;
      if (
        nodeElement.measureType === 'Distance' ||
        nodeElement.measureType === 'Perimeter' ||
        nodeElement.measureType === 'Area' ||
        nodeElement.measureType === 'Volume'
      ) {
    ...
    if (this.commandHandler) {
      ...
      if (node) {
        const annotationSettings: any =
          this.commandHandler.annotationModule.findAnnotationSettings(node);
        let annotationMaxHeight: number = 0;
        let annotationMaxWidth: number = 0;
        let annotationMinHeight: number = 0;
        let annotationMinWidth: number = 0;
        ...
        if (node.vertexPoints.length > 3) {
          const sizeObject: any =
            this.commandHandler.viewerBase.checkAnnotationWidth(
              node.vertexPoints
            );
          ...
          if (
            ...
            if (
              (height > annotationMinHeight && height < annotationMaxHeight) ||
          ...
          if (
            ...
            if (
              ...
              if (
                diff < (annotationMaxHeight || annotationMaxWidth) &&
                diff > (annotationMinHeight || annotationMinWidth)
              ) {
                this.commandHandler.nodePropertyChange(this.prevSource, {
                  vertexPoints: node.vertexPoints,
                  leaderHeight: node.leaderHeight,
        ...
        if (
          node.measureType === 'Distance' ||
          node.measureType === 'Perimeter' ||
          node.measureType === 'Area' ||
          node.measureType === 'Volume'
        ) {
          this.commandHandler.annotation.updateCalibrateValues(
            this.commandHandler.selectedItems.annotations[0]
          );
          newShapeElementObject.notes = (
            args.source as PdfAnnotationBaseModel
          ).notes;
        }
        if (isLineShapes(args.source)) {
          newShapeElementObject.vertexPoints = (
            args.source as PdfAnnotationBaseModel
          ).vertexPoints;
          newShapeElementObject.leaderHeight = (
            args.source as PdfAnnotationBaseModel
          ).leaderHeight;
        }
        if (
          this.redoElement.bounds.height !==
            newShapeElementObject.bounds.height ||
          this.redoElement.bounds.width !==
            newShapeElementObject.bounds.width ||
          this.redoElement.bounds.x !== newShapeElementObject.bounds.x ||
          this.redoElement.bounds.y !== newShapeElementObject.bounds.y
        ) {
          isResized = true;
        }
        if (isResized) {
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (this.currentPosition && this.prevPosition) {
      ...
      if (
        (this.inAction && this.endPoint !== undefined && diffX !== 0) ||
        diffY !== 0
      ) {
        if (!this.helper) {
          let cloneShapebject: any = cloneObject(
            this.commandHandler.selectedItems.annotations[0]
          ) as PdfAnnotationBaseModel;
          ...
          this.helper = cloneShapebject = this.commandHandler.add(
            cloneShapebject as PdfAnnotationBase
          );
...
export class ResizeTool extends ToolBase {
  ...
  public mouseDown(args: MouseEventArgs): void {
    ...
    if (isLineShapes(node)) {
      this.redoElement.vertexPoints = (
        node as PdfAnnotationBaseModel
      ).vertexPoints;
      this.redoElement.leaderHeight = (
        node as PdfAnnotationBaseModel
      ).leaderHeight;
  ...
  public mouseUp(args: MouseEventArgs, isPreventHistory?: boolean): boolean {
    const object: PdfAnnotationBaseModel | SelectorModel = args.source as
      | PdfAnnotationBaseModel
      | Selector;
    ...
    if (this.commandHandler && this.prevSource) {
      ...
      this.commandHandler.select(
        [this.prevSource.id],
        this.prevSource.annotationSelectorSettings
      );
      const deltaValues: Rect = this.updateSize(
        this.prevSource,
        this.currentPosition,
      ...
      if (
        ...
        if (
          this.commandHandler.stampSettings.minHeight ||
          this.commandHandler.stampSettings.minWidth
        ) {
          this.commandHandler.select(
            [this.prevSource.id],
            this.prevSource.annotationSelectorSettings
      ...
      if (
        this.commandHandler.selectedItems.formFields.length > 0 &&
        (this.commandHandler.selectedItems.formFields[0]
          .formFieldAnnotationType === 'Textbox' ||
          this.commandHandler.selectedItems.formFields[0]
            .formFieldAnnotationType === 'Checkbox' ||
          this.commandHandler.selectedItems.formFields[0]
            .formFieldAnnotationType === 'RadioButton' ||
          this.commandHandler.selectedItems.formFields[0]
            .formFieldAnnotationType === 'InitialField' ||
          this.commandHandler.selectedItems.formFields[0]
            .formFieldAnnotationType === 'SignatureField' ||
          this.commandHandler.selectedItems.formFields[0]
            .formFieldAnnotationType === 'DropdownList' ||
          this.commandHandler.selectedItems.formFields[0]
            .formFieldAnnotationType === 'ListBox' ||
          this.commandHandler.selectedItems.formFields[0]
            .formFieldAnnotationType === 'PasswordField')
      ) {
        if (
          this.commandHandler.selectedItems.formFields[0]
            .formFieldAnnotationType === 'SignatureField'
        ) {
          this.commandHandler.selectedItems.formFields[0].signatureIndicatorSettings =
            this.commandHandler.selectedItems.formFields[0]
      ...
      if (this.commandHandler.annotation && args.source.wrapper) {
        ...
        if (isLineShapes(args.source)) {
          newObject.vertexPoints = (
            args.source as PdfAnnotationBaseModel
          ).vertexPoints;
          newObject.leaderHeight = (
            args.source as PdfAnnotationBaseModel
          ).leaderHeight;
        }
        if (
          this.redoElement.bounds.height !== newObject.bounds.height ||
          this.redoElement.bounds.width !== newObject.bounds.width ||
          this.redoElement.bounds.x !== newObject.bounds.x ||
          this.redoElement.bounds.y !== newObject.bounds.y
        ) {
          isResized = true;
        }
        ...
        if (this.prevSource.shapeAnnotationType === 'SignatureText') {
          const oldObjectWidth: number =
            oldObject.bounds && oldObject.bounds.width
              ? oldObject.bounds.width
              : oldObject.width;
          const boundsRatio: number = newObject.bounds.width / oldObjectWidth;
          newObject.fontSize =
            (this.prevSource as any).wrapper.children[1].style.fontSize *
            boundsRatio;
          if (args.target != null) {
            (args.target as PdfAnnotationBaseModel).fontSize =
              newObject.fontSize;
            (
              (args.target as PdfAnnotationBaseModel).wrapper.children[1] as any
            ).style.fontSize = newObject.fontSize;
            (
              args.target as PdfAnnotationBaseModel
            ).wrapper.children[1].horizontalAlignment = 'Center';
            (
              args.target as PdfAnnotationBaseModel
            ).wrapper.children[1].verticalAlignment = 'Center';
            (
              args.target as PdfAnnotationBaseModel
            ).wrapper.children[1].setOffsetWithRespectToBounds(
              0,
              0,
              'Absolute'
            );
            (
              this.commandHandler.selectedItems.annotations[0].wrapper
                .children[1] as any
            ).style.fontSize = newObject.fontSize;
            this.commandHandler.selectedItems.annotations[0].wrapper.children[1].horizontalAlignment =
              'Center';
            this.commandHandler.selectedItems.annotations[0].wrapper.children[1].verticalAlignment =
              'Center';
            this.commandHandler.selectedItems.annotations[0].wrapper.children[1].setOffsetWithRespectToBounds(
              0,
              0,
              'Absolute'
            );
            this.commandHandler.selectedItems.annotations[0].fontSize =
              newObject.fontSize;
          }
        }
        if (
          this.prevSource.shapeAnnotationType === 'SignatureText' &&
          this.commandHandler.selectedItems.annotations &&
          this.commandHandler.selectedItems.annotations.length > 0
        ) {
          this.commandHandler.nodePropertyChange(
            this.commandHandler.selectedItems.annotations[0],
      ...
      if (
        ...
        this.commandHandler.fireFormFieldResizeEvent(
          'formFieldResize',
          field,
          node.pageIndex,
          previousPosition,
      ...
      if (
        ...
        this.commandHandler.annotation.stampAnnotationModule
      ) {
        this.commandHandler.annotation.stampAnnotationModule.updateSessionStorage(
          args.source,
          this.prevSource.id,
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (this.currentElement.wrapper) {
      changes = rotatePoint(
        -this.currentElement.wrapper.rotateAngle,
        undefined,
        undefined,
        changes
      );
  ...
  private getTooltipContent(pdfAnnotationBaseModel: SelectorModel): string {
    return (
      'W:' +
      Math.round(pdfAnnotationBaseModel.wrapper.bounds.width) +
      ' ' +
      'H:' +
      Math.round(pdfAnnotationBaseModel.wrapper.bounds.height)
    );
  }
  ...
  private scaleObjects(
    ...
    if (
      ...
      source.annotations.length === 1 &&
      (source.annotations[0].shapeAnnotationType === 'Perimeter' ||
        source.annotations[0].shapeAnnotationType === 'Radius' ||
        (source as PdfAnnotationBaseModel).shapeAnnotationType === 'Stamp')
    ) {
      if (!(deltaHeight === 1 && deltaWidth === 1)) {
        deltaHeight = deltaWidth = Math.max(
          deltaHeight === 1 ? 0 : deltaHeight,
          deltaWidth === 1 ? 0 : deltaWidth
        );
      ...
      (source as PdfAnnotationBaseModel).shapeAnnotationType === 'Image' ||
      ...
      if (
        (source as PdfAnnotationBaseModel).shapeAnnotationType ===
          'Perimeter' ||
        (source as PdfAnnotationBaseModel).shapeAnnotationType === 'Radius' ||
        (source as PdfAnnotationBaseModel).shapeAnnotationType === 'Stamp'
      ) {
        if (
          (source as PdfAnnotationBaseModel).shapeAnnotationType === 'Stamp'
    ...
    this.blocked = this.commandHandler.scaleSelectedItems(
      deltaWidth,
      deltaHeight,
...
export class NodeDrawingTool extends ToolBase {
  ...
  public mouseDown(args: MouseEventArgs): void {
    if (
      ...
      (event.target as any).className !== 'e-pv-page-container'
    ) {
      super.mouseDown(args);
      this.inAction = true;
      const node: PdfAnnotationBaseModel = {
      ...
      node.id = randomId();
      this.sourceObject.pageIndex = node.pageIndex =
        this.pdfViewerBase.activeElements.activePageID || 0;
      ...
      const formFieldElement: HTMLElement = document.getElementById(
      ...
      this.commandHandler.drawingObject = this.drawingObject =
        this.commandHandler.add(
          this.sourceObject || (node as any)
        ) as PdfAnnotationBaseModel;
      if (
        this.drawingObject.formFieldAnnotationType === 'Textbox' ||
        this.drawingObject.formFieldAnnotationType === 'SignatureField' ||
        this.drawingObject.formFieldAnnotationType === 'InitialField' ||
        this.drawingObject.formFieldAnnotationType === 'Checkbox' ||
        this.drawingObject.formFieldAnnotationType === 'ListBox' ||
        this.drawingObject.formFieldAnnotationType === 'RadioButton' ||
        this.drawingObject.formFieldAnnotationType === 'DropdownList' ||
        this.drawingObject.formFieldAnnotationType === 'PasswordField'
      ) {
        this.inAction = false;
        (this.drawingObject as any).pageNumber =
          this.pdfViewerBase.getActivePage(true);
        const bounds: any =
          this.commandHandler.formDesignerModule.updateFormFieldInitialSize(
            this.drawingObject.wrapper.children[0],
            this.drawingObject.formFieldAnnotationType
          );
        const pageIndex: any = this.drawingObject.pageIndex;
        const page: HTMLElement = this.pdfViewerBase.getElement(
          '_pageDiv_' + pageIndex
        );
        ...
        let offsetX: number;
        if (
          this.currentPosition.y >= pageHeight &&
          event.target &&
        ...
        let rect: any;
        if (
          this.currentPosition.x >= pageWidth &&
          this.currentPosition.y >= pageHeight
        ) {
          rect = {
            x: pageWidth,
        ...
        this.drawingObject.bounds.x =
          this.drawingObject.bounds.x - this.drawingObject.bounds.width / 2;
        this.drawingObject.bounds.y =
          this.drawingObject.bounds.y - this.drawingObject.bounds.height / 2;
        ...
        const formField: FormFieldModel = {
          ...
          bounds: drawingObject.bounds as IFormFieldBound,
          ...
          customData: drawingObject.customData ? drawingObject.customData : '',
        };
        this.commandHandler.formFieldCollections.push(formField);
        this.commandHandler.formDesignerModule.drawHTMLContent(
          this.drawingObject.formFieldAnnotationType,
          this.drawingObject.wrapper.children[0] as DiagramHtmlElement,
          this.drawingObject,
          this.drawingObject.pageIndex,
          this.commandHandler
        );
        this.commandHandler.select(
          [this.commandHandler.drawingObject.id],
          this.commandHandler.annotationSelectorSettings
        );
    ...
    if (
      ...
      if (!isNullOrUndefined(this.drawingObject)) {
        ...
        if (this.drawingObject.shapeAnnotationType === 'Radius') {
          this.updateRadiusLinePosition(
            this.drawingObject.wrapper.children[1],
            this.drawingObject
          );
        }
      }
  ...
  public mouseUp(args: MouseEventArgs): void {
    if (this.drawingObject && this.dragging) {
      ...
      const drawnAnnotation: any =
        this.commandHandler.selectedItems.annotations[0];
      if (
        !isNullOrUndefined(drawnAnnotation) &&
        !isNullOrUndefined(drawnAnnotation.wrapper)
      ) {
   ...
   * @returns {void}
   */
  public updateRadiusLinePosition(
    obj: DrawingElement,
...
export class PolygonDrawingTool extends ToolBase {
  ...
  public mouseDown(args: MouseEventArgs): void {
    ...
    if (!this.drawingObject) {
      ...
      pt = { x: pt.x, y: pt.y };
      const lastPoint: PointModel =
        this.drawingObject.vertexPoints[
          this.drawingObject.vertexPoints.length - 1
        ];
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (
      ...
      if (this.drawingObject && this.currentPosition) {
        obj.vertexPoints[obj.vertexPoints.length - 1].x =
          this.currentPosition.x / this.pdfViewerBase.getZoomFactor();
        obj.vertexPoints[obj.vertexPoints.length - 1].y =
          this.currentPosition.y / this.pdfViewerBase.getZoomFactor();
        this.commandHandler.nodePropertyChange(obj, {
          vertexPoints: obj.vertexPoints,
        });
      }
      if (obj.measureType === 'Perimeter') {
        updatePerimeterLabel(
          obj,
          obj.vertexPoints,
  ...
  public mouseUp(
    ...
    if (
      ...
      currentSelector = (args.source as PdfAnnotationBaseModel)
        .annotationSelectorSettings;
    } else {
      currentSelector = '';
    }
    if (
      this.drawingObject &&
      this.drawingObject.vertexPoints.length === 2 &&
      isDoubleClineck &&
      isMouseLeave
    ) {
          ...
          if (this.drawingObject) {
            if (!isMouseLeave) {
              if (
                ...
                this.drawingObject.vertexPoints.splice(
                  this.drawingObject.vertexPoints.length - 1,
                  1
                );
              }
            ...
            if (this.action === 'Polygon') {
              if (!isMouseLeave) {
                ...
                this.drawingObject.vertexPoints[
                  this.drawingObject.vertexPoints.length
                ] = this.drawingObject.vertexPoints[0];
              }
              this.commandHandler.nodePropertyChange(this.drawingObject, {
                vertexPoints: this.drawingObject.vertexPoints,
              });
              const cobject: PdfAnnotationBase = cloneObject(
                this.drawingObject
              ) as PdfAnnotationBase;
              ...
              const drawingObject: PdfAnnotationBaseModel =
                this.commandHandler.selectedItems.annotations[0];
              if (drawingObject) {
                if (
                  this.commandHandler.enableShapeAnnotation &&
                  (isNullOrUndefined(drawingObject.measureType) ||
                    drawingObject.measureType === '')
                ) {
                  this.commandHandler.annotation.shapeAnnotationModule.renderShapeAnnotations(
                    drawingObject,
                    drawingObject.pageIndex
                  );
                }
                if (
                  this.commandHandler.enableMeasureAnnotation &&
                  (drawingObject.measureType === 'Area' ||
                    drawingObject.measureType === 'Volume')
                ) {
                  if (drawingObject.measureType === 'Area') {
                    drawingObject.notes =
                      this.commandHandler.annotation.measureAnnotationModule.calculateArea(
                        drawingObject.vertexPoints
                      );
                    this.commandHandler.annotation.stickyNotesAnnotationModule.addTextToComments(
                      drawingObject.annotName,
                      drawingObject.notes
                    );
                  } else if (drawingObject.measureType === 'Volume') {
                    drawingObject.notes =
                      this.commandHandler.annotation.measureAnnotationModule.calculateVolume(
                        drawingObject.vertexPoints
                      );
                    this.commandHandler.annotation.stickyNotesAnnotationModule.addTextToComments(
                      drawingObject.annotName,
                  ...
                  this.commandHandler.annotation.measureAnnotationModule.renderMeasureShapeAnnotations(
                    drawingObject,
                    drawingObject.pageIndex
                  );
                }
              ...
              this.commandHandler.nodePropertyChange(this.drawingObject, {
                ...
                sourceDecoraterShapes:
                  this.commandHandler.drawingObject.sourceDecoraterShapes,
                taregetDecoraterShapes:
                  this.commandHandler.drawingObject.taregetDecoraterShapes,
              });
              this.commandHandler.select(
                [this.drawingObject.id],
                currentSelector
              ...
              if (
                this.commandHandler.enableMeasureAnnotation &&
                this.drawingObject.measureType === 'Perimeter'
              ) {
                this.commandHandler.renderDrawing(
                  null,
                  this.drawingObject.pageIndex
                );
                this.drawingObject.notes =
                  this.commandHandler.annotation.measureAnnotationModule.calculatePerimeter(
                    this.drawingObject
                  );
            ...
            const annotationObject: PdfAnnotationBaseModel =
              this.commandHandler.selectedItems.annotations[0];
            this.commandHandler.annotation.addAction(
              (this as any).pageIndex,
              null,
...
export class LineTool extends ToolBase {
  ...
  public mouseUp(args: MouseEventArgs): void {
    if (this.dragging) {
      ...
      if (
        this.endPoint &&
        this.endPoint.indexOf('ConnectorSegmentPoint') > -1 &&
        this.dragging
      ) {
        this.commandHandler.annotation.updateCalibrateValues(
          this.drawingObject
    ...
    if (
      ...
      if (this.currentPosition && this.prevPosition) {
        ...
        if (
          ...
          this.blocked = !this.commandHandler.dragConnectorEnds(
            this.endPoint,
            this.drawingObject as IElement,
            this.currentPosition,
            this.selectedSegment,
            args.target,
            null,
...
export class RotateTool extends ToolBase {
  ...
  public mouseUp(args: MouseEventArgs): void {
    ...
    if (this.undoElement.rotateAngle !== object.wrapper.rotateAngle) {
      const currentSelector: any = (args.source as Selector).annotations[0]
        .annotationSelectorSettings;
      this.commandHandler.renderSelector(
        this.pdfViewerBase.activeElements.activePageID,
        currentSelector
