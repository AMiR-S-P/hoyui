...
import { IElement, ThumbsConstraints } from '@hoyui/drawings';
import { Container } from '@hoyui/drawings';
import { PointModel } from '@hoyui/drawings';
import { Point } from '@hoyui/drawings';
import {
  ...
  public init(diagram: any): Container {
    ...
    if (this.formFields && this.formFields.length > 0) {
      for (let i: number = 0; i < this.formFields.length; i++) {
        const node: PdfFormFieldBaseModel =
          diagram.pdfViewer.nameTable[
            this.formFields[parseInt(i.toString(), 10)].id
          ];
      ...
      for (let i: number = 0; i < this.annotations.length; i++) {
        if (!isNullOrUndefined(this.annotations[parseInt(i.toString(), 10)])) {
          const node: PdfAnnotationBaseModel =
            diagram.pdfViewer.nameTable[
              this.annotations[parseInt(i.toString(), 10)].id
            ];
          const wrapper: Container = node.wrapper;
          container.children.push(wrapper);
        }
