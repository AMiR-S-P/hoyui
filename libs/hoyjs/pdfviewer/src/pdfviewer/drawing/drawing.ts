...
import { DrawingElement } from '@hoyui/drawings';
import { PathElement } from '@hoyui/drawings';
import { createMeasureElements } from '@hoyui/drawings';
import { randomId } from '@hoyui/drawings';
import {
  Size,
...
import { SvgRenderer } from '@hoyui/drawings';
import { SelectorModel } from './selector-model';
import {
  isLineShapes,
  setElementStype,
  findPointsLength,
  getBaseShapeAttributes,
  isLeader,
  Leader,
  cloneObject,
} from './drawing-util';
...
import { isNullOrUndefined, isBlazor, Browser } from '@hoyui/base';
import {
...
export class Drawing {
    ...
    if (annotations) {
      for (let i: number = 0; i < annotations.length; i++) {
        const annotation: PdfAnnotationBaseModel =
          annotations[parseInt(i.toString(), 10)];
  ...
  public getPageTable(pageId: number): ZOrderPageTable {
    ...
    if (this.pdfViewer.zIndexTable.length !== undefined) {
      ...
      for (let i: number = 0; i < this.pdfViewer.zIndexTable.length; i++) {
        if (
          this.pdfViewer.zIndexTable[parseInt(i.toString(), 10)].pageId ===
          pageId
        ) {
          notFound = false;
   ...
   */
  public initObject(
    obj: PdfAnnotationBaseModel | PdfFormFieldBaseModel
  ): PdfAnnotationBaseModel | PdfFormFieldBaseModel {
    ...
    if (
      (obj as PdfAnnotationBaseModel | PdfFormFieldBaseModel).wrapper === null
    ) {
      //Init default wrapper
    }
    (this.pdfViewer.nameTable as any)[
      (obj as PdfAnnotationBaseModel | PdfFormFieldBaseModel).id
    ] = obj;
    if (obj.formFieldAnnotationType) {
      this.nodePropertyChange(obj as PdfFormFieldBaseModel, {
        bounds: { width: obj.bounds.width, height: obj.bounds.height },
      });
    }
  ...
  private initNode(obj: PdfAnnotationBaseModel | PdfFormFieldBaseModel): void {
    ...
    canvas.measure(
      new Size(
        (obj as PdfAnnotationBaseModel | PdfFormFieldBaseModel).wrapper.width,
  ...
  public init(
    ...
    const annotationSettings: any = this.pdfViewer.annotationModule
      ? this.pdfViewer.annotationModule.findAnnotationSettings(obj)
      : {};
    ...
    if (
      annotationSettings.minWidth ||
      annotationSettings.maxWidth ||
      annotationSettings.minHeight ||
      annotationSettings.maxHeight
    ) {
      annotationMaxHeight = annotationSettings.maxHeight
        ? annotationSettings.maxHeight
        : 2000;
      annotationMaxWidth = annotationSettings.maxWidth
        ? annotationSettings.maxWidth
        : 2000;
      annotationMinHeight = annotationSettings.minHeight
        ? annotationSettings.minHeight
        : 0;
      annotationMinWidth = annotationSettings.minWidth
        ? annotationSettings.minWidth
        : 0;
    }
    ...
    if ((obj as PdfAnnotationBaseModel).shapeAnnotationType !== 'Stamp') {
      ...
      if (obj.bounds.height !== undefined) {
        ...
        if (isAnnotationSet) {
          if (
            ...
            content.width > annotationMaxHeight
          ) {
      ...
      if (
        this.pdfViewer.viewerBase.drawSignatureWithTool &&
        (obj as PdfAnnotationBaseModel).shapeAnnotationType === 'SignatureText'
      ) {
  ...
  private initAnnotationObject(
    obj: PdfAnnotationBaseModel,
    pathContent: PathElement,
    content: DrawingElement,
    canvas: Container,
    isStamp: boolean,
    basicElement: DrawingElement,
    isAnnotationSet: boolean,
    annotationMaxHeight: number,
    annotationMaxWidth: number,
    annotationMinWidth: number,
    annotationMinHeight: number
  ): DrawingElement {
    switch (obj.shapeAnnotationType) {
      ...
      case 'Stamp':
        ...
        if (
          obj &&
          obj.annotationAddMode &&
          (obj.annotationAddMode === 'Existing Annotation' ||
            obj.annotationAddMode === 'Imported Annotation')
        ) {
      ...
      case 'Perimeter': {
        ...
        basicElement.height = 0.2;
        basicElement.width = 0.2;
      ...
      case 'Radius': {
        ...
        if (
          !this.pdfViewer.enableImportAnnotationMeasurement &&
          obj.notes &&
          obj.notes !== ''
        ) {
          radiusTextEle.content = obj.notes;
        } else {
          radiusTextEle.content =
            this.pdfViewer.annotation.measureAnnotationModule.setConversion(
              (obj.bounds.width / 2) *
  ...
  private textElement(obj: PdfAnnotationBaseModel): TextElement {
    ...
    textele.verticalAlignment =
      obj.shapeAnnotationType === 'SignatureText' ? 'Center' : 'Top';
    textele.relativeMode = 'Object';
    textele.setOffsetWithRespectToBounds(0.5, 0.5, 'Absolute');
    return textele;
  }
  ...
  public setNodePosition(
    ...
    if (node.shapeAnnotationType === 'Perimeter') {
      ...
      const rotatedPoint: PointModel = transformPointByMatrix(
        matrix,
        trasPoint
      );
  ...
  public initLine(obj: PdfAnnotationBaseModel): Canvas {
    ...
    if (
      (obj.shapeAnnotationType === 'Line' ||
        obj.shapeAnnotationType === 'LineWidthArrowHead') &&
      obj.measureType === 'Perimeter'
    ) {
      labels = initPerimeterLabel(
        obj,
        points,
        this.pdfViewer.annotation.measureAnnotationModule,
        this.pdfViewer
    ...
    if (
      obj.enableShapeLabel === true &&
      !(obj.shapeAnnotationType === 'Distance') &&
      !(obj.measureType === 'Perimeter')
    ) {
    ...
    if (
      ...
      targetDecorator = getDecoratorElement(
        ...
        anglePoints[anglePoints.length - 2],
        false
      );
    }
  ...
  public add(obj: PdfAnnotationBaseModel): PdfAnnotationBaseModel {
    ...
    if (obj.formFieldAnnotationType && this.pdfViewer.formDesignerModule) {
      obj = new PdfFormFieldBase(
        this.pdfViewer,
        'formFields',
        obj as PdfFormFieldBase,
        true
      );
      obj = this.initObject(obj) as PdfFormFieldBaseModel;
      this.pdfViewer.formFields.push(obj);
    } else {
      obj = new PdfAnnotationBase(
        this.pdfViewer,
        'annotations',
        obj as PdfAnnotationBase,
        true
      );
  ...
  public remove(obj: PdfAnnotationBaseModel): void {
    ...
    for (let i: number = 0; i < this.pdfViewer.annotations.length; i++) {
      const annotation: PdfAnnotationBaseModel =
        this.pdfViewer.annotations[parseInt(i.toString(), 10)];
      if (
        (annotation.id &&
          (annotation.id === obj.id ||
            annotation.id.split('_')[0] === obj.id)) ||
        (annotation.wrapper && annotation.wrapper.id === obj.id)
      ) {
        this.pdfViewer.annotations.splice(i, 1);
        const objects: PdfAnnotationBaseModel[] = this.getPageObjects(
          obj.pageIndex
        );
        for (let j: number = 0; j < objects.length; j++) {
          if (objects[parseInt(j.toString(), 10)].id === obj.id) {
            objects.splice(j, 1);
            if (
              obj.shapeAnnotationType === 'Path' ||
              obj.shapeAnnotationType === 'SignatureImage' ||
              obj.shapeAnnotationType === 'SignatureText'
            ) {
    ...
    for (let i: number = 0; i < this.pdfViewer.formFields.length; i++) {
      const element: PdfAnnotationBaseModel =
        this.pdfViewer.formFields[parseInt(i.toString(), 10)];
      if (element.id === obj.id || element.wrapper.id === obj.id) {
        ...
        const objects: PdfFormFieldBaseModel[] = this.getPageObjects(
          obj.pageIndex
        );
        for (let j: number = 0; j < objects.length; j++) {
          if (objects[parseInt(j.toString(), 10)].id === obj.id) {
          ...
          if (
            objects[parseInt(j.toString(), 10)] &&
            objects[parseInt(j.toString(), 10)].id &&
    ...
    if (
      obj.formFieldAnnotationType === 'Textbox' ||
      obj.formFieldAnnotationType === 'Checkbox' ||
      obj.formFieldAnnotationType === 'RadioButton' ||
      obj.formFieldAnnotationType === 'PasswordField' ||
      obj.formFieldAnnotationType === 'DropdownList' ||
      obj.formFieldAnnotationType === 'ListBox' ||
      obj.formFieldAnnotationType === 'SignatureField' ||
      obj.formFieldAnnotationType === 'InitialField'
    ) {
      const inputField: HTMLElement = document.getElementById(
        'form_field_' + obj.id + '_content_html_element'
      );
      if (inputField) {
        inputField.remove();
        this.pdfViewer.renderDrawing(undefined, index);
      ...
      for (
        let i: number = 0;
        i < this.pdfViewer.formFieldCollections.length;
        i++
      ) {
        const element: FormFieldModel =
          this.pdfViewer.formFieldCollections[parseInt(i.toString(), 10)];
      ...
      for (
        let i: number = 0;
        i < this.pdfViewer.viewerBase.formFieldCollection.length;
        i++
      ) {
        // eslint-disable-next-line
        if (
          obj.id ==
          this.pdfViewer.viewerBase.formFieldCollection[
      ...
      const field: IFormField = {
        ...
        alignment: (obj as any).alignment,
        isReadonly: (obj as any).isReadonly,
        visibility: (obj as any).visibility,
        // eslint-disable-next-line
        maxLength: (obj as any).maxLength,
        isRequired: (obj as any).isRequired,
        isPrint: obj.isPrint,
        rotation: (obj as any).rotateAngle,
        tooltip: (obj as any).tooltip,
        // eslint-disable-next-line
        options: (obj as any).options,
        isChecked: (obj as any).isChecked,
        isSelected: (obj as any).isSelected,
      };
      if (!isNullOrUndefined(field) && field.id !== 'diagram_helper') {
        this.pdfViewer.fireFormFieldRemoveEvent(
          'formFieldRemove',
          field,
          obj.pageIndex
        );
      }
      this.pdfViewer.formDesignerModule.removeFieldsFromAnnotationCollections(
        obj.id,
        field.name
      );
    }
   ...
   * @returns {void}
   */
  public refreshCanvasDiagramLayer(
    diagramLayer?: HTMLCanvasElement,
    pageIndex?: number,
    objectId?: string
  ): void {
    if (!diagramLayer) {
      diagramLayer = document.getElementById(
        this.pdfViewer.element.id + '_annotationCanvas_' + pageIndex
    ...
    if (diagramLayer) {
      ...
      const devicePixelRatio: number =
        this.pdfViewer.viewerBase.getWindowDevicePixelRatio();
      ...
      const uniqueObjects: any = objects.filter(
        (obj: any, index: number, self: any) => {
          if (obj.id.split('_')[0] === 'free') {
            ...
            return (
              index ===
              self.findIndex(
                (t: any) => t.id.split('_')[0] === obj.id.split('_')[0]
              )
            );
          }
      ...
      for (let i: number = 0; i < uniqueObjects.length; i++) {
        ...
        if (
          diagramLayer.id ===
          this.pdfViewer.element.id + '_print_annotation_layer_' + pageIndex
        ) {
          if (uniqueObjects[parseInt(i.toString(), 10)].isPrint) {
            renderElement = (this.pdfViewer.nameTable as any)[
              uniqueObjects[parseInt(i.toString(), 10)].id
            ].wrapper;
          ...
          const uniqueObjectId: string =
            uniqueObjects[parseInt(i.toString(), 10)].id;
          const uniqueObject: any = (this.pdfViewer.nameTable as any)[
            `${uniqueObjectId}`
          ];
          if (
            (renderElement && this.shouldRefreshElement(uniqueObject)) ||
  ...
  private shouldRefreshElement(uniqueObject: any): boolean {
    const parentObject: any = (this.pdfViewer.nameTable as any)[
      uniqueObject.id.split('_')[0]
    ];

    return (
      (!isNullOrUndefined(parentObject) &&
        uniqueObject.visibility === 'visible') ||
      (!isNullOrUndefined(uniqueObject.subject) &&
        uniqueObject.annotName !== 'SignatureField' &&
        isNullOrUndefined(uniqueObject.visibility)) ||
  ...
  public clearHighlighter(index?: number): void {
    const adornerSvg: SVGElement = this.getAdornerLayerSvg(
      this.pdfViewer.element.id + index + '_diagramAdornerLayer',
      index
    );
    if (adornerSvg) {
  ...
  public getSelectorElement(diagramId: string, index?: number): SVGElement {
    ...
    if (adornerSvg) {
      adornerLayer = adornerSvg.getElementById(
        diagramId + index + '_SelectorElement'
      ) as SVGElement;
    }
    return adornerLayer;
  }
  ...
  public getAdornerLayerSvg(diagramId: string, index?: number): SVGSVGElement {
    ...
    const diagramElement: HTMLElement = getDiagramElement(
      diagramId + index + '_diagramAdornerLayer'
    );
    let elementcoll: any;
    if (diagramElement) {
      elementcoll = diagramElement.getElementsByClassName(
        'e-adorner-layer' + index
      );
  ...
  public clearSelectorLayer(index?: number): void {
    ...
    if (adornerSvg) {
      const selectionRect: SVGElement = (
        ...
        this.pdfViewer.adornerSvgLayer.id + '_selected_region'
      ) as SVGElement;
      if (selectionRect) {
        selectionRect.parentNode.removeChild(selectionRect);
      }
      this.clearHighlighter(index);
      const childNodes: NodeList = this.getSelectorElement(
        this.pdfViewer.element.id,
        index
      ).childNodes;
      ...
      if (selectorModel.wrapper) {
        ...
        if (selectorModel.annotations.length === 1) {
          ...
          selectorModel.wrapper.rotateAngle =
            selectorModel.annotations[0].rotateAngle;
        ...
        if (selectorModel.formFields.length) {
          for (let i: number = 0; i < selectorModel.formFields.length; i++) {
            const node: PdfAnnotationBaseModel =
              selectorModel.formFields[parseInt(i.toString(), 10)];
            selectorElement = this.getSelectorElement(
              this.pdfViewer.element.id,
              select
            );
            if (
              selectorElement &&
              node.pageIndex === select &&
              this.pdfViewer.designerMode
            ) {
            ...
            if (
              ...
              node.formFieldAnnotationType
            ) {
              if (
                !this.pdfViewer.viewerBase.isFormFieldSelect &&
                !this.pdfViewer.viewerBase.isFormFieldMouseDown &&
                ...
                const field: IFormField = {
                  ...
                  backgroundColor: (node as PdfFormFieldBaseModel)
                    .backgroundColor,
                  alignment: (node as PdfFormFieldBaseModel).alignment,
                  isReadonly: (node as any).isReadonly,
                  visibility: (node as any).visibility,
                  maxLength: (node as any).maxLength,
                  isRequired: (node as any).isRequired,
                  isPrint: node.isPrint,
                  rotation: (node as any).rotateAngle,
                  tooltip: (node as any).tooltip,
                  options: (node as any).options,
                  isChecked: (node as any).isChecked,
                  isSelected: (node as any).isSelected,
                };
                if (!this.pdfViewer.formDesignerModule.isFormFieldSizeUpdated) {
                  this.pdfViewer.fireFormFieldSelectEvent(
                    'formFieldSelect',
                    field,
                    node.pageIndex,
                    this.pdfViewer.formDesignerModule.isProgrammaticSelection
                  );
                }
                this.pdfViewer.formDesignerModule.isFormFieldSizeUpdated =
                  false;
              }
            }
          }
        ...
        if (selectorModel.annotations.length) {
          for (let j: number = 0; j < selectorModel.annotations.length; j++) {
            const node: PdfAnnotationBaseModel =
              selectorModel.annotations[parseInt(j.toString(), 10)];
            selectorElement = this.getSelectorElement(
              this.pdfViewer.element.id,
              select
            );
            if (selectorElement && node.pageIndex === select) {
              if (
                node.shapeAnnotationType === 'Distance' ||
                node.shapeAnnotationType === 'Line' ||
                node.shapeAnnotationType === 'LineWidthArrowHead' ||
                node.shapeAnnotationType === 'Polygon'
              ) {
                this.renderEndPointHandle(
                  node,
                  selectorElement,
                  selectorModel.thumbsConstraints,
                  { scale: zoom, tx: 0, ty: 0 },
                  undefined,
                  undefined,
                  true,
                  currentSelector
                ...
                if (node.shapeAnnotationType === 'StickyNotes') {
                  ...
                  if (this.pdfViewer.tool !== 'Stamp') {
                    const isSignature: any =
                      node.shapeAnnotationType === 'Path' ||
                      node.formFieldAnnotationType === 'SignatureField' ||
                      node.formFieldAnnotationType === 'InitialField';
                    this.renderResizeHandle(
                      node.wrapper.children[0],
                      selectorElement,
                      selectorModel.thumbsConstraints,
                      ...
                      undefined,
                      undefined,
                      node.shapeAnnotationType === 'Stamp',
                      false,
                      isSignature,
                      node.shapeAnnotationType === 'FreeText' ||
                        node.shapeAnnotationType === 'HandWrittenSignature' ||
                        node.shapeAnnotationType === 'SignatureImage' ||
                        node.shapeAnnotationType === 'Image' ||
                        node.shapeAnnotationType === 'SignatureText',
                      currentSelector
                    );
                  }
                }
              ...
              if (
                !this.pdfViewer.viewerBase.isNewSignatureAdded &&
                (node.shapeAnnotationType === 'HandWrittenSignature' ||
                  node.shapeAnnotationType === 'SignatureText' ||
                  node.shapeAnnotationType === 'SignatureImage')
              ) {
                this.pdfViewer.annotationModule.selectSignature(
                  node.signatureName,
                  node.pageIndex,
                  node
                );
              ...
              if (
                (node.annotName !== '' || node.signatureName === 'ink') &&
                node.annotName !== 'SignatureText'
              ) {
                if (
                  helper &&
                  node === helper &&
                  ...
                  if (
                    !this.pdfViewer.viewerBase.isAddComment &&
                    !this.pdfViewer.viewerBase.isAnnotationSelect &&
                    !this.pdfViewer.viewerBase.isAnnotationMouseDown &&
                    !this.pdfViewer.viewerBase.isAnnotationMouseMove &&
                    !this.pdfViewer.viewerBase.isInkAdded &&
                    !this.pdfViewer.viewerBase.isNewStamp
                  ) {
                    this.pdfViewer.viewerBase.isAnnotationSelect = true;
                    this.pdfViewer.annotationModule.annotationSelect(
                      node.annotName,
                      node.pageIndex,
                      node
                    );
   ...
   */
  public rotate(
    obj: PdfAnnotationBaseModel | SelectorModel,
    angle: number,
    pivot?: PointModel,
    currentSelector?: AnnotationSelectorSettingsModel
  ): boolean {
  ...
  public rotateObjects(
    parent: PdfAnnotationBaseModel | SelectorModel,
  ...
  private shownBorder(): boolean {
    ...
    if (this.pdfViewer.annotationModule) {
      ...
      const allowedInteraction: any =
        this.pdfViewer.annotationModule.updateAnnotationAllowedInteractions(
          annotation
        );
      const isLock: boolean =
        this.pdfViewer.annotationModule.checkIsLockSettings(annotation);
      isSelectInAllowed =
        !isNullOrUndefined(allowedInteraction) &&
        (allowedInteraction.includes('Select') || !isLock);
    }
  ...
  public renderBorder(
    ...
    if (!isSticky) {
      ...
      const shapeType: PdfAnnotationBaseModel = this.pdfViewer.selectedItems
        .annotations[0].shapeAnnotationType as PdfAnnotationBaseModel;
      if (
        currentSelector &&
        typeof currentSelector !== 'object' &&
        currentSelector !== ''
      ) {
        const annotationSelector: any = JSON.parse(currentSelector);
        const borderColor: string =
          annotationSelector.selectionBorderColor === ''
            ? 'black'
            : annotationSelector.selectionBorderColor;
        options.stroke = borderColor;
        options.strokeWidth =
          currentSelector.selectionBorderThickness === 1
            ? 1
            : annotationSelector.selectionBorderThickness;
        let lineDash: number[] =
          annotationSelector.selectorLineDashArray.length === 0
            ? [6, 3]
        ...
        const borderColor: string =
          currentSelector.selectionBorderColor === ''
            ? 'black'
            : currentSelector.selectionBorderColor;
        options.stroke = borderColor;
        options.strokeWidth =
          currentSelector.selectionBorderThickness === 1
            ? 1
            : currentSelector.selectionBorderThickness;
        let lineDash: number[] =
          !isNullOrUndefined(currentSelector.selectorLineDashArray) &&
          currentSelector.selectorLineDashArray.length === 0
            ? [6, 3]
            : currentSelector.selectorLineDashArray;
        ...
        if (!this.pdfViewer.designerMode) {
          if (
            ...
            shapeType === 'SignatureImage' ||
            shapeType === 'Ink'
          ) {
            const formField: any = (this.pdfViewer.nameTable as any)[
              selector.id.split('_')[0]
            ];
            const isFormFieldSign: boolean =
              this.pdfViewer.viewerBase.checkSignatureFormField(selector.id);
      ...
      const shapeType: PdfAnnotationBaseModel | PdfFormFieldBaseModel =
        this.pdfViewer.selectedItems.annotations.length > 0
          ? (this.pdfViewer.selectedItems.annotations[0]
              .shapeAnnotationType as PdfAnnotationBaseModel)
          : (this.pdfViewer.selectedItems.formFields[0]
              .formFieldAnnotationType as PdfFormFieldBaseModel);
      if (
        currentSelector &&
        typeof currentSelector !== 'object' &&
        currentSelector !== ''
      ) {
        const annotationSelector: AnnotationSelectorSettingsModel =
          JSON.parse(currentSelector);
        const borderColor: string =
          annotationSelector.selectionBorderColor === ''
            ? 'black'
            : annotationSelector.selectionBorderColor;
    ...
    if (checkBorder) {
      const parentSvg: SVGSVGElement = this.getParentSvg(
        selector,
        'selector'
      ) as SVGSVGElement;

      this.svgRenderer.drawRectangle(
        canvas as SVGElement,
        options as RectAttributes,
        this.pdfViewer.element.id,
        undefined,
        true,
        parentSvg
   ...
   * @returns {void}
   */
  public getSignBorder(
    type: any,
    options: BaseAttributes,
    isFormFieldSign?: boolean
  ): void {
    if (
      !isFormFieldSign &&
      ...
      const borderColor: string =
        ...
        this.pdfViewer.handWrittenSignatureSettings.annotationSelectorSettings
          ...
          : this.pdfViewer.handWrittenSignatureSettings
              .annotationSelectorSettings.selectionBorderColor;
      options.stroke = borderColor;
      const thickness: number = isNullOrUndefined(
        this.pdfViewer.handWrittenSignatureSettings.annotationSelectorSettings
      ...
      let lineDash: number[] =
        ...
        this.pdfViewer.handWrittenSignatureSettings.annotationSelectorSettings
          .selectorLineDashArray.length === 0
          ? [4]
          : this.pdfViewer.handWrittenSignatureSettings
      ...
      options.stroke = borderColor;
      options.strokeWidth =
        annotationSelector.selectionBorderThickness === 1
          ? 1
          : annotationSelector.selectionBorderThickness;
      let lineDash: number[] =
        annotationSelector.selectorLineDashArray.length === 0
   ...
   * @returns {void}
   */
  public getBorderSelector(
    type: PdfAnnotationBaseModel,
    options: BaseAttributes
  ): void {
    const annotationSelector: AnnotationSelectorSettingsModel =
      this.pdfViewer.annotationSelectorSettings;
    const borderColor: string =
      isNullOrUndefined(annotationSelector.selectionBorderColor) ||
      annotationSelector.selectionBorderColor === ''
        ? 'black'
        : annotationSelector.selectionBorderColor;
    options.stroke = borderColor;
    options.strokeWidth =
      isNullOrUndefined(annotationSelector.selectionBorderThickness) ||
      annotationSelector.selectionBorderThickness === 1
        ? 1
        : annotationSelector.selectionBorderThickness;
    let lineDash: number[] =
      isNullOrUndefined(annotationSelector.selectorLineDashArray) ||
      annotationSelector.selectorLineDashArray.length === 0
        ? [6, 3]
        : annotationSelector.selectorLineDashArray;
    if (lineDash.length > 2) {
    ...
    if (
      ...
      const borderColor: string =
        isNullOrUndefined(
          this.pdfViewer.annotationSelectorSettings.selectionBorderColor
        ) ||
        this.pdfViewer.annotationSelectorSettings.selectionBorderColor === ''
          ? 'black'
          : this.pdfViewer.annotationSelectorSettings.selectionBorderColor
      ...
      const thickness: number = isNullOrUndefined(
        this.pdfViewer.annotationSelectorSettings.selectionBorderThickness
      )
        ? 1
        : this.pdfViewer.annotationSelectorSettings.selectionBorderThickness
        ? this.pdfViewer.annotationSelectorSettings.selectionBorderThickness
      ...
      let lineDash: number[] =
        isNullOrUndefined(
          this.pdfViewer.annotationSelectorSettings.selectorLineDashArray
        ) ||
        this.pdfViewer.annotationSelectorSettings.selectorLineDashArray
          .length === 0
          ? [4]
      ...
      type === 'StickyNotes' &&
      this.pdfViewer.stickyNotesSettings.annotationSelectorSettings
    ) {
      const borderColor: string =
        isNullOrUndefined(
          this.pdfViewer.annotationSelectorSettings.selectionBorderColor
  ...
  public renderCircularHandle(
    ...
    if (wrapper.rotateAngle !== 0 || wrapper.parentTransform !== 0) {
      ...
      rotateMatrix(
        matrix,
        wrapper.rotateAngle + wrapper.parentTransform,
        wrapper.offsetX,
        wrapper.offsetY
      );
      newPoint = transformPointByMatrix(matrix, newPoint);
    }
    const options: CircleAttributes = getBaseShapeAttributes(
      wrapper
    ) as CircleAttributes;
    ...
    if (
      ...
      shapeType = this.pdfViewer.selectedItems.annotations[0]
        .measureType as PdfAnnotationBaseModel;
    } else if (this.pdfViewer.selectedItems.formFields.length > 0) {
      shapeType = this.pdfViewer.selectedItems.formFields[0]
        .formFieldAnnotationType as PdfFormFieldBaseModel;
    } else {
      shapeType = this.pdfViewer.selectedItems.annotations[0]
        .shapeAnnotationType as PdfAnnotationBaseModel;
    ...
    if (this.getShape(shapeType, currentSelector) === 'Square') {
      ...
      this.svgRenderer.drawCircle(
        canvas as SVGElement,
        options as CircleAttributes,
        1
      );
    }
  }
  ...
  public getShapeSize(
    ...
    if (
      ...
      const annotationSelector: any = JSON.parse(currentSelector);
      options.radius =
        (isNullOrUndefined(annotationSelector.resizerSize) ||
        annotationSelector.resizerSize === 8
          ? 8
          : annotationSelector.resizerSize) / 2;
      options.width =
        (isNullOrUndefined(annotationSelector.resizerSize) ||
        annotationSelector.resizerSize === 8
          ? 8
          : annotationSelector.resizerSize) * t.scale;
      options.height =
        (isNullOrUndefined(annotationSelector.resizerSize) ||
        annotationSelector.resizerSize === 8
          ? 8
      ...
      options.radius =
        (isNullOrUndefined(currentSelector.resizerSize) ||
        currentSelector.resizerSize === 8
          ? 8
          : currentSelector.resizerSize) / 2;
      options.width =
        (isNullOrUndefined(currentSelector.resizerSize) ||
        currentSelector.resizerSize === 8
          ? 8
          : currentSelector.resizerSize) * t.scale;
      options.height =
        (isNullOrUndefined(currentSelector.resizerSize) ||
        currentSelector.resizerSize === 8
          ? 8
      ...
      if (
        ...
        options.radius =
          isNullOrUndefined(
            this.pdfViewer.annotationSelectorSettings.resizerSize
          ) || this.pdfViewer.annotationSelectorSettings.resizerSize !== 8
            ? this.pdfViewer.annotationSelectorSettings.resizerSize
            : !isNullOrUndefined(
                this.pdfViewer.lineSettings.annotationSelectorSettings
              ) &&
        ...
        options.radius =
          isNullOrUndefined(
            ...
            ? this.pdfViewer.handWrittenSignatureSettings
                .annotationSelectorSettings.resizerSize / 2
            : 8;
        // eslint-disable-next-line max-len
        options.width =
          isNullOrUndefined(
            ...
            ? this.pdfViewer.handWrittenSignatureSettings
                .annotationSelectorSettings.resizerSize * t.scale
            : 8;
        // eslint-disable-next-line max-len
        options.height =
          isNullOrUndefined(
  ...
  public getShape(
    ...
    let shapeType: any;
    {
      if (
        ...
        const annotationSelector: any = JSON.parse(currentSelector);
        shapeType =
          isNullOrUndefined(annotationSelector.resizerShape) ||
          annotationSelector.resizerShape === 'Square'
            ? 'Square'
            : annotationSelector.resizerShape;
      } else if (currentSelector && currentSelector !== '') {
        shapeType =
          isNullOrUndefined(currentSelector.resizerShape) ||
          currentSelector.resizerShape === 'Square'
            ? 'Square'
            : currentSelector.resizerShape;
      } else {
        const annotationSelector: AnnotationSelectorSettingsModel =
          this.pdfViewer.annotationSelectorSettings;
        shapeType =
        ...
        if (
          ...
          shapeType =
            isNullOrUndefined(
              this.pdfViewer.annotationSelectorSettings.resizerShape
            ) ||
            this.pdfViewer.annotationSelectorSettings.resizerShape !== 'Square'
              ? this.pdfViewer.annotationSelectorSettings.resizerShape
              : !isNullOrUndefined(
                  this.pdfViewer.lineSettings.annotationSelectorSettings
                ) &&
          ...
          shapeType =
            ...
            this.pdfViewer.annotationSelectorSettings.resizerShape !== 'Square'
              ...
              : !isNullOrUndefined(
                ...
                !isNullOrUndefined(
                  this.pdfViewer.handWrittenSignatureSettings
                    .annotationSelectorSettings.resizerShape
                )
              ? this.pdfViewer.handWrittenSignatureSettings
                  .annotationSelectorSettings.resizerShape
              : 'Square';
          ...
          shapeType =
            ...
            this.pdfViewer.annotationSelectorSettings.resizerShape !== 'Square'
              ? this.pdfViewer.annotationSelectorSettings.resizerShape
              : !isNullOrUndefined(
                  this.pdfViewer.inkAnnotationSettings
                    .annotationSelectorSettings
                ) &&
                !isNullOrUndefined(
                  this.pdfViewer.inkAnnotationSettings
  ...
  public getResizerColors(
    ...
    if (
      ...
      const annotationSelector: any = JSON.parse(currentSelector);
      options.stroke =
        isNullOrUndefined(annotationSelector.resizerBorderColor) ||
        annotationSelector.resizerBorderColor === 'black'
          ? 'black'
          : annotationSelector.resizerBorderColor;
      options.fill =
        isNullOrUndefined(annotationSelector.resizerFillColor) ||
        annotationSelector.resizerFillColor === '#FF4081'
          ? '#FF4081'
          : annotationSelector.resizerFillColor;
    } else if (currentSelector && currentSelector !== '') {
      options.stroke = !isNullOrUndefined(currentSelector.resizerBorderColor)
        ? currentSelector.resizerBorderColor
        : 'black';
      options.fill = !isNullOrUndefined(currentSelector.resizerFillColor)
        ? currentSelector.resizerFillColor
        : '#FF4081';
    } else {
      const annotationSelector: AnnotationSelectorSettingsModel =
        this.pdfViewer.annotationSelectorSettings;
      ...
      if (
        ...
        options.stroke =
          isNullOrUndefined(
            this.pdfViewer.annotationSelectorSettings.resizerBorderColor
          ) ||
          this.pdfViewer.annotationSelectorSettings.resizerBorderColor !==
            'black'
            ? this.pdfViewer.annotationSelectorSettings.resizerBorderColor
        ...
        options.fill =
          isNullOrUndefined(
            this.pdfViewer.annotationSelectorSettings.resizerFillColor
          ) ||
          this.pdfViewer.annotationSelectorSettings.resizerFillColor !==
            '#FF4081'
            ? this.pdfViewer.annotationSelectorSettings.resizerFillColor
        ...
        options.stroke =
          ...
          this.pdfViewer.annotationSelectorSettings.resizerBorderColor !==
            ...
            : !isNullOrUndefined(
              ...
              !isNullOrUndefined(
                this.pdfViewer.handWrittenSignatureSettings
                  .annotationSelectorSettings.resizerBorderColor
              )
            ? this.pdfViewer.handWrittenSignatureSettings
                .annotationSelectorSettings.resizerBorderColor
            : 'black';
        options.fill =
          ...
          this.pdfViewer.annotationSelectorSettings.resizerFillColor !==
            ...
            : !isNullOrUndefined(
              ...
              !isNullOrUndefined(
                this.pdfViewer.handWrittenSignatureSettings
                  .annotationSelectorSettings.resizerFillColor
              )
            ? this.pdfViewer.handWrittenSignatureSettings
                .annotationSelectorSettings.resizerFillColor
            : 'FF4081';
  ...
  public renderRotateThumb(
    ...
    const top: number =
      wrapper.offsetY - wrapper.actualSize.height * wrapper.pivot.y;
    const left: number =
      wrapper.offsetX - wrapper.actualSize.width * wrapper.pivot.x;
    ...
    if (wrapper.rotateAngle !== 0 || wrapper.parentTransform !== 0) {
      ...
      rotateMatrix(
        matrix,
        wrapper.rotateAngle + wrapper.parentTransform,
        (transform.tx + wrapper.offsetX) * transform.scale,
        (transform.ty + wrapper.offsetY) * transform.scale
      );
      newPoint = transformPointByMatrix(matrix, newPoint);
    }
    const options: CircleAttributes = getBaseShapeAttributes(
  ...
  public renderResizeHandle(
    ...
    const left: number =
      element.offsetX - element.actualSize.width * element.pivot.x;
    const top: number =
      element.offsetY - element.actualSize.height * element.pivot.y;
    ...
    if (!this.pdfViewer.formDesignerModule) {
      ...
      if (
        ...
        this.getAllowedInteractions(allowedInteraction)
      ) {
    ...
    let resizerLocation: AnnotationResizerLocation = this.getResizerLocation(
      shapeType,
      currentSelector
    );
    ...
    if (
      !this.pdfViewer.viewerBase.checkSignatureFormField(element.id) &&
      !nodeConstraints &&
      !isSticky &&
      !isPath &&
      !allowPermission
      ...
      if (
        ...
        this.renderCircularHandle(
          ...
          canvas,
          true,
          constraints & ThumbsConstraints.ResizeNorthWest,
          transform,
          undefined,
          canMask,
          {
            'aria-label':
              'Thumb to resize the selected object on top left side direction',
          },
          undefined,
          'e-pv-diagram-resize-handle e-northwest',
          currentSelector
        );
        this.renderCircularHandle(
          'resizeNorthEast',
          ...
          canvas,
          true,
          constraints & ThumbsConstraints.ResizeNorthEast,
          transform,
          undefined,
          canMask,
          {
            'aria-label':
              'Thumb to resize the selected object on top right side direction',
          },
          undefined,
          'e-pv-diagram-resize-handle e-northeast',
          currentSelector
        );
        this.renderCircularHandle(
          'resizeSouthWest',
          ...
          canvas,
          true,
          constraints & ThumbsConstraints.ResizeSouthWest,
          transform,
          undefined,
          canMask,
          {
            'aria-label':
              'Thumb to resize the selected object on bottom left side direction',
          },
          undefined,
        ...
        this.renderCircularHandle(
          ...
          canvas,
          true,
          constraints & ThumbsConstraints.ResizeSouthEast,
          transform,
          undefined,
          canMask,
          {
            'aria-label':
              'Thumb to resize the selected object on bottom right side direction',
          },
          undefined,
      ...
      if (
        ...
        this.renderCircularHandle(
          ...
          canvas,
          true,
          constraints & ThumbsConstraints.ResizeNorth,
          transform,
          undefined,
          canMask,
          {
            'aria-label':
              'Thumb to resize the selected object on top side direction',
          },
          undefined,
        ...
        this.renderCircularHandle(
          ...
          canvas,
          true,
          constraints & ThumbsConstraints.ResizeSouth,
          transform,
          undefined,
          canMask,
          {
            'aria-label':
              'Thumb to resize the selected object on bottom side direction',
          },
          undefined,
        ...
        this.renderCircularHandle(
          ...
          canvas,
          true,
          constraints & ThumbsConstraints.ResizeWest,
          transform,
          undefined,
          canMask,
          {
            'aria-label':
              'Thumb to resize the selected object on left side direction',
          },
          undefined,
        ...
        this.renderCircularHandle(
          ...
          canvas,
          true,
          constraints & ThumbsConstraints.ResizeEast,
          transform,
          undefined,
          canMask,
          {
            'aria-label':
              'Thumb to resize the selected object on right side direction',
          },
          undefined,
    ...
    if (
      shapeType === 'Textbox' ||
      shapeType === 'Checkbox' ||
      shapeType === 'RadioButton' ||
      shapeType === 'SignatureField' ||
      shapeType === 'InitialField' ||
      shapeType === 'DropdownList' ||
      shapeType === 'ListBox' ||
      shapeType === 'PasswordField'
    ) {
      this.renderCircularHandle(
        'resizeNorth',
        element,
  ...
  private getAllowedInteractions(allowedInteraction: any): boolean {
    if (allowedInteraction && allowedInteraction.length > 0) {
      for (let i: number = 0; i < allowedInteraction.length; i++) {
        if (
          allowedInteraction[0] !== 'None' &&
          allowedInteraction[parseInt(i.toString(), 10)] === 'Resize'
        ) {
          return false;
        }
      }
  ...
  public getResizerLocation(
    ...
    let resizerLocation: any;
    {
      if (
        ...
        const annotationSelector: any = JSON.parse(currentSelector);
        resizerLocation =
          isNullOrUndefined(annotationSelector.resizerLocation) ||
          annotationSelector.resizerLocation === 3
            ? 3
            : annotationSelector.resizerLocation;
      } else if (currentSelector && currentSelector !== '') {
        resizerLocation =
          isNullOrUndefined(currentSelector.resizerLocation) ||
          currentSelector.resizerLocation === 3
            ? 3
            : currentSelector.resizerLocation;
      } else {
        const annotationSelector: AnnotationSelectorSettingsModel =
          this.pdfViewer.annotationSelectorSettings;
        resizerLocation =
        ...
        if (
          ...
          resizerLocation =
            ...
            this.pdfViewer.handWrittenSignatureSettings
              .annotationSelectorSettings.resizerLocation === 3
              ? 3
              : !isNullOrUndefined(
                  this.pdfViewer.handWrittenSignatureSettings
                    .annotationSelectorSettings
  ...
  public renderPivotLine(
    ...
    const checkBorder: boolean = this.shownBorder();
    if (checkBorder) {
      this.svgRenderer.drawLine(
        canvas as SVGElement,
        options as LineAttributes
      );
    }
  }
  ...
  public renderEndPointHandle(
    selector: PdfAnnotationBaseModel,
    canvas: HTMLCanvasElement | SVGElement,
    constraints: ThumbsConstraints,
    transform: Transforms,
    ...
    const wrapper: DrawingElement = selector.wrapper;
    let i: number;
    const checkBorder: boolean = this.shownBorder();
    if (checkBorder) {
      for (i = 0; i < selector.vertexPoints.length; i++) {
        const segment: PointModel =
          selector.vertexPoints[parseInt(i.toString(), 10)];
        this.renderCircularHandle(
          'segementThumb_' + (i + 1),
          wrapper,
          segment.x,
          ...
          canvas,
          true,
          constraints & ThumbsConstraints.ConnectorSource,
          transform,
          connectedSource,
          null,
          null,
    ...
    if (selector.shapeAnnotationType === 'Distance') {
      for (i = 0; i < selector.wrapper.children.length; i++) {
        const segment: DrawingElement =
          selector.wrapper.children[parseInt(i.toString(), 10)];
        let newPoint1: PointModel;
        const angle: number = Point.findAngle(
          selector.sourcePoint,
          selector.targetPoint
        );
        if (segment.id.indexOf('leader') > -1) {
          ...
          if (checkBorder) {
            const rotatedPoint: PointModel = transformPointByMatrix(matrix, {
              x: newPoint1.x,
              y: newPoint1.y,
            });
            this.renderCircularHandle(
  ...
  public select(
    ...
    for (let i: number = 0; i < objArray.length; i++) {
      const obj: any = (this.pdfViewer.nameTable as any)[
        objArray[parseInt(i.toString(), 10)]
      ];
      if (obj.formFieldAnnotationType && this.pdfViewer.designerMode) {
        ...
        if (
          ...
          if (obj.annotationSettings) {
            if (
              !isNullOrUndefined(annotationSettings) &&
              !isNullOrUndefined(annotationSettings.isLock)
            ) {
              annotationSettings.isLock = JSON.parse(annotationSettings.isLock);
            }
          } else if (!obj.formFieldAnnotationType) {
            annotationSettings =
              this.pdfViewer.annotationModule.findAnnotationSettings(obj, true);
            obj.annotationSettings = annotationSettings;
          }
          let isLock: boolean = !obj.formFieldAnnotationType
            ? annotationSettings
              ? annotationSettings.isLock
              : false
            : false;
          if (
            annotationSettings &&
  ...
  public dragSelectedObjects(
    ...
    const obj: SelectorModel | PdfAnnotationBaseModel =
  ...
  public dragAnnotation(
    ...
    this.nodePropertyChange(obj, {
      bounds: { x: obj.wrapper.offsetX, y: obj.wrapper.offsetY },
    } as PdfAnnotationBaseModel);
    obj.wrapper.measureChildren = false;
    const canvas: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_annotationCanvas_' + (obj as any).pageIndex
    );
    this.pdfViewer.renderDrawing(
      canvas as HTMLCanvasElement,
      (obj as any).pageIndex
    );
  }
  ...
  public dragControlPoint(
    ...
    const connector: PdfAnnotationBaseModel = (this.pdfViewer.nameTable as any)[
      obj.id
    ];
  ...
  public nodePropertyChange(
    actualObject: PdfAnnotationBaseModel,
    node: PdfAnnotationBaseModel
  ): void {
    ...
    if (node.bounds) {
      if (node.bounds.width !== undefined) {
        actualObject.bounds.width = actualObject.wrapper.width =
          node.bounds.width;
      }
      if (node.bounds.height !== undefined) {
        actualObject.bounds.height = actualObject.wrapper.height =
          node.bounds.height;
      }
      ...
      if (node.bounds.y !== undefined) {
        actualObject.bounds.y =
          node.bounds.y - actualObject.bounds.height * 0.5;
      ...
      if (actualObject.wrapper.children.length) {
        ...
        for (let i: number = 0; i < children.length; i++) {
          if (children[parseInt(i.toString(), 10)].id) {
            const names: string[] =
              children[parseInt(i.toString(), 10)].id.split('_');
            if (
              names.length &&
              (names.indexOf('perimeter') > -1 || names.indexOf('radius') > -1)
            ) {
              ...
              children[parseInt(i.toString(), 10)].offsetX =
                actualObject.vertexPoints[0].x;
              children[parseInt(i.toString(), 10)].offsetY =
                actualObject.vertexPoints[0].y;
            } else if (names.length && names.indexOf('tarDec') > -1) {
              children[parseInt(i.toString(), 10)].offsetX =
                actualObject.vertexPoints[
                  actualObject.vertexPoints.length - 1
                ].x;
              children[parseInt(i.toString(), 10)].offsetY =
                actualObject.vertexPoints[
                  actualObject.vertexPoints.length - 1
              ...
              if (actualObject.isDynamicStamp) {
                children[parseInt(i.toString(), 10)].width =
                  actualObject.bounds.width - ratio;
                children[parseInt(i.toString(), 10)].height =
                  actualObject.bounds.height / 2 - ratio;
                const element: any = children[1] as TextElement;
                const annotationSettings: any = this.pdfViewer.stampSettings
                  ? this.pdfViewer.stampSettings
                  : this.pdfViewer.annotationSettings;
                if (
                  annotationSettings &&
                  (annotationSettings.maxHeight ||
                    annotationSettings.maxWidth) &&
                  actualObject.bounds.height > 60
                ) {
                  if (ratio !== 0) {
                    element.style.fontSize = actualObject.bounds.width / ratio;
                  } else {
                    element.style.fontSize =
                      actualObject.wrapper.bounds.width / 20;
                  }
                } else {
                  if (ratio !== 0) {
                    element.style.fontSize = this.fontSizeCalculation(
                      actualObject,
                      element,
                      actualObject.bounds.width - 20
                    );
                  } else {
                    element.style.fontSize = this.fontSizeCalculation(
                      actualObject,
                ...
                if (ratio !== 0) {
                  element.margin.bottom = -(
                    children[parseInt(i.toString(), 10)].height / 2
                  );
                }
              } else {
                children[parseInt(i.toString(), 10)].width =
                  actualObject.bounds.width - ratio;
                children[parseInt(i.toString(), 10)].height =
                  actualObject.bounds.height - ratio;
              }
              children[parseInt(i.toString(), 10)].offsetX =
    ...
    if (
      node.isReadonly !== undefined &&
      actualObject.shapeAnnotationType === 'FreeText'
    ) {
    ...
    if (node.fillColor !== undefined) {
      ...
      if (
        (actualObject.enableShapeLabel || actualObject.measureType) &&
        actualObject.wrapper &&
        actualObject.wrapper.children
      ) {
      ...
      if (
        ...
        for (let i: number = 0; i < children.length; i++) {
          if (children[parseInt(i.toString(), 10)].textNodes) {
            ...
            children[parseInt(i.toString(), 10)].style.fill =
              node.labelFillColor;
          }
        }
      }
    ...
    if (node.opacity !== undefined) {
      if (
        actualObject.shapeAnnotationType === 'Stamp' ||
        actualObject.shapeAnnotationType === 'FreeText'
      ) {
        ...
        if (actualObject.shapeAnnotationType === 'StickyNotes') {
          (this.pdfViewer.nameTable as any)[
            actualObject.annotName
          ].wrapper.children[0].style.opacity = node.opacity;
        }
      ...
      if (
        ...
        for (let i: number = 0; i < children.length; i++) {
          if (children[parseInt(i.toString(), 10)].textNodes) {
            children[parseInt(i.toString(), 10)].style.opacity =
              node.labelOpacity;
          }
        }
      }
    ...
    if (node.strokeColor !== undefined) {
      ...
      update = true;
      if (
        actualObject.shapeAnnotationType === 'Radius' &&
        actualObject.wrapper.children[1]
      ) {
    ...
    if (node.fontFamily !== undefined) {
      ...
      if (
        ...
        for (let i: number = 0; i < children.length; i++) {
          if (children[parseInt(i.toString(), 10)].textNodes) {
            children[parseInt(i.toString(), 10)].style.fontFamily =
              node.fontFamily;
          }
        }
      }
    ...
    if (node.fontSize !== undefined) {
      if (
        (actualObject.shapeAnnotationType === 'FreeText' ||
          actualObject.shapeAnnotationType === 'SignatureText') &&
        actualObject.wrapper &&
        actualObject.wrapper.children &&
        actualObject.wrapper.children.length
      ) {
        ...
        if (actualObject.shapeAnnotationType === 'SignatureText') {
          actualObject.wrapper.children[1].bounds.width =
            actualObject.bounds.width;
          actualObject.wrapper.children[1].desiredSize.width =
            actualObject.bounds.width;
          actualObject.wrapper.children[1].actualSize.width =
            actualObject.bounds.width;
          children[1].horizontalAlignment = 'Center';
      ...
      if (this.pdfViewer.enableToolbar && this.pdfViewer.toolbarModule) {
        this.pdfViewer.toolbarModule.annotationToolbarModule.updateFontSizeInIcon(
          node.fontSize
        );
      } else if (
        this.pdfViewer.annotationModule &&
    ...
    if (node.textAlign !== undefined) {
      ...
      if (
        ...
        if (children[1].childNodes.length === 1) {
          ...
          children[1].childNodes.length > 1 &&
          actualObject.textAlign === 'Justify'
        ) {
          children[1].horizontalAlignment = 'Center';
        } else {
          children[1].horizontalAlignment = 'Auto';
    ...
    if (node.borderDashArray !== undefined) {
      ...
      actualObject.wrapper.children[0].style.strokeDashArray =
    ...
    if (node.vertexPoints !== undefined) {
      ...
      (this.pdfViewer.nameTable as any)[actualObject.id].vertexPoints =
        node.vertexPoints;
      this.updateConnector(actualObject, node.vertexPoints);
    }
    if (
      node.leaderHeight !== undefined &&
      actualObject.shapeAnnotationType !== 'Polygon'
    ) {
      actualObject.leaderHeight = node.leaderHeight;
      this.updateConnector(actualObject, actualObject.vertexPoints);
    }
    ...
    if (actualObject.shapeAnnotationType === 'Distance') {
      for (i = 0; i < actualObject.wrapper.children.length; i++) {
        const segment: any =
          actualObject.wrapper.children[parseInt(i.toString(), 10)];
    ...
    if (isLineShapes(actualObject)) {
      for (let i: number = 0; i < actualObject.wrapper.children.length; i++) {
        ...
        if (actualObject.enableShapeLabel === true) {
          if (
            actualObject.wrapper.children[parseInt(i.toString(), 10)] instanceof
            TextElement
          ) {
            actualObject.wrapper.children[
              parseInt(i.toString(), 10)
            ].style.fill = actualObject.labelFillColor;
          }
          if (
            actualObject.wrapper.children[parseInt(i.toString(), 10)] instanceof
              PathElement &&
            actualObject.measureType === 'Perimeter'
          ) {
            actualObject.wrapper.children[
              parseInt(i.toString(), 10)
    ...
    if (
      actualObject &&
      (actualObject.shapeAnnotationType === 'FreeText' ||
        actualObject.enableShapeLabel === true)
    ) {
      if (
        actualObject.wrapper &&
        actualObject.wrapper.children &&
          ...
          if (children[parseInt(i.toString(), 10)].textNodes) {
            if (actualObject.shapeAnnotationType === 'FreeText') {
              if (node.dynamicText) {
                ...
                children[parseInt(i.toString(), 10)].content =
                  actualObject.dynamicText;
              }
              children[parseInt(i.toString(), 10)].width =
                actualObject.bounds.width;
            } else if (
              actualObject.enableShapeLabel === true &&
              actualObject.measureType
            ) {
    ...
    if (
      ...
      if (
        actualObject.wrapper.children &&
        actualObject.wrapper.children.length > 1
      ) {
    ...
    if (
      ...
      this.pdfViewer.annotationModule.stickyNotesAnnotationModule.textFromCommentPanel =
        false;
    } else {
      actualObject.wrapper.measure(
        new Size(
    ...
    if (
      ...
      if (
        ...
        for (let i: number = 0; i < children.length; i++) {
          if (
            children[parseInt(i.toString(), 10)].textNodes &&
            children[parseInt(i.toString(), 10)].textNodes.length > 0
          ) {
            children[parseInt(i.toString(), 10)].isDirt = true;
            let childNodeHeight: number =
              children[parseInt(i.toString(), 10)].textNodes.length *
              children[parseInt(i.toString(), 10)].textNodes[0].dy;
            const heightDiff: number =
              actualObject.bounds.height - childNodeHeight;
            if (
              heightDiff > 0 &&
              heightDiff < children[parseInt(i.toString(), 10)].textNodes[0].dy
            ) {
              childNodeHeight =
                childNodeHeight +
                children[parseInt(i.toString(), 10)].textNodes[0].dy;
            ...
            if (childNodeHeight > actualObject.bounds.height) {
              ...
              for (
                let index: number = 0;
                index < children[parseInt(i.toString(), 10)].textNodes.length;
                index++
              ) {
                contString =
                  contString +
  ...
  public scaleAnnotation(
    ...
    const constraints: boolean = this.checkBoundaryConstraints(
      undefined,
      undefined,
      (obj as PdfAnnotationBaseModel).pageIndex,
      obj.wrapper.bounds
    );
    if (!constraints) {
      ...
      //bug (EJ2-62649) : fixed an issue with difficulty on moving/ resizing free text annotation that added in edge of viewer
      if (
        (obj as any).shapeAnnotationType === 'FreeText' &&
        ((obj as any).id.slice(0, 9) === 'free_text' ||
          (obj as any).id.slice(0, 8) === 'freetext')
      ) {
  ...
  private moveInsideViewer(
    ...
    if (
      ...
      if (canvas) {
        ...
        const width: number =
          canvas.clientWidth / this.pdfViewer.viewerBase.getZoomFactor();
        const height: number =
          canvas.clientHeight / this.pdfViewer.viewerBase.getZoomFactor();
  ...
  public checkBoundaryConstraints(
    ...
    const selectorBounds: Rect = !nodeBounds
      ? this.pdfViewer.selectedItems.wrapper.bounds
      : undefined;
    ...
    if (canvas) {
      ...
      if (isStamp) {
        ...
        if (
          ...
          this.pdfViewer.viewerBase.eventArgs.source
        ) {
      ...
      if (
        right <= width - 3 ||
        (left >= 1 && bottom <= height - 3 && top >= heightDifference) ||
        isSkip
      ) {
        return true;
      }
  ...
  private RestrictStamp(source: any): boolean {
    if (
      ...
      this.pdfViewer.viewerBase.activeElements &&
  ...
  public getShapeBounds(shapeElement: DrawingElement): Rect {
    ...
    if (shapeElement.rotateAngle !== 0 || shapeElement.parentTransform !== 0) {
      ...
      rotateMatrix(
        matrix,
        shapeElement.rotateAngle + shapeElement.parentTransform,
        shapeElement.offsetX,
        shapeElement.offsetY
      );
      shapeElement.corners.topLeft = shapeTopLeft = transformPointByMatrix(
        matrix,
        shapeTopLeft
      );
      shapeElement.corners.topCenter = shapeTopCenter = transformPointByMatrix(
        matrix,
        shapeTopCenter
      );
      shapeElement.corners.topRight = shapeTopRight = transformPointByMatrix(
        matrix,
        shapeTopRight
      );
      shapeElement.corners.middleLeft = shapeMiddleLeft =
        transformPointByMatrix(matrix, shapeMiddleLeft);
      shapeElement.corners.middleRight = shapeMiddleRight =
        transformPointByMatrix(matrix, shapeMiddleRight);
      shapeElement.corners.bottomLeft = shapeBottomLeft =
        transformPointByMatrix(matrix, shapeBottomLeft);
      shapeElement.corners.bottomCenter = shapeBottomCenter =
        transformPointByMatrix(matrix, shapeBottomCenter);
      shapeElement.corners.bottomRight = shapeBottomRight =
        transformPointByMatrix(matrix, shapeBottomRight);
      //Set corners based on rotate angle
    }
    shapeBounds = Rect.toBounds([
      shapeTopLeft,
      shapeTopRight,
      shapeBottomLeft,
      shapeBottomRight,
      ...
      if (connector.shapeAnnotationType === 'Distance') {
        ...
        if (endPoint === 'Leader0') {
          ...
          if (endPoint.split('_')[0] === 'ConnectorSegmentPoint') {
            const matrix: Matrix = identityMatrix();
            rotateMatrix(matrix, -angle, center.x, center.y);
            const rotatedPoint1: PointModel = transformPointByMatrix(
              matrix,
              connector.vertexPoints[0]
            );
            const rotatedPoint2: PointModel = transformPointByMatrix(
              matrix,
              connector.vertexPoints[connector.vertexPoints.length - 1]
            );
            ty = rotatedPoint.y - rotatedPoint1.y;
            if (
              connector.leaderHeight === 0 &&
              connector.leaderHeight != null
            ) {
              connector.leaderHeight =
                this.pdfViewer.distanceSettings.leaderLength;
            } else {
              ...
              const matrix: Matrix = identityMatrix();
              rotateMatrix(matrix, angle, center.x, center.y);
              connector.vertexPoints[0] = transformPointByMatrix(
                matrix,
                rotatedPoint1
              );
              connector.vertexPoints[connector.vertexPoints.length - 1] =
                transformPointByMatrix(matrix, rotatedPoint2);
            }
          }
        }
        ...
        connector.vertexPoints[parseInt(i.toString(), 10)].y += ty;
        if (
          connector.vertexPoints.length > 2 &&
          (obj as PdfAnnotationBaseModel).measureType !== 'Perimeter'
        ) {
          if (parseFloat(endPoint.split('_')[1]) === 0) {
            ...
            parseFloat(endPoint.split('_')[1]) ===
            connector.vertexPoints.length - 1
          ) {
            connector.vertexPoints[0].x += tx;
            connector.vertexPoints[0].y += ty;
          }
  ...
  public updateConnector(
    connector: PdfAnnotationBaseModel,
    points: PointModel[]
  ): void {
    ...
    updateSegmentElement(
      connector,
      points,
      connector.wrapper.children[0] as PathElement
    );
      ...
      if (connector.shapeAnnotationType !== 'Polygon') {
        ...
        if (element.id.indexOf('tarDec') > -1) {
          updateDecoratorElement(
            ...
            element,
            points[points.length - 1],
            anglePoint[anglePoint.length - 2],
            false
          );
        }
      }
  ...
  public copy(): Object {
    ...
    if (!isNullOrUndefined(this.pdfViewer.annotationModule)) {
      annotationSettings =
        this.pdfViewer.annotationModule.findAnnotationSettings(
          this.pdfViewer.selectedItems.annotations[0]
        );
    }
    if (
      ((this.pdfViewer.formDesignerModule &&
        !this.pdfViewer.formDesigner.isPropertyDialogOpen) ||
        this.pdfViewer.annotationModule) &&
      (this.pdfViewer.designerMode || this.pdfViewer.enableAnnotation) &&
      ...
      this.copiedElementID = (
        this.pdfViewer.clipboardData.clipObject as any[]
      )[0].id;
    }
    let isSearchboxDialogOpen: boolean;
    const searchBoxId: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_search_box'
    );
    if (searchBoxId) {
  ...
  public copyObjects(): Object[] {
    ...
    if (this.pdfViewer.selectedItems.annotations.length > 0) {
      ...
      for (let j: number = 0; j < selectedItems.length; j++) {
        const element: Object = cloneObject(
          selectedItems[parseInt(j.toString(), 10)]
        );
        obj.push(element);
      }
    }
          ...
          if (!item.formFieldAnnotationType) {
            if ((item as PdfAnnotationBaseModel).annotName) {
              this.pdfViewer.annotationModule.deleteAnnotationById(
                (item as PdfAnnotationBaseModel).annotName
              );
            } else {
            ...
            this.pdfViewer.formDesignerModule.deleteFormField(
              (item as PdfFormFieldBaseModel).id
            );
          }
        }
      }
  ...
  public paste(obj: PdfAnnotationBaseModel[], index: number): void {
    ...
    if (obj || this.pdfViewer.clipboardData.clipObject) {
      const copiedItems: PdfAnnotationBaseModel[] = obj
        ...
        if (!isNullOrUndefined(copiedItems[0])) {
          if (
            ...
            for (let j: number = 0; j < copiedItems.length; j++) {
              const copy: PdfAnnotationBaseModel =
                copiedItems[parseInt(j.toString(), 10)];
              const pageDiv: HTMLElement = this.pdfViewer.viewerBase.getElement(
                '_pageDiv_' + copy.pageIndex
              );
              ...
              if (isLineShapes(copy)) {
                ...
                if (pageDiv) {
                  if (
                    pageCurrentRect.right - events.clientX <
                    copy.bounds.width * zoomfactor
                  ) {
                    copy.bounds.x =
                      (pageDiv.clientWidth - copy.bounds.width * zoomfactor) /
                      zoomfactor;
                  } else {
                    copy.bounds.x =
                      (events.clientX - pageCurrentRect.left) / zoomfactor;
                  }
                  if (
                    pageCurrentRect.bottom - events.clientY >
                    copy.bounds.height * zoomfactor
                  ) {
                    copy.bounds.y =
                      (events.clientY - pageCurrentRect.top) / zoomfactor;
                  } else {
                    copy.bounds.y =
                      (pageDiv.clientHeight - copy.bounds.height * zoomfactor) /
                      zoomfactor;
                  }
                }
              }
              if (this.isWithinBounds(pageCurrentRect, events)) {
                ...
                if (
                  this.pdfViewer.viewerBase.contextMenuModule.previousAction !==
                  'Cut'
                ) {
                  newNode.id += randomId();
                  if (
                    this.pdfViewer.annotationModule &&
                    newNode.shapeAnnotationType !== 'HandWrittenSignature'
                  ) {
                  ...
                  if (
                    newNode.shapeAnnotationType === 'SignatureText' ||
                    newNode.shapeAnnotationType === 'HandWrittenSignature' ||
                    newNode.shapeAnnotationType === 'SignatureImage'
                  ) {
                    this.pdfViewer.viewerBase.signatureModule.storeSignatureData(
                      newNode.pageIndex,
                      newNode
                    );
                  }
                  if (!newNode.formFieldAnnotationType) {
                    this.pdfViewer.annotation.addAction(
                      this.pdfViewer.viewerBase.getActivePage(false),
                      null,
                      newNode as PdfAnnotationBase,
                      'Addition',
                      '',
                      newNode as PdfAnnotationBase,
                      newNode
                ...
                const addedAnnot:
                  | PdfAnnotationBaseModel
                  | PdfFormFieldBaseModel = this.add(newNode);
                if (
                  this.pdfViewer.formDesigner &&
                  ...
                  this.pdfViewer.annotation.addAction(
                    this.pdfViewer.viewerBase.getActivePage(true),
                    null,
                    addedAnnot as PdfFormFieldBase,
                    'Addition',
                    '',
                    addedAnnot as PdfFormFieldBase,
                    addedAnnot as PdfFormFieldBase
                ...
                if (
                  (newNode.shapeAnnotationType === 'FreeText' ||
                    newNode.enableShapeLabel) &&
                  addedAnnot
                ) {
                ...
                if (
                  ...
                  const formField: FormFieldModel = {
                    ...
                    fontStyle: addedAnnot.fontStyle as unknown as FontStyle,
                    color: (addedAnnot as PdfFormFieldBaseModel).color,
                    backgroundColor: (addedAnnot as PdfFormFieldBaseModel)
                      .backgroundColor,
                    alignment: (addedAnnot as PdfFormFieldBaseModel)
                      .alignment as TextAlign,
                    visibility: (addedAnnot as PdfFormFieldBaseModel)
                      .visibility,
                    maxLength: (addedAnnot as PdfFormFieldBaseModel).maxLength,
                    isRequired: (addedAnnot as PdfFormFieldBaseModel)
                      .isRequired,
                    isPrint: addedAnnot.isPrint,
                    isSelected: (addedAnnot as PdfFormFieldBaseModel)
                      .isSelected,
                    ...
                    borderColor: (addedAnnot as PdfFormFieldBaseModel)
                      .borderColor,
                    signatureIndicatorSettings: (
                      addedAnnot as PdfFormFieldBaseModel
                    ).signatureIndicatorSettings,
                    insertSpaces: (addedAnnot as PdfFormFieldBaseModel)
                      .insertSpaces,
                    isMultiline: (addedAnnot as PdfFormFieldBaseModel)
                      .isMultiline,
                    isTransparent: (addedAnnot as PdfFormFieldBaseModel)
                      .isTransparent,
                    options: (addedAnnot as PdfFormFieldBaseModel).options,
                    pageIndex: addedAnnot.pageIndex,
                    pageNumber: (addedAnnot as PdfFormFieldBaseModel)
                      .pageNumber,
                    rotateAngle: (addedAnnot as PdfFormFieldBaseModel)
                      .rotateAngle,
                    signatureType: (addedAnnot as any).signatureType,
                    zIndex: (addedAnnot as PdfFormFieldBaseModel).zIndex,
                    selectedIndex: (addedAnnot as PdfFormFieldBaseModel)
                      .selectedIndex,
                  };
                  if (
                    (addedAnnot as PdfFormFieldBaseModel).options &&
                  ...
                  this.pdfViewer.formDesigner.drawHTMLContent(
                    addedAnnot.formFieldAnnotationType,
                ...
                this.pdfViewer.select(
                  [newNode.id],
                  this.pdfViewer.annotationSelectorSettings
                );
                if (!addedAnnot.formFieldAnnotationType) {
                  this.pdfViewer.annotationModule.triggerAnnotationAddEvent(
                    newNode
                  );
                }
              }
    ...
    for (let i: number = 0; i < copy.vertexPoints.length; i++) {
      if (pageDiv) {
        if (i === 0) {
          ...
          copy.vertexPoints[parseInt(i.toString(), 10)].x =
            (events.clientX - pageCurrentRect.left) / zoomfactor;
          copy.vertexPoints[parseInt(i.toString(), 10)].y =
            (events.clientY - pageCurrentRect.top) / zoomfactor;
