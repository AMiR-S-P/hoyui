...
export class FormFields {
  ...
  public renderFormFields(pageIndex: number, isImportFormField: boolean): void {
    ...
    if (this.data) {
      ...
      const textLayer: HTMLElement = document.getElementById(
        this.pdfViewer.element.id + '_textLayer_' + pageIndex
      );
      const canvasElement: HTMLElement = document.getElementById(
        this.pdfViewer.element.id + '_pageCanvas_' + pageIndex
      );
      let count: number;
      if (
        ...
        for (let i: number = 0; i < this.formFieldsData.length; i++) {
          const formField: any =
            this.formFieldsData[parseInt(i.toString(), 10)];
          if (
            !flag &&
            isNullOrUndefined(formField.ActualFieldName) &&
            formField.PageIndex === pageIndex
          ) {
            count = parseInt(
              formField.FieldName.slice(
                formField.FieldName.lastIndexOf('_') + 1
              ),
              10
            );
            flag = true;
            ...
            if (this.pdfViewer.formDesigner) {
              if (parseFloat(currentData['PageIndex']) === pageIndex) {
                ...
                const textAlignment: string =
                  currentData.Alignment === 2
                    ? 'right'
                    : currentData.Alignment === 1
                    ? 'center'
                    : 'left';
                ...
                if (currentData['Rotation'] !== 0) {
                  if (
                    currentData['RotationAngle'] === -90 ||
                    currentData['RotationAngle'] === -270 ||
                    currentData['RotationAngle'] === -180
                  ) {
                    boundArray = this.getBounds(
                      boundArray,
                      pageIndex,
                      0,
                      isFieldRotated
                ...
                if (
                  currentData.Name === 'RadioButton' ||
                  currentData.Name === 'CheckBox'
                ) {
                ...
                const fieldProperties: any = {
                  ...
                  font: {
                    ...
                    isStrikeout: !isNullOrUndefined(font)
                      ? font.Strikeout
                      : false,
                    isUnderline: !isNullOrUndefined(font)
                      ? font.Underline
                      : false,
                  },
                ...
                if (
                  ...
                  !isNullOrUndefined(currentData['ActualFieldName'])
                ) {
                  fieldProperties.id =
                    this.pdfViewer.formFieldCollections[
                      parseInt(i.toString(), 10)
                ...
                if (
                  currentData.Name === 'DropDown' ||
                  currentData.Name === 'ListBox'
                ) {
                  fieldProperties.value = currentData['SelectedValue'];
                }
                const fieldType: any = this.getFormFieldType(currentData);
                if (
                  currentData.Name !== 'SignatureText' ||
                  currentData.Name !== 'SignatureImage'
                ) {
                  if (!isNullOrUndefined(this.getFormFieldType(currentData))) {
                    ...
                    const addedElement1: any =
                      this.pdfViewer.formDesignerModule.addFormField(
                        fieldType,
                        fieldProperties,
                        false,
                        fieldProperties.id
                    ...
                    if (addedElement1 && addedElement1.parentElement) {
                      currentData.id =
                        addedElement1.parentElement.id.split('_')[0];
                    }
                    if (
                      addedElement1 &&
                      addedElement1.style.visibility === 'hidden'
                    ) {
                ...
                if (
                  ...
                  fieldType === 'InitialField'
                ) {
                  this.addSignaturePath(currentData, count);
                  if (
                    !isNullOrUndefined(currentData.Value) &&
                    currentData.Value !== ''
                  ) {
                    this.renderExistingAnnnot(
                      currentData,
                      parseFloat(currentData['PageIndex']) + 1,
                      null,
                      isFieldRotated
                    );
                ...
                if (
                  currentData.ActualFieldName == null &&
                  !isNullOrUndefined(currentData.FieldName) &&
                  this.formFieldsData
                    .filter(
                      (item: any) =>
                        !isNullOrUndefined(item.FieldName) &&
                        item.FieldName.includes(
                          currentData.FieldName.replace(/_\d$/, '')
                        )
                    )
                    .filter((value: any) => value.Name !== 'ink').length === 0
                ) {
                  this.renderExistingAnnnot(
                    currentData,
                    parseFloat(currentData['PageIndex']) + 1,
                  ...
                  this.pdfViewerBase.signatureModule.storeSignatureData(
                    pageIndex,
                    currentData
                  );
                  this.isSignatureRendered = true;
              ...
              if (parseFloat(currentData['PageIndex']) === pageIndex) {
                const field: any = this.createFormFields(
                  currentData,
                  pageIndex,
                  i,
                  null,
                ...
                if (inputField) {
                  const divElement: any = this.createParentElement(
                    currentData,
                    pageIndex
                  );
                  const bounds: any = currentData['LineBounds'];
                  ...
                  if (inputField.InsertSpaces) {
                    ...
                    const font: number =
                      parseInt(inputField.style.width, 10) /
                        inputField.maxLength -
                      parseFloat(inputField.style.fontSize) / 2 -
                      0.6 * zoomFactor;
                  ...
                  currentData['uniqueID'] =
                    this.pdfViewer.element.id + 'input_' + pageIndex + '_' + i;
                  for (
                    let j: number = 0;
                    j < this.pdfViewer.formFieldCollections.length;
                    j++
                  ) {
                    if (
                      ...
                      currentData.Name !== 'SignatureField'
                    ) {
                      if (
                        currentData['uniqueID'] ===
                        this.pdfViewer.formFieldCollections[
                  ...
                  inputField.addEventListener(
                    'focus',
                    this.focusFormFields.bind(this)
                  );
                  inputField.addEventListener(
                    'blur',
                    this.blurFormFields.bind(this)
                  );
                  inputField.addEventListener(
                    'click',
                    this.updateFormFields.bind(this)
                  );
                  inputField.addEventListener(
                    'change',
                    this.changeFormFields.bind(this)
                  );
                  inputField.addEventListener(
                    'keydown',
                    this.updateFormFieldsValue.bind(this)
                  );
                  inputField.addEventListener(
                    'keyup',
                    this.updateSameFieldsValue.bind(this)
                  );
                  count = signCount;
                }
              }
        ...
        if (!this.pdfViewer.formDesigner) {
          window.sessionStorage.removeItem(
            this.pdfViewerBase.documentId + '_formfields'
          );
          this.pdfViewerBase.setItemInSessionStorage(
            this.formFieldsData,
            '_formfields'
          );
        }
    ...
    if (this.pdfViewerBase.isFocusField && this.pdfViewerBase.focusField) {
      const currentField: HTMLElement = document.getElementById(
        this.pdfViewerBase.focusField.id
      );
      if (currentField) {
        if (
          (this.pdfViewerBase.focusField.type === 'SignatureField' ||
            this.pdfViewerBase.focusField.type === 'InitialField') &&
          ...
          const height: number =
            this.pdfViewerBase.pageSize[parseInt(pageIndex.toString(), 10)]
              .height;
          this.pdfViewer.bookmark.goToBookmark(
            this.pdfViewerBase.focusField.pageIndex,
            height - y
          );
        } else {
          currentField.focus();
  ...
  private rgbaToHex(rgba: string): string {
    ...
    const a: number = parseFloat(
      parseFloat(
        this.trim(inParts[3].substring(0, inParts[3].length - 1))
      ).toFixed(2)
    );
  ...
  private getListValues(currentData: any): ItemModel[] {
    ...
    for (let i: number = 0; i < listItem.length; i++) {
      if (
        listItem[parseInt(i.toString(), 10)] === currentData['SelectedValue']
      ) {
        this.selectedIndex.push(i);
      }
      options.push({
        itemName: listItem[parseInt(i.toString(), 10)],
        itemValue: listItem[parseInt(i.toString(), 10)],
      });
    }
  ...
  private signatureFieldNavigate(nextSign: boolean): void {
    ...
    const collectionData: FormFieldModel[] =
      this.pdfViewer.formFieldCollections;
    ...
    if (this.currentTarget) {
      if (this.pdfViewer.formDesignerModule) {
        ...
        for (let i: number = 0; i < signatureFields.length; i++) {
          currentField = this.pdfViewer.formDesignerModule
            ? signatureFields[parseInt(i.toString(), 10)].FormField
            : signatureFields[parseInt(i.toString(), 10)];
  ...
  private renderSignatureField(currentSignIndex: number): void {
    ...
    if (curSignIndex < collectionData.length) {
      for (let i: number = 0; i < collectionData.length; i++) {
        if (this.pdfViewer.formDesignerModule) {
          const curSignIndexId: any =
            collectionData[parseInt(curSignIndex.toString(), 10)].id;
          const signatureFieldData: any =
            collectionData[parseInt(i.toString(), 10)];
          if (curSignIndexId === signatureFieldData.id) {
            const pageIndex: any =
              signatureFieldData.pageIndex >= 0
                ? signatureFieldData.pageIndex
                : signatureFieldData.pageNumber;
            const isRender: boolean =
              this.pdfViewer.annotationModule.findRenderPageList(pageIndex);
            if (!isRender) {
          ...
          const curSignIndexId: string = this.pdfViewer.formDesignerModule
            ? signatureFields[parseInt(curSignIndex.toString(), 10)].FormField
                .uniqueID
            : signatureFields[parseInt(curSignIndex.toString(), 10)].uniqueID;
          const signatureFieldData: any = this.pdfViewer.formDesignerModule
            ? signatureFields[parseInt(i.toString(), 10)].FormField
            : signatureFields[parseInt(i.toString(), 10)];
          if (curSignIndexId === signatureFieldData.uniqueID) {
            ...
            this.currentTarget = document.getElementById(
              signatureFieldData.uniqueID
            );
            currentField = signatureFieldData;
            break;
          }
      ...
      if (this.currentTarget) {
        if (
          this.currentTarget.className ===
            'e-pdfviewer-signatureformfields-signature' &&
          !this.pdfViewer.formDesignerModule
        ) {
          ...
          if (this.pdfViewer.formDesignerModule) {
            document
              .getElementById(this.currentTarget.id)
              .parentElement.focus();
          } else {
            document.getElementById(this.currentTarget.id).focus();
          }
          ...
          if (currentData.Name !== 'ink') {
            const formFieldCollection: FormFieldModel = {
              ...
              id:
                ...
                'input_' +
                parseFloat(currentData['PageIndex']) +
                '_' +
                i,
              isReadOnly: false,
  ...
  private retreiveFormFieldsData(
    ...
    if (currentData.FieldName !== '') {
      ...
      const fieldProperties: any = {
        ...
        signatureType: currentData['SignatureType']
          ? currentData['SignatureType']
          : '',
      ...
      const addedElement: any = this.pdfViewer.formDesignerModule.addFormField(
        fieldType,
        fieldProperties,
        isCollection
      ) as any;
  ...
  public updateFormFieldsCollection(formField: any): void {
    ...
    const formFieldCollection: FormFieldModel = {
      ...
      isMultiline: formField.isMultiline
        ? formField.isMultiline
        : formField.Multiline,
      insertSpaces: formField.insertSpaces
        ? formField.insertSpaces
        : formField.InsertSpaces,
      isTransparent: formField.isTransparent
        ? formField.isTransparent
        : formField.IsTransparent,
      rotateAngle: formField.rotateAngle
        ? formField.rotateAngle
        : formField.RotationAngle,
      selectedIndex: formField.selectedIndex
        ? formField.selectedIndex
        : formField.SelectedList,
      ...
      tooltip: formField.tooltip
        ...
        ? formField.ToolTip
        : '',
      signatureIndicatorSettings: formField.signatureIndicatorSettings
        ? formField.signatureIndicatorSettings
        : '',
    };
    this.pdfViewer.formFieldCollections[
      this.pdfViewer.formFieldCollections.findIndex(
        (el: FormFieldModel) => el.id === formFieldCollection.id
      )
    ] = formFieldCollection;
  }
  ...
  public downloadFormFieldsData(): any {
    ...
    if (data) {
      ...
      for (let m: number = 0; m < formFieldsData.length; m++) {
        ...
        if (
          currentData.Name === 'Textbox' ||
          currentData.Name === 'Password' ||
          currentData.Multiline
        ) {
          if (
            isRequired &&
            (currentData.Text === '' || currentData.Text === null)
          ) {
            this.pdfViewerBase.validateForm = true;
            this.pdfViewerBase.nonFillableFields[currentData.FieldName] =
              currentData.Text;
          } else {
          ...
          if (isRequired && currentData.Selected === false) {
            this.pdfViewerBase.validateForm = true;
            this.pdfViewerBase.nonFillableFields[currentData.GroupName] =
              currentData.Value;
          } else {
          ...
          if (currentData.CheckboxIndex && currentData.Selected) {
            fieldDatas = {
              ...
              isReadOnly: currentData.IsReadonly,
              fieldValue: !isNullOrUndefined(currentData.Value)
                ? currentData.Value
                : '',
            };
            datas[currentData.GroupName] = fieldDatas;
          } else if (
            datas[currentData.GroupName] === undefined ||
            datas[currentData.GroupName] === null
          ) {
            fieldDatas = {
              isSelected: currentData.Selected,
              isReadOnly: currentData.IsReadonly,
          ...
          if (isRequired && currentData.SelectedValue === '') {
            this.pdfViewerBase.validateForm = true;
            this.pdfViewerBase.nonFillableFields[currentData.Text] =
          ...
          for (let m: number = 0; m < currentData.SelectedList.length; m++) {
            const currentElement: any =
              currentData.SelectedList[parseInt(m.toString(), 10)];
          ...
          if (
            isRequired &&
            (currentData.Value === null || currentData.Value === '')
          ) {
            this.addSignaturePath(currentData);
          }
          if (currentData.Value && currentData.Value !== '') {
            csData = currentData.Value;
            const fontFamily: any = currentData.fontFamily
              ? currentData.fontFamily
              : currentData.FontFamily;
            if (fontFamily) {
              datas[currentData.FieldName + 'fontName'] = fontFamily;
              datas[currentData.FieldName + 'fontSize'] = currentData.fontSize
          ...
          if (currentData.Bounds) {
            const bounds: any = this.getSignatureBounds(
              currentData.LineBounds,
              currentData.Bounds,
              currentData.PageIndex
            );
            currentData.Bounds.x = bounds.x;
            currentData.Bounds.y = bounds.y;
            datas[currentData.FieldName + 'bounds'] = JSON.stringify(
              currentData.Bounds
            );
          } else {
  ...
  private focusFormFields(event: MouseEvent): void {
    ...
    if (
      currentTarget &&
      currentTarget.className !== 'e-pdfviewer-signatureformfields' &&
      ...
      const currentColor: any =
        backgroundcolor.slice(0, currentIndex + 1) + 0 + ')';
  ...
  private blurFormFields(event: MouseEvent): void {
    ...
    if (currentTarget.InsertSpaces && this.isKeyDownCheck) {
      const font: number =
        parseInt(currentTarget.style.width, 10) -
        parseInt(currentTarget.style.height, 10) / 2;
      currentTarget.style.width = '' + font + 'px';
      this.isKeyDownCheck = false;
    }
    if (currentTarget.type === 'checkbox') {
      this.pdfViewer.fireFocusOutFormField(
        currentTarget.name,
    ...
    const currentIndex: any = backgroundcolor.lastIndexOf(',');
    const currentColor: any =
      backgroundcolor.slice(0, currentIndex + 1) + 0.2 + ')';
    currentTarget.style.backgroundColor = currentColor;
  ...
  public updateFormFields(event: MouseEvent): void {
    ...
    if (currentTarget.className === 'e-pdfviewer-ListBox') {
      ...
      currentTarget.className === 'e-pv-buttonItem' ||
      currentTarget.type === 'button'
    ) {
      this.pdfViewer.fireButtonFieldClickEvent(
        currentTarget.value,
        currentTarget.name,
        currentTarget.id
      );
    ...
    for (
      let m: number = 0;
      m < this.pdfViewer.formFieldCollections.length;
      m++
    ) {
      if (
        currentTarget.id ===
        this.pdfViewer.formFieldCollections[parseInt(m.toString(), 10)].id
      ) {
        if (
          this.pdfViewer.formDesignerModule ||
          this.pdfViewer.annotationModule
        ) {
          this.pdfViewer.fireFormFieldClickEvent(
            'formFieldClicked',
            this.pdfViewer.formFieldCollections[parseInt(m.toString(), 10)]
          );
        }
        if (
          currentTarget.className === 'e-pdfviewer-signatureformfields' ||
  ...
  public drawSignature(
    ...
    if (this.pdfViewer.formDesignerModule) {
      targetName =
        this.currentTarget && this.currentTarget.offsetParent
          ? this.currentTarget.offsetParent.name
          : this.currentTarget
          ? this.currentTarget.name
          : target.name
          ? target.name
          : target.offsetParent.name;
    } else {
      targetName = this.currentTarget
        ? this.currentTarget.name
        : target.name
        ? target.name
        : target.offsetParent.name;
    ...
    for (let i: number = 0; i < formFieldsData.length; i++) {
      const fieldName: string = this.pdfViewer.formDesigner
        ? formFieldsData[parseInt(i.toString(), 10)].FormField.name
        : formFieldsData[parseInt(i.toString(), 10)].FieldName;
      if (
        this.pdfViewer.formDesigner
          ? fieldName === targetName
          : fieldName === targetName &&
            !isNullOrUndefined(
              formFieldsData[parseInt(i.toString(), 10)].ActualFieldName
            )
      ) {
        target = this.pdfViewer.formDesigner
          ? document.getElementById(
              formFieldsData[parseInt(i.toString(), 10)].Key.split('_')[0]
            )
          : document.getElementById(
              formFieldsData[parseInt(i.toString(), 10)].uniqueID
            );
        const currentField: any = target;
        let signatureAdd: boolean = true;
        this.pdfViewer.annotations.filter(function (
          item: PdfAnnotationBase
        ): void {
          if (
            !isNullOrUndefined(currentField) &&
            item.id === target.id + '_content'
          ) {
            signatureAdd = false;
          }
        });
        if (!isNullOrUndefined(currentField)) {
          const elementId: string =
            currentField.offsetParent.offsetParent.id.split('_')[0];
          const signatureField: PdfAnnotationBase = (
            this.pdfViewer.nameTable as any
          )[`${elementId}`];
          if (target && target.offsetParent && signatureField) {
            targetBounds = target.getBoundingClientRect();
            parentElementBounds =
              target.offsetParent.offsetParent.offsetParent.getBoundingClientRect();
            this.pdfViewerBase.drawSignatureWithTool = true;
            if (
              target.nextSibling &&
              target.nextSibling.id.indexOf('initial') !== -1
            ) {
          ...
          let currentValue: string = value
            ? value
            : this.pdfViewerBase.signatureModule.outputString;
          ...
          let currentFont: string = fontname
            ? fontname
            : this.pdfViewerBase.signatureModule.fontName;
          ...
          const currentWidth: number = this.pdfViewerBase.drawSignatureWithTool
            ? targetBounds.width / zoomvalue
            : parseFloat(currentField.style.width) / zoomvalue;
          const currentHeight: number = this.pdfViewerBase.drawSignatureWithTool
            ? targetBounds.height / zoomvalue
            : parseFloat(currentField.style.height) / zoomvalue;
          const currentLeft: number = this.pdfViewerBase.drawSignatureWithTool
            ? (targetBounds.left - parentElementBounds.left) / zoomvalue
            : parseFloat(currentField.style.left) / zoomvalue;
          const currentTop: number = this.pdfViewerBase.drawSignatureWithTool
            ? (targetBounds.top - parentElementBounds.top) / zoomvalue
            : parseFloat(currentField.style.top) / zoomvalue;
          const currentPage: number = this.pdfViewerBase.drawSignatureWithTool
            ? target.nextElementSibling
              ? parseFloat(target.nextElementSibling.id.split('_')[1])
              : parseFloat(currentField.id.split('_')[1])
            : parseFloat(currentField.id.split('_')[1]);
          const currentIndex: number = this.pdfViewerBase.drawSignatureWithTool
            ? target.nextElementSibling
              ? parseFloat(target.nextElementSibling.id.split('_')[2])
              : parseFloat(currentField.id.split('_')[2])
            : parseFloat(currentField.id.split('_')[2]);
          let signString: string =
            this.pdfViewerBase.signatureModule.saveImageString;
          let signatureFontFamily: string;
          let signatureFontSize: number;
          let rotateAngleString: string = currentField.offsetParent.offsetParent
            .style.transform
            ? currentField.offsetParent.offsetParent.style.transform
            : currentField.style.transform;
          rotateAngleString = rotateAngleString.substring(
            rotateAngleString.indexOf('(') + 1,
            rotateAngleString.indexOf('d')
          );
          const rotateAngle: number = rotateAngleString
            ? parseInt(rotateAngleString, 10)
            : 0;
          if (signatureType === 'Type' || signatureType === 'Text') {
            if (!currentFont) {
              currentFont = 'Helvetica';
            }
            bounds = this.getSignBounds(
              currentIndex,
              rotateAngle,
              currentPage,
              zoomvalue,
            ...
            annot = {
              ...
              fontSize: bounds.height / this.signatureFontSizeConstent,
            };
            if (annot.shapeAnnotationType === 'SignatureText') {
              const textWidth: number = this.getTextWidth(
                annot.data,
                annot.fontSize,
                annot.fontFamily
              );
              ...
              annot.fontSize = this.getFontSize(
                Math.floor(annot.fontSize * widthRatio)
              );
            }
            ...
            if (currentValue.indexOf('base64') !== -1) {
              ...
              if (this.pdfViewer.signatureFitMode === 'Default') {
                const signatureBounds: any =
                  this.pdfViewerBase.signatureModule.updateSignatureAspectRatio(
                    currentValue,
                    false,
                    currentField
                  );
                bounds = this.getSignBounds(
                  ...
                  currentLeft,
                  currentTop,
                  signatureBounds.width,
                  signatureBounds.height,
                  true
                );
          ...
          if (
            ...
            const obj: PdfAnnotationBaseModel = this.pdfViewer.add(
              annot as PdfAnnotationBase
            );
            if (
              signatureField.wrapper.children[1] &&
              obj.wrapper.id === signatureField.wrapper.children[1].id
            ) {
              if (
                !isNullOrUndefined(signatureField.wrapper.children[1]) &&
                signatureField.wrapper.children[1].relativeMode === 'Point'
              ) {
          ...
          if (
            annot &&
            annot.shapeAnnotationType === 'Path' &&
            currentValue !== ''
          ) {
            ...
            this.pdfViewerBase.signatureModule.addSignatureCollection(
              bounds,
              position
            );
          ...
          if (signatureType !== 'Image') {
            const canvass: HTMLElement = document.getElementById(
              this.pdfViewer.element.id + '_annotationCanvas_' + currentPage
            );
            ...
            if (this.pdfViewerBase.drawSignatureWithTool && signatureField) {
              const key: string =
                target.offsetParent.offsetParent.id.split('_')[0] + '_content';
              annot.bounds = {
            ...
            if (
              annot.shapeAnnotationType === 'Path' ||
              annot.shapeAnnotationType === 'SignatureText'
            ) {
              this.pdfViewer.fireSignatureAdd(
                annot.pageIndex,
                annot.id,
                annot.shapeAnnotationType,
                annot.bounds,
                annot.opacity,
                null,
                null,
  ...
  private imageOnLoad(
    ...
    rotateAngle: number,
    currentField: any,
    signatureField: PdfAnnotationBase,
    signString: string,
    ...
    if (this.pdfViewer.signatureFitMode === 'Default') {
      const padding: number = Math.min(
        bounds.height / this.paddingDifferenceValue,
        bounds.width / this.paddingDifferenceValue
      );
    ...
    const annot: PdfAnnotationBaseModel = {
      ...
      annotName: 'SignatureField',
      comments: [],
      review: { state: '', stateModel: '', modifiedDate: '', author: '' },
    };
    signString = annot.data;
    if (this.pdfViewerBase.drawSignatureWithTool && signatureField) {
      annot.id = signatureField.id + '_content';
    ...
    const canvass: any = document.getElementById(
      this.pdfViewer.element.id + '_annotationCanvas_' + currentPage
    );
    this.pdfViewer.renderDrawing(canvass as any, currentPage);
    this.pdfViewerBase.signatureModule.showSignatureDialog(false);
  ...
  private updateSignatureDataInSession(annot: any, key: string): void {
    const data: string =
      this.pdfViewerBase.getItemFromSessionStorage('_formDesigner');
    const formFieldsData: any = JSON.parse(data);
    if (!isNullOrUndefined(formFieldsData)) {
      for (let i: number = 0; i < formFieldsData.length; i++) {
        if (formFieldsData[parseInt(i.toString(), 10)].Key === key) {
          const formFieldIndex: number =
            this.pdfViewer.formFieldCollection.findIndex(
              (el: any) =>
                el.id ===
                formFieldsData[parseInt(i.toString(), 10)].FormField.id.split(
                  '_'
                )[0]
            );
          if (annot.shapeAnnotationType === 'SignatureText') {
            ...
            this.pdfViewerBase.formFieldCollection[
              parseInt(i.toString(), 10)
            ].FormField.signatureType = 'Text';
            (this.pdfViewer.nameTable as any)[`${key}`].signatureType = 'Text';
            formFieldsData[parseInt(i.toString(), 10)].FormField.fontFamily =
              annot.fontFamily === 'TimesRoman'
                ? 'Times New Roman'
                : annot.fontFamily;
            this.pdfViewerBase.formFieldCollection[
              parseInt(i.toString(), 10)
            ].FormField.fontFamily = annot.fontFamily;
            (this.pdfViewer.nameTable as any)[`${key}`].fontFamily =
              annot.fontFamily;
            (this.pdfViewer.nameTable as any)[
              `${key}`.split('_')[0]
            ].fontFamily = annot.fontFamily;
            formFieldsData[parseInt(i.toString(), 10)].FormField.fontSize =
              annot.fontSize;
            this.pdfViewerBase.formFieldCollection[
              parseInt(i.toString(), 10)
            ].FormField.fontSize = annot.fontSize;
            (this.pdfViewer.nameTable as any)[`${key}`].fontSize =
              annot.fontSize;
            (this.pdfViewer.nameTable as any)[`${key}`.split('_')[0]].fontSize =
              annot.fontSize;
            if (formFieldIndex > -1) {
              this.pdfViewer.formFieldCollection[
                parseInt(formFieldIndex.toString(), 10)
              ].signatureType = 'Text';
            }
          ...
          this.pdfViewerBase.formFieldCollection[
            parseInt(i.toString(), 10)
          ].FormField.signatureBound = annot.bounds;
          (this.pdfViewer.nameTable as any)[`${key}`].signatureBound =
            annot.bounds;
          if (formFieldIndex > -1) {
            this.pdfViewer.formFieldCollection[
              parseInt(formFieldIndex.toString(), 10)
            ].signatureBound = annot.bounds;
          }
          if (annot.shapeAnnotationType === 'Path') {
            ...
            (this.pdfViewer.nameTable as any)[key.split('_')[0]].value =
              annot.data;
            this.pdfViewerBase.formFieldCollection[
              parseInt(i.toString(), 10)
          ...
          this.pdfViewer.formDesigner.updateFormFieldCollections(
            formFieldsData[parseInt(i.toString(), 10)].FormField
          );
          this.pdfViewer.formDesigner.updateFormFieldPropertiesChanges(
  ...
  public getSignBounds(
    ...
    let bounds: any;
    const signatureId: string = this.pdfViewer.isInitialFieldToolbarSelection
      ? 'initialIcon'
      : 'signIcon';
    const signIcon: HTMLElement = document.getElementById(
      signatureId + '_' + currentPage + '_' + currentIndex
    );
    const signLeft: number = signIcon
      ? parseFloat(signIcon.style.left) * zoomvalue
      : 0;
    ...
    if (this.formFieldsData) {
      for (let i: number = 0; i < this.formFieldsData.length; i++) {
        const currentField: any =
          this.formFieldsData[parseInt(i.toString(), 10)];
        if (
          event.target.name === currentField.FieldName &&
          event.target.id !== currentField.uniqueID
        ) {
          const currentTarget: any = document.getElementById(
            this.formFieldsData[parseInt(i.toString(), 10)].uniqueID
          );
          if (currentTarget) {
  ...
  private updateFormFieldsValue(event: MouseEvent | KeyboardEvent): void {
    ...
    if (
      event.which === 9 &&
      currentTarget &&
      (currentTarget.className === 'e-pdfviewer-formFields' ||
        currentTarget.className ===
          'e-pdfviewer-signatureformfields e-pv-signature-focus' ||
        currentTarget.className === 'e-pdfviewer-signatureformfields-signature')
    ) {
      const id: any = currentTarget.id.split('input_')[1].split('_')[0];
      const textLayer: HTMLElement = document.getElementById(
        this.pdfViewer.element.id + '_textLayer_' + parseInt(id, 10)
      );
      const currentFields: any = textLayer.getElementsByClassName(
        'e-pdfviewer-formFields'
      );
      let istabindexed: boolean = true;
      fieldIndex = this.pdfViewer.formFieldCollections.findIndex(
        (field: FormFieldModel) => field.id === currentTarget.id
      );
      if (!event.shiftKey && (event as KeyboardEvent).key === 'Tab') {
        nextFields =
          fieldIndex + 1 < this.pdfViewer.formFieldCollections.length
            ? this.pdfViewer.formFieldCollections[fieldIndex + 1]
            : this.pdfViewer.formFieldCollections[0];
      }
    ...
    if (event.shiftKey && (event as KeyboardEvent).key === 'Tab') {
      const fieldIndex: number = this.pdfViewer.formFieldCollections.findIndex(
        (field: FormFieldModel) => field.id === currentTarget.id
      );
      const nextField: any =
        fieldIndex > 0
          ? this.pdfViewer.formFieldCollections[fieldIndex - 1]
          : this.pdfViewer.formFieldCollections[
              this.pdfViewer.formFieldCollections.length - 1
            ];
      this.pdfViewer.focusFormField(nextField);
      event.preventDefault();
    }
    if (
      (event.currentTarget as any).classList.contains(
        'e-pdfviewer-signatureformfields'
      ) ||
      (event.currentTarget as any).classList.contains(
        'e-pdfviewer-signatureformfields-signature'
      )
  ...
  public updateDataInSession(
    ...
    if (data && !this.pdfViewer.formDesignerModule) {
      const FormFieldsData: any = JSON.parse(data);
      filterFields = FormFieldsData.filter(
        (item: any) => item.uniqueID === target.id
      );
      if (filterFields.length > 0) {
        fieldsByName = filterFields[0].FieldName;
        filterFieldName = FormFieldsData.filter(
          (item: any) => item.FieldName === fieldsByName
        );
      ...
      for (let m: number = 0; m < FormFieldsData.length; m++) {
        const currentData: any = FormFieldsData[parseInt(m.toString(), 10)];
        if (
          currentData.uniqueID === target.id ||
          fieldsByName === currentData.FieldName
        ) {
          if (
            ...
            if (
              signField.classList.contains('e-pdfviewer-signatureformfields') ||
              signField.classList.contains(
                'e-pdfviewer-signatureformfields-signature'
              )
            ) {
              if (signaturePath) {
            ...
            const filterRadioButtonSameName: any = FormFieldsData.filter(
              (sameNameRadioButtonField: any) =>
                sameNameRadioButtonField.GroupName === target.name &&
                sameNameRadioButtonField.Name === 'RadioButton'
            );
            for (let l: number = 0; l < filterRadioButtonSameName.length; l++) {
              const currentType: any =
                filterRadioButtonSameName[parseInt(l.toString(), 10)];
              if (currentType.uniqueID !== targetRadioButton) {
                currentType.Selected = false;
              }
              const currentTarget: any = document.getElementById(
                currentType.uniqueID
              );
              if (currentTarget) {
            ...
            if (
              target.value === currentData.Value ||
            ...
            const filterCheckBoxSameName: any = FormFieldsData.filter(
              (sameNameCheckboxField: any) =>
                sameNameCheckboxField.GroupName === target.name &&
                sameNameCheckboxField.Name === 'CheckBox'
            );
            for (let l: number = 0; l < filterCheckBoxSameName.length; l++) {
              const currentType: any =
                filterCheckBoxSameName[parseInt(l.toString(), 10)];
            ...
            for (let z: number = 0; z < currentValue.length; z++) {
              ...
              for (let k: number = 0; k < childrens.length; k++) {
                if (
                  childrens[parseInt(k.toString(), 10)] ===
                  currentValue[parseInt(z.toString(), 10)]
                ) {
            ...
            const index: number = currentData.TextList
              ? currentData.TextList.indexOf(target.value)
              : 0;
          ...
          currentData.IsRequired = target.Required
            ? target.Required
            : target.required
            ? target.required
            : false;
          ...
          target &&
          target.getAttribute('list') != null &&
          target.type === 'text' &&
          currentData.uniqueID === target.list.id
        ) {
          currentData.SelectedValue = target.value;
        }
        this.updateFormFieldsCollection(currentData);
    ...
    if (this.pdfViewer.formDesignerModule && target && target.id) {
      const selectedItem: any = (this.pdfViewer.nameTable as any)[
        target.id.split('_')[0]
      ];
      if (
        selectedItem &&
        selectedItem.wrapper &&
        selectedItem.wrapper.children[0]
      ) {
        selectedItem.value = target.value;
        const point: PointModel = cornersPointsBeforeRotation(
          selectedItem.wrapper.children[0]
        ).topLeft;
        this.pdfViewer.formDesignerModule.updateFormDesignerFieldInSessionStorage(
          point,
          selectedItem.wrapper.children[0] as DiagramHtmlElement,
          selectedItem.formFieldAnnotationType,
          selectedItem
        );
      }
    }
  ...
  public removeExistingFormFields(): void {
    ...
    if (formFieldsData) {
      for (let i: number = 0; i < formFieldsData.length; i++) {
        if (
          formFieldsData[parseInt(i.toString(), 10)].FormField
            .formFieldAnnotationType === 'RadioButton'
        ) {
          let buttonItem: any = [];
          buttonItem =
            formFieldsData[parseInt(i.toString(), 10)].FormField
              .radiobuttonItem;
          const sameButtonItemId: string =
            formFieldsData[parseInt(i.toString(), 10)].FormField.id.split(
              '_'
            )[0];
  ...
  private applyCommonProperties(
    ...
    const inputField: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + 'input_' + pageIndex + '_' + index
    );
    if (inputField) {
      const textLayer: any = document.getElementById(
        this.pdfViewer.element.id + '_textLayer_' + pageIndex
      );
    ...
    if (signIcon && !isFieldRotated) {
      ...
      if (signIcon.style.transform === 'rotate(90deg)') {
        ...
        signIcon.style.left =
          left -
          (hightDifference - signIconWidth * zoomvalue) +
          widthDifference / 2 +
          'px';
      }
      ...
      if (signIcon.style.transform === 'rotate(270deg)') {
        ...
        signIcon.style.left =
          left -
          (hightDifference - widthDifference) +
          widthDifference / 2 +
          'px';
        signIcon.style.top = top + 'px';
      }
  ...
  public createFormFields(
    ...
    switch (currentData['Name']) {
      case 'Textbox':
        ...
        break;
      case 'Password':
        currentField = this.createTextBoxField(
          currentData,
          pageIndex,
          'password'
        );
        break;
      case 'RadioButton':
        currentField = this.createRadioBoxField(
          currentData,
          pageIndex,
          'radio'
        );
      ...
      case 'CheckBox':
        ...
        break;
      case 'DropDown':
        currentField = this.createDropDownField(
          currentData,
          pageIndex,
          index,
          printContainer
      ...
      case 'InitialField':
      case 'SignatureField': {
        currentField = this.createSignatureField(
          currentData,
          pageIndex,
          index,
          printContainer,
  ...
  private checkIsReadonly(data: any, inputField: any): void {
    ...
    for (let n: number = 0; n < this.readOnlyCollection.length; n++) {
      if (
        inputField.id === this.readOnlyCollection[parseInt(n.toString(), 10)]
      ) {
        isReadonly = true;
        break;
      }
    ...
    if (
      !this.pdfViewer.formDesignerModule &&
      !this.pdfViewer.annotationModule &&
      (data.IsInitialField || data.IsSignatureField)
    ) {
      isReadonly = true;
    }
  ...
  public formFieldsReadOnly(isReadonly: boolean): void {
    const formFields: HTMLCollectionOf<Element> =
      document.getElementsByClassName('e-pdfviewer-formFields');
    this.makeformFieldsReadonly(formFields, isReadonly);
    const signatureFields: HTMLCollectionOf<Element> =
      document.getElementsByClassName('e-pdfviewer-signatureformfields');
  ...
  private applyDefaultColor(inputField: any): void {
    if (
      ...
      inputField.style.backgroundColor === 'rgba(218, 234, 247, 0.2)'
    ) {
  ...
  private createDropDownField(
    ...
    if (data.Selected && !printContainer) {
      const previousField: any = document.getElementById(
        'editableDropdown' + pageIndex + '_' + index
      );
      if (previousField) {
    ...
    for (let j: number = 0; j < childItems.length; j++) {
      ...
      if (
        data.SelectedValue === childItems[parseInt(j.toString(), 10)] ||
        data.selectedIndex === j
      ) {
  ...
  private createSignatureField(
    ...
    inputField.name = data.FieldName;
    const zoomvalue: number = this.pdfViewerBase.getZoomFactor();
    const previousField: HTMLElement = document.getElementById(
      'signIcon_' + pageIndex + '_' + index
    );
    ...
    const signIndicator: string = this.pdfViewerBase.isInitialField
      ? 'Initial'
      : 'Sign';
    ...
    const signatureFieldIndicatorWidth: number = this.pdfViewer
      .signatureFieldSettings.signatureIndicatorSettings
      ? this.pdfViewer.signatureFieldSettings.signatureIndicatorSettings
          .width === 19
        ? this.pdfViewerBase.isInitialField
          ? 27
          : 19
        : this.pdfViewer.signatureFieldSettings.signatureIndicatorSettings.width
      : 19;
    ...
    const height: number = this.pdfViewer.signatureFieldSettings
      ...
      : (indicatorHeight * zoomvalue) / 2;
    const width: number =
      signatureFieldIndicatorWidth > (indicatorWidth * zoomvalue) / 2
        ? (indicatorWidth * zoomvalue) / 2
        : signatureFieldIndicatorWidth;
    const size: number = this.pdfViewer.signatureFieldSettings
      .signatureIndicatorSettings
      ? this.pdfViewer.signatureFieldSettings.signatureIndicatorSettings
          .fontSize >
        height / 2
    ...
    if (
      !(height + this.indicatorPaddingValue > indicatorHeight * zoomvalue) &&
    ...
    span.innerHTML = this.pdfViewer.signatureFieldSettings
      .signatureIndicatorSettings
      ? this.pdfViewer.signatureFieldSettings.signatureIndicatorSettings.text
        ? this.pdfViewer.signatureFieldSettings.signatureIndicatorSettings.text
        : signIndicator
      : signIndicator;
    span.style.color = this.pdfViewer.signatureFieldSettings
      .signatureIndicatorSettings
      ? this.pdfViewer.signatureFieldSettings.signatureIndicatorSettings.color
        ? this.pdfViewer.signatureFieldSettings.signatureIndicatorSettings.color
        : 'black'
      : 'black';
    span.style.backgroundColor = this.pdfViewer.signatureFieldSettings
      .signatureIndicatorSettings
      ? this.pdfViewer.signatureFieldSettings.signatureIndicatorSettings
          .backgroundColor
        ? this.pdfViewer.signatureFieldSettings.signatureIndicatorSettings
        ...
        : 'orange'
      : 'orange';
    span.style.opacity = this.pdfViewer.signatureFieldSettings
      .signatureIndicatorSettings
    ...
    if (data) {
      ...
      for (let m: number = 0; m < formFieldsData.length; m++) {
        ...
        if (
          ...
          (this.pdfViewer.formDesigner
            ? currentData.FieldName.split('_')[0] ===
              ...
              currentData.FieldName.split('_')[0] === signData.FieldName
            : currentData.FieldName.split('_')[0] === signData.FieldName &&
        ...
        if (
          ...
          this.pdfViewer.formDesigner
            ? currentData.FieldName ===
                signData.ActualFieldName + '_' + count ||
              currentData.FieldName === signData.FieldName + '_' + count
            : (currentData.FieldName === signData.FieldName + '_' + count ||
                currentData.FieldName ===
                  signData.ActualFieldName + '_' + count) &&
              !isNullOrUndefined(signData.ActualFieldName) &&
  ...
  private applyPosition(
    ...
    if (bounds) {
      ...
      const fieldBounds: any = {
        left: left,
        top: top,
        width: width,
        height: height,
      };
      const annotBounds: any = this.getBounds(
        fieldBounds,
        pageIndex,
        rotation,
        isFieldRotated
      ...
      this.pdfViewerBase.setStyleToTextDiv(
        inputField,
        annotBounds.left,
        annotBounds.top,
        fontHeight,
  ...
  private renderExistingAnnnot(
    ...
    if (!printContainer) {
      ...
      const newBounds: any = this.updateSignatureBounds(
        bound,
        currentPage,
        isFieldRotated
      );
      let annot: PdfAnnotationBaseModel;
      const fontFamily: any = data.FontFamily
        ? data.FontFamily
        : data.fontFamily;
      if (this.pdfViewerBase.isSignatureImageData(data.Value)) {
        ...
        annot.fontFamily =
          fontFamily === 'TimesRoman' ? 'Times New Roman' : fontFamily;
        annot.fontSize = data.FontSize ? data.FontSize : data.fontSize;
      }
      if (
        (data.Name === 'SignatureField' || data.Name === 'InitialField') &&
        !isNullOrUndefined(data.id)
      ) {
        const elementId: string = data.id;
        const signatureFieldElement: HTMLElement = document.getElementById(
          elementId + '_content_html_element'
        );
        const signatureField: PdfAnnotationBase = (
          this.pdfViewer.nameTable as any
        )[`${elementId}`];
        ...
        signatureField.wrapper.children.push(obj.wrapper);
        if (
          !isNullOrUndefined(signatureFieldElement) &&
        ...
        if (
          target &&
          (target as any).classList.contains(
            'e-pdfviewer-signatureformfields-signature'
          )
        ) {
      ...
      const obj: any = this.pdfViewer.formFieldCollection.filter(
        (field: any) => {
          return annot.id.split('_')[0] === field.id;
        }
      );
  ...
  public resetFormFields(): void {
    ...
    for (let i: number = 0; i < formFieldData.length; i++) {
      const currentData: FormFieldModel =
        formFieldData[parseInt(i.toString(), 10)];
      ...
      if (
        currentData.type !== 'RadioButton' &&
        currentData.type !== 'SignatureField'
      ) {
        this.updateDataInSession(this.currentTarget);
      }
    }
  ...
  public clearFormFieldStorage(): void {
    const sessionSize: any = Math.round(
      JSON.stringify(window.sessionStorage).length / 1024
    );
    ...
    if (this.pdfViewerBase.isStorageExceed) {
      ...
      for (let i: number = 0; i < storageLength; i++) {
        if (
          ...
          window.sessionStorage.key(i).split('_')[3]
        ) {
          if (window.sessionStorage.key(i).split('_')[3] === 'formfields') {
            ...
            window.sessionStorage.key(i).split('_')[3] === 'formDesigner'
          ) {
            this.pdfViewerBase.formFieldStorage[window.sessionStorage.key(i)] =
              window.sessionStorage.getItem(window.sessionStorage.key(i));
            formFieldsList.push(window.sessionStorage.key(i));
      ...
      if (formFieldsList) {
        for (let i: number = 0; i < formFieldsList.length; i++) {
          window.sessionStorage.removeItem(
            formFieldsList[parseInt(i.toString(), 10)]
          );
        }
      }
    }
    ...
    if (data) {
      ...
      for (let m: number = 0; m < formFieldsData.length; m++) {
        ...
        if (currentData.Name === 'Textbox') {
          ...
          let annotation: PdfAnnotationBaseModel = (
            this.pdfViewer.nameTable as any
          )[currentData.uniqueID];
          if ((annotation as any).propName !== 'annotations') {
            annotation = (this.pdfViewer.nameTable as any)[
              currentData.uniqueID + '_content'
            ];
          }
          if (annotation) {
            if (
