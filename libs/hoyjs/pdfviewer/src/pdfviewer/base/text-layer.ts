import { createElement, isNullOrUndefined, Browser, isBlazor } from '@hoyui/base';
...
export class TextLayer {
    ...
    private removeForeignObjects(element: HTMLElement): void {
        const childElement: HTMLCollectionOf<Element> = element.getElementsByClassName('foreign-object');
        if (childElement) {
            for (let i: number = 0; i < childElement.length; i++) {
                if (Browser.isDevice) {
                    ...
                    if (childElement[parseInt(i.toString(), 10)].parentElement.className === 'e-pv-text-layer' && childElement[0].parentElement.className !== 'e-pv-checkbox-outer-div') {
                        ...
                        const outerDivParent: Element = document.getElementById(childElement[0].parentElement.id);
                        if (outerDivParent) {
