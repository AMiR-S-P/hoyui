import { createElement, Browser, isNullOrUndefined } from '@hoyui/base';
...
export class TextSelection {
   ...
   * @returns {void}
   */
  public textSelectionOnMouseMove(
    target: EventTarget,
    x: number,
    y: number,
    isExtended?: boolean
    ...
    if (targetElement.nodeType === targetElement.TEXT_NODE) {
      if (
        !this.isSelectionStartTriggered &&
        !this.pdfViewerBase.getTextMarkupAnnotationMode()
      ) {
        this.pdfViewer.fireTextSelectionStart(
          this.pdfViewerBase.currentPageNumber
        );
      ...
      if (selection.anchorNode !== null) {
        const position: number = selection.anchorNode.compareDocumentPosition(
          selection.focusNode
        );
        if (
      ...
      while (currentPosition < endPosition) {
        ...
        if (
          rangeBounds.left <= x &&
          rightBounds >= x &&
          parseInt(rangeBounds.top.toString(), 10) <= y &&
          rangeBounds.bottom >= y
        ) {
          if (
            selection.anchorNode !== null &&
            (selection.anchorNode.parentNode as HTMLElement).classList.contains(
              'e-pv-text'
            )
          ) {
          ...
          if (!isIE) {
            if (
              this.isBackwardPropagatedSelection ||
              range.endOffset > currentPosition
            ) {
              if (
                this.backwardStart !== range.startOffset &&
                range.startOffset >= currentPosition
              ) {
      ...
      if (
        ...
        annotationModule.textMarkupAnnotationModule.isEnableTextMarkupResizer(
          annotationModule.textMarkupAnnotationModule.currentTextMarkupAddMode
        )
      ) {
        const leftDivElement: HTMLElement = document.getElementById(
          this.pdfViewer.element.id + '_droplet_left'
        );
        if (
          this.pdfViewerBase.isSelection &&
          selection &&
          ...
          this.pdfViewer.annotation.textMarkupAnnotationModule.updateLeftposition(
            left,
            top
          );
        ...
        this.pdfViewer.annotation.textMarkupAnnotationModule.updatePosition(
          x,
          y
        );
      }
      ...
      for (let i: number = 0; i < targetElement.childNodes.length; i++) {
        if (
          targetElement.childNodes[parseInt(i.toString(), 10)].nodeType ===
          targetElement.TEXT_NODE
        ) {
          const range: Range = this.getSelectionRange(i, targetElement);
          const rangeBounds: ClientRect = range.getBoundingClientRect();
          if (
            rangeBounds.left <= x &&
            rangeBounds.right >= parseInt(x.toString(), 10) &&
            parseInt(rangeBounds.top.toString(), 10) <= y &&
            rangeBounds.bottom >= y
          ) {
            range.detach();
            this.textSelectionOnMouseMove(
              targetElement.childNodes[parseInt(i.toString(), 10)],
              x,
              y,
              isExtended
            );
        ...
        if (
          ...
          if (isforward) {
            if (
              selection.anchorNode !== null &&
              (
                selection.anchorNode.parentNode as HTMLElement
              ).classList.contains('e-pv-text')
            ) {
              range.setStart(selection.anchorNode, selection.anchorOffset);
            }
            selection.removeAllRanges();
  ...
  public selectTextRegion(pageNumbers: number, bounds: IRectangle[]): void {
    ...
    for (let k: number = 0; k < bounds.length; k++) {
      ...
      const x: number =
        (bound.left ? bound.left : bound.Left) *
        this.pdfViewerBase.getZoomFactor();
      const y: number =
        (bound.top ? bound.top : bound.Top) *
        this.pdfViewerBase.getZoomFactor();
      const width: number =
        (bound.width ? bound.width : bound.Width) *
        this.pdfViewerBase.getZoomFactor();
      const textLayer: HTMLElement = this.pdfViewerBase.getElement(
        '_textLayer_' + pageNumber
      );
      if (textLayer) {
        const textDivs: any = textLayer.childNodes;
        for (let n: number = 0; n < textDivs.length; n++) {
          if (textDivs[parseInt(n.toString(), 10)]) {
            const rangebounds: any =
              textDivs[parseInt(n.toString(), 10)].getBoundingClientRect();
            const top: number = this.getClientValueTop(
              rangebounds.top,
              pageNumber
            );
            const currentLeft: number =
              rangebounds.left -
              this.pdfViewerBase
                .getElement('_pageDiv_' + pageNumber)
                .getBoundingClientRect().left;
            const totalLeft: number = currentLeft + rangebounds.width;
            const textDiVLeft: number = parseInt(
              textDivs[parseInt(n.toString(), 10)].style.left,
              10
            );
            const currentTop: number = parseInt(
              textDivs[parseInt(n.toString(), 10)].style.top,
              10
            );
            const isLeftBounds: boolean = this.checkLeftBounds(
              currentLeft,
              textDiVLeft,
              totalLeft,
              x
            );
            const isTopBounds: boolean = this.checkTopBounds(
              top,
              currentTop,
              y
            );
        ...
        if (element != null) {
          const boundingRect: any = this.pdfViewerBase
            .getElement('_textLayer_' + pageNumber)
  ...
  public checkLeftBounds(
    ...
    if (
      left === parseInt(x.toString(), 10) ||
      parseInt(left.toString(), 10) === parseInt(x.toString(), 10) ||
      left + 1 === parseInt(x.toString(), 10) ||
      left - 1 === parseInt(x.toString(), 10) ||
      textDiVLeft === parseInt(x.toString(), 10) ||
      textDiVLeft === x ||
      (totalLeft >= x && left <= x)
    ) {
      isExists = true;
  ...
  public checkTopBounds(top: number, currentTop: number, y: number): boolean {
    let isExists: boolean = false;
    if (
      top === parseInt(y.toString(), 10) ||
      parseInt(top.toString(), 10) === parseInt(y.toString(), 10) ||
      parseInt((top + 1).toString(), 10) === parseInt(y.toString(), 10) ||
      parseInt((top - 1).toString(), 10) === parseInt(y.toString(), 10) ||
      currentTop === parseInt(y.toString(), 10) ||
      currentTop === y
    ) {
      isExists = true;
    }
  ...
  private scrollForwardOnSelection(): void {
    if (!this.pdfViewerBase.isSignInitialClick) {
      ...
      this.pdfViewerBase.viewerContainer.scrollTop =
        this.pdfViewerBase.viewerContainer.scrollTop + 200;
  ...
  public selectAWord(
    ...
    if (element.nodeType === element.TEXT_NODE) {
      ...
      while (currentPosition < endPosition) {
        ...
        const rangeBounds: ClientRect = range.getBoundingClientRect();
        if (
          rangeBounds.left <= x + padding &&
          rangeBounds.right >= x - padding &&
          rangeBounds.top <= y + padding &&
          rangeBounds.bottom >= y - padding
        ) {
          ...
          for (let j: number = 0; j < indices.length; j++) {
            ...
            if (
              currentPosition > indices[parseInt(j.toString(), 10)] &&
              currentPosition < indices[j + 1]
            ) {
              startPosition = indices[parseInt(j.toString(), 10)];
              endPos = indices[j + 1];
          ...
          selection.addRange(range);
          this.isTextSelection = true;
          const startParent: HTMLElement = isNullOrUndefined(
            range.startContainer.parentElement
          )
            ? (range.startContainer.parentNode as HTMLElement)
            : range.startContainer.parentElement;
          this.selectionStartPage = parseInt(
            startParent.id.split('_text_')[1],
            10
          );
          if (isStoreSelection) {
      ...
      for (let i: number = 0; i < element.childNodes.length; i++) {
        ...
        if (
          ...
          range.detach();
          this.selectAWord(
            element.childNodes[parseInt(i.toString(), 10)],
            x,
            y,
            isStoreSelection
          );
  ...
  private getSelectionRange(index: number, element: HTMLElement): Range {
    ...
    range.selectNodeContents(
      element.childNodes[parseInt(index.toString(), 10)]
    );
    return range;
  }
  ...
  public selectEntireLine(event: MouseEvent): void {
    ...
    const targetcentre: number = parseInt(
      (targetRect.top + targetRect.height / 2).toString(),
      10
    );
    const pageNumber: number = parseInt(
      (event.target as HTMLElement).id.split('_text_')[1],
      10
    );
    const textDivs: NodeList = document.querySelectorAll(
      'div[id*="' + this.pdfViewer.element.id + '_text_' + pageNumber + '"]'
    );
    if (targetElement.classList.contains('e-pv-text')) {
      ...
      for (let i: number = 0; i < textDivs.length; i++) {
        const rect: ClientRect = (
          textDivs[parseInt(i.toString(), 10)] as HTMLElement
        ).getBoundingClientRect();
        const topValue: number = parseInt(rect.top.toString(), 10);
        const bottomValue: number = parseInt(rect.bottom.toString(), 10);
        if (
          topValue <= targetcentre &&
          bottomValue > targetcentre &&
          targetRect.bottom + 10 > bottomValue
        ) {
          const textId: string = (
            textDivs[parseInt(i.toString(), 10)] as HTMLElement
          ).id;
      ...
      const d2: HTMLElement = document.getElementById(
        textIds[parseInt(lengths.toString(), 10)]
      );
      ...
      this.selectionStartPage = parseInt(
        range.startContainer.parentElement.id.split('_text_')[1],
        10
      );
      selection.addRange(range);
      this.isTextSelection = true;
      if (
        selection != null &&
        this.pdfViewer.contextMenuSettings.contextMenuAction === 'MouseUp'
      ) {
  ...
  public enableTextSelectionMode(): void {
    ...
    if (!isNullOrUndefined(this.pdfViewerBase.viewerContainer)) {
      this.pdfViewerBase.viewerContainer.classList.remove(
        'e-disable-text-selection'
      );
      this.pdfViewerBase.viewerContainer.classList.add(
        'e-enable-text-selection'
      );
      this.pdfViewerBase.viewerContainer.addEventListener(
        'selectstart',
        (e: any) => {
          e.preventDefault();
          return true;
  ...
  public clearTextSelection(): void {
    if (this.isTextSelection) {
      ...
      if (this.pdfViewer.linkAnnotationModule) {
        ...
        higherPageIndex =
          higherPageIndex < this.pdfViewerBase.pageCount - 1
            ? higherPageIndex
            : this.pdfViewerBase.pageCount - 1;
        for (let i: number = lowerPageIndex; i <= higherPageIndex; i++) {
          this.pdfViewer.linkAnnotationModule.modifyZindexForTextSelection(
            i,
            false
          );
        }
  ...
  public removeTouchElements(): void {
    if (this.dropDivElementLeft) {
      this.dropDivElementLeft.parentElement.removeChild(
        this.dropDivElementLeft
      );
      this.dropDivElementLeft = null;
      this.dropElementLeft.style.transform = 'rotate(0deg)';
    }
    if (this.dropDivElementRight) {
      this.dropDivElementRight.parentElement.removeChild(
        this.dropDivElementRight
      );
  ...
  public resizeTouchElements(): void {
    const viewerContainerLeft: number =
      this.pdfViewerBase.viewerContainer.getBoundingClientRect().left;
    if (this.dropDivElementLeft) {
      const elementClientRect: ClientRect =
        this.dropDivElementLeft.getBoundingClientRect();
      let dropElementHeight: number = 0;
      const leftCurrentPagePosition: ClientRect = this.pdfViewerBase
        .getElement('_pageDiv_' + this.topStoreLeft.pageNumber)
        .getBoundingClientRect();
      this.dropDivElementLeft.style.left =
        parseFloat(this.topStoreLeft.left.toString()) *
          this.pdfViewerBase.getZoomFactor() +
        leftCurrentPagePosition.left -
        viewerContainerLeft -
        elementClientRect.width / 2 +
        'px';
      if (this.topStoreLeft.isHeightNeeded) {
        dropElementHeight =
          (elementClientRect.height / 2) * this.pdfViewerBase.getZoomFactor();
      }
      this.dropDivElementLeft.style.top =
        parseFloat(this.topStoreLeft.pageTop.toString()) *
          this.pdfViewerBase.getZoomFactor() +
        parseFloat(this.topStoreLeft.topClientValue.toString()) *
          this.pdfViewerBase.getZoomFactor() +
        dropElementHeight +
        'px';
    }
    if (this.dropDivElementRight) {
      const elementClientRect: ClientRect =
        this.dropDivElementRight.getBoundingClientRect();
      let dropElementHeight: number = 0;
      const rightCurrentPagePosition: ClientRect = this.pdfViewerBase
        .getElement('_pageDiv_' + this.topStoreRight.pageNumber)
        .getBoundingClientRect();
      this.dropDivElementRight.style.left =
        parseFloat(this.topStoreRight.left.toString()) *
          this.pdfViewerBase.getZoomFactor() +
        rightCurrentPagePosition.left -
        viewerContainerLeft -
        elementClientRect.width / 2 +
        'px';
      ...
      this.dropDivElementRight.style.top =
        parseFloat(this.topStoreRight.pageTop.toString()) *
          this.pdfViewerBase.getZoomFactor() +
        parseFloat(this.topStoreRight.topClientValue.toString()) *
          this.pdfViewerBase.getZoomFactor() +
        dropElementHeight +
        'px';
    }
  ...
  public textSelectionOnMouseup(event: MouseEvent): void {
    ...
    if (window.getSelection().anchorNode !== null) {
      ...
      const isTextSearch: boolean =
        this.pdfViewerBase.textLayer.getTextSearchStatus();
      if (isTextSearch) {
        this.pdfViewerBase.textLayer.clearDivSelection();
        const indexes: any = this.pdfViewer.textSearchModule.getIndexes();
        const lowerPageValue: number = parseFloat(
          indexes.lowerPageValue.toString()
        );
        const higherPageValue: number = parseFloat(
          indexes.higherPageValue.toString()
        );
      ...
      if (this.pdfViewer.linkAnnotationModule) {
        this.pdfViewer.linkAnnotationModule.modifyZindexForTextSelection(
          this.pdfViewerBase.currentPageNumber - 1,
          false
        );
      }
      if (
  ...
  public fireTextSelectEnd(): void {
    if (this.selectionRangeArray.length !== 0) {
      ...
      for (let k: number = 0; k < this.selectionRangeArray.length; k++) {
        selectedText +=
          this.selectionRangeArray[parseInt(k.toString(), 10)].textContent;
        for (
          let j: number = 0;
          j <
          this.selectionRangeArray[parseInt(k.toString(), 10)].rectangleBounds
            .length;
          j++
        ) {
          const currentBound: IRectangle =
            this.selectionRangeArray[parseInt(k.toString(), 10)]
              .rectangleBounds[parseInt(j.toString(), 10)];
          selectedBounds.push({
            ...
            pageIndex:
              this.selectionRangeArray[parseInt(k.toString(), 10)].pageNumber +
              1,
          });
        }
        if (
          this.selectionRangeArray[parseInt(k.toString(), 10)].isBackward &&
          k === 0
        ) {
          selectEndPageIndex =
            this.selectionRangeArray[parseInt(k.toString(), 10)].pageNumber + 1;
        } else if (
          !this.selectionRangeArray[parseInt(k.toString(), 10)].isBackward &&
          k === this.selectionRangeArray.length - 1
        ) {
          selectEndPageIndex =
            this.selectionRangeArray[parseInt(k.toString(), 10)].pageNumber + 1;
        }
      }
      this.pdfViewer.fireTextSelectionEnd(
        selectEndPageIndex,
        selectedText,
        selectedBounds
      );
   ...
   * @returns {void}
   */
  public maintainSelectionOnZoom(
    ...
    if (
      ...
      (!selection.type && !selection.isCollapsed)
    ) {
      const isBackward: boolean =
        this.pdfViewerBase.textLayer.isBackWardSelection(selection);
      if (selection.anchorNode != null) {
        const anchorPageId: number = parseInt(
          this.getNodeElementFromNode(selection.anchorNode).id.split(
            '_text_'
          )[1],
          10
        );
        let focusPageId: number = parseInt(
          this.getNodeElementFromNode(selection.focusNode).id.split(
            '_text_'
          )[1],
          10
        );
        if (this.isTouchSelection && isNaN(focusPageId)) {
          ...
          if (focusElement === this.pdfViewerBase.pageContainer) {
            const lastChildNode: HTMLElement = this.pdfViewerBase.pageContainer
              .lastChild as HTMLElement;
            if (lastChildNode.classList.contains('e-pv-touch-select-drop')) {
              focusPageId = parseInt(
                (
                  lastChildNode.previousSibling.previousSibling as HTMLElement
                ).id.split('_pageDiv_')[1],
                10
              );
            } else if (lastChildNode.classList.contains('e-pv-page-div')) {
              focusPageId = parseInt(
                (lastChildNode as HTMLElement).id.split('_pageDiv_')[1],
                10
              );
            }
          }
  ...
  private applySelectionRange(pageNumber: number): void {
    const selectionObject: {
      ...
      selectionRange: ISelection;
    } = this.getSelectionRangeFromArray(pageNumber);
    const isSelectionAvailable: boolean = selectionObject.isSelectionAvailable;
    const textLayer: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_textLayer_' + pageNumber
    );
    if (textLayer) {
      if (isSelectionAvailable && textLayer.childNodes.length !== 0) {
        const selectionRange: ISelection = selectionObject.selectionRange;
        let anchorOffsetDiv: number;
        let focusOffsetDiv: number;
        let anchorOffset: number;
        let focusOffset: number;
        if (selectionRange.isBackward) {
          const startId: number = parseInt(
            selectionRange.endNode.split('_text_')[1].split('_')[1],
            10
          );
          const endId: number = parseInt(
            selectionRange.startNode.split('_text_')[1].split('_')[1],
            10
          );
          if (startId < endId) {
          ...
          anchorOffsetDiv = parseInt(
            selectionRange.startNode.split('text_')[1].split('_')[1],
            10
          );
          focusOffsetDiv = parseInt(
            selectionRange.endNode.split('text_')[1].split('_')[1],
            10
          );
  ...
  private applySelectionMouseScroll(pageNumber: number): void {
    ...
    if (isSelectionAvailable) {
      const selectionRange: ISelection = selectionObject.selectionRange;
      const selection: Selection = window.getSelection();
      const anchorNode: Node = document.getElementById(selectionRange.startNode)
        .childNodes[0];
      const focusNode: Node = document.getElementById(selectionRange.endNode)
        .childNodes[0];
      ...
      if (selection.anchorNode === null) {
        ...
        let anchorPageIndex: number = isNaN(
          parseInt(selection.anchorNode.parentElement.id.split('_text_')[1], 10)
        )
          ? parseInt(
              (selection.anchorNode as HTMLElement).id.split('_pageDiv_')[1],
              10
            )
          : parseInt(
              selection.anchorNode.parentElement.id.split('_text_')[1],
              10
            );
        if (isNaN(anchorPageIndex)) {
          anchorPageIndex = parseInt(
            (selection.anchorNode as HTMLElement).id.split('_text_')[1],
            10
          );
        }
        const focusPageIndex: number = isNaN(
          parseInt(selection.focusNode.parentElement.id.split('_text_')[1], 10)
        )
          ? parseInt(
              (selection.focusNode as HTMLElement).id.split('_pageDiv_')[1],
              10
            )
          : parseInt(
              selection.focusNode.parentElement.id.split('_text_')[1],
              10
            );
        const currentAnchorIndex: number = parseInt(
          selectionRange.startNode.split('_text_')[1],
          10
        );
        if (
          anchorPageIndex === focusPageIndex &&
          anchorPageIndex === currentAnchorIndex
        ) {
          if (!selectionRange.isBackward) {
            range.setStart(anchorNode, selectionRange.startOffset);
            range.setEnd(focusNode, selectionRange.endOffset);
          ...
          if (!isNaN(anchorPageIndex) && !selectionRange.isBackward) {
            if (
              anchorPageIndex < currentAnchorIndex &&
              currentAnchorIndex < focusPageIndex &&
              anchorPageIndex !== focusPageIndex
            ) {
              range.setStart(selection.anchorNode, selection.anchorOffset);
              range.setEnd(selection.focusNode, selection.focusOffset);
            ...
            if (
              ...
              if (!isBackward) {
                if (
                  currentAnchorIndex !== this.selectionRangeArray[0].pageNumber
                ) {
                  range.setStart(selection.anchorNode, selection.anchorOffset);
                  range.setEnd(focusNode, selectionRange.endOffset);
                } else {
              ...
              const currentAnchorOffset: number = parseInt(
                selectionRange.startNode.split(
                  '_' + currentAnchorIndex + '_'
                )[1],
                10
              );
              const currentFocusOffset: number = parseInt(
                selectionRange.endNode.split('_' + currentAnchorIndex + '_')[1],
                10
              );
              if (isBackward) {
                if (
  ...
  public maintainSelectionOnScroll(pageNumber: number, isStich: boolean): void {
    const isSelectionAvailable: boolean =
      this.isSelectionAvailableOnScroll(pageNumber);
  ...
  public maintainSelection(pageNumber: number, isStich: boolean): void {
    ...
    if (
      ...
      const anchorPageId: number = parseInt(
        this.getNodeElementFromNode(selection.anchorNode).id.split('_text_')[1],
        10
      );
      let focusPageId: number = parseInt(
        this.getNodeElementFromNode(selection.focusNode).id.split('_text_')[1],
        10
      );
      if (isNaN(focusPageId) && selection.anchorNode !== null) {
        ...
        if (!backward) {
          ...
          focusPageId = parseInt(
            (
              this.pdfViewerBase.pageContainer.firstChild as HTMLElement
            ).id.split('_pageDiv_')[1],
            10
          );
        }
      ...
      if (
        ...
        pageNumber > focusPageId &&
        pageNumber > anchorPageId
      ) {
        return;
      }
      if (anchorPageId === focusPageId) {
        let selectionObject: ISelection = null;
        const selectionBounds: IRectangle = this.getSelectionBounds(
          selection.getRangeAt(0),
          pageNumber
        );
        const selectionRectBounds: IRectangle[] =
          this.getSelectionRectangleBounds(selection.getRangeAt(0), pageNumber);
        const anchorOffsetValue: number =
          this.getNodeElementFromNode(selection.anchorNode).childNodes
            .length === 1
            ? selection.anchorOffset
            : this.getCorrectOffset(
                selection.anchorNode,
                selection.anchorOffset
              );
        const focusOffsetValue: number =
          this.getNodeElementFromNode(selection.focusNode).childNodes.length ===
          1
            ? selection.focusOffset
            : this.getCorrectOffset(selection.focusNode, selection.focusOffset);
        selectionObject = {
          isBackward: backward,
        ...
        const selectionObject: ISelection = this.createRangeObjectOnScroll(
          pageNumber,
          anchorPageId,
          focusPageId
        );
  ...
  private getCorrectOffset(node: Node, offset: number): number {
    ...
    for (let i: number = 0; i < parentElement.childNodes.length; i++) {
      if (parentElement.childNodes[parseInt(i.toString(), 10)] === node) {
        ...
        offsetValue =
          offsetValue +
          parentElement.childNodes[parseInt(i.toString(), 10)].textContent
            .length;
      }
    }
    return offsetValue;
  ...
  private pushSelectionRangeObject(
    ...
    if (this.isTouchSelection) {
      ...
      if (currentObject.length > 0) {
        const currentObjectIndex: number = this.selectionRangeArray.indexOf(
          currentObject[0]
        );
    ...
    if (nextPageObject.length === 0) {
      if (this.isTouchSelection && this.selectionRangeArray.length !== 0) {
        ...
        if (prevPageObject.length !== 0) {
          const prevIndex: number = this.selectionRangeArray.indexOf(
            prevPageObject[0]
          );
  ...
  private extendCurrentSelection(
    ...
    const focusPageId: number = isNaN(
      parseInt(currentFocusElement.split('_text_')[1], 10)
    )
      ? parseInt(
          (selection.focusNode as HTMLElement).id.split('_pageDiv_')[1],
          10
    ...
    if (isNaN(parseInt(currentFocusElement.split('_text_')[1], 10))) {
      currentFocusElement = (
        this.pdfViewerBase.getElement('_textLayer_' + (focusPageId + 1))
    ...
    selection.addRange(range);
    selection.extend(
      document.getElementById(currentFocusElement).childNodes[0],
      currentFocusOffset
    );
  }
  ...
  private stichSelection(
    ...
    if (backward) {
      nextPageElement = document.getElementById(
        this.pdfViewer.element.id + '_textLayer_' + (pageNumber - 1)
      );
      if (nextPageElement) {
        ...
        if (lastElement) {
          this.extendCurrentSelection(
            lastElement,
            this.getTextLastLength(lastElement),
            selection,
            range
          );
        } else {
          nextPageElement = document.getElementById(
            this.pdfViewer.element.id + '_textLayer_' + (pageNumber - 2)
          );
        ...
        const lastElement: HTMLElement =
          nextPageElement.firstChild as HTMLElement;
  ...
  public stichSelectionOnScroll(currentPageNumber: number): void {
    ...
    if (this.isTextSelection) {
      ...
      const focusPageId: number = parseInt(
        this.getNodeElementFromNode(selection.focusNode).id.split('_text_')[1],
        10
      );
      let nextPageElement: HTMLElement;
      if (
        anchorPageId !== currentPageNumber &&
        focusPageId !== currentPageNumber
      ) {
        const backward: boolean =
          this.pdfViewerBase.textLayer.isBackWardSelection(selection);
        if (!backward) {
          nextPageElement = document.getElementById(
            this.pdfViewer.element.id + '_textLayer_' + (currentPageNumber - 1)
          );
          if (nextPageElement) {
            const lastElement: HTMLElement =
              nextPageElement.lastChild as HTMLElement;
            if (lastElement) {
              if (lastElement.childNodes[0]) {
                this.extendSelectionStich(
                  lastElement.childNodes[0],
                  this.getTextLastLength(lastElement),
                  selection
                );
              } else {
              ...
              nextPageElement = this.pdfViewerBase.getElement(
                '_textLayer_' + currentPageNumber
              );
              const lastElement: HTMLElement =
                nextPageElement.firstChild as HTMLElement;
              this.extendSelectionStich(
   ...
   */
  public createRangeObjectOnScroll(
    pageNumber: number,
    anchorPageId: number,
    focusPageId: number
  ): ISelection {
      ...
      if (element.childNodes) {
        if (!backward) {
          if (pageNumber === anchorPageId) {
            ...
            startOffset = this.getCorrectOffset(
              selection.anchorNode,
              selection.anchorOffset
            );
            ...
            firstElement = element.firstChild as HTMLElement;
            const pageNumberIndex: number = this.getNodeElementFromNode(
              selection.focusNode
            ).id.indexOf(focusPageId.toString());
            if (pageNumberIndex !== -1) {
              lastElement = this.getNodeElementFromNode(selection.focusNode);
              endOffset = this.getCorrectOffset(
                selection.focusNode,
                selection.focusOffset
              );
            } else {
              lastElement = document.getElementById(
                this.pdfViewer.element.id + '_textLayer_' + focusPageId
              ).lastChild as HTMLElement;
              endOffset = this.getTextLastLength(lastElement);
            }
            startOffset = 0;
        ...
        if (firstElement && lastElement) {
          const selectionRangeObject: Range = this.getSelectionRangeObject(
            firstElement.id,
            startOffset,
            lastElement.id,
            endOffset,
          ...
          const selectionBound: IRectangle = this.getSelectionBounds(
            selectionRangeObject,
            pageNumber
          );
          const selectionRectBounds: IRectangle[] =
            this.getSelectionRectangleBounds(selectionRangeObject, pageNumber);
  ...
  private getSelectionRangeObject(
    ...
    const currentAnchorOffset: number = parseInt(
      startNode.split('_' + pageNumber + '_')[1],
      10
    );
    const currentFocusOffset: number = parseInt(
      endNode.split('_' + pageNumber + '_')[1],
      10
    );
    const range: Range = document.createRange();
    if (currentAnchorOffset <= currentFocusOffset) {
  ...
  private getSelectionBounds(range: Range, pageNumber: number): IRectangle {
    const startElement: HTMLElement = this.getNodeElementFromNode(
      range.startContainer
    );
    const endElement: HTMLElement = this.getNodeElementFromNode(
      range.endContainer
    );
    let bounds: IRectangle = null;
    if (startElement !== endElement) {
      const newStartRange: Range = document.createRange();
      const startRange: Range = this.createRangeForSelection(
        range.startContainer,
        range.endContainer,
        range.startOffset,
        range.endOffset,
  ...
  private getSelectionRectangleBounds(
    ...
    if (startElement !== endElement) {
      ...
      const anchorPageId: number = this.pdfViewerBase.textLayer.getPageIndex(
        range.startContainer
      );
      const anchorTextId: number = this.pdfViewerBase.textLayer.getTextIndex(
        range.startContainer,
        anchorPageId
      );
      const focusPageId: number = this.pdfViewerBase.textLayer.getPageIndex(
        range.endContainer
      );
      const focusTextId: number = this.pdfViewerBase.textLayer.getTextIndex(
        range.endContainer,
        focusPageId
      );
      const textDivs: NodeList = this.pdfViewerBase.getElement(
        '_textLayer_' + focusPageId
      ).childNodes;
      if (pageNumber === anchorPageId) {
      ...
      for (let j: number = currentId; j < textDivs.length; j++) {
        const textElement: HTMLElement = textDivs[
        ...
        if (startOffset !== 0 || endOffset !== 0) {
          ...
          for (let k: number = 0; k < textElement.childNodes.length; k++) {
            const node: Node =
              textElement.childNodes[parseInt(k.toString(), 10)];
            newRange.setStart(node, startOffset);
            newRange.setEnd(node, endOffset);
          }
          let boundingRect: IRectangle;
          if (this.pdfViewerBase.clientSideRendering) {
            boundingRect = this.normalizeBounds(
            ...
            if (textElement && textElement.style.transform !== '') {
              if (textElement.style.transform.startsWith('rotate(90deg)')) {
                ...
                textElement.style.transform.startsWith('rotate(180deg)')
              ) {
                textRotate = 180;
              } else if (
                textElement.style.transform.startsWith('rotate(-90deg)') ||
                textElement.style.transform.startsWith('rotate(270deg)')
              ) {
                textRotate = 270;
              } else {
                textRotate = 0;
      ...
      for (let i: number = 0; i < selectionTexts.length; i++) {
        ...
        if (
          (i !== 0 && text === ' ' && selectionTexts[i - 1].includes('\r\n')) ||
          (i !== selectionTexts.length - 1 &&
            selectionTexts[parseInt(i.toString(), 10)] === ' ' &&
            selectionTexts[i + 1] === '\r\n')
        ) {
          text = '';
        }
      ...
      if (this.pdfViewerBase.clientSideRendering) {
        ...
        if (startElement && startElement.style.transform !== '') {
          if (startElement.style.transform.startsWith('rotate(90deg)')) {
            ...
            startElement.style.transform.startsWith('rotate(180deg)')
          ) {
            textRotate = 180;
          } else if (
            startElement.style.transform.startsWith('rotate(-90deg)') ||
            startElement.style.transform.startsWith('rotate(270deg)')
          ) {
            textRotate = 270;
          } else {
            textRotate = 0;
  ...
  private normalizeBounds(bound: ClientRect, pageNumber: number): IRectangle {
    ...
    let currentPageElement: HTMLElement = this.pdfViewerBase.getElement(
      '_pageDiv_' + pageNumber
    );
    if (this.pdfViewerBase.isMixedSizeDocument) {
      const currentTextElement: HTMLElement = this.pdfViewerBase.getElement(
        '_textLayer_' + pageNumber
      );
    ...
    const currentPageRect: ClientRect =
  ...
  private maintainSelectionArray(): void {
    if (this.selectionRangeArray.length !== 0) {
      ...
      if (isNaN(focusPage)) {
        focusPage = isNaN(
          parseInt(
            (selection.focusNode as HTMLElement).id.split('_text_')[1],
            10
          )
        )
          ? parseInt(
              (selection.focusNode as HTMLElement).id.split('_textLayer_')[1],
              10
            )
          : parseInt(
              (selection.focusNode as HTMLElement).id.split('_text_')[1],
      ...
      if (!isBackward) {
        arrayObject = this.selectionRangeArray.filter((obj: any) => {
          return !(
            this.selectionStartPage <= obj.pageNumber &&
            obj.pageNumber < focusPage
          );
        });
      } else {
        arrayObject = this.selectionRangeArray.filter((obj: any) => {
          return !(
            focusPage < obj.pageNumber &&
            obj.pageNumber <= this.selectionStartPage
          );
        });
      }
      if (arrayObject.length > 0) {
        for (let i: number = 0; i < arrayObject.length; i++) {
          const indexInArray: number = this.selectionRangeArray.indexOf(
            arrayObject[parseInt(i.toString(), 10)]
          );
          if (indexInArray !== -1) {
        ...
        if (this.selectionRangeArray.length === 1) {
          if (
            this.selectionRangeArray[0].pageNumber === anchorPage ||
            this.selectionRangeArray[0].pageNumber === focusPage
          ) {
            arrayObject = [];
          }
        }
  ...
  public applySpanForSelection(): void {
    ...
    if (
      selection.anchorNode !== null &&
      this.pdfViewerBase.viewerContainer.contains(
        this.getNodeElementFromNode(selection.anchorNode)
      )
    ) {
      const isBackWardSelection: boolean =
        this.pdfViewerBase.textLayer.isBackWardSelection(selection);
      let anchorPageId: number;
      let focusPageId: number;
      let anchorOffsetDiv: number;
      let focusOffsetDiv: number;
      let anchorOffset: number;
      let focusOffset: number;
      if (isBackWardSelection) {
        anchorPageId = parseInt(
          this.getNodeElementFromNode(selection.focusNode).id.split(
            '_text_'
          )[1],
          10
        ...
        anchorOffsetDiv = parseInt(
          this.getNodeElementFromNode(selection.focusNode)
            .id.split('_text_')[1]
            .split('_')[1],
          10
        );
        ...
        let anchorElement: HTMLElement = this.getNodeElementFromNode(
          selection.anchorNode
        );
        let focusElement: HTMLElement = this.getNodeElementFromNode(
          selection.focusNode
        );
        anchorPageId =
          anchorElement.id.indexOf('text_') !== -1
            ? parseInt(anchorElement.id.split('text_')[1], 10)
            : parseInt(anchorElement.id.split('_textLayer_')[1], 10);
        focusPageId =
          focusElement.id.indexOf('text_') !== -1
            ? parseInt(focusElement.id.split('text_')[1], 10)
            : parseInt(focusElement.id.split('_textLayer_')[1], 10);
        let isFocusChanged: boolean = false;
        if (this.isTouchSelection) {
          if (
            (selection.focusNode as HTMLElement) ===
            this.pdfViewerBase.pageContainer
          ) {
            const lastChildNode: HTMLElement = this.pdfViewerBase.pageContainer
              .lastChild as HTMLElement;
            if (lastChildNode.classList.contains('e-pv-touch-select-drop')) {
              const lastPageDiv: HTMLElement = lastChildNode.previousSibling
                .previousSibling as HTMLElement;
              focusPageId = parseInt(lastPageDiv.id.split('_pageDiv_')[1], 10);
              focusElement = this.pdfViewerBase.getElement(
                '_textLayer_' + focusPageId
              ).lastChild as HTMLElement;
              isFocusChanged = true;
            } else if (lastChildNode.classList.contains('e-pv-page-div')) {
        ...
        anchorOffsetDiv = anchorElement.id.split('text_')[1]
          ? parseInt(anchorElement.id.split('text_')[1].split('_')[1], 10)
          : null;
        focusOffsetDiv = focusElement.id.split('text_')[1]
          ? parseInt(focusElement.id.split('text_')[1].split('_')[1], 10)
          : null;
        anchorOffsetDiv = isNaN(anchorOffsetDiv)
          ? focusOffsetDiv
          : anchorOffsetDiv;
        focusOffsetDiv = isNaN(focusOffsetDiv)
          ? anchorOffsetDiv
          : focusOffsetDiv;
        anchorOffset = selection.anchorOffset;
        focusOffset = !isFocusChanged
          ? selection.focusOffset
          : focusElement.textContent.length;
      }
  ...
  public initiateTouchSelection(event: TouchEvent, x: number, y: number): void {
    ...
    const belowElements: Element[] = document.elementsFromPoint(
      event.touches[0].clientX,
      event.touches[0].clientY
    );
    if (belowElements.length !== 0) {
      if (
        belowElements[0].classList.contains('e-pv-hyperlink') &&
        belowElements[1].classList.contains('e-pv-text')
      ) {
      ...
      while (currentPosition < endPosition) {
        ...
        if (
          ...
          if (selection.anchorNode !== null) {
            ...
            rangeObject = this.setTouchSelectionStartPosition(
              selection,
              rangeObject,
              isForwardSelection,
              target,
  ...
  private getTouchAnchorElement(
    ...
    const element: HTMLElement = document.getElementById(
      this.selectionAnchorTouch.anchorNode.toString()
    );
  ...
  private getTouchFocusElement(
    ...
    const element: HTMLElement = document.getElementById(
      this.selectionFocusTouch.focusNode.toString()
    );
    let startNode: Node = null;
    let offset: number = 0;
    if (element) {
  ...
  private createTouchSelectElement(event: TouchEvent): void {
    ...
    if (selection.type === 'Range') {
      ...
      const dropElementRect: ClientRect =
        this.dropDivElementLeft.getBoundingClientRect();
      const pageTopValue: number =
        this.pdfViewerBase.pageSize[this.pdfViewerBase.currentPageNumber - 1]
          .top;
      const viewerLeftPosition: number =
        this.pdfViewerBase.viewerContainer.getBoundingClientRect().left;
      const topClientValue: number = this.getClientValueTop(
        rangePosition.top,
        this.pdfViewerBase.currentPageNumber - 1
      );
      const dropElementTop: number =
        this.pdfViewerBase.getZoomFactor() > 2
          ? dropTopAboveTwoHundred
          : this.pdfViewerBase.getZoomFactor() > 1
          ? dropTopAboveHundard
          : 0;
      const topPositionValue: string =
        topClientValue -
        dropElementTop +
        pageTopValue * this.pdfViewerBase.getZoomFactor() +
        (dropElementRect.height / 2) * this.pdfViewerBase.getZoomFactor() +
        'px';
      this.dropDivElementLeft.style.top = topPositionValue;
      this.dropDivElementLeft.style.left =
        rangePosition.left -
        (viewerLeftPosition + dropElementRect.width) +
        this.pdfViewerBase.viewerContainer.scrollLeft +
        'px';
      ...
      this.dropDivElementRight.style.left =
        ...
        this.pdfViewerBase.viewerContainer.scrollLeft +
        'px';
      const currentPageLeft: number = this.pdfViewerBase
        .getElement('_pageDiv_' + (this.pdfViewerBase.currentPageNumber - 1))
        .getBoundingClientRect().left;
      ...
      this.dropDivElementLeft.addEventListener(
        'touchstart',
        this.onLeftTouchSelectElementTouchStart
      );
      this.dropDivElementLeft.addEventListener(
        'touchmove',
        this.onLeftTouchSelectElementTouchMove
      );
      this.dropDivElementLeft.addEventListener(
        'touchend',
        this.onLeftTouchSelectElementTouchEnd
      );
      this.dropDivElementRight.addEventListener(
        'touchstart',
        this.onRightTouchSelectElementTouchStart
      );
      this.dropDivElementRight.addEventListener(
        'touchmove',
        this.onRightTouchSelectElementTouchMove
      );
      this.dropDivElementRight.addEventListener(
        'touchend',
        this.onRightTouchSelectElementTouchEnd
      );
      this.calculateContextMenuPosition(
        event.touches[0].clientY +
          this.dropDivElementLeft.clientHeight +
          topMargin,
  ...
  public calculateContextMenuPosition(top: any, left: any): void {
    ...
    setTimeout(() => {
      ...
      const selectedContent: any = document.getElementsByClassName(
        'e-pv-maintaincontent'
      )[0]
        ? document
            .getElementsByClassName('e-pv-maintaincontent')[0]
            .getBoundingClientRect()
        : null;
      if (selectedContent) {
        if (
          selectedContent.bottom +
            proxy.contextMenuHeight +
            proxy.pdfViewerBase.toolbarHeight >
          window.innerHeight
        ) {
          top =
            selectedContent.top -
        ...
        if (
          ...
          proxy.pdfViewerBase.contextMenuModule.open(
            top,
            left,
            proxy.pdfViewerBase.viewerContainer
          );
        }
      }
    });
  ...
  public initiateSelectionByTouch(): void {
    ...
    if (this.selectionRangeArray.length > 0) {
      this.pdfViewer.fireTextSelectionStart(
        this.selectionRangeArray[0].pageNumber + 1
      );
    }
  }
  ...
  private terminateSelectionByTouch(event: any): void {
    ...
    if (this.pdfViewerBase.getTextMarkupAnnotationMode()) {
      ...
      this.fireTextSelectEnd();
      let top: any =
        event.changedTouches[0].clientY + event.currentTarget.clientHeight;
      const spanBounds: any = this.getSpanBounds();
      if (spanBounds) {
        if (
          spanBounds.bottom +
            this.contextMenuHeight +
            this.pdfViewerBase.toolbarHeight >
          window.innerHeight
        ) {
          top =
            spanBounds.top -
            (this.contextMenuHeight + this.pdfViewerBase.toolbarHeight);
        } else {
          top =
            spanBounds.bottom + this.pdfViewerBase.toolbarHeight - topMargin;
        }
        this.pdfViewerBase.contextMenuModule.open(
          top,
          (spanBounds.right - spanBounds.left) / 2,
  ...
  private getSpanBounds(): any {
    ...
    const selectedContent: HTMLCollectionOf<Element> =
      document.getElementsByClassName('e-pv-maintaincontent');
    if (selectedContent.length > 0) {
      for (let i: number = 0; i < selectedContent.length; i++) {
        const spanElement: any =
          selectedContent[parseInt(i.toString(), 10)].getBoundingClientRect();
      ...
      return {
        ...
        bottom:
          selectedContent[selectedContent.length - 1].getBoundingClientRect()
            .bottom,
  ...
  private onLeftTouchSelectElementTouchMove = (event: TouchEvent): void => {
    ...
    const isTouchedWithinViewerContainer: boolean =
      this.isTouchedWithinContainer(event);
    if (rightElement && isTouchedWithinViewerContainer) {
      ...
      if (nodeElement) {
        const currentDifference: number = Math.sqrt(
          (yTouch - dropBounds.top) * (yTouch - dropBounds.top) +
            (xTouch - dropBounds.left) * (xTouch - dropBounds.left)
        );
        const isCloserMovement: boolean =
          this.isCloserTouchScroll(currentDifference);
        let isTextSelected: boolean = false;
        ...
        const textHeight: number =
          zoomFactor > 1
            ? zoomFactorabovehundard
            : zoomFactor > 0.7
            ? zoomFactorAboveSeventy
            : zoomFactor > 0.5
            ? zoomFactoraboveFifty
            : zoomFactorbelowFifty;
        if (
          (parseInt(yTouch.toString(), 10) <=
            parseInt(dropBounds.top.toString(), 10) &&
            parseInt(topDifference.toString(), 10) >= textHeight) ||
          (parseInt(xTouch.toString(), 10) <=
            parseInt(dropBounds.left.toString(), 10) &&
            parseInt(topDifference.toString(), 10) <= textHeight)
        ) {
          this.dropElementLeft.style.transform = 'rotate(0deg)';
          this.dropElementRight.style.transform = 'rotate(-90deg)';
          isTextSelected = this.selectTextByTouch(
        ...
        if (isTextSelected) {
          ...
          const currentPageLeft: number = this.pdfViewerBase
            .getElement(
              '_pageDiv_' + (this.pdfViewerBase.currentPageNumber - 1)
            )
            .getBoundingClientRect().left;
  ...
  private onRightTouchSelectElementTouchMove = (event: TouchEvent): void => {
    ...
    if (leftElement && isTouchedWithinViewerContainer) {
      ...
      if (nodeElement) {
        const currentDifference: number = Math.sqrt(
          (touchY - dropPosition.top) * (touchY - dropPosition.top) +
        ...
        const textHeight: number =
          zoomFactor > 1
            ? zoomFactor * zoomFactorabovehundard
            : zoomFactor > 0.7
            ? zoomFactorAboveSeventy
            : zoomFactor > 0.5
            ? zoomFactoraboveFifty
            : zoomFactorbelowFifty;
        if (
          (parseInt(touchY.toString(), 10) >=
            parseInt(dropPosition.top.toString(), 10) &&
            parseInt(topDifference.toString(), 10) >= textHeight) ||
          (parseInt(topDifference.toString(), 10) <= textHeight &&
            parseInt(touchX.toString(), 10) >=
              parseInt(dropPosition.left.toString(), 10))
        ) {
          this.dropElementRight.style.transform = 'rotate(-90deg)';
          this.dropElementLeft.style.transform = 'rotate(0deg)';
          isTextSelected = this.selectTextByTouch(
  ...
  private getNodeElement(
    ...
    if (document.caretRangeFromPoint) {
      ...
      nodeElement = this.onTouchElementScroll(
        range,
        nodeElement,
        touchY,
        event
      ...
      const start: any = (document as any).caretPositionFromPoint(
        touchX,
        touchY
      );
  ...
  private isScrolledOnScrollBar(event: TouchEvent): boolean {
    ...
    if (
      event.touches &&
      this.pdfViewerBase.viewerContainer.clientHeight +
        this.pdfViewerBase.viewerContainer.offsetTop <
        event.touches[0].clientY &&
      event.touches[0].clientY <
        this.pdfViewerBase.viewerContainer.offsetHeight +
          this.pdfViewerBase.viewerContainer.offsetTop
    ) {
      isScrollBar = true;
    }
  ...
  public copyText(): void {
    ...
    if (this.selectionRangeArray.length > 0) {
      for (let i: number = 0; i < this.selectionRangeArray.length; i++) {
        selectionText +=
          this.selectionRangeArray[parseInt(i.toString(), 10)].textContent;
      }
    }
    if (selectionText.length > 0) {
      ...
      if (
        ...
        this.pdfViewer.annotation.freeTextAnnotationModule.selectedText =
          selectionText;
      }
