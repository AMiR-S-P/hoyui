...
export class BookmarkView {
  ...
  public createRequestForBookmarks(): void {
    ...
    if (this.pdfViewerBase.clientSideRendering) {
      const data: any =
        this.pdfViewer.pdfRendererModule.getBookmarks(jsonObject);
  ...
  private renderBookmarksOnSuccess(data: any, proxy: BookmarkView): void {
    if (data) {
      ...
      if (data && data.uniqueId === proxy.pdfViewerBase.documentId) {
        ...
        proxy.bookmarksDestination = {
          bookMarkDestination: data.BookmarksDestination,
        };
        if (isBlazor()) {
          const bookmarkCollection: any = {
            bookmarks: proxy.bookmarks,
            bookmarksDestination: proxy.bookmarksDestination,
          };
          if (proxy.pdfViewer && proxy.pdfViewer._dotnetInstance) {
            proxy.pdfViewer._dotnetInstance.invokeMethodAsync(
  ...
  public renderBookmarkcontent(): void {
    if (!this.isBookmarkViewDiv) {
      ...
      this.bookmarkView = isblazor
        ? this.pdfViewer.element.querySelector('.e-pv-bookmark-view')
        : createElement('div', {
            id: this.pdfViewer.element.id + '_bookmark_view',
            className: 'e-pv-bookmark-view',
          });
      this.pdfViewerBase.navigationPane.sideBarContent.appendChild(
        this.bookmarkView
      );
      const bookmarkIconView: HTMLElement = createElement('div', {
        id: this.pdfViewer.element.id + '_bookmark_iconview',
        className: 'e-pv-bookmark-icon-view',
      });
      if (!this.pdfViewer.enableRtl) {
        const bookmarkIcon: HTMLElement = createElement('span', {
          id: this.pdfViewer.element.id + '_bookmark_icon',
          className: 'e-pv-bookmark-icon e-pv-icon',
        });
        bookmarkIconView.appendChild(bookmarkIcon);
      } else {
        const bookmarkIcon: HTMLElement = createElement('span', {
          id: this.pdfViewer.element.id + '_bookmark_icon',
          className: 'e-pv-bookmark-icon e-pv-icon e-right',
        });
        bookmarkIconView.appendChild(bookmarkIcon);
      }
      const bookmarkTitle: HTMLElement = createElement('div', {
        id: this.pdfViewer.element.id + '_bookmark_Title',
        className: 'e-pv-bookmark-title',
      });
      if (this.pdfViewer.enableRtl) {
      ...
      if (!isblazor) {
        this.treeObj = new TreeView({
          ...
          nodeTemplate: initializeCSPTemplate(function (data: any): string {
            return bookmarkIconView.outerHTML.replace('${Title}', data.Title);
          }),
      ...
      for (let m: number = 0; m < event.length; m++) {
        this.bookmarkView.addEventListener(
          event[parseInt(m.toString(), 10)],
          (event: Event) => {
  ...
  public renderBookmarkContentMobile(): void {
    ...
    this.bookmarkView = createElement('div', {
      id: this.pdfViewer.element.id + '_bookmark_view',
      className: 'e-pv-bookmark-view',
    });
    this.pdfViewerBase
      .getElement('_bookmarks_container')
      .appendChild(this.bookmarkView);
  ...
  private bookmarkClick = (args: any): boolean => {
    if (
      !((args.event as any).target as HTMLElement).classList.contains('e-icons')
    ) {
  ...
  private bookmarkKeypress = (args: any): void => {
    if (
      ...
      (args.event.key === 'Enter' || args.event.key === ' ')
    ) {
  ...
  private bookmarkPanelBeforeOpen = (args: DrawNodeEventArgs): void => {
    if (this.pdfViewer.enableBookmarkStyles) {
      for (let k: number = 0; k < this.bookmarkStyles.length; k++) {
        if (
          args.text.trim() ===
          this.bookmarkStyles[parseInt(k.toString(), 10)].Text.trim()
        ) {
          const element: any = args.node.lastElementChild;
          if (element) {
            const fontStyle: any =
              this.bookmarkStyles[parseInt(k.toString(), 10)].FontStyle.split(
                ','
              );
            ...
            const currentElement: any = element.getElementsByClassName(
              'e-pv-bookmark-title'
            )[0];
            if (currentElement) {
              currentElement.style.color =
                this.bookmarkStyles[parseInt(k.toString(), 10)].Color;
            } else {
  ...
  private setHeight(element: any): void {
    if (this.treeObj) {
      if (this.treeObj.fullRowSelect && element.classList) {
        if (
          element.classList.contains('e-treeview') &&
          element.classList.contains('.e-active')
        ) {
          element = element
            .querySelector('.e-active')
            .querySelector('.e-fullrow');
          ...
          if (element.closest && element.closest('.e-list-item')) {
            ...
            if (element.classList.contains('e-list-item')) {
              ...
              element.classList.contains('e-icons') &&
              element.classList.contains('interaction') &&
              element.parentElement.parentElement.classList.contains(
                'e-list-item'
              )
            ) {
              element =
                element.parentElement.parentElement.querySelector('.e-fullrow');
            }
          }
        }
  ...
  private navigateToBookmark(
    ...
    this.pdfViewer.fireBookmarkClick(
  ...
  public getBookmarks(): any {
    if (this.bookmarks && this.bookmarksDestination) {
      return {
        ...
        bookmarksDestination: this.bookmarksDestination,
      };
    }
  }
  ...
  public goToBookmark(pageIndex: number, y: number): boolean {
    // eslint-disable-next-line
    const proxy: BookmarkView = this;
    const destPage: number =
      this.pdfViewerBase.pageSize[parseInt(pageIndex.toString(), 10)].height;
    let scrollValue: number;
    if (y === 0) {
      scrollValue =
        this.pdfViewerBase.pageSize[parseInt(pageIndex.toString(), 10)].top *
        this.pdfViewerBase.getZoomFactor();
    } else {
      scrollValue =
        this.pdfViewerBase.pageSize[parseInt(pageIndex.toString(), 10)].top *
          this.pdfViewerBase.getZoomFactor() +
        (destPage - y) * this.pdfViewerBase.getZoomFactor();
    }
    ...
    if (
      ...
      this.pdfViewerBase.currentPageNumber !== pageIndex + 1
    ) {
