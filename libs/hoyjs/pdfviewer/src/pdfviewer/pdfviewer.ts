...
import {
  ...
  DocumentTextCollectionSettingsModel,
...
import { PointModel, IElement, Rect, Point, Size } from '@hoyui/drawings';
...
import {
  ...
  FormFieldMouseLeaveArgs,
  FormFieldMouseoverArgs,
  FormFieldMoveArgs,
  FormFieldResizeArgs,
  FormFieldSelectArgs,
  FormFieldUnselectArgs,
  FormFieldDoubleClickArgs,
  AnnotationMovingEventArgs,
  KeyboardCustomCommandsEventArgs,
} from './base';
...
import { ClickEventArgs, MenuItemModel } from '@hoyui/navigations';
  ...
  implements INotifyPropertyChanged
{
  ...
  protected render(): void {
    ...
    if (this.viewerBase.clientSideRendering) {
      ...
      const workerBob: any = new Blob(
        [PdfiumRunner.toString().replace(/^[^{]*{([\s\S]*)}$/m, '$1')],
        { type: 'text/javascript' }
      );
      const workerBlobUrl: any = URL.createObjectURL(workerBob);
      (window as any)['pdfViewerRunner_' + this.element.id] =
        this.viewerBase.pdfViewerRunner = new Worker(workerBlobUrl);
  ...
  public exportAsImages(
    ...
    if (!isNullOrUndefined(size)) {
      imageDeatils = this.pdfRendererModule.exportAsImagesWithSize(
        startIndex,
        endIndex,
        size
      );
    } else {
      imageDeatils = this.pdfRendererModule.exportAsImages(
        startIndex,
        endIndex
      );
    }
  ...
  private getScriptPathForPlatform(): string {
    ...
    if (
      ...
      ((this as any).parent && (this as any).parent.isAngular)
    ) {
      return baseUrl + '/assets/ej2-pdfviewer-lib';
    } else if (
      (this as any).isReact ||
      ((this as any).parent && (this as any).parent.isReact)
    ) {
      return baseUrl + '/ej2-pdfviewer-lib';
    } else if (
      (this as any).isVue ||
      ((this as any).parent && (this as any).parent.isVue) ||
      (this as any).isVue3 ||
      ((this as any).parent && (this as any).parent.isVue3)
    ) {
      ...
      const path: string = scriptLinkURL.replace(
  ...
  public onPropertyChanged(
    newProp: PdfViewerModel,
    oldProp: PdfViewerModel
  ): void {
    ...
    for (const prop of properties) {
      switch (prop) {
        ...
        case 'enableCommentPanel':
          this.notify('', {
            module: 'annotation',
            enable: this.enableCommentPanel,
          });
          requireRefresh = true;
          if (
            this.toolbarModule &&
            this.toolbarModule.annotationToolbarModule
          ) {
            this.toolbarModule.annotationToolbarModule.enableCommentPanelTool(
        ...
        case 'documentPath':
          if (!this.viewerBase.isSkipDocumentPath) {
            if (!isBlazor()) {
              ...
              this._dotnetInstance.invokeMethodAsync(
                'LoadDocumentFromClient',
                newProp.documentPath
              );
            }
          } else {
        ...
        case 'customStampSettings':
          if (newProp.customStampSettings.customStamps) {
            for (
              let i: number = 0;
              i < newProp.customStampSettings.customStamps.length;
              i++
            ) {
              this.viewerBase.customStampCollection.push({
                customStampName:
                  this.customStampSettings.customStamps[
        ...
        case 'strikethroughSettings':
          if (
            ...
            this.annotationModule.textMarkupAnnotationModule
          ) {
            this.annotationModule.textMarkupAnnotationModule.updateTextMarkupSettings(
              prop
            );
          }
          break;
        ...
        case 'initialFieldSettings':
          if (this.formDesignerModule) {
            ...
            this.formDesignerModule.updateSignatureSettings(
              newProp[prop],
              isInitialField
            );
          }
        ...
        case 'radioButtonFieldSettings':
          if (this.formDesignerModule) {
            // eslint-disable-next-line security/detect-object-injection
            this.formDesignerModule.updateRadioButtonFieldSettings(
              newProp[prop]
            );
          }
          break;
        ...
        case 'isFormDesignerToolbarVisible':
          if (!Browser.isDevice || this.enableDesktopMode) {
            if (
              this.toolbarModule &&
              this.formDesignerModule &&
              !oldProp.isFormDesignerToolbarVisible &&
              newProp.isFormDesignerToolbarVisible
            ) {
              if (
                this.toolbarModule.annotationToolbarModule &&
                this.isAnnotationToolbarVisible
              ) {
              ...
              if (
                ...
                !isNullOrUndefined(this.formDesignerModule) &&
                this.toolbarModule.formDesignerToolbarModule &&
                !this.isFormDesignerToolbarVisible
              ) {
        ...
        case 'isAnnotationToolbarVisible':
          if (!Browser.isDevice || this.enableDesktopMode) {
            if (
              this.toolbarModule &&
              this.annotationModule &&
              !oldProp.isAnnotationToolbarVisible &&
        ...
        case 'serviceUrl':
          if (
            isNullOrUndefined(newProp.serviceUrl) ||
            newProp.serviceUrl === ''
          ) {
            this.viewerBase.clientSideRendering = true;
          } else {
  ...
  private renderCustomerStamp(customStamp: any): void {
    ...
    this.annotation.stampAnnotationModule.createCustomStampAnnotation(
      customStamp.customStampImageSource,
  ...
  public requiredModules(): ModuleDeclaration[] {
    ...
    if (this.enableMagnification) {
      modules.push({
        member: 'Magnification',
        args: [this, this.viewerBase],
      });
    }
    ...
    if (
      this.enableToolbar ||
      this.enableNavigationToolbar ||
      this.enableAnnotationToolbar ||
      this.enableFormDesignerToolbar
    ) {
      modules.push({
        member: 'Toolbar',
        args: [this, this.viewerBase],
    ...
    if (this.enableHyperlink) {
      modules.push({
        member: 'LinkAnnotation',
        args: [this, this.viewerBase],
      });
    }
    if (this.enableThumbnail) {
      modules.push({
        member: 'ThumbnailView',
        args: [this, this.viewerBase],
      });
    }
  ...
  public defaultLocale: Object = {
    ...
    GoToPage: 'Go to Page',
    'No Matches':
      'PDF Viewer has finished searching the document. No matches were found.',
    'No More Matches':
      'PDF Viewer has finished searching the document. No more matches were found.',
    ...
    'Import PDF Failed':
      'Invalid PDF file type or PDF file not found. Please select a valid PDF file',
  ...
  public load(document: string | Uint8Array, password: string): void {
    ...
    if (
      this.viewerBase.documentPathByteArray instanceof Uint8Array &&
      !this.viewerBase.clientSideRendering
    ) {
      const base64String: string = this.convertByteArrayToBase64(
        this.viewerBase.documentPathByteArray
      );
  ...
  private convertByteArrayToBase64(byteArray: Uint8Array): string {
    ...
    for (let i: number = 0; i < byteArrayLength; i++) {
      binaryString += String.fromCharCode(
        byteArray[parseInt(i.toString(), 10)]
      );
    }
  ...
  public updateFormFieldsValue(fieldValue: any): void {
    ...
    if (target) {
      target = target
        ? target
        : document.getElementById(fieldValue.id + '_content_html_element')
            .children[0].children[0];
      if (
        (target && fieldValue.type === 'Textbox') ||
        fieldValue.type === 'Password' ||
        fieldValue.type === 'PasswordField'
      ) {
        target.value = fieldValue.value;
        target.multiline = fieldValue.isMultiline;
      } else if (
        fieldValue.type === 'Checkbox' ||
        fieldValue.type === 'RadioButton' ||
        fieldValue.type === 'CheckBox'
      ) {
        ...
        if (this.formDesignerModule) {
          if (fieldValue.type === 'RadioButton') {
            const radioButtonOption: any = {
              isSelected: fieldValue.isSelected,
            };
            this.formDesignerModule.updateFormField(
              fieldValue,
              radioButtonOption
            );
          } else {
        ...
        fieldValue.type === 'ListBox' ||
        fieldValue.type === 'DropdownList'
      ) {
        if (this.formDesignerModule) {
      ...
      if (
        fieldValue.type === 'SignatureField' ||
        fieldValue.type === 'InitialField'
      ) {
        ...
        if (
          ...
          if (this.formDesignerModule) {
            this.annotation.deleteAnnotationById(
              fieldValue.id.split('_')[0] + '_content'
            );
          } else {
        ...
        if (!isSameValue) {
          this.formFieldsModule.drawSignature(
      ...
      const data: string =
        this.viewerBase.getItemFromSessionStorage('_formfields');
      if (isNullOrUndefined(data)) {
        const data1: string =
          this.viewerBase.getItemFromSessionStorage('_formDesigner');
        const FormFieldsData1: any = JSON.parse(data1);
        const filteredCollection: any =
          this.viewerBase.formFieldCollection.filter((field: any) => {
        ...
        for (let m: number = 0; m < FormFieldsData1.length; m++) {
          // eslint-disable-next-line
          if (
            FormFieldsData1[parseInt(m.toString(), 10)].FormField.id.split(
              '_'
            )[0] == fieldValue.id
          ) {
            FormFieldsData1[parseInt(m.toString(), 10)].FormField.value =
              fieldValue.value;
            FormFieldsData1[
              parseInt(m.toString(), 10)
            ].FormField.signatureType = fieldValue.signatureType;
          }
        }
      } else {
        ...
        for (let m: number = 0; m < FormFieldsData.length; m++) {
          ...
          const fieldData: any = FormFieldsData.filter(function (
            item: any
          ): boolean {
            return item.FieldName === currentData.FieldName.split('_')[0];
          });
          if (
            !isNullOrUndefined(fieldData) &&
            !isNullOrUndefined(fieldData[0])
          ) {
            if (
              fieldData[0].Name === 'SignatureField' ||
              fieldData[0].Name === 'InitialField'
            ) {
              fieldName = currentData.FieldName.split('_')[0];
              currentData.LineBounds = FormFieldsData.filter(function (
                item: any
              ): boolean {
          ...
          if (fieldName === fieldValue.name) {
            if (
              ...
              const index: number = currentData.TextList
                ? currentData.TextList.indexOf(fieldValue.value)
                : 0;
              ...
              if (fieldValue.value) {
                currentData.Value = fieldValue.value;
                currentData = this.updateSignatureValue(
                  currentData,
                  fieldValue
                );
              }
        ...
        window.sessionStorage.removeItem(
          this.viewerBase.documentId + '_formfields'
        );
  ...
  private getFormFieldByID(id: string): any {
    ...
    const data: string = window.sessionStorage.getItem(
      this.viewerBase.documentId + '_formfields'
    );
    const formFieldsData: any = JSON.parse(data);
    return formFieldsData[
      formFieldsData.findIndex((el: { uniqueID: any }) => el.uniqueID === id)
    ];
  }
  ...
  private updateSignatureValue(currentData: any, fieldValue: any): any {
    if (!fieldValue.signatureType) {
      fieldValue.signatureType = this.viewerBase.isSignatureImageData(
        fieldValue.value
      )
        ? 'Image'
        : this.viewerBase.isSignaturePathData(fieldValue.value)
        ? 'Path'
        : 'Type';
    }
    ...
    if (fieldValue.signatureType === 'Type') {
      ...
      bounds = this.formFieldsModule.getSignBounds(
        currentData.pageIndex,
        currentData.RotationAngle,
        currentData.pageIndex,
        this.viewerBase.getZoomFactor(),
        left,
        top,
        width,
        height
      ...
      const textWidth: number = this.formFieldsModule.getTextWidth(
        currentData.value,
        fontSize,
      ...
      currentData.FontSize = this.formFieldsModule.getFontSize(
      ...
      if (currentData.Value.indexOf('base64') !== -1) {
        ...
        if (this.signatureFitMode === 'Default') {
          const signBounds: any =
            this.viewerBase.signatureModule.updateSignatureAspectRatio(
              currentData.Value,
              false,
              null,
              currentData
      ...
      if (data) {
        ...
        for (let i: number = 0; i < FormFieldsData.length; i++) {
          if (
            ...
            this.formFieldsModule.updateFormFieldsCollection(
              FormFieldsData[parseInt(i.toString(), 10)]
            );
          }
        }
        window.sessionStorage.removeItem(
  ...
  public unload(): void {
    if (
      !isNullOrUndefined(this.viewerBase.pdfViewerRunner) &&
      !this.viewerBase.isPasswordProtected
    ) {
  ...
  public importAnnotation(
    ...
    if (this.annotationModule) {
      if (typeof importData === 'string') {
        const isXfdfFile: boolean =
          importData.indexOf('.xfdf') > -1 ||
          annotationDataFormat.indexOf('Xfdf') > -1
            ? true
            : false;
        if (annotationDataFormat) {
          if (importData.indexOf('</xfdf>') > -1) {
            this.viewerBase.importAnnotations(
              importData,
              annotationDataFormat,
              false
            );
          ...
          if (importData.split('.')[1] === 'json') {
            if (importData.includes('pdfAnnotation')) {
              ...
              const newImportData: any = importData.split(',')[1]
                ? importData.split(',')[1]
                : importData.split(',')[0];
        ...
        if (
          ...
          !isNullOrUndefined(imporedAnnotation) &&
          !isNullOrUndefined(Object.keys(imporedAnnotation)) &&
          !isNullOrUndefined(Object.keys(imporedAnnotation)[0]) &&
          Object.keys(imporedAnnotation[Object.keys(imporedAnnotation)[0]])
            .length > 1
        ) {
          ...
          this.viewerBase.importAnnotations(
            btoa(String.fromCharCode(...Array.from(utf8Bytes))),
  ...
  private importAnnotationsAsJson(importData: any): void {
    ...
    const firstAnnotation: any =
      jsonData.pdfAnnotation[Object.keys(jsonData.pdfAnnotation)[0]];
    if (
      Object.keys(jsonData.pdfAnnotation).length >= 1 &&
      (firstAnnotation.textMarkupAnnotation ||
        firstAnnotation.measureShapeAnnotation ||
        firstAnnotation.freeTextAnnotation ||
        firstAnnotation.stampAnnotations ||
        firstAnnotation.signatureInkAnnotation ||
        (firstAnnotation.shapeAnnotation &&
          firstAnnotation.shapeAnnotation[0].Bounds))
    ) {
      this.viewerBase.isPDFViewerJson = true;
   ...
   */
  public exportAnnotationsAsObject(
    annotationDataFormat: AnnotationDataFormat = AnnotationDataFormat.Json
  ): Promise<object> {
   ...
   */
  public exportAnnotationsAsBase64String(
    annotationDataFormat: AnnotationDataFormat
  ): Promise<string> {
    if (this.annotationModule) {
      return new Promise((resolve: Function, reject: Function) => {
        this.viewerBase
          .createRequestForExportAnnotations(false, annotationDataFormat, true)
          .then((value: string) => {
            resolve(value);
          });
      });
   ...
   * @returns {Promise<object>} - promise
   */
  public exportFormFieldsAsObject(
    formFieldDataFormat: FormFieldDataFormat = FormFieldDataFormat.Json
  ): Promise<object> {
    if (this.formFieldsModule) {
      return new Promise((resolve: Function, reject: Function) => {
        this.viewerBase
  ...
  public fireAjaxRequestInitiate(JsonData: any): void {
    const eventArgs: AjaxRequestInitiateEventArgs = {
  ...
  public firePageRenderInitiate(jsonData: any): void {
    const eventArgs: PageRenderInitiateEventArgs = {
      name: 'pageRenderInitiate',
      jsonData: jsonData,
    };
   ...
   * @returns {void}
   */
  public fireButtonFieldClickEvent(
    value: string,
    fieldName: string,
    id: string
  ): void {
    const eventArgs: ButtonFieldClickEventArgs = {
      name: 'buttonFieldClicked',
      buttonFieldValue: value,
   ...
   */
  public async fireFormFieldClickEvent(
    name: string,
    field: FormFieldModel,
    cancel?: boolean,
    isLeftClick?: boolean
  ): Promise<void> {
    let eventArgs: FormFieldClickArgs = {
      name: name,
      field: field,
      cancel: cancel,
    };
    if (isBlazor()) {
      eventArgs =
        ((await this.triggerEvent(
          'formFieldClick',
          eventArgs
        )) as FormFieldClickArgs) || eventArgs;
    ...
    if (field.type === 'SignatureField' || field.type === 'InitialField') {
      ...
      target = target
        ? target
        : document.getElementById(field.id + '_content_html_element')
        ? document.getElementById(field.id + '_content_html_element')
            .children[0].children[0]
        : null;
      const formFieldCollectionsValue: any = this.formFieldCollections.filter(
      ...
      if (formFieldCollectionsValue) {
        ...
        if (
          ...
          (isLeftClick || isNullOrUndefined(isLeftClick))
        ) {
   ...
   * @returns {void}
   */
  public fireFormFieldAddEvent(
    name: string,
    field: IFormField,
    pageIndex: number
  ): void {
    const eventArgs: FormFieldAddArgs = {
      name: name,
      field: field,
      pageIndex: pageIndex,
    };
   ...
   * @returns {void}
   */
  public fireFormFieldRemoveEvent(
    name: string,
    field: IFormField,
    pageIndex: number
  ): void {
    const eventArgs: FormFieldRemoveArgs = {
      name: name,
      field: field,
      pageIndex: pageIndex,
    };
   ...
   */
  public fireFormFieldDoubleClickEvent(
    eventArgs: FormFieldDoubleClickArgs
  ): FormFieldDoubleClickArgs {
   ...
   * @returns {void}
   */
  public fireFormFieldPropertiesChangeEvent(
    name: string,
    field: IFormField,
    pageIndex: number,
    ...
    const eventArgs: FormFieldPropertiesChangeArgs = {
      ...
      isFontFamilyChanged: isFontFamilyChanged,
      isFontSizeChanged: isFontSizeChanged,
      isFontStyleChanged: isFontStyleChanged,
      isColorChanged: isColorChanged,
      isBackgroundColorChanged: isBackgroundColorChanged,
      isBorderColorChanged: isBorderColorChanged,
      isBorderWidthChanged: isBorderWidthChanged,
      isAlignmentChanged: isAlignmentChanged,
      isReadOnlyChanged: isReadOnlyChanged,
      isVisibilityChanged: isVisibilityChanged,
      isMaxLengthChanged: isMaxLengthChanged,
      isRequiredChanged: isRequiredChanged,
      isPrintChanged: isPrintChanged,
      isToolTipChanged: isToolTipChanged,
      oldValue: oldValue,
      newValue: newValue,
      isNameChanged: !isNullOrUndefined(isNamechanged) ? isNamechanged : false,
      isCustomDataChanged: isCustomDataChanged,
    };
   ...
   * @returns {void}
   */
  public fireFormFieldMouseLeaveEvent(
    name: string,
    field: IFormField,
    pageIndex: number
  ): void {
    const eventArgs: FormFieldMouseLeaveArgs = {
      name: name,
      field: field,
      pageIndex: pageIndex,
    };
   ...
   * @returns {void}
   */
  public fireFormFieldMouseoverEvent(
    name: string,
    field: IFormField,
    pageIndex: number,
    pageX: number,
    ...
    const eventArgs: FormFieldMouseoverArgs = {
      name: name,
      field: field,
      pageIndex: pageIndex,
      pageX: pageX,
  ...
  public fireFormFieldMoveEvent(
    ...
    const eventArgs: FormFieldMoveArgs = {
      name: name,
      field: field,
      pageIndex: pageIndex,
   ...
   * @returns {void}
   */
  public fireFormFieldResizeEvent(
    name: string,
    field: IFormField,
    pageIndex: number,
    previousPosition: IFormFieldBound,
    ...
    const eventArgs: FormFieldResizeArgs = {
      name: name,
      field: field,
      pageIndex: pageIndex,
      previousPosition: previousPosition,
   ...
   * @returns {void}
   */
  public fireFormFieldSelectEvent(
    name: string,
    field: IFormField,
    pageIndex: number,
    isProgrammaticSelection: boolean
  ): void {
    const eventArgs: FormFieldSelectArgs = {
      name: name,
      field: field,
      pageIndex: pageIndex,
   ...
   * @returns {void}
   */
  public fireFormFieldUnselectEvent(
    name: string,
    field: IFormField,
    pageIndex: number
  ): void {
    const eventArgs: FormFieldUnselectArgs = {
      name: name,
      field: field,
      pageIndex: pageIndex,
    };
  ...
  public fireDocumentUnload(fileName: string): void {
    const eventArgs: UnloadEventArgs = {
      name: 'documentUnload',
   ...
   * @returns {void}
   */
  public fireDocumentLoadFailed(
    isPasswordRequired: boolean,
    password: string
  ): void {
    const eventArgs: LoadFailedEventArgs = {
   ...
   * @returns {void}
   */
  public fireAjaxRequestFailed(
    errorStatusCode: number,
    errorMessage: string,
    action: string,
    retryCount?: boolean
  ): void {
    const eventArgs: AjaxRequestFailureEventArgs = {
      name: 'ajaxRequestFailed',
      documentName: this.fileName,
  ...
  public fireAjaxRequestSuccess(action: string, data: any): boolean {
    const eventArgs: AjaxRequestSuccessEventArgs = {
      name: 'ajaxRequestSuccess',
      documentName: this.fileName,
      action: action,
      data: data,
      cancel: false,
  ...
  public firePageRenderComplete(data: any): any {
    const eventArgs: PageRenderCompleteEventArgs = {
      name: 'pageRenderComplete',
      documentName: this.fileName,
      data: data,
    };
  ...
  public fireValidatedFailed(action: string): void {
    if (!isBlazor()) {
      const eventArgs: ValidateFormFieldsArgs = {
        formField: this.formFieldCollections,
        documentName: this.fileName,
        nonFillableFields: this.viewerBase.nonFillableFields,
      };
  ...
  public firePageClick(x: number, y: number, pageNumber: number): void {
    const eventArgs: PageClickEventArgs = {
      name: 'pageClick',
      documentName: this.fileName,
      x: x,
      y: y,
  ...
  public firePageChange(previousPageNumber: number): void {
    const eventArgs: PageChangeEventArgs = {
      name: 'pageChange',
      documentName: this.fileName,
  ...
  public fireZoomChange(): void {
    const eventArgs: ZoomChangeEventArgs = {
      name: 'zoomChange',
      zoomValue: this.magnificationModule.zoomFactor * 100,
   ...
   */
  public async fireHyperlinkClick(
    hyperlink: string,
    hyperlinkElement: HTMLAnchorElement
  ): Promise<boolean> {
    let eventArgs: HyperlinkClickEventArgs = {
      name: 'hyperlinkClick',
      hyperlink: hyperlink,
    ...
    if (isBlazor()) {
      eventArgs =
        ((await this.triggerEvent(
          'hyperlinkClick',
          eventArgs
        )) as HyperlinkClickEventArgs) || eventArgs;
    } else {
  ...
  public fireHyperlinkHover(hyperlinkElement: HTMLAnchorElement): void {
    const eventArgs: HyperlinkMouseOverArgs = {
  ...
  public fireFocusOutFormField(fieldName: string, value: string): void {
    const eventArgs: FormFieldFocusOutEventArgs = {
      name: 'formFieldFocusOut',
      fieldName: fieldName,
      value: value,
    };
  ...
  public fireAnnotationAdd(
    ...
    multiPageCollection?: any,
    customStampName?: string
  ): void {
    const eventArgs: AnnotationAddEventArgs = {
      name: 'annotationAdd',
      pageIndex: pageNumber,
      annotationId: index,
      annotationType: type,
      annotationBound: bounds,
  ...
  public fireAnnotationRemove(
    ...
    const eventArgs: AnnotationRemoveEventArgs = {
      name: 'annotationRemove',
      pageIndex: pageNumber,
      annotationId: index,
      annotationType: type,
  ...
  public fireBeforeAddFreeTextAnnotation(value: string): void {
    const eventArgs: BeforeAddFreeTextEventArgs = {
      name: 'beforeAddFreeText',
      value: value,
    };
  ...
  public fireCommentAdd(id: string, text: string, annotation: any): void {
    const eventArgs: CommentEventArgs = {
      name: 'CommentAdd',
      id: id,
      text: text,
   ...
   * @returns {void}
   */
  public fireCommentStatusChanged(
    id: string,
    text: string,
    annotation: any,
   ...
   * @returns {void}
   */
  public fireAnnotationPropertiesChange(
    pageNumber: number,
    index: string,
    type: AnnotationType,
    isColorChanged: boolean,
    ...
    const eventArgs: AnnotationPropertiesChangeEventArgs = {
      ...
      isCommentsChanged: isCommentsChanged,
    };
    if (textMarkupContent) {
      eventArgs.textMarkupContent = textMarkupContent;
      eventArgs.textMarkupStartIndex = tmStartIndex;
  ...
  public fireSignatureAdd(
    ...
    const eventArgs: AddSignatureEventArgs = {
      pageIndex: pageNumber,
      id: index,
      type: type,
      bounds: bounds,
   ...
   * @returns {void}
   */
  public fireSignatureRemove(
    pageNumber: number,
    index: string,
    type: AnnotationType,
    bounds: any
  ): void {
    const eventArgs: RemoveSignatureEventArgs = {
      pageIndex: pageNumber,
      id: index,
      type: type,
      bounds: bounds,
  ...
  public fireSignatureMove(
    ...
    const eventArgs: MoveSignatureEventArgs = {
      pageIndex: pageNumber,
      id: id,
      type: type,
      opacity: opacity,
   ...
   * @returns {void}
   */
  public fireSignaturePropertiesChange(
    pageNumber: number,
    index: string,
    type: AnnotationType,
    ...
    const eventArgs: SignaturePropertiesChangeEventArgs = {
      pageIndex: pageNumber,
      id: index,
      type: type,
      isStrokeColorChanged: isStrokeColorChanged,
      isOpacityChanged: isOpacityChanged,
      isThicknessChanged: isThicknessChanged,
      oldValue: oldProp,
      newValue: newProp,
    };
  ...
  public fireSignatureResize(
    ...
    const eventArgs: ResizeSignatureEventArgs = {
      pageIndex: pageNumber,
      id: index,
      type: type,
      opacity: opacity,
   ...
   * @returns {void}
   */
  public fireSignatureSelect(
    id: string,
    pageNumber: number,
    signature: object
  ): void {
    const eventArgs: SignatureSelectEventArgs = {
      id: id,
      pageIndex: pageNumber,
      signature: signature,
    };
   ...
   * @returns {void}
   */
  public fireSignatureUnselect(
    id: string,
    pageNumber: number,
    signature: object
  ): void {
    const eventArgs: SignatureUnselectEventArgs = {
      id: id,
      pageIndex: pageNumber,
      signature: signature,
    };
  ...
  public fireAnnotationSelect(
    ...
    let eventArgs: AnnotationSelectEventArgs = {
  ...
  public fireAnnotationUnSelect(
    ...
    const eventArgs: AnnotationUnSelectEventArgs = {
      name: 'annotationUnSelect',
      annotationId: id,
      pageIndex: pageNumber,
      annotation: annotation,
    };
   ...
   * @returns {void}
   */
  public fireAnnotationDoubleClick(
    id: string,
    pageNumber: number,
    annotation: any
  ): void {
    const eventArgs: AnnotationDoubleClickEventArgs = {
      name: 'annotationDblClick',
      annotationId: id,
      pageIndex: pageNumber,
      annotation: annotation,
    };
   ...
   * @returns {void}
   */
  public fireTextSelectionEnd(
    pageNumber: number,
    text: string,
    bound: any[]
  ): void {
    if (this.isTextSelectionStarted) {
      const eventArgs: TextSelectionEndEventArgs = {
        pageIndex: pageNumber,
        textContent: text,
        textBounds: bound,
      };
  ...
  public renderDrawing(canvas?: HTMLCanvasElement, index?: number): void {
    if (
      ...
      this.viewerBase.activeElements.activePageID &&
      !this.viewerBase.isPrint
    ) {
  ...
  public fireAnnotationResize(
    ...
    const eventArgs: AnnotationResizeEventArgs = {
  ...
  public fireAnnotationMoving(
    ...
    const eventArgs: AnnotationMovingEventArgs = {
      name: 'annotationMoving',
      pageIndex: pageNumber,
      annotationId: id,
      annotationType: type,
  ...
  public fireAnnotationMove(
    ...
    const eventArgs: AnnotationMoveEventArgs = {
      name: 'annotationMove',
      pageIndex: pageNumber,
      annotationId: id,
      annotationType: type,
   ...
   * @returns {void}
   */
  public fireAnnotationMouseover(
    id: string,
    pageNumber: number,
    ...
    const eventArgs: AnnotationMouseoverEventArgs = {
      name: 'annotationMouseover',
      annotationId: id,
      pageIndex: pageNumber,
      annotationType: annotationType,
      annotationBounds: bounds,
      annotation: annotation,
      pageX: currentPosition.left,
      pageY: currentPosition.top,
  ...
  public fireAnnotationMouseLeave(pageNumber: number): void {
    const eventArgs: AnnotationMouseLeaveEventArgs = {
      name: 'annotationMouseLeave',
      pageIndex: pageNumber,
    };
  ...
  public fireDownloadStart(fileName: string): boolean {
    const eventArgs: DownloadStartEventArgs = {
      fileName: fileName,
      cancel: false,
    };
  ...
  public fireDownloadEnd(fileName: string, downloadData: string): void {
    const eventArgs: DownloadEndEventArgs = {
      fileName: fileName,
      downloadDocument: downloadData,
    };
  ...
  public async firePrintStart(): Promise<void> {
    let eventArgs: PrintStartEventArgs = {
    ...
    if (isBlazor) {
      eventArgs =
        ((await this.triggerEvent(
          'printStart',
          eventArgs
        )) as PrintStartEventArgs) || eventArgs;
    } else {
  ...
  public fireThumbnailClick(pageNumber: number): void {
    const eventArgs: ThumbnailClickEventArgs = {
      name: 'thumbnailClick',
      pageNumber: pageNumber,
    };
   ...
   * @returns {void}
   */
  public async fireCustomToolbarClickEvent(
    target: ClickEventArgs
  ): Promise<void> {
   ...
   * @returns {void}
   */
  public fireBookmarkClick(
    pageNumber: number,
    position: number,
    text: string,
    fileName: string
  ): void {
    const eventArgs: BookmarkClickEventArgs = {
      name: 'bookmarkClick',
      pageNumber: pageNumber,
      position: position,
      text: text,
  ...
  public fireImportStart(importData: any): void {
    const eventArgs: ImportStartEventArgs = {
      name: 'importAnnotationsStart',
  ...
  public fireExportStart(exportData: any): boolean {
    const eventArgs: ExportStartEventArgs = {
      name: 'exportAnnotationsStart',
  ...
  public fireImportSuccess(importData: any): void {
    const eventArgs: ImportSuccessEventArgs = {
      name: 'importAnnotationsSuccess',
      importData: importData,
      formFieldData: null,
    };
  ...
  public fireExportSuccess(exportData: any, fileName: string): void {
    const eventArgs: ExportSuccessEventArgs = {
      name: 'exportAnnotationsSuccess',
      exportData: exportData,
      fileName: fileName,
      formFieldData: null,
    };
  ...
  public fireImportFailed(data: any, errorDetails: string): void {
    const eventArgs: ImportFailureEventArgs = {
      name: 'importAnnotationsFailed',
      importData: data,
  ...
  public fireExportFailed(data: any, errorDetails: string): void {
    const eventArgs: ExportFailureEventArgs = {
      name: 'exportAnnotationsFailed',
      exportData: data,
      errorDetails: errorDetails,
      formFieldData: null,
    };
   ...
   * @returns {void}
   */
  public fireTextExtractionCompleted(
    documentCollection: DocumentTextCollectionSettingsModel[][]
  ): void {
    const eventArgs: ExtractTextCompletedEventArgs = {
      documentTextCollection: documentCollection,
    };
  ...
  public fireTextSearchStart(searchText: string, isMatchcase: boolean): void {
    const eventArgs: TextSearchStartEventArgs = {
      name: 'textSearchStart',
   ...
   * @returns {void}
   */
  public fireTextSearchComplete(
    searchText: string,
    isMatchcase: boolean
  ): void {
    const eventArgs: TextSearchCompleteEventArgs = {
      name: 'textSearchComplete',
      searchText: searchText,
      matchCase: isMatchcase,
    };
   ...
   * @returns {void}
   */
  public fireTextSearchHighlight(
    searchText: string,
    isMatchcase: boolean,
    bounds: RectangleBoundsModel,
    pageNumber: number
  ): void {
    const eventArgs: TextSearchHighlightEventArgs = {
      name: 'textSearchHighlight',
      searchText: searchText,
      matchCase: isMatchcase,
      bounds: bounds,
      pageNumber: pageNumber,
  ...
  public fireKeyboardCustomCommands(gesture: KeyboardCommandModel): void {
    const eventArgs: KeyboardCustomCommandsEventArgs = {
      keyboardCommand: gesture,
    };
   ...
   * @returns {boolean} - boolean
   */
  public firePageOrganizerSaveAsEventArgs(
    fileName: string,
    downloadData: string
  ): boolean {
    const eventArgs: PageOrganizerSaveAsEventArgs = {
      fileName: fileName,
      downloadDocument: downloadData,
      cancel: false,
    };
  ...
  public select(
    ...
    if (this.annotationModule) {
      const module: TextMarkupAnnotation =
        this.annotationModule.textMarkupAnnotationModule;
      const annotationSelect: number =
        module && module.selectTextMarkupCurrentPage;
      const annotation: any = this.selectedItems.annotations[0];
      if (annotationSelect) {
        const currentAnnot: any =
          this.annotationModule.textMarkupAnnotationModule
            .currentTextMarkupAnnotation;
        this.annotationModule.textMarkupAnnotationModule.clearCurrentAnnotationSelection(
          annotationSelect,
          true
        );
        this.fireAnnotationUnSelect(
          currentAnnot.annotName,
          currentAnnot.pageNumber,
          currentAnnot
        );
      }
      if (!multipleSelection) {
        if (
          this.viewerBase.activeElements &&
          this.viewerBase.activeElements.activePageID >= 0
        ) {
          if (
            !this.viewerBase.isNewStamp &&
            annotation &&
            annotation.shapeAnnotationType !== 'HandWrittenSignature' &&
            annotation.shapeAnnotationType !== 'SignatureText' &&
            annotation.shapeAnnotationType !== 'SignatureImage'
          ) {
            this.fireAnnotationUnSelect(
        ...
        if (selectorModel.annotations.length) {
          for (let j: number = 0; j < selectorModel.annotations.length; j++) {
            ...
            if (
              ...
              if (
                !this.viewerBase.isNewSignatureAdded &&
                (annotation.shapeAnnotationType === 'HandWrittenSignature' ||
                  annotation.shapeAnnotationType === 'SignatureText' ||
                  annotation.shapeAnnotationType === 'SignatureImage')
              ) {
                this.annotationModule.unselectSignature(
                  node.signatureName,
                  node.pageIndex,
                  node
                );
    ...
    if (this.formDesignerModule) {
      ...
      if (formField) {
        if (
          this.formDesignerModule &&
          formField &&
          formField.formFieldAnnotationType
        ) {
          const field: IFormField = {
            name: (formField as any).name,
            id: (formField as any).id,
            ...
            backgroundColor: (formField as PdfFormFieldBaseModel)
              .backgroundColor,
            borderColor: (formField as PdfFormFieldBaseModel).borderColor,
            thickness: (formField as PdfFormFieldBaseModel).thickness,
            alignment: (formField as PdfFormFieldBaseModel).alignment,
            isReadonly: (formField as any).isReadonly,
            visibility: (formField as any).visibility,
            maxLength: (formField as any).maxLength,
            isRequired: (formField as any).isRequired,
            isPrint: formField.isPrint,
            rotation: (formField as any).rotateAngle,
            tooltip: (formField as any).tooltip,
            options: (formField as any).options,
            isChecked: (formField as any).isChecked,
            isSelected: (formField as any).isSelected,
          };
          this.fireFormFieldUnselectEvent(
            'formFieldUnselect',
            field,
            formField.pageIndex
          );
  ...
  public dragSelectedObjects(
    ...
    return this.drawing.dragSelectedObjects(
      diffX,
      diffY,
      pageIndex,
      currentSelector,
  ...
  public dragConnectorEnds(
    ...
    return this.drawing.dragConnectorEnds(
      endPoint,
      obj,
      point,
      segment,
  ...
  public clearSelection(pageId: number): void {
    ...
    this.enableServerDataBinding(false);
    const selectormodel: SelectorModel = this.selectedItems;
    const node: PdfAnnotationBaseModel | PdfFormFieldBaseModel =
      selectormodel.annotations.length > 0
        ? this.selectedItems.annotations[0]
        : this.selectedItems.formFields[0];
  ...
  public getPageNumberFromClientPoint(clientPoint: Point): number {
    const pageNumber: number =
      this.viewerBase.getPageNumberFromClientPoint(clientPoint);
    return pageNumber;
  }
   ...
   * @returns {Point} - point
   */
  public convertClientPointToPagePoint(
    clientPoint: Point,
    pageNumber: number
  ): Point {
    const pagePoint: Point = this.viewerBase.convertClientPointToPagePoint(
      clientPoint,
      pageNumber
    );
    return pagePoint;
   ...
   * @returns {Point} - point
   */
  public convertPagePointToClientPoint(
    pagePoint: Point,
    pageNumber: number
  ): Point {
    const clientPoint: Point = this.viewerBase.convertPagePointToClientPoint(
      pagePoint,
      pageNumber
    );
    return clientPoint;
   ...
   * @returns {Point} - point
   */
  public convertPagePointToScrollingPoint(
    pagePoint: Point,
    pageNumber: number
  ): Point {
    const scrollingPoint: Point =
      this.viewerBase.convertPagePointToScrollingPoint(pagePoint, pageNumber);
      ...
      if (this.zIndexTable.length !== undefined) {
        ...
        for (let i: number = 0; i < this.zIndexTable.length; i++) {
          const objects: PdfAnnotationBaseModel[] =
            this.zIndexTable[parseInt(i.toString(), 10)].objects;
        ...
        if (
          ...
          this.selectedItems.annotations[parseInt(i.toString(), 10)]
        ) {
   ...
   * @private
   */
  public enableServerDataBinding(
    enable: boolean,
    clearBulkChanges: boolean = false
  ): void {
    if (isBlazor()) {
  ...
  public checkBoundaryConstraints(
    ...
    return this.drawing.checkBoundaryConstraints(
      tx,
      ty,
      pageIndex,
      nodeBounds,
