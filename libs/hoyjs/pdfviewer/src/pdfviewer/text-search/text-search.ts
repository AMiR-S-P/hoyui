...
import { PdfPage } from '@hoyui/pdf';
import { AutoComplete } from '@hoyui/dropdowns';
import { regex } from '@hoyui/inputs';
...
export class TextSearch {
  ...
  private currentOccurrence: number = 0;
  ...
  public createTextSearchBox(): void {
    this.searchBox = createElement('div', {
      ...
      className: 'e-pv-text-search-bar',
    });
    ...
    const searchElementsContainer: HTMLElement = createElement('div', {
      id: this.pdfViewer.element.id + '_search_box_elements',
      className: 'e-pv-text-search-bar-elements',
    });
    this.searchInputContainer = createElement('div', {
    ...
    this.searchInput = createElement('input', {
      ...
      className: 'e-input e-pv-search-input-ele',
    });
    const tempData: any = {
      matches: [{ SearchString: '', Count: '' }],
    };
    ...
    this.searchAutocompleteObj = new AutoComplete({
      ...
      headerTemplate: `<div class="e-pv-automplete-header"><span class="e-pv-search-exact-matches">${this.pdfViewer.localeObj.getConstant(
        'Exact Matches'
      )}</span><span class="e-pv-total-exact-matches"></span></div>`,
      itemTemplate:
        '<div class="e-pv-autocomplete-item">' +
      ...
      beforeOpen: (event: any) => {
        if (
          Array.isArray(this.autompleteDataSource) &&
          this.autompleteDataSource.length === 0
        ) {
          event.cancel = true;
        }
      },
        ...
        const updateInterval: any = setInterval(() => {
          if (
            ...
            if (
              ...
              const totalCount: any = dataSourceInfo.reduce(
                (acc: number, obj: { Count: string }) =>
                  acc + parseInt(obj.Count, 10),
                0
              );
              document.querySelector('.e-pv-total-exact-matches').innerHTML =
                totalCount;
              if (this.isMultiSearch) {
                document.querySelector('.e-pv-search-exact-matches').innerHTML =
                  this.pdfViewer.localeObj.getConstant('Total Matches');
              } else {
                document.querySelector('.e-pv-search-exact-matches').innerHTML =
                  this.pdfViewer.localeObj.getConstant('Exact Matches');
              }
      ...
      created: (event: any) => {
        this.searchAutocompleteObj.element.addEventListener(
          'keydown',
          (args: any) => {
            if (args.key === 'Enter') {
              this.isSingleSearch = true;
        ...
        this.searchAutocompleteObj.element.parentElement
          .querySelector('.e-clear-icon')
          .addEventListener('mousedown', (args: any) => {
    ...
    this.searchBtn = createElement('span', {
      ...
      className:
        'e-input-group-icon e-input-search-group-icon e-pv-search-icon',
    });
    ...
    this.searchCountEle = createElement('span', {
      id: this.pdfViewer.element.id + '_search_count',
      className: 'e-pv-search-count',
      innerHTML: '',
    });
    searchElementsContainer.appendChild(this.searchCountEle);
    if (this.pdfViewer.enableRtl) {
      this.prevSearchBtn = this.createSearchBoxButtons(
        'prev_occurrence',
        'e-pv-next-search'
      );
    } else {
    ...
    if (this.pdfViewer.enableRtl) {
      this.nextSearchBtn = this.createSearchBoxButtons(
        'next_occurrence',
        'e-pv-prev-search'
      );
    } else {
    ...
    const matchCaseContainer: HTMLElement = createElement('div', {
      id: this.pdfViewer.element.id + '_match_case_container',
      className: 'e-pv-textsearch-match-case-container',
    });
    const matchCaseInput: HTMLElement = createElement('input', {
    ...
    if (isBlazor()) {
      ...
      matchCaseInput.addEventListener(
        'change',
        this.checkBoxOnChange.bind(this)
      );
    }
    matchCaseContainer.appendChild(matchCaseInput);
    const matchAnyWordInput: HTMLElement = createElement('input', {
      id: this.pdfViewer.element.id + '_match_any_word',
    });
    ...
    if (isBlazor()) {
      const matchCaseText: HTMLElement = createElement('span', {
        id: this.pdfViewer.element.id + '_search_box_text',
        styles:
          'position: absolute; padding-top: 3px; padding-left: 8px; padding-right: 8px; font-size: 13px',
      });
      ...
      this.matchAnyWordCheckBox = new CheckBox({
        cssClass: 'e-pv-match-any-word',
        label: this.pdfViewer.localeObj.getConstant('Match any word'),
        htmlAttributes: { tabindex: '0' },
        change: () => {
          this.isMultiSearch = this.matchAnyWordCheckBox.checked;
        },
    ...
    matchCaseContainer.firstElementChild.addEventListener(
      'keydown',
      (event: KeyboardEvent) => {
    ...
    const waitingPopup: HTMLElement = createElement('div', {
    ...
    this.nextSearchBtn.addEventListener(
      'click',
      this.nextButtonOnClick.bind(this)
    );
    this.prevSearchBtn.addEventListener(
      'click',
      this.prevButtonOnClick.bind(this)
    );
  }

  private setLoaderProperties(element: HTMLElement): void {
    const spinnerElement: HTMLElement = element.firstChild.firstChild
  ...
  private showLoadingIndicator(isShow: boolean): void {
    const waitingPopup: HTMLElement = document.getElementById(
      this.pdfViewer.element.id + '_textSearchLoadingIndicator'
    );
    if (waitingPopup) {
      if (isShow) {
  ...
  public textSearchBoxOnResize(): void {
    if (this.pdfViewer.toolbarModule && this.pdfViewer.enableToolbar) {
      const secondaryToolbar: HTMLElement = this.pdfViewerBase.getElement(
        '_toolbarContainer_popup'
      );
      if (secondaryToolbar) {
        if (
          secondaryToolbar.contains(
            this.pdfViewerBase.getElement('_search').parentElement
          )
        ) {
      ...
      if (
        this.pdfViewerBase.viewerContainer.clientWidth +
          this.pdfViewerBase.viewerContainer.offsetLeft <
        this.searchBox.offsetLeft + this.searchBox.clientWidth
      ) {
        this.searchBox.style.right = '0px';
        this.searchBox.style.width =
          parseInt(this.searchBox.style.width, 10) -
          (this.searchBox.offsetLeft +
            this.searchBox.clientWidth -
            this.pdfViewerBase.viewerContainer.clientWidth) +
          'px';
        this.searchInput.style.width =
          parseInt(this.searchInput.style.width, 10) -
          (this.searchBox.offsetLeft +
            this.searchBox.clientWidth -
            this.pdfViewerBase.viewerContainer.clientWidth) +
          'px';
  ...
  private calculateSearchCount(inputString: string): void {
    this.searchCount = 0;
    ...
    if (this.isMultiSearch) {
      ...
      for (let word of wordsToSearch) {
        ...
        for (let i: number = 0; i < this.documentTextCollection.length; i++) {
          const documentIndex: any =
            this.documentTextCollection[parseInt(i.toString(), 10)][
              parseInt(i.toString(), 10)
            ];
          let pageTextData: string = documentIndex.pageText
            ? documentIndex.pageText
            : documentIndex.PageText;
          if (!this.isMatchCase) {
      ...
      const transformedData: any = Object.keys(this.multiSearchCounts).map(
        (key: string) => ({
          SearchString: key,
    ...
    for (let i: number = 0; i < this.documentTextCollection.length; i++) {
      ...
      let multiSearch: string = pageTextData
        .replace(/(\s\r\n)/gm, ' ')
        .replace(/(\r\n)/gm, ' ');
      let Multiline: string = pageTextData
        .replace(/(\s\r\n)/gm, '  ')
        .replace(/(\r\n)/gm, ' ');
      let specialCharcterSearch: string = multiSearch.replace(
        /[^a-zA-z0-9" "]/g,
        ''
      );
      ...
      while (matchIndex !== 0) {
        ...
        if (this.isExactMatch) {
          ...
          const escapedInputString: string = inputString.replace(
            /[-\/\\^$*+?.()|[\]{}]/g,
            '\\$&'
          );
          // eslint-disable-next-line
          const regex: RegExp = new RegExp(
            `(?<!\\w)${escapedInputString}(?!\\w)`,
            'g'
          );
        ...
        matchIndex = pageTextData.indexOf(
          inputString,
      ...
      if (matches.length === 0) {
        multiSearchIndex = multiSearch.indexOf(
          inputString,
          multiSearchIndex + queryLength
        );
        MultilineIndex = Multiline.indexOf(
          inputString,
          MultilineIndex + queryLength
        );
        specialcharcterIndex = specialCharcterSearch.indexOf(
          inputString,
          specialcharcterIndex + queryLength
        );
        if (multiSearchIndex !== -1) {
          arrayReturns = this.correctLinetext(
            inputString,
            matchIndex,
            pageTextData
          );
          ...
          for (let i: number = 0; i < arrayReturns.length; i++) {
            matchIndex = pageTextData.indexOf(
              arrayReturns[parseInt(i.toString(), 10)].trim(),
              matchIndex +
                (arrayReturns[i - 1] === undefined || null
                  ? arrayReturns[0].length
                  : arrayReturns[i - 1].length)
            );
            matchedArray.push(matchIndex);
            if (matchedArray.length > 1) {
              if (
                matchedArray[1] - (matchedArray[0] + arrayReturns[0].length) <=
                3
              ) {
                matches.push(matchedArray);
                this.searchMatches[parseInt(pageIndex.toString(), 10)] =
                  matches;
              } else {
                i = -1;
    ...
    if (this.isSingleSearch) {
      if (this.searchCountEle) {
        if (this.pdfViewer.enableRtl) {
          ...
          this.searchCountEle.innerHTML = `${
            this.currentOccurrence + 1
          } ${this.pdfViewer.localeObj.getConstant('of')} ${this.searchCount}`;
        }
        if (
          this.searchedOccurrences.indexOf(this.currentOccurrence + 1) === -1
        ) {
  ...
  private handleSearchAfterTextCollectionReady(
    ...
    if (isMobileSearch) {
      const searchCountElement: HTMLElement = document.getElementById(
        this.pdfViewer.element.id + '_search_count'
      );
    ...
    if (
      ...
      if (
        this.searchMatches[this.searchPageIndex] &&
        inputString === this.searchString
      ) {
        ...
        isNullOrUndefined(this.searchMatches[this.searchPageIndex]) &&
        inputString === this.searchString
      ) {
        this.initSearch(this.searchPageIndex, false);
      } else {
  ...
  private textSearch(inputString: string): void {
    if (inputString !== '' || inputString) {
      ...
      this.searchPageIndex = this.findNextPageWithText(
  ...
  private nextSearch(): void {
    ...
    if (this.searchString) {
      ...
      if (this.currentOccurrence !== 0) {
        ...
        if (this.searchedOccurrences.length === 1) {
          this.pdfViewer.fireTextSearchStart(
            this.searchString,
            this.isMatchCase
          );
        }
      ...
      if (this.searchMatches[this.searchPageIndex]) {
        if (
          this.searchIndex >= this.searchMatches[this.searchPageIndex].length
        ) {
          this.searchIndex = 0;
          this.searchPageIndex = this.findNextPageWithText(
            this.searchPageIndex
        ...
        if (
          ...
          if (this.searchCountEle && this.isSingleSearch) {
            this.getSearchCountText();
            if (
              this.searchedOccurrences.indexOf(this.currentOccurrence) === -1
            ) {
              this.searchedOccurrences.push(this.currentOccurrence);
            }
            this.adjustInputContainerWidth();
  ...
  private findNextPageWithText(
    ...
    if (isInitialSearch) {
      ...
      if (
        this.searchMatches[parseInt(currentPageIndex.toString(), 10)] &&
    ...
    for (let i: number = 1; i < this.pdfViewerBase.pageCount; i++) {
      let nextPageIndex: number =
        (currentPageIndex + i) % this.pdfViewerBase.pageCount;
      if (
        this.searchMatches[parseInt(nextPageIndex.toString(), 10)] &&
  ...
  private findFirstNonEmptyPage(): number {
    for (let j: number = 0; j < this.pdfViewerBase.pageCount; j++) {
      if (
        this.searchMatches[parseInt(j.toString(), 10)] &&
        this.searchMatches[parseInt(j.toString(), 10)].length > 0
      ) {
        return j;
      }
    }
  ...
  private findPreviousPageWithText(): any {
    ...
    for (let i: number = 1; i < this.pdfViewerBase.pageCount; i++) {
      let prevPageIndex: any =
        (currentPageIndex - i + this.pdfViewerBase.pageCount) %
        this.pdfViewerBase.pageCount;
      if (
        this.searchMatches[parseInt(prevPageIndex.toString(), 10)] &&
      ...
      if (this.documentTextCollection.length !== 0) {
        const documentIndex: any =
          this.documentTextCollection[parseInt(pageIndex.toString(), 10)][
            parseInt(pageIndex.toString(), 10)
          ];
        const pageTextData: string = documentIndex.pageText
          ? documentIndex.pageText
          : documentIndex.PageText;
        if (
          this.documentTextCollection[parseInt(pageIndex.toString(), 10)] &&
  ...
  private getPossibleMatches(
    ...
    let multiSearch: string = pageText
      .replace(/(\s\r\n)/gm, ' ')
      .replace(/(\r\n)/gm, ' ');
    let Multiline: string = pageString
      .replace(/(\s\r\n)/gm, '  ')
      .replace(/(\r\n)/gm, ' ');
    let specialCharcterSearch: string = multiSearch.replace(
      /[^a-zA-z0-9" "]/g,
    ...
    while (matchIndex !== 0 || matchIndex === 0) {
      ...
      if (this.isExactMatch) {
        let match: RegExpExecArray;
        // eslint-disable-next-line
        const escapedInputString: string = searchString.replace(
          /[-\/\\^$*+?.()|[\]{}]/g,
          '\\$&'
        );
        // eslint-disable-next-line
    ...
    if (
      ...
      if (!isSinglePageSearch) {
        ...
        if (this.pdfViewerBase.currentPageNumber - 1 !== this.searchPageIndex) {
          if (
            ...
            this.startIndex - 1 === this.searchPageIndex
          ) {
            if (!this.isMessagePopupOpened) {
              this.onMessageBoxOpen();
            }
            ...
            searchTextCollection[0] === this.searchPageIndex &&
            this.areAllOccurencesSearched()
          ) {
            if (!this.isMessagePopupOpened) {
              this.onMessageBoxOpen();
            }
          ...
          this.searchMatches[this.searchPageIndex].length > 0 &&
          this.searchedPages.length === this.pdfViewerBase.pageCount &&
          this.startIndex === this.searchPageIndex &&
          this.pdfViewerBase.pageCount > 1
        ) {
          if (!this.isMessagePopupOpened) {
            this.onMessageBoxOpen();
      ...
      if (!isSinglePageSearch) {
        if (this.isPrevSearch) {
          ...
          this.searchPageIndex =
            this.searchPageIndex + 1 < this.pdfViewerBase.pageCount
              ? this.searchPageIndex + 1
              : 0;
        }
        if (
          this.searchedPages.indexOf(this.searchPageIndex) === -1 &&
          this.searchedPages.length !== this.pdfViewerBase.pageCount
        ) {
          this.showLoadingIndicator(true);
          this.initSearch(this.searchPageIndex, false);
          ...
          if (
            ...
            this.searchMatches.length > 0 &&
            (this.searchIndex === 0 || this.searchIndex === -1) &&
            searchPageIndex === this.currentSearchIndex
          ) {
            if (this.isPrevSearch) {
              if (!this.isMessagePopupOpened) {
                this.onMessageBoxOpen();
              ...
              if (
                !this.isMessagePopupOpened &&
                this.pdfViewerBase.currentPageNumber !== 0 &&
                !this.isSearchText
              ) {
                this.onMessageBoxOpen();
              }
  ...
  private correctLinetext(
    ...
    for (let i: number = 0; i < searchArray.length; i++) {
      const searchArrayText: string =
        linestring + searchArray[parseInt(i.toString(), 10)];
      matchIndex = pageText.indexOf(searchArrayText, matchIndex);
      pageCheck = pageCheck
        ? pageCheck.replace(searchArray[i - 1], '')
        : pageText.replace(searchArray[i - 1], '');
      if (
        (pageCheck[
          pageCheck.indexOf(searchArray[parseInt(i.toString(), 10)]) - 1
        ] === '\n' &&
  ...
  private getSearchPage(pageIndex: number): number {
    ...
    if (this.isPrevSearch) {
      ...
      if (!pageNumber) {
        for (
          let j: number = this.pdfViewerBase.pageCount - 1;
          j > pageIndex;
          j--
        ) {
  ...
  private areAllNonEmptyPagesSearched(): boolean {
    ...
    for (let i: number = 0; i < this.searchMatches.length; i++) {
      if (
        this.searchMatches[parseInt(i.toString(), 10)] &&
  ...
  private highlightSearchedTexts(
    ...
    if (
      isSinglePageSearch &&
      this.pdfViewerBase.currentPageNumber - 1 !== this.searchPageIndex
    ) {
      if (this.searchMatches.length > 0) {
        if (
          pageIndex ===
          this.getSearchPage(this.pdfViewerBase.currentPageNumber - 1)
        ) {
          isHighlight = true;
        }
      }
    ...
    if (characterBounds && matches !== undefined) {
      for (let i: number = 0; i < matches.length; i++) {
        if (
          matches[parseInt(i.toString(), 10)].length !== undefined &&
          ArrayReturns !== undefined
        ) {
          if (i === this.searchIndex && pageIndex === this.searchPageIndex) {
            for (let j: number = 0; j < ArrayReturns.length; j++) {
              ...
              this.searchString =
                ArrayReturns[parseInt(j.toString(), 10)].trim();
              this.addDivForSearch(
                i,
                pageIndex,
                characterBounds,
      ...
      if (pageIndex === this.searchPageIndex && !isSinglePageSearch) {
        const element: HTMLElement = this.pdfViewerBase.getElement(
          '_searchtext_' + pageIndex + '_' + this.searchIndex
        );
        if (element) {
          const targetScrollElement: HTMLElement =
            this.getScrollElement(element);
  ...
  private addDivForSearch(
    ...
    const textLayer: HTMLElement = this.pdfViewerBase.getElement(
      '_textLayer_' + pageIndex
    );
    if (
      isNullOrUndefined(textLayer) &&
      className === 'e-pv-search-text-highlight'
    ) {
  ...
  private addDivElement(
    ...
    for (v = v; v < queryLength; v++) {
      if (characterBounds[parseInt(count.toString(), 10)]) {
        ...
        const topDifference: number =
          top < charBound.Y ? charBound.Y - top : top - charBound.Y;
        height =
          height > topDifference + charBound.Height
            ? height
            : topDifference + charBound.Height;
        count++;
      }
    }
    ...
    if (!isRTL) {
      if (initial + queryLength !== count) {
        ...
        const storedData: any = this.pdfViewerBase.clientSideRendering
          ? this.pdfViewerBase.getLinkInformation(pageIndex, true)
          : this.pdfViewerBase.getStoredData(pageIndex, true);
        let pageText: string = null;
        if (storedData) {
          pageText = storedData['pageText'];
        } else if (
          this.pdfViewer.isExtractText &&
          this.documentTextCollection.length !== 0
        ) {
          const documentIndex: any =
            this.documentTextCollection[parseInt(pageIndex.toString(), 10)][
              parseInt(pageIndex.toString(), 10)
            ];
          pageText = documentIndex.pageText
            ? documentIndex.pageText
            : documentIndex.PageText;
        }
        if (characterBounds[parseInt(count.toString(), 10)]) {
          if (
            pageText &&
            (pageText[parseInt(count.toString(), 10)] === '' ||
              pageText[parseInt(count.toString(), 10)] === ' ' ||
              pageText[parseInt(count.toString(), 10)] === '\r' ||
              pageText[parseInt(count.toString(), 10)] === '\n') &&
            characterBounds[parseInt(count.toString(), 10)].Width === 0
          ) {
            width =
              characterBounds[count - 1].X -
              left +
              characterBounds[count - 1].Width;
          } else {
      ...
      left = charBound.X;
      width =
        characterBounds[parseInt(initial.toString(), 10)].X -
        characterBounds[initial + queryLength - 1].X;
      top = top < charBound.Y ? top : charBound.Y;
      const topDifference: number =
        top < charBound.Y ? charBound.Y - top : top - charBound.Y;
      height =
      ...
      let widthDifference: number =
        characterBounds[initial - 1].Y -
        characterBounds[parseInt(initial.toString(), 10)].Y;
  ...
  private createSearchTextDiv(
    ...
    const nestedElement: HTMLCollectionOf<HTMLElement> =
      document.getElementsByClassName(
        'e-pv-search-text-highlight'
      ) as HTMLCollectionOf<HTMLElement>;
    if (
      nestedIndex !== undefined &&
      this.pdfViewerBase.getElement(idString) &&
      !nestedElement[parseInt(nestedIndex.toString(), 10)]
    ) {
      ...
      if (className === 'e-pv-search-text-highlight') {
        textDiv.style.backgroundColor =
          this.pdfViewer.textSearchColorSettings.searchHighlightColor === ''
            ? '#fdd835'
            : this.pdfViewer.textSearchColorSettings.searchHighlightColor;
        const bounds: RectangleBoundsModel = {
          left: left,
          top: top,
          width: width,
          height: height,
        };
        this.pdfViewer.fireTextSearchHighlight(
          this.searchString,
          this.isMatchCase,
          bounds,
          pageIndex + 1
        ...
        textDiv.style.backgroundColor =
          this.pdfViewer.textSearchColorSettings.searchColor === ''
            ? '#8b4c12'
            : this.pdfViewer.textSearchColorSettings.searchColor;
      }
      const textLayer: HTMLElement = this.pdfViewerBase.getElement(
  ...
  private calculateBounds(
    ...
    if (pageDetails.rotation === 0 || pageDetails.rotation === 2) {
      textDiv.style.height =
        Math.ceil(height) * this.pdfViewerBase.getZoomFactor() + 'px';
      textDiv.style.width = width * this.pdfViewerBase.getZoomFactor() + 'px';
      if (pageDetails.rotation === 2) {
        textDiv.style.top =
          (pageDetails.height - top - height) *
            this.pdfViewerBase.getZoomFactor() +
          'px';
        textDiv.style.left =
          Math.ceil(pageDetails.width - left - width) *
            this.pdfViewerBase.getZoomFactor() +
          'px';
      } else {
      ...
      textDiv.style.width = height * this.pdfViewerBase.getZoomFactor() + 'px';
      textDiv.style.left =
        (pageDetails.width - pageDetails.height + top) *
          this.pdfViewerBase.getZoomFactor() +
        'px';
      textDiv.style.top =
        (pageDetails.height - left - width) *
          this.pdfViewerBase.getZoomFactor() +
        'px';
    }
  }
  ...
  private isClassAvailable(): boolean {
    ...
    for (let j: number = 0; j < this.tempElementStorage.length; j++) {
      if (this.tempElementStorage[parseInt(j.toString(), 10)].classString) {
        if (
          this.tempElementStorage[parseInt(j.toString(), 10)].classString ===
            'e-pv-search-text-highlight' ||
          this.tempElementStorage[parseInt(j.toString(), 10)].classString ===
  ...
  private getScrollElement(element: HTMLElement): HTMLElement {
    ...
    if (element.childNodes.length > 0) {
      for (let i: number = 0; i < element.childNodes.length; i++) {
        if (
          (element.childNodes[parseInt(i.toString(), 10)] as HTMLElement)
            .classList
        ) {
          if (
            (
              element.childNodes[parseInt(i.toString(), 10)] as HTMLElement
            ).classList.contains('e-pv-search-text-highlight')
          ) {
            targetElement = element.childNodes[
              parseInt(i.toString(), 10)
            ] as HTMLElement;
          }
        }
  ...
  public resizeSearchElements(pageIndex: number): void {
    const searchDivs: NodeList = document.querySelectorAll(
      'div[id*="' +
    ...
    for (let i: number = 0; i < searchDivs.length; i++) {
      const textDiv: HTMLElement = searchDivs[
        parseInt(i.toString(), 10)
      ] as HTMLElement;
      let previousZoomFactor: number = 1;
      if (this.pdfViewer.magnificationModule) {
        previousZoomFactor =
          this.pdfViewer.magnificationModule.previousZoomFactor;
      }
      ...
      if (textDiv.getAttribute('name') !== outputdata) {
        textDiv.style.width =
          (parseFloat(textDiv.style.width) / previousZoomFactor) *
            this.pdfViewerBase.getZoomFactor() +
          'px';
        textDiv.style.height =
          (parseFloat(textDiv.style.height) / previousZoomFactor) *
            this.pdfViewerBase.getZoomFactor() +
          'px';
        textDiv.style.top =
          (parseFloat(textDiv.style.top) / previousZoomFactor) *
            this.pdfViewerBase.getZoomFactor() +
          'px';
        textDiv.style.left =
          (parseFloat(textDiv.style.left) / previousZoomFactor) *
            this.pdfViewerBase.getZoomFactor() +
          'px';
  ...
  public clearAllOccurrences(): void {
    const searchTextDivs: NodeList = document.querySelectorAll(
      'div[id*="' + this.pdfViewer.element.id + '_searchtext_"]'
    );
    for (let i: number = 0; i < searchTextDivs.length; i++) {
      searchTextDivs[parseInt(i.toString(), 10)].parentElement.removeChild(
        searchTextDivs[parseInt(i.toString(), 10)]
      );
    }
  }
  ...
  public getIndexes(): any {
    ...
    higherPageValue =
      higherPageValue < this.pdfViewerBase.pageCount
        ? higherPageValue
        : this.pdfViewerBase.pageCount - 1;
  ...
  private applyTextSelection(): void {
    if (
      this.pdfViewer.textSelectionModule &&
      !this.pdfViewerBase.isTextSelectionDisabled
    ) {
      const indexes: { [key: string]: object } = this.getIndexes();
      const lowerPageValue: number = parseFloat(
        indexes.lowerPageValue.toString()
  ...
  private createRequestForSearch(pageIndex: number): void {
    ...
    const tileCount: number = this.pdfViewerBase.getTileCount(
      pageWidth,
      pageHeight
    );
    ...
    const tileSettings: TileRenderingSettingsModel =
      this.pdfViewer.tileRenderingSettings;
    if (
      tileSettings.enableTileRendering &&
      tileSettings.x > 0 &&
      tileSettings.y > 0
    ) {
    ...
    for (let x: number = 0; x < noTileX; x++) {
      for (let y: number = 0; y < noTileY; y++) {
        ...
        const zoomFactor: number =
          this.pdfViewerBase.retrieveCurrentZoomFactor();
        this.searchRequestHandler = new AjaxHandler(this.pdfViewer);
        this.searchRequestHandler.url =
        ...
        if (this.pdfViewerBase.clientSideRendering) {
          const textDetailsId: string =
            this.pdfViewerBase.documentId + '_' + pageIndex + '_textDetails';
          const isTextNeed: boolean = this.pdfViewerBase.pageTextDetails
            ? this.pdfViewerBase.pageTextDetails[`${textDetailsId}`]
              ? false
              : true
            : true;
          ...
          const currentPage: PdfPage =
            this.pdfViewer.pdfRenderer.loadedDocument.getPage(pageIndex);
          if (
            currentPage &&
            currentPage._pageDictionary &&
            currentPage._pageDictionary._map &&
          ...
          if (
            ...
            !this.pdfViewer.tileRenderingSettings.enableTileRendering
          ) {
            ...
            switch (event.data.message) {
              case 'imageRenderedSearch':
                if (event.data.message === 'imageRenderedSearch') {
                  ...
                  const hyperlinksDetails: any =
                    proxy.pdfViewer.pdfRendererModule.getHyperlinks(pageIndex);
                  const data: any = {
                    image: imageUrl,
                    pageNumber: pageIndex,
                    uniqueId: proxy.pdfViewerBase.documentId,
                  ...
                  if (event.data.isTextNeed) {
                    ...
                    proxy.pdfViewerBase.storeTextDetails(
                      pageIndex,
                      textBounds,
                      textContent,
                      pageText,
                    ...
                    const textDetails: any = JSON.parse(
                      proxy.pdfViewerBase.pageTextDetails[
                        `${event.data.textDetailsId}`
                      ]
                    );
                  ...
                  if (
                    data &&
                    data.image &&
                    data.uniqueId === proxy.pdfViewerBase.documentId
                  ) {
                    ...
                    const pageNumber: number =
                      data.pageNumber !== undefined
                        ? data.pageNumber
                        : pageIndex;
                    const blobObj: string = proxy.pdfViewerBase.createBlobUrl(
              ...
              case 'renderTileImageSearch':
                if (event.data.message === 'renderTileImageSearch') {
                  const canvas: HTMLCanvasElement =
                    document.createElement('canvas');
                  const { value, w, h, noTileX, noTileY, x, y, pageIndex } =
                    event.data;
                  ...
                  if (
                    tileData &&
                    tileData.image &&
                    tileData.uniqueId === proxy.pdfViewerBase.documentId
                  ) {
                    proxy.pdfViewer.fireAjaxRequestSuccess(
                      proxy.pdfViewer.serverActionSettings.renderPages,
                      tileData
                    ...
                    const pageNumber: number =
                      tileData.pageNumber !== undefined
                        ? tileData.pageNumber
                        : pageIndex;
                    if (x === 0 && y === 0) {
                      const blobObj: string = proxy.pdfViewerBase.createBlobUrl(
                        tileData.image.split('base64,')[1],
                        'image/png'
                      );
                      const Url: any = URL || webkitURL;
                      const blobUrl: string = Url.createObjectURL(blobObj);
                      ...
                      proxy.pdfViewerBase.storeImageData(
                        pageNumber,
                        storeObject,
                        tileData.tileX,
                        tileData.tileY
  ...
  private searchRequestOnSuccess(
    ...
    if (
      !isNullOrUndefined(data.pageText) &&
      data.uniqueId === proxy.pdfViewerBase.documentId
    ) {
      proxy.pdfViewer.fireAjaxRequestSuccess(
        this.pdfViewer.serverActionSettings.renderPages,
      ...
      const pageNumber: number =
        data.pageNumber !== undefined ? data.pageNumber : pageIndex;
      if (viewPortWidth >= pageWidth) {
        proxy.pdfViewerBase.storeWinData(data, pageNumber);
      } else {
        proxy.pdfViewerBase.storeWinData(
          data,
          pageNumber,
          data.tileX,
          data.tileY
   ...
   * @returns {void}
   */
  public createRequestForGetPdfTexts(
    startIndex: number,
    endIndex: number
  ): void {
    // eslint-disable-next-line
    ...
    this.searchRequestHandler.url =
      this.pdfViewer.serviceUrl +
      '/' +
      this.pdfViewer.serverActionSettings.renderTexts;
    this.searchRequestHandler.responseType = 'json';
    if (!this.pdfViewerBase.clientSideRendering) {
      this.searchRequestHandler.send(jsonObject);
    }
    this.searchRequestHandler.onSuccess = function (result: any): void {
      ...
      if (data) {
        ...
        if (data) {
          proxy.pdfTextSearchRequestOnSuccess(
            data,
            proxy,
            startIndex,
            endIndex
   ...
   * @returns {void}
   */
  public pdfTextSearchRequestSuccess(
    data: any,
    startIndex: number,
    endIndex: number
  ): void {
  ...
  private pdfTextSearchRequestOnSuccess(
    ...
    if (
      ...
      if (proxy.documentTextCollection.length > 0) {
        proxy.documentTextCollection = data.documentTextCollection.concat(
          proxy.documentTextCollection
        );
        proxy.documentTextCollection = proxy.orderPdfTextCollections(
          proxy.documentTextCollection
        );
      } else {
      ...
      if (endIndex !== pageCount) {
        ...
        proxy.pdfViewer.fireTextExtractionCompleted(
          proxy.documentTextCollection
        );
  ...
  private orderPdfTextCollections(oldCollection: any): any {
    ...
    for (let i: number = 0; i < oldCollection.length; i++) {
      if (annotationCollectionList.length === 0) {
        annotationCollectionList.push(
          oldCollection[parseInt(i.toString(), 10)]
        );
      } else {
        if (
          parseInt(
            Object.keys(oldCollection[parseInt(i.toString(), 10)])[0],
            10
          ) >
          parseInt(
            Object.keys(
              annotationCollectionList[annotationCollectionList.length - 1]
            )[0],
            10
          )
        ) {
          ...
          for (let j: number = 0; j < annotationCollectionList.length; j++) {
            if (
              ...
              parseInt(
                Object.keys(
                  annotationCollectionList[parseInt(j.toString(), 10)]
                )[0],
                10
              )
            ) {
              annotationCollectionList.splice(
                j,
                0,
                oldCollection[parseInt(i.toString(), 10)]
              );
  ...
  private createSearchBoxButtons(id: string, className: string): HTMLElement {
    const button: HTMLElement = createElement('button', {
      ...
      className: 'e-btn e-icon-btn e-pv-text-search-btn ' + className,
    });
  ...
  private searchKeypressHandler = (event: KeyboardEvent): void => {
    ...
    if (
      (isAlphanumeric || isSpecialCharacter) &&
  ...
  public searchButtonClick(
    element: HTMLElement,
    inputElement: AutoComplete | HTMLElement,
    isMobileSearch?: boolean
  ): void {
    this.isMessagePopupOpened = false;
    if (element.classList.contains('e-pv-search-icon')) {
      this.initiateTextSearch(
        (inputElement as HTMLInputElement).value as string,
        isMobileSearch
      );
  ...
  private onMessageBoxOpen(): void {
    ...
    if (!Browser.isDevice || this.pdfViewer.enableDesktopMode) {
      if (isBlazor()) {
        ...
        if (this.searchMatches.length > 0) {
          this.pdfViewerBase.createNotificationPopup(
            this.pdfViewer.localeObj.getConstant('No More Matches')
          );
        } else {
          this.pdfViewerBase.createNotificationPopup(
            this.pdfViewer.localeObj.getConstant('No Matches')
          );
        }
      }
    } else {
      if (isBlazor()) {
        ...
        if (this.searchMatches.length > 0) {
          this.pdfViewerBase.navigationPane.createTooltipMobile(
            this.pdfViewer.localeObj.getConstant('No More Search Matches')
          );
        } else {
          this.pdfViewerBase.navigationPane.createTooltipMobile(
            this.pdfViewer.localeObj.getConstant('No Search Matches')
          );
        }
      }
    }
  ...
  public searchText(searchText: string, isMatchCase: boolean): void {
    ...
    this.searchIndex = 0;
    this.isSingleSearch = true;
    this.calculateSearchCount(searchText);
  ...
  public findText(
    ...
    isMatchCase: boolean,
    pageIndex?: number
  ): Record<string, SearchResultModel[]>;
  /**
  ...
  public findText(
    ...
    this.isMatchCase = isMatchCase;
    const searchTerms: string[] = Array.isArray(searchText)
      ? searchText
      : [searchText];
    ...
    for (const term of searchTerms) {
      this.calculateSearchCount(term);
      searchResults[`${term}`] = [];
      for (let i: number = startIndex; i < endIndex; i++) {
        ...
        const characterBounds: any = documentIndex.textData
          ? documentIndex.textData
          : documentIndex.TextData;
        if (!characterBounds) {
          continue;
        }
        ...
        for (const matchIndex of matches) {
          const rect: IPdfRectBounds = this.calculateTextBounds(
            term,
            matchIndex,
            characterBounds
          );
    ...
    return Array.isArray(searchText)
      ? searchResults
  ...
  private calculateTextBounds(
    ...
    const startBound: any =
      characterBounds[parseInt(matchIndex.toString(), 10)].Bounds;
    ...
    for (let k: number = 0; k < searchText.length; k++) {
      const currentBound: any =
        characterBounds[parseInt((matchIndex + k).toString(), 10)].Bounds;
