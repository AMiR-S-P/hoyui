...
import {
  ...
  MaskFocusEventArgs,
  MaskBlurEventArgs,
} from '../src/maskedtextbox/maskedtextbox/maskedtextbox';
import {
  maskInput,
  setMaskValue,
  getVal,
...
} from '../src/maskedtextbox/base/mask-base';
...
describe('MaskedTextBox Component', () => {
  ...
  describe('MaskedTextBox creation', () => {
    ...
    it('Render and check MaskedTextBox without mask', () => {
      ...
      expect(
        input.classList.contains('e-maskedtextbox') &&
          input.classList.contains('e-control')
      ).toEqual(true);
      let inputParent: HTMLElement = <HTMLElement>input.parentNode;
      expect(
        inputParent.classList.contains('e-input-group') ||
          inputParent.classList.contains('e-float-input')
      ).toEqual(true);
    ...
    it('Check MaskedTextBox with other elements', () => {
      ...
      let eleParent: HTMLElement = <HTMLElement>(
        document.getElementById('maskParent')
      );
      eleParent.appendChild(divEle);
      maskBox = new MaskedTextBox();
      maskBox.appendTo('#maskDiv');
      expect(!divEle.parentElement.classList.contains('e-input-group')).toEqual(
        true
      );
    });
    it('Render and check MaskedTextBox with "EJS-MASKEDTEXTBOX" tag', () => {
      let customElement: HTMLElement = createElement('EJS-MASKEDTEXTBOX', {
        id: 'customTag',
      });
      document.body.appendChild(customElement);
      maskBox = new MaskedTextBox();
      maskBox.appendTo('#customTag');
      let customParent: HTMLElement = <HTMLElement>(
        document.getElementById('customTag').parentNode
      );
      expect(
        customElement.classList.contains('e-control') &&
          customElement.classList.contains('e-maskedtextbox')
      ).toEqual(false);
      expect(customElement.classList.contains('e-mask-container')).toEqual(
        true
      );
    ...
    it('Apply CssClass, rtl property during initialization', () => {
      ...
      expect(input.parentElement.classList.contains('e-custom-css')).toEqual(
        true
      );
      expect(input.parentElement.classList.contains('e-rtl')).toEqual(true);
      expect(input.classList.contains('e-disabled')).toEqual(false);
      expect(
        input.hasAttribute('disabled') && input.hasAttribute('aria-disabled')
      ).toEqual(false);
    });
    it('Apply enabled false during initialization', () => {
      ...
      expect(input.parentElement.classList.contains('e-disabled')).toEqual(
        true
      );
      expect(
        input.hasAttribute('disabled') && input.hasAttribute('aria-disabled')
  ...
  describe('Edit values in MaskedTextBox through property', () => {
    ...
    it('Enable ShowClearButton through property', () => {
      ...
      maskBox.dataBind();
      expect(
        maskBox.inputObj.clearButton.classList.contains('e-clear-icon-hide')
      ).toBe(true);
      document.getElementById('mask1').focus();
      expect(
        maskBox.inputObj.clearButton.classList.contains('e-clear-icon')
      ).toBe(true);
  ...
  describe('Dynamically change enabled state as true', () => {
    ...
    it('Without mask option', () => {
      ...
      expect(maskBox.inputObj.container.classList.contains('e-disabled')).toBe(
        true
      );
  ...
  describe('Edit values in MaskedTextBox through keyboard keys', () => {
    ...
    it('Edit values in MaskedTextBox -- Check error class', (done: Function) => {
      ...
      input.focus();
      setTimeout(function () {
        expect(input.selectionStart == 0 && input.selectionEnd == 0).toEqual(
          true
        );
        let event: any = eventObject('KeyboardEvent', 'keypress');
        event.key = 'P';
  ...
  describe('Edit values in MaskedTextBox through keyboard keys-Backspace', () => {
    ...
    it('Edit values in MaskedTextBox - clear single value using delete key', () => {
      ...
      EventHandler.trigger(input, 'keydown', event);
      expect(input.value === '98 0_2 5679').toEqual(true);
      expect(input.selectionStart === 7 && input.selectionEnd === 7).toEqual(
        true
      );
    });
    ...
    it('Edit values in MaskedTextBox - clear with custom characters', () => {
      ...
      EventHandler.trigger(input, 'keydown', event);
      EventHandler.trigger(input, 'keydown', event);
      expect(input.value.length === 8 && input.value[2].trim() === '').toEqual(
        true
      );
  ...
  describe('Edit values in MaskedTextBox-- Upper and Lower Case', () => {
    ...
    it('Edit values in the MaskedTextBox with alphabets using masking element "L"', () => {
      ...
      event.key = 'b';
      EventHandler.trigger(input, 'keypress', event);
      expect(input.value.length === 4 && input.value[2].trim() === '').toEqual(
        true
      );
    ...
    it('Edit values in the MaskedTextBox with alphabets using masking element "?"', () => {
      ...
      event.key = ' ';
      EventHandler.trigger(input, 'keypress', event);
      expect(input.value.length === 5 && input.value[2].trim() === '').toEqual(
        true
      );
      expect(input.value === 'sS b ').toEqual(true);
    });
    it('Edit values - check for upper case', () => {
      ...
      expect(input.value.length === 8 && input.value[5].trim() === '').toEqual(
        true
      );
  ...
  describe('HTML attribute API testing', () => {
    ...
    it('Dynamically change attributes with htmlAttributes API', () => {
      ...
      expect(maskBox.element.getAttribute('placeholder')).toBe(
        'Number of states'
      );
      expect(maskBox.inputObj.container.getAttribute('title')).toBe('sample');
      expect(maskBox.inputObj.container.classList.contains('test')).toBe(true);
  ...
  describe('HTML attribute API dynamic testing', () => {
    ...
    it('Dynamically change attributes with htmlAttributes API', () => {
      ...
      expect(maskBox.inputObj.container.getAttribute('title')).toBe('sample');
      expect(maskBox.inputObj.container.classList.contains('test')).toBe(true);
      expect(maskBox.inputObj.container.getAttribute('style')).toBe(
      ...
      expect(maskBox.inputObj.container.classList.contains('multiple')).toBe(
        true
      );
    });
  ...
  describe('Readonly API at dynamic rendering', () => {
    ...
    it('Check readonly attribute with htmlAttributes API', () => {
      ...
      maskBox.appendTo('#mask1');
      (maskBox.htmlAttributes = { readonly: 'true' }), maskBox.dataBind();
      let input: HTMLInputElement = <HTMLInputElement>(
        document.getElementById('mask1')
      );
      expect(input.hasAttribute('readonly')).toEqual(true);
  ...
  describe('HTML attribute API at inital rendering and dynamic rendering', () => {
    ...
    it('Html attributes at initial rendering', () => {
      ...
      expect(maskBox.inputObj.container.classList.contains('sample')).toBe(
        true
      );
    });
  ...
  describe('Floating label MaskedTextBox', () => {
    ...
    it('Floating label MaskedTextBox - Auto', (done: Function) => {
      ...
      expect(input.parentElement.classList.contains('e-float-input')).toEqual(
        true
      );
      expect(
        input.parentElement.getElementsByTagName('label')[0].innerText ===
          'Enter card number'
      ).toEqual(true);
    ...
    it('Floating label MaskedTextBox - Never', () => {
      ...
      expect(
        input.parentElement.classList.contains('e-input-group') &&
          !input.parentElement.classList.contains('e-float-input')
      ).toEqual(true);
    ...
    it('position at focus in and focus out', () => {
      ...
      expect(
        input.parentElement
          .getElementsByTagName('label')[0]
          .classList.contains('e-label-bottom')
      ).toEqual(true);
      input.focus();
      input.selectionStart = input.selectionEnd = 0;
      input.parentElement
  ...
  describe('MaskedTextBox -  Clear Icon Support', () => {
    ...
    it('clear icon', () => {
      expect(
        mask.inputObj.clearButton.classList.contains('e-clear-icon-hide')
      ).toBe(true);
      document.getElementById('mask1').focus();
      expect(mask.inputObj.clearButton.classList.contains('e-clear-icon')).toBe(
        true
      );
    });
  ...
  describe('Event testing', () => {
    ...
    function FocusFn(args: MaskFocusEventArgs): void {
      ...
      if (
        args.container.classList.contains('e-input-focus') &&
...
describe('Masked Textbox without mask', function () {
  ...
  it('Characters equivalence testing', function () {
    ...
    expect(input.value).toBe(
      '10123456789101112131415161718192021222324252627282930313233343536373839404142434445464748495051525354555657585960616263646566676869707172737475767778798081828384858687888990919293949596979899'
    );
    expect(maskBox.getMaskedValue()).toBe(
      '10123456789101112131415161718192021222324252627282930313233343536373839404142434445464748495051525354555657585960616263646566676869707172737475767778798081828384858687888990919293949596979899'
    );
  });
});
describe('Dynamic CssClass testcase', function () {
  ...
  it('single css class', function () {
    ...
    expect(maskBox.inputObj.container.classList.contains('e-custom')).toBe(
      true
    );
  ...
  it('more than one css class', function () {
    ...
    expect(maskBox.inputObj.container.classList.contains('e-secondary')).toBe(
      true
    );
    ...
    expect(maskBox.inputObj.container.classList.contains('e-test')).toBe(true);
    expect(maskBox.inputObj.container.classList.contains('e-ternary')).toBe(
      true
    );
  });
});
describe('Width value with unit em', () => {
  ...
  it('Set the width to unit em', () => {
    ...
    maskBox.appendTo('#mask1');
    expect(
      document.getElementById('mask1').parentElement.style.width === '30em'
    ).toEqual(true);
    maskBox.width = '100px';
    maskBox.dataBind();
    expect(
      document.getElementById('mask1').parentElement.style.width === '100px'
    ).toEqual(true);
    maskBox.width = '50em';
    maskBox.dataBind();
    expect(
      document.getElementById('mask1').parentElement.style.width === '50em'
    ).toEqual(true);
    maskBox.width = '60%';
    maskBox.dataBind();
    expect(
      document.getElementById('mask1').parentElement.style.width === '60%'
    ).toEqual(true);
    maskBox.width = '30';
    maskBox.dataBind();
    expect(
      document.getElementById('mask1').parentElement.style.width === '30px'
    ).toEqual(true);
    maskBox.width = 50;
    maskBox.dataBind();
    expect(
      document.getElementById('mask1').parentElement.style.width === '50px'
    ).toEqual(true);
  });
  ...
  it('Set the width to unit %', () => {
    ...
    maskBox.appendTo('#mask1');
    expect(
      document.getElementById('mask1').parentElement.style.width === '30%'
    ).toEqual(true);
    maskBox.width = '30em';
    maskBox.dataBind();
    expect(
...
describe('EJ2-36604 - While giving the class name with empty space for HtmlAttributes, console error is produced.', function () {
  ...
  it('Entering the class name without any empty space', function () {
    ...
    maskBox.appendTo('#maskbox');
    expect(maskBox.inputObj.container.classList.contains('custom-class')).toBe(
      true
    );
  });
  ...
  it('Giving more than one empty space between two class names', function () {
    ...
    maskBox.appendTo('#maskbox');
    expect(
      maskBox.inputObj.container.classList.contains('custom-class-one')
    ).toBe(true);
    expect(
      maskBox.inputObj.container.classList.contains('custom-class-two')
    ).toBe(true);
  });
  ...
  it('Giving class name with underscore in the beginning', function () {
    ...
    maskBox.appendTo('#maskbox');
    expect(
      maskBox.inputObj.container.classList.contains('_custom-class-one')
    ).toBe(true);
    expect(
      maskBox.inputObj.container.classList.contains('_custom-class-two')
    ).toBe(true);
  });
...
describe('EJ2-54142', function () {
  ...
  it('Testing blur event and change event firing', function (done: Function) {
    ...
    clickEvent.initEvent('mousedown', true, true);
    expect(
      maskBox.inputObj.container.classList.contains('e-static-clear')
    ).toBe(true);
    expect(
      maskBox.inputObj.clearButton.classList.contains('e-clear-icon-hide')
    ).toBe(true);
    (<HTMLInputElement>(
      document.getElementsByClassName('e-clear-icon-hide')[0]
    )).dispatchEvent(clickEvent);
