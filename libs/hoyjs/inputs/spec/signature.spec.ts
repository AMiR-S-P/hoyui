...
} from '../src/common/signature-base';
...
describe('Signature', () => {
  ...
  describe('Properties', () => {
    ...
    it('backgroundImage', () => {
      signature = new Signature({
        backgroundImage: './theme-files/signature.jpg',
      });
      signature.appendTo('#signature');
      signature.dataBind();
      expect(signatureElement.style.backgroundImage).toEqual(
        'url("./theme-files/signature.jpg")'
      );
      signature.backgroundImage =
        'https://images.pexels.com/photos/20538303/pexels-photo-20538303.jpeg';
      signature.dataBind();
      expect(signatureElement.style.backgroundImage).toEqual(
        'url("https://images.pexels.com/photos/20538303/pexels-photo-20538303.jpeg")'
      );
    });
  ...
  describe('Events', () => {
    ...
    it('created event', () => {
      signature = new Signature({
        created: () => {
          expect(signatureElement.classList.contains('e-signature')).toEqual(
            true
          );
        },
      });
    ...
    it('change event', () => {
      ...
      signature.appendTo('#signature');
      mouseEventArs.clientX = 100;
      mouseEventArs.clientY = 100;
      mouseEventArs.buttons = 1;
      mouseEventArs.type = 'mousedown';
      (signature as any).mouseDownHandler(mouseEventArs);
      mouseEventArs.clientX = 130;
      mouseEventArs.clientY = 130;
      mouseEventArs.buttons = 1;
      mouseEventArs.type = 'mousemove';
      (signature as any).mouseMoveHandler(mouseEventArs);
      mouseEventArs.clientX = 150;
      mouseEventArs.clientY = 180;
      mouseEventArs.buttons = 1;
      mouseEventArs.type = 'mouseup';
    ...
    it('Keyboard event', () => {
      ...
      (signature as any).keyboardHandler(keyboardEventArgs);
      keyboardEventArgs.key = 'z';
      keyboardEventArgs.ctrlKey = true;
      (signature as any).keyboardHandler(keyboardEventArgs);
      keyboardEventArgs.key = 'y';
      keyboardEventArgs.ctrlKey = true;
      (signature as any).keyboardHandler(keyboardEventArgs);
    });
    ...
    it('coverage improvement-start draw', () => {
      ...
      (signature as any).calculateCurveControlPoints(
        { x: 100, y: 100 },
        { x: 101, y: 101 },
        { x: 102, y: 102 }
      );
    ...
    it('coverage improvement- temp background image', () => {
      signature = new Signature({});
      signature.appendTo('#signature');
      (signature as any).setBackgroundImage(
        'https://images.pexels.com/photos/20538303/pexels-photo-20538303.jpeg',
        'temp'
      );
    });
