...
import { isNullOrUndefined, EventHandler } from '@hoyui/base';
import { createSpinner, showSpinner } from '@hoyui/popups';
...
describe('Uploader Control', () => {
  ...
  describe('Basics', () => {
    ...
    it('element structure testing', () => {
      expect(
        uploadObj.element.parentElement.classList.contains('e-file-select')
      ).toBe(true);
    ...
    it('file selection validate', () => {
      let fileObj: File = new File(['Nice One'], 'sample.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let fileObj1: File = new File(['2nd File'], 'demo.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
        type: 'click',
        target: { files: [fileObj, fileObj1] },
        preventDefault: (): void => {},
      };
  ...
  describe('API testing for', () => {
    ...
    beforeAll((): void => {
      ...
      uploadObj = new Uploader({
        ...
        asyncSettings: {
          ...
          removeUrl:
            'http://aspnetmvc.syncfusion.com/services/api/uploadbox/Remove',
        },
      });
      uploadObj.appendTo(document.getElementById('upload'));
    });
    ...
    it('Auto Upload', () => {
      ...
      expect(uploadObj.uploadWrapper.querySelector('.e-file-upload-btn')).toBe(
        null
      );
    });
  });
  ...
  describe('prevent file list keyboard navigation testing', () => {
    ...
    it('uploader with disableKeyboardNavigation as false', () => {
      ...
      uploadObj.onSelectFiles(eventArgs);
      expect(
        uploadObj.fileList[0].querySelector('.e-file-remove-btn').tabIndex
      ).toEqual(0);
      expect(
        uploadObj.actionButtons.querySelector('.e-file-clear-btn').tabIndex
      ).toEqual(0);
      expect(
        uploadObj.actionButtons.querySelector('.e-file-upload-btn').tabIndex
      ).toEqual(0);
    });
    ...
    it('Preload files', () => {
      ...
      let liElements: HTMLElement[] =
        uploadObj.listParent.querySelectorAll('li');
      expect(liElements.length).toEqual(3);
      expect(liElements[0].querySelector('.e-file-status').textContent).toEqual(
        'File uploaded successfully'
      );
      expect(liElements[2].querySelector('.e-file-status').textContent).toEqual(
        'File uploaded successfully'
      );
      expect(
        liElements[1]
          .querySelector('.e-icons')
          .classList.contains('e-file-delete-btn')
      ).toBe(true);
      expect(
        liElements[0].querySelector('.e-icons').getAttribute('title')
      ).toEqual('Delete file');
      ...
      expect(
        uploadObj.fileList[0].querySelector('.e-file-delete-btn').tabIndex
      ).toEqual(0);
      expect(
        uploadObj.fileList[1].querySelector('.e-file-delete-btn').tabIndex
      ).toEqual(0);
      expect(
        uploadObj.actionButtons.querySelector('.e-file-clear-btn').tabIndex
      ).toEqual(0);
  ...
  describe('preload file testing', () => {
    ...
    it('preload files in single file upload', () => {
      ...
      expect(liElements[0].querySelector('.e-file-name').textContent).toEqual(
        'Books'
      );
      expect(liElements[0].querySelector('.e-file-status').textContent).toEqual(
        'File uploaded successfully'
  ...
  describe('preload file testing within form', () => {
    ...
    it('files with autoUpload false', () => {
      ...
      expect(
        isNullOrUndefined(
          uploadObj.uploadWrapper.querySelector('.e-file-upload-btn')
        )
      ).toBe(false);
      expect(
        isNullOrUndefined(
          uploadObj.uploadWrapper
  ...
  describe('cssClass Api testing', () => {
    ...
    it('cssClass testing single class', () => {
      ...
      expect(uploadObj.uploadWrapper.classList.contains('class1')).toEqual(
        true
      );
    });
    it('cssClass separated by comma', () => {
      ...
      expect(uploadObj.uploadWrapper.classList.contains('class2')).toEqual(
        true
      );
    });
  });
  ...
  describe('onProperty changes ', () => {
    ...
    it('enabled false ', () => {
      let fileObj1: File = new File(['2nd File'], 'image.png', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
        type: 'click',
        target: { files: [fileObj1] },
        preventDefault: (): void => {},
      };
      uploadObj.onSelectFiles(eventArgs);
      ...
      expect(uploadObj.uploadWrapper.classList.contains('e-disabled')).toBe(
        true
      );
    ...
    it('single file upload ', () => {
      ...
      uploadObj.dataBind();
      let fileObj: File = new File(['Nice One'], 'last.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let fileObj1: File = new File(['2nd File'], 'image.png', {
    ...
    it('drop area set value ', () => {
      ...
      uploadObj.dataBind();
      expect(uploadObj.dropAreaWrapper.querySelector('.e-file-drop'))
        .toBeUndefined;
    });
    ...
    it('change the save and remove urls ', () => {
      ...
      uploadObj.dataBind();
      expect(uploadObj.asyncSettings.saveUrl).toEqual(
        'https://services.syncfusion.com/js/production/api/FileUploader/Save'
      );
      expect(uploadObj.asyncSettings.removeUrl).toEqual(
        'https://services.syncfusion.com/js/production/api/FileUploader/Remove'
      );
    });
  ...
  describe('Angular tag', () => {
    ...
    it('Initial rendering', () => {
      ...
      expect(uploadObj.uploadWrapper.classList.contains('e-upload')).toBe(true);
      expect(uploadObj.uploadWrapper.parentElement.tagName).toBe(
        'EJS-UPLOADER'
      );
  ...
  describe('dropArea testing', () => {
    ...
    it('dropArea as parent element', () => {
      ...
      expect(
        uploadObj.dropAreaWrapper.querySelector('.e-file-drop').textContent
      ).toBe('Or drop files here');
    });
    it('drag enter ', () => {
      ...
      expect(
        uploadObj.dropZoneElement.classList.contains('e-upload-drag-hover')
      ).toBe(true);
    });
    it('drag leave ', () => {
  ...
  describe('Interactions', () => {
    ...
    it('buttons with text', () => {
      uploadObj = new Uploader({
        autoUpload: false,
        buttons: { browse: 'Choose File', clear: '', upload: 'Load File' },
      });
      uploadObj.appendTo(document.getElementById('upload'));
      let fileObj: File = new File(['demo file'], 'first.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
    ...
    it('name truncate', () => {
      ...
      element.textContent =
        'ChatLog 190841  _  Incident 188885 is not closed_ The drop down selected value is not selected_ 2017_11_16 15_34.rtf';
      let parentElement: HTMLElement = createElement('div', {
        id: 'Parentname',
      });
  ...
  describe('sorting', () => {
    ...
    it('sorting drop files', () => {
      ...
      uploadObj.onSelectFiles(eventArgs);
      expect(
        uploadObj.listParent.children[0].querySelector('.e-file-name')
          .textContent
      ).toBe('last');
    });
    it('2nd time drop files for sorting', () => {
      let fileObj: File = new File(['example'], 'awesews.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let fileObj1: File = new File(['demos'], 'first.png', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
      ...
      expect(
        uploadObj.listParent.children[3].querySelector('.e-file-name')
          .textContent
      ).toBe('first');
    });
  });
  ...
  describe('Events', () => {
    ...
    let onfileListRendering: EmitType<Object> =
      jasmine.createSpy('fileListRendering');
    ...
    it('Remove the selected files ', () => {
      let fileObj: File = new File(['Nice One'], 'demo.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let fileObj1: File = new File(['2nd File'], 'image.png', {
      ...
      let removeEventArgs = {
        type: 'click',
        target: uploadObj.fileList[0].children[1],
        preventDefault: (): void => {},
      };
    ...
    it('Rendering event triggered', function () {
      var fileObj = new File(['Nice One'], 'sample.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      var fileObj1 = new File(['2nd File'], 'demo.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      var eventArgs = {
  ...
  describe('Check spinner hided after upload canceled', () => {
    ...
    it('Cancel uploading', function (done) {
      ...
      var fileObj = new File(parts, 'last.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      var eventArgs = {
      ...
      uploadObj.upload([uploadObj.filesData[0]]);
      setTimeout(function () {
        let pausebtn = uploadObj.uploadWrapper
          .querySelectorAll('.e-upload-file-list ')[0]
          .querySelectorAll('.e-file-pause-btn')[0];
        ...
        let playbtn = uploadObj.uploadWrapper
          .querySelectorAll('.e-upload-file-list ')[0]
          .querySelectorAll('.e-file-play-btn')[0];
  ...
  describe('Check upload canceled', () => {
    ...
    it('Cancel uploading', function (done) {
      ...
      uploadObj.appendTo(document.getElementById('upload'));
      expect(uploadObj.buttons.browse.classList.contains('cancel')).toBe(true);
      let fileObj = new File(parts, 'last.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
      ...
      setTimeout(function () {
        let pausebtn = uploadObj.uploadWrapper
          .querySelectorAll('.e-upload-file-list ')[0]
          .querySelectorAll('.e-file-abort-btn')[0];
  ...
  describe('Event canceling ', () => {
    ...
    it('Removing event canceled', () => {
      let uploadObj1: any = new Uploader({
        autoUpload: false,
    ...
    it('clearing event canceled', () => {
      let uploadObj2: any = new Uploader({
        autoUpload: false,
        clearing: function (e: any) {
          e.cancel = true;
        },
      });
    ...
    it('Uploading event cancel', (done) => {
      ...
      uploadObj2.onSelectFiles(eventArgs);
      var pausebtn =
        uploadObj2.uploadWrapper.querySelectorAll('.e-file-upload-btn')[0];
      ...
      setTimeout(function () {
        var retrybtn =
          uploadObj2.uploadWrapper.querySelectorAll('.e-file-reload-btn')[0];
    ...
    it('Uploading event cancel in chunk upload', (done) => {
      ...
      let fileObj: File = new File(['Nice two'], 'sample.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
    ...
    it('args.response in failure event', (done) => {
      uploadObj = new Uploader({
        multiple: true,
        asyncSettings: { saveUrl: 'js.syncfusion.comm', chunkSize: 2 },
  ...
  describe('Methods', () => {
    ...
    it('remove the uploaded files', () => {
      ...
      expect(
        uploadObj.listParent.children[0].querySelector('.e-file-status')
          .textContent
      ).toBe('File uploaded successfully');
      expect(uploadObj.uploadedFilesData.length).toBe(1);
      createSpinner({
        target: uploadObj.uploadWrapper.querySelector(
          '.e-file-delete-btn'
        ) as HTMLElement,
      });
      showSpinner(
        uploadObj.uploadWrapper.querySelector(
          '.e-file-delete-btn'
        ) as HTMLElement
      );
  ...
  describe('Localization', () => {
    ...
    it('Button Text', () => {
      ...
      expect(
        uploadObj.uploadWrapper.querySelector('.e-file-drop').textContent
      ).toBe(localeText);
      let fileList = [
        {
          name: '7z938-x64.msi',
  ...
  describe('Allowed Extenstions and File size validation', () => {
    ...
    it('disable upload button supports', () => {
      uploadObj.autoUpload = false;
      uploadObj.dataBind();
      let fileObj: File = new File(['Nice One'], 'saple.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let fileObj1: File = new File(['2nd File'], 'dmo.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
  ...
  describe('File List Template UI', () => {
    ...
    beforeAll((): void => {
      ...
      document.body.appendChild(element);
      element.setAttribute('type', 'file');
      let item1 = createElement('span', {
        id: 'item1',
        className: 'select',
        innerHTML: 'Select',
      });
      document.body.appendChild(item1);
      let item2 = createElement('span', {
        id: 'item2',
        className: 'load',
        innerHTML: 'Load',
      });
      document.body.appendChild(item2);
      let item3 = createElement('span', {
        id: 'item3',
        className: 'clear',
      ...
      uploadObj = new Uploader({
        ...
        template:
          "<div class='wrapper'><table><tbody><tr><td><span class='file-name'>${name}</span></td><td><span class='file-size'>${size} bytes</span></td></tr></tbody></table></div>",
      });
      uploadObj.appendTo(document.getElementById('upload'));
    });
    afterAll((): void => {
    ...
    it('Initial template support', () => {
      ...
      uploadObj.createFileList(fileList);
      expect(
        uploadObj.uploadWrapper.querySelector('li').querySelector('.file-name')
          .textContent
      ).toEqual('7z938-x64.msi');
      expect(
        uploadObj.uploadWrapper.querySelector('li').querySelector('.file-size')
          .textContent
      ).toEqual('15 bytes');
    });
    ...
    it('Template changed dynamically', () => {
      uploadObj.template =
        "<span class='wrapper'></span><span class='icon file-icon ${type}'></span><span class='name file-name'>${name}</span>";
      uploadObj.dataBind();
      let fileObj1: File = new File(['2nd File'], 'demo.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
    ...
    it('uploadFailed method', () => {
      let fileObj: File = new File(
        ['Nicee1'],
        'ChatLog 192680_Dropdown value property binding is not working in case the dropdown contains large number.txt',
        { lastModified: 0, type: 'overide/mimetype' }
      );
      let eventArgs = {
        type: 'click',
        target: { files: [fileObj] },
  ...
  describe('File List Template UI with ID', () => {
    ...
    beforeAll((): void => {
      ...
      template.innerHTML =
        "<div class='wrapper'><table><tbody><tr><td><span class='file-name'>${name}</span></td><td><span class='file-size'>${size} bytes</span><span class='e-file-delete-btn e-spinner-pane' style='display:block;height:30px; width: 30px;'></span></td></tr></tbody></table></div>";
    ...
    it('customTemplate with uploader', () => {
      ...
      let event = {
        type: 'click',
        target: { files: [uploadObj.filesData[0]] },
        preventDefault: (): void => {},
      };
      createSpinner({
        target: uploadObj.uploadWrapper.querySelector('.e-file-delete-btn'),
      });
      showSpinner(uploadObj.uploadWrapper.querySelector('.e-file-delete-btn'));
      expect(
        isNullOrUndefined(
          uploadObj.uploadWrapper.querySelector('.e-spinner-pane')
        )
      ).toBe(false);
    });
  ...
  describe('KeyBoard Navigation', () => {
    ...
    it('enter key with upload button', () => {
      ...
      expect(iconElement[0].classList.contains('e-file-delete-btn')).toBe(
        false
      );
      // uploadObj.keyActionHandler(keyboardEventArgs);
      // setTimeout(() => {
    ...
    it('Enter key with clear icon ', () => {
      keyboardEventArgs.action = 'enter';
      keyboardEventArgs.target =
        uploadObj.fileList[1].querySelector('.e-icons');
  ...
  describe('worst case testing', () => {
    ...
    it('uploadRetry method', () => {
      ...
      let fileObj1: File = new File(['One'], 'demo.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArg = {
  ...
  describe('Default upload with', () => {
    ...
    it('Reload the canceled file', () => {
      ...
      uploadObj.removecanceledFile(null, uploadObj.filesData[0]);
      expect(uploadObj.pauseButton).not.toBe(undefined);
      uploadObj.reloadcanceledFile(
        null,
        uploadObj.filesData[0],
  ...
  describe('unused test cases', () => {
    ...
    it('removeFailed method', () => {
      ...
      expect(
        uploadObj.listParent.querySelector('.e-file-status').textContent
      ).toBe('Unable to remove file');
    });
    it('uploadFailed method', () => {
      let fileObj: File = new File(
        ['Nice One'],
  ...
  describe('Tooltip support', () => {
    ...
    it('Ensure title attributes', () => {
      expect(uploadObj.browseButton.getAttribute('title')).toEqual(
        'Feuilleter'
      );
      let fileList = [
        {
      ...
      uploadObj.createFileList(fileList);
      uploadObj.renderActionButtons();
      expect(uploadObj.uploadButton.getAttribute('title')).toEqual(
        'télécharger'
      );
  ...
  describe('Form support', () => {
    ...
    beforeAll((): void => {
      ...
      let form: Element = createElement('form', { attrs: { id: 'form1' } });
      let submitButton: HTMLElement = createElement('button', {
        attrs: { type: 'submit' },
      });
      form.appendChild(element);
    ...
    it('name attribute', () => {
      let fileObj: File = new File(['Nice One'], 'sample2.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
      ...
      uploadObj.onSelectFiles(eventArgs);
      let element: HTMLFormElement = <HTMLFormElement>(
    ...
    it('name attribute with isModified', () => {
      let fileObj: File = new File(['One'], 'sample.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
      ...
      let fileObj1: File = new File(['Nice'], 'sample1.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs1 = {
  ...
  describe('HTML attributes at inline element testing', () => {
    ...
    it('Inline and html attributes API testing', () => {
      uploadObj = new Uploader({
        htmlAttributes: { multiple: 'false', disabled: 'true' },
      });
      uploadObj.appendTo(document.getElementById('upload'));
      expect(uploadObj.multiple).toBe(false);
      expect(uploadObj.uploadWrapper.classList.contains('e-disabled')).toBe(
    ...
    it('Inline, API and html attributes API testing', () => {
      uploadObj = new Uploader({
        htmlAttributes: { multipe: 'true', disabled: 'true' },
        multiple: false,
        enabled: true,
      });
      uploadObj.appendTo(document.getElementById('upload'));
  ...
  describe('Chunk upload pause & resume public methods', () => {
    ...
    beforeAll((): void => {
      ...
      uploadObj = new Uploader({
        ...
        template:
          "<span class='wrapper'><span class='icon template-icons sf-icon-${type}'></span>" +
    ...
    it('Pause, resume', (done) => {
      uploadObj.appendTo(document.getElementById('upload'));
      let fileObj: File = new File(['pause upload'], 'pauseData.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
    ...
    it('Remove the paused file', (done) => {
      let fileObj: File = new File(['remove pause'], 'removePauseData.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
  ...
  describe('Form support', () => {
    ...
    it('Clearing values', () => {
      ...
      let fileObj1: File = new File(['Nice One'], 'sample2.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
  ...
  describe('Form support', () => {
    ...
    beforeAll((): void => {
      ...
      let form: Element = createElement('form', { attrs: { id: 'form1' } });
      let resetButton: HTMLElement = createElement('button', {
        attrs: { type: 'reset', id: 'reset' },
      });
      form.appendChild(element);
  ...
  describe('Cancel & retry public methods for', () => {
    ...
    it('template UI upload', (done) => {
      ...
      // Construct a file
      let fileObj: File = new File(parts, 'BlobFile.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
  ...
  describe('Cancel & retry public methods for', () => {
    ...
    it('default UI upload', (done) => {
      ...
      // Construct a file
      let fileObj: File = new File(parts1, 'BlobFile1.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
  ...
  describe('Sequential Upload testing on uploading', () => {
    ...
    it('Dynamically enable sequential upload', (done) => {
      ...
      uploadObj.appendTo(document.getElementById('upload'));
      let fileObj: File = new File(['Nice One'], 'sample1.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let fileObj1: File = new File(['Nice One'], 'sample2.txt', {
    ...
    it('upload with autoupload false', (done) => {
      uploadObj = new Uploader({
        asyncSettings: {
          ...
          removeUrl:
            'https://aspnet.syncfusion.com/services/api/uploadbox/Remove',
        },
        failure: function (args: any) {
      ...
      setTimeout(() => {
        ...
        expect(uploadObj.filesData[0].statusCode).toBe('0');
        expect(
          uploadObj.fileList[0].querySelector('.e-file-status').innerHTML
        ).toBe('upload failed');
        done();
      }, 1000);
    });
  ...
  describe('Sequential Upload testing on canceling', () => {
    ...
    it('sequential upload with abort icon clicked', (done) => {
      ...
      uploadObj.appendTo(document.getElementById('upload'));
      let fileObj: File = new File(parts1, 'sample1.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let fileObj1: File = new File(parts1, 'sample2.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
  ...
  describe('Action complete event in uploader', () => {
    ...
    it('action complete event on upload failed', (done) => {
      uploadObj = new Uploader({
        asyncSettings: {
          saveUrl:
            'https://services.syncfusion.com/js/production/api/FileUploader/Saved',
          removeUrl:
            'https://services.syncfusion.com/js/production/api/FileUploader/Removed',
        },
        autoUpload: false,
        actionComplete: onComplete,
      });
      uploadObj.appendTo(document.getElementById('upload'));
      let fileObj2: File = new File(['Nice One'], 'sample1.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let fileObj3: File = new File(['Nice One'], 'sample2.txt', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      let eventArgs = {
        type: 'click',
        target: { files: [fileObj2, fileObj3] },
        preventDefault: (): void => {},
      };
      uploadObj.onSelectFiles(eventArgs);
      expect(uploadObj.getFilesData().length).toEqual(2);
  ...
  describe('File name truncating in uploader', () => {
    ...
    it('name truncate', () => {
      let element: HTMLElement = createElement('div', { id: 'name' });
      element.textContent =
        'This is testing file for truncation functionality in uploader.rtf';
      let parentElement: HTMLElement = createElement('div', {
        id: 'Parentname',
      });
      parentElement.style.width = '250px';
  ...
  describe('File List Template UI for preload files', () => {
    ...
    beforeAll((): void => {
      ...
      uploadObj = new Uploader({
        files: preLoadFiles,
        template:
          "<div class='wrapper'><table><tbody><tr><td><span class='file-name'>${name}</span></td><td><span class='file-size'>${size} bytes</span><span class='upload-status'>${status}</span></td></tr></tbody></table></div>",
        rendering: function (args: any) {
          if (args.isPreload) {
            args.element.querySelector('.upload-status').innerHTML =
              'Status Customized';
          }
        },
      });
      ...
      setTimeout(() => {
        ...
        expect(
          uploadObj.uploadWrapper
            ...
            .querySelector('.upload-status').innerHTML
        ).toBe('Status Customized');
        expect(
          uploadObj.uploadWrapper
  ...
  describe('Form support - UI changes - Single File', () => {
    ...
    it('ensure form support UI changes with valid files', (done) => {
      ...
      expect(uploadObj.getFilesData()[0].input.getAttribute('name')).toEqual(
        'upload'
      );
      expect(
        uploadObj.getFilesData()[0].list.classList.contains('e-file-invalid')
      ).toBe(false);
      expect(
        uploadObj.getFilesData()[0].list.querySelector('.e-file-status')
      ).toBe(null);
      expect(
        uploadObj.getFilesData()[0].list.querySelector('.e-file-information')
      ).toBe(null);
      expect(
        uploadObj.getFilesData()[0].list.querySelectorAll('.e-file-size').length
      ).toBe(1);
  ...
  describe('Form support - UI changes - Single File - Invalid', () => {
    ...
    it('ensure form support UI changes with invalid files', (done) => {
      ...
      expect(
        uploadObj.getFilesData()[0].list.querySelectorAll('.e-file-status')
          .length
      ).toBe(1);
      expect(
        uploadObj.getFilesData()[0].list.querySelector('.e-file-information')
      ...
      expect(
        uploadObj.getFilesData()[0].list.querySelector('.e-file-size')
      ).toBe(null);
      expect(uploadObj.uploadWrapper.querySelectorAll('input').length).toBe(1);
      uploadObj.uploadWrapper.querySelector('.e-file-remove-btn').click();
      expect(uploadObj.getFilesData().length).toEqual(0);
  ...
  describe('Form support - UI changes - Multiple File', () => {
    ...
    it('ensure form support UI changes with valid files', (done) => {
      ...
      expect(
        uploadObj.getFilesData()[0].list.querySelectorAll('.e-file-information')
          .length
      ).toBe(1);
      expect(
        uploadObj.getFilesData()[0].list.querySelector('.e-file-size')
  ...
  describe('Form support - UI changes - Multiple File - Invalid /valid combination', () => {
    ...
    it('ensure form support UI changes with valid/invalid files', (done) => {
      var fileObj = new File(['Nice One'], 'sample.pdf', {
        lastModified: 0,
        type: 'overide/mimetype',
      });
      var fileObj1 = new File(['2nd File'], 'demo.txt', {
  ...
  describe('EJ2-36604 - While giving the class name with empty space for HtmlAttributes, console error is produced.', function () {
    ...
    it('Entering the class name without any empty space', function () {
      ...
      expect(uploadObj.uploadWrapper.classList.contains('custom-class')).toBe(
        true
      );
    });
    ...
    it('Giving more than one empty space between two class names', function () {
      ...
      uploadObj.appendTo('#uploader');
      expect(
        uploadObj.uploadWrapper.classList.contains('custom-class-one')
      ).toBe(true);
      expect(
        uploadObj.uploadWrapper.classList.contains('custom-class-two')
      ).toBe(true);
    });
    ...
    it('Giving class name with underscore in the beginning', function () {
      ...
      uploadObj.appendTo('#uploader');
      expect(
        uploadObj.uploadWrapper.classList.contains('_custom-class-one')
      ).toBe(true);
      expect(
        uploadObj.uploadWrapper.classList.contains('_custom-class-two')
      ).toBe(true);
    });
  ...
  describe('EJ2-31022 - Provide support for enable and disable the drag and drop option in Uploader', function () {
    ...
    it('No dropArea value is given while rendering', function () {
      ...
      uploadObj.appendTo('#uploader');
      expect(
        !isUndefined(uploadObj.dropAreaWrapper.querySelector('.e-file-drop'))
      ).toBe(true);
      expect(
        uploadObj.dropAreaWrapper.querySelector('.e-file-drop').textContent
      ).toEqual('Or drop files here');
    ...
    it('dropArea value is given as custom-HTMLelement after rendering and give localization also as dynamic', function () {
      ...
      document.body.appendChild(dropElement);
      uploadObj.dropArea = document.getElementById('dropele');
      (uploadObj.locale = 'fr-BE'), uploadObj.dataBind();
      expect(uploadObj.dropAreaWrapper.querySelector('.e-file-drop'))
        .toBeUndefined;
      expect(uploadObj.browseButton.innerText).toEqual('Feuilleter');
      expect(!isUndefined(uploadObj.dropZoneElement)).toBe(true);
