import { INotifyPropertyChanged, NotifyPropertyChanges, Component, Property, getUniqueID, isNullOrUndefined, addClass, attributes, removeClass, remove, EmitType, Event, EventHandler } from '@hoyui/base';
...
export class OtpInput extends Component<HTMLElement> implements INotifyPropertyChanged {
    ...
    private updateSeparatorValue(): void {
        if (this.separator === '') {
            for (let i: number = 0; i < this.separatorElements.length; i++) {
    ...
    private updateVariantClass(): void {
        let variantClass: string = this.stylingMode.toLocaleLowerCase();
