...
export class QueryLibrary {
  ...
  private queryLibrary(args?: {
    ...
    switch (args.prop) {
      case 'getMongoFromRules':
        args.value['obj']['mongoQuery'] = this.getMongoFromRules(
          args.value['rule'],
          args.value['mongoQuery']
        );
        break;
      case 'mongoParser':
        this.mongoParser(
      ...
      case 'getNamedParameterSql':
        args.value['obj']['sql'] = this.getNamedParameterSql(
          args.value['rule']
        );
        break;
      ...
      case 'convertNamedParamSqlToSql':
        args.value['obj']['sql'] = this.convertNamedParamSqlToSql(
          args.value['sql']
        );
        break;
    }
        ...
        case 'in':
          if (item.type === 'string') {
            if (item.value.length > 1) {
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              let s: any = item.value
                .map((x: number, j: number) =>
                  j < item.value.length ? `"${x}"` : ''
                )
                .toString();
  ...
  private mongoRecursion(
    ...
    for (let i: number = 0, len: number = mongoList.length; i < len; i++) {
      ...
      if (condition === '$and') {
        ...
        rules.push(subRules);
        this.mongoRecursion(
          mongoList[i as number][condition as string],
          rules[(rules.length as number) - 1].rules,
          mongoLocale
        );
        ...
        let firstValue: any = [];
        let secondValue: any = [];
        let innerFirstValue: any = [];
        let innerSecondValue: any = [];
        if (Array.isArray(value) && value.length === 2) {
          ...
          if (typeof innerFirstValue === 'object') {
            ...
            innerKeys = Object.keys(
              innerFirstValue[Object.keys(innerFirstValue)[0]]
            );
            firstKey = innerKeys[0];
            secondKey = Object.keys(
              innerSecondValue[Object.keys(innerSecondValue)[0]]
            )[0];
        ...
        if (typeof value === 'number') {
          ...
          for (let i: number = 0; i < keyObj.length; i++) {
            ...
            if (key === '$ne' && typeof stringValue === 'boolean') {
              // not equal operator for boolean
              operatorValue = 'notequal';
            ...
            if (keyObj.length >= 2 && keyObj[i as number]) {
              if (typeof stringValue == 'object') {
                ...
                betweenValue = [
                  Object.keys(stringValue[condition as string])[0],
                ];
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                (betweenOperatorArray as string[]).push(
                  stringValue[condition as string][betweenValue as any]
                );
                type = 'number';
              } else {
        ...
        if (betweenOperatorArray && betweenOperatorArray.length > 1) {
          // between opertor value
          rule = {
            field: condition,
            label: condition,
            value: betweenOperatorArray,
            operator: operatorValue,
            type: type,
          };
  ...
  private convertParamSqlToSql(sql: ParameterizedSql): string {
    const paramSql: string = sql.sql;
    const paramValues: object[] = sql.params;
    const parts: string[] = paramSql.split('?');
    let normalSql: string = parts[0];
    for (let i: number = 0; i < paramValues.length; i++) {
      normalSql +=
        typeof paramValues[i as number] === 'string'
          ? `'${paramValues[i as number]}'` + parts[i + 1]
          : paramValues[i as number] + parts[i + 1];
    }
    if (
      normalSql.length >= 2 &&
      normalSql[0] === '(' &&
      normalSql[normalSql.length - 1] === ')'
    ) {
      normalSql = normalSql.slice(1, -1);
    }
    normalSql = normalSql
      .replace(/!= ''(?! =)/g, 'IS NOT EMPTY')
      .replace(/= ''/g, 'IS EMPTY');
    return normalSql;
  }
  private convertNamedParamSqlToSql(sql: ParameterizedNamedSql): string {
    const namedParamSql: string = sql.sql;
    const params: Record<string, object> = sql.params;
  ...
  private getParameterSql(qbrule: RuleModel): ParameterizedSql {
    ...
    return this.getParameterSQLVal(
      this.parent.getSqlFromRules(qbRule),
      value['ruleVal']
    );
  }
  private getNamedParameterSql(qbrule: RuleModel): ParameterizedNamedSql {
    ...
    return this.getNamedParameterSQLVal(
      this.parent.getSqlFromRules(qbRule),
      value['namedRuleVal']
    );
  }
  ...
  private getNamedParameterSQLVal(
    content: string,
    ruleValue: Record<string, object>
  ): ParameterizedNamedSql {
    const replacedString: string = content.replace(/[%']/g, '');
    return { sql: '(' + replacedString + ')', params: ruleValue };
  }
  ...
  private updateRuleValue(rule: RuleModel, isNamedParameter: boolean): any {
    const ruleVal: object[] = [];
    const namedRuleVal: Record<string, object> = {};
  ...
  private updateValue(
    ...
    ruleVal: object[],
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    namedRuleVal: Record<string, object>,
    ...
    for (let i: number = 0; i < rules.length; i++) {
      if (rules[i as number].rules) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const value: any = this.updateValue(
          rules[i as number].rules,
          isNamedParameter,
          ruleVal,
          namedRuleVal,
          namedParameters
        ...
        if (rules[i as number].value instanceof Array) {
          ...
          for (
            let j: number = 0;
            j < (rules[i as number].value as any).length;
            j++
          ) {
            if (isNamedParameter) {
              namedField = this.getNamedParameter(
                rules[i as number].field,
                namedParameters
              );
            }
            if (!isNullOrUndefined(rules[i as number].value[j as number])) {
              if (
                rules[i as number].type === 'string' ||
                rules[i as number].type === 'date'
              ) {
                if (isNamedParameter) {
                  namedRuleVal[namedField as string] =
            ...
            if (
              ...
              if (
                ...
                if (!isNullOrUndefined(rules[i as number].value)) {
                  if (isNamedParameter) {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    namedRuleVal[namedField as string] = rules[i as number]
                      .value as any;
                  } else {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
