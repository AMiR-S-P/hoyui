...
import {
  ...
  CheckBox,
  RadioButton,
  ChangeEventArgs as ButtonChangeEventArgs,
  RadioButtonModel,
} from '@hoyui/buttons';
import {
  DropDownList,
...
import { Query, Predicate, DataManager, Deferred } from '@hoyui/data';
import {
  TextBox,
  NumericTextBox,
  InputEventArgs,
...
import { TextBoxModel, NumericTextBoxModel } from '@hoyui/inputs';
import {
  DatePicker,
  ChangeEventArgs as CalendarChangeEventArgs,
  DatePickerModel,
} from '@hoyui/calendars';
import { DropDownButton, ItemModel, MenuEventArgs } from '@hoyui/splitbuttons';
import {
  Tooltip,
  createSpinner,
  showSpinner,
...
import { compile as templateCompiler, getNumericObject } from '@hoyui/base';
  ...
  implements INotifyPropertyChanged
{
  ...
  public reset(): void {
    ...
    const inputElement: NodeListOf<HTMLElement> = this.element.querySelectorAll(
      'input.e-control'
    ) as NodeListOf<HTMLElement>;
    for (let i: number = 0, len: number = inputElement.length; i < len; i++) {
      if (inputElement[i as number].className.indexOf('e-tooltip') > -1) {
        (
          getComponent(inputElement[i as number], 'tooltip') as Tooltip
        ).destroy();
      } else if (
        inputElement[i as number].parentElement.className.indexOf('e-tooltip') >
        -1
      ) {
        (
          getComponent(
            inputElement[i as number].parentElement,
            'tooltip'
          ) as Tooltip
        ).destroy();
      }
    ...
    if (bodyElem) {
      ...
      const grpContainer: HTMLElement = this.createElement('div', {
        attrs: { class: 'e-group-container' },
      });
      const grpHeader: HTMLElement = this.createElement('div', {
        attrs: { class: 'e-group-header' },
      });
      const grpBody: HTMLElement = this.createElement('div', {
        attrs: { class: 'e-group-body' },
      });
    ...
    if (this.enableNotCondition) {
      removeClass(
        this.element.querySelectorAll('.e-qb-toggle'),
        'e-active-toggle'
      );
    }
    bodyElem.appendChild(
      this.createElement('div', { attrs: { class: 'e-rule-list' } })
    );
    this.levelColl[this.element.id + '_group0'] = [0];
    this.setProperties(
      { rule: { condition: 'and', not: false, rules: [] } },
      true
    );
      ...
      if (this.columns.length) {
        ...
        for (let i: number = 0, len: number = columns.length; i < len; i++) {
          ...
          if (columns[i as number].category) {
            ...
            columns[i as number].category =
              this.l10n.getConstant('OtherFields');
          }
          if (categories.indexOf(columns[i as number].category) < 0) {
            categories.push(columns[i as number].category);
          }
          if (
            !columns[i as number].operators ||
            (this.isLocale &&
              this.isCustomOprCols.indexOf(columns[i as number].field) !== 0)
          ) {
            columns[i as number].operators =
              this.customOperators[columns[i as number].type + 'Operator'];
          }
        }
        if (
          groupBy &&
          (categories.length > 1 ||
            categories[0] !== this.l10n.getConstant('OtherFields'))
        ) {
        ...
        for (let i: number = 0, len: number = columnKeys.length; i < len; i++) {
          ...
          cols[i as number] = {
            field: columnKeys[i as number],
            label: columnKeys[i as number],
            type: isDate ? 'date' : type,
            validation: validateObj,
          } as Columns;
          isDate = false;
          cols[i as number].operators =
            this.customOperators[cols[i as number].type + 'Operator'];
          if (type === 'object') {
  ...
  private updateSubFieldsFromColumns(
    ...
    for (let i: number = 0; i < col.length; i++) {
      if (
        this.separator !== '' &&
        col[i as number].field.indexOf(this.separator) < 0
      ) {
        col[i as number].field = field
          ? field + this.separator + col[i as number].field
          : col[i as number].field;
      }
      if (col[i as number].operators) {
        ...
        col[i as number].operators =
          this.customOperators[col[i as number].type + 'Operator'];
      }
      if (col[i as number].columns) {
        col[i as number].type = 'object';
        this.updateSubFieldsFromColumns(
          col[i as number].columns,
          col[i as number].field
        );
      }
    }
  ...
  private updateSubFields(
    ...
    let sampCol: ColumnsModel;
    col.columns = [];
    const columnKeys: string[] = Object.keys(value as Object);
    let field: string;
    let label: string;
    let type: string;
    let result: Object;
    ...
    for (let i: number = 0, len: number = columnKeys.length; i < len; i++) {
      ...
      const resData: Object =
        data[
          col.field.split(this.separator)[
            col.field.split(this.separator).length - 1
          ]
        ];
      type = typeof result;
      field = col.field + this.separator + columnKeys[i as number];
      label = columnKeys[i as number];
      type =
        type === 'object' && !isNaN(Date.parse(result as string))
          ? 'date'
          : type;
  ...
  private updateCustomOperator(column: ColumnsModel, from?: string): void {
    if (column.operators) {
      if (
        !this.isLocale &&
        from === 'initial' &&
        !isNullOrUndefined(this.isCustomOprCols)
      ) {
        this.isCustomOprCols.push(column.field);
      }
      for (let j: number = 0; j < column.operators.length; j++) {
        const sqlIdx: number = Object.keys(
          column.operators[j as number]
        ).indexOf('sqlOperator');
        if (sqlIdx > -1) {
          const operator: { [key: string]: object } =
            column.operators[j as number];
          const operColl: string[] = Object.keys(operator);
          const values: string[] = operColl
            .map((key: string) => operator[`${key}`])
            .join(',')
            .split(',');
  ...
  private clickEventHandler(event: MouseEventArgs): void {
    ...
    if (target.className.indexOf('e-collapse-rule') > -1) {
      ...
      const summaryElem: HTMLElement = document.getElementById(
        this.element.id + '_summary_content'
      );
      const txtareaElem: HTMLElement =
        summaryElem.querySelector('.e-summary-text');
      animation.animate('.e-query-builder', { name: 'SlideLeftIn' });
      const groupElem: HTMLElement = this.element.querySelector(
        '.e-group-container'
      ) as HTMLElement;
    ...
    if (
      ...
      target.className.indexOf('e-qb-toggle') < 0
    ) {
      const animation: Animation = new Animation({ duration: 1000, delay: 0 });
      switch (true) {
      ...
      (target.tagName === 'LABEL' &&
        target.parentElement.className.indexOf('e-btn-group') > -1) ||
      ...
      if (!this.headerTemplate) {
        ...
        if (this.enableNotCondition) {
          if (target.className.indexOf('e-qb-toggle') > -1) {
            const toggleElem: Element =
              element.getElementsByClassName('e-qb-toggle')[0];
      ...
      if (!this.isImportRules) {
        this.trigger(
          'beforeChange',
          args,
          (observedChangeArgs: ChangeEventArgs) => {
  ...
  private beforeSuccessCallBack(args: ChangeEventArgs, target: Element): void {
    if (args && !args.cancel) {
      ...
      if (this.enableSeparateConnector) {
        if (
          isNullOrUndefined(closest(target, '.e-rule-container')) &&
          element.classList.contains('e-group-container')
        ) {
          element =
            target.parentElement.previousElementSibling !== null
      ...
      if (this.enableSeparateConnector) {
        const andElem: HTMLInputElement = target.parentElement.querySelector(
          '.e-btngroup-and'
        ) as HTMLInputElement;
        const orElem: HTMLInputElement = target.parentElement.querySelector(
          '.e-btngroup-or'
        ) as HTMLInputElement;
        if (andElem && orElem) {
          if (args.value === 'and') {
  ...
  private selectBtn(target: Element, event: MenuEventArgs): void {
    if (event.name === 'beforeOpen') {
      if (
        this.showButtons.groupInsert ||
        isNullOrUndefined(this.showButtons.groupInsert)
      ) {
        if (
          this.element.querySelectorAll('.e-group-container').length >=
          this.maxGroupCount + 1
        ) {
          addClass(
            [event.element.querySelector('li span.e-addgroup').parentElement],
            'e-button-hide'
          );
        } else {
          removeClass(
  ...
  private appendRuleElem(
    ...
    let elem: Element;
    const ruleListElem: Element = target.querySelector('.e-rule-list');
    let args: ActionEventArgs;
    if (type === 'change') {
      ruleElem = select('#' + parentId, target);
    } else {
      ruleElem = this.createElement('div', {
        attrs: { class: 'e-rule-container' },
      });
      ...
      if (this.showButtons.cloneRule && this.cloneRuleBtnClick) {
        if (this.ruleIndex < 0) {
          ...
          if (this.enableSeparateConnector) {
            let index: number = -1;
            let tempRuleIndex: number = this.ruleIndex + 1;
            for (let i: number = 0; i < tempRuleIndex; i++) {
              if (i === (ruleListElem.children as HTMLCollection).length) {
                break;
              }
              if (
                ruleListElem.children[i as number].classList.contains(
                  'e-rule-container'
                )
              ) {
                tempRuleIndex++;
            ...
            ruleListElem.insertBefore(
              ruleElem,
              ruleListElem.children[this.ruleIndex + index + 1]
            ); // added clone rule to next position
          } else {
            ruleListElem.insertBefore(
              ruleElem,
              ruleListElem.children[this.ruleIndex + 1]
            ); // added clone rule to next position
          }
        }
    ...
    if (column && column.ruleTemplate && rule) {
      ...
      const templateID: string = this.element.id + column.field;
      ...
      if ((this as any).isReact) {
        ...
        const templateColl: Element[] = this.ruleTemplateFn(
          args,
          this,
          ruleElem.id,
          templateID
        ...
        template =
          templateColl[0].nodeType === 3 ? templateColl[1] : templateColl[0];
        elem = template;
        elem.className += ' e-rule-field';
        ruleElem.appendChild(elem);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        if (ruleElem.children.length) {
          ruleElem.children[ruleElem.children.length - 1].className +=
            ' e-rule-field';
        }
      } else {
      ...
      elem = this.ruleElem
        .querySelector('.e-rule-field')
        .cloneNode(true) as Element;
      ruleElem.appendChild(elem);
    }
    if (this.showButtons.lockGroup) {
      removeClass(
        ruleElem.querySelectorAll('.e-lock-grp-btn'),
        'e-button-hide'
      );
    }
    if (this.showButtons.lockRule) {
      removeClass(
        ruleElem.querySelectorAll('.e-lock-rule-btn'),
        'e-button-hide'
      );
    }
    if (this.showButtons.cloneGroup) {
      removeClass(
        ruleElem.querySelectorAll('.e-clone-grp-btn'),
        'e-button-hide'
      );
    }
    if (this.showButtons.cloneRule) {
      removeClass(
        ruleElem.querySelectorAll('.e-clone-rule-btn'),
        'e-button-hide'
      );
    }
  ...
  private addRuleSuccessCallBack(
    ...
    const height: string =
      this.element.className.indexOf('e-device') > -1 ? '250px' : '200px';
    let ruleID: string;
    const column: ColumnsModel =
      rule && rule.field
        ? this.getColumn(rule.field)
        : col
        ? col
        : this.columns[0];
    ...
    if (!args.cancel) {
      if (column && column.ruleTemplate && rule.field) {
        ...
        ruleElem
          .querySelector('.e-filter-input')
          .setAttribute('id', ruleElem.id + '_filterkey');
        const element: Element = ruleElem.querySelector('.e-rule-delete');
        if (
          this.element.className.indexOf('e-device') > -1 ||
      ...
      if (this.enableSeparateConnector) {
        ...
        if (
          previousRuleElem &&
          previousRuleElem.classList.contains('e-group-container')
        ) {
          ruleContainer =
            previousRuleElem.querySelectorAll('.e-rule-container');
          previousRuleElem = ruleContainer[ruleContainer.length - 1];
        }
        if (
          previousRuleElem &&
          previousRuleElem.classList.contains('e-rule-container')
        ) {
          prevRule = this.getRule(previousRuleElem as HTMLElement);
        }
        if (this.headerTemplate && previousRuleElem && prevRule) {
          this.headerTemplateFn(
            previousRuleElem,
            false,
            prevRule.condition,
            prevRule,
          ...
          isNullOrUndefined(previousRuleElem) &&
          ruleElem.id !== this.element.id + '_group0_rule0'
        ) {
          ...
          if (group && group.previousElementSibling) {
            ...
            const prevRuleContainer: NodeListOf<Element> =
              prevElem.querySelectorAll('.e-rule-container');
        ...
        if (
          previousRuleElem &&
          previousRuleElem.className.indexOf('e-rule-container') > -1
        ) {
      ...
      if (
        previousRuleElem &&
        previousRuleElem.className.indexOf('e-group-container') > -1 &&
        ruleElem.className.indexOf('e-separate-rule') < 0
      ) {
        ruleElem.className += ' e-separate-rule';
      }
      if (!this.isImportRules) {
        this.updateAddedRule(
          trgt,
          rule,
          newRule,
          isRlTmp,
          pId,
          this.enableSeparateConnector ? true : null
        );
      }
      if (!column || (column && !column.ruleTemplate) || !rule.field) {
        if (this.fieldMode === 'Default') {
          ...
          if (this.separator && rule.field) {
            ...
            ddlValue = this.isImportRules
              ? this.GetRootColumnName(rule.field as string)
              : rule.field;
          }
          ddlField = {
          ...
          if (this.fieldModel) {
            ddlField = {
              ...ddlField,
              ...(this.fieldModel as DropDownListModel),
            };
          }
          ...
          if (this.separator && rule.field) {
            ...
            ddlVal = this.isImportRules
              ? this.GetRootColumnName(rule.field as string)
              : (dropDownList.value as string | number | boolean);
          }
          this.selectedColumn = dropDownList.getDataByValue(
            ddlVal
          ) as ColumnsModel;
          ...
          const ddlValue: string = this.isImportRules
            ? (rule.field as string)
            : rule.field;
          this.dummyDropdownTreeDs = extend([], this.columns, [], true) as {
            [key: string]: Object;
          }[];
          this.updateDropdowntreeDS(
            this.dummyDropdownTreeDs as { [key: string]: Object }[]
          );
          ddlField = {
            fields: {
              dataSource: this.dummyDropdownTreeDs as {
                [key: string]: Object;
              }[],
              value: 'field',
              text: 'label',
          ...
          if (this.fieldModel) {
            ddlField = {
              ...ddlField,
              ...(this.fieldModel as DropDownTreeModel),
            };
          }
          ...
          if (!isNullOrUndefined(dropdowntree.value)) {
            const value: string = this.getLabelFromColumn(
              dropdowntree.value[0]
            );
            (dropdowntree.element as HTMLInputElement).value = value;
          }
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const ddlVal: string | number | boolean | any = !isNullOrUndefined(
            rule.field
          )
            ? this.GetRootColumnName(rule.field as string)
            : dropdowntree.value;
      ...
      if (rule && rule.isLocked) {
        const lockRuleTarget: HTMLElement =
          ruleElem.querySelector('.e-lock-rule-btn');
      ...
      if (this.headerTemplate && prevElem) {
        ...
        if (
          ...
          let prevElem: Element =
            group.previousElementSibling.previousElementSibling;
          if (prevElem.classList.contains('e-group-container')) {
            const ruleContainer: NodeListOf<Element> =
              prevElem.querySelectorAll('.e-rule-container');
            prevElem = ruleContainer[ruleContainer.length - 1];
          }
          if (prevElem.classList.contains('e-rule-container')) {
  ...
  private dropdownTreeFiltering(args: DdtFilteringEventArgs): void {
    ...
    const inputElem: HTMLElement = isClearIcon
      ? srcElement.parentElement.querySelector('.e-textbox')
      : srcElement;
    ruleElemID = inputElem.id.split('_filterkey')[0];
    const ruleElem: HTMLElement = document.getElementById(ruleElemID);
    this.ddTree = getComponent(
      ruleElem.querySelector('input.e-dropdowntree') as HTMLElement,
      'dropdowntree'
    );
    const hierarchicalData: ColumnsModel[] = extend(
      [],
      this.columns,
      [],
      true
    ...
    if (args.text === '') {
      ...
      const matchedDataSource: { [key: string]: Object }[] = hierarchicalData
        .map((data: ColumnsModel): { [key: string]: Object } =>
          this.nestedChildFilter(args.text, data as { [key: string]: Object })
        )
        .filter(
          (filteredChild: { [key: string]: Object } | null): boolean =>
            filteredChild !== null
        );
      this.changeDataSource(matchedDataSource);
      setTimeout(() => {
        if (
          !isNullOrUndefined(proxy.ddTree) &&
          !isNullOrUndefined(proxy.ddTree.treeObj)
        ) {
  ...
  private nestedChildFilter(
    value: string,
    node: { [key: string]: Object }
  ): { [key: string]: Object } {
    const children: { [key: string]: Object }[] = node[
      this.ddTree.fields.child
    ] as { [key: string]: Object }[];
    if (!children) {
      return this.isMatchedNode(value, node) ? node : null;
    }
    const matchedChildren: { [key: string]: Object }[] = children
      .map((child: ColumnsModel): { [key: string]: Object } =>
        this.nestedChildFilter(value, child as { [key: string]: Object })
      )
      .filter(
        (filteredChild: { [key: string]: Object } | null): boolean =>
          filteredChild !== null
  ...
  private isMatchedNode(
    ...
    const checkValue: string = (
      node[this.ddTree.fields.text] as string
    ).toLowerCase();
  ...
  private dropdownTreeClose(): void {
    if (this.ddTree) {
      this.changeDataSource(
        extend([], this.columns, [], true) as { [key: string]: Object }[]
      );
    }
    this.ddTree = null;
  }
  ...
  private updateAddedRule(
    ...
    if (isRuleTemplate) {
      ...
      while (ruleElem && ruleElem.previousElementSibling !== null) {
        ...
        const enableSeparateCondition: boolean =
          this.enableSeparateConnector &&
          ((!this.headerTemplate &&
            !ruleElem.classList.contains('e-btn-group')) ||
            (this.headerTemplate &&
              (ruleElem.classList.contains('e-rule-container') ||
      ...
      if (Object.keys(rule).length) {
        if (this.ruleIndex < 0) {
          ...
          if (this.enableSeparateConnector) {
            rules.rules[rules.rules.length - 1].condition = rule.condition
              ? rule.condition
              : newRule.condition;
          }
        } else {
          ...
          if (this.enableSeparateConnector) {
            (rules.rules[this.ruleIndex + 1] as RuleModel).condition =
              rule.condition ? rule.condition : newRule.condition;
          }
        }
      } else {
        if (custom) {
  ...
  private changeRuleTemplate(column: ColumnsModel, element: Element): void {
    ...
    if (column.ruleTemplate) {
      ...
      const parentGroup: HTMLElement = closest(
        element,
        '.e-group-container'
      ) as HTMLElement;
      ...
      if (column) {
        const rule: RuleModel = {
          field: column.field,
          label: column.label,
          operator: operVal[0].value as string,
          value: '',
        };
        this.addRuleElement(
          parentGroup,
  ...
  private renderToolTip(element: HTMLElement): void {
    const tooltip: Tooltip = new Tooltip({
      ...
      beforeOpen: (args: TooltipEventArgs): void => {
        const tooltipCloseElement: HTMLElement = args.element.querySelector(
          '.e-tooltip-close'
        ) as HTMLElement;
  ...
  public validateFields(): boolean {
    ...
    let dropDownTreeObj: DropDownTree;
    if (this.allowValidation) {
      const excludeOprs: string[] = [
        'isnull',
        'isnotnull',
        'isempty',
        'isnotempty',
      ...
      let rule: RuleModel;
      const ruleElemCln: NodeListOf<Element> =
        this.element.querySelectorAll('.e-rule-container');
      for (i = 0, len = ruleElemCln.length; i < len; i++) {
        ...
        index = 0;
        indexElem = tempElem = ruleElemCln[i as number];
        if (this.fieldMode === 'DropdownTree') {
          dropDownTreeObj = getComponent(
            ruleElemCln[i as number].querySelector(
              '.e-rule-filter input.e-dropdowntree'
            ) as HTMLElement,
            'dropdowntree'
          );
          if (
            dropDownTreeObj &&
            dropDownTreeObj.value &&
            dropDownTreeObj.value.length
          ) {
            this.selectedColumn = this.getColumn(
              dropDownTreeObj.value[0]
            ) as ColumnsModel;
          ...
          dropDownObj = getComponent(
            ruleElemCln[i as number].querySelector(
              '.e-rule-filter input.e-dropdownlist'
            ) as HTMLElement,
            'dropdownlist'
          );
          if (dropDownObj && dropDownObj.value) {
            this.selectedColumn = dropDownObj.getDataByValue(
              dropDownObj.value as string
            ) as ColumnsModel;
            validateRule =
              !isNullOrUndefined(dropDownObj.index) &&
              (this.selectedColumn as ColumnsModel).validation;
          }
        }
        ...
        if (validateRule && validateRule.isRequired) {
          while (indexElem && indexElem.previousElementSibling !== null) {
            ...
            const separateCondition: boolean =
              this.enableSeparateConnector &&
              ((!this.headerTemplate &&
                !indexElem.classList.contains('e-btn-group')) ||
                (this.headerTemplate &&
                  (indexElem.classList.contains('e-rule-container') ||
                    indexElem.classList.contains('e-group-container'))));
          ...
          if (!rule.rules[index as number].operator) {
            if (
              fieldElem.parentElement.className.indexOf('e-tooltip') < 0 &&
              fieldElem.className.indexOf('e-tooltip') < 0
            ) {
          ...
          if (
            (excludeOprs.indexOf(rule.rules[index as number].operator) < -1 &&
              isNullOrUndefined(rule.rules[index as number].value) &&
              rule.rules[index as number].type !== 'date') ||
            rule.rules[index as number].value === '' ||
            rule.rules[index as number].value === null ||
            (rule.rules[index as number].value instanceof Array &&
              valArray.length < 1)
          ) {
            const valElem: NodeListOf<Element> = tempElem.querySelectorAll(
              '.e-rule-value .e-control'
            );
            if (excludeOprs.indexOf(rule.rules[index as number].operator) < 0) {
              isValid = false;
            }
            for (
              let j: number = 0, jLen: number = valElem.length;
              j < jLen;
              j++
            ) {
              const element: Element = valElem[j as number];
              let elem: Element;
              if (element.parentElement.className.indexOf('e-searcher') > -1) {
                ...
                valElem[j as number].parentElement.className.indexOf(
                  'e-tooltip'
                ) < 0 &&
                valElem[j as number].className.indexOf('e-tooltip') < 0
              ) {
            ...
            rule.rules[index as number].type === 'date' &&
            (rule.rules[index as number].value === null ||
              (rule.rules[index as number].value as any).indexOf(null) > -1)
          ) {
            const valElem: NodeListOf<Element> = tempElem.querySelectorAll(
              '.e-rule-value .e-control'
            );
            ...
            for (
              ...
              if (
                ...
                valElem[j as number].className.indexOf('e-tooltip') < 0 &&
                (isNullOrUndefined(rule.rules[index as number].value) ||
                  rule.rules[index as number].value[j as number] == null)
              ) {
                this.renderToolTip(valElem[j as number].parentElement);
              }
            }
          ...
          (dropDownObj &&
            ...
            isNullOrUndefined(dropDownObj.index)) ||
          (dropDownTreeObj &&
            dropDownTreeObj.element &&
            (isNullOrUndefined(dropDownTreeObj.value) ||
  ...
  private refreshLevel(obj: LevelColl): LevelColl | void {
    const ruleList: HTMLCollection =
      obj.groupElement.querySelector('.e-rule-list').children;
    let childElem: Element;
    const groupElem: Element = obj.groupElement;
    let i: number;
    const iLen: number = ruleList.length;
    ...
    obj.groupElement = ruleListElem
      ? closest(ruleListElem, '.e-group-container')
      : groupElem;
    obj.level = this.levelColl[obj.groupElement.id].slice();
    return obj;
  }
  private groupTemplate(isConnector?: boolean): Element {
    let glueElem: Element;
    let inputElem: Element;
    let dragClsName: string;
    let labelElem: Element;
    let grpActElem: Element;
    let groupBtn: HTMLElement;
    const groupElem: Element = this.createElement('div', {
      attrs: { class: 'e-group-container' },
    });
    const groupHdrElem: Element = this.createElement('div', {
      attrs: { class: 'e-group-header' },
    });
    const grpBodyElem: Element = this.createElement('div', {
      attrs: { class: 'e-group-body' },
    });
    const rulesElem: Element = this.createElement('div', {
      attrs: { class: 'e-rule-list' },
    });
    ...
    if (!this.headerTemplate) {
      ...
      const spanDragElement: HTMLElement = this.createElement('span', {
        attrs: {
          class: dragClsName,
          'aria-lable': 'drag handle',
          title: 'drag handle',
        },
      });
      groupHdrElem.appendChild(spanDragElement);
      const className: string =
        this.enableSeparateConnector && !isConnector
          ? 'e-lib e-btn-group e-qb-toggle-btn'
          : 'e-lib e-btn-group';
      glueElem = this.createElement('div', {
        attrs: { class: className, role: 'group' },
      });
      if (this.enableSeparateConnector) {
      ...
      if (this.enableNotCondition) {
        if (this.enableSeparateConnector) {
          inputElem = this.createElement('input', {
            attrs: { type: 'checkbox', class: 'e-qb-toggle' },
          });
        } else {
          inputElem = this.createElement('button', {
            attrs: { type: 'button', class: 'e-qb-toggle' },
          });
        }
        glueElem.appendChild(inputElem);
      }
      inputElem = this.createElement('input', {
        attrs: { type: 'radio', class: 'e-btngroup-and', value: 'AND' },
      });
      glueElem.appendChild(inputElem);
      ...
      inputElem = this.createElement('input', {
        attrs: { type: 'radio', class: 'e-btngroup-or', value: 'OR' },
      });
      glueElem.appendChild(inputElem);
      labelElem = this.createElement('label', {
      ...
      grpActElem = this.createElement('div', {
        attrs: { class: 'e-group-action' },
      });
      if (this.enableSeparateConnector) {
        groupBtn = this.createElement('button', {
          attrs: { type: 'button', class: 'e-add-condition-btn' },
        });
        grpActElem.appendChild(groupBtn);
        groupBtn = this.createElement('button', {
          attrs: { type: 'button', class: 'e-add-group-btn' },
        });
        grpActElem.appendChild(groupBtn);
      } else {
        groupBtn = this.createElement('button', {
          attrs: { type: 'button', class: 'e-add-btn' },
        });
        grpActElem.appendChild(groupBtn);
      }
  ...
  private ruleTemplate(): Element {
    let tempElem: Element;
    let clsName: string;
    let cloneClsName: string;
    let lockClsName: string;
    let dragClsName: string;
    const ruleElem: Element = this.createElement('div');
    const fieldElem: Element = this.createElement('div', {
      attrs: { class: 'e-rule-field' },
    });
    ...
    const filterElem: Element = this.createElement('input', {
      attrs: { type: 'text', class: 'e-filter-input' },
    });
    tempElem.appendChild(filterElem);
    fieldElem.appendChild(tempElem);
    tempElem = this.createElement('div', {
      attrs: { class: 'e-rule-operator' },
    });
    fieldElem.appendChild(tempElem);
    tempElem = this.createElement('div', { attrs: { class: 'e-rule-value' } });
    fieldElem.appendChild(tempElem);
    tempElem = this.createElement('div', {
      attrs: { class: 'e-rule-value-delete' },
    });
    if (this.showButtons.cloneRule) {
      cloneClsName =
        'e-clone-rule-btn e-clone-rule e-css e-btn e-small e-round e-icon-btn';
    } else {
      cloneClsName =
        'e-clone-rule-btn e-clone-rule e-css e-btn e-small e-round e-icon-btn e-button-hide';
    }
    if (this.showButtons.lockRule) {
      lockClsName =
        'e-lock-rule-btn e-lock-rule e-css e-btn e-small e-round e-icons e-icon-btn';
    } else {
      lockClsName =
        'e-lock-rule-btn e-lock-rule e-css e-btn e-small e-round e-icons e-icon-btn e-button-hide';
    }
    const cloneRuleBtnElem: HTMLElement = this.createElement('button', {
      attrs: {
        title: this.l10n.getConstant('CloneRule'),
        type: 'button',
        class: cloneClsName,
      },
    });
    let spanElement: HTMLElement = this.createElement('span', {
      attrs: { class: 'e-btn-icon e-icons e-copy' },
    });
    cloneRuleBtnElem.appendChild(spanElement);
    const cloneLockBtnElem: HTMLElement = this.createElement('button', {
      attrs: {
        title: this.l10n.getConstant('LockRule'),
        type: 'button',
        class: lockClsName,
      },
    });
    spanElement = this.createElement('span', {
      attrs: { class: 'e-btn-icon e-icons e-unlock' },
    });
    cloneLockBtnElem.appendChild(spanElement);
    if (
      this.showButtons.ruleDelete ||
      isNullOrUndefined(this.showButtons.ruleDelete)
    ) {
    ...
    const delBtnElem: HTMLElement = this.createElement('button', {
      attrs: { type: 'button', class: clsName },
    });
  ...
  private addGroupSuccess(
    ...
    if (
      ...
      if (target.className.indexOf('e-group-container') < 0) {
        groupID =
          target.querySelector('.e-group-container') &&
          target.querySelector('.e-group-container').id;
      } else {
        groupID = target.id;
      }
      if (this.enableSeparateConnector) {
      ...
      groupElem.setAttribute(
        'id',
        this.element.id + '_group' + this.groupIdCounter
      );
      if (
        this.groupIdCounter === 0 &&
        this.allowDragAndDrop &&
        groupElem.querySelector('.e-drag-qb-rule')
      ) {
      ...
      if (!this.headerTemplate) {
        ...
        const andLblElem: Element = groupElem.querySelector(
          '.e-btngroup-and-lbl'
        );
        const orLblElem: Element =
          groupElem.querySelector('.e-btngroup-or-lbl');
        andInpElem.setAttribute(
          'id',
          this.element.id + '_and' + this.btnGroupId
        );
        ...
        orInpElem.setAttribute(
          'name',
          this.element.id + '_and' + this.btnGroupId
        );
        andLblElem.setAttribute(
          'for',
          this.element.id + '_and' + this.btnGroupId
        );
        orLblElem.setAttribute(
          'for',
          this.element.id + '_or' + this.btnGroupId
        );
        this.btnGroupId++;
      }
      if (isGroup) {
        let clsName: string;
        if (
          this.showButtons.groupDelete ||
          isNullOrUndefined(this.showButtons.groupDelete)
        ) {
        ...
        let grpLen: number = 0;
        for (
          let j: number = 0, jLen: number = childElems.length;
          j < jLen;
          j++
        ) {
          if (
            childElems[j as number].className.indexOf('e-group-container') > -1
          ) {
            grpLen += 1;
          }
        }
        if (this.showButtons.cloneGroup && this.cloneGrpBtnClick) {
          if (this.groupIndex === childElems.length - 1) {
            ...
            if (this.enableSeparateConnector) {
              let index: number = 0;
              let tempGroupIndex: number = this.groupIndex + 1;
              for (let i: number = 0; i < tempGroupIndex; i++) {
                if (
                  childElems[i as number].classList.contains('e-rule-container')
                ) {
                  tempGroupIndex++;
                  index++;
                }
              ...
              childElems[idx as number].parentNode.insertBefore(
                groupElem,
                childElems[idx as number]
              );
            } else {
        ...
        if (!this.headerTemplate) {
          ...
          if (this.showButtons.cloneGroup) {
            lockClsName =
              'e-clone-grp-btn e-css e-btn e-small e-round e-icons e-icon-btn';
          } else {
            lockClsName =
              'e-clone-grp-btn e-css e-btn e-small e-round e-icons e-icon-btn e-button-hide';
          }
          const cloneBtnElem: HTMLElement = this.createElement('button', {
            attrs: {
              title: this.l10n.getConstant('CloneGroup'),
              type: 'button',
              class: lockClsName,
            },
          });
          ...
          if (this.showButtons.lockGroup) {
            lockClsName =
              'e-lock-grp-btn e-css e-btn e-small e-round e-icons e-icon-btn';
          } else {
            lockClsName =
              'e-lock-grp-btn e-css e-btn e-small e-round e-icons e-icon-btn e-button-hide';
          }
          const lockBtnElem: HTMLElement = this.createElement('button', {
            attrs: {
              title: this.l10n.getConstant('LockGroup'),
              type: 'button',
              class: lockClsName,
            },
          });
          const lockSpanElement: HTMLElement = this.createElement('span', {
            attrs: { class: 'e-btn-icon e-icons e-unlock' },
          });
      ...
      if (this.enableNotCondition) {
        if (!this.headerTemplate) {
          ...
          if (this.enableSeparateConnector) {
            tglBtn = new CheckBox({
              label: this.l10n.getConstant('NOT'),
              cssClass: 'e-qb-toggle',
            });
          } else {
            tglBtn = new Button({
      ...
      if (this.enableSeparateConnector && !this.headerTemplate) {
        let andElem: HTMLInputElement =
          groupElem.querySelector('.e-btngroup-and');
        let orElem: HTMLInputElement =
          groupElem.querySelector('.e-btngroup-or');
        const btnGroup: HTMLElement = groupElem.querySelector('.e-btn-group');
        for (let i: number = 0; i < btnGroup.childNodes.length; i++) {
          if (
            groupElem
              .querySelector('.e-btn-group')
              .childNodes[i as number].textContent.toLowerCase() === 'not'
          ) {
            if (!this.enableNotCondition) {
              (btnGroup.childNodes[i as number] as HTMLElement).style.display =
                'none';
            } else {
              (btnGroup.childNodes[i as number] as HTMLElement).classList.add(
                'e-multiconnector'
              );
            }
          } else {
        ...
        if (prevSibling) {
          if (isNullOrUndefined(this.headerTemplate)) {
            groupElem.parentElement.insertBefore(
              this.groupTemplate(true).querySelector('.e-btn-group'),
              groupElem
            );
            const notElem: HTMLElement = groupElem.previousElementSibling
          ...
          if (!this.isImportRules) {
            ...
            const newAndElem: HTMLInputElement =
              groupElement.querySelector('.e-btngroup-and');
            const newOrElem: HTMLInputElement =
              groupElement.querySelector('.e-btngroup-or');
            if (!andElem.checked && !orElem.checked) {
              const nextSibling: Element = groupElem.nextElementSibling;
              if (
                nextSibling &&
                nextSibling.classList.contains('e-btn-group')
              ) {
      ...
      if (this.headerTemplate) {
        ...
        if (this.enableSeparateConnector) {
          const conditionBtn: HTMLElement = groupElem.querySelector(
            '.e-add-condition-btn'
          ) as HTMLElement;
          let btnObj: Button = new Button({
            cssClass: this.element.id + '_addConditionbtn',
            content: this.l10n.getConstant('AddCondition'),
          });
          btnObj.appendTo(conditionBtn);
          btnObj.element.onclick = (): void => {
            this.addRuleElement(
              closest(conditionBtn, '.e-group-container'),
              {}
            );
          };
          const groupBtn: HTMLElement = groupElem.querySelector(
            '.e-add-group-btn'
          ) as HTMLElement;
          btnObj = new Button({
            cssClass: this.element.id + '_addGroupbtn',
            content: this.l10n.getConstant('AddGroup'),
          });
          btnObj.appendTo(groupBtn);
          btnObj.element.onclick = (): void => {
            this.addGroupElement(
              true,
              closest(groupBtn, '.e-group-container'),
              '',
              true
            );
          };
  ...
  private setMultiConnector(trgt: Element): void {
    if (this.enableSeparateConnector && !this.headerTemplate) {
      if (
        trgt.previousElementSibling &&
        this.groupElem.querySelector('.e-btn-group')
      ) {
        trgt.parentElement.insertBefore(
          this.groupTemplate(true).querySelector('.e-btn-group'),
          trgt
        );
        const notElem: HTMLElement = trgt.previousElementSibling
          .childNodes[0] as HTMLElement;
        if (notElem.classList.contains('e-qb-toggle')) {
          notElem.style.display = 'none';
        }
  ...
  private addHeaderDiv(elem: Element): void {
    ...
    if (prevElem) {
      ...
      const orElem: HTMLInputElement =
        elem.previousElementSibling.querySelector(
          '.e-btngroup-or'
        ) as HTMLInputElement;
      const andElem: HTMLInputElement =
        elem.previousElementSibling.querySelector(
          '.e-btngroup-and'
        ) as HTMLInputElement;
  ...
  private headerTemplateFn(
    ...
    let template: Element;
    const templateID: string = this.element.id + '_header';
    let args: ActionEventArgs;
    ...
    if (this.headerTemplate) {
      ...
      if ((this as any).isReact) {
        ...
        if (this.enableSeparateConnector && isInitialRule) {
          ...
          this.enableSeparateConnector &&
          groupElem.id.indexOf('rule') !== -1
        ) {
          groupHdr.parentElement.insertBefore(
            template,
            groupHdr.nextElementSibling
          );
        } else {
        ...
        const templateColl: Element[] = this.headerFn(
          args,
          this,
          groupElem.id,
          templateID
        ...
        if (
          this.enableSeparateConnector &&
          (isInitialRule || groupElem.id.indexOf('rule') !== -1)
        ) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  private enableSeparateConnectorInitialRule(
    ...
    const elem: Element = groupElem.nextElementSibling
      ? groupElem.nextElementSibling
      : groupElem;
    let group: HTMLElement = elem.closest('.e-group-container') as HTMLElement;
    if (!groupElem.nextElementSibling && group) {
      group = group.nextElementSibling
        ? (group.nextElementSibling as HTMLElement)
        : group;
    }
    if (group) {
  ...
  public notifyChange(
    ...
    let ruleElement: Element = closest(element, '.e-rule-container');
    let index: number = 0;
    if (type === 'not') {
    ...
    if (type === 'condition') {
      if (this.enableSeparateConnector) {
        ...
        if (
          ...
          ruleElement =
            ruleElement.querySelectorAll('.e-rule-container')[
              ruleElement.querySelectorAll('.e-rule-container').length - 1
            ];
        }
    ...
    while (ruleElement && ruleElement.previousElementSibling !== null) {
      ...
      if (
        ...
        (this.enableSeparateConnector &&
          ((!this.headerTemplate &&
            !ruleElement.classList.contains('e-btn-group')) ||
            (this.headerTemplate &&
              (ruleElement.classList.contains('e-rule-container') ||
                ruleElement.classList.contains('e-group-container')))))
      ) {
        index++;
      }
      ...
      if (value instanceof Date) {
        ...
        for (let i: number = 0; i < value.length; i++) {
          if (value[i as number] && value[i as number] instanceof Date) {
            value[i as number] = this.intl.formatDate(
              value[i as number] as Date,
              format
            ) as string;
          }
        }
  ...
  private templateChange(
    ...
    const arrOper: string[] = ['in', 'notin', 'between', 'notbetween'];
    let prevOper: string;
    switch (type) {
      case 'field':
    ...
    if (this.selectedColumn && this.selectedColumn.ruleTemplate) {
      if (type === 'field' || type === 'operator') {
        ...
        const ruleElement: Element = this.appendRuleElem(
          closest(grpEle, '.e-group-container'),
          this.selectedColumn,
          'change',
          grpEle.id,
          type,
        ...
        if (!this.enableSeparateConnector) {
          if (
            ...
            ruleElement.previousElementSibling.className.indexOf(
              'e-rule-container'
            ) > -1
          ) {
  ...
  private changeValue(
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if (
      (this.isNumInput && typeof (args as any).value === 'number') ||
      ((args as any).type === 'input' &&
        (args as any).target &&
        (args as any).target.classList.contains('e-numerictextbox') &&
        (this.selectedColumn.validation.max !== Number.MAX_VALUE ||
          this.selectedColumn.validation.min > 0))
    ) {
    ...
    if (
      (args as any).element &&
      (args as any).element.classList.contains('e-multiselect')
    ) {
      const multiSelectArgs: MultiSelectChangeEventArgs =
        args as MultiSelectChangeEventArgs;
    ...
    if (
      element.className.indexOf('e-day') > -1 ||
      element.className.indexOf('e-today') > -1 ||
      element.className.indexOf('e-cell') > -1
    ) {
      const calenderArgs: CalendarChangeEventArgs =
        args as CalendarChangeEventArgs;
    ...
    const groupID: string =
      groupElem && groupElem.id.replace(this.element.id + '_', '');
    const ruleID: string = ruleElem.id.replace(this.element.id + '_', '');
    const dateElement: CalendarChangeEventArgs =
      args as CalendarChangeEventArgs;
    let dropDownObj: DropDownList | DropDownTree;
    if (
      dateElement.element &&
      dateElement.element.className.indexOf('e-datepicker') > -1
    ) {
      element = dateElement.element;
      dropDownObj = getComponent(
        closest(element, '.e-rule-container').querySelector(
          '.e-filter-input'
        ) as HTMLElement,
        'dropdownlist'
      );
      if (dropDownObj) {
        this.selectedColumn = (dropDownObj as DropDownList).getDataByValue(
          dropDownObj.value as string
        ) as ColumnsModel;
      }
      dropDownObj = getComponent(
    ...
    if (element.className.indexOf('e-radio') > -1) {
      ...
      if (this.fieldMode === 'Default') {
        ...
        if (this.selectedColumn.columns) {
          dropDownObj = getComponent(
            closest(element, '.e-rule-container').querySelector(
              '.e-rule-sub-filter .e-dropdownlist'
            ) as HTMLElement,
            'dropdownlist'
          );
          this.selectedColumn = this.getColumn(
      ...
      value = (
        getComponent(element as HTMLElement, 'multiselect') as MultiSelect
      ).value as string[];
    } else {
      value = (<
        InputEventArgs | InputChangeEventArgs | CalendarChangeEventArgs
      >args).value;
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if (
        (args as any).type === 'input' &&
        ...
        const numericTextBoxObj: NumericTextBox = getInstance(
          elem,
          NumericTextBox
        ) as NumericTextBox;
        const decimalSeparator: string = getValue(
          'decimal',
          getNumericObject(this.locale)
        );
    ...
    if (
      (args as InputChangeEventArgs).name === 'input' &&
  ...
  private filterValue(
    ...
    if (!this.isImportRules) {
      this.trigger(
        ...
        eventsArgs,
        (observedChangeArgs: ChangeEventArgs) => {
          this.changeValueSuccessCallBack(
            observedChangeArgs,
            ele,
            i,
            grID,
  ...
  private fieldClose(id: string): void {
    ...
    const item: HTMLLIElement =
      ddl.popupObj && ddl.popupObj.element.querySelector('.e-active');
  ...
  private changeField(args: DropDownChangeEventArgs): void {
    if (args.isInteracted) {
      ...
      const fieldElem: Element =
        closest(args.element, '.e-rule-filter') ||
        closest(args.element, '.e-rule-sub-filter');
      const column: ColumnsModel =
        this.fieldMode === 'DropdownTree'
          ? this.getColumn(args.value[0])
          : this.getColumn(args.value as string);
      if (this.fieldMode === 'DropdownTree' && fieldElem !== null) {
        const ddtElem: HTMLInputElement = fieldElem.querySelector(
          '.e-dropdowntree.e-control'
        ) as HTMLInputElement;
        const ddt: DropDownTree = getComponent(
          ddtElem,
          'dropdowntree'
        ) as DropDownTree;
        if (column) {
          if (column.type === 'object') {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ddt.value = (args as any).oldValue;
            ddt.dataBind();
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            if (isNullOrUndefined((args as any).oldValue)) {
              ...
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              const result: string = this.getLabelFromColumn(
                (args as any).oldValue[0]
              );
      ...
      if (column && column.ruleTemplate) {
        ...
        this.templateChange(
          args.element,
          column.columns[0].field as string,
          'field'
        );
      } else {
  ...
  private changeRule(rule: RuleModel, ddlArgs: DropDownChangeEventArgs): void {
    ...
    const tempRule: RuleModel = {};
    let filterElem: Element = closest(ddlArgs.element, '.e-rule-filter');
    filterElem = filterElem
      ? filterElem
      : closest(ddlArgs.element, '.e-rule-sub-filter');
    let ddlObj: DropDownList | DropDownTree = getComponent(
      ddlArgs.element,
      'dropdownlist'
    ) as DropDownList;
  ...
  private changeFilter(
    flt: Element,
    dl: DropDownList | DropDownTree,
    grID: string,
    rl: RuleModel,
    tmpRl: RuleModel,
    dArg: DropDownChangeEventArgs
  ): void {
    if (flt) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      if (this.fieldMode === 'DropdownTree') {
        ...
        ddlValue = this.isImportRules
          ? this.GetRootColumnName(dl.value as string)
          : dl.value;
      }
  ...
  private changeOperator(
    ...
    flt: Element,
    opr: Element,
    dl: DropDownList | DropDownTree | any,
    grID: string,
    rl: RuleModel,
    tmpRl: RuleModel,
    dArg: DropDownChangeEventArgs
  ...
  private fieldChangeSuccess(
    ...
    filterElem: Element,
    rule: RuleModel,
    ddlArgs: DropDownChangeEventArgs
  ): void {
    ...
    const element: Element = closest(ddlArgs.element, '.e-group-container');
    const groupID: string = element.id.replace(this.element.id + '_', '');
    const ddlObj: DropDownList = getComponent(
      ddlArgs.element,
      'dropdownlist'
    ) as DropDownList;
    const tooltipElem: NodeListOf<Element> = ruleElem.querySelectorAll(
      '.e-tooltip.e-input-group'
    );
    for (let i: number = 0; i < tooltipElem.length; i++) {
      (
        getComponent(
          tooltipElem[i as number] as HTMLElement,
          'tooltip'
        ) as Tooltip
      ).destroy();
    }
    if (!args.cancel) {
      ...
      if (
        this.selectedColumn.type === 'object' &&
        this.fieldMode === 'Default'
      ) {
  ...
  private destroySubFields(filterElem: Element): void {
    while (
      filterElem &&
      filterElem.nextElementSibling.classList.contains('e-rule-sub-filter')
    ) {
  ...
  private createSubFields(
    ...
    const tempElem: Element = this.createElement('div', {
      attrs: {
        class: 'e-rule-sub-filter',
        id: 'subfilter' + this.subFilterCounter,
      },
    });
    fieldElem.insertBefore(
      tempElem,
      fieldElem.querySelector('.e-rule-operator')
    );
    const ruleId: string = closest(tempElem, '.e-rule-container').id;
    const subFieldElem: Element = this.createElement('input', {
      attrs: {
        type: 'text',
        id: ruleId + '_subfilterkey' + this.subFilterCounter,
      },
    });
    ...
    dropDownList.appendTo(
      '#' + ruleId + '_subfilterkey' + this.subFilterCounter
    );
    if (
      this.isImportRules ||
      (this.previousColumn &&
        this.previousColumn.ruleTemplate &&
        this.GetRootColumnName(rule.field) ===
          this.GetRootColumnName(this.previousColumn.field))
    ) {
      const subField: ColumnsModel[] = this.selectedColumn.columns;
      for (let i: number = 0; i < subField.length; i++) {
        if (
          rule.field === subField[i as number].field ||
          rule.field.indexOf(subField[i as number].field + this.separator) > -1
        ) {
  ...
  private operatorChangeSuccess(
    ...
    if (!eventsArgs.cancel) {
      const operatorElem: Element = closest(
        ddlArgs.element,
        '.e-rule-operator'
      );
      const valElem: Element = operatorElem.nextElementSibling;
      const dropDownObj: DropDownList = getComponent(
        ddlArgs.element,
        'dropdownlist'
      ) as DropDownList;
      const prevOper: string = rule.operator
        ? rule.operator.toString().toLowerCase()
        : '';
      tempRule.operator = dropDownObj.value.toString();
      const currOper: string = tempRule.operator.toLowerCase();
      if (
        tempRule.operator.toLowerCase().indexOf('between') > -1 ||
        (tempRule.operator.toLowerCase().indexOf('in') > -1 &&
          tempRule.operator.toLowerCase().indexOf('contains') < 0)
      ) {
        filterElem = operatorElem.previousElementSibling;
        tempRule.type = rule.type;
        if (
          tempRule.operator.toLowerCase().indexOf('in') < 0 ||
          prevOper.indexOf('in') < 0
        ) {
          rule.value = [];
        }
      ...
      if (ddlArgs.previousItemData) {
        const prevValue: string = ddlArgs.previousItemData.value
          .toString()
          .toLowerCase();
        if (
          prevValue.indexOf('between') > -1 ||
          ((prevValue.indexOf('in') > -1 ||
            prevValue.indexOf('null') > -1 ||
            prevValue.indexOf('empty') > -1) &&
            prevValue.indexOf('contains') < 0)
        ) {
          filterElem = operatorElem.previousElementSibling;
          tempRule.type = rule.type;
        }
      ...
      if (
        prevOper.indexOf('in') > -1 &&
        prevOper.indexOf('in') < 5 &&
        currOper.indexOf('in') > -1 &&
        currOper.indexOf('in') < 5
      ) {
        filterElem = null;
      }
      if (
        tempRule.operator.indexOf('null') > -1 ||
        tempRule.operator.indexOf('empty') > -1
      ) {
        const parentElem: HTMLElement =
          operatorElem.parentElement.querySelector(
            '.e-rule-value'
          ) as HTMLElement;
        const tooltipElem: HTMLElement = parentElem.querySelector(
          '.e-tooltip.e-input-group'
        );
        if (tooltipElem) {
  ...
  private changeRuleValues(
    ...
    if (filterElem) {
      ...
      if (operatorElem.childElementCount) {
        ddlObj = getComponent(
          operatorElem.querySelector('.e-dropdownlist') as HTMLElement,
          'dropdownlist'
        ) as DropDownList;
        tempRule.operator = ddlObj.value as string;
        let fieldObj: DropDownList | DropDownTree;
        if (this.fieldMode === 'DropdownTree') {
          fieldObj = getComponent(
            filterElem.querySelector('.e-dropdowntree') as HTMLElement,
            'dropdowntree'
          ) as DropDownTree;
        } else {
          fieldObj = getComponent(
            filterElem.querySelector('.e-dropdownlist') as HTMLElement,
            'dropdownlist'
          ) as DropDownList;
        }
        tempRule.type =
          this.fieldMode === 'DropdownTree'
            ? this.getColumn(fieldObj.value[0]).type
        ...
        if (ddlObj.value !== '') {
          this.renderValues(
            operatorElem,
            itemData,
            ddlArgs.previousItemData as ColumnsModel,
            true,
            rule,
            tempRule,
            ddlArgs.element
        ...
        oprElem = this.createElement('input', {
          attrs: { type: 'text', id: ruleId + '_operatorkey' },
        });
        ...
        if (this.selectedColumn.operators) {
          ...
          operatorList =
            this.customOperators[this.selectedColumn.type + 'Operator'];
        }
        const height: string =
          this.element.className.indexOf('e-device') > -1 ? '250px' : '200px';
        ...
        if (rule.operator) {
          operatorList.forEach((obj: { [key: string]: Object }) => {
            if (
              ...
              obj.value.toLowerCase() === rule.operator.toLowerCase()
            ) {
              operator = obj.value;
            }
          });
        }
        let value: string | object = operator
          ? operator
        ...
        tempRule.operator =
          rule && rule.operator !== '' && !isNullOrUndefined(rule.operator)
            ? rule.operator
            : (operatorList[0].value as string);
        if (this.isImportRules) {
          tempRule.type = this.selectedColumn.type;
  ...
  private destroyControls(target: Element, isRuleTemplate?: boolean): void {
    const element: Element = isRuleTemplate
      ? target
      : target.nextElementSibling;
    const inputElement: NodeListOf<HTMLElement> = element.querySelectorAll(
      'input.e-control'
    ) as NodeListOf<HTMLElement>;
    const divElement: NodeListOf<HTMLElement> = element.querySelectorAll(
      'div.e-control:not(.e-handle)'
    );
    const columns: ColumnsModel[] = this.columns;
    for (let i: number = 0, len: number = inputElement.length; i < len; i++) {
      if (inputElement[i as number].classList.contains('e-textbox')) {
        (
          getComponent(inputElement[i as number], 'textbox') as TextBox
        ).destroy();
        detach(select('input#' + inputElement[i as number].id, element));
      } else if (
        inputElement[i as number].classList.contains('e-dropdownlist')
      ) {
        if (
          this.allowValidation &&
          inputElement[i as number].parentElement.className.indexOf(
            'e-tooltip'
          ) > -1
        ) {
          (
        ...
        (
          getComponent(
            inputElement[i as number],
            'dropdownlist'
          ) as DropDownList
        ).destroy();
      } else if (inputElement[i as number].classList.contains('e-radio')) {
        (
          getComponent(inputElement[i as number], 'radio') as RadioButton
        ).destroy();
      } else if (
        inputElement[i as number].classList.contains('e-numerictextbox')
      ) {
        (
          getComponent(
            inputElement[i as number],
        ...
        (
          getComponent(inputElement[i as number], 'datepicker') as DatePicker
        ).destroy();
      } else if (
        inputElement[i as number].classList.contains('e-multiselect')
      ) {
        (
          getComponent(inputElement[i as number], 'multiselect') as MultiSelect
        ).destroy();
      } else if (
        inputElement[i as number].className.indexOf('e-template') > -1
      ) {
        const clsName: string = inputElement[i as number].className;
        for (let j: number = 0, jLen: number = columns.length; j < jLen; j++) {
          if (
            columns[j as number].template &&
            clsName.indexOf(columns[j as number].field) > -1
          ) {
            this.templateDestroy(
              columns[j as number],
              inputElement[i as number].id
            );
            break;
          }
        }
    ...
    const templateElement: NodeListOf<HTMLElement> = element.querySelectorAll(
      '.e-template:not(.e-control)'
    ) as NodeListOf<HTMLElement>;
    for (
      let i: number = 0, len: number = templateElement.length;
      i < len;
      i++
    ) {
  ...
  private templateDestroy(column: ColumnsModel, elemId: string): void {
    ...
    if (
      ...
      (column.template as TemplateColumn).destroy === undefined
    ) {
    ...
    if (template.destroy) {
      ...
      if (document.getElementById(elemId)) {
        templateElements = closest(
          document.getElementById(elemId),
          '.e-rule-field'
        ).querySelectorAll('.e-template');
      }
    ...
    if (this.dataColl[1]) {
      for (let i: number = 0, iLen: number = dataSource.length; i < iLen; i++) {
        ...
        if (fieldColl.length > 1) {
          let dataObj: object = dataSource[i as number];
          let fieldStr: string;
          for (
            let j: number = 0, jLen: number = fieldColl.length;
            j < jLen;
            j++
          ) {
            ...
            if (fieldColl.length === j + 1) {
              ...
              if (
                Number(dataObj[`${fieldStr}`]) === dataObj[`${fieldStr}`] &&
                dataObj[`${fieldStr}`] % 1 !== 0
              ) {
          ...
          if (
            Number(dataSource[i as number][`${field}`]) ===
              dataSource[i as number][`${field}`] &&
            dataSource[i as number][`${field}`] % 1 !== 0
          ) {
  ...
  private createNestedObject(
    ...
    let key: string;
    const lastIndex: number = fieldColl.length - 1;
  ...
  private getDistinctValues(dataSource: object[], field: string): object[] {
    ...
    let nest: string[] = [];
    let value: string = '';
    const isNested: number = field.indexOf(this.separator);
    for (let i: number = 0, iLen: number = dataSource.length; i < iLen; i++) {
      if (isNested === 0) {
        value = dataSource[i as number][`${field}`];
      } else {
  ...
  private renderMultiSelect(
    ...
    if (this.valueModel && this.valueModel.multiSelectModel) {
      multiSelectValue = {
        ...multiSelectValue,
        ...this.valueModel.multiSelectModel,
      };
    }
  ...
  private multiSelectOpen(parentId: string, args: PopupEventArgs): void {
    if (this.dataSource instanceof DataManager) {
      ...
      const value: string = this.selectedColumn.field;
      let isFetched: boolean = false;
      if (this.dataColl[1]) {
  ...
  private getMultiSelectData(element: Element, value: string): void {
    ...
    const data: Promise<Object> = this.dataManager.executeQuery(
      new Query().select(value)
    ) as Promise<Object>;
    const multiselectObj: MultiSelect = getComponent(
      element as HTMLElement,
      'multiselect'
    ) as MultiSelect;
    multiselectObj.hideSpinner();
    this.createSpinner(
      closest(element, '.e-multi-select-wrapper').parentElement
    );
    showSpinner(
      closest(element, '.e-multi-select-wrapper').parentElement as HTMLElement
    );
    data
      .then(
        (e: {
          actual: { result: Object[]; count: number };
          result: Object[];
        }) => {
          ...
          this.dataColl = extend(
            this.dataColl,
            dummyData,
            [],
            true
          ) as object[];
          multiselectObj.dataSource = this.getDistinctValues(
            this.dataColl,
            value
          ) as { [key: string]: object }[];
          this.isGetNestedData = true;
          hideSpinner(
            closest(element, '.e-multi-select-wrapper')
  ...
  private createSpinner(element: Element): void {
    const spinnerElem: HTMLElement = this.createElement('span', {
      attrs: { class: 'e-qb-spinner' },
    });
  ...
  private closePopup(i: number, args: PopupEventArgs): void {
    const element: Element = document.getElementById(
      args.popup.element.id.replace('_popup', '')
    );
    if (element) {
      const ms: MultiSelect = getComponent(
        element as HTMLElement,
        'multiselect'
      ) as MultiSelect;
      if (ms) {
  ...
  private processTemplate(
    target: Element,
    itemData: ColumnsModel,
    rule: RuleModel,
    tempRule: RuleModel
  ): void {
    const container: Element = closest(target, '.e-rule-container');
    const tempElements: NodeListOf<Element> =
      container.querySelectorAll('.e-template');
    const filterElem: HTMLElement = container.querySelector(
      '.e-rule-filter .e-filter-input'
    ) as HTMLElement;
    const ddlObj: DropDownList | DropDownTree =
      this.fieldMode === 'DropdownTree'
        ? (getComponent(filterElem, 'dropdowntree') as DropDownTree)
        : (getComponent(
    ...
    const column: ColumnsModel =
      this.fieldMode === 'DropdownTree'
        ? this.getColumn(ddlObj.value[0])
        : this.getColumn(ddlObj.value as string);
    if (
      typeof itemData.template === 'string' ||
      (itemData.template as TemplateColumn).write === undefined
    ) {
      const args: ActionEventArgs = {
        rule: rule,
        ruleID: container.id,
  ...
  private getItemData(parentId: string): ColumnsModel {
    let fieldObj: DropDownList | DropDownTree = getComponent(
      document.getElementById(parentId + '_filterkey'),
      'dropdownlist'
    ) as DropDownList;
    if (this.fieldMode === 'DropdownTree') {
      fieldObj = getComponent(
    ...
    return this.fieldMode === 'DropdownTree'
      ? this.getColumn(fieldObj.value[0])
      : this.getColumn(fieldObj.value as string);
  }
  ...
  private renderStringValue(
    ...
    if (
      ...
      (this.dataColl.length || columnData.values)
    ) {
      selectedVal =
        this.isImportRules ||
        this.ruleIndex > -1 ||
      ...
      if (operator === 'in' || operator === 'notin') {
        selectedVal =
          this.isImportRules || this.ruleIndex > -1 || this.groupIndex > -1
            ? (rule.value as string[])
            : (this.setDefaultValue(parentId, true, false) as string[]);
  ...
  private renderNumberValue(
    ...
    let selectedVal: number | number[] =
      ...
      typeof rule.value === 'number'
        ...
        : (this.setDefaultValue(parentId, false, true) as number);
    if (
      (operator === 'in' || operator === 'notin') &&
      (this.dataColl.length || columnData.values)
    ) {
      ...
      itemData = columnData;
      const min: number =
        itemData.validation && itemData.validation.min
          ? itemData.validation.min
          : 0;
      const max: number =
        itemData.validation && itemData.validation.max
          ? itemData.validation.max
          : Number.MAX_VALUE;
      const format: string | FormatObject = itemData.format
        ? itemData.format
        : 'n';
      if (length > 1 && rule) {
        selectedVal = rule.value[idx as number]
          ? rule.value[idx as number]
          : (this.setDefaultValue(parentId, true, true) as number);
      }
      let numericTxt: NumericTextBoxModel;
      numericTxt = {
        value:
          selectedVal instanceof Array
            ? selectedVal[idx as number]
  ...
  private parseDate(value: string, format?: string | FormatObject): Date {
    let formatOpt: DateFormatOptions;
    let selectedValue: Date;
    if (format) {
      const dParser: Function = this.intl.getDateParser({
        skeleton: 'full',
        type: 'dateTime',
      });
  ...
  private renderControls(
    ...
    removeClass(
      [target.parentElement.querySelector('.e-rule-value')],
      'e-hide'
    );
    addClass([target.parentElement.querySelector('.e-rule-value')], 'e-show');
    if (
      itemData.template &&
      ((itemData.template as TemplateColumn).create || isRendered)
    ) {
      this.processTemplate(target, itemData, rule, tempRule);
    } else {
      let length: number;
      if (tempRule.type === 'boolean') {
        length = this.selectedColumn.values
          ? this.selectedColumn.values.length
          : 2;
      } else {
        length =
          tempRule.operator &&
          tempRule.operator.toString().toLowerCase().indexOf('between') > -1
            ? 2
            : 1;
      }
      const parentId: string = closest(target, '.e-rule-container').id;
      let ruleValElem: HTMLElement;
      const operator: string = tempRule.operator.toString();
      if (
        target.className.indexOf('e-rule-operator') > -1 ||
        target.className.indexOf('e-rule-filter') > -1
      ) {
        ruleValElem = target.parentElement.querySelector(
          '.e-rule-value'
        ) as HTMLElement;
        if (
          this.element.className.indexOf('e-device') > -1 ||
            ...
            case 'date':
              {
                ...
                let format: string | FormatObject = itemData.format;
                let datepick: DatePicker;
                let datePicker: DatePickerModel;
                ...
                if (rule.value && !isNullOrUndefined(format)) {
                  selVal =
                    length > 1
                      ? (rule.value[i as number] as string)
                      : (rule.value as string);
                ...
                if (
                  ...
                  if (itemData.value instanceof Date) {
                    ...
                    selectedValue = this.parseDate(
                      itemData.value as string,
                      itemData.format
                    );
                  }
                }
                ...
                if (
                  (this.isImportRules || this.isPublic || isTemplate) &&
                  rule
                ) {
                  column = this.getColumn(rule.field);
                  format = column.format;
                ...
                if (format) {
                  ...
                  if (formatObj.skeleton) {
                    ...
                    if (this.valueModel && this.valueModel.datePickerModel) {
                      datePicker = {
                        ...datePicker,
                        ...this.valueModel.datePickerModel,
                      };
                    }
                ...
                if (!rule.value) {
                  const elem: Element = document.getElementById(
                    parentId + '_valuekey' + i
                  );
                  this.updateRules(
                    elem,
                    selectedValue,
                    null,
                    this.isNotValueChange
                  );
  ...
  private processBoolValues(
    ...
    if (itemData.values) {
      ...
      if (rule.type === 'boolean' && !isNullOrUndefined(rule.value)) {
        isCheck =
          values[i as number].toLowerCase() ===
          rule.value.toString().toLowerCase();
      } else if (itemData.value) {
        isCheck =
          values[i as number].toLowerCase() ===
          itemData.value.toString().toLowerCase();
      } else if (i === 0) {
        isCheck = true;
      }
      ...
      if (rule.type === 'boolean' && !isNullOrUndefined(rule.value)) {
        isCheck =
          values[i as number].toString().toLowerCase() ===
          rule.value.toString().toLowerCase();
      } else if (itemData.value) {
        isCheck =
          values[i as number].toString().toLowerCase() ===
      ...
      value = values[i as number].toString();
      orgValue = values[i as number];
      label = this.l10n.getConstant(['True', 'False'][i as number]);
    }
  ...
  private getOperatorIndex(ddlObj: DropDownList, rule: RuleModel): number {
    let i: number;
    const dataSource: { [key: string]: Object }[] = ddlObj.dataSource as {
      [key: string]: Object;
    }[];
  ...
  private getPreviousItemData(
    ...
    if (
      ...
      column.template &&
      prevItemData &&
      Object.keys(prevItemData).length < 4
    ) {
  ...
  private renderValues(
    ...
    const filtObj: DropDownTree | DropDownList =
      this.fieldMode === 'DropdownTree'
        ? getComponent(filtElem, 'dropdowntree')
        : getComponent(filtElem, 'dropdownlist');
    const column: ColumnsModel =
      this.fieldMode === 'DropdownTree'
        ? this.getColumn(filtObj.value[0])
        : // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (isRender) {
      itemData =
        element.id.indexOf('operator') > -1 ? itemData : this.selectedRule;
      if (itemData.operators) {
        ddlObj.value = null;
        ddlObj.dataBind();
        ddlObj.dataSource = itemData.operators;
        ddlObj.index = this.getOperatorIndex(ddlObj, rule);
        ddlObj.value = tempRule.operator = ddlObj.dataSource[ddlObj.index]
          .value as string;
        if (!this.autoSelectOperator) {
          ddlObj.index = -1;
    ...
    const operator: string = tempRule.operator.toString();
    let isTempRendered: boolean = false;
    if (
      !(
        operator.indexOf('null') > -1 ||
        operator.indexOf('isempty') > -1 ||
        operator.indexOf('isnotempty') > -1
      )
    ) {
      const parentId: string = closest(target, '.e-rule-container').id;
      ...
      if (prevItemData && prevItemData.template) {
        ...
        const elem: Element = select(
          '#' + parentId + '_valuekey0',
          target.nextElementSibling
        );
        if (elem && !elem.classList.contains('e-control')) {
          detach(
            select('#' + parentId + '_valuekey0', target.nextElementSibling)
          );
        }
        if (
          typeof prevItemData.template === 'string' ||
          (prevItemData.template as TemplateColumn).create === undefined
        ) {
          if (
            target.nextElementSibling.classList.contains('e-template-value')
          ) {
            this.clearQBTemplate([parentId]);
          }
      ...
      if (this.isImportRules || (ddlObj && ddlObj.value !== '')) {
        ...
        if (itemData.template) {
          ...
          itemData.template = column.template;
          isTempRendered = this.setColumnTemplate(
            itemData,
            parentId,
            column.field,
            (itemData.value as string) || operator,
            target,
            rule
          );
        }
        if (isTempRendered) {
          const parentElem: HTMLElement = target.parentElement.querySelector(
            '.e-rule-value'
          ) as HTMLElement;
          if (
            this.element.className.indexOf('e-device') > -1 ||
          ...
          if (tempRule.type === 'boolean') {
            inputLen = this.selectedColumn.values
              ? this.selectedColumn.values.length
              : 2;
          } else {
            inputLen =
              operator && operator.toLowerCase().indexOf('between') > -1
                ? 2
                : 1;
          }
          for (let i: number = 0; i < inputLen; i++) {
            const valElem: HTMLElement = this.createElement('input', {
              attrs: { type: 'text', id: parentId + '_valuekey' + i },
            });
  ...
  private setColumnTemplate(
    ...
    if (!itemData.template) {
      ...
      if (
        typeof itemData.template === 'string' ||
        (itemData.template as TemplateColumn).create === undefined
      ) {
        args = {
        ...
        this.trigger(
          'actionBegin',
          args,
          (observedActionArgs: ActionEventArgs) => {
            isRendered = this.actionBeginSuccessCallBack(
              observedActionArgs,
              itemData,
              ruleID,
              field,
        ...
        if (typeof template.create === 'string') {
          ...
          valElem = (template.create as Function)({
            field: field,
            operator: operator,
          });
        }
        if (valElem instanceof Element) {
          ...
          addClass(valElem, 'e-template');
          for (
            let i: number = 0, iLen: number = valElem.length;
            i < iLen;
            i++
          ) {
            valElem[i as number].id = ruleID + '_valuekey' + i;
            target.nextElementSibling.appendChild(valElem[i as number]);
          }
        }
        addClass([target.nextElementSibling], 'e-template-value');
        return true;
  ...
  private actionBeginSuccessCallBack(
    ...
    if (args.renderTemplate) {
      ...
      this.columnTemplateFn = this.templateParser(
        typeof itemData.template === 'function'
          ? itemData.template
      ...
      if ((this as any).isReact) {
        ...
        valElem =
          valElemColl[0].nodeType === 3 ? valElemColl[1] : valElemColl[0];
        target.nextElementSibling.appendChild(valElem as Element);
      } // eslint-disable-next-line @typescript-eslint/no-explicit-any
      else if ((this as any).isVue3) {
  ...
  private updateValues(element: HTMLElement, rule: RuleModel): void {
    ...
    switch (controlName) {
      ...
      case 'textbox':
        ...
        break;
      case 'dropdownlist':
        rule.value = (getComponent(element, controlName) as DropDownList)
          .value as string;
        break;
      case 'radio':
      ...
      case 'numerictextbox':
        if (rule.operator.indexOf('between') > -1) {
          ...
          rule.value[i as number] = (
            getComponent(element, controlName) as NumericTextBox
          ).value;
        } else {
          rule.value = (
            getComponent(element, controlName) as NumericTextBox
      ...
      case 'datepicker':
        ...
        break;
      case 'multiselect':
        rule.value = (getComponent(element, controlName) as MultiSelect)
  ...
  private updateRules(
    ...
    let operator: string = rule.rules[index as number].operator
      ? rule.rules[index as number].operator.toString()
      : '';
    ruleElem = closest(target, '.e-rule-container');
    const ruleID: string = ruleElem.id.replace(this.element.id + '_', '');
    if (
      closest(target, '.e-rule-filter') ||
      closest(target, '.e-rule-sub-filter')
    ) {
      ...
      const column: ColumnsModel =
        this.fieldMode === 'DropdownTree'
          ? this.getColumn(dropDownObj.value[0] as any)
          : // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this.getColumn(dropDownObj.value as any);
      if (
        !this.isImportRules &&
        rule.rules[index as number].field &&
        rule.rules[index as number].field.toLowerCase() !==
          column.field.toLowerCase()
      ) {
        if (
          (!ruleElem.querySelectorAll('.e-template') &&
            !(operator.indexOf('null') > -1)) ||
      ...
      rule.rules[index as number].field = this.selectedColumn.field;
      rule.rules[index as number].type = this.selectedColumn.type;
      rule.rules[index as number].label = this.selectedColumn.label;
      let ruleElement: Element = closest(target, '.e-rule-filter');
      ruleElement = ruleElement
        ? ruleElement
        : closest(target, '.e-rule-sub-filter');
      let element: HTMLElement = ruleElement.nextElementSibling.querySelector(
        'input.e-control'
      ) as HTMLElement;
      element = element
        ? element
        : (ruleElement.nextElementSibling.nextElementSibling.querySelector(
            'input.e-control'
          ) as HTMLElement);
      operator = (
        getComponent(element, 'dropdownlist') as DropDownList
      ).value.toString();
      rule.rules[index as number].operator = operator;
      // Value Fields
      const valueContainer: HTMLElement = ruleElement.nextElementSibling
        .nextElementSibling as HTMLElement;
      let elementCln: NodeListOf<HTMLElement> =
        valueContainer.querySelectorAll('input.e-control');
      ...
      eventsArgs = {
        groupID: groupID,
        ruleID: ruleID,
        value: rule.rules[index as number].field,
        type: 'field',
      };
      ...
      if (
        this.allowValidation &&
        rule.rules[index as number].field &&
        target.parentElement.className.indexOf('e-tooltip') > -1
      ) {
        (
          getComponent(
      ...
      const inputElem: NodeListOf<HTMLElement> = ruleElem.querySelectorAll(
        '.e-rule-value input.e-control'
      ) as NodeListOf<HTMLElement>;
      eventsArgs = {
        groupID: groupID,
        ruleID: ruleID,
        value: dropDownObj.value as string | number | boolean,
        type: 'operator',
      };
      if (
        this.allowValidation &&
        rule.rules[index as number].operator &&
        target.parentElement.className.indexOf('e-tooltip') > -1
      ) {
        (
          getComponent(
      ...
      if (
        inputElem.length > 1 &&
        !(inputElem[0].className.indexOf('e-template') > -1)
      ) {
        rule.rules[index as number].value = [];
      }
      for (let i: number = 0; i < inputElem.length; i++) {
        if (
          rule.rules[index as number].operator.indexOf('null') > -1 ||
          rule.rules[index as number].operator.indexOf('empty') > -1
        ) {
          rule.rules[index as number].value = null;
          continue;
  ...
  private filterRules(
    beforeRule: RuleModel,
    afterRule: RuleModel,
    type: string
  ): void {
    const beforeRuleStr: string = JSON.stringify({
      condition: beforeRule.condition,
      not: beforeRule.not,
      rule: beforeRule.rules,
    });
    const afetrRuleStr: string = JSON.stringify({
  ...
  private ruleValueUpdate(
    ...
    if (selectedValue !== null) {
      if (
        target.className.indexOf('e-multiselect') > -1 &&
        rule.rules[index as number].type === 'number' &&
        !this.isNotified
      ) {
        const selVal: number[] = [];
        const dupSelectedValue: string[] | number[] = selectedValue as
          | number[]
          | string[];
        for (
          let k: number = 0, kLen: number = dupSelectedValue.length;
          k < kLen;
          k++
        ) {
          if (typeof dupSelectedValue[k as number] === 'string') {
            selVal.push(
              parseFloat(dupSelectedValue[k as number] as string) as number
            );
          }
        }
        if (selVal.length) {
      ...
      if (this.isNotified) {
        rule.rules[index as number].value = selectedValue as
          | string
          | number
          | string[]
          | number[];
        eventsArgs = {
          ...
          ruleID: ruleElem.id,
          value: rule.rules[index as number].value as string,
          type: 'value',
        };
        if (!this.isImportRules) {
          this.trigger('change', eventsArgs);
        ...
        target.className.indexOf('e-spin') > -1 ||
        target.className.indexOf('e-numeric') > -1
      ) {
        if (arrOperator.indexOf(oper) > -1) {
          rule.rules[index as number].value[i as number] = selectedValue as
            | string
            | number;
        } else {
        ...
        if (oper === 'in' || oper === 'notin') {
          if (rule.rules[index as number].type === 'string') {
            rule.rules[index as number].value = this.processValueString(
              selectedValue as string,
              rule.rules[index as number].type
            ) as string[];
          } else {
            rule.rules[index as number].value = this.processValueString(
              selectedValue as string,
        ...
        const format: DateFormatOptions = this.getFormat(
          this.getColumn(this.selectedColumn.field).format
        );
        if ((<DateFormatOptions>format).type) {
          if (arrOperator.indexOf(oper) > -1) {
            ...
            rule.rules[index as number].value[i as number] =
              this.intl.formatDate(selectedValue as Date, format);
          } else {
            rule.rules[index as number].value = this.intl.formatDate(
  ...
  private validateValue(
    ...
    if (
      ...
      (isNullOrUndefined(index) ||
        (isObject ? (rule.value as string[]).length > 0 : rule.value))
    ) {
      const valElem: NodeListOf<Element> = ruleElem.querySelectorAll(
        '.e-rule-value .e-control'
      );
      if (valElem.length > 0) {
        if (valElem[0].className.indexOf('e-tooltip') > -1) {
          (
            getComponent(valElem[0] as HTMLElement, 'tooltip') as Tooltip
          ).destroy();
        } else if (
          valElem[0].parentElement.className.indexOf('e-tooltip') > -1
        ) {
          (
            getComponent(
              valElem[0].parentElement as HTMLElement,
              'tooltip'
            ) as Tooltip
          ).destroy();
        }
        if (
          valElem[1] &&
          valElem[1].parentElement.className.indexOf('e-tooltip') > -1
        ) {
          (
            getComponent(
              valElem[1].parentElement as HTMLElement,
              'tooltip'
            ) as Tooltip
          ).destroy();
        }
        if (
          valElem[2] &&
          valElem[2].parentElement.className.indexOf('e-tooltip') > -1
        ) {
          (
            getComponent(
              valElem[2].parentElement as HTMLElement,
              'tooltip'
            ) as Tooltip
          ).destroy();
        }
  ...
  public destroy(): void {
    ...
    let i: number;
    let len: number;
    let tooltip: NodeListOf<HTMLElement>;
    super.destroy();
    element = this.element.querySelectorAll(
      '.e-addrulegroup'
    ) as NodeListOf<HTMLElement>;
    len = element.length;
    for (i = 0; i < len; i++) {
      (
        getComponent(element[i as number], 'dropdown-btn') as DropDownButton
      ).destroy();
      detach(element[i as number]);
    }
    tooltip = this.element.querySelectorAll(
      '.e-rule-filter .e-control.e-tooltip'
    ) as NodeListOf<HTMLElement>;
    for (i = 0; i < tooltip.length; i++) {
      (getComponent(tooltip[i as number], 'tooltip') as Tooltip).destroy();
    }
    element = this.element.querySelectorAll(
      '.e-rule-filter .e-control:not(.e-tooltip)'
    ) as NodeListOf<HTMLElement>;
    len = element.length;
    for (i = 0; i < len; i++) {
      if (getComponent(element[i as number], 'dropdownlist') as DropDownList) {
        (
          getComponent(element[i as number], 'dropdownlist') as DropDownList
        ).destroy();
      } else {
        (
          getComponent(element[i as number], 'dropdowntree') as DropDownTree
        ).destroy();
      }
      detach(element[i as number]);
    }
    tooltip = this.element.querySelectorAll(
      '.e-rule-operator .e-control.e-tooltip'
    ) as NodeListOf<HTMLElement>;
    for (i = 0; i < tooltip.length; i++) {
      (getComponent(tooltip[i as number], 'tooltip') as Tooltip).destroy();
    }
    element = this.element.querySelectorAll(
      '.e-rule-operator .e-control:not(.e-tooltip)'
    ) as NodeListOf<HTMLElement>;
    len = element.length;
    for (i = 0; i < len; i++) {
      if (getComponent(element[i as number], 'dropdownlist') as DropDownList) {
    ...
    tooltip = this.element.querySelectorAll(
      '.e-rule-value .e-control.e-tooltip'
    ) as NodeListOf<HTMLElement>;
    for (i = 0; i < tooltip.length; i++) {
      (getComponent(tooltip[i as number], 'tooltip') as Tooltip).destroy();
    }
    ...
    this.element.innerHTML = '';
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if ((this as any).portals && (this as any).portals.length) {
      this.clearQBTemplate();
    }
    const popupElement: NodeListOf<HTMLElement> = document.querySelectorAll(
      '.qb-dropdownlist.e-popup'
    ) as NodeListOf<HTMLElement>;
    if (popupElement) {
  ...
  public addGroups(groups: RuleModel[], groupID: string): void {
    if (
      ...
      const rule: RuleModel = this.getParentGroup(groupElem);
      const grouplen: number = groups.length;
      if (grouplen) {
        this.isPublic = true;
        for (let i: number = 0, len: number = groups.length; i < len; i++) {
          this.updatedRule = {
            isLocked: groups[i as number].isLocked,
            condition: groups[i as number].condition,
        ...
        if (this.groupIndex < 0) {
          ...
          if (this.enableNotCondition) {
            rule.rules.splice(this.groupIndex + 1, 0, {
              condition: condition,
              not: not,
              rules: [],
              isLocked: isLocked,
  ...
  private initWrapper(): void {
    ...
    if (this.cssClass) {
      addClass(
        [this.element],
        this.cssClass.replace(/\s+/g, ' ').trim().split(' ')
      );
    }
    if (this.enableSeparateConnector) {
    ...
    if (this.summaryView) {
      ...
      if (this.columns && this.columns.length && this.isImportRules) {
        ...
        this.addRuleElement(
          this.element.querySelector('.e-group-container'),
          {}
        );
      }
      this.notGroupRtl();
      ...
      const buttons: NodeListOf<Element> =
        this.element.querySelectorAll('label.e-btn');
  ...
  private renderSummary(): void {
    ...
    const textElem: Element = this.createElement('textarea', {
      attrs: { class: 'e-summary-text', readonly: 'true' },
      styles: 'max-height:500px',
    });
    const editElem: Element = this.createElement('button', {
      attrs: {
        type: 'button',
        class: 'e-edit-rule e-css e-btn e-small e-flat e-primary',
      },
    });
    const divElem: Element = this.createElement('div', {
      attrs: { class: 'e-summary-btndiv' },
    });
  ...
  private columnSort(): void {
    if (this.sortDirection.toLowerCase() === 'descending') {
      this.columns = new DataManager(this.columns).executeLocal(
        new Query().sortByDesc('field')
      );
  ...
  private onChangeNotGroup(): void {
    ...
    if (!this.headerTemplate) {
      if (this.enableNotCondition) {
        const inputElem: HTMLElement = this.createElement('button', {
          attrs: { type: 'button', class: 'e-qb-toggle' },
        });
        if (this.groupElem.querySelector('.e-btn-group')) {
          this.groupElem
            ...
            .insertBefore(
              inputElem,
              this.groupElem.querySelector('.e-btngroup-and')
            );
        }
      } else {
  ...
  public onPropertyChanged(
    newProp: QueryBuilderModel,
    oldProp: QueryBuilderModel
  ): void {
    const properties: string[] = Object.keys(newProp);
    const groupElem: HTMLElement = this.element.querySelector(
      '.e-group-container'
    ) as HTMLElement;
    let summaryElem: HTMLElement = this.element.querySelector(
      '.e-summary-content'
    ) as HTMLElement;
      ...
      switch (prop) {
        case 'summaryView':
          if (newProp.summaryView) {
            groupElem.style.display = 'none';
            if (
              this.element.querySelectorAll('.e-summary-content').length < 1
            ) {
              this.renderSummary();
              summaryElem = this.element.querySelector(
                '.e-summary-content'
              ) as HTMLElement;
            } else {
              this.element.querySelector('.e-summary-text').textContent =
                this.getSqlFromRules(this.rule);
            }
            summaryElem.style.display = 'block';
          } else {
            if (groupElem.querySelectorAll('.e-collapse-rule').length > -1) {
        ...
        case 'showButtons':
          if (!isNullOrUndefined(newProp.showButtons.lockGroup)) {
            if (newProp.showButtons.lockGroup) {
              removeClass(
                this.element.querySelectorAll('.e-lock-grp-btn'),
                'e-button-hide'
              );
            } else {
              addClass(
          ...
          if (!isNullOrUndefined(newProp.showButtons.lockRule)) {
            if (newProp.showButtons.lockRule) {
              removeClass(
                this.element.querySelectorAll('.e-lock-rule-btn'),
                'e-button-hide'
              );
            } else {
              addClass(
          ...
          if (!isNullOrUndefined(newProp.showButtons.cloneGroup)) {
            if (newProp.showButtons.cloneGroup) {
              removeClass(
                this.element.querySelectorAll('.e-clone-grp-btn'),
                'e-button-hide'
              );
            } else {
              addClass(
          ...
          if (!isNullOrUndefined(newProp.showButtons.cloneRule)) {
            if (newProp.showButtons.cloneRule) {
              removeClass(
                this.element.querySelectorAll('.e-clone-rule-btn'),
                'e-button-hide'
              );
            } else {
              addClass(
          ...
          if (!isNullOrUndefined(newProp.showButtons.ruleDelete)) {
            if (newProp.showButtons.ruleDelete) {
              removeClass(
                this.element.querySelectorAll('.e-rule-delete'),
                'e-button-hide'
              );
            } else {
              addClass(
          ...
          if (!isNullOrUndefined(newProp.showButtons.groupDelete)) {
            if (newProp.showButtons.groupDelete) {
              removeClass(
                this.element.querySelectorAll('.e-deletegroup'),
                'e-button-hide'
              );
            } else {
              addClass(
        ...
        case 'cssClass':
          ...
          if (newProp.cssClass) {
            addClass(
              [this.element],
              newProp.cssClass.replace(/\s+/g, ' ').trim().split(' ')
            );
          }
          break;
        case 'enableRtl':
  ...
  protected preRender(): void {
    ...
    this.customOperators = {
      stringOperator: [
        ...
        {
          value: 'notstartswith',
          key: this.l10n.getConstant('DoesNotStartWith'),
        },
  ...
  protected render(): void {
    ...
    this.customOperators['stringOperator'] =
      this.customOperators['stringOperator'].concat(stringOper); // tslint:disable-line
    this.customOperators['numberOperator'] =
      this.customOperators['numberOperator'].concat(numberOper); // tslint:disable-line
    if (this.dataSource instanceof DataManager) {
  ...
  private helper(e: { sender: MouseEventArgs }): boolean | HTMLElement {
    ...
    if (
      !target.classList.contains('e-drag-qb-rule') ||
      (target.classList.contains('e-drag-qb-rule') &&
        closest(target as Element, '.e-disable'))
    ) {
      return false;
    }
    ...
    if (
      this.draggable.currentStateTarget.parentElement.classList.contains(
        'e-group-header'
      )
    ) {
      cloneElement = closest(target as Element, '.e-group-container').cloneNode(
        true
      ) as HTMLElement;
      closest(target as Element, '.e-group-container').classList.add(
        'e-qb-dragging-rule'
      );
    } else {
      visualElement.classList.add('e-group-body');
      cloneElement = this.createElement('div', {
        className: 'e-rule-container',
      });
      const ruleElement: HTMLElement = closest(
        target as Element,
        '.e-rule-field'
      ).cloneNode(true) as HTMLElement;
      cloneElement.appendChild(ruleElement);
      closest(target as Element, '.e-rule-field').classList.add(
        'e-qb-dragging-rule'
      );
    }
    visualElement.appendChild(cloneElement);
    const childElements: NodeListOf<Element> =
      visualElement.querySelectorAll('*');
  ...
  private dragStartHandler(e: {
    ...
    const rule: HTMLElement = closest(
      e.target as Element,
      '.e-rule-container'
    ) as HTMLElement;
    const group: HTMLElement = closest(
      e.target as Element,
      '.e-group-container'
    ) as HTMLElement;
    const dragEventArgs: DragEventArgs = {
      dragRuleID: rule !== null ? rule.id : group.id,
  ...
  private dragHandler(e: { target: HTMLElement; event: MouseEventArgs }): void {
    ...
    let borderLineElem: NodeListOf<Element> = this.element.querySelectorAll(
    ...
    if (
      ...
      closest(e.target, '.e-qb-dragging-rule') ||
      closest(e.target, '.e-group-header') ||
      !closest(e.target, '#' + this.element.id) ||
      closest(e.target, '.e-disable') ||
      (e.target.parentElement &&
        e.target.parentElement.classList.contains('e-btn-group') &&
        this.enableSeparateConnector)
    ) {
    ...
    const targetElem: HTMLElement = closest(
      e.target as Element,
      '.e-rule-container'
    ) as HTMLElement;
    let grpElem: HTMLElement = closest(
      e.target as Element,
      '.e-group-container'
    ) as HTMLElement;
    if (targetElem) {
      ...
      const mouseY: number =
        e.event.clientY || e.event.changedTouches[0].clientY;
      ...
      e.target.classList.contains('e-rule-list') &&
      e.target.children.length === 0
    ) {
      e.target.classList.add('e-drag-rule-bottom-line');
    } else if (
      e.target.classList.contains('e-rule-list') &&
      e.target.children[0].classList.contains('e-group-container')
    ) {
      const rect: DOMRect =
        e.target.children[0].getBoundingClientRect() as DOMRect;
      const mouseY: number =
        e.event.clientY || e.event.changedTouches[0].clientY;
      const distanceToTop: number = mouseY - rect.top;
      const threshold: number = 20; // Adjust as needed
      ...
      closest(e.target as Element, '.e-group-container') ||
      e.target.classList.contains('e-group-container')
    ) {
      if (!grpElem) {
        grpElem = e.target;
      }
  ...
  private dragStopHandler(e: {
    ...
    let targetGroup: HTMLElement = closest(
      e.target as Element,
      '.e-rule-container'
    ) as HTMLElement;
    if (
      isNullOrUndefined(targetGroup) &&
      e.target.parentElement &&
      e.target.parentElement.classList.contains('e-btn-group') &&
      this.enableSeparateConnector
    ) {
      targetGroup = closest(
        e.target.parentElement.previousElementSibling as Element,
        '.e-rule-container'
      ) as HTMLElement;
    }
    let isPreventelem: HTMLElement | boolean;
    if (!isNullOrUndefined(e.helper)) {
      isPreventelem = closest(
        e.helper as Element,
        '.e-notallowedcur'
      ) as HTMLElement;
    }
    let prevRule: RuleModel;
    if (!isPreventelem) {
      const targetGrp: HTMLElement = closest(
        e.target as Element,
        '.e-group-container'
      ) as HTMLElement;
    ...
    if (targetGroup && !isPreventelem) {
      ...
      if (
        ...
        if (
          ...
          this.deleteGroup(
            closest(this.draggedRule as Element, '.e-group-container')
          );
        } else {
          rule = this.getRule(this.draggedRule);
          this.deleteRule(this.draggedRule);
        }
        let groupRule: RuleModel = extend(
          [],
          this.getGroup(targetGroup),
          [],
          true
        ) as RuleModel;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        if ((groupRule as any).properties) {
          groupRule = (groupRule as any).properties;
        }
        const groupElem: HTMLElement = closest(
          targetGroup as Element,
          '.e-group-container'
        ) as HTMLElement;
        const nestGrpElem: NodeListOf<Element> =
          groupElem.querySelectorAll('.e-group-container');
        nestGrpElem.forEach((ele: HTMLElement) => {
          if (this.element.querySelector('#' + ele.id)) {
            this.deleteGroup(ele);
          }
        });
        const ruleElems: NodeListOf<Element> =
          groupElem.querySelectorAll('.e-rule-container');
        const ruleIds: string[] = [];
        let dropInd: number;
        for (let i: number = 0; i < ruleElems.length; i++) {
          if (
            ruleElems[i as number].classList.contains('e-drag-rule-top-line') ||
            ruleElems[i as number].classList.contains('e-drag-rule-bottom-line')
          ) {
            dropInd = i;
          }
          ruleIds.push(
            ruleElems[i as number].id.split(this.element.id + '_')[1]
          );
        }
      ...
      tgrt.classList.contains('e-rule-list') &&
      tgrt.children.length === 0 &&
      !isPreventelem
    ) {
      const groupElem: HTMLElement = closest(
        tgrt as Element,
      ...
      const groupId: string = groupElem.id
        .split(this.element.id + '_')[1]
        .split('_')[0];
      let rule: RuleModel;
      if (
      ...
      tgrt.classList.contains('e-rule-list') &&
      tgrt.children[0].classList.contains('e-group-container') &&
      !isPreventelem
    ) {
      const groupElem: HTMLElement = closest(
        tgrt as Element,
      ...
      const groupElems: NodeListOf<Element> =
        tgrt.querySelectorAll('.e-group-container');
    ...
    if (
      e.helper &&
      e.helper.classList.contains('e-cloneproperties') &&
      document.querySelector('.' + e.helper.classList[0])
    ) {
      remove(e.helper);
    }
    if (this.enableSeparateConnector) {
    ...
    if (template) {
      try {
        if (
          ...
          document.querySelectorAll(template).length
        ) {
  ...
  private getParentGroup(
    ...
    const groupLevel: number[] =
      target instanceof Element
        ? this.levelColl[target.id]
        : this.levelColl[`${target}`];
  ...
  public deleteGroup(target: Element | string): void {
    ...
    if (!this.isImportRules && !this.prvtEvtTgrDaD) {
      this.trigger(
        ...
        args,
        (observedChangeArgs: ChangeEventArgs) => {
          this.deleteGroupSuccessCallBack(
            observedChangeArgs,
            target as Element
          );
        }
  ...
  private deleteGroupSuccessCallBack(
    ...
    if (!args.cancel) {
      ...
      const rule: RuleModel = this.getParentGroup(groupElem, true);
      let index: number = 0;
      let i: number;
      let len: number;
      const beforeRules: RuleModel = this.getValidRules(this.rule);
      const nextElem: Element = groupElem.nextElementSibling;
      const prevElem: Element = groupElem.previousElementSibling;
      const element: NodeListOf<Element> =
        groupElem.querySelectorAll('.e-group-container');
      const valElem: NodeListOf<Element> =
        groupElem.querySelectorAll('.e-tooltip');
      len = valElem.length;
      for (i = 0; i < len; i++) {
        (
          getComponent(
            valElem[i as number] as HTMLElement,
            'tooltip'
          ) as Tooltip
        ).destroy();
      }
      ...
      while (groupElem.previousElementSibling !== null) {
        ...
        if (
          ...
          (this.enableSeparateConnector &&
            ((!this.headerTemplate &&
              !groupElem.classList.contains('e-btn-group')) ||
              (this.headerTemplate &&
                (groupElem.classList.contains('e-rule-container') ||
      ...
      const elem: Element = groupElem.parentElement.parentElement.parentElement;
      const removeString: string[] = [];
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if ((this as any).isReact || (this as any).isAngular) {
        const remRule: RuleModel = rule.rules[index as number];
        const ruleElemColl: NodeListOf<Element> =
          target.querySelectorAll('.e-rule-container');
        if (remRule && remRule.rules) {
          for (let r: number = 0; r < remRule.rules.length; r++) {
            const column: ColumnsModel = this.getColumn(
              remRule.rules[r as number].field
            );
            if (ruleElemColl[r as number]) {
              const isTemplateRendered: Element =
                ruleElemColl[r as number].querySelector('.e-template-value');
              if (
                column &&
                (column.ruleTemplate ||
                  (this.isPlatformTemplate(column) && isTemplateRendered))
              ) {
      ...
      if (
        this.enableSeparateConnector &&
        prevElem &&
        ((!this.headerTemplate && prevElem.classList.contains('e-btn-group')) ||
          (this.headerTemplate &&
            prevElem.classList.contains('e-custom-group-btn')))
      ) {
        detach(prevElem);
      }
      if (!this.isImportRules && !this.prvtEvtTgrDaD) {
  ...
  private isPlatformTemplate(column: ColumnsModel): boolean {
    ...
    isTemp =
      column.template &&
      (typeof column.template === 'string' ||
        (column.template as TemplateColumn).create === undefined);
    return isTemp;
  }
  ...
  private deleteRule(target: Element): void {
    const groupElem: Element = closest(target, '.e-group-container');
    const groupID: string = groupElem.id.replace(this.element.id + '_', '');
    const ruleID: string = closest(target, '.e-rule-container').id.replace(
      this.element.id + '_',
      ''
    );
    const args: ChangeEventArgs = {
  ...
  private deleteRuleSuccessCallBack(
    ...
    if (!args.cancel) {
      ...
      let ruleElem: Element = closest(target, '.e-rule-container');
      const beforeRules: RuleModel = this.getValidRules(this.rule);
      const prevElem: Element = ruleElem.previousElementSibling;
      let index: number = 0;
      const clnruleElem: Element = ruleElem;
      const nextElem: Element = ruleElem.nextElementSibling;
      const valElem: NodeListOf<Element> =
        ruleElem.querySelectorAll('.e-tooltip');
      let i: number;
      const len: number = valElem.length;
      for (i = 0; i < len; i++) {
        (
          getComponent(
            valElem[i as number] as HTMLElement,
      ...
      if (
        column &&
        column.template &&
        clnruleElem.querySelector('.e-template')
      ) {
        this.templateDestroy(
          column,
          clnruleElem.querySelector('.e-template').id
        );
      }
      ...
      const isTemplateRendered: Element =
        clnruleElem.querySelector('.e-template-value');
      // eslint-disable
      try {
        if (
          this.enableSeparateConnector &&
          (clnruleElem.previousElementSibling || clnruleElem.nextElementSibling)
        ) {
          const previousElem: Element = clnruleElem.previousElementSibling;
          const nextElem: Element = clnruleElem.nextElementSibling;
          if (
            isNullOrUndefined(nextElem) &&
            ((!this.headerTemplate &&
              previousElem.classList.contains('e-btn-group')) ||
              (this.headerTemplate &&
                previousElem.classList.contains('e-custom-group-btn')))
          ) {
            if (
              previousElem &&
              previousElem.previousElementSibling &&
              previousElem.previousElementSibling.classList.contains(
                'e-rule-container'
              )
            ) {
              const rule: RuleModel = this.getRule(
                previousElem.previousElementSibling as HTMLElement
              );
              rule.condition = null;
            }
            detach(previousElem);
            ...
            (!this.headerTemplate &&
              nextElem.classList.contains('e-btn-group')) ||
            (this.headerTemplate &&
              nextElem.classList.contains('e-custom-group-btn'))
          ) {
            detach(nextElem);
          }
        }
      ...
      if (!(rule.rules[0] && rule.rules[0].rules)) {
        this.disableRuleCondition(
          groupElem,
          rule,
          this.enableSeparateConnector ? true : false
        );
      }
      const tooltipElem: NodeListOf<Element> =
        this.element.querySelectorAll('.e-tooltip');
      for (let i: number = 0; i < tooltipElem.length; i++) {
        (
          getComponent(
            tooltipElem[i as number] as HTMLElement,
        ...
        ).refresh(tooltipElem[i as number] as HTMLElement);
      }
      if (!this.isImportRules && !this.prvtEvtTgrDaD) {
        this.trigger('change', args);
      }
  ...
  private setGroupRules(rule: RuleModel, isRoot?: boolean): void {
    ...
    if (rule.isLocked) {
      const lockGrpTarget: Element = this.element
        .querySelector('.e-group-container')
        .querySelector('.e-lock-grp-btn');
  ...
  private keyBoardHandler(e: KeyboardEventArgs): void {
    if (
      ...
      const dragElemnet: HTMLElement =
        this.element.querySelector('.e-cloneproperties');
      if (dragElemnet) {
        remove(dragElemnet);
      }
      if (this.enableSeparateConnector) {
  ...
  private disableRuleCondition(
    ...
    if (!this.headerTemplate) {
      ...
      const count: number =
        groupElem.querySelector('.e-rule-list').childElementCount;
      let andElem: HTMLInputElement =
        groupElem.querySelector('.e-btngroup-and');
      ...
      if (count > 1) {
        ...
        orElem.disabled = false;
        if (
          orElem.nextElementSibling.classList.contains('e-btn-disable') ||
        ...
        if (this.enableSeparateConnector && !isNewRuleAdded) {
          ...
          while (true) {
            const andGroup: NodeListOf<Element> =
              groupElem.parentElement.querySelectorAll('.e-btngroup-and');
            const orGroup: NodeListOf<Element> =
              groupElem.parentElement.querySelectorAll('.e-btngroup-or');
            andElem = andGroup[
              andGroup.length - (1 + index)
            ] as HTMLInputElement;
          ...
          while (elem && !elem.classList.contains('e-rule-container')) {
            if (elem.classList.contains('e-group-container')) {
              elem =
                elem.querySelectorAll('.e-rule-container')[
                  elem.querySelectorAll('.e-rule-container').length - 1
                ];
              break;
            }
        ...
        if (this.enableSeparateConnector && !isNewRuleAdded) {
          ...
          let elem: Element;
          let tempElem: Element = groupElem.previousElementSibling;
          while (tempElem) {
            if (tempElem.classList.contains('e-group-container')) {
              tempElem =
                tempElem.querySelectorAll('.e-rule-container')[
                  tempElem.querySelectorAll('.e-rule-container').length - 1
                ];
            }
          ...
          if (
            isNullOrUndefined(elem) &&
            groupElem.querySelectorAll('.e-rule-container')[
              groupElem.querySelectorAll('.e-rule-container').length - 1
            ]
          ) {
            elem =
              groupElem.querySelectorAll('.e-rule-container')[
  ...
  public getValidRules(currentRule?: RuleModel): RuleModel {
    ...
    const ruleColl: RuleModel[] = extend(
      [],
      currentRule.rules,
      [],
      true
      ...
      ? this.getRuleCollection(
          {
            ...
            rules: ruleColl,
            not: notCondition,
            isLocked: currentRule.isLocked,
          },
          true
        )
      : this.getRuleCollection(
          { condition: ruleCondtion, rules: ruleColl, not: notCondition },
          true
        );
  ...
  private getRuleCollection(rule: RuleModel, isValidRule: boolean): RuleModel {
    ...
    if (
      rule.rules &&
      rule.rules.length &&
      (Object.keys(rule.rules[0]).length > 6 || isValidRule)
    ) {
    ...
    if (
      rule.field &&
      rule.field !== '' &&
      (isNullOrUndefined(customObj) ||
        (customObj &&
          customObj.type !== 'question' &&
          customObj.type !== 'answer'))
    ) {
      if (rule.operator) {
        if (
          rule.operator.toString().indexOf('null') > -1 ||
          rule.operator.toString().indexOf('empty') > -1
        ) {
          rule.value = null;
        }
      }
      if (
        (this.isRefreshed && this.enablePersistence) ||
        (rule.field !== '' &&
          rule.operator !== '' &&
          (this.isValueEmpty
        ...
        const lockedRule: boolean = rule.isLocked;
        rule = {
          label: rule.label,
          field: rule.field,
          operator: rule.operator,
      ...
      if (
        customObj &&
        (customObj.type === 'question' || customObj.type === 'answer')
      ) {
  ...
  public getGroup(target: Element | string): RuleModel {
    if (
      target instanceof Element &&
      target.className.indexOf('e-group-container') < 1
    ) {
      target = closest(target, '.e-group-container');
    }
    const groupId: string =
      target instanceof Element ? target.id : this.element.id + '_' + target;
  ...
  public deleteGroups(groupIdColl: string[]): void {
    let i: number;
    const len: number = groupIdColl.length;
    let groupID: string;
    for (i = 0; i < len; i++) {
  ...
  public getFilteredRecords(): Promise<Object> | object {
    const predicate: Predicate = this.getPredicate(
      this.getValidRules(this.rule)
    );
    const dataManagerQuery: Query = isNullOrUndefined(predicate)
      ? new Query()
  ...
  public deleteRules(ruleIdColl: string[]): void {
    let i: number;
    const len: number = ruleIdColl.length;
    let ruleID: string;
    for (i = 0; i < len; i++) {
  ...
  public getDataManagerQuery(rule: RuleModel): Query {
    ...
    const fields: string[] = [];
    for (
      let i: number = 0, len: string[] = Object.keys(this.columns);
      i < len.length;
      i++
    ) {
  ...
  public getPredicate(rule: RuleModel): Predicate {
    ...
    for (let i: number = 0, len: number = ruleColl.length; i < len; i++) {
      ...
      if (keys.indexOf('rules') > -1 && ruleColl[i as number].rules) {
        ...
        !isNullOrUndefined(operator.length)
      ) {
        const oper: string = ruleColl[i as number].operator.toLowerCase();
        let isDateFilter: boolean = false;
        const dateOperColl: string[] = [
          'equal',
          'notequal',
          'greaterthan',
          'greaterthanorequal',
        ...
        if (
          ruleColl[i as number].type === 'date' &&
          dateOperColl.indexOf(oper) > -1
        ) {
          ignoreCase = true;
        }
        ...
        if (oper.indexOf('null') > -1 || oper.indexOf('empty') > -1) {
          ...
          ruleColl[i as number].type === 'date' &&
          !(ruleColl[i as number].value instanceof Array)
        ) {
          const format: DateFormatOptions = this.getFormat(column.format);
          if (!isNullOrUndefined(ruleColl[i as number].value)) {
            ruleValue = this.getDate(
              ruleColl[i as number].value as string,
              format
            );
            if (
              dateOperColl.indexOf(oper) > -1 &&
              !this.isTime(ruleColl[i as number].value as string)
            ) {
              isDateFilter = true;
            }
          } else {
        ...
        if (i === 0) {
          if (
            isDateFilter ||
            ((oper.indexOf('in') > -1 ||
              oper.indexOf('between') > -1 ||
              oper.indexOf('null') > -1 ||
              oper.indexOf('empty') > -1) &&
              oper.indexOf('contain') < 0)
          ) {
            pred = isDateFilter
              ? this.datePredicate(ruleColl[i as number], ruleValue as Date)
              : this.arrayPredicate(ruleColl[i as number]);
          } else {
            const value: string | number | Date = ruleValue as
              | string
              | number
              | Date;
            if (value !== '') {
              pred = new Predicate(
                ruleColl[i as number].field,
                operator,
                ruleValue as string | number | Date,
                ignoreCase
              );
            }
          }
    ...
    if (this.dataSource instanceof DataManager) {
      if (
        !isNullOrUndefined(pred) &&
        (pred.operator === 'isnull' || pred.operator === 'isnotnull')
      ) {
  ...
  private getLocale(): string {
    const gregorianFormat: string =
      '.dates.calendars.gregorian.days.format.short';
    let localeString: string = this.locale;
    const mainVal: string = 'main.';
    const cultureObj: object = getValue(
      mainVal + '' + this.locale + gregorianFormat,
      cldrData
    );
    if (!cultureObj) {
      localeString = 'en';
  ...
  private getColumn(field: string, col?: ColumnsModel[]): ColumnsModel {
    ...
    for (let i: number = 0, iLen: number = columns.length; i < iLen; i++) {
      if (columns[i as number].field === field) {
        ...
        if (
          this.separator !== '' &&
          columns[i as number].field === field.split(this.separator)[0]
        ) {
          column = columns[i as number];
          break;
        }
  ...
  private arrayPredicate(
    ...
    const value: number[] | string[] = ruleColl.value as number[] | string[];
    const operator: string = ruleColl.operator.toString();
    const nullValue: number = ruleColl.value as number;
    let pred: Predicate;
    const column: ColumnsModel = this.getColumn(ruleColl.field);
    const format: DateFormatOptions = this.getFormat(column.format);
    if (operator.indexOf('null') > -1 || operator.indexOf('empty') > -1) {
      switch (operator) {
        case 'isnull':
              ...
              case 'between':
                if (column.type !== 'date') {
                  ...
                  pred = new Predicate(
                    ...
                    gte,
                    value[j as number]
                      ? this.isTime(value[j as number] as string)
                        ? this.getDate(value[j as number] as string, format)
                        : this.setTime(
                            this.getDate(value[j as number] as string, format),
                            true
                          )
  ...
  private importRules(
    ...
    if (!isReset) {
      ...
      if (rule.rules && rule.rules.length > 1 && !this.headerTemplate) {
        ...
        let orElem: HTMLInputElement =
          parentElem.querySelector('.e-btngroup-or');
        let andElem: HTMLInputElement =
          parentElem.querySelector('.e-btngroup-and');
        if (this.enableSeparateConnector && parentElem.previousElementSibling) {
          orElem =
            parentElem.previousElementSibling.querySelector('.e-btngroup-or');
          andElem =
            parentElem.previousElementSibling.querySelector('.e-btngroup-and');
        }
        orElem.disabled = false;
        andElem.disabled = false;
        if (this.enableSeparateConnector) {
    ...
    if (!isNullOrUndefined(ruleColl)) {
      for (let i: number = 0, len: number = ruleColl.length; i < len; i++) {
        const keys: string[] = Object.keys(ruleColl[i as number]);
        if (
          !isNullOrUndefined(ruleColl[i as number].rules) &&
          keys.indexOf('rules') > -1 &&
          ruleColl[i as number].rules.length !== 0
        ) {
          if (
            this.element.querySelectorAll('.e-group-container').length >
            this.maxGroupCount
          ...
          parentElem = this.renderGroup(
            ruleColl[i as number],
            ruleColl[i as number].condition,
            parentElem,
            ruleColl[i as number].not
          );
          parentElem = this.importRules(
            ruleColl[i as number],
            parentElem,
            true
          );
        ...
        if (
          !isNullOrUndefined(ruleColl[i as number].rules) &&
          ruleColl[i as number].isLocked
        ) {
          const lockGrpTarget: Element = parentElem
            .querySelector('.e-rule-list')
            .children[i as number].querySelector('.e-lock-grp-btn');
          this.groupLock(lockGrpTarget);
        }
      }
    }
  ...
  private renderGroup(
    ...
    const element: NodeListOf<Element> =
      parentElem.querySelectorAll('.e-group-container');
    const cloneElem: HTMLCollection =
      parentElem.querySelector('.e-rule-list').children;
    if (
      this.showButtons.cloneGroup &&
      this.cloneGrpBtnClick &&
      this.isMiddleGroup
    ) {
  ...
  private enableReadonly(): void {
    ...
    const elem: NodeListOf<Element> = target.querySelectorAll(
      '.e-dropdownlist, .e-dropdowntree, .e-numerictextbox, .e-textbox, .e-datepicker, .e-multiselect .e-lib, .e-radio'
    );
    for (let i: number = 0; i < elem.length; i++) {
      if (elem[i as number].classList.contains('e-dropdownlist')) {
        const dropDownObj: DropDownList = getInstance(
          elem[i as number] as HTMLElement,
          DropDownList
        ) as DropDownList;
        dropDownObj.readonly = this.isReadonly;
      } else if (elem[i as number].classList.contains('e-dropdowntree')) {
        const dropDownTreeObj: DropDownTree = getComponent(
          elem[i as number] as HTMLElement,
          'dropdowntree'
        ) as DropDownTree;
    ...
    const deleteGroupElems: NodeListOf<Element> =
      this.element.querySelectorAll('.e-deletegroup');
    const addRuleGroupElems: NodeListOf<Element> =
      this.element.querySelectorAll('.e-addrulegroup');
    const removeRuleElems: NodeListOf<Element> =
      this.element.querySelectorAll('.e-removerule');
  ...
  private enableBtnGroup(): void {
    const elems: NodeListOf<Element> = this.element.querySelectorAll(
      '.e-btngroup-and-lbl, .e-btngroup-or-lbl, .e-qb-toggle'
    );
    let not: boolean = false;
    elems.forEach((elem: Element) => {
      if (
        elem.classList.contains('e-qb-toggle') &&
        !elem.classList.contains('e-active-toggle') &&
        ...
        elem.classList.contains('e-qb-toggle') &&
        elem.classList.contains('e-not-readonly')
      ) {
        ...
        (elem.previousElementSibling as HTMLInputElement).checked ||
        elem.classList.contains('e-readonly')
      ) {
        elem.classList.remove('e-readonly');
        if (not) {
  ...
  private getSqlString(
    ...
    if (rules.not) {
      ...
      if (isRoot) {
        rulesNotCondition = sqlLocale
          ? this.l10n.getConstant('NOT').toUpperCase() + ' ('
          : 'NOT (';
    ...
    if (rules.rules) {
      for (
        let j: number = 0, jLen: number = rules.rules.length;
        j < jLen;
        j++
      ) {
        if (rules.rules[j as number].rules) {
          queryStr = this.getSqlString(
            rules.rules[j as number],
            enableEscape,
            queryStr,
            sqlLocale
          );
          ...
          const rule: RuleModel = rules.rules[j as number];
          let valueStr: string = '';
          const ruleOpertor: string = sqlLocale
            ? this.sqlOperators[rule.operator]
            : this.operators[rule.operator];
          if (rule.value instanceof Array) {
            if (rule.operator.toString().indexOf('between') > -1) {
              const ruleCondition: string = sqlLocale
                ? ' ' + this.l10n.getConstant('AND').toUpperCase() + ' '
                : ' ' + 'AND' + ' ';
              if (
                rule.type === 'date' &&
                !this.isDateFunction(rule.value[0] as string)
              ) {
                valueStr +=
                  '"' +
                  rule.value[0] +
              ...
              if (typeof rule.value[0] === 'string' && rule.value !== null) {
                ...
                for (
                  let k: number = 1, kLen: number = rule.value.length;
                  k < kLen;
                  k++
                ) {
        ...
        if (j !== jLen - 1) {
          ...
          if (
            ...
            (rule &&
              rule.condition !== '' &&
              rule.custom &&
              (rule.custom as any).isCustom)
          ) {
            condition = rule.condition;
          }
          if (condition) {
            condition = sqlLocale
              ? this.l10n.getConstant(condition.toUpperCase()).toUpperCase()
  ...
  public getRulesFromSql(sqlString: string, sqlLocale?: boolean): RuleModel {
    ...
    const rule: RuleModel = this.processParser(
      this.parser,
      this.rule,
      [0],
      sqlLocale
  ...
  public getSqlFromRules(
    ...
    rule = this.getRuleCollection(rule, false);
    const sqlString: string = this.getSqlString(
      this.getValidRules(rule),
      allowEscape,
      null,
      sqlLocale
    ).replace(/"/g, "'");
  ...
  public getParameterizedSql(rule?: RuleModel): ParameterizedSql {
    ...
    if ((this.moduleLoader as any).loadedModules.length) {
      ...
      this.notify('query-library', {
        prop: 'getParameterSql',
        onPropertyChange: false,
        value: { rule: rule, obj: obj },
      });
      return obj['sql'];
    } else {
      console.warn(
        '[WARNING] :: Module "query-library" is not available in QueryBuilder component! You either misspelled the module name or forgot to load it.'
      );
    }
    return null;
  }
  ...
  public setParameterizedSql(sqlQuery: ParameterizedSql): void {
    ...
    if ((this.moduleLoader as any).loadedModules.length) {
      const obj: Object = { sql: null };
      this.notify('query-library', {
        prop: 'convertParamSqlToSql',
        onPropertyChange: false,
        value: { sql: sqlQuery, obj: obj },
      });
  ...
  public setParameterizedNamedSql(sqlQuery: ParameterizedNamedSql): void {
    ...
    if ((this.moduleLoader as any).loadedModules.length) {
      const obj: Object = { sql: null };
      this.notify('query-library', {
        prop: 'convertNamedParamSqlToSql',
        onPropertyChange: false,
        value: { sql: sqlQuery, obj: obj },
      });
      let sql: string = obj['sql'];
  ...
  public getMongoQuery(rule?: RuleModel): string {
    ...
    if ((this.moduleLoader as any).loadedModules.length) {
      ...
      this.notify('query-library', {
        prop: 'getMongoFromRules',
        onPropertyChange: false,
        value: { rule: rule, mongoQuery: '', obj: obj },
      });
  ...
  public cloneRule(ruleID: string, groupID: string, index: number): void {
    const getRule: RuleModel = this.getRule(
      ruleID.replace(this.element.id + '_', '')
    );
  ...
  public cloneGroup(
    ...
    this.addGroups(
      [{ condition: group.condition, not: group.not, rules: group.rules }],
      groupID
    );
    this.groupIndex = -1;
    this.cloneGrpBtnClick = false;
  ...
  public lockRule(ruleID: string): void {
    ...
    const target: Element = document
      ...
      .querySelectorAll('.e-lock-rule-btn')[0];
    this.ruleLock(target);
  }
  ...
  public lockGroup(groupID: string): void {
    ...
    const target: Element = document
      ...
      .querySelectorAll('.e-lock-grp-btn')[0];
    this.groupLock(target);
  }
  ...
  private parseSqlStrings(sqlString: string, sqlLocale?: boolean): number {
    ...
    if (sqlLocale) {
      conditions = [
        this.l10n.getConstant('AND').toUpperCase(),
        this.l10n.getConstant('OR').toUpperCase(),
        this.l10n.getConstant('NOT').toUpperCase(),
      ];
    } else {
    ...
    if (!lastPasrser || (lastPasrser && lastPasrser[0] !== 'Literal')) {
      for (let i: number = 0, iLen: number = conditions.length; i < iLen; i++) {
        ...
        const regExp: any = RegExp;
        regex = new regExp(
          '^(' + conditions[i as number] + ')' + regexStr,
          'ig'
        );
        if (regex.exec(sqlString)) {
          this.parser.push([
            'Conditions',
            conditions[i as number].toLowerCase(),
          ]);
    ...
    if (
      /^`?([a-z_][a-z0-9_.\\[\]\\(\\)]{0,}(\\:(number|float|string|date|boolean))?)`?/i.exec(
        sqlString
      )
    ) {
      matchValue =
    ...
    if (
      this.checkLiteral() &&
      /^'?([a-z_][a-z0-9 _.\\[\]\\(\\)-]{0,}(\\:(number|float|string|date|boolean))?)'?/i.exec(
        sqlString
      )
    ) {
      matchValue =
  ...
  private checkCondition(sqlString: string, matchValue: string): boolean {
    if (
      sqlString.slice(matchValue.length + 1, matchValue.length + 4) === 'AND' ||
  ...
  private combineSingleQuoteString(
    ...
    if (
      ...
      const tempMatchValue: string = matchValue.substring(
        0,
        matchValue.length - 1
      );
      ...
      if (singleString !== '') {
        const parsedValue: string = singleString.substring(
          1,
          singleString.length
        );
  ...
  private checkLiteral(): boolean {
    ...
    if (!lastParser) {
      ...
      if (
        lastParser[0] === 'Left' &&
        secParser &&
        secParser[0] === 'Conditions'
      ) {
        return true;
      }
      const betweenOperator: string = 'between';
      if (
        lastParser[0] === 'Conditions' &&
        betweenParser &&
        betweenParser[1].indexOf(betweenOperator) < 0
      ) {
        return true;
      }
    }
  ...
  private checkNumberLiteral(sqlString: string, sqlLocale?: boolean): boolean {
    ...
    if (!lastParser) {
      ...
      if (/^[0-9]+(?:\.[0-9]+)$/.exec(sqlString)) {
        ...
        const betweenOperator: string = sqlLocale
          ? this.l10n.getConstant('Between').toLowerCase()
          : 'between';
        if (
          lastParser[0] === 'Conditions' &&
          betweenParser &&
  ...
  private getOperator(
    ...
    if (sqlLocale) {
      ...
      for (let i: number = 0; i < localeOperator.length; i++) {
        if (
          this.sqlOperators[localeOperator[i as number]] ===
          ...
          if (
            ...
            return operator.toUpperCase() === 'NOT LIKE'
              ? 'notcontains'
              : 'contains';
          } else if (
            value &&
  ...
  private processParser(
    ...
    let grpCount: number;
    let operator: string;
    let isLeftOpened: boolean = false;
    ...
    for (let i: number = 0, iLen: number = parser.length; i < iLen; i++) {
      if (parser[i as number][0] === 'Literal') {
        ...
        rule = {
          label: column && column.label ? column.label : parser[i as number][1],
          field: parser[i as number][1],
        };
        if (parser[i + 1][0] === 'SubOperators') {
          if (
            parser[i + 1][1].indexOf('null') > -1 ||
            parser[i + 1][1].indexOf('empty') > -1
          ) {
            rule.operator = this.getOperator(' ', parser[i + 1][1], sqlLocale);
            rule.value = null;
            rule.type = this.getTypeFromColumn(rule);
          } else {
            const oper: string = parser[i + 3][1]
              ? parser[i + 3][1].replace(/'/g, '')
              : parser[i + 3][1];
          ...
          jLen = iLen;
          if (
            (sqlLocale && rule.operator === 'contains') ||
            rule.operator === 'startswith' ||
            rule.operator === 'endswith'
          ) {
            operator = 'like';
          } else if (
            (sqlLocale && rule.operator === 'notcontains') ||
            rule.operator === 'notstartswith' ||
            rule.operator === 'notendswith'
          ) {
            operator = 'not like';
          } else if (sqlLocale) {
            operator = rule.operator;
          ...
          for (j = i + 1; j < jLen; j++) {
            if (
              operator.indexOf('between') < 0 &&
              parser[j as number][0] === 'Left'
            ) {
              isLeftOpened = true;
              ...
              if (
                ...
                parser[j as number][0] === 'String'
              ) {
                let val: string = parser[j as number][1];
                if (
                  parser[j as number][1] &&
                  parser[j as number][1][0] === "'"
                ) {
                  val = parser[j as number][1].substring(
                    1,
                    parser[j as number][1].length - 1
                  );
                }
          ...
          if (operator.indexOf('like') < 0) {
            if (parser[j - 1][0] === 'Number') {
              ...
              operator.indexOf('between') > -1 &&
              parser[j - 1][0] === 'Conditions'
            ) {
              if (strVal.length !== 0) {
                rule.value = strVal;
                rule.type = 'string';
        ...
        if (
          ...
          (parser[i - 1][1] === 'not' ||
            (sqlLocale &&
              this.l10n.getConstant('NOT').toLowerCase() === parser[i - 1][1]))
        ) {
          isNotOpened = true;
          continue;
        }
        ...
        parser[i as number][0] === 'Conditions' &&
        !this.enableSeparateConnector
      ) {
        if (
          parser[i as number][1] === 'not' ||
          (sqlLocale &&
            this.l10n.getConstant('NOT').toLowerCase() ===
              parser[i as number][1])
        ) {
          rules.not = true;
        } else {
  ...
  private groupClone(target: Element): void {
    const groupElem: Element = (target as Element)
      .closest('.e-rule-list')
      .closest('.e-group-container');
    const targetGrpId: string = target.id.replace(this.element.id + '_', '');
    const groupId: string = (groupElem as Element).id.replace(
      this.element.id + '_',
      ''
    );
    const group: RuleModel = this.getGroup(targetGrpId);
    this.groupIndex = Array.prototype.indexOf.call(
      target.closest('.e-rule-list').children,
      target.closest('.e-group-container')
    );
    this.addGroups(
      [{ condition: group.condition, not: group.not, rules: group.rules }],
      groupId
  ...
  private ruleClone(target: Element): void {
    ...
    const groupElem: Element = target
      .closest('.e-rule-list')
      .closest('.e-group-container');
    const getRule: RuleModel = this.getRule(target as HTMLElement);
    const groupId: string = groupElem.id.replace(this.element.id + '_', '');
    const ruleElemColl: NodeListOf<HTMLElement> =
      groupElem.querySelectorAll('.e-rule-container');
  ...
  private ruleLock(target: Element): void {
    const ruleElem: Element = closest(target, '.e-rule-container');
    const rule: RuleModel = this.getRule(
      ruleElem.id.replace(this.element.id + '_', '')
    );
    if (ruleElem.classList.contains('e-disable')) {
      rule.isLocked = false;
      this.lockItems = this.lockItems.filter(
        (lockItem: string) => lockItem !== ruleElem.id
      );
      ...
      (target as HTMLElement).setAttribute(
        'title',
        this.l10n.getConstant('LockRule')
      );
    } else {
      rule.isLocked = true;
      ...
      (target as HTMLElement).setAttribute(
        'title',
        this.l10n.getConstant('UnlockRule')
      );
    }
  }
  ...
  private groupLock(target: Element): void {
    const groupElem: Element = closest(target, '.e-group-container');
    const group: RuleModel = this.getGroup(
      groupElem.id.replace(this.element.id + '_', '')
    );
    ...
    if (groupElem.classList.contains('e-disable')) {
      ...
      this.lockItems = this.lockItems.filter(
        (lockItem: string) => lockItem !== groupElem.id
      );
      ...
      (target as HTMLElement).setAttribute(
        'title',
        this.l10n.getConstant('LockGroup')
      );
      ...
      (target as HTMLElement).setAttribute(
        'title',
        this.l10n.getConstant('UnlockGroup')
      );
    }
  }
  ...
  private updateLockItems(): void {
    for (let i: number = 0; i < this.lockItems.length; i++) {
      ...
      if (idColl.length > 2) {
        let ruleElem: Element = this.element.querySelector(
          '#' + this.lockItems[i as number]
        ) as Element;
        ...
        let groupElem: Element = this.element.querySelector(
          '#' + this.lockItems[i as number]
        ) as Element;
  ...
  private disableHeaderControls(
    ...
    const andElem: NodeListOf<HTMLInputElement> = groupElem.querySelectorAll(
      '.e-btngroup-and'
    ) as NodeListOf<HTMLInputElement>;
    const orElem: NodeListOf<HTMLInputElement> = groupElem.querySelectorAll(
      '.e-btngroup-or'
    ) as NodeListOf<HTMLInputElement>;
    const notElem: NodeListOf<HTMLButtonElement> = groupElem.querySelectorAll(
      '.e-qb-toggle'
    ) as NodeListOf<HTMLButtonElement>;
    const addElem: NodeListOf<HTMLButtonElement> = groupElem.querySelectorAll(
      '.e-add-btn'
    ) as NodeListOf<HTMLButtonElement>;
    const deleteGrpElem: NodeListOf<HTMLButtonElement> =
      groupElem.querySelectorAll(
        '.e-deletegroup'
      ) as NodeListOf<HTMLButtonElement>;
    const lockElem: NodeListOf<HTMLButtonElement> = groupElem.querySelectorAll(
      '.e-lock-grp-btn'
    ) as NodeListOf<HTMLButtonElement>;
    const cloneElem: NodeListOf<HTMLButtonElement> = groupElem.querySelectorAll(
      '.e-clone-grp-btn'
    ) as NodeListOf<HTMLButtonElement>;
    const groupContElem: NodeListOf<HTMLInputElement> =
      groupElem.querySelectorAll(
        '.e-group-container'
      ) as NodeListOf<HTMLInputElement>;
    const addCondition: NodeListOf<HTMLButtonElement> =
      groupElem.querySelectorAll(
        '.e-add-condition-btn'
      ) as NodeListOf<HTMLButtonElement>;
    const addGroup: NodeListOf<HTMLButtonElement> = groupElem.querySelectorAll(
      '.e-add-group-btn'
    ) as NodeListOf<HTMLButtonElement>;
    for (let i: number = 0; i < andElem.length; i++) {
      if (isDisabled) {
        if (
          groupContElem[i as number] &&
          groupContElem[i as number].classList.contains('e-disable')
        ) {
          groupContElem[i as number].classList.add('e-disable');
        }
        andElem[i as number].disabled = true;
        orElem[i as number].disabled = true;
        if (addElem[i as number]) {
          addElem[i as number].disabled = true;
        }
        if (addCondition[i as number]) {
          addCondition[i as number].disabled = true;
        }
        if (addGroup[i as number]) {
          addGroup[i as number].disabled = true;
        }
        if (notElem[i as number]) {
          notElem[i as number].disabled = true;
        }
        if (deleteGrpElem[i as number]) {
          deleteGrpElem[i as number].disabled = true;
        }
        if (cloneElem[i as number]) {
          cloneElem[i as number].disabled = true;
        }
        ...
        andElem[i as number].disabled = false;
        orElem[i as number].disabled = false;
        if (addElem[i as number]) {
          addElem[i as number].disabled = false;
        }
        if (addCondition[i as number]) {
          addCondition[i as number].disabled = false;
        }
        if (addGroup[i as number]) {
          addGroup[i as number].disabled = false;
        }
        if (lockElem[i as number]) {
          lockElem[i as number].disabled = false;
        }
        if (notElem[i as number]) {
          notElem[i as number].disabled = false;
        }
        if (deleteGrpElem[i as number]) {
          deleteGrpElem[i as number].disabled = false;
        }
        if (cloneElem[i as number]) {
          cloneElem[i as number].disabled = false;
        }
  ...
  private disableRuleControls(
    ...
    const ddlElement: NodeListOf<HTMLElement> = groupElem.querySelectorAll(
      '.e-control.e-dropdownlist'
    ) as NodeListOf<HTMLElement>;
    const numericElement: NodeListOf<HTMLElement> = groupElem.querySelectorAll(
      '.e-control.e-numerictextbox'
    ) as NodeListOf<HTMLElement>;
    const textElement: NodeListOf<HTMLElement> = groupElem.querySelectorAll(
      '.e-control.e-textbox'
    ) as NodeListOf<HTMLElement>;
    const dateElement: NodeListOf<HTMLElement> = groupElem.querySelectorAll(
      '.e-control.e-datepicker'
    ) as NodeListOf<HTMLElement>;
    const checkboxElement: NodeListOf<HTMLElement> = groupElem.querySelectorAll(
      '.e-control.e-checkbox'
    ) as NodeListOf<HTMLElement>;
    const radioBtnElement: NodeListOf<HTMLElement> = groupElem.querySelectorAll(
      '.e-control.e-radio'
    ) as NodeListOf<HTMLElement>;
    const multiSelectElement: NodeListOf<HTMLElement> =
      groupElem.querySelectorAll(
        '.e-control.e-multiselect'
      ) as NodeListOf<HTMLElement>;
    const deleteElem: NodeListOf<HTMLButtonElement> =
      groupElem.querySelectorAll(
        '.e-rule-delete'
      ) as NodeListOf<HTMLButtonElement>;
    const lockElem: NodeListOf<HTMLButtonElement> = groupElem.querySelectorAll(
    ...
    const ruleElem: NodeListOf<HTMLButtonElement> = groupElem.querySelectorAll(
      '.e-rule-container'
    ) as NodeListOf<HTMLButtonElement>;
    for (let i: number = 0; i < deleteElem.length; i++) {
      if (isDisabled) {
        if (
          ruleElem[i as number] &&
          ruleElem[i as number].classList.contains('e-disable')
        ) {
        ...
        deleteElem[i as number].disabled = false;
        lockElem[i as number].disabled = false;
        lockElem[i as number].children[0].classList.remove('e-lock');
        lockElem[i as number].children[0].classList.add('e-unlock');
      }
    ...
    for (let i: number = 0; i < numericElement.length; i++) {
      numericObj = getComponent(
        numericElement[i as number],
        'numerictextbox'
      ) as NumericTextBox;
      if (isDisabled) {
    ...
    for (let i: number = 0; i < dateElement.length; i++) {
      dateObj = getComponent(
        dateElement[i as number],
        'datepicker'
      ) as DatePicker;
      if (isDisabled) {
    ...
    for (let i: number = 0; i < checkboxElement.length; i++) {
      checkBoxObj = getComponent(
        checkboxElement[i as number],
        'checkbox'
      ) as CheckBox;
      if (isDisabled) {
    ...
    for (let i: number = 0; i < radioBtnElement.length; i++) {
      radioBtnObj = getComponent(
        radioBtnElement[i as number],
        'radio'
      ) as RadioButton;
      if (isDisabled) {
    ...
    for (let i: number = 0; i < multiSelectElement.length; i++) {
      multiSelectObj = getComponent(
        multiSelectElement[i as number],
        'multiselect'
      ) as MultiSelect;
      if (isDisabled) {
