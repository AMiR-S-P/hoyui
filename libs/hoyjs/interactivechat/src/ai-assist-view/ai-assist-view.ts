...
} from './ai-assist-view-model';
import { InputEventArgs, TextArea } from '@hoyui/inputs';
import {
  ItemModel,
  Toolbar,
  ClickEventArgs,
  ...
  implements INotifyPropertyChanged
{
   ...
   * @default null
   */
  @Complex<PromptToolbarSettingsModel>(
    { width: null, items: [] },
   ...
   * @default []
   */
  @Complex<ResponseToolbarSettingsModel>(
    { width: null, items: [] },
  ...
  private setDimension(): void {
    this.element.style.width = !isNOU(this.width)
      ? formatUnit(this.width)
      : this.element.style.width;
    this.element.style.height = !isNOU(this.height)
      ? formatUnit(this.height)
    ...
    if (this.views.length > 0) {
      for (let index: number = 0; index < this.views.length; index++) {
        if (
          ...
          ].type.toLocaleLowerCase() === 'assist' &&
          !isAssistViewAssigned
        ) {
          assistView = {
            text:
              this.views[parseInt(index.toString(), 10)].name || 'AI Assist',
            prefixIcon:
              this.views[parseInt(index.toString(), 10)].iconCss ||
              'e-icons e-assistview-icon',
          ...
          this.views[
            parseInt(index.toString(), 10)
          ].type.toLocaleLowerCase() === 'custom'
        ) {
          customViewTemplate = this.createElement('div', {
            attrs: {
              class:
                'e-customview-content-section-' +
                customViewCount +
                ' e-custom-view',
            },
          });
          ...
          this.toolbarItems.push({
            ...
            htmlAttributes: {
              'data-index':
                this.element.id + '_view_' + customViewCount.toString(),
            },
          });
          customViewCount++;
        }
        ...
        if (!eventArgs.cancel) {
          if (args.item.htmlAttributes) {
            const currentIndex: number = parseInt(
              args.item.htmlAttributes['data-index'].split(
                this.element.id + '_view_'
              )[1],
              10
            );
  ...
  private renderToolbarSettings(): void {
    if (this.toolbarSettings.items.length > 0) {
      ...
      const pushToolbar: ItemModel[] = this.toolbarSettings.items.map(
        (item: any) => ({
          type: item.type,
  ...
  private getIndex(currentIndex: number): number {
    return currentIndex > this.views.length - (this.isAssistView ? 1 : 0) ||
      currentIndex < 0
      ? 0
      : currentIndex;
  }
  ...
  private updateActiveView(previousIndex?: number): void {
    ...
    if (!this.aiAssistViewRendered) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if (
        (this.toolbar as any).tbarEle[parseInt(activeViewIndex.toString(), 10)]
      ) {
        (this.toolbar as any).tbarEle[
          parseInt(activeViewIndex.toString(), 10)
    ...
    this.previousElement =
      this.displayContents[parseInt(activeViewIndex.toString(), 10)];
  }
  ...
  private removePreviousView(
    ...
    if (
      previousIndex >= 0 &&
      (this.toolbar as any).tbarEle[parseInt(previousIndex.toString(), 10)]
    ) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (this.toolbar as any).tbarEle[
        parseInt(previousIndex.toString(), 10)
  ...
  private renderViews(): void {
    if (
      this.assistViewTemplateIndex >= 0 &&
      this.views[this.assistViewTemplateIndex].viewTemplate
    ) {
      ...
      this.updateContent(
        'assistViewTemplate',
        this.assistCustomSection,
        -1,
        this.assistViewTemplateIndex
      );
    } else {
  ...
  private renderDefaultView(): void {
    this.contentWrapper = this.createElement('div', {
      attrs: { class: 'e-views', 'data-index': this.element.id + '_view_0' },
    });
    const contentContainer: HTMLElement = this.createElement('div', {
      attrs: { class: 'e-view-container' },
    });
    this.content = this.createElement('div', {
      attrs: { class: 'e-content-container' },
    });
    ...
    if (this.bannerTemplate) {
      const introContainer: HTMLElement = this.createElement('div', {
        attrs: { class: 'e-banner-view' },
      });
    ...
    const stopRespondingIcon: HTMLElement = this.createElement('span', {
      attrs: { class: 'e-icons e-assist-stop' },
    });
    const stopRespondingContent: HTMLElement = this.createElement('span', {
      innerHTML: 'Stop Responding',
    });
    this.appendChildren(
      this.stopResponding,
      stopRespondingIcon,
  ...
  private renderFooterContent(): void {
    if (this.footerTemplate) {
      ...
      const textareaEle: HTMLElement = this.createElement('textarea', {
        attrs: { class: 'e-assist-textarea' },
      });
  ...
  private renderSuggestions(): void {
    if (this.promptSuggestions && this.promptSuggestions.length > 0) {
      this.suggestions = this.createElement('div', {
        attrs: {
          class: `e-suggestions ${
            this.promptSuggestionItemTemplate
              ? 'e-suggestion-item-template'
              : ''
          }`,
        },
      });
      this.suggestionHeader = this.createElement('div', {
        attrs: { class: 'e-suggestion-header' },
      });
      const suggestionList: HTMLElement = this.createElement('div', {
        attrs: { class: 'e-suggestion-list' },
      });
  ...
  private renderSuggestionList(suggestionWrapper: HTMLElement): void {
    const suggestionsListElement: HTMLElement = this.createElement('ul', {
      attrs: { tabindex: '-1' },
    }) as HTMLElement;
  ...
  private updateContent(
    ...
    const contextPrompt: string =
      index >= 0
        ? this.prompts[parseInt(contextIndex.toString(), 10)].prompt
        : '';
    const contextOutput: string =
      index >= 0
        ? this.prompts[parseInt(contextIndex.toString(), 10)].response
        : '';
    if (this.isReact) {
    ...
    switch (templateName.toLowerCase()) {
      ...
      case 'promptsuggestionitemtemplate': {
        ...
        context = {
          index: arrayPosition,
          promptSuggestion:
            this.promptSuggestions[parseInt(arrayPosition.toString(), 10)],
        };
        break;
      }
      ...
      case 'assistviewtemplate': {
        template =
          this.views[parseInt(arrayPosition.toString(), 10)].viewTemplate || '';
        break;
      }
      case 'bannertemplate': {
    ...
    const ctn: string | Function = this.getTemplateFunction(
      template,
      notCompile
    );
  ...
  private updateTextAreaObject(textValue: string): void {
    ...
    this.textareaObj.cssClass =
      rowCount >= 10 ? 'show-scrollbar' : 'hide-scrollbar';
  }
  ...
  private detachCodeCopyEventHandler(): void {
    this.preTagElements.forEach(
      ({ preTag, handler }: { preTag: HTMLPreElement; handler: Function }) => {
  ...
  private addPrompt(): void {
    ...
    this.prompts = [
      ...this.prompts,
      { prompt: this.prompt, response: null, isResponseHelpful: null },
    ];
    this.isProtectedOnChange = prevOnChange;
  }
  ...
  private createOutputElement(): void {
    this.outputSuggestionEle = this.createElement('div', {
      attrs: {
        ...
        class: `e-prompt-container ${
          this.promptItemTemplate ? 'e-prompt-item-template' : ''
        }`,
      },
    });
  ...
  private renderOutputContainer(
    ...
    const outputContainer: HTMLElement = this.createElement('div', {
      attrs: {
        id: `e-response-item_${index}`,
        class: `e-output-container ${
          this.responseItemTemplate ? 'e-response-item-template' : ''
        }`,
      },
    });
    this.renderOutput(
  ...
  private renderOutput(
    ...
    const promptIcon: HTMLElement = this.createElement('span', {
      attrs: {
        class:
          'e-output-icon e-icons ' +
          (this.responseIconCss ||
            (this.isAssistView && this.views[0].iconCss) ||
            'e-assistview-icon'),
      },
    });
    const aiOutputEle: HTMLElement = this.createElement('div', {
      attrs: { class: 'e-output' },
    });
  ...
  private renderOutputTextContainer(
    ...
    this.outputContentBodyEle = this.createElement('div', {
      attrs: { class: 'e-content-body', tabindex: '0' },
    });
    if (!isMethodCall) {
      this.outputContentBodyEle.innerHTML = response;
      const preTags: HTMLPreElement[] = Array.from(
        this.outputContentBodyEle.querySelectorAll('pre')
      );
      preTags.forEach((preTag: HTMLPreElement) => {
        ...
        this.preTagElements.push({
          preTag,
          handler: this.getCopyHandler(preTag),
        });
        EventHandler.add(
          copyIcon,
          'click',
          this.preTagElements[this.preTagElements.length - 1].handler
        );
      });
    }
    this.renderOutputToolbarItems(index);
    this.appendChildren(
      aiOutputEle,
      this.outputContentBodyEle,
      this.contentFooterEle
    );
  }
  ...
  private renderOutputToolbarItems(index?: number): void {
    this.contentFooterEle = this.createElement('div', {
      attrs: { class: 'e-content-footer e-assist-toolbar-active' },
    });
  ...
  private renderResponseToolbar(index?: number): void {
    const pushToolbar: ItemModel[] = this.responseToolbarSettings.items.map(
      (item: ToolbarItemModel) => {
        ...
        if (
          toolbarItem.prefixIcon === 'e-icons e-assist-like' &&
          this.prompts[parseInt(index.toString(), 10)].isResponseHelpful
        ) {
          toolbarItem.prefixIcon = 'e-icons e-assist-like-filled';
        } else if (
          toolbarItem.prefixIcon === 'e-icons e-assist-dislike' &&
          this.prompts[parseInt(index.toString(), 10)].isResponseHelpful ===
            false
        ) {
  ...
  private handleItemClick(args: ClickEventArgs, index: number): void {
    ...
    if (args.item.prefixIcon === 'e-icons e-assist-copy') {
      ...
      this.getClipBoardContent(
        this.prompts[parseInt(index.toString(), 10)].response
      );
    ...
    const isLikeInteracted: boolean =
      icon === 'e-icons e-assist-like-filled' ||
      icon === 'e-icons e-assist-like';
    const isDislikeInteracted: boolean =
      icon === 'e-icons e-assist-dislike-filled' ||
      icon === 'e-icons e-assist-dislike';
    if (isLikeInteracted || isDislikeInteracted) {
      ...
      this.prompts[parseInt(index.toString(), 10)].isResponseHelpful =
        isHelpful;
      const promptItem: PromptModel =
        this.prompts[parseInt(index.toString(), 10)];
      // eslint-disable-next-line  @typescript-eslint/no-explicit-any
      const controlParentItems: ItemModel[] = (args.item as any).controlParent
        .items;
      if (isLikeInteracted) {
  ...
  private renderPrompt(promptText?: string, promptIndex?: number): void {
    const outputPrompt: HTMLElement = this.createElement('div', {
      attrs: { class: 'e-prompt-text', tabindex: '0' },
    });
    const promptContent: HTMLElement = this.createElement('div', {
      attrs: { class: 'e-prompt-content' },
    });
    const promptToolbarContainer: HTMLElement = this.createElement('div', {
      attrs: { class: 'e-prompt-toolbar' },
    });
    const promptToolbar: HTMLElement = this.createElement('div');
    const userIcon: HTMLElement = this.createElement('span', {
      attrs: {
        class: this.promptIconCss
          ? 'e-prompt-icon e-icons ' + this.promptIconCss
          : '',
      },
    });
  ...
  private renderPromptToolbar(
    ...
    if (this.promptToolbarSettings.items.length === 0) {
      ...
      pushToolbar = this.promptToolbarSettings.items.map(
        (item: ToolbarItemModel) => ({
          type: item.type,
          template: item.template,
          disabled: item.disabled,
      ...
      clicked: (args: ClickEventArgs) => {
        ...
        if (!eventArgs.cancel) {
          ...
          if (args.item.prefixIcon === 'e-icons e-assist-copy') {
            // eslint-disable-next-line  @typescript-eslint/no-explicit-any
            this.getClipBoardContent(
              this.prompts[parseInt(promptIndex.toString(), 10)].prompt
            );
            args.item.prefixIcon = 'e-icons e-assist-check';
  ...
  private renderSkeleton(): void {
    this.skeletonContainer = this.createElement('div', {
      attrs: { class: 'e-output-container' },
    });
    const outputViewWrapper: HTMLElement = this.createElement('div', {
      attrs: { class: 'e-output', style: 'width: 70%;' },
    });
    const skeletonIconEle: HTMLElement = this.createElement('span', {
      attrs: {
        class: 'e-output-icon e-skeleton e-skeleton-text e-shimmer-wave',
      },
    });
    const skeletonBodyEle: HTMLElement = this.createElement('div', {
      attrs: { class: 'e-loading-body' },
    });
    const skeletonFooterEle: HTMLElement = this.createElement('div', {
      attrs: { class: 'e-loading-footer' },
    });
  ...
  private updateIcons(newCss: string, isPromptIconCss: boolean = false): void {
    ...
    for (
      let index: number = 0;
      index < (elements && elements.length);
      index++
    ) {
      removeClass(
        [elements[parseInt(index.toString(), 10)]],
        elements[parseInt(index.toString(), 10)].classList
          .toString()
          .trim()
          .split(' ')
      );
  ...
  private updateToolbarSettings(previousToolbar: ToolbarSettingsModel): void {
    ...
    for (
      let index: number = this.views.length;
      index < this.toolbarItems.length;
      index++
    ) {
      if (
        previousToolbar.items[parseInt(previousToolbarIndex.toString(), 10)] ===
        this.toolbarItems[parseInt(index.toString(), 10)]
      ) {
    ...
    if (!this.isOutputRenderingStop) {
      if (typeof promptData === 'string') {
        if (!this.isResponseRequested) {
          this.prompts = [
            ...this.prompts,
            { prompt: null, response: null, isResponseHelpful: null },
          ];
        }
      ...
      if (typeof promptData === 'object') {
        ...
        if (this.prompt === tPrompt.prompt) {
          ...
          this.prompts[this.prompts.length - 1].isResponseHelpful =
            tPrompt.isResponseHelpful;
          this.renderOutputContainer(
            undefined,
            tPrompt.response,
            this.prompts.length - 1,
  ...
  public onPropertyChanged(
    newProp: AIAssistViewModel,
    oldProp?: AIAssistViewModel
  ): void {
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        case 'promptSuggestionsHeader': {
          this.suggestionHeader.innerHTML = this.promptSuggestionsHeader;
          const suggestionHeaderElem: HTMLElement = this.element.querySelector(
            '.e-suggestions .e-suggestion-header'
          );
