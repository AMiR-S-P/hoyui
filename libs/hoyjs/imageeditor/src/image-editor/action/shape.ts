...
import { EventHandler, extend, isNullOrUndefined } from '@hoyui/base';
import {
  CurrentObject,
  ImageEditor,
  Point,
...
export class Shape {
    ...
    switch (args.prop) {
      case 'drawEllipse':
        this.drawEllipse(
          ...
          args.value['radiusX'],
          args.value['radiusY'],
          args.value['strokeWidth'],
          args.value['strokeColor'],
          args.value['fillColor'],
          args.value['degree'],
      ...
      case 'drawText':
        this.drawText(
          ...
          args.value['degree'],
          args.value['fillColor'],
          args.value['outlineColor'],
          args.value['outlineWidth']
        );
        break;
      case 'redrawActObj':
        this.redrawActObj(
      ...
      case 'updateShapeChangeEventArgs':
        this.updateShapeChangeEventArgs(
          args.value['shapeSettings'],
          args.value['allowShapeOverflow']
        );
        break;
      ...
      case 'alignRotateFlipColl':
        this.alignRotateFlipColl(
          ...
          args.value['isRotateFlipCollection'],
          args.value['obj']
        );
        break;
      case 'selectShape':
      ...
      case 'getMaxText':
        this.getMaxText(
          args.value['isTextBox'],
          args.value['text'],
          args.value['obj']
        );
        break;
      case 'setPointCollForLineArrow':
        args.value['obj'].pointColl = this.getLinePoints(
          args.value['obj'].activePoint.startX,
          args.value['obj'].activePoint.startY,
          args.value['obj'].activePoint.endX,
      ...
      case 'getSquarePointForRotatedShape':
        this.getSquarePointForRotatedShape(
          args.value['obj'],
          args.value['object']
        );
        break;
      ...
      case 'drawAnnotations':
        this.drawAnnotations(
          ...
          args.value['shape'],
          args.value['pen'],
          args.value['isPreventApply'],
          args.value['x'],
          args.value['y'],
          args.value['panRegion']
        );
        break;
  ...
  private drawShape(
    ...
    if (!parent.disabled && parent.isImageLoaded) {
      ...
      if (type !== 'freehanddraw' && type !== '') {
        ...
        strokeSettings.strokeWidth = strokeWidth
          ? strokeWidth
          : strokeSettings.strokeWidth;
        ...
        strokeSettings.strokeColor = strokeColor
          ? strokeColor
          : strokeSettings.strokeColor;
        strokeSettings.fillColor = fillColor
          ? fillColor
          : strokeSettings.fillColor;
        strokeSettings.radius = radius ? radius : strokeSettings.radius;
        const tempWidth: number =
          parent.img.destWidth > 100 ? 100 : parent.img.destWidth / 2;
        const tempHeight: number =
          parent.img.destHeight > 100 ? 100 : parent.img.destHeight / 2;
        ...
        const shapeChangingArgs: ShapeChangeEventArgs = {
          cancel: false,
          action: 'insert',
        ...
        this.updateShapeChangeEventArgs(
          shapeChangingArgs.currentShapeSettings,
          shapeChangingArgs.allowShapeOverflow
        );
        this.setDimension(width, height);
        parent.notify('draw', {
          prop: 'drawObject',
        ...
        parent.notify('toolbar', {
          prop: 'renderQAT',
          onPropertyChange: false,
          value: { isPenEdit: null },
        });
        parent.notify('selection', {
          prop: 'isShapeInserted',
          onPropertyChange: false,
          value: { bool: true },
        });
        ...
        parent.notify('toolbar', {
          prop: 'update-toolbar-items',
          onPropertyChange: false,
        });
        if (parent.isPublicMethod && !isSelected) {
          parent.notify('undo-redo', {
            prop: 'updateUndoRedo',
            value: { operation: 'shapeInsert' },
            onPropertyChange: false,
          });
        }
  ...
  private setPointCollForLineAndArrow(): void {
    const parent: ImageEditor = this.parent;
    const shape: string = parent.activeObj.shape;
    const { startX, startY, endX, endY }: ActivePoint =
      parent.activeObj.activePoint;
    if (shape === 'line' || shape === 'arrow') {
      parent.activeObj.pointColl = this.getLinePoints(
        startX,
        startY,
        endX,
        endY
      ...
      if (parent.activeObj.pointColl) {
        for (
          let i: number = 0, len: number = parent.activeObj.pointColl.length;
          i < len;
          i++
        ) {
          parent.activeObj.pointColl[i as number].ratioX =
            (parent.activeObj.pointColl[i as number].x - parent.img.destLeft) /
            parent.img.destWidth;
          parent.activeObj.pointColl[i as number].ratioY =
            (parent.activeObj.pointColl[i as number].y - parent.img.destTop) /
  ...
  private prevObjColl(): void {
    ...
    this.prevObj.afterCropActions = extend(
      [],
    ...
    this.prevObj.selPointColl = extend(
      [],
  ...
  private drawShapeText(
    ...
    if (!parent.disabled && parent.isImageLoaded) {
      ...
      if (isNullOrUndefined(parent.activeObj.textSettings.fontSize)) {
        parent.getFontSizes();
        parent.activeObj.textSettings.fontSize = parseInt(
          parent.fontSizeColl[parseInt('3', 10) - 1].text,
          10
        );
      }
      if (parent.img.destWidth < 100) {
        parent.activeObj.textSettings.fontSize = Math.floor(
          parent.img.destWidth / 20
        );
      } else if (parent.img.destHeight < 100) {
        parent.activeObj.textSettings.fontSize = Math.floor(
          parent.img.destHeight / 20
        );
      }
      parent.activeObj.shapeDegree = parent.transform.degree;
      parent.activeObj.shapeFlip = parent.transform.currFlipState;
      ...
      parent.activeObj.order = this.getNewOrder();
      const width: number =
        this.upperContext.measureText(parent.activeObj.textSettings.text)
          .width +
      ...
      if (text && text.indexOf('\n') > -1 && parent.isPublicMethod) {
        ...
        const fontSizeInd: string = String(
          (parent.fontSizeColl as any).findIndex(
            (item: { text: string }) =>
              item.text === String(parent.activeObj.textSettings.fontSize)
          ) + 1
        );
  ...
  private drawShapeImageEvent(
    shapeChangingArgs: ShapeChangeEventArgs,
    isSelect: boolean
  ): void {
    const parent: ImageEditor = this.parent;
    this.updateShapeChangeEventArgs(
    ...
    const prevCropObj: CurrentObject = extend(
      {},
      parent.cropObj,
      {},
      true
  ...
  private initializeTextShape(
    ...
    this.keyHistory = '';
    parent.upperCanvas.style.display = 'block';
    parent.activeObj.strokeSettings.strokeColor =
      strokeColor || parent.activeObj.strokeSettings.strokeColor;
    parent.activeObj.strokeSettings.fillColor =
      fillColor || parent.activeObj.strokeSettings.fillColor;
    parent.activeObj.textSettings.text =
      text || parent.activeObj.textSettings.text;
    ...
    parent.activeObj.textSettings.fontSize =
      fontSize || parent.activeObj.textSettings.fontSize;
    parent.activeObj.textSettings.bold =
      bold || parent.activeObj.textSettings.bold;
    parent.activeObj.textSettings.italic =
      italic || parent.activeObj.textSettings.italic;
    parent.activeObj.strokeSettings.outlineColor =
      outlineColor || parent.activeObj.strokeSettings.outlineColor;
    parent.activeObj.strokeSettings.outlineWidth =
      outlineWidth || parent.activeObj.strokeSettings.outlineWidth;
  }
  ...
  private redrawActObj(x?: number, y?: number, isMouseDown?: boolean): void {
    ...
    if (
      ...
      if (
        ...
        if (x && y) {
          if (
            x !== parent.activeObj.activePoint.startX &&
            y !== parent.activeObj.activePoint.startY
          ) {
  ...
  private apply(shape?: string, obj?: SelectionPoint, canvas?: string): void {
    ...
    if (!parent.disabled) {
      if (parent.togglePen && !parent.currObjType.isCustomCrop) {
        ...
        if (
          isNullOrUndefined(parent.activeObj.shape) &&
          isNullOrUndefined(shape)
        ) {
          parent.currObjType.shape = '';
        } else {
        ...
        if (parent.currObjType.shape !== '') {
          ...
          if (
            !shape &&
            parent.currObjType.shape !== '' &&
            !parent.currObjType.isCustomCrop
          ) {
            parent.objColl.push(
  ...
  private setCenterPoints(
    ...
    parent.activeObj.activePoint.startX =
      parent.lowerCanvas.width / 2 - renderWidth / 2;
    parent.activeObj.activePoint.startY =
      parent.lowerCanvas.height / 2 - renderHeight / 2;
    parent.activeObj.activePoint.endX =
      parent.lowerCanvas.width / 2 + renderWidth / 2;
    parent.activeObj.activePoint.endY =
      parent.lowerCanvas.height / 2 + renderHeight / 2;
  ...
  private updSelChangeEventArgs(
    selectionSettings: CropSelectionSettings
  ): void {
    const parent: ImageEditor = this.parent;
    parent.activeObj.activePoint = {
  ...
  private updateShapeChangeEventArgs(
    ...
    if (
      shapeSettings.id &&
      shapeSettings.id.indexOf('shape_') === -1 &&
    ...
    if (
      shapeSettings.id &&
      shapeSettings.id.split('_')[0] &&
      shapeSettings.id.split('_')[0] === 'pen'
    ) {
      shapeId = parseInt(shapeSettings.id.split('_')[1], 10) - 1;
      parent.pointColl[shapeId as number].points = shapeSettings.points;
      parent.pointColl[shapeId as number].strokeColor =
        shapeSettings.strokeColor;
      parent.pointColl[shapeId as number].strokeWidth =
      ...
      switch (parent.activeObj.shape) {
        case 'ellipse':
          ...
          if (shapeSettings.degree) {
            parent.activeObj.rotatedAngle =
              shapeSettings.degree * (Math.PI / 180);
          }
          break;
        case 'line':
        case 'arrow':
          ...
          break;
        case 'text':
          parent.activeObj.keyHistory = parent.activeObj.textSettings.text =
            shapeSettings.text;
  ...
  private addLetter(letter: string): void {
    ...
    if (
      ...
      (parent.currObjType.isText || parent.activeObj.shape === 'text')
    ) {
      ...
      this.updateFontStyles();
      const width: number =
        this.upperContext.measureText(this.keyHistory).width + fontSize * 0.5;
      const height: number = fontSize;
      this.upperContext.fillText(
        this.keyHistory,
        parent.activeObj.activePoint.startX,
  ...
  private redrawText(): void {
    const parent: ImageEditor = this.parent;
    const { fontSize, fontFamily, bold, italic }: TextSettings =
      parent.activeObj.textSettings;
    ...
    const text: string =
      parent.textArea.style.display === 'block' ||
      parent.textArea.style.display === 'inline-block'
        ? this.getMaxText(true)
        : this.getMaxText();
    const width: number =
      this.upperContext.measureText(text).width + fontSize * 0.5;
  ...
  private updateTextFromTextArea(): void {
    const parent: ImageEditor = this.parent;
    let allowUndoRedo: boolean = false;
    const { fontSize }: TextSettings = parent.activeObj.textSettings;
    ...
    this.updateFontStyles();
    let width: number =
      this.upperContext.measureText(parent.activeObj.keyHistory).width +
      fontSize * 0.5;
    ...
    if (rows.length > 1) {
      ...
      for (let i: number = 0, len: number = rows.length; i < len; i++) {
        widthColl.push(
          this.upperContext.measureText(rows[i as number]).width +
            fontSize * 0.5
        );
      }
    ...
    if (parent.activeObj.rotatedAngle !== 0) {
      const width: number =
        parent.activeObj.activePoint.width - tempActiveObj.activePoint.width;
      const height: number =
        parent.activeObj.activePoint.height - tempActiveObj.activePoint.height;
      let value: string = '';
      if (width > 0 && height > 0) {
        value = 'widthHeight';
      } else if (width !== 0) {
      ...
      parent.activeObj.activePoint = extend(
        {},
        tempActiveObj.activePoint,
        {},
        true
      ) as ActivePoint;
      parent.notify('selection', {
        prop: 'adjustRotationPoints',
        onPropertyChange: false,
        value: {
        ...
        for (let i: number = 0; i < objColl.length; i++) {
          ...
          if (currentObj.order === index) {
            ...
            if (
              ...
              currentObj.redactImage = parent.activeObj.redactImage;
              if (
                parent.objColl[i as number] &&
                JSON.stringify(parent.objColl[i as number].activePoint) ===
                  JSON.stringify(currentObj.activePoint)
              ) {
                parent.objColl[i as number].redactImage =
                  parent.activeObj.redactImage;
              }
            }
            this.refreshActiveObj();
            index++;
  ...
  private getSmallestIndex(): number {
    ...
    for (let i: number = 0, len: number = parent.objColl.length; i < len; i++) {
      ...
      if (
        isNullOrUndefined(smallestIndex) ||
        currentObj.order < smallestIndex
      ) {
  ...
  private updImgRatioForActObj(): void {
    ...
    if (parent.activeObj.rotationCirclePointColl) {
      parent.activeObj.rotationCirclePointColl.ratioX =
        (parent.activeObj.rotationCirclePointColl.x - destLeft) / destWidth;
      parent.activeObj.rotationCirclePointColl.ratioY =
        (parent.activeObj.rotationCirclePointColl.y - destTop) / destHeight;
    }
  ...
  private zoomObjColl(preventApply?: boolean): void {
    ...
    if (parent.objColl.length > 0) {
      for (
        let i: number = 0, len: number = parent.objColl.length;
        i < len;
        i++
      ) {
        ...
        if (currObj.imageRatio) {
          currObj.activePoint.startX =
            currObj.imageRatio.startX * parent.img.destWidth +
            parent.img.destLeft;
          currObj.activePoint.startY =
            currObj.imageRatio.startY * parent.img.destHeight +
            parent.img.destTop;
          currObj.activePoint.endX =
            currObj.imageRatio.endX * parent.img.destWidth +
            parent.img.destLeft;
          currObj.activePoint.endY =
            currObj.imageRatio.endY * parent.img.destHeight +
            parent.img.destTop;
        }
        ...
        if (currObj.shape === 'text') {
          ...
          currObj.pointColl = this.getLinePoints(
            currObj.activePoint.startX,
            currObj.activePoint.startY,
            currObj.activePoint.endX,
            currObj.activePoint.endY
          ...
          for (
            let n: number = 0, len: number = currObj.pointColl.length;
            n < len;
            n++
          ) {
            currObj.pointColl[n as number].ratioX =
              (currObj.pointColl[n as number].x - parent.img.destLeft) /
              parent.img.destWidth;
            currObj.pointColl[n as number].ratioY =
              (currObj.pointColl[n as number].y - parent.img.destTop) /
              parent.img.destHeight;
          }
          ...
          if (
            ...
            (currObj.shape === 'line' || currObj.shape === 'arrow')
          ) {
            this.straightenShapePoints(currObj);
          }
        } else if (currObj.shape === 'path') {
          for (
            let l: number = 0, len: number = currObj.pointColl.length;
            l < len;
            l++
          ) {
            currObj.pointColl[l as number].x =
              currObj.pointColl[l as number].ratioX * parent.img.destWidth +
              parent.img.destLeft;
            currObj.pointColl[l as number].y =
              currObj.pointColl[l as number].ratioY * parent.img.destHeight +
              parent.img.destTop;
          }
        ...
        parent.notify('draw', {
          prop: 'updateActiveObject',
          onPropertyChange: false,
          value: { actPoint: currObj.activePoint, obj: currObj },
        });
        if (
          currObj.shape !== 'line' &&
          currObj.shape !== 'arrow' &&
          currObj.shape !== 'path' &&
          currObj.rotatedAngle !== 0
        ) {
          this.setPointCollForShapeRotation(currObj);
          currObj.rotationCirclePoint.x =
            currObj.rotationCirclePoint.ratioX * parent.img.destWidth +
            parent.img.destLeft;
          currObj.rotationCirclePoint.y =
            currObj.rotationCirclePoint.ratioY * parent.img.destHeight +
            parent.img.destTop;
  ...
  private straightenPath(obj: SelectionPoint): void {
    ...
    for (let j: number = 0, len: number = obj.pointColl.length; j < len; j++) {
      point = this.straightenPoints(
        obj.pointColl[j as number].x,
        obj.pointColl[j as number].y
      );
  ...
  private straightenFHD(): void {
    const parent: ImageEditor = this.parent;
    for (
      let i: number = 0, fLen: number = parent.freehandCounter;
      i < fLen;
      i++
    ) {
      parent.points = extend(
        [],
        parent.pointColl[i as number].points,
        []
      ) as Point[];
      ...
      for (let l: number = 0; l < len; l++) {
        ...
        parent.points[l as number].x = point.x;
        parent.points[l as number].y = point.y;
      }
    }
    const selPointCollObj: Object = { selPointColl: null };
    parent.notify('freehand-draw', {
    ...
    for (
      ...
      if (
        selPointCollObj['selPointColl'][i as number] &&
        selPointCollObj['selPointColl'][i as number].points
      ) {
        const len: number =
          selPointCollObj['selPointColl'][i as number].points.length;
        let point: Point;
        for (let l: number = 0; l < len; l++) {
          point = this.straightenPoints(
            selPointCollObj['selPointColl'][i as number].points[l as number].x,
    ...
    if (
      ...
      const angle: number =
        ((parent.transform.straighten === 360
          ? 0
          : parent.transform.straighten) -
          obj['angle']) *
        (Math.PI / 180);
      const point: Point = this.straightenPoints(
        straightenObj['straightenPoint']['x'],
        straightenObj['straightenPoint']['y'],
        angle
      );
  ...
  private straightenPoints(x: number, y: number, angle?: number): Point {
    ...
    const point: Point = {
      x:
        Math.cos(angle) * (x - center.x) -
        Math.sin(angle) * (y - center.y) +
        center.x,
      y:
        Math.sin(angle) * (x - center.x) +
        Math.cos(angle) * (y - center.y) +
        center.y,
    };
    return point;
  }
  ...
  private straightenShapes(): void {
    ...
    parent.notify('draw', {
      prop: 'getImageCanvasPoints',
      value: { obj: object },
    });
    const center: Point = {
      x: destLeft + destWidth / 2,
      y: destTop + destHeight / 2,
    };
    const angle: number = -(parent.transform.straighten * (Math.PI / 180));
    const p1: Point = {
      x:
        Math.cos(angle) * (object['points'][0]['x'] - center.x) -
        Math.sin(angle) * (object['points'][0]['y'] - center.y) +
        center.x,
      y:
        Math.sin(angle) * (object['points'][0]['x'] - center.x) +
        Math.cos(angle) * (object['points'][0]['y'] - center.y) +
        center.y,
    };
    const p2: Point = {
      x:
        Math.cos(angle) * (object['points'][1]['x'] - center.x) -
        Math.sin(angle) * (object['points'][1]['y'] - center.y) +
        center.x,
      y:
        Math.sin(angle) * (object['points'][1]['x'] - center.x) +
        Math.cos(angle) * (object['points'][1]['y'] - center.y) +
        center.y,
    };
    const p3: Point = {
      x:
        Math.cos(angle) * (object['points'][2]['x'] - center.x) -
        Math.sin(angle) * (object['points'][2]['y'] - center.y) +
        center.x,
      y:
        Math.sin(angle) * (object['points'][2]['x'] - center.x) +
        Math.cos(angle) * (object['points'][2]['y'] - center.y) +
        center.y,
    };
  ...
  private straightenShapePoints(
    ...
    if (obj.shape === 'line' || obj.shape === 'arrow') {
      obj.activePoint.width =
        obj.activePoint.endX > obj.activePoint.startX
          ? obj.activePoint.endX - obj.activePoint.startX
          : obj.activePoint.startX - obj.activePoint.endX;
      obj.activePoint.height =
        obj.activePoint.endY > obj.activePoint.startY
          ? obj.activePoint.endY - obj.activePoint.startY
      ...
      const start: Point = {
        x:
          Math.cos(angle) * (obj.activePoint.startX - center.x) -
          Math.sin(angle) * (obj.activePoint.startY - center.y) +
          center.x,
        y:
          Math.sin(angle) * (obj.activePoint.startX - center.x) +
          Math.cos(angle) * (obj.activePoint.startY - center.y) +
          center.y,
      };
      const end: Point = {
        x:
          Math.cos(angle) * (obj.activePoint.endX - center.x) -
          Math.sin(angle) * (obj.activePoint.endY - center.y) +
          center.x,
        y:
          Math.sin(angle) * (obj.activePoint.endX - center.x) +
          Math.cos(angle) * (obj.activePoint.endY - center.y) +
          center.y,
      };
  ...
  private redrawObj(degree?: number | string): void {
    ...
    if (parent.objColl.length > 0) {
      if (
        ...
        degree === 'Horizontal' ||
        degree === 'Vertical' ||
        degree === 'horizontalVertical' ||
        degree === 'verticalHorizontal'
      ) {
        ...
        for (
          ...
          const splitWords: string[] =
            parent.objColl[i as number].shape.split('-');
          if (splitWords[0] !== 'crop') {
            this.apply(
              parent.objColl[i as number].shape,
              parent.objColl[i as number]
            );
            isShape = true;
          }
  ...
  private updateCurrentActiveObjPoint(degree: number | string): void {
    ...
    const { destLeft, destTop, destWidth, destHeight } = parent.img;
    for (
      let index: number = 0, len: number = parent.objColl.length;
      index < len;
      index++
    ) {
      const currObj: SelectionPoint = parent.objColl[index as number];
      if (
        parent.activeObj.shape === currObj.shape &&
    ...
    if (
        ...
        for (
          ...
          if (currObj.shapeFlip !== parent.transform.currFlipState) {
            if (currObj.activePoint.startX <= destLeft + destWidth / 2) {
              currObj.activePoint.endX =
                destLeft + destWidth - (currObj.activePoint.startX - destLeft);
              currObj.activePoint.startX =
                currObj.activePoint.endX - currObj.activePoint.width;
              parent.notify('draw', {
                prop: 'updateActiveObject',
                onPropertyChange: false,
                value: { actPoint: currObj.activePoint, obj: currObj },
              ...
              currObj.activePoint.startX =
                destLeft + (destLeft + destWidth - currObj.activePoint.endX);
              currObj.activePoint.endX =
                currObj.activePoint.startX + currObj.activePoint.width;
              parent.notify('draw', {
                prop: 'updateActiveObject',
                onPropertyChange: false,
                value: { actPoint: currObj.activePoint, obj: currObj },
            ...
            if (
              ...
              currObj.rotatedAngle =
                currObj.rotatedAngle + (Math.PI - currObj.rotatedAngle) * 2;
              if (
                currObj.rotationCirclePointColl.x <=
                destLeft + destWidth / 2
              ) {
                currObj.rotationCirclePointColl.x =
                  destLeft +
                  destWidth -
        ...
        for (let i: number = 0; i < parent.objColl.length; i++) {
          ...
          if (currObj.shapeFlip !== parent.transform.currFlipState) {
            if (currObj.activePoint.startY <= destTop + destHeight / 2) {
              currObj.activePoint.endY =
                destTop + destHeight - (currObj.activePoint.startY - destTop);
              currObj.activePoint.startY =
                currObj.activePoint.endY - currObj.activePoint.height;
              parent.notify('draw', {
                prop: 'updateActiveObject',
                onPropertyChange: false,
                value: { actPoint: currObj.activePoint, obj: currObj },
              ...
              currObj.activePoint.startY >=
              parent.lowerCanvas.height / 2
            ) {
              currObj.activePoint.startY =
                destTop + (destTop + destHeight - currObj.activePoint.endY);
              currObj.activePoint.endY =
                currObj.activePoint.startY + currObj.activePoint.height;
              parent.notify('draw', {
                prop: 'updateActiveObject',
                onPropertyChange: false,
                value: { actPoint: currObj.activePoint, obj: currObj },
            ...
            if (
              ...
              if (
                ...
                currObj.rotationCirclePointColl.y =
                  destTop +
                  destHeight -
                  (currObj.rotationCirclePointColl.y - destTop);
              } else if (
                currObj.rotationCirclePointColl.y >=
                destTop + destHeight / 2
              ) {
                currObj.rotationCirclePointColl.y =
                  destTop +
                  (destTop + destHeight - currObj.rotationCirclePointColl.y);
              }
      ...
      if (currActObjIndex !== undefined) {
        parent.activeObj = extend(
          {},
          parent.objColl[currActObjIndex as number],
          {},
          true
        ) as SelectionPoint;
      }
  ...
  private rotateObjColl(): void {
    ...
    for (let i: number = 0, len: number = parent.objColl.length; i < len; i++) {
      ...
      currObj.activePoint.startY =
        destTop + destHeight * currObj.imageRatio.startX;
      currObj.activePoint.endY = destTop + destHeight * currObj.imageRatio.endX;
      currObj.activePoint.startX =
        destLeft + destWidth - destWidth * currObj.imageRatio.endY;
      currObj.activePoint.endX =
        destLeft + destWidth - destWidth * currObj.imageRatio.startY;
      ...
      if (shape === 'line' || shape === 'arrow' || shape === 'path') {
        ...
        currObj.rotationCirclePointColl.y =
          destTop + destHeight * currObj.rotationCirclePointColl.ratioX;
        currObj.rotationCirclePointColl.x =
          destLeft +
          destWidth -
    ...
    for (let i: number = 0, len: number = parent.objColl.length; i < len; i++) {
      parent.notify('draw', {
        ...
        onPropertyChange: false,
        value: {
          actPoint: parent.objColl[i as number].activePoint,
  ...
  private rotateLineArrowObj(obj: SelectionPoint): void {
    ...
    if (obj.pointColl.length > 0) {
      for (let n: number = 0; n < obj.pointColl.length; n++) {
        obj.pointColl[n as number].y =
          destTop + destHeight * obj.pointColl[n as number].ratioX;
        obj.pointColl[n as number].x =
          destLeft + destWidth - destWidth * obj.pointColl[n as number].ratioY;
      }
      for (let n: number = 0; n < obj.pointColl.length; n++) {
        obj.pointColl[n as number].ratioX =
          (obj.pointColl[n as number].x - destLeft) / destWidth;
        obj.pointColl[n as number].ratioY =
          (obj.pointColl[n as number].y - destTop) / destHeight;
      }
      let prevPoint: Point;
      if (isNullOrUndefined(obj.pointColl[obj.pointColl.length - 2])) {
        ...
        prevPoint = {
          x: obj.pointColl[obj.pointColl.length - 2].x,
          y: obj.pointColl[obj.pointColl.length - 2].y,
        };
      }
      const diffX: number =
        obj.pointColl[obj.pointColl.length - 1].x - prevPoint.x;
      const diffY: number =
        obj.pointColl[obj.pointColl.length - 1].y - prevPoint.y;
      obj.activePoint.startX = obj.pointColl[0].x;
      obj.activePoint.startY = obj.pointColl[0].y;
      obj.activePoint.endX =
        obj.pointColl[obj.pointColl.length - 1].x + diffX / 2;
      obj.activePoint.endY =
        obj.pointColl[obj.pointColl.length - 1].y + diffY / 2;
  ...
  private flipLineArrowObj(obj: SelectionPoint, value: string): void {
    ...
    if (obj.activePoint.startX > obj.activePoint.endX) {
      let temp: number = obj.activePoint.startX;
      obj.activePoint.startX = obj.activePoint.endX;
  ...
  private lineArrowHorizontalFlip(obj: SelectionPoint): void {
    ...
    if (obj.shapeFlip !== parent.transform.currFlipState) {
      for (
        let l: number = 0, len: number = obj.pointColl.length;
        l < len;
        l++
      ) {
  ...
  private renderTextArea(x: number, y: number, actObj: SelectionPoint): void {
    ...
    const zOrderElem: HTMLElement = parent.element.querySelector(
      '#' + parent.element.id + '_zOrderBtn'
    );
    const dupElem: HTMLElement = parent.element.querySelector(
      '#' + parent.element.id + '_duplicate'
    );
    const removeElem: HTMLElement = parent.element.querySelector(
      '#' + parent.element.id + '_remove'
    );
    const editTextElem: HTMLElement = parent.element.querySelector(
      '#' + parent.element.id + '_editText'
    );
    ...
    const adjustedOutlineWidth: number =
      baseWidth * (Math.floor((fontSize - 1) / 16) * 0.5 + 0.5);
    if (
      ...
      for (
        let x: number = -adjustedOutlineWidth;
        x <= adjustedOutlineWidth;
        x++
      ) {
        for (
          let y: number = -adjustedOutlineWidth;
          y <= adjustedOutlineWidth;
          y++
        ) {
          if (x !== 0 || y !== 0) {
    ...
    parent.textArea.style.fontWeight = actObj.textSettings.bold
      ? 'bold'
      : 'normal';
    parent.textArea.style.fontStyle = actObj.textSettings.italic
      ? 'italic'
      : 'normal';
    parent.textArea.style.border =
      '2px solid ' + parent.themeColl[parent.theme]['primaryColor'];
    ...
    if (
      parseFloat(parent.textArea.style.maxHeight) <
      parent.activeObj.textSettings.fontSize
    ) {
      parent.textArea.style.maxHeight =
        parent.activeObj.textSettings.fontSize + 'px';
    }
    if (degree % 90 === 0 && degree % 180 !== 0) {
      if (
        parseFloat(parent.textArea.style.left) +
          parseFloat(parent.textArea.style.width) >
    ...
    if (actObj.rotatedAngle !== 0) {
      let tempLeft: number = parseFloat(parent.textArea.style.left);
      let tempTop: number = parseFloat(parent.textArea.style.top);
      if (actObj.flipObjColl.length > 0) {
        ...
        if (panObj['panRegion'] !== '') {
          if (panObj['panRegion'] === 'horizontal') {
            ...
            center = {
              x: clientWidth - clientWidth / 2,
              y: clientHeight - clientHeight / 2,
            };
            tempLeft = center.x - tempLeft + center.x;
            tempTop = center.y - tempTop + center.y;
          }
      ...
      let p1: Point = {
        x:
          cosAngle * (left - center.x) - sinAngle * (top - center.y) + center.x,
        y:
          sinAngle * (left - center.x) + cosAngle * (top - center.y) + center.y,
      };
      if (
        p1.x > parent.img.destLeft &&
        p1.x < parent.img.destLeft + parent.img.destWidth &&
        // eslint-disable-next-line max-len
        p1.y > parent.img.destTop &&
        p1.y + parseFloat(parent.textArea.style.fontSize) <
          parent.img.destTop + parent.img.destHeight
      ) {
        ...
        while (true) {
          ...
          p1 = {
            x:
              cosAngle * (left - center.x) -
              sinAngle * (top - center.y) +
              center.x,
            y:
              sinAngle * (left - center.x) +
              cosAngle * (top - center.y) +
              center.y,
          };
          if (
            (p1.x > parent.img.destLeft &&
  ...
  private setTextBoxWidth(e?: KeyboardEvent): void {
    ...
    const textAreaWidth: number =
      this.upperContext.measureText(text).width +
      parseFloat(parent.textArea.style.fontSize) / 2;
    const letterWidth: number = e
      ? this.upperContext.measureText(String.fromCharCode(e.which)).width
      : 0;
    const actObj: SelectionPoint = extend(
      {},
      parent.activeObj,
      {},
      true
    ...
    let flip: string = '';
    const degree: number = this.getRotDegOfShape(actObj);
    ...
    if (
      ...
      if (degree === 0) {
        if (flip.toLowerCase() === 'horizontal') {
          ...
          parent.img.destWidth -
            (parseFloat(parent.textArea.style.left) - parent.img.destLeft) >
          textAreaWidth + letterWidth
        ) {
          parent.textArea.style.width = textAreaWidth + letterWidth + 'px';
        }
        ...
        if (flip.toLowerCase() === 'vertical') {
          if (
            parseFloat(parent.textArea.style.top) -
              parent.img.destTop -
              textAreaWidth -
              letterWidth >
            0
          ...
          parent.img.destHeight -
            (parseFloat(parent.textArea.style.top) - parent.img.destTop) >
          textAreaWidth + letterWidth
        ) {
          parent.textArea.style.width = textAreaWidth + letterWidth + 'px';
        }
        ...
        if (flip.toLowerCase() === 'horizontal') {
          const remainingWidth: number =
            parent.img.destWidth - (textAreaLeft - destLeft);
        ...
        if (flip.toLowerCase() === 'vertical') {
          const remainingHeight: number =
            parent.img.destHeight - (textAreaTop - destTop);
    ...
    switch (degree) {
      case 0:
        if (flip.toLowerCase() === 'vertical') {
          parent.textArea.style.maxHeight =
            parent.img.destHeight -
            (parent.img.destHeight - parseFloat(parent.textArea.style.top)) +
            'px';
        } else {
          textAreaTop =
            parseFloat(parent.textArea.style.top) - parent.img.destTop;
          parent.textArea.style.maxHeight =
            parent.img.destHeight - textAreaTop + 'px';
        }
        break;
      case 90:
        if (flip.toLowerCase() === 'horizontal') {
          ...
          parent.textArea.style.maxHeight =
            parseFloat(parent.textArea.style.left) - parent.img.destLeft + 'px';
        }
        break;
      case 180:
        if (flip.toLowerCase() === 'vertical') {
          ...
          parent.textArea.style.maxHeight =
            parseFloat(parent.textArea.style.top) - parent.img.destTop + 'px';
        }
        break;
      case 270:
        if (flip.toLowerCase() === 'horizontal') {
  ...
  private updatePathRatio(obj: SelectionPoint): void {
    ...
    for (let i: number = 0, len: number = obj.pointColl.length; i < len; i++) {
      ...
      currPoint.ratioX =
        (currPoint.x - parent.img.destLeft) / parent.img.destWidth;
      currPoint.ratioY =
    ...
    if (parent.activeObj.shape === 'path') {
      ...
      if (obj['shape'] === 'path') {
        ...
        if (e) {
          ...
          parent.notify('draw', {
            prop: 'redrawImgWithObj',
            onPropertyChange: false,
          });
          if (parent.objColl.length > 0) {
            const obj1: ActivePoint = parent.activeObj.activePoint;
            const obj2: ActivePoint =
              parent.objColl[parent.objColl.length - 1].activePoint;
            if (
              Math.floor(obj1.startX) === Math.floor(obj2.startX) &&
        ...
        if (parent.objColl[parent.objColl.length - 1]) {
          ...
          parent.selectShape(
            parent.objColl[parent.objColl.length - 1].currIndex
          );
          parent.notify('selection', {
            prop: 'setCurrentDrawingShape',
            value: { value: 'path' },
        ...
        const shapeSettings: ShapeSettings = obj['shapeSettingsObj'];
        const shapeResizingArgs: ShapeChangeEventArgs = {
          cancel: false,
          action: 'draw-end',
          previousShapeSettings: shapeSettings,
        };
        const shapeMovingArgs: ShapeChangeEventArgs = {
          cancel: false,
          action: 'move',
          previousShapeSettings: shapeSettings,
        };
        parent.notify('selection', {
          ...
          onPropertyChange: false,
          value: {
            shapeResizingArgs: shapeResizingArgs,
            shapeMovingArgs: shapeMovingArgs,
            type: 'mouse-up',
          },
        });
        parent.notify('undo-redo', {
          prop: 'updateUndoRedoStack',
          onPropertyChange: false,
        });
      }
    }
  ...
  private findTextTarget(e: MouseEvent & TouchEvent): void {
    ...
    parent.notify('toolbar', {
      prop: 'setPreventZoomBtn',
      onPropertyChange: false,
      value: { isPrevent: true },
    });
    parent.notify('toolbar', {
      prop: 'refresh-toolbar',
      onPropertyChange: false,
    ...
    if (!isNullOrUndefined(x) && !isNullOrUndefined(y)) {
      const bbox: DOMRect =
        parent.lowerCanvas.getBoundingClientRect() as DOMRect;
      x -= bbox.left;
      y -= bbox.top;
      let flip: string = '';
      const degree: number = this.getRotDegOfShape(parent.activeObj);
      ...
      if (parent.textArea.style.display === 'none') {
        ...
        for (let i: number = 0; i < parent.objColl.length; i++) {
          if (
            JSON.stringify(parent.activeObj) ===
            JSON.stringify(parent.objColl[i as number])
          ) {
        ...
        parent.notify('draw', {
          prop: 'clearOuterCanvas',
          onPropertyChange: false,
          value: { context: this.lowerContext },
        });
        parent.notify('draw', {
          prop: 'clearOuterCanvas',
          onPropertyChange: false,
          value: { context: this.upperContext },
        });
        if (
          (parent.currSelectionPoint &&
            parent.currSelectionPoint.shape === 'crop-circle') ||
        ...
        parent.notify('draw', {
          prop: 'checkPointPosition',
          onPropertyChange: false,
          value: { obj: obj },
        });
        if (
          (actObj.rotatedAngle !== 0 &&
            (obj['position'] === 'inside' || obj['position'] === 'on')) ||
          (actObj.rotatedAngle === 0 &&
            x >= actObj.activePoint.startX - radius * 2 &&
          ...
          if (actObj.flipObjColl.length <= 1) {
            const points: Point = this.setTextBoxPos(
              actObj,
              degree,
              flip,
              x,
            ...
            const points: Point = this.setTextBoxPoints(
              actObj,
              degree,
              flip,
              x,
      ...
      const temp: string = parent.textArea.value;
  ...
  private fileChanged(e: any): void {
    ...
    const fileExtension: string =
      fileData.name && fileData.name.split('.').pop().toLowerCase();
    if (
      fileExtension &&
    ...
    (
      document.getElementById(
        this.parent.element.id + '_fileUpload'
      ) as HTMLInputElement
    ).value = '';
  }
  ...
  private onLoadImgShape(
    ...
    if (typeof url === 'string') {
      ...
      parent.inMemoryCanvas
        .getContext('2d')
        .putImageData(url as ImageData, 0, 0);
    ...
    this.shapeImg.onload = () => {
      proxy.upperContext.drawImage(
        proxy.shapeImg,
        0,
        0,
        proxy.shapeImg.width,
        proxy.shapeImg.height
      );
      proxy.updateImgCanvas(
        isSelect,
        x,
        y,
        width,
  ...
  private updateImgCanvas(
    ...
    parent.notify('transform', {
      ...
      onPropertyChange: false,
      value: {
        width: this.shapeImg.width,
        height: this.shapeImg.height,
        obj: dimObj,
        isImgShape: null,
      },
    });
  ...
  private updateObj(dimObj: Object, x: number, y: number): void {
    ...
    parent.activeObj.activePoint.startX = x
      ? x
      : parent.lowerCanvas.width / 2 - dimObj['width'] / 2;
    parent.activeObj.activePoint.startY = y
      ? y
      : parent.lowerCanvas.height / 2 - dimObj['height'] / 2;
    parent.activeObj.activePoint.endX =
      parent.activeObj.activePoint.startX + dimObj['width'];
    parent.activeObj.activePoint.endY =
      parent.activeObj.activePoint.startY + dimObj['height'];
  }
  ...
  private resizeImage(newWidth: number, aspectRatio: string): Object {
    ...
    const newHeight: number = Math.round(
      (newWidth * aspectRatioHeight) / aspectRatioWidth
    );
    return { width: newWidth, height: newHeight };
  }

  private setTextBoxPos(
    actObj: SelectionPoint,
    degree: number,
    flip: string,
    x: number,
    y: number
  ...
  private setTextBoxPoints(
    ...
    switch (degree) {
      case 0:
        if (
          actObj.flipObjColl[0] &&
          actObj.flipObjColl[0].toLowerCase() === 'horizontal'
        ) {
          if (flip === 'horizontal') {
            point.x = startX;
            point.y = startY;
    ...
    if (parent.objColl.length > 0) {
      for (
        ...
        if (currObj.shape === 'line' || currObj.shape === 'arrow') {
          ...
          for (
            let j: number = 0, len: number = currObj.pointColl.length;
            j < len;
            j++
          ) {
            currObj.pointColl[j as number].ratioX =
              (currObj.pointColl[j as number].x - parent.img.destLeft) /
              parent.img.destWidth;
            currObj.pointColl[j as number].ratioY =
              (currObj.pointColl[j as number].y - parent.img.destTop) /
              parent.img.destHeight;
          }
        }
        this.refreshActiveObj();
  ...
  private updateFontStyles(isTextBox?: boolean): void {
    ...
    const fontSize: number = isTextBox
      ? parseFloat(parent.textArea.style.fontSize)
      : parent.activeObj.textSettings.fontSize;
    const fontFamily: string =
      parent.textArea.style.display === 'block' ||
      parent.textArea.style.display === 'inline-block'
        ? parent.textArea.style.fontFamily
        : parent.activeObj.textSettings.fontFamily;
  ...
  private applyFontStyle(item: string): void {
    ...
    const shapeChangedArgs: ShapeChangeEventArgs = {
      action: 'font-style',
      currentShapeSettings: extend(
        {},
        shapeSettings,
        {},
        true
    ...
    if (style.display === 'block' || style.display === 'inline-block') {
      ...
      const value: string =
        style.fontWeight === 'normal' && style.fontStyle === 'normal'
          ? 'default'
          : style.fontWeight === 'bold' && style.fontStyle === 'normal'
          ? 'bold'
          : style.fontWeight === 'normal' && style.fontStyle === 'italic'
          ? 'italic'
          : 'bolditalic';
      const width: number = this.getTextAreaWidth(value);
      style.width = width + 'px';
      this.updateObjColl(item, objColl);
    } else {
      this.textSettings.bold = parent.activeObj.textSettings.bold =
        fontWeight === 'normal' ? false : true;
      this.textSettings.italic = parent.activeObj.textSettings.italic =
        fontStyle === 'normal' ? false : true;
      if (
  ...
  private updateArrowRatio(obj: SelectionPoint): void {
    ...
    for (dimension of dimensions) {
      const ratioX: number =
        length / object['arrowDimension'][dimension as string]['width'];
      const ratioY: number =
        length / object['arrowDimension'][dimension as string]['height'];
  ...
  private updateArrowSize(obj: SelectionPoint): void {
    ...
    for (dimension of dimensions) {
      const ratioX: number =
        object['arrowDimension'][dimension as string]['ratioX'];
      const ratioY: number =
        object['arrowDimension'][dimension as string]['ratioY'];
  ...
  private updateFontRatio(obj: SelectionPoint, isTextArea?: boolean): void {
    ...
    if (isNullOrUndefined(isTextArea)) {
      ...
      obj.textSettings.fontRatio =
        width / parseFloat(parent.textArea.style.fontSize);
    }
  }
  ...
  private updateFontSize(obj: SelectionPoint): void {
    const degree: number = this.getRotDegOfShape(obj);
    if (degree === 0 || Math.abs(degree) === 180) {
      obj.textSettings.fontSize =
        obj.activePoint.width / obj.textSettings.fontRatio;
    } else {
      obj.textSettings.fontSize =
        obj.activePoint.height / obj.textSettings.fontRatio;
    }
  }
  ...
  private pushActItemIntoObj(): void {
    ...
    if (parent.textArea.style.display === 'none') {
      ...
      const temp: SelectionPoint = extend(
        {},
        parent.activeObj,
        {},
        true
  ...
  private applyActObj(isMouseDown?: boolean): void {
    ...
    if (
      parent.activeObj.shape !== undefined &&
      parent.activeObj.shape === 'text' &&
      parent.activeObj.keyHistory === ''
    ) {
      this.refreshActiveObj();
      this.upperContext.clearRect(
        0,
      ...
      if (
        ...
        if (!isActObj) {
          ...
          let tempObjColl: SelectionPoint[] = parent.objColl.splice(
            0,
            parseInt(splitWords[1], 10) - 1
          );
          tempObjColl.push(
            extend({}, parent.activeObj, {}, true) as SelectionPoint
          );
  ...
  private alignTextAreaIntoCanvas(): void {
    const parent: ImageEditor = this.parent;
    const letters: string = parent.textArea.value;
    parent.textArea.value = '';
    for (let i: number = 0, len: number = letters.length; i < len; i++) {
      parent.textArea.value += letters[i as number];
      parent.textArea.style.height = 'auto';
  ...
  private transformTextArea(): void {
    ...
    if (parent.activeObj.shape === 'text') {
      ...
      const rotatedDegree: number =
        parent.activeObj.rotatedAngle * (180 / Math.PI);
      let scale: string = '';
      let degree: number = this.getRotDegOfShape(parent.activeObj);
      if (parent.activeObj.flipObjColl.length > 0) {
        for (let i: number = 0; i < parent.activeObj.flipObjColl.length; i++) {
          if (degree !== 0 && degree % 90 === 0 && degree !== 180) {
            scale +=
              parent.activeObj.flipObjColl[i as number].toLowerCase() ===
              'horizontal'
                ? 'scale(1, -1)'
                : 'scale(-1, 1)';
          } else {
  ...
  private getTextAreaWidth(item: string): number {
    ...
    const isTextArea: boolean =
      parent.textArea.style.display === 'none' ? false : true;
    ...
    if (!isTextArea) {
      ...
      width =
        this.upperContext.measureText(parent.textArea.value).width +
        parent.activeObj.textSettings.fontSize * 0.5;
    }
    parent.activeObj.textSettings.bold = tempBold;
    parent.activeObj.textSettings.italic = tempItalic;
  ...
  private getRedactObjDetails(obj: SelectionPoint): RedactSettings {
    const parent: ImageEditor = this.parent;
    const redactDetails: RedactSettings = {} as RedactSettings;
      ...
      case 'arrow':
        ...
        if (obj.shape === 'arrow') {
          ...
          parent.notify('selection', {
            prop: 'getArrowType',
            onPropertyChange: false,
            value: { type: obj.start, obj: arrowObj },
          });
          shapeDetails.arrowHead = arrowObj['type'];
          parent.notify('selection', {
            prop: 'getArrowType',
            onPropertyChange: false,
            value: { type: obj.end, obj: arrowObj },
          });
  ...
  private getFreehandDrawDetails(index: number): ShapeSettings {
    ...
    shapeDetails.points = extend(
      [],
      parent.pointColl[index as number].points
    ) as Point[];
  ...
  private getShapeSetting(id: string, obj: Object): void {
    ...
    if (!parent.disabled && parent.isImageLoaded) {
      ...
      if (id.split('_')[0] === 'shape') {
        ...
        shapeDetails = this.getFreehandDrawDetails(
          parseInt(id.split('_')[1], 10) - 1
        );
      }
    }
  ...
  private getShapeSettings(obj: Object): void {
    ...
    if (!parent.disabled && parent.isImageLoaded) {
      ...
      for (
        ...
        const shapeDetails: ShapeSettings = this.getObjDetails(
          parent.objColl[i as number]
        );
        shapeDetailsColl.push(shapeDetails);
      }
      for (let i: number = 0; i < parent.freehandCounter; i++) {
        const shapeDetails: ShapeSettings = this.getFreehandDrawDetails(
          i as number
        );
        shapeDetailsColl.push(shapeDetails);
      }
  ...
  private getRedactSettings(obj: Object): void {
    const parent: ImageEditor = this.parent;
    const RedactDetailsColl: RedactSettings[] = [];
    if (!parent.disabled && parent.isImageLoaded) {
      if (parent.textArea.style.display !== 'none') {
        parent.okBtn(null, true);
      } else {
      ...
      for (
        ...
        const redactDetails: RedactSettings = this.getRedactObjDetails(
          parent.objColl[i as number]
        );
  ...
  private alignRotateFlipColl(
    ...
    isRotateFlipCollection?: boolean,
    obj?: Object
  ): number[] | string[] {
  ...
  private popForDefaultTransformedState(
    ...
    let rotateRight: number = 0;
    let rotateleft: number = 0;
    let horizontal: number = 0;
    let vertical: number = 0;
    for (let i: number = 0; i < collection.length; i++) {
      if (
        collection[i as number] === 90 ||
        collection[i as number] === 'rotateRight'
      ) {
        rotateRight++;
        rotateleft = 0;
        horizontal = 0;
        ...
        collection[i as number] === -90 ||
        collection[i as number] === 'rotateLeft'
      ) {
        rotateleft++;
        rotateRight = 0;
        horizontal = 0;
        ...
        collection[i as number] === 'horizontal' ||
        collection[i as number] === 'Horizontal' ||
        ...
        collection[i as number] === 'vertical' ||
        collection[i as number] === 'Vertical' ||
  ...
  private popForDefaultFlipState(
    ...
    for (
      let i: number = 0, iLen: number = collection.length - 3;
      i < iLen;
      i++
    ) {
      ...
      const isVertical: boolean =
        ...
        collection[i as number] === 'Vertical' ||
        collection[i as number] === 'verticalFlip';
      const isNextHorizontal: boolean =
        collection[i + 1] === 'horizontal' ||
        collection[i + 1] === 'Horizontal' ||
        collection[i + 1] === 'horizontalFlip';
      const isNextVertical: boolean =
        collection[i + 1] === 'vertical' ||
        collection[i + 1] === 'Vertical' ||
        collection[i + 1] === 'verticalFlip';
      const isNextToNextHorizontal: boolean =
        collection[i + 2] === 'horizontal' ||
        collection[i + 2] === 'Horizontal' ||
        collection[i + 2] === 'horizontalFlip';
      const isNextToNextVertical: boolean =
        collection[i + 2] === 'vertical' ||
        collection[i + 2] === 'Vertical' ||
        collection[i + 2] === 'verticalFlip';
      const isNextToNextToNextHorizontal: boolean =
        collection[i + 3] === 'horizontal' ||
        collection[i + 3] === 'Horizontal' ||
        collection[i + 3] === 'horizontalFlip';
      if (
        (isHorizontal &&
          isNextVertical &&
          isNextToNextHorizontal &&
          isNextToNextVertical) ||
        (isVertical &&
          isNextHorizontal &&
          isNextToNextVertical &&
          isNextToNextToNextHorizontal)
      ) {
  ...
  private popForDefaultRotateState(
    ...
    for (let i: number = 0; i < collection.length - 1; i++) {
      ...
      if (
        (curr === 90 || curr === 'rotateRight') &&
        (next === -90 || next === 'rotateLeft')
      ) {
        collection.splice(i, 2);
        i -= 2;
      } else if (
        (curr === -90 || curr === 'rotateLeft') &&
        (next === 90 || next === 'rotateRight')
      ) {
        collection.splice(i, 2);
        i -= 2;
      }
  ...
  private selectShape(id: string, obj: Object): void {
    ...
    if (!parent.disabled && parent.isImageLoaded) {
      ...
      if (id.split('_')[0] === 'shape') {
        ...
        parent.notify('freehand-draw', {
          prop: 'isFHDIdx',
          value: { index: parseInt(id.split('_')[1], 10) - 1, obj: obj },
        });
        if (obj['isIndex']) {
          isSelected = true;
          parent.notify('freehand-draw', {
  ...
  private getMaxText(isTextBox?: boolean, text?: string, obj?: Object): string {
    if (isNullOrUndefined(text)) {
      text = isTextBox
        ...
        : this.parent.activeObj.keyHistory;
    }
    let maxi: number;
  ...
  private getLinePoints(
    ...
    if (x1 === x2) {
      ...
      const m: number = this.getSlope(i, j, true);
      const b: number = this.getIntercept(i, m);
      for (let y: number = i[1]; y <= j[1]; y++) {
        const x: number = m * y + b;
        points.push({ x: x, y: y });
      }
    } else {
      if (x1 < x2) {
      ...
      const m: number = this.getSlope(i, j, false);
      const b: number = this.getIntercept(i, m);
    ...
    if (
      ...
      (points.length < 10 && (y2 - y1 > 10 || y1 - y2 > 10))
    ) {
      points = [];
      const lesserY: number = Math.min(y1, y2);
      for (
        let i: number = 0;
        i < Math.abs(Math.floor(y2) - Math.floor(y1));
        i++
      ) {
      ...
      if (points.length > 1) {
        ...
        points.push({
          x: points[points.length - 1]['x'] + diffX / 2,
          y: points[points.length - 1]['y'] + diffY / 2,
        });
      }
    } else if (
      Math.floor(y1) === Math.floor(y2) ||
      (points.length < 10 && (x2 - x1 > 10 || x1 - x2 > 10))
    ) {
      points = [];
      const lesserX: number = Math.min(x1, x2);
      for (
        let i: number = 0;
        i < Math.abs(Math.floor(x2) - Math.floor(x1));
        i++
      ) {
  ...
  private setPointCollForShapeRotation(obj: SelectionPoint): void {
    ...
    for (let i: number = 0; i < obj.horTopLinePointColl.length; i++) {
      obj.horTopLinePointColl[i as number].ratioX =
        (obj.horTopLinePointColl[i as number].x - this.parent.img.destLeft) /
        this.parent.img.destWidth;
      obj.horTopLinePointColl[i as number].ratioY =
        (obj.horTopLinePointColl[i as number].y - this.parent.img.destTop) /
        this.parent.img.destHeight;
    }
    for (let i: number = 0; i < obj.horBottomLinePointColl.length; i++) {
      obj.horBottomLinePointColl[i as number].ratioX =
        (obj.horBottomLinePointColl[i as number].x - this.parent.img.destLeft) /
        this.parent.img.destWidth;
      obj.horBottomLinePointColl[i as number].ratioY =
        (obj.horBottomLinePointColl[i as number].y - this.parent.img.destTop) /
        this.parent.img.destHeight;
    }
    for (let i: number = 0; i < obj.verLeftLinePointColl.length; i++) {
      obj.verLeftLinePointColl[i as number].ratioX =
        (obj.verLeftLinePointColl[i as number].x - this.parent.img.destLeft) /
        this.parent.img.destWidth;
      obj.verLeftLinePointColl[i as number].ratioY =
        (obj.verLeftLinePointColl[i as number].y - this.parent.img.destTop) /
        this.parent.img.destHeight;
    }
    for (let i: number = 0; i < obj.verRightLinePointColl.length; i++) {
      obj.verRightLinePointColl[i as number].ratioX =
        (obj.verRightLinePointColl[i as number].x - this.parent.img.destLeft) /
        this.parent.img.destWidth;
      obj.verRightLinePointColl[i as number].ratioY =
        (obj.verRightLinePointColl[i as number].y - this.parent.img.destTop) /
        this.parent.img.destHeight;
    }
    if (parent.upperCanvas.style.cursor !== 'move') {
      const object: Object = { rotationCirclePoint: null };
      parent.notify('selection', {
        prop: 'getTransRotationPoint',
        value: { obj: obj, object: object },
      });
      const rotationCirclePoint: Point = object['rotationCirclePoint'];
      if (rotationCirclePoint) {
        obj.rotationCirclePointColl = {
          x:
            cosAngle * (rotationCirclePoint.x - center.x) -
            sinAngle * (rotationCirclePoint.y - center.y) +
            center.x,
          y:
            sinAngle * (rotationCirclePoint.x - center.x) +
            cosAngle * (rotationCirclePoint.y - center.y) +
            center.y,
        };
        obj.rotationCirclePointColl.ratioX =
          (obj.rotationCirclePointColl.x - parent.img.destLeft) /
          parent.img.destWidth;
        obj.rotationCirclePointColl.ratioY =
          (obj.rotationCirclePointColl.y - parent.img.destTop) /
          parent.img.destHeight;
      }
    }
  }
  ...
  private reArrangeObjColl(
    ...
    switch (value) {
      case 'sendtoback':
        for (
          let i: number = 0, len: number = parent.shapeColl.length;
          i < len;
          i++
        ) {
  ...
  private reorderRedact(
    tempObjColl: SelectionPoint[] | any
  ): SelectionPoint[] | any {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const nonRedact: any = tempObjColl.filter(
      (item: SelectionPoint) => item.shape !== 'redact'
    );
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const redact: any = tempObjColl.filter(
      (item: SelectionPoint) => item.shape === 'redact'
    );
  ...
  private updateShapeColl(): void {
    ...
    let tempOrder: number = 1;
    let tempObjColl: SelectionPoint[] = extend(
      [],
      parent.objColl,
      [],
      true
    ...
    if (
      parent.shapeColl.length > 0 &&
      parent.shapeColl.length ===
        parent.objColl.length + parent.pointColl.length
    ) {
      for (let i: number = 0; i < parent.shapeColl.length; i++) {
      ...
      if (isOrdered) {
        for (let i: number = 0; i < parent.shapeColl.length; i++) {
          if (
            parent.shapeColl[i as number].currIndex &&
            parent.shapeColl[i as number].currIndex.indexOf('shape') > -1
          ) {
            for (let j: number = 0; j < tempObjColl.length; j++) {
              if (
                parent.shapeColl[i as number].currIndex ===
                tempObjColl[j as number].currIndex
              ) {
                parent.shapeColl[i as number] = extend(
                  {},
                  tempObjColl[j as number],
                  {},
                  true
                );
            ...
            parent.shapeColl[i as number].id &&
            parent.shapeColl[i as number].id.indexOf('pen') > -1
          ) {
            for (let j: number = 0; j < tempPointColl.length; j++) {
              if (
                parent.shapeColl[i as number].id ===
                tempPointColl[j as number].id
              ) {
                parent.shapeColl[i as number] = extend(
                  [],
                  tempPointColl[j as number],
                  [],
                  true
                );
    ...
    while (tempObjColl.length !== 0 || tempPointColl.length !== 0) {
      ...
      for (let i: number = 0; i < tempObjColl.length; i++) {
        if (
          tempObjColl[i as number].order === order ||
          (!tempObjColl[i as number].order &&
            tempObjColl[i as number].shape &&
      ...
      if (!isBreak) {
        for (let i: number = 0; i < tempPointColl.length; i++) {
          if (tempPointColl[i as number].order === order) {
            parent.shapeColl.push(
              extend([], tempPointColl[i as number], [], true)
            );
  ...
  private drawAnnotations(
    ...
    for (let i: number = 0; i < tempShapeColl.length; i++) {
      ...
      if (
        tempShapeColl[i as number].order ||
        (!tempShapeColl[i as number].order &&
          tempShapeColl[i as number].shape &&
          tempShapeColl[i as number].shape.indexOf('crop-') > -1) ||
        (!tempShapeColl[i as number].order &&
          tempShapeColl[i as number].shape === 'path' &&
          parent.drawingShape === 'path')
      ) {
        if (
          tempShapeColl[i as number].currIndex &&
          tempShapeColl[i as number].currIndex.indexOf('shape') > -1
        ) {
          parent.objColl = [];
          parent.objColl.push(
            extend({}, tempShapeColl[i as number], {}, true) as SelectionPoint
          );
          if (shape === 'iterate') {
            const temp: string = this.lowerContext.filter;
            this.lowerContext.filter = 'none';
            ...
            for (let i: number = 0; i < tempObjColl.length; i++) {
              if (
                JSON.stringify(tempObjColl[i as number]) ===
                JSON.stringify(parent.objColl[0])
              ) {
                tempObjCollIndex = i;
                break;
              }
            ...
            if (tempObjCollIndex > -1) {
              tempObjColl[tempObjCollIndex as number] = extend(
                {},
                parent.objColl[0],
                {},
                true
          ...
          tempShapeColl[i as number].id &&
          tempShapeColl[i as number].id.indexOf('pen') > -1
        ) {
          parent.pointColl = [];
          ...
          parent.pointColl.push(
            extend({}, tempShapeColl[i as number], {}, true)
          );
          parent.notify('freehand-draw', {
            ...
            value: {
              obj: {
                selPointColl: selPointCollObj['selPointColl'][i as number],
              },
            },
          });
          ...
          if (pen === 'iterate') {
            ...
            for (let i: number = 0; i < tempPointColl.length; i++) {
              if (tempPointColl[i as number].id === parent.pointColl[0].id) {
                tempPointColl[i as number] = extend(
                  {},
                  parent.pointColl[0],
                  {},
                  true
            ...
            for (
              let i: number = 0,
                len: number = tempSelPointCollObj['selPointColl'].length;
              i < len;
              i++
            ) {
              if (
                tempSelPointCollObj['selPointColl'][i as number].id ===
                selPointCollObj['selPointColl'][i as number].id
              ) {
                tempSelPointCollObj['selPointColl'][i as number] = extend(
                  {},
                  selPointCollObj['selPointColl'][i as number],
                  {},
                  true
        ...
        (!tempShapeColl[i as number].shape && !isPenId) ||
    ...
    parent.notify('freehand-draw', {
      prop: 'setSelPointColl',
      onPropertyChange: false,
      value: { obj: { selPointColl: tempSelPointCollObj['selPointColl'] } },
    });
