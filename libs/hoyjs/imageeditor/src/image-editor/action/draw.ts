...
import { extend, isNullOrUndefined, Browser, getComponent } from '@hoyui/base';
import { ActivePoint, Dimension } from '@hoyui/inputs';
import {
  ImageEditor,
  Point,
  SelectionPoint,
...
import { Dialog, hideSpinner, showSpinner } from '@hoyui/popups';
export class Draw {
  ...
  private tempStrokeSettings: StrokeSettings = {
    strokeColor: '#fff',
    fillColor: '',
    strokeWidth: null,
    outlineColor: '',
    radius: null,
    outlineWidth: null,
  }; // restore stroke settings on cancel
  private tempTextSettings: TextSettings = {
    ...
    italic: false,
    underline: false,
  }; // restore text settings on cancel
    ...
    switch (args.prop) {
      case 'drawObject':
        this.drawObject(
          args.value['canvas'],
          args.value['obj'],
          args.value['isCropRatio'],
          args.value['points'],
          args.value['isPreventDrag'],
          args.value['saveContext'],
      ...
      case 'updateCurrTransState':
        this.updateCurrTransState(
          args.value['type'],
          args.value['isPreventDestination'],
          args.value['isRotatePan']
        );
        break;
      case 'currTransState':
        this.currTransState(
      ...
      case 'setTransform':
        this.setTransform(
          args.value['context'],
          args.value['value'],
          args.value['isReverse']
        );
        break;
      case 'render-image':
        this.renderImage(
          args.value['isMouseWheel'],
          args.value['isPreventClearRect'],
          args.value['isFrame'],
          args.value['isStraighten']
        );
        break;
      ...
      case 'performCancel':
        this.performCancel(
          args.value['isContextualToolbar'],
          args.value['isUndoRedo'],
          args.value['isFinalCancel']
        );
        break;
      case 'updateFlipPan':
      ...
      case 'setCurrentObj':
        this.setCurrentObj(
          args.value['obj'],
          args.value['isUndoRedo'],
          args.value['isCircleCrop']
        );
        break;
      case 'performPointZoom':
        this.performPointZoom(
          args.value['x'],
          args.value['y'],
          args.value['type'],
          args.value['isResize']
      ...
      case 'getArrowDimension':
        args.value['obj']['arrowDimension'] = extend(
          {},
          this.arrowDimension,
          {},
          true
      ...
      case 'downScaleImgCanvas':
        this.downScaleImgCanvas(
          args.value['ctx'],
          args.value['isImgAnnotation'],
          args.value['isHFlip'],
          args.value['isVFlip']
        );
        break;
      ...
      case 'triggerFrameChange':
        args.value['obj']['frameChangeEventArgs'] = this.triggerFrameChange(
          args.value['prevFrameSettings']
        );
        break;
      case 'setImageApply':
      ...
      case 'isLinesIntersect':
        args.value['obj']['isIntersect'] = this.isLinesIntersect(
          args.value['obj']
        );
        break;
      ...
      case 'setTempDestForStraighten':
        this.tempStraightenDestPoints = extend(
          {},
          this.straightenDestPoints,
          {},
          true
        ) as ImageDimension;
        break;
      ...
      case 'isPointsInsideImg':
        args.value['obj']['bool'] =
          this.checkPointPosition(
            args.value['x'],
            args.value['y'],
            this.imgCanvasPoints[0].x,
            this.imgCanvasPoints[0].y,
            this.imgCanvasPoints[1].x,
            this.imgCanvasPoints[1].y,
            this.imgCanvasPoints[2].x,
            this.imgCanvasPoints[2].y,
            this.imgCanvasPoints[3].x,
            this.imgCanvasPoints[3].y
          ) !== 'inside';
        break;
      ...
      case 'checkPointPosition':
        args.value['obj']['position'] = this.checkPointPosition(
          args.value['obj']['x'],
          args.value['obj']['y'],
          args.value['obj']['x1'],
          args.value['obj']['y1'],
  ...
  private reset(): void {
    ...
    this.tempTextSettings = {
      text: 'Enter Text',
      fontFamily: this.parent.fontFamily.default,
      fontSize: null,
      fontRatio: null,
      bold: false,
      italic: false,
    ...
    this.tempUndoRedoStep =
      this.tempFreehandCounter =
      this.tempCurrFhdIndex =
        0;
  ...
  private redrawDownScale(): void {
    ...
    if (parent.transform.zoomFactor && parent.transform.zoomFactor < 0) {
      const activeObj: SelectionPoint = extend(
        {},
        parent.activeObj,
        {},
        true
  ...
  private updateFinetune(): void {
    ...
    if (parent.transform.zoomFactor && parent.transform.zoomFactor < 0) {
      ...
      const inMemoryContext: CanvasRenderingContext2D =
        parent.inMemoryCanvas.getContext('2d');
  ...
  private drawObject(
    ...
    saveContext?: CanvasRenderingContext2D,
    isPreventSelection?: boolean
  ): void {
    const parent: ImageEditor = this.parent;
    ...
    this.upperContext.clearRect(
      0,
      0,
      parent.upperCanvas.width,
      parent.upperCanvas.height
    );
    ...
    actPoint = parent.activeObj.activePoint;
    if (
      isNullOrUndefined(actPoint.startX) &&
      isNullOrUndefined(actPoint.startY)
    ) {
      return;
    }
    ...
    if (canvas !== 'original') {
      ...
      if (
        isNullOrUndefined(isPreventSelection) &&
        (canvasDraw === this.lowerContext || canvasDraw === this.upperContext)
      ) {
    ...
    const object: Object = { keyHistory: '' };
    parent.notify('shape', {
      prop: 'getKeyHistory',
      onPropertyChange: false,
      value: { obj: object },
  ...
  private rotateContext(type: string, ctx: CanvasRenderingContext2D): void {
    ...
    const rotationAngle: number =
      type === 'initial' ? rotatedAngle : -rotatedAngle;
  ...
  private setDragLimit(): void {
    ...
    if (
      ...
      if (actPoint.startX < destLeft) {
        ...
        actPoint.endX = Math.min(
          actPoint.startX + actPoint.width,
          destLeft + destWidth
        );
  ...
  private drawCropRatio(): void {
    ...
    if (parent.transform.zoomFactor > 0 && this.currSelPoint) {
      ...
      if (
        parent.transform.degree % 90 === 0 &&
        parent.transform.degree % 180 !== 0
      ) {
        width =
          actPoint.width < actPoint.height ? actPoint.width : actPoint.height;
        height = width;
      } else {
    ...
    switch (parent.currObjType.shape.toLowerCase()) {
      ...
      default:
        x = parseInt(
          parent.currObjType.shape.toLowerCase().split('crop-')[1].split(':')[0]
        );
        y = parseInt(
          parent.currObjType.shape.toLowerCase().split('crop-')[1].split(':')[1]
        );
        break;
    }
  ...
  private adjToCenter(): void {
    ...
    const { destLeft, destTop, destWidth, destHeight } = parent.img;
    const diffX: number =
      parent.lowerCanvas.width / 2 - (actPoint.endX - actPoint.width / 2);
    const diffY: number =
      parent.lowerCanvas.height / 2 - (actPoint.endY - actPoint.height / 2);
  ...
  private enlargeToImg(): void {
    ...
    if (
      ...
      let actPoint: ActivePoint = parent.activeObj.activePoint;
      let tempActPoint: ActivePoint = extend(
        {},
        actPoint,
        {},
        true
  ...
  private updateActiveObject(
    ...
    actPoint = actPoint
      ? actPoint
      : (extend({}, parent.activeObj.activePoint, {}, true) as ActivePoint);
    obj = obj
      ? obj
      : (extend({}, parent.activeObj, {}, true) as SelectionPoint);
    actPoint.width = actPoint.endX - actPoint.startX;
    actPoint.height = actPoint.endY - actPoint.startY;
    const { startX, startY, endX, endY, width, height } = actPoint;
    x = x ? x : 0;
    y = y ? y : 0;
    const horCircleWidth: number = width / 2;
    const verCircleHeight: number = height / 2;
    ...
    if (obj.rotatedAngle === 0) {
      ...
      obj.rotationCirclePoint.ratioX =
        (obj.rotationCirclePoint.x - parent.img.destLeft) /
        parent.img.destWidth;
      obj.rotationCirclePoint.ratioY =
        (obj.rotationCirclePoint.y - parent.img.destTop) /
  ...
  private drawOuterSelection(
    canvasDraw: CanvasRenderingContext2D,
    ...
    const tempObj: SelectionPoint = extend(
      {},
      actObj,
      {},
      true
    ...
    if (
      ((splitWords && splitWords[0] === 'crop') ||
        actObj.shape === undefined) &&
      !isCropCircle
    ) {
      this.upperContext.fillStyle = 'rgb(0, 0, 0, 0.25)';
      this.upperContext.fillRect(
    ...
    if (actObj.shape === 'arrow' || actObj.shape === 'line') {
      ...
      if (activeObj.pointColl[0]) {
        ...
        if (activeObj.pointColl.length > 1) {
          for (
            let i: number = 1, len: number = activeObj.pointColl.length;
            i < len;
            i++
          ) {
    ...
    if (
      parent.selectionSettings.showCircle &&
      (splitWords === undefined || splitWords[0] !== 'crop')
    ) {
      ...
      if (actObj.shape === 'text') {
        ...
        canvasDraw.arc(
          actObj.rotationCirclePoint.x,
          actObj.rotationCirclePoint.y,
          actObj.bottomCenterCircle.radius,
          0,
          2 * Math.PI
        );
        canvasDraw.stroke();
  ...
  private drawArrowHead(
    ...
    const headType: string = isStartHead
      ? this.parent.activeObj.start
      : this.parent.activeObj.end;
    switch (headType) {
      case 'arrowSolid':
        // eslint-disable-next-line @typescript-eslint/no-unused-expressions
        isStartHead
          ? this.arrowSolid(canvasDraw, true)
          : this.arrowSolid(canvasDraw, false);
        break;
      case 'arrow':
        // eslint-disable-next-line @typescript-eslint/no-unused-expressions
        isStartHead
          ? this.arrow(canvasDraw, true)
          : this.arrow(canvasDraw, false);
        break;
      case 'circleSolid':
        // eslint-disable-next-line @typescript-eslint/no-unused-expressions
        isStartHead
          ? this.arrowCircleSolid(canvasDraw, true)
          : this.arrowCircleSolid(canvasDraw, false);
        break;
      case 'circle':
        // eslint-disable-next-line @typescript-eslint/no-unused-expressions
        isStartHead
          ? this.arrowCircle(canvasDraw, true)
          : this.arrowCircle(canvasDraw, false);
        break;
      case 'bar':
        // eslint-disable-next-line @typescript-eslint/no-unused-expressions
        isStartHead
          ? this.arrowBar(canvasDraw, true)
          : this.arrowBar(canvasDraw, false);
        break;
      case 'square':
      case 'squareSolid':
        // eslint-disable-next-line @typescript-eslint/no-unused-expressions
        isStartHead
          ? this.arrowSquareStart(canvasDraw)
          : this.arrowSquareEnd(canvasDraw);
        break;
    }
  }
  ...
  private drawShapeObj(
    ...
    const currentShape: string =
      shape !== undefined ? shape : parent.currObjType.shape;
    ...
    this.rotateContext('initial', canvasDraw);
    let degree: number;
    const tempFillStyle: string = canvasDraw.fillStyle as string;
    ...
    switch (parent.currObjType.shape.toLowerCase()) {
      ...
      case 'ellipse':
        ...
        canvasDraw.beginPath();
        canvasDraw.ellipse(
          actPoint.startX + selectionWidth / 2,
      ...
      case 'path':
        ...
        if (activeObj.pointColl.length > 1) {
          ...
          if (obj['shape'] === 'path' && parent.isShapeDrawing) {
            ...
            for (
              let i: number = 0, len: number = activeObj.pointColl.length;
              i < len;
              i++
            ) {
  ...
  private drawCornerCircles(canvasDraw: CanvasRenderingContext2D): void {
    ...
    canvasDraw.beginPath();
    canvasDraw.rect(
      tempObj.activePoint.startX,
      tempObj.activePoint.startY,
      tempObj.activePoint.width,
      tempObj.activePoint.height
    );
    canvasDraw.stroke();
    canvasDraw.closePath();
    if (parent.selectionSettings.showCircle) {
      ...
      canvasDraw.beginPath();
      canvasDraw.moveTo(
        tempObj.topLeftCircle.startX,
        tempObj.topLeftCircle.startY
      );
      canvasDraw.arc(
        tempObj.topLeftCircle.startX,
        tempObj.topLeftCircle.startY,
        tempObj.topLeftCircle.radius,
        0,
        2 * Math.PI
      );
      canvasDraw.moveTo(
        tempObj.topRightCircle.startX,
        tempObj.topRightCircle.startY
      );
      canvasDraw.arc(
        tempObj.topRightCircle.startX,
        tempObj.topRightCircle.startY,
        tempObj.topRightCircle.radius,
        0,
        2 * Math.PI
      );
      canvasDraw.moveTo(
        tempObj.bottomLeftCircle.startX,
        tempObj.bottomLeftCircle.startY
      );
      canvasDraw.arc(
        tempObj.bottomLeftCircle.startX,
        tempObj.bottomLeftCircle.startY,
        tempObj.bottomLeftCircle.radius,
        0,
        2 * Math.PI
      );
      canvasDraw.moveTo(
        tempObj.bottomRightCircle.startX,
        tempObj.bottomRightCircle.startY
      );
      canvasDraw.arc(
        tempObj.bottomRightCircle.startX,
        tempObj.bottomRightCircle.startY,
        tempObj.bottomRightCircle.radius,
        0,
        2 * Math.PI
      );
      canvasDraw.stroke();
  ...
  private drawRotationArcLine(canvasDraw: CanvasRenderingContext2D): void {
    ...
    let degree: number;
    let isHorizontalflip: boolean = false;
    let isVerticalflip: boolean = false;
    if (actObj.shapeDegree === 0) {
      degree = parent.transform.degree;
    } else {
      degree = parent.transform.degree - actObj.shapeDegree;
    ...
    if (actObj.flipObjColl) {
      for (
        let i: number = 0, len: number = actObj.flipObjColl.length;
        i < len;
        i++
      ) {
    ...
    switch (degree) {
      ...
      case -270:
        if (isHorizontalflip) {
          actObj.rotationCirclePoint = {
            x: actObj.centerRightCircle.startX + actObj.rotationCircleLine,
            y: actObj.centerLeftCircle.startY,
          };
          canvasDraw.moveTo(
            actObj.rotationCirclePoint.x - actObj.rotationCircleLine,
            actObj.rotationCirclePoint.y
          );
        } else {
          actObj.rotationCirclePoint = {
            x: actObj.centerLeftCircle.startX - actObj.rotationCircleLine,
            y: actObj.centerLeftCircle.startY,
          };
          canvasDraw.moveTo(
            actObj.rotationCirclePoint.x + actObj.rotationCircleLine,
  ...
  private drawSquareLines(canvasDraw: CanvasRenderingContext2D): void {
    ...
    const { fillColor, strokeColor, strokeWidth, radius } =
      actObj.strokeSettings;
    if (actObj.shape) {
      splitWords = actObj.shape.split('-');
    }
    if (splitWords[0] === 'crop') {
    ...
    parent.notify('crop', {
      ...
      value: {
        obj: obj,
        dimension: {
          width: canvasDraw.canvas.width,
          height: canvasDraw.canvas.height,
        },
      },
    });
    ratio = obj as Dimension;
    const isTempCanvas: boolean =
      canvasDraw.canvas.id === parent.element.id + '_tempCanvas';
    const zoomFactor: number = parent.transform.zoomFactor;
    const baseRadius: number = isTempCanvas
      ? radius * 10 * ((ratio.width + ratio.height) / 2)
      : radius * 10;
    const adjustedRadius: number = baseRadius + baseRadius * zoomFactor;
    if (radius !== null) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (canvasDraw as any).roundRect(
        startX,
        startY,
        width,
        height,
  ...
  private drawSelection(horLineWidth: number, verLineHeight: number): void {
    ...
    this.upperContext.strokeStyle =
      parent.themeColl[parent.theme]['primaryColor'];
    this.upperContext.beginPath();
    actObj.horTopInnerLine = {
      startX: startX,
      startY: startY + verLineHeight,
      endX: endX,
      endY: endY + verLineHeight,
    };
    actObj.horBottomInnerLine = {
      startX: startX,
      startY: startY + 2 * verLineHeight,
      endX: endX,
      endY: endY + 2 * verLineHeight,
    };
    ...
    this.upperContext.moveTo(
      actObj.horTopInnerLine.startX,
      actObj.horTopInnerLine.startY
    );
    this.upperContext.lineTo(
      actObj.horTopInnerLine.endX,
      actObj.horTopInnerLine.startY
    );
    this.upperContext.moveTo(
      actObj.horBottomInnerLine.startX,
      actObj.horBottomInnerLine.startY
    );
    this.upperContext.lineTo(
      actObj.horBottomInnerLine.endX,
      actObj.horBottomInnerLine.startY
    );
    this.upperContext.moveTo(
      actObj.verLeftInnerLine.startX,
      actObj.verLeftInnerLine.startY
    );
    this.upperContext.lineTo(
      actObj.verLeftInnerLine.endX,
      actObj.verLeftInnerLine.endY
    );
    this.upperContext.moveTo(
      actObj.verRightInnerLine.startX,
      actObj.verRightInnerLine.startY
    );
    this.upperContext.lineTo(
      actObj.verRightInnerLine.endX,
      actObj.verRightInnerLine.endY
    );
  ...
  private shapeCircle(
    ...
    canvasDraw.beginPath();
    canvasDraw.ellipse(
      parent.activeObj.horTopLine.startX + selectionWidth / 2,
      parent.activeObj.horTopLine.startY + selectionHeight / 2,
      selectionWidth / 2,
      selectionHeight / 2,
      0,
      0,
  ...
  private arrow(canvasDraw: CanvasRenderingContext2D, start: boolean): void {
    ...
    const isEndCircleOrSquare: boolean =
      actObj.end === 'circle' || actObj.end === 'square';
    if (
      (((start && actObj.triangleDirection === 'left') ||
        actObj.triangleDirection === 'right') &&
        ((isStartArrow && actObj.end === 'none') ||
          (isStartArrow && !isEndCircleOrSquare))) ||
      (!start &&
        ((isEndArrow && actObj.start === 'none') ||
          (!isStartArrow && !isEndCircleOrSquare)))
    ) {
  ...
  private arrowSolid(
    ...
    const isEndCircleOrSquare: boolean =
      actObj.end === 'circle' || actObj.end === 'square';
    if (
      (start && isStartArrowSolid && actObj.end === 'none') ||
      (isStartArrowSolid && !isEndCircleOrSquare) ||
      (!start &&
        ((isEndArrowSolid && actObj.start === 'none') ||
          (!isStartArrowSolid && !isEndCircleOrSquare)))
    ) {
      this.shapeLine(canvasDraw, startX, startY, endX, endY);
    }
  ...
  private arrowSquareStart(canvasDraw: CanvasRenderingContext2D): void {
    ...
    if (
      (isStartSquare && actObj.end === 'none') ||
      (isStartSquare && !isEndCircle && actObj.start !== 'square') ||
    ...
    if (actObj.triangleDirection === 'left') {
      ...
      if (
        ...
        this.shapeLine(
          canvasDraw,
          this.endCircleIntersectX1,
          this.endCircleIntersectY1,
          this.squareStartIntersectX1,
          this.squareStartIntersectY1
        );
      } else if (
        actObj.start === 'squareSolid' &&
        actObj.end === 'squareSolid'
      ) {
        this.shapeLine(canvasDraw, startX, startY, endX, endY);
      }
    ...
    if (actObj.triangleDirection === 'right') {
      ...
      if (
        ...
        this.shapeLine(
          ...
          this.squareEndIntersectX1,
          this.squareEndIntersectY1,
          this.startCircleIntersectX1,
          this.startCircleIntersectY1
        );
  ...
  private arrowCircle(
    ...
    if (
      ...
      const point: Point = this.manipulateSaveCtx(
        canvasDraw,
        circleRadius,
        null
      );
      ...
      const b: number =
        2 * (this.dx * (startX - endX) + this.dy * (startY - endY));
      const c: number =
      ...
      const a: number = this.dx * this.dx + this.dy * this.dy;
      const b: number =
        2 * (this.dx * (endX - startX) + this.dy * (endY - startY));
      const c: number =
  ...
  private arrowCircleSolid(
    ...
    const actObj: SelectionPoint = parent.activeObj;
    const { startX, startY, endX, endY } = actObj.activePoint;
    const isStartCircleSolid: boolean = actObj.start === 'circleSolid';
    const strWidth: number = actObj.strokeSettings.strokeWidth;
    if (
      (start && actObj.triangleDirection === 'left') ||
      (!start && actObj.triangleDirection === 'right')
      ...
      canvasDraw.fillStyle = actObj.strokeSettings.strokeColor;
      if (
        (start && isStartCircleSolid && actObj.end === 'none') ||
        (isStartCircleSolid &&
          actObj.end !== 'circle' &&
          actObj.end !== 'square') ||
  ...
  private arrowBar(canvasDraw: CanvasRenderingContext2D, start: boolean): void {
    ...
    if (
      ...
      canvasDraw.fillStyle = actObj.strokeSettings.strokeColor;
      if (
        (start && actObj.start === 'bar' && actObj.end === 'none') ||
        (actObj.start === 'bar' &&
          actObj.end !== 'circle' &&
          actObj.end !== 'square') ||
        (!start &&
          ((actObj.end === 'bar' && actObj.start === 'none') ||
            (actObj.end === 'bar' &&
              actObj.start !== 'circle' &&
              actObj.start !== 'square')))
      ) {
  ...
  private shapeImage(canvasDraw: CanvasRenderingContext2D): void {
    ...
    if (canvasDraw === this.lowerContext && this.isImageApply) {
      ...
      parent.notify('transform', {
        ...
        onPropertyChange: false,
        value: {
          width: actObj.imageElement.width,
          height: actObj.imageElement.height,
          obj: dimObj,
          isImgShape: null,
        },
      });
      if (width < dimObj['width'] / 5 || height < dimObj['height'] / 5) {
        ctx.clearRect(
          0,
          0,
          actObj.imageCanvas.width,
          actObj.imageCanvas.height
        );
        parent.notify('selection', {
          prop: 'applyTransformToImg',
          onPropertyChange: false,
          value: { ctx: ctx },
        });
        parent.notify('draw', {
  ...
  private shapeText(canvasDraw: CanvasRenderingContext2D): void {
    ...
    const lineHeight: number =
      (lHeight * rows.length - fontSize * rows.length) / rows.length;
    ...
    if (actObj.strokeSettings.fillColor !== '') {
      ...
      canvasDraw.fillRect(
        actObj.activePoint.startX,
        actObj.activePoint.startY,
        actObj.activePoint.width,
    ...
    for (let i: number = 0; i < rows.length; i++) {
      ...
      const isTempCanvas: boolean =
        canvasDraw.canvas.id === parent.element.id + '_tempCanvas';
      const baseWidth: number = Math.max(
        1,
        actObj.strokeSettings.outlineWidth / 2
      );
      if (
        /^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})$|^[a-zA-Z]+$/.test(
          actObj.strokeSettings.outlineColor
        )
      ) {
        canvasDraw.lineWidth =
          baseWidth *
          ((isTempCanvas
            ? Math.floor((fontSize - 1) / 60)
            : Math.floor((fontSize - 1) / 16)) *
            0.5 +
            0.5);
        if (isTempCanvas) {
  ...
  private updateActPoint(
    ...
    if (degree.toLowerCase() === 'horizontal') {
      if (actPoint.startX <= canvasDraw.canvas.width / 2) {
        actPoint.startX =
          canvasDraw.canvas.width / 2 +
          (canvasDraw.canvas.width / 2 - actPoint.endX);
        actPoint.endX = actPoint.startX + actPoint.width;
      ...
      if (actPoint.startY <= canvasDraw.canvas.height / 2) {
        actPoint.startY =
          canvasDraw.canvas.height / 2 +
          (canvasDraw.canvas.height / 2 - actPoint.endY);
        actPoint.endY = actPoint.startY + actPoint.height;
        this.updateActiveObject(actPoint, actObj);
        parent.activeObj = actObj;
  ...
  private rotateImage(canvasDraw: CanvasRenderingContext2D): void {
    ...
    let degree: number;
    let actObj: SelectionPoint = parent.activeObj;
    const tempActiveObj: SelectionPoint = extend(
      {},
      parent.activeObj,
      null,
      true
    ...
    /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
    let tempDegree: number;
    const tempColl: any = [];
    canvasDraw.save();
    for (
      let i: number = 0, len: number = actObj.rotateFlipColl.length;
      i < len;
      i++
    ) {
      /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
      ...
      if (typeof coll === 'number') {
        ...
        imgPoint.width =
          tempDegree % 90 === 0 && tempDegree % 180 !== 0
            ? actObj.activePoint.height
            : actObj.activePoint.width;
        imgPoint.height =
          tempDegree % 90 === 0 && tempDegree % 180 !== 0
        ...
        if (
          (tempDegree % 90 === 0 && tempDegree % 270 !== 0) ||
          tempDegree === 0
        ) {
          startY =
            canvasDraw.canvas.width -
            (actObj.activePoint.startX + actObj.activePoint.width);
          startY += (actObj.activePoint.width - imgPoint.height) / 2;
          startX = imgPoint.startY;
        } else if (tempDegree % 270 === 0) {
          startX =
            canvasDraw.canvas.height -
            (actObj.activePoint.startY + actObj.activePoint.height);
    ...
    if (actObj.rotatedAngle !== 0) {
      parent.notify('shape', {
        prop: 'setPointCollForShapeRotation',
        onPropertyChange: false,
        value: { obj: actObj },
      });
    }
  ...
  private textFlipDegree(
    ...
    const lineHeight: number =
      (fontSize * rows.length - fontSize * rows.length) / rows.length;
    ...
    for (let i: number = 0, len: number = rows.length; i < len; i++) {
      ...
      canvasDraw.strokeText(
        text,
        startX + fontSize * 0.15,
        startY + yPoint + (i > 0 ? fontSize * 0.25 : -fontSize * 0.35)
      );
      canvasDraw.fillText(
        text,
        startX + fontSize * 0.15,
  ...
  private clearOuterCanvas(context: CanvasRenderingContext2D): void {
    ...
    context.clearRect(
      destLeft + destWidth,
      0,
      parent.lowerCanvas.width - (destLeft + destWidth),
      parent.lowerCanvas.height
    );
    ...
    context.clearRect(
      ...
      destTop + destHeight,
      parent.lowerCanvas.width,
      parent.lowerCanvas.height - (destTop + destHeight)
    );
  ...
  private setDestPoints(): void {
    ...
    if (degree % 90 === 0 && degree % 180 !== 0) {
      ...
      parent.notify('transform', {
        ...
        onPropertyChange: false,
        value: {
          width: parent.img.srcHeight,
          height: parent.img.srcWidth,
          obj: obj,
          isImgShape: null,
        },
      });
      ...
      parent.img.destLeft =
        (parent.lowerCanvas.clientWidth - maxDimension.height) / 2;
      parent.img.destTop =
        (parent.lowerCanvas.clientHeight - maxDimension.width) / 2;
      parent.img.destWidth = maxDimension.height;
      parent.img.destHeight = maxDimension.width;
    } else {
      const obj: Object = { width: 0, height: 0 };
      ...
      parent.img.destLeft =
        (parent.lowerCanvas.clientWidth - maxDimension.width) / 2;
      if (degree === 0) {
        parent.img.destTop =
          (parent.lowerCanvas.clientHeight - maxDimension.height + 1) / 2;
      } else {
        parent.img.destTop =
          (parent.lowerCanvas.clientHeight - maxDimension.height) / 2;
      }
      parent.img.destWidth = maxDimension.width;
      parent.img.destHeight = maxDimension.height;
    }
  ...
  private updateCurrTransState(
    type: string,
    isPreventDestination?: boolean,
    isRotatePan?: boolean,
    isStraighten?: boolean
  ): void {
    const parent: ImageEditor = this.parent;
  ...
  private currTransState(
    type: string,
    isPreventDimension?: boolean,
    context?: CanvasRenderingContext2D,
    isPreventCircleCrop?: boolean
  ): void {
    const parent: ImageEditor = this.parent;
  ...
  private setTransformColl(
    ...
    if (type === 'initial') {
      for (
        let i: number = 0, len: number = parent.rotateFlipColl.length;
        i < len;
        i++
      ) {
  ...
  private setTransform(
    ...
    parent.notify('transform', {
      prop: 'setReverseFlip',
      onPropertyChange: false,
      value: { isReverseFlip: false },
    });
  }

  private drawImgToCanvas(maxDimension: Dimension): void {
    const parent: ImageEditor = this.parent;
    this.lowerContext.clearRect(
      0,
      0,
      parent.lowerCanvas.width,
      parent.lowerCanvas.height
  ...
  private renderImage(
    ...
    parent.notify('transform', {
      prop: 'setDestPointsForFlipState',
      onPropertyChange: false,
    });
    this.drawImage();
    parent.notify('transform', {
    ...
    parent.baseImg.onload = () => {
      ...
      if (!parent.isUndoRedo) {
        parent.notify('filter', {
          prop: 'update-finetunes',
          onPropertyChange: false,
        });
      }
      proxy.lowerContext.drawImage(
        parent.baseImg,
        0,
        0,
        proxy.parent.lowerCanvas.width,
        proxy.parent.lowerCanvas.height
      );
      let isCropped: boolean = false;
      let isSameDimension: boolean = false;
      if (parent.isImageUpdated) {
        ...
        isSameDimension =
          parent.baseImg.width === width && parent.baseImg.height === height;
      }
      hideSpinner(parent.element);
      parent.element.style.opacity = '1';
      proxy.updateBaseImgCanvas();
      const fileOpened: OpenEventArgs = {
      ...
      if (Browser.isDevice) {
        parent.notify('toolbar', {
          prop: 'destroy-top-toolbar',
          onPropertyChange: false,
        });
        parent.notify('toolbar', {
          prop: 'destroy-bottom-toolbar',
          onPropertyChange: false,
        });
        ...
        parent.notify('toolbar', {
          prop: 'init-main-toolbar',
          onPropertyChange: false,
          value: eventargs,
        });
        parent.notify('toolbar', {
          prop: 'create-bottom-toolbar',
          onPropertyChange: false,
        });
      } else {
        parent.notify('toolbar', {
      ...
      if (parent.isImageLoaded && parent.element.style.opacity !== '0.5') {
        ...
        const action: EditCompleteEventArgs = {
          action: 'file-open',
          actionEventArgs: fileOpened,
        };
  ...
  private updateCanvas(isCropped?: boolean, isSameDimension?: boolean): void {
    ...
    if (parent.zoomSettings.zoomFactor !== 1 || parent.zoomSettings.zoomPoint) {
      parent.zoom(
        parent.zoomSettings.zoomFactor,
        parent.zoomSettings.zoomPoint
      );
    }
  ...
  private resetFrameZoom(isOk: boolean): void {
    ...
    if (!isNullOrUndefined(parent.tempFrameZoomLevel)) {
      ...
      if (isOk && obj) {
        obj.previousObj.frameObj = extend(
          {},
          parent.frameObj,
          null,
          true
        ) as FrameValue;
        obj.currentObj.frameObj = extend(
          {},
          parent.frameObj,
          null,
          true
  ...
  private performCancel(
    isContextualToolbar?: boolean,
    isUndoRedo?: boolean,
    isFinalCancel?: boolean
  ): void {
    const parent: ImageEditor = this.parent;
    if (isFinalCancel) {
    ...
    parent.notify('selection', {
      prop: 'getFreehandDrawEditing',
      onPropertyChange: false,
      value: { obj: obj },
    });
    if (
      isNullOrUndefined(isUndoRedo) &&
      JSON.stringify(parent.frameObj) !== JSON.stringify(parent.tempFrameObj)
    ) {
    ...
    if (obj['bool']) {
      ...
      parent.notify('shape', {
        prop: 'applyActObj',
        onPropertyChange: false,
        value: { isMouseDown: true },
      });
      parent.notify('toolbar', {
        prop: 'refresh-main-toolbar',
        onPropertyChange: false,
      });
      parent.notify('selection', {
        prop: 'setTempActObj',
      ...
      if (parent.drawingShape) {
        parent.enableShapeDrawing(
          parent.toPascalCase(parent.drawingShape) as ShapeType,
          true
        );
      }
    } else if (
      (!parent.activeObj.shape || parent.activeObj.shape !== 'redact') &&
      (((!Browser.isDevice || (Browser.isDevice && !straightenObj['bool'])) &&
        document.querySelector('#' + parent.element.id + '_sliderWrapper')) ||
        parent.currObjType.isFiltered)
    ) {
      editCompleteArgs['action'] = parent.isFinetuneBtnClick
        ? 'fine-tune'
        : 'filter';
      this.lowerContext.filter = this.tempAdjValue;
      parent.canvasFilter = this.tempAdjValue;
      parent.notify('filter', {
        prop: 'setAdjustmentValue',
        onPropertyChange: false,
        value: { adjustmentValue: this.tempAdjValue },
      });
      parent.initialAdjustmentValue = this.tempAdjValue;
      if (
        this.lowerContext.filter.split(' ').length > 1 &&
        this.lowerContext.filter.split(' ')[0].split('(')[1].split(')')[0] ===
          '1'
      ) {
        parent.notify('filter', {
          prop: 'setBrightnessAdjusted',
          onPropertyChange: false,
          value: { isBrightnessAdjusted: false },
        });
      }
      ...
      parent.notify('filter', {
        prop: 'setBevelFilter',
        onPropertyChange: false,
        value: { bevelFilter: this.lowerContext.filter },
      });
      this.lowerContext.clearRect(
        0,
        0,
      ...
      parent.notify('filter', {
        ...
        onPropertyChange: false,
        value: {
          adjustmentLevel: extend({}, obj['tempAdjustmentLevel'], {}, true),
        },
      });
      parent.notify('undo-redo', {
        prop: 'setUndoRedoStep',
        onPropertyChange: false,
        value: { step: this.tempUndoRedoStep },
      });
      ...
      parent.element
        .querySelector('.e-contextual-toolbar-wrapper')
        .classList.add('e-hide');
      parent.notify('toolbar', {
        prop: 'refresh-toolbar',
        onPropertyChange: false,
        value: eventargs,
      });
    ...
    parent.notify('toolbar', {
      prop: 'refresh-dropdown-btn',
      value: { isDisabled: false },
    });
    parent.notify('toolbar', {
      prop: 'setCurrentToolbar',
      value: { type: 'main' },
    });
    if (isFinalCancel) {
      parent.noPushUndo = false;
    ...
    const actionArgs: EditCompleteEventArgs = {
      action: 'cancel',
      actionEventArgs: editCompleteArgs as object,
    };
  ...
  private cancelItems(editCompleteArgs?: object): void {
    ...
    const ascpectIcon: HTMLInputElement = parent.element.querySelector(
      '#' + id + '_aspectratio'
    ) as HTMLInputElement;
    const nonAspectIcon: HTMLInputElement = parent.element.querySelector(
      '#' + id + '_nonaspectratio'
    ) as HTMLInputElement;
    let splitWords: string[];
    const shapes: string[] = [
    ...
    if (parent.isResize) {
      ...
      if (
        ...
        parent.notify('selection', {
          prop: 'getNumTextValue',
          onPropertyChange: false,
          value: { obj: obj },
        });
        const point: Point = { x: obj['width'], y: obj['height'] };
        const aspectRatioElement: HTMLInputElement =
          parent.element.querySelector(
            '#' + parent.element.id + '_aspectratio'
          ) as HTMLInputElement;
        const blrAspRatElem: HTMLInputElement = parent.element.querySelector(
          '.e-ie-toolbar-aspect-ratio-btn'
        ) as HTMLInputElement;
        if (
          point &&
          point.x &&
          point.y &&
          !isNullOrUndefined(parent.aspectWidth)
        ) {
          if (
            aspectRatioElement ||
            (blrAspRatElem && !blrAspRatElem.classList.contains('e-hidden'))
          ) {
            parent.notify('transform', {
              prop: 'resizeImage',
              value: { width: parent.aspectWidth, height: parent.aspectHeight },
            });
          } else {
        ...
        if (obj1['prevCropObj'] && obj2['prevObj']) {
          ...
          parent.notify('transform', {
            ...
            onPropertyChange: false,
            value: {
              zoomFactor: -parent.transform.zoomFactor,
              zoomPoint: null,
              isResize: true,
            },
          });
          parent.notify('transform', {
            ...
            onPropertyChange: false,
            value: {
              zoomFactor: obj2['prevObj']['defaultZoom'],
              zoomPoint: null,
              isResize: true,
            },
          });
          ...
          if (obj2['prevObj'].zoomFactor) {
            parent.setProperties(
              { zoomSettings: { zoomFactor: obj2['prevObj'].zoomFactor } },
              true
            );
          }
          parent.notify('transform', {
        ...
        parent.notify('transform', {
          prop: 'setResizedImgAngle',
          onPropertyChange: false,
          value: { angle: null },
        });
        const temp: boolean = parent.isCropTab;
        parent.isCropTab = false;
        this.updateCropSelObj();
        parent.cancelCropSelection = null;
  ...
  private cancelPen(): void {
    ...
    parent.notify('freehand-draw', {
      ...
      value: { value: this.tempCurrFhdIndex },
    });
    parent.activeObj.strokeSettings = this.tempStrokeSettings;
    parent.notify('shape', {
    ...
    parent.notify('freehand-draw', {
      prop: 'setPenStrokeWidth',
      onPropertyChange: false,
      value: { value: this.tempStrokeWidth },
    });
    parent.notify('undo-redo', {
      prop: 'updateCurrUrc',
      value: { type: 'cancel' },
    ...
    parent.notify('selection', {
      ...
      value: { isFreehandDrawCustomized: false },
    });
  ...
  private cancelText(): void {
    ...
    parent.notify('shape', {
      ...
      onPropertyChange: false,
      value: {
        textSettings: this.tempTextSettings,
        fontFamily: null,
        fontSize: null,
      },
    });
    parent.notify('shape', {
      prop: 'setStrokeSettings',
      value: {
        strokeSettings: this.tempStrokeSettings,
        strokeColor: null,
        fillColor: null,
        strokeWidth: null,
        radius: null,
    ...
    if (isNullOrUndefined(parent.activeObj.currIndex)) {
      ...
      if (
        this.prevActObj &&
        appliedColl &&
        appliedColl.currentObjColl.length &&
        appliedColl.currentObjColl[appliedColl.currentObjColl.length - 1]
  ...
  private cancelShape(): void {
    ...
    if (isNullOrUndefined(parent.activeObj.currIndex)) {
      ...
      for (
        let i: number = 0, iLen: number = object['appliedUndoRedoColl'].length;
        i < iLen;
        i++
      ) {
        const currObjColl: SelectionPoint[] =
          object['appliedUndoRedoColl'][i as number].currentObjColl;
        for (
          let j: number = 0, jLen: number = currObjColl.length;
          j < jLen;
          j++
        ) {
          if (
      ...
      parent.notify('undo-redo', {
        prop: 'getUndoRedoStep',
        value: { obj: undoRedoObj },
      });
      if (
        object['appliedUndoRedoColl'][
          (undoRedoObj['undoRedoStep'] - 1) as number
        ]
      ) {
        parent.objColl = extend(
          [],
          object['appliedUndoRedoColl'][
            (undoRedoObj['undoRedoStep'] - 1) as number
          ].currentObjColl,
  ...
  private cancelSelection(): void {
    ...
    if (parent.cancelCropSelection) {
      ...
      parent.notify('draw', {
        prop: 'render-image',
        value: { isMouseWheel: false },
      });
      parent.notify('draw', {
        prop: 'setStraightenActObj',
        value: { activeObj: parent.activeObj },
      });
      ...
      if (
        ...
        JSON.stringify(this.tempStraightenDestPoints) !==
          JSON.stringify(this.straightenDestPoints)
      ) {
        this.straightenDestPoints = extend(
          {},
          this.tempStraightenDestPoints,
          {},
          true
  ...
  private updateCropSelObj(): void {
    ...
    if (parent.cancelCropSelection) {
      parent.cropObj = extend(
        {},
        parent.cancelCropSelection.previousCropObj,
        {},
        true
      ) as CurrentObject;
      parent.afterCropActions =
        parent.cancelCropSelection.previousObj.afterCropActions;
      parent.notify('undo-redo', {
        prop: 'undoDefault',
        onPropertyChange: false,
        value: { obj: parent.cancelCropSelection },
      });
      parent.currSelectionPoint = extend(
        {},
        parent.cancelCropSelection.previousCropObj.activeObj,
        true
      ) as SelectionPoint;
      if (
        parent.currSelectionPoint &&
        isNullOrUndefined(parent.currSelectionPoint.shape)
      ) {
        parent.currSelectionPoint = null;
      }
      this.clearOuterCanvas(this.lowerContext);
  ...
  private updateCropSelection(): void {
    ...
    currentObj.afterCropActions = extend(
      [],
      parent.afterCropActions,
      [],
      true
    ...
    currentObj.selPointColl = extend(
      [],
      selPointCollObj['selPointColl'],
      [],
      true
    ) as Point[];
  ...
  private select(
    ...
    if (!parent.disabled && parent.isImageLoaded) {
      ...
      previousObj.selPointColl = extend(
        [],
        selPointCollObj['selPointColl'],
        [],
        true
      ...
      parent.notify('crop', {
        prop: 'setPreviousCropCurrentObj',
        onPropertyChange: false,
        value: { obj: previousObj },
      });
      if (
        parent.transform.zoomFactor > 0 &&
        parent.activeObj.shape &&
        parent.activeObj.shape.split('-')[0] === 'crop' &&
        isNullOrUndefined(this.currSelPoint)
      ) {
        this.currSelPoint = extend(
          {},
          parent.activeObj,
          {},
          true
      ...
      if (
        ...
        (parent.transform.degree !== 0 &&
          parent.panPoint.totalPannedInternalPoint.x !== 0 &&
          parent.panPoint.totalPannedInternalPoint.y !== 0 &&
          !isPrevent)
      ) {
        ...
        if (!this.isResizeSelect) {
          parent.notify('crop', {
            prop: 'setCurrSelPoints',
            onPropertyChange: false,
            value: { isSetDimension: true },
          });
        }
  ...
  private drawNewSelection(
    ...
    const cropShape: string = 'crop-' + type.toLowerCase();
    if (cropShape === 'crop-custom') {
      if (
        parent.currObjType.shape === '' ||
        parent.currObjType.shape === 'crop-custom'
      ) {
  ...
  private updateSelectionInsert(points?: ActivePoint): void {
    ...
    parent.notify('selection', {
      prop: 'updatePrevShapeSettings',
      onPropertyChange: false,
      value: { obj: obj },
    });
    const selectionSettings: CropSelectionSettings = {
    ...
    const selectionChangingArgs: SelectionChangeEventArgs = {
      action: 'insert',
    ...
    parent.notify('shape', {
      ...
      onPropertyChange: false,
      value: {
        selectionSettings: selectionChangingArgs.currentSelectionSettings,
      },
    });
  ...
  private drawCustomSelection(
    ...
    if (
      !isNullOrUndefined(startX) &&
      !isNullOrUndefined(startY) &&
      ...
      if (
        isNullOrUndefined(parent.transform.zoomFactor) ||
        parent.transform.zoomFactor === 0
      ) {
        const { destLeft, destTop, destWidth, destHeight } = parent.img;
        const lowerCanvasWidth: number = parent.lowerCanvas.width;
        const lowerCanvasHeight: number = parent.lowerCanvas.height;
        ...
        activePoint.endX = Math.min(
          destLeft + destWidth + 15 < lowerCanvasWidth
            ? destLeft + destWidth - 15
            : lowerCanvasWidth - 15,
          destLeft + destWidth
        );
        activePoint.endY = Math.min(
          destTop + destHeight + 15 < lowerCanvasHeight
            ? destTop + destHeight - 15
            : lowerCanvasHeight - 15,
          destTop + destHeight
        );
      }
      const { destLeft, destTop, destWidth, destHeight } = parent.img;
      ...
      if (
        activePoint.startX === destLeft &&
        destLeft + destWidth > lowerCanvasWidth
      ) {
        activePoint.endX = lowerCanvasWidth - 15;
      }
      if (
        activePoint.startY === destTop &&
        destTop + destHeight > lowerCanvasHeight
      ) {
        activePoint.endY = lowerCanvasHeight - 15;
      }
      if (
  ...
  private adjActObj(): void {
    ...
    while (true) {
      ...
      const object: Object = { isIntersect: null, arr: null };
      parent.notify('draw', {
        prop: 'updateImgCanvasPoints',
        onPropertyChange: false,
      });
      parent.notify('draw', {
        prop: 'isLinesIntersect',
        onPropertyChange: false,
        value: { obj: object },
      });
      if (
  ...
  private callUpdateCurrTransState(): void {
    const parent: ImageEditor = this.parent;
    const tempObjColl: SelectionPoint[] = extend(
      [],
      parent.objColl,
      [],
      true
    ...
    parent.notify('freehand-draw', {
      prop: 'setPenStrokeWidth',
      onPropertyChange: false,
      value: { value: widthObj['penStrokeWidth'] },
    });
    parent.img.destLeft += parent.panPoint.totalPannedInternalPoint.x;
    parent.img.destTop += parent.panPoint.totalPannedInternalPoint.y;
    parent.img.destLeft -= parent.panPoint.totalPannedInternalPoint.x;
  ...
  private setClientTransDim(isPreventDimension?: boolean): void {
    ...
    if (
      ...
      parent.img.destLeft =
        (parent.lowerCanvas.clientWidth - parent.img.destHeight) / 2;
      parent.img.destTop =
        (parent.lowerCanvas.clientHeight - parent.img.destWidth + 1) / 2;
      const temp: number = parent.img.destWidth;
      parent.img.destWidth = parent.img.destHeight;
      parent.img.destHeight = temp;
    } else {
      if (isNullOrUndefined(isPreventDimension)) {
        parent.img.destLeft =
          (parent.lowerCanvas.clientWidth - parent.img.destWidth) / 2;
        parent.img.destTop =
          (parent.lowerCanvas.clientHeight - parent.img.destHeight + 1) / 2;
      }
    }
  }
  ...
  private redrawImgWithObj(): void {
    ...
    if (parent.rotateFlipColl.length !== 0) {
      const totalPannedInternalPoint: Point = extend(
        {},
        parent.panPoint.totalPannedInternalPoint,
        {},
        true
      ) as Point;
      const destPoints: ActivePoint = {
        startX: parent.img.destLeft,
        startY: parent.img.destTop,
        width: parent.img.destWidth,
        height: parent.img.destHeight,
  ...
  private setCurrentObj(
    ...
    parent.setProperties(
      { zoomSettings: { zoomFactor: obj.zoomFactor } },
      true
    );
    parent.notify('transform', {
      prop: 'setPreviousZoomValue',
      onPropertyChange: false,
      value: { previousZoomValue: obj.previousZoomValue },
    });
    parent.panPoint.totalPannedPoint = extend(
      {},
      obj.totalPannedPoint,
      {},
      true
    ) as Point;
    parent.panPoint.totalPannedClientPoint = extend(
      {},
      obj.totalPannedClientPoint,
      {},
      true
    ) as Point;
    parent.panPoint.totalPannedInternalPoint = extend(
      {},
      obj.totalPannedInternalPoint,
      {},
      true
    ) as Point;
    const point: Point = extend({}, obj.tempFlipPanPoint, {}, true) as Point;
    parent.notify('crop', {
      prop: 'setTempFlipPanPoint',
      onPropertyChange: false,
      value: { point: point },
    });
    parent.rotateFlipColl = extend([], obj.rotateFlipColl, [], true) as
      | string[]
      | number[];
    ...
    parent.notify('filter', {
      prop: 'setAdjustmentLevel',
      onPropertyChange: false,
      value: { adjustmentLevel: obj.adjustmentLevel },
    });
    ...
    parent.notify('filter', {
      prop: 'setBrightnessAdjusted',
      onPropertyChange: false,
      value: { isBrightnessAdjusted: obj.isBrightAdjust },
    });
    parent.notify('draw', {
      prop: 'imageBackgroundColor',
      onPropertyChange: false,
      value: { color: obj.bgColor },
    });
    ...
    let activeObj: SelectionPoint = extend(
      {},
      obj.activeObj,
      {},
      true
    ...
    if (parent.afterCropActions.length > 0) {
      ...
      parent.notify('shape', {
        ...
        onPropertyChange: false,
        value: {
          collection: parent.afterCropActions,
          isRotateFlipCollection: null,
          obj: object,
        },
      });
      parent.afterCropActions = object['collection'];
    }
    const afterCropActions: string[] = extend(
      [],
      parent.afterCropActions,
      [],
      true
    ...
    if (!isObj && afterCropActions.length > 0) {
      ...
      for (
        let i: number = 0, len: number = afterCropActions.length;
        i < len;
        i++
      ) {
        if (
          afterCropActions[i as number] === 'horizontalflip' ||
          afterCropActions[i as number] === 'verticalflip'
        ) {
          parent.activeObj = extend(
            {},
            currSelectionPoint,
            {},
            true
        ...
        parent.notify('transform', {
          ...
          onPropertyChange: false,
          value: { text: afterCropActions[i as number] },
        });
      }
      activeObj = extend({}, parent.activeObj, {}, true) as SelectionPoint;
      this.resetPanPoints();
  ...
  private drawCropSelectionImage(obj: CurrentObject, isObj: boolean): void {
    ...
    if (
      obj.activeObj.activePoint.width !== 0 &&
      obj.activeObj.activePoint.height !== 0
    ) {
      const destPoints: ActivePoint = {
        startX: parent.img.destLeft,
        startY: parent.img.destTop,
  ...
  private performPointZoom(
    ...
    if (
      parent.activeObj.shape &&
      parent.activeObj.shape.indexOf('crop-') > -1
    ) {
      isCropSelection = true;
    }
    if (
    ...
    const zoomValue: number = value ? value : type === 'zoomIn' ? 0.1 : -0.1;
    parent.notify('transform', {
      prop: 'zoomAction',
      onPropertyChange: false,
  ...
  private panToPoint(
    ...
    if (parent.transform.zoomFactor > 0) {
      ...
      if (parent.transform.degree === 0) {
        ...
        const isCropTab: boolean = parent.isCropTab;
        parent.isCropTab = true;
        const objColl: SelectionPoint[] = extend(
          [],
          parent.objColl,
          [],
          true
        ) as SelectionPoint[];
        const pointColl: Point[] = extend(
          [],
          parent.pointColl,
          [],
          true
        ...
        parent.freehandCounter = parent.pointColl.length;
        if (
          straightenObj['straightenPoint']['x'] &&
          straightenObj['straightenPoint']['y']
        ) {
          parent.notify('freehand-draw', {
            prop: 'setStraightenPoint',
            onPropertyChange: false,
      ...
      for (let i: number = 0; i < parent.objColl.length; i++) {
        if (
          JSON.stringify(activeObj.activePoint) ===
          JSON.stringify(parent.objColl[i as number].activePoint)
        ) {
          isActObj = true;
          break;
        }
  ...
  private adjustPanning(activeObj: SelectionPoint): void {
    ...
    if (width !== 0 && height !== 0) {
      ...
      if (parent.transform.degree === 0) {
        ...
        parent.img.destTop -= point.y;
        parent.panPoint.currentPannedPoint = {
          x: parent.img.destLeft - destLeft,
          y: parent.img.destTop - destTop,
        };
        parent.notify('transform', {
          prop: 'rotatePan',
          onPropertyChange: false,
  ...
  private panToSel(): void {
    ...
    if (
      ...
      const panX: number =
        startX + width / 2 - straightenObj['straightenPoint']['x'];
      const panY: number =
        startY + height / 2 - straightenObj['straightenPoint']['y'];
      if (parent.transform.degree === 0) {
        parent.img.destLeft += panX;
        parent.img.destTop += panY;
        parent.notify('transform', {
          prop: 'drawPannImage',
          value: { point: { x: panX, y: panY } },
        });
      } else {
        parent.panPoint.currentPannedPoint = { x: panX, y: panY };
        parent.notify('transform', {
          prop: 'drawPannedImage',
          value: { xDiff: panX, yDiff: panY },
        });
      ...
      parent.notify('draw', {
        prop: 'drawObject',
        onPropertyChange: false,
        value: { canvas: 'duplicate', obj: activeObj },
      });
      const { destLeft, destTop, destWidth, destHeight } = parent.img;
  ...
  private openNewImage(): void {
    ...
    const toolbar: HTMLInputElement = document.querySelector(
      '#' + id + '_currPos'
    );
    if (toolbar) {
    ...
    parent.notify('toolbar', {
      prop: 'getDefToolbarItems',
      value: { obj: obj },
    });
    if (
      obj['defToolbarItems'].length === 0 &&
      isNullOrUndefined(document.getElementById(id + '_toolbar')) &&
      parent.element.querySelector('#' + id + '_toolbarArea')
    ) {
      const height: number = parent.element.querySelector(
        '#' + id + '_toolbarArea'
      ).clientHeight;
      parent.notify('toolbar', {
        prop: 'setToolbarHeight',
        value: { height: height },
      });
    }
    parent.reset();
    ...
    parent.currSelectionPoint = null;
    const type: string = typeof this.openURL;
    if (type === 'string') {
      ...
      this.fileType = this.getFileExtensionFromURL(
        this.openURL as string
      ) as FileType;
      if (this.fileType) {
      ...
      if (
        typeof this.openURL !== 'string' ||
        this.openURL.indexOf('localhost') === -1
      ) {
        this.getImageSizeFromURL(
          this.openURL.toString(),
          (imageSizeMB: number | null) => {
            if (imageSizeMB !== null) {
              this.parent.notify('toolbar', {
                prop: 'setInitialSize',
                value: { value: +imageSizeMB },
              });
            }
          }
        );
  ...
  private async getImageSizeFromURL(
    imageUrl: string,
    callback: (imageSizeMB: number | null) => void
  ): Promise<void> {
  ...
  private applyDialogOption(): void {
    ...
    (
      getComponent(
        document.getElementById(parent.element.id + '_dialog'),
        'dialog'
      ) as Dialog
    ).destroy();
    this.isImageEdited = false;
  ...
  private open(data: string | ImageData): void {
    const dropArea: HTMLElement = document.getElementById(
      this.parent.element.id + '_dropArea'
    );
    if (dropArea) {
  ...
  private fileSelect(inputElement: HTMLInputElement, args: Event): void {
    ...
    if (!parent.disabled) {
      ...
      if (fileData.name) {
        ...
        fileExtension =
          fileExtensionArray[fileExtensionArray.length - 1].toLowerCase();
      }
      if (
        fileExtension &&
        ['jpg', 'jpeg', 'png', 'svg'].indexOf(fileExtension) === -1
      ) {
        this.errorLoading();
        return;
      }
      ...
      const url: URL = URL.createObjectURL(filesData);
      this.openURL = url;
      // eslint-disable-next-line max-len
      if (
        parent.isImageLoaded &&
        !parent.isChangesSaved &&
        (this.isImageEdited ||
          parent.pointColl.length > 0 ||
          parent.objColl.length > 0)
      ) {
  ...
  private moveToSelectionRange(type: string, activeObj: SelectionPoint): void {
    ...
    if (parent.activeObj.shape) {
      ...
      if (isRotated) {
        ...
        const currObj: SelectionPoint =
          parent.objColl[parent.objColl.length - 1];
        if (type === 'rotateleft' || type === 'rotateright') {
          if (
            ...
            if (currObj.activePoint.width < activeObj.activePoint.height) {
              for (
                let i: number = 2;
                i < parent.zoomSettings.maxZoomFactor;
                i++
              ) {
                if (
                  currObj.activePoint.width >= activeObj.activePoint.height ||
            ...
            if (currObj.activePoint.height < activeObj.activePoint.width) {
              for (
                ...
                if (
                  currObj.activePoint.height >= activeObj.activePoint.width ||
                  this.isSelectionBiggerThanCanvas(currObj) ||
                  this.isSelectionOutsideCanvas(currObj)
                ) {
                  if (!isNullOrUndefined(zoomFactor)) {
        ...
        const panX: number =
          parent.lowerCanvas.clientWidth / 2 -
          (currObj.activePoint.startX + currObj.activePoint.width / 2);
        const panY: number =
          (parent.lowerCanvas.clientHeight + 1) / 2 -
          (currObj.activePoint.startY + currObj.activePoint.height / 2);
        ...
        parent.notify('draw', {
          prop: 'drawObject',
          onPropertyChange: false,
          value: { canvas: 'duplicate', obj: parent.activeObj },
        });
      }
    }
  }
  ...
  private isSelectionOutsideCanvas(obj: SelectionPoint): boolean {
    ...
    const parent: ImageEditor = this.parent;
    if (
      obj.activePoint.height <
        parent.lowerCanvas.height - parent.toolbarHeight ||
  ...
  private downScaleImgCanvas(
    ...
    isImgAnnotation: boolean,
    isHFlip: boolean,
    isVFlip: boolean
  ): void {
    const parent: ImageEditor = this.parent;
    const canvas: HTMLCanvasElement = isImgAnnotation
      ? parent.activeObj.imageCanvas
      : parent.baseImgCanvas;
    const img: HTMLImageElement = isImgAnnotation
      ? parent.activeObj.imageElement
      : parent.baseImg;
    const width: number = isImgAnnotation
      ? parent.activeObj.activePoint.width
      : parent.img.destWidth;
    const height: number = isImgAnnotation
    ...
    if (
      isImgAnnotation ||
      (parent.allowDownScale &&
        !parent.isCropTab &&
        !parent.isCropToolbar &&
        img.width !== 0 &&
        img.height !== 0 &&
        obj['width'] * 0.75 > width &&
        obj['height'] * 0.75 > height)
    ) {
      ...
      if (isImgAnnotation) {
        tempCanvas
          .getContext('2d')
          .drawImage(img, 0, 0, tempCanvas.width, tempCanvas.height);
      } else {
      ...
      if (isImgAnnotation) {
        ...
        if (isHFlip && isVFlip) {
          ...
          if (isHFlip) {
            if (
              isNullOrUndefined(parent.activeObj.isHorImageFlip) ||
              !parent.activeObj.isHorImageFlip
            ) {
            ...
            if (
              isNullOrUndefined(parent.activeObj.isVerImageFlip) ||
              !parent.activeObj.isVerImageFlip
            ) {
      ...
      if (
        (isNullOrUndefined(isImgAnnotation) || !isImgAnnotation) &&
        parent.baseImgCanvas.width !== 0 &&
  ...
  private downScale(
    ...
    const widthRatio: number = widthSource / width;
    const heightRatio: number = heightSource / height;
    const halfWidthRatio: number = Math.ceil(widthRatio / 2);
    const halfHeightRatio: number = Math.ceil(heightRatio / 2);
    ...
    const data2: Uint8ClampedArray = img2.data;
    for (let j: number = 0; j < height; j++) {
      for (let i: number = 0; i < width; i++) {
        const x2: number = (i + j * width) * 4;
        let weight: number = 0;
        let weights: number = 0;
        let alphaWeights: number = 0;
        let r: number = 0;
        let g: number = 0;
        let b: number = 0;
        let a: number = 0;
        const centerY: number = (j + 0.5) * heightRatio;
        const startY: number = Math.floor(j * heightRatio);
        const stopY: number = Math.ceil((j + 1) * heightRatio);
        for (let y: number = startY; y < stopY; y++) {
          ...
          const startX: number = Math.floor(i * widthRatio);
          const stopX: number = Math.ceil((i + 1) * widthRatio);
  ...
  private drawImgToCtx(
    ...
    const parent: ImageEditor = this.parent;
    if (
      ctx.canvas.id !== parent.element.id + '_tempCanvas' &&
      ctx !== this.upperContext &&
      isNullOrUndefined(preventImg)
    ) {
  ...
  private getFrameColor(
    ...
    if (frameObj.gradientColor) {
      /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
      const gradient: any = ctx.createLinearGradient(
        points.startX,
        points.startY,
        points.startX + points.width,
  ...
  private applyFrame(
    ...
    if (ctx.canvas.id === parent.element.id + '_tempCanvas') {
      const newWidth: number = ctx.canvas.width;
      const newHeight: number = ctx.canvas.height;
      const obj: Object = { width: 0, height: 0 };
      parent.notify('crop', {
        prop: 'calcRatio',
        onPropertyChange: false,
      ...
      parent.notify('export', {
        prop: 'updateSaveContext',
        onPropertyChange: false,
        value: { context: ctx },
      });
    } else if (ctx === this.upperContext && parent.activeObj.shape) {
      points = {
        startX: parent.activeObj.activePoint.startX - ctx.lineWidth,
        startY: parent.activeObj.activePoint.startY - ctx.lineWidth,
        width: parent.activeObj.activePoint.width + 2 * ctx.lineWidth,
        height: parent.activeObj.activePoint.height + 2 * ctx.lineWidth,
      };
    ...
    if (ctx === this.upperContext && parent.activeObj.shape) {
      if (
        (frame === 'mat' &&
          (points.width - 2 * frameObj.size < 0 ||
            points.height - 2 * frameObj.size < 0)) ||
        (frame === 'bevel' &&
          (points.width - 2 * frameObj.size < 40 ||
        ...
        (frame === 'inset' &&
          (points.startX +
            ...
            points.startY +
              ...
              0)) ||
        (frame === 'hook' &&
          (points.width - 2 * frameObj.size < 50 ||
            points.height - 2 * frameObj.size < 50))
      ) {
        return;
      }
    }
      ...
      switch (frame) {
        ...
        case 'bevel':
          ...
          ctx.quadraticCurveTo(
            ...
            points.startY,
            points.startX + points.width,
            points.startY + valueForty
          );
          ctx.lineTo(
            points.startX + points.width,
            points.startY + points.height - valueForty
          );
          ctx.quadraticCurveTo(
            points.startX + points.width,
            points.startY + points.height,
            points.startX + points.width - valueForty,
            points.startY + points.height
          );
        ...
        case 'line':
          ...
          for (let i: number = 0; i < parent.frameObj.amount; i++) {
            ...
            const arcY2: number =
              points.startY + points.height - frameObj.inset - frameObj.radius;
            const lineY: number =
              points.startY + frameObj.inset + frameObj.radius;
            const arcX2: number =
              points.startX + points.width - frameObj.inset - frameObj.radius;
            const lineX: number =
              points.startX + frameObj.inset + frameObj.radius;
            const arcX1: number =
              points.startX + frameObj.inset + frameObj.radius;
            const lineX2: number =
              points.startX + points.width - frameObj.inset - frameObj.radius;
            ...
            if (
              ...
              ctx.arcTo(
                points.startX + points.width - frameObj.inset,
                points.startY + frameObj.inset,
                points.startX + points.width - frameObj.inset,
                points.startY + frameObj.inset + frameObj.radius,
                frameObj.radius
              ...
              ctx.arcTo(
                points.startX + points.width - frameObj.inset,
                points.startY + points.height - frameObj.inset,
                points.startX + points.width - frameObj.inset - frameObj.radius,
                points.startY + points.height - frameObj.inset,
                frameObj.radius
              );
        ...
        case 'inset':
          ...
          ctx.lineTo(
            points.startX + points.width - frameObj.offset,
            points.startY + frameObj.inset
          );
          ctx.moveTo(
            points.startX + points.width - frameObj.inset,
          ...
          ctx.lineTo(
            points.startX + points.width - frameObj.inset,
            points.startY + points.height - frameObj.offset
          );
          ctx.moveTo(
            points.startX + points.width - frameObj.offset,
            points.startY + points.height - frameObj.inset
        ...
        case 'hook':
          ...
          ctx.beginPath();
          ctx.moveTo(
            points.startX + frameObj.inset + valueFifty,
            points.startY + frameObj.inset
          );
          ctx.lineTo(
            points.startX + frameObj.inset,
          ...
          ctx.lineTo(
            points.startX + frameObj.inset,
            points.startY + frameObj.inset + valueFifty
          );
          ctx.moveTo(
            points.startX + points.width - frameObj.inset - valueFifty,
            points.startY + frameObj.inset
          );
          ctx.lineTo(
            points.startX + points.width - frameObj.inset,
          ...
          ctx.lineTo(
            points.startX + points.width - frameObj.inset,
            points.startY + points.height - frameObj.inset - valueFifty
          );
          ctx.moveTo(
            points.startX + frameObj.inset + valueFifty,
            points.startY + points.height - frameObj.inset
      ...
      if (
        ...
        parent.notify('crop', {
          ...
          value: {
            context: ctx,
            isSave:
              ctx.canvas.id === parent.element.id + '_tempCanvas' ? true : null,
            isFlip: null,
          },
        });
      }
  ...
  private triggerFrameChange(
    prevFrameSettings: FrameSettings
  ): FrameChangeEventArgs {
    const parent: ImageEditor = this.parent;
    const currFrameSettings: FrameSettings = {
      ...
      frameLineStyle: parent.toPascalCase(
        parent.frameObj.border
      ) as FrameLineStyle,
      lineCount: parent.frameObj.amount,
    };
    const frameChange: FrameChangeEventArgs = {
      cancel: false,
  ...
  private zoomToSel(activeObj: SelectionPoint, isToolbar: boolean): void {
    const parent: ImageEditor = this.parent;
    if (
      this.straightenActObj &&
      JSON.stringify(this.straightenActObj.activePoint) ===
        JSON.stringify(activeObj.activePoint)
    ) {
      parent.activeObj = extend(
        {},
      ...
      if (parent.transform.straighten === 0) {
        ...
        while (true) {
          if (
            !isNullOrUndefined(this.straightenInitZoom) &&
            Math.round(parent.transform.zoomFactor * Math.pow(10, 3)) /
              Math.pow(10, 3) >
              Math.round(this.straightenInitZoom * Math.pow(10, 3)) /
                Math.pow(10, 3)
          ) {
            this.setZoomPan('out');
            if (
        ...
        if (this.straightenInitZoom - parent.transform.zoomFactor > 0) {
          ...
          parent.notify('transform', {
            ...
            onPropertyChange: false,
            value: {
              zoomFactor: this.straightenInitZoom - parent.transform.zoomFactor,
              zoomPoint: null,
              isResize: true,
            },
          });
  ...
  private isDestPointSmall(): boolean {
    ...
    parent.notify('shape', {
      prop: 'straightenShapes',
      onPropertyChange: false,
    });
    let isSmall: boolean = false;
    if (
      this.straightenDestPoints.destWidth &&
      this.straightenDestPoints.destHeight &&
      (img.destWidth < this.straightenDestPoints.destWidth ||
        img.destHeight < this.straightenDestPoints.destHeight)
    ) {
      isSmall = true;
    }
  ...
  private calcStraightenedPoints(isToolbar: boolean): void {
    ...
    while (true) {
      if (
        this.isLinesIntersect() ||
        this.isSelOutsideImg() ||
        (isToolbar && this.isDestPointSmall())
      ) {
        parent.activeObj = extend(
          {},
          this.straightenActObj,
        ...
        const points: Point[] = this.imgCanvasPoints;
        const left: number = parent.img.destLeft;
        const top: number = parent.img.destTop;
        const width: number = parent.img.destWidth;
        const height: number = parent.img.destHeight;
  ...
  private setZoomPan(type: string): void {
    ...
    if (parent.transform.degree === 0) {
      parent.notify('transform', {
        ...
        onPropertyChange: false,
        value: {
          value: type === 'in' ? 0.025 : -0.025,
          selectionObj: parent.activeObj,
          obj: obj,
        },
      });
  ...
  private updateImgCanvasPoints(): void {
    ...
    const ratio: Dimension = obj as Dimension;
    width =
      parent.transform.degree % 90 === 0 && parent.transform.degree % 180 !== 0
        ? ratio.height
        : ratio.width;
    height =
      parent.transform.degree % 90 === 0 && parent.transform.degree % 180 !== 0
    ...
    if (parent.transform.straighten > 0) {
      ...
      p3 = {
        x: destLeft + destWidth - points[0].x / width,
      ...
      p3 = {
        ...
        y: destTop + destHeight - points[0].y / height,
      };
      p4 = {
        x: destLeft + destWidth - points[1].x / width,
        y: destTop + destHeight,
      };
    } else if (parent.transform.straighten === 0) {
  ...
  private isLinesIntersect(obj?: Object): boolean {
    ...
    if (parent.activeObj.rotatedAngle !== 0) {
      const { startX, startY, endX, endY, width, height } =
      ...
      const p1: Point = {
        x:
          cosAngle * (startX - center.x) -
          sinAngle * (startY - center.y) +
          center.x,
        y:
          sinAngle * (startX - center.x) +
          cosAngle * (startY - center.y) +
          center.y,
      };
      const p2: Point = {
        x:
          cosAngle * (endX - center.x) -
          sinAngle * (startY - center.y) +
          center.x,
        y:
          sinAngle * (endX - center.x) +
          cosAngle * (startY - center.y) +
          center.y,
      };
      const p3: Point = {
        x:
          cosAngle * (startX - center.x) -
          sinAngle * (endY - center.y) +
          center.x,
        y:
          sinAngle * (startX - center.x) +
          cosAngle * (endY - center.y) +
          center.y,
      };
      const p4: Point = {
        x:
      ...
      const isTopIntersect: boolean = this.doIntersect(
        p1,
        p2,
        imgPoints[0],
        imgPoints[1]
      );
      const isRightIntersect: boolean = this.doIntersect(
        p2,
        p4,
        imgPoints[1],
        imgPoints[2]
      );
      const isBottomIntersect: boolean = this.doIntersect(
        p3,
        p4,
        imgPoints[2],
        imgPoints[3]
      );
      const isLeftIntersect: boolean = this.doIntersect(
        p1,
        p3,
        imgPoints[3],
        imgPoints[0]
    ...
    return (
      ...
      isLeftIntersect ||
      isTopLeftInsideRect ||
      isTopRightInsideRect ||
      isBottomRightInsideRect ||
      isBottomLeftInsideRect ||
      (imgPoints[0].x > point.startX &&
        imgPoints[1].x < point.endX &&
        imgPoints[2].x < point.endX &&
        imgPoints[3].x > point.startX &&
        imgPoints[0].y < point.startY &&
        imgPoints[1].y < point.startY &&
        imgPoints[2].y > point.endY &&
        imgPoints[3].y > point.endY) ||
      (imgPoints[0].x < point.startX &&
        imgPoints[1].x > point.endX &&
        imgPoints[2].x > point.endX &&
        imgPoints[3].x < point.startX &&
  ...
  private checkPointPosition(
    ...
    const areaRectangle: number =
      this.calcTriangleArea(x1, y1, x2, y2, x3, y3) +
      this.calcTriangleArea(x3, y3, x4, y4, x1, y1);
    if (area1 + area2 + area3 + area4 > areaRectangle) {
      return 'outside';
    } else if (
      area1 + area2 + area3 + area4 === areaRectangle &&
  ...
  private getImagePoints(): Point[] {
    ...
    const obj: Object = {
      ...
      width: height,
      height: width,
      angle: parent.transform.straighten,
    };
    obj['dim'] = parent.getRotatedCanvasDim(
      obj['width'],
      obj['height'],
      obj['angle']
    );
    const baseImgCanvasWidth: number =
      degree % 90 === 0 && degree % 180 !== 0
        ? obj['dim']['width']
        : parent.baseImgCanvas.width;
    const baseImgCanvasHeight: number =
      degree % 90 === 0 && degree % 180 !== 0
        ? obj['dim']['height']
        : parent.baseImgCanvas.height;
    const baseImgWidth: number =
      degree % 90 === 0 && degree % 180 !== 0 ? height : width;
    const baseImgHeight: number =
      degree % 90 === 0 && degree % 180 !== 0 ? width : height;
    ...
    const p1: Point = {
      x:
        Math.cos(radians) * (startX - center.x) -
        Math.sin(radians) * (startY - center.y) +
        center.x,
      y:
        Math.sin(radians) * (startX - center.x) +
        Math.cos(radians) * (startY - center.y) +
        center.y,
    };
    const p2: Point = {
      x:
        Math.cos(radians) * (endX - center.x) -
        Math.sin(radians) * (startY - center.y) +
        center.x,
      y:
        Math.sin(radians) * (endX - center.x) +
        Math.cos(radians) * (startY - center.y) +
        center.y,
    };
    const p3: Point = {
      x:
        Math.cos(radians) * (endX - center.x) -
        Math.sin(radians) * (endY - center.y) +
        center.x,
      y:
        Math.sin(radians) * (endX - center.x) +
        Math.cos(radians) * (endY - center.y) +
        center.y,
    };
    const p4: Point = {
      x:
  ...
  private isInsideRect(point: Point): boolean {
    ...
    if (
      point.x >= actPoint.startX &&
      point.x <= actPoint.endX &&
      point.y >= actPoint.startY &&
      point.y <= actPoint.endY
    ) {
      isInside = true;
    }
    return isInside;
  ...
  private drawRedact(
    ...
    let isSaveCtx: boolean = false;
    const canvas: HTMLCanvasElement = canvasDraw.canvas;
    ...
    if (width <= 0 || height <= 0) {
      ...
      canvasDraw.drawImage(
        ...
        0,
        0,
        obj.redactImage.width,
        obj.redactImage.height,
        startX,
        startY,
        width,
        height
      ...
      const straighten: number =
        this.parent.transform.straighten !== 0
          ? this.parent.transform.straighten
      ...
      if (this.allowRedactStraighten && straighten !== 0) {
        ...
        const straightenCanvas: HTMLCanvasElement =
          document.createElement('canvas');
        const straightenCtx: CanvasRenderingContext2D =
          straightenCanvas.getContext('2d');
        ...
        const newP1: Point = {
          x:
            cosAngle * (p1.x - center.x) -
            sinAngle * (p1.y - center.y) +
            center.x,
          y:
            sinAngle * (p1.x - center.x) +
            cosAngle * (p1.y - center.y) +
            center.y,
        };
        const newP2: Point = {
          x:
            cosAngle * (p2.x - center.x) -
            sinAngle * (p2.y - center.y) +
            center.x,
          y:
            sinAngle * (p2.x - center.x) +
            cosAngle * (p2.y - center.y) +
            center.y,
        };
        const newP3: Point = {
          x:
            cosAngle * (p3.x - center.x) -
            sinAngle * (p3.y - center.y) +
            center.x,
          y:
            sinAngle * (p3.x - center.x) +
            cosAngle * (p3.y - center.y) +
            center.y,
        };
      ...
      if (this.parent.activeObj.redactType === 'blur') {
        ...
        if (this.parent.isSafari) {
          this.parent.notify('filter', {
            prop: 'apply-filter',
            onPropertyChange: false,
            value: { context: offscreenCtx },
          });
        }
        canvasDraw.drawImage(
          offscreenCanvas,
