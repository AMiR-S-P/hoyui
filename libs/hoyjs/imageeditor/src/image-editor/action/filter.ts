...
export class Filter {
  ...
  private adjustmentLevel: Adjustment = {
    brightness: 0,
    contrast: 0,
    hue: 0,
    opacity: 100,
    ...
    sharpen: false,
    bw: false,
  }; // for toolbar slider value
  private tempAdjustmentLevel: Adjustment = {
    brightness: 0,
    contrast: 0,
    hue: 0,
    opacity: 100,
  ...
  private filter(args?: {
    ...
    switch (args.prop) {
      case 'finetuneImage':
        this.finetuneImage(
          args.value['option'] as ImageFinetuneOption,
          args.value['value'] as number
        );
        break;
      ...
      case 'setCurrAdjValue':
        this.setCurrAdjValue(
          args.value['type'] as string,
          args.value['value'] as number
        );
        break;
      case 'updateAdj':
      ...
      case 'getAdjustmentLevel':
        if (isNullOrUndefined(this.parent.activeObj.opacity)) {
          ...
          this.adjustmentLevel.transparency =
            this.parent.activeObj.opacity * 100;
        }
      ...
      case 'setBrightnessAdjusted':
        ...
        if (
          this.parent.currentFilter.split('_') &&
          this.parent.currentFilter.split('_')[1] === 'cold'
        ) {
    ...
    if (fs) {
      ...
      propertiesToSet.forEach((property: string) => {
        if (fs[property as string]) {
          this.adjustmentLevel[property as string] =
            fs[property as string].defaultValue;
          this.tempAdjustmentLevel[property as string] =
            fs[property as string].defaultValue;
        }
      });
      parent.notify('draw', {
        prop: 'isInitialLoading',
        onPropertyChange: false,
        value: { isInitialLoading: true },
      });
    }
  }
  ...
  private updateAdj(
    ...
    let values: string[] = [];
    const brightness: number = this.getFilterValue(
    ...
    let saturate: number;
    let bright: number;
    let saturatePercent: number;
    let contrast: number;
    let saturatePercentage: number;
    switch (type) {
      case 'brightness':
        value =
          this.getFilterValue(this.adjustmentLevel.exposure) + value * 0.005;
        splitWords[0] = 'brightness(' + value + ')';
        if (this.adjustmentLevel.brightness !== 0) {
          value =
            this.adjustmentLevel.opacity / 100 -
            (this.adjustmentLevel.opacity * 0.3) / 100;
      ...
      case 'exposure':
        ...
        break;
      case 'chrome':
        saturate = this.getSaturationFilterValue(
      ...
      case 'cold':
        ...
        saturatePercentage = this.getSaturationFilterValue(
          this.adjustmentLevel.saturation
        );
        ...
        break;
      case 'warm':
        saturatePercent = this.getSaturationFilterValue(
          this.adjustmentLevel.saturation
        );
        saturatePercent *= 100;
    ...
    if (type !== 'sharpen' && type !== 'blackandwhite') {
      ...
      splitWords = this.setTempFilterValue(
        brightness,
        isPreview,
        splitWords,
        type
  ...
  private setCurrAdjValue(type: string, value: number): void {
    ...
    parent.curFinetuneObjEvent = {
      finetune: parent.toPascalCase(type) as ImageFinetuneOption,
      value: value,
    };
  }
  ...
  private getCurrentObj(dummyObj?: Object): CurrentObject {
    ...
    const obj: CurrentObject = {
      ...
      aspectHeight: null,
      straightenZoom: 0,
      adjustmentLevel: extend({}, this.tempAdjVal, {}, true) as Adjustment,
    ...
    obj.totalPannedPoint = extend(
      {},
      parent.panPoint.totalPannedPoint,
      {},
      true
    ) as Point;
    obj.totalPannedClientPoint = extend(
      {},
      parent.panPoint.totalPannedClientPoint,
      {},
      true
    ) as Point;
    obj.totalPannedInternalPoint = extend(
      {},
      parent.panPoint.totalPannedInternalPoint,
      {},
      true
    ) as Point;
    obj.tempFlipPanPoint = extend(
      {},
      tempFlipPanPointObj['point'],
      {},
      true
    ) as Point;
    obj.activeObj = extend({}, parent.activeObj, {}, true) as SelectionPoint;
    obj.rotateFlipColl = extend([], parent.rotateFlipColl, [], true) as
      | string[]
      | number[];
      ...
      switch (filterArray[i as number].filter) {
        case 'blur':
          imageData = this.blur(
            context,
            imageData,
            filterArray[i as number].value
          );
          break;
        case 'brightness':
          imageData = this.brightness(
            imageData,
            filterArray[i as number].value
          );
          break;
  ...
  private blur(
    ...
    let blurRadius: number = this.getValFromLength(radius);
  ...
  private saturate(
    ...
    if (factor !== 1) {
      ...
      const { data } = imageData;
      const lumR: number = (1 - factor) * 0.3086;
      const lumG: number = (1 - factor) * 0.6094;
      ...
      for (let j: number = 0; j < height; j++) {
        ...
        for (let i: number = 0; i < width; i++) {
          ...
          const r: number = data[pos + 0];
          const g: number = data[pos + 1];
          const b: number = data[pos + 2];
