...
import { ActivePoint } from '@hoyui/inputs';
import {
  FreehandDraw,
  Point,
  SelectionPoint,
...
export class FreehandDrawing {
  ...
  private tempFHDStyles: StrokeSettings = {
    strokeColor: null,
    fillColor: null,
    strokeWidth: null,
  };
  private fhdSelIdx: number;
  private straightenPoint: Point = {
    x: null,
    y: null,
    ratioX: null,
    ratioY: null,
  ...
  private draw(args?: {
    ...
    switch (args.prop) {
      ...
      case 'getStraightenPoint':
        args.value['obj']['straightenPoint'] = extend(
          {},
  ...
  private hoverFhd(fillStyle?: string, strokeWidth?: number): void {
    ...
    parent.points = extend(
      [],
      parent.pointColl[idx as number].points
    ) as Point[];
    this.pointCounter = 0;
    ...
    let endPoint: Point;
    let minStrokeWidth: number = 0;
    let maxStrokeWidth: number = 0;
    context.fillStyle = fillStyle
      ? fillStyle
      : parent.pointColl[idx as number].strokeColor;
    context.strokeStyle = context.fillStyle;
    minStrokeWidth =
      maxStrokeWidth =
      this.penStrokeWidth =
        strokeWidth ? strokeWidth : parent.pointColl[idx as number].strokeWidth;
    if (len === 1) {
    ...
    for (let l: number = 0; l < len - 3; l++) {
      if (
        ...
        controlPoint1 = this.calcCurveCP(
          parent.points[l + 0],
          parent.points[l + 1],
          parent.points[l + 2]
        ).controlPoint2;
        controlPoint2 = this.calcCurveCP(
          parent.points[l + 1],
          parent.points[l + 2],
          parent.points[l + 3]
        ).controlPoint1;
  ...
  private freehandDownHandler(
    ...
    parent.lowerCanvas = document.querySelector(
      '#' + parent.element.id + '_lowerCanvas'
    );
    this.lowerContext = parent.lowerCanvas.getContext('2d');
    parent.upperCanvas = document.querySelector(
      '#' + parent.element.id + '_upperCanvas'
    );
    this.upperContext = parent.upperCanvas.getContext('2d');
    ...
    if (parent.points.length === 0) {
      if (e.type === 'mouseup') {
        ...
        if (!this.isFreehandPointMoved) {
          this.processPoint(
            this.freehandDownPoint.x - rect.left,
            this.freehandDownPoint.y - rect.top,
            true,
            context
          );
        }
  ...
  private freehandMoveHandler(e: MouseEvent & TouchEvent): void {
    ...
    const rect: DOMRect =
      this.parent.upperCanvas.getBoundingClientRect() as DOMRect;
    let x: number;
    let y: number;
    if (e.type === 'mousemove') {
    ...
    if (this.isFreehandDrawing) {
      this.upperContext.fillStyle =
        this.parent.activeObj.strokeSettings.strokeColor;
  ...
  private processPoint(
    ...
    lastPoint =
      parent.points.length > 0 && parent.points[parent.points.length - 1];
    const isLastPointTooClose: boolean = lastPoint
      ? this.distanceTo(lastPoint) <= 5
      : false;
    let controlPoint1: Point;
    let controlPoint2: Point;
    let startPoint: Point;
    ...
    if (!lastPoint || !(lastPoint && isLastPointTooClose) || mouseDown) {
      ...
      if (this.dummyPoints.length > 2) {
        ...
        let minStrokeWidth: number = 0.5;
        let maxStrokeWidth: number = 5;
  ...
  private calcCurveCP(
    ...
    const dx1: number = p1.x - p2.x;
    const dy1: number = p1.y - p2.y;
    const dx2: number = p2.x - p3.x;
    const dy2: number = p2.y - p3.y;
    ...
    const dxm: number = m1.x - m2.x;
    const dym: number = m1.y - m2.y;
    const k: number = l2 / (l1 + l2);
    const cm: Point = { x: m2.x + dxm * k, y: m2.y + dym * k };
    const tx: number = p2.x - cm.x;
    const ty: number = p2.y - cm.y;
    return {
  ...
  private pointVelocity(startPoint: Point): number {
    return this.fhdObj.time !== startPoint.time
      ? this.distanceTo(startPoint) / (this.fhdObj.time - startPoint.time)
      : 0;
  }

  private distanceTo(start: Point): number {
    return Math.sqrt(
      Math.pow(this.fhdObj.pointX - start.x, 2) +
        Math.pow(this.fhdObj.pointY - start.y, 2)
    );
  }
  ...
  private drawCurve(
    ...
    const bezierLength: number = this.bezierLength(
      controlPoint1,
      controlPoint2,
      startPoint,
      endPoint
  ...
  private freehandRedraw(
    ...
    if (points) {
      ...
      this.selPointColl[parent.freehandCounter] = extend(
        {},
        parent.pointColl[parent.freehandCounter],
        {},
        true
    ...
    if (parent.freehandCounter > 0) {
      for (let n: number = 0; n < parent.freehandCounter; n++) {
        ...
        let startPoint: Point;
        let endPoint: Point;
        let minStrokeWidth: number;
        let maxStrokeWidth: number;
        if (len > 0) {
          context.fillStyle = parent.pointColl[n as number].strokeColor;
          minStrokeWidth =
            maxStrokeWidth =
            this.penStrokeWidth =
              parent.pointColl[n as number].strokeWidth;
        }
        if (len === 1) {
          controlPoint1 =
            controlPoint2 =
  ...
  private getSqPtFD(idx: number, obj?: Object): ActivePoint {
    ...
    const sPoints: Point[] = extend(
      [],
      this.selPointColl[idx as number].points,
      []
    ) as Point[];
    this.parent.points = extend(
      [],
      this.parent.pointColl[idx as number].points
    ) as Point[];
    this.pointCounter = 0;
    const len: number = sPoints.length;
    for (let l: number = 0; l < len; l++) {
      if (
        activePoint.startX === 0 &&
        activePoint.startY === 0 &&
        activePoint.endX === 0 &&
        activePoint.endY === 0
      ) {
        ...
        activePoint.startX = Math.min(
          activePoint.startX,
          sPoints[l as number].x
        );
        activePoint.startY = Math.min(
          activePoint.startY,
          sPoints[l as number].y
        );
  ...
  private cancelFhd(): void {
    ...
    parent.activeObj.strokeSettings.strokeColor =
      this.tempFHDStyles.strokeColor;
    parent.activeObj.strokeSettings.strokeWidth = this.penStrokeWidth =
      this.tempFHDStyles.strokeWidth;
    this.tempFHDStyles = {
      strokeColor: null,
      strokeWidth: null,
      fillColor: null,
  ...
  private selectFhd(index?: number): void {
    const parent: ImageEditor = this.parent;
    const tempFHDStyles: StrokeSettings = extend(
      {},
      this.tempFHDStyles,
      {},
      true
    ...
    if (point.strokeColor !== '#42a5f5') {
      parent.activeObj.strokeSettings.strokeColor =
        this.tempFHDStyles.strokeColor = point.strokeColor;
    }
  ...
  private deleteFhd(index: number, isId?: boolean): void {
    ...
    if (this.isFHDIdx(index)) {
      ...
      for (let i: number = 0; i < parent.freehandCounter; i++) {
        if (
          parseInt(tempPointColl[i as number].id.split('_')[1], 10) - 1 !==
          index
        ) {
  ...
  private updateFHDCurPts(): void {
    ...
    for (let n: number = 0; n < parent.freehandCounter; n++) {
      if (this.selPointColl[n as number]) {
        this.selPoints = extend(
          [],
          this.selPointColl[n as number].points,
          []
        ) as Point[];
        this.pointCounter = 0;
  ...
  private pointsHorizontalFlip(): void {
    ...
    for (let n: number = 0; n < parent.freehandCounter; n++) {
      if (
        parent.pointColl[n as number].shapeFlip !==
    ...
    for (let n: number = 0; n < parent.freehandCounter; n++) {
      if (this.selPointColl[n as number]) {
        if (
          this.selPointColl[n as number].shapeFlip !==
          parent.transform.currFlipState
        ) {
          this.selPoints = extend(
            [],
  ...
  private updateFHDColl(isPreventApply?: boolean): void {
    ...
    const { destLeft, destTop, destWidth, destHeight } = parent.img;
    for (
      let i: number = 0, iLen: number = parent.objColl.length;
      i < iLen;
      i++
    ) {
      ...
      if (currObj.shape === 'line' || currObj.shape === 'arrow') {
        parent.notify('shape', {
          prop: 'straightenShapePoints',
          value: { obj: currObj, isReverse: true },
        });
      ...
      if (currObj.shape === 'path') {
        for (
          let j: number = 0, jLen: number = currObj.pointColl.length;
          j < jLen;
          j++
        ) {
    ...
    if (this.straightenPoint.x && this.straightenPoint.y) {
      this.straightenPoint.ratioX =
        (this.straightenPoint.x - destLeft) / destWidth;
      this.straightenPoint.ratioY =
        (this.straightenPoint.y - destTop) / destHeight;
    }
    parent.img.destLeft = destPoints.startX;
    parent.img.destTop = destPoints.startY;
    parent.img.destWidth = destPoints.width;
  ...
  private isFHDIdx(index: number, obj?: Object): boolean {
    ...
    for (let i: number = 0; i < this.parent.freehandCounter; i++) {
      if (
        this.parent.pointColl[i as number].id &&
        parseInt(this.parent.pointColl[i as number].id.split('_')[1], 10) -
          1 ===
          index
      ) {
        isIndex = true;
  ...
  private updateCropPtsForSel(): void {
    ...
    for (let n: number = 0; n < parent.freehandCounter; n++) {
      ...
      if (obj['selPointColl'][n as number]) {
        this.selPoints = extend(
          [],
          obj['selPointColl'][n as number].points,
          []
        ) as Point[];
        this.pointCounter = 0;
        const len: number = this.selPoints.length;
  ...
  private triggerShapeChanging(shapeChangingArgs: ShapeChangeEventArgs): void {
    ...
    if (
      parent.element.getAttribute('data-value') === 'mask-drawing' &&
      !this.isMasking
    ) {
      this.isMasking = true;
    ...
    if (
      shapeChangingArgs.currentShapeSettings.id.indexOf('pen_') === -1 &&
      (shapeChangingArgs.action === 'draw-end' ||
        shapeChangingArgs.action === 'select')
    ) {
  ...
  private setCenterSelPoints(): void {
    ...
    const actPoint: ActivePoint = parent.activeObj.activePoint;
    if (
      isNullOrUndefined(this.prevStraightenObj) ||
      JSON.stringify(this.prevStraightenObj.activePoint) !==
        JSON.stringify(actPoint)
    ) {
      this.straightenPoint = {
