import { Browser, extend, isNullOrUndefined } from '@hoyui/base';
...
export class Crop {
  ...
  private cropDestPoints: ActivePoint = {
    startX: 0,
    startY: 0,
    width: 0,
    height: 0,
  } as ActivePoint; // To redraw old image when navigate to crop tab
  ...
  private cropping(args?: {
    ...
    switch (args.prop) {
      ...
      case 'adjustStraightenForShapes':
        this.adjustStraightenForShapes(
          args.value['type'],
          args.value['isInitialRotated']
        );
        break;
      case 'resizeWrapper':
        this.resizeWrapper();
  ...
  private cropImg(isRotateCrop?: boolean): void {
    const parent: ImageEditor = this.parent;
    const isNullCrop: boolean = isNullOrUndefined(isRotateCrop);
    const resizeIcon: HTMLInputElement = parent.element.querySelector(
      '#' + parent.element.id + '_nonaspectratio'
    ) as HTMLInputElement;
    ...
    if ((isNullCrop && parent.transform.degree !== 0) || isRotated) {
      ...
      parent.notify('draw', {
        prop: 'setTempZoomFactor',
        onPropertyChange: false,
        value: { tempZoomFactor: parent.transform.zoomFactor },
      });
      const ratio: Dimension = this.calcRatio();
      if (isNullCrop || !isRotateCrop) {
        // isRotateCrop is NULL or False
        this.updateCropObj();
        parent.notify('draw', {
          prop: 'resetPanPoints',
          onPropertyChange: false,
  ...
  private adjustStraightenForShapes(
    ...
    for (const obj of parent.objColl) {
      if (
        ['rectangle', 'ellipse', 'text', 'image', 'redact'].indexOf(
          obj.shape
        ) !== -1
      ) {
        if (isInitialRotated || obj.rotatedAngle !== 0) {
          ...
          const angle: number =
            type === 'initial' ? obj.rotatedAngle : -obj.rotatedAngle;
          ...
          const centerX: number =
            cosAngle * diffX - sinAngle * diffY + center.x;
          const centerY: number =
            sinAngle * diffX + cosAngle * diffY + center.y;
  ...
  private rotateCrop(): void {
    ...
    const tempCurrSelObj: SelectionPoint = extend(
      {},
      parent.currSelectionPoint,
      {},
      true
    ...
    parent.notify('transform', {
      prop: 'getPreventSelect',
      onPropertyChange: false,
      value: { obj: preventSelObj },
    });
    parent.notify('transform', {
      prop: 'setPreventSelect',
      onPropertyChange: false,
      value: { bool: true },
    });
    const coll: string[] | number[] = extend(
      [],
      parent.rotateFlipColl,
      [],
      true
    ) as string[] | number[];
      ...
      for (let i: number = coll.length - 1; i >= 0; i--) {
        ...
        switch (value) {
          ...
          default:
            parent.notify('transform', {
              prop: 'flipImage',
              value: { direction: parent.toPascalCase(value.toString()) },
            });
            break;
        }
      }
  ...
  private resetZoom(): void {
    ...
    if (parent.transform.zoomFactor > 0) {
      ...
      parent.setProperties(
        { zoomSettings: { zoomFactor: zoomFactor * 10 } },
        true
      );
      parent.notify('transform', {
  ...
  private flipCrop(): void {
    ...
    if (parent.transform.degree === 0) {
      const panX: number = -parent.cropObj.totalPannedPoint.x;
      const panY: number = -parent.cropObj.totalPannedPoint.y;
    ...
    parent.notify('shape', {
      prop: 'redrawObj',
      onPropertyChange: false,
      value: { degree: this.getCurrFlipState() },
    });
    ...
    parent.notify('draw', {
      ...
      value: {
        type: 'initial',
        isPreventDestination: null,
        context: null,
        isPreventCircleCrop: null,
      },
    });
    parent.notify('draw', { prop: 'drawImage', onPropertyChange: false });
    this.lowerContext.filter = temp;
          ...
          case 'text':
            degree =
              obj.shapeDegree === 0
                ...
                : parent.transform.degree - obj.shapeDegree;
            size =
              degree === 0 || Math.abs(degree) === 180
                ? point.width
                : point.height;
  ...
  private cropFreehandDrawColl(): void {
    ...
    for (let n: number = 0; n < parent.freehandCounter; n++) {
      parent.points = extend(
        [],
        parent.pointColl[n as number].points,
        []
      ) as Point[];
      parent.notify('freehand-draw', {
        prop: 'setPointCounter',
        onPropertyChange: false,
        value: { value: 0 },
      });
      const len: number = parent.points.length;
      for (let l: number = 0; l < len; l++) {
        parent.points[l as number].ratioX =
          (parent.points[l as number].x - startX) / width;
        parent.points[l as number].ratioY =
          (parent.points[l as number].y - startY) / height;
      }
    }
    parent.notify('freehand-draw', {
      prop: 'updateCropPtsForSel',
      onPropertyChange: false,
    });
  }
  ...
  private setCurrSelPoints(isSetDimension?: boolean): void {
    ...
    if (
      ...
      parent.transform.degree === 0 &&
      currSelPoint &&
      currSelPoint.shape !== 'crop-circle' &&
      currSelPoint.shape !== 'crop-square'
    ) {
    ...
    parent.notify('draw', {
      ...
      value: {
        ...
        isPreventDestination: null,
        context: null,
        isPreventCircleCrop: true,
      },
    });
    let cropObjColl: SelectionPoint[] = extend(
      [],
      parent.objColl,
      null,
      true
    ) as SelectionPoint[];
    let cropPointColl: Point[] = extend(
      [],
      parent.pointColl,
      null,
      true
    ...
    if (
      ...
      parent.cropObj.activeObj.shape
    ) {
      parent.activeObj = extend(
        {},
        parent.cropObj.activeObj,
        null,
        true
      ) as SelectionPoint;
    }
    ...
    if (document.querySelector('.e-ie-straighten-value-span')) {
      document.querySelector('.e-ie-straighten-value-span').innerHTML =
        parent.transform.straighten.toString() + '&#176';
    }
  }
  ...
  private cropCircle(
    ...
    if (isFlip && parent.transform.currFlipState !== '') {
      parent.notify('draw', {
        ...
        value: {
          context: context,
          value: parent.transform.currFlipState,
          isReverse: null,
        },
      });
    }
    ...
    context.beginPath();
    const centerX: number = isNullOrUndefined(isSave)
      ? destLeft + destWidth / 2
      : context.canvas.width / 2;
    const centerY: number = isNullOrUndefined(isSave)
  ...
  private getCurrCropState(): string {
    ...
    if (flipState === '') {
      flipState =
        obj['flipColl'].length > 1
          ? this.getCurrFlipState()
  ...
  private updateRotatePan(): void {
    ...
    if (
      parent.rotateFlipColl.length > 0 &&
      typeof parent.rotateFlipColl[0] === 'number' &&
      degree < 0
    ) {
  ...
  private crop(obj: Object): void {
    ...
    if (!parent.disabled && parent.isImageLoaded) {
      ...
      const transitionArgs: CropEventArgs = {
        cancel: false,
  ...
  private cropEvent(
    ...
    if (!transitionArgs.cancel) {
      splitWords = parent.activeObj.shape
      ...
      if (
        ...
        (parent.currObjType.isCustomCrop ||
          (splitWords.length > 0 && splitWords[0] === 'crop'))
      ) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        const prevObj: CurrentObject = extend(
          {},
          this.prevCropCurrObj,
          {},
          true
        ) as CurrentObject;
        ...
        parent.notify('transform', {
          ...
          onPropertyChange: false,
          value: {
            width: parent.cropObj.destPoints.width,
            height: parent.cropObj.destPoints.height,
          },
        });
        const aspectIcon: HTMLInputElement = parent.element.querySelector(
        ...
        if (
          !object['isCropToolbar'] &&
          isNullOrUndefined(aspectIcon) &&
          isNullOrUndefined(nonAspectIcon)
        ) {
          parent.notify('toolbar', {
            prop: 'refresh-toolbar',
            onPropertyChange: false,
  ...
  private updateUndoRedoColl(
    ...
    const parent: ImageEditor = this.parent;
    const currSelPtObj: Object = {
      prevCurrSelectionPoint: parent.prevCurrSelectionPoint,
    };
    prevObj.currSelectionPoint = extend(
      {},
      currSelPtObj['prevCurrSelectionPoint'],
      {},
      true
    ) as SelectionPoint;
    parent.notify('undo-redo', {
  ...
  private resizeWrapper(): void {
    ...
    if (Browser.isDevice) {
      ...
      const ctxToolbar: HTMLElement = elem.querySelector(
        '#' + elem.id + '_contextualToolbarArea'
      ) as HTMLElement;
      if (
        ctxToolbar &&
        ctxToolbar.style.position === '' &&
        !this.isTransformCrop
      ) {
        ...
        parent.notify('filter', {
          ...
          value: { adjustmentValue: parent.canvasFilter },
        });
      }
    }
  }
  ...
  private getCurrFlipState(panObj?: Object): string {
    ...
    parent.notify('shape', {
      ...
      onPropertyChange: false,
      value: {
        collection: parent.rotateFlipColl,
        isRotateFlipCollection: true,
        obj: object,
      },
    });
    ...
    for (
      ...
      parent.notify('transform', {
        ...
        onPropertyChange: false,
        value: {
          region: obj['panRegion'],
          type: parent.rotateFlipColl[i as number],
          obj: obj,
        },
      });
    }
