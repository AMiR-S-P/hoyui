...
import { ListBase, ListBaseOptions } from '@hoyui/lists';
  ...
  implements INotifyPropertyChanged
{
  ...
  private initItems(): void {
    if (!this.items.length) {
      ...
      for (let i: number = 0; i < uri.length; i++) {
        if (uri[i as number]) {
          items.push({
            text: uri[i as number],
            url: baseUri + uri[i as number],
          });
  ...
  private renderItems(items: BreadcrumbItemModel[]): void {
    let item: BreadcrumbItemModel[] | object[];
    let isSingleLevel: boolean;
    ...
    if (itemsLength) {
      ...
      const ol: HTMLElement = this.createElement('ol', {
        className:
          this.overflowMode === 'Wrap' ? 'e-breadcrumb-wrapped-ol' : '',
      });
      const firstOl: HTMLElement = this.createElement('ol', {
        className: this.overflowMode === 'Wrap' ? 'e-breadcrumb-first-ol' : '',
      });
      const showIcon: boolean = this.hasField(items, 'iconCss');
      const isCollasped: boolean =
        this.overflowMode === 'Collapsed' &&
        this._maxItems > 0 &&
        itemsLength > this._maxItems &&
        !this.isExpanded;
      const isDefaultOverflowMode: boolean =
        this.overflowMode === 'Hidden' && this._maxItems > 0;
      ...
      const listBaseOptions: ListBaseOptions = {
        ...
        itemNavigable: true,
        itemCreated: (args: {
          curData: BreadcrumbItemModel;
          item: HTMLElement;
          fields: obj;
        }): void => {
          const isLastItem: boolean = (args.curData as { isLastItem: boolean })
            .isLastItem;
          ...
          const eventArgs: BreadcrumbBeforeItemRenderEventArgs = {
            item: extend(
              {},
              (args.curData as { properties: object }).properties
                ? (args.curData as { properties: object }).properties
                : args.curData
            ),
            element: args.item,
          ...
          const containsRightIcon: boolean =
            isIconRight || eventArgs.element.classList.contains(ICONRIGHT);
          if (containsRightIcon && args.curData.iconCss && !this.itemTemplate) {
            args.item
              .querySelector('.e-anchor-wrap')
              .appendChild(args.item.querySelector(DOT + ICONCLASS));
          }
          ...
          if (
            (eventArgs.item.disabled || this.disabled) &&
            args.item.children.length &&
            !this.itemTemplate
          ) {
          ...
          if ((args.curData as { isEmptyUrl: boolean }).isEmptyUrl) {
            ...
            if (
              (!isLastItem ||
                (isLastItem && this.enableActiveItemNavigation)) &&
              !(eventArgs.item.disabled || this.disabled)
            ) {
      ...
      for (let i: number = 0; i < len; i % 2 && j++, i++) {
        isActiveItem =
          this.activeItem &&
          (this.activeItem === items[j as number].url ||
            this.activeItem === items[j as number].text);
        if (isCollasped && i > 1 && i < len - 2) {
          continue;
        } else if (
          isDefaultOverflowMode &&
          (j < this.startIndex || j > this.endIndex) &&
          (i % 2 ? j !== this.startIndex - 1 : true) &&
          j !== 0
        ) {
          continue;
        }
        if (i % 2) {
          ...
          if (
            (this.separatorTemplate && this.separatorTemplate === '/') ||
            isNullOrUndefined(this.separatorTemplate)
          ) {
            listBaseOptions.template = initializeCSPTemplate(
              function (): string {
                return '/';
              }
            );
          } else {
            listBaseOptions.template = this.separatorTemplate as
              | string
              | Function;
          }
          ...
          if (!(item as BreadcrumbItemModel[])[0].url && !this.itemTemplate) {
            item = [
              extend({}, (item as BreadcrumbItemModel[])[0], {
                isEmptyUrl: true,
                url: '#',
              }),
            ];
          ...
          isLastItem =
            (isDefaultOverflowMode || this.overflowMode === 'Menu') &&
            j === this.endIndex;
          if (
            ((i === len - 1 || isLastItem) && !this.itemTemplate) ||
            isActiveItem
          ) {
        ...
        const lastPopupItemIdx: number =
          this.startIndex + this.endIndex - this._maxItems;
        if (
          this.overflowMode === 'Menu' &&
          ((j >= this.startIndex &&
            j <= lastPopupItemIdx &&
            (i % 2 ? !(j === lastPopupItemIdx) : true) &&
        ...
        const li: NodeList = ListBase.createList(
          this.createElement,
  ...
  private calculateMaxItems(): void {
    if (
      ...
      const liElems: HTMLElement[] = [].slice
        .call(this.element.children[0].children)
        .reverse();
      let liWidth: number =
        this.overflowMode === 'Menu'
          ? 0
          : liElems[liElems.length - 1].offsetWidth +
            (liElems[liElems.length - 2]
              ? liElems[liElems.length - 2].offsetWidth
              : 0);
      if (this.overflowMode === 'Menu') {
      ...
      for (let i: number = 0; i < liElems.length - 2; i++) {
        if (liWidth > width) {
          maxItems =
            Math.ceil((i - 1) / 2) +
            (this.overflowMode === 'Menu' && i <= 2 ? 0 : 1);
          if (
            ((this.maxItems > maxItems &&
              !(this.maxItems > -1 && maxItems === -1)) ||
              this.maxItems === -1) &&
          ...
          if (
            !(
              this.overflowMode === 'Menu' &&
              liElems[i as number].classList.contains(MENUCLASS)
            )
          ) {
      ...
      (this.overflowMode === 'Wrap' || this.overflowMode === 'Scroll') &&
      this._maxItems > 0
    ) {
      let width: number = 0;
      const liElems: NodeListOf<HTMLElement> = this.element.querySelectorAll(
        DOT + ITEMCLASS
      );
      if (liElems.length > this._maxItems + this._maxItems - 1) {
        for (
          let i: number = this.overflowMode === 'Wrap' ? 1 : 0;
          i < this._maxItems + this._maxItems - 1;
          i++
        ) {
        ...
        width =
          width +
          5 +
          parseInt(getComputedStyle(this.element.children[0]).paddingLeft, 10) *
            2;
        if (this.overflowMode === 'Wrap') {
          (
  ...
  private beforeItemRenderChanges(
    prevItem: BreadcrumbItemModel,
    currItem: BreadcrumbItemModel,
    elem: Element,
    isRightIcon: boolean
  ): void {
    ...
    if (currItem.iconCss !== prevItem.iconCss && wrapElem) {
      ...
      if (iconElem) {
        ...
        const iconElem: Element = this.createElement('span', {
  ...
  private reRenderItems(): void {
    if (
      this.overflowMode === 'Menu' &&
      this.popupObj &&
      this.popupObj.element.classList.contains('e-popup-open') &&
      this.popupObj.element.querySelector('.e-edit-template')
    ) {
      this.popupObj.hide();
  ...
  private clickHandler(e: MouseEvent): void {
    const li: Element = closest(
      e.target as Element,
      DOT + ITEMCLASS + ':not(.e-breadcrumb-separator)'
    );
    ...
    if (
      li &&
      (closest(e.target as Element, DOT + ITEMTEXTCLASS) || this.itemTemplate)
    ) {
      let idx: number;
      if (this.overflowMode === 'Wrap') {
        idx = [].slice
          .call(this.element.querySelectorAll(DOT + ITEMCLASS))
          .indexOf(li);
      } else {
      ...
      if (this.overflowMode === 'Menu') {
        if (closest(e.target as Element, DOT + POPUPCLASS)) {
          ...
          if (
            ...
            [].slice
              .call(this.element.children[0].children)
              .indexOf(this.element.querySelector(DOT + MENUCLASS))
          ) {
      ...
      const itemClickArgs: BreadcrumbClickEventArgs = {
        element: li as HTMLElement,
        item: this.items[idx as number],
        event: e,
        cancel: false,
      };
      ...
      if (this.items[idx as number]) {
        this.activeItem =
          this.items[idx as number].url || this.items[idx as number].text;
      }
      this.dataBind();
    }
    ...
    if (
      (e.target as Element).classList.contains(MENUCLASS) &&
      !this.isPopupCreated
    ) {
      this.renderPopup();
    }
  ...
  private renderPopup(): void {
    const wrapper: HTMLElement = this.createElement('div', {
      className:
        POPUPCLASS + ' ' + this.cssClass + (this.enableRtl ? ' e-rtl' : ''),
    });
  ...
  private documentClickHandler(e: Event): void {
    if (
      ...
      this.popupObj &&
      this.popupObj.element.classList.contains('e-popup-open') &&
      !closest(e.target as Element, DOT + MENUCLASS) &&
      !closest(e.target as Element, DOT + 'e-edit-template')
    ) {
      this.popupObj.hide();
      this.popupObj.destroy();
      this.isPopupCreated = false;
  ...
  public onPropertyChanged(
    newProp: BreadcrumbModel,
    oldProp: BreadcrumbModel
  ): void {
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        case 'cssClass':
          ...
          if (
            (oldProp.cssClass &&
              oldProp.cssClass.indexOf(ICONRIGHT) > -1 &&
              !(
                newProp.cssClass && newProp.cssClass.indexOf(ICONRIGHT) > -1
              )) ||
            (!(oldProp.cssClass && oldProp.cssClass.indexOf(ICONRIGHT) > -1) &&
              newProp.cssClass &&
              newProp.cssClass.indexOf(ICONRIGHT) > -1)
          ) {
            this.reRenderItems();
          }
          break;
