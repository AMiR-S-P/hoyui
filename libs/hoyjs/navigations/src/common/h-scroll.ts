...
import { classList, SwipeEventArgs, isNullOrUndefined } from '@hoyui/base';
  ...
  implements INotifyPropertyChanged
{
  ...
  private initialize(): void {
    const scrollEle: HTEle = this.createElement('div', {
      className: CLS_HSCROLLCON,
    });
    const scrollDiv: HTEle = this.createElement('div', {
  ...
  public destroy(): void {
    ...
    const nav: HTEle[] = selectAll(
      '.e-' + ele.id + '_nav.' + CLS_HSCROLLNAV,
      ele
    );
  ...
  public disable(value: boolean): void {
    const navEles: HTMLElement[] = selectAll(
      '.e-scroll-nav:not(.' + CLS_DISABLE + ')',
      this.element
    );
    if (value) {
  ...
  private createOverlay(element: HTEle): void {
    ...
    const rightOverlayEle: HTEle = this.createElement('div', {
      className: CLS_OVERLAY + ' ' + CLS_RIGHTOVERLAY,
    });
    const clsRight: string =
      'e-' +
      element.id.concat('_nav ' + CLS_HSCROLLNAV + ' ' + CLS_HSCROLLNAVRIGHT);
    const rightEle: HTEle = this.createElement('div', {
      id: id.concat('_right'),
      className: clsRight,
    });
    const navItem: HTEle = this.createElement('div', {
      className: CLS_NAVRIGHTARROW + ' ' + CLS_NAVARROW + ' e-icons',
    });
    rightEle.appendChild(navItem);
    const leftEle: HTEle = this.createElement('div', {
  ...
  private createNavIcon(element: HTEle): void {
    ...
    const clsRight: string =
      'e-' +
      element.id.concat('_nav ' + CLS_HSCROLLNAV + ' ' + CLS_HSCROLLNAVRIGHT);
    const rightAttributes: { [key: string]: string } = {
      role: 'button',
      id: id.concat('_right'),
      'aria-label': 'Scroll right',
    };
    const nav: HTEle = this.createElement('div', {
      className: clsRight,
    ...
    const clsLeft: string =
      'e-' +
      element.id.concat('_nav ' + CLS_HSCROLLNAV + ' ' + CLS_HSCROLLNAVLEFT);
    const leftAttributes: { [key: string]: string } = {
      role: 'button',
      id: id.concat('_left'),
      'aria-label': 'Scroll left',
    };
    const navEle: HTEle = this.createElement('div', {
      className: clsLeft + ' ' + CLS_DISABLE,
      attrs: leftAttributes,
    });
    navEle.setAttribute('aria-disabled', 'true');
    const navLeftItem: HTEle = this.createElement('div', {
  ...
  private tabHoldHandler(e: TapEventArgs): void {
    ...
    trgt = this.contains(trgt, CLS_HSCROLLNAV)
      ? <HTEle>trgt.firstElementChild
      : trgt;
  ...
  private frameScrollRequest(
    ...
    if (!this.customStep) {
      [].slice
        .call(selectAll('.' + CLS_OVERLAY, this.element))
  ...
  private arrowDisabling(addDisable: HTEle, removeDisable: HTEle): void {
    if (this.isDevice) {
      const arrowEle: HTMLElement = isNullOrUndefined(addDisable)
        ? removeDisable
        : addDisable;
      const arrowIcon: HTMLElement = arrowEle.querySelector(
        '.' + CLS_NAVARROW
      ) as HTMLElement;
  ...
  private scrollHandler(e: Event): void {
    ...
    const rootEle: HTEle = this.element;
    const navLeftEle: HTEle = <HTEle>(
      this.element.querySelector('.' + CLS_HSCROLLNAVLEFT)
    );
    const navRightEle: HTEle = <HTEle>(
      this.element.querySelector('.' + CLS_HSCROLLNAVRIGHT)
    );
    let leftOverlay: HTEle = <HTEle>(
      this.element.querySelector('.' + CLS_LEFTOVERLAY)
    );
    let rightOverlay: HTEle = <HTEle>(
      this.element.querySelector('.' + CLS_RIGHTOVERLAY)
    );
    ...
    if (this.isDevice) {
      ...
      if (target.scrollWidth - Math.ceil(width + scrollLeft) < OVERLAY_MAXWID) {
        rightOverlay.style.width =
          target.scrollWidth - Math.ceil(width + scrollLeft) + 'px';
      } else {
    ...
    if (scrollLeft === 0) {
      ...
      const disEle: HTEle = <HTEle>(
        this.element.querySelector('.' + CLS_HSCROLLNAV + '.' + CLS_DISABLE)
      );
      if (disEle) {
