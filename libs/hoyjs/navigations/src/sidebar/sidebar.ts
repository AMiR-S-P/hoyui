  ...
  implements INotifyPropertyChanged
{
  ...
  private documentClickContext: EventListenerObject =
  ...
  private setTarget(): void {
    ...
    if (typeof this.target === 'string') {
      this.setProperties(
        { target: <HTMLElement>document.querySelector(this.target) },
        true
      );
    }
    if (this.target) {
      (<HTMLElement>this.target).insertBefore(
        this.element,
  ...
  private getTargetElement(): HTMLElement {
    let siblingElement: HTMLElement = <HTMLElement>(
  ...
  private addClass(): void {
    ...
    const classELement: HTMLElement = <HTMLElement>(
      document.querySelector('.e-main-content')
    );
  ...
  private checkType(val: boolean): void {
    if (
      !(this.type === 'Push' || this.type === 'Over' || this.type === 'Slide')
    ) {
      this.type = 'Auto';
    } else {
  ...
  private destroyBackDrop(): void {
    const sibling: HTMLElement =
      <HTMLElement>document.querySelector('.e-main-content') || this.targetEle;
    if (
      this.target &&
      this.showBackdrop &&
      sibling &&
      !isNullOrUndefined(this.defaultBackdropDiv)
    ) {
  ...
  public hide(e?: Event): void {
    ...
    this.trigger('close', closeArguments, (observedcloseArgs: EventArgs) => {
      if (!observedcloseArgs.cancel) {
        ...
        this.setType(this.type);
        const sibling: HTMLElement =
          <HTMLElement>document.querySelector('.e-main-content') ||
          this.targetEle;
        if (!this.enableDock && sibling) {
          sibling.style.transform = 'translateX(' + 0 + 'px)';
          sibling.style[
            this.position === 'Left' ? 'marginLeft' : 'marginRight'
          ] = '0px';
        }
  ...
  private setTimeOut(): void {
    ...
    if (
      ...
      sibling &&
      !(this.type === 'Over' && this.enableDock)
    ) {
      if (this.position === 'Left') {
        sibling.style.marginLeft = this.setDimension(
          this.width === 'auto' ? elementWidth : this.width
        );
      } else {
        sibling.style.marginRight = this.setDimension(
          this.width === 'auto' ? elementWidth : this.width
        );
      }
      ...
      if (this.position === 'Left') {
        sibling.style.marginLeft = this.setDimension(
          this.dockSize === 'auto' ? elementWidth : this.dockSize
        );
      } else {
        sibling.style.marginRight = this.setDimension(
          this.dockSize === 'auto' ? elementWidth : this.dockSize
  ...
  private triggerChange(): void {
    const changeArguments: ChangeEventArgs = {
      name: 'change',
      element: this.element,
    };
    ...
    if (this.enableDock && this.position === 'Left' && !this.getState()) {
      setStyle(this.element, {
        transform:
          ...
          -100 +
          '%) translateX(' +
          this.setDimension(this.dockSize) +
          ')',
      });
    } else if (
      this.enableDock &&
  ...
  private enableGestureHandler(args: SwipeEventArgs): void {
    if (
      ...
      this.position === 'Right' &&
      args.swipeDirection === 'Left' &&
      window.innerWidth - args.startX <= 20 &&
      args.distanceX >= 50 &&
      args.velocity >= 0.5
    ) {
      this.show();
  ...
  private setEnableGestures(): void {
    if (this.enableGestures) {
      ...
      this.sidebarEle = new Touch(<HTMLElement>this.element, {
        swipe: this.enableGestureHandler.bind(this),
      });
    } else {
    ...
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        case 'position':
          ...
          if (sibling) {
            sibling.style[
              this.position === 'Left' ? 'marginRight' : 'marginLeft'
            ] = '0px';
          }
  ...
  protected setType(type?: string): void {
    ...
    const margin: string =
      this.position === 'Left' ? elementWidth + 'px' : elementWidth + 'px';
    const eleWidth: number =
      this.position === 'Left' ? elementWidth : -elementWidth;
    ...
    switch (type) {
      case 'Push':
        ...
        if (
          sibling &&
          (this.enableDock || this.element.classList.contains(OPEN))
        ) {
          sibling.style[
            this.position === 'Left' ? 'marginLeft' : 'marginRight'
          ] = margin;
      ...
      case 'Over':
        ...
        if (
          this.enableDock &&
          (this.element.classList.contains(CLOSE) || this.isOpen)
        ) {
          if (sibling) {
            sibling.style[
              this.position === 'Left' ? 'marginLeft' : 'marginRight'
