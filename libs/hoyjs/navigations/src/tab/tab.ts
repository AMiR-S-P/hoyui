...
import {
  ...
  SanitizeHtmlHelper,
  Draggable,
  DragEventArgs as DragArgs,
  DropEventArgs,
} from '@hoyui/base';
import { Base } from '@hoyui/base';
import { Popup, PopupModel } from '@hoyui/popups';
...
export class TabAnimationSettings extends ChildProperty<TabAnimationSettings> {
   ...
   */
  @Complex<TabActionSettingsModel>(
    { effect: 'SlideLeftIn', duration: 600, easing: 'ease' },
    TabActionSettings
  )
   ...
   */
  @Complex<TabActionSettingsModel>(
    { effect: 'SlideRightIn', duration: 600, easing: 'ease' },
    TabActionSettings
  )
  ...
  implements INotifyPropertyChanged
{
  ...
  private tbId: string;
  private resizeContext: EventListenerObject =
    this.refreshActiveTabBorder.bind(this);
  /**
  ...
  protected preRender(): void {
    ...
    if (this.allowDragAndDrop) {
      this.dragArea = !isNOU(this.dragArea)
        ? this.dragArea
        : '#' + this.element.id + ' ' + ('.' + CLS_HEADER);
    }
    if (!isNOU(nested)) {
  ...
  protected render(): void {
    ...
    this.tabId =
      this.element.id.length > 0 ? '-' + this.element.id : getRandomId();
    this.renderContainer();
    this.wireEvents();
    this.initRender = false;
    if (
      this.isReact &&
      (this as Record<string, any>).portals &&
      (this as Record<string, any>).portals.length > 0
    ) {
      this.renderReactTemplates(() => {
        if (!isNOU(this.tbObj)) {
  ...
  private renderContainer(): void {
    ...
    this.items.forEach((item: TabItemModel, index: number) => {
      if (isNOU(item.id) && !isNOU((item as Base<HTMLElement>).setProperties)) {
        (item as Base<HTMLElement>).setProperties(
    ...
    if (this.items.length > 0 && ele.children.length === 0) {
      ...
      this.setOrientation(
        ...
        this.createElement('div', { className: CLS_HEADER })
      );
    ...
    if (
      !isNOU(select('.' + CLS_HEADER, this.element)) &&
      !isNOU(select('.' + CLS_CONTENT, this.element))
    ) {
      this.renderHeader();
      this.tbItems = <HTEle>(
        select('.' + CLS_HEADER + ' .' + CLS_TB_ITEMS, this.element)
      );
      ...
      if (selectAll('.' + CLS_TB_ITEM, this.element).length > 0) {
        ...
        const scrCnt: HTEle = <HTEle>(
          select('.' + this.scrCntClass, this.tbItems)
        );
        if (!isNOU(scrCnt)) {
  ...
  private renderHeader(): void {
    ...
    if (!this.isTemplate) {
      ...
      if (
        ...
        this.element.children[1].classList.contains(CLS_HEADER)
      ) {
      ...
      if (count > 0) {
        const tabItems: HTMLElement = this.createElement('div', {
          className: CLS_ITEMS,
        });
    ...
    const toolbarHeader: HTEle = this.tbObj.element.querySelector(
      '.' + CLS_TB_ITEMS
    );
  ...
  private parseObject(items: TabItemModel[], index: number): object[] {
    const tbItems: HTMLElement[] = Array.prototype.slice.call(
      selectAll('.e-tab-header .' + CLS_TB_ITEM, this.element)
    );
    ...
    items.forEach((item: TabItemModel, i: number) => {
      const pos: Str =
        isNOU(item.header) || isNOU(item.header.iconPosition)
          ? ''
          : item.header.iconPosition;
      const css: Str =
        isNOU(item.header) || isNOU(item.header.iconCss)
          ? ''
          : item.header.iconCss;
      if (
        isNOU(item.headerTemplate) &&
        (isNOU(item.header) ||
          isNOU(item.header.text) ||
      ...
      if (this.isReplace && !isNOU(this.tbId) && this.tbId !== '') {
        itemIndex = parseInt(
          this.tbId.substring(this.tbId.lastIndexOf('_') + 1),
          10
        );
        this.tbId = '';
      } else {
      ...
      this.lastIndex =
        tbItems.length === 0 ? i : this.isReplace ? itemIndex : maxId + 1 + i;
      const disabled: Str = item.disabled
        ? ' ' + CLS_DISABLE + ' ' + CLS_OVERLAY
        : '';
      ...
      const tCont: HTEle = this.createElement('div', {
        className: CLS_TEXT_WRAP,
      });
      ...
      const tabIndex: string = isNOU(item.tabIndex)
        ? '-1'
        : item.tabIndex.toString();
      const wrapAttrs: { [key: string]: string } = item.disabled
      ...
      if (this.itemIndexArray instanceof Array) {
        this.itemIndexArray.splice(
          index + i,
          0,
          CLS_ITEM + this.tabId + '_' + this.lastIndex
        );
      }
      ...
      tItem.cssClass =
        (item.cssClass !== undefined ? item.cssClass : ' ') +
        ' ' +
        disabled +
        ' ' +
        hidden +
  ...
  private removeActiveClass(): void {
    ...
    if (tabHeader) {
      const tabItems: HTMLElement[] = selectAll(
        '.' + CLS_TB_ITEM + '.' + CLS_ACTIVE,
        tabHeader
      );
      [].slice
        .call(tabItems)
        .forEach((node: HTMLElement) => node.classList.remove(CLS_ACTIVE));
      [].slice
        .call(tabItems)
        .forEach((node: HTMLElement) =>
          node.firstElementChild.setAttribute('aria-selected', 'false')
        );
    }
  }
  private checkPopupOverflow(ele: HTEle): boolean {
    ...
    if (
      !this.isVertical() &&
      ((this.enableRtl &&
        popIcon.offsetLeft + popIcon.offsetWidth > tbrItems.offsetLeft) ||
        (!this.enableRtl && popIcon.offsetLeft < tbrItems.offsetWidth))
    ) {
      isOverflow = true;
    } else if (
      this.isVertical() &&
      popIcon.offsetTop < lastChild.offsetTop + lastChild.offsetHeight
    ) {
      isOverflow = true;
    }
    if (isOverflow) {
      ele.classList.add(CLS_TB_POPUP);
      this.tbPop.insertBefore(
        <Node>ele,
        selectAll('.' + CLS_TB_POPUP, this.tbPop)[0]
      );
    }
    return true;
  }
  private popupHandler(target: HTEle): number {
    ...
    if (this.tbItem.length !== 0) {
      ...
      if (this.checkPopupOverflow(lastChild)) {
        const prevEle: HTEle = <HTEle>(
          (<HTEle>this.tbItems.lastChild).previousElementSibling
        );
  ...
  private triggerPrevAnimation(oldCnt: HTEle, prevIndex: number): void {
    const animateObj: AnimationModel = this.prevCtnAnimation(
      prevIndex,
      this.selectedItem
    );
    if (!isNOU(animateObj)) {
  ...
  private triggerAnimation(id: Str, value: boolean): void {
    ...
    let oldCnt: HTEle;
    const itemCollection: HTMLElement[] = [].slice.call(
      this.element.querySelector('.' + CLS_CONTENT).children
    );
    ...
    const isPrevent: boolean =
      isNOU(this.animation) ||
      isNOU(this.animation.next.effect) ||
      isNOU(this.animation.previous.effect) ||
      this.animation.previous.effect === 'None' ||
      this.animation.next.effect === 'None';
    ...
    if (this.prevIndex > this.selectedItem && !this.isPopup) {
      ...
      animateObj = {
        name: <Effect>(
          (openEff === <Effect>'None'
            ? ''
            : openEff !== <Effect>'SlideLeftIn'
            ? openEff
            : 'SlideLeftIn')
        ),
        duration:
          this.animation.previous.duration === 0 && animationMode === 'Enable'
            ? 600
            : this.animation.previous.duration,
        timingFunction: this.animation.previous.easing,
      };
    } else if (
      this.isPopup ||
      ...
      animateObj = {
        name: <Effect>(
          (clsEff === <Effect>'None'
            ? ''
            : clsEff !== <Effect>'SlideRightIn'
            ? clsEff
            : 'SlideRightIn')
        ),
        duration:
          this.animation.next.duration === 0 && animationMode === 'Enable'
            ? 600
            : this.animation.next.duration,
        timingFunction: this.animation.next.easing,
      };
    }
    animateObj.progress = () => {
  ...
  private keyPressed(trg: HTEle): void {
    const trgParent: HTEle = <HTEle>(
      closest(trg, '.' + CLS_HEADER + ' .' + CLS_TB_ITEM)
    );
    const trgIndex: number = this.getEleIndex(trgParent);
    if (!isNOU(this.popEle) && trg.classList.contains('e-hor-nav')) {
      this.popEle.classList.contains(CLS_POPUP_OPEN)
      ...
      if (
        !isNOU(trgParent) &&
        trgParent.classList.contains(CLS_ACTIVE) === false
      ) {
  ...
  private getTabHeader(): HTMLElement {
    ...
    const headers: HTMLElement[] = [].slice
      .call(this.element.children)
      .filter((e: HTMLElement) => e.classList.contains(CLS_HEADER));
    if (headers.length > 0) {
      return headers[0];
    } else {
      const wrap: HTMLElement = [].slice
        .call(this.element.children)
        .filter((e: HTMLElement) => !e.classList.contains(CLS_BLA_TEM))[0];
      if (!wrap) {
        return undefined;
      }
      return [].slice
        .call(wrap.children)
        .filter((e: HTMLElement) => e.classList.contains(CLS_HEADER))[0];
    }
  }
  private getEleIndex(item: HTEle): number {
    return Array.prototype.indexOf.call(
      selectAll('.' + CLS_TB_ITEM, this.getTabHeader()),
      item
    );
  }
  ...
  private expTemplateContent(): void {
    this.templateEle.forEach((eleStr: Str): void => {
      if (!isNOU(this.element.querySelector(eleStr))) {
        (<HTEle>(
          document.body.appendChild(this.element.querySelector(eleStr))
        )).style.display = 'none';
      }
    });
  }
  ...
  private compileElement(
    ...
    if (typeof val === 'string') {
      ...
      if (this.isVue) {
        templateFn = compile(
          this.enableHtmlSanitizer ? SanitizeHtmlHelper.sanitize(val) : val
        );
      } else {
        ele.innerHTML = this.enableHtmlSanitizer
          ? SanitizeHtmlHelper.sanitize(val)
          : val;
      }
    } else {
  ...
  private isVertical(): boolean {
    const isVertical: boolean =
      this.headerPlacement === 'Left' || this.headerPlacement === 'Right'
        ? true
        : false;
  ...
  private addVerticalClass(): void {
    if (this.isVertical()) {
      const tbPos: string =
        this.headerPlacement === 'Left' ? CLS_VLEFT : CLS_VRIGHT;
  ...
  private changeOrientation(place: Str): void {
    ...
    const isVertical: boolean = this.hdrEle.classList.contains(CLS_VERTICAL)
      ? true
      : false;
  ...
  private focusItem(): void {
    const curActItem: HTEle = <HTEle>(
      select(
        ' #' + CLS_ITEM + this.tabId + '_' + this.selectedItem,
        this.hdrEle
      )
    );
    if (!isNOU(curActItem)) {
  ...
  private setOrientation(place: Str, ele: HTEle): void {
    const headerPos: number = Array.prototype.indexOf.call(
      this.element.children,
      ele
    );
    const contentPos: number = Array.prototype.indexOf.call(
      this.element.children,
      this.element.querySelector('.' + CLS_CONTENT)
    );
  ...
  private setContentHeight(val: boolean): void {
    ...
    if (this.heightAdjustMode === 'None') {
      ...
      this.cntEle.style.height =
        'calc(100% - ' + this.hdrEle.offsetHeight + 'px)';
    } else if (this.heightAdjustMode === 'Auto') {
      if (this.isTemplate === true) {
        const cnt: HTEle[] = selectAll(
          '.' + CLS_CONTENT + ' > .' + CLS_ITEM,
          this.element
        );
  ...
  private setActiveBorder(): void {
    ...
    const trg: HTEle = <HTEle>(
      select('.' + CLS_TB_ITEM + '.' + CLS_ACTIVE, trgHdrEle)
    );
    if (isNOU(trg)) {
      return;
    }
    if (!this.reorderActiveTab) {
      if (
        trg.classList.contains(CLS_TB_POPUP) &&
        !this.bdrLine.classList.contains(CLS_HIDDEN)
      ) {
    ...
    const scrollCnt: HTEle = <HTEle>(
      select('.' + CLS_TB_ITEMS + ' .' + this.scrCntClass, trgHdrEle)
    );
    if (this.isVertical()) {
      setStyle(bar, { left: '', right: '' });
      const tbHeight: number = isNOU(scrollCnt)
        ? this.tbItems.offsetHeight
        : scrollCnt.offsetHeight;
      if (tbHeight !== 0) {
        setStyle(bar, {
      ...
      if (this.overflowMode === 'MultiRow') {
        const top: number =
          this.headerPlacement === 'Bottom'
            ? trg.offsetTop
            : trg.offsetHeight + trg.offsetTop;
        setStyle(bar, { top: top + 'px', height: '' });
      } else {
        setStyle(bar, { top: '', height: '' });
      }
      let tbWidth: number = isNOU(scrollCnt)
        ? this.tbItems.offsetWidth
        : scrollCnt.offsetWidth;
      if (tbWidth !== 0) {
        setStyle(bar, {
  ...
  private setActive(
    ...
    if (
      value < 0 ||
      isNaN(value) ||
      this.tbItem.length === 0 ||
      (!isNOU(trg) && trg.classList.contains(CLS_DISABLE))
    ) {
      return;
    }
    ...
    if (this.isTemplate) {
      if (select('.' + CLS_CONTENT, this.element).children.length > 0) {
        const trg: HTEle = this.findEle(
          select('.' + CLS_CONTENT, this.element).children,
          CLS_CONTENT + this.tabId + '_' + no
        );
        if (!isNOU(trg)) {
          trg.classList.add(CLS_ACTIVE);
        }
        ...
        this.cntEle.appendChild(
          this.createElement('div', {
            ...
            attrs: {
              role: 'tabpanel',
              'aria-labelledby':
                CLS_ITEM + this.tabId + '_' + this.extIndex(id),
            },
          })
        );
        const eleTrg: HTEle = this.getTrgContent(
          this.cntEle,
          this.extIndex(id)
        );
        const itemIndex: number = Array.prototype.indexOf.call(
          this.itemIndexArray,
          id
        );
        this.getContent(
    ...
    if (!this.initRender && !skipDataBind) {
      const eventArg: SelectEventArgs = {
        ...
        selectedItem: trg,
        selectedIndex: value,
        selectedContent: <HTEle>(
          select(
            '#' + CLS_CONTENT + this.tabId + '_' + this.selectingID,
            this.content
          )
        ),
  ...
  private showPopup(config: object): void {
    ...
    if (tbPop && tbPop.classList.contains('e-popup-close')) {
      const tbPopObj: Popup = (<PopupModel>(
        (tbPop && (<Instance>tbPop).ej2_instances[0])
      )) as Popup;
      tbPopObj.position.X =
        this.headerPlacement === 'Left' ||
        this.element.classList.contains(CLS_RTL)
          ? 'left'
          : 'right';
      tbPopObj.dataBind();
  ...
  private bindDraggable(): void {
    if (this.allowDragAndDrop) {
      ...
      if (tabHeader) {
        const items: NodeList = Array.prototype.slice.call(
          tabHeader.querySelectorAll('.' + CLS_TB_ITEM)
        );
  ...
  private swipeHandler(e: SwipeEventArgs): void {
    ...
    const nestedTab: HTMLElement = this.element.querySelector(
      '[data-swipe="true"]'
    );
    if (nestedTab) {
    ...
    if (e.swipeDirection === 'Right' && this.selectedItem !== 0) {
      ...
      this.selectedItem !==
        selectAll('.' + CLS_TB_ITEM, this.element).length - 1
    ) {
  ...
  private spaceKeyDown(e: KeyboardEvent): void {
    if (
      ...
      (e.keyCode === 35 && e.which === 35)
    ) {
      const clstHead: HTEle = <HTEle>(
        closest(<Element>e.target, '.' + CLS_HEADER)
      );
      if (!isNOU(clstHead)) {
        e.preventDefault();
      }
  ...
  private keyHandler(e: KeyboardEventArgs): void {
    ...
    const item: HTEle = <HTEle>(
      closest(document.activeElement, '.' + CLS_TB_ITEM)
    );
    const trgParent: HTEle = <HTEle>closest(trg, '.' + CLS_TB_ITEM);
    switch (e.action) {
      case 'space':
      ...
      case 'shiftTab':
        if (
          trg.classList.contains(CLS_WRAP) &&
          (<HTEle>closest(trg, '.' + CLS_TB_ITEM)).classList.contains(
            CLS_ACTIVE
          ) === false
        ) {
        ...
        if (
          !isNOU(actEle) &&
          actEle.children.item(0).getAttribute('tabindex') === '-1'
        ) {
      ...
      case 'openPopup':
        ...
        if (
          !isNOU(this.popEle) &&
          this.popEle.classList.contains(CLS_POPUP_CLOSE)
        ) {
          this.popObj.show(this.show);
        }
        break;
    ...
    if (!(newProp.items instanceof Array && oldProp.items instanceof Array)) {
      ...
      for (let i: number = 0; i < changedProp.length; i++) {
        ...
        for (let j: number = 0; j < properties.length; j++) {
          ...
          const newVal: Str | Object = Object(newProp.items[index])[
            properties[j]
          ];
          const hdr: HTEle = <HTEle>(
            this.element.querySelectorAll('.' + CLS_TB_ITEM)[index]
          );
          let itemIndex: number;
          ...
          if (
            properties[j] === 'header' ||
            properties[j] === 'headerTemplate'
          ) {
            const icon: Str | Object =
              isNOU(this.items[index].header) ||
              isNOU(this.items[index].header.iconCss)
                ? ''
                : this.items[index].header.iconCss;
            const textVal: Str | Object =
              this.items[index].headerTemplate || this.items[index].header.text;
            ...
            if (textVal === '' && icon === '') {
              ...
              const isHiddenEle: boolean =
                hdrItem.classList.contains(CLS_HIDDEN);
              detach(hdrItem);
              this.isReplace = true;
          ...
          if (properties[j] === 'content' && !isNOU(cntItem)) {
            const strVal: boolean =
              typeof newVal === 'string' || isNOU((<HTEle>newVal).innerHTML);
            if (
              strVal &&
              ((<Str>newVal)[0] === '.' || (<Str>newVal)[0] === '#') &&
              (<Str>newVal).length
            ) {
              ...
              (<HTEle>(
                document.body.appendChild(this.element.querySelector(oldVal))
              )).style.display = 'none';
      ...
      if (isNOU(this.tbObj)) {
        ...
        this.templateEle = [];
        const selectElement: HTEle = <HTEle>(
          select('.' + CLS_TAB + ' > .' + CLS_CONTENT, this.element)
        );
  ...
  private clearTabTemplate(
    ...
    if (
      (this as Record<string, any>).registeredTemplate &&
      (this as Record<string, any>).registeredTemplate[templateName]
    ) {
      const registeredTemplates: Record<string, any> = (
        this as Record<string, any>
      ).registeredTemplate;
      for (
        let index = 0;
        index < registeredTemplates[templateName].length;
        index++
      ) {
        const registeredItem =
          registeredTemplates[templateName][index].rootNodes[0];
        const closestItem = closest(registeredItem, '.' + className);
        if (!isNullOrUndefined(closestItem) && closestItem === templateEle) {
          this.clearTemplate(
            [templateName],
            [registeredTemplates[templateName][index]]
          );
          break;
        }
      }
      ...
      const portals: Record<string, any>[] = (this as Record<string, any>)
        .portals;
  ...
  private initializeDrag(target: HTEle): void {
    let dragObj: Draggable = new Draggable(target, {
      ...
      drag: (e: DragArgs) => {
        ...
        if (
          !isNOU(e.target.closest('.' + CLS_TAB)) &&
          !e.target.closest('.' + CLS_TAB).isEqualNode(this.element) &&
          this.dragArea !== '.' + CLS_HEADER
        ) {
          this.trigger('dragging', dragArgs);
        } else {
          if (
            !e.target.closest(this.dragArea) &&
            ...
            (<HTEle>(
              this.dragItem.querySelector('.' + CLS_WRAP)
          ...
          if (
            ...
            !isNOU(this.element.querySelector('.e-hscroll'))
          ) {
            let scrollRightNavEle: HTMLElement = this.element.querySelector(
              '.e-scroll-right-nav'
            );
            let scrollLeftNavEle: HTMLElement =
              this.element.querySelector('.e-scroll-left-nav');
            let hscrollBar: HTMLElement =
              this.element.querySelector('.e-hscroll-bar');
            if (
              !isNOU(scrollRightNavEle) &&
              Math.abs(
                scrollRightNavEle.offsetWidth / 2 + scrollRightNavEle.offsetLeft
              ) >
                this.cloneElement.offsetLeft + this.cloneElement.offsetWidth
            ) {
              hscrollBar.scrollLeft -= 10;
            }
            if (
              !isNOU(scrollLeftNavEle) &&
              Math.abs(
                scrollLeftNavEle.offsetLeft +
                  scrollLeftNavEle.offsetWidth -
                  this.cloneElement.offsetLeft
              ) >
                scrollLeftNavEle.offsetWidth / 2
            ) {
              hscrollBar.scrollLeft += 10;
            }
          }
          ...
          if (
            ...
            scrollContentWidth = (<HTMLElement>(
              this.element.querySelector('.e-hscroll-content')
            )).offsetWidth;
          }
          if (
            dropItem != null &&
            !dropItem.isSameNode(this.dragItem) &&
            dropItem
              .closest('.' + CLS_TAB)
              .isSameNode(this.dragItem.closest('.' + CLS_TAB))
          ) {
            ...
            if (
              ...
              Math.abs(
                dropItem.offsetLeft +
                  dropItem.offsetWidth -
                  this.cloneElement.offsetLeft
              ) >
                dropItem.offsetWidth / 2
            ) {
              this.dragAction(dropItem, dragIndex, dropIndex);
            }
            if (
              dropIndex > dragIndex &&
              Math.abs(dropItem.offsetWidth / 2) +
                dropItem.offsetLeft -
                scrollContentWidth <
                this.cloneElement.offsetLeft + this.cloneElement.offsetWidth
            ) {
  ...
  private helper(e: {
    ...
    if (e.element) {
      ...
      if (
        this.element
          .querySelector('.' + CLS_HEADER)
          .classList.contains(CLS_CLOSE_SHOW)
      ) {
        addClass([this.cloneElement], CLS_CLOSE_SHOW);
      }
      removeClass(
        [this.cloneElement.querySelector('.' + CLS_WRAP)],
        'e-ripple'
      );
  ...
  private itemDragStart(e: DragArgs): void {
    ...
    this.trigger('onDragStart', dragArgs, (tabItemDragArgs: DragEventArgs) => {
      if (tabItemDragArgs.cancel) {
        const dragObj: Draggable = (e.element as EJ2Instance)
  ...
  private dragAction(
    ...
    if (this.overflowMode === 'MultiRow') {
      dropItem.parentNode.insertBefore(
        this.dragItem,
        dropItem.nextElementSibling
      );
    }
      ...
      if (!this.dragItem.parentElement.isSameNode(dropItem.parentElement)) {
        if (this.overflowMode === 'Extended') {
          if (dropItem.isSameNode(dropItem.parentElement.lastChild)) {
            ...
            popupContainer.insertBefore(
              dropItem.parentElement.lastChild,
              popupContainer.childNodes[0]
            );
          } else {
            this.dragItem.parentNode.insertBefore(
              dropItem.parentElement.lastChild,
              this.dragItem.parentElement.childNodes[0]
            );
            dropItem.parentNode.insertBefore(this.dragItem, dropItem);
          }
        } else {
    ...
    if (dragsIndex < dropIndex) {
      if (!this.dragItem.parentElement.isSameNode(dropItem.parentElement)) {
        if (this.overflowMode === 'Extended') {
          this.dragItem.parentElement.appendChild(
            dropItem.parentElement.firstElementChild
          );
    ...
    this.trigger('dragged', dragStopArgs, (tabItemDropArgs: DragEventArgs) => {
      if (tabItemDropArgs.cancel) {
        ...
        if (this.items.length > 0 && this.draggingItems.length > 0) {
          ...
          removeClass(
            [<HTEle>this.tbItems.querySelector('.' + CLS_INDICATOR)],
            CLS_HIDDEN
          );
  ...
  public enableTab(index: number, value: boolean): void {
    ...
    if (value === true) {
      ...
      (<HTEle>tbItems.firstElementChild).setAttribute(
        'tabindex',
        (<HTEle>tbItems.firstElementChild).getAttribute('data-tabindex')
      );
    } else {
    ...
    tbItems.firstElementChild.setAttribute(
      'aria-disabled',
  ...
  private addingTabContent(items: TabItemModel[], index?: number): void {
    ...
    if (isNOU(this.hdrEle)) {
      ...
      const tbItems: HTMLElement[] = Array.from(
        selectAll('.e-tab-header .' + CLS_TB_ITEM, this.element)
      );
      ...
      items.forEach((item: TabItemModel, place: number) => {
        ...
        if (
          !(
            isNOU(item.headerTemplate || item.header) ||
            isNOU(textValue) ||
            ((<string>textValue).length === 0 &&
              !isNOU(item.header) &&
              isNOU(item.header.iconCss))
          )
        ) {
          if (tabItems[place]) {
            if (isNOU(item.id)) {
              item.id =
                ...
                '_' +
                TABITEMPREFIX +
                (lastEleIndex + place).toString();
            }
            (tabItems[place] as Record<string, any>).htmlAttributes['data-id'] =
              item.id;
          }
        ...
        if (
          ...
          const ele: HTEle = this.createElement('div', {
            id: CLS_CONTENT + this.tabId + '_' + no,
            className: CLS_ITEM,
  ...
  public removeTab(index: number): void {
    ...
    const removeArgs: RemoveEventArgs = {
      removedItem: trg,
      removedIndex: index,
      cancel: false,
    };
    this.trigger('removing', removeArgs, (tabRemovingArgs: RemoveEventArgs) => {
      if (!tabRemovingArgs.cancel) {
        const header: HTEle = select(
          '#' + CLS_ITEM + this.tabId + '_' + this.extIndex(trg.id),
          select('.' + CLS_TB_ITEMS, this.element)
        );
        if (!isNOU(header)) {
        ...
        this.refreshActiveBorder();
        const cntTrg: HTEle = <HTEle>(
          select(
            '#' + CLS_CONTENT + this.tabId + '_' + this.extIndex(trg.id),
        ...
        if (trg.classList.contains(CLS_ACTIVE)) {
          ...
          if (index < this.selectedItem) {
            index = this.itemIndexArray.indexOf(
              this.tbItem[this.selectedItem].id
            );
            this.setProperties(
  ...
  public select(args: number | HTEle, event?: Event): void {
    ...
    if (
      isNOU(this.selectedItem) ||
      this.selectedItem < 0 ||
      this.tbItem.length <= this.selectedItem ||
      isNaN(this.selectedItem)
    ) {
      this.selectedItem = 0;
    } else {
    ...
    if (
      !isNOU(this.prevItem) &&
      !this.prevItem.classList.contains(CLS_DISABLE)
    ) {
      this.prevItem.children
        .item(0)
        .setAttribute(
          'tabindex',
          this.prevItem.firstElementChild.getAttribute('tabindex')
        );
    }
    const eventArg: SelectingEventArgs = {
      ...
      selectedContent: !isNOU(this.content)
        ? <HTEle>(
            select(
              '#' + CLS_CONTENT + this.tabId + '_' + this.selectedID,
              this.content
            )
          )
        : null,
  ...
  private selectingContent(args: number | HTEle, isInteracted?: boolean): void {
    if (typeof args === 'number') {
      if (
        !isNOU(this.tbItem[args]) &&
        (this.tbItem[<number>args].classList.contains(CLS_DISABLE) ||
      ...
      if (this.tbItem.length > args && args >= 0 && !isNaN(args)) {
        ...
        this.prevItem = this.tbItem[this.prevIndex];
        if (
          this.tbItem[args].classList.contains(CLS_TB_POPUP) &&
          this.reorderActiveTab
        ) {
          this.setActive(
            this.popupHandler(this.tbItem[args]),
            null,
            isInteracted
          );
          if (
    ...
    for (const prop of sortedKeys) {
      switch (prop) {
        ...
        case 'height':
          ...
          break;
        case 'cssClass':
          const headerEle: HTMLElement = this.element.querySelector(
            '.' + CLS_HEADER
          );
  ...
  public refreshActiveTab(): void {
    ...
    if (!this.isTemplate) {
      if (this.element.querySelector('.' + CLS_TB_ITEM + '.' + CLS_ACTIVE)) {
        detach(
          this.element.querySelector('.' + CLS_TB_ITEM + '.' + CLS_ACTIVE)
            .children[0]
        );
        detach(
          this.element
            .querySelector('.' + CLS_CONTENT)
            .querySelector('.' + CLS_ACTIVE).children[0]
        );
        ...
        const css: Str =
          isNOU(item.header) || isNOU(item.header.iconCss)
            ? ''
            : item.header.iconCss;
        const text: Str | HTEle | Function =
          item.headerTemplate || item.header.text;
        const txtWrap: HTEle = this.createElement('div', {
          className: CLS_TEXT,
          attrs: { role: 'presentation' },
        });
        ...
        const tConts: HTEle = this.createElement('div', {
          className: CLS_TEXT_WRAP,
        });
        ...
        const tabIndex: string = isNOU(item.tabIndex)
          ? '-1'
          : item.tabIndex.toString();
        const wrapAtt: { [key: string]: string } = item.disabled
          ? {}
          : {
              tabIndex: tabIndex,
              'data-tabindex': tabIndex,
        ...
        const wraper: HTEle = this.createElement('div', {
          className: CLS_WRAP,
          attrs: wrapAtt,
        });
        ...
        this.element
          .querySelector('.' + CLS_TB_ITEM + '.' + CLS_ACTIVE)
          .appendChild(wraper);
        const crElem: HTEle = this.createElement('div');
        let cnt: string | HTMLElement | Function = item.content;
        let eleStr: string;
        if (typeof cnt === 'string' || isNOU((<HTEle>cnt).innerHTML)) {
          if (typeof cnt === 'string' && this.enableHtmlSanitizer) {
            cnt = SanitizeHtmlHelper.sanitize(<Str>cnt);
        ...
        this.element
          .querySelector('.' + CLS_ITEM + '.' + CLS_ACTIVE)
          .appendChild(crElem);
      }
    } else {
      const tabItems: HTMLElement = this.element.querySelector(
        '.' + CLS_TB_ITEMS
      );
      ...
      const detachContent: Element = this.element
        .querySelector('.' + CLS_CONTENT)
        .querySelector('.' + CLS_ACTIVE).children[0];
      ...
      const tabIndex: string =
        element.firstElementChild.getAttribute('data-tabindex');
      const wrap: HTEle = this.createElement('div', {
        className: CLS_WRAP,
        innerHTML: conte,
        attrs: {
      ...
      tabItems.insertBefore(
        this.createElement('div', attr),
        tabItems.children[index + 1]
      );
      this.element
        .querySelector('.' + CLS_TB_ITEM + '.' + CLS_ACTIVE)
        .appendChild(wrap);
  ...
  public refreshActiveTabBorder(): void {
    const activeEle: Element = select(
      '.' + CLS_TB_ITEM + '.' + CLS_TB_POPUP + '.' + CLS_ACTIVE,
      this.element
    );
