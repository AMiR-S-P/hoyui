...
import { isVisible, classList, createElement } from '@hoyui/base';
...
describe('Accordion Testing', () => {
  ...
  describe('Accordion DOM class Testing ', () => {
    ...
    it('Accordion Item Content DOM class Testing', () => {
      ...
      accordion = new Accordion(
        { items: [{ header: 'Item1', content: 'Content of Item1' }] },
        ele
      );
      let eleItem: HTMLElement = <HTMLElement>ele.children[0];
    ...
    it('Accordion Item Header with icon DOM class Testing', () => {
      ...
      expect(eleItem.firstElementChild.classList.contains(CLS_HEADERCTN)).toBe(
        true
      );
      expect(eleItem.children[1].classList.contains(CLS_TOOGLEICN)).toBe(true);
      expect(
        eleItem.children[1].firstElementChild.classList.contains(
          CLS_COLLAPSEICN
        )
      ).toBe(true);
      expect(
        eleItem.children[1].firstElementChild.classList.contains('e-icons')
      ).toBe(true);
    });
  });
  describe('Accordion Item property behavior Testing ', () => {
    ...
    it('Accordion Item property Testing', () => {
      ...
      expect(
        ele.querySelectorAll('.' + CLS_ITEM).length === accordion.items.length
      ).toBe(true);
    });
    ...
    it('AccordionItem  Content property  Testing', () => {
      ...
      let headerEle: HTMLElement = <HTMLElement>ele.children[0].children[0];
      expect(
        headerEle.children[1] === ele.querySelector('.' + CLS_TOOGLEICN)
      ).toBe(true);
      expect(headerEle.firstChild.textContent === 'Item1').toBe(true);
      expect(
        headerEle.nextElementSibling === ele.querySelector('.' + CLS_CONTENT)
      ).toBe(true);
    });
    it('AccordionItem  Content without header property  Testing', () => {
      let ele: HTMLElement = document.getElementById('accordion');
      accordion = new Accordion(
        { items: [{ content: 'Content of Item1' }] },
        ele
      );
      let headerEle: HTMLElement = <HTMLElement>ele.children[0].children[0];
      expect(headerEle.childElementCount).toBe(1);
      expect(
        headerEle.firstElementChild === ele.querySelector('.' + CLS_TOOGLEICN)
      ).toBe(true);
    ...
    it('AccordionItem  iconCSS property  Testing', () => {
      ...
      expect(
        (<HTMLElement>headerEle.firstChild).classList.contains(CLS_HEADERICN)
      ).toBe(true);
      expect(
        headerEle.firstChild.firstChild === ele.querySelector('span.e-itemIcon')
      ).toBe(true);
    });
    it('AccordionItem  iconCSS property  Testing', () => {
      let ele: HTMLElement = document.getElementById('accordion');
      accordion = new Accordion(
        { items: [{ iconCss: 'e-itemIcon', header: 'item1' }] },
        ele
      );
      let headerEle: HTMLElement = <HTMLElement>ele.children[0].children[0];
      ...
      expect(headerEle.firstChild.nextSibling.textContent === 'item1').toBe(
        true
      );
    });
    it('AccordionItem  iconCSS property  Testing', () => {
          ...
          items: [
            {
              header: 'item1',
              content: 'content1',
              cssClass: 'CustomClass1 CustomClass2',
            },
          ],
        },
        ele
  ...
  describe('Accordion Item expanded Property testing', () => {
    ...
    it('EJ2-51850 - Accessibility validation error in Accordion', () => {
      let ele: HTMLElement = document.getElementById('accordion');
      expect(
        ele.children[0].firstElementChild.getAttribute('aria-expanded')
      ).toBe('true');
      expect(
        ele.children[1].firstElementChild.getAttribute('aria-expanded')
      ).toBe('false');
    });
  });
  describe('Accordion Item expanded Property testing with Fade animation', () => {
    ...
    beforeEach((done: Function) => {
      ...
      accordion = new Accordion(
        {
          animation: {
            expand: { effect: 'FadeZoomIn' },
            collapse: { effect: 'FadeZoomOut' },
          },
          items: [
            { header: 'Item1', content: 'Content of Item1', expanded: true },
            { header: 'Item2', content: 'Content of Item2' },
  ...
  describe('Accordion Control template Testing ', () => {
    ...
    it('Accordion Control with header template', () => {
      ...
      expect((<HTMLElement>acrdnItem[0]).classList.contains(CLS_ITEM)).toBe(
        true
      );
      expect(
        (<HTMLElement>acrdnItem[0].firstChild).classList.contains(CLS_HEADER)
      ).toBe(true);
      expect(
        (<HTMLElement>acrdnItem[0].firstChild.firstChild).classList.contains(
          CLS_HEADERCTN
        )
      ).toBe(true);
      expect(
        acrdnItem[0].firstChild === ele.querySelector('.' + CLS_HEADER)
      ).toBe(true);
    ...
    it('Accordion Control with header and content template', () => {
      ...
      expect(acrdnItemHder.children[1].classList.contains(CLS_TOOGLEICN)).toBe(
        true
      );
    ...
    it('Accordion content template with single expand mode and additional div element testing', () => {
      ele.innerHTML =
        '<div class="e-accordion-container"><div><div><div>  Header name 1 </div></div><div><div>content 1 </div></div></div><div><div><div>  Header name 2 </div></div><div><div>content 2</div></div></div></div>';
      new Accordion({ expandMode: 'Single' }, ele);
      let rootEle: any = document.querySelector('#accordion');
      let acrdn_item1: HTMLElement = rootEle.querySelectorAll(
        '.' + CLS_ITEM
      )[0];
      let acrdn_item2: HTMLElement = rootEle.querySelectorAll(
        '.' + CLS_ITEM
      )[1];
      rootEle.querySelectorAll('.' + CLS_ITEM)[0].click();
  ...
  describe('Accordion Content behaviour with Template Testing', () => {
    ...
    afterEach((): void => {
      ...
      expect(
        eleItem.querySelector('.' + CLS_CTENT).firstElementChild.tagName
      ).toBe('BUTTON');
  ...
  describe('Accordion Header behaviour with Template Testing', () => {
    ...
    afterEach((): void => {
      ...
      expect(ele.children[0].childElementCount).toBe(1);
      expect(
        eleItem.querySelector('.' + CLS_HEADER).firstElementChild
  ...
  describe('Accordion getPersistance Testing', () => {
    ...
    it('Accordion getPersistance data  Testing', (done: Function) => {
      window.localStorage.setItem(
        'accordionaccordion',
        JSON.stringify({ expandedIndices: '[0]' })
      );
      let ele: HTMLElement = document.getElementById('accordion');
      accordion = new Accordion(
        {
  ...
  describe('Accordion OnProperty Change behaviour Testing', () => {
    ...
    it('Accordion Onproperty Change testing for header text', () => {
      ...
      expect(ele.querySelector(accordion.items[0].content).innerHTML).toEqual(
        'content of item1'
      );
      expect(ele.querySelector(accordion.items[1].content).innerHTML).toEqual(
        'content of item2'
      );
  ...
  describe('Accordion Item Content Expand Testing with icon switching Animation', () => {
    ...
    it('Accordion Content Expand Testing', () => {
      ...
      let toggleIcon: Element =
        acrdnItem1.children[0].children[1].firstElementChild;
  ...
  describe('Accordion Item Content Collapse Testing with Animation and multiple ExpandMode', () => {
    ...
    it('Accordion Content Expand Testing', () => {
      ...
      expect(
        accordion.element.querySelectorAll('.' + CLS_EXPANDSTATE).length
      ).toBe(1);
      expect(isVisible(itemContent)).toBe(true);
      expect(itemContent.classList.contains(CLS_CTNHIDE)).toBe(false);
      expect(acrdnItem1.classList.contains(CLS_ACTIVE)).toBe(true);
  ...
  describe('Accordion Item Content Collapse Testing with expandMode', () => {
    ...
    it('Accordion Content Expand Testing', () => {
      ...
      expect(ele.children[1].children[1].classList.contains(CLS_CTNHIDE)).toBe(
        false
      );
  ...
  describe('Nested Accordion Testing expand and collapsing', () => {
    ...
    beforeEach((done: Function) => {
      ...
      accordion = new Accordion(
        {
          ...
          items: [
            {
              header: 'Item1',
              content: "<div id = 'nestedAccordion'></div>",
              expanded: true,
            },
            { header: 'Item2', content: 'Content of Item2' },
          ],
      ...
      let nestAcrdn: HTMLElement = <HTMLElement>(
        accrdItem.children[1].children[0].children[0]
      );
    ...
    afterEach((): void => {
      ...
      expect(nestAccCon.parentElement.classList.contains(CLS_EXPANDSTATE)).toBe(
        true
      );
      expect(
        ele.querySelector('.' + CLS_EXPANDSTATE) === nestAccCon.parentElement
      ).toBe(true);
  ...
  describe('Nested Accordion with root accordion collapse all testing', () => {
    ...
    beforeEach((done: Function) => {
      ...
      expect(document.body.querySelectorAll('.' + CLS_EXPANDSTATE).length).toBe(
        1
      );
      expect(nestAcrdn.classList.contains(CLS_ROOT)).toBe(true);
      expect(nestAcrdn.classList.contains('e-control')).toBe(true);
  ...
  describe('Nested Accordion with nested accordion collapse all testing', () => {
      ...
      nestAcc1 = new Accordion(
        {
          ...
          items: [
            ...
            {
              header: 'nestItem2',
              content: "<div id = 'nestedAccordion2'></div>",
              expanded: true,
            },
          ],
        },
    ...
    beforeEach((done: Function) => {
      ...
      let nestedAccItem: HTMLElement = <HTMLElement>(
        nestedAccordion1.children[1]
      );
      (<HTMLElement>nestedAccItem.children[0]).click();
      setTimeout(() => {
        done();
  ...
  describe('Accordion Public Method with ExpandItem and disabled item Testing ', () => {
    ...
    it('Accordion Public Method ExpandItem Testing', () => {
      ...
      let eleItem: Element = ele.children[1];
      expect(
        !isNOU(eleItem.children[1]) && isVisible(eleItem.children[1])
      ).toBe(false);
    });
  });
  ...
  describe('Accordion Public Method with ExpandItem for all Item for collapse Testing ', () => {
    ...
    it('Accordion Public Method ExpandItem with collapse all Testing', (done: Function) => {
      ...
      accordion.expandItem(false);
      setTimeout(() => {
        expect(ele.children[0].classList.contains('e-expand-state')).toBe(
          false
        );
  ...
  describe('Accordion Public Method Testing ', () => {
    ...
    it('Accordion Public Method addItem Testing', () => {
      ...
      expect(ele.childElementCount).toBe(2);
      accordion.addItem(
        { header: 'Items_Add', content: 'Content of Items_Add' },
        1
      );
    ...
    it('Accordion keyboard Interaction Testing', (done: Function) => {
      ...
      expect(
        document.activeElement ===
          accordion.element.querySelector('.' + CLS_HEADER)
      ).toBe(true);
      setTimeout(() => {
        done();
      }, TIME_DELAY);
  ...
  describe('Accordion Event Flow testing for Expanding and expand ', () => {
    ...
    afterEach((): void => {
      ...
      expect(
        curObject.EventArgs[1].element === curObject.EventArgs[2].element
      ).toBe(true);
      expect(curObject.EventArgs[1].item === curObject.EventArgs[2].item).toBe(
        true
      );
      if (accordion) {
        accordion.destroy();
    ...
    it('Accordion Expanding and Expand Testing', (done: Function) => {
      ...
      expect(
        curObject.EventArgs[1].index === curObject.EventArgs[2].index
      ).toBe(true);
      expect(
        curObject.EventArgs[1].content === curObject.EventArgs[2].content
      ).toBe(true);
  ...
  describe('Accordion Event Flow testing for Expanding and expand with public Methods ', () => {
    ...
    afterEach((): void => {
      ...
      expect(curObject.EventArgs[1].isExpanded).toBe(false);
      expect(
        curObject.EventArgs[0].element === curObject.EventArgs[1].element
      ).toBe(true);
      expect(curObject.EventArgs[0].item === curObject.EventArgs[1].item).toBe(
        true
      );
      if (accordion) {
        accordion.destroy();
  ...
  describe('Accordion Expanded Items Testing with private variables with public methods ', () => {
    ...
    it('Accordion Expanding with  private variables and public Methods testing', () => {
      ...
      expect(accordion.expandedIndices[1]).toBe(1);
      let eleItem: Element = ele.children[0];
      expect(eleItem.children[0].getAttribute('aria-controls')).toBe(
        eleItem.children[1].id
      );
      expect(eleItem.children[1].getAttribute('aria-labelledby')).toBe(
        eleItem.children[0].id
      );
      let eleItem1: Element = ele.children[1];
      expect(eleItem1.children[0].getAttribute('aria-controls')).toBe(
        eleItem1.children[1].id
      );
      expect(eleItem1.children[1].getAttribute('aria-labelledby')).toBe(
        eleItem1.children[0].id
      );
  ...
  describe('Accordion Aria-Attributes Testing', () => {
    ...
    it('Accordion Aria Attributes with default', () => {
      ...
      expect(
        ele.children[0].firstElementChild.getAttribute('aria-disabled')
      ).toBe('false');
      expect(
        ele.children[0].firstElementChild.getAttribute('aria-selected')
      ).toEqual(null);
      expect(
        ele.children[0].firstElementChild.getAttribute('aria-expanded')
      ).toBe('false');
      ...
      expect(ele.children[1].children[0].getAttribute('aria-controls')).toBe(
        null
      );
      expect(
        ele.children[1].firstElementChild.getAttribute('aria-disabled')
      ).toBe('false');
      expect(
        ele.children[1].firstElementChild.getAttribute('aria-selected')
      ).toEqual(null);
      expect(
        ele.children[1].firstElementChild.getAttribute('aria-expanded')
      ).toBe('false');
    ...
    it('Accordion Aria Attributes with disableItem Public method', () => {
      ...
      let index: number = 0;
      expect(
        ele.children[index].firstElementChild.getAttribute('aria-disabled')
      ).toBe('false');
  ...
  describe('Accordion Aria Attributes with ExpandItem Public method Testing', () => {
    ...
    afterEach((): void => {
      ...
      expect(ele.children[1].children[1].getAttribute('aria-hidden')).toBe(
        'true'
      );
      if (accordion) {
        accordion.destroy();
    ...
    it('Accordion Aria Attributes with ExpandItem Public method', (done: Function) => {
      ...
      expect(ele.children[0].children[1].getAttribute('aria-hidden')).toBe(
        'false'
      );
      (<HTMLElement>ele.children[1].children[0]).click();
      setTimeout(() => {
  ...
  describe('Accordion - separate items value change', () => {
    ...
    it('Items - iconCss property change testing', () => {
      let ele: HTMLElement = document.getElementById('accordion');
      expect(
        ele.children[0].querySelector('.e-acrdn-header .e-acrdn-header-icon')
      ).not.toEqual(undefined);
      expect(
        ele.children[0]
          .querySelector('.e-acrdn-header .e-acrdn-header-icon span')
          .classList.contains('e-icon1')
      ).toEqual(true);
      ...
      expect(
        ele.children[0]
          .querySelector('.e-acrdn-header .e-acrdn-header-icon span')
          .classList.contains('e-test-icon1')
      ).toEqual(true);
      accordion.items[1].iconCss = 'e-test-icon2';
      accordion.dataBind();
      expect(accordion.items[1].iconCss).toEqual('e-test-icon2');
      expect(
        ele.children[1].querySelector('.e-acrdn-header .e-acrdn-header-icon')
      ).not.toEqual(undefined);
      expect(
        ele.children[1]
          .querySelector('.e-acrdn-header .e-acrdn-header-icon span')
          .classList.contains('e-test-icon2')
      ).toEqual(true);
    });
    it('Items - header property change testing', () => {
      ...
      expect(
        ele.children[0].querySelectorAll('.e-acrdn-header').length
      ).toEqual(1);
      expect(
        ele.children[0].querySelectorAll('.e-acrdn-header-content').length
      ).toEqual(1);
      expect(
        ele.children[0].querySelectorAll('.e-acrdn-header-content')[0].innerHTML
      ).toEqual('testingItem1');
      accordion.items[1].header = 'testingItem2';
      accordion.dataBind();
      expect(accordion.items[1].header).toEqual('testingItem2');
      expect(
        ele.children[1].querySelectorAll('.e-acrdn-header').length
      ).toEqual(1);
      expect(
        ele.children[1].querySelectorAll('.e-acrdn-header-content').length
      ).toEqual(1);
      expect(
        ele.children[1].querySelectorAll('.e-acrdn-header-content')[0].innerHTML
      ).toEqual('testingItem2');
    });
    it('Items - expanded property change testing', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        expect(ele.children[0].classList.contains('e-expand-state')).toEqual(
          true
        );
    ...
    it('Items - content property change testing', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          expect(
            ele.children[0].querySelectorAll('.e-acrdn-panel').length
          ).toEqual(1);
          expect(
            ele.children[0].querySelectorAll('.e-acrdn-content').length
          ).toEqual(1);
          expect(
            ele.children[0].querySelectorAll('.e-acrdn-content')[0].innerHTML
          ).toEqual('Content1');
          done();
        }, 1000);
      }, 1000);
  ...
  describe('onPropertyChange - value Testing', () => {
    ...
    it('header without content Testing', () => {
      ...
      accordion.dataBind();
      expect(
        ele.children[0].querySelector('e-acrdn-header .e-toggle-icon')
      ).toEqual(null);
      expect(
        ele.children[1].querySelector('e-acrdn-header .e-toggle-icon')
      ).not.toEqual(undefined);
    });
  ...
  describe('Accordion dataSource testing', () => {
    ...
    it('Accordion dataSource rendering testing', () => {
      ...
      expect(
        ele.querySelectorAll('.' + CLS_ITEM).length ===
          accordion.dataSource.length
      ).toBe(true);
    });
    it('Accordion dataSource headerTemplate rendering testing', () => {
      ...
      expect(headerEle.childElementCount).toBe(2);
      expect(
        headerEle.firstChild.textContent === 'Super Mart of the West'
      ).toBe(true);
    });
    it('Accordion dataSource itemTemplate rendering testing', (done: Function) => {
      ...
      expect(
        headerEle.nextElementSibling.firstChild.textContent === 'Bentonville'
      ).toBe(true);
    });
  ...
  describe('Accordion item unique id testing', () => {
    ...
    it('Accordion item unique id value', () => {
      ...
      const accordionItems: NodeListOf<Element> =
        accordion.querySelectorAll('.e-acrdn-item');
  ...
  describe('BLAZ-20806-Nested Accordion rendering testing', () => {
    ...
    it('Check nested accordion', () => {
      const acrdnItemPanel: Element = accordion.element.querySelector(
        '.' + CLS_CONTENT
      );
      const nestAcrdn: HTMLElement = <HTMLElement>(
        acrdnItemPanel.firstElementChild.firstElementChild
      );
      expect(nestAcrdn.classList.contains(CLS_ROOT)).toBe(true);
          ...
          items: [
            {
              header: 'Item1',
              content:
                "<div>nested accordion content</div><div id = 'nestedAccordion'></div>",
              expanded: true,
            },
            { header: 'Item2', content: 'Content of Item2' },
          ],
