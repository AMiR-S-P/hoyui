...
import { CompressedStreamWriter } from '@hoyui/compression';
export class _PdfCrossReference {
  ...
  _setStartXRef(startXRef: number): void {
    ...
    if (
      typeof this._prevXRefOffset === 'undefined' ||
      this._prevXRefOffset === null
    ) {
  ...
  _parse(recoveryMode: boolean): void {
    ...
    if (encrypt) {
      ...
      this._document._fileStructure._crossReferenceType =
        PdfCrossReferenceType.stream;
      this._document._isUserPassword = this._encrypt._isUserPassword;
      this._document._encryptOnlyAttachment =
        this._encrypt._encryptOnlyAttachment;
      if (this._encrypt._encryptOnlyAttachment) {
        this._document._hasUserPasswordOnly = true;
        this._document._encryptMetaData = false;
      } else {
        this._document._hasUserPasswordOnly =
          this._encrypt._hasUserPasswordOnly;
        this._document._encryptMetaData = encrypt.has('EncryptMetadata')
  ...
  _fetchUncompressed(
    ...
    xrefEntry: _PdfObjectInformation,
    ...
    if (xrefEntry.gen !== generationNumber) {
      throw new BaseException(
        `Inconsistent generation in XRef: ${reference}`,
        'XRefEntryException'
      );
    }
    const stream: _PdfStream = this._stream.makeSubStream(
      xrefEntry.offset + this._stream.start,
      undefined
    );
    const parser: _PdfParser = new _PdfParser(
      new _PdfLexicalOperator(stream),
      this,
      true
    );
    ...
    if (
      obj1 !== objectNumber ||
      obj2 !== generationNumber ||
      typeof obj3 === 'undefined'
    ) {
      throw new BaseException(
        `Bad (uncompressed) XRef entry: ${reference}`,
        'XRefEntryException'
      );
    }
    let entry: any; // eslint-disable-line
    if (this._encrypt && !suppressEncryption) {
      entry = parser.getObject(
        this._encrypt._createCipherTransform(
  ...
  _fetchCompressed(ref: _PdfReference, xrefEntry: _PdfObjectInformation): any {
    ...
    let parser: _PdfParser = new _PdfParser(
      new _PdfLexicalOperator(stream),
      this,
      true
    );
    ...
    for (let i: number = 0; i < n; ++i) {
      const length: number =
        i < n - 1 ? offsets[i + 1] - offsets[i] : undefined; // eslint-disable-line
      if (length < 0) {
        throw new FormatError('Invalid offset in the ObjStm stream.');
      }
      parser = new _PdfParser(
        new _PdfLexicalOperator(
          stream.makeSubStream(start + offsets[i], length, stream.dictionary)
        ),
        this,
        true
      ); // eslint-disable-line
    ...
    try {
      while (this._startXRefQueue.length) {
        ...
        if (_isCommand(obj, 'xref')) {
          if (
            typeof this._document._fileStructure._crossReferenceType ===
            'undefined'
          ) {
            this._document._fileStructure._crossReferenceType =
              PdfCrossReferenceType.table;
          }
  ...
  _indexObjects(): _PdfDictionary {
    ...
    const trailerBytes: Uint8Array = new Uint8Array([
      116, 114, 97, 105, 108, 101, 114,
    ]);
    const startxrefBytes: Uint8Array = new Uint8Array([
      ...
      if (
        ...
        if (m) {
          ...
          if (!this._entries[objectNumber]) {
            ...
            // eslint-disable-line
            try {
              const subStream: _PdfStream = stream.makeSubStream(
                startPos,
                stream.length - startPos
              );
              const lexicalOperator: _PdfLexicalOperator =
                new _PdfLexicalOperator(subStream);
          ...
          while (startPos < buffer.length) {
            const endPos: number =
              startPos + this._skipUntil(buffer, startPos, objBytes) + 4;
            contentLength = endPos - position;
            const checkPos: number = Math.max(
              endPos - checkContentLength,
              startPos
            );
            const tokenStr: string = _bytesToString(
          ...
          const content: Uint8Array = buffer.subarray(
            position,
  ...
  _processXRefTable(parser: _PdfParser): _PdfDictionary {
    ...
    if (topDictionary) {
      if (topDictionary instanceof _PdfDictionary) {
        ...
        topDictionary instanceof _PdfBaseStream &&
    ...
    if (!dictionary) {
      throw new FormatError(
        'Invalid cross reference: could not parse trailer dictionary'
      );
    }
  ...
  _readXRefTable(parser: _PdfParser): _PdfCommand {
    ...
    while (true) {
      // eslint-disable-line
      if (
        typeof this._tableState.firstEntryNum === 'undefined' ||
        typeof this._tableState.entryCount === 'undefined'
      ) {
        obj = parser.getObject();
        if (_isCommand(obj, 'trailer')) {
          break;
      ...
      if (!Number.isInteger(first) || !Number.isInteger(count)) {
        throw new FormatError(
          'Invalid cross reference: wrong types in subsection header'
        );
      }
      for (let i: number = this._tableState.entryNum; i < count; i++) {
        ...
        if (
          ...
          throw new FormatError(
            `Invalid entry in cross reference subsection: ${first}, ${count}`
          );
        }
    ...
    while (entryRanges.length > 0) {
      ...
      for (let i: number = this._streamState.entryNum; i < n; ++i) {
        ...
        for (let j: number = 0; j < generationFieldWidth; ++j) {
          ...
          if (generationByte === -1) {
            throw new FormatError(
              'invalid cross reference byte width generation.'
            );
          }
  ...
  _save(): Uint8Array {
    ...
    const buffer: Array<number> = [
      this._newLine.charCodeAt(0),
      this._newLine.charCodeAt(1),
      37,
      80,
      68,
      70,
    ...
    const array: Uint8Array = new Uint8Array(
      this._stream.length + buffer.length
    );
  ...
  _saveAsStream(currentLength: number, buffer: number[]): void {
    ...
    const objectStreamCollection: Map<_PdfReference, _PdfArchievedStream> =
      new Map<_PdfReference, _PdfArchievedStream>();
    this._cacheMap.forEach((value: any, key: _PdfReference) => {
      // eslint-disable-line
      let dictionary: _PdfDictionary;
    ...
    this._cacheMap.forEach(
      (value: _PdfDictionary | _PdfBaseStream, key: _PdfReference) => {
        if (value instanceof _PdfDictionary) {
          if (value._updated && (!value.isCatalog || this._allowCatalog)) {
            if (
              typeof objectStream === 'undefined' ||
              objectStream._length === 100
            ) {
              const archiveObj: _PdfArchievedStream = new _PdfArchievedStream(
                this
              );
    ...
    objectStreamCollection.forEach(
      (value: _PdfArchievedStream, key: _PdfReference) => {
    ...
    const formatValue: number = Math.max(
      _getSize(this._stream.bytes.length + buffer.length),
    ...
    if (this._ids && this._ids.length > 0) {
      newXref.update('ID', [
        this._ids[0],
        this._computeMessageDigest(newStartXref),
      ]);
    }
    ...
    const newXrefStream: _PdfStream = new _PdfStream(
      newXrefData,
      newXref,
      0,
      newXrefData.length
    ...
    if (this._encrypt) {
      cipher = this._encrypt._createCipherTransform(
        newRef.objectNumber,
        newRef.generationNumber
      );
    }
    this._writeObject(newXrefStream, buffer, newRef, cipher, true);
    this._writeString(
      `startxref${this._newLine}${newStartXref}${this._newLine}%%EOF${this._newLine}`,
      buffer
    );
  }
  _saveAsTable(currentLength: number, buffer: number[]): void {
    ...
    this._writeString(
      `xref${this._newLine}0 1${this._newLine}0000000000 65535 f${this._newLine}`,
      buffer
    );
    if (updatedCount > 0) {
  ...
  _computeMessageDigest(size: number): string {
    ...
    if (info && info instanceof _PdfDictionary) {
      info.forEach((key: string, value: any) => {
        // eslint-disable-line
    ...
    crossReferenceInfo.forEach((key: string, value: any) => {
      // eslint-disable-line
      buffer.push(value);
    });
    const array: number[] = [];
  ...
  _getNextReference(): _PdfReference {
    const reference: _PdfReference = new _PdfReference(
  ...
  _writeObject(
    obj: _PdfDictionary | _PdfBaseStream,
    buffer: Array<number>,
    reference?: _PdfReference,
    ...
    if (reference && reference instanceof _PdfReference) {
      this._writeString(
        `${reference.objectNumber} ${reference.generationNumber} obj${this._newLine}`,
        buffer
      );
    }
    if (obj instanceof _PdfDictionary) {
  ...
  _writeDictionary(
    ...
    dictionary.forEach((key: string, value: any) => {
      // eslint-disable-line
...
class _PdfArchievedStream {
  ...
  _save(buffer: number[], currentLength: number): void {
    ...
    const archiveStream: _PdfStream = new _PdfStream(
      data,
      newDict,
      0,
      data.length
    ...
    if (this._crossReference._encrypt) {
      cipher = this._crossReference._encrypt._createCipherTransform(
        this._reference.objectNumber,
        this._reference.generationNumber
      );
    }
    this._crossReference._writeObject(
      archiveStream,
      buffer,
      this._reference,
      cipher
