...
describe('CheckBox', () => {
  ...
  let checkbox: any;
  const element: HTMLFormElement = createElement('input', {
    id: 'checkbox',
  }) as HTMLFormElement;
  ...
  describe('DOM', () => {
    ...
    it('Normal CheckBox Testing', () => {
      ...
      expect(element.nextElementSibling.classList.contains('e-frame')).toEqual(
        true
      );
    });
    ...
    it('CheckBox with Label', () => {
      ...
      expect(
        element.parentElement.children[2].classList.contains('e-label')
      ).toEqual(true);
    ...
    it('CheckBox with checked state', () => {
      ...
      expect(
        element.parentElement.children[1].classList.contains('e-check')
      ).toEqual(true);
    ...
    it('CheckBox with RTL', () => {
      ...
      expect(
        element.parentElement.parentElement.classList.contains('e-rtl')
      ).toEqual(true);
    });

    it('CheckBox with labelPosition', () => {
      checkbox = new CheckBox(
        { label: 'CheckBox', labelPosition: 'Before' },
        '#checkbox'
      );
    ...
    it('CheckBox with indeterminate state', () => {
      ...
      expect(
        element.parentElement.children[1].classList.contains('e-stop')
      ).toEqual(true);
    ...
    it('CheckBox with cssClass', () => {
      ...
      expect(
        element.parentElement.parentElement.classList.contains('class')
      ).toEqual(true);
      checkbox.cssClass = 'syncfusion ';
      checkbox.dataBind();
      expect(
        element.parentElement.parentElement.classList.contains('syncfusion')
      ).toEqual(true);
    });
    ...
    it('Hidden input', () => {
      ...
      expect(element.parentElement.children[1].getAttribute('name')).toEqual(
        'check'
      );
      expect(element.parentElement.children[1].getAttribute('type')).toEqual(
        'hidden'
      );
      expect(element.parentElement.children[1].getAttribute('value')).toEqual(
        'false'
      );
    ...
    it('Enable Html Sanitizer testing', () => {
      checkbox = new CheckBox(
        { label: '<style>body{background:rgb(0, 0, 255)}</style>' },
        '#checkbox'
      );
      const htmlele: Element = document.body;
      expect(window.getComputedStyle(htmlele).backgroundColor).not.toBe(
        'rgb(0, 0, 255)'
      );
    });
    ...
    it('Enable Html Sanitizer disabled testing', () => {
      checkbox = new CheckBox(
        {
          label: 'Banking<style>body{background:rgb(0, 0, 255)}</style>',
          enableHtmlSanitizer: false,
        },
        '#checkbox'
      );
      const htmlele: Element = document.body;
      expect(window.getComputedStyle(htmlele).backgroundColor).toBe(
        'rgb(0, 0, 255)'
      );
    });
  });
  ...
  describe('Notify property changes of', () => {
    ...
    it('CheckBox with Label', () => {
      ...
      expect(element.parentElement.children[2].textContent).toEqual(
        'checkbox1'
      );
    });
    ...
    it('CheckBox with Label with html', () => {
      checkbox = new CheckBox(
        { label: '<div>CheckBox:false</div>', enableHtmlSanitizer: false },
        '#checkbox'
      );
      expect(element.parentElement.children[2].textContent).toEqual(
        'CheckBox:false'
    ...
    it('CheckBox with cssClass', () => {
      ...
      checkbox.dataBind();
      expect(
        element.parentElement.parentElement.classList.contains('newClass')
      ).toEqual(true);
    });
  });
  ...
  describe('Methods', () => {
    ...
    it('Keyboard Event', () => {
      ...
      expect(
        element.parentElement.parentElement.classList.contains('e-focus')
      ).toEqual(true);
      checkbox.focusHandler();
    ...
    it('Pre render method', () => {
      document.body.appendChild(
        createElement('EJS-CHECKBOX', {
          id: 'ngcheckbox',
          attrs: { label: 'Checkbox' },
        })
      );
      checkbox = new CheckBox({}, '#ngcheckbox');
      expect(checkbox.element.parentElement.parentElement.tagName).toEqual(
        'EJS-CHECKBOX'
      );
      expect(checkbox.element.parentElement.children[0].tagName).toEqual(
        'INPUT'
      );
      expect(
        checkbox.element.parentElement.children[1].classList.contains('e-frame')
      ).toEqual(true);
      expect(checkbox.element.getAttribute('label')).toEqual(null);
      checkbox.element.click();
      checkbox.destroy();
      expect(document.getElementById('ngcheckbox').tagName).toBe(
        'EJS-CHECKBOX'
      );
      checkbox = new CheckBox(
        {},
        document.body.appendChild(createElement('input')) as HTMLInputElement
      );
  ...
  describe('creation by util function', () => {
    ...
    it('with checked', () => {
      const checkboxElem: Element = createCheckBox(createElement, false, {
        checked: true,
        cssClass: 'e-small',
      });
  ...
  describe('Notify Html Attributes property changes of', () => {
    ...
    it('CheckBox with Style', () => {
      checkbox = new CheckBox(
        {
          label: 'checkbox',
          htmlAttributes: { style: 'background-color:red' },
        },
        '#checkbox'
      );
      expect(
      ...
      expect(
        element.parentElement.children[1]
          .getAttribute('style')
          .indexOf('background-color:#d3d3d3')
      ).toEqual(0);
    });

    it('CheckBox with Class', () => {
      checkbox = new CheckBox(
        { htmlAttributes: { class: 'e-checkbox-syncfusion' } },
        '#checkbox'
      );
      expect(
        element.parentElement.parentElement.classList.contains(
          'e-checkbox-syncfusion'
        )
      ).toEqual(true);
      checkbox.htmlAttributes = { class: 'e-new-checkbox' };
      checkbox.dataBind();
      expect(
        element.parentElement.parentElement.classList.contains('e-new-checkbox')
      ).toEqual(true);
    });

    it('CheckBox with Title', () => {
      checkbox = new CheckBox(
        { htmlAttributes: { title: 'ejcheckbox' } },
        '#checkbox'
      );
      expect(
  ...
  describe('CR issues', () => {
    ...
    it('HtmlAttributes property not working properly in CheckBox', () => {
      checkbox = new CheckBox(
        {
          htmlAttributes: {
            'data-containerid': 'error-agreement',
            test: 'test',
          },
        },
        '#checkbox'
      ...
      expect(
        element.parentElement.parentElement
          .getAttribute('data-containerid')
          .indexOf('error-agreement')
      ).toEqual(0);
      expect(
        element.parentElement.parentElement.getAttribute('test').indexOf('test')
      ).toEqual(0);
    });
  });
  ...
  describe('Coverage Improvement', function () {
    ...
    it('CheckBox indeterminate with ripple state', () => {
      enableRipple(true);
      checkbox = new CheckBox(
        { indeterminate: true, labelPosition: 'Before' },
        '#checkbox'
      );
      enableRipple(false);
    });
