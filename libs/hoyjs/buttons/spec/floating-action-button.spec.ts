...
import { append, createElement, isNullOrUndefined, remove } from '@hoyui/base';
...
describe('Floating Action Button', () => {
  ...
  describe('Methods and Events', () => {
    ...
    function getTargetPosition(tarEle: HTMLElement, direction: string): string {
      return (
        tarEle[direction == 'Top' ? 'clientHeight' : 'clientWidth'] / 2 + 'px'
      );
    }
  ...
  describe('position Values', () => {
    ...
    function getPosition(direction: string): string {
      return (
        Math.floor(
          window[direction == 'Top' ? 'innerHeight' : 'innerWidth'] / 2
        ) + 'px'
      );
    }
