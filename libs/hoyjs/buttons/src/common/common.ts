...
import { BaseEventArgs, getValue, addClass, deleteObject } from '@hoyui/base';
...
export function wrapperInitialize(
  createElement: CreateElementArgs,
  tag: string,
  type: string,
  ...
  if (element.tagName === tag) {
    ...
    for (
      let index: number = 0, len: number = element.attributes.length;
      index < len;
      index++
    ) {
      if (props.indexOf(element.attributes[index as number].nodeName) === -1) {
        input.setAttribute(
          element.attributes[index as number].nodeName,
          element.attributes[index as number].nodeValue
        );
      }
    }
...
export function createCheckBox(
  ...
  if (enableRipple) {
    const rippleSpan: HTMLElement = createElement('span', {
      className: 'e-ripple-container',
    });
    rippleEffect(rippleSpan, { isCenterRipple: true, duration: 400 });
    wrapper.appendChild(rippleSpan);
  }
  const frameSpan: Element = createElement('span', {
