...
import { EmitType, Event, EventHandler, MouseEventArgs } from '@hoyui/base';
import {
  addClass,
  isRippleEnabled,
  removeClass,
  ...
  implements INotifyPropertyChanged
{
  ...
  private changeState(state?: boolean): void {
    ...
    const handle: Element = wrapper.querySelector(
      '.e-switch-handle'
    ) as Element;
    if (isRippleEnabled) {
  ...
  private initWrapper(): void {
    ...
    const switchInner: Element = this.createElement('span', {
      className: 'e-switch-inner',
    });
    const onLabel: Element = this.createElement('span', {
      className: 'e-switch-on',
    });
    const offLabel: Element = this.createElement('span', {
      className: 'e-switch-off',
    });
    const handle: Element = this.createElement('span', {
  ...
  private rippleHandler(e: MouseEvent): void {
    ...
    if (
      e.type === 'mousedown' &&
      (e.currentTarget as Element).classList.contains('e-switch-wrapper') &&
      e.which === 1
    ) {
      this.isDrag = true;
  ...
  private mouseLeaveHandler(e: MouseEvent): void {
    const rippleSpan: Element = (
      this.element.parentElement as Element
    ).getElementsByClassName(RIPPLE)[0];
    if (rippleSpan) {
  ...
  private switchMouseUp(e: MouseEventArgs): void {
    let aTouchY: number = 0;
    let yDiff: number = 0;
    let aTouchX: number = 0;
    let xDiff: number = 0;
    ...
    if (this.isDrag) {
      if (
        (e.type === 'mouseup' && target.className.indexOf('e-switch') < 0) ||
        e.type === 'touchend'
      ) {
