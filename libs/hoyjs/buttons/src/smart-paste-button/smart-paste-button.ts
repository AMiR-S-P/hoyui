...
import { EventHandler, Property } from '@hoyui/base';
...
export class SmartPasteButton extends Button {
   ...
   */
  constructor(
    options?: SmartPasteButtonModel,
    element?: string | HTMLButtonElement
  ) {
  ...
  protected wireEvents(): void {
    EventHandler.add(
      this.element,
      'click',
      this.smartPasteBtnClickHandler,
      this
    );
  }
  ...
  private async smartPasteBtnClickHandler(args: MouseEvent): Promise<void> {
    ...
    const formElement: HTMLFormElement = target.closest(
      'form'
    ) as HTMLFormElement;
    if (!formElement) {
      return;
    ...
    const clipboardContent: string = await this.getClipboardContent().then(
      (text: string) => {
        return text;
      }
    );
    if (
      clipboardContent !== 'Clipboard API not supported' &&
      clipboardContent !== 'Clipboard access failed'
    ) {
      try {
        this.disabled = true;
...
USER_DATA: ${clipboardContent}
  ...
  private formatFields(fields: FieldInfo[]): string {
    ...
    fields.forEach((field: FieldInfo) => {
      ...
      if (field.allowedValues && field.allowedValues.length > 0) {
        ...
        fieldOutput.push(
          field.allowedValues.map((val: string) => `${val}`).join(',')
        );
        fieldOutput.push(')');
      } else {
    ...
    formFields.forEach((field: FieldInfo) => {
      ...
      if (value !== undefined) {
        ...
        if (
          ...
          const radioButton: HTMLInputElement = this.findRadioButton(
            form,
            field.element.name,
            value
          );
  ...
  private findRadioButton(
    ...
    const radioButtons: { elem: HTMLInputElement; text: string }[] = Array.from(
      form.querySelectorAll('input[type=radio]')
    )
      .filter(
        (radio: HTMLElement) =>
          radio instanceof HTMLInputElement && radio.name === name
      )
      .map((radio: HTMLInputElement) => ({
        elem: radio,
        text: this.getElementDescription(form, radio),
      }));
    const exactMatch: { elem: HTMLInputElement } = radioButtons.find(
      (radio: { text: string }) => radio.text === value
    );
    if (exactMatch) {
      return exactMatch.elem;
    }
    const partialMatch: { elem: HTMLInputElement }[] = radioButtons.filter(
      (radio: { text: string }) => radio.text && radio.text.includes(value)
    );
  ...
  private updateElementValue(element: HTMLElement, value: string): void {
    ...
    if (
      ...
      const responseValue: string | undefined =
        value == null ? undefined : value.toString().toLowerCase();
      const isResponseValue: boolean =
        responseValue === 'true' ||
        responseValue === 'yes' ||
        responseValue === 'on';
      if (element.checked !== isResponseValue) {
        ...
        if (isEjsControl) {
          (
            (element as unknown as { [key: string]: object[] })[
              'ej2_instances'
            ] as HTMLInputElement[]
          )[0].checked = isResponseValue;
        } else {
      ...
      const exactMatch: HTMLOptionElement[] = options.filter(
        (option: HTMLOptionElement) => option.textContent === optionText
      );
      if (exactMatch.length > 0) {
        index = options.indexOf(exactMatch[0]);
      } else {
        const partialMatch: HTMLOptionElement[] = options.filter(
          (option: HTMLOptionElement) =>
            option.textContent && option.textContent.indexOf(optionText) >= 0
        );
        if (partialMatch.length === 1) {
      ...
      this.triggerBeforeChange(element);
      if (
        element.classList.contains('e-rating') ||
        element.classList.contains('e-colorpicker')
      ) {
        (
          (element as unknown as { [key: string]: object[] })[
            'ej2_instances'
  ...
  private getFormFields(form: HTMLFormElement): FieldInfo[] {
    ...
    form
      ...
      .forEach((element: Element) => {
        ...
        const identifier: string = isRadioButton
          ? element.name
          : element.id || element.name || `unidentified_${++uniqueCount}`;
        if (
          isRadioButton &&
          fields.find((field: FieldInfo) => field.fieldName === identifier)
        ) {
          return;
        }
        ...
        if (element instanceof HTMLSelectElement) {
          ...
          fieldInfo.allowedValues = options.map(
          ...
          Array.from(form.querySelectorAll('input[type=radio]')).forEach(
            (radio: HTMLInputElement) => {
              if (radio.name === identifier) {
                const radioDescription: string = this.getElementDescription(
                  form,
                  radio
                );
                if (radioDescription) {
  ...
  private isFieldIgnore(element: Element): boolean {
    return (
      element.hasAttribute('data-smartpaste-ignore') ||
  ...
  private async getClipboardContent(): Promise<string> {
    const navigatorObj: NavigatorClipboard = navigator as Navigator &
      NavigatorClipboard;
    const customClipboard: HTMLInputElement | HTMLTextAreaElement =
      document.getElementById('custom-clipboard') as
        | HTMLInputElement
        | HTMLTextAreaElement;
    if (customClipboard && customClipboard.value) {
      ...
      navigatorObj.clipboard.readText
    ) {
      try {
