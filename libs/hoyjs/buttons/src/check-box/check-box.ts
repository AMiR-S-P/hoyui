...
export class CheckBox
  extends Component<HTMLInputElement>
  implements INotifyPropertyChanged
{
    ...
    if (state === 'check') {
      ...
      if (
        (this.element.required ||
          (closest(this.element, 'form') &&
            closest(this.element, 'form').classList.contains(
              'e-formvalidator'
            ))) &&
        this.validCheck &&
        !isInitialize
        ...
        this.element.required ||
        (closest(this.element, 'form') &&
          closest(this.element, 'form').classList.contains('e-formvalidator'))
      ) {
  ...
  private initWrapper(): void {
    ...
    const label: Element = this.createElement('label', {
      attrs: { for: this.element.id },
    });
    const frameSpan: Element = this.createElement('span', {
    ...
    if (isRippleEnabled) {
      const rippleSpan: HTMLElement = this.createElement('span', {
        className: RIPPLE,
      });
  ...
  private labelMouseDownHandler(e: MouseEvent): void {
    ...
    const rippleSpan: Element = (
      this.getWrapper() as Element
    ).getElementsByClassName(RIPPLE)[0];
    rippleMouseHandler(e, rippleSpan);
  }

  private labelMouseLeaveHandler(e: MouseEvent): void {
    const rippleSpan: Element = (
      this.getLabel() as Element
    ).getElementsByClassName(RIPPLE)[0];
    if (rippleSpan) {
      const rippleElem: NodeListOf<Element> =
        rippleSpan.querySelectorAll('.e-ripple-element');
  ...
  public onPropertyChanged(
    newProp: CheckBoxModel,
    oldProp: CheckBoxModel
  ): void {
    const wrapper: Element = this.getWrapper() as Element;
      ...
      switch (prop) {
        ...
        case 'labelPosition': {
          ...
          if (newProp.labelPosition === 'After') {
            ...
            labelWrap.insertBefore(
              label,
              wrapper.getElementsByClassName(FRAME)[0]
            );
          }
          break;
        }
  ...
  private setText(text: string): void {
    ...
    if (label) {
      label.innerHTML = this.enableHtmlSanitizer
        ? SanitizeHtmlHelper.sanitize(text)
        : text;
    } else {
      text = this.enableHtmlSanitizer
        ? SanitizeHtmlHelper.sanitize(text)
        : text;
  ...
  protected unWireEvents(): void {
    ...
    const formElem: HTMLFormElement = <HTMLFormElement>(
      closest(this.element, 'form')
    );
    if (formElem) {
