...
import { addClass, Event, EmitType, detach, removeClass } from '@hoyui/base';
import {
  rippleEffect,
  EventHandler,
  Observer,
  ...
  implements INotifyPropertyChanged
{
  ...
  private initialize(): void {
    ...
    if (
      !isBlazor() ||
      (isBlazor() && this.getModuleName() !== 'progress-btn')
    ) {
      if (this.content) {
        const tempContent: string = this.enableHtmlSanitizer
      ...
      if (!this.element.textContent.trim()) {
        ...
        if (this.iconPosition === 'Top' || this.iconPosition === 'Bottom') {
          this.element.classList.add(
            'e-' + this.iconPosition.toLowerCase() + '-icon-btn'
          );
        }
      }
  ...
  public destroy(): void {
    ...
    const span: Element = this.element.querySelector(
      'span.e-btn-icon'
    ) as Element;
    if (span) {
      detach(span);
  ...
  public onPropertyChanged(newProp: ButtonModel, oldProp: ButtonModel): void {
    let span: Element = this.element.querySelector(
      'span.e-btn-icon'
    ) as Element;
        ...
        case 'content': {
          ...
          if (
            ...
            (isBlazor() &&
              ...
              this.getModuleName() !== 'progress-btn')
          ) {
            if (this.enableHtmlSanitizer) {
              newProp.content = SanitizeHtmlHelper.sanitize(
                newProp.content as string
              );
            }
