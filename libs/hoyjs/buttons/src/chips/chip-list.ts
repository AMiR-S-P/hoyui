...
import { EventHandler, detach, EmitType, Event, addClass } from '@hoyui/base';
  ...
  implements INotifyPropertyChanged
{
  ...
  protected chipType(): boolean {
    return (this.chips &&
      this.chips.length &&
      this.chips.length > 0) as boolean;
  }
  ...
  protected render(): void {
    this.type =
      !isNullOrUndefined(this.chips) && this.chips.length
        ? 'chipset'
  ...
  private createChip(): void {
    this.innerText =
      this.element.innerText && this.element.innerText.length !== 0
        ? this.element.innerText.trim()
        : this.element.innerText;
    this.element.innerHTML = '';
    this.chipCreation(
      this.type === 'chip'
        ? [this.innerText ? this.innerText : this.text]
        : this.chips
    );
  }
  ...
  private chipCreation(data: string[] | number[] | ChipModel[]): void {
    ...
    for (let i: number = 0; i < data.length; i++) {
      ...
      const attributesValue: { [key: string]: string } =
        fieldsData.htmlAttributes;
      attributeArray.push(attributesValue);
      const chipArray: HTMLElement[] = this.elementCreation(fieldsData);
      const className: string[] = (
        classNames.chip +
        ' ' +
        (fieldsData.enabled ? ' ' : classNames.disabled) +
        ' ' +
        (fieldsData.avatarIconCss || fieldsData.avatarText
          ? classNames.chipWrapper
          : fieldsData.leadingIconCss
      ...
      if (!this.chipType() || this.type === 'chip') {
        ...
        if (!isNullOrUndefined(attributeArray[i as number])) {
          if (
            ...
            Object.keys(attributeArray[i as number]).length
          ) {
            let htmlAttr: string[] = [];
            htmlAttr = Object.keys(attributeArray[i as number]);
            for (let j: number = 0; j < htmlAttr.length; j++) {
              wrapper.setAttribute(
                htmlAttr[j as number],
                attributeArray[i as number][htmlAttr[j as number]]
              );
            }
          }
        }
  ...
  private getFieldValues(data: string | number | ChipModel): ChipFields {
    ...
    const fields: ChipFields = {
      ...
      cssClass:
        typeof data === 'object'
          ? data.cssClass
            ? data.cssClass.toString()
            : this.cssClass.toString()
          : this.cssClass.toString(),
      leadingIconCss:
        typeof data === 'object'
          ? data.leadingIconCss
            ? data.leadingIconCss.toString()
            : this.leadingIconCss.toString()
          : this.leadingIconCss.toString(),
      avatarIconCss:
        typeof data === 'object'
          ? data.avatarIconCss
            ? data.avatarIconCss.toString()
            : this.avatarIconCss.toString()
          : this.avatarIconCss.toString(),
      avatarText:
        typeof data === 'object'
          ? data.avatarText
            ? data.avatarText.toString()
            : this.avatarText.toString()
          : this.avatarText.toString(),
      trailingIconCss:
        typeof data === 'object'
          ? data.trailingIconCss
            ? data.trailingIconCss.toString()
            : this.trailingIconCss.toString()
          : this.trailingIconCss.toString(),
      enabled:
        typeof data === 'object'
          ? data.enabled !== undefined
            ? data.enabled.toString() === 'false'
              ? false
              : true
            : chipEnabled
          : chipEnabled,
      ...
      leadingIconUrl:
        typeof data === 'object'
          ? data.leadingIconUrl
            ? data.leadingIconUrl.toString()
            : this.leadingIconUrl
          : this.leadingIconUrl,
      trailingIconUrl:
        typeof data === 'object'
          ? data.trailingIconUrl
            ? data.trailingIconUrl.toString()
  ...
  private elementCreation(fields: ChipFields): HTMLElement[] {
    ...
    if (fields.avatarText || fields.avatarIconCss) {
      const className: string = (
        classNames.avatar +
        ' ' +
        fields.avatarIconCss
      ).trim();
      const chipAvatarElement: HTMLElement = this.createElement('span', {
        className: className,
      });
      ...
      const className: string = (
        classNames.icon +
        ' ' +
        fields.leadingIconCss
      ).trim();
      const chipIconElement: HTMLElement = this.createElement('span', {
        className: className,
      });
      ...
      chipIconElement.style.backgroundImage =
        'url(' + fields.leadingIconUrl + ')';
      chipArray.push(chipIconElement);
    }
    const chipTextElement: HTMLElement = this.createElement('span', {
    ...
    if (fields.trailingIconCss || (this.chipType() && this.enableDelete)) {
      const className: string = (
        classNames.delete +
        ' ' +
        (fields.trailingIconCss
          ? fields.trailingIconCss
          : classNames.deleteIcon)
      ).trim();
      const chipdeleteElement: HTMLElement = this.createElement('span', {
        className: className,
      });
      chipArray.push(chipdeleteElement);
    } else if (fields.trailingIconUrl) {
      const className: string = 'trailing-icon-url'.trim();
      const chipIconsElement: HTMLElement = this.createElement('span', {
        className: className,
      });
      chipIconsElement.style.backgroundImage =
        'url(' + fields.trailingIconUrl + ')';
  ...
  public find(fields: number | HTMLElement): ChipDataArgs {
    const chipData: ChipDataArgs = {
      text: '',
      index: -1,
      element: this.element,
      data: '',
    };
    const chipElement: HTMLElement =
      fields instanceof HTMLElement
        ? (fields as HTMLElement)
        : (this.element.querySelectorAll('.' + classNames.chip)[
            fields as number
          ] as HTMLElement);
    if (chipElement && this.chipType()) {
      chipData.index = Array.prototype.slice
        .call(this.element.querySelectorAll('.' + classNames.chip))
        .indexOf(chipElement);
      const chip: string | number | ChipModel =
        this.chips[chipData.index as number];
  ...
  public add(
    ...
    if (this.type !== 'chip') {
      const fieldData: string[] | number[] | ChipModel[] =
        chipsData instanceof Array
          ? chipsData
          : <string[] | number[] | ChipModel[]>[
              chipsData as string | number | ChipModel,
            ];
      this.chips = [].slice
        .call(this.chips)
        .concat(...(fieldData as ChipModel[]));
  ...
  private multiSelection(newProp: number[] | string[]): void {
    const items: NodeListOf<Element> = this.element.querySelectorAll(
      '.' + 'e-chip'
    );
      ...
      if (typeof newProp[j as number] === 'string') {
        for (let k: number = 0; k < items.length; k++) {
          if (newProp[j as number] !== k) {
            if (
              newProp[j as number] === items[k as number].attributes[5].value
            ) {
  ...
  private onSelect(
    ...
    if (this.chipType() && this.selection !== 'None') {
      if (callFromProperty) {
        const chipElements: NodeListOf<Element> = this.element.querySelectorAll(
          '.' + classNames.chip
        );
      ...
      const fieldData: number[] | HTMLElement[] | string[] =
        fields instanceof Array ? fields : <number[] | HTMLElement[]>[fields];
      for (let i: number = 0; i < fieldData.length; i++) {
        let chipElement: HTMLElement =
          fieldData[i as number] instanceof HTMLElement
            ? (fieldData[i as number] as HTMLElement)
            : (this.element.querySelectorAll('.' + classNames.chip)[
                fieldData[i as number] as number
              ] as HTMLElement);
        if (selectionType !== 'index') {
          for (let j: number = 0; j < this.chips.length; j++) {
            chipNodes = this.element.querySelectorAll('.' + classNames.chip)[
              j as number
            ] as HTMLElement;
            const fieldsData: ChipFields = this.getFieldValues(
              this.chips[j as number]
            );
            ...
            if (chipValue === fieldData[i as number].toString()) {
              index = j;
              chipElement = this.element.querySelectorAll(
                '.' + classNames.chip
              )[index as number] as HTMLElement;
            }
          }
        }
  ...
  public remove(fields: number | number[] | HTMLElement | HTMLElement[]): void {
    if (this.chipType()) {
      const fieldData: number[] | HTMLElement[] =
        fields instanceof Array ? fields : <number[] | HTMLElement[]>[fields];
      const chipElements: HTMLElement[] = [];
      const chipCollection: NodeListOf<HTMLElement> =
        this.element.querySelectorAll('.' + classNames.chip);
      ...
      chipElements.forEach((element: HTMLElement) => {
        const chips: NodeListOf<HTMLElement> = this.element.querySelectorAll(
          '.' + classNames.chip
        );
  ...
  public getSelectedChips(): SelectedItem | SelectedItems | undefined {
    ...
    if (this.chipType() && this.selection !== 'None') {
      ...
      for (let i: number = 0; i < items.length; i++) {
        ...
        (selectedItems.data as ChipModel[]).push(
          (this.chips as ChipModel[])[index as number]
        );
        const text: string | null | undefined =
          typeof this.chips[index as number] === 'object'
            ? (this.chips[index as number] as ChipModel).text
              ? (this.chips[index as number] as ChipModel).text
              : null
            : this.chips[index as number].toString();
  ...
  private focusOutHandler(e: MouseEventArgs): void {
    const chipWrapper: HTMLElement = <HTMLElement>(
      closest(e.target as HTMLElement, '.' + classNames.chip)
    );
    const focusedElement: HTMLElement | null = !this.chipType()
      ? this.element.classList.contains(classNames.focused)
        ? this.element
        : null
      : this.element.querySelector('.' + classNames.focused);
  ...
  private clickHandler(
    e: MouseEventArgs | KeyboardEventArgs,
    del: boolean = false
  ): void {
    const chipWrapper: HTMLElement = <HTMLElement>(
      closest(e.target as HTMLElement, '.' + classNames.chip)
    ...
    if (chipWrapper) {
      ...
      this.trigger(
        'beforeClick',
        chipDataArgs as ClickEventArgs,
        (observedArgs: ClickEventArgs) => {
  ...
  private clickEventHandler(
    ...
    if (this.chipType()) {
      ...
      const deleteElement: HTMLElement | undefined = (
        e.target as HTMLElement
      ).classList.contains(classNames.deleteIcon)
        ? (e.target as HTMLElement)
        : del
        ? (chipWrapper.querySelector(
            '.' + classNames.deleteIcon
          ) as HTMLElement)
        : undefined;
      if (deleteElement && this.enableDelete) {
        ...
        this.trigger(
          'delete',
          deletedItemArgs,
          (observedArgs: DeleteEventArgs) => {
            if (!observedArgs.cancel) {
              this.deleteHandler(observedArgs.element, observedArgs.index);
              this.selectionHandler(chipWrapper);
              (chipData as ClickEventArgs).selected =
                observedArgs.element.classList.contains(classNames.active);
              const selectedItemArgs: ClickEventArgs =
                chipData as ClickEventArgs;
              this.trigger('click', selectedItemArgs);
              const chipElement: HTMLElement = this.element.querySelectorAll(
                '.' + classNames.chip
              )[observedArgs.index] as HTMLElement;
              if (chipElement) {
                chipElement.focus();
        ...
        this.selectionHandler(chipWrapper);
        (chipData as ClickEventArgs).selected = chipWrapper.classList.contains(
          classNames.active
        );
  ...
  private selectionHandler(chipWrapper: HTMLElement): void {
    if (this.selection === 'Single') {
      const activeElement: HTMLElement = this.element.querySelector(
        '.' + classNames.active
      ) as HTMLElement;
  ...
  private updateSelectedChips(): void {
    const chipListEle: NodeListOf<Element> =
    ...
    for (let i: number = 0; i < chipListEle.length; i++) {
      const selectedEle: Element =
        this.element.querySelectorAll('.e-chip')[i as number];
      if (selectedEle.getAttribute('aria-selected') === 'true') {
        value = selectedEle.getAttribute('data-value');
        if (
          this.selection === 'Single' &&
          selectedEle.classList.contains('e-active')
        ) {
          chip = value ? value : i;
          break;
        } else {
  ...
  public destroy(): void {
    ...
    this.removeMultipleAttributes(
      ['tabindex', 'role', 'aria-label', 'aria-multiselectable'],
      this.element
    );
    this.wireEvent(true);
    this.rippleFunction();
