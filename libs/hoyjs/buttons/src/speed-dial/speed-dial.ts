  ...
  implements INotifyPropertyChanged
{
  ...
  private appendTemplate(): void {
    const templateContainer: HTMLElement = this.createElement('div', {
      className: SDTEMPLATECONTAINER,
    });
    append([templateContainer], this.popupEle);
    const templateFunction: Function = this.getTemplateString(
      this.popupTemplate
    );
    append(
      templateFunction(
  ...
  private getTemplateString(template: string | Function): Function {
    ...
    try {
      ...
      if (typeof template !== 'function' && tempEle) {
        ...
        stringContent =
          tempEle.tagName === 'SCRIPT' ? tempEle.innerHTML : tempEle.outerHTML;
      } else {
  ...
  private createItems(): void {
    this.focusedIndex = -1;
    const ul: HTMLElement = this.popupEle.querySelector(
      '.' + SDUL
    ) as HTMLElement;
    for (let index: number = 0; index < this.items.length; index++) {
      const item: SpeedDialItemModel =
        this.items[parseInt(index.toString(), 10)];
      ...
      if (item.disabled) {
        ...
        EventHandler.add(
          li,
          'click',
          (e: Event) => this.triggerItemClick(e, item),
          this
        );
      }
      if (item.title) {
  ...
  private checkTarget(): void {
    ...
    if (this.target) {
      this.targetEle =
        typeof this.target === 'string' ? select(this.target) : this.target;
      if (this.targetEle) {
  ...
  private popupMouseLeaveHandle(e: MouseEvent): void {
    ...
    if (
      this.opensOnHover &&
      !(
        target.classList.contains(SPEEDDIAL) || closest(target, '.' + SPEEDDIAL)
      )
    ) {
      this.hidePopupEle(e);
    }
  ...
  private mouseLeaveHandle(e: MouseEvent): void {
    const target: HTMLElement = e.relatedTarget as HTMLElement;
    if (
      !(target.classList.contains(SDPOPUP) || closest(target, '.' + SDPOPUP))
    ) {
      this.hidePopupEle(e);
    }
  }
  ...
  private focusLinearElement(isLeftUp: boolean): void {
    const isReversed: boolean =
      this.popupEle.classList.contains(SDVERTICALBOTTOM) ||
  ...
  private focusLeftRightElement(isLeft: boolean): void {
    ...
    const isradialTop: boolean =
      ['TopLeft', 'TopCenter', 'TopRight', 'MiddleLeft'].indexOf(
        this.position
      ) !== -1;
    if (
      (isradialTop && isLeft !== this.isClock) ||
      (!isradialTop && isLeft === this.isClock)
    ) {
      this.focusPrevElement();
    } else {
  ...
  private focusUpDownElement(isUp: boolean): void {
    ...
    const isradialRight: boolean =
      ['TopRight', 'MiddleRight', 'BottomRight', 'BottomCenter'].indexOf(
        this.position
      ) !== -1;
    if (
      (isradialRight && isUp !== this.isClock) ||
      (!isradialRight && isUp === this.isClock)
    ) {
      this.focusPrevElement();
    } else {
  ...
  private validateDirection(): void {
    switch (this.direction) {
      case 'Up':
        this.actualLinDirection =
          topPosition.indexOf(this.position) !== -1 ? 'Auto' : 'Up';
        break;
      case 'Down':
        this.actualLinDirection =
          bottomPosition.indexOf(this.position) !== -1 ? 'Auto' : 'Down';
        break;
      case 'Right':
        this.actualLinDirection =
          rightPosition.indexOf(this.position) !== -1 ? 'Auto' : 'Right';
        break;
      case 'Left':
        this.actualLinDirection =
          leftPosition.indexOf(this.position) !== -1 ? 'Auto' : 'Left';
        break;
      case 'Auto':
      default:
    ...
    this.isVertical = !(
      this.actualLinDirection === 'Left' || this.actualLinDirection === 'Right'
    );
  }
  ...
  private setMaxSize(): void {
    ...
    const right: number =
      (this.isFixed ? window.innerWidth : this.targetEle.clientWidth) -
      this.element.offsetLeft -
      this.element.offsetWidth;
    let limit: number = 0;
    const popupUlEle: HTMLElement = this.popupEle.querySelector(
      '.' + SDUL
    ) as HTMLElement;
    if (this.isVertical) {
      limit =
        this.actualLinDirection === 'Up' ||
        (this.actualLinDirection === 'Auto' &&
          topPosition.indexOf(this.position) === -1)
          ? top
          : bottom;
  ...
  private setLinearPosition(): void {
    ...
    const isMiddle: boolean =
      ['MiddleRight', 'MiddleCenter', 'MiddleLeft'].indexOf(this.position) !==
      -1;
    if (isTop) {
      vertDist =
        this.element.offsetTop +
        (this.isVertical ? this.element.offsetHeight : 0);
      if (isMiddle) {
        if (
          this.actualLinDirection === 'Right' ||
          this.actualLinDirection === 'Left'
        ) {
      ...
      vertDist = this.isFixed
        ? window.document.documentElement.clientHeight
        : this.targetEle.clientHeight;
      vertDist =
        vertDist -
        this.element.offsetTop -
        (this.isVertical ? 0 : this.element.offsetHeight);
      if (isMiddle) {
        if (
          this.actualLinDirection === 'Auto' ||
          this.actualLinDirection === 'Up'
  ...
  private setLinearHorizontalPosition(): void {
    ...
    if (
      this.actualLinDirection === 'Right' ||
      (this.isVertical && rightPosition.indexOf(this.position) === -1)
    ) {
      if (this.enableRtl) {
        this.setRight();
      } else {
      ...
      this.popupEle.classList.add(
        this.isVertical ? SDVERTICALRIGHT : SDHORIZONTALRIGHT
      );
    }
  }
  private setLeft(): void {
    ...
    const isCenter: boolean =
      ['TopCenter', 'MiddleCenter', 'BottomCenter'].indexOf(this.position) !==
      -1;
    let horzDist: number =
      this.element.offsetLeft +
      (this.isVertical ? 0 : this.element.offsetWidth);
    if (isCenter) {
      if (
        this.actualLinDirection === 'Auto' ||
        this.actualLinDirection === 'Down' ||
        ...
        horzDist =
          this.actualLinDirection === 'Right'
            ? this.element.offsetLeft + elementOffSetWidth
            : horzDist + elementOffSetWidth;
      }
    }
  ...
  private setRight(): void {
    ...
    const isCenter: boolean =
      ['TopCenter', 'MiddleCenter', 'BottomCenter'].indexOf(this.position) !==
      -1;
    let horzDist: number = this.isFixed
      ? window.document.documentElement.clientWidth
      : this.targetEle.clientWidth;
    horzDist =
      horzDist -
      this.element.offsetLeft -
      (this.isVertical ? this.element.offsetWidth : 0);
  ...
  private setPosition(): void {
    ...
    if (
      ['MiddleLeft', 'MiddleRight', 'MiddleCenter'].indexOf(this.position) !==
      -1
    ) {
      this.popupEle.classList.add(SDMIDDLE);
      const yoffset: number =
        ((this.isFixed ? window.innerHeight : this.targetEle.clientHeight) -
          this.popupEle.offsetHeight) /
        2;
      this.popupEle.style.setProperty(SDVERTDIST, yoffset + 'px');
    }
    this.popupEle.classList.add(
      bottomPosition.indexOf(this.position) === -1 ? SDTOP : SDBOTTOM
    );
    this.setHorizontalPosition();
  }
  ...
  private setCustomRadialPosition(): void {
    ...
    if (
      ['TopRight', 'BottomRight', 'MiddleRight'].indexOf(this.position) !== -1
    ) {
      let horzDist: number;
      if (this.enableRtl) {
        horzDist = this.element.offsetLeft;
    ...
    if (
      ['BottomLeft', 'BottomCenter', 'BottomRight'].indexOf(this.position) !==
      -1
    ) {
      if (this.isFixed) {
        this.popupEle.style.bottom =
          viewportHeight -
    ...
    if (
      ['MiddleLeft', 'MiddleCenter', 'MiddleRight'].indexOf(this.position) !==
      -1
    ) {
      this.popupEle.style.top =
        this.element.offsetTop - this.popupEle.offsetHeight / 2 + 'px';
    }
  }
  ...
  private setRadialPosition(): void {
    ...
    this.popupEle.style.setProperty(
      SDRADICALMINHEIGHT,
      li[0].offsetHeight + 'px'
    );
    this.popupEle.style.setProperty(
      SDRADICALMINWIDTH,
      li[0].offsetWidth + 'px'
    );
    const availableAngle: number = Math.abs(
      (range.endAngle as number) - (range.startAngle as number)
    );
    //Start and end will be same for Middle Center position, hence available angle will 0 or 360.
    const gaps: number =
      availableAngle === 360 || availableAngle === 0
        ? li.length
        : li.length - 1;
  ...
  private setRadialCorner(): void {
    //topLeftPosition
    if (
      ['TopLeft', 'TopCenter', 'MiddleLeft', 'MiddleCenter'].indexOf(
        this.position
      ) !== -1
    ) {
    ...
    //topRightPosition
    if (
      ['TopRight', 'TopCenter', 'MiddleRight', 'MiddleCenter'].indexOf(
        this.position
      ) !== -1
    ) {
    ...
    if (
      ...
      this.popupEle.classList.add(
        this.enableRtl ? SDBOTTOMRIGHT : SDBOTTOMLEFT
      );
    }
    //bottomRightPosition
    if (
      ['BottomRight', 'BottomCenter', 'MiddleRight', 'MiddleCenter'].indexOf(
        this.position
      ) !== -1
    ) {
      this.popupEle.classList.add(
        this.enableRtl ? SDBOTTOMLEFT : SDBOTTOMRIGHT
      );
    }
  }
    ...
    switch (this.position) {
      ...
      case 'MiddleRight':
        ...
        if (('MiddleLeft' === this.position) !== this.enableRtl) {
          ...
          const switchVal: boolean =
            (isClockwise && end < start) || (!isClockwise && end > start);
      ...
      case 'MiddleCenter':
        ...
        start =
          isNullOrUndefined(start) || start < 0 || start > 360
            ? isClockwise
              ? 0
              : 360
            : start;
        end =
          isNullOrUndefined(end) || end < 0 || end > 360
            ? isClockwise
              ? 360
              : 0
            : end;
  ...
  private hidePopupEle(e?: Event): void {
    ...
    const eventArgs: SpeedDialBeforeOpenCloseEventArgs = {
    ...
    this.trigger(
      'beforeClose',
      eventArgs,
      (args: SpeedDialBeforeOpenCloseEventArgs) => {
        if (args.cancel) {
          return;
        }
        if (this.animation.effect !== 'None') {
          ...
          const eleArray: HTMLElement[] = this.popupTemplate
            ? [this.popupEle.firstElementChild as HTMLElement]
            : selectAll('.' + SDLI, this.popupEle);
          const timeOutInterval: number =
            (this.animation.duration as number) / (eleArray.length + 1);
          ...
          const animateElement: Function = (curIndex: number): void => {
            const ele: HTMLElement =
              eleArray[parseInt(curIndex.toString(), 10)];
            closeAnimation.delay =
              curIndex === eleArray.length - 1 ? this.animation.delay : 0;
    ...
    this.trigger(
      ...
      (args: SpeedDialBeforeOpenCloseEventArgs) => {
        ...
        if (
          ...
          (animationMode === 'Enable' && this.animation.effect === 'None')
        ) {
  ...
  private triggerItemClick(e: Event, item: SpeedDialItemModel): void {
    ...
    target = target.classList.contains(SDLI)
      ? target
      : (closest(target, '.' + SDLI) as HTMLElement);
    const eventArgs: SpeedDialItemEventArgs = {
      element: target,
      item: item,
      event: e,
    };
  ...
  public destroy(): void {
    super.destroy();
    this.unwireEvents();
    ['aria-expanded', 'aria-haspopup', 'aria-controls'].forEach(
      (attr: string) => {
  ...
  public onPropertyChanged(
    newProp: SpeedDialModel,
    oldProp: SpeedDialModel
  ): void {
    const fabProplist: string[] = [
      'content',
      'cssClass',
      'disabled',
      'enablePersistence',
    ...
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        case 'mode':
          ...
          this.popupEle.classList.remove(RADIALSD, LINEARSD);
          this.popupEle.classList.add(
            this.mode === 'Radial' ? RADIALSD : LINEARSD
          );
          this.updatePositionProperties();
          break;
        case 'radialSettings':
