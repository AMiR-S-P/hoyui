...
import { Complex, Collection, ModuleDeclaration } from '@hoyui/base';
import { Event, EmitType, Internationalization } from '@hoyui/base';
...
import {
  ...
  findChildren,
  removeElement,
  setItemTemplateContent,
} from '../treemap/utils/helper';
import {
  removeClassNames,
  removeShape,
  textFormatter,
...
import {
  TreeMapHighlight,
  TreeMapSelection,
} from './user-interaction/highlight-selection';
  ...
  implements INotifyPropertyChanged
{
  ...
  private renderElements(): void {
    ...
    if (!isNullOrUndefined(this.treemapLevelData.levelsData)) {
      this.treemapLevelData.defaultLevelsData =
  ...
  private initPrivateVariable(): void {
    if (this.element.id === '') {
      const collection: number =
        document.getElementsByClassName('e-treemap').length;
  ...
  private createSecondaryElement(): void {
    const secondaryEle: Element = document.getElementById(
  ...
  private elementChange(): void {
    if (
      this.treeMapLegendModule &&
      this.legendSettings.visible &&
      this.treeMapLegendModule.legendGroup &&
      this.layout.layoutGroup &&
      !isNullOrUndefined(this.svgObject) &&
      !isNullOrUndefined(document.getElementById(this.layout.layoutGroup.id)) &&
      !isNullOrUndefined(
        document.getElementById(this.treeMapLegendModule.legendGroup.id)
      )
    ) {
      this.svgObject.insertBefore(
        this.layout.layoutGroup,
        this.treeMapLegendModule.legendGroup
      );
    }
  ...
  private renderBorder(): void {
    ...
    const borderElement: Element = this.svgObject.querySelector(
      '#' + this.element.id + '_TreeMap_Border'
    );
    if (
      (this.border.width > 0 ||
        this.background ||
        this.themeStyle.backgroundColor) &&
  ...
  private renderTitle(
    ...
    const width: number =
      this.availableSize.width - this.margin.right - this.margin.left;
    style.fontFamily = style.fontFamily || this.themeStyle.fontFamily;
    style.fontWeight = style.fontWeight || this.themeStyle.titleFontWeight;
    style.size =
      style.size ||
      (type === 'title'
        ? this.themeStyle.fontSize
    ...
    if (title.text) {
      ...
      const rect: Rect = isNullOrUndefined(bounds)
        ...
        : bounds;
      const location: Location = findPosition(
        rect,
        title.alignment,
        elementSize,
        type
      ...
      const options: TextOption = new TextOption(
        this.element.id + '_TreeMap_' + type,
        location.x,
        location.y,
        'start',
        trimmedTitle
  ...
  protected processingData(): void {
    ...
    if (
      ...
      this.weightValuePath
    ) {
      this.treemapLevelData.levelsData = [];
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      this.dataSource.map((data: any) => {
        data[this.weightValuePath] = data[this.weightValuePath]
          ? (data[this.weightValuePath] as string).toString()
          : data[this.weightValuePath];
      });
      this.leafItemSettings.labelPath =
        this.leafItemSettings.labelPath || this.weightValuePath;
      ...
      if (this.levels.length === 0) {
        ...
        for (let i: number = 0; i < this.dataSource.length; i++) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const child: any[] = findChildren(
            this.dataSource[i as number] as any
          )['values'];
          if (this.isHierarchicalData && child && child.length > 0) {
            ...
            child.forEach((currentData: any) => {
              if (currentData[path as string]) {
                ...
                (<any[]>data[path as string]).push({
                  groupIndex: 0,
                  name: currentData[path as string],
                  levelOrderName: (
                    currentData[path as string] as string
                  ).toString(),
                  data: currentData,
                  weight: currentData[this.weightValuePath],
                });
              }
            });
          } else {
            if (this.dataSource[i as number][path as string]) {
              ...
              (<any[]>data[path as string]).push({
                groupIndex: 0,
                name: this.dataSource[i as number][path as string],
                levelOrderName: (
                  this.dataSource[i as number][path as string] as string
                ).toString(),
        ...
        if (this.isHierarchicalData) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          this.treemapLevelData.hierarchyData = extend(
            [],
            this.dataSource,
            this.treemapLevelData.hierarchyData,
            true
          ...
          for (
            let i: number = 0;
            i < this.treemapLevelData.hierarchyData.length;
            i++
          ) {
            this.processHierarchicalData(
              this.treemapLevelData.hierarchyData[i as number],
              i
            );
          }
          this.treemapLevelData.levelsData =
      ...
      if (!this.isHierarchicalData) {
        this.findTotalWeight(
          this.treemapLevelData.levelsData[0][path as string],
          'Parent'
        );
      }
    }
  ...
  private processHierarchicalData(data: any, dataCount: number): void {
    ...
    const path: string = this.leafItemSettings.labelPath
      ? this.leafItemSettings.labelPath
      : this.weightValuePath;
    let key: string;
    ...
    if (childData && childData.length > 0) {
      ...
      for (let j: number = 0; j < childData.length; j++) {
        ...
        childData[j as number]['levelOrderName'] =
          (levelIndex === 0
            ? childData[j as number]['name']
            : data['levelOrderName'] + '#' + childData[j as number]['name']) +
          '';
        const childItemLevel: string = childData[j as number]['levelOrderName'];
        let childLevel: number;
        if (childItemLevel.search('#') > 0) {
          childLevel = childItemLevel.split('#').length - 1;
        }
        childData[j as number]['groupIndex'] = isNullOrUndefined(levelIndex)
          ? childLevel === this.levels.length
            ? this.levels.length
            : childLevel
          : levelIndex;
        if (levelIndex !== 0) {
        ...
        childData[j as number]['weight'] =
          childData[j as number][this.weightValuePath];
      }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (dataCount === this.treemapLevelData.hierarchyData.length - 1) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      let mainData: any[] =
        this.treemapLevelData.hierarchyData[0][this.levels[0].groupPath];
      if (!isNullOrUndefined(mainData)) {
        for (
          let k: number = 0;
          k < this.treemapLevelData.hierarchyData.length;
          k++
        ) {
          childData = findChildren(
            this.treemapLevelData.hierarchyData[k as number]
          )['values'];
          if (k !== 0 && childData) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            childData.forEach((currentData: any) => {
              mainData.push(currentData);
            });
  ...
  private processFlatJsonData(): void {
    ...
    for (let i: number = 0; i < this.levels.length + 1; i++) {
      groupPath = this.levels[i as number]
        ? this.levels[i as number].groupPath
        : this.leafItemSettings.labelPath;
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const level: any = {};
      level['level'] = i;
      ...
      for (let j: number = 0; j < this.dataSource.length; j++) {
        ...
        if (!isNullOrUndefined(groupPath)) {
          const name: string =
            this.dataSource[j as number][groupPath as string];
          if (i !== 0) {
            for (let k: number = 0; k <= i; k++) {
              const childGroupPath: string = this.levels[k as number]
                ? this.levels[k as number].groupPath
                : groupPath;
              childName +=
                this.dataSource[j as number][childGroupPath as string] +
                (k === i ? '' : '#');
            }
          }
          if (
            !(orderNames.length > 0
              ? orderNames.indexOf(childName ? childName : name) !== -1
              : false)
          ) {
            ...
            (<any[]>(
              this.treemapLevelData.levelsData[
                (this.treemapLevelData.levelsData.length - 1) as number
              ][groupPath as string]
            )).push(currentData);
  ...
  public reOrderLevelData(start: number): void {
    ...
    const currentPath: string = this.levels[start as number]
      ? this.levels[start as number].groupPath
      : this.leafItemSettings.labelPath;
    const prevPath: string = this.levels[start - 1].groupPath;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const currentData: any[] = this.treemapLevelData.levelsData[
      start as number
    ][currentPath as string] as any[];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const previousData: any[] = this.treemapLevelData.levelsData[start - 1][
      prevPath as string
    ] as any[];
    for (let i: number = 0; i < currentData.length; i++) {
      ...
      for (let j: number = 0; j < previousData.length; j++) {
        previousData[j as number][currentPath as string] = isNullOrUndefined(
          previousData[j as number][currentPath as string]
        )
          ? []
          : previousData[j as number][currentPath as string];
        if (
          this.IsChildHierarchy(
            currentName.split('#'),
            (previousData[j as number]['levelOrderName'] as string).split('#')
          )
        ) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (<any[]>previousData[j as number][currentPath as string]).push(
            currentData[i as number]
          );
          break;
        }
      }
    ...
    this.findTotalWeight(
      this.treemapLevelData.levelsData[
        this.treemapLevelData.levelsData.length - 1
  ...
  private IsChildHierarchy(current: string[], previous: string[]): boolean {
    ...
    for (let i: number = 0; i < previous.length; i++) {
      if (
        current.length < i ||
        previous[i as number] !== current[i as number]
      ) {
        return false;
      } else {
        isChild = true;
    ...
    for (let i: number = 0; i < processData.length; i++) {
      ...
      (<any[]>this.dataSource).forEach((data: any) => {
        if (
          isContainsData(
            split,
            processData[i as number]['levelOrderName'],
            data,
            this
          )
        ) {
  ...
  private unWireEVents(): void {
    ...
    EventHandler.remove(
      ...
      this.mouseDownOnTreeMap
    );
    EventHandler.remove(
      this.element,
      Browser.touchMoveEvent,
      this.mouseMoveOnTreeMap
    );
    EventHandler.remove(
      this.element,
      Browser.touchEndEvent,
      this.mouseEndOnTreeMap
    );
    EventHandler.remove(
      this.element,
      'pointerleave mouseleave',
      this.mouseLeaveOnTreeMap
    );
  ...
  private addTabIndex(): void {
    this.element.setAttribute(
      'aria-label',
      this.description || 'TreeMap Element'
    );
  ...
  public resizeOnTreeMap(e: Event): void {
    if (!this.isDestroyed) {
      ...
      if (
        ...
        this.element.classList.contains('e-treemap')
      ) {
  ...
  public clickOnTreeMap(e: PointerEvent): void {
    ...
    const clickArgs: IClickEventArgs = {
      cancel: false,
      name: click,
      treemap: this,
      mouseEvent: e,
    ...
    if (targetId.indexOf('_Item_Index') > -1) {
      ...
      eventArgs = {
        ...
        name: itemClick,
        treemap: this,
        item: this.layout.renderItems[itemIndex as number],
        mouseEvent: e,
        groupIndex: this.layout.renderItems[itemIndex as number]['groupIndex'],
        groupName: this.layout.renderItems[itemIndex as number]['name'],
        text: labelText,
        contentItemTemplate: labelText,
      };
      this.trigger(
        itemClick,
        eventArgs,
        (observedArgs: IItemClickEventArgs) => {
          if (
            observedArgs.text !== labelText ||
            observedArgs.contentItemTemplate !== labelText
          ) {
            if (isNullOrUndefined(this.leafItemSettings.labelTemplate)) {
              observedArgs.text = textFormatter(
                observedArgs.text,
                observedArgs['item']['data'],
                observedArgs.treemap
              );
              ...
              setItemTemplateContent(
                targetId,
                targetEle,
                observedArgs.contentItemTemplate
              );
            }
          }
        }
  ...
  public doubleClickOnTreeMap(e: PointerEvent): void {
    const doubleClickArgs: IDoubleClickEventArgs = {
      cancel: false,
      name: doubleClick,
      treemap: this,
      mouseEvent: e,
  ...
  public rightClickOnTreeMap(e: PointerEvent): void {
    const rightClickArgs: IRightClickEventArgs = {
      cancel: false,
      name: rightClick,
      treemap: this,
      mouseEvent: e,
  ...
  public mouseMoveOnTreeMap(e: PointerEvent): void {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let item: any;
    const moveArgs: IMouseMoveEventArgs = {
      cancel: false,
      name: mouseMove,
      treemap: this,
      mouseEvent: e,
    ...
    if (targetId.indexOf('_Item_Index') > -1) {
      item =
        this.layout.renderItems[parseFloat(targetId.split('_Item_Index_')[1])];
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  public calculatePreviousLevelChildItems(
    ...
    let text1: string;
    const drillTextLevel: number =
      this.layout.renderItems[0]['levelOrderName'].split('#').length;
    for (let h: number = 0; h < drillTextLevel; h++) {
      text1 =
        h === 0
          ? drillLevelValues['levelText'][h as number]
          : text1 + '#' + drillLevelValues['levelText'][h as number];
    }
    ...
    for (levelItems of Object['values'](this.layout.renderItems)) {
      ...
      if (drillLevelText[0] === drillLevelValues['levelText'][0]) {
        text =
          p === 0
            ? isNullOrUndefined(text1 as string)
              ? text1
              : drillLevelValues['levelText'][p as number]
            : directLevel
            ? text1
            : text1 + '#' + drillLevelValues['levelText'][p as number];
        if (text === levelItems['levelOrderName']) {
          this.drilledItems.push({
            name: levelItems['levelOrderName'],
            data: levelItems,
          });
          p++;
          directLevel = true;
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  public compareSelectedLabelWithDrillDownItems(
    drillLevelValues: any,
    item: any,
    i: number
  ): any {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let drillLevelChild: any;
    const newDrillItem: any = {};
    ...
    if (b === this.drilledItems[i as number]['data']['groupIndex']) {
      ...
      newDrillItem[
        drillLevelChild[this.drilledItems[i as number]['data']['groupName']]
      ] = [drillLevelChild];
      // to remove all the items after matched drilled items
      this.drilledItems.splice(i, this.drilledItems.length);
    } else if (
      drillLevelValues['drillLevel'] === this.drilledItems.length - 1 ||
  ...
  public mouseEndOnTreeMap(e: PointerEvent): void {
    ...
    const targetId: string = targetEle.id;
    let totalRect: Rect;
    let startEvent: IDrillStartEventArgs;
    let endEvent: IDrillEndEventArgs;
    let directLevel: boolean = false;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let index: number;
    let newDrillItem: any = {};
    let item: any;
    const process: boolean = true;
    const layoutID: string =
      this.element.id + '_TreeMap_' + this.layoutType + '_Layout';
    let drillLevel: number;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const templateID: string = this.element.id + '_Label_Template_Group';
    let drillLevelValues: any;
    if (
      targetId.indexOf('_Item_Index') > -1 &&
      this.enableDrillDown &&
      !this.drillMouseMove
    ) {
      if (e.cancelable) {
      ...
      if (
        ...
        !item['isLeafItem'] &&
        findChildren(item)['values'] &&
        findChildren(item)['values'].length > 0
      ) {
        if (this.drilledItems.length > 0) {
          item = directLevel
            ? this.drilledItems[this.drilledItems.length - 1]['data']
            : item;
          for (let i: number = 0; i < this.drilledItems.length; i++) {
            if (!isNullOrUndefined(drillLevel)) {
              //Compare the selected text level with drilled items
              const drillLength: number = this.drilledItems.length;
              newDrillItem = this.compareSelectedLabelWithDrillDownItems(
                drillLevelValues,
                item,
                i
              );
            ...
            if (
              item['levelOrderName'] ===
                this.drilledItems[i as number]['name'] &&
              !directLevel &&
              isNullOrUndefined(drillLevel)
            ) {
              if (
                item['groupIndex'] === 0 &&
                item['parent'][item['groupName']] instanceof Array
              ) {
        ...
        startEvent = {
          ...
          groupName:
            ...
            this.drilledItems.length !== 0 &&
            !isNullOrUndefined(drillLevel)
              ? this.drilledItems[this.drilledItems.length - 1]['data']['name']
              : item['name'],
          rightClick: e.which === 3 ? true : false,
          childItems: null,
          item: newDrillItem,
        };
        this.trigger(
          drillStart,
          startEvent,
          (observedArgs: IDrillStartEventArgs) => {
            this.currentLevel =
              item['isDrilled'] && isNullOrUndefined(drillLevel)
                ? item['groupIndex']
                : !isNullOrUndefined(drillLevel) &&
                  this.enableBreadcrumb &&
                  item['isDrilled']
            ...
            if (!observedArgs.cancel) {
              if (document.getElementById(layoutID)) {
                const layerElementId: HTMLElement =
              ...
              if (
                this.legendSettings.visible &&
                !isNullOrUndefined(this.treeMapLegendModule)
              ) {
                if (!isNullOrUndefined(newDrillItem)) {
                  ...
                  if (this.enableBreadcrumb && !isNullOrUndefined(drillLevel)) {
                    this.drilledLegendItems = {
                      name: this.drilledItems[this.drilledItems.length - 1][
                        'data'
                      ]['levelOrderName'],
                      data: this.drilledItems[this.drilledItems.length - 1][
                        'data'
                      ],
                    };
                  } else {
                    this.drilledLegendItems = {
                      name: item['levelOrderName'],
                      data: item,
                    };
                  }
                  this.treeMapLegendModule.renderLegend();
                }
                totalRect = !isNullOrUndefined(this.totalRect)
                  ? this.totalRect
                  : totalRect;
              }
              if (document.getElementById(templateID)) {
                const drillElementId: HTMLElement =
                  document.getElementById(templateID);
                drillElementId.parentNode.removeChild(drillElementId);
              }
              if (
                !isNullOrUndefined(observedArgs.childItems) &&
                !observedArgs.cancel
              ) {
        ...
        endEvent = {
          ...
          name: drillEnd,
          treemap: this,
          renderItems: this.layout.renderItems,
        };
  ...
  public mouseLeaveOnTreeMap(e: PointerEvent): void {
    ...
    removeClassNames(
      document.getElementsByClassName('treeMapHighLight'),
      'treeMapHighLight',
      this
    );
