...
import { Tooltip, TooltipTheme } from '@hoyui/svg-base';
...
export class TreeMapTooltip {
    ...
    if (targetId.indexOf('_Item_Index') > -1) {
      ...
      if (!isNullOrUndefined(item)) {
        ...
        if (this.treemap.enableRtl) {
          tooltipContent = [
            (!isNullOrUndefined(this.tooltipSettings.format)
              ? textFormatter(
                  this.tooltipSettings.format,
                  toolTipData,
                  this.treemap
                )
              : null) ||
              formatValue(value, this.treemap) +
                ' : ' +
                this.treemap.weightValuePath.toString(),
          ];
        } else {
          tooltipContent = [
            (!isNullOrUndefined(this.tooltipSettings.format)
        ...
        if (document.getElementById(this.tooltipId)) {
          ...
          document
            .getElementById(this.treemap.element.id + '_Secondary_Element')
            .appendChild(tooltipEle);
        }
        location = getMousePosition(pageX, pageY, this.treemap.svgObject);
        location.y = this.tooltipSettings.template
          ? location.y + 10
          : location.y;
        this.tooltipSettings.textStyle.size =
          this.tooltipSettings.textStyle.size ||
          this.treemap.themeStyle.tooltipFontSize;
        this.tooltipSettings.textStyle.fontFamily =
          this.tooltipSettings.textStyle.fontFamily ||
          this.treemap.themeStyle.fontFamily;
        this.tooltipSettings.textStyle.fontStyle = !isNullOrUndefined(
          this.tooltipSettings.textStyle.fontStyle
        )
          ? this.tooltipSettings.textStyle.fontStyle
          : 'Normal';
        this.tooltipSettings.textStyle.fontWeight =
          this.tooltipSettings.textStyle.fontWeight ||
          this.treemap.themeStyle.fontWeight;
        ...
        this.tooltipSettings.textStyle.opacity =
          this.tooltipSettings.textStyle.opacity ||
          this.treemap.themeStyle.tooltipTextOpacity;
        const border: BorderModel = {
          width:
            this.tooltipSettings.border.width ||
            this.treemap.themeStyle.tooltipBorderWidth ||
            0,
          color:
            this.tooltipSettings.border.color ||
            this.treemap.themeStyle.tooltipBorderColor ||
            'transparent',
        };
        tootipArgs = {
          cancel: false,
          ...
          options: {
            ...
            data: toolTipData,
            border: border,
            textStyle: this.tooltipSettings.textStyle,
            template: this.tooltipSettings.template,
          },
        ...
        this.treemap.trigger(
          tooltipRendering,
          tootipArgs,
          (args?: ITreeMapTooltipRenderEventArgs) => {
  ...
  private addTooltip(
    tootipArgs: ITreeMapTooltipRenderEventArgs,
    markerFill: string,
    tooltipEle: HTMLElement,
    eventArgs?: ITreeMapTooltipArgs
  ): void {
    let cancel: boolean;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let args: any;
    if (!isNullOrUndefined(tootipArgs)) {
      const { cancel: c, ...otherArgs }: ITreeMapTooltipRenderEventArgs =
        tootipArgs;
      cancel = c;
    ...
    if (!cancel) {
      this.svgTooltip = new Tooltip({
        ...
        fill: this.treemap.tooltipSettings.fill
          ? this.treemap.tooltipSettings.fill
          : this.treemap.themeStyle.tooltipFillColor,
        border: args['border'],
        enableShadow: true,
      });
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      this.svgTooltip.opacity =
        this.treemap.themeStyle.tooltipFillOpacity || this.svgTooltip.opacity;
  ...
  public removeTooltip(): void {
    if (document.getElementsByClassName('EJ2-TreeMap-Tooltip').length > 0) {
      const tooltipElementId: Element = document.getElementsByClassName(
        'EJ2-TreeMap-Tooltip'
      )[0];
