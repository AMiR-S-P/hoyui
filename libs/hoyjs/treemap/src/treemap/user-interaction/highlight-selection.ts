...
export class TreeMapHighlight {
  ...
  public mouseMove(e: PointerEvent): boolean {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let eventArgs: IItemHighlightEventArgs;
    const items: any[] = [];
    ...
    let process: boolean;
    let treeMapElement: Element;
    let element: Element;
    let orders: string[];
    const selectionModule: TreeMapSelection =
      this.treemap.treeMapSelectionModule;
    if (
      targetId.indexOf('_Item_Index') > -1 &&
      (selectionModule ? this.treemap.selectionId !== targetId : true)
    ) {
      if (this.highLightId !== targetId) {
        treeMapElement = document.getElementById(
          treemap.element.id + '_TreeMap_' + treemap.layoutType + '_Layout'
        );
        const selectionElements: HTMLCollection =
          document.getElementsByClassName('treeMapSelection');
        item =
          this.treemap.layout.renderItems[
            parseFloat(targetId.split('_Item_Index_')[1])
          ];
        let index: number;
        if (this.treemap.legendSettings.visible) {
          ...
          const length: number =
            this.treemap.treeMapLegendModule.legendCollections.length;
          index = getLegendIndex(length, item, treemap);
          this.shapeElement =
            this.treemap.legendSettings.mode === 'Default'
              ? document.getElementById(
                  this.treemap.element.id + '_Legend_Shape_Index_' + index
                )
              : document.getElementById(
                  this.treemap.element.id + '_Legend_Index_' + index
                );
          if (
            this.shapeElement !== null &&
            (selectionModule
              ? this.shapeElement.getAttribute('id') !==
                selectionModule.legendSelectId
              : true)
          ) {
            if (
              selectionModule
                ? this.shapeElement !== selectionModule.shapeElement
                : true
            ) {
              this.currentElement.push({ currentElement: this.shapeElement });
              removeShape(this.shapeHighlightCollection);
              this.shapeHighlightCollection.push({
                legendEle: this.shapeElement,
                oldFill: collection[index as number]['legendFill'],
                oldOpacity: collection[index as number]['opacity'],
                oldBorderColor: collection[index as number]['borderColor'],
              ...
              this.currentElement.length > 0 &&
              this.currentElement[this.currentElement.length - 1][
                'currentElement'
              ] !== this.shapeElement
            ) {
              removeSelectionWithHighlight(
        ...
        if (
          this.treemap.legendSettings.visible
            ? selectionModule
              ? this.shapeElement
                ? this.shapeElement.getAttribute('id') !==
                  selectionModule.legendSelectId
      ...
      targetId.indexOf('_Legend_Shape') > -1 ||
      targetId.indexOf('_Legend_Index') > -1
    ) {
      if (
        this.treemap.legendSettings.visible &&
        (selectionModule
          ? selectionModule.legendSelectId !== targetId
          : true) &&
        ...
        const index: number =
          this.treemap.legendSettings.mode === 'Default'
            ? parseFloat(targetId.split('_Legend_Shape_Index_')[1])
            : parseFloat(targetId.split('_Legend_Index_')[1]);
        const dataLength: number =
          this.treemap.treeMapLegendModule.legendCollections[index as number][
            'legendData'
          ].length;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const collection: any[] =
        ...
        for (let i: number = 0; i < dataLength; i++) {
          for (
            let j: number = 0;
            j < this.treemap.layout.renderItems.length;
            j++
          ) {
            if (
              this.treemap.treeMapLegendModule.legendCollections[
                index as number
              ]['legendData'][i as number]['levelOrderName'] ===
              this.treemap.layout.renderItems[j as number]['levelOrderName']
            ) {
              itemIndex = j;
              groupIndex =
                this.treemap.layout.renderItems[j as number]['groupIndex'];
              const nodeEle: Element = document.getElementById(
                this.treemap.element.id +
                  '_Level_Index_' +
                  groupIndex +
                  '_Item_Index_' +
                  itemIndex +
              ...
              if (i === 0) {
                ...
                this.legendHighlightCollection[length - 1]['ShapeCollection'] =
                  { Elements: [] };
              }
              setColor(
                targetEle,
      ...
      if (
        ...
        if (
          selectionModule
            ? this.shapeElement !== selectionModule.shapeElement
            : true && this.treemap.legendSettings.visible
        ) {
          removeClassNames(
            document.getElementsByClassName('treeMapHighLight'),
            'treeMapHighLight',
      ...
      if (
        (this.shapeTarget === 'highlight' || this.target === 'highlight') &&
        this.treemap.legendSettings.visible
      ) {
        if (
          selectionModule
          ...
          if (
            selectionModule
              ? this.shapeElement !== selectionModule.shapeElement
              : true && selectionModule
              ? selectionModule.legendSelect
              : true
          ) {
            removeShape(this.shapeHighlightCollection);
            this.shapeHighlightCollection = [];
...
export class TreeMapSelection {
  ...
  public mouseDown(e: PointerEvent): void {
    ...
    const highlightModule: TreeMapHighlight =
      this.treemap.treeMapHighlightModule;
    ...
    if (targetId.indexOf('_Item_Index') > -1) {
      ...
      if (this.treemap.selectionId !== targetId && this.legendSelect) {
        ...
        for (let k: number = 0; k < selectionElements.length; k++) {
          ...
          if (highLightElements.length > 0) {
            for (let j: number = 0; j < highLightElements.length; j++) {
              if (highLightElements[j as number].id === element.id) {
                highLightElements[j as number].classList.remove(
                  'treeMapHighLight'
                );
              }
              applyOptions(element.childNodes[0] as SVGPathElement, {
          ...
          treemap.trigger(
            itemSelected,
            eventArgs,
            (observedArgs: IItemSelectedEventArgs) => {
      ...
      if (
        ...
        for (let k: number = 0; k < dataLength; k++) {
          for (
            let l: number = 0;
            l < this.treemap.layout.renderItems.length;
            l++
          ) {
            if (
              this.treemap.treeMapLegendModule.legendCollections[
                index as number
              ]['legendData'][k as number]['levelOrderName'] ===
              this.treemap.layout.renderItems[l as number]['levelOrderName']
            ) {
              itemIndex = l;
              groupIndex =
                this.treemap.layout.renderItems[l as number]['groupIndex'];
              const nodeEle: Element = document.getElementById(
                this.treemap.element.id +
                  '_Level_Index_' +
                  groupIndex +
              ...
              if (k === 0) {
                ...
                this.legendSelectionCollection[length - 1]['ShapeCollection'] =
                  { Elements: [] };
              }
              setColor(
                targetEle,
  ...
  public selectTreemapItem(levelOrder: string, enable: boolean): void {
    if (enable) {
      ...
      for (let s: number = 0; s < this.treemap.layout.renderItems.length; s++) {
        if (
          levelOrder ===
          this.treemap.layout.renderItems[s as number]['levelOrderName']
        ) {
      ...
      const layoutID: string =
        this.treemap.element.id +
        '_TreeMap_' +
        this.treemap.layoutType +
        '_Layout';
      const treeMapElement: Element = document.getElementById(layoutID);
      const orders: string[] = findHightLightItems(
        item,
        [],
        selection.mode,
        this.treemap
      ...
      const selectionElement: Element = document.getElementById(
        this.treemap.levelSelection[0]
      );
      ...
      for (let k: number = 0; k < selectionElements.length; k++) {
        ...
        if (highLightElements.length > 0) {
          ...
          selection.fill =
            selection.fill === 'null'
              ? this.treemap.layout.renderItems[
                  parseInt(element.id.split('Item_Index_')[1], 10)
                ]['options']['fill']
              : selection.fill;
          applyOptions(element.childNodes[0] as SVGPathElement, {
            border: selection.border,
