...
import { isNullOrUndefined, createElement, extend } from '@hoyui/base';
...
export class LayoutPanel {
  ...
  public processLayoutPanel(): void {
    ...
    let totalRect: Rect;
    if (
      this.treemap.treemapLevelData.levelsData &&
      this.treemap.treemapLevelData.levelsData.length > 0
    ) {
      data =
        !isNullOrUndefined(this.treemap.initialDrillDown.groupIndex) &&
        !isNullOrUndefined(this.treemap.initialDrillDown.groupName) &&
        (isNullOrUndefined(this.treemap.drilledItems)
          ? isNullOrUndefined(this.treemap.drilledItems)
          : this.treemap.drilledItems.length === 0)
          ? this.getDrilldownData(
              this.treemap.treemapLevelData.levelsData[0],
              []
            )[0]
          : this.treemap.treemapLevelData.levelsData[0];
      totalRect = extend({}, this.treemap.areaRect, totalRect, false) as Rect;
      if (
        !isNullOrUndefined(this.treemap.treeMapLegendModule) &&
        !isNullOrUndefined(this.treemap.totalRect)
      ) {
      ...
      if (
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const y: any = {};
        y[this.treemap.drilledItems[count as number]['data']['groupName']] = [
          x,
        ];
        if (
          !isNullOrUndefined(this.treemap.initialDrillDown.groupIndex) &&
          !this.treemap.enableBreadcrumb
        ) {
          this.treemap.currentLevel =
            this.treemap.drilledItems[count as number]['data']['groupIndex'];
        }
        this.calculateLayoutItems(
          y || this.treemap.treemapLevelData.levelsData[0],
          totalRect
        );
        ...
        this.calculateLayoutItems(
          data || this.treemap.treemapLevelData.levelsData[0],
          totalRect
        );
        this.renderLayoutItems();
      }
  ...
  private getDrilldownData(data: any, drillData: any[]): any {
    ...
    if (child && child.length > 0 && drillData.length === 0) {
      for (let i: number = 0; i < child.length; i++) {
        if (
          child[i as number]['groupIndex'] ===
            treemap.initialDrillDown.groupIndex &&
  ...
  private computeSliceAndDiceDimensional(data: any, coords: Rect): any {
    const leafItem: LeafItemSettings = this.treemap
      .leafItemSettings as LeafItemSettings;
    let rect: Rect;
    const groups: LevelSettings[] = this.treemap.levels as LevelSettings[];
    let groupIndex: number;
    let isLeafItem: boolean = false;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (children && children.length > 0) {
      ...
      if (this.treemap.levels.length > 0) {
        for (let i: number = 0; i < children.length; i++) {
          ...
          gap = isLeafItem
            ? leafItem.gap
            : groups[groupIndex as number].groupGap;
          headerHeight =
            groups.length === 0
              ? 0
              : groups[groupIndex as number]
              ? groups[groupIndex as number].showHeader
                ? groups[groupIndex as number].headerHeight
                : 0
              : groups[groupIndex - 1].showHeader
              ? groups[groupIndex - 1].headerHeight
              : 0;
          rect = children[i as number]['rect'];
          rect = new Rect(
            rect.x + gap / 2,
            rect.y + (headerHeight + gap / 2),
            rect.width - gap,
  ...
  private sliceAndDiceProcess(processData: any[], rect: Rect): void {
    ...
    let alottedValue: number = 0;
    let totalWeight: number = 0;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    processData.forEach((data: any) => {
      totalWeight += data['weight'];
    });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    processData.forEach((child: any) => {
      child['weightArea'] =
        (parentArea * (child['weight'] as number)) / totalWeight;
    });
    const isHorizontal: boolean =
      this.treemap.layoutType === 'SliceAndDiceAuto'
    ...
    for (let i: number = 0; i < processData.length; i++) {
      ...
      item['isLeafItem'] =
        levels.length === 0 ||
        (this.treemap.isHierarchicalData ||
        isNullOrUndefined(this.treemap.leafItemSettings.labelPath)
          ? item['groupIndex'] === levels.length - 1
          : item['groupIndex'] === this.treemap.levels.length);
      if (isHorizontal) {
        childValue =
          ((parentArea / totalWeight) * processData[i as number]['weight']) /
          rect.height;
        if (alottedValue <= rect.width) {
          processData[i as number]['rect'] = new Rect(
            alottedValue + rect.x,
            rect.y,
            childValue,
            rect.height
  ...
  private computeSquarifyDimensional(data: any, coords: Rect): void {
    const leaf: LeafItemSettings = this.treemap
      .leafItemSettings as LeafItemSettings;
    let rect: Rect;
    const levels: LevelSettings[] = this.treemap.levels as LevelSettings[];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let item: any;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const child: any[] = findChildren(data)['values'];
      ...
      if (this.treemap.levels.length > 0) {
        for (let i: number = 0; i < child.length; i++) {
          ...
          padding =
            (item['isLeafItem']
              ? leaf.padding
              : levels[index as number].groupPadding) / 2;
          headerHeight = this.treemap.isHierarchicalData
            ? index === 0 && item['isLeafItem']
              ? 0
              : levels[index as number]
              ? levels[index as number].showHeader
                ? levels[index as number].headerHeight
                : 0
              : 0
            : levels.length === 0
            ? 0
  ...
  private aspectRatio(
    ...
    if (currentRow.length === 0) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const newRow: any[] = currentRow.concat(nextDatum);
      const currentMaxAspectRatio: number = this.findMaxAspectRatio(
        currentRow,
        length
      );
  ...
  private findMaxAspectRatio(row: any[], length: number): number {
    ...
    const result: number = Math.max(
      (Math.pow(length, 2) * maxArea) / Math.pow(sumArea, 2),
      Math.pow(sumArea, 2) / (Math.pow(length, 2) * minArea)
    );
    return result;
  }
  ...
  private getCoordinates(row: any[], rect: Rect): any[] {
    ...
    const areaWidth: number = valueSum / height;
    const areaHeight: number = valueSum / width;
    let subXOffset: number = xOffset;
    let subYOffset: number = yOffset;
    let padding: number;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    for (let i: number = 0; i < rowLength; i += 1) {
      ...
      if (
        item['weight'] > 0 &&
        (isParent ||
          (Math.round(rect.y + padding / 2) <=
            Math.round(parentRect.y + (parentRect.height - parentRect.y)) &&
            Math.round(rect.x + padding / 2) <=
  ...
  public onDemandProcess(childItems: any): void {
    ...
    this.treemap.currentLevel = parentItem['isDrilled']
      ? parentItem['groupIndex']
      : null;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  public renderLayoutItems(): void {
    ...
    const treeMap: TreeMap = this.treemap;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let txtVisible: boolean;
    let getItemColor: any;
    let eventArgs: IItemRenderingEventArgs;
    this.renderer = treeMap.renderer;
    let pathOptions: PathOption;
    const elementID: string = treeMap.element.id;
    let index: number;
    let templatePosition: LabelPosition;
    const mode: string = treeMap.layoutType;
    let rect: Rect;
    let format: string;
    const interSectAction: LabelAlignment =
      this.treemap.leafItemSettings.interSectAction;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let fill: string;
    let item: any;
    ...
    let opacity: number;
    let rectPath: string = '';
    const secondaryEle: HTMLElement = document.getElementById(
      treeMap.element.id + '_Secondary_Element'
    );
    let groupId: string;
    let templateEle: HTMLElement;
    let gap: number;
    let textStyle: Font;
    const levels: LevelSettings[] = treeMap.levels as LevelSettings[];
    this.layoutGroup = this.renderer.createGroup({
      id: elementID + '_TreeMap_' + mode + '_Layout',
    });
    let itemGroup: Element;
    ...
    templateGroup.style.cssText =
      ...
      'px;';
    let isLeafItem: boolean = false;
    const leaf: LeafItemSettings = treeMap.leafItemSettings as LeafItemSettings;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let childItems: any[];
    ...
    for (let i: number = 0; i < this.renderItems.length; i++) {
      ...
      index = item['groupIndex'];
      if (
        (this.treemap.drillDownView &&
          isNullOrUndefined(this.treemap.currentLevel) &&
          index > 0) ||
        (this.treemap.drillDownView && index > this.treemap.currentLevel + 1)
      ) {
        continue;
      }
      rect = item['rect'];
      ...
      const treemapItemRect: Rect = this.treemap.totalRect
        ? convertToContainer(this.treemap.totalRect)
        : this.treemap.areaRect;
      ...
      if (treeMap.renderDirection === 'TopRightBottomLeft') {
        rect.x =
          treemapItemRect.x +
          treemapItemRect.width -
          rect.width -
          Math.abs(treemapItemRect.x - rect.x);
      } else if (treeMap.renderDirection === 'BottomLeftTopRight') {
        rect.y =
          treemapItemRect.y +
          treemapItemRect.height -
          rect.height -
          Math.abs(treemapItemRect.y - rect.y);
      ...
      format = isLeafItem
        ? leaf.labelFormat
        : levels[index as number].headerFormat;
      let levelName: string;
      txtVisible = isLeafItem
        ? leaf.showLabels
        : levels[index as number].showHeader;
      ...
      renderText =
        textFormatter(format, item['data'], this.treemap) ||
        levelName ||
        'undefined';
      ...
      renderText =
        ...
        this.treemap.enableDrillDown
          ...
          : renderText;
      textStyle = (
        isLeafItem ? leaf.labelStyle : levels[index as number].headerStyle
      ) as Font;
      textStyle.fontFamily =
        this.treemap.themeStyle.labelFontFamily || textStyle.fontFamily;
      textStyle.fontWeight =
        textStyle.fontWeight || this.treemap.themeStyle.fontWeight;
      ...
      position = !isLeafItem
        ? levels[index as number].headerAlignment === 'Near'
          ? 'TopLeft'
          : levels[index as number].headerAlignment === 'Center'
          ? 'TopCenter'
          : 'TopRight'
        : leaf.labelPosition;
      templatePosition = isLeafItem
        ? leaf.templatePosition
        : levels[index as number].templatePosition;
      template = isLeafItem
        ? leaf.labelTemplate
        : levels[index as number].headerTemplate;
      ...
      this.treemap.trigger(
        itemRendering,
        eventArgs,
        (observedArgs: IItemRenderingEventArgs) => {
          if (!observedArgs.cancel) {
            ...
            if (txtVisible) {
              if (eventArgs.text !== renderText) {
                eventArgs.text =
                  textFormatter(eventArgs.text, item['data'], this.treemap) ||
                  levelName;
              }
              this.renderItemText(
                ...
                groupId,
                fill,
                position as LabelPosition,
                connectorText
              );
            }
            if (template) {
              templateEle = this.renderTemplate(
                secondaryEle,
                groupId,
                rect,
                templatePosition,
            ...
            if (
              (this.treemap.enableDrillDown && isLeafItem) ||
              this.treemap.selectionSettings.enable ||
              this.treemap.highlightSettings.enable
            ) {
              itemGroup.setAttribute('role', 'button');
              itemGroup.setAttribute(
                'tabindex',
                this.treemap.tabIndex.toString()
              );
              (itemGroup as HTMLElement).style.cursor =
                this.treemap.highlightSettings.enable &&
                !this.treemap.selectionSettings.enable &&
                this.treemap.enableDrillDown &&
                item['groupIndex'] === this.treemap.levels.length - 1
                  ? 'default'
                  : this.treemap.highlightSettings.enable &&
                    !this.treemap.selectionSettings.enable &&
                    !this.treemap.enableDrillDown
  ...
  private renderItemText(
    ...
    let customText: string | string[];
    const tspanText: string[] = [];
    let height: number = 0;
    let textName: string;
    textCollection = text.indexOf('<br>') !== -1 ? text.split('<br>') : null;
    customText = this.labelInterSectAction(
      rect,
      text,
      textStyle,
      interSectAction
    ...
    textSize = measureText(
      (textCollection && textCollection[0]) || customText[0],
      textStyle
    );
    if (this.treemap.enableRtl) {
      ...
      if (rect.width < labelSize.width && drillSymbolCount > 0) {
        ...
        customText['0'] =
          textTrim(
            rect.width - drillSymbolSize.width - padding,
            customText[0],
            textStyle
          ) + label;
      }
    }
    const textLocation: Location = findLabelLocation(
      rect,
      position,
      textSize,
      'Text',
    ...
    if (
      ...
      if (
        interSectAction === 'Wrap' &&
        customText.length !== tspanText.length &&
        tspanText.length
      ) {
    ...
    renderTextElement(
      textOptions,
      textStyle,
      textStyle.color || this.getSaturatedColor(fill),
      parentElement
    );
  }
  ...
  private getItemColor(isLeafItem: boolean, item: any): any {
    const treemap: TreeMap = this.treemap;
    let itemFill: string = isLeafItem
      ? treemap.leafItemSettings.fill
      : treemap.levels[item['groupIndex']].fill;
    let itemOpacity: number = isLeafItem
      ? treemap.leafItemSettings.opacity
      : treemap.levels[item['groupIndex']].opacity;
    if (!isNullOrUndefined(treemap.treemapLevelData.defaultLevelsData)) {
      if (treemap.treemapLevelData.defaultLevelsData.length > 0) {
        treemap.treemapLevelData.levelsData =
          treemap.treemapLevelData.defaultLevelsData;
      }
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const parentData: any[] = findChildren(
      treemap.treemapLevelData.levelsData[0]
    )['values'];
    const colorMapping: ColorMappingModel[] = isLeafItem
    ...
    if (colorMapping.length > 0) {
      ...
      const option: any = colorMap(
        colorMapping,
        item['data'][treemap.equalColorValuePath],
        item['data'][treemap.rangeColorValuePath]
      );
      if (!isNullOrUndefined(option)) {
        itemFill = !isNullOrUndefined(option['fill'])
          ? option['fill']
          : treemap.leafItemSettings.fill;
      ...
      for (let i: number = 0; i < parentData.length; i++) {
        if (
          (parentData[i as number]['levelOrderName'] as string) ===
          (item['levelOrderName'] as string).split('#')[0]
        ) {
          itemFill = !isNullOrUndefined(itemFill)
            ? itemFill
            : !isNullOrUndefined(treemap.colorValuePath)
            ? parentData[i as number]['data'][treemap.colorValuePath]
            : !isNullOrUndefined(treemap.palette) && treemap.palette.length > 0
            ? treemap.palette[i % treemap.palette.length]
            : '#808080';
        }
      }
    }
  ...
  private renderTemplate(
    ...
    const templateId: string = isLeafItem
      ? groupId + '_LabelTemplate'
      : groupId + '_HeaderTemplate';
    const baseTemplateId: string = isLeafItem
      ? '_LabelTemplate'
      : '_HeaderTemplate';
    if (
      isNullOrUndefined(template['prototype']) &&
      ...
      for (let i: number = 0; i < keys.length; i++) {
        ...
        template = template.replace(
          new regExp('{{:' + <string>keys[i as number] + '}}', 'g'),
          item['data'][keys[i as number].toString()]
        );
      }
    }
    ...
    if (
      !isNullOrUndefined(
        document.getElementById(this.treemap.element.id + '_Secondary_Element')
      )
    ) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      const templateElement: HTMLCollection = templateFn(
        ...
        this.treemap,
        template,
        this.treemap.element.id + baseTemplateId,
        false
      );
      ...
      const templateLocation: Location = findLabelLocation(
        rect,
        position,
        templateSize,
        'Template',
  ...
  private labelInterSectAction(
    ...
    const maxWidth: number = rect.width - 10;
    switch (alignment) {
      case 'Hide':
