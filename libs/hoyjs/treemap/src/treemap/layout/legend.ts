...
import { Browser, isNullOrUndefined, EventHandler, extend } from '@hoyui/base';
import { SvgRenderer, GradientColor, LinearGradient } from '@hoyui/svg-base';
...
export class TreeMapLegend {
  ...
  public renderLegend(): void {
    ...
    const eventArgs: ILegendRenderingEventArgs = {
      ...
      _changePosition: this.treemap.legendSettings.position,
      position: this.treemap.legendSettings.position,
    };
    this.treemap.trigger(
      legendRendering,
      eventArgs,
      (observedArgs: ILegendRenderingEventArgs) => {
        if (
          !observedArgs.cancel &&
          observedArgs._changePosition !== this.treemap.legendSettings.position
        ) {
  ...
  public calculateLegendBounds(): void {
    ...
    if (
      ((!isNullOrUndefined(this.treemap.palette) &&
        this.treemap.palette.length > 0) ||
    ...
    if (this.legendCollections.length > 0) {
      ...
      this.legendCollections.sort((firstItem, nextItem) =>
        firstItem.levelIndex > nextItem.levelIndex
          ? 1
          : firstItem.levelIndex < nextItem.levelIndex
          ? -1
          : 0
      ...
      // eslint-disable-next-line @typescript-eslint/tslint/config
      this.legendCollections.sort((firstItem, nextItem) =>
        firstItem.groupIndex > nextItem.groupIndex
          ? 1
          : firstItem.groupIndex < nextItem.groupIndex
          ? -1
          : 0
      ...
      // eslint-disable-next-line @typescript-eslint/tslint/config
      this.legendCollections.sort((firstItem, nextItem) =>
        firstItem.leafIndex > nextItem.leafIndex
          ? 1
          : firstItem.leafIndex < nextItem.leafIndex
          ? -1
          : 0
      ...
      let textX: number = 0;
      let textY: number = 0;
      const shapeHeight: number = legend.shapeHeight;
      const shapeWidth: number = legend.shapeWidth;
      let shapeLocation: Location[] = [];
      let textLocation: Rect[] = [];
      const orientation: LegendOrientation =
        legend.orientation === 'None'
          ? position === 'Top' ||
            position === 'Bottom' ||
            (position === 'Auto' &&
              treemap.availableSize.width <= treemap.availableSize.height)
            ? 'Horizontal'
            : 'Vertical'
          : legend.orientation;
      const leftPadding: number = 10;
      const topPadding: number = 10;
      const spacing: number = 10;
      let legendWidth: number =
        legend.width.length > 1
          ? legend.width.indexOf('%') > -1
            ? (treemap.availableSize.width / 100) * parseFloat(legend.width)
            : parseFloat(legend.width)
          : null;
      let legendHeight: number =
        legend.height.length > 1
          ? legend.height.indexOf('%') > -1
            ? (treemap.availableSize.height / 100) * parseFloat(legend.height)
            : parseFloat(legend.height)
          : null;
      titleTextStyle.fontFamily =
        titleTextStyle.fontFamily || treemap.themeStyle.fontFamily;
      titleTextStyle.fontWeight =
        titleTextStyle.fontWeight || treemap.themeStyle.titleFontWeight;
      titleTextStyle.size =
        titleTextStyle.size || treemap.themeStyle.subtitleFontSize;
      ...
      itemTextStyle.size =
        itemTextStyle.size || treemap.themeStyle.legendFontSize;
      itemTextStyle.fontFamily =
        itemTextStyle.fontFamily || treemap.themeStyle.fontFamily;
      itemTextStyle.fontWeight =
        itemTextStyle.fontWeight || treemap.themeStyle.fontWeight;
      if (legendMode === 'Default') {
        legendWidth = isNullOrUndefined(legendWidth)
          ? treemap.areaRect.width
          : legendWidth;
        legendHeight = isNullOrUndefined(legendHeight)
          ? treemap.areaRect.height
          : legendHeight;
        let j: number = 0;
        for (let i: number = 0; i < this.legendCollections.length; i++) {
          ...
          const legendTextSize: Size = measureText(
          ...
          if (i === 0) {
            ...
            startY = shapeY =
              ...
              (shapeHeight > legendTextSize.height
                ? shapeHeight / 2
                : legendTextSize.height / 4);
          } else {
            const maxSize: number =
              legendTextSize.height > shapeHeight
                ? legendTextSize.height
                : shapeHeight;
            if (orientation === 'Horizontal') {
              const prvePositionX: number =
                textLocation[j - 1].x +
                textLocation[j - 1].width +
                textPadding +
                shapeWidth;
              if (
                prvePositionX + shapePadding + legendTextSize.width >
                legendWidth
              ) {
                const nextPositionY: number =
                  (textLocation[j - 1].y >
                  shapeLocation[j - 1].y + shapeHeight / 2
                    ? textLocation[j - 1].y
                    : shapeLocation[j - 1].y + shapeHeight / 2) + topPadding;
              ...
              const prevPositionY: number =
                textLocation[j - 1].y > shapeLocation[j - 1].y + shapeHeight / 2
                  ? textLocation[j - 1].y
                  : shapeLocation[j - 1].y + shapeHeight / 2;
              if (prevPositionY + topPadding + maxSize > legendHeight) {
                const nextPositionX: number =
                  textLocation[j - 1].x + this.textMaxWidth + textPadding;
                if (
                  nextPositionX + shapePadding + legendTextSize.width >
                  legendWidth
                ) {
                  shapeX = startX;
                  shapeY = startY;
          ...
          (<any[]>this.totalPages[this.page]['Collection']).push({
            ...
            element: legendItem['gradientElement'],
            ...
            Rect: {
              ...
              y:
                shapeLocation[j as number].y - shapeHeight / 2 <
                textY - legendTextSize.height
                  ? shapeLocation[j as number].y - shapeHeight / 2
                  : textY - legendTextSize.height,
              width: Math.abs(
                shapeLocation[j as number].x -
                  shapeWidth / 2 -
                  (textX + legendTextSize.width)
              ),
              height:
                shapeHeight > legendTextSize.height
                  ? shapeHeight
        ...
        collection.forEach((legendObj: any, index: number) => {
          const legendRect: Rect = new Rect(
            legendObj['Rect']['x'],
            legendObj['Rect']['y'],
            legendObj['Rect']['width'],
            legendObj['Rect']['height']
          );
          if (index === 0) {
            startX = legendRect.x;
            startY = legendRect.y;
          }
          this.widthIncrement = Math.max(
            this.widthIncrement,
            Math.abs(startX - (legendRect.x + legendRect.width))
          );
          this.heightIncrement = Math.max(
            this.heightIncrement,
            Math.abs(startY - (legendRect.y + legendRect.height))
          );
        });
        legendWidth =
          this.widthIncrement < legendWidth ? this.widthIncrement : legendWidth;
        legendHeight =
          this.heightIncrement < legendHeight
            ? this.heightIncrement
            : legendHeight;
        this.legendItemRect = {
        ...
        const rectWidth: number =
          orientation === 'Horizontal'
            ? isNullOrUndefined(legendWidth)
              ? treemap.areaRect.width / legendLength
        ...
        const rectHeight: number =
          orientation === 'Horizontal'
            ? isNullOrUndefined(legendHeight)
              ? defaultSize
              : legendHeight
            : isNullOrUndefined(legendHeight)
            ? treemap.areaRect.height / legendLength
            : legendHeight / legendLength;
        startX = 0;
        startY = legendTitleSize.height + spacing;
        const textPadding: number = 10;
        const placement: LabelPlacement = legend.labelPosition;
        let itemStartX: number = 0;
        let itemStartY: number = 0;
        ...
        for (let i: number = 0; i < this.legendCollections.length; i++) {
          ...
          let legendText: string =
            this.legendCollections[i as number]['legendName'];
          let itemTextSize: Size = new Size(0, 0);
          if (labelAction === 'None') {
            itemTextSize = measureText(legendText, itemTextStyle);
          } else if (labelAction === 'Trim') {
            legendText = textTrim(
              orientation === 'Horizontal' ? rectWidth : rectHeight,
              legendText,
              itemTextStyle
            );
            itemTextSize = measureText(legendText, itemTextStyle);
          ...
          if (itemTextSize.width > 0 && itemTextSize.height > 0) {
            if (orientation === 'Horizontal') {
              ...
              textY =
                placement === 'After'
                  ? startY + rectHeight + itemTextSize.height / 2 + textPadding
                  : startY - textPadding;
            } else {
              textX =
                placement === 'After'
                  ? startX - itemTextSize.width / 2 - textPadding
          ...
          if (i === 0) {
            itemStartX =
              orientation === 'Horizontal'
                ? startX
                : placement === 'After'
                ? textX - itemTextSize.width / 2
                : startX;
            itemStartY =
              orientation === 'Horizontal'
                ? placement === 'After'
                  ? startY
                  : textY - itemTextSize.height / 2
                : startY;
          }
          if (i === legendLength - 1) {
            legendWidth =
              orientation === 'Horizontal'
                ? Math.abs(startX + rectWidth - itemStartX)
                : rectWidth + maxTextWidth + textPadding;
            legendHeight =
              orientation === 'Horizontal'
                ? rectHeight + maxTextHeight / 2 + textPadding
                : Math.abs(startY + rectHeight - itemStartY);
          }
          this.legendRenderingCollections.push({
            fill: this.legendCollections[i as number]['legendFill'],
            x: startX,
            y: startY,
            width: rectWidth,
            height: rectHeight,
            element: this.legendCollections[i as number]['gradientElement'],
            text: legendText,
            textX: textX,
            textY: textY,
  ...
  private findPaletteLegendItems(data: any): void {
    ...
    if (!isNullOrUndefined(this.treemap.drilledItems)) {
      if (
        ...
        for (let k: number = 0; k < items.length; k++) {
          if (
            items[k as number]['Name'] ===
            this.treemap.initialDrillDown.groupName
          ) {
            items[k as number]['isDrilled'] = !items[k as number]['isDrilled'];
            data = items[k as number];
            this.treemap.currentLevel =
              this.treemap.initialDrillDown.groupIndex;
            legendFillColor =
              this.treemap.palette.length > 0
                ? this.treemap.palette[k % this.treemap.palette.length]
    ...
    if (child && child.length > 0) {
      for (let i: number = 0; i < child.length; i++) {
        if (
          isNullOrUndefined(
            child[i as number]['data'][
              this.treemap.legendSettings.showLegendPath
            ]
          ) ||
          child[i as number]['data'][this.treemap.legendSettings.showLegendPath]
        ) {
          legendName = child[i as number]['data'][
            this.treemap.legendSettings.valuePath
          ]
            ? child[i as number]['data'][this.treemap.legendSettings.valuePath]
            : child[i as number]['name'];
          isDuplicate = this.treemap.legendSettings.removeDuplicateLegend
            ? this.removeDuplicates(this.legendCollections, legendName)
            : false;
          if (!isDuplicate) {
            this.legendCollections.push({
              ...
              legendFill:
                this.treemap.palette.length > 0
                  ? !isNullOrUndefined(this.treemap.currentLevel)
                    ? legendFillColor
                    : this.treemap.palette[i % this.treemap.palette.length]
      ...
      if (this.treemap.palette.length > 0) {
        for (let j: number = 0; j < this.legendCollections.length; j++) {
          this.legendCollections[j as number]['legendFill'] =
            !isNullOrUndefined(this.treemap.currentLevel)
              ? legendFillColor
              : this.treemap.palette[j % this.treemap.palette.length];
        }
      }
    }
  }
  ...
  private calculateLegendItems(data: any[]): void {
    ...
    for (let i: number = 0; i < data.length; i++) {
      ...
      isLeafItem =
        this.treemap.levels.length === 0 ||
        groupIndex === this.treemap.levels.length;
      colorMapping = isLeafItem
        ? leaf.colorMapping
        : levels[groupIndex as number].colorMapping;
      for (let j: number = 0; j < colorMapping.length; j++) {
        ...
        colorMap.value = !isNullOrUndefined(colorMap.value)
          ? colorMap.value.toString()
          : colorMap.value;
        if (
          !isNullOrUndefined(colorMap.from) &&
          !isNullOrUndefined(colorMap.to) &&
          rangeValue >= colorMap.from &&
          rangeValue <= colorMap.to &&
          colorMap.showLegend
        ) {
          colorMapProcess = true;
          isRange = true;
          actualValue = colorMap.from + ' - ' + colorMap.to;
          legendText = !isNullOrUndefined(colorMap.label)
            ? colorMap.label
            : colorMap.from + ' - ' + colorMap.to;
          fill = isNullOrUndefined(colorMap.color)
            ? fill
            : <string>colorMap.color;
          isAddData = this.isAddNewLegendData(actualValue);
        } else if (
          !isNullOrUndefined(colorMap.value) &&
          ...
          fill = isNullOrUndefined(colorMap.color)
            ? fill
            : Object.prototype.toString.call(colorMap.color) ===
              '[object Array]'
            ? colorMap.color[0]
            : <string>colorMap.color;
          isAddData = this.isAddNewLegendData(actualValue);
        ...
        if (
          ...
          isDesaturation = true;
          if (
            Object.prototype.toString.call(colorMap.color) === '[object Array]'
          ) {
            for (let q: number = 0; q < colorMap.color.length; q++) {
              ...
              const stop1Color: GradientColor = {
                colorStop: offsetValue.toString(),
                color: colorMap.color[q as number],
              };
              colors.push(stop1Color);
            }
          } else {
            const stop1Color: GradientColor = { colorStop: '0%', color: fill };
            const stop2Color: GradientColor = {
              colorStop: '100%',
              color: fill,
            };
            colors.push(stop1Color);
          ...
          const gradient: LinearGradient = {
            id:
              'groupIndex_' + groupIndex + '_colorIndex_' + this.gradientCount,
            x1: '0%',
            y1: '0%',
            x2: x2,
            y2: y2,
          };
          gradientElement = this.treemap.renderer.drawGradient(
            'linearGradient',
            gradient,
            colors
          ...
          if (
            ...
            (gradientElement.childNodes[0] as Element).setAttribute(
              'stop-opacity',
              colorMap.minOpacity.toString()
            );
            (gradientElement.childNodes[1] as Element).setAttribute(
              'stop-opacity',
              colorMap.maxOpacity.toString()
            );
          }
        ...
        isDuplicate = this.treemap.legendSettings.removeDuplicateLegend
          ? this.removeDuplicates(this.legendCollections, legendText)
          : false;
        if (
          isAddData &&
          isAddData['process'] &&
          ...
          fill =
            Object.prototype.toString.call(colorMap.color) ===
              '[object Array]' &&
            isNullOrUndefined(gradientElement) &&
            isNullOrUndefined(colorMap.value)
              ? this.legendGradientColor(colorMap, legendIndex)
              : fill;
          this.legendCollections.push({
            actualValue: actualValue,
            levelIndex: !isLeafItem ? j : -1,
            leafIndex: isLeafItem ? j : -1,
            legendName: legendText,
            legendFill: fill,
            legendData: [],
            groupIndex: !isLeafItem ? groupIndex : -1,
            gradientElement: !isNullOrUndefined(gradientElement)
              ? gradientElement
              : isNullOrUndefined(colorMap.value)
              ? this.legendLinearGradient
              : null,
            name: data[i as number]['name'],
            opacity: this.treemap.legendSettings.opacity,
            borderColor: this.treemap.legendSettings.border.color,
            borderWidth: this.treemap.legendSettings.border.width,
          });
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (<any[]>(
            this.legendCollections[this.legendCollections.length - 1][
              'legendData'
            ]
          )).push(data[i as number]);
          legendIndex++;
        } else if (colorMapProcess && !isDuplicate) {
          colorMapProcess = false;
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (<any[]>(
            this.legendCollections[isAddData['value']]['legendData']
          )).push(data[i as number]);
        }
        if (!isRange && !isDesaturation && !isEqualColor) {
          if (
            ...
            isNullOrUndefined(colorMap.maxOpacity) &&
            isNullOrUndefined(colorMap.value) &&
            ...
            outfill =
              ...
              '[object Array]'
                ? colorMap.color[0]
                : <string>colorMap.color;
            labelLegend = !isNullOrUndefined(colorMap.label)
              ? colorMap.label
              : 'Others';
            isDuplicate = this.treemap.legendSettings.removeDuplicateLegend
              ? this.removeDuplicates(this.legendCollections, labelLegend)
              : false;
            if (isNullOrUndefined(this.outOfRangeLegend) && !isDuplicate) {
              this.legendCollections.push({
                actualValue: labelLegend,
                legendData: [],
                legendName: labelLegend,
                legendFill: outfill,
              });
  ...
  public drawLegend(): void {
    const legend: LegendSettingsModel = <LegendSettingsModel>(
      this.treemap.legendSettings
    );
    const render: SvgRenderer = this.treemap.renderer;
    let fill: string;
    let textOptions: TextOption;
    let gradientElement: Element;
    const textFont: FontModel = legend.textStyle;
    this.legendGroup = render.createGroup({
      id: this.treemap.element.id + '_Legend_Group',
    });
    ...
    if (legend.mode === 'Default') {
      ...
      for (let i: number = 0; i < this.legendRenderingCollections.length; i++) {
        ...
        const textLocation: Location = new Location(
          item['textX'],
          item['textY']
        );
        ...
        if (this.treemap.enableRtl) {
          if (
            this.treemap.legendSettings.position === 'Left' ||
            this.treemap.legendSettings.position === 'Right' ||
            (this.treemap.legendSettings.position === 'Auto' &&
              this.treemap.availableSize.width >=
                this.treemap.availableSize.height)
          ) {
            rectOptions.y =
              this.translate.y +
              this.legendBorderRect.y +
              this.legendBorderRect.height -
              (this.translate.y + rectOptions.height) -
              Math.abs(this.legendBorderRect.y - rectOptions.y);
            textLocation.y =
              this.translate.y +
              this.legendBorderRect.y +
              this.legendBorderRect.height -
            ...
            rectOptions.x =
              this.translate.x +
              this.legendBorderRect.x +
              this.legendBorderRect.width -
            ...
            textLocation.x =
              ...
              this.legendBorderRect.width -
              this.translate.x -
              Math.abs(this.legendBorderRect.x - textLocation.x);
          }
        }
        ...
        renderTextElement(
          textOptions,
          textFont,
          textFont.color || this.treemap.themeStyle.legendTextColor,
          this.legendGroup
        );
  ...
  private defaultLegendRtlLocation(
    ...
    shapeLocation.x =
      ...
      this.legendBorderRect.x +
      this.legendBorderRect.width -
      (this.translate.x + spacing) -
      Math.abs(this.legendBorderRect.x - shapeLocation.x);
    textLocation.x =
      this.translate.x +
      this.legendBorderRect.x +
      this.legendBorderRect.width -
      (this.translate.x + textSize.width + spacing) -
      Math.abs(this.legendBorderRect.x - textLocation.x);
    if (
      ...
      shapeLocation.y =
        ...
        this.legendBorderRect.height -
        this.translate.y -
        Math.abs(Math.abs(this.legendBorderRect.y) - shapeLocation.y) -
        legend.shapeHeight / 2;
      textLocation.y =
        this.translate.y +
        this.legendBorderRect.y +
        this.legendBorderRect.height -
        this.translate.y -
        Math.abs(Math.abs(this.legendBorderRect.y) - textLocation.y);
    }
  ...
  private drawLegendItem(page: number): void {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let textOptions: TextOption;
    let legendRtlLocation: any;
    const render: SvgRenderer = treemap.renderer;
    const shapeBorder: BorderModel = legend.shapeBorder;
    ...
    if (page >= 0 && page < this.totalPages.length) {
      ...
      const isLineShape: boolean =
        legend.shape === 'HorizontalLine' ||
        legend.shape === 'VerticalLine' ||
        legend.shape === 'Cross';
      const strokeColor: string = isLineShape
        ? isNullOrUndefined(legend.fill)
          ? '#000000'
          : legend.fill
        : shapeBorder.color;
      const strokeWidth: number = isLineShape
        ? shapeBorder.width === 0
          ? 1
          : shapeBorder.width
        : shapeBorder.width;
      ...
      for (
        let i: number = 0;
        i < (<any[]>this.totalPages[page as number]['Collection']).length;
        i++
      ) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const collection: any = <any[]>(
          this.totalPages[page as number]['Collection'][i as number]
        );
        const legendText: string = collection['DisplayText'];
        const legendElement: Element = render.createGroup({
          id: treemap.element.id + '_Legend_Index_' + i,
        });
        ...
        if (treemap.enableRtl) {
          legendRtlLocation = this.defaultLegendRtlLocation(
            collection,
            spacing,
            treemap,
            legend
        ...
        treemap.trigger(
          legendItemRendering,
          eventArgs,
          (observedArgs: ILegendItemRenderingEventArgs) => {
            const renderOptions: PathOption = new PathOption(
              shapeId,
              observedArgs.fill,
              strokeWidth,
              isLineShape ? collection['Fill'] : strokeColor,
              legend.opacity,
              ''
            );
            ...
            renderTextElement(
              textOptions,
              legend.textStyle,
              legend.textStyle.color || treemap.themeStyle.legendTextColor,
              legendElement
            );
      ...
      const width: number = spacing;
      const height: number = spacing / 2;
      if (this.page !== 0) {
        ...
        const leftPageX: number =
          this.legendItemRect.x +
          this.legendItemRect.width -
          pagingTextSize.width -
          width * 2 -
          spacing;
        const rightPageX: number =
          this.legendItemRect.x + this.legendItemRect.width;
        const locY: number =
          this.legendItemRect.y +
          this.legendItemRect.height +
          height / 2 +
          spacing;
        const pageTextX: number =
          rightPageX - width - pagingTextSize.width / 2 - spacing / 2;
        pagingGroup = render.createGroup({
          id: treemap.element.id + '_Legend_Paging_Group',
        });
        const leftPageElement: Element = render.createGroup({
          id: treemap.element.id + '_Legend_Left_Paging_Group',
        });
        const rightPageElement: Element = render.createGroup({
          id: treemap.element.id + '_Legend_Right_Paging_Group',
        });
        ...
        const leftRectPageOptions: RectOption = new RectOption(
          treemap.element.id + '_Left_Page_Rect',
          'transparent',
          {},
          1,
          new Rect(
        ...
        const rightPageOptions: PathOption = new PathOption(
          treemap.element.id + '_Right_Page',
          '#a6a6a6',
          0,
          '#a6a6a6',
          1,
        ...
        const rightRectPageOptions: RectOption = new RectOption(
          treemap.element.id + '_Right_Page_Rect',
          'transparent',
          {},
          1,
        ...
        rightPageElement.appendChild(
          render.drawRectangle(rightRectPageOptions)
        );
  ...
  private renderLegendBorder(): void {
    ...
    const renderOptions: RectOption = new RectOption(
      treemap.element.id + '_Legend_Border',
      legend.background,
      borderStyle,
      1,
      this.legendBorderRect,
      ''
    );
    ...
    this.getLegendAlignment(
      treemap,
      this.legendBorderRect.width,
      this.legendBorderRect.height,
      legend
    );
    this.legendGroup.setAttribute(
      'transform',
      'translate( ' +
        (this.translate.x + -this.legendBorderRect.x) +
        ' ' +
  ...
  private renderLegendTitle(): void {
    ...
    let textOptions: TextOption;
    const spacing: number = 10;
    const trimTitle: string = textTrim(
      this.legendItemRect.width + spacing * 2,
      legendTitle,
      textStyle
    );
    ...
    if (legendTitle) {
      ...
      const textElement: Element = renderTextElement(
        textOptions,
        textStyle,
        textStyle.color || this.treemap.themeStyle.legendTitleColor,
        this.legendGroup
      );
  ...
  public renderInteractivePointer(e: PointerEvent | TouchEvent): void {
    ...
    if (
      target.id.indexOf('_Item_Index') > -1 &&
      legend.visible &&
      this.legendRenderingCollections.length > 0
    ) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      let currentData: any;
      let legendRect: ClientRect;
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      let data: any[];
      let fill: string;
      let stroke: string;
      ...
      targetItem =
        treemap.layout.renderItems[
          parseFloat(target.id.split('_Item_Index_')[1])
        ];
      const svgRect: ClientRect = treemap.svgObject.getBoundingClientRect();
      for (let i: number = 0; i < this.legendCollections.length; i++) {
        currentData = this.legendCollections[i as number];
        legendElement = document.getElementById(
          treemap.element.id + '_Legend_Index_' + i
        );
        legendRect = <ClientRect>legendElement.getBoundingClientRect();
        const rect: Rect = new Rect(
          Math.abs(legendRect.left - svgRect.left),
          Math.abs(legendRect.top - svgRect.top),
          legendRect.width,
          legendRect.height
        );
        ...
        if (!isNullOrUndefined(currentData['legendData'])) {
          ...
          for (let j: number = 0; j < data.length; j++) {
            if (
              data[j as number]['levelOrderName'] ===
              targetItem['levelOrderName']
            ) {
              this.drawInteractivePointer(
                legend,
                fill,
                stroke,
                interactiveId,
  ...
  private drawInteractivePointer(
    ...
    const direction: LegendOrientation =
      legend.orientation === 'None'
    ...
    this.treemap.svgObject.appendChild(
      this.treemap.renderer.drawPath(pathOptions) as SVGPathElement
    );
  }
  ...
  public mouseUpHandler(e: PointerEvent): void {
    ...
    this.clearTimeout = setTimeout(
      this.removeInteractivePointer.bind(this),
      3000
    );
  }
  // eslint-disable-next-line valid-jsdoc
  ...
  public removeInteractivePointer(): void {
    if (
      document.getElementById(this.treemap.element.id + '_Interactive_Legend')
    ) {
      const legendElementId: HTMLElement = document.getElementById(
        this.treemap.element.id + '_Interactive_Legend'
      );
  ...
  public changeNextPage(e: PointerEvent): void {
    this.currentPage =
      (<Element>e.target).id.indexOf('_Left_Page_') > -1
        ? this.currentPage - 1
        : this.currentPage + 1;
   ...
   * @private
   */
  public legendGradientColor(
    colorMap: ColorMappingModel,
    legendIndex: number
  ): string {
    let legendFillColor: string;
    ...
    if (
      ...
      const position: LegendPosition = this.treemap.legendSettings.position;
      const x2: string =
        position === 'Top' || position === 'Bottom' ? '100' : '0';
      const y2: string =
        position === 'Top' || position === 'Bottom' ? '0' : '100';
