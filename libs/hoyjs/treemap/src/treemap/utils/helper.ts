...
export function measureText(text: string, font: FontModel): Size {
  let measureObject: HTMLElement =
    document.getElementById('treeMapMeasureText');
...
export function renderTextElement(
  ...
  const text: string =
    typeof options.text === 'string'
      ? options.text
      : isMinus
      ? options.text[options.text.length - 1]
      : options.text[0];
  let tspanElement: Element;
  const renderer: SvgRenderer = new SvgRenderer('');
  let height: number;
  let htmlObject: HTMLElement;
  const breadCrumbText: boolean =
    !isNullOrUndefined(text) && !isNullOrUndefined(options.connectorText)
      ? text.search(options.connectorText[1]) >= 0
      : false;
  if (breadCrumbText) {
    ...
    for (let z: number = 0; z < drillLevelText.length; z++) {
      let drillText: string =
        drillLevelText[z as number].search(options.connectorText) !== -1 &&
        !isNullOrUndefined(options.connectorText)
          ? options.connectorText
          : drillLevelText[z as number];
      ...
      renderOptions['x'] =
        z !== 0
          ? renderOptions['x'] + size.width
          : renderOptions['x'] + size.width + spacing;
 ...
 * @returns {void}
 */
export function setItemTemplateContent(
  targetId: string,
 ...
 */
export function isContainsData(
  source: string[],
  pathName: string,
  processData: any,
  treemap: TreeMap
  ...
  let name: string = '';
  let path: string;
  const leaf: LeafItemSettingsModel = treemap.leafItemSettings;
  for (let i: number = 0; i < source.length; i++) {
    path = treemap.levels[i as number]
      ? treemap.levels[i as number].groupPath
      : leaf.labelPath
      ? leaf.labelPath
 ...
 */
export function findHightLightItems(
  data: any,
  items: string[],
  mode: string,
  treeMap: TreeMap
  ...
  if (mode === 'Child') {
    ...
    if (
      typeof data['levelOrderName'] === 'string' &&
      items.indexOf(data['levelOrderName']) === -1
    ) {
      items.push(data['levelOrderName']);
    ...
    for (let i: number = 0; i < treeMap.layout.renderItems.length; i++) {
      ...
      if (
        currentItem['levelOrderName'].indexOf(parentName) > -1 &&
        items.indexOf(currentItem['levelOrderName']) === -1
      ) {
...
export function convertElement(
  ...
  for (let i: number = 0; i < keys.length; i++) {
    ...
    templateHtml = templateHtml.replace(
      ...
      data[keys[i as number].toString()]
    );
  }
    ...
    Math.abs(
      x -
        (position.indexOf('Left') > -1
          ...
          : rect.x + rect.width - labelSize.width)
    ) - paddings;
  if (
    treemap.enableDrillDown &&
    ...
    location.y = Math.abs(
      y -
        (position.indexOf('Top') > -1
          ...
          ? type === 'Template'
            ? rect.y + (rect.height / 2 - labelSize.height / 2)
            : rect.y + rect.height / 2 + labelSize.height / 4
          : rect.y + rect.height - labelSize.height)
    );
  }
...
export function colorMap(
  ...
  for (let i: number = 0; i < colorMapping.length; i++) {
    let isEqualColor: boolean = false;
    const dataValue: number = <number>value;
    if (
      !isNullOrUndefined(colorMapping[i as number].from) &&
      !isNullOrUndefined(colorMapping[i as number].to) &&
      !isNullOrUndefined(colorMapping[i as number].value)
    ) {
      if (
        value >= colorMapping[i as number].from &&
        colorMapping[i as number].to >= value &&
        colorMapping[i as number].value === equalValue
      ) {
        isEqualColor = true;
        if (
          Object.prototype.toString.call(colorMapping[i as number].color) ===
          '[object Array]'
        ) {
          fill = !isEqualColor
            ? colorCollections(colorMapping[i as number], dataValue)
            : colorMapping[i as number].color[0];
        } else {
    ...
    if (
      ((value >= colorMapping[i as number].from &&
        value <= colorMapping[i as number].to) ||
        colorMapping[i as number].value === equalValue) &&
      !isNullOrUndefined(colorMapping[i as number].minOpacity) &&
      !isNullOrUndefined(colorMapping[i as number].maxOpacity) &&
      fill
    ) {
      opacity = deSaturationColor(colorMapping[i as number], value as number);
    }
    if (
      (fill === '' || isNullOrUndefined(fill)) &&
      isNullOrUndefined(colorMapping[i as number].from) &&
      isNullOrUndefined(colorMapping[i as number].to) &&
      isNullOrUndefined(colorMapping[i as number].minOpacity) &&
      isNullOrUndefined(colorMapping[i as number].maxOpacity) &&
      ...
      fill =
        Object.prototype.toString.call(colorMapping[i as number].color) ===
        '[object Array]'
          ? <string>colorMapping[i as number].color[0]
          : <string>colorMapping[i as number].color;
    }
...
export function deSaturationColor(
  ...
  if (rangeValue >= colorMapping.from && rangeValue <= colorMapping.to) {
    const ratio: number =
      (rangeValue - colorMapping.from) / (colorMapping.to - colorMapping.from);
    opacity =
      ratio * (colorMapping.maxOpacity - colorMapping.minOpacity) +
      colorMapping.minOpacity;
  }
...
export function getGradientColor(
  ...
  if (colorMap.color.length <= 2) {
    previousColor =
      colorMap.color[0].charAt(0) === '#'
        ? colorMap.color[0]
        : colorNameToHex(colorMap.color[0]);
    nextColor =
      colorMap.color[colorMap.color.length - 1].charAt(0) === '#'
        ? colorMap.color[colorMap.color.length - 1]
        : colorNameToHex(colorMap.color[colorMap.color.length - 1]);
  } else {
    previousColor =
      colorMap.color[0].charAt(0) === '#'
        ? colorMap.color[0]
    ...
    nextColor =
      colorMap.color[colorMap.color.length - 1].charAt(0) === '#'
        ? colorMap.color[colorMap.color.length - 1]
        : colorNameToHex(colorMap.color[colorMap.color.length - 1]);
    const a: number = full / (colorMap.color.length - 1);
    let b: number;
    let c: number;

    const length: number = colorMap.color.length - 1;
    const splitColorValueOffset: any[] = [];
    let splitColor: any = {};
...
export function hide(
  ...
  hideText =
    textSize.width > maxWidth || textSize.height > maxHeight ? ' ' : text;
  return hideText;
}
/**
 *
    ...
    for (let index: number = 0; index < elementId.length; index++) {
      if (element.getAttribute('id') === elementId[index as number]) {
        if (element.childElementCount > 0) {
          ...
          applyOptions(element.childNodes[0] as SVGPathElement, {
            border: treemap.selectionSettings.border,
            fill: treemap.selectionSettings.fill,
  ...
  if (elementId) {
    for (let i: number = 0; i < elementId.length; i++) {
      for (let j: number = 0; j < legendGroup.childElementCount; j++) {
        if (
          legendGroup.childNodes[j as number]['id'] === elementId[i as number]
        ) {
          const treemapSVGRectElement: SVGRectElement = <SVGRectElement>(
            legendGroup.childNodes[j as number]
          );
          treemapSVGRectElement.setAttribute(
            'fill',
            treemap.selectionSettings.fill
          );
          treemapSVGRectElement.setAttribute(
            'stroke',
            treemap.selectionSettings.border.color
          );
          treemapSVGRectElement.setAttribute(
            'stroke-width',
          ...
          treemapSVGRectElement.setAttribute(
            'opacity',
            treemap.selectionSettings.opacity
          );
        }
      }
    }
...
export function removeClassNames(
  ...
  for (let j: number = 0; j < elements.length; j++) {
    element = isNullOrUndefined(
      elements[j as number].childNodes[0] as SVGPathElement
    )
      ? (elements[j as number] as SVGPathElement)
      : (elements[j as number].childNodes[0] as SVGPathElement);
    options =
      treemap.layout.renderItems[
        parseFloat(element.id.split('_Item_Index_')[1])
      ]['options'];
...
export function textFormatter(
  ...
  for (const key of keys) {
    format = format
      .split('${' + key + '}')
      .join(formatValue(data[key as string], treemap).toString());
  }
  return format;
}
/**
...
export function formatValue(value: number, treemap: TreeMap): string | number {
  ...
  if (treemap.format && !isNaN(Number(value))) {
    formatFunction = treemap.intl.getNumberFormat({
      format: treemap.format,
      useGrouping: treemap.useGroupingSeparator,
    });
...
export function drawSymbol(
  ...
  const htmlElement: Element = svgRenderer['draw' + temp.functionName](
    temp.renderOption
  );
  return htmlElement;
}
  ...
  if (collection.length > 0) {
    for (let j: number = 0; j < collection.length; j++) {
      ...
      for (let k: number = 0; k < dataCount; k++) {
        setColor(
          item['ShapeCollection']['Elements'][k as number],
          item['shapeOldFill'],
          item['shapeOldOpacity'],
          item['shapeOldBorderColor'],
          item['shapeOldBorderWidth']
        );
      }
    }
  }
 ...
 * @returns {void}
 */
export function removeSelectionWithHighlight(
  collection: any[],
  element: any[],
  treemap: TreeMap
): void {
 ...
 */
export function getLegendIndex(
  length: number,
  item: any,
  treemap: TreeMap
): number {
  ...
  for (let i: number = 0; i < length; i++) {
    const dataLength: number =
      treemap.treeMapLegendModule.legendCollections[i as number]['legendData']
        .length;
    for (let j: number = 0; j < dataLength; j++) {
      if (
        treemap.treeMapLegendModule.legendCollections[i as number][
          'legendData'
        ][j as number]['levelOrderName'] === item['levelOrderName']
      ) {
        index = i;
        break;
      }
...
export function pushCollection(
  ...
  collection.push({
    legendEle: legendElement,
    oldFill: legendCollection[index as number]['legendFill'],
    oldOpacity: legendCollection[index as number]['opacity'],
    oldBorderColor: legendCollection[index as number]['borderColor'],
    oldBorderWidth: legendCollection[index as number]['borderWidth'],
    shapeElement: shapeElement,
    shapeOldFill: renderItems[number as number]['options']['fill'],
    shapeOldOpacity: renderItems[number as number]['options']['opacity'],
    shapeOldBorderColor:
      renderItems[number as number]['options']['border']['color'],
    shapeOldBorderWidth:
      renderItems[number as number]['options']['border']['width'],
  });
}
