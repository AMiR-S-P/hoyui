...
import { ITooltipEventArgs } from '@hoyui/svg-base';
...
describe('TreeMap component Spec', () => {
  ...
  describe('TreeMap drill down spec', () => {
    ...
    it('Checking with currency format', () => {
      treemap.loaded = (args: ILoadedEventArgs) => {
        ...
        let rectEle: Element = document.getElementById(
          args.treemap.element.id + '_Level_Index_1_Item_Index_17_RectPath'
        );
      ...
      treemap.loaded = (args: ILoadedEventArgs) => {
        ...
        let eventObj: Object = {
          ...
          changedTouches: [
            {
              pageX: rectEle.getBoundingClientRect().left,
              pageY: rectEle.getBoundingClientRect().top,
            },
          ],
        };
        treemap.treeMapTooltipModule.renderTooltip(<PointerEvent>eventObj);
    ...
    it('Checking with tooltip template', () => {
      ...
      treemap.tooltipSettings.template =
        '<div style="border 1px solid;">${EmployeesCount}</div>';
      treemap.refresh();
    });
