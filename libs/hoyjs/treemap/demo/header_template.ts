...
import { LabelPosition } from '../src/treemap/utils/enum';
...
let treemap: TreeMap = new TreeMap({
  ...
  levels: [
    { groupPath: 'dataType', headerTemplate: '<div>{{:type}}<div>' },
    { groupPath: 'product', headerTemplate: '<div>{{:product}}<div>' },
  ],
  leafItemSettings: {
    labelPath: 'type',
    fill: '#8ebfe2',
...
document.getElementById('labels').onchange = () => {
  ...
  treemap.levels[1].templatePosition = <LabelPosition>value;
  treemap.refresh();
};
