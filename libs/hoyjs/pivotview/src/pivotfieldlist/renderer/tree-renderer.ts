...
import { closest } from '@hoyui/base';
...
import { MaskedTextBox, MaskChangeEventArgs, TextBox } from '@hoyui/inputs';
...
export class TreeViewRenderer implements IAction {
  ...
  public render(axis?: number): void {
    ...
    this.fieldListSort = this.parent.pivotGridModule
      ? this.parent.pivotGridModule.defaultFieldListOrder
      : this.parent.defaultFieldListOrder;
    this.fieldListSort =
      this.fieldListSort === 'Ascending'
        ? 'Ascend'
        : this.fieldListSort === 'Descending'
        ? 'Descend'
        : 'None';
    if (!this.parent.isAdaptive) {
      const fieldTable: Element = createElement('div', {
        className:
          cls.FIELD_TABLE_CLASS +
          ' ' +
          (this.parent.dataType === 'olap' ? cls.OLAP_FIELD_TABLE_CLASS : ''),
      });
      ...
      const treeOuterDiv: HTMLElement = createElement('div', {
        className:
          cls.FIELD_LIST_TREE_OUTER_DIV_CLASS + ' ' + cls.TREE_CONTAINER,
      });
      this.treeViewElement = createElement('div', {
        id: this.parent.element.id + '_TreeView',
        className:
          cls.FIELD_LIST_CLASS +
          ' ' +
          (this.parent.dataType === 'olap' ? cls.OLAP_FIELD_LIST_CLASS : ''),
      });
      const fieldHeaderWrappper: Element = createElement('div', {
        className: cls.FIELD_HEADER_CONTAINER_CLASS,
      });
      ...
      if (this.parent.enableFieldSearching) {
        ...
        const searchInput: HTMLInputElement = createElement('input', {
          attrs: { type: 'text' },
        }) as HTMLInputElement;
        ...
        this.fieldSearch = new TextBox({
          ...
          cssClass:
            cls.FIELD_LIST_SEARCH_INPUT_CLASS +
        ...
        this.fieldSearch.addIcon(
          'append',
          cls.FIELD_LIST_SEARCH_ICON_CLASS + ' ' + cls.ICON
        );
      ...
      if (this.parent.renderMode === 'Fixed') {
        const centerDiv: Element = createElement('div', {
        ...
        axisHeader.innerText =
          this.parent.localeObj.getConstant('centerHeader');
  ...
  private renderTreeView(): void {
    ...
    const dragEle: HTMLElement =
      this.parent.renderMode === 'Fixed'
        ? this.parent.element
        : this.parentElement;
    if (!isNullOrUndefined(dragEle.querySelector('.' + cls.FIELD_LIST_CLASS))) {
      (
        getInstance(
          dragEle.querySelector('.' + cls.FIELD_LIST_CLASS) as HTMLElement,
          TreeView
        ) as TreeView
      )['dragObj'].enableAutoScroll = false;
    }
  }
  private updateNodeIcon(args: NodeExpandEventArgs): void {
    if (this.parent.dataType === 'olap') {
      if (
        args.node &&
        args.node.querySelector('.e-list-icon') &&
        args.node
          .querySelector('.e-list-icon')
          .className.indexOf('e-folderCDB-icon') > -1
      ) {
        ...
        args.node
          .querySelector('.e-list-icon')
          .className.indexOf('e-folderCDB-open-icon') > -1
      ) {
        const node: HTMLElement = args.node.querySelector('.e-list-icon');
  ...
  private updateTreeNode(args: DrawNodeEventArgs): void {
    ...
    if (this.parent.dataType === 'olap') {
      ...
      if (args.nodeData.hasChildren) {
        allowDrag = false;
        (
          args.node.querySelector('.' + cls.CHECKBOX_CONTAINER) as HTMLElement
        ).style.display = 'none';
    ...
    const liTextElement: HTMLElement = args.node.querySelector(
      '.' + cls.TEXT_CONTENT_CLASS
    );
    if (args.node.querySelector('.e-list-icon') && liTextElement) {
      const liIconElement: HTMLElement =
        args.node.querySelector('.e-list-icon');
      liTextElement.insertBefore(
        liIconElement,
        args.node.querySelector('.e-list-text')
      );
    }
    if (allowDrag && !this.parent.isAdaptive) {
      const field: FieldItemInfo = PivotUtil.getFieldInfo(
      ...
      const dragElement: Element = createElement('span', {
        attrs: {
          tabindex: '-1',
          title: field.fieldItem
            ? field.fieldItem.allowDragAndDrop
              ? this.parent.localeObj.getConstant('drag')
              : ''
            : this.parent.localeObj.getConstant('drag'),
          'aria-disabled': 'false',
        },
      ...
      if (
        args.node.querySelector('.' + cls.CHECKBOX_CONTAINER) &&
        !args.node.querySelector('.cls.DRAG_CLASS') &&
        liTextElement
      ) {
        liTextElement.insertBefore(
          dragElement,
    ...
    if (args.node.querySelector('.' + cls.NODE_CHECK_CLASS)) {
      addClass(
        [args.node.querySelector('.' + cls.LIST_TEXT_CLASS)],
        cls.LIST_SELECT_CLASS
      );
    }
    if (this.parent.enableFieldSearching && this.isSearching) {
      ...
      if (this.parent.dataType === 'olap') {
        ...
        const searchItem: HTMLElement[] =
          this.parent.pivotCommon.eventBase.searchListItem;
        for (let i: number = 0; i < this.parentIDs.length; i++) {
          ...
          for (const li2 of searchItem) {
            const parentID: string[] =
              this.parent.pivotCommon.eventBase.getParentIDs(
                this.fieldTable,
                li2.getAttribute('data-uid'),
                []
              );
        ...
        for (let i: number = 0; i < this.nonSearchList.length; i++) {
          if (
            ...
            this.nonSearchList[i as number].textContent
          ) {
            addClass([liElement], cls.ICON_DISABLE);
            break;
          } else {
  ...
  private updateOlapTreeNode(args: DrawNodeEventArgs): boolean {
    ...
    if (this.parent.dataType === 'olap') {
      if (
        args.node &&
        args.node.querySelector(
          '.e-calcMemberGroupCDB,.e-measureGroupCDB-icon,.e-folderCDB-icon,.e-folderCDB-open-icon,.e-dimensionCDB-icon,.e-kpiCDB-icon'
        )
      ) {
        (
          args.node.querySelector('.' + cls.CHECKBOX_CONTAINER) as HTMLElement
      ...
      if (
        ...
        args.node
          .querySelector('.e-list-icon')
          .className.indexOf('e-level-members') > -1
      ) {
        if (this.parent.isAdaptive) {
          (
            args.node.querySelector('.' + cls.CHECKBOX_CONTAINER) as HTMLElement
      ...
      if (
        args.node &&
        (args.node.querySelector(
          '.e-hierarchyCDB-icon,.e-attributeCDB-icon,.e-namedSetCDB-icon'
        ) ||
          args.node.querySelector(
            '.e-measure-icon,.e-kpiGoal-icon,.e-kpiStatus-icon,.e-kpiTrend-icon,.e-kpiValue-icon'
          ) ||
          args.node.querySelector(
            '.e-calc-measure-icon,.e-calc-dimension-icon'
          ))
      ) {
        if (args.node.querySelector('.e-measure-icon')) {
          (
            args.node.querySelector('.e-list-icon') as HTMLElement
          ).style.display = 'none';
          allowDrag = true;
        } else {
          allowDrag = true;
  ...
  private renderTreeDialog(axis?: number): void {
    const fieldListDialog: HTMLElement = createElement('div', {
      ...
      className:
        cls.ADAPTIVE_FIELD_LIST_DIALOG_CLASS +
        ' ' +
        (this.parent.dataType === 'olap' ? 'e-olap-editor-dialog' : ''),
    });
  ...
  private createTreeView(treeData: { [key: string]: Object }[]): HTMLElement {
    ...
    const treeOuterDiv: HTMLElement = createElement('div', {
      className: cls.FIELD_LIST_TREE_OUTER_DIV_CLASS,
    });
    const treeViewContainer: HTMLElement = createElement('div', {
      className:
        cls.EDITOR_TREE_CONTAINER_CLASS +
        ' ' +
        (this.parent.dataType === 'olap' ? 'e-olap-field-list-tree' : ''),
    });
  ...
  private textChange(e: MaskChangeEventArgs): void {
    ...
    if (this.parent.isAdaptive) {
      promptDiv = this.fieldDialog.element.querySelector(
        '.' + cls.EMPTY_MEMBER_CLASS
      );
    } else {
      promptDiv = this.parentElement.querySelector(
        '.' + cls.EMPTY_MEMBER_CLASS
      );
      treeOuterDiv = this.parentElement.querySelector('.' + cls.TREE_CONTAINER);
    }
    const liList: HTMLElement[] = [].slice.call(
      this.fieldTable.element.querySelectorAll('li')
    ) as HTMLElement[];
    const disabledList: HTMLElement[] = [].slice.call(
      this.fieldTable.element.querySelectorAll('li.' + cls.ICON_DISABLE)
    ) as HTMLElement[];
  ...
  private dragStart(args: DragAndDropEventArgs): void {
    if (
      ...
      !(args.event.target as HTMLElement).classList.contains(
        cls.DRAG_DISABLE_CLASS
      )
    ) {
      const fieldInfo: FieldItemInfo = PivotUtil.getFieldInfo(
        args.draggedNode.getAttribute('data-uid'),
        this.parent
      );
      ...
      const control: PivotView | PivotFieldList = this.parent.isPopupView
        ? this.parent.pivotGridModule
        : this.parent;
      control.trigger(
        events.fieldDragStart,
        dragEventArgs,
        (observedArgs: FieldDragStartEventArgs) => {
          if (!observedArgs.cancel) {
            this.parent.isDragging = true;
            addClass(
              [args.draggedNode.querySelector('.' + cls.LIST_TEXT_CLASS)],
              cls.SELECTED_NODE_CLASS
            );
            let data: IField;
            if (this.parent.dataType === 'olap') {
              data =
                this.parent.olapEngineModule.fieldList[
                  args.draggedNode.getAttribute('data-uid')
                ];
            } else {
              data =
            ...
            const axis: string[] = [
              cls.ROW_AXIS_CLASS,
              cls.COLUMN_AXIS_CLASS,
              cls.FILTER_AXIS_CLASS,
            ];
            if (data && data.aggregateType === 'CalculatedField') {
              for (const axisContent of axis) {
                addClass(
                  [this.parentElement.querySelector('.' + axisContent)],
                  cls.NO_DRAG_CLASS
                );
              }
            }
            ...
            if (
              ...
              dragItem.style.zIndex = (
                this.parent.dialogRenderer.fieldListDialog.zIndex + 1
              ).toString();
            }
          } else {
  ...
  private dragStop(args: DragAndDropEventArgs): void {
    ...
    for (const axisElement of axis) {
      removeClass(
        [this.parentElement.querySelector('.' + axisElement)],
        cls.NO_DRAG_CLASS
      );
    }
    removeClass(
    ...
    const selectedNode: { [key: string]: Object } = list[
      fieldName as string
    ] as { [key: string]: Object };
    this.parent.pivotCommon.dataSourceUpdate.control =
      this.parent.getModuleName() === 'pivotview'
        ? this.parent
        : this.parent.isPopupView &&
          (this.parent as PivotFieldList).pivotGridModule
        ? (this.parent as PivotFieldList).pivotGridModule
        : this.parent;
    if (
      this.parent.pivotCommon.nodeStateModified.onStateModified(args, fieldName)
    ) {
      if (
  ...
  private isNodeDropped(args: DragAndDropEventArgs, targetID: string): boolean {
    ...
    if (args.draggedNodeData.isChecked === 'true') {
      ...
      const axisPanel: HTMLElement = closest(
        target,
        '.' + cls.DROPPABLE_CLASS
      ) as HTMLElement;
      const droppableElement: HTMLElement = closest(
        args.target,
        '.' + cls.DROPPABLE_CLASS
      ) as HTMLElement;
      if (target && axisPanel === droppableElement) {
        const pivotButtons: HTMLElement[] = [].slice.call(
          axisPanel.querySelectorAll('.' + cls.PIVOT_BUTTON_CLASS)
        );
        const dropTarget: HTMLElement = closest(
          args.target,
          '.' + cls.PIVOT_BUTTON_WRAPPER_CLASS
        ) as HTMLElement;
        ...
        for (let i: number = 0, n: number = pivotButtons.length; i < n; i++) {
          ...
          if (dropTarget) {
            const droppableButton: HTMLElement = dropTarget.querySelector(
              '.' + cls.PIVOT_BUTTON_CLASS
            ) as HTMLElement;
        ...
        if (
          ...
          (sourcePosition === pivotButtons.length - 1 && dropPosition === -1)
        ) {
          const parentElement: HTMLElement = document.getElementById(
            this.parent.element.id + '_Container'
          );
          removeClass(
            [].slice.call(
              parentElement.querySelectorAll('.' + cls.DROP_INDICATOR_CLASS)
            ),
            cls.INDICATOR_HOVER_CLASS
          );
          isDropped = false;
        }
      }
  ...
  private getButton(fieldName: string): HTMLElement {
    ...
    const pivotButtons: HTMLElement[] = [].slice.call(
      wrapperElement.querySelectorAll('.' + cls.PIVOT_BUTTON_CLASS)
    );
  ...
  private nodeChecked(args: NodeCheckEventArgs): void {
    if (this.isSpaceKey) {
      const node: HTMLElement = closest(
        args.node,
        '.' + cls.TEXT_CONTENT_CLASS
      ) as HTMLElement;
  ...
  private nodeStateChange(
    ...
    const list: { [key: string]: Object } = this.parent.pivotFieldList;
    const selectedNode: { [key: string]: Object } = list[id as string] as {
      [key: string]: Object;
    };
    if (!isNullOrUndefined(args)) {
      this.isSpaceKey =
        (args.event as KeyboardEventArgs).action &&
        (args.event as KeyboardEventArgs).action === 'space';
      if (
        isNullOrUndefined(selectedNode) ||
        node.classList.contains(cls.ICON_DISABLE) ||
        (args.event.target &&
          ((args.event.target as HTMLElement).classList.contains(
            cls.COLLAPSIBLE
          ) ||
      ...
      isChecked = false;
      const getNodeDetails: { [key: string]: Object } =
        this.fieldTable.getNode(node);
      if (
        args.event &&
        args.event.target &&
        !(args.event.target as HTMLElement).closest(
          '.' + cls.CHECKBOX_CONTAINER
        )
      ) {
    ...
    if (isChecked) {
      const eventdrop: FieldDropEventArgs = {
        ...
        dropAxis:
          ...
          (selectedNode.type === 'CalculatedField' &&
            selectedNode.formula &&
            (selectedNode.formula as string).indexOf('Measure') > -1 &&
      ...
      control.trigger(
        events.fieldDrop,
        eventdrop,
        (observedArgs: FieldDropEventArgs) => {
          if (!observedArgs.cancel) {
            addClass(
              [node.querySelector('.' + cls.LIST_TEXT_CLASS)],
              cls.LIST_SELECT_CLASS
            );
            if (parentNode) {
              addClass(
                [parentNode.querySelector('.' + cls.LIST_TEXT_CLASS)],
                cls.LIST_SELECT_CLASS
              );
            }
            this.updateSelectedNodes(node, 'check');
            const addNode: IFieldOptions =
              this.parent.pivotCommon.dataSourceUpdate.getNewField(
                id,
                fieldInfo.fieldItem
              );
      ...
      control.trigger(
        events.fieldRemove,
        removeFieldArgs,
        (observedArgs: FieldRemoveEventArgs) => {
          if (!observedArgs.cancel) {
            removeClass(
              [node.querySelector('.' + cls.LIST_TEXT_CLASS)],
              cls.LIST_SELECT_CLASS
            ...
            if (
              ...
              isNullOrUndefined(
                parentNode.querySelector(
                  '.' + cls.FIELD_TREE_CHILD + ' .' + cls.NODE_CHECK_CLASS
                )
              )
            ) {
              removeClass(
            ...
            if (
              ...
              this.parent.pivotCommon.dataSourceUpdate.removeFieldFromReport(
                '[Measures]'
              );
            }
            this.updateNodeStateChange(id, selectedNode, isChecked);
  ...
  private updateReportSettings(
    newField: IFieldOptions,
    dropArgs: FieldDropEventArgs
  ): void {
    ...
    if (
      ...
      const dropAxisFields: IFieldOptions[] =
        this.parent.dataSourceSettings.valueAxis === 'row' &&
        dropClass === 'rows'
      ...
      if (!isNullOrUndefined(dropAxisFields)) {
        if (
          dropPosition === -1 &&
          this.parent.dataSourceSettings.valueIndex === -1
        ) {
          ...
          dropPosition > -1 &&
          dropPosition <= this.parent.dataSourceSettings.valueIndex
        ) {
          this.parent.setProperties(
            {
              dataSourceSettings: {
    ...
    switch (dropClass) {
      case 'filters':
        if (dropPosition !== -1) {
          this.parent.dataSourceSettings.filters.splice(
            dropPosition,
            0,
            newField
          );
      ...
      case 'columns':
        if (dropPosition !== -1) {
          this.parent.dataSourceSettings.columns.splice(
            dropPosition,
            0,
            newField
          );
      ...
      case 'values':
        if (dropPosition !== -1) {
          this.parent.dataSourceSettings.values.splice(
            dropPosition,
            0,
            newField
          );
  ...
  private updateCheckState(
    selectedNode: { [key: string]: Object },
    action: string
  ): void {
    const chkState: NodeListOf<Element> =
      this.fieldTable.element.querySelectorAll('.' + cls.CHECKBOX_CONTAINER);
    const innerText: NodeListOf<Element> =
      this.fieldTable.element.querySelectorAll('.e-list-text');
  ...
  private updateNodeStateChange(
    ...
    const selectedLi: HTMLElement = this.treeViewElement.querySelector(
      '[data-uid="' + id + '"]'
    );
    selectedLi.focus();
  ...
  private updateSelectedNodes(li: HTMLElement, state: string): void {
    if (li && li.querySelector('ul')) {
      for (const element of [].slice.call(li.querySelectorAll('li'))) {
        if (state === 'check') {
          addClass(
            [element.querySelector('.' + cls.LIST_TEXT_CLASS)],
            cls.LIST_SELECT_CLASS
          );
        } else {
          removeClass(
  ...
  private updateDataSource(): void {
    if (this.parent.isPopupView) {
      if (this.parent.dataType === 'olap') {
        (this.parent as PivotFieldList).pivotGridModule.olapEngineModule = (
          this.parent as PivotFieldList
        ).olapEngineModule;
      } else {
        (this.parent as PivotFieldList).pivotGridModule.engineModule = (
          this.parent as PivotFieldList
        ).engineModule;
      }
      (this.parent as PivotFieldList).pivotGridModule.setProperties(
        {
          dataSourceSettings: (<{ [key: string]: Object }>(
            this.parent.dataSourceSettings
          )).properties as IDataOptions,
        },
        true
      );
      (this.parent as PivotFieldList).pivotGridModule.notify(
        events.uiUpdate,
        this
      );
    } else {
  ...
  private addNode(
    ...
    if (!isNullOrUndefined(args)) {
      this.isSpaceKey =
        (args.event as KeyboardEvent).key &&
        (args.event as KeyboardEvent).key === ' ';
      if (
        isNullOrUndefined(selectedNode) ||
        args.node.classList.contains(cls.ICON_DISABLE) ||
        (args.event.target &&
          ((args.event.target as HTMLElement).classList.contains(
            cls.COLLAPSIBLE
          ) ||
        ...
        ((args.event as KeyboardEvent).key &&
          (args.event as KeyboardEvent).key !== 'Enter')
      ) {
        return;
      }
      isChecked = false;
      const getNodeDetails: { [key: string]: Object } = this.fieldTable.getNode(
        args.node
      );
      if (
        args.event &&
      ...
      control.trigger(
        ...
        (observedArgs: FieldRemoveEventArgs) => {
          if (!observedArgs.cancel) {
            ...
            while (count--) {
              if (
                this.selectedNodes[count as number] ===
                selectedNode.id.toString()
              ) {
  ...
  private getTreeData(axis?: number): { [key: string]: Object }[] {
    ...
    if (this.parent.dataType === 'olap') {
      ...
      const keys: string[] = this.parent.pivotFieldList
        ? Object.keys(this.parent.pivotFieldList)
        : [];
      const treeDataInfo: {
        [key: string]: {
          id?: string;
      ...
      for (const key of keys) {
        ...
        if (
          !isNullOrUndefined(member.pid) &&
          !treeDataInfo[key as string].hasChildren
        ) {
          ...
          treeDataInfo[parentId as string] = {
            ...
            isSelected:
              treeDataInfo[parentId as string] &&
              treeDataInfo[parentId as string].isSelected
                ? treeDataInfo[parentId as string].isSelected
                : member.isSelected,
            hasChildren: true,
          };
      ...
      if (this.parent.isAdaptive) {
        const fields: IFieldOptions[][] = [
          this.parent.dataSourceSettings.filters,
          this.parent.dataSourceSettings.columns,
          this.parent.dataSourceSettings.rows,
      ...
      for (const member of members) {
        const obj: { [key: string]: Object } = treeDataInfo[
          member as string
        ] as { [key: string]: Object };
        data.push(obj);
      }
  ...
  private getOlapTreeData(axis?: number): { [key: string]: Object }[] {
    let data: { [key: string]: Object }[] = [];
    const fieldListData: IOlapField[] = this.parent.olapEngineModule
      .fieldListData
      ? this.parent.olapEngineModule.fieldListData
      : [];
    if (this.parent.isAdaptive) {
      const fields: IFieldOptions[][] = [
        this.parent.dataSourceSettings.filters,
      ...
      while (i < fieldListData.length) {
        ...
        if (axis === 3) {
          if (
            ((item.id as string).toLowerCase() !== '[measures]' &&
              ((item.id as string).toLowerCase().indexOf('[measures]') === 0 ||
                (item.spriteCssClass &&
                  item.spriteCssClass.indexOf('e-measureCDB') !== -1))) ||
            (item.id as string).toLowerCase() === '[calculated members].[_0]' ||
            (item.spriteCssClass &&
              item.spriteCssClass.indexOf('e-calc-measure-icon') !== -1)
          ) {
            framedSet = {
              id: item.id,
            ...
            if (
              framedSet.spriteCssClass &&
              (framedSet.spriteCssClass as string).indexOf('e-measureCDB') !==
                -1
            ) {
              framedSet.spriteCssClass = (
                framedSet.spriteCssClass as string
              ).replace('e-folderCDB-icon', 'e-measureGroupCDB-icon');
      ...
      data = isNullOrUndefined(this.parent.olapEngineModule.fieldListData)
        ? []
        : PivotUtil.getClonedData(
            this.parent.olapEngineModule.fieldListData as {
              [key: string]: Object;
            }[]
          );
    }
      ...
      for (const field of data) {
        if (expandedNodes.indexOf((field as IOlapField).id) > -1) {
          ...
          (field as IOlapField).spriteCssClass =
            ...
            (field as IOlapField).spriteCssClass
              .toString()
              .indexOf('e-folderCDB-icon') > -1
              ? (field as IOlapField).spriteCssClass
                  .toString()
                  .replace('e-folderCDB-icon', 'e-folderCDB-open-icon')
              : (field as IOlapField).spriteCssClass;
  ...
  private applySorting(
    ...
    if (treeData.length > 0) {
      if (this.parent.dataType === 'olap') {
        ...
        if (
          this.parent.dataSourceSettings.calculatedFieldSettings.length > 0 &&
          (treeData[0].id as string).toLowerCase() ===
            '[calculated members].[_0]'
        ) {
  ...
  private closeTreeDialog(): void {
    ...
    if (
      document.getElementById(this.parent.element.id + '_FieldListTreeView')
    ) {
      remove(
        document.getElementById(this.parent.element.id + '_FieldListTreeView')
      );
