...
export class KeyboardInteraction {
  ...
  private getNextButton(target: HTMLElement): HTMLElement {
    const allPivotButtons: HTMLElement[] = this.allpivotButtons(
    ...
    if (this.parent.grid.element.querySelector('.' + cls.PIVOT_BUTTON_CLASS)) {
      ...
      for (let i: number = 0; i < len; i++) {
        if (
          allPivotButtons[i as number].getAttribute('data-uid') ===
  ...
  private allpivotButtons(target: HTMLElement): HTMLElement[] {
    ...
    if (target && this.parent.showGroupingBar) {
      const columnFilterValueGroup: HTMLElement = closest(
        target,
        '.' + cls.GRID_GROUPING_BAR_CLASS
      ) as HTMLElement;
      const rowGroup: HTMLElement = closest(
        target,
        '.' + cls.GROUP_PIVOT_ROW
      ) as HTMLElement;
      const chartGroup: HTMLElement = closest(
        target,
        '.' + cls.CHART_GROUPING_BAR_CLASS
      ) as HTMLElement;
      ...
      if (columnFilterValueGroup !== null) {
        rowAxis = columnFilterValueGroup.classList.contains(
          cls.GRID_GROUPING_BAR_CLASS
        );
      } else if (rowGroup !== null) {
        columnFilterValueAxis = rowGroup.classList.contains(
          cls.GROUP_PIVOT_ROW
        );
      ...
      if (rowAxis || columnFilterValueAxis || tableAxis) {
        const groupingbarButton: HTMLElement[] = [].slice.call(
          this.parent.element
            .querySelector('.' + cls.GRID_GROUPING_BAR_CLASS)
            .querySelectorAll('.' + cls.PIVOT_BUTTON_CLASS)
        );
        const headerButton: HTMLElement[] = [].slice.call(
          this.parent.element
            .querySelector('.' + cls.GROUP_PIVOT_ROW)
            .querySelectorAll('.' + cls.PIVOT_BUTTON_CLASS)
        );
        ...
        buttons = [].slice.call(
          this.parent.element
            .querySelector('.' + cls.CHART_GROUPING_BAR_CLASS)
            .querySelectorAll('.' + cls.PIVOT_BUTTON_CLASS)
        );
      }
    }
  ...
  private processTab(e: Event): void {
    ...
    if (
      target &&
      (closest(target, '.' + cls.PIVOT_BUTTON_CLASS) ||
        target.classList.contains('e-group-row'))
    ) {
      if (this.parent.grid) {
        ...
        if (
          target.classList.contains('e-group-row') &&
          target.querySelector('.e-btn-focused')
        ) {
        ...
        const nextButton: HTMLElement = this.getNextButton(
          target as HTMLElement
        );
        if (
          nextButton.getAttribute('data-uid') !==
          target.getAttribute('data-uid')
        ) {
          if (this.parent.element.querySelector('.e-focused')) {
            this.parent.element
              .querySelector('.e-focused')
              .setAttribute('tabindex', '-1');
            removeClass(
              this.parent.element.querySelectorAll('.e-focus'),
              'e-focus'
            );
            removeClass(
              this.parent.element.querySelectorAll('.e-focused'),
              'e-focused'
            );
            gridFocus.setFocusedElement(
              this.parent.element.querySelector('.e-headercell')
            );
            this.parent.element
      ...
      target &&
      closest(target, '.' + cls.TOGGLE_FIELD_LIST_CLASS)
    ) {
      if (this.parent.grid) {
        const gridFocus: FocusStrategy =
          this.parent.grid.serviceLocator.getService<FocusStrategy>('focus');
      ...
      !this.parent.showFieldList &&
      target &&
      closest(target, '.' + cls.PIVOT_VIEW_CLASS) &&
      !closest(target, '.e-popup.e-popup-open')
    ) {
      if (this.parent.grid) {
        const gridElement: HTMLElement = closest(
          target,
          '.' + cls.PIVOT_VIEW_CLASS
        ) as HTMLElement;
        const gridFocus: FocusStrategy =
          this.parent.grid.serviceLocator.getService<FocusStrategy>('focus');
        const rows: HTMLElement[] = [].slice.call(
          gridElement.getElementsByTagName('tr')
        ) as HTMLElement[];
        if (
          target.innerHTML ===
          (rows[rows.length - 1].lastChild as HTMLElement).innerHTML
        ) {
          gridFocus.currentInfo.skipAction = true;
        } else {
          gridFocus.focus();
      ...
      target &&
      closest(target, '.' + cls.GRID_TOOLBAR) &&
      this.parent.toolbar &&
      ...
      this.timeOutObj = setTimeout(() => {
        removeClass(
          closest(target, '.' + cls.GRID_TOOLBAR).querySelectorAll(
        ...
        if (
          ...
          document.activeElement.classList.contains('e-menu-item')
        ) {
    ...
    if (
      ...
      if (this.parent.grid) {
        ...
        if (
          ...
          target = this.parent.element.querySelector('.e-btn-focused')
            ? this.parent.element.querySelector('.e-btn-focused')
          ...
          if (
            allPivotButtons.length > 0 &&
            allPivotButtons[allPivotButtons.length - 1]
          ) {
            gridFocus.currentInfo.skipAction = true;
        ...
        const prevButton: HTMLElement = this.getPrevButton(
          target as HTMLElement
        );
        if (
          prevButton.getAttribute('data-uid') !==
          target.getAttribute('data-uid')
        ) {
          gridFocus.currentInfo.skipAction = true;
          prevButton.focus();
      ...
      target &&
      this.parent.grid &&
      (target.classList.contains('e-movablefirst') ||
        (target.classList.contains('e-rowsheader') &&
          closest(target, 'tr').getAttribute('data-uid') ===
            this.parent.grid.element
              .querySelector('.e-frozencontent tr')
      ...
      if (target.classList.contains('e-movablefirst')) {
        target = this.parent.element.querySelector(
          '.' + cls.GROUP_ROW_CLASS + ' .e-btn-focused'
        )
          ? this.parent.element.querySelector(
              '.' + cls.GROUP_ROW_CLASS + ' .e-btn-focused'
            )
  ...
  private clearSelection(): void {
    ...
    removeClass(
      control.element.querySelectorAll(
        '.' + cls.CELL_SELECTED_BGCOLOR + ',.' + cls.SELECTED_BGCOLOR
      ),
      [cls.SELECTED_BGCOLOR, cls.CELL_SELECTED_BGCOLOR, cls.CELL_ACTIVE_BGCOLOR]
    );
  ...
  private processSelection(e: KeyboardEventArgs): void {
    ...
    if (
      this.parent.grid &&
      this.parent.gridSettings.allowSelection &&
      this.parent.gridSettings.selectionSettings.mode !== 'Row' &&
      !target.classList.contains('e-numerictextbox')
    ) {
      const control: PivotView = this.parent as PivotView;
      let colIndex: number = Number(
        (e.target as HTMLElement).getAttribute('data-colindex')
      );
      let rowIndex: number = Number(
      ...
      if (target.nodeName === 'TH' || target.nodeName === 'TD') {
        if (e.action === 'shiftUp' || e.action === 'upArrow') {
          ele =
            ...
            colIndex === 0 ||
            (target.nodeName !== 'TH' &&
              control.renderModule.rowStartPos !== rowIndex)
              ? null
      ...
      if (!isNullOrUndefined(ele)) {
        if (
          control.gridSettings.selectionSettings.mode === 'Both'
            ? !ele.classList.contains(cls.ROW_CELL_CLASS)
            : true
        ) {
          ...
          const selectArgs: PivotCellSelectedEventArgs = {
            ...
            isCellClick: true,
            currentCell: ele,
            data: control.pivotValues[rowIndex as number][
              colIndex as number
            ] as IAxisSet,
          };
          control.trigger(
            ...
            (observedArgs: PivotCellSelectedEventArgs) => {
              if (!observedArgs.cancel) {
                control.applyColumnSelection(
                  e,
                  ele,
                  colIndex,
                  colIndex + (colSpan > 0 ? colSpan - 1 : 0),
                  rowIndex
                );
              }
            }
        ...
        if (e.action === 'upArrow') {
          ele = control.element.querySelector(
            '[data-colindex="' + colIndex + '"][index="' + (rowIndex - 1) + '"]'
          );
          rowIndex--;
      ...
      if (
        this.parent.grid &&
        this.parent.element.querySelector('.e-frozenheader') &&
        this.parent.element
          .querySelector('.e-frozenheader')
          .querySelectorAll('.e-focus').length > 0
      ) {
        removeClass(
          this.parent.element
            .querySelector('.e-frozenheader')
            .querySelectorAll('.e-focus'),
          'e-focus'
        );
        removeClass(
          this.parent.element
            .querySelector('.e-frozenheader')
            .querySelectorAll('.e-focused'),
          'e-focused'
        );
        this.parent.element
          .querySelector('.e-headercell')
      ...
      target.classList.contains('e-numerictextbox') &&
      (e.action === 'rightArrow' || e.action === 'leftArrow')
    ) {
      (target as HTMLElement).click();
    }
  }
  private getParentElement(
    ...
    while (!ele) {
      ele = control.element.querySelector(
        '[data-colindex="' + colIndex + '"][index="' + rowIndex + '"]'
      );
      colIndex--;
    }
    return ele;
  ...
  private toggleFieldList(e: Event): void {
    if (
      ...
      if (
        !this.parent.element
          .querySelector('.' + cls.TOGGLE_FIELD_LIST_CLASS)
          .classList.contains(cls.ICON_HIDDEN)
      ) {
        (
          this.parent.element.querySelector(
        ...
        this.parent.pivotFieldListModule.dialogRenderer.fieldListDialog &&
        !this.parent.pivotFieldListModule.dialogRenderer.fieldListDialog
          .isDestroyed
      ) {
