...
export class PDFExport {
  ...
  private addPage(
    eventParams: { document: PdfDocument; args: EnginePopulatedEventArgs },
    pdfExportProperties?: PdfExportProperties
  ): PdfPage {
    pdfExportProperties = pdfExportProperties
      ? pdfExportProperties
      : this.exportProperties.pdfExportProperties;
    const documentSection: PdfSection =
      eventParams.document.sections.add() as PdfSection;
    ...
    if (this.exportProperties.width || this.exportProperties.height) {
      eventParams.document.pageSettings.orientation =
        this.exportProperties.width > this.exportProperties.height ||
        (!this.exportProperties.height &&
          this.exportProperties.width > documentHeight) ||
        (!this.exportProperties.width &&
          documentWidth > this.exportProperties.height)
          ? PdfPageOrientation.Landscape
          : PdfPageOrientation.Portrait;
      eventParams.document.pageSettings.size = new SizeF(
        this.exportProperties.width
          ? this.exportProperties.width
          : documentWidth,
        this.exportProperties.height
      ...
      eventParams.document.pageSettings.orientation =
        this.exportProperties.orientation === 0 ||
        this.exportProperties.orientation
          ? this.exportProperties.orientation
      ...
      if (
        ...
        eventParams.document.pageSettings.size = PivotUtil.getPageSize(
    ...
    if (!isNullOrUndefined(this.exportProperties.pdfMargins)) {
      ...
      margins.top = !isNullOrUndefined(this.exportProperties.pdfMargins.top)
        ? this.exportProperties.pdfMargins.top
        : margins.top;
      margins.bottom = !isNullOrUndefined(
        this.exportProperties.pdfMargins.bottom
      )
        ? this.exportProperties.pdfMargins.bottom
        : margins.bottom;
      margins.left = !isNullOrUndefined(this.exportProperties.pdfMargins.left)
        ? this.exportProperties.pdfMargins.left
        : margins.left;
      margins.right = !isNullOrUndefined(this.exportProperties.pdfMargins.right)
        ? this.exportProperties.pdfMargins.right
        : margins.right;
    }
    ...
    if (
      ...
      this.pdfExportHelper.drawHeader(
        pdfExportProperties,
        eventParams.document
      );
    }
    if (
      !isNullOrUndefined(pdfExportProperties) &&
      !isNullOrUndefined(pdfExportProperties.footer)
    ) {
      this.pdfExportHelper.drawFooter(
        pdfExportProperties,
        eventParams.document
      );
    }
  ...
  private getBorderStyle(borderStyle: PdfBorder): PdfBorders {
    ...
    if (!isNullOrUndefined(borderStyle)) {
      ...
      if (!isNullOrUndefined(borderStyle.color)) {
        const borderColor: { r: number; g: number; b: number } =
          this.pdfExportHelper.hexDecToRgb(borderStyle.color);
      ...
      if (!isNullOrUndefined(borderStyle.dashStyle)) {
        pen.dashStyle = this.pdfExportHelper.getDashStyle(
    ...
    if (!isNullOrUndefined(this.gridStyle)) {
      ...
      if (!isNullOrUndefined(this.gridStyle.header.fontColor)) {
        const penBrushColor: { r: number; g: number; b: number } =
          this.pdfExportHelper.hexDecToRgb(this.gridStyle.header.fontColor);
  ...
  private setRecordThemeStyle(row: PdfGridRow, border: PdfBorders): PdfGridRow {
    if (
      !isNullOrUndefined(this.gridStyle) &&
      !isNullOrUndefined(this.gridStyle.record)
    ) {
      const fontFamily: number = !isNullOrUndefined(
        this.gridStyle.record.fontName
      )
        ? this.getFontFamily(this.gridStyle.record.fontName)
        : PdfFontFamily.Helvetica;
      const fontSize: number = !isNullOrUndefined(
        this.gridStyle.record.fontSize
      )
        ? this.gridStyle.record.fontSize
        : 9.75;
      ...
      if (!isNullOrUndefined(this.gridStyle.record.fontColor)) {
        const penBrushColor: { r: number; g: number; b: number } =
          this.pdfExportHelper.hexDecToRgb(this.gridStyle.record.fontColor);
        pdfColor = new PdfColor(
          penBrushColor.r,
          penBrushColor.g,
          penBrushColor.b
    ...
    const borderRecord: PdfBorders =
      this.gridStyle && this.gridStyle.record && this.gridStyle.record.border
        ? this.getBorderStyle(this.gridStyle.record.border)
        : border;
  ...
  public exportToPDF(
    ...
    this.gridStyle = !isNullOrUndefined(
      this.exportProperties.pdfExportProperties
    )
      ? this.exportProperties.pdfExportProperties.theme
      : undefined;
    const eventParams: {
      document: PdfDocument;
      args: EnginePopulatedEventArgs;
    } = this.applyEvent();
    ...
    const fileName: string =
      !isNullOrUndefined(this.exportProperties) &&
      !isNullOrUndefined(this.exportProperties.fileName)
        ? this.exportProperties.fileName
        : !isNullOrUndefined(pdfExportProperties) &&
          !isNullOrUndefined(pdfExportProperties.fileName)
        ? pdfExportProperties.fileName
        : 'default';
    const indent: number = this.parent.renderModule.maxIndent
      ? this.parent.renderModule.maxIndent
      : 5;
    ...
    let pivotValues: IAxisSet[][] = eventParams.args.pivotValues[
      dataCollIndex as number
    ] as IAxisSet[][];
    if (
      this.exportProperties.columnSize ||
      this.exportProperties.width ||
      this.exportProperties.height
    ) {
      size =
        this.exportProperties.columnSize > 0
          ? this.exportProperties.columnSize
          : pivotValues[0].length;
    }
    this.exportProperties.allowRepeatHeader =
      this.exportProperties.allowRepeatHeader === true ||
      isNullOrUndefined(this.exportProperties.allowRepeatHeader);
    const allowRepeatHeader: boolean = this.exportProperties.allowRepeatHeader
      ? this.exportProperties.allowRepeatHeader
      : false;
    for (
      let vLen: number = 0;
      allowRepeatHeader && size > 1 && vLen < pivotValues.length;
      vLen++
    ) {
      for (
        let vCnt: number = size;
        pivotValues[vLen as number] &&
        vCnt < pivotValues[vLen as number].length;
        vCnt += size
      ) {
        (pivotValues[vLen as number] as IAxisSet[]).splice(
          vCnt,
          0,
          pivotValues[vLen as number][0] as IAxisSet
        );
      }
    }
    let colLength: number =
      pivotValues && pivotValues.length > 0 ? pivotValues[0].length : 0;
    let integratedCnt: number = 0;
    do {
      if (!isNullOrUndefined(pdfExportProperties)) {
        this.exportProperties.header =
          !isNullOrUndefined(pdfExportProperties.header) &&
          !isNullOrUndefined(pdfExportProperties.header.contents) &&
          !isNullOrUndefined(pdfExportProperties.header.contents[0].value)
            ? pdfExportProperties.header.contents[0].value
            : this.exportProperties.header;
        this.exportProperties.footer =
          !isNullOrUndefined(pdfExportProperties.footer) &&
          !isNullOrUndefined(pdfExportProperties.footer.contents) &&
          !isNullOrUndefined(pdfExportProperties.footer.contents[0].value)
            ? pdfExportProperties.footer.contents[0].value
            : this.exportProperties.footer;
      }
      const page: PdfPage = this.addPage(eventParams, pdfExportProperties);
      const pdfGrid: PdfGrid = new PdfGrid();
      const pageSize: number = size > 0 ? size : 5;
      if (pivotValues && pivotValues.length > 0) {
        pdfGrid.columns.add(
          pivotValues[0].length - integratedCnt >= pageSize
            ? pageSize
            : pivotValues[0].length - integratedCnt
        );
        ...
        for (let rCnt: number = 0; rCnt < rowLen; rCnt++) {
          if (pivotValues[rCnt as number]) {
            const isColHeader: boolean = !(
              pivotValues[rCnt as number][0] &&
              (pivotValues[rCnt as number][0] as IAxisSet).axis === 'row'
            );
            const colLen: number =
              pivotValues[rCnt as number].length > integratedCnt + pageSize
                ? integratedCnt + pageSize
                : pivotValues[rCnt as number].length;
            if (isColHeader) {
              pdfGrid.headers.add(1);
            }
            let pdfGridRow: PdfGridRow = !isColHeader
              ? pdfGrid.rows.addRow()
              : pdfGrid.headers.getHeader(actualrCnt);
            if (isColHeader) {
            ...
            for (let cCnt: number = integratedCnt; cCnt < colLen; cCnt++) {
              ...
              if (pivotValues[rCnt as number][cCnt as number]) {
                const pivotCell: IAxisSet = pivotValues[rCnt as number][
                  cCnt as number
                ] as IAxisSet;
                ...
                cellValue =
                  ...
                  this.parent.dataSourceSettings.columns.length === 0
                    ? this.parent.getValuesHeader(pivotCell, 'value')
                    : cellValue;
                cellValue =
                  pivotCell.type === 'grand sum'
                    ? this.parent.dataSourceSettings.rows.length === 0 ||
                      this.parent.dataSourceSettings.columns.length === 0
                      ? this.parent.getValuesHeader(pivotCell, 'grandTotal')
                ...
                if (!(pivotCell.level === -1 && !pivotCell.rowSpan)) {
                  if (!(pivotCell.level === -1 && !pivotCell.rowSpan)) {
                    pdfGridRow.cells.getCell(localCnt).columnSpan =
                      pivotCell.colSpan
                    ...
                    if (
                      ...
                      pdfGridRow.cells.getCell(localCnt).rowSpan =
                        pivotCell.rowSpan ? pivotCell.rowSpan : 1;
                    }
                    pdfGridRow.cells.getCell(localCnt).value = cellValue
                      ? cellValue.toString()
                      : '';
                  }
                  if (cellValue !== '') {
                ...
                maxLevel =
                  pivotCell.level > maxLevel ? pivotCell.level : maxLevel;
                isValueCell = pivotCell.axis === 'value';
                cCnt =
                  cCnt +
                  (pdfGridRow.cells.getCell(localCnt).columnSpan
                    ? pdfGridRow.cells.getCell(localCnt).columnSpan - 1
                    : 0);
                localCnt =
                  localCnt +
                  (pdfGridRow.cells.getCell(localCnt).columnSpan
                ...
                if (pivotCell.axis === 'column') {
                  ...
                  pdfGridRow.cells.getCell(localCnt).value = (
                    args.gridCell as IAxisSet
                  ).formattedText
                    ? (args.gridCell as IAxisSet).formattedText
                    : cellValue;
                } else {
                  args = {
                    style: args.style,
                  ...
                  pdfGridRow.cells.getCell(localCnt).value = args.value
                    ? args.value
                    : cellValue;
                }
                if (args.style) {
                  this.processCellStyle(
                    pdfGridRow.cells.getCell(localCnt),
                    args
                  );
                }
              } else {
              ...
              if (this.parent.dataType === 'olap') {
                const indent: number =
                  ...
                  (pivotValues[rCnt as number][cCnt as number] as IAxisSet)
                    ? this.parent.renderModule.indentCollection[
                        (
                ...
                if (
                  ...
                  const cell: IAxisSet = pivotValues[rCnt as number][
                    cCnt as number
                  ] as IAxisSet;
              ...
              stringFormat.alignment = isValueCell
                ? PdfTextAlignment.Right
                : PdfTextAlignment.Left;
              stringFormat.lineAlignment = PdfVerticalAlignment.Middle;
              pdfGridRow.cells.getCell(localCnt).style.stringFormat =
                stringFormat;
              localCnt++;
            }
            if (isEmptyRow) {
        ...
        pdfGrid.columns.getColumn(0).width =
          columnWidth > 0 ? columnWidth : 100 + maxLevel * 20;
      }
      if (
        integratedCnt === 0 &&
        this.parent.dataSourceSettings.columns &&
      ...
      if (
        integratedCnt >= colLength &&
        eventParams.args.pivotValues.length > dataCollIndex + 1
      ) {
        dataCollIndex++;
        pivotValues = eventParams.args.pivotValues[
          dataCollIndex as number
        ] as IAxisSet[][];
        colLength =
          pivotValues && pivotValues.length > 0 ? pivotValues[0].length : 0;
  ...
  private applyStyle(
    ...
    let color: { r: number; g: number; b: number } =
      this.parent.conditionalFormattingModule.hexToRgb(
        pivotCell.style.backgroundColor
      );
    let brush: PdfSolidBrush = new PdfSolidBrush(
      new PdfColor(color.r, color.g, color.b)
    );
    ...
    color = this.parent.conditionalFormattingModule.hexToRgb(
  ...
  private getFont(
    ...
    const fontSize: number =
      (theme.cell as PdfGridCell)['cellStyle'].font &&
      (theme.cell as PdfGridCell)['cellStyle'].font.fontSize
        ? (theme.cell as PdfGridCell)['cellStyle'].font.fontSize
        : !isNullOrUndefined(theme.style.fontSize)
        ? theme.style.fontSize * 0.75
        : 9.75;

    const fontFamily: number = !isNullOrUndefined(theme.style.fontFamily)
      ? this.getFontFamily(theme.style.fontFamily)
      : PdfFontFamily.TimesRoman;
  ...
  private processCellStyle(
    ...
    if (!isNullOrUndefined(arg.style.backgroundColor)) {
      const backColor: { r: number; g: number; b: number } =
        this.pdfExportHelper.hexDecToRgb(arg.style.backgroundColor);
      gridCell.style.backgroundBrush = new PdfSolidBrush(
        new PdfColor(backColor.r, backColor.g, backColor.b)
      );
    }
    if (!isNullOrUndefined(arg.style.textBrushColor)) {
      const textBrushColor: { r: number; g: number; b: number } =
        this.pdfExportHelper.hexDecToRgb(arg.style.textBrushColor);
      gridCell.style.textBrush = new PdfSolidBrush(
        new PdfColor(textBrushColor.r, textBrushColor.g, textBrushColor.b)
      );
    }
    if (!isNullOrUndefined(arg.style.textPenColor)) {
      const textColor: { r: number; g: number; b: number } =
        this.pdfExportHelper.hexDecToRgb(arg.style.textPenColor);
      gridCell.style.textPen = new PdfPen(
        new PdfColor(textColor.r, textColor.g, textColor.b)
      );
    }
    if (
      !isNullOrUndefined(arg.style.fontFamily) ||
      !isNullOrUndefined(arg.style.fontSize) ||
      !isNullOrUndefined(arg.style.bold) ||
      !isNullOrUndefined(arg.style.italic) ||
      !isNullOrUndefined(arg.style.underline) ||
      !isNullOrUndefined(arg.style.strikeout)
    ) {
    ...
    if (!isNullOrUndefined(arg.style.border)) {
      ...
      if (!isNullOrUndefined(arg.style.border.color)) {
        const borderColor: { r: number; g: number; b: number } =
          this.pdfExportHelper.hexDecToRgb(arg.style.border.color);
        color = new PdfColor(borderColor.r, borderColor.g, borderColor.b);
      }
      const pen: PdfPen = new PdfPen(color, width);
      // set border dashStyle 'Solid <default>, Dash, Dot, DashDot, DashDotDot'
  ...
  private applyEvent(): {
    ...
    let mdxQuery: string;
    const currentPivotValues: IAxisSet[][] =
      PivotExportUtil.getClonedPivotValues(
        this.engine.pivotValues
      ) as IAxisSet[][];
    if (
      this.parent.exportAllPages &&
      (this.parent.enableVirtualization || this.parent.enablePaging) &&
      ...
      (this.engine as PivotEngine).generateGridData(
        this.parent.dataSourceSettings as IDataOptions,
        true,
        true
      );
      this.parent.applyFormatting(this.engine.pivotValues);
      clonedValues = PivotExportUtil.getClonedPivotValues(
        this.engine.pivotValues
      ) as IAxisSet[][];
  ...
  private updateOlapPageSettings(isUpdate: boolean): void {
    ...
    if (!this.parent.exportSpecifiedPages) {
      this.parent.olapEngineModule.pageSettings = isUpdate
        ? null
        : this.parent.olapEngineModule.pageSettings;
      this.parent.olapEngineModule.isPaging = isUpdate ? false : true;
    } else {
      this.parent.olapEngineModule.exportSpeciedPages =
        this.parent.exportSpecifiedPages = isUpdate
          ? this.parent.exportSpecifiedPages
          : undefined;
    }
  }
