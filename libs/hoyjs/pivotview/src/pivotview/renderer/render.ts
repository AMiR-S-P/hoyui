...
} from '../../base/olap/engine';
import { PivotUtil } from '../../base/util';
import { SelectedCellsInfo } from '../../common/popups/grouping';
import { AggregateTypes } from '../../common/base/enum';
import { FocusStrategy } from '@hoyui/grids/src/grid/services/focus-strategy';
import { FieldOptionsModel } from '../../model/datasourcesettings-model';
import { ExportType } from '@hoyui/charts';
import { PdfImage, PdfTextWebLink } from '@hoyui/pdf-export';
...
export class Render {
  ...
  private lvlCollection: {
    [key: string]: { position: number; hasChild: boolean };
  } = {};
  private hierarchyCollection: {
    [key: string]: { lvlPosition: number; hierarchyPOs: number };
  } = {};
  ...
  public render(refreshRequired?: boolean): void {
    ...
    if (
      ...
      this.parent.grid.setProperties(
        {
          ...
          dataSource:
            ...
            this.parent.dataSourceSettings.url !== ''
              ? true
              : this.parent.dataSourceSettings.dataSource &&
                this.parent.engineModule.data.length > 0 &&
                this.parent.dataSourceSettings.values.length > 0) &&
            !this.engine.isEmptyData
              ? this.frameGridDataSource(this.engine.valueContent)
      ...
      if (
        this.parent.showGroupingBar &&
        this.parent.groupingBarModule &&
      ...
      const e: HTMLElement = this.parent.element.querySelector(
        '.' + cls.GRID_CLASS
      ) as HTMLElement;
      e.querySelector('colGroup').innerHTML = this.parent.grid
        .getHeaderContent()
        .querySelector('colgroup').innerHTML;
      this.parent.grid.width = this.calculateGridWidth();
      if (
        !this.gridSettings.allowAutoResizing &&
        this.parent.showGroupingBar &&
        this.parent.groupingBarModule &&
        this.parent.element.querySelector('.' + cls.GROUPING_BAR_CLASS)
      ) {
      ...
      const gridElement: HTMLElement = createElement('div', {
  ...
  private refreshHeader(): void {
    const mCont: HTMLElement = this.parent.element.querySelector(
      '.' + cls.CONTENT_VIRTUALTABLE_DIV
    ) as HTMLElement;
    if (this.parent.enableVirtualization && !isNullOrUndefined(mCont)) {
      const virtualTable: HTMLElement = (
        closest(mCont, '.' + cls.GRID_CONTENT) as HTMLElement
      ).querySelector('.' + cls.VIRTUALTABLE_DIV);
      const mHdr: HTMLElement = this.parent.element.querySelector(
        '.' + cls.MOVABLEHEADER_DIV
      ) as HTMLElement;
      const vtr: HTMLElement = mCont.querySelector(
        '.' + cls.VIRTUALTRACK_DIV
      ) as HTMLElement;
      this.parent.virtualHeaderDiv = mHdr.querySelector(
        '.' + cls.VIRTUALTRACK_DIV
      ) as HTMLElement;
      ...
      if (mHdr.querySelector('.' + cls.TABLE)) {
        setStyleAttribute(mHdr.querySelector('.' + cls.TABLE) as HTMLElement, {
          transform:
            (
              mCont.querySelector('.' + cls.TABLE) as HTMLElement
            ).style.transform.split(',')[0] +
            ',' +
            0 +
            'px)',
        });
        const freezedCellValue: number = parseInt(
          (mHdr.querySelector('.' + cls.TABLE) as HTMLElement).style.transform
            .split('(')[1]
            .split(',')[0]
            .split('px')[0],
          10
        ...
        if (this.parent.enableRtl) {
          ...
          ).style.right = Number(freezedCellValue) + 'px';
        } else {
          (
            this.parent.element.querySelector(
              '.' + cls.FREEZED_CELL
            ) as HTMLElement
          ).style.left = Number(-freezedCellValue) + 'px';
        }
      }
      const ele: HTMLElement = this.parent.isAdaptive
        ? mCont
        : virtualTable && virtualTable.style.display !== 'none'
        ? virtualTable
        : mCont.parentElement;
  ...
  public bindGrid(parent: PivotView, isEmpty: boolean): void {
    ...
    this.parent.grid = new Grid({
      ...
      allowReordering: this.parent.showGroupingBar
        ? false
        : this.gridSettings.allowReordering,
      allowSelection: this.gridSettings.allowSelection,
      contextMenuItems: this.gridSettings.contextMenuItems as
        | ContextMenuItem[]
        | ContextMenuItemModel[],
      selectedRowIndex: this.gridSettings.selectedRowIndex,
      selectionSettings: this.gridSettings
        .selectionSettings as SelectionSettingsModel,
      ...
      beforeCopy: this.gridSettings.beforeCopy
        ? this.gridSettings.beforeCopy.bind(this.parent)
        : undefined,
      beforePrint: this.gridSettings.beforePrint
        ? this.gridSettings.beforePrint.bind(this.parent)
        : undefined,
      printComplete: this.gridSettings.printComplete
        ? this.gridSettings.printComplete.bind(this.parent)
        : undefined,
      rowSelecting: this.gridSettings.rowSelecting
        ? this.gridSettings.rowSelecting.bind(this.parent)
        : undefined,
      rowSelected: this.rowSelected.bind(this),
      rowDeselecting: this.gridSettings.rowDeselecting
        ? this.gridSettings.rowDeselecting.bind(this.parent)
        : undefined,
      rowDeselected: this.rowDeselected.bind(this),
      cellSelecting: this.gridSettings.cellSelecting
        ? this.gridSettings.cellSelecting.bind(this.parent)
        : undefined,
      cellSelected: this.cellSelected.bind(this),
      cellDeselecting: this.gridSettings.cellDeselecting
        ? this.gridSettings.cellDeselecting.bind(this.parent)
        : undefined,
      cellDeselected: this.cellDeselected.bind(this),
      resizeStart: this.gridSettings.resizeStart
        ? this.gridSettings.resizeStart.bind(this.parent)
        : undefined,
      columnDragStart: this.gridSettings.columnDragStart
        ? this.gridSettings.columnDragStart.bind(this)
        : undefined,
      columnDrag: this.gridSettings.columnDrag
        ? this.gridSettings.columnDrag.bind(this)
        : undefined,
      columnDrop: this.gridSettings.columnDrop
        ? this.gridSettings.columnDrop.bind(this)
        : undefined,
      ...
      beforePdfExport: this.gridSettings.beforePdfExport
        ? this.gridSettings.beforePdfExport.bind(this)
        : undefined,
  ...
  private headerRefreshed(): void {
    ...
    if (
      this.parent.lastGridSettings &&
      Object.keys(this.parent.lastGridSettings).indexOf('allowResizing') > -1 &&
      !isNullOrUndefined(mHdr) &&
      mHdr.querySelector('.' + cls.TABLE) &&
      this.parent.showGroupingBar &&
      this.parent.groupingBarModule &&
      this.parent.element.querySelector('.' + cls.GROUPING_BAR_CLASS)
    ) {
  ...
  private beforeExcelExport(args: { gridObject: IGrid }): void {
    if (
      !isNullOrUndefined(args.gridObject.columns) &&
      !isNullOrUndefined(this.parent.pivotColumns)
    ) {
      (
        args.gridObject.columns[args.gridObject.columns.length - 1] as Column
      ).width =
  ...
  private cellSelected(args: CellSelectEventArgs): void {
    if (this.parent.rowRangeSelection.enable) {
      this.parent.grid.selectionModule.selectRowsByRange(
        this.parent.rowRangeSelection.startIndex,
        this.parent.rowRangeSelection.endIndex
      );
  ...
  private excelHeaderQueryCellInfo(
    args: ExcelHeaderQueryCellInfoEventArgs
  ): void {
  ...
  private setFocusOnLastCell(): void {
    if (
      ...
      const gridFocus: FocusStrategy =
        this.parent.grid.serviceLocator.getService<FocusStrategy>('focus');
      gridFocus.setFocusedElement(
        this.parent.keyboardModule.event.target as HTMLElement
      );
      ...
      (this.parent.keyboardModule.event.target as HTMLElement).setAttribute(
        'tabindex',
        '0'
      );
  ...
  private getCellElement(target: Element): Element {
    const axis: string =
      target.parentElement.classList.contains(cls.ROWSHEADER) ||
      target.classList.contains(cls.ROWSHEADER)
        ? 'row'
        : 'column';
    const currentElement: Element =
      axis === 'column' ? closest(target, 'th') : closest(target, 'td');
  ...
  private contextMenuOpen(args: BeforeOpenCloseMenuEventArgs): void {
    ...
    for (const item of args.items) {
      ...
      if (
        !elem ||
        (elem &&
          Number(elem.getAttribute('index')) === 0 &&
          Number(elem.getAttribute('data-colindex')) === 0)
      ) {
        args.cancel = true;
        return;
      }
      if (
        elem.classList.contains('e-valuesheader') ||
        elem.classList.contains('e-stot')
      ) {
        bool = true;
      }
      if (
        this.parent.allowGrouping &&
        this.parent.groupingModule &&
        !this.validateField(elem as HTMLElement)
      ) {
        isGroupElement = true;
      }
      const rowIndex: number = Number(elem.getAttribute('index'));
      const colIndex: number = Number(elem.getAttribute('data-colindex'));
      const pivotValue1: IAxisSet = this.parent.pivotValues[rowIndex as number][
        colIndex as number
      ] as IAxisSet;
      ...
      switch (selectedID) {
        case this.parent.element.id + '_expand':
          if (elem.querySelectorAll('.' + cls.EXPAND).length > 0) {
            if (
              selectAll('#' + this.parent.element.id + '_expand', args.element)
            ) {
              select(
                '#' + this.parent.element.id + '_expand',
                args.element
              ).classList.add(cls.MENU_DISABLE);
            }
            if (
              select(
                '#' + this.parent.element.id + '_expand',
                args.element
              ).classList.contains(cls.MENU_DISABLE)
            ) {
              select(
                '#' + this.parent.element.id + '_expand',
                args.element
              ).classList.remove(cls.MENU_DISABLE);
            }
            if (
              select(
                '#' + this.parent.element.id + '_expand',
                args.element
              ).classList.contains(cls.MENU_HIDE)
            ) {
              select(
                '#' + this.parent.element.id + '_expand',
                args.element
              ).classList.remove(cls.MENU_HIDE);
              select(
                '#' + this.parent.element.id + '_collapse',
                args.element
              ).classList.remove(cls.MENU_HIDE);
            }
          } else {
        ...
        case this.parent.element.id + '_collapse':
          if (elem.querySelectorAll('.' + cls.COLLAPSE).length > 0) {
            if (
              select('#' + this.parent.element.id + '_expand', args.element)
            ) {
              select(
                '#' + this.parent.element.id + '_expand',
                args.element
        ...
        case this.parent.element.id + '_custom_group':
          ...
          if (
            selectAll(
              '#' + this.parent.element.id + '_custom_group',
              args.element
            )
          ) {
            addClass(
        ...
        case this.parent.element.id + '_custom_ungroup':
          if (
            selectAll(
              '#' + this.parent.element.id + '_custom_ungroup',
              args.element
            )
          ) {
            addClass(
          ...
          if (isGroupElement) {
            ...
            let groupField: IGroupSettings = PivotUtil.getFieldByName(
              fieldName,
              this.parent.dataSourceSettings.groupSettings
            ) as IGroupSettings;
            if (
              (groupField && groupField.type === 'Custom') ||
              (this.parent.engineModule.fieldList[fieldName as string]
                .isCustomField &&
                fieldName.indexOf('_custom_group') > -1)
            ) {
              groupField = PivotUtil.getFieldByName(
                fieldName.replace('_custom_group', ''),
                this.parent.dataSourceSettings.groupSettings
              ) as IGroupSettings;
              if (groupField) {
                const cell: IAxisSet = this.parent.engineModule.pivotValues[
                  Number(elem.getAttribute('index'))
                ][Number(elem.getAttribute('data-colindex'))] as IAxisSet;
                const selectedCellsInfo: SelectedCellsInfo[] =
                  this.parent.groupingModule.getSelectedCells(
                    cell.axis,
                    fieldName,
                ...
                const selectedOptions: string[] =
                  this.parent.groupingModule.getSelectedOptions(
                    selectedCellsInfo
                  );
              ...
              (groupField &&
                (groupField.type === 'Date' || groupField.type === 'Number')) ||
              (this.parent.engineModule.fieldList[fieldName as string]
                .isCustomField &&
                fieldName.indexOf('_date_group') > -1)
            ) {
              isUngroupOption = true;
            }
            if (
        ...
        case this.parent.element.id + '_drillthrough':
          if (!this.parent.allowDrillThrough) {
            if (
              select(
                '#' + this.parent.element.id + '_drillthrough',
                args.element
              )
            ) {
              select(
        ...
        case this.parent.element.id + '_sortasc':
          if (!this.parent.enableValueSorting) {
            if (
              select('#' + this.parent.element.id + '_sortasc', args.element)
            ) {
              select(
                '#' + this.parent.element.id + '_sortasc',
                args.element
              ).classList.add(cls.MENU_DISABLE);
            }
          } else if (elem.querySelectorAll('.e-icon-descending').length > 0) {
            if (
              select('#' + this.parent.element.id + '_sortdesc', args.element)
            ) {
              select(
                '#' + this.parent.element.id + '_sortdesc',
                args.element
              ).classList.add(cls.MENU_DISABLE);
            } else {
              select(
        ...
        case this.parent.element.id + '_CalculatedField':
          if (!this.parent.allowCalculatedField) {
            select(
              '#' + this.parent.element.id + '_CalculatedField',
              args.element
            ).classList.add(cls.MENU_DISABLE);
          }
          break;
        ...
        case this.parent.element.id + '_exporting':
          if (!this.parent.allowExcelExport && !this.parent.allowPdfExport) {
            select(
              '#' + this.parent.element.id + '_exporting',
              args.element
            ).classList.add(cls.MENU_DISABLE);
          }
          break;
        case this.parent.element.id + '_aggregate':
          if (
            select('#' + this.parent.element.id + '_aggregate', args.element) &&
            !select(
              '#' + this.parent.element.id + '_aggregate',
              args.element
            ).classList.contains(cls.MENU_DISABLE)
          ) {
            select(
          ...
          if (
            (elem.classList.contains('e-valuesheader') ||
              elem.classList.contains('e-valuescontent') ||
              (elem.classList.contains('e-stot') &&
                elem.classList.contains('e-rowsheader'))) &&
            this.parent.dataType !== 'olap'
          ) {
            let fieldType: string;
            if (!((elem as HTMLElement).innerText === '')) {
              fieldType =
                this.parent.engineModule.fieldList[
                  pivotValue1.actualText.toString()
                ].type;
            }
            ...
            this.parent.trigger(
              events.aggregateMenuOpen,
              eventArgs,
              (observedArgs: AggregateMenuOpenEventArgs) => {
                if (
                  !observedArgs.cancel &&
                  ...
                  for (
                    let i: number = 0;
                    i < observedArgs.aggregateTypes.length;
                    i++
                  ) {
                    const key: AggregateTypes = observedArgs.aggregateTypes[
                      i as number
                    ] as AggregateTypes;
                    if (fieldType !== 'number') {
                      if (
                        ['Count', 'DistinctCount'].indexOf(key) > -1 &&
                        checkDuplicates.indexOf(key) < 0
                      ) {
                        menuItem.push({
                          text: this.parent.localeObj.getConstant(key),
                          id: this.parent.element.id + '_Agg' + key,
                        });
                        checkDuplicates.push(key);
                      }
                    } else {
                      if (
                        this.parent.getAllSummaryType().indexOf(key) > -1 &&
                        checkDuplicates.indexOf(key) < 0
                      ) {
                        menuItem.push({
                          text: this.parent.localeObj.getConstant(key),
  ...
  private getMenuItem(isStringField: string): AggregateTypes[] {
    ...
    for (let i: number = 0; i < this.parent.aggregateTypes.length; i++) {
      const key: AggregateTypes = this.parent.aggregateTypes[
        i as number
      ] as AggregateTypes;
      if (isStringField !== 'string') {
        if (
          this.parent.getAllSummaryType().indexOf(key) > -1 &&
          menuItems.indexOf(key) === -1
        ) {
          menuItems.push(key);
        }
      } else {
  ...
  private contextMenuClick(args: MenuEventArgs): void {
    ...
    const pivotValue: IAxisSet = this.parent.pivotValues[rowIndx as number][
      colIndx as number
    ] as IAxisSet;
    let aggregateType: string;
    if (args.item.id.indexOf(this.parent.element.id + '_Agg') > -1) {
      this.field =
        this.parent.engineModule.fieldList[pivotValue.actualText.toString()].id;
      this.fieldCaption =
        this.parent.engineModule.fieldList[
          pivotValue.actualText.toString()
        ].caption;
    ...
    switch (selected) {
      case this.parent.element.id + '_pdf':
        ...
        this.parent.trigger(
          events.beforeExport,
          exportArgs,
          (observedArgs: BeforeExportEventArgs) => {
            this.parent.pdfExport(
              observedArgs.pdfExportProperties,
              observedArgs.isMultipleExport,
              observedArgs.pdfDoc,
              observedArgs.isBlob
            );
          }
        ...
        this.parent.trigger(
          ...
          (observedArgs: BeforeExportEventArgs) => {
            this.parent.excelExport(
              observedArgs.excelExportProperties,
              observedArgs.isMultipleExport,
              observedArgs.workbook,
              observedArgs.isBlob
            );
          }
        );
      ...
      case this.parent.element.id + '_sortasc':
        this.parent.setProperties({
          dataSourceSettings: {
            valueSortSettings: {
              headerText: (pivotValue as IAxisSet).valueSort
                .levelName as string,
              headerDelimiter:
        ...
        this.parent.dataSourceSettings.valueSortSettings.sortOrder =
          'Ascending';
        break;
      case this.parent.element.id + '_sortdesc':
        this.parent.setProperties({
      ...
      case this.parent.element.id + '_expand':
        if (ele.querySelectorAll('.' + cls.EXPAND)) {
          const exp: Element = ele.querySelectorAll(
            '.' + cls.EXPAND
          )[0] as Element;
      ...
      case this.parent.element.id + '_collapse':
        if (ele.querySelectorAll('.' + cls.COLLAPSE)) {
          const colp: Element = ele.querySelectorAll(
            '.' + cls.COLLAPSE
          )[0] as Element;
      ...
      case this.parent.element.id + '_AggPercentageOfParentTotal':
        ...
        ele.setAttribute(
          'data-type',
          this.engine.fieldList[pivotValue.actualText.toString()].aggregateType
        );
        ele.setAttribute(
          'data-basefield',
          this.engine.fieldList[pivotValue.actualText.toString()].baseField
        );
        ele.setAttribute(
          'data-baseItem',
          this.engine.fieldList[pivotValue.actualText.toString()].baseItem
        );
        this.aggMenu.createValueSettingsDialog(
          ele as HTMLElement,
          this.parent.element,
          aggregateType
        );
  ...
  private validateColumnTotalcell(columnIndex: number): boolean {
    ...
    for (let pos: number = keysLength - 1; pos >= 0; pos--) {
      const cell: IAxisSet =
        this.engine.headerContent[Number(headerPosKeys[pos as number])][
          columnIndex as number
        ];
      if (cell) {
        sumLock =
          sumLock && fieldName !== ''
            ? fieldName === cell.valueSort.axis
            : false;
  ...
  private validateField(target: HTMLElement): boolean {
    ...
    if (
      target.classList.contains('e-stot') ||
      target.classList.contains('e-gtot') ||
      target.classList.contains('e-valuescontent') ||
      target.classList.contains('e-valuesheader')
    ) {
      isValueField = true;
    } else {
      ...
      if (!fieldName || fieldName === '') {
        ...
        fieldName = (
          this.engine.pivotValues[rowIndx as number][
            colIndx as number
          ] as IAxisSet
        ).actualText as string;
      }
      const valuefields: IFieldOptions[] =
        this.parent.dataSourceSettings.values;
      for (
        let valueCnt: number = 0;
        valueCnt < valuefields.length;
        valueCnt++
      ) {
        if (
          this.parent.dataSourceSettings.values[valueCnt as number].name ===
          fieldName
        ) {
          isValueField = true;
          break;
  ...
  private updateAggregate(aggregate: string): void {
    if (
      this.parent.getAllSummaryType().indexOf(aggregate as AggregateTypes) > -1
    ) {
      const valuefields: IFieldOptions[] =
        this.parent.dataSourceSettings.values;
      for (
        let valueCnt: number = 0;
        valueCnt < this.parent.dataSourceSettings.values.length;
        valueCnt++
      ) {
        if (
          this.parent.dataSourceSettings.values[valueCnt as number].name ===
          ...
          >valuefields[valueCnt as number];
  ...
  public updateGridSettings(): void {
    ...
    this.parent.grid.allowReordering = this.parent.showGroupingBar
      ? false
      : this.gridSettings.allowReordering;
    this.parent.grid.allowSelection = this.gridSettings.allowSelection;
    this.parent.grid.contextMenuItems = this.gridSettings.contextMenuItems as
      | ContextMenuItem[]
      | ContextMenuItemModel[];
    this.parent.grid.selectedRowIndex = this.gridSettings.selectedRowIndex;
    this.parent.grid.selectionSettings = this.gridSettings
      .selectionSettings as SelectionSettingsModel;
  ...
  private updatePivotColumns(): void {
    ...
    for (
      let colPos: number = 0;
      colPos < this.parent.pivotColumns.length;
      colPos++
    ) {
      const pivotColumn: PivotColumn =
        this.parent.pivotColumns[colPos as number];
      for (let keyPos: number = 0; keyPos < keys.length; keyPos++) {
        const key: string = keys[keyPos as number];
        if (
          !isNullOrUndefined(
            this.parent.pivotColumns[colPos as number][key as keyof PivotColumn]
          )
        ) {
          (pivotColumn as ReturnType<typeof Object>)[key as string] = (
            this.parent.lastGridSettings as {
              [key: string]: Object;
            }
          )[key as string];
        }
  ...
  private clearColumnSelection(): void {
    removeClass(
      this.parent.element.querySelectorAll('.' + cls.CELL_ACTIVE_BGCOLOR),
      [cls.CELL_ACTIVE_BGCOLOR, cls.SELECTED_BGCOLOR]
    );
  }
  ...
  private appendValueSortIcon(
    ...
    const engine: PivotEngine | OlapEngine =
      this.parent.dataType === 'pivot'
        ? this.parent.engineModule
        : this.parent.olapEngineModule;
    const vSort: IValueSortSettings =
      this.parent.dataSourceSettings.valueSortSettings;
    if (this.parent.enableValueSorting && vSort && vSort.headerText !== '') {
      const len: number =
        ...
        !this.parent.dataSourceSettings.alwaysShowValueHeader
          ...
          : engine.headerContent.length - 1;
      const lock: boolean =
        vSort && vSort.headerText
          ? cell.valueSort.levelName === vSort.headerText
          : cCnt === vSort.columnIndex;
      if (
        vSort !== undefined &&
        lock &&
        (rCnt === len ||
        ...
        tCell.querySelector('div div').appendChild(
          createElement('span', {
            className:
              (vSort.sortOrder === 'Descending'
              ...
              (cell.hasChild ? ' e-value-sort-align' : ''),
  ...
  private onResizeStop(args: ResizeArgs): void {
    if (args.column.columns && args.column.columns.length > 0) {
      ...
      const column: string =
        args.column.field === '0.formattedText'
          ? '0.formattedText'
          : ((args.column.customAttributes.cell as IAxisSet).valueSort
              .levelName as string);
      this.parent.resizeInfo[column as string] = Number(
        args.column.width.toString().split('px')[0]
      );
    }
    if (
      ...
      if (
        ...
        !isNullOrUndefined(
          (
            this.parent.grid.columns[
              this.parent.grid.columns.length - 1
            ] as ColumnModel
          ).columns
        )
      ) {
        ...
        (gridColumns[gridColumns.length - 1] as ColumnModel).minWidth =
        ...
        ).minWidth = this.parent.gridSettings.columnWidth;
      }
      this.parent.layoutRefresh();
    }
  ...
  private getChildColumnWidth(parentColumn: ColumnModel[]): void {
    for (const column of parentColumn) {
      if (column.columns && column.columns.length > 0) {
        ...
        const colName: string = (column.customAttributes.cell as IAxisSet)
          .valueSort.levelName as string;
        this.parent.resizeInfo[colName as string] = Number(
          column.width.toString().split('px')[0]
        );
      }
    }
  }
  ...
  private setGroupWidth(args: ResizeArgs): void {
    if (
      ...
      if (
        ...
        this.parent.groupingBarModule &&
        this.parent.element.querySelector('.' + cls.GROUPING_BAR_CLASS) &&
        Number(args.column.width.toString().split('px')[0]) < 250
      ) {
        args.cancel = true;
      }
      // else {
    ...
    if (
      ...
      this.parent.groupingBarModule.refreshUI();
      if (
        (this.parent.element.querySelector('.e-group-row') as HTMLElement)
          .offsetWidth < 245 &&
        !this.parent.firstColWidth
      ) {
        args.cancel = true;
        ...
        this.parent.element
          .querySelector('.' + cls.HEADERCONTENT)
          .querySelector('col').style.width = this.resColWidth + 'px';
        this.parent.element
          .querySelector('.' + cls.CONTENT_CLASS)
          .querySelector('col').style.width = this.resColWidth + 'px';
      }
      (
        this.parent.element.querySelector('.e-group-rows') as HTMLElement
      ).style.height = 'auto';
      (
        this.parent.element.querySelector('.e-group-values') as HTMLElement
      ).style.width =
        (this.parent.element.querySelector('.e-group-row') as HTMLElement)
          .offsetWidth + 'px';
      const firstRowHeight: number = (
        this.parent.element.querySelector(
          '.' + cls.HEADERCONTENT
        ) as HTMLElement
      ).offsetHeight;
    ...
    if (this.parent.enableVirtualization) {
      this.parent.resizedValue =
        args.cancel || args.column.field !== '0.formattedText'
          ? this.parent.resizedValue
          : Number(args.column.width.toString().split('px')[0]);
    }
    this.parent.trigger(
      args.e.type === 'touchend' || args.e.type === 'mouseup'
        ? events.resizeStop
        : events.resizing,
      args
    );
  ...
  private onSelect(): void {
    const pivotArgs: PivotCellSelectedEventArgs = {
      selectedCellsInfo: [],
    ...
    for (let i: number = 0; i < selectedElements.length; i++) {
      ...
      const cell: IAxisSet = this.engine.pivotValues[rowIndex as number][
        colIndex as number
      ] as IAxisSet;
      if (cell) {
        if (cell.axis === 'value') {
  ...
  private rowCellBoundEvent(args: QueryCellInfoEventArgs): void {
    ...
    if (tCell && this.parent.notEmpty && this.engine.headerContent) {
      ...
      const isRowFieldsAvail: boolean =
        cell.valueSort &&
        cell.valueSort.levelName ===
          (this.parent.dataSourceSettings.rows.length === 0 &&
            this.parent.dataSourceSettings.valueAxis === 'row' &&
            this.parent.localeObj.getConstant('grandTotal') +
              this.parent.dataSourceSettings.valueSortSettings.headerDelimiter +
              cell.formattedText);
      tCell.setAttribute(
        'index',
        cell.rowIndex ? cell.rowIndex.toString() : '0'
      );
      const pivotValue: IAxisSet =
        this.parent.pivotValues[cell.rowIndex] &&
        this.parent.pivotValues[cell.rowIndex][
          Number(tCell.getAttribute('data-colindex'))
        ]
          ? (this.parent.pivotValues[cell.rowIndex][
              Number(tCell.getAttribute('data-colindex'))
            ] as IAxisSet)
      ...
      if (tCell.getAttribute('data-colindex') === '0') {
        if (this.parent.dataType === 'pivot') {
          ...
          tCell.innerText = '';
          const levelName: string = cell.valueSort
            ? cell.valueSort.levelName.toString()
            : '';
          ...
          const levelPosition: number =
            levelName.split(
              this.parent.dataSourceSettings.valueSortSettings.headerDelimiter
            ).length - (memberPos ? memberPos - 1 : memberPos);
          let level: number = levelPosition ? levelPosition - 1 : 0;
          if (
            this.parent.dataSourceSettings.subTotalsPosition === 'Bottom' &&
            !isNullOrUndefined(levelName)
          ) {
            const cellLevelName: string = !cell.isSum
              ? levelName
              : cell.type === 'value'
              ? levelName.split(
                  this.parent.dataSourceSettings.valueSortSettings
                    .headerDelimiter +
                    (this.parent.engineModule.valueAxisFields[cell.actualText]
                      .caption
                      ? this.parent.engineModule.valueAxisFields[
                          cell.actualText
                        ].caption
                      : this.parent.engineModule.valueAxisFields[
                          cell.actualText
            ...
            if (
              cell.isSum &&
              (cell.type === 'value'
                ? this.drilledLevelInfo[cellLevelName as string]
                : true)
            ) {
              level = level - 1;
            } else if (!cell.isSum) {
          ...
          if (
            ...
            if (isValueCell) {
              ...
              fieldName =
                cell.level > -1 &&
                this.parent.dataSourceSettings.rows[cell.level]
        ...
        if (cell.type) {
          if (cell.type === 'grand sum') {
            ...
            const values: FieldOptionsModel[] =
              this.parent.dataSourceSettings.values;
            localizedText = isNullOrUndefined(cell.valueSort.axis)
              ? this.parent.dataSourceSettings.rows.length === 0 &&
                values.length === 1 &&
                this.parent.dataSourceSettings.valueAxis === 'row'
                ? this.parent.localeObj.getConstant('total') +
                  ' ' +
                  this.parent.localeObj.getConstant(
                    values[values.length - 1].type
                  ) +
                  ' ' +
                  this.parent.localeObj.getConstant('of') +
                  ' ' +
                  (!isNullOrUndefined(values[values.length - 1].caption)
                    ? values[values.length - 1].caption
            ...
            tCell.classList.add('e-gtot');
            localizedText = isRowFieldsAvail
              ? this.parent.localeObj.getConstant('total') +
                ' ' +
                this.parent.localeObj.getConstant(
                  this.parent.engineModule.fieldList[cell.actualText]
                    .aggregateType
                ) +
                ' ' +
                this.parent.localeObj.getConstant('of') +
            ...
            localizedText =
              cell.formattedText.split('Total')[0] +
              this.parent.localeObj.getConstant('total');
          } else {
        ...
        if (cell.hasChild === true && !cell.isNamedSet) {
          tCell.appendChild(
            createElement('div', {
              className:
                (cell.isDrilled === true ? cls.COLLAPSE : cls.EXPAND) +
                ' ' +
                cls.ICON,
              attrs: {
                title:
                  cell.isDrilled === true
                    ? this.parent.localeObj.getConstant('collapse')
        ...
        tCell.appendChild(
          createElement('span', {
            ...
            innerHTML:
              this.parent.isRowCellHyperlink || cell.enableHyperlink
                ? '<a  data-url="' +
                  localizedText +
                  '" class="e-hyperlinkcell ' +
                  customClass +
                  '">' +
                  localizedText +
                  '</a>'
        ...
        const vSort: IValueSortSettings =
          this.parent.pivotView.dataSourceSettings.valueSortSettings;
        if (this.parent.enableValueSorting) {
          if (
            vSort &&
            vSort.headerText &&
            ...
            (
              this.parent.pivotValues[
                Number(tCell.getAttribute('index'))
              ][0] as IAxisSet
            ).valueSort.levelName
          ) {
            if (
              ...
              ).valueSort.levelName === vSort.headerText
            ) {
              tCell.appendChild(
                createElement('span', {
                  className:
                    ...
                    (cell.hasChild ? ' e-value-sort-align' : ''),
                  styles:
                    tCell.style.textAlign === 'right' ? 'float: left' : '',
                })
              );
            }
          }
        ...
        cell = (args.data as IGridValues)[
          Number(tCell.getAttribute('data-colindex'))
        ] as IAxisSet;
        cell = isNullOrUndefined(cell)
          ? (args.column.customAttributes.cell as IAxisSet)
          : cell;
        cell.isGrandSum = isRowFieldsAvail ? true : cell.isGrandSum;
        if (cell.isSum) {
          tCell.classList.add(cls.SUMMARY);
        }
        const isGrandSum: boolean =
          isNullOrUndefined(cell.isGrandSum) &&
          !isNullOrUndefined(this.parent.olapEngineModule) &&
          this.parent.olapEngineModule.olapValueAxis === 'column' &&
          this.parent.dataType === 'olap' &&
          this.colGrandPos - this.parent.dataSourceSettings.values.length <
            Number(tCell.getAttribute('data-colindex'));
        if (
          cell.isGrandSum ||
          isGrandSum ||
          this.colGrandPos === Number(tCell.getAttribute('data-colindex')) ||
          this.rowGrandPos === Number(tCell.getAttribute('index'))
        ) {
          tCell.classList.add('e-gtot');
        } else if (
          this.parent.dataType === 'olap'
            ? cell.isSum
            : this.validateColumnTotalcell(cell.colIndex)
        ) {
        ...
        tCell.appendChild(
          createElement('span', {
            className: cls.CELLVALUE,
            innerHTML:
              (tCell.className.indexOf('e-summary') !== -1 &&
                this.parent.isSummaryCellHyperlink) ||
              (tCell.className.indexOf('e-summary') === -1 &&
                this.parent.isValueCellHyperlink) ||
              cell.enableHyperlink
                ? '<a data-url="' +
                  innerText +
                  '" class="e-hyperlinkcell ' +
                  customClass +
        ...
        if (
          this.parent.gridSettings.allowReordering &&
          !this.parent.showGroupingBar
        ) {
          tCell.setAttribute(
            'data-colindex',
            args.column.customAttributes
              ? (
                  args.column.customAttributes.cell as IAxisSet
                ).colIndex.toString()
              : args.column.index.toString()
          );
        }
      }
  ...
  private appendTemplate(tCell: HTMLElement, pivotValue: IAxisSet): void {
    const element: NodeListOf<HTMLElement> | string =
      this.parent.getCellTemplate()(
        { targetCell: tCell, cellInfo: pivotValue },
        this.parent,
        'cellTemplate',
        this.parent.element.id + '_cellTemplate',
        null,
        null,
        tCell
      );
    if (element && element !== '' && element.length > 0) {
      if (this.parent.enableHtmlSanitizer) {
        if (
          (<{ isVue?: boolean }>this.parent).isVue ||
          (<{ isVue3?: boolean }>this.parent).isVue3
        ) {
          const container: HTMLElement = createElement('div');
          container.innerHTML = SanitizeHtmlHelper.sanitize(element as string);
          append([container.firstChild as Element], tCell);
        } else {
          this.parent.appendHtml(
            tCell,
            SanitizeHtmlHelper.sanitize((element[0] as Element).outerHTML)
          );
        }
      } else {
        if (
  ...
  private onOlapRowCellBoundEvent(
    ...
    if (this.parent.enableVirtualization) {
      if (
        cell.ordinal > -1 &&
        this.parent.olapEngineModule.tupRowInfo.length > 0
      ) {
        const tupInfo: ITupInfo =
          this.parent.olapEngineModule.tupRowInfo[cell.ordinal];
        const cellActualText: string =
          cell.memberType === 3
            ? (
                this.engine.fieldList[
                  cell.actualText.toString()
                ] as IOlapFieldListOptions
              ).tag.toString()
            : cell.actualText.toString();
        const memberPosition: number =
          tupInfo.uNameCollection.indexOf(cellActualText);
        const cropUName: string =
          tupInfo.uNameCollection.substring(0, memberPosition) +
          (cell.memberType === 3 ? '' : cell.actualText.toString());
        const fieldSep: string[] = cropUName
          .split('::[')
          .map((item: string) => {
        ...
        for (let fPos: number = 0; fPos < fieldSep.length; fPos++) {
          ...
          const hasChild: boolean =
            tupInfo.typeCollection[fPos as number] !== '2'
              ? this.engine.fieldList[
                  tupInfo.members[fPos as number].getAttribute('Hierarchy')
                ] &&
                (this.engine.fieldList as IOlapFieldListOptions)[
                  tupInfo.members[fPos as number].getAttribute('Hierarchy')
                ].isHierarchy &&
                fPos < this.parent.dataSourceSettings.rows.length - 1 &&
                !this.parent.dataSourceSettings.rows[fPos + 1].isNamedSet &&
                this.parent.dataSourceSettings.rows[fPos + 1].name.indexOf(
                  '[Measures]'
                ) < 0 &&
                (this.engine.fieldList as IOlapFieldListOptions)[
                  this.parent.dataSourceSettings.rows[fPos + 1].name
                ] &&
                (this.engine.fieldList as IOlapFieldListOptions)[
                  this.parent.dataSourceSettings.rows[fPos + 1].name
                ].hasAllMember
                ? true
                : Number(
                    tupInfo.members[fPos as number].querySelector(
        ...
        for (
          let iPos: number = 0;
          iPos < lastIndextCount && nxtIndextCount > 0;
          iPos++
        ) {
          tCell.appendChild(
            createElement('span', {
      ...
      const actualLevelName: string = cell.valueSort
        ? cell.valueSort.levelName.toString()
        : '';
      const levelPosition: number =
        cell.level === -1
          ? this.measurePos
          : actualLevelName.split(
              this.parent.dataSourceSettings.valueSortSettings.headerDelimiter
            ).length - 1;
      const levelName: string =
        cell.memberType === 3
          ? levelPosition + '.' + cell.levelUniqueName
          : cell.levelUniqueName;
      const hasChild: boolean = cell.hasChild;
      let isSubTotalCell: boolean = false;
      if (cell.isSum && cell.memberType === 3) {
        const currPos: number =
          this.lvlCollection[cell.parentUniqueName].position;
        ...
        this.position =
          this.position > currPos + 1 ? currPos + 1 : this.position;
        isSubTotalCell = true;
      ...
      if (!this.lvlCollection[levelName as string] && levelName) {
        ...
        this.lvlCollection[levelName as string] = {
        ...
        const currPos: number =
          this.lvlCollection[levelName as string].position;
      ...
      if (!this.hierarchyCollection[hierarchyName as string] && hierarchyName) {
        ...
        const currPos: number =
          this.hierarchyCollection[hierarchyName as string].hierarchyPOs;
        for (let pos: number = currPos + 1; pos < this.hierarchyCount; pos++) {
          delete this.hierarchyCollection[
            this.hierarchyPosCollection[pos as number]
          ];
          delete this.hierarchyPosCollection[pos as number];
        }
        this.hierarchyCount =
          this.hierarchyCount > currPos + 1 ? currPos + 1 : this.hierarchyCount;
      }
      if (
        cell.memberType !== 3 &&
        levelName &&
        ...
        const currHierarchyPos: number = this.hierarchyCollection[
          hierarchyName as string
        ]
          ? this.hierarchyCollection[hierarchyName as string].hierarchyPOs
          : -1;
        this.measurePos =
          rowMeasurePos <= currHierarchyPos &&
          this.hierarchyPosCollection[rowMeasurePos + 1]
            ? this.measurePos
            : this.lvlCollection[levelName as string].position;
      }
      let currPos: number = this.lvlCollection[levelName as string]
        ? this.lvlCollection[levelName as string].position
        : -1;
      ...
      while (lvlPos <= currPos && currPos > 0 && cell.level > -1) {
        const hasChild: boolean =
          this.lvlCollection[this.lvlPosCollection[lvlPos as number]].hasChild;
        const prevHasChild: boolean =
          lvlPos > 0
            ? this.lvlCollection[this.lvlPosCollection[lvlPos - 1]].hasChild
            : false;
      ...
      if (
        this.parent.dataSourceSettings.grandTotalsPosition === 'Top' &&
        !isNullOrUndefined(this.parent.olapEngineModule) &&
        this.parent.olapEngineModule.olapValueAxis === 'row' &&
        this.parent.dataType === 'olap' &&
        cell.valueSort.levelName
          .toString()
          .indexOf(
      ...
      if (
        cell.memberType === 3 &&
        cell.level === -1 &&
        Object.keys(this.lvlCollection).length > 1
      ) {
        tCell.appendChild(
          createElement('span', {
            className: cls.NEXTSPAN,
    ...
    const grandTotal: boolean = this.parent.olapEngineModule.tupRowInfo[
      cell.ordinal
    ]
      ? this.parent.olapEngineModule.tupRowInfo[cell.ordinal]
          .measurePosition === 0
        ? this.parent.olapEngineModule.tupRowInfo[cell.ordinal].allStartPos ===
          1
        : this.parent.olapEngineModule.tupRowInfo[cell.ordinal].allStartPos ===
          0
      : false;
      ...
      if (cell) {
        ...
        if ((cell.level === -1 && !cell.rowSpan) || cell.rowSpan === -1) {
          ...
          if (
            ...
            this.engine.headerContent.length
          ) {
        ...
        if (this.parent.dataType === 'pivot') {
          if (
            ...
            !(
              ...
              this.parent.dataSourceSettings.values.length > 1 &&
              isValueCell &&
              cell.rowIndex === this.engine.headerContent.length - 1
            ) &&
            this.parent.dataSourceSettings.columns &&
            this.parent.dataSourceSettings.columns.length > 0
          ) {
            fieldName =
              cell.level > -1 &&
              this.parent.dataSourceSettings.columns[cell.level]
        ...
        const isColumnFieldsAvail: boolean =
          this.parent.dataSourceSettings.columns.length === 0 &&
          this.parent.dataSourceSettings.valueAxis === 'column' &&
          cell.valueSort &&
          cell.valueSort.levelName ===
        ...
        if (cell.type || isColumnFieldsAvail) {
          ...
          let localizedText: string =
            cell.type === 'grand sum'
              ? isNullOrUndefined(cell.valueSort.axis) ||
                this.parent.dataType === 'olap'
                ? this.parent.localeObj.getConstant('grandTotal')
                : cell.formattedText
              : cell.formattedText.split('Total')[0] +
                this.parent.localeObj.getConstant('total');
          localizedText =
            isColumnFieldsAvail && engine.fieldList
              ? this.parent.localeObj.getConstant('total') +
                ' ' +
                this.parent.localeObj.getConstant(
                  engine.fieldList[cell.actualText].aggregateType
                ) +
                ' ' +
                this.parent.localeObj.getConstant('of') +
                ' ' +
          ...
          if ((tCell.querySelector('.e-headertext') as HTMLElement) !== null) {
            (tCell.querySelector('.e-headertext') as HTMLElement).innerText =
              localizedText;
          } else {
            (
              tCell.querySelector('.e-stackedheadercelldiv') as HTMLElement
            ).innerText = localizedText;
          }
        }
        ...
        if (this.parent.isColumnCellHyperlink || cell.enableHyperlink) {
          if (tCell.querySelector('.e-stackedheadercelldiv') as HTMLElement) {
            ...
            const innerText: string = (
              tCell.querySelector('.e-headertext') as HTMLElement
            ).innerText;
        ...
        if (cell.hasChild === true && !cell.isNamedSet) {
          const hdrdiv: HTMLElement = tCell.querySelector(
            '.e-headercelldiv'
          ) as HTMLElement;
          if (hdrdiv) {
        ...
        tCell = this.appendValueSortIcon(
          cell,
          tCell,
          cell.rowIndex,
          cell.colIndex,
        ...
        if (this.parent.cellTemplate) {
          this.appendTemplate(
            tCell,
            this.parent.pivotValues[cell.rowIndex][cell.colIndex] as IAxisSet
          );
        }
        const len: number = this.parent.dataSourceSettings.values.length;
        for (let vCnt: number = 0; vCnt < len; vCnt++) {
          if (
            this.parent.dataSourceSettings.values[vCnt as number].name ===
            cell.actualText
          ) {
            if (this.parent.dataType === 'olap') {
              const grandTotal: boolean = this.parent.olapEngineModule
                .tupColumnInfo[cell.ordinal]
                ? this.parent.olapEngineModule.tupColumnInfo[cell.ordinal]
                    .measurePosition === 0
                  ? this.parent.olapEngineModule.tupColumnInfo[cell.ordinal]
                      .allStartPos === 1
                  : this.parent.olapEngineModule.tupColumnInfo[cell.ordinal]
  ...
  private updateWrapper(tCell: HTMLElement, div?: HTMLElement): HTMLElement {
    if (
      tCell.querySelectorAll('.e-headercelldiv').length > 0 ||
      tCell.querySelectorAll('.e-stackedheadercelldiv').length > 0
    ) {
      ...
      const innerDiv: HTMLElement = createElement('div', {
        className:
          (div ? 'e-stackedheadertext' : 'e-headertext') + ' ' + cls.CELLVALUE,
        innerHTML:
          tCell.querySelectorAll('.e-headercelldiv').length > 0
            ? (tCell.querySelector('.e-headercelldiv') as HTMLElement).innerHTML
            : (tCell.querySelector('.e-stackedheadercelldiv') as HTMLElement)
                .innerHTML,
      });
      if (div) {
  ...
  private onOlapColumnCellBoundEvent(
    ...
    tCell.setAttribute(
      'fieldname',
      cell.memberType === 3 ? cell.actualText.toString() : cell.hierarchy
    );
    const prevCell: IAxisSet = this.engine.headerContent[cell.rowIndex]
      ? this.engine.headerContent[cell.rowIndex][cell.colIndex - 1]
      : undefined;
    if (
      prevCell &&
      prevCell.actualText === cell.actualText &&
    ...
    if (
      ...
      tCell.style.display = this.isSpannedCell(
        this.engine.headerContent.length,
        cell
      )
        ? 'none'
  ...
  private isSpannedCell(colLength: number, currCell: IAxisSet): boolean {
    let prevCell: IAxisSet = this.engine.headerContent[currCell.rowIndex - 1]
      ? this.engine.headerContent[currCell.rowIndex - 1][currCell.colIndex]
      : undefined;
    let parentCellSpan: number;
    let parentCellPos: number;
    while (
      prevCell &&
      (prevCell.memberType === currCell.memberType ||
      ...
      prevCell = this.engine.headerContent[prevCell.rowIndex - 1]
        ? this.engine.headerContent[prevCell.rowIndex - 1][currCell.colIndex]
        : undefined;
    }
  ...
  private onHyperCellClick(e: MouseEvent): void {
    ...
    cell =
      cell.className.indexOf('e-headercelldiv') > -1
        ? cell.parentElement
        : cell;
    const args: HyperCellClickEventArgs = {
      currentCell: cell,
      data: this.engine.pivotValues[Number(cell.getAttribute('index'))][
    ...
    this.parent.trigger(
      events.hyperlinkCellClick,
      args,
      (observedArgs: HyperCellClickEventArgs) => {
        if (!observedArgs.cancel) {
          args.currentCell = getElement(args.currentCell) as Element;
          const url: string = args.currentCell.getAttribute('data-url')
            ? args.currentCell.getAttribute('data-url')
            : (args.currentCell.querySelector('a') as HTMLElement).getAttribute(
                'data-url'
              );
          window.open(url);
        }
  ...
  private getRowStartPos(): number {
    ...
    for (
      let rCnt: number = 0;
      rCnt < (pivotValues ? pivotValues.length : 0);
      rCnt++
    ) {
      if (
        pivotValues[rCnt as number] &&
        pivotValues[rCnt as number][0] &&
        (pivotValues[rCnt as number][0] as IAxisSet).axis === 'row'
      ) {
        rowPos = rCnt;
        break;
      }
    ...
    if (
      ((this.parent.dataType === 'pivot' &&
        ...
        (this.parent.dataSourceSettings.mode === 'Server' &&
          this.parent.dataSourceSettings.url !== '' &&
          this.engine.pivotValues.length > 0)) &&
      this.parent.dataSourceSettings.values.length > 0 &&
      !this.engine.isEmptyData
    ) {
      if (this.parent.enableValueSorting || !this.engine.isEngineUpdated) {
        ...
        for (let rCnt: number = start; rCnt < end; rCnt++) {
          if (pivotValues[rCnt as number]) {
            ...
            for (
              let cCnt: number = 0;
              cCnt < pivotValues[rCnt as number].length;
              cCnt++
            ) {
              if (pivotValues[rCnt as number][cCnt as number]) {
                dataContent[rowCnt as number][cCnt as number] = pivotValues[
                  rCnt as number
                ][cCnt as number] as IAxisSet;
              }
            }
            rowCnt++;
          }
        ...
        dataContent =
          type === 'value'
            ? this.engine.valueContent
            : this.engine.headerContent;
      }
    } else {
  ...
  public calculateColWidth(colCount: number): number {
    if (!isNullOrUndefined(this.parent.resizedValue)) {
      this.parent.resizedValue =
        this.parent.showGroupingBar && this.parent.resizedValue < 250
          ? 250
          : this.parent.resizedValue;
    }
    this.resColWidth = !isNullOrUndefined(this.parent.resizedValue)
      ? this.parent.resizedValue
      : this.resColWidth;
    const offsetWidth: number = this.calculateGridWidth() as number;
    let parWidth: number = isNaN(this.parent.width as number)
      ? this.parent.width.toString().indexOf('%') > -1
        ? (parseFloat(this.parent.width.toString()) / 100) * offsetWidth
        : offsetWidth
      : Number(this.parent.width);
    parWidth =
      parWidth -
      (this.gridSettings.columnWidth > this.resColWidth
        ? this.gridSettings.columnWidth
        : this.resColWidth);
    colCount = colCount - 1;
    ...
    if (!this.isOverflows) {
      ...
      const parentHeight: number =
        gridHeight === 'auto'
          ? this.parent.getHeightAsNumber()
          : (gridHeight as number);
      const headersLength: number =
        this.engine && this.engine.headerContent
          ? Object.keys(this.engine.headerContent).length
          : 1;
      const height: number =
        parentHeight - this.gridSettings.rowHeight * headersLength;
      if (
        this.engine &&
        this.engine.valueContent &&
        this.gridSettings.rowHeight * this.engine.valueContent.length > height
      ) {
        parWidth = parWidth - getScrollBarWidth();
      }
    }
    const colWidth: number =
      colCount * this.gridSettings.columnWidth < parWidth
        ? parWidth / colCount
        : this.gridSettings.columnWidth;
    return !this.isOverflows && !this.gridSettings.allowAutoResizing
      ? this.gridSettings.columnWidth
      : Math.floor(colWidth);
  }
  ...
  public resizeColWidth(colCount: number): number {
    ...
    let parWidth: number = isNaN(this.parent.width as number)
      ? this.parent.width.toString().indexOf('%') > -1
        ? (parseFloat(this.parent.width.toString()) / 100) *
  ...
  public calculateGridWidth(): number | string {
    ...
    if (eleWidth === 0) {
      eleWidth = this.parent.element.parentElement.getBoundingClientRect().width
        ? this.parent.element.parentElement.getBoundingClientRect().width
        : this.parent.element.parentElement.offsetWidth;
    }
    ...
    return !this.gridSettings.allowAutoResizing &&
      (parWidth as number) > this.parent.totColWidth
      ? this.parent.totColWidth
      : parWidth;
  }
  ...
  public calculateGridHeight(elementCreated?: boolean): number | string {
    const contentElement: HTMLElement = this.parent.element.querySelector(
      '.' + cls.GRID_CLASS + ' .' + cls.CONTENT_CLASS
    ) as HTMLElement;
    ...
    if (contentElement) {
      if (isNaN(parHeight)) {
        parHeight =
          parHeight > this.parent.minHeight ? parHeight : this.parent.minHeight;
      } else {
        parHeight =
          contentElement.offsetWidth <
          (contentElement.querySelector('.' + cls.TABLE) as HTMLElement)
            .offsetWidth
            ? parHeight - getScrollBarWidth()
            : parHeight;
      }
      if (
        (this.parent.showToolbar && this.parent.currentView !== 'Chart') ||
        (!this.parent.showToolbar && this.parent.displayOption.view !== 'Chart')
      ) {
        if (
          this.gridSettings.height === 'auto' &&
          parHeight &&
          this.parent.element.querySelector('.' + cls.GRID_HEADER)
        ) {
          const rowColHeight: number = (
            this.parent.element.querySelector(
              '.' + cls.GRID_HEADER
            ) as HTMLElement
          ).offsetHeight;
          const gBarHeight: number =
            rowColHeight +
            (this.parent.element.querySelector(
              '.' + cls.GRID_GROUPING_BAR_CLASS
            )
              ? (
                  this.parent.element.querySelector(
                    '.' + cls.GRID_GROUPING_BAR_CLASS
              ...
              : 0);
          const toolBarHeight: number = this.parent.element.querySelector(
            '.' + cls.GRID_TOOLBAR
          )
            ? 42
            : 0;
          const pagerHeight: number = this.parent.element.querySelector(
            '.' + cls.GRID_PAGER
          )
            ? (
                this.parent.element.querySelector(
            ...
            : 0;
          gridHeight =
            parHeight - (gBarHeight + toolBarHeight + pagerHeight) - 1;
          ...
          if (elementCreated) {
            ...
            const commonOverflow: boolean =
              horizontalOverflow && gridHeight - tableHeight < 18
                ? true
                : false;
            if (
              gridHeight >= tableHeight &&
              (horizontalOverflow ? gridHeight >= contentHeight : true) &&
              !commonOverflow
            ) {
            ...
            if (
              gridHeight >
              this.engine.valueContent.length * this.gridSettings.rowHeight
            ) {
              gridHeight = 'auto';
            }
          }
    ...
    return (gridHeight as number) < this.parent.gridSettings.rowHeight
      ? this.parent.gridSettings.rowHeight
      : gridHeight;
  }
  ...
  public frameStackedHeaders(): ColumnModel[] {
    ...
    const refreshColumn: boolean =
      this.parent.toolbarModule && this.parent.toolbarModule.isReportChange
        ? true
        : this.parent.actionObj
        ? (this.parent.actionObj.actionName === 'Sort value' &&
            engine.valueAxis === 1) ||
          (this.parent.actionObj.actionName === 'Sort field' &&
            this.parent.actionObj.fieldInfo.axis === 'columns') ||
          (this.parent.pivotFieldListModule &&
            this.parent.pivotFieldListModule.actionObj.actionName ===
              'Sort field' &&
            this.parent.pivotFieldListModule.actionObj.fieldInfo.axis ===
              'columns')
        : false;
    this.pivotColumns = [];
    if (
      ...
      const colWidth: number = this.calculateColWidth(
        this.engine.pivotValues && this.engine.pivotValues[0]
          ? this.engine.pivotValues[0].length
          : 0
      );
      const measureFlag: boolean =
        this.parent.dataType === 'olap' &&
        !isNullOrUndefined((this.engine as OlapEngine).colMeasurePos) &&
        (this.engine as OlapEngine).colDepth - 1 ===
          this.parent.olapEngineModule.colMeasurePos;
      do {
        const columnModel: ColumnModel[] = [];
        let actualCnt: number = 0;
        headerCnt--;
        const colField: IAxisSet[] =
          this.engine.headerContent[headerCnt as number];
        const colCount: number = colField ? Object.keys(colField).length : 0;
        if (colField) {
          for (
            let cCnt: number = 0,
              cLen: number =
                Object.keys(colField).length + (colField[0] ? 0 : 1);
            cCnt < cLen;
            cCnt++
          ) {
            let colSpan: number =
              colField[cCnt as number] && colField[cCnt as number].colSpan
                ? colField[cCnt as number].memberType !== 3 ||
                  (colField[cCnt as number].memberType === 3 && !measureFlag) ||
                  headerCnt === 0
                  ? colField[cCnt as number].colSpan
                  : (headerSplit[cCnt as number] as number)
                : 1;
            colSpan =
              this.parent.dataType === 'olap' && isNullOrUndefined(colSpan)
                ? 1
                : colSpan;
            let formattedText: string = colField[cCnt as number]
              ? colField[cCnt as number].type === 'grand sum'
                ? isNullOrUndefined(colField[cCnt as number].valueSort.axis)
                  ? this.parent.localeObj.getConstant('grandTotal')
                  : colField[cCnt as number].formattedText
                : colField[cCnt as number].type === 'sum'
                ? colField[cCnt as number].formattedText.split('Total')[0] +
                  this.parent.localeObj.getConstant('total')
                : colField[cCnt as number].formattedText
              : '';
            formattedText = this.parent.enableHtmlSanitizer
              ? SanitizeHtmlHelper.sanitize(formattedText)
              : formattedText;
            if (headerCnt === this.engine.headerContent.length - 1) {
              colSpan = 1;
              autoFitApplied =
                pivotColumns.length - 1 !== colCount
                  ? false
                  : !refreshColumn &&
                    !this.parent.isEmptyGrid &&
                    pivotColumns[actualCnt as number] &&
                    pivotColumns[actualCnt as number].autoFit;
              columnModel[actualCnt as number] = {
                field: cCnt + '.formattedText',
                headerText: formattedText,
                customAttributes: {
                  cell: this.cloneDataWithoutIndex(
                    colField[cCnt as number] as Record<string, unknown>
                  ),
                },
                width: autoFitApplied
                  ? (gridColumns[actualCnt as number] as ColumnModel).width
                  : colField[cCnt as number]
                  ? colField[cCnt as number].valueSort
                    ? this.setSavedWidth(
                        (colField[cCnt as number].valueSort as IDataSet)
                          .levelName as string,
                        colWidth
                      )
                    : this.resColWidth
                  : this.resColWidth,
                minWidth:
                  autoFitApplied && actualCnt === colCount
                    ? (gridColumns[gridColumns.length - 1] as ColumnModel)
                        .minWidth
                    : 30,
                allowReordering: this.parent.showGroupingBar
                  ? false
              ...
              if (cCnt === colCount) {
                columnModel[actualCnt as number].width =
                  (columnModel[actualCnt as number].width as number) - 3;
              ...
              while (tmpSpan > 0) {
                if (columnModel[actualCnt as number]) {
                  if (
                    !this.pivotColumns[splitPos[innerPos as number] as number]
                  ) {
                    break;
                  }
                  innerModel.push(
                  ...
                  columnModel[actualCnt as number] = {
                    ...
                    width:
                      ...
                      !refreshColumn &&
                      !this.parent.isEmptyGrid &&
                      pivotColumns[0].autoFit
                        ? (gridColumns[0] as ColumnModel).width
                        : colField[cCnt as number]
                        ? this.setSavedWidth(
                            (colField[cCnt as number].valueSort as IDataSet)
                              .levelName as string,
                  ...
                  innerModel = [
                    this.pivotColumns[
                      splitPos[innerPos as number] as number
                    ] as Column,
                  ];
                }
                this.isAutoFitEnabled = this.isAutoFitEnabled
                  ? true
                  : autoFitApplied;
                tmpSpan = tmpSpan - (headerSplit[innerPos as number] as number);
                innerPos =
                  innerPos + (headerSplit[innerPos as number] as number);
              }
              columnModel[actualCnt as number].columns = innerModel;
            }
            if (columnModel[actualCnt as number]) {
              columnModel[actualCnt as number].clipMode =
                this.gridSettings.clipMode;
            }
        ...
        this.pivotColumns =
          columnModel.length > 0 ? columnModel : this.pivotColumns;
      } while (headerCnt > 0);
      this.pivotColumns[0] = {
    ...
    autoFitApplied =
      this.parent.pivotColumns.length > 0 &&
      this.parent.pivotColumns[this.parent.pivotColumns.length - 1].autoFit;
  ...
  public setSavedWidth(column: string, width: number): number {
    if (
      column === '0.formattedText' &&
      !isNullOrUndefined(this.parent.resizedValue)
    ) {
      width = this.parent.resizedValue;
    } else {
      width = this.parent.resizeInfo[column as string]
        ? this.parent.resizeInfo[column as string]
        : width;
    }
    return width;
  }
  ...
  public getFormatList(): { [key: string]: string } {
    ...
    for (
      let vCnt: number = 0;
      vCnt < this.parent.dataSourceSettings.values.length;
      vCnt++
    ) {
      const field: IFieldOptions =
        this.parent.dataSourceSettings.values[vCnt as number];
      let format: string = 'N';
      if (this.parent.dataType === 'olap') {
        if (
          this.parent.olapEngineModule.fieldList[field.name] &&
          !isNullOrUndefined(
            this.parent.olapEngineModule.fieldList[field.name].formatString
          )
        ) {
          let fString: string = this.parent.olapEngineModule.formatFields[
            field.name
          ]
          ...
          format =
            fString.indexOf('#') > -1 || fString.match(/\d/) !== null
              ? fString
              : fString[0] + '2';
        }
      } else {
        ...
        if (this.parent.dataSourceSettings.formatSettings.length > 0) {
          for (
            let fCnt: number = 0;
            fCnt < this.parent.dataSourceSettings.formatSettings.length;
            fCnt++
          ) {
            const formatSettings: IFormatSettings =
              this.parent.dataSourceSettings.formatSettings[fCnt as number];
  ...
  private getValidHeader(
    args:
      ...
      | PdfQueryCellInfoEventArgs,
    axis: string
  ): Date | string | number | boolean | PdfTextWebLink | PdfImage {
    ...
    if (axis === 'row') {
      ...
      if (
        ...
        if (
          ...
          return (
            ...
            ' ' +
            this.parent.localeObj.getConstant(
              this.parent.engineModule.fieldList[cellInfo.value.toString()]
                .aggregateType
            ) +
            ' ' +
            this.parent.localeObj.getConstant('of') +
          ...
          return (
            this.parent.localeObj.getConstant('total') +
            ' ' +
            this.parent.localeObj.getConstant(values[values.length - 1].type) +
            ' ' +
            this.parent.localeObj.getConstant('of') +
            ' ' +
            (!isNullOrUndefined(values[values.length - 1].caption)
      ...
      const cellInfo:
        | ExcelHeaderQueryCellInfoEventArgs
        | PdfHeaderQueryCellInfoEventArgs = args as
        | ExcelHeaderQueryCellInfoEventArgs
        | PdfHeaderQueryCellInfoEventArgs;
      if (
        this.parent.dataSourceSettings.rows.length === 0 ||
        ...
        if (
          ...
          this.parent.localeObj.getConstant('grandTotal') +
            this.parent.dataSourceSettings.valueSortSettings.headerDelimiter +
            (
              (cellInfo.gridCell as Cell<Column>).column.customAttributes
                .cell as IAxisSet
            ).formattedText ===
            (
              (cellInfo.gridCell as Cell<Column>).column.customAttributes
  ...
  private excelColumnEvent(args: ExcelHeaderQueryCellInfoEventArgs): void {
    if (
      ...
      (args.cell as { [key: string]: Object }).value = this.getValidHeader(
        args,
        'column'
      );
    }
    if (
      args.gridCell !== undefined &&
      (args.gridCell as Cell<Column>).column.width === 'auto'
    ) {
      this.parent.lastColumn = (args.gridCell as Cell<Column>).column;
      (args.gridCell as Cell<Column>).column.width = (
        args.gridCell as Cell<Column>
      ).column.minWidth;
    }
    args = this.exportHeaderEvent(
      args,
      'XLSX'
    ) as ExcelHeaderQueryCellInfoEventArgs;
  ...
  private excelRowEvent(args: ExcelQueryCellInfoEventArgs): void {
    ...
    if (args.column.field === '0.formattedText') {
      ...
      this.colPos++;
      pivotValue = (args.data as { [key: string]: Object })[
        args.column.customAttributes.cell
          ? (args.column.customAttributes.cell as IAxisSet).colIndex
          : this.colPos
      ];
      if (
        isNullOrUndefined(pivotValue.value) ||
        isNullOrUndefined(pivotValue.formattedText) ||
        pivotValue.formattedText === ''
      ) {
        args.value = this.parent.exportType === 'Excel' ? null : '';
      } else {
        const aggMatrix: IMatrix2D =
          this.parent.dataType === 'pivot' && this.parent.engineModule
            ? this.parent.engineModule.aggregatedValueMatrix
            : undefined;
        if (
          aggMatrix &&
          aggMatrix[pivotValue.rowIndex] &&
          aggMatrix[pivotValue.rowIndex][pivotValue.colIndex]
        ) {
          args.value = aggMatrix[pivotValue.rowIndex][pivotValue.colIndex];
        } else {
          args.value = !isNullOrUndefined(pivotValue.value)
            ? pivotValue.formattedText === '#DIV/0!'
              ? pivotValue.formattedText
              : pivotValue.value
            : pivotValue.formattedText;
        }
    ...
    if (
      ...
      args.value =
        args.column.field === '0.formattedText'
          ? (this.getValidHeader(args, 'row') as
              | string
              | number
              | boolean
              | Date)
    ...
    if (pivotValue) {
      if (args.style && this.formatList[pivotValue.actualText]) {
        ...
        args.style.numberFormat = args.style.numberFormat
          ? args.style.numberFormat
          : this.formatList[pivotValue.actualText];
      }
  ...
  private exportHeaderEvent(
    args: ExcelHeaderQueryCellInfoEventArgs | PdfHeaderQueryCellInfoEventArgs,
    ...
    if ((args.gridCell as Cell<Column>).column.customAttributes) {
      const cell: IAxisSet = (args.gridCell as Cell<Column>).column
        .customAttributes.cell;
  ...
  public frameGridDataSource(actualData: IGridValues): IGridValues {
    ...
    for (let i: number = 0; i < actualData.length; i++) {
      ...
      for (
        let j: number = 0;
        actualData[i as number] != null && j < keys.length;
        j++
      ) {
        while (keyPos < keys.length) {
          framedDataSource[i as number][Number(keys[keyPos as number])] =
            this.cloneDataWithoutIndex(
              actualData[i as number][Number(keys[keyPos as number])] as Record<
                string,
                unknown
              >
            );
  ...
  public cloneDataWithoutIndex(data: Record<string, unknown>): IAxisSet {
    if (!isNullOrUndefined(data)) {
      ...
      while (keyPos < keys.length) {
        if (
          keys[keyPos as number] !== 'indexObject' &&
          keys[keyPos as number] !== 'index'
        ) {
  ...
  private unWireEvents(cell: HTMLElement): void {
    if (cell.querySelector('.e-hyperlinkcell')) {
      EventHandler.remove(
        cell.querySelector('.e-hyperlinkcell'),
        this.parent.isAdaptive ? 'touchend' : 'click',
        this.onHyperCellClick
      );
    } else {
      return;
    }
