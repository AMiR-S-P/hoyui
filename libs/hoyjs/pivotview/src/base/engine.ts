...
import { isNullOrUndefined, L10n, SanitizeHtmlHelper } from '@hoyui/base';
...
export class PivotEngine {
  ...
  public dateFormatFunction: {
    [key: string]: { exactFormat: Function; fullFormat: Function };
  } = {};
  /** @hidden */
  ...
  private selectedHeaders: AggregateCollection = {
    selectedHeader: [],
    values: [],
  };
  ...
  public renderEngine(
    ...
    if (this.fieldList) {
      for (
        let i: number = 0, j: IFormatSettings[] = dataSource.formatSettings;
        i < j.length;
        i++
      ) {
        if (
          !this.formatFields[j[i as number].name] &&
          this.fieldList[j[i as number].name]
        ) {
    ...
    this.rowStartPos = 0;
    this.colStartPos = 0;
    this.dataSourceSettings.excludeFields = isNullOrUndefined(
    ...
    let fields: IDataSet;
    this.globalize =
      customProperties && customProperties.globalize
        ? customProperties.globalize
        : new Internationalization();
    this.currencyCode =
      customProperties && customProperties.currenyCode
        ? customProperties.currenyCode
        : undefined;
    ...
    this.clonedReport = customProperties
    ...
    this.isPagingOrVirtualizationEnabled =
      this.enablePaging || this.enableVirtualization;
    this.enableSort = dataSource.enableSorting;
    this.dataSourceSettings.alwaysShowValueHeader =
      dataSource.alwaysShowValueHeader;
    this.dataSourceSettings.showHeaderWhenEmpty = isNullOrUndefined(
      dataSource.showHeaderWhenEmpty
    )
      ? true
      : dataSource.showHeaderWhenEmpty;
    this.dataSourceSettings.showSubTotals = isNullOrUndefined(
      dataSource.showSubTotals
    )
      ? true
      : dataSource.showSubTotals;
    this.dataSourceSettings.showRowSubTotals = isNullOrUndefined(
      dataSource.showRowSubTotals
    )
      ? true
      : dataSource.showRowSubTotals;
    this.dataSourceSettings.showColumnSubTotals = isNullOrUndefined(
      dataSource.showColumnSubTotals
    )
      ? true
      : dataSource.showColumnSubTotals;
    this.dataSourceSettings.showGrandTotals = isNullOrUndefined(
      dataSource.showGrandTotals
    )
      ? true
      : dataSource.showGrandTotals;
    this.dataSourceSettings.grandTotalsPosition = isNullOrUndefined(
      dataSource.grandTotalsPosition
    )
      ? 'Bottom'
      : dataSource.grandTotalsPosition;
    this.dataSourceSettings.showRowGrandTotals = isNullOrUndefined(
      dataSource.showRowGrandTotals
    )
      ? true
      : dataSource.showRowGrandTotals;
    this.dataSourceSettings.showColumnGrandTotals = isNullOrUndefined(
    ...
    this.enableValueSorting = customProperties
      ...
      : false;
    this.isDrillThrough = customProperties
      ? customProperties.isDrillThrough
        ? customProperties.isDrillThrough
        : false
      : false;
    this.valueContent = [];
    ...
    if (this.data && (this.data as IDataSet[])[0]) {
      if (!this.fieldList) {
        ...
        for (let i: number = 0; i < this.fields.length; i++) {
          this.fieldKeys[this.fields[i as number]] =
            dataSource.type === 'CSV' ? i : this.fields[i as number];
        }
      }
      if (
        customProperties &&
        customProperties.pageSettings &&
        customProperties.allowDataCompression
      ) {
      ...
      this.dataSourceSettings.columns = dataSource.columns
        ? dataSource.columns
        : [];
      this.dataSourceSettings.filters = dataSource.filters
        ? dataSource.filters
        : [];
      this.dataSourceSettings.values = dataSource.values
        ? dataSource.values
        : [];
      this.dataSourceSettings.formatSettings = dataSource.formatSettings
        ? dataSource.formatSettings
        : [];
      this.dataSourceSettings.groupSettings = dataSource.groupSettings
        ? dataSource.groupSettings
        : [];
      this.dataSourceSettings.calculatedFieldSettings =
        dataSource.calculatedFieldSettings
          ? dataSource.calculatedFieldSettings
          : [];
      this.enableSort =
        dataSource.enableSorting === undefined
          ? true
          : dataSource.enableSorting;
      this.dataSourceSettings.fieldMapping = dataSource.fieldMapping
      ...
      this.isExpandAll =
        this.isValueFiltersAvail && dataSource.allowValueFilter
          ? true
          : dataSource.expandAll;
      this.drilledMembers = dataSource.drilledMembers
        ? this.isValueFiltersAvail && dataSource.allowValueFilter
          ? []
      ...
      this.measureIndex = !isNullOrUndefined(dataSource.valueIndex)
        ? dataSource.valueIndex
        : -1;
      this.emptyCellTextContent = dataSource.emptyCellsTextContent
        ? dataSource.emptyCellsTextContent
        : '';
      this.rowValuesLength =
        this.valueAxis === 1 ? this.dataSourceSettings.values.length : 1;
      this.colValuesLength =
        this.valueAxis === 0 ? this.dataSourceSettings.values.length : 1;
      ...
      this.pageSettings = customProperties
        ? customProperties.pageSettings
          ...
          : this.pageSettings
        : undefined;
      this.allowDataCompression =
        customProperties && customProperties.allowDataCompression;
      ...
      while (columnLength > -1) {
        this.columnKeys[
          this.dataSourceSettings.columns[columnLength as number].name
        ] = this.dataSourceSettings.columns[columnLength as number];
        columnLength--;
      }
    ...
    while (pos < 4) {
      if (report[pos as number]) {
        for (let cnt: number = 0; cnt < report[pos as number].length; cnt++) {
          ...
          if (
            this.dataSourceSettings.excludeFields.indexOf(fieldName) > -1 ||
            (!isNullOrUndefined(fields) && fields.indexOf(fieldName) === -1)
          ) {
            report[pos as number].splice(cnt, 1);
            cnt--;
          } else if (pos === 2) {
            this.measureNames[fieldName as string] = report[pos as number][
              cnt as number
            ].caption
              ? report[pos as number][cnt as number].caption
              : fieldName;
            this.measureNames[
              report[pos as number][cnt as number].caption
                ? report[pos as number][cnt as number].caption
  ...
  private updateDataSourceSettings(
    dataSource: IDataOptions,
    requireDatasourceUpdate: boolean
  ): void {
  ...
  private getGroupedRawData(
    ...
    for (const data of this.actualData as IDataSet[]) {
      this.data[this.data.length] = PivotUtil.frameHeaderWithKeys(data) as
        | IDataSet
        | string[];
    }
    const countFields: string[] = dataSourceSettings.values
      .filter((item: IFieldOptions) => {
        return item.type === 'Count' || item.type === 'DistinctCount';
      })
      .map((item: IFieldOptions) => {
        return item.name;
      });
    const hasCountField: boolean = countFields.length > 0;
    const realData: IDataSet[] | string[][] = this.data;
    const headerFields: string[] = dataSourceSettings.rows
      .concat(dataSourceSettings.columns.concat(dataSourceSettings.filters))
      .map((item: IFieldOptions) => {
        return item.name;
      });
    ...
    for (let i: number = 0; i < realData.length; i++) {
      ...
      if (hasCountField) {
        for (let vPos: number = 0; vPos < countFields.length; vPos++) {
          (currData as IDataSet)[
            this.fieldKeys[countFields[vPos as number]] as string | number
          ] = isNullOrUndefined(
            (currData as IDataSet)[
              this.fieldKeys[countFields[vPos as number]] as string | number
            ]
      ...
      for (let hPos: number = 0; hPos < headerFields.length; hPos++) {
        members.push(
          (currData as string[])[
            this.fieldKeys[headerFields[hPos as number]] as number
          ]
        );
      }
      ...
      if (groupRawData[memberJoin as string]) {
        for (
          let vPos: number = 0;
          vPos < dataSourceSettings.values.length;
          vPos++
        ) {
          const currFieldName: string =
            dataSourceSettings.values[vPos as number].name;
          const currValue: string | number | Date = (currData as IDataSet)[
            this.fieldKeys[currFieldName as string] as string | number
          ];
          const savedData: IDataSet = groupRawData[memberJoin as string];
          let summType: SummaryTypes =
            dataSourceSettings.values[vPos as number].type;
          if (!isNullOrUndefined(currValue)) {
            ...
            if (isNullOrUndefined(savedData[currFieldName as string])) {
              savedData[currFieldName as string] =
                summType === 'Product'
                  ? 1
                  : summType === 'Min' || summType === 'Max'
                  ? undefined
                  : 0;
            ...
            if (summType === 'Count') {
              ...
              if (!isNullOrUndefined(savedData[currFieldName as string])) {
                savedData[currFieldName as string] =
                  savedData[currFieldName as string] > currValue
                    ? currValue
                    : savedData[currFieldName as string];
              }
              ...
              (savedData[currFieldName as string] as number) *=
                currValue as number;
            } else {
              (savedData[currFieldName as string] as number) +=
                currValue as number;
  ...
  private getGroupData(data: IDataSet[]): IDataSet {
    ...
    for (const group of this.dataSourceSettings.groupSettings) {
      ...
      if (this.fields.indexOf(fieldName) > -1) {
        ...
        if (
          (group.type === 'Date' && this.groupingFields[fieldName as string]) ||
          (group.type === 'Custom' && this.groupingFields[fieldName as string])
        ) {
          return fieldkeySet;
        } else if (group.type === 'Number') {
          if (
            PivotUtil.getType(fieldkeySet[fieldName as string] as Date) ===
              'number' ||
            !this.groupingFields[fieldName as string]
          ) {
            if (group.rangeInterval) {
              data.sort((a: IDataSet, b: IDataSet) =>
                Number(
                  a[this.fieldKeys[fieldName as string] as string | number]
                ) >
                Number(
                  b[this.fieldKeys[fieldName as string] as string | number]
                )
                  ? 1
                  : Number(
                      b[this.fieldKeys[fieldName as string] as string | number]
        ...
        while (len--) {
          ...
          if (
            ...
            const date: Date = new Date(
              item[
                this.fieldKeys[fieldName as string] as string | number
              ].toString()
            );
            if (!isNullOrUndefined(date) && group.groupInterval.length > 0) {
              for (
                let i: number = 0, len: number = group.groupInterval.length;
                i < len;
                i++
              ) {
                const interval: DateGroup = group.groupInterval[i as number];
                const isInRangeAvail: boolean = this.getRange(
                  group,
                  date.getTime()
                );
                  ...
                  case 'Years':
                    {
                      ...
                      item[
                        this.fieldKeys[newFieldName as string] as
                          | string
                          | number
                      ] = isInRangeAvail
                        ? undefined
                  ...
                  case 'Quarters':
                    {
                      ...
                      item[
                        ...
                        ? undefined
                        : (this.localeObj
                            ? this.localeObj.getConstant('qtr')
                            : 'Qtr') + month.toString();
                    }
                    break;
                  case 'QuarterYear':
            ...
            for (
              let i: number = 0, len: number = customGroups.length;
              i < len;
              i++
            ) {
              ...
              if (cGroup.items && cGroup.items.length > 1) {
                ...
                const isDataMatch: boolean =
                  PivotUtil.inArray(
                    ...
                    : true;
                item[
                  this.fieldKeys[newFieldName as string] as string | number
                ] = isDataMatch
                  ? cGroup.groupName && cGroup.groupName !== ''
                    ? cGroup.groupName
          ...
          for (const key of keys) {
            isCompleteSet.push(item[key as string] ? true : false);
          }
          fieldkeySet =
            isCompleteSet.indexOf(false) === -1 &&
            keys.length === Object.keys(data[0]).length
              ? item
              : fieldkeySet;
        ...
        if (group.type === 'Date') {
          ...
          const axisFields: IFieldOptions[][] = [
            ...
            if (!isDataSource && axis) {
              ...
              while (i < cnt) {
                if (axis[i as number].name === fieldName) {
                  ...
                  while (gCnt--) {
                    let caption: string = actualField.caption
                      ? actualField.caption
                      : actualField.name;
                    if (this.clonedReport) {
                      let clonedFields: IFieldOptions[] =
                        this.clonedReport.rows;
                      clonedFields = clonedFields.concat(
                        this.clonedReport.columns,
                        this.clonedReport.values,
                        this.clonedReport.filters
                      );
                      const cloneField: IFieldOptions =
                        PivotUtil.getFieldByName(
                          groupKeys[gCnt as number],
                          clonedFields
                        ) as IFieldOptions;
                      if (cloneField) {
                        caption = cloneField.caption
                          ? cloneField.caption
                          : cloneField.name;
                      }
                    }
                    if (
                      ...
                      const newField: IFieldOptions = {
                        ...
                        caption:
                          (this.localeObj
                            ? this.localeObj.getConstant(groupField)
                            : groupField) +
                          ' (' +
                          caption +
                          ')',
                      ...
                      this.groupingFieldsInfo[groupKeys[gCnt as number]] =
                        fieldName;
                    }
                  }
                  break;
                  ...
                  const currentField: IFieldOptions = axis.filter(
                    (axisField: IFieldOptions) => {
                      return axisField.name === fieldName;
                    }
                  )[0];
                  if (!isNullOrUndefined(currentField)) {
                    const currentFieldCaption: string =
                      currentField.caption.indexOf(' (') !== -1 &&
                      currentField.caption.indexOf(')') !== -1
                        ? currentField.caption.slice(
                            currentField.caption.indexOf('(') + 1,
                            currentField.caption.length - 1
                          )
                        : currentField.caption;
                    axis[i as number].caption =
                      (this.localeObj
                        ? this.localeObj.getConstant(axisField)
                        : currentField) +
                      ' (' +
                      currentFieldCaption +
                      ')';
          ...
          const field: IFieldOptions = this.getMappingField(
            fieldName,
            this.clonedReport
              ? this.clonedReport.fieldMapping
          ...
          while (gCnt--) {
            ...
            for (
              let i: number = 0,
                len: number = this.dataSourceSettings.formatSettings.length;
              i < len;
              i++
            ) {
              if (
                this.dataSourceSettings.formatSettings[i as number].name ===
                groupKeys[gCnt as number]
              ) {
            ...
            if (groupField !== 'Quarters' && groupField !== 'QuarterYear') {
              const formatSettings: IFormatSettings = {
                name: groupKeys[gCnt as number],
                type:
                  ['Years', 'Months', 'Days'].indexOf(groupField) > -1
                    ? 'date'
                    : 'time',
                format:
                  groupField === 'Years'
            ...
            if (!isDataSource) {
              ...
              if (
                groupKeys[gCnt as number] !== fieldName &&
                isNullOrUndefined(mappingField.name)
              ) {
                const newField: IFieldOptions = {
                  name: groupKeys[gCnt as number],
                  caption:
          ...
          while (cnt < dataLength) {
            const numericFieldName: number = Number(
              data[cnt as number][fieldName as string]
            );
            unframedSet.push(numericFieldName);
            if (
              !isNaN(numericFieldName) &&
              framedSet.indexOf(numericFieldName) === -1
            ) {
          ...
          while (cnt < dataLength) {
            ...
            if (
              currentData &&
              !isNaN(Number(currentData[fieldName as string]))
            ) {
              endingAt =
                typeof group.endingAt === 'string'
                  ? parseInt(group.endingAt, 10)
                  : (group.endingAt as number);
              cEndValue = endingAt ? endingAt : Math.max(...framedSet);
              const currentStartValue: number = Math.round(
                Number(currentData[fieldName as string])
              );
              const currentEndValue: number = Math.round(
                currentStartValue + (group.rangeInterval - 1)
              );
              if (
                currentStartValue >= groupName[0] &&
                currentStartValue <= groupName[groupName.length - 1]
              ) {
                const startValue: number = groupName[0];
                const endValue: number = groupName[groupName.length - 1];
                currentData[fieldName as string] = this.getNumberGroupHeaders(
                  startValue,
                  endValue,
                  cEndValue
                );
                if (
                  isNullOrUndefined(this.groupedDataType[fieldName as string])
                ) {
                ...
                if (groupName.length === 0) {
                  for (
                    let i: number = currentStartValue;
                    i <= currentEndValue;
                    i++
                  ) {
                    groupName.push(i);
                  ...
                  while (grouping) {
                    if (
                      ...
                      currentStartValue <= endValue
                    ) {
                      currentData[fieldName as string] =
                        this.getNumberGroupHeaders(
                          startValue,
                          endValue,
                          cEndValue
                        );
                      if (
                        isNullOrUndefined(
                          this.groupedDataType[fieldName as string]
                        )
                      ) {
                        this.groupedDataType[fieldName as string] = 'string';
                      }
              ...
              const isCompleteSet: boolean[] = [];
              for (const key of keys) {
                isCompleteSet.push(currentData[key as string] ? true : false);
              }
              fieldkeySet =
                isCompleteSet.indexOf(false) === -1 &&
                keys.length === Object.keys(data[0]).length
          ...
          for (const fields of axisFields) {
            let field: IFieldOptions = PivotUtil.getFieldByName(
              fieldName,
              fields
            ) as IFieldOptions;
            if (field) {
              field = (<{ [key: string]: Object }>field).properties
                ? (<{ [key: string]: Object }>field).properties
                : field;
              field.type = 'Count';
            }
          }
          ...
          const parentField: IFieldOptions = PivotUtil.getFieldByName(
            fieldName.replace(/_custom_group/g, ''),
            dataFields
          ) as IFieldOptions;
          const customGroupField: IFieldOptions = PivotUtil.getFieldByName(
            customFieldName,
            dataFields
          ) as IFieldOptions;
          for (const axis of axisFields) {
            if (!isDataSource && axis) {
              ...
              while (i < cnt) {
                if (
                  axis[i as number].name === group.name &&
                  !customGroupField
                ) {
                  isDataSource = true;
                  const actualField: IFieldOptions = axis[i as number];
                  ...
                  axis[i as number].name === customFieldName &&
                  customGroupField
                ) {
                  const newField: IFieldOptions = {
          ...
          const formatfield: IFormatSettings = PivotUtil.getFieldByName(
            fieldName,
            PivotUtil.cloneFormatSettings(
              this.dataSourceSettings.formatSettings
            )
          ) as IFormatSettings;
          if (formatfield) {
        ...
        this.groupingFields = extend(this.groupingFields, groupFields) as {
          [key: string]: string;
        };
      } else {
        return fieldkeySet;
  ...
  private getNumberGroupHeaders(
    ...
    const fieldName: string =
      startValue === endValue
        ? startValue.toString()
        : cEndValue >= startValue && cEndValue <= endValue
        ? cEndValue === startValue
          ? startValue.toString()
          : startValue.toString() + '-' + cEndValue.toString()
  ...
  private getRange(group: IGroupSettings, cValue: number): boolean {
    ...
    if (group.type === 'Date') {
      ...
      const startDate: Date =
        typeof group.startingAt === 'string'
          ? new Date(group.startingAt)
          : (group.startingAt as Date);
      const endDate: Date =
        typeof group.endingAt === 'string'
          ? new Date(group.endingAt)
          : (group.endingAt as Date);
      if (
        (startDate && cDate.getTime() < startDate.getTime()) ||
      ...
      const startValue: number =
        typeof group.startingAt === 'string'
          ? parseInt(group.startingAt, 10)
          : (group.startingAt as number);
      const endValue: number =
        typeof group.endingAt === 'string'
          ? parseInt(group.endingAt, 10)
          : (group.endingAt as number);
      if (
        (!isNaN(startValue) && cValue < startValue) ||
        (!isNaN(endValue) && cValue > endValue)
      ) {
        isRangeAvail = true;
      } else {
        isRangeAvail = false;
  ...
  private getPercentFormat(
    formatField: { [key: string]: IFormatSettings },
    currentField: string
  ): number {
    const isHavingFormat: RegExpMatchArray =
      !isNullOrUndefined(formatField[currentField as string]) &&
      !isNullOrUndefined(this.formatFields[currentField as string].format)
        ? this.formatFields[currentField as string].format
            .toLowerCase()
            .match(/p[0-9]/)
        : undefined;
    return !isNullOrUndefined(isHavingFormat)
      ? Number(
          this.formatFields[currentField as string].format.replace(
            /[^0-9]/g,
            ''
          )
        )
  ...
  private getFormattedFields(dataSourceSettings: IDataOptions): void {
    this.formatFields = this.setFormattedFields(
  ...
  private getFieldList(
    ...
    isSort: boolean,
    isValueFilteringEnabled: boolean
  ): void {
    let type: string;
    let lenE: number = this.dataSourceSettings.excludeFields.length - 1;
    while (lenE > -1) {
      const index: number = this.fields.indexOf(
        this.dataSourceSettings.excludeFields[lenE as number]
      );
      if (index !== -1) {
    ...
    if (this.savedFieldList) {
      ...
      while (len--) {
        ...
        if (this.fieldList[key as string]) {
          ...
          this.fieldList[key as string].showFilterIcon =
            field && 'showFilterIcon' in field ? field.showFilterIcon : true;
          this.fieldList[key as string].showRemoveIcon =
            field && 'showRemoveIcon' in field ? field.showRemoveIcon : true;
          this.fieldList[key as string].showSortIcon =
            field && 'showSortIcon' in field ? field.showSortIcon : true;
          this.fieldList[key as string].showEditIcon =
            field && 'showEditIcon' in field ? field.showEditIcon : true;
          this.fieldList[key as string].showValueTypeIcon =
            field && 'showValueTypeIcon' in field
              ? field.showValueTypeIcon
              : true;
          this.fieldList[key as string].allowDragAndDrop =
            field && 'allowDragAndDrop' in field
              ? field.allowDragAndDrop
              : true;
          this.fieldList[key as string].isCalculatedField =
            field && 'isCalculatedField' in field
              ? field.isCalculatedField
              : false;
          this.fieldList[key as string].showNoDataItems =
            field && 'showNoDataItems' in field ? field.showNoDataItems : false;
          this.fieldList[key as string].showSubTotals =
            field && 'showSubTotals' in field ? field.showSubTotals : true;
          this.fieldList[key as string].expandAll =
            field && 'expandAll' in field ? field.expandAll : false;
          this.fieldList[key as string].pid =
            field && 'groupName' in field && field.groupName
              ? field.groupName
              : this.groupingFieldsInfo[key as string]
              ? this.groupingFieldsInfo[key as string]
              : undefined;
          ...
          if (!isNullOrUndefined(this.groupedDataType[key as string])) {
            ...
            type =
              ...
              'dataType' in field &&
              field.dataType &&
              dataTypes.indexOf(field.dataType.toLowerCase()) > -1
                ? field.dataType.toLowerCase()
                : type;
          }
          this.fieldList[key as string] = {
            caption:
              field && 'caption' in field && field.caption
                ? field.caption
                : key,
            id: key,
            type:
              key.indexOf('_custom_group') !== -1 ||
              key.indexOf('_date_group') !== -1
                ? 'string'
            ...
            showSortIcon:
              field && 'showSortIcon' in field ? field.showSortIcon : true,
            showNoDataItems:
              field && 'showNoDataItems' in field
                ? field.showNoDataItems
                : false,
            isCalculatedField:
              field && 'isCalculatedField' in field
                ? field.isCalculatedField
            ...
            aggregateType:
              field && 'type' in field
                ...
                : 'Count',
            baseField:
              field && 'baseField' in field ? field.baseField : undefined,
        ...
        this.fieldList[key as string] = {
          ...
          pid:
            field && 'groupName' in field && field.groupName
              ...
              : undefined,
          caption:
            field && 'caption' in field && field.caption ? field.caption : key,
          type:
            key.indexOf('_custom_group') !== -1 ||
            key.indexOf('_date_group') !== -1
              ? 'string'
  ...
  private getMappingField(
    ...
    if (fieldMapping && fieldMapping.length > 0) {
      for (
        let index: number = 0, cnt: number = fieldMapping.length;
        index < cnt;
        index++
      ) {
  ...
  private updateFieldList(savedFieldList: IFieldListOptions): void {
    ...
    while (len--) {
      /** while is used for better performance than for */
      this.fieldList[keys[len as number]].isExcelFilter =
        savedFieldList[keys[len as number]].isExcelFilter;
    }
  }
  private updateTreeViewData(fields: IFieldOptions[]): void {
    ...
    while (cnt--) {
      if (this.fieldList[fields[cnt as number].name]) {
        ...
        field.type = fields[cnt as number].dataType
          ? fields[cnt as number].dataType.toLowerCase()
          : field.type;
        field.caption = fields[cnt as number].caption
          ? fields[cnt as number].caption
          : fields[cnt as number].name;
    ...
    while (lnt--) {
      if (
        this.fieldList[
          this.dataSourceSettings.calculatedFieldSettings[lnt as number].name
        ]
      ) {
        this.fieldList[
          this.dataSourceSettings.calculatedFieldSettings[lnt as number].name
  ...
  private getCalculatedField(keys: string[]): void {
    for (const field of this.dataSourceSettings.calculatedFieldSettings) {
      this.calculatedFields[field.name] = extend(
        {},
        field,
        null,
        true
      ) as ICalculatedFields;
      this.calculatedFields[field.name].actualFormula = field.formula;
    }
    const fieldKeys: string[] = Object.keys(this.calculatedFields);
    for (
      let calc: number = 0, cnt: number = fieldKeys.length;
      calc < cnt;
      calc++
    ) {
      const field: ICalculatedFields =
        this.calculatedFields[fieldKeys[calc as number]];
      const calcProperties: ICalculatedFields = (<{ [key: string]: Object }>(
        field
      )).properties as ICalculatedFields;
      const actualFormula: string = (
        calcProperties ? calcProperties.formula : field.formula
      ).trim();
      const formula: string = actualFormula.replace(/"/g, '');
      field.formula =
        formula.indexOf('^') > -1 ? this.powerFunction(formula) : formula;
      if (
        field.formula.indexOf('Math.min(') === -1 &&
        field.formula.indexOf('min(') > -1
      ) {
        field.formula = field.formula.replace(/min\(/g, 'Math.min(');
      }
      if (
        field.formula.indexOf('Math.max(') === -1 &&
        field.formula.indexOf('max(') > -1
      ) {
        field.formula = field.formula.replace(/max\(/g, 'Math.max(');
      }
      if (
        field.formula.indexOf('Math.abs(') === -1 &&
        field.formula.indexOf('abs(') > -1
      ) {
      ...
      for (
        let len: number = 0, lmt: number = formulaType.length;
        len < lmt;
        len++
      ) {
        ...
        const selectedString: string =
          aggregateValue[0] === 'DistinctCount'
            ? 'DistinctCount'
            : aggregateValue[0] === 'PopulationStDev'
            ? 'PopulationStDev'
            : aggregateValue[0] === 'SampleStDev'
            ? 'SampleStDev'
            : aggregateValue[0] === 'PopulationVar'
            ? 'PopulationVar'
            : aggregateValue[0] === 'SampleVar'
            ? 'SampleVar'
            : aggregateValue[0];
        if (
          [
            ...
            'SampleVar',
            'Median',
          ].indexOf(selectedString) !== -1
        ) {
          const index: number =
            keys.indexOf(aggregateValue[1]) === -1 && matchStrings[2]
              ? keys.indexOf(matchStrings[2])
              : keys.indexOf(aggregateValue[1]);
  ...
  private validateFilters(data: IDataOptions): void {
    ...
    const filterElements: IFilter[] = data.filterSettings
      ? data.filterSettings
      : [];
    let dataFields: IFieldOptions[] = extend(
      [],
  ...
  private generateMembers(kl: number, formulaFields: Set<number>): void {
    ...
    if (
      (fList[key as string].isSelected || formulaFields.has(kl as number)) &&
      !fList[key as string].isMembersFilled
    ) {
      ...
      for (let dl: number = 0; dl < dlen; dl++) {
        const memberkey: string | number | Date = (this.data as IDataSet[])[
          dl as number
        ][this.fieldKeys[key as string] as string];
        let mkey: string = memberkey as string;
        mkey = this.enableHtmlSanitizer
          ? SanitizeHtmlHelper.sanitize(mkey)
          : mkey;
        ...
        if (!isDataAvail) {
          const formattedValue: IAxisSet = isDateType
            ...
            : {
                formattedText:
                  mkey === null
                    ? this.localeObj
                      ? this.localeObj.getConstant('null')
                      : String(mkey)
                    : mkey === undefined
                    ? this.localeObj
                      ? key in this.groupingFields
                        ? this.localeObj.getConstant('groupOutOfRange')
                        : this.localeObj.getConstant('undefined')
                      : String(mkey)
                    : mkey.toString(),
                actualText:
                  mkey === null
          ...
          if (!Object.prototype.hasOwnProperty.call(members, mkey)) {
            ...
            dateMember.push({
              ...
              actualText: isDateType
                ? formattedValue.dateText
                : formattedValue.actualText,
            });
            //sort.push(mkey);
          } else {
        ...
        const memberName: string | number | Date = isNullOrUndefined(memberkey)
          ? memberkey
          : fList[key as string].type === 'number'
          ? !isNaN(Number(memberkey))
            ? Number(memberkey)
            : undefined
          : 1;
    ...
    while (len--) {
      ...
      while (tkln--) {
        ...
        if (field.isMembersFilled) {
          const fieldValue: string | number | Date = (this.data as IDataSet[])[
            len as number
          ][this.fieldKeys[key as string] as string | number];
  ...
  private updateSortSettings(sortSettings: ISort[], isSort: boolean): void {
    for (
      let sln: number = 0, slt: number = sortSettings ? sortSettings.length : 0;
      sln < slt && isSort;
      sln++
    ) {
      if (this.fieldList[sortSettings[sln as number].name]) {
        this.fieldList[sortSettings[sln as number].name].sort =
          sortSettings[sln as number].order;
      }
    }
  }
  private updateFilterMembers(source: IDataOptions): void {
    ...
    if (isInclude) {
      ...
      for (let ln: number = 0; ln < keys.length; ln++) {
        if (
          list.exclude === undefined ||
          list.exclude.indexObject[keys[ln as number]] === undefined
        ) {
      ...
      for (let ln: number = 0; ln < dln; ln++) {
        if (
          list.exclude === undefined ||
          list.exclude.indexObject[ln as number] === undefined
        ) {
          filterRw.push(ln);
        }
      }
  ...
  private getFilters(source: IDataOptions, ilist: IIterator): boolean {
    const filterElements: IFilter[] = source.filterSettings
      ? source.filterSettings
      : [];
    let isInclude: boolean = false;
    let filter: string[] = [];
    for (
      let rln: number = 0, rlt: number = filterElements.length;
      rln < rlt;
      rln++
    ) {
      const filterElement: IFilter = (<{ [key: string]: Object }>(
        filterElements[rln as number]
      )).properties
        ? (<{ [key: string]: Object }>filterElements[rln as number]).properties
        : filterElements[rln as number];
      if (
        this.fieldList[filterElement.name] &&
        this.fieldList[filterElement.name].isSelected &&
        this.isValidFilterField(
          filterElement,
          source.allowMemberFilter,
          source.allowLabelFilter
        )
      ) {
        ...
        if (filterElement.type && filterElement.type === 'Include') {
          this.frameFilterList(
            filter,
            filterElement.name,
            ilist,
            'include',
            filterElement.showLabelFilter,
            isInclude
          );
          isInclude = true;
        } else {
        ...
        if (filterElement.showLabelFilter) {
          filterElement.items = [];
          filterElement.type = filterElement.showDateFilter
            ? 'Date'
            : filterElement.showNumberFilter
            ? 'Number'
            : 'Label';
        }
      }
  ...
  private isValidFilterField(
    ...
    dataFields = dataFields.concat(this.dataSourceSettings.columns);
    if (
      this.fieldList[fieldName as string].isSelected &&
      allowMemberFiltering &&
      filterTypes.indexOf(filterElement.type) >= 0
    ) {
      ...
      for (const field of dataFields) {
        if (
          ...
          (['Label', 'Date', 'Number'] as FilterType[]).indexOf(
            filterElement.type
          ) >= 0
        ) {
          isValidFilterElement = true;
  ...
  private applyLabelFilter(filterElement: IFilter): void {
    if (
      ...
      const members: string[] = Object.keys(
        this.fieldList[filterElement.name].members
      );
      filterElement.showLabelFilter = true;
      if (filterElement.type === 'Label') {
        filterElement.items = this.getLabelFilterMembers(
          members,
          filterElement.condition as LabelOperators,
          filterElement.value1 as string,
          filterElement.value2 as string
        );
      } else if (filterElement.type === 'Date') {
        filterElement.showDateFilter = true;
        const date1: Date =
          typeof filterElement.value1 === 'string'
            ? new Date(filterElement.value1)
            : filterElement.value1;
        const date2: Date =
          typeof filterElement.value2 === 'string'
            ? new Date(filterElement.value2)
            : filterElement.value2;
        filterElement.items = this.getDateFilterMembers(
          members,
          filterElement.name,
          filterElement.condition as DateOperators,
          date1,
          date2
        );
      } else {
        ...
        for (const member of members) {
          const operand1: number = this.getParsedValue(
            filterElement.name,
            filterElement.value1 as string
          );
          const operand2: number = this.getParsedValue(
            filterElement.name,
            filterElement.value2 as string
          );
          const cValue: number = this.getParsedValue(
            filterElement.name,
            member as string
          );
          if (
            this.validateFilterValue(
              cValue,
              filterElement.condition as ValueOperators,
              operand1,
              operand2
            )
          ) {
      ...
      filterElement.type = filterElement.condition
        ? excludeOperators.indexOf(filterElement.condition as LabelOperators) >
            -1 && !filterElement.showNumberFilter
          ? 'Exclude'
          : 'Include'
        : 'Exclude';
    } else {
    ...
    for (const member of members) {
      ...
      if (value1.toString()) {
        switch (operator) {
          ...
          case 'NotBetween':
            if (
              filterValue >= value1.toLowerCase() &&
              filterValue <= value2.toLowerCase()
            ) {
              items.push(member);
            }
            break;
    ...
    for (const member of members) {
      ...
      if (value1) {
        switch (operator) {
          ...
          case 'DoesNotEquals':
            if (
              this.getFormattedValue(filterValue.toString(), name)
                .formattedText ===
              this.getFormattedValue(value1.toString(), name).formattedText
            ) {
          ...
          case 'NotBetween':
            if (
              filterValue.getTime() >= value1.getTime() &&
  ...
  private updateFilter(
    ...
    const members: IMembers = PivotUtil.getFormattedMembers(
      field.members,
      name,
      this
    );
    ...
    while (!isNullOrUndefined(filter[fln as number])) {
      if (members[filter[fln as number]]) {
        ...
        if (type === 'include') {
          for (
            let iln: number = 0, ilt: number = indx.length;
            iln < ilt;
            iln++
          ) {
            if (
              !allowFil ||
              list[type as string].indexObject[indx[iln as number]] !==
                undefined
            ) {
              final[type as string].indexObject[indx[iln as number]] =
                indx[iln as number];
          ...
          this.fieldFilterMem[name as string].memberObj[filter[fln as number]] =
            filter[fln as number];
        }
      }
      fln++;
    }
    if (type === 'include') {
      ...
      for (let iln: number = 0, ilt: number = items.length; iln < ilt; iln++) {
        if (filterObj[items[iln as number]] === undefined) {
          this.fieldFilterMem[name as string].memberObj[items[iln as number]] =
            items[iln as number];
        }
      }
    }
  ...
  private applyValueFiltering(
    rowData: IFieldOptions,
    level: number,
    rows: IAxisSet[],
    columns: IAxisSet,
    valueFilter: IValueFilterSettings,
    rowFilterData: IAxisSet[],
    type: string
  ): IAxisSet[] {
    this.isValueFiltered = false;
    const allMember: IAxisSet = extend(
      {},
      type === 'row' && this.rowGrandTotal
        ? this.rowGrandTotal
        : type === 'column' && this.columnGrandTotal
    ...
    if (this.isValueFiltered) {
      if (
        (type === 'row' && this.rowGrandTotal === null) ||
        (type === 'column' && this.columnGrandTotal === null)
      ) {
  ...
  private getFilteredData(
    rows: IAxisSet[],
    columns: IAxisSet,
    filterSettings: IValueFilterSettings,
    rowFilterData: IAxisSet[],
    level: number,
    fieldName: string,
    allMember: IAxisSet,
      ...
      if (filterSettings[fieldName as string]) {
        if (rows[i as number].level === level) {
          ...
          const aggregate: string =
            this.fieldList[measure as string].aggregateType;
          this.rawIndexObject = {};
          value =
            type === 'row'
              ? this.getAggregateValue(
                  rows[i as number].index,
                  columns.indexObject,
                  mPos,
                  aggregate,
                  false
          ...
          const cellDetails: AggregateEventArgs = {
            ...
            column: columns,
            value: value,
            cellSets: this.getValueCellInfo
              ? this.getCellSet(this.rawIndexObject)
              : [],
            rowCellType:
              rows[i as number].hasChild && rows[i as number].isDrilled
                ? 'subTotal'
                : rows[i as number].type === 'grand sum'
                ? 'grandTotal'
                : 'value',
            columnCellType:
              columns.hasChild && columns.isDrilled
                ? 'subTotal'
                : columns.type === 'grand sum'
                ? 'grandTotal'
                : 'value',
          ...
          const operand1: number = this.getParsedValue(
            measure,
            filterSettings[fieldName as string].value1 as string
          );
          const operand2: number = this.getParsedValue(
            measure,
            filterSettings[fieldName as string].value2 as string
          );
          if (
            !this.validateFilterValue(
              value,
              filterSettings[fieldName as string].condition as ValueOperators,
              operand1,
              operand2
            ) &&
            rows[i as number].type !== 'grand sum'
          ) {
            const data: IAxisSet = this.removefilteredData(
              rows[i as number],
              this.valueFilteredData
            );
            const row: IAxisSet = data ? data : rows[i as number];
            this.validateFilteredParentData(
              row,
              this.valueFilteredData,
              allMember,
              0,
          ...
          rows[i as number].hasChild &&
          rows[i as number].members.length > 0 &&
          rows[i as number].type !== 'grand sum'
        ) {
          rowFilterData.push(extend({}, rows[i as number], null, true));
  ...
  private getParsedValue(measure: string, value: string): number {
    ...
    if (this.formatFields[measure as string] && value) {
      const formatSetting: IFormatSettings = extend(
        {},
        this.formatFields[measure as string],
        null,
        true
      ) as IFormatSettings;
  ...
  private removefilteredData(
    ...
    for (let i: number = 0; i < rows.length; i++) {
      if (
        row.isLevelFiltered &&
        row.axis === rows[i as number].axis &&
        row.valueSort.levelName === rows[i as number].valueSort.levelName &&
        row.actualText === rows[i as number].actualText &&
        row.axis === rows[i as number].axis &&
        row.level === rows[i as number].level &&
        row.ordinal === rows[i as number].ordinal
      ) {
        filteredData = rows[i as number];
        rowFilterData.splice(i, 1);
        break;
      } else if (
        rowFilterData[i as number].hasChild &&
        rowFilterData[i as number].members.length > 0
      ) {
        ...
        if (rowFilteredData.level === i) {
          if (type === 'row') {
            ...
            for (const key of index) {
              ...
              if (
                row.valueSort.levelName
                  .toString()
                  .indexOf(rowFilteredData.valueSort.levelName.toString()) >=
                  0 &&
                rowFilteredData.level !== level &&
                rowFilteredData.index.indexOf(key) >= 0
              ) {
                rowFilteredData.index.splice(
                  rowFilteredData.index.indexOf(key),
                  1
                );
              }
            }
            ...
            for (const key of Object.keys(index)) {
              if (Object.prototype.hasOwnProperty.call(index, key)) {
                delete (<{ [key: string]: Object }>allMemberData.indexObject)[
                  key as string
                ];
                if (
                  row.valueSort.levelName
          ...
          if (
            rowFilteredData &&
            rowFilteredData.members.length > 0 &&
            rowFilteredData.members[0].level === i + 1 &&
            rowFilteredData.members[0].level !== level
          ) {
            this.validateFilteredParentData(
              row,
              rowFilteredData.members,
              allMemberData,
              i + 1,
              level,
              type
  ...
  private updateFramedHeaders(
    framedHeaders: IAxisSet[],
    dataHeaders: IAxisSet[],
    filteredHeaders: IAxisSet[],
    headers: IAxisSet[],
    type: string
  ): IAxisSet[] {
    for (const dHeader of framedHeaders) {
      ...
      if (
        this.validateFilteredHeaders(dHeader, filteredHeaders, type) ||
        dHeader.type === 'grand sum'
      ) {
        if (type === 'row') {
        ...
        if (
          dHeader.hasChild &&
          dHeader.isDrilled &&
          dHeader.members.length > 0
        ) {
          this.updateFramedHeaders(
            dHeader.members,
            dataHeaders,
            filteredHeaders,
            headers[headers.length - 1].members,
  ...
  private validateFilteredHeaders(
    ...
    for (const vHeader of filteredHeaders) {
      if (!this.isHeaderAvail) {
        if (
          dHeader.actualText === vHeader.actualText &&
          ...
          vHeader.hasChild &&
          vHeader.members.length > 0 &&
          vHeader.type !== 'grand sum'
        ) {
  ...
  private isEmptyDataAvail(
    ...
    if (
      rowHeaders.length > 0 &&
      rowHeaders[rowHeaders.length - 1].type === 'grand sum' &&
    ...
    if (
      ...
      columnHeaders[columnHeaders.length - 1].type === 'grand sum' &&
      Object.keys(columnHeaders[columnHeaders.length - 1].indexObject)
        .length === 0
    ) {
      this.isEmptyData = true;
    }
  ...
  public generateGridData(
    dataSource: IDataOptions,
    requireDatasourceUpdate: boolean = false,
    isExport?: boolean,
    headerCollection?: HeaderCollection
  ): void {
    this.updateDataSourceSettings(dataSource, requireDatasourceUpdate);
    const columns: IFieldOptions[] = dataSource.columns
      ? dataSource.columns
      : [];
    ...
    const showNoDataItems: boolean =
      (rows[0] && rows[0].showNoDataItems) ||
      (columns[0] && columns[0].showNoDataItems);
    ...
    for (let ln: number = 0; ln < this.filterMembers.length; ln++) {
      this.filterPosObj[this.filterMembers[ln as number]] =
        this.filterMembers[ln as number];
    }
      ...
      if (!headerCollection) {
        ...
        if (rows.length !== 0 && values.length !== 0) {
          this.rMembers = this.getIndexedHeaders(
            ...
            data,
            0,
            rows[0].showNoDataItems ? filterMembers : this.filterMembers,
            'row',
            '',
        ...
        if (columns.length !== 0 && values.length !== 0) {
          this.cMembers = this.getIndexedHeaders(
            ...
            data,
            0,
            columns[0].showNoDataItems ? filterMembers : this.filterMembers,
            'column',
            '',
            this.allowValueFilter
          );
    ...
    const rowheads: IAxisSet[] = [];
    const colheads: IAxisSet[] = [];
    let rowFilteredData: IAxisSet[] = [];
    let columnFilteredData: IAxisSet[] = [];
    const updatedRowMembers: IAxisSet[] = [];
    const valuesCount: number = this.dataSourceSettings.values.length;
    if (
      this.isValueFiltersAvail &&
      dataSource.allowValueFilter &&
      !headerCollection
    ) {
      this.valueFilteredData = [];
      let rowHeaders: IAxisSet[] = this.saveDataHeaders.rowHeaders
        ? this.saveDataHeaders.rowHeaders
        : [];
      ...
      if (filterSettings.length > 0) {
        ...
        for (const filter of filterSettings) {
          rowHeaders =
            rowFilteredData.length > 0 ? rowFilteredData : rowHeaders;
          columnHeaders =
            columnFilteredData.length > 0 ? columnFilteredData : columnHeaders;
          this.valueFilteredData = [];
          const filterElement: IFilter = (<{ [key: string]: Object }>filter)
            .properties
            ? (<{ [key: string]: Object }>filter).properties
            : filter;
          if (
            filterElement.type === 'Value' &&
            this.fieldList[filter.name] &&
            this.fieldList[filter.name].isSelected
          ) {
            ...
            for (let i: number = 0; i < rLen; i++) {
              if (
                filterElement.name === rows[i as number].name &&
                valueFields[filterElement.measure] &&
                !isAvail
              ) {
                isAvail = true;
                rowFilteredData = this.applyValueFiltering(
                  rows[i as number],
                  i,
                  rowHeaders,
                  this.columnGrandTotal
            ...
            for (let j: number = 0; j < cLen; j++) {
              if (
                filterElement.name === columns[j as number].name &&
                valueFields[filterElement.measure] &&
                !isAvail
              ) {
                isAvail = true;
                columnFilteredData = this.applyValueFiltering(
                  columns[j as number],
                  j,
                  columnHeaders,
                  this.rowGrandTotal
                    ...
                    : this.dataSourceSettings.grandTotalsPosition === 'Top' &&
                      this.dataSourceSettings.showGrandTotals
                    ? rowHeaders[0]
                    : rowHeaders[rowHeaders.length - 1],
                  valueFilters,
                  this.valueFilteredData,
                  'column'
                );
      ...
      const savedFieldList: IFieldListOptions = PivotUtil.getClonedFieldList(
        this.fieldList
      );
      ...
      this.rowCount = 0;
      this.columnCount = 0;
      this.rMembers = this.updateFramedHeaders(
        this.rMembers,
        this.rMembers,
        rowFilteredData,
        this.filterFramedHeaders,
      ...
      this.filterFramedHeaders = [];
      this.cMembers = this.updateFramedHeaders(
        this.cMembers,
        this.cMembers,
        columnFilteredData,
        this.filterFramedHeaders,
    ...
    this.getTableData(
      updatedRowMembers.length > 0 ? updatedRowMembers : this.rMembers,
      rowheads,
      colheads,
      0,
      this.pivotValues,
      ...
      this.rowGrandTotal
        ? this.rowGrandTotal
        : this.rMembers[this.rMembers.length - 1],
      this.columnGrandTotal
        ? this.columnGrandTotal
  ...
  private updateHeaders(rowFlag?: boolean, columnFlag?: boolean): void {
    ...
    rowFlag = isNullOrUndefined(rowFlag)
      ? this.dataSourceSettings.showGrandTotals &&
      ...
      : rowFlag;
    if (
      ((this.pageSettings && this.removeRowGrandTotal) ||
        (!rowFlag && !this.rowGrandTotal)) &&
    ...
    columnFlag = isNullOrUndefined(columnFlag)
      ? this.dataSourceSettings.showGrandTotals &&
      ...
      : columnFlag;
    if (
      ((this.pageSettings && this.removeColumnGrandTotal) ||
        (!columnFlag && !this.columnGrandTotal)) &&
  ...
  private updatePivotValues(updateHeaders?: boolean): void {
    ...
    if (
      ...
      if (
        this.pivotValues[this.pivotValues.length - slicePos] &&
        (this.pivotValues[this.pivotValues.length - slicePos][0] as IAxisSet)
          .type === 'grand sum'
      ) {
        this.pivotValues = (this.pivotValues as []).slice(
          0,
          this.pivotValues.length - slicePos
        );
        this.valueContent = (this.valueContent as []).slice(
          0,
          this.valueContent.length - slicePos
        );
      }
    }
    ...
    if (
      ...
      if (
        this.pivotValues[0][this.pivotValues[0].length - slicePos] &&
        (this.pivotValues[0][this.pivotValues[0].length - slicePos] as IAxisSet)
          .type === 'grand sum'
      ) {
        for (let ln: number = 0; ln < this.pivotValues.length; ln++) {
          if (this.pivotValues[ln as number]) {
            this.pivotValues[ln as number] = (
              this.pivotValues[ln as number] as []
            ).slice(0, this.pivotValues[ln as number].length - slicePos);
          }
          if (this.headerContent[ln as number]) {
            for (
              let pos: number = this.pivotValues[ln as number].length;
              pos < this.pivotValues[ln as number].length + slicePos;
              pos++
            ) {
  ...
  public onDrill(drilledItem: IDrilledItem): void {
    ...
    const headersInfo: IHeadersInfo = this.getHeadersInfo(
      drilledItem.fieldName,
      drilledItem.axis
    );
    this.performDrillOperation(
      headersInfo.headers,
      drilledItem,
      headersInfo.fields,
      headersInfo.position,
      0
    );
    this.headerCollection.rowHeadersCount = this.rowCount;
    this.headerCollection.columnHeadersCount = this.columnCount;
  ...
  public onFilter(filterItem: IFilter, dataSource: IDataOptions): void {
    ...
    this.isLastHeaderHasMeasures =
      (this.valueAxis && headersInfo.axis === 'row') ||
      (!this.valueAxis && headersInfo.axis === 'column')
        ? true
        : this.isLastHeaderHasMeasures;
    if (
      filterItem.type === 'Include' &&
      filterItem.items.length ===
        this.fieldList[filterItem.name].dateMember.length
    ) {
    ...
    this.fieldFilterMem = {};
    this.updateFilterMembers(dataSource);
    let addPos: number[] = this.filterMembers.filter((pos: number) => {
      return posObj[pos as number] === undefined;
    });
    const itemsObj: { [key: string]: string } = {};
    for (const item of filterItem.items) {
      itemsObj[item as string] = item;
    }
    let showNoDataItems: boolean =
      (this.dataSourceSettings.rows[0] &&
        this.dataSourceSettings.rows[0].showNoDataItems) ||
      (this.dataSourceSettings.columns[0] &&
        this.dataSourceSettings.columns[0].showNoDataItems);
   ...
   * @hidden
   */
  public onCalcOperation(
    field: ICalculatedFields,
    dataSourceSettings: IDataOptions
  ): void {
    this.dataSourceSettings.calculatedFieldSettings =
      dataSourceSettings.calculatedFieldSettings
        ? dataSourceSettings.calculatedFieldSettings
        : [];
    this.dataSourceSettings.values = dataSourceSettings.values
    ...
    if (this.fieldList[field.name]) {
      ...
      if (this.valueAxis === 1) {
        this.headerCollection.rowHeadersCount = this.rowCount =
          (this.rowCount / (this.dataSourceSettings.values.length - 1)) *
          this.dataSourceSettings.values.length;
      } else {
        this.headerCollection.columnHeadersCount = this.columnCount =
          (this.columnCount / (this.dataSourceSettings.values.length - 1)) *
          this.dataSourceSettings.values.length;
      }
    }
    ...
    while (count < headers.length) {
      if (position === currentPosition) {
        const levelName: string[] = (
          headers[count as number].valueSort.levelName as string
        ).split(this.valueSortSettings.headerDelimiter);
        if (
          drilledItem.memberName ===
          levelName.join(drilledItem.delimiter ? drilledItem.delimiter : '**')
        ) {
          if (drilledItem.action === 'down') {
            ...
            headers[count as number].members = this.getIndexedHeaders(
              ...
              this.data,
              position + 1,
              headers[count as number].index,
              drilledItem.axis,
              drilledItem.memberName
                .split(drilledItem.delimiter ? drilledItem.delimiter : '**')
                .join(this.valueSortSettings.headerDelimiter)
            );
            let sortedHeaders: ISortedHeaders;
            if (drilledItem.axis === 'row') {
              sortedHeaders = this.applyValueSorting(
  ...
  private performSortOperation(
    ...
    while (count < headers.length) {
      if (headersInfo.position === currentPosition) {
        ...
        headers[count as number].members = this.performSortOperation(
          headers[count as number].members,
          sortItem,
          headersInfo,
          currentPosition + 1
  ...
  private performFilterDeletion(
    ...
    while (count < headers.length && loopIn) {
      if (headersInfo.position === currentPosition) {
        ...
        headers = headers.filter((item: IAxisSet) => {
          return engine.fieldFilterMem[filterItem.name].memberObj[
            item.formattedText
          ] === ''
            ? false
            : !engine.fieldFilterMem[filterItem.name].memberObj[
                item.formattedText
              ] || item.type === 'grand sum';
        });
        loopIn = false;
    ...
    return headers.filter((item: IAxisSet) => {
      return item.members.length > 0 || item.type === 'grand sum'
        ? true
        : engine.matchIndexes(item.indexObject, filterObjects);
    });
  }
  ...
  private performFilterAddition(
    ...
    let count: number = 0;
    while (count < headers.length) {
      const levelName: string = headers[count as number].valueSort
        .levelName as string;
      if (this.headerObjectsCollection[levelName as string]) {
        ...
        for (const member of headers[count as number].members) {
          memberNameObj[member.valueSort.levelName.toString()] =
            member.valueSort.levelName.toString();
        }
        const excessHeaders: IAxisSet[] = this.headerObjectsCollection[
          levelName as string
        ].filter((member: IAxisSet) => {
          return !memberNameObj[member.valueSort.levelName.toString()];
        });
        headers[count as number].members =
          headers[count as number].members.concat(excessHeaders);
        headers[count as number].members = this.getSortedHeaders(
          headers[count as number].members,
          this.fieldList[
            headersInfo.fields[headers[count as number].members[0].level].name
          ].sort
        );
        delete this.headerObjectsCollection[levelName as string];
      }
      if (headers[count as number].members.length > 0) {
        headers[count as number].members = this.performFilterAddition(
          headers[count as number].members,
          fields,
          headersInfo
        );
  ...
  private performFilterCommonUpdate(
    ...
    if (headersInfo.axis === 'row' || headersInfo.axis === 'column') {
      let rawHeaders: IAxisSet[] =
        headersInfo.axis === 'row' ? this.rMembers : this.cMembers;
      ...
      if (this.fieldFilterMem[filterItem.name]) {
        rawHeaders = this.performFilterDeletion(
          headersInfo.headers,
          filterItem,
          headersInfo,
          filterObjects,
      ...
      if (addPos.length > 0 && headersInfo.fields.length > 0) {
        ...
        if (
          headersInfo.fields.filter((item: IFieldOptions) => {
        ...
        this.headerObjectsCollection['parent'] = this.getIndexedHeaders(
          headersInfo.fields,
          this.data,
          0,
          addPos,
        ...
        rawHeaders = this.performFilterAddition(
          rawHeaders,
          headersInfo.fields,
          headersInfo
        );
        ...
        for (const header of rawHeaders) {
          headerNames[header.valueSort.levelName.toString()] =
            header.valueSort.levelName.toString();
        }
        const excessHeaders: IAxisSet[] = this.headerObjectsCollection[
          'parent'
        ].filter((header: IAxisSet) => {
          return !headerNames[header.valueSort.levelName.toString()];
        });
        const grandHeader: IAxisSet[] = rawHeaders.filter((item: IAxisSet) => {
        ...
        rawHeaders = this.getSortedHeaders(
          rawHeaders.concat(excessHeaders),
          this.fieldList[headersInfo.fields[0].name].sort
        ).concat(grandHeader);
        if (headersInfo.axis === 'row') {
          this.cMembers = this.getIndexedHeaders(
            this.dataSourceSettings.columns,
      ...
      this.rMembers = this.getIndexedHeaders(
        ...
        this.data,
        0,
        showNoDataItems ? addPos : this.filterMembers,
        'row',
        ''
      );
      this.cMembers = this.getIndexedHeaders(
  ...
  private getAxisByFieldName(fieldName: string): string {
    ...
    while (axisCount < 4 && axis === '') {
      switch (axisCount) {
        case 0:
          axis = this.getFieldByName(fieldName, this.dataSourceSettings.rows)
            ? 'row'
            : '';
          break;
        case 1:
          axis = this.getFieldByName(fieldName, this.dataSourceSettings.columns)
            ? 'column'
            : '';
          break;
      }
  ...
  private getFieldByName(
    ...
    return new DataManager({ json: fields }).executeLocal(
      new Query().where('name', 'equal', fieldName)
    )[0] as IFieldOptions;
  }
  private updateHeadersCount(
    ...
    while (lenCnt < headers.length) {
      ...
      if (headers[lenCnt as number].members.length > 0) {
        this.updateHeadersCount(
          headers[lenCnt as number].members,
          axis,
          position + 1,
          fields,
          action,
        ...
        if (axis === 'row') {
          this.rowCount +=
            this.showSubTotalsAtBottom &&
            headers[lenCnt as number].hasChild &&
            headers[lenCnt as number].isDrilled
              ? 1
              : 0;
        }
      }
    ...
    if (axis === 'column' && !showSubTotals && isDrill) {
      this.columnCount +=
        action === 'plus' ? -this.colValuesLength : this.colValuesLength;
    }
  }
  /**
  ...
  private getSortedHeaders(headers: IAxisSet[], sortOrder: string): IAxisSet[] {
    const fieldName: string | number | Date =
      headers[0].actualText !== 'Grand Total'
        ? headers[0].valueSort.axis
        : headers[1].valueSort.axis;
    const isNotDateType: boolean = !(
      this.formatFields &&
    ...
    if (isNotDateType) {
      if (
        ...
        headers.length > 0 &&
        typeof headers[0].actualText === 'string'
      ) {
        ...
        for (let i: number = 0; i < headers.length; i++) {
          if (
            isNaN(Number(headers[i as number].actualText.toString().charAt(0)))
          ) {
        ...
        if (alphaNumbervalue.length > 0) {
          alphaNumbervalue = this.sortHeaders(
            fieldName as string,
            childrens,
            headers,
            childrens.sort,
            childrens.isAlphanumeric
          );
        }
        return headers;
      } else {
  ...
  private sortHeaders(
    ...
    if (this.dataSourceSettings.groupSettings.length > 0) {
      const groupField: IGroupSettings[] =
        this.dataSourceSettings.groupSettings.filter(
          (field: IGroupSettings) => {
            return field.name === (childrens.id as string);
          }
        );
      if (!isNullOrUndefined(groupField) && groupField.length > 0) {
        isNumberGroupSorting =
          groupField[0].type.toLocaleLowerCase() === 'number' &&
  ...
  public applyValueSorting(
    ...
    if (rMembers === undefined || cMembers === undefined) {
      ...
      if (
        ...
        this.valueSortSettings.headerText &&
        !this.valueSortHeaderText &&
        this.valueSortSettings.headerText !== '' &&
        this.dataSourceSettings.values.length > 0
      ) {
        this.valueSortHeaderText = this.valueSortSettings.headerText;
        const textArray: string[] = this.valueSortHeaderText.split(
        ...
        for (const field of this.dataSourceSettings.values) {
          ...
          if (valueIndex > -1) {
            ...
            textArray.push(name);
            this.valueSortHeaderText = textArray.join(
              this.valueSortSettings.headerDelimiter
            );
            break;
          }
    ...
    if (this.valueSortHeaderText) {
      ...
      if (
        ((this.dataSourceSettings.values.length === 1 &&
          this.dataSourceSettings.columns.length === 0) ||
          this.dataSourceSettings.values.length > 1) &&
        caption &&
        this.fieldList[caption as string]
      ) {
        for (let i: number = 0; i < textArray.length - 1; i++) {
          hText =
            hText === ''
              ? textArray[i as number]
              : hText +
                this.valueSortSettings.headerDelimiter +
                textArray[i as number];
        }
        ...
        mType =
          this.fieldList[this.dataSourceSettings.values[0].name].aggregateType;
      }
      let member: IAxisSet;
      if (this.valueAxis === 0) {
        member = this.getMember(cMembers, hText);
        if (member) {
          rMembers = this.sortByValueRow(
            rMembers,
            member,
  ...
  private sortByValueRow(
    ...
    for (const header of rMembers) {
      if (header.type === 'grand sum') {
        ...
        this.rawIndexObject = {};
        let value: number = this.getAggregateValue(
          header.index,
          member.indexObject,
          mIndex,
          mType,
        ...
        const cellDetails: AggregateEventArgs = {
          ...
          cellSets: this.getValueCellInfo
            ...
            : [],
          rowCellType:
            header.hasChild && header.isDrilled
              ? 'subTotal'
              : header.type === 'grand sum'
              ? 'grandTotal'
              : 'value',
          columnCellType:
            member.hasChild && member.isDrilled
              ? 'subTotal'
              : member.type === 'grand sum'
              ? 'grandTotal'
              : 'value',
    ...
    for (const header of rMembers) {
      if (header.members.length > 0) {
        header.members = this.sortByValueRow(
          header.members,
          member,
          sortOrder,
          mIndex,
  ...
  private insertAllMembersCommon(): void {
    ...
    if (this.isValueHasAdvancedAggregate) {
      ...
      if (rowFlag) {
        ...
        this.rowGrandTotal = this.insertAllMember(
          [],
          this.filterMembers,
          '',
          'row'
      ...
      if (columnFlag) {
        ...
        this.columnGrandTotal = this.insertAllMember(
          [],
          this.filterMembers,
          '',
          'column'
  ...
  private insertSubTotals(): void {
    ...
    for (let rowCnt: number = 0; rowCnt < rowLength; rowCnt++) {
      const rowCells: IAxisSet[] = this.pivotValues[
        rowCnt as number
      ] as IAxisSet[];
      if (rowCells) {
        ...
        const lastIndex: number = this.showSubTotalsAtTop
          ? this.reformAxisCount
          : colLength;
        let colCnt: number = this.showSubTotalsAtTop
          ? startIndex
          : lastIndex - 1;
        ...
        while (startIndex < lastIndex) {
          ...
          if (cell) {
            ...
            if (savedCell) {
              ...
              savedCell.colIndex = this.showSubTotalsAtTop
                ? savedCell.colIndex + (spanCnt - 1)
                : savedCell.colIndex - (spanCnt - 1);
            }
            if (this.showSubTotalsAtTop) {
              columnSpan = memberCnt = this.getMemberSpanCount(
                this.pivotValues[rowCnt as number] as IAxisSet[],
                colCnt
              );
            }
            ...
            while (
              this.pivotValues[rowPos as number] &&
              !this.pivotValues[rowPos as number][colCnt as number]
            ) {
              const curentCell: IAxisSet = this.pivotValues[rowCnt as number][
                colCnt as number
              ] as IAxisSet;
              if (
                !curentCell.isDrilled &&
                !(
                  !this.valueAxis &&
                  !this.isLastHeaderHasMeasures &&
                  curentCell.members &&
                  curentCell.members.length > 0 &&
              ...
              const cellType: string =
                cell.type === 'sum' || cell.type === 'grand sum'
                  ? cell.type
                  : 'sum';
              this.pivotValues[rowPos as number][colCnt as number] =
              ...
              if (
                cell.valueSort &&
                cell.valueSort[this.valueSortSettings.headerText]
              ) {
              ...
              if (cellType === 'grand sum') {
                ...
                curentCell.type !== 'sum' &&
                (curentCell.isDrilled ||
                  (this.valueAxisFields[curentCell.actualText] &&
                    !this.valueAxis &&
                    !this.isLastHeaderHasMeasures &&
                    curentCell.members &&
                    curentCell.members.length > 0))
                ...
                (
                  this.pivotValues[rowCnt + 1][colCnt as number] as IAxisSet
                ).rowSpan = rowPos - rowCnt;
                isSpanned = true;
              } else {
                (
                  this.pivotValues[rowPos as number][
            ...
            rowCells[colCnt as number] = this.headerContent[rowCnt as number][
              colCnt as number
            ] = extend({}, savedCell, null, true);
            rowCells[colCnt as number].index = this.headerContent[
              rowCnt as number
            ][colCnt as number].index = indexObj.index;
            rowCells[colCnt as number].indexObject = this.headerContent[
              rowCnt as number
            ][colCnt as number].indexObject = indexObj.indexObject;
            spanCnt++;
            if (this.showSubTotalsAtTop) {
              memberCnt--;
              rowCells[colCnt as number].colSpan = memberCnt;
              rowCells[colCnt as number].colIndex =
                rowCells[colCnt as number].colIndex + (spanCnt - 1);
            } else {
          ...
          if (
            this.showSubTotalsAtTop &&
            (rowCells[colCnt + 1] || colCnt + 1 === this.reformAxisCount)
          ) {
            const cell: IAxisSet = this.pivotValues[rowCnt as number][
              colCnt - spanCnt + 1
            ] as IAxisSet;
  ...
  private frameDrillObject(vDrilledItem: IDrilledItem = null): void {
    this.fieldDrillCollection = {};
    for (
      let fieldCnt: number = 0;
      fieldCnt < this.drilledMembers.length;
      fieldCnt++
    ) {
      const drillOption: IDrillOptions =
        this.drilledMembers[fieldCnt as number];
      ...
      for (let i: number = 0; i < this.dataSourceSettings.rows.length; i++) {
        if (
          this.dataSourceSettings.rows[i as number].name === drillOption.name
        ) {
          const hasMeasureIndex: boolean =
            this.valueAxis &&
            (this.isMultiMeasures ||
              this.dataSourceSettings.alwaysShowValueHeader) &&
            this.measureIndex > -1 &&
      ...
      if (!isFieldAvail) {
        for (
          let i: number = 0;
          i < this.dataSourceSettings.columns.length;
          i++
        ) {
          if (
            this.dataSourceSettings.columns[i as number].name ===
            drillOption.name
          ) {
            const hasMeasureIndex: boolean =
              !this.valueAxis &&
          ...
          if (hasValueField) {
            ...
            if (this.isExpandAll || isDrillMemberExpand) {
              for (const field of this.dataSourceSettings.values) {
                ...
                if (
                  drillOption.items.indexOf(
                    members.join(drillOption.delimiter)
                  ) > -1
                ) {
            ...
            if (
              ((this.isExpandAll || isDrillMemberExpand) && isAllValuesAvail) ||
              !this.isExpandAll ||
              !isDrillMemberExpand
            ) {
  ...
  private getIndexedHeaders(
    ...
    if (keys && keys.length > 0) {
      ...
      if (isNullOrUndefined(this.reportDataType)) {
        ...
        for (
          let i: number = 0;
          i < this.dataSourceSettings.values.length;
          i++
        ) {
          this.reportDataType[
      ...
      childrens.type = !isNullOrUndefined(this.reportDataType[childrens.id])
        ? this.reportDataType[childrens.id]
        : childrens.type;
      ...
      const showNoDataItems: boolean =
        (position.length < 1 && keyInd > 0) || field.showNoDataItems;
      const savedMembers: IStringIndex = {};
      if (showNoDataItems) {
        const members: string[] = Object.keys(childrens.members);
        for (
          let pos: number = 0, lt: number = members.length;
          pos < lt;
          pos++
        ) {
          if (
      ...
      if (
        axis === this.dataSourceSettings.valueAxis &&
        this.measureIndex === keyInd &&
        (this.dataSourceSettings.values.length > 1 ||
          this.dataSourceSettings.alwaysShowValueHeader)
      ) {
      ...
      for (let pos: number = 0, lt: number = position.length; pos < lt; pos++) {
        ...
        if (
          !isNullOrUndefined(keys[keyInd as number].showSubTotals) &&
          !keys[keyInd as number].showSubTotals
        ) {
        ...
        member.axis = axis;
        member.colSpan = 1;
        const memInd: number = isNoData
          ? childrens.members[Object.keys(savedMembers)[0]].ordinal
          : this.valueMatrix[position[pos as number]][childrens.index].ordinal;
        let headerValue: string = isNoData
          ? Object.keys(savedMembers)[0]
          : ((data as IDataSet[])[position[pos as number]][
              this.fieldKeys[fieldName as string] as string | number
            ] as string);
        headerValue = this.enableHtmlSanitizer
          ? SanitizeHtmlHelper.sanitize(headerValue)
          : headerValue;
        if (
          (isNullOrUndefined(headerValue) ||
            (this.localeObj &&
        ...
        if (
          showNoDataItems &&
          this.fieldFilterMem[fieldName as string] &&
          this.fieldFilterMem[fieldName as string].memberObj[
          ...
          : {
              formattedText:
                headerValue === null
                  ...
                  ? this.localeObj
                    ? fieldName in this.groupingFields
                      ? this.localeObj.getConstant('groupOutOfRange')
                      : this.localeObj.getConstant('undefined')
                    : String(headerValue)
                  : String(headerValue),
        ...
        const availData: boolean = showNoDataItems
          ? this.filterPosObj[position[pos as number]] !== undefined &&
            !isNoData
            ? true
            : false
          : true;
        ...
        if (
          showNoDataItems &&
          childrens.filter.length > 0 &&
          childrens.filterType === 'include'
        ) {
          isFiltered = true;
        }
        if (
          !(decisionObj && decisionObj[memInd as number]) &&
          (!isFiltered ||
            (isFiltered &&
              childrens.filter.indexOf(headerValue.toString()) > -1))
        ) {
          ...
          if (availData) {
            ...
            decisionObj[memInd as number].indexObject[position[pos as number]] =
              position[pos as number];
            member.indexObject = decisionObj[memInd as number].indexObject;
          }
          ...
          if (keyInd !== 0) {
            ...
            member.valueSort.uniqueName = member.actualText
              ? member.actualText
              : member.formattedText;
          }
          const memberString: string =
            member.valueSort.axis +
            this.valueSortSettings.headerDelimiter +
            member.valueSort.levelName;
          const isExpandMember: boolean =
            this.isExpandAll || (field && field.expandAll);
          member.isDrilled =
            valueFil && this.isValueFiltersAvail
              ? true
              : member.hasChild &&
                this.fieldDrillCollection[memberString as string]
              ? isExpandMember
                ? !this.valueAxis &&
                  !this.isLastHeaderHasMeasures &&
                  keysPos >= this.measureIndex
                  ? true
                  : false
                : true
              : isExpandMember;
      ...
      for (
        let iln: number = 0, ilt: number = hierarchy.length;
        iln < ilt;
        iln++
      ) {
        if (!this.frameHeaderObjectsCollection) {
          if (axis === 'row') {
            this.rowCount +=
              this.rowValuesLength +
              (hierarchy[iln as number].isDrilled &&
              hierarchy[iln as number].hasChild &&
              this.showSubTotalsAtBottom
                ? 1
                : 0);
          } else {
        ...
        if (
          hierarchy[iln as number].valueSort &&
          hierarchy[iln as number].valueSort.levelName
        ) {
          level = hierarchy[iln as number].valueSort.levelName as string;
        }
        parentMember = (level ||
          hierarchy[iln as number].formattedText) as string;
        if (
          !this.dataSourceSettings.showHeaderWhenEmpty &&
          rlen - 1 > keyInd &&
          hierarchy[iln as number].index &&
          ...
          if (
            ...
            for (let len: number = 0; len < hIndLen; len++) {
              const headerValue: string = (data as IDataSet[])[
                hierarchy[iln as number].index[len as number]
              ][
                this.fieldKeys[keys[keyInd + 1].name] as string | number
              ] as string;
              if (
                isNullOrUndefined(headerValue) ||
                (this.localeObj &&
        ...
        if (rlen - 1 > keyInd && hierarchy[iln as number].isDrilled) {
          ...
          if (this.frameHeaderObjectsCollection) {
            this.headerObjectsCollection[parentMember as string] =
              hierarchy[iln as number].members;
          }
        }
      }
      if (this.enableSort) {
        ...
        if (isDateType) {
          ...
          if (
            childrens.type.toLowerCase() === 'number' &&
            hierarchy.length > 0 &&
            typeof hierarchy[0].actualText === 'string' &&
            hierarchy[0].actualText.match(/[a-zA-Z]+/g)
          ) {
            const stringValue: IAxisSet[] = [];
            ...
            for (let i: number = 0; i < hierarchy.length; i++) {
              if (
                isNullOrUndefined(
                  hierarchy[i as number].actualText.toString().match(/\d+/)
                )
              ) {
            ...
            if (outOfRange) {
              if (childrens.sort === 'Ascending') {
                ...
                if (
                  hierarchy[hierarchy.length - 1].actualText === 'Grand Total'
                ) {
  ...
  private insertPosition(
    ...
    axis?: string,
    parentMember?: string,
    slicedHeaders?: IAxisSet[]
  ): IAxisSet[] {
    const hierarchy: IAxisSet[] = [];
    const orderedIndex: { [key: number]: number } =
      this.getOrderedIndex(slicedHeaders);
    if (keys) {
      const decisionObj: IIterator = {};
      ...
      for (let pos: number = 0, lt: number = position.length; pos < lt; pos++) {
        ...
        const memInd: number =
          this.valueMatrix[position[pos as number]][childrens.index].ordinal;
        const slicedHeader: IAxisSet =
          slicedHeaders[orderedIndex[memInd as number]];
        let value: string | number | Date = (data as IDataSet[])[
          position[pos as number]
        ...
        const formattedValue: IAxisSet =
          this.formatFields[field as string] &&
          ['date', 'dateTime', 'time'].indexOf(
            this.formatFields[field as string].type
          ) > -1
        ...
        if (
          !(
            slicedHeader &&
            slicedHeader.formattedText === formattedValue.formattedText
          )
        ) {
          continue;
        }
        if (!(decisionObj && decisionObj[memInd as number])) {
          decisionObj[memInd as number] = { index: [], indexObject: {} };
          slicedHeader.index = decisionObj[memInd as number].index = [
            position[pos as number],
          ];
          decisionObj[memInd as number].indexObject[position[pos as number]] =
            position[pos as number];
          ...
          if (keyInd !== 0) {
            slicedHeader.valueSort.levelName =
              parentMember +
              this.valueSortSettings.headerDelimiter +
            ...
            slicedHeader.valueSort.uniqueName = slicedHeader.actualText
              ? slicedHeader.actualText
              : slicedHeader.formattedText;
          }
          member.members = [];
      ...
      for (
        ...
        if (slicedHeaders[iln as number].members.length > 0) {
          let level: string = null;
          if (
            slicedHeaders[iln as number].valueSort &&
            slicedHeaders[iln as number].valueSort.levelName
          ) {
            level = slicedHeaders[iln as number].valueSort.levelName as string;
          }
          parentMember = (level ||
            slicedHeaders[iln as number].formattedText) as string;
          hierarchy[iln as number].members = this.insertPosition(
            keys,
            data,
            keyInd + 1,
            slicedHeaders[iln as number].index,
            axis,
            parentMember,
  ...
  private insertTotalPosition(headers: IAxisSet[]): IAxisSet[] {
    ...
    if (summCell && summCell.type === 'grand sum') {
      ...
      for (
        let ln: number = 0, lt: number = this.filterMembers.length;
        ln < lt;
        ln++
      ) {
        summCell.indexObject[this.filterMembers[ln as number]] =
          this.filterMembers[ln as number];
      }
    }
    return headers;
  ...
  private calculatePagingValues(): void {
    if (this.isPagingOrVirtualizationEnabled) {
      ...
      this.columnPageCount = Math.ceil(
        this.columnCount / this.pageSettings.columnPageSize
      );
      this.rowPageCount = Math.ceil(
        this.rowCount / this.pageSettings.rowPageSize
      );
      this.pageSettings.currentColumnPage =
        this.pageSettings.currentColumnPage >= this.columnPageCount ||
        (this.enableOptimizedRendering &&
          this.pageSettings.currentColumnPage > 1 &&
          (this.pageSettings.currentColumnPage + 1) * this.colValuesLength >=
            this.columnPageCount)
          ? this.columnPageCount
          : this.pageSettings.currentColumnPage;
      this.pageSettings.currentRowPage =
        this.pageSettings.currentRowPage >= this.rowPageCount ||
        (this.enableOptimizedRendering &&
          this.pageSettings.currentRowPage > 1 &&
          (this.pageSettings.currentRowPage + 1) * this.rowValuesLength >=
            this.rowPageCount)
          ? this.rowPageCount
          : this.pageSettings.currentRowPage;
      const requirePageCount: number =
        this.enablePaging || this.enableOptimizedRendering ? 1 : 3;
      this.memberCnt = this.enablePaging ? 0 : -this.rowValuesLength;
      this.rowStartPos =
        (this.pageSettings.currentRowPage * this.pageSettings.rowPageSize -
          this.pageSettings.rowPageSize) *
          (this.enablePaging ? 1 : this.rowValuesLength) +
        (this.enablePaging ? 1 : 0);
      let exactStartPos: number = this.enablePaging
        ? this.rowStartPos
        : this.rowStartPos +
            this.pageSettings.rowPageSize *
              requirePageCount *
      ...
      this.rowFirstLvl =
        (this.rowStartPos - exactStartPos) % this.pageSettings.rowPageSize;
      ...
      this.endPos =
        ...
        (this.enablePaging ? 1 : 0);
      this.endPos =
        this.endPos > this.rowCount + 1 ? this.rowCount + 1 : this.endPos;
      this.rMembers = this.performSlicing(
        this.rMembers,
        [],
        this.rowStartPos,
        'row'
      ...
      this.colStartPos =
        (this.pageSettings.currentColumnPage *
          ...
          this.pageSettings.columnPageSize) *
          (this.enablePaging ? 1 : this.colValuesLength) +
        (this.enablePaging ? 1 : 0);
      exactStartPos = this.enablePaging
        ? this.colStartPos
        : this.colStartPos +
            this.pageSettings.columnPageSize *
              requirePageCount *
      ...
      this.colFirstLvl =
        (this.colStartPos - exactStartPos) % this.pageSettings.columnPageSize;
      ...
      this.endPos =
        ...
        (this.enablePaging ? 1 : 0);
      this.endPos =
        this.endPos > this.columnCount + 1 ? this.columnCount + 1 : this.endPos;
      this.cMembers = this.performSlicing(
        this.cMembers,
        [],
        this.colStartPos,
        'column'
      ...
      if (this.isValueHasAdvancedAggregate) {
        if (
          this.rMembers[this.rMembers.length - 1].type !== 'grand sum' &&
          this.headerCollection.rowHeaders[
            this.headerCollection.rowHeaders.length - 1
          ].type === 'grand sum'
        ) {
          this.rMembers.push(
            this.headerCollection.rowHeaders[
        ...
        if (
          this.cMembers[this.cMembers.length - 1].type !== 'grand sum' &&
          this.headerCollection.columnHeaders[
            this.headerCollection.columnHeaders.length - 1
          ].type === 'grand sum'
        ) {
          this.cMembers.push(
            this.headerCollection.columnHeaders[
        ...
        this.rowGrandTotal = this.rowGrandTotal
          ? this.rowGrandTotal
          : this.headerCollection.rowHeaders[
              this.headerCollection.rowHeaders.length - 1
            ];
        this.columnGrandTotal = this.columnGrandTotal
          ? this.columnGrandTotal
          : this.headerCollection.columnHeaders[
              this.headerCollection.columnHeaders.length - 1
            ];
  ...
  private performSlicing(
    ...
    while (headers[pos as number]) {
      ...
      this.memberCnt +=
        headers[pos as number].level <= this.measureIndex
          ? axis === 'column'
            ? this.colValuesLength
            : this.rowValuesLength
          : 1;
      if (
        startPos <= this.memberCnt &&
        this.endPos >= this.memberCnt &&
        !this.pageInLimit
      ) {
        if (axis === 'column') {
        ...
        if (this.endPos <= this.memberCnt) {
          if (axis === 'column') {
            if (headers[pos as number].members.length === 0) {
              ...
              this.endPos +=
                headers[pos as number].level * this.colValuesLength;
      ...
      slicedHeaders.push(
        headers[pos as number].members.length > 0
          ? this.removeChildMembers(
              headers[pos as number] as { [key: string]: Object }
            )
          : headers[pos as number]
      );
      if (headers[pos as number].members.length > 0) {
        if (axis === 'column') {
          this.memberCnt -= !(
            ...
            this.dataSourceSettings.showColumnSubTotals &&
            this.columnKeys[
              (headers[pos as number].valueSort as { [key: string]: number })
                .axis
            ].showSubTotals
          )
            ? this.colValuesLength
        ...
        slicedHeaders[slicedHeaders.length - 1].members = this.performSlicing(
          headers[pos as number].members,
          [],
          startPos,
          axis
        );
      ...
      if (!this.isParentLevelAdded && this.enablePaging) {
        this.memberCnt +=
          (slicedHeaders[slicedHeaders.length - 1].level <= this.measureIndex
            ? axis === 'column'
              ? this.colValuesLength
              : this.rowValuesLength
            : 1) * slicedHeaders[slicedHeaders.length - 1].level;
  ...
  private getTableData(
    ...
    rTotal?: IAxisSet,
    cTotal?: IAxisSet,
    valueParentIndex?: number
  ): void {
    for (let rlt: number = rows.length, rln: number = 0; rln < rlt; rln++) {
      const tnum: number =
        !this.valueAxis &&
        !this.isLastHeaderHasMeasures &&
        data.length < this.dataSourceSettings.columns.length + 1
      ...
      if (
        ...
        this.rowIndex =
          isNullOrUndefined(this.rowIndex) &&
          !isLeastNode &&
          this.dataSourceSettings.rows.length === 0
            ? row.index
            : this.rowIndex;
        isRowFieldsAvail =
          ...
          row.valueSort.levelName
            .toString()
            .indexOf('Grand Total' + delimiter) !== 0;
        if (
          this.valueAxis &&
          this.dataSourceSettings.rows.length === 0 &&
          row.valueSort.levelName
      ...
      if (!isRowFieldsAvail) {
        ...
        if (
          this.showSubTotalsAtBottom &&
          reformAxis[pIndex as number] &&
          reformAxis[pIndex as number].hasChild
        ) {
          let axis: IAxisSet;
        ...
        if (
          isValue &&
          !reformAxis[tnum as number].isSum &&
        ...
        data[tnum as number][0] =
          reformAxis[tnum as number] =
          this.valueContent[actCnt as number][0] =
            PivotUtil.getFormattedHeader(row, this);
        if (
          this.valueAxis &&
          (this.isMultiMeasures ||
            this.dataSourceSettings.alwaysShowValueHeader)
          ...
          if (
            (((!(level === 0 && this.measureIndex === 0) && !isLeastNode) ||
              isLeastNode) &&
              row.type === 'value' &&
              !row.valueSort.axis) ||
            (level > this.measureIndex &&
              row.axis === 'row' &&
              row.valueSort.axis)
          ) {
            let vln: number = 0;
            ...
            for (
              ...
              if (!isValueIndexFound) {
                for (vln = 0; vln < vlt; vln++) {
                  if (
                    rowUniqueName.indexOf(
                      this.dataSourceSettings.values[vln as number].name
                    ) > -1
                  ) {
              ...
              if (
                ...
                const parentIndex: number =
                  this.showSubTotalsAtBottom && valueParentIndex
                    ? valueParentIndex
                    : pIndex;
                this.updateRowData(
                  reformAxis,
          ...
          if (!isValueCellUpdated) {
            for (
              ...
              dln = data[tnum as number].length;
              data[hPos as number][dln as number] = this.valueContent[
                actpos as number
              ][dln as number] = {
                axis: 'value',
                actualText: '',
          ...
          for (
            let cln: number = 0, dln: number = 1, clt: number = columns.length;
            cln < clt;
            ++cln
          ) {
            const columnUniqueName: string[] = columns[cln as number].valueSort
              .uniqueName
              ? columns[cln as number].valueSort.uniqueName
                  .toString()
                  .split(this.valueSortSettings.headerDelimiter)
              : [];
  ...
  private insertRowSubTotals(
    ...
    const isValueAxis: boolean = reformAxis[tnum as number].type
      ? reformAxis[tnum as number].type === 'value' &&
        (reformAxis[tnum as number].valueSort.levelName as string) !==
          reformAxis[tnum as number].actualText
      : false;
    if (
      reformAxis[tnum as number].hasChild &&
      reformAxis[tnum as number].members.length > 0
    ) {
      let parentIndexes: number[] = [];
      const subTotal: IAxisSet = PivotUtil.frameHeaderWithKeys(
        reformAxis[tnum as number]
      );
      if (reformAxis[tnum as number].members[0].type === 'value') {
        ...
        while (startIndex < (valueCells as IAxisSet[]).length) {
          const valueCell: IAxisSet = PivotUtil.frameHeaderWithKeys(
            valueCells[startIndex as number]
          );
          let index: number;
          for (i; i < reformAxis.length; i++) {
            if (
              reformAxis[i as number] &&
              reformAxis[i as number].valueSort.levelName ===
                valueCell.valueSort.levelName
            ) {
          ...
          valueCell.formattedText =
            subTotal.formattedText + ' ' + valueCell.formattedText;
        ...
        subTotal.isSum = true;
        subTotal.type = 'sum';
        const parentIndex: number = isValueAxis
          ? this.getParentIndex(reformAxis, subTotal)
          : 0;
        this.getTableData(
          [subTotal],
          reformAxis,
      ...
      for (let index: number = 0; index < parentIndexes.length; index++) {
        let parentInfo: IAxisSet[] = data[
          parentIndexes[index as number]
        ] as IAxisSet[];
        parentInfo = parentInfo.map((dataValue: IAxisSet) => {
          dataValue.formattedText =
            dataValue.colIndex === 0 ? dataValue.formattedText : '';
          return dataValue;
        });
  ...
  private getParentIndex(reformAxis: IAxisSet[], axis: IAxisSet): number {
    ...
    for (let i: number = reformAxis.length - 1; i > 0; i--) {
      if (
        axis.level === reformAxis[i as number].level &&
        reformAxis[i as number].index
      ) {
        parentIndex = i;
        break;
      }
        ...
        case 'PercentageOfDifferenceFrom':
          {
            ...
            if (
              values[vln as number].baseField &&
              values[vln as number].baseItem
            ) {
              ...
              baseItem = Object.keys(
                this.fieldList[columns[0].name].members
              )[0];
            ...
            for (const row of rows) {
              if (row.name === baseField) {
                this.getAggregatedHeaderData(
                  rMembers,
                  values[vln as number].name,
                  baseItem,
                  false,
                  'row',
                  values[vln as number].type,
                  this.selectedHeaders.selectedHeader,
                  vln
                );
        ...
        case 'PercentageOfParentColumnTotal':
          {
            this.selectedHeaders.values.push(values[vln as number].name);
            this.getAggregatedHeaderData(
              values[vln as number].type === 'PercentageOfParentRowTotal'
                ? rMembers
                : cMembers,
              values[vln as number].name,
              undefined,
        ...
        case 'PercentageOfParentTotal':
          {
            ...
            if (!isHeaderSelected) {
              for (
                let len: number = columns.length, i: number = 0;
                i < len;
                i++
              ) {
  ...
  private getAggregatedHeaderData(
    ...
    for (const rln of headers) {
      switch (aggregateType) {
        ...
        case 'PercentageOfDifferenceFrom':
          {
            const levelName: string[] = rln.valueSort.levelName
              .toString()
              .split(this.valueSortSettings.headerDelimiter);
            if (levelName.indexOf(baseItem) !== -1) {
              ...
              selectedHeaders.push(
                this.updateSelectedHeaders(
                  ...
                  name,
                  aggregateType,
                  rln.valueSort.levelName as string,
                  isChildren
                    ? [rln as IAxisSet]
                    : type === 'column'
                    ? headers
              ...
              if (rln.members.length > 0) {
                ...
                if (
                  (!isValuesAvail && members.length > 0) ||
                  type === 'column'
                ) {
                  this.getAggregatedHeaderData(
                    type === 'column' ? rln.members : members,
                    name,
                    baseItem,
                    true,
                    type,
                    aggregateType,
                    selectedHeaders[selectedHeaders.length - 1].childMembers,
                    vln
                  );
                }
              }
        ...
        case 'PercentageOfParentTotal':
          {
            if (rln.type !== 'grand sum') {
              if (
                (rln.valueSort.levelName as string).split(
                  this.valueSortSettings.headerDelimiter
                ).length -
                  1 ===
                level
                ...
                if (rln.members.length > 0) {
                  if (isChildren) {
                    const aggregateHeaders: IAxisSet[] =
                      selectedHeaders[selectedHeaders.length - 1]
                        .aggregateHeaders;
                    for (const member of rln.members) {
                      aggregateHeaders.push(
                        extend({}, member, null, true) as IAxisSet
                      );
                    }
                  } else {
                    const children: IAxisSet[] = extend(
                      [],
                      rln.members,
                      null,
                      true
                    ...
                    const aggregateHeaders: IAxisSet[] =
                      selectedHeaders[selectedHeaders.length - 1]
                        .aggregateHeaders;
                    aggregateHeaders.push(
                      extend({}, rln, null, true) as IAxisSet
                    );
                  }
                  this.getAggregatedHeaderData(
                    rln.members,
  ...
  private applyAdvancedAggregate(
    ...
    if (this.selectedHeaders.values.length > 0) {
      ...
      for (let rlt: number = data.length, rln: number = 0; rln < rlt; rln++) {
        if (
          data[rln as number] !== undefined &&
          data[rln as number][0] !== undefined
        ) {
          if (!isIndexFilled) {
            for (
              let clt: number = (data[rln as number] as IAxisSet[][]).length,
                cln: number = 0;
              cln < clt;
              cln++
            ) {
              const actualText: string = (
                data[rln as number][cln as number] as IAxisSet
              ).actualText as string;
              if (
                (data[rln as number][cln as number] as IAxisSet).axis ===
                  'value' &&
          ...
          const isTotal: boolean =
            this.showSubTotalsAtBottom &&
            (data[rln as number][0] as IAxisSet).isDrilled &&
            (data[rln as number][0] as IAxisSet).hasChild;
          if (!isTotal) {
            if (
              colIndex.length > 0 &&
              (data[rln as number][colIndex[0]] as IAxisSet).axis === 'value' &&
              this.selectedHeaders.values.indexOf(
                (data[rln as number][colIndex[0]] as IAxisSet)
                  .actualText as string
              ) !== -1
            ) {
              rowIndex.push(rln);
      ...
      for (const index of indexCollection) {
        const currentSet: IAxisSet = data[pivotIndex[index as string][0]][
          pivotIndex[index as string][1]
        ] as IAxisSet;
        // currentSet.formattedText = '0';
        currentSet.formattedText =
          this.selectedHeaders.selectedHeader.length > 0
  ...
  private updateAggregates(
    ...
    colIndex: number[],
    rowIndex: number[],
    pivotIndex: { [key: string]: [number, number] }
  ): void {
      ...
      if (
        ...
        if (isRowBaseField) {
          ...
          for (let len: number = data.length, i: number = 0; i < len; i++) {
            if (
              data[i as number] !== undefined &&
              data[i as number][0] === undefined
            ) {
          ...
          while (len--) {
            const axisObj: IAxisSet = (
              columnHeaders[len as number] as IAxisSet[]
            )[colIndex[0]];
          ...
          if (headers.type !== 'RunningTotals') {
            for (
              let clt: number = activeColumn.length, cln: number = 0;
              cln < clt;
              cln++
            ) {
              let isSelectedColumn: boolean = false;
              if (
                activeColumn[cln as number] !== undefined &&
                activeColumn[cln as number].valueSort[uniqueName as string]
              ) {
                activeValues = activeColumn[cln as number];
                for (
                  let len: number = data.length, i: number = 0;
                  i < len;
                  i++
                ) {
                  ...
                  if (
                    axisObj !== undefined &&
                    axisObj[0] !== undefined &&
                    (axisObj[cln as number] as IAxisSet).axis === 'value' &&
                    this.selectedHeaders.values.indexOf(
                      (axisObj[cln as number] as IAxisSet).actualText as string
                    ) !== -1
                  ) {
                    isSelectedColumn = true;
                    selectedColumnValues[i as number] = axisObj[
                      cln as number
                    ] as IAxisSet;
            ...
            if (
              (selectedColumnValues as []).length === 0 &&
              rowindexCollection.length === 0
            ) {
              for (
                let clt: number = activeColumn.length, cln: number = 0;
                cln < clt;
                ...
                if (
                  activeColumn[cln as number] !== undefined &&
                  (
                    activeColumn[cln as number].valueSort.levelName as string
                  ).indexOf(uniqueName) === 0
                ) {
                  activeValues = activeColumn[cln as number];
                  for (
                    let lnt: number = data.length, j: number = 0;
                    j < lnt;
                    j++
                  ) {
                    ...
                    if (
                      ...
                      (axisObj[cln as number] as IAxisSet).axis === 'value' &&
                      this.selectedHeaders.values.indexOf(
                        (axisObj[cln as number] as IAxisSet)
                          .actualText as string
                      ) !== -1
                    ) {
                      isSelectedColumn = true;
        ...
        case 'PercentageOfDifferenceFrom':
          {
            ...
            if (isRowBaseField) {
              if (!isChildren) {
                for (const item of selectedHeaderCollection) {
                  for (const rln of rowIndex) {
                    if (rowheads[rln as number] !== undefined) {
                      if (
                        rowheads[rln as number].valueSort[
                          item.valueSort.levelName as string
                        ] &&
                        rowheads[rln as number].level === activeValues.level &&
                        rowheads[rln as number].type !== 'grand sum'
                      ) {
                        for (const index of colIndex) {
                          const currentSet: IAxisSet = data[rln as number][
                            index as number
                          ] as IAxisSet;
                          if (
                ...
                const uniqueLevelName: string[] = uniqueName.split(
                  this.valueSortSettings.headerDelimiter
                );
                for (const rlen of rowIndex) {
                  if (rowheads[rlen as number] !== undefined) {
                    const levelName: string[] = (
                      rowheads[rlen as number].valueSort.levelName as string
                    ).split(this.valueSortSettings.headerDelimiter);
                    if (
                      levelName.indexOf(
                        uniqueLevelName[uniqueLevelName.length - 1]
                      ) !== -1 &&
                      activeValues &&
                      rowheads[rlen as number].level === activeValues.level
                    ) {
                      for (const index of colIndex) {
                        const currentSet: IAxisSet = data[rlen as number][
                          index as number
                        ] as IAxisSet;
                        if (
                          currentSet.axis === 'value' &&
              ...
              for (const index of indexCollection) {
                const currentSet: IAxisSet = data[index[0]][
                  index[1]
                ] as IAxisSet;
                const actualValue: number = isNullOrUndefined(
                  (selectedRowValues[index[1]] as IAxisSet).actualValue
                )
                  ? 0
                  : (selectedRowValues[index[1]] as IAxisSet).actualValue;
                // let cVal: number = currentSet.value - (selectedRowValues[index[1]] as IAxisSet).value;
                let cVal: number =
                  (isNullOrUndefined(currentSet.actualValue)
                    ? 0
                    : currentSet.actualValue) - actualValue;
                cVal = isNaN(cVal)
                  ? 0
                  : currentSet.value === 0 &&
                    (selectedRowValues[index[1]] as IAxisSet).value === 0
                  ? 0
                  : cVal;
                ...
                if (aggregateType === 'DifferenceFrom') {
                  ...
                  this.aggregatedValueMatrix[index[0]][index[1]] = cVal;
                  currentSet.formattedText = currentSet.showSubTotals
                    ? cVal !== 0
                      ? this.globalize.formatNumber(cVal, {
                          format: 'P',
                          maximumFractionDigits: this.getPercentFormat(
                            this.formatFields,
                            currentSet.actualText as string
                          ),
                        })
                      : this.emptyCellTextContent
                    : currentSet.formattedText;
              ...
              if (!isChildren) {
                for (const item of selectedHeaderCollection) {
                  for (
                    ...
                    if (
                      activeColumn[cln as number] !== undefined &&
                      activeColumn[cln as number].valueSort[
                        item.valueSort.levelName as string
                      ] &&
                      activeValues &&
                      activeColumn[cln as number].level ===
                        activeValues.level &&
                      activeColumn[cln as number].type !== 'grand sum'
                    ) {
                      for (const index of rowindexCollection) {
                        const currentSet: IAxisSet = data[index as number][
                          cln as number
                        ] as IAxisSet;
                        if (
                          currentSet.axis === 'value' &&
                ...
                for (
                  let clt: number = activeColumn.length, clen: number = 0;
                  clen < clt;
                  clen++
                ) {
                  let isSelectedColumn: boolean = false;
                  if (activeColumn[clen as number] !== undefined) {
                    const levelName: string[] = (
                      activeColumn[clen as number].valueSort.levelName as string
                    ).split(this.valueSortSettings.headerDelimiter);
                    if (
                      levelName.indexOf(
                        uniqueLevelName[uniqueLevelName.length - 1]
              ...
              if ((selectedColumnValues as []).length === 0) {
                selectedColumn = this.getSelectedColumn(
                  headers.uniqueName,
                  colheads
                );
              }
              for (const index of indexCollection) {
                ...
                if ((selectedColumnValues as []).length === 0) {
                  const selectedRow: IAxisSet = this.getSelectedRow(
                    currentSet.rowHeaders as string,
                    rowheads
                  );
                  selectedColumnValue = this.getAggregateValue(
                    selectedRow.index,
                    selectedColumn.indexObject,
                    this.fieldList[name as string].index,
                    headers.type,
                    false
                  );
                } else {
                  selectedColumnValue = (
                    selectedColumnValues[index[0]] as IAxisSet
                  ).value;
                }
                ...
                if (aggregateType === 'DifferenceFrom') {
                  ...
                  cVal =
                    (selectedColumnValues[index[0]] as IAxisSet).value === 0
                      ? 0
                      : cVal /
                        (selectedColumnValues[index[0]] as IAxisSet).value;
                  currentSet.formattedText =
                    cVal !== 0
                      ? this.globalize.formatNumber(cVal, {
                          format: 'P',
        ...
        case 'PercentageOfParentTotal':
          {
            if (isRowBaseField) {
              for (const item of selectedHeaderCollection) {
                for (const i of rowIndex) {
                  if (rowheads[i as number] !== undefined) {
                    if (
                      rowheads[i as number].valueSort[
                        item.valueSort.levelName as string
                      ] &&
                      rowheads[i as number].level === item.level
                    ) {
                      for (const index of colIndex) {
                        const currentSet: IAxisSet = data[i as number][
                          index as number
                        ] as IAxisSet;
                        if (
                          currentSet.axis === 'value' &&
              ...
              for (const i of indexCollection) {
                ...
                if (selectedRowValues.length === 0 && activeValues) {
                  selectedRowValue = this.getAggregateValue(
                    activeValues.index,
                    colheads[i[1] - 1].indexObject,
                    this.fieldList[name as string].index,
                    headers.type,
                    false
                  );
                } else {
                  selectedRowValue = selectedRowValues[i[1]]
                    ? (selectedRowValues[i[1]] as IAxisSet).actualValue
                    : 0;
                }
              ...
              for (const item of selectedHeaderCollection) {
                for (
                  let clt: number = activeColumn.length, j: number = 0;
                  j < clt;
                  j++
                ) {
                  let isSelectedColumn: boolean = false;
                  if (
                    activeColumn[j as number] !== undefined &&
                    activeColumn[j as number].valueSort[
                      item.valueSort.levelName as string
                    ]
                  ) {
                    for (const index of rowindexCollection) {
              ...
              if ((selectedColumnValues as []).length === 0) {
                selectedCol = this.getSelectedColumn(
                  headers.uniqueName,
                  colheads
                );
              }
              for (const i of indexCollection) {
                ...
                if ((selectedColumnValues as []).length === 0) {
                  ...
                  selectedColValue = this.getAggregateValue(
                    selectedRow.index,
                    selectedCol.indexObject,
                    this.fieldList[name as string].index,
                    headers.type,
                    false
                  );
                } else {
                  selectedColValue = (selectedColumnValues[i[0]] as IAxisSet)
                    .value;
                }
                ...
                currentSet.formattedText =
                  val !== 0
                    ? this.globalize.formatNumber(val, {
                        format: 'P',
                        maximumFractionDigits: this.getPercentFormat(
                          this.formatFields,
                          currentSet.actualText as string
                ...
                for (const item of selectedHeaderCollection) {
                  for (const rlen of rowIndex) {
                    if (rowheads[rlen as number] !== undefined) {
                      ...
                      if (
                        rowheads[rlen as number] !== undefined &&
                        rowheads[rlen as number].valueSort[
                          item.valueSort.levelName as string
                        ] &&
                        rowheads[rlen as number].level === item.level &&
                        currentSet.axis === 'value' &&
                        currentSet.actualText === name
                      ) {
                        if (rowheads[rlen as number].type !== 'grand sum') {
                          cVal +=
                            !currentSet.showSubTotals &&
                            !(
                              !isNullOrUndefined(currentSet.actualValue) &&
                              isNaN(currentSet.actualValue)
                            )
                              ? currentSet.actualValue
                              : !isNullOrUndefined(currentSet.value) &&
                                !isNaN(currentSet.value)
                              ? currentSet.value
                              : null;
                          currentSet.formattedText = currentSet.showSubTotals
                            ? cVal === 0 &&
                              currentSet.actualValue &&
                              currentSet.actualValue !== 0
                              ? ''
                              : this.getFormattedValue(cVal, name).formattedText
                            : currentSet.formattedText;
                ...
                if (data[rln as number] !== undefined) {
                  ...
                  for (const item of selectedHeaderCollection) {
                    const subTotal: boolean =
                      rowheads[rln as number].hasChild &&
                      rowheads[rln as number].isDrilled &&
                      ((!isNullOrUndefined(
                        rowheads[rln as number].showSubTotals
                      ) &&
                        !rowheads[rln as number].showSubTotals) ||
                        !this.dataSourceSettings.showSubTotals ||
                        !this.dataSourceSettings.showRowSubTotals);
                    for (
                      ...
                      if (
                        ...
                        if (activeColumn[cln as number].type !== 'grand sum') {
                          ...
                          currentSet.formattedText = subTotal
                            ? ''
                            : this.getFormattedValue(cVal, name).formattedText;
  ...
  private getSelectedColumn(name: string, colheads: IAxisSet[]): IAxisSet {
    ...
    for (const head of colheads) {
      if (
        head &&
        head.valueSort &&
        (head.valueSort.levelName as string).indexOf(name) === 0
      ) {
        set.index = [...set.index, ...head.index].sort(function (
          a: number,
          b: number
        ): number {
          return a - b;
  ...
  private recursiveRowData(
    ...
    if (
      this.showSubTotalsAtBottom &&
      (!isNullOrUndefined(rows[rln as number].showSubTotals)
        ? rows[rln as number].showSubTotals
        : true)
    ) {
      this.insertRowSubTotals(
        reformAxis,
        columns,
        tnum,
        data,
  ...
  private updateRowData(
    ...
    const mPos: number =
      this.fieldList[this.dataSourceSettings.values[vln as number].name].index;
    const aggregate: string =
      this.fieldList[this.dataSourceSettings.values[vln as number].name]
        .aggregateType;
    ...
    switch (aggregate) {
      case 'Index':
        {
          gTotalIndex = [
            [rows[rln as number], columns[cln as number]],
            [rows[rln as number], cTotal],
            [rTotal, columns[cln as number]],
            [rTotal, cTotal],
          ];
          ...
          const val: number =
            (totalValues.cVal * totalValues.gTotalVal) /
            (totalValues.rTotalVal * totalValues.cTotalVal);
          value =
            rows[rln as number].members.length > 0 &&
            rows[rln as number].hasChild &&
            rows[rln as number].isDrilled &&
            ((!isNullOrUndefined(rows[rln as number].showSubTotals) &&
              !rows[rln as number].showSubTotals) ||
              !this.dataSourceSettings.showRowSubTotals ||
              !this.dataSourceSettings.showSubTotals)
              ? undefined
              : isNullOrUndefined(totalValues.cVal)
              ? totalValues.cVal
              : isNaN(val)
              ? 0
              : val;
          actualValue = isNullOrUndefined(totalValues.cVal)
            ? totalValues.cVal
            : isNaN(val)
            ? 0
            : val;
      ...
      case 'PercentageOfRowTotal':
        {
          ...
          gTotalIndex.push(
            aggregate === 'PercentageOfGrandTotal'
              ? [rTotal, cTotal]
              : aggregate === 'PercentageOfColumnTotal'
              ? [rTotal, columns[cln as number]]
              : [rows[rln as number], cTotal]
          );
      ...
      default:
        {
          const val: number = this.getAggregateValue(
            rows[rln as number].index,
            columns[cln as number].indexObject,
            mPos,
            aggregate,
            false
          );
    ...
    const cellDetails: AggregateEventArgs = {
      fieldName: this.dataSourceSettings.values[vln as number].name,
      row: rows[rln as number],
      column: columns[cln as number],
      value: value,
      cellSets: this.getValueCellInfo
        ? this.getCellSet(this.rawIndexObject)
        : [],
      rowCellType:
        rows[rln as number].hasChild && rows[rln as number].isDrilled
          ? 'subTotal'
          : rows[rln as number].type === 'grand sum'
          ? 'grandTotal'
          : 'value',
      columnCellType:
        columns[cln as number].hasChild && columns[cln as number].isDrilled
          ? 'subTotal'
          : columns[cln as number].type === 'grand sum'
          ? 'grandTotal'
          : 'value',
      aggregateType: aggregate as SummaryTypes,
      skipFormatting: false,
    ...
    const isSum: boolean =
      rows[rln as number].hasChild ||
      columns[cln as number].hasChild ||
      rows[rln as number].type === 'grand sum' ||
      columns[cln as number].type === 'grand sum' ||
      (this.showSubTotalsAtBottom && rows[rln as number].isSum);
    ...
    let formattedText: string = subTotal
      ...
      : value === undefined
      ? this.emptyCellTextContent
      : aggregate === 'Count' || aggregate === 'DistinctCount'
    ...
    if (
      ...
      isNaN(value) &&
      !isNullOrUndefined(value) &&
      ['PopulationStDev', 'SampleStDev', 'PopulationVar', 'SampleVar'].indexOf(
        aggregate
      ) !== -1
    ) {
      formattedText = '#DIV/0!';
    ...
    //dln = data[tnum as number].length;
    formattedText = cellDetails.skipFormatting
      ? isNullOrUndefined(value)
        ? this.emptyCellTextContent
        : value.toString()
      : formattedText;
    data[tnum as number][dln as number] = this.valueContent[actCnt as number][
      dln as number
    ] = {
      axis: 'value',
      actualText: field,
      ...
      rowHeaders:
        rows[rln as number].type === 'grand sum'
          ? ''
          : rows[rln as number].valueSort.levelName,
      columnHeaders:
        columns[cln as number].type === 'grand sum'
          ? ''
          : columns[cln as number].valueSort.levelName,
  ...
  private updateValueMembers(
    ...
    const levelName: string =
      levelInfo && !isNullOrUndefined(levelInfo.levelName)
        ? levelInfo.levelName.toString()
        : undefined;
    const uniqueName: string =
      levelInfo && !isNullOrUndefined(levelInfo.uniqueName)
        ? levelInfo.uniqueName.toString()
        : undefined;
    if (hasMeasureIndex) {
      for (let vln: number = 0; vln < vcnt; vln++) {
        const field: IFieldOptions =
          this.dataSourceSettings.values[vln as number];
        const name: string = field.caption ? field.caption : field.name;
        const calObj: IAxisSet = {
          axis: this.valueAxis ? 'row' : 'column',
          rowIndex: !this.valueAxis
            ? headerInfo && levelIndex <= headerInfo.rowIndex
          ...
          indexObject:
            !this.valueAxis && headerInfo ? headerInfo.indexObject : null,
        };
        if (axis.length > 0) {
        ...
        vData[
          (levelName
            ? levelName + this.valueSortSettings.headerDelimiter
            : '') + name
        ] = 1;
        vData[
          (uniqueName
            ? uniqueName + this.valueSortSettings.headerDelimiter
            : '') + field.name
        ] = 1;
        vData.levelName =
          (levelName
      ...
      for (let rln: number = 0, rlt: number = axis.length; rln < rlt; rln++) {
        const header: IAxisSet = PivotUtil.frameHeaderWithKeys(
          axis[rln as number]
        );
        header.members = [];
        header.rowIndex = !this.valueAxis
          ? levelIndex < 0
            ? 0
            : levelIndex
          : header.rowIndex;
        ...
        const drillInfo: string =
          hData.axis + this.valueSortSettings.headerDelimiter + hData.levelName;
        const isFieldValueHeader: IField = this.fieldList[hData.axis as string];
        if (
          header.isDrilled &&
          (((this.isExpandAll || isFieldValueHeader.expandAll) &&
            this.fieldDrillCollection[drillInfo as string]) ||
            (!this.isExpandAll &&
              !isFieldValueHeader.expandAll &&
              !this.fieldDrillCollection[drillInfo as string]))
        ) {
        ...
        if (header.isDrilled && axis[rln as number].members.length > 0) {
          if (
            this.valueAxis &&
            levelIndex < this.measureIndex &&
            this.measureIndex !== levelIndex + 1 &&
            !(
              axis[rln as number].hasChild &&
              ((!isNullOrUndefined(axis[rln as number].showSubTotals) &&
                !axis[rln as number].showSubTotals) ||
                !this.dataSourceSettings.showSubTotals ||
                !this.dataSourceSettings.showRowSubTotals)
            )
          ) {
            this.updateValueMembers(
              ...
              header,
              hData,
              columnHeaders[columnHeaders.length - 1].members,
              [],
              vcnt,
              levelIndex
            );
  ...
  private frameDefinedHeaderData(
    ...
    for (let rln: number = 0, rlt: number = axis.length; rln < rlt; rln++) {
      let showSubTotals: boolean = true;
      if (
        axis[rln as number].members.length > 0 &&
        ((!isNullOrUndefined(axis[rln as number].showSubTotals) &&
          !axis[rln as number].showSubTotals) ||
          !this.dataSourceSettings.showSubTotals ||
          !this.dataSourceSettings.showColumnSubTotals)
      ...
      const index: number =
        this.measureIndex < levelIndex && showSubTotals
          ? 1
          : (this.measureIndex > levelIndex &&
              axis[rln as number].members.length > 0) ||
            !(
              this.measureIndex === levelIndex + 1 &&
              axis[rln as number].isDrilled
            )
          ? this.measureIndex === levelIndex &&
            this.measureIndex !== 0 &&
            showSubTotals
            ...
            : this.measureIndex > levelIndex &&
              showSubTotals &&
              axis[rln as number].valueSort &&
              axis[rln as number].valueSort.axis &&
              this.measureNames[axis[rln as number].valueSort.axis.toString()]
            ? 1
            : 0
          : this.measureIndex === 0 || !showSubTotals
          ? 0
          : 1;
      ...
      if (!reformAxis[tnum - 1]) {
        reformAxis[tnum - 1] = PivotUtil.frameHeaderWithKeys(
          axis[rln as number]
        );
      }
      ...
      if (!data[level as number]) {
        ...
        data[level as number][tnum as number] = this.headerContent[
          level as number
        ][tnum as number] = PivotUtil.frameHeaderWithKeys(axis[rln as number]);
      } else {
        data[level as number][tnum as number] = this.headerContent[
          level as number
        ][tnum as number] = PivotUtil.frameHeaderWithKeys(axis[rln as number]);
      ...
      if (
        !this.isPagingOrVirtualizationEnabled &&
        (this.showSubTotalsAtTop ? index !== 0 : true)
      ) {
        if (reformAxis[tnum - 1]) {
  ...
  private getHeaderData(
    ...
    if (!this.valueAxis && !this.isLastHeaderHasMeasures) {
      ...
      if (
        this.dataSourceSettings.showGrandTotals &&
        this.dataSourceSettings.showColumnGrandTotals &&
        axis[axis.length - 1].type === 'grand sum'
      ) {
        this.updateValueMembers(
          this.measureIndex === 0 && axis.length > 1,
          null,
          null,
          columnHeaders,
        ...
        this.updateValueMembers(
          ...
          null,
          columnHeaders,
          axis.slice(axis.length - 1, axis.length),
          vcnt,
          -1
        );
      } else if (
        ...
        this.updateValueMembers(
          ...
          null,
          columnHeaders,
          axis.slice(1, axis.length),
          vcnt,
          0
        );
      } else {
        const hasColumnTotal: boolean =
          columns.length === 0 &&
          axis.length === 1 &&
          axis[0].type === 'grand sum';
        this.updateValueMembers(
          !hasColumnTotal && this.measureIndex === 0,
          null,
          null,
          columnHeaders,
          axis,
      ...
      if (this.selectedHeaders.values.length > 0) {
        for (
          let clt: number = this.selectedHeaders.selectedHeader.length,
            i: number = 0;
          i < clt;
          i++
        ) {
          const headerData: IHeaderData =
            this.selectedHeaders.selectedHeader[i as number];
          if (headerData.axis === 'column') {
            if (headerData.uniqueName === headerData.value) {
              ...
              for (
                let clt: number = headerData.aggregateHeaders.length,
                  j: number = 0;
                j < clt;
                j++
              ) {
                if (
                  headerData.aggregateHeaders[j as number] &&
                  headerData.aggregateHeaders[j as number].members.length > 0
                ) {
                  for (const member of headerData.aggregateHeaders[j as number]
                    .members) {
                    if (member.actualText === headerData.value) {
                      headerData.aggregateHeaders[j as number] = member;
                      if (member.type === 'grand sum') {
                        headerData.uniqueName = member.valueSort
                          .levelName as string;
                      }
                    }
                  }
      ...
      rowAxis.length === 0 &&
      this.valueAxis &&
      (this.isMultiMeasures || this.dataSourceSettings.alwaysShowValueHeader) &&
      this.dataSourceSettings.values.length > 0
    ) {
      if (
        this.dataSourceSettings.showGrandTotals &&
        ...
        this.updateValueMembers(
          this.measureIndex === 0 && this.rMembers.length > 1,
          null,
          null,
          rowAxis,
          this.rMembers.slice(0, this.rMembers.length - 1),
          this.dataSourceSettings.values.length,
          0
        );
        this.updateValueMembers(
          ...
          null,
          rowAxis,
          this.rMembers.slice(this.rMembers.length - 1, this.rMembers.length),
          this.dataSourceSettings.values.length,
          -1
        );
      } else if (
        ...
        this.updateValueMembers(
          ...
          null,
          rowAxis,
          this.rMembers.slice(1, this.rMembers.length),
          this.dataSourceSettings.values.length,
          0
        );
      } else {
        const hasRowTotal: boolean =
          rows.length === 0 &&
          this.rMembers.length === 1 &&
          this.rMembers[0].type === 'grand sum';
        this.updateValueMembers(
          !hasRowTotal && this.measureIndex === 0,
          null,
          null,
          rowAxis,
          this.rMembers,
        ...
        for (
          ...
          if (headerData.axis === 'row') {
            for (
              ...
              if (
                headerData.aggregateHeaders[j as number].actualText ===
                headerData.value
              ) {
                if (
                  (
                    headerData.aggregateHeaders[j as number].valueSort
                      .levelName as string
                  ).indexOf(headerData.uniqueName) !== -1
                ) {
                  headerData.uniqueName = headerData.aggregateHeaders[
                    j as number
                  ...
                  for (
                    let count: number =
                        headerData.aggregateHeaders[j as number].members.length,
                      k: number = 0;
                    k < count;
                    k++
                  ) {
                    const member: IAxisSet =
                      headerData.aggregateHeaders[j as number].members[
                        k as number
                      ];
                    if (
                      member.type !== 'value' &&
                      ...
                      headerData.aggregateHeaders = [].concat(
                        headerData.aggregateHeaders,
                        members,
                        headerData.aggregateHeaders.splice(j)
                      );
                ...
                for (
                  ...
                  if (member.actualText === headerData.value) {
                    ...
                    headerData.aggregateHeaders[j as number].members.splice(
                      k,
                      1
                    );
                    headerData.aggregateHeaders[j as number] = member;
    ...
    const rlt: number = axis.length;
    const colItmLn: number = this.dataSourceSettings.columns.length;
    ...
    for (let rln: number = 0; rln < rlt; rln++) {
      ...
      if (axis[rln as number].members.length) {
        this.getHeaderData(
          ...
          reformAxis,
          data,
          this.showSubTotalsAtTop && !isHidden ? tnum + 1 : tnum,
          vcnt
        );
      }
      ...
      if (
        ...
        if (!(axis[rln as number].members.length > 0)) {
          reformAxis[this.showSubTotalsAtTop ? tnum : reformAxis.length] =
        ...
        reformAxis[tnum as number] = PivotUtil.frameHeaderWithKeys(
          axis[rln as number]
        );
      }
      ...
      data[lvl as number][index as number] = this.headerContent[lvl as number][
        index as number
      ] = PivotUtil.getFormattedHeader(axis[rln as number], this);
      const isSingleMeasure: boolean =
        this.dataSourceSettings.columns.length === 0 &&
        this.dataSourceSettings.values.length === 1
          ? true
          : false;
      if (
        ...
        for (let vln: number = 0; vln < vcnt; vln++) {
          const name: string = this.dataSourceSettings.values[vln as number]
            .caption
            ? this.dataSourceSettings.values[vln as number].caption
            : this.dataSourceSettings.values[vln as number].name;
          const calObj: Object = {
            axis: 'column',
            actualText: this.dataSourceSettings.values[vln as number].name,
            formattedText: name,
            level: 0,
            valueSort: {},
          ...
          if (!data[colItmLn as number]) {
            ...
            data[colItmLn as number][tnum * vcnt + 1 + vln] =
              this.headerContent[colItmLn as number][tnum * vcnt + 1 + vln] =
                calObj;
          } else {
            data[colItmLn as number][tnum * vcnt + 1 + vln] =
              this.headerContent[colItmLn as number][tnum * vcnt + 1 + vln] =
          ...
          const vData: IDataSet = (
            data[colItmLn as number][tnum * vcnt + 1 + vln] as IAxisSet
          ).valueSort;
          vData[
            axis[rln as number].valueSort.levelName +
              this.valueSortSettings.headerDelimiter +
              name
          ] = 1;
          vData.levelName =
            ...
            name;
          vData[
            axis[rln as number].valueSort.uniqueName +
              this.valueSortSettings.headerDelimiter +
              this.dataSourceSettings.values[vln as number].name
          ] = 1;
          vData.uniqueName =
        ...
        axis[rln as number].valueSort &&
        axis[rln as number].valueSort[sortText as string]
      ) {
  ...
  public getAggregateValue(
    ...
    switch (type.toLowerCase()) {
      case 'median':
        {
          ...
          while (rowIndex[ri as number] !== undefined) {
            ...
            if (columnIndex[index as number] !== undefined) {
              ...
              if (
                !isNullOrUndefined(
                  this.valueMatrix[index as number][value as number].member
                )
              ) {
                values.push(
                  this.valueMatrix[index as number][value as number].member
          ...
          if (len > 0) {
            ...
            if (len % 2 === 0) {
              ...
              cellValue =
                (values[position as number] + values[position + 1]) / 2;
            } else {
      ...
      case 'distinctcount':
        {
          ...
          while (rowIndex[ri as number] !== undefined) {
            if (columnIndex[rowIndex[ri as number]] !== undefined) {
              this.rawIndexObject[rowIndex[ri as number]] =
                rowIndex[ri as number];
              isValueExist = true;
              const val: string | number | Date = (this.data as IDataSet[])[
                rowIndex[ri as number]
              ][
              ...
              if (!isNullOrUndefined(val)) {
                ...
                if (
                  ...
                  cellValue +=
                    this.allowDataCompression && typeof val === 'number'
                      ? val
                      : 1;
          ...
          while (rowIndex[ri as number] !== undefined) {
            ...
            if (columnIndex[index as number] !== undefined) {
              ...
              if (!isNullOrUndefined(currentVal)) {
                cellValue =
                  isInit || isNullOrUndefined(cellValue) ? 1 : cellValue;
      ...
      case 'samplevar':
        {
          ...
          if (i > 0) {
            ...
            if (
              ['populationstdev', 'samplestdev'].indexOf(type.toLowerCase()) !==
              -1
            ) {
              cVal = Math.sqrt(
                avgDifferenceVal /
                  (type.toLowerCase() === 'populationstdev' ? i : i - 1)
              );
            } else {
              cVal =
                avgDifferenceVal /
                (type.toLowerCase() === 'populationvar' ? i : i - 1);
            }
      ...
      case 'min':
        {
          ...
          while (rowIndex[ri as number] !== undefined) {
            const index: number = rowIndex[ri as number];
            if (
              columnIndex[index as number] !== undefined &&
              this.valueMatrix[index as number][value as number].member !==
                undefined
            ) {
              isValueExist = true;
              ...
              if (
                ...
                if (isFirst) {
                  ...
                  cellValue =
                    this.valueMatrix[index as number][value as number].member <
                    cellValue
                      ? this.valueMatrix[index as number][value as number]
                          .member
                      : cellValue;
                }
              }
      ...
      case 'calculatedfield':
        {
          isValueExist = true;
          const calcField: ICalculatedFields =
            this.calculatedFields[this.fields[value as number]];
          ...
          if (this.calculatedFormulas[calcField.name]) {
            const calculatedFormulas: Object[] = <Object[]>(
              this.calculatedFormulas[calcField.name]
            );
            for (
              let len: number = 0, lmt: number = calculatedFormulas.length;
              len < lmt;
              len++
            ) {
              const aggregatedValue: { [key: string]: Object } = <
                { [key: string]: Object }
              >calculatedFormulas[len as number];
              let value: number = <number>(
                aggregateField[<string>aggregatedValue.formula]
              );
              if (value === undefined) {
                const type: string = <string>aggregatedValue.type;
                value = this.getAggregateValue(
                  rowIndex,
                  columnIndex,
                  <number>aggregatedValue.index,
                  type,
                  false
                );
              ...
              actualFormula = actualFormula.replace(
                <string>aggregatedValue.formula,
                String(value)
              );
            }
          }
          ...
          cellValue =
            cellValue === Infinity || cellValue === -Infinity
              ? Infinity
              : cellValue === undefined || isNaN(cellValue)
              ? undefined
              : JSON.parse(String(cellValue));
        }
        break;
      default:
        {
  ...
  public getFormattedValue(
    ...
    const commonValue: number | string =
      value === null
        ? this.localeObj
          ? this.localeObj.getConstant('null')
          : String(value)
    ...
    if (
      this.formatFields[fieldName as string] &&
      (this.formatFields[fieldName as string].format ||
        this.formatFields[fieldName as string].skeleton) &&
      !isNullOrUndefined(value)
    ) {
      try {
        ...
        delete formatSetting.name;
        if (
          !formatSetting.minimumSignificantDigits &&
          formatSetting.minimumSignificantDigits < 1
        ) {
          delete formatSetting.minimumSignificantDigits;
        }
        if (
          !formatSetting.maximumSignificantDigits &&
          formatSetting.maximumSignificantDigits < 1
        ) {
          delete formatSetting.maximumSignificantDigits;
        }
        if (formatSetting.type) {
          formattedValue.formattedText = this.dateFormatFunction[
            fieldName as string
          ].exactFormat(new Date(value as string));
          formattedValue.actualText = value;
        } else {
          delete formatSetting.type;
          if (
            formatSetting.format &&
            !this.formatRegex.test(formatSetting.format)
          ) {
          ...
          formattedValue.formattedText = this.globalize.formatNumber(
            !isNaN(Number(value)) ? Number(value) : (value as number),
            formatSetting
          );
          formattedValue.actualText = !isNaN(Number(value))
            ? Number(value)
            : value;
          formattedValue.dateText = !isNaN(Number(value))
            ? Number(value)
            : value;
        }
        if (
          this.fieldList[fieldName as string].sort !== 'None' &&
          formatSetting.type &&
          ['date', 'dateTime', 'time'].indexOf(
            this.formatFields[fieldName as string].type
          ) > -1
        ) {
          formattedValue.dateText = this.dateFormatFunction[
            fieldName as string
          ].fullFormat(new Date(value as string));
        }
        if (this.fieldList[fieldName as string].isCustomField) {
          formattedValue.formattedText =
            formattedValue.formattedText === 'NaN'
              ? commonValue.toString()
              : formattedValue.formattedText;
          formattedValue.dateText =
            formattedValue.dateText === 'NaN'
              ? commonValue.toString()
              : formattedValue.dateText;
        }
      } catch (exception) {
        ...
        if (this.fieldList[fieldName as string].isCustomField) {
          formattedValue.formattedText =
            isNullOrUndefined(formattedValue.formattedText) ||
            formattedValue.formattedText === 'NaN'
              ? commonValue.toString()
              : formattedValue.formattedText;
          formattedValue.dateText =
            isNullOrUndefined(formattedValue.dateText) ||
            formattedValue.dateText === 'NaN'
              ? commonValue.toString()
              : formattedValue.dateText;
        }
      ...
      while (formula.indexOf('~') > -1) {
        formula = formula.replace(
          ...
          (text: string, index: number): string => {
            return items[index as number].replace(
              /(\w*)\^(\w*)/g,
              'Math.pow' + '($1,$2)'
            );
          }
