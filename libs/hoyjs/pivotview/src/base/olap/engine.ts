...
export class OlapEngine {
  ...
  private headerGrouping: {
    [key: number]: {
      UName: { [key: number]: string };
      Caption: { [key: number]: string };
    };
  } = {};
  ...
  public renderEngine(
    ...
    this.enableValueSorting = customProperties
      ? customProperties.enableValueSorting
      : false;
    if (dataSourceSettings.url) {
      // this.isMondrian = (dataSourceSettings.providerType === 'mondrian');
      this.dataSourceSettings = dataSourceSettings;
      this.measureIndex = !isNullOrUndefined(dataSourceSettings.valueIndex)
        ? dataSourceSettings.valueIndex
        : -1;
      this.valueAxis =
        dataSourceSettings.valueAxis === 'row' ? 'row' : 'column';
      this.getAxisFields();
      this.formats = dataSourceSettings.formatSettings
        ? dataSourceSettings.formatSettings
        : [];
      this.enableSort =
        dataSourceSettings.enableSorting === undefined
          ? true
          : dataSourceSettings.enableSorting;
      this.valueSortSettings = dataSourceSettings.valueSortSettings
        ? dataSourceSettings.valueSortSettings
        : undefined;
      this.filterSettings = dataSourceSettings.filterSettings
        ? dataSourceSettings.filterSettings
        : [];
      this.sortSettings = dataSourceSettings.sortSettings
        ? dataSourceSettings.sortSettings
        : [];
      this.allowMemberFilter = dataSourceSettings.allowMemberFilter
        ? true
        : false;
      this.allowLabelFilter = dataSourceSettings.allowLabelFilter
        ? true
        : false;
      this.allowValueFilter = dataSourceSettings.allowValueFilter
        ? true
        : false;
      this.drilledMembers = dataSourceSettings.drilledMembers
        ? this.updateDrilledItems(dataSourceSettings.drilledMembers)
        : [];
      this.calculatedFieldSettings = dataSourceSettings.calculatedFieldSettings
        ? dataSourceSettings.calculatedFieldSettings
        : [];
      this.emptyCellTextContent = dataSourceSettings.emptyCellsTextContent
        ? dataSourceSettings.emptyCellsTextContent
        : '';
      this.pageSettings = customProperties
      ...
      this.isPaging =
        this.pageSettings &&
        (customProperties.enablePaging || customProperties.enableVirtualization)
          ? true
          : false;
      this.frameSortObject();
      this.getFormattedFields(this.formats);
      this.savedFieldList = customProperties
        ? customProperties.savedFieldList
        : undefined;
      this.savedFieldListData = customProperties
        ? customProperties.savedFieldListData
        : undefined;
      this.fieldListData = [];
      this.fieldListObj = {};
      ...
      if (
        !(
          this.savedFieldList &&
          Object.keys(this.savedFieldList).length > 0 &&
          this.savedFieldListData
        )
      ) {
      ...
      this.olapVirtualization =
        this.isPaging &&
        dataSourceSettings.showSubTotals &&
        measuresInfo.measureIndex ===
          (measuresInfo.measureAxis === 'column'
            ? dataSourceSettings.columns.length - 1
            : dataSourceSettings.rows.length - 1) &&
        !isNullOrUndefined(this.pageSettings) &&
        dataSourceSettings.showGrandTotals;
  ...
  public generatePagingData(
    xmlDoc: Document,
    request: Ajax,
    customArgs: FieldData
  ): void {
    const xmlaCellSet: HTMLElement[] = [].slice.call(
      xmlDoc.querySelectorAll('Axes, CellData')
    );
    // this.rowCount =
    ...
    const countCells: NodeListOf<Element> = xmlaCellSet[1]
      ? xmlaCellSet[1].querySelectorAll('FmtValue')
      : null;
  ...
  public scrollPage(): void {
    if (this.olapVirtualization) {
      const virtualScrollingData: VirtualScrollingData =
        this.getVirtualScrollingData(
      ...
      if (virtualScrollingData.isCalculated) {
        ...
        this.performEngine(
          virtualScrollingData.columnTuple,
          virtualScrollingData.rowTuple,
          virtualScrollingData.valueTuple
        );
      }
  ...
  private getVirtualScrollingData(
    ...
    let calColPage: number =
      (this.pageSettings.currentColumnPage - 1) *
      this.pageSettings.columnPageSize;
    let calRowPage: number =
      (this.pageSettings.currentRowPage - 1) * this.pageSettings.rowPageSize;
    const calColSize: number = this.pageSettings.columnPageSize * 3;
    const calRowSize: number = this.pageSettings.rowPageSize * 3;
    calColPage =
      this.columnCount < calColPage + calColSize
        ? this.columnCount > calColSize
          ? this.columnCount - calColSize
          : 0
        : calColPage;
    calRowPage =
      this.rowCount < calRowPage + calRowSize
        ? this.rowCount > calRowSize
          ? this.rowCount - calRowSize
          : 0
        : calRowPage;
    if (
      calColPage !== this.pageColStartPos ||
      calRowPage !== this.pageRowStartPos ||
      !(colTuples.length <= calColSize && rowTuples.length <= calRowSize)
    ) {
      isCalculated = true;
      const measureInfo: IMeasureInfo = this.getMeasureInfo();
      const isColGrandTolExists: boolean =
        !isNullOrUndefined(colTuples[0]) &&
        Number(
          colTuples[0].querySelectorAll('Member')[0].querySelector('LNum')
            .textContent
        ) === 0;
      const isRowGrandTolExists: boolean =
        !isNullOrUndefined(rowTuples[0]) &&
        Number(
          rowTuples[0].querySelectorAll('Member')[0].querySelector('LNum')
            .textContent
        ) === 0;
      const isAddColGrandTotals: boolean = isColGrandTolExists
        ? calColPage + calColSize >= colTuples.length - 1
        : false;
      const isAddRowGrandTotals: boolean = isRowGrandTolExists
        ? calRowPage + calRowSize >= rowTuples.length - 1
        : false;
      const colDepth: number = isColGrandTolExists
        ? this.getAxisdepth(colTuples)
        : measureInfo.measureAxis === 'column'
        ? measureInfo.valueInfo.length
        : 1;
      const rowDepth: number = isRowGrandTolExists
        ? this.getAxisdepth(rowTuples)
        : measureInfo.measureAxis === 'row'
        ? measureInfo.valueInfo.length
        : 1;
      let colTuplesOffset: number =
        (isColGrandTolExists ? colDepth : 0) + calColPage;
      let rowTuplesOffset: number =
        (isRowGrandTolExists ? rowDepth : 0) + calRowPage;
      let virtualColTuples: Element[] = colTuples.slice(
        colTuplesOffset,
        colTuplesOffset + calColSize
      );
      let virtualRowTuples: Element[] = rowTuples.slice(
        rowTuplesOffset,
        rowTuplesOffset + calRowSize
      );
      const colLastLevel: number = virtualColTuples[0]
        ? Number(
            virtualColTuples[0]
              .querySelectorAll('Member')[0]
              .querySelector('LNum').textContent
          )
        : 0;
      ...
      const colData: VirtualTotals = this.getVirtualTotals(
        colTuples.slice(0, colTuplesOffset),
        colLastLevel,
        isAddColGrandTotals,
        'column',
        colDepth
      ...
      const rowData: VirtualTotals = this.getVirtualTotals(
        rowTuples.slice(0, rowTuplesOffset),
        rowLastLevel,
        isAddRowGrandTotals,
        'row',
        rowDepth
      ...
      colTuplesOffset =
        virtualColTuples.length + colData.totalsCollection.length > calColSize
          ? Math.max(
              virtualColTuples.length +
                colData.totalsCollection.length -
                calColSize
            )
          : 0;
      rowTuplesOffset =
        virtualRowTuples.length + rowData.totalsCollection.length > calRowSize
          ? Math.max(
              virtualRowTuples.length +
                rowData.totalsCollection.length -
                calRowSize
            )
          : 0;
      virtualColTuples = colData.totalsCollection.concat(
        virtualColTuples.slice(colTuplesOffset, virtualColTuples.length)
      );
      virtualRowTuples = rowData.totalsCollection.concat(
        virtualRowTuples.slice(rowTuplesOffset, virtualRowTuples.length)
      );
      const virtualValuesTupples: Element[] = this.getVirtualValues(
        valTuples,
        calColPage + colTuplesOffset,
        calRowPage + rowTuplesOffset,
        calColSize,
        calRowSize,
        colData.indexCollection,
        rowData.indexCollection,
        colTuples.length,
        rowTuples.length,
        colDepth,
        rowDepth,
  ...
  private getVirtualTotals(
    tuplesCollection: Element[],
    lastLevel: number,
    isAddGrandTotals: boolean,
    axis: string,
    axisDepth: number
  ): VirtualTotals {
      ...
      for (let i: number = tuplesCollection.length - 1; i > 0; i--) {
        ...
        if (currLevel === 0) {
          ...
          const nextLevel: number = Number(
            tuplesCollection[i - 1]
              .querySelectorAll('Member')[0]
              .querySelector('LNum').textContent
          );
          if (nextLevel === currLevel) {
            for (let offset: number = 0; offset < axisDepth; offset++) {
              totalsCollection[totalsCollection.length] =
                tuplesCollection[i - offset];
  ...
  private getVirtualValues(
    valueTuples: Element[],
    calColumnPage: number,
    calRowPage: number,
    calColunmnSize: number,
    calRowSize: number,
    colTotalsIndex: number[],
    rowTotalsIndex: number[],
    colTuplesLen: number,
    rowTuplesLen: number,
    columnDepth: number,
    rowDepth: number,
    isRowGrandTolExists: boolean
  ): Element[] {
    let framedVirtValTuples: Element[] = [];
    let virtValTuples: Element[] = valueTuples;
    if (rowTuplesLen > calRowSize) {
      const rowValuesOffset: number =
        ((isRowGrandTolExists ? rowDepth : 0) + calRowPage) * colTuplesLen;
      virtValTuples = valueTuples.slice(
        rowValuesOffset,
        rowValuesOffset + calRowSize * colTuplesLen
      );
      let virtRowTotalValues: Element[] = [];
      for (let i: number = 0; i < rowTotalsIndex.length; i++) {
        virtRowTotalValues = virtRowTotalValues.concat(
          valueTuples.slice(
            rowTotalsIndex[i as number] * colTuplesLen,
            rowTotalsIndex[i as number] * colTuplesLen + colTuplesLen
          )
        );
      }
    ...
    for (
      let i: number = 0, j: number = virtValTuples.length / colTuplesLen;
      i < j;
      i++
    ) {
      const rows: Element[] = virtValTuples.slice(
        i * colTuplesLen,
        i * colTuplesLen + colTuplesLen
      );
      const virtRows: Element[] = rows.slice(
        calColumnPage + columnDepth,
        calColumnPage + columnDepth + calColunmnSize
      );
      ...
      framedVirtValTuples = framedVirtValTuples.concat(
        virtTotals.concat(virtRows)
      );
    }
  ...
  public generateEngine(
    ...
    this.xmlaCellSet = xmlDoc
      ? xmlDoc.querySelectorAll('Axes, CellData')
      : undefined;
    const columnTuples: Element[] =
      this.xmlaCellSet && this.xmlaCellSet.length > 0
        ? [].slice.call(
            this.xmlaCellSet[0].querySelectorAll('Axis[name|="Axis0"] Tuple')
          )
        : [];
    const rowTuples: Element[] =
      this.xmlaCellSet && this.xmlaCellSet.length > 0
        ? [].slice.call(
            this.xmlaCellSet[0].querySelectorAll('Axis[name|="Axis1"] Tuple')
          )
        : [];
    let valCollection: Element[] =
      this.xmlaCellSet && this.xmlaCellSet.length > 1
    ...
    if (this.olapVirtualization && !isNullOrUndefined(this.pageSettings)) {
      if (columnTuples.length * rowTuples.length !== valCollection.length) {
        ...
        for (let colPos: number = 0; colPos < valCollection.length; colPos++) {
          if (!isNullOrUndefined(valCollection[colPos as number])) {
            valueCollection[
              Number(
                valCollection[colPos as number].getAttribute('CellOrdinal')
              )
      ...
      let columnData: { tupColls: Element[]; indexColls: number[] } =
        this.getActualTuples(columnTuples, drillInfo);
      drillInfo = this.getDrillInfo('rows');
      let rowData: { tupColls: Element[]; indexColls: number[] } =
        this.getActualTuples(
          rowTuples,
          drillInfo,
          columnData.indexColls,
          columnTuples.length
        );
  ...
  private getDrillInfo(axis: string): { [key: string]: string[] } {
    ...
    if (axis === 'columns') {
      fieldColls = this.dataSourceSettings.columns.map(
        (field: IFieldOptions) => field.name
      );
    } else if (axis === 'rows') {
      fieldColls = this.dataSourceSettings.rows.map(
        (field: IFieldOptions) => field.name
      );
    }
    if (fieldColls.length > 0) {
      for (
        let i: number = 0, j: number = this.drilledMembers.length;
        i < j;
        i++
      ) {
        ...
        if (index > -1) {
          ...
          drilledMembers[index as number] = drilledMembers[
            index as number
          ].concat(drilledMember.items);
        }
      }
    }
  ...
  private getActualTuples(
    ...
    if (tuplesColl.length > 0) {
      for (let i: number = 0, j: number = tuplesColl.length; i < j; i++) {
        ...
        const memTypeColl: HTMLCollectionOf<Element> =
          tuples.getElementsByTagName('MEMBER_TYPE');
        const uNameColl: HTMLCollectionOf<Element> =
          tuples.getElementsByTagName('UName');
        for (let k: number = 0, l: number = memTypeColl.length; k < l; k++) {
          ...
          if (
            drillInfo[k as number] &&
            drillInfo[k as number].indexOf(uName) > -1
          ) {
            isSum = true;
          }
          if (!isAddElement) {
        ...
        if (isAddElement) {
          ...
          if (indexCollection) {
            const rowColls: Element[] = this.clonedValTuple.slice(
              i * deapth,
              i * deapth + deapth
            );
            valueCollection = valueCollection.concat(
              indexCollection.map((index: number) => rowColls[index as number])
            );
          } else {
  ...
  private performEngine(
    columnTuples: Element[],
    rowTuples: Element[],
    valCollection: Element[]
  ): void {
    ...
    if (this.drilledMembers.length > 0) {
      ...
      orderedInfo = this.frameMeasureOrder(
        ...
        valCollection,
        columnTuples.length,
        columnTuples.length * rowTuples.length
      );
    ...
    if (this.customArgs.action === 'down') {
      this.updateTupCollection(
        this.customArgs.drillInfo.axis === 'row'
          ? rowTuples.length
          : columnTuples.length
      );
    }
    ...
    if (
      this.enableValueSorting &&
      this.valueSortSettings &&
      !this.isPaging &&
      !this.olapVirtualization
    ) {
      valueSortData = this.getValueSortInfo(
        columnTuples,
        rowTuples,
        measureInfo
      );
    ...
    if (
      this.customArgs.action === 'down'
        ? this.customArgs.drillInfo.axis === 'column'
        : true
    ) {
    ...
    if (!this.isPaging && !this.olapVirtualization && this.enableSort) {
      for (let i: number = 0; i < this.headerContent.length; i++) {
        this.headerContent[i as number] = this.pivotValues[
          i as number
        ] as IAxisSet[];
      }
    }
  ...
  private getValueSortInfo(
    columnTuples: Element[],
    rowTuples: Element[],
    measureInfo: IMeasureInfo
  ): ValueSortInfo {
    let memberIndex: number;
    if (this.valueSortSettings.headerText) {
      let headersCollection: string[] = this.valueSortSettings.headerText.split(
        this.valueSortSettings.headerDelimiter
      );
      const granSumPos: number = headersCollection.indexOf('Grand Total');
      if (granSumPos > -1) {
        const measure: string = headersCollection
          .join('')
          .split(headersCollection[granSumPos as number])
          .join('');
        headersCollection = [];
      ...
      for (let i: number = 0; i < this.dataSourceSettings.values.length; i++) {
        measureIndex = headersCollection.indexOf(
          this.dataSourceSettings.values[i as number].caption
        );
        if (
          this.dataSourceSettings.values[i as number].caption ===
          headersCollection[measureIndex as number]
        ) {
          headersCollection[measureIndex as number] =
            this.fieldList[
              this.dataSourceSettings.values[i as number].name
            ].name;
      ...
      if (measureIndex !== -1) {
        if (measureIndex < measureInfo.measureIndex) {
          headersCollection = this.reArrangeHeaders(
            headersCollection,
            measureInfo.measureIndex,
            measureAxis
          );
        }
        const membersColls: Element[] =
          this.olapValueAxis === 'column' ? columnTuples : rowTuples;
        if (membersColls.length > 0) {
          const level: number =
            membersColls[0].getElementsByTagName('Member').length -
            (granSumPos > -1 ? measureIndex : 0);
  ...
  private getParentElement(
    ...
    const index: number =
      membersColls[0].getElementsByTagName('Member').length - level;
    for (let i: number = 0; i < membersColls.length; i++) {
      if (
        isNullOrUndefined(
          membersColls[i as number].getElementsByTagName('Member')[
            index as number
          ]
        )
      ) {
      ...
      const memberUName: string =
        membersColls[i as number].getElementsByTagName('UName')[index as number]
          .textContent;
      const memberCaption: string =
        membersColls[i as number].getElementsByTagName('Caption')[
          index as number
        ].textContent;
      const memberLevel: number = Number(
        membersColls[i as number].getElementsByTagName('LNum')[index as number]
          .textContent
      );
      const isParent: boolean =
        memberUName === headersCollection[index as number] ||
        memberCaption === headersCollection[index as number] ||
        (isNullOrUndefined(headersCollection[index as number]) &&
          memberLevel === 0);
      if (isParent) {
        ...
        for (let j: number = i; j < membersColls.length; j++) {
          const childUName: string =
            membersColls[j as number].getElementsByTagName('UName')[
              index as number
            ].textContent;
          const childCaption: string =
            membersColls[j as number].getElementsByTagName('Caption')[
              index as number
            ].textContent;
          const childLevel: number = Number(
            membersColls[j as number].getElementsByTagName('LNum')[
              index as number
            ].textContent
          );
          const isChild: boolean =
            childUName === headersCollection[index + 1] ||
            childCaption === headersCollection[index + 1] ||
            (isNullOrUndefined(headersCollection[index + 1]) &&
              childLevel === 0);
        ...
        if (isChildAvail) {
          let childHeaderCollection: string[] = headersCollection
            .slice(0, index)
            .concat(
              headersCollection.slice(index + 1, headersCollection.length)
            );
          const childMeasureIndex: number =
            childHeaderCollection.indexOf(measureAxis);
          if (childMeasureIndex < measureIndex) {
            childHeaderCollection = this.reArrangeHeaders(
              childHeaderCollection,
              measureIndex,
              measureAxis
            );
          }
          selectedMember = this.getParentElement(
            childMember,
            childHeaderCollection,
            childMember[0].getElementsByTagName('Member').length,
            measureIndex,
            measureAxis
          );
        }
    ...
    if (isGrandTotal) {
      ...
      index < headersCollection.length - 1 &&
  ...
  private reArrangeHeaders(
    ...
    const headerColl2: string[] = headersCollection.slice(
      actualIndex + 1,
      headersCollection.length
    );
    headerColl1[measureIndex as number] =
      headersCollection[actualIndex as number];
  ...
  private getSubTotalsVisibility(): void {
    ...
    this.showColumnSubTotals =
      this.dataSourceSettings.showColumnSubTotals &&
      this.dataSourceSettings.showSubTotals;
    this.showSubTotalsAtTop =
      this.showColumnSubTotals &&
      this.dataSourceSettings.subTotalsPosition === 'Top';
    this.showSubTotalsAtBottom =
      this.showRowSubTotals &&
    ...
    do {
      if (axisCount === 1) {
        if (this.showColumnSubTotals) {
          for (
            let cCnt: number = 0;
            cCnt < this.dataSourceSettings.columns.length;
            cCnt++
          ) {
            if (
              this.dataSourceSettings.columns[cCnt as number].showSubTotals ===
              false
            ) {
              this.hideColumnTotalsObject[
                this.dataSourceSettings.columns[cCnt as number].name
              ] = cCnt;
            }
          }
        }
        ...
        if (this.showRowSubTotals) {
          for (
            let rCnt: number = 0;
            rCnt < this.dataSourceSettings.rows.length;
            rCnt++
          ) {
            if (
              this.dataSourceSettings.rows[rCnt as number].showSubTotals ===
              false
            ) {
              this.hideRowTotalsObject[
                this.dataSourceSettings.rows[rCnt as number].name
              ] = rCnt;
            }
          }
        }
  ...
  private frameRowHeader(tuples: Element[]): void {
    ...
    const newTupPosition: number =
      this.customArgs.drillInfo && this.customArgs.drillInfo.axis === 'row'
        ? this.customArgs.drillInfo.currentCell.ordinal + 1
        : 0;
    while (tupPos < tuples.length) {
      const members: NodeListOf<Element> =
        tuples[tupPos as number].querySelectorAll('Member');
      maxLevel = this.frameTupCollection(
        members,
        maxLevel,
        tupPos + newTupPosition,
        this.tupRowInfo,
        this.showRowSubTotals,
        this.hideRowTotalsObject,
        'row'
    ...
    while (tupPos < tuplesLength && pagingAllowFlag) {
      const members: NodeListOf<Element> =
        tuples[
          this.customArgs.action === 'down'
            ? tupPos - (this.customArgs.drillInfo.currentCell.ordinal + 1)
            : tupPos
      ...
      const typeColl: string[] =
        this.tupRowInfo[tupPos as number].typeCollection;
      // let drillInfo: IDrillInfo[] = this.tupRowInfo[tupPos].drillInfo;
      const drillStartPos: number =
        this.tupRowInfo[tupPos as number].drillStartPos;
      const startDrillUniquename: string =
        this.tupRowInfo[tupPos as number].startDrillUniquename;
      ...
      if (tupPos === 0 || tupPos === startTupPos) {
        const firstTupMembers: NodeListOf<Element> =
          this.customArgs.action === 'down'
            ? this.tupRowInfo[0].members
            : members;
        while (memPos < firstTupMembers.length) {
          if (
            firstTupMembers[memPos as number].querySelector('MEMBER_TYPE')
              .textContent === '1' &&
            Number(
              firstTupMembers[memPos as number].querySelector('LNum')
                .textContent
            ) === 0
          ) {
            minLevel[memPos as number] = 0;
          } else {
            minLevel[memPos as number] = Number(
              firstTupMembers[memPos as number].querySelector('LNum')
                .textContent
            );
          }
          ...
          if (
            ...
            withoutAllStartPos =
              withoutAllStartPos === -1 ? memPos : withoutAllStartPos;
      ...
      if (
        tupPos === 0 &&
        (members.length > allCount + (measure ? 1 : 0) ||
          (members.length === 1 && measure))
      ) {
        gTotals.pop();
      }
      if (
        tupPos === 0 && this.isPaging
          ? gTotals.length === 0
          : !availAllMember ||
            allCount === lastAllCount ||
            allStartPos !== lastAllStartPos ||
            (members.length === 1 && measure)
      ) {
        const attrDrill: boolean = this.checkAttributeDrill(
          this.tupRowInfo[tupPos as number].drillInfo,
          'rows'
        );
        let drillAllow: boolean =
          drillStartPos > -1
            ? allCount > 0
              ? attrDrill || allStartPos > drillStartPos
              : true
            : true;
        drillAllow =
          prevTupInfo && drillAllow && drillStartPos > -1
            ? prevTupInfo.startDrillUniquename !== startDrillUniquename
              ? true
              : (withoutAllEndPos > prevTupInfo.measurePosition
                  ? false
                  : prevTupInfo.measureName !==
                    this.tupRowInfo[tupPos as number].measureName) &&
                (allStartPos === drillStartPos + 1 ||
                  this.tupRowInfo[tupPos as number].measurePosition ===
                    drillStartPos + 1)
            : drillAllow;
        const withoutAllAllow: boolean =
          withoutAllStartPos > -1 && allCount > 0
            ? attrDrill || allStartPos > withoutAllEndPos
            : true;
        isGrandTotalTop =
          this.dataSourceSettings.grandTotalsPosition === 'Top' &&
          this.olapRowValueIndex === 0 &&
          this.olapValueAxis === 'row' &&
          this.dataSourceSettings.showGrandTotals &&
          this.dataSourceSettings.showRowGrandTotals &&
          (this.olapValueAxis === 'row'
            ? this.dataSourceSettings.rows.length > 1
            : true);
        if (isGrandTotalTop && gTotals.length === 1) {
          gTotals = this.frameGrandTotalValues(
            tuples,
            gTotals,
            typeColl,
            measurePos
        ...
        if (
          ...
          const levelName: string =
            ...
            this.dataFields[
              this.getUniqueName(
                members[measurePos as number].querySelector('UName').textContent
              )
            ].caption
              ? this.dataFields[
                  this.getUniqueName(
                    members[measurePos as number].querySelector('UName')
                      .textContent
                  )
                ].caption
              : members[measurePos as number].querySelector('Caption')
                  .textContent;
          const formattedText: string =
            typeColl[measurePos as number] === '3' &&
            this.dataFields[
              this.getUniqueName(
                members[measurePos as number].querySelector('UName').textContent
              )
          ...
          gTotals = this.frameGrandTotalAxisSet(
            ...
            position,
            tupPos,
            Number(typeColl[measurePos as number]),
            members[measurePos as number].querySelector('PARENT_UNIQUE_NAME')
              ? members[measurePos as number].querySelector(
                  'PARENT_UNIQUE_NAME'
                ).textContent
              : undefined,
            ...
            {
              levelName: levelName,
              axis: members[measurePos as number].getAttribute('Hierarchy'),
            }
          );
          gTotals[gTotals.length - 1].valueSort[levelName as string] = 1;
        } else if (
          !(allStartPos === 0 || (measurePos === 0 && allStartPos === 1)) &&
          drillAllow &&
          withoutAllAllow
        ) {
          if (
          ...
          while (memPos < members.length && pagingAllowFlag) {
            ...
            if (
              member.querySelector('UName').textContent !==
                prevUNArray[memPos as number] &&
              typeColl[memPos as number] !== '2' &&
              ((
                Object.keys(prevParent).length > 0
                  ? prevParent.isDrilled &&
                    !this.fieldList[prevParent.hierarchy].isHierarchy
                  : withoutAllDrilled
              )
                ? typeColl[memPos as number] === '3' &&
                  allType[memPos - 1] &&
                  allType[memPos + 1] !== 0
                : true)
            ) {
              const lvl: number =
                Number(member.querySelector('LNum').textContent) -
                (allType[memPos as number] && typeColl[memPos as number] !== '3'
                  ? 1
                  : minLevel[memPos as number]);
              const isNamedSet: boolean = this.namedSetsPosition['row'][
                memPos as number
              ]
                ? true
                : false;
              const uniqueName: string = this.getUniqueName(
                member.querySelector('UName').textContent
              );
              const depth: number = this.getDepth(
                this.tupRowInfo[tupPos as number],
                uniqueName,
                Number(typeColl[memPos as number])
              );
              let levelName: string = this.getCaptionCollectionWithMeasure(
                this.tupRowInfo[tupPos as number],
                memPos,
                true
              );
              if (this.showSubTotalsAtBottom && position > this.rowStartPos) {
                lastPos = position = this.insertRowSubTotal(
                  pivotValues,
                  valueContent,
                  subTotals,
                  position,
              ...
              if (
                !(
                  this.isPaging &&
                  pivotValues[lastMesPos as number][0] &&
                  this.fieldList[
                    (pivotValues[lastMesPos as number][0] as IAxisSet).hierarchy
                  ] &&
                  this.fieldList[
                    (pivotValues[lastMesPos as number][0] as IAxisSet).hierarchy
                  ].isHierarchy &&
                  (pivotValues[lastMesPos as number][0] as IAxisSet).hasChild &&
                  !(pivotValues[lastMesPos as number][0] as IAxisSet)
                    .isDrilled &&
                  !this.rows[memPos as number].isNamedSet &&
                  (this.rows[memPos as number].name.indexOf('[Measures]') ===
                    0 ||
                    (this.fieldList[member.getAttribute('Hierarchy')] &&
                      (this.fieldList[member.getAttribute('Hierarchy')]
                        ...
                        this.fieldList[member.getAttribute('Hierarchy')]
                          .hasAllMember))) &&
                  (pivotValues[lastMesPos as number][0] as IAxisSet).depth <
                    depth
                )
              ) {
                (pivotValues[position as number] as IAxisSet[]) = [
                  {
                    ...
                    formattedText:
                      typeColl[memPos as number] === '3' &&
                      this.dataFields[uniqueName as string] &&
                      this.dataFields[uniqueName as string].caption
                        ? this.dataFields[uniqueName as string].caption
                    ...
                    hasChild:
                      ...
                      this.fieldList[member.getAttribute('Hierarchy')]
                        .isHierarchy &&
                      memPos < this.rows.length - 1 &&
                      !this.rows[memPos + 1].isNamedSet &&
                      this.rows[memPos + 1].name.indexOf('[Measures]') < 0 &&
                      this.fieldList[this.rows[memPos + 1].name] &&
                      this.fieldList[this.rows[memPos + 1].name].hasAllMember
                        ? true
                        : Number(
                            member.querySelector('CHILDREN_CARDINALITY')
                              .textContent
                          ) > 0
                        ? true
                        : false,
                ...
                if (
                  this.olapVirtualization &&
                  (pivotValues[position as number] as IAxisSet[])[0].type ===
                    'header'
                ) {
                  delete (pivotValues[position as number] as IAxisSet[])[0]
                    .type;
                }
                prevParent =
                  typeColl[memPos as number] !== '3'
                    ? (pivotValues[position as number] as IAxisSet[])[0]
                    : prevParent;
                if (!prevParent) {
                  rowMembers.push(member.querySelector('Caption').textContent);
                }
                levelName = this.getCaptionCollectionWithMeasure(
                  this.tupRowInfo[tupPos as number],
                  memPos,
                  false
                );
                (
                  pivotValues[position as number] as IAxisSet[]
                )[0].valueSort.levelName = levelName;
                (pivotValues[position as number] as IAxisSet[])[0].valueSort[
                  levelName as string
                ] = 1;
                valueContent[position - this.rowStartPos] = {} as IAxisSet[];
                valueContent[position - this.rowStartPos][0] = pivotValues[
                  position as number
                ][0] as IAxisSet;
                ...
                prevUNArray[memPos as number] =
                  member.querySelector('UName').textContent;
                lastMesPos =
                  Number(typeColl[memPos as number]) !== 3
                    ? position
                    : lastMesPos;
                position++;
                lastMemPos = memPos;
              ...
              if (
                this.tupRowInfo[tupPos as number].drillInfo[memPos as number]
                  .isDrilled &&
                allType[memPos as number] === 0
              ) {
            ...
            if (
              ...
              this.tupRowInfo[tupPos as number].showTotals = !this
                .showRowSubTotals
                ? false
                : this.hideRowTotalsObject[
                    this.tupRowInfo[tupPos as number].drillInfo[
          ...
          if (lastPos < position && lastMemPos >= members.length - 1) {
            ...
            if (
              (pivotValues[position - 1] as IAxisSet[])[0].type === 'header'
            ) {
  ...
  private insertRowSubTotal(
    ...
    const prevRowParent: IAxisSet = PivotUtil.frameHeaderWithKeys(
      pivotValues[position - 1][0] as IAxisSet
    );
    if (
      prevRowParent.level < lvl &&
      prevRowParent.type !== 'grand sum' &&
      (prevRowParent.isDrilled || prevRowParent.memberType === 3)
    ) {
      if (prevRowParent.memberType === 3) {
        const valueCells: IAxisSet[] = [];
        let index: number = 1;
        while (
          (pivotValues[position - index][0] as IAxisSet).memberType === 3
        ) {
          valueCells[valueCells.length] = pivotValues[
            position - index
          ][0] as IAxisSet;
          index++;
        }
        if (
          (pivotValues[position - index][0] as IAxisSet).isDrilled &&
          valueCells.length !== 0
        ) {
          for (
            let i: number = 0, axislength: number = valueCells.length;
            i < axislength;
            i++
          ) {
            valueCells[i as number].formattedText =
              (pivotValues[position - index][0] as IAxisSet).formattedText +
              ' ' +
              valueCells[i as number].formattedText;
            valueCells[i as number].isSum = true;
            valueCells[i as number].type = 'sum';
            valueCells[i as number].parentUniqueName = (
              pivotValues[position - index][0] as IAxisSet
            ).levelUniqueName;
    ...
    if (subTotals.length > 0) {
      ...
      let subTotalLevel: number = (
        subTotals[j as number].valueSort.levelName as string
      ).split('.').length;
  ...
  private insertRowGrandTotal(
    ...
    // }
    if (this.customArgs.action !== 'down') {
      const grandTotalFlag: boolean =
        this.dataSourceSettings.rows.length === 0 ||
        (this.dataSourceSettings.rows.length === 1 &&
          this.dataSourceSettings.rows[0].name === '[Measures]');
      if (
        (this.dataSourceSettings.showGrandTotals &&
          this.dataSourceSettings.showRowGrandTotals) ||
        grandTotalFlag
  ...
  private getDepth(
    ...
    const cropUName: string =
      tupInfo.uNameCollection.substring(0, memberPosition) +
  ...
  private checkAttributeDrill(drillInfo: IDrillInfo[], axis: string): boolean {
    ...
    for (let i: number = 0; i < drillInfo.length; i++) {
      isDrill = this.isAttributeDrill(
        drillInfo[i as number].hierarchy,
        drillInfo,
        axis
      );
      if (isDrill) {
  ...
  private frameTupCollection(
    ...
    hideTotalsObject: { [key: string]: number },
    axis: string
  ): number[] {
    let memPos: number = 0;
    ...
    while (memPos < members.length) {
      const member: Element = members[memPos as number];
      const memberlevel: number = Number(
        member.querySelector('LNum').textContent
      );
      ...
      if (Number(member.querySelector('MEMBER_TYPE').textContent) > 3) {
        member.querySelector('MEMBER_TYPE').textContent =
          memberUName.indexOf('[Measures]') === 0 ? '3' : '1';
      }
      const memberType: string =
        memberUName.indexOf('[Measures]') === 0
          ? '3'
          : Number(member.querySelector('MEMBER_TYPE').textContent) > 3
          ? '1'
          : member.querySelector('MEMBER_TYPE').textContent;
      let memberCaption: string = member.querySelector('Caption').textContent;
      if (
        this.fieldList[memberCaption as string] &&
        this.fieldList[memberCaption as string].type === 'CalculatedField'
      ) {
      ...
      if (memberType !== '2') {
        if (this.headerGrouping[memPos as number]) {
          if (memberlevel > this.lastLevel[memPos as number]) {
            ...
            while (levelPos >= memberlevel) {
              delete this.headerGrouping[memPos as number].UName[
                levelPos as number
              ];
              delete this.headerGrouping[memPos as number].Caption[
                levelPos as number
              ];
              levelPos--;
            }
          ...
          this.headerGrouping[memPos as number].UName[memberlevel as number] =
            memberUName;
          this.headerGrouping[memPos as number].Caption[memberlevel as number] =
            memberCaption;
        } else {
          this.lastLevel[memPos as number] = memberlevel;
        ...
        if (this.isPaging) {
          ...
          while (this.drilledSets[currUName as string]) {
            const currCaption: string =
              this.drilledSets[currUName as string].querySelector(
                'Caption'
              ).textContent;
            const currLevel: number = Number(
              this.drilledSets[currUName as string].querySelector('LNum')
                .textContent
            );
            this.headerGrouping[memPos as number].UName[currLevel as number] =
              currUName;
            this.headerGrouping[memPos as number].Caption[currLevel as number] =
              currCaption;
            currUName =
              this.drilledSets[currUName as string].querySelector(
                'PARENT_UNIQUE_NAME'
        ...
        let uNames: string = '';
        const uNamesKeys: string[] = Object.keys(
          this.headerGrouping[memPos as number].UName
        );
        for (let i: number = 0; i < uNamesKeys.length; i++) {
          ...
          if (i === 0) {
            ...
            uNames =
              uNames +
              '~~' +
              this.headerGrouping[memPos as number].UName[Number(j)];
          }
        }
        uNameCollection =
          uNameCollection === '' ? uNames : uNameCollection + '::' + uNames;
        let captions: string = '';
        const captionsKeys: string[] = Object.keys(
          this.headerGrouping[memPos as number].Caption
        );
        for (let i: number = 0; i < captionsKeys.length; i++) {
          ...
          if (i === 0) {
            ...
            captions =
              captions +
              '~~' +
              this.headerGrouping[memPos as number].Caption[Number(j)];
          }
        }
      ...
      if (
        isNullOrUndefined(maxLevel[memPos as number]) ||
      ...
      if (
        tupInfo[tupPos - 1] &&
        tupInfo[tupPos - 1].typeCollection[memPos as number] === '1' &&
        drillState[memPos as number].level >
          tupInfo[tupPos - 1].drillInfo[memPos as number].level
      ) {
        const uCollection: string[] = uNameCollection
          .split(/~~|::\[/)
          .map((item: string) => {
            return item[0] === '[' ? item : '[' + item;
          });
    ...
    tupInfo[tupPos as number] = {
      ...
      showTotals:
        !showSubTotals &&
        allCount > 0 &&
        allStartPos > (measurePosition === 0 ? 1 : 0)
          ? false
          : showTotals,
    };
    return maxLevel;
  ...
  private getCaptionCollectionWithMeasure(
    ...
    if (!isAddTotals && tuple.typeCollection[memPos as number] !== '3') {
      for (let i: number = 0; i < this.measurePosition; i++) {
        if (
          tuple.drillInfo[memPos as number].hierarchy ===
          this.dataSourceSettings.rows[i as number].name
        ) {
          isMeasureAdd = false;
          break;
        }
    ...
    if (tuple.measure && isMeasureAdd) {
      const uName: string = this.getUniqueName(
        tuple.measure.querySelector('UName').textContent
      );
      const measureName: string =
        this.dataFields[uName as string] &&
        this.dataFields[uName as string].caption
          ? this.dataFields[uName as string].caption
          : tuple.measure.querySelector('Caption').textContent;
      const measurePosition: number = tuple.uNameCollection
        .split(/[~~::]+/g)
        .indexOf(tuple.measureName);
      const captionCollectionArray: string[] =
        tuple.captionCollection.split(/[~~::]+/g);
  ...
  public setNamedSetsPosition(): void {
    ...
    do {
      ...
      const axisFields: IFieldOptions[] = axis
        ? this.dataSourceSettings.rows
        : this.dataSourceSettings.columns;
  ...
  private updateRowEngine(
    ...
    while (currEngineCount > this.customArgs.drillInfo.currentCell.rowIndex) {
      this.pivotValues[currEngineCount + newEngineCount] =
        this.pivotValues[currEngineCount as number];
      (
        this.pivotValues[currEngineCount + newEngineCount][0] as IAxisSet
      ).ordinal += tuplesLength;
      (
        this.pivotValues[currEngineCount + newEngineCount][0] as IAxisSet
      ).rowIndex += newEngineCount;
    ...
    for (let key: number = 0; key < pivotValues.length; key++) {
      ...
      this.valueContent[Number(key) - this.rowStartPos] =
        valueContent[Number(key) - this.rowStartPos];
    }
    (
      this.pivotValues[
        this.customArgs.drillInfo.currentCell.rowIndex
      ][0] as IAxisSet
    ).isDrilled = true;
  }

  private updateTupCollection(newTuplesCount: number): void {
    const tupCollection: ITupInfo[] =
      this.customArgs.drillInfo.axis === 'row'
        ? this.tupRowInfo
        : this.tupColumnInfo;
    let currTupCount: number = tupCollection.length - 1;
    while (currTupCount > this.customArgs.drillInfo.currentCell.ordinal) {
      tupCollection[currTupCount + newTuplesCount] =
        tupCollection[currTupCount as number];
      currTupCount--;
    }
  }
  ...
  private frameGrandTotalValues(
    ...
    while (tupPos < tuples.length && !isGrandtoalDataAdd) {
      ...
      isGrandtoalDataAdd =
        this.dataSourceSettings.values.length + 1 === gTotals.length
          ? true
          : false;
      tupPos++;
    }
  ...
  private frameColumnHeader(tuples: Element[]): void {
    ...
    let withoutAllEndPos: number = -1;
    const newTupPosition: number =
      this.customArgs.drillInfo && this.customArgs.drillInfo.axis === 'column'
        ? this.customArgs.drillInfo.currentCell.ordinal + 1
        : 0;
    while (tupPos < tuples.length) {
      const members: NodeListOf<Element> =
    ...
    if (this.olapVirtualization) {
      maxLevel = maxLevel
        .slice(0, maxLevel.length - 1)
        .map((level: number) => (level === 0 ? 1 : level))
    ...
    if (tuples.length > 0) {
      ...
      while (memPos < members.length) {
        minLevel[memPos as number] =
          members[memPos as number].querySelector('MEMBER_TYPE').textContent ===
            '1' &&
          Number(
            members[memPos as number].querySelector('LNum').textContent
          ) === 0
            ? 0
            : Number(
                members[memPos as number].querySelector('LNum').textContent
    ...
    while (tupPos < tuples.length) {
      ...
      const allStartPos: number =
        this.tupColumnInfo[tupPos as number].allStartPos;
      const measure: Element = this.tupColumnInfo[tupPos as number].measure;
      const typeColl: string[] =
        this.tupColumnInfo[tupPos as number].typeCollection;
      const drillInfo: IDrillInfo[] =
        this.tupColumnInfo[tupPos as number].drillInfo;
      const drillStartPos: number =
        this.tupColumnInfo[tupPos as number].drillStartPos;
      const startDrillUniquename: string =
        this.tupColumnInfo[tupPos as number].startDrillUniquename;
      const endDrillUniquename: string =
        this.tupColumnInfo[tupPos as number].endDrillUniquename;
      const drillEndPos: number =
        this.tupColumnInfo[tupPos as number].drillEndPos;
      const levelColl: number[] =
        this.tupColumnInfo[tupPos as number].levelCollection;
      let isStartCol: boolean =
        typeColl[0] === '2'
          ? false
          : typeColl[0] === '3'
          ? typeColl[1] !== '2'
      ...
      maxLevel.map((item: number, pos: number) => {
        depth =
          depth +
          (allType[pos as number] === 0
            ? item +
              (1 - (minLevel[pos as number] > 1 ? 1 : minLevel[pos as number]))
            : item === 0
            ? this.isPaging && typeColl[pos as number] === '2'
              ? 0
              : 1
            : item);
      });
      ...
      if (tupPos === 0 && members.length > allCount + (measure ? 1 : 0)) {
        ...
        isStartCol =
          allCount > 0 && isStartCol
            ? allStartPos > withoutAllStartPos
            : isStartCol;
      }
      let isGrandTotalTop: boolean = false;
      if (
        ...
        isGrandTotalTop =
          count ===
          (this.olapValueAxis === 'column'
            ? this.dataSourceSettings.columns.length - 1
      ...
      if (isStartCol || isGrandTotalTop) {
        if (allCount === 0 || isGrandTotalTop) {
          ...
          if (
            this.tupColumnInfo[tupPos - 1] &&
            this.tupColumnInfo[tupPos - 1].allCount === 0
          ) {
            levelComp = this.levelCompare(
              levelColl,
              this.tupColumnInfo[tupPos - 1].levelCollection
            );
          } else if (withoutAllAvail && lastRealTup) {
            levelComp = this.levelCompare(
              levelColl,
              lastRealTup.levelCollection
            );
          }
          if (
            this.tupColumnInfo[tupPos as number].drillStartPos < 0 ||
            this.showSubTotalsAtTop
          ) {
            ...
            (lastSavedInfo as ITupInfo).drillStartPos === drillStartPos
              ? (lastSavedInfo as ITupInfo).startDrillUniquename !==
                  startDrillUniquename ||
            ...
            if (withoutAllAvail ? withoutAllEndPos <= drillStartPos : true) {
              ...
              (lastSavedInfo as ITupInfo).startDrillUniquename =
                startDrillUniquename;
              (lastSavedInfo as ITupInfo).endDrillUniquename =
                endDrillUniquename;
            }
          }
      ...
      if (
        allCount > 0 &&
        (withoutAllAvail
          ? isStartCol && (attrDrill || withoutAllEndPos < allStartPos)
          : true)
      ) {
        if (
          allCount === (lastSavedInfo as ITupInfo).allCount ||
          allStartPos !== (lastSavedInfo as ITupInfo).allStartPos
        ) {
          const endAllow: boolean =
            drillEndPos !== drillStartPos
              ? (lastSavedInfo as ITupInfo).endDrillUniquename ===
                endDrillUniquename
              : true;
          const allow: boolean =
          ...
          if (drillStartPos > -1 ? allow : true) {
            ...
            this.setParentCollection(members);
            if (
              withoutAllAvail && drillStartPos > -1
                ? withoutAllEndPos <= drillStartPos
                : true
            ) {
              if (!isGrandTotalTop) {
                this.totalCollection[this.totalCollection.length] = {
    ...
    if (this.totalCollection.length > 0) {
      if (Object.keys(this.colMeasures).length > 1) {
        ...
        for (const coll of this.totalCollection) {
          const isGrandTotal: boolean =
            this.tupColumnInfo[coll.ordinal].measurePosition === 0
              ? this.tupColumnInfo[coll.ordinal].allStartPos === 1
              : this.tupColumnInfo[coll.ordinal].allStartPos === 0;
          if (
            isGrandTotal
              ? this.dataSourceSettings.showGrandTotals &&
                this.dataSourceSettings.showColumnGrandTotals
            ...
            const attrDrill: boolean = this.checkAttributeDrill(
              this.tupColumnInfo[coll.ordinal].drillInfo,
              'columns'
            );
  ...
  private orderTotals(
    ...
    const groupColl: {
      [key: string]: { coll: ITotCollection[]; count: number };
    } = {};
    let maxCnt: number = 1;
    for (const coll of this.totalCollection) {
      const isGrandTotal: boolean =
      ...
      if (
        ...
        const measureName: string =
          this.tupColumnInfo[coll.ordinal].measure.querySelector(
            'UName'
          ).textContent;
        if (groupColl[measureName as string]) {
          ...
          maxCnt =
            maxCnt < groupColl[measureName as string].count
              ? groupColl[measureName as string].count
              : maxCnt;
        } else {
    ...
    while (collLength > -1) {
      for (const key of keys) {
        const coll: ITotCollection =
          groupColl[key as string].coll[collLength as number];
        if (coll) {
          this.frameCommonColumnLoop(
            coll.members,
            coll.ordinal,
  ...
  private setParentCollection(members: NodeListOf<Element>): void {
    ...
    while (members.length > memPos) {
      const member: Element = members[memPos as number];
      const memberType: string =
        Number(member.querySelector('MEMBER_TYPE').textContent) > 2
          ? '3'
          : member.querySelector('MEMBER_TYPE').textContent;
      const memberlevel: number = Number(
        member.querySelector('LNum').textContent
      ...
      if (this.parentObjCollection[memPos as number]) {
        const levelCollection: string[] = Object.keys(
          this.parentObjCollection[memPos as number]
        );
        const parentMember: Element =
          this.parentObjCollection[memPos as number][memberlevel - 1];
        isSameParent = parentMember
          ? parentUName === parentMember.querySelector('UName').textContent
          : levelCollection.length === 0;
        isWithoutAllMember =
          this.tupColumnInfo[0].typeCollection[memPos as number] === '1';
      }
      if (memberType === '2') {
        ...
        if (!this.parentObjCollection[memPos as number]) {
          this.parentObjCollection[memPos as number] = {};
          this.parentObjCollection[memPos as number][memberlevel as number] =
            member;
        } else if (
          !this.parentObjCollection[memPos as number][memberlevel as number] ||
          this.parentObjCollection[memPos as number][
            memberlevel as number
          ].querySelector('UName').textContent !== memberUName
        ) {
          this.parentObjCollection[memPos as number][memberlevel as number] =
            member;
        }
  ...
  private setDrillInfo(
    ...
    while (
      tupInfo[tupPos as number] &&
      tupInfo[tupPos as number].drillInfo[memPos as number].uName === pUName
    ) {
      const prevUcollection: string[] = tupInfo[
        tupPos as number
      ].uNameCollection
        .split(/~~|::\[/)
        .map((item: string) => {
      ...
      if (
        ...
        tupInfo[tupPos as number].drillStartPos === -1
      ) {
        tupInfo[tupPos as number].drillStartPos = memPos;
      }
  ...
  private levelCompare(newLevels: number[], oldLevels: number[]): number[] {
    ...
    for (let lPos: number = 0; lPos < oldLevels.length; lPos++) {
      if (newLevels[lPos as number] !== oldLevels[lPos as number]) {
        changePos = [
          lPos,
          newLevels[lPos as number],
          oldLevels[lPos as number] - newLevels[lPos as number],
        ];
        break;
      }
    }
  ...
  private mergeTotCollection(
    ...
    const prevHdrPos: number = isNullOrUndefined(allStartPos)
      ? levelComp[0]
      : allStartPos - (this.colMeasurePos === allStartPos - 1 ? 2 : 1);
    const flagLevel: number =
      drillInfo[prevHdrPos as number] && drillInfo[prevHdrPos as number].level;
    const flagLevelString: string = this.getLevelsAsString(
      prevHdrPos - 1,
      drillInfo
    );
    const groupColl: {
    ...
    for (const coll of this.totalCollection) {
      if (
        enterFlag ||
        (coll.allCount <= allCount &&
          (flagLevel > -1 && coll.drillInfo[prevHdrPos as number]
            ? coll.drillInfo[prevHdrPos as number].level >= flagLevel &&
              this.getLevelsAsString(prevHdrPos - 1, coll.drillInfo) ===
                flagLevelString
            : true))
      ) {
        const measureName: string = this.tupColumnInfo[coll.ordinal].measure
          ? this.tupColumnInfo[coll.ordinal].measure.querySelector('UName')
              .textContent
          : 'measure';
        if (groupColl[measureName as string]) {
          groupColl[measureName as string].coll.push(coll);
    ...
    while (collLength > -1) {
      for (const key of keys) {
        const coll1: ITotCollection =
          groupColl[key as string].coll[collLength as number];
        if (coll1) {
          const isGrandTotal: boolean =
            this.tupColumnInfo[coll1.ordinal].measurePosition === 0
              ? this.tupColumnInfo[coll1.ordinal].allStartPos === 1
              : this.tupColumnInfo[coll1.ordinal].allStartPos === 0;
          if (
            isGrandTotal
              ? this.dataSourceSettings.showGrandTotals &&
                this.dataSourceSettings.showColumnGrandTotals
  ...
  private frameCommonColumnLoop(
    ...
    if (this.tupColumnInfo[tupPos as number].showTotals) {
      ...
      while (memberPos < members.length) {
        memberDepth +=
          allType[memberPos as number] > 0 &&
          this.getMeasurePosition(
            this.tupColumnInfo[tupPos as number].uNameCollection,
            this.tupColumnInfo[tupPos as number].measurePosition
          ) !== memberPos
            ? maxLevel[memberPos as number]
            : maxLevel[memberPos as number] +
              (1 - minLevel[memberPos as number]);
        if (
          this.tupColumnInfo[tupPos as number].drillInfo[memberPos as number]
            .isDrilled &&
          this.tupColumnInfo[tupPos as number].showTotals
        ) {
          this.tupColumnInfo[tupPos as number].showTotals = !this
            .showColumnSubTotals
            ? false
            : this.hideColumnTotalsObject[
                this.tupColumnInfo[tupPos as number].drillInfo[
          ...
          drillMemberPosition = this.tupColumnInfo[tupPos as number].showTotals
            ? -1
            : memberDepth - 1;
        }
        memberPos++;
    ...
    if (this.tupColumnInfo[tupPos as number].showTotals || attrDrill) {
      ...
      let isGrandTotal: boolean =
        members[0].querySelector('LNum').textContent === '0';
      while (memPos < members.length) {
        ...
        for (
          let rowDepthPos: number =
            memberType !== '2'
              ? allType[memPos as number]
                ? 1
                : minLevel[memPos as number]
              : 1;
          rowDepthPos <= (memberType === '3' ? 1 : maxLevel[memPos as number]);
          rowDepthPos++
        ) {
          ...
          if (
            ...
            if (
              !this.parentObjCollection[memPos as number][rowDepthPos as number]
            ) {
              this.getDrilledParent(
                members[memPos as number],
                rowDepthPos,
                this.parentObjCollection[memPos as number]
              );
            }
          ...
          let levelName: string =
            memPos >= this.measurePosition &&
            Number(member.getElementsByTagName('LNum')[0].textContent) === 0 &&
            levelNameKeys.length === 1
          ...
          for (let i: number = 0; i < levelNameKeys.length; i++) {
            ...
            if (i === 0) {
              ...
              levelName =
                levelName +
                this.valueSortSettings.headerDelimiter +
                colMembers[j as string];
            }
          }
          const isNamedSet: boolean = this.namedSetsPosition['column'][
            memPos as number
          ]
            ? true
            : false;
          const depth: number = this.getDepth(
            this.tupColumnInfo[tupPos as number],
            uName,
            Number(memberType)
          );
          if (
            !(
              this.isPaging &&
              this.pivotValues[spanMemPos - 1] &&
              this.pivotValues[spanMemPos - 1][position as number] &&
              this.fieldList[
                (
                  this.pivotValues[spanMemPos - 1][
                    position as number
              ...
              (this.pivotValues[spanMemPos - 1][position as number] as IAxisSet)
                .hasChild &&
              !(
                this.pivotValues[spanMemPos - 1][position as number] as IAxisSet
              ).isDrilled &&
              !this.columns[memPos as number].isNamedSet &&
              this.fieldList[member.getAttribute('Hierarchy')] &&
            ...
            (this.pivotValues[spanMemPos as number] as IAxisSet[])[
              ...
              hasChild:
                this.fieldList[member.getAttribute('Hierarchy')] &&
                this.fieldList[member.getAttribute('Hierarchy')].isHierarchy &&
                memPos < this.columns.length - 1 &&
                !this.columns[memPos + 1].isNamedSet &&
                this.columns[memPos + 1].name.indexOf('[Measures]') < 0 &&
                this.fieldList[this.columns[memPos + 1].name] &&
                this.fieldList[this.columns[memPos + 1].name].hasAllMember
                  ? true
                  : Number(
                      member.querySelector('CHILDREN_CARDINALITY').textContent
                    ) > 0
                  ? true
                  : false,
              level:
      ...
      if (drillMemberPosition > -1) {
        (this.pivotValues[drillMemberPosition as number] as IAxisSet[])[
          position - 1
        ].ordinal = tupPos;
        ...
        if (
          this.tupColumnInfo[tupPos as number].allStartPos ===
          this.tupColumnInfo[tupPos as number].measurePosition + 1
        ) {
          memberDepth -=
            maxLevel[this.tupColumnInfo[tupPos as number].allStartPos - 2] -
            this.tupColumnInfo[tupPos as number].levelCollection[
              this.tupColumnInfo[tupPos as number].allStartPos - 2
            ] +
            1;
        } else {
          memberDepth -=
            maxLevel[this.tupColumnInfo[tupPos as number].allStartPos - 1] -
            this.tupColumnInfo[tupPos as number].levelCollection[
              this.tupColumnInfo[tupPos as number].allStartPos - 1
            ];
        }
        if (this.pivotValues[memberDepth - 1]) {
          (this.pivotValues[memberDepth - 1] as IAxisSet[])[
            position - 1
          ].ordinal = tupPos;
        }
      }
    ...
    if (!isAdjacent) {
      for (let i: number = 0; i < this.drilledMembers.length; i++) {
        if (this.drilledMembers[i as number].name === hierarchy) {
          for (
            let j: number = 0;
            j < this.drilledMembers[i as number].items.length;
            j++
          ) {
            const delimiter: string =
              this.drilledMembers[i as number].delimiter;
            const drillItems: string[] =
              this.drilledMembers[i as number].items[j as number].split(
                delimiter
              );
            let levelName: string = '';
            ...
            if (
              levelName === this.drilledMembers[i as number].items[j as number]
            ) {
              isDrill = true;
              break;
            }
  ...
  private isAdjacentToMeasure(hierarchy: string, axis: string): boolean {
    ...
    for (let i: number = 0; i < fields.length; i++) {
      if (
        fields[i as number].name === hierarchy &&
        fields[i + 1] &&
        (fields[i + 1].name === '[Measures]' ||
          fields[i + 1].isNamedSet ||
          (this.fieldList[fields[i + 1].name] &&
  ...
  private getDrilledParent(
    childMember: Element,
    parentLevel: number,
    savedCollection: { [key: number]: Element }
  ): void {
    const childlevel: number = Number(
      childMember.querySelector('LNum').textContent
    );
    let currentChild: Element = childMember;
    for (let lvl: number = childlevel - 1; lvl >= parentLevel; lvl--) {
      const currentParent: Element =
        this.drilledSets[
          currentChild.querySelector('PARENT_UNIQUE_NAME').textContent
        ];
      if (currentParent) {
  ...
  private performRowSorting(
    valCollection: { [key: string]: Element },
    valueSortData: ValueSortInfo
  ): void {
    if (
      (this.enableSort || this.enableValueSorting) &&
      this.tupRowInfo.length > 0
    ) {
      ...
      const withoutAllLastPos: number =
        this.tupRowInfo[0].typeCollection.lastIndexOf('1');
      let isDrilled: boolean;
      for (let rPos: number = this.colDepth; rPos < rowCount; rPos++) {
        const currentCell: IAxisSet = (
          this.pivotValues[rPos as number] as IAxisSet[]
        )[0];
        if (
          this.showSubTotalsAtBottom &&
          currentCell.isSum &&
          currentCell.memberType !== 3
        ) {
          continue;
        }
        const currentTuple: ITupInfo = this.tupRowInfo[currentCell.ordinal];
        let uniqueName: string = currentTuple
          ? currentTuple.measurePosition === 0 && currentCell.memberType === 3
            ? currentTuple.measureName
            : currentTuple.uNameCollection
          : '';
        if (uniqueName !== '') {
          if (withoutAllLastPos > -1) {
            uniqueName = this.frameUniqueName(
              uniqueName,
              currentCell,
              currentTuple
            );
          ...
          if (
            currentCell.memberType === 3 &&
            this.tupRowInfo[0].measurePosition > 0
          ) {
            const parentUName: string = this.getParentUname(
              uniqueName,
              currentCell,
              true,
              true
      ...
      const isMeasureAvail: boolean =
        Object.keys(measureObjects).length > 0 &&
        this.tupRowInfo[0].measurePosition > 0;
      const levels: number[] = Object.keys(lvlGrouping)
        .map((item: string) => {
          return Number(item);
        })
      ...
      for (let lPos: number = levels.length - 1; lPos >= 0; lPos--) {
        ...
        const objCollection: { [key: string]: IAxisSet[] } =
          lvlGrouping[levels[lPos as number]];
        ...
        for (let oPos: number = 0; oPos < objKeys.length; oPos++) {
          const parentUName: string =
            lPos === 0
              ? 'parent'
              : this.getParentUname(
                  objKeys[oPos as number],
                  objCollection[objKeys[oPos as number]][0],
                  isMeasureAvail,
                  false
                );
          if (parentGrouping[parentUName as string]) {
            parentGrouping[parentUName as string].push(
              objCollection[objKeys[oPos as number]][0]
            );
          } else {
            parentGrouping[parentUName as string] = [
              objCollection[objKeys[oPos as number]],
            ][0];
          }
        }
        ...
        for (let kPos: number = 0; kPos < pKeys.length; kPos++) {
          parentGrouping[pKeys[kPos as number]] = this.sortRowHeaders(
            parentGrouping[pKeys[kPos as number]],
            valCollection,
            valueSortData
          );
        }
        if (sortLvlGrouping[levels[lPos + 1]]) {
          for (let kPos: number = 0; kPos < pKeys.length; kPos++) {
            ...
            for (let aPos: number = 0; aPos < axisSets.length; aPos++) {
              const tupInfo: ITupInfo =
                this.tupRowInfo[axisSets[aPos as number].ordinal];
              let uName: string =
                tupInfo.measurePosition === 0 &&
                axisSets[aPos as number].memberType === 3
                  ? tupInfo.measureName
                  : tupInfo.uNameCollection;
              groupSets.push(axisSets[aPos as number]);
              if (withoutAllLastPos > -1) {
                uName = this.frameUniqueName(
                  uName,
                  axisSets[aPos as number],
                  tupInfo
                );
              }
              ...
              if (isMeasureAvail) {
                ...
                if (measureObjects[parentUName as string]) {
                  measureObjects[parentUName as string] = this.sortRowHeaders(
                    measureObjects[parentUName as string],
                    valCollection,
                    valueSortData
                  );
                  const isLastMeasure: boolean =
                    uName.lastIndexOf('::[') === uName.indexOf('::[Measures]');
                  const isFullLength: boolean =
                    uName.split('::[').length - 1 === tupInfo.measurePosition;
                  const isLastNotDrilledMember: boolean =
                    !tupInfo.drillInfo[tupInfo.measurePosition - 1].isDrilled;
                  const isActualLastMember: boolean =
                    tupInfo.members.length > tupInfo.measurePosition + 1;
                  if (
                    isLastMeasure &&
                    isFullLength &&
                    isLastNotDrilledMember &&
                    isActualLastMember
                  ) {
                    isMembersIncluded = true;
                    for (
                      let mPos: number = 0;
                      mPos < measureObjects[parentUName as string].length;
                      mPos++
                    ) {
                      groupSets.push(
                        measureObjects[parentUName as string][mPos as number]
                      );
                      const matchParent: string =
                        uName.substring(0, uName.indexOf('::[Measures]')) +
                        '::' +
                        measureObjects[parentUName as string][mPos as number]
                          .actualText;
                      if (
                        sortLvlGrouping[levels[lPos + 1]][matchParent as string]
                      ) {
                        groupSets = groupSets.concat(
                          sortLvlGrouping[levels[lPos + 1]][
                            matchParent as string
                          ]
                        );
                      }
                    }
                    ...
                    isDrilled =
                      this.showSubTotalsAtBottom &&
                      groupSets[groupSets.length - 1].isDrilled;
                    groupSets = groupSets.concat(
                      measureObjects[parentUName as string]
                    );
                  }
              ...
              if (
                ...
                if (
                  this.showSubTotalsAtBottom &&
                  groupSets[groupSets.length - 1].memberType === 3 &&
                  groupSets[groupSets.length - 1].level > -1 &&
                  isDrilled
                ) {
                  ...
                  groupSets = groupSets
                    .slice(0, sliceIndex)
                    .concat(sortLvlGrouping[levels[lPos + 1]][uName as string])
                    .concat(valueColl);
                } else {
      ...
      gSumFlag = false;
      gSumGrouping = this.sortRowHeaders(
        gSumGrouping,
        valCollection,
        valueSortData
      );
      for (let rPos: number = this.colDepth; rPos < rowCount; rPos++) {
        if (
          ...
          this.dataSourceSettings.showGrandTotals &&
          this.dataSourceSettings.showRowGrandTotals &&
          ((this.olapValueAxis === 'column' && this.colDepth + 1 === rPos) ||
            (this.olapValueAxis === 'row' &&
              this.colDepth + this.dataSourceSettings.values.length + 1 ===
                rPos))
        ) {
          newPos = 0;
          gSumFlag = false;
        }
        const cell: IAxisSet[] = gSumFlag
          ? gSumGrouping
          : sortLvlGrouping[levels[0]]['parent'];
        const currPos: number =
          this.dataSourceSettings.grandTotalsPosition === 'Top' &&
          this.dataSourceSettings.showGrandTotals &&
          this.dataSourceSettings.showRowGrandTotals &&
          this.colDepth === rPos
        ...
        if (cell[currPos as number]) {
          ...
          this.valueContent[valuePos as number][0] = this.pivotValues[
            rPos as number
          ][0] as IAxisSet;
        }
        newPos++;
        valuePos++;
        if (
          (this.pivotValues[rPos as number][0] as IAxisSet).type === 'grand sum'
        ) {
          gSumFlag = true;
          totPos = newPos;
        }
    ...
    if (this.enableSort || this.enableValueSorting) {
      for (let i: number = 0; i < this.dataSourceSettings.columns.length; i++) {
        ...
        for (let j: number = 0; j < this.pivotValues.length; j++) {
          const header: IAxisSet[] = this.pivotValues[
            j as number
          ] as IAxisSet[];
          let keys: string[];
          ...
          for (k; k < header.length; k++) {
            if (!header[k as number].isNamedSet) {
              if (
                (header[k as number] as IAxisSet).memberType !== 2 &&
                header[k as number].hierarchy !== '[Measures]' &&
                (header[k as number] as IAxisSet).level !== -1
              ) {
                if (
                  isNullOrUndefined(
                  ...
                  if (
                    !header[k + 1] ||
                    header[k as number].formattedText !==
                      header[k + 1].formattedText
                  ) {
                    arrangeHeaders[arrangeHeaders.length] = header[k as number];
                  }
                  arrange[header[k as number].formattedText][
                    header[k as number].colIndex
                  ] = header[k as number];
                } else if (
                  arrange[header[k as number].formattedText] &&
                  this.pivotValues[j - 1]
                ) {
                  const prevRowCell: IAxisSet =
                    this.pivotValues[j - 1][header[k as number].colIndex];
                  const prevColValue: number = Number(
                    Object.keys(arrange[header[k as number].formattedText])[0]
                  );
                  const prevColIndex: number =
                    arrange[header[k as number].formattedText][
                      prevColValue as number
                    ].colIndex;
                  const prevColRowCell: IAxisSet =
                    this.pivotValues[j - 1][prevColIndex as number];
                  if (
                    prevRowCell.formattedText !== prevColRowCell.formattedText
                  ) {
                    keys = this.sortColumnHeaders(
                      arrange,
                      arrangeHeaders,
                      this.sortObject[header[k - 1].levelUniqueName] ||
                    ...
                    temporary[index as number] = isNullOrUndefined(
                      temporary[index as number]
                    )
                      ? []
                      : temporary[index as number];
                    for (
                      ...
                      const length: number = Object.keys(
                        arrange[keys[keyPos as number]]
                      ).length;
                      for (
                        let cellPos: number = 0;
                        cellPos < length;
                        cellPos++
                      ) {
                        value =
                          (temporary[index as number] as Object[]).length === 0
                            ? 1
                            : 0;
                        (temporary[index as number] as Object[])[
                          (temporary[index as number] as Object[]).length +
                            value
                        ] =
                          arrange[keys[keyPos as number]][
                            Number(
                              Object.keys(arrange[keys[keyPos as number]])[
                                cellPos as number
                              ]
                            )
                          ];
                ...
                for (let l: number = 0; l < keys.length; l++) {
                  ...
                  for (let q: number = 0; q < length; q++) {
                    ...
                    (temporary[index as number] as Object[])[
                      ...
                      arrange[keys[l as number]][
                        Number(
                          Object.keys(arrange[keys[l as number]])[q as number]
                        )
                      ];
                  }
                }
                ...
                ((header[k as number] as IAxisSet).level === -1 ||
                  (header[k as number] as IAxisSet).level === 0) &&
                Object.keys(arrange).length >= 0 &&
                header[k as number].hierarchy !== '[Measures]'
              ) {
                (grandTotal[index as number] as Object[])[
                  (grandTotal[index as number] as Object[]).length + value
              ...
              if (
                (header[k as number] as IAxisSet).level !== -1 &&
                Object.keys(arrange).length === 1 &&
                header[k as number].hierarchy !== '[Measures]' &&
                !isNullOrUndefined(header[k + 1]) &&
                (header[k + 1] as IAxisSet).level === -1
              ) {
                const height: number = Object.keys(
                  arrange[(header[k as number] as IAxisSet).formattedText]
                ).length;
                const weight: string[] = Object.keys(
                  arrange[(header[k as number] as IAxisSet).formattedText]
                );
                if (height > 1) {
                  for (let hgt: number = 0; hgt < height; hgt++) {
                    value =
                      (grandTotal[index as number] as Object[]).length === 0
                        ? 1
                        : 0;
                    (grandTotal[index as number] as Object[])[
                      (grandTotal[index as number] as Object[]).length + value
              ...
              if (Object.keys(grandTotal[index as number]).length > 0) {
                value =
                  (temporary[index as number] as Object[]).length === 0 ? 1 : 0;
                ...
                if (height1 > 2) {
                  ...
                  (temporary[index as number] as Object[])[
                    ...
                    (grandTotal[index as number] as Object[])[1] ||
                    (grandTotal[index as number] as Object[])[0];
                }
                arrange = {};
                arrangeHeaders = [];
                grandTotal[index as number] = [];
          ...
          if (Object.keys(arrange).length > 0) {
            ...
            const order: string =
              this.sortObject[header[k - 1].levelUniqueName] ||
              this.sortObject[header[k - 1].hierarchy];
            keys = this.sortColumnHeaders(
              arrange,
              arrangeHeaders,
              order,
            ...
            for (let len: number = 0; len < keys.length; len++) {
              const leng: number = Object.keys(
                arrange[keys[len as number]]
              ).length;
              for (let q: number = 0; q < leng; q++) {
                ...
                (temporary[index as number] as Object[])[
                  ...
                  arrange[keys[len as number]][
                    Number(
                      Object.keys(arrange[keys[len as number]])[q as number]
                    )
                  ];
              }
            }
          ...
          for (
            let m: number = 1;
            m < (temporary[index as number] as Object[]).length;
            m++
          ) {
            this.pivotValues[index as number][m as number] = (
              temporary[index as number] as Object[]
            )[m as number];
          }
          for (let n: number = j; n < this.pivotValues.length; n++) {
            const pElement: IPivotRows = extend(
              {},
              this.pivotValues[n + 1],
              null,
              true
            ) as IAxisSet[][];
            const cElement: IPivotRows = extend(
              {},
              this.pivotValues[n as number],
              null,
              true
            ) as IAxisSet[][];
            if (
              Object.keys(pElement).length === Object.keys(cElement).length &&
              Object.keys(pElement).length > 2
            ) {
              for (
                let o: number = 1;
                o < this.pivotValues[j as number].length;
                o++
              ) {
                if (
                  Object.keys(pElement).length > 0 &&
                  (cElement[o as number] as IAxisSet).colIndex !==
                    (pElement[o as number] as IAxisSet).colIndex
                ) {
                  this.pivotValues[n + 1][o as number] = pElement[
                    (cElement[o as number] as IAxisSet).colIndex
                  ] as IAxisSet;
                }
              }
  ...
  private frameUniqueName(
    ...
    currentTuple: ITupInfo
  ): string {
    const hasLastMeasure: boolean =
      uniqueName.indexOf(currentCell.actualText.toString() + '::[Measures]') >
      -1;
    uniqueName =
      uniqueName.substring(
        0,
        uniqueName.indexOf(currentCell.actualText.toString())
      ) + currentCell.actualText.toString();
    const measureAvail: boolean =
      uniqueName.split('::[').length <=
      this.getMeasurePosition(uniqueName, currentTuple.measurePosition);
    uniqueName =
      uniqueName +
      (hasLastMeasure || measureAvail ? '::' + currentTuple.measureName : '');
    return uniqueName;
  }
  private getMeasurePosition(
    ...
    for (let i: number = 0; i < collection.length; i++) {
      if (
        collection[i as number] &&
        collection[i as number].indexOf('Measures') > -1
      ) {
        position = i;
        break;
      }
  ...
  private sortRowHeaders(
    ...
    if (
      this.enableValueSorting &&
      this.olapValueAxis === 'column' &&
      valueSortData.isValueSorting &&
      !isNullOrUndefined(this.valueSortSettings)
    ) {
      ...
      for (let i: number = 0; i < headers.length; i++) {
        ...
        const valueElement: Element =
          valCollection[
            header.ordinal * valueSortData.columnLength +
              valueSortData.memberIndex
          ];
        const valueArray: HTMLCollectionOf<Element> = valueElement
          ? valueElement.getElementsByTagName('Value')
          : undefined;
        const value: number =
          valueElement && valueArray.length > 0
            ? Number(valueArray[0].textContent)
            : 0;
        aggreColl.push({ header: header, value: value });
      }
      headers = PivotUtil.getSortedValue(
        aggreColl,
        this.valueSortSettings.sortOrder
      );
    } else if (
      this.enableSort &&
      headers.length > 0 &&
      headers[0].memberType !== 3 &&
      !headers[0].isNamedSet
    ) {
      ...
      const fieldName: string =
        headers[0].actualText !== 'Grand Total'
          ? headers[0].hierarchy
          : headers[1].hierarchy;
      const membersInfo: string[] | number[] =
        this.fieldList[fieldName as string] &&
        this.fieldList[fieldName as string].membersOrder
          ? ([...this.fieldList[fieldName as string].membersOrder] as
              | string[]
              | number[])
          : [];
      const sortDetails: HeadersSortEventArgs = {
        ...
        sortOrder: (this.sortObject[headers[0].hierarchy] ||
          this.sortObject[headers[0].levelUniqueName]) as Sorting,
        members:
          membersInfo && membersInfo.length > 0 ? membersInfo : sortMembers,
      ...
      if (membersInfo && membersInfo.length > 0) {
        ...
        if (
          sortDetails.sortOrder === 'Ascending' ||
          sortDetails.sortOrder === undefined
        ) {
          headers.sort((a: IAxisSet, b: IAxisSet) =>
            a.formattedText > b.formattedText
              ? 1
              : b.formattedText > a.formattedText
              ? -1
              : 0
          );
  ...
  private sortColumnHeaders(
    ...
    if (
      ...
      for (let i: number = 0; i < keys.length; i++) {
        ...
        const header: IAxisSet = this.showSubTotalsAtTop
          ? childCollection[0]
          : childCollection[childCollection.length - 1];
        const valueElement: Element =
          valCollection[
            valueSortData.memberIndex * valueSortData.columnLength +
              header.ordinal
          ];
        const valueArray: HTMLCollectionOf<Element> = valueElement
          ? valueElement.getElementsByTagName('Value')
      ...
      let isHeaderSortByDefault: boolean = false;
      const membersInfo: string[] | number[] =
        this.fieldList[header as string] &&
        this.fieldList[header as string].membersOrder
          ? ([...this.fieldList[header as string].membersOrder] as
              | string[]
              | number[])
          : [];
      const sortDetails: HeadersSortEventArgs = {
  ...
  private applyCustomSort(
    headers: string[],
    sortDetails: HeadersSortEventArgs,
    hasMembersOrder?: boolean
  ): string[] {
    ...
    if (sortDetails.IsOrderChanged) {
      ...
      order =
        sortDetails.sortOrder === 'Ascending' ||
        sortDetails.sortOrder === 'None' ||
        sortDetails.sortOrder === undefined
          ? [].concat(sortDetails.members)
          : [].concat(sortDetails.members).reverse();
    }
  ...
  private frameSortObject(): void {
    if (this.enableSort) {
      for (let fPos: number = 0; fPos < this.sortSettings.length; fPos++) {
        this.sortObject[this.sortSettings[fPos as number].name] =
          this.sortSettings[fPos as number].order;
      }
    }
  }
  private getParentUname(
    uniqueNameColl: string,
    cell: IAxisSet,
    ...
    if (isMeasureAvail && !isLastMeasure) {
      ...
      for (let i: number = 0; i < uniqueNameColl.length; i++) {
        if (
          uniqueNameColl[i as number] === '~' ||
          uniqueNameColl[i as number] === ':'
        ) {
          sepPos.push(i);
          sepObjects[i as number] =
            uniqueNameColl[i as number] + uniqueNameColl[i as number];
          i++;
        }
      }
      if (
        this.getMeasurePosition(uniqueNameColl, tuple.measurePosition) >=
        uniqueNameColl.split('::[').length - 1
      ) {
        if (sepPos[sepPos.length - 2] > -1) {
          parentString =
            uniqueNameColl.substring(0, sepPos[sepPos.length - 2]) +
            sepObjects[sepPos[sepPos.length - 1]] +
            tuple.measureName;
        } else {
          parentString = 'parent';
        }
        ...
        const lastPosition: number =
          uniqueNameColl.lastIndexOf('~~') > uniqueNameColl.lastIndexOf('::[')
            ? uniqueNameColl.lastIndexOf('~~')
            : uniqueNameColl.lastIndexOf('::[');
        parentString =
          lastPosition > -1
            ? uniqueNameColl.substring(0, lastPosition)
            : 'parent';
      }
    } else {
      const lastPosition: number =
  ...
  private performColumnSpanning(): void {
    ...
    let measurePosition: number = this.tupColumnInfo[0]
      ? this.tupColumnInfo[0].measurePosition
      : this.measureIndex;
    ...
    while (rowPos > -1) {
      ...
      let colPos: number =
        (this.pivotValues[rowPos as number] as IAxisSet[]).length - 1;
      while (colPos > 0) {
        spanCollection[rowPos as number][colPos as number] = 1;
        const nextColCell: IAxisSet = (
          this.pivotValues[rowPos as number] as IAxisSet[]
        )[colPos + 1];
        const nextRowCell: IAxisSet =
          (this.pivotValues[rowPos + 1] as IAxisSet[]) &&
          this.rowStartPos - rowPos > 1
            ? (this.pivotValues[rowPos + 1] as IAxisSet[])[colPos as number]
            : undefined;
        const currCell: IAxisSet = (
          this.pivotValues[rowPos as number] as IAxisSet[]
        )[colPos as number];
        let colflag: boolean = false;
        let rowflag: boolean = false;
        const tupColInfo: ITupInfo = this.tupColumnInfo[currCell.ordinal];
        const isSubTot: boolean =
          tupColInfo.allStartPos >
          (tupColInfo.typeCollection[0] === '3' ? 1 : 0);
        const attrDrill: boolean = this.checkAttributeDrill(
          tupColInfo.drillInfo,
          'columns'
        );
        if (
          this.showSubTotalsAtTop &&
          ...
          currCell.ordinal = this.getOrdinal(
            currCell,
            this.pivotValues[rowPos + 1] as IAxisSet[]
          );
        }
        if (
          nextRowCell &&
          nextColCell &&
          (currCell.memberType === 2 || currCell.level === -1
            ? nextColCell.actualText === currCell.actualText
            : (currCell.memberType === 3 &&
                currCell.actualText === nextColCell.actualText) ||
          ...
          if (currCell.memberType === 2) {
            if (isSubTot ? nextColCell.type === 'sum' : true) {
              currCell.colSpan =
                nextColCell.colSpan + 1 >
                (measurePosition > rowPos ? colMeasureCount : 0)
                  ? 1
                  : nextColCell.colSpan + 1;
            } else {
              currCell.colSpan = 1;
        ...
        if (nextRowCell) {
          if (
            (currCell.memberType === 2 && nextRowCell.memberType === 2) ||
            nextRowCell.actualText === currCell.actualText
          ) {
            spanCollection[rowPos as number][colPos as number] = spanCollection[
              rowPos + 1
            ]
            ...
            if (
              rowPos === 0 ||
              (currCell.memberType === 1 &&
                currCell.level > -1 &&
                nextRowCell.memberType === 1 &&
                nextRowCell.level === -1)
            ) {
              currCell.rowSpan =
                currCell.isDrilled &&
                (this.fieldList[currCell.hierarchy] &&
                this.fieldList[currCell.hierarchy].isHierarchy
                  ? currCell.hasChild
                  : true)
                  ? 1
                  : spanCollection[rowPos + 1][colPos as number] + 1;
              nextRowCell.rowSpan =
                nextRowCell.isDrilled &&
                (this.fieldList[nextRowCell.hierarchy] &&
                this.fieldList[nextRowCell.hierarchy].isHierarchy
                  ? nextRowCell.hasChild
                  : true) &&
                nextRowCell.level === -1
                  ? spanCollection[rowPos + 1][colPos as number]
                  : nextRowCell.rowSpan;
            } else {
            ...
            currCell.rowSpan =
              rowPos === 0
                ? spanCollection[rowPos as number][colPos as number]
                : -1;
            nextRowCell.rowSpan =
              (nextRowCell.level > -1 &&
                !(
          ...
          currCell.rowSpan =
            currCell.level > -1 || this.rowStartPos === 1
              ? spanCollection[rowPos as number][colPos as number]
              : -1;
        }
        if (!colflag) {
  ...
  private getOrdinal(currCell: IAxisSet, nextRow: IAxisSet[]): number {
    ...
    for (
      let cellIndex: number = currCell.colIndex;
      cellIndex > 0;
      cellIndex--
    ) {
  ...
  private frameValues(
    ...
    if (this.customArgs.action === 'down') {
      ...
      startRowOrdinal = (
        this.onDemandDrillEngine[rowStartPos as number][0] as IAxisSet
      ).ordinal;
    }
    for (let rowPos: number = rowStartPos; rowPos < rowEndPos; rowPos++) {
      const columns: IAxisSet[] = this.pivotValues[
        rowPos as number
      ] as IAxisSet[];
      const rowOrdinal: number = columns[0].ordinal;
      for (
        let colPos: number = 1;
        colPos < (this.pivotValues[0] as IAxisSet[]).length;
        colPos++
      ) {
        if (
          (this.pivotValues[this.colDepth - 1] as IAxisSet[])[colPos as number]
        ) {
          const colOrdinal: number = (
            this.pivotValues[this.colDepth - 1] as IAxisSet[]
          )[colPos as number].ordinal;
          const lastColCell: IAxisSet = this.pivotValues[this.colDepth - 1][
            colPos as number
          ] as IAxisSet;
          const measure: string | number =
            columns[0].memberType === 3
              ? columns[0].actualText.toString()
              : this.tupColumnInfo[lastColCell.ordinal] &&
                this.tupColumnInfo[lastColCell.ordinal].measure
              ? this.tupColumnInfo[lastColCell.ordinal].measure.querySelector(
                  'UName'
                ).textContent
          ...
          if (columns[0].type === 'header') {
            ...
            let showTotals: boolean = true;
            const attrDrill: boolean =
              this.fieldList[columns[0].hierarchy] &&
              this.fieldList[columns[0].hierarchy].isHierarchy
                ? columns[0].isDrilled
                : true;
            ...
            const valElement: Element =
              valCollection[
                (rowOrdinal - startRowOrdinal) * colLength + colOrdinal
              ];
            formattedText =
              this.showSubTotalsAtBottom && columns[0].isDrilled
                ? ''
                : !showTotals && attrDrill
                ? ''
                : !isNullOrUndefined(valElement) &&
                  !isNullOrUndefined(valElement.querySelector('FmtValue'))
            ...
            value =
              !showTotals && attrDrill
                ? '0'
                : !isNullOrUndefined(valElement) &&
                  !isNullOrUndefined(valElement.querySelector('Value'))
                ? valElement.querySelector('Value').textContent
                : null;
            formattedText =
              this.showSubTotalsAtBottom && columns[0].isDrilled
                ? ''
                : showTotals && !isNullOrUndefined(value)
            ...
            const isSum: boolean =
              (this.tupColumnInfo[colOrdinal as number]
                ? this.tupColumnInfo[colOrdinal as number].allCount > 0 ||
                  this.tupColumnInfo[colOrdinal as number].drillStartPos > -1
                : true) ||
              (this.tupRowInfo[rowOrdinal as number]
                ? this.tupRowInfo[rowOrdinal as number].allCount > 0 ||
                  this.tupRowInfo[rowOrdinal as number].drillStartPos > -1
                : true);
            const isGrand: boolean =
              (this.tupRowInfo[rowOrdinal as number]
                ? this.tupRowInfo[rowOrdinal as number].measurePosition === 0
                  ? this.tupRowInfo[rowOrdinal as number].allStartPos === 1
                  : this.tupRowInfo[rowOrdinal as number].allStartPos === 0
                : false) ||
              (this.tupColumnInfo[colOrdinal as number]
                ? this.tupColumnInfo[colOrdinal as number].measurePosition === 0
                  ? this.tupColumnInfo[colOrdinal as number].allStartPos === 1
                  : this.tupColumnInfo[colOrdinal as number].allStartPos === 0
                : false);
            columns[colPos as number] = {
              axis: 'value',
              ...
              columnHeaders: this.tupColumnInfo[colOrdinal as number]
                ? this.tupColumnInfo[colOrdinal as number].captionCollection
                : '',
              rowHeaders: this.tupRowInfo[rowOrdinal as number]
                ? this.tupRowInfo[rowOrdinal as number].captionCollection
                : '',
          ...
          this.valueContent[rowPos - this.rowStartPos][colPos as number] =
            columns[colPos as number];
        }
      }
    }
  ...
  public getFormattedValue(
    ...
    if (this.formatFields[fieldName as string] && !isNullOrUndefined(value)) {
      const formatField: IFormatSettings = (<{ [key: string]: Object }>(
        this.formatFields[fieldName as string]
      )).properties
        ? (<{ [key: string]: Object }>this.formatFields[fieldName as string])
            .properties
        : this.formatFields[fieldName as string];
      const formatObj: IFormatSettings = extend(
        {},
        formatField,
        null,
        true
      ...
      if (
        !formatObj.minimumSignificantDigits &&
        formatObj.minimumSignificantDigits < 1
      ) {
        delete formatObj.minimumSignificantDigits;
      }
      if (
        !formatObj.maximumSignificantDigits &&
        formatObj.maximumSignificantDigits < 1
      ) {
        delete formatObj.maximumSignificantDigits;
      }
      if (formatObj.type) {
        formattedValue = this.globalize.formatDate(
  ...
  private getMeasureInfo(): IMeasureInfo {
    ...
    for (const field of this.values) {
      values[values.length] = field.isCalculatedField
        ? this.fieldList[field.name].tag
        : field.name;
    }
    if (values.length > 0) {
      if (this.isMeasureAvail) {
        ...
        if (!isAvail) {
          for (
            let i: number = 0, cnt: number = this.columns.length;
            i < cnt;
            i++
          ) {
        ...
        mIndex =
          mAxis === 'row' ? this.rows.length - 1 : this.columns.length - 1;
      }
  ...
  private frameMeasureOrder(
    ...
    if (measureAxis === axis && values.length > 0) {
      ...
      for (let j: number = 0, lnt: number = tuples.length; j < lnt; j++) {
        ...
        const members: HTMLElement[] = [].slice.call(
        ...
        while (i < members.length) {
          level =
            level +
            (level !== '' ? '~~' : '') +
            members[i as number].querySelector('UName').textContent;
        ...
        if (levelCollection[cLevel as string]) {
          levelCollection[cLevel as string][
            levelCollection[cLevel as string].length
          ] = level;
        } else {
              ...
              for (const key of uniqueLevels) {
                if (key === name) {
                  for (const level of levelCollection[key as string]) {
                    orderedIndex[orderedIndex.length] =
                      levels[level as string].index;
                    orderedTuples[orderedTuples.length] =
                      levels[level as string].node;
                  }
                }
              }
            }
      ...
      if (vTuples.length > 0) {
        ...
        for (let j: number = 0, cnt: number = valuesCount; j < cnt; j++) {
          ...
          if (vTuples[vOrdinalIndexPos as number]) {
            vTupleOrdinal = this.olapVirtualization
              ? j
              : Number(
                  vTuples[vOrdinalIndexPos as number].getAttribute(
                    'CellOrdinal'
                  )
                );
          } else {
          ...
          if (vTupleOrdinal === j) {
            ...
            clonedValTuple[vTupleOrdinal as number] =
              vTuples[vOrdinalIndexPos as number];
            vOrdinalIndexPos++;
          }
        }
        ...
        if (measureAxis === 'column') {
          if (
            valueIndex.length > 0 &&
            valueIndex[0].length === orderedIndex.length
          ) {
            for (
              let i: number = 0, cnt: number = orderedIndex.length;
              i < cnt;
              i++
            ) {
              let j: number = 0;
              while (j < valueIndex.length) {
                ...
                if (
                  !isNullOrUndefined(valueIndex[j as number]) &&
                  !isNullOrUndefined(orderedIndex[i as number])
                ) {
                  const ordinalValue: string = vOrdinalIndex[index as number]
                    ? vOrdinalIndex[index as number].toString()
                    : index.toString();
                  const tuple: Element =
                    vTuples[
                      Number(valueIndex[j as number][orderedIndex[i as number]])
                    ];
                  if (tuple) {
          ...
          if (valueIndex.length === orderedIndex.length) {
            for (
              ...
              while (j < valueIndex[orderedIndex[i as number]].length) {
                ...
                const tuple: Element =
                  vTuples[
                    Number(valueIndex[orderedIndex[i as number]][j as number])
                  ];
                if (tuple) {
                  tuple.setAttribute('CellOrdinal', ordinalValue.toString());
  ...
  public getDrilledSets(
    ...
    const tupCollection: ITupInfo[] =
      axis === 'row' ? this.tupRowInfo : this.tupColumnInfo;
    ...
    let tupPos: number = 0;
    const isWithoutAllMember: boolean =
      tupCollection[0].typeCollection[fieldPos as number] === '1';
    while (tupPos < tupCollection.length) {
      if (
        isNullOrUndefined(tupCollection[tupPos as number].allStartPos) ||
    ...
    const memberArray: string[] = uNameCollection
      .split('::[')
      .map((item: string) => {
        return item[0] === '[' ? item : '[' + item;
      });
      ...
      if (memberArray[memPos as number]) {
        if ((isWithoutAllMember || this.isPaging) && memPos === fieldPos) {
          const splitLevels: string[] =
            memberArray[memPos as number].split('~~');
          ...
          joinArray[joinArray.length] =
            cropLevels.length > 0
              ? cropLevels.join('~~')
              : memberArray[memPos as number];
        } else {
    ...
    for (const item of levels) {
      if (item.indexOf(uNameCollection) === 0) {
        ...
        if (this.isPaging) {
          ...
          for (
            let fPos: number = drillFieldSep.indexOf(memberName);
            fPos < drillFieldSep.length;
            fPos++
          ) {
            memberObj[drillFieldSep[fPos as number]] =
              drillFieldSep[fPos as number];
          }
        }
      }
    }
    if (this.isPaging) {
      const fieldSep: string[] = currTuple.uNameCollection
        .split('::[')
        .map((item: string) => {
          return item[0] === '[' ? item : '[' + item;
        });
  ...
  public updateCalcFields(
    dataSourceSettings: IDataOptions,
    lastcalcInfo: ICalculatedFieldSettings
  ): void {
    this.dataSourceSettings = dataSourceSettings;
    this.calculatedFieldSettings = dataSourceSettings.calculatedFieldSettings
      ? dataSourceSettings.calculatedFieldSettings
  ...
  private updateFieldlist(isInit?: boolean): void {
    ...
    while (i < this.savedFieldListData.length) {
      ...
      if (this.savedFieldList[fieldName as string]) {
        const sortOrder: string = this.enableSort
          ? this.sortObject[fieldName as string]
            ? this.sortObject[fieldName as string]
            : 'Ascending'
          : 'None';
      ...
      if (
        this.dataFields[fieldName as string] &&
        this.savedFieldList[fieldName as string] &&
        this.selectedItems.indexOf(fieldName) > -1
      ) {
        this.savedFieldList[fieldName as string].isSelected = true;
        this.savedFieldList[fieldName as string].allowDragAndDrop = this
          .dataFields[fieldName as string]
          ? this.dataFields[fieldName as string].allowDragAndDrop
          : true;
        this.savedFieldList[fieldName as string].showFilterIcon = this
          .dataFields[fieldName as string]
          ? this.dataFields[fieldName as string].showFilterIcon
          : true;
        this.savedFieldList[fieldName as string].showSortIcon = this.dataFields[
          fieldName as string
        ]
          ? this.dataFields[fieldName as string].showSortIcon
          : true;
        this.savedFieldList[fieldName as string].showEditIcon = this.dataFields[
          fieldName as string
        ]
          ? this.dataFields[fieldName as string].showEditIcon
          : true;
        this.savedFieldList[fieldName as string].showRemoveIcon = this
          .dataFields[fieldName as string]
          ? this.dataFields[fieldName as string].showRemoveIcon
          : true;
        this.savedFieldList[fieldName as string].showValueTypeIcon = this
          .dataFields[fieldName as string]
          ? this.dataFields[fieldName as string].showValueTypeIcon
          : true;
        this.savedFieldList[fieldName as string].showSubTotals = this
          .dataFields[fieldName as string]
          ? this.dataFields[fieldName as string].showSubTotals
          : true;
        this.savedFieldListData[i as number].isSelected = true;
        this.savedFieldListData[i as number].allowDragAndDrop = this.dataFields[
          fieldName as string
        ]
          ? this.dataFields[fieldName as string].allowDragAndDrop
          : true;
        this.savedFieldListData[i as number].showFilterIcon = this.dataFields[
          fieldName as string
        ]
          ? this.dataFields[fieldName as string].showFilterIcon
          : true;
        this.savedFieldListData[i as number].showSortIcon = this.dataFields[
          fieldName as string
        ]
          ? this.dataFields[fieldName as string].showSortIcon
          : true;
        this.savedFieldListData[i as number].showEditIcon = this.dataFields[
          fieldName as string
        ]
          ? this.dataFields[fieldName as string].showEditIcon
          : true;
        this.savedFieldListData[i as number].showRemoveIcon = this.dataFields[
          fieldName as string
        ]
          ? this.dataFields[fieldName as string].showRemoveIcon
          : true;
        this.savedFieldListData[i as number].showValueTypeIcon = this
          .dataFields[fieldName as string]
          ? this.dataFields[fieldName as string].showValueTypeIcon
          : true;
        this.savedFieldListData[i as number].showSubTotals = this.dataFields[
          fieldName as string
        ]
          ? this.dataFields[fieldName as string].showSubTotals
          : true;
        ...
        if (
          this.dataFields[parentID as string] &&
          this.savedFieldList[parentID as string] &&
      ...
      if (
        (this.savedFieldList[fieldName as string] &&
          this.savedFieldList[fieldName as string].isCalculatedField) ||
        ...
        for (const field of this.calculatedFieldSettings) {
          if (fieldName === field.name) {
            ...
            this.savedFieldListData[i as number].parentHierarchy =
            ...
            this.savedFieldList[fieldName as string].formatString =
              formatString;
            this.savedFieldList[fieldName as string].parentHierarchy =
              this.savedFieldListData[i as number].parentHierarchy;
            isAvail = true;
          }
        }
  ...
  private getCubes(dataSourceSettings: IDataOptions): void {
    const connectionString: ConnectionInfo = this.getConnectionInfo(
      ...
      dataSourceSettings.localeIdentifier.toString()
    );
    const soapMessage: string =
      '<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/"><Header/><Body>' +
      '<Discover xmlns="urn:schemas-microsoft-com:xml-analysis"><RequestType>MDSCHEMA_CUBES</RequestType>' +
      '<Restrictions><RestrictionList><CATALOG_NAME>' +
      dataSourceSettings.catalog +
      '</CATALOG_NAME></RestrictionList></Restrictions><Properties><PropertyList><Catalog>' +
      dataSourceSettings.catalog +
      '</Catalog> <LocaleIdentifier>' +
      connectionString.LCID +
      '</LocaleIdentifier>' +
      (dataSourceSettings.roles
        ? '<Roles>' + dataSourceSettings.roles + '</Roles>'
        : '') +
    ...
    this.doAjaxPost(
      ...
      soapMessage,
      this.validateCube.bind(this),
      { dataSourceSettings: dataSourceSettings, action: 'getCubes' }
    );
    if (this.errorInfo) {
  ...
  public getTreeData(
    ...
    const soapMessage: string =
      '<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/"><Header/><Body><Discover xmlns="urn:schemas-microsoft-com:xml-analysis"><RequestType>' +
      args.request +
      '</RequestType><Restrictions><RestrictionList><CATALOG_NAME>' +
      args.catalog +
      '</CATALOG_NAME><CUBE_NAME>' +
      args.cube +
      '</CUBE_NAME></RestrictionList></Restrictions><Properties><PropertyList><Catalog>' +
      args.catalog +
      '</Catalog> <LocaleIdentifier>' +
      connectionString.LCID +
      '</LocaleIdentifier>' +
      (args.roles ? '<Roles>' + args.roles + '</Roles>' : '') +
  ...
  private getAxisFields(): void {
    this.rows = this.dataSourceSettings.rows
      ? this.dataSourceSettings.rows
      : [];
    this.columns = this.dataSourceSettings.columns
      ? this.dataSourceSettings.columns
      : [];
    this.filters = this.dataSourceSettings.filters
      ? this.dataSourceSettings.filters
      : [];
    this.values = this.dataSourceSettings.values
      ? this.dataSourceSettings.values
      : [];
    let dataFields: IFieldOptions[] = extend(
      [],
      this.rows,
      null,
      true
    ...
    while (len--) {
      this.dataFields[dataFields[len as number].name] =
        dataFields[len as number];
      ...
      if (
        ...
        if (isMembersAvail) {
          ...
          while (i < selectedElements.length) {
            ...
            parentNodes = this.getParentNode(
              selectedElements[i as number],
              members,
              parentNodes
            );
        ...
        for (const selectedElement of filter.items) {
          ...
          if (isMembersAvail) {
            this.fieldList[filter.name].filter.push(
              members[selectedElement as string].caption
            );
          } else {
            this.fieldList[filter.name].filter.push(
              filterCaption ? filterCaption : selectedElement
            );
          }
        }
        ...
        (this.allowValueFilter || this.allowLabelFilter) &&
        ['Date', 'Label', 'Number', 'Value'].indexOf(filter.type) !== -1
      ) {
        for (const item of dataFields) {
          if (item.name === filter.name) {
            const filterMembers: IFilter[] = this.filterMembers[
              filter.name
            ] as IFilter[];
            if (
              filterMembers &&
              typeof filterMembers[0] === 'object' &&
              filterMembers[0].type === filter.type
            ) {
  ...
  private getParentNode(
    ...
    if (
      members[name as string].parent &&
      name !== members[name as string].parent
    ) {
  ...
  public getDrillThroughData(pivotValue: IAxisSet, maxRows: number): void {
    const column: string[] =
      this.tupColumnInfo[pivotValue.colOrdinal] &&
      ...
      this.tupColumnInfo[pivotValue.colOrdinal].uNameCollection !== ''
        ...
        : [];
    const row: string[] =
      this.tupRowInfo[pivotValue.rowOrdinal] &&
    ...
    for (let i: number = 0; i < this.filters.length; i++) {
      ...
      if (filters) {
        for (let j: number = 0; j < filters.length; j++) {
          ...
          filterQuery =
            j < filters.length - 1 ? filterQuery + ',' : filterQuery + '';
        }
    ...
    for (let i: number = 0; i < filteritems.length; i++) {
      filterQuery =
        filterQuery === ''
          ? '{' + filteritems[i as number] + '}'
          : filterQuery + ',' + '{' + filteritems[i as number] + '}';
    }
    let columnQuery: string = '';
    let rowQuery: string = '';
    for (let i: number = 0; i < column.length; i++) {
      columnQuery =
        (columnQuery.length > 0 ? columnQuery + ',' : '') +
        (column[i as number].split('~~').length > 1
          ? column[i as number].split('~~')[
              column[i as number].split('~~').length - 1
            ]
          : column[i as number]);
    }
    for (let i: number = 0; i < row.length; i++) {
      rowQuery =
        (rowQuery.length > 0 ? rowQuery + ',' : '') +
        (row[i as number].split('~~').length > 1
          ? row[i as number].split('~~')[
              row[i as number].split('~~').length - 1
            ]
          : row[i as number]);
    }
    let drillQuery: string =
      ...
      ' Select(' +
      (columnQuery.length > 0 ? columnQuery : '') +
      (columnQuery.length > 0 && rowQuery.length > 0 ? ',' : '') +
      (rowQuery.length > 0 ? rowQuery : '') +
      ') on 0 from ' +
      (filterQuery === ''
    ...
    const connectionString: ConnectionInfo = this.getConnectionInfo(
      this.dataSourceSettings.url,
      this.dataSourceSettings.localeIdentifier
    );
    this.doAjaxPost(
      'POST',
      connectionString.url,
      xmla,
      this.drillThroughSuccess.bind(this),
    ...
    if (tag.length > 0) {
      ...
      while (i < tag.length) {
        ...
        while (j < child.length) {
          json.push(
            '"' +
              child[j as number].tagName +
              '"' +
              ':' +
              '"' +
              child[j as number].textContent +
              '"'
          );
          j++;
        }
      ...
      while (k < json.length) {
        if (
          Object.keys(JSON.parse('[{' + json[k as number] + '}]')[0])[0] ===
          Object.keys(JSON.parse('[{' + value + '}]')[0])[0]
        ) {
          gridJSON +=
            gridJSON === ''
              ? '[{' + json[k as number]
      ...
      const tag: HTMLElement[] = [].slice.call(
        xmlDoc.querySelectorAll('faultstring')
      );
      let i: number = 0;
      while (i < tag.length) {
  ...
  public getFilterMembers(
    dataSourceSettings: IDataOptions,
    fieldName: string,
    levelCount: number,
    isSearchFilter?: boolean,
    loadLevelMember?: boolean
  ): string {
    ...
    if (loadLevelMember) {
      filterQuery =
        'Descendants({' +
        levels[cLevel as number].id +
        '}, ' +
        levels[levelCount - 1].id +
        ', ' +
        (levelCount - cLevel === 1 ? 'SELF)' : 'SELF_AND_BEFORE)');
    } else {
      filterQuery =
        fieldName +
        ', Descendants({' +
  ...
  public getMembers(
    dataSourceSettings: IDataOptions,
    fieldName: string,
    isAllFilterData?: boolean,
    filterParentQuery?: string,
    loadLevelMember?: boolean,
    filterItemName?: string
  ): void {
    // dimProp = "dimension properties CHILDREN_CARDINALITY, MEMBER_TYPE";
    const dimProp: string =
      'DIMENSION PROPERTIES PARENT_UNIQUE_NAME, HIERARCHY_UNIQUE_NAME, CHILDREN_CARDINALITY, MEMBER_TYPE, MEMBER_VALUE';
    let mdxQuery: string;
    const hasAllMember: boolean =
      this.fieldList[fieldName as string].hasAllMember;
    const hierarchy: string = (
      hasAllMember ? fieldName : fieldName + '.LEVELS(0)'
    )
      .replace(/&/g, '&amp;')
      .replace(/>/g, '&gt;')
      .replace(/</g, '&lt;')
    ...
    const connectionString: ConnectionInfo = this.getConnectionInfo(
      dataSourceSettings.url,
      dataSourceSettings.localeIdentifier
    );
    if (!loadLevelMember) {
    ...
    this.doAjaxPost(
      ...
      xmla,
      filterItemName
        ? this.getOlapFilterText.bind(this)
        : this.generateMembers.bind(this),
      {
        dataSourceSettings: dataSourceSettings,
        fieldName: fieldName,
  ...
  private getOlapFilterText(
    ...
    const fields: HTMLElement[] = [].slice.call(
      xmlDoc.querySelectorAll('Axis[name="Axis0"] Tuple')
    );
    if (
      fields.length > 0 &&
      this.fieldList[customArgs.fieldName] &&
      fields[fields.length - 1].getElementsByTagName('Caption') &&
      fields[fields.length - 1].getElementsByTagName('Caption')[0]
    ) {
      this.fieldList[customArgs.fieldName].actualFilter[0] =
        fields[fields.length - 1].getElementsByTagName(
          'Caption'
        )[0].textContent;
    }
  }
  public getChildMembers(
    dataSourceSettings: IDataOptions,
    memberUQName: string,
    fieldName: string
  ): void {
  ...
  public getSearchMembers(
    ...
    if (searchString !== '') {
      ...
      const dimProp: string =
        'DIMENSION PROPERTIES PARENT_UNIQUE_NAME, HIERARCHY_UNIQUE_NAME, CHILDREN_CARDINALITY, MEMBER_TYPE, MEMBER_VALUE';
      const hierarchy: string = fieldName
        .replace(/&/g, '&amp;')
        .replace(/>/g, '&gt;')
        .replace(/</g, '&lt;')
        .replace(/'/g, '&apos;')
        .replace(/"/g, '&quot;');
      const mdxQuery: string =
        'WITH SET [SearchMembersSet] AS &#39;FILTER(' +
        (isAllFilterData
          ? hierarchy + '.ALLMEMBERS, '
          : '{' +
            (levelCount > 1
  ...
  private generateMembers(
    ...
    for (const field of fields) {
      ...
      const parentUqName: string = member.querySelector('PARENT_UNIQUE_NAME')
        ? member.querySelector('PARENT_UNIQUE_NAME').textContent
        : '';
      const nodeAttr: { [key: string]: string } = {
      ...
      if (parentUqName === '' && memberType === '1') {
        filterMembers = {
          hasChildren: field.querySelector('CHILDREN_CARDINALITY')
            ? field.querySelector('CHILDREN_CARDINALITY').textContent !== '0'
              ? true
              : false
            : false,
          isSelected: false,
        ...
        if (
          customArgs.action === 'fetchMembers' ||
          customArgs.action === 'fetchChildMembers'
        ) {
        ...
        if (parentUqName === allMember && memberType === '1') {
          ...
          if (
            ...
            this.fieldList[fieldName as string].filterMembers.push(
              filterMembers
            );
            this.fieldList[fieldName as string].childMembers.push(
              filterMembers
            );
            this.fieldList[fieldName as string].members[
              memberUqName as string
            ...
            filterMembers.expanded = true;
            this.fieldList[fieldName as string].searchMembers.push(
              filterMembers
            );
            this.fieldList[fieldName as string].currrentMembers[
              memberUqName as string
  ...
  private getFieldListItems(
    ...
    const fields: HTMLElement[] = [].slice.call(xmlDoc.querySelectorAll('row'));
    for (const field of fields) {
      const isAllMemberAvail: boolean =
        [].slice.call(field.querySelectorAll('ALL_MEMBER')).length > 0;
      const dimensionName: string = field.querySelector(
        'DIMENSION_UNIQUE_NAME'
      ).textContent;
      const hierarchyName: string = field.querySelector(
        'HIERARCHY_UNIQUE_NAME'
      ).textContent;
      const isSameDim: boolean =
        dimensionName === hierarchyName &&
        dimensionName.toLowerCase() !== '[measures]' &&
        hierarchyName.toLowerCase() !== '[measures]';
      hierarchyElements.push({
        pid:
          this.isMondrian || isSameDim
            ? dimensionName + '~#^Dim'
            : dimensionName,
        id: hierarchyName,
        ...
        allMember: isAllMemberAvail
          ? field.querySelectorAll('ALL_MEMBER')[0].textContent
          : undefined,
  ...
  private loadCalculatedMemberElements(
    calcMembers: ICalculatedFieldSettings[]
  ): void {
    if (calcMembers.length > 0) {
      ...
      if (
        ...
        fieldListElements[0].id.toLowerCase() === '[measures]'
      ) {
      ...
      for (const field of calcMembers) {
        if (!this.fieldList[field.name]) {
          ...
          const caption: string =
            this.dataFields[field.name] && this.dataFields[field.name].caption
              ? this.dataFields[field.name].caption
              : this.mappingFields[field.name] &&
                this.mappingFields[field.name].caption
              ? this.mappingFields[field.name].caption
              : field.name;
          const formatString: string = field.formatString;
          const calcField: IOlapField = {
            ...
            allowDragAndDrop: this.dataFields[field.name]
              ? this.dataFields[field.name].allowDragAndDrop
              : this.mappingFields[field.name]
              ? this.mappingFields[field.name].allowDragAndDrop
              : true,
            showFilterIcon: this.dataFields[field.name]
              ? this.dataFields[field.name].showFilterIcon
              : this.mappingFields[field.name]
              ? this.mappingFields[field.name].showFilterIcon
              : true,
            showSortIcon: this.dataFields[field.name]
              ? this.dataFields[field.name].showSortIcon
              : this.mappingFields[field.name]
              ? this.mappingFields[field.name].showSortIcon
              : true,
            showEditIcon: this.dataFields[field.name]
              ? this.dataFields[field.name].showEditIcon
              : this.mappingFields[field.name]
              ? this.mappingFields[field.name].showEditIcon
              : true,
            showRemoveIcon: this.dataFields[field.name]
              ? this.dataFields[field.name].showRemoveIcon
              : this.mappingFields[field.name]
              ? this.mappingFields[field.name].showRemoveIcon
              : true,
            showValueTypeIcon: this.dataFields[field.name]
              ? this.dataFields[field.name].showValueTypeIcon
              : this.mappingFields[field.name]
              ? this.mappingFields[field.name].showValueTypeIcon
              : true,
            showSubTotals: this.dataFields[field.name]
              ? this.dataFields[field.name].showSubTotals
              : this.mappingFields[field.name]
              ? this.mappingFields[field.name].showSubTotals
              : true,
            fieldType:
              expression.toLowerCase().indexOf('measure') > -1
                ? 'Measure'
  ...
  private loadDimensionElements(
    ...
    for (const field of fields) {
      ...
      const defaultHierarchy: string =
        field.querySelector('DEFAULT_HIERARCHY').textContent;
      const dimensionCaption: string =
        field.querySelector('DIMENSION_CAPTION').textContent;
      ...
      if (dimensionName.toLowerCase().indexOf('[measure') >= 0) {
        measure = {
          ...
          spriteCssClass:
            dimensionName.toLowerCase() === '[measures]'
              ? 'e-measureGroupCDB-icon' + ' ' + cls.ICON
              : 'e-dimensionCDB-icon' + ' ' + cls.ICON,
          tag: dimensionName,
          // aggregateType: this.getAggregateType(dimensionName),
          type: 'string',
        };
      } else if (
        isNullOrUndefined(fields[0].querySelector('HIERARCHY_CAPTION'))
      ) {
        hierarchyElements.push({
          hasChildren: true,
          isSelected: false,
  ...
  private loadNamedSetElements(
    ...
    for (const field of fields) {
      if (
        !(
          measureGroupItems.indexOf(
            field.querySelector('DIMENSIONS').textContent.split('.')[0]
          ) >= 0
        )
      ) {
        dimensionElements.push({
          ...
          id:
            field.querySelector('SET_DISPLAY_FOLDER').textContent +
            '_' +
            field.querySelector('DIMENSIONS').textContent.split('.')[0],
          name: field.querySelector('SET_DISPLAY_FOLDER').textContent,
          spriteCssClass:
            'e-folderCDB-icon' + ' ' + cls.ICON + ' ' + 'namedSets',
        ...
        measureGroupItems.push(
          field.querySelector('DIMENSIONS').textContent.split('.')[0]
        );
      }
      const id: string =
        '[' + field.querySelector('SET_NAME').textContent.trim() + ']';
      const fieldObj: IOlapField = {
        hasChildren: true,
        isNamedSets: true,
        isSelected:
          reportElement.indexOf(
            '[' + field.querySelector('SET_NAME').textContent + ']'
          ) >= 0,
        pid:
          field.querySelector('SET_DISPLAY_FOLDER').textContent +
          '_' +
        ...
        caption:
          this.dataFields[id as string] && this.dataFields[id as string].caption
            ? this.dataFields[id as string].caption
            : this.mappingFields[id as string] &&
              this.mappingFields[id as string].caption
            ? this.mappingFields[id as string].caption
            : field.querySelector('SET_CAPTION').textContent,
        ...
        isHierarchy: true,
        isExcelFilter: false,
        allowDragAndDrop: this.dataFields[id as string]
          ? this.dataFields[id as string].allowDragAndDrop
          : this.mappingFields[id as string]
          ? this.mappingFields[id as string].allowDragAndDrop
          : true,
        showFilterIcon: this.dataFields[id as string]
          ? this.dataFields[id as string].showFilterIcon
          : this.mappingFields[id as string]
          ? this.mappingFields[id as string].showFilterIcon
          : true,
        showSortIcon: this.dataFields[id as string]
          ? this.dataFields[id as string].showSortIcon
          : this.mappingFields[id as string]
          ? this.mappingFields[id as string].showSortIcon
          : true,
        showEditIcon: this.dataFields[id as string]
          ? this.dataFields[id as string].showEditIcon
          : this.mappingFields[id as string]
          ? this.mappingFields[id as string].showEditIcon
          : true,
        showRemoveIcon: this.dataFields[id as string]
          ? this.dataFields[id as string].showRemoveIcon
          : this.mappingFields[id as string]
          ? this.mappingFields[id as string].showRemoveIcon
          : true,
        showValueTypeIcon: this.dataFields[id as string]
          ? this.dataFields[id as string].showValueTypeIcon
          : this.mappingFields[id as string]
          ? this.mappingFields[id as string].showValueTypeIcon
          : true,
        showSubTotals: this.dataFields[id as string]
          ? this.dataFields[id as string].showSubTotals
          : this.mappingFields[id as string]
          ? this.mappingFields[id as string].showSubTotals
          : true,
      };
  ...
  private loadHierarchyElements(customArgs: FieldData): void {
    ...
    for (const field of fields) {
      ...
      let allMember: string;
      if (isAllMemberAvail) {
        const stringCollection: string[] = field
          .querySelectorAll('ALL_MEMBER')[0]
      ...
      const hierarchyFolderName: string = field.querySelector(
        'HIERARCHY_DISPLAY_FOLDER'
      )
        ? field.querySelector('HIERARCHY_DISPLAY_FOLDER').textContent
        : '';
      ...
      if (curElement.length > 0) {
        ...
        if (
          ...
          let parentID: string =
            dimensionName + (this.isMondrian || isSameDim ? '~#^Dim' : '');
          if (hierarchyFolderName !== '') {
            const folderName: string =
              dimensionName +
              (this.isMondrian || isSameDim ? '~#^Dim' : '') +
              '_' +
              hierarchyFolderName;
          ...
          const fieldObj: IOlapField = {
            hasChildren: field.querySelector('HIERARCHY_ORIGIN')
              ? field.querySelector('HIERARCHY_ORIGIN').textContent !== '2' &&
                field.querySelector('HIERARCHY_ORIGIN').textContent !== '6'
                ? true
                : false
              : true,
            // hasChildren: true,
            ...
            name: field.querySelector('HIERARCHY_CAPTION').textContent,
            spriteCssClass:
              (field.querySelector('HIERARCHY_ORIGIN')
                ? field.querySelector('HIERARCHY_ORIGIN').textContent !== '2' &&
                  field.querySelector('HIERARCHY_ORIGIN').textContent !== '6'
                  ? 'e-hierarchyCDB-icon'
                  : 'e-attributeCDB-icon'
            ...
            tag: hierarchyName,
            caption:
              this.dataFields[hierarchyName as string] &&
              this.dataFields[hierarchyName as string].caption
                ? this.dataFields[hierarchyName as string].caption
                : this.mappingFields[hierarchyName as string] &&
                  this.mappingFields[hierarchyName as string].caption
                ? this.mappingFields[hierarchyName as string].caption
                : field.querySelector('HIERARCHY_CAPTION').textContent,
            // aggregateType: this.getAggregateType(hierarchyName),
            type: 'string',
            filter: [],
            dateMember: [],
            sort: this.enableSort
              ? this.sortObject[hierarchyName as string]
                ? this.sortObject[hierarchyName as string]
                : 'Ascending'
              : 'None',
            actualFilter: [],
            ...
            levels: [],
            levelCount: 1,
            isHierarchy: field.querySelector('HIERARCHY_ORIGIN')
              ? field.querySelector('HIERARCHY_ORIGIN').textContent !== '2' &&
                field.querySelector('HIERARCHY_ORIGIN').textContent !== '6'
                ? false
                : true
            ...
            allowDragAndDrop: this.dataFields[hierarchyName as string]
              ? this.dataFields[hierarchyName as string].allowDragAndDrop
              : this.mappingFields[hierarchyName as string]
              ? this.mappingFields[hierarchyName as string].allowDragAndDrop
              : true,
            showFilterIcon: this.dataFields[hierarchyName as string]
              ? this.dataFields[hierarchyName as string].showFilterIcon
              : this.mappingFields[hierarchyName as string]
              ? this.mappingFields[hierarchyName as string].showFilterIcon
              : true,
            showSortIcon: this.dataFields[hierarchyName as string]
              ? this.dataFields[hierarchyName as string].showSortIcon
              : this.mappingFields[hierarchyName as string]
              ? this.mappingFields[hierarchyName as string].showSortIcon
              : true,
            showEditIcon: this.dataFields[hierarchyName as string]
              ? this.dataFields[hierarchyName as string].showEditIcon
              : this.mappingFields[hierarchyName as string]
              ? this.mappingFields[hierarchyName as string].showEditIcon
              : true,
            showRemoveIcon: this.dataFields[hierarchyName as string]
              ? this.dataFields[hierarchyName as string].showRemoveIcon
              : this.mappingFields[hierarchyName as string]
              ? this.mappingFields[hierarchyName as string].showRemoveIcon
              : true,
            showValueTypeIcon: this.dataFields[hierarchyName as string]
              ? this.dataFields[hierarchyName as string].showValueTypeIcon
              : this.mappingFields[hierarchyName as string]
              ? this.mappingFields[hierarchyName as string].showValueTypeIcon
              : true,
            showSubTotals: this.dataFields[hierarchyName as string]
              ? this.dataFields[hierarchyName as string].showSubTotals
              : this.mappingFields[hierarchyName as string]
              ? this.mappingFields[hierarchyName as string].showSubTotals
              : true,
          };
          dimensionElements.push(fieldObj);
  ...
  private loadLevelElements(
    ...
    for (const field of fields) {
      if (
        parseInt(field.querySelector('LEVEL_TYPE').textContent, 10) !== 1 &&
        field
          .querySelector('HIERARCHY_UNIQUE_NAME')
          .textContent.toLowerCase() !== '[measures]'
      ) {
        const dimensionName: string = field.querySelector(
          'HIERARCHY_UNIQUE_NAME'
        ).textContent;
        const levelName: string =
          field.querySelector('LEVEL_UNIQUE_NAME').textContent;
        const levelCaption: string =
          field.querySelector('LEVEL_CAPTION').textContent;
        const levelObj: IOlapField = {
          ...
          tag: levelName,
          spriteCssClass:
            'e-level-members e-hierarchy-level-' +
            parseInt(field.querySelector('LEVEL_NUMBER').textContent, 10) +
            '-icon' +
            ' ' +
            cls.ICON,
        ...
        if (
          this.fieldList[dimensionName as string] &&
          this.fieldList[dimensionName as string].spriteCssClass &&
          this.fieldList[dimensionName as string].spriteCssClass.indexOf(
            'e-attributeCDB-icon'
          ) === -1
        ) {
    ...
    this.fieldListData = dimensionElements =
      dimensionElements.concat(newDataSource);
    const args: ConnectionInfo = {
      catalog: customArgs.dataSourceSettings.catalog,
      cube: customArgs.dataSourceSettings.cube,
      url: customArgs.dataSourceSettings.url,
  ...
  private loadMeasureElements(
    ...
    const measureGroupItems: string[] = [];
    let caption: string;
    const dataFields: IFieldOptions[] = extend(
      [],
      this.values,
      null,
      true
    ...
    const fields: HTMLElement[] = [].slice.call(xmlDoc.querySelectorAll('row'));
    for (const field of fields) {
      const measureGRPName: string = isNullOrUndefined(
        field.querySelector('MEASUREGROUP_NAME')
      )
        ? ''
        : field.querySelector('MEASUREGROUP_NAME').textContent;
      const measureName: string = field.querySelector(
        'MEASURE_UNIQUE_NAME'
      ).textContent;
      ...
      const aggregateType: string = field.querySelector('MEASURE_AGGREGATOR')
        ? field.querySelector('MEASURE_AGGREGATOR').textContent
        : '1';
      if (!(measureGroupItems.indexOf(measureGRPName) >= 0)) {
        if (this.locale !== 'en-US') {
          ...
          for (const item of this.fieldListObj.measuresGroups) {
            if (
              item.querySelector('MEASUREGROUP_NAME').textContent ===
              measureGRPName
            ) {
          ...
          caption =
            measureInfo.length > 0
              ? measureInfo[0].querySelector('MEASUREGROUP_CAPTION').textContent
              : measureGRPName;
        } else {
      ...
      const fieldObj: IOlapField = {
        ...
        tag: measureName,
        caption:
          this.dataFields[measureName as string] &&
          this.dataFields[measureName as string].caption
            ? this.dataFields[measureName as string].caption
            : this.mappingFields[measureName as string] &&
              this.mappingFields[measureName as string].caption
            ? this.mappingFields[measureName as string].caption
        ...
        currrentMembers: {},
        formatString: formatString,
        allowDragAndDrop: this.dataFields[measureName as string]
          ? this.dataFields[measureName as string].allowDragAndDrop
          : this.mappingFields[measureName as string]
          ? this.mappingFields[measureName as string].allowDragAndDrop
          : true,
        showFilterIcon: this.dataFields[measureName as string]
          ? this.dataFields[measureName as string].showFilterIcon
          : this.mappingFields[measureName as string]
          ? this.mappingFields[measureName as string].showFilterIcon
          : true,
        showSortIcon: this.dataFields[measureName as string]
          ? this.dataFields[measureName as string].showSortIcon
          : this.mappingFields[measureName as string]
          ? this.mappingFields[measureName as string].showSortIcon
          : true,
        showEditIcon: this.dataFields[measureName as string]
          ? this.dataFields[measureName as string].showEditIcon
          : this.mappingFields[measureName as string]
          ? this.mappingFields[measureName as string].showEditIcon
          : true,
        showRemoveIcon: this.dataFields[measureName as string]
          ? this.dataFields[measureName as string].showRemoveIcon
          : this.mappingFields[measureName as string]
          ? this.mappingFields[measureName as string].showRemoveIcon
          : true,
        showValueTypeIcon: this.dataFields[measureName as string]
          ? this.dataFields[measureName as string].showValueTypeIcon
          : this.mappingFields[measureName as string]
          ? this.mappingFields[measureName as string].showValueTypeIcon
          : true,
        showSubTotals: this.dataFields[measureName as string]
          ? this.dataFields[measureName as string].showSubTotals
          : this.mappingFields[measureName as string]
          ? this.mappingFields[measureName as string].showSubTotals
          : true,
      };
      dimensionElements.push(fieldObj);
  ...
  private loadMeasureGroups(xmlDoc: Document): void {
    ...
    this.fieldListObj.measuresGroups = [].slice.call(
        ...
        if (!body.querySelector('OlapInfo')) {
          if (!body.querySelector('DiscoverResponse')) {
            ...
            if (body.querySelector('Fault')) {
              this.errorInfo = body
                .querySelector('Fault')
                .querySelector('faultstring').innerHTML;
            } else {
              this.errorInfo = body
  ...
  private beforeSend(args: string | Object): void {
    if (
      this.dataSourceSettings.authentication.userName &&
      this.dataSourceSettings.authentication.password
    ) {
      (args as { httpRequest: XMLHttpRequest }).httpRequest.setRequestHeader(
        'Authorization',
        'Basic ' +
          btoa(
            this.dataSourceSettings.authentication.userName +
  ...
  private getSoapMsg(dataSourceSettings: IDataOptions, query: string): string {
    ...
    if (this.isMondrian) {
      ...
      xmlMsg =
        ...
        query +
        ']]></Statement></Command><Properties><PropertyList><DataSourceInfo>' +
        sourceInfo +
        '</DataSourceInfo><Catalog>' +
        dataSourceSettings.catalog +
        '</Catalog><AxisFormat>TupleFormat</AxisFormat><Content>Data</Content><Format>Multidimensional</Format></PropertyList></Properties></Execute></SOAP-ENV:Body></SOAP-ENV:Envelope>';
    } else {
      xmlMsg =
        '<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/"> <Header></Header> <Body> <Execute xmlns="urn:schemas-microsoft-com:xml-analysis"> <Command> <Statement> ' +
        query +
        ' </Statement> </Command> <Properties> <PropertyList> <Catalog>' +
        dataSourceSettings.catalog +
        '</Catalog> <LocaleIdentifier>' +
        connectionString.LCID +
        '</LocaleIdentifier>' +
  ...
  public getConnectionInfo(
    ...
    if (connectionString !== '') {
      for (const obj of connectionString.split(';')) {
        if (
          obj.toLowerCase().indexOf('locale') < 0 &&
          connectionInfo.url.length === 0
        ) {
