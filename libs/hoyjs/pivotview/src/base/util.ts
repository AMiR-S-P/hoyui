...
import {
  IOlapCustomProperties,
  IOlapField,
  IOlapFieldListOptions,
} from './olap/engine';
import { HeadersSortEventArgs } from '../common/base/interface';
import { PdfPageSize } from '@hoyui/grids';
import { SizeF } from '@hoyui/pdf-export';
...
export class PivotUtil {
  ...
  private static getDefinedObj(
    ...
    if (!(data === null || data === undefined)) {
      ...
      while (keyPos < fields.length) {
        if (
          !(
            (data as { [key: string]: string })[fields[keyPos as number]] ===
              null ||
            (data as { [key: string]: string })[fields[keyPos as number]] ===
              undefined
          )
        ) {
          framedSet[fields[keyPos as number]] = (
            data as { [key: string]: string }
          )[fields[keyPos as number]];
        }
        keyPos++;
      }
    } else {
  ...
  public static setPivotProperties(
    control: PivotView | PivotFieldList,
    properties: { [key: string]: Object }
  ): void {
    control.allowServerDataBinding = false;
    if ((control as PivotFieldList).pivotGridModule) {
      (control as PivotFieldList).pivotGridModule.allowServerDataBinding =
        false;
    }
  ...
  public static getClonedDataSourceSettings(
    ...
    const clonesDataSource: IDataOptions = this.getDefinedObj({
      ...
      excludeFields: isNullOrUndefined(dataSourceSettings.excludeFields)
        ? []
      ...
      formatSettings: this.cloneFormatSettings(
      ...
      conditionalFormatSettings: this.cloneConditionalFormattingSettings(
        dataSourceSettings.conditionalFormatSettings
      ),
      ...
      showAggregationOnValueField:
        dataSourceSettings.showAggregationOnValueField,
      authentication: this.CloneAuthenticationObject(
        dataSourceSettings.authentication
      ),
    } as IDataOptions);
    return clonesDataSource;
  }

  public static getClonedFieldList(
    fieldListObj: IFieldListOptions | IOlapFieldListOptions
  ): IFieldListOptions | IOlapFieldListOptions {
    const keys: string[] = Object.keys(fieldListObj);
    const clonedfieldlistObj: IOlapFieldListOptions = {};
    for (
      let i: number = 0, keysLength: number = keys.length;
      i < keysLength;
      i++
    ) {
      ...
      if (fieldListObj[keys[i as number]]) {
        clonedfieldlistObj[keys[i as number]] = {
          ...
          membersOrder: (fieldlistObj.membersOrder
          ...
          currrentMembers: this.cloneFormatMembers(
            fieldlistObj.currrentMembers
          ),
          isHierarchy: fieldlistObj.isHierarchy,
          isNamedSets: fieldlistObj.isNamedSets,
          actualFilter: fieldlistObj.actualFilter
            ? [...fieldlistObj.actualFilter]
            : fieldlistObj.actualFilter,
  ...
  public static cloneFormatMembers(collection: IMembers): IMembers {
    if (collection) {
      ...
      for (
        ...
        const cloneFormatMembersObj: IMembers = collection[
          keys[i as number]
        ] as IMembers;
        clonedFormatMembers[keys[i as number]] = {
          index: cloneFormatMembersObj.index
            ? [...(cloneFormatMembersObj.index as number[])]
            : cloneFormatMembersObj.index,
  ...
  public static updateDataSourceSettings(
    control: PivotView | PivotFieldList,
    dataSourceSettings: IDataOptions
  ): void {
    if (control) {
  ...
  public static cloneFieldSettings(
    collection: IFieldOptions[]
  ): IFieldOptions[] {
    if (collection) {
   ...
   * @hidden
   */
  public static getFormattedHeader(
    headerCell: IAxisSet,
    engine: PivotEngine
  ): IAxisSet {
    ...
    if (clonedHeader.valueSort && clonedHeader.valueSort['axis']) {
      ...
      clonedHeader.formattedText =
        isDateType ||
        !engine.formatFields[fieldName as string] ||
        headerCell.isSum === true
          ? clonedHeader.formattedText
          : engine.getFormattedValue(
              clonedHeader.actualText,
              fieldName as string
            ).formattedText;
    }
    return clonedHeader;
  ...
  public static getFormattedMembers(
    ...
    if (isDateField || engine.groupingFields[fieldName as string]) {
      ...
      const dateMember: IAxisSet[] =
        engine.fieldList[fieldName as string].dateMember;
      for (let i: number = 0, j: number = keys.length; i < j; i++) {
        ...
        if (isDateField) {
          ...
          const commonValue: string | number =
            dateMember[values.ordinal - 1].actualText;
   ...
   * @hidden
   */
  public static isDateField(
    fieldName: string,
    engineModule: PivotEngine
  ): boolean {
    return (
      engineModule.formatFields[fieldName as string] &&
      ['date', 'dateTime', 'time'].indexOf(
        engineModule.formatFields[fieldName as string].type
      ) > -1
    );
  }
   ...
   * @hidden
   */
  public static formatChartHeaders(
    values: string[],
    ...
    for (let i: number = 0, j: number = values.length; i < j; i++) {
      const fieldName: string = isColumnHeader
        ? chartModule.parent.dataSourceSettings.columns.length > 0 &&
          !isNullOrUndefined(
            chartModule.parent.dataSourceSettings.columns[i as number]
          )
          ? (chartModule.parent.dataSourceSettings.columns[i as number]
              .name as string)
          : undefined
        : chartModule.parent.dataSourceSettings.rows.length > 0 &&
          !isNullOrUndefined(
            chartModule.parent.dataSourceSettings.rows[i as number]
          )
        ? (chartModule.parent.dataSourceSettings.rows[i as number]
            .name as string)
        : undefined;
      if (!isNullOrUndefined(fieldName)) {
        if (
          chartModule.engineModule.formatFields[fieldName as string] &&
          ['date', 'dateTime', 'time'].indexOf(
            chartModule.engineModule.formatFields[fieldName as string].type
          ) > -1
        ) {
          formattedValues.push(values[i as number] as string);
        } else {
          formattedValues.push(
            (chartModule.engineModule as PivotEngine).getFormattedValue(
              values[i as number] as string,
              fieldName as string
            ).formattedText
          );
        }
  ...
  public static cloneDrillMemberSettings(
    collection: IDrillOptions[]
  ): IDrillOptions[] {
    if (collection) {
  ...
  public static cloneFormatSettings(
    collection: IFormatSettings[]
  ): IFormatSettings[] {
    if (collection) {
  ...
  private static CloneAuthenticationObject(
    collection: IAuthenticationInfo
  ): IAuthenticationInfo {
    if (collection) {
  ...
  public static cloneCalculatedFieldSettings(
    collection: ICalculatedFieldSettings[]
  ): ICalculatedFieldSettings[] {
    if (collection) {
  ...
  private static cloneConditionalFormattingSettings(
    collection: IConditionalFormatSettings[]
  ): IConditionalFormatSettings[] {
    if (collection) {
  ...
  public static cloneGroupSettings(
    collection: IGroupSettings[]
  ): IGroupSettings[] {
    if (collection) {
  ...
  public static getFilterItemByName(
    fieldName: string,
    fields: IFilter[]
  ): IFilter {
    const filterItems: IFilter[] = new DataManager({
      json: fields,
  ...
  public static getFieldInfo(
    ...
    if (!hasAllField) {
      const rows: IFieldOptions[] = this.cloneFieldSettings(
        control.dataSourceSettings.rows
      );
      const columns: IFieldOptions[] = this.cloneFieldSettings(
        control.dataSourceSettings.columns
      );
      const values: IFieldOptions[] = this.cloneFieldSettings(
        control.dataSourceSettings.values
      );
      const filters: IFieldOptions[] = this.cloneFieldSettings(
        control.dataSourceSettings.filters
      );
      const fields: IFieldOptions[][] = [rows, columns, values, filters];
      for (let i: number = 0, len: number = fields.length; i < len; i++) {
        for (
          let j: number = 0,
            cnt: number = fields[i as number] ? fields[i as number].length : 0;
          j < cnt;
          j++
        ) {
          if (
            fields[i as number][j as number] &&
            fields[i as number][j as number].name === fieldName
          ) {
            return {
              fieldName: fieldName,
              fieldItem: fields[i as number][j as number],
              axis:
                i === 0
                  ? 'rows'
                  : i === 1
    ...
    const fieldList: IOlapField =
      control.dataType === 'olap'
        ? control.olapEngineModule.fieldList[fieldName as string]
        : control.engineModule.fieldList[fieldName as string];
  ...
  public static isButtonIconRefesh(
    prop: string,
    oldProp: PivotViewModel | PivotFieldListModel,
    newProp: PivotViewModel | PivotFieldListModel
  ): boolean {
    let isButtonRefresh: boolean = false;
    try {
      if (
        prop === 'dataSourceSettings' &&
        oldProp.dataSourceSettings &&
        newProp.dataSourceSettings
      ) {
        const propValidation: string[] = [
          'notAvail',
          'notAvail',
          'notAvail',
          'notAvail',
        ...
        if (
          oldAxesProp &&
          newAxesProp &&
          newAxesProp.length > 0 &&
          oldAxesProp.length === newAxesProp.length
        ) {
          const axes: string[] = ['rows', 'columns', 'values', 'filters'];
          for (let i: number = 0; i < newAxesProp.length; i++) {
            const oldAxis: string[] =
              newAxesProp[i as number] in oldProp.dataSourceSettings &&
              !isNullOrUndefined(
                (oldProp.dataSourceSettings as { [key: string]: Object })[
                  newAxesProp[i as number]
                ]
              )
                ? Object.keys(
                    (oldProp.dataSourceSettings as { [key: string]: Object })[
            ...
            const newAxis: string[] =
              newAxesProp[i as number] in newProp.dataSourceSettings &&
              !isNullOrUndefined(
                (newProp.dataSourceSettings as { [key: string]: Object })[
                  newAxesProp[i as number]
                ]
              )
                ? Object.keys(
                ...
                : [];
            if (
              axes.indexOf(newAxesProp[i as number]) !== -1 &&
              axes.indexOf(oldAxesProp[i as number]) !== -1 &&
              oldAxis &&
              newAxis &&
              newAxis.length > 0 &&
              ...
              for (let j: number = 0; j < newAxis.length; j++) {
                const oldAxisProp: string[] = Object.keys(
                  (
                    (oldProp.dataSourceSettings as { [key: string]: Object })[
                      newAxesProp[i as number]
                    ] as {
                ...
                const newAxisProp: string[] = Object.keys(
                  (
                    (newProp.dataSourceSettings as { [key: string]: Object })[
                      newAxesProp[i as number]
                    ] as {
                ...
                for (let k: number = 0; k < newAxisProp.length; k++) {
                  if (
                    options.indexOf(newAxisProp[k as number]) !== -1 &&
  ...
  public static formatPivotValues(
    pivotValues: { [key: string]: Object }[][]
  ): IAxisSet[][] {
        ...
        for (let j: number = 0; j < pivotValues[i as number].length; j++) {
          if (pivotValues[i as number][j as number]) {
            values[i as number][j as number] = {
              ...
              actualText: pivotValues[i as number][j as number]
                .ActualText as string,
              indexObject: pivotValues[i as number][j as number]
                .IndexObject as INumberIndex,
              index: pivotValues[i as number][j as number].Index as number[],
              rowHeaders: pivotValues[i as number][j as number]
                .RowHeaders as string,
              columnHeaders: pivotValues[i as number][j as number]
                .ColumnHeaders as string,
              formattedText: pivotValues[i as number][j as number]
                .FormattedText as string,
              actualValue: pivotValues[i as number][j as number]
                .ActualValue as number,
              rowIndex: pivotValues[i as number][j as number]
                .RowIndex as number,
              colIndex: pivotValues[i as number][j as number]
                .ColIndex as number,
              ...
              isGrandSum: pivotValues[i as number][j as number]
                .IsGrandSum as boolean,
              valueSort: pivotValues[i as number][j as number]
                .ValueSort as IDataSet,
              ordinal: pivotValues[i as number][j as number].Ordinal as number,
              hasChild: pivotValues[i as number][j as number]
                .HasChild as boolean,
              isDrilled: pivotValues[i as number][j as number]
                .IsDrilled as boolean,
              value: pivotValues[i as number][j as number].Value as number,
              type: pivotValues[i as number][j as number].Type as SummaryTypes,
              members: pivotValues[i as number][j as number]
                .Members as IAxisSet[],
            };
          }
        }
  ...
  public static formatFieldList(fieldList: {
  ...
  public static frameContent(
    ...
    if (
      pivot.dataSourceSettings.values.length > 0 &&
      !pivot.engineModule.isEmptyData
    ) {
  ...
  public static getLocalizedObject(
    control: PivotView | PivotFieldList
  ): Object {
    ...
    locale['GroupOutOfRange'] =
      control.localeObj.getConstant('groupOutOfRange');
  ...
  public static updateReport(
    ...
    for (let i: number = 0; i < report.Rows.length; i++) {
      control.dataSourceSettings.rows.push({
        ...
        isCalculatedField: report.Rows[i as number]
          .IsCalculatedField as boolean,
        showValueTypeIcon: report.Rows[i as number]
          .ShowValueTypeIcon as boolean,
    ...
    for (let i: number = 0; i < report.Columns.length; i++) {
      control.dataSourceSettings.columns.push({
        ...
        allowDragAndDrop: report.Columns[i as number]
          .AllowDragAndDrop as boolean,
        axis: report.Columns[i as number].Axis as string,
        dataType: report.Columns[i as number].DataType as string,
        isCalculatedField: report.Columns[i as number]
          .IsCalculatedField as boolean,
        showValueTypeIcon: report.Columns[i as number]
          .ShowValueTypeIcon as boolean,
    ...
    for (let i: number = 0; i < report.FormatSettings.length; i++) {
      control.dataSourceSettings.formatSettings.push({
        ...
        maximumFractionDigits: report.FormatSettings[i as number]
          .MaximumFractionDigits as number,
        maximumSignificantDigits: report.FormatSettings[i as number]
          .MaximumSignificantDigits as number,
        minimumFractionDigits: report.FormatSettings[i as number]
          .MinimumFractionDigits as number,
        minimumIntegerDigits: report.FormatSettings[i as number]
          .MinimumIntegerDigits as number,
        minimumSignificantDigits: report.FormatSettings[i as number]
          .MinimumSignificantDigits as number,
  ...
  public static applyCustomSort(
    ...
    if (!hasMembersOrder) {
      for (
        let i: number = updatedMembers.length;
        i < sortMembersOrder.length;
        i++
      ) {
        const member: IAxisSet = sortMembersOrder[i as number];
        const sortText: string | number = isOlap
  ...
  public static applyHeadersSort(
    ...
    sortOrder: string,
    type: string | boolean,
    isNumberGroupSorting?: boolean
  ): IAxisSet[] {
    if (isNumberGroupSorting) {
      sortMembersOrder = sortMembersOrder.sort(
        (a: IAxisSet, b: IAxisSet): number => {
          const aText: string = a.actualText as string;
          const bText: string = b.actualText as string;
          return aText === 'Grand Total' || bText === 'Grand Total'
            ? 0
            : aText === 'Out of Range'
            ? 1
            : bText === 'Out of Range'
            ? -1
            : !isNaN(parseFloat(aText)) && !isNaN(parseFloat(bText))
            ? sortOrder === 'Ascending'
      ...
      sortMembersOrder =
        sortOrder === 'Ascending'
          ? sortMembersOrder.sort((a: IAxisSet, b: IAxisSet): number =>
              a.dateText > b.dateText ? 1 : b.dateText > a.dateText ? -1 : 0
            )
      ...
      sortMembersOrder =
        sortOrder === 'Ascending'
          ? sortMembersOrder.sort((a: IAxisSet, b: IAxisSet): number =>
              a.actualText === 'Grand Total' || b.actualText === 'Grand Total'
                ? 0
                : a.actualText === 'Out of Range'
                ? 1
                : b.actualText === 'Out of Range'
                ? -1
                : Number(a.actualText.toString().match(/\d+/)) >
                  Number(b.actualText.toString().match(/\d+/))
                ? 1
                : Number(b.actualText.toString().match(/\d+/)) >
                  Number(a.actualText.toString().match(/\d+/))
                ? -1
      ...
      sortMembersOrder =
        sortOrder === 'Ascending'
          ? sortMembersOrder.sort(function (a: IAxisSet, b: IAxisSet): number {
              return a.actualText === 'Grand Total' ||
                b.actualText === 'Grand Total'
                ? 0
                : a.actualText > b.actualText
                ? 1
                : b.actualText > a.actualText
                ? -1
                : 0;
            })
          : sortOrder === 'Descending'
   ...
   * @hidden
   */
  public static renderOlapEngine(
    pivot: PivotView | PivotFieldList,
    customProperties?: IOlapCustomProperties
  ): void {
    try {
      pivot.olapEngineModule.renderEngine(
        pivot.dataSourceSettings as IDataOptions,
        customProperties
          ? customProperties
          : (pivot as PivotFieldList).frameCustomProperties(
              pivot.olapEngineModule.fieldListData,
              pivot.olapEngineModule.fieldList
            ),
        pivot.onHeadersSort
          ? (pivot as PivotView).getHeaderSortInfo.bind(pivot)
          : undefined
      );
      pivot.setProperties(
        {
   ...
   * @hidden */
  public static frameHeaderWithKeys(
    header: IDataSet | IAxisSet
  ): IAxisSet | IDataSet {
    const keys: string[] = Object.keys(header);
    let keyPos: number = 0;
    const framedHeader: IDataSet = {};
    while (keyPos < keys.length) {
      framedHeader[keys[keyPos as number]] = (header as IDataSet)[
   ...
   * @hidden */
  public static getSortedValue(
    aggreColl: { header: IAxisSet; value?: number }[],
    sortOrder: string
  ): IAxisSet[] {
    aggreColl.sort(
      (
        a: { value?: number; header: { type: string } },
        b: { value?: number; header: { type: string } }
      ) => {
