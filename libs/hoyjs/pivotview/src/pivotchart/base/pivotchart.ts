...
import {
  ColumnModel,
  IPointEventArgs,
  IMultiLevelLabelClickEventArgs,
  LegendSettingsModel,
  BubbleSeries,
} from '@hoyui/charts';
import {
...
import { createElement, remove, isNullOrUndefined, select } from '@hoyui/base';
import {
  ChartSettingsModel,
  PivotSeriesModel,
} from '../../pivotview/model/chartsettings-model';
import { PivotView } from '../../pivotview/base/pivotview';
import {
  RowHeaderPositionGrouping,
  ChartSeriesType,
  ChartSeriesCreatedEventArgs,
  RowHeaderLevelGrouping,
  ChartLabelInfo,
} from '../../common';
import {
  DrillArgs,
  ...
  MultiLevelLabelRenderEventArgs,
} from '../../common';
...
import { hideSpinner, OffsetPosition } from '@hoyui/popups';
...
export class PivotChart {
  ...
  private columnGroupObject: {
    [key: string]: { x: string; y: number; rIndex: number; cIndex: number }[];
  } = {};
  ...
  private accumulationType: ChartSeriesType[] = [
    'Pie',
    'Pyramid',
    'Doughnut',
    'Funnel',
  ...
  public loadChart(parent: PivotView, chartSettings: ChartSettingsModel): void {
    ...
    const isDataAvail: boolean =
      parent.dataType === 'olap'
        ? parent.dataSourceSettings.url !== '' &&
          !parent.olapEngineModule.isEmptyData &&
          parent.olapEngineModule.tupColumnInfo.length > 0 &&
          parent.olapEngineModule.tupRowInfo.length > 0 &&
          (!isNullOrUndefined(parent.olapEngineModule.colMeasurePos) ||
            !isNullOrUndefined(parent.olapEngineModule.rowMeasurePos))
        : this.parent.dataSourceSettings.mode === 'Server'
        ? !isNullOrUndefined(parent.dataSourceSettings.url) &&
          parent.dataSourceSettings.url !== '' &&
          parent.dataSourceSettings.values.length > 0 &&
          !parent.engineModule.isEmptyData
        : parent.dataSourceSettings.values.length > 0 &&
          parent.dataSourceSettings.dataSource &&
          ((parent.dataSourceSettings.dataSource as IDataSet[]).length > 0 ||
            parent.dataSourceSettings.dataSource instanceof DataManager) &&
          !parent.engineModule.isEmptyData;
    if (isDataAvail) {
      if (
        !this.parent.chart &&
        (this.parent.element.querySelector('.e-chart') ||
          this.parent.element.querySelector('.e-accumulationchart'))
      ) {
        remove(
          select('#' + this.parent.element.id + '_chart', this.parent.element)
        );
      }
      if (
        this.chartSettings.enableMultipleAxis &&
        this.accumulationType.indexOf(chartSettings.chartSeries.type) < 0 &&
        this.chartSettings.chartSeries.type !== 'Pareto'
      ) {
        this.measureList = this.dataSourceSettings.values.map(
          (item: IFieldOptions) => {
            return item.name;
          }
        );
        ...
        this.measureList = [
          chartSettings.value === ''
            ? this.dataSourceSettings.values[0].name
            : chartSettings.value,
        ];
      }
      for (const field of this.dataSourceSettings.values) {
        const fieldName: string = field.name.replace(/[^A-Z0-9]+/gi, '_');
        this.measuresNames[field.name] = fieldName;
        this.measuresNames[fieldName as string] = field.name;
        if (
          this.chartSettings.chartSeries.type === 'Polar' ||
          this.chartSettings.chartSeries.type === 'Radar'
        ) {
          this.measuresNames[field.caption ? field.caption : field.name] =
            field.name;
        }
      }
      ...
      if (this.parent.element.querySelector('.e-chart')) {
        ...
        (this.parent.chart as Chart).primaryXAxis.zoomFactor =
          isNullOrUndefined(this.parent.chartSettings.primaryXAxis.zoomFactor)
            ? 1
      ...
      if (
        ...
        if (
          ...
          this.accumulationType.indexOf(this.chartSettings.chartSeries.type) >
            -1
        ) {
        ...
        if (this.parent.showToolbar) {
          if (
            this.parent.displayOption.view === 'Both' &&
    ...
    this.currentMeasure =
      chartSettings.enableMultipleAxis &&
      this.accumulationType.indexOf(chartSettings.chartSeries.type) < 0 &&
      this.chartSettings.chartSeries.type !== 'Pareto'
        ? this.measureList[0]
        : (chartSettings.value === '' ||
            this.dataSourceSettings.values.filter((item: IFieldOptions) => {
    ...
    for (const field of this.dataSourceSettings.values) {
      ...
      measureNames[fieldName as string] = field.caption
        ? field.caption
        : fieldName;
    ...
    if (this.parent.dataType === 'olap') {
      ...
      levelPos = this.groupHierarchyWithLevels(
        pivotValues as IAxisSet[],
        fieldPosition
      );
      lastHierarchy = fieldPosition[fieldPosition.length - 1];
      lastDimension =
        this.measurePos === fieldPosition.length - 1 && fieldPosition.length > 1
          ? fieldPosition[fieldPosition.length - 2]
          : lastHierarchy;
    ...
    for (const rKey of rKeys) {
      ...
      if (!isNullOrUndefined(pivotValues[rowIndex as number])) {
        ...
        const valueSort: string[] =
          header &&
          header.valueSort &&
          !isNullOrUndefined(header.valueSort.levelName)
            ? header.valueSort.levelName.toString().split(delimiter)
            : undefined;
        isValidHeader = false;
        if (valueSort && valueSort[0] !== 'Grand Total') {
          if (
            (chartSettings.enableMultipleAxis &&
              this.accumulationType.indexOf(chartSettings.chartSeries.type) <
                0 &&
              this.chartSettings.chartSeries.type !== 'Pareto') ||
        ...
        if (
          ...
          const firstRowCell: IAxisSet = pivotValues[
            rowIndex as number
          ][0] as IAxisSet;
          ...
          const tupInfo: ITupInfo =
            this.parent.dataType === 'olap'
              ? (this.engineModule as OlapEngine).tupRowInfo[
                  firstRowCell.ordinal
                ]
              : undefined;
          ...
          if (this.parent.dataType === 'olap') {
            isDrill =
              firstRowCell.hierarchy === '[Measures]'
                ? isDrill
                : this.isAttributeDrill(
                    firstRowCell.hierarchy,
                    tupInfo.drillInfo
                  );
            drillDimension =
              drillDimension === lastDimension
                ? lastDimension
                : firstRowCell.hierarchy === '[Measures]' ||
                  firstRowCell.isNamedSet ||
                  (this.engineModule.fieldList[firstRowCell.hierarchy] &&
                    !(this.engineModule as OlapEngine).fieldList[
                      firstRowCell.hierarchy
                    ].hasAllMember)
                ? lastDimension
                : drillDimension;
            ...
            if (
              firstRowCell.memberType !== 3 &&
              (tupInfo.measureName
                ? tupInfo.measureName === this.dataSourceSettings.values[0].name
                : true)
            ) {
              firstLevelUName =
                firstLevelUName === undefined
                  ? firstRowCell.levelUniqueName
                  : firstLevelUName;
              integratedLevel =
                firstLevelUName === firstRowCell.levelUniqueName
                  ? 0
                  : integratedLevel;
              levelCollection = integratedLevel === 0 ? {} : levelCollection;
              integratedLevel =
                prevCell && firstLevelUName !== firstRowCell.levelUniqueName
                  ? prevCell.hierarchy === firstRowCell.hierarchy
                    ? integratedLevel + (firstRowCell.level - prevCell.level)
                    : isNullOrUndefined(
                        levelCollection[firstRowCell.levelUniqueName]
                      )
              ...
              indexCount +=
                prevCell &&
                drillDimension === prevCell.hierarchy &&
                !(prevCell.isDrilled && prevCell.hasChild)
                  ? 1
                  : 0;
          ...
          this.maxLevel =
            currrentLevel > this.maxLevel ? currrentLevel : this.maxLevel;
          const name: string =
            this.parent.dataType === 'olap'
              ? firstRowCell.formattedText
              : firstRowCell.actualText
              ? firstRowCell.actualText.toString()
              : firstRowCell.formattedText.toString();
          const values: IFieldListOptions = this.engineModule.fieldList[
            this.currentMeasure
          ] as IFieldListOptions;
          const text: string =
            this.parent.dataSourceSettings.rows.length === 0
              ? this.parent.localeObj.getConstant('total') +
                ' ' +
                this.parent.localeObj.getConstant(
                  values.aggregateType as string
                ) +
                ' ' +
                this.parent.localeObj.getConstant('of') +
                ' ' +
                (!isNullOrUndefined(values.caption)
                  ? values.caption
                  : values.name)
              : firstRowCell.formattedText
              ? firstRowCell.formattedText.toString()
          ...
          const caption: string =
            firstRowCell.hasChild && !firstRowCell.isNamedSet
              ? (firstRowCell.isDrilled ? ' - ' : ' + ') + text
              : text;
          const levelName: string = tupInfo
            ? tupInfo.uNameCollection
            : firstRowCell.valueSort.levelName.toString();
          const cellInfo: ChartLabelInfo = {
            ...
            levelName: levelName,
            level: currrentLevel,
            fieldName: firstRowCell.valueSort.axis
              ? firstRowCell.valueSort.axis.toString()
              : '',
            rowIndex: rowIndex,
            colIndex: 0,
            cell: firstRowCell,
          ...
          if (
            this.parent.dataType === 'olap'
              ? firstRowCell.memberType !== 3
              : firstRowCell.type !== 'value'
          ) {
            if (this.headerColl[indexCount as number]) {
              this.headerColl[indexCount as number][currrentLevel as number] =
                cellInfo;
            } else {
          ...
          if (this.parent.dataType === 'olap') {
            memberCell =
              firstRowCell.memberType !== 3 ? firstRowCell : memberCell;
          } else {
            memberCell =
              firstRowCell.type !== 'value' ? firstRowCell : memberCell;
            if (firstRowCell.type !== 'value') {
              memberCell = firstRowCell;
            } else {
              const valueSort: string[] =
                firstRowCell &&
                firstRowCell.valueSort &&
                firstRowCell.valueSort.levelName &&
              ...
              if (
                this.parent.dataSourceSettings.valueIndex <= 0 ||
                ((this.engineModule as PivotEngine).valueAxis &&
                  this.dataSourceSettings.rows.length ===
                    (this.engineModule as PivotEngine).measureIndex) ||
                isNullOrUndefined(memberCell.valueSort) ||
                levelName === memberCell.valueSort.levelName
              ) {
                ...
                while (prevIndex > -1) {
                  if (
                    pivotValues[prevIndex as number] &&
                    pivotValues[prevIndex as number][0] &&
                    (pivotValues[prevIndex as number][0] as IAxisSet)
                      .valueSort &&
                    (pivotValues[prevIndex as number][0] as IAxisSet).valueSort
                      .levelName === levelName
                  ) {
                    memberCell = pivotValues[
                      prevIndex as number
                    ][0] as IAxisSet;
                    prevIndex = 0;
                  }
          ...
          for (const cKey of cKeys) {
            ...
            const cell: IAxisSet = pivotValues[rowIndex as number][
              cellIndex as number
            ] as IAxisSet;
            const measureAllow: boolean = isNullOrUndefined(cell.rowHeaders)
              ? this.dataSourceSettings.rows.length === 0
              : true;
            const actualText: string | number =
              this.parent.dataType === 'olap' && tupInfo && tupInfo.measureName
                ? tupInfo.measureName
                : cell.actualText;
            if (
              !(this.parent.dataType === 'olap' && cell.isGrandSum) &&
              !totColIndex[cell.colIndex] &&
              cell.axis === 'value' &&
              firstRowCell.type !== 'header' &&
              actualText !== '' &&
              (chartSettings.enableMultipleAxis &&
              this.accumulationType.indexOf(chartSettings.chartSeries.type) <
                0 &&
              this.chartSettings.chartSeries.type !== 'Pareto'
                ? true
                : actualText === this.currentMeasure)
            ) {
              ...
              if (
                this.parent.dataType === 'olap'
                  ? lastHierarchy === firstRowCell.hierarchy || isDrill
                    ? firstRowCell.memberType === 3 && prevMemberCell
                      ? fieldPos === this.measurePos
                        ? prevMemberCell.isDrilled && prevMemberCell.hasChild
                        : true
                      : firstRowCell.isDrilled && firstRowCell.hasChild
                    : true
                  : (firstRowCell.type === 'value' && prevMemberCell
                      ? !isNullOrUndefined(prevMemberCell.members) &&
                        prevMemberCell.hasChild &&
                        prevMemberCell.isDrilled
                      : firstRowCell.hasChild && firstRowCell.isDrilled) ||
                    !measureAllow
              ) {
                break;
              }
              ...
              if (this.parent.dataType === 'olap') {
                ...
                const values: string[] = cell.columnHeaders
                  .toString()
                  .split(delimiter);
              ...
              if (this.parent.dataType === 'olap') {
                ...
                const values: string[] = cell.rowHeaders
                  .toString()
                  .split(delimiter);
              ...
              this.chartSeriesInfo[colHeaders as string] = {
                ...
                caption:
                  cell.hierarchyName &&
                  cell.hierarchyName.toString().split(delimiter).join(' - '),
                colorIndex: [],
              };
              this.chartSeriesInfo[
                this.chartSeriesInfo[colHeaders as string].caption
              ] = this.chartSeriesInfo[colHeaders as string];
              let yValue: number =
  ...
  public refreshChart(): void {
    ...
    this.persistSettings = JSON.parse(
      this.parent.getChartSettings()
    ).chartSettings;
    const seriesColors: string[] =
      this.persistSettings.palettes && this.persistSettings.palettes.length > 0
        ? this.persistSettings.palettes
        : getSeriesColor(this.chartSettings.theme);
    const delimiter: string = (this.parent as PivotView).chartSettings
      .columnDelimiter
      ? (this.parent as PivotView).chartSettings.columnDelimiter
      : '-';
    const columnHeader: string =
      (this.parent as PivotView).chartSettings.columnHeader &&
      (this.parent as PivotView).chartSettings.columnHeader !== ''
        ? (this.parent as PivotView).chartSettings.columnHeader
            .split(delimiter)
            .join(' - ')
        : '';
    const chartType: ChartSeriesType = this.chartSettings.chartSeries
      ? this.chartSettings.chartSeries.type
      : undefined;
    const fieldWithCaption: { [key: string]: string } = {};
    for (
      let i: number = 0;
      i < this.parent.dataSourceSettings.values.length;
      i++
    ) {
      fieldWithCaption[
        this.parent.dataSourceSettings.values[i as number].name
      ] = !isNullOrUndefined(
        this.parent.dataSourceSettings.values[i as number].caption
      )
        ? this.parent.dataSourceSettings.values[i as number].caption
        : undefined;
    }
    if (
      this.accumulationType.indexOf(chartType) > -1 &&
      columnKeys.length > 0
    ) {
      this.currentColumn =
        columnKeys.indexOf(columnHeader + ' | ' + this.currentMeasure) > -1 &&
        columnHeader !== undefined
          ? columnHeader + ' | ' + this.currentMeasure
          : columnKeys[0];
      let currentSeries: AccumulationSeriesModel = {};
      currentSeries = this.persistSettings.chartSeries
        ? (this.frameChartSeries(
            this.persistSettings.chartSeries as {
              [key: string]: Object;
            }
          ) as AccumulationSeriesModel)
        : currentSeries;
      if (
        (isNullOrUndefined(currentSeries.palettes) ||
          currentSeries.palettes.length === 0) &&
        !isNullOrUndefined(this.persistSettings.palettes) &&
      ...
      if (
        this.persistSettings.chartSeries &&
        this.persistSettings.chartSeries.dataLabel
      ) {
        currentSeries.dataLabel = this.persistSettings.chartSeries
          .dataLabel as AccumulationDataLabelSettingsModel;
      ...
      if (chartType === 'Doughnut') {
        ...
        currentSeries.innerRadius = this.chartSettings.chartSeries.innerRadius
      ...
      this.chartSeries = (this.chartSeries as AccumulationSeriesModel[]).concat(
        currentSeries
      );
    } else {
      for (const key of columnKeys) {
        ...
        currentSeries = this.persistSettings.chartSeries
          ...
          : currentSeries;
        if (
          !isNullOrUndefined((currentSeries as PivotSeriesModel).palettes) &&
          (currentSeries as PivotSeriesModel).palettes.length > 0 &&
          (isNullOrUndefined(this.persistSettings.palettes) ||
            this.persistSettings.palettes.length === 0)
        ) {
          this.chartSettings.palettes = (
            currentSeries as PivotSeriesModel
          ).palettes;
        }
        for (
          let i: number = 0;
          i < this.columnGroupObject[key as string].length;
          i++
        ) {
          const values: IFieldListOptions = this.engineModule.fieldList[
            this.currentMeasure
          ] as IFieldListOptions;
          this.columnGroupObject[key as string][i as number].x =
            this.parent.dataSourceSettings.rows.length === 0 &&
            !this.chartSettings.showMultiLevelLabels
              ? this.parent.localeObj.getConstant('total') +
                ' ' +
                this.parent.localeObj.getConstant(
                  values.aggregateType as string
                ...
                (!isNullOrUndefined(values.caption)
                  ...
                  : values.name)
              : this.columnGroupObject[key as string][i as number].x === ''
              ? this.parent.localeObj.getConstant('blank')
              : this.columnGroupObject[key as string][i as number].x;
        }
        ...
        for (
          let i: number = 0, j: string[] = key.split(' | ');
          i < j.length;
          i++
        ) {
          if (
            this.measuresNames &&
            this.measuresNames[j[i as number] as string]
          ) {
        ...
        if (this.chartSettings.enableMultipleAxis) {
          ...
          fieldCaptionName = !isNullOrUndefined(
            fieldWithCaption[fieldCaptionName as string]
          )
            ? fieldWithCaption[fieldCaptionName as string]
            : fieldCaptionName;
          multiAxisKey = currentSeriesName + ' | ' + fieldCaptionName;
        }
        currentSeries.name = this.chartSettings.enableMultipleAxis
          ? multiAxisKey
          : currentSeriesName;
        if (
          this.chartSettings.showPointColorByMembers &&
          this.chartSettings.enableMultipleAxis
        ) {
          currentSeries.name = currentSeriesName;
          const seriesName: string =
            this.chartSeriesInfo[currentSeries.name].caption;
          currentSeries.name =
            seriesName !== undefined && seriesName !== null
              ? seriesName
              : currentSeries.name;
          if (!chartSeriesInfo[currentSeries.name]) {
            prevColorIndex = seriesColors[prevColorIndex as number]
              ? prevColorIndex
              : 0;
            chartSeriesInfo[currentSeries.name] = {
              name: currentSeries.name,
              color: seriesColors[prevColorIndex as number],
            };
            currentSeries.fill = seriesColors[prevColorIndex++];
            this.chartSeriesInfo[currentSeries.name].colorIndex.push(
              this.selectedLegend++
            );
          } else {
        ...
        if (['Radar', 'Polar'].indexOf(chartType) < 0) {
          ...
          (currentSeries as SeriesModel).tooltipMappingName =
            this.measuresNames[measure as string];
          (currentSeries as SeriesModel).yAxisName =
            this.chartSettings.enableMultipleAxis &&
            this.chartSettings.multipleAxisMode === 'Combined'
              ? this.measureList.join('_')
              : this.measuresNames[measure as string]
              ? this.measuresNames[measure as string]
              : measure;
        }
        if (
          this.persistSettings.chartSeries &&
          this.persistSettings.chartSeries.emptyPointSettings
        ) {
        ...
        this.chartSeries = (this.chartSeries as SeriesModel[]).concat(
          currentSeries
        );
      }
    }
    const seriesEvent: ChartSeriesCreatedEventArgs = {
      series: this.chartSeries as SeriesModel[],
      cancel: false,
    };
    const pivotChart: PivotChart = this as PivotChart;
    this.parent.trigger(
      events.chartSeriesCreated,
      seriesEvent,
      (observedArgs: ChartSeriesCreatedEventArgs) => {
  ...
  private frameObjectWithKeys(series: {
    [key: string]: Object;
  }): SeriesModel | AxisModel {
  ...
  private frameChartSeries(series: {
    [key: string]: Object;
    ...
    while (keyPos < keys.length) {
      if (
        (this.accumulationType.indexOf(
          this.parent.chartSettings.chartSeries.type
        ) > -1 &&
          [
            'fill',
            'dashArray',
            ...
            'columnSpacing',
            'cornerRadius',
          ].indexOf(keys[keyPos as number]) > -1) ||
        (this.accumulationType.indexOf(
          this.parent.chartSettings.chartSeries.type
        ) < 0 &&
          [
  ...
  private bindChart(): void {
    ...
    const currentTooltipSettings: TooltipSettingsModel =
      this.configTooltipSettings();
    const currentLegendSettings: LegendSettingsModel =
      this.configLegendSettings();
    const currentZoomSettings: ZoomSettingsModel = this.configZoomSettings();
    const axesWithRows: {
      axes: AxisModel[];
      rows: RowModel[];
      columns: ColumnModel[];
    } = this.frameAxesWithRows();
    ...
    if (this.parent.displayOption.view === 'Both') {
      ...
      this.element = (this.parent.element as HTMLElement).appendChild(
        createElement('div', {
          className: cls.PIVOTCHART,
          id: this.parent.element.id + '_chart',
        })
    ...
    if (
      !this.chartElement &&
      this.parent.chartSettings.enableScrollOnMultiAxis &&
      this.parent.chartSettings.enableMultipleAxis &&
      this.chartSettings.chartSeries.type !== 'Pareto'
    ) {
      (
        (this.parent.element as HTMLElement).querySelector(
          '.' + cls.PIVOTCHART
        ) as HTMLElement
      ).innerHTML = '';
      this.chartElement = (this.parent.element as HTMLElement)
        .querySelector('.' + cls.PIVOTCHART)
        .appendChild(
          createElement('div', {
    ...
    if (
      ...
      if (
        this.parent.chartSettings.chartSeries.type !== 'Polar' &&
        this.parent.chartSettings.chartSeries.type !== 'Radar'
      ) {
        (
          (this.parent.element as HTMLElement).querySelector(
            '.' + cls.PIVOTCHART
    ...
    if (
      this.parent.chart &&
      ((this.parent.chart.getModuleName() === 'accumulationchart' &&
        this.accumulationType.indexOf(type) < 0) ||
        (this.parent.chart.getModuleName() === 'chart' &&
    ...
    if (
      ...
      if (
        ...
        this.parent.toolbarModule.isMultiAxisChange &&
        this.parent.chart
      ) {
      ...
      if (this.accumulationType.indexOf(type) > -1) {
        this.parent.chart = new AccumulationChart({
          series:
            this.chartSeries.length > 0
              ? (this.chartSeries as AccumulationSeriesModel[])
              : [{}],
        ...
        this.parent.chart = new Chart({
          series:
            this.chartSeries.length > 0
              ? (this.chartSeries as SeriesModel[])
              : [{}],
          legendSettings: currentLegendSettings,
          tooltip: currentTooltipSettings,
          ...
          rows:
            type === 'Polar' || type === 'Radar'
              ? [{}]
              : type === 'Bar' ||
                type === 'StackingBar' ||
                type === 'StackingBar100' ||
                (type === 'Pareto' && this.chartSettings.enableMultipleAxis)
              ? [{ height: '100%' }]
              : axesWithRows.rows,
          columns:
            type === 'Polar' || type === 'Radar'
          ...
          height:
            ...
            this.parent.chartSettings.enableScrollOnMultiAxis &&
            this.parent.chartSettings.enableMultipleAxis &&
            this.parent.chartSettings.chartSeries.type !== 'Pareto' &&
            this.parent.dataSourceSettings.values.length > 0
              ? Number(height) >
                this.parent.dataSourceSettings.values.length * 235 + 100
                ? isNaN(Number(height))
                  ? height.toString()
                  : (Number(height) - 5).toString()
                : !isNaN(Number(height)) ||
                  this.parent.dataSourceSettings.values.length > 1
                ? (
                    this.parent.dataSourceSettings.values.length * 235 +
                    100
                  ).toString()
                : height.toString()
              : height.toString(),
          ...
          border: this.chartSettings.border,
          background: this.chartSettings.background,
          chartArea: !isNullOrUndefined(this.persistSettings.chartArea)
            ? this.persistSettings.chartArea
          ...
          enableSideBySidePlacement:
            this.chartSettings.enableSideBySidePlacement,
          beforePrint: this.beforePrint.bind(this),
          animationComplete: this.animationComplete.bind(this),
          legendRender: this.legendRender.bind(this),
          textRender: this.textRender.bind(this),
      ...
      this.parent.chart.enableAnimation =
        this.parent.chartSettings.enableAnimation;
      this.parent.chart.useGroupingSeparator =
        this.parent.chartSettings.useGroupingSeparator;
      this.parent.chart.highlightPattern =
        this.parent.chartSettings.highlightPattern;
      if (this.accumulationType.indexOf(type) > -1) {
        (this.parent.chart as AccumulationChart).enableBorderOnMouseMove =
          this.parent.chartSettings.enableBorderOnMouseMove;
        (this.parent.chart as AccumulationChart).highlightMode =
          this.parent.chartSettings.highlightMode;
        (this.parent.chart as AccumulationChart).enableSmartLabels =
          this.parent.chartSettings.enableSmartLabels;
      } else {
        (this.parent.chart as Chart).palettes =
          this.parent.chartSettings.palettes;
        (this.parent.chart as Chart).isTransposed =
          this.parent.chartSettings.isTransposed;
        (this.parent.chart as Chart).enableSideBySidePlacement =
          this.parent.chartSettings.enableSideBySidePlacement;
        (this.parent.chart as Chart).tabIndex =
          this.parent.chartSettings.tabIndex;
        (this.parent.chart as Chart).description =
          this.parent.chartSettings.description;
        (this.parent.chart as Chart).enableCanvas =
          this.parent.chartSettings.enableCanvas;
      }
      if (type === 'Polar' || type === 'Radar') {
        ...
        if (
          type === 'Bar' ||
          type === 'StackingBar' ||
          (type === 'StackingBar100' && this.chartSettings.enableMultipleAxis)
        ) {
        ...
        if (this.parent.pivotFieldListModule) {
          hideSpinner(
            this.parent.pivotFieldListModule
              .fieldListSpinnerElement as HTMLElement
          );
        }
      }
    }
    if (
      ...
      this.parent.chart.appendTo(
        select(
          '#' + this.parent.element.id + '_chartInner',
          this.parent.element
        ) as HTMLElement
      );
    } else {
  ...
  private legendClick(args: ILegendClickEventArgs): void {
    if (
      ...
      const colorIndex: number[] =
        this.chartSeriesInfo[args.legendText].colorIndex;
      for (let i: number = 1; i < colorIndex.length; i++) {
        args.chart.series[colorIndex[i as number]].visible =
          !args.chart.series[colorIndex[i as number]].visible;
      }
    }
  ...
  private pointClick(args: IPointEventArgs): void {
    const dataSource: Object | DataManager = args.series.dataSource
      ? args.series.dataSource
      : this.parent.chart.series[args.seriesIndex].dataSource;
    if (
      (['Pie', 'Funnel', 'Doughnut', 'Pyramid', 'Radar', 'Polar'].indexOf(
        this.parent.chartSettings.chartSeries.type
      ) > -1 ||
        !this.parent.chartSettings.showMultiLevelLabels) &&
      (this.parent.dataType === 'olap'
        ? true
      ...
      this.pivotIndex = {
        rIndex: dataSource
          ? (dataSource as { [key: string]: number }[])[args.pointIndex].rIndex
          : undefined,
        cIndex: dataSource
          ? (dataSource as { [key: string]: number }[])[args.pointIndex].cIndex
          : undefined,
      };
      this.creatMenu();
      const pos: OffsetPosition = this.parent.element.getBoundingClientRect();
      const y: number =
        (this.parent.element.querySelector('.e-pivot-toolbar')
          ? this.parent.element.querySelector('.e-pivot-toolbar').clientHeight
          : 0) +
      ...
      (this.parent.allowDrillThrough ||
        this.parent.editSettings.allowEditing) &&
      this.parent.drillThroughModule
    ) {
      const rIndex: number = (dataSource as { [key: string]: number }[])[
        args.pointIndex
      ].rIndex;
      const cIndex: number = (dataSource as { [key: string]: number }[])[
        args.pointIndex
      ].cIndex;
      this.parent.drillThroughModule.executeDrillThrough(
        this.parent.pivotValues[rIndex as number][cIndex as number] as IAxisSet,
        rIndex,
        cIndex
      );
    }
  ...
  private frameAxesWithRows(): {
    ...
    const percentAggregateTypes: SummaryTypes[] = [
    ...
    if (this.chartSettings.enableMultipleAxis) {
      ...
      for (const item of this.dataSourceSettings.values) {
        ...
        const format: string =
          PivotUtil.inArray(
            measureField.aggregateType,
            percentAggregateTypes
          ) !== -1
            ? 'P2'
            : formatSetting
            ? formatSetting.format.toLowerCase().match(/n|p|c/) === null
              ? 'N'
              : formatSetting.format
            : this.parent.dataType === 'olap'
            ? this.getFormat(measureField.formatString)
            : 'N';
        ...
        currentYAxis = this.persistSettings.primaryYAxis
          ? this.frameObjectWithKeys(
              this.persistSettings.primaryYAxis as {
                [key: string]: Object;
              }
            )
          : currentYAxis;
        currentYAxis.labelFormat = currentYAxis.labelFormat
          ? currentYAxis.labelFormat
          : percentChart
          ? ''
          : !resFormat
          ? format
        ...
        currentYAxis.title =
          this.chartSettings.multipleAxisMode === 'Combined'
            ...
            : currentYAxis.title
            ? currentYAxis.title
            : measureAggregatedName;
        currentYAxis.zoomFactor = isNullOrUndefined(
          this.chartSettings.primaryYAxis.zoomFactor
        )
          ? 1
          : this.chartSettings.primaryYAxis.zoomFactor;
        currentYAxis.edgeLabelPlacement = this.chartSettings.primaryYAxis
          .edgeLabelPlacement
          ? this.chartSettings.primaryYAxis.edgeLabelPlacement
          : this.persistSettings.primaryYAxis.edgeLabelPlacement;
        if (
          this.chartSettings.chartSeries.type === 'Bar' ||
          this.chartSettings.chartSeries.type === 'StackingBar' ||
          this.chartSettings.chartSeries.type === 'StackingBar100'
        ) {
          currentYAxis.plotOffsetRight = currentYAxis.plotOffsetRight
            ? currentYAxis.plotOffsetRight
            : 30;
        } else {
          currentYAxis.plotOffsetTop = currentYAxis.plotOffsetTop
            ? currentYAxis.plotOffsetTop
            : 30;
        }
        if (!resFormat) {
          currentYAxis.name =
      ...
      const measureField: IField =
        this.engineModule.fieldList[this.currentMeasure];
      const measureAggregatedName: string =
        (this.parent.dataType === 'olap'
          ? ''
          : this.parent.localeObj.getConstant(measureField.aggregateType) +
      ...
      let currentYAxis: AxisModel = {};
      const format: string =
        PivotUtil.inArray(measureField.aggregateType, percentAggregateTypes) !==
        -1
          ? 'P2'
          : formatSetting
          ? formatSetting.format
              .toLowerCase()
              .match(/n[0-10]|p[0-10]|c[0-10]/) === null || lengthofFormat > 3
            ? 'N'
            : formatSetting.format
          : this.parent.dataType === 'olap'
          ? this.getFormat(measureField.formatString)
      ...
      if (
        ...
        currentYAxis.name = this.measuresNames[this.currentMeasure]
          ? this.measuresNames[this.currentMeasure]
          : this.currentMeasure;
      }
      currentYAxis.labelFormat = currentYAxis.labelFormat
        ? currentYAxis.labelFormat
        ...
        ? ''
        : format;
      currentYAxis.title = currentYAxis.title
        ? currentYAxis.title
        : measureAggregatedName;
      axes = axes.concat(currentYAxis);
  ...
  private groupHierarchyWithLevels(
    ...
    this.measurePos = (
      this.engineModule as OlapEngine
    ).tupRowInfo[0].measurePosition;
        ...
        if (isNullOrUndefined(group[cell.hierarchy])) {
          if (cell.memberType === 3) {
            if (fieldCount === this.measurePos) {
              ...
              group[cell.hierarchy] = {
                [cell.levelUniqueName]: cell.levelUniqueName,
              };
            } else {
              fieldCount--;
            }
      ...
      if (this.measurePos !== pos) {
        levelPos[fieldPosition[pos as number]] = {
          start: lastEnd + 1,
            ...
            lastEnd + Object.keys(group[fieldPosition[pos as number]]).length,
        };
  ...
  private frameMultiLevelLabels(): MultiLevelLabelsModel[] {
    ...
    for (const startKey of startKeys) {
      ...
      for (const levelKey of lKeys) {
        ...
        if (gRows[lKey as number]) {
          ...
          if (
            headers[lKey as number].levelName ===
            parentHeaders[lKey as number].levelName
          ) {
            gRows[lKey as number][len - 1].end =
              (gRows[lKey as number][len - 1].end as number) + 1;
          } else {
            gRows[lKey as number].push({
              start: sKey,
              end: sKey + 1,
              text: headers[lKey as number].text,
              type:
                headers[lKey as number].span === -1
    ...
    for (let level: number = levellength - 1; level > -1; level--) {
      multiLevelLabels.push({
        categories: gRows[level as number] as MultiLevelCategoriesModel[],
        border: { width: 1 },
        overflow: 'Trim',
      });
    }
  ...
  private getZoomFactor(): number {
    ...
    const seriesLength: number =
      this.chartSeries.length * 10 > 120 ? this.chartSeries.length * 10 : 120;
    let zoomFactor: number =
      this.chartSeries.length > 0
        ? this.calculatedWidth /
          (Object.keys(this.chartSeries[0].dataSource).length * seriesLength)
        : 1;
  ...
  public getCalulatedWidth(): number {
    if (!isNaN(Number(this.parent.width))) {
      ...
      this.calculatedWidth =
        ...
        (parseFloat(this.parent.width as string) / 100);
    } else if ((this.parent.width as string).indexOf('px') > -1) {
      this.calculatedWidth = Number(
        this.parent.width.toString().split('px')[0]
      );
    } else {
  ...
  private configTooltipSettings(): TooltipSettingsModel {
    const tooltip: TooltipSettingsModel = this.persistSettings.tooltip
      ? this.persistSettings.tooltip
    ...
    if (this.parent.tooltipTemplate) {
      tooltip.template = tooltip.template
        ? tooltip.template
        : this.parent.tooltipTemplate;
    }
    tooltip.header = tooltip.header ? tooltip.header : '';
    tooltip.enableMarker =
      tooltip.enableMarker === undefined ? true : tooltip.enableMarker;
    return tooltip;
  }
  ...
  private configLegendSettings(): LegendSettingsModel {
    ...
    if (this.chartSettings.legendSettings) {
      ...
      while (keyPos < keys.length) {
        legendSettings[keys[keyPos as number]] = (
          this.chartSettings.legendSettings as {
            [key: string]: Object;
          }
    ...
    legendSettings.padding = legendSettings.padding
      ? legendSettings.padding
      : 25;
    legendSettings.shapePadding = legendSettings.shapePadding
  ...
  private configXAxis(): AxisModel {
    ...
    currentXAxis = this.persistSettings.primaryXAxis
      ? this.frameObjectWithKeys(
          this.persistSettings.primaryXAxis as {
            [key: string]: Object;
          }
        )
      : currentXAxis;
    currentXAxis.valueType = 'Category';
    currentXAxis.labelIntersectAction = currentXAxis.labelIntersectAction
      ? currentXAxis.labelIntersectAction
      : 'Rotate45';
    currentXAxis.title = currentXAxis.title
      ? currentXAxis.title
      : this.dataSourceSettings.rows
          .map((args: IFieldOptions) => {
            return args.caption || args.name;
          })
          .join(' / ');
    currentXAxis.zoomFactor = isNullOrUndefined(
      this.parent.chartSettings.primaryXAxis.zoomFactor
    )
      ? this.getZoomFactor()
      : this.parent.chartSettings.primaryXAxis.zoomFactor;
  ...
  private configZoomSettings(): ZoomSettingsModel {
    ...
    zoomSettings.enableSelectionZooming =
      zoomSettings.enableSelectionZooming === undefined
        ? true
        : zoomSettings.enableSelectionZooming;
    zoomSettings.enableScrollbar =
      zoomSettings.enableScrollbar === undefined
        ? true
        : zoomSettings.enableScrollbar;
    return zoomSettings;
  }
  ...
  private tooltipRender(args: ITooltipRenderEventArgs): void {
    const measure: string =
      this.chartSettings.enableMultipleAxis &&
      this.chartSettings.multipleAxisMode === 'Combined' &&
      this.accumulationType.indexOf(this.chartSettings.chartSeries.type) < 0 &&
      this.chartSettings.chartSeries.type !== 'Radar' &&
      this.chartSettings.chartSeries.type !== 'Polar'
        ? (args.series as Series).tooltipMappingName
        : (args.series as Series).yAxisName
        ? (args.series as Series).yAxisName.split('_CumulativeAxis')[0]
        : this.chartSettings.enableMultipleAxis &&
          this.accumulationType.indexOf(this.chartSettings.chartSeries.type) <
            0 &&
          this.chartSettings.chartSeries.type !== 'Pareto'
        ? args.series.name
          ? args.series.name.split(' | ')[1]
          : args.data.seriesName
          ? args.data.seriesName.split(' | ')[1]
          : this.currentMeasure
        : this.measuresNames[this.currentMeasure]
        ? this.measuresNames[this.currentMeasure]
        : this.currentMeasure;
    ...
    const rowIndex: number = dataSource
      ? (dataSource as { [key: string]: number }[])[args.data.pointIndex].rIndex
      : undefined;
    const colIndex: number = dataSource
      ? (dataSource as { [key: string]: number }[])[args.data.pointIndex].cIndex
      : undefined;
    const measureField: IField =
    ...
    const measureAggregatedName: string =
      (this.parent.dataType === 'olap'
        ? ''
        : aggregateType + ' ' + this.parent.localeObj.getConstant('of') + ' ') +
      measureField.caption;
    const formattedText: string = (
      this.engineModule.pivotValues[rowIndex as number][
        colIndex as number
      ] as IAxisSet
    ).formattedText;
    const formatField: IField = this.engineModule.formatFields[measureField.id];
    const valueFormat: string | IAxisSet = this.engineModule.getFormattedValue(
      args.point.y as number,
      measureField.id,
      formattedText
    );
    const formattedValue: string =
      formatField &&
      formatField.format &&
      formatField.format.toLowerCase().match(/n|p|c/) !== null &&
      this.chartSettings.useGroupingSeparator
        ? this.parent.dataType === 'olap'
          ? valueFormat.toString()
          : (valueFormat as IAxisSet).formattedText
        : formattedText;
    let text: string | number | Date = (
      this.parent.pivotValues[rowIndex as number][
        colIndex as number
      ] as IAxisSet
    ).columnHeaders;
    ...
    if (isNullOrUndefined(text)) {
      ...
      columnText =
        this.chartSeriesInfo[text.toString().split(/~~|::/).join(' - ')]
          .uniqueName;
    } else {
    ...
    if (
      (this.parent.tooltipTemplate &&
        this.parent.getTooltipTemplate() !== undefined) ||
      this.chartSettings.tooltip.template
    ) {
      const rowFields: string = dataSource
        ? this.parent.getHeaderField(rowIndex, colIndex, 'row')
        : '';
      const columnFields: string = dataSource
        ? this.parent.getHeaderField(rowIndex, colIndex, 'Column')
        : '';
      ...
      if (
        ...
        this.parent.chartSettings.tooltip &&
        this.parent.chartSettings.tooltip.enable &&
        this.parent.chartSettings.tooltip.template
      ) {
        template = this.tooltipTemplateFn()(
          templateVariable,
          this,
          'tooltipTemplate',
          this.element.id + '1tooltipTemplate'
        )[0].outerHTML;
      } else {
        template = this.parent.getTooltipTemplate()(
          ...
          this,
          'tooltipTemplate',
          this.element.id + 'tooltipTemplate'
        )[0].outerHTML;
      }
      ...
      args.text =
        ...
        (this.dataSourceSettings.columns.length === 0
          ? ''
          : ' <br/>' +
            this.parent.localeObj.getConstant('column') +
            ': ' +
            columnText) +
        (this.dataSourceSettings.rows.length === 0
          ? ''
          : ' <br/>' +
            this.parent.localeObj.getConstant('row') +
            ': ' +
            rowText);
  ...
  private loaded(args: ILoadedEventArgs): void {
    ...
    const width: string = this.parent.grid
      ? this.parent.getGridWidthAsNumber().toString()
      : this.parent.getWidthAsNumber().toString();
    if (
      this.parent.chart &&
      this.parent.showGroupingBar &&
      this.parent.groupingBarModule &&
      this.parent.showFieldList &&
      this.parent.currentView === 'Chart'
    ) {
    ...
    if (this.chartSettings.showMultiLevelLabels) {
      const multilabelAxisName: string =
        PivotUtil.inArray(this.chartSettings.chartSeries.type, [
          'Bar',
          'StackingBar',
          'StackingBar100',
        ]) > -1
          ? '_chartYAxisMultiLevelLabel0'
          : '_chartXAxisMultiLevelLabel0';
      if (
        !isNullOrUndefined(
          select(
            '#' + this.parent.element.id + multilabelAxisName,
            this.parent.element
          )
        )
      ) {
        this.parent.element
          .querySelector('#' + this.parent.element.id + multilabelAxisName)
    ...
    if (
      ...
      if (
        [
          ...
          'Polar',
          'Pareto',
        ].indexOf(this.parent.chartSettings.chartSeries.type) >= 0
      ) {
        (
          this.parent.element.querySelector('.' + cls.PIVOTCHART) as HTMLElement
    ...
    if (
      this.parent.displayOption.primary === 'Chart' ||
      this.parent.displayOption.view === 'Chart'
    ) {
      this.parent.notify(events.contentReady, {});
    }
    ...
    if (
      (this.parent.dataSourceSettings.mode === 'Server' &&
        this.parent.isServerWaitingPopup) ||
      this.parent.dataSourceSettings.mode === 'Local'
    ) {
      this.parent.hideWaitingPopup();
    }
  }
  ...
  public updateView(): void {
    if (this.parent.grid && this.parent.chart && this.parent.showToolbar) {
      const groupingTable: HTMLElement = this.parent.element.querySelector(
        '.e-pivot-grouping-bar'
      ) as HTMLElement;
      const groupingChartTable: HTMLElement = this.parent.element.querySelector(
        '.e-chart-grouping-bar'
      ) as HTMLElement;
      if (this.parent.currentView === 'Table') {
        ...
        if (this.parent.showGroupingBar && this.parent.groupingBarModule) {
          if (groupingTable) {
            ...
            if (
              groupingTable.querySelector('.' + cls.ALL_FIELDS_PANEL_CLASS) &&
              this.parent.groupingBarModule.gridPanel != null &&
        ...
        if (
          ...
          (
            this.parent.element.querySelector('.e-pivotchart') as HTMLElement
          ).style.display = 'none';
        }
      } else {
        this.parent.grid.element.style.display = 'none';
        ...
        if (this.parent.showGroupingBar && this.parent.groupingBarModule) {
          ...
          if (groupingChartTable) {
            ...
            if (
              groupingChartTable.querySelector(
                '.' + cls.ALL_FIELDS_PANEL_CLASS
              ) &&
              this.parent.groupingBarModule.chartPanel != null &&
              !this.parent.groupingBarModule.chartPanel.isDestroyed
            ) {
  ...
  private creatMenu(): void {
    ...
    const items: string[] =
      (this.parent.allowDrillThrough ||
        this.parent.editSettings.allowEditing) &&
      this.parent.drillThroughModule
        ? ['expand', 'collapse', 'drillThrough', 'exit']
    ...
    const getString: { key: number; type: string; value: string }[] =
      this.getMenuItems();
    ...
    for (let i: number = 0; i < getString.length; i++) {
      if (getString[i as number].type === 'expand') {
        expand.push({
          id: this.element.id + 'drillExpand_' + getString[i as number].key,
          text: getString[i as number].value,
        });
      } else {
        collapse.push({
          id: this.element.id + 'drillCollapse_' + getString[i as number].key,
          text: getString[i as number].value,
        });
      }
    }
    ...
    const menuOptions: ContextMenuModel = {
      cssClass:
        ...
        '_accumulationChart' +
        (this.parent.cssClass ? ' ' + this.parent.cssClass : ''),
      items: option,
      enableRtl: this.parent.enableRtl,
  ...
  private drillMenuOpen(args: BeforeOpenCloseMenuEventArgs): void {
    if (
      args.items[0] &&
      args.items[0].text === this.parent.localeObj.getConstant('expand') &&
      args.items[0].items &&
      args.items[0].items.length === 0
    ) {
      this.accumulationMenu.enableItems(
    ...
    if (
      args.items[1] &&
      args.items[1].text === this.parent.localeObj.getConstant('collapse') &&
      args.items[1].items &&
      args.items[1].items.length === 0
    ) {
      this.accumulationMenu.enableItems(
  ...
  private getMenuItems(): { key: number; type: string; value: string }[] {
    ...
    const pivotValues: IAxisSet[][] = this.engineModule
      .pivotValues as IAxisSet[][];
    let levelCol: number[] = [];
    let pivotValue: IAxisSet =
      pivotValues[rowIndex as number][this.pivotIndex.cIndex];
    // let hierarchy: string = pivotValues[rowIndex][0].hierarchy;
    let level: number =
      !pivotValues[rowIndex as number][0].isNamedSet &&
      pivotValues[rowIndex as number][0].hasChild
        ? pivotValues[rowIndex as number][0].level
        : undefined;
    const levels: string[] =
      this.parent.dataType === 'olap'
        ? pivotValue.rowHeaders.toString().split(/~~|::/)
        : pivotValue.rowHeaders
            .toString()
            .split(this.engineModule.valueSortSettings.headerDelimiter);
    while (pivotValues[rowIndex as number][0]) {
      pivotValue = pivotValues[rowIndex as number][0];
      if (
        levels.length !== 0 &&
        levels.indexOf(pivotValue.formattedText.toString()) ===
          levels.length - 1
      ) {
        if (
          pivotValue.hasChild &&
          !pivotValue.isNamedSet &&
          levelCol.indexOf(pivotValue.level) < 0 &&
          (level
        ...
        const index: number = levels.indexOf(
          pivotValue.formattedText.toString()
        );
  ...
  private drillMenuSelect(args: MenuEventArgs): void {
    const pivotValues: IAxisSet[][] = (
      this.parent.dataType === 'olap'
        ? this.parent.olapEngineModule.pivotValues
        : this.parent.engineModule.pivotValues
    ) as IAxisSet[][];
    const option: string = args.element.id.split('_DrillMenuChart_')[1];
    if (args.element.id.indexOf(this.element.id + 'drill') === 0) {
      const type: string =
        args.element.id.split(this.element.id + 'drill')[1].indexOf('Expand') >=
        0
          ? 'drillExpand'
          : 'drillCollapse';
      const rowIndex: number = Number(
        args.element.id.split(this.element.id + type + '_')[1]
      );
      const pivotValue: IAxisSet = pivotValues[rowIndex as number][0];
      const name: string =
        this.parent.dataType === 'olap'
          ? pivotValue.formattedText
          : pivotValue.actualText
          ? pivotValue.actualText.toString()
          : pivotValue.formattedText.toString();
      const text: string = pivotValue.formattedText
        ? pivotValue.formattedText.toString()
        : name;
      const caption: string =
        pivotValue.hasChild && !pivotValue.isNamedSet
          ? (pivotValue.isDrilled ? ' - ' : ' + ') + text
          : text;
      const tupInfo: ITupInfo =
        this.parent.dataType === 'olap'
          ? (this.engineModule as OlapEngine).tupRowInfo[pivotValue.ordinal]
          : undefined;
      const levelName: string = tupInfo
        ? tupInfo.uNameCollection
        : pivotValue.valueSort.levelName.toString();
      ...
      this.parent.drillThroughModule.executeDrillThrough(
        pivotValues[this.pivotIndex.rIndex][this.pivotIndex.cIndex] as IAxisSet,
        this.pivotIndex.rIndex,
        this.pivotIndex.rIndex
      );
    } else if (option === 'exit') {
  ...
  public getChartHeight(): string {
    ...
    if (!isNullOrUndefined(this.parent.getHeightAsNumber())) {
      ...
      if (
        this.parent.element.querySelector('.e-chart-grouping-bar') !== null &&
        (
      ...
      if (this.parent.showToolbar && this.parent.showGroupingBar) {
        ...
        (this.parent.chart && parseInt(this.parent.chart.height, 10) < 200) ||
        offSetHeight < 200
      ) {
        height = '200';
      }
  ...
  private getChartAutoHeight(): number {
    let height: number =
      this.parent.element.offsetHeight < this.parent.minHeight
  ...
  private axisLabelRender(args: IAxisLabelRenderEventArgs): void {
    ...
    if (args.axis.name !== 'primaryXAxis') {
      ...
      const valueFormat: string | IAxisSet =
        this.engineModule.getFormattedValue(
          args.value,
          this.chartSettings.enableMultipleAxis &&
            this.chartSettings.multipleAxisMode === 'Combined'
            ? this.currentMeasure
  ...
  private multiLevelLabelClick(args: IMultiLevelLabelClickEventArgs): void {
    const eventArgs: MultiLevelLabelClickEventArgs = {
      axis: args.axis,
      text: args.text,
      cell: !isNullOrUndefined(args.customAttributes)
        ? (args.customAttributes as ChartLabelInfo).cell
        : undefined,
      cancel: false,
    };
    ...
    if (
      ...
      (args.customAttributes as ChartLabelInfo).hasChild &&
   ...
   */
  public onDrill(
    args: IMultiLevelLabelClickEventArgs | { [key: string]: Object }
  ): void {
    const labelInfo: { [key: string]: Object } = args.customAttributes as {
      [key: string]: Object;
    };
    let delimiter: string =
      this.dataSourceSettings.drilledMembers[0] &&
      this.dataSourceSettings.drilledMembers[0].delimiter
        ? this.dataSourceSettings.drilledMembers[0].delimiter
        : '**';
    const fieldName: string = labelInfo.fieldName as string;
    const currentCell: IAxisSet = this.engineModule.pivotValues[
      labelInfo.rowIndex as number
    ][labelInfo.colIndex as number] as IAxisSet;
    let memberUqName: string = (currentCell.valueSort.levelName as string)
      .split(this.engineModule.valueSortSettings.headerDelimiter)
      .join(delimiter);
    ...
    if (this.dataSourceSettings.drilledMembers.length === 0) {
      this.parent.setProperties(
        {
          dataSourceSettings: {
            drilledMembers: [
              { name: fieldName, items: [memberUqName], delimiter: delimiter },
            ],
          },
        },
        true
      ...
      for (
        let fCnt: number = 0;
        fCnt < this.dataSourceSettings.drilledMembers.length;
        fCnt++
      ) {
        const field: DrillOptionsModel =
          this.dataSourceSettings.drilledMembers[fCnt as number];
        memberUqName = memberUqName
          .split(delimiter)
          .join(field.delimiter ? field.delimiter : delimiter);
        delimiter = field.delimiter = field.delimiter
          ? field.delimiter
          : delimiter;
    ...
    const enginePopulatingEventArgs: EnginePopulatingEventArgs = {
      dataSourceSettings: PivotUtil.getClonedDataSourceSettings(
    ...
    if (pivot.parent.enableVirtualization || pivot.parent.enablePaging) {
      if (pivot.parent.dataSourceSettings.mode === 'Server') {
        ...
        pivot.engineModule.drilledMembers =
      ...
      (pivot.engineModule as PivotEngine).generateGridData(
        pivot.dataSourceSettings,
        true
      );
    }
  ...
  private isAttributeDrill(
    ...
    for (
      let i: number = 0;
      i < this.dataSourceSettings.drilledMembers.length;
      i++
    ) {
      if (
        this.dataSourceSettings.drilledMembers[i as number].name === hierarchy
      ) {
        for (
          let j: number = 0;
          j < this.dataSourceSettings.drilledMembers[i as number].items.length;
          j++
        ) {
          const drillItems: string[] = this.dataSourceSettings.drilledMembers[
            i as number
          ].items[j as number].split(
            this.dataSourceSettings.drilledMembers[i as number].delimiter
          );
          let levelName: string = '';
          for (let k: number = 0; k < drillItems.length; k++) {
            if (drillInfo[k as number] && drillInfo[k as number].uName) {
              levelName =
                ...
                (levelName === ''
                  ? ''
                  : this.dataSourceSettings.drilledMembers[i as number]
                      .delimiter) +
                (drillInfo[k as number].uName.indexOf('[Measures]') > -1
                  ? '[Measures]'
          ...
          if (
            levelName ===
            this.dataSourceSettings.drilledMembers[i as number].items[
              j as number
            ]
          ) {
            isDrill = true;
  ...
  private resized(args: IResizeEventArgs): void {
    if (
      ...
      (args.chart as Chart).primaryXAxis.zoomFactor = isNullOrUndefined(
        this.parent.chartSettings.primaryXAxis.zoomFactor
      )
        ? this.getZoomFactor()
        : this.parent.chartSettings.primaryXAxis.zoomFactor;
  ...
  public getResizedChartHeight(): string {
    const height: string =
      ...
      this.parent.chartSettings.enableMultipleAxis &&
      this.parent.dataSourceSettings.values.length > 0
        ? Number(this.parent.chart.height) >
          this.parent.dataSourceSettings.values.length * 235 + 100
          ? isNaN(Number(this.getChartHeight()))
            ? this.getChartHeight().toString()
            : (Number(this.getChartHeight()) - 5).toString()
          : !isNaN(Number(this.getChartHeight())) ||
            this.parent.dataSourceSettings.values.length > 1
          ? (
              this.parent.dataSourceSettings.values.length * 235 +
