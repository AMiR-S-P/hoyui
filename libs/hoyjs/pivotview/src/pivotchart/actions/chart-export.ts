...
import { Chart, AccumulationChart } from '@hoyui/charts';
...
import { PdfExportProperties } from '@hoyui/grids';
...
export class ChartExport {
  ...
  public pdfChartExport(
    ...
    let element: HTMLCanvasElement = this.parent.chart
    ...
    const url: string = window.URL.createObjectURL(
      new Blob([new XMLSerializer().serializeToString(chartInfo.svg)], {
        type: 'image/svg+xml',
      })
    );
  ...
  private getChartInfo(
    controls: (Chart | AccumulationChart)[],
    isVertical?: boolean
  ): IChartInfo {
    let width: number = 0;
    let height: number = 0;
  ...
  private exportPdf(
    ...
    const fileName: string = this.exportProperties.fileName
    ...
    if (this.exportProperties.width || this.exportProperties.height) {
      pdfDocument.pageSettings.orientation =
      ...
      pdfDocument.pageSettings.size = new SizeF(
        this.exportProperties.width
          ? this.exportProperties.width
          : documentWidth,
        this.exportProperties.height
      ...
      if (
        ...
        pdfDocument.pageSettings.size = PivotUtil.getPageSize(
    ...
    let imageString: string = element
    ...
    pdfPage.graphics.drawImage(
      ...
      documentHeight < height ||
        this.exportProperties.width ||
        pdfDocument.pageSettings.size
        ? pdfPage.getClientSize().width
        : chartWidth,
      documentHeight < height ? pdfPage.getClientSize().height : height
    );
