import { createElement, remove, select } from '@hoyui/base';
...
export class PivotContextMenu {
  ...
  private onBeforeMenuOpen(args: MenuEventArgs): void {
    const items: HTMLLIElement[] = [].slice.call(
      args.element.querySelectorAll('li')
    );
    const fieldType: SummaryTypes | string =
      this.parent.dataType === 'olap'
        ? this.fieldElement.getAttribute('data-type')
        : (this.fieldElement
            .querySelector('.' + cls.PIVOT_BUTTON_CONTENT_CLASS)
            .getAttribute('data-type') as SummaryTypes);
    removeClass(items, cls.MENU_DISABLE);
    if (
      fieldType === 'CalculatedField' ||
      fieldType === 'isMeasureFieldsAvail'
    ) {
      for (const item of items) {
        if (
          item.textContent !== this.parent.localeObj.getConstant('addToValue')
        ) {
      ...
      for (const item of items) {
        if (
          item.textContent !== this.parent.localeObj.getConstant('addToRow') &&
  ...
  private onSelectContextMenu(menu: MenuEventArgs): void {
    if (menu.element.textContent !== null) {
      ...
      const dropClass: string = menu.item.id
        .replace(this.parent.element.id + '_', '')
        .toLowerCase();
      ...
      this.parent.pivotCommon.dataSourceUpdate.updateDataSource(
        fieldName,
        dropClass,
        -1
      );
  ...
  public destroy(): void {
    ...
    if (this.menuObj && !this.menuObj.isDestroyed) {
      ...
      if (
        select('#' + this.parent.element.id + '_PivotContextMenu', document)
      ) {
        remove(
          select('#' + this.parent.element.id + '_PivotContextMenu', document)
        );
