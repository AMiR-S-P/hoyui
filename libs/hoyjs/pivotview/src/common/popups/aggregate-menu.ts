...
import { MouseEventArgs, SanitizeHtmlHelper } from '@hoyui/base';
...
export class AggregateMenu {
  ...
  private openContextMenu(args: MouseEventArgs): void {
    ...
    ).parentElement.getAttribute('data-uid');
    const fieldInfo: FieldItemInfo = PivotUtil.getFieldInfo(
      fieldName,
      this.parent
    );
    this.buttonElement = (args.target as HTMLElement).parentElement;
    const isStringField: number =
      this.parent.engineModule.fieldList[fieldName as string].type !== 'number'
        ? 1
        : 0;
    ...
    const control: PivotView | PivotFieldList =
      ...
      (this.parent as PivotFieldList).isPopupView
        ...
        (observedArgs: AggregateMenuOpenEventArgs) => {
          if (!observedArgs.cancel) {
            ...
            this.createContextMenu(
              isStringField,
              summaryTypes,
              observedArgs.displayMenuCount
            );
            this.currentMenu = args.target as Element;
            const pos: OffsetPosition =
              this.currentMenu.getBoundingClientRect();
  ...
  private createContextMenu(
    isStringField: number,
    summaryTypes: AggregateTypes[],
    displayMenuCount?: number
  ): void {
    const menuItems: MenuItemModel[][] = [];
    menuItems[isStringField as number] = [];
    if (
      this.menuInfo[isStringField as number] &&
      !this.menuInfo[isStringField as number].isDestroyed
    ) {
    ...
    for (let i: number = 0; i < summaryTypes.length; i++) {
      ...
      if (isStringField) {
        if (
          this.stringAggregateTypes.indexOf(key) > -1 &&
    ...
    const menuOptions: ContextMenuModel = {
      ...
      onClose: () => {
        (
          select('#' + this.buttonElement.id, this.parentElement) as HTMLElement
        ).focus();
      },
      select: this.selectOptionInContextMenu.bind(this),
    };
    let contextMenu: HTMLElement = document.getElementById(
      this.parent.element.id +
        (isStringField
          ? 'valueFieldStringContextMenu'
  ...
  private beforeMenuOpen(
    ...
    args.element.style.zIndex = (
      this.menuInfo[isString as number].element.style.zIndex + 3
    ).toString();
   ...
   * @returns {void}
   * @hidden */
  public createValueSettingsDialog(
    target: HTMLElement,
    ...
    const valueDialogElement: HTMLElement = createElement('div', {
      ...
      attrs: {
        'data-field': target.getAttribute('data-uid')
          ? target.getAttribute('data-uid')
  ...
  private createFieldOptions(
    ...
    let summaryType: string =
      type && type !== 'MoreOption'
        ? type
        : buttonElement.getAttribute('data-type');
    ...
    for (let i: number = 0; i < summaryItems.length; i++) {
      if (
        this.parent.getAllSummaryType().indexOf(summaryItems[i as number]) >
          -1 &&
    ...
    const baseItemTypes: string[] = [
      'DifferenceFrom',
      'PercentageOfDifferenceFrom',
    ];
    const baseFieldTypes: string[] = [
      'DifferenceFrom',
      'PercentageOfDifferenceFrom',
    ...
    if (Object.keys(fieldDataSource).length === 0) {
      ...
      baseField =
        baseField &&
        baseField.toString() !== 'undefined' &&
        baseField.toString() !== 'null'
          ? baseField
          : (fieldDataSource[0].value as string);
      const fieldName: string =
        baseField.toString() !== 'undefined'
          ? baseField
          : (fieldDataSource[0].value as string);
      const isDateField: boolean = PivotUtil.isDateField(
        fieldName as string,
        this.parent.engineModule as PivotEngine
      );
      fieldItemDataSource = this.parent.engineModule.fieldList[
        fieldName as string
      ].dateMember.map((item: IAxisSet) =>
        isDateField
          ? item.formattedText
          : this.parent.engineModule.getFormattedValue(
              item.actualText,
              fieldName
            ).formattedText
      );
    ...
    baseItem =
      baseItem.toString() !== 'undefined' ? baseItem : fieldItemDataSource[0];
    ...
    const textWrappper: HTMLElement = createElement('div', {
      className: 'e-field-name-text-container',
    });
    const filterWrapperDiv1: HTMLElement = createElement('div', {
      className: 'e-field-option-container',
    });
    const optionWrapperDiv1: HTMLElement = createElement('div', {
      className: 'e-type-option-container',
    });
    const optionWrapperDiv2: HTMLElement = createElement('div', {
      className: 'e-base-field-option-container',
    });
    const optionWrapperDiv3: HTMLElement = createElement('div', {
      className: 'e-base-item-option-container',
    });
    const texttitle: HTMLElement = createElement('div', {
      className: 'e-field-name-title',
    });
    texttitle.innerText = this.parent.localeObj.getConstant('sourceName') + ' ';
    const textContent: HTMLElement = createElement('div', {
      className: 'e-field-name-content',
    });
    textContent.innerText = this.parent.enableHtmlSanitizer
      ? SanitizeHtmlHelper.sanitize(
          buttonElement.getAttribute('data-uid')
            ? buttonElement.getAttribute('data-uid')
    ...
    this.buttonElement = this.buttonElement
      ? this.buttonElement
      : document.querySelector('.' + textContent.innerText);
    const inputTextDiv1: HTMLElement = createElement('div', {
      className: 'e-type-option-text',
    });
    inputTextDiv1.innerText =
      this.parent.localeObj.getConstant('sourceCaption');
    const optionTextDiv1: HTMLElement = createElement('div', {
      className: 'e-base-field-option-text',
    });
    optionTextDiv1.innerText =
      this.parent.localeObj.getConstant('summarizeValuesBy');
    ...
    const inputDiv1: HTMLElement = createElement('div', {
      className: 'e-caption-input-container',
    });
    const dropOptionDiv1: HTMLElement = createElement('div', {
      id: this.parentElement.id + '_type_option',
    });
    const dropOptionDiv2: HTMLElement = createElement('div', {
      id: this.parentElement.id + '_base_field_option',
    });
    const dropOptionDiv3: HTMLElement = createElement('div', {
    ...
    const optionWrapper1: DropDownList = new DropDownList({
      ...
      // popupWidth: 'auto',
      cssClass:
        cls.VALUE_OPTIONS_CLASS +
        (this.parent.cssClass ? ' ' + this.parent.cssClass : ''),
      width: '100%',
      change: (args: ChangeEventArgs) => {
        optionWrapper2.enabled =
          baseFieldTypes.indexOf(args.value as string) !== -1 ? true : false;
        optionWrapper3.enabled =
          baseItemTypes.indexOf(args.value as string) !== -1 ? true : false;
        if (
          optionWrapper3.enabled &&
          (optionWrapper3.dataSource as string[]).length === 1
        ) {
    ...
    const optionWrapper2: DropDownList = new DropDownList({
      ...
      change: (args: ChangeEventArgs) => {
        ...
        fieldItemDataSource = popupInstance.parent.engineModule.fieldList[
          args.value as string
        ].dateMember.map((item: IAxisSet) =>
          isDateField
            ? item.formattedText
        ...
        optionWrapper3.filterBarPlaceholder =
          popupInstance.parent.localeObj.getConstant('example') +
          ' ' +
          fieldItemDataSource[0];
    ...
    const optionWrapper3: DropDownList = new DropDownList({
      ...
      filterBarPlaceholder:
        this.parent.localeObj.getConstant('example') +
        ' ' +
        fieldItemDataSource[0],
      enabled: baseItemTypes.indexOf(summaryType) !== -1 ? true : false,
      cssClass:
        cls.FILTER_OPERATOR_CLASS +
        (this.parent.cssClass ? ' ' + this.parent.cssClass : ''),
      width: '100%',
    });
  ...
  private selectOptionInContextMenu(menu: MenuEventArgs): void {
    if (menu.item.text !== null) {
      const buttonElement: HTMLElement = this.currentMenu
        .parentElement as HTMLElement;
      const fieldInfo: FieldItemInfo = PivotUtil.getFieldInfo(
        buttonElement ? buttonElement.getAttribute('data-uid') : '',
        this.parent
      );
      this.parent.actionObj.actionName = events.aggregateField;
      this.parent.actionObj.fieldInfo = fieldInfo.fieldItem;
      ...
      try {
        if (
          ...
          this.createValueSettingsDialog(
            buttonElement,
            this.parentElement,
            type
          );
          ...
          const contentElement: HTMLElement = buttonElement.querySelector(
            '.' + cls.PIVOT_BUTTON_CONTENT_CLASS
          ) as HTMLElement;
          ...
          captionName = this.parent.enableHtmlSanitizer
            ? SanitizeHtmlHelper.sanitize(captionName)
            : captionName;
          ...
          for (
            ...
            if (
              ...
              const dataSourceItem: IFieldOptions = extend(
                {},
                (<{ [key: string]: IFieldOptions }>valuefields[vCnt as number])
                  .properties
                  ? (<{ [key: string]: IFieldOptions }>(
                      valuefields[vCnt as number]
                    )).properties
                  : valuefields[vCnt as number],
                null,
                true
  ...
  private updateDataSource(isRefreshed?: boolean): void {
    if (
      (this.parent as PivotFieldList).isDeferLayoutUpdate === false ||
      ((this.parent as PivotFieldList).pivotGridModule &&
        (this.parent as PivotFieldList).pivotGridModule
          .pivotDeferLayoutUpdate === false) ||
  ...
  private updateValueSettings(): void {
    const valueDialog: Dialog = getInstance(
      select(
        '#' + this.parentElement.id + '_ValueDialog',
        document
      ) as HTMLElement,
      Dialog
    ) as Dialog;
    const dialogElement: HTMLElement = valueDialog.element;
    const captionInstance: MaskedTextBox = getInstance(
      select('#' + this.parentElement.id + 'type_input_option'),
      MaskedTextBox
    ) as MaskedTextBox;
    const summaryInstance: DropDownList = getInstance(
      select('#' + this.parentElement.id + '_type_option'),
      DropDownList
    ) as DropDownList;
    const baseFieldInstance: DropDownList = getInstance(
      select('#' + this.parentElement.id + '_base_field_option'),
      DropDownList
    ) as DropDownList;
    const baseItemInstance: DropDownList = getInstance(
      select('#' + this.parentElement.id + '_base_item_option'),
      DropDownList
    ) as DropDownList;
    ...
    if (this.parentElement.querySelector('.' + cls.PIVOT_BUTTON_CLASS)) {
      buttonElement = this.parentElement.querySelector(
        '.' +
          cls.PIVOT_BUTTON_CLASS +
          '.' +
          fieldName.replace(/[^A-Z0-9]/gi, '')
      ) as HTMLElement;
    }
    if (buttonElement) {
      const contentElement: HTMLElement = buttonElement.querySelector(
      ...
      buttonElement.setAttribute(
        'data-basefield',
        baseFieldInstance.value as string
      );
      buttonElement.setAttribute(
        'data-baseitem',
        baseItemInstance.value as string
      );
    }
    let selectedField: IFieldOptions = PivotUtil.getFieldByName(
      fieldName,
      this.parent.dataSourceSettings.values
    ) as IFieldOptions;
    selectedField = (<{ [key: string]: Object }>selectedField).properties
      ? (<{ [key: string]: Object }>selectedField).properties
      : selectedField;
  ...
  private removeDialog(): void {
    if (
      this.buttonElement &&
      select('#' + this.buttonElement.id, this.parentElement)
    ) {
      (
        select('#' + this.buttonElement.id, this.parentElement) as HTMLElement
      ).focus();
    }
    const element: HTMLElement = select(
      '#' + this.parentElement.id + '_ValueDialog',
      document
    );
    const valueDialog: Dialog = element
