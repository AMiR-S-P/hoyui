...
import { CheckBox, ChangeEventArgs as StateChange } from '@hoyui/buttons';
...
export class Toolbar {
  ...
  private createToolbar(): void {
    ...
    if (
      select(
        '#' + this.parent.element.id + 'pivot-toolbar',
        this.parent.element
      ) !== null
    ) {
      remove(
    ...
    if (
      ...
      select(
        '#' + this.parent.element.id + '_PivotFieldList',
        this.parent.element
      )
    ) {
      ...
      select(
        '#' + this.parent.element.id + ' .' + 'e-pivot-grouping-bar',
        this.parent.element
      )
    ) {
      this.parent.element.insertBefore(
      ...
      this.parent.element.insertBefore(
        element,
        select('#' + this.parent.element.id + '_grid', this.parent.element)
      );
    }
    this.toolbar = new tool({
      ...
      width: !this.parent.gridSettings.allowAutoResizing
        ? this.parent.grid
          ? this.parent.getGridWidthAsNumber() - 2
    ...
    if (this.parent[viewStr as keyof PivotView]) {
      ...
      this.toolbar[viewStr as keyof Object] =
        this.parent[viewStr as keyof PivotView];
    }
    if (
      this.parent.toolbarTemplate &&
      typeof this.parent.toolbarTemplate === 'string'
    ) {
      this.toolbar.appendTo(this.parent.toolbarTemplate);
      this.parent.element.replaceChild(
        this.toolbar.element,
        this.parent.element.querySelector('.' + cls.GRID_TOOLBAR)
      );
      this.toolbar.element.classList.add(cls.GRID_TOOLBAR);
    } else {
      this.toolbar.appendTo(element);
    }
    this.toolbar.width = this.parent.grid
      ? this.parent.getGridWidthAsNumber()
      : this.parent.getWidthAsNumber();
  ...
  private getItems(): ItemModel[] {
    const toolbar: ToolbarItems[] = (
      this.parent.toolbar as ToolbarItems[]
    ).filter(
    ...
    for (const item of toolbar) {
      switch (item) {
        ...
        case 'Chart':
          {
            const validTypes: boolean =
              this.parent.displayOption.view === 'Table';
            items.push({
              template:
                '<ul id="' + this.parent.element.id + 'chart_menu"></ul>',
              id: this.parent.element.id + 'chartmenu',
              cssClass:
                (validTypes ? cls.MENU_DISABLE : '') +
                (this.parent.cssClass ? ' ' + this.parent.cssClass : ''),
            });
          }
          break;
        ...
        case 'Export':
          items.push({
            template:
              '<ul id="' + this.parent.element.id + 'export_menu"></ul>',
        ...
        case 'SubTotal':
          items.push({
            template:
              '<ul id="' + this.parent.element.id + 'subtotal_menu"></ul>',
        ...
        case 'GrandTotal':
          items.push({
            template:
              '<ul id="' + this.parent.element.id + 'grandtotal_menu"></ul>',
        ...
        case 'Formatting':
          items.push({
            template:
              '<ul id="' + this.parent.element.id + 'formatting_menu"></ul>',
  ...
  private saveReport(args: ClickEventArgs): void {
    if (
      this.currentReport &&
      this.currentReport !== '' &&
      args.item.id === this.parent.element.id + 'save'
    ) {
      ...
      this.currentReport === '' &&
      (args.item.id === this.parent.element.id + 'save' ||
        args.item.id === this.parent.element.id + 'saveas')
    ) {
      this.parent.pivotCommon.errorDialog.createErrorDialog(
        this.parent.localeObj.getConstant('error'),
        this.parent.localeObj.getConstant('emptyReport')
      );
      return;
    } else {
  ...
  private mdxQueryDialog(): void {
    ...
    const mdxDialog: Dialog = getInstance(
      select('#' + this.parent.element.id + 'mdx-dialog', document),
      Dialog
    ) as Dialog;
    ...
    textarea.innerText = this.parent.olapEngineModule
      .getMDXQuery(this.parent.dataSourceSettings)
      .trim();
  ...
  private dialogShow(args: ClickEventArgs, action?: string): void {
    if (args) {
      const dialog: Dialog = getInstance(
        select('#' + this.parent.element.id + 'report-dialog', document),
        Dialog
      ) as Dialog;
  ...
  private actionClick(args: ClickEventArgs): void {
    const actionName: string =
      args.item.id === this.parent.element.id + 'new'
        ? events.addNewReport
        : args.item.id === this.parent.element.id + 'save'
        ? events.saveCurrentReport
        : args.item.id === this.parent.element.id + 'saveas'
        ? events.saveAsCurrentReport
        : args.item.id === this.parent.element.id + 'rename'
        ? events.renameCurrentReport
        : args.item.id === this.parent.element.id + 'remove'
        ? events.removeCurrentReport
        : args.item.id === this.parent.element.id + 'load'
        ? events.loadReports
        : args.item.id === this.parent.element.id + 'formatting'
        ? events.openConditionalFormatting
        : args.item.id === this.parent.element.id + 'numberFormatting'
        ? events.openNumberFormatting
        : args.item.id === this.parent.element.id + 'mdxQuery'
        ? events.MdxQuery
        : args.item.id === this.parent.element.id + 'fieldlist'
        ? events.showFieldList
        : '';
  ...
  private renderDialog(): void {
    if (
      select(
        '#' + this.parent.element.id + 'report-dialog',
        this.parent.element
      ) !== null
    ) {
      remove(
  ...
  private renderMDXDialog(): void {
    if (
      select(
        '#' + this.parent.element.id + 'mdx-dialog',
        this.parent.element
      ) !== null
    ) {
      remove(
        select('#' + this.parent.element.id + 'mdx-dialog', this.parent.element)
      );
    }
  ...
  private copyMDXQuery(): void {
    ...
    const textArea: HTMLInputElement = mdxDialog.element.querySelector(
      '.' + cls.MDX_QUERY_CONTENT
    );
    try {
      textArea.select();
  ...
  private okBtnClick(): void {
    ...
    const reportInput: HTMLInputElement = dialog.element.querySelector(
      '.' + cls.GRID_REPORT_INPUT
    ) as HTMLInputElement;
    ...
    if (!this.parent.localeObj) {
      this.parent = getInstance(
        select('#' + this.parent.element.id, document) as HTMLElement,
        PivotView
      ) as PivotView;
    }
    if (
      (dialog.header === this.parent.localeObj.getConstant('save') ||
        ...
        (observedArgs: FetchReportArgs) => {
          ...
          if (isExist) {
            _this.createConfirmDialog(
              ...
              _this.parent.localeObj.getConstant('replaceConfirmBefore') +
                '"' +
                reportInput.value +
                '"' +
      ...
      dialog.header === this.parent.localeObj.getConstant('new') &&
      reportInput.value &&
      reportInput.value !== ''
    ) {
      this.action = 'New';
      ...
      dialog.header === this.parent.localeObj.getConstant('rename') &&
      reportInput.value &&
      reportInput.value !== ''
    ) {
      ...
      if (isExist) {
        this.createConfirmDialog(
          this.parent.localeObj.getConstant('alert'),
          this.parent.localeObj.getConstant('replaceConfirmBefore') +
            '"' +
            reportInput.value +
            '"' +
  ...
  private createConfirmDialog(title: string, description: string): void {
    if (document.getElementById(this.parent.element.id + '_ConfirmDialog')) {
      remove(
        document.getElementById(this.parent.element.id + '_ConfirmDialog')
          .parentElement
      );
    }
    ...
    (
      confirmPopUp.element.querySelector('.e-dlg-header') as HTMLElement
    ).innerText = this.parent.enableHtmlSanitizer
      ? SanitizeHtmlHelper.sanitize(title)
      : title;
  }
  ...
  private okButtonClick(): void {
    ...
    const confirmPopUp: Dialog = getInstance(
      select('#' + this.parent.element.id + '_ConfirmDialog', document),
      Dialog
    ) as Dialog;
    confirmPopUp.hide();
  }
  ...
  private cancelButtonClick(): void {
    ...
    if (this.action === 'New') {
      ...
      if (
        select(
          '#' + this.parent.element.id + '_reportlist',
          this.parent.element
        )
      ) {
        const reportList: DropDownList = getInstance(
          select(
            '#' + this.parent.element.id + '_reportlist',
            this.parent.element
          ),
  ...
  public createChartMenu(): void {
    if (
      select('#' + this.parent.element.id + 'chart_menu', this.parent.element)
    ) {
      const chartMenuElement: HTMLElement = select(
      ...
      const toDisable: boolean =
        menuItems.length <= 0 || this.parent.displayOption.view === 'Table';
      menuItems.push({
        separator: true,
      });
      menuItems.push({
      ...
      let chartMenu: Menu = chartMenuElement
        ? (getInstance(chartMenuElement, Menu) as Menu)
        : null;
  ...
  private create(): void {
    ...
    if (
      select('#' + this.parent.element.id + 'export_menu', this.parent.element)
    ) {
      const exportMenuElement: HTMLElement = select(
    ...
    if (
      ...
      const subTotalMenuElement: HTMLElement = select(
        '#' + this.parent.element.id + 'subtotal_menu',
        this.parent.element
      );
      const menu: MenuItemModel[] = [
        {
          ...
          items: [
            ...
            {
              text: this.parent.localeObj.getConstant(
                'showSubTotalsColumnsOnly'
              ),
    ...
    if (
      ...
      const grandTotalMenuElement: HTMLElement = select(
        '#' + this.parent.element.id + 'grandtotal_menu',
        this.parent.element
      );
      const menu: MenuItemModel[] = [
    ...
    if (
      ...
      const formattingMenuElement: HTMLElement = select(
        '#' + this.parent.element.id + 'formatting_menu',
        this.parent.element
      );
      const menu: MenuItemModel[] = [
    ...
    this.parent.trigger(events.saveReport, saveArgs);
    if (
      select('#' + this.parent.element.id + '_reportlist', this.parent.element)
    ) {
      const reportListElement: HTMLElement = select(
        '#' + this.parent.element.id + '_reportlist',
        this.parent.element
      );
      const reports: FetchReportArgs = this.fetchReports();
      const reportList: DropDownList = new DropDownList({
        ...
        placeholder:
          this.currentReport === ''
            ? this.parent.localeObj.getConstant('reportList')
            : '',
        enableRtl: this.parent.enableRtl,
        locale: this.parent.locale,
        cssClass:
          cls.REPORT_LIST_DROP +
          (this.parent.cssClass ? ' ' + this.parent.cssClass : ''),
  ...
  private getCurrentReport(): string {
    ...
    if (
      this.parent.dataSourceSettings.type === 'CSV' &&
      this.parent.dataSourceSettings.mode !== 'Server'
    ) {
      const reportSettings: PivotView = JSON.parse(reportStr);
      (reportSettings.dataSourceSettings.dataSource as string[][]).splice(
        0,
        0,
        this.parent.engineModule.fields
      );
  ...
  private updateItemElements(): void {
    const itemElements: HTMLElement[] = [].slice.call(
      this.toolbar.element.querySelectorAll('.e-toolbar-item')
    );
    for (const element of itemElements) {
      if (element.querySelector('button')) {
        ...
        element
          .querySelector('.e-menu.e-menu-parent')
          .setAttribute('tabindex', '-1');
        if (element.querySelector('.e-menu-item.e-menu-caret-icon')) {
          element
            .querySelector('.e-menu-item.e-menu-caret-icon')
  ...
  private multipleAxesCheckbox(args: MenuEventArgs): void {
    if (this.parent.element.id + '_' + 'multipleAxes' === args.element.id) {
      ...
      const checkbox: CheckBox = new CheckBox({
        ...
        change: () => {
          document
            .getElementById(this.parent.element.id + '_' + 'multipleAxes')
            .click();
        },
        enableRtl: this.parent.enableRtl,
        locale: this.parent.locale,
      ...
      if (
        ['Pie', 'Funnel', 'Pyramid', 'Doughnut', 'Pareto'].indexOf(
      ...
      const checkboxObj: HTMLElement = this.parent.element.querySelector(
        '.' + cls.CHECKBOX_CONTAINER + '.e-multipleAxes'
      );
      args.element.appendChild(checkboxObj);
    } else if (
      this.parent.element.id + '_' + 'showLegend' ===
      args.element.id
    ) {
      const inputCheckbox: HTMLElement = createElement('input', {
      ...
      const checkbox: CheckBox = new CheckBox({
        ...
        change: () => {
          document
            .getElementById(this.parent.element.id + '_' + 'showLegend')
            .click();
        },
        enableRtl: this.parent.enableRtl,
        locale: this.parent.locale,
      ...
      checkbox.appendTo(inputCheckbox);
      const checkboxObj: HTMLElement = this.parent.element.querySelector(
        '.' + cls.CHECKBOX_CONTAINER + '.e-showLegend'
      );
      args.element.appendChild(checkboxObj);
    }
  }
  ...
  private getLableState(): boolean {
    const chartSettings: ChartSettingsModel = JSON.parse(
      this.parent.getChartSettings()
    ).chartSettings;
    if (
      chartSettings &&
      chartSettings.legendSettings &&
      chartSettings.legendSettings.visible !== undefined
    ) {
  ...
  private updateExportMenu(args: BeforeOpenCloseMenuEventArgs): void {
    const items: HTMLElement[] = [].slice.call(
      args.element.querySelectorAll('li')
    );
  ...
  private updateSubtotalSelection(args: BeforeOpenCloseMenuEventArgs): void {
    if (
      !(args.parentItem.id === this.parent.element.id + 'subtotalpositions')
    ) {
      if (
        !select(
          '#' +
            ...
            'subtotal' +
            ' .' +
            cls.PIVOT_SELECT_ICON,
          args.element
        ).classList.contains(cls.PIVOT_DISABLE_ICON)
      ) {
        select(
          '#' +
            this.parent.element.id +
            ...
            cls.PIVOT_SELECT_ICON,
          args.element
        ).classList.add(cls.PIVOT_DISABLE_ICON);
      }
      if (
        !select(
          '#' +
      ...
      if (
        ...
        select(
          '#' +
            ...
            cls.PIVOT_SELECT_ICON,
          args.element
        ).classList.remove(cls.PIVOT_DISABLE_ICON);
      } else if (
        this.parent.dataSourceSettings.showSubTotals &&
        !this.parent.dataSourceSettings.showRowSubTotals &&
        this.parent.dataSourceSettings.showColumnSubTotals
  ...
  private updateGrandtotalSelection(args: BeforeOpenCloseMenuEventArgs): void {
    if (
      !(args.parentItem.id === this.parent.element.id + 'grandtotalpositions')
    ) {
      if (
        !select(
          '#' +
      ...
      if (
        this.parent.dataSourceSettings.showGrandTotals &&
        this.parent.dataSourceSettings.showRowGrandTotals &&
  ...
  private updateReportList(): void {
    if (
      ...
      if (this.currentReport === '' && reportList.dataSource.length > 0) {
        reportList.value =
          reportList.dataSource[reportList.dataSource.length - 1];
        reportList.text =
          reportList.dataSource[reportList.dataSource.length - 1];
        this.currentReport =
          reportList.dataSource[reportList.dataSource.length - 1];
  ...
  private menuItemClick(args: ClickEventArgs): void {
    ...
    let type: string;
    const actionName: string =
      args.item.id === this.parent.element.id + 'grid'
        ? events.tableView
        : args.item.id === this.parent.element.id + '_' + 'Column'
        ? events.chartView
        : args.item.id === this.parent.element.id + '_' + 'Bar'
        ? events.chartView
        : args.item.id === this.parent.element.id + '_' + 'Line'
        ? events.chartView
        : args.item.id === this.parent.element.id + '_' + 'Area'
        ? events.chartView
        : args.item.id === this.parent.element.id + '_' + 'Scatter'
        ? events.chartView
        : args.item.id === this.parent.element.id + '_' + 'Polar'
        ? events.chartView
        : args.item.id === this.parent.element.id + '_' + 'ChartMoreOption'
        ? events.chartView
        : args.item.id === this.parent.element.id + '_' + 'multipleAxes'
        ? events.multipleAxis
        : args.item.id === this.parent.element.id + '_' + 'showLegend'
        ? events.showLegend
        : args.item.id === this.parent.element.id + 'pdf'
        ? events.pdfExport
        : args.item.id === this.parent.element.id + 'png'
        ? events.pngExport
        : args.item.id === this.parent.element.id + 'excel'
        ? events.excelExport
        : args.item.id === this.parent.element.id + 'csv'
        ? events.csvExport
        : args.item.id === this.parent.element.id + 'jpeg'
        ? events.jpegExport
        : args.item.id === this.parent.element.id + 'svg'
        ? events.svgExport
        : args.item.id === this.parent.element.id + 'notsubtotal'
        ? events.hideSubTotals
        : args.item.id === this.parent.element.id + 'subtotalrow'
        ? events.subTotalsRow
        : args.item.id === this.parent.element.id + 'subtotalcolumn'
        ? events.subTotalsColumn
        : args.item.id === this.parent.element.id + 'subtotal'
        ? events.showSubTotals
        : args.item.id === this.parent.element.id + 'notgrandtotal'
        ? events.hideGrandTotals
        : args.item.id === this.parent.element.id + 'grandtotalrow'
        ? events.grandTotalsRow
        : args.item.id === this.parent.element.id + 'grandtotalcolumn'
        ? events.grandTotalsColumn
        : args.item.id === this.parent.element.id + 'grandtotal'
        ? events.showGrandTotals
        : args.item.id === this.parent.element.id + 'numberFormattingMenu'
        ? events.numberFormattingMenu
        : args.item.id === this.parent.element.id + 'conditionalFormattingMenu'
        ? events.conditionalFormattingMenu
        : '';
    this.parent.actionObj.actionName = actionName;
    if (this.parent.actionBeginMethod()) {
      return;
          ...
          if (args.item && args.item.text) {
            if (type === 'ChartMoreOption') {
              ...
              if (this.parent.chartSettings.legendSettings) {
                this.parent.chartSettings.legendSettings.visible =
                  !this.showLableState;
              } else {
                this.parent.setProperties(
                  {
                    chartSettings: {
                      legendSettings: { visible: !this.showLableState },
                    },
                  },
                  true
                );
  ...
  private getValidChartType(): ChartSeriesType[] {
    ...
    for (let i: number = 0; i <= this.parent.chartTypes.length; i++) {
      ...
      if (
        this.getAllChartItems().indexOf(type) > -1 &&
        menuItems.indexOf(type) < 0
      ) {
        menuItems.push(type);
      }
    }
  ...
  private createChartTypeDialog(): void {
    const chartDialog: HTMLElement = this.parent.element.appendChild(
      createElement('div', {
  ...
  private removeDialog(): void {
    const chartTypesDialog: Dialog = select(
      '#' + this.parent.element.id + '_ChartTypeDialog',
      this.parent.element
    )
      ? (getInstance(
          select(
    ...
    if (document.getElementById(this.parent.element.id + '_ChartTypeDialog')) {
      remove(
        document.getElementById(this.parent.element.id + '_ChartTypeDialog')
      );
    }
  }
  private chartTypeDialogUpdate(): void {
    const chartType: ChartSeriesType = (
      getInstance(
        select('#' + this.parent.element.id + '_ChartTypeOption'),
        DropDownList
      ) as DropDownList
    ).value as ChartSeriesType;
    const checked: boolean = (
      getInstance(
        select('#' + this.parent.element.id + '_DialogMultipleAxis'),
        CheckBox
      ) as CheckBox
    ).checked;
    const checkedShow: boolean = (
      getInstance(
        select('#' + this.parent.element.id + '_DialogShowLabel'),
        CheckBox
      ) as CheckBox
    ).checked;
    ...
    if (this.chartLableState) {
      ...
      if (this.parent.chartSettings.legendSettings) {
        ...
        this.parent.setProperties(
          { chartSettings: { legendSettings: { visible: checkedShow } } },
          true
        );
      }
    }
    ...
    this.parent.chartSettings.multipleAxisMode = (
      getInstance(
        select('#' + this.parent.element.id + '_AxisModeOption'),
        DropDownList
      ) as DropDownList
    ).value as MultipleAxisMode;
    const chartTypesDialog: Dialog = select(
      '#' + this.parent.element.id + '_ChartTypeDialog',
      document
    )
      ? (getInstance(
          select('#' + this.parent.element.id + '_ChartTypeDialog', document),
          Dialog
        ) as Dialog)
      : null;
  ...
  private getDialogContent(): HTMLElement {
    const mainWrapper: HTMLElement = createElement('div', {
      className: 'e-chart-type-div-content',
    });
    const optionWrapperDiv: HTMLElement = createElement('div', {
      className: 'e-chart-type-option-container',
    });
    const axisModeWrapperDiv: HTMLElement = createElement('div', {
    ...
    axisModeTextDiv.innerText =
      this.parent.localeObj.getConstant('multipleAxisMode');
    const dropOptionDiv: HTMLElement = createElement('div', {
      id: this.parent.element.id + '_ChartTypeOption',
    });
    const dropModeOptionDiv: HTMLElement = createElement('div', {
    ...
    for (let i: number = 0; i < chartType.length; i++) {
      chartTypeDatasource.push({
        value: chartType[i as number],
        text: this.parent.localeObj.getConstant(
          chartType[i as number].toLowerCase()
        ),
      });
    }
    const optionWrapper: DropDownList = new DropDownList({
      ...
      value: this.parent.chartSettings.chartSeries.type
        ? this.parent.chartSettings.chartSeries.type
        : this.getValidChartType()[0],
      width: '100%',
    ...
    const axisModeWrapper: DropDownList = new DropDownList({
      ...
      locale: this.parent.locale,
      fields: { value: 'value', text: 'text' },
      value: this.parent.chartSettings.multipleAxisMode
        ? this.parent.chartSettings.multipleAxisMode
        : 'Stacked',
      width: '100%',
  ...
  private changeDropDown(args: ChangeEventArgs): void {
    ...
    if (
      ...
      const multipleAxisCheckBox: CheckBox = getInstance(
        select('#' + this.parent.element.id + '_DialogMultipleAxis'),
        CheckBox
      ) as CheckBox;
      ...
      ).enabled = multipleAxisCheckBox.checked;
    }
  }
  ...
  public destroy(): void {
    ...
    let element: HTMLElement = select(
      '#' + this.parent.element.id + '_ConfirmDialog',
      document
    );
    let confirmPopUp: Dialog = element
      ? (getInstance(element, Dialog) as Dialog)
      : null;
    ...
    let chartTypesDialog: Dialog = element
      ? (getInstance(element, Dialog) as Dialog)
      : null;
    if (chartTypesDialog && !chartTypesDialog.isDestroyed) {
      chartTypesDialog.destroy();
    ...
    let exportMenu: Menu = element
      ? (getInstance(element, Menu) as Menu)
      : null;
    ...
    let grandTotalMenu: Menu = element
      ? (getInstance(element, Menu) as Menu)
      : null;
    ...
    let formattingMenu: Menu = element
      ? (getInstance(element, Menu) as Menu)
      : null;
    ...
    let reportList: DropDownList = element
      ? (getInstance(element, DropDownList) as DropDownList)
      : null;
  ...
  private focusToolBar(): void {
    removeClass(
      document
        .querySelector('.' + cls.GRID_TOOLBAR)
        .querySelectorAll('.e-menu-item.e-focused'),
      'e-focused'
    );
    removeClass(
      document
        .querySelector('.' + cls.GRID_TOOLBAR)
        .querySelectorAll('.e-menu-item.e-selected'),
      'e-selected'
    );
