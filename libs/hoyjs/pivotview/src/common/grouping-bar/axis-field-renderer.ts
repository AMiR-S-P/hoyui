...
import { createElement, isNullOrUndefined, prepend } from '@hoyui/base';
...
export class AxisFields {
  ...
  public render(): void {
    if (
      !this.parent.pivotButtonModule ||
      (this.parent.pivotButtonModule &&
        this.parent.pivotButtonModule.isDestroyed)
    ) {
      new PivotButton(this.parent);
    }
    this.createPivotButtons();
    let pivotButtons: HTMLElement[] = [];
    pivotButtons = pivotButtons.concat(
      [].slice.call(
        this.parent.groupingBarModule.rowPanel.querySelectorAll(
  ...
  private createPivotButtons(): void {
    ...
    if (
      isNullOrUndefined(
        this.parent.element.querySelector('.' + cls.GROUP_PIVOT_ROW)
      )
    ) {
    ...
    for (let i: number = 0; i < elements.length; i++) {
      ...
      if (
        (this.parent.dataSourceSettings.values.length > 0
          ? !element.classList.contains(cls.GROUP_CHART_VALUE)
          : true) ||
        (this.parent.dataSourceSettings.columns.length > 0
          ? !element.classList.contains(cls.GROUP_CHART_COLUMN)
          : true)
      ) {
        element.innerHTML = '';
      }
    ...
    if (
      (this.parent.dataType === 'pivot' &&
        ...
        ((!(this.parent.dataSourceSettings.dataSource instanceof DataManager) &&
          (this.parent.dataSourceSettings.dataSource as IDataSet[]).length >
            0) ||
          (this.parent.dataSourceSettings.dataSource instanceof DataManager &&
            this.parent.engineModule.data &&
