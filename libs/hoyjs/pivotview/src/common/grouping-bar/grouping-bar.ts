...
import { addClass, formatUnit } from '@hoyui/base';
...
import { Column, ColumnModel } from '@hoyui/grids';
import { Toolbar } from '@hoyui/navigations';
...
export class GroupingBar implements IAction {
  ...
  public renderLayout(): void {
    ...
    const leftAxisPanel: HTMLElement = createElement('div', {
      className: cls.LEFT_AXIS_PANEL_CLASS,
    });
    this.rightAxisPanel = createElement('div', {
      className: cls.RIGHT_AXIS_PANEL_CLASS,
    });
    const rowAxisPanel: HTMLElement = createElement('div', {
      className: cls.AXIS_ROW_CLASS + ' ' + cls.AXIS_ICON_CLASS + 'container',
    });
    const columnAxisPanel: HTMLElement = createElement('div', {
      className:
        cls.AXIS_COLUMN_CLASS + ' ' + cls.AXIS_ICON_CLASS + 'container',
    });
    ...
    const filterAxisPanel: HTMLElement = createElement('div', {
      className:
        cls.AXIS_FILTER_CLASS + ' ' + cls.AXIS_ICON_CLASS + 'container',
    });
    this.rowPanel = createElement('div', {
      className: cls.GROUP_ROW_CLASS + ' ' + cls.ROW_AXIS_CLASS,
    });
    const columnPanel: HTMLElement = createElement('div', {
      className: cls.GROUP_COLUMN_CLASS + ' ' + cls.COLUMN_AXIS_CLASS,
    });
    const valuePanel: HTMLElement = createElement('div', {
      className: cls.GROUP_VALUE_CLASS + ' ' + cls.VALUE_AXIS_CLASS,
    });
    const filterPanel: HTMLElement = createElement('div', {
    ...
    this.groupingTable.appendChild(
      createElement('div', { styles: 'display:flex;' })
    );
    ...
    if (
      ...
      this.parent.groupingBarSettings != null &&
      this.parent.groupingBarSettings.showFieldsPanel
    ) {
      this.gridPanel = this.createToolbarUI(this.groupingTable);
    }
    this.groupingTable.classList.add(cls.GRID_GROUPING_BAR_CLASS);
    this.groupingTable
      .querySelector('.' + cls.GROUP_ROW_CLASS)
      .classList.add(cls.GROUP_PIVOT_ROW);
    const axisPanels: HTMLElement[] = [
      this.rowPanel,
      columnPanel,
      valuePanel,
      filterPanel,
    ...
    if (
      ...
      this.parent.groupingBarSettings.displayMode !== 'Table'
    ) {
      this.groupingChartTable = this.groupingTable.cloneNode(
        true
      ) as HTMLElement;
      if (
        select(
          '#' + this.parent.element.id + '_AllFields',
      ...
      this.groupingChartTable
        .querySelector('.' + cls.GROUP_ROW_CLASS)
        .classList.add(cls.GROUP_CHART_ROW);
      this.groupingChartTable
        .querySelector('.' + cls.GROUP_ROW_CLASS)
        .classList.remove(cls.GROUP_PIVOT_ROW);
      if (
        this.parent.chartSettings.enableMultipleAxis &&
        this.parent.chartSettings.chartSeries &&
        ['Pie', 'Pyramid', 'Doughnut', 'Funnel'].indexOf(
          this.parent.chartSettings.chartSeries.type
        ) < 0
      ) {
        this.groupingChartTable
          .querySelector('.' + cls.GROUP_VALUE_CLASS)
          .classList.add(cls.GROUP_CHART_MULTI_VALUE);
      } else {
        this.groupingChartTable
          .querySelector('.' + cls.GROUP_VALUE_CLASS)
          .classList.add(cls.GROUP_CHART_VALUE);
      }
      if (
        this.parent.chartSettings.chartSeries &&
        ['Pie', 'Pyramid', 'Doughnut', 'Funnel'].indexOf(
        ...
        this.groupingChartTable
          .querySelector('.' + cls.GROUP_COLUMN_CLASS)
          .classList.add(cls.GROUP_CHART_COLUMN);
      } else {
        this.groupingChartTable
          .querySelector('.' + cls.GROUP_COLUMN_CLASS)
          .classList.add(cls.GROUP_CHART_ACCUMULATION_COLUMN);
      }
      this.groupingChartTable
        .querySelector('.' + cls.GROUP_FILTER_CLASS)
        .classList.add(cls.GROUP_CHART_FILTER);
    } else {
      this.groupingChartTable = undefined;
    }
    if (
      this.parent.displayOption.view === 'Chart' ||
      this.parent.groupingBarSettings.displayMode === 'Chart'
    ) {
  ...
  private appendToElement(): void {
    const element: HTMLElement = this.groupingTable
      ? this.groupingTable
      : this.groupingChartTable;
    if (isNullOrUndefined(element)) {
      return;
    }
    const leftAxisPanel: HTMLElement = element.getElementsByClassName(
      cls.LEFT_AXIS_PANEL_CLASS
    )[0] as HTMLElement;
    const filterPanel: HTMLElement = element.getElementsByClassName(
      cls.GROUP_FILTER_CLASS + ' ' + cls.FILTER_AXIS_CLASS
    )[0] as HTMLElement;
    const columnPanel: HTMLElement = element.getElementsByClassName(
      cls.GROUP_COLUMN_CLASS + ' ' + cls.COLUMN_AXIS_CLASS
    )[0] as HTMLElement;
    const valuePanel: HTMLElement = element.getElementsByClassName(
      cls.GROUP_VALUE_CLASS + ' ' + cls.VALUE_AXIS_CLASS
    )[0] as HTMLElement;
    if (
      this.parent.element.querySelector('.' + cls.GRID_CLASS) ||
      this.parent.element.querySelector('.' + cls.PIVOTCHART)
    ) {
      if (this.parent.showGroupingBar) {
        if (this.parent.element.querySelector('.' + cls.GROUPING_BAR_CLASS)) {
          const elements: NodeListOf<HTMLElement> =
            this.parent.element.querySelectorAll('.' + cls.GROUPING_BAR_CLASS);
        ...
        if (this.groupingChartTable) {
          if (
            ...
            if (
              this.groupingChartTable.querySelector(
                '.' + cls.ALL_FIELDS_PANEL_CLASS
              ) &&
              this.chartPanel != null &&
              !this.chartPanel.isDestroyed
            ) {
              const chartPanelWidth: string | number = this.parent.grid
                ? this.parent.getGridWidthAsNumber() - 2
                : this.parent.getWidthAsNumber() - 2;
              this.chartPanel.width =
                chartPanelWidth < 400 ? '398px' : chartPanelWidth;
        ...
        if (this.parent.displayOption.view !== 'Chart' && this.groupingTable) {
          ...
          if (this.parent.dataType === 'olap') {
            emptyRowCount = this.parent.olapEngineModule.headerContent
              ? Object.keys(this.parent.olapEngineModule.headerContent).length
              : 0;
          } else {
          ...
          if (!isNullOrUndefined(emptyRowCount)) {
            const emptyHeader: HTMLElement = this.parent.element
              .querySelector('.' + cls.HEADERCONTENT)
              .querySelector('.e-columnheader') as HTMLElement;
            emptyHeader.removeAttribute('style');
            addClass(
              [emptyHeader.querySelector('.' + cls.HEADERCELL)],
              'e-group-row'
            );
            (
              emptyHeader
                .querySelector('.e-group-row')
                .querySelector('.e-headercelldiv') as HTMLElement
            ).style.display = 'none';
            (
              emptyHeader
                .querySelector('.e-group-row')
                .querySelector('.e-sortfilterdiv') as HTMLElement
            ).style.display = 'none';
          }
          this.parent.element.insertBefore(
            this.groupingTable,
          ...
          if (
            this.groupingTable &&
            this.groupingTable.querySelector(
              '.' + cls.ALL_FIELDS_PANEL_CLASS
            ) &&
            this.gridPanel != null &&
            !this.gridPanel.isDestroyed
          ) {
            const gridPanelWidth: string | number = this.parent.grid
              ? this.parent.getGridWidthAsNumber() - 2
              : this.parent.getWidthAsNumber() - 2;
            this.gridPanel.width =
              gridPanelWidth < 400 ? '398px' : gridPanelWidth;
            this.gridPanel.refreshOverflow();
          }
          this.groupingTable.style.minWidth = '400px';
          this.parent.axisFieldModule.render();
          this.setGridRowWidth();
          const colGroupElement: HTMLElement = this.parent.element
            .querySelector('.' + cls.HEADERCONTENT)
            .querySelector('colgroup').children[0] as HTMLElement;
          const rightAxisPanelWidth: string = formatUnit(
            this.groupingTable.offsetWidth -
          ...
          if (
            this.parent.element
              .querySelector('.' + cls.HEADERCONTENT)
              .querySelector('.e-rhandler')
          ) {
            (
              this.parent.element
                .querySelector('.' + cls.HEADERCONTENT)
                .querySelector('.e-rhandler') as HTMLElement
            ).style.height = topLeftHeight + 'px';
          }

          const colRows: HTMLTableElement[] = [].slice.call(
            this.parent.element
              .querySelector('.' + cls.HEADERCONTENT)
              .querySelector('thead')
              .querySelectorAll('tr')
          );
          const columnRows: HTMLTableElement[] = colRows.filter(
            (trCell: HTMLTableElement) => {
          ...
          for (const element of columnRows) {
            ...
            const rowHeader: HTMLElement[] = [].slice.call(
              element.querySelectorAll('.e-rhandler')
            ) as HTMLElement[];
        ...
        if (
          ...
          const groupingBarSelector: string =
            this.parent.currentView === 'Table'
              ? '.e-chart-grouping-bar'
              : '.e-pivot-grouping-bar';
          const groupingBar: HTMLElement = this.parent.element.querySelector(
            groupingBarSelector
          ) as HTMLElement;
          if (groupingBar) {
  ...
  private updateChartAxisHeight(): void {
    if (
      ...
      const rowPanel: HTMLElement = this.groupingChartTable.querySelector(
        '.' + cls.GROUP_ROW_CLASS
      ) as HTMLElement;
      const valuePanel: HTMLElement = this.groupingChartTable.querySelector(
        '.' + cls.GROUP_VALUE_CLASS
      ) as HTMLElement;
      const filterPanel: HTMLElement = this.groupingChartTable.querySelector(
        '.' + cls.GROUP_FILTER_CLASS
      ) as HTMLElement;
      const columnPanel: HTMLElement = this.groupingChartTable.querySelector(
        '.' + cls.GROUP_COLUMN_CLASS
      ) as HTMLElement;
      if (rowPanel && columnPanel) {
        ...
        if (rowPanel.offsetHeight > 0 && columnPanel.offsetHeight > 0) {
          const maxHeight: number =
            rowPanel.offsetHeight > columnPanel.offsetHeight
              ? rowPanel.offsetHeight
      ...
      if (valuePanel && filterPanel) {
        ...
        if (valuePanel.offsetHeight > 0 && filterPanel.offsetHeight > 0) {
          const maxHeight: number =
            valuePanel.offsetHeight > filterPanel.offsetHeight
              ? valuePanel.offsetHeight
  ...
  public refreshUI(): void {
    ...
    if (this.groupingTable) {
      const valuePanel: HTMLElement = this.groupingTable.getElementsByClassName(
        cls.GROUP_VALUE_CLASS + ' ' + cls.VALUE_AXIS_CLASS
      )[0] as HTMLElement;
      setStyleAttribute(this.groupingTable, {
        width: formatUnit(
      ...
      if (
        this.groupingTable &&
        this.groupingTable.querySelector('.' + cls.ALL_FIELDS_PANEL_CLASS) &&
        this.gridPanel != null &&
        !this.gridPanel.isDestroyed
      ) {
        const gridPanelWidth: string | number = this.parent.grid
      ...
      const colGroupElement: HTMLElement = this.parent.element
        .querySelector('.' + cls.HEADERCONTENT)
        .querySelector('colgroup').children[0] as HTMLElement;
      const rightAxisWidth: string = formatUnit(
        this.groupingTable.offsetWidth -
          parseInt(colGroupElement.style.width, 10)
      );
      setStyleAttribute(valuePanel, { width: colGroupElement.style.width });
      ...
      if (!this.parent.grid.element.querySelector('.e-group-row')) {
        const emptyRowHeader: HTMLElement = this.parent.element
          .querySelector('.' + cls.HEADERCONTENT)
          .querySelector('.e-columnheader') as HTMLElement;
        addClass(
          [emptyRowHeader.querySelector('.' + cls.HEADERCELL)],
          'e-group-row'
        );
        setStyleAttribute(this.rowPanel, {
          height:
        ...
        const colRowElements: HTMLTableElement[] = [].slice.call(
          this.parent.element
            .querySelector('.' + cls.HEADERCONTENT)
            .querySelector('thead')
            .querySelectorAll('tr')
        );
        const columnRows: HTMLTableElement[] = colRowElements.filter(
          (trCell: HTMLTableElement) => {
            return trCell.childNodes.length > 0;
          }
        );
        ...
        for (const element of columnRows) {
          ...
          for (const handlerElement of rowHeader) {
            if (
              !handlerElement.parentElement.parentElement.querySelector(
                '.' + cls.FREEZED_CELL
              )
            ) {
  ...
  public alignIcon(): void {
    if (this.parent.pivotFieldListModule) {
      ...
      if (this.parent.currentView === 'Table') {
        currentWidth = this.parent.grid
          ? this.parent.grid.element.offsetWidth
          : currentWidth;
      } else {
        currentWidth = this.parent.chart
          ? this.parent.pivotChartModule.getCalulatedWidth()
          : currentWidth;
      }
      if (currentWidth) {
    ...
    if (this.rowPanel.querySelector('.' + cls.PIVOT_BUTTON_CLASS)) {
      if (!this.parent.isAdaptive) {
        const pivotButtons: HTMLElement[] = [].slice.call(
          this.rowPanel.querySelectorAll('.' + cls.PIVOT_BUTTON_WRAPPER_CLASS)
        );
        const lastButton: HTMLElement = pivotButtons[pivotButtons.length - 1];
        const lastButtonWidth: number =
          (
            lastButton.querySelector(
              '.' + cls.PIVOT_BUTTON_CLASS
            ) as HTMLElement
          ).offsetWidth +
          (lastButton.querySelector('.e-indent-div') as HTMLElement)
            .offsetWidth +
          20;
        let buttonWidth: string = formatUnit(
          lastButtonWidth < this.resColWidth
            ? this.resColWidth
            : lastButtonWidth
        );
        const rowHeaderTable: HTMLElement = this.parent.element
          .querySelector('.' + cls.HEADERCONTENT)
          .querySelector('.' + cls.HEADERCELL) as HTMLElement;
        const rowContentTable: HTMLElement = this.parent.element
          .querySelector('.' + cls.CONTENT_CLASS)
          .querySelector('tbody')
          .querySelector('.' + cls.FREEZED_CELL);
        const rowContent: HTMLElement = this.parent.element
          .querySelector('.' + cls.CONTENT_CLASS)
          .querySelector('colgroup').children[0] as HTMLElement;
        ...
        if (this.parent.dataType === 'olap') {
          valueColWidth = this.parent.renderModule.calculateColWidth(
            this.parent.olapEngineModule.pivotValues.length > 0
              ? this.parent.olapEngineModule.pivotValues[0].length
              : 2
          );
        } else {
          valueColWidth = this.parent.renderModule.calculateColWidth(
            this.parent.dataSourceSettings.values.length > 0 &&
              ...
              ? this.parent.engineModule.pivotValues[0].length
              : 2
          );
        }
          ...
          if (cCnt !== 0) {
            if ((gridColumn[cCnt as number] as Column).columns) {
              this.parent.setCommonColumnsWidth(
                (
                  this.parent.renderModule.pivotColumns[
                    cCnt as number
                  ] as Column
                ).columns as Column[],
                valueColWidth
              ...
              if ((gridColumn[cCnt as number] as Column).width !== 'auto') {
                const levelName: string = gridColumn[cCnt as number]
                  .customAttributes
                  ? ((
                      gridColumn[cCnt as number].customAttributes
                        .cell as IAxisSet
                    ).valueSort.levelName as string)
                  : '';
                const columnWidth: number =
                  this.parent.renderModule.setSavedWidth(
                    levelName,
                    valueColWidth
                  );
                gridColumn[cCnt as number].width =
                  gridColumn[cCnt as number].autoFit ||
                  (hasPivotColumns &&
                    this.parent.pivotColumns[cCnt as number].autoFit)
                    ? gridColumn[cCnt as number].width
                    : this.parent.renderModule.lastColumn &&
                      this.parent.renderModule.lastColumn.field ===
                        gridColumn[cCnt as number].field
                    ? columnWidth - 3
                    : columnWidth;
              } else {
        ...
        if (!this.parent.firstColWidth) {
          buttonWidth = gridColumn[0].autoFit
            ? gridColumn[0].width.toString()
            : buttonWidth;
  ...
  private dropIndicatorUpdate(e: MouseEvent): void {
    if (
      (this.parent.isDragging &&
        (e.target as HTMLElement).classList.contains(cls.DROPPABLE_CLASS) &&
        e.type === 'mouseover') ||
      !this.parent.isDragging ||
      (!(e.target as HTMLElement).classList.contains(cls.DROPPABLE_CLASS) &&
        e.type === 'mouseleave')
      ...
      removeClass(
        [].slice.call(
          this.parent.element.querySelectorAll('.' + cls.DROP_INDICATOR_CLASS)
        ),
  ...
  private createToolbarUI(element: HTMLElement): Toolbar {
    ...
    element.prepend(
      createElement('div', { id: this.parent.element.id + '_AllFields' })
    );
  ...
  public destroy(): void {
    ...
    if (
      this.groupingChartTable &&
      this.groupingChartTable.querySelector('.' + cls.ALL_FIELDS_PANEL_CLASS) &&
      this.chartPanel != null &&
      !this.chartPanel.isDestroyed
    ) {
