import { Dialog, ButtonPropsModel } from '@hoyui/popups';
import {
  isNullOrUndefined as isNaN,
  createElement,
  extend,
  remove,
  addClass,
...
import { ChangeEventArgs } from '@hoyui/buttons';
import { ColorPicker, ColorPickerEventArgs, TextBox } from '@hoyui/inputs';
import {
  DropDownList,
  ChangeEventArgs as DropDownArgs,
...
export class ConditionalFormatting {
  ...
  private createDialog(): void {
    if (
      select('#' + this.parentID + 'conditionalformatting', document) !== null
    ) {
        ...
        buttonModel: {
          cssClass:
            (this.parent.isAdaptive
              ? cls.FORMAT_ROUND_BUTTON + ' ' + cls.FORMAT_CONDITION_BUTTON
              : cls.FORMAT_CONDITION_BUTTON) +
            (this.parent.cssClass ? ' ' + this.parent.cssClass : ''),
          iconCss: cls.ICON + ' ' + cls.ADD_ICON_CLASS,
          content: this.parent.isAdaptive
            ? ''
            : this.parent.localeObj.getConstant('condition'),
        },
      },
      {
  ...
  private beforeOpen(): void {
    select('#' + this.parentID + 'conditionalformatting', document)
      .querySelector('.' + cls.DIALOG_HEADER)
      .setAttribute(
        'title',
        this.parent.localeObj.getConstant('conditionalFormatting')
      );
  }
  ...
  private addButtonClick(): void {
    ...
    const conditionalFormating: ConditionalFormatting =
      this as ConditionalFormatting;
    this.parent.trigger(
      events.conditionalFormatting,
      format,
      (observedArgs: ConditionalFormatSettingsModel) => {
  ...
  private applyButtonClick(): void {
    if (this.refreshConditionValues()) {
      this.parent.setProperties(
        { dataSourceSettings: { conditionalFormatSettings: this.newFormat } },
        true
      );
      const actionInfo: PivotActionInfo = {
        conditionalFormattingInfo: this.parent.dataSourceSettings
          .conditionalFormatSettings as IConditionalFormatSettings[],
      };
      ...
      const dialog: Dialog = getInstance(
        select(
          '#' + this.parentID + 'conditionalformatting',
          document
        ) as HTMLElement,
        Dialog
      ) as Dialog;
  ...
  private refreshConditionValues(): boolean {
    for (let i: number = 0; i < this.newFormat.length; i++) {
      if (
        (
          select(
            '#' + this.parentID + 'conditionvalue1' + i,
            document
          ) as HTMLInputElement
        ).value === '' ||
        (
          select(
            '#' + this.parentID + 'conditionvalue2' + i,
            document
          ) as HTMLInputElement
        ).value === ''
      ) {
      ...
      this.newFormat[i as number].value1 = Number(
        (
          select(
            '#' + this.parentID + 'conditionvalue1' + i,
            document
      ...
      this.newFormat[i as number].value2 = Number(
        (
          select(
            '#' + this.parentID + 'conditionvalue2' + i,
            document
  ...
  private addFormat(): void {
    const format: HTMLElement = createElement('div', {
  ...
  private createDialogElements(i?: number): HTMLElement {
    ...
    if (i !== undefined) {
      ...
      let valueTableRow: HTMLElement = createElement('tr');
      let valueTableElements: HTMLElement = createElement('td');
      ...
      const measureDropdown: HTMLElement = createElement('div', {
      ...
      valueTableRow.appendChild(valueTableElements);
      valueTableElements = createElement('td');
      const conditionDropdown: HTMLElement = createElement('div', {
      ...
      const formatValueClassName: string = !(
        format.conditions === 'Between' || format.conditions === 'NotBetween'
      )
        ? cls.HIDDEN
        : '';
      ...
      const grandTotalTable: HTMLElement = createElement('table', {
        ...
        className: cls.FORMAT_TABLE + ' ' + cls.GRANDTOTAL_CHECKBOX_TABLE,
        attrs: { role: 'table' },
      });
      const grandTotalTableRow: HTMLElement = createElement('tr');
      const grandTotalTableElements: HTMLElement = createElement('td');
      ...
      let formatTableRow: HTMLElement = createElement('tr');
      let formatTableElements: HTMLElement = createElement('td');
      ...
      const fontNameDropdown: HTMLElement = createElement('div', {
      ...
      formatTableRow.appendChild(formatTableElements);
      formatTableElements = createElement('td');
      const fontSizeDropdown: HTMLElement = createElement('div', {
  ...
  private renderDropDowns(i: number): void {
    ...
    for (
      ...
      caption = this.parent.enableHtmlSanitizer
        ? SanitizeHtmlHelper.sanitize(caption)
        : caption;
      fields.push({
        index: i + 1,
        name: caption,
    ...
    let value: string = isNaN(format.measure)
      ? this.parent.localeObj.getConstant('AllValues')
      : format.measure;
    ...
    fieldsDropDown[i as number].appendTo(
      select(
        '#' + this.parentID + 'measureinput' + i,
        dialogElement
      ) as HTMLElement
    );
    ...
    conditionsDropDown[i as number].appendTo(
      select(
        '#' + this.parentID + 'conditioninput' + i,
        dialogElement
      ) as HTMLElement
    );
    const formatValueClassName: string = !(
      this.newFormat[i as number].conditions === 'Between' ||
      this.newFormat[i as number].conditions === 'NotBetween'
    )
      ? cls.HIDDEN
      : '';
    ...
    grandTotalCheckBox.appendTo(
      select(
        '#' + this.parentID + 'grandtotalcheckbox' + i,
        dialogElement
      ) as HTMLElement
    );
    const fontNames: { [key: string]: Object }[] = [
      { index: 0, name: 'Arial' },
      { index: 1, name: 'San Serif' },
      { index: 2, name: 'Impact' },
      { index: 3, name: 'Trebuchet MS' },
      { index: 4, name: 'Serif' },
      { index: 5, name: 'Verdana' },
      { index: 6, name: 'Courier New' },
      { index: 7, name: 'Times New Roman' },
      { index: 8, name: 'Tahoma' },
    ...
    fontNameDropDown[i as number].appendTo(
      select(
        '#' + this.parentID + 'fontnameinput' + i,
        dialogElement
      ) as HTMLElement
    );
    ...
    fontSizeDropDown[i as number].appendTo(
      select(
        '#' + this.parentID + 'fontsizeinput' + i,
        dialogElement
      ) as HTMLElement
    );
  }
  ...
  private conditionChange(i: number, args: DropDownArgs): void {
    ...
    const valuespan: HTMLElement = select(
      '#' + this.parentID + 'valuespan' + i,
      document
    ) as HTMLElement;
    const conditionvalue2: HTMLElement = select(
      '#' + this.parentID + 'conditionvalue2' + i,
      document
    ) as HTMLElement;
  ...
  private fontNameChange(i: number, args: DropDownArgs): void {
    ...
    (
      select('#' + this.parentID + 'valuepreview' + i, document) as HTMLElement
    ).style.fontFamily = args.value as string;
  }
  ...
  private fontSizeChange(i: number, args: DropDownArgs): void {
    ...
    (
      select('#' + this.parentID + 'valuepreview' + i, document) as HTMLElement
    ).style.fontSize = args.value as string;
  }

  private measureChange(i: number, args: DropDownArgs): void {
    this.newFormat[i as number].measure =
      args.value.toString() === this.parent.localeObj.getConstant('AllValues')
        ? undefined
        : args.value.toString();
  }
  ...
  private renderColorPicker(i: number): void {
    ...
    const dialogElement: HTMLElement = dialog.element;
    const format: ConditionalFormatSettingsModel = this.newFormat[i as number];
    let value: string = isNaN(format.style.color)
      ? 'black'
      : format.style.color;
    let color: string =
      value.charAt(0) === '#' && this.isHex(value.substr(1))
        ? value
        : this.colourNameToHex(value);
    (
      select('#' + this.parentID + 'valuepreview' + i, document) as HTMLElement
    ).style.color = color;
    this.fontColor[i as number] = new ColorPicker({
      cssClass:
        cls.FORMAT_COLOR_PICKER +
        ' ' +
        cls.FORMAT_FONT_COLOR_PICKER +
        (this.parent.cssClass ? ' ' + this.parent.cssClass : ''),
      value: color,
      mode: 'Palette',
    ...
    this.fontColor[i as number].appendTo(
      select(
        '#' + this.parentID + 'fontcolor' + i,
        dialogElement
      ) as HTMLElement
    );
    addClass(
      [
        this.fontColor[i as number].element.nextElementSibling.querySelector(
          '.' + cls.SELECTED_COLOR
        ),
      ],
      cls.ICON
    );
    value = isNaN(format.style.backgroundColor)
      ? 'white'
    ...
    (
      select('#' + this.parentID + 'valuepreview' + i, document) as HTMLElement
    ).style.fontFamily = format.style.fontFamily;
    (
      select('#' + this.parentID + 'valuepreview' + i, document) as HTMLElement
    ).style.fontSize = format.style.fontSize;
    ...
    this.backgroundColor[i as number].appendTo(
      select(
        '#' + this.parentID + 'backgroundcolor' + i,
        dialogElement
      ) as HTMLElement
    );
    addClass(
      [
        this.backgroundColor[
          i as number
        ].element.nextElementSibling.querySelector('.e-selected-color'),
      ],
      cls.ICON
    );
    const toggleBtn: Button = new Button({
      ...
      cssClass:
        cls.FLAT + (this.parent.cssClass ? ' ' + this.parent.cssClass : ''),
      locale: this.parent.locale,
      enableRtl: this.parent.enableRtl,
      enableHtmlSanitizer: this.parent.enableHtmlSanitizer,
    });
  ...
  private fontColorChange(i: number, args: ColorPickerEventArgs): void {
    ...
    (
      select('#' + this.parentID + 'valuepreview' + i, document) as HTMLElement
    ).style.color = args.currentValue.hex;
  }
  ...
  private destroyColorPickers(): void {
    for (
      let i: number = 0;
      i < (this.newFormat ? this.newFormat.length : 0);
      i++
    ) {
      if (
        this.fontColor &&
        this.fontColor[i as number] &&
        !this.fontColor[i as number].isDestroyed
      ) {
        this.fontColor[i as number].destroy();
      }
      if (
        this.backgroundColor &&
        this.backgroundColor[i as number] &&
        !this.backgroundColor[i as number].isDestroyed
      ) {
  ...
  public showConditionalFormattingDialog(): void {
    ...
    for (
      let i: number = 0;
      i < this.parent.dataSourceSettings.conditionalFormatSettings.length;
      i++
    ) {
      this.newFormat.push(
        extend(
          {},
          (<{ [key: string]: Object }>(
            this.parent.dataSourceSettings.conditionalFormatSettings[
              i as number
            ]
          )).properties,
          null,
