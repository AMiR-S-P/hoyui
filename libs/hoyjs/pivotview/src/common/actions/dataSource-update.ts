...
export class DataSourceUpdate {
  ...
  public updateDataSource(
    ...
    for (let len: number = 0, lnt: number = field.length; len < lnt; len++) {
      if (field[len as number]) {
        for (
          let i: number = 0, n: number = field[len as number].length;
          i < n;
          i++
        ) {
          if (
            field[len as number][i as number].name === fieldName ||
            (this.parent.dataType === 'olap' &&
              field[len as number][i as number].name.toLowerCase() ===
                '[measures]' &&
              field[len as number][i as number].name.toLowerCase() ===
                fieldName)
          ) {
    ...
    const control: PivotView | PivotFieldList =
      this.control.getModuleName() === 'pivotfieldlist' &&
      (this.control as PivotFieldList).isPopupView
        ? (this.control as PivotFieldList).pivotGridModule
        : this.control;
    control.trigger(
      events.fieldDrop,
      eventdrop,
      (observedArgs: FieldDropEventArgs) => {
        if (!observedArgs.cancel) {
          ...
          fieldName = observedArgs.dropField
            ? observedArgs.dropField.name
            : observedArgs.fieldName;
          dataSourceItem = observedArgs.dropField;
          if (
            this.control &&
            this.btnElement &&
            this.btnElement.getAttribute('isvalue') === 'true'
          ) {
            switch (droppedClass) {
              case '':
              ...
              case 'rows':
                droppedPosition =
                  droppedPosition === this.parent.dataSourceSettings.rows.length
                    ? -1
                    : droppedPosition;
                this.control.setProperties(
                  {
              ...
              case 'columns':
                droppedPosition =
                  droppedPosition ===
                  this.parent.dataSourceSettings.columns.length
                    ? -1
                    : droppedPosition;
                this.control.setProperties(
                  {
              ...
              const dropAxisFields: IFieldOptions[] =
                ...
                droppedClass === 'rows'
                  ...
                  : this.parent.dataSourceSettings.valueAxis === 'column' &&
                    droppedClass === 'columns'
                  ? this.parent.dataSourceSettings.columns
                  : undefined;
              if (
                draggedPosition < this.parent.dataSourceSettings.valueIndex &&
                ((this.parent.dataSourceSettings.valueAxis === 'row' &&
                  draggedClass === 'rows') ||
                  (this.parent.dataSourceSettings.valueAxis === 'column' &&
                    draggedClass === 'columns'))
              ) {
                this.control.setProperties(
                  {
                    dataSourceSettings: {
            ...
            dataSourceItem = this.getNewField(
              fieldName.toString(),
              observedArgs.dropField
            );
            if (
              dataSourceItem.type === 'CalculatedField' &&
              droppedClass !== ''
            ) {
              droppedClass = 'values';
            }
          ...
          if (this.control) {
            ...
            control.trigger(
              events.onFieldDropped,
              eventArgs,
              (droppedArgs: FieldDroppedEventArgs) => {
                ...
                if (dataSourceItem) {
                  ...
                  switch (droppedClass) {
                    case 'filters':
                      if (droppedPosition !== -1) {
                        ...
                        this.parent.dataSourceSettings.filters.push(
                          dataSourceItem
                        );
                      }
                      break;
                    case 'rows':
                      if (droppedPosition !== -1) {
                        this.parent.dataSourceSettings.rows.splice(
                          droppedPosition,
                          0,
                          dataSourceItem
                        );
                      } else {
                        this.parent.dataSourceSettings.rows.push(
                          dataSourceItem
                        );
                      }
                      break;
                    case 'columns':
                      if (droppedPosition !== -1) {
                        ...
                        this.parent.dataSourceSettings.columns.push(
                          dataSourceItem
                        );
                      }
                      break;
                    case 'values':
                      if (droppedPosition !== -1) {
                        ...
                        this.parent.dataSourceSettings.values.push(
                          dataSourceItem
                        );
                      }
                      if (
                        this.parent.dataType === 'olap' &&
                        !(this.parent.engineModule as OlapEngine)
                          .isMeasureAvail &&
                        !(this.parent.dataSourceSettings.values.length > 1)
                      ) {
                  ...
                  const fieldCount: number =
                    droppedClass === 'columns'
                      ? control.dataSourceSettings.columns.length
                      : droppedClass === 'rows'
                      ? control.dataSourceSettings.rows.length
                      : 0;
                  if (
                    fieldCount !== 0 &&
                    control.dataSourceSettings.valueIndex === fieldCount
                  ) {
                    this.control.setProperties(
  ...
  public removeFieldFromReport(fieldName: string): IFieldOptions {
    ...
    for (let len: number = 0, lnt: number = fields.length; len < lnt; len++) {
      if (!isDataSource && fields[len as number]) {
        for (
          let i: number = 0, n: number = fields[len as number].length;
          i < n;
          i++
        ) {
          if (
            fields[len as number][i as number].name === fieldName ||
            (this.parent.dataType === 'olap' &&
              fields[len as number][i as number].name.toLowerCase() ===
                '[measures]' &&
              fields[len as number][i as number].name.toLowerCase() ===
                fieldName)
          ) {
            dataSourceItem = (<{ [key: string]: IFieldOptions }>(
            ...
            if (this.parent.dataType === 'olap') {
              const engineModule: OlapEngine = this.parent
                .engineModule as OlapEngine;
  ...
  public getNewField(
    ...
    if (this.parent.dataType === 'olap') {
      const field: IOlapField = (this.parent.engineModule as OlapEngine)
        .fieldList[fieldName as string];
      newField = {
        ...
        isCalculatedField: fieldItem
          ? fieldItem.isCalculatedField
          : field.isCalculatedField,
        type: fieldItem
          ? fieldItem.type === undefined
            ? field.type === 'number'
              ? ('Sum' as SummaryTypes)
              : ('Count' as SummaryTypes)
            : fieldItem.type
          : (field.aggregateType as SummaryTypes) === undefined
          ? field.type === 'number'
            ? ('Sum' as SummaryTypes)
            : ('Count' as SummaryTypes)
          : (field.aggregateType as SummaryTypes),
        showFilterIcon: fieldItem
          ? fieldItem.showFilterIcon
        ...
        showRemoveIcon: fieldItem
          ? fieldItem.showRemoveIcon
          : field.showRemoveIcon,
        showValueTypeIcon: fieldItem
          ? fieldItem.showValueTypeIcon
          : field.showValueTypeIcon,
        allowDragAndDrop: fieldItem
          ? fieldItem.allowDragAndDrop
          : field.allowDragAndDrop,
        showSubTotals: fieldItem
          ? fieldItem.showSubTotals
          : field.showSubTotals,
      ...
      newField = {
        ...
        caption: fieldItem ? fieldItem.caption : field.caption,
        type: fieldItem
          ? fieldItem.type === undefined || fieldItem.type === null
            ? field.type === 'number'
              ? ('Sum' as SummaryTypes)
              : ('Count' as SummaryTypes)
            : fieldItem.type
          : (field.aggregateType as SummaryTypes) === undefined ||
            (field.aggregateType as SummaryTypes) === null
          ? field.type === 'number'
            ? ('Sum' as SummaryTypes)
            : ('Count' as SummaryTypes)
          : (field.aggregateType as SummaryTypes),
        showNoDataItems: fieldItem
          ? fieldItem.showNoDataItems
