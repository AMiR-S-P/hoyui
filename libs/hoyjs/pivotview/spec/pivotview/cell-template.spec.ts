...
describe('Pivot Grid Cell Template', () => {
  ...
  describe('- Grouping Bar with injected Module - ', () => {
    ...
    it('grouping bar render testing', () => {
      expect(
        pivotGridObj.element.children[0].classList.contains('e-grouping-bar')
      ).not.toBeTruthy;
    ...
    it('check drag and drop pivot button', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          (pivotButton[2].querySelector('.e-pvt-btn-content') as HTMLElement)
            .innerText
        ).toEqual('droppedButton');
        done();
      }, 1000);
    });
    it('check cell template', () => {
      expect(
        pivotGridObj.element.querySelectorAll('td')[4].childElementCount == 2
      ).toBeTruthy();
      pivotGridObj.cellTemplate =
        '<script type="text/javascript">${getCellContent(data)}</script>';
