...
import { ContextMenu, MenuEventArgs } from '@hoyui/navigations';
...
describe('Calculated Field', () => {
    ...
    pivotGridObj = new PivotView({
      dataSourceSettings: {
        ...
        calculatedFieldSettings: [
          { name: 'price', formula: '"Sum(balance)"+"Count(quantity)"' },
        ],
        filterSettings: [
          {
            name: 'state',
  ...
  it('Open Dialog1', () => {
    expect((pivotGridObj.pivotValues[2][4] as IAxisSet).formattedText).toBe(
      '$43,242.53'
    );
  ...
  it('nodeExpanding event is triggered', () => {
    ...
    let li: Element[] = <Element[] & NodeListOf<Element>>(
      cf.treeObj.element.querySelectorAll('li')
    );
  ...
  it('drag and drop node to drop field', () => {
    let treeObj: any = cf.treeObj;
    let filterAxiscontent: HTMLElement = document.querySelector(
  ...
  it('OK Button Click1', () => {
    let calcField: any = document.querySelector(
    ...
    let formatString: MaskedTextBox = getInstance(
      document.querySelector(
        '#' + pivotGridObj.element.id + 'Custom_Format_Element'
      ) as HTMLElement,
      MaskedTextBox
    ) as MaskedTextBox;
  ...
  it('Context menu click', () => {
    const element: HTMLElement = document.querySelector(
      '#' + pivotGridObj.element.id + 'CalcContextmenu'
    );
  ...
  it('treeview click', function () {
    expect(document.querySelector('#' + cf.parentID + 'CalcContextmenu'))
      .toBeTruthy;
  ...
  it('Open Dialog5', () => {
    expect((pivotGridObj.pivotValues[2][3] as IAxisSet).formattedText).toBe(
      '10'
    );
    cf.createCalculatedFieldDialog(pivotGridObj);
  ...
  it('OK Button Click3', () => {
    let calcInfo: ICalculatedFields = {
      name: 'price1',
      formula: '10',
      formatString: '',
    };
  ...
  it('check field list icon', () => {
    let calcField: any = document.querySelector(
      '#' +
        (pivotGridObj.pivotFieldListModule.calculatedFieldModule as any)
          .parentID +
        'calculateddialog'
    );
    (
      getInstance(
        calcField.querySelector(
          '#' +
            (pivotGridObj.pivotFieldListModule.calculatedFieldModule as any)
              .parentID +
            'ddlelement'
  ...
  it('check calculated field button for edit option1', () => {
    ...
    let pivotButtons: HTMLElement[] = [].slice.call(
      rightAxisPanel.querySelectorAll('.e-pivot-button')
    );
  ...
  it('check calculated field button for edit option in grouping bar', () => {
    let leftAxisPanel: HTMLElement = pivotGridObj.element.querySelector(
      '.e-left-axis-fields'
    );
    let pivotButtons: HTMLElement[] = [].slice.call(
...
describe(' - Opening the calculated dialog dialog', () => {
  ...
  it('- Using the corresponding method', (done: Function) => {
    setTimeout(() => {
      ...
      expect(document.querySelectorAll('.e-ddl-hidden')[0].textContent).toBe(
        'None'
      );
      (
        document.querySelectorAll('.e-dlg-closeicon-btn')[0] as HTMLElement
      ).click();
      done();
    }, 1000);
  });
