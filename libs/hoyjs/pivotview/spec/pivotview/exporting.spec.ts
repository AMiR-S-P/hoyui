...
import { ExcelExportProperties, PdfExportProperties } from '@hoyui/grids';
import {
  PdfFontFamily,
  PdfFontStyle,
  PdfPageSize,
...
describe('PDF Export', () => {
    ...
    beforeAll(() => {
      ...
      pivotGridObj = new PivotView({
        ...
        onPdfCellRender: (args: PdfCellRenderArgs) => {
          if (
            args.pivotCell &&
            args.pivotCell.valueSort &&
            args.pivotCell.valueSort.levelName === 'Bike'
          ) {
      ...
      pivotGridObj = new PivotView({
        ...
        onPdfCellRender: (args: PdfCellRenderArgs) => {
          if (
            args.pivotCell &&
            args.pivotCell.valueSort &&
            args.pivotCell.valueSort.levelName === 'male.balance'
          ) {
  ...
  describe('- Applying the theme to the cells in the PDF', () => {
    ...
    it('- Table render', (done: Function) => {
      setTimeout(() => {
        expect(
          (pivotGridObj.engineModule.pivotValues[0][1] as IDataSet)
            .formattedText
        ).toBe('female');
        done();
      }, 1000);
