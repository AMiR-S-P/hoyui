...
describe('Chart - ', () => {
  ...
  describe('Grouping bar - ', () => {
    ...
    it('Check initial render 1', (done: Function) => {
      setTimeout(() => {
        expect(
          document
            .getElementById('PivotView_chart_Series_0_Point_0')
            .getAttribute('aria-label')
        ).toBe('Canada:100, FY 2005');
      ...
      setTimeout(() => {
        expect(
          document.querySelectorAll(
            '#PivotView_chart0_Axis_MultiLevelLabel_Level_0_Text_4'
          )[0].textContent
        ).toBe(' + United States');
        expect(
          document.querySelectorAll('#PivotView_chart2_AxisLabel_5')[0]
            .textContent
        ).toBe('$500.00');
        expect(
          document.querySelectorAll('#PivotView_chart_AxisTitle_0')[0]
            .textContent
        ).toBe('Country / State');
        expect(
          document.querySelectorAll('#PivotView_chart_AxisTitle_2')[0]
            .textContent
        ).toBe('Sum of Amount');
        expect(
          document.querySelectorAll('#PivotView_chart_chart_legend_text_0')[0]
            .textContent
        ).toBe('FY 2005');
        done();
      }, 1000);
    ...
    it('sort descending -> Country', (done: Function) => {
      util.triggerMouseEvent(
        document.querySelectorAll('.e-sort')[0] as HTMLElement,
        'click'
      );
      setTimeout(() => {
        expect(
          document
            .getElementById('PivotView_chart_Series_0_Point_0')
            .getAttribute('aria-label')
        ).toBe('United States:400, FY 2005');
        expect(
          document.querySelectorAll(
            '#PivotView_chart0_Axis_MultiLevelLabel_Level_0_Text_4'
          )[0].textContent
    ...
    it('remove Date from column', (done: Function) => {
      util.triggerMouseEvent(
        document.querySelectorAll('.e-remove')[2] as HTMLElement,
        'click'
      );
      setTimeout(() => {
        expect(
    ...
    it('remove Country from row', function (done) {
      util.triggerMouseEvent(
        document.querySelectorAll('.e-remove')[0] as HTMLElement,
        'click'
      );
      setTimeout(function () {
        expect(
      ...
      setTimeout(() => {
        expect(
          document
            .getElementById('PivotView_chart_Series_0_Point_0')
            .getAttribute('aria-label')
        ).toBe('Grand Total:4600, ');
        expect(
          document.querySelectorAll(
            '#PivotView_chart0_Axis_MultiLevelLabel_Level_0_Text_0'
          )[0].textContent
        ).toBe('Total Sum of Amount');
        expect(
          document.getElementById('PivotView_chart_chart_legend_text_0')
        ).toBe(null);
        done();
      }, 1000);
    });
    ...
    it('perform drill up operation', (done: Function) => {
      expect(
        document.querySelectorAll(
          '#PivotView_chart0_Axis_MultiLevelLabel_Level_1_Text_0'
        )[0].textContent
      ).toBe(' - United States');
      ...
      setTimeout(function () {
        expect(
          document
            .getElementById('PivotView_chart_Series_0_Point_0')
            .getAttribute('aria-label')
        ).toBe('Grand Total:600, FY 2005 - Bike | Amount');
        expect(
          document
            .getElementById('PivotView_chart_Series_5_Point_0')
            .getAttribute('aria-label')
        ).toBe('Grand Total:2, FY 2006 - Bike | Quantity');
        expect(
          document.getElementById('PivotView_chart_AxisTitle_0')
        ).toBeNull();
        done();
      }, 1000);
    });
    it('empty rows3', (done: Function) => {
      setTimeout(function () {
        ...
        expect(
          document.querySelectorAll('#PivotView_chart_chart_legend_text_0')[0]
            .textContent
        ).toBe('FY 2005 - Bike | Amount');
        expect(
          document.querySelectorAll('#PivotView_chart_chart_legend_text_3')[0]
            .textContent
        ).toBe('FY 2005 - Van | Quantity');
        done();
      }, 1000);
    });
    it('chart type changed to stackingarea100 1', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          document.querySelectorAll('#PivotView_chart2_AxisLabel_1')[0]
            .textContent
        ).toBe('$50.00');
        done();
      }, 1000);
    ...
    it('load y axis properties', () => {
      pivotGridObj.setProperties(
        {
          chartSettings: {
            primaryYAxis: { labelFormat: 'C', title: 'Title', plotOffset: 30 },
          },
        },
        true
      );
    ...
    it('Set display option view as both, primary as table', (done: Function) => {
      (pivotGridObj.chartSeriesCreated = function (
        args: ChartSeriesCreatedEventArgs
      ) {
        args.cancel = true;
      }),
      ...
      setTimeout(function () {
        expect(
          document
            .querySelectorAll('.e-grid,.e-chart')[0]
            .classList.contains('e-pivotchart')
        ).toBeFalsy();
        done();
      }, 1000);
    });
  ...
  describe('Normal - ', () => {
      ...
      setTimeout(() => {
        ...
        pivotGridObj = new PivotView({
          ...
          chartSettings: {
            ...
            chartMouseMove: (args: any) => {
              eventArgs = args;
            },
            chartMouseClick: (args: any) => {
              eventArgs = args;
            },
            pointMove: (args: any) => {
              eventArgs = args;
            ...
            chartMouseLeave: (args: any) => {
              eventArgs = args;
            },
            chartMouseDown: (args: any) => {
              eventArgs = args;
            },
            chartMouseUp: (args: any) => {
              eventArgs = args;
            },
            dragComplete: (args: any) => {
              eventArgs = args;
            },
            zoomComplete: (args: any) => {
              eventArgs = args;
            },
            scrollStart: (args: any) => {
              eventArgs = args;
            },
            scrollEnd: (args: any) => {
              eventArgs = args;
            },
            scrollChanged: (args: any) => {
              eventArgs = args;
            },
    ...
    it('Check initial render 1', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          document
            .getElementById('PivotView_chart_Series_1_Point_0')
            .getAttribute('aria-label')
        ).toBe('Canada:400, FY 2006');
        expect(
          document
            .getElementById('PivotView_chart_Series_0_Point_1')
            .getAttribute('aria-label')
        ).toBe('France:200, FY 2005');
        expect(
          document
            .getElementById('PivotView_chart_Series_3_Point_4')
            .getAttribute('aria-label')
        ).toBe('United States:400, FY 2008');
        expect(
          document.getElementById('PivotView_chart_Series_4_Point_4')
        ).toBeNull();
        done();
      }, 1000);
    });
    ...
    it('change width to  800px', (done: Function) => {
      ...
      pivotGridObj.pivotChartModule.loadChart(
        pivotGridObj,
        pivotGridObj.chartSettings
      );
      setTimeout(() => {
        expect(
          document.getElementById('PivotView_chart_scrollBarThumb_primaryXAxis')
        ).toBe(null);
        done();
      }, 1000);
    });
  ...
  describe('ZoomFactor in chart', () => {
    ...
    it('Find zoomfactor value', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          (pivotGridObj.chart as Chart).primaryXAxis.zoomFactor === 1
        ).toBeTruthy();
        done();
      }, 1000);
    });
