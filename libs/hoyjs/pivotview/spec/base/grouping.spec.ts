...
describe('Group By Date feature', () => {
  ...
  describe(' -  Initial Rendering and Value Sorting', () => {
      ...
      pivotGridObj = new PivotView({
        dataSourceSettings: {
          ...
          groupSettings: [
            {
              name: 'date',
              groupInterval: ['Years', 'Quarters', 'Months', 'Days'],
            },
          ],
    ...
    it('Check group settings update using on proptery', (done: Function) => {
      pivotGridObj.dataSourceSettings.groupSettings[0].groupInterval = [
        'Years',
        'Quarters',
        'Months',
        'Days',
    ...
    it('Check date groups initially', (done: Function) => {
      setTimeout(() => {
        expect(
          pivotGridObj.element.querySelectorAll('th[aria-colindex="2"]')[0]
            .textContent
        ).toBe('Balance');
        (
          pivotGridObj.element.querySelector('.e-firstcell') as HTMLInputElement
    ...
    it('Update dataSourceSettings', (done: Function) => {
      ...
      pivotGridObj.dataSourceSettings.filters = [
    ...
    it('Check group settings update using on proptery', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          pivotGridObj.element.querySelectorAll('th[data-colindex="3"]')[0]
            .textContent
        ).toBe('Out of Range');
        expect(
          (
  ...
  describe(' -  Initial Rendering with range - PivotChart', () => {
    ...
    it('Group settings with chart - Days on values', (done: Function) => {
      pivotGridObj.dataSourceSettings.rows = [
        { name: 'date_date_group_years' },
        { name: 'date_date_group_quarters' },
        { name: 'date_date_group_months' },
      ];
      pivotGridObj.dataSourceSettings.values = [{ name: 'date' }];
      setTimeout(() => {
        expect(
          document
            .getElementById('PivotGrid_chart_Series_0_Point_0')
            .getAttribute('aria-label')
        ).toBe('Out of Range:4856.54, 27-31');
        done();
      }, 100);
    });
  ...
  describe('- Editing - normal', () => {
    ...
    it('render testing', (done: Function) => {
      setTimeout(() => {
        document
          .querySelectorAll('td[aria-colindex="4"]')[0]
          .dispatchEvent(event);
        done();
      }, 100);
    });
    ...
    it('click female-quantity', () => {
      expect(
        document.querySelectorAll('td[aria-colindex="3"]')[0].textContent
      ).toBe('47');
      if (
        document.querySelectorAll(
          '.e-drillthrough-grid td[aria-colindex="11"]'
        )[0]
      ) {
        document
          .querySelectorAll('.e-drillthrough-grid td[aria-colindex="11"]')[0]
          .dispatchEvent(event);
        document
          .querySelectorAll('.e-drillthrough-grid .e-numeric span')[1]
          .dispatchEvent(mousedown);
        document
          .querySelectorAll('.e-drillthrough-grid .e-numeric span')[1]
          .dispatchEvent(mouseup);
      ...
      pivotGridObj = new PivotView({
        dataSourceSettings: {
          ...
          calculatedFieldSettings: [
            { name: 'Total', formula: '"Sum(Total Cost)"' },
          ],
        },
        showFieldList: true,
        allowGrouping: true,
  ...
  describe('Chart-Grouping', () => {
    ...
    beforeAll((done: Function) => {
      ...
      pivotGridObj = new PivotView({
        ...
        chartSettings: {
          value: 'Amount',
          enableExport: true,
          chartSeries: { type: 'Pie', animation: { enable: false } },
    ...
    it('initial table rendering', (done: Function) => {
      setTimeout(() => {
        expect(pivotGridObj.engineModule.pivotValues[1][2].formattedText).toBe(
          'quantity'
        );
        done();
      }, 300);
    ...
    it('change column value', () => {
      document
        .querySelectorAll('.e-group-chart-columns-dropdown')[0]
        .dispatchEvent(mousedown);
      (
        document.querySelectorAll(
          '.e-group-chart-columns-dropdown .e-list-item'
        )[2] as HTMLElement
      ).click();
      expect(pivotGridObj.engineModule.pivotValues[2][2].formattedText).toBe(
        '15'
      );
    });
    it('change value', () => {
      document
        .querySelectorAll('.e-group-chart-values-drodown')[0]
        .dispatchEvent(mousedown);
      (
        document.querySelectorAll(
          '.e-group-chart-values-drodown .e-list-item'
        )[1] as HTMLElement
      ).click();
      expect(pivotGridObj.engineModule.pivotValues[2][2].formattedText).toBe(
        '15'
