...
import {
  ...
  PDFExport,
  VirtualScroll,
} from '../../src/pivotview/actions';
import { Toolbar } from '../../src/common/popups/toolbar';
import * as util from '../utils.spec';
import { getMemoryProfile, inMB, profile } from '../common.spec';
import { CalculatedField } from '../../src/common/calculatedfield/calculated-field';
import { MaskedTextBox, TextArea, TextBox } from '@hoyui/inputs';
...
describe('Pivot Olap Engine', () => {
  ...
  describe('Initial rendering', () => {
    ...
    it('Randering empty table', (done: Function) => {
      setTimeout(() => {
        ...
        expect(
          pivotGridObj.element.querySelectorAll('.e-content .e-rowcell')[1]
            .textContent
        ).toBe('No records to display');
        done();
      }, 2000);
    });
    it('inital rendering', (done: Function) => {
      ...
      setTimeout(() => {
        expect(pivotGridObj.pivotValues[0][1].actualText).toBe(
          '[Account].[Accounts].&[1]'
        );
        expect(pivotGridObj.pivotValues[0][1].formattedText).toBe(
          'Balance Sheet'
        );
    ...
    it('Drilling column header', (done: Function) => {
      (
        pivotGridObj.element.querySelectorAll('.e-expand')[0] as HTMLElement
      ).click();
      setTimeout(() => {
        expect(pivotGridObj.pivotValues[1][1].actualText).toBe(
          '[Account].[Accounts].&[2]'
        );
    ...
    it('Field list search checking - 1', (done: Function) => {
      ...
      setTimeout(() => {
        searchField = getInstance(
          document.querySelectorAll('.e-textbox.e-input')[0] as HTMLElement,
          TextBox
        ) as TextBox;
        ...
        expect(
          document.querySelector(
            '.e-pivotfieldlist-container .e-field-table .e-field-list ul li:not(.e-disable)'
          ).textContent
        ).toBe(
          'MeasuresFinancial ReportingAmountExchange RatesAverage RateEnd of Day Rate'
        );
        done();
      }, 3000);
    });
    ...
    it('Validating search & select node', (done: Function) => {
      ...
      ).toBe('ScenarioCustomer Geography');
      expect(
        document.querySelectorAll(
          '.e-pivotfieldlist-container .e-left-axis-fields .e-field-list-rows .e-pivot-button'
        )[0].textContent
      ).toBe('Customer Geography');
      let checkEle: NodeListOf<Element> = document.querySelectorAll(
        '.e-pivotfieldlist-container .e-field-table .e-field-list ul li:not(.e-disable) ul li .e-checkbox-wrapper'
      );
  ...
  describe('Features ensuring - Value sorting', () => {
    ...
    it('Perform value sorting - column - 1', (done: Function) => {
      (
        pivotGridObj.element
          .querySelectorAll('.e-table thead tr')[1]
          .querySelectorAll('.e-headercell')[1] as HTMLElement
      ).click();
      setTimeout(() => {
    ...
    it('Perform value sorting - User defined - 1', (done: Function) => {
      ...
      setTimeout(() => {
        expect(pivotGridObj.pivotValues[3][2].formattedText).toBe(
          '$3,250,075.00'
        );
        (
          pivotGridObj.element
            .querySelectorAll('.e-table thead tr')[2]
            .querySelectorAll('.e-headercell')[1] as HTMLElement
        ).click();
        done();
      }, 3000);
    ...
    it('Perform value sorting - User defined - 2', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          document.querySelectorAll('.e-table tr')[3].querySelectorAll('td')[0]
            .textContent
        ).toBe('Actual');
        done();
      }, 3000);
    ...
    it('Perform value sorting - User defined - grand total', (done: Function) => {
      (
        pivotGridObj.element
          .querySelectorAll('.e-table thead tr')[1]
          .querySelectorAll('.e-headercell')[4] as HTMLElement
      ).click();
      setTimeout(() => {
        expect(
          document.querySelectorAll('.e-table tr')[3].querySelectorAll('td')[0]
            .textContent
        ).toBe('Budget Variance %');
        done();
      }, 1000);
    });
    ...
    it('Perform value sorting - child level - 3', (done: Function) => {
      (
        pivotGridObj.element
          .querySelectorAll('.e-table thead tr')[2]
          .querySelectorAll('.e-headercell')[2] as HTMLElement
      ).click();
      setTimeout(() => {
        expect(
          pivotGridObj.element
    ...
    beforeAll((done: Function) => {
      ...
      pivotGridObj = new PivotView({
        dataSourceSettings: {
          ...
          rows: [
            { name: '[Account].[Account Number]', caption: 'Account Number' },
          ],
          values: [{ name: '[Measures].[Amount]', caption: 'Amount' }],
        },
        height: 350,
  ...
  describe('Calculated field', () => {
    ...
    it('Initial rendering', (done: Function) => {
      setTimeout(() => {
        expect(pivotGridObj.pivotValues[1][2].actualText).toBe(
          'Calculated field 1'
        );
        (
          pivotGridObj.element.querySelector(
    ...
    it('Open calculated Filed popup ', (done: Function) => {
      setTimeout(() => {
        ...
        (
          document.querySelectorAll('.e-calculated-field')[0] as HTMLElement
        ).click();
        done();
      }, 1000);
    });
    it('Open calculated popup - To check the clearFormula coverage', function (done) {
      setTimeout(function () {
        (
          document.querySelectorAll('.e-calc-clear-btn')[0] as HTMLElement
        ).click();
        done();
      }, 1000);
    });
    ...
    it('Creating calculated field 2 - 1', (done: Function) => {
      setTimeout(() => {
        expect(document.querySelectorAll('.e-olap-calc-outer-div').length).toBe(
          1
        );
        (
          getInstance(
            document.querySelectorAll(
              '.e-olap-calc-outer-div .e-pivot-calc-input'
            )[0] as HTMLElement,
            MaskedTextBox
          ) as MaskedTextBox
        ).value = 'Calculated field 2';
        (
          document.querySelectorAll(
            '.e-olap-calc-outer-div .e-pivot-formula'
    ...
    it('Creating calculated field 2 - 2', (done: Function) => {
      setTimeout(() => {
        ...
        (
          document.querySelectorAll(
            '.e-olap-calc-dialog-div .e-primary'
          )[0] as HTMLElement
        ).click();
        done();
      }, 1000);
    ...
    it('Removing calculated field - 1', (done: Function) => {
      setTimeout(() => {
        ...
        let treeObj: TreeView = getInstance(
          document.querySelectorAll('.e-pivot-treeview')[0] as HTMLElement,
          TreeView
        ) as TreeView;
        ...
        let mousedown: any = util.getEventObject(
          ...
          'mousedown',
          treeObj.element,
          li[0].querySelector('.e-icon-expandable')
        );
    ...
    it('Removing calculated field - 2', (done: Function) => {
      setTimeout(() => {
        ...
        let li: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
          treeObj.element.querySelectorAll('li .e-remove-report')
        );
        let mousedown: any = util.getEventObject(
          'MouseEvents',
          'mousedown',
    ...
    it('Removing calculated field - 3', (done: Function) => {
      setTimeout(() => {
        ...
        (
          document.querySelectorAll(
            '.e-pivot-error-dialog .e-primary'
