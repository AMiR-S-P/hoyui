...
import { Grid } from '@hoyui/grids';
import { CalculatedField } from '../../src/common/calculatedfield/calculated-field';
import { DrillThrough } from '../../src/pivotview/actions';
import { MaskedTextBox, NumericTextBox } from '@hoyui/inputs';
...
describe('Group By Date feature', () => {
  ...
  describe(' -  With All Features(Field list, Grouping Bar, Value sorting, Single value header and Editing', () => {
      ...
      pivotGridObj = new PivotView({
        dataSourceSettings: {
          ...
          formatSettings: [
            ...
            { name: 'date', format: 'dd/MM/yyyy-hh:mm a', type: 'date' },
          ],
          filterSettings: [
            { name: 'gender', items: ['male'], type: 'Exclude' },
          ],
          rows: [{ name: 'product', caption: 'Category' }],
          values: [
            { name: 'balance', caption: 'Balance' },
            { name: 'price', caption: 'Totals' },
          ],
          columns: [{ name: 'age' }],
          filters: [
            { name: 'gender', caption: 'Population' },
            { name: 'date', caption: 'TimeLine' },
          ],
          ...
          groupSettings: [
            ...
            },
            {
              ...
              type: 'Custom',
              customGroups: [
                { groupName: 'Four wheelers', items: ['Car', 'Tempo', 'Van'] },
                { groupName: 'Airways', items: ['Jet', 'Flight'] },
              ],
            },
          ],
    ...
    it('grouping bar render testing', () => {
      expect(
        pivotGridObj.element.querySelector('.e-grouping-bar')
      ).toBeDefined();
      pivotGridObj.dataBind();
      pivotGridObj.groupingBarSettings = {
        showFilterIcon: true,
        showRemoveIcon: true,
        showSortIcon: true,
      };
    ...
    it('check open field list popup', () => {
      (
        pivotGridObj.pivotFieldListModule.element.querySelector(
          '.e-toggle-field-list'
        ) as HTMLElement
      ).click();
    ...
    it('check close field list', () => {
      let controlWrapper: HTMLElement = document.querySelector(
        '.e-pivotfieldlist-container'
      );
      (controlWrapper.querySelector('.e-cancel-btn') as HTMLElement).click();
      expect(
        document
          .querySelector('.e-pivotfieldlist-container')
          .classList.contains('e-popup-close')
      );
    });
    it('Check code-behind groups initially', () => {
      expect(
        pivotGridObj.element.querySelectorAll('td[aria-colindex="1"]')[0]
          .textContent
      ).toBe('Airways');
      (
        pivotGridObj.element.querySelector('.e-firstcell') as HTMLInputElement
      ).click();
    });
    ...
    it('check sorting order field', () => {
      ...
      let pivotButtons: HTMLElement[] = [].slice.call(
        pivotGridObj.element
          .querySelector('.e-rows')
          .querySelectorAll('.e-pivot-button')
      );
    ...
    it('check filtering field', (done: Function) => {
      ...
      let pivotButtons: HTMLElement[] = [].slice.call(
        pivotGridObj.element
          .querySelector('.e-filters')
          .querySelectorAll('.e-pivot-button')
      );
      expect(pivotButtons.length).toBe(3);
      (pivotButtons[0].querySelector('.e-btn-filter') as HTMLElement).click();
      setTimeout(() => {
        let filterDialog: Dialog =
          pivotGridObj.pivotCommon.filterDialog.dialogPopUp;
        expect(filterDialog.element.classList.contains('e-popup-open')).toBe(
          true
        );
        done();
      }, 200);
    ...
    it('check all nodes on filter popup', () => {
      let treeObj: TreeView =
        pivotGridObj.pivotCommon.filterDialog.allMemberSelect;
      let memberTreeObj: TreeView =
        pivotGridObj.pivotCommon.filterDialog.memberTreeView;
      let filterDialog: Dialog =
        pivotGridObj.pivotCommon.filterDialog.dialogPopUp;
      let allNode: HTMLElement = treeObj.element.querySelector(
        '.e-checkbox-wrapper'
      );
      let checkEle: Element[] = <Element[] & NodeListOf<Element>>(
        memberTreeObj.element.querySelectorAll('.e-checkbox-wrapper')
      );
      ...
      let checkedEle: Element[] = <Element[] & NodeListOf<Element>>(
        memberTreeObj.element.querySelectorAll('.e-check')
      );
      expect(checkEle.length).toEqual(checkedEle.length);
      expect(
        filterDialog.element.querySelector('.e-ok-btn').getAttribute('disabled')
      ).toBe(null);
    ...
    it('check remove pivot button', () => {
      let pivotButton: HTMLElement = pivotGridObj.element
        .querySelector('.e-filters')
        .querySelector('.e-pivot-button') as HTMLElement;
      expect(pivotButton.id).toBe('PivotGrid_gender');
      (pivotButton.querySelector('.e-remove') as HTMLElement).click();
      pivotButton = pivotGridObj.element
        .querySelector('.e-filters')
        .querySelector('.e-pivot-button') as HTMLElement;
      expect(pivotButton).toBeTruthy();
    });
    it('check drag and drop pivot button', () => {
      let filterAxiscontent: HTMLElement =
        pivotGridObj.element.querySelector('.e-filters');
      let pivotButton: HTMLElement[] = [].slice.call(
        filterAxiscontent.querySelectorAll('.e-pivot-button')
      );
      expect(pivotButton.length).toEqual(2);
      let dragElement: HTMLElement =
        pivotButton[0].querySelector('.e-pvt-btn-content');
      ...
      (pivotGridObj.groupingBarModule as any).dropIndicatorUpdate(
        mouseOverEventArgs
      );
    ...
    it('Perform group option for false statement', () => {
      (
        document.querySelector(
          '#' + pivotGridObj.element.id + '_custom_group'
        ) as HTMLElement
      ).click();
      expect(document.querySelector('.e-pivot-error-dialog')).toBeTruthy();
      (
        document
          .querySelector('.e-pivot-error-dialog')
          .querySelector('.e-ok-btn') as HTMLElement
      ).click();
    });
    it('Perform ungroup option', (done: Function) => {
      setTimeout(function () {
        ...
        expect(
          document.querySelector('.e-pivot-error-dialog') === null
        ).toBeTruthy();
        (
          document.querySelector(
            '#' + pivotGridObj.element.id + '_custom_ungroup'
          ) as HTMLElement
        ).click();
        expect(
          pivotGridObj.element.querySelectorAll('td[aria-colindex="1"]')[1]
            .textContent
        ).toBe('Bike');
        expect(
          pivotGridObj.element
            .querySelectorAll('td[aria-colindex="1"]')[1]
            .querySelector('.e-expand')
        ).toBeTruthy();
        expect(
          pivotGridObj.element.querySelectorAll('td[aria-colindex="1"]')[3]
            .textContent
        ).toBe('Grand Total');
        done();
      }, 1000);
    ...
    it('Create new group from selction Jet keyboard ctrl + mouse click', function (done) {
      document
        .querySelector('[aria-colindex="1"][index="3"]')
        .dispatchEvent(ctrlClick);
      setTimeout(function () {
        expect(args.selectedCellsInfo[0].rowHeaders).toBe('Flight');
        document
          .querySelector('[aria-colindex="1"][index="4"]')
          .dispatchEvent(ctrlClick);
        done();
      }, 500);
    });
    it('Flight keyboard ctrl + mouse click', function () {
      expect(args.selectedCellsInfo[0].rowHeaders).toBe('Flight');
      pivotGridObj.lastCellClicked = document.querySelector(
        '[aria-colindex="1"][index="4"]'
      );
      let cell: HTMLElement = document.querySelector(
        '[aria-colindex="1"][index="4"]'
      );
      util.triggerMouseEvent(cell, 'contextmenu');
    });
    ...
    it('Perform group option for selected headers', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          document
            .querySelector('.e-group-field-settings')
            .querySelector('.e-group-caption-text')
            .getAttribute('value')
        ).toBe('Category3');
        done();
      }, 100);
    ...
    it('Assign new group name for selected headers', (done: Function) => {
      let input1: MaskedTextBox = getInstance(
        document.querySelector(
          '#' + pivotGridObj.element.id + 'group_input_option'
        ) as HTMLElement,
        MaskedTextBox
      ) as MaskedTextBox;
    ...
    it('Check updated new group selected headers in table', (done: Function) => {
      expect(
        document.querySelector('.e-group-field-settings') == null
      ).toBeTruthy();
      setTimeout(() => {
        expect(
          pivotGridObj.element.querySelectorAll('td[aria-colindex="1"]')[0]
        ...
        expect(
          pivotGridObj.element.querySelectorAll('td[aria-colindex="1"]')[2]
            .textContent
        ).toBe('Jet');
        done();
      }, 100);
    ...
    it('Context menu in column header for number grouping', (done: Function) => {
      expect(
        pivotGridObj.element.querySelectorAll('th[data-colindex="1"]')[0]
          .textContent
      ).toBe('27-31');
    ...
    it('Perform group option', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          document
            .querySelector('.e-group-field-settings')
            .querySelector('.e-group_interval_input')
            .getAttribute('value')
        ).toBe('5');
        done();
      }, 100);
    ...
    it('Change grouping interval to 10', (done: Function) => {
      let option1: CheckBox = getInstance(
        document.querySelector(
          '#' + pivotGridObj.element.id + 'group_start_option'
        ) as HTMLElement,
        CheckBox
      ) as CheckBox;
      let input1: NumericTextBox = getInstance(
        document.querySelector(
          '#' + pivotGridObj.element.id + 'group_interval_input'
        ) as HTMLElement,
        NumericTextBox
      ) as NumericTextBox;
    ...
    it('Check updated number grouping in table', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        pivotGridObj.lastCellClicked = pivotGridObj.element.querySelectorAll(
          'td[aria-colindex="1"]'
        )[2];
        let cell: HTMLElement = pivotGridObj.element.querySelectorAll(
          'td[aria-colindex="1"]'
        )[2] as HTMLElement;
        util.triggerMouseEvent(cell, 'contextmenu');
        done();
    ...
    it('Perform ungrouping custom groups', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        pivotGridObj.lastCellClicked =
          document.querySelector('.e-columnsheader');
        let cell: HTMLElement = document.querySelector('.e-columnsheader');
        util.triggerMouseEvent(cell, 'contextmenu');
        done();
      }, 100);
    ...
    it('Create new group from selction 20 keyboard shift + mouse click', function (done) {
      document
        .querySelectorAll('th[data-colindex="1"]')[0]
        .dispatchEvent(shiftClick);
      setTimeout(function () {
        expect(args.selectedCellsInfo[0].columnHeaders).toBe('20');
        document
          .querySelectorAll('th[data-colindex="11"]')[0]
          .dispatchEvent(shiftClick);
        done();
      }, 500);
    });
    it('25 keyboard shift + mouse click', function (done) {
      ...
      setTimeout(function () {
        pivotGridObj.lastCellClicked = document.querySelectorAll(
          'th[data-colindex="1"]'
        )[0];
        let cell: HTMLElement = document.querySelectorAll(
          'th[data-colindex="1"]'
        )[0] as HTMLElement;
        util.triggerMouseEvent(cell, 'contextmenu');
        done();
    ...
    it('click 20-balance for editing', (done: Function) => {
      document
        .querySelectorAll('td[aria-colindex="2"]')[0]
        .dispatchEvent(event);
      setTimeout(() => {
        document
          .querySelectorAll('.e-drillthrough-grid td[aria-colindex="11"]')[0]
          .dispatchEvent(event);
        document
          .querySelectorAll('.e-drillthrough-grid .e-numeric span')[0]
        ...
        expect(
          document
            .querySelectorAll('.e-drillthrough-grid .e-numeric input')[0]
            .getAttribute('aria-valuenow')
        ).toBe('19');
        document
          .querySelectorAll('.e-drillthrough-grid .e-tbar-btn')[3]
          .dispatchEvent(click);
        (
          document.querySelectorAll(
            '.e-drillthrough-dialog .e-dlg-closeicon-btn'
          )[0] as HTMLElement
        ).click();
        done();
      }, 400);
    ...
    it('check drag and drop pivot button from row to filter axis', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(pivotButton[3].id).toBe(
          'PivotGrid_product_custom_group_custom_group'
        );
        done();
      }, 100);
    });
    ...
    it('Context menu in row header for date grouping', (done: Function) => {
      expect(
        pivotGridObj.element.querySelector('.e-rowsheader').textContent
      ).toBe('Bike');
      pivotGridObj.lastCellClicked = document.querySelector('.e-rowsheader');
      let cell: HTMLElement = document.querySelector('.e-rowsheader');
      util.triggerMouseEvent(cell, 'contextmenu');
    ...
    it('Perform group option', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          document
            .querySelector('.e-group-field-settings')
            .querySelector('.e-multi-select-wrapper')
            .querySelector('.e-delim-view.e-delim-values').textContent
        ).toBe('Years, Quarters');
        done();
      }, 400);
    });
    it('Change grouping interval to Months', (done: Function) => {
      ...
      let input1: MultiSelect = getInstance(
        document.querySelector(
          '#' + pivotGridObj.element.id + 'group_interval_input'
        ) as HTMLElement,
        MultiSelect
  ...
  describe('Filter member', () => {
    ...
    it('Filter testing4', () => {
      expect(
        (document.querySelectorAll('.e-maskedtextbox')[0] as HTMLInputElement)
          .value === ''
      ).toBeTruthy();
      (
        document.querySelectorAll('.e-maskedtextbox')[0] as HTMLInputElement
    ...
    it('Filter testing6', function (done) {
      setTimeout(function () {
        expect(
          document.querySelectorAll('.e-btn-filter').length > 10
        ).toBeTruthy();
        done();
      }, 1000);
    });
    it('Filter testing7', () => {
      (document.querySelectorAll('.e-btn-filter')[4] as HTMLElement).click();
      expect(
        document.querySelectorAll('.e-member-editor-container-outer-div')
          .length > 0
      ).toBeTruthy();
    });
    it('Filter testing8', () => {
  ...
  describe('- Dynamically updating the data source with group settings', () => {
    ...
    it('- Updating entire data source setting', (done: Function) => {
      ...
      setTimeout(() => {
        expect(document.querySelectorAll('.e-rowsheader')[0].textContent).toBe(
          '1991'
        );
        done();
      }, 1000);
