...
import { createElement, remove, EmitType, closest, extend } from '@hoyui/base';
...
describe('Pivot Field List Rendering', () => {
  ...
  describe('Check node drag and drop Actions', () => {
    ...
    it('check node drag start for false case', () => {
      ...
      let filterAxiscontent: HTMLElement =
        leftAxisPanel.querySelector('.e-filters');
      let li: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
        treeObj.element.querySelectorAll('li')
      );
      ...
      let mousedown: any = util.getEventObject(
        ...
        'mousedown',
        treeObj.element,
        li[1].querySelector('.e-list-text'),
        15,
        10
      );
      EventHandler.trigger(treeObj.element, 'mousedown', mousedown);
    ...
    it('drag and drop node to filter axis with button axis', (done: Function) => {
      ...
      (fieldListObj.pivotButtonModule as any).parentElement =
        fieldListObj.element.querySelector('.e-pivotfieldlist-container');
      (fieldListObj.pivotButtonModule as any).updateDropIndicator(
        mouseEventArgs
      );
      let mouseOverEventArgs: any = extend({}, mousemove, null, true);
      mouseOverEventArgs.type = 'mouseover';
      mouseOverEventArgs.srcElement = mouseOverEventArgs.target = closest(
        mousemove.srcElement,
        '.e-droppable'
      );
      (fieldListObj.axisTableModule as any).updateDropIndicator(
        mouseOverEventArgs
      );
    ...
    it('show filter popup', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          pivotCommon.filterDialog.dialogPopUp.element.classList.contains(
            'e-popup-open'
          )
        ).toBe(true);
        done();
    ...
    it('drag and drop node to value axis', (done: Function) => {
      ...
      let valueAxiscontent: HTMLElement =
        rightAxisPanel.querySelector('.e-values');
      let li: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
        treeObj.element.querySelectorAll('li')
      );
      let pivotButton: HTMLElement[] = [].slice.call(
    ...
    it('drag and drop node to value axis with button axis', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(pivotButton[0].getAttribute('data-uid')).toBe('advance');
        expect(
          (pivotButton[0].querySelector('.e-pvt-btn-content') as HTMLElement)
            .innerText
        ).toEqual('Sum of droppedButton');
        done();
      }, 1000);
    });
  ...
  describe('Check pivot button drag and drop Actions', () => {
    ...
    it('drag/drop pivot button from axis field to same axis field', (done: Function) => {
      ...
      expect(pivotButton.length).toEqual(2);
      let dragElement: HTMLElement =
        pivotButton[pivotButton.length - 1].querySelector('.e-draggable');
      let mousedown: any = util.getEventObject(
        'MouseEvents',
        'mousedown',
        dragElement,
    ...
    it('drag/drop pivot button from axis field to button position', (done: Function) => {
      ...
      let columnButtonElement: Element =
        columnAxiscontent.querySelectorAll('.e-pivot-button')[0];
      expect(pivotButton.length).toEqual(2);
      let dragElement: HTMLElement =
        pivotButton[0].querySelector('.e-draggable');
      let mousedown: any = util.getEventObject(
    ...
    it('drag/drop calculated field to filter axis field', (done: Function) => {
      ...
      expect(pivotButton.length).toEqual(3);
      let dragElement: HTMLElement =
        pivotButton[1].querySelector('.e-draggable');
      let mousedown: any = util.getEventObject(
        'MouseEvents',
        'mousedown',
        dragElement,
      ...
      setTimeout(() => {
        expect(
          pivotCommon.errorDialog.errorPopUp.element.classList.contains(
            'e-popup-open'
          )
        ).toBeTruthy;
        (
          pivotCommon.errorDialog.errorPopUp.element.querySelector(
            '.e-ok-btn'
          ) as HTMLElement
        ).click();
        done();
  ...
  describe('Drag and drop restriction event support', () => {
    ...
    beforeAll((done: Function) => {
      ...
      pivotGridObj = new PivotView({
        ...
        fieldDrop: function (args) {
          if (args.draggedAxis != args.dropAxis) args.cancel = true;
        },
      });
    ...
    it('Drag value axis field to row axis field in grouping bar', (done: Function) => {
      ...
      let dragElement: HTMLElement =
        pivotButton[0].querySelector('.e-draggable');
      let draggedElement: string =
        pivotButton[0].querySelector('.e-pvt-btn-content').textContent;
      let mousedown: any = util.getEventObject(
        'MouseEvents',
        'mousedown',
        dragElement,
    ...
    it('Drag row axis field to row axis field in grouping bar', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(draggedElement == pivotButton[axisLength - 1].innerText).toEqual(
          true
        );
        done();
      }, 1000);
    ...
    it('Drag row axis field to row axis field in field list', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(draggedElement == pivotButton[axisLength - 2].innerText).toEqual(
          true
        );
        done();
      }, 1000);
    ...
    it('drag value axis field to row axis field in grouping bar & check filter state', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        let dragElement: HTMLElement =
          pivotButton[0].querySelector('.e-draggable');
        gridFilterElement =
          pivotGridObj.element.querySelectorAll('td')[2].innerText;
        let mousedown: any = util.getEventObject(
          'MouseEvents',
          'mousedown',
          dragElement,
      ...
      setTimeout(() => {
        ...
        expect(
          pivotGridObj.element
            .querySelectorAll('.e-rows .e-pivot-button .e-btn-filter')[1]
            .classList.contains('e-pv-filtered')
        ).toBe(true);
        done();
      }, 1000);
    });
    ...
    it('expect sort state icon after drag row axis field to value axis field', () => {
      ...
      expect(
        document
          .querySelectorAll('.e-content tr')[0]
          .querySelector('td .e-cellvalue').textContent == sortElement
      ).toBe(true);
      expect(
        document
          .querySelectorAll('.e-group-rows .e-sort')[0]
    ...
    it('uncheck country field', () => {
      document
        .querySelectorAll('.e-treeview li')[4]
    ...
    it('expect country field check state', () => {
      expect(
        document
          .querySelectorAll('.e-treeview li')[4]
          .querySelector('.e-frame')
          .classList.contains('e-check') === true
      ).toBe(true);
    });
    it('check country field', () => {
      document
        .querySelectorAll('.e-treeview li')[1]
        .querySelector('.e-frame')
        .dispatchEvent(click);
      expect(true).toBe(true);
    });
    it('expect quantity field check state', () => {
      expect(
        document
          .querySelectorAll('.e-treeview li')[1]
          .querySelector('.e-frame')
          .classList.contains('e-check') === false
      ).toBe(true);
    });
  });
  ...
  describe('Grid base module - ', () => {
    describe('- Grid properties - ', () => {
      ...
      beforeAll(() => {
        ...
        pivotGridObj = new PivotView({
          dataSourceSettings: {
            ...
            drilledMembers: [
              ...
              },
              {
                ...
                items: [
                  ...
                  '[Customer].[Customer Geography].[State-Province].&[NSW]&[AU]',
                ],
                delimiter: '##',
              },
              {
            ...
            rows: [{ name: '[Date].[Fiscal]', caption: 'Date Fiscal' }],
            columns: [
              {
                name: '[Customer].[Customer Geography]',
      ...
      it('pivotgrid setPersist', () => {
        ...
        expect(
          !isNullOrUndefined(JSON.parse(persistdata).dataSourceSettings)
        ).toBeTruthy();
      });
      it('Mouse hover event testing - Value cell', (done: Function) => {
        let target: HTMLElement = pivotGridObj.element.querySelector(
          'td[aria-colindex="3"]'
        );
        jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000;
        ...
        setTimeout(() => {
          expect(
            document.querySelectorAll('.e-tooltip-wrap p.e-tooltipcontent')[2]
              .innerHTML
          ).toBe('$72,200.48');
          done();
        }, 2000);
      ...
      it('Mouse hover event testing - Value cell', (done: Function) => {
        let target: HTMLElement = pivotGridObj.element.querySelectorAll(
          'td[aria-colindex="3"]'
        )[1] as HTMLElement;
        jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000;
        triggerMouseEvent(target, 'mouseover');
      ...
      it('Mouse hover event testing - top left cell', (done: Function) => {
        let target: HTMLElement =
          pivotGridObj.element.querySelector('.e-rowcell');
        triggerMouseEvent(target, 'mouseover');
        setTimeout(() => {
          expect(
            pivotGridObj.element.querySelectorAll('.e-expand').length
          ).toBe(27);
          done();
        }, 2000);
      });
