...
describe('PivotFieldList spec', () => {
  ...
  describe('Pivot Field List base module', () => {
    describe('Field List properties', () => {
      ...
      beforeAll((done: Function) => {
        ...
        util.disableDialogAnimation(
          fieldListObj.dialogRenderer.fieldListDialog
        );
      });
      ...
      it('set locale property', () => {
        let element: HTMLElement = document
          .getElementById('PivotFieldList_Container_title')
          .querySelector('.e-title-content');
    ...
    describe('Binding an empty data source', () => {
      ...
      it('Loading the remote data', function (done) {
        ...
        setTimeout(function () {
          expect(
            document.querySelector('.e-pivotfieldlist-container') !== null
          ).toBeTruthy;
          done();
        }, 3000);
      });
      it('Checking the pivot buttons', function (done) {
        setTimeout(function () {
          expect(
            document
              .querySelectorAll('.e-axis-content')[1]
              .querySelectorAll('.e-pivot-button').length > 0
          ).toBeTruthy;
          expect(
            document
              .querySelectorAll('.e-axis-content')[2]
              .querySelectorAll('.e-pivot-button').length > 0
          ).toBeTruthy;
          expect(
            document
              .querySelectorAll('.e-axis-content')[3]
              .querySelectorAll('.e-pivot-button').length > 0
          ).toBeTruthy;
          done();
        }, 1000);
  ...
  describe('Pivot Field List Rendering', () => {
    describe('Field List with Tree Node Action', () => {
      ...
      beforeAll(() => {
        ...
        fieldListObj = new PivotFieldList({
          ...
          calculatedFieldCreate: (args: CalculatedFieldCreateEventArgs) => {
            ...
            expect(args.cancel).toBe(false);
            console.log(
              'CreateCalcaltedFieldName: ' + args.calculatedField.name
            );
          },
        });
        fieldListObj.appendTo('#PivotFieldList');
      ...
      it('check field list tree view', () => {
        ...
        expect(
          fieldListObj.treeViewModule.fieldTable.element.classList.contains(
            'e-field-list'
          )
        );
      });
      ...
      it('checked node check axis button', (done: Function) => {
        setTimeout(() => {
          ...
          expect(
            pivotButtons[pivotButtons.length - 1].getAttribute('data-uid')
          ).toBe('pno');
          done();
        });
      });
      ...
      it('check tree header node with filter popup', (done: Function) => {
        setTimeout(() => {
          ...
          (
            fieldListObj.element
              .getElementsByClassName('e-member-editor-dialog')[0]
              .getElementsByClassName('e-ok-btn')[0] as HTMLElement
          ).click();
          done();
        }, 2000);
      });
      it('Node selecting with space key - check', (done: Function) => {
        ...
        let fileds: HTMLElement[] = [].slice.call(
          fieldListObj.element.querySelectorAll('.e-list-item')
        );
        (fieldListObj.treeViewModule.fieldTable as any).focusIn();
        keyboardEventArgs.action = 'space';
        (fieldListObj.treeViewModule.fieldTable as any).keyActionHandler(
          keyboardEventArgs
        );
        setTimeout(() => {
  ...
  describe('Pivot Field List Rendering', () => {
    describe('Check Sort Actions', () => {
      ...
      it('check sort icon disabled', () => {
        ...
        expect(pivotButtons.length).toBeGreaterThan(0);
        expect(
          pivotButtons[0].querySelector('.e-icons').classList.contains('e-drag')
        ).toBeTruthy;
      });
      ...
      it('testing on open error dialog with message', (done: Function) => {
        ...
        setTimeout(() => {
          expect(
            fieldListObj.pivotCommon.errorDialog.errorPopUp.element.classList.contains(
              'e-popup-open'
            )
          ).toBe(true);
          expect(
            fieldListObj.pivotCommon.errorDialog.errorPopUp.element.querySelector(
              '.e-dlg-header'
            ).textContent
          ).toBe(title);
          expect(
      ...
      it('Sort descending order field', (done: Function) => {
        setTimeout(() => {
          fieldListObj.dataSourceSettings.sortSettings = [
            {
              name: 'state',
              order: 'Descending',
              membersOrder: ['New Jercy', 'Delhi'],
            },
          ];
          expect(true).toBe(true);
          done();
  ...
  describe('Exclude Fields from fieldlist', () => {
    ...
    it('exclude fields ui check', (done: Function) => {
      setTimeout(() => {
        expect(
          (fieldListObj.element.querySelector('.e-list-parent') as HTMLElement)
            .children.length
        ).toBe(13);
  ...
  describe('Field list - Field searching', () => {
    ...
    it('checking search - 1 && validating - field list', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          fieldListObj.element.querySelectorAll(
            '.e-field-table .e-field-list ul li:not(.e-disable)'
          )[0].textContent
        ).toBe('pno');
    ...
    it('Ensuring the searched node - on node clicking', (done: Function) => {
      let checkEle: Element[] = <Element[] & NodeListOf<Element>>(
        fieldListObj.element.querySelectorAll(
          '.e-field-table .e-field-list ul li:not(.e-disable) .e-checkbox-wrapper'
        )
      );
      closest(checkEle[0], 'li').dispatchEvent(down);
      closest(checkEle[0], 'li').dispatchEvent(up);
  ...
  describe('Field searching - popup', () => {
    ...
    it('Initial rendering', (done: Function) => {
      setTimeout(() => {
        ...
        (
          pivotGridObj.element.querySelector('.e-select-table') as HTMLElement
        ).click();
        done();
      }, 2000);
    });
