...
import { createElement, remove, closest, getInstance } from '@hoyui/base';
...
describe('Pivot Field List Slicer Appearance', () => {
  ...
  describe('Check Filter Actions', () => {
        ...
        dataSourceSettings: {
          ...
          filterSettings: [
            ...
            { name: 'gender', type: 'Exclude', items: ['male'] },
            {
              name: 'email',
              type: 'Exclude',
              items: ['gwendixon@icology.com', 'skinnerward@grok.com'],
            },
          ],
          rows: [{ name: 'product' }],
          columns: [{ name: 'gender' }],
    ...
    it('close filter popup by cancel', (done: Function) => {
      (
        pivotCommon.filterDialog.dialogPopUp.element.querySelector(
          '.e-cancel-btn'
        ) as HTMLElement
      ).click();
      jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000;
    ...
    it('check all nodes on filter popup', () => {
      ...
      expect(
        pivotCommon.filterDialog.dialogPopUp.element
    ...
    it('check filter button state after update', (done: Function) => {
      ...
      setTimeout(() => {
        let pivotButtons: HTMLElement[] = [].slice.call(
          filterAxis.querySelectorAll('.e-pivot-button')
        );
        expect(
          (pivotButtons[0].querySelector('.e-pvt-btn-content') as HTMLElement)
            .innerText
        ).toEqual('eyeColor (All)');
        done();
      }, 1000);
    });
    ...
    it('un-check single node on filter popup', () => {
      ...
      let allNode: HTMLElement =
        pivotCommon.filterDialog.allMemberSelect.element.querySelector(
          '.e-checkbox-wrapper'
        );
      expect(allNode.querySelector('.e-frame').classList.contains('e-stop'))
        .toBeTruthy;
      expect(
        pivotCommon.filterDialog.dialogPopUp.element
          .querySelector('.e-ok-btn')
    ...
    it('check filter button after update', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          (pivotButtons[3].querySelector('.e-pvt-btn-content') as HTMLElement)
            .innerText
        ).toEqual('state (All)');
        done();
      }, 1000);
    ...
    it('check filter popup after update', () => {
      ...
      expect(pivotButtons.length).toBeGreaterThan(0);
      expect(
        pivotButtons[pivotButtons.length - 1]
          .querySelector('.e-btn-filter')
          .classList.contains('e-pv-filtered')
      ).toBeTruthy;
    });
    it('check filter button state', (done: Function) => {
      fieldListObj.dataSourceSettings.filterSettings = [
    ...
    it('check single node on filter', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          (pivotButtons[4].querySelector('.e-pvt-btn-content') as HTMLElement)
            .innerText
        ).toEqual('pno (MEWD9812)');
        done();
      }, 1000);
    });
    ...
    it('drag and drop pivot button from filter axis to row axis', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(
          (
            pivotButton[pivotButton.length - 1].querySelector(
              '.e-pvt-btn-content'
            ) as HTMLElement
          ).innerText
        ).toEqual('eyeColor');
  ...
  describe('- Searching for field members', () => {
    ...
    it('Searching for field members that start with the letter A', (done: Function) => {
      ...
      (document.querySelectorAll('.e-pv-filter')[0] as HTMLElement).click();
      (
        document.querySelectorAll('.e-maskedtextbox.e-input')[0] as any
      ).ej2_instances[0].value = 'A';
      (
        document.querySelectorAll('.e-maskedtextbox.e-input')[0] as any
      ).ej2_instances[0].element.dispatchEvent(
        new Event('keyup', { bubbles: true })
      );
      setTimeout(() => {
        expect(document.querySelectorAll('.e-list-text')[0].textContent).toBe(
          'All'
        );
    ...
    it('Selecting field member', (done: Function) => {
      setTimeout(() => {
        ...
        let checkEle: Element[] = <Element[] & NodeListOf<Element>>(
          treeObj.element.querySelectorAll('.e-fullrow')
        );
        expect(checkEle.length).toBeGreaterThan(0);
    ...
    it('Sorting field member', (done: Function) => {
      setTimeout(() => {
        (document.querySelectorAll('.e-pv-filter')[0] as HTMLElement).click();
        (
          document.querySelectorAll('.e-sort-ascend-icon')[0] as HTMLElement
        ).click();
        done();
      }, 1000);
    });
    ...
    it('Selecting a field from the dropdown menu', (done: Function) => {
      setTimeout(() => {
        ...
        document
          .getElementsByClassName('e-ddl')[0]
          .dispatchEvent(new Event('mousedown', { bubbles: true }));
        document
          .querySelectorAll('.e-value-options .e-list-item')[0]
    ...
    it('Selecting a filter operator from the dropdown menu', (done: Function) => {
      setTimeout(() => {
        (document.querySelectorAll('.e-btn-filter')[0] as HTMLElement).click();
        document
          .getElementsByClassName('e-ddl')[1]
          .dispatchEvent(new Event('mousedown', { bubbles: true }));
        document
          .querySelectorAll('.e-filter-operator .e-list-item')[1]
          .dispatchEvent(new Event('click', { bubbles: true }));
        document
          .getElementsByClassName('e-ddl')[1]
          .dispatchEvent(new Event('mousedown', { bubbles: true }));
        document
          .querySelectorAll('.e-filter-operator .e-list-item')[6]
          .dispatchEvent(new Event('click', { bubbles: true }));
        document
          .querySelectorAll('.e-spin-down')[0]
          .dispatchEvent(new Event('mouseup', { bubbles: true }));
        document
          .querySelectorAll('.e-spin-down')[1]
          .dispatchEvent(new Event('mouseup', { bubbles: true }));
        (document.querySelector('.e-cancel-btn') as HTMLElement).click();
        done();
      }, 1000);
