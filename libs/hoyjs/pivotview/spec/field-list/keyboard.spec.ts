...
describe('Pivot Rendering', () => {
  ...
  describe('Testing on keyboard interaction with Field List-Popup mode', () => {
    ...
    it('Check enter key for filter action', () => {
      let fieldListIcon: HTMLElement = fieldListObj.element.querySelector(
        '.e-toggle-field-list'
      ) as HTMLElement;
    ...
    it('check field list dialog with targetID', () => {
      expect(
        !isNullOrUndefined(elem.querySelector('.e-pivotfieldlist-container'))
      );
    });
  });
  describe('Testing on keyboard interaction with PivotGrid with GroupingBar', () => {
    ...
    it('Check sort action', (done: Function) => {
      ...
      let pivotButtons: HTMLElement[] = [].slice.call(
        pivotGridObj.element.querySelectorAll('.e-pivot-button')
      );
      ...
      setTimeout(() => {
        expect(
          pivotButtons[pivotButtons.length - 1].querySelector('.e-descend')
        ).toBeTruthy;
        done();
      }, 1000);
    });
    ...
    it('Check remove action', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(pivotButtons.length).toBeGreaterThan(0);
        let pivotButtonUpdate: HTMLElement[] = [].slice.call(
          pivotGridObj.element.querySelectorAll('.e-pivot-button')
        );
    ...
    it('Check tab action to grid cell focus', (done: Function) => {
      jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000;
      setTimeout(() => {
        expect(pivotGridObj.grid.element.querySelector('.e-focused'))
          .toBeTruthy;
        done();
      }, 1000);
    });
  ...
  describe('Testing on keyboard interaction with PivotGrid with FieldList', () => {
    ...
    it('Check enter action to grid cell', (done: Function) => {
      ...
      let gridcell: Element = closest(
        pivotGridObj.grid.element.querySelector('.e-expand'),
        '.e-rowcell'
      );
  ...
  describe('Testing Pivot Grid aggregation module on keyboard interaction in grouping bar with fieldlist', () => {
    ...
    it('check dropdown icon', () => {
      var valueField: HTMLElement =
        pivotGridObj.element.querySelector('.e-group-values');
      var pivotButtons: HTMLElement[] = [].slice.call(
    ...
    it('agrregation options check', (done: Function) => {
      const ele: HTMLElement = select(
        '#' + pivotGridObj.element.id + '_ValueDialog',
        document
      );
      let dialog: Dialog = ele
        ? (getInstance(ele, Dialog) as Dialog)
        : undefined;
      let dialogElement: HTMLElement = dialog.element;
      let dropdownlist: any = getInstance(
        dialogElement.querySelector(
          '#' + 'PivotGrid_type_option'
        ) as HTMLElement,
        DropDownList
      );
      let fieldsddl: any = getInstance(
        dialogElement.querySelector(
          '#' + 'PivotGrid_base_field_option'
        ) as HTMLElement,
        DropDownList
      );
    ...
    it('check pivot grid aggregation', () => {
      expect(pivotGridObj.pivotValues[1][2].formattedText).toEqual(
        '$29,322.76'
      );
    });
    ...
    it('select context Menu', (done: Function) => {
      expect(
        document.getElementById('PivotGrid_PivotFieldListvalueFieldContextMenu')
      ).toBeTruthy;
      let menuObj: any = (
    ...
    it('close the fieldlist dialog', (done: Function) => {
      let fieldListWrapper = document.getElementById(
        'PivotGrid_PivotFieldList_Container'
      ) as HTMLElement;
      jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000;
      setTimeout(() => {
        expect(
          (
            fieldListWrapper.querySelector('.e-cancel-btn') as HTMLElement
          ).click()
        ).toBeTruthy;
        done();
      }, 1000);
    ...
    it('destroy aggregate menu', () => {
      ...
      menuOption.destroy();
      expect((pivotGridObj.pivotButtonModule.menuOption as any).menuInfo[0])
        .toBeUndefined;
      expect((pivotGridObj.pivotButtonModule.menuOption as any).valueDialog)
        .toBeUndefined;
    });
  });
  it('memory leak', () => {
...
describe('Testing', () => {
  ...
  describe('on keyboard interaction with Field List', () => {
    ...
    it('Check shiftS key for sort action', () => {
      let pivotButtons: HTMLElement[] = [].slice.call(
        fieldListObj.element.querySelectorAll('.e-pivot-button')
      );
      expect(pivotButtons.length).toBeGreaterThan(0);
      keyModule.keyActionHandler({
        action: 'shiftS',
        target: pivotButtons[2],
        preventDefault: (): void => {
          /** Null */
        },
      });
      ...
      addClass(
        [
          document.querySelectorAll(
            '.e-pivot-calc-dialog-div .e-list-item'
          )[0] as HTMLElement,
        ],
        ['e-hover', 'e-node-focus']
      );
      const ele: HTMLElement = select(
        '#' + fieldListObj.element.id + 'calculateddialog',
        document
      );
      const cfDialog: Dialog = ele
        ? (getInstance(ele, Dialog) as Dialog)
        : undefined;
      cField.keyActionHandler({
        action: 'enter',
        currentTarget: cfDialog.element,
        preventDefault: (): void => {
          /** Null */
        },
      });
