...
import { MenuEventArgs, TreeView } from '@hoyui/navigations';
import {
  LoadEventArgs,
...
describe('Field List rendering on mobile device', () => {
  ...
  describe('Static rendering', () => {
    ...
    it('control class testing for mobile device', () => {
      expect(
        fieldListObj.element.classList.contains('e-pivotfieldlist')
      ).toEqual(true);
    ...
    it('check on axis view change', (done: Function) => {
      let element: HTMLElement = fieldListObj.element.querySelector(
        '.e-adaptive-container'
      );
      expect(
        [].slice.call(element.querySelectorAll('.e-toolbar-item')).length
      ).toEqual(5);
      let headerElement: HTMLElement[] = [].slice.call(
      ...
      setTimeout(() => {
        ...
        let addButton: HTMLElement = element
          .querySelector('.e-field-list-footer')
          .querySelector('.e-field-list-btn');
    ...
    it('check on axis view change to calculated field', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        let addButton: HTMLElement = element
          .querySelector('.e-field-list-footer')
          .querySelector('.e-calculated-field-btn');
        expect(addButton.classList.contains('e-disable')).not.toBeTruthy;
        done();
      }, 100);
    });
    ...
    it('check sorting pivotbutton', (done: Function) => {
      ...
      expect(
        element.querySelector('.e-content').querySelector('.e-item.e-active')
      ).not.toBeUndefined;
      let contentElement: HTMLElement = element
        .querySelector('.e-content')
        .querySelector('.e-item.e-active');
      let pivotButtons: HTMLElement[] = [].slice.call(
        contentElement.querySelectorAll('.e-pivot-button')
      );
    ...
    it('Open fields dialog', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          document
            .querySelector('.e-adaptive-field-list-dialog')
    ...
    it('check add fields to current axis', (done: Function) => {
      ...
      setTimeout(() => {
        expect(document.querySelector('.e-adaptive-field-list-dialog'))
          .toBeUndefined;
        let contentElement: HTMLElement = element
          .querySelector('.e-content')
          .querySelector('.e-item.e-active');
    ...
    it('check add fields on search option', (done: Function) => {
      ...
      let searchOption: MaskedTextBox = (fieldListObj.treeViewModule as any)
        .editorSearch;
    ...
    it('check on calculated field add button', (done: Function) => {
      ...
      setTimeout(() => {
        expect(document.querySelectorAll('.e-pivot-accord').length > 0)
          .toBeTruthy;
    ...
    it('check on calculated field apply button', (done: Function) => {
      ...
      (
        getInstance(
          fieldListObj.element.querySelector(
            '#' +
              (fieldListObj.calculatedFieldModule as any).parentID +
              'ddlelement'
          ) as HTMLElement,
          MaskedTextBox
        ) as MaskedTextBox
      ).value = 'ss';
      let formatString: MaskedTextBox = getInstance(
        document.querySelector(
          '#' + fieldListObj.element.id + 'Custom_Format_Element'
        ) as HTMLElement,
        MaskedTextBox
      ) as MaskedTextBox;
      ...
      setTimeout(() => {
        expect(
          (document.querySelector('.e-pivot-calc-input') as any).value === ''
        ).toBeTruthy;
        (document.querySelector('.e-calculated-field-btn') as any).click();
        done();
      }, 1000);
    ...
    it('check on calculated field add field', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          (document.querySelector('.e-pivot-formula') as any).value ===
            '"DistinctCount(pno)""Sum(advance)"'
        ).toBeTruthy();
    ...
    it('check on calculated field change existing formula', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        expect(document.querySelectorAll('.e-pivot-error-dialog').length > 0)
          .toBeTruthy;
    ...
    it('check on calculated field edit icon', (done: Function) => {
      ...
      setTimeout(() => {
        expect(document.querySelectorAll('.e-pivot-accord').length > 0)
          .toBeTruthy;
        let accordions: HTMLElement[] = [].slice.call(
          document.querySelectorAll('.e-pivot-accord .e-acrdn-item')
        );
        expect(accordions.length).toBe(20);
        let calcElement: HTMLElement = accordions[
          accordions.length - 1
        ] as HTMLElement;
        expect(calcElement).toBeTruthy;
        expect(calcElement.textContent).toBe('New (Calculated Field)');
        (
          calcElement.querySelector(
            'div.e-acrdn-header-icon > span'
          ) as HTMLElement
        ).click();
        done();
      }, 1000);
    ...
    it('check on edited calculated field info', (done: Function) => {
      ...
      expect(
        (document.querySelector('.e-custom-format-input') as any).value
      ).toBe('');
      (document.querySelector('.e-pivot-formula') as any).value = '(100/10)+5';
      let inputObj = getInstance(
        fieldListObj.element.querySelector(
          '#' +
            (fieldListObj.calculatedFieldModule as any).parentID +
            'ddlelement'
    ...
    it('check on calculated field button edit option', (done: Function) => {
      ...
      setTimeout(() => {
        ...
        let addButton: HTMLElement = element
          ...
          .querySelector('.e-field-list-btn');
        expect(addButton.classList.contains('e-disable')).not.toBeTruthy;
        let contentElement: HTMLElement = element.querySelector(
          '.e-field-list-values'
        );
        let pivotButtons: HTMLElement[] = [].slice.call(
          contentElement.querySelectorAll('.e-pivot-button')
        ...
        expect(pivotButtons.length).toBeGreaterThan(0);
        expect(pivotButtons[pivotButtons.length - 1].id).toBe(
          'PivotFieldList_New'
        );
        expect(pivotButtons[pivotButtons.length - 1].textContent).toBe(
          'New -1'
        );
        expect(pivotButtons[pivotButtons.length - 1].querySelector('.e-edit'))
          .toBeTruthy;
        (
          pivotButtons[pivotButtons.length - 1].querySelector(
            '.e-edit'
          ) as HTMLElement
        ).click();
        done();
  ...
  describe('Dynamic rendering', () => {
    ...
    it('control class testing for mobile device', () => {
      expect(
        document
          .getElementById('PivotFieldList')
          .classList.contains('e-pivotfieldlist')
      ).toEqual(true);
      expect(
        document.getElementById('PivotFieldList').classList.contains('e-device')
      ).toEqual(true);
    });
    it('get component name testing', () => {
      expect(fieldListObj.getModuleName()).toEqual('pivotfieldlist');
    ...
    it('check field list icon', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          fieldListObj.dialogRenderer.fieldListDialog.element.classList.contains(
            'e-popup-open'
          )
        ).toBe(true);
        done();
    ...
    it('check on axis view change', (done: Function) => {
      let dialogElement: HTMLElement =
        fieldListObj.dialogRenderer.fieldListDialog.element;
      let element: HTMLElement = dialogElement.querySelector(
        '.e-adaptive-container'
      );
      expect(
        [].slice.call(element.querySelectorAll('.e-toolbar-item')).length
    ...
    it('set rtl property', (done: Function) => {
      ...
      setTimeout(() => {
        expect(
          document.getElementById('PivotFieldList').classList.contains('e-rtl')
        ).toBeTruthy;
        done();
      }, 1000);
    });
  ...
  describe('Context menu rendering', () => {
    ...
    beforeAll((done: Function) => {
      ...
      pivotGridObj = new PivotView({
        dataSourceSettings: {
          ...
          values: [
            { name: 'balance', type: 'isMeasureFieldsAvail' as any },
            { name: 'quantity', type: 'isMeasureAvail' as any },
          ],
          filters: [],
        },
        gridSettings: {
          contextMenuItems: [
            ...
            {
              separator: true,
              target: 'td.e-valuescontent,th.e-columnsheader,td.e-rowsheader',
            },
            {
              text: 'Tooltip',
              id: 'tooltip',
    ...
    it('context menu', (done: Function) => {
      setTimeout(() => {
        document
          .querySelectorAll('.e-pivot-button.balance')[0]
          .dispatchEvent(mousedown);
        expect(
          document.querySelectorAll('.e-pivot-button').length > 10
        ).toBeTruthy();
        done();
      }, 1000);
    });
    it('context menu select', (done: Function) => {
      setTimeout(() => {
        ...
        expect(
          document.querySelectorAll('#PivotGrid_PivotContextMenu').length > 0
        ).toBeTruthy();
        done();
      }, 1000);
    });
