...
import { getIndex, cloneObject } from '../src/diagram/utility/base-util';
...
export class DeepDiffMapper {
  ...
  public getOldObject(
    ...
    let oldObject: Node[] | Connector[] = isNode
      ? diagram.oldNodeObjects
      : diagram.oldConnectorObjects;
  ...
  public removeNullObjectValues(segment: Object) {
    ...
    for (let prop of Object.keys(segment)) {
      if (segment[prop] !== undefined) {
        newSegObject[prop] =
          segment[prop] instanceof Object
            ? this.removeNullObjectValues(segment[prop])
            : segment[prop];
      }
    }
    return newSegObject;
  ...
  /** @private */
  public getDifferenceValues(
    selectedObject: Node | Connector,
    args: MouseEventArgs,
    labelDrag?: boolean,
    diagram?: Diagram
  ): void {
    ...
    if (selectedObject) {
      oldObject = isNode
        ? this.getOldObject(selectedObject.id, true, diagram)
        : this.getOldObject(selectedObject.id, false, diagram);
      if (oldObject) {
        ...
        if ((diff as any).children) {
          (diff as any).children = (
            cloneObject(selectedObject) as any
          ).children;
        }
        if ((diff as any).ports && (diff as any).ports.length) {
          for (var i = 0; i < (diff as any).ports.length; i++) {
            if ((newObject as Node).ports[i].outEdges) {
              (diff as any).ports[i].outEdges = (newObject as Node).ports[
                i
              ].outEdges;
            }
            if ((newObject as Node).ports[i].inEdges) {
              (diff as any).ports[i].inEdges = (newObject as Node).ports[
                i
              ].inEdges;
            }
          }
        }
        return this.getDiagramObjects(
          diff,
          selectedObject.id,
          isNode,
          args,
  ...
  public getLayerObject(
    ...
    if (isBlazor()) {
      ...
      for (let i: number = 0; i < newDiagram['layers'].length; i++) {
        if (!temp) {
          result = this.map(
            cloneObject(newDiagram['layers'][i]),
            oldDiagram['layers'][i]
          );
        } else {
          result = this.map(
  ...
  public getDiagramObjects(
    ...
    if (
      args &&
      ((args.sourceWrapper instanceof TextElement && labelDrag) || args.portId)
    ) {
      let tempObject: object;
      let objectValue: any;
      if (isNode) {
        objectValue = args.portId
          ? (this.newNodeObject[0] as Node).ports
          : (this.newNodeObject[0] as Node).annotations;
      } else {
        objectValue =
          (this.newConnectorObject[0] as Connector).annotations || [];
      }
      for (let i: number = 0; i < objectValue.length; i++) {
        if (Object.keys(objectValue[i]).length > 0) {
          ...
          if (args.portId) {
            ...
            for (
              let j: number = 0;
              j < selectedObject.annotations.length;
              j++
            ) {
              if (
                args.sourceWrapper.id ===
                selectedObject.id + '_' + selectedObject.annotations[j].id
              ) {
  ...
  public removeEmptyValues(frame: object): object {
    ...
    for (let prop of Object.keys(frame)) {
      if (
        prop !== 'wrapper' &&
        (prop !== 'data' ||
          (prop === 'data' && !(frame[prop] instanceof Array)))
      ) {
    ...
    if (this.isArray(obj1)) {
      ...
      for (let key in obj1) {
        ...
        let kk = this.map(
          obj1[key],
          value2,
          this.isArray(value2) ? key : undefined
        );
    ...
    if (this.isArray(obj2)) {
      for (var i = obj2.length - 1; i >= 0; i--) {
        ...
        if (this.isFunction(obj2[i]) || diff[arrayName][i] !== undefined) {
          if (
            diff[arrayName][i].type &&
            diff[arrayName][i].type !== this.valueUpdated
          ) {
      ...
      for (let key in obj2) {
        if (this.isFunction(obj2[key]) || diff[key] !== undefined) {
          if (
            diff[key].type &&
            diff[key].type !== this.valueUpdated &&
            diff[key].type !== this.valueDeleted
          ) {
            delete diff[key];
          }
          continue;
    ...
    for (let key in obj) {
      if (this.isArray(obj[key])) {
        ...
        if (
          key != 'type' ||
          (key == 'type' &&
            obj[key] !== this.valueUpdated &&
            obj[key] !== this.valueUnchanged &&
            obj[key] !== this.valueDeleted &&
            obj[key] !== this.valueCreated)
        ) {
