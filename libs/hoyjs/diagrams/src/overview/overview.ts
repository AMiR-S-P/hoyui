  ...
  implements INotifyPropertyChanged
{
  ...
  protected preRender(): void {
    if (this.element.id === '') {
      const collection: number =
        document.getElementsByClassName('e-overview').length;
  ...
  private renderCanvas(options?: OverviewModel): void {
    let canvas: HTMLElement = document.getElementById(
    ...
    const attribute: Object = {
      ...
      style:
        ...
        this.getSizeValue(this.model.height) +
        '; width:' +
  ...
  private getDiagram(element: HTMLElement, instance: string): Diagram {
    ...
    for (let i: number = 0; i < n; i++) {
      if (
        hasClass(
          element[`${instance}`][parseInt(i.toString(), 10)].element,
          'e-diagram'
        )
      ) {
  ...
  public renderDocument(view: Overview): void {
    ...
    const screenX: number =
      window.screenX < 0 ? window.screenX * -1 : window.screenX;
    const screenY: number =
      window.screenY < 0 ? window.screenY * -1 : window.screenY;
    if (eWidth === 0) {
      const widthValue: number = Math.floor(
        window.innerWidth - screenX - Math.floor(bRect.left)
      );
      eWidth = widthValue > 0 ? widthValue : Math.floor(window.innerWidth);
    }
    if (eHeight === 0) {
      const heightValue: number = Math.floor(
        window.innerHeight - screenY - Math.floor(bRect.top)
      );
    ...
    if (!view.diagramLayerDiv) {
      ...
      view.diagramLayer = CanvasRenderer.createCanvas(
        ...
        this.model.width as number,
        this.model.height as number
      );
      view.diagramLayer.setAttribute(
        'style',
  ...
  public removeDocument(view: Overview): void {
    ...
    const htmlLayer: HTMLElement = document.getElementById(
      this.element.id + '_htmlLayer'
    );
    this.canvas.removeChild(htmlLayer);
    const diagramLayer: HTMLElement = document.getElementById(
      this.element.id + '_diagramLayer_div'
    );
  ...
  private renderHtmlLayer(canvas: HTMLElement): HTMLElement {
    const htmlLayer: HTMLDivElement = createHtmlElement('div', {
    ...
    const htmlDiv: HTMLDivElement = createHtmlElement(
      'div',
      options
    ) as HTMLDivElement;
  ...
  private addOverviewRectPanel(view: Overview): void {
    let svg: HTMLElement | SVGElement = document.getElementById(
      this.canvas.id + '_overviewsvg'
    );
    if (svg) {
    ...
    const attr: Object = {
      ...
      version: '1.1',
      style:
        'position:absolute;left:0px;top:0px; aria-label:Specifies overview',
      width: this.model.width,
      height: this.model.height,
    };
    svg = createSvgElement('svg', attr);
    view.canvas.appendChild(svg);
    const ovw: SVGElement = createSvgElement('g', {
    ...
    const g: SVGElement = createSvgElement('g', {
      id: this.canvas.id + 'overviewhandle',
      style: 'pointer-events:all',
    });
    ovw.appendChild(g);
    const innerrect: SVGElement = createSvgElement('rect', {
  ...
  private renderOverviewCorner(name: string, parent: SVGElement): void {
    ...
    const innerrect: SVGElement = createSvgElement(shape, {
      id: this.canvas.id + 'visible' + name,
      class: 'overview-resizer',
    });
  ...
  private updateHelper(
    ...
    const rect: SVGRectElement = (svg as SVGSVGElement).getElementById(
      'helper'
    ) as SVGRectElement;
    if (size) {
      bounds = rect.getBBox();
      ...
      bounds = rect.getBBox();
      x = this.startPoint.x > this.currentPoint.x ? bounds.x - difx : bounds.x;
  ...
  private updateOverviewCorner(
    ...
    let attr: Object;
    let transattr: Object;
    const rectname: string = 'visible' + name;
    const rect: HTMLElement = document.getElementById(
      this.canvas.id + rectname
    );
    if (
      name === 'top' ||
      name === 'bottom' ||
    ...
    setAttributeHtml(rect, attr);
    const transrect: HTMLElement = document.getElementById(
      this.canvas.id + name
    );
  ...
  private translateOverviewRectangle(): void {
    ...
    const svg: SVGSVGElement = <SVGSVGElement>(
      this.element.getElementsByClassName('overviewsvg')[0]
    );
    const panel: SVGRectElement = (svg as SVGSVGElement).getElementById(
      this.canvas.id + 'overviewrect'
    ) as SVGRectElement;
    ...
    this.updateParentView(
      this.parent.scroller.currentZoom,
      x,
      y,
      width,
      height,
  ...
  private renderOverviewRect(
    ...
    const zoom: number = Math.max(
      this.parent.scroller.viewPortWidth / viewwidth,
      this.parent.scroller.viewPortHeight / viewheight
    );
    if (zoom >= 0.25 && zoom <= 30) {
      ...
      const bounds: Rect = this.scrollOverviewRect(
        this.parent.scroller.horizontalOffset,
        this.parent.scroller.verticalOffset,
        this.parent.scroller.currentZoom,
        true
      );
      if (this.helper) {
        const panel: SVGRectElement =
          this.element.getElementsByTagName('rect')[10];
  ...
  private scrollOverviewRect(
    ...
    if (!this.actionName || scaled) {
      ...
      const scale: number = Math.min(
        this.contentWidth / offwidth,
        this.contentHeight / offheight
      );
      ...
      const container: HTMLElement = document.getElementById(
        this.parent.element.id + 'content'
      );
      let wScrollBar: number = 0;
      let hScrollBar: number = 0;
      if (container) {
        wScrollBar = container.offsetWidth - container.clientWidth;
        hScrollBar = container.offsetHeight - container.clientHeight;
      }
      const width: number = (bounds.width =
        (this.parent.scroller.viewPortWidth - rulerSize - wScrollBar) /
        currentZoom /
        scale);
      const height: number = (bounds.height =
        (this.parent.scroller.viewPortHeight - rulerSize - hScrollBar) /
        currentZoom /
        scale);
  ...
  private updateParentView(
    ...
    if (this.actionName === 'scale' || this.actionName === 'draw') {
      this.parent.scroller.zoom(
        zoom / this.parent.scroller.currentZoom,
        delx,
        dely,
        focusPoint
      );
  ...
  public updateHtmlLayer(view: Overview): void {
    ...
    const h: number = Math.max(
      height,
      this.parent.scroller.viewPortHeight / this.parent.scroller.currentZoom
    );
    const scale: number = Math.min(
      Number(this.model.width) / w,
      Number(this.model.height) / h
    );
    htmlLayer.style.transform =
      'scale(' +
      scale +
      ') translate(' +
      this.parent.scroller.transform.tx +
      'px,' +
      this.parent.scroller.transform.ty +
      'px)';
  }
  ...
  public updateView(view: Overview): void {
    ...
    htmlLayer.style.webkitTransform =
      'scale(' + scale + ') translate(' + -bounds.x + 'px,' + -bounds.y + 'px)';
    htmlLayer.style.transform =
      'scale(' + scale + ') translate(' + -bounds.x + 'px,' + -bounds.y + 'px)';
    const ovw: HTMLElement = document.getElementById(
      this.element.id + '_overviewlayer'
    );
    ovw.setAttribute(
      'transform',
      'translate(' + -bounds.x * scale + ',' + -bounds.y * scale + ')'
    );
  ...
  public mouseDown(evt: PointerEvent | TouchEvent): void {
    ...
    this.viewPortRatio =
      this.parent.scroller.viewPortWidth / this.parent.scroller.viewPortHeight;
    const overViewHandle: HTMLElement = document.getElementById(
      this.canvas.id + 'overviewhandle'
    );
    ...
    if (this.actionName) {
      if (
        ...
        if (this.actionName === 'pan') {
          ...
          if (this.helper) {
            const bounds: SVGRect = this.element
              .getElementsByTagName('rect')[10]
              .getBBox();
        ...
        if (
          (evt.target as HTMLElement).id === this.canvas.id + '_overviewsvg' ||
          (evt.target as HTMLElement).id === 'helper'
        ) {
          const svg: SVGSVGElement =
            this.element.getElementsByTagName('svg')[2];
          const g: SVGRectElement = (svg as SVGSVGElement).getElementById(
            this.canvas.id + 'overviewrect'
          ) as SVGRectElement;
          ...
          this.inAction = true;
          this.actionName = 'pan';
          this.renderOverviewRect(
            this.currentPoint.x - width / 2,
            this.currentPoint.y - height / 2,
            width,
            height
          );
          this.inAction = false;
  ...
  private initHelper(): void {
    ...
    if (this.actionName === 'scale') {
      const rect: SVGRectElement = (g as SVGSVGElement).getElementById(
        this.canvas.id + 'overviewrect'
      ) as SVGRectElement;
    ...
    const overviewLayer: SVGElement = (g as SVGSVGElement).getElementById(
      this.element.id + '_overviewlayer'
    ) as SVGElement;
  ...
  private mousePosition(
    evt: PointerEvent | TouchEvent | WheelEvent
  ): PointModel {
