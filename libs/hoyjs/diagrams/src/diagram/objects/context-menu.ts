...
export class DiagramContextMenu {
  ...
  private getMenuItems(): ContextMenuItemModel[] {
    ...
    if (!this.parent.contextMenuSettings.showCustomMenuOnly) {
      ...
      const groupMenus: ContextMenuItemModel =
        this.buildDefaultItems('grouping');
  ...
  private updateItems(): void {
    ...
    for (
      let i: number = 0;
      i < this.parent.contextMenuSettings.items.length;
      i++
    ) {
      const items: ContextMenuItemModel =
        this.parent.contextMenuSettings.items[parseInt(i.toString(), 10)];
      for (let j: number = 0; j < this.contextMenu.items.length; j++) {
        if (
          this.contextMenu.items[parseInt(j.toString(), 10)].text ===
      ...
      if (canInsert) {
        this.contextMenu.insertAfter(
          [items],
          this.contextMenu.items[this.contextMenu.items.length - 1].text
        );
      }
    }
  }
  private async contextMenuBeforeOpen(
    ...
    const diagramArgs: DiagramBeforeMenuOpenEventArgs =
      args as DiagramBeforeMenuOpenEventArgs;
    ...
    for (let i: number = 0; i < contextItems.contextMenu.items.length; i++) {
      const item: MenuItemModel =
        contextItems.contextMenu.items[parseInt(i.toString(), 10)];
      if (contextItems.hiddenItems.indexOf(item.id) === -1) {
        hidden = false;
    ...
    /* tslint:disable */
    if (
      this.parent.selectedItems.nodes.length &&
      (this.parent.selectedItems.nodes[0] as any).isPhase &&
  ...
  private ensureTarget(item: MenuItemModel): boolean {
    ...
    switch (itemText) {
      case 'undo':
        target =
          this.parent.historyManager && this.parent.historyManager.canUndo
            ? true
            : false;
        break;
      case 'redo':
        target =
          this.parent.historyManager && this.parent.historyManager.canRedo
            ? true
            : false;
        break;
      case 'paste':
        target = this.parent.commandHandler.clipboardData.clipObject
          ? true
          : false;
        break;
      case 'selectAll':
        target =
          this.parent.nodes.length + this.parent.connectors.length
            ? true
            : false;
        break;
      case 'grouping':
        target =
          selectedLength > 1 ||
          (this.parent.selectedItems.nodes[0] &&
            this.parent.selectedItems.nodes[0].children &&
            this.parent.selectedItems.nodes[0].children.length > 1)
            ? true
            : false;
        break;
      case 'group':
