...
import {
  ...
  MethodArgumentsModel,
  UmlClassModel,
  BpmnTextAnnotationModel,
} from './node-model';
import {
  BpmnEventModel,
  BpmnSubEventModel,
...
export class BpmnTransactionSubProcess extends ChildProperty<BpmnTransactionSubProcess> {
   ...
   */
  @Complex<BpmnSubEventModel>(
    { id: 'success', event: 'End', offset: { x: 1, y: 0.5 } },
    BpmnSubEvent
  )
...
export class UmlClassAttribute extends MethodArguments {
  ...
  @Complex<ShapeStyleModel>(
    { fill: '#F9F9F9', strokeColor: '#CCCCCC' },
    ShapeStyle
  )
...
export class Node extends NodeBase implements IElement {
  ...
  public init(diagram: any): DiagramElement {
    ...
    switch (this.shape.type) {
      case 'Path':
        if (
          !isBlazor() &&
          (this as unknown as ConnectorModel).type === 'Freehand'
        ) {
          const path: PathElement = new PathElement();
          path.data = getFreeHandPath(
            (this.shape as BasicShape).points
          ) as string;
          content = path;
        } else {
      ...
      case 'Image':
        ...
        imageContent.imageScale = (this.shape as Image).scale;
      ...
      case 'Text':
        ...
        content = textContent;
        textStyle = this.style as TextStyle;
      ...
      case 'Basic':
        if (!isBlazor() && (this.shape as BasicShape).shape === 'Rectangle') {
          ...
          !isBlazor() &&
          (this.shape as BasicShape).shape === 'Polygon'
        ) {
          const path: PathElement = new PathElement();
          path.data = getPolygonPath(
            (this.shape as BasicShape).points
          ) as string;
          content = path;
        } else {
          ...
          const basicshapedata: string = getBasicShape(
            (this.shape as BasicShape).shape
          );
      ...
      case 'Flow':
        ...
        const flowshapedata: string = getFlowShape(
          (this.shape as FlowShape).shape
        );
      ...
      case 'Bpmn':
        if (diagram.bpmnModule) {
          ...
          this.wrapper.elementActions =
            this.wrapper.elementActions | ElementAction.ElementIsGroup;
          const subProcess: BpmnSubProcessModel = (this.shape as BpmnShape)
            .activity.subProcess;
          if (subProcess.processes && subProcess.processes.length) {
            const children: string[] = (this.shape as BpmnShape).activity
              .subProcess.processes;
            for (const i of children) {
              if (
                diagram.nameTable[`${i}`] &&
                (!diagram.nameTable[`${i}`].processId ||
                  diagram.nameTable[`${i}`].processId === this.id)
              ) {
                diagram.nameTable[`${i}`].processId = this.id;
                if (subProcess.collapsed) {
                  diagram.updateElementVisibility(
                    diagram.nameTable[`${i}`].wrapper,
                    diagram.nameTable[`${i}`],
                    !subProcess.collapsed
                  );
                }
                (content as Container).children.push(
                  diagram.nameTable[`${i}`].wrapper
                );
              }
            }
          ...
          console.warn(
            '[WARNING] :: Module "BpmnDiagrams" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
          );
        }
        break;
      case 'Native':
      ...
      case 'HTML':
        ...
        if (
          (this.shape as Html).content &&
          (typeof (this.shape as Html).content === 'string' || !diagram.isReact)
        ) {
          htmlContent.content = (this.shape as Html).content;
        } else if (diagram.nodeTemplate) {
          htmlContent.isTemplate = true;
          htmlContent.template = htmlContent.content = getContent(
            htmlContent,
            true,
            this
          ) as HTMLElement;
        ...
        if (
          (this.shape as Html).content &&
          typeof (this.shape as Html).content === 'function' &&
          diagram.isReact
        ) {
          htmlContent.isTemplate = true;
          (htmlContent as any).templateFn = baseTemplateCompiler(
            (this.shape as Html).content as Function
          );
          htmlContent.template = htmlContent.content = getContent(
            htmlContent,
            true,
      ...
      case 'SwimLane':
        ...
        this.container = {
          type: 'Grid',
          orientation: (this.shape as SwimLaneModel).orientation,
        };
        content.id = this.id;
    ...
    if (
      this.shape.type === 'Basic' &&
      (this.shape as BasicShape).shape === 'Rectangle'
    ) {
    ...
    if (
      !isBlazor() &&
      (this.shape as BasicShape).shape === 'Rectangle' &&
      !(this.shape as BasicShape).cornerRadius
    ) {
    ...
    if (
      (this.shape.type !== 'Bpmn' ||
        (!isBlazor() && (this.shape as BpmnShape).shape === 'Message') ||
        (!isBlazor() && (this.shape as BpmnShape).shape === 'DataSource')) &&
      (this.shape.type !== 'UmlActivity' ||
        (!isBlazor() && (this.shape as UmlActivityShape).shape !== 'FinalNode'))
    ) {
      if (this.shape.type !== 'Text') {
        content.style = this.style;
        this.oldGradientValue = this.style.gradient
          ? cloneObject(this.style.gradient)
          : null;
      }
    }
    if (
  ...
  public initContainer(): Container {
    ...
    if (this.container && this.shape.type !== 'SwimLane') {
      ...
      if (this.container.type === 'Stack') {
        (canvas as ChildContainerModel).orientation =
  ...
  public initIcons(
    ...
    const icon: IconShapeModel = this.isExpanded
      ? this.expandIcon
      : this.collapseIcon;
    if (icon.shape !== 'None') {
      ...
      iconContainer.setOffsetWithRespectToBounds(
        offset.x,
        offset.y,
        'Fraction'
      );
      ...
      iconContainer.description = wrapperContent
        ? wrapperContent
        : iconContainer.id;
  ...
  /** @private */
  public initFixedUserHandles(
    fixedUserHandle: NodeFixedUserHandleModel | NodeFixedUserHandle,
    ...
    if (fixedUserHandle.pathData === '' && fixedUserHandleTemplate) {
      ...
      fixedUserHandleContainer.template = getContent(
        ...
        true,
        fixedUserHandle as NodeFixedUserHandle
      ) as HTMLElement;
    ...
    const offset: PointModel = this.getfixedUserHandleOffet(
      fixedUserHandle as NodeFixedUserHandle
    );
    fixedUserHandleContainer.setOffsetWithRespectToBounds(
      offset.x,
      offset.y,
      'Fraction'
    );
  ...
  private getfixedUserHandleOffet(
    fixedUserHandle: NodeFixedUserHandle
  ): PointModel {
    //let x: number;
    //let y: number;
  ...
  public initAnnotations(
    ...
    for (
      ...
      annotation = this.initAnnotationWrapper(
        this.annotations[parseInt(i.toString(), 10)] as Annotation,
        diagramId,
        virtualize,
        i,
        annotationTemplate
  ...
  public initAnnotationWrapper(
    ...
    if (
      ...
      annotationcontent = new DiagramHtmlElement(
        this.id,
        diagramId,
        annotation.id,
      ...
      if (
        ...
        (annotationcontent as any).templateFn = baseTemplateCompiler(
          annotation.template
        );
        annotationcontent.isTemplate = true;
      }
      annotationcontent = getTemplateContent(
        annotationcontent,
        annotation,
        annotationTemplate,
        diagram
      ...
      (annotationcontent as TextElement).style = {
        ...
        color: link
          ? link.color || (annotationcontent as TextElement).hyperlink.color
          : style.color,
        ...
        textDecoration: link
          ? link.textDecoration ||
            (annotationcontent as TextElement).hyperlink.textDecoration
          : style.textDecoration,
      };
      (annotationcontent as TextElement).hyperlink.link =
        annotation.hyperlink.link || undefined;
      (annotationcontent as TextElement).hyperlink.hyperlinkOpenState =
        annotation.hyperlink.hyperlinkOpenState || undefined;
      (annotationcontent as TextElement).hyperlink.content =
        annotation.hyperlink.content || undefined;
      (annotationcontent as TextElement).hyperlink.textDecoration =
        annotation.hyperlink.textDecoration || undefined;
      (annotationcontent as TextElement).content = link
        ? link.content || (annotationcontent as TextElement).hyperlink.link
        : annotation.content;
    }
    ...
    (annotationcontent as TextElement).annotationVisibility =
      annotationcontent.visible ? 'Visible' : 'Collapsed';
    annotationcontent.rotateAngle = annotation.rotateAngle;
    (annotationcontent as TextElement).rotationReference =
      annotation.rotationReference;
    ...
    if (this.width !== undefined && !annotation.template) {
      if (
        ...
        (annotation.width > this.width &&
          (annotation.style.textWrapping === 'Wrap' ||
    ...
    annotationcontent.setOffsetWithRespectToBounds(
      label.offset.x,
      label.offset.y,
      'Fraction'
    );
  ...
  private initIconContainer(
    options: IconShapeModel,
    iconContainer: Canvas
  ): DiagramElement {
  ...
  private initIconSymbol(
    ...
    if (options.shape === 'Template') {
      ...
      iconContent.height =
        iconContentBounds.height < 10
          ? iconContentBounds.height
          : 10 - (options.padding.bottom + options.padding.top);
      iconContent.width =
        iconContentBounds.width < 10
          ? iconContentBounds.width
          : 10 - (options.padding.left + options.padding.right);
    }
    ...
    iconContent.description =
      iconContainer.description || 'Click here to expand or collapse';
...
export class Lane extends ChildProperty<Shape> {
  ...
  @Complex<HeaderModel>(
    {
      style: { fill: '#E7F4FF', strokeColor: '#CCCCCC' },
      annotation: { content: 'Function' } as Annotation,
    },
    Header
  )
...
export class Phase extends ChildProperty<Shape> {
   ...
   */
  @Complex<ShapeStyleModel>(
    { fill: '#FFFFFF', strokeColor: '#CCCCCC' },
    ShapeStyle
  )
  public style: ShapeStyleModel;
   ...
   */
  @Complex<HeaderModel>(
    { annotation: { content: 'Phase' } as Annotation },
    Header
  )
  public header: HeaderModel;
  ...
  public init(diagram: Diagram): Container {
    ...
    if (this.annotation) {
      const object: Container =
        this.nodes.length > 0
          ? diagram.nameTable[this.nodes[0].id].wrapper
          : diagram.nameTable[this.connectors[0].id].wrapper;
      const wrapper: DiagramElement = diagram.getWrapper(
        object,
        this.annotation.id
      );
      ...
      if (this.nodes || this.connectors) {
        for (let i: number = 0; i < this.nodes.length; i++) {
          const node: NodeModel =
            diagram.nameTable[this.nodes[parseInt(i.toString(), 10)].id];
        ...
        for (let j: number = 0; j < this.connectors.length; j++) {
          const connector: ConnectorModel =
            diagram.nameTable[this.connectors[parseInt(j.toString(), 10)].id];
    ...
    const diagramProtectPropertyChange: boolean =
      diagram[`${isProtectedOnChange}`];
