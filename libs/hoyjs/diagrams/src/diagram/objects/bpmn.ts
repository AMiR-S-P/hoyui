...
export class BpmnDiagrams {
  ...
  public initBPMNContent(
    ...
    if (bpmnShape === 'TextAnnotation') {
      if (
        ...
        content = this.getBpmnTextAnnotationShape(
          node,
          node.shape as BpmnShape,
          diagram
        );
      } else {
        content = this.getBpmnTextAnnotationSymbol(
          node,
          node.shape as BpmnShape,
          diagram
        );
  ...
  private getBpmnTextAnnotationShape(
    textAnnotation: NodeModel,
    bpmnTextAnnotation: BpmnShape,
    diagram: Diagram
  ): Canvas {
    ...
    const oldIsProtectedOnChange: boolean = (diagram as any)
      .isProtectedOnChange;
    ...
    this.setAnnotationPath(
      ...
      textAnnotation,
      bpmnTextAnnotation,
      bpmnTextAnnotation.textAnnotation.textAnnotationDirection,
      diagram
    );
  ...
  private getBpmnTextAnnotationSymbol(
    annotation: NodeModel,
    bpmnTextAnnotation: BpmnShape,
    diagram: Diagram
  ): PathElement {
    let data: string =
      'M33.33,15 L0,43 M39.33,0 L33.33,0 L33.33,30 L39.33,30 M60,0';
    if (bpmnTextAnnotation.textAnnotation.textAnnotationDirection === 'Top') {
      ...
      bpmnTextAnnotation.textAnnotation.textAnnotationDirection === 'Right'
    ) {
      data = 'M6,15 L39.33,43 M0,0 L6,0 L6,30 L0,30 M-20,0';
    } else if (
      bpmnTextAnnotation.textAnnotation.textAnnotationDirection === 'Bottom'
    ) {
  ...
  public getBPMNShapes(node: Node): PathElement {
    ...
    const bpmnShapeData: string = getBpmnShapePathData(
  ...
  public getBPMNGroup(node: Node, diagram: Diagram): Container {
    ...
    if ((node.shape as any).group) {
      for (let i: number = 0; i < groupShapeObj.children.length; i++) {
        const child: Node | Connector =
          diagram.nameTable[groupShapeObj.children[parseInt(i.toString(), 10)]];
  ...
  public getBPMNGatewayShape(node: Node): Canvas {
    ...
    const gatewayTypeNodeShapeData: string = getBpmnGatewayShapePathData(
      shapeType.type
    );
    if (
      shapeType.type === 'EventBased' ||
      shapeType.type === 'ExclusiveEventBased' ||
      shapeType.type === 'ParallelEventBased'
    ) {
    ...
    this.setSizeForBPMNGateway(
      (node.shape as BpmnShape).gateway,
      gatewayshape,
      size.width,
      size.height
    );
  ...
  public getBPMNDataObjectShape(node: Node): Canvas {
    ...
    dataobjNode.data =
      'M29.904,5 L7.853,5 L7.853,45 L42.147,45 L42.147,17.242,L29.932,5,L29.932,17.242,L42.147,17.242';
    ...
    dataobjTypeNode.data =
      'M 3 9.4 l 6 0 v 2.4 l 3.6 -4 L 9 4 v 2.5 H 3 V 9.4 Z';
    //set style - opacity
    ...
    dataobjCollectionNode.data =
      'M 0 0 L 0.1 0 L 0.1 2 L 0 2 Z M 0.4 0 L 0.6 0 L 0.6 2 L0.4 2 Z M 0.9 0 L 1 0 L 1 2 L 0.9 2 Z';
    ...
    dataObjectshape.children = [
      dataobjNode,
      dataobjTypeNode,
      dataobjCollectionNode,
    ];
  ...
  /** @private */
  public getBPMNEventShape(
    node: Node,
    subEvent: BpmnSubEventModel,
    sub?: boolean,
    id?: string
  ): Canvas {
    ...
    const pathdata: string =
      'M164.1884,84.6909000000001C156.2414,84.6909000000001,149.7764,78.2259000000001,149.7764,70.2769000000001' +
  ...
  private setSubProcessVisibility(node: Node): void {
    const subProcess: BpmnSubProcessModel = (node.shape as BpmnShape).activity
      .subProcess;
    const eventLength: number = subProcess.events.length;
    const index: number =
      (node.shape as BpmnShape).activity.subProcess.type === 'Transaction'
        ? 2
        : 0;

    const elementWrapper: Canvas = (node.wrapper.children[0] as Canvas)
      .children[0] as Canvas;
    ...
    if (eventLength > 0) {
      for (let i: number = 0; i < eventLength; i++) {
        this.setEventVisibility(
          node,
          (elementWrapper.children[2 + i] as Container).children
        );
      }
    }
  }
  ...
  public getBPMNSubProcessShape(node: Node): Canvas {
    ...
    let loopType: string;
    let events: Object[];
    let event: BpmnSubEventModel;
    let subprocessAdhoc: PathElement = new PathElement();
    const subProcessEventsShapes: Canvas = new Canvas();
    let subProcessLoopShapes: PathElement = new PathElement();
    const shape: BpmnActivityModel = (node.shape as BpmnShape).activity;
    const subProcess: BpmnSubProcessModel = shape.subProcess;
    const subChildCount: number = this.getSubprocessChildCount(node);
    let x: number;
    ...
    if (shape.subProcess.type === 'Transaction') {
      this.getBPMNSubProcessTransaction(
        node,
        node.shape as BpmnShape,
        subProcessShapes
      );
  ...
  private getBPMNSubEvent(
    ...
    const eventContainer: Canvas = this.getBPMNEventShape(
      node,
      event,
      true,
      id
    ...
    eventContainer.setOffsetWithRespectToBounds(
      event.offset.x,
      event.offset.y,
      'Fraction'
    );
  ...
  private getBPMNSubProcessTransaction(
    ...
    const transaction: BpmnTransactionSubProcessModel =
      shape.activity.subProcess.transaction;

    this.getBPMNSubEvent(
      transaction.success as BpmnSubEvent,
      node,
      transactionEvents,
      node.id + '_success'
    );
    ...
    this.getBPMNSubEvent(
      transaction.cancel as BpmnSubEvent,
      node,
      transactionEvents,
      node.id + '_cancel'
    );
    ...
    this.getBPMNSubEvent(
      transaction.failure as BpmnSubEvent,
      node,
      transactionEvents,
      node.id + '_failure'
    );
    ...
    this.updateDiagramContainerVisibility(
      transactionEvents.children[0],
      transaction.success.visible
    );

    this.updateDiagramContainerVisibility(
      transactionEvents.children[1],
      transaction.cancel.visible
    );

    this.updateDiagramContainerVisibility(
      transactionEvents.children[2],
    ...
    transactionEvents.style.fill = transactionEvents.style.strokeColor =
      'transparent';
  ...
  public getBPMNSubProcessLoopShape(node: Node): PathElement {
    const shape: BpmnActivityModel = (node.shape as BpmnShape).activity;
    let loopType: string;
    const subprocessLoop: PathElement = new PathElement();
    let subprocessLoopData: string;
    const subProcess: BpmnSubProcessModel = shape.subProcess;
  ...
  public drag(obj: Node, tx: number, ty: number, diagram: Diagram): void {
    ...
    if (obj.margin.top + ty >= 0) {
      diagram.nodePropertyChange(
        obj as Node,
        {} as Node,
        { margin: { top: obj.margin.top + ty } } as Node
      );
    }
    if (obj.margin.left + tx >= 0) {
      diagram.nodePropertyChange(
        obj as Node,
        {} as Node,
        { margin: { left: obj.margin.left + tx } } as Node
      );
    }
  ...
  public dropBPMNchild(target: Node, source: Node, diagram: Diagram): void {
    if (
      source &&
      source.shape.type === 'Bpmn' &&
      target.shape.type === 'Bpmn' &&
      !isBlazor() &&
      (source.shape as BpmnShape).shape !== 'TextAnnotation'
    ) {
      const subProcess: BpmnSubProcessModel = (
        diagram.nameTable[target.id].shape as BpmnShape
      ).activity.subProcess;
      if (
      ...
      if (
        ...
        subProcess.processes.indexOf(source.id) === -1 &&
        !subProcess.collapsed
      ) {
        ...
        sources.margin.top =
          sources.offsetY -
          sources.actualSize.height / 2 -
          (target.offsetY - target.actualSize.height / 2);
        sources.margin.left =
          sources.offsetX -
          sources.actualSize.width / 2 -
  ...
  private sortProcessOrder(processes: string[], diagram: Diagram): void {
    ...
    processes.sort(
      (a: string, b: string) =>
        diagram.nameTable[`${a}`].zIndex - diagram.nameTable[`${b}`].zIndex
    );
  }
  ...
  private updateSubprocessNodeIndex(
    ...
    if (
      (source.shape as BpmnShapeModel).activity.subProcess.processes &&
      (source.shape as BpmnShapeModel).activity.subProcess.processes.length > 0
    ) {
      for (
        let i: number = 0;
        i <
        (source.shape as BpmnShapeModel).activity.subProcess.processes.length;
        i++
      ) {
        this.updateIndex(diagram, source);
        const processes: string = (source.shape as BpmnShapeModel).activity
          .subProcess.processes[parseInt(i.toString(), 10)];
        if (
          diagram.nameTable[`${processes}`].shape.activity.subProcess.processes
            .length > 0
        ) {
          this.updateSubprocessNodeIndex(
            diagram.nameTable[`${processes}`],
            diagram,
            target
          );
        } else {
  ...
  public updateDocks(obj: Node, diagram: Diagram): void {
    if (
      obj.shape.type === 'Bpmn' &&
      (obj.shape as BpmnShape).activity.subProcess.processes &&
      !(obj.shape as BpmnShape).activity.subProcess.collapsed
    ) {
      const processTable: string[] = (obj.shape as BpmnShape).activity
        .subProcess.processes;
      for (const i of processTable) {
        ...
        if (actualObject) {
          ...
          if (
            (actualObject.shape as BpmnShape).activity.subProcess.processes &&
            (actualObject.shape as BpmnShape).activity.subProcess.processes
              .length
          ) {
  ...
  public removeBpmnProcesses(currentObj: Node, diagram: Diagram): void {
    ...
    if (
      ...
      (currentObj.shape as BpmnShape).activity.subProcess.processes &&
      (currentObj.shape as BpmnShape).activity.subProcess.processes.length > 0
    ) {
      const processes: string[] = (currentObj.shape as BpmnShape).activity
        .subProcess.processes;
      for (let j: number = processes.length - 1; j >= 0; j--) {
        diagram.remove(
          diagram.nameTable[processes[parseInt(j.toString(), 10)]]
        );
      }
    }
    if (element) {
      diagram.removeDependentConnector(currentObj);
      const processes: string[] = (element.shape as BpmnShape).activity
        .subProcess.processes;
  ...
  private removeGElement(id: string, diagram: Diagram): void {
    ...
    const diagramLayer: HTMLElement = document.getElementById(
      diagram.element.id + '_diagramLayer'
    );
    ...
    const subProcess: NodeModel =
      diagram.nameTable[`${(node as Node).processId}`];
    const beforeIndex: number = node.zIndex + 1;
    const getNode: NodeModel[] = this.getNode(beforeIndex, diagram);
    if (getNode.length > 0) {
      const neighborElement: HTMLElement = document.getElementById(
        getNode[0].id + '_groupElement'
      );
      const index: number = Array.from(diagramLayer.children).findIndex(
        (child: Element) => child === neighborElement
      );
      ...
      if (node.zIndex < subProcess.zIndex) {
        diagramLayer.children[
          parseInt(domIndex.toString(), 10)
        ].insertAdjacentElement('beforebegin', element);
      }
    }
  }
  private getNode(index: number, diagram: Diagram): NodeModel[] {
    const getNode: NodeModel[] = diagram.nodes.filter(
      (x: any) => x.zIndex === index
    );
  ...
  public removeProcess(id: string, diagram: Diagram): void {
    ...
    if (node) {
      ...
      if (parent && parent.shape.type === 'Bpmn') {
        const processes: string[] = (parent.shape as BpmnShape).activity
          .subProcess.processes;
  ...
  public addProcess(
    ...
    const subProcess: BpmnSubProcessModel = (parentNode.shape as BpmnShape)
      .activity.subProcess;
    if (
      node &&
      parentNode &&
      parentNode.shape.type === 'Bpmn' &&
      node.shape.type === 'Bpmn' &&
      subProcess.processes
    ) {
      node.processId = parentId;
      const processes: string[] = (parentNode.shape as BpmnShape).activity
        .subProcess.processes;
  ...
  public updateSubProcessess(
    ...
    if (
      node.wrapper.bounds.left + obj.margin.left + obj.width >
      node.wrapper.bounds.right
    ) {
      right = true;
    }
    if (
      node.wrapper.bounds.top + obj.margin.top + obj.height >
      node.wrapper.bounds.bottom
    ) {
      bottom = true;
    }
    if (right) {
    ...
    if (right) {
      diffX =
        (obj.wrapper.margin.left + obj.wrapper.bounds.width) / actualSize.width;
    }
    if (bottom) {
      diffY =
        (obj.wrapper.margin.top + obj.wrapper.bounds.height) /
        actualSize.height;
    }
    if (diffX > 0 || diffY > 0) {
      diagram.commandHandler.scale(
  ...
  /** @private */
  public getBPMNCompensationShape(
    node: Node,
    compensationNode: PathElement
  ): PathElement {
    ...
    compensationNode.data =
      'M 22.462 18.754 l -6.79 3.92 l 6.79 3.92 V 22.89 l 6.415 3.705 v -7.841 l -6.415 3.705 V 18.754 Z' +
  ...
  /** @private */
  public getBPMNSubprocessEvent(
    node: Node,
    subProcessEventsShapes: Canvas,
    events: BpmnSubEventModel
  ): void {
    let annotations: DiagramElement;
    let ports: DiagramElement;
  ...
  /** @private */
  public getBPMNAdhocShape(
    node: Node,
    adhocNode: PathElement,
    ...
    adhocNode.data =
      'M 49.832 76.811 v -2.906 c 0 0 0.466 -1.469 1.931 -1.5 c 1.465 -0.031 2.331 1.219 2.897 1.688 ' +
   ...
   */
  public setAnnotationPath(
    sourceBounds: Rect,
    wrapper: Canvas,
    node: NodeModel,
    bpmnShape: BpmnShape,
    direction: TextAnnotationDirection,
    diagram: Diagram
  ): void {
    const pathElement: PathElement = new PathElement();
    ...
    const pointPort: PointPortModel =
      node.ports.length > 0
        ? node.ports[0]
        : new PointPort(node, 'ports', '', true);
    ...
    switch (direction) {
      ...
      default:
        ...
        if (bpmnShape.textAnnotation.textAnnotationTarget !== '') {
          if (
            diagram.nameTable[bpmnShape.textAnnotation.textAnnotationTarget]
          ) {
            const node2: Node =
              diagram.nameTable[bpmnShape.textAnnotation.textAnnotationTarget];
            ...
            if (wrapper2 !== null) {
              ...
              if (doubleValue !== 0.0 && doubleValue2 !== 0.0) {
                targetBounds = new Rect(
                  node2.offsetX - doubleValue / 2,
                  node2.offsetY - doubleValue2 / 2,
                  doubleValue,
                  doubleValue2
                );
              }
  ...
  private setAnnotationPosition(
    ...
    const rotateAngle: number = this.getAnnotationPathAngle(
      position,
      targetBounds
    );
    if (rotateAngle === 90) {
  ...
  private getSubprocessChildCount(node: Node): number {
    ...
    if (
      (subProcess as BpmnSubProcess).compensation !== undefined &&
      (subProcess as BpmnSubProcess).compensation === true
    ) {
      count++;
    }
    if (
      (subProcess as BpmnSubProcess).collapsed !== undefined &&
      (subProcess as BpmnSubProcess).collapsed === true
    ) {
      count++;
    }
    if (
      (subProcess as BpmnSubProcess).adhoc !== undefined &&
  ...
  private getTaskChildCount(node: Node): number {
    let count: number = 0;
    const shape: BpmnActivityModel = (node.shape as BpmnShape).activity;
    const task: BpmnTaskModel = (shape as BpmnActivity).task;
    let loopType: string;
    if (
      (task as BpmnTask).compensation !== undefined &&
      (task as BpmnTask).compensation === true
    ) {
      count++;
    }
    if (
      (task as BpmnTask).loop !== undefined &&
  ...
  public updateBPMN(
    ...
    const newShape: BpmnShapeModel =
      (changedProp.shape as BpmnShapeModel) || {};
    const elementWrapper: DiagramElement = actualObject.wrapper.children[0];
    const actualShape: BpmnShapes =
      (actualObject.shape as BpmnShapeModel).shape ||
    ...
    if (newShape.textAnnotation && actualShape === 'TextAnnotation') {
      if (newShape.textAnnotation.textAnnotationDirection !== undefined) {
        ...
        this.setAnnotationPath(
          ...
          (actualObject.shape as BpmnShape).textAnnotation
            .textAnnotationDirection,
          diagram
        );
        //Reset the id of path element
        (actualObject.wrapper.children[0] as Canvas).children[0].id =
          actualObject.id + '_textannotation_path';
        const newValue: Node = {
          ports: {
            0: { offset: actualObject.ports[0].offset },
          } as PointPortModel,
        } as Node;
      ...
      if (newShape.textAnnotation.textAnnotationTarget !== undefined) {
        const sourceNode: Node =
          diagram.nameTable[newShape.textAnnotation.textAnnotationTarget];
        if (sourceNode && sourceNode.shape.type === 'Bpmn') {
          const textAnnotationConnector: any =
            diagram.nameTable[actualObject.inEdges[0]];
          if (textAnnotationConnector) {
            ...
            diagram.connectorPropertyChange(
              textAnnotationConnector,
              {} as Connector,
              { sourceID: sourceNode.id } as Connector
            );
          }
          ...
          if (textAnnotationConnector) {
            const oldParent: any =
              diagram.nameTable[
                (oldObject.shape as BpmnShape).textAnnotation
                  .textAnnotationTarget
              ];
            const index: Number = oldParent.outEdges.indexOf(
              textAnnotationConnector.id
            );
    ...
    if (newShape.shape === 'Gateway' && newShape.gateway) {
      ...
      this.removeBPMNElementFromDOM(actualObject, diagram);
      actualObject.wrapper.children[0] =
        this.getBPMNDataObjectShape(actualObject);
    } else if (newShape.shape === 'Activity' && newShape.activity) {
      this.removeBPMNElementFromDOM(actualObject, diagram);
      actualObject.wrapper.children[0] =
        this.getBPMNActivityShape(actualObject);
    } else if (
    /**
     * EJ2-EJ2-60644 - Bpmn event fill color does not applied while changing event in runtime.
     */
      ((actualObject.shape as DiagramShapeModel).bpmnShape === 'Event' ||
        (actualObject.shape as BpmnShape).shape === 'Event' ||
        newShape.shape === 'Event') &&
      newShape.event
    ) {
      this.removeBPMNElementFromDOM(actualObject, diagram);
      const shapeEvent: Object = newShape.event;
      actualObject.wrapper.children[0] = this.getBPMNEventShape(
        actualObject,
        shapeEvent
      );
    } else if (
      newShape.shape === 'Message' ||
    ...
    if (changedProp.style) {
      ...
      if (changedProp.style && changedProp.style.strokeColor) {
        ...
        if (
          (elementWrapper as Container) &&
          (elementWrapper as Container).children !== undefined &&
          ...
          if (
            ...
            const child: DiagramElement = (elementWrapper as Container)
              .children[0];
            this.updateBPMNStyle(child, changedProp.style.strokeColor);
          } else if (
            (!isBlazor() &&
              (actualObject.shape as BpmnShape).shape === 'Gateway') ||
      ...
      if (changedProp.style && changedProp.style.opacity !== undefined) {
        this.updateBpmnChildOpacity(
          actualObject.wrapper,
          changedProp.style.opacity
        );
      }
    }
  }
    ...
    if (wrapper.children && wrapper.children.length > 0) {
      for (let i: number = 0; i < wrapper.children.length; i++) {
        const child: DiagramElement =
          wrapper.children[parseInt(i.toString(), 10)];
  ...
  /** @private */
  public updateBPMNStyle(
    elementWrapper: DiagramElement,
    changedProp: string
  ): void {
    for (
      let i: number = 0;
      i < (elementWrapper as Container).children.length;
      i++
    ) {
      const child: DiagramElement = (elementWrapper as Container).children[
        parseInt(i.toString(), 10)
      ];
  ...
  public updateBPMNGateway(node: Node, changedProp: Node): void {
    ...
    if (bpmnShape) {
      ...
      const pathData: string = getBpmnGatewayShapePathData(
        bpmnShape.gateway.type
      );
      const dataobjTypeNode: PathElement = this.updateGatewaySubType(
        elementWrapper,
        node,
        pathData
      );
    ...
    if (changedProp.width !== undefined || changedProp.height !== undefined) {
      this.setSizeForBPMNGateway(
        (node.shape as BpmnShapeModel).gateway,
        elementWrapper,
        changedProp.width || node.width,
        changedProp.height || node.height
      );
    }
  }
  /**
   ...
   * @returns
   */
  updateGatewaySubType(
    elementWrapper: Canvas,
    node: Node,
    pathData: string
  ): PathElement {
    const dataobjTypeNode: PathElement = new PathElement();
    ...
    dataobjTypeNode.horizontalAlignment =
      elementWrapper.children[1].horizontalAlignment;
    dataobjTypeNode.verticalAlignment =
      elementWrapper.children[1].verticalAlignment;
  ...
  public updateBPMNDataObject(
    ...
    if (bpmnShape) {
      ...
      if (bpmnShape.dataObject) {
        ...
        if (
          (newObject.shape as BpmnShape).dataObject.collection !== undefined
        ) {
    ...
    if (newObject.width !== undefined || newObject.height !== undefined) {
      this.setSizeForBPMNDataObjects(
        (node.shape as BpmnShapeModel).dataObject,
        elementWrapper,
        newObject.width || node.width,
        newObject.height || node.height
      );
    }
  }
  /** @private */
  public getEvent(
    ...
    oldObject: Node,
    event: string,
    child0: DiagramElement,
    child1: DiagramElement,
    child2: DiagramElement
  ): void {
    switch (event) {
      case 'Start':
  ...
  private updateEventVisibility(oldObject: Node, child1: DiagramElement): void {
    if (
      ...
      (oldObject.shape as BpmnShape).activity.subProcess.events[0] !==
        undefined &&
      (oldObject.shape as BpmnShape).activity.subProcess.events[0].event !==
        undefined
    ) {
      if (
        (oldObject.shape as BpmnShape).activity.subProcess.events[0].event ===
      ...
      if (
        (oldObject.shape as BpmnShape).event.event === 'NonInterruptingStart' ||
  ...
  public updateBPMNEvent(node: Node, newObject: Node, oldObject: Node): void {
    ...
    // }
    if (
      newObject.width !== undefined ||
      newObject.height !== undefined ||
      trigger !== undefined
    ) {
      this.setSizeForBPMNEvents(
        (node.shape as BpmnShapeModel).event,
        elementWrapper,
        newObject.width || node.width,
        newObject.height || node.height
      );
  ...
  public updateBPMNActivity(
    ...
    if (
      ...
      (elementWrapper.children[0] as Container)
    ) {
      const size: Size = this.getSize(
        node,
        (elementWrapper.children[0] as Container).children[0] as PathElement
      );
      if (bpmnShape) {
        const oldProp: BpmnActivities = (oldObject.shape as BpmnShape).activity
          .activity;
        const actualObjectProp: BpmnActivities = (node.shape as BpmnShape)
          .activity.activity;
        if (
          (oldProp === 'SubProcess' || oldProp === 'Task') &&
          (actualObjectProp === 'SubProcess' || actualObjectProp === 'Task')
        ) {
          diagram.removeElements(node);
          node.wrapper.children[0] = this.getBPMNActivityShape(node);
        } else {
          if (
            actualObjectProp === 'Task' &&
            bpmnShape.activity.task !== undefined
          ) {
            this.updateBPMNActivityTask(node, newObject);
            const subChildCount: number = this.getTaskChildCount(node);
            let x: number;
            ...
            if (bpmnShape.activity.task.loop !== undefined) {
              this.updateBPMNActivityTaskLoop(
                node,
                newObject,
                x,
                subChildCount,
          ...
          if (
            actualObjectProp === 'SubProcess' &&
            bpmnShape.activity.subProcess !== undefined
          ) {
            this.updateBPMNActivitySubProcess(
              node,
              newObject,
              oldObject,
              diagram
        ...
        this.setSizeForBPMNActivity(
          (node.shape as BpmnShapeModel).activity,
          elementWrapper,
          newObject.width || size.width,
          newObject.height || size.height,
          node
        );
      }
      if (newObject.width !== undefined || newObject.height !== undefined) {
  ...
  public updateBPMNActivityTask(node: Node, newObject: Node): void {
    ...
    for (let i: number = 0; i < elementWrapper.children.length; i++) {
      ...
      if (
        elementWrapper.children[parseInt(i.toString(), 10)].id ===
        node.id + '_2_loop'
      ) {
    ...
    if (task.type !== undefined) {
      ...
      if (task.type === 'Receive' || task.type === 'Send') {
        ...
        taskTypeNodeService.margin.left =
          elementWrapper.children[1].margin.left + 9;
        taskTypeNodeService.margin.top =
          elementWrapper.children[1].margin.top + 9;
        taskTypeNodeService.style.fill = 'white';
        taskTypeNodeService.style.opacity = node.style.opacity;
  ...
  public updateBPMNActivityTaskLoop(
    ...
    if (activity.subProcess !== undefined) {
      ...
      (elementWrapper.children[2 + index] as PathModel).data =
        bpmnshapeLoopdata;
      elementWrapper.children[2 + index].visible =
        loop === 'None' ? false : true;
      //EJ2-907764-Changing Loop for Activity node with task type, result in change in task type symbols
    } else if (
      activity.task !== undefined &&
      activity.task.loop !== undefined &&
      taskType !== 'Service'
    ) {
      ...
      elementWrapper.children[2].visible =
        activity.task.loop === 'None' ? false : true;
    } else if (
      activity.task !== undefined &&
      activity.task.loop !== undefined &&
      taskType === 'Service'
      ...
      elementWrapper.children[3].visible =
        activity.task.loop === 'None' ? false : true;
    }
  ...
  private updateChildMargin(
    ...
    if (subChildCount === 1) {
      for (let i: number = start; i < elementWrapper.children.length; i++) {
        if (
          i !== 2 &&
          elementWrapper.children[parseInt(i.toString(), 10)].visible === true
        ) {
  ...
  /** @private */
  public updateBPMNActivitySubProcess(
    node: Node,
    newObject: Node,
    oldObject: Node,
    diagram: Diagram
    ...
    updateStyle(
      node.style,
      ((elementWrapper as Canvas).children[0] as Canvas).children[0]
    );
    ...
    if (subProcess.adhoc !== undefined) {
      this.updateBPMNSubProcessAdhoc(
        node,
        oldObject,
        subProcess,
        x,
    ...
    if (subProcess.collapsed !== undefined) {
      this.updateBPMNSubProcessCollapsed(
        node,
        oldObject,
        subProcess,
        x,
    ...
    if (subProcess.compensation !== undefined) {
      this.updateBPMNSubProcessCompensation(
        node,
        oldObject,
        subProcess,
        x,
  ...
  /** @private */
  public updateBPMNSubProcessEvent(
    node: Node,
    newObject: Node,
    oldObject: Node,
    diagram: Diagram
    ...
    const nodeContent: Canvas = (elementWrapper as Canvas)
      .children[0] as Canvas;
    let index: number;
    const j: number = 0;
    const elementWrapperChildLen: number = (
      (elementWrapper as Canvas).children[0] as Canvas
    ).children.length;
    const subProcess: BpmnSubProcessModel = bpmnShape.activity.subProcess;
    ...
    for (const key of Object.keys(subProcess.events)) {
      ...
      const eventWrapper: Canvas = nodeContent.children[
        eventIndex + start
      ] as Canvas;
      const actualEvent: BpmnSubEventModel = (node.shape as BpmnShape).activity
        .subProcess.events[parseInt(eventIndex.toString(), 10)];
  ...
  private updateBPMNSubEvent(
    node: NodeModel,
    newEvent: BpmnSubEventModel,
    actualEvent: BpmnSubEventModel,
    eventWrapper: Canvas,
    newObject: NodeModel,
    oldObject: NodeModel,
    diagram: Diagram
    ...
    if (newObject.style) {
      if (
        newObject.style.strokeColor !== undefined ||
        newObject.style.opacity !== undefined
      ) {
        (
          (elementWrapper as Canvas).children[0] as Canvas
        ).children[1].style.strokeColor = newObject.style.strokeColor;
        (
          (elementWrapper as Canvas).children[0] as Canvas
        ).children[1].style.opacity = newObject.style.opacity;
      }
    }
    ...
    if (newEvent.offset !== undefined) {
      eventWrapper.setOffsetWithRespectToBounds(
        actualEvent.offset.x,
        actualEvent.offset.y,
        'Fraction'
      );
    ...
    if (newEvent.ports !== undefined) {
      ...
      const port: Container = ((elementWrapper as Canvas).children[0] as Canvas)
        .children[2] as Container;
  ...
  private updateBPMNSubProcessTransaction(
    ...
    const transaction: BpmnTransactionSubProcessModel = (
      newObject.shape as BpmnShape
    ).activity.subProcess.transaction;
    const eventContainer: Canvas = (
      ((node.wrapper as Canvas).children[0] as Canvas).children[0] as Canvas
    ).children[2] as Canvas;

    let actualEvent: BpmnSubEventModel;
    if (transaction.success !== undefined) {
      actualEvent = (node.shape as BpmnShape).activity.subProcess.transaction
        .success;
      this.updateBPMNSubEvent(
        node,
        transaction.success,
        actualEvent,
        eventContainer.children[0] as Canvas,
        newObject,
        oldObject,
        diagram
      );
    ...
    if (transaction.cancel !== undefined) {
      ...
      this.updateBPMNSubEvent(
        ...
        transaction.cancel,
        actualEvent,
        eventContainer.children[1] as Canvas,
        newObject,
        oldObject,
        diagram
      );
    ...
    if (transaction.failure !== undefined) {
      ...
      this.updateBPMNSubEvent(
        ...
        transaction.failure,
        actualEvent,
        eventContainer.children[2] as Canvas,
        newObject,
        oldObject,
        diagram
      );
    ...
    for (const edge of (node as Node).outEdges) {
      ...
      switch (connector.sourcePortID) {
        case 'success':
          if (
            transaction.success &&
            transaction.success.visible !== undefined
          ) {
            diagram.connectorPropertyChange(
              connector,
              {} as Connector,
              { sourcePortID: 'success' } as Connector
            );
          }
          break;
        case 'cancel':
          if (transaction.cancel && transaction.cancel.visible !== undefined) {
            diagram.connectorPropertyChange(
              connector,
              {} as Connector,
              { sourcePortID: 'cancel' } as Connector
            );
          }
          break;
        case 'failure':
          if (
            transaction.failure &&
            transaction.failure.visible !== undefined
          ) {
            diagram.connectorPropertyChange(
              connector,
              {} as Connector,
              { sourcePortID: 'failure' } as Connector
            );
          }
          break;
      }
  ...
  /** @private */
  public updateBPMNSubProcessBoundary(
    node: Node,
    subProcess: BpmnSubProcessModel
  ): void {
    const shape: BpmnShapeModel = node.shape as BpmnShapeModel;
    const elementWrapper: DiagramElement = (
  ...
  public updateElementVisibility(
    ...
    if (
      ...
      for (let j: number = processes.length - 1; j >= 0; j--) {
        ...
        if (visible) {
          if (
            !isBlazor() &&
            (currentNode.shape as BpmnShape).shape === 'Event'
          ) {
            this.setEventVisibility(
              currentNode,
              (currentNode.wrapper.children[0] as Container).children
            );
          }
          if (
            ((currentNode.shape as BpmnShape).activity as BpmnShapeModel)
              .activity === 'SubProcess'
          ) {
            this.setSubProcessVisibility(currentNode);
          }
        }
        const connectors: string[] = currentNode.inEdges.concat(
          currentNode.outEdges
        );
        for (let i: number = connectors.length - 1; i >= 0; i--) {
          const connector: Connector =
            diagram.nameTable[connectors[parseInt(i.toString(), 10)]];
    ...
    if (visible) {
      if (!isBlazor() && (node.shape as BpmnShape).shape === 'Event') {
        this.setEventVisibility(
          node,
          (node.wrapper.children[0] as Container).children
        );
      }
      if (
        ((node.shape as BpmnShape).activity as BpmnShapeModel).activity ===
        'SubProcess'
      ) {
      ...
      if (
        ((node.shape as BpmnShape).activity as BpmnShapeModel).activity ===
          'Task' &&
        !isBlazor() &&
        (node.shape as BpmnShape).shape === 'Activity' &&
        (node.shape as BpmnShape).activity.subProcess.loop === 'None'
      ) {
        (
          (node.wrapper.children[0] as Container).children[0] as Container
      ...
      if ((node.shape as BpmnShape).shape === 'DataObject') {
        if (
          (node.shape as BpmnShape).dataObject.collection === false &&
          (node.shape as BpmnShape).dataObject.type === 'None'
        ) {
          for (
            let i: number =
              (node.wrapper.children[0] as Container).children.length - 1;
            i > 0;
            i--
          ) {
            (node.wrapper.children[0] as Container).children[
          ...
          (node.shape as BpmnShape).dataObject.collection === true &&
          (node.shape as BpmnShape).dataObject.type === 'None'
        ) {
          (node.wrapper.children[0] as Container).children[1].visible = false;
        } else if (
          (node.shape as BpmnShape).dataObject.collection === false &&
          ((node.shape as BpmnShape).dataObject.type === 'Input' ||
            (node.shape as BpmnShape).dataObject.type === 'Output')
        ) {
  ...
  public updateBPMNSubProcessCollapsed(
    ...
    const eventLength: number = (node.shape as BpmnShape).activity.subProcess
      .events.length;
    const elementWrapper: Canvas = (node.wrapper.children[0] as Canvas)
      .children[0] as Canvas;
    const index: number =
  ...
  public updateBPMNConnector(
    ...
    if (
      (flowType.flow === 'Sequence' || flowType.sequence) &&
      flow === 'Sequence'
    ) {
      actualObject = this.getSequence(
        actualObject,
        oldObject,
        connection,
        diagram
    ...
    if (
      (flowType.flow === 'Association' || flowType.association) &&
      flow === 'Association'
    ) {
      actualObject = this.getAssociation(
        actualObject,
        oldObject,
        connection,
        diagram
    ...
    if (
      (flowType.flow === 'Message' || flowType.message) &&
      flow === 'Message'
    ) {
      actualObject = this.getMessage(
        actualObject,
        oldObject,
        connection,
        diagram
  ...
  public getSequence(
    ...
    if (
      (connection.shape as BpmnFlowModel).sequence === 'Normal' ||
      (actualObject.shape as BpmnFlowModel).sequence === 'Normal'
    ) {
    ...
    if (
      (connection.shape as BpmnFlowModel).sequence === 'Default' ||
      ...
      const pathseq: PathElement = new PathElement();
      let pathseqData: Object;
      segment = actualObject.getSegmentElement(actualObject, segment);
      const anglePoints: PointModel[] =
        actualObject.intermediatePoints as PointModel[];
      for (let j: number = 0; j < anglePoints.length - 1; j++) {
        // eslint-disable-next-line no-global-assign
        length =
          length +
          actualObject.distance(
            anglePoints[parseInt(j.toString(), 10)],
            anglePoints[j + 1]
          );
        pathseqData = actualObject.findPath(
          anglePoints[parseInt(j.toString(), 10)],
          anglePoints[j + 1]
        );
      }
      ...
      content.id =
        actualObject.id + '_' + (connection.shape as BpmnFlowModel).sequence ||
        (actualObject.shape as BpmnFlowModel).sequence;
    ...
    if (
      (connection.shape as BpmnFlowModel).sequence === 'Conditional' ||
      (actualObject.shape as BpmnFlowModel).sequence === 'Conditional'
    ) {
  ...
  public getAssociation(
    ...
    if (
      (connection.shape as BpmnFlowModel).association === 'Default' ||
      (actualObject.shape as BpmnFlowModel).association === 'Default'
    ) {
      actualObject.sourceDecorator.shape = 'None';
    ...
    if (
      (connection.shape as BpmnFlowModel).association === 'Directional' ||
      (actualObject.shape as BpmnFlowModel).association === 'Directional'
    ) {
      actualObject.sourceDecorator.shape = 'None';
      actualObject.style.strokeDashArray = '2 2';
    ...
    if (
      (connection.shape as BpmnFlowModel).association === 'BiDirectional' ||
      (actualObject.shape as BpmnFlowModel).association === 'BiDirectional'
    ) {
  ...
  public getMessage(
    ...
    const segmentOffset: number = 0.5;
    let angle: number;
    let pt: PointModel;
    ...
    diagram.connectorPropertyChange(actualObject, oldObject, {
      ...
      targetDecorator: { shape: 'Arrow', style: { fill: 'white' } },
    } as Connector);
    const anglePoints: PointModel[] =
      actualObject.intermediatePoints as PointModel[];
    let length: number = 0;
    for (let j: number = 0; j < anglePoints.length - 1; j++) {
      /* eslint-disable */
      length =
        length + actualObject.distance(anglePoints[j], anglePoints[j + 1]);
      /* eslint-enable */
    ...
    content.id =
      actualObject.id + '_' + (connection.shape as BpmnFlowModel).message ||
      (actualObject.shape as BpmnFlowModel).message;
    content.width = 25;
    content.height = 15;
    ...
    if (
      ((connection.shape as BpmnFlowModel).message !== 'Default' &&
        (connection.shape as BpmnFlowModel).message !== undefined) ||
      (actualObject.shape as BpmnFlowModel).message !== 'Default'
    ) {
      content.style.fill =
        ((connection.shape as BpmnFlowModel).message ||
          (actualObject.shape as BpmnFlowModel).message) ===
  ...
  private setSizeForBPMNActivity(
    ...
    if (activity.subProcess.type === 'Transaction') {
      (wrapper.children[0] as Canvas).children[1].width = Math.max(
        width - 6,
        1
      );
      (wrapper.children[0] as Canvas).children[1].height = Math.max(
        height - 6,
        1
      );
    ...
    for (
      let i: number = 0;
      i < (wrapper.children[0] as Canvas).children.length;
      i++
    ) {
      if (
        (wrapper.children[0] as Canvas).children[parseInt(i.toString(), 10)]
          .visible &&
        ((wrapper.children[0] as Canvas).children[
          parseInt(i.toString(), 10)
        ].id.indexOf('_loop') > -1 ||
          (wrapper.children[0] as Canvas).children[
            parseInt(i.toString(), 10)
          ].id.indexOf('_0_compensation') > -1 ||
          (wrapper.children[0] as Canvas).children[
            parseInt(i.toString(), 10)
          ].id.indexOf('_0_adhoc') > -1 ||
          (wrapper.children[0] as Canvas).children[
            parseInt(i.toString(), 10)
          ].id.indexOf('_0_collapsed') > -1)
      ) {
        (wrapper.children[0] as Canvas).children[
          parseInt(i.toString(), 10)
        ].margin.left = x;
...
const bpmnShapes: {} = {
  ...
  DataSource:
    'M 0 10.6 c 0 5.9 16.8 10.6 37.5 10.6 S 75 16.4 75 10.6 v 0 v 68.9 v -0.1 C 75 85.3 58.2 90 37.5 90 ' +
  ...
  SubProcess:
    'M100,100 h200 a20,20 0 0 1 20,20 v200 a20,20 0 0 1 -20,20 h-200 ' +
    'a20,20 0 0 1 -20,-20 v-200 a20,20 0 0 1 20,-20 z',

  collapsedShape:
    'M 8.13789 15 H 0 V 0 H 8.13789 V 15 Z M 0.625991 13.75 H 7.51189 V 1.25 H 0.625991 V 13.75 Z ' +
...
const bpmnTriggerShapes: {} = {
  ...
  Timer:
    'M40,20c0,8.654-5.496,16.024-13.189,18.81' +
  ...
  Error:
    'M 23.77 18.527 l -7.107 27.396 l 8.507 -17.247 L 36.94 40.073 l 6.394 -25.997 l -8.497 15.754 L 23.77 18.527 Z',

  Escalation:
    'M 30.001 8.098 L 11.842 43.543 l 18.159 -18.882 l 18.162 18.882 L 30.001 8.098 Z ',

  Cancel:
    'M 3.5 16 L 0 12.6 L 4.6 8 L 0 3.5 L 3.4 0 L 8 4.6 l 4.5 -4.5 L 16 3.5 L 11.5 8 l 4.5 4.5 l -3.4 3.5 L 8 11.4 L 3.5 16 Z',

  Compensation:
    'M 25.7086 0 L 0 25 L 25.7086 50 V 26.3752 L 50 50 V 0 L 25.7086 23.6248 V 0 Z ',

  Conditional:
    'M 0 0 H 16 V 16 H 0 z M 1.14 3.2 H 14.85 M 1.14 6.4 H 14.85 M 1.14 9.6 H 14.85 M 1.14 12.8 H 14.85',
  ...
  Terminate:
    'M 25 50 C 11.21 50 0 38.79 0 25 C 0 11.21 11.21 0 25 0 C 38.78 0 50 11.21 50 25 C 50 38.79 38.78 50 25 50',

  Multiple:
    'M 17.784 48.889 H 42.21 l 7.548 -23.23 L 29.997 11.303 L 10.236 25.658 L 17.784 48.889 Z',

  Parallel:
    'M 27.276 49.986 h 5.58 v -17.15 h 17.146 V 27.17 h -17.15 l 0.004 -17.15 h -5.58 l -0.004 17.15 ' +
...
const bpmnGatewayShapes: {} = {
  ...
  //exclusive
  Exclusive:
    'M 11.196 29.009 l 6.36 -9.712 l -5.764 -8.899 h 4.393 l 3.732 5.979 l 3.656 -5.979 h 4.354 l -5.789 9.039' +
    'l 6.36 9.572 h -4.532 l -4.126 -6.437 l -4.139 6.437 H 11.196 Z',

  //inclusive
  Inclusive:
    'M 20.323 31.333 c -6.625 0 -12.015 -5.39 -12.015 -12.015 s 5.39 -12.015 12.015 -12.015 ' +
  ...
  //parallel
  Parallel:
    'M 18.394 29.542 v -8.833 H 9.626 v -3.691 h 8.768 V 8.251 h 3.734 v 8.767 h 8.768 v 3.691 h -8.768 v 8.833 H 18.394 Z',

  //complex
  Complex:
    'M29.198,19.063L23.089,19.063L27.794,14.358L26.38,12.944L21.223,18.101L21.223,10.443L19.223,10.443L19.223,17.976' +
  ...
  //eventbased
  EventBased:
    'M 20.322 29.874 c -5.444 0 -9.873 -4.43 -9.873 -9.874 s 4.429 -9.874 9.873 -9.874 s 9.874 4.429 9.874 9.874 ' +
  ...
  ExclusiveEventBased:
    'M 30 15 C 30 23.28 23.28 30 15 30 S 0 23.28 0 15 S 6.72 0 15 0 S 30 6.72 30 15 z M 15 5 ' +
    'L 5 12.5 L 8 22.5 H 22 L 25 12.5 z',

  //parallel event based
  ParallelEventBased:
    'M 35 17.5 C 35 27.16 27.16 35 17.5 35 S 0 27.16 0 17.5 S 7.84 0 17.5 0 S 35 7.84 35 17.5 z M 14.58 5.83 ' +
...
const bpmnTaskShapes: {} = {
  None: '',
  Service:
    'M 32.699 20.187 v -4.005 h -3.32 c -0.125 -0.43 -0.292 -0.83 -0.488 -1.21 l 2.373 -2.375 ' +
  ...
  Receive:
    'M 12.217 12.134 v 13.334 h 20 V 12.134 H 12.217 Z M 30.44 13.007 l -8.223 5.35 l -8.223 -5.35 H 30.44 Z M 13.09' +
  ...
  InstantiatingReceive:
    'M 16.306 17.39 v 8.79 h 13.198 v -8.79 H 16.306 Z M 28.375 17.946 l -5.47 3.558 l -5.47 -3.558 ' +
  ...
  User:
    'M8.6751149,13.75 L10.79009,18.703003 11.664047,18.696991 13.792084,13.75 13.824066,13.75 C18.177001,13.75 21.709022,' +
  ...
  Script:
    'M22.3,80.4 A82.9529,77.0159,-180,0,0,10.3,46.7 A69.8467,64.8477,0,0,1,0,19.7 A27.6207,25.6439,0,0,1,12.1,0 L84.7,0 A37.4124,34.7347,-180,0,0,77.7,19.6 A82.8871,76.9547,' +
...
const bpmnLoopShapes: {} = {
  None: '',
  Standard:
    'M 52.002 73.379 c -2.494 -2.536 -6.55 -2.534 -9.043 0 c -1.208 1.228 -1.874 2.861 -1.874 4.598 ' +
  ...
  ParallelMultiInstance:
    'M 51.5,69.5 L52.5,69.5 L52.5,84.5 L51.5 84.5 Z M 46.5,69.5 L47.5,69.5 L47.5,84.5 L46.5 84.5 Z' +
    ' M 41.5,69.5 L42.5,69.5 L42.5,84.5 L41.5 84.5 Z  ',

  SequenceMultiInstance:
    'M 40.375,71.5 L 55.375,71.5 L 55.375,72.5 L 40.375,72.5 Z M 40.375,76.5 L 55.375,76.5 ' +
