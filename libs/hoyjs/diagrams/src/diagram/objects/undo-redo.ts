...
export class UndoRedo {
  ...
  public initHistory(diagram: Diagram): void {
    diagram.historyManager = {
      ...
      undoStack: [],
      redoStack: [],
      stackLimit: diagram.historyManager
  ...
  public addHistoryEntry(entry: HistoryEntry, diagram: Diagram): boolean {
    ...
    if (
      entry.type === 'EndGroup' &&
      diagram.historyManager.currentEntry.type === 'StartGroup'
    ) {
      ...
      diagram.historyManager.currentEntry =
        diagram.historyManager.currentEntry.previous;
    ...
    if (
      diagram.historyManager &&
      diagram.historyManager.canUndo &&
      diagram.historyManager.currentEntry
    ) {
  ...
  private removeFromStack(entyList: HistoryEntry[], list: HistoryEntry): void {
    if (entyList.length) {
      for (let i: number = 0; i <= entyList.length; i++) {
        if (
          entyList[parseInt(i.toString(), 10)].undoObject === list.undoObject &&
  ...
  private undoEntry(entry: HistoryEntry, diagram: Diagram): void {
    ...
    if (
      ...
      entry.type !== 'CollectionChanged' &&
      entry.type !== 'LabelCollectionChanged'
    ) {
      obj = entry.undoObject
        ? (entry.undoObject as SelectorModel)
        : (entry.previous.undoObject as SelectorModel);
      nodeObject = obj;
    }
    if (entry.type !== 'StartGroup' && entry.type !== 'EndGroup') {
      if (diagram.historyManager.undoStack.length > 0) {
        const addObject: HistoryEntry[] =
          diagram.historyManager.undoStack.splice(0, 1);
    ...
    switch (entry.type) {
      ...
      case 'CollectionChanged':
        if (
          entry &&
          entry.next &&
          entry.next.type === 'AddChildToGroupNode' &&
          entry.next.changeType === 'Insert'
        ) {
          const group: NodeModel = diagram.getObject(
            (entry.next.undoObject as NodeModel).id
          );
    ...
    if (nodeObject) {
      const object: NodeModel | ConnectorModel = this.checkNodeObject(
        nodeObject,
        diagram
      );
      if (object) {
  ...
  private checkNodeObject(
    value: SelectorModel | Node,
    diagram: Diagram
  ): NodeModel | ConnectorModel {
    let object: NodeModel | ConnectorModel;
    if (!(value as Node).id) {
      if (
        ((value as SelectorModel).nodes &&
          (value as SelectorModel).nodes.length > 0) ||
        ((value as SelectorModel).connectors &&
          (value as SelectorModel).connectors.length > 0)
      ) {
        const undoNode: NodeModel[] | ConnectorModel[] =
          (value as SelectorModel).nodes.length > 0
            ? (value as SelectorModel).nodes
            : (value as SelectorModel).connectors;
        ...
        const knownNode: NodeModel[] | ConnectorModel[] = (
          value as SelectorModel
        ).nodes
          ? (value as SelectorModel).nodes
          : (value as SelectorModel).connectors;
        if (knownNode) {
          for (const key of Object.keys(knownNode)) {
            ...
            object = (value as SelectorModel).nodes
              ? (diagram.nodes[parseInt(index.toString(), 10)] as Node)
  ...
  private recordLaneOrPhaseCollectionChanged(
    ...
    const obj: LaneModel | PhaseModel = entry.undoObject as
      | LaneModel
      | PhaseModel;
  ...
  private recordAnnotationChanged(
    ...
    const entryObject: NodeModel | ConnectorModel = (
      isRedo ? entry.redoObject : entry.undoObject
    ...
    const undoChanges: Object = diagram.commandHandler.getAnnotationChanges(
      diagram.nameTable[entryObject.id],
      oldElement
    );
    ...
    if (currentObject instanceof Node) {
      diagram.nodePropertyChange(
        currentObject as Node,
        {} as Node,
        undoChanges as Node
      );
    } else {
      diagram.connectorPropertyChange(
        currentObject as Connector,
        {} as Connector,
        undoChanges as Connector
      );
    }
  }
  ...
  private recordChildCollectionChanged(
    ...
    let parentNode: NodeModel =
      diagram.nameTable[(entryObject as Node).parentId];
    ...
    if (parentNode) {
      addChildToContainer(
        ...
        actualObject,
        !isRedo,
        entry.historyAction === 'AddNodeToLane'
      );
    } else {
      if (actualObject.parentId) {
        parentNode = diagram.nameTable[actualObject.parentId];
        parentNode.children.splice(
          parentNode.children.indexOf(actualObject.id),
          1
        );
        parentNode.wrapper.children.splice(
          parentNode.wrapper.children.indexOf(actualObject.wrapper),
          1
        );
      }
      if (
        (entryObject as Node).parentId &&
        (entryObject as Node).parentId !== ''
      ) {
  ...
  private recordStackPositionChanged(
    ...
    const entryObject: StackEntryObject = (
      isRedo ? entry.redoObject : entry.undoObject
    ) as StackEntryObject;
    if (entryObject.source) {
      const parent: Node =
        diagram.nameTable[(entryObject.source as Node).parentId];
      if (parent) {
        if (entryObject.target) {
          parent.wrapper.children.splice(entryObject.targetIndex, 1);
          parent.wrapper.children.splice(
            entryObject.sourceIndex,
            0,
            entryObject.source.wrapper
          );
        } else {
          if (entryObject.sourceIndex !== undefined) {
            ...
            parent.wrapper.children.splice(
              entryObject.sourceIndex,
              0,
              diagram.nameTable[entryObject.source.id].wrapper
            );
            diagram.nameTable[entryObject.source.id].parentId = parent.id;
          } else {
            parent.wrapper.children.splice(
              parent.wrapper.children.indexOf(
                diagram.nameTable[entryObject.source.id].wrapper
              ),
              1
            );
  ...
  private recordGridSizeChanged(
    ...
    const obj: Node = isRedo
      ? (entry.redoObject as Node)
      : (entry.undoObject as Node);
    const node: Node = !isRedo
      ? (entry.redoObject as Node)
      : (entry.undoObject as Node);
    if (obj.parentId) {
      ...
      if (swimlane.shape.type === 'SwimLane') {
        ...
        if (isRow) {
          ...
          if (obj.isPhase) {
            actualObject.maxWidth = actualObject.wrapper.maxWidth =
              obj.wrapper.actualSize.width;
          }
        }
  ...
  private recordPortChanged(
    ...
    const entryObject: NodeModel = isRedo
      ? (entry.redoObject as SelectorModel).nodes[0]
      : (entry.undoObject as SelectorModel).nodes[0];
    if (diagram.canEnableBlazorObject) {
      const node: object = cloneObject(diagram.nameTable[entryObject.id]);
      diagram.insertValue(node, true);
    }
    const oldElement: PointPort = findPort(
      entryObject,
      entry.objectId
    ) as PointPort;
    const undoChanges: Object = diagram.commandHandler.getPortChanges(
      diagram.nameTable[entryObject.id],
      oldElement
    );
    const currentObject: NodeModel | ConnectorModel =
    ...
    if ((currentObject as Node).parentId) {
      diagram.updateConnectorEdges(
        diagram.nameTable[(currentObject as Node).parentId]
      );
    }
  }
  ...
  private recordPropertyChanged(
    ...
    this.getProperty(
      diagram as Object,
      (isRedo ? redoObject : undoObject) as Object
    );
    // eslint-disable-next-line @typescript-eslint/no-unused-expressions
    isRedo
      ? diagram.onPropertyChanged(redoObject, undoObject)
      : diagram.onPropertyChanged(undoObject, redoObject);
  ...
  private recordOrderCommandChanged(
    ...
    diagram.commandHandler.orderCommands(
      isRedo,
      isRedo ? redoObject : undoObject,
      entry.type
    );
    diagram.diagramActions = diagram.diagramActions | DiagramAction.UndoRedo;
  }
  ...
  private positionChanged(obj: NodeModel, diagram: Diagram): void {
    ...
    if ((node as Node).processId) {
      ...
      if ((node as Node).parentId) {
        ...
        if (parent.isLane) {
          obj.wrapper.offsetX =
            obj.width / 2 + (parent.wrapper.bounds.x + obj.margin.left);
          obj.wrapper.offsetY =
            obj.height / 2 + (parent.wrapper.bounds.y + obj.margin.top);
        }
      }
      ...
      for (i = 0; i < obj.nodes.length; i++) {
        ...
        if (node.children && !node.container) {
          ...
          const nodes: (NodeModel | ConnectorModel)[] =
            diagram.commandHandler.getAllDescendants(node, elements);
          for (let i: number = 0; i < nodes.length; i++) {
            const tempNode: NodeModel | ConnectorModel =
              entry.childTable[nodes[parseInt(i.toString(), 10)].id];
  ...
  private sizeChanged(
    ...
    const scaleWidth: number =
      obj.wrapper.actualSize.width / node.wrapper.actualSize.width;
    const scaleHeight: number =
      obj.wrapper.actualSize.height / node.wrapper.actualSize.height;
      ...
      for (i = 0; i < obj.nodes.length; i++) {
        ...
        if (node.children && node.children.length > 0) {
          for (let j: number = 0; j < node.children.length; j++) {
            const child: NodeModel | ConnectorModel =
              diagram.nameTable[node.children[parseInt(j.toString(), 10)]];
            if (!(getObjectType(child) === Node)) {
              if (
                (child as ConnectorModel).sourceID === '' ||
                (child as ConnectorModel).targetID === ''
              ) {
                isConnect = true;
                break;
              }
          ...
          if (isConnect && Object.keys(entry.childTable).length > 0) {
            ...
            for (let i2: number = 0; i2 < nodes.length; i2++) {
              const tempNode: NodeModel | ConnectorModel =
                entry.childTable[nodes[parseInt(i2.toString(), 10)].id];
              if (getObjectType(tempNode) === Node) {
                let object: any = {
                  id: '',
                  rotateAngle: 0,
                  wrapper: { offsetX: 0, offsetY: 0 },
                };
                if (type === 'redo') {
                ...
                if (type === 'undo') {
                  ...
                  this.undoOffsets.push({
                    id: offNode.id,
                    offsetX: offNode.offsetX,
                    offsetY: offNode.offsetY,
                  });
                } else {
                  let lastIndex: number = -1;
                  for (
                    let i: number = this.undoOffsets.length - 1;
                    i >= 0;
                    i--
                  ) {
                    if (
                      this.undoOffsets[parseInt(i.toString(), 10)].id ===
                      object.id
                    ) {
                      object.wrapper =
                        this.undoOffsets[parseInt(i.toString(), 10)];
                      lastIndex = i;
                      break;
                    }
                  }
  ...
  private recordConnectionChanged(
    obj: SelectorModel | ConnectorModel,
    diagram: Diagram
  ): void {
    let connector: ConnectorModel;
  ...
  private connectionChanged(
    ...
    if (obj.sourcePortID !== connector.sourcePortID) {
      diagram.removePortEdges(
        diagram.nameTable[connector.sourceID],
      ...
      diagram.connectorPropertyChange(
        ...
        {} as Connector,
        { sourcePortID: obj.sourcePortID } as Connector
      );
    }
    if (obj.targetPortID !== connector.targetPortID) {
      ...
      diagram.connectorPropertyChange(
        connector as Connector,
        {} as Connector,
        { targetPortID: obj.targetPortID } as Connector
      );
    }
    if (obj.sourceID !== connector.sourceID) {
      ...
      diagram.connectorPropertyChange(
        connector as Connector,
        {} as Connector,
        { sourceID: obj.sourceID } as Connector
      );
    }
    if (obj.targetID !== connector.targetID) {
      ...
      diagram.connectorPropertyChange(
        connector as Connector,
        {} as Connector,
        { targetID: obj.targetID } as Connector
      );
    }
    if (entry && entry.childTable) {
  ...
  private recordCollectionChanged(entry: HistoryEntry, diagram: Diagram): void {
    const obj: NodeModel | ConnectorModel = entry.undoObject as
      | NodeModel
      | ConnectorModel;
    if (entry && entry.changeType) {
      ...
      if (changeType === 'Remove') {
        if ((obj as BpmnAnnotation).nodeId) {
          diagram.remove(
            diagram.nameTable[
              (obj as BpmnAnnotation).nodeId + '_textannotation_' + obj.id
            ]
          );
        } else {
          diagram.remove(obj);
        ...
        if ((obj as Node | Connector).parentId) {
          const parentNode: NodeModel =
            diagram.nameTable[(obj as Node | Connector).parentId];
          if (parentNode) {
          ...
          diagram.addTextAnnotation(
            obj,
            diagram.nameTable[(obj as BpmnAnnotation).nodeId]
          );
        } else {
          if (!diagram.nameTable[obj.id]) {
            if (
              obj &&
              obj.shape &&
              (obj as Node).shape.type === 'SwimLane' &&
              entry.isUndo
            ) {
              pasteSwimLane(
                obj as NodeModel,
  ...
  private recordLabelCollectionChanged(
    ...
    const label: ShapeAnnotationModel | PathAnnotationModel =
      entry.undoObject as ShapeAnnotationModel | PathAnnotationModel;
    ...
    if (entry && entry.changeType) {
      ...
      if (changeType === 'Remove') {
        diagram.removeLabels(node, [label] as
          | ShapeAnnotationModel[]
          | PathAnnotationModel[]);
        diagram.clearSelectorLayer();
      } else {
        diagram.clearSelectorLayer();
        diagram.addLabels(node, [label] as
          | ShapeAnnotationModel[]
          | PathAnnotationModel[]);
      }
      if (diagram.mode !== 'SVG') {
  ...
  private redoEntry(historyEntry: HistoryEntry, diagram: Diagram): void {
    ...
    if (
      historyEntry.type !== 'PropertyChanged' &&
      historyEntry.type !== 'CollectionChanged'
    ) {
    ...
    diagram.diagramActions |= DiagramAction.UndoRedo;
    if (
      historyEntry.type !== 'StartGroup' &&
      historyEntry.type !== 'EndGroup'
    ) {
      if (diagram.historyManager.redoStack.length > 0) {
        const addObject: HistoryEntry[] =
          diagram.historyManager.redoStack.splice(0, 1);
    ...
    if (redovalue) {
      const value: NodeModel | ConnectorModel = this.checkNodeObject(
        redovalue,
        diagram
      );
      if (value) {
