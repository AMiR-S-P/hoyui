...
import { Browser, ModuleDeclaration, Event, EmitType } from '@hoyui/base';
import {
...
import {
  PageSettingsModel,
  ScrollSettingsModel,
} from './diagram/page-settings-model';
...
import {
  ...
  ConnectorStyle,
  MermaidStyle,
} from './objects/interface/interfaces';
...
import {
  ...
  AnnotationModel,
  PathAnnotationModel,
} from './objects/annotation-model';
import {
  ShapeAnnotation,
  PathAnnotation,
  Annotation,
} from './objects/annotation';
...
import {
  ...
  getSelectorElement,
  getGridLayerSvg,
  getBackgroundLayerSvg,
} from './utility/dom-util';
import {
  CommandManager,
  ContextMenuSettings,
} from './diagram/keyboard-commands';
import {
  CommandManagerModel,
  CommandModel,
  ContextMenuSettingsModel,
} from './diagram/keyboard-commands-model';
import {
  canDelete,
  canInConnect,
  canOutConnect,
...
import { Tooltip } from '@hoyui/popups';
...
} from './interaction/container-interaction';
import { DataManager } from '@hoyui/data';
import {
  getConnectors,
  updateConnectorsProperties,
  phaseDefine,
} from './utility/swim-lane-util';
...
import {
  ...
  NodeFixedUserHandle,
} from './objects/fixed-user-handle';
import {
  NodeFixedUserHandleModel,
  ConnectorFixedUserHandleModel,
  FixedUserHandleModel,
} from './objects/fixed-user-handle-model';
...
import {
  getClassAttributesChild,
  getClassMembersChild,
  getClassNodesChild,
} from './utility/uml-util';
  ...
  implements INotifyPropertyChanged
{
  ...
  public commands: {};
  /** @private */
  public activeLabel: ActiveLabel = {
    id: '',
    parentId: '',
    isGroup: false,
    text: undefined,
  ...
  /** @private */
  public selectedObject: { helperObject: NodeModel; actualObject: NodeModel } =
    { helperObject: undefined, actualObject: undefined };
  /** @private */
  ...
  constructor(options?: DiagramModel, element?: HTMLElement | string) {
    ...
    for (
      let i: number = 0;
      options && options.nodes && i < options.nodes.length;
      i++
    ) {
    ...
    if (options && options.connectors) {
      for (
        let i: number = 0;
        options && options.connectors && i < options.connectors.length;
        i++
      ) {
    ...
    for (
      ...
      const defaultConnector: ConnectorModel =
        options.connectors[parseInt(i.toString(), 10)];
      const connector: ConnectorModel =
        this.connectors[parseInt(i.toString(), 10)];
  ...
  private updateAnnotationText(
    annotations?: PathAnnotationModel[] | ShapeAnnotationModel[]
  ): void {
    //Removed isBlazor code
  }
  ...
  private clearCollection(isConnector?: boolean): void {
    ...
    for (const key of Object.keys(this.nameTable)) {
      ...
      if (
        obj &&
        ((isConnector && obj instanceof Connector) ||
          (!isConnector && obj instanceof Node))
      ) {
        collection.push(obj);
      }
    }
    ...
    if (!(this as any).rotateUsingButton) {
      ...
      if (this.diagramActions & DiagramAction.Render) {
        for (const prop of Object.keys(newProp)) {
          switch (prop) {
            ...
            case 'nodes':
              if (newProp.nodes.length > 0 && oldProp.nodes.length === 0) {
                ...
                for (const key of Object.keys(newProp.nodes)) {
                  ...
                  const actualObject: Node = this.nodes[
                    parseInt(index.toString(), 10)
                  ] as Node;
                  const changedProp: Node = newProp.nodes[
                    parseInt(index.toString(), 10)
                  ] as Node;
                  if (
                    newProp.nodes[parseInt(index.toString(), 10)].style &&
                    newProp.nodes[parseInt(index.toString(), 10)].style.gradient
                  ) {
                    this.updateGradient(
                      newProp.nodes[parseInt(index.toString(), 10)],
                      oldProp.nodes[parseInt(index.toString(), 10)],
                      this.nodes[parseInt(index.toString(), 10)] as Node
                    );
                    (
                      this.nodes[parseInt(index.toString(), 10)] as Node
                    ).oldGradientValue = cloneObject(
                      newProp.nodes[parseInt(index.toString(), 10)].style
                        .gradient
                    );
                  }
                  refreshLayout =
                    refreshLayout ||
                    changedProp.excludeFromLayout !== undefined;
                  this.nodePropertyChange(
                    actualObject,
                    oldProp.nodes[parseInt(index.toString(), 10)] as Node,
                    changedProp,
                    undefined,
                    true,
                    true
                  ...
                  const args:
                    ...
                    oldValue: cloneBlazorObject(
                      oldProp.nodes[parseInt(index.toString(), 10)]
                    ) as Node,
                    newValue: cloneBlazorObject(
                      newProp.nodes[parseInt(index.toString(), 10)]
                    ) as Node,
                  };
                  // Removed isBlazor code
            ...
            case 'connectors':
              ...
              if (
                newProp.connectors.length > 0 &&
                oldProp.connectors.length === 0
              ) {
                ...
                for (const key of Object.keys(newProp.connectors)) {
                  const index: number = Number(key);
                  const actualObject: Connector = this.connectors[
                    parseInt(index.toString(), 10)
                  ] as Connector;
                  const changedProp: Connector = newProp.connectors[
                    parseInt(index.toString(), 10)
                  ] as Connector;
                  if (
                    changedProp &&
                    (changedProp.sourceDecorator || changedProp.targetDecorator)
                  ) {
                    this.diagramActions |=
                      DiagramAction.DecoratorPropertyChange;
                  }
                  this.connectorPropertyChange(
                    actualObject,
                    oldProp.connectors[
                      parseInt(index.toString(), 10)
                    ] as Connector,
                  ...
                  const args:
                    ...
                    diagramAction: this.getDiagramAction(this.diagramActions),
                    oldValue: cloneBlazorObject(
                      oldProp.connectors[parseInt(index.toString(), 10)]
                    ) as Connector,
                    newValue: cloneBlazorObject(
                      newProp.connectors[parseInt(index.toString(), 10)]
                    ) as Connector,
                  };
                  // Removed isBlazor code
                  this.triggerEvent(DiagramEvent.propertyChange, args);
                  if (
                    ...
                    this.nameTable[actualObject.parentId].shape.type ===
                      'UmlClassifier'
                  ) {
                    this.updateConnectorEdges(
                      this.nameTable[actualObject.parentId] || actualObject
                    );
                  }
                  if (isPropertyChanged) {
                    isPropertyChanged = false;
            ...
            case 'selectedItems':
              if (
                newProp.selectedItems.userHandles &&
                this.selectedItems.wrapper &&
                this.selectedItems.userHandles
              ) {
            ...
            case 'scrollSettings':
              ...
              this.scrollSettings.horizontalOffset =
                -this.scroller.horizontalOffset || 0;
              this.scrollSettings.verticalOffset =
            ...
            case 'contextMenuSettings':
              if (
                newProp.contextMenuSettings.showCustomMenuOnly !== undefined
              ) {
                this.contextMenuSettings.showCustomMenuOnly =
                  newProp.contextMenuSettings.showCustomMenuOnly;
              }
              if (newProp.contextMenuSettings.show !== undefined) {
                this.contextMenuSettings.show =
                  newProp.contextMenuSettings.show;
              }
              if (newProp.contextMenuSettings.items) {
                const items: ContextMenuItemModel[] =
                  newProp.contextMenuSettings.items;
                for (const key of Object.keys(items)) {
                  const index: number = Number(key);
                  this.contextMenuSettings.items[
                    parseInt(index.toString(), 10)
                  ] = items[parseInt(index.toString(), 10)];
                }
                if (this.contextMenuModule) {
                  this.contextMenuModule.refreshItems();
                } else {
                  console.warn(
                    '[WARNING] :: Module "DiagramContextMenu" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
                  );
                }
              }
              break;
            case 'serializationSettings':
              if (newProp.serializationSettings.preventDefaults !== undefined) {
                this.serializationSettings.preventDefaults =
                  newProp.serializationSettings.preventDefaults;
              }
              break;
          }
        }
        if (refreshLayout && !refereshColelction) {
          if (
            (oldProp.layout &&
              oldProp.layout.connectionPointOrigin === 'DifferentPoint' &&
              newProp.layout.connectionPointOrigin === 'SamePoint') ||
            (oldProp.layout &&
              newProp.layout &&
              !newProp.layout.enableRouting &&
              oldProp.layout.enableRouting)
          ) {
            for (let i: number = 0; i < this.nodes.length; i++) {
              ...
              if (node.ports && node.ports.length > 0) {
                ...
                for (let j: number = node.ports.length - 1; j >= 0; j--) {
                  if (
                    node.ports[parseInt(j.toString(), 10)].id.split('_')[1] ===
                    'LineDistribution'
                  ) {
            ...
            for (let j: number = 0; j < this.connectors.length; j++) {
              const connector: ConnectorModel =
                this.connectors[parseInt(j.toString(), 10)];
              ...
              this.connectorPropertyChange(
                ...
                { sourcePortID: '', targetPortID: '' } as Connector
              );
            }
          }
          this.doLayout();
        ...
        if (
          !refereshColelction &&
          this.canLogChange() &&
          this.modelChanged(newProp, oldProp)
        ) {
          ...
          if (newProp.nodes && Object.keys(newProp.nodes).length > 0) {
            for (const key of Object.keys(newProp.nodes)) {
              ...
              nodeObjects.push(
                this.nodes[parseInt(nodeIndex.toString(), 10)].id
              );
            }
          }
          if (
            newProp.connectors &&
            Object.keys(newProp.connectors).length > 0
          ) {
            for (const key of Object.keys(newProp.connectors)) {
              const connIndex: number = parseInt(key, 10);
              connObjects.push(
                this.connectors[parseInt(connIndex.toString(), 10)].id
              );
            }
          }
          propertyObjects = nodeObjects.concat(connObjects);
          //To prevent history entry for text annotation connector while dragging node.
          const textCon = connObjects.filter(
            (id) => this.nameTable[`${id}`].isBpmnAnnotationConnector
          );
          if (textCon.length === 0) {
        ...
        const scrollAlone: boolean =
          Object.keys(newProp).length === 1 &&
          newProp.scrollSettings !== undefined;
        if (!refereshColelction) {
          for (const temp of this.views) {
            const view: View = this.views[`${temp}`];
            if (!(view instanceof Diagram)) {
              if (
                newProp.scrollSettings &&
                newProp.scrollSettings.currentZoom !==
                  oldProp.scrollSettings.currentZoom
              ) {
  ...
  private updateSnapSettings(newProp: DiagramModel): void {
    if (
      newProp.snapSettings.constraints !== undefined ||
      newProp.snapSettings.horizontalGridlines ||
      newProp.snapSettings.verticalGridlines ||
      newProp.snapSettings.gridType
    ) {
      this.diagramRenderer.updateGrid(
        this.snapSettings,
        getGridLayerSvg(this.element.id),
  ...
  private updateGradient(
    ...
    if (nodeObj.oldGradientValue) {
      ...
      const linearProp: LinearGradient = oldProp.style
        .gradient as LinearGradient;
      const radialProp: RadialGradient = oldProp.style
      ...
      for (const key of Object.keys(newProp.style.gradient)) {
        switch (key) {
          ...
          case 'stops':
            if (
              (nodeObj.oldGradientValue as LinearGradient | RadialGradient)
                .stops
            ) {
              const stops: StopModel[] = (Object as any).values(
                cloneObject(
                  (nodeObj.oldGradientValue as LinearGradient | RadialGradient)
                    .stops
                )
  ...
  private updateRulerSettings(newProp: DiagramModel): void {
    ...
    if (newProp.rulerSettings.showRulers !== undefined) {
      ...
      newProp.rulerSettings.horizontalRuler !== undefined ||
      newProp.rulerSettings.verticalRuler !== undefined
    ) {
      if (
        newProp.rulerSettings.horizontalRuler.thickness !== undefined ||
  ...
  private initializePrivateVariables(): void {
    if (this.element.id === '') {
      const collection: number =
        document.getElementsByClassName('e-diagram').length;
    ...
    if (
      this.swimlaneChildTable === undefined &&
      this.swimlaneZIndexTable === undefined
    ) {
  ...
  public render(): void {
    ...
    if (collapsedNode.length) {
      for (let i: number = collapsedNode.length - 1; i >= 0; i--) {
        if (i === 0) {
          this.commandHandler.expandNode(
            collapsedNode[parseInt(i.toString(), 10)] as Node,
            this,
            false
          );
        } else {
    ...
    if (this.lineRoutingModule) {
      ...
      if (
        (this.connectors as ConnectorModel).sourceID !==
        (this.connectors as ConnectorModel).targetID
      ) {
      ...
      console.warn(
        '[WARNING] :: Module "LineRouting" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
      );
    }
    this.validatePageSize();
    ...
    if (
      this.scrollSettings.verticalOffset > 0 ||
      this.scrollSettings.horizontalOffset > 0
    ) {
    ...
    this.initCommands();
    const hiddenUserHandleTemplate: HTMLCollection =
      document.getElementsByClassName(
        this.element.id + '_hiddenUserHandleTemplate'
      );
    createUserHandleTemplates(
      this.userHandleTemplate,
      hiddenUserHandleTemplate,
      this.selectedItems,
      this.element.id
    );
    //Removed isBlazor code.
  ...
  private updateFitToPage(): void {
    if (
      this.pageSettings &&
      this.pageSettings.fitOptions &&
      this.pageSettings.fitOptions.canFit
    ) {
  ...
  private renderInitialCrud(): void {
    ...
    if (tempObj.dataSourceSettings.crudAction.read) {
      const callback: Fetch = new Fetch(
        tempObj.dataSourceSettings.crudAction.read,
        'GET'
      );
    ...
    if (tempObj.dataSourceSettings.connectionDataSource.crudAction.read) {
      const callback: Fetch = new Fetch(
        tempObj.dataSourceSettings.connectionDataSource.crudAction.read,
        'GET'
      );
      callback.onSuccess = (data: string): void => {
        tempObj.dataSourceSettings.connectionDataSource.dataManager =
          JSON.parse(data);
        tempObj.dataBind();
      };
      callback.send().then();
  ...
  public requiredModules(): ModuleDeclaration[] {
    ...
    if (
      this.dataSourceSettings.dataManager ||
      this.dataSourceSettings.dataSource ||
      this.dataSourceSettings.crudAction.read ||
      this.dataSourceSettings.connectionDataSource.crudAction.read
    ) {
      modules.push({
        member: 'DataBinding',
        args: [],
    ...
    if (
      (this.layout &&
        (this.layout.type === 'ComplexHierarchicalTree' ||
          this.layout.type === 'HierarchicalTree')) ||
      this.layout.arrangement === 'Linear' ||
      this.layout.enableRouting
    ) {
      modules.push({
    ...
    if (this.selectedItems.userHandles.length) {
      for (let i: number = 0; i < this.selectedItems.userHandles.length; i++) {
        for (const elementId of this.views) {
          removeElement(
            this.selectedItems.userHandles[parseInt(i.toString(), 10)].name +
              '_template_hiddenUserHandle',
            elementId
          );
        }
      }
  ...
  public destroy(): void {
    ...
    if (document.getElementById(this.element.id)) {
      ...
      const tooltipelement: HTMLCollection =
        document.getElementsByClassName('e-diagram-tooltip');
      ...
      if (window[`${measureWindowElement}`]) {
        ...
        if (window[`${measureElementCount}`] === 0) {
          window[`${measureWindowElement}`].parentNode.removeChild(
            window[`${measureWindowElement}`]
          );
          window[`${measureWindowElement}`] = null;
        }
      }
    ...
    for (let i: number = 0; i < this.layers.length; i++) {
      const currentLayer: Layer = this.layers[
  ...
  private wireEvents(): void {
    ...
    const cancelEvent: string = Browser.isPointer
      ? 'pointerleave'
      : 'mouseleave';
    ...
    EventHandler.add(
      this.diagramCanvas,
      cancelEvent,
      this.eventHandler.mouseLeave,
      this.eventHandler
    );
    EventHandler.add(
      this.diagramCanvas,
      'keydown',
      this.eventHandler.keyDown,
      this.eventHandler
    );
    EventHandler.add(
      this.diagramCanvas,
      'keyup',
      this.eventHandler.keyUp,
      this.eventHandler
    );
    EventHandler.add(
      this.diagramCanvas,
      'dblclick',
      this.eventHandler.doubleClick,
      this.eventHandler
    );
    EventHandler.add(
      this.diagramCanvas,
      'scroll',
      this.eventHandler.scrolled,
      this.eventHandler
    );
    EventHandler.add(
      this.diagramCanvas,
      wheelEvent,
      this.eventHandler.mouseWheel,
      this.eventHandler
    );
    EventHandler.add(
      <HTMLElement & Window>window,
      'resize',
      this.eventHandler.windowResize,
      this.eventHandler
    );
    this.initDroppables();
  }
  ...
  private nudgeCommand(
    ...
    if (
      ...
      (this.selectedItems.nodes.length ||
        this.selectedItems.connectors.length) > 0
    ) {
      let type: string;
  ...
  private insertBlazorDiagramObjects(
    actualObject: NodeModel | ConnectorModel | SelectorModel
  ): void {
    //Removed isBlazor code
  }
  ...
  public drag(
    ...
    if (this.bpmnModule && obj instanceof Node) {
      const updated: boolean = this.bpmnModule.updateAnnotationDrag(
        obj as Node,
        this,
        tx,
        ty
      ...
      if (obj.nodes && obj.nodes.length) {
        for (const node of obj.nodes) {
          ...
          if ((node as Node).parentId) {
            const parent: Node = this.nameTable[
    ...
    if (obj instanceof Selector) {
      ...
      for (const connectors of this.selectionConnectorsList) {
        ...
        if (
          connectors.shape.type === 'Bpmn' &&
          (connectors.shape as BpmnFlowModel).sequence === 'Default' &&
          (connectors.shape as BpmnFlowModel).flow === 'Sequence'
        ) {
    ...
    if (
      this.callBlazorModel &&
      !(this.blazorActions & BlazorAction.interaction) &&
  ...
  public scale(
    ...
    if (obj instanceof Selector) {
      if (obj.nodes && obj.nodes.length) {
        this.callBlazorModel = false;
        for (const node of obj.nodes) {
          checkBoundaryConstraints = this.commandHandler.scale(
            node,
            sx,
            sy,
            pivot,
      ...
      this.commandHandler.scale(
        obj as NodeModel | ConnectorModel,
        sx,
        sy,
        pivot,
        (obj as NodeModel).children ? (obj as IElement) : undefined
      );
    }
    if (
      this.callBlazorModel &&
  ...
  public rotate(
    ...
    if (!rotateUsingHandle) {
      ...
      if (
        undoObject.nodes &&
        undoObject.nodes.length > 0 &&
        undoObject.nodes[0].children
      ) {
        const elements: any = [];
        if (!(this as any).fromUndo) {
          (this as any).rotateUsingButton = true;
        }
        const nodes: (NodeModel | ConnectorModel)[] =
          this.commandHandler.getAllDescendants(undoObject.nodes[0], elements);
        for (let i: number = 0; i < nodes.length; i++) {
          const node: NodeModel = this.commandHandler.cloneChild(
            nodes[parseInt(i.toString(), 10)].id
          );
    ...
    if (obj) {
      ...
      if (obj instanceof Selector) {
        ...
        checkBoundaryConstraints = this.commandHandler.checkBoundaryConstraints(
          undefined,
          undefined,
          bounds
        );
        ...
        this.commandHandler.rotateObjects(
          obj as NodeModel,
          [obj] as (NodeModel | ConnectorModel)[],
          angle,
          pivot
  ...
  public findTargetObjectUnderMouse(
    ...
    return this.eventHandler.findTargetUnderMouse(
      objects,
      action,
      inAction,
      position,
   ...
   */
  public findElementUnderMouse(
    obj: IElement,
    position: PointModel,
    diagram: Diagram,
    padding?: number
  ): DiagramElement {
    return this.eventHandler.findElementUnderMouse(
      obj,
      position,
      diagram,
      padding
  ...
  private updateConnectorPort(connector: Connector): void {
    if (connector.ports.length) {
      ...
      for (const port of connector.ports) {
        portContent = this.getWrapper(
          connector.wrapper,
          port.id
        ) as PathElement;
        connector.initPortWrapper(
          port as Port,
          connector.intermediatePoints,
          connector.wrapper.bounds,
          portContent
        );
      }
    }
    connector.wrapper.measure(
      new Size(connector.wrapper.width, connector.wrapper.height)
    );
  ...
  public addHistoryEntry(entry: HistoryEntry, sourceId?: string[]): void {
    if (
      ...
      (!this.currentSymbol ||
        this.checkCurrentSymbol(this.currentSymbol, entry))
    ) {
  ...
  private checkCurrentSymbol(
    ...
    if (entry.undoObject && entry.redoObject) {
      const undoObjects: NodeModel | ConnectorModel = entry.undoObject as
        | NodeModel
        | ConnectorModel;
      const redoObject: NodeModel | ConnectorModel = entry.undoObject as
        | NodeModel
        | ConnectorModel;
      if (
        redoObject.id &&
        undoObjects.id &&
        redoObject.id !== currentSymbol.id &&
        undoObjects.id !== currentSymbol.id
      ) {
        return (check = true);
      } else {
        return check;
  ...
  public historyChangeTrigger(
    ...
    const entryType: string = 'entryType';
    let source: (NodeModel | ConnectorModel)[] = [];
    if (entry.category === 'Internal') {
      if (
        entry &&
        entry.redoObject &&
        (entry.redoObject as SelectorModel).nodes instanceof Array &&
        (entry.redoObject as SelectorModel).connectors instanceof Array
      ) {
        source = ((entry.redoObject as SelectorModel).nodes as object[]).concat(
          (entry.redoObject as SelectorModel).connectors
        );
      } else {
        if (entry.redoObject) {
      ...
      switch (entry.type) {
        ...
        case 'RotationChanged':
          change[`${oldValue}`] = {
            rotateAngle: (entry.undoObject as NodeModel).rotateAngle,
          };
          change[`${newValue}`] = {
            rotateAngle: (entry.redoObject as NodeModel).rotateAngle,
          };
          break;
        case 'SizeChanged':
          change[`${oldValue}`] = {
            offsetX: (entry.undoObject as NodeModel).offsetX,
            offsetY: (entry.undoObject as NodeModel).offsetY,
            width: (entry.undoObject as NodeModel).width,
            height: (entry.undoObject as NodeModel).height,
          };
          change[`${newValue}`] = {
            offsetX: (entry.redoObject as NodeModel).offsetX,
            offsetY: (entry.redoObject as NodeModel).offsetY,
            width: (entry.redoObject as NodeModel).width,
            height: (entry.redoObject as NodeModel).height,
          };
          break;
      ...
      if (sourceId === undefined && entry.type === 'PropertyChanged') {
        for (let i: number = 0; i < Object.keys(entry.undoObject).length; i++) {
          if (
            Object.keys(entry.undoObject)[parseInt(i.toString(), 10)] ===
            'nodes'
          ) {
            for (const key of Object.keys(
              (entry.undoObject as DiagramModel).nodes
            )) {
              const undoIndex: number = parseInt(key, 10);
              nodeSourceId.push(
                (this.nodes[parseInt(undoIndex.toString(), 10)] as NodeModel).id
              );
            }
          }
        }
        for (let i: number = 0; i < Object.keys(entry.undoObject).length; i++) {
          if (
            ...
            for (const key of Object.keys(
              (entry.undoObject as DiagramModel).connectors
            )) {
              const undoIndex: number = parseInt(key, 10);
              connectorSourceId.push(
                (
                  this.connectors[
                    parseInt(undoIndex.toString(), 10)
                  ] as ConnectorModel
                ).id
              );
            }
      ...
      const arg: IHistoryChangeArgs | IBlazorHistoryChangeArgs = {
        ...
        source: cloneBlazorObject(source) as NodeModel[],
  ...
  public undo(): void {
    ...
    if (this.undoRedoModule && this.constraints & DiagramConstraints.UndoRedo) {
      ...
      console.warn(
        '[WARNING] :: Module "UndoRedo" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
      );
    }
    this.commandHandler.getBlazorOldValues();
    this.callBlazorModel = true;
  ...
  public pan(
    ...
    focusedPoint?: PointModel,
    isInteractiveZoomPan?: boolean
  ): void {
    const attribute: string[] = this.getZoomingAttribute();
    this.updateBlazorDiagramProperties(attribute);
  ...
  public resetSegments(): void {
    ...
    if (
      this.constraints & DiagramConstraints.LineRouting &&
      ...
      for (let i: number = 0; i < this.connectors.length; i++) {
        ...
        this.connectorPropertyChange(
          connector,
          {} as Connector,
          { segments: connector.segments } as Connector
        );
      }
      this.protectPropertyChange(false);
    }
  ...
  public async triggerEvent(
    eventName: DiagramEvent,
    args: Object
  ): Promise<void | object> {
    if (args) {
      this.updateEventValue(args as IDropEventArgs);
    }
    const eventArgs: void | object = await this.trigger(
      DiagramEvent[`${eventName}`],
      args
    );
    //Removed isBlazor code.
    return eventArgs;
  ...
  private updateEventValue(args: IDropEventArgs): void {
    ...
    if (
      (args as IDropEventArgs).element &&
      element instanceof Selector &&
      element.nodes.length + element.connectors.length === 1
    ) {
      args.element =
        element.nodes.length === 1 ? element.nodes[0] : element.connectors[0];
    }
  }
  ...
  public addNodeToLane(node: NodeModel, swimLane: string, lane: string): void {
    if (this.nameTable[`${swimLane}`]) {
      ...
      if (!this.nameTable[node.id]) {
        node.offsetX =
          swimlaneNode.wrapper.bounds.width + swimlaneNode.wrapper.bounds.x;
        node.offsetY =
          swimlaneNode.wrapper.bounds.height + swimlaneNode.wrapper.bounds.y;
      ...
      if (!(swimlaneNode.shape as SwimLane).phases.length) {
        ...
        for (
          let i: number = 0;
          i < (swimlaneNode.shape as SwimLane).phases.length;
          i++
        ) {
          const laneId: string = swimLane + lane + i;
          if (
            this.nameTable[`${laneId}`] &&
            this.nameTable[`${laneId}`].isLane
          ) {
            ...
            const focusPoint: PointModel = {
              x:
                laneNode.x +
                (laneNode.x -
                  swimlaneNode.wrapper.bounds.x +
                  node.margin.left +
                  node.wrapper.bounds.width / 2),
            ...
            if ((swimlaneNode.shape as SwimLane).orientation === 'Horizontal') {
              ...
              const laneHeaderId: string =
                this.nameTable[`${laneId}`].parentId +
                (swimlaneNode.shape as SwimLane).lanes[0].id +
                '_0_header';
              focusPoint.y =
                laneNode.y +
                (swimlaneNode.wrapper.bounds.y -
                  this.nameTable[`${laneHeaderId}`].wrapper.bounds.height +
                  node.margin.top +
                  node.wrapper.bounds.height / 2);
            }
            if (
              laneId ===
                swimLane +
                  lane +
                  ((swimlaneNode.shape as SwimLane).phases.length - 1) ||
  ...
  public showTooltip(obj: NodeModel | ConnectorModel): void {
    ...
    if (obj && obj.tooltip.openOn === 'Custom') {
      if (obj instanceof Node) {
        ...
        position = {
          x: (obj as ConnectorModel).targetPoint.x,
          y: (obj as ConnectorModel).targetPoint.x,
        };
      }
  ...
  public addChildToGroup(
    ...
    //Removed isBlazor code
    const isHistoryAdded: boolean =
      !(this.diagramActions & DiagramAction.UndoRedo) &&
      !(this.diagramActions & DiagramAction.Group) &&
    ...
    const element: Node | Connector = this.nameTable[child as string]
      ? this.nameTable[child as string]
      : child;
    const childElementToMove: HTMLElement = document.getElementById(
      element.id + '_groupElement'
    );
    const targetGroupElement: HTMLElement = document.getElementById(
      group.id + '_groupElement'
    );
  ...
  public removeChildFromGroup(
    ...
    if (isHistoryAdded) {
      ...
      const entry: HistoryEntry = {
        ...
        changeType: 'Remove',
        undoObject: cloneObject(undoGroup),
        redoObject: cloneObject(group),
        category: 'Internal',
        objectId: id,
        childTable: childTable,
    ...
    const layerNum: number = this.layers.indexOf(
      this.commandHandler.getObjectLayer(element.id)
    );
    const insertBeforeObj: string =
      this.layers[parseInt(layerNum.toString(), 10)].objects[elementZindex + 1];
    const insertBeforeElement: HTMLElement = document.getElementById(
      insertBeforeObj + '_groupElement'
    );
    const childElementToMove: HTMLElement = document.getElementById(
      element.id + '_groupElement'
    );
    ...
    if (targetGroupElement && childElementToMove) {
      if (insertBeforeObj && insertBeforeElement) {
        if (targetGroupElement.contains(insertBeforeElement)) {
          targetGroupElement.insertBefore(
            childElementToMove,
            insertBeforeElement
          );
        } else if (
          targetGroupElement.parentNode.contains(insertBeforeElement) &&
          insertBeforeElement.parentElement === targetGroupElement.parentElement
        ) {
          targetGroupElement.parentNode.insertBefore(
            childElementToMove,
            insertBeforeElement
          );
        } else {
  ...
  public getHistoryStack(isUndoStack: boolean): HistoryEntry[] {
    ...
    const temp: HistoryEntry[] = isUndoStack
  ...
  public getEdges(args: Object): string[] {
    return args['outEdge']
      ? this.nameTable[args['id']].outEdges
      : this.nameTable[args['id']].inEdges;
  }
  /* tslint:enable */
  ...
  public UpdateBlazorDiagramModelCollection(
    obj: Node | Connector,
    copiedObject?: (NodeModel | ConnectorModel)[],
    multiSelectDelete?: (NodeModel | ConnectorModel)[],
    isBlazorGroupUpdate?: boolean
  ): void {
    /* eslint-enable */
    //Removed isBlazor code
  }
  ...
  public UpdateBlazorDiagramModel(
    ...
    objectType: string,
    removalIndex?: number,
    annotationNodeIndex?: number
  ): void {
    //Removed isBlazor code
  }
  // eslint-disable-next-line max-len
  private UpdateBlazorLabelOrPortObjects(
    obj: (ShapeAnnotation | PathAnnotation | PortModel)[],
    objectType: string,
    removalIndex?: number[],
    nodeIndex?: number
  ): void {
  ...
  public add(
    ...
    const propertyChangeValue: boolean = this.isProtectedOnChange;
    this.protectPropertyChange(true);
    ...
    if (obj) {
        ...
        if (getObjectType(args.element) === Connector) {
          if ((args.element as Connector).sourceID) {
            ...
            if (sourceNodee.ports.length > 0) {
              ...
              this.removePortEdges(
                sourceNodee,
                (args.element as Connector).sourcePortID,
                (args.element as Connector).id,
                isOutEdgee
              );
            }
          ...
          if ((args.element as Connector).targetID) {
            this.removeNodeEdges(
              (args.element as Connector).targetID,
              (args.element as Connector).id,
              false
            );
          }
      ...
      const layers: LayerModel = this.activeLayer;
      ...
      if (!args.cancel && layers && !layers.lock) {
        ...
        if (getObjectType(obj) === Connector) {
          ...
          if (
            this.nameTable[newObj.targetID] &&
            (this.nameTable[newObj.targetID].shape as BpmnShape).shape ===
              'TextAnnotation'
          ) {
            (newObj as any).isBpmnAnnotationConnector = true;
            newObj.constraints =
              newObj.constraints & ~ConnectorConstraints.Delete;
          }
          ...
          isTextAnnotationNode =
            (newObj.shape as BpmnShape).shape === 'TextAnnotation';
          if (isTextAnnotationNode && !(obj as any).isTextAnnotationCopied) {
            newObj.inEdges = (obj as Node).inEdges
              ? (obj as Node).inEdges
              : newObj.inEdges;
          }
          ...
          if (isTextAnnotationNode) {
            if (this.bpmnModule) {
              for (
                let i: number = 0;
                i < this.bpmnModule.bpmnTextAnnotationConnector.length;
                i++
              ) {
                if (
          ...
          if (this.bpmnModule) {
            if (
              (newObj.shape as BpmnShape).activity &&
              (newObj.shape as BpmnShape).activity.subProcess.processes &&
          ...
          if (
            ...
            const objects: Object[] = this.spatialSearch.findObjects(
              newObj.wrapper.outerBounds as Rect
            );
            for (let i: number = 0; i < objects.length; i++) {
              const object: Connector = objects[
                parseInt(i.toString(), 10)
              ] as Connector;
          ...
          if (
            (newObj as Node).umlIndex > -1 &&
            (obj as Node).parentId &&
            this.nameTable[(obj as Node).parentId] &&
            this.nameTable[(obj as Node).parentId].shape.type ===
              'UmlClassifier'
          ) {
        ...
        if (
          ...
          !(
            this.diagramActions &
            DiagramAction.PreventCollectionChangeOnDragOver
          )
        ) {
          this.triggerEvent(DiagramEvent.collectionChange, args);
        }
    ...
    if (node.children) {
      for (const j of node.children) {
        if (this.nameTable[`${j}`] && this.nameTable[`${j}`].parentId) {
          const child: HTMLElement = getDiagramElement(
            j + '_groupElement',
            this.element.id
          );
          if (child) {
  ...
  public updateProcesses(node: Node | Connector): void {
    if (
      ...
      for (const j of processes) {
        ...
        let edges: string[] = [];
        edges = edges.concat(
          this.nameTable[`${j}`].outEdges,
          this.nameTable[`${j}`].inEdges
        );
      ...
      for (const j of processes) {
        if (
          (this.nameTable[`${j}`].shape as BpmnShape).activity.subProcess
            .processes &&
          (this.nameTable[`${j}`].shape as BpmnShape).activity.subProcess
            .processes.length
        ) {
  ...
  private spliceConnectorEdges(
    ...
    const node: Node = isSource
      ? this.nameTable[connector.sourceID]
      : this.nameTable[connector.targetID];
    if (node) {
      ...
      for (let j: number = 0; node.ports && j < node.ports.length; j++) {
        ...
        this.removePortEdges(
          node,
          node.ports[parseInt(j.toString(), 10)].id,
          connector.id,
          isInEdge
        );
      }
  ...
  public removeObjectsFromLayer(obj: NodeModel | ConnectorModel): void {
    if ((obj as Node).children) {
      for (let i: number = 0; i < (obj as Node).children.length; i++) {
        const object: NodeModel =
          this.nameTable[(obj as Node).children[parseInt(i.toString(), 10)]];
        if (object) {
          this.removeObjectsFromLayer(object);
        }
      }
    }
    const layer: number = this.layers.indexOf(
      this.commandHandler.getObjectLayer(obj.id)
    );
    const objects: string[] =
      this.layers[parseInt(layer.toString(), 10)].objects;
    ...
    if (objIndex > -1) {
      ...
      delete (this.layers[parseInt(layer.toString(), 10)] as Layer).zIndexTable[
        this.nameTable[obj.id].zIndex
      ];
    }
  }
  /**
  ...
  public removeElements(currentObj: NodeModel | ConnectorModel): void {
    if (
      this.mode === 'SVG' ||
      (this.mode === 'Canvas' && currentObj.shape.type === 'Native')
    ) {
      const removeElement: HTMLElement = getDiagramElement(
        currentObj.id + '_groupElement',
        this.element.id
      );
      ...
      if (object.ports && object.ports.length > 0) {
        for (let i: number = 0; i < object.ports.length; i++) {
          ...
          const removePort: HTMLElement = getDiagramElement(
            object.id + '_' + port.id + '_groupElement',
            this.element.id
          );
          if (removePort) {
    ...
    if (currentObj.wrapper) {
      ...
      if (children) {
        for (let i: number = 0; i < children.length; i++) {
          if (
            children[parseInt(i.toString(), 10)] instanceof
              DiagramNativeElement ||
            (children[parseInt(i.toString(), 10)].id &&
              children[parseInt(i.toString(), 10)].id.indexOf('icon_content') >
                0)
          ) {
            if (
              children[parseInt(i.toString(), 10)].id.indexOf('icon_content') >
              ...
              element = getDiagramElement(
                children[parseInt(i.toString(), 10)].id + '_shape_groupElement',
                this.element.id
              );
              if (element) {
              ...
              element = getDiagramElement(
                children[parseInt(i.toString(), 10)].id + '_rect_groupElement',
                this.element.id
              );
              if (element) {
                element.parentNode.removeChild(element);
            ...
            for (const elementId of this.views) {
              removeElement(
                children[parseInt(i.toString(), 10)].id + '_groupElement',
                elementId
              );
              const nodeIndex: number = this.scroller.removeCollection.indexOf(
                currentObj.id
              );
              this.scroller.removeCollection.splice(nodeIndex, 1);
            }
          } else if (
            children[parseInt(i.toString(), 10)] instanceof DiagramHtmlElement
          ) {
            for (const elementId of this.views) {
              removeElement(currentObj.id + '_html_element', elementId);
              removeElement(
                children[parseInt(i.toString(), 10)].id + '_html_element',
                elementId
              );
              ...
              const parent: NodeModel =
                this.nameTable[(currentObj as Node).parentId];
              //893691: HTML Template nodes are not visible after Zooming with Virtualisation
              if (
                ((this as any).isAngular ||
                  (this as any).isReact ||
                  (this as any).isVue) &&
                ((parent && (parent as Node).isLane) ||
                  this.constraints & DiagramConstraints.Virtualization)
              ) {
                canUpdate = false;
              }
              if (canUpdate) {
  ...
  public remove(obj?: NodeModel | ConnectorModel): void {
    ...
    if (obj) {
      ...
      if (
        obj &&
        (canDelete(obj) || this.diagramActions & DiagramAction.Clear)
      ) {
        args = {
          element: obj,
          cause: this.diagramActions,
        ...
        //Removed isBlazor code
        if (
          !(this.diagramActions & DiagramAction.Clear) &&
          obj.id !== 'helper'
        ) {
          this.triggerEvent(DiagramEvent.collectionChange, args);
        }
        if (!args.cancel) {
          ...
          if (
            ...
            if (!(this.diagramActions & DiagramAction.Clear)) {
              if (
                (obj as Node).children &&
                !(obj as Node).isLane &&
                !(obj as Node).isPhase &&
                (obj as Node).children.length > 0 &&
                this.undoRedoModule &&
          ...
          if ((obj as Node | Connector).parentId) {
            ...
            if (
              this.nameTable[(obj as Node | Connector).parentId] &&
              this.nameTable[(obj as Node | Connector).parentId].shape.type ===
                'UmlClassifier'
            ) {
              this.updateDiagramObject(
                this.nameTable[(obj as Node | Connector).parentId]
              );
              this.updateConnectorEdges(
                this.nameTable[(obj as Node | Connector).parentId]
          ...
          if (currentObj instanceof Node) {
            ...
            if (
              currentObj.isLane ||
              currentObj.isPhase ||
              currentObj.shape.type === 'SwimLane'
            ) {
              const swimLaneNode: NodeModel =
                currentObj.isLane || currentObj.isPhase
                  ? this.nameTable[(currentObj as Node).parentId]
                  : this.nameTable[currentObj.id];

              const grid: GridPanel = swimLaneNode.wrapper
                .children[0] as GridPanel;
          ...
          if (!(currentObj as Node).isPhase) {
            ...
            if (
              ...
              selectedItems[0].id === currentObj.id &&
              (currentObj as Node).parentId
            ) {
      ...
      if (groupAction) {
        ...
        this.commandHandler.getBlazorOldValues();
        this.UpdateBlazorDiagramModelCollection(
          undefined,
          undefined,
          undefined,
          true
    ...
    if (
      !(obj && (canDelete(obj) || this.diagramActions & DiagramAction.Clear))
    ) {
      if (
        !(this.diagramActions & DiagramAction.UndoRedo) &&
        !(this.diagramActions & DiagramAction.PreventHistory) &&
  ...
  public deleteChild(
    node: NodeModel | ConnectorModel | string,
    parentNode?: NodeModel,
    allowChildInSwimlane?: boolean
  ): void {
    let id: string;
    parentNode = parentNode
      ? this.nameTable[parentNode.id]
      ...
      for (let i: number = 0; i < parentNode.children.length; i++) {
        if (parentNode.children[parseInt(i.toString(), 10)] === id) {
          ...
          for (let j: number = 0; j < parentNode.wrapper.children.length; j++) {
            if (
              parentNode.wrapper.children[parseInt(j.toString(), 10)].id === id
            ) {
          ...
          if (!allowChildInSwimlane) {
            ...
            const swimlaneNode: NodeModel = this.getObject(
              (parentNode as Node).parentId
            );
            if (
              swimlaneNode &&
              (swimlaneNode as Node).shape instanceof SwimLane
            ) {
              for (
                let h: number = 0;
                h < ((swimlaneNode as Node).shape as SwimLane).lanes.length;
                h++
              ) {
                const laneId: string[] = (node as Node).parentId.split(
                  (swimlaneNode as Node).id
                );
                if (
                  ((swimlaneNode as Node).shape as SwimLane).lanes[
                    parseInt(h.toString(), 10)
                  ].id === laneId[1].slice(0, -1)
                ) {
                  for (
                    let y: number = 0;
                    y <
                    ...
                    if (
                      ...
                      ((swimlaneNode as Node).shape as SwimLane).lanes[
                        parseInt(h.toString(), 10)
                      ].children[parseInt(y.toString(), 10)].id
                    ) {
                      ((swimlaneNode as Node).shape as SwimLane).lanes[
                        parseInt(h.toString(), 10)
                      ].children.splice(y, 1);
  ...
  public addChild(
    ...
    if (parentNode.children) {
      ...
      if (
        id &&
        (!(child as Node).umlIndex || (child as Node).umlIndex === -1)
      ) {
        ...
        if (index) {
          ...
          if (swimlane && (swimlane as Node).shape instanceof SwimLane) {
            for (
              let h: number = 0;
              h < ((swimlane as Node).shape as SwimLane).lanes.length;
              h++
            ) {
              const laneId: string[] = (childNode as Node).parentId.split(
                (node as Node).parentId
              );
              if (
                ((swimlane as Node).shape as SwimLane).lanes[
                  parseInt(h.toString(), 10)
                ].id === laneId[1].slice(0, -1)
              ) {
                ...
                for (
                  ...
                  if (
                    ((swimlane as Node).shape as SwimLane).lanes[
                      parseInt(h.toString(), 10)
                    ].children[parseInt(i.toString(), 10)].id === childNode.id
                  ) {
        ...
        if (
          ...
          ((this.nameTable[(parentNode as Node).parentId] as Node) &&
            (this.nameTable[(parentNode as Node).parentId] as Node).shape
              .type !== 'SwimLane')
        ) {
      ...
      if (
        ...
        for (let i: number = 0; i < parentNode.children.length; i++) {
          if (parentNode.children[parseInt(i.toString(), 10)] === id) {
            ...
            for (
              let j: number = 0;
              j < parentNode.wrapper.children.length;
              j++
            ) {
              if (
                parentNode.wrapper.children[parseInt(j.toString(), 10)] ===
                childNode.wrapper
              ) {
                parentNode.wrapper.children.splice(j, 1);
              }
  ...
  private startEditCommad(): void {
    ...
    const node: NodeModel = this.selectedItems.nodes[0]
      ? this.selectedItems.nodes[0]
      : undefined;
    if (node && node instanceof Node) {
      if ((node as Node).isLane && (node as Node).parentId) {
        ...
        const currentParentId: string = canvasId.substring(
          0,
          canvasId.length - 1
        );
        for (let i: number = 0; i < lanes.length; i++) {
          if (
            node.isLane &&
            currentParentId === lanes[parseInt(i.toString(), 10)].id
          ) {
            laneHeader =
              this.nameTable[
                (lanes[parseInt(i.toString(), 10)] as LaneModel).header.id
              ];
          }
        }
      } else if (
        node.shape.type === 'SwimLane' &&
        (node.shape as SwimLaneModel).header &&
        (node.shape as SwimLane).hasHeader
      ) {
        const id: string = (node.wrapper.children[0] as GridPanel).rows[0]
  ...
  public startTextEdit(node?: NodeModel | ConnectorModel, id?: string): void {
    if ((!canZoomPan(this) && !canMultiSelect(this)) || canSingleSelect(this)) {
      ...
      const transform: TransformFactor = this.scroller.transform;
      const scale: number = canZoomTextEdit(this) ? transform.scale : 1;
      const minWidth: number = 90;
      let text: string;
      let bounds: Size;
      ...
      if (!node) {
        node = this.selectedItems.nodes[0]
          ? this.selectedItems.nodes[0]
      ...
      if (node) {
        ...
        if (!textWrapper) {
          ...
          if (
            !id &&
            ((node.shape.type !== 'Text' && node.annotations.length > 0) ||
        ...
        if (
          node &&
          textWrapper &&
          !(textWrapper instanceof DiagramHtmlElement) &&
          (!enableReadOnly(textWrapper, node) || bpmnAnnotation)
        ) {
          const style: TextStyleModel = textWrapper.style as TextStyleModel;
          let maxWidth: number;
          maxWidth =
            textWrapper.bounds.width < node.wrapper.bounds.width
              ? textWrapper.bounds.width
              : node.wrapper.bounds.width;
          maxWidth = minWidth > maxWidth ? minWidth : maxWidth;
          let textEditing: HTMLElement = document.getElementById(
            this.element.id + '_editTextBoxDiv'
          );
          let textArea: HTMLTextAreaElement = document.getElementById(
            this.element.id + '_editBox'
          ) as HTMLTextAreaElement;
          text = textArea
            ? textArea.value
            : (textWrapper as TextElement).content;
            ...
            if (node.shape.type !== 'Text') {
              bounds = new Size(
                (
                  findAnnotation(
                    node,
                    textWrapper.id.split(node.id + '_')[1]
                  ) as PathAnnotationModel | ShapeAnnotationModel
                ).width || 50,
          ...
          if (
            (node as Node).parentId &&
            this.nameTable[(node as Node).parentId].shape.type ===
              'UmlClassifier'
          ) {
            bounds.width = node.wrapper.bounds.width - 20;
            x =
              (node.wrapper.bounds.center.x + transform.tx) * transform.scale -
              (bounds.width / 2) * scale -
              2.5;
            y =
              (node.wrapper.bounds.center.y + transform.ty) * transform.scale -
            ...
            x =
              (textWrapper.bounds.center.x + transform.tx) * transform.scale -
              (bounds.width / 2) * scale -
              2.5;
            y =
              (textWrapper.bounds.center.y + transform.ty) * transform.scale -
              (bounds.height / 2) * scale -
              3;
          }
          attributes = {
          ...
          attributes = {
            ...
            style:
              ...
              'px;text-align:' +
              (
                textWrapper.style as TextStyleModel
              ).textAlign.toLocaleLowerCase() +
              ';',
  ...
  private updateConnectorfixedUserHandles(connector: Connector): void {
    if (connector.fixedUserHandles.length) {
      ...
      for (const fixedUserHandle of connector.fixedUserHandles) {
        fixedUserHandleWrapper = this.getWrapper(
          connector.wrapper,
          fixedUserHandle.id
        ) as DiagramElement;
  ...
  private updateNodeExpand(node: Node, visibility: boolean): void {
    for (let i: number = 0; i < node.outEdges.length; i++) {
      const connector: Connector =
        this.nameTable[node.outEdges[parseInt(i.toString(), 10)]];
  ...
  private updateConnectorAnnotation(connector: Connector): void {
    if (connector.annotations.length) {
      ...
      for (const annotation of connector.annotations) {
        annotationWrapper = this.getWrapper(
          connector.wrapper,
          annotation.id
        ) as TextElement;
        connector.updateAnnotation(
  ...
  public doLayout(): ILayout | boolean {
    ...
    let layout: ILayout;
    const canDoOverlap: boolean =
      this.layout.type === 'ComplexHierarchicalTree' ||
      this.layout.type === 'HierarchicalTree';
    const propChange: boolean = this.isProtectedOnChange;
    this.protectPropertyChange(true);
    const nodes: INode[] = this.removeChildrenFromLayout(this.nodes as INode[]);
    const canEnableRouting: boolean =
      this.layout.enableRouting &&
      this.layout.type === 'ComplexHierarchicalTree';
    const viewPort: PointModel = {
    ...
    if (this.layout.type !== 'None') {
      ...
      if (this.organizationalChartModule) {
        layout = this.organizationalChartModule.updateLayout(
          ...
          viewPort,
          this.dataSourceSettings.id,
          this.diagramActions
        );
        update = true;
        if (this.canDistribute(canEnableRouting, canDoOverlap)) {
          this.lineDistributionModule.initLineDistribution(
            this.layout as Layout,
            this
          );
        }
        if (
          this.layoutAnimateModule &&
          layout.rootNode &&
          !this.diagramActions
        ) {
          this.updateNodeExpand(
            layout.rootNode as Node,
            layout.rootNode.isExpanded
          );
        }
        ...
        this.symmetricalLayoutModule.updateLayout(
          nodes as IGraphObject[],
          this.connectors as IGraphObject[],
          this.symmetricalLayoutModule,
          this.nameTable,
          this.layout as Layout,
          viewPort
        );
        ...
        const nodes: INode[] =
          this.complexHierarchicalTreeModule.getLayoutNodesCollection(
            this.nodes as INode[]
          );
        if (nodes.length > 0) {
          // eslint-disable-next-line max-len
        ...
        console.warn(
          '[WARNING] :: Module ' +
            moduleName +
            ' is not available in Diagram component! You either misspelled the module name or forgot to load it.'
        );
      }
      if (update) {
        ...
        for (const obj of updatedNodes) {
          ...
          this.nodePropertyChange(
            node,
            {} as Node,
            { offsetX: node.offsetX, offsetY: node.offsetY } as Node,
            true
          );
          this.preventNodesUpdate = false;
          node.wrapper.measure(
            new Size(node.wrapper.width, node.wrapper.height)
          );
          node.wrapper.arrange(node.wrapper.desiredSize);
          ...
          if (node.inEdges.length > 0) {
            for (let j: number = 0; j < node.inEdges.length; j++) {
              const connector: Connector =
                this.nameTable[node.inEdges[parseInt(j.toString(), 10)]];
          ...
          if (node.outEdges.length > 0) {
            for (let k: number = 0; k < node.outEdges.length; k++) {
              const connection: Connector =
                this.nameTable[node.outEdges[parseInt(k.toString(), 10)]];
        ...
        for (const conn of Object.keys(connectors)) {
          if (canEnableRouting) {
            this.lineDistributionModule.resetConnectorSegments(
              this.nameTable[`${conn}`]
            );
          }
          ...
          if (canEnableRouting) {
            this.lineDistributionModule.resetRoutingSegments(
              connector,
              this,
              points
            );
          ...
          if (
            ...
            (connector.shape as BpmnFlowModel).sequence === 'Default' &&
            (connector.shape as BpmnFlowModel).flow === 'Sequence'
          ) {
        ...
        if (
          canEnableRouting ||
          ((this.layout as Layout).connectionPointOrigin === 'DifferentPoint' &&
            this.lineDistributionModule &&
            canDoOverlap)
        ) {
          this.lineDistributionModule.distributeLines(
            this.layout as Layout,
            this
          );
        }
  ...
  private refreshRoutingConnectors() {
    ...
    if ((this as any).routingConnectors) {
      for (let i = 0; i < (this as any).routingConnectors.length; i++) {
        const connector = (this as any).routingConnectors[
          parseInt(i.toString(), 10)
        ];
  ...
  private refreshFlowChartConnectors() {
    if (this.flowchartLayoutModule) {
      this.flowchartLayoutModule.reRouteFlowChartConnectors(
        (this.layout as any).flowChartData,
        this
      );
      for (let i: number = 0; i < this.connectors.length; i++) {
        const connector: ConnectorModel =
  ...
  private canDistribute(canEnableRouting: boolean, canDoOverlap: boolean) {
    if (
      (canEnableRouting && this.lineDistributionModule) ||
      ((this.layout as Layout).connectionPointOrigin === 'DifferentPoint' &&
        this.lineDistributionModule &&
        canDoOverlap) ||
      (this.layout.arrangement === 'Linear' && this.lineDistributionModule)
    ) {
      return true;
    } else {
      if (
        ...
        console.warn(
          '[WARNING] :: Module "LineDistribution" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
        );
      }
      return false;
    }
  ...
  public saveDiagram(): string {
    ...
    let node: Node;
    let grid: GridPanel;
    let childTable: DiagramElement[];
    let child: Canvas;
    const gridChild: string = 'childTable';
    for (let i: number = 0; i < this.nodes.length; i++) {
  ...
  public loadDiagram(data: string, isEJ1Data?: boolean): Object {
    if (isEJ1Data && this.ej1SerializationModule) {
      ...
      console.warn(
        '[WARNING] :: Module "Ej1Serialization" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
      );
    }
  ...
  private saveMindmapDiagramInMermaidFormat(): string {
    ...
    if (this.layout && this.layout.type === 'MindMap') {
      ...
      if (this.nodes.length > 0) {
        const rootNode: NodeModel = this.nodes.filter(
          (node: NodeModel) => (node as Node).inEdges.length === 0
        )[0];
        ...
        dataSourceCollection = dataSourceCollection.filter(
  ...
  private updateTextDataSource(
    ...
    while (count < outEdges.length) {
      const connector: ConnectorModel = this.getObject(
        outEdges[parseInt(count.toString(), 10)]
      ) as Connector;
  ...
  private convertMindmapToMermaid(node: NodeModel, level: number): string {
    ...
    const annotationContent: string =
  ...
  private saveFlowDiagramInMermaidFormat(): string {
    ...
    const nodeLabels: { [key: string]: string } = graph.nodes.reduce(
      (accumulator: { [key: string]: string }, node: NodeModel) => {
        accumulator[node.id] = node.annotations.length
    ...
    graph.edges.forEach((edge: ConnectorModel) => {
      ...
      const condition: string =
        edge.annotations[0] && edge.annotations[0].content !== ''
          ? '|' + edge.annotations[0].content + '|'
          : '';
  ...
  private getNodeShape(node: NodeModel): string {
    ...
    if (shape) {
      ...
      if (
        data ===
        'M 0 0 A 1 1 0 0 0 7 0 A 1 1 0 0 0 0 0 M -1 0 A 1 1 0 0 0 8 0 A 1 1 0 0 0 -1 0'
      ) {
        return '(((' + label + ')))';
      } else if (data === 'M 0 0 L 1 -1 L 5 -1 L 6 0 L 0 0') {
        return '[/' + label + '\\]';
      } else if (
        data === 'M 0 1 L 0 6 C 2 7 4 7 6 6 L 6 1 C 5 0 1 0 0 1 C 1 2 5 2 6 1'
      ) {
        return '[(' + label + ')]';
  ...
  public loadDiagramFromMermaid(data: string): void {
    if (
      this.layout &&
      this.layout.type === 'Flowchart' &&
      this.flowchartLayoutModule
    ) {
      //Task 896394: To load the mermaid data as flowchart
      this.convertMermaidToFlowChart(data);
    } else if (
      this.layout &&
      this.layout.type === 'MindMap' &&
      this.mindMapChartModule
    ) {
      //Task 900266: To load the mermaid data as mindmap
      this.convertMermaidToMindmap(data);
    } else {
      console.warn(
        '[WARNING] :: Module "FlowchartLayout" or "MindMap" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
      );
    }
  }
  /**
  ...
  private convertMermaidToMindmap(data: string): void {
    if (
      ...
      let previousItem: HierarchyData = {
        text: '',
        children: [],
        currentLevel: 0,
        branch: 'Left',
      ...
      if (dataSource.length > 0) {
        for (let index: number = 0; index < dataSource.length; index++) {
          ...
          canCreateMindMap =
            !haveBackticks && !canCreateMindMap
              ? leadingWhiteSpace === 0 && index > 0
              : canCreateMindMap;
          ...
          if (
            word.length > 0 &&
            ((/\s/.test(word[0]) && index > 0) || leadingWhiteSpace === 0)
          ) {
            const spaceIndex: number = spaceAndItsLevels.findIndex(
              (space: SpaceLevel) =>
            ...
            if (spaceIndex !== -1) {
              for (let i: number = spaceAndItsLevels.length - 1; i >= 0; i--) {
                const currentSpace: SpaceLevel =
                  spaceAndItsLevels[parseInt(i.toString(), 10)];
                ...
                if (currentKey > leadingWhiteSpace) {
                  ...
                  spaceAndItsLevels.push({
                    space: leadingWhiteSpace.toString(),
              ...
              if (spaceAndItsLevels.length === 0) {
                ...
                for (
                  let i: number = spaceAndItsLevels.length - 1;
                  i >= 0;
                  i--
                ) {
                  const currentElement: SpaceLevel =
                    spaceAndItsLevels[parseInt(i.toString(), 10)];
                  ...
                  if (currentKey > leadingWhiteSpace) {
                    ...
                    const lastElement: SpaceLevel =
                      spaceAndItsLevels[spaceAndItsLevels.length - 1];
                    spaceAndItsLevels.push({
                      space: leadingWhiteSpace.toString(),
    ...
    for (let i: number = 1; i < lines.length; i++) {
      ...
      if (line !== '') {
        if (line.startsWith('style')) {
          ...
          const data: FlowChartData[] = this.getNodeData(
            parts,
            dataCollection,
            lineSplit[2]
          );
          ...
          data
            .filter(
              (flowData: FlowChartData) =>
                flowData.parentId && flowData.parentId.length === 0
            )
            .forEach(function (node: FlowChartData): void {
              node.parentId = null;
            });
  ...
  private createFlowChart(dataCollection: FlowChartData[]): void {
    ...
    for (let c: number = 0; c < dataCollection.length; c++) {
      ...
      const connectorStyle: ConnectorStyle = getConnectorArrowType(
        data
      ) as ConnectorStyle;
        ...
        for (let i: number = 0; i < data.parentId.length; i++) {
          const connector: ConnectorModel = {
            ...
            annotations: [
              {
                content: data.label
                  ? data.label[parseInt(i.toString(), 10)]
                  : '',
              },
            ],
            style: {
              strokeWidth: connectorStyle.strokeWidth
                ? connectorStyle.strokeWidth
                : 1,
              strokeDashArray: connectorStyle.strokeDashArray
                ? connectorStyle.strokeDashArray
                : '',
              opacity:
                connectorStyle.opacity !== undefined
                  ? connectorStyle.opacity
                  : 1,
            },
            targetDecorator: {
              shape: connectorStyle.targetDecorator as DecoratorShapes,
            },
          };
        ...
        const connector: ConnectorModel = {
          ...
          style: {
            ...
            strokeDashArray: connectorStyle.strokeDashArray
              ...
              : '',
            opacity:
              connectorStyle.opacity !== undefined ? connectorStyle.opacity : 1,
          },
          targetDecorator: {
            shape: connectorStyle.targetDecorator as DecoratorShapes,
          },
  ...
  private parseStyle(line: string, dataCollection: FlowChartData[]): void {
    ...
    if (line.startsWith('style')) {
      ...
      if (match) {
        ...
        const matchData: FlowChartData[] = dataCollection.filter(
          (x: FlowChartData) => x.id === data.id
        );
  ...
  private getNodeData(
    ...
    for (let i: number = 0; i < lines.length; i++) {
      ...
      const exsist: FlowChartData = dataCollection.find(
        (data: FlowChartData) => data.id === id
      );
      if (!exsist) {
        const labelShape: string = text.length > 1 ? text[1] : text[0];
        const shape: BasicShapeModel | FlowShapeModel | PathModel =
          this.getShape(labelShape);
        const label: string = labelShape.replace(
          /[\\[\]\\(\\)\\{\\}\\{\\}\\/>]/g,
          ''
        );
    ...
    if (dataArray.length) {
      ...
      if (lastItem.id !== firstId) {
        ...
        const data: FlowChartData = dataCollection.find(
          (data: FlowChartData) => data.id === secondId
        );
        if (data.parentId) {
      ...
      const [filteredData]: FlowChartData[] = dataCollection.filter(
        (flowData: FlowChartData) => flowData.id === secondId
      );
      ...
      (filteredData.label as string[])[filteredData.parentId.length - 1] =
        connectorLabel;
    }
    if (arrowType) {
      const filteredData: FlowChartData = dataCollection.filter(function (
        flowData: FlowChartData
      ): boolean {
  ...
  private getShape(text: string): BasicShapeModel | FlowShapeModel | PathModel {
    ...
    if (text.startsWith('(((')) {
      shape = {
        type: 'Path',
        data: 'M 0 0 A 1 1 0 0 0 7 0 A 1 1 0 0 0 0 0 M -1 0 A 1 1 0 0 0 8 0 A 1 1 0 0 0 -1 0',
      };
      ...
      shape = {
        type: 'Path',
        data: 'M 0 0 L 2 -2 L 11 -2 L 13 0 L 11 2 L 2 2 L 0 0',
      };
    } else if (text.startsWith('{')) {
  ...
  public getDiagramContent(styleSheets?: StyleSheetList): string {
    if (this.printandExportModule) {
      const data: string | SVGElement =
        this.printandExportModule.getDiagramContent(styleSheets);
      return data;
    } else {
      console.warn(
        '[WARNING] :: Module "PrintAndExport" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
      );
      return '';
    }
  }
  ...
  public setStackLimit(stackLimit: number): void {
    if (this.undoRedoModule && stackLimit) {
      ...
      this.undoRedoModule.applyLimit(
        this.historyManager.currentEntry,
        stackLimit,
        this,
        true
      );
  ...
  public getDiagramBounds(): Rect {
    if (this.printandExportModule) {
      ...
      bounds.width =
        bounds.width > this.scrollSettings.viewPortWidth
          ? bounds.width + (bounds.x > 0 ? bounds.x : 0)
          : this.scrollSettings.viewPortWidth;
      bounds.height =
        bounds.height > this.scrollSettings.viewPortHeight
  ...
  public exportDiagram(options: IExportOptions): string | SVGElement {
    if (this.printandExportModule) {
      const data: string | SVGElement =
        this.printandExportModule.exportDiagram(options);
      return data;
    } else {
      console.warn(
        '[WARNING] :: Module "PrintAndExport" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
  ...
  public addPorts(
    ...
    for (let i: number = 0; i < ports.length; i++) {
      ...
      exist = obj.ports.some(
        (port: Port) => port.id === ports[parseInt(i.toString(), 10)].id
      );
      if (exist) {
        continue;
      }
      ...
      if (obj instanceof Node) {
        newObj = new PointPort(
          obj,
          'ports',
          ports[parseInt(i.toString(), 10)],
          true
        );
      ...
      if ((obj as NodeModel).children) {
        ...
        for (let k: number = 0; k < container.children.length; k++) {
          const wrapper: DiagramElement =
            container.children[parseInt(k.toString(), 10)];
          if (wrapper.id.match(container.id)) {
            (obj as Node).initPort(
              this.getDescription,
        ...
        if (obj instanceof Node) {
          canvas.children.push(
            (obj as Node).initPortWrapper(
              obj.ports[obj.ports.length - 1] as Port
            )
          );
        } else if (obj instanceof Connector) {
          let points: PointModel[] =
            obj.type === 'Bezier'
              ? obj.intermediatePoints
              : obj.getConnectorPoints(obj.type);
          points = obj.clipDecorators(obj, points);
          const bounds: Rect = Rect.toBounds(points);
          // eslint-disable-next-line max-len
          canvas.children.push(
            (obj as Connector).initPort(
              obj.ports[obj.ports.length - 1] as Port,
              points,
              bounds,
              undefined
  ...
  public addLabels(
    ...
    obj = this.nameTable[obj.id] || obj;
    const canvas: Container = obj.wrapper;
    let newObj: ShapeAnnotation | PathAnnotation;
    if (labels.length > 1) {
      this.startGroupAction();
    }
    for (let i: number = 0; i < labels.length; i++) {
      if (obj instanceof Node) {
        newObj = new ShapeAnnotation(
          obj,
          'annotations',
          labels[parseInt(i.toString(), 10)],
          true
        );
        ...
        if ((obj as Node).children) {
          ...
          for (let i: number = 0; i < node.wrapper.children.length; i++) {
            if (
              node.wrapper.children[parseInt(i.toString(), 10)].id ===
              node.id + 'group_container'
            ) {
              const container: Container = node.wrapper.children[
                parseInt(i.toString(), 10)
              ] as Container;
              container.children.push(
                obj.initAnnotationWrapper(
                  obj.annotations[obj.annotations.length - 1] as Annotation,
                  this.element.id
                )
              );
            }
        ...
        const bounds: Rect = new Rect(
          ...
          segment.offsetY - segment.height / 2,
          segment.width,
          segment.height
        );
        canvas.children.push(
          obj.getAnnotationElement(
  ...
  public addChildToUmlNode(
    ...
    if (
      (node.shape as UmlClassifierShapeModel).classifier === 'Class' ||
      (node.shape as UmlClassifierShapeModel).classifier === 'Interface'
    ) {
      if (umlChildType === 'Method') {
        ...
        if ((node.shape as UmlClassifierShapeModel).classifier === 'Class') {
          (node.shape as UmlClassifierShapeModel).classShape.methods.push(
            method
          );
          ...
          (node.shape as UmlClassifierShapeModel).interfaceShape.methods.push(
            method
          );
        ...
        if ((node.shape as UmlClassifierShapeModel).classifier === 'Class') {
          (node.shape as UmlClassifierShapeModel).classShape.attributes.push(
            attribute
          );
          classifier = (node.shape as UmlClassifierShapeModel).classShape;
        } else if (
          ...
          (
            node.shape as UmlClassifierShapeModel
          ).interfaceShape.attributes.push(attribute);
          classifier = (node.shape as UmlClassifierShapeModel).interfaceShape;
        }
      ...
      (node.shape as UmlClassifierShapeModel).classifier === 'Enumeration' &&
      umlChildType === 'Member'
    ) {
      member = new UmlEnumerationMember(
        node as UmlEnumerationMember,
        '',
        child
      );
      (node.shape as UmlClassifierShapeModel).enumerationShape.members.push(
        member
      );
  ...
  public editSegment(editOptions: IEditSegmentOptions): void {
    if (editOptions.connector.type === 'Straight') {
      ...
      const mode: string =
        editOptions && editOptions.SegmentEditing
          ? editOptions.SegmentEditing
          : 'Toggle';
      ...
      for (let i: number = 0; i < connector.segments.length; i++) {
        const segment: StraightSegment = connector.segments[
          parseInt(i.toString(), 10)
        ] as StraightSegment;
      ...
      if (connector.type === 'Straight' && (allowEdit || mode === 'Toggle')) {
        this.connectorEditingToolModule.addOrRemoveSegment(
          connector,
          point,
          this.commandHandler
        );
  ...
  private removelabelExtension(
    ...
    for (let i: number = 0; i < (wrapper as Container).children.length; i++) {
      const canvas: DiagramElement = (wrapper as Container).children[
        parseInt(i.toString(), 10)
      ];
      if (
        canvas instanceof TextElement ||
        canvas instanceof DiagramHtmlElement
      ) {
        if (
          canvas.id.match('_' + labels[parseInt(j.toString(), 10)].id + '$')
        ) {
          for (let k: number = 0; k < obj.annotations.length; k++) {
            if (
              canvas.id.match(
                '_' + obj.annotations[parseInt(k.toString(), 10)].id + '$'
              )
            ) {
              if (!(this.diagramActions & DiagramAction.UndoRedo)) {
                const entry: HistoryEntry = {
                  type: 'LabelCollectionChanged',
                  changeType: 'Remove',
                  undoObject: cloneObject(
                    obj.annotations[parseInt(k.toString(), 10)]
                  ),
                  redoObject: cloneObject(obj),
                  category: 'Internal',
                };
          ...
          if (this.mode === 'SVG') {
            let element: HTMLElement = getDiagramElement(
              canvas.id,
              this.element.id
            );
            if (element) {
              const element: HTMLElement = getDiagramElement(
                canvas.id,
                this.element.id
              );
              element.parentNode.removeChild(element);
            }
            const textElement: HTMLElement = getDiagramElement(
              canvas.id + '_text',
              this.element.id
            );
            if (textElement) {
            ...
            const htmlElement: HTMLElement = getDiagramElement(
              canvas.id + '_html_element',
              this.element.id
            );
            if (htmlElement) {
  ...
  public removeLabels(
    ...
    for (let j: number = labels.length - 1; j >= 0; j--) {
      if (
        (obj as NodeModel).children &&
        (obj as NodeModel).children.length > 0
      ) {
        ...
        const groupWrapper: any = obj.wrapper.children.filter(
          (wrapper: any) => wrapper.id === obj.id + 'group_container'
        );
      ...
      if (canvas instanceof PathElement) {
        if (canvas.id.match('_' + ports[parseInt(j.toString(), 10)].id + '$')) {
          for (let k: number = 0; k < obj.ports.length; k++) {
            if (
              canvas.id.match(
                '_' + obj.ports[parseInt(k.toString(), 10)].id + '$'
              )
            ) {
              if (!(this.diagramActions & DiagramAction.UndoRedo)) {
                const entry: HistoryEntry = {
                  ...
                  changeType: 'Remove',
                  undoObject: cloneObject(
                    obj.ports[parseInt(k.toString(), 10)]
                  ),
                  redoObject: cloneObject(obj),
                  category: 'Internal',
                };
    ...
    for (let j: number = ports.length - 1; j >= 0; j--) {
      if (
        ...
        for (let k: number = 0; k < obj.wrapper.children.length; k++) {
          ...
          const wrapper: DiagramElement =
            obj.wrapper.children[parseInt(k.toString(), 10)];
          if (wrapper.id.match(obj.wrapper.id)) {
            this.removePortsExtenion(
              obj,
              ports,
              j,
              obj.wrapper.children[parseInt(k.toString(), 10)]
  ...
  private initDiagram(): void {
    ...
    setAttributeHtml(this.element, {
      style:
        'width:' +
        this.getSizeValue(this.width) +
        '; height:' +
        this.getSizeValue(this.height) +
  ...
  private renderBackgroundLayer(bounds: ClientRect, commonStyle: string): void {
    const bgLayer: SVGElement = this.createSvg(
      this.element.id + '_backgroundLayer_svg',
      bounds.width,
      bounds.height
    );
  ...
  private renderGridLayer(bounds: ClientRect, commonStyle: string): void {
    const svgGridSvg: SVGElement = this.createSvg(
      this.element.id + '_gridline_svg',
      bounds.width,
      bounds.height
    );
    svgGridSvg.setAttribute('class', 'e-grid-layer');
    const svgGrid: SVGElement = createSvgElement('g', {
  ...
  private renderDiagramLayer(bounds: ClientRect, commonStyle: string): void {
    ...
    if (this.mode === 'SVG') {
      const diagramSvg: SVGElement = this.createSvg(
        this.element.id + '_diagramLayer_svg',
        bounds.width,
        bounds.height
      );
      diagramSvg.style['pointer-events'] = 'none';
      diagramSvg.setAttribute('class', 'e-diagram-layer');
      const diagramLayer: SVGElement = createSvgElement('g', {
      ...
      this.diagramLayer = CanvasRenderer.createCanvas(
        this.element.id + '_diagram',
        bounds.width,
        bounds.height
      );
      applyStyleAgainstCsp(
  ...
  private initLayers(): void {
    const commonStyle: string =
      'position:absolute;top:0px;left:0px;overflow:hidden;pointer-events:none;';
    ...
    if (window[`${measureWindowElement}`]) {
      ...
      const measureElements: HTMLElement =
        document.getElementById('measureElement');
  ...
  private renderAdornerLayer(bounds: ClientRect, commonStyle: string): void {
    ...
    const svgAdornerSvg: SVGElement = this.createSvg(
      this.element.id + '_diagramAdorner_svg',
      bounds.width,
      bounds.height
    );
    ...
    const svgSelector: SVGElement = createSvgElement('g', {
  ...
  private renderPortsExpandLayer(
    ...
    const svgPortsSvg: SVGElement = this.createSvg(
      this.element.id + '_diagramPorts_svg',
      bounds.width,
      bounds.height
    );
  ...
  private renderNativeLayer(bounds: ClientRect, commonStyle: string): void {
    const nativeLayerSvg: SVGElement = this.createSvg(
      this.element.id + '_nativeLayer_svg',
      bounds.width,
      bounds.height
    );
    const nativeLayer: SVGElement = createSvgElement('g', {
  ...
  public createSvg(
    ...
    const svgObj: SVGElement = document.createElementNS(
  ...
  public initLayerObjects(): void {
    ...
    for (const layer of this.layers) {
        ...
        if (tempTabel[`${obj}`]) {
          if (!(tempTabel[`${obj}`] instanceof Connector)) {
            if ((tempTabel[`${obj}`] as Node).children) {
              ...
              tempTabel[`${obj}`].shape instanceof BpmnShape &&
              (tempTabel[`${obj}`].shape as BpmnShape).activity.subProcess
                .processes &&
              (tempTabel[`${obj}`].shape as BpmnShape).activity.subProcess
                .processes.length > 0
            ) {
      ...
      if (this.bpmnModule) {
        for (const obj of (this.bpmnModule as any)
          .bpmnTextAnnotationConnector) {
    ...
    for (const connector of connectors) {
      const layer: LayerModel = this.commandHandler.getObjectLayer(
        connector.id
      );
    ...
    for (i = 0; i < parents.length; i++) {
      ...
      if (childNode && childNode.children.length) {
        for (j = 0; j < childNode.children.length; j++) {
          ...
          if (
            parents.indexOf(node) > -1 &&
            newList.indexOf(node) === -1 &&
  ...
  private addToLayer(
    ...
    if (
      (obj instanceof Node || obj instanceof Connector) &&
      (obj.shape.type !== 'SwimLane' ||
        ((obj as Node).children && (obj as Node).children.length > 0))
    ) {
      if (obj.parentId) {
      ...
      if (obj instanceof Connector && obj.sourceID && obj.targetID) {
        ...
        if (
          this.activeLayer.objects.indexOf(obj.sourceID) !== -1 &&
  ...
  private updateScrollSettings(newProp: DiagramModel): void {
    const hPan: number =
      -this.scroller.horizontalOffset +
        newProp.scrollSettings.horizontalOffset || 0;
    const vPan: number =
      -this.scroller.verticalOffset + newProp.scrollSettings.verticalOffset ||
      0;
  ...
  private initData(): void {
    const dataSourceSettings: DataManager =
      this.dataSourceSettings.dataManager || this.dataSourceSettings.dataSource;
    const adapter: string = 'adaptorName';
    if (
      this.dataBindingModule &&
      !(this.realActions & RealAction.PreventDataInit)
    ) {
      if (
        dataSourceSettings &&
        this.dataSourceSettings.connectionDataSource.dataManager
      ) {
        ...
        (dataSourceSettings.dataSource.url ||
          (dataSourceSettings[`${adapter}`] === 'BlazorAdaptor' &&
            !dataSourceSettings.dataSource.url))
      ) {
      ...
      console.warn(
        '[WARNING] :: Module "DataBinding" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
      );
    }
  }

  private generateData(
    dataSource: DataSourceModel | DataManager,
    isNode: boolean
    ...
    for (i = 0; i < (dataSource as object[]).length; i++) {
      ...
      const node: Connector | Node = isNode
        ? this.makeData(row as object[], true)
        : this.makeData(row as object[], false);
  ...
  private makeData(row: object[], isNode: boolean): Node | Connector {
    let i: number;
    const fields: IFields = isNode
      ? (this.dataSourceSettings as IFields)
      : (this.dataSourceSettings.connectionDataSource as IFields);
  ...
  private setZIndex(layer: LayerModel, obj: NodeModel | ConnectorModel): void {
    ...
    if (obj.zIndex === Number.MIN_VALUE) {
      ...
      if (currentLayer.zIndexTable[parseInt(index.toString(), 10)]) {
        const tabelLength: number = Object.keys(
          currentLayer.zIndexTable
        ).length;
        let j: number = 0;
        for (let i: number = 0; i < tabelLength; i++) {
          if (i === index) {
            for (let j: number = tabelLength; j > index; j--) {
              currentLayer.zIndexTable[parseInt(j.toString(), 10)] =
                currentLayer.zIndexTable[j - 1];
              if (
                this.nameTable[
                  currentLayer.zIndexTable[parseInt(j.toString(), 10)]
                ]
  ...
  private initializeDiagramLayers(): void {
    ...
    for (let i: number = 0; i < this.layers.length; i++) {
      if (this.layers[parseInt(i.toString(), 10)].zIndex !== -1) {
        ...
        this.layers[parseInt(i.toString(), 10)] =
          this.layers[this.layers[parseInt(i.toString(), 10)].zIndex];
    ...
    for (const layer of this.layers) {
      layer.zIndex =
        layer.zIndex !== -1 ? layer.zIndex : this.layers.indexOf(layer);
      this.layerZIndexTable[layer.zIndex] = layer.id;
    }
    for (let i: number = 0; i < this.layers.length; i++) {
      for (let j: number = i + 1; j < this.layers.length; j++) {
        if (
          this.layers[parseInt(i.toString(), 10)].zIndex >
          this.layers[parseInt(j.toString(), 10)].zIndex
        ) {
          const temp: LayerModel = this.layers[parseInt(i.toString(), 10)];
          this.layers[parseInt(i.toString(), 10)] =
            this.layers[parseInt(j.toString(), 10)];
  ...
  private initObjectExtend(
    ...
    if (independentObj) {
      const checkBoundaryConstraints: boolean =
        this.commandHandler.checkBoundaryConstraints(
          undefined,
          undefined,
          obj.wrapper.bounds,
      ...
      if ((obj as NodeModel).shape.type === 'SwimLane') {
        for (
          let i: number = 0,
            a: string[] = Object.keys((layer as Layer).zIndexTable);
          i < a.length;
          i++
        ) {
          if (
            (layer as Layer).zIndexTable[a[parseInt(i.toString(), 10)]] &&
            (layer as Layer).zIndexTable[a[parseInt(i.toString(), 10)]] ===
              (obj as NodeModel).id
          ) {
      ...
      if (!checkBoundaryConstraints) {
        const node: (NodeModel | ConnectorModel)[] =
          obj instanceof Node ? this.nodes : this.connectors;
        for (let i: number = 0; i <= node.length; i++) {
          if (
            node[parseInt(i.toString(), 10)] &&
            (obj as NodeModel).id === node[parseInt(i.toString(), 10)].id
          ) {
            node.splice(i, 1);
          }
        }
  ...
  public initObject(
    ...
    if (obj !== undefined) {
      ...
      if (obj instanceof Node) {
        ...
        const sourceNode: NodeModel = this.nameTable[obj.sourceID];
        const targetNode: NodeModel = this.nameTable[obj.targetID];
        const port: PointPortModel = this.getConnectedPort(
          sourceNode,
          obj,
          true
        );
        const targetPort: PointPortModel = this.getConnectedPort(
          targetNode,
          obj
        );
        const outPort: PointPortModel = this.findInOutConnectPorts(
          sourceNode,
          false
        );
        const inPort: PointPortModel = this.findInOutConnectPorts(
          targetNode,
          true
        );
        if (
          (sourceNode !== undefined && canOutConnect(sourceNode)) ||
          ((obj as Connector).sourcePortID !== '' && canPortOutConnect(outPort))
        ) {
          (obj as Connector).sourceWrapper = this.getEndNodeWrapper(
            sourceNode,
            obj as Connector,
            true
          );
          if ((obj as Connector).sourcePortID) {
            ...
            if (
              ...
              port.constraints & PortConstraints.OutConnect
            ) {
              (obj as Connector as Connector).sourcePortWrapper =
                this.getWrapper(
                  sourceNode.wrapper,
                  (obj as Connector).sourcePortID
                );
            }
          }
        }
        if (
          (targetNode !== undefined && canInConnect(targetNode)) ||
          ((obj as Connector).targetPortID !== '' && canPortInConnect(inPort))
        ) {
          (obj as Connector).targetWrapper = this.getEndNodeWrapper(
            targetNode,
            obj as Connector,
            false
          );
          if ((obj as Connector).targetPortID) {
            ...
            if (
              targetPort &&
              targetPort.constraints &&
              !(targetPort.constraints & PortConstraints.None) &&
              targetPort.constraints & PortConstraints.InConnect
            ) {
              (obj as Connector).targetPortWrapper = this.getWrapper(
                targetNode.wrapper,
                (obj as Connector).targetPortID
              );
            }
          }
        }
        if (!independentObj) {
          const points: PointModel[] = (obj as Connector).getConnectorPoints(
            (obj as Connector).type
          );
          updateConnector(obj as Connector, points);
        }
        if (independentObj) {
          (obj as Connector).init(this);
        }
        for (
          let k: number = 0;
          k < (obj as Connector).wrapper.children.length;
          k++
        ) {
          if (
            this.pathTable[
              (
                (obj as Connector).wrapper.children[
        ...
        (obj as Connector).wrapper.arrange(
          (obj as Connector).wrapper.desiredSize
        );
        ...
        for (
          let j: number = 0;
          j < (obj as Connector).wrapper.children.length;
          j++
        ) {
          this.pathTable[
            (
              (obj as Connector).wrapper.children[
                parseInt(j.toString(), 10)
              ] as PathElement
            ).data
          ] = {};
          this.pathTable[
      ...
      if (obj instanceof Node && obj.children && obj.container) {
        for (let i: number = 0; i < obj.children.length; i++) {
          this.nameTable[obj.children[parseInt(i.toString(), 10)]].offsetX =
            this.nameTable[
              obj.children[parseInt(i.toString(), 10)]
            ].wrapper.offsetX;
      ...
      if (
        ...
        (obj.shape as BpmnShape).activity.activity === 'SubProcess'
      ) {
        if ((obj.shape as BpmnShape).activity.subProcess.processes) {
          const children: any = (obj.shape as BpmnShape).activity.subProcess
            .processes;
          for (let i: number = 0; i < children.length; i++) {
            if (this.nameTable[children[parseInt(i.toString(), 10)]]) {
              this.nameTable[children[parseInt(i.toString(), 10)]].offsetX =
                this.nameTable[
                  children[parseInt(i.toString(), 10)]
                ].wrapper.offsetX;
      ...
      if (!this.refreshing) {
        ...
        if (
          ((obj as Node).shape as BpmnShape).annotations &&
          ((obj as Node).shape as BpmnShape).annotations.length > 0
        ) {
          for (
            let i: number = 0;
            i < ((obj as Node).shape as BpmnShape).annotations.length &&
            ((obj as Node).shape as BpmnShape).annotations[
              parseInt(i.toString(), 10)
            ].text;
            i++
      ...
      if (obj instanceof Node && obj.children) {
        ...
        for (let i: number = 0; i < (obj as Node).children.length; i++) {
          const node: Node | Connector =
            this.nameTable[obj.children[parseInt(i.toString(), 10)]];
          if (node) {
    ...
    if ((obj as Node | Connector).visible === false) {
      this.updateElementVisibility(
        (obj as IElement).wrapper,
        obj as Node | Connector,
        false
      );
    }
  }
  ...
  private scaleObject(obj: NodeModel, size: number, isWidth: boolean): void {
    const actualSize: number = isWidth
      ? obj.wrapper.actualSize.width
      : obj.wrapper.actualSize.height;
  ...
  private updateDefaultLayoutIcon(node: NodeModel, icon: IconShapeModel): void {
    if (icon.shape !== 'None') {
      if (
        icon.horizontalAlignment === 'Auto' &&
        icon.verticalAlignment === 'Auto' &&
        icon.offset.x === 0.5 &&
        icon.offset.y === 1
      ) {
        const iconWrapper: DiagramElement = this.getWrapper(
          node.wrapper,
          'icon_content'
        );
        let offsetX: number;
        let offsetY: number;
        if (
          this.layout.orientation === 'TopToBottom' ||
          this.layout.orientation === 'BottomToTop'
        ) {
          offsetX = 0.5;
          offsetY = this.layout.orientation === 'TopToBottom' ? 1 : 0;
        } else if (
          this.layout.orientation === 'RightToLeft' ||
          this.layout.orientation === 'LeftToRight'
        ) {
   ...
   * @private
   */
  public updateGroupOffset(
    node: NodeModel | ConnectorModel,
    isUpdateSize?: boolean
  ): void {
    const isUpdateGroupToBlazor: boolean = false;
    if (
      ((node as Node).children &&
        (node as Node).children.length > 0 &&
        !(node as Node).container) ||
      (node as Node).processId
    ) {
      ...
      if (
        ...
        if (
          node1.offsetX &&
          (this.realActions & RealAction.EnableGroupAction ||
            (!(this.diagramActions & DiagramAction.UndoRedo) &&
  ...
  private initNode(obj: Node, diagramId: string, group?: boolean): void {
    const canvas: Container = obj.initContainer();
    const portContainer: Canvas = new Canvas();
    ...
    if (obj.children) {
      ...
      let checkPorts: boolean =
        obj.ports && obj.ports.length > 0 ? true : false;
    ...
    if (wrapperContent) {
      ...
      (obj.children ? canvas : content).description = obj.annotations.length
        ? obj.annotations[0].content
        : obj.id;
    }
    ...
    for (
      let i: number = 0;
      obj.fixedUserHandles !== undefined, i < obj.fixedUserHandles.length;
      i++
    ) {
      const fixedUserHandles: DiagramElement = obj.initFixedUserHandles(
        obj.fixedUserHandles[i],
        this.fixedUserHandleTemplate,
        this.element.id
      );
    ...
    if (
      obj.wrapper.flip !== 'None' &&
      obj.wrapper.elementActions & ElementAction.ElementIsGroup
    ) {
      alignElement(
        obj.wrapper,
        obj.wrapper.offsetX,
    ...
    if (obj.shape.type === 'Bpmn') {
      if ((obj.shape as BpmnShape).shape === 'TextAnnotation') {
        ...
        for (let i: number = 0; i < this.connectors.length; i++) {
          if (
            this.connectors[parseInt(i.toString(), 10)].id ===
            obj.id + '_connector'
          ) {
            (
              this.connectors[parseInt(i.toString(), 10)] as any
    ...
    if (
      obj instanceof Node &&
      obj.container &&
      (obj.width < canvas.outerBounds.width ||
        obj.height < canvas.outerBounds.height) &&
      canvas.bounds.x <= canvas.outerBounds.x &&
      canvas.bounds.y <= canvas.outerBounds.y
    ) {
  ...
  private addBpmnAnnotationConnector(node: NodeModel, wrapper: Container) {
    if (
      (node as any).parentObj instanceof Diagram ||
      (node as any).parentObj instanceof Lane
    ) {
      let bpmnAnnotation: BpmnShape = node.shape as BpmnShape;
      let direction: TextAnnotationDirection =
        bpmnAnnotation.textAnnotation.textAnnotationDirection;
      let hasTarget: boolean =
        bpmnAnnotation.textAnnotation.textAnnotationTarget !== '' &&
        this.nameTable[bpmnAnnotation.textAnnotation.textAnnotationTarget];
      let targetNode: Node = hasTarget
        ? this.nameTable[bpmnAnnotation.textAnnotation.textAnnotationTarget]
        : null;
      let targetWrapper: DiagramElement =
        targetNode != null ? targetNode.wrapper : null;
      ...
      let connector: Connector = new Connector(
        ...
        'connectors',
        {
          ...
          constraints:
            ...
            ~(ConnectorConstraints.DragTargetEnd | ConnectorConstraints.Delete),
          isBpmnAnnotationConnector: true,
        },
        true
      );
  ...
  private getBPMNTextAnnotation(
    ...
    let obj: NodeModel = {
      ...
      annotations: [
        {
          id: (annotation.id ? annotation.id : randomId()) + annotation.text,
          content: annotation.text,
        },
      ],
      offsetX:
        node.offsetX +
        annotation.length * Math.cos(annotation.angle * (Math.PI / 180)),
      offsetY:
        node.offsetY +
        annotation.length * Math.sin(annotation.angle * (Math.PI / 180)),
      shape: {
        type: 'Bpmn',
        shape: 'TextAnnotation',
        textAnnotation: {
          textAnnotationDirection: 'Auto',
          textAnnotationTarget: node.id,
        },
      },
      constraints:
        NodeConstraints.Default &
    ...
    const endPoint = Point.transform(
      position,
      annotation.angle,
      Math.max(parentBounds.width, parentBounds.height)
    );
    let point = getIntersectionPoints(
      segment,
      [center, endPoint],
      false,
      center
  ...
  public updateDiagramElementQuad(): void {
    for (let i: number = 0; i < this.nodes.length; i++) {
      if (
        this.nodes[parseInt(i.toString(), 10)].wrapper &&
        this.nodes[parseInt(i.toString(), 10)].wrapper instanceof Container
      ) {
  ...
  private onLoadImageSize(id: string, size: Size): void {
    ...
    this.nodePropertyChange(
      obj as Node,
      {} as Node,
      { width: size.width, height: size.height } as Node
    );
  ...
  private updateChildPosition(obj: NodeModel): void {
    for (let i: number = 0; i < obj.children.length; i++) {
      const child: NodeModel = this.getObject(
        obj.children[parseInt(i.toString(), 10)]
      );
  ...
  private initCommands(): void {
    ...
    const commands: {} = {
      ...
      focusToPreviousItem: {
        execute: this.navigateItems.bind(this, false),
        canExecute: this.canExecute.bind(this),
      ...
      bold: {
        execute: this.fontStyleCommand.bind(this, 'bold'),
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.B, keyModifiers: KeyModifiers.Control },
      },
      italic: {
        execute: this.fontStyleCommand.bind(this, 'italic'),
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.I, keyModifiers: KeyModifiers.Control },
      },
      underline: {
        execute: this.fontStyleCommand.bind(this, 'underline'),
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.U, keyModifiers: KeyModifiers.Control },
      },
      duplicate: {
        execute: this.duplicateCommand.bind(this),
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.D, keyModifiers: KeyModifiers.Control },
      },
      group: {
        execute: this.groupCommand.bind(this, 'group'),
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.G, keyModifiers: KeyModifiers.Control },
      },
      ungroup: {
        execute: this.groupCommand.bind(this, 'ungroup'),
        canExecute: this.canExecute.bind(this),
        gesture: {
          key: Keys.U,
      ...
      shiftUp: {
        ...
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.Up, keyModifiers: KeyModifiers.Shift },
      },
      shiftDown: {
        execute: this.shiftCommand.bind(this, 'Down'),
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.Down, keyModifiers: KeyModifiers.Shift },
      },
      shiftLeft: {
        execute: this.shiftCommand.bind(this, 'Left'),
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.Left, keyModifiers: KeyModifiers.Shift },
      },
      shiftRight: {
        execute: this.shiftCommand.bind(this, 'Right'),
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.Right, keyModifiers: KeyModifiers.Shift },
      },
      alignTextCenter: {
      ...
      sendBackward: {
        ...
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.BracketLeft, keyModifiers: KeyModifiers.Control },
      },
      bringForward: {
        execute: this.orderCommand.bind(this, 'bringForward'),
        canExecute: this.canExecute.bind(this),
        gesture: { key: Keys.BracketRight, keyModifiers: KeyModifiers.Control },
      },
    };
  ...
  private overrideCommands(newCommand: CommandModel, commands: {}): void {
    ...
    for (const key of Object.keys(commands)) {
      ...
      if (
        newCommand.gesture.key === command.gesture.key &&
        newCommand.gesture.keyModifiers === command.gesture.keyModifiers
      ) {
        delete commands[`${key}`];
        break;
      }
    ...
    if (newCommands) {
      for (i = 0; i < newCommands.length; i++) {
        if (
          ...
          newCommands[parseInt(i.toString(), 10)]
        ) {
          ...
          if (
            newCommands[parseInt(i.toString(), 10)].gesture.key ||
  ...
  private updateIconVisibility(node: Node, visibility: boolean): void {
    for (let i: number = 0; i < node.wrapper.children.length; i++) {
      const child: DiagramElement =
        node.wrapper.children[parseInt(i.toString(), 10)];
      if (child.id) {
  ...
  public updatePortEdges(
    ...
    if (node) {
      for (let i: number = 0; i < node.ports.length; i++) {
        const port: PointPortModel | PortModel =
          node.ports[parseInt(i.toString(), 10)];
  ...
  private updateCanupdateStyle(
    ...
    for (let j: number = 0; j < element.length; j++) {
      if ((element[parseInt(j.toString(), 10)] as Container).children) {
        this.updateCanupdateStyle(
          (element[parseInt(j.toString(), 10)] as Container).children,
          value
        );
      }
    ...
    if (obj.zIndex !== Number.MIN_VALUE) {
      for (let i: number = 0; i < objects.length; i++) {
        if (objects[parseInt(i.toString(), 10)].zIndex > obj.zIndex) {
          ...
          index = getDomIndex(
            viewId,
            objects[parseInt(i.toString(), 10)].id,
            type
          );
          break;
        }
   ...
   * @private
   */
  public updateDiagramObject(
    obj: NodeModel | ConnectorModel,
    canIgnoreIndex?: boolean,
    isUpdateObject?: boolean
  ): void {
    let view: View;
    const domTable: string = 'domTable';
    for (const temp of this.views) {
      ...
      if (this.diagramActions) {
        if (view.mode === 'SVG') {
          const hasLayers: boolean = this.layers.length > 1;
          let layer: LayerModel;
          if (hasLayers) {
            layer = this.commandHandler.getObjectLayer(obj.id);
          }
          if (
            ...
            const diagramElementsLayer: HTMLCanvasElement = window[
              `${domTable}`
            ][view.element.id + '_diagramLayer'] as HTMLCanvasElement;
            ...
            this.diagramRenderer.updateNode(
              obj.wrapper as DiagramElement,
              diagramElementsLayer,
              htmlLayer,
              undefined,
              canIgnoreIndex
                ? undefined
                : this.getZindexPosition(obj, view.element.id),
              centerPoint,
              this.portCenterPoint
            );
            this.updateCanupdateStyle(obj.wrapper.children, true);
          }
        }
  ...
  private getMidPoint(obj: NodeModel | ConnectorModel): object {
    ...
    if (obj instanceof Connector && obj.type === 'Bezier') {
      ...
      for (let i: number = 0; i < obj.annotations.length; i++) {
        const centerPoint: number | PointModel =
          obj.annotations[parseInt(i.toString(), 10)].offset;
        const absoluteLength: number = (centerPoint as number) * totalLength;
        const position: PointModel = this.commandHandler.getPointAtLength(
          absoluteLength,
          totalPoints,
          0
        );
        ...
        for (let j: number = 0; j < obj.wrapper.children.length; j++) {
          if (
            obj.wrapper &&
            obj.wrapper.children[parseInt(j.toString(), 10)] instanceof
              TextElement
          ) {
            if (
              obj.wrapper.children[parseInt(j.toString(), 10)].id ===
              obj.id + '_' + obj.annotations[parseInt(i.toString(), 10)].id
            ) {
              this.applyMarginBezier(
                obj.wrapper.children[parseInt(j.toString(), 10)] as TextElement,
                annotationPosition
              );
              finalPoint[
                obj.id + '_' + obj.annotations[parseInt(i.toString(), 10)].id
                ...
                obj.wrapper.children[parseInt(j.toString(), 10)] as TextElement,
                annotationPosition,
                obj.annotations[parseInt(i.toString(), 10)].displacement
              );
            }
          }
        }
      ...
      for (let i: number = 0; i < obj.ports.length; i++) {
        ...
        const portPosition: PointModel = this.commandHandler.getPointAtLength(
          length,
          totalPoints,
          0
        );
        ...
        for (let j: number = 0; j < obj.wrapper.children.length; j++) {
          if (
            ...
            if (
              obj.wrapper.children[parseInt(j.toString(), 10)].id ===
              obj.id + '_' + obj.ports[parseInt(i.toString(), 10)].id
            ) {
              this.applyMarginBezier(
                obj.wrapper.children[parseInt(j.toString(), 10)] as PathElement,
                portPoint
              );
              this.portCenterPoint[
                obj.id + '_' + obj.ports[parseInt(i.toString(), 10)].id
                ...
                obj.wrapper.children[parseInt(j.toString(), 10)] as PathElement,
                portPoint,
                obj.ports[parseInt(i.toString(), 10)].displacement
              );
            }
          }
        }
  ...
  private applyAlignment(
    ...
    switch (child.horizontalAlignment) {
      case 'Auto':
      case 'Left':
        finalPoint.cx = child.inversedAlignment
          ? finalPoint.cx
          : finalPoint.cx - child.desiredSize.width;
    ...
    switch (child.verticalAlignment) {
      case 'Auto':
      case 'Top':
        finalPoint.cy = child.inversedAlignment
          ? finalPoint.cy
          : finalPoint.cy - child.desiredSize.height;
  ...
  private getBezierPoints(obj: ConnectorModel): PointModel[] {
    ...
    for (i = 0; i < obj.segments.length; i++) {
      const total: PointModel[] = (
        obj.segments[parseInt(i.toString(), 10)] as BezierSegment
      ).getPoints(
        obj.segments[parseInt(i.toString(), 10)] as BezierSegment,
        source
      );
  ...
  public updateGridContainer(grid: GridPanel): void {
    ...
    for (const temp of this.views) {
      ...
      if (view.mode === 'SVG' && this.diagramActions) {
        const diagramElementsLayer: HTMLCanvasElement = document.getElementById(
          view.element.id + '_diagramLayer'
  ...
  private renderBasicElement(view: View): void {
    ...
    for (let i: number = 0; i < this.basicElements.length; i++) {
      const element: DiagramElement =
        this.basicElements[parseInt(i.toString(), 10)];
  ...
  private refreshElements(view: View): void {
    ...
    if (!this.isDestroyed) {
      ...
      if (view instanceof Diagram) {
        (view.diagramLayer as HTMLCanvasElement)
          .getContext('2d')
          .setTransform(
            view.scroller.currentZoom,
            0,
            0,
            view.scroller.currentZoom,
            0,
      ...
      for (let i: number = 0; i < this.basicElements.length; i++) {
        ...
        element.measure(new Size(element.width, element.height));
        element.arrange(element.desiredSize);
        view.diagramRenderer.renderElement(
          element,
          view.diagramLayer,
          htmlLayer
        );
  ...
  public refreshCanvasDiagramLayer(view: View): void {
    if (view.mode !== 'SVG' && !this.isDestroyed) {
      ...
      if (
        !this.diagramActions ||
        (this.diagramActions & DiagramAction.Render) === 0 ||
        DiagramAction.ToolAction & this.diagramActions ||
        canVitualize(this) ||
        this.scroller.currentZoom !== 1
      ) {
   ...
   * @private
   */
  public updatePortVisibility(
    obj: Node | Connector,
    portVisibility: PortVisibility,
    inverse?: Boolean
  ): void {
    let portElement: DiagramElement;
    const drawingObject: boolean = !(
      this.drawingObject && this.drawingObject.shape
    )
      ? true
      : false;
    if (
      ...
      for (let i: number = 0; i < ports.length; i++) {
        portElement = this.getWrapper(
          obj.wrapper,
          ports[parseInt(i.toString(), 10)].id
        );
        if (
          portVisibility & PortVisibility.Hover ||
          portVisibility & PortVisibility.Connect
        ) {
          if (
            checkPortRestriction(
              ports[parseInt(i.toString(), 10)],
  ...
  public refreshSvgDiagramLayer(view: View): void {
    ...
    if (!canVitualize(this)) {
      for (let i: number = 0; i < this.basicElements.length; i++) {
        ...
        element.measure(new Size(element.width, element.height));
        (element as GridPanel).arrange(
          element.desiredSize,
          !(this.diagramActions & DiagramAction.Render) ? true : false
        );
  ...
  public removeVirtualObjects(clearIntervalVal: Object): void {
    if (this.deleteVirtualObject) {
      for (let i: number = 0; i < this.scroller.removeCollection.length; i++) {
        const obj: NodeModel | ConnectorModel =
          this.nameTable[
            this.scroller.removeCollection[parseInt(i.toString(), 10)]
          ];
  ...
  public updateTextElementValue(object: NodeModel | ConnectorModel): void {
    for (let j: number = 0; j < object.wrapper.children.length; j++) {
      const element: TextElement = object.wrapper.children[
        parseInt(j.toString(), 10)
      ] as TextElement;
      if (element instanceof TextElement) {
        ...
        if (
          (object as Node).offsetX < viewPortWidth &&
          (object as Node).offsetY < viewPortHeight
        ) {
          measureText = true;
        }
        if (
          measureText &&
          canVitualize(this) &&
          element.actualSize.height === undefined &&
          element.actualSize.width === undefined
        ) {
          object.wrapper.measure(
            new Size((object as Node).width, (object as Node).height),
            object.id,
            this.onLoadImageSize.bind(this)
          );
  ...
  public updateVirtualObjects(
    ...
    const diagramElementsLayer: HTMLCanvasElement = document.getElementById(
      this.element.id + '_diagramLayer'
    ) as HTMLCanvasElement;
    const htmlLayer: HTMLElement = getHTMLLayer(this.element.id);
    if (this.mode === 'SVG') {
      for (let i: number = 0; i < collection.length; i++) {
        const index: number = this.scroller.removeCollection.indexOf(
          collection[parseInt(i.toString(), 10)]
        );
        if (index >= 0) {
          this.scroller.removeCollection.splice(index, 1);
        }
        const object: NodeModel | ConnectorModel =
          this.nameTable[collection[parseInt(i.toString(), 10)]];
      ...
      for (let k: number = 0; k < tCollection.length; k++) {
        this.scroller.removeCollection.push(
          tCollection[parseInt(k.toString(), 10)]
        );
      }
  ...
  public renderDiagramElements(
    ...
    for (const layerId of Object.keys(this.layerZIndexTable)) {
      const layer: LayerModel = this.commandHandler.getLayer(
        this.layerZIndexTable[`${layerId}`]
      );
      let left: string;
      let top: string;
      if (
      ...
      const id: string[] =
        ...
        this.scroller.oldCollectionObjects.length > 0
          ? this.scroller.oldCollectionObjects
          : undefined;
      ...
      const layerObjects: string[] = Object.keys(
        id || (layer as Layer).zIndexTable
      );
      layerObjects.sort(
      ...
      for (const node of layerObjects) {
        const renderNode: Node = id
          ? this.nameTable[id[`${node}`]]
          : this.nameTable[(layer as Layer).zIndexTable[`${node}`]];
        if (
          renderNode &&
          !renderNode.parentId &&
          layer.visible &&
          ...
          if (canVitualize(this)) {
            if (this.scroller.currentZoom < 1) {
              if (pageBounds.x < 0 || this.scroller.horizontalOffset < 0) {
                const verticalValue: number =
                  this.scroller.verticalOffset < 0
                    ? this.scroller.verticalOffset
                    : 0;
                left = (difX > 0 ? difX : 0) + 'px';
                top =
                  (this.realActions & RealAction.vScrollbarMoved
                    ? 0
                    : -verticalValue) + 'px';
              } else {
                left = 0 + 'px';
              ...
              if (this.realActions & RealAction.hScrollbarMoved) {
                this.realActions =
                  this.realActions & ~RealAction.hScrollbarMoved;
              }
              ...
              left =
                (pageBounds.x < 0 ? difX : -this.scroller.horizontalOffset) +
                'px';
              top =
                (pageBounds.y < 0 ? difY : -this.scroller.verticalOffset) +
                'px';
            }
          ...
          if (this.refreshing) {
            if (
              (renderNode.shape as BpmnShapeModel).activity &&
              (renderNode.shape as BpmnShapeModel).activity.subProcess &&
              ...
              for (
                ...
                const process: string = (renderNode.shape as BpmnShapeModel)
                  .activity.subProcess.processes[parseInt(i.toString(), 10)];
                renderNode.wrapper.children.push(
                  this.nameTable[`${process}`].wrapper
                );
              }
              renderNode.wrapper.measure(
                new Size(
                  renderNode.wrapper.bounds.width,
                  renderNode.wrapper.bounds.height
                )
              );
          ...
          renderer.renderElement(
            ...
            undefined,
            undefined,
            status && (!this.diagramActions || isOverView),
            undefined,
            undefined,
            getCenterPoint,
            this.portCenterPoint
  ...
  public updateBridging(isLoad?: boolean): void {
    if (this.bridgingModule) {
      for (let i: number = 0; i < this.connectors.length; i++) {
        const connector: Connector = this.connectors[
          parseInt(i.toString(), 10)
        ] as Connector;
        this.bridgingModule.updateBridging(connector, this);
        const canvas: Container =
          this.connectors[parseInt(i.toString(), 10)].wrapper;
        if (canvas && canvas.children && canvas.children.length > 0) {
          ...
          if (
            (connector.isBezierEditing &&
              this.selectedItems.connectors[0].id === connector.id) ||
      ...
      console.warn(
        '[WARNING] :: Module "ConnectorBridging" is not available in Diagram component! You either misspelled the module name or forgot to load it.'
      );
    }
  }
  ...
  public clearCanvas(view: View): void {
    ...
    const width: number =
      view.contentWidth ||
      (view.diagramLayer as HTMLCanvasElement).width /
        this.scroller.currentZoom;
    const height: number =
      view.contentHeight ||
      (view.diagramLayer as HTMLCanvasElement).height /
        this.scroller.currentZoom;
    if (view.mode !== 'SVG') {
      const ctx: CanvasRenderingContext2D = CanvasRenderer.getContext(
        view.diagramLayer as HTMLCanvasElement
      );
  ...
  public updateScrollOffset(): void {
    this.scroller.setScrollOffset(
      this.diagramCanvas.scrollLeft,
      this.diagramCanvas.scrollTop
    );
    updateRuler(this);
    if (canVitualize(this)) {
      this.scroller.virtualizeElements();
  ...
  public setOffset(offsetX: number, offsetY: number): void {
    ...
    if (!window[`${domTable}`][this.element.id + 'content']) {
      window[`${domTable}`][this.element.id + 'content'] =
        document.getElementById(this.element.id + 'content');
    }
  ...
  public setSize(width: number, height: number): void {
    if (this.diagramLayer && !this.preventDiagramUpdate) {
      ...
      const diagramLayer: HTMLElement | SVGElement =
        this.mode === 'SVG'
          ? (getDiagramLayerSvg(this.element.id) as SVGElement)
          : this.diagramLayer;
      const w: number =
        this.mode === 'Canvas' &&
        this.constraints & DiagramConstraints.Virtualization
          ? (this.scroller.viewPortWidth as number)
          : width;
      const h: number =
        this.mode === 'Canvas' &&
        this.constraints & DiagramConstraints.Virtualization
          ? (this.scroller.viewPortHeight as number)
          : height;
      diagramLayer.setAttribute('width', (factor * w).toString());
      diagramLayer.setAttribute('height', (factor * h).toString());
      const hiddenUserHandleTemplate: HTMLElement = document.getElementById(
        this.element.id + '_diagramUserHandleLayer'
      );
  ...
  public transformLayers(): void {
    ...
    this.diagramRenderer.transformLayers(
      this.scroller.transform,
      this.mode === 'SVG'
    );
    if (!(this.diagramActions & DiagramAction.DragUsingMouse)) {
  ...
  public renderPageBreaks(bounds?: Rect): void {
    ...
    if (backgroundLayer) {
      ...
      const x: number =
        (this.scroller.transform.tx + bounds.x) * this.scroller.currentZoom;
      const y: number =
        (this.scroller.transform.ty + bounds.y) * this.scroller.currentZoom;
      const height: number = bounds.height * this.scroller.currentZoom;
      const width: number = bounds.width * this.scroller.currentZoom;
      DiagramRenderer.renderSvgBackGroundImage(
        this.pageSettings.background,
        this.element,
        x,
        y,
        width,
      ...
      if (this.pageSettings.showPageBreaks) {
        ...
        for (i = 0; i < collection.length; i++) {
          this.diagramRenderer.drawLine(backgroundLayer, {
            ...
            startPoint: {
              x:
                (collection[parseInt(i.toString(), 10)].x1 +
                  this.scroller.transform.tx) *
                this.scroller.currentZoom,
              y:
                (collection[parseInt(i.toString(), 10)].y1 +
                  this.scroller.transform.ty) *
                this.scroller.currentZoom,
            },
            endPoint: {
              x:
                (collection[parseInt(i.toString(), 10)].x2 +
                  this.scroller.transform.tx) *
                this.scroller.currentZoom,
              y:
                (collection[parseInt(i.toString(), 10)].y2 +
                  this.scroller.transform.ty) *
                this.scroller.currentZoom,
            },
            id:
  ...
  private renderNodes(overview: View): void {
    if (overview) {
      const renderer: DiagramRenderer = new DiagramRenderer(
        overview.id,
        new SvgRenderer(),
        false
      );
      const g: HTMLElement = document.getElementById(
        overview.element.id + '_diagramLayer'
      ) as HTMLCanvasElement;
      const htmlLayer: HTMLElement = getHTMLLayer(overview.element.id);
      this.renderDiagramElements(
        g as HTMLCanvasElement,
        overview.diagramRenderer || renderer,
        htmlLayer,
        undefined,
        undefined,
        true
  ...
  private updateThumbConstraints(
    ...
    for (let i: number = 0; i < length; i++) {
      const obj: NodeModel | ConnectorModel | ShapeAnnotation | PathAnnotation =
        node[parseInt(i.toString(), 10)];
      let hideRotate: boolean = false;
      const hideResize: boolean = false;
      let thumbConstraints: ThumbsConstraints = (selectorModel as Selector)
        .thumbsConstraints;
      if (obj instanceof Node) {
        hideRotate =
          obj.shape.type === 'Bpmn' &&
          (obj.shape as BpmnShapeModel).shape === 'Activity' &&
        ...
        if (!canResize(obj as ShapeAnnotation | PathAnnotation)) {
          thumbConstraints =
            thumbConstraints &
            ~(
              ThumbsConstraints.ResizeSouthEast |
              ThumbsConstraints.ResizeSouthWest |
              ThumbsConstraints.ResizeSouth |
              ThumbsConstraints.ResizeEast |
              ThumbsConstraints.ResizeWest |
              ThumbsConstraints.ResizeNorth |
              ThumbsConstraints.ResizeNorthEast |
  ...
  public renderSelector(
    ...
    const rendererActions: RendererAction =
      this.diagramRenderer.rendererActions;
    const innertemplate: HTMLCollection = document.getElementsByClassName(
      'blazor-inner-template'
    );
    let i: number;
    let div: HTMLElement;
    this.diagramRenderer.rendererActions = this.currentSymbol
      ...
      : this.removeConstraints(
          rendererActions,
          RendererAction.DrawSelectorBorder
        );
    this.clearSelectorLayer();
    if (this.commandHandler.hasSelection()) {
      if (
        selectorModel.nodes.length === 1 &&
        selectorModel.connectors.length === 0
      ) {
      ...
      const selectorElement: SVGElement | HTMLCanvasElement =
        getSelectorElement(this.element.id);
      //let diagramUserHandlelayer: (SVGElement | HTMLElement);
      const diagramUserHandlelayer: SVGElement | HTMLElement =
        getUserHandleLayer(this.element.id);
      selectorModel.thumbsConstraints = ThumbsConstraints.Default;
      if (selectorModel.annotation) {
        this.updateThumbConstraints(
          [selectorModel.annotation] as ShapeAnnotation[] | PathAnnotation[],
          selectorModel
        );
      } else {
      ...
      if (selectorModel.annotation) {
        ...
        selectorModel.nodes.length + selectorModel.connectors.length === 1 ||
        this.nameTable['helper']
      ) {
        if (selectorModel.nodes[0] instanceof Node) {
          ...
          const constraints: boolean = isSwimLane
            ? true
            : node.constraints & NodeConstraints.HideThumbs
            ? true
            : false;
          const swimlane: boolean =
            node.shape.type === 'SwimLane' ||
            node.isLane ||
            node.isPhase ||
            isSwimLane
          ...
          this.diagramRenderer.renderResizeHandle(
            ...
            selectorElement,
            selectorModel.thumbsConstraints,
            this.scroller.currentZoom,
            selectorModel.constraints,
            this.scroller.transform,
            undefined,
            canMove(node),
            constraints,
            swimlane,
            selectorModel.handleSize
          );
        } else if (
          selectorModel.connectors[0] instanceof Connector &&
          canDrawThumbs(this.diagramRenderer.rendererActions)
        ) {
          ...
          this.diagramRenderer.renderEndPointHandle(
            ...
            selectorModel.constraints,
            this.scroller.transform,
            connector.sourceWrapper !== undefined,
            connector.targetWrapper !== undefined,
            this.connectorEditingToolModule && canDragSegmentThumb(connector)
              ? true
              : false,
            this.connectorEditingToolModule ? true : false,
            selectorModel.handleSize
          );
        }
      } else {
        this.diagramRenderer.renderResizeHandle(
          selectorModel.wrapper,
          selectorElement,
          selectorModel.thumbsConstraints,
          this.scroller.currentZoom,
          selectorModel.constraints,
      ...
      if (!selectorModel.annotation && !this.currentSymbol) {
        this.diagramRenderer.renderUserHandler(
          selectorModel,
          selectorElement,
          this.scroller.transform,
          diagramUserHandlelayer,
          (this.eventHandler as any).currentAction,
          (this.eventHandler as any).inAction
        );
        //Removed isBlazor code
      }
    }
  ...
  private updateSelectionRectangle(): void {
    const selectorElement: SVGElement | HTMLCanvasElement = getSelectorElement(
      this.element.id
    );
    let isFirst: boolean = false;
    for (
      let i: number = 0;
      i < this.selectedItems.selectedObjects.length;
      i++
    ) {
      ...
      if (
        getObjectType(
          this.selectedItems.selectedObjects[parseInt(i.toString(), 10)]
        ) === Connector
      ) {
        ...
        this.diagramRenderer.renderSelectionLine(
          this.selectedItems.selectedObjects[parseInt(i.toString(), 10)].wrapper
            .children[0] as PathElement,
          selectorElement,
          this.scroller.transform,
          isFirst
        );
        ...
        this.diagramRenderer.renderSelectionRectangle(
          this.selectedItems.selectedObjects[parseInt(i.toString(), 10)]
            .wrapper,
          selectorElement,
          this.scroller.transform,
  ...
  public updateSelector(): void {
    ...
    if (
      ...
      this.selectedItems.wrapper.rotateAngle =
        this.selectedItems.nodes[0].rotateAngle;
    }
    if (this.selectedItems !== undefined) {
      ...
      if (
        ...
        selector.wrapper.children.length
      ) {
        let canUpdate: boolean = true;
        ...
        if (selectionHasConnector(this, selector)) {
          ...
          const isSelect: boolean =
            rotate.includes('None') ||
            rotate.includes('Select') ||
            rotate.includes('Drag');
        ...
        if (
          selector.rotateAngle !== 0 ||
          selector.rotateAngle !== selector.wrapper.prevRotateAngle
        ) {
        ...
        const selectorEle: SVGElement | HTMLCanvasElement = getSelectorElement(
          this.element.id
        );
        //let diagramUserHandlelayer: (SVGElement | HTMLElement);
        const diagramUserHandlelayer: SVGElement | HTMLElement =
        ...
        if (selector.annotation) {
          this.updateThumbConstraints(
            [selector.annotation] as ShapeAnnotation[] | PathAnnotation[],
            selector
          );
        } else {
        ...
        if (
          this.selectedItems.constraints & SelectorConstraints.UserHandle &&
          !selector.annotation &&
          !this.currentSymbol
        ) {
          this.diagramRenderer.renderUserHandler(
        ...
        if (selector.annotation) {
          ...
          if (
            selector.connectors[0] instanceof Connector &&
            canDrawThumbs(this.diagramRenderer.rendererActions)
          ) {
            ...
            if (checkParentAsContainer(this, selector.nodes[0])) {
              if (
                stackPanel.shape.type !== 'UmlClassifier' &&
                !(
                  (stackPanel as Node).parentId &&
                  this.nameTable[(stackPanel as Node).parentId] &&
                  this.nameTable[(stackPanel as Node).parentId].shape.type ===
                    'UmlClassifier'
                )
              ) {
            ...
            const swimlane: boolean =
              stackPanel.shape.type === 'SwimLane' ||
              (stackPanel as Node).isLane ||
              (stackPanel as Node).isPhase
                ? true
                : false;
            this.diagramRenderer.renderResizeHandle(
              selector.wrapper.children[0],
              selectorEle,
              selector.thumbsConstraints,
              this.scroller.currentZoom,
              ...
              this.scroller.transform,
              canHideResizers,
              canMove(selector.nodes[0]),
              selector.nodes[0].constraints & NodeConstraints.HideThumbs
                ? true
                : false,
              swimlane,
              selector.handleSize
          ...
          if (this.diagramActions & DiagramAction.Interactions) {
            this.diagramRenderer.rendererActions =
              this.diagramRenderer.rendererActions |
              RendererAction.PreventRenderSelector;
          }
  ...
  public renderSelectorForAnnotation(
    selectorModel: Selector,
    selectorElement: SVGElement | HTMLCanvasElement
  ): void {
    this.diagramRenderer.renderResizeHandle(
      selectorModel.wrapper.children[0],
      selectorElement,
      selectorModel.thumbsConstraints,
      this.scroller.currentZoom,
      selectorModel.constraints,
  ...
  public drawSelectionRectangle(
    ...
    this.clearSelectorLayer();
    this.diagramRenderer.drawSelectionRectangle(
      x,
      y,
      width,
      height,
  ...
  public renderHighlighter(element: DiagramElement): void {
    const adornerSvg: SVGElement = getAdornerLayerSvg(this.element.id);
    this.diagramRenderer.renderHighlighter(
      element,
      adornerSvg,
      this.scroller.transform
    );
  ...
  public clearHighlighter(): void {
    ...
    const highlighter: SVGElement = (
      adornerSvg as SVGSVGElement
    ).getElementById(adornerSvg.id + '_highlighter') as SVGElement;
    if (highlighter) {
   ...
   * @private
   */
  public getNodesConnectors(
    selectedItems: (NodeModel | ConnectorModel)[]
  ): (NodeModel | ConnectorModel)[] {
    for (let i: number = 0; i < this.nodes.length; i++) {
      const node: NodeModel = this.nodes[parseInt(i.toString(), 10)];
  ...
  public clearSelectorLayer(): void {
    ...
    if (!this.currentSymbol) {
      const selectionRect: SVGElement = (
        adornerSvg as SVGSVGElement
      ).getElementById(this.adornerLayer.id + '_selected_region') as SVGElement;
      if (selectionRect) {
        selectionRect.parentNode.removeChild(selectionRect);
      }
      this.clearHighlighter();
      const childNodes: NodeList = getSelectorElement(
        this.element.id
      ).childNodes;
      ...
      //Removed isBlazor code
      if (!isBlazor()) {
        const templates: NodeList = getUserHandleLayer(
          this.element.id
        ).childNodes;
        for (i = templates.length; i > 0; i--) {
          (templates[i - 1] as HTMLElement).parentNode.removeChild(
            templates[i - 1]
          );
        }
      }
    } else {
      const symbolBorder: SVGElement = (
        adornerSvg as SVGSVGElement
      ).getElementById('borderRect_symbol') as SVGElement;
      if (symbolBorder) {
  ...
  public getWrapper(nodes: Container, id: string): DiagramElement {
    ...
    const container: Container =
      nodes instanceof Canvas
        ? nodes
        : this.getPortContainer(this.nameTable[nodes.id]);
  ...
  public getEndNodeWrapper(
    ...
    if (
      ...
      node.wrapper.children[0] instanceof Canvas
    ) {
      if (!isBlazor() && (node.shape as BpmnShape).shape === 'Activity') {
        if (
          source &&
          (node.shape as BpmnShape).activity.subProcess.type ===
            'Transaction' &&
          connector.sourcePortID
        ) {
          const portId: string = connector.sourcePortID;
          const parent: Canvas = (
            (node.wrapper.children[0] as Canvas).children[0] as Canvas
          ).children[2] as Canvas;
          if (parent.children) {
        ...
        return ((node.wrapper.children[0] as Canvas).children[0] as Canvas)
          .children[0];
      }
  ...
  private fontStyleCommand(format: string): void {
    for (let i: number = 0; i < this.selectedItems.nodes.length; i++) {
      const node: NodeModel =
        this.selectedItems.nodes[parseInt(i.toString(), 10)];
      if (node.shape.type === 'SwimLane') {
        if ((node.shape as SwimLane).hasHeader) {
          this.applyStyleText(
            format,
            (node.shape as SwimLane).header.annotation
          );
        }
      }
      if ((node as SwimLane).isLane) {
        const laneHeader: NodeModel = this.getObject(
          (node.shape as LaneModel).header[0].id
        );
    ...
    for (let i: number = 0; i < this.selectedItems.connectors.length; i++) {
      if (
        this.selectedItems.connectors[parseInt(i.toString(), 10)].annotations
          .length > 0
      ) {
        const annotationLength: PathAnnotationModel[] =
          this.selectedItems.connectors[parseInt(i.toString(), 10)].annotations;
        this.applyStyle(format, annotationLength);
      }
    }
  ...
  private applyStyle(
    format: string,
    annotationLength: ShapeAnnotationModel[] | PathAnnotationModel[]
  ): void {
    for (let j: number = 0; j < annotationLength.length; j++) {
      switch (format) {
        case 'bold':
          annotationLength[parseInt(j.toString(), 10)].style.bold =
            !annotationLength[parseInt(j.toString(), 10)].style.bold;
          break;
        case 'italic':
          annotationLength[parseInt(j.toString(), 10)].style.italic =
            !annotationLength[parseInt(j.toString(), 10)].style.italic;
          break;
        case 'underline':
          if (
            annotationLength[parseInt(j.toString(), 10)].style
              .textDecoration === 'None'
          ) {
            annotationLength[parseInt(j.toString(), 10)].style.textDecoration =
              'Underline';
          } else if (
            annotationLength[parseInt(j.toString(), 10)].style
  ...
  private toolCommand(tool: string): void {
    switch (tool) {
      ...
      case 'ellipse': {
        const drawingNode: BasicShapeModel = {
          type: 'Basic',
          shape: 'Ellipse',
        };
  ...
  private alignCommand(alignDirection: string): void {
    if (this.selectedItems.nodes.length > 0) {
      for (let i: number = 0; i < this.selectedItems.nodes.length; i++) {
        this.updateNodesAndConnectorAnnotation(
          this.selectedItems.nodes[parseInt(i.toString(), 10)],
          alignDirection
        );
      }
  ...
  private navigateItems(tabCommand: boolean): void {
    ...
    if (selectedItems.length > 0) {
      currentSelectedNodeIndex =
        selectedItems[0].zIndex + (tabCommand ? 1 : -1);
    } else {
      currentSelectedNodeIndex = tabCommand
        ? 0
        : this.nodes.length + this.connectors.length - 1;
    }
    if (currentSelectedNodeIndex < 0) {
      currentSelectedNodeIndex = this.nodes.length + this.connectors.length - 1;
    } else if (
      currentSelectedNodeIndex ===
      this.nodes.length + this.connectors.length
    ) {
    ...
    for (let i: number = 0; i < this.nodes.length; i++) {
      if (
        currentSelectedNodeIndex ===
        this.nodes[parseInt(i.toString(), 10)].zIndex
      ) {
    ...
    for (let j: number = 0; j < this.connectors.length; j++) {
      if (
        currentSelectedNodeIndex ===
        this.connectors[parseInt(j.toString(), 10)].zIndex
      ) {
    ...
    if (this.diagramActions & DiagramAction.TextEdit) {
      ...
      if (!isBlazor()) {
        ...
        const annotation:
          | ShapeAnnotationModel
          | PathAnnotationModel
          | TextModel = findAnnotation(node, this.activeLabel.id);
        // eslint-disable-next-line max-len
        const args: ITextEditEventArgs | IBlazorTextEditEventArgs = {
        ...
        if (annotation && !(annotation instanceof Text)) {
          // eslint-disable-next-line max-len
          const index: string = findObjectIndex(
            node as NodeModel,
            (annotation as ShapeAnnotationModel | PathAnnotationModel).id,
            true
          );
        ...
        if (
          !deleteNode &&
          (element.textContent !== text || text !== this.activeLabel.text)
        ) {
        ...
        if (annotation.content !== text && !args.cancel) {
          ...
          if (
            ...
            text.indexOf('#') === -1 &&
            text.indexOf('~') === -1 &&
            node.id.indexOf('_umlClass_header') === -1
          ) {
            text = ' + ' + text;
          }
          ...
          if (nodeIndex) {
            ...
            if (getObjectType(node) === Node) {
              this.onPropertyChanged(
                { nodes: newnodes } as DiagramModel,
                { nodes: oldnodes } as DiagramModel
              );
            } else {
              this.onPropertyChanged(
                { connectors: newnodes } as DiagramModel,
                { connectors: oldnodes } as DiagramModel
              );
            }
          }
          this.protectPropertyChange(true);
        ...
        if (this.selectedItems.nodes.length) {
          ...
          if (
            (swimLaneNode && swimLaneNode.shape.type === 'SwimLane') ||
            selectedNode.shape.type === 'SwimLane'
          ) {
            ...
            if (selectedNode.isLane || selectedNode.isPhase) {
              const collection: (PhaseModel | LaneModel)[] = selectedNode.isLane
                ? (swimLaneNode.shape as SwimLaneModel).lanes
                : (swimLaneNode.shape as SwimLaneModel).phases;
              for (let j: number = 0; j < collection.length; j++) {
                if (
                  collection[parseInt(j.toString(), 10)].id ===
                  (selectedNode[`${laneHeader}`] ||
                    selectedNode[`${phaseHeader}`])
                ) {
                  collection[
                    parseInt(j.toString(), 10)
              ...
              selectedNode.isHeader &&
              (swimLaneNode.shape as SwimLane).hasHeader
            ) {
              (swimLaneNode.shape as SwimLaneModel).header.annotation.content =
                selectedNode.annotations[0].content;
            }
          }
          this.dataBind();
  ...
  public getIndex(node: NodeModel | ConnectorModel, id: string) {
    //let index: number;
    const collection: (NodeModel | ConnectorModel)[] =
      getObjectType(node) === Node ? this.nodes : this.connectors;
    for (let i: number = 0; i < collection.length; i++) {
      if (
        collection[parseInt(i.toString(), 10)].id.toString() === id.toString()
      ) {
        return i.toString();
      }
    }
  ...
  private modelChanged(newProp: DiagramModel, oldProp: DiagramModel): boolean {
    if (
      ...
      newProp.nodes ||
      oldProp.connectors ||
      newProp.pageSettings ||
      oldProp.pageSettings ||
      newProp.bridgeDirection ||
      oldProp.bridgeDirection
    ) {
      return true;
    }
    return false;
  ...
  private resetDiagramActions(action?: DiagramAction): void {
    ...
    if (
      this.diagramActions & DiagramAction.UndoRedo &&
      (!isAction || action === DiagramAction.UndoRedo)
    ) {
      this.diagramActions = this.diagramActions & ~DiagramAction.UndoRedo;
    }
    if (
      this.diagramActions & DiagramAction.PublicMethod &&
      (!isAction || action === DiagramAction.PublicMethod)
    ) {
      this.diagramActions = this.diagramActions & ~DiagramAction.PublicMethod;
    }
  }
  ...
  public removeNode(node: NodeModel, childrenCollection: string[]): void {
    ...
    const groupElement: HTMLElement = document.getElementById(
      node.id + '_groupElement'
    );
   ...
   */
  public updateObject(
    actualObject: Node | Connector,
    oldObject: Node | Connector,
    changedProp: Node | Connector
  ): void {
        ...
        if (actualObject instanceof Node) {
          ...
          for (const key of Object.keys(changedProp)) {
            ...
            switch (key) {
              case 'sourcePoint':
                actualSourcePoint.x =
                  oldConnector.sourcePoint.x || actualSourcePoint.x;
                actualSourcePoint.y =
                  oldConnector.sourcePoint.y || actualSourcePoint.y;
                break;
              case 'targetPoint':
                actualTargetPoint.x =
                  oldConnector.targetPoint.x || actualTargetPoint.x;
                actualTargetPoint.y =
                  oldConnector.targetPoint.y || actualTargetPoint.y;
            }
          }
          this.connectorPropertyChange(
            actualObject,
            changedProp as Connector,
            oldObject as Connector
          );
        }
      }
    }
  ...
  private nodePropertyChangeExtend(
    ...
    if (node.constraints !== undefined) {
      if (
        oldObject.constraints & NodeConstraints.Select &&
  ...
  private updateTextAnnotationInSwimlane(
    ...
    if (
      (actualObject as any).hasTextAnnotation &&
      (this as any).isPositionUndo
    ) {
      for (let i: number = 0; i < (actualObject as Node).outEdges.length; i++) {
        const con: ConnectorModel =
          this.nameTable[
            (actualObject as Node).outEdges[parseInt(i.toString(), 10)]
          ];
        if ((con as any).isBpmnAnnotationConnector) {
          ...
          if ((actualObject as any).laneMargin && textNode) {
            const dx: number =
              actualObject.margin.left - (actualObject as any).laneMargin.left;
            const dy: number =
              actualObject.margin.top - (actualObject as any).laneMargin.top;
            ...
            textNode.wrapper.measure(
              new Size(textNode.wrapper.width, textNode.wrapper.height)
            );
  ...
  private swimLaneNodePropertyChange(
    ...
    if (actualObject.shape.type === 'SwimLane' && !this.currentSymbol) {
      if (oldObject.shape) {
        ...
        const orientation: boolean =
          actualShape.orientation === 'Horizontal' ? true : false;
        const padding: number = (actualShape as SwimLane).padding;
        const oldShape: SwimLaneModel = oldObject.shape as SwimLaneModel;
        const grid: GridPanel = actualObject.wrapper.children[0] as GridPanel;
        let oldObjects: NodeModel;
        let newObjects: NodeModel;
        let id: string;
          ...
          if (oldShape.lanes) {
            for (const count of Object.keys(shape.lanes)) {
              ...
              const lane: LaneModel =
                oldShape.lanes[parseInt(indexValue.toString(), 10)];
              let laneIndex: number;
              const newLane: LaneModel =
                shape.lanes[parseInt(indexValue.toString(), 10)];
              if (newLane && newLane.header) {
                id =
                  actualShape.lanes[parseInt(indexValue.toString(), 10)].header
                    .id;
              ...
              if (lane.children) {
                for (const childNodeIndex of Object.keys(lane.children)) {
                  id =
                    actualShape.lanes[parseInt(indexValue.toString(), 10)]
                      .children[Number(childNodeIndex)].id;
                  const node: Node = this.nameTable[`${id}`];
                  oldObjects = lane.children[Number(childNodeIndex)];
                  newObjects = (newLane as LaneModel).children[
                    Number(childNodeIndex)
                  ];
                  this.nodePropertyChange(
                    node,
                    oldObjects as Node,
              ...
              if (lane.width && !orientation) {
                laneIndex =
                  actualShape.phases && actualShape.phaseSize
                    ? indexValue + 1
                    : indexValue;
              ...
              if (lane.height && orientation) {
                laneIndex =
                  actualShape.header && (actualShape as SwimLane).hasHeader
                    ? indexValue + 1
                    : indexValue;
                laneIndex +=
                  actualShape.phases && actualShape.phaseSize ? 1 : 0;
          ...
          if (shape.phases) {
            for (const key of Object.keys(shape.phases)) {
              ...
              const phase: PhaseModel =
                shape.phases[parseInt(indexValue.toString(), 10)];
              let size: number;
              ...
              if (phase && phase.header) {
                id =
                  actualShape.phases[parseInt(indexValue.toString(), 10)].header
                    .id;
                oldObjects =
                  oldShape.phases[parseInt(indexValue.toString(), 10)].header;
              ...
              if (phase.offset) {
                if (indexValue === 0) {
                  ...
                  const previousPhase: PhaseModel =
                    actualShape.phases[indexValue - 1];
                ...
                if (orientation) {
                  ...
                  updatePhaseMaxWidth(
                    actualObject,
                    this,
                    grid.rows[parseInt(rowIndex.toString(), 10)].cells[
                      parseInt(indexValue.toString(), 10)
                    ],
                    indexValue
                  );
                } else {
            ...
            if (oldShape.phaseSize) {
              if (orientation) {
                ...
                if (actualShape.header && (actualShape as SwimLane).hasHeader) {
                  ...
                  grid.rows[0].cells[1].columnSpan =
              ...
              if (orientation) {
                ...
                actualObject.height = actualObject.wrapper.height +=
                  shape.phaseSize;
              } else {
                ...
                if (actualShape.header && (actualShape as SwimLane).hasHeader) {
                  ...
                  grid.rows[0].cells[0].columnSpan =
                    actualShape.lanes.length + 1;
                }
              }
              for (let k: number = 0; k < actualShape.phases.length; k++) {
                if (actualShape.phases[parseInt(k.toString(), 10)].id === '') {
                  actualShape.phases[parseInt(k.toString(), 10)].id =
                    randomId();
                }
                phaseDefine(
                  grid,
        ...
        if (
          actualShape.header &&
          (actualShape as SwimLane).hasHeader &&
          oldShape.header
        ) {
          const id: string = grid.rows[0].cells[0].children[0].id;
          const headerNode: Node = this.nameTable[`${id}`];
          this.nodePropertyChange(
            headerNode,
            oldShape.header as Node,
            shape.header as Node
          );
        }
        actualObject.height = actualObject.wrapper.height = grid.height;
        actualObject.width = actualObject.wrapper.width = grid.width;
      } else if (oldObject.constraints) {
        const oldSelectConstraints =
          oldObject.constraints & NodeConstraints.Select;
        ...
        if (oldSelectConstraints !== newSelectConstraints) {
          ...
          // Header - constraints
          const headerNode: NodeModel =
            this.nameTable[actualObject.id + shape.header.id];
          headerNode.constraints = !newSelectConstraints
          ...
          if (shape.phaseSize > 0) {
            for (let i: number = 0; i < shape.phases.length; i++) {
              phaseNode =
                this.nameTable[
                  actualObject.id +
                    shape.phases[parseInt(i.toString(), 10)].id +
                    '_header'
                ];
              phaseNode.constraints = !newSelectConstraints
          ...
          for (let i: number = 0; i < shape.lanes.length; i++) {
            for (let l: number = 0; l < value; l++) {
              laneNode =
                this.nameTable[
                  actualObject.id +
                    shape.lanes[parseInt(i.toString(), 10)].id +
                    l
                ];
              laneNode.constraints = !newSelectConstraints
              ...
              if (l === 0) {
                ...
                laneHeader.constraints = !newSelectConstraints
  ...
  public insertValue(oldNodeObject: any, isNode: boolean): void {
    if (!(this.blazorActions & BlazorAction.GroupClipboardInProcess)) {
      let value: boolean;
      const oldObjects: (Node | Connector)[] = isNode
        ? this.oldNodeObjects
        : this.oldConnectorObjects;
      ...
      if (!value) {
        ...
        isNode
          ? this.oldNodeObjects.push(oldNodeObject as Node)
          : this.oldConnectorObjects.push(oldNodeObject as Connector);
      }
    }
  }
  ...
  public nodePropertyChange(
    ...
    update = this.nodePropertyChangeExtend(
      actualObject,
      oldObject,
      node,
      update
    ...
    if (
      ...
      canShadow(oldObject) !== canShadow(node)
    ) {
      actualObject.wrapper.children[0].shadow = canShadow(actualObject)
        ? actualObject.shadow
        : null;
    }
    ...
    if (node.padding !== undefined) {
      actualObject.wrapper.padding.left =
        node.padding.left !== undefined
          ? node.padding.left
          : actualObject.wrapper.padding.left;
      actualObject.wrapper.padding.right =
        node.padding.right !== undefined
          ? node.padding.right
          : actualObject.wrapper.padding.right;
      actualObject.wrapper.padding.top =
        node.padding.top !== undefined
          ? node.padding.top
          : actualObject.wrapper.padding.top;
      actualObject.wrapper.padding.bottom =
        node.padding.bottom !== undefined
          ? node.padding.bottom
          : actualObject.wrapper.padding.bottom;
      update = true;
    }
    ...
    if (node.minWidth !== undefined) {
      actualObject.wrapper.minWidth =
        actualObject.wrapper.children[0].minWidth = node.minWidth;
      update = true;
      updateConnector = true;
      sizeChanged = true;
    }
    if (node.minHeight !== undefined) {
      actualObject.wrapper.minHeight =
        actualObject.wrapper.children[0].minHeight = node.minHeight;
      update = true;
      updateConnector = true;
      sizeChanged = true;
    }
    ...
    if (node.flip !== undefined) {
      ...
      if (actualObject.wrapper.elementActions & ElementAction.ElementIsGroup) {
        alignElement(
          actualObject.wrapper,
          actualObject.offsetX,
          actualObject.offsetY,
          this,
          node.flip
        );
        ...
        if (
          ...
          (actualObject.flipMode === 'Port' || actualObject.flipMode === 'All')
        ) {
    ...
    if (
      node.rotateAngle !== undefined &&
      actualObject.constraints & NodeConstraints.Rotate
    ) {
      if (actualObject.children && rotate) {
        ...
        this.commandHandler.rotateObjects(
          ...
          { x: actualObject.offsetX, y: actualObject.offsetY },
          false
        );
      }
    ...
    if (node.visible !== undefined) {
      this.updateElementVisibility(
        actualObject.wrapper,
        actualObject as Node,
        actualObject.visible
      );
    }
    ...
    if (
      ((node.shape !== undefined && node.shape.type === undefined) ||
        node.width !== undefined ||
        node.height !== undefined ||
        node.style !== undefined) &&
      actualObject.shape.type === 'Bpmn' &&
      this.bpmnModule
    ) {
      update = true;
      updateConnector = true;
    ...
    if (
      actualObject.shape.type === 'UmlActivity' &&
      !isBlazor() &&
      (actualObject.shape as UmlActivityShapeModel).shape === 'FinalNode'
    ) {
      update = true;
      updateConnector = true;
    ...
    if (
      (actualObject.shape && actualObject.shape.type === 'UmlClassifier') ||
      (actualObject.parentId &&
        this.nameTable[actualObject.parentId] &&
        this.nameTable[actualObject.parentId].shape.type === 'UmlClassifier')
    ) {
      update = true;
      updateConnector = true;
    }
    if (node.ports !== undefined) {
      for (const key of Object.keys(node.ports)) {
        const index: number = Number(key);
        update = true;
        const changedObject: PointPortModel = node.ports[`${key}`];
        const actualPort: PointPortModel =
          actualObject.ports[parseInt(index.toString(), 10)];
        this.updatePort(changedObject, actualPort, actualObject.wrapper);
        updateConnector = true;
      }
    }
    if (
      (node as HeaderModel).annotation !== undefined ||
      node.annotations !== undefined ||
      node.width !== undefined
    ) {
      for (const key of Object.keys(
        node.annotations || actualObject.annotations
      )) {
        const index: number = Number(key);
        update = true;
        ...
        if ((node as HeaderModel).annotation) {
          ...
          changedObject = node.annotations
            ? node.annotations[`${key}`]
            : actualObject.annotations;
        }
        const actualAnnotation: ShapeAnnotationModel =
          actualObject.annotations[parseInt(index.toString(), 10)];
        if (actualAnnotation) {
          ...
          if (swimLaneNode && swimLaneNode.shape.type === 'SwimLane') {
            ...
            if (actualObject.isLane || actualObject.isPhase) {
              ...
              for (let j: number = 0; j < collection.length; j++) {
                if (
                  collection[parseInt(j.toString(), 10)].id ===
                  (actualObject[`${laneHeader}`] ||
                    actualObject[`${phaseHeader}`])
                ) {
                  collection[
                    parseInt(j.toString(), 10)
                  ].header.annotation.content =
                    actualObject.annotations[0].content;
                  collection[
                    parseInt(j.toString(), 10)
                  ].header.annotation.style = actualObject.annotations[0].style;
                  break;
                }
              }
            }
    ...
    if (
      node.expandIcon !== undefined ||
      node.collapseIcon !== undefined ||
    ...
    if (node.fixedUserHandles !== undefined) {
      ...
      for (const key of Object.keys(node.fixedUserHandles)) {
        ...
        actualfixedUserHandle =
          actualObject.fixedUserHandles[parseInt(index.toString(), 10)];
        if (actualfixedUserHandle) {
          this.updateNodefixedUserHandle(
            changedObject,
            actualfixedUserHandle,
            actualObject.wrapper,
            actualObject
    ...
    if (update) {
      if (
        this.bpmnModule !== undefined &&
        offsetChanged &&
        !sizeChanged &&
        !angleChanged &&
        !(this as any).sizeUndo
      ) {
        // eslint-disable-next-line max-len
        this.updateBpmnAnnotationPosition(
          oldBpmnOffsetX,
          oldBpmnOffsetY,
          newBpmnOffsetX,
          newBpmnOffsetY,
          actualObject,
          actualObject.wrapper,
          actualObject.shape as BpmnShape,
          (actualObject.shape as BpmnShape).shape === 'TextAnnotation'
        );
      }
      if (
        this.checkSelectedItem(actualObject) &&
        actualObject.wrapper.children[0] instanceof TextElement
      ) {
        (actualObject.wrapper.children[0] as TextElement).refreshTextElement();
      }
      actualObject.wrapper.measure(
        new Size(
          actualObject.wrapper.bounds.width,
          actualObject.wrapper.bounds.height
        ),
        actualObject.id,
        this.onLoadImageSize.bind(this)
      );
      ...
      if (
        actualObject.shape.type === 'SwimLane' &&
        !this.currentSymbol &&
        !(this.diagramActions & DiagramAction.ToolAction)
      ) {
        ...
        if (shape.orientation === 'Horizontal') {
          const index: number =
            shape.header && (shape as SwimLane).hasHeader ? 1 : 0;
          updatePhaseMaxWidth(
            actualObject,
            this,
            grid.rows[parseInt(index.toString(), 10)].cells[column - 1],
            column - 1
          );
        }
        actualObject.wrapper.measure(
      ...
      if (
        existingBounds.equals(
          existingBounds,
          actualObject.wrapper.outerBounds
        ) === false
      ) {
      ...
      if (!isLayout) {
        // eslint-disable-next-line max-len
        this.commandHandler.connectorSegmentChange(
          actualObject,
          existingInnerBounds,
          node.rotateAngle !== undefined ? true : false
        );
        // if (updateConnector) {
        ...
        if (actualObject && actualObject.visible && actualObject.outEdges) {
          this.updateIconVisibility(
            actualObject,
            actualObject.outEdges.length === 0 ? false : true
          );
        }
      }
      ...
      if (actualObject.parentId && this.nameTable[actualObject.parentId]) {
        ...
        parent.wrapper.measure(
          new Size(parent.wrapper.width, actualObject.wrapper.height)
        );
        parent.wrapper.arrange(parent.wrapper.desiredSize);
      ...
      if (
        ...
        const connectors: string[] = getConnectors(
          this,
          actualObject.wrapper.children[0] as GridPanel,
          undefined,
          true
        );
      ...
      if (!this.preventNodesUpdate) {
        if (
          !canVitualize(this) ||
          (canVitualize(this) &&
            this.scroller.oldCollectionObjects.indexOf(actualObject.id) > -1)
        ) {
        ...
        if (!isLayout && updateConnector) {
          ...
          if (
            ...
            const objects: Object[] = this.spatialSearch.findObjects(
              actualObject.wrapper.outerBounds as Rect
            );
            for (let i: number = 0; i < objects.length; i++) {
              const object: Object = objects[parseInt(i.toString(), 10)];
              if (object instanceof Connector) {
                this.connectorPropertyChange(
                  objects[parseInt(i.toString(), 10)] as Connector,
                  {} as Connector,
                  {
  ...
  private updateBpmnAnnotationPosition(
    ...
    if (width !== 0 && height !== 0) {
      bounds = new Rect(
        (newX !== 0 ? newX : node.offsetX) - width / 2,
        (newY !== 0 ? newY : node.offsetY) - height / 2,
        width,
        height
      );
    }
    ...
    if (isTextAnnotation) {
      ...
      const hasTarget: boolean =
        bpmnAnnotation.textAnnotation.textAnnotationTarget !== '' &&
        this.nameTable[bpmnAnnotation.textAnnotation.textAnnotationTarget];
      const selectedNode = this.selectedItems.nodes
        ? this.selectedItems.nodes[0]
        : undefined;
      const isTextNodeSelected =
        selectedNode &&
        selectedNode.shape &&
        (selectedNode.shape as BpmnShape).shape === 'TextAnnotation';
      if (hasTarget) {
        //To check whether the text annotation inside the swimlane
        if ((node as Node).parentId === '' || isTextNodeSelected) {
          if (
            bpmnAnnotation.textAnnotation.textAnnotationDirection === 'Auto'
          ) {
            if (
              wrapper.children[0] instanceof Canvas &&
              (wrapper.children[0] as Canvas).children[0] instanceof PathElement
            ) {
              const diagramCanvas = (wrapper as any).children[0];
              const parentElement = document.getElementById(
                diagramCanvas.id + '_groupElement'
              );
              const elementToRemove = document.getElementById(
                diagramCanvas.children[0].id + '_groupElement'
              );
              parentElement.removeChild(elementToRemove);
              diagramCanvas.children.splice(0, 1);
              this.isProtectedOnChange = true;
              this.bpmnModule.setAnnotationPath(
                bounds,
                diagramCanvas,
                node,
                bpmnAnnotation,
        ...
        if ((node as Node).inEdges.length > 0) {
          ...
          if (connector && (connector as any).isBpmnAnnotationConnector) {
            connector.sourcePoint = {
              x:
                newX > oldX
                  ? connector.sourcePoint.x + x
                  : connector.sourcePoint.x - x,
              y:
                newY > oldY
                  ? connector.sourcePoint.y + y
                  : connector.sourcePoint.y - y,
            };
          }
        }
          ...
          if ((targetNode.shape as BpmnShape).shape === 'TextAnnotation') {
            ...
            if (laneX !== undefined && laneY !== undefined) {
              if (targetNode.parentId) {
                oldValue = {
                  margin: {
                    left: targetNode.margin.left,
                    top: targetNode.margin.top,
                  },
                };
              ...
              targetNode.offsetX =
                newX > oldX ? targetNode.offsetX + x : targetNode.offsetX - x;
              targetNode.offsetY =
                newY > oldY ? targetNode.offsetY + y : targetNode.offsetY - y;
              newValue = {
                offsetX: targetNode.offsetX,
                offsetY: targetNode.offsetY,
              };
  ...
  private updatePorts(actualObject: Node, flip: FlipDirection): void {
    if (actualObject && actualObject.ports.length > 0) {
      for (const key of Object.keys(actualObject.ports)) {
        ...
        const actualPort: PointPortModel =
          actualObject.ports[parseInt(index.toString(), 10)];
        let portWrapper: DiagramElement = this.getWrapper(
          actualObject.wrapper,
          actualPort.id
        );
  ...
  private updateUMLActivity(
    ...
    const sizeChanged: boolean =
      changedProp.width !== undefined || changedProp.height !== undefined;
    if (sizeChanged) {
      const innerFinalNode: DiagramElement = (
        actualObject.wrapper.children[0] as Canvas
      ).children[0];
      innerFinalNode.width = changedProp.width;
      innerFinalNode.height = changedProp.height;
      const outerFinalNode: DiagramElement = (
        actualObject.wrapper.children[0] as Canvas
      ).children[1];
  ...
  public updateConnectorEdges(actualObject: Node | Connector): void {
    if (actualObject.inEdges.length > 0) {
      for (let j: number = 0; j < actualObject.inEdges.length; j++) {
        this.updateConnectorProperties(
          this.nameTable[actualObject.inEdges[parseInt(j.toString(), 10)]]
        );
      }
    }
    if (actualObject.outEdges.length > 0) {
      for (let k: number = 0; k < actualObject.outEdges.length; k++) {
        this.updateConnectorProperties(
          this.nameTable[actualObject.outEdges[parseInt(k.toString(), 10)]]
        );
      }
    }
    if (actualObject.parentId && this.nameTable[actualObject.parentId]) {
  ...
  private connectorProprtyChangeExtend(
    ...
    if (
      newProp.shape !== undefined &&
      actualObject.shape !== undefined &&
      (actualObject.shape as BpmnFlowModel) &&
      actualObject.shape.type === 'Bpmn' &&
      this.bpmnModule
    ) {
  ...
  public connectorPropertyChange(
    ...
    const existingBounds: Rect = actualObject.wrapper.bounds;
    let updateSelector: boolean = false;
    let points: PointModel[] = [];
    updateSelector = this.connectorProprtyChangeExtend(
      actualObject,
      oldProp,
      newProp,
      updateSelector
    ...
    if (
      newProp.sourcePoint !== undefined ||
      newProp.targetPoint !== undefined ||
      newProp.sourceID !== undefined ||
      newProp.targetID !== undefined ||
      newProp.targetPadding !== undefined ||
      newProp.sourcePortID !== undefined ||
      newProp.targetPortID !== undefined ||
      newProp.sourcePadding !== undefined ||
      newProp.type !== undefined ||
      newProp.segments !== undefined ||
      newProp.flip !== undefined
    ) {
      if (
        (newProp.sourceID !== undefined &&
          newProp.sourceID !== oldProp.sourceID) ||
        newProp.sourcePortID
      ) {
        const sourceNode: Node = this.nameTable[actualObject.sourceID];
        outPort = this.findInOutConnectPorts(sourceNode, false);
        if (
          !sourceNode ||
          canOutConnect(sourceNode) ||
          (actualObject.sourcePortID !== '' && canPortOutConnect(outPort))
        ) {
          actualObject.sourceWrapper = sourceNode
            ? this.getEndNodeWrapper(sourceNode, actualObject, true)
            : undefined;
          if (actualObject.sourcePortID && newProp.sourcePortID === undefined) {
            actualObject.sourcePortWrapper = sourceNode
              ? this.getWrapper(sourceNode.wrapper, actualObject.sourcePortID)
              : undefined;
          }
          this.removePortEdges(
            this.nameTable[oldProp.sourceID] || sourceNode,
            oldProp.sourcePortID || actualObject.sourcePortID,
            actualObject.id,
            false
          );
        }
        if (
          ...
          if (
            oldSource !== undefined &&
            oldSource.outEdges &&
            oldSource.outEdges.indexOf(actualObject.id) !== -1
          ) {
      ...
      if (
        newProp.targetID !== undefined &&
        newProp.targetID !== oldProp.targetID
      ) {
        const targetNode: Node = this.nameTable[newProp.targetID];
        inPort = this.findInOutConnectPorts(targetNode, true);
        if (
          !targetNode ||
          canInConnect(targetNode) ||
          (actualObject.targetPortID !== '' && canPortInConnect(inPort))
        ) {
          actualObject.targetWrapper = targetNode
            ? this.getEndNodeWrapper(targetNode, actualObject, false)
            : undefined;
          if (actualObject.targetPortID && newProp.targetPortID === undefined) {
            actualObject.targetPortWrapper = targetNode
              ? this.getWrapper(targetNode.wrapper, actualObject.targetPortID)
              : undefined;
          }
          this.removePortEdges(
            this.nameTable[oldProp.targetID] || targetNode,
            oldProp.targetPortID || actualObject.targetPortID,
            actualObject.id,
            true
          );
        }
        if (
          ...
          if (
            oldTarget !== undefined &&
            oldTarget.inEdges &&
            oldTarget.inEdges.indexOf(actualObject.id) !== -1
          ) {
      ...
      if (
        newProp.sourcePortID !== undefined &&
        newProp.sourcePortID !== oldProp.sourcePortID
      ) {
        ...
        if (
          ...
          actualObject.sourcePortWrapper = source
            ? this.getWrapper(source, newProp.sourcePortID)
            : undefined;
        } else if (
          actualObject.sourcePortID === '' &&
          !canOutConnect(sourceNode)
        ) {
      ...
      if (
        newProp.targetPortID !== undefined &&
        newProp.targetPortID !== oldProp.targetPortID
      ) {
        ...
        if (
          ...
          actualObject.targetPortWrapper = target
            ? this.getWrapper(target, newProp.targetPortID)
            : undefined;
        } else if (
          actualObject.targetPortID === '' &&
          !canInConnect(targetNode)
        ) {
      ...
      if (
        ...
        // EJ2-65876 - Exception occurs on line routing injection module
        if (
          actualObject.sourceID !== actualObject.targetID &&
          actualObject.segments.length > 1
        ) {
          //EJ2-69573 - Excecption occurs when calling doLayout method with the lineRouting module
          this.lineRoutingModule.refreshConnectorSegments(
            this,
            actualObject,
            false
          );
    ...
    if (
      points.length > 0 ||
      newProp.sourceDecorator !== undefined ||
      (newProp.targetDecorator !== undefined &&
        canMeasureDecoratorPath(Object.keys(newProp.targetDecorator))) ||
      newProp.cornerRadius !== undefined
    ) {
      updateConnector(
        actualObject,
        points.length > 0 ? points : actualObject.intermediatePoints,
        this.diagramActions
      );
      ...
      if (points.length > 0) {
        actualObject.wrapper.measure(
          new Size(actualObject.wrapper.width, actualObject.wrapper.height)
        );
        actualObject.wrapper.arrange(actualObject.wrapper.desiredSize);
        // eslint-disable-next-line max-len
    ...
    if (
      (newProp.sourcePoint || newProp.targetPoint || newProp.segments) &&
      this.diagramActions === DiagramAction.Render
    ) {
      updateSelector = true;
    }
    if (
      actualObject.shape.type === 'Bpmn' &&
      (actualObject.shape as BpmnFlowModel).sequence === 'Default' &&
      (actualObject.shape as BpmnFlowModel).flow === 'Sequence'
    ) {
    ...
    actualObject.wrapper.arrange(actualObject.wrapper.desiredSize);
    if (
      existingBounds.equals(existingBounds, actualObject.wrapper.bounds) ===
      false
    ) {
      this.updateQuad(actualObject);
      if ((this.eventHandler as any).currentAction !== 'Drag') {
  ...
  private getPoints(
    ...
    //let pts: PointModel[];
    const lineDistributionModule: boolean = this.lineDistributionModule
      ? true
      : false;
      ...
      for (let i: number = 0; i < element.children.length; i++) {
        if (element.children[parseInt(i.toString(), 10)] instanceof Container) {
          this.updateNodeProperty(
            element.children[parseInt(i.toString(), 10)] as Container,
            undefined,
            opacity
          );
        }
  ...
  private updateDiagramContainerVisibility(
    ...
    if (element instanceof Container) {
      for (let i: number = 0; i < element.children.length; i++) {
        this.updateDiagramContainerVisibility(
          element.children[parseInt(i.toString(), 10)],
          visible
        );
      }
    }
   ...
   * @private
   */
  public updateElementVisibility(
    element: Container,
    obj: Connector | Node,
    visible: boolean
  ): void {
    if (visible !== undefined) {
      ...
      if (obj instanceof Node) {
        ...
        if (!obj.children) {
          ...
          for (const child of obj.children) {
            this.updateElementVisibility(
              this.nameTable[`${child}`].wrapper,
              this.nameTable[`${child}`],
              visible
            );
          }
        }
        ...
        if (obj.annotations) {
          for (const annotation of obj.annotations) {
            const wrapper: DiagramElement = this.getWrapper(
              element,
              annotation.id
            );
            if (visible) {
              wrapper.visible =
                (wrapper as TextElement).annotationVisibility === 'Visible'
                  ? true
                  : false;
            } else {
              wrapper.visible = visible;
      ...
      if ((obj as NodeModel).expandIcon || (obj as NodeModel).collapseIcon) {
        const wrapper: Container = this.getWrapper(
          element,
          'icon_content'
        ) as Container;
        if (wrapper) {
        ...
        if (obj && obj.visible && (obj as Node).outEdges) {
          this.updateIconVisibility(
            obj as Node,
            (obj as Node).outEdges.length === 0 ? false : true
          );
        }
      }
      if (visible === false) {
      ...
      if (
        (obj instanceof Node && !this.preventNodesUpdate) ||
        (obj instanceof Connector && !this.preventConnectorsUpdate)
      ) {
  ...
  private updateAnnotations(newProp: Connector, actualObject: Connector): void {
    if (newProp.annotations !== undefined) {
      for (const key of Object.keys(newProp.annotations)) {
        ...
        const actualAnnotation: AnnotationModel =
          actualObject.annotations[parseInt(index.toString(), 10)];
        this.updateAnnotation(
          changedObject,
          actualAnnotation,
  ...
  private updatefixedUserHandle(
    ...
    if (newProp.fixedUserHandles !== undefined) {
      ...
      for (const key of Object.keys(newProp.fixedUserHandles)) {
        ...
        actualAnnotation =
          actualObject.fixedUserHandles[parseInt(index.toString(), 10)];
        this.updateConnectorfixedUserHandle(
          changedObject,
          actualAnnotation,
          actualObject.wrapper,
          actualObject
  ...
  public updateConnectorfixedUserHandle(
    changedObject: ConnectorFixedUserHandleModel,
    actualfixedUserHandle: ConnectorFixedUserHandleModel,
    nodes: Container,
    actualObject?: Object,
    canUpdateSize?: boolean
  ): void {
    let isMeasure: boolean = false;
    const fixedUserHandleWrapper: Canvas = this.getWrapper(
      nodes,
      actualfixedUserHandle.id
    ) as Canvas;
    if (fixedUserHandleWrapper !== undefined) {
      ...
      if (
        actualfixedUserHandle instanceof ConnectorFixedUserHandle &&
        (changedObject as ConnectorFixedUserHandle).offset !== undefined
      ) {
        (actualObject as Connector).updateAnnotation(
          actualfixedUserHandle,
          (actualObject as Connector).intermediatePoints,
          (actualObject as Connector).wrapper.bounds,
          fixedUserHandleWrapper as Canvas
        );
      }
      if (
        actualfixedUserHandle instanceof ConnectorFixedUserHandle &&
        (changedObject as ConnectorFixedUserHandle).displacement
      ) {
        if (
          (changedObject as ConnectorFixedUserHandle).displacement.x !==
            undefined ||
          (changedObject as ConnectorFixedUserHandle).displacement.y !==
            undefined
        ) {
          isMeasure = true;
        }
      ...
      if (changedObject.handleStrokeColor !== undefined) {
        fixedUserHandleWrapper.style.strokeColor =
          changedObject.handleStrokeColor;
      }
      if (changedObject.handleStrokeWidth !== undefined) {
        fixedUserHandleWrapper.style.strokeWidth =
          changedObject.handleStrokeWidth;
      }
      ...
      this.updatefixedUserHandleContent(
        changedObject,
        isMeasure,
        fixedUserHandleWrapper,
        actualObject,
      ...
      if (isMeasure === true) {
        fixedUserHandleWrapper.measure(
          new Size(fixedUserHandleWrapper.width, fixedUserHandleWrapper.height)
        );
  ...
  public updateAnnotation(
    ...
    let isMeasure: boolean = false;
    // eslint-disable-next-line max-len
    const annotationWrapper: TextElement | DiagramHtmlElement = this.getWrapper(
      nodes,
      actualAnnotation.id
    ) as TextElement | DiagramHtmlElement;
    if (annotationWrapper !== undefined) {
      if (
        changedObject.width !== undefined &&
        changedObject.height !== undefined
      ) {
      ...
      if (
        actualAnnotation instanceof PathAnnotation &&
      ...
      if (changedObject.rotationReference !== undefined) {
        (annotationWrapper as TextElement).rotationReference =
          changedObject.rotationReference;
      }
      if (
        actualAnnotation instanceof ShapeAnnotation &&
        (changedObject as ShapeAnnotationModel).offset !== undefined
      ) {
        const offset: PointModel = (changedObject as ShapeAnnotationModel)
          .offset;
        isMeasure = true;
        const offsetX: number =
          offset.x !== undefined ? offset.x : actualAnnotation.offset.x;
        const offsetY: number =
          offset.y !== undefined ? offset.y : actualAnnotation.offset.y;
        annotationWrapper.setOffsetWithRespectToBounds(
          offsetX,
          offsetY,
          'Fraction'
        );
        ...
        (actualObject as Connector).updateAnnotation(
          ...
          (actualObject as Connector).intermediatePoints,
          (actualObject as Connector).wrapper.bounds,
          annotationWrapper as TextElement
        );
      }
      if (
        actualAnnotation instanceof PathAnnotation &&
        (changedObject as PathAnnotation).displacement
      ) {
        if (
          (changedObject as PathAnnotation).displacement.x !== undefined ||
      ...
      if (isMeasure || canUpdateSize) {
        annotationWrapper.width =
          actualAnnotation.width || (actualObject as Node).width;
        if (actualAnnotation.template) {
          annotationWrapper.width =
            annotationWrapper.width || annotationWrapper.actualSize.width;
          annotationWrapper.height =
            actualAnnotation.height ||
            (actualObject as Node).height ||
            annotationWrapper.actualSize.height;
        }
      }
      if (changedObject.horizontalAlignment !== undefined) {
        annotationWrapper.horizontalAlignment =
          changedObject.horizontalAlignment;
        isMeasure = true;
      }
      ...
      if (changedObject.visibility !== undefined) {
        annotationWrapper.visible =
          nodes.visible && changedObject.visibility ? true : false;
        (annotationWrapper as TextElement).annotationVisibility =
          annotationWrapper.visible ? 'Visible' : 'Collapsed';
      }
      if (changedObject.constraints !== undefined) {
        const updateSelector: boolean = false;
        if (
          annotationWrapper.constraints & AnnotationConstraints.Select &&
      ...
      if (isMeasure === true) {
        annotationWrapper.measure(
          new Size(annotationWrapper.width, annotationWrapper.height)
        );
  ...
  private updatefixedUserHandleContent(
    changedObject: ConnectorFixedUserHandleModel,
    isMeasure: boolean,
    fixedUserHandleWrapper: Canvas,
    actualObject: Object,
    fixedUserHandleAnnotation: ConnectorFixedUserHandleModel,
    nodes: Container
  ): void {
    if (changedObject !== undefined) {
      this.updateConnectorfixedUserHandleWrapper(
        fixedUserHandleWrapper,
        actualObject,
        fixedUserHandleAnnotation,
        nodes
      );
    }
  }
  private updateConnectorfixedUserHandleWrapper(
    fixedUserHandleWrapper: Canvas | DiagramHtmlElement,
    actualObject: Object,
    actualAnnotation: ConnectorFixedUserHandleModel,
    nodes: Container
  ): void {
    for (const elementId of this.views) {
    ...
    if (actualObject instanceof Connector) {
      ...
      fixedUserHandleWrapper = actualObject.getFixedUserHandle(
        actualObject.fixedUserHandles[
          actualObject.fixedUserHandles.length - 1
        ] as ConnectorFixedUserHandle,
    ...
    for (let i: number = 0; i < nodes.children.length; i++) {
      if (
        fixedUserHandleWrapper.id ===
        nodes.children[parseInt(i.toString(), 10)].id
      ) {
  ...
  private updateAnnotationContent(
    changedObject: AnnotationModel,
    isMeasure: boolean,
    annotationWrapper: TextElement | DiagramHtmlElement,
    actualObject: Object,
    actualAnnotation: ShapeAnnotationModel,
    nodes: Container
  ): void {
    if (changedObject.content !== undefined) {
      if (annotationWrapper as TextElement) {
        isMeasure = true;
        if (
          (actualObject as Node).shape.type === 'UmlActivity' &&
          !isBlazor() &&
          ((actualObject as Node).shape as UmlActivityShapeModel).shape ===
            'StructuredNode'
        ) {
          (annotationWrapper as TextElement).content =
            '<<' + changedObject.content + '>>';
        } else {
          (annotationWrapper as TextElement).content = changedObject.content;
        }
      }
      if (annotationWrapper instanceof DiagramHtmlElement) {
        this.updateAnnotationWrapper(
          annotationWrapper,
          actualObject,
          actualAnnotation,
          nodes
    ...
    if (changedObject.template !== undefined) {
      (annotationWrapper as DiagramHtmlElement).content =
        changedObject.template;
      this.updateAnnotationWrapper(
        annotationWrapper,
        actualObject,
        actualAnnotation,
  ...
  private updateAnnotationWrapper(
    ...
    if (actualObject instanceof Node) {
      annotationWrapper = (actualObject as Node).initAnnotationWrapper(
        actualAnnotation as Annotation,
        this.element.id
      ) as DiagramHtmlElement | TextElement;
      ...
      annotationWrapper = actualObject.getAnnotationElement(
        actualObject.annotations[
          actualObject.annotations.length - 1
        ] as PathAnnotation,
        actualObject.intermediatePoints,
        bounds,
        this.getDescription,
    ...
    for (let i: number = 0; i < nodes.children.length; i++) {
      if (
        annotationWrapper.id === nodes.children[parseInt(i.toString(), 10)].id
      ) {
  ...
  public updateNodefixedUserHandle(
    changedObject: NodeFixedUserHandleModel,
    actualfixedUserHandle: NodeFixedUserHandleModel,
    nodes: Container,
    actualObject?: Object
  ): void {
    ...
    if (fixedUserHandleWrapper !== undefined) {
      ...
      if (
        actualfixedUserHandle instanceof NodeFixedUserHandle &&
        (changedObject as NodeFixedUserHandleModel).offset !== undefined
      ) {
        const offset: PointModel = (changedObject as NodeFixedUserHandleModel)
          .offset;
        isMeasure = true;
        const offsetX: number =
          offset.x !== undefined ? offset.x : actualfixedUserHandle.offset.x;
        const offsetY: number =
          offset.y !== undefined ? offset.y : actualfixedUserHandle.offset.y;
        fixedUserHandleWrapper.setOffsetWithRespectToBounds(
          offsetX,
          offsetY,
          'Fraction'
        );
      ...
      this.updatefixedUserHandleWrapper(
        fixedUserHandleWrapper,
        actualObject,
        actualfixedUserHandle,
        nodes
  ...
  private updatefixedUserHandleWrapper(
    fixedUserHandleWrapper: DiagramElement,
    actualObject: Object,
    actualAnnotation: FixedUserHandleModel,
    nodes: Container
  ): void {
    for (const elementId of this.views) {
      removeElement(fixedUserHandleWrapper.id + '_groupElement', elementId);
    ...
    if (actualObject instanceof Node) {
      fixedUserHandleWrapper = (actualObject as Node).initFixedUserHandles(
        actualAnnotation,
        undefined,
        undefined
      );
   ...
   */
  public updatePort(
    changedObject: PointPortModel,
    actualPort: PointPortModel,
    nodes: Container,
    actualObject?: Connector
  ): void {
    let isMeasure: boolean = false;
    ...
    if (portWrapper !== undefined) {
      if (changedObject.offset !== undefined) {
        ...
        if (!actualObject) {
          const offsetX: number =
            changedObject.offset.x !== undefined
              ? changedObject.offset.x
              : actualPort.offset.x;
          const offsetY: number =
            changedObject.offset.y !== undefined
              ? changedObject.offset.y
              : actualPort.offset.y;
          portWrapper.setOffsetWithRespectToBounds(
            offsetX,
            offsetY,
            'Fraction'
          );
      ...
      if (changedObject.visibility !== undefined) {
        portWrapper.visible =
          nodes.visible &&
          checkPortRestriction(actualPort, PortVisibility.Visible)
            ? true
            : false;
      }
      if (changedObject.margin !== undefined) {
      ...
      if (changedObject.style !== undefined) {
        ...
        if (changedObject.style.strokeDashArray !== undefined) {
          portWrapper.style.strokeDashArray =
  ...
  public updateIcon(actualObject: Node): void {
    const iconContainer: Canvas = this.getWrapper(
      actualObject.wrapper,
      'icon_content'
    ) as Canvas;
    const diagramId: string =
      this.diagramActions & DiagramAction.Render ? this.element.id : undefined;
    if (iconContainer) {
      if (this.mode === 'SVG') {
        const icon: HTMLElement = getDiagramElement(
          actualObject.wrapper.id + '_icon_content',
          diagramId
        );
        if (icon) {
          const iconRect: HTMLElement = getDiagramElement(
            icon.id + '_rect',
            diagramId
          );
          const iconShape: HTMLElement = getDiagramElement(
            icon.id + '_shape',
            diagramId
          );
          const nativeContent: HTMLElement = getDiagramElement(
            iconShape.id + '_native_element',
            diagramId
          );
          if (nativeContent) {
  ...
  private getPortContainer(actualObject: Node): Container {
    if (actualObject.children) {
      for (let i: number = 0; i < actualObject.wrapper.children.length; i++) {
        if (
          actualObject.wrapper.children[parseInt(i.toString(), 10)].id ===
          actualObject.id + 'group_container'
        ) {
          return actualObject.wrapper.children[
            parseInt(i.toString(), 10)
          ] as Container;
        }
      }
  ...
  public removeFromAQuad(obj: IElement): void {
    ...
    const isSwimLane: boolean =
      (obj as NodeModel).shape.type === 'SwimLane' ? true : false;
    const modified: boolean = this.spatialSearch.updateBounds(
      obj.wrapper,
      isSwimLane
    );
    if (modified && !this.preventDiagramUpdate) {
  ...
  private updatePage(): void {
    ...
    if (this.diagramActions & DiagramAction.DragUsingMouse) {
      ...
      if (
        !(this.pageSettings.width && this.pageSettings.height) ||
        this.pageSettings.multiplePage
      ) {
        this.scroller.updateScrollOffsets();
        this.scroller.setSize();
      }
   ...
   */
  public updateShadow(
    nodeShadow: ShadowModel,
    changedShadow: ShadowModel
  ): void {
  ...
  private removePreviewChildren(preview: Node): void {
    if (
      ...
      preview.shape &&
      preview.shape.type === 'SwimLane'
    ) {
      for (let z: number = 0; z < preview.children.length; z++) {
        const previewChildId: string =
          preview.children[parseInt(z.toString(), 10)];
        const previewIndex: number = this.nodes.indexOf(
          this.nameTable[`${previewChildId}`]
        );
        if (previewIndex >= 0) {
      ...
      const previewIndex: number = this.nodes.indexOf(
        this.nameTable[this.currentSymbol.id]
      );
      if (previewIndex >= 0) {
        this.nodes.splice(previewIndex, 1);
      }
  ...
  private selectDragedNode(
    newObj: NodeModel | Connector,
    args: any,
    selectedSymbol: HTMLElement
  ): void {
  ...
  private initDroppables(): void {
    ...
    this.droppable.over = (args: any) => {
      ...
      if (!this.currentSymbol) {
        ...
        if (args.dragData || dragDataHelper) {
          ...
          const position: PointModel = this.eventHandler.getMousePosition(
            args.event
          );
          let clonedObject: Object;
          const selectedSymbol: HTMLElement =
            dragDataHelper || args.dragData.helper;
          const paletteId: string = selectedSymbol.getAttribute('paletteId');
          let nodeDragSize: SymbolSizeModel;
          let nodePreviewSize: SymbolSizeModel;
          let paletteDragSize: SymbolSizeModel;
          let preview: Size;
          if (!paletteId && args.dragData) {
            const arg: IDragEnterEventArgs | IBlazorDragEnterEventArgs = {
              source: null,
              element: newObj as Node,
              cancel: false,
              diagram: this,
            ...
            if (arg.dragItem) {
              if (
                (arg.dragItem as Connector).sourcePoint &&
                (arg.dragItem as Connector).targetPoint
              ) {
          ...
          if (paletteId) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const sourceElement: Object = (
              document.getElementById(paletteId) as any
            ).ej2_instances[0];
            ...
            if (sourceElement) {
              ...
              const wrapper: DiagramElement = (
                obj.wrapper.children[0] as Container
              ).children[0];
              preview = getPreviewSize(
                sourceElement,
                clonedObject as Node,
                wrapper
              );
              if (sourceElement[`${selectedSymbols}`] instanceof Node) {
                if (
                  ((obj as Node).shape as BpmnShape).shape === 'TextAnnotation'
                ) {
                  // eslint-disable-next-line max-len
                  (clonedObject as Node).offsetX =
                    position.x +
                    11 +
                    (preview as Size).width * (clonedObject as Node).pivot.x;
                  // eslint-disable-next-line max-len
                  (clonedObject as Node).offsetY =
                    position.y +
                    11 +
                    (preview as Size).height * (clonedObject as Node).pivot.y;
                } else {
                  // eslint-disable-next-line max-len
                  (clonedObject as Node).offsetX =
                    position.x +
                ...
                let newNode: Node = new Node(
                  this,
                  'nodes',
                  clonedObject as NodeModel,
                  true
                );
                if (
                  newNode.shape.type === 'Bpmn' &&
                ...
                const palettePreview: SymbolSizeModel =
                  sourceElement['symbolPreview'];
                // eslint-disable-next-line max-len
                newNode.width =
                  nodeDragSize.width ||
                  paletteDragSize.width ||
                  nodePreviewSize.width ||
                  palettePreview.width ||
                  newNode.width;
                // eslint-disable-next-line max-len
                newNode.height =
                  nodeDragSize.height ||
                  paletteDragSize.height ||
                  nodePreviewSize.height ||
                  palettePreview.height ||
                  newNode.height;
                if (newNode.shape.type === 'SwimLane') {
                  ...
                  if ((newNode.shape as SwimLane).isLane) {
                    ...
                    if (
                      (newNode.shape as SwimLane).orientation === 'Horizontal'
                    ) {
                      header.width = (
                        newNode.shape as SwimLane
                      ).lanes[0].header.width;
                      header.height = (
                        newNode.shape as SwimLane
                      ).lanes[0].height;
                    } else {
                      ...
                      header.height = (
                        newNode.shape as SwimLane
                      ).lanes[0].header.height;
                    }
                    header.style = (
                      newNode.shape as SwimLane
                    ).lanes[0].header.style;
                    ...
                    if (
                      ...
                      lane.width =
                        (newNode.shape as SwimLane).lanes[0].width -
                        header.width;
                      ...
                      lane.offsetX =
                        position.x +
                        dragLeft +
                        ((newNode.shape as SwimLane).lanes[0].header.width +
                          lane.width / 2);
                      lane.offsetY = position.y + lane.height / 2;
                    } else {
                      lane.width = (newNode.shape as SwimLane).lanes[0].width;
                      lane.height =
                        (newNode.shape as SwimLane).lanes[0].height -
                        header.height;
                      lane.offsetX = position.x + dragLeft + lane.width / 2;
                      // eslint-disable-next-line max-len
                      lane.offsetY =
                        position.y +
                        ((newNode.shape as SwimLane).lanes[0].header.height +
                          lane.height / 2);
                    }
                ...
                if ((newNode.shape as SwimLane).isPhase) {
                  ...
                  if (isHorizontal) {
                    newNode.offsetX =
                      position.x +
                      dragLeft +
                      (newNode.width || wrapper.actualSize.width) / 2;
                    ...
                    newNode.offsetY =
                      position.y +
                      (newNode.height || wrapper.actualSize.height) / 2;
                    (newNode.shape as Path).data =
                      'M' +
                      newNode.width / 2 +
                      ',' +
                ...
                if (newNode.shape.type === 'UmlClassifier') {
                  ...
                  if (
                    (newNode.shape as UmlClassifierShapeModel).classifier ===
                    'Class'
                  ) {
                    if (
                      (newNode.shape as UmlClassifierShapeModel).classShape
                        .methods.length <= 0 &&
                      (newNode.shape as UmlClassifierShapeModel).classShape
                        .attributes.length <= 0
                    ) {
                      (
                        newNode.shape as UmlClassifierShapeModel
                      ).classShape.attributes = [
                  ...
                  if (
                    ...
                    if (
                      (newNode.shape as UmlClassifierShapeModel)
                        .enumerationShape.members.length <= 0
                    ) {
                      (
                        newNode.shape as UmlClassifierShapeModel
                      ).enumerationShape.members = [
                        {
                          name: 'Name',
                        },
                      ];
                  ...
                  if (
                    ...
                    if (
                      (newNode.shape as UmlClassifierShapeModel).interfaceShape
                        .methods.length <= 0 &&
                      (newNode.shape as UmlClassifierShapeModel).interfaceShape
                        .attributes.length <= 0
                    ) {
                ...
                sourceElement[`${selectedSymbols}`] instanceof Connector
              ) {
                newObj = new Connector(
                  this,
                  'connectors',
                  clonedObject as ConnectorModel,
                  true
                );
                const bounds: Rect = Rect.toBounds([
                  newObj.sourcePoint,
                  newObj.targetPoint,
                ]);
              ...
              this.triggerEvent(DiagramEvent.dragEnter, arg);
              if (
                newObj instanceof Node &&
                newObj.shape.type === 'SwimLane' &&
                (newObj.shape as SwimLane).isLane
              ) {
                const swimLaneObj: NodeModel = arg.element as NodeModel;
                const laneObj: LaneModel = (swimLaneObj.shape as SwimLane)
                  .lanes[0];
                //let child1: NodeModel; let child2: NodeModel;
                isHorizontal =
                  (swimLaneObj.shape as SwimLane).orientation === 'Horizontal'
                    ? true
                    : false;
                ...
                if (isHorizontal) {
                  ...
                  lane.offsetX =
                    position.x +
                    dragLeft +
                    (laneObj.header.width + child2.width / 2);
                  ...
                  lane.offsetY =
                    position.y + (laneObj.header.height + child2.height / 2);
                }
              ...
              if (
                newObj instanceof Node &&
                (newObj.shape as SwimLane).isPhase
              ) {
                if (isHorizontal) {
                  newObj.height = 1;
                } else {
    ...
    this.droppable.drop = async (args: any) => {
      ...
      if (this.currentSymbol) {
        let isPhase: boolean = false;
        let orientation: Orientation; //let isConnector: boolean;
        const isConnector: boolean =
          this.currentSymbol instanceof Connector ? true : false;
        ...
        const arg: IDropEventArgs | IBlazorDropEventArgs = {
          ...
          target:
            this.eventHandler['hoverNode'] ||
            this.findObjectsUnderMouse(
              this.eventHandler.getMousePosition(args.event)
            )[0] ||
            this,
          cancel: false,
          position: {
        ...
        const id: string = 'id';
        const clonedObject: Object = cloneObject(this.currentSymbol);
        ...
        if (
          (this.currentSymbol.shape as SwimLaneModel).isLane ||
          (this.currentSymbol.shape as SwimLaneModel).isPhase ||
          this.currentSymbol.shape.type === 'UmlClassifier'
        ) {
          this.removeChildInNodes(this.currentSymbol as Node);
        }
        if (arg.cancel) {
          removeChildNodes(this.currentSymbol as Node, this);
        }
        if ((this.currentSymbol.shape as SwimLaneModel).isPhase) {
          isPhase = true;
          orientation = (this.currentSymbol.shape as SwimLaneModel).orientation;
          ((clonedObject as Node).shape as SwimLaneModel).phases = (
            this.currentSymbol.shape as SwimLaneModel
          ).phases;
        }
        ...
        if (!arg.cancel) {
          ...
          if (
            clonedObject &&
            (((clonedObject as Node).shape as SwimLaneModel).isLane || isPhase)
          ) {
            if (isPhase) {
              ((clonedObject as Node).shape as SwimLaneModel).isPhase = isPhase;
              ((clonedObject as Node).shape as SwimLaneModel).orientation =
                orientation;
            }
          ...
          if (
            clonedObject &&
            (clonedObject as Node).shape.type === 'UmlClassifier' &&
            !((clonedObject as Node).shape as RelationShipModel).relationship
          ) {
            (clonedObject as Node).children = undefined;
            this.clearSelectorLayer();
          ...
          if (
            ((clonedObject as Node).shape as BpmnShape).type === 'Bpmn' &&
            ((clonedObject as Node).shape as BpmnShape).annotation &&
            clonedObject['hasTarget']
          ) {
            const nodeId: string = (
              ((clonedObject as Node).shape as BpmnShape)
                .annotation as BpmnAnnotation
            ).nodeId;
            ((clonedObject as Node).shape as BpmnShape).annotation.id = (
              clonedObject as Node
            ).id;
            this.addTextAnnotation(
              ((clonedObject as Node).shape as BpmnShape).annotation,
              this.nameTable[`${nodeId}`]
            );
            (clonedObject as BpmnAnnotation).nodeId = '';
          }
          if (
            !((clonedObject as Node).shape as SwimLaneModel).isLane &&
            !isPhase &&
            ((clonedObject as Connector).type !== undefined ||
              (clonedObject as Node).shape.type !== 'UmlClassifier')
          ) {
            if ((clonedObject as Node).children) {
            ...
            if (
              arg.target &&
              arg.target instanceof Node &&
              !isConnector &&
              checkParentAsContainer(this, arg.target) &&
              canAllowDrop(arg.target) &&
              !this.commandHandler.isTargetSubProcess(arg.target)
            ) {
              ...
              if (
                (clonedObject as Connector).type === 'Bezier' &&
                (clonedObject as Connector).segments.length > 0
              ) {
            ...
            if (
              (clonedObject || value) &&
              (canSingleSelect(this) || canMultiSelect(this))
            ) {
              this.select(
                [this.nameTable[clonedObject[`${id}`]]],
                false,
        ...
        this.protectPropertyChange(false);
        const newObj: NodeModel | Connector =
          this.nameTable[clonedObject[`${id}`]];
        ...
        if (
          this.bpmnModule &&
          newObj instanceof Node &&
          (clonedObject as Node).processId
        ) {
          newObj.processId = (clonedObject as Node).processId;
          this.bpmnModule.dropBPMNchild(
            this.nameTable[newObj.processId],
            newObj,
            this
          );
        }
        ...
        const arg: IDropEventArgs | IBlazorDropEventArgs = {
          ...
          element: undefined,
          target: cloneBlazorObject(
            this.eventHandler['hoverNode'] ||
              this.eventHandler['lastObjectUnderMouse'] ||
              this
          ),
          cancel: false,
          position: undefined,
      ...
      // eslint-disable-next-line max-len
      if (
        this.droppable[`${selectedSymbols}`] &&
        this.droppable[`${selectedSymbols}`].parentNode
      ) {
        remove(this.droppable[`${selectedSymbols}`]);
      } else {
        const draggableElement: HTMLCollection =
          document.getElementsByClassName('e-dragclone') as HTMLCollection;
    ...
    this.droppable.out = (args: Object) => {
      ...
      if (this.currentSymbol && !this.eventHandler.focus) {
        this.unSelect(this.currentSymbol);
        this.removeFromAQuad(this.currentSymbol);
        ...
        this.removeObjectsFromLayer(this.nameTable[this.currentSymbol.id]);
        if (
          this.currentSymbol.shape &&
          (this.currentSymbol.shape as BpmnShape).shape === 'TextAnnotation'
        ) {
        ...
        if (this.mode !== 'SVG') {
          ...
          if (
            (this.currentSymbol as Node).shape.type !== 'SwimLane' &&
            (this.currentSymbol as Node).children &&
            (this.currentSymbol as Node).children.length > 0
          ) {
            for (
              let i: number = 0;
              i < (this.currentSymbol as Node).children.length;
              i++
            ) {
        ...
        const source: string = 'sourceElement';
        delete this.droppable[`${source}`];
        this.diagramRenderer.rendererActions = this.removeConstraints(
          this.diagramRenderer.rendererActions,
          RendererAction.DrawSelectorBorder
        );
        if (this.previousSelectedObject) {
          this.select(
            this.previousSelectedObject,
            this.previousSelectedObject.length > 1 ? true : false
          );
        }
  ...
  private getDropEventArgs(arg: IBlazorDropEventArgs) {
    if (
      ...
      // eslint-disable-next-line max-len
      arg.target =
        getObjectType(object) === Connector
          ? { connector: cloneBlazorObject(object) as ConnectorModel }
          : { node: cloneBlazorObject(object) as NodeModel };
    } else {
    ...
    if (node) {
      if (node.children) {
        for (let i: number = 0; i < node.children.length; i++) {
          this.removeChildInNodes(
            this.nameTable[node.children[parseInt(i.toString(), 10)]]
          );
        }
      }
  ...
  private getBlazorDragEventArgs(
    args: IBlazorDragEnterEventArgs | IDragEnterEventArgs
  ): IBlazorDragEnterEventArgs {
    args = {
      // eslint-disable-next-line max-len
      source: cloneBlazorObject(args.source),
      element:
        getObjectType(args.element) === Connector
  ...
  private moveNode(node: NodeModel | ConnectorModel): void {
    const currentLayer: LayerModel = this.commandHandler.getObjectLayer(
      node.id
    );
    ...
    if (length > 1) {
      ...
      let num = 2;
      if (
        (node as NodeModel).children &&
        (node as NodeModel).children.length > 0
      ) {
        while (
          (node as NodeModel).children.indexOf(
            currentLayer.objects[length - num]
          ) > -1
        ) {
          num++;
        }
      }
      this.commandHandler.moveSvgNode(
        node.id,
        currentLayer.objects[length - num]
      );
      this.commandHandler.moveSvgNode(
      ...
      if (targetLayer) {
        const targetObject: string = this.commandHandler.getLayer(
          this.layerZIndexTable[targetLayer.zIndex]
        ).objects[0];
        if (targetObject) {
  ...
  public moveObjectsUp(
    ...
    for (let i: number = this.layers.length - 1; i >= 0; i--) {
      ...
      if (currentLayer.id !== targetLayer.id) {
        ...
        const targetObject: string = this.commandHandler.getLayer(
          this.layerZIndexTable[targetLayer.zIndex]
        ).objects[targetLayer.objects.length - 1];
        if (targetObject) {
          this.commandHandler.moveSvgNode(node.id, targetObject);
  ...
  private crudOperation(
    node: Node | Connector,
    crud: string,
    getNodesCollection: IDataSource
  ) {
    if (node) {
      const data: object = this.parameterMap(
        node,
        node instanceof Connector ? false : true
      );
      if (data) {
        // eslint-disable-next-line max-len
        const url: string =
          node instanceof Connector
            ? this.dataSourceSettings.connectionDataSource.crudAction[`${crud}`]
            : this.dataSourceSettings.crudAction[`${crud}`];
  ...
  private processCrudCollection(
    newObjects: IDataSource,
    nodeCrudAction: string,
    connectorCrudAction: string
  ): void {
    if (newObjects.nodes) {
      const data: Object[] = [];
      let i: number;
      for (i = 0; i < newObjects.nodes.length; i++) {
        data.push(
          this.parameterMap(
            newObjects.nodes[parseInt(i.toString(), 10)] as Node,
            true
          )
        );
      }
    ...
    if (newObjects.connectors) {
      ...
      for (i = 0; i < newObjects.connectors.length; i++) {
        data.push(
          this.parameterMap(
            newObjects.connectors[parseInt(i.toString(), 10)] as Connector,
            false
          )
        );
      }
  ...
  private getDeletedNodes(): IDataSource {
    ...
    for (i = 0; i < this.crudDeleteNodes.length; i++) {
      const node: NodeModel | ConnectorModel =
        this.crudDeleteNodes[parseInt(i.toString(), 10)];
  ...
  private raiseAjaxPost(value: string, url: string): void {
    const callback: Fetch = new Fetch(url, 'POST', 'application/json');
