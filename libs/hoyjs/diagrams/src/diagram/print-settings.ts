...
import { Browser, isBlazor } from '@hoyui/base';
...
export class PrintAndExport {
  ...
  public exportDiagram(options: IExportOptions): string | SVGElement {
    ...
    const region: DiagramRegions =
      options && options.region ? options.region : 'Content';
    ...
    if (mode === 'Download' && data !== null) {
      ...
      for (let b: number = 0; b < buffers.length; b++) {
        const blob: Blob = new Blob([buffers[parseInt(b.toString(), 10)]], {
          type: 'application/octet-stream',
        });
        if (Browser.info.name === 'msie') {
          window.navigator.msSaveOrOpenBlob(blob, fileName + '.' + fileType);
        } else {
          const pom: HTMLAnchorElement = createHtmlElement('a', {
  ...
  private canvasMultiplePage(
    ...
    const imageData: string = image.substring(
      image.indexOf(':') + 1,
      image.indexOf(';')
    );
    ...
    if (options.multiplePage) {
      options.pageHeight = options.pageHeight
        ? options.pageHeight
        : this.diagram.pageSettings.height;
      options.pageWidth = options.pageWidth
        ? options.pageWidth
        : this.diagram.pageSettings.width;
      options.pageHeight = options.pageHeight
        ? options.pageHeight
        : canvas.width;
      ...
      const img: HTMLImageElement = createHtmlElement(
        'img',
        attr
      ) as HTMLImageElement;
      img.onload = () => {
  ...
  private exportImage(
    ...
    for (let j: number = 0; j < buffers.length; j++) {
      const b: Blob = new Blob([buffers[parseInt(j.toString(), 10)]], {
        type: 'application/octet-stream',
      });
      if (Browser.info.name === 'msie') {
        window.navigator.msSaveOrOpenBlob(b, fileName + '.' + fileType);
      } else {
        const htmlElement: HTMLAnchorElement = createHtmlElement('a', {
          download: fileName + '.' + fileType,
        }) as HTMLAnchorElement;
  ...
  public getObjectsBound(options?: IExportOptions): Rect {
    ...
    for (const node of nodes) {
      if (node.visible) {
        if (
          (options.format !== 'SVG' &&
            !(node instanceof DiagramNativeElement) &&
            !(node instanceof DiagramHtmlElement)) ||
    ...
    if (mode !== 'Content') {
      if (this.diagram.pageSettings && this.diagram.pageSettings.multiplePage) {
        ...
        if (this.diagram.pageSettings.width) {
          left =
            Math.floor(left / this.diagram.pageSettings.width) *
            this.diagram.pageSettings.width;
          right =
            Math.ceil(rect.right / this.diagram.pageSettings.width) *
              this.diagram.pageSettings.width -
            left;
        }
        if (this.diagram.pageSettings.height) {
          top =
            Math.floor(top / this.diagram.pageSettings.height) *
            this.diagram.pageSettings.height;
          bottom =
            Math.ceil(rect.bottom / this.diagram.pageSettings.height) *
              this.diagram.pageSettings.height -
            top;
        }
  ...
  private setScaleValueforCanvas(options: IExportOptions, bounds: Rect): void {
    ...
    let pageOrientation: PageOrientation =
      options.pageOrientation || this.diagram.pageSettings.orientation;
    if (!pageOrientation) {
    ...
    if (
      ...
      if (options.stretch === 'Meet') {
        options[`${scaleX}`] = options[`${scaleY}`] = Math.min(
          options[`${scaleX}`],
          options[`${scaleY}`]
        );
        options[`${scaleOffsetY}`] =
          (options.pageHeight - bounds.height * options[`${scaleX}`]) / 2;
        options[`${scaleOffsetX}`] =
          (options.pageWidth - bounds.width * options[`${scaleX}`]) / 2;
      } else if (options.stretch === 'Slice') {
        options[`${scaleX}`] = options[`${scaleY}`] = Math.max(
          options[`${scaleX}`],
          options[`${scaleY}`]
        );
      }
  ...
  private diagramAsSvg(
    ...
    const svg: SVGElement = this.diagram.createSvg(
      this.diagram.element.id + '_diagram_svg',
      options.bounds.width,
      options.bounds.height
    );
    document.body.appendChild(svg);
    let g: SVGElement = createSvgElement('g', {
    ...
    const gradient: HTMLElement = document.getElementById(
      this.diagram.element.id + 'gradient_pattern'
    );
    if (gradient) {
    ...
    const backimage: SVGElement = document
      .getElementById(this.diagram.element.id + '_backgroundImageLayer')
      .cloneNode(true) as SVGElement;
    setAttributeSvg(backimage, attr);
    svg.appendChild(backimage);
    this.setTransform(backimage, bounds, margin);
    const backRect: SVGElement = document
      .getElementById(this.diagram.element.id + '_backgroundLayerrect')
      .cloneNode(true) as SVGElement;
    ...
    if (this.diagram.mode === 'SVG') {
      ...
      let i: number;
      const diagramLayerSVG: SVGSVGElement = getDiagramLayerSvg(
        this.diagram.element.id
      );
      svg.appendChild(
        diagramLayerSVG
          .getElementById(this.diagram.diagramLayer.id)
          .cloneNode(true)
      );
  ...
  private diagramAsCanvas(
    ...
    const canvas: HTMLCanvasElement = CanvasRenderer.createCanvas(
      this.diagram.element.id + '_diagram',
      options.bounds.width,
      options.bounds.height
    );
    ...
    context.save();
    context.fillStyle =
      this.diagram.pageSettings.background.color === 'transparent'
        ? 'white'
    ...
    if (bgImg && bgImg.source) {
      ...
      if (bgImg.scale !== 'None' && bgImg.align !== 'None') {
        const proportion: number =
          bgImg.scale === 'Meet'
            ? Math.min(proportionX, proportionY)
            : Math.max(proportionX, proportionY);
        ...
        if (
          this.diagram.pageSettings.background.color === 'none' ||
          this.diagram.pageSettings.background.color === 'transparent'
        ) {
          context.fillStyle = 'white';
          context.fillRect(
            pageBounds.x * options[`${scaleX}`],
            pageBounds.y * options[`${scaleY}`],
            pageBounds.width * options[`${scaleX}`],
        ...
        if (exportable) {
          context.drawImage(
            ...
            x,
            y,
            proportion * image.width,
            proportion * image.height
          );
        }
      } else if (exportable) {
      ...
      context.fillRect(
        pageBounds.x * options[`${scaleX}`] - margin.left,
        pageBounds.y * options[`${scaleY}`] - margin.top,
    ...
    this.diagram.renderDiagramElements(
      canvas,
      renderer,
      htmlLayer,
      false,
  ...
  private updateWrapper(
    canvas: Container | DiagramElement[],
    value: number,
    scaleOffsetX: number,
    scaleOffsetY: number,
    isExport: boolean
    ...
    if (canvas && (canvas as DiagramElement[]).length > 0) {
      for (let j: number = 0; j < (canvas as DiagramElement[]).length; j++) {
        if (canvas[parseInt(j.toString(), 10)].children) {
          this.updateWrapper(
            canvas[parseInt(j.toString(), 10)].children,
            value,
            scaleOffsetX,
            scaleOffsetY,
            isExport
  ...
  private scaleGradientValue(
    ...
    if (node.style.gradient.stops.length > 0) {
      ...
      if (node.style.gradient instanceof LinearGradient) {
        gradients.x1 = isExport
          ? gradients.x1 * scaleValue
          : gradients.x1 / scaleValue;
        gradients.y1 = isExport
          ? gradients.y1 * scaleValue
          : gradients.y1 / scaleValue;
        gradients.x2 = isExport
          ? gradients.x2 * scaleValue
          : gradients.x2 / scaleValue;
        gradients.y2 = isExport
          ? gradients.y2 * scaleValue
          : gradients.y2 / scaleValue;
      } else {
        gradients.fx = isExport
          ? gradients.fx * scaleValue
          : gradients.fx / scaleValue;
        gradients.fy = isExport
          ? gradients.fy * scaleValue
          : gradients.fy / scaleValue;
        gradients.cx = isExport
          ? gradients.cx * scaleValue
          : gradients.cx / scaleValue;
        gradients.cy = isExport
          ? gradients.cy * scaleValue
          : gradients.cy / scaleValue;
        gradients.r = isExport
          ? gradients.r * scaleValue
          : gradients.r / scaleValue;
      }
    }
  }
  private updateObjectValue(
    ...
    for (let j: number = 0; j < this.diagram.connectors.length; j++) {
      ...
      for (let k: number = 0; k < wrapper.children.length; k++) {
        ...
        if (isExport) {
          wrapper.children[parseInt(k.toString(), 10)].exportScaleValue.x =
            value;
          wrapper.children[parseInt(k.toString(), 10)].exportScaleValue.y =
            value;
          wrapper.children[parseInt(k.toString(), 10)].exportScaleOffset.x =
            scaleOffsetX;
          wrapper.children[parseInt(k.toString(), 10)].exportScaleOffset.y =
            scaleOffsetY;
        }
      }
    }
  ...
  private isImageExportable(
    backgroundImage: BackgroundModel | ImageModel
  ): boolean {
    ...
    if (backgroundImage.source) {
      try {
        canvas = CanvasRenderer.createCanvas(
          this.diagram.element.id + 'temp_canvas',
          100,
          100
        );
  ...
  private getPrintCanvasStyle(
    ...
    if (options.pageOrientation) {
      if (
        (options.pageOrientation === 'Landscape' && height > width) ||
        (options.pageOrientation === 'Portrait' && width > height)
      ) {
    ...
    if (options && options.multiplePage) {
      ...
      do {
        do {
          ...
          canvas = CanvasRenderer.createCanvas(
            this.diagram.element.id + '_multiplePrint',
            pageSize.width,
            pageSize.height
          );
      ...
      ctx.drawImage(
        ...
        x + mLeft,
        y + mTop,
        img.width - (mRight + mLeft),
        img.height - (mTop + mBottom),
        0 + mLeft,
        0 + mTop,
        pageWidth - (mRight + mLeft),
        pageHeight - (mTop + mBottom)
      );
  ...
  private printImage(
    ...
    const innerDiv: HTMLDivElement = createHtmlElement(
      'div',
      attr
    ) as HTMLDivElement;
    attr = {
  ...
  private getContent(styleSheets?: StyleSheetList): string {
    ...
    for (let i: number = 0; i < styleSheets.length; i++) {
      if (
        styleSheets[parseInt(i.toString(), 10)].href ||
        typeof styleSheets[parseInt(i.toString(), 10)] === 'string'
      ) {
        styleSheetRef +=
          "<link href='" +
          (styleSheets[parseInt(i.toString(), 10)].href ||
            styleSheets[parseInt(i.toString(), 10)]) +
          "' rel='stylesheet' />";
      }
    }
    let htmlData: string = document.getElementById(
      this.diagram.element.id + 'content'
    ).innerHTML;
    const marginStyle: string =
      'margin-left:' +
      0 +
      'px;margin-top:' +
      0 +
  ...
  public getDiagramContent(styleSheets?: StyleSheetList): string {
    if (this.diagram.scroller.currentZoom === 1) {
      ...
      const oldHorizontalOffset: number =
      ...
      const oldWidth: number = Number(String(this.diagram.width).split('%')[0])
        ...
        : Number(String(this.diagram.width).split('px')[0]);
      const oldHeight: number = Number(
        String(this.diagram.height).split('%')[0]
      )
        ? container.clientHeight
        : Number(String(this.diagram.height).split('px')[0]);
      ...
      this.diagram.scroller.zoom(
        oldZoom / this.diagram.scrollSettings.currentZoom
      );
      this.diagram.dataBind();
  ...
  public exportImages(image: string, options: IExportOptions): void {
    ...
    img.onload = () => {
      const canvas: HTMLCanvasElement = CanvasRenderer.createCanvas(
        context.diagram.element.id + 'innerImage',
        bounds.width + (margin.left + margin.right),
        bounds.height + (margin.top + margin.bottom)
      );
      const ctx: CanvasRenderingContext2D = canvas.getContext('2d');
