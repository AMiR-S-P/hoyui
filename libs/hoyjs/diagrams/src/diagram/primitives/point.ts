...
export class Point extends ChildProperty<Point> {
  ...
  public static transform(
    ...
    pt.x =
      Math.round((point.x + length * Math.cos((angle * Math.PI) / 180)) * 100) /
      100;
    pt.y =
      Math.round((point.y + length * Math.sin((angle * Math.PI) / 180)) * 100) /
      100;
    return pt as Point;
  }
   ...
   * @private
   */
  public static adjustPoint(
    source: PointModel,
    target: PointModel,
    isStart: boolean,
    length: number
  ): PointModel {
    let pt: PointModel = isStart
      ? { x: source.x, y: source.y }
      : { x: target.x, y: target.y };
    let angle: number;
    if (source.x === target.x) {
      if (
        (source.y < target.y && isStart) ||
        (source.y > target.y && !isStart)
      ) {
        pt.y += length;
      } else {
