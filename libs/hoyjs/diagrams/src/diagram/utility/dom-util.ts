...
import { compile, createElement, Browser, isBlazor } from '@hoyui/base';
...
function getTextOptions(
  ...
  (options as TextAttributes).whiteSpace = whiteSpaceToString(
    element.style.whiteSpace,
    element.style.textWrapping
  );
  (options as TextAttributes).content = element.content;
  (options as TextAttributes).textWrapping = element.style.textWrapping;
  (options as TextAttributes).breakWord = wordBreakToString(
    element.style.textWrapping
  );
  (options as TextAttributes).textAlign = textAlignToString(
 ...
 * @private
 */
function wordWrapping(
  text: TextAttributes,
  textValue?: string,
  laneWidth?: number
): SubTextElement[] {
  const childNodes: SubTextElement[] = [];
  let txtValue: string = '';
  let j: number = 0;
  let i: number = 0;
  ...
  for (j = 0; j < eachLine.length; j++) {
    ...
    words =
      text.textWrapping !== 'NoWrap'
        ? eachLine[parseInt(j.toString(), 10)].split(' ')
        : text.textWrapping === 'NoWrap'
        ? [eachLine[parseInt(j.toString(), 10)]]
        : eachLine;
    for (i = 0; i < words.length; i++) {
      txtValue +=
        ((i !== 0 || words.length === 1) && wrap && txtValue.length > 0
          ? ' '
          : '') + words[parseInt(i.toString(), 10)];
      ...
      if (
        Math.floor(width) > (laneWidth || text.width) - 2 &&
        txtValue.length > 0
      ) {
        childNodes[childNodes.length] = {
          text: txtValue,
...
export function measureHtmlText(
  ...
  text.style.whiteSpace = whiteSpaceToString(
    style.whiteSpace,
    style.textWrapping
  );
  if (maxWidth !== undefined) {
...
export function measureText(
  ...
  const options: TextAttributes = getTextOptions(
    text,
    maxWidth
  ) as TextAttributes;
  text.childNodes = childNodes = wrapSvgText(
    options,
    textValue,
    text.isLaneOrientation ? maxWidth : undefined
  );
...
export function measureImage(
  ...
  const imageElement: HTMLImageElement =
    window[`${measureWindowElement}`].children[1];
...
export function measureNativeSvg(nativeContent: SVGElement): Rect {
  ...
  const svgBounds: Rect | any =
    nativeSVG.getBoundingClientRect() as unknown as Rect;
  nativeSVG.removeChild(nativeContent);
  window[measureWindowElement].style.visibility = 'hidden';
  return svgBounds;
}
...
export function updatePath(
  ...
  if (
    element.actualSize.width !== bBox.width ||
    element.actualSize.height !== bBox.height ||
    options
  ) {
    scaleX =
      ((options && options.width) || element.actualSize.width) /
      Number(bBox.width ? bBox.width : 1);
    scaleY =
      ((options && options.height) || element.actualSize.height) /
      Number(bBox.height ? bBox.height : 1);
    isScale = true;
  }
...
export function getDiagramLayerSvg(diagramId: string): SVGSVGElement {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const elementcoll: NodeList | any =
    diagramElement.getElementsByClassName('e-diagram-layer');
...
export function getDiagramElement(
  ...
  if (Browser.info.name === 'msie' || Browser.info.name === 'edge') {
    diagramElement = element
      ? (element.querySelector('#' + elementId) as HTMLElement)
      : document.getElementById(elementId);
  } else {
    diagramElement = element
      ? (element.querySelector('#' + CSS.escape(elementId)) as HTMLElement)
      : document.getElementById(elementId);
  }
...
export function getDomIndex(
  ...
  for (
    let i: number = 0;
    parentElement.childNodes && i < parentElement.childNodes.length;
    i++
  ) {
...
export function getAdornerLayerSvg(diagramId: string): SVGSVGElement {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const elementcoll: NodeList | any =
    diagramElement.getElementsByClassName('e-adorner-layer');
...
export function getAdornerLayer(diagramId: string): SVGElement {
  ...
  adornerLayer = diagramAdornerSvg.getElementById(
    diagramId + '_diagramAdorner'
  ) as SVGElement;
  return adornerLayer;
}
...
export function getUserHandleLayer(diagramId: string): HTMLElement {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const elementcoll: NodeList | any =
    diagramUserHandleLayer.getElementsByClassName('e-userHandle-layer');
...
export function getDiagramLayer(diagramId: string): SVGElement {
  ...
  const diagramLayer: SVGElement = diagramLayerSvg.getElementById(
    diagramId + '_diagramLayer'
  ) as SVGElement;
  return diagramLayer;
}
...
export function getPortLayerSvg(diagramId: string): SVGSVGElement {
  ...
  const diagramElement: HTMLElement = getDiagramElement(diagramId);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const elementcoll: NodeList | any = diagramElement.getElementsByClassName(
    'e-ports-expand-layer'
  );
  adornerLayerSvg = elementcoll[0] as SVGSVGElement;
  return adornerLayerSvg;
}
...
export function getNativeLayerSvg(diagramId: string): SVGSVGElement {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const elementcoll: NodeList | any =
    diagramElement.getElementsByClassName('e-native-layer');
...
export function getGridLayerSvg(diagramId: string): SVGSVGElement {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const elementcoll: NodeList | any =
    diagramElement.getElementsByClassName('e-grid-layer');
...
export function getBackgroundLayerSvg(diagramId: string): SVGSVGElement {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const elementcoll: NodeList | any =
    diagramElement.getElementsByClassName('e-background-layer');
...
export function getGridLayer(diagramId: string): SVGElement {
  ...
  if (!window[domTable][diagramId + '_gridline']) {
    ...
    expandCollapse = diagramGridSvg.getElementById(
      diagramId + '_gridline'
    ) as SVGElement;
...
export function getNativeLayer(diagramId: string): SVGElement {
  ...
  nativeLayer = nativeLayerSvg.getElementById(
    diagramId + '_nativeLayer'
  ) as SVGElement;
  return nativeLayer;
}
...
export function getHTMLLayer(diagramId: string): HTMLElement {
  ...
  if (!window[domTable][diagramId + 'html_layer']) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const elementcoll: NodeList | any =
      element.getElementsByClassName('e-html-layer');
...
export function parentsUntil(
  ...
  while (parent) {
    if (
      isID ? parent.id === selector : hasClass(parent as HTMLElement, selector)
    ) {
      break;
    }
...
export function hasClass(element: HTMLElement, className: string): boolean {
  const eClassName: string | SVGAnimatedString =
    typeof element.className === 'object'
      ? (element.className as SVGAnimatedString).animVal
      : element.className;
  return (' ' + eClassName + ' ').indexOf(' ' + className + ' ') > -1
    ? true
    : false;
}
/**
...
export function getScrollerWidth(): number {
  const outer: HTMLElement = createHtmlElement('div', {
 ...
 * @private
 */
export function addTouchPointer(
  touchList: ITouches[],
  e: PointerEvent,
  touches: TouchList
): ITouches[] {
  ...
  for (let i: number = 0, length: number = touches.length; i < length; i++) {
    touchList.push({
      pageX: touches[parseInt(i.toString(), 10)].clientX,
      pageY: touches[parseInt(i.toString(), 10)].clientY,
      pointerId: null,
    });
  }
  return touchList;
...
export function getContent(
  ...
  if (node instanceof Node) {
    ...
    if (node.shape.type === 'Native') {
      ...
      const div: SVGSVGElement = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'svg'
      );
      ...
      /* tslint:disable */
      svgContent =
        div.getElementsByTagName('svg').length > 0
          ? div.getElementsByTagName('svg')[0].outerHTML
          : div.getElementsByTagName('g').length > 0
          ? div.getElementsByTagName('g')[0].outerHTML
          : '';
      /* tslint:disable */
  ...
  if (
    ...
    !(element as DiagramHtmlElement).isTemplate
  ) {
    const template: HTMLElement = document.getElementById(
  ...
  return (element as DiagramHtmlElement).isTemplate
    ? div
    : isHtml
    ? (div.cloneNode(true) as HTMLElement)
    : (div.cloneNode(true) as SVGElement);
}
/* eslint-enable */
 ...
 * @private
 */
export function applyStyleAgainstCsp(
 ...
 * @private
 */
export function setAttributeHtml(
  element: HTMLElement,
  attributes: Object
): void {
  const keys: string[] = Object.keys(attributes);
...
export function createMeasureElements(): void {
  ...
  if (!window[`${measureWindowElement}`]) {
    ...
    const imageElement: HTMLImageElement = createHtmlElement('img', {
    ...
    const element: SVGPathElement = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'path'
    );
    ...
    const tSpan: SVGTextElement = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'text'
    );
...
export function getTemplateContent(
  ...
  annotationcontent: DiagramHtmlElement,
  ...
  if (
    ...
    annotationcontent.template = annotationcontent.content = getContent(
      annotationcontent,
      true,
      annotation
    ) as HTMLElement;
...
export function createUserHandleTemplates(
  ...
  if (userHandleTemplate && template) {
    ...
    for (handle of selectedItems.userHandles) {
      if (userHandleFn) {
        compiledString = userHandleFn(
          cloneObject(handle),
          diagram,
          'userHandleTemplate' + '_' + handle.name,
          undefined,
          undefined,
          false
        );
