...
import {
  TransformFactor as Transforms,
  Segment,
} from '../interaction/scroller';
...
import { isBlazor, Browser, isNullOrUndefined } from '@hoyui/base';
...
export function completeRegion(
  ...
  for (let i: number = 0; i < selectedObjects.length; i++) {
    const obj: NodeModel | ConnectorModel =
      selectedObjects[parseInt(i.toString(), 10)];
...
export function findObjectType(
  ...
  if (drawingObject) {
    if ((drawingObject as Connector).type) {
      ...
      (drawingObject as Node).shape &&
      !(drawingObject as Connector).type
    ) {
      type = 'Node';
    }
  }
 ...
 * @private
 */
export function setSwimLaneDefaults(
  child: NodeModel | ConnectorModel,
 ...
 * @private
 */
export function setUMLActivityDefaults(
  child: NodeModel | ConnectorModel,
  node: NodeModel | ConnectorModel
): void {
  if (node instanceof Node) {
    const shape: UmlActivityShapes = (child.shape as UmlActivityShape).shape;
    switch (shape) {
      case 'JoinNode':
    ...
    switch (flow) {
      case 'Object':
        ...
        if (
          !(child as ConnectorModel).targetDecorator ||
          !(child as ConnectorModel).targetDecorator.shape
        ) {
      ...
      case 'Control':
        ...
        if (
          !(child as ConnectorModel).sourceDecorator ||
          !(child as ConnectorModel).sourceDecorator.shape
        ) {
 ...
 * @private
 */
export function setConnectorDefaults(
  child: ConnectorModel,
  node: ConnectorModel
): void {
  switch (child.shape.type) {
    case 'Bpmn':
      ...
      switch (bpmnFlow) {
        case 'Sequence':
          if (
            ((child.shape as BpmnFlow).sequence === 'Normal' &&
              child.type !== 'Bezier') ||
            (child.shape as BpmnFlow).sequence === 'Default' ||
            (child.shape as BpmnFlow).sequence === 'Conditional'
          ) {
            if (node.targetDecorator && node.targetDecorator.style) {
              node.targetDecorator.style.fill =
                (child.targetDecorator &&
                  child.targetDecorator.style &&
                  child.targetDecorator.style.fill) ||
                'black';
            }
            if (
              (child.shape as BpmnFlow).sequence === 'Conditional' &&
              node.sourceDecorator
            ) {
              if (node.sourceDecorator.style) {
                node.sourceDecorator.style.fill =
                  (child.sourceDecorator &&
                    child.sourceDecorator.style &&
                    child.sourceDecorator.style.fill) ||
                  'white';
              }
              node.sourceDecorator.width =
                (child.sourceDecorator && child.sourceDecorator.width) || 20;
              node.sourceDecorator.height =
                (child.sourceDecorator && child.sourceDecorator.width) || 10;
            }
          }
          break;
        case 'Association':
          if (
            (child.shape as BpmnFlow).association === 'Default' ||
            ...
            if ((child.shape as BpmnFlow).association === 'BiDirectional') {
              if (node.sourceDecorator && node.sourceDecorator.style) {
                node.sourceDecorator.style.fill =
                  ...
                  'white';
                node.sourceDecorator.width =
                  (child.sourceDecorator && child.sourceDecorator.width) || 5;
                node.sourceDecorator.height =
                  (child.sourceDecorator && child.sourceDecorator.height) || 10;
              }
            }
          }
          break;
        case 'Message':
          if (node.style && !node.style.strokeDashArray) {
            node.style.strokeDashArray =
              (child.style && child.style.strokeDashArray) || '4 4';
          }
          break;
      }
      break;
      ...
      switch (flow) {
        case 'Exception':
          if (
            ...
            node.style.strokeDashArray =
              (child.style && child.style.strokeDashArray) || '2 2';
          }
          break;
      }
      break;
    case 'UmlClassifier':
      ...
      if ((child.shape as RelationShip).relationship === 'Association') {
        ...
        (child.shape as RelationShip).relationship === 'Aggregation' ||
...
export function findNearestPoint(
  ...
  const vaAngle: number =
    angleBWStAndRef + (angleBWStAndEnd - angleBWStAndRef) * 2;
  return {
...
function pointsForBezier(connector: ConnectorModel): PointModel[] {
  ...
  if (connector.type === 'Bezier') {
    ...
    for (let i: number = 0; i < connector.segments.length; i++) {
      const tolerance: number = 1.5;
      const segment: BezierSegment = connector.segments[
        parseInt(i.toString(), 10)
      ] as BezierSegment;
      //const pt: PointModel = { x: 0, y: 0 };
      const point1: PointModel = !Point.isEmptyPoint(segment.point1)
        ? segment.point1
        : segment.bezierPoint1;
      const point2: PointModel = !Point.isEmptyPoint(segment.point2)
        ? segment.point2
        : segment.bezierPoint2;
      const max: number = Number(
        ((connector as Connector).distance(point1, segment.points[0]) +
          (connector as Connector).distance(point2, point1) +
          (connector as Connector).distance(segment.points[1], point2)) /
          tolerance
      );
      for (let j: number = 0; j < max - 1; j = j + 10) {
        points[parseInt(k.toString(), 10)] = bezierPoints(
          connector,
          segment.points[0],
          !Point.isEmptyPoint(segment.point1)
            ? segment.point1
            : segment.bezierPoint1,
          !Point.isEmptyPoint(segment.point2)
            ? segment.point2
            : segment.bezierPoint2,
          segment.points[1],
          j,
...
export function updateDefaultValues(
  actualNode: NodeModel | ConnectorModel,
  plainValue: NodeModel | ConnectorModel,
  // eslint-disable-next-line @typescript-eslint/ban-types
  defaultValue: object,
  property?: NodeModel | ConnectorModel,
  oldKey?: string
): void {
  if (
    defaultValue &&
    (actualNode instanceof Connector ||
      (actualNode &&
        ((actualNode.shape && actualNode.shape.type !== 'SwimLane') ||
          actualNode.shape === undefined)))
  ) {
    // eslint-disable-next-line @typescript-eslint/ban-types
    let keyObj: object;
        ...
        if (
          ...
          if (actualNode[`${key}`].length > 0) {
            for (let i: number = 0; i <= actualNode[`${key}`].length; i++) {
              updateDefaultValues(
                actualNode[`${key}`],
                plainValue ? plainValue[`${key}`] : undefined,
                defaultValue[`${key}`],
                key === 'annotations' || key === 'ports'
                  ? actualNode
                  : undefined,
                key
              );
            ...
            updateDefaultValues(
              ...
              plainValue ? plainValue[`${key}`] : undefined,
              defaultValue[`${key}`],
              key === 'annotations' || key === 'ports' ? actualNode : undefined,
              key
            );
          }
        } else if (
          keyObj instanceof Object &&
          plainValue &&
          oldKey !== 'annotations' &&
          oldKey !== 'ports'
        ) {
            ...
            if (oldKey === 'annotations') {
              ...
              if (!actualNode[`${key}`]) {
                if (getObjectType(property) === Connector) {
                  createObject = new PathAnnotation(
                    property,
                    'annotations',
                    defaultValue[`${key}`]
                  );
                  ...
                  createObject = new ShapeAnnotation(
                    property,
                    'annotations',
                    defaultValue[`${key}`]
                  );
 ...
 */
// eslint-disable-next-line @typescript-eslint/ban-types
export function updateLayoutValue(
  actualNode: TreeInfo,
  defaultValue: object,
  nodes?: INode[],
  node?: INode
        ...
        if (node.data[`${assistantKey}`] === defaultValue[`${key}`]['root']) {
          ...
          for (let i: number = 0; i < assitants.length; i++) {
            for (let j: number = 0; j < nodes.length; j++) {
              if (
                nodes[parseInt(j.toString(), 10)].data[`${assistantKey}`] ===
                assitants[parseInt(i.toString(), 10)]
              ) {
                actualNode.assistants.push(
                  nodes[parseInt(j.toString(), 10)].id
                );
  ...
  if (!actualNode.hasSubTree && (defaultValue as TreeInfo).canEnableSubTree) {
    ...
    if (
      (node as Node).layoutInfo.offset !== actualNode.offset &&
      (node as Node).layoutInfo.offset !== undefined
    ) {
 ...
 * @private
 */
export function isPointOverConnector(
  connector: ConnectorModel,
  reference: PointModel
): boolean {
  //let intermediatePoints: PointModel[];
  const intermediatePoints: PointModel[] =
    connector.type === 'Bezier'
      ? pointsForBezier(connector)
  ...
  for (let i: number = 0; i < intermediatePoints.length - 1; i++) {
    ...
    if (rect.containsPoint(reference)) {
      ...
      if (intersectDetails.enabled) {
        const distance: number = Point.findLength(
          reference,
          intersectDetails.intersectPt
        );
...
export function intersect3(
  ...
  if (
    d === 0 ||
    ((lineUtil1.x1 === lineUtil2.x1 || lineUtil1.y1 === lineUtil2.y1) &&
      (lineUtil1.x2 === lineUtil2.x2 || lineUtil1.y2 === lineUtil2.y2) &&
      (na === 0 || nb === 0) &&
      d > 0)
  ) {
 ...
 * @private
 */
export function getTooltipOffset(
  diagram: Diagram,
  mousePosition: PointModel,
  node: NodeModel | ConnectorModel | PointPortModel,
  type?: string
): PointModel {
  //let offset: PointModel;
  const inheritTooltip: number =
    node instanceof Node
      ? (node as NodeModel).constraints & NodeConstraints.InheritTooltip
      : node instanceof Connector
      ? (node as ConnectorModel).constraints &
        ConnectorConstraints.InheritTooltip
      : (node as PointPort).constraints & PortConstraints.InheritTooltip;
  const objectTooltip: number =
    node instanceof Node
      ? (node as NodeModel).constraints & NodeConstraints.Tooltip
      : node instanceof Connector
      ? (node as ConnectorModel).constraints & ConnectorConstraints.Tooltip
      : (node as PointPort).constraints & PortConstraints.ToolTip;
  let isMouseBased: boolean =
    (!inheritTooltip && objectTooltip
      ? node.tooltip.relativeMode
  ...
  const offset: PointModel = tooltipOffset(
    node,
    mousePosition,
    diagram,
    isMouseBased
...
function tooltipOffset(
  ...
  if (node instanceof Node || node instanceof Connector) {
    ...
    const obj: IElement = diagram.findObjectUnderMouse(
      objects,
      'Select',
      false
    );
    const portElement: DiagramElement = diagram.findElementUnderMouse(
      obj,
      mousePosition,
      diagram
    );
...
function offsetPoint(
  ...
  point.x =
    (isMouseBased ? mousePosition.x : bound.x) * scale + horizontalOffset - x;
  point.y =
    (isMouseBased ? mousePosition.y : bound.y) * scale + verticalOffset - y;
  return point;
}
...
export function initFixedUserHandlesSymbol(
  options: ConnectorFixedUserHandleModel | NodeFixedUserHandleModel,
  fixedUserHandleContainer: Canvas
): DiagramElement {
  //let fixedUserHandleContent: PathElement | DiagramNativeElement;
  const fixedUserHandleContent: PathElement | DiagramNativeElement =
    new PathElement();
  fixedUserHandleContent.data = options.pathData;
  fixedUserHandleContent.height =
    options.height > 10
      ? options.height - (options.padding.bottom + options.padding.top)
      : options.height;
  fixedUserHandleContent.width =
    options.width > 10
      ? options.width - (options.padding.left + options.padding.right)
      : options.width;
  ...
  fixedUserHandleContent.description =
    fixedUserHandleContainer.description || '';
  ...
  for (i = 0; i < objects.length; i++) {
    ...
    for (j = i + 1; j < objects.length; j++) {
      const bounds: Rect = getBounds(
        objects[parseInt(j.toString(), 10)].wrapper
      );
      if (
        option === 'Top' ||
        option === 'Bottom' ||
...
export function getAnnotationPosition(
  pts: PointModel[],
  annotation: PathAnnotation | ConnectorFixedUserHandle,
  bound: Rect
): SegmentInfo {
  //let angle: number;
  ...
  const point: PointModel = Point.transform(
    getloop.point,
    angle + 45,
    alignednumber
  );
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export function getPortsPosition(
  pts: PointModel[],
  ports: Port,
  bound: Rect
): SegmentInfo {
 ...
 * @private
 */
export function getOffsetOfPorts(
  points: PointModel[],
  ports: Port
): SegmentInfo {
  // eslint-disable-next-line
  let distance: number = 0;
  let offset: number = (ports as PathPort).offset;
  let point: PointModel;
  let angle: number;
  ...
  for (let j: number = 0; j < points.length - 1; j++) {
    distance += Point.distancePoints(
      points[parseInt(j.toString(), 10)],
      points[j + 1]
    );
 ...
 * @private
 */
export function getOffsetOfConnector(
  points: PointModel[],
  annotation: PathAnnotation | ConnectorFixedUserHandle
): SegmentInfo {
  // eslint-disable-next-line
  let length: number = 0;
  let offset: number = annotation.offset;
  let point: PointModel;
  let angle: number;
 ...
 * @private
 */
export function alignLabelOnSegments(
  obj: PathAnnotation | ConnectorFixedUserHandle | PathPort,
  ang: number,
  pts: PointModel[]
): Alignment {
  //let angle: number = ang % 360;
  ang %= 360;
  const fourty5: number = 45;
  const one35: number = 135;
  const two25: number = 225;
  const three15: number = 315;
  let vAlign: string;
  let hAlign: string;
  switch (obj.alignment) {
    ...
    case 'After':
      ...
      break;
    case 'Center':
      hAlign = !isNullOrUndefined((obj as PathAnnotation).horizontalAlignment)
        ? ((obj as PathAnnotation).horizontalAlignment as string).toLowerCase()
        : 'center';
      vAlign = !isNullOrUndefined((obj as PathAnnotation).verticalAlignment)
        ? ((obj as PathAnnotation).verticalAlignment as string).toLowerCase()
        : 'center';
      break;
  }
...
export function removeChildNodes(node: NodeModel, diagram: Diagram): void {
  if (node instanceof Node && node.children) {
    for (let i: number = 0; i < node.children.length; i++) {
      if (
        diagram.nameTable[node.children[parseInt(i.toString(), 10)]].children
      ) {
        removeChildNodes(node, diagram);
      }
      diagram.removeFromAQuad(
        diagram.nameTable[node.children[parseInt(i.toString(), 10)]]
      );
      diagram.removeObjectsFromLayer(
        diagram.nameTable[node.children[parseInt(i.toString(), 10)]]
      );
...
function preventDefaults(
  ...
  for (const property of properties) {
    ...
    if (clonedObject[`${property}`] instanceof Array) {
      ...
      clonedObject[`${property}`] instanceof Object &&
      defaultObject &&
      defaultObject[`${property}`]
    ) {
      if (property !== 'wrapper') {
        clonedObject[`${property}`] = preventDefaults(
      ...
      (defaultObject &&
        clonedObject[`${property}`] === defaultObject[`${property}`]) ||
      clonedObject[`${property}`] === undefined
    ) {
      if (
        !(isNodeShape && property === 'type') &&
        !(model instanceof SwimLane && property === 'orientation')
      ) {
...
function preventArrayDefaults(
  ...
  if (clonedObject[`${property}`].length === 0) {
    ...
    clonedObject[property].every((element: any): boolean => {
      return typeof element === 'number';
    })
  ) {
    let i: number;
    let isSameArray: boolean = true;
    for (i = 0; i < clonedObject[`${property}`].length; i++) {
      if (
        isSameArray &&
        clonedObject[`${property}`][parseInt(i.toString(), 10)] ===
          defaultObject[`${property}`][parseInt(i.toString(), 10)]
      ) {
        isSameArray = true;
      } else {
        isSameArray = false;
    ...
    if (clonedObject[`${property}`] && property !== 'layers') {
      for (i = clonedObject[`${property}`].length - 1; i >= 0; i--) {
        if (property === 'nodes' || property === 'connectors') {
          clonedObject[`${property}`][parseInt(i.toString(), 10)].wrapper =
            null;
        }
        if (property !== 'dataManager') {
          clonedObject[`${property}`][parseInt(i.toString(), 10)] =
            preventDefaults(
              clonedObject[`${property}`][parseInt(i.toString(), 10)],
              model[`${property}`][parseInt(i.toString(), 10)],
              defaultObject[`${property}`] !== undefined
                ? defaultObject[`${property}`][parseInt(i.toString(), 10)]
                : []
            );
          if (
  ...
  if (model[getClassName]) {
    ...
    switch (model[getClassName]()) {
      ...
      case 'Node':
        ...
        break;
      case 'Path':
        constructor = new Path(parent as Shape, '', obj);
        break;
      case 'Native':
        constructor = new Native(parent as Shape, '', obj);
        break;
      case 'Html':
        constructor = new Html(parent as Shape, '', obj);
        break;
      case 'Image':
        constructor = new Image(parent as Shape, '', obj);
        break;
      case 'Text':
        constructor = new Text(parent as Shape, '', obj);
        break;
      case 'BasicShape':
        constructor = new BasicShape(parent as Shape, '', obj);
        break;
      case 'FlowShape':
        constructor = new FlowShape(parent as Shape, '', obj);
        break;
      case 'BpmnShape':
        constructor = new BpmnShape(parent as Shape, '', obj);
        break;
      case 'UmlActivityShape':
        constructor = new UmlActivityShape(parent as Shape, '', obj);
        break;
      case 'UmlClassifierShape':
        constructor = new UmlClassifierShape(parent as Shape, '', obj);
        break;
      case 'SwimLane':
        constructor = new SwimLane(parent as Shape, '', obj);
        if ((model as SwimLane).header) {
          (constructor as SwimLane).header = new Header(
            parent as Shape,
            '',
            obj
          );
      ...
      case 'PointPort':
        ...
        break;
      case 'BpmnGateway':
        constructor = new BpmnGateway(parent as BpmnGateway, '', obj);
        break;
      case 'BpmnDataObject':
        constructor = new BpmnDataObject(parent as BpmnDataObject, '', obj);
        break;
      case 'BpmnEvent':
        constructor = new BpmnEvent(parent as BpmnEvent, '', obj);
        break;
      case 'BpmnSubEvent':
        constructor = new BpmnSubEvent(parent as BpmnSubEvent, '', obj);
        break;
      case 'BpmnActivity':
        constructor = new BpmnActivity(parent as BpmnActivity, '', obj);
        break;
      case 'BpmnAnnotation':
        constructor = new BpmnAnnotation(parent, '', obj);
        break;
      case 'MethodArguments':
        constructor = new MethodArguments(parent as MethodArguments, '', obj);
        break;
      case 'UmlClassAttribute':
        constructor = new UmlClassAttribute(parent as MethodArguments, '', obj);
        break;
      case 'UmlClassMethod':
        constructor = new UmlClassMethod(parent as MethodArguments, '', obj);
        break;
      case 'UmlClass':
        constructor = new UmlClass(parent as UmlClass, '', obj);
        break;
      case 'UmlInterface':
        constructor = new UmlInterface(parent as UmlClass, '', obj);
        break;
      case 'UmlEnumerationMember':
        constructor = new UmlEnumerationMember(
          parent as UmlEnumerationMember,
          '',
          obj
        );
        break;
      case 'UmlEnumeration':
        constructor = new UmlEnumeration(parent as UmlEnumeration, '', obj);
        break;
      case 'Lane':
        constructor = new Lane(parent as Shape, '', obj);
        break;
      case 'Phase':
        constructor = new Phase(parent as Shape, '', obj);
        break;
      case 'ChildContainer':
      ...
      case 'PathAnnotation':
        ...
        break;
      case 'Stop':
        constructor = new Stop(parent as Stop, '', obj);
        break;
      case 'Point':
        if (!defaultObject) {
        ...
        break;
      case 'UserHandle':
        constructor = new UserHandle(parent as UserHandle, '', obj);
        break;
      case 'Command':
        constructor = new Command(parent as Command, '', obj);
        break;
    }
  } else {
    constructor = defaultObject;
...
export function deserialize(model: string | Object, diagram: Diagram): Object {
  ...
  diagram.blazorActions = diagram.addConstraints(
    blazorAction,
    BlazorAction.ClearObject
  );
  diagram.clear();
  diagram.blazorActions = diagram.removeConstraints(
    blazorAction,
    BlazorAction.ClearObject
  );
  ...
  const arrangeTickHorizontal: Function | string =
    diagram.rulerSettings.horizontalRuler.arrangeTick;
  const arrangeTickVertical: Function | string =
    diagram.rulerSettings.verticalRuler.arrangeTick;
  ...
  if (dataObj.connectors) {
    for (let i: number = 0; i < dataObj.connectors.length; i++) {
      ...
      if (
        dataObj.connectors[i].type === 'Bezier' &&
        dataObj.connectors[i].segments.length > 0 &&
        dataObj.connectors[i].bezierSettings &&
        dataObj.connectors[i].bezierSettings.allowSegmentsReset
      ) {
  ...
  if (dataObj.nodes) {
    for (let i: number = 0; i < dataObj.nodes.length; i++) {
      if (
        dataObj.nodes[i].shape &&
        dataObj.nodes[i].shape.type === 'SwimLane'
      ) {
  ...
  for (let i: number = 0; i < diagram.views.length; i++) {
    ...
    if (component instanceof Overview) {
      (component as Overview).onPropertyChanged(
        { sourceID: (component as Overview).sourceID },
        {}
      );
    }
    diagram.blazorActions = diagram.removeConstraints(
...
export function upgrade(dataObj: Diagram): Diagram {
  if (
    dataObj &&
    (dataObj.version === undefined || dataObj.version < 17.1) &&
    dataObj.nodes
  ) {
  ...
  for (const key of Object.keys(changedGradient)) {
    switch (key) {
      case 'type':
        ...
        break;
      case 'x1':
        (targetGradient as LinearGradient).x1 = (
          changedGradient as LinearGradient
        ).x1;
        break;
      case 'x2':
        (targetGradient as LinearGradient).x2 = (
          changedGradient as LinearGradient
        ).x2;
        break;
      case 'y1':
        (targetGradient as LinearGradient).y1 = (
          changedGradient as LinearGradient
        ).y1;
        break;
      case 'y2':
        (targetGradient as LinearGradient).y2 = (
          changedGradient as LinearGradient
        ).y2;
        break;
      case 'cx':
        (targetGradient as RadialGradient).cx = (
          changedGradient as RadialGradient
        ).cx;
        break;
      case 'cy':
        (targetGradient as RadialGradient).cy = (
          changedGradient as RadialGradient
        ).cy;
        break;
      case 'fx':
        (targetGradient as RadialGradient).fx = (
          changedGradient as RadialGradient
        ).fx;
        break;
      case 'fy':
        (targetGradient as RadialGradient).fy = (
          changedGradient as RadialGradient
        ).fy;
        break;
      case 'r':
        (targetGradient as RadialGradient).r = (
          changedGradient as RadialGradient
        ).r;
        break;
      case 'stops':
...
export function updateHyperlink(
  changedObject: HyperlinkModel,
  target: DiagramElement,
  actualAnnotation: AnnotationModel
): void {
  const textElement: TextElement = target as TextElement;
  ...
  for (const key of Object.keys(changedObject)) {
    switch (key) {
      case 'color':
        ...
        break;
      case 'content':
        textElement.content = hyperlink.content =
          changedObject.content || hyperlink.link;
        break;
      case 'link':
        const labelStyle: TextStyleModel = actualAnnotation.style;
        textElement.style.color = changedObject.link
          ? hyperlink.color
          : labelStyle.color;
        textElement.style.textDecoration = changedObject.link
          ? hyperlink.textDecoration
          : actualAnnotation.style.textDecoration;
        textElement.content = changedObject.link
          ? hyperlink.content || changedObject.link
          : actualAnnotation.content;
        hyperlink.link = changedObject.link;
        break;
      case 'textDecoration':
        textElement.style.textDecoration = hyperlink.textDecoration =
          changedObject.textDecoration;
        break;
 ...
 * @private
 */
export function updateShapeContent(
  ...
  content.horizontalAlignment =
    actualObject.wrapper.children[0].horizontalAlignment;
  content.verticalAlignment =
    actualObject.wrapper.children[0].verticalAlignment;
...
export function updateShape(
  ...
  switch (node.shape.type) {
    ...
    case 'Basic':
      const element: DiagramElement =
        (actualObject.shape as BasicShape).shape === 'Rectangle'
          ? new DiagramElement()
          : new PathElement();
      if (
        !isBlazor() &&
        (actualObject.shape as BasicShape).shape === 'Polygon'
      ) {
        (element as PathElement).data = getPolygonPath(
          (actualObject.shape as BasicShape).points
        ) as string;
      } else {
    ...
    case 'Flow':
      ...
      break;
    case 'Native':
      const nativeContent: DiagramNativeElement = new DiagramNativeElement(
        node.id,
        diagram.element.id
      );
  ...
  if (
    node.shape.type === undefined ||
    node.shape.type === oldObject.shape.type
  ) {
...
export function updateContent(
  ...
  if (Object.keys(newValues.shape).length > 0) {
    if (
      actualObject.shape.type === 'Path' &&
      (newValues.shape as PathModel).data !== undefined
    ) {
      (actualObject.wrapper.children[0] as PathModel).data = (
        newValues.shape as PathModel
      ).data;
    } else if (
      actualObject.shape.type === 'Text' &&
      (newValues.shape as TextModel).content !== undefined
    ) {
      (actualObject.wrapper.children[0] as TextModel).content = (
        newValues.shape as TextModel
      ).content;
    } else if (
      actualObject.shape.type === 'Image' &&
      (newValues.shape as ImageModel).source !== undefined
    ) {
      (actualObject.wrapper.children[0] as ImageModel).source = (
        newValues.shape as ImageModel
      ).source;
    } else if (actualObject.shape.type === 'Native') {
      let nativeElement: HTMLElement;
      for (let i: number = 0; i < diagram.views.length; i++) {
        nativeElement = getDiagramElement(
          actualObject.wrapper.children[0].id + '_native_element',
          diagram.views[parseInt(i.toString(), 10)]
        );
        if (
          (newValues.shape as NativeModel).content !== undefined &&
          nativeElement
        ) {
          nativeElement.removeChild(nativeElement.children[0]);
          (actualObject.wrapper.children[0] as DiagramNativeElement).content = (
            newValues.shape as NativeModel
          ).content;
          nativeElement.appendChild(
            getContent(
              actualObject.wrapper.children[0] as DiagramNativeElement,
              false
            )
          );
        }
      }
      (actualObject.wrapper.children[0] as NativeModel).scale = (
        newValues.shape as NativeModel
      ).scale
        ? (newValues.shape as NativeModel).scale
        : (actualObject.wrapper.children[0] as NativeModel).scale;
    } else if (actualObject.shape.type === 'HTML') {
      let htmlElement: HTMLElement;
      for (let i: number = 0; i < diagram.views.length; i++) {
        htmlElement = getDiagramElement(
          actualObject.wrapper.children[0].id + '_html_element',
          diagram.views[parseInt(i.toString(), 10)]
        );
        if (htmlElement) {
          htmlElement.removeChild(htmlElement.children[0]);
          (actualObject.wrapper.children[0] as DiagramHtmlElement).content = (
            newValues.shape as HtmlModel
          ).content;
          htmlElement.appendChild(
            getContent(
              actualObject.wrapper.children[0] as DiagramHtmlElement,
              true
            )
          );
        }
      ...
      actualObject.shape.type === 'Flow' &&
      (newValues.shape as FlowShapeModel).shape !== undefined
    ) {
      (actualObject.shape as FlowShapeModel).shape = (
        newValues.shape as FlowShapeModel
      ).shape;
      ...
      !isBlazor() &&
      (newValues.shape as UmlActivityShapeModel).shape !== undefined
    ) {
      updateUmlActivityNode(actualObject, newValues);
    } else if (
      (newValues.shape as BasicShapeModel).cornerRadius !== undefined
    ) {
      (actualObject.wrapper.children[0] as BasicShapeModel).cornerRadius = (
        newValues.shape as BasicShapeModel
      ).cornerRadius;
    } else if (
      actualObject.shape.type === 'Basic' &&
      oldObject &&
      (oldObject.shape as BasicShape).shape === 'Rectangle'
    ) {
      const basicshape: PathElement = new PathElement();
      const basicshapedata: string = getBasicShape(
        (actualObject.shape as BasicShape).shape
      );
      ...
      (actualObject.shape as BasicShapeModel).shape = (
        newValues.shape as BasicShapeModel
      ).shape;
 ...
 * @private
 */
export function updateUmlActivityNode(
  actualObject: Node,
  newValues: Node
): void {
  if (!isBlazor()) {
    (actualObject.shape as UmlActivityShapeModel).shape = (
      newValues.shape as UmlActivityShapeModel
    ).shape;
  } else {
    (actualObject.shape as DiagramShape).umlActivityShape = (
      newValues.shape as DiagramShape
    ).umlActivityShape;
  }
  const shapes: UmlActivityShapes = !isBlazor()
    ? (actualObject.shape as UmlActivityShapeModel).shape
    : (actualObject.shape as DiagramShape).umlActivityShape;
  const umlActivityShapeData: string = getUMLActivityShape(shapes.toString());
  if (
    !isBlazor() &&
    (actualObject.shape as UmlActivityShapeModel).shape === 'InitialNode'
  ) {
    actualObject.wrapper.children[0].style.fill = 'black';
  } else if (
    !isBlazor() &&
    ((actualObject.shape as UmlActivityShapeModel).shape === 'ForkNode' ||
...
export function getUMLFinalNode(node: Node): Canvas {
  ...
  const pathData: string =
    'M 25 50 C 11.21 50 0 38.79 0 25 C 0 11.21 11.21 0 25 0 C 38.78 0 50 11.21 50 25' +
 ...
 * @private
 */
export function getUMLActivityShapes(
  umlActivityShape: PathElement,
  content: DiagramElement,
  node: Node
): DiagramElement {
    ...
    case 'StructuredNode':
      if (node.annotations) {
        for (let i: number = 0; i < node.annotations.length; i++) {
          node.annotations[parseInt(i.toString(), 10)].content =
            '<<' + node.annotations[parseInt(i.toString(), 10)].content + '>>';
        }
      }
      content = umlActivityShape;
...
export function updateConnector(
  ...
  connector.updateSegmentElement(
    connector,
    points,
  ...
  if (connector.type === 'Bezier') {
    ...
    const lastSegment: BezierSegment = connector.segments[
      connector.segments.length - 1
    ] as BezierSegment;
    anglePoint = [
      !Point.isEmptyPoint(lastSegment.point2)
        ? lastSegment.point2
        : lastSegment.bezierPoint2,
      !Point.isEmptyPoint(firstSegment.point1)
        ? firstSegment.point1
        : firstSegment.bezierPoint1,
    ];
  } else {
  ...
  connector.updateDecoratorElement(
    element,
    points[0],
    anglePoint[1],
    srcDecorator
 ...
 * @private
 */
export function getUserHandlePosition(
  selectorItem: SelectorModel,
  ...
  if (selectorItem.nodes.length > 0) {
    ...
    point.x +=
      (margin.left - margin.right) / transform.scale +
      (size / 2) *
        (handle.horizontalAlignment === 'Center'
          ? 0
          : handle.horizontalAlignment === 'Right'
          ? -1
          : 1);
    point.y +=
      (margin.top - margin.bottom) / transform.scale +
      (size / 2) *
        (handle.verticalAlignment === 'Center'
          ? 0
    ...
    rotateMatrix(
      matrix,
      -angle,
      connector.intermediatePoints[parseInt(index.toString(), 10)].x,
...
export function canResizeCorner(
  ...
  if (selectedItems.annotation) {
    ...
    SelectorConstraints[`${action}`] & selectorConstraints &&
    ThumbsConstraints[`${action}`] & thumbsConstraints
  ) {
    return true;
  }
  return false;
 ...
 * @private
 */
export function canShowControlPoints(
  bezierControlPoints: ControlPointsVisibility,
  action: string
): boolean {
 ...
 * @private
 */
export function checkPortRestriction(
  port: PointPortModel,
 ...
 * @private
 */
export function getInOutConnectPorts(
  node: NodeModel | ConnectorModel,
  isInConnect: boolean
): PointPortModel {
  let port: PointPortModel = {};
  ...
  if (node.ports) {
    ...
    for (i = 0; i < ports.length; i++) {
      if (isInConnect) {
        if (
          ports[parseInt(i.toString(), 10)].constraints &
 ...
 * @private
 */
export function getObjectFromCollection(
...
export function scaleElement(
  ...
  if (element instanceof Container) {
    ...
    if (width !== undefined && height !== undefined) {
      ...
      refPoint = rotatePoint(
        refObject.rotateAngle,
        refObject.offsetX,
        refObject.offsetY,
        refPoint
      );
 ...
 */
// eslint-disable-next-line @typescript-eslint/ban-types
export function arrangeChild(
  obj: Node,
  x: number,
  y: number,
  nameTable: {},
  drop: boolean,
  diagram: Diagram | SymbolPalette
): void {
  ...
  if (collection.length === 0) {
    ...
    while (mid !== low) {
      if (collection[parseInt(mid.toString(), 10)][`${key}`] < obj[`${key}`]) {
        ...
        collection[parseInt(mid.toString(), 10)][`${key}`] > obj[`${key}`]
      ) {
        high = mid;
        mid = Math.floor((low + high) / 2);
      } else {
    ...
    if (collection[parseInt(high.toString(), 10)][`${key}`] < obj[`${key}`]) {
      ...
      collection[parseInt(low.toString(), 10)][`${key}`] > obj[`${key}`]
    ) {
      collection.splice(low, 0, obj);
    } else if (
      collection[parseInt(low.toString(), 10)][`${key}`] < obj[`${key}`] &&
      collection[parseInt(high.toString(), 10)][`${key}`] > obj[`${key}`]
    ) {
...
export function getElement(
  element: DiagramHtmlElement | DiagramNativeElement
): Object {
  ...
  if (
    enterObject &&
    (enterObject['id'] === element.nodeId || enterObject['children'])
  ) {
    if (
      enterObject['children'] &&
      groupHasType(
        enterObject as Node,
        'HTML',
        diagramElement[`${instance}`][0].enterTable
      )
    ) {
...
function getAnnotation(
  ...
  for (j = 0; annotations && j < annotations[`${length}`]; j++) {
    if (
      (element as DiagramHtmlElement).annotationId &&
      annotations[parseInt(j.toString(), 10)].id ===
        (element as DiagramHtmlElement).annotationId
    ) {
...
function getPaletteSymbols(symbolPalette: SymbolPalette): NodeModel[] {
  ...
  for (let i: number = 0; i < symbolPalette.palettes.length; i++) {
    const symbols: (NodeModel | ConnectorModel)[] =
      symbolPalette.palettes[parseInt(i.toString(), 10)].symbols;
 ...
 * @private
 */
export function getDropEventArguements(
  args: MouseEventArgs,
  arg: IBlazorDropEventArgs
): IBlazorDropEventArgs {
  //Removed isBlazor code
  return arg;
}
...
export let getObjectType: Function = (obj: Object): Object => {
  if (obj) {
    if (
      (obj as Connector).sourceID !== undefined ||
      (obj as Connector).sourcePoint !== undefined ||
      (obj as Connector).targetID !== undefined ||
      (obj as Connector).targetPoint !== undefined ||
...
export let updatePathElement: Function = (
  ...
  for (let j: number = 0; j < anglePoints.length - 1; j++) {
    ...
    pathElement.id =
      connector.id + '_' + (connector.shape as BpmnFlow).sequence;
...
/** @private */
export let getPathOffset: Function = (
  anglePoints: PointModel[],
  connector: ConnectorModel,
  ...
  for (let i: number = 0; i < anglePoints.length - 1; i++) {
    totalLength += findDistance(
      anglePoints[parseInt(i.toString(), 10)],
      anglePoints[i + 1]
    );
  }
  ...
  for (var i: number = 0; i < anglePoints.length - 1; i++) {
    var segmentLength = findDistance(
      anglePoints[parseInt(i.toString(), 10)],
      anglePoints[i + 1]
    );
...
export let findPath: Function = (
  ...
  let transferpt: PointModel = Point.transform(
    { x: beginningpoint.x, y: beginningpoint.y },
    angle,
    distance
  );
  let startpoint: PointModel = Point.transform(
    { x: transferpt.x, y: transferpt.y },
    angle,
    -11
  );
  let endpoint: PointModel = Point.transform(
...
/** @private */
export let getConnectorDirection: Function = (
  src: PointModel,
  tar: PointModel
): string => {
  if (Math.abs(tar.x - src.x) > Math.abs(tar.y - src.y)) {
...
export function canMeasureDecoratorPath(objects: string[]): boolean {
  if (
    objects.indexOf('shape') !== -1 ||
    objects.indexOf('pathData') !== -1 ||
    objects.indexOf('width') !== -1 ||
    objects.indexOf('height') !== -1
  ) {
    return true;
  }
  return false;
...
export function getPreviewSize(
  sourceElement: SymbolPaletteModel,
  clonedObject: Node,
  wrapper: DiagramElement
): Size {
  //let previewWidth: number;
  //let previewHeight: number;
  const previewWidth: number = getSymbolSize(
    sourceElement as SymbolPaletteModel,
    clonedObject as Node,
    wrapper,
    'width'
  );
  const previewHeight: number = getSymbolSize(
    sourceElement as SymbolPaletteModel,
    clonedObject as Node,
    wrapper,
    'height'
...
export function getSymbolSize(
  ...
  if ((clonedObject as Node).previewSize[`${size}`] !== undefined) {
    ...
    (sourceElement as SymbolPaletteModel).symbolPreview[`${size}`] !== undefined
  ) {
    previewSize = (sourceElement as SymbolPaletteModel).symbolPreview[
      `${size}`
    ];
  } else {
    previewSize =
      (clonedObject as Node)[`${size}`] || wrapper.actualSize[`${size}`];
  }
  return previewSize;
}
 ...
 * @private
 */
export function findParentInSwimlane(
  node: Node,
  diagram: Diagram,
  parent: string
): string {
 ...
 * @private
 */
export function selectionHasConnector(
  diagram: Diagram,
  selector: Selector
): boolean {
  if (
    diagram.selectedItems.connectors.length > 1 &&
