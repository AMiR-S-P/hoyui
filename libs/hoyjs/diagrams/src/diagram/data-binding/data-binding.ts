...
export class DataBinding {
  ...
  public initData(data: DataSourceModel, diagram: Diagram): void {
    ...
    const dataManager: DataManager =
      data.dataManager || data.dataSource || ({} as DataManager);
    dataSource =
      dataManager[`${dataProp}`] ||
      dataManager[`${jsonProp}`] ||
      (dataManager.dataSource ? dataManager.dataSource.json : undefined);
    if (
      dataSource &&
      (dataSource as Object[]).length === 0 &&
  ...
  public initSource(data: DataSourceModel, diagram: Diagram): void {
    const dataSource: DataSourceModel = data;
    let result: Object[];
    const mapper: DataSourceModel = data;
    if (
      dataSource.dataManager instanceof DataManager ||
  ...
  private applyDataSource(
    mapper: DataSourceModel,
    data: Object[],
    diagram: Diagram
  ): void {
    this.dataTable = {};
    let obj: Object;
    let firstNode: Object;
    let node: Node;
    ...
    if (data !== undefined) {
      for (let r: number = 0; r < data.length; r++) {
        ...
        if (
          obj[mapper.parentId] === undefined ||
          obj[mapper.parentId] === null ||
          typeof obj[mapper.parentId] !== 'object'
        ) {
          if (
            isNaN(obj[mapper.parentId]) &&
            obj[mapper.parentId] !== undefined
          ) {
            if (
              rootNodes[
                obj[mapper.parentId]
                  ? obj[mapper.parentId].toLowerCase()
                  : obj[mapper.parentId]
              ] !== undefined
            ) {
              (
                rootNodes[obj[mapper.parentId].toLowerCase()] as DataItems
              ).items.push(obj);
            } else {
              rootNodes[
                obj[mapper.parentId]
          ...
          rootNodes = this.updateMultipleRootNodes(
            obj,
            rootNodes,
            mapper,
            data
      ...
      for (let i: number = 0; i < firstLevel.length; i++) {
        for (
          let j: number = 0;
          j <
          (firstLevel[parseInt(i.toString(), 10)] as DataItems).items.length;
          j++
        ) {
          item = (firstLevel[parseInt(i.toString(), 10)] as DataItems).items[
          ...
          if (
            isNaN(node.data[mapper.id] as any) &&
            node.data[mapper.id] !== undefined
          ) {
  ...
  private updateMultipleRootNodes(
    ...
    const parents: string[] = obj[mapper.parentId];
    let parent: string;
  ...
  private applyNodeTemplate(
    ...
    if (diagram.layout.type === 'Flowchart') {
      const shape: BasicShapeModel = this.getFlowChartNodeShape(
        item as FlowChartData
      ) as BasicShapeModel;
      const style: ShapeStyleModel = {
        fill: (item as FlowChartData).color
          ? (item as FlowChartData).color
          : 'white',
        strokeColor: (item as FlowChartData).stroke
          ? (item as FlowChartData).stroke
          : 'black',
        strokeWidth: (item as FlowChartData).strokeWidth
          ? (item as FlowChartData).strokeWidth
          : 1,
      };
      nodeModel.shape = shape as
        | BasicShapeModel
        | BpmnShapeModel
        | FlowShapeModel;
      ...
      nodeModel.annotations = [
        {
          content: (item as FlowChartData).name
            ? (item as FlowChartData).name
            : '',
        },
      ];
    }
    ...
    if (mapper.dataMapSettings) {
      ...
      for (let i: number = 0; i < mapper.dataMapSettings.length; i++) {
        if (
          mapper.dataMapSettings[parseInt(i.toString(), 10)].property.indexOf(
            '.'
          ) !== -1
        ) {
          innerProperty = this.splitString(
            mapper.dataMapSettings[parseInt(i.toString(), 10)].property
          );
          for (let p: number = 0; p < innerProperty.length; p++) {
            if (innerProperty[parseInt(p.toString(), 10)].indexOf('[') !== -1) {
              ...
              arrayProperty =
                innerProperty[parseInt(p.toString(), 10)].split('[');
            }
          }
          if (index) {
            if (innerProperty[2]) {
              obj[arrayProperty[0]][innerProperty[0].charAt(index + 1)][
                innerProperty[1]
              ][innerProperty[2]] =
            ...
            if (innerProperty[2]) {
              ...
              obj[innerProperty[0]][innerProperty[1]] =
                item[mapper.dataMapSettings[parseInt(i.toString(), 10)].field];
            }
          }
        } else {
  ...
  private getFlowChartNodeShape(
    data: FlowChartData
  ): BasicShapeModel | FlowShapeModel | PathModel {
    if (data.shape !== '') {
      switch (data.shape) {
        case 'Rectangle':
  ...
  private splitString(property: string): string[] {
    ...
    for (let i: number = 0; i < temp.length; i++) {
      temp[parseInt(i.toString(), 10)] =
        temp[parseInt(i.toString(), 10)].charAt(0).toLowerCase() +
    ...
    for (let j: number = 0; j < (parent as DataItems).items.length; j++) {
      ...
      if (!this.containsConnector(diagram, value, node.id)) {
        diagram.connectors.push(
          this.applyConnectorTemplate(value, node.id, diagram)
        );
      }
      if (!canBreak) {
  ...
  private containsConnector(
    ...
    return diagram.connectors.some((connector: Connector) => {
      return (
        connector !== undefined &&
        connector.sourceID === sourceNode &&
        connector.targetID === targetNode
      );
    });
  }
  ...
  private collectionContains(
    ...
    if (
      obj !== undefined &&
      obj.data[`${id}`] === node.data[`${id}`] &&
      obj.data[`${parentId}`] === node.data[`${parentId}`]
    ) {
      return true;
    } else {
      return false;
  ...
  private applyConnectorTemplate(
    ...
    if (diagram.layout.type === 'Flowchart') {
      const targetNode: NodeModel = diagram.nodes.find(
        (node: Node) => node.id === tNode
      ) as Node;

      if (typeof (targetNode.data as FlowChartData).label === 'string') {
        connModel.annotations = [
          { content: (targetNode.data as FlowChartData).label as string },
        ];
      } else if (Array.isArray((targetNode.data as FlowChartData).label)) {
        const inConnectors: ConnectorModel[] = diagram.connectors.filter(
          (connector: Connector) => connector.targetID === tNode
        );
        let index: number = 0;
        ...
        connModel.annotations = [
          {
            content: (targetNode.data as FlowChartData).label[
              parseInt(index.toString(), 10)
            ],
          },
        ];
      }
      arrowType = getConnectorArrowType(
        targetNode.data as FlowChartData
      ) as ArrowStyle;
    }
    const obj: Connector = new Connector(
      diagram,
      'connectors',
      connModel,
      true
    ...
    if (arrowType) {
      ...
      obj.targetDecorator.shape = arrowType.targetDecorator
        ? (arrowType.targetDecorator as DecoratorShapes)
        : ('Arrow' as DecoratorShapes);
      obj.style.strokeDashArray = arrowType.strokeDashArray
        ? arrowType.strokeDashArray
        : '';
      obj.style.opacity =
        arrowType.opacity !== undefined ? arrowType.opacity : 1;
    }
