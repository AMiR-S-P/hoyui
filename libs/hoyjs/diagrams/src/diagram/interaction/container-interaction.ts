...
export function updateCanvasBounds(
  ...
  if (checkParentAsContainer(diagram, obj, true)) {
    ...
    if (container && container.container.type === 'Canvas') {
      if (
        isBoundsUpdate ||
        (wrapper.bounds.x <= position.x &&
          wrapper.bounds.right >= position.x &&
          wrapper.bounds.y <= position.y &&
          wrapper.bounds.bottom >= position.y)
      ) {
        let parentWrapper: GridPanel;
        const y: number = wrapper.bounds.y;
        const x: number = wrapper.bounds.x;

        const parent: NodeModel =
          diagram.nameTable[(container as Node).parentId] || container;
        const shape: SwimLaneModel = parent.shape as SwimLaneModel;
        if (shape.type === 'SwimLane') {
          groupAction = updateLaneBoundsAfterAddChild(
            container,
            parent,
            obj as NodeModel,
            diagram,
          ...
          if (
            ...
            if (container.rowIndex !== undefined) {
              ...
              diagram.scale(
                container,
                1 +
                  (wrapper.outerBounds.width - wrapper.actualSize.width) /
                    wrapper.actualSize.width,
                1,
                wrapper.outerBounds.x < wrapper.bounds.x
                  ? { x: 1, y: 0.5 }
                  : { x: 0, y: 0.5 }
              );
            }
          ...
          if (
            wrapper.actualSize.height < wrapper.outerBounds.height &&
            !(wrapper.bounds.y > wrapper.outerBounds.y)
          ) {
            if (container.rowIndex !== undefined) {
              parentWrapper.updateRowHeight(
                container.rowIndex,
                wrapper.outerBounds.height,
                true
              );
              diagram.drag(parent, x - wrapper.bounds.x, y - wrapper.bounds.y);
            } else {
              diagram.scale(
                ...
                1,
                1 +
                  (wrapper.outerBounds.height - wrapper.actualSize.height) /
                    wrapper.actualSize.height,
                wrapper.outerBounds.y < wrapper.bounds.y
                  ? { x: 0.5, y: 1 }
                  : { x: 0.5, y: 0 }
              );
            }
...
export function findBounds(
  ...
  const rows: GridRow[] =
    (obj as Node).shape.type === 'SwimLane'
      ? (obj.wrapper.children[0] as GridPanel).rows
      : (obj.wrapper as GridPanel).rows;
 ...
 * @private
 */
export function renderContainerHelper(
  diagram: Diagram,
  obj: SelectorModel | NodeModel | ConnectorModel
  ...
  if (!isBlazor()) {
    if (diagram.selectedObject.helperObject) {
      ...
      if (
        ...
        obj.nodes.length + obj.connectors.length === 1
      ) {
      ...
      if (
        ...
        (((object as Node).isLane &&
          canLaneInterchange(object as Node, diagram) &&
          checkParentAsContainer(diagram, object)) ||
          (!(object as Node).isLane &&
            checkParentAsContainer(diagram, object)) ||
          (diagram.constraints & DiagramConstraints.LineRouting &&
            diagram.selectedItems.nodes.length > 0))
      ) {
 ...
 * @private
 */
export function checkParentAsContainer(
  diagram: Diagram,
  obj: NodeModel | ConnectorModel,
  isChild?: boolean
): boolean {
  const parentNode: NodeModel = isChild
...
function removeChildrenInLane(diagram: Diagram, node: NodeModel): void {
  if ((node as Node).parentId && (node as Node).parentId !== '') {
    const prevParentNode: Node = diagram.nameTable[
...
export function addChildToContainer(
  ...
  if (!diagram.currentSymbol) {
    ...
    const child: string | NodeModel =
      diagram.nodes.indexOf(node) !== -1 ? node.id : node;
    if (parent.container.type === 'Canvas' && !historyAction) {
      const left: number =
        node.wrapper.offsetX -
        node.wrapper.actualSize.width / 2 -
        (parent.wrapper.offsetX - parent.wrapper.actualSize.width / 2);
      const top: number =
        node.wrapper.offsetY -
        node.wrapper.actualSize.height / 2 -
      ...
      if ((swimlane.shape as SwimLane).orientation === 'Horizontal') {
        ...
        const laneHeaderId: string =
          (parent as Node).parentId +
          (swimlane.shape as SwimLane).lanes[0].id +
          '_0_header';
        node.margin.top -=
          parentBounds.y -
          swimLaneBounds.y -
          diagram.nameTable[`${laneHeaderId}`].wrapper.bounds.height;
      }
    }
...
function moveSwinLaneChild(
  node: NodeModel | ConnectorModel,
  diagram: Diagram
): void {
  const sourceNode: HTMLElement = getDiagramElement(
    node.id + '_groupElement',
    diagram.element.id
  );
  const targetId: string = (node as Node).parentId
    ? (node as Node).parentId + '_groupElement'
    : diagram.element.id + '_diagramLayer';
  const targetNode: HTMLElement = getDiagramElement(
    targetId,
    diagram.element.id
  );
...
export function updateLaneBoundsAfterAddChild(
  ...
  if (
    ...
    const x: number = grid.bounds.x;
    const y: number = grid.bounds.y;
    let size: number;
    if (
      containerBounds.right < nodeBounds.right + padding &&
 ...
 * @private
 */
export function moveChildInStack(
  sourceNode: Node,
  target: Node,
  diagram: Diagram,
  action: Actions
  ...
  if (
    target &&
    sourceParent &&
    sourceParent.container &&
    sourceParent.container.type === 'Stack' &&
    target.container &&
    target.container.type === 'Stack' &&
    sourceParent.id !== target.parentId
  ) {
