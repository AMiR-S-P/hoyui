...
export class ConnectorEditing extends ToolBase {
  ...
  public mouseDown(args: MouseEventArgs): void {
    ...
    if (args.info && args.actualObject) {
      edit =
        args.info.ctrlKey &&
        (args.actualObject as ConnectorModel).type !== 'Orthogonal';
    }
    if (connectors && edit) {
      this.inAction = true;
      ...
      for (let i: number = 0; i < connectors.segments.length; i++) {
        ...
        if (this.endPoint === 'OrthoThumb') {
          for (let j: number = 0; j < segment.points.length - 1; j++) {
            ...
            segPoint.x =
              (segment.points[parseInt(j.toString(), 10)].x +
                segment.points[j + 1].x) /
              2;
            segPoint.y =
              (segment.points[parseInt(j.toString(), 10)].y +
                segment.points[j + 1].y) /
              2;
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (this.currentPosition && this.prevPosition) {
      ...
      let arg: ISegmentChangeEventArgs = {
        source: connector,
        state: 'Start',
      ...
      arg = {
        source: cloneBlazorObject(connector),
        state: 'Start',
        oldValue: cloneBlazorObject(this.oldValue),
        newValue: cloneBlazorObject(this.oldValue),
        segment: cloneBlazorObject(this.selectedSegment) as
          | StraightSegment
          | OrthogonalSegment
          | BezierSegment,
        cancel: false,
      ...
      if (
        ...
        if (this.endPoint === 'OrthoThumb') {
          this.blocked = !this.dragOrthogonalSegment(
            connector,
            this.selectedSegment as OrthogonalSegment,
            this.currentPosition,
            this.segmentIndex
          );
          if (
            (this.oldValue as OrthogonalSegment).length === null &&
            (this.oldValue as OrthogonalSegment).direction === null
          ) {
            this.oldValue = cloneObject(this.selectedSegment);
          }
          ...
          const tx: number =
            ...
            (this.selectedSegment as StraightSegment | BezierSegment).point.x;
          const ty: number =
            this.currentPosition.y -
            (this.selectedSegment as StraightSegment | BezierSegment).point.y;
          const index: number = connector.segments.indexOf(
            this.selectedSegment
          );
          this.blocked = !this.commandHandler.dragControlPoint(
            connector,
            tx,
            ty,
            false,
        ...
        this.currentPosition = args.position;
        const arg1: ISegmentChangeEventArgs = {
          source: cloneBlazorObject(connector),
          state: 'Progress',
          oldValue: cloneBlazorObject(this.oldValue),
          newValue: cloneBlazorObject(newValue),
          segment: cloneBlazorObject(this.selectedSegment),
          cancel: false,
        };
  ...
  public mouseUp(args: MouseEventArgs): void {
    ...
    if (
      ...
      if (this.endPoint === 'OrthoThumb' && this.selectedSegment) {
        ...
        const next: OrthogonalSegment = connector.segments[
          index + 1
        ] as OrthogonalSegment;
        if (
          index === connector.segments.length - 2 &&
          ...
          if (prev && Math.abs(prev.length) < 5 && prev.length > 0) {
            ...
            const len: number = Point.distancePoints(
              next.points[0],
              next.points[1]
            );
            const length: number =
              next.length || next.length === 0 ? next.length : len;
    ...
    if (this.prevPosition !== this.startPosition) {
      const arg: ISegmentChangeEventArgs = {
        source: cloneBlazorObject(connector),
        state: 'Completed',
        oldValue: cloneBlazorObject(this.oldValue),
        newValue: cloneObject(this.selectedSegment),
  ...
  private removePrevSegment(connector: ConnectorModel, index: number): void {
    const first: OrthogonalSegment = connector.segments[
      index - 2
    ] as OrthogonalSegment;
    const next: OrthogonalSegment = connector.segments[
      index + 1
    ...
    if (next) {
      const length: number =
        next.length || next.length === 0
          ? next.length
          : Point.distancePoints(next.points[0], next.points[1]);
      if (!(length <= 5)) {
        const removeSegments: OrthogonalSegmentModel[] =
          connector.segments.slice(index - 1, index + 1);
        ...
        this.commandHandler.triggerEvent(
          DiagramEvent.segmentCollectionChange,
          args
        );
        if (!args.cancel) {
          const last: OrthogonalSegment = connector.segments[
            index + 1
          ] as OrthogonalSegment;
          connector.segments.splice(index - 1, 2);
          const segment: OrthogonalSegment = this
            .selectedSegment as OrthogonalSegment;
  ...
  private findSegmentDirection(segment: OrthogonalSegment): void {
    if (segment.direction && (segment.length || segment.length === 0)) {
      segment.length = Point.distancePoints(
        segment.points[0],
        segment.points[segment.points.length - 1]
      );
      segment.direction = Point.direction(
        segment.points[0],
        segment.points[segment.points.length - 1]
      ) as Direction;
    }
  ...
  private removeNextSegment(connector: ConnectorModel, index: number): void {
    ...
    let removeSegments: OrthogonalSegmentModel[];
    let args:
      | ISegmentCollectionChangeEventArgs
      | IBlazorSegmentCollectionChangeEventArgs;
    if (next.length || next.length === 0) {
      ...
      args = {
        ...
        removeSegments: cloneBlazorObject(
          removeSegments
        ) as OrthogonalSegmentModel[],
        type: 'Removal',
        cancel: false,
      };
      //Removed isBlazor code
      ...
      if (!args.cancel) {
        ...
        if (segment.direction === 'Top' || segment.direction === 'Bottom') {
          first.points[first.points.length - 1].x =
            next.points[next.points.length - 1].x;
        } else {
          first.points[first.points.length - 1].y =
            next.points[next.points.length - 1].y;
        }
        this.findSegmentDirection(first);
    ...
    if (first && last && !args.cancel) {
      ...
      first.direction = Point.direction(
        first.points[0],
        last.points[0]
      ) as Direction;
      if (last.length || last.length === 0) {
        last.length = Point.distancePoints(
          first.points[first.points.length - 1],
          last.points[last.points.length - 1]
        );
        const point1: PointModel[] = first.points;
        const point2: PointModel[] = last.points;
        last.direction = Point.direction(
          point1[point1.length - 1],
          point2[point2.length - 1]
        ) as Direction;
      }
    }
  }
  ...
  public addOrRemoveSegment(
    ...
    let updateSeg: boolean;
    let segmentIndex: number;
    const segmentsChanged: StraightSegmentModel[] = [];
    ...
    for (let i: number = 0; i < connector.segments.length; i++) {
      ...
      if (contains(point, segment.point, connector.hitPadding)) {
        ...
        const args: ISegmentCollectionChangeEventArgs = {
          element: connector,
          removeSegments: segmentsChanged,
          type: 'Removal',
          cancel: false,
        };
        if (commandHandler) {
    ...
    if (updateSeg && segmentIndex !== undefined) {
      if (
        connector.segments &&
        connector.segments[parseInt(segmentIndex.toString(), 10)] &&
        connector.segments[parseInt(segmentIndex.toString(), 10)].type ===
          'Straight'
      ) {
        const segment: StraightSegment = connector.segments[
          parseInt(segmentIndex.toString(), 10)
        ] as StraightSegment;
        const previous: StraightSegment = connector.segments[
          segmentIndex + 1
        ] as StraightSegment;
        if (previous) {
      ...
      if (
        connector.segments &&
        connector.segments[parseInt(index.toString(), 10)] &&
        connector.segments[parseInt(index.toString(), 10)].type === 'Straight'
      ) {
        const segment: StraightSegment = connector.segments[
          parseInt(index.toString(), 10)
        ] as StraightSegment;
        const newseg: StraightSegment = new StraightSegment(
          connector,
          'segments',
          { type: 'Straight', point: point },
          true
        );
  ...
  private findIndex(connector: Connector, point: PointModel): number {
    ...
    if (intersectingSegs.length === 1) {
      ...
      return connector.segments.indexOf(
        intersectingSegs[parseInt(index.toString(), 10)]
      );
    }
  }
  private dragOrthogonalSegment(
    ...
    segmentPoint.x =
      (segment.points[parseInt(segmentIndex.toString(), 10)].x +
        segment.points[segmentIndex + 1].x) /
      2;
    segmentPoint.y =
      (segment.points[parseInt(segmentIndex.toString(), 10)].y +
        segment.points[segmentIndex + 1].y) /
      2;
    const ty: number = point.y - segmentPoint.y;
    const tx: number = point.x - segmentPoint.x;
    let index: number = obj.segments.indexOf(segment);
    ...
    if (index !== -1) {
      if (
        index === 0 &&
        obj.segments.length === 1 &&
        segment.points.length === 2
      ) {
        index = this.addSegments(obj as Connector, segment, tx, ty, index);
        update = true;
      } else if (
        index === obj.segments.length - 1 &&
        (segment.direction === null || segment.length === null)
      ) {
      ...
      if (index) {
        if (update) {
          this.selectedSegment = segment = obj.segments[
            parseInt(index.toString(), 10)
          ] as OrthogonalSegment;
          this.segmentIndex = 0;
        }
  ...
  private addSegments(
    ...
    let index: number;
    const segments: OrthogonalSegmentModel[] = []; //let len: number;
    const length: number = Point.distancePoints(
      segment.points[0],
      segment.points[1]
    );
    const segmentDirection: Direction = Point.direction(
      segment.points[0],
      segment.points[1]
    ) as Direction;
    segments.push(
      new OrthogonalSegment(
        obj,
        'segments',
        { type: 'Orthogonal', direction: segmentDirection, length: length / 4 },
        true
      )
    );
    ...
    segments.push(
      new OrthogonalSegment(
        obj,
        'segments',
        { type: 'Orthogonal', direction: direction, length: len },
        true
      )
    );
    segments.push(
      new OrthogonalSegment(
        obj,
        'segments',
        { type: 'Orthogonal', direction: segmentDirection, length: length / 2 },
        true
      )
    );
    const args: ISegmentCollectionChangeEventArgs = {
  ...
  private insertFirstSegment(
    ...
    let index: number;
    let insertseg: OrthogonalSegment;
    if (
      obj.sourcePortID &&
      segment.length &&
      (obj.segments[0] as OrthogonalSegment).points.length > 2
    ) {
      let prev: OrthogonalSegment;
      for (let i: number = 0; i < segment.points.length - 1; i++) {
        const len: number = Point.distancePoints(
          segment.points[parseInt(i.toString(), 10)],
          segment.points[i + 1]
        );
        const dir: Direction = Point.direction(
          segment.points[parseInt(i.toString(), 10)],
          segment.points[i + 1]
        ) as Direction;
        insertseg = new OrthogonalSegment(
          obj,
          'segments',
          { type: 'Orthogonal', direction: dir, length: len },
          true
        );
        if (insertseg.length === 0) {
          if (
            prev &&
            (prev.direction === 'Top' || prev.direction === 'Bottom')
          ) {
      ...
      insertseg = new OrthogonalSegment(
        obj,
        'segments',
        { type: 'Orthogonal', direction: direction, length: length },
        true
      );
      segments.push(insertseg);
    }
  ...
  private updateAdjacentSegments(
    ...
    const current: OrthogonalSegment = obj.segments[
      parseInt(index.toString(), 10)
    ] as OrthogonalSegment;
    ...
    if (current.type === 'Orthogonal') {
      ...
      const prev: OrthogonalSegmentModel = obj.segments[
        index - 1
      ] as OrthogonalSegment;
      if (prev) {
      ...
      if (obj.segments.length - 1 > index && isNextUpdate) {
        const nextSegment: OrthogonalSegment = obj.segments[
          index + 1
        ] as OrthogonalSegment;
  ...
  private addTerminalSegment(
    ...
    //const oldValues: Connector = { segments: connector.segments } as Connector;
    let index: number = connector.segments.indexOf(segment);
    let first: OrthogonalSegment;
    let insertseg: OrthogonalSegment;
    let len: number;
    let dir: Direction;
    const segments: OrthogonalSegmentModel[] = [];
    ...
    for (let i: number = 0; i < segment.points.length - 2; i++) {
      ...
      insertseg = new OrthogonalSegment(
        connector,
        'segments',
        { type: 'Orthogonal', length: len, direction: dir },
        true
      );
      segments.push(insertseg);
      first = insertseg;
    ...
    const args1:
      ...
      | IBlazorSegmentCollectionChangeEventArgs = {
      element: cloneBlazorObject(connector),
      addSegments: cloneBlazorObject(segments) as OrthogonalSegmentModel[],
      type: 'Addition',
      cancel: args.cancel,
    };
    //Removed isBlazor code
  ...
  private updatePortSegment(
    ...
    if (index === 1 && prev.points.length === 2 && prev.length < 0) {
      ...
      const current: OrthogonalSegment = connector.segments[
        parseInt(index.toString(), 10)
      ] as OrthogonalSegment;
      const next: OrthogonalSegment = connector.segments[
        index + 1
  ...
  private updatePreviousSegment(
    ...
    const isSourceNode: boolean =
      connector.sourceID && connector.sourcePortID === '' ? false : true;
    let isNextUpdate: boolean = true;
  ...
  private changeSegmentDirection(
    ...
    if (!connector.maxSegmentThumb) {
      if (segment.length < 0) {
        segment.direction = getOppositeDirection(
          segment.direction
        ) as Direction;
        segment.length *= -1;
      }
    ...
    if (next && next.type === 'Orthogonal') {
      if (next.length || next.length === 0) {
        if (next.direction === 'Left' || next.direction === 'Right') {
          if (tx !== 0) {
            next.length =
              next.direction === 'Right' ? next.length - tx : next.length + tx;
            if (next.length || next.length === 0) {
          ...
          if (ty !== 0) {
            next.length =
              next.direction === 'Bottom' ? next.length - ty : next.length + ty;
            if (next.length || next.length === 0) {
              this.changeSegmentDirection(next, obj);
            }
          }
  ...
  private updateFirstSegment(
    connector: ConnectorModel,
    selectedSegment: OrthogonalSegment
  ): void {
    const index: number = connector.segments.indexOf(selectedSegment);
    let insertfirst: boolean = false;
    const current: OrthogonalSegment = connector.segments[
      parseInt(index.toString(), 10)
    ] as OrthogonalSegment;
    const prev: OrthogonalSegment = connector.segments[
    ...
    const con: Connector = connector as Connector;
    let sourcePoint: PointModel;
    const oldValues: Connector = { segments: connector.segments } as Connector;
    if (prev.length < 0 && connector.sourceID) {
      const sourceNode: Corners = (connector as Connector).sourceWrapper
        .corners;
      const segments: OrthogonalSegmentModel[] = [];
      let segValues: Object;
      let removeCurrentPrev: boolean = false;
      this.changeSegmentDirection(current, connector);
      const next: OrthogonalSegment = connector.segments[
        index + 1
      ] as OrthogonalSegment;
      const nextNext: OrthogonalSegment = connector.segments[
        index + 2
      ] as OrthogonalSegment;
      if (next) {
      ...
      switch (prev.direction) {
        ...
        case 'Bottom':
          sourcePoint =
            current.length > 0 && current.direction === 'Left'
              ? sourceNode.middleLeft
              : sourceNode.middleRight;
          if (current.length > sourceNode.width / 2) {
            if (Math.abs(prev.length) < sourceNode.height / 2) {
              ...
              current.length = Point.distancePoints(
                current.points[0],
            ...
            if (next && next.direction && next.length) {
              ...
              next.points[0].x = next.points[next.points.length - 1].x =
        ...
        case 'Right':
          sourcePoint =
            current.length > 0 && current.direction === 'Top'
              ? sourceNode.topCenter
              : sourceNode.bottomCenter;
          if (current.length > sourceNode.height / 2) {
            ...
            if (next && next.direction && next.length) {
              ...
              next.points[0].y = next.points[next.points.length - 1].y =
        ...
        if (removeCurrentPrev) {
          ...
          if (nextNext && nextNext.direction && nextNext.length) {
            nextNext.length = Point.distancePoints(
              next.points[next.points.length - 1],
              nextNext.points[nextNext.points.length - 1]
            );
          }
          connector.segments.splice(index - 1, 2);
        }
  ...
  private updateLastSegment(
    ...
    if (connector.targetID && connector.targetPortID === '') {
      let line1Start: PointModel;
      let line1End: PointModel;
      let line2Start: PointModel;
      let line2End: PointModel;
      const corners: Corners = connector.targetWrapper.corners;
      const firstSegPoint: PointModel = selectedSegment.points[0];
      const lastSegPoint: PointModel =
        selectedSegment.points[selectedSegment.points.length - 1];

      if (
        selectedSegment.direction === 'Right' ||
        selectedSegment.direction === 'Left'
      ) {
        line1Start = { x: firstSegPoint.x, y: firstSegPoint.y };
        line1End = {
          x:
            selectedSegment.direction === 'Left'
              ? lastSegPoint.x - corners.width / 2
              : lastSegPoint.x + corners.width / 2,
          y: lastSegPoint.y,
        };
        line2Start = {
        ...
        line1End = {
          x: lastSegPoint.x,
          y:
            selectedSegment.direction === 'Bottom'
              ? lastSegPoint.y + corners.height / 2
              : lastSegPoint.y - corners.height / 2,
        };
        line2Start = {
