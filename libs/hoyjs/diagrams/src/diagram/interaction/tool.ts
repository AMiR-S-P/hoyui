...
import {
  BlazorFixedUserHandleClickEventArgs,
  DiagramEventObject,
} from '../objects/interface/IElement';
import {
  IEndChangeEventArgs,
...
import { initializeCSPTemplate, isBlazor, remove } from '@hoyui/base';
import {
...
export class ToolBase {
  ...
  protected updateSize(
    ...
    shape =
      (this.commandHandler.renderContainerHelper(shape) as NodeModel) || shape;
    const horizontalsnap: Snap = {
      snapped: false,
      offset: 0,
      left: false,
      right: false,
    };
    const verticalsnap: Snap = {
      snapped: false,
      offset: 0,
      top: false,
      bottom: false,
    ...
    const snapEnabled: boolean =
      !(shape instanceof TextElement) &&
      this.commandHandler.snappingModule &&
      this.commandHandler.snappingModule.canSnap();
    const snapLine: SVGElement = snapEnabled
      ? this.commandHandler.snappingModule.getLayer()
      : null;
    ...
    let x: number = shape.offsetX;
    let y: number = shape.offsetY;
    ...
    let diff: PointModel;
    const width: number =
      shape instanceof TextElement ? shape.actualSize.width : shape.width;
    const height: number =
      shape instanceof TextElement ? shape.actualSize.height : shape.height;
    switch (corner) {
      case 'ResizeWest':
        ...
        deltaHeight = 1;
        difx = snapEnabled
          ? this.commandHandler.snappingModule.snapLeft(
              horizontalsnap,
              verticalsnap,
              snapLine,
              difx,
              dify,
              shape as SelectorModel,
              endPoint === startPoint,
              initialBounds
            )
          : difx;
      ...
      case 'ResizeEast':
        ...
        dify = diff.y;
        difx = snapEnabled
          ? this.commandHandler.snappingModule.snapRight(
              horizontalsnap,
              verticalsnap,
              snapLine,
              difx,
      ...
      case 'ResizeNorth':
        ...
        dify = diff.y;
        dify = snapEnabled
          ? this.commandHandler.snappingModule.snapTop(
              horizontalsnap,
              verticalsnap,
              snapLine,
              difx,
      ...
      case 'ResizeSouth':
        ...
        dify = diff.y;
        dify = snapEnabled
          ? this.commandHandler.snappingModule.snapBottom(
              horizontalsnap,
              verticalsnap,
              snapLine,
              difx,
  ...
  public getShapeType(): string {
    ...
    if (
      ...
      shape = (
        this.commandHandler.diagram.drawingObject.shape as BasicShapeModel
      ).shape;
    }
    return shape;
  }
  ...
  public triggerElementDrawEvent(
    source: NodeModel | ConnectorModel,
    state: State,
    objectType: string,
    elementType: string,
    isMouseDownAction: boolean
  ): void {
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (
      ...
      if (this.mouseDownElement && !canMove(this.mouseDownElement)) {
        ...
        this.commandHandler.drawSelectionRectangle(
          rect.x,
          rect.y,
          rect.width,
          rect.height
  ...
  public mouseUp(args: MouseEventArgs): void {
    ...
    if (!this.commandHandler.isUserHandle(this.currentPosition)) {
      if (
        ...
        const region: Rect = Rect.toBounds([
          this.prevPosition,
          this.currentPosition,
        ]);
        ...
        const arrayNodes: (NodeModel | ConnectorModel)[] =
          this.commandHandler.getSelectedObject();
        if (
          !this.commandHandler.hasSelection() ||
          !args.info ||
          !args.info.ctrlKey
        ) {
          this.commandHandler.clearSelection(
            args.source === null ? true : false
          );
...
export class FixedUserHandleTool extends ToolBase {
  ...
  public mouseUp(args: MouseEventArgs): void {
    ...
    for (let i: number = 0; i < val.fixedUserHandles.length; i++) {
      if (
        iconId.indexOf(val.fixedUserHandles[parseInt(i.toString(), 10)].id) > -1
      ) {
...
export class ConnectTool extends ToolBase {
  ...
  public async mouseDown(args: MouseEventArgs): Promise<void> {
    ...
    if (
      ...
      this.endPoint === 'BezierTargetThumb'
    ) {
      for (let i: number = 0; i < connectors.segments.length; i++) {
        const segment: BezierSegment = connectors.segments[
          parseInt(i.toString(), 10)
        ] as BezierSegment;
        const segmentpoint1: PointModel = !Point.isEmptyPoint(segment.point1)
          ? segment.point1
          : segment.bezierPoint1;
        const segmentpoint2: PointModel = !Point.isEmptyPoint(segment.point2)
          ? segment.point2
        ...
        if (
          (this.currentElement as Selector).handleSize !== connectors.hitPadding
        ) {
  ...
  public async mouseUp(args: MouseEventArgs): Promise<void> {
    if (
      !isBlazor() &&
      this.isConnected &&
      (args.source as SelectorModel).connectors
    ) {
      const connector: ConnectorModel = (args.source as SelectorModel)
        .connectors[0];
      const nodeEndId: string =
        this.endPoint === 'ConnectorSourceEnd' ? 'sourceID' : 'targetID';
      ...
      if (
        connector[`${nodeEndId}`] !== this.oldConnector[`${nodeEndId}`] ||
        connector[`${portEndId}`] !== this.oldConnector[`${portEndId}`]
      ) {
    ...
    if (
      !(this instanceof ConnectorDrawingTool) &&
      ((this.endPoint === 'ConnectorSourceEnd' &&
        (args.source as SelectorModel).connectors.length &&
        (!Point.equals(
          (args.source as SelectorModel).connectors[0].sourcePoint,
          this.undoElement.connectors[0].sourcePoint
        ) ||
          (args.source as SelectorModel).connectors[0].sourceID !==
            this.undoElement.connectors[0].sourceID)) ||
        (this.endPoint === 'ConnectorTargetEnd' &&
          (!Point.equals(
            (args.source as SelectorModel).connectors[0].targetPoint,
            this.undoElement.connectors[0].targetPoint
          ) ||
            (args.source as SelectorModel).connectors[0].targetID !==
              this.undoElement.connectors[0].targetID)))
    ) {
      ...
      if (args.target) {
        const target: NodeModel | PointPortModel =
      ...
      const trigger: number =
        this.endPoint === 'ConnectorSourceEnd'
          ? DiagramEvent.sourcePointChange
          : DiagramEvent.targetPointChange;
      this.commandHandler.triggerEvent(trigger, arg);
    ...
    if (args.source && (args.source as SelectorModel).connectors) {
      let connector: ConnectorModel = (args.source as SelectorModel)
        .connectors[0];
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (
      ...
      if (args.target) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unused-expressions
        target instanceof PointPort || target instanceof BpmnSubEvent
    ...
    if (this.currentPosition && this.prevPosition) {
      ...
      this.currentPosition = this.commandHandler.snapConnectorEnd(
        this.currentPosition
      );
      let connector: ConnectorModel;
      if (args.source && (args.source as SelectorModel).connectors) {
      ...
      if (
        ...
        if (
          args.target &&
          ((this.endPoint === 'ConnectorSourceEnd' &&
            (canOutConnect(args.target) || canPortOutConnect(outPort))) ||
            (this.endPoint === 'ConnectorTargetEnd' &&
              (canInConnect(args.target) || canPortInConnect(inPort))))
        ) {
          if (
            this.commandHandler.canDisconnect(
              this.endPoint,
              args,
              targetPortId,
              targetNodeId
          ...
          if (target instanceof Node) {
            ...
            const isConnect: boolean = this.checkConnect(
              target as PointPortModel
            );
            if (isConnect) {
              this.isConnected = true;
              tempArgs = this.commandHandler.connect(
      ...
      if (this.commandHandler.canEnableDefaultTooltip()) {
        ...
        this.commandHandler.showTooltip(
          args.source,
          args.position,
          initializeCSPTemplate(contentTemp),
          'ConnectTool',
  ...
  private checkConnect(target: PointPortModel): boolean {
    if (canPortInConnect(target) && this.endPoint === 'ConnectorTargetEnd') {
      ...
      canPortOutConnect(target) &&
      this.endPoint === 'ConnectorSourceEnd'
    ) {
      return true;
    } else if (
      !(target.constraints & PortConstraints.None) &&
      !canPortInConnect(target) &&
      !canPortOutConnect(target) &&
      (target.constraints === undefined ||
        (target.constraints &
          (PortConstraints.Default &
            ~(PortConstraints.InConnect | PortConstraints.OutConnect))) >
          0)
    ) {
      return true;
    }
...
export class MoveTool extends ToolBase {
  ...
  public async mouseUp(
    ...
    this.checkPropertyValue();
    let obj: SelectorModel;
    let historyAdded: boolean = false;
    ...
    if (this.objectType !== 'Port') {
      ...
      object =
        (this.commandHandler.renderContainerHelper(
          args.source as NodeModel
        ) as Node) ||
        (args.source as Selector) ||
        (this.commandHandler.renderContainerHelper(
          args.source as ConnectorModel
        ) as Connector);
      if (
        ...
        if (
          (((object instanceof Selector &&
            ((Math.round(object.width) === Math.round(this.undoElement.width) &&
              Math.round(object.height) ===
                Math.round(this.undoElement.height)) ||
              (object.selectedObjects &&
                object.selectedObjects.length === 1))) ||
            !(object instanceof Selector)) &&
            ((object as NodeModel).offsetX !== this.undoElement.offsetX ||
              (object as NodeModel).offsetY !== this.undoElement.offsetY ||
              (object as ConnectorModel).sourcePoint !==
                (this.undoElement as any).sourcePoint ||
              // eslint-disable-next-line max-len
              (object as ConnectorModel).targetPoint !==
                (this.undoElement as any).targetPoint)) ||
          ...
          arg = {
            ...
            state: 'Completed',
            oldValue: cloneBlazorObject(this.intialValue),
          ...
          if ((object as Node).id === 'helper') {
            if (this.canTrigger) {
              this.commandHandler.triggerEvent(
                DiagramEvent.positionChange,
                arg
              );
      ...
      if (
            ...
            if (this.currentElement && this.currentElement instanceof Node) {
              if (this.currentElement.children !== undefined) {
                for (let i = 0; i < this.currentElement.children.length; i++) {
                  if (
                    this.currentElement.children[parseInt(i.toString(), 10)] ===
                      this.currentTarget.sourceID ||
                    this.currentElement.children[parseInt(i.toString(), 10)] ===
                ...
                if (
                  this.currentElement.id === this.currentTarget.sourceID ||
                  this.currentElement.id === this.currentTarget.targetID
                ) {
                  canSplit = false;
                }
              }
              ...
              this.currentElement instanceof Selector &&
              !this.commandHandler.PreventConnectorSplit
            ) {
              if (this.currentElement.nodes[0].children) {
                for (
                  let i = 0;
                  i < this.currentElement.nodes[0].children.length;
                  i++
                ) {
                  if (
                    this.currentElement.nodes[0].children[
                      parseInt(i.toString(), 10)
                    ] === this.currentTarget.sourceID ||
                    this.currentElement.nodes[0].children[
                      parseInt(i.toString(), 10)
                ...
                if (
                  this.currentElement.nodes[0].id ===
                    this.currentTarget.sourceID ||
                  this.currentElement.nodes[0].id ===
                    this.currentTarget.targetID
                ) {
        ...
        if (
          !arg.cancel &&
          args.source &&
          this.commandHandler.isParentAsContainer(this.currentTarget) &&
          !this.commandHandler.isTargetSubProcess(this.currentTarget)
        ) {
          let nodes: NodeModel[] =
          ...
          for (let i = 0; i < nodes.length; i++) {
            if (
              (nodes[0] as Node).parentId ===
              (nodes[parseInt(i.toString(), 10)] as Node).parentId
            ) {
              temp = true;
            } else {
              temp = false;
          ...
          if (
            this.commandHandler.diagram.selectedItems.nodes.length !==
            nodes.length
          ) {
          ...
          for (let i: number = 0; i < nodes.length; i++) {
            if (!nodes[parseInt(i.toString(), 10)].container && temp) {
              isEndGroup = true;
              this.commandHandler.updateLaneChildrenZindex(
                nodes[parseInt(i.toString(), 10)] as Node,
                this.currentTarget
              );
  ...
  private connectorEndPointChangeEvent(
    arg: any,
    snappedPoint?: PointModel
  ): void {
    let selectedElement: any = arg.source;
    if (
      selectedElement instanceof Selector &&
      selectedElement.nodes.length > 0
    ) {
      for (let i: number = 0; i < selectedElement.nodes.length; i++) {
        ...
        if (node && (node as any).inEdges.length > 0) {
          for (let j: number = 0; j < (node as any).inEdges.length; j++) {
            let connector: ConnectorModel =
              this.commandHandler.diagram.getObject(
                (node as any).inEdges[parseInt(j.toString(), 10)]
              );
            this.triggerEndPointEvent(
              connector,
              arg,
              snappedPoint,
              'targetPointChange'
        ...
        if (node && (node as any).outEdges.length > 0) {
          for (let j: number = 0; j < (node as any).outEdges.length; j++) {
            let connector: ConnectorModel =
              this.commandHandler.diagram.getObject(
                (node as any).outEdges[parseInt(j.toString(), 10)]
              );
            this.triggerEndPointEvent(
              connector,
              arg,
  ...
  private triggerEndPointEvent(
    ...
    let args: IEndChangeEventArgs = {
      connector: connector,
      state: arg.state,
      targetNode: connector.targetID,
      targetPort: connector.targetPortID,
      sourceNode: connector.sourceID,
      sourcePort: connector.sourcePortID,
      oldValue: { x: connector.targetPoint.x, y: connector.targetPoint.y },
      newValue: {
        x: connector.targetPoint.x + (snappedPoint ? snappedPoint.x : 0),
        y: connector.targetPoint.y + (snappedPoint ? snappedPoint.y : 0),
      },
      cancel: arg.cancel,
    };
    this.commandHandler.triggerEvent(
      eventName === 'targetPointChange'
        ? DiagramEvent.targetPointChange
        : DiagramEvent.sourcePointChange,
      args
    );
  ...
  private isSelectionHasConnector(args: any): boolean {
    if (
      ...
      (args.width !== this.undoElement.width ||
        args.height !== this.undoElement.height)
    ) {
      return true;
    }
    return false;
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    object =
      (this.commandHandler.renderContainerHelper(
        args.source as NodeModel
      ) as Node) || (args.source as Node | Connector | Selector);
    if (object instanceof Node || object instanceof Connector) {
      if (object instanceof Node) {
        if (
          object.offsetX === this.undoElement.nodes[0].offsetX &&
      ...
      if (
        object.wrapper.offsetX === this.undoElement.wrapper.offsetX &&
    ...
    let arg: IDraggingEventArgs | IBlazorDraggingEventArgs = {
      source: object as SelectorModel,
      state: 'Start',
      oldValue: oldValues,
      newValue: oldValues,
      target: args.target,
    ...
    arg = {
      source: cloneBlazorObject(object) as SelectorModel,
      state: 'Start',
    ...
    this.currentPosition = args.position;
    if (this.objectType !== 'Port') {
      const x: number = this.currentPosition.x - this.prevPosition.x;
      const y: number = this.currentPosition.y - this.prevPosition.y;
      const diffX: number =
        this.initialOffset.x + (this.currentPosition.x - this.prevPosition.x);
      const diffY: number =
        this.initialOffset.y + (this.currentPosition.y - this.prevPosition.y);
      ...
      if (this.currentTarget && args.target !== this.currentTarget) {
        this.commandHandler.removeChildFromBPmn(
          args.source,
          args.target,
          this.currentTarget
        );
      ...
      if (!arg.cancel && !this.canCancel) {
        this.blocked = !this.commandHandler.dragSelectedObjects(
          snappedPoint.x,
          snappedPoint.y
        );
        const blocked: boolean = !this.commandHandler.mouseOver(
          this.currentElement,
          this.currentTarget,
          this.currentPosition
        );
      ...
      if (
        this.currentTarget &&
        args.source !== this.currentTarget &&
        this.commandHandler.isDroppable(args.source, this.currentTarget) &&
        (args.source as Node).id !== 'helper'
      ) {
        const object: NodeModel =
          args.source instanceof Selector ? args.source.nodes[0] : args.source;
        if (
          !this.commandHandler.isParentAsContainer(object, true) &&
          object.shape.type !== 'SwimLane' &&
          !(object.shape as SwimLaneModel).isPhase
        ) {
          if ((this.currentTarget as Node).isLane) {
            this.commandHandler.renderStackHighlighter(
              args,
              this.currentTarget
            );
          } else {
      ...
      if (this.commandHandler.canEnableDefaultTooltip()) {
        const content: string = this.getTooltipContent(
          args.source as SelectorModel
        );
        const contentTemp = function () {
          return content;
        };
      ...
      const node: NodeModel = args.source as Node;
      rotateMatrix(
        matrix,
        -node.rotateAngle || -node.wrapper.rotateAngle,
        node.offsetX || node.wrapper.offsetX,
        node.offsetY || node.wrapper.offsetY
      );
      const prevPosition: PointModel = transformPointByMatrix(matrix, {
        x: this.prevPosition.x,
        y: this.prevPosition.y,
      });
      const position: PointModel = transformPointByMatrix(matrix, {
        x: args.position.x,
        y: args.position.y,
      });
  ...
  private getTooltipContent(node: SelectorModel): string {
    return (
      'X:' +
      Math.round(node.wrapper.bounds.x) +
      ' ' +
      'Y:' +
      Math.round(node.wrapper.bounds.y)
    );
  }
...
export class RotateTool extends ToolBase {
  ...
  public mouseDown(args: MouseEventArgs): void {
    ...
    if (this.undoElement.nodes[0] && this.undoElement.nodes[0].children) {
      ...
      const nodes: (NodeModel | ConnectorModel)[] =
        this.commandHandler.getAllDescendants(
          this.undoElement.nodes[0],
          objects
        );
      for (let i: number = 0; i < nodes.length; i++) {
        ...
        this.childTable[nodes[parseInt(i.toString(), 10)].id] =
          cloneObject(node);
      }
    }
  ...
  public async mouseUp(args: MouseEventArgs): Promise<void> {
    ...
    let object: NodeModel | ConnectorModel | SelectorModel;
    object =
      (this.commandHandler.renderContainerHelper(args.source) as Node) ||
      (args.source as Node | Selector);
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    object =
      (this.commandHandler.renderContainerHelper(
        args.source as NodeModel
      ) as Node) || (args.source as Node | Selector);
    if (
      this.undoElement.rotateAngle === object.wrapper.rotateAngle &&
      !this.rotateStart
    ) {
      const oldValue: SelectorModel = {
        rotateAngle: object.wrapper.rotateAngle,
...
export class ResizeTool extends ToolBase {
  ...
  public async mouseUp(
    ...
    if (
      this.undoElement.offsetX !== object.wrapper.offsetX ||
      this.undoElement.offsetY !== object.wrapper.offsetY ||
      this.undoElement.width !== object.wrapper.bounds.width ||
      this.undoElement.height !== object.wrapper.bounds.height
    ) {
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    const sx: number =
      (this.currentElement.wrapper.actualSize.width + changes.x) /
      this.currentElement.wrapper.actualSize.width;
    const sy: number =
      (this.currentElement.wrapper.actualSize.height + changes.y) /
      this.currentElement.wrapper.actualSize.height;
  ...
  private getTooltipContent(node: SelectorModel): string {
    return (
      'W:' +
      Math.round(node.wrapper.bounds.width) +
      ' ' +
      'H:' +
      Math.round(node.wrapper.bounds.height)
    );
  }
  ...
  private scaleObjects(
    ...
    if (
      ...
      source.nodes[0].constraints & NodeConstraints.AspectRatio
    ) {
      if (
        corner === 'ResizeWest' ||
        corner === 'ResizeEast' ||
    ...
    arg1 = {
      source: cloneBlazorObject(source) as Selector,
      state: 'Progress',
      oldValue: cloneBlazorObject(oldValue),
      newValue: cloneBlazorObject(newValue),
      cancel: arg.cancel,
...
export class ConnectorDrawingTool extends ConnectTool {
  ...
  public async mouseDown(args: MouseEventArgs): Promise<void> {
    ...
    this.triggerElementDrawEvent(
      ...
      'Start',
      'Connector',
      (this.commandHandler.diagram.drawingObject as ConnectorModel).type,
      true
    );
  }
  /**
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (this.inAction) {
      ...
      if (!this.drawingObject) {
        this.drawingObject = this.commandHandler.drawObject(
      ...
      this.triggerElementDrawEvent(
        ...
        'Progress',
        'Connector',
        (this.drawingObject as ConnectorModel).type,
        false
      );
      //Bug 874781: Port Draw Connection is not proper with group node.
      if (
        args.actualObject &&
        ((args.actualObject as Node).parentId ||
          (args.actualObject as Node).children) &&
        (this.drawingObject as ConnectorModel).sourceID === ''
      ) {
        this.setTarget(args);
      }
      if (
        ((args.target && args.target instanceof Node) ||
          (args.actualObject &&
            args.sourceWrapper &&
            checkPort(args.actualObject, args.sourceWrapper))) &&
        (this.endPoint !== 'ConnectorTargetEnd' ||
          canInConnect(args.target as NodeModel))
      ) {
    ...
    if (!this.inAction) {
      ...
      if (
        args.source &&
        !(args.source as SwimLaneModel).isLane &&
        args.sourceWrapper
      ) {
  ...
  private setTarget(args: MouseEventArgs) {
    if (args.target) {
      if (!args.sourceWrapper.id.includes((args.target as NodeModel).id)) {
        if (
          (args.target as Node).parentId &&
          args.sourceWrapper.id.includes((args.target as Node).parentId)
        ) {
          args.target =
            this.commandHandler.diagram.nameTable[
              (args.target as Node).parentId
            ];
        }
      }
      ...
      if (
        !args.sourceWrapper.id.includes((args.actualObject as NodeModel).id)
      ) {
        if (
          (args.actualObject as Node).parentId &&
          args.sourceWrapper.id.includes((args.actualObject as Node).parentId)
        ) {
          args.target =
            this.commandHandler.diagram.nameTable[
              (args.actualObject as Node).parentId
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (this.inAction) {
      if (
        ...
        args.moveTouches &&
        args.moveTouches.length &&
        args.moveTouches.length >= 2
      ) {
        ...
        const scale: number =
          this.getDistance(moveTouch0, moveTouch1) /
          this.getDistance(startTouch0, startTouch1);
...
export class LabelTool extends ToolBase {
  ...
  public mouseUp(args: MouseEventArgs): void {
    ...
    if (
      (args.sourceWrapper as TextElement).hyperlink.hyperlinkOpenState ===
      'CurrentTab'
    ) {
      tab = '_self';
    } else if (
    ...
    const win: Window = window.open(
      (args.sourceWrapper as TextElement).hyperlink.link,
      tab,
      windowOption
    );
    win.focus();
...
export class PolygonDrawingTool extends ToolBase {
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (this.inAction) {
      ...
      (this.drawingObject.wrapper.children[0] as PathElement).data =
        getPolygonPath((this.drawingObject.shape as BasicShapeModel).points);
      if (
        this.inAction &&
        Point.equals(this.currentPosition, this.prevPosition) === false
      ) {
  ...
  public mouseUp(
    args: MouseEventArgs,
    dblClickArgs?: IDoubleClickEventArgs | IClickEventArgs
  ): void {
    this.checkPropertyValue();
    super.mouseMove(args);
    if (this.inAction) {
  ...
  public mouseUp(args: MouseEventArgs): void {
    ...
    if (this.inAction) {
      if (this.drawingObject) {
        ...
        (
          drawObject.segments[drawObject.segments.length - 1] as StraightSegment
        ).point = { x: 0, y: 0 };
        this.commandHandler.addObjectToDiagram(this.drawingObject);
      }
    }
...
export class LabelDragTool extends ToolBase {
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (node instanceof Node) {
      ...
      const diff: PointModel = transformPointByMatrix(matrix, {
        x: difx,
        y: dify,
      });
      difx = diff.x;
...
export class LabelResizeTool extends ToolBase {
  ...
  public mouseDown(args: MouseEventArgs): void {
    this.inAction = true;
    const object: NodeModel | ConnectorModel = (args.source as Selector).nodes
      .length
      ? (args.source as Selector).nodes[0]
  ...
  public mouseUp(args: MouseEventArgs): void {
    this.checkPropertyValue();
    const redoObject: NodeModel | ConnectorModel = (args.source as Selector)
      .nodes.length
      ? (args.source as Selector).nodes[0]
      : (args.source as Selector).connectors[0];
    this.inAction = false;
  ...
  public resizeObject(args: MouseEventArgs): void {
    ...
    object = (args.source as Selector).nodes.length
      ? (args.source as Selector).nodes[0]
      : (args.source as Selector).connectors[0];
    ...
    const pivot: Rect = this.updateSize(
      textElement,
      this.startPosition,
      this.currentPosition,
      corner,
    ...
    const x: number =
      textElement.offsetX - textElement.actualSize.width * textElement.pivot.x;
    const y: number =
      textElement.offsetY - textElement.actualSize.height * textElement.pivot.y;
    ...
    this.commandHandler.labelResize(
      object,
      (args.source as Selector).annotation as ShapeAnnotation,
      deltaWidth,
      deltaHeight,
      pivot,
      args.source as Selector
...
export class LabelRotateTool extends ToolBase {
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (args.source) {
      if (this.inAction) {
        const object: NodeModel | ConnectorModel = (args.source as Selector)
          .nodes[0]
          ? (args.source as Selector).nodes[0]
          : (args.source as Selector).connectors[0];
        let annotation: ShapeAnnotation | PathAnnotation;
        annotation = (args.source as Selector).annotation as
          | ShapeAnnotation
          | PathAnnotation;
  ...
  public mouseUp(args: MouseEventArgs): void {
    this.checkPropertyValue();
    this.inAction = false;
    const redoEntry: NodeModel | ConnectorModel = (args.source as Selector)
      .nodes.length
      ? (args.source as Selector).nodes[0]
      : (args.source as Selector).connectors[0];
...
export class FreeHandTool extends ToolBase {
  ...
  public mouseMove(args: MouseEventArgs): boolean {
    ...
    if (this.inAction) {
      ...
      (this.drawingObject.wrapper.children[0] as PathElement).data =
        getFreeHandPath((this.drawingObject.shape as BasicShapeModel).points);
      (obj as PathModel).data = getFreeHandPath(
        (obj as BasicShapeModel).points
      );
      if (
        this.inAction &&
        Point.equals(this.currentPosition, this.prevPosition) === false
  ...
  public mouseUp(args: MouseEventArgs): void {
    ...
    if (this.inAction) {
      ...
      const newObj: ConnectorModel = {
        id: 'newConnector' + randomId(),
        type: 'Bezier',
        sourcePoint: { x: points[0].x, y: points[0].y },
        targetPoint: {
      ...
      this.drawingObject = this.bezierCurveSmoothness(
        points,
        smoothValue,
        this.drawingObject,
        obj
  ...
  public pointReduction(points: PointModel[], tolerance: number) {
    ...
    while (
      points[parseInt(firstPoint.toString(), 10)] ===
      points[parseInt(lastPoint.toString(), 10)]
    ) {
      lastPoint--;
    }
  ...
  public reduction(
    ...
    for (let i: number = firstPoint; i < lastPoint; i++) {
      let distance: number = this.perpendicularDistance(
        points[parseInt(firstPoint.toString(), 10)] as Point,
        points[parseInt(lastPoint.toString(), 10)] as Point,
        points[parseInt(i.toString(), 10)] as Point
      );
  ...
  private bezierCurveSmoothness(
    ...
    for (let i: number = 0; i < points.length - 1; i++) {
      ...
      let Controlpointx1: number =
        xm1 + (xc2 - xm1) * smoothValue + pointx1 - xm1;
      let Controlpointy1: number =
        ym1 + (yc2 - ym1) * smoothValue + pointy1 - ym1;
      let Controlpointx2: number =
        xm2 + (xc2 - xm2) * smoothValue + pointx2 - xm2;
      let Controlpointy2: number =
        ym2 + (yc2 - ym2) * smoothValue + pointy2 - ym2;
      ...
      segment.type = 'Bezier';
      (drawingObject as Connector).segments[parseInt(i.toString(), 10)] =
        segment;
      if (i === 0) {
      ...
      (
        (drawingObject as Connector).segments[
          parseInt(i.toString(), 10)
        ] as BezierSegment
      ).vector1 = {
        angle: findAngle(segSourcePoint, cnPt1),
        distance: Point.findLength(segSourcePoint, cnPt1),
      };
      (
        (drawingObject as Connector).segments[
          parseInt(i.toString(), 10)
        ...
        angle: findAngle(segTargetPoint, cnPt2),
        distance: Point.findLength(segTargetPoint, cnPt2),
      };
      (
        (drawingObject as Connector).segments[
          parseInt(i.toString(), 10)
        ] as BezierSegment
      ).point = segTargetPoint;
    }
    return drawingObject;
  }
}
