...
export class CommandHandler {
  ...
  public showTooltip(
    ...
    if (node instanceof Selector) {
      if (node.nodes.length === 1 && node.connectors.length === 0) {
        ...
        targetEle = document.getElementById(
          this.diagram.element.id + '_SelectorElement'
        );
      }
    ...
    if (this.diagram.selectedItems.setTooltipTemplate) {
      ...
      const setTooltipTemplate: Function = getFunction(
        this.diagram.selectedItems.setTooltipTemplate
      );
    ...
    if ((node as Node).tooltip) {
      (this.diagram.tooltipObject as DiagramTooltipModel).openOn = (
        (node as Node).tooltip as DiagramTooltipModel
      ).openOn;
    }
   ...
   * @private
   */
  public connectorSplit(
    droppedObject: NodeModel,
    targetConnector: ConnectorModel
  ): void {
    const droppedNodeId: string = droppedObject.id;
    const existingConnector: ConnectorModel = cloneObject(targetConnector);
    const connectorIndex: number =
      this.diagram.connectors.indexOf(targetConnector);
    ...
    connectorOldChanges[parseInt(connectorIndex.toString(), 10)] =
      connectorOldProperty;
    ...
    if (
      existingConnector.sourceID !== '' &&
      existingConnector.targetID !== ''
    ) {
      connectorNewProperty.targetID = this.ConnectorTargetChange(
        targetConnector,
        droppedNodeId
      );
    } else if (
      ...
      this.nodeOffsetChange(
        nodeNewProperty,
        droppedNode,
        targetConnector.targetPoint
      );
      connectorNewProperty.targetID = this.ConnectorTargetChange(
        targetConnector,
        droppedNodeId
      ...
      (existingConnector.sourceID === '' &&
        existingConnector.targetID === '') ||
      (existingConnector.sourceID === '' && existingConnector.targetID !== '')
    ) {
      this.nodeOffsetChange(
        nodeNewProperty,
        droppedNode,
        targetConnector.sourcePoint
      );
      connectorNewProperty.sourceID = this.ConnectorSourceChange(
        targetConnector,
        droppedNodeId
      );
    }
    connectorNewChanges[parseInt(connectorIndex.toString(), 10)] =
      connectorNewProperty;
    nodeNewChanges[parseInt(nodeIndex.toString(), 10)] = nodeNewProperty;
    this.diagram.nodePropertyChange(
      droppedObject as Node,
      nodeOldProperty as Node,
      nodeNewProperty as Node
    );
    this.diagram.updateSelector();
    this.diagram.connectorPropertyChange(
      targetConnector as Connector,
      connectorOldProperty as Connector,
    ...
    if (
      ...
      if (
        existingConnector.sourcePortID !== '' &&
        existingConnector.targetPortID !== ''
      ) {
    ...
    const entry: HistoryEntry = {
      ...
      redoObject: { nodes: nodeNewChanges as NodeModel[] },
      undoObject: { nodes: nodeOldChanges as NodeModel[] },
      category: 'Internal',
    };
    this.diagram.addHistoryEntry(entry);
    const entry1: HistoryEntry = {
      type: 'PropertyChanged',
      redoObject: { connectors: connectorNewChanges as ConnectorModel[] },
      undoObject: { connectors: connectorOldChanges as ConnectorModel[] },
      category: 'Internal',
    };
  ...
  private nodeOffsetChange(
    propertyChangeArg: NodeModel,
    node: NodeModel,
    nodeNewOffset: PointModel
  ): void {
  ...
  public async triggerEvent(
    event: DiagramEvent,
    args: Object
  ): Promise<void | object | IBlazorConnectionChangeEventArgs> {
    if (
      event === DiagramEvent.drop ||
      event === DiagramEvent.positionChange ||
      ...
      if (
        this.diagram.currentDrawingObject &&
        event !== DiagramEvent.positionChange
      ) {
        return;
      }
    ...
    const temparg: IBlazorConnectionChangeEventArgs =
      (await this.diagram.triggerEvent(
        event,
        args
  ...
  public disConnect(
    ...
    if (
      obj &&
      connector &&
      (hasSingleConnection(this.diagram) || this.diagram.currentDrawingObject)
    ) {
      if (
        endPoint &&
        (endPoint === 'ConnectorSourceEnd' || endPoint === 'ConnectorTargetEnd')
      ) {
        const nodeEndId: string =
          endPoint === 'ConnectorSourceEnd' ? 'sourceID' : 'targetID';
        const portEndId: string =
          endPoint === 'ConnectorSourceEnd' ? 'sourcePortID' : 'targetPortID';
        if (connector[`${nodeEndId}`]) {
          ...
          returnargs = this.connectionEventChange(
            connector,
            oldChanges,
            newChanges,
            endPoint,
  ...
  private connectionEventChange(
    connector: Connector,
    oldChanges: Connector,
    newChanges: Connector,
    endPoint: string,
    canCancel?: boolean
  ): IConnectionChangeEventArgs | IBlazorConnectionChangeEventArgs {
    const nodeEndId: string =
    ...
    if (this.diagram.bpmnModule) {
      if ((connector as any).isBpmnAnnotationConnector) {
        const textAnnotationNode: NodeModel =
          this.diagram.nameTable[connector.targetID];
        (
          textAnnotationNode.shape as BpmnShape
        ).textAnnotation.textAnnotationTarget = connector.sourceID;
      }
    }
    return returnargs;
  }
  ...
  public findTarget(
    ...
    if (argsTarget instanceof Node) {
      ...
      if (
        source &&
        argsTarget.shape.type === 'Bpmn' &&
        !isBlazor() &&
        (argsTarget.shape as BpmnShape).shape === 'Activity'
      ) {
        if (
          (argsTarget.shape as BpmnShape).activity.subProcess.type ===
          'Transaction'
        ) {
          const transaction: BpmnTransactionSubProcessModel = (
            argsTarget.shape as BpmnShape
          ).activity.subProcess.transaction;
          if (
            transaction.success.visible &&
            element.id.indexOf(argsTarget.id + '_success') === 0
          ) {
            return transaction.success;
          }

          if (
            transaction.cancel.visible &&
            element.id.indexOf(argsTarget.id + '_cancel') === 0
          ) {
          ...
          if (
            ...
            element.id.indexOf(argsTarget.id + '_failure') === 0
          ) {
      ...
      if (element instanceof PathElement) {
        for (let i: number = 0; i < argsTarget.ports.length; i++) {
          const port: PointPortModel =
            argsTarget.ports[parseInt(i.toString(), 10)];
    ...
    if (!connection) {
      ...
      for (
        let i: number = 0;
        i < (argsTarget as NodeModel | ConnectorModel).annotations.length;
        i++
      ) {
        annotation = (argsTarget as NodeModel | ConnectorModel).annotations[
          parseInt(i.toString(), 10)
        ];
        if (
          element.id ===
          (argsTarget as NodeModel | ConnectorModel).id + '_' + annotation.id
        ) {
          return annotation;
        }
      }
  ...
  public canDisconnect(
    ...
    const targetObject: NodeModel | PointPortModel = this.findTarget(
      args.targetWrapper,
      args.target,
      endPoint === 'ConnectorSourceEnd',
      true
    ) as NodeModel | PointPortModel;
    const nodeEnd: string =
      endPoint === 'ConnectorSourceEnd' ? 'sourceID' : 'targetID';
    const portEnd: string =
      endPoint === 'ConnectorSourceEnd' ? 'sourcePortID' : 'targetPortID';
    if (
      connect[`${nodeEnd}`] !== targetNodeId ||
      connect[`${portEnd}`] !== targetPortId
    ) {
      return true;
    }
    return false;
  ...
  public connect(
    ...
    if (target instanceof Node) {
      ...
      connector[`${nodeEndId}`] =
        (args.target && (args.target as Node).id) ||
        (args.actualObject as Node).id;
      newChanges[`${nodeEndId}`] = connector[`${nodeEndId}`] as Connector;
      newChanges[`${portEndId}`] = connector[`${portEndId}`] as Connector;
      let arg: IConnectionChangeEventArgs | IBlazorConnectionChangeEventArgs = {
        connector: cloneBlazorObject(connector),
        oldValue: { nodeId: oldNodeId, portId: oldPortId },
        newValue: {
          nodeId: newChanges[`${nodeEndId}`],
          portId: newChanges[`${portEndId}`],
        },
  ...
  public getObjectLayer(objectName: string): LayerModel {
    ...
    if (layers.length > 1) {
      for (let i: number = 0; i < layers.length; i++) {
        const objIndex: number =
          layers[parseInt(i.toString(), 10)].objects.indexOf(objectName);
        if (objIndex > -1) {
  ...
  public removeLayer(layerId: string, isServerUpdate: boolean = true): void {
    ...
    if (layers) {
      ...
      for (let i: number = layerObject.length - 1; i >= 0; i--) {
        this.diagram.unSelect(
          this.diagram.nameTable[layerObject[parseInt(i.toString(), 10)]]
        );
        this.diagram.remove(
          this.diagram.nameTable[layerObject[parseInt(i.toString(), 10)]]
        );
        if (layers.id !== 'default_layer') {
          if (this.diagram.activeLayer.id === layerId) {
            this.diagram.activeLayer =
              this.diagram.layers[this.diagram.layers.length - 1];
          }
        }
      }
      // if (isServerUpdate) {
  ...
  public moveObjects(objects: string[], targetLayer?: string): void {
    ...
    let childNodes: any = [];
    for (let i: number = 0; i < objects.length; i++) {
      const obj: Node | Connector =
        this.diagram.nameTable[objects[parseInt(i.toString(), 10)]];
    ...
    const layer: LayerModel =
      this.getLayer(targetLayer) || this.diagram.activeLayer;
  ...
  private setConnectorDetails(
    obj: ConnectorModel | NodeModel,
    connectorObjectsDetails: object
  ): void {
      ...
      if (details) {
          ...
          for (let i: number = 0; i < details.inEdges.length; i++) {
            const con: ConnectorModel =
              this.diagram.nameTable[
                details.inEdges[parseInt(i.toString(), 10)]
              ];
            con.targetID = obj.id;
          }
        }
        if (details.outEdges && details.outEdges.length > 0) {
          for (let i: number = 0; i < details.outEdges.length; i++) {
            const con: ConnectorModel =
              this.diagram.nameTable[
                details.outEdges[parseInt(i.toString(), 10)]
              ];
            con.sourceID = obj.id;
          }
        }
  ...
  public copyObjects(): Object[] {
    ...
    if (this.diagram.selectedItems.connectors.length > 0) {
      ...
      for (let j: number = 0; j < selectedItems.length; j++) {
        ...
        if (
          this.diagram.bpmnModule &&
          (selectedItems[parseInt(j.toString(), 10)] as any)
            .isBpmnAnnotationConnector
        ) {
          element = cloneObject(
            this.diagram.nameTable[
              (selectedItems[parseInt(j.toString(), 10)] as Connector).targetID
            ]
          );
        } else {
    ...
    if (this.diagram.selectedItems.nodes.length > 0) {
      ...
      for (
        let j: number = 0;
        j < this.diagram.selectedItems.nodes.length;
        j++
      ) {
        if (
          !(
            this.diagram.selectedItems.nodes[parseInt(j.toString(), 10)] as Node
          ).isPhase
        ) {
          const node: NodeModel = clone(
            this.diagram.selectedItems.nodes[parseInt(j.toString(), 10)]
          );
          if (
            !(
              ((node as Node).isLane || (node as Node).isHeader) &&
              this.checkSwimlaneInSelection(node as Node, obj)
            )
          ) {
            ...
            if (node.children) {
              ...
              const nodes: (NodeModel | ConnectorModel)[] =
                this.getAllDescendants(node, elements, true);
              for (let i: number = 0; i < nodes.length; i++) {
                tempNode =
                  this.diagram.nameTable[nodes[parseInt(i.toString(), 10)].id];
                const clonedObject: NodeModel | ConnectorModel = (childTable[
                  tempNode.id
                ] = clone(tempNode));
                const newOffset: PointModel = transformPointByMatrix(matrix, {
    ...
    if (
      this.diagram.selectedItems.nodes.some(function (node: NodeModel): any {
  ...
  private removeDuplicateObjects(objects: NodeModel[]): any {
    ...
    for (let i: number = 0; i < objects.length; i++) {
      uniqueObjects[objects[parseInt(i.toString(), 10)].id] =
        objects[parseInt(i.toString(), 10)];
    }
    const result: any = [];
    ...
    for (let i: number = 0; i < result.length; i++) {
      if (result[parseInt(i.toString(), 10)].parentId) {
        const lane: NodeModel = this.diagram.getObject(
          result[parseInt(i.toString(), 10)].parentId
        );
        if (
          lane &&
          (lane as Node).isLane &&
          (lane as Node).parentId &&
  ...
  private checkSwimlaneInSelection(node: Node, obj: NodeModel[]): any {
    return this.diagram.selectedItems.nodes.some(
      (item: NodeModel) => item.id === node.parentId
    );
  }
  private copyProcesses(node: Node): void {
    if (
      ...
      for (const i of processes) {
        ...
        if (
          (this.processTable[`${i}`].shape as BpmnShape).activity.subProcess
            .processes &&
          (this.processTable[`${i}`].shape as BpmnShape).activity.subProcess
            .processes.length > 0
        ) {
  ...
  public group(): void {
    ...
    this.diagram.diagramActions =
      this.diagram.diagramActions | DiagramAction.Group;
    let selectedItems: (NodeModel | ConnectorModel)[] = [];
    let obj: NodeModel = {};
    ...
    const order: (NodeModel | ConnectorModel)[] = selectedItems.sort(function (
      a: NodeModel | ConnectorModel,
      b: NodeModel | ConnectorModel
    ): number {
  ...
  public unGroup(obj?: NodeModel): void {
    ...
    for (let i: number = 0; i < selectedItems.length; i++) {
      ...
      if (node.children) {
        ...
        if (
          (node as Node).annotations &&
          (node as Node).annotations.length > 0 &&
          !isBlazor()
        ) {
          this.diagram.removeLabels(node as Node, (node as Node).annotations);
        }
        const parentNode: NodeModel =
          this.diagram.nameTable[(node as Node).parentId];
        for (let j: number = node.children.length - 1; j >= 0; j--) {
          this.diagram.nameTable[
            node.children[parseInt(j.toString(), 10)]
          ].parentId = '';
          this.diagram.deleteChild(
            this.diagram.nameTable[node.children[parseInt(j.toString(), 10)]],
            node
          );
          if (
            (node as Node).parentId &&
  ...
  private resetDependentConnectors(edges: string[], isInEdges: boolean): void {
    for (let i: number = 0; i < edges.length; i++) {
      const newConnector: ConnectorModel =
        this.diagram.nameTable[edges[parseInt(i.toString(), 10)]];
      ...
      this.diagram.connectorPropertyChange(
        newConnector as Connector,
        {} as Connector,
        newProp
      );
      const entry: HistoryEntry = {
        type: 'ConnectionChanged',
        undoObject: { connectors: [undoObject], nodes: [] },
        redoObject: { connectors: [cloneObject(newConnector)], nodes: [] },
        category: 'Internal',
      };
      if (!(this.diagram.diagramActions & DiagramAction.UndoRedo)) {
        this.addHistoryEntry(entry);
  ...
  public paste(obj: (NodeModel | ConnectorModel)[]): void {
    if (obj || this.clipboardData.clipObject) {
      this.diagram.protectPropertyChange(true);
      const copiedItems: (NodeModel | ConnectorModel)[] = obj
        ? this.getNewObject(obj)
      ...
      if (copiedItems) {
        ...
        for (let j: number = 0; j < copiedItems.length; j++) {
          const copy: NodeModel | ConnectorModel =
            copiedItems[parseInt(j.toString(), 10)];
          //EJ2-841227-Copy paste of child node from group node
          if ((copy as Node).parentId) {
            const parentObj: NodeModel = this.diagram.getObject(
              (copy as Node).parentId
            );
            if (
              parentObj.shape.type !== 'SwimLane' &&
              (copy as Node).parentId
            ) {
              (copy as Node).parentId = '';
            }
          ...
          if (getObjectType(copy) === Connector) {
            ...
            if (clonedObj.targetID) {
              const targetNode: Node =
                this.diagram.nameTable[clonedObj.targetID];
              if (
                (targetNode.shape as BpmnShape).shape === 'TextAnnotation' &&
                targetNode.inEdges &&
                targetNode.inEdges.length > 0
              ) {
                allowClone = false;
              }
            }
            if (allowClone) {
              const newObj: ConnectorModel = this.cloneConnector(
                clonedObj,
                multiSelect
              );
            ...
            const newNode: NodeModel = this.cloneNode(
              copy as NodeModel,
              multiSelect
            );
              ...
              if (edges) {
                for (const edge of edges) {
                  if (objectTable[`${edge}`] && keyTable[`${edge}`]) {
                    const newConnector: ConnectorModel =
                      this.diagram.nameTable[keyTable[`${edge}`]];
                    newConnector.targetID = keyTable[copy.id];
                    this.diagram.connectorPropertyChange(
                      newConnector as Connector,
                      { targetID: '', targetPortID: '' } as Connector,
                      {
                        targetID: newConnector.targetID,
                        targetPortID: newConnector.targetPortID,
                      } as Connector
              ...
              if (edges) {
                for (const edge of edges) {
                  if (objectTable[`${edge}`] && keyTable[`${edge}`]) {
                    ...
                    this.diagram.connectorPropertyChange(
                      newConnector as Connector,
                      { sourceID: '', sourcePortID: '' } as Connector,
                      {
                        sourceID: newConnector.sourceID,
                        sourcePortID: newConnector.sourcePortID,
                      } as Connector
  ...
  private cloneNode(
    ...
    if (
      ...
      process = ((cloneObject as Node).shape as BpmnShape).activity.subProcess
        .processes;
      (cloneObject as Node).zIndex = Number.MIN_VALUE;
    ...
    if (node.shape && node.shape.type === 'SwimLane') {
      ...
      (cloneObject as Node).zIndex = Number.MIN_VALUE;
      if (children) {
        (cloneObject as Node).children = children;
      }
      if (
        (cloneObject as Node).shape &&
        ((cloneObject as Node).shape as BpmnShape).shape === 'TextAnnotation'
      ) {
    ...
    if (process && process.length) {
      ((newNode as Node).shape as BpmnShape).activity.subProcess.processes =
        process;
  ...
  private cloneSubProcesses(node: NodeModel): void {
    ...
    if (
      ...
      const process: string[] = (node.shape as BpmnShape).activity.subProcess
        .processes;
      for (let g: number = 0; g < process.length; g++) {
        const child: Node =
          this.diagram.nameTable[process[parseInt(g.toString(), 10)]] ||
        ...
        const innerChild: Node = cloneObject(
          this.clipboardData.processTable[process[parseInt(g.toString(), 10)]]
        ) as Node;
        ...
        for (const i of connector) {
          const node: ConnectorModel =
            this.diagram.nameTable[`${i}`] ||
            this.diagram.connectorTable[`${i}`];
          const clonedNode: Object = cloneObject(node);
          if (
            temp[(clonedNode as Connector).sourceID] &&
            temp[(clonedNode as Connector).targetID]
          ) {
            (clonedNode as Connector).zIndex = -1;
            (clonedNode as Connector).id += randomId();
            (clonedNode as Connector).sourceID =
              temp[(clonedNode as Connector).sourceID];
            (clonedNode as Connector).targetID =
              temp[(clonedNode as Connector).targetID];
  ...
  private cloneGroup(obj: NodeModel, multiSelect: boolean): NodeModel {
    ...
    if (this.clipboardData.childTable || obj.children.length > 0) {
      for (let i: number = 0; i < children.length; i++) {
        ...
        if (this.clipboardData.childTable) {
          childObj =
            this.clipboardData.childTable[children[parseInt(i.toString(), 10)]];
          // EJ2-905181 - Added to consider copy paste and clone the new group node.
          if (!childObj) {
            childObj =
              this.diagram.nameTable[children[parseInt(i.toString(), 10)]];
          }
        } else {
          childObj =
            this.diagram.nameTable[children[parseInt(i.toString(), 10)]];
        ...
        if (childObj) {
          if (getObjectType(childObj) === Connector) {
            ...
            newObj = this.cloneNode(
              childObj as NodeModel,
              multiSelect,
              undefined,
              id
            );
    ...
    for (let k: number = 0; k < connectorObj.length; k++) {
      if (
        connectorObj[parseInt(k.toString(), 10)].sourceID ||
        connectorObj[parseInt(k.toString(), 10)].targetID
      ) {
        for (let j: number = 0; j < oldID.length; j++) {
          if (
            connectorObj[parseInt(k.toString(), 10)].sourceID ===
            oldID[parseInt(j.toString(), 10)]
          ) {
      ...
      newObj = this.cloneConnector(
        connectorObj[parseInt(k.toString(), 10)],
        multiSelect
      );
    ...
    if (
      ...
      parentObj.container &&
      parentObj.shape &&
      parentObj.shape.type === 'UmlClassifier'
    ) {
    ...
    this.diagram.blazorActions &= ~BlazorAction.GroupClipboardInProcess;
    if (!this.diagram.isServerUpdate) {
      this.diagram.UpdateBlazorDiagramModelCollection(
        undefined,
        objectCollection,
        undefined,
        true
  ...
  public translateObject(obj: Node | Connector, groupnodeID?: string): void {
    ...
    if (getObjectType(obj) === Connector) {
      (obj as Connector).sourcePoint = {
        x: (obj as Connector).sourcePoint.x + diff,
        y: (obj as Connector).sourcePoint.y + diff,
      };
      (obj as Connector).targetPoint = {
        x: (obj as Connector).targetPoint.x + diff,
        y: (obj as Connector).targetPoint.y + diff,
      };
      if ((obj as Connector).type === 'Bezier') {
        const segments: BezierSegment[] = (obj as Connector)
          .segments as BezierSegment[];
        for (let i: number = 0; i < segments.length; i++) {
          if (
            !Point.isEmptyPoint(segments[parseInt(i.toString(), 10)].point1)
          ) {
            segments[parseInt(i.toString(), 10)].point1 = {
              x: segments[parseInt(i.toString(), 10)].point1.x + diff,
              y: segments[parseInt(i.toString(), 10)].point1.y + diff,
            };
          }
          if (
            !Point.isEmptyPoint(segments[parseInt(i.toString(), 10)].point2)
          ) {
            segments[parseInt(i.toString(), 10)].point2 = {
              x: segments[parseInt(i.toString(), 10)].point2.x + diff,
              y: segments[parseInt(i.toString(), 10)].point2.y + diff,
            };
          }
        }
      }
      if (
        (obj as Connector).type === 'Straight' ||
        (obj as Connector).type === 'Bezier'
      ) {
        if (
          (obj as Connector).segments &&
          (obj as Connector).segments.length > 0
        ) {
          const segments: (StraightSegmentModel | BezierSegmentModel)[] = (
            obj as Connector
          ).segments;
  ...
  public drawObject(obj: Node | Connector): Node | Connector {
    ...
    if (obj && obj.shape) {
      if (obj.shape.type === 'Text') {
        (obj as Node).width = (this.diagram.drawingObject as Node).width
          ? (this.diagram.drawingObject as Node).width
          : 50;
        (obj as Node).height = (this.diagram.drawingObject as Node).height
          ? (this.diagram.drawingObject as Node).height
          : 20;
      }
    }
    const cloneObject: Node | Connector = clone(this.diagram.drawingObject) as
      | Node
      | Connector;
    ...
    if (
      getObjectType(this.diagram.drawingObject) === Node ||
      (getObjectType(this.diagram.drawingObject) === Connector &&
        (this.diagram.drawingObject as Connector).type === 'Freehand' &&
        (obj as Connector).type !== 'Bezier')
    ) {
  ...
  public addObjectToDiagram(obj: Node | Connector): void {
    ...
    if (
      ...
      (this.diagram.drawingObject as ConnectorModel) &&
      (this.diagram.drawingObject as ConnectorModel).type === 'Bezier'
    ) {
      obj.segments = [];
    }
  ...
  public addText(obj: Node | Connector, currentPosition: PointModel): void {
    const annotation: DiagramElement = this.diagram.findElementUnderMouse(
      obj,
      currentPosition,
      this.diagram
    );
    this.diagram.startTextEdit(
      obj,
      annotation instanceof TextElement
  ...
  public isUserHandle(position: PointModel): boolean {
    ...
    if (handle.wrapper && canShowCorner(handle.constraints, 'UserHandle')) {
      for (const obj of handle.userHandles) {
        if (obj.visible) {
          const paddedBounds: PointModel = getUserHandlePosition(
            handle,
            obj,
          ...
          if (
            contains(
              position,
              paddedBounds,
              obj.size / (2 * this.diagram.scroller.transform.scale)
            )
          ) {
            return true;
          }
  ...
  public async selectObjects(
    ...
    if (obj.length > 0 && obj[0] && (obj[0] as SwimLaneModel).isLane) {
      const swimlaneNode: NodeModel = this.diagram.getObject(
        (obj[0] as Node).parentId
      );
      ...
      for (let j: number = 0; j < obj.length; j++) {
        for (
          let i: number = 0;
          i < (swimlaneNode.shape as SwimLaneModel).lanes.length;
          i++
        ) {
          const parentId: string[] = (obj[0] as Node).id.split(
            (obj[0] as Node).parentId
          );
          ...
          if (
            ...
            (obj[0].shape as any).header.push(
              (swimlaneNode.shape as SwimLaneModel).lanes[
                parseInt(i.toString(), 10)
              ].header
            );
    ...
    const oldSelectedItems: NodeModel[] =
      this.diagram.selectedItems.nodes.concat(
        this.diagram.selectedItems.connectors as NodeModel
      );
    ...
    if (canDoSingleSelection || canDoMultipleSelection) {
      if (
        !canDoMultipleSelection &&
        (obj.length > 1 || (multipleSelection && obj.length === 1))
      ) {
        if (obj.length === 1) {
          this.clearSelection();
        } else {
      ...
      if (
        !(canDoSingleSelection || canDoMultipleSelection) &&
        obj.length === 1 &&
    ...
    if (!arg.cancel) {
      for (let i: number = 0; i < obj.length; i++) {
        const newObj: NodeModel | ConnectorModel =
        ...
        if (newObj) {
          ...
          if (!hasSelection(this.diagram)) {
            ...
            if (
              (i > 0 || multipleSelection) &&
              (newObj as Node).children &&
              !(newObj as Node).parentId
            ) {
              for (
                let i: number = 0;
  ...
  private selectProcesses(newObj: Node): void {
    if (this.hasProcesses(newObj)) {
      const processes: string[] = (newObj.shape as BpmnShape).activity
        .subProcess.processes;
      for (let i: number = 0; i < processes.length; i++) {
        const innerChild: NodeModel | ConnectorModel =
          this.diagram.nameTable[processes[parseInt(i.toString(), 10)]];
  ...
  private selectGroup(newObj: Node): void {
    for (let j: number = 0; j < (newObj as Node).children.length; j++) {
      const innerChild: NodeModel | ConnectorModel =
        this.diagram.nameTable[
          (newObj as Node).children[parseInt(j.toString(), 10)]
        ];
      if ((innerChild as Node).children) {
        this.selectGroup(innerChild as Node);
      }
      this.unSelect(
        this.diagram.nameTable[
          (newObj as Node).children[parseInt(j.toString(), 10)]
  ...
  private selectBpmnSubProcesses(node: Node): boolean {
    ...
    if (node.processId) {
      if (isSelected(this.diagram, this.diagram.nameTable[node.processId])) {
        ...
        select = this.selectBpmnSubProcesses(
          this.diagram.nameTable[node.processId]
        );
      }
    } else if (node instanceof Connector) {
      if (
        node.sourceID &&
        this.diagram.nameTable[node.sourceID] &&
      ...
      if (
        node.targetID &&
        this.diagram.nameTable[node.targetID] &&
      ...
      if (parent) {
        if (isSelected(this.diagram, this.diagram.nameTable[`${parent}`])) {
          ...
          select = this.selectBpmnSubProcesses(
            this.diagram.nameTable[`${parent}`]
          );
        }
      }
    } else if (
      node.parentId &&
      this.diagram.nameTable[node.parentId] &&
    ...
    if (
      ...
      if (!multipleSelection) {
        ...
        if (
          ...
          selectorModel.wrapper.rotateAngle =
            selectorModel.nodes[0].rotateAngle;
  ...
  public labelSelect(
    ...
    selectorModel.annotation = this.findTarget(textWrapper, obj as IElement) as
      | PathAnnotationModel
      | ShapeAnnotationModel;
  ...
  public unSelect(obj: NodeModel | ConnectorModel): void {
    ...
    objArray.push(obj);
    const items: NodeModel[] = this.diagram.selectedItems.nodes.concat(
      this.diagram.selectedItems.connectors as NodeModel
    );
    const selectedObjects: NodeModel[] = items.filter(function (
      items: NodeModel
    ): boolean {
    ...
    if (isSelected(this.diagram, obj)) {
      ...
      arg = {
        oldValue: cloneBlazorObject(items) as NodeModel[],
        newValue: selectedObjects,
        cause: this.diagram.diagramActions,
        state: 'Changed',
        type: 'Removal',
  ...
  public moveSvgNode(nodeId: string, targetID: string): void {
    const diagramDiv: HTMLElement = getDiagramElement(
      targetID + '_groupElement',
      this.diagram.element.id
    );
    const backNode: HTMLElement = getDiagramElement(
  ...
  public sendLayerBackward(layerName: string): void {
    ...
    if (layer && layer.zIndex !== 0) {
      ...
      if (this.diagram.mode === 'SVG') {
        ...
        const targetObject: string = this.getLayer(
          this.diagram.layerZIndexTable[index - 1]
        ).objects[0];
        if (targetObject) {
      ...
      const targetLayer: LayerModel = this.getLayer(
        this.diagram.layerZIndexTable[index - 1]
      );
      targetLayer.zIndex = targetLayer.zIndex + 1;
      layer.zIndex = layer.zIndex - 1;
      const temp: string =
        this.diagram.layerZIndexTable[parseInt(index.toString(), 10)];
      this.diagram.layerZIndexTable[parseInt(index.toString(), 10)] =
        this.diagram.layerZIndexTable[index - 1];
  ...
  public sendToBack(object?: NodeModel | ConnectorModel): void {
    ...
    if (hasSelection(this.diagram) || object) {
      ...
      for (let i: number = 0; i < objects.length; i++) {
        const clonedObject: object = cloneObject(
          objects[parseInt(i.toString(), 10)]
        );
        objectId = objects[parseInt(i.toString(), 10)].id;
        const index: number = this.diagram.nameTable[`${objectId}`].zIndex;
        const layerNum: number = this.diagram.layers.indexOf(
          this.getObjectLayer(objectId)
        );
        const zIndexTable: {} = (
          this.diagram.layers[parseInt(layerNum.toString(), 10)] as Layer
        ).zIndexTable;
        const layerObjects: string[] = (
          this.diagram.layers[parseInt(layerNum.toString(), 10)] as Layer
        ).objects;
        ...
        for (let i: number = 0; i < layerObjects.length; i++) {
          const obj: NodeModel | ConnectorModel =
            this.diagram.nameTable[layerObjects[parseInt(i.toString(), 10)]];
        ...
        const undoObject: SelectorModel = cloneObject(
        ...
        if (
          this.diagram.nodes.length !== 1 &&
          (this.diagram.nameTable[`${objectId}`].children === undefined ||
          ...
          if (obj.zIndex > minZindex && obj.shape.type !== 'SwimLane') {
            ...
            obj.zIndex =
              minZindex -
              ((objects.indexOf(obj) !== -1 ? objects.indexOf(obj) : 0) + 1);
            if (obj.children) {
              minZindex =
                this.findLowestZIndex(obj as Node, layerObjects) || minZindex;
            }
            ...
            tempIndex = this.swapZIndexObjects(
              index,
              zIndexTable,
              objectId,
              tempTable
          ...
          if (this.diagram.mode === 'SVG') {
            const obj: NodeModel = this.diagram.nameTable[`${objectId}`];
            let i: number =
              obj.shape.type !== 'SwimLane' ? minZindex : tempIndex;
            // if (i !== tempIndex) {
            ...
            if (target) {
              target = this.diagram.nameTable[`${target}`].parentId
                ? this.checkParentExist(target)
                : target;
          ...
          const redoObject: SelectorModel = cloneObject(
            this.diagram.selectedItems
          );
          const entry: HistoryEntry = {
            type: 'SendToBack',
  ...
  private updateGroupZindex(
    ...
    if (command === 'SendToBack' || command === 'SendBackward') {
      ...
      for (let i: number = 0; i < parent.children.length; i++) {
        const child: any =
          this.diagram.nameTable[parent.children[parseInt(i.toString(), 10)]];
        if (child.children) {
  ...
  private updateLayerZindexTable(layerIndex: number): void {
    const layer: LayerModel =
      this.diagram.layers[parseInt(layerIndex.toString(), 10)];
    (layer as Layer).zIndexTable = {};
    for (let i: number = 0; i < layer.objects.length; i++) {
      const obj: NodeModel | ConnectorModel =
        this.diagram.nameTable[layer.objects[parseInt(i.toString(), 10)]];
  ...
  private swapZIndexObjects(
    ...
    for (let i: number = 0; i <= index; i++) {
      if (
        zIndexTable[parseInt(i.toString(), 10)] &&
        this.diagram.nameTable[zIndexTable[parseInt(i.toString(), 10)]]
          .parentId === objectId
      ) {
    ...
    for (let i: number = 0; i <= index; i++) {
      if (zIndexTable[parseInt(i.toString(), 10)] && j <= childCount) {
        ...
        zIndexTable[parseInt(i.toString(), 10)] =
          zIndexTable[parseInt(childIndex.toString(), 10)];
        this.diagram.nameTable[zIndexTable[parseInt(i.toString(), 10)]].zIndex =
          i;
        childIndex++;
        j++;
      }
    ...
    for (let i: number = 0; i < childCount; i++) {
      ...
      tempIndex =
        this.diagram.nameTable[zIndexTable[parseInt(k.toString(), 10)]].zIndex;
      k++;
    }
    ...
    zIndexTable[parseInt(tempIndex.toString(), 10)] =
      this.diagram.nameTable[`${objectId}`].id;
    ...
    for (let i: number = tempIndex; i <= index; i++) {
      if (zIndexTable[parseInt(i.toString(), 10)]) {
        ...
        zIndexTable[parseInt(i.toString(), 10)] =
          tempTable[parseInt(k.toString(), 10)];
        this.diagram.nameTable[zIndexTable[parseInt(i.toString(), 10)]].zIndex =
          i;
        k++;
      }
  ...
  private resetTargetNode(
    ...
    if (
      this.diagram.nameTable[`${objectId}`].shape.type === 'SwimLane' &&
      this.diagram.nameTable[`${target}`].parentId !== undefined &&
      this.diagram.nameTable[`${target}`].parentId !== '' &&
      this.diagram.nameTable[this.diagram.nameTable[`${target}`].parentId]
        .isLane
    ) {
      i = i + 1;
      if (zIndexTable[parseInt(i.toString(), 10)]) {
        target = zIndexTable[parseInt(i.toString(), 10)];
        return (target = this.resetTargetNode(
          objectId,
          target,
          i,
          zIndexTable
  ...
  public bringToFront(obj?: NodeModel | ConnectorModel): void {
    ...
    if (hasSelection(this.diagram) || obj) {
      ...
      for (let i: number = 0; i < objects.length; i++) {
        ...
        const layerNum: number = this.diagram.layers.indexOf(
          this.getObjectLayer(objectName)
        );
        const zIndexTable: {} = (
          this.diagram.layers[parseInt(layerNum.toString(), 10)] as Layer
        ).zIndexTable;
        ...
        let tabelLength: number = Number(
          Object.keys(zIndexTable)
        ...
        if (obj.zIndex < maxZindex && obj.shape.type !== 'SwimLane') {
          ...
          if (obj.children) {
            maxZindex =
              this.findGreatestZIndex(obj as Node, layerObjects) || maxZindex;
        ...
        // }
          ...
          for (let i: number = 0; i <= tabelLength; i++) {
            if (
              tempTable[parseInt(i.toString(), 10)] &&
              tempTable[parseInt(i.toString(), 10)] !== objectName &&
              this.diagram.nameTable[tempTable[parseInt(i.toString(), 10)]]
                .parentId !== objectName
            ) {
              const node: Node =
                this.diagram.nameTable[tempTable[parseInt(i.toString(), 10)]];
              const swimlaneObject: Node =
                this.diagram.nameTable[`${objectName}`];
          ...
          for (let i: number = 0; i <= childCount; i++) {
            ...
            laneIndex =
              this.diagram.nameTable[zIndexTable[parseInt(k.toString(), 10)]]
                .zIndex;
            k++;
          }
          for (let i: number = laneIndex; i <= tabelLength; i++) {
            ...
            zIndexTable[parseInt(i.toString(), 10)] =
              cloneTable[parseInt(childIndex.toString(), 10)];
            this.diagram.nameTable[
              zIndexTable[parseInt(i.toString(), 10)]
            ].zIndex = i;
            childIndex++;
          }
          zIndexTable[parseInt(tabelLength.toString(), 10)] =
            this.diagram.nameTable[`${objectName}`].id;
          this.diagram.nameTable[`${objectName}`].zIndex = tabelLength;
          k = index + 1;
          ...
          for (let i: number = 0; i < childCount; i++) {
            ...
            while (
              this.diagram.nameTable[tempTable[parseInt(k.toString(), 10)]]
                .parentId === objectName
            ) {
              k++;
            }
            ...
            zIndexTable[parseInt(j.toString(), 10)] =
              tempTable[parseInt(k.toString(), 10)];
            this.diagram.nameTable[
              zIndexTable[parseInt(j.toString(), 10)]
            ].zIndex = j;
        ...
        if (this.diagram.mode === 'SVG') {
          ...
          const obj: NodeModel = this.diagram.nameTable[`${objectName}`];
          const i: number =
            obj.shape.type !== 'SwimLane' ? maxZindex : tempIndex;
          // if (i !== tempIndex) {
          //     i = (obj.children && obj.children.length > 0) ? index : 1;
          // }
          let target: string = zIndexTable[parseInt(i.toString(), 10)];
          ...
          const diagramLayer: SVGGElement = this.diagram
            .diagramLayer as SVGGElement;
          let element: HTMLElement = getDiagramElement(
            objectName + '_groupElement',
            this.diagram.element.id
          );
          const nodes: NodeModel[] = this.diagram.selectedItems.nodes;
          if (
            nodes.length > 0 &&
            (nodes[0].shape.type === 'Native' || nodes[0].shape.type === 'HTML')
          ) {
            element.parentNode.removeChild(element);
            for (let j: number = 0; j < this.diagram.views.length; j++) {
              element = getDiagramElement(
                objectName +
                  (nodes[0].shape.type === 'HTML'
                    ? '_html_element'
                    : '_content_groupElement'),
                this.diagram.views[parseInt(j.toString(), 10)]
              );
              const lastChildNode: HTMLElement = element.parentNode
            ...
            const htmlLayer: HTMLElement = getHTMLLayer(
              this.diagram.element.id
            );
            this.diagram.diagramRenderer.renderElement(
              this.diagram.nameTable[`${objectName}`].wrapper,
              diagramLayer,
              htmlLayer
            );
          }
  ...
  private findGreatestZIndex(parent: Node, layerObjects: string[]): number {
    const layerObj: NodeModel[] = this.diagram.nodes.filter(
      (node: any) => layerObjects.indexOf(node.id) !== -1
    );
    // Get zIndex values of nodes excluding children of the parent
    const zIndexes: number[] = layerObj
      .filter((node: any) => node.parentId === '' && node.id !== parent.id)
      .map((node: NodeModel) => (node as Node).zIndex);
  ...
  private triggerOrderCommand(
    oldObj: NodeModel | ConnectorModel,
    newObj: NodeModel | ConnectorModel,
    obj: NodeModel | ConnectorModel
  ): void {
    ...
    const arg: IPropertyChangeEventArgs = {
      element: obj,
      cause: this.diagram.diagramActions,
      diagramAction: this.diagram.getDiagramAction(this.diagram.diagramActions),
      oldValue: oldValue,
      newValue: newValue,
    };
  ...
  private checkGroupNode(
    ...
    return (
      nameTable[`${layerObject}`].parentId ===
      nameTable[`${selectedNodeName}`].parentId
    );
  }
  ...
  public orderCommands(
    ...
    if (isRedo) {
      ...
      const endZIndex: number =
        this.diagram.nameTable[selectedObject[0].id].zIndex;
      ...
      if (action === 'SendBackward' || action === 'SendForward') {
        for (let i: number = 0; i < selectedObject.length; i++) {
          const undoObject: NodeModel = selectedObject[
            parseInt(i.toString(), 10)
          ] as NodeModel;
          const layer: number = this.diagram.layers.indexOf(
            this.getObjectLayer(undoObject.id)
          );
          const node: NodeModel =
            this.diagram.nameTable[
              selectedObject[parseInt(i.toString(), 10)].id
            ];
          node.zIndex = undoObject.zIndex;
          (
            this.diagram.layers[parseInt(layer.toString(), 10)] as Layer
          ).zIndexTable[undoObject.zIndex] = undoObject.id;
        }
      } else if (action === 'BringToFront') {
      ...
      if (action === 'BringToFront' || action === 'SendToBack') {
        ...
        node.zIndex = undoObject.zIndex;
        (
          this.diagram.layers[parseInt(layerIndex.toString(), 10)] as Layer
        ).zIndexTable[undoObject.zIndex] = undoObject.id;
      ...
      if (this.diagram.mode === 'SVG') {
        ...
        // }
          ...
          if (selectedObject[0].shape.type !== 'SwimLane') {
            ...
            //     }
            // }
            const zIndexArray: any[] = layer.objects.map(
              (x: string) => this.diagram.nameTable[`${x}`].zIndex
            );
            const filteredZIndexArray: any[] = zIndexArray.filter(
              (zIndex: any) => zIndex > startZIndex
            );
            const filteredZIndexArray2: any[] = zIndexArray.filter(
              (zIndex: any) => zIndex < startZIndex
            );
            const nextLowestZindex: number =
              filteredZIndexArray2.length > 0
                ? Math.max(...filteredZIndexArray2)
                : startZIndex;
            const nextGreatestZIndex: number =
              filteredZIndexArray.length > 0
                ? Math.min(...filteredZIndexArray)
                : startZIndex;
            if (startZIndex !== nextGreatestZIndex) {
              const first: string = (layer as Layer).zIndexTable[
                parseInt(nextGreatestZIndex.toString(), 10)
              ];
              const second: string = (layer as Layer).zIndexTable[
                parseInt(startZIndex.toString(), 10)
              ];
              ...
              const second: string = (layer as Layer).zIndexTable[
                parseInt(nextLowestZindex.toString(), 10)
              ];
  ...
  private moveSBObject: (targetId: string) => void = function (
    targetId: string
  ): void {
    if (targetId) {
  ...
  private moveBackUndoNode: (targetId: string) => void = function (
    targetId: string
  ): void {
    ...
    for (let j: number = 0; j < this.diagram.connectors.length; j++) {
      const connector: ConnectorModel =
        this.diagram.connectors[parseInt(j.toString(), 10)];
      originalZIndexTable[connector.id] = connector.zIndex;
    }
    const sortedNodeIds: string[] = Object.keys(originalZIndexTable).sort(
      (a: string, b: string) =>
        originalZIndexTable[`${a}`] - originalZIndexTable[`${b}`]
    );
      ...
      if (currentIndex === sortedNodeIds.length - 1) {
        ...
        if (nextNodeId) {
          ...
          if (newnode.parentId && newnode.parentId !== target.parentId) {
            backNode = getDiagramElement(
              newnode.parentId + '_groupElement',
              this.diagram.element.id
            );
          }
  ...
  private moveFBObject: (targetId: string) => void = function (
    targetId: string
  ): void {
    if (targetId) {
  ...
  private moveForwardUndoNode: (targetID: string) => void = function (
    targetID: string
  ): void {
    const originalZIndexTable: object = {}; // New variable to store original zIndex values
    // Store original zIndex values
  ...
  public sendForward(obj?: NodeModel | ConnectorModel): void {
    ...
    if (hasSelection(this.diagram) || obj) {
      ...
      const layerIndex: number = this.diagram.layers.indexOf(
        this.getObjectLayer(nodeId)
      );
      ...
      let temp: Object[] = this.diagram.spatialSearch.findObjects(
        index.wrapper.bounds
      );
      if (temp.length > 2) {
      ...
      for (const i of temp) {
        if (index.id !== (i as NodeModel).id) {
          const currentLayer: number = this.getObjectLayer(
            (i as NodeModel).id
          ).zIndex;
          if (
            layerIndex === currentLayer &&
            Number(this.diagram.nameTable[`${nodeId}`].zIndex) <
              Number((i as NodeModel).zIndex) &&
            (i as Node).parentId === '' &&
      ...
      for (let i: number = intersectArray.length - 1; i >= 0; i--) {
        const child: Node =
          this.diagram.nameTable[intersectArray[parseInt(i.toString(), 10)].id];
      ...
      if (intersectArray.length > 0) {
        const node: Node =
          this.diagram.nameTable[intersectArray[intersectArray.length - 1].id];
        if (node.parentId) {
          const parentId: string = '';
          const parent: string = findParentInSwimlane(
            node,
            this.diagram,
            parentId
          );
        ...
        for (let i: number = 0; i < this.diagram.nodes.length; i++) {
          const node1: NodeModel =
            this.diagram.nodes[parseInt(i.toString(), 10)];
          originalZIndexTable[node1.id] = node1.zIndex;
        }
        ...
        const temp: string =
          zIndexTable[parseInt(overlapObject.toString(), 10)];
        const undoObject: SelectorModel = cloneObject(
          this.diagram.selectedItems
        );
        this.diagram.startGroupAction();
        ...
        Object.keys(this.changedNodeZIndexes).forEach((nodeId: string) => {
          const originalZIndex: any = originalNameTable[`${nodeId}`]
            ? originalNameTable[`${nodeId}`].zIndex
            : null;
          ...
          if (originalZIndex !== changedZIndex) {
            const node: NodeModel | ConnectorModel = cloneObject(
              originalNameTable[`${nodeId}`]
            );
        ...
        if (this.diagram.mode === 'SVG') {
          const nodeIdToUpdate: string =
            intersectArray[intersectArray.length - 1].id;
          const element: NodeModel | ConnectorModel =
            intersectArray[intersectArray.length - 1];
          if (
            element &&
            !(element.shape.type === 'HTML' || element.shape.type === 'Native')
          ) {
  ...
  private updateZIndexBySendForward(
    selectedNode: NodeModel | ConnectorModel,
    greaterItems: number[],
    layerNum: number
  ): void {
    ...
    let highIndex: any;
    const layerObjects: string[] =
      this.diagram.layers[parseInt(layerNum.toString(), 10)].objects;
    const objects: NodeModel[] | ConnectorModel[] = this.diagram.nodes.filter(
      (node: any) => layerObjects.indexOf(node.id) !== -1
    );
    if ((selectedNode as Node).children) {
      this.updateGroupZindex(
        selectedNode as Node,
        'BringForward',
        selectedNode.zIndex
      );
    }
    const frontNode: NodeModel[] = objects.filter(
      (x: NodeModel) =>
        x.zIndex === Math.min(...greaterItems) && x.id !== selectedNode.id
    );
    if (frontNode && frontNode.length > 0 && frontNode[0].children) {
      ...
      for (let i: number = 0; i < frontNode[0].children.length; i++) {
        const frontNodeChild: any =
          this.diagram.nameTable[
            frontNode[0].children[parseInt(i.toString(), 10)]
          ];
        frontNodeChild.zIndex = frontNodeIndex - (i + 1);
        highChildIndex = frontNodeIndex - (i + 1);
      }
      this.arrangeZIndexForLowerNodes(
        highChildIndex,
        frontNode[0] as Node,
        objects
      );
    ...
    const sortedObjects: NodeModel[] = objects
      .slice()
    ...
    for (let i: number = 0; i < sortedObjects.length; i++) {
      ...
      if (
        node.zIndex > index &&
        node !== selectedNode &&
        (node as Node).parentId !== selectedNode.id &&
        frontNode &&
        frontNode.length > 0 &&
        (node as Node).parentId !== frontNode[0].id
      ) {
        const clonedNode: Object = cloneObject(node);
  ...
  private arrangeZIndexForLowerNodes(
    index: number,
    parent: Node,
    objects: NodeModel[]
  ): void {
    const lesserIndexNodes: NodeModel[] = objects.filter(
      (x: NodeModel) => x.zIndex <= index && (x as Node).parentId !== parent.id
    );
  ...
  public sendBackward(obj?: NodeModel | ConnectorModel): void {
    ...
    if (hasSelection(this.diagram) || obj) {
      ...
      for (const i of temp) {
        if (node.id !== (i as NodeModel).id) {
          ...
          if (
            ...
            Number(this.diagram.nameTable[`${objectId}`].zIndex) >
              Number((i as NodeModel).zIndex) &&
            (i as Node).parentId === '' &&
      ...
      if (intersectArray.length > 0) {
        ...
        const overlapObject: number =
          intersectArray[intersectArray.length - 1].zIndex;
        const temp: string =
          zIndexTable[parseInt(overlapObject.toString(), 10)];
        const undoObject: SelectorModel = cloneObject(
          this.diagram.selectedItems
  ...
  private updateZIndexBySendBackward(
    ...
    const backNode: any = objects.filter(
      (x: NodeModel) =>
        x.zIndex === Math.max(...lesserItems) && x.id !== selectedNode.id
    );
    if (backNode && backNode.length > 0 && backNode[0].children) {
      ...
      for (let i: number = 0; i < backNode[0].children.length; i++) {
        const backNodeChild: any =
          this.diagram.nameTable[
            backNode[0].children[parseInt(i.toString(), 10)]
          ];
    ...
    for (let i: number = 0; i < sortedObjects.length; i++) {
      ...
      if (
        ...
        backNode &&
        backNode.length > 0 &&
        (node as Node).parentId !== backNode[0].id
      ) {
        const clonedNode: Object = cloneObject(node);
  ...
  public updateNativeNodeIndex(nodeId: string, targetID?: string): void {
    const node: NodeModel =
      this.diagram.selectedItems.nodes[0] || this.diagram.getObject(targetID);
    for (let i: number = 0; i < this.diagram.views.length; i++) {
      if (
        node &&
        (node.shape.type === 'HTML' || node.shape.type === 'Native')
      ) {
        const id: string =
        ...
        const backNode: HTMLElement = getDiagramElement(
          nodeId + id,
          this.diagram.views[parseInt(i.toString(), 10)]
        );
        const diagramDiv: HTMLElement = targetID
          ? getDiagramElement(
              targetID + id,
              this.diagram.views[parseInt(i.toString(), 10)]
            )
        ...
        if (backNode && diagramDiv) {
          if (
            (backNode.parentNode as HTMLElement).id ===
            (diagramDiv.parentNode as HTMLElement).id
          ) {
            diagramDiv.parentNode.insertBefore(backNode, diagramDiv);
          }
        }
  ...
  public doRubberBandSelection(region: Rect): void {
    ...
    if (
      this.diagram.selectedItems.rubberBandSelectionMode === 'CompleteIntersect'
    ) {
  ...
  private clearSelectionRectangle(): void {
    const adornerSvg: SVGSVGElement = getAdornerLayerSvg(
      this.diagram.element.id
    );
    const element: SVGElement = adornerSvg.getElementById(
      this.diagram.element.id + '_diagramAdorner_selected_region'
    ) as SVGElement;
    if (element) {
      remove(element);
    }
  ...
  public dragConnectorEnds(
    ...
    if (endPoint === 'ConnectorSourceEnd' || endPoint === 'BezierSourceThumb') {
      tx =
        point.x -
        (checkBezierThumb
          ? (segment as BezierSegment).bezierPoint1.x
          : connector.sourcePoint.x);
      ty =
        point.y -
        (checkBezierThumb
          ? (segment as BezierSegment).bezierPoint1.y
      ...
      tx =
        point.x -
        (checkBezierThumb
          ? (segment as BezierSegment).bezierPoint2.x
          : connector.targetPoint.x);
      ty =
        point.y -
        (checkBezierThumb
          ? (segment as BezierSegment).bezierPoint2.y
          : connector.targetPoint.y);
      return this.dragTargetEnd(
        connector,
        tx,
        ty,
        null,
  ...
  public async clearSelection(
    ...
    const enableServerDataBinding: boolean =
      this.diagram.allowServerDataBinding;
    this.diagram.enableServerDataBinding(false);
    if (hasSelection(this.diagram)) {
      const selectormodel: SelectorModel = this.diagram.selectedItems;
      const arrayNodes: (NodeModel | ConnectorModel)[] =
      ...
      if (!arg.cancel) {
        ...
        if (triggerAction) {
          arg = {
            oldValue: cloneBlazorObject(arrayNodes) as NodeModel[],
            newValue: [],
            cause: this.diagram.diagramActions,
            state: 'Changed',
            type: 'Removal',
  ...
  public clearSelectedItems(): void {
    const selectedNodes: number = this.diagram.selectedItems.nodes
      ? this.diagram.selectedItems.nodes.length
      : 0;
    const selectedConnectors: number = this.diagram.selectedItems.connectors
  ...
  public removeStackHighlighter(): void {
    ...
    ).getElementById(adornerSvg.id + '_stack_highlighter') as SVGElement;
    if (highlighter) {
  ...
  public renderStackHighlighter(args: MouseEventArgs, target?: IElement): void {
    ...
    let symbolDrag: boolean;
    let node: NodeModel;
    let selectorModel: SelectorModel;
    if (!target) {
      const objects: IElement[] = this.diagram.findObjectsUnderMouse(
        args.position
      );
      ...
      if (
        target &&
        !(
          (target as Node).isLane ||
          (target as Node).isPhase ||
          (target as Node).isHeader
        )
      ) {
        for (let i: number = 0; i < objects.length; i++) {
          const laneNode: Node =
            this.diagram.nameTable[
              (objects[parseInt(i.toString(), 10)] as NodeModel).id
            ];
    ...
    if (
      ...
      (target as Node).isLane &&
      source.shape &&
      !(source.shape as SwimLaneModel).isPhase
    ) {
      ...
      if (
        (source && !source.parentId && source.shape.type !== 'SwimLane') ||
        (source &&
          source.parentId &&
          this.diagram.nameTable[source.parentId] &&
          this.diagram.nameTable[source.parentId].isLane &&
          source.parentId !== (target as Node).parentId &&
          source.parentId !== (target as Node).id)
      ) {
        selectorModel = this.diagram.selectedItems;
        const canvas: Canvas = gridSelection(
          this.diagram,
          selectorModel,
          (target as Node).id,
          true
    ...
    if (
      ...
      if (node.shape.type === 'SwimLane') {
        ...
        //Instead of assigning the canvas to the selectorModel children, pass them to render selector to highlight the swimlane.
        const canvas: Canvas =
          this.diagram.nameTable[(target as Node).parentId].wrapper;
        this.diagram.renderSelector(false, true, canvas);
      }
    }
    if (
      (symbolDrag &&
        ((this.diagram.currentSymbol as Node).shape as SwimLaneModel).isLane) ||
      (source &&
        target &&
        source.parentId &&
        (target as Node).parentId &&
        !source.isPhase &&
        source.parentId === (target as Node).parentId &&
        source.id !== (target as Node).id &&
        node &&
        node.container &&
        (node.container.type === 'Stack' || node.container.type === 'Grid'))
    ) {
      let canvas: Canvas;
      const value: boolean = node.container.orientation === 'Vertical';
      const isVertical: boolean = node.container === 'Stack' ? value : !value;
      if (
        node.container.type === 'Grid' &&
        (target as Node).isLane &&
        (((node.shape as SwimLaneModel).orientation === 'Horizontal' &&
          (target as Node).rowIndex !== source.rowIndex) ||
          ((node.shape as SwimLaneModel).orientation === 'Vertical' &&
            (target as Node).columnIndex !== source.columnIndex) ||
          (this.diagram.currentSymbol &&
            (this.diagram.currentSymbol.shape as SwimLaneModel).orientation ===
              node.container.orientation))
      ) {
        selectorModel = this.diagram.selectedItems;
        if (
          (source.isLane && canLaneInterchange(source, this.diagram)) ||
          !source.isLane
        ) {
          canvas = gridSelection(
            this.diagram,
            selectorModel,
            (target as Node).id,
            symbolDrag
      ...
      const wrapper: DiagramElement =
        node.container.type === 'Stack' ? target.wrapper : canvas;
      if (wrapper) {
  ...
  public insertBlazorConnector(obj: Selector): void {
    if (obj instanceof Selector) {
      for (let i: number = 0; i < obj.connectors.length; i++) {
        this.diagram.insertBlazorConnector(
          obj.connectors[parseInt(i.toString(), 10)] as Connector
        );
      }
    } else {
  ...
  public drag(obj: NodeModel | ConnectorModel, tx: number, ty: number): void {
    ...
    if (
      !(this.diagram as any).rotateUsingButton &&
      obj.shape &&
    ...
    if (
      canMove(obj) &&
      this.checkBoundaryConstraints(tx, ty, obj.wrapper.bounds) &&
      canPageEditable(this.diagram)
    ) {
      if (obj instanceof Node) {
        const oldValues: NodeModel = {
          offsetX: obj.offsetX,
          offsetY: obj.offsetY,
        };
        obj.offsetX += tx;
        ...
        if (obj.children && !obj.container) {
          if (!checkParentAsContainer(this.diagram, obj, true)) {
            this.diagram.diagramActions =
              this.diagram.diagramActions | DiagramAction.isGroupDragging;
          }
          const nodes: (NodeModel | ConnectorModel)[] = this.getAllDescendants(
            obj,
            elements
          );
          for (let i: number = 0; i < nodes.length; i++) {
        ...
        if (checkParentAsContainer(this.diagram, obj, true)) {
          ...
          this.diagram.nodePropertyChange(
            obj as Node,
            oldValues as Node,
            { offsetX: obj.offsetX, offsetY: obj.offsetY } as Node,
            undefined,
            undefined,
            false
          );
  ...
  /**   @private  */
  public connectorSegmentChange(
    actualObject: Node,
    existingInnerBounds: Rect,
    isRotate: boolean
  ): void {
    let tx: number;
    let ty: number;
    let segmentChange: boolean = true;
    if (
      ...
      if (actualObject.outEdges.length > 0) {
        for (let k: number = 0; k < actualObject.outEdges.length; k++) {
          const connector: Connector =
            this.diagram.nameTable[
              actualObject.outEdges[parseInt(k.toString(), 10)]
            ];
          if (connector.targetID !== '') {
            segmentChange = this.isSelected(
              this.diagram.nameTable[connector.targetID]
            )
              ? false
              : true;
            ...
            segmentChange = this.isSelected(
              this.diagram.nameTable[connector.id]
            )
              ? false
              : true;
          }
          if (
            ...
            connector.segments &&
            connector.segments.length > 1
          ) {
            if (!isRotate) {
              if (segmentChange) {
                switch (
                  (connector.segments[0] as OrthogonalSegment).direction
                ) {
                  case 'Bottom':
                    tx =
                      actualObject.wrapper.bounds.bottomCenter.x -
                      existingInnerBounds.bottomCenter.x;
                    ty =
                      actualObject.wrapper.bounds.bottomCenter.y -
                      existingInnerBounds.bottomCenter.y;
                    break;
                  case 'Top':
                    tx =
                      actualObject.wrapper.bounds.topCenter.x -
                      existingInnerBounds.topCenter.x;
                    ty =
                      actualObject.wrapper.bounds.topCenter.y -
                      existingInnerBounds.topCenter.y;
                    break;
                  case 'Left':
                    tx =
                      actualObject.wrapper.bounds.middleLeft.x -
                      existingInnerBounds.middleLeft.x;
                    ty =
                      actualObject.wrapper.bounds.middleLeft.y -
                      existingInnerBounds.middleLeft.y;
                    break;
                  case 'Right':
                    tx =
                      actualObject.wrapper.bounds.middleRight.x -
                      existingInnerBounds.middleRight.x;
                    ty =
                      actualObject.wrapper.bounds.middleRight.y -
                      existingInnerBounds.middleRight.y;
                    break;
                }
                this.dragSourceEnd(
                  connector,
              ...
              const firstSegment: OrthogonalSegment = connector
                .segments[0] as OrthogonalSegment;
              const secondSegment: OrthogonalSegment = connector
                .segments[1] as OrthogonalSegment;
              const cornerPoints: Corners = swapBounds(
                actualObject.wrapper,
                actualObject.wrapper.corners,
                actualObject.wrapper.bounds
              );
              let sourcePoint: PointModel = findPoint(
                cornerPoints,
                firstSegment.direction
              );
              sourcePoint = getIntersection(
                connector,
                connector.sourceWrapper,
                sourcePoint,
                {
              ...
              const intermediatePoints: PointModel[] = orthoConnection2Segment(
                source,
                target
              );
  ...
  public updateEndPoint(connector: Connector, oldChanges?: Connector): void {
    ...
    this.diagram.connectorPropertyChange(
      connector as Connector,
      oldChanges ? oldChanges : ({} as Connector),
      newValue
    );
    // this.diagram.refreshDiagramLayer();
    this.diagram.updateSelector();
  ...
  public dragSourceEnd(
    ...
    let oldChanges: Connector = {} as Connector;
    const checkBoundaryConstraints: boolean = this.checkBoundaryConstraints(
      tx,
      ty,
      connector.wrapper.bounds
    );
    if (
      canDragSourceEnd(connector as Connector) &&
      checkBoundaryConstraints &&
      endPoint !== 'BezierSourceThumb' &&
      canPageEditable(this.diagram)
    ) {
    ...
    if (connector.type === 'Bezier') {
      ...
      if (segment) {
        this.translateBezierPoints(
          obj,
          endPoint === '' ? 'ConnectorSourceEnd' : endPoint,
          tx,
          ty,
          segment,
          point,
        ...
        for (let i: number = 0; i < obj.segments.length; i++) {
          this.translateBezierPoints(
            ...
            tx,
            ty,
            obj.segments[parseInt(i.toString(), 10)],
            point,
            !update
          );
        }
  ...
  public updatePathElementOffset(connector: ConnectorModel): void {
    ...
    const anglePoints: PointModel[] = (connector as Connector)
      .intermediatePoints as PointModel[];
  ...
  private changeSegmentLength(
    ...
    const canChangeSegment: boolean = target
      ? this.canConnect(connector, target)
      : true;
    if (
      connector.segments &&
      (connector.segments[0] as OrthogonalSegment).direction !== null &&
      ((!target && connector.sourceID === '') || isDragSource) &&
      canChangeSegment
    ) {
      const first: OrthogonalSegment = connector
        .segments[0] as OrthogonalSegment;
      const second: OrthogonalSegment = connector
        .segments[1] as OrthogonalSegment;
      const node: NodeModel = this.diagram.nameTable[connector.sourceID];
      let secPoint: PointModel;
      ...
      if (first.direction && (first.length || first.length === 0)) {
        first.length = Point.distancePoints(
          first.points[0],
          first.points[first.points.length - 1]
        );
      }
      if (second.direction && (second.length || second.length === 0)) {
        second.length = Point.distancePoints(
          first.points[first.points.length - 1],
          second.points[second.points.length - 1]
        );
      ...
      if (connector.sourcePortID !== '' && first.length < 10) {
        if (connector.segments.length > 2) {
          const next: OrthogonalSegment = connector
            .segments[2] as OrthogonalSegment;
          const nextDirection: Direction = Point.direction(
            next.points[0],
            next.points[1]
          ) as Direction;
          if (first.direction === getOppositeDirection(nextDirection)) {
            if (first.direction === 'Right') {
              next.points[0].x = first.points[first.points.length - 1].x =
                node.wrapper.corners.middleRight.x + 20;
            } else if (first.direction === 'Left') {
              next.points[0].x = first.points[first.points.length - 1].x =
                node.wrapper.corners.middleLeft.x - 20;
            } else if (first.direction === 'Top') {
              next.points[0].y = first.points[first.points.length - 1].y =
                node.wrapper.corners.topCenter.y - 20;
            } else {
              next.points[0].y = first.points[first.points.length - 1].y =
                node.wrapper.corners.bottomCenter.y + 20;
            }
            if (next.direction && next.length) {
              next.length = Point.distancePoints(
                next.points[0],
                next.points[next.points.length - 1]
              );
            }
            first.length = Point.distancePoints(
              first.points[0],
      ...
      if (
        target &&
        targetPortId &&
        connector.sourcePortID !== targetPortId &&
        connector.segments &&
        (connector.segments[0] as OrthogonalSegment).direction !== null &&
        target &&
        target instanceof Node &&
  ...
  private changeSourceEndToPort(
    ...
    const port: DiagramElement = this.diagram.getWrapper(
      target.wrapper,
      targetPortId
    );
    const point: PointModel = { x: port.offsetX, y: port.offsetY };
    const direction: Direction = getPortDirection(
      point,
      cornersPointsBeforeRotation(target.wrapper),
      target.wrapper.bounds,
      false
    );
    const firstSegment: OrthogonalSegment = connector
      .segments[0] as OrthogonalSegment;
    ...
    if (firstSegment.direction !== direction) {
      ...
      if (firstSegment.direction === getOppositeDirection(direction)) {
        ...
        segments.push(
          new OrthogonalSegment(connector, 'segments', segValues1, true)
        );
        segments.push(
          new OrthogonalSegment(connector, 'segments', segValues, true)
        );
      } else {
        segValues = {
          type: 'Orthogonal',
      ...
      if (firstSegment.direction !== getOppositeDirection(direction)) {
        if (direction === 'Top' || direction === 'Bottom') {
          firstSegment.points[0].x = point.x;
          firstSegment.points[0].y = firstSegment.points[
            firstSegment.points.length - 1
          ].y = direction === 'Top' ? point.y - 20 : point.y + 20;
        } else {
          firstSegment.points[0].y = point.y;
          firstSegment.points[0].x = firstSegment.points[
            firstSegment.points.length - 1
          ].x = direction === 'Right' ? point.x + 20 : point.x - 20;
        }
        firstSegment.length = Point.distancePoints(
          firstSegment.points[0],
          firstSegment.points[firstSegment.points.length - 1]
        );
        secondSegment.length = Point.distancePoints(
          firstSegment.points[firstSegment.points.length - 1],
          secondSegment.points[secondSegment.points.length - 1]
        );
      }
...
Remove terinal segment in initial
  ...
  private changeSourceEndToNode(connector: ConnectorModel, target: Node): void {
    ...
    const sourceWrapper: Corners = target.wrapper.children[0].corners;
    let sourcePoint: PointModel;
    let sourcePoint2: PointModel;
    const firstSegment: OrthogonalSegment = connector
      .segments[0] as OrthogonalSegment;
    const nextSegment: OrthogonalSegment = connector
      .segments[1] as OrthogonalSegment;
    const segments: OrthogonalSegmentModel[] = [];
    if (
      firstSegment.direction === 'Right' ||
      firstSegment.direction === 'Left'
    ) {
      sourcePoint =
        firstSegment.direction === 'Left'
          ? sourceWrapper.middleLeft
          : sourceWrapper.middleRight;
      if (
        firstSegment.length > sourceWrapper.width ||
        (firstSegment.direction === 'Left' &&
        ...
        sourcePoint2 =
          direction === 'Bottom'
            ? sourceWrapper.bottomCenter
            : sourceWrapper.topCenter;
        if (nextSegment.length && nextSegment.direction) {
          nextSegment.length =
            direction === 'Top'
              ? firstSegment.points[firstSegment.points.length - 1].y -
                (sourcePoint2.y + 20)
              : sourcePoint2.y +
                20 -
                firstSegment.points[firstSegment.points.length - 1].y;
        ...
        firstSegment.length =
          firstSegment.points[firstSegment.points.length - 1].x -
          sourcePoint2.x;
        firstSegment.direction = firstSegment.length > 0 ? 'Right' : 'Left';
        const segValues: Object = {
          type: 'Orthogonal',
          direction: direction,
          length: 20,
        };
      ...
      if (
        firstSegment.length > sourceWrapper.height ||
        (firstSegment.direction === 'Top' &&
  ...
  private translateBezierPoints(
    ...
    tx: number,
    ty: number,
    seg: BezierSegmentModel,
    point?: PointModel,
    update?: boolean
  ): void {
    const index: number = connector.segments.indexOf(seg);
    const segment: BezierSegment = connector.segments[
      parseInt(index.toString(), 10)
    ] as BezierSegment;
    const prevSegment: BezierSegment =
      index > 0 ? (connector.segments[index - 1] as BezierSegment) : null;
    const startPoint: PointModel =
      prevSegment !== null ? prevSegment.point : connector.sourcePoint;
    const endPoint: PointModel =
      index === connector.segments.length - 1
        ? connector.targetPoint
        : segment.point;

    if (segment) {
      if (
        value === 'BezierSourceThumb' &&
        (segment.vector1.angle || segment.vector1.distance)
      ) {
        ...
        this.translateSubsequentSegment(
          connector,
          seg,
          true,
          deltaLength,
        ...
        value === 'BezierTargetThumb' &&
        (segment.vector2.angle || segment.vector2.distance)
      ) {
        ...
        ((value === 'ConnectorSourceEnd' && !connector.sourceID) ||
          (value === 'ConnectorTargetEnd' && !connector.targetID)) &&
        update &&
        isEmptyVector(segment.vector1) &&
        isEmptyVector(segment.vector2)
      ) {
        if (Point.isEmptyPoint(segment.point1)) {
          segment.bezierPoint1 = getBezierPoints(
            connector.sourcePoint,
            connector.targetPoint
          );
        }
        if (Point.isEmptyPoint(segment.point2)) {
          segment.bezierPoint2 = getBezierPoints(
            connector.targetPoint,
            connector.sourcePoint
          );
        }
  ...
  private translateSubsequentSegment(
    ...
    if (
      !(connector.bezierSettings.smoothness & BezierSmoothness.SymmetricAngle)
    ) {
      deltaAngle = null;
    }
    ...
    if (
      !(
        connector.bezierSettings.smoothness & BezierSmoothness.SymmetricDistance
      )
    ) {
      deltaLength = null;
    }
    ...
    if (isSourceEnd) {
      if (index !== 0) {
        this.updatePreviousBezierSegment(
          connector,
          index,
          deltaLength,
          deltaAngle
  ...
  public dragTargetEnd(
    ...
    endPoint?: string,
    update?: boolean,
    segment?: OrthogonalSegmentModel | BezierSegmentModel | StraightSegmentModel
  ): boolean {
    const connector: ConnectorModel = this.diagram.nameTable[obj.id];
    let oldChanges: Connector;
    const boundaryConstraints: boolean = this.checkBoundaryConstraints(
      tx,
      ty,
      connector.wrapper.bounds
    );
    if (
      canDragTargetEnd(connector as Connector) &&
      endPoint !== 'BezierTargetThumb' &&
      boundaryConstraints &&
      canPageEditable(this.diagram)
    ) {
      ...
      if (
        endPoint === 'ConnectorTargetEnd' &&
        connector.type === 'Orthogonal' &&
        connector.segments &&
        connector.segments.length > 0
      ) {
        const prev: OrthogonalSegment = connector.segments[
          connector.segments.length - 2
        ...
        if (
          ...
          prev.length = Point.distancePoints(
            prev.points[0],
            prev.points[prev.points.length - 1]
          );
          prev.direction = Point.direction(
            prev.points[0],
            prev.points[prev.points.length - 1]
          ) as Direction;
        }
    ...
    if (connector.type === 'Bezier') {
      ...
      if (segment) {
        this.translateBezierPoints(
          obj,
          endPoint === '' ? 'ConnectorTargetEnd' : endPoint,
          tx,
          ty,
          segment,
          point,
  ...
  public dragControlPoint(
    ...
    const connector: ConnectorModel = this.diagram.nameTable[obj.id];
    if (
      (connector.type === 'Straight' || connector.type === 'Bezier') &&
      connector.segments.length > 0
    ) {
      if (
        segmentNumber !== undefined &&
        connector.segments[parseInt(segmentNumber.toString(), 10)]
      ) {
        if (connector.type === 'Bezier') {
          const seg: BezierSegmentModel = connector.segments[
            parseInt(segmentNumber.toString(), 10)
          ] as BezierSegmentModel;
          const isInternalSegment = (seg as BezierSegment).isInternalSegment;
          if (
            !isInternalSegment ||
            connector.bezierSettings === null ||
            connector.bezierSettings.segmentEditOrientation === 'FreeForm'
          ) {
            seg.point.x += tx;
            seg.point.y += ty;
          } else {
  ...
  public rotateObjects(
    parent: NodeModel | SelectorModel,
      ...
      if (obj instanceof Node) {
        if (canRotate(obj) && canPageEditable(this.diagram)) {
          ...
          if (obj.processId) {
            ...
            const bound: Rect = this.diagram.bpmnModule.getChildrenBound(
              parent,
              obj.id,
              this.diagram
            );
            this.diagram.bpmnModule.updateSubProcessess(
              bound,
              obj,
              this.diagram
            );
        ...
        this.rotatePoints(
          obj as Connector,
          angle,
          pivot || { x: obj.wrapper.offsetX, y: obj.wrapper.offsetY }
        );
      }
  ...
  public snapConnectorEnd(currentPosition: PointModel): PointModel {
    if (
      this.diagram.snapSettings.constraints & SnapConstraints.SnapToLines &&
      this.snappingModule
    ) {
  ...
  public rotatePoints(conn: Connector, angle: number, pivot: PointModel): void {
    if (!conn.sourceWrapper || !conn.targetWrapper) {
      ...
      if (
        conn.shape.type === 'Bpmn' &&
        (conn.shape as BpmnFlow).sequence === 'Default' &&
        (conn.shape as BpmnFlow).flow === 'Sequence'
      ) {
        this.updatePathElementOffset(conn);
      }
      const newProp: Connector = {
  ...
  private updateInnerParentProperties(tempNode: NodeModel): void {
    ...
    this.diagram.protectPropertyChange(true);
    const innerParents: (NodeModel | ConnectorModel)[] = this.getAllDescendants(
      tempNode,
      elements,
      false,
      true
    ...
    for (let i: number = 0; i < innerParents.length; i++) {
      const obj: NodeModel =
        this.diagram.nameTable[innerParents[parseInt(i.toString(), 10)].id];
  ...
  public scale(
    ...
    if (!refObject) {
      refObject = obj as IElement;
    }
    const refWrapper: Container = refObject.wrapper;
    const x: number =
      refWrapper.offsetX - refWrapper.actualSize.width * refWrapper.pivot.x;
    const y: number =
      refWrapper.offsetY - refWrapper.actualSize.height * refWrapper.pivot.y;
    const refPoint: PointModel = getPoint(
      x,
      y,
      refWrapper.actualSize.width,
      refWrapper.actualSize.height,
      refWrapper.rotateAngle,
      refWrapper.offsetX,
      refWrapper.offsetY,
      pivot
    );
  ...
  public getAllDescendants(
    node: NodeModel,
    nodes: (NodeModel | ConnectorModel)[],
    includeParent?: boolean,
    innerParent?: boolean
  ): (NodeModel | ConnectorModel)[] {
    const temp: NodeModel = node;
    const parentNodes: NodeModel[] = [];
  ...
  public getChildren(
    ...
    if (node.children) {
      for (let i: number = 0; i < temp.children.length; i++) {
        node =
          this.diagram.nameTable[temp.children[parseInt(i.toString(), 10)]];
        nodes.push(node);
      }
    }
    return nodes;
    ...
    if (sw !== 1 || sh !== 1) {
      ...
      if (obj instanceof Node) {
        ...
        if (
          ...
          bound = this.diagram.bpmnModule.getChildrenBound(
            node,
            node.id,
            this.diagram
          );
      ...
      if (obj instanceof Node) {
        ...
        const newPosition: PointModel = transformPointByMatrix(matrix, {
          x: node.wrapper.offsetX,
          y: node.wrapper.offsetY,
        });
        const oldleft: number =
          node.wrapper.offsetX - node.wrapper.actualSize.width * node.pivot.x;
        const oldtop: number =
          node.wrapper.offsetY - node.wrapper.actualSize.height * node.pivot.y;
        if (width > 0) {
          if (node.processId) {
            const parent: Node = this.diagram.nameTable[node.processId];
            if (
              !parent.maxWidth ||
              node.margin.left + width < parent.maxWidth
            ) {
              node.width = width;
        ...
        if (height > 0) {
          if (node.processId) {
            ...
            if (
              !parent.maxHeight ||
              node.margin.top + height < parent.maxHeight
            ) {
              node.height = height;
            ...
            if (!canUpdate) {
              this.diagram.nodePropertyChange(
                ...
                oldValues as Node,
                {
                  ...
                  margin: {
                    top: node.margin.top + (top - oldtop),
      ...
      const parentNode: NodeModel =
        this.diagram.nameTable[(obj as Node).processId];
      if (parentNode) {
        const parent: Rect = parentNode.wrapper.bounds;
        const child: Rect = (obj as Node).wrapper.bounds;
        const bound: Rect = this.diagram.bpmnModule.getChildrenBound(
          parentNode,
          (obj as Node).id,
          this.diagram
  ...
  private scaleConnector(
    ...
    connector.sourcePoint = transformPointByMatrix(
      matrix,
      connector.sourcePoint
    );
    connector.targetPoint = transformPointByMatrix(
      matrix,
      connector.targetPoint
    );
    if (
    ...
    if (selectionHasConnector(this.diagram, selector)) {
      const clonedSelectedItems: object = cloneObject(
        this.diagram.selectedItems
      );
      const nodeModel: NodeModel = {
        offsetX: (clonedSelectedItems as any).offsetX,
        offsetY: (clonedSelectedItems as any).offsetY,
        height: (clonedSelectedItems as any).height,
        width: (clonedSelectedItems as any).width,
  ...
  private measureSelector(selector: Selector) {
    ...
    for (let i: number = 0; i < objects.length; i++) {
      ...
      objects[parseInt(i.toString(), 10)].sourcePoint = transformPointByMatrix(
        matrix,
        objects[parseInt(i.toString(), 10)].sourcePoint
      );
      objects[parseInt(i.toString(), 10)].targetPoint = transformPointByMatrix(
        matrix,
    ...
    this.diagram.diagramRenderer.renderResizeHandle(
      ...
      this.diagram.scroller.currentZoom,
  ...
  public portDrag(
    obj: NodeModel | ConnectorModel,
    portElement: DiagramElement,
    tx: number,
    ty: number
  ): void {
    let oldValues: Object;
    let changedvalues: Object;
    const port: PointPortModel = this.findTarget(
      portElement,
      obj as IElement
    ) as PointPortModel;
    const bounds: Rect = getBounds(obj.wrapper);
    if (port && canDrag(port, this.diagram)) {
      ...
      if (obj instanceof Node) {
        ...
        this.diagram.connectorPropertyChange(
          obj as Connector,
          oldValues as Connector,
          changedvalues as Connector
        );
      }
      this.diagram.updateDiagramObject(obj);
    }
  ...
  public labelDrag(
    ...
    // eslint-disable-next-line max-len
    const label: ShapeAnnotationModel | PathAnnotationModel = this.findTarget(
      textElement,
      obj as IElement
    ) as ShapeAnnotationModel | PathAnnotationModel;
    const bounds: Rect = cornersPointsBeforeRotation(obj.wrapper);
    const oldValues: Object = this.getAnnotationChanges(
      obj,
      label as ShapeAnnotation | PathAnnotation
    );
    if (label instanceof ShapeAnnotation) {
      label.offset.x += tx / bounds.width;
      label.offset.y += ty / bounds.height;
    } else {
      this.updatePathAnnotationOffset(
        obj as Connector,
        label as PathAnnotation,
        tx,
        ty
      );
    ...
    const changedvalues: Object = this.getAnnotationChanges(
      obj,
      label as ShapeAnnotation | PathAnnotation
    );
    if (obj instanceof Node) {
  ...
  private updatePathAnnotationOffset(
    object: Connector,
    label: PathAnnotation,
    tx: number,
    ty: number,
    newPosition?: PointModel,
    size?: Size
  ): void {
    const textWrapper: DiagramElement = this.diagram.getWrapper(
      object.wrapper,
      label.id
    );
    const offsetX: number = textWrapper.offsetX;
    const offsetY: number = textWrapper.offsetY;
      ...
      case 'Center':
        if (label.horizontalAlignment === 'Center') {
          currentPosition = newPosition
            ? newPosition
            : { x: offsetX + tx, y: offsetY + ty };
        } else if (label.horizontalAlignment === 'Right') {
          currentPosition = newPosition
            ? newPosition
            : {
                x: offsetX + textWrapper.actualSize.width / 2 + tx,
                y: offsetY + ty,
              };
      ...
      case 'Top':
        if (label.horizontalAlignment === 'Center') {
          currentPosition = newPosition
            ...
            : {
                x: offsetX + tx,
                y: offsetY - textWrapper.actualSize.height / 2 + ty,
              };
        } else if (label.horizontalAlignment === 'Right') {
          currentPosition = newPosition
            ? newPosition
    ...
    const intersetingPts: PointModel[] = this.getInterceptWithSegment(
      currentPosition,
      intermediatePoints
    );
    let newOffset: PointModel =
    ...
    if (intersetingPts.length > 0) {
      ...
      if (
        label.dragLimit.top ||
        label.dragLimit.bottom ||
        label.dragLimit.left ||
        label.dragLimit.right
      ) {
        const minDistance: Distance = { minDistance: null };
        newOffset = this.getRelativeOffset(
          currentPosition,
          intermediatePoints,
          minDistance
        );
        const distance: Distance = { minDistance: null };
        intersectingOffset = this.getRelativeOffset(
          currentPosition,
          intersetingPts,
          distance
        );
        if (
          minDistance != null &&
          (distance as Distance).minDistance <
            (minDistance as Distance).minDistance
        ) {
          newOffset = intersectingOffset;
        } else {
          const connectorOffset: SegmentInfo = getOffsetOfConnector(
  ...
  private updatePortOffset(
    ...
    const intersectingPoints: PointModel[] = this.getInterceptWithSegment(
      currentPosition,
      intermediatePoints
    );
    let newOffset: PointModel =
  ...
  private getRelativeOffset(
    ...
    for (i = 0; i < points.length; i++) {
      ...
      distance = Math.round(
        Math.sqrt(
          Math.pow(currentPosition.x - pt.x, 2) +
            Math.pow(currentPosition.y - pt.y, 2)
        )
      );
      if (
        (minDistance as Distance).minDistance === null ||
        Math.min(
          Math.abs((minDistance as Distance).minDistance),
          Math.abs(distance)
        ) === Math.abs(distance)
      ) {
        newOffset = pt;
  ...
  private dragLimitValue(
    ...
    point: PointModel,
    tempPt: PointModel,
    contentDimension: Rect
  ): IsDragArea {
    let x: boolean = false;
    let y: boolean = false;
    if (
      tempPt.x >= point.x - label.dragLimit.left - contentDimension.width / 2 &&
      tempPt.x <= point.x + label.dragLimit.right + contentDimension.width / 2
    ) {
      x = true;
    }
    if (
      tempPt.y >= point.y - label.dragLimit.top - contentDimension.height / 2 &&
  ...
  private updateLabelMargin(
    ...
    if (label && offset && offset.x > 0 && offset.x < 1) {
      ...
      const length: number = Point.getLengthFromListOfPoints(
        node.intermediatePoints
      );
      const point: PointModel = this.getPointAtLength(
        length * offset.x,
        node.intermediatePoints,
        0
      );
      ...
      if (
        ...
        const labelBounds: DiagramElement = this.diagram.getWrapper(
          node.wrapper,
          label.id
        );
        const contentDimension: Rect = new Rect(0, 0, 0, 0);
        const annotationWrtapper: DiagramElement = this.diagram.getWrapper(
          node.wrapper,
          label.id
        );
        ...
        contentDimension.width =
          annotationWrtapper.bounds.width / curZoomfactor;
        contentDimension.height =
          annotationWrtapper.bounds.height / curZoomfactor;
        ...
        const cursorLimit: IsDragArea = this.dragLimitValue(
          label,
          point,
          tempPt,
          contentDimension as Rect
        );
        label.margin = {
  ...
  private getInterceptWithSegment(
    currentPosition: PointModel,
    conPoints: PointModel[]
  ): PointModel[] {
    const intercepts: PointModel[] = [];
    let imgLine: PointModel[] = [];
    let segemnt: PointModel[] = [];
    let tarAngle: number;
    let srcAngle: number; //let maxLength: number;
    const maxLength: number = Point.findLength(
      { x: 0, y: 0 },
      {
    ...
    for (let i: number = 1; i < conPoints.length; i++) {
      ...
      const angleAdd: number =
        (srcAngle > 0 && srcAngle <= 90) || (srcAngle > 180 && srcAngle <= 270)
          ? 90
          : -90;
      imgLine.push(
        Point.transform(currentPosition, srcAngle + angleAdd, maxLength)
      );
      imgLine.push(
        Point.transform(currentPosition, tarAngle + angleAdd, maxLength)
      );
      const lineUtil1: Segment = {
        x1: segemnt[0].x,
        y1: segemnt[0].y,
        x2: segemnt[1].x,
        y2: segemnt[1].y,
      };
      const lineUtil2: Segment = {
        x1: imgLine[0].x,
        y1: imgLine[0].y,
        x2: imgLine[1].x,
        y2: imgLine[1].y,
  ...
  /** @private */
  public getAnnotationChanges(
    object: NodeModel | ConnectorModel,
    label: ShapeAnnotation | PathAnnotation
  ): Object {
    ...
    annotations[index] = {
      ...
      alignment:
        object instanceof Connector
          ? (label as PathAnnotation).alignment
          : undefined,
    };
  ...
  /** @private */
  public getConnectorPortChanges(
    object: NodeModel | ConnectorModel,
    label: PathPort
  ): Object {
    ...
    ports[index] = {
      width: label.width,
      height: label.height,
      offset: (label as PathPort).offset,
      margin: {
        left: label.margin.left,
        right: label.margin.right,
        top: label.margin.top,
      ...
      verticalAlignment: label.verticalAlignment,
      alignment:
        object instanceof Connector ? (label as PathPort).alignment : undefined,
    };
    return { ports: ports };
  }

  /** @private */
  public getPortChanges(
    object: NodeModel | ConnectorModel,
    port: PointPort
  ): Object {
  ...
  public labelRotate(
    ...
    const labelWrapper: DiagramElement = this.diagram.getWrapper(
      object.wrapper,
      label.id
    );
    let angle: number =
      findAngle(
        { x: labelWrapper.offsetX, y: labelWrapper.offsetY },
        currentPosition
      ) + 90;
    ...
    if (label instanceof PathAnnotation && label.segmentAngle) {
      const getPointloop: SegmentInfo = getAnnotationPosition(
        (object as Connector).intermediatePoints,
        label as PathAnnotation,
        (object as Connector).wrapper.bounds
      );
      angle -= getPointloop.angle;
    }
    angle = (angle + 360) % 360;
    label.rotateAngle +=
      angle - (label.rotateAngle + labelWrapper.parentTransform);
    if (label instanceof PathAnnotation) {
      label.alignment = 'Center';
    }
  ...
  public labelResize(
    ...
    label: ShapeAnnotation | PathAnnotationModel,
    deltaWidth: number,
    deltaHeight: number,
    pivot: PointModel,
    selector: Selector
    ...
    oldValues = this.getAnnotationChanges(
      node,
      label as ShapeAnnotation | PathAnnotation
    );
    ...
    if (
      ...
      const newMat: Matrix = identityMatrix();
      const matrix: Matrix = identityMatrix();
      rotateMatrix(
        newMat,
        -(node as NodeModel).rotateAngle,
        (node as NodeModel).offsetX,
        (node as NodeModel).offsetY
      );
      rotateAngle =
        (textElement.rotateAngle +
          (node instanceof Node ? (node as NodeModel).rotateAngle : 0) +
          360) %
        360;
      rotateMatrix(matrix, -rotateAngle, pivot.x, pivot.y);
      scaleMatrix(matrix, deltaWidth, deltaHeight, pivot.x, pivot.y);
      rotateMatrix(matrix, rotateAngle, pivot.x, pivot.y);
      let newPosition: PointModel = transformPointByMatrix(matrix, {
      ...
      const shape: ShapeAnnotationModel | PathAnnotationModel = this.findTarget(
        textElement,
        node as IElement
      ) as ShapeAnnotation;
      ...
      if (shape instanceof PathAnnotation) {
        ...
        newPosition.x =
          newPosition.x - textElement.margin.left + textElement.margin.right;
        newPosition.y =
          newPosition.y - textElement.margin.top + textElement.margin.bottom;
        newPosition.y +=
          shape.verticalAlignment === 'Top'
            ? -height / 2
            : shape.verticalAlignment === 'Bottom'
    ...
    changedvalues = this.getAnnotationChanges(
      node,
      label as ShapeAnnotation | PathAnnotation
    );
  ...
  public getSubProcess(source: IElement): SelectorModel {
    ...
    if (source instanceof Node) {
      ...
      source &&
      (source as SelectorModel).nodes &&
      (source as SelectorModel).nodes.length &&
  ...
  /**   @private  */
  public checkBoundaryConstraints(
    tx: number,
    ty: number,
    nodeBounds?: Rect,
    isInitialRendering?: boolean
  ): boolean {
    const pageSettings: PageSettings = this.diagram
      .pageSettings as PageSettings;
    const boundaryConstraints: BoundaryConstraints = (
      this.diagram.pageSettings as PageSettings
    ).boundaryConstraints;
    const scroller: DiagramScroller = this.diagram.scroller;
    if (boundaryConstraints === 'Page' || boundaryConstraints === 'Diagram') {
      const selectorBounds: Rect = !nodeBounds
        ? this.diagram.selectedItems.wrapper.bounds
        : undefined;
      const width: number =
      ...
      const right: number =
        (nodeBounds ? bounds.right : selectorBounds.right) + (tx || 0);
      const left: number =
        (nodeBounds ? bounds.left : selectorBounds.left) + (tx || 0);
      const top: number =
        (nodeBounds ? bounds.top : selectorBounds.top) + (ty || 0);
      const bottom: number =
        (nodeBounds ? bounds.bottom : selectorBounds.bottom) + (ty || 0);
      //Bug 906963: Layout is not rendered when boundaryConstraints set to "Page/Diagram"
      if (
        (right <= width && left >= 0 && bottom <= height && top >= 0) ||
        (this.diagram.layout.type !== 'None' && isInitialRendering)
      ) {
        return true;
      }
      return false;
  ...
  public dragSelectedObjects(tx: number, ty: number): boolean {
    let obj: SelectorModel | NodeModel | ConnectorModel =
      this.diagram.selectedItems;
    ...
    const swimLaneInSelection = this.diagram.selectedItems.nodes.filter(
      (node: Node) => node.shape.type === 'SwimLane'
    );
    if (swimLaneInSelection.length > 0 && !this.diagram.currentSymbol) {
      if (this.diagram.selectedItems.nodes.length > 1) {
        let filterObjects = this.diagram.selectedItems.nodes.filter(
          (node: Node) =>
            (node as Node).parentId === '' ||
            this.diagram.nameTable[node.parentId].shape.type !== 'SwimLane'
        );
        if (filterObjects.length > 0) {
          for (let i: number = 0; i < filterObjects.length; i++) {
            if (
              (this.diagram.nameTable[
                (filterObjects[parseInt(i.toString(), 10)] as Node).parentId
              ] &&
                this.diagram.nameTable[
                  (filterObjects[parseInt(i.toString(), 10)] as Node).parentId
                ].isLane) ||
              filterObjects[parseInt(i.toString(), 10)].shape.type ===
                'SwimLane'
            ) {
          ...
          const uniqueSwimlane: NodeModel[] = swimLaneInSelection.filter(
            (obj: NodeModel, index: number, self: NodeModel[]) =>
          ...
          if (filterObjects.length > 0 || uniqueSwimlane.length > 1) {
            ...
            this.diagram.selectedItems.nodes.splice(
              0,
              this.diagram.selectedItems.nodes.length
            );
    ...
    if (this.checkBoundaryConstraints(tx, ty)) {
      this.diagram.diagramActions =
        this.diagram.diagramActions |
        (DiagramAction.PreventZIndexOnDragging | DiagramAction.DragUsingMouse);
      const actualObject: Node = this.diagram.selectedObject
        .actualObject as Node;
      if (
        (actualObject &&
          actualObject instanceof Node &&
          actualObject.isLane &&
          canLaneInterchange(actualObject, this.diagram)) ||
        !actualObject ||
        !actualObject.isLane
      ) {
  ...
  private checkHtmlObjectDrag(obj: SelectorModel | NodeModel | ConnectorModel) {
    ...
    if (this.diagram.views && this.diagram.views.length > 1) {
      if (obj instanceof Selector) {
        isHtmlObjDragged = obj.nodes.some(
          (node) => node.shape && node.shape.type === 'HTML'
        );
      } else if (
        (obj as NodeModel).shape &&
        (obj as NodeModel).shape.type === 'HTML'
      ) {
    ...
    for (const temp of this.diagram.views) {
      ...
      if (!(view instanceof Diagram)) {
        const rect: HTMLElement = document.getElementById(
          (view as any).canvas.id + 'overviewrect'
        );
  ...
  public expandNode(
    ...
    if (
      this.diagram.diagramActions != DiagramAction.Render &&
      this.diagram.layoutAnimateModule &&
      this.diagram.layout.enableAnimation &&
      this.diagram.organizationalChartModule
    ) {
    ...
    if (!canLayout) {
      if (
        ...
        this.layoutAnimateModule.setIntervalObject.length > 0
      ) {
        this.layoutAnimateModule.stopCurrentAnimation(
          this.objectStore[0],
          diagram,
          node
        );
    ...
    if (
      ...
      this.diagram.allowServerDataBinding = false;
      this.layoutAnimateModule.expand(
        this.diagram.layout.enableAnimation,
        objects,
        node,
        this.diagram
      ...
      if (
        this.diagram.lineRoutingModule &&
        this.diagram.constraints & DiagramConstraints.LineRouting &&
        this.diagram.layout.type !== 'ComplexHierarchicalTree'
      ) {
        this.diagram.resetSegments();
      }
    }
    this.diagram.layout.fixedNode =
      fixedNode === '' ? '' : this.diagram.layout.fixedNode;
    return objects;
  }
  ...
  private getparentexpand(
    ...
    for (let i: number = 0; i < target.inEdges.length; i++) {
      ...
      if (
        ...
        !visibility &&
        previousNode.id !== connector.sourceID &&
        newConnector.visible
      ) {
        return false;
      } else {
        boolean = true;
  ...
  public expandCollapse(
    ...
    for (let i: number = 0; i < source.outEdges.length; i++) {
      ...
      const value: boolean = this.getparentexpand(
        target,
        diagram,
        visibility,
        connector
  ...
  public updateConnectorPoints(obj: Node | Connector, rect?: Rect): void {
    if (obj instanceof Connector && obj.type !== 'Bezier') {
      ...
      this.diagram.connectorPropertyChange(
        obj as Connector,
        {} as Connector,
        { segments: (obj as Connector).segments } as Connector
      );
      this.diagram.updateDiagramObject(obj);
    }
  }
   ...
   * @private
   */
  public updateSelectedNodeProperties(
    object?: NodeModel | ConnectorModel[]
  ): void {
    if (
      this.diagram.lineRoutingModule &&
      ...
      if (helperObject && actualObject) {
        ...
        const rotateAngle: number =
          helperObject.rotateAngle - actualObject.rotateAngle;
        if (
          this.diagram.selectedItems.nodes.length +
            this.diagram.selectedItems.connectors.length >
          0
        ) {
          this.diagram.selectedItems.wrapper.rotateAngle =
            this.diagram.selectedItems.rotateAngle = helperObject.rotateAngle;
        }
        if (
          actualObject instanceof Node &&
          actualObject.shape.type !== 'SwimLane' &&
          !actualObject.isLane &&
          !actualObject.isPhase &&
          !actualObject.isHeader
        ) {
          if (
            actualObject.offsetX !== actualObject.wrapper.offsetX ||
            actualObject.offsetY !== actualObject.wrapper.offsetY ||
            actualObject.width !== actualObject.wrapper.width ||
          ...
          objects = this.diagram.spatialSearch.findObjects(
          ...
          for (let i: number = 0; i < actualObject.nodes.length; i++) {
            ...
            if (
              ...
              objects = objects.concat(
                this.diagram.spatialSearch.findObjects(
                  actualObject.wrapper.outerBounds as Rect
                )
              );
            }
      ...
      for (let i: number = 0; i < objects.length; i++) {
        if (
          objects[i] instanceof Connector &&
          connectors.indexOf(objects[i].id) === -1
        ) {
      ...
      for (let i: number = 0; i < connectors.length; i++) {
        ...
        if (connector instanceof Connector && connector.type === 'Orthogonal') {
          ...
          if (
            ...
            let sourceNode: NodeModel = this.diagram.getObject(
              connector.sourceID
            );
            let targetNode: NodeModel = this.diagram.getObject(
              connector.targetID
            );
            let Connectorlength1 =
              targetNode.wrapper.outerBounds.middleLeft.x -
              sourceNode.wrapper.outerBounds.middleRight.x;
            let Connectorlength2 =
              targetNode.wrapper.outerBounds.topCenter.y -
              sourceNode.wrapper.outerBounds.bottomCenter.y;
            if (Connectorlength1 > 30 || Connectorlength2 > 30) {
              this.diagram.lineRoutingModule.refreshConnectorSegments(
                this.diagram,
                connector,
                true
              );
  ...
  /** @private */
  public removeChildFromBPmn(
    child: IElement,
    newTarget: IElement,
    oldTarget: IElement
  ): void {
    const obj: Node =
      this.diagram.nameTable[(child as Node).id] ||
      (child as SelectorModel).nodes[0];
    if (oldTarget) {
      if (obj && obj.processId && obj.processId === oldTarget.wrapper.id) {
        ...
        //     }
        // }
        const nodeCollection: string[] =
          (this.diagram.nameTable[obj.processId].shape as BpmnShape).activity
            .subProcess.processes || [];
        nodeCollection.splice(nodeCollection.indexOf(obj.id), 1);
        this.diagram.bpmnModule.removeChildFromBPMN(
          this.diagram.nameTable[obj.processId].wrapper,
          obj.id,
          this.diagram
        );
        ...
        const entry: HistoryEntry = {
          ...
          redoObject: { nodes: [undoElement] },
          undoObject: { nodes: [node] },
          category: 'Internal',
        };
        this.addHistoryEntry(entry);
        this.diagram.endGroupAction();
  ...
  public isDroppable(source: IElement, targetNodes: IElement): boolean {
    const node: Node =
      this.diagram.nameTable[(source as Node).id] ||
      (source as SelectorModel).nodes[0];
    if (node instanceof Node) {
      if (
        !isBlazor() &&
      ...
      if (((targetNodes as Node).shape as BpmnShape).shape === 'Group') {
        (
          (targetNodes as Node).shape as BpmnShape
        ).activity.subProcess.collapsed = false;
      }
      if (node && node.shape.type === 'Bpmn') {
        //905238 - Exception thrown while dropping BPMN shapes over a BPMN connector
        if (
          node.processId === (targetNodes as Node).id ||
          node.id === (targetNodes as Node).processId ||
          ((targetNodes as Node).shape.type === 'Bpmn' &&
            !(targetNodes instanceof Connector) &&
            ((targetNodes as Node).shape as BpmnShape).activity &&
            ((targetNodes as Node).shape as BpmnShape).activity.subProcess &&
            ((targetNodes as Node).shape as BpmnShape).activity.subProcess
              .collapsed)
        ) {
          return false;
   ...
   * @private
   */
  public renderHighlighter(
    args: MouseEventArgs,
    connectHighlighter?: boolean,
    source?: boolean
  ): void {
    const bounds: Rect = new Rect();
    if (
      args.target instanceof Node ||
      args.target instanceof Connector ||
      (connectHighlighter &&
        (args.source instanceof Node || args.source instanceof Connector))
    ) {
      const tgt: IElement = connectHighlighter ? args.source : args.target;
      const tgtWrap: DiagramElement = connectHighlighter
        ? args.sourceWrapper
        : args.targetWrapper;
      const target: NodeModel | PointPortModel = this.findTarget(
        tgtWrap,
    ...
    if (source instanceof Node || source instanceof Selector) {
      const node: Node =
        source instanceof Node
          ? source
          : ((source as Selector).nodes[0] as Node);
      if (
        this.diagram.bpmnModule &&
        (target as Node).shape.type === 'Bpmn' &&
        !isBlazor() &&
        (node.shape as BpmnShape).shape === 'TextAnnotation'
        //  ||(isBlazor() && (node.shape as DiagramShape).bpmnShape === 'TextAnnotation')
      ) {
        ...
        if (!this.diagram.currentSymbol) {
          this.diagram.addTextAnnotation(
            (node.shape as BpmnShape).annotation,
            target
          );
          ((node.shape as BpmnShape).annotation as BpmnAnnotation).nodeId = '';
  ...
  public distribute(
    ...
    if (objects.length > 0) {
      ...
      for (i = 1; i < objects.length; i++) {
        right =
          right +
          (objects[i] as Node).wrapper.bounds.topRight.x -
          (objects[i - 1] as Node).wrapper.bounds.topRight.x;
        left =
          left +
          (objects[i] as Node).wrapper.bounds.topLeft.x -
          (objects[i - 1] as Node).wrapper.bounds.topLeft.x;
        top =
          top +
          (objects[i] as Node).wrapper.bounds.topRight.y -
          (objects[i - 1] as Node).wrapper.bounds.topRight.y;
        bottom =
          bottom +
          (objects[i] as Node).wrapper.bounds.bottomRight.y -
          (objects[i - 1] as Node).wrapper.bounds.bottomRight.y;
        center =
          center +
          (objects[i] as Node).wrapper.bounds.center.x -
          (objects[i - 1] as Node).wrapper.bounds.center.x;
        middle =
          middle +
          (objects[i] as Node).wrapper.bounds.center.y -
          (objects[i - 1] as Node).wrapper.bounds.center.y;
        btt =
          btt +
          (objects[i] as Node).wrapper.bounds.topRight.y -
          (objects[i - 1] as Node).wrapper.bounds.bottomRight.y;
      ...
      for (i = 1; i < objects.length - 1; i++) {
        ...
        if (option === 'RightToLeft' || option === 'Center') {
          ...
          tx =
            prev.topRight.x - current.topRight.x + right / (objects.length - 1);
        } else if (option === 'Left') {
          ...
          ty =
            prev.topRight.y - current.topRight.y + top / (objects.length - 1);
        } else if (option === 'Bottom') {
          ty =
            prev.bottomRight.y -
            current.bottomRight.y +
            bottom / (objects.length - 1);
        } else if (option === 'BottomToTop') {
          ty =
            prev.bottomRight.y -
            current.topRight.y +
            btt / (objects.length - 1);
        }
  ...
  public sameSize(
    ...
    if (objects.length > 0) {
      ...
      for (i = 1; i < objects.length; i++) {
        objects[parseInt(i.toString(), 10)] =
          this.diagram.nameTable[objects[parseInt(i.toString(), 10)].id] ||
          objects[0];
        const rect: Rect = getBounds(
          objects[parseInt(i.toString(), 10)].wrapper
        );
        let sw: number = 1;
        let sh: number = 1;
        if (option === 'Width') {
  ...
  public updatePanState(eventCheck: boolean): any {
    if (eventCheck) {
      this.diagram.realActions =
        this.diagram.realActions | RealAction.PanInProgress;
    } else {
  ...
  public scroll(
    ...
    this.diagram.pan(
      ...
      (scrollY = pany ? scrollY : 0) * this.diagram.scroller.currentZoom,
      focusPoint,
      canPan
    );
  }
   ...
   * @private
   */
  public renderContainerHelper(
    node: NodeModel | SelectorModel | ConnectorModel
  ): NodeModel | ConnectorModel {
  ...
  public isTargetSubProcess(node: NodeModel): boolean {
    if (
      ...
      node.shape.type === 'Bpmn' &&
      (node.shape as BpmnShape).activity &&
      (node.shape as BpmnShape).activity.activity === 'SubProcess'
    ) {
      return true;
    }
    return false;
  ...
  public dropChildToContainer(parent: NodeModel, node: NodeModel): void {
    if (
      !(this.diagram.diagramActions & DiagramAction.PreventLaneContainerUpdate)
    ) {
      ...
      if ((node as any).hasTextAnnotation) {
        for (let i = 0; i < (node as Node).outEdges.length; i++) {
          const con =
            this.diagram.nameTable[
              (node as Node).outEdges[parseInt(i.toString(), 10)]
            ];
  ...
  public updateLaneChildrenZindex(node: Node, target: IElement): void {
    const lowerIndexobject: Node = this.findLeastIndexObject(
      node,
      target
    ) as Node;
      ...
      for (let i = endIndex; i >= startIndex; i--) {
        if (startIndex !== i) {
          if (!layerZIndexTable[i - 1]) {
            layerZIndexTable[i - 1] =
              layerZIndexTable[parseInt(i.toString(), 10)];
          ...
          const tempIndex: number = this.swapZIndexObjects(
            endIndex,
            layerZIndexTable,
            swimlane.id,
            tempTable
  ...
  private findLeastIndexObject(node: Node, target: IElement): Node | Connector {
    ...
    if (node instanceof Node) {
      lowerIndexobject = this.findLeastIndexConnector(
        node.inEdges,
        target,
        lowerIndexobject
      ) as Node;
  ...
  public checkSelection(selector: SelectorModel, corner: string): void {
    ...
    if (selector.nodes.length === 1 && selector.connectors.length === 0) {
      if (checkParentAsContainer(this.diagram, selector.nodes[0], true)) {
        node =
          selector.nodes[0].shape === 'SwimLane'
            ? selector.nodes[0]
        ...
        if (node.shape.type === 'SwimLane') {
          ...
          if (
            (child.isPhase &&
              ((orientation && corner === 'ResizeSouth') ||
                (!orientation && corner === 'ResizeEast'))) ||
            (child.isLane &&
              ((orientation && corner === 'ResizeEast') ||
                (!orientation && corner === 'ResizeSouth')))
          ) {
          ...
          if (
            (node.container.orientation === 'Horizontal' &&
              child.rowIndex === 1) ||
            (node.container.orientation === 'Vertical' &&
              child.rowIndex > 0 &&
              child.columnIndex > 0)
          ) {
            if (corner === 'ResizeSouth') {
              for (let i: number = 0; i < this.diagram.nodes.length; i++) {
                const obj: NodeModel =
                  this.diagram.nodes[parseInt(i.toString(), 10)];
                if (
                  obj.rowIndex === node.rows.length - 1 &&
                  obj.columnIndex === 0
                ) {
                  this.select(obj);
                  break;
            ...
            if (corner === 'ResizeEast') {
              for (let i: number = 0; i < this.diagram.nodes.length; i++) {
                ...
                if (
                  obj.rowIndex === 1 &&
                  obj.columnIndex === node.columns.length - 1
                ) {
                  this.select(obj);
                  break;
                }
  ...
  public zoom(
    ...
    this.diagram.scroller.zoom(
      scale,
      scrollX * this.diagram.scroller.currentZoom,
      scrollY * this.diagram.scroller.currentZoom,
      focusPoint
    );
  }
}
