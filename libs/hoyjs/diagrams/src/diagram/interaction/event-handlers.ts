...
import { Browser } from '@hoyui/base';
...
import { InputArgs } from '@hoyui/inputs';
...
import { isBlazor } from '@hoyui/base';
...
export class DiagramEventHandler {
  ...
  private set action(action: Actions) {
    if (action !== this.currentAction) {
      ...
      if (action === 'Rotate' || action === 'LabelRotate') {
        ...
        this.currentAction === 'LabelRotate'
      ) {
      ...
      if (
        ...
        (this.currentPosition &&
          this.commandHandler.isUserHandle(this.currentPosition))
      ) {
        this.diagram.diagramActions =
          this.diagram.diagramActions | DiagramAction.ToolAction;
      } else {
        this.diagram.diagramActions =
          this.diagram.diagramActions & ~DiagramAction.ToolAction;
      }
  ...
  private set blocked(blocked: boolean) {
    ...
    if (this.blocked) {
      ...
      this.diagram.setCursor(
        this.diagram.getCursor(this.action, this.inAction)
      );
    }
  }
  ...
  public getMousePosition(
    ...
    const boundingRect: ClientRect =
      this.diagram.element.getBoundingClientRect();
    offsetX =
      offsetX +
  ...
  public updateViewPortSize(element: HTMLElement): void {
    ...
    if (container) {
      ...
      let position: Size = new Size();
      position = getRulerSize(this.diagram);
      const width: string = this.diagram.getSizeValue(
        this.diagram.width,
        position.width
      );
      const height: string = this.diagram.getSizeValue(
        this.diagram.height,
        position.height
      );
      ...
      if (this.diagram.views.length > 1) {
        ...
        for (const temp of this.diagram.views) {
          ...
          if (view instanceof Overview) {
            ...
            (view as Overview).onPropertyChanged(
              { sourceID: (view as Overview).sourceID },
              {}
            );
          }
        }
  ...
  public canHideResizers(): boolean {
    return (
      (this.tool instanceof MoveTool || this.tool instanceof RotateTool) &&
      this.isMouseDown
    );
  }
  ...
  private updateCursor(): void {
    if (
      ...
      this.diagram.selectedItems.connectors.length === 1
    ) {
      ...
      this.blocked =
        this.eventArgs &&
        this.eventArgs.source &&
        !canMove(this.eventArgs.source)
          ? false
          : this.isMouseDown &&
            list.length === 1 &&
            this.tool instanceof SelectTool &&
            !canMove(list[0]);
    }
  }
  private isForeignObject(
    ...
    if (foreignobject) {
      while (foreignobject.parentNode !== null) {
        if (
          typeof foreignobject.className === 'string' &&
          ((!isTextBox &&
            foreignobject.className.indexOf('foreign-object') !== -1) ||
            (isTextBox &&
              foreignobject.className.indexOf('e-diagram-text-edit') !== -1))
        ) {
  ...
  private renderUmlHighLighter(args: MouseEventArgs): void {
    ...
    if (
      ...
      if (!bound.containsPoint(this.currentPosition)) {
        ...
        const target: IElement = this.diagram.findObjectUnderMouse(
          objects,
          this.action,
          this.inAction
        );
        if (
          ...
          const isVertical: boolean =
            this.diagram.nameTable[(target as Node).parentId].container
  ...
  private checkPreviousAction(): void {
    if (
      ...
      this.diagram.selectedItems.userHandles.length
    ) {
      for (
        let i: number = 0;
        i < this.diagram.selectedItems.userHandles.length;
        i++
      ) {
        if (
          this.previousAction &&
          this.diagram.selectedItems.userHandles[parseInt(i.toString(), 10)]
        ) {
    ...
    if (this.action !== this.previousAction) {
      ...
      this.checkFixedUserHandleEvent(
        DiagramEvent.onFixedUserHandleMouseLeave,
        this.targetItem,
        this.previousTarget
      );
  ...
  private checkUserHandleEvent(eventName: DiagramEvent): void {
    if (
      ...
      this.diagram.selectedItems.userHandles.length > 0
    ) {
      const currentAction: Actions =
        eventName === DiagramEvent.onUserHandleMouseLeave
          ? this.previousAction
          : this.action;
      ...
      for (
        ...
        if (
          currentAction ===
            this.diagram.selectedItems.userHandles[parseInt(i.toString(), 10)]
              .name ||
          (eventName === DiagramEvent.onUserHandleMouseUp &&
  ...
  private userHandle(
    ...
    i: number,
    arg: UserHandleEventsArgs,
    targetItem: NodeModel | ConnectorModel
  ): void {
    if (
      eventName === DiagramEvent.onUserHandleMouseEnter ||
      eventName === DiagramEvent.onFixedUserHandleMouseEnter
    ) {
      ...
      if (
        arg.element.tooltip &&
        arg.element.tooltip.openOn === 'Auto' &&
        arg.element.tooltip.content !== ''
      ) {
        ...
        if (arg.element.pathData) {
          if (eventName === DiagramEvent.onUserHandleMouseEnter) {
            ...
            targetEle = document.getElementById(
              targetItem.id +
                '_' +
                targetItem.fixedUserHandles[parseInt(i.toString(), 10)].id +
                '_groupElement'
            );
          }
          ...
          targetEle = document.getElementById(
            this.diagram.selectedItems.userHandles[parseInt(i.toString(), 10)]
              .name + '_shape_native_element'
          );
        } else {
          targetEle = document.getElementById(
            this.diagram.selectedItems.userHandles[parseInt(i.toString(), 10)]
              .name + '_shape_html_element'
          );
        }
        //892828: Flickering of tooltip while hovering userhandle
        if (
          arg.element.tooltip.openOn === 'Auto' &&
          arg.element !== this.isUserHandleHover
        ) {
    ...
    if (eventName === DiagramEvent.onUserHandleMouseDown) {
      this.userHandleObject =
        this.diagram.selectedItems.userHandles[parseInt(i.toString(), 10)].name;
      this.diagram.triggerEvent(eventName, arg);
    ...
    if (eventName === DiagramEvent.onUserHandleMouseUp) {
      ...
      if (
        this.commandHandler.isUserHandle(this.currentPosition) &&
        element &&
        element.id === this.userHandleObject + '_userhandle'
      ) {
    ...
    if (
      eventName === DiagramEvent.onUserHandleMouseLeave ||
      eventName === DiagramEvent.onFixedUserHandleMouseLeave
    ) {
      if (
        this.diagram.tooltipObject &&
        (this.diagram.tooltipObject as DiagramTooltipModel).openOn !== 'Custom'
      ) {
  ...
  public mouseDown(evt: PointerEvent): void {
    ...
    if (
      this.inAction === true &&
      (this.tool instanceof NodeDrawingTool ||
        this.tool instanceof ConnectorDrawingTool)
    ) {
      return;
    }
    this.focus = true;
    ...
    const isSymblDragging: boolean = document.getElementsByClassName(
      'e-dragclone'
    )[0]
      ? true
      : false;
    ...
    if (
      ...
      if (
        ...
        const arg: IClickEventArgs = {
          ...
          button:
            evt.button === 0 ? 'Left' : evt.button === 1 ? 'Middle' : 'Right',
        };
        this.inAction = false;
        ...
        if (this.diagram.textEditing && !this.isMouseOnScrollBar(evt)) {
          ...
          if (
            this.diagram.mode === 'Canvas' &&
            this.diagram.scroller.currentZoom !== 1
          ) {
            this.diagram.refreshDiagramLayer();
          }
        }
        const targetObject: any = this.getTargetElement();
        this.action = this.diagram.findActionToBeDone(
          targetObject.obj,
          targetObject.sourceElement,
          this.currentPosition,
          targetObject.target
        );
        ...
        if (
          ctrlKey &&
          evt.shiftKey &&
          this.diagram.connectorEditingToolModule
        ) {
          ...
          (ctrlKey || evt.shiftKey) &&
          (canSingleSelect(this.diagram) || canMultiSelect(this.diagram))
        ) {
          this.action = 'Select';
        }
        ...
        if (ctrlKey || evt.shiftKey) {
          const info: Info =
            ctrlKey && evt.shiftKey
              ? { ctrlKey: ctrlKey, shiftKey: evt.shiftKey }
              : { ctrlKey: true };
        ...
        if (
          ...
          prevSelectedNode &&
          this.diagram.selectedItems.nodes &&
          this.tool instanceof MoveTool
        ) {
          for (let i: number = 0; i < prevSelectedNode.length; i++) {
            if (
              prevSelectedNode[parseInt(i.toString(), 10)].id !==
        ...
        if (evt.type === 'touchstart') {
          if (touches && touches.length >= 2) {
            this.touchStartList = addTouchPointer(
              <ITouches[]>this.touchStartList,
              evt,
              touches
            );
          }
    ...
    if (
      !this.isForeignObject(evt.target as HTMLElement) &&
      !this.isForeignObject(evt.target as HTMLElement, true) &&
      !touches
    ) {
      evt.preventDefault();
    }
  ...
  public mouseMoveExtend(e: PointerEvent | TouchEvent, obj: IElement): void {
    ...
    const isNode: boolean =
      (this.eventArgs.target instanceof Node ||
        this.eventArgs.target instanceof Connector) &&
    ...
    if (this.tool instanceof ConnectTool) {
      this.diagram.updatePortVisibility(
        this.hoverElement instanceof Node ||
          ...
          ? this.hoverElement
          : (this.hoverNode as Node | Connector),
        PortVisibility.Connect | PortVisibility.Hover,
        isNode
      );
    }
    if (
      this.hoverElement instanceof Node &&
      this.hoverNode instanceof Node &&
      this.hoverNode &&
      this.hoverNode.id !== this.hoverElement.id
    ) {
      this.diagram.updatePortVisibility(
        this.hoverNode,
        PortVisibility.Connect | PortVisibility.Hover,
    ...
    if (
      this.hoverElement instanceof Connector &&
      this.hoverElement.ports.length > 0
    ) {
      this.diagram.updatePortVisibility(
        this.hoverElement,
        PortVisibility.Connect | PortVisibility.Hover,
    ...
    if (
      this.previousElement instanceof Node &&
      this.hoverElement &&
      this.previousElement.id !== this.hoverElement.id
    ) {
      this.diagram.updatePortVisibility(
  ...
  public checkAction(obj: IElement): void {
    if (
      this.action === 'LabelSelect' &&
      this.eventArgs.sourceWrapper &&
      (this.eventArgs.sourceWrapper instanceof TextElement ||
        this.eventArgs.sourceWrapper instanceof DiagramHtmlElement)
    ) {
      const annotation: ShapeAnnotation | PathAnnotation =
        this.commandHandler.findTarget(
          this.eventArgs.sourceWrapper,
          this.eventArgs.source
        ) as ShapeAnnotation | PathAnnotation;
      if (
        !isSelected(
          this.diagram,
          annotation,
      ...
      canSelect(obj) &&
      this.initialEventArgs &&
      this.initialEventArgs.source &&
      this.action === 'Select'
    ) {
      if (
        !isSelected(this.diagram, this.eventArgs.source, false) &&
  ...
  public mouseMove(e: PointerEvent | TouchEvent, touches: TouchList): void {
    ...
    if (
      ...
      const obj: IElement = this.diagram.findObjectUnderMouse(
        objects,
        this.action,
        this.inAction
      );
      ...
      if (e.type === 'touchmove') {
        ...
        if (touches && touches.length > 1) {
          this.touchMoveList = addTouchPointer(
            <ITouches[]>this.touchMoveList,
            <PointerEvent>e,
            touches
          );
      ...
      if (
        Point.equals(this.currentPosition, this.prevPosition) === false ||
        this.inAction
      ) {
        if (this.isMouseDown === false || force) {
          ...
          if (obj !== null) {
            sourceElement = this.diagram.findElementUnderMouse(
              obj,
              this.currentPosition,
              this.diagram
            );
            ...
            if (sourceElement) {
              tooltipTarget = this.commandHandler.findTarget(
                sourceElement,
                obj
              );
            }
            ...
            if (
              tooltipTarget instanceof ShapeAnnotation ||
              tooltipTarget instanceof PathAnnotation
            ) {
              if (
                !(tooltipTarget.tooltip && tooltipTarget.tooltip.content !== '')
              ) {
                tooltipTarget = obj;
              }
            }
            if (tooltipTarget !== this.hoverElement) {
              ...
              if (
                this.hoverElement &&
                this.hoverElement.tooltip.openOn === 'Auto' &&
                content !== ''
              ) {
                this.elementLeave();
              }
              ...
              if (
                !this.isSwimlaneElements(obj as Node) &&
                this.hoverElement &&
                this.isSwimlaneElements(this.hoverElement as Node)
              ) {
              ...
              let portElement: DiagramElement = null;
              let portTarget;
              portElement = this.diagram.findElementUnderMouse(
                obj,
                this.currentPosition,
                this.diagram
              );
              if (portElement instanceof PathElement) {
                ...
                if (
                  (portTarget instanceof PointPort ||
                    portTarget instanceof PathPort) &&
                  (this.hoverElement as PointPort).constraints &
              ...
              if (portElement instanceof TextElement) {
                annotationTarget = this.commandHandler.findTarget(
                  portElement,
                  obj
                );
                if (
                  (annotationTarget instanceof ShapeAnnotation ||
                    annotationTarget instanceof PathAnnotation) &&
                  (annotationTarget as ShapeAnnotation | PathAnnotation)
            ...
            // Updating the tooltip position based on Mouse move
              ...
              if (
                this.hoverElement === tooltipTarget &&
                this.hoverElement.tooltip.content &&
                this.diagram.tooltipObject !== undefined &&
                this.hoverElement.tooltip.relativeMode === 'Mouse'
              ) {
          ...
          this.checkFixedUserHandleEvent(
            DiagramEvent.onFixedUserHandleMouseEnter,
            obj,
            sourceElement
          );
          ...
          if (
            !(
              this.hoverElement &&
              !(this.tool instanceof ZoomPanTool) &&
              obj instanceof Node &&
              this.isSwimlaneElements(obj) &&
              (this.diagram.selectedItems.nodes.length === 0 ||
                !isSelected(this.diagram, this.hoverElement))
            ) &&
            !(this.hoverElement instanceof Connector)
          ) {
            isNode = true;
          ...
          if (
            !(
              ...
              !(this.tool instanceof ZoomPanTool) &&
              obj instanceof Connector &&
              (this.diagram.selectedItems.connectors.length === 0 ||
                !isSelected(this.diagram, this.hoverElement))
            ) &&
            !(this.hoverElement instanceof Node)
          ) {
          ...
          if (this.action === 'Drag' || this.action === 'Rotate') {
            this.diagram.diagramActions =
              this.diagram.diagramActions | DiagramAction.Interactions;
          }
          ...
          if (
            obj &&
            (preDragBounds.width !== postDragBounds.width ||
  ...
  private getContent(): string | HTMLElement {
    const isPrivateTooltip: number =
      (this.hoverElement instanceof Node &&
        (this.hoverElement as Node).constraints & NodeConstraints.Tooltip) ||
      (this.hoverElement instanceof Connector &&
        (this.hoverElement as Connector).constraints &
          ConnectorConstraints.Tooltip) ||
      ((this.hoverElement instanceof PointPort ||
        this.hoverElement instanceof PathPort) &&
        (this.hoverElement as PointPort).constraints &
          PortConstraints.ToolTip) ||
      ((this.hoverElement instanceof ShapeAnnotation ||
        this.hoverElement instanceof PathAnnotation) &&
        (this.hoverElement as ShapeAnnotation | PathAnnotation).constraints &
          AnnotationConstraints.Tooltip);
    ...
    let content: string | HTMLElement = isPrivateTooltip
      ? this.hoverElement.tooltip.content
  ...
  private findIntersectChild(node: NodeModel): NodeModel | ConnectorModel {
    ...
    for (let i: number = 0; i < node.children.length; i++) {
      childNode = this.diagram.getObject(
        node.children[parseInt(i.toString(), 10)]
      );
    ...
    if (!autoScrollPosition && this.doingAutoScroll) {
      ...
      if (
        (this.tool instanceof NodeDrawingTool ||
          this.tool instanceof ConnectorDrawingTool ||
          this.tool instanceof MoveTool ||
          this.tool instanceof ResizeTool ||
          this.tool instanceof SelectTool ||
          this.tool instanceof ConnectTool) &&
        this.inAction
      ) {
        ...
        if (
          this.diagram.scrollSettings.canAutoScroll &&
          autoScrollPosition &&
          !this.doingAutoScroll
        ) {
          this.doingAutoScroll = true;
          this.timeOutValue = setInterval((args: PointerEvent | TouchEvent) => {
  ...
  public mouseUp(evt: PointerEvent): void {
    ...
    if (
      this.eventArgs &&
      this.eventArgs.target &&
      this.eventArgs.target !== this.hoverNode &&
    ...
    this.checkFixedUserHandleEvent(
      DiagramEvent.onFixedUserHandleMouseUp,
      this.targetItem,
      this.previousTarget
    );
    ...
    const prevSelectedConnectors: ConnectorModel[] =
      this.diagram.selectedItems.connectors;
    ...
    if (
      ...
      if (
        ...
        if (!this.inAction && evt.which !== 3) {
          if (this.action === 'Drag') {
            this.action = 'Select';
            const oldSelectedValue = this.diagram.selectedItems.nodes.concat(
            ...
            if (
              ...
              this.diagram.selectedItems.nodes.length +
                ...
                1 &&
              evt.which === 1 &&
              !canPreventClearSelection(this.diagram.diagramActions)
            ) {
              isMultipleSelect = false;
              this.commandHandler.clearSelection();
            }
            if (!isSelected(this.diagram, obj) || !isMultipleSelect) {
              this.commandHandler.selectObjects(
                [obj],
                undefined,
                oldSelectedValue
              );
        ...
        if (
          this.tool &&
          (this.tool.prevPosition || this.tool instanceof LabelTool)
        ) {
          this.eventArgs.position = this.currentPosition;
          const padding: number = this.getConnectorPadding(this.eventArgs);
          this.getMouseEventArgs(
          ...
          if (
            this.diagram.selectedObject.helperObject &&
            (this.tool instanceof MoveTool || this.tool instanceof ResizeTool)
          ) {
            if (
              this.diagram.selectedObject.actualObject &&
            ...
            avoidDropChildren =
              ...
              this.diagram.nameTable['helper'] &&
              this.eventArgs.target &&
              (this.eventArgs.target as Node).isLane &&
              ((this.eventArgs.source instanceof Selector &&
                (this.eventArgs.source as Selector).nodes.length > 0 &&
                ((this.eventArgs.source as Selector).nodes[0] as Node)
                  .parentId === '') ||
                (this.eventArgs.source as Node).parentId === '');
            ...
            if (this.diagram.selectedItems.canToggleSelection) {
                  ...
                  if (!swimlane) {
                    if (
                      !(
                        ...
                        ).parentObj instanceof Lane ||
                        (
                          this.diagram.selectedItems.nodes[
                            parseInt(i.toString(), 10)
                          ] as any
                        ).parentObj instanceof Phase
                      )
                    ) {
                      isGroupObj =
                        (
              ...
              const currSelectedNodes: NodeModel[] =
                this.diagram.selectedItems.nodes;
              for (let i: number = 0; i < prevSelectedNodes.length; i++) {
                if (
                  (prevSelectedNodes.length > 1 &&
                    currSelectedNodes[0].id ===
                      prevSelectedNodes[parseInt(i.toString(), 10)].id) ||
            ...
            if (
              ...
              if (
                ...
                this.diagram.contextMenuModule.contextMenu.open(
        ...
        if (this.eventArgs.clickCount !== 2) {
          this.commandHandler.updateSelectedNodeProperties(
            this.eventArgs.source
          );
          if (avoidDropChildren) {
            this.diagram.diagramActions =
            ...
            const nodes: NodeModel[] =
              this.eventArgs.source instanceof Selector
                ? (this.eventArgs.source as Selector).nodes
                : [this.eventArgs.source as Node];
            if (nodes) {
              for (let i = 0; i < nodes.length; i++) {
                if (!nodes[parseInt(i.toString(), 10)].container) {
                  this.commandHandler.dropChildToContainer(
                    this.eventArgs.target,
                    nodes[parseInt(i.toString(), 10)]
                  );
                  this.commandHandler.renderContainerHelper(
                    nodes[parseInt(i.toString(), 10)]
                  );
                }
              }
        ...
        if (
          ...
          const isInsideLane = this.isSwimlaneChild(
            this.diagram.selectedObject.actualObject
          );
          ...
          const isSwimlaneChildHelper =
            this.diagram.selectedObject.actualObject &&
            this.diagram.selectedObject.actualObject.width ===
              this.diagram.selectedObject.helperObject.width &&
            this.diagram.selectedObject.actualObject.height ===
              this.diagram.selectedObject.helperObject.height;
          this.diagram.remove(this.diagram.selectedObject.helperObject);
          if (
            this.commandHandler.isTargetSubProcess(
              this.diagram.selectedObject.actualObject
            ) &&
            (this.diagram.selectedObject.actualObject as Node).parentId === ''
          ) {
            this.swapProcessChildInDom(
              this.diagram.element.id + '_diagramLayer',
              this.diagram.selectedObject.actualObject
            );
          }
          this.diagram.selectedObject = {
          ...
          // eslint-disable-next-line
          if (
            (this.diagram.diagramActions & DiagramAction.PublicMethod &&
              this.diagram.constraints & DiagramConstraints.LineRouting) ||
            (isInsideLane && isSwimlaneChildHelper)
          ) {
            this.diagram.diagramActions =
              this.diagram.diagramActions & ~DiagramAction.PublicMethod;
          }
      ...
      if (
        !this.inAction &&
        selector.wrapper &&
        selector.userHandles.length > 0 &&
        !disbleRenderSelector
      ) {
      ...
      if (!this.inAction && !this.diagram.currentSymbol && this.eventArgs) {
        ...
        const arg: IClickEventArgs | IBlazorClickEventArgs = {
          element:
            targetObject.target instanceof PointPort
              ? targetObject.target
              : cloneBlazorObject(this.eventArgs.source) ||
                cloneBlazorObject(this.diagram),
  ...
  private isSwimlaneChild(node: NodeModel): boolean {
    if (node.shape && node.shape.type !== 'SwimLane') {
      ...
      const swimlane = parent.parentId
        ? this.diagram.nameTable[parent.parentId]
        : parent;
  ...
  private swapProcessChildInDom(diagramLayerId: string, node: NodeModel): void {
    ...
    if ((node.shape as BpmnShape).activity.subProcess.processes) {
      for (
        let i: number = 0;
        i < (node.shape as BpmnShape).activity.subProcess.processes.length;
        i++
      ) {
  ...
  private getConnectorPadding(eventArgs: MouseEventArgs): number {
    ...
    if (
      targetObject &&
      targetObject instanceof Selector &&
      targetObject.connectors.length
    ) {
      const selectedConnector: ConnectorModel = targetObject.connectors[0];
      padding =
        selectedConnector.constraints & ConnectorConstraints.ConnectToNearByPort
          ? selectedConnector.connectionPadding
          : 0;
    } else if (
      targetObject &&
      targetObject instanceof Connector &&
  ...
  public addSwimLaneObject(selectedNode: NodeModel): void {
    let swimlaneNode: NodeModel;
    let targetNode: NodeModel;
    let shape: SwimLaneModel;
    let value: number;
    ...
    if (!targetNode) {
      targetNode = this.diagram.findObjectUnderMouse(
        objects,
        this.action,
        this.inAction
      );
    ...
    if (
      targetNode &&
      !(
        (targetNode as Node).isLane ||
        (targetNode as Node).isPhase ||
        (targetNode as Node).isHeader
      )
    ) {
    ...
    if (
      targetNode &&
      (actualShape.isPhase ||
        (actualShape.isLane && (targetNode as SwimLaneModel).isLane))
    ) {
    ...
    if (canInsert && (targetNode as Node)) {
      ...
      if (actualShape.isPhase) {
        if (shape.orientation === 'Horizontal') {
          ...
          offset =
            this.currentPosition.y -
            (swimlaneNode.wrapper.bounds.y + shape.header.height);
        }
        const phases: PhaseModel = {
          ...
          header: {
            annotation: {
              content:
                actualShape.phases[0].header === undefined
                  ? 'Phase'
                  : actualShape.phases[0].header.annotation.content,
              style:
                actualShape.phases[0].header === undefined
                  ? {}
          ...
          style:
            actualShape.phases[0] === undefined
              ? {}
              : actualShape.phases[0].style,
        } as PhaseModel;
        ...
        orientation
          ? (lane.height = actualShape.lanes[0].height)
          : (lane.width = actualShape.lanes[0].width);
        if (shape.orientation === 'Horizontal') {
          ...
          value = (targetNode as Node).columnIndex
            ? (targetNode as Node).columnIndex
        ...
        if (shape.lanes.length > value) {
          lane.header.width =
            shape.lanes[parseInt(value.toString(), 10)].header.width;
          lane.header.height =
            shape.lanes[parseInt(value.toString(), 10)].header.height;
        } else {
          ...
          lane.header.width =
            shape.lanes[parseInt(ind.toString(), 10)].header.width;
          lane.header.height =
            shape.lanes[parseInt(ind.toString(), 10)].header.height;
        }
        ...
        this.diagram.addLanes(
          swimlaneNode,
          [lane],
          shape.orientation === 'Horizontal' ? value - index : value
        );
      }
      ...
      const swimLaneobj: NodeModel = {
        ...
        shape: {
          ...
          header: {
            ...
            annotation: {
              content:
                actualShape.header === undefined
                  ? 'Header'
                  : actualShape.header.annotation.content,
              style:
                actualShape.header === undefined
                  ? {}
                  : actualShape.header.annotation.style,
            },
            height: 50,
            style: actualShape.header
              ? actualShape.header.style
              : actualShape.lanes[0].header.style,
          },
          phases: [
            {
              id: randomId(),
  ...
  public mouseLeave(evt: PointerEvent): void {
    ...
    if (
      ...
      this.diagram.updatePortVisibility(
        this.eventArgs.target as Node,
        PortVisibility.Hover | PortVisibility.Connect,
        true
      );
      this.hoverElement = null;
    }
  ...
  public mouseWheel(evt: WheelEvent): void {
    ...
    if (!arg.cancel) {
      ...
      const up: boolean =
        (evt as any).wheelDelta > 0 || -40 * evt.detail > 0 ? true : false;
      ...
      if (ctrlKey) {
        ...
        if (evt.isTrusted) {
          ...
          if (
            (Math.abs(evt.deltaY) < 100 && Math.abs(evt.deltaX) === -0) ||
            // 878719: Resolve ESLint errors
            // eslint-disable-next-line no-compare-neg-zero
        ...
        this.diagram.scrollActions &= ~ScrollActions.Interaction;
        if (
          horizontalOffset !== this.diagram.scroller.horizontalOffset ||
      ...
      if (this.diagram.textEditing) {
        ...
        if (this.diagram.activeLabel.parentId) {
          const node: NodeModel | ConnectorModel = this.diagram.getObject(
            this.diagram.activeLabel.parentId
          );
          this.diagram.startTextEdit(node, this.diagram.activeLabel.id);
        }
        this.diagram.isTriggerEvent = false;
      }
      this.diagram.blazorActions =
        this.diagram.blazorActions & ~BlazorAction.interaction;
    }
  }
  ...
  public keyDown(evt: KeyboardEvent): void {
    ...
    if (
      (!(this.diagram.diagramActions & DiagramAction.TextEdit) &&
        !this.checkEditBoxAsTarget(evt)) ||
      evt.key === 'Escape' ||
      evt.keyCode === 27
    ) {
      let i: string;
      const inAction: string = 'inAction';
      ...
      if (this.diagram.commandManager && this.diagram.commands) {
        ...
        for (const i of Object.keys(commands)) {
          ...
          if (
            command &&
            (command.gesture.keyModifiers || command.gesture.key)
          ) {
            //Added the split method to split the Number string from the enum while clicking the number keys
            if (
              (keycode === command.gesture.key ||
                (Keys[command.gesture.key] &&
                  key === Keys[command.gesture.key]) ||
                key === Keys[command.gesture.key].split('Number')[1] ||
                this.isDeleteKey(key, i)) &&
              ((!command.gesture.keyModifiers &&
                !evt.altKey &&
                !evt.shiftKey &&
                !ctrlKey) ||
                (command.gesture.keyModifiers &&
                  (ctrlKey || evt.altKey || evt.shiftKey) &&
                  this.altKeyPressed(command.gesture.keyModifiers) &&
                  evt.altKey) ||
                (this.shiftKeyPressed(command.gesture.keyModifiers) &&
                  evt.shiftKey) ||
                //added the comparision condition of keyargs to execute the condition onclicking the Ctrl + Shift key
                (this.ctrlKeyPressed(command.gesture.keyModifiers) &&
                  ctrlKey &&
              ...
              if (
                ...
                if (evt.key === 'Escape') {
                  if (this.checkEditBoxAsTarget(evt)) {
                    document
                      .getElementById(this.diagram.diagramCanvas.id)
                      .focus();
                    ...
                    this.diagram.removeObjectsFromLayer(
                      this.diagram.nameTable[this.diagram.currentSymbol.id]
                    );
                    this.diagram.removeElements(this.diagram.currentSymbol);
                    removeChildNodes(
                      this.diagram.currentSymbol as Node,
                      this.diagram
                    );
                    delete this.diagram.nameTable[
                      this.diagram.currentSymbol.id
                    ];
                    const sourceElement: HTMLElement =
                      this.diagram.droppable[`${source}`];
                    sourceElement.draggable[`${intDestroy}`]();
                    const element: HTMLElement =
                      this.diagram.droppable[`${selectedSymbols}`];
                    element.parentNode.removeChild(element);
                    const diagramActions: DiagramAction =
                      this.diagram.diagramActions;
                    this.diagram.diagramActions = this.diagram.addConstraints(
                      diagramActions,
                      DiagramAction.PreventClearSelection
                    );
                    this.tool.mouseUp(this.eventArgs);
                    this.diagram.diagramRenderer.rendererActions =
                      this.diagram.removeConstraints(
    ...
    // eslint-disable-next-line
    let selectedObject: (NodeModel | ConnectorModel)[] = this.diagram
    ...
    if (document.getElementById(this.diagram.element.id + '_editBox')) {
      args.text = (
        document.getElementById(
          this.diagram.element.id + '_editBox'
  ...
  public keyUp(evt: KeyboardEvent): void {
    ...
    const selectedObject: (NodeModel | ConnectorModel)[] = this.diagram
      .selectedItems.nodes.length
      ? this.diagram.selectedItems.nodes
      : this.diagram.selectedItems.connectors;
    this.getKeyModifier(this.keyArgs, evt);
    ...
    if (!this.isKeyUp && this.isNudgeKey) {
      ...
      execute({
        keyDownEventArgs: KeyboardEvent,
        parameter: (this.commandObj as any).parameter,
        type: 'KEYUP',
      });
  ...
  private startAutoScroll(e: PointerEvent | TouchEvent): string {
    ...
    const autoScrollBorder: MarginModel =
      this.diagram.scrollSettings.autoScrollBorder;
    if (Browser.info.name === 'mozilla') {
      if (this.diagram.scroller.viewPortWidth === 0) {
        const bounds: ClientRect | DOMRect = document
          .getElementById(this.diagram.element.id)
    ...
    if (this.diagram.scrollSettings.canAutoScroll) {
      if (
        position.x +
          this.diagram.scroller.horizontalOffset +
          autoScrollBorder.right +
          rulerSize.width >=
        this.diagram.scroller.viewPortWidth - 18
      ) {
        movingPosition = 'right';
      } else if (
        position.x + this.diagram.scroller.horizontalOffset <
        autoScrollBorder.left
      ) {
        movingPosition = 'left';
      } else if (
        position.y +
          this.diagram.scroller.verticalOffset +
          autoScrollBorder.bottom +
          rulerSize.height >
        this.diagram.scroller.viewPortHeight - 18
      ) {
        movingPosition = 'bottom';
      } else if (
        position.y + this.diagram.scroller.verticalOffset <
        autoScrollBorder.top
      ) {
        movingPosition = 'top';
      }
    }
  ...
  private doAutoScroll(
    ...
    if (
      ...
      if (
        this.tool instanceof ResizeTool &&
        (this.tool.corner === 'ResizeSouthEast' ||
          this.tool.corner === 'ResizeSouthWest' ||
          this.tool.corner === 'ResizeNorthWest' ||
          this.tool.corner === 'ResizeNorthEast')
      ) {
        canUpdate = true;
      ...
      this.eventArgs.target = this.diagram.findObjectUnderMouse(
        objects,
        this.action,
        this.inAction
      );
  ...
  private mouseEvents(): void {
    const target: (NodeModel | ConnectorModel)[] =
      this.diagram.findObjectsUnderMouse(this.currentPosition);
    ...
    if (!isBlazor()) {
      arg = {
        ...
        element: cloneBlazorObject(
          this.eventArgs.source === this.eventArgs.actualObject
            ? undefined
            : this.eventArgs.source
        ),
        actualObject: cloneBlazorObject(this.eventArgs.actualObject),
    ...
    //Removed isBlazor code
    if (
      this.lastObjectUnderMouse &&
      this.diagram.mouseLeave &&
      (!this.eventArgs.actualObject ||
    ...
    if (
      (!this.lastObjectUnderMouse && this.eventArgs.source) ||
      this.lastObjectUnderMouse !== this.eventArgs.actualObject
    ) {
      this.lastObjectUnderMouse = this.eventArgs.actualObject;
  ...
  private elementEnter(mousePosition: PointModel, elementOver: boolean): void {
    if (!elementOver) {
      ...
      if (
        this.hoverElement &&
        (this.hoverElement as NodeModel).children &&
        (this.hoverElement as NodeModel).children.length > 0
      ) {
      ...
      if (this.hoverElement.tooltip.openOn === 'Auto' && content !== '') {
        ...
        if (children) {
          ...
          updateTooltip(
            this.diagram,
            isPrivateTooltip ? this.hoverElement : undefined
          );
        }
      }
      // EJ2-66418 - set tooltip relativeMode as mouse
      ...
      let obj: IElement = this.diagram.findObjectUnderMouse(
        objects,
        this.action,
        this.inAction
      );
      ...
      if (obj !== null) {
        ...
        if (
          (obj as Node | Connector).ports &&
          (this.hoverElement instanceof PointPort ||
            this.hoverElement instanceof PathPort)
        ) {
          //executed to set target as port
          targetEle = document.getElementById(
            (obj as Node).id + '_' + (this.hoverElement as any).id
          );
        } else if (
          (obj as Node | Connector).annotations &&
          (this.hoverElement instanceof ShapeAnnotation ||
            this.hoverElement instanceof PathAnnotation)
        ) {
          ...
          const idName: string =
            (obj as Node).shape && (obj as Node).shape instanceof Native
        ...
        if (this.hoverElement.tooltip.openOn === 'Auto' && content !== '') {
          ...
          (this.diagram.tooltipObject as DiagramTooltipModel).openOn = (
            this.hoverElement.tooltip as DiagramTooltipModel
          ).openOn;
          //Removed isBlazor code
          (this.diagram.tooltipObject as Tooltip).dataBind();
        }
        if (
          canEnableToolTip(this.hoverElement, this.diagram) &&
          this.hoverElement.tooltip.openOn === 'Auto'
        ) {
  ...
  private elementLeave(): void {
    if (
      this.diagram.tooltipObject &&
      !(this.diagram.tooltipObject as Tooltip).isSticky &&
      (this.diagram.tooltipObject as DiagramTooltipModel).openOn !== 'Custom'
    ) {
      this.diagram.tooltipObject.close();
    }
  ...
  private setTooltipOffset(mousePosition: PointModel): void {
    const offset: PointModel = getTooltipOffset(
      this.diagram,
      mousePosition,
      this.hoverElement
    );
  ...
  public doubleClick(evt: PointerEvent): void {
    if (canUserInteract(this.diagram)) {
      ...
      if (obj !== null && canUserInteract(this.diagram)) {
        ...
        annotation = this.diagram.findElementUnderMouse(
          obj,
          this.currentPosition,
          this.diagram
        );
        if (
          ...
          const layer: LayerModel = this.diagram.commandHandler.getObjectLayer(
            (obj as NodeModel).id
          );
              ...
              this.diagram.startTextEdit(
                ...
                id ||
                  (annotation instanceof TextElement
                    ? annotation.id.split((obj as Node).id + '_')[1]
                    : undefined)
              );
            }
          }
  ...
  public inputChange(evt: InputArgs): void {
    ...
    let scale: number;
    const editTextBox: HTMLElement = document.getElementById(
      this.diagram.element.id + '_editBox'
    );
    const editTextBoxDiv: HTMLElement = document.getElementById(
      this.diagram.element.id + '_editTextBoxDiv'
    );
    const text: string = (editTextBox as HTMLTextAreaElement).value;
    const line: string[] = text.split('\n');
    node = this.diagram.selectedItems.nodes[0]
      ? this.diagram.selectedItems.nodes[0]
      : this.diagram.selectedItems.connectors[0];
    if (
      (!node && this.tool instanceof TextDrawingTool) ||
      (node && node.shape.type === 'SwimLane')
    ) {
    ...
    if (
      ...
      textWrapper = this.diagram.getWrapper(
        node.wrapper,
        this.diagram.activeLabel.id
      );

      maxWidth =
        node.wrapper.bounds.width < textWrapper.bounds.width
          ? node.wrapper.bounds.width
          : textWrapper.bounds.width;
      maxWidth = maxWidth > minWidth ? maxWidth : minWidth;
      textBounds = measureHtmlText(
        textWrapper.style,
        text,
        undefined,
        undefined,
      ...
      height =
        (minHeight > textBounds.height ? minHeight : textBounds.height) * scale;
      editTextBoxDiv.style.left =
        (textWrapper.bounds.center.x + transforms.tx) * transforms.scale -
        width / 2 -
        2.5 +
        'px';
      editTextBoxDiv.style.top =
        (textWrapper.bounds.center.y + transforms.ty) * transforms.scale -
        height / 2 -
        3 +
        'px';
  ...
  public isAddTextNode(node: Node | Connector, focusOut?: boolean): boolean {
    if (this.tool instanceof TextDrawingTool || focusOut) {
      ...
      if (
        getObjectFromCollection(this.diagram.nodes, node.id) ||
  ...
  private checkEditBoxAsTarget(
    evt: PointerEvent | KeyboardEvent | TouchEvent
  ): boolean {
    if (
      evt.target &&
      (evt.target as HTMLElement).id === this.diagram.element.id + '_editBox'
    ) {
      return true;
    }
    return false;
  ...
  private getMouseEventArgs(
    ...
    if (!source) {
      if (
        this.action === 'Drag' ||
        this.action === 'ConnectorSourceEnd' ||
        this.action === 'SegmentEnd' ||
        this.action === 'OrthoThumb' ||
        this.action === 'BezierSourceThumb' ||
        this.action === 'BezierTargetThumb' ||
        this.action === 'ConnectorTargetEnd' ||
        this.action.indexOf('Rotate') !== -1 ||
        this.action.indexOf('Resize') !== -1
      ) {
        ...
        if (
          ...
          obj = this.diagram.findObjectUnderMouse(
            objects,
            this.action,
            this.inAction
          );
      ...
      obj = this.diagram.findTargetObjectUnderMouse(
        objects,
        this.action,
        this.inAction,
        args.position,
    ...
    if (obj) {
      wrapper = this.diagram.findElementUnderMouse(
        obj,
        this.currentPosition,
        this.diagram,
        padding
      ...
      if (
        ((wrapper &&
          (obj as Node | Connector).ports &&
          (obj as Node | Connector).ports.length &&
          !checkPort(obj, wrapper)) ||
          !wrapper ||
          !(obj as Node)) &&
        objects &&
        objects.length &&
        ...
        for (i = objects.length - 1; i >= 0; i--) {
          ...
          if (
            (nearNode instanceof Node || nearNode instanceof Connector) &&
            currentConnector &&
            ...
            if (
              ...
              currentConnector &&
              currentConnector.connectionPadding &&
              nearNode.wrapper.outerBounds.containsPoint(
                this.currentPosition
              ) &&
              currentConnector.constraints &
    ...
    if (
      args.source instanceof Selector &&
      args.actualObject === undefined &&
      (args.source.nodes.length > 0 || args.source.connectors.length > 0)
    ) {
      args.actualObject =
        args.source.nodes.length > 0
          ? this.diagram.nameTable[args.source.nodes[0].id]
  ...
  public getTool(action: Actions): ToolBase {
    switch (action) {
      ...
      case 'Draw':
        ...
        const basicShape: string = 'basicShape';
        ...
        if (
          ...
          type === 'Node' &&
          this.diagram.drawingObject.shape[`${shape}`] === 'Polygon' &&
          ...
          return new NodeDrawingTool(
            this.commandHandler,
            this.diagram.drawingObject as Node
          );
        } else if (
          type === 'Connector' &&
          (this.diagram.drawingObject as Connector).type === 'Polyline'
        ) {
  ...
  public getCursor(action: Actions): string {
    const object: DiagramElement | SelectorModel = (
      this.diagram.selectedItems as Selector
    ).annotation
      ? this.diagram.selectedItems.wrapper.children[0]
      : this.diagram.selectedItems;
    const rotateAngle: number = (this.diagram.selectedItems as Selector)
      .annotation
      ? object.rotateAngle + (object as DiagramElement).parentTransform
      : object.rotateAngle;
  ...
  public findElementUnderMouse(
    ...
    return this.objectFinder.findElementUnderSelectedItem(
      obj,
      position,
      diagram,
      padding
  ...
  public findObjectsUnderMouse(
    ...
    return this.objectFinder.findObjectsUnderMouse(
      position,
      this.diagram,
      this.eventArgs,
      source
  ...
  public findObjectUnderMouse(
    ...
    return this.objectFinder.findObjectUnderMouse(
      this.diagram,
      objects,
      action,
      inAction,
  ...
  private updateContainerBounds(isAfterMouseUp?: boolean): boolean {
    ...
    if (
      this.diagram.selectedObject.helperObject &&
      this.diagram.selectedObject.actualObject instanceof Node
    ) {
      const boundsUpdate: boolean =
        this.tool instanceof ResizeTool ? true : false;
      ...
      if (isAfterMouseUp) {
        ...
        if (
          !parentNode ||
          (parentNode && parentNode.shape.type !== 'SwimLane')
        ) {
          this.diagram.updateDiagramObject(obj);
        }
        isGroupAction = updateCanvasBounds(
          this.diagram,
          obj,
          this.currentPosition,
          boundsUpdate
  ...
  private updateContainerProperties(): HistoryLog {
    ...
    let isChangeProperties: boolean = false;
    ...
    if (this.diagram.selectedObject.helperObject) {
      ...
      let target: IElement = this.diagram.findObjectUnderMouse(
        objects,
        this.action,
        this.inAction
      );
      ...
      if (obj instanceof Node) {
        if (obj.shape.type === 'SwimLane') {
          connectors = getConnectors(
            this.diagram,
            obj.wrapper.children[0] as GridPanel,
            0,
            true
          );
        }
        if (
          ...
          (this.diagram.getObject((obj as Node).parentId) as NodeModel).shape
            .type === 'SwimLane'
        ) {
          if (
            target instanceof Node &&
            this.diagram.getObject((target as Node).parentId) &&
            (this.diagram.getObject((target as Node).parentId) as NodeModel)
              .shape.type !== 'SwimLane'
          ) {
        ...
        if (
          ...
          obj.container &&
          obj.container.type === 'Canvas' &&
          (obj as Node).parentId &&
          (this.diagram.getObject((obj as Node).parentId) as NodeModel).shape
            .type === 'SwimLane' &&
          target &&
          target !== obj &&
          (target as Node).container &&
          (target as Node).container.type === 'Canvas' &&
          (target as Node).isLane &&
          (obj as Node).isLane &&
          (target as Node).parentId === (obj as Node).parentId
        ) {
          laneInterChanged(
            this.diagram,
            obj,
          ...
          if (
            ...
            if (
              ...
              const swimlane: Node = this.diagram.getObject(
                parentNode.parentId
              ) as Node;
              const laneId: string =
                swimlane.id +
                (swimlane.shape as SwimLaneModel).lanes[0].id +
                '0';
              const firstlane: NodeModel = this.diagram.getObject(
                laneId
              ) as Node;
              const x: number = firstlane.wrapper.bounds.x;
              const y: number = firstlane.wrapper.bounds.y;
              ...
              if (this.diagram.selectedItems.nodes.length === 1) {
                ...
                const offsetX: number =
                  helperObject.offsetX - this.diagram.selectedItems.offsetX;
                const offsetY: number =
                  helperObject.offsetY - this.diagram.selectedItems.offsetY;
                const rotateAngle: number =
                  helperObject.rotateAngle -
                  this.diagram.selectedItems.rotateAngle;
                for (
                  let i: number = 0;
                  i < this.diagram.selectedItems.nodes.length;
                  i++
          ...
          if (
            ...
            parentNode.container.type === 'Stack'
          ) {
          ...
          if (
            ...
            const index: number = parentNode.wrapper.children.indexOf(
              obj.wrapper
            );
            undoElement = {
            ...
            if (index > -1) {
              const children: string[] = parentNode.children;
          ...
          parentNode = checkParentAsContainer(this.diagram, obj)
            ? this.diagram.nameTable[(obj as Node).parentId]
          ...
          if (
            ...
            parentNode.wrapper.maxWidth = parentNode.maxWidth =
              parentNode.wrapper.actualSize.width;
            parentNode.wrapper.maxHeight = parentNode.maxHeight =
              parentNode.wrapper.actualSize.height;
          ...
          if (
            ...
            history = this.updateContainerPropertiesExtend(
              parentNode,
              obj,
              connectors,
              helperObject,
    ...
    if (
      obj &&
      ((obj as SwimLane).isPhase ||
        (obj as SwimLane).isLane ||
  ...
  private updateLaneChildNode(obj: NodeModel): void {
    for (
      let i: number = 0;
      i < (obj.shape as SwimLaneModel).lanes.length;
      i++
    ) {
      if (
        (obj.shape as SwimLaneModel).lanes[parseInt(i.toString(), 10)]
          .children &&
        (obj.shape as SwimLaneModel).lanes[parseInt(i.toString(), 10)].children
          .length > 0
      ) {
        for (
          let j: number = 0;
          ...
          const id: string = (obj.shape as SwimLaneModel).lanes[
            parseInt(i.toString(), 10)
          ].children[parseInt(j.toString(), 10)].id;
  ...
  private updateContainerPropertiesExtend(
    ...
    if (
      this.currentAction === 'ResizeEast' ||
      this.currentAction === 'ResizeSouth' ||
      obj.shape.type === 'SwimLane'
    ) {
      const undoObj: NodeModel = cloneObject(obj);
      let isUpdateRow: boolean = false;
      if (
        parentNode &&
        parentNode.container &&
        parentNode.container.type === 'Grid'
      ) {
        const shape: boolean =
          parentNode.shape.type === 'SwimLane' ? true : false;
        const container: GridPanel = (
          shape ? parentNode.wrapper.children[0] : parentNode.wrapper
        ) as GridPanel;
        const padding: number = shape
          ? (parentNode.shape as SwimLane).padding
          : undefined;
        const x: number = parentNode.wrapper.bounds.x;
        const y: number = parentNode.wrapper.bounds.y;
        if (
          (obj.columnIndex !== undefined &&
            parentNode.container.orientation === 'Horizontal' &&
            ((shape && (obj as Node).isPhase) ||
              (!shape && obj.rowIndex === 1))) ||
          (parentNode.container.orientation === 'Vertical' &&
            ((!shape && obj.rowIndex > 0 && obj.columnIndex > 0) ||
              (shape && (obj as Node).isLane)))
        ) {
          if (
            parentNode.container.orientation === 'Horizontal' &&
            (obj as Node).isPhase &&
            obj.wrapper.width > obj.maxWidth
          ) {
            obj.maxWidth = obj.wrapper.width;
            obj.wrapper.maxWidth = obj.wrapper.width;
          }
          updateSwimLaneObject(
            this.diagram,
            obj as Node,
            parentNode,
            helperObject
          );
          container.updateColumnWidth(
            obj.columnIndex,
            helperObject.width,
            true,
            padding
          ...
          if ((obj as Node).isPhase) {
            const id: string = (parentNode.shape as SwimLaneModel).phases[
              obj.columnIndex
            ].header.id;
          ...
          if (parentNode.shape.type === 'SwimLane') {
            parentNode.width = parentNode.width
              ? container.width
              : parentNode.width;
        ...
        this.diagram.drag(
          parentNode,
          x - parentNode.wrapper.bounds.x,
          y - parentNode.wrapper.bounds.y
        );
      } else {
        if (obj && obj.shape && obj.shape.type === 'UmlClassifier') {
          obj.wrapper.measureChildren = true;
  ...
  private addUmlNode(): void {
    const node: NodeModel = this.diagram.selectedItems.nodes[0];
    let objects: IElement[] = this.diagram.findObjectsUnderMouse({
      x: this.currentPosition.x + 20,
      y: this.currentPosition.y,
    });
    let target: IElement = this.diagram.findObjectUnderMouse(
    ...
    if (
      ...
      const adornerSvg: SVGElement = getAdornerLayerSvg(
      ...
      if (highlighter) {
        ...
        const temp: NodeModel = new Node(
          ...
          'nodes',
          {
            ...
            annotations: (target as Node).annotations,
        ...
        if ((node.shape as UmlClassifierShapeModel).classifier === 'Class') {
          if (
            (target as Node).id.includes('_umlProperty') &&
            (node.shape as UmlClassifierShapeModel).classShape.attributes
              .length > 0
          ) {
            attribute = new UmlClassAttribute(
              node as MethodArguments,
              '',
              child
            );
            ...
            (target as Node).id.includes('_umlMethods') &&
            (node.shape as UmlClassifierShapeModel).classShape.methods.length >
              0
          ) {
        ...
        if (
          ...
          if (
            (target as Node).id.includes('_umlProperty') &&
            (node.shape as UmlClassifierShapeModel).interfaceShape.attributes
              .length > 0
          ) {
            attribute = new UmlClassAttribute(
              node as MethodArguments,
            ...
            (target as Node).id.includes('_umlMethods') &&
            (node.shape as UmlClassifierShapeModel).interfaceShape.methods
              .length > 0
          ) {
            method = new UmlClassMethod(node as MethodArguments, '', child);
        ...
        if (
          ...
          if (
            (target as Node).id.includes('_umlMember') &&
            (node.shape as UmlClassifierShapeModel).enumerationShape.members
              .length > 0
          ) {
            const member: UmlEnumerationMemberModel = new UmlEnumerationMember(
            ...
            ).enumerationShape.members.push(member);
          }
        }
...
class ObjectFinder {
  ...
  public findObjectsUnderMouse(
    ...
    if (source && source instanceof Selector) {
      if (source.nodes.length + source.connectors.length === 1) {
        ...
        if (
          ...
          (source as Node).children.length === 0
        ) {
    ...
    const endPadding: number =
      (source &&
        source instanceof Connector &&
        (source.constraints & ConnectorConstraints.ConnectToNearByNode ||
          source.constraints & ConnectorConstraints.ConnectToNearByPort) &&
        source.connectionPadding) ||
      0;
    ...
    for (const obj of objArray) {
      ...
      let pointInBounds: boolean = (obj as NodeModel).rotateAngle
        ? false
        : bounds.containsPoint(point, endPadding);
      if (
        (obj !== source || diagram.currentDrawingObject instanceof Connector) &&
        obj instanceof Connector
          ? obj !== diagram.currentDrawingObject
          : true && (obj as NodeModel).wrapper.visible
      ) {
        const layer: LayerModel = diagram.commandHandler.getObjectLayer(
          (obj as NodeModel).id
        );
        if (layer && !layer.lock && layer.visible) {
          layerTarger = layerObjTable[layer.zIndex] =
            layerObjTable[layer.zIndex] || [];
          if ((obj as NodeModel).rotateAngle) {
            ...
            for (child of container.children) {
              ...
              rotateMatrix(
                matrix,
                -(child.rotateAngle + child.parentTransform),
                child.offsetX,
                child.offsetY
              );
              point = transformPointByMatrix(matrix, pt);
              if (
                cornersPointsBeforeRotation(child).containsPoint(
                  point,
                  endPadding
                )
              ) {
          ...
          if (!source || isSelected(diagram, obj) === false) {
            if (canEnablePointerEvents(obj, diagram)) {
              if (
                obj instanceof Connector
                  ? isPointOverConnector(obj, pt)
                  : pointInBounds
              ) {
                const padding: number =
                  obj instanceof Connector ? obj.hitPadding || 0 : 0; //let element: DiagramElement;
                const element: DiagramElement = this.findElementUnderMouse(
                  obj as IElement,
                  pt,
                  diagram,
                  endPadding || padding
    ...
    for (const layer of diagram.layers) {
      ...
      for (const obj of actualTarget) {
        ...
        if (
          obj.shape.type === 'Bpmn' &&
          (obj as Node).processId &&
          (!(diagram[`${eventHandler}`].tool instanceof MoveTool) ||
            (diagram[`${eventHandler}`].tool instanceof MoveTool &&
              canAllowDrop(obj)))
        ) {
          const index: number = actualTarget.indexOf(
            diagram.nameTable[(obj as Node).processId]
          );
          if (index > -1) {
        ...
        if (
          obj.shape.type === 'UmlClassifier' &&
          (obj as Node).container &&
          (obj as Node).container.type === 'Stack'
        ) {
          const index: number = actualTarget.indexOf(
            diagram.nameTable[diagram.nameTable[obj.id].wrapper.children[0].id]
          );
          if (index > -1) {
            actualTarget.splice(index, 1);
          }
    ...
    for (let i: number = 0; i < actualTarget.length; i++) {
      let parentObj: NodeModel =
        diagram.nameTable[
          (actualTarget[parseInt(i.toString(), 10)] as Node).parentId
        ];
      if (parentObj) {
        const portElement: DiagramElement = this.findElementUnderMouse(
          parentObj as IElement,
          pt,
          diagram
        );
        for (let j: number = 0; j < parentObj.ports.length; j++) {
          if (
            portElement &&
            portElement.id.match(
              '_' + parentObj.ports[parseInt(j.toString(), 10)].id + '$'
            )
          ) {
            const port: PointPortModel =
              parentObj.ports[parseInt(j.toString(), 10)];
      ...
      for (let i: number = 0; i < actualTarget.length; i++) {
        ...
        if (parentNode && parentNode.shape.type === 'SwimLane') {
          for (let j: number = 0; j < actualTarget.length; j++) {
            const connector: ConnectorModel | NodeModel =
              actualTarget[parseInt(j.toString(), 10)];
  ...
  /** @private */
  public checkSwimlane(
    actualTarget: (NodeModel | ConnectorModel)[],
    diagram: Diagram
  ): void {
    let isNode: Boolean;
    for (let m: number = 0; m < actualTarget.length; m++) {
      const obj: NodeModel | ConnectorModel =
        actualTarget[parseInt(m.toString(), 10)];
      let parentNode: string;
      let node: Node;
      if (obj instanceof Node) {
        parentNode = (actualTarget[parseInt(m.toString(), 10)] as Node)
          .parentId;
        node = obj as Node;
      }
      if (parentNode === '') {
      ...
      if (
        parent &&
        (parent as Node).isLane &&
        diagram.nameTable[(parent as Node).parentId].zIndex >
          (obj as Node).zIndex
      ) {
      ...
      if (
        ...
        if (
          (actualTarget[parseInt(m.toString(), 10)] as Node).zIndex <
          (actualTarget[m - 1] as Node).zIndex
        ) {
          const swap: NodeModel | ConnectorModel =
            actualTarget[parseInt(m.toString(), 10)];
    ...
    if (actualTarget.length >= 2) {
      ...
      for (let i: number = actualTarget.length - 1; i >= 0; i--) {
        if ((actualTarget[parseInt(i.toString(), 10)] as Node).parentId) {
          const parent1: string = findParentInSwimlane(
            actualTarget[parseInt(i.toString(), 10)] as Node,
            diagram,
            parent
          );
          const parent2: string = findParentInSwimlane(
            actualTarget[i - 1] as Node,
            diagram,
            parent
          );
          ...
          if (
            parentNode2 &&
            parent1 !== parent2 &&
            parentNode1.zIndex < parentNode2.zIndex
          ) {
            actualTarget.splice(i, 1);
          }
        }
  ...
  public isTarget(
    ...
    node =
      action === 'ConnectorSourceEnd'
        ? diagram.nameTable[connector.targetID]
    ...
    if (action === 'ConnectorSourceEnd' && connector.targetID) {
      ...
      if (
        ...
        if (
          actualTarget &&
          actualTarget.parentId &&
          diagram.nameTable[actualTarget.parentId].shape.type ===
            'UmlClassifier'
        ) {
          actualTarget = diagram.nameTable[actualTarget.parentId];
        }
  ...
  public findObjectUnderMouse(
    ...
    if (objects.length !== 0) {
      ...
      if (
        (action === 'ConnectorSourceEnd' && source) ||
        action === 'PortDraw' ||
        ((canDrawOnce(diagram) || canContinuousDraw(diagram)) &&
          getObjectType(diagram.drawingObject) === Connector)
      ) {
        const connector: ConnectorModel = diagram.selectedItems.connectors[0];
        for (let i: number = objects.length - 1; i >= 0; i--) {
          outPort = getInOutConnectPorts(
            objects[parseInt(i.toString(), 10)] as Node | Connector,
            false
          );
          inPort = getInOutConnectPorts(
            objects[parseInt(i.toString(), 10)] as Node | Connector,
            true
          );
          const tool: ConnectTool = diagram[`${eventHandler}`]
            .tool as ConnectTool;
          const portElement: DiagramElement = this.findTargetElement(
          ...
          if (
            action === 'Draw' &&
            portElement &&
            (objects[parseInt(i.toString(), 10)] instanceof Node ||
              objects[parseInt(i.toString(), 10)] instanceof Connector) &&
            !checkPort(objects[parseInt(i.toString(), 10)], portElement)
          ) {
            if (
              (tool &&
                tool[`${endPoint}`] === 'ConnectorSourceEnd' &&
                !canOutConnect(
                  objects[parseInt(i.toString(), 10)] as NodeModel
                )) ||
              (tool &&
                tool[`${endPoint}`] === 'ConnectorTargetEnd' &&
                !canInConnect(objects[parseInt(i.toString(), 10)] as NodeModel))
            ) {
              return actualTarget as IElement;
            }
          }
          ...
          if (
            ...
            if (connector) {
              actualTarget = this.isTarget(
                actualTarget as Node | Connector,
                diagram,
                action
              );
            }
        ...
        let index: number = 0;
        for (let i: number = 0; i < objects.length; i++) {
          const temp: NodeModel | ConnectorModel =
            objects[parseInt(i.toString(), 10)];
          if (
            source !== temp &&
            (temp instanceof Connector ||
              !position ||
              temp.wrapper.bounds.containsPoint(position))
          ) {
            if (canAllowDrop(temp as NodeModel | ConnectorModel)) {
              if (!actualTarget) {
                ...
                actualTarget =
                  index >= (temp as Node | Connector).zIndex
                    ? actualTarget
                    : temp;
        ...
        if (actualTarget && actualTarget.shape.type === 'Bpmn') {
          if (
            ...
            diagram.selectedItems.nodes[0].shape.type === 'Bpmn'
          ) {
        ...
        (action === 'Select' || action === 'Pan') &&
        diagram[`${eventHandler}`].tool
      ) {
        for (let i: number = objects.length - 1; i >= 0; i--) {
          if (objects[parseInt(i.toString(), 10)] instanceof Connector) {
            ...
            if (objects[i - 1] instanceof Node && objj1.ports) {
              ...
              if (
                portElement &&
                (portElement.id.match('_icon_content_shape$') ||
                  portElement.id.match('_icon_content_rect$'))
              ) {
              ...
              for (let j: number = 0; j < objj1.ports.length; j++) {
                if (
                  portElement &&
                  portElement.id.match(
                    '_' + objj1.ports[parseInt(j.toString(), 10)].id + '$'
                  )
                ) {
                  if (
                    canDraw(objj1.ports[parseInt(j.toString(), 10)], diagram)
                  ) {
                    return objj1 as IElement;
                  }
                }
        ...
        for (let i: number = objects.length - 1; i >= 0; i--) {
          if (
            ...
            const portElement = this.findTargetElement(
              objects[parseInt(i.toString(), 10)].wrapper,
              position,
              diagram,
              undefined
  ...
  /* tslint:enable */
  /** @private */
  public findElementUnderSelectedItem(
    obj: IElement,
    position: PointModel,
    diagram: Diagram,
    padding?: number
    ...
    if (obj instanceof Selector) {
      if (
        obj.nodes.length === 1 &&
        (!obj.connectors || !obj.connectors.length)
      ) {
        return this.findElementUnderMouse(
          obj.nodes[0] as IElement,
          position,
          diagram
        );
      } else if (
        (!obj.nodes || obj.nodes.length) &&
        obj.connectors.length === 1
      ) {
        return this.findElementUnderMouse(
          obj.connectors[0] as IElement,
          position,
          diagram
        );
      }
  ...
  public findTargetElement(
    ...
    for (let i: number = container.children.length - 1; i >= 0; i--) {
      const element: DiagramElement =
        container.children[parseInt(i.toString(), 10)];
      ...
      if (
        ...
        if (element.visible) {
          if (element instanceof Container) {
            const target: DiagramElement = this.findTargetElement(
              element,
              position,
              diagram
            );
          ...
          if (
            ...
            (element.style as TextStyleModel).textOverflow !== 'Clip' &&
            (element.style as TextStyleModel).textOverflow !== 'Ellipsis'
          ) {
            return element;
          }
        }
    ...
    if (
      container.bounds.containsPoint(position, padding) &&
      container.style.fill !== 'none'
    ) {
      return container;
    }
    return null;
