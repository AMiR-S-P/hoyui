...
export class SvgRenderer implements IRenderer {
  ...
  public renderShadow(
    ...
    const point: PointModel = Point.transform(
      pointModel,
      options.shadow.angle,
      options.shadow.distance
    );
    ...
    if (parentSvg) {
      shadowElement = parentSvg.getElementById(
        canvas.id + '_shadow'
      ) as SVGPathElement;
    }
    if (!shadowElement) {
      type = collection ? 'path' : 'rect';
      shadowElement = document.createElementNS(
        'http://www.w3.org/2000/svg',
        type
      ) as SVGRectElement | SVGPathElement;
    ...
    const attr: Object = {
      ...
      transform:
        ...
        options.angle +
        ',' +
        (options.x + options.width * options.pivotX) +
        ',' +
    ...
    if (parentSvg) {
      const svgContainer: HTMLElement = parentSvg.getElementById(
        canvas.id
      ) as HTMLElement;
      if (svgContainer) {
  ...
  public drawRectangle(
    ...
    if (parentSvg && !options.shadow) {
      shadowElement = parentSvg.getElementById(
        options.id + '_groupElement_shadow'
      ) as HTMLElement;
      if (shadowElement) {
    ...
    if (!enableSelector) {
      if (
        classval.includes('e-diagram-resize-handle') ||
        classval.includes('e-diagram-endpoint-handle') ||
        classval.includes('e-diagram-bezier-control-handle')
      ) {
        classval += ' e-disabled';
      }
    }
   ...
   *  @private
   */
  public updateSelectionRegion(
    gElement: SVGElement,
    options: RectAttributes
  ): void {
    let rect: SVGElement;
    rect = (gElement.parentNode as SVGSVGElement).getElementById(
      options.id
    ) as SVGElement;
    const attr: Object = {
      id: options.id,
  ...
  public createGElement(elementType: string, attribute: Object): SVGGElement {
    const gElement: SVGGElement = createSvgElement(
      elementType,
      attribute
    ) as SVGGElement;
    return gElement;
  ...
  public drawLine(gElement: SVGElement, options: LineAttributes): void {
    const line: SVGElement = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'line'
    );
  ...
  public drawPath(
    ...
    if (parentSvg) {
      path = (parentSvg as SVGSVGElement).getElementById(
        options.id
      ) as SVGPathElement;
    }
    if (!path || isSelector) {
      ...
      switch (char) {
        ...
        case 'A':
          d =
            ...
            segment.r2.toString() +
            ',' +
            segment.angle.toString() +
            ',';
          d +=
            segment.largeArc.toString() +
            ',' +
            segment.sweep +
            ',' +
            x.toString() +
  ...
  public drawText(
    ...
    if (options.content !== undefined) {
      ...
      let position: PointModel;
      let child: SubTextElement;
      let tspanElement: SVGElement;
      ...
      if (text) {
        ...
        if (parentNode) {
          nodeContent = document.getElementById(
            parentNode.id + '_content_groupElement'
          );
        }
        if (
          nodeContent &&
          parentNode &&
          parentNode.children &&
          parentNode.children[0] instanceof DiagramNativeElement
        ) {
          const textTag: SVGGElement = this.createGElement('g', {
            id: ariaLabel + '_groupElement',
          });
      ...
      if (options.doWrap || options.textOverflow !== 'Wrap') {
        ...
        if (
          wrapBounds.width > options.width &&
          options.textOverflow !== 'Wrap' &&
          options.textWrapping === 'NoWrap'
        ) {
        ...
        for (i = 0; i < childNodes.length; i++) {
          ...
          textNode = document.createTextNode(
            childNodes[parseInt(i.toString(), 10)].text
          );
          ...
          if (
            (options.textOverflow === 'Clip' ||
              options.textOverflow === 'Ellipsis') &&
            (options.textWrapping === 'WrapWithOverflow' ||
              options.textWrapping === 'Wrap') &&
            parentNode
          ) {
            const size: number = options.isHorizontalLane
              ? parentNode.actualSize.width
              : parentNode.actualSize.height;
            if (offsetY < size) {
              if (options.textOverflow === 'Ellipsis' && childNodes[i + 1]) {
                const temp: SubTextElement = childNodes[i + 1];
                const y: number =
                  position.y + temp.dy * (i + 1) + options.fontSize * 0.8;
                if (y > size) {
      ...
      if (childNodesHeight && options.isHorizontalLane) {
        ...
        options.y =
          options.parentOffsetY - childNodesHeight * options.pivotY + 0.5;
      }
  ...
  private alignText(
    text: SVGTextElement,
    tspanElement: SVGElement,
    child: SubTextElement,
    textNode: Text,
    offsetX: number,
    offsetY: number,
    i: number,
  ...
  private setText(
    ...
    if (options.textAlign !== 'justify') {
      ...
      setAttributeSvg(tspanElement, {
        ...
        lengthAdjust: adjustlen ? adjustlen : 'spacing',
      });
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  public drawImage(
    canvas: SVGElement | HTMLCanvasElement,
    obj: ImageAttributes,
    parentSvg?: SVGSVGElement,
    fromPalette?: boolean
  ): void {
    ...
    let aspectRatio: string =
      imgAlign.charAt(0).toLowerCase() + imgAlign.slice(1);
    if (scale !== 'Stretch') {
    ...
    const attr: Object = {
      ...
      transform:
        ...
        obj.angle +
        ',' +
        (obj.x + obj.width * obj.pivotX) +
        ',' +
        (obj.y + obj.height * obj.pivotY) +
        ')',
  ...
  public drawHTMLContent(
    ...
    if (!htmlElement) {
      parentHtmlElement =
        canvas.querySelector('#' + element.id + '_html_element') ||
      ...
      const diagram: any = (document.getElementById(element.diagramId) as any)
        .ej2_instances[0];
      let isOverviewLayer: boolean = false;
      if (
        canvas.parentNode &&
        canvas.parentNode.parentNode &&
        canvas.parentNode.parentNode.parentNode &&
        (canvas.parentNode.parentNode.parentNode as any).classList.contains(
          'e-overview'
        )
      ) {
      ...
      if (isOverviewLayer) {
        ...
        if (typeof element.template === 'string') {
          ...
          const handle: UserHandleModel[] =
            diagram.selectedItems.userHandles.filter((x: UserHandleModel) => {
              return (
                x.name === element.id.split('_shape')[0] &&
                (x.template as any) !== ''
              );
            });
      ...
      if (indexValue !== undefined && canvas.childNodes.length > indexValue) {
        canvas.insertBefore(
          htmlElement,
          canvas.childNodes[parseInt(indexValue.toString(), 10)]
        );
      }
    ...
    htmlElement.setAttribute(
      ...
      'height:' +
        ...
        (element.rotateAngle + element.parentTransform) +
        'deg);' +
        'pointer-events:' +
        (value ? 'all' : 'none') +
        ';visibility:' +
  ...
  public drawNativeContent(
    ...
    if (parentSvg) {
      nativeElement = parentSvg.getElementById(
        element.id + '_native_element'
      ) as SVGPathElement;
      clipPath = parentSvg.getElementById(
        element.id + '_clip'
      ) as SVGClipPathElement;
    }
    if (!nativeElement) {
      nativeElement = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'g'
      );
      nativeElement.setAttribute('id', element.id + '_native_element');
      nativeElement.appendChild(element.template.cloneNode(true));
      const svgContentTag: SVGGElement = this.createGElement('g', {
        id: element.id + '_inner_native_element',
      });
  ...
  private setNativTransform(
    ...
    const contentWidth: number =
      element.contentSize.width !== 0 ? element.contentSize.width : 1;
    const contentHeight: number =
      element.contentSize.height !== 0 ? element.contentSize.height : 1;
    ...
    nativeElement.setAttribute(
      ...
      'rotate(' +
        ...
        element.offsetY +
        ') translate(' +
        (element.offsetX - x - width * element.pivot.x) +
        ',' +
        (element.offsetY - y - height * element.pivot.y) +
        ') scale(' +
        width / contentWidth +
        ',' +
  ...
  public drawClipPath(
    ...
    const x: number =
      node.templatePosition.x +
      (node.width >= node.height ? 0 : (contentWidth - clipWidth) / 2);
    const y: number =
      node.templatePosition.y +
      (node.height >= node.width ? 0 : (contentHeight - clipHeight) / 2);
    let clipPath: SVGClipPathElement = parentSvg.getElementById(
      node.id + '_clip'
    ) as SVGClipPathElement;
    ...
    let rect: SVGRectElement = parentSvg.getElementById(
      node.id + '_clip_rect'
    ) as SVGRectElement;
    rect = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
  ...
  public renderGradient(
    ...
    let defs: SVGElement = svgContainer.getElementById(
      diagramId + 'gradient_pattern'
    ) as SVGElement;
    if (!defs) {
    ...
    let linear: LinearGradientModel; //let stop: StopModel; let offset: number;
    removeGradient(svg.id);
    if (options.gradient.type !== 'None') {
      for (let i: number = 0; i < options.gradient.stops.length; i++) {
        max = !max
          ? options.gradient.stops[parseInt(i.toString(), 10)].offset
          : Math.max(
              max,
              options.gradient.stops[parseInt(i.toString(), 10)].offset
            );
      ...
      for (let i: number = 0; i < options.gradient.stops.length; i++) {
        const stop: StopModel =
          options.gradient.stops[parseInt(i.toString(), 10)];
        const offset: number =
          min < 0 ? (max + stop.offset) / (2 * max) : stop.offset / max;
        const stopElement: SVGElement = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'stop'
        );
  ...
  public createLinearGradient(linear: LinearGradientModel): SVGElement {
    const lineargradient: SVGElement = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'linearGradient'
    );
    const attr: Object = {
  ...
  public createRadialGradient(radial: RadialGradientModel): SVGElement {
    const radialgradient: SVGElement = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'radialGradient'
    );
    const attr: Object = {
  ...
  public setSvgStyle(
    ...
    if (
      (style as BaseAttributes).canApplyStyle ||
      (style as BaseAttributes).canApplyStyle === undefined
    ) {
      if (style.fill === 'none') {
   ...
   * @private
   */
  public svgLabelAlign(
    text: TextAttributes,
    wrapBound: TextBounds,
    childNodes: SubTextElement[]
  ): PointModel {
    const bounds: Size = new Size(
      wrapBound.width,
      childNodes.length * (text.fontSize * 1.2)
    );
    ...
    if (text.textAlign === 'left' || text.textAlign === 'justify') {
      ...
      if (
        wrapBound.width > text.width &&
        (text.textOverflow === 'Ellipsis' || text.textOverflow === 'Clip')
      ) {
