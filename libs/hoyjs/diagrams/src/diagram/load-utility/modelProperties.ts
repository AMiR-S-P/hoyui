...
import { SelectedItem } from '@hoyui/lists';
import { ContextMenu, ContextMenuModel } from '@hoyui/navigations';
...
export class Ej1Serialization {
  ...
  public getSerializedData(data: Object): any {
    ...
    if (
      (data as Diagram).connectors &&
      (data as Diagram).connectors.length > 0
    ) {
      this.connectorProperties.renderConnectorsCollection(
        this.convertedData,
        data as Diagram
      );
    } else {
    ...
    if ((data as any).defaultSettings !== undefined) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      this.convertedData.getNodeDefaults = this.getNodeDefaults(
        (data as any).defaultSettings.node
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      this.convertedData.getConnectorDefaults = this.getConnectorDefaults(
        (data as any).defaultSettings.connector
      );
    }
    ...
    if ((data as any).snapSettings.enableSnapToObject) {
      this.convertedData.snapSettings.constraints =
        this.convertedData.snapSettings.constraints |
    ...
    this.convertedData.constraints = this.setDiagramConstraints(
      (data as any).constraints
    );
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  public setDataSourceSettings(
    convertedData: Object | any,
    data: Object | any
  ): void {
    convertedData.dataSourceSettings = {};
    convertedData.dataSourceSettings.id = data.dataSourceSettings.id;
    convertedData.dataSourceSettings.dataManager =
    ...
    if (data.dataSourceSettings.connectionDataSource) {
      ...
      convertedData.dataSourceSettings.connectionDataSource.id =
        data.dataSourceSettings.connectionDataSource.id;
      convertedData.dataSourceSettings.connectionDataSource.dataManager =
        data.dataSourceSettings.connectionDataSource.dataSource;
      convertedData.dataSourceSettings.connectionDataSource.sourceID =
        data.dataSourceSettings.connectionDataSource.sourceNode;
      convertedData.dataSourceSettings.connectionDataSource.targetID =
        data.dataSourceSettings.connectionDataSource.targetNode;
      convertedData.dataSourceSettings.connectionDataSource.sourcePointX =
        data.dataSourceSettings.connectionDataSource.sourcePointX;
      convertedData.dataSourceSettings.connectionDataSource.sourcePointY =
        data.dataSourceSettings.connectionDataSource.sourcePointY;
      convertedData.dataSourceSettings.connectionDataSource.targetPointX =
        data.dataSourceSettings.connectionDataSource.targetPointX;
      convertedData.dataSourceSettings.connectionDataSource.targetPointY =
        data.dataSourceSettings.connectionDataSource.targetPointY;
      convertedData.dataSourceSettings.connectionDataSource.crudAction = {
        ...
        destroy:
          data.dataSourceSettings.connectionDataSource.crudAction.destroy,
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  public setRulerSettings(
    convertedData: Object | any,
    data: Object | any
  ): void {
    convertedData.rulerSettings = {};
    convertedData.rulerSettings.showRulers = data.rulerSettings.showRulers;
    convertedData.rulerSettings.horizontalRuler = this.setRulerProperties(
      data.rulerSettings.horizontalRuler
    );
    convertedData.rulerSettings.verticalRuler = this.setRulerProperties(
      data.rulerSettings.verticalRuler
    );
  }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  public setSnapSettings(
    convertedData: Object | any,
    data: Object | any
  ): void {
    convertedData.snapSettings = {};
    if (data.snapSettings) {
      convertedData.snapSettings.horizontalGridLines = this.setGridLines(
        data.snapSettings.horizontalGridLines
      );
      convertedData.snapSettings.verticalGridLines = this.setGridLines(
        data.snapSettings.verticalGridLines
      );
      convertedData.snapSettings.snapAngle = data.snapSettings.snapAngle;
      convertedData.snapSettings.snapObjectDistance =
        data.snapSettings.snapObjectDistance;
      convertedData.snapSettings.constraints = this.setSnapConstraints(
        data.snapSettings.snapConstraints
      );
    }
  }
  ...
  public setScrollSettings(
    ...
    convertedData.scrollSettings = {
      ...
      scrollLimit:
        data.pageSettings.scrollLimit.charAt(0).toUpperCase() +
        data.pageSettings.scrollLimit.slice(1),
  ...
  public setPageSettings(
    ...
    convertedData.pageSettings = {
      ...
      margin: {
        left: data.pageSettings.pageMargin,
        right: data.pageSettings.pageMargin,
        top: data.pageSettings.pageMargin,
        bottom: data.pageSettings.pageMargin,
      },
      orientation:
        data.pageSettings.pageOrientation.charAt(0).toUpperCase() +
        data.pageSettings.pageOrientation.slice(1),
      // eslint-disable-next-line max-len
      boundaryConstraints:
        data.pageSettings.boundaryConstraints.charAt(0).toUpperCase() +
        data.pageSettings.boundaryConstraints.slice(1),
      // eslint-disable-next-line max-len
      background: {
        color: data.pageSettings.pageBackgroundColor,
        source: data.backgroundImage.source,
        scale: data.backgroundImage.scale,
        align: data.backgroundImage.align,
      },
  ...
  public setContextMenu(convertedData: Object | any, data: Object | any): void {
    ...
    if (data.contextMenu !== undefined) {
      convertedData.contextMenuSettings.showCustomMenuOnly =
        data.contextMenu.showCustomMenuItemsOnly;
      convertedData.contextMenuSettings.items = this.getContextMenuItems(
        data.contextMenu.items
      );
    }
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  public setModelLayout(convertedData: Object | any, data: Object | any): void {
    convertedData.layout = {
      ...
      // eslint-disable-next-line max-len
      margin: {
        left: data.layout.margin.left,
        right: data.layout.margin.right,
        top: data.layout.margin.top,
        bottom: data.layout.margin.bottom,
      },
      horizontalAlignment:
        data.layout.horizontalAlignment.charAt(0).toUpperCase() +
        data.layout.horizontalAlignment.slice(1),
      verticalAlignment:
        data.layout.verticalAlignment.charAt(0).toUpperCase() +
        data.layout.verticalAlignment.slice(1),
      orientation:
        data.layout.orientation === 'toptobottom'
          ? 'TopToBottom'
          : data.layout.orientation === 'bottomtotop'
          ? 'BottomToTop'
          : data.layout.orientation === 'righttoleft'
          ? 'RightToLeft'
          : 'LeftToRight',
      connectorSegments: data.layout.getConnectorSegments,
      type:
        data.layout.type.charAt(0).toUpperCase() + data.layout.type.slice(1),
  ...
  public setSelectedItems(
    ...
    convertedData.selectedItems.constraints = this.setSelectorConstraints(
      data.selectedItems.constraints
    );
  ...
  public setSelectorConstraints(constraints: number): number {
    ...
    if (constraints & SelectorConstraints.ResizeSouthEast) {
      selectorConstraints =
        selectorConstraints | SelectorConstraints.UserHandle;
    }
  ...
  public setDiagramConstraints(constraints: number): number {
    ...
    if (constraints & DiagramConstraints.UserInteraction) {
      diagramConstraints =
        diagramConstraints | DiagramConstraints.UserInteraction;
    }
