import { NumericOptions } from '@hoyui/base/src/intl/parser-base';
...
export class ConnectorProperties {
  ...
  public renderConnectorsCollection(
    ...
    for (let i: number = 0; i < data.connectors.length; i++) {
      const connector: ConnectorModel =
        data.connectors[parseInt(i.toString(), 10)];
  ...
  public convertToConnector(connector: ConnectorModel): ConnectorModel {
    ...
    if (connector.constraints) {
      newConnector.constraints = this.setConnectorConstraints(
    ...
    if ((connector as EJ1Connector).labels) {
      newConnector.annotations = this.labelProperties.setLabelProperties(
        (connector as EJ1Connector).labels,
        connector
    ...
    if ((connector as EJ1Connector).lineDashArray) {
      newConnector.style.strokeDashArray = (
        connector as EJ1Connector
      ).lineDashArray;
    }
    ...
    if (connector.margin) {
      ...
      newConnector.margin = {
        left: connector.margin.left,
        right: connector.margin.right,
        top: connector.margin.top,
        bottom: connector.margin.bottom,
      };
    }
    if (connector.segments) {
      (newConnector as any).type =
        connector.segments[0].type.charAt(0).toUpperCase() +
        connector.segments[0].type.slice(1);
    ...
    if (shape) {
      if (shape.type === 'bpmn') {
        ...
        if (shape.flow === 'sequence') {
          connectorShape.sequence =
            shape.sequence.charAt(0).toUpperCase() + shape.sequence.slice(1);
        } else if (shape.flow === 'association') {
          connectorShape.association =
            shape.association.charAt(0).toUpperCase() +
            shape.association.slice(1);
        } else {
          connectorShape.message =
            shape.message.charAt(0).toUpperCase() + shape.message.slice(1);
        }
      }
      if (shape.type === 'umlclassifier') {
        connectorShape = {
          type: 'UmlClassifier',
          relationship:
            shape.relationship.charAt(0).toUpperCase() +
            shape.relationship.slice(1),
          multiplicity: {
            type:
              shape.multiplicity.type.charAt(0).toUpperCase() +
              shape.multiplicity.type.slice(1),
            source: {
  ...
  public setConnectorSegments(segments: any): any {
    ...
    if (segments.length > 0) {
      for (let i: number = 0; i < segments.length; i++) {
        ...
        // eslint-disable-next-line max-len
        segment.direction = segmentProp.direction
          ? segmentProp.direction.charAt(0).toUpperCase() +
            segmentProp.direction.slice(1)
          : segmentProp._direction
          ? segmentProp._direction.charAt(0).toUpperCase() +
            segmentProp._direction.slice(1)
          : null;
        segment.length = segmentProp.length
          ...
          ? segmentProp._length
          : null;
        segment.point = segmentProp.point
          ? { x: segmentProp.point.x, y: segmentProp.point.y }
          : null;
        segment.point1 = segmentProp.point1
          ? { x: segmentProp.point1.x, y: segmentProp.point1.y }
          : null;
        segment.point2 = segmentProp.point2
          ? { x: segmentProp.point2.x, y: segmentProp.point2.y }
          : null;
        // eslint-disable-next-line max-len
        segment.vector1 = segmentProp.vector1
          ? {
              angle: segmentProp.vector1.angle,
              distance: segmentProp.vector1.distance,
            }
          : null;
        // eslint-disable-next-line max-len
        segment.vector2 = segmentProp.vector2
          ? {
              angle: segmentProp.vector2.angle,
              distance: segmentProp.vector2.distance,
            }
          : null;
        ...
        if (segmentProp.type) {
          segment.type =
            segmentProp.type.charAt(0).toUpperCase() +
            segmentProp.type.slice(1);
        }
  ...
  public setConnectorConstraints(constraints: number): number {
    ...
    if (constraints & ConnectorConstraints.DragSourceEnd) {
      connectorConstraints =
        connectorConstraints | ConnectorConstraints.DragSourceEnd;
    }
    if (constraints & ConnectorConstraints.DragTargetEnd) {
      connectorConstraints =
        connectorConstraints | ConnectorConstraints.DragTargetEnd;
    }
    if (constraints & ConnectorConstraints.DragSegmentThumb) {
      connectorConstraints =
        connectorConstraints | ConnectorConstraints.DragSegmentThumb;
    }
    if (constraints & ConnectorConstraints.Bridging) {
      connectorConstraints =
        connectorConstraints | ConnectorConstraints.Bridging;
    }
    if (constraints & ConnectorConstraints.InheritBridging) {
      connectorConstraints =
        connectorConstraints | ConnectorConstraints.InheritBridging;
    }
    if (constraints & ConnectorConstraints.AllowDrop) {
      connectorConstraints =
        connectorConstraints | ConnectorConstraints.AllowDrop;
    }
    if (constraints & ConnectorConstraints.InheritTooltip) {
      connectorConstraints =
        connectorConstraints | ConnectorConstraints.InheritTooltip;
    }
    if (constraints & ConnectorConstraints.PointerEvents) {
      connectorConstraints =
        connectorConstraints | ConnectorConstraints.PointerEvents;
    }
    if (constraints & ConnectorConstraints.BridgeObstacle) {
      connectorConstraints =
        connectorConstraints | ConnectorConstraints.BridgeObstacle;
    }
    if (constraints & ConnectorConstraints.Interaction) {
      connectorConstraints =
        connectorConstraints | ConnectorConstraints.Interaction;
    }
