...
import { Component, Property } from '@hoyui/base';
import { INotifyPropertyChanged } from '@hoyui/base';
  ...
  implements INotifyPropertyChanged
{
  ...
  private updateRuler(): void {
    ...
    if (svg) {
      ...
      if (length && length !== Infinity) {
        ...
        this.updateSegments(
          unitOffset,
          unitLength + Math.abs(unitOffset),
          svg,
          rulerSize
        );
      }
  ...
  private updateSegment(
    ...
    if (run === start) {
      ...
      this.startValue =
        this.startValue % 1 !== 0
          ? Number(this.startValue.toFixed(1))
          : this.startValue;
  ...
  private updateTickLabel(rulerSegment: RulerSegment, rulerSize: number): void {
    const bBox: DOMRect = (
      rulerSegment.segment.lastChild as SVGTextElement
    ).getBBox() as DOMRect;
    const isHorizontal: boolean =
      this.orientation === 'Horizontal' ? true : false;
    const isRightOrBottom: boolean =
      this.tickAlignment === 'RightOrBottom' ? true : false;
    const x: number = isHorizontal ? 2 : 0;
    const y: number = isHorizontal
      ? isRightOrBottom
        ? rulerSize / 2 + 11 / 2 - 11 / 2
        : rulerSize / 2 + 11 / 2
      : bBox.height;
    const translate: string = isRightOrBottom
      ? -(bBox.width + 2) + ',' + (rulerSize / 2 - bBox.height)
      : -(bBox.width + 2) + ',' + (rulerSize / 2 - bBox.height / 2);
    const attr: Object = isHorizontal
      ? { x: x, y: y }
      : {
          x: x,
          y: y,
  ...
  private getNewSegment(run: number, svg: SVGElement): RulerSegment {
    ...
    const label: SVGTextElement = this.createTickLabel(
      svg,
      segment
    ) as SVGTextElement;
  ...
  private createTick(
    ...
    const args: IArrangeTickOptions = {
      ruler: ruler,
    ...
    const x2: number = isHorizontal
      ? point
      : isRightOrBottom
      ? rulerSize - linePoint
      : rulerSize - (rulerSize - linePoint);
    const y1: number = isHorizontal
      ? isRightOrBottom
        ? rulerSize
        : rulerSize - (rulerSize - linePoint)
      : point;
    const y2: number = isHorizontal
      ? isRightOrBottom
        ? rulerSize - linePoint
        : 0
      : point;
  ...
  public updateSegmentWidth(scale: number): number {
    ...
    const scaleRound: number = Math.pow(
      2,
  ...
  private createMarkerLine(
    rulerSvg: SVGSVGElement,
    rulerObj: HTMLElement,
    attr: Object
  ): SVGElement {
    let line: SVGElement;
    if (rulerObj) {
  ...
  public drawRulerMarker(
    ...
    const rulerSvgElements: NodeListOf<SVGSVGElement> | any =
      rulerObj.getElementsByTagName('svg');
    ...
    if (rulerSvg) {
      ...
      line.setAttribute(
        'transform',
        'translate(' +
          (isHorizontal ? move + 0.5 + ' 0.5' : '0.5 ' + (move + 0.5)) +
          ')'
      );
