...
import { isBlazor, BlazorDragEventArgs } from '@hoyui/base';
import {
  Browser,
  EventHandler,
  Draggable,
...
import { remove, EmitType } from '@hoyui/base';
import {
  Accordion,
  AccordionItemModel,
  ExpandMode,
...
import {
  ...
  updateDefaultValues,
} from '../diagram/utility/diagram-util';
import {
  getFunction,
  randomId,
  cloneObject,
} from '../diagram/utility/base-util';
...
} from '../diagram/objects/preview-model';
import { Tooltip, TooltipModel } from '@hoyui/popups';
...
let getObjectType: Function = (obj: Object): Object => {
  ...
  if (
    (obj as Node).shape &&
    ((obj as Node).shape instanceof Shape || (obj as Node).shape.type)
  ) {
    return Node;
  }
  return Node;
  ...
  implements INotifyPropertyChanged
{
  ...
  constructor(options?: SymbolPaletteModel, element?: Element) {
    ...
    for (
      let i: number = 0;
      this && this.palettes && i < this.palettes.length;
      i++
    ) {
      for (
        let j: number = 0;
        j < this.palettes[parseInt(i.toString(), 10)].symbols.length;
        j++
      ) {
        child = this.palettes[parseInt(i.toString(), 10)].symbols[
          parseInt(j.toString(), 10)
        ] as NodeModel;
        node = options.palettes[parseInt(i.toString(), 10)].symbols[
          parseInt(j.toString(), 10)
        ] as NodeModel;
        ...
        if (this.nodeDefaults || this.connectorDefaults) {
          updateDefaultValues(
            child,
            node,
            child instanceof Node ? this.nodeDefaults : this.connectorDefaults
          );
        }
      }
    }
  ...
  public onPropertyChanged(
        ...
        case 'palettes':
          for (const i of Object.keys(newProp.palettes)) {
            ...
            if (
              ...
              this.accordionElement.items[parseInt(index.toString(), 10)] = {
                header:
                  newProp.palettes[parseInt(index.toString(), 10)].title || '',
                expanded:
                  newProp.palettes[parseInt(index.toString(), 10)].expanded,
                iconCss:
                  newProp.palettes[parseInt(index.toString(), 10)].iconCss ||
                  '',
              };
            }
            if (newProp.palettes[parseInt(index.toString(), 10)].height) {
              const paletteDiv: HTMLElement = document.getElementById(
                (this.palettes[parseInt(index.toString(), 10)] as Palette).id +
                  '_content'
              );
              paletteDiv.style.height =
                newProp.palettes[parseInt(index.toString(), 10)].height + 'px';
            }
            if (
              newProp.palettes[parseInt(index.toString(), 10)].iconCss !==
              undefined
            ...
            if (
              ...
              if (
                !(this.palettes[parseInt(index.toString(), 10)] as Palette)
                  .isInteraction
              ) {
                this.accordionElement.items[
                  parseInt(index.toString(), 10)
    ...
    if (this.isExpand && !refresh) {
      ...
      for (let p: number = 0; p < this.palettes.length; p++) {
        const paletteElement: string =
          this.palettes[parseInt(p.toString(), 10)].id;
        if (window[`${paletteElement}`]) {
          if (window[`${paletteElement}`].length > 1) {
            window[`${paletteElement}`][1].parentNode.removeChild(
              window[`${paletteElement}`][1]
            );
  ...
  protected preRender(): void {
    ...
    if (this.element.id === '') {
      const collection: number =
        document.getElementsByClassName('e-symbolpalette').length;
    ...
    if (!isBlazor()) {
      const accordionDiv: HTMLElement = createHtmlElement('div', {
      ...
      this.accordionElement.expanded = (args: ExpandEventArgs) => {
        ...
        this.palettes[parseInt(index.toString(), 10)].expanded =
          args.isExpanded;
        (
          this.palettes[parseInt(index.toString(), 10)] as Palette
        ).isInteraction = true;
  ...
  public render(): void {
    ...
    this.svgRenderer = new DiagramRenderer(
      this.element.id,
      new SvgRenderer(),
      true
    );
      ...
      if (content) {
        ...
        if (window[`${measureElemnt}`]) {
          ...
          if (window[`${measureElementCount}`] === 0) {
            window[`${measureElemnt}`].parentNode.removeChild(
              window[`${measureElemnt}`]
            );
  ...
  public addPalettes(palettes: PaletteModel[]): void {
    ...
    for (let i: number = 0; i < palettes.length; i++) {
      ...
      palette = new Palette(
        this,
        'palettes',
        palettes[parseInt(i.toString(), 10)],
        true
      );
  ...
  private initSymbols(symbolGroup: PaletteModel): void {
    ...
    let laneHeight: number = 0;
    let laneWidth: number = 0;
    for (const symbol of symbolGroup.symbols) {
      if (symbol.shape.type === 'SwimLane') {
        ...
        const isHorizontal: boolean =
          swimLaneShape.orientation === 'Horizontal' ? true : false;
        if (swimLaneShape.isLane) {
          laneHeight = isHorizontal
            ? this.symbolHeight - this.symbolHeight / 2
            : this.symbolHeight - this.symbolHeight / 4;
          laneWidth = isHorizontal
            ? this.symbolWidth - this.symbolWidth / 4
            : this.symbolWidth - this.symbolWidth / 2;
          ...
          const laneObj: NodeModel = {
            ...
            shape: { type: 'Basic', shape: 'Rectangle' },
            width: isHorizontal
              ? swimLaneObj.width - header.width
              : swimLaneObj.width,
            height: isHorizontal
              ? swimLaneObj.height
              : swimLaneObj.height - header.height,
            style: laneStyle,
          };
          laneObj.offsetX = isHorizontal
            ? headerObj.width + laneObj.width / 2
            : laneObj.width / 2;
          laneObj.offsetY = isHorizontal
            ? laneObj.height / 2
            : headerObj.height + laneObj.height / 2;
          this.addPaletteItem(symbolGroup.id, laneObj);
          swimLaneObj.children = [headerObj.id, laneObj.id];
        } else if (swimLaneShape.isPhase) {
          laneHeight = swimLaneObj.height
            ? swimLaneObj.height
            : this.symbolHeight;
      ...
      if (
        symbol.shape.type === 'UmlClassifier' &&
        !(symbol.shape as RelationShipModel).relationship
      ) {
        ...
        if ((symbol.shape as UmlClassifierShapeModel).classifier === 'Class') {
          ...
          (symbol.shape as UmlClassifierShapeModel).classifier === 'Enumeration'
        ) {
          newObj = {
            id: 'node' + randomId(),
            shape: {
          ...
          (symbol.shape as UmlClassifierShapeModel).classifier === 'Interface'
        ) {
          newObj = {
            id: 'node' + randomId(),
            shape: {
        ...
        const memberObj: NodeModel = {
          ...
          shape: {
            type: 'HTML',
            content:
              '<div style="width:100%;height:100%;border:1px solid #000;background:white"></div>',
          },
          width: umlObj.width ? umlObj.width : 100,
          height: umlObj.height ? umlObj.height : 100,
          offsetX: umlObj.width ? umlObj.width / 2 : 50,
          offsetY: umlObj.height
            ? umlObj.height / 2 + newObj.height / 2
            : 50 + newObj.height / 2,
        };
      ...
      if (
        ...
        if (
          (symbol.shape as RelationShipModel).relationship === 'Inheritance'
        ) {
          symbol.style.strokeDashArray = symbol.style.strokeDashArray
            ? symbol.style.strokeDashArray
            : '4 4';
        }
      ...
      if (symbol instanceof Node) {
        ...
        const getConnectorDefaults: Function = getFunction(
    ...
    for (let i: number = 0; i < group.length; i++) {
      ...
      for (
        let j: number = 0;
        j < group[parseInt(i.toString(), 10)].children.length;
        j++
      ) {
        node =
          this.symbolTable[
            group[parseInt(i.toString(), 10)].children[
              parseInt(j.toString(), 10)
            ]
          ];
        if (node) {
  ...
  private renderPalette(symbolGroup: PaletteModel): void {
    ...
    const paletteParentDiv: HTMLElement = document.getElementById(
      symbolGroup.id
    );
    const paletteDiv: HTMLElement = createHtmlElement('div', {
      id: symbolGroup.id,
      style: style,
  ...
  public addPaletteItem(
    ...
    //let refresh: boolean;
    for (let i: number = 0; i < this.palettes.length; i++) {
      const symbolPaletteGroup: PaletteModel =
        this.palettes[parseInt(i.toString(), 10)];
      if (symbolPaletteGroup.id.indexOf(paletteName) !== -1) {
        // eslint-disable-next-line
        const param: any = [undefined, symbolPaletteGroup, 'symbols', {}, true];
        // eslint-disable-next-line
        const obj: any = new (Function.prototype.bind.apply(
          getObjectType(paletteSymbol),
          param
        ))();
        for (let i: number = 0; i < Object.keys(paletteSymbol).length; i++) {
          ...
          obj[Object.keys(paletteSymbol)[parseInt(i.toString(), 10)]] =
            paletteSymbol[
              Object.keys(paletteSymbol)[parseInt(i.toString(), 10)]
            ];
          this.allowServerDataBinding = isEnableServerDatabind;
        }
        updateDefaultValues(
          obj,
          paletteSymbol,
          obj instanceof Node ? this.nodeDefaults : this.connectorDefaults
        );
  ...
  private prepareSymbol(symbol: NodeModel | ConnectorModel): void {
    ...
    const symbolContainer: Canvas = new Canvas();
    const container: Container =
      symbol instanceof Node ? (symbol as Node).initContainer() : null;
    ...
    if (!content) {
      if (obj.children) {
        ...
        if (
          symbol instanceof Connector &&
          symbol.shape.type === 'UmlClassifier' &&
          (symbol.shape as RelationShipModel).relationship
        ) {
          content = (symbol as Connector).init(this);
          for (
            let i: number = (content as Canvas).children.length - 1;
            i >= 0;
            i--
          ) {
            if (
              (content as Canvas).children[
    ...
    if (!(symbol as Node | Connector).parentId) {
      ...
      width = symbolInfo.width =
        symbolInfo.width ||
        (obj.width !== undefined ? content.actualSize.width : undefined) ||
        this.symbolWidth;
      height = symbolInfo.height =
        symbolInfo.height ||
        (obj.height !== undefined ? content.actualSize.height : undefined) ||
        this.symbolHeight;
      if (width !== undefined && height !== undefined) {
        ...
        const isPhase: boolean = (symbol.shape as SwimLane).isPhase
          ? true
          : false;
        ...
        if (this.symbolHeight !== undefined) {
          actualHeight =
            this.symbolHeight -
            this.symbolMargin.top -
            this.symbolMargin.bottom;
        } else {
        ...
        if (
          symbol.shape &&
          symbol.shape.type !== 'Bpmn' &&
          symbolInfo.description &&
          symbolInfo.description.text !== ''
        ) {
        ...
        content.width =
          (symbol as Node).width || outerBounds
            ? outerBounds.width
            : content.actualSize.width;
        content.height =
          (symbol as Node).height || outerBounds
            ? outerBounds.height
            : content.actualSize.height;
      }
    ...
    for (let i: number = 0; i < child.length; i++) {
      if (this.symbolTable[child[parseInt(i.toString(), 10)]]) {
        container.children.push(
          this.symbolTable[child[parseInt(i.toString(), 10)]].wrapper
        );
      }
    }
    container.measure(new Size(obj.width, obj.height));
  ...
  private getSymbolDescription(
    ...
    width: number,
    parent: StackPanel | Container,
    symbol: NodeModel | ConnectorModel
  ): void {
    if (symbolInfo && symbolInfo.description && symbolInfo.description.text) {
      const textElement: TextElement = new TextElement();
      //symbol description-textElement
      symbolInfo.description.overflow =
        symbolInfo.description.overflow || 'Ellipsis';
      symbolInfo.description.wrap =
        symbolInfo.description.wrap || 'WrapWithOverflow';
      ...
      textElement.style.fontFamily =
        symbolInfo.description.fontFamily || 'Arial';
      ...
      textElement.style.textDecoration =
        symbolInfo.description.textDecoration || 'None';
      ...
      if (symbol.shape && symbol.shape.type === 'Bpmn') {
        textElement.margin = {
          left: 0,
          right: 0,
          top: symbolInfo.description.margin
            ? symbolInfo.description.margin.top
            : 0,
          bottom: symbolInfo.description.margin
            ? symbolInfo.description.margin.bottom
            : this.symbolMargin.bottom,
        };
      } else {
        textElement.margin = {
  ...
  private getSymbolPreview(
    ...
    let sw: number;
    let sh: number;
    let symbolPreviewWidth: number =
      symbol.wrapper.children[0].desiredSize.width + symbol.style.strokeWidth;
    let symbolPreviewHeight: number =
      symbol.wrapper.children[0].desiredSize.height + symbol.style.strokeWidth;
    const content: DiagramElement = (symbol.wrapper.children[0] as Container)
      .children[0];
    const symbolPreview: SymbolSizeModel = (symbol as Node).previewSize;
    if (
      (symbol && (symbolPreview.width || symbolPreview.height)) ||
      this.symbolPreview.width !== undefined ||
      this.symbolPreview.height !== undefined
    ) {
      symbolPreviewWidth =
        (symbolPreview.width ||
          this.symbolPreview.width ||
          symbolPreviewWidth) - symbol.style.strokeWidth;
      symbolPreviewHeight =
        (symbolPreview.height ||
          this.symbolPreview.height ||
          symbolPreviewHeight) - symbol.style.strokeWidth;
      ...
      this.scaleSymbol(
        symbol,
        symbol.wrapper.children[0] as Container,
        sw,
        sh,
        symbolWidth,
        symbolHeight,
    ...
    this.measureAndArrangeSymbol(content, symbol instanceof Node);
    const previewContainer: HTMLElement = createHtmlElement('div', {
      draggable: 'true',
      class: 'e-dragclone',
    ...
    if (symbol.shape.type === 'Native') {
      ...
      const gElement: SVGElement = createSvgElement('g', {
        id: symbol.id + '_g',
      });
      canvas.appendChild(gElement);
      previewContainer.appendChild(canvas);
      this.svgRenderer.renderElement(
        content,
        gElement,
        undefined,
        undefined,
      ...
      if (
        (symbol as NodeModel).children &&
        (symbol as NodeModel).children.length > 0 &&
        groupHasType(symbol as Node, 'HTML', this.childTable)
      ) {
        div = this.getHtmlSymbol(
          symbol,
          canvas,
          previewContainer,
          symbol.wrapper.actualSize.height,
          symbol.wrapper.actualSize.width,
          true
        );
        this.prepareSymbol(symbol);
      }
  ...
  private measureAndArrangeSymbol(
    ...
    content.arrange(content.desiredSize);
    if (
      (content as Container).children &&
      (content as Container).children.length > 0
    ) {
  ...
  private updateSymbolSize(
    ...
    const element: DiagramElement = (symbol.wrapper.children[0] as Container)
      .children[0];
  ...
  private getSymbolContainer(
    ...
    const symbolId: string =
      parentDiv.id === 'SearchPalette' ? symbol.id + 'SearchSymbol' : symbol.id;
    ...
    //This method is responsible for rendering the UML node, as the UML node is displayed as an HTML group node.
      ...
      if (
        ...
        canvas = CanvasRenderer.createCanvas(
          symbolId,
          Math.ceil(
            (symbol.wrapper.actualSize.width + symbol.style.strokeWidth) * 2
          ) + 1,
          Math.ceil(
            (symbol.wrapper.actualSize.height + symbol.style.strokeWidth) * 2
          ) + 1
        );
        container.appendChild(canvas);
        let index: number = 2;
    ...
    if (!preview) {
      const actualWidth: number =
        symbol.wrapper.actualSize.width + symbol.style.strokeWidth;
      const actualHeight: number =
        symbol.wrapper.actualSize.height + symbol.style.strokeWidth;
      let style: string =
        'pointer-events:none;transform-origin:0 0;overflow:hidden;';
      if ((symbol.shape as SwimLaneModel).isPhase) {
        if ((symbol.shape as SwimLaneModel).orientation === 'Horizontal') {
          ...
          style +=
            ...
            size.width / 2 +
            'px;margin-top:' +
            Math.max(this.symbolMargin.top, (height - actualHeight) / 2) +
            'px;';
        }
      } else {
        style +=
  ...
  private getGroupParent(
    ...
    const div: HTMLElement = createHtmlElement('div', {
      id: item.id + (isPreview ? '_html_div_preview' : '_html_div'),
    });
    const htmlLayer: HTMLElement = createHtmlElement('div', {
    ...
    canvas = CanvasRenderer.createCanvas(
      isPreview ? item.id + '_preview' : item.id,
      Math.ceil(width) + 1,
      Math.ceil(height) + 1
    );
    ...
    this.diagramRenderer.renderElement(
      (item.wrapper.children[0] as Container).children[0],
      canvas,
      htmlLayer
    );
    return div;
  ...
  private getHtmlSymbol(
    ...
    width: number,
    isPreview: boolean,
    isSearchSymbol?: boolean
  ): HTMLElement {
    //Bug 857673: Symbol palette tooltip is not rendered properly after search symbols and hover over palette shapes
    // If the shape is to be rendered in search palette, then the id of the shape is appended with 'SearchSymbol'
    // To create a unique id for the shape in search palette to avoid tooltip issue.
    const symbolId: string = isSearchSymbol
      ? symbol.id + 'SearchSymbol'
      : symbol.id;
    const div: HTMLElement = createHtmlElement('div', {
    ...
    if (isPreview) {
      ...
      if (symbol.shape.type === 'UmlClassifier') {
        if ((symbol.shape as UmlClassifierShapeModel).classifier === 'Class') {
          (
            ((symbol.wrapper.children[0] as Container).children[0] as Container)
              .children[0] as DiagramHtmlElement
          ).content =
            '<div id="parentDiv" style="width:100%; height:50%; border:1px solid #000; background:#6BA5D7; display: flex; align-items: center; justify-content: center;"><div id="textDiv" style="font-size:1vw;">class</div></div>';
        } else if (
          (symbol.shape as UmlClassifierShapeModel).classifier === 'Enumeration'
        ) {
          (
            ...
            '<div id="parentDiv2" style="width:100%; height:50%; border:1px solid #000; background:#6BA5D7; display: flex; align-items: center; justify-content: center;"><div id="textDiv2" style="font-size:0.8vw;">Enumeration</div></div>';
        } else if (
          (symbol.shape as UmlClassifierShapeModel).classifier === 'Interface'
        ) {
          (
            ...
            '<div id="parentDiv3" style="width:100%; height:50%; border:1px solid #000; background:#6BA5D7; display: flex; align-items: center; justify-content: center;"><div id="textDiv3" style="font-size:0.9vw;">Interface</div></div>';
        }
      }
      this.diagramRenderer.renderElement(
        (symbol.wrapper.children[0] as Container).children[0],
        canvas,
        htmlLayer
      );
    } else {
  ...
  private getSymbolSize(
    ...
    if (!this.symbolWidth && !this.symbolHeight) {
      width +=
        this.symbolMargin.left +
        this.symbolMargin.right +
  ...
  private getContent(obj: object): string | HTMLElement {
    ...
    const content: string | HTMLElement = isPrivateTooltip
  ...
  private initTooltip(element: NodeModel | ConnectorModel): Tooltip {
    ...
    if (!isBlazor()) {
      ...
      tooltipOption = this.updateTooltipContent(
  ...
  private elementEnter(
    ...
    if (!elementOver) {
      ...
      const targetId: string = isSearchSymbol
        ? this.hoverElement.id + 'SearchSymbol'
      ...
      if (
        ...
        (this.symbolTooltipObject as TooltipModel).opensOn = (
  ...
  private mouseMove(e: PointerEvent | TouchEvent, touches: TouchList): void {
    ...
    if (
      ...
      (id !== 'SearchPalette' &&
        e.target &&
        (e.target as HTMLElement).id.includes('SearchSymbol'))
    ) {
    ...
    if (
      obj === undefined &&
      this.hoverElement &&
      !this.hoverElement.tooltip.isSticky &&
      this.hoverElement.tooltip.openOn === 'Auto'
    ) {
  ...
  private elementLeave(): void {
    if (
      this.symbolTooltipObject &&
      (this.symbolTooltipObject as TooltipModel).opensOn !== 'Custom'
    ) {
    ...
    if (evt && evt.target) {
      if (evt.srcElement.id === 'iconSearch') {
        ...
        if (id && this.selectedSymbol) {
          const args: IPaletteSelectionChangeArgs = {
  ...
  private mouseDown(evt: PointerEvent): void {
    ...
    if (this.selectedSymbol) {
      const oldSymbol: HTMLElement = document.getElementById(
        this.selectedSymbol.id + '_container'
      );
      //Bug 857673: Symbol palette tooltip is not rendered properly after search symbols and hover over palette shapes
      // To highlight and remove highlight of the selected symbol in search palette on mouse down.
      const oldSearchSymbol: HTMLElement = document.getElementById(
        this.selectedSymbol.id + 'SearchSymbol' + '_container'
      );
    ...
    if (this.allowDrag) {
      ...
      this.draggable = new Draggable(this.element, {
        ...
        drag: (args: Object) => {
          ...
          const parent: Element = parentsUntil(
            args[`${target}`],
            'e-droppable'
          );
  ...
  private helper: Function = (e: {
    target: HTMLElement;
    sender: PointerEvent | TouchEvent;
  }) => {
    let clonedElement: HTMLElement;
    const id: string =
      (this.selectedSymbol && this.selectedSymbol.id) ||
      (e.sender.target as Element).id.split('_container')[0];
    const symbol: IElement = this.symbolTable[`${id}`];
    if (symbol && this.selectedSymbol) {
      this.selectedSymbols =
        this.selectedSymbol.id === (symbol as NodeModel | ConnectorModel).id
          ? symbol
          : this.selectedSymbol;
      //const position: PointModel = this.getMousePosition(e.sender);
      clonedElement = this.getSymbolPreview(
        this.selectedSymbols,
        e.sender,
        this.element
      );
  ...
  private scaleSymbol(
    ...
    if (symbol instanceof Connector) {
      ...
      const point: PointModel = symbol.scale(
        sw,
        sh,
        width,
        height,
        symbolContainer.children[0]
      );
      const difX: number =
        width / 2 -
        (symbolContainer.children[0] as Container).children[0].offsetX +
        point.x / 2;
      const difY: number =
        height / 2 -
        (symbolContainer.children[0] as Container).children[0].offsetY +
        point.y / 2;
      ...
      if ((symbol as Node).children) {
        ...
        if (!preview) {
          ...
          for (let i: number = 0; i < parentNode.length; i++) {
            container = (symbolContainer.children[0] as Container).children[
              parseInt(i.toString(), 10)
            ];
            if (container) {
              if (
                ((container as Container).children[0] as Container).children
              ) {
              ...
              container.arrange(
                (container as Container).children[0].desiredSize
              );
            }
          }
        }
        ...
        symbolContainer.children[0].arrange(
          symbolContainer.children[0].desiredSize
        );
        if (!preview) {
          let children: DiagramElement;
          for (let i: number = 0; i < parentNode.length; i++) {
            children = (symbolContainer.children[0] as Container).children[
              parseInt(i.toString(), 10)
            ];
            if (children) {
        ...
        if (preview) {
          ...
          for (let i: number = 0; i < parentNode.length; i++) {
            ...
            scaleHeight =
              height / symbol.wrapper.children[0].desiredSize.height;
            children = (symbolContainer.children[0] as Container).children[
              parseInt(i.toString(), 10)
            ];
            if (children) {
          ...
          symbol.wrapper.children[0].arrange(
            symbol.wrapper.children[0].desiredSize
          );
        }
      } else {
  ...
  private measureChild(container: DiagramElement): void {
    ...
    let node: NodeModel;
    for (let i: number = 0; i < (container as Container).children.length; i++) {
      childContainer = (container as Container).children[
        parseInt(i.toString(), 10)
      ];
      if (!((childContainer as Container).children[0] as Container).children) {
        ...
        childContainer.arrange(
          (childContainer as Container).children[0].desiredSize
        );
      } else {
  ...
  private scaleGroup(
    ...
    child.offsetX = preview
      ? child.offsetX * w - symbol.style.strokeWidth
      : child.offsetX * w + symbol.style.strokeWidth / 2;
    child.offsetY = preview
      ? child.offsetY * h - symbol.style.strokeWidth
      : child.offsetY * h + symbol.style.strokeWidth / 2;
  ...
  private createTextbox(): void {
    const searchDiv: HTMLElement = createHtmlElement('div', {
    ...
    const textBox: HTMLInputElement = createHtmlElement(
      'input',
      {}
    ) as HTMLInputElement;
    ...
    const span: HTMLElement = createHtmlElement('span', {
      id: 'iconSearch',
  ...
  private getFilterSymbol(
    ...
    items = symbol.filter(
      (element: NodeModel | ConnectorModel) =>
        !(this.ignoreSymbolsOnSearch as any).includes(element.id)
    );
    return items;
  }
  ...
  private searchPalette(value: string): void {
    ...
    for (let i: number = 0; i < this.palettes.length; i++) {
      ...
      for (let j: number = 0; j < symbolPaletteGroup.symbols.length; j++) {
        const item: NodeModel | ConnectorModel =
          symbolPaletteGroup.symbols[parseInt(j.toString(), 10)];
        if (
          value !== '' &&
    ...
    if (symbolGroup.length > 0) {
      ...
      const emptyDiv: HTMLElement = createHtmlElement('div', {
        id: 'EmptyDiv',
      ...
      if (!isBlazor()) {
        ...
        const searchPalette: HTMLElement =
          document.getElementById('SearchPalette');
        if (searchPalette) {
