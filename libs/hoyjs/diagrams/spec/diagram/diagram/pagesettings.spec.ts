...
} from '../../../src/diagram/objects/interface/IElement';
import {
  Matrix,
...
describe('PageSettings', () => {
  describe('Exception occurs when try to draw SVG node issue', () => {
    ...
    it('Draw Normal node and check nodes collection', function (done) {
      ...
      let diagramCanvas: Element = document.getElementById(
    ...
    it('Draw SVG node and check nodes collection', function (done) {
      ...
      expect(
        diagram.nodes.length === 2 && diagram.nodes[1].shape.type === 'Native'
      ).toBe(true);
      done();
    });
  });
  describe('Bezier connector not rendered properly issue', () => {
    ...
    it('Draw Bezier connector and check segment collection', function (done) {
      ...
      diagram.dataBind();
      expect(
        diagram.connectors.length === 1 &&
          diagram.connectors[0].segments.length === 2
      ).toBe(true);
      done();
    });
  });
  describe('Page Settings', () => {
    ...
    it('Checking swapping based on orientation', (done: Function) => {
      ...
      diagram.dataBind();
      expect(
        diagram.pageSettings.width === 600 && diagram.pageSettings.height == 300
      ).toBe(true);
      done();
    });
  });
  describe('BPMN Sequence connector', () => {
    ...
    it('BPMN Sequence default Rendering', (done: Function) => {
      let pathElement: HTMLElement =
        document.getElementById('Connector1_Default');
      console.log('first path', pathElement.getAttribute('transform'));
      expect(
        pathElement.getAttribute('transform') ===
          'rotate(45,707.07,107.07)translate(699.2900012207032,99.29000122070312)'
      ).toBe(true);
      done();
    });
    ...
    it('BPMN Sequence connector Dragging with node move', (done: Function) => {
      ...
      let pathElement: HTMLElement =
        document.getElementById('Connector2_Default');
      expect(
        pathElement.getAttribute('transform') ===
          'rotate(45,100,125)translate(100,114)'
      ).toBe(true);
      done();
    });
...
describe('PageSettings boundary constraints', () => {
  describe('Page Settings with orientation', () => {
    ...
    it('changing connector properties  at runtime', (done: Function) => {
      ...
      diagram.dataBind();
      expect(
        diagram.connectors[0].sourcePoint.x == 300 &&
          diagram.connectors[0].sourcePoint.y == 400
      ).toBe(true);
      done();
    });
  ...
  describe('Checking Page Break', () => {
    ...
    it('By moving node into negative axis', (done: Function) => {
      let pageArea: any = document.getElementById(
        'diagramconstraints_backgroundLayerrect'
      );
      let diagramCanvas: HTMLElement = document.getElementById(
        diagram.element.id + 'content'
      );
  ...
  describe('Boundary Constraints rotate angle issue', () => {
    ...
    beforeAll((): void => {
      ...
      diagram = new Diagram({
        width: '900',
        height: '900',
        nodes: [node], //connectors: connectors,
...
describe('Page Settings with orientation', () => {
  ...
  it('boundary constraints for drag and drop', (done: Function) => {
    ...
    mouseEvents.dragAndDropEvent(
      diagramCanvas,
      diagram.nodes[0].offsetX,
      diagram.nodes[0].offsetY,
      600,
      600
    );
  ...
  it('boundary constraints for drag and drop Native node', (done: Function) => {
    ...
    mouseEvents.dragAndDropEvent(
      diagramCanvas,
      diagram.nodes[1].offsetX,
      diagram.nodes[1].offsetY,
      200,
      600
    );
...
describe('Swimlane Resize functionality', () => {
  ...
  it('Check whether swimlane is resized properly or not ', (done: Function) => {
    ...
    let x =
      diagram.selectedItems.wrapper.bounds.x +
      diagram.selectedItems.wrapper.bounds.width / 2 +
      diagram.element.offsetLeft;
    let y =
      diagram.selectedItems.wrapper.bounds.y +
      diagram.selectedItems.wrapper.bounds.height +
      diagram.element.offsetTop;
...
describe('Connector Segment -Rotate', () => {
  ...
  it('Rotate the connector with segment and check points', (done: Function) => {
    ...
    diagram.rotate(diagram.selectedItems, 300);
    expect(
      diagram.connectors[1].sourcePoint.x > 355 &&
        diagram.connectors[1].sourcePoint.y > 174 &&
        diagram.connectors[1].targetPoint.x > 392 &&
        diagram.connectors[1].targetPoint.y > 37
    ).toBe(true);
    done();
  });
});
...
describe('Connector Annotation Displacement issue', () => {
  ...
  it('Change the connector displacement to 50', (done: Function) => {
    ...
    diagram.dataBind();
    expect(diagram.connectors[0].annotations[0].displacement.x === 50).toBe(
      true
    );
    done();
  });
...
describe('Bezier annotation bounds Issue', () => {
  ...
  it('Check the annotation bounds', (done: Function) => {
    expect(
      Math.round(diagram.connectors[0].wrapper.children[3].bounds.x) === 191 &&
        Math.round(diagram.connectors[0].wrapper.children[3].bounds.y) === 300
    ).toBe(true);
    done();
  });
  ...
  it('Drag the connector target end and check event argument', (done: Function) => {
    ...
    diagram.propertyChange = (args: IPropertyChangeEventArgs) => {
      ...
      if (oldValue.targetPoint) {
        ...
        expect(
          newValue.targetPoint.y === 320 || newValue.targetPoint.y === 330
        ).toBe(true);
        done();
      }
    };
    ...
    diagram.select([diagram.connectors[0]]);
    let resizeOptions: HTMLElement = document.getElementById(
...
describe('Distribute command not working properly issue', () => {
  ...
  it('Check node does not get changed position while use distribute command again', (done: Function) => {
    ...
    expect(
      diagram.nodes[1].offsetX === 150 &&
        diagram.nodes[1].offsetY === 150 &&
        diagram.nodes[2].offsetX === 250 &&
        diagram.nodes[2].offsetY === 250
    ).toBe(true);
    diagram.distribute('RightToLeft', diagram.nodes);
    expect(
      diagram.nodes[1].offsetX === 150 &&
...
describe('Connector gets crossed each other issue', () => {
  ...
  it('Check whether connector are arranged properly in layout', function (done) {
    ...
    let connector1: ConnectorModel = diagram.getObject(
      (node as Node).inEdges[0]
    );
    let connector2: ConnectorModel = diagram.getObject(
      (node as Node).inEdges[1]
    );
  ...
  it('Check whether Only ZIndex values updated in propertyChange event newValue argument - bringToFront', function (done) {
    ...
    diagram.propertyChange = (args: IPropertyChangeEventArgs) => {
      if (args.newValue && (args.newValue as any).zIndex !== undefined) {
        ...
        expect(
          (args.newValue as any).zIndex === 4 ||
            (args.newValue as any).zIndex === 3 ||
            (args.newValue as any).zIndex === 2 ||
            (args.newValue as any).zIndex === 1 ||
            (args.newValue as any).zIndex === 0 ||
            (args.newValue as any).zIndex === 5
        ).toBe(true);
        expect(
          (args.oldValue as any).zIndex === 0 ||
            (args.oldValue as any).zIndex === 1 ||
            (args.oldValue as any).zIndex === 2 ||
            (args.oldValue as any).zIndex === 3 ||
            (args.oldValue as any).zIndex === 4
        ).toBe(true);
      }
    };
    diagram.bringToFront();
...
describe('BPMN text annotation not dragged properly issue', () => {
  ...
  it('Check whether BPMN text node update properly while drag the swimlane', function (done) {
    ...
    expect(
      Math.round(node.offsetX) === 36 && Math.round(node.offsetY) === 330
    ).toBe(true);
    done();
  });
});
...
describe('Resize handle not rendered properly issue', () => {
  ...
  it('Check whether resize handle renders properly for node pivot - 1, 1', function (done) {
    ...
    let southEastelement: HTMLElement =
      document.getElementById('resizeSouthEast');
    expect(
      northWestelement.getAttribute('x') === '193' &&
        northWestelement.getAttribute('y') === '93'
    ).toBe(true);
    expect(
      northEastelement.getAttribute('x') === '293' &&
        northEastelement.getAttribute('y') === '93'
    ).toBe(true);
    expect(
      southWestelement.getAttribute('x') === '193' &&
        southWestelement.getAttribute('y') === '193'
    ).toBe(true);
    expect(
      southEastelement.getAttribute('x') === '293' &&
        southEastelement.getAttribute('y') === '193'
    ).toBe(true);
    done();
  });
  ...
  it('Check whether resize handle renders properly after rotate the node', function (done) {
    ...
    expect(
      northEastelement.getAttribute('x') === '93' &&
        northEastelement.getAttribute('y') === '193'
    ).toBe(true);
    expect(
      southWestelement.getAttribute('x') === '-7' &&
        southWestelement.getAttribute('y') === '93'
    ).toBe(true);
    expect(
      southEastelement.getAttribute('x') === '-7' &&
        southEastelement.getAttribute('y') === '193'
    ).toBe(true);
    done();
  });
...
describe('Unable to drag connector end thumb and resize node handler when we increase the size of handleSize property', () => {
  ...
  it('Check whether the connector position is same or not after we dragging it', (done: Function) => {
    ...
    expect(true).toBe(true);
    expect(
      diagram.connectors[0].targetPoint.y == 200 ||
        diagram.connectors[0].targetPoint.y == 192
    ).toBe(true);
    done();
  });
...
describe('Unable to drag bezier connector control thumb while increasing the handleSize', () => {
  ...
  beforeAll((): void => {
    ...
    let connector1: ConnectorModel = {
      ...
      segments: [
        { type: 'Bezier', point: { x: 750, y: 250 } },
        { type: 'Bezier', point: { x: 900, y: 350 } },
      ],
    };
    diagram = new Diagram({
      width: '1000px',
  ...
  it('Unable to drag bezier connector control thumb while increasing the handleSize', (done: Function) => {
    ...
    expect(
      (diagram.connectors[0].segments[0] as BezierSegment).bezierPoint2.x == 750
    ).toBe(true);
    //Need to evaluate testcase
