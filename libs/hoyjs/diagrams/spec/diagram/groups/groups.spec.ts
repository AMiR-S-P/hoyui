...
import { MenuItemModel } from '@hoyui/navigations';
...
describe('Group', () => {
  describe('Group', () => {
    ...
    it('Group With Default', (done: Function) => {
      ...
      //check size too
      expect(
        diagram.selectedItems.nodes[0].wrapper.offsetX === 400 &&
          diagram.selectedItems.nodes[0].wrapper.offsetY === 300
      ).toBe(true);
      done();
    });
    ...
    it('Drag the group', (done: Function) => {
      ...
      expect(
        diagram.selectedItems.nodes[0].wrapper.offsetX === 450 &&
          diagram.selectedItems.nodes[0].wrapper.offsetY === 250
      ).toBe(true);
      done();
      diagram.undo();
    });
    it('Background color for the group', (done: Function) => {
      ...
      expect(diagram.selectedItems.nodes[0].backgroundColor).toBe(
        'transparent'
      );
      done();
      diagram.clearSelection();
  ...
  describe('Group With context Menu', () => {
    ...
    it('Context Menu - Group', (done: Function) => {
      ...
      (diagram.contextMenuModule as any).eventArgs = {
        target: document.getElementById('diagramdraw_diagramAdorner_svg'),
      };
      let e = {
        event: (diagram.contextMenuModule as any).eventArgs,
        items: (diagram.contextMenuModule.contextMenu.items[6] as MenuItemModel)
          .items,
      };
    ...
    it('Ungroup - Internal Group', (done: Function) => {
      ...
      let groupElement: HTMLElement = document.getElementById(
        internalGroup.id + '_groupElement'
      );
  ...
  describe('Group with annnotation', () => {
    ...
    it('Group With Default along with annotation rotate', (done: Function) => {
      ...
      expect(
        Math.round(
          diagram.selectedItems.nodes[0].wrapper.children[0].parentTransform
        ) === 331 ||
          Math.round(
            diagram.selectedItems.nodes[0].wrapper.children[0].parentTransform
          ) === 332 ||
    ...
    it('Rotated Group', (done: Function) => {
      ...
      expect(
        Math.round(diagram.nodes[diagram.nodes.length - 2].rotateAngle) == 16 ||
          Math.round(diagram.nodes[diagram.nodes.length - 2].rotateAngle) ===
            17 ||
          Math.round(diagram.nodes[diagram.nodes.length - 2].rotateAngle) == 18
      ).toBe(true);
      done();
    });
  });
  describe('Group with crash issue', () => {
    ...
    it('Drag GroupPort and drag child node of the group ', (done: Function) => {
      ...
      mouseEvents.dragAndDropEvent(
        diagramCanvas,
        (bounds as DOMRect).x + (bounds as DOMRect).width / 2,
        (bounds as DOMRect).y + (bounds as DOMRect).height / 2,
        70,
        170
      );
      console.log(
        'diagram.nodes[2].ports[0].offset.x: +' +
          diagram.nodes[2].ports[0].offset.x
      );
      console.log(
        'diagram.nodes[2].ports[0].offset.y: +' +
          diagram.nodes[2].ports[0].offset.y
      );
      expect(
        diagram.nodes[2].ports[0].offset.x === 0.02666666666666667 &&
          diagram.nodes[2].ports[0].offset.y === 0.06666666666666667
      ).toBe(true);
      mouseEvents.clickEvent(diagramCanvas, 10, 10);
      mouseEvents.clickEvent(diagramCanvas, 400, 400);
      mouseEvents.clickEvent(diagramCanvas, 400, 400);
      ...
      console.log(
        'element.attributes[2].value: ' + element.attributes[2].value
      );
      expect(
        element.attributes[2].value ===
          'rotate(0,562.5,520.5)translate(556.5,514.5)'
      ).toBe(false);
      done();
    });
  });
  describe('Group - width and height', () => {
    ...
    beforeAll((): void => {
      ...
      let nodes: NodeModel[] = [
        ...
        },
        {
          ...
          children: ['node1', 'node2'],
          style: { fill: 'gray', strokeColor: 'black' },
          rotateAngle: 0,
          width: 40,
          height: 40,
        },
    ...
    it('Checking group have width and height in rendering', (done: Function) => {
      expect(
        Math.round(diagram.nodes[0].width) === 20 &&
          Math.round(diagram.nodes[0].height) === 20 &&
          Math.round(diagram.nodes[1].width) === 20 &&
          Math.round(diagram.nodes[1].height) === 20 &&
          diagram.nodes[2].width === 40 &&
          diagram.nodes[2].height === 40 &&
    ...
    it('Update group width and height at run time', (done: Function) => {
      ...
      diagram.dataBind();
      expect(
        Math.round(diagram.nodes[0].width) === 50 &&
          Math.round(diagram.nodes[0].height) === 50 &&
          Math.round(diagram.nodes[1].width) === 50 &&
          Math.round(diagram.nodes[1].height) === 50 &&
          diagram.nodes[2].width === 100 &&
    ...
    it('group property issue and save and load isse', (done: Function) => {
      ...
      expect(
        group.attributes[15].value === 'gray' &&
          group.attributes[12].value === 'black'
      ).toBe(true);
  ...
  describe('Group - width and height', () => {
    ...
    it('Checking group have width and height in rendering', (done: Function) => {
      expect(
        diagram.nodes[3].width == 50 &&
          diagram.nodes[3].height == 120 &&
          diagram.nodes[3].offsetX == 750 &&
          diagram.nodes[3].offsetY == 135
      ).toBe(true);
      ...
      diagram.dataBind();
      expect(
        diagram.nodes[3].offsetX == 733.3299999999999 &&
          diagram.nodes[3].offsetY == 135
      ).toBe(true);
      done();
    });
    it('Checking group have width and height in rendering', (done: Function) => {
      ...
      let selectorElement = document.getElementById(
        'diagram_group_group_width_height_SelectorElement'
      );
      var children = selectorElement.children[3] as HTMLElement;
      expect(
        children.getAttribute('cx') === '699.9966666666667' ||
          children.getAttribute('x') === '692.9966666666667'
      ).toBe(true);
  ...
  describe('Performance Fix break issue', () => {
    ...
    it('Checking node drag and changing the text element by double click', (done: Function) => {
      var diagramCanvas = document.getElementById(
        diagram.element.id + 'content'
      );
      ...
      var temp = document.getElementById(
        diagram.nodes[3].wrapper.children[1].id + '_groupElement'
      );
      expect(temp.children[0].getAttribute('fill') === 'transparent').toBe(
        true
      );
      var shape = document.getElementById(
        diagram.nodes[3].wrapper.children[0].id + '_groupElement'
      );
      expect(
        shape.children[0].getAttribute('d') === 'M50,0 L100,100 L0,100 L50,0 Z '
      ).toBe(true);
      let left: number;
      let top: number;
      ...
      mouseEvents.clickEvent(
        diagramCanvas,
        annotation.offsetX + left,
        annotation.offsetY + top
      );
      diagram.nudge('Right');
      done();
    });
    it('Checking source and target decorator color', (done: Function) => {
      let diagramCanvas = document.getElementById(
        diagram.element.id + 'content'
      );
      ...
      var element = document.getElementById(
        (connector as Connector).id + '_tarDec_groupElement'
      );
    ...
    it('copy and paste group issue', (done: Function) => {
      ...
      //Need to evaluate testcase
      var element = document.getElementById(
        (node as Node).id + '_content_groupElement'
      );
  ...
  describe('Resize handle not rendered properly issue', () => {
    ...
    it('Check whether resize handle renders properly for node pivot - 0, 0', function (done) {
      ...
      let northWestelement: HTMLElement =
        document.getElementById('resizeNorthWest');
      let northEastelement: HTMLElement =
        document.getElementById('resizeNorthEast');
      let southWestelement: HTMLElement =
        document.getElementById('resizeSouthWest');
      let southEastelement: HTMLElement =
        document.getElementById('resizeSouthEast');
      expect(
        northWestelement.getAttribute('x') === '93' &&
          northWestelement.getAttribute('y') === '93'
      ).toBe(true);
      expect(
        northEastelement.getAttribute('x') === '193' &&
          northEastelement.getAttribute('y') === '93'
      ).toBe(true);
      expect(
        southWestelement.getAttribute('x') === '93' &&
          southWestelement.getAttribute('y') === '193'
      ).toBe(true);
      expect(
        southEastelement.getAttribute('x') === '193' &&
          southEastelement.getAttribute('y') === '193'
      ).toBe(true);
      done();
    });
  });
  describe('Group - with selection issue', () => {
    ...
    it('Checking group selection issue', (done: Function) => {
      ...
      diagram.dataBind();
      expect(
        Math.round(diagram.nodes[3].wrapper.bounds.left) == 690 &&
          Math.round(diagram.nodes[3].wrapper.bounds.right) == 810
      ).toBe(true);
      done();
    });
  });
    ...
    beforeAll((): void => {
      ...
      let nodes: NodeModel[] = [
        ...
        },
        {
          ...
          offsetX: 300,
          offsetY: 300,
          style: { fill: 'orange', strokeColor: 'red' },
        },
      ];
  ...
  describe('Group width and height does not update', () => {
    ...
    it('When child is added to a group', (done: Function) => {
      ...
      expect(
        diagram.nodes[2].height === 300 &&
          diagram.nodes[2].width === 350 &&
          diagram.nodes[2].offsetX === 225 &&
          diagram.nodes[2].offsetY === 200
      ).toBe(true);
      done();
    });
  ...
  describe('Group width and height does not update', () => {
    ...
    it('Dragging issue - group have a one of conector as a child', (done: Function) => {
      ...
      let oldOffsetX = group.offsetX;
      let oldOffsetY = group.offsetY;
      mouseevents.clickEvent(
        diagramCanvas,
        group.offsetX + diagram.element.offsetLeft,
        group.offsetY + diagram.element.offsetTop
      );
      //Need to evaluate testcase
      ...
      mouseevents.mouseMoveEvent(
        diagramCanvas,
        group.offsetX + diagram.element.offsetLeft + 500,
        group.offsetY + diagram.element.offsetTop + 100
      );
      mouseevents.mouseUpEvent(
        diagramCanvas,
        group.offsetX + diagram.element.offsetLeft + 500 + 10,
        group.offsetY + diagram.element.offsetTop + 100
      );
      let newOffsetX = group.offsetX;
      let newOffsetY = group.offsetY;
      expect(newOffsetX != oldOffsetX && newOffsetY != oldOffsetY).toBe(false);
      diagram.clearSelection();
      expect(diagram.selectedItems.nodes.length == 0).toBe(true);
      diagram.undo();
      expect(group.offsetX == oldOffsetX && group.offsetY == oldOffsetY).toBe(
        true
      );
      diagram.redo();
      expect(group.offsetX == newOffsetX && group.offsetY == newOffsetY).toBe(
        true
      );
      done();
    });
    it('AddChildToGroup method', (done: Function) => {
      ...
      diagram.undo();
      expect(group.children.length == 2 && diagram.nodes.length == 2).toBe(
        true
      );
      diagram.redo();
...
describe('change styles of group', () => {
  ...
  it('applying styles for the group in runtime', (done: Function) => {
    ...
    applyStyle(
      diagram.selectedObject.actualObject,
      2,
      '5,5',
      undefined,
      undefined,
    ...
    expect(
      diagram.selectedItems.nodes[0].wrapper.children[1].style.fill ===
        'transparent'
    ).toBe(true);
    done();
  });
...
describe('Check connector when in grouping ', () => {
  ...
  beforeAll((): void => {
    ...
    connector.segments = [
      { type: 'Straight', direction: 'Right', length: 70 },
      { type: 'Straight', direction: 'Bottom', length: 20 },
    ];
...
describe('Group Node padding', () => {
  ...
  it('Group node initial rendering without padding', (done: Function) => {
    expect(
      diagram.nodes[3].height == 150 &&
        ...
        diagram.nodes[3].padding.left == 0
    ).toBe(true);
  ...
  it('Group node initial rendering with padding', (done: Function) => {
    ...
    expect(
      diagram.nodes[3].height == 190 &&
        diagram.nodes[3].width == 170 &&
        padding.left == 10 &&
        padding.top == 20
    ).toBe(true);
  ...
  it('Rotating group node with padding', (done: Function) => {
    ...
    mouseEvents.clickEvent(
      diagramCanvas,
      diagram.nodes[3].offsetX,
      diagram.nodes[3].offsetY,
      true
    );
  ...
  it('Dragging group node with padding', (done: Function) => {
    ...
    console.log(
      prevOffsetX + ' Dragging group node with padding ' + curOffsetX
    );
    done();
  });
  ...
  it('Resize group node with padding', (done: Function) => {
    ...
    let padding = diagram.nodes[3].padding;
    expect(
      preWidth !== curWidth && padding.bottom == 20 && padding.right == 10
    ).toBe(true);
    console.log(
      'Before resizing :' +
        preWidth +
  ...
  it('Grouping in runtime and drag and rotate with and without padding', (done: Function) => {
    ...
    expect(
      preWidth !== curWidth &&
        ...
        diagram.nodes[4].rotateAngle == 40 &&
        diagram.nodes[4].padding.bottom == 10
    ).toBe(true);
    console.log(
      'without padding ' +
        preWidth +
...
describe('UnGroup Issue - EJ2-66928', () => {
  describe('checks Ungrouping Performing Correctly and Grouping Ports are Removed ', () => {
    ...
    it('Ungroup and remove Ports', (done: Function) => {
      ...
      diagram.unGroup();
      let portElementId_1: HTMLElement = document.getElementById(
        addedPorts.id + '_g_port1'
      );
      let portElementId_2: HTMLElement = document.getElementById(
        addedPorts.id + '_g_port2'
      );
      let portElementId_3: HTMLElement = document.getElementById(
        addedPorts.id + '_g_port3'
      );
      let portElementId_4: HTMLElement = document.getElementById(
        addedPorts.id + '_g_port4'
      );
    ...
    it('Add Ports to the selected single node', (done: Function) => {
      ...
      let portElementId_1: HTMLElement = document.getElementById(
        addedPorts.id + '_runtimeport_1'
      );
      let portElementId_2: HTMLElement = document.getElementById(
        addedPorts.id + '_runtimeport_2'
      );
    ...
    it('Add and remove ports for single group node ', (done: Function) => {
      ...
      let addedPorts = diagram.nodes[11];
      let portElementId_1: HTMLElement = document.getElementById(
        addedPorts.id + '_group_port_1'
      );
      let portElementId_2: HTMLElement = document.getElementById(
        addedPorts.id + '_group_port_2'
      );
      expect(portElementId_1).toBeNull;
      expect(portElementId_2).toBeNull;
      done();
...
describe('Copy Paste the child node of group outside the group', () => {
  ...
  it('Check whether the copied child node parentid is empty', (done: Function) => {
    ...
    diagram.copy();
    diagram.paste();
    expect((diagram.selectedItems.nodes[0] as any).parentId === '').toBe(true);
    done();
  });
});
describe('Provide support to delete child from group', () => {
  ...
  beforeAll((): void => {
    ...
    let nodes: any = [
      ...
      {
        id: 'group3',
        children: ['node3', 'node4', 'connector3'],
      },
      {
        id: 'node5',
        width: 100,
      ...
      {
        id: 'group4',
        children: ['node4', 'node5', 'connector4'],
      },
    ];
    let connectors: any = [
      {
...
describe('880811-grouping child nodes with addChildToGroup method does not push child to group at DOM Level', () => {
  ...
  it('Group the node and connector', (done: Function) => {
    ...
    expect((diagram.selectedItems.nodes[0] as any).children.length === 3).toBe(
      true
    );
    done();
  });
...
describe('Ungrouping group node with connector', () => {
  ...
  beforeAll((): void => {
    ...
    let nodes: NodeModel[] = [
      ...
      {
        id: 'group10',
        children: ['transaction_com', 'node2'],
      },
      {
        id: 'node4',
        width: 50,
  ...
  beforeAll((): void => {
    ...
    let nodes: NodeModel[] = [
      ...
      },
      {
        ...
        annotations: [
          { content: 'Group' },
          { content: '222', offset: { x: 0.5, y: 0.8 } },
          { content: '111', offset: { x: 0.8, y: 0.2 } },
        ],
        style: { fill: 'green' },
      },
    ];
