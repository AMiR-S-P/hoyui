...
describe('Diagram Control', () => {
  describe('Annotation property change', () => {
    ...
    it('Checking annotation width and height change', (done: Function) => {
      ...
      diagram.dataBind();
      let label: ShapeAnnotationModel = (diagram.nodes[0] as NodeModel)
        .annotations[0];
      expect(label.visibility == false).toBe(true);
      ((diagram.nodes[0] as NodeModel).annotations[0].visibility = true),
        diagram.dataBind();
      expect(
        label.width == 100 && label.height == 60 && label.visibility == true
      ).toBe(true);
      done();
    });
    it('Checking annotation content change', (done: Function) => {
      ...
      diagram.dataBind();
      let label: ShapeAnnotationModel = (diagram.nodes[1] as NodeModel)
        .annotations[0];
    ...
    it('Checking annotation Alignment change', (done: Function) => {
      (diagram.nodes[2] as NodeModel).annotations[0].horizontalAlignment =
        'Right';
      (diagram.nodes[2] as NodeModel).annotations[0].verticalAlignment =
        'Bottom';
      diagram.dataBind();
      let label: ShapeAnnotationModel = (diagram.nodes[2] as NodeModel)
        .annotations[0];
      expect(
    ...
    it('Checking annotation style change', (done: Function) => {
      ...
      (diagram.nodes[3] as NodeModel).annotations[0].style.strokeDashArray =
        '5,5';
      ...
      (diagram.nodes[3] as NodeModel).annotations[0].style.fontFamily =
        'Fantasy';
      (diagram.nodes[3] as NodeModel).annotations[0].style.whiteSpace =
        'CollapseSpace';
      (diagram.nodes[3] as NodeModel).annotations[0].style.textWrapping =
        'WrapWithOverflow';
      (diagram.nodes[3] as NodeModel).annotations[0].style.textAlign = 'Left';
      (diagram.nodes[3] as NodeModel).annotations[0].style.textDecoration =
        'Overline';
      diagram.dataBind();
      let label: ShapeAnnotationModel = (diagram.nodes[3] as NodeModel)
        .annotations[0];
      expect(
        label.style.textWrapping == 'WrapWithOverflow' &&
    ...
    it('Checking without label', (done: Function) => {
      ...
      (diagram.nodes[4] as NodeModel).annotations[0].constraints =
