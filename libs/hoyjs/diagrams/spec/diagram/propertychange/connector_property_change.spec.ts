...
import {
  ...
  BpmnFlowModel,
  ConnectorShapeModel,
} from '../../../src/diagram/objects/connector-model';
...
describe('Diagram Control', () => {
  describe('Connetor property change', () => {
    ...
    beforeAll((): void => {
      ...
      diagram = new Diagram({
        width: 600,
        height: 500,
        connectors: [connector1, connectors2],
      });
    ...
    it('Checking connector source point change', (done: Function) => {
      ...
      let connector: ConnectorModel = diagram.connectors[0];
      expect(
        connector.sourcePoint.x === 300 && connector.sourcePoint.y === 300
      ).toBe(true);
      done();
    });
    it('Checking connector target point change', (done: Function) => {
      ...
      let connector: ConnectorModel = diagram.connectors[0];
      expect(
        connector.targetPoint.x === 550 && connector.targetPoint.y === 200
      ).toBe(true);
      done();
    });
    ...
    it('Checking connector target Decorator change', (done: Function) => {
      ...
      expect(
        connector.targetDecorator.style.fill === 'red' &&
          connector.targetDecorator.shape === 'Diamond' &&
          connector.targetDecorator.width === 15
      ).toBe(true);
      done();
    });
    it('Checking connector source decorator change', (done: Function) => {
      ...
      expect(
        connector.sourceDecorator.shape === 'Diamond' &&
          connector.targetDecorator.style.strokeWidth === 4
      ).toBe(true);
      done();
    });
    it('Checking connector annotation', (done: Function) => {
      let mouseEvents: MouseEvents = new MouseEvents();
      let label: PathAnnotationModel = (diagram.connectors[1] as ConnectorModel)
        .annotations[1];
      expect(label.visibility == false).toBe(true);
      ((diagram.connectors[1] as ConnectorModel).annotations[1].visibility =
        true),
        diagram.dataBind();
    ...
    it('Checking connector type change to bezier', (done: Function) => {
      ...
      let connector: ConnectorModel = diagram.connectors[0];
      expect(
        (connector.wrapper.children[0] as PathModel).data ===
          'M300.5 300C360.46 299.81 489.53999999999996 200.19 549.5 200'
      ).toBe(true);
      done();
    });
  });
  ...
  describe('Connetor property change ', () => {
    ...
    it('Checking connector source port change', (done: Function) => {
      ...
      diagram.connectors[0].sourcePortID = (
        diagram.nodes[2] as NodeModel
      ).ports[0].id;
      diagram.dataBind();
      let connector: ConnectorModel = diagram.connectors[0];
      expect(
        connector.sourcePortID === (diagram.nodes[2] as NodeModel).ports[0].id
      ).toBe(true);
      done();
    });
    it('Checking connector target port change', (done: Function) => {
      ...
      diagram.connectors[0].targetPortID = (
        diagram.nodes[0] as NodeModel
      ).ports[0].id;
      diagram.dataBind();
      let connector: ConnectorModel = diagram.connectors[0];
      expect(
        connector.targetPortID === (diagram.nodes[0] as NodeModel).ports[0].id
      ).toBe(true);
      done();
    });
  ...
  describe('Connetor property change ', () => {
    ...
    it('Checking connector source node and target node id change and same port does not be changed', (done: Function) => {
      console.log(
        'Checking connector source node and target node id change and same port does not be changed'
      );
      diagram.connectors[0].sourceID = 'node3';
      diagram.connectors[0].targetID = 'node1';
      diagram.dataBind();
      expect(
        (diagram.connectors[0] as Connector).intermediatePoints[0].x == 450 &&
          (diagram.connectors[0] as Connector).intermediatePoints[0].y == 250 &&
          (diagram.connectors[0] as Connector).intermediatePoints[1].x == 30 &&
          (diagram.connectors[0] as Connector).intermediatePoints[1].y == 250 &&
          (diagram.connectors[0] as Connector).intermediatePoints[2].x == 30 &&
          (diagram.connectors[0] as Connector).intermediatePoints[2].y == 100 &&
          (diagram.connectors[0] as Connector).intermediatePoints[3].x == 50 &&
          (diagram.connectors[0] as Connector).intermediatePoints[3].y == 100
      ).toBe(true);
      done();
    });
  });
