...
describe('Diagram Control', () => {
  ...
  describe('BPMN Text Annotations', () => {
      ...
      let node: NodeModel[] = [
        ...
        },
        {
          ...
          shape: {
            ...
            dataObject: { collection: false, type: 'Input' },
            annotations: [
              { id: 'left', angle: 170, length: 150, text: 'Left' },
    ...
    it('Checking annotation dragging in the same direction', (done: Function) => {
      ...
      expect(
        Math.round(node2.offsetX) === 270 && Math.round(node2.offsetY) === 140
      ).toBe(true);
      done();
    });
    it('Checking annotation dragging in the oppsite direction', (done: Function) => {
      ...
      console.log('offsetY', Math.round(node2.offsetY));
      expect(
        Math.round(node2.offsetX) === 250 && Math.round(node2.offsetY) > 250
      ).toBe(true);
      done();
    });
    it('Checking dragging a BPMN Shape with annotations', (done: Function) => {
      ...
      diagram.select([node]);
      let oldTextNodeOffsetX =
        diagram.nameTable[
          (diagram.selectedItems.nodes[0].shape as BpmnShape).annotations[0].id
        ].offsetX;
      let oldTextNodeOffsetY =
        diagram.nameTable[
          (diagram.selectedItems.nodes[0].shape as BpmnShape).annotations[0].id
        ].offsetY;
      diagram.select([node]);
      ...
      let newTextNodeOffsetX =
        diagram.nameTable[
          (diagram.selectedItems.nodes[0].shape as BpmnShape).annotations[0].id
        ].offsetX;
      let newTextNodeOffsety =
        diagram.nameTable[
          (diagram.selectedItems.nodes[0].shape as BpmnShape).annotations[0].id
        ].offsetY;
      expect(
        oldTextNodeOffsetX !== newTextNodeOffsetX &&
          oldTextNodeOffsetY > newTextNodeOffsety
      ).toBe(true);
      done();
    });
    ...
    it('Clicking at connector to select annotation', (done: Function) => {
      ...
      expect(
        diagram.selectedItems.nodes.length == 1 &&
          diagram.selectedItems.nodes[0].id == 'annot4'
      ).toBe(true);
      ...
      let node: NodeModel[] = [
        ...
        },
        {
          ...
          shape: {
            ...
            shape: 'TextAnnotation',
            textAnnotation: {
              textAnnotationDirection: 'Left',
              textAnnotationTarget: 'start',
            },
          } as BpmnShapeModel,
        },
        {
        ...
        },
        {
          ...
          shape: {
            ...
            shape: 'TextAnnotation',
            textAnnotation: {
              textAnnotationDirection: 'Bottom',
              textAnnotationTarget: 'start',
            },
          } as BpmnShapeModel,
        },
  ...
  describe('BPMN Text Annotations Interactions', () => {
    ...
    it('Checking drag of text annotation without parent node', (done: Function) => {
      ...
      expect(
        oldOffset !== newOffset &&
          oldOffset === 600 &&
          oldSourcePointX > newSourcePointX
      ).toBe(true);
      done();
    });
    ...
    it('Checking resizing of text annotation node with parent', (done: Function) => {
      ...
      mouseEvents.mouseDownEvent(
        diagramCanvas,
        bounds.middleLeft.x + 2,
        bounds.middleLeft.y + 2
      );
      mouseEvents.mouseMoveEvent(
        diagramCanvas,
        bounds.middleLeft.x - 10,
        bounds.middleLeft.y
      );
      mouseEvents.mouseMoveEvent(
        diagramCanvas,
        bounds.middleLeft.x - 20,
        bounds.middleLeft.y
      );
    ...
    it('Editing text annotation node content dynamically', (done: Function) => {
      ...
      expect(
        newTextContent === 'updated' && oldTextContent !== newTextContent
      ).toBe(true);
      done();
    });
    ...
    it('Adding text annotation node dynamically and checking node and connector collections', (done: Function) => {
      let newNode: NodeModel = {
        ...
        offsetX: 100,
        offsetY: 100,
        annotations: [{ content: 'newNodeText' }],
        shape: {
          type: 'Bpmn',
          shape: 'TextAnnotation',
          textAnnotation: {
      ...
      expect(
        oldNodesLength !== newNodesLength &&
          oldConLength !== newConLength &&
          newNodesLength === oldNodesLength + 1 &&
          newConLength === oldConLength + 1
      ).toBe(true);
      diagram.undo();
      expect(
        oldNodesLength === diagram.nodes.length &&
          oldConLength === diagram.connectors.length
      ).toBe(true);
      done();
    });
    ...
    it('Copy, paste text annotation node', (done: Function) => {
      ...
      diagram.paste();
      expect(
        oldNodeLength !== diagram.nodes.length &&
          oldConnectorLength !== diagram.connectors.length
      ).toBe(true);
      done();
    });
  });
  ...
  describe('BPMN Text Annotation throws error when start Text Edit', () => {
    ...
    it('Checking Text Annotation node textBox by double clicking the node', (done: Function) => {
      ...
      expect(
        node.annotations.length > 0 && node.annotations[0].content === ''
      ).toBe(true);
  ...
  describe('Bug 892767-Unable to update BPMN text annotation direction dynamically', () => {
      ...
      let nodes: NodeModel[] = [
        ...
        },
        {
          ...
          shape: {
            type: 'Bpmn',
            shape: 'TextAnnotation',
            textAnnotation: { textAnnotationDirection: 'Left' },
          },
        },
      ];
      diagram = new Diagram({
    ...
    it('Changing text annotation direction at runtime', (done: Function) => {
      ...
      (
        textAnnotation.shape as BpmnShape
      ).textAnnotation.textAnnotationDirection = 'Right';
      diagram.dataBind();
      let path = document
        .getElementById('node1_textannotation_path_groupElement')
