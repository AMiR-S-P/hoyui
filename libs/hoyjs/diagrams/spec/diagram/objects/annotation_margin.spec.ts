...
describe('Diagram Control', () => {
  describe('Annotations with margin', () => {
    ...
    let pathData: string =
      'M540.3643,137.9336L546.7973,159.7016L570.3633,159.7296L550.7723,171.9366L558.9053,' +
    ...
    beforeAll((): void => {
      ...
      annotation = {
        content: 'top left margin, top left alignment, offset 0',
        offset: { x: 0, y: 0 },
      ...
      annotation = {
        content: 'bottom right margin bottom right alignment, offset 1',
        offset: { x: 1, y: 1 },
      ...
      annotation = {
        content: 'top margin, offset 0, top left alignment',
        offset: { x: 0, y: 0 },
        horizontalAlignment: 'Left',
        verticalAlignment: 'Top',
        margin: { top: 15 },
      ...
      diagram = new Diagram({
        ...
        width: '800px',
        height: '800px',
        nodes: [node, node2, node3, node4, node6],
      });
    ...
    it('Checking different labels margin with different values in SVG rendering Mode', (done: Function) => {
      ...
      console.log(
        (diagram.nodes[0] as NodeModel).wrapper.children[1].actualSize.width
      );
      console.log(
        (diagram.nodes[0] as NodeModel).wrapper.children[1].actualSize.height
      );
      ...
      expect(
        diagram.nodes[0].wrapper.children[1].actualSize.width === 80.375 &&
          diagram.nodes[0].wrapper.children[1].actualSize.height ===
            57.599999999999994 &&
          diagram.nodes[0].wrapper.children[1].offsetX === 105.1875 &&
          diagram.nodes[0].wrapper.children[1].offsetY === 88.8
      ).toBe(false);
      done();
    });
    it('annotation OverFlow issue and node readonly issue', (done: Function) => {
      ...
      let annotationBounds = diagram.getWrapper(
        nodes.wrapper,
        nodes.annotations[0].id
      );
      ...
      let ele1 = document.getElementById(
        nodes.wrapper.children[1].id + '_text'
      );
      expect(ele1.getAttribute('text-decoration') === 'line-through').toBe(
        true
      );
      ...
      mouseEvents.clickEvent(
        diagramCanvas,
        annotationBounds.bounds.center.x,
        annotationBounds.bounds.center.y
      );
      mouseEvents.dblclickEvent(
        diagramCanvas,
        annotationBounds.bounds.center.x + 5,
        annotationBounds.bounds.center.y
      );
    ...
    it('Checking annotation margin wrapping', (done: Function) => {
      ...
      let temp: HTMLElement = document.getElementById(
        'Decision_label6_groupElement'
      );
      console.log(
        'temp.children[1].childElementCount  ' +
          temp.children[1].childElementCount
      );
      expect(
        temp.children[1].childElementCount === 4 ||
          temp.children[1].childElementCount === 2 ||
          temp.children[1].childElementCount === 5
      ).toBe(true);
      done();
    });
    it('memory leak', () => {
