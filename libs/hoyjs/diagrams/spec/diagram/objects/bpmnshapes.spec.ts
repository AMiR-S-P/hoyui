...
import {
  ShapeStyleModel,
  ShadowModel,
} from '../../../src/diagram/core/appearance-model';
...
describe('Diagram Control', () => {
  describe('BPMN Shapes', () => {
    ...
    beforeAll((): void => {
      ...
      let node: NodeModel = {
        ...
        shape: {
          type: 'Bpmn',
          shape: 'DataObject',
          dataObject: { type: 'Input', collection: true },
        } as BpmnShapeModel,
        shadow: shadow,
    ...
    it('BPMN shapes -  Message', (done: Function) => {
      let path: PathElement = (diagram.nodes[0] as Node).wrapper
  ...
  describe('BPMN Shapes', () => {
      ...
      let nodes: NodeModel[] = [
        ...
        },
        {
          ...
          shape: {
            type: 'Bpmn',
            shape: 'Activity',
            activity: { activity: 'Task', task: { type: 'BusinessRule' } },
          } as BpmnShapeModel,
        },
        {
          id: 'node7',
    ...
    it('Checking BPMN shapes without size', (done: Function) => {
      expect(
        Math.round((diagram.nodes[0] as NodeModel).wrapper.actualSize.width)
      ).toBe(34);
      expect(
        Math.round((diagram.nodes[0] as NodeModel).wrapper.actualSize.height)
      ).toBe(40);

      expect(
        Math.round((diagram.nodes[1] as NodeModel).wrapper.actualSize.width)
      ).toBe(29);
      expect(
        Math.round((diagram.nodes[1] as NodeModel).wrapper.actualSize.height)
      ).toBe(29);

      expect(
        Math.round((diagram.nodes[2] as NodeModel).wrapper.actualSize.width)
      ).toBe(40);
      expect(
        Math.round((diagram.nodes[2] as NodeModel).wrapper.actualSize.height)
      ).toBe(40);

      expect(
        Math.round((diagram.nodes[3] as NodeModel).wrapper.actualSize.width)
      ).toBe(50);
      expect(
        Math.round((diagram.nodes[3] as NodeModel).wrapper.actualSize.height)
      ).toBe(50);

      expect(
        Math.round((diagram.nodes[4] as NodeModel).wrapper.actualSize.width)
      ).toBe(50);
      expect(
        Math.round((diagram.nodes[4] as NodeModel).wrapper.actualSize.height)
      ).toBe(50);

      expect(
        Math.round((diagram.nodes[5] as NodeModel).wrapper.actualSize.width)
      ).toBe(50);
      expect(
        Math.round((diagram.nodes[5] as NodeModel).wrapper.actualSize.height)
      ).toBe(50);

      expect(
        Math.round((diagram.nodes[6] as NodeModel).wrapper.actualSize.width)
      ).toBe(50);
      expect(
        Math.round((diagram.nodes[6] as NodeModel).wrapper.actualSize.height)
      ).toBe(50);
      expect(
        Math.round((diagram.nodes[7] as NodeModel).wrapper.actualSize.height)
      ).toBe(50);
      expect(
        Math.round((diagram.nodes[7] as NodeModel).wrapper.actualSize.width)
      ).toBe(50);
      expect(
        Math.round((diagram.nodes[8] as NodeModel).wrapper.actualSize.height)
      ).toBe(40);
      expect(
        Math.round((diagram.nodes[8] as NodeModel).wrapper.actualSize.width)
      ).toBe(40);
      expect(
        Math.round((diagram.nodes[9] as NodeModel).wrapper.actualSize.height)
      ).toBe(40);
      expect(
        Math.round((diagram.nodes[9] as NodeModel).wrapper.actualSize.width)
      ).toBe(40);
  ...
  describe('Child is not positioned properly when calling doLayout() method', () => {
    ...
    it('Child is not positioned properly when calling doLayout() method', (done: Function) => {
      ...
      let connector = document.getElementById(
        'Connector1_Default_groupElement'
      );
      expect(connector.children[0].getAttribute('d') === 'M0,0 L22,0 ').toBe(
        true
      );
      done();
    });
  ...
  describe('Conditional sequence connector is improper when connected with BPMN Service shape', () => {
    ...
    it('Conditional sequence connector is improper when connected with BPMN Service shape', (done: Function) => {
      ...
      let ele2 = document.getElementById(
        'node1-processesTaskBpmn_groupElement'
      );
      console.log('ele22', ele2.children[0].getAttribute('transform'));
      expect(
        ele2.children[0].getAttribute('transform') ===
          'rotate(0,248,200.5)translate(248,200.5)'
      ).toBe(false);
      done();
    });
  });
  describe('Style for inner elements of BPMN Gateway and subprocess Shapes', () => {
    ...
    it('Strokecolor for BPMN Gateway and subprocess initial rendering', (done: Function) => {
      let gatewayElement = document.getElementById(
        'node1_1_gatewayType_groupElement'
      );
      expect(
        gatewayElement.firstElementChild.getAttribute('stroke') === 'red'
      ).toBe(true);
      let subprocessElement = document.getElementById(
        'node2_0_collapsed_groupElement'
      );
      expect(
        subprocessElement.firstElementChild.getAttribute('stroke') === 'red'
      ).toBe(true);
      done();
    });
    it('Strokecolor for BPMN Gateway and subprocess runtime', (done: Function) => {
      ...
      expect(
        gatewayElement.firstElementChild.getAttribute('stroke') === 'green'
      ).toBe(true);
      ...
      expect(
        subprocessElement.firstElementChild.getAttribute('stroke') === 'green'
      ).toBe(true);
      done();
    });
  });
  ...
  describe('BPMN subprocess save and load issue ', () => {
    ...
    beforeAll((): void => {
      ...
      diagram = new Diagram({
        width: '74%',
        height: '600px', // connectors: connector6,
        nodes: nodes,
      });
      diagram.appendTo('#diagram');
    });
...
describe('BPMN Flow connectors not changed properly at runtime ', () => {
  ...
  beforeAll((): void => {
    ...
    let connectors: ConnectorModel[] = [
      ...
      {
        id: 'message',
        sourcePoint: { x: 450, y: 100 },
        targetPoint: { x: 650, y: 300 },
        type: 'Straight',
        shape: {
          type: 'Bpmn',
  ...
  it('Checking Bpmn association flow connector after changing its association type at runtime', (done: Function) => {
    let preAssociationType = (diagram.connectors[0].shape as BpmnFlowModel)
      .association;
    let preSrcDec = diagram.connectors[0].sourceDecorator.shape;
    (diagram.connectors[0].shape as BpmnFlowModel).association =
      'BiDirectional';
    diagram.dataBind();
    let currAssociationType = (diagram.connectors[0].shape as BpmnFlowModel)
      .association;
  ...
  it('Checking Bpmn message flow connector after changing its message type at runtime', (done: Function) => {
    ...
    let preMessageColor =
      diagram.connectors[2].wrapper.children[3].style.fill.toLowerCase();
    (diagram.connectors[2].shape as BpmnFlowModel).message =
      'NonInitiatingMessage';
    diagram.dataBind();
    let currMessageType = (diagram.connectors[2].shape as BpmnFlowModel)
      .message;
    let currMessageColor =
      diagram.connectors[2].wrapper.children[3].style.fill.toLowerCase();
    expect(
      preMessageType === 'InitiatingMessage' &&
        currMessageType === 'NonInitiatingMessage' &&
        preMessageColor === 'white' &&
        currMessageColor === 'lightgrey'
    ).toBe(true);
    done();
  });
