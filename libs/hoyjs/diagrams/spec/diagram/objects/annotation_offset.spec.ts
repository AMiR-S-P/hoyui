...
describe('Diagram Control', () => {
    ...
    it('Checking annotation offsets as 0 in SVG rendering Mode', (done: Function) => {
      expect(
        (diagram.nodes[0] as Node).wrapper.children[1].offsetX === 50 &&
          ...
          (diagram.nodes[2] as Node).wrapper.children[1].offsetY === 150
      ).toBe(true);
      done();
    });
  ...
  describe('Annotations with zero offsets ', () => {
    ...
    it('Checking annotation offset as 1 in SVG rendering Mode', (done: Function) => {
      expect(
        (diagram.basicElements[0] as TextElement).offsetX == 400 &&
  ...
  describe('Remove labels API ', () => {
    ...
    it('Checking remove labels API', (done: Function) => {
      ...
      expect(
        label1.childNodes.length == 0 &&
          label2.childNodes.length == 0 &&
          label3.childNodes.length == 0
      ).toBe(true);
      diagram.addLabels(node, label);
      diagram.startTextEdit(
        diagram.nodes[0],
        diagram.nodes[0].annotations[0].id
      );
      expect(
        document.getElementById('diagram50_editBox').style.background ===
          'black'
      );
      mouseEvents.clickEvent(diagramCanvas, 10, 10);
      diagram.startTextEdit(
        diagram.nodes[0],
        diagram.nodes[0].annotations[1].id
      );
      expect(
        document.getElementById('diagram50_editBox').style.background ===
          'white'
  ...
  describe('Bezier connector annotation center alignment is not working properly  ', () => {
    ...
    it('Checking bezier connector annotation vertical and horizontal alignment as center', (done: Function) => {
      ...
      expect(
        Math.round(textElement.bounds.x) === 272 &&
          Math.round(textElement.bounds.y) === 193
      ).toBe(true);
      done();
    });
  });
