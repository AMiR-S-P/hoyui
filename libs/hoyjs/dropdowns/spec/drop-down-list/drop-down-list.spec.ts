...
import { DataManager, ODataV4Adaptor, Query, WebApiAdaptor } from '@hoyui/data';
import { isCollide } from '@hoyui/popups';
...
describe('DDList', () => {
  ...
  let css: string =
    ".e-spinner-pane::after { content: 'Material'; display: none;} ";
  ...
  describe('Local data binding with default values', () => {
    ...
    it('tab index of focus element', () => {
      expect(
        listObj.inputWrapper.container.getAttribute('tabindex') === '0'
      ).toBe(true);
    });
    /**
     * Template properties
    ...
    it('string header template property', (done) => {
      ...
      listObj.open = function () {
        expect(listObj.headerTemplate).toEqual(
          listObj.popupObj.element.firstChild.innerText
        );
        listObj.hidePopup();
        listObj.open = null;
        done();
    ...
    it('string footer template property', (done) => {
      ...
      listObj.dataBind();
      listObj.open = function () {
        expect(listObj.footerTemplate).toEqual(
          listObj.popupObj.element.lastChild.innerText
        );
        listObj.hidePopup();
        listObj.open = null;
        done();
    ...
    it('value template', (done) => {
      ...
      listObj.change = function () {
        let valueEle: HTMLElement =
          listObj.element.parentElement.querySelector('.e-input-value');
        expect(valueEle.innerHTML).toEqual('<div class="ename"> list1 </div>');
        expect(
          listObj.element.parentElement.childNodes[1].classList.contains(
            'e-input-value'
          )
        ).toBe(true);
  ...
  describe('Local data binding with Disable Item', () => {
    ...
    let localData: { [key: string]: Object }[] = [
      ...
      { id: 'list1', text: 'JAVA' },
      {
        id: 'list2',
        text: 'Phython',
        htmlAttributes: { class: 'e-disabled', title: 'Phython' },
      },
      { id: 'list5', text: 'Oracle' },
    ];
    let element: HTMLInputElement = <HTMLInputElement>(
    ...
    it('choose disabled item', (done) => {
      ...
      let items: HTMLElement[] =
        listObj.popupObj.element.querySelectorAll('li');
      listObj.setSelection(items[0], e);
    });
    it('check data attribute', () => {
      expect(
        listObj.hiddenElement.getAttribute('data-msg-container-id')
      ).not.toBeNull();
  ...
  describe('Floating label', () => {
    ...
    it('floating-Auto: check floating to top', () => {
      ...
      let floatElement =
        listObj.inputWrapper.container.querySelector('.e-float-text');
  ...
  describe('Clear button', () => {
    ...
    let element1: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'ddlelement' })
    );
    let mouseEventArgs: any = { preventDefault: function () {}, target: null };
    beforeAll(() => {
      document.body.appendChild(element);
    ...
    it('clear icon', () => {
      expect(
        listObj.inputWrapper.clearButton.classList.contains('e-clear-icon')
      ).toBe(true);
    });
    ...
    it('click on clear button', () => {
      ...
      expect(
        listObj.inputWrapper.clearButton.classList.contains('e-clear-icon-hide')
      ).toBe(true);
    });
    it('show clear icon when key navigation', (done) => {
      ...
      setTimeout(() => {
        expect(
          listObj.inputWrapper.clearButton.classList.contains(
            'e-clear-icon-hide'
          )
        ).toBe(true);
    ...
    it('show clear icon with allowFiltering', (done) => {
      ...
      setTimeout(() => {
        expect(
          listObj1.inputWrapper.clearButton.classList.contains(
            'e-clear-icon-hide'
          )
        ).toBe(true);
        done();
  ...
  describe('Remote data binding', () => {
    ...
    it('value at dynamic changes with allowObjectBinding', (done) => {
      ...
      setTimeout(() => {
        expect(
          listObj.isObjectInArray({ id: 'id1', text: 'HTML' }, [listObj.value])
        ).toBe(true);
        listObj.value = { id: 'list2', text: 'Phython' };
        setTimeout(() => {
          expect(
            listObj.isObjectInArray({ id: 'list2', text: 'Phython' }, [
              listObj.value,
            ])
          ).toBe(true);
          expect(isNullOrUndefined(listObj.popupObj)).toBe(true);
  ...
  describe('Add multiple cssClass', () => {
    ...
    it('Dynamically change multiple cssClass', () => {
      ...
      listObj.appendTo(element);
      listObj.showPopup();
      expect(listObj.element.parentElement.classList.contains('sample')).toBe(
        true
      );
      expect(listObj.popupObj.element.classList.contains('sample')).toEqual(
        true
      );
      listObj.cssClass = 'test highlight';
      listObj.dataBind();
      expect(listObj.element.parentElement.classList.contains('test')).toBe(
        true
      );
      expect(
        listObj.element.parentElement.classList.contains('highlight')
      ).toBe(true);
      expect(listObj.popupObj.element.classList.contains('test')).toEqual(true);
      expect(listObj.popupObj.element.classList.contains('highlight')).toEqual(
        true
      );
    });
    ...
    it('Dynamically change cssClass as null', () => {
      ...
      expect(listObj.popupObj.element.classList.contains('test')).toEqual(
        false
      );
      expect(listObj.popupObj.element.classList.contains('highlight')).toEqual(
        false
  ...
  describe('Dynamic property changes', () => {
    ...
    it('cssClass ', () => {
      ...
      listObj.dataBind();
      expect(
        listObj.element.parentElement.classList.contains('closeState')
      ).toBe(true);
    });
    /**
    ...
    it('text based selection', () => {
      ...
      let li: Element[] & NodeListOf<HTMLLIElement> = <
        Element[] & NodeListOf<HTMLLIElement>
      >ele.querySelectorAll('li');
    ...
    it('cssClass ', () => {
      ...
      listObj.dataBind();
      expect(listObj.element.parentElement.classList.contains('temp')).toEqual(
        true
      );
    ...
    it('enabled ', () => {
      ...
      expect(
        listObj.inputWrapper.container.classList.contains('e-disabled')
      ).toEqual(true);
    ...
    it('enableRtl ', () => {
      ...
      listObj.dataBind();
      listObj.showPopup();
      expect(listObj.element.parentElement.classList.contains('e-rtl')).toEqual(
        true
      );
    ...
    it('htmlAttributes', () => {
      ...
      expect(
        listObj.element.parentElement.classList.contains('e-ddl-list')
      ).toBe(true);
      expect(
        listObj.element.parentElement.classList.contains('e-disabled')
      ).toBe(true);
      expect(listObj.element.getAttribute('placeholder')).toBe('new text');
      expect(listObj.element.parentElement.getAttribute('role')).toBe(
        'combobox'
      );
      expect(listObj.element.parentElement.getAttribute('style')).toBe(
        'margin: 0'
      );
    });
    /**
    ...
    it('footerTemplate ', () => {
      ...
      listObj.dataBind();
      expect(listObj.popupObj.element.lastElementChild.innerHTML).toEqual(
        listObj.footerTemplate
      );
      listObj.footerTemplate = 'Overall 5 cources available';
      listObj.dataBind();
      expect(listObj.popupObj.element.lastElementChild.innerHTML).not.toEqual(
        'Total items count 5'
      );
    });
    /**
    ...
    it('locale property - es', (done) => {
      ...
      listObj.open = (args: PopupEventArgs): void => {
        expect(
          (args.popup.element.querySelector('.e-nodata') as HTMLElement)
            .innerText
        ).toEqual('Pas de');
        done();
      };
  ...
  describe('Initialize properties changes', () => {
    ...
    it('value in integer type', () => {
      let datasource: { [key: string]: Object }[] = [
        { id: 1, text: 'JAVA', icon: 'icon' },
        { id: 2, text: 'C#' },
        { id: 3, text: 'C++' },
        { id: 4, text: '.NET', icon: 'icon' },
        { id: 5, text: 'Oracle' },
      ];
      listObj = new DropDownList({
        dataSource: datasource,
  ...
  describe('Template support', () => {
    ...
    let listObj2: any;
    let element1: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'ddl' })
    );
    let element2: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'ddl2' })
    );
    let keyEventArgs: any = {
      preventDefault: (): void => {
        /** NO Code */
    ...
    it('string header template property', () => {
      ...
      listObj1.appendTo(element1);
      listObj1.showPopup();
      expect(listObj1.headerTemplate).toEqual(
        listObj1.popupObj.element.firstChild.innerText
      );
    });
    it('string footer template property', () => {
      expect(listObj1.footerTemplate).toEqual(
        listObj1.popupObj.element.lastChild.innerText
      );
    });
    it('value template', () => {
      document.body.innerHTML = '';
      ...
      listObj2.dataBind();
      let valueEle: HTMLElement =
        listObj2.element.parentElement.querySelector('.e-input-value');
      expect(valueEle.innerHTML).toEqual('<div class="ename"> list1 </div>');
      expect(
        listObj2.element.parentElement.childNodes[1].classList.contains(
          'e-input-value'
        )
      ).toBe(true);
    ...
    it('value template with null value', () => {
      ...
      expect(
        listObj2.element.parentElement.firstChild.classList.contains(
          'e-input-value'
        )
      ).not.toBe(true);
    });
    ...
    it('escape key - after value select and previous not empty text box', (done) => {
      ...
      setTimeout(() => {
        ...
        listObj2.keyActionHandler(keyEventArgs);
        setTimeout(() => {
          let valueTemp: HTMLElement = <HTMLElement>(
  ...
  describe('method testing ', () => {
    ...
    it('show popup method', (done) => {
      expect(listObj.inputWrapper.container.getAttribute('aria-expanded')).toBe(
        'false'
      );
      listObj.showPopup();
      setTimeout(function () {
        ...
        expect(
          listObj.element.parentElement.classList.contains('e-icon-anim')
        ).toEqual(true);
        expect(
          listObj.element.parentElement.classList.contains('e-input-focus')
        ).toEqual(true);
        expect(
          listObj.inputWrapper.container.getAttribute('aria-expanded')
        ).toBe('true');
        listObj.showPopup();
        expect(listObj.isPopupOpen).toEqual(true);
        expect(
    ...
    it('removeHover method ', () => {
      let hoveredLi: Element =
        listObj.popupObj.element.querySelector('.e-hover');
    ...
    it('addItem with selection ', () => {
      ...
      expect(listObj.list.querySelector('li').textContent).not.toBe(
        'TypeScript'
      );
    });
    ...
    it('destroy method ', () => {
      listObj.destroy();
      expect(!!listObj.element.classList.contains('e-dropdownlist')).toBe(
        false
      );
      document.body.innerHTML = '';
    });
  ...
  describe('Angular tag testing ', () => {
    ...
    beforeAll(() => {
      let element: any = createElement('EJS-DROPDOWNLIST', {
        id: 'dropdownlist',
      });
      document.body.appendChild(element);
      listObj = new DropDownList({ dataSource: datasource2 });
    ...
    it('Wrapper testing ', () => {
      ...
      expect(listObj.inputWrapper.container.parentElement).toBe(
        listObj.element
      );
    });
  });
  ...
  describe('Select event triggered', () => {
    ...
    it('select event trigger at initialize and check aria-activedescendant', (done) => {
      ...
      expect(
        listObj.inputWrapper.container.getAttribute('aria-activedescendant') !==
          'null'
      ).toBe(true);
      listObj.showPopup();
      setTimeout(() => {
        expect(
          listObj.inputWrapper.container.getAttribute(
  ...
  describe('event testing ', () => {
    ...
    it('mouse click on list ', (done) => {
      ...
      setTimeout(() => {
        let item: HTMLElement[] =
          listObj.popupObj.element.querySelectorAll('li')[3];
    ...
    it('change & close event trigger ', (done) => {
      ...
      setTimeout(() => {
        let item: HTMLElement[] =
          listObj.popupObj.element.querySelectorAll('li')[4];
        mouseEventArgs.target = item;
        mouseEventArgs.type = 'click';
        listObj.onMouseClick(mouseEventArgs);
        setTimeout(function () {
          ...
          expect(listObj.value).toEqual(
            listObj.listData[4][listObj.fields.value]
          );
          expect(listObj.isPopupOpen).toBe(false);
          done();
        }, 450);
  ...
  describe(' hidden element ', () => {
    ...
    it('name attribute as id of this.element', () => {
      ...
      let select: HTMLSelectElement[] = <
        HTMLSelectElement[] & NodeListOf<HTMLSelectElement>
      >element.parentElement.querySelectorAll('select');
    ...
    it('select value checking', () => {
      ...
      let option: HTMLOptionElement[] = <
        HTMLOptionElement[] & NodeListOf<HTMLElement>
      >element.parentElement.querySelectorAll('select>option');
  ...
  describe('select element Rendering', () => {
    let listObj: any;
    let element: string =
      "<select id='select1'><option>option1</option><option value='option2'>option2</option></select>";
    let groupelement: string =
      "<select id='select2'><optgroup label='option'><option>option1</option><option value='option2' disabled>option2</option></optgroup><option>option3</option></select>";
    /**
     * Select Rendering
     */
    it('Select Rendering with options only', () => {
      ...
      expect(ele.querySelectorAll('li')[0].getAttribute('data-value')).toBe(
        'option1'
      );
      expect(ele.querySelectorAll('li')[1].getAttribute('data-value')).toBe(
        'option2'
      );
    });
    it('Select Rendering with options and optgroup', () => {
      document.body.innerHTML = groupelement;
      let groupselect: HTMLSelectElement = document.getElementById(
        'select2'
      ) as HTMLSelectElement;
      listObj = new DropDownList();
      ...
      expect(ele.querySelectorAll('li')[1].classList).toContain(
        'e-list-group-item'
      );
      expect(ele.querySelectorAll('li')[2].getAttribute('data-value')).toBe(
        'option1'
      );
      expect(ele.querySelectorAll('li')[3].getAttribute('data-value')).toBe(
        'option2'
      );
  ...
  describe('key actions', () => {
    ...
    it('Down key to last li ', () => {
      ...
      let li: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
        ele.querySelectorAll('li')
      );
      list.setSelection(li[li.length - 1]);
      expect(
        (li[li.length - 1] as Element).classList.contains('e-active')
      ).toBe(true);
      list.keyActionHandler(keyEventArgs);
  ...
  describe('with group', () => {
    ...
    let scrollItemData: { [key: string]: Object }[] = [
      { text: 'Audi A6', id: 'e807', category: 'Audi' },
      { text: 'Audi A7', id: 'a0cc', category: 'Audi' },
      { text: 'BMW 501', id: 'f8435', category: 'BMW' },
  ...
  describe('Animation ', function () {
    ...
    it('hide method ', (done) => {
      listObj.hidePopup();
      setTimeout(() => {
        expect(listObj.popupObj.element.classList.contains('e-control')).toBe(
          false
        );
        done();
      }, 500);
    ...
    describe('Basic rendering', () => {
      ...
      it('header position while enable the searchBox', () => {
        let element = <HTMLElement[] | NodeListOf<HTMLElement>>(
          document.querySelectorAll('.e-filter-parent')
        );
        expect(element[0].nextElementSibling.textContent === 'header').toBe(
          true
        );
      });
      ...
      it('check search box parent element', () => {
        let element = <HTMLElement>document.querySelector('.e-input-filter');
        expect(element.parentElement.classList.contains('e-input-group')).toBe(
          true
        );
      });
      it('check search box icon element', () => {
        let element = <HTMLElement>document.querySelector('.e-input-filter');
        expect(
          (element.nextSibling as HTMLElement).classList.contains('e-icons')
        ).toBe(true);
      });
      it('search box wrapper insert before of list element', () => {
        let element = <HTMLElement>document.querySelector('.e-popup');
        expect(
          (element.childNodes[0] as HTMLElement).classList.contains(
            'e-filter-parent'
          )
        ).toBe(true);
      });
  ...
  describe('Allowfiltering support in mobile', () => {
    ...
    it('clear text & search icon in search textbox', () => {
      listObj.clearText();
      var clearElement =
        listObj.filterInput.parentElement.querySelector('.e-clear-icon');
  ...
  describe('focus and blur event triggered', () => {
    describe('basic rendering', () => {
      ...
      it('focus event trigger when click on dropdown input', (done) => {
        ...
        setTimeout(() => {
          ...
          expect(
            listObj1.inputWrapper.container.classList.contains('e-input-focus')
          ).toBe(true);
          done();
        }, 450);
      });
      ...
      it('focus event hold when click a list', (done) => {
        let item: HTMLElement[] =
          listObj1.popupObj.element.querySelectorAll('li')[2];
        mouseEventArgs.target = item;
        mouseEventArgs.type = 'click';
      ...
      it('IE 11 issue: focus the current focus element when click on header at popup open state', (done) => {
        ...
        setTimeout(() => {
          ...
          expect(
            listObj1.inputWrapper.container === document.activeElement
          ).toBe(true);
          expect(
            listObj1.inputWrapper.container.classList.contains('e-input-focus')
          ).toBe(true);
    ...
    describe('render with filtering', () => {
      ...
      it('filterBarPlaceholder property', (done) => {
        ...
        listObj1.dataBind();
        expect(listObj1.filterInput.getAttribute('placeholder')).toEqual(
          listObj1.filterBarPlaceholder
        );
        listObj1.hidePopup();
        setTimeout(() => {
          done();
      ...
      it('hold focus event when click a list', (done) => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            expect(
              listObj1.inputWrapper.container.classList.contains(
                'e-input-focus'
              )
            ).toBe(true);
            done();
  ...
  describe('HTML UL element data binding', () => {
    ...
    it('click on list item', () => {
      let item: HTMLElement[] =
        listObj.popupObj.element.querySelectorAll('li')[2];
      mouseEventArgs.target = item;
      mouseEventArgs.type = 'click';
      listObj.onMouseClick(mouseEventArgs);
  ...
  describe('select element Rendering', () => {
    ...
    it('click on list item', () => {
      let item: HTMLElement[] =
        listObj.popupObj.element.querySelectorAll('li')[1];
      mouseEventArgs.target = item;
      mouseEventArgs.type = 'click';
      listObj.onMouseClick(mouseEventArgs);
  ...
  describe('Spinner support', () => {
    ...
    it(' - spinner show instead of dropdown icon at initial time', () => {
      ...
      expect(
        isNullOrUndefined(
          listObj.inputWrapper.buttons[0].querySelector('e-spinner-pane')
        )
      ).toBe(true);
    });
    it(' - spinner show instead of clear icon in filter bar', (done) => {
      setTimeout(() => {
        ...
        expect(
          isNullOrUndefined(
            listObj.filterInputObj.buttons[0].querySelector('e-spinner-pane')
          )
        ).toBe(true);
        listObj.hidePopup();
        setTimeout(() => {
    ...
    it(' - spinner show on mobile filter bar', (done) => {
      ...
      setTimeout(() => {
        ...
        listObj.searchLists();
        expect(
          isNullOrUndefined(listObj.list.querySelector('e-spinner-pane'))
        ).toBe(true);
        Browser.userAgent = navigator.userAgent;
        done();
      }, 400);
  ...
  describe('with group alignemnt', () => {
    ...
    it('grouping header alignment', (done) => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          expect(
            (document.getElementsByClassName('e-fixed-head')[0] as any).style
              .zIndex === '10'
          ).toBe(true);
          list.scrollHandler();
          list.dataBind();
  ...
  describe('nested data binding to fields', () => {
    ...
    it('select the complex data of text and value fields while click on popup list', (done) => {
      ...
      setTimeout(() => {
        let item: HTMLElement[] =
          list.popupObj.element.querySelectorAll('li')[1];
        mouseEventArgs.target = item;
        mouseEventArgs.type = 'click';
        list.onMouseClick(mouseEventArgs);
  ...
  describe('Value get as object if selected value as 0 (I197200||EJ2-7752)', () => {
    ...
    it('select the 0 value item', (done) => {
      ...
      setTimeout(() => {
        let item: HTMLElement[] =
          list.popupObj.element.querySelectorAll('li')[0];
        mouseEventArgs.target = item;
        mouseEventArgs.type = 'click';
        list.onMouseClick(mouseEventArgs);
  ...
  describe('Grouping in DropDownList', () => {
    ...
    it('Render the DropDowns with grouping and sortOrder:None', (done) => {
      let groupData = [
        ...
        { vegetable: 'Wheatgrass', category: 'Leafy and Salad' },
        { vegetable: 'Yarrow', category: 'Leafy and Salad' },
        { vegetable: 'Chickpea', category: 'Beans' },
        { vegetable: 'Green bean', category: 'Beans' },
        { vegetable: 'Horse gram', category: 'Beans' },
        { vegetable: 'Garlic', category: 'Bulb and Stem' },
        { vegetable: 'Nopal', category: 'Bulb and Stem' },
      ...
      dropDowns.open = function (args: any) {
        let groupElemenet: HTMLElement =
          dropDowns.list.querySelector('.e-list-group-item');
  ...
  describe('GetItems related bug', () => {
    ...
    beforeAll(() => {
      ...
      element1 = <HTMLInputElement>(
        createElement('input', { id: 'dropdownlist1' })
      );
      document.body.appendChild(element1);
    });
    afterAll(() => {
  ...
  describe('EJ2-15820 - select event not trigger issue', () => {
    ...
    it('check same value selection', (done) => {
      listObj = new DropDownList({
        ...
        fields: { text: 'text', value: 'id' },
        open: () => {
          let items: HTMLLIElement = (<any>(
            listObj
          )).popupObj.element.querySelector('.e-list-parent').firstChild;
  ...
  describe('bug(EJ2-21907): Dropdowns html5 validation attributes are added.', () => {
    let ddlObj: any;
    let ddlEle: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'ddl' })
    );
    let empList: { [key: string]: Object }[] = [
    ...
    it('Check attributes', () => {
      expect(
        isNullOrUndefined(
          ddlObj.inputWrapper.container.getAttribute('readonly')
        )
      ).toBe(true);
      expect(
        isNullOrUndefined(
          ddlObj.inputWrapper.container.getAttribute('aria-placeholder')
        )
      ).toBe(true);
      expect(
        isNullOrUndefined(
          ddlObj.inputWrapper.container.getAttribute('autocorrect')
        )
      ).toBe(true);
      expect(
        isNullOrUndefined(
          ddlObj.inputWrapper.container.getAttribute('autocomplete')
        )
      ).toBe(true);
      expect(
        isNullOrUndefined(
          ddlObj.inputWrapper.container.getAttribute('autocapitalize')
        )
      ).toBe(true);
      expect(
        isNullOrUndefined(
          ddlObj.inputWrapper.container.getAttribute('spellcheck')
        )
      ).toBe(true);
      expect(!isNullOrUndefined(ddlObj.inputElement.getAttribute('role'))).toBe(
        true
      );
      expect(!isNullOrUndefined(ddlObj.inputElement.getAttribute('type'))).toBe(
        true
      );
    });
  });
  ...
  describe('DDL-add item', () => {
    ...
    it('Add item with string and array', () => {
      ...
      expect(ddlObj.list.querySelectorAll('li')[1].textContent).toBe(
        'Basketball'
      );
  ...
  describe('DDL-hidepopup', () => {
    ...
    it('when crosses view port', () => {
      (<HTMLInputElement>(
        document.getElementsByClassName('e-ddl-icon')[0]
      )).click();
      ddlObj.showPopup();
  ...
  describe('EJ2-33258', () => {
    ...
    beforeAll(() => {
      element = <HTMLInputElement>(
        createElement('input', { id: 'dropdownlist.list' })
      );
      document.body.appendChild(element);
      listObj = new DropDownList({
        dataSource: datasource,
  ...
  describe('Fixed header maintained after reopening the popup', () => {
    ...
    it('Pressing Down key in to scroll the popup down', () => {
      ...
      listObj.onSearch(keyEventArgs);
      listObj.showPopup();
      expect(listObj.popupObj.element.querySelector('.e-fixed-header')).toBe(
        null
      );
    });
  });
  describe('EJ2-36604 - While giving the class name with empty space for HtmlAttributes, console error is produced.', function () {
    ...
    it('Entering the class name without any empty space', function () {
      ...
      expect(
        listObj.inputWrapper.container.classList.contains('custom-class')
      ).toBe(true);
    });
    ...
    it('Giving more than one empty space between two class names', function () {
      ...
      listObj.appendTo('#dropdownlist');
      expect(
        listObj.inputWrapper.container.classList.contains('custom-class-one')
      ).toBe(true);
      expect(
        listObj.inputWrapper.container.classList.contains('custom-class-two')
      ).toBe(true);
    });
    ...
    it('Giving class name with underscore in the beginning', function () {
      ...
      listObj.appendTo('#dropdownlist');
      expect(
        listObj.inputWrapper.container.classList.contains('_custom-class-one')
      ).toBe(true);
      expect(
        listObj.inputWrapper.container.classList.contains('_custom-class-two')
      ).toBe(true);
    });
  ...
  describe('EJ2-39852', () => {
    ...
    it('Newly added item position changes on popup open and close action when allowfiltring is enabled', () => {
      ...
      ddlObj.hidePopup();
      ddlObj.showPopup();
      expect((ddlObj as any).list.querySelectorAll('li')[1].textContent).toBe(
        'Archery'
      );
      expect((ddlObj as any).list.querySelectorAll('li')[3].textContent).toBe(
        'Baseball'
      );
    });
  });
  describe('EJ2-39852', () => {
    ...
    let Country: { [key: string]: Object }[] = [
      { id: 'list1', text: 'Australia' },
      { id: 'list2', text: 'Belgium' },
      { id: 'list3', text: 'Canada' },
      { id: 'list4', text: 'France' },
      { id: 'list5', text: 'India' },
    ];
    let listObj: any;
    let element: HTMLInputElement = <HTMLInputElement>(
    ...
    it('Filter newly added item from the list', () => {
      ...
      listObj.onFilterUp(keyEventArgs);
      expect((listObj as any).list.querySelectorAll('li')[0].textContent).toBe(
        'Belgium'
      );
      expect((listObj as any).list.querySelectorAll('li')[1].textContent).toBe(
        'Brazil'
      );
    });
  });
  ...
  describe('EJ2-51558 - Incremental search not working in grid', () => {
    ...
    let element2: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'dropdownlist2' })
    );
    beforeAll(() => {
      originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
  ...
  describe('EJ2CORE-663-Additem method not working properly for x-template script', () => {
    ...
    it('AddItem testing ', () => {
      let element1: string =
        "<script id='xtemplate' type='text/x-template'><div class='list-wrapper'><span class='${pic} e-avatar e-avatar-xlarge e-avatar-circle'></span><span class='text'>${text}</span><span class='description'> +${description}</span></div></script>";
      ...
      expect(listObj.liCollections[2].textContent).toBe(
        'JavaScript1 +added item'
      );
    });
  });
  describe('EJ2-60347', () => {
    ...
    it('to check the mouse click action', function (done) {
      setTimeout(function () {
        ...
        mouseEventArgs.target =
          listObj.popupObj.element.querySelectorAll('li')[0];
        listObj.onMouseClick(mouseEventArgs);
        expect(listObj.liCollections[0].classList.contains('e-active')).toBe(
          true
        );
        listObj.showPopup();
        listObj.filterInput.value = 'c';
  ...
  describe('keyboard interaction with disabled items', () => {
    ...
    it('up and down action', (done) => {
      ...
      setTimeout(() => {
        ...
        listObj.keyActionHandler(keyEventArgs);
        expect(
          listObj.list.querySelector('.e-active').getAttribute('data-value') ===
            'Game2'
        ).toBe(true);
        listObj.keyActionHandler(keyEventArgs);
        expect(
