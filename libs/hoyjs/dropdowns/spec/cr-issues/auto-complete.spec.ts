...
import { DataManager, Query, WebApiAdaptor } from '@hoyui/data';
...
xdescribe('AutoComplete', () => {
  ...
  describe('EJ2-26008-The autocomplete is not focused when click the tab key at single time', () => {
    let autoObj: any;
    let autoEle: HTMLInputElement = <HTMLInputElement>(
    ...
    it('Remove tabindex attribute in wrapper element', () => {
      expect(autoObj.inputWrapper.container.hasAttribute('tabindex')).toBe(
        false
      );
    });
  });
  ...
  describe('template with highlight search ', () => {
    ...
    beforeAll(() => {
      ...
      atcObj = new AutoComplete({
        ...
        fields: { value: 'text' },
        itemTemplate:
          '<div><img class="eimg" src="./../Employees/${eimg}.png" alt="employee"/>' +
  ...
  describe('EJ2CORE-353-Tab Key support', () => {
    ...
    let autocompleteEle: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'auto' })
    );
    let countries: { [key: string]: Object }[] = [
      { Name: 'Australia', Code: 'AU' },
  ...
  describe('EJ2-45069- Highlight search with iconcss', () => {
    ...
    let socialMedia: { [key: string]: Object }[] = [
      { class: 'sf-icon-facebook', country: 'Facebook' },
      { class: 'sf-icon-twitter', country: 'Twitter ' },
      { class: 'sf-icon-whatsapp', country: 'WhatsApp' },
      { class: 'sf-icon-tumblr', country: 'Tumblr' },
      { class: 'sf-icon-google-plus', country: 'Google plus' },
      { class: 'sf-icon-skype-01', country: 'Skype' },
      { class: 'sf-icon-vimeo', country: 'Vimeo' },
      { class: 'sf-icon-instagram', country: 'Instagram' },
      { class: 'sf-icon-youtube1', country: 'YouTube' },
      { class: 'sf-icon-reddit', country: 'Reddit' },
    ];
    beforeAll(() => {
      document.body.appendChild(element);
    });
    ...
    it('highlight by EndsWith', () => {
      ...
      let highlight: HTMLElement[] =
        atcObj1.liCollections[0].querySelectorAll('.e-highlight');
      expect(highlight.length === 1).toBe(true);
    });
    it('highlight by Contains', () => {
      ...
      let highlight: HTMLElement[] =
        atcObj2.liCollections[0].querySelectorAll('.e-highlight');
