...
let mockAjax: Function = (
  d: { data: { [o: string]: Object | Object[] } | Object[]; dm?: DataManager },
...
describe('DropDownList', () => {
  ...
  describe('EJ2-10838 - Change event trigger while component initial render with empty and then set the data at any action', () => {
    ...
    let ddlEle: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'ddl', attrs: { autofocus: 'autofocus' } })
    );
  ...
  describe('EJ2-14939 - CssClass applied to the DropDown element is not removed when it is changes dynamically', () => {
    ...
    it('check whether the change cssclass dynamically', () => {
      expect(ddlObj.inputWrapper.container.classList.contains('abc')).toBe(
        true
      );
      ddlObj.cssClass = 'dce';
      ddlObj.dataBind();
      expect(ddlObj.inputWrapper.container.classList.contains('abc')).not.toBe(
        true
      );
      expect(ddlObj.inputWrapper.container.classList.contains('dce')).toBe(
        true
      );
    });
  });
  ...
  describe('EJ2-16375: Form inside form reset', () => {
    ...
    beforeAll(() => {
      ...
      form2.id = 'form2';
      form2.innerHTML =
        '<input id="tempInput" type="text"/> <input type="reset" id="resetForm"/>';
  ...
  describe('EJ2-27976 - Dynamic filtering issue', () => {
    ...
    it('filter a suggestion list with select element', () => {
      ...
      let len: number =
        listObj.list.getElementsByClassName('e-list-item').length;
      li[0].click();
      listObj.hidePopup();
      let len2: number =
        listObj.list.getElementsByClassName('e-list-item').length;
  ...
  describe('EJ2-17694 - Multiple time ajax request while change the dataSource ', () => {
    let listObj: any;
    let controlEle: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'auto' })
    );
    let result: any = [];
    beforeAll((done) => {
      ...
      let mAjax: MockAjaxReturn = mockAjax(
        {
          data: {
            d: new DataManager(data).executeLocal(
              new Query()
                .take(10)
  ...
  describe(' dynamic datasource changes', () => {
    ...
    beforeAll(() => {
      ...
      listObj = new DropDownList({
        ...
        valueTemplate:
          '<div class="name" style="display:inline;"> ${Name} </div>',
        value: '4',
      });
      listObj.appendTo(element);
    });
  ...
  describe('template item not generate', () => {
    ...
    it('while change the datasource dynamically', function () {
      ...
      expect(
        (
          (listObj as any).ulElement.querySelector('li')
            .firstElementChild as HTMLElement
        ).innerText === 'Andrew Fuller'
      ).toBe(true);
      expect(
        (listObj as any).ulElement.querySelectorAll('li').length === 2
      ).toBe(true);
      listObj.dataSource = changeData;
      listObj.dataBind();
      expect(
        (
          (listObj as any).ulElement.querySelector('li')
            .firstElementChild as HTMLElement
        ).innerText === 'Janet Leverling'
      ).toBe(true);
      expect(
        (listObj as any).ulElement.querySelectorAll('li').length ===
          (listObj.dataSource as any).length
      ).toBe(true);
    });
    afterAll(() => {
      if (element) {
  ...
  describe(' add item with Template', () => {
    ...
    it('item template with add item values', function (done) {
      expect(
        (listObj.dataSource as string[]).length === templateDataSource.length
      ).toBe(true);
      listObj.addItem({ Name: 'dropdown', Eimg: 100 });
      listObj.open = function (args) {
        popup = document.getElementById('dropdownlist_popup');
        divNode = popup.querySelectorAll('li div.ename')[
          templateDataSource.length
        ] as HTMLDivElement;
        textContent = divNode.innerText;
  ...
  describe('EJ2-24975: Form reset clear icon', () => {
    ...
    it(' reset the form', (done) => {
      ...
      setTimeout(() => {
        expect(listObj.value === 'list1').toBe(true);
        expect(
          (listObj as any).inputWrapper.container
  ...
  describe('EJ2-24995: re selected value not set', () => {
    ...
    it(' when datasource is observable using AsyncPipe', () => {
      ...
      listObj.showPopup();
      expect(
        (listObj as any).list.querySelectorAll('li').length === data1.length
      ).toBe(true);
  ...
  describe('BLAZ-762: float values are rounded to int', () => {
    ...
    it(' value select', () => {
      listObj.showPopup();
      let item: any = (listObj as any).popupObj.element.querySelectorAll(
        'li'
      )[1];
  ...
  describe('EJ2-36017: dropdown cascading', () => {
    ...
    it('previous value maintained', () => {
      ...
      listObj1.showPopup();
      let item1: any = (listObj1 as any).popupObj.element.querySelectorAll(
        'li'
      )[0];
      ...
      listObj2.showPopup();
      let item2: any = (listObj2 as any).popupObj.element.querySelectorAll(
        'li'
      )[0];
      ...
      listObj1.showPopup();
      let item3: any = (listObj1 as any).popupObj.element.querySelectorAll(
        'li'
      )[1];
      ...
      listObj1.onMouseClick(mouseEventArgs);
      listObj2.showPopup();
      let item4: any = (listObj2 as any).popupObj.element.querySelectorAll(
        'li'
      );
  ...
  describe('EJ2-26287: popup collision not working', () => {
    ...
    beforeAll(() => {
      ...
      let divEle: any = <HTMLInputElement>(
        createElement('div', { styles: 'height:700px; position: relative;' })
      );
  ...
  describe('EJ2-39447- Dropdownlist', () => {
    ...
    it('Filtering dropdown popup using paste testcase', () => {
      ...
      ddlObj.showPopup();
      expect((<any>ddlObj).list.querySelectorAll('li').length === 10).toBe(
        true
      );
      ...
      setTimeout(function () {
        expect((<any>ddlObj).list.querySelectorAll('li').length === 1).toBe(
          true
        );
  ...
  describe('EJ2-46380 - dynamic fields property changing', () => {
    let ddlObj: any;
    let ddlEle1: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'ddl1' })
    );
    let empList: any = [
      { id: 'level1', sports: 'American Football' },
      { id: 'level2', sports: 'Badminton' },
    ...
    it('field property changes on dynamically', () => {
      ddlObj.showPopup();
      expect(ddlObj.list.querySelectorAll('li')[2].textContent).toBe(
        'Basketball'
      );
      ddlObj.hidePopup();
      ...
      let item: any = (ddlObj as any).popupObj.element.querySelectorAll(
        'li'
      )[2];
      let mouseEventArgs: any = {
        preventDefault: function () {},
  ...
  describe('EJ2-41417', () => {
    ...
    var state = [
      { StateName: 'US 1', CountryId: '1', StateId: '101' },
      { StateName: 'Australia 2', CountryId: '2', StateId: '104' },
      { StateName: 'Australia 1 ', CountryId: '2', StateId: '105' },
      { StateName: 'Australia 3', CountryId: '2', StateId: '106' },
      { StateName: 'US 2 ', CountryId: '1', StateId: '102' },
      { StateName: 'US 3', CountryId: '1', StateId: '103' },
    ];
    let mouseEventArgs: any = { preventDefault: function () {}, target: null };
    let keyEventArgs: any = {
      preventDefault: (): void => {
    ...
    it('Popup list items is not updated properly in the cascading dropdown when filtered', () => {
      ...
      let newItem: any = (listObj1 as any).popupObj.element.querySelectorAll(
        'li'
      )[0];
  ...
  describe('EJ2-45073- Dropdownlist with select tag updating value wrongly when it contains Empty as inner text and “” as value', () => {
    let listObj: any;
    let element: string =
      "<select id='select1'><option value = '0'>option1</option><option value=''>Empty</option><option value='1'>Option3</option></select>";
    let keyEventArgs: any = {
      preventDefault: (): void => {
        /** NO Code */
      },
  ...
  describe('EJ2-48999- Improper data source values are loaded in the popup while modifying query property', () => {
    let listObj: any;
    let listObj1: any;
    let element: any = <HTMLInputElement>(
      createElement('input', { id: 'dropdownlist' })
    );
    let element1: any = <HTMLInputElement>(
      createElement('input', { id: 'dropdownlist1' })
    );
    let pred: Predicate = new Predicate(
      'CustomerID',
      'notequal',
      null,
      true,
      ...
      listObj = new DropDownList({
        ...
        created: function () {
          listObj1 = new DropDownList({
            ...
            open: (args) => {
              expect(
                (listObj1 as any).liCollections.length ==
                  (listObj as any).liCollections.length
              ).toBe(true);
              listObj1.hidePopup();
            },
          });
  ...
  describe('EJ2-56514', () => {
    ...
    let empList: any = [
      { id: 1, sports: 'American Football' },
      { id: 2, sports: 'Badminton' },
      { id: 3, sports: 'Badminton' },
      { id: 4, sports: 'Cricket' },
    ];
    beforeAll(() => {
      document.body.appendChild(ddlEle1);
