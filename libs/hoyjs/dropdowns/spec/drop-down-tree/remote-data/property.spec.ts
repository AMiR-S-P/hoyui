import { Browser, createElement, setCulture } from '@hoyui/base';
import { DropDownTree } from '../../../src/drop-down-tree/drop-down-tree';
import { DataManager, Query, ODataV4Adaptor } from '@hoyui/data';
import {
  remoteData,
  remoteData2,
  remoteData2_1,
...
xdescribe('DropDown Tree control Remote datasource', () => {
  describe('property testing', () => {
    describe('enableRtl', () => {
      ...
      let dataManager1: DataManager = new DataManager({
      ...
      it('functionality testing', (done: Function) => {
        expect(ddtreeObj.treeObj.element.classList.contains('e-rtl')).toBe(
          true
        );
        done();
      }, 100);
    ...
    describe('enabled', () => {
      ...
      it('functionality testing', (done: Function) => {
        ...
        ele.dispatchEvent(e);
        expect(
          ddtreeObj.element.parentElement.getAttribute('aria-expanded')
        ).toEqual('false');
        ddtreeObj.showPopup();
        expect(
          ddtreeObj.element.parentElement.getAttribute('aria-expanded')
    ...
    describe('zIndex', () => {
      ...
      it('functionality testing', (done: Function) => {
        ddtreeObj.showPopup();
        expect(
          (ddtreeObj as any).popupObj.element.style.zIndex === '1233'
        ).toBe(true);
        done();
      }, 100);
    });
    ...
    describe('showClearIcon', () => {
      ...
      it('functionality testing', (done: Function) => {
        expect(
          ddtreeObj.element.nextElementSibling.classList.contains(
            'e-clear-icon'
          )
        ).toBe(false);
        done();
    ...
    describe('sortOrder', () => {
      ...
      it('functionality testing', (done: Function) => {
        jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000;
        setTimeout(function () {
          expect(ddtreeObj.treeObj.element.querySelectorAll('li').length).toBe(
            2
          );
          expect(
    ...
    describe('headerTemplate', () => {
      ...
      beforeEach((done: Function) => {
        ...
        ddtreeObj = new DropDownTree({
          ...
          headerTemplate:
            '<div class="header"><span >HEADER VIA STRING</span></div>',
        });
        ddtreeObj.appendTo(ele);
        setTimeout(function () {
          done();
    ...
    describe('footerTemplate', () => {
      ...
      beforeEach((done: Function) => {
        ...
        ddtreeObj = new DropDownTree({
          ...
          footerTemplate:
            '<div class="header"><span >FOOTER VIA STRING</span></div>',
        });
        ddtreeObj.appendTo(ele);
        setTimeout(function () {
          done();
    ...
    describe('itemTemplate', () => {
      ...
      it('as script', (done: Function) => {
        let txt: Element[] = <Element[] & NodeListOf<Element>>(
          ddtreeObj.treeObj.element.querySelectorAll('.e-text-content')
        );
    ...
    describe('action Failure', () => {
      ...
      it('with default value', () => {
        ddtreeObj.showPopup();
        expect(
          ddtreeObj.popupObj.element.firstElementChild.classList.contains(
            'e-no-data'
          )
        ).toBe(true);
        expect(ddtreeObj.popupObj.element.firstElementChild.innerText).toBe(
          'The Request Failed'
        );
      });
    });
      ...
      beforeEach((done: Function) => {
        ...
        ddtreeObj = new DropDownTree({
          ...
          actionFailureTemplate:
            '<div><span>Server exception: 404 Not found</span></div>',
        });
        ddtreeObj.appendTo(ele);
        setTimeout(function () {
          done();
