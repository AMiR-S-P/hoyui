...
describe('DropDown Tree control hierarchical datasource', () => {
  describe('combinational testing', () => {
    ...
    it('for showCheckBox with autoCheck and value', () => {
      ...
      expect(ddtreeObj.text).toBe(
        'India,Brazil,Assam,Bihar,Tamil Nadu,Punjab,Paraná,Ceará,Acre'
      );
    });
    it('for showCheckBox with Delimiter mode and autocheck & loadOnDemand enabled', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        let ncheckEle: Element[] = <Element[] & NodeListOf<Element>>(
          ddtreeObj.treeObj.element.querySelectorAll('.e-checkbox-wrapper')
        );
    ...
    it('Enabling multiselect and using selectAll method and check for the select all checkbox state', () => {
      ...
      ddtreeObj.showPopup();
      expect(
        ddtreeObj.popupEle.querySelectorAll(
          '.e-selectall-parent[aria-checked=true]'
        ).length === 1
      ).toBe(true);
      ddtreeObj.hidePopup();
    ...
    it('updating Mode', () => {
      ...
      ddtreeObj.treeObj.touchClickObj.tap(tapEvent);
      expect(
        ddtreeObj.element.parentElement.classList.contains('e-show-chip')
      ).toBe(true);
    ...
    it('updating selectAll method with showCheckBox', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          var selectAllElement = ddtreeObj.popupEle.firstElementChild;
          expect(
            selectAllElement.classList.contains('e-selectall-parent')
          ).toBe(true);
          expect(
            selectAllElement.querySelector('.e-all-text').textContent
          ).toBe('Select All');
    ...
    it('showClearButton and allowMultiSelection enabled ', () => {
      ...
      expect(
        ddtreeObj.element.parentElement
          .querySelectorAll('.e-chips-wrapper')[0]
          .querySelector('.e-chips-close')
    ...
    it('IE browser testing', (done: Function) => {
      ...
      setTimeout(function () {
        ...
        ddtreeObj.showPopup();
        expect(
          ddtreeObj.element.parentElement.classList.contains('e-input-focus')
        ).toBe(true);
        var liEle = ddtreeObj.treeObj.element.querySelectorAll('li');
        mouseEventArgs.target = liEle[0].querySelector('.e-list-text');
        ddtreeObj.treeObj.touchClickObj.tap(tapEvent);
    ...
    it('for showDropDownIcon and placeholder', () => {
      ...
      expect(
        ddtreeObj.element.parentElement.classList.contains('e-input-group')
      ).toBe(true);
    });
  ...
  describe('combinational testing', () => {
    ...
    it('reset the form with multiple selection', (done) => {
      expect(ddtTreeObj.hiddenElement.querySelectorAll('option')[1].value).toBe(
        '21'
      );
      document.getElementById('resetForm').click();
      setTimeout(() => {
  ...
  describe('combinational testing', () => {
    ...
    it('htmlAttributes with aria-required attribute', () => {
      expect(
        ddTreeObj.hiddenElement.getAttribute('aria-required')
      ).not.toBeNull();
      expect(ddTreeObj.hiddenElement.getAttribute('aria-required')).toBe(
        'true'
      );
    });
  ...
  describe('Angular tag testing ', () => {
    let ddTreeObj: any;
    beforeAll(() => {
      let element: any = createElement('EJS-DROPDOWNTREE', {
        id: 'dropdowntree',
      });
      document.body.appendChild(element);
      ddTreeObj = new DropDownTree({
