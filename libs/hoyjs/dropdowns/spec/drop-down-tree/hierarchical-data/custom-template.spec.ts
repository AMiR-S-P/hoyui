...
describe('DropDown Tree control Hierarchical datasource', () => {
  ...
  describe('Custom Mode testing -', () => {
    ...
    it('With allowMultiSelection', () => {
      ...
      expect(
        ddtreeObj.element.parentElement.firstElementChild.classList.contains(
          'e-chips-wrapper'
        )
      ).toBe(true);
      let nchips =
        ddtreeObj.element.parentElement.firstElementChild.querySelectorAll(
          '.e-chips'
        );
    ...
    it('allowMultiSelection enabled dynamically', () => {
      ...
      ddtreeObj.dataBind();
      let chipElement =
        ddtreeObj.element.parentElement.firstElementChild.nextElementSibling;
      expect(chipElement.classList.contains('e-chips-wrapper')).toBe(true);
      let chips = chipElement.querySelectorAll('.e-chips');
      expect(chips.length).toBe(1);
