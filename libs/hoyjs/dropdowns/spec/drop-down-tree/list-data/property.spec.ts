import { createElement, L10n, setCulture } from '@hoyui/base';
...
describe('DropDown Tree control List datasource', () => {
  describe('Property testing', () => {
    ...
    it('for cssClass with popup', () => {
      ...
      expect(
        (ddtreeObj as any).element.parentElement.classList.contains(null)
      ).toBe(false);
      ddtreeObj.destroy();
      ddtreeObj = new DropDownTree(
        {
      ...
      expect(
        (ddtreeObj as any).element.parentElement.classList.contains(undefined)
      ).toBe(false);
      ddtreeObj.destroy();
      ddtreeObj = new DropDownTree(
        {
    ...
    it('checkDisabledChildren - ensure auto check on disabled Nodes', (done: Function) => {
      ddtreeObj = new DropDownTree(
        {
          ...
          showCheckBox: true,
          treeSettings: { checkDisabledChildren: false, autoCheck: true },
        },
        '#ddtree'
      );
      ddtreeObj.showPopup();
      let li: Element[] = <Element[] & NodeListOf<Element>>(
        ddtreeObj.popupObj.element.querySelectorAll('li')
      );
      expect(li[2].classList.contains('e-disable')).toBe(true);
      let checkEle: Element[] = <Element[] & NodeListOf<Element>>(
        ddtreeObj.popupObj.element.querySelectorAll('.e-checkbox-wrapper')
      );
      var e = new MouseEvent('mousedown', {
        view: window,
        bubbles: true,
      ...
      checkEle[0].querySelector('.e-frame').dispatchEvent(e);
      let checkedEle: Element[] = <Element[] & NodeListOf<Element>>(
        ddtreeObj.popupObj.element.querySelectorAll('.e-check')
      );
      expect(checkedEle.length).toBe(4);
      checkEle = <Element[] & NodeListOf<Element>>(
        ddtreeObj.popupObj.element.querySelectorAll('.e-checkbox-wrapper')
      );
      var e = new MouseEvent('mousedown', {
        view: window,
      ...
      checkEle[0].querySelector('.e-frame').dispatchEvent(e);
      checkedEle = <Element[] & NodeListOf<Element>>(
        ddtreeObj.popupObj.element.querySelectorAll('.e-check')
      );
      ...
      expect(
        (newli[5].querySelector('.e-icons') as Element).classList.contains(
          'e-icon-expandable'
        )
      ).toBe(true);
      expect(
      ...
      jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000;
      setTimeout(function () {
        let newCheckedEle: Element[] = <Element[] & NodeListOf<Element>>(
          ddtreeObj.popupObj.element.querySelectorAll('.e-check')
        );
    ...
    it('for zIndex', () => {
      ...
      ddtreeObj.showPopup();
      expect((ddtreeObj as any).popupObj.element.style.zIndex === '2000').toBe(
        true
      );
    });
      ...
      setTimeout(function () {
        ...
        expect(
          li[5]
            ...
            .classList.contains('e-icon-expandable')
        ).toBe(true);
        expect(
          li[5]
            .querySelector('.e-icons')
            .classList.contains('e-icon-collapsible')
        ).toBe(false);
    ...
    it('without mapping fields', () => {
      ...
      expect(
        (newli[2].querySelector('.e-icons') as Element).classList.contains(
          'e-icon-expandable'
        )
      ).toBe(false);
      expect(
      ...
      expect(
        (newli[3].querySelector('.e-icons') as Element).classList.contains(
          'e-icon-expandable'
        )
      ).toBe(false);
      expect(
      ...
      ddtreeObj = new DropDownTree(
        {
          ...
          showCheckBox: true,
          mode: 'Delimiter',
          selectAllText: 'check All Nodes',
          unSelectAllText: 'unCheck All Nodes',
        },
        '#ddtree'
      );
      let ele = ddtreeObj.element;
      ...
      ddtreeObj = new DropDownTree(
        {
          ...
          showCheckBox: true,
          mode: 'Delimiter',
          treeSettings: { autoCheck: false },
        },
        '#ddtree'
      );
      ddtreeObj.showPopup();
      let checkEle1: Element[] = <Element[] & NodeListOf<Element>>(
        ddtreeObj.treeObj.element.querySelectorAll('li')
      );
      var e = new MouseEvent('mousedown', {
        view: window,
    ...
    it('for footerTemplate support with script', (done: Function) => {
      ...
      footerTemplate.innerHTML =
        '<div class="header"><span >FOOTER Via SCRIPT</span></div>';
    ...
    it('for valueTemplate support with string', (done: Function) => {
      ddtreeObj = new DropDownTree(
        {
          ...
          value: ['01'],
          valueTemplate:
            '${if(nodeChild == undefined)}<b>${nodeText} template</b>${else}<i>${nodeText} template</i>${/if}',
        },
        '#ddtree'
      );
      jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000;
      setTimeout(function () {
        ddtreeObj.showPopup();
        let value: Element = <Element & NodeListOf<Element>>(
          ddtreeObj.element.parentElement.querySelector('.e-input-value')
        );
      ...
      ddtreeObj = new DropDownTree(
        {
          ...
          treeSettings: { loadOnDemand: false },
        },
        '#ddtree'
      );
      var li = (ddtreeObj as any).treeObj.element.querySelectorAll('li');
  ...
  describe('htmlAttributes testing', () => {
    ...
    it('functionaliy testing', () => {
      expect(
        ddTreeObj.hiddenElement.getAttribute('data-msg-container-id')
      ).not.toBeNull();
      expect(ddTreeObj.hiddenElement.getAttribute('data-required')).toBe(
        'name'
      );
      ...
      ddTreeObj = new DropDownTree({
        ...
        htmlAttributes: { class: 'myTree' },
      });
      ddTreeObj.appendTo(element);
      expect(ddTreeObj.element.parentElement.classList.contains('myTree')).toBe(
        true
      );
      ddTreeObj.destroy();
      ddTreeObj = new DropDownTree({
      ...
      ddTreeObj = new DropDownTree({
        ...
        htmlAttributes: { style: 'background: red' },
      });
      ddTreeObj.appendTo(element);
  ...
  describe('noRecordTemplate testing', () => {
    ...
    beforeAll(() => {
      setCulture('en-US');
      document.body.appendChild(element);
      let noRecordTemplate: Element = createElement('div', {
        id: 'noRecordTemplate',
      });
      noRecordTemplate.innerHTML =
        '<div><span>There is no record found</span></div>';
