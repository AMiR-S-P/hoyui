...
import {
  listData,
  hierarchicalData3,
} from '../../spec/drop-down-tree/dataSource.spec';
...
describe('DropDownTree control', () => {
  describe('DOM element testing', () => {
    ...
    beforeEach((): void => {
      ...
      let element: HTMLInputElement = <HTMLInputElement>(
        createElement('input', { id: 'default' })
      );
    ...
    it('with base class', () => {
      ...
      expect(
        document.getElementById('default').classList.contains('e-control')
      ).toEqual(true);
      expect(
        document.getElementById('default').classList.contains('e-dropdowntree')
      ).toEqual(true);
    });
    it('Element Structure', () => {
      ddTreeObj = new DropDownTree({}, '#default');
      expect(ddTreeObj.element.classList.contains('e-control')).toEqual(true);
      expect(ddTreeObj.element.classList.contains('e-dropdowntree')).toEqual(
        true
      );
      expect(
        ddTreeObj.element.parentElement.classList.contains('e-control-wrapper')
      ).toEqual(true);
      expect(
        ddTreeObj.element.parentElement.classList.contains('e-ddt')
      ).toEqual(true);
      expect(
        ddTreeObj.element.previousElementSibling.classList.contains(
          'e-ddt-hidden'
        )
      ).toBe(true);
  ...
  describe('Dropdown tree  default property testing without popup', () => {
    ...
    it('Default initialize', () => {
      ...
      expect(ddtreeObj.element.classList.contains('e-dropdowntree')).not.toBe(
        null
      );
    });
    it('Width', () => {
    ...
    it('showDropDownIcon', () => {
      ddtreeObj = new DropDownTree({}, '#ddtree');
      expect(
        (ddtreeObj as any).inputObj.buttons[0].classList.contains('e-ddt-icon')
      ).toBe(true);
    });
    it('showClearButton', () => {
      ddtreeObj = new DropDownTree({}, '#ddtree');
  ...
  describe('List property testing', () => {
    ...
    it('mouse up on clear icon of chip element testing', () => {
      ...
      var ele = ddtreeObj.element;
      var ttTarget = (ddtreeObj as any).element.parentElement.querySelectorAll(
        '.e-chips-close'
      )[1];
      var e = new MouseEvent('mousedown', {
        view: window,
    ...
    it('Selecting values using fullrow and clearing it', () => {
      ...
      let checkEle: Element[] = <Element[] & NodeListOf<Element>>(
        (ddtreeObj as any).treeObj.element.querySelectorAll(
          '.e-checkbox-wrapper'
        )
      );
      expect(
        checkEle[0].querySelector('.e-frame').classList.contains('e-check')
      ).toBe(true);
      var icon: HTMLElement = (ddtreeObj as any).element.nextElementSibling;
      var e = new MouseEvent('mousedown', {
        view: window,
      ...
      expect(
        ncheckEle[0].querySelector('.e-frame').classList.contains('e-check')
      ).toBe(true);
    });
    ...
    it('dropdown treeview mouse hover and mouse leave testing', () => {
      ...
      expect((ddtreeObj as any).inputEle.getAttribute('aria-expanded')).toBe(
        'true'
      );
      expect(
        document.querySelector('.e-popup').querySelector('.e-treeview')
      ).not.toBe(null);
      expect(
        (ddtreeObj as any).element.nextElementSibling.classList.contains(
          'e-clear-icon'
    ...
    it('Value changed dynamically while focus in - Delimiter', () => {
      ...
      expect((ddtreeObj as any).element.value).toBe('Australia');
      expect(
        (ddtreeObj as any).inputEle.classList.contains('e-chip-input')
      ).toBe(false);
      expect(
        (ddtreeObj as any).inputWrapper
          .querySelector('.e-overflow')
    ...
    it('empty value at initial rendering - Default', () => {
      ...
      var chipWrapper = (ddtreeObj as any).inputWrapper.querySelector(
        '.e-chips-wrapper'
      );
...
describe('Dropdown Tree With Id starts with number', () => {
  ...
  beforeEach((): void => {
    ...
    ddtreeObj = undefined;
    let ele: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: '11ddtree' })
    );
    document.body.appendChild(ele);
  });
  afterEach((): void => {
...
describe('Tab focus testing', () => {
  ...
  beforeEach((): void => {
    ...
    let ele1: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'ddtree1' })
    );
    document.body.appendChild(ele);
  ...
  it('tab key pressed', () => {
    ...
    expect(ddtreeObj.inputWrapper.classList.contains('e-input-focus')).toBe(
      true
    );
    expect(ddtreeObj1.inputWrapper.classList.contains('e-input-focus')).toBe(
      false
    );
  ...
  it('ensure Box mode', () => {
    ...
    ddtreeObj.appendTo('#ddtree');
    ddtreeObj.showPopup();
    expect(document.querySelectorAll('.e-chipcontent')[0].textContent).toBe(
      'New South Wales'
    );
    ddtreeObj.hidePopup();
  });
...
describe('Footer Template', () => {
  ...
  it('string footer template property', () => {
    ...
    ddtreeObj.appendTo('#ddtree');
    ddtreeObj.showPopup();
    expect(ddtreeObj.footerTemplate).toEqual(
      ddtreeObj.popupObj.element.lastChild.innerText
    );
    ddtreeObj.hidePopup();
  });
...
describe('DropdownTree', () => {
  ...
  it('aria-label attribute value changes dynamically', () => {
    ...
    ele.focus();
    ddtreeObj.showPopup();
    expect((ddtreeObj.element.parentElement as any).ariaLabel).toBe(
      'dropdowntree'
    );
    expect(
      ddtreeObj.element.nextElementSibling.classList.contains('e-icon-hide')
  ...
  it('aria-label attribute value changes dynamically when enabling showCheckBox', () => {
    ...
    ddtreeObj.showPopup();
    expect((ddtreeObj.element.parentElement as any).ariaLabel).toBe(
      'Victoria, Western Australia, Ceará'
    );
    ddtreeObj.value = ['1'];
    ddtreeObj.dataBind();
    expect((ddtreeObj as any).element.value).toBe('Australia');
