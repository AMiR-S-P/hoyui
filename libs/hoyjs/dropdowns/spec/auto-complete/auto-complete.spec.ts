...
import { DataManager, Query, ODataV4Adaptor, WebApiAdaptor } from '@hoyui/data';
...
describe('AutoComplete', () => {
  ...
  describe('Basic rendering', () => {
    ...
    it('check root component root class', () => {
      expect(atcObj.inputElement.classList.contains('e-autocomplete')).toBe(
        true
      );
    });
  ...
  describe('method testing ', () => {
    ...
    it('destroy method ', () => {
      ...
      atcObj.destroy = function () {
        expect(!!atcObj.element.classList.contains('e-autocomplete')).toBe(
          false
        );
      };
    });
  ...
  describe('Searching ', () => {
      ...
      it('customvalue with allowObjectBinding', (done) => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            expect(
              atcObj.isObjectInArray({ id: null, text: 'ASP' }, [atcObj.value])
            ).toBe(true);
    ...
    describe('internal searching with complex data', () => {
      ...
      it('press tab key while hide a popup', (done) => {
        ...
        setTimeout(() => {
          expect(
            atcObj.inputWrapper.container.classList.contains('e-input-focus')
          ).toEqual(true);
          done();
        }, 450);
      });
    ...
    it('highlight by StartsWith', (done) => {
      ...
      setTimeout(() => {
        let highlight: HTMLElement[] =
          atcObj.liCollections[0].querySelectorAll('.e-highlight');
  ...
  describe('AutoFill', () => {
    ...
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'ComboBox1' })
    );
    beforeAll(() => {
      document.body.appendChild(element);
      atcObj = new AutoComplete({
  ...
  describe('Spinner support', () => {
    ...
    describe('disable the clear button', () => {
      ...
      it(' - spinner show without enable the popup button', () => {
        ...
        expect(
          isNullOrUndefined(
            listObj.inputWrapper.container.querySelector('e-spinner-pane')
          )
        ).toBe(true);
      });
    });
  ...
  describe('nested data binding to fields', () => {
    ...
    it('mouse click on list with allowObjectBinding', (done) => {
      ...
      setTimeout(() => {
        let item: HTMLElement[] =
          list.popupObj.element.querySelectorAll('li')[3];
        mouseEventArgs.target = item;
        mouseEventArgs.type = 'click';
        list.onMouseClick(mouseEventArgs);
        setTimeout(function () {
  ...
  describe('ignoreAccent support', () => {
    ...
    it('search diacritics data', (done) => {
      ...
      setTimeout(() => {
        let item: HTMLElement[] =
          atcObj.popupObj.element.querySelectorAll('li');
  ...
  describe('Highlight search with HTML data: EJ2-857692', () => {
    ...
    let htmlData: string[] = [
      '<div class="e-item"><img src="https://upload.wikimedia.org/wikipedia/commons/1/11/Test-Logo.svg" /></div>',
    ];
    beforeAll(() => {
      document.body.appendChild(element);
      atcObj = new AutoComplete({
  ...
  describe('EJ2-21720 - custom value not maintain after the reload the data source ', () => {
    ...
    let data: string[] = ['Bad', 'ball'];
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'autocomplete' })
    );
    let btnEle: HTMLButtonElement = <HTMLButtonElement>(
      createElement('input', { id: 'btn', attrs: { type: 'button' } })
    );
    beforeAll(() => {
      document.body.appendChild(element);
...
describe('bug(EJ2-44058): When template is used, highlight is not working in the DropDown components', () => {
  ...
  let empList: { [key: string]: Object }[] = [
    ...
    {
      Name: '111Nancy Davolio',
      Eimg: '4',
      Designation: 'hahahProduct Manager111',
      Country: 'USA11',
    },
    {
      Name: 'Robert King',
  ...
  it('highlight by StartsWith', (done) => {
    atcObj = new AutoComplete({
      ...
      highlight: true,
      itemTemplate:
        '<div class="ename"> ${Name}</div><div class="job"> ${Designation} </div>',
    });
    atcObj.appendTo(element);
    atcObj.filterType = 'StartsWith';
    atcObj.dataBind();
...
describe('Select values using the up key', () => {
  let keyObj: any;
  let keyEle: HTMLInputElement = <HTMLInputElement>(
    createElement('input', { id: 'keyInput' })
  );
...
describe('Select values using the page up', () => {
  let keyObj1: any;
  let keyEle1: HTMLInputElement = <HTMLInputElement>(
    createElement('input', { id: 'keyInput' })
  );
  let dataSource = ['Java', 'JavaScript', 'Python', 'C#', 'Ruby', 'Perl'];
  let e: any = { preventDefault: function () {}, target: null, type: null };
...
describe('Select values using the page down', () => {
  let keyObj2: any;
  let keyEle2: HTMLInputElement = <HTMLInputElement>(
    createElement('input', { id: 'keyInput' })
  );
  let dataSource = ['Java', 'JavaScript', 'Python', 'C#', 'Ruby', 'Perl'];
  let e: any = { preventDefault: function () {}, target: null, type: null };
...
describe('EJ2-842578 - Check that the aria-owns attribute does indeed contain the correct ID for its popup list element', () => {
  let ariaObj: any;
  let ariaEle: HTMLInputElement = <HTMLInputElement>(
    createElement('input', { id: 'ariaInput' })
  );
  let dataSource = [
    'Java',
    'JavaScript',
  ...
  it('select the non-listed value with allowCustom as false', (done) => {
    ...
    setTimeout(() => {
      const element = document.querySelector(
        '.e-control.e-autocomplete.e-input'
      );
      expect(
        (element as any).getAttribute('aria-owns') === 'ariaInput_popup'
      ).toBe(true);
      done();
    }, 100);
  });
...
describe('keyboard interaction with disabled items', () => {
  ...
  it('all disabled items', (done) => {
    ...
    setTimeout(() => {
      ...
      expect(listObj.list.querySelectorAll('.e-item-focus').length === 0).toBe(
        true
      );
      keyEventArgs.action = 'down';
      listObj.keyActionHandler(keyEventArgs);
