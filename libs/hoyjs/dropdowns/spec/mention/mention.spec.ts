...
describe('Mention', () => {
  ...
  describe('Local data binding with default values for input', () => {
    ...
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'inputMention' })
    );
    beforeAll(() => {
  ...
  describe('Local data binding with default values for textarea', () => {
    ...
    let popupObj: any;
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('textarea', { id: 'inputMention' })
    );
    beforeAll(() => {
      document.body.appendChild(element);
      mentionObj = new Mention({ dataSource: datasource2 });
  ...
  describe('Local data binding with default values div', () => {
    ...
    let popupObj: any;
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('div', { id: 'divMention' })
    );
    beforeAll(() => {
  ...
  describe('Add multiple cssClass', () => {
    ...
    it('Dynamically change multiple cssClass', () => {
      ...
      mentionObj.initValue();
      mentionObj.showPopup();
      expect(mentionObj.popupObj.element.classList.contains('sample')).toEqual(
        true
      );
      mentionObj.cssClass = 'test highlight';
      mentionObj.dataBind();
      expect(mentionObj.popupObj.element.classList.contains('test')).toEqual(
        true
      );
      expect(
        mentionObj.popupObj.element.classList.contains('highlight')
      ).toEqual(true);
    });
  ...
  describe('Mention method"s testing ', () => {
    ...
    it('show popup method', (done) => {
      ...
      setTimeout(function () {
        ...
        expect(mentionObj.element.classList.contains('e-mention')).toEqual(
          true
        );
        mentionObj.showPopup();
        expect(mentionObj.isPopupOpen).toEqual(true);
  ...
  describe('Show popup onKeyup and navigating lists', () => {
    ...
    it('ShowPopup with @ as value and navigations between the lists', () => {
      ...
      mentionObj.keyActionHandler(keyEventArgs);
      expect(
        mentionObj.list.childNodes[0].childNodes[1].classList.contains(
          'e-active'
        )
      ).toBe(true);
    });
  ...
  describe('Show popup onKeyup with mentionChar changed dynamically', () => {
    ...
    it('ShowPopup with @ mention mentionChar changed dynamically', () => {
      ...
      mentionObj.onKeyUp(keyEventArgs);
      expect(document.querySelectorAll('.e-mention.e-popup').length > 0).toBe(
        true
      );
    });
  });
  ...
  describe('Show popup set and remove mouse hover', () => {
    ...
    it('for the LI element using mouse and methods', () => {
      ...
      expect(
        mentionObj.list.childNodes[0].childNodes[0].classList.contains(
          'e-hover'
        )
      ).toBe(true);
    ...
    it('Render the Meniton list with grouping and sortOrder:None', (done) => {
      ...
      mentions.opened = function (args: any) {
        let groupElemenet: HTMLElement =
          mentions.list.querySelector('.e-list-group-item');
  ...
  describe('GetItems Checking', () => {
    ...
    beforeAll(() => {
      element = <HTMLInputElement>(
        createElement('input', { id: 'inputMention1' })
      );
      document.body.appendChild(element);
      element1 = <HTMLInputElement>(
        createElement('input', { id: 'inputMention2' })
      );
  ...
  describe('Highlight Testing', () => {
    ...
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'Mention' })
    );
    beforeAll(() => {
      document.body.appendChild(element);
      mentionObj = new Mention({
    ...
    it('highlight by StartsWith', (done) => {
      ...
      setTimeout(() => {
        let highlight: HTMLElement[] =
          mentionObj.liCollections[0].querySelectorAll('.e-highlight');
    ...
    it('check highlight value in popup list', () => {
      ...
      mentionObj.inputElement.value = '@p';
      let highlight: HTMLElement =
        mentionObj.list.querySelector('.e-highlight');
  ...
  describe('Show popup onKeyUp and inserting texts in div', () => {
    ...
    it('insert the list item using mouseclick', () => {
      mouseEventArgs.target = mentionObj.list.childNodes[0].childNodes[2];
      mentionObj.onMouseClick(mouseEventArgs);
      expect(mentionObj.element.innerHTML).toBe(
        '<span contenteditable="false" class="e-mention-chip">@PERL</span>​'
      );
    });
  });
  ...
  describe('Dynamically enable the showMentionChar ', () => {
    ...
    it('insert the list item using mouseclick and check change event being triggered', () => {
      ...
      mentionObj.onMouseClick(mouseEventArgs);
      expect(mentionObj.element.innerHTML).toBe(
        '<p>@<span contenteditable="false" class="e-mention-chip">#PERL</span>​</p>'
      );
  ...
  describe('RemoteData loading', () => {
    ...
    it('check spinner show and hidden', (done) => {
      ...
      setTimeout(() => {
        ...
        expect(document.querySelectorAll('.e-spinner-inner').length > 0).toBe(
          true
        );
        done();
      }, 450);
  ...
  describe('Show popup onKeyup and inserting texts with target property set for div', () => {
    ...
    it('ShowPopup with @ as value', () => {
      ...
      expect(document.querySelector('.e-mention.e-popup')).not.toBe(null);
      expect(
        (document.querySelector('.e-mention.e-popup') as any)
          .querySelector('li')
  ...
  describe('Show popup onKeyup and inserting texts with target property set for textarea', () => {
    ...
    let popupObj: any;
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('textarea', { id: 'textareaMention' })
    );
    let divElement: HTMLDivElement = document.createElement('div');
    divElement.id = 'divElement';
    let mouseEventArgs: any = { preventDefault: function () {}, target: null };
  ...
  describe('Checking different dataSource binding', () => {
    let mentionObj: any;
    const empList: { [key: string]: Object }[] = [
      { id: 1, country: 'American Football' },
      { id: 2, country: 'Badminton' },
    ...
    it('Check if popup open or not for text as word', () => {
      ...
      mentionObj.onKeyUp(keyMentionEventArgs);
      expect(
        document.querySelectorAll('.e-mention.e-popup li').length > 0
      ).toBe(true);
    });
  });
  ...
  describe('Locale testing for norecordstemplate', () => {
    ...
    it('ShowPopup with norecordstemplate locale value', (done) => {
      ...
      mentionObj.onKeyUp(keyMentionEventArgs);
      expect(
        (document.querySelector('.e-nodata') as HTMLElement).innerText
      ).toEqual('Aucun enregistrement trouvé');
      done();
    });
  });
  ...
  describe('Accessibility testing', () => {
    ...
    it('Attributes checking for the mention and the popup elements', (done) => {
      ...
      expect(
        (
          mentionObj.popupObj.element.querySelector('ul') as HTMLElement
        ).hasAttribute('id')
      ).toBe(true);
      expect(
        (
      ...
      expect(
        (mentionObj.inputElement as HTMLElement).hasAttribute('aria-owns')
      ).toBe(true);
      expect(
        (mentionObj.inputElement as HTMLElement).hasAttribute(
  ...
  describe('Coverage improvements for mention component ', () => {
    ...
    it('- checkAndUpdateInternalComponent', () => {
      ...
      let textAreaelement: HTMLInputElement = <HTMLInputElement>(
        createElement('textarea', { id: 'mentiontextarea' })
      );
    ...
    it('- onmouseclick', () => {
      ...
      let mouseEventArgs: any = {
        preventDefault: function () {},
        target: (<any>listObj).popupObj.element.querySelector('li'),
      };
    ...
    it('- actioncomplete with isactive as true', () => {
      ...
      textAreaelement.focus();
      (<any>listObj).showPopup();
      (<any>listObj).onActionComplete(
        (<any>listObj).ulElement,
        (<any>listObj).list,
        keyEventArgs,
        true
      );
  ...
  describe('Showpopup with mention character as "a"', () => {
    ...
    it('insert the list item using mouseclick', () => {
      ...
      mentionObj.onMouseClick(mouseEventArgs);
      expect(mentionObj.element.innerHTML).toBe(
        '<span contenteditable="false" class="e-mention-chip">aPERL</span>​'
      );
      element.innerHTML =
        '<span contenteditable="false" class="e-mention-chip">aPERL</span>a';
      ...
      mentionObj.showPopup();
      element.innerHTML =
        '<span contenteditable="false" class="e-mention-chip">aPERL</span>aa';
  ...
  describe('EJ2-902235 Mention styles affect the Slash menu popup.', () => {
    ...
    beforeAll(() => {
      ...
      editor = mention.createElement('div', {
        id: 'defaultRTE_rte-edit-view',
        attrs: { contenteditable: 'true' },
      });
      const editorWrapper = mention.createElement('div', {
    ...
    it('Should have the input element id have slash menu', (done: DoneFn) => {
      ...
      mention.showPopup();
      setTimeout(() => {
        expect(((mention as any).popupObj.element as HTMLElement).id).toBe(
          'defaultRTE_rte-edit-view_slash_menu_popup'
        );
        expect(
          ((mention as any).popupObj.element as HTMLElement).querySelector('ul')
            .id
        ).toBe('defaultRTE_rte-edit-view_slash_menu_options');
        done();
      }, 300);
    });
  });
