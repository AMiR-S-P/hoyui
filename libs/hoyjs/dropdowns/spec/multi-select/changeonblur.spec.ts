...
describe('MultiSelect - changeonblur', () => {
  ...
  describe('CR issue - EJ2-17555 - validation issue', () => {
    ...
    let ele: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'textChild', attrs: { type: 'text' } })
    );
    beforeAll(() => {
    ...
    it('ensure change event', (done) => {
      listObj = new MultiSelect({
        ...
        open: () => {
          var mouseEventArgs = {
            preventDefault: function () {},
            currentTarget: (<any>listObj).checkBoxSelectionModule
              .checkAllParent,
          };
  ...
  describe('EJ2-20084 - Multiselect select all and un select all not working properly', () => {
    ...
    it('check select all without open', (done) => {
      listObj = new MultiSelect({
        ...
        showSelectAll: true,
        open: (): void => {
          expect((<any>listObj).list.querySelectorAll('.e-check').length).toBe(
            0
          );
          done();
        },
