...
} from '../../src/multi-select/multi-select';
import { Browser, isNullOrUndefined, EmitType } from '@hoyui/base';
...
import {
  ...
  PopupEventArgs,
  FocusEventArgs,
} from '../../src/drop-down-base/drop-down-base';
import {
  DataManager,
  ODataV4Adaptor,
  Query,
...
let datasource2: { [key: string]: Object }[] = [
  { id: 'id2', text: 'PHP' },
  { id: 'id1', text: 'HTML' },
  { id: 'id3', text: 'PERL' },
  { id: 'list1', text: 'JAVA' },
  { id: 'list2', text: 'Python' },
  { id: 'list5', text: 'Oracle' },
];
...
let multiSelectData: multiSelectStyles = {
  ...
  popupListWrapper:
    'e-ddl e-popup e-multi-select-list-wrapper e-control e-popup-open',
...
describe('MultiSelect', () => {
  ...
  let css: string =
    ".e-spinner-pane::after { content: 'Material'; display: none;} .e-multi-select-wrapper .e-multi-hidden {border: 0;height: 0;visibility: hidden; width: 0;}";
  let style: HTMLStyleElement = document.createElement('style');
  style.type = 'text/css';
  let styleNode: Node = style.appendChild(document.createTextNode(css));
  document.getElementsByTagName('head')[0].appendChild(style);
  ...
  describe('rendering validation', () => {
    ...
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'multiselect', attrs: { type: 'text' } })
    );
    beforeAll(() => {
    ...
    it('wrapper element - Box Mode', () => {
      ...
      let wrapper: HTMLElement = (<any>listObj).inputElement.parentElement
        .parentElement;
      if (!Browser.isDevice) {
        ...
        if (wrapper.firstChild) {
          ...
          expect(wrapper.firstElementChild.classList.toString()).toEqual(
            multiSelectData.selectedListContainer
          ); //5
          expect(wrapper.firstElementChild.childNodes.length).toEqual(1); //14
          if (wrapper.firstElementChild.childNodes.length) {
            expect(
              wrapper.firstElementChild.firstElementChild.nodeName
            ).toEqual('SPAN'); //15
            expect(
              wrapper.firstElementChild.firstElementChild.classList.toString()
            ).toEqual(multiSelectData.chips); //16
            expect(
              wrapper.firstElementChild.firstElementChild.lastElementChild
                .nodeName
            ).toEqual('SPAN'); //17
            expect(
              wrapper.firstElementChild.firstElementChild.lastElementChild.classList.toString()
            ).toEqual(multiSelectData.chipsClose); //18
          }
          if (wrapper.firstChild.nextSibling) {
            ...
            expect(
              wrapper.firstElementChild.nextElementSibling.classList.contains(
                'e-multiselect-box'
              )
            ).toEqual(true);
            wrapper.firstElementChild.nextElementSibling.classList.remove(
              'e-multiselect-box'
            );
            expect(
              wrapper.firstElementChild.nextElementSibling.classList.toString()
            ).toEqual(multiSelectData.inputContainer); //7
            if (wrapper.firstChild.nextSibling.nextSibling) {
              //wrapper element validation.
              expect(
                wrapper.firstChild.nextSibling.nextSibling.nodeName
              ).toEqual('SPAN'); //8
              expect(
                wrapper.firstElementChild.nextElementSibling.nextElementSibling.classList.toString()
              ).toEqual(multiSelectData.overAllClose); //9
            } else {
        ...
        expect((<any>listObj).inputElement.classList.toString()).toEqual(
          multiSelectData.inputElement
        ); //11
        for (let a = 0; a < multiSelectData.inputARIA.length; a++) {
          expect(
            (<any>listObj).inputElement.getAttribute(
              multiSelectData.inputARIA[a]
            )
          ).not.toBe(null); //12
        }
        expect((<any>listObj).inputElement.classList.toString()).toEqual(
    ...
    it('wrapper element - Delim Mode', () => {
      ...
      if (!Browser.isDevice) {
        ...
        expect(wrapper.nodeName).toEqual('DIV'); //1
        expect(wrapper.classList.toString()).toEqual(multiSelectData.container); //2
        expect(wrapper.childNodes.length).toEqual(
          multiSelectData.containerChildlength
        ); //3
        //Selected items list structure validation.
        //<span class="e-chips">sample1<span class="e-chips-close e-icon"></span></span>
        if (wrapper.firstChild) {
          expect(wrapper.firstChild.nodeName).toEqual('SPAN'); //4
          expect(wrapper.firstElementChild.classList.toString()).toEqual(
            multiSelectData.delimContainer
          ); //5
          expect(
            wrapper.firstElementChild.textContent.split(',').length
          ).toEqual(2); //14
          if (wrapper.firstChild.nextSibling) {
            //Input Wrapper structure validation.
            expect(wrapper.firstChild.nextSibling.nodeName).toEqual('SPAN'); //6
            expect(
              wrapper.firstElementChild.nextElementSibling.classList.toString()
            ).toEqual(multiSelectData.delimViewContainer); //7
            if (wrapper.firstChild.nextSibling.nextSibling) {
              //wrapper element validation.
              expect(
                wrapper.firstChild.nextSibling.nextSibling.nodeName
              ...
              expect(
                wrapper.firstElementChild.nextElementSibling.nextElementSibling.classList.toString()
              ).toEqual(multiSelectData.inputContainer); //9
              if (wrapper.firstChild.nextSibling.nextSibling.nextSibling) {
                //Close element validation.
                expect(
                  wrapper.firstChild.nextSibling.nextSibling.nextSibling
                    .nodeName
                ).toEqual('SPAN'); //8
                expect(
                  wrapper.firstElementChild.nextElementSibling.nextElementSibling.nextElementSibling.classList.toString()
                ).toEqual(multiSelectData.overAllClose); //9
              } else {
                expect(true).toBe(false);
              }
    ...
    it('wrapper element - Default Mode', () => {
      ...
      if (!Browser.isDevice) {
        ...
        expect(wrapper.classList.toString()).toEqual(multiSelectData.container); //2
        expect(wrapper.childNodes.length).toEqual(
          multiSelectData.defaultChildlength
        ); //3
        //Selected items list structure validation.
        //<span class="e-chips">sample1<span class="e-chips-close e-icon"></span></span>
        if (wrapper.firstChild) {
          ...
          expect(
            wrapper.firstElementChild.nextElementSibling.classList.toString()
          ).toEqual(multiSelectData.delimViewContainer); //5
          if (wrapper.firstChild.nextSibling) {
            //Input Wrapper structure validation.
            if (wrapper.firstChild.nextSibling.nextSibling) {
              //Close element validation.
    ...
    it('List Popup Element Validation.', () => {
      ...
      if (listWarapper) {
        ...
        if (listWarapper.firstElementChild) {
          //list container validation.
          expect(listWarapper.firstChild.nodeName).toEqual('DIV'); //21
          expect(listWarapper.firstElementChild.classList.toString()).toEqual(
            multiSelectData.listContainer
          ); //22
          if (listWarapper.firstElementChild.firstChild) {
            //list element validation.
            expect(listWarapper.firstElementChild.firstChild.nodeName).toEqual(
              'UL'
            ); //23
            expect(
              listWarapper.firstElementChild.firstElementChild.classList
                .toString()
                .trim()
            ).toEqual(multiSelectData.overAllList); //24
            for (let a = 0; a < multiSelectData.listARIA.length; a++) {
              expect(
                (<any>listObj).ulElement.getAttribute(
                  multiSelectData.listARIA[a]
                )
              ).not.toBe(null); //20
            }
            if (
              listWarapper.firstElementChild.firstElementChild.firstElementChild
            ) {
              expect(
                listWarapper.firstElementChild.firstChild.firstChild.nodeName
              ).toEqual('LI'); //25
              expect(
                listWarapper.firstElementChild.firstElementChild.firstElementChild.classList
                  .toString()
                  .trim()
              ).toEqual(multiSelectData.listItem); //26
            } else {
              expect(true).toBe(false);
            }
          } else {
    ...
    it('List Popup Element Validation step - 1.', () => {
      ...
      let listWarapper: HTMLElement = (<any>listObj).popupObj.element;
      if (listWarapper) {
        let listElement: HTMLElement = <HTMLElement>(
          listWarapper.querySelector('.e-list-parent')
        );
        expect(
          listElement.firstElementChild.classList.contains(
            multiSelectData.ListItemSelected
          )
        ).toBe(true); //28
        expect(
          listElement.firstElementChild.classList.contains(
            multiSelectData.ListItemHighlighted
          )
        ).toBe(true); //29
      } else {
        expect(true).toBe(false);
    ...
    it('Chip rendering for mobile scenarios:', () => {
      ...
      listObj.appendTo(element);
      let elem: HTMLElement = (<any>(
        listObj
      )).chipCollectionWrapper.querySelector('span[data-value="JAVA"]');
      ...
      (<any>listObj).onMobileChipInteraction({
        target: elem,
        preventDefault: function () {},
      });
    ...
    it('Chip rendering for mobile scenarios: readonly true', () => {
      ...
      expect((<HTMLElement>elem.lastElementChild).style.display).toBe('none');
      expect(elem.classList.contains(multiSelectData.chipSelection)).toBe(
        false
      );
      (<any>listObj).onMobileChipInteraction({
        target: elem,
  ...
  describe('Placeholder testing through inline', () => {
    ...
    it('Adding placeholder attribute through inline with float type auto', (done) => {
      ...
      listObj.appendTo(element);
      setTimeout(() => {
        let floatElement = (listObj as any).componentWrapper.querySelector(
          '.e-float-text'
        );
    ...
    it('Adding placeholder attribute through inline with float type never', () => {
      ...
      listObj.appendTo(element);
      expect((<any>listObj).inputElement.getAttribute('placeholder')).toBe(
        'Select a game'
      );
    });
  });
  describe('Placeholder testing through inline and API', () => {
    ...
    it('Adding placeholder attribute through API at initial rendering with float type auto', () => {
      ...
      listObj.appendTo(element);
      expect((<any>listObj).inputElement.hasAttribute('placeholder')).toBe(
        false
      );
      let floatElement = (listObj as any).componentWrapper.querySelector(
        '.e-float-text'
  ...
  describe('Angular tag testing ', () => {
    ...
    it('Wrapper testing ', () => {
      ...
      expect(
        (<HTMLElement>listObj.element).contains(listObj.overAllWrapper)
      ).toBe(true);
  ...
  describe('Add multiple CssClass', () => {
    ...
    it('Dynamically change multiple cssClass', () => {
      ...
      listObj.appendTo(element);
      expect(
        (<any>listObj).overAllWrapper.classList.contains('sample')
      ).toEqual(true);
      expect((<any>listObj).popupWrapper.classList.contains('sample')).toBe(
        true
      );
      listObj.cssClass = 'test highlight';
      listObj.dataBind();
      expect((<any>listObj).overAllWrapper.classList.contains('test')).toEqual(
        true
      );
      expect(
        (<any>listObj).overAllWrapper.classList.contains('highlight')
      ).toEqual(true);
      expect((<any>listObj).overAllWrapper.classList.contains('test')).toEqual(
        true
      );
    ...
    it('Initially render multiple cssClass', () => {
      ...
      expect((<any>listObj).popupWrapper.classList.contains('highlight')).toBe(
        true
      );
  ...
  describe('Property validation on initial render', () => {
    ...
    it('validate dimention APIs', (done) => {
      ...
      datasource33.push({
        id: 'list6',
        text: 'Oracle_Java_C#_Python_Flask_DJango',
      });
      L10n.load({
        'fr-BE': {
          dropdowns: {
      ...
      listObj.change = function () {
        expect(
          (<HTMLElement>(<any>listObj).viewWrapper).childElementCount
        ).toBe(1);
        listObj.destroy();
        done();
      };
    ...
    it('validate dimention APIs', () => {
      ...
      let wrapper: HTMLElement = (<any>listObj).inputElement.parentElement
        .parentElement;
      if (wrapper) expect(wrapper.getBoundingClientRect().width).toBe(300); //31
      ...
      listObj.width = '200px';
      listObj.dataBind();
      if (wrapper) expect(wrapper.getBoundingClientRect().width).toBe(200); //31
      else expect(true).toBe(false);
      if (listWarapper) {
        let listElement: HTMLElement = <HTMLElement>(
          listWarapper.querySelector('.e-list-parent')
    ...
    it('Validating the Delimeter Cahr', () => {
      ...
      listObj.appendTo(element);
      expect((<any>listObj).delimiterWrapper.innerHTML.trim()).toBe(
        'PHP, JAVA,'
      );
    ...
    it('validate  on render API-placeholder', () => {
      ...
      if (
        ...
        expect(
          (<any>listObj).inputElement.getAttribute('placeholder')
        ).not.toBe(null); //35
      } else expect(true).toBe(false);
    ...
    it('cssClass ', () => {
      ...
      listObj.dataBind();
      expect(
        (<any>listObj).overAllWrapper.classList.contains('closeState')
      ).toEqual(true); //27
      //popupWrapper
      expect((<any>listObj).popupWrapper.classList.contains('closeState')).toBe(
        true
      ); //37
      ...
      expect(
        (<any>listObj).overAllWrapper.classList.contains('CloseSet')
      ).toEqual(true); //27
      //popupWrapper
      expect((<any>listObj).popupWrapper.classList.contains('CloseSet')).toBe(
        true
      ); //37
    ...
    it('htmlAttributes', () => {
      ...
      listObj.dataBind();
      expect((<any>listObj).hiddenElement.getAttribute('name')).toBe(
        'dropdown'
      ); //Need tp add it to the select element/
    ...
    it('showClearButton false', () => {
      ...
      listObj.dataBind();
      expect(
        (<any>listObj).componentWrapper.classList.contains(
          multiSelectData.closeiconhide
        )
      ).toBe(true);
      listObj.destroy();
    });
    ...
    it('allowCustomValue.', () => {
      ...
      (<any>listObj).onMouseClick(mouseEventArgs);
      expect(
        (<any>listObj).value && (<any>listObj).value.length
      ).not.toBeNull();
      listObj.destroy();
    });
    ...
    it('readonly.', () => {
      ...
      listObj.dataBind();
      expect((<any>listObj).inputElement.hasAttribute('readonly')).not.toEqual(
        null
      ); //52
      ...
      listObj.dataBind();
      expect((<any>listObj).inputElement.getAttribute('readonly')).toEqual(
        null
      ); //54
      listObj.destroy();
    });
    ...
    it('enabled ', (done) => {
      ...
      expect(
        (<any>listObj).overAllWrapper.classList.contains('e-disabled')
      ).toEqual(true); //55
      expect((<any>listObj).inputElement.getAttribute('aria-disabled')).toEqual(
        'true'
      );
      expect((<any>listObj).inputElement.getAttribute('disabled')).not.toBe(
        null
      );
      listObj.value = ['JAVA'];
      listObj.dataBind();
      ...
      setTimeout(function () {
        ...
        expect(
          (<any>listObj).inputElement.getAttribute('aria-disabled')
        ).toEqual('false');
    ...
    it('Hover event validation', () => {
      ...
      listObj.appendTo(element);
      listObj.showPopup();
      let element1: HTMLElement = (<any>listObj).list.querySelector(
        'li[data-value="JAVA"]'
      );
      expect(element1.classList.contains(dropDownBaseClasses.hover)).toBe(
        false
      );
      ...
      listObj.enabled = false;
      (<any>listObj).onMouseOver(mouseEventArgs);
      expect(element1.classList.contains(dropDownBaseClasses.hover)).not.toBe(
        true
      );
      listObj.destroy();
    });
    ...
    it('select event validation', () => {
      ...
      expect(element.classList.contains(dropDownBaseClasses.selected)).toBe(
        false
      );
      mouseEventArgs.target = element1;
      mouseEventArgs.type = 'click';
      (<any>listObj).onMouseClick(mouseEventArgs);
      expect(element1.classList.contains(dropDownBaseClasses.selected)).toBe(
        true
      );
      (<any>listObj).onMouseClick(mouseEventArgs);
      expect(element1.classList.contains(dropDownBaseClasses.selected)).toBe(
      ...
      let elements: HTMLElement[] = (<any>listObj).list.querySelectorAll(
        'li[data-value]'
      );
    ...
    it('select event validation with mouse', () => {
      listObj = new MultiSelect({
        ...
        filtering: function (e) {
          ...
          query =
            e.text !== ''
              ? query.where('text', 'startswith', e.text, true)
              : query;
    ...
    it('filtering with same selected value', () => {
      ...
      (<any>listObj).keyUp(keyboardEventArgs);
      expect(
        (<any>listObj).list.classList.contains(dropDownBaseClasses.noData)
      ).toBe(false);
      listObj.destroy();
    });
    ...
    it('filtering basic coverage', () => {
      ...
      (<any>listObj).onInput();
      (<any>listObj).keyUp(keyboardEventArgs);
      let elem: HTMLElement[] = (<any>listObj).list.querySelectorAll(
        'li.' + dropDownBaseClasses.focus
      );
    ...
    it('filtering methode', () => {
      ...
      let elem: HTMLElement[] = (<any>listObj1).list.querySelectorAll(
        'li.' + dropDownBaseClasses.focus
      );
      expect(elem.length).toBe(0);
      listObj1.destroy();
    ...
    it('clear event validation', () => {
      ...
      listObj.showPopup();
      (<any>listObj).inputFocus = true;
      elem = (<any>listObj).chipCollectionWrapper.querySelector(
        'span[data-value="PHP"]'
      );
      (<any>listObj).onChipRemove({
        which: 1,
    ...
    it('List hover event validation', () => {
      ...
      (<any>listObj).showPopup();
      (<any>listObj).windowResize();
      expect((<any>listObj).popupWrapper.getBoundingClientRect().width).toBe(
        400
      ); //32
    ...
    it('Multiselect-Chip interaction validation', () => {
      listObj = new MultiSelect({
        ...
        fields: { text: 'text', value: 'text' },
        closePopupOnSelect: true,
        value: ['JAVA', 'Python', 'Oracle', 'HTML', 'PHP'],
      ...
      let elem: HTMLElement[] = (<any>(
        listObj
      )).chipCollectionWrapper.querySelectorAll(
        'span.' + multiSelectData.chips
      );
      //left Validation
      ...
      (<any>listObj).onKeyDown(keyboardEventArgs);
      expect(
        elem[elem.length - 1].classList.contains(multiSelectData.chipSelection)
      ).toBe(true); //1
      (<any>listObj).onKeyDown(keyboardEventArgs);
      expect(
        elem[elem.length - 2].classList.contains(multiSelectData.chipSelection)
      ).toBe(true); //2
      (<any>listObj).onKeyDown(keyboardEventArgs);
      expect(
        elem[elem.length - 3].classList.contains(multiSelectData.chipSelection)
      ).toBe(true); //3
      (<any>listObj).onKeyDown(keyboardEventArgs);
      expect(
        elem[elem.length - 4].classList.contains(multiSelectData.chipSelection)
      ).toBe(true); //4
      (<any>listObj).onKeyDown(keyboardEventArgs);
      expect(
        elem[elem.length - 5].classList.contains(multiSelectData.chipSelection)
      ).toBe(true); //5
      (<any>listObj).onKeyDown(keyboardEventArgs);
      expect(
        elem[elem.length - 5].classList.contains(multiSelectData.chipSelection)
      ...
      expect(
        (<any>listObj).chipCollectionWrapper.querySelectorAll(
          'span.' + multiSelectData.chipSelection
        ).length
      ).toBe(0); //7
      ...
      listObj.appendTo(element);
      mouseEventArgs.target = (<any>(
        listObj
      )).chipCollectionWrapper.querySelector('span.' + multiSelectData.chips);
      mouseEventArgs.type = 'click';
      (<any>listObj).chipClick(mouseEventArgs);
      let chipSelected = (<any>listObj).chipCollectionWrapper.querySelector(
        'span.' + multiSelectData.chipSelection
      );
      expect(chipSelected).not.toEqual(null);
      listObj.setProperties({ enabled: false });
      (<any>listObj).chipClick(mouseEventArgs);
      chipSelected = (<any>listObj).chipCollectionWrapper.querySelector(
        'span.' + multiSelectData.chipSelection
      );
      expect(chipSelected).not.toEqual(null);
      listObj.destroy();
    ...
    it('Multiselect-popup interaction validation', () => {
      ...
      listObj.selectAll(true);
      listObj.showPopup();
      let element1: HTMLElement = <HTMLElement>(
        (<any>listObj).ulElement.querySelector('li[data-value="PHP"]')
      );
      keyboardEventArgs.altKey = false;
      keyboardEventArgs.keyCode = 13;
      expect(
        (<HTMLElement>(
          (<any>listObj).ulElement.querySelector('li[data-value="HTML"]')
        )).classList.contains('e-hide-listitem')
      ).toBe(true);
      (<any>listObj).onKeyDown(keyboardEventArgs);
      ...
      expect(
        (<HTMLElement>(
          (<any>listObj).ulElement.querySelector('li[data-value="Python"]')
        )).classList.contains('e-hide-listitem')
      ).toBe(true);
      listObj.destroy();
    });
    ...
    it('Multiselect input interaction validation', () => {
      ...
      (<any>listObj).onInput();
      (<any>listObj).keyUp(keyboardEventArgs);
      expect((<any>listObj).list.querySelector('li[data-value="JAVA"]')).toBe(
        (<any>listObj).list.querySelector('li.' + dropDownBaseClasses.focus)
      );
      (<any>listObj).inputElement.value = 'Python';
      (<any>listObj).keyDownStatus = true;
      (<any>listObj).onInput();
      (<any>listObj).keyUp(keyboardEventArgs);
      expect((<any>listObj).list.querySelector('li[data-value="Python"]')).toBe(
        (<any>listObj).list.querySelector('li.' + dropDownBaseClasses.focus)
      );
      listObj.destroy();
    });
  ...
  describe('Remote data binding - with-out initial Value', () => {
    ...
    let popupObj: any;
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'multiselect' })
    );
    let remoteData: DataManager = new DataManager({
      url: 'https://services.syncfusion.com/js/production/api/Employees',
      adaptor: new WebApiAdaptor(),
  ...
  describe('Remote data binding - action failure', () => {
    ...
    beforeAll((done) => {
      ...
      originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
      jasmine.DEFAULT_TIMEOUT_INTERVAL = 10000;
      let remoteData1: DataManager = new DataManager({
        url: '/api/dummy',
        adaptor: new ODataV4Adaptor(),
      });
      listObj = new MultiSelect({
        hideSelectedItem: false,
        dataSource: remoteData1,
        value: [1004],
        fields: { value: 'text', text: 'text' },
      });
      listObj.appendTo(element);
    ...
    it('no data text when ajax failure', (done) => {
      ...
      setTimeout(() => {
        ...
        listObj.dataBind();
        expect((<any>listObj).list.classList.contains('e-nodata')).not.toBe(
          true
        );
        listObj.destroy();
        done();
  ...
  describe('Validation for public methods.', () => {
    ...
    it('showPopup & hidePopup Method.', () => {
      ...
      listObj.appendTo(element);
      let listWarapper: HTMLElement = <HTMLElement>(
        document.querySelector('#multiselect_popup')
      );
      listObj.hidePopup();
  ...
  describe('templating behavior validation', () => {
    ...
    it('Validation for the group template', () => {
      let listObj: MultiSelect = new MultiSelect({
        ...
        headerTemplate:
          '<div class="head">Photo<span style="padding-left:42px">Contact Info</span></div>',
        itemTemplate:
          '<div><img class="eimg" src="https://ej2.syncfusion.com/demos/src/drop-down-list/Employees/${eimg}.png" alt="employee"/>' +
          '<div class="ename"> ${text} </div><div class="temp"> ${country} </div></div>',
        footerTemplate: '<div class="Foot"> Total Items Count: 5 </div>',
        valueTemplate:
          '<span><img class="tempImg" src="https://ej2.syncfusion.com/demos/src/drop-down-list/Employees/${eimg}.png" height="20px" width="20px" alt="employee"/>' +
      ...
      expect(
        '<div class="head">Photo<span style="padding-left:42px">Contact Info</span></div>'
      ).toBe((<any>listObj).header.innerHTML);
      expect('<div class="Foot"> Total Items Count: 5 </div>').toBe(
        (<any>listObj).footer.innerHTML
      );
      expect(
        '<div><img class="eimg" src="https://ej2.syncfusion.com/demos/src/drop-down-list/Employees/1.png" alt="employee"><div class="ename"> Mona Sak </div><div class="temp"> USA </div></div>'
      ).toBe(
        (<any>listObj).ulElement.querySelector('li.e-list-item').innerHTML
      );
      mouseEventArgs.target = (<any>listObj).ulElement.querySelector(
        'li.e-list-item'
      );
      mouseEventArgs.type = 'click';
      (<any>listObj).onMouseClick(mouseEventArgs);
      let elem: HTMLElement = (<any>(
        listObj
      )).chipCollectionWrapper.querySelector('span.e-chipcontent');
      expect(
        '<span><img class="tempImg" src="https://ej2.syncfusion.com/demos/src/drop-down-list/Employees/1.png" height="20px" width="20px" alt="employee"><span class="tempName"> Mona Sak </span></span>'
      ).toBe(elem.innerHTML);
      mouseEventArgs.target = (<any>listObj).popupWrapper.querySelector(
        '.head'
      );
      (<any>listObj).onMouseClick(mouseEventArgs);
      listObj.destroy();
  ...
  describe('Floating label', () => {
    ...
    it('floating-Auto: checking value property', () => {
      ...
      listObj.value = <string[]>[];
      listObj.dataBind();
      floatElement = (listObj as any).componentWrapper.querySelector(
        '.e-float-text'
      );
      expect(floatElement.classList.contains('e-label-bottom')).toBe(true);
    });
  ...
  describe('Spinner support', () => {
    ...
    it(' - spinner show instead of clear icon at initial time', () => {
      ...
      expect(
        isNullOrUndefined(
          listObj.overAllWrapper.querySelector('e-spinner-pane')
        )
      ).toBe(true);
    });
  });
  ...
  describe('Add item using addItem method in existing group item', () => {
    ...
    it('Adding item in the existing group', () => {
      ...
      expect(multiObj.ulElement.querySelectorAll('li').length === 11).toBe(
        true
      );
    });
  });
  describe('Add item using addItem method in new group item', () => {
    ...
    it('filtering basic coverage', () => {
      ...
      expect(multiObj.ulElement.querySelectorAll('li').length === 9).toBe(true);
      multiObj.addItem(item);
      expect(multiObj.ulElement.querySelectorAll('li').length === 12).toBe(
        true
      );
    });
  });
  ...
  describe('mulitselect enable and refresh method', () => {
    ...
    it('enabled the component', (done) => {
      (multiObj.enabled = true), multiObj.showPopup();
      setTimeout(() => {
        expect(
          multiObj.list.getElementsByClassName('e-list-item e-active')
            .length === 3
        ).toBe(true);
        done();
      }, 400);
  ...
  describe('EJ2-245633-ClearIcon is enabled while setting readonly property in mutliselect', () => {
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'dropdown' })
    );
    let dropDowns: any;
    beforeAll(() => {
      document.body.appendChild(element);
  ...
  describe('mulitselect chip select event-EJ2-7802', () => {
    ...
    it('chip select event trigger', () => {
      (<any>multiObj).dispatchEvent(
        multiObj.chipCollectionWrapper.firstElementChild,
        'mousedown'
      );
  ...
  describe('ignoreAccent support', () => {
    ...
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'autocomplete' })
    );
    ...
    it('search diacritics data', (done) => {
      ...
      setTimeout(() => {
        let item: HTMLElement[] =
          comboObj.popupObj.element.querySelectorAll('li');
  ...
  describe('mulitselect openOnClick API validation', () => {
    ...
    it('openOnClick', () => {
      ...
      expect(document.body.contains((<any>multiObj).popupObj.element)).toBe(
        false
      );
      ...
      let element1: HTMLElement = (<any>multiObj).list.querySelector(
        'li[data-value="JAVA"]'
      );
      expect(element1.classList.contains(dropDownBaseClasses.selected)).toBe(
        false
  ...
  describe('list items focus', () => {
    ...
    it('wrongly focused item', () => {
      ...
      expect(
        (<any>multiObj).list.querySelector('.e-list-item.e-item-focus')
      ).toBe(null);
  ...
  describe(' bug(EJ2-8805): htmlAttributes properties are set into disabled input element.', () => {
    ...
    it(' set the attributes to corresponding element', () => {
      dropDowns = new MultiSelect({
        dataSource: ['Java Script', 'AS.NET MVC'],
        htmlAttributes: {
          title: 'Select Multiple value',
          id: 'dropdown',
          form: 'formname',
        },
      });
      dropDowns.appendTo(element);
      expect(dropDowns.hiddenElement.getAttribute('form') === 'formname').toBe(
        true
      );
      expect(dropDowns.element.getAttribute('id') === 'dropdown').toBe(true);
      expect(
        dropDowns.overAllWrapper.getAttribute('title') ===
    ...
    it(' set the inbuilt validation attributes in input', () => {
      ...
      expect(dropDowns1.hiddenElement.getAttribute('required') === 'true').toBe(
        true
      );
      expect(dropDowns1.hiddenElement.getAttribute('form') === 'formName').toBe(
        true
      );
      expect(
        dropDowns1.hiddenElement.getAttribute('aria-required') === 'required'
      ).toBe(true);
      dropDowns1.destroy();
    });
  });
  describe('page up first list item focus', () => {
    ...
    it('first list item focus', (done) => {
      ...
      setTimeout(() => {
        ...
        (<any>multiObj).onKeyDown(keyboardEventArgs);
        expect(
          multiObj.ulElement.querySelectorAll('li.e-item-focus')[0]
            .textContent === 'Oracle'
        ).toBe(true);
        keyboardEventArgs.keyCode = 33;
        (<any>multiObj).onKeyDown(keyboardEventArgs);
  ...
  describe('dynamic change sortorder', () => {
    ...
    it('sortorder changed', () => {
      ...
      multiObj.showPopup();
      expect(
        multiObj.ulElement.querySelector('li').textContent === 'Python'
      ).toBe(true);
      multiObj.hidePopup();
      ...
      multiObj.showPopup();
      expect(
        multiObj.ulElement.querySelector('li').textContent === 'HTML'
      ).toBe(true);
      (<any>multiObj).destroy();
    });
  });
  ...
  describe(' bug(EJ2-8802): No Records data is visible after clear the selected value.', () => {
    ...
    it(' close the popup while press the tab key', (done) => {
      ...
      setTimeout(() => {
        ...
        expect(
          dropDowns.list.classList.contains(dropDownBaseClasses.noData)
        ).toBe(true);
  ...
  describe('GetItems related bug', () => {
    ...
    beforeAll(() => {
      ...
      element1 = <HTMLInputElement>(
        createElement('input', { id: 'multiSelect1' })
      );
  ...
  describe('Boolean value support', () => {
    ...
    let data: boolean[] = [true, false];
    let ddl: any;
    let jsonData: { [key: string]: Object }[] = [
      { id: false, text: 'failure' },
      { id: true, text: 'success' },
    ];
    beforeAll(() => {
      element = <HTMLInputElement>(
        createElement('input', { id: 'dropdownlist' })
      );
      document.body.appendChild(element);
    });
    afterAll(() => {
  ...
  describe('Check end of Space value select', () => {
    ...
    beforeAll(() => {
      ...
      selectElement = <HTMLDivElement>(
        createElement('div', { id: 'multiselect2' })
      );
  ...
  describe('EJ2-13211 - remote selection not maintain', () => {
    ...
    let datasource: { [key: string]: Object }[] = [
      { id: 'level1', sports: 'American Football' },
      { id: 'level2', sports: 'Badminton' },
      { id: 'level3', sports: 'Basketball' },
      { id: 'level4', sports: 'Cricket' },
      { id: 'level5', sports: 'Football' },
      { id: 'level6', sports: 'Golf' },
      { id: 'level7', sports: 'Hockey' },
      { id: 'level8', sports: 'Rugby' },
      { id: 'level9', sports: 'Snooker' },
      { id: 'level10', sports: 'Tennis' },
    ];
    ...
    it('bug(EJ2-13211): ensure list scroll', () => {
      expect((<any>listObj).list.querySelector('.e-active').innerText).toBe(
        'Tennis'
      );
      listObj.hidePopup();
    });
    ...
    it('bug(EJ2-14587): ensure showDropDownIcon - popup open', (done) => {
      ...
      let iconEle: HTMLElement = <HTMLElement>(
        dropEle.querySelector('.e-ddl-icon')
      );
    ...
    it('bug(EJ2-14587): ensure showDropDownIcon', () => {
      ...
      expect(!isNullOrUndefined(dropEle.querySelector('.e-ddl-icon'))).toBe(
        true
      );
    });
  });
  ...
  describe('EJ2-21465 - Data attribute validation is not working in multiselect', () => {
    ...
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', {
        id: 'multiselect',
        attrs: { type: 'text', 'data-val': 'true', 'aria-disabled': 'false' },
      })
    );
    let datasource: { [key: string]: Object }[] = [
      { Id: 1, item: 'Fruits and Vegetables' },
    ...
    it('Check data attribute value', () => {
      expect((<any>listObj).hiddenElement.getAttribute('data-val')).not.toBe(
        null
      );
    });
  ...
  describe('Bootstrap model placeholder length', () => {
    ...
    it('Lengthy placeholder', () => {
      listObj = new MultiSelect({
        hideSelectedItem: false,
        dataSource: datasource2,
        placeholder: 'select counties Select or search maximum 8 players',
        showDropDownIcon: true,
        width: 300,
      });
      listObj.appendTo(element);
      ...
      expect(
        (listObj as any).searchWrapper.classList.contains(
          'e-search-custom-width'
        )
      ).toBe(true);
      listObj.destroy();
    });
  ...
  describe('EJ2-13148 - Multiselect key navigation is not working with Home , Endkeys', () => {
    ...
    it('Check End key Navigation', (done) => {
      listObj = new MultiSelect({
        ...
        popupHeight: 50,
        change: (): void => {
          (<any>listObj).onKeyDown({
            keyCode: 35,
            preventDefault: function () {},
          });
          let ele: HTMLElement =
            listObj.ulElement.querySelector('.e-item-focus');
  ...
  describe('EJ2-19524 - UI breaking when use lengthy place holder', () => {
    ...
    it('Lengthy placeholder when input is empty and focusout', () => {
      listObj = new MultiSelect({
        ...
        dataSource: datasource2,
        placeholder:
          'select counties Select or search maximum 8 playersssssssssssssssss',
        showDropDownIcon: true,
        width: 300,
      });
      listObj.appendTo(element);
      ...
      if (
        ...
        expect(getComputedStyle((<any>listObj).searchWrapper).width).toBe(
          'calc(100% - 20px)'
        );
      } else expect(true).toBe(false);
      listObj.destroy();
    ...
    it('Lengthy placeholder when input given & focusout', () => {
      ...
      if (
        ...
        expect(
          (<any>listObj).searchWrapper.classList.contains('e-zero-size')
        ).toBe(true);
      } else expect(true).toBe(false);
      listObj.destroy();
    });
    ...
    it('Removing individual chip', () => {
      ...
      (<any>listObj).onChipRemove({
        ...
        target: document.querySelector(
          '.e-chips-collection .e-chips .e-chips-close'
        ),
      });
      let wrapper: HTMLElement = (<any>listObj).inputElement.parentElement
        .parentElement;
  ...
  describe('EJ2-22960 - Exception throws while use datasource string inside string', () => {
    let listObj: MultiSelect;
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'license', attrs: { type: 'text' } })
    );
    let data: { [key: string]: Object }[] = [
      { id: 'list1', text: '"JAVA"', icon: 'icon' },
      { id: 'list2', text: 'C#' },
    ...
    it('remove Value selection', (done) => {
      ...
      listObj.focus = (): void => {
        let closeELe: HTMLElement = document.querySelector(
          '.e-chips .e-chips-close'
        );
        let clickEvent: MouseEvent = document.createEvent('MouseEvents');
        clickEvent.initEvent('mousedown', true, true);
  ...
  describe('EJ2-23146 - floating label misplaced when it is focused', () => {
    let listObj: MultiSelect;
    let divElement: HTMLDivElement = <HTMLDivElement>(
      createElement('div', { id: 'licensediv' })
    );
    let element: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'license', attrs: { type: 'text' } })
    );
    ...
    it('remove Value selection', (done) => {
      listObj.removed = (): void => {
        expect(divElement.querySelector('.e-float-text.e-label-top')).not.toBe(
          null
        );
        done();
      };
      (<any>listObj).onChipRemove({
        preventDefault: function () {},
        which: 22,
        target: divElement.querySelector(
          '.e-chips-collection .e-chips .e-chips-close'
        ),
      });
    });
  ...
  describe('Filtering API', () => {
    ...
    it(' check the filter', () => {
      ...
      (<any>listObj).onInput();
      (<any>listObj).keyUp(keyboardEventArgs);
      expect(listObj.list.classList.contains(dropDownBaseClasses.noData)).toBe(
        false
      );
      expect(
        listObj.liCollections[0].getAttribute('data-value') === 'list1'
      ).toBe(true);
  ...
  describe('Grouping in CheckBox Mode', () => {
    ...
    it('Validation for the grouping in CheckBox Mode', () => {
      ...
      mouseEventArgs.type = 'click';
      (<any>listObj).onMouseClick(mouseEventArgs);
      let listElement: any = (<any>listObj).ulElement.querySelector(
        'li.e-list-item'
      );
      expect(
      ...
      expect(
        listElement.previousElementSibling.firstElementChild.lastElementChild.classList.contains(
          'e-check'
        )
      ).toBe(true);
      mouseEventArgs.target = listObj.ulElement.querySelector(
        'li.e-list-group-item'
      ).firstElementChild.lastElementChild;
      let groupElement = listObj.ulElement.querySelector(
        'li.e-list-group-item'
      );
      mouseEventArgs.type = 'click';
      (<any>listObj).onMouseClick(mouseEventArgs);
      ...
      expect(
        listElement.nextElementSibling.firstElementChild.lastElementChild.classList.contains(
          'e-check'
        )
      ).toBe(false);
      listObj.hidePopup();
    ...
    it('Validation for clear all for grouping in CheckBox Mode', () => {
      ...
      expect(
        (<any>listObj).list
          .querySelector('li.e-list-group-item')
          .firstElementChild.lastElementChild.classList.contains('e-check')
      ).toBe(false);
      expect(
        (<any>listObj).list
          .querySelector('li.e-list-item')
          .firstElementChild.lastElementChild.classList.contains('e-check')
      ).toBe(false);
      listObj.hidePopup();
      listObj.destroy();
  ...
  describe('Select All functionality against Maximumselection length', () => {
    ...
    it('Without grouping', () => {
      ...
      listObj.showPopup();
      expect((<any>listObj).isPopupOpen()).toBe(true);
      mouseEventArgs.target = (listObj as any).popupWrapper.querySelectorAll(
        '.e-selectall-parent'
      )[0];
      mouseEventArgs.type = 'click';
      (<any>listObj).selectAllItem(true, mouseEventArgs);
      expect(
        (listObj as any).list.querySelectorAll('.e-active').length ==
          (listObj as any).maximumSelectionLength
      ).toBe(true);
      expect(
        (listObj as any).value.length == (listObj as any).maximumSelectionLength
      ).toBe(true);
      listObj.hidePopup();
      listObj.destroy();
    });
    it('With grouping', () => {
      ...
      (<any>listObj).selectAllItem(true, mouseEventArgs);
      expect(
        (listObj as any).list.querySelectorAll('.e-list-item.e-active')
          .length == (listObj as any).maximumSelectionLength
      ).toBe(true);
      expect(
        (listObj as any).value.length == (listObj as any).maximumSelectionLength
      ).toBe(true);
  ...
  describe('Select All Public method functionality', () => {
    ...
    it('Without grouping', () => {
      ...
      expect(
        (<any>listObj).value.length === (<any>listObj).liCollections.length
      ).toBe(true);
      listObj.destroy();
    });
    it('With grouping', () => {
  ...
  describe('Set Outline theme with float type always', () => {
    ...
    it('Set floatlabeltype always', () => {
      ...
      listObj.appendTo(element);
      expect(
        listObj.overAllWrapper.classList.contains('e-valid-input')
      ).toEqual(true);
      listObj.floatLabelType = 'Auto';
      listObj.dataBind();
      expect(
  ...
  describe('Update value in focus state', () => {
    ...
    it('Update value', () => {
      ...
      expect(
        (<HTMLElement>(
          (<any>listObj).ulElement.querySelector('li[data-value="GO"]')
        )).style.display === ''
      ).toBe(true);
    });
  });
  describe('EJ2-36604 - While giving the class name with empty space for HtmlAttributes, console error is produced.', function () {
    ...
    it('Entering the class name without any empty space', function () {
      ...
      expect(listObj.overAllWrapper.classList.contains('custom-class')).toBe(
        true
      );
      expect(listObj.popupWrapper.classList.contains('custom-class')).toBe(
        true
      );
    });
    ...
    it('Giving more than one empty space between two class names', function () {
      ...
      listObj.appendTo('#multiselect');
      expect(
        listObj.overAllWrapper.classList.contains('custom-class-one')
      ).toBe(true);
      expect(listObj.popupWrapper.classList.contains('custom-class-one')).toBe(
        true
      );
      expect(
        listObj.overAllWrapper.classList.contains('custom-class-two')
      ).toBe(true);
      expect(listObj.popupWrapper.classList.contains('custom-class-two')).toBe(
        true
      );
    });
    ...
    it('Giving class name with underscore in the beginning', function () {
      ...
      listObj.appendTo('#multiselect');
      expect(
        listObj.overAllWrapper.classList.contains('_custom-class-one')
      ).toBe(true);
      expect(listObj.popupWrapper.classList.contains('_custom-class-one')).toBe(
        true
      );
      expect(
        listObj.overAllWrapper.classList.contains('_custom-class-two')
      ).toBe(true);
      expect(listObj.popupWrapper.classList.contains('_custom-class-two')).toBe(
        true
      );
    });
  ...
  describe('EJ2-40111: Incorrect count in the multiselect field when multiple items are selected', () => {
    ...
    let empList: { [key: string]: Object }[] = [
      ...
      {
        displayName: 'SANDERS, JASON ADAMCILGIRIST  - 00000003',
        npi: '00000003',
      },
    ...
    it('Update with 3rd value which has more length than the input element', () => {
      ...
      listObj.appendTo(element);
      listObj.showPopup();
      expect((<any>listObj).text).toBe(
        'SANDERS, JASON ADAMCILGIRIST  - 00000003'
      );
      listObj.hidePopup();
      listObj.destroy();
    });
    it('Update with 2nd and then 1st value respectively', () => {
      ...
      listObj.showPopup();
      expect((<any>listObj).text).toBe(
        'JOHNSON, JAMES WILLIAM- 00000002,SMITH,- 00000001'
      );
      listObj.hidePopup();
      listObj.destroy();
    });
  ...
  describe('EJ2-41323: Not able to select the text and edit', () => {
    ...
    it('Check the cursor position on checkbox selection', () => {
      ...
      expect(
        (<any>listObj).checkBoxSelectionModule.filterInput.selectionStart
      ).toBe(2);
      expect(
        (<any>listObj).checkBoxSelectionModule.filterInput.selectionEnd
      ).toBe(3);
      listObj.destroy();
    });
  });
  ...
  describe('EJ2-42061 - Preselected value is added to the control, if we provide invalid value', () => {
    let listObj: any;
    let mEle: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'multi' })
    );
  ...
  describe('EJ2-40997', () => {
    ...
    let mouseDownEvent: MouseEvent = document.createEvent('MouseEvent');
    mouseDownEvent.initEvent('mousedown');
    let mEle1: HTMLInputElement = <HTMLInputElement>(
      createElement('input', { id: 'multi' })
    );
    let dataSource: any = [];
    ...
    it('Performance checking when clicking li item for larger values', (done) => {
      ...
      if (listObj1.isPopupOpen()) {
        listObj1.popupObj.element
          .querySelector('.e-selectall-parent')
  ...
  describe(' EJ2-47405 ', () => {
    ...
    it('Testing with string as dataSource field value', () => {
      ...
      let elem: HTMLElement = (<any>(
        listObj
      )).chipCollectionWrapper.querySelector('span[title="11"]');
      (<any>listObj).onChipRemove({
        which: 1,
        button: 1,
        target: elem.lastElementChild,
  ...
  describe('EJ2-48286 - When we paste the content in the MultiSelect, the pasted content gets hidden in the input', () => {
    ...
    it('Check with default case with long content', (done) => {
      ...
      setTimeout(() => {
        ...
        let listElement: any = (<any>mulObj).ulElement.querySelectorAll(
          'li.e-list-item'
        );
  ...
  describe('EJ2-48220 - While scrolling headers are duplicated and overlapped with items in Multiselect with Grouping case', () => {
    let element: HTMLInputElement;
    let datasource: { [key: string]: Object }[] = [
      { vegetable: 'Cabbage', category: 'Leafy and Salad', id: 'theme1' },
      { vegetable: 'Spinach', category: 'Leafy and Salad', id: 'theme2' },
      { vegetable: 'Chickpea', category: 'Beans', id: 'theme3' },
      { vegetable: 'Green bean', category: 'Beans', id: 'theme4' },
      { vegetable: 'Horse gram', category: 'Beans', id: 'theme5' },
      { vegetable: 'Garlic', category: 'Bulb and Stem', id: 'theme6' },
      { vegetable: 'Nopal', category: 'Bulb and Stem', id: 'theme7' },
      { vegetable: 'Onion', category: 'Bulb and Stem', id: 'theme8' },
    ];
    let multiObj: any;
    ...
    it('Testing the fixed header value updation for value select and popup close cases and resolve the coverage issue', (done) => {
      ...
      setTimeout(() => {
        ...
        let listItems: Array<HTMLElement> = (<any>(
          multiObj
        )).list.querySelectorAll('li' + ':not(.e-list-group-item)');
    ...
    it('Testing the fixed header value updation for value remove cases and resolve the coverage issue', (done) => {
      ...
      setTimeout(() => {
        expect(!isNullOrUndefined(multiSelectObj.fixedHeaderElement)).toBe(
          true
        );
        expect(multiSelectObj.fixedHeaderElement.innerHTML).toBe(
          'Bulb and Stem'
        );
  ...
  describe('EJ2-49608', () => {
    ...
    it('Fixed header width not applying in Firefox', (done) => {
      ...
      multiselectInstance.onKeyDown(keyboardEventArgs);
      setTimeout(() => {
        expect(!isNullOrUndefined(multiselectInstance.fixedHeaderElement)).toBe(
          true
        );
        expect(
          !isNaN(parseInt(multiselectInstance.fixedHeaderElement.style.width))
        ).toBe(true);
  ...
  describe('EJ2MVC-335 - Chip value updated incorrectly in the multiselect component.', () => {
    ...
    it('check the value selection for issue reported case', (done) => {
      ...
      multiObj.showPopup();
      setTimeout(() => {
        let list: Array<HTMLElement> = (<any>multiObj).list.querySelectorAll(
          'li'
        );
        mouseEventArgs.target = list[0];
        mouseEventArgs.type = 'click';
  ...
  describe('EJ2-51217 - Placeholder encoding in the multiselect component', () => {
    ...
    it('Testing floatLabelType Always case', () => {
      ...
      multiObj.appendTo(element);
      let floatElement = (multiObj as any).componentWrapper.querySelector(
        '.e-float-text'
      );
      expect(floatElement.innerText).toBe('&');
      (multiObj.placeholder = '<'), multiObj.dataBind();
      floatElement = (multiObj as any).componentWrapper.querySelector(
        '.e-float-text'
      );
      expect(floatElement.innerText).toBe('<');
      (multiObj.placeholder = 'Mask >LLL<LL (ex: SAMple)'), multiObj.dataBind();
      floatElement = (multiObj as any).componentWrapper.querySelector(
        '.e-float-text'
      );
      ...
      expect(floatElement.innerText).toBe(
        "<img src='fail1' onerror='alert();' /> test"
      );
      (multiObj.placeholder = 'Hi&eacute;rachie article'), multiObj.dataBind();
      floatElement = (multiObj as any).componentWrapper.querySelector(
        '.e-float-text'
      );
      ...
      expect(floatElement.innerText).toBe('&>');
      (multiObj.placeholder = 'JAVA & ANGULAR'), multiObj.dataBind();
      floatElement = (multiObj as any).componentWrapper.querySelector(
        '.e-float-text'
      );
  ...
  describe('EJ2-36414 - Provide support to maintain the typed value as chip when control gets out of focus on Box mode', () => {
    ...
    it('Testing the custom chip creation on blur', () => {
      ...
      expect(
        (<any>multiObj).chipCollectionWrapper.querySelector(
          'span[data-value="j"]'
        )
      ).not.toBe(null);
      expect(
        (<any>multiObj).chipCollectionWrapper.querySelectorAll(
          'span.' + multiSelectData.chips
        ).length
      ).toBe(1);
      customSearch('j');
  ...
  describe('Provide support to maintain the typed value as chip when control gets out of focus for remote date on Box mode', () => {
    ...
    it('Testing the custom chip creation on blur', (done) => {
      multiObj = new MultiSelect({
        dataSource: remoteData,
        query: new Query().select('FirstName').take(6).requiresCount(),
        fields: { text: 'FirstName', value: 'FirstName' },
        addTagOnBlur: true,
        mode: 'Box',
        allowCustomValue: true,
      ...
      setTimeout(() => {
        ...
        expect(
          (<any>multiObj).chipCollectionWrapper.querySelector(
            'span[data-value="Laura Callahan"]'
          )
        ).not.toBe(null);
        expect(
          (<any>multiObj).chipCollectionWrapper.querySelectorAll(
  ...
  describe('EJ2-36414 - Provide support to maintain the typed value as chip when control gets out of focus on Delimiter mode', () => {
    ...
    it('Testing the custom chip creation on blur', () => {
      ...
      expect(multiObj.inputElement.value).toBe('');
      expect(
        (<any>multiObj).delimiterWrapper.parentElement.querySelector(
          '.e-delim-view'
        ).innerText
      ).toEqual('j');
      customSearch('JAVA');
  ...
  describe('EJ2-56422-Empty header is created while typing custom value in the input.', () => {
    ...
    let datasource: { [key: string]: Object }[] = [
      ...
      { vegetable: 'Wheatgrass', category: 'Leafy and Salad', Id: 'item3' },
    ...
    it('testing allowcustom with groupby', () => {
      ...
      (<any>listObj).keyUp(keyboardEventArgs);
      keyboardEventArgs.altKey = false;
      expect(listObj.liCollections[0].classList.contains('e-list-item')).toBe(
        true
      );
      expect(
        listObj.liCollections[0].classList.contains('e-list-group-item')
      ).toBe(false);
  ...
  describe('EJ2-59153', () => {
    ...
    it('Multiselect Item template with allowFiltering and maximumSelectionLength property enables the filtered item in the popup from second time', () => {
      ...
      expect(listObj.ulElement.childElementCount === 1).toBe(true);
      expect(
        listObj.ulElement.children[0].classList.contains('e-disable')
      ).toBe(true);
      (<any>listObj).inputElement.value = '';
      keyboardEventArgs.keyCode = 8;
      (<any>listObj).keyDownStatus = true;
  ...
  describe('EJ2-60441', () => {
    ...
    it('allowCustom feature does not work for a single digit character.', () => {
      ...
      expect(listObj.ulElement.querySelector('li').textContent === '4').toBe(
        true
      );
      ...
      expect((<any>listObj).isPopupOpen()).toBe(true);
      expect(listObj.ulElement.childElementCount === 4).toBe(true);
      expect(listObj.ulElement.querySelector('li').textContent === '45').toBe(
        true
      );
      let list: Array<HTMLElement> = (<any>listObj).list.querySelectorAll('li');
      mouseEventArgs.target = list[0];
      ...
      expect(listObj.ulElement.querySelector('li').textContent === '6').toBe(
        true
      );
    });
  ...
  describe('Disable items', () => {
    ...
    it('checked with disableItem method', (done) => {
      ...
      setTimeout(() => {
        expect(
          listObj.list.querySelectorAll('.e-list-item:not(.e-disabled)').length
        ).toBe(8);
        listObj.disableItem('Game4');
        expect(listObj.liCollections[3].classList.contains('e-disabled')).toBe(
          true
        );
        expect(listObj.liCollections[3].getAttribute('aria-selected')).toBe(
          'false'
        );
        expect(listObj.liCollections[3].getAttribute('aria-disabled')).toBe(
          'true'
        );
        expect(listObj.liCollections[6].classList.contains('e-disabled')).toBe(
          true
        );
        expect(listObj.liCollections[6].getAttribute('aria-selected')).toBe(
          'false'
        );
        expect(listObj.liCollections[6].getAttribute('aria-disabled')).toBe(
          'true'
        );
        expect(
          listObj.list.querySelectorAll('.e-list-item:not(.e-disabled)').length
        ...
        expect(listObj.liCollections[0].classList.contains('e-disabled')).toBe(
          true
        );
        expect(listObj.liCollections[0].getAttribute('aria-selected')).toBe(
          'false'
        );
        expect(listObj.liCollections[0].getAttribute('aria-disabled')).toBe(
          'true'
        );
        expect(
          listObj.list.querySelectorAll('.e-list-item:not(.e-disabled)').length
        ...
        expect(listObj.liCollections[7].classList.contains('e-disabled')).toBe(
          true
        );
        expect(listObj.liCollections[7].getAttribute('aria-selected')).toBe(
          'false'
        );
        expect(listObj.liCollections[7].getAttribute('aria-disabled')).toBe(
          'true'
        );
        expect(
          listObj.list.querySelectorAll('.e-list-item:not(.e-disabled)').length
        ...
        expect(listObj.liCollections[9].classList.contains('e-disabled')).toBe(
          true
        );
        expect(
          listObj.list.querySelectorAll('.e-list-item:not(.e-disabled)').length
        ...
        expect(listObj.liCollections[8].classList.contains('e-disabled')).toBe(
          true
        );
        expect(listObj.liCollections[8].getAttribute('aria-selected')).toBe(
          'false'
        );
        expect(listObj.liCollections[8].getAttribute('aria-disabled')).toBe(
          'true'
        );
        expect(
          listObj.list.querySelectorAll('.e-list-item:not(.e-disabled)').length
  ...
  describe('keyboard interaction with disabled items', () => {
    ...
    it('up and down action', (done) => {
      ...
      setTimeout(() => {
        expect(
          listObj.list
            .querySelector('.e-item-focus')
            .getAttribute('data-value') === 'Game2'
        ).toBe(true);
    ...
    it('all disabled items', (done) => {
      ...
      setTimeout(() => {
        ...
        expect(
          listObj.list.querySelectorAll('.e-item-focus').length === 0
        ).toBe(true);
        keyEventArgs.action = 'down';
        keyEventArgs.keyCode = 40;
        listObj.onKeyDown(keyEventArgs);
  ...
  describe('Disable items', () => {
    ...
    it('with Grouping Checkbox', (done) => {
      ...
      setTimeout(() => {
        expect(
          listObj.list
            .querySelector('.e-list-group-item')
            .classList.contains('e-disabled')
        ).toBe(true);
        expect(
          listObj.list.querySelector('.e-list-group-item:not(.e-disabled)')
            .innerText === 'Beans'
        ).toBe(true);
        listObj.disableItem('item7');
        expect(
          listObj.list.querySelector('.e-list-group-item:not(.e-disabled)')
            .innerText === 'Bulb and Stem'
        ).toBe(true);
        listObj.onKeyDown(keyEventArgs);
        listObj.onKeyDown(keyEventArgs);
        expect(
          listObj.list.querySelector('.e-item-focus').innerText ===
            'Bulb and Stem'
        ).toBe(true);
        done();
      }, 450);
  ...
  describe('Code Coverage improvements', () => {
    ...
    it('- SelectByKey without list', () => {
      ...
      (<any>listObj).renderPopup();
      listObj.showPopup();
      var activeList = (<any>listObj).list.querySelector(
        '.e-list-item.e-item-focus'
      );
      if (activeList) {
        activeList.classList.remove('e-item-focus');
      }
      var activeSelectAll = (listObj as any).popupWrapper.querySelector(
        '.e-selectall-parent'
      );
    ...
    it('- onMouseOver', () => {
      listObj = new MultiSelect({
        allowObjectBinding: true,
        fields: { text: 'text', value: 'text', groupBy: 'id' },
        mode: 'CheckBox',
        showSelectAll: true,
        enableGroupCheckBox: true,
        dataSource: datasource2,
