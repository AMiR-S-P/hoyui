...
describe('MultiSelect_Virtualization', () => {
  ...
  let css: string =
    ".e-spinner-pane::after { content: 'Material'; display: none;} .e-list-item{height:36px} .e-dropdownbase.e-content{overflow: auto;} ";
  ...
  describe('Code Coverage improvements', () => {
    ...
    it('- updateSelectionList virtual', () => {
      ...
      (<any>virtual).parent.currentFocuedListElement =
        listObj.list.firstElementChild;
      (<any>virtual).parent.currentFocuedListElement.setAttribute(
        'data-value',
        'Item 1'
      );
      (<any>virtual).setCurrentViewDataAsync();
  ...
  describe('virtualization key actions', () => {
    ...
    it('virtualization Down key pressed ', (done) => {
      ...
      expect(
        multiObj.list.querySelectorAll('li:not(.e-virtual-list)').length
      ).toBe(30);
      expect(multiObj.list.querySelectorAll('.e-virtual-list').length).toBe(15);
      expect(
        multiObj.list
          .querySelectorAll('li:not(.e-virtual-list)')[0]
          .textContent.trim()
      ).toBe('Item 1');
      let li: Element[] = multiObj.list.querySelectorAll(
        'li:not(.e-virtual-list)'
      );
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          expect(multiObj.list.querySelectorAll('.e-virtual-list').length).toBe(
            15
          );
          done();
        }, 850);
    ...
    it('virtualization Home and End key pressed ', (done) => {
      ...
      expect(
        (
          multiObj.list.querySelectorAll(
            'li:not(.e-virtual-list)'
          )[0] as Element
        ).classList.contains('e-item-focus')
      ).toBe(true);
      keyEventArgs.keyCode = 35;
      multiObj.onKeyDown(keyEventArgs);
      expect(
        (
          multiObj.list.querySelectorAll('li:not(.e-virtual-list)')[
            li.length - 1
          ] as Element
        ).classList.contains('e-item-focus')
      ).toBe(true);
    ...
    it('virtualization Page_down and Page_up key pressed ', (done) => {
      ...
      setTimeout(() => {
        ...
        expect((li[0] as Element).classList.contains('e-item-focus')).toBe(
          true
        );
        keyEventArgs.keyCode = 34;
        multiObj.onKeyDown(keyEventArgs);
        expect((li[6] as Element).classList.contains('e-item-focus')).toBe(
          true
        );
        keyEventArgs.keyCode = 34;
        multiObj.onKeyDown(keyEventArgs);
        expect((li[12] as Element).classList.contains('e-item-focus')).toBe(
          true
        );
        keyEventArgs.keyCode = 34;
        multiObj.onKeyDown(keyEventArgs);
        expect((li[18] as Element).classList.contains('e-item-focus')).toBe(
          true
        );
    ...
    describe('virtualization IncrementalSearch', () => {
      ...
      it('with key action.', () => {
        ...
        expect(
          (<any>multiObj).value && (<any>multiObj).value.length
        ).not.toBeNull();
        multiObj.destroy();
      });
    });
      ...
      it('virtualization up actions scroll by manually', (done) => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            keyEventArgs.keyCode = 40;
            multiObj.onKeyDown(keyEventArgs);
            expect((li[1] as Element).classList.contains('e-item-focus')).toBe(
              true
            );
            done();
          }, 850);
    ...
    describe('virtualization filtering actions', () => {
      ...
      it('With grouping', () => {
        ...
        mouseEventArgs.target = (multiObj as any).popupWrapper.querySelectorAll(
          '.e-selectall-parent'
        )[0];
        mouseEventArgs.type = 'click';
    ...
    describe('Virtualization Template support', () => {
      ...
      beforeAll(() => {
        ...
        multiObj = new MultiSelect({
          ...
          allowFiltering: true,
          fields: { text: 'text', value: 'id' },
          itemTemplate: '<div class="ename"> ${text} </div></div>',
          valueTemplate: '<div class="tempName"> ${text} </div>',
        });
        multiObj.appendTo(ele);
      });
      afterAll(() => {
    ...
    describe('Virtualization with value selection', () => {
      ...
      it('Removing chip using backspace key', () => {
        ...
        let wrapper: HTMLElement = (<any>multiObj).inputElement.parentElement
          .parentElement;
        if (
          wrapper &&
          ...
          expect(getComputedStyle((<any>multiObj).searchWrapper).width).toBe(
            'calc(100% - 20px)'
          );
