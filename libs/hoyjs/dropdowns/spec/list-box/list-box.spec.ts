 ...
 *
 */
import { createElement, EventHandler } from '@hoyui/base';
import { ListBox, SelectionSettings } from '../../src/index';
import { cssClass } from '@hoyui/lists';
...
let data: { [key: string]: Object }[] = [
  { id: 'list1', text: 'JAVA', value: '1', icon: 'icon' },
  { id: 'list2', value: '2', text: 'C#' },
  { id: 'list3', value: '3', text: 'C++' },
  { id: 'list4', value: '4', text: 'NET', icon: 'icon' },
  { id: 'list5', value: '5', text: 'Oracle' },
];

let eData: { [key: string]: Object }[] = [
  { id: 'list0', text: '', value: '0', icon: 'icon' },
  { id: 'list1', text: 'JAVA', value: '1', icon: 'icon' },
  { id: 'list2', value: '2', text: 'C#' },
  { id: 'list3', value: '3', text: 'C++' },
  { id: 'list4', value: '4', text: 'NET', icon: 'icon' },
...
describe('ListBox', () => {
  ...
  beforeAll(() => {
    document.body.appendChild(
      createElement('link', {
        attrs: {
          rel: 'stylesheet',
          href: '/base/styles/list-box/material.css',
          type: 'text/css',
        },
      })
    );
  ...
  describe('Selection', () => {
    ...
    it('Normal', () => {
      ...
      expect(
        listObj.list.querySelector('.e-hidden-select').children[0].value
      ).toEqual('JAVA');
    ...
    it('Ctrl', () => {
      listObj = new ListBox(
        { dataSource: data, fields: { value: 'value', text: 'text' } },
        elem
      );
      li = listObj.getItems();
      li[2].click();
      ...
      expect(
        listObj.list.querySelector('.e-hidden-select').children[1].value
      ).toEqual('4');
    ...
    it('Checkbox', () => {
      ...
      expect(listObj.value[0]).toEqual('C++');
      expect(liEle[2].getElementsByClassName('e-frame')[0].classList).toContain(
        'e-check'
      );
      liEle[4].click();
      expect(listObj.value[1]).toEqual('Oracle');
      expect(liEle[4].getElementsByClassName('e-frame')[0].classList).toContain(
        'e-check'
      );
      liEle[2].click();
      expect(listObj.value[0]).not.toEqual('C++');
      expect(
        liEle[2].getElementsByClassName('e-frame')[0].classList
      ).not.toContain('e-check');
    ...
    it('Checkbox - SelectAll', () => {
      ...
      (listObj as any).selectHandler({
        target: (listObj as any).list.querySelector('li'),
      });
    });
    ...
    it('Select All Checkbox', () => {
      ...
      let ele: any =
        listObj.list.getElementsByClassName('e-selectall-parent')[0];
      EventHandler.trigger(ele, 'mousedown', {
        currentTarget: ele,
        preventDefault: () => {},
      });
      expect(ele.getElementsByClassName('e-frame')[0].classList).toContain(
        'e-check'
      );
      ...
      expect(ele.getElementsByClassName('e-frame')[0].classList).not.toContain(
        'e-check'
      );
      ...
      liele[0].click();
      expect(
        liele[0].getElementsByClassName('e-frame')[0].classList
      ).not.toContain('e-check');
      expect(ele.getElementsByClassName('e-frame')[0].classList).not.toContain(
        'e-check'
      );
  ...
  describe('Property', () => {
    ...
    it('Value', () => {
      ...
      let ele: any =
        listObj.list.getElementsByClassName('e-selectall-parent')[0];
      let liEle: any = listObj.list.getElementsByClassName('e-list-item');
      expect(liEle[0].getElementsByClassName('e-frame')[0].classList).toContain(
        'e-check'
      );
      expect(ele.getElementsByClassName('e-frame')[0].classList).toContain(
        'e-check'
      ...
      listObj.value = ['C#'];
      listObj.dataBind();
      expect(liEle[1].getElementsByClassName('e-frame')[0].classList).toContain(
        'e-check'
      );
      expect(ele.getElementsByClassName('e-frame')[0].classList).not.toContain(
        'e-check'
    ...
    it('allowFiltering', () => {
      ...
      expect(
        listObj.list.getElementsByClassName('e-ul')[0].childElementCount
      ).toEqual(5);
      expect(
        listObj.list
        ...
        listObj.list
          .getElementsByClassName('e-list-item')[1]
          .getAttribute('data-value')
          .charAt(0)
      );
      ele.click();
  ...
  describe('Angular', () => {
    ...
    it('Listbox', () => {
      ...
      expect(listObj.element.firstElementChild.classList).toContain(
        'e-listbox-wrapper'
      );
  ...
  describe('Toolbar with sorting', () => {
    ...
    beforeEach(() => {
      ...
      listObj2 = new ListBox(
        { dataSource: vegetableData, sortOrder: 'Ascending' },
        elem2
      );
    });
  ...
  describe('Methods', () => {
    ...
    it('SelectItems', () => {
      ...
      expect(JSON.stringify(listObj.value)).toEqual(
        JSON.stringify(['JAVA', 'NET'])
      );
    ...
    it('SelectAll', () => {
      ...
      expect(listObj.getSelectedItems().length).toEqual(5);
      expect(JSON.stringify(listObj.value)).toEqual(
        JSON.stringify(['JAVA', 'C#', 'C++', 'NET', 'Oracle'])
      );
  ...
  describe('Drag and Drop', () => {
    ...
    let helper: Element = createElement('div', {
      className: 'e-listbox-wrapper e-wrapper e-lib e-sortableclone',
    });
    beforeAll(() => {
      document.body.appendChild(elem);
      document.body.appendChild(elem1);
    ...
    it('single listbox drag and drop action', () => {
      listObj = new ListBox(
        {
          ...
          fields: { value: 'value', text: 'text', groupBy: 'icon' },
        },
        elem
      );
      ...
      let dragArgs: any = {
        bindEvents: null,
        dragElement: listItem[0],
        element: listObj.element,
        event: mouseEventArs,
        name: 'dragStart',
        target: listItem[0],
      };
    ...
    it('Multiple listbox drag and drop action', () => {
      ...
      dragArgs = {
        ...
        target: listItem1[2],
        previousIndex: 0,
        element: listObj1.list,
      };
      listObj.beforeDragEnd(dragArgs);
      dragArgs.event = mouseEventArs;
      dragArgs.name = 'drop';
      dragArgs.droppedElement = listItem1[2];
      listObj.dragEnd(dragArgs);
    });
    ...
    it('Multiple listbox drag and drop action with empty datasource', () => {
      ...
      listObj1 = new ListBox(
        { allowDragAndDrop: true, sortOrder: 'Ascending' },
        elem1
      );
      let listItem: any = listObj.list.querySelectorAll('.e-list-item');
      ...
      dragArgs.event = mouseEventArs;
      dragArgs.name = 'drop';
      dragArgs.droppedElement = listItem[0];
      listObj.dragEnd(dragArgs);
    });
  ...
  describe('Customer Reported Bug', () => {
      ...
      listObj1 = new ListBox(
        {
          ...
          noRecordsTemplate:
            '<div style="padding:10px; padding-left:20%; position:relative;"> <span class = "img" > <img src="http://www.404errorpages.com/images/image001.png" height="40px", width="40px"/></span> <span style="padding:10px; position: absolute;">No records found</span></div>',
        },
        elem1
      );
      listObj2 = new ListBox(
    ...
    it('EJ2-48312', () => {
      let vegetableData: { [key: string]: Object }[] = [];
      listObj = new ListBox(
        { dataSource: vegetableData, allowFiltering: true },
        elem
      );
      var ele = listObj.list.getElementsByClassName('e-input-filter')[0];
      var clearEle =
        listObj.list.getElementsByClassName('e-input-group-icon')[0];
      expect(
        listObj.list.getElementsByClassName('e-ul')[0].childElementCount
      ).toEqual(1);
      ele.click();
    ...
    it('toolbarSettings', () => {
      listObj1.toolbarSettings.items = [
        'moveUp',
        'moveDown',
        'moveTo',
        'moveAllTo',
    ...
    it('EJ2-50548 - Toolbar settings not aligned properly when Rtl mode is applied', () => {
      expect(
        listObj1.list.nextElementSibling.classList.contains('e-rtl')
      ).toBeFalsy();
    ...
    it('EJ2-38419-Need to provide noRecordsTemplate support in Listbox', () => {
      ...
      expect(listObj1.ulElement.childElementCount).toEqual(1);
      expect(
        listObj1.list.getElementsByClassName('e-ul')[0].children[0].children[1]
          .textContent
      ).toEqual('No records found');
      expect(
        listObj1.list.getElementsByClassName('e-ul')[0].children[0].children[0]
          .classList
      ).toContain('img');
      expect(listObj2.ulElement.childElementCount).toEqual(16);
    ...
    it('EJ2-61108 - Datasource not update properly after clearing the filtering in the dual listbox', () => {
      ...
      expect(
        listObj2.list.getElementsByClassName('e-ul')[0].childElementCount
      ).toEqual(11);
      ele.click();
      ele.value = 'c';
      listObj2.dataBind();
      ...
      toolChild[4].click();
      toolChild[5].click();
      var ele1 = listObj2.list.getElementsByClassName(
  ...
  describe('Coverage improvement', () => {
    ...
    it('filterBarPlaceholder', () => {
      listObj = new ListBox(
        { dataSource: data, filterBarPlaceholder: '' },
        elem
      );
    ...
    it('getSortedList', () => {
      listObj = new ListBox(
        { dataSource: data, fields: { text: 'text', groupBy: 'icon' } },
        elem
      );
      ...
      listObj = new ListBox(
        {
          ...
          selectionSettings: { showCheckbox: false, mode: 'Single' },
        },
        elem
      );
      listObj.selectItems(['folder1\\item1'], true, true);
    ...
    it('removeItem', function () {
      listObj = new ListBox(
        { dataSource: vegetableData, fields: { text: 'text', value: 'id' } },
        elem
      );
  ...
  describe('Coverage improvements with form', () => {
    ...
    it('Coverage improvements with form', () => {
      listObj = new ListBox(
        { dataSource: vegetableData, allowDragAndDrop: true },
        elem
      );
    });
  });
  describe('Coverage improvements drag and drop', () => {
    ...
    it('ListBox With allowDragAndDrop', () => {
      ...
      let draggableElement: Element =
        listObj.element.parentElement.querySelector('.e-list-item');
    ...
    it('listbox with responsive height', () => {
      listObj = new ListBox(
        { dataSource: data, allowFiltering: true, height: '100%' },
        elem
      );
    });
    it('SelectItems', () => {
      listObj = new ListBox(
        { dataSource: data, selectionSettings: { mode: 'Single' } },
        elem
      );
      listObj.selectItems(['JAVA', 'NET']);
    ...
    it('Validation attribute checking', () => {
      ...
      (listObj as any).validationAttribute(
        listObj.element,
        listObj.element.parentElement.querySelector('.e-hidden-select')
      );
    });
  ...
  describe('Coverage improvements toolbar', () => {
    ...
    it('On Action Complete dummy', () => {
      (listObj1 as any).onActionComplete(
        (listObj1 as any).ulElement,
        (listObj1 as any).jsonData,
        {}
      );
    });
    ...
    it('Get Valid Index', () => {
      let cli: HTMLElement = (listObj1 as any).ulElement
    ...
    it('toolbar setting position', () => {
      (listObj1 as any).setProperties({
        toolbarSettings: { position: 'Right' },
      });
    });
