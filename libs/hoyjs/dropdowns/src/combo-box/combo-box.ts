...
import {
  DropDownList,
  dropDownListClasses,
} from '../drop-down-list/drop-down-list';
...
import { DataManager, DataOptions, Predicate, Query } from '@hoyui/data';
...
export class ComboBox extends DropDownList {
  ...
  private preventBlur(e: MouseEvent): void {
    if (
      (!this.allowFiltering &&
        ...
        !document.activeElement.classList.contains(dropDownListClasses.input) &&
        Browser.isDevice) ||
      !Browser.isDevice
    ) {
      e.preventDefault();
  ...
  protected onBlurHandler(e: MouseEvent): void {
    const inputValue: string =
      this.inputElement && this.inputElement.value === ''
        ...
        : this.inputElement && this.inputElement.value;
    if (
      !isNullOrUndefined(this.listData) &&
  ...
  private valueMuteChange(value: string | number | boolean | object): void {
    value =
      this.allowObjectBinding && !isNullOrUndefined(value)
        ? getValue(this.fields.value ? this.fields.value : '', value)
        : value;
    const inputValue: string = isNullOrUndefined(value)
      ? null
      : value.toString();
    Input.setValue(
      inputValue,
    ...
    if (this.allowObjectBinding) {
      ...
      if (isNullOrUndefined(value)) {
        ...
        changeData = {
          text: isValidValue
            ? isvalidTextField
              ? getValue(fields.text, this.value)
              : getValue(fields.value, this.value)
            : null,
    ...
    if (this.allowObjectBinding) {
      ...
      this.setProperties(
        { value: value, text: value ? value.toString() : value, index: null },
        true
      );
    }
  ...
  protected updateValues(): void {
    ...
    if (!isNullOrUndefined(this.value)) {
      ...
      if (li) {
        ...
        (this.enableVirtualization && !doesItemExist)
      ) {
        this.valueMuteChange(null);
      }
  ...
  protected setSearchBox(): InputObject {
    ...
    const searchBoxContainer: InputObject =
      this.isFiltering() ||
      ((this as any).isReact && this.getModuleName() === 'combobox')
        ? this.inputWrapper
        : inputObject;
  ...
  protected getFocusElement(): Element {
    const dataItem: { [key: string]: string } = this.isSelectCustom
      ? { text: '' }
      : this.getItemData();
    const selected: HTMLElement = !isNullOrUndefined(this.list)
      ? <HTMLElement>this.list.querySelector('.' + dropDownListClasses.selected)
      : this.list;
    const isSelected: boolean =
      dataItem.text &&
      dataItem.text.toString() === this.inputElement.value &&
      !isNullOrUndefined(selected);
    if (isSelected) {
      return selected;
    }
    if (
      ((Browser.isDevice && !this.isDropDownClick) || !Browser.isDevice) &&
      !isNullOrUndefined(this.liCollections) &&
      this.liCollections.length > 0
    ) {
      const inputValue: string = this.inputElement.value;
      const dataSource: { [key: string]: Object }[] = this.sortedData as {
        [key: string]: Object;
      }[];
      const type: string = this.typeOfData(dataSource).typeof as string;
      let activeItem: { [key: string]: Element | number } = Search(
        inputValue,
        this.liCollections,
        this.filterType,
        true,
      ...
      if (
        ...
        const isIncrementalEndIndexZero: boolean =
        ...
        if (isEndIndexValid || isIncrementalEndIndexZero) {
          ...
          if (isIncrementalEndIndexZero) {
            ...
            this.incrementalEndIndex = Math.min(
        ...
        while (
          isNullOrUndefined(activeItem.item) &&
          this.incrementalEndIndex < this.totalItemCount
        ) {
          this.incrementalStartIndex = this.incrementalEndIndex;
          this.incrementalEndIndex =
          ...
          if (!isNullOrUndefined(activeItem)) {
            activeItem.index =
              (activeItem.index as number) + this.incrementalStartIndex;
            break;
          }
          if (
            isNullOrUndefined(activeItem) &&
            this.incrementalEndIndex >= this.totalItemCount
          ) {
        ...
        if (!isNullOrUndefined(activeItem.item)) {
          ...
          const endIndex: number =
            this.viewPortInfo.startIndex + this.itemCount > this.totalItemCount
              ...
              : this.viewPortInfo.startIndex + this.itemCount;
          this.updateIncrementalView(startIndex, endIndex);
          activeItem.item = this.getElementByValue(
            (activeItem.item as Element).getAttribute('data-value')
          );
        } else {
          this.updateIncrementalView(0, this.itemCount);
      ...
      if (!isNullOrUndefined(activeElement)) {
        const count: number =
          this.getIndexByValue(activeElement.getAttribute('data-value')) - 1;
        const height: number = parseInt(
          getComputedStyle(this.liCollections[0], null).getPropertyValue(
            'height'
          ),
          10
        );
        if (!isNaN(height) && this.getModuleName() !== 'autocomplete') {
          this.removeFocus();
          const fixedHead: number = this.fields.groupBy
            ? this.liCollections[0].offsetHeight
            : 0;
          if (!this.enableVirtualization) {
            this.list.scrollTop = count * height + fixedHead;
          } else {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const virtualContent: any = this.list.getElementsByClassName(
              'e-virtual-ddl-content'
            )[0];
            virtualContent.style = this.getTransformValues();
            if (this.enableVirtualization && !this.fields.groupBy) {
              const selectedLiOffsetTop: number = (activeElement as HTMLElement)
                .offsetTop;
              const virtualListInfoStartIndex: number =
                this.virtualListInfo && this.virtualListInfo.startIndex
                  ? this.virtualListInfo.startIndex
                  : 0;
              const virtualListHeight: number = (activeElement as HTMLElement)
                .offsetHeight;
              const selectedLiOffsetTopWithStartIndex: number =
                selectedLiOffsetTop +
                virtualListInfoStartIndex * virtualListHeight;
              const virtualListLength: number =
                this.list.querySelectorAll('.e-virtual-list').length;
              const scrollTopOffset: number =
                virtualListLength * virtualListHeight;
              this.list.scrollTop =
                selectedLiOffsetTopWithStartIndex - scrollTopOffset;
            }
          }
  ...
  protected setValue(e?: KeyboardEventArgs): boolean {
    if (
      (e && e.type === 'keydown' && e.action === 'enter') ||
      (e && e.type === 'click')
    ) {
  ...
  protected checkCustomValue(): void {
    ...
    const dataItem: { [key: string]: string } = this.getItemData();
    const setValue: string | number | boolean | Object = this.allowObjectBinding
      ? this.itemData
      : dataItem.value;
    if (
      !(
        ...
        isNullOrUndefined(dataItem.text)
      )
    ) {
      ...
      this.spinnerElement =
        this.getModuleName() === 'autocomplete'
          ? this.inputWrapper.buttons[0] ||
            this.inputWrapper.clearButton ||
            Input.appendSpan(
              'e-input-group-icon ' + SPINNER_CLASS,
              this.inputWrapper.container,
              this.createElement
            )
  ...
  protected setAutoFill(activeElement: Element, isHover?: boolean): void {
    ...
    if (this.autofill && !this.preventAutoFill) {
      const currentValue: string = <string>(
        this.getTextByValue(activeElement.getAttribute('data-value')).toString()
      );
      const currentFillValue: string | number | boolean =
      ...
      if (this.getModuleName() === 'combobox') {
        if (
          (!this.isSelected &&
            !this.allowObjectBinding &&
            this.previousValue !== currentFillValue) ||
          (this.allowObjectBinding &&
            this.previousValue &&
            currentFillValue &&
            !this.isObjectInArray(this.previousValue, [
              this.getDataByValue(currentFillValue),
            ]))
        ) {
  ...
  private isAndroidAutoFill(value: string): boolean {
    if (Browser.isAndroid) {
      ...
      if (
        prevEnd !== 0 &&
        ((prevEnd === value.length && prevStart === value.length) ||
          (prevStart > curStart && prevEnd > curEnd) ||
          (prevEnd === curEnd && prevStart === curStart))
      ) {
        return true;
      } else {
        return false;
  ...
  protected clearAll(
    ...
    property?: ComboBoxModel
  ): void {
    if (
      isNullOrUndefined(property) ||
      (!isNullOrUndefined(property) && isNullOrUndefined(property.dataSource))
    ) {
      super.clearAll(e);
    }
    if (
      this.isFiltering() &&
      !isNullOrUndefined(e) &&
      e.target === this.inputWrapper.clearButton
    ) {
      this.searchLists(e);
    }
  }
  ...
  private inlineSearch(e?: KeyboardEventArgs): void {
    const isKeyNavigate: boolean =
      ...
      (e.action === 'down' ||
        ...
        e.action === 'pageUp' ||
        e.action === 'pageDown');
    const activeElement: Element = isKeyNavigate
      ? this.liCollections[this.activeIndex]
      : this.getFocusElement();
    if (!isNullOrUndefined(activeElement)) {
      if (!isKeyNavigate) {
        const value: string | number | boolean = this.getFormattedValue(
          activeElement.getAttribute('data-value')
        );
        this.activeIndex = this.getIndexByValue(value);
        this.activeIndex = !isNullOrUndefined(this.activeIndex)
          ? this.activeIndex
          : null;
      }
      this.preventAutoFill =
        this.inputElement.value === '' ? false : this.preventAutoFill;
      ...
      if (!isNullOrUndefined(this.list)) {
        ...
        const focusItem: Element = this.list.querySelector(
          '.' + dropDownListClasses.li
        );
  ...
  private setAutoFillSelection(
    ...
    if (
      value &&
      value.toLowerCase() ===
        currentValue.substr(0, selection.start).toLowerCase()
    ) {
      const inputValue: string =
        value + currentValue.substr(value.length, currentValue.length);
      Input.setValue(
        inputValue,
        this.inputElement,
        this.floatLabelType,
  ...
  protected setHoverList(li: Element): void {
    ...
    if (
      this.isValidLI(li) &&
      !li.classList.contains(dropDownListClasses.selected)
    ) {
      this.removeFocus();
  ...
  private customValue(e?: MouseEvent | KeyboardEventArgs | TouchEvent): void {
    let value: string | number | boolean | object = this.getValueByText(
      this.inputElement.value
    );
    if (!this.allowCustom && this.inputElement.value !== '') {
      const previousValue: string | number | boolean | object =
        this.previousValue;
      ...
      if (
        ...
        ((!this.allowObjectBinding && previousValue === this.value) ||
          (this.allowObjectBinding &&
            previousValue &&
            this.isObjectInArray(previousValue, [this.value]))) &&
        ((!this.allowObjectBinding && currentValue !== this.value) ||
          (this.allowObjectBinding &&
            currentValue &&
            !this.isObjectInArray(currentValue, [this.value])))
      ) {
      ...
      if (isNullOrUndefined(value)) {
        ...
        const eventArgs: { [key: string]: Object | string | number } = <
        ...
        if (!this.initial) {
          this.trigger(
            ...
            (eventArgs: { [key: string]: Object | string | number }) => {
              this.updateCustomValueCallback(
                value,
                eventArgs,
                previousValue,
                e
  ...
  private updateCustomValueCallback(
    ...
    const fields: FieldSettingsModel = this.fields;
    const item: { [key: string]: string | Object } = <
    ...
    if (this.allowObjectBinding) {
      ...
      if (
        !(this.listData && this.listData.length > 0) &&
        (this.getModuleName() === 'autocomplete' ||
          (this.getModuleName() === 'combobox' && this.allowFiltering))
      ) {
        keys = this.firstItem
      ...
      keys.forEach((key: string) => {
        emptyObject[key as any] =
          key === fields.value || key === fields.text
            ? getValue(fields.value, this.itemData)
            : null;
      });
    }
    ...
    if (
      (!this.allowObjectBinding && previousValue !== this.value) ||
      (this.allowObjectBinding &&
        ((previousValue == null && this.value !== null) ||
          (previousValue &&
            !this.isObjectInArray(previousValue, [this.value]))))
    ) {
  ...
  public onPropertyChanged(
    newProp: ComboBoxModel,
    oldProp: ComboBoxModel
  ): void {
    if (this.getModuleName() === 'combobox') {
    ...
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        default: {
          // eslint-disable-next-line max-len
          const comboProps: { [key: string]: Object } = this.getPropObject(
            prop,
            <{ [key: string]: string }>newProp,
            <{ [key: string]: string }>oldProp
          );
          super.onPropertyChanged(
            comboProps.newProperty,
            comboProps.oldProperty
          );
          if (
            this.isFiltering() &&
            prop === 'dataSource' &&
  ...
  public hidePopup(e?: MouseEvent | KeyboardEventArgs | TouchEvent): void {
    /* eslint-enable valid-jsdoc, jsdoc/require-param */
    const inputValue: string | Object =
      this.inputElement && this.inputElement.value === ''
        ? null
        : this.inputElement && this.inputElement.value;
    if (!isNullOrUndefined(this.listData)) {
      ...
      const selected: HTMLElement = !isNullOrUndefined(this.list)
        ? <HTMLElement>(
            this.list.querySelector('.' + dropDownListClasses.selected)
          )
        : null;
      if (
        this.inputElement &&
        dataItem.text === this.inputElement.value &&
        !isNullOrUndefined(selected)
      ) {
        if (this.isSelected) {
          this.onChangeEvent(e);
      ...
      if (
        ...
        const type: string = this.typeOfData(dataSource).typeof as string;
        const searchItem: { [key: string]: number | Element } = Search(
          this.inputElement.value,
          this.liCollections,
          'Equal',
          true,
