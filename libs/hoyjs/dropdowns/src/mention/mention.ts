...
export class Mention extends DropDownBase {
  ...
  private hideWaitingSpinner(): void {
    ...
    if (
      !isNullOrUndefined(this.spinnerTemplate) &&
      !isNullOrUndefined(this.spinnerTemplateElement)
    ) {
  ...
  private checkAndUpdateInternalComponent(
    ...
    if (
      !(this as any).isVue &&
      targetElement.classList.contains('e-richtexteditor')
    ) {
    ...
    if (
      ...
      targetElement.classList.contains('e-rte-hidden')
    ) {
      const parentElement: HTMLElement = targetElement.parentElement;
      if (
        parentElement &&
        parentElement.classList.contains('e-richtexteditor')
      ) {
    ...
    if (
      ...
      targetElement.parentElement.classList.contains('e-rte-content')
    ) {
      this.isRTE = true;
  ...
  private showWaitingSpinner(): void {
    if (!isNullOrUndefined(this.popupObj)) {
      if (
        isNullOrUndefined(this.spinnerTemplate) &&
        isNullOrUndefined(this.spinnerElement)
      ) {
  ...
  private keyActionHandler(e: KeyboardEventArgs): void {
    ...
    if (
      ...
      e.action !== 'escape' &&
      e.action !== 'space' &&
      this.mentionChar.charCodeAt(0) ===
        this.getLastLetter(this.getTextRange()).charCodeAt(0)
    ) {
      this.renderList();
    }
    if (
    ...
    switch (e.action) {
      ...
      case 'enter':
        if (this.isPopupOpen) {
          ...
          if (
            this.popupObj &&
            this.popupObj.element.contains(this.selectedLI)
          ) {
  ...
  private onKeyUp(e: KeyboardEventArgs): void {
    ...
    this.isTyped =
      ...
      e.code !== 'ArrowUp'
        ? true
        : false;
    const isRteImage: boolean =
      document.activeElement.parentElement &&
      document.activeElement.parentElement.querySelector('.e-rte-image')
        ? true
        : false;
    ...
    const lastTwoLetters: string =
      this.mentionChar.toString() + this.mentionChar.toString();
    // eslint-disable-next-line security/detect-non-literal-regexp
    const Regex: RegExp = new RegExp(
      this.mentionChar.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&'),
      'g'
    );
    ...
    if (
      ...
      (this.isContentEditable(this.inputElement) &&
        this.range.startContainer &&
        (this.range.startContainer as HTMLElement).previousElementSibling &&
        (this.range.startContainer as HTMLElement).previousElementSibling
          .tagName !== 'BR' &&
        this.range.startContainer.textContent.split('').length > 0 &&
        (rangetextContent.length === 1 ||
          rangetextContent[rangetextContent.length - 2].indexOf('') === -1 ||
      ...
      if (
        ...
        currentRange &&
        currentRange.trim() !== '' &&
        charRegex.test(currentRange) &&
        currentRange.indexOf(this.mentionChar) !== -1 &&
        !this.isMatchedText() &&
        currentRange.length > 1 &&
        currentRange.replace(/\u00A0/g, ' ').charAt(currentRange.length - 2) !==
          ' ' &&
        this.list &&
        this.list.querySelectorAll('ul').length > 0 &&
        e.code !== 'Enter'
      ) {
        this.queryString = currentRange
          .substring(currentRange.lastIndexOf(this.mentionChar) + 1)
          .replace('\u00a0', ' ');
        this.searchLists(e);
      } else if (
        this.isPopupOpen &&
        (!this.allowSpaces || !lastWordRange) &&
        e.code !== 'ArrowDown' &&
        e.code !== 'ArrowUp'
      ) {
        this.hidePopup();
    ...
    if (
      this.mentionChar.charCodeAt(0) === lastWordRange.charCodeAt(0) &&
      this.queryString !== '' &&
      e.keyCode !== 38 &&
      e.keyCode !== 40 &&
      !this.lineBreak
      ...
      if (!this.isPopupOpen && this.queryString.length >= this.minLength) {
        if (!this.isContentEditable(this.inputElement)) {
          ...
          this.isContentEditable(this.inputElement) &&
          this.range &&
          this.range.startContainer !== this.inputElement &&
          e.keyCode !== 9
        ) {
          this.showPopup();
        }
      ...
      lastWordRange.indexOf(this.mentionChar) === 0 &&
      !this.isPopupOpen &&
      e.keyCode !== 8 &&
      (!this.popupObj ||
      ...
      if (
        ...
        e.keyCode !== 38 &&
        e.keyCode !== 40 &&
        this.queryString.length >= this.minLength
      ) {
      ...
      currentRange &&
      currentRange.trim() !== '' &&
      currentRange.replace('\u00a0', ' ').lastIndexOf(' ') <
        currentRange.length - 1 &&
      e.keyCode !== 38 &&
      e.keyCode !== 40 &&
      e.keyCode !== 8 &&
  ...
  private isMatchedText(): boolean {
    ...
    for (
      let i: number = 0;
      i < (this.liCollections && this.liCollections.length);
      i++
    ) {
      if (
        this.getTextRange() &&
        this.getTextRange()
          .substring(this.getTextRange().lastIndexOf(this.mentionChar) + 1)
          .replace('\u00a0', ' ')
          .trim() ===
          this.liCollections[i as number]
  ...
  private searchLists(e: KeyboardEventArgs | MouseEvent): void {
    ...
    if (
      e.type !== 'mousedown' &&
      ((<KeyboardEventArgs>e).keyCode === 40 ||
        (<KeyboardEventArgs>e).keyCode === 38)
    ) {
  ...
  protected onActionComplete(
    ...
    if (this.isActive) {
      if (!isNullOrUndefined(ulElement)) {
        ...
        const isRTESlashMenuPopup: boolean =
          this.isRTE &&
          this.cssClass &&
          this.cssClass.indexOf('e-slash-menu') > -1;
        if (isRTESlashMenuPopup) {
          ulElement.id = this.inputElement.id + '_slash_menu_options';
        }
      }
      const focusItem: HTMLLIElement = this.fields.disabled
        ? ulElement.querySelector(
            '.' + dropDownBaseClasses.li + ':not(.e-disabled)'
          )
        : ulElement.querySelector('.' + dropDownBaseClasses.li);
      if (focusItem) {
        focusItem.classList.add(dropDownBaseClasses.selected);
        this.selectedLI = focusItem;
        const value: string | number | boolean = this.getFormattedValue(
          focusItem.getAttribute('data-value')
        );
  ...
  private setDataIndex(): void {
    for (
      let i: number = 0;
      this.liCollections && i < this.liCollections.length;
      i++
    ) {
  ...
  protected listOption(
    ...
    fieldsSettings: FieldSettingsModel
  ): FieldSettingsModel {
    const fields: { [key: string]: Object } = <{ [key: string]: Object }>(
      super.listOption(dataSource, fieldsSettings)
    );
    if (isNullOrUndefined(fields.itemCreated)) {
      fields.itemCreated = (e: { [key: string]: HTMLElement }) => {
        if (this.highlight) {
          if (
            this.inputElement.tagName === this.getNgDirective() &&
            this.itemTemplate
          ) {
            setTimeout((): void => {
              highlightSearch(
  ...
  private elementValue(): string {
    if (!this.isContentEditable(this.inputElement)) {
      ...
      ).value.replace(this.mentionChar, '');
    } else {
      return (this.inputElement as HTMLElement).textContent.replace(
        this.mentionChar,
        ''
      );
    }
  ...
  protected getQuery(query: Query): Query {
    ...
    const filterType: string =
      this.queryString === '' && !isNullOrUndefined(this.elementValue())
        ? 'equal'
        : this.filterType;
    ...
    if (this.queryString !== null && this.queryString !== '') {
      ...
      if (
        ...
        const mapping: string = !isNullOrUndefined(this.fields.text)
          ? this.fields.text
          : '';
        filterQuery.where(
          mapping,
  ...
  private renderHightSearch(): void {
    if (this.highlight) {
      for (let i: number = 0; i < this.liCollections.length; i++) {
        const isHighlight: HTMLElement =
          this.ulElement.querySelector('.e-active');
        if (!isHighlight) {
  ...
  private getTextRange(): string {
    let text: string;
    if (!this.isContentEditable(this.inputElement)) {
      const component: HTMLInputElement | HTMLTextAreaElement = this
  ...
  public showPopup(): void {
    ...
    this.queryString = this.didPopupOpenByTypingInitialChar
      ? this.queryString
      : '';
      ...
      if (!args.cancel) {
        ...
        if (isNullOrUndefined(this.target)) {
          popupEle = this.createElement('div', {
            id: this.inputElement.id + '_popup',
            className:
              'e-mention e-popup ' +
        ...
        if (
          ...
          const li: HTMLLIElement = this.list.querySelector(
            '.' + dropDownBaseClasses.focus
          );
          if (!isNullOrUndefined(li)) {
            this.selectedLI = li;
        ...
        if (this.inputElement.parentElement) {
          const rteRootElement: HTMLElement =
            this.inputElement.parentElement.closest(
              '.e-richtexteditor'
            ) as HTMLElement;
          if (
            rteRootElement &&
            popupEle.firstElementChild &&
            popupEle.firstElementChild.childElementCount > 0
          ) {
            popupEle.firstElementChild.setAttribute(
              'aria-owns',
              rteRootElement.id
            );
        ...
        const popupLeft: number =
          popupEle.parentElement.offsetWidth - popupEle.offsetWidth;
          ...
          if (!eventArgs.cancel) {
            ...
            if (this.popupObj) {
              this.popupObj.show(
                ...
                this.zIndex === 1000 ? this.inputElement : null
              );
            }
            ...
            coordinates = this.getCoordinates(
              this.inputElement,
              this.getTriggerCharPosition()
            );
            if (!this.isCollided) {
              popupEle.style.cssText = 'top: '
                .concat(coordinates.top.toString(), 'px;\n left: ')
                .concat(
                  coordinates.left.toString(),
                  'px;\nposition: absolute;\n display: block;'
                );
            } else {
              if (
                this.collision.length > 0 &&
                this.collision.indexOf('right') > -1 &&
                this.collision.indexOf('bottom') === -1
              ) {
                popupEle.style.cssText = 'top: '
                  .concat(coordinates.top.toString(), 'px;\n left: ')
                  .concat(
                ...
                popupEle.style.top = formatUnit(
                  coordinates.top - parseInt(popupHeight.toString(), 10)
                );
              } else if (
                this.collision &&
                this.collision.length > 0 &&
                ...
                popupEle.style.left = formatUnit(coordinates.left);
                popupEle.style.top = formatUnit(
                  coordinates.top - parseInt(this.popupHeight.toString(), 10)
                );
              }
              this.isCollided = false;
              this.collision = [];
            }
            popupEle.style.width =
              this.popupWidth !== '100%' && !isNullOrUndefined(this.popupWidth)
                ? formatUnit(this.popupWidth)
                : 'auto';
            this.setHeight(popupEle);
            popupEle.style.zIndex =
              this.zIndex === 1000
                ? getZindexPartial(popupEle).toString()
  ...
  private setHeight(popupEle: HTMLElement): void {
    if (this.popupHeight !== 'auto' && this.list) {
      this.list.style.maxHeight =
        (parseInt(this.listHeight, 10) - 2).toString() + 'px'; // due to box-sizing property
  ...
  private checkCollision(popupEle: HTMLElement): void {
    if (
      !Browser.isDevice ||
      (Browser.isDevice && !(this.getModuleName() === 'mention'))
    ) {
      const coordinates: { [key: string]: number } = this.getCoordinates(
        this.inputElement,
        this.getTriggerCharPosition()
      );
      this.collision = isCollide(
        popupEle,
        null,
        coordinates.left,
        coordinates.top
  ...
  private setWidth(): string {
    ...
    if (width.indexOf('%') > -1) {
      const inputWidth: number =
        (this.inputElement.offsetWidth * parseFloat(width)) / 100;
  ...
  private getCoordinates(
    ...
    if (!this.isContentEditable(this.inputElement)) {
      ...
      if (
        this.getTextRange() &&
        this.getTextRange().lastIndexOf(this.mentionChar) !== -1
      ) {
      ...
      rect =
        range.getBoundingClientRect().top === 0
          ? (range.startContainer as any).getClientRects()[0]
    ...
    if (iframes.length > 0) {
      for (let i: number = 0; i < iframes.length; i++) {
        ...
        if (
          ...
          (iframe.contentDocument as Document).contains(element)
        ) {
    ...
    if (!isNullOrUndefined(range) && range.getBoundingClientRect().top === 0) {
      for (
        let i: number = 0;
        i < this.range.startContainer.childNodes.length;
        i++
      ) {
        if (
          this.range.startContainer.childNodes[i as number].nodeType !==
            Node.TEXT_NODE &&
          ...
          width += (
            this.range.startContainer.childNodes[i as number] as any
          ).getClientRects()[0].width;
        } else if (
          this.range.startContainer.childNodes[i as number].textContent !== ''
        ) {
          const span: HTMLElement = document.createElement('span');
          span.innerHTML =
            this.range.startContainer.childNodes[i as number].nodeValue;
    ...
    if (!this.isContentEditable(this.inputElement)) {
      coordinates = {
        top:
          ...
          3 -
          (element as HTMLInputElement | HTMLTextAreaElement).scrollTop -
      ...
      if (
        ...
        coordinates = {
          top:
            rectTop +
            windowTop +
            parseInt(getComputedStyle(this.inputElement).fontSize, 10),
  ...
  private updateValues(): void {
    const li: HTMLElement = this.list.querySelector(
      '.' + dropDownBaseClasses.focus
    );
    if (!isNullOrUndefined(li)) {
  ...
  private updateSelectedItem(
    ...
    if (
      ...
      !isNullOrUndefined(e) &&
      !(
        (e as KeyboardEventArgs).action === 'down' ||
        (e as KeyboardEventArgs).action === 'up'
      )
    ) {
  ...
  private setSelectOptions(
    ...
    if (
      e &&
      ((e as KeyboardEventArgs).keyCode === 38 ||
        (e as KeyboardEventArgs).keyCode === 40)
    ) {
      return;
    }
  ...
  private updateMentionValue(e?: KeyboardEventArgs | MouseEvent): void {
    ...
    if (!this.isContentEditable(this.inputElement)) {
      const myField: HTMLInputElement | HTMLTextAreaElement = this
        .inputElement as HTMLInputElement | HTMLTextAreaElement;
      const currentTriggerSnippet: string = this.getTextRange().substring(
        startPos + this.mentionChar.length,
        this.getTextRange().length
      );
      value += textSuffix;
      ...
      if (
        this.range &&
        this.range.startContainer.textContent.trim() !== this.mentionChar
      ) {
      ...
      if (
        (this.getTextRange() &&
          this.getTextRange().lastIndexOf(this.mentionChar) !== -1) ||
        (this.getTextRange() && this.getTextRange().trim() === this.mentionChar)
      ) {
        range.setStart(globalRange.startContainer, startPos);
        range.setEnd(globalRange.startContainer, endPos);
      } else {
        if (
          globalRange.commonAncestorContainer.textContent.trim() !== '' &&
          !isNullOrUndefined(
            globalRange.commonAncestorContainer.textContent.trim()
          ) &&
          this.getTextRange() &&
          ...
          range.setStart(
            globalRange.startContainer,
            globalRange.startOffset - 1
          );
      ...
      if (
        !isNullOrUndefined((e as PointerEvent).pointerType) &&
        (e as PointerEvent).pointerType === 'mouse'
      ) {
        const event: Event = new CustomEvent('content-changed', {
          detail: { click: true },
        });
  ...
  private mentionVal(value: string): string {
    ...
    if (
      !isNullOrUndefined(this.displayTemplate) &&
      !isNullOrUndefined(this.displayTempElement)
    ) {
    ...
    if (this.isContentEditable(this.inputElement)) {
      if (Browser.isAndroid) {
        return (
          '<span contenteditable="true" class="e-mention-chip">' +
          showChar +
          value +
          '</span>'.concat(
            typeof this.suffixText === 'string' ? this.suffixText : '&#8203;'
  ...
  private setDisplayTemplate(e?: KeyboardEventArgs | MouseEvent): void {
    ...
    const displayCompTemp: any = compiledString(
      ...
      this,
      'displayTemplate',
      this.displayTemplateId,
      this.isStringTemplate,
      null,
      this.displayTempElement
    );
  ...
  private setSpinnerTemplate(): void {
    ...
    const spinnerCompTemp: any = compiledString(
      ...
      this,
      'spinnerTemplate',
      this.spinnerTemplateId,
      this.isStringTemplate,
      null,
      this.spinnerTemplateElement
    );
  ...
  public search(text: string, positionX: number, positionY: number): void {
    ...
    if (
      (this.ignoreCase &&
        (text === lastWordRange || text === lastWordRange.toLowerCase())) ||
  ...
  public destroy(): void {
    ...
    if (
      this.inputElement &&
      !this.inputElement.classList.contains('e-' + this.getModuleName())
    ) {
      return;
    }
