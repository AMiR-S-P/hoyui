  ...
  implements INotifyPropertyChanged
{
  ...
  protected getPropObject(
    ...
    oldProp: { [key: string]: string }
  ): { [key: string]: Object } {
    const newProperty: { [key: string]: string } = <{ [key: string]: string }>(
      new Object()
    );
    const oldProperty: { [key: string]: string } = <{ [key: string]: string }>(
      new Object()
    );
    const propName: Function = (prop: string) => {
      return prop;
    };
    newProperty[propName(prop)] = (newProp as { [key: string]: string })[
      propName(prop)
    ];
    oldProperty[propName(prop)] = (oldProp as { [key: string]: string })[
      propName(prop)
    ];
    const data: { [key: string]: Object } = <{ [key: string]: Object }>(
      new Object()
    );
  ...
  private checkValueCase(
    ...
    const dataSource: { [key: string]: Object }[] = this.listData as {
      [key: string]: Object;
    }[];
    ...
    if (type === 'string' || type === 'number' || type === 'boolean') {
      ...
      if (ignoreCase) {
        (dataSource as { [key: string]: Object }[]).filter(
          (item: { [key: string]: Object }) => {
            const itemValue: string | number = getValue(fields.value, item);
            if (
              !isNullOrUndefined(itemValue) &&
              this.checkIgnoreCase(getValue(fields.text, item).toString(), text)
            ) {
  ...
  private checkingAccent(
    ...
    const textItem: string | object = DataUtil.ignoreDiacritics(
      text.toString()
    );
    let value: string | number = null;
    if (ignoreCase) {
    ...
    if (isTextByValue) {
      ...
      if (ignoreCase) {
        value =
          type === 'string'
            ? String(dataItem)
            : this.getFormattedValue(String(dataItem));
      } else {
        value =
          type === 'string' ? typedText : this.getFormattedValue(typedText);
      }
    }
    return value as string;
  }
  ...
  protected l10nUpdate(actionFailure?: boolean): void {
    const ele: Element =
      this.getModuleName() === 'listbox' ? this.ulElement : this.list;
    if (
      (!isNullOrUndefined(this.noRecordsTemplate) &&
        this.noRecordsTemplate !== 'No records found') ||
      this.actionFailureTemplate !== 'Request failed'
    ) {
      const template: string | Function = actionFailure
        ? this.actionFailureTemplate
        : this.noRecordsTemplate;
      let compiledString: Function;
      const templateId: string = actionFailure
        ? this.actionFailureTemplateId
        : this.noRecordsTemplateId;
      ele.innerHTML = '';
      ...
      if (noDataElement && noDataElement.length > 0) {
        for (let i: number = 0; i < noDataElement.length; i++) {
          if (
            ...
            templateName === 'noRecordsTemplate'
          ) {
            ...
            if (
              noDataElement[i as number] instanceof HTMLElement ||
              (noDataElement[i as number] instanceof Text &&
                noDataElement[i as number].textContent !== '')
            ) {
              ele.appendChild(noDataElement[i as number]);
            }
          }
      ...
      const componentLocale: L10n = new L10n(
        this.getLocaleName(),
        {},
        this.locale
      );
      if (
        componentLocale.getConstant('actionFailureTemplate') !== '' ||
        componentLocale.getConstant('noRecordsTemplate') !== ''
      ) {
      ...
      const content: string = actionFailure
        ? this.l10n.getConstant('actionFailureTemplate')
        : this.l10n.getConstant('noRecordsTemplate');
  ...
  protected getFilteringSkeletonCount(): void {
    ...
    this.skeletonCount =
      this.dataCount < this.itemCount * 2 &&
    ...
    if (!this.list.classList.contains(dropDownBaseClasses.noData)) {
      if (currentSkeletonCount !== this.skeletonCount && skeletonUpdated) {
        this.UpdateSkeleton(
          true,
          Math.abs(currentSkeletonCount - this.skeletonCount)
        );
      } else {
        this.UpdateSkeleton();
      }
      ...
      if (this.list.getElementsByClassName('e-virtual-ddl').length > 0) {
        ...
        this.skeletonCount > 0 &&
        this.list.querySelector('.e-dropdownbase')
      ) {
      ...
      if (
        this.list.getElementsByClassName('e-virtual-ddl-content').length > 0
      ) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (
  ...
  protected getSkeletonCount(retainSkeleton?: boolean): void {
    ...
    const actualCount: number =
      this.virtualListHeight > 0
        ...
        : 0;
    this.skeletonCount =
      actualCount * 4 < this.itemCount ? this.itemCount : actualCount * 4;
  ...
  protected GetVirtualTrackHeight(): string {
    let height: number =
      this.totalItemCount === this.viewPortInfo.endIndex
        ? this.totalItemCount * this.listItemHeight -
          this.itemCount * this.listItemHeight
        : this.totalItemCount * this.listItemHeight;
    height = this.isVirtualTrackHeight ? 0 : height;
    const heightDimension: string = `height: ${
      height - this.itemCount * this.listItemHeight
    }px;`;
    if (
      (this.getModuleName() === 'autocomplete' ||
        this.getModuleName() === 'multiselect') &&
  ...
  protected UpdateSkeleton(
    isSkeletonCountChange?: boolean,
    skeletonCount?: number
  ): void {
    const isContainSkeleton: Element | null = this.list.querySelector(
      '.e-virtual-ddl-content'
    );
    const isContainVirtualList: Element | null =
      this.list.querySelector('.e-virtual-list');
    if (
      isContainSkeleton &&
      (!isContainVirtualList || isSkeletonCountChange) &&
      this.isVirtualizationEnabled
    ) {
      const totalSkeletonCount: number = isSkeletonCountChange
        ? skeletonCount
        : this.skeletonCount;
      for (let i: number = 0; i < totalSkeletonCount; i++) {
        ...
        if (isContainSkeleton.firstChild) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          isContainSkeleton.firstChild.insertBefore(
            liElement,
            (isContainSkeleton.firstChild as any).children[0]
          );
        }
      }
    }
  ...
  protected getTextByValue(value: string | number | boolean): string {
    const text: string = this.checkValueCase(
      value as string,
      false,
      false,
      true
    ...
    if (this.listData && this.listData.length) {
      ...
      if (
        typeof getValue(
          ...
          item.item as { [key: string]: Object }
        ) === 'number' ||
        item.typeof === 'number'
      ) {
  ...
  private initialize(e?: MouseEvent | KeyboardEventArgs | TouchEvent): void {
    ...
    if (this.element.tagName === 'UL') {
      const jsonElement: { [key: string]: Object }[] =
        ListBase.createJsonFromElement(this.element);
      this.setProperties({ fields: { text: 'text', value: 'text' } }, true);
      this.resetList(jsonElement, this.fields);
    } else if (this.element.tagName === 'SELECT') {
      const dataSource: boolean =
  ...
  protected updateDataAttribute(value: { [key: string]: string }): void {
    ...
    for (let a: number = 0; a < this.element.attributes.length; a++) {
      if (
        ...
        attr[this.element.attributes[a as number].name] =
  ...
  private renderItemsBySelect(): void {
    ...
    const jsonElement: { [key: string]: Object }[] = [];
    const group: HTMLElement[] = <HTMLElement[] & NodeListOf<HTMLElement>>(
      element.querySelectorAll('select>optgroup')
    );
    const option: HTMLOptionElement[] = <
      HTMLOptionElement[] & NodeListOf<HTMLOptionElement>
    >element.querySelectorAll('select>option');
    this.getJSONfromOption(jsonElement, option, fields);
    if (group.length) {
      for (let i: number = 0; i < group.length; i++) {
        const item: HTMLOptGroupElement = group[
          i as number
        ] as HTMLOptGroupElement;
        const optionGroup: { [key: string]: {} } = {};
        optionGroup[fields.text] = item.label;
        optionGroup.isHeader = true;
        const child: HTMLOptionElement[] = <
          HTMLOptionElement[] & NodeListOf<HTMLOptionElement>
        >item.querySelectorAll('option');
  ...
  private updateFields(
    ...
    const field: Object = {
      fields: {
        text: text,
        value: value,
        groupBy: !isNullOrUndefined(groupBy)
          ? groupBy
          : this.fields && this.fields.groupBy,
        htmlAttributes: !isNullOrUndefined(htmlAttributes)
          ? htmlAttributes
          : this.fields && this.fields.htmlAttributes,
        iconCss: !isNullOrUndefined(iconCss)
          ? iconCss
          : this.fields && this.fields.iconCss,
        disabled: !isNullOrUndefined(disabled)
          ? disabled
          : this.fields && this.fields.disabled,
      },
    };
  ...
  private getJSONfromOption(
    ...
    for (const option of options) {
      ...
      json[fields.value] = !isNullOrUndefined(option.getAttribute(fields.value))
        ? option.getAttribute(fields.value)
        : option.innerText;
      items.push(json);
    }
  }
  ...
  private setListData(
    ...
    this.isActive = true;
    const eventArgs: ActionBeginEventArgs = {
      cancel: false,
      data: dataSource,
      query: query,
    };
    this.isPreventChange =
      this.isAngular && this.preventChange ? true : this.isPreventChange;
        ...
        (eventArgs: ActionBeginEventArgs) => {
          if (!eventArgs.cancel) {
            ...
            if (dataSource instanceof DataManager) {
              ...
              (eventArgs.data as DataManager)
                .executeQuery(this.getQuery(eventArgs.query as Query))
                .then((e: Object) => {
                  this.isPreventChange =
                  ...
                  if (!this.virtualSelectAll) {
                    const newQuery: Query = this.getQuery(
                      eventArgs.query as Query
                    );
                    for (
                      let queryElements: number = 0;
                      queryElements < newQuery.queries.length;
                      queryElements++
                    ) {
                      if (
                        newQuery.queries[queryElements as number].fn ===
                        'onWhere'
                      ) {
                        isWhereExist = true;
                      }
                    ...
                    if (
                      this.isVirtualizationEnabled &&
                      (e as any).count !== 0 &&
                      (e as any).count < this.itemCount * 2
                    ) {
                      if (newQuery) {
                        for (
                          let queryElements: number = 0;
                          ...
                          if (
                            ...
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            newQuery.queries[queryElements as number].e.nos = (
                              e as any
                            ).count;
                          }
                          if (
                            this.getModuleName() === 'multiselect' &&
                            (newQuery.queries[queryElements as number].e
                              .condition === 'or' ||
                              newQuery.queries[queryElements as number].e
                                .operator === 'equal')
                          ) {
                  ...
                  this.trigger('actionComplete', e, (e: Object) => {
                    if (!(e as { [key: string]: object }).cancel) {
                      ...
                      this.dataCount = !this.virtualSelectAll
                        ? (e as any).count
                        : this.dataCount;
                      this.totalItemCount = !this.virtualSelectAll
                        ? (e as any).count
                        : this.totalItemCount;
                      ulElement = this.renderItems(listItems, fields);
                      ...
                      if (this.keyboardEvent != null) {
                        this.handleVirtualKeyboardActions(
                          this.keyboardEvent,
                          this.pageCount
                        );
                      }
                      ...
                      if (this.virtualSelectAll && this.virtualSelectAllData) {
                        this.virtualSelectionAll(
                          this.virtualSelectAllState,
                          this.liCollections,
                          this.CurrentEvent
                        );
              ...
              if (
                ...
                const data: any = <{ [key: string]: Object }[]>(
                  new DataManager(<any[]>this.dataSource).executeLocal(
                    new Query().group(this.fields.groupBy)
                  )
                );
                  ...
                  if (newQuery) {
                    for (
                      ...
                      if (
                        ...
                        listItems = <{ [key: string]: Object }[]>(
                          newQuery.executeLocal(dataManager)
                        );
                      }
                      if (
                        this.getModuleName() === 'multiselect' &&
              ...
              if (
                ...
                // eslint-disable @typescript-eslint/no-explicit-any
                this.dataCount = this.totalItemCount = this.virtualSelectAll
                  ? (listItems as any).length
                  : (listItems as any).count;
              }
              listItems = this.isVirtualizationEnabled
                ? (listItems as any).result
                : listItems;
              ...
              this.trigger(
                'actionComplete',
                localDataArgs,
                (localDataArgs: { [key: string]: object }) => {
                  if (this.isIncrementalRequest) {
                    ulElement = this.renderItems(
                      localDataArgs.result as { [key: string]: Object }[],
                      fields
                    );
                    return;
                  }
                  if (!localDataArgs.cancel) {
                    ...
                    setTimeout(() => {
                      if (
                        ...
                        ulElement.childElementCount > 0 &&
                        (ulElement.children[0].childElementCount > 0 ||
                          (this.fields.groupBy &&
                            ulElement.children[1] &&
                            ulElement.children[1].childElementCount > 0))
                      ) {
                        this.updateDataList();
                      }
                    });
  ...
  private bindChildItems(
    ...
    if (listItems.length >= 100 && this.getModuleName() === 'autocomplete') {
      setTimeout(() => {
        const childNode: HTMLElement[] = this.remainingItems(
          this.sortedData,
          fields
        );
  ...
  protected isObjectInArray(objectToFind: any, array: any[]): boolean {
    return array.some((item: any) => {
      return Object.keys(objectToFind).every((key: string) => {
  ...
  protected findListElement(
    ...
    if (list) {
      const listArr: HTMLElement[] = [].slice.call(
        list.querySelectorAll(findNode)
      );
  ...
  private remainingItems(
    ...
    >new DataManager(dataSource as DataOptions | JSON[]).executeLocal(
      new Query().skip(100)
    );
    if (this.itemTemplate) {
      const listElements: HTMLElement = this.templateListItem(
        spliceData as { [key: string]: Object }[],
        fields
      );
  ...
  protected onActionComplete(
    ...
    if (!this.isVirtualizationEnabled) {
      this.fixedHeaderElement = isNullOrUndefined(this.fixedHeaderElement)
    ...
    if (
      ...
      for (let i: number = 0; i < ulElement.childElementCount; i++) {
        if (
          ulElement.children[i as number].classList.contains(
            'e-list-group-item'
          )
        ) {
          if (
            isNullOrUndefined(ulElement.children[i as number].innerHTML) ||
            ulElement.children[i as number].innerHTML === ''
          ) {
            addClass([ulElement.children[i as number]], HIDE_GROUPLIST);
          }
        }
        if (
          (ulElement.children[0] as HTMLElement).classList.contains(
            'e-hide-group-header'
          )
        ) {
          setStyleAttribute(ulElement.children[1] as HTMLElement, {
            zIndex: 11,
          });
        }
      }
  ...
  protected postRender(
    ...
    if (this.fields.disabled) {
      const liCollections: NodeListOf<Element> = <NodeListOf<Element>>(
        listElement.querySelectorAll('.' + dropDownBaseClasses.li)
      );
    ...
    /* eslint-enable @typescript-eslint/no-unused-vars */
    const focusItem: Element = this.fields.disabled
      ? listElement.querySelector(
          '.' + dropDownBaseClasses.li + ':not(.e-disabled'
        )
      : listElement.querySelector('.' + dropDownBaseClasses.li);
    const selectedItem: Element = listElement.querySelector(
  ...
  protected updateVirtualizationProperties(
    itemCount: number,
    filtering: boolean,
    isCheckbox?: boolean
  ): void {
  ...
  private renderGroupTemplate(listEle: HTMLElement): void {
    if (
      (this.fields.groupBy !== null && this.dataSource) ||
      this.element.querySelector('.' + dropDownBaseClasses.group)
    ) {
      const dataSource: { [key: string]: Object }[] = <
        { [key: string]: Object }[]
      >this.dataSource;
      const option: { [key: string]: Object } = {
        groupTemplateID: this.groupTemplateId,
        isStringTemplate: this.isStringTemplate,
      };
      const headerItems: Element[] = <NodeListOf<Element> & Element[]>(
      ...
      if (typeof this.groupTemplate !== 'function' && groupcheck) {
        const groupValue: string = select(
          this.groupTemplate,
          document
        ).innerHTML.trim();
        ...
        const tempHeaders: Element[] = ListBase.renderGroupTemplate(
          ...
          (this.fields as FieldSettingsModel & { properties: Object })
            .properties,
          headerItems,
          option,
          this
  ...
  protected createListItems(
    ...
    if (dataSource) {
      if (fields.groupBy || this.element.querySelector('optgroup')) {
        ...
        this.getModuleName() !== 'listbox' ||
        (this.getModuleName() === 'listbox' && !this.preventDefActionFilter)
      ) {
        dataSource = this.getSortedDataSource(dataSource);
      }
      const options: { [key: string]: Object } = <{ [key: string]: Object }>(
        this.listOption(dataSource, fields)
      );
      const spliceData: { [key: string]: Object }[] =
      ...
      return ListBase.createList(
        this.createElement,
        this.getModuleName() === 'autocomplete' ? spliceData : dataSource,
        options,
        true,
        this
      );
  ...
  protected listOption(
    ...
    const fieldValues: FieldSettingsModel = !isNullOrUndefined(
      (fields as FieldSettingsModel & { properties: Object }).properties
    )
      ? (fields as FieldSettingsModel & { properties: Object }).properties
      : fields;
    ...
    if (
      ...
      if (isNullOrUndefined(this.fixedHeaderElement)) {
        ...
        if (
          ...
          !this.list
            .querySelector('li')
            .classList.contains(dropDownBaseClasses.group)
        ) {
          this.fixedHeaderElement.style.display = 'none';
        }
        if (
          !isNullOrUndefined(this.fixedHeaderElement) &&
          !isNullOrUndefined(this.list)
        ) {
  ...
  protected scrollStop(e?: Event, isDownkey?: boolean): void {
    const target: Element = !isNullOrUndefined(e)
      ? <Element>e.target
      : this.list;
    const liHeight: number = parseInt(
      getComputedStyle(this.getValidLi(), null).getPropertyValue('height'),
      10
    );
    const topIndex: number = Math.round(target.scrollTop / liHeight);
    const liCollections: NodeListOf<Element> = <NodeListOf<Element>>(
      this.list.querySelectorAll('li' + ':not(.e-hide-listitem)')
    );
    ...
    for (let i: number = topIndex; i > -1; i--) {
      const index: number = this.isVirtualizationEnabled
        ? i + virtualListCount
        : i;
      if (this.isVirtualizationEnabled) {
        if (isCount) {
        ...
        if (
          this.fixedHeaderElement &&
          this.updateGroupHeader(index, liCollections, target)
        ) {
          break;
        }
        if (isDownkey) {
          if (
            (!isNullOrUndefined(liCollections[index as number]) &&
              liCollections[index as number].classList.contains(
                dropDownBaseClasses.selected
              ) &&
              this.getModuleName() !== 'autocomplete') ||
            (!isNullOrUndefined(liCollections[index as number]) &&
  ...
  protected getPageCount(returnExactCount?: boolean): number {
    if (this.list) {
      const liHeight: string = this.list.classList.contains(
        dropDownBaseClasses.noData
      )
        ? null
        : getComputedStyle(this.getItems()[0], null).getPropertyValue('height');
      const pageCount: number = Math.round(
        this.list.getBoundingClientRect().height / parseInt(liHeight, 10)
      );
  ...
  private updateGroupHeader(
    index: number,
    liCollections: NodeListOf<Element>,
    target: Element
  ): boolean {
    if (
      !isNullOrUndefined(liCollections[index as number]) &&
      ...
      this.updateGroupFixedHeader(
        liCollections[index as number] as HTMLElement,
        target
      );
      return true;
    } else {
  ...
  private updateGroupFixedHeader(element: HTMLElement, target: Element): void {
    if (this.fixedHeaderElement) {
      ...
      this.fixedHeaderElement.style.top =
        this.list.parentElement.offsetTop +
        this.list.offsetTop -
        window.scrollY +
        'px';
  ...
  protected getValidLi(): HTMLElement {
    if (this.isVirtualizationEnabled) {
      return this.liCollections[0].classList.contains('e-virtual-list')
    ...
    if (this.itemTemplate && listData) {
      ...
      const spliceData: { [key: string]: Object }[] =
        dataSource.length > 100
          ...
          : dataSource;
      ulElement = this.templateListItem(
        this.getModuleName() === 'autocomplete' ? spliceData : dataSource,
        fields
      );
      const oldUlElement: HTMLElement =
        this.list.querySelector('.e-list-parent');
      const virtualUlElement: HTMLElement = this.list.querySelector(
        '.e-virtual-ddl-content'
      );
      if (
        (listData.length >= this.virtualizedItemsCount &&
          oldUlElement &&
          virtualUlElement) ||
        (oldUlElement && virtualUlElement && this.isAllowFiltering) ||
        (oldUlElement &&
          virtualUlElement &&
          this.getModuleName() === 'autocomplete')
      ) {
        virtualUlElement.replaceChild(ulElement, oldUlElement);
        const reOrderList: NodeListOf<Element> =
          this.list.querySelectorAll('.e-reorder');
        if (
      ...
      if (this.getModuleName() === 'multiselect' && this.virtualSelectAll) {
        ...
        listData = listData.slice(
          this.virtualItemStartIndex,
          this.virtualItemEndIndex
        );
      }
      ulElement = this.createListItems(listData, fields);
      if (this.isIncrementalRequest) {
        this.incrementalLiCollections = <HTMLElement[] & NodeListOf<Element>>(
          ulElement.querySelectorAll('.' + dropDownBaseClasses.li)
        );
      ...
      if (this.isVirtualizationEnabled) {
        let oldUlElement: Element = this.list.querySelector(
          '.e-list-parent' + ':not(.e-reorder)'
        );
        const virtualUlElement: Element = this.list.querySelector(
          '.e-virtual-ddl-content'
        );
        const isRemovedUlelement: boolean = false;
        if (
          !oldUlElement &&
          this.list.querySelector('.e-list-parent' + '.e-reorder')
        ) {
          oldUlElement = this.list.querySelector(
            '.e-list-parent' + '.e-reorder'
          );
        }
        if (
          (listData.length >= this.virtualizedItemsCount &&
  ...
  protected templateListItem(
    ...
    const option: { [key: string]: Object } = <{ [key: string]: Object }>(
      this.listOption(dataSource, fields)
    );
  ...
  protected typeOfData(
    items: { [key: string]: Object }[] | string[] | number[] | boolean[]
  ): { [key: string]: Object } {
    let item: { [key: string]: Object } = { typeof: null, item: null };
    for (
      let i: number = 0;
      !isNullOrUndefined(items) && i < items.length;
      i++
    ) {
      if (!isNullOrUndefined(items[i as number])) {
        const listDataType: boolean =
          typeof items[i as number] === 'string' ||
          typeof items[i as number] === 'number' ||
          typeof items[i as number] === 'boolean';
        const isNullData: boolean = listDataType
          ? isNullOrUndefined(items[i as number])
          : isNullOrUndefined(
              getValue(
                this.fields.value ? this.fields.value : 'value',
                items[i as number]
        ...
        if (!isNullData) {
          return (item = {
            typeof: typeof items[i as number],
  ...
  protected setFixedHeader(): void {
    ...
    if (this.list && this.list.parentElement) {
      borderWidth = parseInt(
        document.defaultView
          .getComputedStyle(this.list.parentElement, null)
    ...
    const firstLi: HTMLElement = this.ulElement.querySelector(
      '.' + dropDownBaseClasses.group + ':not(.e-hide-listitem)'
    ) as HTMLElement;
  ...
  private getSortedDataSource(
    ...
    if (dataSource && this.sortOrder !== 'None') {
      ...
      if (
        this.typeOfData(dataSource).typeof === 'string' ||
        this.typeOfData(dataSource).typeof === 'number' ||
        this.typeOfData(dataSource).typeof === 'boolean'
      ) {
        textField = '';
      }
      dataSource = ListBase.getDataSource(
        dataSource,
        ListBase.addSorting(this.sortOrder, textField)
      );
    }
    return dataSource;
  }
  ...
  protected getIndexByValue(value: string | number | boolean | object): number {
    ...
    for (let i: number = 0; i < listItems.length; i++) {
      if (
        !isNullOrUndefined(value) &&
        listItems[i as number].getAttribute('data-value') === value.toString()
      ) {
        index = i;
        break;
      }
   ...
   * @returns {number} Returns the index of the item.
   */
  protected getIndexByValueFilter(
    ...
    const listItems: NodeListOf<Element> = ulElement.querySelectorAll(
      'li' + ':not(.e-list-group-item)'
    );
    if (listItems) {
      for (let i: number = 0; i < listItems.length; i++) {
        if (
    ...
    if (this.list) {
      if (
        ...
        (this.element.tagName === 'UL' &&
          (<HTMLUListElement>this.element).childNodes.length > 0)
      ) {
        const data: boolean =
      ...
      dataSource =
        ...
        this.selectData &&
        dataSource instanceof Array &&
        dataSource.length < this.selectData.length &&
        this.addedNewItem
          ? this.selectData
          : dataSource;
  ...
  protected updateSelectElementData(isFiltering: boolean): void {
    if (
      (isFiltering || this.isVirtualizationEnabled) &&
      isNullOrUndefined(this.selectData) &&
      this.listData &&
      this.listData.length > 0
    ) {
  ...
  protected updateDataSource(
    props?: DropDownBaseModel,
    oldProps?: DropDownBaseModel
  ): void {
    this.resetList(this.dataSource);
    this.totalItemCount =
      this.dataSource instanceof DataManager
        ? this.dataSource.dataSource.json.length
        : 0;
  }
  protected setUpdateInitial(
    props: string[],
    newProp: { [key: string]: string },
    oldProp?: { [key: string]: string }
  ): void {
    ...
    for (let j: number = 0; props.length > j; j++) {
      if (
        (newProp as { [key: string]: string })[props[j as number]] &&
        props[j as number] === 'fields'
      ) {
        this.setFields();
        (updateData as { [key: string]: string })[props[j as number]] = (
    ...
    if (Object.keys(updateData).length > 0) {
      if (Object.keys(updateData).indexOf('dataSource') === -1) {
        (
          updateData as { [key: string]: { [key: string]: Object }[] }
        ).dataSource = this.dataSource as { [key: string]: Object }[];
      }
      if (this.getModuleName() === 'listbox') {
        if (
          !this.isReact ||
          (this.isReact && !isNullOrUndefined(newProp.dataSource))
        ) {
  ...
  public onPropertyChanged(
    newProp: DropDownBaseModel,
    oldProp: DropDownBaseModel
  ): void {
  ...
  public render(
    ...
    const group: HTMLElement = <HTMLElement>(
      this.element.querySelector('select>optgroup')
    );
    if (
      (this.fields.groupBy || !isNullOrUndefined(group)) &&
  ...
  public getItems(): Element[] {
    return <HTMLElement[] & NodeListOf<Element>>(
  ...
  public addItem(
    ...
    if (this.sortOrder !== 'None' && isNullOrUndefined(itemIndex)) {
      let newList: { [key: string]: Object }[] = [].slice.call(
        this.listData as { [key: string]: Object }[]
      );
    ...
    const selectedItemValue: Element = this.list.querySelector(
      '.' + dropDownBaseClasses.selected
    );
    ...
    if (items && fields.groupBy) {
      items = ListBase.groupDataSource(
        items as { [key: string]: Object }[],
        (fields as FieldSettingsModel & { properties: Object }).properties
      );
    }
    const liCollections: HTMLElement[] = [];
    for (let i: number = 0; i < items.length; i++) {
      const item: { [key: string]: Object } | string | boolean | number =
        items[i as number];
      const isHeader: boolean = (item as { [key: string]: Object })
        .isHeader as boolean;
      ...
      const itemText: string =
        item instanceof Object ? getValue(fields.text, item) : item;
      if (isHeader) {
        li.innerText = itemText;
      }
      if (this.itemTemplate && !isHeader) {
        const itemCheck: boolean = this.templateCompiler(this.itemTemplate);
        const compiledString: Function =
          typeof this.itemTemplate !== 'function' && itemCheck
            ? compile(select(this.itemTemplate, document).innerHTML.trim())
            : compile(this.itemTemplate);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      li.setAttribute(
        'data-value',
        item instanceof Object ? getValue(fields.value, item) : item
      );
      li.setAttribute('role', 'option');
      this.notify('addItem', { module: 'CheckBoxSelection', item: li });
      liCollections.push(li);
      if (this.getModuleName() === 'listbox') {
        (this.listData as { [key: string]: Object }[]).splice(
          isListboxEmpty ? this.listData.length : index,
          0,
          item as { [key: string]: Object }
        );
        if (this.listData.length !== this.sortedData.length) {
          this.sortedData = this.listData;
        }
      } else {
        (this.listData as { [key: string]: Object }[]).push(
          item as { [key: string]: Object }
        );
      }
      if (
      ...
      if (this.getModuleName() === 'listbox') {
        this.updateActionCompleteData(
          li,
          item as { [key: string]: Object },
          isListboxEmpty ? null : index + i
        );
      } else {
        this.updateActionCompleteData(
          li,
    ...
    if (itemsCount === 0 && isNullOrUndefined(this.list.querySelector('ul'))) {
      ...
      const attr: string[] = [];
      for (let i: number = 0; i < items.length; i++) {
        const listGroupItem: NodeList =
          this.ulElement.querySelectorAll('.e-list-group-item');
        for (let j: number = 0; j < listGroupItem.length; j++) {
          attr[j as number] = (
            listGroupItem[j as number] as HTMLElement
          ).innerText;
        }
        if (
          attr.indexOf(liCollections[i as number].innerText) > -1 &&
          fields.groupBy
        ) {
          for (let j: number = 0; j < listGroupItem.length; j++) {
            if (attr[j as number] === liCollections[i as number].innerText) {
              if (this.sortOrder === 'None') {
                ...
                this.ulElement.insertBefore(
                  liCollections[i + 1],
                  this.ulElement.childNodes[itemIndex as number]
                );
              }
              i = i + 1;
              break;
          ...
          if (
            ...
            this.liCollections[index as number].parentNode.insertBefore(
              liCollections[i as number],
              this.liCollections[index as number]
            );
          } else {
            if (itemIndex && this.getModuleName() === 'listbox') {
              this.ulElement.insertBefore(
                liCollections[i as number],
                this.ulElement.childNodes[itemIndex + i]
              );
            } else {
  ...
  protected isDisabledItemByIndex(index: number): boolean {
    if (this.fields.disabled && this.liCollections) {
      return this.isDisabledElement(
        this.liCollections[index as number] as HTMLElement
      );
    }
    return false;
  }
      ...
      if (type === 'string' || type === 'number' || type === 'boolean') {
        ...
        for (const item of this.listData) {
          if (
            ...
            getValue(this.fields.value ? this.fields.value : 'value', item) ===
              value
          ) {
            return item;
          }
