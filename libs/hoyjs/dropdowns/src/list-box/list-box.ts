...
import { Input, InputObject } from '@hoyui/inputs';
import {
  DropDownBase,
  dropDownBaseClasses,
  FilteringEventArgs,
...
import { prepend, append } from '@hoyui/base';
import { cssClass, Sortable, moveTo, SortOrder } from '@hoyui/lists';
import {
...
export class ListBox extends DropDownBase {
  ...
  private checkBoxSelectionModule: {
    ...
    checkAllParent: HTMLElement;
    clearIconElement: HTMLElement;
  };
  ...
  private initWrapper(): void {
    const hiddenSelect: HTMLElement = this.createElement('select', {
    ...
    (this.list.querySelector('.e-list-parent') as any).setAttribute(
      'role',
      'presentation'
    );
    const groupHdrs: HTMLElement[] = <HTMLElement[] & NodeListOf<Element>>(
      this.list.querySelectorAll('.e-list-group-item')
    );
    ...
    if (this.list.getElementsByClassName('e-list-item')[0]) {
      this.list
        .getElementsByClassName('e-list-item')[0]
        .classList.remove(dropDownBaseClasses.focus);
    }
    if (this.itemTemplate) {
    ...
    removeClass(
      [this.list],
      [dropDownBaseClasses.content, dropDownBaseClasses.root]
    );
    this.validationAttribute(
      this.element as HTMLInputElement,
      hiddenSelect as HTMLSelectElement
    );
    ...
    attributes(this.list, {
      ...
      'aria-multiselectable':
        this.selectionSettings.mode === 'Multiple' ? 'true' : 'false',
    });
  ...
  private initDraggable(): void {
    ...
    if (this.allowDragAndDrop) {
      new Sortable(this.ulElement, {
        ...
        placeHolder: () => {
          return this.createElement('span', { className: 'e-placeholder' });
        },
  ...
  protected updateActionCompleteData(
    ...
    (this.jsonData as { [key: string]: Object }[]).splice(
      index === null ? this.jsonData.length : index,
      0,
      item
    );
  }
  ...
  private initToolbar(): void {
    ...
    this.toolbarSettings.items = isNullOrUndefined(this.toolbarSettings.items)
      ? []
      : this.toolbarSettings.items;
    if (this.toolbarSettings.items.length) {
      const toolElem: Element = this.createElement('div', {
      ...
      if (this.scope) {
        document
          .querySelector(this.scope)
          .setAttribute('data-value', this.element.id);
      } else {
    ...
    if (prevScope && scope && prevScope !== scope) {
      this.tBListBox = getComponent(
        document.getElementById(prevScope),
        this.getModuleName()
      );
  ...
  private setHeight(): void {
    const ele: HTMLElement = this.toolbarSettings.items.length
  ...
  private setCssClass(): void {
    const wrap: Element = this.toolbarSettings.items.length
      ? this.list.parentElement
      : this.list;
    if (this.cssClass) {
  ...
  private setEnable(): void {
    const ele: Element = this.toolbarSettings.items.length
      ? this.list.parentElement
      : this.list;
    if (this.enabled) {
  ...
  protected onActionComplete(
    ...
    let txtLength: number;
    if (
      this.allowFiltering &&
      this.list.getElementsByClassName('e-filter-parent')[0]
    ) {
      searchEle = this.list
        .getElementsByClassName('e-filter-parent')[0]
    ...
    if (this.allowFiltering) {
      filterElem = this.list.getElementsByClassName(
        'e-input-filter'
    ...
    if (this.allowFiltering && !isNullOrUndefined(searchEle)) {
      ...
      this.filterParent = this.list.getElementsByClassName(
        'e-filter-parent'
      )[0] as HTMLElement;
      this.filterWireEvents(searchEle);
      const inputSearch: HTMLElement =
        searchEle.querySelector('.e-input-filter');
      if (inputSearch) {
        inputSearch.addEventListener('focus', function (): void {
          if (
            !(searchEle.childNodes[0] as HTMLElement).classList.contains(
              'e-input-focus'
            )
          ) {
            (searchEle.childNodes[0] as HTMLElement).classList.add(
              'e-input-focus'
            );
          }
        });
        inputSearch.addEventListener('blur', function (): void {
          if (
            ...
            (searchEle.childNodes[0] as HTMLElement).classList.remove(
              'e-input-focus'
            );
          }
        });
    ...
    if (
      this.toolbarSettings.items.length &&
      this.scope &&
      this.scope.indexOf('#') > -1 &&
      !isNullOrUndefined(e)
    ) {
      const scope: string = this.scope.replace('#', '');
      const scopedLB: ListBox = getComponent(
        document.getElementById(scope),
        this.getModuleName()
      );
  ...
  private triggerDrag(args: DragEventArgs): void {
    ...
    if (this.itemTemplate && args.target) {
      if (args.target && args.target.closest('.e-list-item')) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const listItem: HTMLElement = (args as any).element.querySelector(
          '.e-list-item'
        );
        if (listItem) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const event: any = (args as any).event;
    let wrapper: HTMLElement;
    this.stopTimer();
    if (
      args.target &&
      (args.target.classList.contains('e-listbox-wrapper') ||
        args.target.classList.contains('e-list-item') ||
        args.target.classList.contains('e-filter-parent') ||
      ...
      if (scrollParent) {
        ...
        if (
          boundRect.y +
            ...
            (event.clientY + scrollMoved) <
          1
        ) {
          this.timer = window.setInterval(() => {
            this.setScrollDown(scrollParent, scrollHeight, true);
          }, 70);
        } else if (event.clientY - scrollMoved - boundRect.y < 1) {
          this.timer = window.setInterval(() => {
            this.setScrollDown(scrollParent, scrollHeight, false);
          }, 70);
        }
      }
    }
  ...
  private beforeDragEnd(args: DropEventArgs): void {
    this.stopTimer();
    const items: object[] = [];
    this.dragValue = this.getFormattedValue(
      args.droppedElement.getAttribute('data-value')
    ) as string;
  ...
  private dragEnd(args: DropEventArgs): void {
    ...
    let liColl: HTMLElement[];
    let jsonData: dataType[];
    let droppedData: dataType;
    let selectedOptions: (string | boolean | number)[];
    let sortedData: dataType[];
    const dropValue: string | number | boolean = this.getFormattedValue(
      args.droppedElement.getAttribute('data-value')
    );
    const listObj: ListBox = this.getComponent(args.droppedElement);
    const getArgs: Object = this.getDragArgs(
      { target: args.droppedElement } as DragEventArgs,
      true
    );
    const sourceArgs: Object = { previousData: this.dataSource };
    const destArgs: Object = { previousData: listObj.dataSource };
    let dragArgs: Object = extend({}, getArgs, {
      target: args.target,
    ...
    if (listObj !== this) {
      const sourceArgs1: Object = extend(sourceArgs, {
    ...
    if (targetListObj && targetListObj.listData.length === 0) {
      const noRecElem: Element = targetListObj.ulElement
    ...
    if (listObj === this) {
      ...
      let toIdx: number = (args.currentIndex = this.getCurIdx(
        this,
        args.currentIndex
      ));
      ...
      if (this.allowDragAll) {
        selectedOptions =
          this.value && Array.prototype.indexOf.call(this.value, dropValue) > -1
            ? this.value
            : [dropValue];
        ...
        selectedOptions.forEach((value: string) => {
          if (value !== dropValue) {
            ...
            const jsonIdx: number = jsonData.indexOf(
            ...
            sortedData.splice(
              toSortIdx,
              0,
              sortedData.splice(sIdx, 1)[0] as obj
            );
            liColl.splice(toIdx, 0, liColl.splice(idx, 1)[0] as HTMLElement);
            ul.insertBefore(
              this.getItems()[this.getIndexByValue(value)],
              ul.getElementsByClassName('e-placeholder')[0]
            );
          }
        });
      }
      (this.listData as dataType[]) = listData;
      (this.jsonData as dataType[]) = jsonData;
      (this.sortedData as dataType[]) = sortedData;
      this.liCollections = liColl;
    } else {
      let li: Element;
      const fLiColl: HTMLElement[] = [].slice.call(this.liCollections);
      let currIdx: number = (args.currentIndex = this.getCurIdx(
        listObj,
        args.currentIndex
      ));
      const ul: Element = listObj.ulElement;
      ...
      selectedOptions =
        this.value &&
        Array.prototype.indexOf.call(this.value, dropValue) > -1 &&
        this.allowDragAll
          ? this.value
          : [dropValue];
      if (!isNullOrUndefined(this.customDraggedItem)) {
      ...
      const fListData: dataType[] = [].slice.call(this.listData);
      const fSortData: dataType[] = [].slice.call(this.sortedData);
      selectedOptions.forEach((value: string) => {
        droppedData = this.getDataByValue(value);
        const srcIdx: number = (this.listData as dataType[]).indexOf(
          droppedData
        );
        const jsonSrcIdx: number = (this.jsonData as dataType[]).indexOf(
          droppedData
        );
        const sortIdx: number = (this.sortedData as dataType[]).indexOf(
          droppedData
        );
        fListData.splice(srcIdx, 1);
        this.jsonData.splice(jsonSrcIdx, 1);
        fSortData.splice(sortIdx, 1);
        (this.listData as dataType[]) = fListData;
        (this.sortedData as dataType[]) = fSortData;
        const destIdx: number =
          value === dropValue ? args.currentIndex : currIdx;
        ...
        if (!value) {
          ...
          for (let i: number = 0; i < liCollElem.length; i++) {
            if (
              liCollElem[i as number].getAttribute('data-value') === null &&
              liCollElem[i as number].classList.contains('e-list-item')
            ) {
        ...
        this.removeSelected(
          this,
          value === dropValue ? [args.droppedElement] : [li]
        );
        ul.insertBefore(li, ul.getElementsByClassName('e-placeholder')[0]);
        currIdx++;
      });
      if (this.fields.groupBy) {
        const sourceElem: HTMLElement = this.renderItems(
          this.listData as obj[],
          this.fields
        );
      ...
      if (
        listObj.sortOrder !== 'None' ||
        this.selectionSettings.showCheckbox !==
          listObj.selectionSettings.showCheckbox ||
        listObj.fields.groupBy ||
        listObj.itemTemplate ||
        this.itemTemplate
      ) {
        const sortable: { placeHolderElement: Element } = getComponent(
          ul as HTMLElement,
          'sortable'
        );
        const sourceElem: HTMLElement = listObj.renderItems(
          listData as obj[],
          listObj.fields
        );
        listObj.updateListItems(sourceElem, ul as HTMLElement);
        this.setSelection();
      ...
      (listObj.jsonData as dataType[]) = extend(
        [],
        [],
        jsonData,
        false
      ) as dataType[];
      (listObj.listData as dataType[]) = extend(
        [],
        [],
        listData,
        false
      ) as dataType[];
      (listObj.sortedData as dataType[]) = extend(
        [],
        [],
        sortedData,
        false
  ...
  private getCurIdx(listObj: ListBox, idx: number): number {
    if (listObj.fields.groupBy) {
      idx -= [].slice
        .call(listObj.ulElement.children)
        .slice(0, idx)
        .filter((ele: Element) =>
          ele.classList.contains(cssClass.group)
        ).length;
    }
    return idx;
  }
  ...
  private getComponent(li: Element): ListBox {
    ...
    const ele: HTMLElement = (
      this.element.tagName === 'EJS-LISTBOX'
        ? closest(li, '.e-listbox')
        : closest(li, '.e-listbox-wrapper') &&
          closest(li, '.e-listbox-wrapper').querySelector('.e-listbox')
    ) as HTMLElement;
    if (ele) {
  ...
  protected listOption(
    ...
    this.listCurrentOptions = extend(
      {},
  ...
  public enableItems(
    ...
    items.forEach((item: string) => {
      ...
      li = this.findListElement(
        this.list,
        'li',
        'data-value',
        isValue ? text : this.getValueByText(text)
      );
      if (!li) {
        return;
      }
  ...
  public selectItems(
    ...
    for (let i: number = 0; i < items.length; i++) {
      const liColl: NodeListOf<Element> = this.list.querySelectorAll(
        '[aria-selected="true"]'
      );
      for (let j: number = 0; j < liColl.length; j++) {
        if (
          ...
          (this.getFormattedValue(
            liColl[j as number].getAttribute('data-value')
          ) as string)
        ) {
  ...
  public removeItem(
    ...
    const liElement: HTMLElement[] | NodeListOf<HTMLLIElement> =
      this.list.querySelectorAll('.' + dropDownBaseClasses.li);
    if (items) {
      items = (items instanceof Array ? items : [items]) as
        | { [key: string]: Object }[]
        | string[]
        | boolean[]
        | number[];
      ...
      const objValue: { [key: string]: number } = {};
      const dupData: { [key: string]: Object }[] = [];
      extend(dupData, [], this.jsonData as { [key: string]: Object }[]);
      const removeIdxes: number[] = [];
      const removeLiIdxes: number[] = [];
      for (let i: number = 0; i < dupData.length; i++) {
        const value: any =
          dupData[i as number] instanceof Object
            ? dupData[i as number][fields.value]
            : dupData[i as number].toString();
        objValue[value as any] = i;
      }
      for (let j: number = 0; j < items.length; j++) {
        dataValue =
          items[j as number] instanceof Object
            ? (items[j as number] as any)[fields.value]
      ...
      const validRemoveIdxes: number[] = removeIdxes.sort(
   ...
   * @returns {object[]}.
   */
  public getDataByValues(
    value: string[] | number[] | boolean[]
  ): { [key: string]: Object }[] {
    const data: string | number | boolean | { [key: string]: Object }[] = [];
    for (let i: number = 0; i < value.length; i++) {
      data.push(
        this.getDataByValue(value[i as number]) as { [key: string]: Object }
      );
    }
    return data;
  }
  ...
  public moveTo(
    ...
    const elem: Element[] = value
      ? this.getElemByValue(value)
      : this.getSelectedItems();
    const tlistbox: ListBox = targetId
      ? getComponent(targetId, ListBox)
      : this.getScopedListBox();
  ...
  public getSortedList():
    ...
    let tempData: { [key: string]: Object }[] | string[] | boolean[] | number[];
  ...
  private selectAllItems(state: boolean, event?: MouseEvent): void {
        ...
        if (this.selectionSettings.showCheckbox) {
          ...
          if ((!ele && state) || (ele && !state)) {
            ...
            if (
              ...
              this.list.querySelectorAll('.e-list-item span.e-check').length
            ) {
    ...
    if (this.allowFiltering && this.selectionSettings.showCheckbox) {
      const liEle: HTMLCollectionOf<HTMLLIElement> =
        this.list.getElementsByTagName('li') as HTMLCollectionOf<HTMLLIElement>;
      let index: number = 0;
      if (state) {
        for (index = 0; index < liEle.length; index++) {
          const dataValue1: string = this.getFormattedValue(
            liEle[index as number].getAttribute('data-value')
          ) as string;
        ...
        for (index = 0; index < liEle.length; index++) {
          const dataValue2: string = this.getFormattedValue(
            liEle[index as number].getAttribute('data-value')
          ) as string;
          this.value = (this.value as string[]).filter(
            (e: string) => e !== dataValue2
          );
        }
      }
  ...
  private updateMainList(): void {
    const mainList: NodeListOf<Element> =
      this.mainList.querySelectorAll('.e-list-item');
    const ulList: NodeListOf<Element> =
      this.ulElement.querySelectorAll('.e-list-item');
    const mainCount: number = mainList.length;
    const ulEleCount: number = ulList.length;
    if (
      this.selectionSettings.showCheckbox ||
      document.querySelectorAll('ul').length > 1 ||
      mainCount !== ulEleCount
    ) {
      ...
      for (listindex; listindex < mainCount; ) {
        if (this.value) {
          for (valueindex; valueindex < this.value.length; valueindex++) {
            if (
              mainList[listindex as number].getAttribute('data-value') ===
              this.value[valueindex as number]
            ) {
              count++;
            }
          }
        ...
        if (
          ...
          this.mainList.removeChild(
            this.mainList.getElementsByTagName('li')[listindex as number]
          );
          listindex = 0;
        } else {
          listindex++;
  ...
  private wireEvents(): void {
    ...
    const wrapper: Element =
      this.element.tagName === 'EJS-LISTBOX' ? this.element : this.list;
    ...
    if (this.selectionSettings.showCheckbox) {
      EventHandler.remove(
        document,
        'mousedown',
        this.checkBoxSelectionModule.onDocumentClick
      );
    }
  ...
  private checkSelectAll(): void {
    ...
    const liItems: NodeListOf<Element> = this.list.querySelectorAll(
  ...
  protected getQuery(query: Query): Query {
    ...
    if (this.allowFiltering) {
      const filterType: string =
        this.inputString === '' ? 'contains' : this.filterType;
      let dataType: string = <string>(
        this.typeOfData(this.dataSource as { [key: string]: Object }[]).typeof
      );
      if (dataType === null) {
        dataType = <string>(
          this.typeOfData(this.jsonData as { [key: string]: Object }[]).typeof
        );
      }
      if (
  ...
  private setFiltering(): InputObject | void {
    ...
    if (this.initLoad || isNullOrUndefined(this.filterParent)) {
      ...
      if (this.height.toString().indexOf('%') < 0) {
        ...
        this.ulElement.style.height =
          this.ulElement.offsetHeight - this.filterParent.offsetHeight + 'px';
      }
      this.inputString = this.filterInput.value;
      this.filterWireEvents();
      this.ulElement.style.setProperty(
        'height',
        'calc(100% - ' + this.filterParent.offsetHeight + 'px)',
        'important'
      );
  ...
  private filterWireEvents(filterElem?: Element): void {
    ...
    this.clearFilterIconElem = this.filterInput.parentElement.querySelector(
      '.' + listBoxClasses.clearIcon
    );
  ...
  private selectHandler(
    ...
    if (li && li.parentElement) {
      ...
      if (!this.selectionSettings.showCheckbox) {
        if (
          (e.ctrlKey || e.metaKey || Browser.isDevice) &&
          this.isSelected(li)
        ) {
          li.classList.remove(cssClass.selected);
          li.removeAttribute('aria-selected');
      ...
      if (
        ...
        selectedLi = [].slice
          ...
          .slice(
            Math.min(currSelIdx, this.prevSelIdx),
            Math.max(currSelIdx, this.prevSelIdx) + 1
          )
          .filter((ele: Element) => {
      ...
      if (this.allowFiltering && !isKey) {
        const liDataValue: string = this.getFormattedValue(
          li.getAttribute('data-value')
        ) as string;
        if (!isSelect) {
          this.value = (this.value as string[]).filter(
            (value1: string) => value1 !== liDataValue
          );
        } else {
          const values: string[] = [];
        ...
        for (const item of this.listData) {
          for (let i: number = sIdx, len: number = elems.length; i < len; i++) {
            value = this.getFormattedValue(
              (elems[i as number] as Element).getAttribute('data-value')
            );
  ...
  private checkMaxSelection(): InputObject | void {
    const limit: number = this.list.querySelectorAll(
      '.e-list-item span.e-check'
    ).length;
    if (this.selectionSettings.showCheckbox) {
      let index: number = 0;
      const liCollElem: HTMLCollectionOf<HTMLLIElement> =
      ...
      for (index; index < liCollElem.length; index++) {
        if (!liCollElem[index as number].querySelector('.e-frame.e-check')) {
          if (limit === this.maximumSelectionLength) {
            ...
            liCollElem[index as number].classList.contains('e-disable')
          ) {
  ...
  private moveUpDown(
    ...
    if (
      ((isUp && this.isSelected(this.ulElement.firstElementChild)) ||
    ...
    const localDataArgs: { [key: string]: Object } = {
      cancel: false,
      items: tempItems,
      eventName: this.toolbarAction,
    };
    ...
    (isUp ? elems : elems.reverse()).forEach((ele: Element) => {
      const jsonToIdx: number = Array.prototype.indexOf.call(
        this.ulElement.querySelectorAll('.e-list-item'),
        ele
      );
      const idx: number = Array.prototype.indexOf.call(
        this.ulElement.children,
        ele
      );
      if (isTop) {
        ...
        moveTo(
          ...
          this.ulElement,
          [idx],
          this.ulElement.querySelectorAll('.e-list-item').length
        );
        this.changeData(
          idx,
          this.ulElement.querySelectorAll('.e-list-item').length,
    ...
    if (!isKey && this.toolbarSettings.items.length) {
      (
        this.getToolElem().querySelector(
          '[data-value=' + (isUp ? 'moveUp' : 'moveDown') + ']'
        ) as HTMLElement
      ).focus();
    }
    this.updateToolBarState();
  ...
  private moveData(
    ...
    const jsonIdx: number[] = [];
    const sortIdx: number[] = [];
    const listData: dataType[] = [].slice.call(fListBox.listData);
    const tListData: dataType[] = [].slice.call(tListBox.listData);
    const sortData: dataType[] = [].slice.call(fListBox.sortedData);
    let tSortData: dataType[] = [].slice.call(tListBox.sortedData);
    const fliCollections: HTMLElement[] = [].slice.call(fListBox.liCollections);
    const dataLiIdx: number[] = [];
    ...
    const isRefresh: boolean | string | Function =
      tListBox.sortOrder !== 'None' ||
      tListBox.selectionSettings.showCheckbox !==
        fListBox.selectionSettings.showCheckbox ||
      tListBox.fields.groupBy ||
      tListBox.itemTemplate ||
      fListBox.itemTemplate;
    fListBox.value = [];
    if (elems.length) {
      this.removeSelected(fListBox, elems);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const ulChildren: any = Array.prototype.slice.call(
        fListBox.ulElement.children
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const listItems: any = Array.prototype.slice.call(
        fListBox.ulElement.querySelectorAll('.e-list-item')
      );
      ...
      if (elems.length > 199) {
        for (const item of fListBox.listData) {
          if (!isNullOrUndefined(item)) {
            const key: string | number | boolean = fListBox.getFormattedValue(
              getValue(
                fListBox.fields.value ? fListBox.fields.value : 'value',
                item
              )
            );
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      if (index) {
        ...
        tListBox.liCollections = toColl
          .concat(rLiCollection.reverse())
          .concat(tliCollections);
      } else {
      ...
      if (isRefresh) {
        if (fListBox.fields.groupBy) {
          const sourceElem: HTMLElement = fListBox.renderItems(
            listData as obj[],
            fListBox.fields
          );
      ...
      if (isRefresh) {
        const sourceElem: HTMLElement = tListBox.renderItems(
          tListData as obj[],
          tListBox.fields
        );
    ...
    if (fListBox.value.length === 1 && fListBox.getSelectedItems().length) {
      fListBox.value[0] = fListBox.getFormattedValue(
        fListBox.getSelectedItems()[0].getAttribute('data-value')
      );
    }
  }
  ...
  private selectNextList(
    ...
    const childCnt: number =
      inst.ulElement.querySelectorAll('.e-list-item').length;
    let ele: Element;
    let liIdx: number;
  ...
  private moveAllData(
    ...
    const jsonData: { [key: string]: object }[] = [].slice.call(
      tListBox.jsonData
    );
    const isRefresh: boolean | string | Function =
      tListBox.sortOrder !== 'None' ||
      ...
      tListBox.itemTemplate ||
      fListBox.itemTemplate;
    const tempLiColl: HTMLElement[] = [];
    const tempData: { [key: string]: Object }[] = [];
    let flistboxarray: number[] = [];
    ...
    for (let i: number = 0; i < flistboxarray.length; i++) {
      if (this.sortOrder != null) {
        listData.splice(
          index + i,
          0,
          fListBox.sortedData[flistboxarray[i as number]]
        );
      } else {
        listData.splice(
          index + i,
          0,
          fListBox.listData[flistboxarray[i as number]]
        );
      }
    }
    for (let i: number = 0; i < flistboxarray.length; i++) {
      jsonData.splice(
        index + i,
        0,
        fListBox.jsonData[flistboxarray[i as number]] as {
          [key: string]: object;
        }
      );
    }
    ...
    if (index) {
      const toColl: HTMLElement[] = tliCollections.splice(0, index);
      tListBox.liCollections = toColl
        .concat(fliCollections)
        .concat(tliCollections);
    } else {
      tListBox.liCollections = tliCollections.concat(fliCollections);
    }
    fListBox.value = [];
    (listData as sortedType[]) = (listData as sortedType[]).filter(
      (data: sortedType) => data !== undefined
    );
    (listData as sortedType[]) = (listData as sortedType[]).filter(
      (data: sortedType) => (data as { isHeader: boolean }).isHeader !== true
    );
    const sortedData: dataType[] = listData.filter(function (
      val: dataType
    ): dataType {
    ...
    if (fListBox.listData.length === fListBox.jsonData.length) {
      ...
      if (tempLiColl.length > 0) {
        for (let i: number = 0; i < tempLiColl.length; i++) {
          ...
          disabledData.push(
            this.getDataByValue(
              tempLiColl[i as number].getAttribute('data-value')
            ) as any
          );
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        fListBox.listData = (fListBox as any).listData.filter(
          (obj1: any) => !disabledData.some((obj2: any) => obj1 === obj2)
        ) as any;
      }
      for (let i: number = 0; i < fListBox.listData.length; i++) {
        for (let j: number = 0; j < fListBox.jsonData.length; j++) {
          if (
            fListBox.listData[i as number] === fListBox.jsonData[j as number]
          ) {
    ...
    if (tempLiColl.length > 0) {
      const wrap: Element =
        this.list.parentElement.getElementsByClassName('e-listbox-tool')[0];
      const btn: HTMLButtonElement = wrap.querySelector(
        '[data-value="' + this.toolbarAction + '"]'
      );
      btn.disabled = true;
    }
  ...
  private changeData(
    ...
    const jsonIdx: number = Array.prototype.indexOf.call(
      this.jsonData,
      this.getDataByElems([ele])[0]
    );
    const sortIdx: number = Array.prototype.indexOf.call(
      this.sortedData,
      this.getDataByElems([ele])[0]
    );
  ...
  private getSelectedItems(): Element[] {
    ...
    if (this.selectionSettings.showCheckbox) {
      [].slice
        .call(this.ulElement.getElementsByClassName('e-check'))
        .forEach((cbox: Element) => {
          ele.push(closest(cbox, '.' + 'e-list-item'));
        });
    } else {
      ele = [].slice.call(
        this.ulElement.getElementsByClassName(cssClass.selected)
      );
    }
    return ele;
  }
  ...
  private getScopedListBox(): ListBox {
    ...
    if (this.scope) {
      [].slice
        .call(document.querySelectorAll(this.scope))
        .forEach((ele: Element) => {
  ...
  private getGrabbedItems(args: DragEventArgs): Element[] {
    ...
    for (let i: number = 0; i < this.value.length; i++) {
      if (
        ...
        (this.getFormattedValue(
          args.target.getAttribute('data-value')
        ) as string)
      ) {
        grabbItems = true;
        break;
    ...
    if (this.isAngular) {
      elems = Array.prototype.slice.call(
        this.element
          .getElementsByClassName('e-list-parent')[0]
          .querySelectorAll('.e-grabbed')
      );
    } else {
      elems = Array.prototype.slice.call(
        this.element.nextElementSibling.querySelectorAll('.e-grabbed')
      );
    }
    return elems;
  }
  ...
  private keyDownHandler(e: KeyboardEvent): void {
    if (
      [32, 35, 36, 37, 38, 39, 40, 65].indexOf(e.keyCode) > -1 &&
      (!this.allowFiltering ||
        (this.allowFiltering && e.target !== this.filterInput))
    ) {
      if (
        e.target &&
        (e.target as Element).className.indexOf('e-edit-template') > -1
      ) {
        return;
      }
      e.preventDefault();
      if (e.keyCode === 32 && this.ulElement.children.length) {
        ...
        (this.toolbarSettings.items.length || this.tBListBox) &&
  ...
  private upDownKeyHandler(e: KeyboardEvent): void {
    ...
    const defaultIdx: number =
      e.keyCode === 40 || e.keyCode === 36 ? 0 : ul.childElementCount - 1;
    let fliIdx: number = defaultIdx;
    const fli: Element =
      ul.getElementsByClassName('e-focused')[0] ||
      ul.getElementsByClassName(cssClass.selected)[0];
    if (fli) {
    ...
    if (cli) {
      ...
      if (
        ...
        this.selectHandler(
          {
            target: ul.children[fliIdx as number],
            ctrlKey: e.ctrlKey,
            shiftKey: e.shiftKey,
          },
          true
      ...
      if (
        ...
        const selectedidx: number = Array.prototype.indexOf.call(
          ul.children,
          fli
        );
        const sidx: number = e.code === 'Home' ? 0 : selectedidx;
        const eidx: number =
          e.code === 'Home' ? selectedidx : ul.children.length - 1;
          ...
          if (this.allowFiltering) {
            const eventArgsData: { [key: string]: Object } = {
              ...
              updateData: (
                ...
                this.preventDefActionFilter =
                  eventArgsData.preventDefaultAction as boolean;
            ...
            this.trigger(
              ...
              (args: FilteringEventArgs) => {
                ...
                if (
                  args.cancel ||
                  (this.filterInput.value !== '' && this.isFiltered)
                ) {
                  return;
                }
                this.preventDefActionFilter = false;
                if (
                  !args.cancel &&
                  !this.isCustomFiltering &&
                  !args.preventDefaultAction
                ) {
                  this.inputString = this.filterInput.value;
                  this.filteringAction(this.jsonData, new Query(), this.fields);
  ...
  protected targetElement(): string {
    this.targetInputElement = this.list.getElementsByClassName(
      'e-input-filter'
    )[0] as HTMLInputElement;
    return isNullOrUndefined(this.targetInputElement)
      ? null
  ...
  private dataUpdater(
    ...
    if (this.targetElement() && this.targetElement().trim() === '') {
      let list: HTMLElement = this.mainList.cloneNode
  ...
  private resizeHandler(): void {
    if (
      this.list &&
      !(this.cssClass && this.cssClass.indexOf('e-horizontal-listbox') > -1)
    ) {
      if (
        this.list.getElementsByClassName('e-filter-parent').length > 0 &&
        this.allowFiltering
      ) {
        this.ulElement.style.setProperty(
          'height',
  ...
  private getValidIndex(cli: Element, index: number, keyCode: number): number {
    ...
    if (
      cli.classList.contains('e-disabled') ||
      cli.classList.contains(cssClass.group)
    ) {
    ...
    this.getSelectedItems().forEach((ele: Element) => {
      if (!ele.classList.contains('e-grabbed')) {
        selectedOptions.push(
          this.getFormattedValue(ele.getAttribute('data-value')) as string
        );
      }
    });
  ...
  private clearSelection(
    values: (string | number | boolean)[] = this.value
  ): void {
    if (this.selectionSettings.showCheckbox) {
  ...
  private setSelection(
    ...
    values: (string | boolean | number | object)[] = this.value,
    ...
    if (values) {
      values.forEach((value: string) => {
        ...
        if (typeof text === 'string') {
          ...
          li = this.list.querySelector(
            '[data-value="' + text.replace(/"/g, '\\"') + '"]'
          );
        } else {
        ...
        if (li) {
          if (
            this.selectionSettings.showCheckbox &&
            !li.classList.contains('e-disabled')
          ) {
            liselect = li
              .getElementsByClassName('e-frame')[0]
              .classList.contains('e-check');
  ...
  private updateSelectTag(): void {
    const ele: Element = this.getSelectTag();
    ...
    if (this.value) {
      for (let i: number = 0, len: number = this.value.length; i < len; i++) {
        innerHTML +=
          '<option selected>' + this.value[i as number] + '</option>';
      }
      ele.innerHTML += innerHTML;
      for (
        let i: number = 0, len: number = ele.childNodes.length;
        i < len;
        i++
      ) {
        (ele.childNodes[i as number] as HTMLElement).setAttribute(
          'value',
          this.value[i as number].toString()
        );
      }
    }
    this.checkSelectAll();
  ...
  private checkDisabledState(inst: ListBox): boolean {
    if (isNullOrUndefined(inst.ulElement)) {
      ...
      if (
        ...
        isNullOrUndefined((this.dataSource as any).length)
      ) {
        return false;
      } else {
        return true;
        ...
        switch (value) {
          ...
          case 'moveUp':
            btn.disabled =
              this.value &&
              this.value.length &&
              !this.isSelected(this.ulElement.children[0])
                ? false
                : true;
            break;
          case 'moveDown':
  ...
  private showCheckbox(showCheckbox: boolean): void {
    let index: number = 0;
    const liColl: NodeListOf<Element> =
      this.list.lastElementChild.querySelectorAll('li');
    const liCollLen: number =
      this.list.lastElementChild.getElementsByClassName('e-list-item').length;
    if (showCheckbox) {
      ...
      if (
        this.selectionSettings.showSelectAll &&
        !this.list.getElementsByClassName('e-selectall-parent')[0]
      ) {
      ...
      for (index; index < liCollLen; index++) {
        if (liColl[index as number].classList.contains('e-list-item')) {
          liColl[index as number].removeChild(
            liColl[index as number].getElementsByClassName(
              'e-checkbox-wrapper'
            )[0]
          );
        }
  ...
  private isSelected(ele: Element): boolean {
    if (!isNullOrUndefined(ele)) {
      return (
        ele.classList.contains(cssClass.selected) ||
        ele.querySelector('.e-check') !== null
      );
    } else {
      return false;
    }
  ...
  public destroy(): void {
    ...
    if (this.element.tagName === 'EJS-LISTBOX') {
      ...
      if (this.toolbarSettings.items.length) {
        this.list.parentElement.parentElement.insertBefore(
          this.list,
          this.list.parentElement
        );
  ...
  public onPropertyChanged(newProp: ListBoxModel, oldProp: ListBoxModel): void {
    ...
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        case 'value':
          removeClass(
            this.list.querySelectorAll('.' + cssClass.selected),
            cssClass.selected
          );
        ...
        case 'scope':
          if (this.allowDragAndDrop) {
            (getComponent(this.ulElement, 'sortable') as Sortable).scope =
              newProp.scope;
          }
          if (this.toolbarSettings.items.length) {
            if (oldProp.scope) {
              (
                getComponent(
                  document.querySelector(oldProp.scope) as HTMLElement,
                  this.getModuleName()
                ) as ListBox
              ).tBListBox = null;
            }
            if (newProp.scope) {
              (
                getComponent(
                  document.querySelector(newProp.scope) as HTMLElement,
                  this.getModuleName()
                ) as ListBox
              ).tBListBox = this;
            }
        ...
        case 'toolbarSettings': {
          ...
          if (newProp.toolbarSettings.items) {
            oldProp.toolbarSettings.items = isNullOrUndefined(
              oldProp.toolbarSettings.items
            )
              ? []
              : oldProp.toolbarSettings.items;
            if (
              oldProp.toolbarSettings &&
              oldProp.toolbarSettings.items.length
            ) {
        ...
        case 'selectionSettings': {
          const showSelectAll: boolean =
            newProp.selectionSettings.showSelectAll;
