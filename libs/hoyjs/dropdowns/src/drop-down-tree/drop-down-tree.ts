...
import { DataManager, Query, DataUtil } from '@hoyui/data';
...
import { NodeCheckEventArgs, FailureEventArgs } from '@hoyui/navigations';
import {
  DropDownTreeModel,
  FieldsModel,
  TreeSettingsModel,
} from './drop-down-tree-model';
  ...
  implements INotifyPropertyChanged
{
  ...
  public render(): void {
    ...
    if (isTree) {
      const popupDiv: Element = select(
        '#' + this.element.id + '_options',
        document
      );
    ...
    if (this.element.tagName === 'INPUT') {
      ...
      if (!isNOU(this.element.id)) {
        this.inputEle = this.createElement('input', {
          attrs: { role: 'textbox', type: 'text', id: this.element.id },
        }) as HTMLInputElement;
      } else {
        this.inputEle = this.createElement('input', {
    ...
    if (this.allowMultiSelection || this.showCheckBox) {
      ...
      if (!this.wrapText && this.mode !== 'Custom') {
        ...
        this.inputWrapper.insertBefore(
  ...
  private hideCheckAll(flag: boolean): void {
    const checkAllEle: HTMLElement = !isNOU(this.popupEle)
      ? (this.popupEle.querySelector('.' + CHECKALLPARENT) as HTMLElement)
      : null;
  ...
  private filterKeyAction(e: KeyboardEventArgs): void {
    ...
    this.trigger(
      'keyPress',
      eventArgs,
      (observedArgs: DdtKeyPressEventArgs) => {
    ...
    this.trigger('filtering', args, (args: DdtFilteringEventArgs) => {
      if (!args.cancel) {
        ...
        if (
          this.hasTemplate &&
          (this as any).portals &&
          (this.treeObj as any).portals
        ) {
          for (
            let i: number = 0;
            i < (this.treeObj as any).portals.length;
            i++
          ) {
            if (
              (this as any).portals.indexOf(
                (this.treeObj as any).portals[i as number]
              ) === -1
            ) {
              (this as any).portals.push(
                (this.treeObj as any).portals[i as number]
              );
            }
          }
  ...
  private remoteDataFilter(
    value: string,
    filteredFields: FieldsModel
  ): FieldsModel {
    filteredFields.dataSource = this.treeData
      .map((item: { [key: string]: Object }) =>
        this.remoteChildFilter(value, item)
      )
      .filter(
        (filteredChild: { [key: string]: Object }) => !isNOU(filteredChild)
      );
  ...
  private remoteChildFilter(
    ...
    isChild?: boolean,
    isChildFiltering?: boolean
  ): { [key: string]: Object } {
    const children: { [key: string]: Object }[] = this.isChildObject()
      ? (node['child'] as { [key: string]: Object }[])
      : (node[this.fields.child as string] as { [key: string]: Object }[]);
    if (isNOU(children)) {
      return this.isMatchedNode(value, node, isChild, isChildFiltering)
        ? node
        : null;
    }
    const matchedChildren: { [key: string]: Object }[] = [];
    for (let i: number = 0; i < children.length; i++) {
      const filteredChild: { [key: string]: Object } = this.remoteChildFilter(
        value,
    ...
    let filteredItems: { [key: string]: Object } = (<any>Object).assign(
      {},
      node
    );
    ...
    if (matchedChildren.length !== 0) {
      ...
      filteredItems = this.isMatchedNode(value, filteredItems)
        ? filteredItems
        : null;
    }
  ...
  private nestedFilter(
    ...
    for (let i: number = 0; i < this.treeData.length; i++) {
      const filteredChild: { [key: string]: Object } = this.nestedChildFilter(
        value,
  ...
  private nestedChildFilter(
    ...
    const children: { [key: string]: Object }[] = <{ [key: string]: Object }[]>(
      node[this.fields.child as string]
    );
    if (isNOU(children)) {
      ...
      const filteredItems: { [key: string]: Object } = (<any>Object).assign(
        {},
        node
      );
      if (matchedChildren.length !== 0) {
  ...
  private selfReferencefilter(
    ...
    for (let i: number = 0; i < matchedData.length; i++) {
      if (matchedDataSource.indexOf(matchedData[i as number]) === -1) {
        ...
        let parentId: object =
          matchedData[parseInt(i.toString(), 10)][this.fields.parentValue];
        while (!isNOU(parentId)) {
          let parent: { [key: string]: Object } = null;
          for (let j: number = 0; j < this.treeData.length; j++) {
            const value: object =
              this.treeData[parseInt(j.toString(), 10)][this.fields.value];
  ...
  private isMatchedNode(
    ...
    const isObjectValue: boolean =
      isChild && isChildFiltering && this.isChildObject();
    checkValue = isObjectValue
      ? (node[(this.fields.child as FieldsModel).text] as string)
      : (node[this.fields.text] as string);
  ...
  private focusOut(e: MouseEvent): void {
    ...
    if (
      ...
      isNOU(target) &&
      (e.target !== this.inputWrapper || !this.isPopupOpen)
    ) {
      this.onFocusOut(e);
    }
  }
  ...
  private onFocusOut(event?: MouseEvent): void {
    ...
    if (this.showClearButton) {
      this.clearIconWidth = (<HTMLElement>(
        select('.e-clear-icon', this.inputWrapper)
      )).offsetWidth;
      addClass([this.overAllClear], HIDEICON);
      removeClass([this.inputWrapper], SHOW_CLEAR);
    }
    ...
    if (this.allowMultiSelection || this.showCheckBox) {
      const isValue: boolean = this.value
        ? this.value.length
          ? true
          : false
        : false;
  ...
  private updateView(): void {
    if (
      (!this.showCheckBox && !this.allowMultiSelection) ||
      this.mode === 'Custom' ||
      this.inputFocus
    ) {
      return;
        ...
        if (!observedArgs.cancel) {
          switch (e.action) {
            ...
            case 'moveDown':
              e.preventDefault();
              focusedElement =
                this.treeObj.element.querySelector('li[tabindex="0"]') ||
                this.treeObj.element.querySelector('li');
              focusedElement.focus();
  ...
  private updateOverFlowView(): void {
    ...
    if (this.value && this.value.length) {
      ...
      const remainContent: string = this.l10n.getConstant(
        'overflowCountTemplate'
      );
      const totalContent: string = this.l10n.getConstant('totalCountTemplate');
      const remainElement: HTMLElement = this.createElement('span', {
        className: REMAIN_WRAPPER,
      });
      this.overFlowWrapper.appendChild(remainElement);
      remainElement.innerText = remainContent.replace(
        '${count}',
      ...
      if (this.showDropDownIcon) {
        downIconWidth = (<HTMLElement>select('.' + DDTICON, this.inputWrapper))
          .offsetWidth;
      }
      if (!isNOU(this.value)) {
        if (this.mode !== 'Box') {
          if (this.valueTemplate) {
            remaining = this.updateChipAndValueTemplate(
              false,
              downIconWidth,
              remainSize
            );
            this.checkRemainingTemplate(
              remaining,
              remainElement,
              remainContent,
              totalContent
          ...
          for (
            let index: number = 0;
            !isNOU(this.value[index as number]);
            index++
          ) {
            ...
            if (this.enableHtmlSanitizer) {
              this.overFlowWrapper.innerText =
                SanitizeHtmlHelper.sanitize(data);
            } else {
            ...
            if (
              ...
              while (
                ...
                const textArr: string[] = this.overFlowWrapper.innerHTML.split(
                  this.delimiterChar
                );
                textArr.pop();
                this.overFlowWrapper.innerHTML = textArr.join(
                  this.delimiterChar
                );
                remaining++;
                wrapperleng = this.overFlowWrapper.offsetWidth;
  ...
  private checkRemainingTemplate(
    ...
    if (
      this.mode === 'Box' &&
      !this.overFlowWrapper.classList.contains(TOTAL_COUNT_WRAPPER)
    ) {
  ...
  private updateChipAndValueTemplate(
    isChip: boolean,
    downIconWidth: number = 0,
    remainSize: number
  ): number {
    let currentHtmlContent: string = '';
    let overAllContainer: number;
    let previousHtmlContent: string;
    let previousData: string;
    let index: number = 1;
    let wrapperLength: number;
    let remainingItemsCount: number;
    addClass(
      [isChip ? this.chipWrapper : this.valueTemplateContainer],
      HIDEICON
    );
    const clonedElement: HTMLElement = <HTMLElement>(
      (isChip
        ? (this.chipWrapper as Node)
        : (this.valueTemplateContainer as Node)
      ).cloneNode(true)
    );
    const valueElements: Element[] = isChip
      ? selectAll('.' + CHIP, clonedElement)
      : Array.prototype.slice.call(clonedElement.children);
    for (let i: number = 0; i < valueElements.length; i++) {
      ...
      overAllContainer = this.inputWrapper.offsetWidth;
      if (
        wrapperLength + downIconWidth + this.clearIconWidth >
        overAllContainer
      ) {
        ...
        wrapperLength = this.overFlowWrapper.offsetWidth;
        while (
          wrapperLength + remainSize + downIconWidth + this.clearIconWidth >=
            overAllContainer &&
          wrapperLength !== 0 &&
          this.overFlowWrapper.innerHTML !== ''
        ) {
  ...
  private updateRemainTemplate(
    ...
    if (
      ...
      this.overFlowWrapper.firstChild.nodeType === 3 &&
    ...
    remainElement.innerText =
      ...
      (this.overFlowWrapper.firstChild.nodeType === 3 ||
        this.mode === 'Box' ||
        this.valueTemplateContainer)
        ? remainContent.replace('${count}', remaining.toString())
        : totalContent.replace('${count}', remaining.toString());
    if (
      this.overFlowWrapper.firstChild &&
      (this.overFlowWrapper.firstChild.nodeType === 3 || this.mode === 'Box')
    ) {
  ...
  private getOverflowVal(index: number): string {
    const selectedData: { [key: string]: Object } = this.getSelectedData(
      this.value[parseInt(index.toString(), 10)]
    );
    return getValue(
      this.treeSettings.loadOnDemand ? this.fields.text : 'text',
      selectedData
    );
  }
  ...
  private updateDelimMode(): void {
    if (this.mode !== 'Box') {
      if (
        select('.' + REMAIN_WRAPPER, this.overFlowWrapper) &&
        !this.overFlowWrapper.classList.contains(TOTAL_COUNT_WRAPPER)
      ) {
  ...
  private createClearIcon(): void {
    ...
    if (this.showClearButton) {
      this.inputWrapper.insertBefore(
        this.overAllClear,
        this.inputObj.buttons[0]
      );
    }
  }

  private validationAttribute(): void {
    const name: string = this.inputEle.getAttribute('name')
      ? this.inputEle.getAttribute('name')
      : this.inputEle.getAttribute('id');
  ...
  private getValidMode(): boolean {
    if (this.allowMultiSelection || this.showCheckBox) {
      return this.mode === 'Box'
        ? true
        : this.mode === 'Default' && this.inputFocus
        ? true
        : false;
    } else {
      return false;
  ...
  private clickHandler(e: MouseEvent | KeyboardEvent): void {
    ...
    if (
      e.currentTarget &&
      (e.currentTarget as HTMLElement).classList.contains(CHECKALLPARENT)
    ) {
      target = (e.currentTarget as HTMLElement).firstElementChild
        .lastElementChild;
    } else {
      target = <Element>e.target;
    }
    this.checkWrapper = closest(
      target as HTMLElement,
      '.' + CHECKBOXWRAP
    ) as HTMLElement;
    if (!isNOU(this.checkWrapper)) {
      this.isClicked = true;
      const checkElement: Element = select(
        '.' + CHECKBOXFRAME,
        this.checkWrapper
      );
      this.changeState(
        this.checkWrapper,
        checkElement.classList.contains(CHECK) ? 'uncheck' : 'check',
        e
      );
  ...
  private setLocale(unSelect?: boolean): void {
    ...
    if (
      ...
      const template: string = unSelect
        ? this.unSelectAllText
        : this.selectAllText;
      this.selectAllSpan.textContent = '';
      const compiledString: Function = compile(template);
      const templateName: string = unSelect
        ? 'unSelectAllText'
        : 'selectAllText';
      for (const item of compiledString(
        {},
        this,
        templateName,
        null,
      ...
      this.selectAllSpan.textContent = unSelect
        ? this.unSelectAllText
        : this.selectAllText;
    }
  }
    ...
    if (this.valueTemplate) {
      ...
      if (this.hasTemplate && (this as any).portals) {
        if ((this.treeObj as any).portals) {
          (this as any).portals = (this as any).portals.concat(
            (this.treeObj as any).portals.filter(
              (item: any) => !(this as any).portals.includes(item)
            )
          );
        }
        if ((this as any).isReact) {
  ...
  private getValueTemplateElement(
    ...
    const selectedData: { [key: string]: Object } = this.getNodeData(
      value,
      this.isFilteredData ? this.treeData : this.treeItems
    );
    let templateElements: Element[] = compiledString(
      selectedData,
      this,
      'valueTemplate',
      `${this.element.id}valueTemplate`,
      this.isStringTemplate,
      undefined,
      this.valueTemplateContainer
    );
  ...
  private setValidValue(): void {
    if (!this.showCheckBox && !this.allowMultiSelection) {
      ...
      if (this.showCheckBox) {
        const difference: boolean =
          this.value.length !== this.treeObj.checkedNodes.length ||
          this.value.filter((e: string) => {
    ...
    if (
      !isNOU(this.value) &&
      this.value.length > 0 &&
      !isNOU(this.currentText)
    ) {
      this.inputWrapper.setAttribute(
        'aria-label',
        this.currentText.replace(/,/g, ', ')
      );
    }
    if (this.isInitialized) {
  ...
  private getItems(givenText: string): { [key: string]: Object } {
    ...
    if (this.treeDataType === 1) {
      for (let i: number = 0; i < this.treeItems.length; i++) {
        const text: Object = getValue(
          this.fields.text,
          this.treeItems[parseInt(i.toString(), 10)]
        );
        if (
          !isNOU(this.treeItems[i as number]) &&
          !isNOU(text) &&
          text === givenText
        ) {
  ...
  private getNestedItems(
    ...
    for (let i: number = 0, objlen: number = data.length; i < objlen; i++) {
      ...
      if (data[i as number] && dataId && dataId.toString() === givenText) {
        ...
        typeof field.child === 'string' &&
        !isNOU(getValue(field.child, data[i as number]))
      ) {
        const childData: Object = getValue(
          field.child,
          data[parseInt(i.toString(), 10)]
        );
        newData = this.getNestedItems(
          <{ [key: string]: Object }[]>childData,
          this.getChildType(field),
          givenText
        );
        ...
        this.fields.dataSource instanceof DataManager &&
        !isNOU(getValue('child', data[i as number]))
      ) {
        const child: string = 'child';
        newData = this.getNestedItems(
          <{ [key: string]: Object }[]>(
            getValue(child, data[parseInt(i.toString(), 10)])
          ),
          this.getChildType(field),
          givenText
        );
  ...
  private getChildType(mapper: FieldsModel): FieldsModel {
    return typeof mapper.child === 'string' || isNOU(mapper.child)
      ? mapper
      : mapper.child;
  }
    ...
    this.trigger('beforeOpen', args, (args: DdtBeforeOpenEventArgs) => {
      if (!args.cancel) {
        ...
        if (this.isFirstRender) {
          this.popupEle = this.createElement('div', {
            ...
            className:
              POPUP_CLASS + ' ' + (this.cssClass != null ? this.cssClass : ''),
          });
      ...
      if ((this.isFirstRender && !isCancelled) || this.isFilteredData) {
        ...
        if (
          this.showCheckBox &&
          this.showSelectAll &&
          !this.popupDiv.classList.contains(NODATA)
        ) {
      ...
      if (!isCancelled) {
        ...
        this.popupObj.refreshPosition();
        if (
          !(this.showCheckBox && this.showSelectAll) &&
          !this.popupDiv.classList.contains(NODATA) &&
          this.treeItems.length > 0
        ) {
          let focusedElement: HTMLElement =
            this.value != null && this.text != null
              ? this.treeObj.element.querySelector(
                  '[data-uid="' + this.value[0] + '"]'
                )
              : null;
          if (focusedElement) {
            ...
            const oldFocussedNode: HTMLElement =
              this.treeObj.element.querySelector('.e-node-focus');
            focusedElement =
              this.treeObj.element.querySelector(
                'li[tabindex="0"]:not(.e-disable):not(.e-prevent)'
              ) ||
              this.treeObj.element.querySelector(
                'li:not(.e-disable):not(.e-prevent)'
              );
        ...
        if (this.treeObj.checkedNodes.length > 0 && !this.isFilterRestore) {
          ...
          const checkedNodes: NodeList = this.treeObj.element.querySelectorAll(
            'li[aria-checked=true]'
          );
          if (
            (checkedNodes.length === nodes.length || this.checkSelectAll) &&
            this.checkBoxElement
          ) {
            const wrap: HTMLElement = closest(
              this.checkBoxElement as HTMLElement,
              '.' + CHECKBOXWRAP
            ) as HTMLElement;
  ...
  private updatePopupHeight(): void {
    ...
    if (this.allowFiltering) {
      const height: number = Math.round(
        this.filterContainer.getBoundingClientRect().height
      );
    ...
    if (
      ...
      const height: number = Math.round(
        this.checkAllParent.getBoundingClientRect().height
      );
      popupHeight = formatUnit(parseInt(popupHeight, 10) - height + 'px');
    }
    if (this.footerTemplate) {
    ...
    let border: number = parseInt(
      window.getComputedStyle(this.popupEle).borderTopWidth,
      10
    );
    border =
      border +
      parseInt(window.getComputedStyle(this.popupEle).borderBottomWidth, 10);
  ...
  private setElementWidth(inputWidth: string | number): void {
    ...
    if (!isNOU(inputWidth)) {
      if (typeof inputWidth === 'number') {
        ...
        ddElement.style.width = inputWidth.match(/px|%|em/)
          ? inputWidth
  ...
  private setWidth(): string {
    ...
    if (width.indexOf('%') > -1) {
      width =
        ((this.inputWrapper.offsetWidth * parseFloat(width)) / 100).toString() +
        'px';
  ...
  private getHeight(): string {
    ...
    if (height.indexOf('%') > -1) {
      ...
      height =
        (
          (document.documentElement.clientHeight * parseFloat(height)) /
          100
        ).toString() + 'px';
  ...
  private onDocumentClick(e: MouseEvent): void {
    ...
    const isScroller: boolean = target.classList.contains(DROPDOWN)
      ? true
      : matches(target, '.e-ddt .e-popup') ||
        matches(target, '.e-ddt .e-treeview');
    if (
      (this.isPopupOpen &&
        ((!isNOU(this.inputWrapper) && this.inputWrapper.contains(target)) ||
          isTree ||
          isScroller ||
          isHeader ||
          isFooter)) ||
      ((this.allowMultiSelection || this.showCheckBox) &&
        ((this.isPopupOpen && target.classList.contains(CHIP_CLOSE)) ||
          (this.isPopupOpen &&
            (target.classList.contains(CHECKALLPARENT) ||
              target.classList.contains(ALLTEXT) ||
              target.classList.contains(CHECKBOXFRAME)))))
    ) {
      this.isDocumentClick = false;
      e.preventDefault();
    } else if (
      !isNOU(this.inputWrapper) &&
  ...
  private setCssClass(newClass: string, oldClass: string): void {
    const elements: HTMLElement[] = this.popupObj
      ? [this.inputWrapper, this.popupObj.element]
      : [this.inputWrapper];
  ...
  private setEnable(): void {
    ...
    if (this.enabled) {
      ...
      if (
        this.inputWrapper &&
        this.inputWrapper.classList.contains(INPUTFOCUS)
      ) {
        removeClass([this.inputWrapper], [INPUTFOCUS]);
      }
  ...
  private getTreeChildren(
    mapper: FieldsModel | string
  ): FieldsSettingsModel | string {
    if (typeof mapper === 'string') {
      ...
      if (mapper.child) {
        (childFields as FieldsSettingsModel).child = this.getTreeChildren(
          mapper.child
        );
      }
  ...
  private getTreeDataType(
    ...
    if (this.fields.dataSource instanceof DataManager) {
      for (let i: number = 0; i < ds.length; i++) {
        if (
          typeof field.child === 'string' &&
          isNOU(getValue(field.child, ds[i as number]))
        ) {
          return 1;
        }
      }
    ...
    for (
      let i: number = 0, len: number = this.fields.dataSource.length;
      i < len;
      i++
    ) {
      if (
        typeof field.child === 'string' &&
        !isNOU(getValue(field.child, this.fields.dataSource[i as number]))
      ) {
        return 2;
      }
      if (
        !isNOU(
          getValue(field.parentValue, this.fields.dataSource[i as number])
        ) ||
  ...
  private getEventArgs(
    args: NodeCheckEventArgs | NodeSelectEventArgs
  ): DdtSelectEventArgs {
    const checkData: { [key: string]: Object }[] = (args as NodeCheckEventArgs)
      .data;
    const selectData: { [key: string]: Object } = (args as NodeSelectEventArgs)
      .nodeData;
    let state: string;
    if (this.showCheckBox) {
  ...
  private onNodeClicked(args: NodeClickEventArgs): void {
    ...
    if (
      (target.classList.contains('e-fullrow') ||
        target.classList.contains('e-list-text')) &&
      this.showCheckBox
    ) {
      this.isClicked = true;
      const getNodeDetails: { [key: string]: Object } = this.treeObj.getNode(
        args.node
      );
    ...
    if (this.showSelectAll && this.checkBoxElement) {
      ...
      if (
        ...
        (args.isInteracted ||
          ...
          (!isNOU(args.data[0]) && args.data[0].isChecked === 'false'))
      ) {
        ...
        args.action === 'check' &&
        checkedNodes.length ===
          (this.fields.dataSource as { [key: string]: Object }[]).length &&
        (args.isInteracted ||
          this.isCheckAllCalled ||
          (!isNOU(args.data[0]) && args.data[0].isChecked === 'true'))
      ) {
        this.isReverseUpdate = true;
  ...
  private updateClearButton(state: boolean): void {
    if (state) {
      if (!this.inputWrapper.contains(this.overAllClear)) {
        this.inputEle.parentElement.insertBefore(
          this.overAllClear,
          this.inputEle.nextSibling
        );
      } else {
        removeClass([this.overAllClear], HIDEICON);
        addClass([this.inputWrapper], SHOW_CLEAR);
    ...
    if ((this.allowMultiSelection || this.showCheckBox) && this.chipWrapper) {
      const chipClose: Element[] = selectAll(
        '.' + CHIP_CLOSE,
        this.chipWrapper
      );
  ...
  private updateMode(): void {
    ...
    if (this.mode !== 'Delimiter') {
      ...
      const isValue: boolean =
        this.value !== null ? (this.value.length !== 0 ? true : false) : false;
      if (isValid && isValue) {
        addClass([this.inputEle], CHIP_INPUT);
      } else {
        removeClass([this.inputEle], CHIP_INPUT);
  ...
  private setMultiSelectValue(newValues: string[]): void {
    if (!this.isFilteredData) {
      this.setProperties(
        {
          value:
            this.isFromFilterChange && newValues && newValues.length === 0
              ? this.value
              : newValues,
        },
        true
      ...
      for (let i: number = 0; i < newValues.length; i++) {
        if (
          isNOU(this.value) ||
          this.value.indexOf(newValues[i as number]) === -1
        ) {
  ...
  private setMultiSelect(): void {
    if (this.showCheckBox && !this.isDynamicChange) {
      ...
      const ddtValue: string[] = this.allowMultiSelection
        ? this.showCheckBox
    ...
    this.selectedText = [];
    const checkSelection: boolean = this.allowMultiSelection
      ? true
      : this.showCheckBox
      ? true
      : false;
  ...
  private getSelectedData(value: string): { [key: string]: Object } {
    ...
    if (this.isFilteredData) {
      for (let i: number = 0; i < this.selectedData.length; i++) {
        if (
          getValue(
            this.treeSettings.loadOnDemand ? this.fields.value : 'id',
            this.selectedData[i as number]
          ).toString() === value
        ) {
  ...
  private getNodeData(
    ...
    if (isNOU(id)) {
      ...
      for (
        let i: number = 0, objlen: number = dataSource.length;
        i < objlen;
        i++
      ) {
        const dataId: Object = getValue(
          this.fields.value,
          dataSource[i as number]
        );
        if (
          !isNOU(dataSource[i as number]) &&
          !isNOU(dataId) &&
          dataId.toString() === id
        ) {
  ...
  private getChildNodeData(
    ...
    for (let i: number = 0, objlen: number = obj.length; i < objlen; i++) {
      ...
      if (obj[i as number] && dataValue && dataValue.toString() === id) {
        ...
        typeof mapper.child === 'string' &&
        !isNOU(getValue(mapper.child, obj[i as number]))
      ) {
        const childNodeData: Object = getValue(mapper.child, obj[i as number]);
        newChildItems = this.getChildNodeData(
          <{ [key: string]: Object }[]>childNodeData,
          this.getChildMapperFields(mapper),
          id
        );
        ...
        this.fields.dataSource instanceof DataManager &&
        !isNOU(getValue('child', obj[i as number]))
      ) {
        const child: string = 'child';
        newChildItems = this.getChildNodeData(
          <{ [key: string]: Object }[]>getValue(child, obj[i as number]),
          this.getChildMapperFields(mapper),
          id
        );
        if (newChildItems !== undefined) {
  ...
  private removeSelectedData(value: string, muteOnChange: boolean): void {
    const selectedValues: string[] = isNOU(this.value)
      ? []
      : this.value.slice();
  ...
  private initializeValueTemplate(): Function {
    ...
    if (this.valueTemplateContainer) {
      while (this.valueTemplateContainer.firstChild) {
        this.valueTemplateContainer.removeChild(
          this.valueTemplateContainer.firstChild
        );
      }
    } else {
  ...
  private updateSelectedValues(): void {
    ...
    if (
      (!this.isChipDelete || this.treeSettings.autoCheck) &&
      this.inputWrapper.contains(this.chipWrapper)
    ) {
    ...
    if (!isNOU(this.value)) {
      ...
      for (let i: number = 0, len: number = this.value.length; i < len; i++) {
        ...
        hiddenInputValue +=
          ...
          '">' +
          this.selectedText[this.selectedText.length - 1] +
          '</option>';
        if (this.valueTemplate) {
  ...
  private setTagValues(): void {
    ...
    let tempArr: Element[] = compiledString(
  ...
  private setSelectAllWrapper(state: boolean): void {
    ...
    if (
      state &&
      !this.popupEle.contains(this.checkAllParent) &&
      this.showCheckBox
    ) {
      this.createSelectAllWrapper();
      this.popupEle.insertBefore(this.checkAllParent, this.popupDiv);
  ...
  private setHeaderTemplate(): void {
    ...
    this.popupEle.insertBefore(
      this.header,
      this.checkAllParent ? this.checkAllParent : this.popupDiv
    );
  }
  ...
  private updateTreeSettings(prop: string): void {
    if (prop === 'autoCheck') {
      ...
      this.treeObj.checkDisabledChildren =
        this.treeSettings.checkDisabledChildren;
    }
    this.treeObj.dataBind();
    this.setMultiSelect();
  ...
  private l10nUpdate(actionFailure?: boolean): void {
    ...
    if (
      ...
      this.actionFailureTemplate !== 'The Request Failed'
    ) {
      ...
      const templatestring: string = actionFailure
  ...
  private updateFilterPlaceHolder(): void {
    if (this.filterObj) {
      ...
      this.filterObj.element.setAttribute(
        'aria-label',
        this.filterBarPlaceholder
      );
    }
  }
  ...
  private updateModelMode(): void {
    const validMode: boolean = this.allowMultiSelection
      ? true
      : this.showCheckBox
      ? true
      : false;
    ...
    if (!this.wrapText) {
      const overFlow: Element = select(
        '.e-overflow:not(.e-input-value)',
        this.inputWrapper
      );
      if (overFlow) {
        overFlow.innerHTML = '';
  ...
  private updateOption(): void {
    if (
      !this.hiddenElement.hasAttribute('multiple') &&
      (this.allowMultiSelection || this.showCheckBox)
    ) {
  ...
  public onPropertyChanged(
    newProp: DropDownTreeModel,
    oldProp: DropDownTreeModel
  ): void {
  ...
  public destroy(): void {
    ...
    if (this.element.tagName !== this.getDirective()) {
      this.inputWrapper.parentElement.insertBefore(
        this.element,
        this.inputWrapper
      );
    }
    ...
    if (
      this.showCheckBox ||
      this.allowMultiSelection ||
      (this.value && this.valueTemplateContainer)
    ) {
    ...
    if (this.chipCollection) {
      const chipsIcons: HTMLElement[] = selectAll(
        '.e-chips-close',
        this.chipCollection
      );
  ...
  public hidePopup(): void {
    const eventArgs: DdtPopupEventArgs = {
      popup: this.popupObj,
      cancel: false,
    };
