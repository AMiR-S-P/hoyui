...
export class CheckBoxSelection {
  ...
  public listOption(args: { [key: string]: Object }): void {
    if (isNullOrUndefined(this.parent.listCurrentOptions.itemCreated)) {
      ...
      const itemCreated: Function = <Function>(
        this.parent.listCurrentOptions.itemCreated
      );
      this.parent.listCurrentOptions.itemCreated = (e: {
        [key: string]: HTMLElement;
      }) => {
  ...
  private setPlaceholder(props: MultiSelectModel): void {
    Input.setPlaceholder(
      props.filterBarPlaceholder,
      this.filterInput as HTMLInputElement
    );
  }
  private checboxCreate(
    e: { [key: string]: HTMLElement } | HTMLElement
    ...
    if (
      ...
      const checkboxEle: HTMLElement | Element | string = createCheckBox(
        this.parent.createElement,
        true
      );
      const icon: Element = select('div.' + ICON, item as HTMLElement);
      (item as HTMLElement).insertBefore(
        checkboxEle,
        (item as HTMLElement).childNodes[isNullOrUndefined(icon) ? 0 : 1]
      );
  ...
  private setSelectAll(): void {
    if (this.parent.showSelectAll) {
      if (isNullOrUndefined(this.checkAllParent)) {
        ...
        this.setLocale();
        this.checboxCreate(
          this.checkAllParent as { [key: string]: HTMLElement } | HTMLElement
        );
  ...
  public listSelection(args: IUpdateListArgs): void {
    ...
    if (!isNullOrUndefined(args.e)) {
      const frameElm: Element = args.li.querySelector(
      ...
      target = !isNullOrUndefined(args.e.target)
        ? (args.e.target as HTMLElement).classList.contains('e-frame') &&
          (!this.parent.showSelectAll ||
            (this.checkAllParent &&
              !this.checkAllParent.contains(args.e.target as Element)))
          ? args.e.target
          : args.li.querySelector('.e-checkbox-wrapper').childNodes[1]
        : args.li.querySelector('.e-checkbox-wrapper').childNodes[1];
    } else {
      const checkboxWrapper: Element = args.li.querySelector(
        '.e-checkbox-wrapper'
      );
      target = checkboxWrapper
        ? checkboxWrapper.childNodes[1]
        : args.li.lastElementChild.childNodes[1];
    }
    ...
    if (!isNullOrUndefined(this.checkWrapper)) {
      ...
      this.validateCheckNode(
        this.checkWrapper,
        checkElement.classList.contains(CHECK),
        args.li,
        args.e,
        selectAll
      );
  ...
  private validateCheckNode(
    ...
    e?: KeyboardEventArgs | MouseEvent,
    selectAll?: boolean
  ): void {
    this.changeState(
      checkWrap,
      isCheck ? 'uncheck' : 'check',
      e,
      true,
      selectAll
    );
  ...
  private clickHandler(e: MouseEvent): void {
    ...
    if (
      (e.currentTarget as HTMLElement).classList.contains(
        this.checkAllParent.className
      ) ||
      (e.currentTarget as HTMLElement).classList.value ===
        this.checkAllParent.className
    ) {
  ...
  private changeState(
    ...
    if (state === 'check' && !frameSpan.classList.contains(CHECK)) {
      ...
      state === 'uncheck' &&
      (frameSpan.classList.contains(CHECK) ||
        frameSpan.classList.contains(INDETERMINATE))
    ) {
  ...
  protected setSearchBox(args: IUpdateListArgs): InputObject | void {
    if (isNullOrUndefined(this.parent.filterParent)) {
      ...
      this.parent.element.parentNode.insertBefore(
        this.filterInput,
        this.parent.element
      );
      ...
      this.filterInputObj = Input.createInput(
        {
          ...
          buttons: backIcon
            ? [searchBackIcon, filterBarClearIcon]
            : [filterBarClearIcon],
      ...
      this.parent.searchBoxHeight =
        this.filterInputObj.container.parentElement.getBoundingClientRect().height;
  ...
  private clearText(e: MouseEvent): void {
    ...
    this.parent.popupWrapper.addEventListener(
      'mouseup',
      this.boundPreventListSelection,
      true
    );
    e.preventDefault();
  }
  ...
  private setDeviceSearchBox(): void {
    ...
    this.backIconElement =
      this.filterInputObj.container.querySelector('.e-back-icon');
    this.clearIconElement = this.filterInputObj.container.querySelector(
      '.' + clearIcon
    );
    (this.clearIconElement as HTMLElement).style.visibility = 'hidden';
  ...
  protected targetElement(): string {
    if (!isNullOrUndefined(this.clearIconElement)) {
      ...
      (this.clearIconElement as HTMLElement).style.visibility =
        this.parent.targetInputElement.value === '' ? 'hidden' : 'visible';
    }
  ...
  private onBlurHandler(e: MouseEvent): void {
    if (!this.parent.element.classList.contains('e-listbox')) {
      ...
      if (
        ...
        this.parent.popupObj.element.contains(target) &&
        !Browser.isIE &&
        this.filterInput
      ) {
        this.filterInput.focus();
      ...
      if (
        ...
        this.parent.updateValueState(
          e,
          this.parent.value,
          this.parent.tempValues
        );
        this.parent.dispatchEvent(
          this.parent.hiddenElement as HTMLElement,
          'change'
        );
      }
      if (
  ...
  protected onDocumentClick(e: MouseEvent): void {
    if (this.parent.getLocaleName() !== 'listbox') {
      ...
      if (
        !isNullOrUndefined(this.parent.popupObj) &&
        closest(target, '[id="' + this.parent.popupObj.element.id + '"]')
      ) {
      ...
      if (
        ...
        !isNullOrUndefined(this.parent.overAllWrapper) &&
        !this.parent.overAllWrapper.contains(e.target as Node)
      ) {
        if (
          this.parent.overAllWrapper.classList.contains(
  ...
  private setLocale(unSelect?: boolean): void {
    if (
      this.parent.selectAllText !== 'Select All' ||
      this.parent.unSelectAllText !== 'Unselect All'
    ) {
      const template: string = unSelect
        ? this.parent.unSelectAllText
        : this.parent.selectAllText;
      this.selectAllSpan.textContent = '';
      ...
      if (!isNullOrUndefined(this.selectAllSpan)) {
        this.selectAllSpan.textContent = unSelect
          ? l10n.getConstant('unSelectAllText')
          : l10n.getConstant('selectAllText');
      }
    }
  }
  ...
  private setReorder(args: IUpdateListArgs | MouseEvent): void {
    if (
      ...
      !isNullOrUndefined(this.parent.value)
    ) {
      const activeLiCount: number =
        this.parent.ulElement.querySelectorAll('li.e-active').length;
      let remLi: NodeListOf<Element>;
      const ulEle: HTMLElement = this.parent.createElement('ul', {
        className: 'e-list-parent e-ul e-reorder',
      });
      if (activeLiCount > 0) {
        const activeListItems: NodeListOf<Element> =
          this.parent.ulElement.querySelectorAll('li.e-active');
        activeListItems.forEach((item: Element) => {
          // Add type definition for 'item' parameter
          ulEle.appendChild(item);
        });
        remLi = this.parent.ulElement.querySelectorAll('li.e-active');
        addClass(remLi, 'e-reorder-hide');
        if (this.parent.enableVirtualization) {
          const virtualUlElement: Element = this.parent.list.querySelector(
