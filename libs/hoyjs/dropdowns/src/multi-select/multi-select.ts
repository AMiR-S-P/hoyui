...
import {
  ...
  PopupEventArgs,
  FilteringEventArgs,
} from '../drop-down-base/drop-down-base';
import {
  FocusEventArgs,
  BeforeOpenEventArgs,
  FilterType,
...
import { IInput, FloatLabelType, Input } from '@hoyui/inputs';
import { attributes, setValue, getValue } from '@hoyui/base';
import { NotifyPropertyChanges, extend } from '@hoyui/base';
import {
  EventHandler,
  Property,
  Event,
...
import { SortOrder } from '@hoyui/lists';
import {
  createFloatLabel,
  removeFloating,
  floatLabelFocus,
...
export class MultiSelect extends DropDownBase implements IInput {
   ...
   */
  @Complex<FieldSettingsModel>(
    { text: null, value: null, iconCss: null, groupBy: null, disabled: null },
    FieldSettings
  )
  ...
  private updateHTMLAttribute(): void {
    if (Object.keys(this.htmlAttributes).length) {
      for (const htmlAttr of Object.keys(this.htmlAttributes)) {
        switch (htmlAttr) {
          case 'class': {
            const updatedClassValue: string = this.htmlAttributes[`${htmlAttr}`]
              .replace(/\s+/g, ' ')
              .trim();
          ...
          default: {
            ...
            if (defaultAttr.indexOf(htmlAttr) > -1) {
              ...
              htmlAttr.indexOf('data') === 0 ||
              validateAttr.indexOf(htmlAttr) > -1
            ) {
              ...
              this.overAllWrapper.setAttribute(
                htmlAttr,
                this.htmlAttributes[`${htmlAttr}`]
              );
            } else if (
              htmlAttr !== 'size' &&
              !isNullOrUndefined(this.inputElement)
            ) {
              this.inputElement.setAttribute(
                htmlAttr,
                this.htmlAttributes[`${htmlAttr}`]
  ...
  private onPopupShown(
    e?: MouseEvent | KeyboardEventArgs | TouchEvent | Object
  ): void {
      ...
      if (!eventArgs.cancel) {
        ...
        if (
          ...
          this.updateListItems(
            this.list.querySelectorAll('li.e-list-item'),
            this.mainList.querySelectorAll('li.e-list-item')
          );
        }
        ...
        if (this.popupObj) {
          this.popupObj.show(
            eventArgs.animation,
            this.zIndex === 1000 ? this.element : null
          );
        }
  ...
  private updateVirtualReOrderList(isCheckBoxUpdate?: boolean): void {
    ...
    this.UpdateSkeleton();
    this.liCollections = <HTMLElement[] & NodeListOf<Element>>(
      this.list.querySelectorAll('.' + dropDownBaseClasses.li)
    );
    this.virtualItemCount = this.itemCount;
    if (this.mode !== 'CheckBox') {
      this.totalItemCount =
        this.value && this.value.length
          ? this.totalItemCount - this.value.length
          : this.totalItemCount;
    }
    ...
    if (!this.list.querySelector('.e-virtual-ddl')) {
      ...
      this.popupWrapper
        .querySelector('.e-dropdownbase')
        .appendChild(virualElement);
    } else {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (this.list.getElementsByClassName('e-virtual-ddl')[0] as any).style =
        this.GetVirtualTrackHeight();
    }
    if (this.list.querySelector('.e-virtual-ddl-content')) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (
        this.list.getElementsByClassName('e-virtual-ddl-content')[0] as any
      ).style = this.getTransformValues();
    }
    if (isCheckBoxUpdate) {
      this.loadTemplate();
    }
  ...
  private updateListItems(
    ...
    mainListItems: NodeListOf<Element>
  ): void {
  ...
  private setScrollPosition(): void {
    if (
      ((!this.hideSelectedItem && this.mode !== 'CheckBox') ||
        (this.mode === 'CheckBox' && !this.enableSelectionOrder)) &&
    ...
    if (this.enableVirtualization) {
      const focusedItem: HTMLElement = <HTMLElement>(
        this.list.querySelector('.' + dropDownBaseClasses.focus)
      );
  ...
  private focusAtFirstListItem(): void {
    if (
      this.ulElement &&
      this.ulElement.querySelector('li.' + dropDownBaseClasses.li)
    ) {
      ...
      if (this.mode === 'CheckBox') {
        ...
        if (this.enableVirtualization) {
          ...
          if (this.fields.disabled) {
            ...
            element = <HTMLElement>(
              this.ulElement.querySelector(
                'li.' + dropDownBaseClasses.li + ':not(.' + HIDE_LIST + ')'
              )
            );
          }
        }
  ...
  private focusAtLastListItem(data: string): void {
    ...
    if (activeElement && activeElement.item !== null) {
      ...
      if (
        ((this.allowCustomValue || this.allowFiltering) &&
          this.isPopupOpen() &&
          this.closePopupOnSelect &&
          !this.enableVirtualization) ||
        (this.closePopupOnSelect && !this.enableVirtualization)
      ) {
        this.scrollBottom(
          <HTMLElement>activeElement.item,
          <number>activeElement.index
        );
      }
    }
  }
  ...
  private updateListARIA(): void {
    ...
    const disableStatus: boolean =
      !isNullOrUndefined(this.inputElement) && this.inputElement.disabled
        ? true
        : false;
  ...
  private removelastSelection(e: KeyboardEventArgs): void {
    const selectedElem: Element = <HTMLElement>(
      this.chipCollectionWrapper.querySelector('span.' + CHIP_SELECTED)
    );
    ...
    const value: string =
      elements[elements.length - 1].getAttribute('data-value');
    if (!isNullOrUndefined(this.value)) {
      this.tempValues = this.allowObjectBinding
        ? this.value.slice()
        : <string[]>this.value.slice();
    }
    let customValue: string | number | boolean | object = this
      .allowObjectBinding
      ? this.getDataByValue(this.getFormattedValue(value))
      : this.getFormattedValue(value);
    if (
      this.allowCustomValue &&
      ((value !== 'false' && customValue === false) ||
        (!isNullOrUndefined(customValue) && customValue.toString() === 'NaN'))
    ) {
      customValue = value;
    }
  ...
  private getForQuery(
    valuecheck: string[] | number[] | boolean[] | object[],
    isCheckbox?: boolean
  ): Query {
    let predicate: Predicate;
    const field: string = isNullOrUndefined(this.fields.value)
      ? this.fields.text
      : this.fields.value;
      ...
      if (isCheckbox) {
        for (let i: number = 0; i < valuecheck.length; i++) {
          const value: string | number | boolean = this.allowObjectBinding
            ? getValue(
                this.fields.value ? this.fields.value : '',
                valuecheck[i as number] as string
              )
            : (valuecheck[i as number] as string);
          if (i === 0) {
    ...
    if (
      ...
      (this.dataSource as DataManager).adaptor instanceof JsonAdaptor
    ) {
      return new Query().where(predicate);
    } else {
  ...
  protected onActionComplete(
    ...
    this.skeletonCount =
      ...
      this.totalItemCount < this.itemCount * 2 &&
      (!(this.dataSource instanceof DataManager) ||
        (this.dataSource instanceof DataManager &&
    ...
    if (
      ...
      for (let i: number = 0; i < this.value.length; i++) {
        ...
        const checkEle: Element = this.findListElement(
          this.allowFiltering && !isNullOrUndefined(this.mainList)
            ? this.mainList
            : ulElement,
          'li',
          'data-value',
    ...
    if (
      this.dataSource instanceof DataManager &&
      this.allowCustomValue &&
      !this.isCustomRendered &&
      this.inputElement.value &&
      this.inputElement.value !== ''
    ) {
      let query: Query = new Query();
      query = this.allowFiltering
        ? query.where(
            this.fields.text,
            'startswith',
            this.inputElement.value,
      ...
      this.remoteCustomValue = this.enableVirtualization
        ? false
        : this.remoteCustomValue;
    }
    if (
      this.mode === 'CheckBox' &&
      this.enableGroupCheckBox &&
      !isNullOrUndefined(this.fields.groupBy) &&
      !isNullOrUndefined(this.fields.disabled)
    ) {
  ...
  private updateActionList(
    ...
    if (!this.mainList && !this.mainData) {
      this.mainList = ulElement.cloneNode
        ? <HTMLElement>ulElement.cloneNode(true)
        : ulElement;
      this.mainData = list;
    ...
    if (
      (this.remoteCustomValue || list.length <= 0) &&
      this.allowCustomValue &&
      this.inputFocus &&
      this.allowFiltering &&
      this.inputElement.value &&
    ...
    if (
      ...
      ((this.mode !== 'CheckBox' &&
        ...
        this.mode === 'CheckBox' ||
        ((this.keyCode === 8 || this.keyCode === 46) &&
          this.allowFiltering &&
          this.allowCustomValue &&
          this.dataSource instanceof DataManager &&
          this.inputElement.value === ''))
    ...
    if (this.setDynValue) {
      ...
      if (
        !this.enableVirtualization ||
        (this.enableVirtualization && !(this.dataSource instanceof DataManager))
      ) {
  ...
  private refreshSelection(): void {
    ...
    if (!isNullOrUndefined(this.value)) {
      for (
        let index: number = 0;
        !isNullOrUndefined(this.value[index as number]);
        index++
      ) {
        ...
        if (element) {
          ...
          if (
            this.hideSelectedItem &&
            element.classList.contains(dropDownBaseClasses.focus)
          ) {
            removeClass([element], dropDownBaseClasses.focus);
            const listEle: NodeListOf<Element> =
          ...
          if (
            this.mode === 'CheckBox' &&
            element.classList.contains('e-active')
          ) {
            const ariaValue: number =
              element.getElementsByClassName('e-check').length;
            if (ariaValue === 0) {
  ...
  private hideGroupItem(value: string | number | boolean): void {
    ...
    const className: string = this.hideSelectedItem
      ? HIDE_LIST
      : dropDownBaseClasses.selected;
    element1 = element = this.findListElement(
      this.ulElement,
      'li',
      'data-value',
      value
    ...
    do {
      ...
      if (
        ...
        !element1 ||
        (element1.nextElementSibling &&
          element1.nextElementSibling.classList.contains(
            dropDownBaseClasses.group
          ))
      ) {
        j = 10;
  ...
  protected getValidLi(): HTMLElement {
    const liElement: HTMLElement = this.ulElement.querySelector(
      'li.' + dropDownBaseClasses.li + ':not(.' + HIDE_LIST + ')'
    );
  ...
  private checkSelectAll(): void {
    const groupItemLength: number = !isNullOrUndefined(this.fields.disabled)
      ? this.list.querySelectorAll(
          'li.e-list-group-item.e-active:not(.e-disabled)'
        ).length
      : this.list.querySelectorAll('li.e-list-group-item.e-active').length;
    const listItem: NodeListOf<Element> =
      this.list.querySelectorAll('li.e-list-item');
    const searchCount: number = this.enableVirtualization
      ? this.list.querySelectorAll(
          'li.' + dropDownBaseClasses.li + ':not(.e-virtual-list)'
        ).length
      : !isNullOrUndefined(this.fields.disabled)
      ? this.list.querySelectorAll(
          'li.' + dropDownBaseClasses.li + ':not(.e-disabled)'
        ).length
      : this.list.querySelectorAll('li.' + dropDownBaseClasses.li).length;
    let searchActiveCount: number = this.list.querySelectorAll(
      'li.' + dropDownBaseClasses.selected
    ).length;
    ...
    if (
      ...
      (this.enableVirtualization &&
        ...
        this.value &&
        this.value.length === this.totalItemCount)
    ) {
      ...
      this.showSelectAll &&
      (!this.enableVirtualization ||
        (this.enableVirtualization && !this.virtualSelectAll))
    ) {
      this.notify('checkSelectAll', {
        module: 'CheckBoxSelection',
        enable: this.mode === 'CheckBox',
  ...
  private keyUp(e: KeyboardEventArgs): void {
    ...
    this.isValidKey =
      (this.isPopupOpen() && e.keyCode === 8) || this.isValidKey;
  ...
  protected getQuery(query: Query): Query {
    ...
    if (this.isFiltered) {
      if (
        this.enableVirtualization &&
        !isNullOrUndefined(this.customFilterQuery)
      ) {
    ...
    if (this.filterAction) {
      if (
        (this.targetElement() !== null && !this.enableVirtualization) ||
        (this.enableVirtualization &&
          this.targetElement() !== null &&
          this.targetElement().trim() !== '')
      ) {
        const dataType: string = <string>(
          this.typeOfData(this.dataSource as { [key: string]: Object }[]).typeof
        );
        if (
          (!(this.dataSource instanceof DataManager) &&
            dataType === 'string') ||
          ...
          (this.enableVirtualization && this.targetElement() !== '') ||
          !this.enableVirtualization
        ) {
      ...
      if (this.virtualSelectAll) {
        return query
          ? query.take(this.maximumSelectionLength).requiresCount()
          : this.query
  ...
  private virtualFilterQuery(filterQuery: Query): Query {
    ...
    for (
      let queryElements: number = 0;
      queryElements < filterQuery.queries.length;
      queryElements++
    ) {
      if (
        this.getModuleName() === 'multiselect' &&
        ((filterQuery.queries[queryElements as number].e &&
          filterQuery.queries[queryElements as number].e.condition === 'or') ||
          (filterQuery.queries[queryElements as number].e &&
            filterQuery.queries[queryElements as number].e.operator ===
              'equal'))
      ) {
        isReOrder = false;
      }
    ...
    if (filterQuery && filterQuery.queries.length > 0) {
      for (
        ...
        if (filterQuery.queries[queryElements as number].fn === 'onTake') {
          queryTakeValue =
            takeValue <= filterQuery.queries[queryElements as number].e.nos
    ...
    if (queryTakeValue <= 0 && this.query && this.query.queries.length > 0) {
      for (
        let queryElements: number = 0;
        queryElements < this.query.queries.length;
        queryElements++
      ) {
        if (this.query.queries[queryElements as number].fn === 'onTake') {
          queryTakeValue =
            takeValue <= this.query.queries[queryElements as number].e.nos
              ? this.query.queries[queryElements as number].e.nos
              : takeValue;
        }
      }
    }
  ...
  protected getTakeValue(): number {
    return this.allowFiltering && Browser.isDevice
      ? Math.round(window.outerHeight / this.listItemHeight)
      : this.itemCount;
  }
  ...
  private dataUpdater(
    ...
    const isNoData: boolean = this.list.classList.contains(
      dropDownBaseClasses.noData
    );
    if (this.targetElement().trim() === '') {
      const list: HTMLElement = this.enableVirtualization
        ? <HTMLElement>this.list.cloneNode(true)
        : this.mainList.cloneNode
        ? <HTMLElement>this.mainList.cloneNode(true)
        : this.mainList;
      if (this.backCommand) {
        ...
        if (this.enableVirtualization) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          this.totalItemCount =
            this.dataSource && (this.dataSource as any).length
              ? (this.dataSource as any).length
              : 0;
          ...
          this.UpdateSkeleton();
          if (
            (isNoData || this.allowCustomValue) &&
            ...
            if (!this.list.querySelector('.e-virtual-ddl')) {
              ...
              document
                .getElementsByClassName('e-multi-select-list-wrapper')[0]
                .querySelector('.e-dropdownbase')
                .appendChild(virualElement);
            }
          }
  ...
  private checkForCustomValue(
    ...
    const dataChecks: boolean = !this.getValueByText(
      this.inputElement.value,
      this.ignoreCase
    );
    ...
    if (this.allowCustomValue && dataChecks) {
      ...
      const customData: Object | string =
        !isNullOrUndefined(this.mainData) && this.mainData.length > 0
          ? (this.mainData as { [key: string]: Object }[])[0]
          : this.mainData;
      if (
        customData &&
        typeof customData !== 'string' &&
        typeof customData !== 'number' &&
        typeof customData !== 'boolean'
      ) {
        let dataItem: { [key: string]: string | Object } = {};
        setValue(field.text, value, dataItem);
        if (
          typeof getValue(
            this.fields.value ? this.fields.value : 'value',
            customData as { [key: string]: Object }
          ) === 'number' &&
        ...
        if (this.enableVirtualization) {
          ...
          const tempData: { [key: string]: Object }[] =
            this.dataSource instanceof DataManager
              ? JSON.parse(JSON.stringify(this.listData))
              : JSON.parse(JSON.stringify(this.dataSource));
          let totalData: { [key: string]: Object }[] = [];
          if (
            this.virtualCustomSelectData &&
            this.virtualCustomSelectData.length > 0
          ) {
          ...
          this.viewPortInfo.endIndex = this.virtualItemEndIndex =
            this.itemCount;
          ...
          const tempData: [{ [key: string]: Object }] = JSON.parse(
        ...
        (tempData[0] as string | number) =
          typeof customData === 'number' && !isNaN(parseFloat(tempData[0]))
            ? parseFloat(tempData[0])
            : tempData[0];
        (tempData[0] as string | boolean) =
          typeof customData === 'boolean'
            ? tempData[0] === 'true'
              ? true
              : tempData[0] === 'false'
      ...
      if (
        ...
        >this.list.querySelectorAll(
          'li.' + dropDownBaseClasses.li + ':not(.e-hide-listitem)'
        );
        const activeElement: { [key: string]: Element | number } = Search(
          this.targetElement(),
          liCollections,
          'StartsWith',
          this.ignoreCase
  ...
  private wrapperClick(e: MouseEvent): void {
    ...
    if (!this.readonly) {
      if (
        e.target &&
        (<HTMLElement>e.target).classList.toString().indexOf(CHIP_CLOSE) !== -1
      ) {
        if (this.isPopupOpen()) {
          this.refreshPopup();
        }
      ...
      if (
        ...
        (this.openOnClick ||
          (this.showDropDownIcon &&
            e.target &&
            (<HTMLElement>e.target).className === dropdownIcon))
      ) {
        this.showPopup(e);
      } else {
        this.hidePopup(e);
  ...
  private onBlurHandler(eve?: MouseEvent, isDocClickFromCheck?: boolean): void {
    ...
    if (
      ...
      this.popupObj.element.contains(target)
    ) {
      if (this.mode !== 'CheckBox') {
        this.inputElement.focus();
      } else if (
        this.floatLabelType === 'Auto' &&
        (this.overAllWrapper.classList.contains('e-outline') ||
          this.overAllWrapper.classList.contains('e-filled'))
      ) {
    ...
    if (this.addTagOnBlur) {
      const dataChecks: string | boolean | number = this.getValueByText(
        this.inputElement.value,
        this.ignoreCase,
        this.ignoreAccent
      );
      const listLiElement: HTMLElement = this.findListElement(
        this.list,
        'li',
        'data-value',
        dataChecks
      ...
      const className: string = this.hideSelectedItem
        ? HIDE_LIST
        : dropDownBaseClasses.selected;
      const allowChipAddition: boolean =
        listLiElement && !listLiElement.classList.contains(className)
          ? true
          : false;
    ...
    if (
      (this.allowFiltering ||
        (this.enableSelectionOrder === true && this.mode === 'CheckBox')) &&
    ...
    this.calculateWidth();
    if (
      !isNullOrUndefined(this.overAllWrapper) &&
      !isNullOrUndefined(
        this.overAllWrapper.getElementsByClassName('e-ddl-icon')[0] &&
          this.overAllWrapper.getElementsByClassName(
      ...
      this.overAllWrapper
        .getElementsByClassName('e-float-text-content')[0]
        .classList.add('e-icon');
    }
  }
  private calculateWidth(): void {
    ...
    if (this.overAllWrapper) {
      if (
        !this.showDropDownIcon ||
        this.overAllWrapper.querySelector('.' + 'e-label-top')
      ) {
        elementWidth =
          this.overAllWrapper.clientWidth -
          2 * parseInt(getComputedStyle(this.inputElement).paddingRight, 10);
      } else {
        const downIconWidth: number =
          this.dropIcon.offsetWidth +
    ...
    if (this.showDropDownIcon) {
      ...
      if (!isNullOrUndefined(this.dropIcon)) {
        this.setPlaceholderSize(
          this.showDropDownIcon ? this.dropIcon.offsetWidth : 0
        );
      }
    }
  }
  private setPlaceholderSize(downIconWidth: number): void {
    if (isNullOrUndefined(this.value) || this.value.length === 0) {
      if (this.dropIcon.offsetWidth !== 0) {
        this.searchWrapper.style.width =
          'calc(100% - ' + (downIconWidth + 10) + 'px';
      } else {
  ...
  private refreshInputHight(): void {
    if (!isNullOrUndefined(this.searchWrapper)) {
      if (
        (!this.value || !this.value.length) &&
        (isNullOrUndefined(this.text) || this.text === '')
      ) {
  ...
  private validateValues(
    ...
    return (
      JSON.stringify((newValue as string[]).slice().sort()) !==
      JSON.stringify((oldValue as string[]).slice().sort())
    );
  }
  private updateValueState(
    event: KeyboardEventArgs | MouseEvent,
    newVal: string[] | number[] | boolean[] | object[],
    oldVal: string[] | number[] | boolean[] | object[]
  ): void {
    const newValue: string[] | number[] | boolean[] | object[] = newVal
      ? newVal
      : <string[]>[];
    const oldValue: string[] | number[] | boolean[] | object[] = oldVal
      ? oldVal
      : <string[]>[];
  ...
  private updateAriaActiveDescendant(): void {
    if (
      !isNullOrUndefined(this.ulElement) &&
      !isNullOrUndefined(
        this.ulElement.getElementsByClassName('e-item-focus')[0]
      )
    ) {
      attributes(this.inputElement, {
        'aria-activedescendant':
          this.ulElement.getElementsByClassName('e-item-focus')[0].id,
      });
    }
  }

  private pageUpSelection(steps: number, isVirtualKeyAction?: boolean): void {
    const collection: NodeListOf<Element> = <NodeListOf<HTMLElement>>(
      this.list.querySelectorAll(
        'li.' +
          dropDownBaseClasses.li +
          ':not(.' +
  ...
  private pageDownSelection(steps: number, isVirtualKeyAction?: boolean): void {
    ...
    if (this.enableVirtualization && this.skeletonCount > 0) {
      previousItem =
        steps < list.length
          ? this.liCollections[steps as number]
          : this.liCollections[list.length - 1];
    }
    if (this.enableVirtualization && isVirtualKeyAction) {
      previousItem =
        steps <= list.length
  ...
  public getItems(): Element[] {
    ...
    return this.ulElement &&
      ...
      ? <HTMLElement[] & NodeListOf<Element>>(
          this.ulElement.querySelectorAll(
            '.' + dropDownBaseClasses.li + ':not(.' + HIDE_LIST + ')'
          )
        )
      : [];
  ...
  private focusInHandler(
    e?: FocusEvent | MouseEvent | KeyboardEvent | TouchEvent
  ): boolean {
    if (this.enabled) {
      this.showOverAllClear();
      this.inputFocus = true;
    ...
    if (this.placeholder) {
      ...
      const sizeMultiplier: number =
        0xac00 <= codePoint && codePoint <= 0xd7af
          ...
          : 1;
      size =
        size > this.inputElement.placeholder.length
          ? size
          : this.inputElement.placeholder.length * sizeMultiplier;
    }
  ...
  private isPopupOpen(): boolean {
    return (
      this.popupWrapper !== null && this.popupWrapper.parentElement !== null
    );
  }
  ...
  private updateAriaAttribute(): void {
    ...
    if (!isNullOrUndefined(focusedItem)) {
      ...
      if (this.allowFiltering) {
        const filterInput: Element = this.popupWrapper.querySelector(
          '.' + FILTERINPUT
        );
        if (filterInput) {
  ...
  private homeNavigation(isHome: boolean, isVirtualKeyAction?: boolean): void {
    ...
    this.UpdateSkeleton();
    const scrollEle: NodeListOf<HTMLElement> = this.ulElement.querySelectorAll(
      'li.' +
        dropDownBaseClasses.li +
        ':not(.' +
        HIDE_LIST +
    ...
    if (!isNullOrUndefined(this.value) && this.value.length) {
      for (let index: number = 0; index < this.value.length; index++) {
        const value: string | number | boolean = this.allowObjectBinding
          ...
          : this.value[index as number];
        const selectedItem: HTMLElement | Element =
          this.getElementByValue(value);
        if (
          selectedItem &&
          !selectedItem.classList.contains(dropDownBaseClasses.selected)
        ) {
  ...
  protected handleVirtualKeyboardActions(
    e: KeyboardEventArgs,
    pageCount: number
  ): void {
    const focusedItem: Element = this.list.querySelector(
      '.' + dropDownBaseClasses.focus
    );
        ...
        if (focusedItem) {
          activeIndex =
            this.getIndexByValue(
              this.previousFocusItem.getAttribute('data-value')
            ) - 1;
  ...
  private arrowDown(e: KeyboardEventArgs, isVirtualKeyAction?: boolean): void {
    ...
    this.keyAction = true;
    if (
      document.activeElement.classList.contains(FILTERINPUT) ||
      (this.mode === 'CheckBox' &&
        !this.allowFiltering &&
        document.activeElement !== this.list)
    ) {
  ...
  private arrowUp(e: KeyboardEventArgs, isVirtualKeyAction?: boolean): void {
    e.preventDefault();
    this.keyAction = true;
    let list: NodeListOf<Element> = <NodeListOf<HTMLElement>>(
      this.list.querySelectorAll(
        'li.' +
          dropDownBaseClasses.li +
          ':not(.' +
    ...
    if (
      ...
      list = this.list.querySelectorAll(
        'li.' +
          dropDownBaseClasses.li +
          ',li.' +
          dropDownBaseClasses.group +
    ...
    const focuseElem: Element = <HTMLElement>(
      this.list.querySelector('li.' + dropDownBaseClasses.focus)
    );
    this.focusFirstListItem = !isNullOrUndefined(this.liCollections[0])
      ? this.liCollections[0].classList.contains('e-item-focus')
      : false;
  ...
  private spaceKeySelection(e: KeyboardEventArgs): void {
    if (this.mode === 'CheckBox') {
      const li: HTMLElement = <HTMLElement>(
        this.list.querySelector('li.' + dropDownBaseClasses.focus)
      );
      const selectAllParent: Element =
        document.getElementsByClassName('e-selectall-parent')[0];
      if (
        !isNullOrUndefined(li) ||
        (selectAllParent && selectAllParent.classList.contains('e-item-focus'))
      ) {
        e.preventDefault();
        this.keyAction = true;
      }
  ...
  private checkBackCommand(e: KeyboardEventArgs): void {
    if (
      e.keyCode === 8 && this.allowFiltering
        ? this.targetElement() !== this.previousFilterText
        : this.targetElement() === ''
    ) {
  ...
  private escapeAction(): void {
    let temp: string[] | number[] | object[] = this.tempValues
      ? <string[]>this.tempValues.slice()
      : <string[]>[];
    if (this.allowObjectBinding) {
      temp = this.tempValues
        ? (this.tempValues.slice() as number[] | string[] | object[])
        : [];
    }
  ...
  private scrollBottom(
    ...
    if (
      (!isNullOrUndefined(selectedLI) &&
        selectedLI.classList.contains('e-virtual-list')) ||
      (this.enableVirtualization && isNullOrUndefined(selectedLI))
    ) {
      selectedLI = this.liCollections[this.skeletonCount];
    }
    const selectedListMargin: number =
      selectedLI &&
      !isNaN(parseInt(window.getComputedStyle(selectedLI).marginBottom, 10))
        ? parseInt(window.getComputedStyle(selectedLI).marginBottom, 10)
        : 0;
    this.isUpwardScrolling = false;
    const virtualListCount: number =
      this.list.querySelectorAll('.e-virtual-list').length;
    const lastElementValue: string = this.list.querySelector('li:last-of-type')
      ? this.list.querySelector('li:last-of-type').getAttribute('data-value')
      : null;
    const selectedLiOffsetTop: number =
      this.virtualListInfo && this.virtualListInfo.startIndex
        ? selectedLI.offsetTop +
          this.virtualListInfo.startIndex *
            (selectedLI.offsetHeight + selectedListMargin)
        : selectedLI.offsetTop;
    const currentOffset: number = this.list.offsetHeight;
    const nextBottom: number =
      selectedLiOffsetTop -
      virtualListCount * (selectedLI.offsetHeight + selectedListMargin) +
      (selectedLI.offsetHeight + selectedListMargin) -
      this.list.scrollTop;
    ...
    let boxRange: number =
      ...
      this.list.scrollTop;
    boxRange =
      this.fields.groupBy && !isNullOrUndefined(this.fixedHeaderElement)
        ? boxRange - this.fixedHeaderElement.offsetHeight
        : boxRange;
    if (activeIndex === 0 && !this.enableVirtualization) {
      this.list.scrollTop = 0;
    } else if (
      nextBottom > currentOffset ||
      !(boxRange > 0 && this.list.offsetHeight > boxRange)
    ) {
      const currentElementValue: string = selectedLI
        ? selectedLI.getAttribute('data-value')
        : null;
      const liCount: number = keyCode === 34 ? this.getPageCount() - 1 : 1;
      if (
        !this.enableVirtualization ||
        this.isKeyBoardAction ||
        isInitialSelection
      ) {
        if (
          this.isKeyBoardAction &&
          this.enableVirtualization &&
          lastElementValue &&
          currentElementValue === lastElementValue &&
          keyCode !== 35 &&
          !this.isVirtualScrolling
        ) {
          this.isPreventKeyAction = true;
          this.list.scrollTop +=
            (selectedLI.offsetHeight + selectedListMargin) * liCount;
          this.isPreventKeyAction = this.isScrollerAtEnd()
            ? false
            : this.isPreventKeyAction;
          this.isKeyBoardAction = false;
  ...
  private scrollTop(
    ...
    let nextOffset: number =
      ...
      virtualListCount * (selectedLI.offsetHeight + selectedListMargin) -
      this.list.scrollTop;
    const firstElementValue: string = this.list.querySelector(
      'li.e-list-item:not(.e-virtual-list)'
    )
      ? this.list
          .querySelector('li.e-list-item:not(.e-virtual-list)')
          .getAttribute('data-value')
      : null;
    nextOffset =
      this.fields.groupBy && !isUndefined(this.fixedHeaderElement)
        ? nextOffset - this.fixedHeaderElement.offsetHeight
        : nextOffset;
    const boxRange: number =
      selectedLiOffsetTop -
      virtualListCount * (selectedLI.offsetHeight + selectedListMargin) +
      (selectedLI.offsetHeight + selectedListMargin) -
      this.list.scrollTop;
    const isPageUpKeyAction: boolean =
      this.enableVirtualization &&
      this.getModuleName() === 'autocomplete' &&
      nextOffset <= 0;
    if (activeIndex === 0 && !this.enableVirtualization) {
      this.list.scrollTop = 0;
      ...
      if (
        ...
        this.isKeyBoardAction &&
        firstElementValue &&
        currentElementValue === firstElementValue &&
        keyCode !== 36 &&
        !this.isVirtualScrolling
      ) {
        ...
        this.list.scrollTop -=
          (selectedLI.offsetHeight + selectedListMargin) * liCount;
        this.isPreventKeyAction =
          this.list.scrollTop !== 0 ? this.isPreventKeyAction : false;
        this.isPreventScrollAction = false;
      ...
      this.list.scrollTop =
        selectedLI.offsetTop -
        (this.fields.groupBy && !isNullOrUndefined(this.fixedHeaderElement)
          ? this.fixedHeaderElement.offsetHeight
          : 0);
    }
  }
  ...
  private selectListByKey(e: KeyboardEventArgs): void {
    ...
    if (selectAllParent && selectAllParent.classList.contains('e-item-focus')) {
      const selectAllCheckBox: HTMLElement = selectAllParent
  ...
  private refreshListItems(data: string): void {
    if (
      (this.allowFiltering ||
        (this.mode === 'CheckBox' && this.enableSelectionOrder === true) ||
        this.allowCustomValue) &&
      this.mainList &&
      this.listData
    ) {
      const list: HTMLElement = this.mainList.cloneNode
        ? <HTMLElement>this.mainList.cloneNode(true)
        : this.mainList;
      if (this.enableVirtualization) {
        if (
          this.allowCustomValue &&
          this.virtualCustomData &&
          data == null &&
          this.virtualCustomData &&
          this.viewPortInfo &&
          this.viewPortInfo.startIndex === 0 &&
  ...
  private removeSelectedChip(e: KeyboardEventArgs): void {
    ...
    if (selectedElem !== null) {
      ...
      const currentChip: string | number | boolean | object = this
        .allowObjectBinding
        ? this.getDataByValue(
            this.getFormattedValue(selectedElem.getAttribute('data-value'))
          )
  ...
  private clickHandler(e: MouseEvent): void {
    ...
    const selectAllParent: Element =
      document.getElementsByClassName('e-selectall-parent')[0];
    if (
      (filterInputClassName === 'e-input-filter e-input' ||
  ...
  private moveByList(position: number, isVirtualKeyAction?: boolean): void {
    if (this.list) {
      let elements: NodeListOf<Element> = <NodeListOf<HTMLElement>>(
        this.list.querySelectorAll(
          'li.' +
            dropDownBaseClasses.li +
            ':not(.' +
      ...
      if (
        ...
        elements = this.list.querySelectorAll(
          'li.' +
            dropDownBaseClasses.li +
            ',li.' +
            dropDownBaseClasses.group +
      ...
      let selectedElem: Element = <HTMLElement>(
        this.list.querySelector('li.' + dropDownBaseClasses.focus)
      );
      if (
        this.enableVirtualization &&
        isVirtualKeyAction &&
        !isNullOrUndefined(this.currentFocuedListElement)
      ) {
        selectedElem = this.getElementByValue(
          this.getFormattedValue(
            this.currentFocuedListElement.getAttribute('data-value')
          )
        );
      }
      let temp: number = -1;
      ...
      if (
        ...
        this.showSelectAll &&
        position === 1 &&
        !isNullOrUndefined(selectAllParent) &&
        !selectAllParent.classList.contains('e-item-focus') &&
        this.list.getElementsByClassName('e-item-focus').length === 0 &&
        ...
        if (
          this.mode === 'CheckBox' &&
          this.showSelectAll &&
          !isNullOrUndefined(selectAllParent && position === -1)
        ) {
          if (
            !this.focusFirstListItem &&
            selectAllParent.classList.contains('e-item-focus')
        ...
        if (position > 0) {
          if (temp < elements.length - 1) {
            ...
            if (this.enableVirtualization && isVirtualKeyAction) {
              ...
              if (
                this.enableVirtualization &&
                (<HTMLElement>elements[temp + 1]).classList.contains(
                  'e-virtual-list'
                )
              ) {
            ...
            if (temp > -1) {
              ...
              this.currentFocuedListElement = elements[
                temp as number
              ] as HTMLElement;
            }
          }
          ...
          if (temp > 0) {
            if (this.enableVirtualization) {
              const isVirtualElement: boolean = (<HTMLElement>(
                elements[temp - 1]
              )).classList.contains('e-virtual-list');
              ...
              if (isVirtualKeyAction || !isVirtualElement) {
                this.addListFocus(
                  <HTMLElement>elements[elementIndex as number]
                );
    ...
    const focusedLi: HTMLElement = this.list
      ? this.list.querySelector('.e-item-focus')
      : null;
    if (this.isDisabledElement(focusedLi)) {
      if (
        this.list.querySelectorAll(
          '.e-list-item:not(.e-hide-listitem):not(.e-disabled)'
        ).length === 0 ||
        (this.keyCode === 38 &&
          this.mode === 'CheckBox' &&
          this.enableGroupCheckBox &&
          !isNullOrUndefined(this.fields.groupBy) &&
          focusedLi === this.list.querySelector('li.e-list-group-item'))
      ) {
        this.removeFocus();
        return;
      }
      const index: number = this.getIndexByValue(
        focusedLi.getAttribute('data-value')
      );
      ...
      if (
        index ===
        this.list.querySelectorAll('.e-list-item:not(.e-hide-listitem)')
          .length -
          1
      ) {
        position = -1;
  ...
  private chipClick(e: MouseEvent): void {
    if (this.enabled) {
      const elem: HTMLElement = <HTMLElement>(
        closest(<HTMLElement>e.target, '.' + CHIP)
      );
      this.removeChipSelection();
  ...
  private onChipRemove(e: MouseEvent): void {
    ...
    if (this.enabled && !this.readonly) {
      ...
      const customVal: string | number | boolean =
        element.getAttribute('data-value');
      let value: string | number | boolean | object = this.allowObjectBinding
        ? this.getDataByValue(this.getFormattedValue(customVal))
        : this.getFormattedValue(customVal);
      if (
        this.allowCustomValue &&
        ((customVal !== 'false' && value === false) ||
      ...
      if (
        isNullOrUndefined(
          this.findListElement(this.list, 'li', 'data-value', value as string)
        ) &&
        this.mainList &&
        this.listData
      ) {
  ...
  private removeAllItems(
    ...
    const index: number = this.allowObjectBinding
      ? this.indexOfObjectInArray(value, this.value)
      : (this.value as string[]).indexOf(value as string);
    const removeVal: number[] | string[] | boolean[] | object[] =
      this.value.slice(0);
    removeVal.splice(index, 1);
    this.setProperties(
      { value: <[number | string]>[].concat([], removeVal) },
      true
    );
    ...
    const currentValue: string | number | boolean = this.allowObjectBinding
      ? getValue(this.fields.value ? this.fields.value : '', value)
      : value;
  ...
  private invokeCheckboxSelection(
    ...
    if (
      this.value &&
      this.value.length !== this.mainData.length &&
      this.mode === 'CheckBox' &&
      this.showSelectAll &&
      !(this.isSelectAll || isClearAll)
    ) {
      this.notify('checkSelectAll', {
        module: 'CheckBoxSelection',
        enable: this.mode === 'CheckBox',
  ...
  private removeValue(
    ...
    let index: number = this.allowObjectBinding
      ? this.indexOfObjectInArray(value, this.value)
      : (this.value as string[]).indexOf(
          this.getFormattedValue(<string>value) as string
        );
    if (index === -1 && this.allowCustomValue && !isNullOrUndefined(value)) {
      index = this.allowObjectBinding
        ? this.indexOfObjectInArray(value, this.value)
    ...
    isClearAll =
      isClearAll ||
      (targetEle && targetEle.classList.contains('e-close-hooker'))
        ? true
        : null;
    const className: string = this.hideSelectedItem
      ? HIDE_LIST
    ...
    if (index !== -1) {
      const currentValue: string | number | boolean = this.allowObjectBinding
        ? getValue(this.fields.value ? this.fields.value : '', value)
        : value;
      const element: HTMLElement = this.virtualSelectAll
        ? null
        : this.findListElement(this.list, 'li', 'data-value', currentValue);
      const val: FieldSettingsModel = this.allowObjectBinding
        ? (value as object)
        : (this.getDataByValue(value) as FieldSettingsModel);
      ...
      this.trigger('removing', eventArgs, (eventArgs: RemoveEventArgs) => {
        if (eventArgs.cancel) {
          ...
          this.currentRemoveValue = this.allowObjectBinding
            ? getValue(this.fields.value ? this.fields.value : '', value)
            : value;
          this.virtualSelectAll = false;
          let removeVal: number[] | string[] | boolean[] | object[] =
            this.value.slice(0);
          ...
          if (this.enableVirtualization) {
            const currentText: string =
              index === 0 &&
              this.text.split(this.delimiterChar) &&
              this.text.split(this.delimiterChar).length === 1
                ? this.text.replace(
                    this.text.split(this.delimiterChar)[index as number],
                    ''
                  )
                : index === 0
                ? this.text.replace(
          ...
          if (element !== null) {
            const currentValue: string | number | boolean = this
              .allowObjectBinding
              ? getValue(this.fields.value ? this.fields.value : '', value)
              : value;
            const hideElement: HTMLElement = this.findListElement(
              this.mainList,
              'li',
              'data-value',
              currentValue
          ...
          if (limit < this.maximumSelectionLength) {
            const collection: NodeListOf<Element> = <NodeListOf<HTMLElement>>(
              this.list.querySelectorAll(
                'li.' + dropDownBaseClasses.li + ':not(.e-active)'
              )
            );
            removeClass(collection, 'e-disable');
          }
          this.trigger('removed', eventArgs);
          const targetEle: HTMLElement =
            eve && (eve.currentTarget as HTMLElement);
          const isSelectAll: boolean =
            targetEle && targetEle.classList.contains('e-selectall-parent')
              ? true
              : null;
          if (
            !this.changeOnBlur &&
          ...
          if (
            this.isPopupOpen() &&
            element &&
            element.parentElement.classList.contains('e-reorder')
          ) {
            if (
              this.hideSelectedItem &&
              this.value &&
  ...
  private updateMainList(
    ...
    if (this.allowFiltering || this.mode === 'CheckBox') {
      const element2: HTMLElement = mainElement
        ? mainElement
      ...
      if (element2) {
        if (state) {
          ...
          if (this.mode === 'CheckBox') {
            removeClass(
              [element2.firstElementChild.lastElementChild],
              'e-check'
            );
          }
        } else {
  ...
  private removeChip(
    ...
    if (this.chipCollectionWrapper) {
      if (!(this.enableVirtualization && isClearAll)) {
        const element: HTMLElement = this.findListElement(
          this.chipCollectionWrapper,
          'span',
          'data-value',
          value
        );
  ...
  private setWidth(width: number | string): void {
    if (!isNullOrUndefined(width)) {
      if (typeof width === 'number') {
        ...
        this.overAllWrapper.style.width = width.match(/px|%|em/)
          ? <string>width
          : <string>formatUnit(width);
      }
    }
  }
  ...
  private indexOfObjectInArray(objectToFind: any, array: any[]): number {
    for (let i: number = 0; i < array.length; i++) {
      ...
      if (
        Object.keys(objectToFind).every(
          (key: string) =>
  ...
  private addValue(
    ...
    const currentValue: string | number | boolean | object = this
      .allowObjectBinding
      ? this.getDataByValue(value)
      : value;
    if (
      (this.allowObjectBinding &&
        !this.isObjectInArray(this.getDataByValue(value), this.value)) ||
      (!this.allowObjectBinding &&
        (this.value as string[]).indexOf(currentValue as string) < 0)
    ) {
      this.setProperties(
        { value: <[number | string]>[].concat([], this.value, [currentValue]) },
        true
      );
      ...
      if (this.enableVirtualization && this.mode === 'CheckBox') {
        ...
        const textValues: string =
          this.text != null && this.text !== '' ? this.text + ',' + temp : temp;
  ...
  private checkMaxSelection(): void {
    const limit: number =
      this.value && this.value.length ? this.value.length : 0;
    if (limit === this.maximumSelectionLength) {
      const activeItems: NodeListOf<Element> = <NodeListOf<HTMLElement>>(
        this.list.querySelectorAll('li.' + dropDownBaseClasses.li + '.e-active')
      );
      removeClass(activeItems, 'e-disable');

      const inactiveItems: NodeListOf<Element> = <NodeListOf<HTMLElement>>(
        this.list.querySelectorAll(
          'li.' + dropDownBaseClasses.li + ':not(.e-active)'
        )
      );
  ...
  private dispatchSelect(
    ...
    const list: string[] | number[] | boolean[] | { [key: string]: Object }[] =
      this.listData;
    if (this.initStatus && !isNotTrigger) {
      value = this.allowObjectBinding
        ? getValue(this.fields.value ? this.fields.value : '', value)
        : value;
      const val: FieldSettingsModel = this.getDataByValue(
        value
      ) as FieldSettingsModel;
  ...
  private removeChipFocus(): void {
    const elements: NodeListOf<Element> = <NodeListOf<HTMLElement>>(
      this.chipCollectionWrapper.querySelectorAll(
        'span.' + CHIP + '.' + CHIP_SELECTED
      )
    );
  ...
  private onMobileChipInteraction(e: MouseEvent): void {
    const chipElem: HTMLElement = <HTMLElement>(
      closest(<HTMLElement>e.target, '.' + CHIP)
    );
    const chipClose: HTMLElement = <HTMLElement>(
      chipElem.querySelector('span.' + CHIP_CLOSE.split(' ')[0])
    );
    if (this.enabled && !this.readonly) {
  ...
  private multiCompiler(multiselectTemplate: string | Function): boolean {
    ...
    if (typeof multiselectTemplate !== 'function' && multiselectTemplate) {
      try {
        checkTemplate = selectAll(multiselectTemplate, document).length
          ? true
          : false;
      } catch (exception) {
        checkTemplate = false;
  ...
  private getChip(
    ...
    let itemData: { [key: string]: Object } | string | boolean | number = {
      text: value,
      value: value,
    };
    ...
    const chipContent: HTMLElement = this.createElement('span', {
      className: CHIP_CONTENT,
    });
    const chipClose: HTMLElement = this.createElement('span', {
    ...
    if (this.valueTemplate && !isNullOrUndefined(itemData)) {
      ...
      if (typeof this.valueTemplate !== 'function' && valuecheck) {
        compiledString = compile(
          select(this.valueTemplate, document).innerHTML.trim()
        );
      } else {
  ...
  private calcPopupWidth(): string {
    ...
    if (width.indexOf('%') > -1) {
      const inputWidth: number =
        (this.componentWrapper.offsetWidth * parseFloat(width)) / 100;
  ...
  private renderPopup(): void {
    ...
    if (!this.popupObj) {
      if (!isNullOrUndefined(this.popupWrapper)) {
        ...
        const checkboxFilter: HTMLElement = this.popupWrapper.querySelector(
          '.' + FILTERPARENT
        );
        if (
          this.mode === 'CheckBox' &&
        ...
        if (
          !this.list.classList.contains(dropDownBaseClasses.noData) &&
          (this.getItems()[1] as HTMLElement)
        ) {
          this.listItemHeight =
            (this.getItems()[1] as HTMLElement).offsetHeight +
            parseInt(
              window.getComputedStyle(this.getItems()[1] as HTMLElement)
                .marginBottom,
              10
            );
        }
        if (
          ...
          if (
            ...
            this.list.querySelector('.e-list-parent')
          ) {
            this.list
              .appendChild(
                this.createElement('div', {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (
              this.list.getElementsByClassName(
                'e-virtual-ddl-content'
              )[0] as any
            ).style = this.getTransformValues();
          }
          this.UpdateSkeleton();
          ...
          if (!this.list.querySelector('.e-virtual-ddl')) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (
              this.list.getElementsByClassName('e-virtual-ddl')[0] as any
            ).style = this.GetVirtualTrackHeight();
          }
        }
        ...
        this.popupObj = new Popup(this.popupWrapper, {
          ...
          close: () => {
            if (this.popupObj.element.parentElement) {
              ...
              const checkboxFilterInput: HTMLElement =
                this.popupWrapper.querySelector('.' + FILTERINPUT);
              if (
                this.mode === 'CheckBox' &&
                checkboxFilterInput &&
                document.activeElement === checkboxFilterInput
              ) {
            ...
            if (!this.isFirstClick) {
              ...
              if (ulElement) {
                if (
                  !(
                    this.mode !== 'CheckBox' &&
                    (this.allowFiltering || this.allowCustomValue) &&
                    this.targetElement().trim() !== ''
                  )
                ) {
                  this.mainList = ulElement.cloneNode
                    ? (ulElement.cloneNode(true) as HTMLElement)
                    : ulElement;
                }
              }
        ...
        this.popupContentElement =
          this.popupObj.element.querySelector('.e-content');

        if (
          this.mode === 'CheckBox' &&
          Browser.isDevice &&
          this.allowFiltering &&
          this.isDeviceFullScreen
        ) {
  ...
  private checkCollision(popupEle: HTMLElement): void {
    if (
      ...
      if (collision.length > 0) {
        popupEle.style.marginTop =
          -parseInt(getComputedStyle(popupEle).marginTop, 10) + 'px';
      }
      this.popupObj.resolveCollision();
    }
  }
  private setHeaderTemplate(): void {
    ...
    if (typeof this.headerTemplate !== 'function' && headercheck) {
      compiledString = compile(
        select(this.headerTemplate, document).innerHTML.trim()
      );
    } else {
      compiledString = compile(this.headerTemplate);
    }
    // eslint-disable-next-line
    let elements: any = compiledString(
      {},
      this,
      'headerTemplate',
  ...
  private setFooterTemplate(): void {
    ...
    if (typeof this.footerTemplate !== 'function' && footercheck) {
      compiledString = compile(
        select(this.footerTemplate, document).innerHTML.trim()
      );
    } else {
  ...
  private updateInitialData(): void {
    ...
    if (this.dataSource instanceof DataManager) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      this.totalItemCount = this.dataCount =
        this.dataSource && (this.dataSource as any).length
          ? (this.dataSource as any).length
          : 0;
    }
    ...
    if (this.list.getElementsByClassName('e-virtual-ddl')[0] as any) {
      ...
      !this.list.querySelector('.e-virtual-ddl') &&
    ...
    this.liCollections = <HTMLElement[] & NodeListOf<Element>>(
      this.list.querySelectorAll('.e-list-item')
    );
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (this.enabled && !this.readonly) {
      ...
      if (this.value && this.value.length > 0) {
        if (this.allowFiltering) {
          ...
          if (
            this.mode === 'CheckBox' &&
            (this.targetInputElement as HTMLInputElement)
          ) {
            (this.targetInputElement as HTMLInputElement).value = '';
          }
        }
        const liElement: NodeListOf<Element> =
          this.list && this.list.querySelectorAll('li.e-list-item');
        if (liElement && liElement.length > 0) {
          this.selectAllItems(false, e);
        } else {
          this.removeIndex = 0;
          for (
            temp = this.value[this.removeIndex];
            this.removeIndex < this.value.length;
            temp = this.value[this.removeIndex]
          ) {
  ...
  private clearAllCallback(e: MouseEvent, isClearAll?: boolean): void {
    const tempValues: string[] | number[] = this.value
      ? <string[]>this.value.slice()
      : <string[]>[];
    if (
      this.mainList &&
      this.listData &&
      ((this.allowFiltering && this.mode !== 'CheckBox') ||
        this.allowCustomValue)
    ) {
      const list: HTMLElement = this.mainList.cloneNode
        ? <HTMLElement>this.mainList.cloneNode(true)
  ...
  private windowResize(): void {
    this.refreshPopup();
    if (
      (!this.inputFocus || this.mode === 'CheckBox') &&
      this.viewWrapper &&
      this.viewWrapper.parentElement
    ) {
      this.updateDelimView();
    }
  }
  private resetValueHandler(e: Event): void {
    if (!isNullOrUndefined(this.inputElement)) {
      const formElement: HTMLFormElement = closest(
        this.inputElement,
        'form'
      ) as HTMLFormElement;
      if (formElement && e.target === formElement) {
        const textVal: string =
          this.element.tagName === this.getNgDirective()
            ? null
            : this.element.getAttribute('data-initial-value');
        this.text = textVal;
      }
    }
  }
  protected wireEvent(): void {
    ...
    EventHandler.add(
      <HTMLElement & Window>(<unknown>window),
      'resize',
      this.windowResize,
      this
    );
  ...
  protected search(e: KeyboardEventArgs): void {
    ...
    this.preventSetCurrentData = false;
    this.firstItem =
      this.dataSource && (this.dataSource as any).length > 0
        ? (this.dataSource as any)[0]
        : null;
    ...
    if (
      ...
      if (this.allowFiltering) {
        ...
        let activeElement: { [key: string]: Element | number } = Search(
          this.targetElement(),
          liCollections,
          'StartsWith',
          this.ignoreCase
        ...
        if (
          ...
          if (
            ...
            this.incrementalEndIndex =
              100 > this.totalItemCount ? this.totalItemCount : 100;
            this.updateIncrementalInfo(
              this.incrementalStartIndex,
              this.incrementalEndIndex
            );
          ...
          while (
            isNullOrUndefined(activeElement) &&
            this.incrementalEndIndex < this.totalItemCount
          ) {
            this.incrementalStartIndex = this.incrementalEndIndex;
            this.incrementalEndIndex =
              this.incrementalEndIndex + 100 > this.totalItemCount
                ? this.totalItemCount
          ...
          if (activeElement.index) {
            if (
              !(
                this.viewPortInfo.startIndex >= (activeElement.index as number)
              ) ||
          ...
          if (!isNullOrUndefined(activeElement.item)) {
            const index1: number =
              this.getIndexByValue(
                (activeElement.item as Element).getAttribute('data-value')
              ) - this.skeletonCount;
            ...
            activeElement.item = this.getElementByValue(
              (activeElement.item as Element).getAttribute('data-value')
            );
          } else {
            this.updateIncrementalView(0, this.itemCount);
        ...
        if (activeElement && activeElement.item) {
          this.addListFocus(<HTMLElement>activeElement.item);
          this.list.scrollTop =
            (<HTMLElement>activeElement.item).offsetHeight *
            <number>activeElement.index;
  ...
  private updateData(
    ...
    let data: string = '';
    const delim: boolean =
      this.mode === 'Delimiter' || this.mode === 'CheckBox';
    ...
    if (!isNullOrUndefined(this.value)) {
          ...
          if (this.listData) {
            if (this.enableVirtualization) {
              ...
              if (this.isRemoveSelection) {
                data =
                  this.text.replace(/,/g, delimiterChar + ' ') +
                  delimiterChar +
                  ' ';
              ...
              if (
                (e &&
                  e.currentTarget &&
                  (e.currentTarget as any).classList.contains(
                    'e-chips-close'
                  )) ||
                (e && (e as KeyboardEventArgs).key === 'Backspace')
              ) {
                const item: any = (e.target as any).parentElement.getAttribute(
                  'data-value'
                );
                if ((e as KeyboardEventArgs).key === 'Backspace') {
                  ...
                  this.hiddenElement.childNodes.forEach(
      ...
      if (!isNullOrUndefined(this.hiddenElement)) {
        if (this.isRemoveSelection) {
          if (
            this.findListElement(
              this.hiddenElement,
              'option',
              'value',
              this.currentRemoveValue
            )
          ) {
            this.hiddenElement.removeChild(
              this.findListElement(
                this.hiddenElement,
                'option',
                'value',
    ...
    const isChipRemove: boolean =
      e &&
      (e.target as HTMLElement) &&
      (e.target as HTMLElement).classList.contains('e-chips-close');
    if (
      !this.enableVirtualization ||
      (this.enableVirtualization && this.mode !== 'CheckBox' && !isChipRemove)
    ) {
    ...
    if (delim) {
      ...
      this.inputElement.setAttribute(
        'aria-describedby',
        this.delimiterWrapper.id
      );
    }
    const targetEle: HTMLElement = e && (e.target as HTMLElement);
    const isClearAll: boolean =
      targetEle && targetEle.classList.contains('e-close-hooker') ? true : null;
  ...
  private initialTextUpdate(): void {
    if (!isNullOrUndefined(this.text)) {
      ...
      for (let index: number = 0; textArr.length > index; index++) {
        const val: string | boolean | number = this.getValueByText(
          textArr[index as number]
        );
      ...
      if (textVal && textVal.length) {
        const value: string | boolean | number | Object = this
          .allowObjectBinding
          ? this.getDataByValue(textVal)
          : textVal;
  ...
  protected renderList(isEmptyData?: boolean): void {
    if (
      ...
      this.allowCustomValue &&
      this.list &&
      (this.list.textContent === this.noRecordsTemplate ||
        (this.list.querySelector('.e-ul') &&
          this.list.querySelector('.e-ul').childElementCount === 0))
    ) {
      isEmptyData = true;
    }
  ...
  private initialValueUpdate(
    ...
    if (this.list) {
      ...
      if (!isNullOrUndefined(this.value)) {
        for (
                    ...
                    if (isNullOrUndefined(this.selectedListData)) {
                      ...
                      if (Array.isArray(this.selectedListData)) {
                        this.selectedListData.push(
                          listItems[i as number] as never
                        );
                      } else {
                        this.selectedListData = [
                          this.selectedListData,
                          listItems[i as number] as any,
                        ];
                      }
                    }
                  }
          ...
          if (
            ...
            const noDataEle: HTMLElement = this.popupWrapper.querySelector(
              '.' + dropDownBaseClasses.noData
            );
            if (!this.enableVirtualization) {
            ...
            if (this.enableVirtualization) {
              if (
                ...
                (
                  this.virtualCustomSelectData as { [key: string]: object }[]
                ).push(newValue as { [key: string]: object });
              } else {
                (this.virtualCustomSelectData as { [key: string]: object }[]) =
                  [newValue as { [key: string]: object }];
              }
            }
            element = element
              ? element
      ...
      if (
        this.mode === 'CheckBox' &&
        this.showSelectAll &&
        (isNullOrUndefined(this.value) || !this.value.length)
      ) {
        this.notify('checkSelectAll', {
          module: 'CheckBoxSelection',
          enable: this.mode === 'CheckBox',
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  protected updateActionCompleteData(
    li: HTMLElement,
    ...
    if (
      this.value &&
      ((!this.allowObjectBinding &&
        (this.value as string[]).indexOf(li.getAttribute('data-value')) > -1) ||
        (this.allowObjectBinding &&
          this.isObjectInArray(
            this.getDataByValue(li.getAttribute('data-value')),
            this.value
          )))
    ) {
  ...
  protected updateAddItemList(list: HTMLElement, itemCount: number): void {
    if (
      ...
      this.popupObj.element.querySelector('.' + dropDownBaseClasses.noData) &&
      list
    ) {
      this.list = list;
  ...
  protected updateDataList(): void {
    if (
      this.mainList &&
      this.ulElement &&
      !(this.isFiltered || this.filterAction || this.targetElement().trim())
    ) {
      const isDynamicGroupItemUpdate: boolean =
        this.mainList.childElementCount < this.ulElement.childElementCount;
      const isReactTemplateUpdate: boolean =
        this.ulElement.childElementCount > 0 &&
        this.ulElement.children[0].childElementCount > 0 &&
        this.mainList.children[0] &&
        this.mainList.children[0].childElementCount <
          this.ulElement.children[0].childElementCount;
      const isAngularTemplateUpdate: boolean =
        this.itemTemplate &&
        this.ulElement.childElementCount > 0 &&
        !(this.ulElement.childElementCount < this.mainList.childElementCount) &&
        (this.ulElement.children[0].childElementCount > 0 ||
          (this.fields.groupBy &&
            this.ulElement.children[1] &&
            this.ulElement.children[1].childElementCount > 0));
      if (
        isDynamicGroupItemUpdate ||
        isReactTemplateUpdate ||
        isAngularTemplateUpdate
      ) {
  ...
  protected isValidLI(li: Element | HTMLElement): boolean {
    return (
      li &&
      !li.classList.contains(dropDownBaseClasses.disabled) &&
      !li.classList.contains(dropDownBaseClasses.group) &&
      li.classList.contains(dropDownBaseClasses.li)
    );
  }
  protected updateListSelection(
    li: Element,
    e: MouseEvent | KeyboardEventArgs,
    length?: number
  ): void {
    ...
    if (
      !this.value ||
      (!this.allowObjectBinding &&
        (this.value as string[]).indexOf(value as string) === -1) ||
      (this.allowObjectBinding &&
        this.indexOfObjectInArray(value, this.value) === -1)
    ) {
      this.dispatchSelect(
        value,
        e,
  ...
  private updateListSelectEventCallback(
    ...
    value = this.allowObjectBinding
      ? getValue(this.fields.value ? this.fields.value : '', value)
      : value;
    const text: string = this.getTextByValue(
    ...
    if (
      ...
      (!this.enableVirtualization ||
        (this.enableVirtualization && this.virtualCustomData))
    ) {
      const temp: HTMLElement = <HTMLElement>li
        ? <HTMLElement>li.cloneNode(true)
        : <HTMLElement>li;
      const fieldValue: string = this.fields.value
        ? this.fields.value
        : 'value';
      if (
        this.allowCustomValue &&
        this.mainData.length &&
        typeof getValue(fieldValue, this.mainData[0]) === 'number'
      ) {
      ...
      this.trigger(
        ...
        eventArgs,
        (eventArgs: CustomValueEventArgs) => {
          if (!eventArgs.cancel) {
            if (this.enableVirtualization && this.virtualCustomData) {
              if (
                ...
                (
                  this.virtualCustomSelectData as { [key: string]: object }[]
                ).push(data as { [key: string]: object });
              } else {
                (this.virtualCustomSelectData as { [key: string]: object }[]) =
              ...
              (this.mainData as { [key: string]: object }[]).push(
                data as { [key: string]: object }
              );
              this.remoteCustomValue = false;
              this.addValue(value as string | number | boolean, text, e);
  ...
  protected removeListSelection(): void {
    const className: string = this.hideSelectedItem
      ? HIDE_LIST
      : dropDownBaseClasses.selected;
    const selectedItems: Element[] = <NodeListOf<Element> & Element[]>(
      this.list.querySelectorAll('.' + className)
    );
    ...
    if (!isNullOrUndefined(this.mainList)) {
      const selectItems: Element[] = <NodeListOf<Element> & Element[]>(
        this.mainList.querySelectorAll('.' + className)
      );
        ...
        while (temp1 > 0) {
          ...
          if (this.mode === 'CheckBox') {
            if (selectedItems && selectedItems.length > temp1 - 1) {
              removeClass(
                [selectedItems[temp1 - 1].firstElementChild.lastElementChild],
                'e-check'
              );
            }
            removeClass(
              [selectItems[temp1 - 1].firstElementChild.lastElementChild],
              'e-check'
            );
          }
          temp1--;
  ...
  private removeHover(): void {
    const hoveredItem: Element[] = <NodeListOf<Element> & Element[]>(
  ...
  private removeFocus(): void {
    if (this.list && this.mainList) {
      const hoveredItem: Element[] = <NodeListOf<Element> & Element[]>(
        this.list.querySelectorAll('.' + dropDownBaseClasses.focus)
      );
      const mainlist: Element[] = <NodeListOf<Element> & Element[]>(
        this.mainList.querySelectorAll('.' + dropDownBaseClasses.focus)
      );
      if (hoveredItem && hoveredItem.length) {
  ...
  private addListHover(li: HTMLElement): void {
    if (this.enabled && this.isValidLI(li)) {
      ...
      if (
        li !== null &&
        li.classList.contains('e-list-group-item') &&
        this.enableGroupCheckBox &&
        this.mode === 'CheckBox' &&
        !isNullOrUndefined(this.fields.groupBy)
      ) {
  ...
  private addListFocus(element: HTMLElement): void {
    if (
      this.enabled &&
      (this.isValidLI(element) ||
        (this.fields.disabled && this.isDisabledElement(element)))
    ) {
      this.removeFocus();
  ...
  private addListSelection(
    ...
    if (
      this.isValidLI(element) &&
      !element.classList.contains(dropDownBaseClasses.hover)
    ) {
      addClass([element], className);
      this.updateMainList(
        false,
        <string>element.getAttribute('data-value'),
        mainElement
      );
      element.setAttribute('aria-selected', 'true');
      if (this.mode === 'CheckBox' && element.classList.contains('e-active')) {
        const ariaCheck: number =
          element.getElementsByClassName('e-check').length;
        if (ariaCheck === 0) {
          this.notify('updatelist', {
            module: 'CheckBoxSelection',
  ...
  private onMouseClick(e: MouseEvent): void {
    ...
    const li: HTMLElement = <HTMLElement>(
      closest(target, '.' + dropDownBaseClasses.li)
    );
    if (
      this.enableVirtualization &&
      li &&
      li.classList.contains('e-virtual-list')
    ) {
      return;
    }
    const headerLi: HTMLElement = <HTMLElement>(
      closest(target, '.' + dropDownBaseClasses.group)
    );
    if (
      headerLi &&
      this.enableGroupCheckBox &&
      ...
      target = target.classList.contains('e-list-group-item')
        ? target.firstElementChild.lastElementChild
        : <Element>e.target;
      if (target.classList.contains('e-check')) {
      ...
      if (this.enableVirtualization && this.hideSelectedItem) {
        const visibleListElements: NodeListOf<Element> = <
        ...
        if (visibleListElements.length) {
          const actualCount: number =
            this.virtualListHeight > 0
              ? Math.floor(this.virtualListHeight / this.listItemHeight)
              : 0;
  ...
  private findGroupStart(target: HTMLElement): void {
    if (
      ...
      if (
        this.itemTemplate &&
        !target.getElementsByClassName('e-frame').length
      ) {
      ...
      groupValues = this.findGroupAttrtibutes(
        target,
        liChecked,
        liUnchecked,
        count,
    ...
    while (!listElement.classList.contains('e-list-group-item')) {
      if (
        !(
          ...
          this.isDisabledElement(listElement as HTMLElement)
        )
      ) {
        ...
        if (
          listElement
            .getElementsByClassName('e-frame')[0]
      ...
      (listElement as Element) = position
  ...
  private updateCheckBox(
    ...
    if (
      groupHeader === null ||
      (!isNullOrUndefined(this.fields.disabled) && count === 0)
    ) {
      return;
    }
    const checkBoxElement: Element =
      groupHeader.getElementsByClassName('e-frame')[0];
    if (count === checked) {
      checkBoxElement.classList.remove('e-stop');
      checkBoxElement.classList.add('e-check');
      closest(checkBoxElement, '.' + 'e-list-group-item').classList.add(
        'e-active'
      );
      ...
      checkBoxElement.classList.remove('e-stop');
      closest(checkBoxElement, '.' + 'e-list-group-item').classList.remove(
        'e-active'
      );
  ...
  private disableGroupHeader(): void {
    const collection: NodeListOf<HTMLLIElement> = this.list.querySelectorAll(
      'li.e-list-group-item'
    );
    if (collection) {
      for (let index: number = 0; index < collection.length; index++) {
        let isDisabled: boolean = true;
        let target: Element = collection[index as number]
  ...
  private deselectHeader(): void {
    const limit: number =
      this.value && this.value.length ? this.value.length : 0;
    const collection: NodeList = this.list.querySelectorAll(
      'li.e-list-group-item:not(.e-active)'
    );
    if (limit < this.maximumSelectionLength) {
      removeClass(collection, 'e-disable');
    }
  ...
  private onMouseOver(e: MouseEvent): void {
    let currentLi: HTMLElement = <HTMLElement>(
      closest(<Element>e.target, '.' + dropDownBaseClasses.li)
    );
    if (
      currentLi === null &&
      this.mode === 'CheckBox' &&
      ...
      currentLi = <HTMLElement>(
        closest(<Element>e.target, '.' + dropDownBaseClasses.group)
      );
    }
  ...
  private onDocumentClick(e: MouseEvent): void {
    if (this.mode !== 'CheckBox') {
      ...
      if (
        !(
          !isNullOrUndefined(this.popupObj) &&
          closest(target, '[id="' + this.popupObj.element.id + '"]')
        ) &&
        !this.overAllWrapper.contains(e.target as Node)
      ) {
        this.scrollFocusStatus = false;
      } else {
        this.scrollFocusStatus =
          (Browser.isIE || Browser.info.name === 'edge') &&
  ...
  private showOverAllClear(): void {
    if (
      ((this.value && this.value.length) || this.inputElement.value !== '') &&
  ...
  public showSpinner(): void {
    if (isNullOrUndefined(this.spinnerElement)) {
      const filterClear: HTMLElement =
        this.filterParent &&
        this.filterParent.querySelector('.e-clear-icon.e-icons');
  ...
  protected updateWrapperText(
    wrapperType: HTMLElement,
    wrapperData: string
  ): void {
  ...
  private updateDelimView(): void {
    ...
    if (this.value && this.value.length) {
      ...
      const remainCompildTemp: string = remainContent.replace(
        '${count}',
      ...
      if (
        !isNullOrUndefined(this.value) &&
        (this.allowCustomValue || (this.listData && this.listData.length > 0))
      ) {
        for (
          let index: number = 0;
          !isNullOrUndefined(this.value[index as number]);
          ...
          const items: string[] =
            this.text && this.text.split(this.delimiterChar);
          if (!this.enableVirtualization) {
          ...
          wrapperleng =
            this.viewWrapper.offsetWidth +
            parseInt(
              window.getComputedStyle(this.viewWrapper).paddingRight,
              10
            );
          overAllContainer =
            this.componentWrapper.offsetWidth -
            parseInt(
              window.getComputedStyle(this.componentWrapper).paddingLeft,
              10
            ) -
            parseInt(
              window.getComputedStyle(this.componentWrapper).paddingRight,
              10
            );
          if (
            wrapperleng + downIconWidth + this.clearIconWidth >
            overAllContainer
          ) {
            ...
            while (
              wrapperleng + remainSize + downIconWidth + this.clearIconWidth >
                overAllContainer &&
              wrapperleng !== 0 &&
      ...
      if (remaining > 0) {
        const totalWidth: number =
          overAllContainer - downIconWidth - this.clearIconWidth;
        this.viewWrapper.appendChild(
          this.updateRemainTemplate(
            raminElement,
            this.viewWrapper,
            remaining,
            remainContent,
  ...
  private updateRemainWidth(
    ...
    if (
      viewWrapper.classList.contains(TOTAL_COUNT_WRAPPER) &&
      totalWidth <
        viewWrapper.offsetWidth +
  ...
  private updateRemainTemplate(
    ...
    if (
      viewWrapper.firstChild &&
      viewWrapper.firstChild.nodeType === 3 &&
      viewWrapper.firstChild.nodeValue === ''
    ) {
      viewWrapper.removeChild(viewWrapper.firstChild);
    }
    raminElement.innerHTML = '';
    const remainTemp: string = remainContent.replace(
      '${count}',
      remaining.toString()
    );
    const totalTemp: string = totalContent.replace(
      '${count}',
      remaining.toString()
    );
    raminElement.innerText =
  ...
  private updateRemainingText(
    ...
    if (
      wrapperleng + downIconWidth >= overAllContainer &&
      wrapperleng !== 0 &&
      this.viewWrapper.firstChild &&
      ...
      while (
        ...
        const textArr: string[] = (
          this.viewWrapper.firstChild as Text
        ).nodeValue.split(this.delimiterChar);
        textArr.pop();
        this.viewWrapper.firstChild.nodeValue = textArr.join(
          this.delimiterChar
        );
  ...
  private selectAllItem(
    ...
    if (!isNullOrUndefined(this.list)) {
      li = <HTMLElement[] & NodeListOf<Element>>(
        this.list.querySelectorAll(
          state
    ...
    if (
      ...
      let target: Element = <Element>(
        (event
          ? this.groupTemplate
            ? closest(event.target as Element, '.e-list-group-item')
            : event.target
          : null)
      );
      target = event && (event as KeyboardEvent).keyCode === 32 ? list : target;
      target =
        target && target.classList.contains('e-frame')
      ...
      if (target && target.classList.contains('e-list-group-item')) {
        ...
        while (listElement.classList.contains('e-list-item')) {
          if (
            ...
            if (state) {
              if (
                !listElement.firstElementChild.lastElementChild.classList.contains(
                  'e-check'
                )
              ) {
                let selectionLimit: number =
                  this.value && this.value.length ? this.value.length : 0;
        ...
        if (target.classList.contains('e-list-group-item')) {
          const focusedElement: Element =
            this.list.getElementsByClassName('e-item-focus')[0];
          if (focusedElement) {
  ...
  protected virtualSelectionAll(
    ...
    if (state) {
      ...
      length =
        this.virtualSelectAllData && this.virtualSelectAllData.length !== 0
          ? this.virtualSelectAllData.length
          : length;
      this.listData = this.virtualSelectAllData;
      const ulElement: HTMLElement = this.createListItems(
        this.virtualSelectAllData.slice(0, 30) as { [key: string]: Object }[],
        this.fields
      );
      const firstItems: NodeListOf<HTMLLIElement> =
      ...
      const concatenatedNodeList: NodeListOf<HTMLElement> | HTMLElement[] =
        fragment.childNodes as NodeListOf<HTMLElement>;
      ...
      if (this.virtualSelectAllData instanceof Array) {
        while (index < length && index <= 50 && index < count) {
          ...
          if (concatenatedNodeList[index as number]) {
            const value: string | number | boolean | object = this
              .allowObjectBinding
              ? this.getDataByValue(
                  concatenatedNodeList[index as number].getAttribute(
                    'data-value'
                  )
                )
              : this.getFormattedValue(
            ...
            if (
              (!this.allowObjectBinding &&
                this.value &&
                (this.value as string[]).indexOf(value as string) >= 0) ||
              (this.allowObjectBinding &&
                this.indexOfObjectInArray(value, this.value) >= 0)
            ) {
              index++;
              continue;
            }
            this.updateListSelection(
              concatenatedNodeList[index as number],
              event,
              length - index
            );
          } else {
            ...
            if (
              ...
              Array.isArray(this.value) &&
              ((!this.allowObjectBinding &&
                this.value.indexOf(value as never) < 0) ||
                (this.allowObjectBinding &&
                  !this.isObjectInArray(value, this.value)))
            ) {
        ...
        if (length > 50) {
          setTimeout((): void => {
            if (
              ...
              this.virtualSelectAllData.length > 0
            ) {
              (this.virtualSelectAllData as any[]).map((obj: any) => {
                if (
                  this.value &&
                  obj[this.fields.value] != null &&
                  Array.isArray(this.value) &&
                  ((!this.allowObjectBinding &&
                    this.value.indexOf(obj[this.fields.value] as never) < 0) ||
                    (this.allowObjectBinding &&
                      !this.isObjectInArray(
                        obj[this.fields.value],
                        this.value
                      )))
                ) {
                  this.dispatchSelect(
            ...
            if (
              this.popupWrapper &&
              li[index - 1] &&
              li[index - 1].classList.contains('e-item-focus')
            ) {
              const selectAllParent: Element =
                document.getElementsByClassName('e-selectall-parent')[0];
              if (
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const virtualTrackElement: Element =
      this.list.getElementsByClassName('e-virtual-ddl')[0];
    if (virtualTrackElement) {
      (virtualTrackElement as any).style = this.GetVirtualTrackHeight();
    }
    this.UpdateSkeleton();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const virtualContentElement: Element = this.list.getElementsByClassName(
      'e-virtual-ddl-content'
    )[0];
  ...
  private updateValue(
    ...
    const beforeSelectArgs: ISelectAllEventArgs = {
      ...
      itemData: state
        ? (this.listData as FieldSettingsModel[])
        : ([] as FieldSettingsModel[]),
      isInteracted: event ? true : false,
      isChecked: state,
    ...
    if ((li && li.length) || (this.enableVirtualization && !state)) {
      ...
      if (this.enableGroupCheckBox) {
        count = state
          ? this.maximumSelectionLength - (this.value ? this.value.length : 0)
          : li.length;
      } else {
        count = state
          ? this.maximumSelectionLength - (this.value ? this.value.length : 0)
      ...
      if (!beforeSelectArgs.preventSelectEvent) {
        ...
        for (let i: number = 0; i < li.length && i < count; i++) {
          ...
          let value: string | number | boolean | object =
            this.getFormattedValue(customVal);
          value = this.allowObjectBinding ? this.getDataByValue(value) : value;
          const mainElement: HTMLElement = this.mainList
            ? (this.mainList.querySelectorAll(
                state
                  ? 'li.e-list-item:not([aria-selected="true"]):not(.e-reorder-hide)'
                  : 'li.e-list-item[aria-selected="true"]:not(.e-reorder-hide)'
              )[i as number] as HTMLElement)
            : null;
          if (state) {
    ...
    if (length <= 50 && !beforeSelectArgs.preventSelectEvent) {
      ...
      if (
        (this.enableVirtualization && this.value && this.value.length > 0) ||
        !this.enableVirtualization
      ) {
        this.updateHiddenElement();
      }
  ...
  private updateHiddenElement(): void {
    ...
    if (this.mode === 'CheckBox') {
      (this.value as string[]).map((value: string, index: number): void => {
        hiddenValue +=
          '<option selected value ="' + value + '">' + index + '</option>';
        if (this.listData) {
  ...
  private textboxValueUpdate(event?: MouseEvent | KeyboardEventArgs): void {
    const isRemoveAll: Element =
      event &&
      event.target &&
      (closest(event.target as Element, '.e-selectall-parent') ||
        closest(event.target as Element, '.e-close-hooker'));
    if (
      this.mode !== 'Box' &&
      !this.isPopupOpen() &&
      !(this.mode === 'CheckBox' && (this.isSelectAll || isRemoveAll))
    ) {
      this.updateDelimView();
    } else {
      this.searchWrapper.classList.remove(ZERO_SIZE);
    ...
    if (this.mode === 'CheckBox') {
      this.updateDelimView();
      if (
        (!(isRemoveAll || this.isSelectAll) && this.isSelectAllTarget) ||
        (this.isSelectAll && this.isSelectAllTarget)
      ) {
  ...
  protected selectAllItems(state: boolean, event?: MouseEvent): void {
    ...
    if (
      !(this.dataSource instanceof DataManager) ||
      (this.dataSource instanceof DataManager && this.virtualSelectAllData)
    ) {
      this.virtualSelectAll = false;
    }
  }
  ...
  public onPropertyChanged(
    newProp: MultiSelectModel,
    oldProp: MultiSelectModel
  ): void {
    if (
      (newProp.dataSource &&
        !isNullOrUndefined(Object.keys(newProp.dataSource))) ||
      (newProp.query && !isNullOrUndefined(Object.keys(newProp.query)))
    ) {
      if (this.resetFilteredData) {
        // The filtered data is not being reset in the component after the user focuses out.
        this.resetMainList = !this.resetMainList
          ? this.mainList
          : this.resetMainList;
    ...
    if (this.getModuleName() === 'multiselect') {
      this.filterAction = false;
      this.setUpdateInitial(
        ['fields', 'query', 'dataSource'],
        newProp as { [key: string]: string }
      );
    }
      ...
      switch (prop) {
        ...
        case 'text':
          if (this.fields.disabled) {
            this.text =
              this.text &&
              !this.isDisabledItemByIndex(
                this.getIndexByValue(this.getValueByText(this.text))
              )
                ? this.text
                : null;
          }
        ...
        case 'value':
          ...
          if (
            ...
            this.mode === 'CheckBox' &&
            this.list &&
            this.list.querySelector('.e-active.e-disable')
          ) {
            const activeItems: NodeListOf<Element> = <NodeListOf<HTMLElement>>(
              this.list.querySelectorAll(
          ...
          this.preventChange =
            this.isAngular && this.preventChange
              ? !this.preventChange
              : this.preventChange;
          break;
        case 'width':
        ...
        case 'popupHeight':
          if (this.popupObj) {
            const overAllHeight: number = parseInt(
              <string>this.popupHeight,
              10
            );
            if (this.popupHeight !== 'auto') {
              this.list.style.maxHeight = formatUnit(overAllHeight);
        ...
        default:
          {
            const msProps: { [key: string]: Object } = this.getPropObject(
              prop,
              <{ [key: string]: string }>newProp,
              <{ [key: string]: string }>oldProp
            );
  ...
  private totalItemsCount(): void {
    ...
    if (this.mode === 'CheckBox') {
      this.totalItemCount =
        dataSourceCount !== 0 ? dataSourceCount : this.totalItemCount;
    } else {
      if (this.hideSelectedItem) {
        this.totalItemCount =
          dataSourceCount !== 0 && this.value
            ? dataSourceCount - this.value.length
            : this.totalItemCount;
        if (
          this.allowCustomValue &&
          this.virtualCustomSelectData &&
          ...
          for (
            let i: number = 0;
            i < this.virtualCustomSelectData.length;
            i++
          ) {
            for (let j: number = 0; j < this.value.length; j++) {
              const value: string | number | boolean = this.allowObjectBinding
                ? getValue(
                    this.fields.value ? this.fields.value : '',
                    this.value[j as number]
                  )
                : this.value[j as number];
              const customValue: string | number | boolean = getValue(
                this.fields.value ? this.fields.value : '',
                this.virtualCustomSelectData[i as number]
              );
  ...
  private presentItemValue(
    ...
    for (let i: number = 0; i < this.value.length; i++) {
      ...
      if (!checkEle) {
        const checkvalue: string | object | boolean | number = this
          .allowObjectBinding
          ? this.getDataByValue(this.value[i as number])
  ...
  private addNonPresentItems(
    ...
    (this.dataSource as DataManager)
      .executeQuery(this.getForQuery(valuecheck))
      .then((e: Object) => {
  ...
  private updateVal(
    ...
    if (!this.list) {
      ...
      this.dataSource instanceof DataManager &&
      (!this.listData || !(this.mainList && this.mainData))
    ) {
      this.onLoadSelect();
    } else {
      ...
      if (
        ...
        if (
          this.allowCustomValue &&
          (this.mode === 'Default' || this.mode === 'Box') &&
          (this as any).isReact &&
          this.inputFocus &&
          this.isPopupOpen() &&
          this.mainData !== this.listData
        ) {
          const list: HTMLElement = this.mainList.cloneNode
            ? <HTMLElement>this.mainList.cloneNode(true)
            : this.mainList;
      ...
      this.trigger('close', eventArgs, (eventArgs: PopupEventArgs) => {
        if (!eventArgs.cancel) {
          ...
          const typedValue: string =
            this.mode === 'CheckBox' ? this.targetElement() : null;
          ...
          this.popupObj.hide();
          removeClass(
            [document.body, this.popupObj.element],
            'e-popup-full-page'
          );
  ...
  public showPopup(e?: MouseEvent | KeyboardEventArgs | TouchEvent): void {
    ...
    this.trigger('beforeOpen', args, (args: BeforeOpenEventArgs) => {
      if (!args.cancel) {
        ...
        const mainLiLength: number = this.ulElement.querySelectorAll(
          'li.' + 'e-list-item'
        ).length;
        const liLength: number = this.ulElement.querySelectorAll(
          'li.' + dropDownBaseClasses.li + '.' + HIDE_LIST
        ).length;
        if (
          mainLiLength > 0 &&
          mainLiLength === liLength &&
          liLength === this.mainData.length &&
        ...
        if (
          ...
          this.selectedValueInfo.startIndex > 0 &&
          this.value != null
        ) {
        ...
        if (this.enableVirtualization) {
          ...
          if (this.value && this.value.length) {
            ...
            for (const value of this.value) {
              const checkValue: string | number | boolean = this
                .allowObjectBinding
                ? getValue(this.fields.value ? this.fields.value : '', value)
                : value;
  ...
  public render(): void {
    ...
    this.searchWrapper = this.createElement('span', {
      className:
        SEARCHBOX_WRAPPER + ' ' + (this.mode === 'Box' ? BOX_ELEMENT : ''),
    });
    ...
    if (this.mode === 'Delimiter' || this.mode === 'CheckBox') {
      ...
      if (this.mode === 'Default') {
        this.chipCollectionWrapper.setAttribute(
          'id',
          getUniqueID('chip_default')
        );
    ...
    if (
      !isNullOrUndefined(
        closest(this.element, 'fieldset') as HTMLFieldSetElement
      ) &&
    ...
    if (this.enableVirtualization) {
      this.updateVirtualizationProperties(
        this.itemCount,
        this.allowFiltering,
        this.mode === 'CheckBox'
      );
  ...
  private removeDisabledItemsValue(
    ...
    if (value) {
      ...
      for (let index: number = 0; index < value.length; index++) {
        let indexValue: number | string | boolean | object =
          value[index as number];
        if (typeof indexValue === 'object') {
          indexValue = JSON.parse(JSON.stringify(indexValue))[
            this.fields.value
          ];
        }
        if (
          indexValue != null &&
          !this.isDisabledItemByIndex(this.getIndexByValue(indexValue))
        ) {
  ...
  private checkInitialValue(): void {
    ...
    const isData: boolean =
      this.dataSource instanceof Array
    ...
    if (
      ...
      const optionsElement: HTMLOptionsCollection = (<HTMLSelectElement>(
        this.element
      )).options;
      const valueCol: string[] | object[] = [];
      let textCol: string = '';
      for (
        let index: number = 0, optionsLen: number = optionsElement.length;
        index < optionsLen;
        index++
      ) {
        ...
        if (!isNullOrUndefined(opt.getAttribute('selected'))) {
          if (opt.getAttribute('value')) {
            const value:
              ...
              ? this.getDataByValue(opt.getAttribute('value'))
    ...
    if (this.value && this.value.length) {
      ...
      if (this.enableVirtualization) {
        ...
        if (this.dataSource instanceof DataManager) {
          ...
          >new DataManager(
            this.dataSource as DataOptions | JSON[]
  ...
  private initialUpdate(): void {
    if (
      this.mode !== 'Box' &&
      !(this.setDynValue && this.mode === 'Default' && this.inputFocus)
    ) {
      this.updateDelimView();
    }
    this.viewPortInfo.startIndex = this.virtualItemStartIndex = 0;
      ...
      if (
        ...
        if (typeof item === 'string') {
          ...
          if (item instanceof HTMLLIElement) {
            for (
              let index: number = 0;
              index < this.liCollections.length;
              index++
            ) {
              if (
                (this.liCollections[index as number] as HTMLLIElement) === item
              ) {
                itemIndex = this.getIndexByValue(
            ...
            const value: string = JSON.parse(JSON.stringify(item))[
              this.fields.value
            ];
            for (let index: number = 0; index < this.listData.length; index++) {
              if (
                JSON.parse(JSON.stringify(this.listData[index as number]))[
                  this.fields.value
                ] === value
              ) {
        ...
        const isValidIndex: boolean =
          itemIndex < this.liCollections.length && itemIndex > -1;
        if (
          isValidIndex &&
          !JSON.parse(JSON.stringify(this.listData[itemIndex as number]))[
            this.fields.disabled
          ]
        ) {
          const li: HTMLLIElement = this.liCollections[
            itemIndex as number
          ] as HTMLLIElement;
          if (li) {
            this.disableListItem(li);
            const parsedData: { [key: string]: Object } = JSON.parse(
              JSON.stringify(this.listData[itemIndex as number])
            );
            ...
            if (
              li.classList.contains(HIDE_LIST) ||
              li.classList.contains(dropDownBaseClasses.selected)
            ) {
              const oldValue: string[] | number[] | boolean[] | object[] =
                this.value;
              this.removeDisabledItemsValue(this.value);
  ...
  public destroy(): void {
    ...
    if (this.overAllWrapper && this.overAllWrapper.parentElement) {
      if (this.overAllWrapper.parentElement.tagName === this.getNgDirective()) {
        ...
        this.overAllWrapper.parentElement.insertBefore(
          this.element,
          this.overAllWrapper
        );
        remove(this.overAllWrapper);
