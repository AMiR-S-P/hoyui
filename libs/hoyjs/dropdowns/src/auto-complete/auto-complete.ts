...
import { Event, EmitType, Complex } from '@hoyui/base';
import { removeClass, NotifyPropertyChanges } from '@hoyui/base';
...
import { FloatLabelType, Input } from '@hoyui/inputs';
import { DataManager, Predicate, Query } from '@hoyui/data';
...
export class AutoComplete extends ComboBox {
  ...
  @Complex<FieldSettingsModel>(
    { value: null, iconCss: null, groupBy: null, disabled: null },
    FieldSettings
  )
  ...
  protected getQuery(query: Query): Query {
    ...
    const filterType: string =
      this.queryString === '' && !isNullOrUndefined(value)
        ? 'equal'
        : this.filterType;
    ...
    if (this.queryString !== null && this.queryString !== '') {
      ...
      if (
        ...
        const mapping: string = !isNullOrUndefined(this.fields.value)
          ? this.fields.value
          : '';
        filterQuery.where(
          mapping,
    ...
    if (
      !isNullOrUndefined(this.suggestionCount) &&
      !this.enableVirtualization
    ) {
      ...
      if (queryTakeValue <= 0 && this.query && this.query.queries.length > 0) {
        for (
          ...
          if (this.query.queries[queryElements as number].fn === 'onTake') {
            const currentTakeValue: number =
              this.query.queries[queryElements as number].e.nos;
            queryTakeValue =
              takeValue <= currentTakeValue ? currentTakeValue : takeValue;
          }
        }
      }
      if (filterQuery && filterQuery.queries.length > 0) {
        for (
          ...
          if (filterQuery.queries[queryElements as number].fn === 'onTake') {
            const currentQueryTakeValue: number =
              filterQuery.queries[queryElements as number].e.nos;
            queryTakeValue =
              currentQueryTakeValue <= queryTakeValue
                ? queryTakeValue
                : currentQueryTakeValue;
            filterQuery.queries.splice(queryElements, 1);
            --queryElements;
          }
        }
    ...
    if (this.queryString !== '' && this.queryString.length >= this.minLength) {
      ...
      if (this.enableVirtualization && !isNoDataElement) {
        if (
          ...
          this.list
            .appendChild(virtualElement)
            .appendChild(this.list.querySelector('.e-list-parent'));
        }
        if (
          !this.list.querySelector('.e-virtual-ddl') &&
          document.getElementsByClassName('e-popup') &&
          document.getElementsByClassName('e-popup')[0]
        ) {
          const virtualElement: HTMLElement = this.createElement('div', {
  ...
  private postBackAction(): void {
    if (
      this.autofill &&
      !isNullOrUndefined(this.liCollections[0]) &&
      this.searchList
    ) {
  ...
  protected setSelection(
    ...
    if (
      ...
      (e as KeyboardEventArgs).action !== 'tab' &&
      this.isValidLI(li)
    ) {
      ...
      if (this.autofill && this.isPopupOpen) {
        ...
        const isKeyNavigate: boolean =
          (e && (e as KeyboardEventArgs).action === 'down') ||
          (e as KeyboardEventArgs).action === 'up' ||
          (e as KeyboardEventArgs).action === 'home' ||
          (e as KeyboardEventArgs).action === 'end' ||
          (e as KeyboardEventArgs).action === 'pageUp' ||
          (e as KeyboardEventArgs).action === 'pageDown';
  ...
  protected setInputValue(newProp?: any, oldProp?: any): void {
    let oldValue: string | number | boolean =
      oldProp && oldProp.text
        ? oldProp.text
        : oldProp
        ? oldProp.value
        : oldProp;
    let value: string | number | boolean =
      newProp && newProp.text
        ? newProp.text
    ...
    if (
      ...
      const checkFields1: string =
        this.typeOfData(this.dataSource).typeof === 'string'
          ? ''
          : this.fields.value;
      ...
      new DataManager(this.dataSource)
        .executeQuery(query.where(new Predicate(checkFields1, 'equal', value)))
        .then(function (e: Object): void {
          if ((e as ResultData).result.length > 0) {
            _this.value =
              checkFields1 !== ''
                ? _this.allowObjectBinding
                  ? (e as ResultData).result[0]
                  : (e as ResultData).result[0][_this.fields.value].toString()
                : (e as ResultData).result[0].toString();
  ...
  public onPropertyChanged(
    newProp: AutoCompleteModel,
    oldProp: AutoCompleteModel
  ): void {
    ...
    for (const prop of Object.keys(newProp)) {
      switch (prop) {
        ...
        default: {
          ...
          const atcProps: { [key: string]: Object } = this.getPropObject(
            prop,
