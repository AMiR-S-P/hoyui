...
import { debounce, Touch } from '@hoyui/base';
...
export class VirtualScroll {
  ...
  private observe(callback: Function): void {
    this.containerElementRect =
      this.parent.popupContentElement.getBoundingClientRect();
    EventHandler.add(
      this.parent.popupContentElement,
      'wheel mousedown',
      this.popupScrollHandler,
      this
    );
    ...
    EventHandler.add(
      this.parent.popupContentElement,
      'scroll',
      this.virtualScrollHandler(callback),
      this
    );
  }
  ...
  private setGeneratedData(
    qStartIndex: number,
    recentlyGeneratedData: object[]
  ): void {
    let loopIteration: number = 0;
    const endIndex: number =
      this.parent.listData.length + this.parent.virtualItemStartIndex;
    for (let i: number = this.parent.virtualItemStartIndex; i < endIndex; i++) {
      const alreadyAddedData: object | undefined =
        this.parent.generatedDataObject[i as number];
      if (!alreadyAddedData) {
        if (
          recentlyGeneratedData !== null &&
          this.parent.listData.slice(loopIteration, loopIteration + 1).length >
            0
        ) {
          const slicedData:
            | { [key: string]: Object }[]
            | string[]
            | boolean[]
            | number[] = this.parent.listData.slice(
            loopIteration,
            loopIteration + 1
          );
  ...
  private generateAndExecuteQueryAsync(
    ...
    const dataSource:
      ...
      | string[]
      | number[]
      | boolean[] = this.parent.dataSource;
    if (!isQueryGenerated) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if (!isNullOrUndefined((this.parent as any).query)) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const newQuery: Query = this.removeSkipAndTakeEvents(
          (this.parent as any).query.clone()
        );
    ...
    if (this.component === 'combobox') {
      ...
      if (this.parent.dataSource instanceof DataManager) {
        ...
        (this.parent.dataSource as any).length > 0
      ) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  public setCurrentViewDataAsync(component?: object): void {
      ...
      if (this.parent.viewPortInfo.startIndex < this.parent.value.length) {
        ...
        if (this.parent.viewPortInfo.startIndex === 0) {
          ...
          if (this.parent.value.length < this.parent.itemCount) {
            const oldUlElement: Element = this.parent.list.querySelector(
              '.e-list-parent' + ':not(.e-reorder)'
            );
            if (oldUlElement) {
              this.parent.list
                .querySelector('.e-virtual-ddl-content')
                .removeChild(oldUlElement);
            }
            let query: Query = this.parent
              .getForQuery(this.parent.value)
              .clone();
            query = query
              .skip(0)
              .take(
                this.parent.itemCount -
                  (this.parent.value.length -
                    this.parent.viewPortInfo.startIndex)
              );
          ...
          this.parent.updateVirtualReOrderList(true);
          const oldUlElement: HTMLElement = this.parent.list.querySelector(
            '.e-list-parent' + ':not(.e-reorder)'
          );
          if (oldUlElement) {
            this.parent.list
        ...
        if (
          ...
          this.parent.viewPortInfo.startIndex - this.parent.value.length !==
            this.parent.itemCount &&
          this.parent.viewPortInfo.startIndex + this.parent.itemCount >
            this.parent.value.length
        ) {
          ...
          query = query
            ...
            .take(
              this.parent.itemCount -
                (this.parent.value.length - this.parent.viewPortInfo.startIndex)
            );
          this.parent.appendUncheckList = true;
          this.parent.setCurrentView = false;
          this.parent.resetList(
    ...
    if (isListUpdated) {
      for (
        let i: number = this.parent.viewPortInfo.startIndex;
        i < endIndex;
        i++
      ) {
        const index: number = i;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const alreadyAddedData: any =
          this.parent.generatedDataObject[index as number];
        if (this.component === 'multiselect' && this.parent.hideSelectedItem) {
          if (alreadyAddedData) {
            const value: any = getValue(
              this.parent.fields.value,
              alreadyAddedData[0]
            );
            if (
              this.parent.value &&
              ...
              let query: Query = this.parent
                .getForQuery(this.parent.value as any)
                .clone();
              if (
                this.parent.viewPortInfo.endIndex ===
                  this.parent.totalItemCount +
                    (this.parent.value as any).length &&
                this.parent.hideSelectedItem
              ) {
                query = query.skip(
                  this.parent.totalItemCount - this.parent.itemCount
                );
              } else {
              ...
              (this.parent.value && (this.parent.value as any).length === 0)
            ) {
    ...
    this.parent.skeletonCount =
      ...
      (!(this.parent.dataSource instanceof DataManager) ||
        ...
        : this.parent.skeletonCount;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const virtualTrackElement: any = this.parent.list.getElementsByClassName(
      'e-virtual-ddl'
    )[0] as HTMLElement;
    if (virtualTrackElement) {
      ...
      !virtualTrackElement &&
      this.parent.skeletonCount > 0 &&
      this.parent.popupWrapper
    ) {
    ...
    this.parent.liCollections = <HTMLElement[] & NodeListOf<Element>>(
      this.parent.list.querySelectorAll('.e-list-item')
    );
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const virtualContentElement: any = this.parent.list.getElementsByClassName(
    ...
    if (
      ...
      const currentSelectElem: Element = this.parent.getElementByValue(
        this.parent.currentFocuedListElement.getAttribute('data-value')
      );
  ...
  private async virtualScrollRefreshAsync(): Promise<void> {
    this.parent.isCustomFilter =
      (!(
        ...
        (this.component === 'combobox' &&
          this.parent.allowFiltering &&
          this.parent.queryString !== this.parent.typedString) ||
        (!isNullOrUndefined(this.parent.filterInput) &&
          !isNullOrUndefined(this.parent.filterInput.value) &&
    ...
    if (this.parent.allowFiltering || this.component === 'autocomplete') {
      if (
        !isNullOrUndefined(this.parent.typedString) &&
        !(
          this.component === 'combobox' &&
          !isNullOrUndefined(this.parent.typedString) &&
          this.parent.allowFiltering
        ...
        if (this.parent.viewPortInfo.startIndex >= this.parent.dataCount) {
          this.parent.viewPortInfo.startIndex =
            this.parent.dataCount - this.parent.itemCount;
        }
      } else {
    ...
    if (
      ...
      this.parent.handleVirtualKeyboardActions(
        this.parent.keyboardEvent,
        this.parent.pageCount
      );
    }
  ...
  public scrollListener(scrollArgs: ScrollArg): void {
    if (
      !this.parent.isPreventScrollAction &&
      !this.parent.isVirtualTrackHeight
    ) {
      ...
      this.parent.viewPortInfo = this.getInfoFromView(
      ...
      if (
        this.parent.previousStartIndex !== pStartIndex &&
        !this.parent.isKeyBoardAction
      ) {
        this.parent.isScrollActionTriggered = false;
        this.parent.currentPageNumber =
          this.parent.viewPortInfo.currentPageNumber;
        ...
        setTimeout(() => {
          ...
          this.virtualScrollRefreshAsync().then(() => {
            if (this.parent.popupObj) {
              this.parent.list =
                this.parent.popupObj.element.querySelector('.' + 'e-content') ||
                select('.' + 'e-content');
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (
          this.parent.list.getElementsByClassName(
            'e-virtual-ddl-content'
          )[0] as any
        ).style = this.parent.getTransformValues();
      }
  ...
  private getInfoFromView(
    ...
    const vHeight: string | number =
      this.parent.popupContentElement.getBoundingClientRect().height;
    ...
    if (infoType.direction === 'down') {
      const sIndex: number =
        Math.round(exactEndIndex) - Math.round(pageSizeBy4);
      if (
        isNullOrUndefined(infoType.startIndex) ||
        (exactEndIndex >
          infoType.startIndex +
            Math.round(this.parent.virtualItemCount / 2 + pageSizeBy4) &&
          infoType.endIndex !== totalItemCount)
      ) {
        infoType.startIndex = sIndex >= 0 ? Math.round(sIndex) : 0;
        infoType.startIndex =
          infoType.startIndex > exactTopIndex
            ? Math.floor(exactTopIndex)
            : infoType.startIndex;
        const eIndex: number =
          infoType.startIndex + this.parent.virtualItemCount;
        infoType.startIndex =
          eIndex < exactEndIndex
            ? Math.ceil(exactEndIndex) - this.parent.virtualItemCount
            : infoType.startIndex;
        infoType.endIndex = eIndex < totalItemCount ? eIndex : totalItemCount;
        infoType.startIndex =
          eIndex >= totalItemCount
            ? infoType.endIndex - this.parent.virtualItemCount > 0
              ? infoType.endIndex - this.parent.virtualItemCount
              : 0
            : infoType.startIndex;
        infoType.currentPageNumber = Math.ceil(
          infoType.endIndex / this.parent.virtualItemCount
        );
      }
    } else if (infoType.direction === 'up') {
      if (
        (infoType.startIndex && infoType.endIndex) ||
        Math.ceil(exactTopIndex) > this.parent.previousStartIndex
      ) {
        const loadAtIndex: number = Math.round(
          (infoType.startIndex * rowHeight + pageSizeBy4 * rowHeight) /
            rowHeight
        );
        if (
          exactTopIndex < loadAtIndex ||
          Math.ceil(exactTopIndex) > this.parent.previousStartIndex
        ) {
          const idxAddedToExactTop: number =
            pageSizeBy4 > infoViewIndices
              ? pageSizeBy4
          ...
          infoType.endIndex =
            sIndex < 0 ? this.parent.virtualItemCount : infoType.endIndex;
          infoType.currentPageNumber = Math.ceil(
            infoType.startIndex / this.parent.virtualItemCount
          );
        }
  ...
  private virtualScrollHandler(callback?: Function): Function {
    ...
    return (e: Event) => {
      ...
      const direction: ScrollDirection =
        prevTop < top && !this.parent.isUpwardScrolling ? 'down' : 'up';
      prevTop = top;
      const current: SentinelType =
        this.sentinelInfo[direction as 'up' | 'down'];
      ...
      if (
        this.parent.list &&
        this.parent.list.querySelectorAll('.e-virtual-list').length > 0
      ) {
        this.getInfoFromView(direction, current, scrollOffsetargs, true);
        if (
          this.parent.scrollPreStartIndex !== pstartIndex &&
          !this.parent.isPreventScrollAction
        ) {
          this.parent.isScrollActionTriggered = true;
          const virtualPoup: HTMLElement = this.parent.list.querySelector(
            '.e-virtual-ddl-content'
          );
