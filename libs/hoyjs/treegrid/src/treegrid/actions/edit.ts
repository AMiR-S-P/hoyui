...
export class Edit {
  ...
  private gridDblClick(e: MouseEvent): void {
    ...
    if (
      (e.target as HTMLElement).classList.contains('e-frame') &&
    ...
    if (
      ...
      const rowIndex: number =
        tr && parseInt(tr.getAttribute('data-rowindex'), 10);
      if (
        !isNullOrUndefined(rowIndex) &&
        rowIndex >= 0 &&
        this.parent.allowPaging
      ) {
        /* eslint-disable-next-line */
        (
          this.parent.grid.getDataRows()[rowIndex] as HTMLTableRowElement
        ).dataset.uid = (
          this.parent.grid.contentModule.getRows()[rowIndex] as any
        ).uid;
      }
    }
  }
  private getRowPosition(addArgs: {
  ...
  private editActionEvents(args: Object): void {
    const eventArgs: ActionEventArgs | CellSaveEventArgs = getObject(
      'editAction',
      args
    );
    const eventName: string = getObject('name', eventArgs);
    const treeObj: TreeGrid = this.parent;
    const adaptor: AdaptorOptions =
      !isNullOrUndefined(treeObj.dataSource as DataManager) &&
      (treeObj.dataSource as DataManager).adaptor;
    if (
      !isNullOrUndefined(adaptor) &&
      (isRemoteData(treeObj) || adaptor instanceof RemoteSaveAdaptor) &&
      eventArgs.requestType === 'save' &&
      eventArgs.action === 'add' &&
      (treeObj.editSettings.newRowPosition === 'Child' ||
        treeObj.editSettings.newRowPosition === 'Below' ||
        treeObj.editSettings.newRowPosition === 'Above')
    ) {
      if (eventName === 'actionBegin') {
        const rowIndex: number =
          isNullOrUndefined(eventArgs.row) || !Object.keys(eventArgs.row).length
            ? this.selectedIndex
            : (<HTMLTableRowElement>eventArgs.row).rowIndex - 1;
        const keyData: string =
          !isNullOrUndefined(rowIndex) && rowIndex !== -1
            ? treeObj.getCurrentViewRecords()[
                parseInt(rowIndex.toString(), 10)
              ][treeObj.getPrimaryKeyFieldNames()[0]]
            : -1;
        ...
        for (let i: number = 0; i < paramsLength; i++) {
          if (
            treeObj.grid.query.params[parseInt(i.toString(), 10)].key ===
            'relationalKey'
          ) {
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          newRowObjectIndex += findChildrenRecords(
            currentData[newRowObjectIndex + 1]
          ).length;
        }
        newRowObjectIndex =
          this.parent.editSettings.newRowPosition === 'Below'
      ...
      if (!isCache) {
        resetRowIndex(
          this.parent.grid,
          this.parent.grid.getRowsObject(),
          this.parent.grid.getRows() as HTMLTableRowElement[],
          0
        );
        this.updateIndex(
      ...
      if (!isCache && this.parent.getFrozenColumns() > 0) {
        resetRowIndex(
          this.parent.grid,
          movableRowsObject,
          movableRows as HTMLTableRowElement[],
          0
        );
        this.updateIndex(
          this.parent.grid.dataSource,
  ...
  private updateInfiniteCurrentViewData(
    ...
    const updateCurrentViewData: string = 'updateCurrentViewData';
    const size: number = Math.ceil(
      newRowIndex / this.parent.grid.pageSettings.pageSize
    );
    let page: number = size > 0 ? size : 1;
    let dataIndex: number =
      newRowIndex - (page - 1) * this.parent.pageSettings.pageSize;
    const infiniteCurrentViewData: { [x: number]: Object[] } =
      this.parent.grid.infiniteScrollModule[`${infiniteData}`];
    ...
    if (dataIndex >= this.parent.pageSettings.pageSize) {
      ...
      dataIndex =
        dataIndex - this.parent.pageSettings.pageSize >= 0
          ? dataIndex - this.parent.pageSettings.pageSize
          : 0;
    }
    dataIndex =
      this.parent.editSettings.newRowPosition === 'Below'
  ...
  private recordDoubleClick(args: RecordDoubleClickEventArgs): void {
    ...
    if (
      !this.parent.grid.editSettings.allowEditing ||
      this.parent.grid.isEdit
    ) {
      return;
    }
    const column: Column = this.parent.grid.getColumnByIndex(
      +target.closest('td.e-rowcell').getAttribute('data-colindex')
    );
    if (
      ...
      if (this.parent.enableVirtualization) {
        const tr: HTMLTableRowElement = <HTMLTableRowElement>(
          parentsUntil(args.target as Element, 'e-row')
        );
      ...
      this.parent.editSettings.mode === 'Cell' &&
      (!column.allowEditing || column.isPrimaryKey)
    ) {
      this.isOnBatch = true;
      this.updateGridEditMode('Batch');
    }
  ...
  private updateGridEditMode(mode: string): void {
    ...
    const updateMethod: Function = getObject(
      'updateEditObj',
      this.parent.grid.editModule
    ) as Function;
  ...
  private cellEdit(args: CellEditArgs): Deferred | void {
    ...
    if (
      this.parent.enableVirtualization &&
      !isNullOrUndefined(this.prevAriaRowIndex) &&
    ...
    if (this.parent.enableVirtualization) {
      this.parent.grid.contentModule['editedRowIndex'] =
        this.parent.grid.editModule.editModule['index'];
    }
  ...
  private enableToolbarItems(request: string): void {
    if (!isNullOrUndefined(this.parent.grid.toolbarModule)) {
      ...
      this.parent.grid.toolbarModule.enableItems(
        [toolbarID + 'add', toolbarID + 'edit', toolbarID + 'delete'],
        request === 'save'
      );
      this.parent.grid.toolbarModule.enableItems(
        [toolbarID + 'update', toolbarID + 'cancel'],
        request === 'edit'
      );
    }
  }
  ...
  private batchCancel(): void {
    if (this.parent.editSettings.mode === 'Cell') {
      const cellDetails: RowInfo = getValue(
        'editModule.cellDetails',
        this.parent.grid.editModule
      );
      const treeCell: HTMLElement = this.parent.getCellFromIndex(
  ...
  private cellSave(args: CellSaveArgs): void {
    if (
      this.parent.editSettings.mode === 'Cell' &&
      this.parent.element.querySelector('form')
    ) {
      args.cancel = true;
      ...
      if (isNullOrUndefined(args.cell)) {
        row =
          this.parent.grid.editModule[`${editModule}`].form.parentElement
            .parentNode;
      } else {
      ...
      if (isNullOrUndefined(row)) {
        ...
        const freeze: boolean =
          this.parent.getFrozenLeftColumnsCount() > 0 ||
          this.parent.getFrozenRightColumnsCount() > 0
            ? true
            : false;
        if (freeze) {
          ...
          rowIndex =
            this.parent.getRows().indexOf(row) === -1 &&
            this.parent.getFrozenColumns() > 0
              ? this.parent.grid.getRows().indexOf(row)
              : this.parent.getRows().indexOf(row);
        }
      }
      ...
      if (!arg.cancel) {
        if (
          row.rowIndex === this.parent.getCurrentViewRecords().length - 1 &&
          this.keyPress === 'tab'
        ) {
        ...
        if (
          ...
          if (isCountRequired(this.parent)) {
            ...
            const editArgs: ActionEventArgs = {
              requestType: 'save',
              data: args.rowData,
              action: 'edit',
              row: row,
            ...
            this.parent.grid
              .getDataModule()
              [`${eventPromise}`](editArgs, this.parent.grid.query);
          } else {
          ...
          if (this.parent['isGantt'] && this.parent.loadChildOnDemand) {
            ...
            crud = <Promise<Object>>(
              (this.parent.grid.dataSource as DataManager).update(
                primaryKeys[0],
                args.rowData,
                query.fromTable,
                query,
  ...
  private afterCellSave(args: CellSaveArgs, row: HTMLTableRowElement): void {
    ...
    this.parent.grid.editModule.destroyWidgets([
      this.parent.grid.getColumnByField(args.columnName),
    ]);
    ...
    removeClass(row.querySelectorAll('.e-rowcell'), [
      'e-editedbatchcell',
      'e-updatedtd',
    ]);
    ...
    editAction(
      { value: <ITreeData>args.rowData, action: 'edit' },
      this.parent,
    ...
    const saveArgs: CellSaveEventArgs = {
      ...
      row: row,
      target: args.cell as HTMLElement,
    };
    if (
      this.parent.aggregates.map(
        (ag: AggregateRow) => ag.showChildSummary === true
      ).length
    ) {
  ...
  private updateCell(args: CellSaveArgs, rowIndex: number): void {
    this.parent.grid.editModule.updateCell(
      rowIndex,
      args.columnName,
      args.rowData[args.columnName]
    );
    this.parent.grid.getRowsObject()[parseInt(rowIndex.toString(), 10)].data =
      args.rowData;
  }
  private crudAction(
    details: { value: ITreeData; action: string },
    columnName?: string
  ): void {
    editAction(
      details,
    ...
    const data: Object =
      this.parent.grid.dataSource instanceof DataManager
    ...
    for (let i: number = 0; i < (<Object[]>data).length; i++) {
      ...
      if (
        details.value[`${key}`] === data[parseInt(i.toString(), 10)][`${key}`]
      ) {
        if (details.action === 'add') {
          data[parseInt(i.toString(), 10)].level =
            this.internalProperties.level;
          data[parseInt(i.toString(), 10)].taskData =
            this.internalProperties.taskData;
          data[parseInt(i.toString(), 10)].uniqueID =
            this.internalProperties.uniqueID;
          if (!isNullOrUndefined(this.internalProperties.parentItem)) {
            data[parseInt(i.toString(), 10)].parentItem =
              this.internalProperties.parentItem;
            data[parseInt(i.toString(), 10)].parentUniqueID =
              this.internalProperties.parentUniqueID;
          }
          data[parseInt(i.toString(), 10)].childRecords =
            this.internalProperties.childRecords;
        }
      }
      setValue(
        'uniqueIDCollection.' +
      ...
      const adaptor: AdaptorOptions = (this.parent.dataSource as DataManager)
        .adaptor;
      if (isRemoteData(this.parent) || adaptor instanceof RemoteSaveAdaptor) {
        setValue(
          'uniqueIDCollection.' + data[parseInt(i.toString(), 10)].uniqueID,
    ...
    if (!this.parent.enableInfiniteScrolling) {
      if (details.action === 'add' && this.previousNewRowPosition != null) {
        this.parent.setProperties(
          { editSettings: { newRowPosition: this.previousNewRowPosition } },
          true
        );
        this.previousNewRowPosition = null;
      }
  ...
  private updateIndex(data: Object, rows: Object, records: Object): void {
    for (let j: number = 0; j < this.parent.getDataRows().length; j++) {
      ...
      if (!isNullOrUndefined(data1)) {
        const index: number = getValue(
          'uniqueIDCollection.' + data1.uniqueID + '.index',
          this.parent
        );
        data1.index = index;
        if (!isNullOrUndefined(data1.parentItem)) {
          const parentIndex: number = getValue(
            'uniqueIDCollection.' + data1.parentItem.uniqueID + '.index',
            this.parent
          );
    ...
    if (this.parent.getFrozenColumns() > 0) {
      const cells: NodeListOf<Element> = (rows[0] as Element).querySelectorAll(
        '.e-rowcell'
      );
      for (let l: number = 0; l < cells.length; l++) {
        if (
          cells[parseInt(l.toString(), 10)].classList.contains(
            'e-gridrowindex0level0'
          )
        ) {
          treeColIndex = l;
          break;
    ...
    for (let k: number = 0; k < this.parent.getRows().length; k++) {
      ...
      if (!isNullOrUndefined(data2)) {
        ...
        if (!isNullOrUndefined(data2.parentItem)) {
          index = getValue(
            'uniqueIDCollection.' + data2.parentItem.uniqueID + '.index',
            this.parent
          );
        }
        const treecell: HTMLElement = (row as HTMLTableRowElement).cells[
          parseInt(treeColIndex.toString(), 10)
        ];
        if (!isNullOrUndefined(treecell)) {
          for (let l: number = 0; l < treecell.classList.length; l++) {
            const value: string =
              treecell.classList[parseInt(l.toString(), 10)];
          ...
          if (
            !rows[parseInt(k.toString(), 10)].classList.contains('e-detailrow')
          ) {
            addClass([treecell], 'e-gridrowindex' + index + 'level' + level);
          } else {
            addClass(
              [treecell],
              'e-griddetailrowindex' + index + 'level' + level
            );
          }
        }
      }
  ...
  private beginAdd(): void {
    ...
    const firstAriaIndex: number = rows.length
      ? +rows[0].getAttribute('data-rowindex')
      : 0;
    const lastAriaIndex: number = rows.length
      ? +rows[rows.length - 1].getAttribute('data-rowindex')
      : 0;
    const withinRange: boolean =
      this.selectedIndex >= firstAriaIndex &&
      this.selectedIndex <= lastAriaIndex;
    const isVirtualization: boolean =
      this.parent.enableVirtualization &&
      this.addRowIndex > -1 &&
      this.prevAriaRowIndex !== '-1';
    if (this.parent.editSettings.mode !== 'Dialog') {
      if (this.parent.editSettings.newRowPosition === 'Above') {
        ...
        (this.parent.editSettings.newRowPosition === 'Below' ||
          this.parent.editSettings.newRowPosition === 'Child') &&
        (this.selectedIndex > -1 || isVirtualization) &&
        withinRange
      ) {
        position = 'after';
        if (
          !isNullOrUndefined(records[parseInt(index.toString(), 10)]) &&
          ...
          if (
            ...
            index += findChildrenRecords(
              records[parseInt(index.toString(), 10)]
            ).length;
            if (this.parent.editSettings.newRowPosition !== 'Child') {
              const batchChildCount: number =
      ...
      if (
        ...
        const r: string = 'rows';
        const newRowObject: Row<Column> =
          this.parent.grid.contentModule[`${r}`][0];
        const focussedElement: HTMLInputElement = <HTMLInputElement>(
          document.activeElement
        );
        rows[index + 1][`${position}`](rows[0]);
        setValue('batchIndex', index + 1, this.batchEditModule);
        const rowObjectIndex: number =
          this.parent.editSettings.newRowPosition === 'Above'
            ? index
            : index + 1;
        if (this.parent.editSettings.mode === 'Batch') {
          this.parent.grid.contentModule[`${r}`].splice(0, 1);
          this.parent.grid.contentModule[`${r}`].splice(
            rowObjectIndex,
            0,
            newRowObject
          );
        ...
        if (
          this.parent.editSettings.mode === 'Row' ||
          this.parent.editSettings.mode === 'Cell'
        ) {
          const errors: NodeListOf<Element> = this.parent.grid
            .getContentTable()
            .querySelectorAll('.e-griderror');
        ...
        if (
          !this.parent.enableVirtualization ||
          (this.parent.enableVirtualization &&
            !Object.keys(this.parent.grid.contentModule['emptyRowData']).length)
        ) {
    ...
    if (
      ...
      index = this.batchEditModule.getAddRowIndex();
      this.selectedIndex = this.batchEditModule.getSelectedIndex();
      const batchAddedRecords: ITreeData[] =
        this.parent.getBatchChanges()['addedRecords'];
      let newlyAddedRecord: ITreeData;
      if (batchAddedRecords.length) {
        for (let i: number = 0; i < batchAddedRecords.length; i++) {
          if (
            isNullOrUndefined(
              batchAddedRecords[parseInt(i.toString(), 10)].uniqueID
            )
          ) {
      ...
      this.batchEditModule['batchAddRowRecord'].push(
        this.batchEditModule['addRowRecord']
      );
  ...
  private beginEdit(args: SaveEventArgs): void {
    ...
    if (args.requestType === 'delete') {
      ...
      if (isNullOrUndefined((args.data[0] as ITreeData).uniqueID)) {
        ...
        for (let i: number = 0; i < data.length; i++) {
          this.parent.flatData.filter((e: ITreeData) => {
            if (
              e[`${primaryKeys[0]}`] ===
              args.data[parseInt(i.toString(), 10)][primaryKeys[0]]
            ) {
      ...
      for (let i: number = 0; i < data.length; i++) {
        ...
        const childs: ITreeData[] = findChildrenRecords(
          data[parseInt(i.toString(), 10)]
        );
    ...
    if (
      ...
      if (
        (!(
          ...
          args.index === this.parent.grid.selectedRowIndex) ||
        args.index === 0
      ) {
      ...
      if (this.parent.enableVirtualization) {
        let selector: string =
          '.e-row[data-rowindex="' + this.selectedIndex + '"]';
        let row: HTMLTableRowElement;
        if (
          this.selectedIndex > -1 &&
          this.parent.editSettings.newRowPosition !== 'Top' &&
          ...
          row = <HTMLTableRowElement>(
            this.parent.getContent().querySelector(selector)
          );
        ...
        if (
          ...
          this.addRowIndex =
            this.parent.grid.selectedRowIndex > -1
              ? this.parent.grid.selectedRowIndex
              : 0;
        }
      }
      ...
      if (
        ...
        this.addRowRecord =
          this.parent.flatData[this.parent.grid.selectedRowIndex];
        if (this.parent.enableVirtualization && this.isAddedRowByContextMenu) {
          this.addRowRecord =
            this.parent.getCurrentViewRecords()[this.addRowIndex];
        }
    ...
    if (
      isNullOrUndefined(this.addRowRecord) &&
  ...
  private beginAddEdit(args: SaveEventArgs): SaveEventArgs {
    ...
    if (args.action === 'add') {
      ...
      value.taskData = isNullOrUndefined(value.taskData)
        ? extend({}, args.data)
        : value.taskData;
      ...
      let level: number = 0;
      let idMapping: Object;
      let parentUniqueID: string;
      let parentItem: Object;
      let parentIdMapping: string;
      const isVirtualization: boolean =
        this.parent.enableVirtualization &&
        this.addRowIndex > -1 &&
        this.prevAriaRowIndex !== '-1';
      ...
      const firstAriaIndex: number = rows.length
        ? currentData.indexOf(currentData[0])
        : 0;
      const lastAriaIndex: number = rows.length
        ? +rows[rows.length - 1].getAttribute('data-rowindex')
        : 0;
      const withinRange: boolean =
        this.parent.enableVirtualization && args.index !== 0
          ? true
          : this.selectedIndex >= firstAriaIndex &&
            this.selectedIndex <= lastAriaIndex;
      if (currentData.length) {
        ...
        parentIdMapping =
          currentData[this.addRowIndex][this.parent.parentIdMapping];
      ...
      if (
        ...
        if (this.parent.editSettings.newRowPosition === 'Above') {
          ...
          position = 'after';
          const childRecordCount: number = findChildrenRecords(
            currentData[this.addRowIndex]
          ).length;
          const currentDataIndex: number = currentData[this.addRowIndex].index;
          index =
            childRecordCount > 0
              ? currentDataIndex + childRecordCount
              : currentDataIndex;
          ...
          const childRecordCount1: number = findChildrenRecords(
            currentData[this.addRowIndex]
          ).length;
          ...
          index =
            childRecordCount1 > 0
              ? currentDataIndex1 + childRecordCount1
              : currentDataIndex1;
          if (this.isSelfReference) {
            if (
              !this.parent.isLocalData &&
              this.parent.editModule.selectedIndex === -1
            ) {
              value.taskData[this.parent.parentIdMapping] = value[
        ...
        if (this.parent.editSettings.newRowPosition === 'Bottom') {
          level = 0;
          const dataSource: Object[] = <Object[]>(
            (this.parent.grid.dataSource instanceof DataManager
              ? this.parent.grid.dataSource.dataSource.json
              : this.parent.grid.dataSource)
          );
    ...
    if (args.action === 'add') {
      this.internalProperties = {
        ...
        parentUniqueID: isNullOrUndefined(value.parentItem)
          ? undefined
          : value.parentItem.uniqueID,
    ...
    if (args.requestType === 'delete') {
      ...
      for (let i: number = 0; i < deletedValues.length; i++) {
        if (deletedValues[parseInt(i.toString(), 10)].parentItem) {
          const parentItem: ITreeData = getParentData(
            this.parent,
            deletedValues[parseInt(i.toString(), 10)].parentItem.uniqueID
          );
          if (!isNullOrUndefined(parentItem) && parentItem.hasChildRecords) {
            const childIndex: number = parentItem.childRecords.indexOf(
              deletedValues[parseInt(i.toString(), 10)]
            );
  ...
  public addRecord(
    ...
    if (
      ...
      Object.hasOwnProperty.call(data, this.parent.childMapping)
    ) {
      ...
      if (!isNullOrUndefined(position)) {
        this.parent.setProperties(
          { editSettings: { newRowPosition: position } },
          true
        );
      }
      const updatedRecords: BatchChanges = {
      ...
      this.parent.setProperties(
        { editSettings: { mode: previousEditMode } },
        true
      );
      this.parent.grid.setProperties(
        { editSettings: { mode: previousGridEditMode } },
        true
      );
      this.parent.refresh();
    } else {
  ...
  private contentready(e: { rows: Row<Column>[]; args?: NotifyArgs }): void {
    if (
      !isNullOrUndefined(e.args.requestType) &&
      (e.args.requestType.toString() === 'delete' ||
        e.args.requestType.toString() === 'save' ||
        (this.parent.editSettings.mode === 'Batch' &&
          e.args.requestType.toString() === 'batchsave'))
    ) {
      this.updateIndex(
        this.parent.grid.dataSource,
        this.parent.getRows(),
      ...
      if (
        ...
        this.parent.frozenColumns
      ) {
        if (
          (this.parent.grid.dataSource as Object[]).length ===
          this.parent.getDataRows().length
        ) {
          this.updateIndex(
            this.parent.grid.dataSource,
