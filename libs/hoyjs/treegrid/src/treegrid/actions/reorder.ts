...
import { getObject, Grid, Reorder as GridReorder } from '@hoyui/grids';
...
export class Reorder {
  ...
  private getTreeColumn(): void {
    ...
    const treeColumn: Column | string | ColumnModel =
      this.parent[`${columnModel}`][this.parent.treeColumnIndex];
    ...
    for (let f: number = 0; f < updatedCols.length; f++) {
      ...
      const parentColumnfield: string = getObject(
        'field',
        updatedCols[parseInt(f.toString(), 10)]
      );
