...
export class Logger extends GridLogger {
  ...
  public log(types: string | string[], args: Object): void {
    ...
    for (let i: number = 0; i < type.length; i++) {
      ...
      if (cOp.success) {
        let message: string = item.generateMessage(
          args,
          this.parent,
          cOp.options
        );
        message = message
          .replace('EJ2Grid', 'EJ2TreeGrid')
          .replace('* Hierarchy Grid', '')
          .replace('* Grouping', '');
        if (
          IsRowDDEnabled &&
          type[parseInt(i.toString(), 10)] === 'primary_column_missing'
        ) {
        ...
        if (type[parseInt(i.toString(), 10)] === 'module_missing') {
          ...
          type[parseInt(i.toString(), 10)] === 'primary_column_missing' ||
          type[parseInt(i.toString(), 10)] === 'selection_key_missing'
        ) {
          message = message.replace(
            gridurl,
            BASE_DOC_URL + '/api/treegrid/column/#isprimarykey'
          );
  ...
  public treeLog(
    ...
    if (
      ...
      !(<Column[]>treeGrid.columns).filter(
    ...
    for (let i: number = 0; i < type.length; i++) {
      const item: TreeItemDetails =
        treeGridDetails[type[parseInt(i.toString(), 10)]];
      const cOp: CheckOptions = item.check(args, treeGrid);
      if (cOp.success) {
        const message: string = item.generateMessage(
          args,
          treeGrid,
          cOp.options
        );
...
export const treeGridDetails: { [key: string]: TreeItemDetails } = {
  ...
  mapping_fields_missing: {
    ...
    check(args: Object, parent: TreeGrid): CheckOptions {
      ...
      if (
        (isNullOrUndefined(parent.idMapping) &&
          isNullOrUndefined(parent.childMapping) &&
