...
import { getObject, Filter as GridFilter, Grid } from '@hoyui/grids';
import { isNullOrUndefined, setValue, getValue } from '@hoyui/base';
...
export class Filter {
  ...
  private updatedFilteredRecord(dataDetails: { data: Object }): void {
    ...
    for (let f: number = 0; f < this.flatFilteredData.length; f++) {
      ...
      const hierarchyMode: FilterHierarchyMode =
        this.parent.grid.searchSettings.key === ''
          ? this.parent.filterSettings.hierarchyMode
          : this.parent.searchSettings.hierarchyMode;
      if (
        (hierarchyMode === 'Child' || hierarchyMode === 'None') &&
      ...
      if (!isNullOrUndefined(parent)) {
        const parRecord: ITreeData = getParentData(
          this.parent,
          rec.parentItem.uniqueID,
          true
        );
  ...
  private updateParentFilteredRecord(record: ITreeData): void {
    const parRecord: ITreeData = getParentData(
      this.parent,
      record.parentItem.uniqueID,
      true
    );
    const uniqueIDValue: Object = getValue(
      'uniqueIDFilterCollection',
      this.parent
    );
    if (
      parRecord &&
      Object.prototype.hasOwnProperty.call(uniqueIDValue, parRecord.uniqueID)
    ) {
      setValue('hasFilteredChildRecords', true, parRecord);
    }
    if (parRecord && parRecord.parentItem) {
      ...
      if (isNullOrUndefined(parent)) {
        if (this.flatFilteredData.indexOf(record) !== -1) {
          if (this.filteredResult.indexOf(record) === -1) {
            ...
            setValue(
              'uniqueIDFilterCollection.' + record.uniqueID,
              record,
              this.parent
            );
        ...
        if (
          this.flatFilteredData.indexOf(parent) !== -1 ||
          this.filteredResult.indexOf(parent) !== -1
        ) {
          if (this.filteredResult.indexOf(record) === -1) {
            this.filteredResult.push(record);
            setValue(
          ...
          if (
            this.filteredResult.indexOf(record) === -1 &&
            this.flatFilteredData.indexOf(record) !== -1
          ) {
            this.filteredResult.push(record);
            setValue(
              'uniqueIDFilterCollection.' + record.uniqueID,
    ...
    for (let count: number = 0; count < childRec.length; count++) {
      ...
      const hierarchyMode: FilterHierarchyMode =
        this.parent.grid.searchSettings.key === ''
          ...
          : this.parent.searchSettings.hierarchyMode;
      if (
        (hierarchyMode === 'Child' || hierarchyMode === 'Both') &&
        (this.parent.grid.filterSettings.columns.length !== 0 ||
          this.parent.grid.searchSettings.key !== '')
      ) {
        const uniqueIDValue: Object = getValue(
        ...
        if (
          !Object.prototype.hasOwnProperty.call(
            uniqueIDValue,
            childRec[parseInt(count.toString(), 10)].uniqueID
          )
        ) {
          ...
          setValue(
            'uniqueIDFilterCollection.' +
              childRec[parseInt(count.toString(), 10)].uniqueID,
            childRec[parseInt(count.toString(), 10)],
            this.parent
          );
          isExist = true;
        }
  ...
  private updateFilterLevel(): void {
    ...
    for (let c: number = 0; c < len; c++) {
      const parent: ITreeData = getParentData(
        this.parent,
        record[parseInt(c.toString(), 10)].parentUniqueID
      );
  ...
  private clearFilterLevel(data: { flatData: Object[] }): void {
    ...
    for (count; count < len; count++) {
      ...
      if (
        fLevel ||
        fLevel === 0 ||
        !isNullOrUndefined(currentRecord.hasFilteredChildRecords)
      ) {
