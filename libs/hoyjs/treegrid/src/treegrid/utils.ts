import { getObject, IGrid } from '@hoyui/grids';
...
import { WebApiAdaptor, WebMethodAdaptor, CacheAdaptor } from '@hoyui/data';
...
export function isRemoteData(parent: TreeGrid): boolean {
  if (parent.dataSource instanceof DataManager) {
    ...
    return (
      ...
      adaptor instanceof WebMethodAdaptor ||
      adaptor instanceof CacheAdaptor ||
...
export function isCheckboxcolumn(parent: TreeGrid): boolean {
  for (let i: number = 0; i < parent.columns.length; i++) {
    if (
      (parent.columns[parseInt(i.toString(), 10)] as ColumnModel).showCheckbox
    ) {
      return true;
    }
  }
...
export function isFilterChildHierarchy(parent: TreeGrid): boolean {
  if (
    (!isNullOrUndefined(parent.grid.searchSettings.key) &&
      parent.grid.searchSettings.key !== '' &&
      (parent.searchSettings.hierarchyMode === 'Child' ||
        parent.searchSettings.hierarchyMode === 'None')) ||
    (parent.allowFiltering &&
      parent.grid.filterSettings.columns.length &&
      (parent.filterSettings.hierarchyMode === 'Child' ||
        parent.filterSettings.hierarchyMode === 'None'))
  ) {
    return true;
  }
  return false;
...
export function findParentRecords(records: Object): Object {
  ...
  for (let i: number = 0, len: number = recordsLength; i < len; i++) {
    const hasChild: boolean = getObject(
      'hasChildRecords',
 ...
 * @hidden
 */
export function getExpandStatus(
  parent: TreeGrid,
  record: ITreeData,
  parents: ITreeData[]
): boolean {
  const parentRecord: ITreeData = isNullOrUndefined(record.parentItem)
    ? null
    : getParentData(parent, record.parentItem.uniqueID);
  let childParent: ITreeData;
  if (parentRecord != null) {
    if (
      parent.initialRender &&
      !isNullOrUndefined(parentRecord[parent.expandStateMapping]) &&
      ...
      if (
        childParent &&
        parent.initialRender &&
        !isNullOrUndefined(childParent[parent.expandStateMapping]) &&
...
export function findChildrenRecords(records: ITreeData): Object[] {
  ...
  if (
    ...
    (!records.hasChildRecords &&
      !isNullOrUndefined(records.childRecords) &&
      !records.childRecords.length)
  ) {
    return [];
  }
  if (!isNullOrUndefined(records.childRecords)) {
    const childRecords: ITreeData[] = records.childRecords.filter(
      (item: ITreeData) => !item.isSummaryRow
    );
    ...
    for (let i: number = 0, len: number = keys.length; i < len; i++) {
      ...
      if (
        childRecords[parseInt(i.toString(), 10)].hasChildRecords ||
        (!isNullOrUndefined(
        ...
        datas = [
          ...datas,
          ...findChildrenRecords(childRecords[parseInt(i.toString(), 10)]),
        ];
      }
    }
  }
...
export function extendArray(array: Object[]): Object[] {
  ...
  for (let i: number = 0; array && i < array.length; i++) {
    ...
    for (let j: number = 0; j < keys.length; j++) {
      obj[keys[parseInt(j.toString(), 10)]] =
        array[parseInt(i.toString(), 10)][keys[parseInt(j.toString(), 10)]];
    }
    objArr.push(obj);
  }
  return objArr;
