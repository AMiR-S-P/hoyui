...
gulp.task('scripts', function (done) {
  ...
  var tsResult = gulp
    ...
    .pipe(tsProject());
  tsResult.js.pipe(gulp.dest('./')).on('end', function () {
    done();
  });
});
...
gulp.task('test', function (done) {
  ...
  if (
    packageJson.dependencies['@hoyui/data'] ||
    packageJson.name === '@hoyui/data'
  ) {
    console.log('Service Started');
    var spawn = require('child_process').spawn;
    service = spawn('node', [
      path.join(__dirname, '/spec/services/V4service.js'),
    ]);
