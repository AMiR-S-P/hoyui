...
import { isNullOrUndefined, select } from '@hoyui/base';
...
describe('Batch Edit module', () => {
  ...
  describe('Hierarchy - Batch Add', () => {
    ...
    it('Add - Batch Editing', (done: Function) => {
      ...
      expect(gridObj.getRowByIndex(0).classList.contains('e-insertedrow')).toBe(
        true
      );
      (
        gridObj.element
      ...
      select(
        '#' + gridObj.element.id + '_gridcontrol' + 'EditConfirm',
        gridObj.element
      )
...
describe('Hierarchy - Batch Add NewRowPosition Below', () => {
  ...
  it('Add - Batch Editing', (done: Function) => {
    ...
    expect(gridObj.getRowByIndex(2).classList.contains('e-insertedrow')).toBe(
      true
    );
    (
      gridObj.element
...
describe('Hierarchy - Batch Add NewRowPosition Child', () => {
  ...
  it('Add - Batch Editing', (done: Function) => {
    actionComplete = (args?: Object): void => {
      if (args['requestType'] === 'batchSave') {
        expect(gridObj.dataSource[0].subtasks[0].subtasks.length === 1).toBe(
          true
        );
        expect(
          gridObj.getRowByIndex(1).querySelectorAll('.e-treegridexpand')
            .length === 1
        ).toBe(true);
      }
      done();
...
describe('Hierarchy - Random action checking', () => {
  ...
  it('Add - Batch Editing', (done: Function) => {
    ...
    expect(gridObj.getRowByIndex(6).classList.contains('e-insertedrow')).toBe(
      true
    );
    (
      gridObj.element
...
describe('Hierarchy - Batch cancel checking', () => {
  ...
  it('Add - Batch Editing', (done: Function) => {
    ...
    expect(gridObj.getRowByIndex(4).classList.contains('e-insertedrow')).toBe(
      true
    );
    (
      gridObj.element
...
describe('Hierarchy - gotopage delete and add', () => {
  ...
  it('Add - Batch Editing', (done: Function) => {
    actionComplete = (args?: Object): void => {
      if (args['requestType'] === 'batchSave') {
        expect(
          gridObj.dataSource[2].subtasks[1].subtasks[0].subtasks[6] === 42
        ).toBe(true);
      }
      done();
    };
...
describe('Hirarchy editing - Batch Edit with expand/collapse request', () => {
  ...
  it('collapse record', () => {
    ...
    gridObj[method](
      gridObj.getRowByIndex(0).querySelector('.e-treegridexpand')
    );
    expect(gridObj.getBatchChanges()['changedRecords'].length === 1).toBe(true);
    select(
      '#' + gridObj.element.id + '_gridcontrol' + 'EditConfirm',
...
describe('FlatData - Batch Add NewRowPosition Above', () => {
  ...
  it('Add - Batch Editing', (done: Function) => {
    ...
    expect(gridObj.getRowByIndex(1).classList.contains('e-insertedrow')).toBe(
      true
    );
    (
      gridObj.element
...
describe(' Batch  Delete cancel checking', () => {
  ...
  it('Batch Delete cancel action ', () => {
    ...
    select(
      ...
      .querySelectorAll('button')[0]
      .click();
    expect(gridObj.getCurrentViewRecords()[0][childRecords].length === 4).toBe(
      true
    );
  });
  afterAll(() => {
...
describe('Batch Editing - validation checking for in-between position', () => {
  ...
  it('validation rule', () => {
    ...
    expect(gridObj.element.querySelectorAll('.e-griderror').length === 1).toBe(
      true
    );
  });
  afterAll(() => {
...
describe('EJ2-70058 -When rowdd is enabled with batch edit mode, the expand/collapse icon is not shown for the edited cell', () => {
  ...
  it('Batch edit with rowDragandDrop', (done: Function) => {
    ...
    gridObj.grid.keyboardModule.keyAction({
      action: 'enter',
      preventDefault: preventDefault,
      target: (<any>gridObj.getContent().querySelector('.e-row')).cells[2],
    } as any);
    expect(
      gridObj
        .getRows()[0]
...
describe('coverage improvement', () => {
  ...
  it('Delete - collapse action', (done: Function) => {
    ...
    (
      gridObj.getRows()[0].querySelector('.e-treegridexpand') as HTMLElement
    ).click();
    (<any>gridObj.grid.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.grid.element.id + '_update' },
    });
...
describe('Code coverage improment', () => {
  ...
  it('update cell', (done: Function) => {
    ...
    select(
      '#' + treegrid.element.id + '_gridcontrol' + 'EditConfirm',
      treegrid.element
    )
      .querySelectorAll('button')[0]
      .click();
