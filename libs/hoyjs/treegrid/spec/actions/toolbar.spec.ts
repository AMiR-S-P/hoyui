...
import { isNullOrUndefined, createElement } from '@hoyui/base';
...
describe('TreeGrid Toolbar module', () => {
  ...
  describe('Toolbar string', () => {
    ...
    it('Toolbars string', () => {
      let toolbarElements: Element =
        gridObj.toolbarModule.getToolbar().firstElementChild;
      expect(
        toolbarElements
  ...
  describe('Script error throws in inline editing', () => {
    ...
    it('Script Error', (done: Function) => {
      ...
      (select('#' + gridObj.grid.element.id + 'taskID', formEle) as any).value =
        '124';
      (
        select('#' + gridObj.grid.element.id + 'taskName', formEle) as any
      ).value = 'fourth';
      (
        select('#' + gridObj.grid.element.id + 'startDate', formEle) as any
      ).value = '2/3/2017';
      (<any>gridObj.grid.toolbarModule).toolbarClickHandler({
        item: { id: gridObj.grid.element.id + '_update' },
      });
  ...
  describe('Toolbar enum', () => {
    ...
    it('click events', () => {
      (<HTMLElement>(
        select(
          '#' + gridObj.grid.element.id + '_collapseall',
      ...
      (<HTMLElement>(
        select(
          '#' + gridObj.grid.element.id + '_expandall',
          gridObj.toolbarModule.getToolbar()
        )
      )).click();
      expect((<HTMLTableRowElement>gridObj.getRows()[1]).style.display).toBe(
    ...
    it('setmodel', () => {
      ...
      expect(
        gridObj.toolbarModule
          .getToolbar()
          .firstElementChild.querySelector('.e-tbar-btn-text').textContent
      ).toBe('testToolbar');
    });
    afterAll(() => {
      destroy(gridObj);
  ...
  describe('Toolbar Module test', () => {
    ...
    it('methods', () => {
      ...
      gridObj.toolbarModule.enableItems(
        [gridObj.element.id + '_gridcontrol_expandall'],
        false
      );
      expect(
        tool.firstElementChild.firstElementChild.classList.contains('e-overlay')
      ).toBeTruthy();
    });
    afterAll(() => {
      destroy(gridObj);
  ...
  describe('EJ2-57737 - Indent/Outdent Toolbar icon based on rowselection and deselection', () => {
    ...
    it('Checking Indent/Outdent Toolbar on Initial rendering', (done: Function) => {
      expect(
        TreeGridObj.toolbarModule
          .getToolbar()
          .querySelector('#' + TreeGridObj.element.id + '_gridcontrol_indent')
          .parentElement.classList.contains('e-hidden')
      ).toBe(true);
      expect(
        TreeGridObj.toolbarModule
          .getToolbar()
          .querySelector('#' + TreeGridObj.element.id + '_gridcontrol_outdent')
          .parentElement.classList.contains('e-hidden')
      ).toBe(true);
      done();
    });
    it('Checking Indent/Outdent Toolbar when selecting the row', (done: Function) => {
      rowSelected = (): void => {
        ...
        expect(
          tool.firstElementChild.lastElementChild.classList.contains(
            'e-overlay'
          ) === false
        ).toBe(true);
        done();
      ...
      rowDeselected = (): void => {
        ...
        expect(
          TreeGridObj.toolbarModule
            .getToolbar()
            .querySelector(
              '#' + TreeGridObj.element.id + '_gridcontrol_outdent'
            )
            .parentElement.classList.contains('e-hidden')
        ).toBe(true);
        done();
...
describe('Newly added bottom record get indented', () => {
  ...
  it('Indent', (done: Function) => {
    ...
    (select('#' + gridObj.grid.element.id + 'taskID', formEle) as any).value =
      '124';
    (select('#' + gridObj.grid.element.id + 'taskName', formEle) as any).value =
      'fourth';
    (<any>gridObj.grid.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.grid.element.id + '_update' },
    });
