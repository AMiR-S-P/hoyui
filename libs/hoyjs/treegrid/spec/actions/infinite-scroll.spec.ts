...
import { QueryCellInfoEventArgs, RowSelectEventArgs, Grid } from '@hoyui/grids';
...
const filterColumn: Function = (
  ...
  const filterElement: any = gridObj.element.querySelector(
...
describe('TreeGrid Infinite Scroll', () => {
  describe('Rendering and basic actions', () => {
    ...
    it('initial render', () => {
      ...
      expect(
        parseInt(treegrid.getRows()[0].getAttribute('data-rowindex'), 10)
      ).toBe(0);
      ...
      expect(
        Object.keys((treegrid.grid.infiniteScrollModule as any).infiniteCache)
          .length
      ).toBe(0);
      expect(
        Object.keys(
          (treegrid.grid.infiniteScrollModule as any).infiniteCurrentViewData
        ).length
      ).toBe(treegrid.grid.infiniteScrollSettings.initialBlocks);
    });
    it('collapse test', () => {
    ...
    it('scroll bottom', () => {
      ...
      expect(
        parseInt(treegrid.getRows()[150].getAttribute('data-rowindex'), 10)
      ).toBe(150);
      ...
      ).toBe(treegrid.infiniteScrollSettings.initialBlocks + 1);
    });
    it('Filter string column testing', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'filtering') {
          expect(
            checkFilterObj(
              treegrid.grid.filterSettings.columns[0],
              'FIELD1',
              'startswith',
              'VINET',
              'and',
  ...
  describe('Add New Row', () => {
    ...
    it('Add New Row Begin', (done: Function) => {
      actionComplete = (args?: any): void => {
        if (args.requestType === 'add') {
          expect(
            treegrid.grid.element.querySelectorAll('.e-addedrow').length
          ).toBe(1);
          expect(
            treegrid.grid.element.querySelectorAll('.e-normaledit').length
          ).toBe(1);
          expect(
            treegrid.grid.element.querySelectorAll('.e-gridform').length
          ).toBe(1);
          expect(treegrid.grid.element.querySelectorAll('form').length).toBe(1);
          expect(document.activeElement.id).toBe(
            treegrid.grid.element.id + 'TaskID'
          );
          //toolbar status check
          expect(
            treegrid.grid.element.querySelectorAll('.e-overlay').length
          ).toBe(4);
      ...
      (<any>treegrid.grid.toolbarModule).toolbarClickHandler({
        item: { id: treegrid.grid.element.id + '_add' },
      });
    });
    ...
    it('Save New Row', (done: Function) => {
      ...
      (
        select(
          '#' + treegrid.grid.element.id + 'TaskID',
          treegrid.grid.element
        ) as any
      ).value = '98765';
      (
        select(
          '#' + treegrid.grid.element.id + 'FIELD1',
          treegrid.grid.element
        ) as any
      ).value = 'New Row';
      (<any>treegrid.grid.toolbarModule).toolbarClickHandler({
        item: { id: treegrid.grid.element.id + '_update' },
      });
    });
    afterAll(() => {
      destroy(treegrid);
  ...
  describe('Delete Row', () => {
    ...
    it('Delete First Parent Row', (done: Function) => {
      ...
      (<any>treegrid.grid.toolbarModule).toolbarClickHandler({
        item: { id: treegrid.grid.element.id + '_delete' },
      });
    });
  ...
  describe('Add New Row', () => {
    ...
    it('Add New Row', (done: Function) => {
      ...
      treegrid.addRecord(
        { TaskID: 10007, FIELD1: 'shk', FIELD2: 1999, FIELD3: 0, FIELD4: 88 },
        0,
        'Below'
      );
