  ...
  describe('Hirarchy editing', () => {
      ...
      actionComplete = (args?: any): void => {
        expect(
          document.getElementById(
            gridObj.grid.element.id + '_dialogEdit_wrapper'
          )
        ).not.toBeNull();
        done();
      };
    ...
    it('edit row - modify data', (done: Function) => {
      ...
      actionComplete = (args?: any): void => {
        ...
        expect(
          gridObj.dataSource[0].subtasks[1].taskName === 'test'
        ).toBeTruthy();
        done();
      };
      gridObj.actionComplete = actionComplete;
...
describe('EJ2-32503 - Grid Column Model is not updated when me made changes in Tree Grid Column', () => {
  ...
  it('Grid Column Model is not updated when me made changes in Tree Grid Column', (done: Function) => {
    actionComplete = (args?: any): void => {
      ...
      expect(
        document
          .getElementById(gridObj.grid.element.id + '_dialogEdit_wrapper')
          .querySelectorAll('label')[2].innerHTML === 'duration'
      ).toBe(true);
