...
import { SaveEventArgs, CellEditArgs } from '@hoyui/grids';
...
describe('Cell Editing - cell alone refresh', () => {
  ...
  it('cell refresh', () => {
    ...
    gridObj.actionComplete = (args?: SaveEventArgs): void => {
      ...
      expect(
        (gridObj.getCellFromIndex(1, 1) as HTMLElement).style.background
      ).toBe('red');
    };
    gridObj.grid.editModule.formObj.element.getElementsByTagName(
      'input'
...
describe('Cell Editing With scroller', () => {
  ...
  it('cell edit', () => {
    ...
    expect(
      (<HTMLElement>gridObj.getContent().firstChild).classList.contains(
        'e-content'
      )
    ).toBeTruthy();
    const event: MouseEvent = new MouseEvent('dblclick', {
    ...
    const scrollPosition: number =
      gridObj.getContent().firstElementChild.scrollLeft;
    gridObj.getCellFromIndex(2, 1).dispatchEvent(event);
    expect(gridObj.getContent().firstElementChild.scrollLeft).toEqual(
      scrollPosition
    );
    (<any>gridObj.grid.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.grid.element.id + '_update' },
      ...
      if (args.type === 'save') {
        expect(
          (
            gridObj
              .getRows()[0]
              .getElementsByClassName('e-treecell')[0] as HTMLElement
          ).innerText === 'Planned'
        ).toBe(true);
        done();
      }
    ...
    actionComplete = (args?: Object): void => {
      ...
      expect(
        gridObj
          .getRows()[2]
          .getElementsByClassName('e-rowcell')[1]
          .querySelector('div>.e-treecell').innerHTML === 'Design Documentation'
      ).toBe(true);
      expect(
        gridObj
          .getRows()[3]
...
describe('Check the expanding state of record after delete operation', () => {
  ...
  it('Check expand state of record after deletion of another record', (done: Function) => {
    (<HTMLElement>(
      gridObj.getRows()[5].querySelectorAll('.e-treegridexpand')[0]
    )).click();
    (<HTMLElement>(
      gridObj.getRows()[11].querySelectorAll('.e-treegridexpand')[0]
    )).click();
    gridObj.selectRow(2);
    (<any>gridObj.grid.toolbarModule).toolbarClickHandler({
    ...
    gridObj.actionComplete = (args?: Object): void => {
      (<HTMLElement>(
        gridObj.getRows()[10].querySelectorAll('.e-treegridcollapse')[0]
      )).click();
      expect(
        gridObj
          .getRows()[11]
          .getElementsByClassName('e-treecolumn-container')[0]
          .children[1].classList.contains('e-treegridexpand')
      ).toBe(true);
      expect(
        gridObj
          .getRows()[12]
          .getElementsByClassName('e-treecolumn-container')[0]
          .children[2].classList.contains('e-treegridexpand')
      ).toBe(true);
      done();
    };
  });
...
describe('Expand/Collpase icon testing at the time of cell edit cancel', () => {
  ...
  it('double click on a record, cell edit and cancel', () => {
    ...
    expect(
      gridObj.getRows()[0].getElementsByClassName('e-treecell').length
    ).toBe(1);
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('EJ2-26550 - Edit cell through method', () => {
  ...
  it('Edit Cell', () => {
    cellEdit = (args?: any): void => {
      setTimeout(() => {
        expect(
          gridObj.grid.editModule.formObj.element.getElementsByTagName('input')
            .length
        ).toBe(1);
      }, 0);
    };
...
describe('EJ2-61461 - Parent Icon disappears when we delete multiple records', () => {
  ...
  it('Delete multiple records ', () => {
    ...
    expect(
      gridObj.grid.getRows()[0].getElementsByClassName('e-treegridexpand')
        .length
    ).toBe(1);
  });
  afterAll(() => {
...
describe('EJ2-32869 - Clicking on Expand icon while in edit state', () => {
  ...
  it('Clicking on expand icon in edit state', () => {
    ...
    (gridObj.editModule as any).doubleClickTarget.getElementsByTagName(
      'input'
    )[0].value = '20';
    (
      gridObj
        .getRows()[0]
        .getElementsByClassName('e-treegridexpand')[0] as HTMLElement
    ).click();
    const cells: NodeListOf<Element> = gridObj.grid
      .getRows()[0]
...
describe('checkbox retained after cell edit and cancel', () => {
  ...
  it('CheckBox rendering', () => {
    ...
    expect(
      gridObj.getRows()[2].getElementsByClassName('e-treecolumn-container')[0]
        .children[4].className
    ).toBe('e-treecell');
  });
  afterAll(() => {
    destroy(gridObj);
  ...
  it('Edit Frozen Cell', () => {
    ...
    expect(
      (
        gridObj
          .getRows()[0]
          .getElementsByClassName('e-rowcell')[0] as HTMLElement
      ).innerText === 'Planning Completed'
    ).toBe(true);
  });
  afterAll(() => {
...
describe('EJ2-57487 - edit the cell using editCell method and press tab key for moving to another cell', () => {
  ...
  it('Edit mode continued to the next cell on tab click', (done: Function) => {
    ...
    expect(gridObj.getRows()[2].getAttribute('data-rowindex') === '2').toBe(
      true
    );
    done();
  });
...
describe('Record double click the edit action Testing', () => {
  ...
  it('Record double click the edit action testing on f2 click', () => {
    gridObj.grid.keyboardModule.keyAction({
      action: 'f2',
      preventDefault: preventDefault,
      target: gridObj.getCellFromIndex(0, 1),
    } as any);
    expect(
      gridObj.getRows()[0].querySelectorAll('.e-treegridexpand').length === 1
    ).toBe(true);
