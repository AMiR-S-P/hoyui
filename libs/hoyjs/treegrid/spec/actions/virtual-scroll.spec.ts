...
import { QueryCellInfoEventArgs, RowSelectEventArgs } from '@hoyui/grids';
  ...
  it('Edit Start in Current View Records', (done: Function) => {
    actionComplete = (args?: any): void => {
      if (args.requestType === 'beginEdit') {
        ...
        expect(gridObj.grid.element.querySelectorAll('.e-overlay').length).toBe(
          4
        );
...
describe('EJ2-63548 - Indent/Outdent action check after edited the row with virtualization ', () => {
  ...
  it('Edit and update', (done: Function) => {
    actionComplete = (args?: any): void => {
      expect(TreeGridObj2.getCurrentViewRecords()[1]['FIELD1']).toEqual(
        'updated'
      );
      done();
    };
  ...
  it('Rendering Test', () => {
    ...
    expect(
      gridObj
        ...
        .querySelector('div>.e-treecell').innerHTML == 'test'
    ).toBe(true);
  });
  afterAll(() => {
    destroy(gridObj);
