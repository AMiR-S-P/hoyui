...
import { CellEditArgs, dataBound } from '@hoyui/grids';
...
describe('Render detailTemplate', () => {
  ...
  it('Render Collapse', () => {
    gridObj.collapseRow(
      gridObj.getRows()[4],
      gridObj.getCurrentViewRecords()[2]
    );
...
describe('Searching detailTemplate', () => {
      ...
      if (args.requestType == 'searching') {
        expect(
          gridObj
            ...
            .querySelector('td')
            .children[0].classList.contains('photo')
        ).toBe(true);
        done();
      }
    };
  ...
  it('Searching Collapse', () => {
    gridObj.collapseRow(
      gridObj.getRows()[2],
      gridObj.getCurrentViewRecords()[1]
    );
    expect((gridObj.getRows()[5] as HTMLTableRowElement).style.display).toBe(
      'none'
    );
  });
  it('Searching expand', () => {
    ...
    actionComplete = (args?: Object): void => {
      expect(
        (
          gridObj
            .getDataRows()[3]
            .getElementsByClassName('e-rowcell')[1] as HTMLElement
        ).innerText == 'Tedd Lawson'
      ).toBe(true);
      expect(
        (
          gridObj
