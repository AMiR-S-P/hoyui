...
import { getObject } from '@hoyui/grids';
...
describe('Treegrid Row Reorder using Indent and Outdent Icon', () => {
  ...
  it('Using Indent Icon and Outdent Icon', () => {
    ...
    expect(
      (TreeGridObj.grid.dataSource[1] as ITreeData).childRecords.length
    ).toBe(1);
    TreeGridObj.selectRow(2);
    (<any>TreeGridObj.grid.toolbarModule).toolbarClickHandler({
...
describe('Treegrid Row Reorder', () => {
  ...
  it('Drop at Above', () => {
    expect(
      (TreeGridObj.grid.dataSource[0] as ITreeData).childRecords.length
    ).toBe(4);
...
describe('EJ2-48275-Issue in Row Drag and Drop of TreeGrid', () => {
  ...
  it('---colapse testing---', (done: Function) => {
    (
      (TreeGridObj.getRows()[0] as HTMLTableRowElement).getElementsByClassName(
        'e-treegridexpand'
      )[0] as HTMLElement
    ).click();
  ...
  it('---expand testing---', (done: Function) => {
    (
      (TreeGridObj.getRows()[2] as HTMLTableRowElement).getElementsByClassName(
        'e-treegridcollapse'
      )[0] as HTMLElement
    ).click();
    expect(
      (TreeGridObj.getRows()[3] as HTMLTableRowElement).style.display
    ).toBe('table-row');
    done();
  });
  afterAll(() => {
...
describe('Treegrid Row Drop as Child', () => {
  ...
  it('Expand Icon Checking', (done: Function) => {
    actionComplete = (args?: any): void => {
      expect(
        (gridObj.getRows()[2] as HTMLTableRowElement).getElementsByClassName(
          'e-treegridexpand'
        ).length
      ).toBe(1);
      done();
  ...
  it('Expand Testing', () => {
    ...
    expect((gridObj.getRows()[3] as HTMLTableRowElement).style.display).toBe(
      'none'
    );
  });
  ...
  describe('childMapping property to newly added row', () => {
    ...
    it('Adding new record', () => {
      (<any>TreeGridObj.grid.toolbarModule).toolbarClickHandler({
        item: { id: TreeGridObj.grid.element.id + '_add' },
      });
      TreeGridObj.grid.editModule.formObj.element.getElementsByTagName(
        'input'
      )[0].value = '99';
      TreeGridObj.grid.editModule.formObj.element.getElementsByTagName(
        'input'
      ...
      (<any>TreeGridObj.grid.toolbarModule).toolbarClickHandler({
        item: { id: TreeGridObj.grid.element.id + '_update' },
      });
...
describe('EJ2-71626- Last row border is not added while drag and drop a row to the last index)', () => {
  ...
  it('Drop the record at bottom using RowDD and checking the last row border', (done: Function) => {
    actionComplete = (args?: any): void => {
      expect(
        (
          gridObj.getVisibleRecords()[
            gridObj.getVisibleRecords().length - 1
          ] as any
        ).taskID
      ).toBe(1);
      expect(
        gridObj.getRows()[31].cells[0].classList.contains('e-lastrowcell')
      ).toBe(true);
      expect(
        gridObj.getRows()[31].cells[7].classList.contains('e-lastrowcell')
      ).toBe(true);
      done();
    };
    gridObj.actionComplete = actionComplete;
...
describe('Drag and drop with in the treegrid', () => {
  ...
  it('coverage improvement single treegrid with data drag and drop', () => {
    ...
    const dragRowElem: Element = TreeGridObj.getRowByIndex(2).querySelector(
      '.e-rowdragdrop.e-rowdragdropcell'
    );
    const dropRowElem: Element = TreeGridObj.getRowByIndex(1).querySelector(
      '.e-rowdragdrop.e-rowdragdropcell'
    );
    ...
    (
      TreeGridObj.grid.rowDragAndDropModule as any
    ).draggable.currentStateTarget = dragRowElem;
    ...
    const dropClone: HTMLElement = TreeGridObj.element.querySelector(
...
describe('Drag and drop with two treegrid', () => {
  ...
  it('coverage improvement multiple treegrid with data drag and drop', () => {
    ...
    dragRowElem.classList.add('e-rowcell');
    (gridObj1.grid.rowDragAndDropModule as any).draggable.currentStateTarget =
      dragRowElem;
