...
import { L10n, select } from '@hoyui/base';
import { CellEditArgs } from '@hoyui/grids';
...
describe('ContextMenu module', () => {
  ...
  describe('Default context menu', () => {
    ...
    it('Context menu property changes', () => {
      (gridObj.grid.contextMenuModule as any).eventArgs = {
      ...
      gridObj.contextMenuItems = [
        { text: 'Expand all', target: '.e-content', id: 'expandall' },
      ];
      gridObj.dataBind();
      expect(
        document
          .getElementById(gridObj.element.id + '_gridcontrol_cmenu')
    ...
    it('Add new row with AddRow - Above', (done: Function) => {
      ...
      actionComplete = (args: CellSaveEventArgs): void => {
        if (args.requestType === 'add') {
          expect(
            (<any>gridObj.getContentTable().querySelectorAll('.e-addedrow')[0])
              .rowIndex === 2
          ).toBe(true);
        }
        done();
      ...
      (gridObj.grid.contextMenuModule as any).eventArgs = {
        target: gridObj.getContentTable().querySelectorAll('tr')[2],
      };
      const e: Object = {
        event: (gridObj.grid.contextMenuModule as any).eventArgs,
        items: gridObj.grid.contextMenuModule.contextMenu.items,
      ...
      (gridObj.grid.contextMenuModule as any).contextMenuOpen();
      document
        .getElementsByClassName('e-menu-item')[10]
      ...
      document
        .getElementsByClassName('e-menu-item')[10]
        .dispatchEvent(mouseEve);
      (<HTMLElement>(
        document.getElementsByClassName('e-menu-parent e-ul ')[0].children[0]
      )).click();
    });
    it('Save the added row', (done: Function) => {
      actionComplete = (args: CellSaveEventArgs): void => {
        if (args.requestType === 'save') {
          expect(
            gridObj.getRows()[2].querySelector('td').innerText === '44'
          ).toBe(true);
        }
        done();
      };
      gridObj.actionComplete = actionComplete;
      (<any>(
        gridObj.getContentTable().getElementsByClassName('e-numerictextbox')[0]
      )).ej2_instances[0].value = '44';
      (<HTMLElement>(
        gridObj.element.getElementsByClassName('e-tbar-btn-text')[2]
      )).click();
    });
    afterAll(() => {
      destroy(gridObj);
  ...
  describe('Add new row with AddRow - Below', () => {
    ...
    it('Add new row with AddRow - Below', (done: Function) => {
      ...
      document
        ...
        .dispatchEvent(mouseEve);
      (<HTMLElement>(
        document.getElementsByClassName('e-menu-parent e-ul ')[0].children[1]
      )).click();
    });
    it('Save the added row', (done: Function) => {
      actionComplete = (args: CellSaveEventArgs): void => {
        if (args.requestType === 'save') {
          expect(
            gridObj.getRows()[3].getElementsByTagName('td')[0].innerText ===
              '55'
          ).toBe(true);
        }
        done();
  ...
  describe('Check whether Edit Record is hidden in Edit Mode set as Cell', () => {
    ...
    it('Check whether Edit Record is hidden in Edit Mode set as Cell', () => {
      ...
      (gridObj.grid.contextMenuModule as any).contextMenuBeforeOpen(e);
      (gridObj.grid.contextMenuModule as any).contextMenuOpen();
      const editRecord: HTMLElement = select(
        '#' + gridObj.element.id + '_gridcontrol_cmenu_Edit',
        document.getElementById(gridObj.element.id + '_gridcontrol_cmenu')
      );
  ...
  describe('Save after cell editing', () => {
    ...
    it('Save the cell edited record with contextmenu save', (done: Function) => {
      ...
      gridObj.actionComplete = actionComplete;
      document
        .getElementById(gridObj.element.id + '_gridcontrol_cmenu_Save')
        .click();
    });
  ...
  describe('Cancel after cell editing', () => {
    ...
    it('Cancel the cell edited record with contextmenu cancel', () => {
      ...
      (gridObj.grid.contextMenuModule as any).contextMenuOpen();
      document
        .getElementById(gridObj.element.id + '_gridcontrol_cmenu_Cancel')
        .click();
      expect(
        gridObj.getRows()[2].querySelectorAll('td')[1].innerText ===
          'Plan budget'
      ).toBe(true);
    });
    afterAll(() => {
  ...
  describe('Localization', () => {
    ...
    it('render testing', () => {
      expect(
        document.getElementsByClassName('e-menu-item')[0].textContent
      ).toBe('Voeg een rij toe');
    });
    afterAll(() => {
      destroy(gridObj);
    });
  ...
  describe('EJ2-49323-Add new row with AddRow - Child', () => {
    ...
    it('Add new row with AddRow - Child', (done: Function) => {
      ...
      (gridObj.grid.contextMenuModule as any).contextMenuOpen();
      document
        .getElementsByClassName('e-menu-item')[0]
        .dispatchEvent(new Event('mouseover'));
      const mouseEve: MouseEvent = document.createEvent('MouseEvents');
      mouseEve.initEvent('mouseover', true, true);
      document.getElementsByClassName('e-menu-item')[0].dispatchEvent(mouseEve);
      (<HTMLElement>(
        document.getElementsByClassName('e-menu-parent e-ul')[0].children[2]
      )).click();
    });
    it('Save the added row', (done: Function) => {
      actionComplete = (args: CellSaveEventArgs): void => {
    ...
    it('Indent Action with contextmenu and ensure outdent disabling ', (done: Function) => {
      ...
      expect(
        document.getElementById(
          gridObj.element.id + '_gridcontrol_cmenu_Outdent'
        ).style.display === 'none'
      ).toBe(true);
      document
        .getElementById(gridObj.element.id + '_gridcontrol_cmenu_Indent')
        .click();
    });
  ...
  describe('EJ2-59747 - Outdent using context menu', () => {
    ...
    it('Outdent Action with contextmenu and ensure indent disabling ', (done: Function) => {
      ...
      expect(
        document.getElementById(
          gridObj.element.id + '_gridcontrol_cmenu_Indent'
        ).style.display === 'none'
      ).toBe(true);
      document
        .getElementById(gridObj.element.id + '_gridcontrol_cmenu_Outdent')
        .click();
    });
    it('DataSource check after record indent using contextmenu', (done: Function) => {
...
describe('Indent/Outdent not shown in context menu of batch edit mode.', () => {
  ...
  it('ensure the oudent action in context menu', () => {
    ...
    gridObj.getCellFromIndex(2, 1).dispatchEvent(event);
    (gridObj.grid.contextMenuModule as any).eventArgs = {
      target: gridObj.getCellFromIndex(2, 1),
    };
    const e: Object = {
      event: (gridObj.grid.contextMenuModule as any).eventArgs,
      items: gridObj.grid.contextMenuModule.contextMenu.items,
    ...
    document
      ...
      .click();
    expect(
      gridObj.getRows()[2].querySelectorAll('td')[1].innerText === 'Plan budget'
    ).toBe(true);
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('Indent/Outdent in first record', () => {
  ...
  it('ensure the oudent action in context menu', () => {
    ...
    (gridObj.grid.contextMenuModule as any).contextMenuOpen();
    expect(
      document.getElementById(gridObj.element.id + '_gridcontrol_cmenu_Indent')
        .style.display === 'none'
    ).toBe(true);
    expect(
      document.getElementById(gridObj.element.id + '_gridcontrol_cmenu_Outdent')
        .style.display === 'none'
    ).toBe(true);
  });
...
describe('Indent/Outdent in parent record', () => {
  ...
  it('ensure the oudent action in parent record', () => {
    gridObj.selectRow(5);
    (gridObj.grid.contextMenuModule as any).eventArgs = {
      target: gridObj.getCellFromIndex(5, 1),
    };
    const e: Object = {
      event: (gridObj.grid.contextMenuModule as any).eventArgs,
      items: gridObj.grid.contextMenuModule.contextMenu.items,
...
describe('Indent/Outdent with checkbox selection', () => {
  ...
  it('ensure the indent/outdent action with checkbox selection', () => {
    gridObj.selectRow(1);
    (gridObj.grid.contextMenuModule as any).eventArgs = {
      target: gridObj.getCellFromIndex(1, 2),
    };
    const e: Object = {
      event: (gridObj.grid.contextMenuModule as any).eventArgs,
      items: gridObj.grid.contextMenuModule.contextMenu.items,
