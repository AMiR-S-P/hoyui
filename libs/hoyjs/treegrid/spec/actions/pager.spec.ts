...
import { ActionEventArgs } from '@hoyui/grids';
...
describe('Pager setModel', () => {
  ...
  it('set model testing pager', (done: Function) => {
    gridObj.actionComplete = () => {
      expect(
        gridObj.getPager().querySelectorAll('.e-pagercontainer').length
      ).toBe(1);
      done();
    };
...
describe('Pager Modes - Checking visibility', () => {
  ...
  it('Checking visibility of the inner level records', (done: Function) => {
    gridObj.collapsed = (args?: RowCollapsedEventArgs) => {
      expect(
        (<HTMLElement>gridObj.getRows()[1].querySelector('span.e-treecell'))
          .innerText === 'Phase 3'
      ).toBe(true);
      done();
    };
...
describe('Pager Modes with Filtering functionality', () => {
  ...
  it('Check data count', () => {
    ...
    expect(
      (<HTMLElement>gridObj.getRowByIndex(0).querySelector('span.e-treecell'))
        .innerText === 'Implementation Phase'
    ).toBe(true);
  });
  afterAll(() => {
    destroy(gridObj);
...
describe(' EJ2-27659 - CollaspeAll method with current page other than 1 ', () => {
  ...
  it('Check the collapseAll Method -- All Mode', (done: Function) => {
    gridObj.actionComplete = () => {
      ...
      expect(
        gridObj.getRows()[0].querySelector('.e-treecell').innerHTML ==
          'Planning'
      ).toBe(true);
      expect(
        gridObj.getRows()[1].querySelector('.e-treecell').innerHTML == 'Design'
      ...
      expect(
        gridObj.getRows()[2].querySelector('.e-treecell').innerHTML ==
...
describe('While Selecting ALL, displays only 3 records', () => {
  ...
  it('Checking the totalRecordCount', (done: Function) => {
    if (gridObj.pageSettings.pageSizes) {
      gridObj.collapseAll();
      gridObj.grid.pagerModule.pagerObj.pagerdropdownModule[
        'dropDownListObject'
      ].value = gridObj.grid.pagerModule.pagerObj.getLocalizedLabel('All');
      gridObj.expandAll();
