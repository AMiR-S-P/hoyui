...
describe('TreeGrid Reordering', () => {
  ...
  describe('Hierarchy data Reorder', () => {
    ...
    it('Reorder Testing', (done: Function) => {
      actionComplete = (): void => {
        headers =
          TreeGridObj.getHeaderContent().querySelectorAll('.e-headercell');
...
describe('Resizing in Hierarchy Data', () => {
  ...
  it('Resizing', () => {
    ...
    let columnwidth: string | number = getObject(
      'width',
      TreeGridObj.columns[0]
    );
...
describe('Flat data Reorder', () => {
  let TreeGridObj: TreeGrid;
  let headers: any;
  let colDrag: EmitType<ColumnDragEventArgs> =
    jasmine.createSpy('colDragevent');
  let coldragStart: EmitType<ColumnDragEventArgs> =
    jasmine.createSpy('colDragStart');
...
describe('Stacked data Reorder', () => {
  ...
  it('Reorder Testing', () => {
    actionComplete = (): void => {
      ...
      expect(headers[0].querySelector('.e-headercelldiv')).toBe(
        'Order Details'
      );
      expect(headers[1].querySelector('.e-headercelldiv')).toBe(
        'Shipment Details'
      );
      expect(headers[2].querySelector('.e-headercelldiv')).toBe(
        'Price Details'
      );
      ...
      expect(
        TreeGridObj.getRows()[0]
          .getElementsByClassName('e-rowcell')[3]
          .querySelector('.e-icons')
      );
    };
    TreeGridObj.actionComplete = actionComplete;
