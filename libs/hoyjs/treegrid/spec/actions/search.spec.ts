...
import { SaveEventArgs, ActionEventArgs } from '@hoyui/grids';
    ...
    actionComplete = (args?: Object): void => {
      expect(
        gridObj
          .getRows()[0]
          .getElementsByClassName('e-rowcell')[1]
          .querySelector('div>.e-treecell').innerHTML === 'Parent Task 1'
      ).toBe(true);
      expect(
        gridObj
          .getRows()[1]
          .getElementsByClassName('e-rowcell')[1]
          .querySelector('div>.e-treecell').innerHTML === 'Parent Task 2'
      ).toBe(true);
      done();
    };
    ...
    actionComplete = (args?: Object): void => {
      expect(
        gridObj
          .getRows()[0]
          .getElementsByClassName('e-rowcell')[1]
          .querySelector('div>.e-treecell').innerHTML === 'Child Task 1'
      ).toBe(true);
      expect(
        gridObj
          .getRows()[1]
...
describe('EJ2-23097: Records are not properly collapsed after filter/search is performed', () => {
  ...
  it('Searching followed by collapsing', () => {
    ...
    expect(
      (<HTMLElement>gridObj.getRowByIndex(3)).style.display === 'none'
    ).toBe(true);
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('EJ2-23098: Editing with searching ', () => {
  ...
  it('Search after editing', (done: Function) => {
    gridObj.grid.actionComplete = (args?: ActionEventArgs) => {
      expect(
        gridObj.getRows()[1].querySelector('span.e-treecell').innerHTML
      ).toBe('SP');
      done();
    };
    gridObj.search('SP');
...
describe('Hierarchy Search Mode Testing - deep child', () => {
  ...
  it('Check the search records for parent mode', (done: Function) => {
    actionComplete = (args?: Object): void => {
      ...
      expect(
        gridObj.getRows()[1].querySelector('.e-treegridexpand')
      ).not.toBeNull();
      done();
    };
    gridObj.grid.actionComplete = actionComplete;
...
describe('Hierarchy Search Mode Testing for expand icon - Child', () => {
  ...
  it('Check the search records for parent mode', (done: Function) => {
    actionComplete = (args?: Object): void => {
      expect(
        gridObj.getRows()[2].cells[1].querySelector('.e-treegridexpand')
      ).not.toBe(null);
      done();
    };
    gridObj.grid.actionComplete = actionComplete;
...
describe('EJ2-44620: Collapsing and expanding the searched records with hierarchyMode as child', () => {
  ...
  it('Check the searched records and collapse', () => {
    ...
    expect(
      gridObj.getRows()[0].getElementsByClassName('e-rowcell')[3].innerHTML ===
        '100'
    ).toBe(true);
    gridObj.collapseAll();
    gridObj.expandAll();
    expect(
      gridObj.getRowByIndex(0).getElementsByClassName('e-treegridexpand').length
    ).toBe(1);
