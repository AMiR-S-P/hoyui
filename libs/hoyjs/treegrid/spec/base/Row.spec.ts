...
describe('TreeGrid Row module', () => {
  ...
  describe('gridLines', () => {
    ...
    it('gridline setmodel testing', () => {
      ...
      gridObj.dataBind();
      expect(gridObj.grid.element.classList.contains('e-verticallines')).toBe(
        true
      );
    });
    afterAll(() => {
...
describe('rows methods', () => {
  ...
  it('methods', () => {
    ...
    expect(
      gridObj.getRowInfo(<Element | EventTarget>gridObj.element).toString()
    ).toBe({}.toString());
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('Indent for 0th level Row', () => {
  ...
  it('Testing Indent for 0th level Row that has no childrecords and 0th level record with chidrecord', () => {
    expect(gridObj.getRowByIndex(0).querySelector('.e-icons.e-none')).not.toBe(
      null
    );
    expect(gridObj.getRowByIndex(1).querySelector('.e-icons.e-none')).not.toBe(
      null
    );
    expect(
      gridObj.getRowByIndex(0).querySelectorAll('.e-icons.e-none').length
    ).toBe(2);
    expect(
      gridObj.getRowByIndex(1).querySelectorAll('.e-icons.e-none').length
    ).toBe(1);
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('height set through setmodel', () => {
  ...
  it('height set through setmodel method', () => {
    gridObj.height = 600;
    gridObj.dataBind();
    expect((gridObj.getContent().firstChild as HTMLElement).style.height).toBe(
      '600px'
    );
  });
  afterAll(() => {
...
describe('alt row without paging', () => {
  ...
  it('checking alt row without paging', () => {
    ...
    (
      rows[5].getElementsByClassName('e-treegridexpand')[0] as HTMLElement
    ).click();
