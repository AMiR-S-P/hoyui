...
import { PageEventArgs, FilterEventArgs, EditEventArgs } from '@hoyui/grids';
...
describe('HierarchySelection with filtering', () => {
  ...
  it('select the checkBoxes', (done: Function) => {
    ...
    (<HTMLElement>(
      gridObj.getRows()[6].querySelector('.e-treecheckselect')
    )).click();
    expect(
      gridObj
        .getHeaderTable()
...
describe('HierarchySelection with Editing - Row', () => {
  ...
  it('select the checkBoxes', () => {
    ...
    (<HTMLElement>(
      gridObj.getRows()[1].querySelector('.e-treecheckselect')
    )).click();
    expect(
      gridObj
        .getRows()[1]
...
describe('HierarchySelection with Adding - Row', () => {
  ...
  it('select the checkBoxes', (done: Function) => {
    ...
    (select('#' + gridObj.grid.element.id + 'taskID', formEle) as any).value =
      '122';
    (select('#' + gridObj.grid.element.id + 'taskName') as any).value =
      'second';
    (<any>gridObj.grid.toolbarModule).toolbarClickHandler({
      item: { id: gridObj.grid.element.id + '_update' },
    });
...
describe('Bug 861737: Checkbox column behavior is not working when using displayAsCheckbox column as first column', () => {
  ...
  it('Check checkbox column behavior ', () => {
    ...
    expect(
      gridObj
        .getRows()[0]
        .cells[1].querySelector('.e-frame')
        .classList.contains('e-check')
    ).toBeFalsy();
    expect(
      gridObj
        .getRows()[0]
        .cells[2].querySelector('.e-frame')
        .classList.contains('e-check')
    ).toBeTruthy();
  });
  afterAll(() => {
