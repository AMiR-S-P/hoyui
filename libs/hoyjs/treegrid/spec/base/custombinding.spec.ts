...
describe('Custom Binding', () => {
  ...
  it('Expand Testing', (done: Function) => {
    ...
    gridObj.expanded = (args: any) => {
      expect(
        gridObj.getRows()[0].getElementsByClassName('e-treegridexpand')
          .length === 1
      ).toBe(true);
      expect(
        gridObj.getRows()[1].getElementsByClassName('e-treegridexpand')
          .length === 1
      ).toBe(true);
    };
    done();
  ...
  it('Nested record expand/collapse icon test', () => {
    expect(
      gridObj.getRows()[1].getElementsByClassName('e-treegridcollapse')
        .length === 1
    ).toBe(true);
  });
      ...
      if (args.requestType === 'paging') {
        expect(
          gridObj.grid
            ...
            .querySelectorAll('.e-treecell')[0].innerHTML == 'Parent Task 2'
        ).toBe(true);
        expect(
          gridObj
            .getPager()
  ...
  it('Add Row', (done: Function) => {
    ...
    dataStateChange = (args: any) => {
      if (args.action.action == 'add') {
        ...
        gridObj.dataSource = {
          result: (gridObj.dataSource as any).result,
          count: (gridObj.dataSource as any).count + 1,
        };
      }
    };
    gridObj.actionComplete = (args: any) => {
      if (args.requestType === 'add') {
        const formEle: HTMLFormElement =
          gridObj.grid.editModule.formObj.element;
        (
          select('#' + gridObj.grid.element.id + 'TaskID', formEle) as any
        ).value = '121';
        (
  ...
  it('Edit Row', (done: Function) => {
    ...
    dataStateChange = (args: any) => {
      if (args.action.requestType == 'save') {
        gridObj.dataSource = {
          result: (gridObj.dataSource as any).result,
          count: (gridObj.dataSource as any).count,
        };
      }
    };
    gridObj.selectRow(0);
    ...
    gridObj.actionComplete = (args?: any): void => {
      if (args.requestType === 'save') {
        const cells: NodeListOf<Element> = gridObj.grid
          ...
          .querySelectorAll('.e-rowcell');
        expect(cells[0].textContent === '121').toBeTruthy();
        expect(cells[1].querySelectorAll('.e-treecell')[0].innerHTML).toBe(
          'test1'
        );
      }
      done();
...
describe('Custom Binding with loadChildOnDemand enabled', () => {
  ...
  it('Expand Testing', (done: Function) => {
    expect(
      gridObj.getRows()[0].getElementsByClassName('e-treegridexpand').length ===
        1
    ).toBe(true);
    done();
  });
...
describe('Custom Binding with loadChildOnDemand disabled', () => {
  ...
  it('Expand Testing', (done: Function) => {
    expect(
      gridObj.getRows()[0].getElementsByClassName('e-treegridcollapse')
        .length === 1
    ).toBe(true);
    done();
  });
...
describe('EJ2-61682 - Expand/collapse in observable binding', () => {
  ...
  it('Expand testing second parent record', () => {
    ...
    expect(
      gridObj.getRows()[3].getElementsByClassName('e-treegridexpand').length ===
        1
    ).toBe(true);
  });
  afterAll(() => {
  ...
  describe('EJ2-67375 - Data is hidden while expanding the childs parents', () => {
    ...
    it('Collapse and expand first parent record then expand second record - icon check', (done: Function) => {
      ...
      expanded = (args?: any) => {
        expect(
          args.row.getElementsByClassName('e-treegridexpand').length === 1
        ).toBe(true);
