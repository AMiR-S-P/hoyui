...
describe('TreeGrid base module', () => {
  ...
  describe('Hierarchy Data Basic Rendering', () => {
    ...
    it('expand testing', () => {
      ...
      (
        rows[0].getElementsByClassName('e-treegridexpand')[0] as HTMLElement
      ).click();
    ...
    it('collapse testing', () => {
      rows = gridObj.getRows();
      (
        rows[0].getElementsByClassName('e-treegridcollapse')[0] as HTMLElement
      ).click();
  ...
  it('empty dataSource Update', (done: Function) => {
    gridObj.dataBound = (args: Object) => {
      expect(
        ...
        )).rows[0].classList.contains('e-emptyrow')
      ).toBe(true);
      done();
    };
    gridObj.dataSource = [];
...
describe('Self Reference DataSource update', () => {
  ...
  it('dataSource Update', (done: Function) => {
    ...
    gridObj.dataBound = (args: PageEventArgs) => {
      rows = gridObj.getRows();
      expect(
        (rows[rows.length - 1] as HTMLTableRowElement).cells[1].textContent
      ).toBe('Test');
      done();
    };
...
describe('expandcollapse method', () => {
  ...
  it('treecolumnIndex set model testing', () => {
    ...
    rows = gridObj.getRows();
    expect(
      (rows[0] as HTMLTableRowElement).cells[2]
        .getElementsByClassName('e-treegridexpand')
        .length.toFixed(1)
    );
  });
...
describe('RTL Testing', () => {
  ...
  it('enable RTL testing', () => {
    ...
    gridObj.dataBind();
    expect(
      gridObj.element.querySelector('.e-grid').classList.contains('e-rtl')
    ).toBeTruthy();
  });
  it('presence of expand/collapse tools', () => {
    const toolbarElements: Element =
      gridObj.grid.toolbarModule.getToolbar().firstElementChild;
    expect(
      toolbarElements
...
describe('grid expand methods and properties', () => {
  ...
  it('enableCollapseAll testing', () => {
    expect(gridObj.element.querySelectorAll('.e-treegridexpand').length).toBe(
      0
    );
    ...
    gridObj.dataBind();
    expect(gridObj.element.querySelectorAll('.e-treegridcollapse').length).toBe(
      9
    );
    gridObj.enableCollapseAll = false;
  });
  it('expandstatemapping testing', (done: Function) => {
    gridObj.actionComplete = (args: Object) => {
      expect(
        gridObj.element.querySelectorAll('.e-treegridcollapse').length
      ).toBe(0);
      done();
    };
  ...
  it('collapseAtLevel testing', () => {
    ...
    expect(
      gridObj.getRows()[1].querySelectorAll('.e-treegridcollapse').length
    ).toBe(0);
    expect(
      gridObj.getRows()[12].querySelectorAll('.e-treegridcollapse').length
    ).toBe(1);
    gridObj.expandAtLevel(1);
    expect(
      gridObj.getRows()[12].querySelectorAll('.e-treegridexpand').length
    ).toBe(1);
...
describe('keyBoard Interaction for collapse particular parent row', () => {
  ...
  it('keyBoard Interaction', () => {
    ...
    expect((<HTMLTableRowElement>gridObj.getRows()[1]).style.display).toBe(
      'none'
    );
    gridObj.keyboardModule.keyAction({
      action: 'ctrlDownArrow',
      preventDefault: preventDefault,
      target: gridObj.getRows()[0].getElementsByClassName('e-rowcell')[1],
    } as any);
    expect((<HTMLTableRowElement>gridObj.getRows()[1]).style.display).toBe(
...
describe('Self Reference -multiple child levels', () => {
  ...
  it('third inner level child', () => {
    ...
    expect(
      gridObj.getRows()[4].querySelectorAll('.e-treegridexpand').length
    ).toBe(1);
    expect(gridObj.getRows()[5].querySelector('td').innerText).toBe('9');
    expect(gridObj.getRows()[6].querySelector('td').innerText).toBe('444');
    expect(
      gridObj.getRows()[9].querySelectorAll('.e-treegridexpand').length
    ).toBe(1);
    expect(
      gridObj.getRows()[10].querySelectorAll('.e-treegridexpand').length
    ).toBe(1);
  });
  afterAll(() => {
    destroy(gridObj);
    ...
    actionComplete = (args?: Object): void => {
      ...
      expect(
        isNullOrUndefined(
          gridObj
            .getRows()[0]
            .getElementsByClassName('e-rowcell')[1]
            .querySelector('div>.e-treegridexpand')
        )
      ).toBe(false);
      expect(
        gridObj
          .getRows()[0]
          .getElementsByClassName('e-rowcell')[1]
          .querySelector('div>.e-treecell').innerHTML == 'Planning'
      ).toBe(true);
      expect(
        isNullOrUndefined(
          gridObj
      ...
      expect(
        gridObj
          .getRows()[5]
          .getElementsByClassName('e-rowcell')[1]
          .querySelector('div>.e-treecell').innerHTML == 'Design'
      ).toBe(true);
      done();
    };
...
describe('columnMenu, setmodel', () => {
  ...
  it('setmodel', () => {
    gridObj.columnMenuItems = [
      { text: 'Clear Sorting', id: 'gridclearsorting' },
    ];
    expect(
      gridObj.columnMenuModule.getColumnMenu().children.length
    ).toBeGreaterThan(0);
  });
  afterAll(() => {
    destroy(gridObj);
...
describe('Checking dataSource when Children property is empty', () => {
  ...
  it('Checking dataSource when Children property is empty', () => {
    ...
    expect(gridObj.getRows()[6].classList.contains('e-treegridexpand')).toBe(
      false
    );
    expect(gridObj.getRows()[6].classList.contains('e-treegridcollapse')).toBe(
      false
    );
  });
  afterAll(() => {
...
describe('EJ2-22983: DataSource is not proper whose parentIDMapping record is not defined', () => {
  ...
  it('Rendering of unordered list', () => {
    expect(
      Object(gridObj.dataSource).length ===
        gridObj.getCurrentViewRecords().length
    ).toBe(true);
    expect(
    ...
    gridObj.collapseAtLevel(1);
    let h: number = 0;
    (<any>gridObj.element.querySelectorAll('.e-treegridcollapse')).forEach(
      (args: any) => {
  ...
  it('Collapsing testing', () => {
    ...
    (<any>gridObj.element.querySelectorAll('.e-gridrowindex2level3')).forEach(
      (args: any) => {
...
describe('EJ2-25219: uniqueIDCollection is not updated if the datasource contains level property', () => {
  ...
  it('Checking uniqueIDCollection values', () => {
    expect(
      Object.keys(getObject('uniqueIDCollection', gridObj)).length !== 0
    ).toBe(true);
  });
  afterAll(() => {
    destroy(gridObj);
  ...
  it('Checking sortSettings property with enablePersistence ', (done: Function) => {
    actionComplete = (args?: SortEventArgs): void => {
      if (args.requestType == 'sorting') {
        expect(gridObj.sortSettings.columns[0].direction == 'Ascending').toBe(
          true
        );
    ...
    dataBound = (args?: Object): void => {
      expect(
        (
          gridObj
            .getPager()
            .getElementsByClassName('e-parentmsgbar')[0] as HTMLElement
        ).innerText.search('von')
      ).toBe(2);
    };
  });
...
describe('Checking template position', () => {
    ...
    gridObj = createGrid(
      {
        ...
        columns: [
          ...
          },
          {
            headerText: 'Template',
            textAlign: 'Center',
            template: '<button id="button">Button</button>',
            width: 90,
          },
        ],
      },
  ...
  it('Checking template position when the template column is marked as treeColumnIndex ', () => {
    ...
    expect(
      (cell.getElementsByClassName('e-treecell')[0] as any).innerText ==
        'Button'
    ).toBe(true);
  });
  afterAll(() => {
...
describe('sort comparer', () => {
  ...
  it('Sort comparer check', (done: Function) => {
    actionComplete = (args?: Object): void => {
      expect(
        gridObj.getRows()[3].getElementsByClassName('e-rowcell')[2].innerHTML ==
          ''
      ).toBe(true);
      done();
    };
    ...
    actionComplete = (args?: Object): void => {
      expect(
        (
          gridObj
            .getRows()[0]
            .getElementsByClassName('e-rowcell')[2] as HTMLElement
        ).innerText == '11'
      ).toBeTruthy();
      expect(
        (
          gridObj
            .getRows()[0]
            .getElementsByClassName('e-rowcell')[3] as HTMLElement
        ).innerText == '10'
      ).toBeTruthy();
      done();
    };
...
describe('EJ2-51954-Expand/Collapse At level method', () => {
    ...
    gridObj = createGrid(
      {
        dataSource: [
          {
            ID: 'projectXYZ',
            GUID: 'A66B74DE-97B2-4A77-B6D5-7B9D5353C458',
          ...
          {
            ID: '2361848',
            GUID: '8C1B0509-B50C-4DEA-A2DC-9049F6FA0D99',
          ...
          {
            ID: '2361827',
            GUID: '677DE6EA-FACF-4B4F-BBCF-E2003B7AC98F',
          ...
          {
            ID: '2361857',
            GUID: '9F5E2D4A-60C5-40A2-8273-BF6A8A2E97B0',
  ...
  it('ExpandAtLevel testing', () => {
    ...
    (
      rows[1].getElementsByClassName('e-treegridcollapse')[0] as HTMLElement
    ).click();
    (
      rows[2].getElementsByClassName('e-treegridcollapse')[0] as HTMLElement
    ).click();
    gridObj.selectRow(3);
...
describe('EJ2-53010-Expand At level method', () => {
  ...
  it('ExpandAtLevel', () => {
    ...
    expect(
      gridObj.getRows()[0].querySelectorAll('.e-treegridexpand').length
    ).toBe(1);
  });
...
describe('EJ2-54516-expandByKey method testing', () => {
  ...
  it('Expand/Collapse By Key', (done: Function) => {
    ...
    expect(
      gridObj
        .getRows()[5]
        .querySelector('.e-treegridcollapse')
    ...
    expect(
      gridObj
        .getRows()[5]
        .querySelector('.e-treegridexpand')
    ...
    expect(
      (
        gridObj.element
          ...
          .querySelectorAll('.e-treerowcell')[0] as any
...
describe('EJ2-65701- With the frozenRows property, the methods expand & collpase do not work properly using external button', () => {
  ...
  it('expandall & collapseall action checking while enable the frozen row', () => {
    gridObj.collapseAll();
    expect(
      gridObj.getRows()[11].cells[0].classList.contains('e-lastrowcell')
    ).toBe(true);
    gridObj.expandAll();
    expect(gridObj.element.querySelectorAll('.e-treegridcollapse').length).toBe(
      0
...
describe('EJ2-58631 - Script Error thrown while calling lastRowBorder method', () => {
  ...
  beforeAll((done: Function) => {
    ...
    spyOn(window, 'fetch').and.returnValue(
      Promise.resolve(
        new Response(
          JSON.stringify({
            d: data.filter((e: { [x: string]: Object }) => {
  ...
  it('checking script error', (done: Function) => {
    ...
    expect(rows[lenValue].cells[0].classList.contains('e-lastrowcell')).toBe(
      true
    );
    done();
  });
...
describe('keyBoard Interaction for expand/collapse child row', () => {
  ...
  it('keyBoard Interaction', () => {
    ...
    expect(
      gridObj
        .getRows()[0]
        .getElementsByClassName('e-treecolumn-container')[0]
        .children[0].classList.contains('e-treegridexpand')
    ).toBe(true);
    gridObj.keyboardModule.keyAction({
      action: 'ctrlShiftDownArrow',
      preventDefault: preventDefault,
...
describe('EJ2-68334 - Column width(resizing) not persisted while using Stacked Columns in case of enablePersistence enabled', () => {
  ...
  let headers: any;
  const resizeStartevent: EmitType<ResizeArgs> =
    jasmine.createSpy('resizeStartevent');
  ...
  it('Resizing and refreshing the treegrid', () => {
    ...
    headers = (<HTMLElement>(
      TreeGridObj.getHeaderTable().querySelectorAll('th')[0]
    )).style.width;
    TreeGridObj.refresh();
    expect(headers).toBeFalsy();
    const columnwidth: string | number = getObject(
      'width',
      (TreeGridObj.columns[0] as ColumnModel).columns[1]
    );
...
describe('EJ2-69752 - Resizing not works when persistence enable while resizing in combination of both individual and Stacked column', () => {
  ...
  it('Resizing both individual and Stacked column and refreshing the treegrid', () => {
    ...
    TreeGridObj.refresh();
    expect(headers).toBeFalsy();
    const normalColumnWidth: string | number = getObject(
      'width',
      TreeGridObj.columns[0] as ColumnModel
    );
    expect(normalColumnWidth === '71px').toBe(true);
    const columnWidth: string | number = getObject(
      'width',
      (TreeGridObj.columns[1] as ColumnModel).columns[0]
    );
    expect(columnWidth === '165px').toBe(true);
...
describe('EJ2-70639 - Provide XSS- security for Tree Grid', () => {
  ...
  it('test the html sanitizer', () => {
    expect(
      (gridObj.getRowByIndex(0) as any).cells[1].innerHTML.includes(
        '<img id="target" src="x" onerror="alert(document.domain)">'
      )
    ).toBe(false);
    expect(
      (gridObj.getRowByIndex(4) as any).cells[1].innerHTML.includes(
        '<img id="target" src="x" onerror="alert(document.domain)">'
      )
    ).toBe(false);
  });
...
describe('878792 - OnClick event was not binded while creating button in treegrid with HtmlEncode enabled in Javascript Treegrid', () => {
  ...
  it('check button click present', () => {
    expect(document.getElementById('editComment').onclick.length === 1).toBe(
      true
    );
  });
  afterAll(() => {
...
describe('EJ2-71118 - Tab navigation throws script error while navigating to the next row of the collapsed items.', () => {
  ...
  it('Record and navigate over the cells through Tab', (done: Function) => {
    ...
    gridObj.grid.keyboardModule.keyAction({
      action: 'tab',
      preventDefault: preventDefault,
      target: gridObj.element.querySelector('.e-rowcell.e-focus'),
    });
    expect(gridObj.getRows()[5].cells[0].classList.contains('e-focus')).toBe(
      true
    );
    done();
  });
...
describe('Bug 839261: Column template is not working properly when using getPersistData method', () => {
  ...
  it('column template is not visible', () => {
    ...
    expect(
      gridObj.getRows()[0].cells[1].classList.contains('e-templatecell')
    ).toBe(true);
  });
  afterAll(() => {
    destroy(gridObj);
    ...
    expect(
      (
        gridObj.element
          ...
          .querySelector('.e-treecell') as any
      ).innerText == 'Button'
    ).toBe(true);
  });
...
describe('Remote data', () => {
  ...
  it('expand action with paging', (done: Function) => {
    expect(
      gridObj.getRows()[0].querySelectorAll('.e-treegridexpand').length == 1
    ).toBe(true);
    done();
  });
  afterAll(() => {
...
describe('HasChildRecords property value is not updated properly ', () => {
  ...
  it('check currentview data', () => {
    ...
    expect(
      (gridObj.getCurrentViewRecords()[1] as ITreeData).hasChildRecords
    ).toBe(false);
  });
  afterAll(() => {
    destroy(gridObj);
