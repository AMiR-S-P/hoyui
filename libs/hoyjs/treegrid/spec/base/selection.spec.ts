...
import { getObject, IIndex } from '@hoyui/grids';
import { rowSelected, CellSaveEventArgs } from '../../src';
import { Sort } from '../../src/treegrid/actions/sort';
import { Page } from '../../src/treegrid/actions/page';
import { Filter } from '../../src/treegrid/actions/filter';
import { Toolbar } from '../../src/treegrid/actions/toolbar';
import { SelectedEventArgs } from '@hoyui/inputs';
...
describe('Selection module', () => {
  ...
  describe('Row Selection', () => {
    ...
    it('Row Selection and Public Methods', () => {
      gridObj.selectRow(2);
      expect(
        gridObj.getRows()[2].getElementsByClassName('e-active').length > 0
      ).toBe(true);
    });
    afterAll(() => {
      destroy(gridObj);
...
describe('Row Selection Disable', () => {
  ...
  it('Disable the Row selection method', (done: Function) => {
    ...
    setTimeout(() => {
      expect(
        gridObj.getRows()[1].getElementsByClassName('e-active').length === 0
      ).toBe(true);
      done();
    }, 1000);
  });
...
describe('TreeGrid CheckBoxSelection1', () => {
  ...
  it('Selecting First row', (done: Function) => {
    ...
    (<HTMLElement>(
      gridObj.element.querySelectorAll('.e-row')[5].querySelector('.e-rowcell')
    )).click();
    expect(
      gridObj
        .getRows()[5]
        .children[0].querySelector('.e-checkselect')
    ...
    expect(
      gridObj
        .getRows()[5]
        .firstElementChild.classList.contains('e-selectionbackground')
    ).toBeTruthy();
  ...
  it('Selecting Second row', (done: Function) => {
    ...
    gridObj.rowSelected = rowSelected;
    (<HTMLElement>(
      gridObj.element.querySelectorAll('.e-row')[8].querySelector('.e-rowcell')
    )).click();
    expect(
      gridObj
        .getRows()[8]
...
describe('TreeGrid CheckBoxSelection2', () => {
  ...
  it('Selecting Header CheckBox', (done: Function) => {
    ...
    expect(gridObj.getSelectedRecords()[13]['taskName']).toBe(
...
describe('TreeGrid CheckBoxSelection4', () => {
  ...
  it('CheckBox Selection with Sorting', (done: Function) => {
    ...
    (<HTMLElement>(
      gridObj.element.querySelectorAll('.e-row')[9].querySelector('.e-rowcell')
    )).click();
    (<HTMLElement>(
      gridObj.element.querySelectorAll('.e-row')[8].querySelector('.e-rowcell')
    )).click();
    (<HTMLElement>(
      gridObj.element.querySelectorAll('.e-row')[6].querySelector('.e-rowcell')
    )).click();
...
describe('EJ2-62012 - selection not persisted for parent records on Expand/Collapse action', () => {
  ...
  it('Checkbox selection on Expand/Collapse', () => {
    ...
    (<HTMLElement>(
      gridObj.element.querySelectorAll('.e-row')[1].querySelector('.e-rowcell')
    )).click();
    expect(gridObj.getSelectedRowIndexes().length).toBe(2);
    gridObj.collapseRow(
      gridObj.getRows()[0],
...
describe('TreeGrid enabletoggle', () => {
  ...
  it('set model for enabletoggle', (done: Function) => {
    (
      gridObj.getRows()[0].getElementsByClassName('e-rowcell')[0] as HTMLElement
    ).click();
    (
      gridObj.getRows()[0].getElementsByClassName('e-rowcell')[0] as HTMLElement
    ).click();
    expect(
      gridObj.getRows()[0].getElementsByClassName('e-active').length > 0
    ).toBe(true);
    done();
  });
  afterAll(() => {
  ...
  it('CheckBox Selection with searching', (done: Function) => {
    actionComplete = (args?: CellSaveEventArgs): void => {
      if (args.requestType === 'searching') {
        expect((gridObj.getCurrentViewRecords()[3] as any).taskID === 13).toBe(
          true
        );
        expect(
          (gridObj.getCurrentViewRecords()[3] as any).checkboxState === 'check'
        ).toBe(true);
        gridObj.actionComplete = null;
        done();
      }
    ...
    (
      gridObj.element
        .querySelectorAll('.e-row')[12]
        .getElementsByClassName('e-frame e-icons')[0] as any
    ).click();
  ...
  it('CheckBox Selection with filtering', (done: Function) => {
    actionComplete = (args?: CellSaveEventArgs): void => {
      if (args.requestType === 'filtering') {
        ...
        expect(gridObj.selectionModule.getCheckedrecords().length == 0).toBe(
          true
        );
        done();
      }
  ...
  it('aria-checked attribute checked', () => {
    ...
    expect(
      (
        gridObj.element
          .querySelectorAll('.e-row')[0]
          .querySelectorAll('.e-treecheckselect')[0] as any
...
describe('EJ2-65673-When performing an expand or collapse operation the row is selected', () => {
  ...
  it('checking the selection records while expand the row', () => {
    ...
    (
      gridObj
        .getRows()[0]
        .getElementsByClassName('e-treegridcollapse')[0] as HTMLElement
    ).click();
    expect(gridObj.getSelectedRowIndexes()[0]).toBe(1);
  });
  afterAll(() => {
...
describe('EJ2-70486 - Checkbox selection check on expand/collapse action when checkboxOnly mode enabled ', () => {
  ...
  it('Selecting the nested row checkbox and Selection case checking', () => {
    (
      gridObj.getRows()[2].querySelector('.e-checkselect') as HTMLElement
    ).click();
    gridObj.collapseRow(gridObj.getRows()[1]);
    expect(gridObj.getRows()[2].getAttribute('aria-selected') === null).toBe(
      true
    );
  });
  afterAll(() => {
...
describe('Bug 837157: Unwanted display of tooltip in checkbox column feature', () => {
  ...
  it('Select the checkbox', (done: Function) => {
    ...
    expect(
      gridObj.getRows()[1].querySelector('.e-frame').getAttribute('title') ===
        null
    ).toBe(true);
    done();
  });
