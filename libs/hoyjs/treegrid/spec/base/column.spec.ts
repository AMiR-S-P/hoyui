...
import { select, isNullOrUndefined } from '@hoyui/base';
...
describe('TreeGrid Column Module', () => {
  ...
  describe('conditional formatting - queryCellInfo and rowdatabound', () => {
    ...
    it('querycell', () => {
      ...
      expect((rows[1] as HTMLTableRowElement).cells[2].style.color).toBe(
        'yellow'
      );
    });
    afterAll(() => {
...
describe('columns method', () => {
  ...
  it('column methods', () => {
    expect(
      gridObj.getCellFromIndex(2, 1).querySelector('.e-treecell').textContent
    ).toBe('Plan budget');
    expect(gridObj.getColumnByField('taskName').headerText).toBe('Task Name');
    const uid: string = gridObj
      .getHeaderContent()
      .querySelectorAll('.e-headercelldiv')[1]
    ...
    expect(
      gridObj.getColumnHeaderByField('startDate').querySelector('.e-headertext')
        .textContent
    ).toBe('Start Date');
    expect(
      gridObj.getColumnHeaderByIndex(2).querySelector('.e-headertext')
        .textContent
    ).toBe('Start Date');
    expect(
      gridObj.getColumnHeaderByUid(uid).querySelector('.e-headertext')
        .textContent
    ).toBe('Task Name');
    ...
    expect(gridObj.getHeaderContent().classList.contains('e-gridheader')).toBe(
      true
    );
    expect(gridObj.getHeaderTable().classList.contains('e-table')).toBe(true);
    expect(gridObj.getRowByIndex(1).children[0].textContent).toBe(
      '' + gridObj.flatData[1][gridObj.getColumns()[0].field]
    );
...
describe('Stacked Header', () => {
  ...
  it('column Checking after stacked header added dynamically', () => {
    ...
    const cols: ColumnModel = {
      ...
      columns: [{ field: 'unitPrice' }, { field: 'price' }],
    };
    ...
    gridObj = createGrid(
      {
        ...
        dataBound: function (args: Object) {
          expect(
            this.getHeaderContent().getElementsByTagName('thead')[0].childNodes
              .length
          ).toBe(2);
...
describe('Checkbox Column', () => {
  ...
  it('Intremediate state for parent checkbox by child checkbox enabled', () => {
    (<HTMLElement>(
      gridObj.getRows()[1].querySelectorAll('.e-treecheckselect')[0]
    )).click();
    expect((<ITreeData>gridObj.grid.currentViewData[1]).checkboxState).toBe(
      'check'
    );
    expect((<ITreeData>gridObj.grid.currentViewData[0]).checkboxState).toBe(
      'indeterminate'
    );
  });
  it('Header checkbox check state', () => {
    ...
    (<HTMLElement>(
      gridObj.element.querySelectorAll('.e-treeselectall')[0]
    )).click();
    expect((<ITreeData>gridObj.grid.currentViewData[0]).checkboxState).toBe(
      'check'
    );
    expect((<ITreeData>gridObj.grid.currentViewData[6]).checkboxState).toBe(
      'check'
    );
  });
...
describe('Column Chooser', () => {
  ...
  it('Column chooser testing', () => {
    ...
    (<HTMLElement>(
      select(
        '#' + gridObj.grid.element.id + '_columnchooser',
        gridObj.grid.toolbarModule.getToolbar()
      )
    )).click();
...
describe('Column Chooser', () => {
  ...
  it('Check ShowInColumnChooser method', () => {
    ...
    expect(gridObj.element.querySelectorAll('.e-cclist')[0].textContent).toBe(
      'Select All'
    );
    expect(gridObj.element.querySelectorAll('.e-cclist')[1].textContent).toBe(
      'Task ID'
    );
    expect(gridObj.element.querySelectorAll('.e-cclist')[2].textContent).toBe(
      'Priority'
    );
    expect(gridObj.element.querySelectorAll('.e-cclist')[3].textContent).toBe(
      'Duration'
    );
    expect(gridObj.element.querySelectorAll('.e-cclist')[4].textContent).toBe(
      'Progress'
    );
  });
...
describe('Checkbox Column', () => {
  ...
  it('Checkbox render', () => {
    expect(
      gridObj.getCellFromIndex(2, 0).classList.contains('e-treegridcheckbox')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(2, 1).classList.contains('e-treegridcheckbox')
    ).toBe(false);
  });
...
describe('stacked header with template tree column- EJ2-48776', () => {
  ...
  it('check template rendering', () => {
    expect(
      gridObj.getCellFromIndex(0, 0).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(1, 2).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(2, 3).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(3, 4).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(4, 6).classList.contains('e-templatecell')
    ).toBe(true);
  });
  ...
  it('Column CheckBox after collapsing with Virtualization', (done: Function) => {
    actionComplete = (args?: Object): void => {
      if (args['action'] === 'filter') {
        (<HTMLElement>(
          gridObj.getRows()[0].querySelectorAll('.e-treecheckselect')[0]
        )).click();
...
describe('Stacked Header with checkbox column - EJ2-59568', () => {
  ...
  it('Header Checkboxcolumn rendering test', () => {
    expect(document.querySelectorAll('.e-treeselectall').length == 1).toBe(
      true
    );
  });
  ...
  it('Checkbox column rendering check after paging', () => {
    actionComplete = (args?: Object): void => {
      if (args['requestType'] == 'paging') {
        expect(
          document.querySelectorAll('.e-hierarchycheckbox').length > 0
        ).toBe(true);
      }
    };
...
describe('stacked header with template tree columnindex-EJ2-62337', () => {
  ...
  it('check template rendering', () => {
    expect(
      gridObj.getCellFromIndex(0, 3).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(1, 3).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(2, 3).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(3, 3).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(4, 3).classList.contains('e-templatecell')
    ).toBe(true);
  });
...
describe('EJ2-65931- Template column with Stacked Header not working properly', () => {
  ...
  it('check template rendering', () => {
    expect(
      gridObj.getCellFromIndex(0, 2).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(1, 2).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(2, 2).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(3, 2).classList.contains('e-templatecell')
    ).toBe(true);
    expect(
      gridObj.getCellFromIndex(4, 2).classList.contains('e-templatecell')
    ).toBe(true);
  });
...
describe('853984- Resolved header check box not working properly in checkbox box column', () => {
  ...
  it('Searching after header selection ', () => {
    ...
    expect(
      (
        document.getElementsByClassName('e-hierarchycheckbox')[0]
          .childNodes[1] as any
