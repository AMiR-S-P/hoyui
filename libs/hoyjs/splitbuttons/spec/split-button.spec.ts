...
describe('Split Button', () => {
  ...
  it('Create popup on open', () => {
    button = new SplitButton(
      { items: items, createPopupOnClick: true },
      '#splitbtn'
    );
    expect(button.secondaryBtnObj.dropDown).toEqual(undefined);
    button.secondaryBtnObj.element.click();
    expect(
      button.secondaryBtnObj.dropDown.element.classList.contains('e-popup-open')
    ).toEqual(true);
  });
  ...
  it('Alt + down key checking', () => {
    ...
    expect(
      button.dropDown.element.classList.contains('e-popup-open')
    ).toBeTruthy();
  });
  ...
  it('Enter key checking', () => {
    ...
    const li: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
      button.dropDown.element.querySelectorAll('li')
    );
  ...
  it('Toggle method', () => {
    ...
    button.toggle();
    expect(
      button.secondaryBtnObj.dropDown.element.classList.contains(
        'e-popup-close'
      )
    ).toBeTruthy();
  });
  ...
  it('Angular support', () => {
    document.body.appendChild(
      createElement('EJS-SPLITBUTTON', { id: 'angsplitbtn' })
    );
    button = new SplitButton({}, '#angsplitbtn');
    expect(button.element.parentElement.tagName).toEqual('EJS-SPLITBUTTON');
    expect(button.element.parentElement.classList).toContain(
      'e-split-btn-wrapper'
    );
    expect(button.element.nextElementSibling.classList).toContain(
      'e-dropdown-btn'
    );
  });
