...
describe('DropDownButton', () => {
  ...
  describe('DOM', () => {
    ...
    it('DropDown Popup Testing', () => {
      ...
      drpButton.appendTo('#drp-button');
      expect(
        drpButton.dropDown.element.classList.contains('e-dropdown-popup')
      ).toEqual(true);
    });
    ...
    it('RTL testing', () => {
      ...
      expect(drpButton.dropDown.element.classList.contains('e-rtl')).toEqual(
        true
      );
    });
    ...
    it('Add items & Remove items testing', () => {
      ...
      element.click();
      expect(
        drpButton.dropDown.element.querySelectorAll('.e-item')[0].textContent
      ).toEqual('Cut');
      expect(
        drpButton.dropDown.element.querySelectorAll('.e-item')[1].textContent
      ).toEqual('Insert');
      drpButton.removeItems(['Cut']);
      expect(
        drpButton.dropDown.element.querySelectorAll('.e-item')[2].textContent
      ).toEqual('Copy');
    });
    ...
    it('DropdownButton popup without icon', () => {
      ...
      element.click();
      expect(
        drpButton.dropDown.element
          .querySelectorAll('li')[0]
          .classList.contains('e-blank-icon')
      ).toBeFalsy();
    });
    ...
    it('Dropdown close on mousedown', () => {
      ...
      expect(
        drpButton.dropDown.element.classList.contains('e-popup-close')
      ).toBe(true);
      // mousedown on popup
      drpButton.toggle();
      drpButton.mousedownHandler({ target: drpButton.getPopUpElement() });
      expect(
        drpButton.dropDown.element.classList.contains('e-popup-open')
      ).toBe(true);
      drpButton.destroy();
    });
    ...
    it('Enable Html Sanitizer', () => {
      drpButton = new DropDownButton({
        items: [
          { text: 'cut<style>body{background:rgb(0, 0, 255)}</style>' },
          { text: 'copy' },
          { text: 'paste' },
        ],
      });
  ...
  describe('Property', () => {
    ...
    items = [
      { text: 'Cut', iconCss: 'cut', id: 'cut' },
      { text: 'Copy', iconCss: 'copy', id: 'copy' },
      { text: 'Paste', iconCss: 'paste', url: 'http://www.google.com' },
      { separator: true },
      { text: 'Paste Text' },
      { text: 'Paste Special' },
    ];
    ...
    it('Items with icons testing and separator', () => {
      ...
      expect(
        drpButton.dropDown.element.querySelectorAll('li')[0].children[0].tagName
      ).toEqual('SPAN');
      expect(
        drpButton.dropDown.element.querySelector('.e-separator')
      ).toBeTruthy();
    });
    ...
    it('Blank icon testing', () => {
      ...
      element.click();
      expect(
        drpButton.dropDown.element.querySelector('.e-blank-icon')
      ).toBeTruthy();
    });
    ...
    it('Vertical Bottom cssClass testing', () => {
      ...
      expect(
        element.querySelector('.e-caret').classList.contains('e-icon-bottom')
      ).toEqual(true);
    });
    ...
    it('target', () => {
      ...
      drpButton.appendTo('#drp-button');
      expect(
        drpButton.getULElement().classList.contains('targetElement')
      ).toBeTruthy();
      element.click();
      expect(
        drpButton.dropDown.element.classList.contains('e-popup-open')
  ...
  describe('notify property changes of', () => {
    ...
    it('IconCss in onPropertyChanged', () => {
      ...
      drpButton.dataBind();
      expect(element.children[0].classList.contains('iconcss-top')).toEqual(
        true
      );
    });
    ...
    it('IconPosition in onPropertyChanged', () => {
      ...
      drpButton.iconPosition = 'Top';
      drpButton.dataBind();
      expect(element.children[0].classList.contains('e-icon-top')).toEqual(
        true
      );
      drpButton.content = 'icontop';
      drpButton.iconPosition = 'Left';
      drpButton.dataBind();
      expect(element.children[0].classList.contains('e-icon-left')).toEqual(
        true
      );
    });
    ...
    it('CssClass in onPropertyChanged', () => {
      ...
      expect(drpButton.dropDown.element.classList.contains('class')).toEqual(
        true
      );
      ...
      expect(
        drpButton.dropDown.element.classList.contains('styleclass')
      ).toEqual(true);
    ...
    it('target', () => {
      ...
      drpButton.dataBind();
      expect(
        drpButton.dropDown.element.children[0].classList.contains(
          'targetElement'
        )
      ).toBeTruthy();
    });
  ...
  describe('methods', () => {
    ...
    it('Custom selection method testing', () => {
      ...
      drpButton.element.click();
      expect(
        drpButton.getULElement().children[1].classList.contains('e-selected')
      ).toBeTruthy();
      drpButton.element.click();
    });
    ...
    it('focusoutHandler method', () => {
      drpButton = new DropDownButton({
        items: items,
        closeActionEvents: 'mousedown',
      });
      drpButton.appendTo('#drp-button');
      drpButton.element.click();
      const ele: HTMLElement =
        drpButton.dropDown.element.querySelector('.e-item');
      const EventArgs: any = {
        preventDefault: (): void => {
          /** NO Code */
        },
  ...
  describe('Keyboard action', () => {
    ...
    it('down arrow action', () => {
      ...
      const li: Element[] = <Element[] & NodeListOf<HTMLLIElement>>(
        drpButton.dropDown.element.querySelectorAll('li')
      );
    ...
    it('Up arrow with disabled item checking', () => {
      drpButton = new DropDownButton({
        ...
        beforeItemRender: (args: MenuEventArgs) => {
          if (
            args.item.text === 'Paste Text' ||
            args.item.text === 'Paste' ||
            args.item.text === 'Cut'
          ) {
  ...
  describe('EventArgs Checking', () => {
    ...
    it('beforeOpen Event Args Testing', () => {
      drpButton = new DropDownButton({
        ...
        beforeOpen: (args: BeforeOpenCloseMenuEventArgs) => {
          ...
          expect(args.element).toBe(
            drpButton.dropDown.element.children[0] as HTMLElement
          );
    ...
    it('beforeClose Event Args Testing', () => {
      ...
      (
        drpButton.dropDown.element.querySelectorAll('li')[0] as HTMLElement
      ).click();
      expect(element.classList.contains('e-active')).toBeTruthy();
      expect(
        drpButton.dropDown.element.classList.contains('e-popup-open')
  ...
  describe('CR issues', () => {
    ...
    it('EJ2-69116 - Dynamically changeing the dropdown button property using the setProperties', () => {
      ...
      expect(
        drpButton.dropDown.element.classList.contains('e-vertical')
      ).toBeTruthy();
    });
  });
