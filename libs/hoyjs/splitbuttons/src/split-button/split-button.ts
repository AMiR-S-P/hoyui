  ...
  implements INotifyPropertyChanged
{
  ...
  protected preRender(): void {
    ...
    if (ele.tagName === TAGNAME) {
      ...
      const btn: Element = this.createElement('button', {
        attrs: { type: 'button' },
      });
      const wrapper: HTMLElement = this.createElement(TAGNAME, {
        className: 'e-' + this.getModuleName() + '-wrapper',
      });
      for (
        let idx: number = 0, len: number = ele.attributes.length;
        idx < len;
        idx++
      ) {
        btn.setAttribute(
          ele.attributes[idx as number].nodeName,
          ele.attributes[idx as number].nodeValue
        );
      }
  ...
  private createSecondaryButton(): void {
    ...
    dropDownBtnModel.beforeOpen = (
      ...
      this.trigger(
        'beforeOpen',
        args,
        (observedArgs: BeforeOpenCloseMenuEventArgs) => {
          callBackPromise.resolve(observedArgs);
        }
      );
      return callBackPromise;
    };
    dropDownBtnModel.beforeClose = (
      args: BeforeOpenCloseMenuEventArgs
    ): Deferred | void => {
      const callBackPromise: Deferred = new Deferred();
      this.trigger(
  ...
  private setAria(): void {
    attributes(this.element, {
      'aria-expanded': 'false',
      'aria-haspopup': 'true',
      'aria-label': this.element.textContent
        ? this.element.textContent + ' splitbutton'
        : 'splitbutton',
    ...
    if (this.element) {
      ...
      if (element && element.parentElement === this.wrapper) {
        if (this.wrapper.tagName === TAGNAME) {
          ...
          removeClass(
            [this.wrapper],
            ['e-rtl', 'e-' + this.getModuleName() + '-wrapper']
          );
    ...
    switch (e.action) {
      ...
      case 'enter':
        ...
        if (
          ...
          !this.getPopUpElement().classList.contains('e-popup-close')
        ) {
...
export class Deferred {
   ...
   */
  public promise: Promise<Object> = new Promise(
