...
import { Complex, ChildProperty, SanitizeHtmlHelper } from '@hoyui/base';
import { createSpinner, showSpinner, hideSpinner } from '@hoyui/popups';
import {
  ProgressButtonModel,
  SpinSettingsModel,
  AnimationSettingsModel,
} from './progress-button-model';
...
export class ProgressButton extends Button implements INotifyPropertyChanged {
  ...
  public destroy(): void {
    ...
    const css: string[] = this.element.getAttribute('class')
      ? ['aria-label', 'aria-valuemin', 'aria-valuemax', 'aria-valuenow']
      : [
          'aria-label',
          'aria-valuemin',
          'aria-valuemax',
  ...
  private createSpinner(): void {
    const spinner: HTMLElement = this.createElement('span', {
      className: 'e-spinner',
    });
    ...
    createSpinner(
      {
        ...
        template: this.spinSettings.template as string,
      },
      this.createElement
    );
  }
  ...
  private setSpinPosition(ele: HTMLElement): void {
    ...
    if (position === 'Left' || position === 'Top') {
      this.element.insertBefore(
        ele,
        this.element.getElementsByClassName(CONTENTCLS)[0]
      );
    } else {
  ...
  private createProgress(): void {
    this.element.appendChild(
      this.createElement('span', { className: PROGRESS })
    );
  }
  ...
  private setContent(): void {
    ...
    this.element.innerHTML = '';
    this.element.appendChild(
      this.createElement('span', { className: CONTENTCLS, innerHTML: cont })
    );
  }
  ...
  private startProgress(percent?: number, progressTime?: number): void {
    ...
    this.startAnimate(
      ...
      progressTime ? Date.now() - (this.duration * 1) / 100 : Date.now(),
      percent ? percent : 0,
      0,
      this.step,
      0,
  ...
  private successCallback(
    ...
    let prevPercent: number = pPerc;
    const timeDiffBuffer: number = timDif;
    const progressTime: number = prgTim;
    let prevProgressTime: number = pPrgTim;
    const prevTime: number = pTim;
    const isVertical: boolean = this.eIsVertical;
    ...
    this.step = args.step;
    if (
      (progressTime - prevProgressTime) %
        ((this.duration * args.step) / 100) ===
        0 ||
      percent === 100
    ) {
      this.timerId = requestAnimationFrame(() => {
        if (this.enableProgress && this.getProgress()) {
          this.getProgress().style[isVertical ? 'height' : 'width'] =
            percent + '%';
        }
  ...
  private startContAnimate(): void {
    const ele: HTMLElement = this.element.getElementsByClassName(
      CONTENTCLS
    )[0] as HTMLElement;
    if (this.animationSettings.effect !== 'None') {
      new Animation({}).animate(ele, {
        duration:
          this.animationSettings.duration === 0 && animationMode === 'Enable'
            ? 400
  ...
  private setSpinnerSize(): void {
    ...
    spinner.style.height =
      Math.max(spinner.offsetHeight, ele.offsetHeight) + 'px';
  ...
  private setIconSpan(): void {
    ...
    const iconSpan: Element =
      this.element.getElementsByClassName('e-btn-icon')[0];
    if (
      cont.childNodes[0] &&
  ...
  public onPropertyChanged(
    newProp: ProgressButtonModel,
    oldProp: ProgressButtonModel
  ): void {
    const ele: HTMLButtonElement = this.element;
    let isSpinning: boolean = false;
    const clsList: DOMTokenList =
      this.element.querySelector('.e-spinner-pane').classList;
        ...
        case 'spinSettings':
          if (newProp.spinSettings.position) {
            ele.classList.remove(
              'e-spin-' + oldProp.spinSettings.position.toLowerCase()
            );
